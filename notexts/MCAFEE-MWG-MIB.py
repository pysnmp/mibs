...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsBytesFromClient=stHttpsBytesFromClient, stClientCount=stClientCount, pAMProactiveVersion=pAMProactiveVersion, pMFEDATVersion=pMFEDATVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stFtpBytesFromClient=stFtpBytesFromClient, pAMEngineVersion=pAMEngineVersion, stConnectionsLegitimate=stConnectionsLegitimate, mwgStatistics=mwgStatistics, kBuildNumber=kBuildNumber, stMalwareDetected=stMalwareDetected, stBlockedByMediaFilter=stBlockedByMediaFilter, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stCategoriesTable=stCategoriesTable, kProductVersion=kProductVersion, stConnectionsBlocked=stConnectionsBlocked, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpBytesFromServer=stHttpBytesFromServer, stCPULoad=stCPULoad, stCategoryName=stCategoryName, stHttpsTraffic=stHttpsTraffic, kCustomVersion=kCustomVersion, kMajorVersion=kMajorVersion, trSystem=trSystem, mwgTraps=mwgTraps, notifyReasonString=notifyReasonString, stFtpBytesFromServer=stFtpBytesFromServer, pMFEEngineVersion=pMFEEngineVersion, notifyReason=notifyReason, mwgEngineVersions=mwgEngineVersions, stResolveHostViaDNS=stResolveHostViaDNS, stHttpsBytesToClient=stHttpsBytesToClient, stHttpsRequests=stHttpsRequests, stFtpTraffic=stFtpTraffic, stHandleConnectToServer=stHandleConnectToServer, kCompanyName=kCompanyName, mwgMiscellaneous=mwgMiscellaneous, stMemConsumed=stMemConsumed, kMicroVersion=kMicroVersion, mwgHttps=mwgHttps, mwgContent=mwgContent, stConnectedSockets=stConnectedSockets, stBadReputation=stBadReputation, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, pAMSignatureVersion=pAMSignatureVersion, notifyOrigin=notifyOrigin, stBlockedByURLFilter=stBlockedByURLFilter, pTSDBVersion=pTSDBVersion, stMimeType=stMimeType, notifyOriginName=notifyOriginName, stTimeForTransaction=stTimeForTransaction, trUser=trUser, stCategoryCount=stCategoryCount, kMinorVersion=kMinorVersion, stHttpRequests=stHttpRequests, stHttpBytesToServer=stHttpBytesToServer, mwgFTP=mwgFTP, stHttpTraffic=stHttpTraffic, stCategories=stCategories, mwg=mwg, kProductName=kProductName, notifyAdditional=notifyAdditional, PYSNMP_MODULE_ID=mwg, notifySeverity=notifySeverity, trApplication=trApplication, kHotfixVersion=kHotfixVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCategoriesEntry=stCategoriesEntry, stHttpsBytesToServer=stHttpsBytesToServer, mwgInfo=mwgInfo, stHttpBytesFromClient=stHttpBytesFromClient, stFtpBytesToServer=stFtpBytesToServer, stFtpBytesToClient=stFtpBytesToClient, stHttpBytesToClient=stHttpBytesToClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgHttp=mwgHttp, stCPULoadRaw=stCPULoadRaw, stHttpsBytesFromServer=stHttpsBytesFromServer, notifyAffectedHost=notifyAffectedHost, kRevision=kRevision, stCPUIOWait=stCPUIOWait, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgTrapVariables=mwgTrapVariables)
