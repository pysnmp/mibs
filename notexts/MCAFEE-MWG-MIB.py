...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpBytesToClient=stFtpBytesToClient, mwgTrapVariables=mwgTrapVariables, stFtpBytesFromServer=stFtpBytesFromServer, stConnectedSockets=stConnectedSockets, PYSNMP_MODULE_ID=mwg, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgStatistics=mwgStatistics, stResolveHostViaDNS=stResolveHostViaDNS, stMalwareDetected=stMalwareDetected, stClientCount=stClientCount, mwgTraps=mwgTraps, mwg=mwg, kCustomVersion=kCustomVersion, stMimeType=stMimeType, kMicroVersion=kMicroVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, kMajorVersion=kMajorVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyOriginName=notifyOriginName, stCategories=stCategories, stBlockedByURLFilter=stBlockedByURLFilter, stHttpBytesFromServer=stHttpBytesFromServer, stMemConsumed=stMemConsumed, stHttpBytesToServer=stHttpBytesToServer, kProductName=kProductName, stHttpRequests=stHttpRequests, pMFEDATVersion=pMFEDATVersion, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesToServer=stFtpBytesToServer, stCPULoadRaw=stCPULoadRaw, mwgFTP=mwgFTP, stTimeForTransaction=stTimeForTransaction, stCategoriesTable=stCategoriesTable, stHttpTraffic=stHttpTraffic, stHttpsBytesFromClient=stHttpsBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, notifyAdditional=notifyAdditional, stHttpsTraffic=stHttpsTraffic, notifyAffectedHost=notifyAffectedHost, stHttpBytesFromClient=stHttpBytesFromClient, stCPULoad=stCPULoad, notifyReason=notifyReason, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgHttp=mwgHttp, kBuildNumber=kBuildNumber, mwgContent=mwgContent, notifyReasonString=notifyReasonString, pAMSignatureVersion=pAMSignatureVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpsBytesToClient=stHttpsBytesToClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, pAMProactiveVersion=pAMProactiveVersion, kCompanyName=kCompanyName, notifyOrigin=notifyOrigin, trUser=trUser, pMFEEngineVersion=pMFEEngineVersion, kHotfixVersion=kHotfixVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgInfo=mwgInfo, stHttpsRequests=stHttpsRequests, stFtpBytesFromClient=stFtpBytesFromClient, stCategoryCount=stCategoryCount, stCategoryName=stCategoryName, stCategoriesEntry=stCategoriesEntry, stHandleConnectToServer=stHandleConnectToServer, pTSDBVersion=pTSDBVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgHttps=mwgHttps, kRevision=kRevision, stBadReputation=stBadReputation, trSystem=trSystem, mwgMiscellaneous=mwgMiscellaneous, trApplication=trApplication, kMinorVersion=kMinorVersion, stFtpTraffic=stFtpTraffic, stHttpBytesToClient=stHttpBytesToClient, stHttpsBytesToServer=stHttpsBytesToServer, stBlockedByMediaFilter=stBlockedByMediaFilter, stCPUIOWait=stCPUIOWait, mwgEngineVersions=mwgEngineVersions, pAMEngineVersion=pAMEngineVersion, kProductVersion=kProductVersion, notifySeverity=notifySeverity)
