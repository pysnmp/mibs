...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stConnectionsLegitimate=stConnectionsLegitimate, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, PYSNMP_MODULE_ID=mwg, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpBytesToClient=stHttpBytesToClient, mwg=mwg, stBlockedByURLFilter=stBlockedByURLFilter, mwgTraps=mwgTraps, stCategories=stCategories, notifyAffectedHost=notifyAffectedHost, kRevision=kRevision, stClientCount=stClientCount, stHttpBytesToServer=stHttpBytesToServer, kMinorVersion=kMinorVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgEngineVersions=mwgEngineVersions, notifyReasonString=notifyReasonString, kProductVersion=kProductVersion, stFtpTraffic=stFtpTraffic, stMimeType=stMimeType, mwgFTP=mwgFTP, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategoriesEntry=stCategoriesEntry, stHttpRequests=stHttpRequests, stHttpBytesFromClient=stHttpBytesFromClient, stFtpBytesFromClient=stFtpBytesFromClient, stResolveHostViaDNS=stResolveHostViaDNS, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stTimeForTransaction=stTimeForTransaction, stConnectionsBlocked=stConnectionsBlocked, stCPULoad=stCPULoad, pAMEngineVersion=pAMEngineVersion, trUser=trUser, pMFEEngineVersion=pMFEEngineVersion, stCategoryCount=stCategoryCount, pAMSignatureVersion=pAMSignatureVersion, stMemConsumed=stMemConsumed, stHttpBytesFromServer=stHttpBytesFromServer, trSystem=trSystem, stBadReputation=stBadReputation, notifyReason=notifyReason, stCPULoadRaw=stCPULoadRaw, mwgTrapVariables=mwgTrapVariables, kProductName=kProductName, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpTraffic=stHttpTraffic, mwgHttp=mwgHttp, notifySeverity=notifySeverity, mwgContent=mwgContent, mwgInfo=mwgInfo, kMajorVersion=kMajorVersion, mwgHttps=mwgHttps, mwgStatistics=mwgStatistics, stHttpsRequests=stHttpsRequests, pTSDBVersion=pTSDBVersion, kBuildNumber=kBuildNumber, stHttpsBytesFromServer=stHttpsBytesFromServer, notifyOriginName=notifyOriginName, kHotfixVersion=kHotfixVersion, stCategoryName=stCategoryName, stCPUIOWait=stCPUIOWait, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCategoriesTable=stCategoriesTable, kMicroVersion=kMicroVersion, stMalwareDetected=stMalwareDetected, stFtpBytesFromServer=stFtpBytesFromServer, notifyOrigin=notifyOrigin, stHttpsBytesToClient=stHttpsBytesToClient, stHttpsTraffic=stHttpsTraffic, stHttpsBytesToServer=stHttpsBytesToServer, kCompanyName=kCompanyName, mwgMiscellaneous=mwgMiscellaneous, stHttpsBytesFromClient=stHttpsBytesFromClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpBytesToClient=stFtpBytesToClient, pAMProactiveVersion=pAMProactiveVersion, pMFEDATVersion=pMFEDATVersion, stFtpBytesToServer=stFtpBytesToServer, stHandleConnectToServer=stHandleConnectToServer, trApplication=trApplication, notifyAdditional=notifyAdditional, kCustomVersion=kCustomVersion, stConnectedSockets=stConnectedSockets)
