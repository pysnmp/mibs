...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgHttp=mwgHttp, mwgFTP=mwgFTP, notifyAffectedHost=notifyAffectedHost, stFtpBytesToServer=stFtpBytesToServer, stHttpsBytesToClient=stHttpsBytesToClient, notifyOrigin=notifyOrigin, stHttpBytesToServer=stHttpBytesToServer, trUser=trUser, stHttpsBytesFromServer=stHttpsBytesFromServer, stConnectionsLegitimate=stConnectionsLegitimate, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesFromClient=stFtpBytesFromClient, stFtpTraffic=stFtpTraffic, pMFEDATVersion=pMFEDATVersion, mwg=mwg, stCategoryCount=stCategoryCount, mwgTraps=mwgTraps, kProductVersion=kProductVersion, mwgStatistics=mwgStatistics, stBadReputation=stBadReputation, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, trApplication=trApplication, stCategoryName=stCategoryName, stMalwareDetected=stMalwareDetected, mwgEngineVersions=mwgEngineVersions, stCPULoadRaw=stCPULoadRaw, pAMEngineVersion=pAMEngineVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpsTraffic=stHttpsTraffic, stMimeType=stMimeType, kHotfixVersion=kHotfixVersion, notifyAdditional=notifyAdditional, stHttpsRequests=stHttpsRequests, stCPUIOWait=stCPUIOWait, kBuildNumber=kBuildNumber, stBlockedByURLFilter=stBlockedByURLFilter, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyReason=notifyReason, stHttpBytesToClient=stHttpBytesToClient, mwgHttps=mwgHttps, kMajorVersion=kMajorVersion, kCustomVersion=kCustomVersion, stFtpBytesFromServer=stFtpBytesFromServer, stFtpBytesToClient=stFtpBytesToClient, stCategoriesTable=stCategoriesTable, notifyReasonString=notifyReasonString, kProductName=kProductName, mwgMiscellaneous=mwgMiscellaneous, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, pAMProactiveVersion=pAMProactiveVersion, pTSDBVersion=pTSDBVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesToServer=stHttpsBytesToServer, kRevision=kRevision, notifyOriginName=notifyOriginName, stHttpBytesFromClient=stHttpBytesFromClient, mwgTrapVariables=mwgTrapVariables, stHttpTraffic=stHttpTraffic, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHandleConnectToServer=stHandleConnectToServer, kMicroVersion=kMicroVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpBytesFromServer=stHttpBytesFromServer, stBlockedByAntiMalware=stBlockedByAntiMalware, kCompanyName=kCompanyName, stHttpRequests=stHttpRequests, stClientCount=stClientCount, trSystem=trSystem, kMinorVersion=kMinorVersion, stMemConsumed=stMemConsumed, pMFEEngineVersion=pMFEEngineVersion, stTimeForTransaction=stTimeForTransaction, pAMSignatureVersion=pAMSignatureVersion, PYSNMP_MODULE_ID=mwg, mwgContent=mwgContent, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategories=stCategories, stConnectedSockets=stConnectedSockets, mwgInfo=mwgInfo, stCategoriesEntry=stCategoriesEntry, stCPULoad=stCPULoad, stResolveHostViaDNS=stResolveHostViaDNS, notifySeverity=notifySeverity)
