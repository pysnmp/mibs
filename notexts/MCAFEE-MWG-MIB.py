...
Bits, Gauge32, ModuleIdentity, Counter32, ObjectIdentity, IpAddress, Counter64, iso, NotificationType, TimeTicks, Integer32, MibIdentifier, mib_2, NotificationType, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Gauge32", "ModuleIdentity", "Counter32", "ObjectIdentity", "IpAddress", "Counter64", "iso", "NotificationType", "TimeTicks", "Integer32", "MibIdentifier", "mib-2", "NotificationType", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpsTraffic=stHttpsTraffic, stConnectedSockets=stConnectedSockets, stHttpBytesToClient=stHttpBytesToClient, stFtpTraffic=stFtpTraffic, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyReasonString=notifyReasonString, trApplication=trApplication, mwgTrapVariables=mwgTrapVariables, kMinorVersion=kMinorVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgTraps=mwgTraps, stCPUIOWait=stCPUIOWait, notifyAdditional=notifyAdditional, stTimeForTransaction=stTimeForTransaction, stBadReputation=stBadReputation, stFtpBytesToClient=stFtpBytesToClient, mwgInfo=mwgInfo, PYSNMP_MODULE_ID=mwg, stMalwareDetected=stMalwareDetected, stHttpBytesFromClient=stHttpBytesFromClient, stHttpTraffic=stHttpTraffic, kRevision=kRevision, stFtpBytesFromClient=stFtpBytesFromClient, mwgMiscellaneous=mwgMiscellaneous, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kMicroVersion=kMicroVersion, stClientCount=stClientCount, pMFEDATVersion=pMFEDATVersion, stHttpBytesFromServer=stHttpBytesFromServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stMimeType=stMimeType, notifySeverity=notifySeverity, stBlockedByURLFilter=stBlockedByURLFilter, mwgEngineVersions=mwgEngineVersions, pAMEngineVersion=pAMEngineVersion, stMemConsumed=stMemConsumed, stHandleConnectToServer=stHandleConnectToServer, stHttpsBytesToServer=stHttpsBytesToServer, kMajorVersion=kMajorVersion, stHttpBytesToServer=stHttpBytesToServer, notifyAffectedHost=notifyAffectedHost, notifyOriginName=notifyOriginName, stCategoriesTable=stCategoriesTable, stCPULoad=stCPULoad, stCategoriesEntry=stCategoriesEntry, kCustomVersion=kCustomVersion, notifyReason=notifyReason, stHttpsBytesFromServer=stHttpsBytesFromServer, mwg=mwg, kHotfixVersion=kHotfixVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pAMProactiveVersion=pAMProactiveVersion, kProductName=kProductName, stHttpsRequests=stHttpsRequests, stCategoryCount=stCategoryCount, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsBytesFromClient=stHttpsBytesFromClient, mwgContent=mwgContent, trUser=trUser, stCPULoadRaw=stCPULoadRaw, kCompanyName=kCompanyName, kProductVersion=kProductVersion, stFtpBytesToServer=stFtpBytesToServer, stResolveHostViaDNS=stResolveHostViaDNS, mwgHttps=mwgHttps, mwgFTP=mwgFTP, trSystem=trSystem, stCategories=stCategories, mwgHttp=mwgHttp, stFtpBytesFromServer=stFtpBytesFromServer, pTSDBVersion=pTSDBVersion, stHttpsBytesToClient=stHttpsBytesToClient, stCategoryName=stCategoryName, mwgStatistics=mwgStatistics, pMFEEngineVersion=pMFEEngineVersion, stHttpRequests=stHttpRequests, pAMSignatureVersion=pAMSignatureVersion, stConnectionsBlocked=stConnectionsBlocked, kBuildNumber=kBuildNumber, notifyOrigin=notifyOrigin)
