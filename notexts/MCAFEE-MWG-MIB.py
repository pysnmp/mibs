...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpBytesFromClient=stHttpBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, mwgContent=mwgContent, stHttpsBytesToServer=stHttpsBytesToServer, notifyReason=notifyReason, stConnectedSockets=stConnectedSockets, stHttpBytesFromServer=stHttpBytesFromServer, stFtpBytesFromClient=stFtpBytesFromClient, stCategoryCount=stCategoryCount, notifyAffectedHost=notifyAffectedHost, kProductVersion=kProductVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpsTraffic=stHttpsTraffic, stCPULoad=stCPULoad, notifyOrigin=notifyOrigin, stHttpsBytesFromServer=stHttpsBytesFromServer, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgStatistics=mwgStatistics, kMajorVersion=kMajorVersion, stHttpsRequests=stHttpsRequests, mwgHttps=mwgHttps, trSystem=trSystem, mwgMiscellaneous=mwgMiscellaneous, kMinorVersion=kMinorVersion, stFtpBytesToClient=stFtpBytesToClient, stHandleConnectToServer=stHandleConnectToServer, pAMSignatureVersion=pAMSignatureVersion, notifyReasonString=notifyReasonString, pAMProactiveVersion=pAMProactiveVersion, pAMEngineVersion=pAMEngineVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpBytesToServer=stHttpBytesToServer, stCPUIOWait=stCPUIOWait, stMemConsumed=stMemConsumed, stHttpsBytesFromClient=stHttpsBytesFromClient, stCategoriesEntry=stCategoriesEntry, mwgInfo=mwgInfo, stMimeType=stMimeType, stTimeForTransaction=stTimeForTransaction, kCompanyName=kCompanyName, kMicroVersion=kMicroVersion, stCategoriesTable=stCategoriesTable, stCategoryName=stCategoryName, mwgTrapVariables=mwgTrapVariables, kRevision=kRevision, stBlockedByMediaFilter=stBlockedByMediaFilter, PYSNMP_MODULE_ID=mwg, kProductName=kProductName, stBlockedByURLFilter=stBlockedByURLFilter, notifySeverity=notifySeverity, trUser=trUser, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stBadReputation=stBadReputation, stResolveHostViaDNS=stResolveHostViaDNS, stHttpTraffic=stHttpTraffic, stHttpBytesToClient=stHttpBytesToClient, stCategories=stCategories, mwgHttp=mwgHttp, pTSDBVersion=pTSDBVersion, mwgEngineVersions=mwgEngineVersions, kHotfixVersion=kHotfixVersion, mwg=mwg, stConnectionsBlocked=stConnectionsBlocked, stCPULoadRaw=stCPULoadRaw, stFtpBytesFromServer=stFtpBytesFromServer, notifyOriginName=notifyOriginName, kBuildNumber=kBuildNumber, stFtpBytesToServer=stFtpBytesToServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgTraps=mwgTraps, stHttpsBytesToClient=stHttpsBytesToClient, stMalwareDetected=stMalwareDetected, pMFEEngineVersion=pMFEEngineVersion, kCustomVersion=kCustomVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, notifyAdditional=notifyAdditional, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpRequests=stHttpRequests, trApplication=trApplication, stClientCount=stClientCount, stFtpTraffic=stFtpTraffic, mwgFTP=mwgFTP, pMFEDATVersion=pMFEDATVersion)
