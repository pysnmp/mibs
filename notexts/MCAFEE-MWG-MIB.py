...
# ASN.1 source MCAFEE-MWG-MIB
# Source digest sha256:6f6665d572bad08b78dcf93cbb84c9adf643af3fdcb4135c3140b150c50dcc88
...
if mibBuilder.loadTexts: mwg.setLastUpdated('2014-11-10 00:00')
...
stCategoriesTable = MibTable((1, 3, 6, 1, 4, 1, 1230, 2, 7, 2, 1, 10), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: stCategoriesTable.setStatus('current')
stCategoriesEntry = MibTableRow((1, 3, 6, 1, 4, 1, 1230, 2, 7, 2, 1, 10, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "MCAFEE-MWG-MIB", "stCategoryName"))
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", PYSNMP_MODULE_ID=mwg, kBuildNumber=kBuildNumber, kCompanyName=kCompanyName, kCustomVersion=kCustomVersion, kHotfixVersion=kHotfixVersion, kMajorVersion=kMajorVersion, kMicroVersion=kMicroVersion, kMinorVersion=kMinorVersion, kProductName=kProductName, kProductVersion=kProductVersion, kRevision=kRevision, mwg=mwg, mwgContent=mwgContent, mwgEngineVersions=mwgEngineVersions, mwgFTP=mwgFTP, mwgHttp=mwgHttp, mwgHttps=mwgHttps, mwgInfo=mwgInfo, mwgMiscellaneous=mwgMiscellaneous, mwgStatistics=mwgStatistics, mwgTrapVariables=mwgTrapVariables, mwgTraps=mwgTraps, notifyAdditional=notifyAdditional, notifyAffectedHost=notifyAffectedHost, notifyOrigin=notifyOrigin, notifyOriginName=notifyOriginName, notifyReason=notifyReason, notifyReasonString=notifyReasonString, notifySeverity=notifySeverity, pAMEngineVersion=pAMEngineVersion, pAMProactiveVersion=pAMProactiveVersion, pAMSignatureVersion=pAMSignatureVersion, pMFEDATVersion=pMFEDATVersion, pMFEEngineVersion=pMFEEngineVersion, pTSDBVersion=pTSDBVersion, stBadReputation=stBadReputation, stBlockedByAntiMalware=stBlockedByAntiMalware, stBlockedByMediaFilter=stBlockedByMediaFilter, stBlockedByURLFilter=stBlockedByURLFilter, stCPUIOWait=stCPUIOWait, stCPULoad=stCPULoad, stCPULoadRaw=stCPULoadRaw, stCategories=stCategories, stCategoriesEntry=stCategoriesEntry, stCategoriesTable=stCategoriesTable, stCategoryCount=stCategoryCount, stCategoryName=stCategoryName, stClientCount=stClientCount, stConnectedSockets=stConnectedSockets, stConnectionsBlocked=stConnectionsBlocked, stConnectionsLegitimate=stConnectionsLegitimate, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stFtpBytesFromClient=stFtpBytesFromClient, stFtpBytesFromServer=stFtpBytesFromServer, stFtpBytesToClient=stFtpBytesToClient, stFtpBytesToServer=stFtpBytesToServer, stFtpTraffic=stFtpTraffic, stHandleConnectToServer=stHandleConnectToServer, stHttpBytesFromClient=stHttpBytesFromClient, stHttpBytesFromServer=stHttpBytesFromServer, stHttpBytesToClient=stHttpBytesToClient, stHttpBytesToServer=stHttpBytesToServer, stHttpRequests=stHttpRequests, stHttpTraffic=stHttpTraffic, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpsBytesToClient=stHttpsBytesToClient, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsRequests=stHttpsRequests, stHttpsTraffic=stHttpsTraffic, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stMalwareDetected=stMalwareDetected, stMemConsumed=stMemConsumed, stMimeType=stMimeType, stResolveHostViaDNS=stResolveHostViaDNS, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stTimeForTransaction=stTimeForTransaction, trApplication=trApplication, trSystem=trSystem, trUser=trUser)
