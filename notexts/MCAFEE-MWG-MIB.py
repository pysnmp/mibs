...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stTimeForTransaction=stTimeForTransaction, notifyOrigin=notifyOrigin, kMinorVersion=kMinorVersion, kCompanyName=kCompanyName, stMalwareDetected=stMalwareDetected, mwg=mwg, stFtpBytesFromClient=stFtpBytesFromClient, stBlockedByMediaFilter=stBlockedByMediaFilter, kRevision=kRevision, stCPUIOWait=stCPUIOWait, notifyReason=notifyReason, stHandleConnectToServer=stHandleConnectToServer, mwgHttp=mwgHttp, stHttpsBytesToServer=stHttpsBytesToServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsRequests=stHttpsRequests, stMimeType=stMimeType, stFtpBytesFromServer=stFtpBytesFromServer, stCategoryName=stCategoryName, trUser=trUser, mwgTraps=mwgTraps, stHttpBytesToServer=stHttpBytesToServer, stHttpsTraffic=stHttpsTraffic, notifyAffectedHost=notifyAffectedHost, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifyOriginName=notifyOriginName, stHttpTraffic=stHttpTraffic, stConnectionsLegitimate=stConnectionsLegitimate, mwgEngineVersions=mwgEngineVersions, stCPULoadRaw=stCPULoadRaw, stHttpsBytesToClient=stHttpsBytesToClient, pAMProactiveVersion=pAMProactiveVersion, stCategories=stCategories, trApplication=trApplication, kProductName=kProductName, stConnectedSockets=stConnectedSockets, stCPULoad=stCPULoad, pAMEngineVersion=pAMEngineVersion, mwgFTP=mwgFTP, mwgTrapVariables=mwgTrapVariables, stClientCount=stClientCount, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsBytesFromClient=stHttpsBytesFromClient, pAMSignatureVersion=pAMSignatureVersion, PYSNMP_MODULE_ID=mwg, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kCustomVersion=kCustomVersion, pTSDBVersion=pTSDBVersion, stBlockedByURLFilter=stBlockedByURLFilter, mwgMiscellaneous=mwgMiscellaneous, stResolveHostViaDNS=stResolveHostViaDNS, kHotfixVersion=kHotfixVersion, notifyAdditional=notifyAdditional, stFtpBytesToServer=stFtpBytesToServer, stCategoriesEntry=stCategoriesEntry, mwgStatistics=mwgStatistics, stHttpBytesToClient=stHttpBytesToClient, trSystem=trSystem, stFtpBytesToClient=stFtpBytesToClient, kMicroVersion=kMicroVersion, kMajorVersion=kMajorVersion, mwgContent=mwgContent, stBadReputation=stBadReputation, stCategoryCount=stCategoryCount, stConnectionsBlocked=stConnectionsBlocked, mwgHttps=mwgHttps, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stCategoriesTable=stCategoriesTable, stHttpBytesFromClient=stHttpBytesFromClient, mwgInfo=mwgInfo, stHttpsBytesFromServer=stHttpsBytesFromServer, pMFEDATVersion=pMFEDATVersion, notifySeverity=notifySeverity, stBlockedByAntiMalware=stBlockedByAntiMalware, kProductVersion=kProductVersion, notifyReasonString=notifyReasonString, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stMemConsumed=stMemConsumed, pMFEEngineVersion=pMFEEngineVersion, stHttpBytesFromServer=stHttpBytesFromServer, stHttpRequests=stHttpRequests, kBuildNumber=kBuildNumber, stFtpTraffic=stFtpTraffic)
