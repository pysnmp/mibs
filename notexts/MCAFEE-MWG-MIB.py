...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stBlockedByURLFilter=stBlockedByURLFilter, stHttpTraffic=stHttpTraffic, kMinorVersion=kMinorVersion, mwgTrapVariables=mwgTrapVariables, pMFEEngineVersion=pMFEEngineVersion, stCPULoad=stCPULoad, mwgEngineVersions=mwgEngineVersions, kProductName=kProductName, stCategoryCount=stCategoryCount, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesFromClient=stHttpBytesFromClient, stFtpBytesToClient=stFtpBytesToClient, pAMProactiveVersion=pAMProactiveVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategoryName=stCategoryName, stHttpBytesFromServer=stHttpBytesFromServer, stHttpsBytesFromClient=stHttpsBytesFromClient, notifyReason=notifyReason, notifyOrigin=notifyOrigin, mwgInfo=mwgInfo, kBuildNumber=kBuildNumber, kHotfixVersion=kHotfixVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgContent=mwgContent, mwgTraps=mwgTraps, stCategories=stCategories, stHandleConnectToServer=stHandleConnectToServer, mwgMiscellaneous=mwgMiscellaneous, notifyReasonString=notifyReasonString, trApplication=trApplication, stHttpsBytesFromServer=stHttpsBytesFromServer, stCategoriesEntry=stCategoriesEntry, stTimeForTransaction=stTimeForTransaction, stMimeType=stMimeType, notifyAffectedHost=notifyAffectedHost, kCustomVersion=kCustomVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpTraffic=stFtpTraffic, notifyOriginName=notifyOriginName, mwgHttp=mwgHttp, notifyAdditional=notifyAdditional, stFtpBytesToServer=stFtpBytesToServer, pTSDBVersion=pTSDBVersion, trSystem=trSystem, kProductVersion=kProductVersion, stResolveHostViaDNS=stResolveHostViaDNS, pMFEDATVersion=pMFEDATVersion, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesToClient=stHttpBytesToClient, kMajorVersion=kMajorVersion, stFtpBytesFromClient=stFtpBytesFromClient, PYSNMP_MODULE_ID=mwg, kRevision=kRevision, stHttpsBytesToServer=stHttpsBytesToServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgHttps=mwgHttps, stCPUIOWait=stCPUIOWait, kCompanyName=kCompanyName, kMicroVersion=kMicroVersion, stConnectionsLegitimate=stConnectionsLegitimate, stHttpBytesToServer=stHttpBytesToServer, stConnectedSockets=stConnectedSockets, stHttpsRequests=stHttpsRequests, stBadReputation=stBadReputation, stConnectionsBlocked=stConnectionsBlocked, pAMEngineVersion=pAMEngineVersion, stHttpRequests=stHttpRequests, stMemConsumed=stMemConsumed, mwgFTP=mwgFTP, notifySeverity=notifySeverity, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, trUser=trUser, mwgStatistics=mwgStatistics, stMalwareDetected=stMalwareDetected, stHttpsTraffic=stHttpsTraffic, stClientCount=stClientCount, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwg=mwg, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFtpBytesFromServer=stFtpBytesFromServer, stCPULoadRaw=stCPULoadRaw, stCategoriesTable=stCategoriesTable)
