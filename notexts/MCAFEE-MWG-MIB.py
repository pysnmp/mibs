...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", trApplication=trApplication, stFtpBytesToClient=stFtpBytesToClient, mwg=mwg, stHttpBytesFromClient=stHttpBytesFromClient, kRevision=kRevision, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyAdditional=notifyAdditional, stHttpBytesToServer=stHttpBytesToServer, stConnectionsBlocked=stConnectionsBlocked, stCategoriesTable=stCategoriesTable, notifyAffectedHost=notifyAffectedHost, stTimeForTransaction=stTimeForTransaction, stConnectedSockets=stConnectedSockets, stCPULoadRaw=stCPULoadRaw, stFtpBytesFromClient=stFtpBytesFromClient, stHttpsBytesToServer=stHttpsBytesToServer, pAMSignatureVersion=pAMSignatureVersion, stFtpBytesToServer=stFtpBytesToServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stMalwareDetected=stMalwareDetected, kMinorVersion=kMinorVersion, kCompanyName=kCompanyName, kMicroVersion=kMicroVersion, kHotfixVersion=kHotfixVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFtpTraffic=stFtpTraffic, stClientCount=stClientCount, kBuildNumber=kBuildNumber, pMFEDATVersion=pMFEDATVersion, mwgHttps=mwgHttps, mwgStatistics=mwgStatistics, stCategories=stCategories, mwgTrapVariables=mwgTrapVariables, stCategoryName=stCategoryName, stHttpsBytesFromClient=stHttpsBytesFromClient, pMFEEngineVersion=pMFEEngineVersion, mwgContent=mwgContent, stHttpBytesToClient=stHttpBytesToClient, kMajorVersion=kMajorVersion, trSystem=trSystem, notifyOrigin=notifyOrigin, stHttpsTraffic=stHttpsTraffic, stHttpTraffic=stHttpTraffic, stResolveHostViaDNS=stResolveHostViaDNS, stFtpBytesFromServer=stFtpBytesFromServer, kProductVersion=kProductVersion, pAMEngineVersion=pAMEngineVersion, stBlockedByURLFilter=stBlockedByURLFilter, stCategoryCount=stCategoryCount, pTSDBVersion=pTSDBVersion, stHttpRequests=stHttpRequests, kCustomVersion=kCustomVersion, notifyOriginName=notifyOriginName, stCPUIOWait=stCPUIOWait, stBadReputation=stBadReputation, stHttpBytesFromServer=stHttpBytesFromServer, mwgMiscellaneous=mwgMiscellaneous, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsBytesFromServer=stHttpsBytesFromServer, stMemConsumed=stMemConsumed, mwgFTP=mwgFTP, pAMProactiveVersion=pAMProactiveVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifySeverity=notifySeverity, notifyReason=notifyReason, trUser=trUser, stConnectionsLegitimate=stConnectionsLegitimate, stMimeType=stMimeType, stCategoriesEntry=stCategoriesEntry, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHandleConnectToServer=stHandleConnectToServer, stHttpsBytesToClient=stHttpsBytesToClient, mwgHttp=mwgHttp, mwgInfo=mwgInfo, kProductName=kProductName, stHttpsRequests=stHttpsRequests, stCPULoad=stCPULoad, mwgEngineVersions=mwgEngineVersions, mwgTraps=mwgTraps, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifyReasonString=notifyReasonString, PYSNMP_MODULE_ID=mwg)
