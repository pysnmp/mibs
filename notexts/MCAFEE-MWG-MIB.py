...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kBuildNumber=kBuildNumber, stMemConsumed=stMemConsumed, kCompanyName=kCompanyName, stTimeForTransaction=stTimeForTransaction, stCategoryCount=stCategoryCount, kMicroVersion=kMicroVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stHandleConnectToServer=stHandleConnectToServer, kMinorVersion=kMinorVersion, notifyOriginName=notifyOriginName, mwgFTP=mwgFTP, pAMProactiveVersion=pAMProactiveVersion, stCategoriesEntry=stCategoriesEntry, PYSNMP_MODULE_ID=mwg, kCustomVersion=kCustomVersion, trUser=trUser, stBlockedByURLFilter=stBlockedByURLFilter, stBlockedByMediaFilter=stBlockedByMediaFilter, kRevision=kRevision, stCategories=stCategories, mwgHttps=mwgHttps, notifySeverity=notifySeverity, stHttpsBytesToServer=stHttpsBytesToServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifyAffectedHost=notifyAffectedHost, mwgMiscellaneous=mwgMiscellaneous, stResolveHostViaDNS=stResolveHostViaDNS, stHttpBytesToServer=stHttpBytesToServer, stFtpTraffic=stFtpTraffic, mwg=mwg, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesToClient=stHttpBytesToClient, stHttpsRequests=stHttpsRequests, stCPUIOWait=stCPUIOWait, stBadReputation=stBadReputation, stClientCount=stClientCount, stCategoryName=stCategoryName, mwgTrapVariables=mwgTrapVariables, kProductName=kProductName, stHttpsBytesFromServer=stHttpsBytesFromServer, stCPULoad=stCPULoad, notifyReason=notifyReason, kMajorVersion=kMajorVersion, stConnectedSockets=stConnectedSockets, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, trSystem=trSystem, stHttpBytesFromServer=stHttpBytesFromServer, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stFtpBytesToClient=stFtpBytesToClient, pMFEDATVersion=pMFEDATVersion, pMFEEngineVersion=pMFEEngineVersion, stHttpRequests=stHttpRequests, kHotfixVersion=kHotfixVersion, notifyOrigin=notifyOrigin, stMalwareDetected=stMalwareDetected, mwgEngineVersions=mwgEngineVersions, pTSDBVersion=pTSDBVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectionsBlocked=stConnectionsBlocked, notifyAdditional=notifyAdditional, stHttpBytesFromClient=stHttpBytesFromClient, stHttpsTraffic=stHttpsTraffic, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stMimeType=stMimeType, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgStatistics=mwgStatistics, stHttpTraffic=stHttpTraffic, stConnectionsLegitimate=stConnectionsLegitimate, stCategoriesTable=stCategoriesTable, trApplication=trApplication, stHttpsBytesToClient=stHttpsBytesToClient, mwgContent=mwgContent, notifyReasonString=notifyReasonString, pAMEngineVersion=pAMEngineVersion, stFtpBytesToServer=stFtpBytesToServer, stFtpBytesFromServer=stFtpBytesFromServer, kProductVersion=kProductVersion, stFtpBytesFromClient=stFtpBytesFromClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgTraps=mwgTraps, stCPULoadRaw=stCPULoadRaw, mwgHttp=mwgHttp, mwgInfo=mwgInfo)
