...
MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Bits, NotificationType, MibIdentifier, IpAddress, Gauge32, Unsigned32, TimeTicks, ObjectIdentity, Counter64, ModuleIdentity, Integer32, Opaque, enterprises, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Bits", "NotificationType", "MibIdentifier", "IpAddress", "Gauge32", "Unsigned32", "TimeTicks", "ObjectIdentity", "Counter64", "ModuleIdentity", "Integer32", "Opaque", "enterprises", "Counter32")
...
mibBuilder.exportSymbols("UCD-SNMP-MIB", ssIORawReceived=ssIORawReceived, memTotalReal=memTotalReal, ssCpuIdle=ssCpuIdle, ssCpuRawWait=ssCpuRawWait, ssCpuRawInterrupt=ssCpuRawInterrupt, fileSize=fileSize, memAvailRealX=memAvailRealX, dskPercent=dskPercent, logMatchCycle=logMatchCycle, laLoad=laLoad, versionClearCache=versionClearCache, memTotalSwap=memTotalSwap, memTotalSwapX=memTotalSwapX, mrModuleName=mrModuleName, fileName=fileName, dskErrorFlag=dskErrorFlag, prErrFix=prErrFix, logMatchEntry=logMatchEntry, snmperrErrMessage=snmperrErrMessage, snmperrNames=snmperrNames, fileErrorFlag=fileErrorFlag, ssCpuRawSystem=ssCpuRawSystem, ssRawContexts=ssRawContexts, hpux9=hpux9, logMatchRegEx=logMatchRegEx, dskDevice=dskDevice, ssCpuRawGuest=ssCpuRawGuest, ssRawInterrupts=ssRawInterrupts, hpux11=hpux11, laErrorFlag=laErrorFlag, ultrix=ultrix, versionDate=versionDate, dskAvail=dskAvail, irix=irix, dskUsed=dskUsed, win32=win32, ssCpuRawIdle=ssCpuRawIdle, memAvailSwap=memAvailSwap, fileTable=fileTable, dskErrorMsg=dskErrorMsg, versionConfigureOptions=versionConfigureOptions, memUsedSwapTXT=memUsedSwapTXT, mrIndex=mrIndex, ssCpuRawSteal=ssCpuRawSteal, ucdInternal=ucdInternal, fileErrorMsg=fileErrorMsg, UCDErrorFix=UCDErrorFix, extResult=extResult, memAvailSwapTXT=memAvailSwapTXT, prIndex=prIndex, ssErrorName=ssErrorName, ssCpuSystem=ssCpuSystem, ssSwapIn=ssSwapIn, versionUpdateConfig=versionUpdateConfig, laTable=laTable, dskEntry=dskEntry, logMatchIndex=logMatchIndex, memCachedX=memCachedX, logMatchErrorFlag=logMatchErrorFlag, dskTable=dskTable, dskIndex=dskIndex, prEntry=prEntry, ssCpuRawGuestNice=ssCpuRawGuestNice, logMatchName=logMatchName, versionCDate=versionCDate, ucdStart=ucdStart, UCDErrorFlag=UCDErrorFlag, logMatchCurrentCounter=logMatchCurrentCounter, dskMinimum=dskMinimum, extNames=extNames, dskPath=dskPath, logMatchCurrentCount=logMatchCurrentCount, linux=linux, ssIOSent=ssIOSent, logMatchTable=logMatchTable, versionDoDebugging=versionDoDebugging, ssCpuRawNice=ssCpuRawNice, logMatchCounter=logMatchCounter, logMatchGlobalCounter=logMatchGlobalCounter, dskAvailHigh=dskAvailHigh, versionSavePersistentData=versionSavePersistentData, memAvailRealTXT=memAvailRealTXT, dragonfly=dragonfly, ucdavis=ucdavis, memAvailReal=memAvailReal, hpux10=hpux10, versionRestartAgent=versionRestartAgent, openbsd=openbsd, laNames=laNames, netbsd1=netbsd1, ucdExperimental=ucdExperimental, ssCpuUser=ssCpuUser, fileMax=fileMax, dskMinPercent=dskMinPercent, versionIndex=versionIndex, ssIOReceive=ssIOReceive, logMatchRegExCompilation=logMatchRegExCompilation, extEntry=extEntry, prTable=prTable, snmperrIndex=snmperrIndex, memTotalRealTXT=memTotalRealTXT, extOutput=extOutput, memErrorName=memErrorName, memUsedRealTXT=memUsedRealTXT, memShared=memShared, ssRawSwapIn=ssRawSwapIn, memBuffer=memBuffer, memMinimumSwapX=memMinimumSwapX, laIndex=laIndex, dskAvailLow=dskAvailLow, ssSysInterrupts=ssSysInterrupts, prCount=prCount, dskTotal=dskTotal, ucdShutdown=ucdShutdown, extCommand=extCommand, ssCpuRawKernel=ssCpuRawKernel, snmperrErrorFlag=snmperrErrorFlag, memBufferX=memBufferX, prErrMessage=prErrMessage, freebsd=freebsd, logMatchFilename=logMatchFilename, laErrMessage=laErrMessage, laLoadInt=laLoadInt, extIndex=extIndex, versionTag=versionTag, fileIndex=fileIndex, logMatchGlobalCount=logMatchGlobalCount, sunos4=sunos4, memTotalFree=memTotalFree, laLoadFloat=laLoadFloat, memCached=memCached, ssCpuRawSoftIRQ=ssCpuRawSoftIRQ, logMatchMaxEntries=logMatchMaxEntries, dskTotalHigh=dskTotalHigh, prNames=prNames, ssSwapOut=ssSwapOut, laEntry=laEntry, extErrFixCmd=extErrFixCmd, versionIdent=versionIdent, Float=Float, memIndex=memIndex, ssRawSwapOut=ssRawSwapOut, bsdi=bsdi, memMinimumSwap=memMinimumSwap, ssCpuRawUser=ssCpuRawUser, prMax=prMax, prMin=prMin, memTotalRealX=memTotalRealX, mrEntry=mrEntry, memAvailSwapX=memAvailSwapX, ssIORawSent=ssIORawSent, memSwapError=memSwapError, PYSNMP_MODULE_ID=ucdavis, logMatchCount=logMatchCount, memSharedX=memSharedX, unknown=unknown, ssCpuNumCpus=ssCpuNumCpus, osf=osf, aix=aix, snmperrs=snmperrs, systemStats=systemStats, extTable=extTable, version=version, memTotalFreeX=memTotalFreeX, fileEntry=fileEntry, prErrorFlag=prErrorFlag, logMatch=logMatch, dskPercentNode=dskPercentNode, ssSysContext=ssSysContext, ucdSnmpAgent=ucdSnmpAgent, ucdTraps=ucdTraps, ssIndex=ssIndex, dskUsedHigh=dskUsedHigh, mrTable=mrTable, dskUsedLow=dskUsedLow, extErrFix=extErrFix, memSwapErrorMsg=memSwapErrorMsg, prErrFixCmd=prErrFixCmd, memory=memory, memTotalSwapTXT=memTotalSwapTXT, dskTotalLow=dskTotalLow, laConfig=laConfig, macosx=macosx, solaris=solaris)
