...
mibBuilder.exportSymbols("POSEIDON-MIB", outType=outType, sensUnit=sensUnit, tsAlarmTime=tsAlarmTime, inpEntry=inpEntry, tsAlarmsPresent=tsAlarmsPresent, sensLimitMin=sensLimitMin, outTable=outTable, charonII=charonII, TimeStamp=TimeStamp, sensName=sensName, sensFlags=sensFlags, sensHysteresis=sensHysteresis, inpAlarmSetup=inpAlarmSetup, OutputMode=OutputMode, SensorUnitString=SensorUnitString, SensorValue=SensorValue, outMode=outMode, sensValue=sensValue, sensSetup=sensSetup, sensSetupIndex=sensSetupIndex, outEntry=outEntry, sensString=sensString, sensIndex=sensIndex, tsTrapAlarmStart=tsTrapAlarmStart, SensorName=SensorName, sensState=sensState, sensValueRaw=sensValueRaw, sensUnitString=sensUnitString, tsAlarmIdx=tsAlarmIdx, inpIndex=inpIndex, tsAlarmSensName=tsAlarmSensName, setup=setup, tsAlarmTable=tsAlarmTable, sensAlarmStateChanged=sensAlarmStateChanged, OutputType=OutputType, inpTable=inpTable, sensSetupName=sensSetupName, outIndex=outIndex, PositiveInteger=PositiveInteger, hwgroup=hwgroup, inpValue=inpValue, inpName=inpName, sensTable=sensTable, sensSetupEntry=sensSetupEntry, sensEntry=sensEntry, infoAddressMAC=infoAddressMAC, SensorString=SensorString, inpAlarmStateChanged=inpAlarmStateChanged, poseidon=poseidon, tsTrapAlarmEnd=tsTrapAlarmEnd, IOName=IOName, SensorFlags=SensorFlags, tsAlarmEntry=tsAlarmEntry, tsAlarmId=tsAlarmId, info=info, OnOff=OnOff, tsAlarm=tsAlarm, unitType=unitType, sensLimitMax=sensLimitMax, SensorState=SensorState, UnitType=UnitType, outValue=outValue, sensSetupTable=sensSetupTable, SensorID=SensorID, inpAlarmState=inpAlarmState, outName=outName, sensID=sensID, InputAlarmSetup=InputAlarmSetup, InputAlarmState=InputAlarmState, tsAlarmDescr=tsAlarmDescr)
