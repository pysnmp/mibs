...
mibBuilder.exportSymbols("POSEIDON-MIB", sensState=sensState, unitType=unitType, sensHysteresis=sensHysteresis, inpEntry=inpEntry, SensorName=SensorName, inpTable=inpTable, inpAlarmSetup=inpAlarmSetup, sensTable=sensTable, sensIndex=sensIndex, sensValue=sensValue, sensSetupTable=sensSetupTable, sensFlags=sensFlags, info=info, sensID=sensID, tsAlarmsPresent=tsAlarmsPresent, sensUnit=sensUnit, sensEntry=sensEntry, sensSetupEntry=sensSetupEntry, sensLimitMax=sensLimitMax, outMode=outMode, TimeStamp=TimeStamp, SensorFlags=SensorFlags, infoAddressMAC=infoAddressMAC, tsAlarm=tsAlarm, inpAlarmState=inpAlarmState, inpIndex=inpIndex, IOName=IOName, sensSetupIndex=sensSetupIndex, SensorValue=SensorValue, hwgroup=hwgroup, SensorState=SensorState, SensorID=SensorID, setup=setup, outType=outType, tsAlarmDescr=tsAlarmDescr, charonII=charonII, sensAlarmStateChanged=sensAlarmStateChanged, sensName=sensName, outIndex=outIndex, OutputMode=OutputMode, outEntry=outEntry, inpName=inpName, tsTrapAlarmStart=tsTrapAlarmStart, sensUnitString=sensUnitString, poseidon=poseidon, outName=outName, OutputType=OutputType, inpAlarmStateChanged=inpAlarmStateChanged, inpValue=inpValue, tsAlarmIdx=tsAlarmIdx, PositiveInteger=PositiveInteger, tsAlarmSensName=tsAlarmSensName, sensValueRaw=sensValueRaw, tsAlarmEntry=tsAlarmEntry, InputAlarmState=InputAlarmState, sensSetup=sensSetup, UnitType=UnitType, sensLimitMin=sensLimitMin, OnOff=OnOff, InputAlarmSetup=InputAlarmSetup, tsTrapAlarmEnd=tsTrapAlarmEnd, outTable=outTable, tsAlarmId=tsAlarmId, sensString=sensString, outValue=outValue, SensorUnitString=SensorUnitString, sensSetupName=sensSetupName, tsAlarmTime=tsAlarmTime, tsAlarmTable=tsAlarmTable, SensorString=SensorString)
