...
mibBuilder.exportSymbols("POSEIDON-MIB", outName=outName, outIndex=outIndex, sensSetupIndex=sensSetupIndex, tsAlarmTable=tsAlarmTable, OutputType=OutputType, sensEntry=sensEntry, inpName=inpName, OutputMode=OutputMode, inpTable=inpTable, sensHysteresis=sensHysteresis, sensAlarmStateChanged=sensAlarmStateChanged, hwgroup=hwgroup, outEntry=outEntry, tsTrapAlarmStart=tsTrapAlarmStart, inpEntry=inpEntry, sensUnitString=sensUnitString, outMode=outMode, unitType=unitType, infoAddressMAC=infoAddressMAC, SensorID=SensorID, sensSetupTable=sensSetupTable, inpAlarmStateChanged=inpAlarmStateChanged, sensName=sensName, poseidon=poseidon, SensorUnitString=SensorUnitString, sensState=sensState, charonII=charonII, sensID=sensID, outType=outType, tsAlarmEntry=tsAlarmEntry, sensValueRaw=sensValueRaw, outTable=outTable, tsAlarmIdx=tsAlarmIdx, sensIndex=sensIndex, tsAlarmSensName=tsAlarmSensName, SensorState=SensorState, tsAlarmsPresent=tsAlarmsPresent, sensTable=sensTable, inpAlarmSetup=inpAlarmSetup, tsAlarmDescr=tsAlarmDescr, UnitType=UnitType, sensSetupEntry=sensSetupEntry, info=info, SensorString=SensorString, TimeStamp=TimeStamp, sensUnit=sensUnit, sensSetup=sensSetup, sensFlags=sensFlags, IOName=IOName, tsAlarm=tsAlarm, SensorName=SensorName, inpValue=inpValue, PositiveInteger=PositiveInteger, setup=setup, SensorValue=SensorValue, sensString=sensString, tsTrapAlarmEnd=tsTrapAlarmEnd, inpIndex=inpIndex, InputAlarmSetup=InputAlarmSetup, tsAlarmId=tsAlarmId, sensSetupName=sensSetupName, sensLimitMin=sensLimitMin, sensValue=sensValue, inpAlarmState=inpAlarmState, tsAlarmTime=tsAlarmTime, OnOff=OnOff, sensLimitMax=sensLimitMax, SensorFlags=SensorFlags, outValue=outValue, InputAlarmState=InputAlarmState)
