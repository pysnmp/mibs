...
mibBuilder.exportSymbols("POSEIDON-MIB", sensLimitMin=sensLimitMin, sensEntry=sensEntry, tsTrapAlarmEnd=tsTrapAlarmEnd, SensorValue=SensorValue, sensID=sensID, inpValue=inpValue, tsTrapAlarmStart=tsTrapAlarmStart, SensorString=SensorString, outIndex=outIndex, tsAlarm=tsAlarm, SensorFlags=SensorFlags, info=info, sensSetupName=sensSetupName, sensValue=sensValue, sensValueRaw=sensValueRaw, unitType=unitType, sensTable=sensTable, sensSetupIndex=sensSetupIndex, sensIndex=sensIndex, tsAlarmIdx=tsAlarmIdx, tsAlarmTime=tsAlarmTime, inpAlarmStateChanged=inpAlarmStateChanged, tsAlarmEntry=tsAlarmEntry, OutputType=OutputType, tsAlarmTable=tsAlarmTable, outName=outName, sensSetup=sensSetup, sensSetupTable=sensSetupTable, sensLimitMax=sensLimitMax, setup=setup, TimeStamp=TimeStamp, hwgroup=hwgroup, charonII=charonII, OnOff=OnOff, SensorName=SensorName, outTable=outTable, sensFlags=sensFlags, UnitType=UnitType, tsAlarmSensName=tsAlarmSensName, sensName=sensName, tsAlarmDescr=tsAlarmDescr, PositiveInteger=PositiveInteger, sensUnitString=sensUnitString, sensSetupEntry=sensSetupEntry, SensorState=SensorState, inpName=inpName, SensorUnitString=SensorUnitString, InputAlarmState=InputAlarmState, IOName=IOName, sensHysteresis=sensHysteresis, outEntry=outEntry, sensString=sensString, inpAlarmState=inpAlarmState, SensorID=SensorID, inpIndex=inpIndex, outType=outType, infoAddressMAC=infoAddressMAC, InputAlarmSetup=InputAlarmSetup, OutputMode=OutputMode, sensUnit=sensUnit, sensAlarmStateChanged=sensAlarmStateChanged, poseidon=poseidon, tsAlarmId=tsAlarmId, outValue=outValue, inpAlarmSetup=inpAlarmSetup, inpTable=inpTable, sensState=sensState, outMode=outMode, inpEntry=inpEntry, tsAlarmsPresent=tsAlarmsPresent)
