...
# ASN.1 source FROGFOOT-RESOURCES-MIB
# Source digest sha256:95e300154ddc2fae9686a91df2269a2e27afcf011c92ecd00cac017c507a7f5a
...
if mibBuilder.loadTexts: resources.setLastUpdated('2004-07-17 00:00')
...
diskTable = MibTable((1, 3, 6, 1, 4, 1, 10002, 1, 1, 1, 3, 2), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: diskTable.setStatus('current')
diskEntry = MibTableRow((1, 3, 6, 1, 4, 1, 10002, 1, 1, 1, 3, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "FROGFOOT-RESOURCES-MIB", "diskIndex"))
if mibBuilder.loadTexts: diskEntry.setStatus('current')
diskIndex = MibTableColumn((1, 3, 6, 1, 4, 1, 10002, 1, 1, 1, 3, 2, 1, 1), TableIndex()).setMaxAccess("notaccessible")
...
loadTable = MibTable((1, 3, 6, 1, 4, 1, 10002, 1, 1, 1, 4, 2), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: loadTable.setStatus('current')
loadEntry = MibTableRow((1, 3, 6, 1, 4, 1, 10002, 1, 1, 1, 4, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "FROGFOOT-RESOURCES-MIB", "loadIndex"))
if mibBuilder.loadTexts: loadEntry.setStatus('current')
loadIndex = MibTableColumn((1, 3, 6, 1, 4, 1, 10002, 1, 1, 1, 4, 2, 1, 1), TableIndex()).setMaxAccess("notaccessible")
...
mibBuilder.exportSymbols("FROGFOOT-RESOURCES-MIB", PYSNMP_MODULE_ID=resources, TableIndex=TableIndex, diskDev=diskDev, diskDir=diskDir, diskEntry=diskEntry, diskFSType=diskFSType, diskFree=diskFree, diskIndex=diskIndex, diskNumber=diskNumber, diskTable=diskTable, diskTotal=diskTotal, frogfoot=frogfoot, load=load, loadDescr=loadDescr, loadEntry=loadEntry, loadIndex=loadIndex, loadNumber=loadNumber, loadTable=loadTable, loadValue=loadValue, memBuffer=memBuffer, memCache=memCache, memFree=memFree, memTotal=memTotal, memory=memory, resCompliance=resCompliance, resCompliances=resCompliances, resConformance=resConformance, resDiskGroup=resDiskGroup, resGroups=resGroups, resLoadGroup=resLoadGroup, resMIB=resMIB, resMIBObjects=resMIBObjects, resMemGroup=resMemGroup, resSwapGroup=resSwapGroup, resources=resources, servers=servers, storage=storage, swap=swap, swapFree=swapFree, swapTotal=swapTotal, system=system)
