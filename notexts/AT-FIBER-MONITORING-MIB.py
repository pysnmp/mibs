...
# ASN.1 source AT-FIBER-MONITORING-MIB
# Source digest sha256:c343046dfa7748b4211c1d7c5e67fdd80f63237f04f18e4179b117a1a657958c
...
if mibBuilder.loadTexts: atFiberMon.setLastUpdated('2015-09-08 00:00')
...
atFiberMonConfigTable = MibTable((1, 3, 6, 1, 4, 1, 207, 8, 4, 4, 3, 27, 2), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: atFiberMonConfigTable.setStatus('current')
atFiberMonConfigEntry = MibTableRow((1, 3, 6, 1, 4, 1, 207, 8, 4, 4, 3, 27, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AT-FIBER-MONITORING-MIB", "atFiberMonIfname"))
...
atFiberMonStateTable = MibTable((1, 3, 6, 1, 4, 1, 207, 8, 4, 4, 3, 27, 3), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: atFiberMonStateTable.setStatus('current')
atFiberMonStateEntry = MibTableRow((1, 3, 6, 1, 4, 1, 207, 8, 4, 4, 3, 27, 3, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AT-FIBER-MONITORING-MIB", "atFiberMonIfindex"), (0, "AT-FIBER-MONITORING-MIB", "atFiberMonChannel"))
...
mibBuilder.exportSymbols("AT-FIBER-MONITORING-MIB", PYSNMP_MODULE_ID=atFiberMon, atFiberMon=atFiberMon, atFiberMonActualBaseline=atFiberMonActualBaseline, atFiberMonAlarmAction=atFiberMonAlarmAction, atFiberMonAlarmClearedNotify=atFiberMonAlarmClearedNotify, atFiberMonAlarmSetNotify=atFiberMonAlarmSetNotify, atFiberMonBaseline=atFiberMonBaseline, atFiberMonChannel=atFiberMonChannel, atFiberMonConfigEntry=atFiberMonConfigEntry, atFiberMonConfigTable=atFiberMonConfigTable, atFiberMonEnable=atFiberMonEnable, atFiberMonIfindex=atFiberMonIfindex, atFiberMonIfname=atFiberMonIfname, atFiberMonInterval=atFiberMonInterval, atFiberMonMaxReading=atFiberMonMaxReading, atFiberMonMinReading=atFiberMonMinReading, atFiberMonNotifications=atFiberMonNotifications, atFiberMonReading=atFiberMonReading, atFiberMonReadingHistory=atFiberMonReadingHistory, atFiberMonSensitivity=atFiberMonSensitivity, atFiberMonStateEntry=atFiberMonStateEntry, atFiberMonStateTable=atFiberMonStateTable, atFiberMonThreshold=atFiberMonThreshold, atFiberMonTrapVariable=atFiberMonTrapVariable, atFiberMonlIfDescription=atFiberMonlIfDescription)
