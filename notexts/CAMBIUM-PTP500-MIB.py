...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", sMTPServerPortNumber=sMTPServerPortNumber, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, telecomsCableLength=telecomsCableLength, transmitFreqKHz=transmitFreqKHz, sNMPTrapTableIndex=sNMPTrapTableIndex, phyStatusGroup=phyStatusGroup, tDDControlGroup=tDDControlGroup, transmitModulationMode=transmitModulationMode, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, vLANPriorityTable=vLANPriorityTable, receiveChannel=receiveChannel, noWirelessChannelAvailable=noWirelessChannelAvailable, userTableNumber=userTableNumber, timeZone=timeZone, tDDSynchronizationAlarm=tDDSynchronizationAlarm, licenceGroup=licenceGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, vLANManagementPriority=vLANManagementPriority, versions=versions, vLANManagementVID=vLANManagementVID, wirelessLinkAvailability=wirelessLinkAvailability, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, ethernetGroup=ethernetGroup, dataPortDisabledWarning=dataPortDisabledWarning, alarmsGroup=alarmsGroup, management=management, userTableIndex=userTableIndex, transmitPower=transmitPower, dfsTableNumber=dfsTableNumber, sNTPServerIPAddress=sNTPServerIPAddress, maximumTransmitPower=maximumTransmitPower, installStatusTrap=installStatusTrap, ptp500=ptp500, sNTPSyncTrap=sNTPSyncTrap, dfs=dfs, smtp=smtp, alarms=alarms, telecomsInterface=telecomsInterface, receiveDataRate=receiveDataRate, bandwidthVariant=bandwidthVariant, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, constantPowerSpectralDensity=constantPowerSpectralDensity, reset=reset, linkLoss=linkLoss, syslogStateTrap=syslogStateTrap, sNMPTrapVersion=sNMPTrapVersion, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, syslogControl=syslogControl, masterSlaveMode=masterSlaveMode, phyStatus=phyStatus, sNMPPortNumber=sNMPPortNumber, vectorError=vectorError, userEnabled=userEnabled, antennaGain=antennaGain, tDDSynchronizationStatus=tDDSynchronizationStatus, remoteIPAddress=remoteIPAddress, vLANPriorityTableNumber=vLANPriorityTableNumber, cambium=cambium, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sNMPCommunityString=sNMPCommunityString, dataPortStatusTrap=dataPortStatusTrap, telecomsLineCode=telecomsLineCode, sNTPSync=sNTPSync, transmitDataRate=transmitDataRate, syslogLocalWrappedTrap=syslogLocalWrappedTrap, transmitFreqMHz=transmitFreqMHz, installArmState=installArmState, syslogState=syslogState, snmpControlGroup=snmpControlGroup, resetGroup=resetGroup, notificationsGroup=notificationsGroup, dataPortWirelessDownAlert=dataPortWirelessDownAlert, frequencyVariant=frequencyVariant, daylightSaving=daylightSaving, ptp=ptp, useVLANForManagementInterfaces=useVLANForManagementInterfaces, sNTPServerPortNumber=sNTPServerPortNumber, dataPortStatus=dataPortStatus, searchState=searchState, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, bootVersion=bootVersion, userTableEntry=userTableEntry, encryption=encryption, ptpTrapPrefix=ptpTrapPrefix, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, vLANPriorityTableEntry=vLANPriorityTableEntry, linkSymmetry=linkSymmetry, linkModeOptimizationMismatch=linkModeOptimizationMismatch, configuration=configuration, eIRP=eIRP, sMTPEnabledMessages=sMTPEnabledMessages, productVariant=productVariant, sNMPTrapTable=sNMPTrapTable, hTTPSAccessEnabled=hTTPSAccessEnabled, snmpControl=snmpControl, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, tDDControl=tDDControl, hardwareVersion=hardwareVersion, syslogClient=syslogClient, systemReset=systemReset, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, vLANPriorityQueueMapping=vLANPriorityQueueMapping, telecom=telecom, encryptionGroup=encryptionGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, dfsTableIndex=dfsTableIndex, sNMPv3Enable=sNMPv3Enable, tFTPServerPortNumber=tFTPServerPortNumber, dfsPeaks=dfsPeaks, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, licensedCapacity=licensedCapacity, sntp=sntp, sNMPTrapTableNumber=sNMPTrapTableNumber, channelChangeTrap=channelChangeTrap, receiveFreqMHz=receiveFreqMHz, phyControlGroup=phyControlGroup, dfsTable=dfsTable, PYSNMP_MODULE_ID=cambium, licence=licence, systemClock=systemClock, versionsGroup=versionsGroup, regionCode=regionCode, telnetPortNumber=telnetPortNumber, altitude=altitude, receiveModulationModeDetail=receiveModulationModeDetail, targetMACAddress=targetMACAddress, latitude=latitude, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, softwareVersion=softwareVersion, telecomsLoopbackTrap=telecomsLoopbackTrap, remoteMaximumTransmitPower=remoteMaximumTransmitPower, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, receiveFreqKHz=receiveFreqKHz, byteErrorRatio=byteErrorRatio, sntpGroup=sntpGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, tDDSynchronizationMode=tDDSynchronizationMode, receiveModulationMode=receiveModulationMode, sNTPLastSync=sNTPLastSync, installArmStateTrap=installArmStateTrap, sNMPTrapIPAddress=sNMPTrapIPAddress, ethernetFiberSupport=ethernetFiberSupport, incompatibleRegionCodes=incompatibleRegionCodes, dataPortAutoMdix=dataPortAutoMdix, vLANPriorityTableIndex=vLANPriorityTableIndex, sMTPEmailAlert=sMTPEmailAlert, ptmp=ptmp, supplementary=supplementary, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, signalStrengthRatio=signalStrengthRatio, subnetMask=subnetMask, channelBandwidth=channelBandwidth, gatewayIPAddress=gatewayIPAddress, transmitChannel=transmitChannel, telecomsChannelStatus=telecomsChannelStatus, dfsMeans=dfsMeans, supplementaryGroup=supplementaryGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, range=range, sNTPState=sNTPState, longitude=longitude, userTable=userTable, groupID=groupID, dfsGroup=dfsGroup, aggregateDataRate=aggregateDataRate, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, userRole=userRole, userPassword=userPassword, pubStatsGroup=pubStatsGroup, telecomsLoopback=telecomsLoopback, ptpTraps=ptpTraps, sNTPPollInterval=sNTPPollInterval, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, hTTPSPortNumber=hTTPSPortNumber, telecomsChannelStatusTrap=telecomsChannelStatusTrap, telecomGroup=telecomGroup, aAAControlGroup=aAAControlGroup, hTTPPortNumber=hTTPPortNumber, aAAControl=aAAControl, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, wirelessLinkStatus=wirelessLinkStatus, regionCodeTrap=regionCodeTrap, ptpCompliance=ptpCompliance, syslogClientTrap=syslogClientTrap, managementGroup=managementGroup, dfsTableEntry=dfsTableEntry, productName=productName, cableLoss=cableLoss, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, smtpGroup=smtpGroup, sNMPEnabledTraps=sNMPEnabledTraps, telnetAccessEnabled=telnetAccessEnabled, enabledDiagnosticAlarms=enabledDiagnosticAlarms, iPAddress=iPAddress, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, receivePower=receivePower, configurationGroup=configurationGroup, incompatibleMasterAndSlave=incompatibleMasterAndSlave, unitOutOfCalibration=unitOutOfCalibration, dataPortConfigurationMismatch=dataPortConfigurationMismatch, ethernet=ethernet, syslogControlGroup=syslogControlGroup, installStatus=installStatus, linkName=linkName, sMTPSourceEmailAddress=sMTPSourceEmailAddress, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, hTTPAccessEnabled=hTTPAccessEnabled, sMTPServerIPAddress=sMTPServerIPAddress, linkModeOptimisation=linkModeOptimisation, secondarySoftwareVersion=secondarySoftwareVersion, siteName=siteName, accessMethod=accessMethod, pubStats=pubStats, tFTPServerIPAddress=tFTPServerIPAddress, userName=userName, capacityLicenseMismatch=capacityLicenseMismatch, phyControl=phyControl, encryptionAlgorithm=encryptionAlgorithm, ptpGroups=ptpGroups)
