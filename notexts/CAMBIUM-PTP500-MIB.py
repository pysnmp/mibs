...
# Produced by pysmi-1.1.3 at Tue Dec  7 14:16:14 2021
...
enterprises, iso, IpAddress, Counter64, MibIdentifier, NotificationType, Unsigned32, Counter32, Bits, TimeTicks, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Integer32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "iso", "IpAddress", "Counter64", "MibIdentifier", "NotificationType", "Unsigned32", "Counter32", "Bits", "TimeTicks", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Integer32", "ModuleIdentity")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", dfsTable=dfsTable, transmitModulationMode=transmitModulationMode, transmitFreqKHz=transmitFreqKHz, syslogClient=syslogClient, versionsGroup=versionsGroup, receiveFreqKHz=receiveFreqKHz, syslogState=syslogState, hTTPAccessEnabled=hTTPAccessEnabled, userEnabled=userEnabled, supplementary=supplementary, telecomGroup=telecomGroup, tDDControl=tDDControl, dfsTableNumber=dfsTableNumber, vLANPriorityTableNumber=vLANPriorityTableNumber, secondarySoftwareVersion=secondarySoftwareVersion, vLANPriorityTableIndex=vLANPriorityTableIndex, linkName=linkName, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sMTPEnabledMessages=sMTPEnabledMessages, linkModeOptimizationMismatch=linkModeOptimizationMismatch, tDDControlGroup=tDDControlGroup, groupID=groupID, phyControlGroup=phyControlGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, pubStatsGroup=pubStatsGroup, ethernet=ethernet, telecomsLoopbackTrap=telecomsLoopbackTrap, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, longitude=longitude, aggregateDataRate=aggregateDataRate, systemReset=systemReset, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, capacityLicenseMismatch=capacityLicenseMismatch, ptpTrapPrefix=ptpTrapPrefix, tDDSynchronizationStatus=tDDSynchronizationStatus, sNMPTrapTable=sNMPTrapTable, softwareVersion=softwareVersion, receiveDataRate=receiveDataRate, vLANPriorityTableEntry=vLANPriorityTableEntry, iPAddress=iPAddress, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, sMTPServerPortNumber=sMTPServerPortNumber, configuration=configuration, installArmStateTrap=installArmStateTrap, sMTPEmailAlert=sMTPEmailAlert, reset=reset, regionCodeTrap=regionCodeTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, signalStrengthRatio=signalStrengthRatio, dataPortStatusTrap=dataPortStatusTrap, ptp500=ptp500, dfs=dfs, dfsPeaks=dfsPeaks, aAAControl=aAAControl, transmitPower=transmitPower, gatewayIPAddress=gatewayIPAddress, management=management, receiveChannel=receiveChannel, dfsGroup=dfsGroup, licenceGroup=licenceGroup, searchState=searchState, telecomsInterface=telecomsInterface, smtpGroup=smtpGroup, sMTPSourceEmailAddress=sMTPSourceEmailAddress, byteErrorRatio=byteErrorRatio, latitude=latitude, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, receivePower=receivePower, versions=versions, linkSymmetry=linkSymmetry, sNMPv3Enable=sNMPv3Enable, linkModeOptimisation=linkModeOptimisation, userRole=userRole, phyControl=phyControl, ethernetFiberSupport=ethernetFiberSupport, timeZone=timeZone, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, remoteIPAddress=remoteIPAddress, vLANPriorityQueueMapping=vLANPriorityQueueMapping, telecomsLineCode=telecomsLineCode, ptpTraps=ptpTraps, pubStats=pubStats, syslogControlGroup=syslogControlGroup, sMTPServerIPAddress=sMTPServerIPAddress, range=range, sntp=sntp, ptp=ptp, altitude=altitude, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, daylightSaving=daylightSaving, PYSNMP_MODULE_ID=cambium, ptmp=ptmp, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, encryption=encryption, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, dataPortAutoNegotiation=dataPortAutoNegotiation, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, systemClock=systemClock, eIRP=eIRP, snmpControlGroup=snmpControlGroup, supplementaryGroup=supplementaryGroup, targetMACAddress=targetMACAddress, hTTPSAccessEnabled=hTTPSAccessEnabled, sNMPTrapIPAddress=sNMPTrapIPAddress, sNTPPollInterval=sNTPPollInterval, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, sNMPTrapTableNumber=sNMPTrapTableNumber, telnetAccessEnabled=telnetAccessEnabled, alarmsGroup=alarmsGroup, phyStatusGroup=phyStatusGroup, unitOutOfCalibration=unitOutOfCalibration, dataPortAutoMdix=dataPortAutoMdix, userTableNumber=userTableNumber, resetGroup=resetGroup, userTableEntry=userTableEntry, licence=licence, alarms=alarms, userName=userName, enabledDiagnosticAlarms=enabledDiagnosticAlarms, vLANPriorityTable=vLANPriorityTable, siteName=siteName, encryptionGroup=encryptionGroup, aAAControlGroup=aAAControlGroup, transmitFreqMHz=transmitFreqMHz, vectorError=vectorError, managementGroup=managementGroup, frequencyVariant=frequencyVariant, sNMPTrapPortNumber=sNMPTrapPortNumber, sNTPSync=sNTPSync, userTableIndex=userTableIndex, channelChangeTrap=channelChangeTrap, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, sNTPServerIPAddress=sNTPServerIPAddress, sNTPServerPortNumber=sNTPServerPortNumber, tDDSynchronizationMode=tDDSynchronizationMode, cambium=cambium, configurationGroup=configurationGroup, encryptionAlgorithm=encryptionAlgorithm, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, ethernetGroup=ethernetGroup, installStatus=installStatus, receiveModulationMode=receiveModulationMode, transmitChannel=transmitChannel, cableLoss=cableLoss, bootVersion=bootVersion, phyStatus=phyStatus, sNMPEnabledTraps=sNMPEnabledTraps, telecomsCableLength=telecomsCableLength, useVLANForManagementInterfaces=useVLANForManagementInterfaces, linkLoss=linkLoss, masterSlaveMode=masterSlaveMode, syslogControl=syslogControl, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, dataPortStatus=dataPortStatus, ptpGroups=ptpGroups, vLANManagementPriority=vLANManagementPriority, dataPortConfigurationMismatch=dataPortConfigurationMismatch, hardwareVersion=hardwareVersion, userPassword=userPassword, syslogStateTrap=syslogStateTrap, notificationsGroup=notificationsGroup, smtp=smtp, dfsTableIndex=dfsTableIndex, sNMPCommunityString=sNMPCommunityString, receiveModulationModeDetail=receiveModulationModeDetail, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dataPortWirelessDownAlert=dataPortWirelessDownAlert, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sNTPState=sNTPState, subnetMask=subnetMask, telecom=telecom, dfsMeans=dfsMeans, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, tFTPServerIPAddress=tFTPServerIPAddress, antennaGain=antennaGain, sNMPPortNumber=sNMPPortNumber, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sNTPSyncTrap=sNTPSyncTrap, dataPortDisabledWarning=dataPortDisabledWarning, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sNTPLastSync=sNTPLastSync, telnetPortNumber=telnetPortNumber, constantPowerSpectralDensity=constantPowerSpectralDensity, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, hTTPPortNumber=hTTPPortNumber, transmitDataRate=transmitDataRate, regionCode=regionCode, channelBandwidth=channelBandwidth, installStatusTrap=installStatusTrap, hTTPSPortNumber=hTTPSPortNumber, userTable=userTable, incompatibleRegionCodes=incompatibleRegionCodes, telecomsChannelStatusTrap=telecomsChannelStatusTrap, snmpControl=snmpControl, tFTPServerPortNumber=tFTPServerPortNumber, tDDSynchronizationAlarm=tDDSynchronizationAlarm, receiveFreqMHz=receiveFreqMHz, wirelessLinkStatus=wirelessLinkStatus, productName=productName, licensedCapacity=licensedCapacity, wirelessLinkAvailability=wirelessLinkAvailability, syslogClientTrap=syslogClientTrap, productVariant=productVariant, ptpCompliance=ptpCompliance, accessMethod=accessMethod, maximumTransmitPower=maximumTransmitPower, telecomsChannelStatus=telecomsChannelStatus, bandwidthVariant=bandwidthVariant, sNMPTrapVersion=sNMPTrapVersion, dfsTableEntry=dfsTableEntry, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPTrapTableIndex=sNMPTrapTableIndex, telecomsLoopback=telecomsLoopback, installArmState=installArmState, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, vLANManagementVID=vLANManagementVID, sntpGroup=sntpGroup)
