...
MibIdentifier, iso, Integer32, enterprises, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Counter32, Gauge32, NotificationType, Counter64, IpAddress, Unsigned32, ModuleIdentity, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "iso", "Integer32", "enterprises", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Counter32", "Gauge32", "NotificationType", "Counter64", "IpAddress", "Unsigned32", "ModuleIdentity", "ObjectIdentity")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", softwareVersion=softwareVersion, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, vLANManagementPriority=vLANManagementPriority, targetMACAddress=targetMACAddress, transmitPower=transmitPower, sNMPEnabledTraps=sNMPEnabledTraps, systemClock=systemClock, pubStats=pubStats, dataPortAutoNegotiation=dataPortAutoNegotiation, receiveModulationMode=receiveModulationMode, dfsPeaks=dfsPeaks, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, smtpGroup=smtpGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, receiveFreqKHz=receiveFreqKHz, userName=userName, vLANPriorityTable=vLANPriorityTable, sNMPTrapTableNumber=sNMPTrapTableNumber, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, transmitModulationMode=transmitModulationMode, ptpTraps=ptpTraps, userRole=userRole, linkSymmetry=linkSymmetry, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, alarmsGroup=alarmsGroup, pubStatsGroup=pubStatsGroup, transmitChannel=transmitChannel, dfsGroup=dfsGroup, wirelessLinkStatus=wirelessLinkStatus, smtp=smtp, phyControlGroup=phyControlGroup, sNMPTrapVersion=sNMPTrapVersion, encryption=encryption, ethernetFiberSupport=ethernetFiberSupport, vectorError=vectorError, gatewayIPAddress=gatewayIPAddress, telecomsChannelStatus=telecomsChannelStatus, userTable=userTable, installStatusTrap=installStatusTrap, sNMPTrapTable=sNMPTrapTable, snmpControl=snmpControl, hTTPAccessEnabled=hTTPAccessEnabled, phyStatus=phyStatus, channelBandwidth=channelBandwidth, eIRP=eIRP, longitude=longitude, constantPowerSpectralDensity=constantPowerSpectralDensity, maximumTransmitPower=maximumTransmitPower, enabledDiagnosticAlarms=enabledDiagnosticAlarms, tDDSynchronizationMode=tDDSynchronizationMode, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, notificationsGroup=notificationsGroup, latitude=latitude, licensedCapacity=licensedCapacity, antennaGain=antennaGain, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, cableLoss=cableLoss, sNTPState=sNTPState, versions=versions, sNMPTrapIPAddress=sNMPTrapIPAddress, sNTPPollInterval=sNTPPollInterval, dataPortStatusTrap=dataPortStatusTrap, transmitFreqKHz=transmitFreqKHz, ethernetGroup=ethernetGroup, dfsTableIndex=dfsTableIndex, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, transmitDataRate=transmitDataRate, syslogClient=syslogClient, sNTPServerIPAddress=sNTPServerIPAddress, management=management, hTTPPortNumber=hTTPPortNumber, incompatibleMasterAndSlave=incompatibleMasterAndSlave, dfs=dfs, subnetMask=subnetMask, linkLoss=linkLoss, tDDSynchronizationStatus=tDDSynchronizationStatus, licenceGroup=licenceGroup, cambium=cambium, ptmp=ptmp, dfsTable=dfsTable, telnetPortNumber=telnetPortNumber, receivePower=receivePower, tDDControlGroup=tDDControlGroup, sNTPServerPortNumber=sNTPServerPortNumber, wirelessLinkAvailability=wirelessLinkAvailability, tDDControl=tDDControl, syslogState=syslogState, frequencyVariant=frequencyVariant, aAAControlGroup=aAAControlGroup, bandwidthVariant=bandwidthVariant, hardwareVersion=hardwareVersion, versionsGroup=versionsGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, receiveDataRate=receiveDataRate, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, dataPortAutoMdix=dataPortAutoMdix, regionCode=regionCode, hTTPSPortNumber=hTTPSPortNumber, unitOutOfCalibration=unitOutOfCalibration, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNMPTrapTableEntry=sNMPTrapTableEntry, productName=productName, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, installStatus=installStatus, vLANPriorityTableEntry=vLANPriorityTableEntry, userTableEntry=userTableEntry, ptpGroups=ptpGroups, syslogLocalWrappedTrap=syslogLocalWrappedTrap, productVariant=productVariant, ptp=ptp, regionCodeTrap=regionCodeTrap, aAAControl=aAAControl, dataPortStatus=dataPortStatus, phyStatusGroup=phyStatusGroup, sntpGroup=sntpGroup, useVLANForManagementInterfaces=useVLANForManagementInterfaces, resetGroup=resetGroup, remoteMaximumTransmitPower=remoteMaximumTransmitPower, noWirelessChannelAvailable=noWirelessChannelAvailable, syslogControlGroup=syslogControlGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPTrapPortNumber=sNMPTrapPortNumber, encryptionAlgorithm=encryptionAlgorithm, altitude=altitude, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, byteErrorRatio=byteErrorRatio, supplementary=supplementary, tFTPServerPortNumber=tFTPServerPortNumber, ethernet=ethernet, telnetAccessEnabled=telnetAccessEnabled, linkName=linkName, telecomsCableLength=telecomsCableLength, dataPortConfigurationMismatch=dataPortConfigurationMismatch, vLANPriorityTableNumber=vLANPriorityTableNumber, vLANPriorityTableIndex=vLANPriorityTableIndex, receiveFreqMHz=receiveFreqMHz, ptpCompliance=ptpCompliance, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, dataPortWirelessDownAlert=dataPortWirelessDownAlert, aggregateDataRate=aggregateDataRate, telecomsLoopback=telecomsLoopback, dfsMeans=dfsMeans, sNTPSyncTrap=sNTPSyncTrap, sMTPSourceEmailAddress=sMTPSourceEmailAddress, installArmState=installArmState, secondarySoftwareVersion=secondarySoftwareVersion, reset=reset, range=range, accessMethod=accessMethod, PYSNMP_MODULE_ID=cambium, receiveModulationModeDetail=receiveModulationModeDetail, daylightSaving=daylightSaving, tFTPServerIPAddress=tFTPServerIPAddress, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, dfsTableEntry=dfsTableEntry, configuration=configuration, sMTPEnabledMessages=sMTPEnabledMessages, signalStrengthRatio=signalStrengthRatio, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, alarms=alarms, userTableNumber=userTableNumber, sMTPServerPortNumber=sMTPServerPortNumber, remoteIPAddress=remoteIPAddress, telecom=telecom, tDDSynchronizationAlarm=tDDSynchronizationAlarm, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, hTTPSAccessEnabled=hTTPSAccessEnabled, managementGroup=managementGroup, vLANPriorityQueueMapping=vLANPriorityQueueMapping, syslogClientTrap=syslogClientTrap, sntp=sntp, timeZone=timeZone, sNMPv3Enable=sNMPv3Enable, encryptionGroup=encryptionGroup, sMTPServerIPAddress=sMTPServerIPAddress, sNTPSync=sNTPSync, phyControl=phyControl, installArmStateTrap=installArmStateTrap, ptpTrapPrefix=ptpTrapPrefix, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, dataPortDisabledWarning=dataPortDisabledWarning, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, siteName=siteName, groupID=groupID, syslogStateTrap=syslogStateTrap, licence=licence, linkModeOptimisation=linkModeOptimisation, iPAddress=iPAddress, telecomGroup=telecomGroup, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, systemReset=systemReset, ptp500=ptp500, searchState=searchState, telecomsChannelStatusTrap=telecomsChannelStatusTrap, channelChangeTrap=channelChangeTrap, bootVersion=bootVersion, incompatibleRegionCodes=incompatibleRegionCodes, linkModeOptimizationMismatch=linkModeOptimizationMismatch, telecomsLineCode=telecomsLineCode, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, userTableIndex=userTableIndex, sNTPLastSync=sNTPLastSync, telecomsLoopbackTrap=telecomsLoopbackTrap, sNMPCommunityString=sNMPCommunityString, sNMPPortNumber=sNMPPortNumber, userPassword=userPassword, masterSlaveMode=masterSlaveMode, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, configurationGroup=configurationGroup, dfsTableNumber=dfsTableNumber, sMTPEmailAlert=sMTPEmailAlert, userEnabled=userEnabled, snmpControlGroup=snmpControlGroup, vLANManagementVID=vLANManagementVID, receiveChannel=receiveChannel, telecomsInterface=telecomsInterface, transmitFreqMHz=transmitFreqMHz, syslogControl=syslogControl, capacityLicenseMismatch=capacityLicenseMismatch, supplementaryGroup=supplementaryGroup)
