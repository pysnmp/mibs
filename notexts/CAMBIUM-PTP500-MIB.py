...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", aggregateDataRate=aggregateDataRate, dataPortAutoMdix=dataPortAutoMdix, installArmStateTrap=installArmStateTrap, receiveDataRate=receiveDataRate, remoteIPAddress=remoteIPAddress, incompatibleRegionCodes=incompatibleRegionCodes, sMTPEmailAlert=sMTPEmailAlert, latitude=latitude, pubStats=pubStats, ptpTrapPrefix=ptpTrapPrefix, daylightSaving=daylightSaving, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, sNMPEnabledTraps=sNMPEnabledTraps, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, phyControl=phyControl, hTTPSPortNumber=hTTPSPortNumber, vLANPriorityTableEntry=vLANPriorityTableEntry, dfs=dfs, notificationsGroup=notificationsGroup, constantPowerSpectralDensity=constantPowerSpectralDensity, hardwareVersion=hardwareVersion, ptpTraps=ptpTraps, syslogControl=syslogControl, sMTPServerPortNumber=sMTPServerPortNumber, incompatibleMasterAndSlave=incompatibleMasterAndSlave, encryptionAlgorithm=encryptionAlgorithm, userName=userName, enabledDiagnosticAlarms=enabledDiagnosticAlarms, syslogClient=syslogClient, regionCode=regionCode, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, searchState=searchState, secondarySoftwareVersion=secondarySoftwareVersion, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, channelChangeTrap=channelChangeTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sNMPTrapTableEntry=sNMPTrapTableEntry, licence=licence, remoteMaximumTransmitPower=remoteMaximumTransmitPower, subnetMask=subnetMask, systemClock=systemClock, transmitPower=transmitPower, transmitModulationMode=transmitModulationMode, installStatusTrap=installStatusTrap, syslogClientTrap=syslogClientTrap, gatewayIPAddress=gatewayIPAddress, sMTPSourceEmailAddress=sMTPSourceEmailAddress, telecomsLineCode=telecomsLineCode, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, dfsTableNumber=dfsTableNumber, sNMPTrapTableNumber=sNMPTrapTableNumber, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, sNTPServerIPAddress=sNTPServerIPAddress, linkModeOptimizationMismatch=linkModeOptimizationMismatch, transmitDataRate=transmitDataRate, ptpGroups=ptpGroups, wirelessLinkAvailability=wirelessLinkAvailability, tDDSynchronizationStatus=tDDSynchronizationStatus, configuration=configuration, ethernet=ethernet, versionsGroup=versionsGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, ethernetGroup=ethernetGroup, sNMPTrapIPAddress=sNMPTrapIPAddress, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, sNMPTrapPortNumber=sNMPTrapPortNumber, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, licenceGroup=licenceGroup, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, encryption=encryption, receiveModulationModeDetail=receiveModulationModeDetail, linkName=linkName, antennaGain=antennaGain, dataPortConfigurationMismatch=dataPortConfigurationMismatch, versions=versions, iPAddress=iPAddress, ptp500=ptp500, targetMACAddress=targetMACAddress, telecom=telecom, ptmp=ptmp, alarms=alarms, syslogControlGroup=syslogControlGroup, dfsTable=dfsTable, telecomsChannelStatusTrap=telecomsChannelStatusTrap, receivePower=receivePower, eIRP=eIRP, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, syslogStateTrap=syslogStateTrap, phyStatusGroup=phyStatusGroup, telnetPortNumber=telnetPortNumber, sMTPEnabledMessages=sMTPEnabledMessages, pubStatsGroup=pubStatsGroup, dfsTableEntry=dfsTableEntry, userRole=userRole, aAAControl=aAAControl, userPassword=userPassword, smtp=smtp, tDDSynchronizationMode=tDDSynchronizationMode, altitude=altitude, sNTPSync=sNTPSync, frequencyVariant=frequencyVariant, receiveFreqMHz=receiveFreqMHz, byteErrorRatio=byteErrorRatio, dfsPeaks=dfsPeaks, reset=reset, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, installStatus=installStatus, bandwidthVariant=bandwidthVariant, range=range, dfsGroup=dfsGroup, noWirelessChannelAvailable=noWirelessChannelAvailable, dataPortStatus=dataPortStatus, syslogState=syslogState, unitOutOfCalibration=unitOutOfCalibration, vLANPriorityQueueMapping=vLANPriorityQueueMapping, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, capacityLicenseMismatch=capacityLicenseMismatch, alarmsGroup=alarmsGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, supplementary=supplementary, snmpControlGroup=snmpControlGroup, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, transmitFreqKHz=transmitFreqKHz, masterSlaveMode=masterSlaveMode, vLANPriorityTableIndex=vLANPriorityTableIndex, receiveFreqKHz=receiveFreqKHz, dfsMeans=dfsMeans, dfsTableIndex=dfsTableIndex, signalStrengthRatio=signalStrengthRatio, sntp=sntp, encryptionGroup=encryptionGroup, tFTPServerPortNumber=tFTPServerPortNumber, userTableEntry=userTableEntry, userTable=userTable, ptp=ptp, hTTPSAccessEnabled=hTTPSAccessEnabled, ethernetFiberSupport=ethernetFiberSupport, linkSymmetry=linkSymmetry, resetGroup=resetGroup, tFTPServerIPAddress=tFTPServerIPAddress, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, vLANManagementPriority=vLANManagementPriority, smtpGroup=smtpGroup, installArmState=installArmState, productName=productName, tDDControl=tDDControl, telnetAccessEnabled=telnetAccessEnabled, receiveModulationMode=receiveModulationMode, telecomGroup=telecomGroup, sNTPServerPortNumber=sNTPServerPortNumber, sNMPTrapTableIndex=sNMPTrapTableIndex, PYSNMP_MODULE_ID=cambium, userTableNumber=userTableNumber, sNTPLastSync=sNTPLastSync, vLANPriorityTable=vLANPriorityTable, management=management, telecomsLoopback=telecomsLoopback, sNMPv3Enable=sNMPv3Enable, phyControlGroup=phyControlGroup, sNMPTrapVersion=sNMPTrapVersion, wirelessLinkStatus=wirelessLinkStatus, vLANPriorityTableNumber=vLANPriorityTableNumber, maximumTransmitPower=maximumTransmitPower, sNTPSyncTrap=sNTPSyncTrap, ptpCompliance=ptpCompliance, sMTPServerIPAddress=sMTPServerIPAddress, snmpControl=snmpControl, regionCodeTrap=regionCodeTrap, hTTPPortNumber=hTTPPortNumber, telecomsChannelStatus=telecomsChannelStatus, dataPortDisabledWarning=dataPortDisabledWarning, dataPortAutoNegotiation=dataPortAutoNegotiation, configurationGroup=configurationGroup, cableLoss=cableLoss, groupID=groupID, sNMPPortNumber=sNMPPortNumber, sNTPPollInterval=sNTPPollInterval, softwareVersion=softwareVersion, longitude=longitude, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, vectorError=vectorError, receiveChannel=receiveChannel, dataPortStatusTrap=dataPortStatusTrap, managementGroup=managementGroup, accessMethod=accessMethod, hTTPAccessEnabled=hTTPAccessEnabled, userEnabled=userEnabled, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, sNTPState=sNTPState, timeZone=timeZone, systemReset=systemReset, bootVersion=bootVersion, sntpGroup=sntpGroup, sNMPCommunityString=sNMPCommunityString, phyStatus=phyStatus, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, transmitFreqMHz=transmitFreqMHz, dataPortWirelessDownAlert=dataPortWirelessDownAlert, vLANManagementVID=vLANManagementVID, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, telecomsInterface=telecomsInterface, useVLANForManagementInterfaces=useVLANForManagementInterfaces, telecomsLoopbackTrap=telecomsLoopbackTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sNMPTrapTable=sNMPTrapTable, aAAControlGroup=aAAControlGroup, cambium=cambium, telecomsCableLength=telecomsCableLength, userTableIndex=userTableIndex, tDDControlGroup=tDDControlGroup, siteName=siteName, transmitChannel=transmitChannel, tDDSynchronizationAlarm=tDDSynchronizationAlarm, channelBandwidth=channelBandwidth, productVariant=productVariant, linkLoss=linkLoss, linkModeOptimisation=linkModeOptimisation, supplementaryGroup=supplementaryGroup, licensedCapacity=licensedCapacity)
