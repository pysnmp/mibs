...
ObjectIdentity, MibIdentifier, ModuleIdentity, TimeTicks, IpAddress, iso, Gauge32, Bits, Integer32, NotificationType, enterprises, Counter64, Unsigned32, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "MibIdentifier", "ModuleIdentity", "TimeTicks", "IpAddress", "iso", "Gauge32", "Bits", "Integer32", "NotificationType", "enterprises", "Counter64", "Unsigned32", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", vLANPriorityTableEntry=vLANPriorityTableEntry, bandwidthVariant=bandwidthVariant, sNTPState=sNTPState, masterSlaveMode=masterSlaveMode, antennaGain=antennaGain, linkModeOptimisation=linkModeOptimisation, telnetAccessEnabled=telnetAccessEnabled, longitude=longitude, hTTPSPortNumber=hTTPSPortNumber, capacityLicenseMismatch=capacityLicenseMismatch, sMTPServerIPAddress=sMTPServerIPAddress, telecomsLoopback=telecomsLoopback, maximumTransmitPower=maximumTransmitPower, receiveModulationModeDetail=receiveModulationModeDetail, userTableNumber=userTableNumber, hTTPPortNumber=hTTPPortNumber, PYSNMP_MODULE_ID=cambium, dfsTableNumber=dfsTableNumber, telnetPortNumber=telnetPortNumber, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, frequencyVariant=frequencyVariant, supplementaryGroup=supplementaryGroup, configuration=configuration, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, aAAControlGroup=aAAControlGroup, ptp=ptp, transmitChannel=transmitChannel, remoteIPAddress=remoteIPAddress, linkLoss=linkLoss, syslogControlGroup=syslogControlGroup, userName=userName, sMTPServerPortNumber=sMTPServerPortNumber, aggregateDataRate=aggregateDataRate, vectorError=vectorError, receiveFreqKHz=receiveFreqKHz, dfsGroup=dfsGroup, syslogStateTrap=syslogStateTrap, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, licensedCapacity=licensedCapacity, incompatibleRegionCodes=incompatibleRegionCodes, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sMTPSourceEmailAddress=sMTPSourceEmailAddress, ptp500=ptp500, alarms=alarms, telecomsLineCode=telecomsLineCode, smtp=smtp, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dfsTableIndex=dfsTableIndex, searchState=searchState, telecom=telecom, vLANPriorityTableNumber=vLANPriorityTableNumber, incompatibleMasterAndSlave=incompatibleMasterAndSlave, installStatusTrap=installStatusTrap, wirelessLinkAvailability=wirelessLinkAvailability, ptpGroups=ptpGroups, cambium=cambium, regionCode=regionCode, sNMPTrapTableEntry=sNMPTrapTableEntry, phyStatusGroup=phyStatusGroup, regionCodeTrap=regionCodeTrap, installArmStateTrap=installArmStateTrap, constantPowerSpectralDensity=constantPowerSpectralDensity, tDDSynchronizationAlarm=tDDSynchronizationAlarm, phyControlGroup=phyControlGroup, ptpTraps=ptpTraps, userPassword=userPassword, dfsMeans=dfsMeans, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sNTPPollInterval=sNTPPollInterval, encryption=encryption, channelBandwidth=channelBandwidth, telecomsChannelStatus=telecomsChannelStatus, cableLoss=cableLoss, ethernet=ethernet, dataPortStatus=dataPortStatus, sNMPTrapPortNumber=sNMPTrapPortNumber, phyControl=phyControl, eIRP=eIRP, syslogClient=syslogClient, latitude=latitude, installArmState=installArmState, sNTPServerPortNumber=sNTPServerPortNumber, encryptionGroup=encryptionGroup, linkName=linkName, receiveModulationMode=receiveModulationMode, licence=licence, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, vLANManagementVID=vLANManagementVID, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, gatewayIPAddress=gatewayIPAddress, transmitFreqKHz=transmitFreqKHz, tDDSynchronizationStatus=tDDSynchronizationStatus, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, encryptionAlgorithm=encryptionAlgorithm, syslogControl=syslogControl, pubStats=pubStats, systemClock=systemClock, transmitPower=transmitPower, transmitDataRate=transmitDataRate, dfs=dfs, versionsGroup=versionsGroup, tFTPServerIPAddress=tFTPServerIPAddress, timeZone=timeZone, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, linkSymmetry=linkSymmetry, range=range, sNMPTrapTable=sNMPTrapTable, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, siteName=siteName, notificationsGroup=notificationsGroup, sntp=sntp, sntpGroup=sntpGroup, syslogLocalWrappedTrap=syslogLocalWrappedTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, groupID=groupID, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, sMTPEnabledMessages=sMTPEnabledMessages, supplementary=supplementary, dataPortAutoNegotiation=dataPortAutoNegotiation, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, alarmsGroup=alarmsGroup, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, sNTPServerIPAddress=sNTPServerIPAddress, resetGroup=resetGroup, sNTPSync=sNTPSync, productName=productName, bootVersion=bootVersion, syslogClientTrap=syslogClientTrap, snmpControlGroup=snmpControlGroup, smtpGroup=smtpGroup, sNMPEnabledTraps=sNMPEnabledTraps, sMTPEmailAlert=sMTPEmailAlert, unitOutOfCalibration=unitOutOfCalibration, sNMPTrapTableNumber=sNMPTrapTableNumber, softwareVersion=softwareVersion, telecomGroup=telecomGroup, userEnabled=userEnabled, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNMPTrapVersion=sNMPTrapVersion, licenceGroup=licenceGroup, receivePower=receivePower, noWirelessChannelAvailable=noWirelessChannelAvailable, tDDSynchronizationMode=tDDSynchronizationMode, userTable=userTable, sNMPCommunityString=sNMPCommunityString, ptmp=ptmp, hardwareVersion=hardwareVersion, receiveChannel=receiveChannel, hTTPSAccessEnabled=hTTPSAccessEnabled, targetMACAddress=targetMACAddress, userRole=userRole, aAAControl=aAAControl, transmitFreqMHz=transmitFreqMHz, transmitModulationMode=transmitModulationMode, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, reset=reset, sNTPSyncTrap=sNTPSyncTrap, ptpTrapPrefix=ptpTrapPrefix, ethernetGroup=ethernetGroup, subnetMask=subnetMask, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, tDDControlGroup=tDDControlGroup, secondarySoftwareVersion=secondarySoftwareVersion, signalStrengthRatio=signalStrengthRatio, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, tDDControl=tDDControl, telecomsCableLength=telecomsCableLength, vLANPriorityQueueMapping=vLANPriorityQueueMapping, systemReset=systemReset, dfsTable=dfsTable, productVariant=productVariant, sNMPPortNumber=sNMPPortNumber, sNMPTrapIPAddress=sNMPTrapIPAddress, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, receiveFreqMHz=receiveFreqMHz, vLANManagementPriority=vLANManagementPriority, ethernetFiberSupport=ethernetFiberSupport, hTTPAccessEnabled=hTTPAccessEnabled, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, linkModeOptimizationMismatch=linkModeOptimizationMismatch, phyStatus=phyStatus, altitude=altitude, managementGroup=managementGroup, dfsTableEntry=dfsTableEntry, channelChangeTrap=channelChangeTrap, dataPortStatusTrap=dataPortStatusTrap, accessMethod=accessMethod, vLANPriorityTableIndex=vLANPriorityTableIndex, configurationGroup=configurationGroup, snmpControl=snmpControl, dataPortAutoMdix=dataPortAutoMdix, management=management, useVLANForManagementInterfaces=useVLANForManagementInterfaces, receiveDataRate=receiveDataRate, dataPortDisabledWarning=dataPortDisabledWarning, telecomsChannelStatusTrap=telecomsChannelStatusTrap, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, telecomsLoopbackTrap=telecomsLoopbackTrap, enabledDiagnosticAlarms=enabledDiagnosticAlarms, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, vLANPriorityTable=vLANPriorityTable, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dfsPeaks=dfsPeaks, syslogState=syslogState, sNTPLastSync=sNTPLastSync, daylightSaving=daylightSaving, installStatus=installStatus, telecomsInterface=telecomsInterface, wirelessLinkStatus=wirelessLinkStatus, userTableEntry=userTableEntry, versions=versions, iPAddress=iPAddress, byteErrorRatio=byteErrorRatio, tFTPServerPortNumber=tFTPServerPortNumber, ptpCompliance=ptpCompliance, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, pubStatsGroup=pubStatsGroup, sNMPv3Enable=sNMPv3Enable, sNMPTrapTableIndex=sNMPTrapTableIndex, userTableIndex=userTableIndex)
