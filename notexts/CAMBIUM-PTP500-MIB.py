...
# Produced by pysmi-1.1.3 at Wed Dec  8 21:18:36 2021
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", ptp=ptp, pubStatsGroup=pubStatsGroup, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, configurationGroup=configurationGroup, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, phyControlGroup=phyControlGroup, phyStatusGroup=phyStatusGroup, sMTPServerPortNumber=sMTPServerPortNumber, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, telecomGroup=telecomGroup, telnetPortNumber=telnetPortNumber, signalStrengthRatio=signalStrengthRatio, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dataPortConfigurationMismatch=dataPortConfigurationMismatch, latitude=latitude, tDDSynchronizationAlarm=tDDSynchronizationAlarm, userTableIndex=userTableIndex, sNTPSyncTrap=sNTPSyncTrap, syslogStateTrap=syslogStateTrap, installArmStateTrap=installArmStateTrap, sMTPEnabledMessages=sMTPEnabledMessages, byteErrorRatio=byteErrorRatio, installStatusTrap=installStatusTrap, ptmp=ptmp, noWirelessChannelAvailable=noWirelessChannelAvailable, incompatibleMasterAndSlave=incompatibleMasterAndSlave, accessMethod=accessMethod, sNTPSync=sNTPSync, regionCodeTrap=regionCodeTrap, linkModeOptimisation=linkModeOptimisation, iPAddress=iPAddress, channelBandwidth=channelBandwidth, userEnabled=userEnabled, syslogClient=syslogClient, dfsGroup=dfsGroup, transmitDataRate=transmitDataRate, sNMPTrapIPAddress=sNMPTrapIPAddress, incompatibleRegionCodes=incompatibleRegionCodes, PYSNMP_MODULE_ID=cambium, ptp500=ptp500, syslogControl=syslogControl, ptpTrapPrefix=ptpTrapPrefix, sNTPPollInterval=sNTPPollInterval, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, smtp=smtp, tDDSynchronizationStatus=tDDSynchronizationStatus, dataPortAutoNegotiation=dataPortAutoNegotiation, vLANPriorityTableEntry=vLANPriorityTableEntry, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, alarmsGroup=alarmsGroup, encryptionAlgorithm=encryptionAlgorithm, productName=productName, phyControl=phyControl, siteName=siteName, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, vLANManagementVID=vLANManagementVID, longitude=longitude, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, smtpGroup=smtpGroup, dfsTableNumber=dfsTableNumber, groupID=groupID, vLANPriorityTable=vLANPriorityTable, antennaGain=antennaGain, dataPortAutoMdix=dataPortAutoMdix, channelChangeTrap=channelChangeTrap, notificationsGroup=notificationsGroup, sNTPLastSync=sNTPLastSync, masterSlaveMode=masterSlaveMode, sntpGroup=sntpGroup, licence=licence, telecomsInterface=telecomsInterface, sMTPServerIPAddress=sMTPServerIPAddress, transmitChannel=transmitChannel, telecomsChannelStatusTrap=telecomsChannelStatusTrap, cableLoss=cableLoss, syslogState=syslogState, syslogClientTrap=syslogClientTrap, sNTPServerIPAddress=sNTPServerIPAddress, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, snmpControlGroup=snmpControlGroup, aggregateDataRate=aggregateDataRate, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, receiveModulationModeDetail=receiveModulationModeDetail, managementGroup=managementGroup, vLANManagementPriority=vLANManagementPriority, userRole=userRole, encryptionGroup=encryptionGroup, daylightSaving=daylightSaving, tDDControlGroup=tDDControlGroup, sNMPTrapTable=sNMPTrapTable, vLANPriorityTableNumber=vLANPriorityTableNumber, useVLANForManagementInterfaces=useVLANForManagementInterfaces, pubStats=pubStats, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, sNTPState=sNTPState, dfsMeans=dfsMeans, versions=versions, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, linkSymmetry=linkSymmetry, transmitPower=transmitPower, unitOutOfCalibration=unitOutOfCalibration, wirelessLinkStatus=wirelessLinkStatus, tDDSynchronizationMode=tDDSynchronizationMode, hTTPAccessEnabled=hTTPAccessEnabled, syslogControlGroup=syslogControlGroup, receiveChannel=receiveChannel, receiveFreqKHz=receiveFreqKHz, capacityLicenseMismatch=capacityLicenseMismatch, telecomsLineCode=telecomsLineCode, systemClock=systemClock, sNMPCommunityString=sNMPCommunityString, linkName=linkName, transmitFreqKHz=transmitFreqKHz, sntp=sntp, searchState=searchState, syslogLocalWrappedTrap=syslogLocalWrappedTrap, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, telecom=telecom, dfsPeaks=dfsPeaks, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, wirelessLinkAvailability=wirelessLinkAvailability, dfsTable=dfsTable, dataPortWirelessDownAlert=dataPortWirelessDownAlert, sNMPTrapPortNumber=sNMPTrapPortNumber, vLANPriorityTableIndex=vLANPriorityTableIndex, tFTPServerPortNumber=tFTPServerPortNumber, timeZone=timeZone, receiveDataRate=receiveDataRate, dataPortDisabledWarning=dataPortDisabledWarning, bootVersion=bootVersion, softwareVersion=softwareVersion, telecomsCableLength=telecomsCableLength, sMTPSourceEmailAddress=sMTPSourceEmailAddress, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNMPTrapTableNumber=sNMPTrapTableNumber, cambium=cambium, userTableNumber=userTableNumber, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, snmpControl=snmpControl, hTTPPortNumber=hTTPPortNumber, userTableEntry=userTableEntry, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, configuration=configuration, aAAControlGroup=aAAControlGroup, productVariant=productVariant, userName=userName, ethernet=ethernet, userPassword=userPassword, supplementary=supplementary, aAAControl=aAAControl, tFTPServerIPAddress=tFTPServerIPAddress, management=management, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sNTPServerPortNumber=sNTPServerPortNumber, eIRP=eIRP, versionsGroup=versionsGroup, vLANPriorityQueueMapping=vLANPriorityQueueMapping, alarms=alarms, hTTPSPortNumber=hTTPSPortNumber, supplementaryGroup=supplementaryGroup, sNMPv3Enable=sNMPv3Enable, frequencyVariant=frequencyVariant, installStatus=installStatus, secondarySoftwareVersion=secondarySoftwareVersion, bandwidthVariant=bandwidthVariant, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, licenceGroup=licenceGroup, linkModeOptimizationMismatch=linkModeOptimizationMismatch, hTTPSAccessEnabled=hTTPSAccessEnabled, userTable=userTable, ptpTraps=ptpTraps, telecomsChannelStatus=telecomsChannelStatus, dataPortStatus=dataPortStatus, ethernetGroup=ethernetGroup, maximumTransmitPower=maximumTransmitPower, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, hardwareVersion=hardwareVersion, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dfs=dfs, sNMPEnabledTraps=sNMPEnabledTraps, tDDControl=tDDControl, dfsTableIndex=dfsTableIndex, constantPowerSpectralDensity=constantPowerSpectralDensity, transmitModulationMode=transmitModulationMode, sNMPTrapTableIndex=sNMPTrapTableIndex, remoteIPAddress=remoteIPAddress, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, sMTPEmailAlert=sMTPEmailAlert, ptpCompliance=ptpCompliance, dfsTableEntry=dfsTableEntry, phyStatus=phyStatus, telecomsLoopbackTrap=telecomsLoopbackTrap, range=range, remoteMaximumTransmitPower=remoteMaximumTransmitPower, telnetAccessEnabled=telnetAccessEnabled, subnetMask=subnetMask, resetGroup=resetGroup, ethernetFiberSupport=ethernetFiberSupport, ptpGroups=ptpGroups, sNMPPortNumber=sNMPPortNumber, systemReset=systemReset, sNMPTrapTableEntry=sNMPTrapTableEntry, licensedCapacity=licensedCapacity, linkLoss=linkLoss, vectorError=vectorError, transmitFreqMHz=transmitFreqMHz, gatewayIPAddress=gatewayIPAddress, receiveModulationMode=receiveModulationMode, installArmState=installArmState, dataPortStatusTrap=dataPortStatusTrap, telecomsLoopback=telecomsLoopback, altitude=altitude, reset=reset, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, encryption=encryption, regionCode=regionCode, receivePower=receivePower, sNMPTrapVersion=sNMPTrapVersion, targetMACAddress=targetMACAddress, receiveFreqMHz=receiveFreqMHz, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex)
