...
# Produced by pysmi-1.1.3 at Sun Nov 28 19:21:08 2021
...
Counter32, Counter64, MibIdentifier, IpAddress, iso, Gauge32, enterprises, ModuleIdentity, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, NotificationType, TimeTicks, Bits, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "Counter64", "MibIdentifier", "IpAddress", "iso", "Gauge32", "enterprises", "ModuleIdentity", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "NotificationType", "TimeTicks", "Bits", "Integer32")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", vectorError=vectorError, maximumTransmitPower=maximumTransmitPower, dfsTable=dfsTable, ethernet=ethernet, secondarySoftwareVersion=secondarySoftwareVersion, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sMTPServerPortNumber=sMTPServerPortNumber, vLANPriorityTable=vLANPriorityTable, capacityLicenseMismatch=capacityLicenseMismatch, frequencyVariant=frequencyVariant, sMTPSourceEmailAddress=sMTPSourceEmailAddress, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, licenceGroup=licenceGroup, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, vLANPriorityQueueMapping=vLANPriorityQueueMapping, longitude=longitude, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, installArmState=installArmState, sMTPEnabledMessages=sMTPEnabledMessages, pubStats=pubStats, bootVersion=bootVersion, syslogState=syslogState, userTableNumber=userTableNumber, receiveChannel=receiveChannel, searchState=searchState, supplementaryGroup=supplementaryGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNMPTrapTable=sNMPTrapTable, resetGroup=resetGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, aAAControlGroup=aAAControlGroup, telecomsCableLength=telecomsCableLength, management=management, antennaGain=antennaGain, licence=licence, aAAControl=aAAControl, transmitFreqMHz=transmitFreqMHz, reset=reset, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, vLANPriorityTableEntry=vLANPriorityTableEntry, userTableIndex=userTableIndex, encryption=encryption, smtp=smtp, receiveDataRate=receiveDataRate, linkLoss=linkLoss, dataPortDisabledWarning=dataPortDisabledWarning, daylightSaving=daylightSaving, byteErrorRatio=byteErrorRatio, telecomsInterface=telecomsInterface, ptpTrapPrefix=ptpTrapPrefix, eIRP=eIRP, encryptionAlgorithm=encryptionAlgorithm, wirelessLinkAvailability=wirelessLinkAvailability, altitude=altitude, linkModeOptimizationMismatch=linkModeOptimizationMismatch, cableLoss=cableLoss, configuration=configuration, ptpGroups=ptpGroups, useVLANForManagementInterfaces=useVLANForManagementInterfaces, constantPowerSpectralDensity=constantPowerSpectralDensity, sMTPEmailAlert=sMTPEmailAlert, transmitDataRate=transmitDataRate, versionsGroup=versionsGroup, dataPortConfigurationMismatch=dataPortConfigurationMismatch, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, sNTPServerPortNumber=sNTPServerPortNumber, ptpCompliance=ptpCompliance, managementGroup=managementGroup, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, snmpControl=snmpControl, phyControlGroup=phyControlGroup, sNTPServerIPAddress=sNTPServerIPAddress, tFTPServerIPAddress=tFTPServerIPAddress, hTTPPortNumber=hTTPPortNumber, dataPortAutoNegotiation=dataPortAutoNegotiation, sNMPTrapPortNumber=sNMPTrapPortNumber, receiveFreqMHz=receiveFreqMHz, userTable=userTable, userTableEntry=userTableEntry, transmitChannel=transmitChannel, tDDSynchronizationMode=tDDSynchronizationMode, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sntp=sntp, phyStatus=phyStatus, ethernetFiberSupport=ethernetFiberSupport, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNMPTrapTableNumber=sNMPTrapTableNumber, latitude=latitude, telecom=telecom, installStatusTrap=installStatusTrap, productVariant=productVariant, syslogControlGroup=syslogControlGroup, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, unitOutOfCalibration=unitOutOfCalibration, ptp500=ptp500, PYSNMP_MODULE_ID=cambium, receiveModulationMode=receiveModulationMode, alarmsGroup=alarmsGroup, pubStatsGroup=pubStatsGroup, userPassword=userPassword, masterSlaveMode=masterSlaveMode, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, dfsGroup=dfsGroup, sNTPState=sNTPState, userEnabled=userEnabled, dataPortStatusTrap=dataPortStatusTrap, regionCodeTrap=regionCodeTrap, sNTPSyncTrap=sNTPSyncTrap, versions=versions, channelBandwidth=channelBandwidth, syslogClientTrap=syslogClientTrap, vLANManagementVID=vLANManagementVID, systemClock=systemClock, sMTPServerIPAddress=sMTPServerIPAddress, sNMPTrapVersion=sNMPTrapVersion, syslogClient=syslogClient, dfsTableNumber=dfsTableNumber, snmpControlGroup=snmpControlGroup, transmitModulationMode=transmitModulationMode, userName=userName, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, sNMPTrapTableEntry=sNMPTrapTableEntry, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, dfs=dfs, linkModeOptimisation=linkModeOptimisation, sNTPSync=sNTPSync, timeZone=timeZone, userRole=userRole, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, telecomsLoopback=telecomsLoopback, linkName=linkName, ptpTraps=ptpTraps, dfsTableEntry=dfsTableEntry, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, vLANPriorityTableNumber=vLANPriorityTableNumber, phyStatusGroup=phyStatusGroup, iPAddress=iPAddress, systemReset=systemReset, dfsPeaks=dfsPeaks, tDDSynchronizationStatus=tDDSynchronizationStatus, smtpGroup=smtpGroup, transmitPower=transmitPower, incompatibleRegionCodes=incompatibleRegionCodes, receiveFreqKHz=receiveFreqKHz, licensedCapacity=licensedCapacity, sntpGroup=sntpGroup, regionCode=regionCode, noWirelessChannelAvailable=noWirelessChannelAvailable, sNTPLastSync=sNTPLastSync, softwareVersion=softwareVersion, telnetPortNumber=telnetPortNumber, dataPortWirelessDownAlert=dataPortWirelessDownAlert, dataPortAutoMdix=dataPortAutoMdix, encryptionGroup=encryptionGroup, sNMPPortNumber=sNMPPortNumber, sNMPCommunityString=sNMPCommunityString, sNMPTrapTableIndex=sNMPTrapTableIndex, aggregateDataRate=aggregateDataRate, ptp=ptp, telecomsChannelStatusTrap=telecomsChannelStatusTrap, phyControl=phyControl, bandwidthVariant=bandwidthVariant, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, sNMPv3Enable=sNMPv3Enable, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, syslogStateTrap=syslogStateTrap, tDDControlGroup=tDDControlGroup, groupID=groupID, targetMACAddress=targetMACAddress, alarms=alarms, telecomsLineCode=telecomsLineCode, telnetAccessEnabled=telnetAccessEnabled, subnetMask=subnetMask, syslogControl=syslogControl, sNMPEnabledTraps=sNMPEnabledTraps, cambium=cambium, supplementary=supplementary, ptmp=ptmp, hTTPAccessEnabled=hTTPAccessEnabled, receiveModulationModeDetail=receiveModulationModeDetail, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, wirelessLinkStatus=wirelessLinkStatus, hTTPSPortNumber=hTTPSPortNumber, ethernetGroup=ethernetGroup, transmitFreqKHz=transmitFreqKHz, linkSymmetry=linkSymmetry, range=range, hardwareVersion=hardwareVersion, receivePower=receivePower, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, productName=productName, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, dataPortStatus=dataPortStatus, installStatus=installStatus, telecomGroup=telecomGroup, vLANPriorityTableIndex=vLANPriorityTableIndex, hTTPSAccessEnabled=hTTPSAccessEnabled, channelChangeTrap=channelChangeTrap, installArmStateTrap=installArmStateTrap, siteName=siteName, notificationsGroup=notificationsGroup, configurationGroup=configurationGroup, dfsTableIndex=dfsTableIndex, sNMPTrapIPAddress=sNMPTrapIPAddress, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, gatewayIPAddress=gatewayIPAddress, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, remoteIPAddress=remoteIPAddress, dfsMeans=dfsMeans, sNTPPollInterval=sNTPPollInterval, signalStrengthRatio=signalStrengthRatio, telecomsLoopbackTrap=telecomsLoopbackTrap, vLANManagementPriority=vLANManagementPriority, telecomsChannelStatus=telecomsChannelStatus, tDDControl=tDDControl, tFTPServerPortNumber=tFTPServerPortNumber, accessMethod=accessMethod)
