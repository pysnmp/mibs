...
TimeTicks, Bits, Gauge32, enterprises, Integer32, ModuleIdentity, Counter32, Unsigned32, IpAddress, ObjectIdentity, Counter64, MibIdentifier, iso, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Bits", "Gauge32", "enterprises", "Integer32", "ModuleIdentity", "Counter32", "Unsigned32", "IpAddress", "ObjectIdentity", "Counter64", "MibIdentifier", "iso", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", dfsTable=dfsTable, dataPortAutoMdix=dataPortAutoMdix, ptp=ptp, sNMPCommunityString=sNMPCommunityString, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, telecomsInterface=telecomsInterface, encryption=encryption, constantPowerSpectralDensity=constantPowerSpectralDensity, telecomsChannelStatusTrap=telecomsChannelStatusTrap, telecomsLoopbackTrap=telecomsLoopbackTrap, regionCode=regionCode, sNMPTrapTable=sNMPTrapTable, receiveModulationMode=receiveModulationMode, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, secondarySoftwareVersion=secondarySoftwareVersion, telecomsChannelStatus=telecomsChannelStatus, phyStatusGroup=phyStatusGroup, tFTPServerIPAddress=tFTPServerIPAddress, productName=productName, sMTPServerIPAddress=sMTPServerIPAddress, smtp=smtp, vLANPriorityQueueMapping=vLANPriorityQueueMapping, range=range, latitude=latitude, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, syslogState=syslogState, pubStats=pubStats, installStatusTrap=installStatusTrap, ptmp=ptmp, timeZone=timeZone, dfsGroup=dfsGroup, sNTPSyncTrap=sNTPSyncTrap, configuration=configuration, antennaGain=antennaGain, signalStrengthRatio=signalStrengthRatio, sNTPState=sNTPState, telecomsLineCode=telecomsLineCode, telecomsLoopback=telecomsLoopback, smtpGroup=smtpGroup, licensedCapacity=licensedCapacity, sNTPSync=sNTPSync, licence=licence, reset=reset, pubStatsGroup=pubStatsGroup, dfsTableEntry=dfsTableEntry, sNTPLastSync=sNTPLastSync, capacityLicenseMismatch=capacityLicenseMismatch, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPTrapTableIndex=sNMPTrapTableIndex, dfsPeaks=dfsPeaks, ptpTrapPrefix=ptpTrapPrefix, encryptionAlgorithm=encryptionAlgorithm, management=management, byteErrorRatio=byteErrorRatio, ethernetGroup=ethernetGroup, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, userName=userName, installArmStateTrap=installArmStateTrap, userTable=userTable, sMTPEmailAlert=sMTPEmailAlert, ptpGroups=ptpGroups, snmpControl=snmpControl, receiveChannel=receiveChannel, dfsMeans=dfsMeans, useVLANForManagementInterfaces=useVLANForManagementInterfaces, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, vectorError=vectorError, transmitFreqMHz=transmitFreqMHz, maximumTransmitPower=maximumTransmitPower, aAAControl=aAAControl, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, eIRP=eIRP, vLANPriorityTable=vLANPriorityTable, dfsTableIndex=dfsTableIndex, searchState=searchState, dataPortAutoNegotiation=dataPortAutoNegotiation, remoteMaximumTransmitPower=remoteMaximumTransmitPower, userRole=userRole, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sMTPServerPortNumber=sMTPServerPortNumber, bootVersion=bootVersion, sNMPTrapTableEntry=sNMPTrapTableEntry, PYSNMP_MODULE_ID=cambium, managementGroup=managementGroup, transmitModulationMode=transmitModulationMode, receiveFreqMHz=receiveFreqMHz, masterSlaveMode=masterSlaveMode, userEnabled=userEnabled, dataPortConfigurationMismatch=dataPortConfigurationMismatch, phyStatus=phyStatus, incompatibleMasterAndSlave=incompatibleMasterAndSlave, userTableEntry=userTableEntry, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, softwareVersion=softwareVersion, sntp=sntp, ethernetFiberSupport=ethernetFiberSupport, dataPortStatusTrap=dataPortStatusTrap, alarms=alarms, wirelessLinkStatus=wirelessLinkStatus, noWirelessChannelAvailable=noWirelessChannelAvailable, altitude=altitude, phyControl=phyControl, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, receiveFreqKHz=receiveFreqKHz, ptpCompliance=ptpCompliance, remoteIPAddress=remoteIPAddress, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, vLANPriorityTableNumber=vLANPriorityTableNumber, tFTPServerPortNumber=tFTPServerPortNumber, tDDSynchronizationStatus=tDDSynchronizationStatus, transmitDataRate=transmitDataRate, versionsGroup=versionsGroup, targetMACAddress=targetMACAddress, frequencyVariant=frequencyVariant, receiveModulationModeDetail=receiveModulationModeDetail, sNMPTrapTableNumber=sNMPTrapTableNumber, subnetMask=subnetMask, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, vLANManagementPriority=vLANManagementPriority, sMTPSourceEmailAddress=sMTPSourceEmailAddress, dataPortStatus=dataPortStatus, notificationsGroup=notificationsGroup, tDDControlGroup=tDDControlGroup, configurationGroup=configurationGroup, dataPortDisabledWarning=dataPortDisabledWarning, incompatibleRegionCodes=incompatibleRegionCodes, dfs=dfs, sNMPTrapIPAddress=sNMPTrapIPAddress, systemClock=systemClock, resetGroup=resetGroup, hardwareVersion=hardwareVersion, sNTPPollInterval=sNTPPollInterval, installStatus=installStatus, cableLoss=cableLoss, linkName=linkName, telecom=telecom, vLANPriorityTableEntry=vLANPriorityTableEntry, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sMTPEnabledMessages=sMTPEnabledMessages, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, syslogControlGroup=syslogControlGroup, regionCodeTrap=regionCodeTrap, hTTPPortNumber=hTTPPortNumber, ethernet=ethernet, sntpGroup=sntpGroup, wirelessLinkAvailability=wirelessLinkAvailability, sNMPTrapPortNumber=sNMPTrapPortNumber, productVariant=productVariant, unitOutOfCalibration=unitOutOfCalibration, sNTPServerPortNumber=sNTPServerPortNumber, groupID=groupID, transmitFreqKHz=transmitFreqKHz, systemReset=systemReset, gatewayIPAddress=gatewayIPAddress, vLANManagementVID=vLANManagementVID, supplementary=supplementary, enabledDiagnosticAlarms=enabledDiagnosticAlarms, syslogLocalWrappedTrap=syslogLocalWrappedTrap, cambium=cambium, tDDControl=tDDControl, hTTPAccessEnabled=hTTPAccessEnabled, hTTPSPortNumber=hTTPSPortNumber, hTTPSAccessEnabled=hTTPSAccessEnabled, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, channelChangeTrap=channelChangeTrap, sNMPPortNumber=sNMPPortNumber, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, syslogControl=syslogControl, vLANPriorityTableIndex=vLANPriorityTableIndex, dataPortWirelessDownAlert=dataPortWirelessDownAlert, versions=versions, alarmsGroup=alarmsGroup, ptp500=ptp500, sNMPv3Enable=sNMPv3Enable, syslogClient=syslogClient, longitude=longitude, encryptionGroup=encryptionGroup, telnetAccessEnabled=telnetAccessEnabled, receivePower=receivePower, daylightSaving=daylightSaving, iPAddress=iPAddress, userTableIndex=userTableIndex, tDDSynchronizationMode=tDDSynchronizationMode, syslogClientTrap=syslogClientTrap, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, userTableNumber=userTableNumber, bandwidthVariant=bandwidthVariant, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, linkLoss=linkLoss, snmpControlGroup=snmpControlGroup, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, sNTPServerIPAddress=sNTPServerIPAddress, sNMPTrapVersion=sNMPTrapVersion, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, accessMethod=accessMethod, installArmState=installArmState, phyControlGroup=phyControlGroup, sNMPEnabledTraps=sNMPEnabledTraps, userPassword=userPassword, linkSymmetry=linkSymmetry, licenceGroup=licenceGroup, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, telecomsCableLength=telecomsCableLength, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, aggregateDataRate=aggregateDataRate, ptpTraps=ptpTraps, tDDSynchronizationAlarm=tDDSynchronizationAlarm, transmitPower=transmitPower, linkModeOptimisation=linkModeOptimisation, aAAControlGroup=aAAControlGroup, supplementaryGroup=supplementaryGroup, telecomGroup=telecomGroup, receiveDataRate=receiveDataRate, dfsTableNumber=dfsTableNumber, syslogStateTrap=syslogStateTrap, telnetPortNumber=telnetPortNumber, siteName=siteName, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, transmitChannel=transmitChannel, channelBandwidth=channelBandwidth)
