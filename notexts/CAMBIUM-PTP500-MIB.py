...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", supplementary=supplementary, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, encryption=encryption, masterSlaveMode=masterSlaveMode, sNTPSyncTrap=sNTPSyncTrap, sMTPEnabledMessages=sMTPEnabledMessages, syslogControlGroup=syslogControlGroup, constantPowerSpectralDensity=constantPowerSpectralDensity, tFTPServerIPAddress=tFTPServerIPAddress, siteName=siteName, pubStatsGroup=pubStatsGroup, syslogStateTrap=syslogStateTrap, incompatibleMasterAndSlave=incompatibleMasterAndSlave, dataPortStatus=dataPortStatus, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, phyControlGroup=phyControlGroup, supplementaryGroup=supplementaryGroup, unitOutOfCalibration=unitOutOfCalibration, syslogControl=syslogControl, eIRP=eIRP, telecomsLoopback=telecomsLoopback, syslogClient=syslogClient, vLANManagementPriority=vLANManagementPriority, syslogState=syslogState, vLANPriorityTableNumber=vLANPriorityTableNumber, dfsTableNumber=dfsTableNumber, sNMPTrapTable=sNMPTrapTable, installArmState=installArmState, userName=userName, licence=licence, dfsPeaks=dfsPeaks, productVariant=productVariant, sNMPTrapIPAddress=sNMPTrapIPAddress, sMTPEmailAlert=sMTPEmailAlert, userRole=userRole, searchState=searchState, ethernetGroup=ethernetGroup, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, ethernetFiberSupport=ethernetFiberSupport, transmitFreqKHz=transmitFreqKHz, ptpTraps=ptpTraps, userEnabled=userEnabled, signalStrengthRatio=signalStrengthRatio, wirelessLinkStatus=wirelessLinkStatus, ptpTrapPrefix=ptpTrapPrefix, dfsTableEntry=dfsTableEntry, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, sNMPPortNumber=sNMPPortNumber, ptpCompliance=ptpCompliance, receiveFreqKHz=receiveFreqKHz, cableLoss=cableLoss, phyStatusGroup=phyStatusGroup, ethernet=ethernet, userPassword=userPassword, hTTPAccessEnabled=hTTPAccessEnabled, sNMPTrapPortNumber=sNMPTrapPortNumber, tDDSynchronizationMode=tDDSynchronizationMode, pubStats=pubStats, hTTPSAccessEnabled=hTTPSAccessEnabled, smtp=smtp, gatewayIPAddress=gatewayIPAddress, wirelessLinkAvailability=wirelessLinkAvailability, vLANPriorityQueueMapping=vLANPriorityQueueMapping, accessMethod=accessMethod, telnetPortNumber=telnetPortNumber, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, regionCodeTrap=regionCodeTrap, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, capacityLicenseMismatch=capacityLicenseMismatch, telecomsInterface=telecomsInterface, bootVersion=bootVersion, longitude=longitude, vLANPriorityTable=vLANPriorityTable, productName=productName, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, enabledDiagnosticAlarms=enabledDiagnosticAlarms, dataPortStatusTrap=dataPortStatusTrap, latitude=latitude, sNMPEnabledTraps=sNMPEnabledTraps, sMTPServerPortNumber=sMTPServerPortNumber, sNMPTrapTableIndex=sNMPTrapTableIndex, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, iPAddress=iPAddress, dataPortAutoMdix=dataPortAutoMdix, linkLoss=linkLoss, reset=reset, aAAControlGroup=aAAControlGroup, encryptionGroup=encryptionGroup, subnetMask=subnetMask, sNTPSync=sNTPSync, snmpControlGroup=snmpControlGroup, dataPortWirelessDownAlert=dataPortWirelessDownAlert, licensedCapacity=licensedCapacity, receiveModulationModeDetail=receiveModulationModeDetail, byteErrorRatio=byteErrorRatio, sNTPServerPortNumber=sNTPServerPortNumber, aggregateDataRate=aggregateDataRate, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, installStatusTrap=installStatusTrap, tDDControlGroup=tDDControlGroup, alarms=alarms, sNMPCommunityString=sNMPCommunityString, receivePower=receivePower, smtpGroup=smtpGroup, ptpGroups=ptpGroups, userTableEntry=userTableEntry, channelBandwidth=channelBandwidth, dfsTableIndex=dfsTableIndex, tDDSynchronizationStatus=tDDSynchronizationStatus, telnetAccessEnabled=telnetAccessEnabled, tDDSynchronizationAlarm=tDDSynchronizationAlarm, ptmp=ptmp, transmitModulationMode=transmitModulationMode, transmitDataRate=transmitDataRate, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, linkModeOptimizationMismatch=linkModeOptimizationMismatch, maximumTransmitPower=maximumTransmitPower, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, dataPortConfigurationMismatch=dataPortConfigurationMismatch, hTTPPortNumber=hTTPPortNumber, telecomsLineCode=telecomsLineCode, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, linkModeOptimisation=linkModeOptimisation, telecomsCableLength=telecomsCableLength, bandwidthVariant=bandwidthVariant, sNMPTrapTableEntry=sNMPTrapTableEntry, installStatus=installStatus, dfsMeans=dfsMeans, vectorError=vectorError, systemReset=systemReset, aAAControl=aAAControl, receiveFreqMHz=receiveFreqMHz, channelChangeTrap=channelChangeTrap, daylightSaving=daylightSaving, sntpGroup=sntpGroup, receiveModulationMode=receiveModulationMode, sNTPServerIPAddress=sNTPServerIPAddress, sNTPPollInterval=sNTPPollInterval, telecom=telecom, groupID=groupID, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, alarmsGroup=alarmsGroup, range=range, managementGroup=managementGroup, hTTPSPortNumber=hTTPSPortNumber, versions=versions, dfsGroup=dfsGroup, timeZone=timeZone, sntp=sntp, sNTPState=sNTPState, cambium=cambium, resetGroup=resetGroup, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sMTPServerIPAddress=sMTPServerIPAddress, ptp=ptp, PYSNMP_MODULE_ID=cambium, management=management, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPTrapTableNumber=sNMPTrapTableNumber, userTableNumber=userTableNumber, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, transmitChannel=transmitChannel, phyControl=phyControl, dfsTable=dfsTable, regionCode=regionCode, installArmStateTrap=installArmStateTrap, snmpControl=snmpControl, ptp500=ptp500, versionsGroup=versionsGroup, transmitFreqMHz=transmitFreqMHz, dfs=dfs, incompatibleRegionCodes=incompatibleRegionCodes, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, syslogClientTrap=syslogClientTrap, phyStatus=phyStatus, remoteMaximumTransmitPower=remoteMaximumTransmitPower, vLANPriorityTableIndex=vLANPriorityTableIndex, transmitPower=transmitPower, sNMPv3Enable=sNMPv3Enable, useVLANForManagementInterfaces=useVLANForManagementInterfaces, dataPortAutoNegotiation=dataPortAutoNegotiation, frequencyVariant=frequencyVariant, encryptionAlgorithm=encryptionAlgorithm, userTableIndex=userTableIndex, vLANPriorityTableEntry=vLANPriorityTableEntry, tFTPServerPortNumber=tFTPServerPortNumber, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, telecomsLoopbackTrap=telecomsLoopbackTrap, antennaGain=antennaGain, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, telecomGroup=telecomGroup, notificationsGroup=notificationsGroup, userTable=userTable, secondarySoftwareVersion=secondarySoftwareVersion, telecomsChannelStatusTrap=telecomsChannelStatusTrap, sNMPTrapVersion=sNMPTrapVersion, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, sNTPLastSync=sNTPLastSync, receiveDataRate=receiveDataRate, configuration=configuration, licenceGroup=licenceGroup, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, dataPortDisabledWarning=dataPortDisabledWarning, altitude=altitude, remoteIPAddress=remoteIPAddress, tDDControl=tDDControl, linkSymmetry=linkSymmetry, vLANManagementVID=vLANManagementVID, telecomsChannelStatus=telecomsChannelStatus, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, targetMACAddress=targetMACAddress, configurationGroup=configurationGroup, sMTPSourceEmailAddress=sMTPSourceEmailAddress, softwareVersion=softwareVersion, hardwareVersion=hardwareVersion, systemClock=systemClock, linkName=linkName, receiveChannel=receiveChannel, noWirelessChannelAvailable=noWirelessChannelAvailable)
