...
# Produced by pysmi-1.1.3 at Sat Nov 20 22:32:18 2021
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sNMPTrapTableIndex=sNMPTrapTableIndex, ptpTraps=ptpTraps, eIRP=eIRP, managementGroup=managementGroup, userEnabled=userEnabled, bootVersion=bootVersion, ptp500=ptp500, telecomsLineCode=telecomsLineCode, supplementaryGroup=supplementaryGroup, incompatibleMasterAndSlave=incompatibleMasterAndSlave, remoteMaximumTransmitPower=remoteMaximumTransmitPower, longitude=longitude, configurationGroup=configurationGroup, sNMPTrapTableNumber=sNMPTrapTableNumber, regionCodeTrap=regionCodeTrap, snmpControlGroup=snmpControlGroup, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, ptpCompliance=ptpCompliance, tFTPServerPortNumber=tFTPServerPortNumber, capacityLicenseMismatch=capacityLicenseMismatch, reset=reset, productVariant=productVariant, telecomsCableLength=telecomsCableLength, sntp=sntp, telecomsChannelStatusTrap=telecomsChannelStatusTrap, gatewayIPAddress=gatewayIPAddress, cambium=cambium, aggregateDataRate=aggregateDataRate, targetMACAddress=targetMACAddress, linkName=linkName, telecomsLoopbackTrap=telecomsLoopbackTrap, installStatus=installStatus, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, transmitModulationMode=transmitModulationMode, bandwidthVariant=bandwidthVariant, sNMPEnabledTraps=sNMPEnabledTraps, encryption=encryption, softwareVersion=softwareVersion, receiveModulationModeDetail=receiveModulationModeDetail, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, dfsTable=dfsTable, tFTPServerIPAddress=tFTPServerIPAddress, latitude=latitude, daylightSaving=daylightSaving, dfsMeans=dfsMeans, antennaGain=antennaGain, range=range, syslogClient=syslogClient, ptpGroups=ptpGroups, sNTPSync=sNTPSync, transmitChannel=transmitChannel, telnetAccessEnabled=telnetAccessEnabled, encryptionGroup=encryptionGroup, ptpTrapPrefix=ptpTrapPrefix, frequencyVariant=frequencyVariant, sNMPTrapTable=sNMPTrapTable, accessMethod=accessMethod, vLANPriorityTable=vLANPriorityTable, secondarySoftwareVersion=secondarySoftwareVersion, smtp=smtp, receiveModulationMode=receiveModulationMode, phyControlGroup=phyControlGroup, noWirelessChannelAvailable=noWirelessChannelAvailable, hardwareVersion=hardwareVersion, sNMPTrapIPAddress=sNMPTrapIPAddress, hTTPSPortNumber=hTTPSPortNumber, ptmp=ptmp, useVLANForManagementInterfaces=useVLANForManagementInterfaces, installArmStateTrap=installArmStateTrap, userTableIndex=userTableIndex, telecomGroup=telecomGroup, receiveFreqMHz=receiveFreqMHz, tDDSynchronizationAlarm=tDDSynchronizationAlarm, byteErrorRatio=byteErrorRatio, sNTPPollInterval=sNTPPollInterval, alarms=alarms, receiveFreqKHz=receiveFreqKHz, hTTPSAccessEnabled=hTTPSAccessEnabled, iPAddress=iPAddress, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, dataPortConfigurationMismatch=dataPortConfigurationMismatch, syslogClientTrap=syslogClientTrap, dataPortAutoMdix=dataPortAutoMdix, constantPowerSpectralDensity=constantPowerSpectralDensity, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, wirelessLinkStatus=wirelessLinkStatus, timeZone=timeZone, pubStatsGroup=pubStatsGroup, unitOutOfCalibration=unitOutOfCalibration, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, linkLoss=linkLoss, subnetMask=subnetMask, incompatibleRegionCodes=incompatibleRegionCodes, ethernetGroup=ethernetGroup, aAAControlGroup=aAAControlGroup, sNTPServerIPAddress=sNTPServerIPAddress, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, phyStatus=phyStatus, installArmState=installArmState, syslogControl=syslogControl, versionsGroup=versionsGroup, sMTPSourceEmailAddress=sMTPSourceEmailAddress, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, masterSlaveMode=masterSlaveMode, receivePower=receivePower, transmitPower=transmitPower, systemReset=systemReset, resetGroup=resetGroup, dataPortStatus=dataPortStatus, notificationsGroup=notificationsGroup, siteName=siteName, licensedCapacity=licensedCapacity, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, vLANPriorityTableEntry=vLANPriorityTableEntry, systemClock=systemClock, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, vLANManagementPriority=vLANManagementPriority, tDDControl=tDDControl, tDDSynchronizationStatus=tDDSynchronizationStatus, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, dfsTableEntry=dfsTableEntry, sMTPEmailAlert=sMTPEmailAlert, dataPortDisabledWarning=dataPortDisabledWarning, vLANManagementVID=vLANManagementVID, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, dfsTableNumber=dfsTableNumber, dataPortWirelessDownAlert=dataPortWirelessDownAlert, sMTPServerPortNumber=sMTPServerPortNumber, sNMPTrapVersion=sNMPTrapVersion, ptp=ptp, groupID=groupID, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, versions=versions, phyControl=phyControl, sntpGroup=sntpGroup, sNMPPortNumber=sNMPPortNumber, enabledDiagnosticAlarms=enabledDiagnosticAlarms, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, linkSymmetry=linkSymmetry, installStatusTrap=installStatusTrap, vLANPriorityTableNumber=vLANPriorityTableNumber, userRole=userRole, transmitDataRate=transmitDataRate, syslogLocalWrappedTrap=syslogLocalWrappedTrap, maximumTransmitPower=maximumTransmitPower, dataPortAutoNegotiation=dataPortAutoNegotiation, vLANPriorityQueueMapping=vLANPriorityQueueMapping, dfsGroup=dfsGroup, ethernetFiberSupport=ethernetFiberSupport, channelChangeTrap=channelChangeTrap, transmitFreqKHz=transmitFreqKHz, signalStrengthRatio=signalStrengthRatio, telecomsChannelStatus=telecomsChannelStatus, sNMPCommunityString=sNMPCommunityString, linkModeOptimisation=linkModeOptimisation, sNMPTrapPortNumber=sNMPTrapPortNumber, transmitFreqMHz=transmitFreqMHz, wirelessLinkAvailability=wirelessLinkAvailability, supplementary=supplementary, licenceGroup=licenceGroup, alarmsGroup=alarmsGroup, remoteIPAddress=remoteIPAddress, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, aAAControl=aAAControl, pubStats=pubStats, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNTPServerPortNumber=sNTPServerPortNumber, syslogState=syslogState, userTable=userTable, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, userTableEntry=userTableEntry, altitude=altitude, userPassword=userPassword, vectorError=vectorError, cableLoss=cableLoss, phyStatusGroup=phyStatusGroup, smtpGroup=smtpGroup, hTTPPortNumber=hTTPPortNumber, searchState=searchState, receiveChannel=receiveChannel, sMTPServerIPAddress=sMTPServerIPAddress, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sMTPEnabledMessages=sMTPEnabledMessages, sNTPState=sNTPState, productName=productName, telnetPortNumber=telnetPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, syslogControlGroup=syslogControlGroup, telecomsLoopback=telecomsLoopback, hTTPAccessEnabled=hTTPAccessEnabled, PYSNMP_MODULE_ID=cambium, channelBandwidth=channelBandwidth, regionCode=regionCode, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, syslogStateTrap=syslogStateTrap, configuration=configuration, tDDControlGroup=tDDControlGroup, dfs=dfs, userTableNumber=userTableNumber, licence=licence, userName=userName, telecom=telecom, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, encryptionAlgorithm=encryptionAlgorithm, ethernet=ethernet, sNTPLastSync=sNTPLastSync, sNMPTrapTableEntry=sNMPTrapTableEntry, dfsTableIndex=dfsTableIndex, telecomsInterface=telecomsInterface, sNMPv3Enable=sNMPv3Enable, dfsPeaks=dfsPeaks, vLANPriorityTableIndex=vLANPriorityTableIndex, receiveDataRate=receiveDataRate, sNTPSyncTrap=sNTPSyncTrap, tDDSynchronizationMode=tDDSynchronizationMode, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, snmpControl=snmpControl, management=management, dataPortStatusTrap=dataPortStatusTrap)
