...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", encryptionAlgorithm=encryptionAlgorithm, wirelessLinkAvailability=wirelessLinkAvailability, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, dfsMeans=dfsMeans, telecomsLineCode=telecomsLineCode, tFTPServerPortNumber=tFTPServerPortNumber, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, receiveModulationModeDetail=receiveModulationModeDetail, userTableNumber=userTableNumber, channelChangeTrap=channelChangeTrap, tFTPServerIPAddress=tFTPServerIPAddress, licence=licence, sNMPEnabledTraps=sNMPEnabledTraps, cambium=cambium, installArmState=installArmState, aggregateDataRate=aggregateDataRate, dataPortConfigurationMismatch=dataPortConfigurationMismatch, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPTrapVersion=sNMPTrapVersion, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, receiveFreqKHz=receiveFreqKHz, ethernetGroup=ethernetGroup, frequencyVariant=frequencyVariant, enabledDiagnosticAlarms=enabledDiagnosticAlarms, phyStatusGroup=phyStatusGroup, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, bandwidthVariant=bandwidthVariant, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, productName=productName, softwareVersion=softwareVersion, secondarySoftwareVersion=secondarySoftwareVersion, aAAControlGroup=aAAControlGroup, sNTPLastSync=sNTPLastSync, syslogControl=syslogControl, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, gatewayIPAddress=gatewayIPAddress, aAAControl=aAAControl, dfsTableEntry=dfsTableEntry, PYSNMP_MODULE_ID=cambium, telecomsCableLength=telecomsCableLength, alarmsGroup=alarmsGroup, hardwareVersion=hardwareVersion, vLANManagementPriority=vLANManagementPriority, transmitFreqKHz=transmitFreqKHz, sNTPSync=sNTPSync, hTTPSPortNumber=hTTPSPortNumber, dataPortAutoMdix=dataPortAutoMdix, receivePower=receivePower, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, configuration=configuration, accessMethod=accessMethod, telnetAccessEnabled=telnetAccessEnabled, siteName=siteName, tDDControlGroup=tDDControlGroup, smtp=smtp, transmitPower=transmitPower, dfsTableNumber=dfsTableNumber, dfsTableIndex=dfsTableIndex, phyControl=phyControl, sMTPEmailAlert=sMTPEmailAlert, linkLoss=linkLoss, hTTPAccessEnabled=hTTPAccessEnabled, phyControlGroup=phyControlGroup, sMTPSourceEmailAddress=sMTPSourceEmailAddress, masterSlaveMode=masterSlaveMode, cableLoss=cableLoss, dfsPeaks=dfsPeaks, altitude=altitude, ptpGroups=ptpGroups, syslogControlGroup=syslogControlGroup, searchState=searchState, receiveChannel=receiveChannel, sNMPTrapTableIndex=sNMPTrapTableIndex, smtpGroup=smtpGroup, ptpTrapPrefix=ptpTrapPrefix, versions=versions, iPAddress=iPAddress, syslogClientTrap=syslogClientTrap, productVariant=productVariant, linkModeOptimisation=linkModeOptimisation, constantPowerSpectralDensity=constantPowerSpectralDensity, longitude=longitude, remoteMaximumTransmitPower=remoteMaximumTransmitPower, incompatibleRegionCodes=incompatibleRegionCodes, sNTPState=sNTPState, dfsTable=dfsTable, installStatus=installStatus, installStatusTrap=installStatusTrap, snmpControlGroup=snmpControlGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, telecomsInterface=telecomsInterface, sMTPEnabledMessages=sMTPEnabledMessages, userTableIndex=userTableIndex, receiveFreqMHz=receiveFreqMHz, sNMPTrapTable=sNMPTrapTable, vLANPriorityTableNumber=vLANPriorityTableNumber, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, telecomsChannelStatus=telecomsChannelStatus, sNTPSyncTrap=sNTPSyncTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, transmitChannel=transmitChannel, sNMPv3Enable=sNMPv3Enable, sNTPServerPortNumber=sNTPServerPortNumber, linkName=linkName, sNMPTrapIPAddress=sNMPTrapIPAddress, antennaGain=antennaGain, transmitModulationMode=transmitModulationMode, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, ptpCompliance=ptpCompliance, userPassword=userPassword, vLANPriorityTableEntry=vLANPriorityTableEntry, eIRP=eIRP, userTableEntry=userTableEntry, tDDSynchronizationMode=tDDSynchronizationMode, targetMACAddress=targetMACAddress, receiveModulationMode=receiveModulationMode, transmitFreqMHz=transmitFreqMHz, dataPortStatusTrap=dataPortStatusTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, sNTPServerIPAddress=sNTPServerIPAddress, byteErrorRatio=byteErrorRatio, encryption=encryption, vLANPriorityQueueMapping=vLANPriorityQueueMapping, pubStatsGroup=pubStatsGroup, notificationsGroup=notificationsGroup, reset=reset, vLANPriorityTable=vLANPriorityTable, licensedCapacity=licensedCapacity, syslogClient=syslogClient, telecomsChannelStatusTrap=telecomsChannelStatusTrap, configurationGroup=configurationGroup, vLANPriorityTableIndex=vLANPriorityTableIndex, phyStatus=phyStatus, userEnabled=userEnabled, bootVersion=bootVersion, managementGroup=managementGroup, encryptionGroup=encryptionGroup, linkSymmetry=linkSymmetry, ptmp=ptmp, sNMPCommunityString=sNMPCommunityString, daylightSaving=daylightSaving, syslogState=syslogState, pubStats=pubStats, capacityLicenseMismatch=capacityLicenseMismatch, dfs=dfs, dataPortAutoNegotiation=dataPortAutoNegotiation, resetGroup=resetGroup, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, syslogStateTrap=syslogStateTrap, tDDSynchronizationAlarm=tDDSynchronizationAlarm, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, management=management, ethernet=ethernet, telecomsLoopback=telecomsLoopback, telecom=telecom, telecomGroup=telecomGroup, licenceGroup=licenceGroup, groupID=groupID, systemReset=systemReset, ethernetFiberSupport=ethernetFiberSupport, dataPortStatus=dataPortStatus, userName=userName, systemClock=systemClock, hTTPSAccessEnabled=hTTPSAccessEnabled, telnetPortNumber=telnetPortNumber, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, subnetMask=subnetMask, remoteIPAddress=remoteIPAddress, dfsGroup=dfsGroup, ptp=ptp, range=range, unitOutOfCalibration=unitOutOfCalibration, sNTPPollInterval=sNTPPollInterval, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, supplementaryGroup=supplementaryGroup, ptpTraps=ptpTraps, noWirelessChannelAvailable=noWirelessChannelAvailable, tDDControl=tDDControl, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, alarms=alarms, timeZone=timeZone, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, snmpControl=snmpControl, channelBandwidth=channelBandwidth, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, dataPortDisabledWarning=dataPortDisabledWarning, sNMPPortNumber=sNMPPortNumber, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, signalStrengthRatio=signalStrengthRatio, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, regionCodeTrap=regionCodeTrap, hTTPPortNumber=hTTPPortNumber, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sNMPTrapPortNumber=sNMPTrapPortNumber, sntp=sntp, wirelessLinkStatus=wirelessLinkStatus, latitude=latitude, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, sNMPTrapTableNumber=sNMPTrapTableNumber, vLANManagementVID=vLANManagementVID, vectorError=vectorError, receiveDataRate=receiveDataRate, installArmStateTrap=installArmStateTrap, telecomsLoopbackTrap=telecomsLoopbackTrap, sMTPServerIPAddress=sMTPServerIPAddress, sntpGroup=sntpGroup, transmitDataRate=transmitDataRate, supplementary=supplementary, ptp500=ptp500, maximumTransmitPower=maximumTransmitPower, sMTPServerPortNumber=sMTPServerPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, tDDSynchronizationStatus=tDDSynchronizationStatus, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, regionCode=regionCode, versionsGroup=versionsGroup, userTable=userTable, userRole=userRole, useVLANForManagementInterfaces=useVLANForManagementInterfaces, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap)
