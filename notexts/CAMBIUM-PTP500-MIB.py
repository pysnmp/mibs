...
ObjectIdentity, enterprises, MibIdentifier, Counter64, Unsigned32, IpAddress, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Counter32, iso, ModuleIdentity, Integer32, Bits, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "enterprises", "MibIdentifier", "Counter64", "Unsigned32", "IpAddress", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Counter32", "iso", "ModuleIdentity", "Integer32", "Bits", "Gauge32")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", latitude=latitude, dfs=dfs, tDDSynchronizationMode=tDDSynchronizationMode, userTable=userTable, gatewayIPAddress=gatewayIPAddress, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, productName=productName, ptpTraps=ptpTraps, sMTPEmailAlert=sMTPEmailAlert, ptp=ptp, management=management, channelBandwidth=channelBandwidth, tDDSynchronizationAlarm=tDDSynchronizationAlarm, dataPortDisabledWarning=dataPortDisabledWarning, telecomGroup=telecomGroup, regionCode=regionCode, siteName=siteName, sMTPServerIPAddress=sMTPServerIPAddress, sNMPCommunityString=sNMPCommunityString, softwareVersion=softwareVersion, iPAddress=iPAddress, dfsTableIndex=dfsTableIndex, bandwidthVariant=bandwidthVariant, installArmState=installArmState, sNMPTrapTable=sNMPTrapTable, dfsTableNumber=dfsTableNumber, secondarySoftwareVersion=secondarySoftwareVersion, receiveFreqMHz=receiveFreqMHz, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, bootVersion=bootVersion, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, hTTPSPortNumber=hTTPSPortNumber, tDDControl=tDDControl, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, transmitFreqKHz=transmitFreqKHz, pubStats=pubStats, sNMPPortNumber=sNMPPortNumber, sMTPEnabledMessages=sMTPEnabledMessages, maximumTransmitPower=maximumTransmitPower, ethernetGroup=ethernetGroup, resetGroup=resetGroup, aAAControlGroup=aAAControlGroup, telecom=telecom, ethernet=ethernet, aggregateDataRate=aggregateDataRate, timeZone=timeZone, pubStatsGroup=pubStatsGroup, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, vLANPriorityTableEntry=vLANPriorityTableEntry, supplementary=supplementary, accessMethod=accessMethod, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sntpGroup=sntpGroup, receiveModulationModeDetail=receiveModulationModeDetail, userTableIndex=userTableIndex, vectorError=vectorError, userName=userName, snmpControl=snmpControl, syslogState=syslogState, telecomsCableLength=telecomsCableLength, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sNMPEnabledTraps=sNMPEnabledTraps, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, userTableEntry=userTableEntry, linkModeOptimizationMismatch=linkModeOptimizationMismatch, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, transmitDataRate=transmitDataRate, vLANPriorityTableNumber=vLANPriorityTableNumber, vLANPriorityTableIndex=vLANPriorityTableIndex, configurationGroup=configurationGroup, transmitChannel=transmitChannel, unitOutOfCalibration=unitOutOfCalibration, versions=versions, receiveDataRate=receiveDataRate, sNMPTrapTableIndex=sNMPTrapTableIndex, sNTPServerIPAddress=sNTPServerIPAddress, cableLoss=cableLoss, eIRP=eIRP, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, receiveFreqKHz=receiveFreqKHz, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, userRole=userRole, vLANPriorityTable=vLANPriorityTable, installArmStateTrap=installArmStateTrap, installStatus=installStatus, telnetPortNumber=telnetPortNumber, sNMPTrapPortNumber=sNMPTrapPortNumber, ptpTrapPrefix=ptpTrapPrefix, tFTPServerPortNumber=tFTPServerPortNumber, signalStrengthRatio=signalStrengthRatio, dataPortAutoNegotiation=dataPortAutoNegotiation, telecomsChannelStatusTrap=telecomsChannelStatusTrap, smtpGroup=smtpGroup, tFTPServerIPAddress=tFTPServerIPAddress, sNTPPollInterval=sNTPPollInterval, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, hTTPAccessEnabled=hTTPAccessEnabled, telecomsLoopbackTrap=telecomsLoopbackTrap, targetMACAddress=targetMACAddress, transmitPower=transmitPower, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, sNTPState=sNTPState, hardwareVersion=hardwareVersion, tDDControlGroup=tDDControlGroup, useVLANForManagementInterfaces=useVLANForManagementInterfaces, linkName=linkName, ptp500=ptp500, supplementaryGroup=supplementaryGroup, phyStatusGroup=phyStatusGroup, installStatusTrap=installStatusTrap, masterSlaveMode=masterSlaveMode, hTTPSAccessEnabled=hTTPSAccessEnabled, configuration=configuration, incompatibleRegionCodes=incompatibleRegionCodes, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, transmitFreqMHz=transmitFreqMHz, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, ptpCompliance=ptpCompliance, snmpControlGroup=snmpControlGroup, telecomsLoopback=telecomsLoopback, systemReset=systemReset, frequencyVariant=frequencyVariant, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, encryption=encryption, sNMPTrapTableEntry=sNMPTrapTableEntry, groupID=groupID, alarms=alarms, reset=reset, altitude=altitude, linkSymmetry=linkSymmetry, ptmp=ptmp, sntp=sntp, ptpGroups=ptpGroups, dfsTableEntry=dfsTableEntry, dataPortStatus=dataPortStatus, hTTPPortNumber=hTTPPortNumber, dfsMeans=dfsMeans, licenceGroup=licenceGroup, productVariant=productVariant, userPassword=userPassword, telecomsLineCode=telecomsLineCode, linkModeOptimisation=linkModeOptimisation, wirelessLinkAvailability=wirelessLinkAvailability, transmitModulationMode=transmitModulationMode, capacityLicenseMismatch=capacityLicenseMismatch, aAAControl=aAAControl, range=range, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, tDDSynchronizationStatus=tDDSynchronizationStatus, dataPortStatusTrap=dataPortStatusTrap, receiveChannel=receiveChannel, licensedCapacity=licensedCapacity, cambium=cambium, sNTPServerPortNumber=sNTPServerPortNumber, searchState=searchState, licence=licence, userEnabled=userEnabled, phyControlGroup=phyControlGroup, sMTPSourceEmailAddress=sMTPSourceEmailAddress, subnetMask=subnetMask, remoteMaximumTransmitPower=remoteMaximumTransmitPower, dataPortAutoMdix=dataPortAutoMdix, syslogStateTrap=syslogStateTrap, enabledDiagnosticAlarms=enabledDiagnosticAlarms, syslogControlGroup=syslogControlGroup, phyStatus=phyStatus, remoteIPAddress=remoteIPAddress, systemClock=systemClock, wirelessLinkStatus=wirelessLinkStatus, phyControl=phyControl, managementGroup=managementGroup, telecomsChannelStatus=telecomsChannelStatus, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, receivePower=receivePower, alarmsGroup=alarmsGroup, antennaGain=antennaGain, encryptionGroup=encryptionGroup, telnetAccessEnabled=telnetAccessEnabled, incompatibleMasterAndSlave=incompatibleMasterAndSlave, constantPowerSpectralDensity=constantPowerSpectralDensity, notificationsGroup=notificationsGroup, sNMPv3Enable=sNMPv3Enable, sNTPSync=sNTPSync, dataPortWirelessDownAlert=dataPortWirelessDownAlert, dataPortConfigurationMismatch=dataPortConfigurationMismatch, smtp=smtp, byteErrorRatio=byteErrorRatio, dfsPeaks=dfsPeaks, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, userTableNumber=userTableNumber, dfsTable=dfsTable, syslogControl=syslogControl, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, syslogClientTrap=syslogClientTrap, encryptionAlgorithm=encryptionAlgorithm, daylightSaving=daylightSaving, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, ethernetFiberSupport=ethernetFiberSupport, versionsGroup=versionsGroup, noWirelessChannelAvailable=noWirelessChannelAvailable, channelChangeTrap=channelChangeTrap, sNMPTrapVersion=sNMPTrapVersion, longitude=longitude, regionCodeTrap=regionCodeTrap, sNTPSyncTrap=sNTPSyncTrap, syslogClient=syslogClient, dfsGroup=dfsGroup, linkLoss=linkLoss, sNMPTrapIPAddress=sNMPTrapIPAddress, PYSNMP_MODULE_ID=cambium, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, receiveModulationMode=receiveModulationMode, vLANManagementPriority=vLANManagementPriority, sNTPLastSync=sNTPLastSync, telecomsInterface=telecomsInterface, vLANManagementVID=vLANManagementVID, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, sNMPTrapTableNumber=sNMPTrapTableNumber, sMTPServerPortNumber=sMTPServerPortNumber, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap)
