...
# Produced by pysmi-1.1.8 at Thu Sep  8 09:25:01 2022
...
mibBuilder.exportSymbols("DKSF-70-6-X-X-1", npIoSinglePulseDuration=npIoSinglePulseDuration, npThermoSensorN=npThermoSensorN, npRelHumTrapDataMemo=npRelHumTrapDataMemo, npReboot=npReboot, npRelHumTrap=npRelHumTrap, npGsmFailed=npGsmFailed, npCurLoopTraps=npCurLoopTraps, npRelayStartReset=npRelayStartReset, npRelHumTrapTempAllChannels=npRelHumTrapTempAllChannels, npGsm=npGsm, npGsmRegistration=npGsmRegistration, npIoTrapLevelLegend=npIoTrapLevelLegend, npIoSinglePulseStart=npIoSinglePulseStart, npRelHumTempValue=npRelHumTempValue, npSmokeEntry=npSmokeEntry, npIoLevelOut=npIoLevelOut, npIoMemo=npIoMemo, npRelHumSafeRangeHigh=npRelHumSafeRangeHigh, npCurLoopTrapR=npCurLoopTrapR, npSmokeTrapSensorN=npSmokeTrapSensorN, npRelHumidity=npRelHumidity, npGsmInfo=npGsmInfo, FixedPoint1000=FixedPoint1000, npRelayMode=npRelayMode, npIoTrapLineN=npIoTrapLineN, npRelHumValue=npRelHumValue, npThermoTraps=npThermoTraps, npThermoHigh=npThermoHigh, npRelHumTrapTempFail=npRelHumTrapTempFail, npIoTrapLevelIn=npIoTrapLevelIn, npCurLoopR=npCurLoopR, npSmokeTrapMemo=npSmokeTrapMemo, uniPingServerSolutionV3=uniPingServerSolutionV3, npRelayEntry=npRelayEntry, npIoLevelIn=npIoLevelIn, npIrCtrl=npIrCtrl, npIoTrapMemo=npIoTrapMemo, npSmokeSensorN=npSmokeSensorN, npThermoTable=npThermoTable, npRelHumTrapDataN=npRelHumTrapDataN, npSmokeTrapStatus=npSmokeTrapStatus, npGsmTrap=npGsmTrap, npIoPulseCounter=npIoPulseCounter, npRelHumSafeRangeLow=npRelHumSafeRangeLow, npThermoTrapHigh=npThermoTrapHigh, npGsmTraps=npGsmTraps, npForcedReboot=npForcedReboot, PYSNMP_MODULE_ID=uniPingServerSolutionV3, npIrReset=npIrReset, npThermoValuePrecise=npThermoValuePrecise, npCurLoopTrapPower=npCurLoopTrapPower, npRelHumTrapTempSafe=npRelHumTrapTempSafe, npSmokeTrapPrefix=npSmokeTrapPrefix, npRelHumTrapAboveSafe=npRelHumTrapAboveSafe, npCurLoopI=npCurLoopI, npRelayState=npRelayState, npCurLoopTrapI=npCurLoopTrapI, npRelHumMemo=npRelHumMemo, npSmokePower=npSmokePower, npCurLoopTrapPrefix=npCurLoopTrapPrefix, npThermoTrapMemo=npThermoTrapMemo, npIoTrap=npIoTrap, npRelHumTrapSafe=npRelHumTrapSafe, npRelHumTrapTempAboveSafe=npRelHumTrapTempAboveSafe, npSmokeTrap=npSmokeTrap, npSmokeMemo=npSmokeMemo, npThermoMemo=npThermoMemo, npSmokeTable=npSmokeTable, npSmokeReset=npSmokeReset, npSmokeStatus=npSmokeStatus, npRelHumTempSafeRangeLow=npRelHumTempSafeRangeLow, npRelHumTrapDataSafeRangeHigh=npRelHumTrapDataSafeRangeHigh, npSoftReboot=npSoftReboot, npRelayN=npRelayN, npRelayTable=npRelayTable, npSmokeTraps=npSmokeTraps, npGsmTrapPrefix=npGsmTrapPrefix, npIrStatus=npIrStatus, npRelHumTrapBelowSafe=npRelHumTrapBelowSafe, npThermoValue=npThermoValue, npRelHumTrapTempBelowSafe=npRelHumTrapTempBelowSafe, npIo=npIo, npThermoTrapSensorN=npThermoTrapSensorN, npCurLoopTable=npCurLoopTable, npSmoke=npSmoke, lightcom=lightcom, npRelHumTrapDataValue=npRelHumTrapDataValue, npIoTrapPrefix=npIoTrapPrefix, npThermoTrapStatus=npThermoTrapStatus, npCurLoopTrapV=npCurLoopTrapV, npCurLoopV=npCurLoopV, npThermoTrap=npThermoTrap, npCurLoopTrapStatus=npCurLoopTrapStatus, npThermoStatus=npThermoStatus, npIrPlayCmd=npIrPlayCmd, npCurLoop=npCurLoop, npRelHumTempSafeRangeHigh=npRelHumTempSafeRangeHigh, npRelHumTrapAllEvents=npRelHumTrapAllEvents, npThermoEntry=npThermoEntry, npCurLoopTrapN=npCurLoopTrapN, npRelHumTrapData=npRelHumTrapData, npThermo=npThermo, npThermoTrapValue=npThermoTrapValue, npRelayMemo=npRelayMemo, npRelHumTable=npRelHumTable, npIoLineN=npIoLineN, npThermoTrapLow=npThermoTrapLow, npRelHumTrapAllChannels=npRelHumTrapAllChannels, npIoTraps=npIoTraps, npRelay=npRelay, npRelHumTrapDataSafeRangeLow=npRelHumTrapDataSafeRangeLow, npRelHumTrapFail=npRelHumTrapFail, npIoTable=npIoTable, npCurLoopEntry=npCurLoopEntry, npRelHumN=npRelHumN, npRelHumTrapTempAllEvents=npRelHumTrapTempAllEvents, npIoEntry=npIoEntry, npRelHumTrapDataStatus=npRelHumTrapDataStatus, npGsmSendSmsUtf8=npGsmSendSmsUtf8, npGsmSendSmsWin1251=npGsmSendSmsWin1251, npTrapEmailTo=npTrapEmailTo, npCurLoopN=npCurLoopN, npIr=npIr, npCurLoopPower=npCurLoopPower, npTrapInfo=npTrapInfo, npResetStack=npResetStack, npCurLoopTrap=npCurLoopTrap, npGsmStrength=npGsmStrength, npRelHumTrapTemp=npRelHumTrapTemp, npThermoTrapPrefix=npThermoTrapPrefix, npCurLoopStatus=npCurLoopStatus, npRelHumStatus=npRelHumStatus, npThermoLow=npThermoLow, npRelHumTempStatus=npRelHumTempStatus, npRelHumEntry=npRelHumEntry)
