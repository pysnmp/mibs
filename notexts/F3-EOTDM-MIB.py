...
VariablePointer, RowStatus, StorageType, TruthValue, TextualConvention, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "VariablePointer", "RowStatus", "StorageType", "TruthValue", "TextualConvention", "DisplayString")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", WtrTime=WtrTime, vcgEntry=vcgEntry, vcgWtrTimer=vcgWtrTimer, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgMemberSoLcasState=vcgMemberSoLcasState, VcgType=VcgType, vcgMemberIndex=vcgMemberIndex, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgMemberRowStatus=vcgMemberRowStatus, f3EotdmConformance=f3EotdmConformance, LcasSoState=LcasSoState, vcgAdminState=vcgAdminState, vcgOperationalState=vcgOperationalState, vcgMemberSkLcasState=vcgMemberSkLcasState, f3EOTDMMIB=f3EOTDMMIB, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, LcasSkState=LcasSkState, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgTable=vcgTable, vcgMemberEntry=vcgMemberEntry, f3EotdmCompliances=f3EotdmCompliances, vcgClearWtrTimer=vcgClearWtrTimer, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgIndex=vcgIndex, vcgMemberTable=vcgMemberTable, vcgType=vcgType, f3EotdmGroups=f3EotdmGroups, CtrlState=CtrlState, vcgIfIndex=vcgIfIndex, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, vcgHoldOffTimer=vcgHoldOffTimer, f3EotdmCompliance=f3EotdmCompliance, MstState=MstState, vcgLcasEnabled=vcgLcasEnabled, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgSecondaryState=vcgSecondaryState, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, f3EotdmObjects=f3EotdmObjects, HoldOffTime=HoldOffTime, vcgMemberIfIndex=vcgMemberIfIndex, vcgRowStatus=vcgRowStatus)
