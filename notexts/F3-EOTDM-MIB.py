...
DisplayString, TextualConvention, RowStatus, TruthValue, VariablePointer, StorageType = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TextualConvention", "RowStatus", "TruthValue", "VariablePointer", "StorageType")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgMemberSoLcasState=vcgMemberSoLcasState, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, WtrTime=WtrTime, MstState=MstState, vcgAdminState=vcgAdminState, vcgWtrTimer=vcgWtrTimer, vcgRowStatus=vcgRowStatus, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgLcasEnabled=vcgLcasEnabled, vcgMemberIndex=vcgMemberIndex, f3EotdmCompliance=f3EotdmCompliance, VcgType=VcgType, CtrlState=CtrlState, HoldOffTime=HoldOffTime, f3EotdmCompliances=f3EotdmCompliances, f3EotdmObjects=f3EotdmObjects, LcasSkState=LcasSkState, vcgMemberTable=vcgMemberTable, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, vcgIndex=vcgIndex, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgSecondaryState=vcgSecondaryState, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgMemberSkLcasState=vcgMemberSkLcasState, f3EotdmConformance=f3EotdmConformance, vcgTable=vcgTable, LcasSoState=LcasSoState, vcgType=vcgType, vcgMemberIfIndex=vcgMemberIfIndex, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgMemberEntry=vcgMemberEntry, vcgEntry=vcgEntry, vcgOperationalState=vcgOperationalState, vcgIfIndex=vcgIfIndex, vcgMemberRowStatus=vcgMemberRowStatus, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, vcgHoldOffTimer=vcgHoldOffTimer, PYSNMP_MODULE_ID=f3EOTDMMIB, f3EotdmGroups=f3EotdmGroups, f3EOTDMMIB=f3EOTDMMIB, vcgClearWtrTimer=vcgClearWtrTimer, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst)
