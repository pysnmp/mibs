...
TruthValue, StorageType, VariablePointer, RowStatus, DisplayString, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "StorageType", "VariablePointer", "RowStatus", "DisplayString", "TextualConvention")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", PYSNMP_MODULE_ID=f3EOTDMMIB, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, f3EotdmGroups=f3EotdmGroups, LcasSkState=LcasSkState, vcgType=vcgType, vcgMemberIndex=vcgMemberIndex, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, LcasSoState=LcasSoState, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgIndex=vcgIndex, f3EotdmConformance=f3EotdmConformance, f3EOTDMMIB=f3EOTDMMIB, VcgType=VcgType, HoldOffTime=HoldOffTime, f3EotdmCompliances=f3EotdmCompliances, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, CtrlState=CtrlState, vcgMemberIfIndex=vcgMemberIfIndex, WtrTime=WtrTime, vcgMemberEntry=vcgMemberEntry, vcgAdminState=vcgAdminState, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgOperationalState=vcgOperationalState, vcgMemberTable=vcgMemberTable, vcgRowStatus=vcgRowStatus, vcgEntry=vcgEntry, f3EotdmObjects=f3EotdmObjects, vcgTable=vcgTable, vcgIfIndex=vcgIfIndex, vcgLcasEnabled=vcgLcasEnabled, MstState=MstState, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgClearWtrTimer=vcgClearWtrTimer, vcgMemberSoLcasState=vcgMemberSoLcasState, f3EotdmCompliance=f3EotdmCompliance, vcgWtrTimer=vcgWtrTimer, vcgSecondaryState=vcgSecondaryState, vcgHoldOffTimer=vcgHoldOffTimer, vcgMemberRowStatus=vcgMemberRowStatus, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl)
