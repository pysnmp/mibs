...
mibBuilder.exportSymbols("F3-EOTDM-MIB", PYSNMP_MODULE_ID=f3EOTDMMIB, vcgMemberEntry=vcgMemberEntry, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgClearWtrTimer=vcgClearWtrTimer, f3EotdmCompliance=f3EotdmCompliance, f3EotdmGroups=f3EotdmGroups, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, f3EOTDMMIB=f3EOTDMMIB, VcgType=VcgType, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgAdminState=vcgAdminState, LcasSkState=LcasSkState, f3EotdmConformance=f3EotdmConformance, vcgMemberSoLcasState=vcgMemberSoLcasState, LcasSoState=LcasSoState, vcgRowStatus=vcgRowStatus, vcgMemberRowStatus=vcgMemberRowStatus, MstState=MstState, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgIndex=vcgIndex, vcgWtrTimer=vcgWtrTimer, vcgMemberIndex=vcgMemberIndex, vcgLcasEnabled=vcgLcasEnabled, vcgOperationalState=vcgOperationalState, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgHoldOffTimer=vcgHoldOffTimer, vcgMemberTable=vcgMemberTable, HoldOffTime=HoldOffTime, vcgType=vcgType, WtrTime=WtrTime, vcgMemberIfIndex=vcgMemberIfIndex, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, f3EotdmObjects=f3EotdmObjects, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgTable=vcgTable, vcgIfIndex=vcgIfIndex, vcgEntry=vcgEntry, f3EotdmCompliances=f3EotdmCompliances, CtrlState=CtrlState, vcgSecondaryState=vcgSecondaryState)
