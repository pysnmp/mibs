...
StorageType, VariablePointer, TruthValue, DisplayString, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "StorageType", "VariablePointer", "TruthValue", "DisplayString", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgLcasEnabled=vcgLcasEnabled, f3EotdmObjects=f3EotdmObjects, vcgWtrTimer=vcgWtrTimer, f3EOTDMMIB=f3EOTDMMIB, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgMemberIfIndex=vcgMemberIfIndex, vcgRowStatus=vcgRowStatus, LcasSkState=LcasSkState, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgMemberIndex=vcgMemberIndex, vcgClearWtrTimer=vcgClearWtrTimer, LcasSoState=LcasSoState, MstState=MstState, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, CtrlState=CtrlState, vcgMemberEntry=vcgMemberEntry, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgOperationalState=vcgOperationalState, f3EotdmConformance=f3EotdmConformance, vcgMemberTable=vcgMemberTable, vcgTable=vcgTable, vcgAdminState=vcgAdminState, vcgSecondaryState=vcgSecondaryState, vcgEntry=vcgEntry, f3EotdmGroups=f3EotdmGroups, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgHoldOffTimer=vcgHoldOffTimer, f3EotdmCompliance=f3EotdmCompliance, vcgIndex=vcgIndex, vcgType=vcgType, vcgMemberRowStatus=vcgMemberRowStatus, f3EotdmCompliances=f3EotdmCompliances, HoldOffTime=HoldOffTime, f3EotdmObjectGroup=f3EotdmObjectGroup, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, VcgType=VcgType, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, WtrTime=WtrTime, vcgIfIndex=vcgIfIndex)
