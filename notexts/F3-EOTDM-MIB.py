...
TruthValue, StorageType, TextualConvention, VariablePointer, RowStatus, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "StorageType", "TextualConvention", "VariablePointer", "RowStatus", "DisplayString")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", WtrTime=WtrTime, MstState=MstState, vcgOperationalState=vcgOperationalState, vcgMemberRowStatus=vcgMemberRowStatus, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgMemberIndex=vcgMemberIndex, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgEntry=vcgEntry, f3EOTDMMIB=f3EOTDMMIB, vcgMemberTable=vcgMemberTable, CtrlState=CtrlState, vcgWtrTimer=vcgWtrTimer, vcgMemberIfIndex=vcgMemberIfIndex, f3EotdmCompliances=f3EotdmCompliances, vcgSecondaryState=vcgSecondaryState, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgHoldOffTimer=vcgHoldOffTimer, LcasSkState=LcasSkState, vcgIfIndex=vcgIfIndex, f3EotdmObjects=f3EotdmObjects, HoldOffTime=HoldOffTime, VcgType=VcgType, f3EotdmCompliance=f3EotdmCompliance, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgMemberEntry=vcgMemberEntry, vcgAdminState=vcgAdminState, f3EotdmConformance=f3EotdmConformance, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgClearWtrTimer=vcgClearWtrTimer, vcgIndex=vcgIndex, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgLcasEnabled=vcgLcasEnabled, vcgMemberSoSendSeq=vcgMemberSoSendSeq, LcasSoState=LcasSoState, vcgRowStatus=vcgRowStatus, vcgType=vcgType, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgTable=vcgTable, f3EotdmGroups=f3EotdmGroups)
