...
TextualConvention, RowStatus, StorageType, VariablePointer, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "StorageType", "VariablePointer", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgLcasEnabled=vcgLcasEnabled, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgWtrTimer=vcgWtrTimer, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgOperationalState=vcgOperationalState, vcgMemberEntry=vcgMemberEntry, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgTable=vcgTable, f3EotdmObjectGroup=f3EotdmObjectGroup, CtrlState=CtrlState, vcgType=vcgType, vcgMemberIndex=vcgMemberIndex, vcgAdminState=vcgAdminState, f3EotdmConformance=f3EotdmConformance, vcgEntry=vcgEntry, f3EotdmCompliances=f3EotdmCompliances, vcgMemberIfIndex=vcgMemberIfIndex, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgSecondaryState=vcgSecondaryState, vcgIndex=vcgIndex, vcgIfIndex=vcgIfIndex, MstState=MstState, vcgMemberRowStatus=vcgMemberRowStatus, vcgMemberTable=vcgMemberTable, f3EotdmGroups=f3EotdmGroups, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, LcasSkState=LcasSkState, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, WtrTime=WtrTime, f3EotdmCompliance=f3EotdmCompliance, VcgType=VcgType, vcgClearWtrTimer=vcgClearWtrTimer, vcgRowStatus=vcgRowStatus, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, f3EOTDMMIB=f3EOTDMMIB, HoldOffTime=HoldOffTime, f3EotdmObjects=f3EotdmObjects, vcgMemberSoSendSeq=vcgMemberSoSendSeq, LcasSoState=LcasSoState, vcgHoldOffTimer=vcgHoldOffTimer)
