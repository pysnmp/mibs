...
TextualConvention, TruthValue, RowStatus, StorageType, VariablePointer, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "RowStatus", "StorageType", "VariablePointer", "DisplayString")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgMemberTable=vcgMemberTable, f3EotdmObjects=f3EotdmObjects, vcgAdminState=vcgAdminState, vcgOperationalState=vcgOperationalState, CtrlState=CtrlState, vcgType=vcgType, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, LcasSoState=LcasSoState, vcgWtrTimer=vcgWtrTimer, LcasSkState=LcasSkState, vcgHoldOffTimer=vcgHoldOffTimer, vcgMemberEntry=vcgMemberEntry, VcgType=VcgType, f3EotdmCompliance=f3EotdmCompliance, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgSecondaryState=vcgSecondaryState, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgIfIndex=vcgIfIndex, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgClearWtrTimer=vcgClearWtrTimer, vcgMemberRowStatus=vcgMemberRowStatus, MstState=MstState, f3EOTDMMIB=f3EOTDMMIB, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgRowStatus=vcgRowStatus, vcgMemberIndex=vcgMemberIndex, vcgTable=vcgTable, vcgEntry=vcgEntry, vcgMemberIfIndex=vcgMemberIfIndex, HoldOffTime=HoldOffTime, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, f3EotdmCompliances=f3EotdmCompliances, f3EotdmGroups=f3EotdmGroups, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgIndex=vcgIndex, vcgLcasEnabled=vcgLcasEnabled, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, f3EotdmConformance=f3EotdmConformance, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgMemberSkLcasState=vcgMemberSkLcasState, WtrTime=WtrTime)
