...
RowStatus, StorageType, TruthValue, VariablePointer, DisplayString, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "StorageType", "TruthValue", "VariablePointer", "DisplayString", "TextualConvention")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", f3EotdmObjects=f3EotdmObjects, LcasSkState=LcasSkState, HoldOffTime=HoldOffTime, vcgEntry=vcgEntry, vcgSecondaryState=vcgSecondaryState, vcgIfIndex=vcgIfIndex, vcgOperationalState=vcgOperationalState, vcgIndex=vcgIndex, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgHoldOffTimer=vcgHoldOffTimer, f3EotdmConformance=f3EotdmConformance, vcgMemberIfIndex=vcgMemberIfIndex, f3EotdmCompliances=f3EotdmCompliances, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, MstState=MstState, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, f3EOTDMMIB=f3EOTDMMIB, vcgTable=vcgTable, LcasSoState=LcasSoState, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, WtrTime=WtrTime, vcgAdminState=vcgAdminState, vcgMemberTable=vcgMemberTable, vcgMemberEntry=vcgMemberEntry, vcgWtrTimer=vcgWtrTimer, f3EotdmGroups=f3EotdmGroups, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, f3EotdmObjectGroup=f3EotdmObjectGroup, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgMemberIndex=vcgMemberIndex, f3EotdmCompliance=f3EotdmCompliance, VcgType=VcgType, vcgLcasEnabled=vcgLcasEnabled, vcgMemberRowStatus=vcgMemberRowStatus, vcgRowStatus=vcgRowStatus, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgClearWtrTimer=vcgClearWtrTimer, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, vcgMemberSoSendSeq=vcgMemberSoSendSeq, CtrlState=CtrlState, vcgType=vcgType, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst)
