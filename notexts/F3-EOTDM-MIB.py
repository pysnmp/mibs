...
StorageType, TruthValue, TextualConvention, RowStatus, DisplayString, VariablePointer = mibBuilder.importSymbols("SNMPv2-TC", "StorageType", "TruthValue", "TextualConvention", "RowStatus", "DisplayString", "VariablePointer")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgMemberRowStatus=vcgMemberRowStatus, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgIfIndex=vcgIfIndex, f3EotdmObjects=f3EotdmObjects, f3EotdmConformance=f3EotdmConformance, LcasSoState=LcasSoState, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgLcasEnabled=vcgLcasEnabled, f3EOTDMMIB=f3EOTDMMIB, vcgType=vcgType, LcasSkState=LcasSkState, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, f3EotdmObjectGroup=f3EotdmObjectGroup, VcgType=VcgType, CtrlState=CtrlState, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, vcgSecondaryState=vcgSecondaryState, vcgMemberEntry=vcgMemberEntry, HoldOffTime=HoldOffTime, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgIndex=vcgIndex, vcgTable=vcgTable, vcgEntry=vcgEntry, vcgOperationalState=vcgOperationalState, vcgMemberIndex=vcgMemberIndex, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgMemberIfIndex=vcgMemberIfIndex, f3EotdmCompliance=f3EotdmCompliance, vcgHoldOffTimer=vcgHoldOffTimer, vcgWtrTimer=vcgWtrTimer, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, vcgRowStatus=vcgRowStatus, WtrTime=WtrTime, MstState=MstState, vcgMemberTable=vcgMemberTable, f3EotdmCompliances=f3EotdmCompliances, vcgClearWtrTimer=vcgClearWtrTimer, vcgMemberSkLcasState=vcgMemberSkLcasState, f3EotdmGroups=f3EotdmGroups, vcgAdminState=vcgAdminState)
