...
VariablePointer, RowStatus, TruthValue, StorageType, DisplayString, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "VariablePointer", "RowStatus", "TruthValue", "StorageType", "DisplayString", "TextualConvention")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", MstState=MstState, VcgType=VcgType, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgAdminState=vcgAdminState, LcasSoState=LcasSoState, vcgHoldOffTimer=vcgHoldOffTimer, vcgTable=vcgTable, f3EOTDMMIB=f3EOTDMMIB, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgType=vcgType, vcgMemberEntry=vcgMemberEntry, f3EotdmConformance=f3EotdmConformance, vcgMemberSoSendSeq=vcgMemberSoSendSeq, f3EotdmObjects=f3EotdmObjects, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgLcasEnabled=vcgLcasEnabled, vcgMemberTable=vcgMemberTable, f3EotdmGroups=f3EotdmGroups, vcgSecondaryState=vcgSecondaryState, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgMemberIfIndex=vcgMemberIfIndex, vcgMemberIndex=vcgMemberIndex, vcgOperationalState=vcgOperationalState, vcgRowStatus=vcgRowStatus, vcgEntry=vcgEntry, f3EotdmCompliance=f3EotdmCompliance, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgIndex=vcgIndex, LcasSkState=LcasSkState, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, CtrlState=CtrlState, vcgMemberRowStatus=vcgMemberRowStatus, f3EotdmCompliances=f3EotdmCompliances, WtrTime=WtrTime, vcgIfIndex=vcgIfIndex, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, HoldOffTime=HoldOffTime, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, vcgWtrTimer=vcgWtrTimer, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgClearWtrTimer=vcgClearWtrTimer)
