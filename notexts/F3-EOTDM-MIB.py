...
TextualConvention, VariablePointer, TruthValue, DisplayString, StorageType, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "VariablePointer", "TruthValue", "DisplayString", "StorageType", "RowStatus")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgSecondaryState=vcgSecondaryState, CtrlState=CtrlState, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgType=vcgType, MstState=MstState, vcgLcasEnabled=vcgLcasEnabled, vcgWtrTimer=vcgWtrTimer, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgMemberIndex=vcgMemberIndex, f3EOTDMMIB=f3EOTDMMIB, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgClearWtrTimer=vcgClearWtrTimer, vcgOperationalState=vcgOperationalState, WtrTime=WtrTime, f3EotdmCompliances=f3EotdmCompliances, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgTable=vcgTable, LcasSoState=LcasSoState, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgAdminState=vcgAdminState, vcgIndex=vcgIndex, vcgRowStatus=vcgRowStatus, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, vcgIfIndex=vcgIfIndex, VcgType=VcgType, vcgMemberTable=vcgMemberTable, vcgMemberIfIndex=vcgMemberIfIndex, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, HoldOffTime=HoldOffTime, f3EotdmObjects=f3EotdmObjects, vcgEntry=vcgEntry, f3EotdmCompliance=f3EotdmCompliance, vcgMemberRowStatus=vcgMemberRowStatus, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, LcasSkState=LcasSkState, vcgHoldOffTimer=vcgHoldOffTimer, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgMemberEntry=vcgMemberEntry, vcgMemberSkLcasState=vcgMemberSkLcasState, f3EotdmGroups=f3EotdmGroups, f3EotdmConformance=f3EotdmConformance)
