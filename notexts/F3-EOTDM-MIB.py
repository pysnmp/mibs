...
DisplayString, TextualConvention, StorageType, VariablePointer, RowStatus, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TextualConvention", "StorageType", "VariablePointer", "RowStatus", "TruthValue")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, f3EOTDMMIB=f3EOTDMMIB, vcgSecondaryState=vcgSecondaryState, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, f3EotdmConformance=f3EotdmConformance, vcgIfIndex=vcgIfIndex, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, LcasSkState=LcasSkState, vcgMemberIfIndex=vcgMemberIfIndex, vcgMemberSoLcasState=vcgMemberSoLcasState, f3EotdmGroups=f3EotdmGroups, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgIndex=vcgIndex, PYSNMP_MODULE_ID=f3EOTDMMIB, f3EotdmObjects=f3EotdmObjects, LcasSoState=LcasSoState, HoldOffTime=HoldOffTime, vcgClearWtrTimer=vcgClearWtrTimer, vcgLcasEnabled=vcgLcasEnabled, WtrTime=WtrTime, vcgMemberRowStatus=vcgMemberRowStatus, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, vcgMemberIndex=vcgMemberIndex, CtrlState=CtrlState, vcgWtrTimer=vcgWtrTimer, f3EotdmCompliance=f3EotdmCompliance, MstState=MstState, vcgMemberTable=vcgMemberTable, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgType=vcgType, f3EotdmCompliances=f3EotdmCompliances, vcgAdminState=vcgAdminState, vcgRowStatus=vcgRowStatus, vcgMemberEntry=vcgMemberEntry, VcgType=VcgType, vcgTable=vcgTable, vcgHoldOffTimer=vcgHoldOffTimer, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgOperationalState=vcgOperationalState, vcgEntry=vcgEntry, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort)
