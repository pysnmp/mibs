...
TextualConvention, TruthValue, VariablePointer, RowStatus, DisplayString, StorageType = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "VariablePointer", "RowStatus", "DisplayString", "StorageType")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, f3EotdmCompliances=f3EotdmCompliances, PYSNMP_MODULE_ID=f3EOTDMMIB, f3EotdmConformance=f3EotdmConformance, LcasSoState=LcasSoState, vcgMemberRowStatus=vcgMemberRowStatus, vcgOperationalState=vcgOperationalState, VcgType=VcgType, vcgWtrTimer=vcgWtrTimer, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgMemberIndex=vcgMemberIndex, vcgMemberIfIndex=vcgMemberIfIndex, vcgMemberSoSendSeq=vcgMemberSoSendSeq, LcasSkState=LcasSkState, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, f3EotdmCompliance=f3EotdmCompliance, vcgEntry=vcgEntry, vcgIndex=vcgIndex, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, MstState=MstState, vcgHoldOffTimer=vcgHoldOffTimer, WtrTime=WtrTime, vcgClearWtrTimer=vcgClearWtrTimer, f3EOTDMMIB=f3EOTDMMIB, CtrlState=CtrlState, vcgRowStatus=vcgRowStatus, vcgAdminState=vcgAdminState, vcgIfIndex=vcgIfIndex, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgTable=vcgTable, HoldOffTime=HoldOffTime, vcgLcasEnabled=vcgLcasEnabled, vcgMemberTable=vcgMemberTable, vcgMemberEntry=vcgMemberEntry, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgType=vcgType, vcgSecondaryState=vcgSecondaryState, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, f3EotdmObjects=f3EotdmObjects, f3EotdmGroups=f3EotdmGroups)
