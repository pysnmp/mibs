...
PortId, PortDetectedMode, FirmwareSource, EqActionType, CardId, ShelfId, DuplexStatus, SecondaryState, PortMode, UnknownState, PicType, CardSubType, PortType, DiskVolumeUsageLevel, FlowControlMode, CardType, PrimaryState, AdminState = mibBuilder.importSymbols("CADANT-TC", "PortId", "PortDetectedMode", "FirmwareSource", "EqActionType", "CardId", "ShelfId", "DuplexStatus", "SecondaryState", "PortMode", "UnknownState", "PicType", "CardSubType", "PortType", "DiskVolumeUsageLevel", "FlowControlMode", "CardType", "PrimaryState", "AdminState")
...
MacAddress, TimeStamp, DisplayString, RowStatus, TruthValue, TextualConvention, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TimeStamp", "DisplayString", "RowStatus", "TruthValue", "TextualConvention", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTemperatureHighError=cardTemperatureHighError, cardDplxStatusChange=cardDplxStatusChange, cardNumber=cardNumber, cardBootVersion=cardBootVersion, cardTable=cardTable, cardTemperature=cardTemperature, cardTestResultNotification=cardTestResultNotification, cardSwVersion=cardSwVersion, shelfName=shelfName, portDplxStatus=portDplxStatus, cardDetectedChange=cardDetectedChange, cardTestIndex=cardTestIndex, cmResetStatus=cmResetStatus, cardTestSchedule=cardTestSchedule, cardLastResetReason=cardLastResetReason, cmIpAddress=cmIpAddress, diskVolumeNumber=diskVolumeNumber, cmResetInfo=cmResetInfo, portAdminState=portAdminState, cardTrapInh=cardTrapInh, cardTemperatureHighWarn=cardTemperatureHighWarn, portSecStateChange=portSecStateChange, cmRegistrationNotification=cmRegistrationNotification, equipmentShelfGroup=equipmentShelfGroup, diskVolumeSize=diskVolumeSize, auditDescription=auditDescription, cardTestCommand=cardTestCommand, cardAction=cardAction, systemKey=systemKey, portLastChangeTime=portLastChangeTime, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestDescription=cardTestDescription, auditTime=auditTime, auditPassedCount=auditPassedCount, equipmentCardTestGroup=equipmentCardTestGroup, cardPrStateChange=cardPrStateChange, portTable=portTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardEntry=cardEntry, cardSpareGroupId=cardSpareGroupId, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestType=cardTestType, diagTestId=diagTestId, TestResult=TestResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeGroup=diskVolumeGroup, trapSeverity=trapSeverity, diskVolumeUsageLevel=diskVolumeUsageLevel, auditEntry=auditEntry, shelfObjects=shelfObjects, portConnectorId=portConnectorId, errEvRowStatus=errEvRowStatus, cardCpuType=cardCpuType, cardCPLDVersion=cardCPLDVersion, auditCommand=auditCommand, auditTotalFailedCount=auditTotalFailedCount, portRxFlowControlMode=portRxFlowControlMode, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeEntry=diskVolumeEntry, cmResetNotification=cmResetNotification, cmDevice=cmDevice, cardBusSpeed=cardBusSpeed, cardTestTransId=cardTestTransId, errEventId=errEventId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardShelfTestIndex=cardShelfTestIndex, equipmentError=equipmentError, portPrStateChange=portPrStateChange, TestTransactionId=TestTransactionId, cadEquipmentMib=cadEquipmentMib, TestId=TestId, downstreamPowerLoss=downstreamPowerLoss, systemGeneralGroup=systemGeneralGroup, cardSerialNum=cardSerialNum, equipmentCardGroup=equipmentCardGroup, auditAutoScheduling=auditAutoScheduling, cardPicHwVersion=cardPicHwVersion, auditId=auditId, TestSchedule=TestSchedule, cardHwVersion=cardHwVersion, cardDplxStatus=cardDplxStatus, portShelfId=portShelfId, portType=portType, equipmentAuditGroup=equipmentAuditGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPicDetected=cardPicDetected, portId=portId, TestType=TestType, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibConformance=cadEquipmentMibConformance, cardFwUpdateStatus=cardFwUpdateStatus, portBgpId=portBgpId, cardSecStateChange=cardSecStateChange, verboseLevel=verboseLevel, portCardId=portCardId, cardPicSerialNum=cardPicSerialNum, equipmentState=equipmentState, diskVolumeTable=diskVolumeTable, TestCommand=TestCommand, portTrapInh=portTrapInh, equipmentStateGroup=equipmentStateGroup, auditShelfId=auditShelfId, auditFailedCount=auditFailedCount, diskVolumeId=diskVolumeId, diskVolumeDiskSize=diskVolumeDiskSize, cardTestScheduleCommand=cardTestScheduleCommand, diskDriveNumber=diskDriveNumber, cardFlashSize=cardFlashSize, portCardSubType=portCardSubType, cardFwVersion=cardFwVersion, cardSubDetected=cardSubDetected, cardType=cardType, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastChangeTime=cardLastChangeTime, cmResetUpTime=cmResetUpTime, auditResult=auditResult, cardLastBootVersion=cardLastBootVersion, systemGeneral=systemGeneral, portSecState=portSecState, portDocsIfIndex=portDocsIfIndex, portMode=portMode, cardSecState=cardSecState, systemClock=systemClock, portPrState=portPrState, diskVolumeName=diskVolumeName, trapCounter=trapCounter, cardRamSize=cardRamSize, cardTestResultDesc=cardTestResultDesc, diskVolumeTrapInh=diskVolumeTrapInh, equipmentAudit=equipmentAudit, auditLogOutput=auditLogOutput, auditTotalPassedCount=auditTotalPassedCount, cmUChannelID=cmUChannelID, shelfSwVersion=shelfSwVersion, portCurrDsPower=portCurrDsPower, portDplxStatusChange=portDplxStatusChange, diskVolumeShelfId=diskVolumeShelfId, cmMacAddress=cmMacAddress, cardTestResult=cardTestResult, cardLastBootSource=cardLastBootSource, cardTestTable=cardTestTable, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeFileName=diskVolumeFileName, cmPrimarySID=cmPrimarySID, TestScheduleCommand=TestScheduleCommand, fpgaErrorEventTable=fpgaErrorEventTable, cardTestName=cardTestName, equipmentPortGroup=equipmentPortGroup, portMacIfIndex=portMacIfIndex, diskVolumeDriveId=diskVolumeDriveId, cmVendor=cmVendor, auditLogThrottle=auditLogThrottle, portMinDsPower=portMinDsPower, cardTestId=cardTestId, portMaxDsPower=portMaxDsPower, portNumChans=portNumChans, cardUpTime=cardUpTime, auditCardId=auditCardId, portMacAddress=portMacAddress, auditStatus=auditStatus, cardFpgaSource=cardFpgaSource, equipmentTbl=equipmentTbl, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageNotification=diskVolumeUsageNotification, cardShelfId=cardShelfId, cardTestTime=cardTestTime, cardSubType=cardSubType, cardId=cardId, cardDetected=cardDetected, shelfNumber=shelfNumber, cardName=cardName, cardCarrierFwVersion=cardCarrierFwVersion, portAction=portAction, portTxFlowControlMode=portTxFlowControlMode, portDescription=portDescription, eqDiagConfig=eqDiagConfig, diskVolumeCardId=diskVolumeCardId, auditCycleCount=auditCycleCount, portNumber=portNumber, portEntry=portEntry, cardSpareGroupMode=cardSpareGroupMode, remainInDiagMode=remainInDiagMode, auditTable=auditTable, errEvRecoveryEnabled=errEvRecoveryEnabled, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCpuSpeed=cardCpuSpeed, equipmentDiag=equipmentDiag, equipmentTraps=equipmentTraps, portDetectedMode=portDetectedMode, cardAdminState=cardAdminState, cmResetReason=cmResetReason, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, errEvLogLevel=errEvLogLevel, equipmentDiagGroup=equipmentDiagGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardNumPorts=cardNumPorts, auditName=auditName, cardPrState=cardPrState, cmResetClearNotification=cmResetClearNotification, cardTestEntry=cardTestEntry, consoleOutput=consoleOutput, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold)
