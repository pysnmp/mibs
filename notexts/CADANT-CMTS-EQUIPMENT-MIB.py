...
AdminState, CardSubType, PicType, PortType, FirmwareSource, ShelfId, SecondaryState, EqActionType, PrimaryState, FlowControlMode, UnknownState, PortDetectedMode, PortMode, DiskVolumeUsageLevel, PortId, DuplexStatus, CardId, CardType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "CardSubType", "PicType", "PortType", "FirmwareSource", "ShelfId", "SecondaryState", "EqActionType", "PrimaryState", "FlowControlMode", "UnknownState", "PortDetectedMode", "PortMode", "DiskVolumeUsageLevel", "PortId", "DuplexStatus", "CardId", "CardType")
...
DateAndTime, TextualConvention, TruthValue, RowStatus, MacAddress, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TextualConvention", "TruthValue", "RowStatus", "MacAddress", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portSecStateChange=portSecStateChange, cardPicSerialNum=cardPicSerialNum, cardHwVersion=cardHwVersion, errEvLogLevel=errEvLogLevel, shelfNumber=shelfNumber, auditStatus=auditStatus, portMacAddress=portMacAddress, cardFpgaSource=cardFpgaSource, cardSwVersion=cardSwVersion, auditDescription=auditDescription, cmResetNotification=cmResetNotification, cmResetInfo=cmResetInfo, cardEntry=cardEntry, equipmentTraps=equipmentTraps, cardTestTime=cardTestTime, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCarrierSerialNum=cardCarrierSerialNum, cmResetClearNotification=cmResetClearNotification, cardSecState=cardSecState, portNumChans=portNumChans, equipmentCardTestGroup=equipmentCardTestGroup, equipmentError=equipmentError, diskVolumeTable=diskVolumeTable, auditResult=auditResult, auditCommand=auditCommand, portDplxStatusChange=portDplxStatusChange, cardTestTable=cardTestTable, cadEquipmentMib=cadEquipmentMib, cardUpTime=cardUpTime, cardCpuSpeed=cardCpuSpeed, trapSeverity=trapSeverity, TestType=TestType, TestTransactionId=TestTransactionId, cardTestName=cardTestName, cardPrStateChange=cardPrStateChange, cardSecStateChange=cardSecStateChange, cardSpareGroupMode=cardSpareGroupMode, equipmentCardGroup=equipmentCardGroup, portMode=portMode, portBgpId=portBgpId, diskVolumeCardId=diskVolumeCardId, diskVolumeFileName=diskVolumeFileName, cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventGroup=fpgaErrorEventGroup, cardDplxStatus=cardDplxStatus, downstreamPowerLoss=downstreamPowerLoss, cmVendor=cmVendor, cardLastBootSource=cardLastBootSource, auditTable=auditTable, equipmentAudit=equipmentAudit, verboseLevel=verboseLevel, cardLastChangeTime=cardLastChangeTime, diskDriveNumber=diskDriveNumber, diskVolumeName=diskVolumeName, cardTestIndex=cardTestIndex, errEvLoggingEnabled=errEvLoggingEnabled, cardTable=cardTable, cardFwVersion=cardFwVersion, systemGeneral=systemGeneral, TestCommand=TestCommand, consoleOutput=consoleOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditCycleCount=auditCycleCount, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeNumber=diskVolumeNumber, cardNumPorts=cardNumPorts, cardTemperatureHighError=cardTemperatureHighError, equipmentDiag=equipmentDiag, cardDetectedChange=cardDetectedChange, cardBusSpeed=cardBusSpeed, errEvRecoveryEnabled=errEvRecoveryEnabled, systemClock=systemClock, portSecState=portSecState, cardPicHwVersion=cardPicHwVersion, portTxFlowControlMode=portTxFlowControlMode, portConnectorId=portConnectorId, equipmentTbl=equipmentTbl, cardRamSize=cardRamSize, auditTotalFailedCount=auditTotalFailedCount, shelfName=shelfName, portDplxStatus=portDplxStatus, portPrStateChange=portPrStateChange, cardSubDetected=cardSubDetected, diskVolumeId=diskVolumeId, cardTestResultDesc=cardTestResultDesc, diagTestId=diagTestId, shelfObjects=shelfObjects, equipmentState=equipmentState, cardName=cardName, auditShelfId=auditShelfId, cardTemperature=cardTemperature, diskVolumeDriveId=diskVolumeDriveId, cmMacAddress=cmMacAddress, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditFailedCount=auditFailedCount, cardCpuType=cardCpuType, cmIpAddress=cmIpAddress, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestTransId=cardTestTransId, cardCPLDVersion=cardCPLDVersion, cardTestCommand=cardTestCommand, cmPrimarySID=cmPrimarySID, cardCarrierFwVersion=cardCarrierFwVersion, portAction=portAction, cardNumber=cardNumber, diskVolumeShelfId=diskVolumeShelfId, auditCardId=auditCardId, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPrState=cardPrState, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeTrapInh=diskVolumeTrapInh, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestId=cardTestId, systemGeneralGroup=systemGeneralGroup, cardTempNoReportNotification=cardTempNoReportNotification, cardTrapInh=cardTrapInh, portMacIfIndex=portMacIfIndex, shelfSwVersion=shelfSwVersion, cardCarrierHwVersion=cardCarrierHwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, eqDiagConfig=eqDiagConfig, cmDevice=cmDevice, cardTestType=cardTestType, equipmentStateGroup=equipmentStateGroup, cardFlashSize=cardFlashSize, cardTemperatureHighWarn=cardTemperatureHighWarn, cardFwUpdateStatus=cardFwUpdateStatus, portPrState=portPrState, diskVolumeDiskSize=diskVolumeDiskSize, auditAutoScheduling=auditAutoScheduling, auditEntry=auditEntry, diskVolumeEntry=diskVolumeEntry, cardLastResetReason=cardLastResetReason, cardLastBootVersion=cardLastBootVersion, portLastChangeTime=portLastChangeTime, portDocsIfIndex=portDocsIfIndex, cardDplxStatusChange=cardDplxStatusChange, TestId=TestId, portShelfId=portShelfId, cardTempOverHeatNotification=cardTempOverHeatNotification, cardId=cardId, cardTestSchedule=cardTestSchedule, PYSNMP_MODULE_ID=cadEquipmentMib, cardType=cardType, portAdminState=portAdminState, cadEquipmentMibGroup=cadEquipmentMibGroup, portDetectedMode=portDetectedMode, auditLogOutput=auditLogOutput, cardSubType=cardSubType, cardPicDetected=cardPicDetected, fpgaErrorEventTable=fpgaErrorEventTable, equipmentDiagGroup=equipmentDiagGroup, portTrapInh=portTrapInh, auditTotalPassedCount=auditTotalPassedCount, auditId=auditId, portMinDsPower=portMinDsPower, auditLogThrottle=auditLogThrottle, cmResetStatus=cmResetStatus, systemKey=systemKey, cardSpareGroupId=cardSpareGroupId, portTable=portTable, diskVolumeSize=diskVolumeSize, cardTestDescription=cardTestDescription, portTxFlowControlDetected=portTxFlowControlDetected, errEventId=errEventId, auditTime=auditTime, cardAdminState=cardAdminState, cardShelfId=cardShelfId, equipmentAuditGroup=equipmentAuditGroup, auditPassedCount=auditPassedCount, portNumber=portNumber, cardSerialNum=cardSerialNum, equipmentShelfGroup=equipmentShelfGroup, auditName=auditName, portId=portId, portType=portType, cardTestEntry=cardTestEntry, cmResetUpTime=cmResetUpTime, cmUChannelID=cmUChannelID, equipmentPortGroup=equipmentPortGroup, cardTestResultNotification=cardTestResultNotification, portDescription=portDescription, portCardSubType=portCardSubType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portCardId=portCardId, portCurrDsPower=portCurrDsPower, trapCounter=trapCounter, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, remainInDiagMode=remainInDiagMode, cmResetReason=cmResetReason, errEvRowStatus=errEvRowStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, TestScheduleCommand=TestScheduleCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, portMaxDsPower=portMaxDsPower, cardAction=cardAction, TestSchedule=TestSchedule, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardBootVersion=cardBootVersion, cardTestResult=cardTestResult, diskVolumeGroup=diskVolumeGroup, TestResult=TestResult, cardDetected=cardDetected, portEntry=portEntry)
