...
CardId, ShelfId, DiskVolumeUsageLevel, PortMode, FirmwareSource, EqActionType, SecondaryState, DuplexStatus, PortId, PrimaryState, PortType, CardSubType, FlowControlMode, PicType, AdminState, CardType, PortDetectedMode, UnknownState = mibBuilder.importSymbols("CADANT-TC", "CardId", "ShelfId", "DiskVolumeUsageLevel", "PortMode", "FirmwareSource", "EqActionType", "SecondaryState", "DuplexStatus", "PortId", "PrimaryState", "PortType", "CardSubType", "FlowControlMode", "PicType", "AdminState", "CardType", "PortDetectedMode", "UnknownState")
...
TruthValue, MacAddress, DisplayString, RowStatus, DateAndTime, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "MacAddress", "DisplayString", "RowStatus", "DateAndTime", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardId=cardId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSecStateChange=cardSecStateChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestType=cardTestType, diskVolumeEntry=diskVolumeEntry, cardPrState=cardPrState, shelfName=shelfName, eqDiagConfig=eqDiagConfig, portMode=portMode, portMacAddress=portMacAddress, systemGeneralGroup=systemGeneralGroup, cmMacAddress=cmMacAddress, diskVolumeSize=diskVolumeSize, auditAutoScheduling=auditAutoScheduling, equipmentShelfGroup=equipmentShelfGroup, portCardId=portCardId, cardTestName=cardTestName, equipmentStateGroup=equipmentStateGroup, systemGeneral=systemGeneral, equipmentCardGroup=equipmentCardGroup, errEvLoggingEnabled=errEvLoggingEnabled, cardTestTransId=cardTestTransId, portRxFlowControlDetected=portRxFlowControlDetected, portAdminState=portAdminState, equipmentPortGroup=equipmentPortGroup, TestId=TestId, portTrapInh=portTrapInh, portTxFlowControlMode=portTxFlowControlMode, remainInDiagMode=remainInDiagMode, cardSubType=cardSubType, auditShelfId=auditShelfId, portMaxDsPower=portMaxDsPower, errEvRecoveryEnabled=errEvRecoveryEnabled, portPrStateChange=portPrStateChange, cardTestResultNotification=cardTestResultNotification, TestTransactionId=TestTransactionId, auditDescription=auditDescription, diskVolumeId=diskVolumeId, cardName=cardName, portAction=portAction, portTxFlowControlDetected=portTxFlowControlDetected, cardShelfTestIndex=cardShelfTestIndex, cardCarrierSerialNum=cardCarrierSerialNum, cardTestTable=cardTestTable, portDetectedMode=portDetectedMode, cmResetUpTime=cmResetUpTime, equipmentTbl=equipmentTbl, cardFwUpdateStatus=cardFwUpdateStatus, auditTime=auditTime, diskDriveNumber=diskDriveNumber, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portEntry=portEntry, TestScheduleCommand=TestScheduleCommand, cardLastResetReason=cardLastResetReason, diskVolumeShelfId=diskVolumeShelfId, equipmentError=equipmentError, cmPrimarySID=cmPrimarySID, shelfObjects=shelfObjects, cardDetectedChange=cardDetectedChange, portShelfId=portShelfId, portDocsIfIndex=portDocsIfIndex, equipmentDiagGroup=equipmentDiagGroup, cardAction=cardAction, TestResult=TestResult, cardPicSerialNum=cardPicSerialNum, cmResetNotification=cmResetNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDetected=cardDetected, diskVolumeTable=diskVolumeTable, portId=portId, cardType=cardType, cardCpuType=cardCpuType, portTable=portTable, cardDplxStatus=cardDplxStatus, cardTestIndex=cardTestIndex, cmResetInfo=cmResetInfo, errEventId=errEventId, cadEquipmentMib=cadEquipmentMib, diskVolumeNumber=diskVolumeNumber, auditCycleCount=auditCycleCount, cardPrStateChange=cardPrStateChange, cardTempNoReportNotification=cardTempNoReportNotification, auditCardId=auditCardId, cardSpareGroupId=cardSpareGroupId, diagTestId=diagTestId, trapSeverity=trapSeverity, diskVolumeGroup=diskVolumeGroup, cardUpTime=cardUpTime, cmResetReason=cmResetReason, shelfSwVersion=shelfSwVersion, cmDevice=cmDevice, portSecStateChange=portSecStateChange, cardCpuSpeed=cardCpuSpeed, portBgpId=portBgpId, portCardSubType=portCardSubType, diskVolumeUsageLevel=diskVolumeUsageLevel, cardFlashSize=cardFlashSize, trapCounter=trapCounter, auditTotalFailedCount=auditTotalFailedCount, cardSubDetected=cardSubDetected, auditEntry=auditEntry, cardTemperatureHighError=cardTemperatureHighError, fpgaErrorEventGroup=fpgaErrorEventGroup, TestSchedule=TestSchedule, cardShelfId=cardShelfId, auditTable=auditTable, auditStatus=auditStatus, equipmentState=equipmentState, fpgaErrorEventEntry=fpgaErrorEventEntry, cardBootVersion=cardBootVersion, cardTable=cardTable, cardTestCommand=cardTestCommand, cardNumPorts=cardNumPorts, cadEquipmentMibConformance=cadEquipmentMibConformance, cmUChannelID=cmUChannelID, portDescription=portDescription, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditId=auditId, portDplxStatus=portDplxStatus, portNumber=portNumber, cmIpAddress=cmIpAddress, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, systemClock=systemClock, cardRamSize=cardRamSize, auditResult=auditResult, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardAdminState=cardAdminState, portDplxStatusChange=portDplxStatusChange, auditPassedCount=auditPassedCount, cardSecState=cardSecState, cardFpgaSource=cardFpgaSource, cardLastChangeTime=cardLastChangeTime, portSecState=portSecState, auditName=auditName, equipmentTraps=equipmentTraps, cardBusSpeed=cardBusSpeed, cardCPLDVersion=cardCPLDVersion, diskVolumeName=diskVolumeName, equipmentDiag=equipmentDiag, auditFailedCount=auditFailedCount, cardTemperature=cardTemperature, portMacIfIndex=portMacIfIndex, portType=portType, cardDplxStatusChange=cardDplxStatusChange, errEvRowStatus=errEvRowStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCarrierHwVersion=cardCarrierHwVersion, cardPicHwVersion=cardPicHwVersion, shelfNumber=shelfNumber, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeDiskSize=diskVolumeDiskSize, consoleOutput=consoleOutput, auditTotalPassedCount=auditTotalPassedCount, cardLastBootSource=cardLastBootSource, cardTrapInh=cardTrapInh, portLastChangeTime=portLastChangeTime, systemKey=systemKey, TestType=TestType, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentCardTestGroup=equipmentCardTestGroup, errEvLogLevel=errEvLogLevel, cardFwVersion=cardFwVersion, verboseLevel=verboseLevel, cardSerialNum=cardSerialNum, cmResetStatus=cmResetStatus, cmResetClearNotification=cmResetClearNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditCommand=auditCommand, fpgaErrorEventTable=fpgaErrorEventTable, cardTestDescription=cardTestDescription, auditLogThrottle=auditLogThrottle, portConnectorId=portConnectorId, diskVolumeFileName=diskVolumeFileName, auditLogOutput=auditLogOutput, diskVolumeDriveId=diskVolumeDriveId, cardHwVersion=cardHwVersion, portRxFlowControlMode=portRxFlowControlMode, cardTestSchedule=cardTestSchedule, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCarrierFwVersion=cardCarrierFwVersion, portMinDsPower=portMinDsPower, portPrState=portPrState, downstreamPowerLoss=downstreamPowerLoss, TestCommand=TestCommand, cardSwVersion=cardSwVersion, cardLastBootVersion=cardLastBootVersion, cardTestEntry=cardTestEntry, cardTestResultDesc=cardTestResultDesc, cmVendor=cmVendor, diskVolumeCardId=diskVolumeCardId, cardTestTime=cardTestTime, equipmentAudit=equipmentAudit, portNumChans=portNumChans, cardTestResult=cardTestResult, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestId=cardTestId, cardPicDetected=cardPicDetected, cardNumber=cardNumber, cardTestScheduleCommand=cardTestScheduleCommand, cardEntry=cardEntry, portCurrDsPower=portCurrDsPower, equipmentAuditGroup=equipmentAuditGroup, cardSpareGroupMode=cardSpareGroupMode)
