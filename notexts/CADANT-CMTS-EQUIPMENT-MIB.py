...
CardId, PortId, CardSubType, EqActionType, DiskVolumeUsageLevel, DuplexStatus, AdminState, PrimaryState, CardType, ShelfId, PicType, PortMode, FlowControlMode, PortDetectedMode, PortType, SecondaryState, UnknownState, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortId", "CardSubType", "EqActionType", "DiskVolumeUsageLevel", "DuplexStatus", "AdminState", "PrimaryState", "CardType", "ShelfId", "PicType", "PortMode", "FlowControlMode", "PortDetectedMode", "PortType", "SecondaryState", "UnknownState", "FirmwareSource")
...
TimeStamp, TextualConvention, MacAddress, DisplayString, RowStatus, DateAndTime, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TextualConvention", "MacAddress", "DisplayString", "RowStatus", "DateAndTime", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmDevice=cmDevice, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestDescription=cardTestDescription, cardPicSerialNum=cardPicSerialNum, cardLastResetReason=cardLastResetReason, portCardSubType=portCardSubType, systemKey=systemKey, eqDiagConfig=eqDiagConfig, consoleOutput=consoleOutput, auditStatus=auditStatus, diskVolumeDiskSize=diskVolumeDiskSize, trapCounter=trapCounter, portDplxStatusChange=portDplxStatusChange, TestSchedule=TestSchedule, cardEntry=cardEntry, cardTestTime=cardTestTime, cardNumber=cardNumber, portTxFlowControlDetected=portTxFlowControlDetected, fpgaErrorEventEntry=fpgaErrorEventEntry, portDescription=portDescription, PYSNMP_MODULE_ID=cadEquipmentMib, portDplxStatus=portDplxStatus, equipmentDiagGroup=equipmentDiagGroup, portPrStateChange=portPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetInfo=cmResetInfo, cardTrapInh=cardTrapInh, auditName=auditName, TestId=TestId, equipmentPortGroup=equipmentPortGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTemperatureHighWarn=cardTemperatureHighWarn, auditCommand=auditCommand, cardCPLDVersion=cardCPLDVersion, equipmentDiag=equipmentDiag, cardAdminState=cardAdminState, equipmentState=equipmentState, auditId=auditId, remainInDiagMode=remainInDiagMode, cardTestSchedule=cardTestSchedule, auditEntry=auditEntry, cardDetectedChange=cardDetectedChange, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portCardId=portCardId, portTable=portTable, cmRegistrationNotification=cmRegistrationNotification, errEvRowStatus=errEvRowStatus, cardFpgaSource=cardFpgaSource, cardPicHwVersion=cardPicHwVersion, portConnectorId=portConnectorId, cadEquipmentMib=cadEquipmentMib, shelfNumber=shelfNumber, cardDplxStatus=cardDplxStatus, diskVolumeNumber=diskVolumeNumber, cardTestTable=cardTestTable, cmUChannelID=cmUChannelID, equipmentCardTestGroup=equipmentCardTestGroup, cardPicDetected=cardPicDetected, diskVolumeName=diskVolumeName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSpareGroupMode=cardSpareGroupMode, errEvLoggingEnabled=errEvLoggingEnabled, portAdminState=portAdminState, diskVolumeTrapInh=diskVolumeTrapInh, cmResetClearNotification=cmResetClearNotification, auditTotalFailedCount=auditTotalFailedCount, portCurrDsPower=portCurrDsPower, cardTable=cardTable, cardCarrierSerialNum=cardCarrierSerialNum, equipmentError=equipmentError, shelfSwVersion=shelfSwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeEntry=diskVolumeEntry, cardUpTime=cardUpTime, cardId=cardId, cardSubType=cardSubType, cardTestCommand=cardTestCommand, diskVolumeUsageNotification=diskVolumeUsageNotification, portSecStateChange=portSecStateChange, portBgpId=portBgpId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditTable=auditTable, diskVolumeDriveId=diskVolumeDriveId, diskDriveNumber=diskDriveNumber, cmResetUpTime=cmResetUpTime, equipmentStateGroup=equipmentStateGroup, portType=portType, cardFwVersion=cardFwVersion, portEntry=portEntry, fpgaErrorEventTable=fpgaErrorEventTable, cmIpAddress=cmIpAddress, equipmentShelfGroup=equipmentShelfGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardShelfTestIndex=cardShelfTestIndex, auditAutoScheduling=auditAutoScheduling, portPrState=portPrState, cardTestType=cardTestType, cardLastBootVersion=cardLastBootVersion, diskVolumeCardId=diskVolumeCardId, equipmentCardGroup=equipmentCardGroup, cardBootVersion=cardBootVersion, TestTransactionId=TestTransactionId, cardSecState=cardSecState, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestEntry=cardTestEntry, equipmentTraps=equipmentTraps, trapSeverity=trapSeverity, cardTestName=cardTestName, auditLogThrottle=auditLogThrottle, cmPrimarySID=cmPrimarySID, TestScheduleCommand=TestScheduleCommand, systemGeneral=systemGeneral, portMinDsPower=portMinDsPower, cardTemperatureHighError=cardTemperatureHighError, auditDescription=auditDescription, equipmentTbl=equipmentTbl, cardTestResultNotification=cardTestResultNotification, auditTotalPassedCount=auditTotalPassedCount, cmResetReason=cmResetReason, TestCommand=TestCommand, cardTestScheduleCommand=cardTestScheduleCommand, cardRamSize=cardRamSize, portDetectedMode=portDetectedMode, cardTestResult=cardTestResult, cardPrState=cardPrState, auditFailedCount=auditFailedCount, cardTestResultDesc=cardTestResultDesc, cardFlashSize=cardFlashSize, portMaxDsPower=portMaxDsPower, auditShelfId=auditShelfId, cardTestIndex=cardTestIndex, portDocsIfIndex=portDocsIfIndex, portNumber=portNumber, cardAction=cardAction, auditCardId=auditCardId, systemGeneralGroup=systemGeneralGroup, portAction=portAction, diskVolumeSize=diskVolumeSize, portMode=portMode, cardLastChangeTime=cardLastChangeTime, portMacIfIndex=portMacIfIndex, portNumChans=portNumChans, cmResetStatus=cmResetStatus, systemClock=systemClock, portRxFlowControlDetected=portRxFlowControlDetected, cardDetected=cardDetected, TestType=TestType, cardLastBootSource=cardLastBootSource, equipmentAuditGroup=equipmentAuditGroup, diskVolumeFileName=diskVolumeFileName, verboseLevel=verboseLevel, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditCycleCount=auditCycleCount, cardPrStateChange=cardPrStateChange, auditPassedCount=auditPassedCount, cardTempNoReportNotification=cardTempNoReportNotification, cardDplxStatusChange=cardDplxStatusChange, cardCpuType=cardCpuType, cardTestTransId=cardTestTransId, cmResetNotification=cmResetNotification, portSecState=portSecState, auditTime=auditTime, errEventId=errEventId, portTxFlowControlMode=portTxFlowControlMode, diskVolumeShelfId=diskVolumeShelfId, shelfName=shelfName, cardSecStateChange=cardSecStateChange, diagTestId=diagTestId, cardSerialNum=cardSerialNum, cardShelfId=cardShelfId, cardSwVersion=cardSwVersion, cardTemperature=cardTemperature, cardBusSpeed=cardBusSpeed, shelfObjects=shelfObjects, errEvLogLevel=errEvLogLevel, diskVolumeId=diskVolumeId, portRxFlowControlMode=portRxFlowControlMode, cardName=cardName, portMacAddress=portMacAddress, auditLogOutput=auditLogOutput, cardHwVersion=cardHwVersion, portShelfId=portShelfId, cardType=cardType, portTrapInh=portTrapInh, downstreamPowerLoss=downstreamPowerLoss, cardCpuSpeed=cardCpuSpeed, cardCarrierHwVersion=cardCarrierHwVersion, cardTestId=cardTestId, cardSubDetected=cardSubDetected, TestResult=TestResult, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmVendor=cmVendor, diskVolumeUsageLevel=diskVolumeUsageLevel, cmMacAddress=cmMacAddress, equipmentAudit=equipmentAudit, portId=portId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditResult=auditResult, diskVolumeTable=diskVolumeTable, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, portLastChangeTime=portLastChangeTime, cardCarrierFwVersion=cardCarrierFwVersion)
