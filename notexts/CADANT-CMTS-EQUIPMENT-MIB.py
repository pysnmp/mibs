...
DuplexStatus, PortDetectedMode, ShelfId, FlowControlMode, SecondaryState, PortType, DiskVolumeUsageLevel, UnknownState, PortId, PicType, EqActionType, CardId, PortMode, FirmwareSource, CardType, CardSubType, AdminState, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortDetectedMode", "ShelfId", "FlowControlMode", "SecondaryState", "PortType", "DiskVolumeUsageLevel", "UnknownState", "PortId", "PicType", "EqActionType", "CardId", "PortMode", "FirmwareSource", "CardType", "CardSubType", "AdminState", "PrimaryState")
...
DisplayString, DateAndTime, RowStatus, TimeStamp, TextualConvention, TruthValue, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "DateAndTime", "RowStatus", "TimeStamp", "TextualConvention", "TruthValue", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeDiskSize=diskVolumeDiskSize, portTrapInh=portTrapInh, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeId=diskVolumeId, errEvLogLevel=errEvLogLevel, cmRegistrationNotification=cmRegistrationNotification, auditLogOutput=auditLogOutput, auditPassedCount=auditPassedCount, portNumber=portNumber, cardAction=cardAction, cmResetClearNotification=cmResetClearNotification, shelfNumber=shelfNumber, diskVolumeGroup=diskVolumeGroup, cardFpgaSource=cardFpgaSource, cardTestTime=cardTestTime, systemGeneral=systemGeneral, cardSwVersion=cardSwVersion, cardTestEntry=cardTestEntry, cmResetInfo=cmResetInfo, cardShelfId=cardShelfId, cardCPLDVersion=cardCPLDVersion, equipmentStateGroup=equipmentStateGroup, diskVolumeShelfId=diskVolumeShelfId, trapCounter=trapCounter, cardHwVersion=cardHwVersion, TestType=TestType, equipmentError=equipmentError, cardSpareGroupMode=cardSpareGroupMode, equipmentPortGroup=equipmentPortGroup, portAction=portAction, consoleOutput=consoleOutput, TestId=TestId, cardTestScheduleCommand=cardTestScheduleCommand, cmDevice=cmDevice, remainInDiagMode=remainInDiagMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, TestSchedule=TestSchedule, cardLastBootSource=cardLastBootSource, cardPicHwVersion=cardPicHwVersion, cardSpareGroupId=cardSpareGroupId, cmIpAddress=cmIpAddress, cardLastResetReason=cardLastResetReason, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeSize=diskVolumeSize, auditId=auditId, cardPrStateChange=cardPrStateChange, portCardSubType=portCardSubType, fpgaErrorEventTable=fpgaErrorEventTable, fpgaErrorEventGroup=fpgaErrorEventGroup, portType=portType, portCardId=portCardId, portConnectorId=portConnectorId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardFwVersion=cardFwVersion, cardDetected=cardDetected, auditStatus=auditStatus, cardTestTable=cardTestTable, cardSubType=cardSubType, auditTotalFailedCount=auditTotalFailedCount, equipmentCardTestGroup=equipmentCardTestGroup, trapSeverity=trapSeverity, cardBootVersion=cardBootVersion, cardUpTime=cardUpTime, portMacIfIndex=portMacIfIndex, cardTestResultDesc=cardTestResultDesc, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierHwVersion=cardCarrierHwVersion, equipmentDiagGroup=equipmentDiagGroup, equipmentAuditGroup=equipmentAuditGroup, cardTestCommand=cardTestCommand, TestTransactionId=TestTransactionId, auditResult=auditResult, cardLastChangeTime=cardLastChangeTime, portDetectedMode=portDetectedMode, cardTestResultNotification=cardTestResultNotification, cmVendor=cmVendor, cardTemperature=cardTemperature, cardSerialNum=cardSerialNum, cardSecStateChange=cardSecStateChange, cardCarrierSerialNum=cardCarrierSerialNum, cardId=cardId, equipmentState=equipmentState, auditFailedCount=auditFailedCount, cardTestType=cardTestType, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSecState=cardSecState, downstreamPowerLoss=downstreamPowerLoss, cmMacAddress=cmMacAddress, cardTable=cardTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardNumPorts=cardNumPorts, cardCpuType=cardCpuType, auditName=auditName, cardPrState=cardPrState, TestCommand=TestCommand, cardTestDescription=cardTestDescription, auditDescription=auditDescription, TestScheduleCommand=TestScheduleCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portRxFlowControlMode=portRxFlowControlMode, cardBusSpeed=cardBusSpeed, equipmentShelfGroup=equipmentShelfGroup, portDocsIfIndex=portDocsIfIndex, cardTestSchedule=cardTestSchedule, shelfName=shelfName, diskVolumeName=diskVolumeName, cardTrapInh=cardTrapInh, cardTestId=cardTestId, portDescription=portDescription, cardTemperatureHighWarn=cardTemperatureHighWarn, auditTable=auditTable, portId=portId, cardCarrierFwVersion=cardCarrierFwVersion, portTable=portTable, diskVolumeCardId=diskVolumeCardId, portSecState=portSecState, diskVolumeTrapInh=diskVolumeTrapInh, TestResult=TestResult, portBgpId=portBgpId, portMinDsPower=portMinDsPower, portPrStateChange=portPrStateChange, portLastChangeTime=portLastChangeTime, cardAdminState=cardAdminState, systemKey=systemKey, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetUpTime=cmResetUpTime, cardLastBootVersion=cardLastBootVersion, diskVolumeEntry=diskVolumeEntry, portDplxStatus=portDplxStatus, shelfObjects=shelfObjects, cardSubDetected=cardSubDetected, eqDiagConfig=eqDiagConfig, cardDplxStatusChange=cardDplxStatusChange, equipmentCardGroup=equipmentCardGroup, portEntry=portEntry, systemGeneralGroup=systemGeneralGroup, equipmentAudit=equipmentAudit, portDplxStatusChange=portDplxStatusChange, cardTemperatureHighError=cardTemperatureHighError, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditAutoScheduling=auditAutoScheduling, auditCommand=auditCommand, cardTestTransId=cardTestTransId, diskVolumeDriveId=diskVolumeDriveId, cardType=cardType, portRxFlowControlDetected=portRxFlowControlDetected, portMode=portMode, diskVolumeFileName=diskVolumeFileName, cardTestIndex=cardTestIndex, cmUChannelID=cmUChannelID, cardFlashSize=cardFlashSize, cardRamSize=cardRamSize, portShelfId=portShelfId, errEventId=errEventId, diskVolumeTable=diskVolumeTable, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditCardId=auditCardId, cardEntry=cardEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cardNumber=cardNumber, portAdminState=portAdminState, auditCycleCount=auditCycleCount, cardTestName=cardTestName, cmPrimarySID=cmPrimarySID, cardDplxStatus=cardDplxStatus, cmResetNotification=cmResetNotification, cardCpuSpeed=cardCpuSpeed, portPrState=portPrState, verboseLevel=verboseLevel, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwUpdateStatus=cardFwUpdateStatus, portTxFlowControlMode=portTxFlowControlMode, cardTestResult=cardTestResult, errEvRowStatus=errEvRowStatus, auditLogThrottle=auditLogThrottle, cardDetectedChange=cardDetectedChange, portNumChans=portNumChans, systemClock=systemClock, diagTestId=diagTestId, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentDiag=equipmentDiag, diskVolumeUsageLevel=diskVolumeUsageLevel, cadEquipmentMibGroup=cadEquipmentMibGroup, auditTotalPassedCount=auditTotalPassedCount, cmResetReason=cmResetReason, auditEntry=auditEntry, equipmentTraps=equipmentTraps, auditShelfId=auditShelfId, diskVolumeNumber=diskVolumeNumber, shelfSwVersion=shelfSwVersion, auditTime=auditTime, cmResetStatus=cmResetStatus, portSecStateChange=portSecStateChange, diskDriveNumber=diskDriveNumber, portCurrDsPower=portCurrDsPower, cadEquipmentMib=cadEquipmentMib, cardShelfTestIndex=cardShelfTestIndex, portMaxDsPower=portMaxDsPower, cardPicDetected=cardPicDetected, portMacAddress=portMacAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, errEvLoggingEnabled=errEvLoggingEnabled, equipmentTbl=equipmentTbl, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibConformance=cadEquipmentMibConformance, cardName=cardName)
