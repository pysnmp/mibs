...
ShelfId, CardId, CardType, FlowControlMode, PortType, PicType, PortDetectedMode, PrimaryState, SecondaryState, CardSubType, PortMode, DuplexStatus, UnknownState, EqActionType, AdminState, PortId, FirmwareSource, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "CardId", "CardType", "FlowControlMode", "PortType", "PicType", "PortDetectedMode", "PrimaryState", "SecondaryState", "CardSubType", "PortMode", "DuplexStatus", "UnknownState", "EqActionType", "AdminState", "PortId", "FirmwareSource", "DiskVolumeUsageLevel")
...
DateAndTime, TextualConvention, TimeStamp, TruthValue, DisplayString, MacAddress, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TextualConvention", "TimeStamp", "TruthValue", "DisplayString", "MacAddress", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastResetReason=cardLastResetReason, cardBootVersion=cardBootVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, cardLastBootVersion=cardLastBootVersion, cardCarrierHwVersion=cardCarrierHwVersion, portNumber=portNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardNumPorts=cardNumPorts, portCurrDsPower=portCurrDsPower, cardEntry=cardEntry, auditLogOutput=auditLogOutput, auditId=auditId, TestId=TestId, cardFwVersion=cardFwVersion, cardSecStateChange=cardSecStateChange, trapSeverity=trapSeverity, cardTestEntry=cardTestEntry, portSecStateChange=portSecStateChange, cardNumber=cardNumber, cmResetInfo=cmResetInfo, cardDetected=cardDetected, cmVendor=cmVendor, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditTable=auditTable, portDplxStatusChange=portDplxStatusChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageNotification=diskVolumeUsageNotification, portType=portType, cardAdminState=cardAdminState, portAction=portAction, cardName=cardName, diskVolumeDiskSize=diskVolumeDiskSize, cardLastChangeTime=cardLastChangeTime, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestIndex=cardTestIndex, cardFwUpdateStatus=cardFwUpdateStatus, cardPicSerialNum=cardPicSerialNum, auditResult=auditResult, cardTestId=cardTestId, auditCycleCount=auditCycleCount, cardUpTime=cardUpTime, remainInDiagMode=remainInDiagMode, cardCarrierSerialNum=cardCarrierSerialNum, eqDiagConfig=eqDiagConfig, diskVolumeName=diskVolumeName, portDescription=portDescription, equipmentTbl=equipmentTbl, cardSwVersion=cardSwVersion, portMaxDsPower=portMaxDsPower, cardTestTable=cardTestTable, auditLogThrottle=auditLogThrottle, cardTrapInh=cardTrapInh, auditEntry=auditEntry, cadEquipmentMib=cadEquipmentMib, auditName=auditName, verboseLevel=verboseLevel, equipmentTraps=equipmentTraps, cardCpuSpeed=cardCpuSpeed, cardSerialNum=cardSerialNum, diskVolumeTrapInh=diskVolumeTrapInh, portDetectedMode=portDetectedMode, cardSpareGroupId=cardSpareGroupId, cardPrStateChange=cardPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portBgpId=portBgpId, cardTestTransId=cardTestTransId, auditCommand=auditCommand, TestCommand=TestCommand, cmPrimarySID=cmPrimarySID, systemKey=systemKey, cadEquipmentMibGroup=cadEquipmentMibGroup, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetUpTime=cmResetUpTime, cardAction=cardAction, equipmentAudit=equipmentAudit, equipmentCardTestGroup=equipmentCardTestGroup, cardSpareGroupMode=cardSpareGroupMode, auditTotalFailedCount=auditTotalFailedCount, cardTestResultDesc=cardTestResultDesc, cardPicHwVersion=cardPicHwVersion, diskVolumeSize=diskVolumeSize, portPrStateChange=portPrStateChange, auditAutoScheduling=auditAutoScheduling, portNumChans=portNumChans, cardTemperatureHighError=cardTemperatureHighError, auditCardId=auditCardId, cardTestScheduleCommand=cardTestScheduleCommand, auditDescription=auditDescription, diskVolumeCardId=diskVolumeCardId, equipmentAuditGroup=equipmentAuditGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardPicDetected=cardPicDetected, systemGeneralGroup=systemGeneralGroup, cmRegistrationNotification=cmRegistrationNotification, auditShelfId=auditShelfId, cardTempOverHeatNotification=cardTempOverHeatNotification, downstreamPowerLoss=downstreamPowerLoss, diskDriveNumber=diskDriveNumber, portMacIfIndex=portMacIfIndex, cardTable=cardTable, cardFlashSize=cardFlashSize, cardDplxStatus=cardDplxStatus, diskVolumeShelfId=diskVolumeShelfId, cardTemperature=cardTemperature, cmIpAddress=cmIpAddress, cardDplxStatusChange=cardDplxStatusChange, portTable=portTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portEntry=portEntry, cardCarrierFwVersion=cardCarrierFwVersion, cmDevice=cmDevice, consoleOutput=consoleOutput, TestTransactionId=TestTransactionId, shelfNumber=shelfNumber, errEventId=errEventId, portCardSubType=portCardSubType, cardTestResult=cardTestResult, diskVolumeDriveId=diskVolumeDriveId, portConnectorId=portConnectorId, TestType=TestType, cardTestTime=cardTestTime, diskVolumeEntry=diskVolumeEntry, cardTestSchedule=cardTestSchedule, cardRamSize=cardRamSize, auditTotalPassedCount=auditTotalPassedCount, portTxFlowControlMode=portTxFlowControlMode, cardId=cardId, cmUChannelID=cmUChannelID, cardBusSpeed=cardBusSpeed, TestSchedule=TestSchedule, diskVolumeNumber=diskVolumeNumber, cmResetReason=cmResetReason, portRxFlowControlMode=portRxFlowControlMode, equipmentDiagGroup=equipmentDiagGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, auditTime=auditTime, equipmentDiag=equipmentDiag, cmMacAddress=cmMacAddress, cmResetNotification=cmResetNotification, cardLastBootSource=cardLastBootSource, diskVolumeGroup=diskVolumeGroup, portAdminState=portAdminState, equipmentPortGroup=equipmentPortGroup, portTrapInh=portTrapInh, fpgaErrorEventTable=fpgaErrorEventTable, portDplxStatus=portDplxStatus, auditStatus=auditStatus, cardSecState=cardSecState, cardDetectedChange=cardDetectedChange, cardTempNoReportNotification=cardTempNoReportNotification, portPrState=portPrState, portSecState=portSecState, TestScheduleCommand=TestScheduleCommand, cmResetStatus=cmResetStatus, equipmentError=equipmentError, cardTestName=cardTestName, errEvLogLevel=errEvLogLevel, diskVolumeFileName=diskVolumeFileName, cardTestDescription=cardTestDescription, portMacAddress=portMacAddress, cardTestResultNotification=cardTestResultNotification, equipmentStateGroup=equipmentStateGroup, cardFpgaSource=cardFpgaSource, diskVolumeId=diskVolumeId, auditFailedCount=auditFailedCount, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portId=portId, auditPassedCount=auditPassedCount, portMinDsPower=portMinDsPower, PYSNMP_MODULE_ID=cadEquipmentMib, portMode=portMode, portLastChangeTime=portLastChangeTime, cadEquipmentMibConformance=cadEquipmentMibConformance, portTxFlowControlDetected=portTxFlowControlDetected, cardCPLDVersion=cardCPLDVersion, systemGeneral=systemGeneral, cardTestCommand=cardTestCommand, equipmentState=equipmentState, cardShelfTestIndex=cardShelfTestIndex, cardHwVersion=cardHwVersion, shelfObjects=shelfObjects, diskVolumeTable=diskVolumeTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCpuType=cardCpuType, equipmentCardGroup=equipmentCardGroup, cardSubDetected=cardSubDetected, diagTestId=diagTestId, cardSubType=cardSubType, errEvRowStatus=errEvRowStatus, portDocsIfIndex=portDocsIfIndex, cardTestType=cardTestType, shelfSwVersion=shelfSwVersion, equipmentShelfGroup=equipmentShelfGroup, portShelfId=portShelfId, TestResult=TestResult, cardPrState=cardPrState, portCardId=portCardId, cardType=cardType, cmResetClearNotification=cmResetClearNotification, cardShelfId=cardShelfId, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfName=shelfName, trapCounter=trapCounter, systemClock=systemClock)
