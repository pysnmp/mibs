...
AdminState, DuplexStatus, PicType, PrimaryState, DiskVolumeUsageLevel, PortDetectedMode, FirmwareSource, PortMode, UnknownState, FlowControlMode, SecondaryState, EqActionType, CardType, CardSubType, PortId, CardId, ShelfId, PortType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "DuplexStatus", "PicType", "PrimaryState", "DiskVolumeUsageLevel", "PortDetectedMode", "FirmwareSource", "PortMode", "UnknownState", "FlowControlMode", "SecondaryState", "EqActionType", "CardType", "CardSubType", "PortId", "CardId", "ShelfId", "PortType")
...
TimeStamp, DisplayString, DateAndTime, MacAddress, TextualConvention, RowStatus, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DisplayString", "DateAndTime", "MacAddress", "TextualConvention", "RowStatus", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSecState=cardSecState, cardSpareGroupId=cardSpareGroupId, equipmentDiagGroup=equipmentDiagGroup, cardCarrierHwVersion=cardCarrierHwVersion, portTxFlowControlMode=portTxFlowControlMode, cardTestTime=cardTestTime, equipmentAudit=equipmentAudit, remainInDiagMode=remainInDiagMode, TestScheduleCommand=TestScheduleCommand, systemClock=systemClock, cardTestTable=cardTestTable, cardFpgaSource=cardFpgaSource, errEventId=errEventId, cardTempNoReportNotification=cardTempNoReportNotification, cardDetectedChange=cardDetectedChange, cardFlashSize=cardFlashSize, portPrState=portPrState, cardTestTransId=cardTestTransId, equipmentPortGroup=equipmentPortGroup, cardCPLDVersion=cardCPLDVersion, cardDplxStatusChange=cardDplxStatusChange, portEntry=portEntry, errEvLogLevel=errEvLogLevel, diskVolumeDiskSize=diskVolumeDiskSize, systemKey=systemKey, cardTestId=cardTestId, TestId=TestId, diagTestId=diagTestId, equipmentAuditGroup=equipmentAuditGroup, cmVendor=cmVendor, portShelfId=portShelfId, equipmentError=equipmentError, diskVolumeGroup=diskVolumeGroup, diskVolumeName=diskVolumeName, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestDescription=cardTestDescription, systemGeneral=systemGeneral, auditLogThrottle=auditLogThrottle, auditLogOutput=auditLogOutput, cardId=cardId, cardNumPorts=cardNumPorts, portCurrDsPower=portCurrDsPower, auditCardId=auditCardId, cmRegistrationNotification=cmRegistrationNotification, auditEntry=auditEntry, portType=portType, cardDetected=cardDetected, shelfSwVersion=shelfSwVersion, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvRowStatus=errEvRowStatus, diskVolumeSize=diskVolumeSize, auditAutoScheduling=auditAutoScheduling, equipmentCardGroup=equipmentCardGroup, PYSNMP_MODULE_ID=cadEquipmentMib, auditFailedCount=auditFailedCount, portBgpId=portBgpId, cardSubType=cardSubType, cardPicHwVersion=cardPicHwVersion, cardTemperature=cardTemperature, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portSecStateChange=portSecStateChange, cardUpTime=cardUpTime, cardSecStateChange=cardSecStateChange, auditPassedCount=auditPassedCount, cmUChannelID=cmUChannelID, cmResetNotification=cmResetNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTemperatureHighError=cardTemperatureHighError, cmResetClearNotification=cmResetClearNotification, portDocsIfIndex=portDocsIfIndex, shelfObjects=shelfObjects, portLastChangeTime=portLastChangeTime, portMaxDsPower=portMaxDsPower, equipmentCardTestGroup=equipmentCardTestGroup, TestType=TestType, portAdminState=portAdminState, portConnectorId=portConnectorId, auditTable=auditTable, auditCommand=auditCommand, diskVolumeDriveId=diskVolumeDriveId, portRxFlowControlDetected=portRxFlowControlDetected, portPrStateChange=portPrStateChange, cmResetReason=cmResetReason, cardBusSpeed=cardBusSpeed, cardFwUpdateStatus=cardFwUpdateStatus, cardTestEntry=cardTestEntry, cardSpareGroupMode=cardSpareGroupMode, cardFwVersion=cardFwVersion, portId=portId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTrapInh=cardTrapInh, trapCounter=trapCounter, cardTestScheduleCommand=cardTestScheduleCommand, auditTime=auditTime, portAction=portAction, portDplxStatus=portDplxStatus, TestCommand=TestCommand, cardSubDetected=cardSubDetected, downstreamPowerLoss=downstreamPowerLoss, cardPicDetected=cardPicDetected, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastResetReason=cardLastResetReason, cardCarrierFwVersion=cardCarrierFwVersion, cardSerialNum=cardSerialNum, portMacIfIndex=portMacIfIndex, cardTestType=cardTestType, cardTestResult=cardTestResult, equipmentStateGroup=equipmentStateGroup, portCardId=portCardId, cardLastBootVersion=cardLastBootVersion, cardShelfId=cardShelfId, cardTestIndex=cardTestIndex, cardEntry=cardEntry, auditName=auditName, diskVolumeUsageNotification=diskVolumeUsageNotification, auditCycleCount=auditCycleCount, eqDiagConfig=eqDiagConfig, diskVolumeTable=diskVolumeTable, auditTotalFailedCount=auditTotalFailedCount, cardAdminState=cardAdminState, cardName=cardName, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCpuSpeed=cardCpuSpeed, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestSchedule=TestSchedule, cadEquipmentMib=cadEquipmentMib, diskVolumeShelfId=diskVolumeShelfId, cardTestResultDesc=cardTestResultDesc, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, errEvLoggingEnabled=errEvLoggingEnabled, verboseLevel=verboseLevel, equipmentDiag=equipmentDiag, cardTemperatureHighWarn=cardTemperatureHighWarn, portDplxStatusChange=portDplxStatusChange, equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, cardRamSize=cardRamSize, cardPicSerialNum=cardPicSerialNum, diskDriveNumber=diskDriveNumber, auditDescription=auditDescription, portNumChans=portNumChans, cmPrimarySID=cmPrimarySID, cardTestName=cardTestName, equipmentTbl=equipmentTbl, cmResetUpTime=cmResetUpTime, portDetectedMode=portDetectedMode, cardLastChangeTime=cardLastChangeTime, portMacAddress=portMacAddress, consoleOutput=consoleOutput, portCardSubType=portCardSubType, cmResetStatus=cmResetStatus, shelfName=shelfName, portMode=portMode, cardShelfTestIndex=cardShelfTestIndex, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeFileName=diskVolumeFileName, auditShelfId=auditShelfId, cmMacAddress=cmMacAddress, auditResult=auditResult, TestResult=TestResult, portMinDsPower=portMinDsPower, cmIpAddress=cmIpAddress, cardPrStateChange=cardPrStateChange, cardTable=cardTable, cardTestSchedule=cardTestSchedule, portDescription=portDescription, cardTestCommand=cardTestCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSwVersion=cardSwVersion, diskVolumeEntry=diskVolumeEntry, cardDplxStatus=cardDplxStatus, cmResetInfo=cmResetInfo, cardType=cardType, portNumber=portNumber, cardTestResultNotification=cardTestResultNotification, portTrapInh=portTrapInh, systemGeneralGroup=systemGeneralGroup, TestTransactionId=TestTransactionId, equipmentShelfGroup=equipmentShelfGroup, cardPrState=cardPrState, fpgaErrorEventEntry=fpgaErrorEventEntry, cardNumber=cardNumber, cadEquipmentMibConformance=cadEquipmentMibConformance, cardLastBootSource=cardLastBootSource, diskVolumeNumber=diskVolumeNumber, cardCpuType=cardCpuType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, shelfNumber=shelfNumber, auditId=auditId, cmDevice=cmDevice, diskVolumeUsageLevel=diskVolumeUsageLevel, cardBootVersion=cardBootVersion, diskVolumeId=diskVolumeId, cardHwVersion=cardHwVersion, trapSeverity=trapSeverity, cardAction=cardAction, auditStatus=auditStatus, equipmentTraps=equipmentTraps, portSecState=portSecState, fpgaErrorEventTable=fpgaErrorEventTable, portTable=portTable, portRxFlowControlMode=portRxFlowControlMode)
