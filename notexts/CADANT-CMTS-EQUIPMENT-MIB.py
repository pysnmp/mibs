...
CardType, PortType, DuplexStatus, DiskVolumeUsageLevel, PrimaryState, PortId, PortMode, FlowControlMode, PortDetectedMode, ShelfId, CardId, CardSubType, AdminState, UnknownState, PicType, FirmwareSource, SecondaryState, EqActionType = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortType", "DuplexStatus", "DiskVolumeUsageLevel", "PrimaryState", "PortId", "PortMode", "FlowControlMode", "PortDetectedMode", "ShelfId", "CardId", "CardSubType", "AdminState", "UnknownState", "PicType", "FirmwareSource", "SecondaryState", "EqActionType")
...
TruthValue, MacAddress, TextualConvention, DisplayString, RowStatus, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "MacAddress", "TextualConvention", "DisplayString", "RowStatus", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestEntry=cardTestEntry, auditCardId=auditCardId, equipmentAudit=equipmentAudit, auditPassedCount=auditPassedCount, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestScheduleCommand=cardTestScheduleCommand, cardTestTransId=cardTestTransId, cardFwUpdateStatus=cardFwUpdateStatus, portCardSubType=portCardSubType, cardPrState=cardPrState, diskVolumeNumber=diskVolumeNumber, errEvLogLevel=errEvLogLevel, auditId=auditId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeShelfId=diskVolumeShelfId, eqDiagConfig=eqDiagConfig, cardType=cardType, trapCounter=trapCounter, diskVolumeDiskSize=diskVolumeDiskSize, TestId=TestId, cardDplxStatus=cardDplxStatus, portConnectorId=portConnectorId, cardShelfId=cardShelfId, portDocsIfIndex=portDocsIfIndex, portMaxDsPower=portMaxDsPower, diskVolumeGroup=diskVolumeGroup, cardTable=cardTable, cardDetected=cardDetected, cardBootVersion=cardBootVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, auditAutoScheduling=auditAutoScheduling, cardEntry=cardEntry, cardSubType=cardSubType, cardTempOverHeatNotification=cardTempOverHeatNotification, cmUChannelID=cmUChannelID, cmResetClearNotification=cmResetClearNotification, cardId=cardId, portSecState=portSecState, portMacIfIndex=portMacIfIndex, cardTestTime=cardTestTime, cmResetUpTime=cmResetUpTime, cardCpuSpeed=cardCpuSpeed, cardPicSerialNum=cardPicSerialNum, errEventId=errEventId, portDplxStatus=portDplxStatus, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsageNotification=diskVolumeUsageNotification, cmResetReason=cmResetReason, auditTable=auditTable, cmResetNotification=cmResetNotification, portCurrDsPower=portCurrDsPower, cmIpAddress=cmIpAddress, equipmentTbl=equipmentTbl, cadEquipmentMibGroup=cadEquipmentMibGroup, TestTransactionId=TestTransactionId, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogOutput=auditLogOutput, cardCpuType=cardCpuType, portNumber=portNumber, auditTotalPassedCount=auditTotalPassedCount, cardSecState=cardSecState, cardLastBootSource=cardLastBootSource, cardCarrierSerialNum=cardCarrierSerialNum, cardRamSize=cardRamSize, diskVolumeTable=diskVolumeTable, portShelfId=portShelfId, portTxFlowControlMode=portTxFlowControlMode, cardTestDescription=cardTestDescription, portPrState=portPrState, cmDevice=cmDevice, cardLastChangeTime=cardLastChangeTime, cardSpareGroupMode=cardSpareGroupMode, diskVolumeId=diskVolumeId, shelfName=shelfName, cardTempNoReportNotification=cardTempNoReportNotification, portBgpId=portBgpId, equipmentDiagGroup=equipmentDiagGroup, auditEntry=auditEntry, cmResetInfo=cmResetInfo, cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventGroup=fpgaErrorEventGroup, cardDetectedChange=cardDetectedChange, verboseLevel=verboseLevel, systemGeneral=systemGeneral, auditLogThrottle=auditLogThrottle, auditFailedCount=auditFailedCount, cmPrimarySID=cmPrimarySID, cardSecStateChange=cardSecStateChange, diskVolumeCardId=diskVolumeCardId, auditStatus=auditStatus, cardTestName=cardTestName, cadEquipmentMib=cadEquipmentMib, TestType=TestType, remainInDiagMode=remainInDiagMode, portTable=portTable, diskVolumeDriveId=diskVolumeDriveId, auditName=auditName, portPrStateChange=portPrStateChange, portSecStateChange=portSecStateChange, portEntry=portEntry, equipmentStateGroup=equipmentStateGroup, cardNumber=cardNumber, equipmentAuditGroup=equipmentAuditGroup, cardPrStateChange=cardPrStateChange, portTrapInh=portTrapInh, cardTestResultDesc=cardTestResultDesc, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, PYSNMP_MODULE_ID=cadEquipmentMib, cardAction=cardAction, cardTemperature=cardTemperature, portId=portId, portRxFlowControlMode=portRxFlowControlMode, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTable=cardTestTable, auditCommand=auditCommand, auditCycleCount=auditCycleCount, trapSeverity=trapSeverity, auditTotalFailedCount=auditTotalFailedCount, errEvRowStatus=errEvRowStatus, diskVolumeEntry=diskVolumeEntry, equipmentShelfGroup=equipmentShelfGroup, diagTestId=diagTestId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardBusSpeed=cardBusSpeed, TestSchedule=TestSchedule, cardSubDetected=cardSubDetected, cardTestSchedule=cardTestSchedule, equipmentPortGroup=equipmentPortGroup, cardAdminState=cardAdminState, cmVendor=cmVendor, portType=portType, cardTestId=cardTestId, equipmentError=equipmentError, portAdminState=portAdminState, cardDplxStatusChange=cardDplxStatusChange, portAction=portAction, diskVolumeTrapInh=diskVolumeTrapInh, auditDescription=auditDescription, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResultNotification=cardTestResultNotification, cardTestCommand=cardTestCommand, fpgaErrorEventTable=fpgaErrorEventTable, portDescription=portDescription, diskDriveNumber=diskDriveNumber, equipmentCardTestGroup=equipmentCardTestGroup, cardFlashSize=cardFlashSize, cardName=cardName, portCardId=portCardId, systemClock=systemClock, cardSwVersion=cardSwVersion, portLastChangeTime=portLastChangeTime, auditTime=auditTime, errEvRecoveryEnabled=errEvRecoveryEnabled, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentCardGroup=equipmentCardGroup, cardFpgaSource=cardFpgaSource, portMode=portMode, cardPicHwVersion=cardPicHwVersion, auditShelfId=auditShelfId, diskVolumeFileName=diskVolumeFileName, cardFwVersion=cardFwVersion, cardCPLDVersion=cardCPLDVersion, portMinDsPower=portMinDsPower, cardTemperatureHighError=cardTemperatureHighError, portTxFlowControlDetected=portTxFlowControlDetected, cardNumPorts=cardNumPorts, equipmentDiag=equipmentDiag, consoleOutput=consoleOutput, equipmentState=equipmentState, cardCarrierHwVersion=cardCarrierHwVersion, cmMacAddress=cmMacAddress, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardHwVersion=cardHwVersion, systemKey=systemKey, shelfNumber=shelfNumber, cardSpareGroupId=cardSpareGroupId, equipmentTraps=equipmentTraps, cardPicDetected=cardPicDetected, cardUpTime=cardUpTime, diskVolumeSize=diskVolumeSize, TestResult=TestResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditResult=auditResult, portNumChans=portNumChans, portMacAddress=portMacAddress, TestCommand=TestCommand, cardTestResult=cardTestResult, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestType=cardTestType, cardTrapInh=cardTrapInh, cardLastBootVersion=cardLastBootVersion, portDetectedMode=portDetectedMode, cardTemperatureHighWarn=cardTemperatureHighWarn, shelfObjects=shelfObjects, downstreamPowerLoss=downstreamPowerLoss, cardTestIndex=cardTestIndex, cmResetStatus=cmResetStatus, cardLastResetReason=cardLastResetReason, TestScheduleCommand=TestScheduleCommand, cardSerialNum=cardSerialNum, diskVolumeName=diskVolumeName, shelfSwVersion=shelfSwVersion)
