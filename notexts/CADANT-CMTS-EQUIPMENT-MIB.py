...
PortMode, PortId, PicType, FlowControlMode, PrimaryState, AdminState, CardType, EqActionType, PortDetectedMode, ShelfId, UnknownState, SecondaryState, CardSubType, DuplexStatus, FirmwareSource, PortType, DiskVolumeUsageLevel, CardId = mibBuilder.importSymbols("CADANT-TC", "PortMode", "PortId", "PicType", "FlowControlMode", "PrimaryState", "AdminState", "CardType", "EqActionType", "PortDetectedMode", "ShelfId", "UnknownState", "SecondaryState", "CardSubType", "DuplexStatus", "FirmwareSource", "PortType", "DiskVolumeUsageLevel", "CardId")
...
TruthValue, TimeStamp, RowStatus, DisplayString, TextualConvention, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TimeStamp", "RowStatus", "DisplayString", "TextualConvention", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResultDesc=cardTestResultDesc, portTable=portTable, cardAction=cardAction, equipmentAuditGroup=equipmentAuditGroup, cardTestResult=cardTestResult, auditTime=auditTime, portLastChangeTime=portLastChangeTime, portId=portId, cmPrimarySID=cmPrimarySID, diskVolumeId=diskVolumeId, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmRegistrationNotification=cmRegistrationNotification, cardSerialNum=cardSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditStatus=auditStatus, cardFpgaSource=cardFpgaSource, portPrStateChange=portPrStateChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portSecState=portSecState, cardTestId=cardTestId, equipmentState=equipmentState, portTxFlowControlMode=portTxFlowControlMode, cardName=cardName, portPrState=portPrState, cardTemperatureHighError=cardTemperatureHighError, auditTotalPassedCount=auditTotalPassedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSubType=cardSubType, equipmentDiagGroup=equipmentDiagGroup, cmResetInfo=cmResetInfo, auditName=auditName, equipmentDiag=equipmentDiag, auditFailedCount=auditFailedCount, cardTestDescription=cardTestDescription, cardCarrierFwVersion=cardCarrierFwVersion, portAction=portAction, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentShelfGroup=equipmentShelfGroup, portNumChans=portNumChans, verboseLevel=verboseLevel, shelfNumber=shelfNumber, cadEquipmentMib=cadEquipmentMib, cardFwUpdateStatus=cardFwUpdateStatus, cardSwVersion=cardSwVersion, cardHwVersion=cardHwVersion, cardTestEntry=cardTestEntry, cardTestIndex=cardTestIndex, cardTemperature=cardTemperature, cmMacAddress=cmMacAddress, errEventId=errEventId, cardPrStateChange=cardPrStateChange, cardSecStateChange=cardSecStateChange, auditCommand=auditCommand, auditLogOutput=auditLogOutput, cmUChannelID=cmUChannelID, trapCounter=trapCounter, cardUpTime=cardUpTime, portCardSubType=portCardSubType, cmResetClearNotification=cmResetClearNotification, cardCarrierHwVersion=cardCarrierHwVersion, cardTestType=cardTestType, cmDevice=cmDevice, systemKey=systemKey, diskVolumeSize=diskVolumeSize, cardFlashSize=cardFlashSize, cardBusSpeed=cardBusSpeed, portEntry=portEntry, cardTempNoReportNotification=cardTempNoReportNotification, trapSeverity=trapSeverity, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portNumber=portNumber, diagTestId=diagTestId, auditDescription=auditDescription, TestId=TestId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardLastBootVersion=cardLastBootVersion, cardLastBootSource=cardLastBootSource, cardShelfId=cardShelfId, cardSpareGroupMode=cardSpareGroupMode, TestSchedule=TestSchedule, cardTestScheduleCommand=cardTestScheduleCommand, cardTestTime=cardTestTime, cardNumber=cardNumber, cardDplxStatusChange=cardDplxStatusChange, TestTransactionId=TestTransactionId, portCurrDsPower=portCurrDsPower, systemGeneral=systemGeneral, portType=portType, portDocsIfIndex=portDocsIfIndex, portMacIfIndex=portMacIfIndex, errEvRowStatus=errEvRowStatus, portDescription=portDescription, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibGroup=cadEquipmentMibGroup, auditTable=auditTable, cardNumPorts=cardNumPorts, portDplxStatus=portDplxStatus, cardTrapInh=cardTrapInh, auditId=auditId, shelfName=shelfName, diskVolumeDiskSize=diskVolumeDiskSize, cardTestSchedule=cardTestSchedule, cardPicHwVersion=cardPicHwVersion, portMaxDsPower=portMaxDsPower, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, portTrapInh=portTrapInh, diskVolumeTrapInh=diskVolumeTrapInh, downstreamPowerLoss=downstreamPowerLoss, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastResetReason=cardLastResetReason, cardCpuSpeed=cardCpuSpeed, consoleOutput=consoleOutput, cardTable=cardTable, cardLastChangeTime=cardLastChangeTime, auditCycleCount=auditCycleCount, cardFwVersion=cardFwVersion, cardSubDetected=cardSubDetected, portRxFlowControlMode=portRxFlowControlMode, portCardId=portCardId, TestResult=TestResult, auditPassedCount=auditPassedCount, diskVolumeEntry=diskVolumeEntry, diskVolumeTable=diskVolumeTable, cardEntry=cardEntry, diskVolumeName=diskVolumeName, cardTestTransId=cardTestTransId, cardAdminState=cardAdminState, cardTemperatureHighWarn=cardTemperatureHighWarn, fpgaErrorEventTable=fpgaErrorEventTable, cardShelfTestIndex=cardShelfTestIndex, portTxFlowControlDetected=portTxFlowControlDetected, remainInDiagMode=remainInDiagMode, shelfSwVersion=shelfSwVersion, auditTotalFailedCount=auditTotalFailedCount, cmResetStatus=cmResetStatus, portAdminState=portAdminState, auditShelfId=auditShelfId, PYSNMP_MODULE_ID=cadEquipmentMib, cardBootVersion=cardBootVersion, equipmentError=equipmentError, cardCarrierSerialNum=cardCarrierSerialNum, diskDriveNumber=diskDriveNumber, equipmentAudit=equipmentAudit, eqDiagConfig=eqDiagConfig, equipmentTraps=equipmentTraps, cardPrState=cardPrState, portDetectedMode=portDetectedMode, equipmentPortGroup=equipmentPortGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, systemGeneralGroup=systemGeneralGroup, equipmentTbl=equipmentTbl, cardTestCommand=cardTestCommand, diskVolumeGroup=diskVolumeGroup, auditAutoScheduling=auditAutoScheduling, cardPicDetected=cardPicDetected, cardPicSerialNum=cardPicSerialNum, cmResetUpTime=cmResetUpTime, TestType=TestType, diskVolumeDriveId=diskVolumeDriveId, diskVolumeNumber=diskVolumeNumber, cardDetected=cardDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeShelfId=diskVolumeShelfId, cardTestResultNotification=cardTestResultNotification, portSecStateChange=portSecStateChange, equipmentCardGroup=equipmentCardGroup, cmResetReason=cmResetReason, portMacAddress=portMacAddress, errEvLogLevel=errEvLogLevel, cmVendor=cmVendor, portBgpId=portBgpId, equipmentCardTestGroup=equipmentCardTestGroup, portMode=portMode, shelfObjects=shelfObjects, TestCommand=TestCommand, cardTestName=cardTestName, cardId=cardId, cardDplxStatus=cardDplxStatus, cmResetNotification=cmResetNotification, portRxFlowControlDetected=portRxFlowControlDetected, auditEntry=auditEntry, cardSpareGroupId=cardSpareGroupId, cardRamSize=cardRamSize, systemClock=systemClock, errEvRecoveryEnabled=errEvRecoveryEnabled, cardDetectedChange=cardDetectedChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditCardId=auditCardId, TestScheduleCommand=TestScheduleCommand, auditLogThrottle=auditLogThrottle, equipmentStateGroup=equipmentStateGroup, cardType=cardType, portShelfId=portShelfId, diskVolumeCardId=diskVolumeCardId, portMinDsPower=portMinDsPower, cmIpAddress=cmIpAddress, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditResult=auditResult, cardCpuType=cardCpuType, portConnectorId=portConnectorId, cardCPLDVersion=cardCPLDVersion, cardSecState=cardSecState, cardTestTable=cardTestTable, diskVolumeFileName=diskVolumeFileName)
