...
PortDetectedMode, PicType, CardId, PortId, PortType, UnknownState, DuplexStatus, PortMode, PrimaryState, CardType, AdminState, CardSubType, SecondaryState, ShelfId, FirmwareSource, FlowControlMode, DiskVolumeUsageLevel, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "PicType", "CardId", "PortId", "PortType", "UnknownState", "DuplexStatus", "PortMode", "PrimaryState", "CardType", "AdminState", "CardSubType", "SecondaryState", "ShelfId", "FirmwareSource", "FlowControlMode", "DiskVolumeUsageLevel", "EqActionType")
...
DisplayString, TextualConvention, DateAndTime, RowStatus, TruthValue, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TextualConvention", "DateAndTime", "RowStatus", "TruthValue", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeDriveId=diskVolumeDriveId, cmVendor=cmVendor, portAction=portAction, cardCarrierFwVersion=cardCarrierFwVersion, cardType=cardType, cardTempNoReportNotification=cardTempNoReportNotification, trapCounter=trapCounter, diskVolumeTrapInh=diskVolumeTrapInh, cardTestDescription=cardTestDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, remainInDiagMode=remainInDiagMode, auditLogOutput=auditLogOutput, cardTestId=cardTestId, fpgaErrorEventGroup=fpgaErrorEventGroup, cmUChannelID=cmUChannelID, cardTestResultNotification=cardTestResultNotification, cardAdminState=cardAdminState, auditTime=auditTime, cmPrimarySID=cmPrimarySID, errEventId=errEventId, portMaxDsPower=portMaxDsPower, TestTransactionId=TestTransactionId, cardSwVersion=cardSwVersion, cardBusSpeed=cardBusSpeed, cmDevice=cmDevice, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentError=equipmentError, diskVolumeDiskSize=diskVolumeDiskSize, auditPassedCount=auditPassedCount, cardTestTable=cardTestTable, cardFlashSize=cardFlashSize, cmResetStatus=cmResetStatus, portCurrDsPower=portCurrDsPower, diskVolumeUsageLevel=diskVolumeUsageLevel, cmIpAddress=cmIpAddress, portPrStateChange=portPrStateChange, cardDplxStatus=cardDplxStatus, cardTestScheduleCommand=cardTestScheduleCommand, portCardSubType=portCardSubType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, systemKey=systemKey, shelfObjects=shelfObjects, diskVolumeFileName=diskVolumeFileName, cardCpuSpeed=cardCpuSpeed, auditId=auditId, cmResetReason=cmResetReason, diskVolumeGroup=diskVolumeGroup, cardLastBootSource=cardLastBootSource, TestId=TestId, equipmentStateGroup=equipmentStateGroup, cardTestTransId=cardTestTransId, cardFwUpdateStatus=cardFwUpdateStatus, portPrState=portPrState, TestResult=TestResult, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestCommand=TestCommand, portMacAddress=portMacAddress, TestType=TestType, equipmentState=equipmentState, fpgaErrorEventEntry=fpgaErrorEventEntry, cardLastResetReason=cardLastResetReason, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardName=cardName, cardUpTime=cardUpTime, portAdminState=portAdminState, cmResetNotification=cmResetNotification, verboseLevel=verboseLevel, portNumber=portNumber, equipmentDiag=equipmentDiag, diskVolumeName=diskVolumeName, auditCommand=auditCommand, diskVolumeEntry=diskVolumeEntry, diskVolumeCardId=diskVolumeCardId, auditFailedCount=auditFailedCount, diskVolumeShelfId=diskVolumeShelfId, equipmentDiagGroup=equipmentDiagGroup, cardSecState=cardSecState, auditLogThrottle=auditLogThrottle, diskVolumeNumber=diskVolumeNumber, cardTestTime=cardTestTime, cardTable=cardTable, equipmentTbl=equipmentTbl, cardTemperature=cardTemperature, PYSNMP_MODULE_ID=cadEquipmentMib, downstreamPowerLoss=downstreamPowerLoss, equipmentAuditGroup=equipmentAuditGroup, consoleOutput=consoleOutput, shelfNumber=shelfNumber, cadEquipmentMib=cadEquipmentMib, shelfSwVersion=shelfSwVersion, auditAutoScheduling=auditAutoScheduling, cardHwVersion=cardHwVersion, cardNumPorts=cardNumPorts, errEvLogLevel=errEvLogLevel, trapSeverity=trapSeverity, equipmentTraps=equipmentTraps, portCardId=portCardId, cardShelfTestIndex=cardShelfTestIndex, TestScheduleCommand=TestScheduleCommand, cardTrapInh=cardTrapInh, cardSerialNum=cardSerialNum, portTxFlowControlDetected=portTxFlowControlDetected, cardTestName=cardTestName, portDplxStatusChange=portDplxStatusChange, cardId=cardId, cardCPLDVersion=cardCPLDVersion, portSecStateChange=portSecStateChange, cmResetUpTime=cmResetUpTime, cardDplxStatusChange=cardDplxStatusChange, cardBootVersion=cardBootVersion, cardDetectedChange=cardDetectedChange, diskVolumeSize=diskVolumeSize, portType=portType, cardTestIndex=cardTestIndex, cardCarrierHwVersion=cardCarrierHwVersion, equipmentCardGroup=equipmentCardGroup, cardLastChangeTime=cardLastChangeTime, portId=portId, portRxFlowControlDetected=portRxFlowControlDetected, equipmentCardTestGroup=equipmentCardTestGroup, eqDiagConfig=eqDiagConfig, cardCarrierSerialNum=cardCarrierSerialNum, cardTestType=cardTestType, equipmentAudit=equipmentAudit, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResult=cardTestResult, errEvLoggingEnabled=errEvLoggingEnabled, diskDriveNumber=diskDriveNumber, shelfName=shelfName, systemClock=systemClock, cardPrState=cardPrState, systemGeneral=systemGeneral, portMinDsPower=portMinDsPower, cmRegistrationNotification=cmRegistrationNotification, portTable=portTable, portBgpId=portBgpId, cardPicSerialNum=cardPicSerialNum, cardNumber=cardNumber, auditDescription=auditDescription, portMacIfIndex=portMacIfIndex, TestSchedule=TestSchedule, portTrapInh=portTrapInh, auditTotalFailedCount=auditTotalFailedCount, auditTotalPassedCount=auditTotalPassedCount, systemGeneralGroup=systemGeneralGroup, portEntry=portEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicHwVersion=cardPicHwVersion, portConnectorId=portConnectorId, auditTable=auditTable, cardTestCommand=cardTestCommand, auditEntry=auditEntry, equipmentShelfGroup=equipmentShelfGroup, portDescription=portDescription, cardCpuType=cardCpuType, diskVolumeUsageNotification=diskVolumeUsageNotification, portDetectedMode=portDetectedMode, cardTestSchedule=cardTestSchedule, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDocsIfIndex=portDocsIfIndex, cardFpgaSource=cardFpgaSource, portDplxStatus=portDplxStatus, cardSecStateChange=cardSecStateChange, portShelfId=portShelfId, errEvRowStatus=errEvRowStatus, cardSubType=cardSubType, auditCycleCount=auditCycleCount, portSecState=portSecState, cardTemperatureHighWarn=cardTemperatureHighWarn, cardShelfId=cardShelfId, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPrStateChange=cardPrStateChange, cardTestResultDesc=cardTestResultDesc, auditCardId=auditCardId, equipmentPortGroup=equipmentPortGroup, cardTemperatureHighError=cardTemperatureHighError, cardDetected=cardDetected, portNumChans=portNumChans, portLastChangeTime=portLastChangeTime, cardFwVersion=cardFwVersion, portMode=portMode, diskVolumeTable=diskVolumeTable, cmMacAddress=cmMacAddress, cardTestEntry=cardTestEntry, fpgaErrorEventTable=fpgaErrorEventTable, cmResetClearNotification=cmResetClearNotification, auditShelfId=auditShelfId, cardSubDetected=cardSubDetected, cardAction=cardAction, auditResult=auditResult, diagTestId=diagTestId, diskVolumeId=diskVolumeId, cardSpareGroupMode=cardSpareGroupMode, auditName=auditName, auditStatus=auditStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetInfo=cmResetInfo, cardPicDetected=cardPicDetected, cardEntry=cardEntry, cardSpareGroupId=cardSpareGroupId, portTxFlowControlMode=portTxFlowControlMode, cardRamSize=cardRamSize, cardLastBootVersion=cardLastBootVersion)
