...
ShelfId, PortType, AdminState, FlowControlMode, DiskVolumeUsageLevel, PortId, PortMode, EqActionType, FirmwareSource, PicType, CardId, UnknownState, SecondaryState, CardSubType, PortDetectedMode, PrimaryState, CardType, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "PortType", "AdminState", "FlowControlMode", "DiskVolumeUsageLevel", "PortId", "PortMode", "EqActionType", "FirmwareSource", "PicType", "CardId", "UnknownState", "SecondaryState", "CardSubType", "PortDetectedMode", "PrimaryState", "CardType", "DuplexStatus")
...
TimeStamp, TextualConvention, TruthValue, MacAddress, DisplayString, RowStatus, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TextualConvention", "TruthValue", "MacAddress", "DisplayString", "RowStatus", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentPortGroup=equipmentPortGroup, cardTemperature=cardTemperature, errEvRecoveryEnabled=errEvRecoveryEnabled, cardLastBootSource=cardLastBootSource, portMaxDsPower=portMaxDsPower, diskVolumeName=diskVolumeName, portRxFlowControlDetected=portRxFlowControlDetected, auditShelfId=auditShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfSwVersion=shelfSwVersion, errEventId=errEventId, cmResetStatus=cmResetStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, portLastChangeTime=portLastChangeTime, cardTrapInh=cardTrapInh, diskVolumeId=diskVolumeId, trapCounter=trapCounter, cardHwVersion=cardHwVersion, diskVolumeFileName=diskVolumeFileName, cmMacAddress=cmMacAddress, equipmentShelfGroup=equipmentShelfGroup, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSubType=cardSubType, verboseLevel=verboseLevel, cardDetectedChange=cardDetectedChange, diskVolumeDriveId=diskVolumeDriveId, equipmentAuditGroup=equipmentAuditGroup, portDetectedMode=portDetectedMode, diskDriveNumber=diskDriveNumber, cardFwUpdateStatus=cardFwUpdateStatus, TestResult=TestResult, cardAction=cardAction, cardBootVersion=cardBootVersion, cardTestResult=cardTestResult, cmResetReason=cmResetReason, portPrState=portPrState, diskVolumeCardId=diskVolumeCardId, cardSubDetected=cardSubDetected, cardTestResultDesc=cardTestResultDesc, cardTestTable=cardTestTable, auditPassedCount=auditPassedCount, equipmentState=equipmentState, cardDplxStatus=cardDplxStatus, TestType=TestType, cmVendor=cmVendor, systemClock=systemClock, portMinDsPower=portMinDsPower, diskVolumeTable=diskVolumeTable, diskVolumeSize=diskVolumeSize, equipmentDiag=equipmentDiag, portAdminState=portAdminState, cmRegistrationNotification=cmRegistrationNotification, cardFwVersion=cardFwVersion, cardSecState=cardSecState, cardShelfTestIndex=cardShelfTestIndex, portConnectorId=portConnectorId, cardName=cardName, diagTestId=diagTestId, cardDetected=cardDetected, cmIpAddress=cmIpAddress, cardTestResultNotification=cardTestResultNotification, equipmentStateGroup=equipmentStateGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cardPicDetected=cardPicDetected, trapSeverity=trapSeverity, TestScheduleCommand=TestScheduleCommand, portDplxStatus=portDplxStatus, auditDescription=auditDescription, auditTotalFailedCount=auditTotalFailedCount, portCurrDsPower=portCurrDsPower, cardTestEntry=cardTestEntry, diskVolumeNumber=diskVolumeNumber, cardCpuType=cardCpuType, portSecStateChange=portSecStateChange, TestId=TestId, cardFpgaSource=cardFpgaSource, cardDplxStatusChange=cardDplxStatusChange, TestCommand=TestCommand, cardCPLDVersion=cardCPLDVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, cadEquipmentMib=cadEquipmentMib, cardTestTransId=cardTestTransId, TestTransactionId=TestTransactionId, cardLastBootVersion=cardLastBootVersion, diskVolumeDiskSize=diskVolumeDiskSize, systemGeneral=systemGeneral, cardNumber=cardNumber, equipmentTraps=equipmentTraps, cardSerialNum=cardSerialNum, cardTestTime=cardTestTime, equipmentCardTestGroup=equipmentCardTestGroup, cardSwVersion=cardSwVersion, portCardId=portCardId, cardNumPorts=cardNumPorts, portMode=portMode, cardTestCommand=cardTestCommand, portMacIfIndex=portMacIfIndex, diskVolumeShelfId=diskVolumeShelfId, auditLogThrottle=auditLogThrottle, systemKey=systemKey, portAction=portAction, downstreamPowerLoss=downstreamPowerLoss, portNumChans=portNumChans, portPrStateChange=portPrStateChange, portCardSubType=portCardSubType, cardFlashSize=cardFlashSize, cardPicSerialNum=cardPicSerialNum, portRxFlowControlMode=portRxFlowControlMode, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSpareGroupId=cardSpareGroupId, cardTemperatureHighWarn=cardTemperatureHighWarn, systemGeneralGroup=systemGeneralGroup, cardTable=cardTable, cardCarrierHwVersion=cardCarrierHwVersion, auditEntry=auditEntry, auditResult=auditResult, portMacAddress=portMacAddress, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portSecState=portSecState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardCarrierFwVersion=cardCarrierFwVersion, auditTotalPassedCount=auditTotalPassedCount, errEvLoggingEnabled=errEvLoggingEnabled, cardPrStateChange=cardPrStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfObjects=shelfObjects, equipmentDiagGroup=equipmentDiagGroup, auditName=auditName, cmResetNotification=cmResetNotification, diskVolumeEntry=diskVolumeEntry, cardSecStateChange=cardSecStateChange, errEvLogLevel=errEvLogLevel, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestType=cardTestType, PYSNMP_MODULE_ID=cadEquipmentMib, portTxFlowControlMode=portTxFlowControlMode, portDplxStatusChange=portDplxStatusChange, diskVolumeGroup=diskVolumeGroup, portDocsIfIndex=portDocsIfIndex, equipmentTbl=equipmentTbl, cmResetClearNotification=cmResetClearNotification, cardLastResetReason=cardLastResetReason, portTable=portTable, eqDiagConfig=eqDiagConfig, cardRamSize=cardRamSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditId=auditId, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeTrapInh=diskVolumeTrapInh, equipmentAudit=equipmentAudit, portBgpId=portBgpId, portShelfId=portShelfId, portId=portId, portTxFlowControlDetected=portTxFlowControlDetected, cardPrState=cardPrState, consoleOutput=consoleOutput, cardLastChangeTime=cardLastChangeTime, auditTime=auditTime, cardUpTime=cardUpTime, cardType=cardType, cmPrimarySID=cmPrimarySID, cardSpareGroupMode=cardSpareGroupMode, equipmentError=equipmentError, cardAdminState=cardAdminState, portEntry=portEntry, auditCardId=auditCardId, shelfName=shelfName, cardShelfId=cardShelfId, cardTestIndex=cardTestIndex, TestSchedule=TestSchedule, cardPicHwVersion=cardPicHwVersion, cardTestDescription=cardTestDescription, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditStatus=auditStatus, cardTestId=cardTestId, cmResetInfo=cmResetInfo, cmDevice=cmDevice, cardBusSpeed=cardBusSpeed, portDescription=portDescription, auditCycleCount=auditCycleCount, auditFailedCount=auditFailedCount, remainInDiagMode=remainInDiagMode, auditLogOutput=auditLogOutput, shelfNumber=shelfNumber, cardTestName=cardTestName, auditTable=auditTable, auditCommand=auditCommand, cmResetUpTime=cmResetUpTime, cmUChannelID=cmUChannelID, cardCpuSpeed=cardCpuSpeed, cardTestSchedule=cardTestSchedule, cardEntry=cardEntry, cardId=cardId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestScheduleCommand=cardTestScheduleCommand, portTrapInh=portTrapInh, cardTempOverHeatNotification=cardTempOverHeatNotification, portNumber=portNumber, equipmentCardGroup=equipmentCardGroup, portType=portType, cardTempNoReportNotification=cardTempNoReportNotification, errEvRowStatus=errEvRowStatus, auditAutoScheduling=auditAutoScheduling, cardTemperatureHighError=cardTemperatureHighError)
