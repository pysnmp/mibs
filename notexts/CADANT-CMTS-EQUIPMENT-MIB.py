...
CardType, PortType, SecondaryState, DiskVolumeUsageLevel, PortMode, ShelfId, PortDetectedMode, CardSubType, CardId, FirmwareSource, EqActionType, PrimaryState, FlowControlMode, DuplexStatus, PicType, AdminState, UnknownState, PortId = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortType", "SecondaryState", "DiskVolumeUsageLevel", "PortMode", "ShelfId", "PortDetectedMode", "CardSubType", "CardId", "FirmwareSource", "EqActionType", "PrimaryState", "FlowControlMode", "DuplexStatus", "PicType", "AdminState", "UnknownState", "PortId")
...
TruthValue, DateAndTime, DisplayString, MacAddress, RowStatus, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "DisplayString", "MacAddress", "RowStatus", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditPassedCount=auditPassedCount, systemGeneralGroup=systemGeneralGroup, portAdminState=portAdminState, cardShelfTestIndex=cardShelfTestIndex, cardShelfId=cardShelfId, cmResetInfo=cmResetInfo, TestId=TestId, portPrStateChange=portPrStateChange, cardRamSize=cardRamSize, cmPrimarySID=cmPrimarySID, portMinDsPower=portMinDsPower, systemGeneral=systemGeneral, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentPortGroup=equipmentPortGroup, cardFwVersion=cardFwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, fpgaErrorEventTable=fpgaErrorEventTable, cardTrapInh=cardTrapInh, cardDetectedChange=cardDetectedChange, cardTestType=cardTestType, auditEntry=auditEntry, errEvLogLevel=errEvLogLevel, portSecStateChange=portSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, portNumChans=portNumChans, portBgpId=portBgpId, cmResetNotification=cmResetNotification, cardTestResultDesc=cardTestResultDesc, cardSecStateChange=cardSecStateChange, cmResetClearNotification=cmResetClearNotification, auditFailedCount=auditFailedCount, shelfNumber=shelfNumber, cmResetStatus=cmResetStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, cmMacAddress=cmMacAddress, cmResetUpTime=cmResetUpTime, portLastChangeTime=portLastChangeTime, TestSchedule=TestSchedule, equipmentCardGroup=equipmentCardGroup, systemClock=systemClock, auditCycleCount=auditCycleCount, portDplxStatus=portDplxStatus, portTxFlowControlMode=portTxFlowControlMode, auditStatus=auditStatus, TestTransactionId=TestTransactionId, cardTemperatureHighError=cardTemperatureHighError, diskVolumeFileName=diskVolumeFileName, cardType=cardType, auditTable=auditTable, cardUpTime=cardUpTime, portDescription=portDescription, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditShelfId=auditShelfId, cardSubDetected=cardSubDetected, portDetectedMode=portDetectedMode, cardTestResultNotification=cardTestResultNotification, diskVolumeId=diskVolumeId, cardCarrierHwVersion=cardCarrierHwVersion, cardId=cardId, cardSubType=cardSubType, portDocsIfIndex=portDocsIfIndex, remainInDiagMode=remainInDiagMode, cardTestScheduleCommand=cardTestScheduleCommand, equipmentState=equipmentState, TestCommand=TestCommand, shelfName=shelfName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestDescription=cardTestDescription, cardPrState=cardPrState, equipmentError=equipmentError, cardPrStateChange=cardPrStateChange, diskVolumeCardId=diskVolumeCardId, cardTestTime=cardTestTime, cadEquipmentMibConformance=cadEquipmentMibConformance, cardFwUpdateStatus=cardFwUpdateStatus, equipmentAudit=equipmentAudit, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, verboseLevel=verboseLevel, cardFlashSize=cardFlashSize, cardPicSerialNum=cardPicSerialNum, cardSerialNum=cardSerialNum, auditAutoScheduling=auditAutoScheduling, trapCounter=trapCounter, cardTestName=cardTestName, cmVendor=cmVendor, cardPicHwVersion=cardPicHwVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, cardEntry=cardEntry, cardSwVersion=cardSwVersion, portShelfId=portShelfId, portTrapInh=portTrapInh, cmResetReason=cmResetReason, auditTotalPassedCount=auditTotalPassedCount, errEvLoggingEnabled=errEvLoggingEnabled, cardPicDetected=cardPicDetected, diskVolumeSize=diskVolumeSize, cardNumber=cardNumber, diskVolumeEntry=diskVolumeEntry, cardTempNoReportNotification=cardTempNoReportNotification, cardSpareGroupMode=cardSpareGroupMode, portConnectorId=portConnectorId, diagTestId=diagTestId, cardFpgaSource=cardFpgaSource, cardCarrierFwVersion=cardCarrierFwVersion, auditName=auditName, portCurrDsPower=portCurrDsPower, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMib=cadEquipmentMib, cardTemperature=cardTemperature, downstreamPowerLoss=downstreamPowerLoss, TestScheduleCommand=TestScheduleCommand, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditCommand=auditCommand, cmUChannelID=cmUChannelID, auditTime=auditTime, cardDetected=cardDetected, auditResult=auditResult, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeShelfId=diskVolumeShelfId, diskVolumeGroup=diskVolumeGroup, shelfSwVersion=shelfSwVersion, portRxFlowControlMode=portRxFlowControlMode, cardTestResult=cardTestResult, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestType=TestType, cardTestTransId=cardTestTransId, diskVolumeDriveId=diskVolumeDriveId, cardTable=cardTable, cmDevice=cmDevice, cardLastChangeTime=cardLastChangeTime, equipmentTbl=equipmentTbl, portId=portId, diskVolumeNumber=diskVolumeNumber, auditId=auditId, auditDescription=auditDescription, equipmentDiagGroup=equipmentDiagGroup, diskDriveNumber=diskDriveNumber, cardSpareGroupId=cardSpareGroupId, equipmentStateGroup=equipmentStateGroup, portAction=portAction, shelfObjects=shelfObjects, cardAdminState=cardAdminState, equipmentTraps=equipmentTraps, portMacIfIndex=portMacIfIndex, diskVolumeTrapInh=diskVolumeTrapInh, cardCPLDVersion=cardCPLDVersion, portPrState=portPrState, cardCarrierSerialNum=cardCarrierSerialNum, portType=portType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestEntry=cardTestEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestSchedule=cardTestSchedule, errEvRowStatus=errEvRowStatus, diskVolumeDiskSize=diskVolumeDiskSize, cardCpuType=cardCpuType, portMaxDsPower=portMaxDsPower, cardNumPorts=cardNumPorts, portNumber=portNumber, consoleOutput=consoleOutput, portTable=portTable, portRxFlowControlDetected=portRxFlowControlDetected, cardTestCommand=cardTestCommand, portEntry=portEntry, equipmentCardTestGroup=equipmentCardTestGroup, cardSecState=cardSecState, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestResult=TestResult, portSecState=portSecState, trapSeverity=trapSeverity, cardName=cardName, cardLastBootSource=cardLastBootSource, errEventId=errEventId, auditLogOutput=auditLogOutput, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeTable=diskVolumeTable, cardDplxStatus=cardDplxStatus, cardHwVersion=cardHwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, cardBusSpeed=cardBusSpeed, cmRegistrationNotification=cmRegistrationNotification, portCardId=portCardId, systemKey=systemKey, cardCpuSpeed=cardCpuSpeed, cardLastResetReason=cardLastResetReason, cmIpAddress=cmIpAddress, diskVolumeName=diskVolumeName, auditCardId=auditCardId, cardBootVersion=cardBootVersion, cardTestId=cardTestId, cardAction=cardAction, auditTotalFailedCount=auditTotalFailedCount, cardLastBootVersion=cardLastBootVersion, cardTestTable=cardTestTable, cardTestIndex=cardTestIndex, auditLogThrottle=auditLogThrottle, portMode=portMode, portCardSubType=portCardSubType, equipmentDiag=equipmentDiag, portMacAddress=portMacAddress, eqDiagConfig=eqDiagConfig, portTxFlowControlDetected=portTxFlowControlDetected, cardDplxStatusChange=cardDplxStatusChange, equipmentShelfGroup=equipmentShelfGroup)
