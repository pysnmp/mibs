...
DiskVolumeUsageLevel, PortMode, ShelfId, EqActionType, CardType, CardId, PrimaryState, AdminState, CardSubType, PortType, PicType, UnknownState, PortDetectedMode, PortId, FlowControlMode, DuplexStatus, SecondaryState, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "PortMode", "ShelfId", "EqActionType", "CardType", "CardId", "PrimaryState", "AdminState", "CardSubType", "PortType", "PicType", "UnknownState", "PortDetectedMode", "PortId", "FlowControlMode", "DuplexStatus", "SecondaryState", "FirmwareSource")
...
TextualConvention, RowStatus, DisplayString, DateAndTime, TruthValue, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "DisplayString", "DateAndTime", "TruthValue", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFwVersion=cardFwVersion, portSecState=portSecState, errEvLoggingEnabled=errEvLoggingEnabled, equipmentTbl=equipmentTbl, portNumber=portNumber, cardTestType=cardTestType, portPrStateChange=portPrStateChange, cardShelfId=cardShelfId, portDocsIfIndex=portDocsIfIndex, cardBusSpeed=cardBusSpeed, portTxFlowControlMode=portTxFlowControlMode, cardTemperatureHighError=cardTemperatureHighError, shelfObjects=shelfObjects, cardUpTime=cardUpTime, portEntry=portEntry, cardNumber=cardNumber, cardSpareGroupId=cardSpareGroupId, diskVolumeShelfId=diskVolumeShelfId, auditCommand=auditCommand, cardTestTime=cardTestTime, cardTestDescription=cardTestDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTrapInh=cardTrapInh, portTable=portTable, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestCommand=cardTestCommand, auditTotalFailedCount=auditTotalFailedCount, cmVendor=cmVendor, errEvLogLevel=errEvLogLevel, cardEntry=cardEntry, diskDriveNumber=diskDriveNumber, equipmentDiag=equipmentDiag, portMacAddress=portMacAddress, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentCardTestGroup=equipmentCardTestGroup, cardCPLDVersion=cardCPLDVersion, portAction=portAction, shelfName=shelfName, equipmentTraps=equipmentTraps, cardName=cardName, diskVolumeCardId=diskVolumeCardId, portAdminState=portAdminState, TestResult=TestResult, TestSchedule=TestSchedule, auditFailedCount=auditFailedCount, diskVolumeName=diskVolumeName, cmResetReason=cmResetReason, cardLastBootSource=cardLastBootSource, auditAutoScheduling=auditAutoScheduling, TestCommand=TestCommand, diskVolumeDiskSize=diskVolumeDiskSize, cmResetStatus=cmResetStatus, auditDescription=auditDescription, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuSpeed=cardCpuSpeed, eqDiagConfig=eqDiagConfig, auditTable=auditTable, cmResetUpTime=cmResetUpTime, auditCardId=auditCardId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmPrimarySID=cmPrimarySID, portCardSubType=portCardSubType, trapCounter=trapCounter, auditName=auditName, cardTestTable=cardTestTable, cardCarrierHwVersion=cardCarrierHwVersion, portPrState=portPrState, TestId=TestId, portBgpId=portBgpId, portNumChans=portNumChans, auditResult=auditResult, remainInDiagMode=remainInDiagMode, TestScheduleCommand=TestScheduleCommand, cardCpuType=cardCpuType, cardDetected=cardDetected, auditStatus=auditStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, cardNumPorts=cardNumPorts, cmDevice=cmDevice, cardTable=cardTable, cardSubDetected=cardSubDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeGroup=diskVolumeGroup, cmResetInfo=cmResetInfo, cmRegistrationNotification=cmRegistrationNotification, shelfSwVersion=shelfSwVersion, auditShelfId=auditShelfId, systemKey=systemKey, trapSeverity=trapSeverity, cardDplxStatusChange=cardDplxStatusChange, systemClock=systemClock, cardLastBootVersion=cardLastBootVersion, cardTestResultNotification=cardTestResultNotification, downstreamPowerLoss=downstreamPowerLoss, cardTestEntry=cardTestEntry, cardPrStateChange=cardPrStateChange, auditCycleCount=auditCycleCount, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardFlashSize=cardFlashSize, cmUChannelID=cmUChannelID, cardPicHwVersion=cardPicHwVersion, shelfNumber=shelfNumber, cardAdminState=cardAdminState, cardHwVersion=cardHwVersion, cmIpAddress=cmIpAddress, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestScheduleCommand=cardTestScheduleCommand, cardSecStateChange=cardSecStateChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardPrState=cardPrState, portDescription=portDescription, cardSerialNum=cardSerialNum, consoleOutput=consoleOutput, equipmentShelfGroup=equipmentShelfGroup, diskVolumeDriveId=diskVolumeDriveId, cardTestSchedule=cardTestSchedule, auditPassedCount=auditPassedCount, diskVolumeFileName=diskVolumeFileName, auditId=auditId, equipmentAuditGroup=equipmentAuditGroup, cardSubType=cardSubType, fpgaErrorEventEntry=fpgaErrorEventEntry, cardId=cardId, cardSpareGroupMode=cardSpareGroupMode, fpgaErrorEventTable=fpgaErrorEventTable, auditTotalPassedCount=auditTotalPassedCount, cmMacAddress=cmMacAddress, diskVolumeEntry=diskVolumeEntry, cardShelfTestIndex=cardShelfTestIndex, auditLogOutput=auditLogOutput, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, systemGeneralGroup=systemGeneralGroup, portType=portType, equipmentAudit=equipmentAudit, diskVolumeId=diskVolumeId, portMacIfIndex=portMacIfIndex, diskVolumeTrapInh=diskVolumeTrapInh, cmResetNotification=cmResetNotification, cardFwUpdateStatus=cardFwUpdateStatus, equipmentPortGroup=equipmentPortGroup, portDplxStatus=portDplxStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetClearNotification=cmResetClearNotification, diskVolumeTable=diskVolumeTable, cardDetectedChange=cardDetectedChange, cardPicSerialNum=cardPicSerialNum, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portTxFlowControlDetected=portTxFlowControlDetected, errEvRowStatus=errEvRowStatus, cardTempNoReportNotification=cardTempNoReportNotification, cardTestIndex=cardTestIndex, cardFpgaSource=cardFpgaSource, portCardId=portCardId, cardDplxStatus=cardDplxStatus, cadEquipmentMib=cadEquipmentMib, cardBootVersion=cardBootVersion, cardTestId=cardTestId, cardTestTransId=cardTestTransId, equipmentStateGroup=equipmentStateGroup, cardPicDetected=cardPicDetected, cardType=cardType, portDplxStatusChange=portDplxStatusChange, cardTemperature=cardTemperature, diskVolumeUsageNotification=diskVolumeUsageNotification, cardLastResetReason=cardLastResetReason, equipmentState=equipmentState, cardTestName=cardTestName, errEventId=errEventId, cardSwVersion=cardSwVersion, portLastChangeTime=portLastChangeTime, auditEntry=auditEntry, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestType=TestType, cardTestResult=cardTestResult, equipmentError=equipmentError, cardSecState=cardSecState, portTrapInh=portTrapInh, portDetectedMode=portDetectedMode, portId=portId, portConnectorId=portConnectorId, portSecStateChange=portSecStateChange, equipmentCardGroup=equipmentCardGroup, portMaxDsPower=portMaxDsPower, verboseLevel=verboseLevel, portRxFlowControlMode=portRxFlowControlMode, TestTransactionId=TestTransactionId, equipmentDiagGroup=equipmentDiagGroup, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeSize=diskVolumeSize, cardCarrierSerialNum=cardCarrierSerialNum, cardRamSize=cardRamSize, diskVolumeNumber=diskVolumeNumber, cardCarrierFwVersion=cardCarrierFwVersion, cardLastChangeTime=cardLastChangeTime, portShelfId=portShelfId, diagTestId=diagTestId, cardAction=cardAction, portMode=portMode, portMinDsPower=portMinDsPower, portCurrDsPower=portCurrDsPower, auditLogThrottle=auditLogThrottle, auditTime=auditTime, systemGeneral=systemGeneral)
