...
EqActionType, FlowControlMode, DiskVolumeUsageLevel, PortMode, AdminState, PrimaryState, CardType, DuplexStatus, ShelfId, PortId, PicType, PortDetectedMode, CardId, PortType, FirmwareSource, UnknownState, CardSubType, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "FlowControlMode", "DiskVolumeUsageLevel", "PortMode", "AdminState", "PrimaryState", "CardType", "DuplexStatus", "ShelfId", "PortId", "PicType", "PortDetectedMode", "CardId", "PortType", "FirmwareSource", "UnknownState", "CardSubType", "SecondaryState")
...
RowStatus, TimeStamp, DateAndTime, DisplayString, MacAddress, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "DateAndTime", "DisplayString", "MacAddress", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditFailedCount=auditFailedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCarrierHwVersion=cardCarrierHwVersion, cardTestType=cardTestType, errEvLogLevel=errEvLogLevel, cadEquipmentMib=cadEquipmentMib, shelfNumber=shelfNumber, cardDplxStatusChange=cardDplxStatusChange, cardPrStateChange=cardPrStateChange, TestType=TestType, portAction=portAction, portDplxStatus=portDplxStatus, portTxFlowControlDetected=portTxFlowControlDetected, portTable=portTable, cardTestCommand=cardTestCommand, trapCounter=trapCounter, cardCarrierSerialNum=cardCarrierSerialNum, TestSchedule=TestSchedule, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTable=cardTestTable, cardEntry=cardEntry, systemKey=systemKey, diskVolumeFileName=diskVolumeFileName, cardTemperatureHighError=cardTemperatureHighError, equipmentTbl=equipmentTbl, auditLogThrottle=auditLogThrottle, portCardId=portCardId, portSecState=portSecState, auditEntry=auditEntry, auditCycleCount=auditCycleCount, systemGeneralGroup=systemGeneralGroup, equipmentStateGroup=equipmentStateGroup, cardSecState=cardSecState, equipmentDiagGroup=equipmentDiagGroup, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibConformance=cadEquipmentMibConformance, cardLastBootSource=cardLastBootSource, portTxFlowControlMode=portTxFlowControlMode, shelfName=shelfName, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardType=cardType, diskVolumeId=diskVolumeId, cardLastResetReason=cardLastResetReason, TestCommand=TestCommand, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portAdminState=portAdminState, portConnectorId=portConnectorId, downstreamPowerLoss=downstreamPowerLoss, cardFwUpdateStatus=cardFwUpdateStatus, cardLastChangeTime=cardLastChangeTime, remainInDiagMode=remainInDiagMode, cardNumPorts=cardNumPorts, cardTestEntry=cardTestEntry, portCardSubType=portCardSubType, cmResetStatus=cmResetStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemGeneral=systemGeneral, cardPrState=cardPrState, cmUChannelID=cmUChannelID, shelfObjects=shelfObjects, auditDescription=auditDescription, eqDiagConfig=eqDiagConfig, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvRecoveryEnabled=errEvRecoveryEnabled, portPrState=portPrState, portNumber=portNumber, auditStatus=auditStatus, auditAutoScheduling=auditAutoScheduling, equipmentError=equipmentError, shelfSwVersion=shelfSwVersion, auditCommand=auditCommand, cardSecStateChange=cardSecStateChange, portLastChangeTime=portLastChangeTime, diskVolumeGroup=diskVolumeGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, TestScheduleCommand=TestScheduleCommand, diskVolumeDriveId=diskVolumeDriveId, cardRamSize=cardRamSize, auditCardId=auditCardId, portMinDsPower=portMinDsPower, portDetectedMode=portDetectedMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeSize=diskVolumeSize, cardTestName=cardTestName, cardLastBootVersion=cardLastBootVersion, TestTransactionId=TestTransactionId, portEntry=portEntry, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portCurrDsPower=portCurrDsPower, cardCpuType=cardCpuType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diagTestId=diagTestId, cardPicSerialNum=cardPicSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestId=cardTestId, cmPrimarySID=cmPrimarySID, cardDetected=cardDetected, cardTestSchedule=cardTestSchedule, equipmentCardTestGroup=equipmentCardTestGroup, cardName=cardName, cmResetReason=cmResetReason, diskVolumeName=diskVolumeName, fpgaErrorEventTable=fpgaErrorEventTable, portRxFlowControlMode=portRxFlowControlMode, cardTrapInh=cardTrapInh, portMode=portMode, equipmentAuditGroup=equipmentAuditGroup, trapSeverity=trapSeverity, diskVolumeNumber=diskVolumeNumber, auditShelfId=auditShelfId, diskDriveNumber=diskDriveNumber, cardTemperature=cardTemperature, diskVolumeTrapInh=diskVolumeTrapInh, cardCpuSpeed=cardCpuSpeed, auditResult=auditResult, equipmentCardGroup=equipmentCardGroup, cardSubType=cardSubType, TestId=TestId, auditTime=auditTime, portDescription=portDescription, cardSpareGroupId=cardSpareGroupId, cardSerialNum=cardSerialNum, portMacAddress=portMacAddress, portTrapInh=portTrapInh, cmResetClearNotification=cmResetClearNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestResultNotification=cardTestResultNotification, auditName=auditName, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSpareGroupMode=cardSpareGroupMode, portNumChans=portNumChans, cardTestResult=cardTestResult, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardDetectedChange=cardDetectedChange, cardPicHwVersion=cardPicHwVersion, auditId=auditId, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentState=equipmentState, errEventId=errEventId, diskVolumeCardId=diskVolumeCardId, auditTable=auditTable, cmIpAddress=cmIpAddress, diskVolumeDiskSize=diskVolumeDiskSize, cardFwVersion=cardFwVersion, diskVolumeTable=diskVolumeTable, portShelfId=portShelfId, cmDevice=cmDevice, cardShelfTestIndex=cardShelfTestIndex, cardTestIndex=cardTestIndex, cardUpTime=cardUpTime, cardFpgaSource=cardFpgaSource, errEvRowStatus=errEvRowStatus, cardSwVersion=cardSwVersion, verboseLevel=verboseLevel, portId=portId, cardTestTime=cardTestTime, cardHwVersion=cardHwVersion, diskVolumeEntry=diskVolumeEntry, cardNumber=cardNumber, cardId=cardId, auditPassedCount=auditPassedCount, cardCarrierFwVersion=cardCarrierFwVersion, cardTable=cardTable, equipmentDiag=equipmentDiag, diskVolumeShelfId=diskVolumeShelfId, portDocsIfIndex=portDocsIfIndex, cardBusSpeed=cardBusSpeed, cardFlashSize=cardFlashSize, cardTestDescription=cardTestDescription, consoleOutput=consoleOutput, diskVolumeUsageLevel=diskVolumeUsageLevel, cardPicDetected=cardPicDetected, portDplxStatusChange=portDplxStatusChange, cmMacAddress=cmMacAddress, cardTestResultDesc=cardTestResultDesc, cardBootVersion=cardBootVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, portSecStateChange=portSecStateChange, cardAction=cardAction, cmResetInfo=cmResetInfo, systemClock=systemClock, cardTempOverHeatNotification=cardTempOverHeatNotification, portBgpId=portBgpId, cardSubDetected=cardSubDetected, equipmentAudit=equipmentAudit, equipmentTraps=equipmentTraps, TestResult=TestResult, auditTotalPassedCount=auditTotalPassedCount, cardCPLDVersion=cardCPLDVersion, cmResetUpTime=cmResetUpTime, cmVendor=cmVendor, portMacIfIndex=portMacIfIndex, cardTestTransId=cardTestTransId, auditLogOutput=auditLogOutput, cardAdminState=cardAdminState, cmRegistrationNotification=cmRegistrationNotification, auditTotalFailedCount=auditTotalFailedCount, cardDplxStatus=cardDplxStatus, cardTempNoReportNotification=cardTempNoReportNotification, cardShelfId=cardShelfId, portPrStateChange=portPrStateChange, portType=portType, portMaxDsPower=portMaxDsPower, cmResetNotification=cmResetNotification)
