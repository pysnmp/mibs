...
FirmwareSource, EqActionType, CardSubType, PrimaryState, CardId, PortId, CardType, PortDetectedMode, ShelfId, DuplexStatus, PortMode, DiskVolumeUsageLevel, FlowControlMode, PortType, UnknownState, SecondaryState, PicType, AdminState = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "EqActionType", "CardSubType", "PrimaryState", "CardId", "PortId", "CardType", "PortDetectedMode", "ShelfId", "DuplexStatus", "PortMode", "DiskVolumeUsageLevel", "FlowControlMode", "PortType", "UnknownState", "SecondaryState", "PicType", "AdminState")
...
TextualConvention, MacAddress, DisplayString, DateAndTime, TruthValue, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "DisplayString", "DateAndTime", "TruthValue", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestScheduleCommand=TestScheduleCommand, shelfName=shelfName, cardNumPorts=cardNumPorts, diskVolumeDriveId=diskVolumeDriveId, portMacAddress=portMacAddress, portDocsIfIndex=portDocsIfIndex, portLastChangeTime=portLastChangeTime, auditId=auditId, cmRegistrationNotification=cmRegistrationNotification, cardTrapInh=cardTrapInh, cardPicDetected=cardPicDetected, portDplxStatus=portDplxStatus, cardSpareGroupMode=cardSpareGroupMode, cardTestTime=cardTestTime, diskVolumeNumber=diskVolumeNumber, portDplxStatusChange=portDplxStatusChange, cardTestName=cardTestName, cardShelfId=cardShelfId, errEvLogLevel=errEvLogLevel, portId=portId, portMode=portMode, portTxFlowControlMode=portTxFlowControlMode, diskVolumeShelfId=diskVolumeShelfId, portNumber=portNumber, portNumChans=portNumChans, equipmentState=equipmentState, fpgaErrorEventTable=fpgaErrorEventTable, portAdminState=portAdminState, cardTestCommand=cardTestCommand, cmIpAddress=cmIpAddress, portMacIfIndex=portMacIfIndex, cardPicHwVersion=cardPicHwVersion, TestCommand=TestCommand, diskVolumeEntry=diskVolumeEntry, diskVolumeDiskSize=diskVolumeDiskSize, eqDiagConfig=eqDiagConfig, cardTestId=cardTestId, portBgpId=portBgpId, cardBusSpeed=cardBusSpeed, cadEquipmentMibGroup=cadEquipmentMibGroup, trapSeverity=trapSeverity, diskVolumeFileName=diskVolumeFileName, auditDescription=auditDescription, cardTempOverHeatNotification=cardTempOverHeatNotification, auditResult=auditResult, portAction=portAction, consoleOutput=consoleOutput, auditShelfId=auditShelfId, portDescription=portDescription, cardShelfTestIndex=cardShelfTestIndex, auditTotalFailedCount=auditTotalFailedCount, cmResetNotification=cmResetNotification, cardFlashSize=cardFlashSize, diskVolumeSize=diskVolumeSize, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTable=cardTable, equipmentError=equipmentError, equipmentCardGroup=equipmentCardGroup, cardAction=cardAction, portEntry=portEntry, cardNumber=cardNumber, cardSwVersion=cardSwVersion, cardTestEntry=cardTestEntry, diskDriveNumber=diskDriveNumber, equipmentCardTestGroup=equipmentCardTestGroup, portMaxDsPower=portMaxDsPower, cmResetInfo=cmResetInfo, trapCounter=trapCounter, cardSubDetected=cardSubDetected, portType=portType, cardTempNoReportNotification=cardTempNoReportNotification, equipmentDiag=equipmentDiag, downstreamPowerLoss=downstreamPowerLoss, auditName=auditName, verboseLevel=verboseLevel, cmUChannelID=cmUChannelID, diskVolumeId=diskVolumeId, cardCarrierSerialNum=cardCarrierSerialNum, auditAutoScheduling=auditAutoScheduling, portCardSubType=portCardSubType, portShelfId=portShelfId, cardName=cardName, cardCPLDVersion=cardCPLDVersion, auditStatus=auditStatus, cardSpareGroupId=cardSpareGroupId, TestTransactionId=TestTransactionId, portTxFlowControlDetected=portTxFlowControlDetected, auditTotalPassedCount=auditTotalPassedCount, portSecStateChange=portSecStateChange, auditTime=auditTime, equipmentAudit=equipmentAudit, cmResetReason=cmResetReason, shelfNumber=shelfNumber, portPrStateChange=portPrStateChange, auditPassedCount=auditPassedCount, portTrapInh=portTrapInh, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCpuSpeed=cardCpuSpeed, fpgaErrorEventEntry=fpgaErrorEventEntry, cardDetectedChange=cardDetectedChange, cardLastBootSource=cardLastBootSource, cardCarrierFwVersion=cardCarrierFwVersion, cardCpuType=cardCpuType, TestType=TestType, shelfObjects=shelfObjects, cmDevice=cmDevice, errEvRowStatus=errEvRowStatus, cardPrStateChange=cardPrStateChange, cardFwVersion=cardFwVersion, cardAdminState=cardAdminState, cardDetected=cardDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetClearNotification=cmResetClearNotification, TestSchedule=TestSchedule, cardHwVersion=cardHwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cmVendor=cmVendor, equipmentDiagGroup=equipmentDiagGroup, equipmentAuditGroup=equipmentAuditGroup, equipmentTraps=equipmentTraps, cardSecState=cardSecState, portPrState=portPrState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diagTestId=diagTestId, portDetectedMode=portDetectedMode, cardFwUpdateStatus=cardFwUpdateStatus, cmMacAddress=cmMacAddress, cmResetStatus=cmResetStatus, cardSerialNum=cardSerialNum, cardTestType=cardTestType, cardDplxStatus=cardDplxStatus, portSecState=portSecState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestResult=TestResult, cardTestScheduleCommand=cardTestScheduleCommand, equipmentPortGroup=equipmentPortGroup, auditCardId=auditCardId, cardLastChangeTime=cardLastChangeTime, cardUpTime=cardUpTime, cardDplxStatusChange=cardDplxStatusChange, cardRamSize=cardRamSize, auditCommand=auditCommand, shelfSwVersion=shelfSwVersion, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeTable=diskVolumeTable, systemKey=systemKey, cardTemperature=cardTemperature, auditTable=auditTable, systemClock=systemClock, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeCardId=diskVolumeCardId, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPrState=cardPrState, diskVolumeGroup=diskVolumeGroup, equipmentTbl=equipmentTbl, cardLastResetReason=cardLastResetReason, portCardId=portCardId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResultNotification=cardTestResultNotification, cardFpgaSource=cardFpgaSource, auditLogOutput=auditLogOutput, cardSubType=cardSubType, PYSNMP_MODULE_ID=cadEquipmentMib, cardPicSerialNum=cardPicSerialNum, portConnectorId=portConnectorId, remainInDiagMode=remainInDiagMode, cardTestDescription=cardTestDescription, auditEntry=auditEntry, cardSecStateChange=cardSecStateChange, cadEquipmentMib=cadEquipmentMib, auditFailedCount=auditFailedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, portRxFlowControlMode=portRxFlowControlMode, portMinDsPower=portMinDsPower, auditLogThrottle=auditLogThrottle, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetUpTime=cmResetUpTime, cardId=cardId, cardType=cardType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardEntry=cardEntry, cardLastBootVersion=cardLastBootVersion, cardTestTransId=cardTestTransId, TestId=TestId, cardBootVersion=cardBootVersion, cardTestResult=cardTestResult, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, errEventId=errEventId, auditCycleCount=auditCycleCount, cardTestIndex=cardTestIndex, cardTestTable=cardTestTable, diskVolumeName=diskVolumeName, cardTestSchedule=cardTestSchedule, systemGeneralGroup=systemGeneralGroup, equipmentStateGroup=equipmentStateGroup, portCurrDsPower=portCurrDsPower, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTemperatureHighError=cardTemperatureHighError, portTable=portTable, cardCarrierHwVersion=cardCarrierHwVersion, systemGeneral=systemGeneral)
