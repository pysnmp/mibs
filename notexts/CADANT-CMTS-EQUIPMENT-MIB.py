...
PortDetectedMode, FlowControlMode, DuplexStatus, UnknownState, AdminState, CardType, PortType, EqActionType, DiskVolumeUsageLevel, FirmwareSource, ShelfId, PrimaryState, PicType, CardId, PortMode, SecondaryState, PortId, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "FlowControlMode", "DuplexStatus", "UnknownState", "AdminState", "CardType", "PortType", "EqActionType", "DiskVolumeUsageLevel", "FirmwareSource", "ShelfId", "PrimaryState", "PicType", "CardId", "PortMode", "SecondaryState", "PortId", "CardSubType")
...
DisplayString, TruthValue, MacAddress, DateAndTime, TimeStamp, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TruthValue", "MacAddress", "DateAndTime", "TimeStamp", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditLogOutput=auditLogOutput, cardSubDetected=cardSubDetected, portTxFlowControlMode=portTxFlowControlMode, cardDetectedChange=cardDetectedChange, trapSeverity=trapSeverity, cardSpareGroupMode=cardSpareGroupMode, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTrapInh=cardTrapInh, eqDiagConfig=eqDiagConfig, cardTestEntry=cardTestEntry, PYSNMP_MODULE_ID=cadEquipmentMib, portRxFlowControlMode=portRxFlowControlMode, cardFwUpdateStatus=cardFwUpdateStatus, cardFpgaSource=cardFpgaSource, portShelfId=portShelfId, cardTestTable=cardTestTable, cardNumber=cardNumber, cardTable=cardTable, diskVolumeCardId=diskVolumeCardId, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicHwVersion=cardPicHwVersion, auditTotalPassedCount=auditTotalPassedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestCommand=cardTestCommand, equipmentShelfGroup=equipmentShelfGroup, cardLastBootSource=cardLastBootSource, trapCounter=trapCounter, shelfNumber=shelfNumber, auditFailedCount=auditFailedCount, equipmentAuditGroup=equipmentAuditGroup, equipmentTraps=equipmentTraps, diskVolumeShelfId=diskVolumeShelfId, portTable=portTable, cardSwVersion=cardSwVersion, equipmentTbl=equipmentTbl, cardRamSize=cardRamSize, portNumber=portNumber, errEvLoggingEnabled=errEvLoggingEnabled, equipmentState=equipmentState, cmResetInfo=cmResetInfo, cardTestTime=cardTestTime, cmUChannelID=cmUChannelID, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portCardSubType=portCardSubType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditDescription=auditDescription, portAdminState=portAdminState, cardUpTime=cardUpTime, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portPrStateChange=portPrStateChange, cardTestId=cardTestId, cmIpAddress=cmIpAddress, equipmentAudit=equipmentAudit, TestSchedule=TestSchedule, cardTestName=cardTestName, diskVolumeDriveId=diskVolumeDriveId, portDplxStatus=portDplxStatus, cardBusSpeed=cardBusSpeed, portDocsIfIndex=portDocsIfIndex, diskVolumeFileName=diskVolumeFileName, portId=portId, auditId=auditId, cmMacAddress=cmMacAddress, cardCarrierHwVersion=cardCarrierHwVersion, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetStatus=cmResetStatus, cardCPLDVersion=cardCPLDVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPrStateChange=cardPrStateChange, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMibGroup=cadEquipmentMibGroup, auditName=auditName, cardCpuSpeed=cardCpuSpeed, portDescription=portDescription, portMaxDsPower=portMaxDsPower, cardTestDescription=cardTestDescription, cardTemperature=cardTemperature, cadEquipmentMib=cadEquipmentMib, TestTransactionId=TestTransactionId, diskVolumeUsageLevel=diskVolumeUsageLevel, auditShelfId=auditShelfId, auditAutoScheduling=auditAutoScheduling, cardAdminState=cardAdminState, errEvRecoveryEnabled=errEvRecoveryEnabled, portConnectorId=portConnectorId, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, portMode=portMode, diskVolumeNumber=diskVolumeNumber, auditStatus=auditStatus, cardNumPorts=cardNumPorts, cardName=cardName, diagTestId=diagTestId, cardTestResultNotification=cardTestResultNotification, portSecStateChange=portSecStateChange, portTxFlowControlDetected=portTxFlowControlDetected, cardShelfId=cardShelfId, cmVendor=cmVendor, equipmentCardGroup=equipmentCardGroup, equipmentPortGroup=equipmentPortGroup, auditCardId=auditCardId, cardTempNoReportNotification=cardTempNoReportNotification, cardHwVersion=cardHwVersion, TestResult=TestResult, diskVolumeEntry=diskVolumeEntry, cardCpuType=cardCpuType, cardTestTransId=cardTestTransId, equipmentDiagGroup=equipmentDiagGroup, cardBootVersion=cardBootVersion, cardDetected=cardDetected, errEventId=errEventId, portCurrDsPower=portCurrDsPower, cardPicDetected=cardPicDetected, verboseLevel=verboseLevel, cardDplxStatusChange=cardDplxStatusChange, cardSecStateChange=cardSecStateChange, cardTestIndex=cardTestIndex, cardAction=cardAction, diskVolumeName=diskVolumeName, TestType=TestType, cardFwVersion=cardFwVersion, equipmentStateGroup=equipmentStateGroup, equipmentDiag=equipmentDiag, remainInDiagMode=remainInDiagMode, auditEntry=auditEntry, TestId=TestId, cardLastChangeTime=cardLastChangeTime, shelfObjects=shelfObjects, cardSerialNum=cardSerialNum, TestCommand=TestCommand, auditResult=auditResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portTrapInh=portTrapInh, equipmentError=equipmentError, portLastChangeTime=portLastChangeTime, portEntry=portEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portMinDsPower=portMinDsPower, portRxFlowControlDetected=portRxFlowControlDetected, shelfName=shelfName, cardTestSchedule=cardTestSchedule, cardLastResetReason=cardLastResetReason, portMacIfIndex=portMacIfIndex, portDetectedMode=portDetectedMode, auditTime=auditTime, cmResetClearNotification=cmResetClearNotification, auditTable=auditTable, diskVolumeTable=diskVolumeTable, cardDplxStatus=cardDplxStatus, auditCycleCount=auditCycleCount, portPrState=portPrState, consoleOutput=consoleOutput, systemGeneralGroup=systemGeneralGroup, portSecState=portSecState, portAction=portAction, cardTestType=cardTestType, cardPicSerialNum=cardPicSerialNum, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardEntry=cardEntry, portType=portType, cmRegistrationNotification=cmRegistrationNotification, cardPrState=cardPrState, TestScheduleCommand=TestScheduleCommand, systemKey=systemKey, cardId=cardId, portNumChans=portNumChans, cmPrimarySID=cmPrimarySID, portCardId=portCardId, portDplxStatusChange=portDplxStatusChange, diskDriveNumber=diskDriveNumber, cardFlashSize=cardFlashSize, cardTemperatureHighError=cardTemperatureHighError, cardTestResult=cardTestResult, cmResetUpTime=cmResetUpTime, errEvRowStatus=errEvRowStatus, cardShelfTestIndex=cardShelfTestIndex, downstreamPowerLoss=downstreamPowerLoss, cmResetReason=cmResetReason, cardSecState=cardSecState, cmResetNotification=cmResetNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, portMacAddress=portMacAddress, auditLogThrottle=auditLogThrottle, cmDevice=cmDevice, shelfSwVersion=shelfSwVersion, cardType=cardType, systemGeneral=systemGeneral, cardCarrierFwVersion=cardCarrierFwVersion, systemClock=systemClock, diskVolumeId=diskVolumeId, diskVolumeTrapInh=diskVolumeTrapInh, cardSpareGroupId=cardSpareGroupId, cardSubType=cardSubType, cardTestResultDesc=cardTestResultDesc, cardTestScheduleCommand=cardTestScheduleCommand, auditCommand=auditCommand, fpgaErrorEventTable=fpgaErrorEventTable, errEvLogLevel=errEvLogLevel, auditPassedCount=auditPassedCount, diskVolumeSize=diskVolumeSize, cardCarrierSerialNum=cardCarrierSerialNum, portBgpId=portBgpId, auditTotalFailedCount=auditTotalFailedCount, fpgaErrorEventGroup=fpgaErrorEventGroup)
