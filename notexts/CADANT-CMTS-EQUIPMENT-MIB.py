...
ShelfId, DuplexStatus, PortType, PicType, PrimaryState, FirmwareSource, CardSubType, CardId, FlowControlMode, CardType, AdminState, DiskVolumeUsageLevel, PortId, PortMode, PortDetectedMode, SecondaryState, EqActionType, UnknownState = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "DuplexStatus", "PortType", "PicType", "PrimaryState", "FirmwareSource", "CardSubType", "CardId", "FlowControlMode", "CardType", "AdminState", "DiskVolumeUsageLevel", "PortId", "PortMode", "PortDetectedMode", "SecondaryState", "EqActionType", "UnknownState")
...
MacAddress, RowStatus, TruthValue, TimeStamp, DisplayString, TextualConvention, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "TruthValue", "TimeStamp", "DisplayString", "TextualConvention", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastBootSource=cardLastBootSource, cardTestSchedule=cardTestSchedule, portCardId=portCardId, cardPrStateChange=cardPrStateChange, cardEntry=cardEntry, portType=portType, auditShelfId=auditShelfId, cardTrapInh=cardTrapInh, portMacAddress=portMacAddress, diskVolumeCardId=diskVolumeCardId, portCurrDsPower=portCurrDsPower, trapSeverity=trapSeverity, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetInfo=cmResetInfo, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portMaxDsPower=portMaxDsPower, cardSecState=cardSecState, cardDplxStatus=cardDplxStatus, equipmentTraps=equipmentTraps, cardDplxStatusChange=cardDplxStatusChange, cardBootVersion=cardBootVersion, cardFlashSize=cardFlashSize, auditLogOutput=auditLogOutput, TestCommand=TestCommand, equipmentState=equipmentState, cardUpTime=cardUpTime, portBgpId=portBgpId, auditAutoScheduling=auditAutoScheduling, cardTable=cardTable, cmResetNotification=cmResetNotification, portNumChans=portNumChans, portTrapInh=portTrapInh, auditTotalFailedCount=auditTotalFailedCount, cmUChannelID=cmUChannelID, errEvRecoveryEnabled=errEvRecoveryEnabled, cardAction=cardAction, portTxFlowControlDetected=portTxFlowControlDetected, portSecStateChange=portSecStateChange, equipmentAudit=equipmentAudit, portDplxStatusChange=portDplxStatusChange, portDetectedMode=portDetectedMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diagTestId=diagTestId, systemClock=systemClock, portRxFlowControlMode=portRxFlowControlMode, cardCpuSpeed=cardCpuSpeed, shelfSwVersion=shelfSwVersion, cardHwVersion=cardHwVersion, cardPicHwVersion=cardPicHwVersion, cardAdminState=cardAdminState, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeShelfId=diskVolumeShelfId, cardTestEntry=cardTestEntry, cardTestTransId=cardTestTransId, cardCarrierFwVersion=cardCarrierFwVersion, portConnectorId=portConnectorId, portPrStateChange=portPrStateChange, auditName=auditName, cmResetStatus=cmResetStatus, portTxFlowControlMode=portTxFlowControlMode, cmPrimarySID=cmPrimarySID, cardPrState=cardPrState, portDplxStatus=portDplxStatus, diskVolumeTable=diskVolumeTable, diskVolumeUsagePercentage=diskVolumeUsagePercentage, fpgaErrorEventTable=fpgaErrorEventTable, portAdminState=portAdminState, cardSpareGroupId=cardSpareGroupId, cardId=cardId, portDocsIfIndex=portDocsIfIndex, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portTable=portTable, portEntry=portEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, auditLogThrottle=auditLogThrottle, TestScheduleCommand=TestScheduleCommand, downstreamPowerLoss=downstreamPowerLoss, cardTestIndex=cardTestIndex, cardShelfId=cardShelfId, cardTestResult=cardTestResult, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardRamSize=cardRamSize, cardPicDetected=cardPicDetected, verboseLevel=verboseLevel, cmDevice=cmDevice, shelfNumber=shelfNumber, cardTestCommand=cardTestCommand, auditDescription=auditDescription, diskVolumeSize=diskVolumeSize, portLastChangeTime=portLastChangeTime, eqDiagConfig=eqDiagConfig, portDescription=portDescription, cardCpuType=cardCpuType, cardTempNoReportNotification=cardTempNoReportNotification, cardFwVersion=cardFwVersion, cardTestScheduleCommand=cardTestScheduleCommand, cardTestType=cardTestType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetReason=cmResetReason, cardCPLDVersion=cardCPLDVersion, cardCarrierHwVersion=cardCarrierHwVersion, auditTable=auditTable, cardNumber=cardNumber, TestId=TestId, equipmentStateGroup=equipmentStateGroup, cardSwVersion=cardSwVersion, portSecState=portSecState, diskVolumeGroup=diskVolumeGroup, equipmentPortGroup=equipmentPortGroup, remainInDiagMode=remainInDiagMode, cardShelfTestIndex=cardShelfTestIndex, errEventId=errEventId, cardTestDescription=cardTestDescription, portId=portId, cardTestTable=cardTestTable, cardDetected=cardDetected, equipmentDiag=equipmentDiag, auditCardId=auditCardId, equipmentShelfGroup=equipmentShelfGroup, cardSubDetected=cardSubDetected, auditTime=auditTime, auditCommand=auditCommand, diskVolumeDiskSize=diskVolumeDiskSize, fpgaErrorEventEntry=fpgaErrorEventEntry, auditCycleCount=auditCycleCount, equipmentError=equipmentError, portMacIfIndex=portMacIfIndex, diskVolumeFileName=diskVolumeFileName, cardTemperature=cardTemperature, errEvRowStatus=errEvRowStatus, cardSpareGroupMode=cardSpareGroupMode, cardTempOverHeatNotification=cardTempOverHeatNotification, auditPassedCount=auditPassedCount, errEvLogLevel=errEvLogLevel, cmIpAddress=cmIpAddress, cardLastBootVersion=cardLastBootVersion, cardTestTime=cardTestTime, cardTestResultNotification=cardTestResultNotification, cmResetClearNotification=cmResetClearNotification, cardTestId=cardTestId, auditFailedCount=auditFailedCount, auditTotalPassedCount=auditTotalPassedCount, systemKey=systemKey, systemGeneral=systemGeneral, cardName=cardName, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSubType=cardSubType, TestTransactionId=TestTransactionId, equipmentAuditGroup=equipmentAuditGroup, equipmentCardTestGroup=equipmentCardTestGroup, PYSNMP_MODULE_ID=cadEquipmentMib, portRxFlowControlDetected=portRxFlowControlDetected, equipmentTbl=equipmentTbl, portNumber=portNumber, cardCarrierSerialNum=cardCarrierSerialNum, consoleOutput=consoleOutput, cmMacAddress=cmMacAddress, equipmentDiagGroup=equipmentDiagGroup, diskVolumeEntry=diskVolumeEntry, portShelfId=portShelfId, diskVolumeId=diskVolumeId, cmVendor=cmVendor, cadEquipmentMib=cadEquipmentMib, portMode=portMode, diskDriveNumber=diskDriveNumber, cardFpgaSource=cardFpgaSource, errEvLoggingEnabled=errEvLoggingEnabled, TestSchedule=TestSchedule, portMinDsPower=portMinDsPower, diskVolumeNumber=diskVolumeNumber, shelfObjects=shelfObjects, cardTestName=cardTestName, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastChangeTime=cardLastChangeTime, TestResult=TestResult, auditStatus=auditStatus, cardDetectedChange=cardDetectedChange, cardTemperatureHighWarn=cardTemperatureHighWarn, fpgaErrorEventGroup=fpgaErrorEventGroup, cardNumPorts=cardNumPorts, portAction=portAction, auditId=auditId, diskVolumeDriveId=diskVolumeDriveId, cardFwUpdateStatus=cardFwUpdateStatus, shelfName=shelfName, diskVolumeName=diskVolumeName, auditEntry=auditEntry, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardSerialNum=cardSerialNum, cardTemperatureHighError=cardTemperatureHighError, diskVolumeUsageNotification=diskVolumeUsageNotification, cardBusSpeed=cardBusSpeed, cardPicSerialNum=cardPicSerialNum, TestType=TestType, auditResult=auditResult, cardType=cardType, portPrState=portPrState, trapCounter=trapCounter, cardSecStateChange=cardSecStateChange, cardTestResultDesc=cardTestResultDesc, portCardSubType=portCardSubType, systemGeneralGroup=systemGeneralGroup, equipmentCardGroup=equipmentCardGroup, cmResetUpTime=cmResetUpTime, cardLastResetReason=cardLastResetReason)
