...
FirmwareSource, PortMode, AdminState, CardType, PortType, ShelfId, DiskVolumeUsageLevel, PrimaryState, FlowControlMode, SecondaryState, EqActionType, PicType, CardId, CardSubType, PortId, UnknownState, DuplexStatus, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "PortMode", "AdminState", "CardType", "PortType", "ShelfId", "DiskVolumeUsageLevel", "PrimaryState", "FlowControlMode", "SecondaryState", "EqActionType", "PicType", "CardId", "CardSubType", "PortId", "UnknownState", "DuplexStatus", "PortDetectedMode")
...
TruthValue, DisplayString, MacAddress, TimeStamp, DateAndTime, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DisplayString", "MacAddress", "TimeStamp", "DateAndTime", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSecStateChange=cardSecStateChange, auditAutoScheduling=auditAutoScheduling, cardFlashSize=cardFlashSize, portDplxStatusChange=portDplxStatusChange, cardTestIndex=cardTestIndex, equipmentPortGroup=equipmentPortGroup, cmIpAddress=cmIpAddress, equipmentDiag=equipmentDiag, cardId=cardId, equipmentAuditGroup=equipmentAuditGroup, auditStatus=auditStatus, portLastChangeTime=portLastChangeTime, auditCommand=auditCommand, auditLogThrottle=auditLogThrottle, cardType=cardType, auditTable=auditTable, systemKey=systemKey, cardHwVersion=cardHwVersion, cardDetectedChange=cardDetectedChange, cardCarrierSerialNum=cardCarrierSerialNum, eqDiagConfig=eqDiagConfig, shelfSwVersion=shelfSwVersion, cardAdminState=cardAdminState, portEntry=portEntry, portCardSubType=portCardSubType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeCardId=diskVolumeCardId, diskVolumeDiskSize=diskVolumeDiskSize, cmMacAddress=cmMacAddress, cardLastResetReason=cardLastResetReason, cardCpuSpeed=cardCpuSpeed, portMaxDsPower=portMaxDsPower, cardTestScheduleCommand=cardTestScheduleCommand, cardSwVersion=cardSwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, portId=portId, cardTable=cardTable, equipmentTbl=equipmentTbl, equipmentCardTestGroup=equipmentCardTestGroup, cmVendor=cmVendor, portTable=portTable, trapSeverity=trapSeverity, portRxFlowControlDetected=portRxFlowControlDetected, cardSpareGroupId=cardSpareGroupId, cardTrapInh=cardTrapInh, shelfNumber=shelfNumber, equipmentDiagGroup=equipmentDiagGroup, diskVolumeNumber=diskVolumeNumber, errEvRecoveryEnabled=errEvRecoveryEnabled, TestCommand=TestCommand, cardTemperature=cardTemperature, cardDplxStatus=cardDplxStatus, cmDevice=cmDevice, portDplxStatus=portDplxStatus, auditName=auditName, remainInDiagMode=remainInDiagMode, equipmentCardGroup=equipmentCardGroup, auditCycleCount=auditCycleCount, equipmentTraps=equipmentTraps, cardTestCommand=cardTestCommand, shelfObjects=shelfObjects, portConnectorId=portConnectorId, cardTestName=cardTestName, portPrState=portPrState, portMacAddress=portMacAddress, diskVolumeSize=diskVolumeSize, portDocsIfIndex=portDocsIfIndex, equipmentState=equipmentState, auditId=auditId, cardName=cardName, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestResult=cardTestResult, cardNumber=cardNumber, cardPrState=cardPrState, auditPassedCount=auditPassedCount, portAdminState=portAdminState, cardEntry=cardEntry, cardFpgaSource=cardFpgaSource, downstreamPowerLoss=downstreamPowerLoss, cmResetUpTime=cmResetUpTime, fpgaErrorEventTable=fpgaErrorEventTable, cmResetInfo=cmResetInfo, cardSecState=cardSecState, diskVolumeGroup=diskVolumeGroup, auditFailedCount=auditFailedCount, auditCardId=auditCardId, cardPicSerialNum=cardPicSerialNum, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetNotification=cmResetNotification, cardShelfId=cardShelfId, cardLastBootSource=cardLastBootSource, cardLastChangeTime=cardLastChangeTime, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestResultDesc=cardTestResultDesc, trapCounter=trapCounter, equipmentAudit=equipmentAudit, cardPrStateChange=cardPrStateChange, errEventId=errEventId, cardTemperatureHighError=cardTemperatureHighError, cardCarrierFwVersion=cardCarrierFwVersion, portNumChans=portNumChans, cmPrimarySID=cmPrimarySID, portSecStateChange=portSecStateChange, cardPicHwVersion=cardPicHwVersion, cardSubType=cardSubType, cardPicDetected=cardPicDetected, diskVolumeEntry=diskVolumeEntry, auditTotalPassedCount=auditTotalPassedCount, cmResetReason=cmResetReason, cardFwVersion=cardFwVersion, auditDescription=auditDescription, portType=portType, cardTestDescription=cardTestDescription, portNumber=portNumber, cardTestTransId=cardTestTransId, portCurrDsPower=portCurrDsPower, errEvRowStatus=errEvRowStatus, portDetectedMode=portDetectedMode, portRxFlowControlMode=portRxFlowControlMode, cardUpTime=cardUpTime, auditShelfId=auditShelfId, equipmentError=equipmentError, PYSNMP_MODULE_ID=cadEquipmentMib, portMinDsPower=portMinDsPower, portDescription=portDescription, auditEntry=auditEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, portMode=portMode, equipmentShelfGroup=equipmentShelfGroup, cardCPLDVersion=cardCPLDVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, cardFwUpdateStatus=cardFwUpdateStatus, cardTestSchedule=cardTestSchedule, cmUChannelID=cmUChannelID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestSchedule=TestSchedule, TestScheduleCommand=TestScheduleCommand, cardTestEntry=cardTestEntry, diskVolumeDriveId=diskVolumeDriveId, diagTestId=diagTestId, systemGeneral=systemGeneral, portMacIfIndex=portMacIfIndex, portAction=portAction, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeTrapInh=diskVolumeTrapInh, shelfName=shelfName, diskVolumeName=diskVolumeName, auditLogOutput=auditLogOutput, cardTestId=cardTestId, diskVolumeTable=diskVolumeTable, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentStateGroup=equipmentStateGroup, verboseLevel=verboseLevel, errEvLoggingEnabled=errEvLoggingEnabled, consoleOutput=consoleOutput, cardTestType=cardTestType, cmResetClearNotification=cmResetClearNotification, diskDriveNumber=diskDriveNumber, cardSubDetected=cardSubDetected, cmResetStatus=cmResetStatus, portPrStateChange=portPrStateChange, cardTestTime=cardTestTime, cadEquipmentMib=cadEquipmentMib, cardTestResultNotification=cardTestResultNotification, cardCpuType=cardCpuType, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditResult=auditResult, errEvLogLevel=errEvLogLevel, portCardId=portCardId, auditTotalFailedCount=auditTotalFailedCount, portShelfId=portShelfId, portBgpId=portBgpId, cardLastBootVersion=cardLastBootVersion, diskVolumeId=diskVolumeId, TestResult=TestResult, cardBusSpeed=cardBusSpeed, cardRamSize=cardRamSize, cardShelfTestIndex=cardShelfTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestId=TestId, auditTime=auditTime, cardTemperatureHighWarn=cardTemperatureHighWarn, systemClock=systemClock, portTrapInh=portTrapInh, TestType=TestType, portTxFlowControlDetected=portTxFlowControlDetected, cardTestTable=cardTestTable, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSpareGroupMode=cardSpareGroupMode, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDetected=cardDetected, cardNumPorts=cardNumPorts, TestTransactionId=TestTransactionId, cardAction=cardAction, diskVolumeFileName=diskVolumeFileName, portSecState=portSecState, cardSerialNum=cardSerialNum, cardBootVersion=cardBootVersion, diskVolumeShelfId=diskVolumeShelfId, cardDplxStatusChange=cardDplxStatusChange, cardTempNoReportNotification=cardTempNoReportNotification, systemGeneralGroup=systemGeneralGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardCarrierHwVersion=cardCarrierHwVersion)
