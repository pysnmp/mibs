...
AdminState, DiskVolumeUsageLevel, PicType, PortId, PortType, FirmwareSource, SecondaryState, CardId, DuplexStatus, PortMode, UnknownState, EqActionType, PrimaryState, CardType, CardSubType, PortDetectedMode, FlowControlMode, ShelfId = mibBuilder.importSymbols("CADANT-TC", "AdminState", "DiskVolumeUsageLevel", "PicType", "PortId", "PortType", "FirmwareSource", "SecondaryState", "CardId", "DuplexStatus", "PortMode", "UnknownState", "EqActionType", "PrimaryState", "CardType", "CardSubType", "PortDetectedMode", "FlowControlMode", "ShelfId")
...
TextualConvention, DisplayString, DateAndTime, RowStatus, MacAddress, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "DateAndTime", "RowStatus", "MacAddress", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeFileName=diskVolumeFileName, cmPrimarySID=cmPrimarySID, portDplxStatusChange=portDplxStatusChange, portType=portType, portDocsIfIndex=portDocsIfIndex, portEntry=portEntry, portMaxDsPower=portMaxDsPower, diskVolumeSize=diskVolumeSize, cmMacAddress=cmMacAddress, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastResetReason=cardLastResetReason, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentCardGroup=equipmentCardGroup, cmResetReason=cmResetReason, cardTemperatureHighError=cardTemperatureHighError, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeUsageLevel=diskVolumeUsageLevel, portCardSubType=portCardSubType, diskVolumeUsageNotification=diskVolumeUsageNotification, portAdminState=portAdminState, equipmentDiag=equipmentDiag, cmResetNotification=cmResetNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, portCurrDsPower=portCurrDsPower, auditTotalFailedCount=auditTotalFailedCount, cardTestResult=cardTestResult, systemGeneral=systemGeneral, cardFlashSize=cardFlashSize, auditTable=auditTable, fpgaErrorEventTable=fpgaErrorEventTable, portLastChangeTime=portLastChangeTime, portTxFlowControlDetected=portTxFlowControlDetected, equipmentStateGroup=equipmentStateGroup, auditCycleCount=auditCycleCount, cardCarrierFwVersion=cardCarrierFwVersion, portAction=portAction, cmResetStatus=cmResetStatus, cardShelfTestIndex=cardShelfTestIndex, shelfName=shelfName, auditCardId=auditCardId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTempNoReportNotification=cardTempNoReportNotification, cardLastBootSource=cardLastBootSource, portNumber=portNumber, cmResetUpTime=cmResetUpTime, cardPrStateChange=cardPrStateChange, diskVolumeName=diskVolumeName, diskVolumeNumber=diskVolumeNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, cardDplxStatusChange=cardDplxStatusChange, cmResetClearNotification=cmResetClearNotification, cardSwVersion=cardSwVersion, cardSpareGroupMode=cardSpareGroupMode, portSecStateChange=portSecStateChange, portPrStateChange=portPrStateChange, cardTestTransId=cardTestTransId, portTable=portTable, cardName=cardName, diskVolumeCardId=diskVolumeCardId, equipmentTbl=equipmentTbl, auditPassedCount=auditPassedCount, portTxFlowControlMode=portTxFlowControlMode, TestId=TestId, systemKey=systemKey, diagTestId=diagTestId, portPrState=portPrState, trapCounter=trapCounter, cardTestCommand=cardTestCommand, consoleOutput=consoleOutput, shelfObjects=shelfObjects, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentCardTestGroup=equipmentCardTestGroup, cardSerialNum=cardSerialNum, cardRamSize=cardRamSize, TestCommand=TestCommand, cardNumPorts=cardNumPorts, cardDetectedChange=cardDetectedChange, downstreamPowerLoss=downstreamPowerLoss, auditAutoScheduling=auditAutoScheduling, auditLogThrottle=auditLogThrottle, cardDplxStatus=cardDplxStatus, TestScheduleCommand=TestScheduleCommand, cardTestIndex=cardTestIndex, auditEntry=auditEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portBgpId=portBgpId, auditDescription=auditDescription, cardTestEntry=cardTestEntry, portDescription=portDescription, equipmentError=equipmentError, cmUChannelID=cmUChannelID, cardCpuSpeed=cardCpuSpeed, portDplxStatus=portDplxStatus, cardTable=cardTable, auditShelfId=auditShelfId, diskDriveNumber=diskDriveNumber, cmDevice=cmDevice, cardLastBootVersion=cardLastBootVersion, cardTemperature=cardTemperature, errEvRowStatus=errEvRowStatus, portDetectedMode=portDetectedMode, portMacAddress=portMacAddress, diskVolumeEntry=diskVolumeEntry, cardShelfId=cardShelfId, cardTestTime=cardTestTime, portTrapInh=portTrapInh, errEventId=errEventId, shelfNumber=shelfNumber, cardTestTable=cardTestTable, portMinDsPower=portMinDsPower, cardCarrierSerialNum=cardCarrierSerialNum, portSecState=portSecState, auditId=auditId, cardAdminState=cardAdminState, diskVolumeShelfId=diskVolumeShelfId, cardTestResultNotification=cardTestResultNotification, cmRegistrationNotification=cmRegistrationNotification, cardFwUpdateStatus=cardFwUpdateStatus, TestResult=TestResult, systemGeneralGroup=systemGeneralGroup, auditFailedCount=auditFailedCount, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditStatus=auditStatus, equipmentState=equipmentState, cardAction=cardAction, eqDiagConfig=eqDiagConfig, cardTestScheduleCommand=cardTestScheduleCommand, errEvLoggingEnabled=errEvLoggingEnabled, portMode=portMode, portRxFlowControlDetected=portRxFlowControlDetected, cardEntry=cardEntry, diskVolumeDriveId=diskVolumeDriveId, cardBusSpeed=cardBusSpeed, cardPrState=cardPrState, auditLogOutput=auditLogOutput, cardId=cardId, cardTestResultDesc=cardTestResultDesc, cardFwVersion=cardFwVersion, cardBootVersion=cardBootVersion, cmIpAddress=cmIpAddress, diskVolumeDiskSize=diskVolumeDiskSize, TestType=TestType, equipmentAudit=equipmentAudit, portId=portId, cardSecStateChange=cardSecStateChange, auditTime=auditTime, portNumChans=portNumChans, TestSchedule=TestSchedule, cardCarrierHwVersion=cardCarrierHwVersion, portShelfId=portShelfId, equipmentDiagGroup=equipmentDiagGroup, equipmentTraps=equipmentTraps, cardSubDetected=cardSubDetected, cardNumber=cardNumber, diskVolumeTrapInh=diskVolumeTrapInh, auditResult=auditResult, cardPicSerialNum=cardPicSerialNum, equipmentAuditGroup=equipmentAuditGroup, equipmentShelfGroup=equipmentShelfGroup, cardCpuType=cardCpuType, diskVolumeId=diskVolumeId, auditCommand=auditCommand, cardHwVersion=cardHwVersion, portCardId=portCardId, cardUpTime=cardUpTime, cardFpgaSource=cardFpgaSource, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSubType=cardSubType, cardTestSchedule=cardTestSchedule, systemClock=systemClock, cardTrapInh=cardTrapInh, cardDetected=cardDetected, portRxFlowControlMode=portRxFlowControlMode, shelfSwVersion=shelfSwVersion, TestTransactionId=TestTransactionId, cardPicDetected=cardPicDetected, cardTestType=cardTestType, cardSecState=cardSecState, portMacIfIndex=portMacIfIndex, diskVolumeTable=diskVolumeTable, trapSeverity=trapSeverity, auditTotalPassedCount=auditTotalPassedCount, cardCPLDVersion=cardCPLDVersion, cardType=cardType, cmResetInfo=cmResetInfo, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentPortGroup=equipmentPortGroup, cardTestId=cardTestId, remainInDiagMode=remainInDiagMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupId=cardSpareGroupId, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardLastChangeTime=cardLastChangeTime, cadEquipmentMib=cadEquipmentMib, cmVendor=cmVendor, errEvLogLevel=errEvLogLevel, cardTestDescription=cardTestDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditName=auditName, portConnectorId=portConnectorId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestName=cardTestName, diskVolumeGroup=diskVolumeGroup, verboseLevel=verboseLevel, cardPicHwVersion=cardPicHwVersion)
