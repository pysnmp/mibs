...
PrimaryState, FirmwareSource, DiskVolumeUsageLevel, PortMode, PortDetectedMode, PortType, CardType, ShelfId, DuplexStatus, PicType, SecondaryState, EqActionType, AdminState, FlowControlMode, CardId, UnknownState, CardSubType, PortId = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "FirmwareSource", "DiskVolumeUsageLevel", "PortMode", "PortDetectedMode", "PortType", "CardType", "ShelfId", "DuplexStatus", "PicType", "SecondaryState", "EqActionType", "AdminState", "FlowControlMode", "CardId", "UnknownState", "CardSubType", "PortId")
...
MacAddress, RowStatus, TimeStamp, TruthValue, TextualConvention, DisplayString, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "TimeStamp", "TruthValue", "TextualConvention", "DisplayString", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBootVersion=cardBootVersion, cardAdminState=cardAdminState, diskVolumeGroup=diskVolumeGroup, portCardId=portCardId, diskVolumeTable=diskVolumeTable, cadEquipmentMib=cadEquipmentMib, cardTestScheduleCommand=cardTestScheduleCommand, cmRegistrationNotification=cmRegistrationNotification, errEvLogLevel=errEvLogLevel, portShelfId=portShelfId, fpgaErrorEventGroup=fpgaErrorEventGroup, PYSNMP_MODULE_ID=cadEquipmentMib, cardFwUpdateStatus=cardFwUpdateStatus, cardCpuSpeed=cardCpuSpeed, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSecStateChange=cardSecStateChange, portAdminState=portAdminState, cardCpuType=cardCpuType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardRamSize=cardRamSize, downstreamPowerLoss=downstreamPowerLoss, TestSchedule=TestSchedule, equipmentCardTestGroup=equipmentCardTestGroup, cardCarrierFwVersion=cardCarrierFwVersion, portDocsIfIndex=portDocsIfIndex, cardCPLDVersion=cardCPLDVersion, auditFailedCount=auditFailedCount, portTable=portTable, cardTestType=cardTestType, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSerialNum=cardSerialNum, cardFpgaSource=cardFpgaSource, cardCarrierHwVersion=cardCarrierHwVersion, portId=portId, auditCommand=auditCommand, portRxFlowControlMode=portRxFlowControlMode, cardTempNoReportNotification=cardTempNoReportNotification, cardLastBootSource=cardLastBootSource, cardUpTime=cardUpTime, cardPrState=cardPrState, cardType=cardType, portMode=portMode, cardShelfTestIndex=cardShelfTestIndex, diskVolumeShelfId=diskVolumeShelfId, cmVendor=cmVendor, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portConnectorId=portConnectorId, portBgpId=portBgpId, auditLogOutput=auditLogOutput, diskVolumeEntry=diskVolumeEntry, cardTestIndex=cardTestIndex, auditResult=auditResult, trapSeverity=trapSeverity, cmResetNotification=cmResetNotification, portType=portType, consoleOutput=consoleOutput, equipmentTbl=equipmentTbl, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResultDesc=cardTestResultDesc, auditName=auditName, cardLastChangeTime=cardLastChangeTime, equipmentShelfGroup=equipmentShelfGroup, diskVolumeDiskSize=diskVolumeDiskSize, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsageLevel=diskVolumeUsageLevel, systemClock=systemClock, auditLogThrottle=auditLogThrottle, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDplxStatusChange=portDplxStatusChange, fpgaErrorEventEntry=fpgaErrorEventEntry, systemGeneralGroup=systemGeneralGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, shelfSwVersion=shelfSwVersion, cmMacAddress=cmMacAddress, auditDescription=auditDescription, verboseLevel=verboseLevel, equipmentStateGroup=equipmentStateGroup, TestResult=TestResult, auditShelfId=auditShelfId, cardTemperatureHighWarn=cardTemperatureHighWarn, portNumber=portNumber, diskVolumeCardId=diskVolumeCardId, cmIpAddress=cmIpAddress, auditTable=auditTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSubDetected=cardSubDetected, systemGeneral=systemGeneral, cardLastResetReason=cardLastResetReason, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSubType=cardSubType, cmResetClearNotification=cmResetClearNotification, cardTestCommand=cardTestCommand, portAction=portAction, diskVolumeDriveId=diskVolumeDriveId, TestScheduleCommand=TestScheduleCommand, cardHwVersion=cardHwVersion, diskVolumeTrapInh=diskVolumeTrapInh, cardName=cardName, equipmentError=equipmentError, equipmentAudit=equipmentAudit, cardBusSpeed=cardBusSpeed, remainInDiagMode=remainInDiagMode, errEventId=errEventId, cardDetectedChange=cardDetectedChange, cardPrStateChange=cardPrStateChange, cardDplxStatusChange=cardDplxStatusChange, diskVolumeId=diskVolumeId, cardId=cardId, cardSpareGroupId=cardSpareGroupId, diskVolumeFileName=diskVolumeFileName, portSecStateChange=portSecStateChange, cmUChannelID=cmUChannelID, portEntry=portEntry, portPrState=portPrState, portSecState=portSecState, equipmentDiag=equipmentDiag, cardFlashSize=cardFlashSize, portMacAddress=portMacAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardSecState=cardSecState, diagTestId=diagTestId, cardTestName=cardTestName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardNumber=cardNumber, cardPicSerialNum=cardPicSerialNum, cardPicHwVersion=cardPicHwVersion, portTxFlowControlMode=portTxFlowControlMode, cardTestId=cardTestId, cardTestDescription=cardTestDescription, portMaxDsPower=portMaxDsPower, portTrapInh=portTrapInh, fpgaErrorEventTable=fpgaErrorEventTable, TestTransactionId=TestTransactionId, equipmentAuditGroup=equipmentAuditGroup, equipmentTraps=equipmentTraps, TestId=TestId, eqDiagConfig=eqDiagConfig, auditTotalFailedCount=auditTotalFailedCount, shelfName=shelfName, errEvRowStatus=errEvRowStatus, equipmentPortGroup=equipmentPortGroup, cmResetInfo=cmResetInfo, cardTestTransId=cardTestTransId, cardTable=cardTable, cardFwVersion=cardFwVersion, cardDetected=cardDetected, diskVolumeSize=diskVolumeSize, auditTime=auditTime, portMacIfIndex=portMacIfIndex, cmDevice=cmDevice, auditCardId=auditCardId, cardTemperature=cardTemperature, auditAutoScheduling=auditAutoScheduling, shelfNumber=shelfNumber, cmResetStatus=cmResetStatus, diskDriveNumber=diskDriveNumber, cmPrimarySID=cmPrimarySID, systemKey=systemKey, auditCycleCount=auditCycleCount, auditPassedCount=auditPassedCount, portNumChans=portNumChans, trapCounter=trapCounter, cardShelfId=cardShelfId, cmResetReason=cmResetReason, cardCarrierSerialNum=cardCarrierSerialNum, auditTotalPassedCount=auditTotalPassedCount, diskVolumeNumber=diskVolumeNumber, shelfObjects=shelfObjects, cardPicDetected=cardPicDetected, portMinDsPower=portMinDsPower, cardTestTable=cardTestTable, cmResetUpTime=cmResetUpTime, errEvLoggingEnabled=errEvLoggingEnabled, equipmentDiagGroup=equipmentDiagGroup, portCurrDsPower=portCurrDsPower, cardTestTime=cardTestTime, auditEntry=auditEntry, portDescription=portDescription, portCardSubType=portCardSubType, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentCardGroup=equipmentCardGroup, auditStatus=auditStatus, cardLastBootVersion=cardLastBootVersion, TestType=TestType, cardSpareGroupMode=cardSpareGroupMode, portDplxStatus=portDplxStatus, portRxFlowControlDetected=portRxFlowControlDetected, cardTestSchedule=cardTestSchedule, cardTestResult=cardTestResult, portDetectedMode=portDetectedMode, cardTemperatureHighError=cardTemperatureHighError, auditId=auditId, cardTrapInh=cardTrapInh, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeName=diskVolumeName, cardDplxStatus=cardDplxStatus, TestCommand=TestCommand, cardNumPorts=cardNumPorts, cardTestEntry=cardTestEntry, cardTestResultNotification=cardTestResultNotification, portLastChangeTime=portLastChangeTime, cardSwVersion=cardSwVersion, cardAction=cardAction, portPrStateChange=portPrStateChange, equipmentState=equipmentState, cardEntry=cardEntry)
