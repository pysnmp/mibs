...
PortDetectedMode, CardId, PicType, SecondaryState, EqActionType, CardSubType, CardType, DiskVolumeUsageLevel, FlowControlMode, PortId, PortMode, ShelfId, PrimaryState, PortType, AdminState, DuplexStatus, FirmwareSource, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "CardId", "PicType", "SecondaryState", "EqActionType", "CardSubType", "CardType", "DiskVolumeUsageLevel", "FlowControlMode", "PortId", "PortMode", "ShelfId", "PrimaryState", "PortType", "AdminState", "DuplexStatus", "FirmwareSource", "UnknownState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", systemClock=systemClock, fpgaErrorEventEntry=fpgaErrorEventEntry, cardLastBootVersion=cardLastBootVersion, cardFwVersion=cardFwVersion, auditLogOutput=auditLogOutput, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPicSerialNum=cardPicSerialNum, cardDplxStatusChange=cardDplxStatusChange, cmResetReason=cmResetReason, cmResetClearNotification=cmResetClearNotification, cardCPLDVersion=cardCPLDVersion, cmVendor=cmVendor, TestResult=TestResult, portAdminState=portAdminState, cardTestType=cardTestType, verboseLevel=verboseLevel, cardTemperature=cardTemperature, cmIpAddress=cmIpAddress, cardSwVersion=cardSwVersion, portCardId=portCardId, cardTemperatureHighWarn=cardTemperatureHighWarn, portDescription=portDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, portDplxStatusChange=portDplxStatusChange, TestScheduleCommand=TestScheduleCommand, diskVolumeCardId=diskVolumeCardId, diskVolumeDriveId=diskVolumeDriveId, portShelfId=portShelfId, cardSpareGroupMode=cardSpareGroupMode, cardFlashSize=cardFlashSize, trapSeverity=trapSeverity, auditCommand=auditCommand, auditCycleCount=auditCycleCount, cardShelfId=cardShelfId, diskVolumeName=diskVolumeName, auditTotalFailedCount=auditTotalFailedCount, cardTestId=cardTestId, cardLastChangeTime=cardLastChangeTime, cardTempOverHeatNotification=cardTempOverHeatNotification, TestType=TestType, diskVolumeTable=diskVolumeTable, auditId=auditId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResult=cardTestResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestCommand=TestCommand, cardNumPorts=cardNumPorts, cardSubDetected=cardSubDetected, cardEntry=cardEntry, cardTestIndex=cardTestIndex, errEvLogLevel=errEvLogLevel, cardUpTime=cardUpTime, cardLastResetReason=cardLastResetReason, auditTime=auditTime, portSecState=portSecState, portType=portType, portCurrDsPower=portCurrDsPower, cardTempNoReportNotification=cardTempNoReportNotification, equipmentStateGroup=equipmentStateGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardName=cardName, cardFwUpdateStatus=cardFwUpdateStatus, cardTestScheduleCommand=cardTestScheduleCommand, cmPrimarySID=cmPrimarySID, eqDiagConfig=eqDiagConfig, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portRxFlowControlDetected=portRxFlowControlDetected, trapCounter=trapCounter, portMaxDsPower=portMaxDsPower, cardTestTable=cardTestTable, portCardSubType=portCardSubType, equipmentAudit=equipmentAudit, equipmentPortGroup=equipmentPortGroup, cardBootVersion=cardBootVersion, diagTestId=diagTestId, cardTestTransId=cardTestTransId, cmUChannelID=cmUChannelID, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentTbl=equipmentTbl, cardPicDetected=cardPicDetected, cmResetInfo=cmResetInfo, cardTestName=cardTestName, cardShelfTestIndex=cardShelfTestIndex, cardAdminState=cardAdminState, diskVolumeGroup=diskVolumeGroup, cmResetNotification=cmResetNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeFileName=diskVolumeFileName, errEventId=errEventId, diskVolumeId=diskVolumeId, shelfNumber=shelfNumber, cardPicHwVersion=cardPicHwVersion, auditCardId=auditCardId, systemKey=systemKey, auditEntry=auditEntry, diskVolumeNumber=diskVolumeNumber, portMacAddress=portMacAddress, diskVolumeUsageLevel=diskVolumeUsageLevel, TestId=TestId, cardNumber=cardNumber, cardCarrierHwVersion=cardCarrierHwVersion, equipmentState=equipmentState, diskDriveNumber=diskDriveNumber, portEntry=portEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, fpgaErrorEventTable=fpgaErrorEventTable, portLastChangeTime=portLastChangeTime, cardPrStateChange=cardPrStateChange, cardTemperatureHighError=cardTemperatureHighError, portDocsIfIndex=portDocsIfIndex, cardSpareGroupId=cardSpareGroupId, portConnectorId=portConnectorId, equipmentShelfGroup=equipmentShelfGroup, portNumber=portNumber, portBgpId=portBgpId, equipmentDiag=equipmentDiag, cardTrapInh=cardTrapInh, auditLogThrottle=auditLogThrottle, portTxFlowControlMode=portTxFlowControlMode, auditTable=auditTable, auditStatus=auditStatus, cardAction=cardAction, cmResetStatus=cmResetStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, portRxFlowControlMode=portRxFlowControlMode, diskVolumeEntry=diskVolumeEntry, equipmentCardGroup=equipmentCardGroup, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestSchedule=cardTestSchedule, cardRamSize=cardRamSize, cardCarrierFwVersion=cardCarrierFwVersion, portMacIfIndex=portMacIfIndex, errEvLoggingEnabled=errEvLoggingEnabled, TestSchedule=TestSchedule, auditShelfId=auditShelfId, portPrState=portPrState, portTable=portTable, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMib=cadEquipmentMib, cardCpuType=cardCpuType, downstreamPowerLoss=downstreamPowerLoss, auditResult=auditResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditPassedCount=auditPassedCount, portMinDsPower=portMinDsPower, shelfObjects=shelfObjects, portDetectedMode=portDetectedMode, cardDetected=cardDetected, diskVolumeSize=diskVolumeSize, auditAutoScheduling=auditAutoScheduling, cardDplxStatus=cardDplxStatus, cardSubType=cardSubType, auditDescription=auditDescription, cardSerialNum=cardSerialNum, cardDetectedChange=cardDetectedChange, cmRegistrationNotification=cmRegistrationNotification, systemGeneral=systemGeneral, cardPrState=cardPrState, cardSecState=cardSecState, fpgaErrorEventGroup=fpgaErrorEventGroup, cardBusSpeed=cardBusSpeed, equipmentError=equipmentError, equipmentDiagGroup=equipmentDiagGroup, cardType=cardType, cardTestDescription=cardTestDescription, cardHwVersion=cardHwVersion, cardTestEntry=cardTestEntry, portMode=portMode, cmResetUpTime=cmResetUpTime, cardTestResultNotification=cardTestResultNotification, cardLastBootSource=cardLastBootSource, auditName=auditName, cardFpgaSource=cardFpgaSource, systemGeneralGroup=systemGeneralGroup, portTrapInh=portTrapInh, cardCarrierSerialNum=cardCarrierSerialNum, equipmentCardTestGroup=equipmentCardTestGroup, cmMacAddress=cmMacAddress, cardId=cardId, auditTotalPassedCount=auditTotalPassedCount, cardTestResultDesc=cardTestResultDesc, shelfName=shelfName, portTxFlowControlDetected=portTxFlowControlDetected, auditFailedCount=auditFailedCount, consoleOutput=consoleOutput, errEvRowStatus=errEvRowStatus, equipmentAuditGroup=equipmentAuditGroup, portAction=portAction, diskVolumeShelfId=diskVolumeShelfId, portPrStateChange=portPrStateChange, portId=portId, cardCpuSpeed=cardCpuSpeed, cmDevice=cmDevice, cardSecStateChange=cardSecStateChange, cardTable=cardTable, TestTransactionId=TestTransactionId, portDplxStatus=portDplxStatus, portSecStateChange=portSecStateChange, portNumChans=portNumChans, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentTraps=equipmentTraps, remainInDiagMode=remainInDiagMode, cardTestTime=cardTestTime, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfSwVersion=shelfSwVersion, cardTestCommand=cardTestCommand)
