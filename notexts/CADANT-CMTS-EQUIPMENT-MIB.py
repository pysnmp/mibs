...
PicType, AdminState, CardId, FirmwareSource, CardSubType, DiskVolumeUsageLevel, PortMode, EqActionType, UnknownState, CardType, PortDetectedMode, DuplexStatus, SecondaryState, PortId, PrimaryState, FlowControlMode, PortType, ShelfId = mibBuilder.importSymbols("CADANT-TC", "PicType", "AdminState", "CardId", "FirmwareSource", "CardSubType", "DiskVolumeUsageLevel", "PortMode", "EqActionType", "UnknownState", "CardType", "PortDetectedMode", "DuplexStatus", "SecondaryState", "PortId", "PrimaryState", "FlowControlMode", "PortType", "ShelfId")
...
DisplayString, MacAddress, DateAndTime, TruthValue, TimeStamp, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "DateAndTime", "TruthValue", "TimeStamp", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestType=TestType, portMacIfIndex=portMacIfIndex, diskVolumeCardId=diskVolumeCardId, cardNumber=cardNumber, diskVolumeNumber=diskVolumeNumber, cardShelfId=cardShelfId, cardType=cardType, cmResetStatus=cmResetStatus, cardSerialNum=cardSerialNum, cmResetReason=cmResetReason, errEvLoggingEnabled=errEvLoggingEnabled, cardSubDetected=cardSubDetected, cmResetNotification=cmResetNotification, cmMacAddress=cmMacAddress, cardSpareGroupId=cardSpareGroupId, auditTime=auditTime, cardTestTime=cardTestTime, diskDriveNumber=diskDriveNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, systemGeneralGroup=systemGeneralGroup, portAction=portAction, cardRamSize=cardRamSize, cardTestType=cardTestType, cmIpAddress=cmIpAddress, portBgpId=portBgpId, cmPrimarySID=cmPrimarySID, portNumChans=portNumChans, auditCycleCount=auditCycleCount, cardUpTime=cardUpTime, auditName=auditName, portDocsIfIndex=portDocsIfIndex, portSecState=portSecState, cardTestId=cardTestId, cardSecState=cardSecState, cardHwVersion=cardHwVersion, portTxFlowControlMode=portTxFlowControlMode, cardNumPorts=cardNumPorts, TestResult=TestResult, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardPicHwVersion=cardPicHwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestIndex=cardTestIndex, cardPicDetected=cardPicDetected, eqDiagConfig=eqDiagConfig, portDetectedMode=portDetectedMode, TestId=TestId, verboseLevel=verboseLevel, cardSwVersion=cardSwVersion, errEventId=errEventId, diskVolumeShelfId=diskVolumeShelfId, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastBootSource=cardLastBootSource, fpgaErrorEventGroup=fpgaErrorEventGroup, portConnectorId=portConnectorId, cmVendor=cmVendor, auditPassedCount=auditPassedCount, cmRegistrationNotification=cmRegistrationNotification, diskVolumeId=diskVolumeId, cardTestResultDesc=cardTestResultDesc, TestScheduleCommand=TestScheduleCommand, cardFlashSize=cardFlashSize, portCardSubType=portCardSubType, auditStatus=auditStatus, cardTable=cardTable, portDplxStatus=portDplxStatus, portRxFlowControlMode=portRxFlowControlMode, diskVolumeName=diskVolumeName, portLastChangeTime=portLastChangeTime, cardDplxStatus=cardDplxStatus, trapCounter=trapCounter, cardBusSpeed=cardBusSpeed, equipmentAudit=equipmentAudit, auditDescription=auditDescription, fpgaErrorEventEntry=fpgaErrorEventEntry, systemGeneral=systemGeneral, portMaxDsPower=portMaxDsPower, equipmentPortGroup=equipmentPortGroup, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentTraps=equipmentTraps, portMode=portMode, equipmentShelfGroup=equipmentShelfGroup, cardTestResult=cardTestResult, portCardId=portCardId, cardDetected=cardDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, cardFwVersion=cardFwVersion, portTxFlowControlDetected=portTxFlowControlDetected, cardCPLDVersion=cardCPLDVersion, PYSNMP_MODULE_ID=cadEquipmentMib, auditFailedCount=auditFailedCount, portNumber=portNumber, cardPrStateChange=cardPrStateChange, TestCommand=TestCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentState=equipmentState, cmResetClearNotification=cmResetClearNotification, cardLastResetReason=cardLastResetReason, cardTestCommand=cardTestCommand, auditResult=auditResult, cardTestResultNotification=cardTestResultNotification, cardTestName=cardTestName, shelfNumber=shelfNumber, cardTestEntry=cardTestEntry, diagTestId=diagTestId, cardFwUpdateStatus=cardFwUpdateStatus, cardPrState=cardPrState, portId=portId, cardBootVersion=cardBootVersion, auditTotalPassedCount=auditTotalPassedCount, cardName=cardName, cardCpuSpeed=cardCpuSpeed, cardLastBootVersion=cardLastBootVersion, cardTrapInh=cardTrapInh, remainInDiagMode=remainInDiagMode, portMacAddress=portMacAddress, cmDevice=cmDevice, diskVolumeSize=diskVolumeSize, cardCarrierSerialNum=cardCarrierSerialNum, errEvRecoveryEnabled=errEvRecoveryEnabled, cardEntry=cardEntry, cardSubType=cardSubType, cardShelfTestIndex=cardShelfTestIndex, cmUChannelID=cmUChannelID, diskVolumeDriveId=diskVolumeDriveId, auditLogOutput=auditLogOutput, cardTempNoReportNotification=cardTempNoReportNotification, downstreamPowerLoss=downstreamPowerLoss, cardTemperature=cardTemperature, portPrState=portPrState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTemperatureHighError=cardTemperatureHighError, equipmentDiagGroup=equipmentDiagGroup, auditEntry=auditEntry, auditCommand=auditCommand, cmResetUpTime=cmResetUpTime, cardFpgaSource=cardFpgaSource, auditShelfId=auditShelfId, portShelfId=portShelfId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portAdminState=portAdminState, errEvLogLevel=errEvLogLevel, diskVolumeGroup=diskVolumeGroup, portTrapInh=portTrapInh, cardTestDescription=cardTestDescription, systemKey=systemKey, portCurrDsPower=portCurrDsPower, cardSecStateChange=cardSecStateChange, TestSchedule=TestSchedule, equipmentDiag=equipmentDiag, diskVolumeTrapInh=diskVolumeTrapInh, cmResetInfo=cmResetInfo, shelfObjects=shelfObjects, shelfName=shelfName, auditCardId=auditCardId, auditId=auditId, equipmentTbl=equipmentTbl, portEntry=portEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, shelfSwVersion=shelfSwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, portDescription=portDescription, equipmentAuditGroup=equipmentAuditGroup, cardTestSchedule=cardTestSchedule, consoleOutput=consoleOutput, cardDetectedChange=cardDetectedChange, cardCarrierHwVersion=cardCarrierHwVersion, cadEquipmentMib=cadEquipmentMib, equipmentError=equipmentError, portTable=portTable, cardLastChangeTime=cardLastChangeTime, auditLogThrottle=auditLogThrottle, equipmentCardGroup=equipmentCardGroup, equipmentStateGroup=equipmentStateGroup, auditTable=auditTable, errEvRowStatus=errEvRowStatus, portMinDsPower=portMinDsPower, cardAdminState=cardAdminState, portPrStateChange=portPrStateChange, diskVolumeEntry=diskVolumeEntry, cardTestTransId=cardTestTransId, TestTransactionId=TestTransactionId, cardSpareGroupMode=cardSpareGroupMode, portType=portType, diskVolumeFileName=diskVolumeFileName, cardPicSerialNum=cardPicSerialNum, cardCpuType=cardCpuType, trapSeverity=trapSeverity, diskVolumeDiskSize=diskVolumeDiskSize, cardTestScheduleCommand=cardTestScheduleCommand, cardDplxStatusChange=cardDplxStatusChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTable=cardTestTable, portDplxStatusChange=portDplxStatusChange, cardAction=cardAction, portSecStateChange=portSecStateChange, fpgaErrorEventTable=fpgaErrorEventTable, auditTotalFailedCount=auditTotalFailedCount, cardId=cardId, portRxFlowControlDetected=portRxFlowControlDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeTable=diskVolumeTable, auditAutoScheduling=auditAutoScheduling, systemClock=systemClock)
