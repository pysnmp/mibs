...
ShelfId, FirmwareSource, PicType, FlowControlMode, SecondaryState, EqActionType, UnknownState, CardType, PortMode, PortDetectedMode, CardId, DiskVolumeUsageLevel, DuplexStatus, PortId, CardSubType, AdminState, PortType, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "FirmwareSource", "PicType", "FlowControlMode", "SecondaryState", "EqActionType", "UnknownState", "CardType", "PortMode", "PortDetectedMode", "CardId", "DiskVolumeUsageLevel", "DuplexStatus", "PortId", "CardSubType", "AdminState", "PortType", "PrimaryState")
...
DateAndTime, RowStatus, DisplayString, TextualConvention, MacAddress, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "RowStatus", "DisplayString", "TextualConvention", "MacAddress", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portShelfId=portShelfId, cardCarrierFwVersion=cardCarrierFwVersion, systemKey=systemKey, equipmentAudit=equipmentAudit, cardSubType=cardSubType, cardPicDetected=cardPicDetected, portTable=portTable, TestType=TestType, TestSchedule=TestSchedule, cmResetNotification=cmResetNotification, auditEntry=auditEntry, cmUChannelID=cmUChannelID, portBgpId=portBgpId, shelfObjects=shelfObjects, cardBusSpeed=cardBusSpeed, portCardId=portCardId, cardTemperatureHighError=cardTemperatureHighError, portPrStateChange=portPrStateChange, portCardSubType=portCardSubType, portConnectorId=portConnectorId, cardNumber=cardNumber, cardType=cardType, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentCardTestGroup=equipmentCardTestGroup, cardTestScheduleCommand=cardTestScheduleCommand, auditFailedCount=auditFailedCount, portMinDsPower=portMinDsPower, cardTestCommand=cardTestCommand, portMacIfIndex=portMacIfIndex, equipmentCardGroup=equipmentCardGroup, cmDevice=cmDevice, cardAdminState=cardAdminState, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentDiag=equipmentDiag, cardCarrierSerialNum=cardCarrierSerialNum, cmResetStatus=cmResetStatus, auditName=auditName, diskVolumeShelfId=diskVolumeShelfId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portCurrDsPower=portCurrDsPower, cardTestDescription=cardTestDescription, cardSubDetected=cardSubDetected, cardSpareGroupId=cardSpareGroupId, errEventId=errEventId, diagTestId=diagTestId, diskVolumeCardId=diskVolumeCardId, shelfNumber=shelfNumber, cadEquipmentMib=cadEquipmentMib, auditStatus=auditStatus, errEvLoggingEnabled=errEvLoggingEnabled, portTxFlowControlDetected=portTxFlowControlDetected, TestId=TestId, cardTestTransId=cardTestTransId, diskVolumeDiskSize=diskVolumeDiskSize, cardHwVersion=cardHwVersion, consoleOutput=consoleOutput, portDetectedMode=portDetectedMode, portDescription=portDescription, cardTestResultNotification=cardTestResultNotification, cmRegistrationNotification=cmRegistrationNotification, equipmentError=equipmentError, cardTrapInh=cardTrapInh, cardTestResultDesc=cardTestResultDesc, auditPassedCount=auditPassedCount, diskVolumeId=diskVolumeId, cardAction=cardAction, diskVolumeUsageLevel=diskVolumeUsageLevel, cadEquipmentMibCompliances=cadEquipmentMibCompliances, downstreamPowerLoss=downstreamPowerLoss, equipmentState=equipmentState, cardTable=cardTable, trapSeverity=trapSeverity, cardPicHwVersion=cardPicHwVersion, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCpuType=cardCpuType, cadEquipmentMibConformance=cadEquipmentMibConformance, cardEntry=cardEntry, portAdminState=portAdminState, auditTotalFailedCount=auditTotalFailedCount, cardLastBootVersion=cardLastBootVersion, cardId=cardId, diskDriveNumber=diskDriveNumber, shelfName=shelfName, cardDetected=cardDetected, cardBootVersion=cardBootVersion, diskVolumeGroup=diskVolumeGroup, portNumChans=portNumChans, diskVolumeEntry=diskVolumeEntry, TestTransactionId=TestTransactionId, cmIpAddress=cmIpAddress, auditTime=auditTime, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeNumber=diskVolumeNumber, systemClock=systemClock, portId=portId, cardLastBootSource=cardLastBootSource, cardTestResult=cardTestResult, cardTempNoReportNotification=cardTempNoReportNotification, verboseLevel=verboseLevel, portLastChangeTime=portLastChangeTime, auditCommand=auditCommand, equipmentTraps=equipmentTraps, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestType=cardTestType, auditId=auditId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portMode=portMode, cardFlashSize=cardFlashSize, equipmentTbl=equipmentTbl, portEntry=portEntry, fpgaErrorEventGroup=fpgaErrorEventGroup, auditResult=auditResult, shelfSwVersion=shelfSwVersion, systemGeneral=systemGeneral, cardLastChangeTime=cardLastChangeTime, cardName=cardName, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPrStateChange=cardPrStateChange, cmResetReason=cmResetReason, TestScheduleCommand=TestScheduleCommand, portMaxDsPower=portMaxDsPower, portPrState=portPrState, cardCPLDVersion=cardCPLDVersion, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestId=cardTestId, cmResetInfo=cmResetInfo, trapCounter=trapCounter, cardCarrierHwVersion=cardCarrierHwVersion, cardTestSchedule=cardTestSchedule, TestCommand=TestCommand, cardPicSerialNum=cardPicSerialNum, portTxFlowControlMode=portTxFlowControlMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentPortGroup=equipmentPortGroup, cardTestEntry=cardTestEntry, cardCpuSpeed=cardCpuSpeed, portNumber=portNumber, diskVolumeTable=diskVolumeTable, portAction=portAction, portMacAddress=portMacAddress, auditCardId=auditCardId, TestResult=TestResult, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portTrapInh=portTrapInh, portDplxStatusChange=portDplxStatusChange, diskVolumeTrapInh=diskVolumeTrapInh, cardRamSize=cardRamSize, errEvLogLevel=errEvLogLevel, portRxFlowControlMode=portRxFlowControlMode, diskVolumeSize=diskVolumeSize, cmResetClearNotification=cmResetClearNotification, diskVolumeName=diskVolumeName, cardTestTable=cardTestTable, cmPrimarySID=cmPrimarySID, cardSerialNum=cardSerialNum, cmResetUpTime=cmResetUpTime, equipmentAuditGroup=equipmentAuditGroup, systemGeneralGroup=systemGeneralGroup, cardTestTime=cardTestTime, auditTable=auditTable, portDocsIfIndex=portDocsIfIndex, cardNumPorts=cardNumPorts, cardSecStateChange=cardSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, portRxFlowControlDetected=portRxFlowControlDetected, portType=portType, cmMacAddress=cmMacAddress, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastResetReason=cardLastResetReason, diskVolumeDriveId=diskVolumeDriveId, cardSecState=cardSecState, eqDiagConfig=eqDiagConfig, auditAutoScheduling=auditAutoScheduling, cardUpTime=cardUpTime, auditLogOutput=auditLogOutput, diskVolumeFileName=diskVolumeFileName, cardDplxStatusChange=cardDplxStatusChange, auditDescription=auditDescription, portSecStateChange=portSecStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSwVersion=cardSwVersion, portDplxStatus=portDplxStatus, cmVendor=cmVendor, auditCycleCount=auditCycleCount, auditLogThrottle=auditLogThrottle, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestIndex=cardTestIndex, cardTestName=cardTestName, errEvRowStatus=errEvRowStatus, equipmentStateGroup=equipmentStateGroup, cardDplxStatus=cardDplxStatus, cardTemperature=cardTemperature, cardShelfId=cardShelfId, auditTotalPassedCount=auditTotalPassedCount, cardDetectedChange=cardDetectedChange, cardFwVersion=cardFwVersion, cardPrState=cardPrState, remainInDiagMode=remainInDiagMode, auditShelfId=auditShelfId, equipmentDiagGroup=equipmentDiagGroup, cardFpgaSource=cardFpgaSource, portSecState=portSecState)
