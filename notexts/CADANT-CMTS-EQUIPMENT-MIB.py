...
DuplexStatus, UnknownState, FlowControlMode, FirmwareSource, EqActionType, CardId, PrimaryState, PortType, PortMode, PortDetectedMode, PortId, AdminState, DiskVolumeUsageLevel, CardType, CardSubType, ShelfId, SecondaryState, PicType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "UnknownState", "FlowControlMode", "FirmwareSource", "EqActionType", "CardId", "PrimaryState", "PortType", "PortMode", "PortDetectedMode", "PortId", "AdminState", "DiskVolumeUsageLevel", "CardType", "CardSubType", "ShelfId", "SecondaryState", "PicType")
...
DisplayString, MacAddress, DateAndTime, TextualConvention, TruthValue, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "DateAndTime", "TextualConvention", "TruthValue", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCarrierFwVersion=cardCarrierFwVersion, cmResetStatus=cmResetStatus, shelfName=shelfName, cardTestTable=cardTestTable, portSecStateChange=portSecStateChange, diskVolumeNumber=diskVolumeNumber, cardLastBootVersion=cardLastBootVersion, cardTemperatureHighError=cardTemperatureHighError, equipmentDiag=equipmentDiag, cmMacAddress=cmMacAddress, cardPrState=cardPrState, portDocsIfIndex=portDocsIfIndex, cardTestCommand=cardTestCommand, cardPicDetected=cardPicDetected, auditAutoScheduling=auditAutoScheduling, cardAdminState=cardAdminState, portTrapInh=portTrapInh, portTable=portTable, diskVolumeUsageNotification=diskVolumeUsageNotification, auditShelfId=auditShelfId, cardFpgaSource=cardFpgaSource, cardTestResult=cardTestResult, systemGeneralGroup=systemGeneralGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestScheduleCommand=TestScheduleCommand, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardDplxStatus=cardDplxStatus, downstreamPowerLoss=downstreamPowerLoss, cmDevice=cmDevice, cmIpAddress=cmIpAddress, cardSubDetected=cardSubDetected, equipmentCardTestGroup=equipmentCardTestGroup, cardTestIndex=cardTestIndex, systemGeneral=systemGeneral, portNumChans=portNumChans, diskVolumeId=diskVolumeId, equipmentCardGroup=equipmentCardGroup, cardFwVersion=cardFwVersion, cardDetectedChange=cardDetectedChange, cardSpareGroupId=cardSpareGroupId, cardLastResetReason=cardLastResetReason, cardSerialNum=cardSerialNum, portShelfId=portShelfId, cardFwUpdateStatus=cardFwUpdateStatus, auditDescription=auditDescription, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMibConformance=cadEquipmentMibConformance, cardPicSerialNum=cardPicSerialNum, cardSpareGroupMode=cardSpareGroupMode, cardBootVersion=cardBootVersion, portId=portId, cardNumPorts=cardNumPorts, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardUpTime=cardUpTime, portMode=portMode, cardTestEntry=cardTestEntry, diskDriveNumber=diskDriveNumber, portAction=portAction, portPrStateChange=portPrStateChange, cardShelfId=cardShelfId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditTotalFailedCount=auditTotalFailedCount, cmUChannelID=cmUChannelID, cmRegistrationNotification=cmRegistrationNotification, diskVolumeTrapInh=diskVolumeTrapInh, cardDplxStatusChange=cardDplxStatusChange, TestType=TestType, portDescription=portDescription, cardSecState=cardSecState, errEvLogLevel=errEvLogLevel, cmVendor=cmVendor, cmResetReason=cmResetReason, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTempOverHeatNotification=cardTempOverHeatNotification, portDplxStatusChange=portDplxStatusChange, portMaxDsPower=portMaxDsPower, cardTable=cardTable, errEventId=errEventId, cardFlashSize=cardFlashSize, diskVolumeFileName=diskVolumeFileName, cardCpuSpeed=cardCpuSpeed, portDplxStatus=portDplxStatus, portRxFlowControlMode=portRxFlowControlMode, portSecState=portSecState, equipmentDiagGroup=equipmentDiagGroup, diagTestId=diagTestId, portLastChangeTime=portLastChangeTime, cardType=cardType, consoleOutput=consoleOutput, cardHwVersion=cardHwVersion, portMinDsPower=portMinDsPower, cardTestResultNotification=cardTestResultNotification, portEntry=portEntry, portBgpId=portBgpId, diskVolumeCardId=diskVolumeCardId, cardSubType=cardSubType, cardTestTransId=cardTestTransId, eqDiagConfig=eqDiagConfig, fpgaErrorEventEntry=fpgaErrorEventEntry, shelfSwVersion=shelfSwVersion, cardLastChangeTime=cardLastChangeTime, diskVolumeGroup=diskVolumeGroup, cardNumber=cardNumber, portNumber=portNumber, cadEquipmentMib=cadEquipmentMib, equipmentError=equipmentError, equipmentState=equipmentState, auditCommand=auditCommand, verboseLevel=verboseLevel, cardTemperatureHighWarn=cardTemperatureHighWarn, portConnectorId=portConnectorId, TestTransactionId=TestTransactionId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeShelfId=diskVolumeShelfId, portTxFlowControlMode=portTxFlowControlMode, auditTime=auditTime, shelfNumber=shelfNumber, diskVolumeEntry=diskVolumeEntry, equipmentAuditGroup=equipmentAuditGroup, TestSchedule=TestSchedule, cardCPLDVersion=cardCPLDVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTime=cardTestTime, cardPrStateChange=cardPrStateChange, auditTotalPassedCount=auditTotalPassedCount, diskVolumeDiskSize=diskVolumeDiskSize, TestCommand=TestCommand, auditResult=auditResult, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestDescription=cardTestDescription, cardTemperature=cardTemperature, portRxFlowControlDetected=portRxFlowControlDetected, cardSecStateChange=cardSecStateChange, diskVolumeDriveId=diskVolumeDriveId, cardTestSchedule=cardTestSchedule, auditName=auditName, auditFailedCount=auditFailedCount, TestId=TestId, cardBusSpeed=cardBusSpeed, diskVolumeName=diskVolumeName, cardName=cardName, fpgaErrorEventTable=fpgaErrorEventTable, cardTestId=cardTestId, cardCarrierSerialNum=cardCarrierSerialNum, portDetectedMode=portDetectedMode, equipmentPortGroup=equipmentPortGroup, trapCounter=trapCounter, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, shelfObjects=shelfObjects, portCurrDsPower=portCurrDsPower, portMacIfIndex=portMacIfIndex, auditId=auditId, cardEntry=cardEntry, cardShelfTestIndex=cardShelfTestIndex, cardTestScheduleCommand=cardTestScheduleCommand, equipmentTbl=equipmentTbl, cmPrimarySID=cmPrimarySID, cardTestResultDesc=cardTestResultDesc, portMacAddress=portMacAddress, errEvRowStatus=errEvRowStatus, cardTrapInh=cardTrapInh, auditEntry=auditEntry, equipmentShelfGroup=equipmentShelfGroup, cardCpuType=cardCpuType, equipmentAudit=equipmentAudit, portCardId=portCardId, PYSNMP_MODULE_ID=cadEquipmentMib, systemKey=systemKey, cardRamSize=cardRamSize, portType=portType, cardLastBootSource=cardLastBootSource, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCardId=auditCardId, portCardSubType=portCardSubType, equipmentStateGroup=equipmentStateGroup, remainInDiagMode=remainInDiagMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestName=cardTestName, auditCycleCount=auditCycleCount, cardAction=cardAction, portPrState=portPrState, auditStatus=auditStatus, trapSeverity=trapSeverity, auditLogOutput=auditLogOutput, TestResult=TestResult, cardId=cardId, cmResetClearNotification=cmResetClearNotification, cardDetected=cardDetected, cardPicHwVersion=cardPicHwVersion, cardCarrierHwVersion=cardCarrierHwVersion, cardSwVersion=cardSwVersion, portAdminState=portAdminState, diskVolumeTable=diskVolumeTable, cardTempNoReportNotification=cardTempNoReportNotification, auditLogThrottle=auditLogThrottle, cardTestType=cardTestType, auditTable=auditTable, systemClock=systemClock, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeSize=diskVolumeSize, cmResetInfo=cmResetInfo, auditPassedCount=auditPassedCount, cmResetUpTime=cmResetUpTime, cmResetNotification=cmResetNotification, errEvLoggingEnabled=errEvLoggingEnabled, equipmentTraps=equipmentTraps)
