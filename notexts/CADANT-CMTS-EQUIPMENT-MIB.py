...
FlowControlMode, UnknownState, EqActionType, PrimaryState, AdminState, CardSubType, DuplexStatus, PortType, ShelfId, DiskVolumeUsageLevel, SecondaryState, PortDetectedMode, PicType, CardType, FirmwareSource, PortId, CardId, PortMode = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "UnknownState", "EqActionType", "PrimaryState", "AdminState", "CardSubType", "DuplexStatus", "PortType", "ShelfId", "DiskVolumeUsageLevel", "SecondaryState", "PortDetectedMode", "PicType", "CardType", "FirmwareSource", "PortId", "CardId", "PortMode")
...
TextualConvention, RowStatus, DisplayString, TimeStamp, TruthValue, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "DisplayString", "TimeStamp", "TruthValue", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTemperatureHighError=cardTemperatureHighError, cardCPLDVersion=cardCPLDVersion, auditTotalPassedCount=auditTotalPassedCount, cardAdminState=cardAdminState, TestTransactionId=TestTransactionId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestId=TestId, cmDevice=cmDevice, portTxFlowControlDetected=portTxFlowControlDetected, systemGeneral=systemGeneral, cardTestType=cardTestType, cardId=cardId, cardTestTransId=cardTestTransId, cardBootVersion=cardBootVersion, TestResult=TestResult, cardFpgaSource=cardFpgaSource, equipmentError=equipmentError, portDocsIfIndex=portDocsIfIndex, portAdminState=portAdminState, auditPassedCount=auditPassedCount, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentDiagGroup=equipmentDiagGroup, cardSecState=cardSecState, cardType=cardType, cardTestName=cardTestName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestCommand=cardTestCommand, auditName=auditName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portCurrDsPower=portCurrDsPower, diagTestId=diagTestId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portMinDsPower=portMinDsPower, cardTemperatureHighWarn=cardTemperatureHighWarn, errEventId=errEventId, cardTestEntry=cardTestEntry, errEvLogLevel=errEvLogLevel, cardCarrierHwVersion=cardCarrierHwVersion, portShelfId=portShelfId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portLastChangeTime=portLastChangeTime, cardSubType=cardSubType, cardLastBootVersion=cardLastBootVersion, cardRamSize=cardRamSize, portMode=portMode, cardTestResultDesc=cardTestResultDesc, auditCardId=auditCardId, cardCpuSpeed=cardCpuSpeed, cardPrStateChange=cardPrStateChange, cmRegistrationNotification=cmRegistrationNotification, auditStatus=auditStatus, portSecStateChange=portSecStateChange, cadEquipmentMibConformance=cadEquipmentMibConformance, cadEquipmentMib=cadEquipmentMib, cmMacAddress=cmMacAddress, cardLastResetReason=cardLastResetReason, consoleOutput=consoleOutput, auditDescription=auditDescription, portPrStateChange=portPrStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, portDplxStatusChange=portDplxStatusChange, cardPicDetected=cardPicDetected, shelfNumber=shelfNumber, cardSubDetected=cardSubDetected, cardSpareGroupMode=cardSpareGroupMode, diskVolumeGroup=diskVolumeGroup, cardTemperature=cardTemperature, portAction=portAction, portNumChans=portNumChans, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCpuType=cardCpuType, fpgaErrorEventEntry=fpgaErrorEventEntry, shelfSwVersion=shelfSwVersion, cardPicSerialNum=cardPicSerialNum, cmUChannelID=cmUChannelID, cardFwUpdateStatus=cardFwUpdateStatus, cardNumber=cardNumber, auditResult=auditResult, equipmentState=equipmentState, equipmentCardTestGroup=equipmentCardTestGroup, shelfName=shelfName, cardLastChangeTime=cardLastChangeTime, errEvRowStatus=errEvRowStatus, cardCarrierSerialNum=cardCarrierSerialNum, downstreamPowerLoss=downstreamPowerLoss, systemKey=systemKey, equipmentTbl=equipmentTbl, portNumber=portNumber, shelfObjects=shelfObjects, diskVolumeTable=diskVolumeTable, diskVolumeSize=diskVolumeSize, cardTrapInh=cardTrapInh, cardEntry=cardEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, auditAutoScheduling=auditAutoScheduling, diskVolumeCardId=diskVolumeCardId, cardDplxStatusChange=cardDplxStatusChange, cardTestTime=cardTestTime, diskVolumeName=diskVolumeName, auditEntry=auditEntry, portType=portType, cardUpTime=cardUpTime, cardCarrierFwVersion=cardCarrierFwVersion, portRxFlowControlMode=portRxFlowControlMode, cardTestDescription=cardTestDescription, cardSecStateChange=cardSecStateChange, auditLogOutput=auditLogOutput, cardTempNoReportNotification=cardTempNoReportNotification, cardTestTable=cardTestTable, equipmentPortGroup=equipmentPortGroup, auditTable=auditTable, auditCommand=auditCommand, cmResetStatus=cmResetStatus, TestSchedule=TestSchedule, equipmentTraps=equipmentTraps, cardTestIndex=cardTestIndex, cardName=cardName, portDetectedMode=portDetectedMode, diskVolumeDriveId=diskVolumeDriveId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetNotification=cmResetNotification, diskDriveNumber=diskDriveNumber, cardTestResult=cardTestResult, portConnectorId=portConnectorId, remainInDiagMode=remainInDiagMode, diskVolumeDiskSize=diskVolumeDiskSize, equipmentDiag=equipmentDiag, cmResetInfo=cmResetInfo, cardPicHwVersion=cardPicHwVersion, eqDiagConfig=eqDiagConfig, TestScheduleCommand=TestScheduleCommand, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestScheduleCommand=cardTestScheduleCommand, portEntry=portEntry, auditId=auditId, cardDetectedChange=cardDetectedChange, auditLogThrottle=auditLogThrottle, cardDplxStatus=cardDplxStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeTrapInh=diskVolumeTrapInh, cardFwVersion=cardFwVersion, cardPrState=cardPrState, cmResetUpTime=cmResetUpTime, cardLastBootSource=cardLastBootSource, diskVolumeNumber=diskVolumeNumber, portMaxDsPower=portMaxDsPower, cardShelfTestIndex=cardShelfTestIndex, equipmentAuditGroup=equipmentAuditGroup, cardBusSpeed=cardBusSpeed, cardSwVersion=cardSwVersion, diskVolumeId=diskVolumeId, cardHwVersion=cardHwVersion, diskVolumeFileName=diskVolumeFileName, cardTestId=cardTestId, auditTotalFailedCount=auditTotalFailedCount, portPrState=portPrState, systemGeneralGroup=systemGeneralGroup, equipmentShelfGroup=equipmentShelfGroup, cardFlashSize=cardFlashSize, portSecState=portSecState, cardAction=cardAction, portCardSubType=portCardSubType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmVendor=cmVendor, cmIpAddress=cmIpAddress, portRxFlowControlDetected=portRxFlowControlDetected, cardNumPorts=cardNumPorts, trapSeverity=trapSeverity, verboseLevel=verboseLevel, fpgaErrorEventTable=fpgaErrorEventTable, cardTestSchedule=cardTestSchedule, equipmentAudit=equipmentAudit, trapCounter=trapCounter, diskVolumeUsageLevel=diskVolumeUsageLevel, cardShelfId=cardShelfId, portMacIfIndex=portMacIfIndex, cmPrimarySID=cmPrimarySID, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvLoggingEnabled=errEvLoggingEnabled, cardSpareGroupId=cardSpareGroupId, TestCommand=TestCommand, portBgpId=portBgpId, portMacAddress=portMacAddress, cardDetected=cardDetected, portTable=portTable, portTxFlowControlMode=portTxFlowControlMode, portId=portId, diskVolumeEntry=diskVolumeEntry, portDescription=portDescription, auditCycleCount=auditCycleCount, auditFailedCount=auditFailedCount, cardTable=cardTable, cmResetReason=cmResetReason, diskVolumeShelfId=diskVolumeShelfId, cmResetClearNotification=cmResetClearNotification, portCardId=portCardId, auditTime=auditTime, cardTestResultNotification=cardTestResultNotification, equipmentCardGroup=equipmentCardGroup, portDplxStatus=portDplxStatus, portTrapInh=portTrapInh, auditShelfId=auditShelfId, equipmentStateGroup=equipmentStateGroup, TestType=TestType, systemClock=systemClock, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSerialNum=cardSerialNum)
