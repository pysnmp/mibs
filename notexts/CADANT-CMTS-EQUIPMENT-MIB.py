...
CardType, PortMode, PrimaryState, EqActionType, FirmwareSource, DiskVolumeUsageLevel, PortId, SecondaryState, ShelfId, UnknownState, CardSubType, DuplexStatus, PortType, AdminState, CardId, PortDetectedMode, FlowControlMode, PicType = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortMode", "PrimaryState", "EqActionType", "FirmwareSource", "DiskVolumeUsageLevel", "PortId", "SecondaryState", "ShelfId", "UnknownState", "CardSubType", "DuplexStatus", "PortType", "AdminState", "CardId", "PortDetectedMode", "FlowControlMode", "PicType")
...
RowStatus, DateAndTime, DisplayString, MacAddress, TimeStamp, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "DisplayString", "MacAddress", "TimeStamp", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeDriveId=diskVolumeDriveId, diskVolumeId=diskVolumeId, portMacIfIndex=portMacIfIndex, TestId=TestId, portCurrDsPower=portCurrDsPower, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfSwVersion=shelfSwVersion, diskVolumeEntry=diskVolumeEntry, cardSecState=cardSecState, cardFwVersion=cardFwVersion, cardTemperatureHighError=cardTemperatureHighError, portMaxDsPower=portMaxDsPower, diskVolumeName=diskVolumeName, portPrState=portPrState, portMode=portMode, errEvRecoveryEnabled=errEvRecoveryEnabled, cmMacAddress=cmMacAddress, TestSchedule=TestSchedule, cardTestId=cardTestId, portSecStateChange=portSecStateChange, cardLastBootSource=cardLastBootSource, cardDetected=cardDetected, cardCpuType=cardCpuType, auditStatus=auditStatus, portCardSubType=portCardSubType, cardTestDescription=cardTestDescription, cmVendor=cmVendor, auditLogOutput=auditLogOutput, cardPrStateChange=cardPrStateChange, portPrStateChange=portPrStateChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDplxStatusChange=portDplxStatusChange, auditCycleCount=auditCycleCount, auditCommand=auditCommand, auditTotalPassedCount=auditTotalPassedCount, errEvRowStatus=errEvRowStatus, cmResetNotification=cmResetNotification, cardTestIndex=cardTestIndex, portDocsIfIndex=portDocsIfIndex, cardSubType=cardSubType, cardPicSerialNum=cardPicSerialNum, cmResetInfo=cmResetInfo, downstreamPowerLoss=downstreamPowerLoss, portId=portId, cardSwVersion=cardSwVersion, cardTestResultNotification=cardTestResultNotification, portSecState=portSecState, portShelfId=portShelfId, cmResetUpTime=cmResetUpTime, cardUpTime=cardUpTime, cardTemperatureHighWarn=cardTemperatureHighWarn, shelfNumber=shelfNumber, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCarrierSerialNum=cardCarrierSerialNum, cardTestResult=cardTestResult, cardSubDetected=cardSubDetected, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeShelfId=diskVolumeShelfId, auditDescription=auditDescription, cardTrapInh=cardTrapInh, cardPicHwVersion=cardPicHwVersion, cardCarrierFwVersion=cardCarrierFwVersion, trapSeverity=trapSeverity, portDescription=portDescription, cardTemperature=cardTemperature, cardTestResultDesc=cardTestResultDesc, consoleOutput=consoleOutput, auditFailedCount=auditFailedCount, equipmentAuditGroup=equipmentAuditGroup, cardEntry=cardEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, portBgpId=portBgpId, diskVolumeTable=diskVolumeTable, verboseLevel=verboseLevel, auditId=auditId, cardTable=cardTable, portDetectedMode=portDetectedMode, portTxFlowControlDetected=portTxFlowControlDetected, systemKey=systemKey, TestScheduleCommand=TestScheduleCommand, cardHwVersion=cardHwVersion, cardCpuSpeed=cardCpuSpeed, cmUChannelID=cmUChannelID, diskDriveNumber=diskDriveNumber, cardSpareGroupId=cardSpareGroupId, systemGeneral=systemGeneral, cardShelfTestIndex=cardShelfTestIndex, remainInDiagMode=remainInDiagMode, cmPrimarySID=cmPrimarySID, portTable=portTable, equipmentAudit=equipmentAudit, equipmentCardTestGroup=equipmentCardTestGroup, cmResetReason=cmResetReason, auditCardId=auditCardId, equipmentStateGroup=equipmentStateGroup, cardTempNoReportNotification=cardTempNoReportNotification, cardName=cardName, portNumChans=portNumChans, cadEquipmentMib=cadEquipmentMib, trapCounter=trapCounter, shelfObjects=shelfObjects, cardSpareGroupMode=cardSpareGroupMode, diskVolumeSize=diskVolumeSize, cardNumber=cardNumber, cmIpAddress=cmIpAddress, cardNumPorts=cardNumPorts, auditName=auditName, cardTestTime=cardTestTime, cardDplxStatusChange=cardDplxStatusChange, portLastChangeTime=portLastChangeTime, equipmentTbl=equipmentTbl, cardBootVersion=cardBootVersion, cmResetClearNotification=cmResetClearNotification, TestType=TestType, TestTransactionId=TestTransactionId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentError=equipmentError, cardType=cardType, equipmentDiag=equipmentDiag, diskVolumeGroup=diskVolumeGroup, equipmentTraps=equipmentTraps, portType=portType, cardCPLDVersion=cardCPLDVersion, systemGeneralGroup=systemGeneralGroup, portAction=portAction, portTrapInh=portTrapInh, portMacAddress=portMacAddress, equipmentState=equipmentState, cardSerialNum=cardSerialNum, errEvLogLevel=errEvLogLevel, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardId=cardId, cardTestTransId=cardTestTransId, portAdminState=portAdminState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditShelfId=auditShelfId, cardTestType=cardTestType, PYSNMP_MODULE_ID=cadEquipmentMib, portDplxStatus=portDplxStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentShelfGroup=equipmentShelfGroup, cardPrState=cardPrState, auditTable=auditTable, cardPicDetected=cardPicDetected, TestResult=TestResult, cardLastBootVersion=cardLastBootVersion, portRxFlowControlMode=portRxFlowControlMode, cardBusSpeed=cardBusSpeed, eqDiagConfig=eqDiagConfig, cardFlashSize=cardFlashSize, errEventId=errEventId, diskVolumeNumber=diskVolumeNumber, portRxFlowControlDetected=portRxFlowControlDetected, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditPassedCount=auditPassedCount, portMinDsPower=portMinDsPower, auditEntry=auditEntry, errEvLoggingEnabled=errEvLoggingEnabled, cardTestCommand=cardTestCommand, cardTestSchedule=cardTestSchedule, cmResetStatus=cmResetStatus, diskVolumeFileName=diskVolumeFileName, auditAutoScheduling=auditAutoScheduling, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardShelfId=cardShelfId, cardTestTable=cardTestTable, cardTestScheduleCommand=cardTestScheduleCommand, portNumber=portNumber, systemClock=systemClock, diagTestId=diagTestId, cardTestName=cardTestName, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTrapInh=diskVolumeTrapInh, auditTotalFailedCount=auditTotalFailedCount, cmDevice=cmDevice, auditTime=auditTime, portConnectorId=portConnectorId, cardLastResetReason=cardLastResetReason, equipmentCardGroup=equipmentCardGroup, cardDplxStatus=cardDplxStatus, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, portEntry=portEntry, cardLastChangeTime=cardLastChangeTime, portCardId=portCardId, diskVolumeDiskSize=diskVolumeDiskSize, cardAdminState=cardAdminState, cardFwUpdateStatus=cardFwUpdateStatus, auditResult=auditResult, fpgaErrorEventTable=fpgaErrorEventTable, cardDetectedChange=cardDetectedChange, equipmentPortGroup=equipmentPortGroup, cardFpgaSource=cardFpgaSource, cardSecStateChange=cardSecStateChange, TestCommand=TestCommand, cardTestEntry=cardTestEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, shelfName=shelfName, cardRamSize=cardRamSize, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeCardId=diskVolumeCardId, cardAction=cardAction, auditLogThrottle=auditLogThrottle)
