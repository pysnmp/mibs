...
CardSubType, PortId, PortType, FlowControlMode, DiskVolumeUsageLevel, AdminState, PrimaryState, CardType, PortDetectedMode, UnknownState, ShelfId, PicType, CardId, DuplexStatus, FirmwareSource, EqActionType, SecondaryState, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "PortId", "PortType", "FlowControlMode", "DiskVolumeUsageLevel", "AdminState", "PrimaryState", "CardType", "PortDetectedMode", "UnknownState", "ShelfId", "PicType", "CardId", "DuplexStatus", "FirmwareSource", "EqActionType", "SecondaryState", "PortMode")
...
RowStatus, TruthValue, DateAndTime, MacAddress, TextualConvention, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TruthValue", "DateAndTime", "MacAddress", "TextualConvention", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetReason=cmResetReason, cardCarrierSerialNum=cardCarrierSerialNum, cardNumPorts=cardNumPorts, equipmentDiag=equipmentDiag, auditTime=auditTime, shelfNumber=shelfNumber, cardTestName=cardTestName, diskVolumeGroup=diskVolumeGroup, cardSwVersion=cardSwVersion, cardTemperature=cardTemperature, diskVolumeDriveId=diskVolumeDriveId, portDocsIfIndex=portDocsIfIndex, portLastChangeTime=portLastChangeTime, equipmentAudit=equipmentAudit, cardTestSchedule=cardTestSchedule, diskDriveNumber=diskDriveNumber, consoleOutput=consoleOutput, cardLastResetReason=cardLastResetReason, cardTestId=cardTestId, fpgaErrorEventEntry=fpgaErrorEventEntry, portNumChans=portNumChans, systemClock=systemClock, portDescription=portDescription, diskVolumeId=diskVolumeId, equipmentCardGroup=equipmentCardGroup, portMacAddress=portMacAddress, cardTrapInh=cardTrapInh, cardSecState=cardSecState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portSecStateChange=portSecStateChange, cardFwUpdateStatus=cardFwUpdateStatus, cmUChannelID=cmUChannelID, shelfObjects=shelfObjects, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditDescription=auditDescription, portTxFlowControlMode=portTxFlowControlMode, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPicHwVersion=cardPicHwVersion, cardNumber=cardNumber, portShelfId=portShelfId, diskVolumeUsageNotification=diskVolumeUsageNotification, auditCommand=auditCommand, cardSubDetected=cardSubDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditResult=auditResult, portMacIfIndex=portMacIfIndex, equipmentTraps=equipmentTraps, cardPicDetected=cardPicDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, portNumber=portNumber, diskVolumeNumber=diskVolumeNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeEntry=diskVolumeEntry, cardShelfTestIndex=cardShelfTestIndex, cardId=cardId, portTxFlowControlDetected=portTxFlowControlDetected, cmIpAddress=cmIpAddress, errEvLogLevel=errEvLogLevel, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupMode=cardSpareGroupMode, auditName=auditName, portDplxStatus=portDplxStatus, auditCardId=auditCardId, cardDplxStatusChange=cardDplxStatusChange, cmVendor=cmVendor, diskVolumeCardId=diskVolumeCardId, auditLogThrottle=auditLogThrottle, cardSerialNum=cardSerialNum, cardBootVersion=cardBootVersion, cardName=cardName, systemKey=systemKey, cmMacAddress=cmMacAddress, portMode=portMode, TestSchedule=TestSchedule, cardTestTransId=cardTestTransId, cardShelfId=cardShelfId, cadEquipmentMibConformance=cadEquipmentMibConformance, cadEquipmentMib=cadEquipmentMib, cmPrimarySID=cmPrimarySID, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeName=diskVolumeName, cardTestIndex=cardTestIndex, auditTable=auditTable, errEvRowStatus=errEvRowStatus, portTrapInh=portTrapInh, TestTransactionId=TestTransactionId, trapCounter=trapCounter, portEntry=portEntry, portCurrDsPower=portCurrDsPower, TestCommand=TestCommand, equipmentAuditGroup=equipmentAuditGroup, cardTestDescription=cardTestDescription, cardType=cardType, cardAction=cardAction, diskVolumeTable=diskVolumeTable, portRxFlowControlDetected=portRxFlowControlDetected, portDetectedMode=portDetectedMode, portCardSubType=portCardSubType, cardTestType=cardTestType, downstreamPowerLoss=downstreamPowerLoss, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastBootVersion=cardLastBootVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCarrierHwVersion=cardCarrierHwVersion, cardPrStateChange=cardPrStateChange, portBgpId=portBgpId, auditAutoScheduling=auditAutoScheduling, auditFailedCount=auditFailedCount, cmResetUpTime=cmResetUpTime, cardCpuSpeed=cardCpuSpeed, cardUpTime=cardUpTime, cmResetNotification=cmResetNotification, portRxFlowControlMode=portRxFlowControlMode, auditCycleCount=auditCycleCount, auditTotalFailedCount=auditTotalFailedCount, cardLastBootSource=cardLastBootSource, shelfName=shelfName, cardTestTable=cardTestTable, cmResetInfo=cmResetInfo, remainInDiagMode=remainInDiagMode, cardTestResultNotification=cardTestResultNotification, eqDiagConfig=eqDiagConfig, equipmentTbl=equipmentTbl, cardFlashSize=cardFlashSize, portSecState=portSecState, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentDiagGroup=equipmentDiagGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfSwVersion=shelfSwVersion, cardTestScheduleCommand=cardTestScheduleCommand, portType=portType, cardTemperatureHighError=cardTemperatureHighError, cardTestResultDesc=cardTestResultDesc, auditEntry=auditEntry, cardLastChangeTime=cardLastChangeTime, cmDevice=cmDevice, systemGeneral=systemGeneral, cardDetectedChange=cardDetectedChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDetected=cardDetected, verboseLevel=verboseLevel, cardEntry=cardEntry, cardHwVersion=cardHwVersion, cardTestResult=cardTestResult, cardFpgaSource=cardFpgaSource, cmResetStatus=cmResetStatus, auditPassedCount=auditPassedCount, auditTotalPassedCount=auditTotalPassedCount, portMinDsPower=portMinDsPower, cardDplxStatus=cardDplxStatus, TestResult=TestResult, cardRamSize=cardRamSize, cardTestEntry=cardTestEntry, diskVolumeFileName=diskVolumeFileName, cmRegistrationNotification=cmRegistrationNotification, auditId=auditId, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierFwVersion=cardCarrierFwVersion, portAdminState=portAdminState, equipmentPortGroup=equipmentPortGroup, TestType=TestType, portId=portId, fpgaErrorEventTable=fpgaErrorEventTable, auditStatus=auditStatus, errEventId=errEventId, diskVolumeShelfId=diskVolumeShelfId, cardFwVersion=cardFwVersion, cardTestCommand=cardTestCommand, auditLogOutput=auditLogOutput, trapSeverity=trapSeverity, cardPrState=cardPrState, TestId=TestId, portPrState=portPrState, portTable=portTable, equipmentState=equipmentState, portPrStateChange=portPrStateChange, portCardId=portCardId, equipmentShelfGroup=equipmentShelfGroup, portAction=portAction, portMaxDsPower=portMaxDsPower, cardSpareGroupId=cardSpareGroupId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentError=equipmentError, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestScheduleCommand=TestScheduleCommand, auditShelfId=auditShelfId, cardSecStateChange=cardSecStateChange, equipmentCardTestGroup=equipmentCardTestGroup, cardCPLDVersion=cardCPLDVersion, systemGeneralGroup=systemGeneralGroup, cardCpuType=cardCpuType, cardTestTime=cardTestTime, cardSubType=cardSubType, cmResetClearNotification=cmResetClearNotification, portConnectorId=portConnectorId, cardAdminState=cardAdminState, cardBusSpeed=cardBusSpeed, diskVolumeSize=diskVolumeSize, diskVolumeTrapInh=diskVolumeTrapInh, errEvLoggingEnabled=errEvLoggingEnabled, equipmentStateGroup=equipmentStateGroup, cardPicSerialNum=cardPicSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, diagTestId=diagTestId, cardTable=cardTable)
