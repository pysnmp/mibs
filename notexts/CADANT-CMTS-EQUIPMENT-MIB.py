...
AdminState, CardSubType, DuplexStatus, DiskVolumeUsageLevel, PortDetectedMode, CardId, FlowControlMode, PortType, UnknownState, PicType, PortId, FirmwareSource, EqActionType, ShelfId, PrimaryState, SecondaryState, CardType, PortMode = mibBuilder.importSymbols("CADANT-TC", "AdminState", "CardSubType", "DuplexStatus", "DiskVolumeUsageLevel", "PortDetectedMode", "CardId", "FlowControlMode", "PortType", "UnknownState", "PicType", "PortId", "FirmwareSource", "EqActionType", "ShelfId", "PrimaryState", "SecondaryState", "CardType", "PortMode")
...
DateAndTime, DisplayString, MacAddress, TimeStamp, TruthValue, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "DisplayString", "MacAddress", "TimeStamp", "TruthValue", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", systemClock=systemClock, cardSwVersion=cardSwVersion, diskVolumeSize=diskVolumeSize, portPrStateChange=portPrStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentPortGroup=equipmentPortGroup, trapCounter=trapCounter, cardTemperatureHighWarn=cardTemperatureHighWarn, TestSchedule=TestSchedule, cardSpareGroupId=cardSpareGroupId, cmDevice=cmDevice, cardBootVersion=cardBootVersion, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portPrState=portPrState, cardBusSpeed=cardBusSpeed, cardTestResult=cardTestResult, TestTransactionId=TestTransactionId, fpgaErrorEventGroup=fpgaErrorEventGroup, portTxFlowControlDetected=portTxFlowControlDetected, equipmentShelfGroup=equipmentShelfGroup, portRxFlowControlDetected=portRxFlowControlDetected, cmResetClearNotification=cmResetClearNotification, cardCarrierFwVersion=cardCarrierFwVersion, cardPrState=cardPrState, auditId=auditId, cardAdminState=cardAdminState, auditCommand=auditCommand, diskVolumeNumber=diskVolumeNumber, TestScheduleCommand=TestScheduleCommand, cardTestTable=cardTestTable, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardFwVersion=cardFwVersion, cardCpuType=cardCpuType, shelfName=shelfName, auditAutoScheduling=auditAutoScheduling, cardTestName=cardTestName, cardCPLDVersion=cardCPLDVersion, cardTestEntry=cardTestEntry, portSecState=portSecState, auditTable=auditTable, auditDescription=auditDescription, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentTbl=equipmentTbl, cmIpAddress=cmIpAddress, cardDplxStatus=cardDplxStatus, cardLastBootSource=cardLastBootSource, auditLogThrottle=auditLogThrottle, auditEntry=auditEntry, cardShelfTestIndex=cardShelfTestIndex, portTxFlowControlMode=portTxFlowControlMode, portDescription=portDescription, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetReason=cmResetReason, TestId=TestId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardId=cardId, cmRegistrationNotification=cmRegistrationNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPrStateChange=cardPrStateChange, eqDiagConfig=eqDiagConfig, cardTestId=cardTestId, cmVendor=cmVendor, systemGeneral=systemGeneral, cmPrimarySID=cmPrimarySID, portId=portId, cardTestScheduleCommand=cardTestScheduleCommand, equipmentTraps=equipmentTraps, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestCommand=cardTestCommand, systemKey=systemKey, portNumber=portNumber, shelfObjects=shelfObjects, auditTime=auditTime, cardSubDetected=cardSubDetected, cardPicSerialNum=cardPicSerialNum, auditCardId=auditCardId, cardUpTime=cardUpTime, cardLastChangeTime=cardLastChangeTime, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardAction=cardAction, cardNumPorts=cardNumPorts, auditTotalPassedCount=auditTotalPassedCount, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portNumChans=portNumChans, equipmentError=equipmentError, diskVolumeName=diskVolumeName, portDetectedMode=portDetectedMode, consoleOutput=consoleOutput, auditResult=auditResult, cardEntry=cardEntry, portEntry=portEntry, auditLogOutput=auditLogOutput, portMacIfIndex=portMacIfIndex, portDplxStatusChange=portDplxStatusChange, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeEntry=diskVolumeEntry, portTable=portTable, equipmentDiagGroup=equipmentDiagGroup, portShelfId=portShelfId, portCardSubType=portCardSubType, equipmentDiag=equipmentDiag, cmResetStatus=cmResetStatus, diskVolumeDiskSize=diskVolumeDiskSize, portAction=portAction, downstreamPowerLoss=downstreamPowerLoss, portConnectorId=portConnectorId, cardDplxStatusChange=cardDplxStatusChange, equipmentAudit=equipmentAudit, portMode=portMode, cardFpgaSource=cardFpgaSource, auditFailedCount=auditFailedCount, errEventId=errEventId, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfNumber=shelfNumber, cardRamSize=cardRamSize, cmResetUpTime=cmResetUpTime, cardPicHwVersion=cardPicHwVersion, cardSubType=cardSubType, TestResult=TestResult, TestCommand=TestCommand, portBgpId=portBgpId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCarrierSerialNum=cardCarrierSerialNum, equipmentStateGroup=equipmentStateGroup, cardType=cardType, shelfSwVersion=shelfSwVersion, equipmentAuditGroup=equipmentAuditGroup, cardSecStateChange=cardSecStateChange, auditPassedCount=auditPassedCount, portCardId=portCardId, cardTestResultDesc=cardTestResultDesc, cardLastBootVersion=cardLastBootVersion, diskVolumeTable=diskVolumeTable, cardTempOverHeatNotification=cardTempOverHeatNotification, cardNumber=cardNumber, diskVolumeDriveId=diskVolumeDriveId, errEvRowStatus=errEvRowStatus, cardTemperatureHighError=cardTemperatureHighError, portMacAddress=portMacAddress, diskVolumeGroup=diskVolumeGroup, cardTestDescription=cardTestDescription, fpgaErrorEventTable=fpgaErrorEventTable, cardTestResultNotification=cardTestResultNotification, auditTotalFailedCount=auditTotalFailedCount, cmResetNotification=cmResetNotification, TestType=TestType, cmUChannelID=cmUChannelID, portSecStateChange=portSecStateChange, portMinDsPower=portMinDsPower, portMaxDsPower=portMaxDsPower, systemGeneralGroup=systemGeneralGroup, diagTestId=diagTestId, cardFlashSize=cardFlashSize, equipmentCardTestGroup=equipmentCardTestGroup, cardShelfId=cardShelfId, cardSpareGroupMode=cardSpareGroupMode, verboseLevel=verboseLevel, cmMacAddress=cmMacAddress, portDocsIfIndex=portDocsIfIndex, portCurrDsPower=portCurrDsPower, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portDplxStatus=portDplxStatus, cardTrapInh=cardTrapInh, equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, cardTemperature=cardTemperature, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMib=cadEquipmentMib, auditShelfId=auditShelfId, auditCycleCount=auditCycleCount, portType=portType, errEvLogLevel=errEvLogLevel, cardTable=cardTable, cardDetectedChange=cardDetectedChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeShelfId=diskVolumeShelfId, diskDriveNumber=diskDriveNumber, cardDetected=cardDetected, cardTestType=cardTestType, portLastChangeTime=portLastChangeTime, diskVolumeId=diskVolumeId, equipmentCardGroup=equipmentCardGroup, cardCpuSpeed=cardCpuSpeed, cardFwUpdateStatus=cardFwUpdateStatus, cmResetInfo=cmResetInfo, portAdminState=portAdminState, cardTestTransId=cardTestTransId, errEvLoggingEnabled=errEvLoggingEnabled, cardPicDetected=cardPicDetected, auditStatus=auditStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, trapSeverity=trapSeverity, cardSecState=cardSecState, fpgaErrorEventEntry=fpgaErrorEventEntry, portRxFlowControlMode=portRxFlowControlMode, cardName=cardName, diskVolumeFileName=diskVolumeFileName, cardTestIndex=cardTestIndex, cardTestTime=cardTestTime, cardSerialNum=cardSerialNum, remainInDiagMode=remainInDiagMode, portTrapInh=portTrapInh, auditName=auditName, cardTestSchedule=cardTestSchedule, cardLastResetReason=cardLastResetReason, cardHwVersion=cardHwVersion)
