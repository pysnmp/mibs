...
DuplexStatus, CardSubType, SecondaryState, EqActionType, PortMode, CardId, PortType, ShelfId, PortId, FlowControlMode, PortDetectedMode, PicType, AdminState, UnknownState, DiskVolumeUsageLevel, PrimaryState, FirmwareSource, CardType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "CardSubType", "SecondaryState", "EqActionType", "PortMode", "CardId", "PortType", "ShelfId", "PortId", "FlowControlMode", "PortDetectedMode", "PicType", "AdminState", "UnknownState", "DiskVolumeUsageLevel", "PrimaryState", "FirmwareSource", "CardType")
...
TimeStamp, RowStatus, MacAddress, TextualConvention, DateAndTime, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "RowStatus", "MacAddress", "TextualConvention", "DateAndTime", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCpuSpeed=cardCpuSpeed, auditResult=auditResult, cardTestDescription=cardTestDescription, auditStatus=auditStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, systemGeneral=systemGeneral, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portCardId=portCardId, cardLastResetReason=cardLastResetReason, portMacIfIndex=portMacIfIndex, cardTestSchedule=cardTestSchedule, cardTempNoReportNotification=cardTempNoReportNotification, TestType=TestType, cardTemperatureHighError=cardTemperatureHighError, cardTestScheduleCommand=cardTestScheduleCommand, cardTable=cardTable, shelfName=shelfName, cardCarrierHwVersion=cardCarrierHwVersion, cardSwVersion=cardSwVersion, portLastChangeTime=portLastChangeTime, cardFlashSize=cardFlashSize, cardTestIndex=cardTestIndex, cmResetInfo=cmResetInfo, cmDevice=cmDevice, cardSecState=cardSecState, cardDetectedChange=cardDetectedChange, cadEquipmentMib=cadEquipmentMib, cardTestTransId=cardTestTransId, cardSubDetected=cardSubDetected, diskVolumeFileName=diskVolumeFileName, cardBusSpeed=cardBusSpeed, portMode=portMode, systemClock=systemClock, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmRegistrationNotification=cmRegistrationNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardEntry=cardEntry, portDetectedMode=portDetectedMode, consoleOutput=consoleOutput, portAction=portAction, auditLogThrottle=auditLogThrottle, cadEquipmentMibGroup=cadEquipmentMibGroup, errEventId=errEventId, TestScheduleCommand=TestScheduleCommand, errEvLoggingEnabled=errEvLoggingEnabled, errEvLogLevel=errEvLogLevel, portTrapInh=portTrapInh, PYSNMP_MODULE_ID=cadEquipmentMib, portTable=portTable, cardAdminState=cardAdminState, trapSeverity=trapSeverity, portTxFlowControlMode=portTxFlowControlMode, auditEntry=auditEntry, diskVolumeSize=diskVolumeSize, errEvRowStatus=errEvRowStatus, cmResetStatus=cmResetStatus, diskVolumeNumber=diskVolumeNumber, trapCounter=trapCounter, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicHwVersion=cardPicHwVersion, equipmentState=equipmentState, cmResetReason=cmResetReason, portNumChans=portNumChans, cardTestCommand=cardTestCommand, portMacAddress=portMacAddress, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardPicDetected=cardPicDetected, equipmentAudit=equipmentAudit, TestResult=TestResult, auditName=auditName, auditShelfId=auditShelfId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardType=cardType, cardTestType=cardTestType, portMinDsPower=portMinDsPower, cadEquipmentMibConformance=cadEquipmentMibConformance, portDocsIfIndex=portDocsIfIndex, cardName=cardName, cardDplxStatusChange=cardDplxStatusChange, portRxFlowControlMode=portRxFlowControlMode, cardPrStateChange=cardPrStateChange, portType=portType, portMaxDsPower=portMaxDsPower, auditTotalPassedCount=auditTotalPassedCount, cardCarrierFwVersion=cardCarrierFwVersion, auditId=auditId, diskVolumeTable=diskVolumeTable, diagTestId=diagTestId, cardTestResultDesc=cardTestResultDesc, cmResetUpTime=cmResetUpTime, equipmentTraps=equipmentTraps, diskVolumeGroup=diskVolumeGroup, shelfSwVersion=shelfSwVersion, portDplxStatus=portDplxStatus, cmVendor=cmVendor, portNumber=portNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestId=cardTestId, diskVolumeName=diskVolumeName, cardNumPorts=cardNumPorts, cardFwVersion=cardFwVersion, cmResetClearNotification=cmResetClearNotification, cardHwVersion=cardHwVersion, cardDplxStatus=cardDplxStatus, cardRamSize=cardRamSize, shelfNumber=shelfNumber, cardTestEntry=cardTestEntry, cmPrimarySID=cmPrimarySID, cardLastBootSource=cardLastBootSource, cardTestTime=cardTestTime, auditCardId=auditCardId, fpgaErrorEventTable=fpgaErrorEventTable, cmResetNotification=cmResetNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardCPLDVersion=cardCPLDVersion, shelfObjects=shelfObjects, downstreamPowerLoss=downstreamPowerLoss, cardSpareGroupMode=cardSpareGroupMode, verboseLevel=verboseLevel, cardId=cardId, auditPassedCount=auditPassedCount, portId=portId, portSecState=portSecState, TestSchedule=TestSchedule, systemKey=systemKey, equipmentShelfGroup=equipmentShelfGroup, portTxFlowControlDetected=portTxFlowControlDetected, portPrStateChange=portPrStateChange, cardPrState=cardPrState, diskDriveNumber=diskDriveNumber, errEvRecoveryEnabled=errEvRecoveryEnabled, auditLogOutput=auditLogOutput, cardSubType=cardSubType, equipmentPortGroup=equipmentPortGroup, eqDiagConfig=eqDiagConfig, cardUpTime=cardUpTime, cardNumber=cardNumber, cardCpuType=cardCpuType, portAdminState=portAdminState, equipmentDiag=equipmentDiag, cardTestTable=cardTestTable, cardDetected=cardDetected, portPrState=portPrState, cardTestResult=cardTestResult, cmMacAddress=cmMacAddress, equipmentStateGroup=equipmentStateGroup, auditCommand=auditCommand, cardFpgaSource=cardFpgaSource, cardLastChangeTime=cardLastChangeTime, TestCommand=TestCommand, cardShelfTestIndex=cardShelfTestIndex, cmUChannelID=cmUChannelID, portCurrDsPower=portCurrDsPower, auditTime=auditTime, portBgpId=portBgpId, auditTable=auditTable, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeCardId=diskVolumeCardId, portCardSubType=portCardSubType, portDescription=portDescription, cardTestResultNotification=cardTestResultNotification, cardFwUpdateStatus=cardFwUpdateStatus, equipmentCardTestGroup=equipmentCardTestGroup, portDplxStatusChange=portDplxStatusChange, auditTotalFailedCount=auditTotalFailedCount, cardTrapInh=cardTrapInh, auditDescription=auditDescription, equipmentTbl=equipmentTbl, equipmentError=equipmentError, diskVolumeTrapInh=diskVolumeTrapInh, cardTestName=cardTestName, cardSerialNum=cardSerialNum, cardShelfId=cardShelfId, diskVolumeDriveId=diskVolumeDriveId, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentAuditGroup=equipmentAuditGroup, portEntry=portEntry, portShelfId=portShelfId, cardLastBootVersion=cardLastBootVersion, auditFailedCount=auditFailedCount, diskVolumeShelfId=diskVolumeShelfId, cardBootVersion=cardBootVersion, equipmentCardGroup=equipmentCardGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, systemGeneralGroup=systemGeneralGroup, diskVolumeId=diskVolumeId, TestId=TestId, cardTemperature=cardTemperature, cardSpareGroupId=cardSpareGroupId, TestTransactionId=TestTransactionId, diskVolumeEntry=diskVolumeEntry, cardSecStateChange=cardSecStateChange, portSecStateChange=portSecStateChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portConnectorId=portConnectorId, diskVolumeUsageNotification=diskVolumeUsageNotification, auditCycleCount=auditCycleCount, auditAutoScheduling=auditAutoScheduling, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmIpAddress=cmIpAddress, cardAction=cardAction, fpgaErrorEventEntry=fpgaErrorEventEntry, remainInDiagMode=remainInDiagMode)
