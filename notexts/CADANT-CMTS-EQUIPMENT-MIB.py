...
FlowControlMode, PicType, CardType, PortType, PortDetectedMode, UnknownState, ShelfId, AdminState, PortMode, EqActionType, DiskVolumeUsageLevel, DuplexStatus, CardSubType, PrimaryState, CardId, FirmwareSource, PortId, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PicType", "CardType", "PortType", "PortDetectedMode", "UnknownState", "ShelfId", "AdminState", "PortMode", "EqActionType", "DiskVolumeUsageLevel", "DuplexStatus", "CardSubType", "PrimaryState", "CardId", "FirmwareSource", "PortId", "SecondaryState")
...
TextualConvention, TimeStamp, TruthValue, DateAndTime, RowStatus, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TimeStamp", "TruthValue", "DateAndTime", "RowStatus", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageLevel=diskVolumeUsageLevel, portLastChangeTime=portLastChangeTime, equipmentPortGroup=equipmentPortGroup, diskVolumeDriveId=diskVolumeDriveId, cardTestTime=cardTestTime, cardTempOverHeatNotification=cardTempOverHeatNotification, portMacIfIndex=portMacIfIndex, portSecState=portSecState, auditCardId=auditCardId, cardHwVersion=cardHwVersion, auditTable=auditTable, cardCPLDVersion=cardCPLDVersion, cardAdminState=cardAdminState, cmVendor=cmVendor, equipmentAuditGroup=equipmentAuditGroup, equipmentStateGroup=equipmentStateGroup, portRxFlowControlMode=portRxFlowControlMode, cardTestId=cardTestId, cardSecState=cardSecState, portCardSubType=portCardSubType, auditStatus=auditStatus, portSecStateChange=portSecStateChange, cardName=cardName, portDplxStatusChange=portDplxStatusChange, cardTrapInh=cardTrapInh, portAction=portAction, cardSpareGroupId=cardSpareGroupId, trapCounter=trapCounter, portMacAddress=portMacAddress, errEvLogLevel=errEvLogLevel, equipmentCardTestGroup=equipmentCardTestGroup, remainInDiagMode=remainInDiagMode, auditCycleCount=auditCycleCount, systemGeneral=systemGeneral, shelfName=shelfName, cardShelfId=cardShelfId, cardTestTable=cardTestTable, TestResult=TestResult, diskVolumeTable=diskVolumeTable, TestScheduleCommand=TestScheduleCommand, equipmentState=equipmentState, diskVolumeGroup=diskVolumeGroup, cardSecStateChange=cardSecStateChange, cardSerialNum=cardSerialNum, eqDiagConfig=eqDiagConfig, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentTbl=equipmentTbl, cardCarrierFwVersion=cardCarrierFwVersion, TestSchedule=TestSchedule, cardEntry=cardEntry, diskVolumeId=diskVolumeId, cmResetUpTime=cmResetUpTime, auditLogOutput=auditLogOutput, cardPicDetected=cardPicDetected, portBgpId=portBgpId, cardTestResultDesc=cardTestResultDesc, systemGeneralGroup=systemGeneralGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardPrState=cardPrState, auditTotalPassedCount=auditTotalPassedCount, errEvRecoveryEnabled=errEvRecoveryEnabled, TestTransactionId=TestTransactionId, shelfNumber=shelfNumber, cardSpareGroupMode=cardSpareGroupMode, cardTestName=cardTestName, TestId=TestId, portNumber=portNumber, cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, portRxFlowControlDetected=portRxFlowControlDetected, cmResetInfo=cmResetInfo, portType=portType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditName=auditName, diskVolumeShelfId=diskVolumeShelfId, portTable=portTable, portDescription=portDescription, cardFwVersion=cardFwVersion, auditId=auditId, equipmentTraps=equipmentTraps, auditShelfId=auditShelfId, shelfObjects=shelfObjects, portTxFlowControlMode=portTxFlowControlMode, equipmentAudit=equipmentAudit, TestType=TestType, cardFwUpdateStatus=cardFwUpdateStatus, auditFailedCount=auditFailedCount, systemClock=systemClock, cardSwVersion=cardSwVersion, diskDriveNumber=diskDriveNumber, cadEquipmentMibCompliance=cadEquipmentMibCompliance, errEvRowStatus=errEvRowStatus, portDplxStatus=portDplxStatus, portNumChans=portNumChans, cmResetReason=cmResetReason, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portPrStateChange=portPrStateChange, cardTemperatureHighError=cardTemperatureHighError, cardTestEntry=cardTestEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardBusSpeed=cardBusSpeed, cardLastBootSource=cardLastBootSource, cardCarrierHwVersion=cardCarrierHwVersion, portId=portId, cardPicHwVersion=cardPicHwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, auditResult=auditResult, cardTempNoReportNotification=cardTempNoReportNotification, cardSubDetected=cardSubDetected, portShelfId=portShelfId, portCardId=portCardId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardCpuSpeed=cardCpuSpeed, cardCpuType=cardCpuType, portTxFlowControlDetected=portTxFlowControlDetected, equipmentDiag=equipmentDiag, cardType=cardType, PYSNMP_MODULE_ID=cadEquipmentMib, consoleOutput=consoleOutput, equipmentError=equipmentError, cardDetectedChange=cardDetectedChange, TestCommand=TestCommand, portDetectedMode=portDetectedMode, trapSeverity=trapSeverity, cardUpTime=cardUpTime, cardNumber=cardNumber, cardTestSchedule=cardTestSchedule, systemKey=systemKey, diskVolumeDiskSize=diskVolumeDiskSize, cmMacAddress=cmMacAddress, cardLastBootVersion=cardLastBootVersion, equipmentCardGroup=equipmentCardGroup, cardTable=cardTable, cardLastChangeTime=cardLastChangeTime, cardFlashSize=cardFlashSize, diskVolumeSize=diskVolumeSize, portEntry=portEntry, portDocsIfIndex=portDocsIfIndex, auditTime=auditTime, cardTestIndex=cardTestIndex, errEvLoggingEnabled=errEvLoggingEnabled, downstreamPowerLoss=downstreamPowerLoss, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetClearNotification=cmResetClearNotification, cardRamSize=cardRamSize, cadEquipmentMib=cadEquipmentMib, cardSubType=cardSubType, errEventId=errEventId, auditDescription=auditDescription, cardTestResultNotification=cardTestResultNotification, portMode=portMode, cardTestType=cardTestType, portMaxDsPower=portMaxDsPower, cmDevice=cmDevice, cardTestDescription=cardTestDescription, cardTestCommand=cardTestCommand, auditAutoScheduling=auditAutoScheduling, portTrapInh=portTrapInh, diskVolumeName=diskVolumeName, portAdminState=portAdminState, auditEntry=auditEntry, shelfSwVersion=shelfSwVersion, portPrState=portPrState, cardTestTransId=cardTestTransId, auditLogThrottle=auditLogThrottle, cardId=cardId, cardNumPorts=cardNumPorts, cardBootVersion=cardBootVersion, portConnectorId=portConnectorId, cardFpgaSource=cardFpgaSource, auditPassedCount=auditPassedCount, cmResetNotification=cmResetNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTemperature=cardTemperature, cardCarrierSerialNum=cardCarrierSerialNum, equipmentShelfGroup=equipmentShelfGroup, diskVolumeCardId=diskVolumeCardId, cardAction=cardAction, auditTotalFailedCount=auditTotalFailedCount, portCurrDsPower=portCurrDsPower, diskVolumeFileName=diskVolumeFileName, cmIpAddress=cmIpAddress, fpgaErrorEventTable=fpgaErrorEventTable, cardDetected=cardDetected, auditCommand=auditCommand, cmRegistrationNotification=cmRegistrationNotification, cardDplxStatus=cardDplxStatus, cardTestScheduleCommand=cardTestScheduleCommand, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentDiagGroup=equipmentDiagGroup, cardDplxStatusChange=cardDplxStatusChange, portMinDsPower=portMinDsPower, diskVolumeNumber=diskVolumeNumber, diskVolumeEntry=diskVolumeEntry, fpgaErrorEventEntry=fpgaErrorEventEntry, cmPrimarySID=cmPrimarySID, cmUChannelID=cmUChannelID, cardLastResetReason=cardLastResetReason, diskVolumeTrapInh=diskVolumeTrapInh, cmResetStatus=cmResetStatus, diagTestId=diagTestId, cardPrStateChange=cardPrStateChange, cardTestResult=cardTestResult, diskVolumeUsagePercentage=diskVolumeUsagePercentage, verboseLevel=verboseLevel)
