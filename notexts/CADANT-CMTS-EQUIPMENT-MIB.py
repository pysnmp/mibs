...
PortId, CardId, PrimaryState, CardSubType, AdminState, PortMode, ShelfId, CardType, UnknownState, DuplexStatus, PortType, SecondaryState, FlowControlMode, DiskVolumeUsageLevel, EqActionType, PortDetectedMode, FirmwareSource, PicType = mibBuilder.importSymbols("CADANT-TC", "PortId", "CardId", "PrimaryState", "CardSubType", "AdminState", "PortMode", "ShelfId", "CardType", "UnknownState", "DuplexStatus", "PortType", "SecondaryState", "FlowControlMode", "DiskVolumeUsageLevel", "EqActionType", "PortDetectedMode", "FirmwareSource", "PicType")
...
TextualConvention, DisplayString, MacAddress, RowStatus, TimeStamp, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "MacAddress", "RowStatus", "TimeStamp", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentTraps=equipmentTraps, cardRamSize=cardRamSize, errEvRowStatus=errEvRowStatus, cardPicDetected=cardPicDetected, portDocsIfIndex=portDocsIfIndex, auditTotalPassedCount=auditTotalPassedCount, cardSwVersion=cardSwVersion, cardPrState=cardPrState, cardBootVersion=cardBootVersion, cardId=cardId, portPrState=portPrState, eqDiagConfig=eqDiagConfig, fpgaErrorEventTable=fpgaErrorEventTable, portCardId=portCardId, diskVolumeDriveId=diskVolumeDriveId, cmMacAddress=cmMacAddress, TestTransactionId=TestTransactionId, equipmentCardTestGroup=equipmentCardTestGroup, cardFwUpdateStatus=cardFwUpdateStatus, equipmentDiagGroup=equipmentDiagGroup, equipmentShelfGroup=equipmentShelfGroup, portNumber=portNumber, cardTemperature=cardTemperature, cardPrStateChange=cardPrStateChange, portSecState=portSecState, cardDetected=cardDetected, cardType=cardType, auditLogThrottle=auditLogThrottle, auditPassedCount=auditPassedCount, cardCarrierSerialNum=cardCarrierSerialNum, portTrapInh=portTrapInh, diskVolumeDiskSize=diskVolumeDiskSize, cardTestType=cardTestType, cmPrimarySID=cmPrimarySID, auditName=auditName, portPrStateChange=portPrStateChange, cardFpgaSource=cardFpgaSource, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, systemGeneral=systemGeneral, auditFailedCount=auditFailedCount, equipmentAuditGroup=equipmentAuditGroup, cardCarrierHwVersion=cardCarrierHwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastChangeTime=cardLastChangeTime, equipmentState=equipmentState, auditCycleCount=auditCycleCount, portMode=portMode, portBgpId=portBgpId, cardName=cardName, auditShelfId=auditShelfId, cardCpuType=cardCpuType, diskVolumeTrapInh=diskVolumeTrapInh, cmResetReason=cmResetReason, cardTestResultDesc=cardTestResultDesc, portDplxStatus=portDplxStatus, auditAutoScheduling=auditAutoScheduling, cardTrapInh=cardTrapInh, cardTemperatureHighError=cardTemperatureHighError, cardTestDescription=cardTestDescription, shelfName=shelfName, diskVolumeNumber=diskVolumeNumber, TestType=TestType, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestIndex=cardTestIndex, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditResult=auditResult, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDescription=portDescription, TestId=TestId, cadEquipmentMib=cadEquipmentMib, equipmentStateGroup=equipmentStateGroup, diskVolumeId=diskVolumeId, cardTestTransId=cardTestTransId, cardTempNoReportNotification=cardTempNoReportNotification, cardSpareGroupId=cardSpareGroupId, verboseLevel=verboseLevel, portAction=portAction, cardSecState=cardSecState, cardShelfTestIndex=cardShelfTestIndex, cardHwVersion=cardHwVersion, cardLastBootSource=cardLastBootSource, cardTestTime=cardTestTime, shelfSwVersion=shelfSwVersion, trapSeverity=trapSeverity, fpgaErrorEventEntry=fpgaErrorEventEntry, cardNumber=cardNumber, cardAdminState=cardAdminState, diskVolumeCardId=diskVolumeCardId, portAdminState=portAdminState, portMacAddress=portMacAddress, cmResetStatus=cmResetStatus, portSecStateChange=portSecStateChange, shelfNumber=shelfNumber, diskVolumeName=diskVolumeName, cardFlashSize=cardFlashSize, cardLastResetReason=cardLastResetReason, diskVolumeShelfId=diskVolumeShelfId, errEvLoggingEnabled=errEvLoggingEnabled, TestCommand=TestCommand, cardPicSerialNum=cardPicSerialNum, auditTotalFailedCount=auditTotalFailedCount, TestScheduleCommand=TestScheduleCommand, cardDplxStatus=cardDplxStatus, cardCPLDVersion=cardCPLDVersion, diskDriveNumber=diskDriveNumber, cardSubDetected=cardSubDetected, diskVolumeEntry=diskVolumeEntry, portDplxStatusChange=portDplxStatusChange, equipmentCardGroup=equipmentCardGroup, auditCommand=auditCommand, cardDetectedChange=cardDetectedChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeTable=diskVolumeTable, cardDplxStatusChange=cardDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestSchedule=cardTestSchedule, auditCardId=auditCardId, systemGeneralGroup=systemGeneralGroup, equipmentTbl=equipmentTbl, auditEntry=auditEntry, portConnectorId=portConnectorId, portLastChangeTime=portLastChangeTime, portTxFlowControlMode=portTxFlowControlMode, portCurrDsPower=portCurrDsPower, auditLogOutput=auditLogOutput, auditDescription=auditDescription, cmUChannelID=cmUChannelID, shelfObjects=shelfObjects, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, remainInDiagMode=remainInDiagMode, cardLastBootVersion=cardLastBootVersion, systemClock=systemClock, cadEquipmentMibConformance=cadEquipmentMibConformance, errEventId=errEventId, cardTestEntry=cardTestEntry, portDetectedMode=portDetectedMode, errEvLogLevel=errEvLogLevel, cardFwVersion=cardFwVersion, cardCarrierFwVersion=cardCarrierFwVersion, systemKey=systemKey, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSpareGroupMode=cardSpareGroupMode, cmResetClearNotification=cmResetClearNotification, cardEntry=cardEntry, diskVolumeGroup=diskVolumeGroup, equipmentAudit=equipmentAudit, portTxFlowControlDetected=portTxFlowControlDetected, cardSerialNum=cardSerialNum, cardTestId=cardTestId, auditTable=auditTable, cmResetInfo=cmResetInfo, cardSecStateChange=cardSecStateChange, diskVolumeSize=diskVolumeSize, cardCpuSpeed=cardCpuSpeed, portShelfId=portShelfId, cardTable=cardTable, diagTestId=diagTestId, cmIpAddress=cmIpAddress, downstreamPowerLoss=downstreamPowerLoss, cmVendor=cmVendor, cardUpTime=cardUpTime, portId=portId, cardSubType=cardSubType, equipmentDiag=equipmentDiag, auditId=auditId, portTable=portTable, cmResetUpTime=cmResetUpTime, cardShelfId=cardShelfId, portEntry=portEntry, cardTestResult=cardTestResult, trapCounter=trapCounter, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portRxFlowControlDetected=portRxFlowControlDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, cardAction=cardAction, consoleOutput=consoleOutput, portMinDsPower=portMinDsPower, cmDevice=cmDevice, diskVolumeFileName=diskVolumeFileName, cardTestCommand=cardTestCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, portMacIfIndex=portMacIfIndex, cardPicHwVersion=cardPicHwVersion, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestName=cardTestName, portNumChans=portNumChans, portMaxDsPower=portMaxDsPower, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResultNotification=cardTestResultNotification, cardTestScheduleCommand=cardTestScheduleCommand, equipmentPortGroup=equipmentPortGroup, TestSchedule=TestSchedule, portType=portType, cmResetNotification=cmResetNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, cmRegistrationNotification=cmRegistrationNotification, auditTime=auditTime, auditStatus=auditStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestTable=cardTestTable, portCardSubType=portCardSubType, equipmentError=equipmentError, cardNumPorts=cardNumPorts, cardBusSpeed=cardBusSpeed, TestResult=TestResult)
