...
ShelfId, PortMode, CardType, AdminState, PortId, PortType, PortDetectedMode, SecondaryState, PicType, DiskVolumeUsageLevel, CardSubType, DuplexStatus, UnknownState, CardId, PrimaryState, EqActionType, FlowControlMode, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "PortMode", "CardType", "AdminState", "PortId", "PortType", "PortDetectedMode", "SecondaryState", "PicType", "DiskVolumeUsageLevel", "CardSubType", "DuplexStatus", "UnknownState", "CardId", "PrimaryState", "EqActionType", "FlowControlMode", "FirmwareSource")
...
RowStatus, TruthValue, MacAddress, DisplayString, DateAndTime, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TruthValue", "MacAddress", "DisplayString", "DateAndTime", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditLogOutput=auditLogOutput, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeNumber=diskVolumeNumber, diskVolumeName=diskVolumeName, cardSecState=cardSecState, eqDiagConfig=eqDiagConfig, equipmentShelfGroup=equipmentShelfGroup, cardTestResultNotification=cardTestResultNotification, portDetectedMode=portDetectedMode, cardUpTime=cardUpTime, portMacAddress=portMacAddress, portMacIfIndex=portMacIfIndex, fpgaErrorEventTable=fpgaErrorEventTable, equipmentDiag=equipmentDiag, cardLastResetReason=cardLastResetReason, downstreamPowerLoss=downstreamPowerLoss, cardTestEntry=cardTestEntry, cardTestScheduleCommand=cardTestScheduleCommand, portCardSubType=portCardSubType, portTable=portTable, cardCpuSpeed=cardCpuSpeed, equipmentDiagGroup=equipmentDiagGroup, cardPrStateChange=cardPrStateChange, cardTemperatureHighWarn=cardTemperatureHighWarn, portPrStateChange=portPrStateChange, portMaxDsPower=portMaxDsPower, portAction=portAction, cardSpareGroupMode=cardSpareGroupMode, consoleOutput=consoleOutput, diskDriveNumber=diskDriveNumber, auditEntry=auditEntry, cardTestTransId=cardTestTransId, cardDetectedChange=cardDetectedChange, portTxFlowControlMode=portTxFlowControlMode, cmVendor=cmVendor, equipmentTbl=equipmentTbl, cardBusSpeed=cardBusSpeed, cardPicHwVersion=cardPicHwVersion, portShelfId=portShelfId, portCurrDsPower=portCurrDsPower, auditDescription=auditDescription, cardSubType=cardSubType, portTxFlowControlDetected=portTxFlowControlDetected, auditResult=auditResult, cardTempOverHeatNotification=cardTempOverHeatNotification, portCardId=portCardId, portPrState=portPrState, auditTime=auditTime, shelfName=shelfName, equipmentState=equipmentState, systemGeneral=systemGeneral, cardDetected=cardDetected, shelfNumber=shelfNumber, cardSubDetected=cardSubDetected, cmResetReason=cmResetReason, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFpgaSource=cardFpgaSource, cardSecStateChange=cardSecStateChange, cardBootVersion=cardBootVersion, portBgpId=portBgpId, cardTestResult=cardTestResult, PYSNMP_MODULE_ID=cadEquipmentMib, cardEntry=cardEntry, equipmentAudit=equipmentAudit, auditTotalFailedCount=auditTotalFailedCount, cardSwVersion=cardSwVersion, TestScheduleCommand=TestScheduleCommand, TestTransactionId=TestTransactionId, shelfSwVersion=shelfSwVersion, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, cadEquipmentMib=cadEquipmentMib, systemClock=systemClock, cardAction=cardAction, portSecStateChange=portSecStateChange, cmResetClearNotification=cmResetClearNotification, cardFwUpdateStatus=cardFwUpdateStatus, auditFailedCount=auditFailedCount, portConnectorId=portConnectorId, cardSerialNum=cardSerialNum, cardTestIndex=cardTestIndex, portAdminState=portAdminState, cardShelfId=cardShelfId, diskVolumeEntry=diskVolumeEntry, auditStatus=auditStatus, auditCardId=auditCardId, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestTable=cardTestTable, cardCarrierSerialNum=cardCarrierSerialNum, portLastChangeTime=portLastChangeTime, cardAdminState=cardAdminState, cardPicDetected=cardPicDetected, auditTable=auditTable, portNumChans=portNumChans, cmIpAddress=cmIpAddress, diskVolumeTrapInh=diskVolumeTrapInh, TestType=TestType, cardPrState=cardPrState, cardTestCommand=cardTestCommand, portSecState=portSecState, cardTestDescription=cardTestDescription, trapCounter=trapCounter, diskVolumeSize=diskVolumeSize, portId=portId, portDocsIfIndex=portDocsIfIndex, errEvLogLevel=errEvLogLevel, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardPicSerialNum=cardPicSerialNum, TestCommand=TestCommand, cardCpuType=cardCpuType, equipmentCardGroup=equipmentCardGroup, cardTestResultDesc=cardTestResultDesc, cardTemperatureHighError=cardTemperatureHighError, cardCarrierFwVersion=cardCarrierFwVersion, cardTestId=cardTestId, cmResetInfo=cmResetInfo, equipmentTraps=equipmentTraps, cardDplxStatusChange=cardDplxStatusChange, cardSpareGroupId=cardSpareGroupId, TestSchedule=TestSchedule, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, errEventId=errEventId, cmResetStatus=cmResetStatus, auditCommand=auditCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestResult=TestResult, equipmentCardTestGroup=equipmentCardTestGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardTestSchedule=cardTestSchedule, portDplxStatusChange=portDplxStatusChange, equipmentAuditGroup=equipmentAuditGroup, auditLogThrottle=auditLogThrottle, cardNumber=cardNumber, cardNumPorts=cardNumPorts, cardLastBootVersion=cardLastBootVersion, cardShelfTestIndex=cardShelfTestIndex, diskVolumeShelfId=diskVolumeShelfId, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardRamSize=cardRamSize, portMode=portMode, portTrapInh=portTrapInh, errEvLoggingEnabled=errEvLoggingEnabled, cardHwVersion=cardHwVersion, cardLastChangeTime=cardLastChangeTime, cardTrapInh=cardTrapInh, cardTestName=cardTestName, cmDevice=cmDevice, portDplxStatus=portDplxStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, portType=portType, systemKey=systemKey, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetNotification=cmResetNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeId=diskVolumeId, diskVolumeDiskSize=diskVolumeDiskSize, auditCycleCount=auditCycleCount, systemGeneralGroup=systemGeneralGroup, cardFwVersion=cardFwVersion, cardLastBootSource=cardLastBootSource, TestId=TestId, cardTemperature=cardTemperature, cardDplxStatus=cardDplxStatus, auditPassedCount=auditPassedCount, cardTestType=cardTestType, errEvRowStatus=errEvRowStatus, trapSeverity=trapSeverity, diskVolumeUsageLevel=diskVolumeUsageLevel, cardCPLDVersion=cardCPLDVersion, diskVolumeFileName=diskVolumeFileName, cardFlashSize=cardFlashSize, verboseLevel=verboseLevel, equipmentPortGroup=equipmentPortGroup, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentStateGroup=equipmentStateGroup, portNumber=portNumber, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentError=equipmentError, cmUChannelID=cmUChannelID, cardTestTime=cardTestTime, cardTable=cardTable, cardType=cardType, diagTestId=diagTestId, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeCardId=diskVolumeCardId, portMinDsPower=portMinDsPower, auditShelfId=auditShelfId, auditId=auditId, cardName=cardName, portDescription=portDescription, cardId=cardId, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditName=auditName, shelfObjects=shelfObjects, cmResetUpTime=cmResetUpTime, cmPrimarySID=cmPrimarySID, portEntry=portEntry, diskVolumeDriveId=diskVolumeDriveId, diskVolumeGroup=diskVolumeGroup, remainInDiagMode=remainInDiagMode, diskVolumeTable=diskVolumeTable, cardCarrierHwVersion=cardCarrierHwVersion, cmMacAddress=cmMacAddress)
