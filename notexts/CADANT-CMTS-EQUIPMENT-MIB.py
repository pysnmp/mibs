...
CardType, PicType, PortId, EqActionType, CardId, DuplexStatus, PortDetectedMode, PortMode, CardSubType, ShelfId, FlowControlMode, FirmwareSource, PortType, SecondaryState, AdminState, PrimaryState, UnknownState, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "CardType", "PicType", "PortId", "EqActionType", "CardId", "DuplexStatus", "PortDetectedMode", "PortMode", "CardSubType", "ShelfId", "FlowControlMode", "FirmwareSource", "PortType", "SecondaryState", "AdminState", "PrimaryState", "UnknownState", "DiskVolumeUsageLevel")
...
DateAndTime, TruthValue, TextualConvention, DisplayString, TimeStamp, RowStatus, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TruthValue", "TextualConvention", "DisplayString", "TimeStamp", "RowStatus", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentTbl=equipmentTbl, cmResetUpTime=cmResetUpTime, cardLastBootSource=cardLastBootSource, cardCarrierSerialNum=cardCarrierSerialNum, equipmentCardGroup=equipmentCardGroup, portAction=portAction, cardTestDescription=cardTestDescription, cmResetNotification=cmResetNotification, auditCycleCount=auditCycleCount, cardFpgaSource=cardFpgaSource, cardSpareGroupId=cardSpareGroupId, trapSeverity=trapSeverity, cardSerialNum=cardSerialNum, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighError=cardTemperatureHighError, cardTestResultDesc=cardTestResultDesc, cardShelfId=cardShelfId, diskVolumeSize=diskVolumeSize, downstreamPowerLoss=downstreamPowerLoss, equipmentDiagGroup=equipmentDiagGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardCarrierHwVersion=cardCarrierHwVersion, portMaxDsPower=portMaxDsPower, portTxFlowControlMode=portTxFlowControlMode, auditTotalPassedCount=auditTotalPassedCount, cardNumPorts=cardNumPorts, cmResetStatus=cmResetStatus, portDetectedMode=portDetectedMode, cmMacAddress=cmMacAddress, cardTestTime=cardTestTime, auditName=auditName, portNumber=portNumber, auditLogOutput=auditLogOutput, cardCPLDVersion=cardCPLDVersion, systemGeneralGroup=systemGeneralGroup, diskVolumeTable=diskVolumeTable, cardTestScheduleCommand=cardTestScheduleCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentAudit=equipmentAudit, cardBootVersion=cardBootVersion, diskVolumeCardId=diskVolumeCardId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeGroup=diskVolumeGroup, cardUpTime=cardUpTime, TestId=TestId, portMacIfIndex=portMacIfIndex, cmResetInfo=cmResetInfo, auditPassedCount=auditPassedCount, cmVendor=cmVendor, portPrStateChange=portPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, cardShelfTestIndex=cardShelfTestIndex, portRxFlowControlDetected=portRxFlowControlDetected, TestCommand=TestCommand, cardRamSize=cardRamSize, diskVolumeNumber=diskVolumeNumber, TestType=TestType, portBgpId=portBgpId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardLastChangeTime=cardLastChangeTime, portId=portId, diskVolumeEntry=diskVolumeEntry, cardBusSpeed=cardBusSpeed, fpgaErrorEventGroup=fpgaErrorEventGroup, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibConformance=cadEquipmentMibConformance, auditEntry=auditEntry, portCardSubType=portCardSubType, portCurrDsPower=portCurrDsPower, errEventId=errEventId, cardTrapInh=cardTrapInh, cardDetectedChange=cardDetectedChange, diskVolumeUsageNotification=diskVolumeUsageNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, shelfNumber=shelfNumber, cardTable=cardTable, equipmentStateGroup=equipmentStateGroup, PYSNMP_MODULE_ID=cadEquipmentMib, portPrState=portPrState, consoleOutput=consoleOutput, portRxFlowControlMode=portRxFlowControlMode, cmResetClearNotification=cmResetClearNotification, cardSecState=cardSecState, cardType=cardType, cardPicSerialNum=cardPicSerialNum, cardId=cardId, cardSubType=cardSubType, cmResetReason=cmResetReason, systemKey=systemKey, portLastChangeTime=portLastChangeTime, diagTestId=diagTestId, cardEntry=cardEntry, portNumChans=portNumChans, cardTestResult=cardTestResult, auditShelfId=auditShelfId, cardTempNoReportNotification=cardTempNoReportNotification, equipmentShelfGroup=equipmentShelfGroup, cardTemperature=cardTemperature, shelfSwVersion=shelfSwVersion, portAdminState=portAdminState, auditLogThrottle=auditLogThrottle, cardHwVersion=cardHwVersion, cardPrStateChange=cardPrStateChange, portSecStateChange=portSecStateChange, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardDplxStatus=cardDplxStatus, cardCarrierFwVersion=cardCarrierFwVersion, portSecState=portSecState, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeFileName=diskVolumeFileName, systemGeneral=systemGeneral, cmRegistrationNotification=cmRegistrationNotification, cardNumber=cardNumber, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestId=cardTestId, fpgaErrorEventEntry=fpgaErrorEventEntry, trapCounter=trapCounter, auditAutoScheduling=auditAutoScheduling, cardPrState=cardPrState, portMinDsPower=portMinDsPower, TestTransactionId=TestTransactionId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestType=cardTestType, equipmentDiag=equipmentDiag, TestScheduleCommand=TestScheduleCommand, diskVolumeDriveId=diskVolumeDriveId, portEntry=portEntry, portType=portType, portDescription=portDescription, auditTable=auditTable, diskVolumeId=diskVolumeId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResultNotification=cardTestResultNotification, diskVolumeName=diskVolumeName, cardCpuType=cardCpuType, TestResult=TestResult, cardAdminState=cardAdminState, portTrapInh=portTrapInh, cardDetected=cardDetected, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, shelfObjects=shelfObjects, cardFwVersion=cardFwVersion, portDocsIfIndex=portDocsIfIndex, cardFwUpdateStatus=cardFwUpdateStatus, cardTestTable=cardTestTable, cmUChannelID=cmUChannelID, equipmentPortGroup=equipmentPortGroup, portDplxStatus=portDplxStatus, equipmentTraps=equipmentTraps, diskVolumeDiskSize=diskVolumeDiskSize, equipmentError=equipmentError, cardCpuSpeed=cardCpuSpeed, portConnectorId=portConnectorId, auditTime=auditTime, errEvLogLevel=errEvLogLevel, cardPicHwVersion=cardPicHwVersion, auditStatus=auditStatus, systemClock=systemClock, errEvRowStatus=errEvRowStatus, auditResult=auditResult, cardSecStateChange=cardSecStateChange, portMacAddress=portMacAddress, cardDplxStatusChange=cardDplxStatusChange, cardFlashSize=cardFlashSize, auditFailedCount=auditFailedCount, cardName=cardName, cardTestSchedule=cardTestSchedule, shelfName=shelfName, auditCommand=auditCommand, cardTestIndex=cardTestIndex, fpgaErrorEventTable=fpgaErrorEventTable, cardLastResetReason=cardLastResetReason, verboseLevel=verboseLevel, cardTestName=cardTestName, cardTestCommand=cardTestCommand, portShelfId=portShelfId, cmPrimarySID=cmPrimarySID, diskVolumeShelfId=diskVolumeShelfId, cardPicDetected=cardPicDetected, cardAction=cardAction, portCardId=portCardId, portDplxStatusChange=portDplxStatusChange, portTable=portTable, cardLastBootVersion=cardLastBootVersion, auditCardId=auditCardId, remainInDiagMode=remainInDiagMode, cardTestEntry=cardTestEntry, cardSubDetected=cardSubDetected, equipmentAuditGroup=equipmentAuditGroup, diskDriveNumber=diskDriveNumber, auditDescription=auditDescription, TestSchedule=TestSchedule, auditId=auditId, eqDiagConfig=eqDiagConfig, auditTotalFailedCount=auditTotalFailedCount, equipmentState=equipmentState, portMode=portMode, cmIpAddress=cmIpAddress, cmDevice=cmDevice, cardTestTransId=cardTestTransId, cardSwVersion=cardSwVersion)
