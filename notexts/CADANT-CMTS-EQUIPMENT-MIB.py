...
PortMode, PicType, CardSubType, FirmwareSource, FlowControlMode, EqActionType, PortDetectedMode, PortId, PrimaryState, PortType, CardId, DiskVolumeUsageLevel, AdminState, SecondaryState, ShelfId, UnknownState, CardType, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PortMode", "PicType", "CardSubType", "FirmwareSource", "FlowControlMode", "EqActionType", "PortDetectedMode", "PortId", "PrimaryState", "PortType", "CardId", "DiskVolumeUsageLevel", "AdminState", "SecondaryState", "ShelfId", "UnknownState", "CardType", "DuplexStatus")
...
MacAddress, RowStatus, DisplayString, DateAndTime, TruthValue, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "DisplayString", "DateAndTime", "TruthValue", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentTbl=equipmentTbl, portDplxStatus=portDplxStatus, cmVendor=cmVendor, cardSubDetected=cardSubDetected, cardPrStateChange=cardPrStateChange, cardFlashSize=cardFlashSize, portPrState=portPrState, diskVolumeName=diskVolumeName, cardTestDescription=cardTestDescription, portSecState=portSecState, portDescription=portDescription, cardTestEntry=cardTestEntry, cmMacAddress=cmMacAddress, cardFpgaSource=cardFpgaSource, cardSecState=cardSecState, cardCpuSpeed=cardCpuSpeed, portLastChangeTime=portLastChangeTime, portAdminState=portAdminState, cardTestIndex=cardTestIndex, portCurrDsPower=portCurrDsPower, diskVolumeEntry=diskVolumeEntry, cardTestResultNotification=cardTestResultNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestTransId=cardTestTransId, cardTestTable=cardTestTable, cardSecStateChange=cardSecStateChange, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetUpTime=cmResetUpTime, equipmentError=equipmentError, cardDetected=cardDetected, portTrapInh=portTrapInh, TestResult=TestResult, cardTestScheduleCommand=cardTestScheduleCommand, auditEntry=auditEntry, equipmentCardGroup=equipmentCardGroup, equipmentDiag=equipmentDiag, auditTable=auditTable, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portEntry=portEntry, cardDplxStatus=cardDplxStatus, TestType=TestType, cardFwUpdateStatus=cardFwUpdateStatus, auditShelfId=auditShelfId, portConnectorId=portConnectorId, equipmentAudit=equipmentAudit, cardEntry=cardEntry, auditName=auditName, diskVolumeNumber=diskVolumeNumber, PYSNMP_MODULE_ID=cadEquipmentMib, portSecStateChange=portSecStateChange, portAction=portAction, cmIpAddress=cmIpAddress, cardSubType=cardSubType, auditStatus=auditStatus, cardTestResult=cardTestResult, auditCardId=auditCardId, cardType=cardType, cardTestId=cardTestId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portBgpId=portBgpId, cardTestTime=cardTestTime, diskVolumeCardId=diskVolumeCardId, cardPrState=cardPrState, cardTemperatureHighError=cardTemperatureHighError, cardSerialNum=cardSerialNum, cardTestName=cardTestName, cardCarrierSerialNum=cardCarrierSerialNum, verboseLevel=verboseLevel, auditCycleCount=auditCycleCount, portMinDsPower=portMinDsPower, cmResetNotification=cmResetNotification, systemClock=systemClock, portRxFlowControlMode=portRxFlowControlMode, cmUChannelID=cmUChannelID, portRxFlowControlDetected=portRxFlowControlDetected, cardAdminState=cardAdminState, cardFwVersion=cardFwVersion, shelfNumber=shelfNumber, cardLastResetReason=cardLastResetReason, diskVolumeTable=diskVolumeTable, TestTransactionId=TestTransactionId, cardTestResultDesc=cardTestResultDesc, cmResetInfo=cmResetInfo, shelfName=shelfName, auditResult=auditResult, portCardSubType=portCardSubType, cardTable=cardTable, portNumChans=portNumChans, trapSeverity=trapSeverity, cardAction=cardAction, cardShelfId=cardShelfId, equipmentShelfGroup=equipmentShelfGroup, errEvLogLevel=errEvLogLevel, cardTestSchedule=cardTestSchedule, portShelfId=portShelfId, cardSwVersion=cardSwVersion, cardNumPorts=cardNumPorts, cardTempOverHeatNotification=cardTempOverHeatNotification, cardName=cardName, portMacAddress=portMacAddress, TestId=TestId, cadEquipmentMibConformance=cadEquipmentMibConformance, auditFailedCount=auditFailedCount, portMacIfIndex=portMacIfIndex, portDplxStatusChange=portDplxStatusChange, cmRegistrationNotification=cmRegistrationNotification, auditTime=auditTime, cadEquipmentMib=cadEquipmentMib, cardPicSerialNum=cardPicSerialNum, diskVolumeId=diskVolumeId, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentCardTestGroup=equipmentCardTestGroup, cardLastBootSource=cardLastBootSource, portMode=portMode, auditPassedCount=auditPassedCount, TestCommand=TestCommand, portId=portId, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeGroup=diskVolumeGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmPrimarySID=cmPrimarySID, cardPicDetected=cardPicDetected, errEvRowStatus=errEvRowStatus, shelfSwVersion=shelfSwVersion, cardCpuType=cardCpuType, cardTestCommand=cardTestCommand, equipmentStateGroup=equipmentStateGroup, auditTotalFailedCount=auditTotalFailedCount, auditAutoScheduling=auditAutoScheduling, errEvLoggingEnabled=errEvLoggingEnabled, auditLogThrottle=auditLogThrottle, shelfObjects=shelfObjects, cardSpareGroupId=cardSpareGroupId, cardCPLDVersion=cardCPLDVersion, auditCommand=auditCommand, portCardId=portCardId, cardDetectedChange=cardDetectedChange, portMaxDsPower=portMaxDsPower, downstreamPowerLoss=downstreamPowerLoss, diskVolumeTrapInh=diskVolumeTrapInh, cardDplxStatusChange=cardDplxStatusChange, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeSize=diskVolumeSize, auditDescription=auditDescription, auditLogOutput=auditLogOutput, diagTestId=diagTestId, cardTemperature=cardTemperature, equipmentDiagGroup=equipmentDiagGroup, cardRamSize=cardRamSize, equipmentAuditGroup=equipmentAuditGroup, diskVolumeDriveId=diskVolumeDriveId, cardTempNoReportNotification=cardTempNoReportNotification, systemKey=systemKey, equipmentPortGroup=equipmentPortGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, systemGeneralGroup=systemGeneralGroup, TestSchedule=TestSchedule, systemGeneral=systemGeneral, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTemperatureHighWarn=cardTemperatureHighWarn, portTable=portTable, auditTotalPassedCount=auditTotalPassedCount, cardLastBootVersion=cardLastBootVersion, cardShelfTestIndex=cardShelfTestIndex, cmDevice=cmDevice, diskVolumeUsageNotification=diskVolumeUsageNotification, portTxFlowControlMode=portTxFlowControlMode, diskDriveNumber=diskDriveNumber, cardLastChangeTime=cardLastChangeTime, portDetectedMode=portDetectedMode, cardBootVersion=cardBootVersion, cardId=cardId, diskVolumeDiskSize=diskVolumeDiskSize, portDocsIfIndex=portDocsIfIndex, portTxFlowControlDetected=portTxFlowControlDetected, cardCarrierHwVersion=cardCarrierHwVersion, cardPicHwVersion=cardPicHwVersion, equipmentTraps=equipmentTraps, cardTrapInh=cardTrapInh, portPrStateChange=portPrStateChange, consoleOutput=consoleOutput, cmResetStatus=cmResetStatus, cardNumber=cardNumber, cardTestType=cardTestType, cardBusSpeed=cardBusSpeed, cardSpareGroupMode=cardSpareGroupMode, remainInDiagMode=remainInDiagMode, portType=portType, errEventId=errEventId, cardCarrierFwVersion=cardCarrierFwVersion, equipmentState=equipmentState, trapCounter=trapCounter, cmResetReason=cmResetReason, cardUpTime=cardUpTime, cmResetClearNotification=cmResetClearNotification, portNumber=portNumber, cardHwVersion=cardHwVersion, TestScheduleCommand=TestScheduleCommand, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeShelfId=diskVolumeShelfId, eqDiagConfig=eqDiagConfig, diskVolumeFileName=diskVolumeFileName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditId=auditId)
