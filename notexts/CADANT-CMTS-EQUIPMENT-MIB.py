...
DuplexStatus, FirmwareSource, PicType, DiskVolumeUsageLevel, AdminState, CardSubType, UnknownState, FlowControlMode, PortMode, CardType, PortDetectedMode, ShelfId, PortType, CardId, PrimaryState, SecondaryState, PortId, EqActionType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "FirmwareSource", "PicType", "DiskVolumeUsageLevel", "AdminState", "CardSubType", "UnknownState", "FlowControlMode", "PortMode", "CardType", "PortDetectedMode", "ShelfId", "PortType", "CardId", "PrimaryState", "SecondaryState", "PortId", "EqActionType")
...
TimeStamp, RowStatus, DateAndTime, MacAddress, TextualConvention, DisplayString, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "RowStatus", "DateAndTime", "MacAddress", "TextualConvention", "DisplayString", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portCurrDsPower=portCurrDsPower, cardSecState=cardSecState, cardDetected=cardDetected, cardShelfTestIndex=cardShelfTestIndex, cardTestType=cardTestType, cardSpareGroupMode=cardSpareGroupMode, portConnectorId=portConnectorId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMib=cadEquipmentMib, auditStatus=auditStatus, diskVolumeFileName=diskVolumeFileName, auditAutoScheduling=auditAutoScheduling, equipmentShelfGroup=equipmentShelfGroup, cardSubDetected=cardSubDetected, portDocsIfIndex=portDocsIfIndex, cardFpgaSource=cardFpgaSource, portTxFlowControlMode=portTxFlowControlMode, cardTestScheduleCommand=cardTestScheduleCommand, cmResetNotification=cmResetNotification, portTable=portTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardLastResetReason=cardLastResetReason, diskVolumeNumber=diskVolumeNumber, cardPicDetected=cardPicDetected, portSecState=portSecState, portMacIfIndex=portMacIfIndex, equipmentError=equipmentError, equipmentTbl=equipmentTbl, auditResult=auditResult, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCPLDVersion=cardCPLDVersion, cardEntry=cardEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, cmRegistrationNotification=cmRegistrationNotification, auditShelfId=auditShelfId, portDetectedMode=portDetectedMode, cardLastBootSource=cardLastBootSource, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestName=cardTestName, portDplxStatus=portDplxStatus, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeTrapInh=diskVolumeTrapInh, portPrStateChange=portPrStateChange, cardTestTime=cardTestTime, cmDevice=cmDevice, portBgpId=portBgpId, errEventId=errEventId, cardTestIndex=cardTestIndex, portCardId=portCardId, diskVolumeEntry=diskVolumeEntry, diagTestId=diagTestId, eqDiagConfig=eqDiagConfig, cardTestCommand=cardTestCommand, auditId=auditId, cardFwUpdateStatus=cardFwUpdateStatus, cardId=cardId, diskDriveNumber=diskDriveNumber, errEvLogLevel=errEvLogLevel, portMaxDsPower=portMaxDsPower, equipmentCardGroup=equipmentCardGroup, equipmentCardTestGroup=equipmentCardTestGroup, errEvLoggingEnabled=errEvLoggingEnabled, cardCarrierSerialNum=cardCarrierSerialNum, portLastChangeTime=portLastChangeTime, auditDescription=auditDescription, cadEquipmentMibConformance=cadEquipmentMibConformance, portNumber=portNumber, auditTable=auditTable, portAction=portAction, equipmentTraps=equipmentTraps, auditEntry=auditEntry, cardSubType=cardSubType, systemClock=systemClock, cardSerialNum=cardSerialNum, shelfNumber=shelfNumber, cardPicSerialNum=cardPicSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, auditCommand=auditCommand, equipmentAudit=equipmentAudit, portEntry=portEntry, cardDetectedChange=cardDetectedChange, equipmentState=equipmentState, portTxFlowControlDetected=portTxFlowControlDetected, portMacAddress=portMacAddress, verboseLevel=verboseLevel, cardTestTransId=cardTestTransId, equipmentPortGroup=equipmentPortGroup, systemKey=systemKey, cardType=cardType, TestResult=TestResult, portAdminState=portAdminState, cardSwVersion=cardSwVersion, portTrapInh=portTrapInh, cardShelfId=cardShelfId, portRxFlowControlMode=portRxFlowControlMode, auditLogOutput=auditLogOutput, auditTime=auditTime, cmPrimarySID=cmPrimarySID, auditLogThrottle=auditLogThrottle, cmResetUpTime=cmResetUpTime, cardTemperatureHighError=cardTemperatureHighError, diskVolumeName=diskVolumeName, cardTestResultDesc=cardTestResultDesc, auditTotalPassedCount=auditTotalPassedCount, cardDplxStatus=cardDplxStatus, equipmentStateGroup=equipmentStateGroup, TestId=TestId, auditFailedCount=auditFailedCount, cardName=cardName, trapCounter=trapCounter, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestTable=cardTestTable, diskVolumeShelfId=diskVolumeShelfId, cmIpAddress=cmIpAddress, portType=portType, cardNumPorts=cardNumPorts, cardDplxStatusChange=cardDplxStatusChange, TestScheduleCommand=TestScheduleCommand, cardTempNoReportNotification=cardTempNoReportNotification, auditPassedCount=auditPassedCount, cardBootVersion=cardBootVersion, portPrState=portPrState, cardCarrierFwVersion=cardCarrierFwVersion, cmResetReason=cmResetReason, auditCardId=auditCardId, auditName=auditName, cmMacAddress=cmMacAddress, cardCarrierHwVersion=cardCarrierHwVersion, portMode=portMode, TestSchedule=TestSchedule, portDescription=portDescription, auditTotalFailedCount=auditTotalFailedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, cardFwVersion=cardFwVersion, shelfName=shelfName, TestCommand=TestCommand, equipmentDiagGroup=equipmentDiagGroup, TestTransactionId=TestTransactionId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portSecStateChange=portSecStateChange, portId=portId, shelfSwVersion=shelfSwVersion, diskVolumeSize=diskVolumeSize, cardPrStateChange=cardPrStateChange, systemGeneralGroup=systemGeneralGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTemperatureHighWarn=cardTemperatureHighWarn, portShelfId=portShelfId, equipmentAuditGroup=equipmentAuditGroup, cardAction=cardAction, cardNumber=cardNumber, PYSNMP_MODULE_ID=cadEquipmentMib, cardTrapInh=cardTrapInh, diskVolumeDriveId=diskVolumeDriveId, cmVendor=cmVendor, diskVolumeTable=diskVolumeTable, cardTestEntry=cardTestEntry, cardCpuType=cardCpuType, cardCpuSpeed=cardCpuSpeed, diskVolumeGroup=diskVolumeGroup, cardSecStateChange=cardSecStateChange, consoleOutput=consoleOutput, cardTemperature=cardTemperature, downstreamPowerLoss=downstreamPowerLoss, diskVolumeCardId=diskVolumeCardId, cardFlashSize=cardFlashSize, diskVolumeUsageNotification=diskVolumeUsageNotification, cardLastBootVersion=cardLastBootVersion, equipmentDiag=equipmentDiag, cardTestResultNotification=cardTestResultNotification, cardTable=cardTable, portDplxStatusChange=portDplxStatusChange, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmUChannelID=cmUChannelID, cardTestSchedule=cardTestSchedule, cardBusSpeed=cardBusSpeed, cardUpTime=cardUpTime, remainInDiagMode=remainInDiagMode, cardHwVersion=cardHwVersion, cardTestResult=cardTestResult, cmResetClearNotification=cmResetClearNotification, TestType=TestType, cardAdminState=cardAdminState, trapSeverity=trapSeverity, cardTestDescription=cardTestDescription, systemGeneral=systemGeneral, diskVolumeId=diskVolumeId, auditCycleCount=auditCycleCount, cardPrState=cardPrState, errEvRowStatus=errEvRowStatus, cardRamSize=cardRamSize, cardTestId=cardTestId, cmResetInfo=cmResetInfo, cardLastChangeTime=cardLastChangeTime, portCardSubType=portCardSubType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, shelfObjects=shelfObjects, portMinDsPower=portMinDsPower, cmResetStatus=cmResetStatus, fpgaErrorEventTable=fpgaErrorEventTable, cardPicHwVersion=cardPicHwVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portNumChans=portNumChans)
