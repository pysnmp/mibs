...
PrimaryState, PortDetectedMode, SecondaryState, PortType, DuplexStatus, FlowControlMode, CardType, FirmwareSource, AdminState, PicType, CardSubType, EqActionType, PortId, CardId, DiskVolumeUsageLevel, ShelfId, UnknownState, PortMode = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "PortDetectedMode", "SecondaryState", "PortType", "DuplexStatus", "FlowControlMode", "CardType", "FirmwareSource", "AdminState", "PicType", "CardSubType", "EqActionType", "PortId", "CardId", "DiskVolumeUsageLevel", "ShelfId", "UnknownState", "PortMode")
...
TextualConvention, TruthValue, MacAddress, DisplayString, RowStatus, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "MacAddress", "DisplayString", "RowStatus", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestTable=cardTestTable, remainInDiagMode=remainInDiagMode, cardTestSchedule=cardTestSchedule, portId=portId, cardAction=cardAction, auditCommand=auditCommand, consoleOutput=consoleOutput, auditTime=auditTime, cardTestDescription=cardTestDescription, cardSubType=cardSubType, equipmentStateGroup=equipmentStateGroup, portMacIfIndex=portMacIfIndex, PYSNMP_MODULE_ID=cadEquipmentMib, auditShelfId=auditShelfId, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeShelfId=diskVolumeShelfId, cardNumPorts=cardNumPorts, errEvLogLevel=errEvLogLevel, cardTemperatureHighError=cardTemperatureHighError, cmIpAddress=cmIpAddress, trapSeverity=trapSeverity, auditPassedCount=auditPassedCount, portMinDsPower=portMinDsPower, cardCpuType=cardCpuType, TestTransactionId=TestTransactionId, auditEntry=auditEntry, shelfObjects=shelfObjects, cardFwUpdateStatus=cardFwUpdateStatus, cardLastResetReason=cardLastResetReason, portTxFlowControlDetected=portTxFlowControlDetected, cardDetectedChange=cardDetectedChange, cardTestResultNotification=cardTestResultNotification, systemGeneral=systemGeneral, diskVolumeNumber=diskVolumeNumber, equipmentTbl=equipmentTbl, diskVolumeFileName=diskVolumeFileName, cardRamSize=cardRamSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portTxFlowControlMode=portTxFlowControlMode, portSecState=portSecState, equipmentDiag=equipmentDiag, eqDiagConfig=eqDiagConfig, auditLogOutput=auditLogOutput, cmUChannelID=cmUChannelID, shelfNumber=shelfNumber, cardPicDetected=cardPicDetected, portPrState=portPrState, diskVolumeId=diskVolumeId, portEntry=portEntry, cmRegistrationNotification=cmRegistrationNotification, verboseLevel=verboseLevel, cardSpareGroupId=cardSpareGroupId, auditFailedCount=auditFailedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestId=cardTestId, errEvRowStatus=errEvRowStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetInfo=cmResetInfo, TestSchedule=TestSchedule, cardBootVersion=cardBootVersion, cardTestEntry=cardTestEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, cardPicHwVersion=cardPicHwVersion, cardUpTime=cardUpTime, portCardSubType=portCardSubType, cardTestIndex=cardTestIndex, cmVendor=cmVendor, equipmentCardGroup=equipmentCardGroup, portLastChangeTime=portLastChangeTime, equipmentShelfGroup=equipmentShelfGroup, cmResetReason=cmResetReason, cardCPLDVersion=cardCPLDVersion, equipmentError=equipmentError, shelfSwVersion=shelfSwVersion, TestId=TestId, equipmentAuditGroup=equipmentAuditGroup, portType=portType, portNumChans=portNumChans, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierFwVersion=cardCarrierFwVersion, cardDplxStatusChange=cardDplxStatusChange, cardLastBootSource=cardLastBootSource, equipmentAudit=equipmentAudit, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTrapInh=cardTrapInh, cmResetNotification=cmResetNotification, cardTestResult=cardTestResult, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestType=TestType, portConnectorId=portConnectorId, portSecStateChange=portSecStateChange, cardPicSerialNum=cardPicSerialNum, portTable=portTable, auditCardId=auditCardId, cmResetClearNotification=cmResetClearNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardSerialNum=cardSerialNum, diskVolumeName=diskVolumeName, cardSwVersion=cardSwVersion, portDocsIfIndex=portDocsIfIndex, portCurrDsPower=portCurrDsPower, portAdminState=portAdminState, portDplxStatusChange=portDplxStatusChange, TestScheduleCommand=TestScheduleCommand, cardCarrierHwVersion=cardCarrierHwVersion, portDescription=portDescription, shelfName=shelfName, cardTestScheduleCommand=cardTestScheduleCommand, cardTestName=cardTestName, portTrapInh=portTrapInh, cardDetected=cardDetected, diskVolumeSize=diskVolumeSize, portDetectedMode=portDetectedMode, cadEquipmentMib=cadEquipmentMib, portPrStateChange=portPrStateChange, diskDriveNumber=diskDriveNumber, auditId=auditId, equipmentState=equipmentState, cardBusSpeed=cardBusSpeed, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetUpTime=cmResetUpTime, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardShelfId=cardShelfId, cardFwVersion=cardFwVersion, auditStatus=auditStatus, equipmentTraps=equipmentTraps, trapCounter=trapCounter, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, systemKey=systemKey, cardTemperature=cardTemperature, auditTotalFailedCount=auditTotalFailedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, cmMacAddress=cmMacAddress, cardTestCommand=cardTestCommand, cardTestTime=cardTestTime, cmResetStatus=cmResetStatus, cardFlashSize=cardFlashSize, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeGroup=diskVolumeGroup, portShelfId=portShelfId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardId=cardId, cardTestTransId=cardTestTransId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTable=cardTable, diskVolumeDriveId=diskVolumeDriveId, auditLogThrottle=auditLogThrottle, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardLastBootVersion=cardLastBootVersion, portAction=portAction, auditAutoScheduling=auditAutoScheduling, systemClock=systemClock, diskVolumeDiskSize=diskVolumeDiskSize, cardFpgaSource=cardFpgaSource, auditTable=auditTable, cardTempNoReportNotification=cardTempNoReportNotification, auditName=auditName, TestCommand=TestCommand, cardHwVersion=cardHwVersion, cardSecStateChange=cardSecStateChange, cardLastChangeTime=cardLastChangeTime, equipmentCardTestGroup=equipmentCardTestGroup, portMaxDsPower=portMaxDsPower, auditCycleCount=auditCycleCount, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardType=cardType, portNumber=portNumber, diskVolumeCardId=diskVolumeCardId, auditResult=auditResult, cardDplxStatus=cardDplxStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSubDetected=cardSubDetected, equipmentPortGroup=equipmentPortGroup, equipmentDiagGroup=equipmentDiagGroup, cardCpuSpeed=cardCpuSpeed, portBgpId=portBgpId, cardName=cardName, portMode=portMode, diskVolumeEntry=diskVolumeEntry, cardSecState=cardSecState, cmDevice=cmDevice, errEventId=errEventId, cardTestResultDesc=cardTestResultDesc, portCardId=portCardId, portDplxStatus=portDplxStatus, portMacAddress=portMacAddress, cardPrStateChange=cardPrStateChange, cardPrState=cardPrState, errEvLoggingEnabled=errEvLoggingEnabled, diagTestId=diagTestId, cardTestType=cardTestType, auditDescription=auditDescription, TestResult=TestResult, cmPrimarySID=cmPrimarySID, cardSpareGroupMode=cardSpareGroupMode, cardNumber=cardNumber, systemGeneralGroup=systemGeneralGroup, diskVolumeTable=diskVolumeTable, auditTotalPassedCount=auditTotalPassedCount, cardAdminState=cardAdminState, downstreamPowerLoss=downstreamPowerLoss, cardEntry=cardEntry)
