...
CardType, DuplexStatus, PortType, ShelfId, CardSubType, CardId, PortMode, PrimaryState, PortDetectedMode, SecondaryState, PortId, AdminState, FlowControlMode, FirmwareSource, UnknownState, PicType, EqActionType, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "CardType", "DuplexStatus", "PortType", "ShelfId", "CardSubType", "CardId", "PortMode", "PrimaryState", "PortDetectedMode", "SecondaryState", "PortId", "AdminState", "FlowControlMode", "FirmwareSource", "UnknownState", "PicType", "EqActionType", "DiskVolumeUsageLevel")
...
DateAndTime, TimeStamp, DisplayString, RowStatus, TruthValue, TextualConvention, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TimeStamp", "DisplayString", "RowStatus", "TruthValue", "TextualConvention", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portAdminState=portAdminState, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestIndex=cardTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardDplxStatusChange=cardDplxStatusChange, cardSubDetected=cardSubDetected, portNumber=portNumber, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeCardId=diskVolumeCardId, cardFlashSize=cardFlashSize, fpgaErrorEventEntry=fpgaErrorEventEntry, auditLogThrottle=auditLogThrottle, verboseLevel=verboseLevel, cardRamSize=cardRamSize, systemGeneralGroup=systemGeneralGroup, portMacAddress=portMacAddress, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeGroup=diskVolumeGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestDescription=cardTestDescription, portAction=portAction, cardSerialNum=cardSerialNum, cardNumber=cardNumber, cardSecState=cardSecState, portPrState=portPrState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogOutput=auditLogOutput, diskVolumeTable=diskVolumeTable, portConnectorId=portConnectorId, portRxFlowControlMode=portRxFlowControlMode, auditDescription=auditDescription, cardId=cardId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCpuType=cardCpuType, cardShelfTestIndex=cardShelfTestIndex, portNumChans=portNumChans, cardTestTime=cardTestTime, auditTotalPassedCount=auditTotalPassedCount, cmResetReason=cmResetReason, equipmentError=equipmentError, cardTempOverHeatNotification=cardTempOverHeatNotification, portDetectedMode=portDetectedMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portCurrDsPower=portCurrDsPower, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTransId=cardTestTransId, cardLastChangeTime=cardLastChangeTime, cardTestName=cardTestName, auditCardId=auditCardId, cmResetUpTime=cmResetUpTime, portSecStateChange=portSecStateChange, portSecState=portSecState, equipmentAuditGroup=equipmentAuditGroup, errEvLogLevel=errEvLogLevel, diagTestId=diagTestId, cardLastResetReason=cardLastResetReason, shelfNumber=shelfNumber, cardFpgaSource=cardFpgaSource, portDplxStatus=portDplxStatus, cardDetectedChange=cardDetectedChange, trapCounter=trapCounter, cmUChannelID=cmUChannelID, cardTrapInh=cardTrapInh, cardCarrierFwVersion=cardCarrierFwVersion, cardAction=cardAction, cmResetClearNotification=cmResetClearNotification, diskVolumeDriveId=diskVolumeDriveId, equipmentStateGroup=equipmentStateGroup, diskDriveNumber=diskDriveNumber, cardCarrierSerialNum=cardCarrierSerialNum, cmDevice=cmDevice, equipmentCardTestGroup=equipmentCardTestGroup, cardTestResultDesc=cardTestResultDesc, equipmentTraps=equipmentTraps, diskVolumeFileName=diskVolumeFileName, auditStatus=auditStatus, downstreamPowerLoss=downstreamPowerLoss, portTrapInh=portTrapInh, cardLastBootSource=cardLastBootSource, auditTime=auditTime, TestSchedule=TestSchedule, systemClock=systemClock, cardUpTime=cardUpTime, equipmentCardGroup=equipmentCardGroup, cardTestType=cardTestType, portTxFlowControlDetected=portTxFlowControlDetected, portEntry=portEntry, portShelfId=portShelfId, auditShelfId=auditShelfId, auditCommand=auditCommand, cadEquipmentMibConformance=cadEquipmentMibConformance, cardFwUpdateStatus=cardFwUpdateStatus, equipmentState=equipmentState, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardHwVersion=cardHwVersion, cardPicHwVersion=cardPicHwVersion, cardSpareGroupId=cardSpareGroupId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentDiag=equipmentDiag, cardTemperatureHighError=cardTemperatureHighError, cardCarrierHwVersion=cardCarrierHwVersion, TestScheduleCommand=TestScheduleCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestSchedule=cardTestSchedule, cardCPLDVersion=cardCPLDVersion, cmResetStatus=cmResetStatus, portDescription=portDescription, cardEntry=cardEntry, equipmentPortGroup=equipmentPortGroup, portLastChangeTime=portLastChangeTime, diskVolumeShelfId=diskVolumeShelfId, cardTestTable=cardTestTable, auditTotalFailedCount=auditTotalFailedCount, errEvLoggingEnabled=errEvLoggingEnabled, TestType=TestType, cmPrimarySID=cmPrimarySID, diskVolumeNumber=diskVolumeNumber, auditName=auditName, equipmentDiagGroup=equipmentDiagGroup, cardPicSerialNum=cardPicSerialNum, cmIpAddress=cmIpAddress, cardPrStateChange=cardPrStateChange, cadEquipmentMib=cadEquipmentMib, shelfSwVersion=shelfSwVersion, errEvRowStatus=errEvRowStatus, auditId=auditId, cmResetNotification=cmResetNotification, diskVolumeId=diskVolumeId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardShelfId=cardShelfId, cardDetected=cardDetected, auditEntry=auditEntry, portCardSubType=portCardSubType, diskVolumeEntry=diskVolumeEntry, cardBusSpeed=cardBusSpeed, consoleOutput=consoleOutput, fpgaErrorEventTable=fpgaErrorEventTable, cardSpareGroupMode=cardSpareGroupMode, cardTestEntry=cardTestEntry, systemGeneral=systemGeneral, TestCommand=TestCommand, cardPicDetected=cardPicDetected, remainInDiagMode=remainInDiagMode, errEventId=errEventId, cardPrState=cardPrState, cardTestId=cardTestId, portMinDsPower=portMinDsPower, equipmentTbl=equipmentTbl, portBgpId=portBgpId, auditResult=auditResult, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeSize=diskVolumeSize, cmMacAddress=cmMacAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDplxStatus=cardDplxStatus, cardTestResult=cardTestResult, cardBootVersion=cardBootVersion, diskVolumeName=diskVolumeName, cmVendor=cmVendor, shelfObjects=shelfObjects, portDplxStatusChange=portDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, portTable=portTable, portPrStateChange=portPrStateChange, portMaxDsPower=portMaxDsPower, auditPassedCount=auditPassedCount, cardNumPorts=cardNumPorts, trapSeverity=trapSeverity, cardName=cardName, cardTable=cardTable, portId=portId, eqDiagConfig=eqDiagConfig, portTxFlowControlMode=portTxFlowControlMode, cardAdminState=cardAdminState, cardSecStateChange=cardSecStateChange, auditFailedCount=auditFailedCount, TestResult=TestResult, cardType=cardType, TestTransactionId=TestTransactionId, cardFwVersion=cardFwVersion, systemKey=systemKey, portMacIfIndex=portMacIfIndex, cmResetInfo=cmResetInfo, equipmentShelfGroup=equipmentShelfGroup, cardSwVersion=cardSwVersion, auditCycleCount=auditCycleCount, TestId=TestId, diskVolumeDiskSize=diskVolumeDiskSize, cardTestCommand=cardTestCommand, equipmentAudit=equipmentAudit, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTemperature=cardTemperature, cardTestResultNotification=cardTestResultNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCpuSpeed=cardCpuSpeed, cardTestScheduleCommand=cardTestScheduleCommand, portMode=portMode, auditTable=auditTable, cmRegistrationNotification=cmRegistrationNotification, portCardId=portCardId, portDocsIfIndex=portDocsIfIndex, cardSubType=cardSubType, portType=portType, shelfName=shelfName)
