...
UnknownState, SecondaryState, DiskVolumeUsageLevel, CardSubType, ShelfId, DuplexStatus, PicType, CardId, AdminState, PrimaryState, FirmwareSource, PortId, PortDetectedMode, FlowControlMode, CardType, EqActionType, PortType, PortMode = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "SecondaryState", "DiskVolumeUsageLevel", "CardSubType", "ShelfId", "DuplexStatus", "PicType", "CardId", "AdminState", "PrimaryState", "FirmwareSource", "PortId", "PortDetectedMode", "FlowControlMode", "CardType", "EqActionType", "PortType", "PortMode")
...
MacAddress, TimeStamp, TruthValue, RowStatus, TextualConvention, DisplayString, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TimeStamp", "TruthValue", "RowStatus", "TextualConvention", "DisplayString", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPrState=cardPrState, auditEntry=auditEntry, auditTotalPassedCount=auditTotalPassedCount, cardSpareGroupMode=cardSpareGroupMode, errEvRowStatus=errEvRowStatus, TestType=TestType, fpgaErrorEventGroup=fpgaErrorEventGroup, portMacIfIndex=portMacIfIndex, auditResult=auditResult, TestId=TestId, cardFpgaSource=cardFpgaSource, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastBootVersion=cardLastBootVersion, diskVolumeSize=diskVolumeSize, auditTable=auditTable, equipmentTraps=equipmentTraps, cardNumber=cardNumber, portShelfId=portShelfId, shelfNumber=shelfNumber, shelfName=shelfName, TestScheduleCommand=TestScheduleCommand, auditFailedCount=auditFailedCount, cardTable=cardTable, auditLogThrottle=auditLogThrottle, cardTestName=cardTestName, cmResetUpTime=cmResetUpTime, portNumber=portNumber, equipmentCardTestGroup=equipmentCardTestGroup, auditDescription=auditDescription, cardTestScheduleCommand=cardTestScheduleCommand, cardTestDescription=cardTestDescription, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageNotification=diskVolumeUsageNotification, portCardSubType=portCardSubType, cardTestEntry=cardTestEntry, equipmentState=equipmentState, portTable=portTable, cardSubDetected=cardSubDetected, auditAutoScheduling=auditAutoScheduling, trapSeverity=trapSeverity, cardType=cardType, auditCycleCount=auditCycleCount, cmResetNotification=cmResetNotification, auditTime=auditTime, cardSubType=cardSubType, systemGeneralGroup=systemGeneralGroup, diskDriveNumber=diskDriveNumber, portMode=portMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDetectedMode=portDetectedMode, cardBusSpeed=cardBusSpeed, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetReason=cmResetReason, cardRamSize=cardRamSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardDplxStatus=cardDplxStatus, equipmentTbl=equipmentTbl, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardFwVersion=cardFwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portAdminState=portAdminState, portDescription=portDescription, portNumChans=portNumChans, cardTestId=cardTestId, systemGeneral=systemGeneral, cardSwVersion=cardSwVersion, cardNumPorts=cardNumPorts, portTxFlowControlDetected=portTxFlowControlDetected, cardTestType=cardTestType, equipmentAudit=equipmentAudit, portConnectorId=portConnectorId, diskVolumeGroup=diskVolumeGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardBootVersion=cardBootVersion, cardPicSerialNum=cardPicSerialNum, cardId=cardId, cardSerialNum=cardSerialNum, cardPicDetected=cardPicDetected, diskVolumeEntry=diskVolumeEntry, eqDiagConfig=eqDiagConfig, cardTemperatureHighWarn=cardTemperatureHighWarn, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeNumber=diskVolumeNumber, cmResetInfo=cmResetInfo, diskVolumeTrapInh=diskVolumeTrapInh, equipmentDiagGroup=equipmentDiagGroup, cardPicHwVersion=cardPicHwVersion, portSecStateChange=portSecStateChange, downstreamPowerLoss=downstreamPowerLoss, cardTemperatureHighError=cardTemperatureHighError, cardTestResultNotification=cardTestResultNotification, cardTestResult=cardTestResult, auditCardId=auditCardId, portMinDsPower=portMinDsPower, diskVolumeDriveId=diskVolumeDriveId, auditId=auditId, cardSecState=cardSecState, TestTransactionId=TestTransactionId, TestSchedule=TestSchedule, cardShelfTestIndex=cardShelfTestIndex, portRxFlowControlMode=portRxFlowControlMode, cardFwUpdateStatus=cardFwUpdateStatus, portBgpId=portBgpId, portPrState=portPrState, equipmentPortGroup=equipmentPortGroup, portEntry=portEntry, auditPassedCount=auditPassedCount, diagTestId=diagTestId, systemClock=systemClock, portRxFlowControlDetected=portRxFlowControlDetected, auditName=auditName, cardName=cardName, fpgaErrorEventTable=fpgaErrorEventTable, portTrapInh=portTrapInh, portAction=portAction, cmResetClearNotification=cmResetClearNotification, cardTestCommand=cardTestCommand, diskVolumeId=diskVolumeId, cardPrStateChange=cardPrStateChange, cardEntry=cardEntry, cardAdminState=cardAdminState, errEvLogLevel=errEvLogLevel, portSecState=portSecState, cmMacAddress=cmMacAddress, cmRegistrationNotification=cmRegistrationNotification, systemKey=systemKey, shelfSwVersion=shelfSwVersion, cmVendor=cmVendor, diskVolumeCardId=diskVolumeCardId, shelfObjects=shelfObjects, portPrStateChange=portPrStateChange, auditCommand=auditCommand, equipmentStateGroup=equipmentStateGroup, portCardId=portCardId, cardTestTime=cardTestTime, diskVolumeFileName=diskVolumeFileName, cardTestIndex=cardTestIndex, portCurrDsPower=portCurrDsPower, auditShelfId=auditShelfId, verboseLevel=verboseLevel, errEvLoggingEnabled=errEvLoggingEnabled, cardCPLDVersion=cardCPLDVersion, trapCounter=trapCounter, cardTestTable=cardTestTable, TestCommand=TestCommand, cardCpuSpeed=cardCpuSpeed, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, cardCarrierHwVersion=cardCarrierHwVersion, portDplxStatus=portDplxStatus, equipmentDiag=equipmentDiag, cardSecStateChange=cardSecStateChange, cardTrapInh=cardTrapInh, cardTestSchedule=cardTestSchedule, cardFlashSize=cardFlashSize, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentAuditGroup=equipmentAuditGroup, cardAction=cardAction, cadEquipmentMib=cadEquipmentMib, diskVolumeShelfId=diskVolumeShelfId, cardDplxStatusChange=cardDplxStatusChange, cardTestTransId=cardTestTransId, equipmentShelfGroup=equipmentShelfGroup, remainInDiagMode=remainInDiagMode, cardLastChangeTime=cardLastChangeTime, cardCpuType=cardCpuType, cmPrimarySID=cmPrimarySID, cardDetectedChange=cardDetectedChange, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portDocsIfIndex=portDocsIfIndex, equipmentCardGroup=equipmentCardGroup, cardHwVersion=cardHwVersion, cmResetStatus=cmResetStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, cmUChannelID=cmUChannelID, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTable=diskVolumeTable, cardTemperature=cardTemperature, fpgaErrorEventEntry=fpgaErrorEventEntry, auditStatus=auditStatus, TestResult=TestResult, cardShelfId=cardShelfId, cardDetected=cardDetected, portMacAddress=portMacAddress, portType=portType, cardTestResultDesc=cardTestResultDesc, auditLogOutput=auditLogOutput, cardTempNoReportNotification=cardTempNoReportNotification, cardLastBootSource=cardLastBootSource, cardLastResetReason=cardLastResetReason, portMaxDsPower=portMaxDsPower, auditTotalFailedCount=auditTotalFailedCount, equipmentError=equipmentError, portDplxStatusChange=portDplxStatusChange, diskVolumeName=diskVolumeName, portId=portId, errEventId=errEventId, cardCarrierFwVersion=cardCarrierFwVersion, cardUpTime=cardUpTime, cmIpAddress=cmIpAddress, portLastChangeTime=portLastChangeTime, cmDevice=cmDevice, consoleOutput=consoleOutput)
