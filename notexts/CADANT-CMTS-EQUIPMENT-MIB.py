...
CardSubType, FirmwareSource, PicType, DuplexStatus, PortType, PrimaryState, PortDetectedMode, ShelfId, DiskVolumeUsageLevel, CardType, PortId, PortMode, CardId, EqActionType, UnknownState, AdminState, SecondaryState, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "FirmwareSource", "PicType", "DuplexStatus", "PortType", "PrimaryState", "PortDetectedMode", "ShelfId", "DiskVolumeUsageLevel", "CardType", "PortId", "PortMode", "CardId", "EqActionType", "UnknownState", "AdminState", "SecondaryState", "FlowControlMode")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfSwVersion=shelfSwVersion, cardTestScheduleCommand=cardTestScheduleCommand, errEvLogLevel=errEvLogLevel, equipmentStateGroup=equipmentStateGroup, cadEquipmentMib=cadEquipmentMib, cardFlashSize=cardFlashSize, cardPrState=cardPrState, portCurrDsPower=portCurrDsPower, cmUChannelID=cmUChannelID, auditTime=auditTime, cardCarrierSerialNum=cardCarrierSerialNum, portMacAddress=portMacAddress, diskVolumeFileName=diskVolumeFileName, auditShelfId=auditShelfId, diskVolumeDiskSize=diskVolumeDiskSize, equipmentDiag=equipmentDiag, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestTable=cardTestTable, auditEntry=auditEntry, cardSwVersion=cardSwVersion, cardFwVersion=cardFwVersion, portSecState=portSecState, TestType=TestType, portDescription=portDescription, portShelfId=portShelfId, errEventId=errEventId, cardAdminState=cardAdminState, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardId=cardId, portTable=portTable, portMinDsPower=portMinDsPower, cardSubType=cardSubType, cardHwVersion=cardHwVersion, portAction=portAction, portPrState=portPrState, cardDetected=cardDetected, portBgpId=portBgpId, auditCardId=auditCardId, diskDriveNumber=diskDriveNumber, cardLastBootSource=cardLastBootSource, diskVolumeDriveId=diskVolumeDriveId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeNumber=diskVolumeNumber, trapSeverity=trapSeverity, portType=portType, cardPicHwVersion=cardPicHwVersion, portNumChans=portNumChans, diskVolumeSize=diskVolumeSize, cardSubDetected=cardSubDetected, cardTestName=cardTestName, cmDevice=cmDevice, cardTestTime=cardTestTime, equipmentCardGroup=equipmentCardGroup, portMode=portMode, cardTemperatureHighError=cardTemperatureHighError, cardFpgaSource=cardFpgaSource, eqDiagConfig=eqDiagConfig, portMaxDsPower=portMaxDsPower, systemKey=systemKey, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardPicSerialNum=cardPicSerialNum, portDplxStatus=portDplxStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, portDetectedMode=portDetectedMode, systemClock=systemClock, cardTestResultDesc=cardTestResultDesc, diskVolumeTable=diskVolumeTable, cmResetClearNotification=cmResetClearNotification, remainInDiagMode=remainInDiagMode, cardType=cardType, TestId=TestId, auditTable=auditTable, TestTransactionId=TestTransactionId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestDescription=cardTestDescription, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlDetected=portRxFlowControlDetected, PYSNMP_MODULE_ID=cadEquipmentMib, cmRegistrationNotification=cmRegistrationNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditResult=auditResult, portPrStateChange=portPrStateChange, portMacIfIndex=portMacIfIndex, cardTemperatureHighWarn=cardTemperatureHighWarn, cardShelfTestIndex=cardShelfTestIndex, portTrapInh=portTrapInh, cardName=cardName, portEntry=portEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, cardNumber=cardNumber, cardLastChangeTime=cardLastChangeTime, shelfName=shelfName, diagTestId=diagTestId, auditLogOutput=auditLogOutput, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentDiagGroup=equipmentDiagGroup, portId=portId, portDplxStatusChange=portDplxStatusChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardDplxStatusChange=cardDplxStatusChange, equipmentAuditGroup=equipmentAuditGroup, fpgaErrorEventTable=fpgaErrorEventTable, cardTestResult=cardTestResult, TestScheduleCommand=TestScheduleCommand, cmResetReason=cmResetReason, cmResetNotification=cmResetNotification, cardDetectedChange=cardDetectedChange, equipmentAudit=equipmentAudit, equipmentError=equipmentError, cardTempNoReportNotification=cardTempNoReportNotification, cardTestType=cardTestType, consoleOutput=consoleOutput, portSecStateChange=portSecStateChange, portNumber=portNumber, cardPrStateChange=cardPrStateChange, cardBootVersion=cardBootVersion, auditCommand=auditCommand, cardTestIndex=cardTestIndex, downstreamPowerLoss=downstreamPowerLoss, cardLastResetReason=cardLastResetReason, systemGeneral=systemGeneral, systemGeneralGroup=systemGeneralGroup, cardDplxStatus=cardDplxStatus, cardLastBootVersion=cardLastBootVersion, equipmentTbl=equipmentTbl, cardFwUpdateStatus=cardFwUpdateStatus, cardUpTime=cardUpTime, equipmentPortGroup=equipmentPortGroup, TestCommand=TestCommand, portLastChangeTime=portLastChangeTime, fpgaErrorEventGroup=fpgaErrorEventGroup, auditLogThrottle=auditLogThrottle, cardSpareGroupId=cardSpareGroupId, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, verboseLevel=verboseLevel, cardCPLDVersion=cardCPLDVersion, portCardSubType=portCardSubType, auditId=auditId, cmResetUpTime=cmResetUpTime, cmIpAddress=cmIpAddress, equipmentState=equipmentState, auditAutoScheduling=auditAutoScheduling, diskVolumeName=diskVolumeName, cardTestResultNotification=cardTestResultNotification, cardSecStateChange=cardSecStateChange, TestSchedule=TestSchedule, trapCounter=trapCounter, diskVolumeCardId=diskVolumeCardId, cardSpareGroupMode=cardSpareGroupMode, cardTestId=cardTestId, cardTestTransId=cardTestTransId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCpuSpeed=cardCpuSpeed, diskVolumeEntry=diskVolumeEntry, cardNumPorts=cardNumPorts, portDocsIfIndex=portDocsIfIndex, cardTestCommand=cardTestCommand, errEvRowStatus=errEvRowStatus, auditCycleCount=auditCycleCount, diskVolumeUsageLevel=diskVolumeUsageLevel, cardCarrierHwVersion=cardCarrierHwVersion, auditTotalPassedCount=auditTotalPassedCount, diskVolumeShelfId=diskVolumeShelfId, portAdminState=portAdminState, cardSecState=cardSecState, cardTestEntry=cardTestEntry, cardTable=cardTable, cmPrimarySID=cmPrimarySID, cardTemperature=cardTemperature, auditFailedCount=auditFailedCount, diskVolumeGroup=diskVolumeGroup, equipmentCardTestGroup=equipmentCardTestGroup, auditStatus=auditStatus, equipmentTraps=equipmentTraps, cardTrapInh=cardTrapInh, cardTestSchedule=cardTestSchedule, cardShelfId=cardShelfId, diskVolumeId=diskVolumeId, cardBusSpeed=cardBusSpeed, shelfObjects=shelfObjects, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardEntry=cardEntry, cmMacAddress=cmMacAddress, cardCpuType=cardCpuType, portCardId=portCardId, cmResetInfo=cmResetInfo, portConnectorId=portConnectorId, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfNumber=shelfNumber, auditName=auditName, cmResetStatus=cmResetStatus, cardSerialNum=cardSerialNum, cardRamSize=cardRamSize, cardPicDetected=cardPicDetected, auditTotalFailedCount=auditTotalFailedCount, TestResult=TestResult, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeTrapInh=diskVolumeTrapInh, cmVendor=cmVendor, portTxFlowControlMode=portTxFlowControlMode, auditDescription=auditDescription, auditPassedCount=auditPassedCount, portRxFlowControlMode=portRxFlowControlMode, cardAction=cardAction)
