...
CardType, DiskVolumeUsageLevel, AdminState, FirmwareSource, CardSubType, PortType, FlowControlMode, PicType, PortId, PortDetectedMode, ShelfId, DuplexStatus, CardId, PortMode, UnknownState, PrimaryState, SecondaryState, EqActionType = mibBuilder.importSymbols("CADANT-TC", "CardType", "DiskVolumeUsageLevel", "AdminState", "FirmwareSource", "CardSubType", "PortType", "FlowControlMode", "PicType", "PortId", "PortDetectedMode", "ShelfId", "DuplexStatus", "CardId", "PortMode", "UnknownState", "PrimaryState", "SecondaryState", "EqActionType")
...
TextualConvention, DisplayString, TimeStamp, MacAddress, DateAndTime, TruthValue, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "TimeStamp", "MacAddress", "DateAndTime", "TruthValue", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardShelfId=cardShelfId, portPrStateChange=portPrStateChange, cardTestResultDesc=cardTestResultDesc, cardTestType=cardTestType, cardLastBootVersion=cardLastBootVersion, portTable=portTable, cmUChannelID=cmUChannelID, cadEquipmentMibCompliances=cadEquipmentMibCompliances, trapSeverity=trapSeverity, TestCommand=TestCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardNumber=cardNumber, portId=portId, cardPrStateChange=cardPrStateChange, cardId=cardId, auditTotalPassedCount=auditTotalPassedCount, systemClock=systemClock, auditTable=auditTable, fpgaErrorEventEntry=fpgaErrorEventEntry, portShelfId=portShelfId, cmRegistrationNotification=cmRegistrationNotification, portNumber=portNumber, portConnectorId=portConnectorId, shelfSwVersion=shelfSwVersion, diskVolumeId=diskVolumeId, cardTestResultNotification=cardTestResultNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCpuType=cardCpuType, auditStatus=auditStatus, cmResetReason=cmResetReason, cmResetClearNotification=cmResetClearNotification, cardTable=cardTable, cardSwVersion=cardSwVersion, portCardId=portCardId, diskVolumeSize=diskVolumeSize, cardDplxStatus=cardDplxStatus, cardTestSchedule=cardTestSchedule, cardEntry=cardEntry, diskVolumeUsageNotification=diskVolumeUsageNotification, cardRamSize=cardRamSize, diskVolumeTrapInh=diskVolumeTrapInh, cardTestEntry=cardTestEntry, eqDiagConfig=eqDiagConfig, systemGeneral=systemGeneral, portRxFlowControlMode=portRxFlowControlMode, diskVolumeGroup=diskVolumeGroup, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeDiskSize=diskVolumeDiskSize, cardTestName=cardTestName, cardTestTransId=cardTestTransId, TestScheduleCommand=TestScheduleCommand, shelfObjects=shelfObjects, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeEntry=diskVolumeEntry, portLastChangeTime=portLastChangeTime, errEvLoggingEnabled=errEvLoggingEnabled, trapCounter=trapCounter, TestId=TestId, cardPicSerialNum=cardPicSerialNum, cardFpgaSource=cardFpgaSource, shelfNumber=shelfNumber, cardUpTime=cardUpTime, equipmentAudit=equipmentAudit, auditLogOutput=auditLogOutput, portNumChans=portNumChans, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSubType=cardSubType, equipmentState=equipmentState, cardTemperatureHighError=cardTemperatureHighError, cardTestResult=cardTestResult, shelfName=shelfName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditTotalFailedCount=auditTotalFailedCount, cardFlashSize=cardFlashSize, cardAdminState=cardAdminState, systemKey=systemKey, cardType=cardType, cardBootVersion=cardBootVersion, equipmentError=equipmentError, fpgaErrorEventTable=fpgaErrorEventTable, auditTime=auditTime, portDocsIfIndex=portDocsIfIndex, portSecState=portSecState, diskVolumeFileName=diskVolumeFileName, cardTestTime=cardTestTime, portDetectedMode=portDetectedMode, cardCpuSpeed=cardCpuSpeed, portMode=portMode, equipmentDiagGroup=equipmentDiagGroup, errEventId=errEventId, portMacIfIndex=portMacIfIndex, cardSecState=cardSecState, cardSecStateChange=cardSecStateChange, auditShelfId=auditShelfId, cardTemperature=cardTemperature, portDescription=portDescription, diskVolumeShelfId=diskVolumeShelfId, cardTestDescription=cardTestDescription, cardBusSpeed=cardBusSpeed, cardAction=cardAction, equipmentPortGroup=equipmentPortGroup, cardPrState=cardPrState, portSecStateChange=portSecStateChange, equipmentStateGroup=equipmentStateGroup, cardTestTable=cardTestTable, diagTestId=diagTestId, auditEntry=auditEntry, cardLastResetReason=cardLastResetReason, cmResetUpTime=cmResetUpTime, equipmentCardTestGroup=equipmentCardTestGroup, cardSpareGroupMode=cardSpareGroupMode, auditName=auditName, cardLastChangeTime=cardLastChangeTime, cmPrimarySID=cmPrimarySID, errEvLogLevel=errEvLogLevel, cardName=cardName, errEvRecoveryEnabled=errEvRecoveryEnabled, systemGeneralGroup=systemGeneralGroup, cardHwVersion=cardHwVersion, equipmentShelfGroup=equipmentShelfGroup, portPrState=portPrState, portBgpId=portBgpId, portCardSubType=portCardSubType, diskVolumeName=diskVolumeName, equipmentDiag=equipmentDiag, cardSerialNum=cardSerialNum, portEntry=portEntry, diskVolumeCardId=diskVolumeCardId, TestType=TestType, portAction=portAction, auditCycleCount=auditCycleCount, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastBootSource=cardLastBootSource, errEvRowStatus=errEvRowStatus, cardDetectedChange=cardDetectedChange, downstreamPowerLoss=downstreamPowerLoss, equipmentCardGroup=equipmentCardGroup, cardTempNoReportNotification=cardTempNoReportNotification, portAdminState=portAdminState, portMinDsPower=portMinDsPower, diskVolumeTable=diskVolumeTable, diskVolumeDriveId=diskVolumeDriveId, cadEquipmentMib=cadEquipmentMib, portTxFlowControlMode=portTxFlowControlMode, cardTestIndex=cardTestIndex, cardFwVersion=cardFwVersion, verboseLevel=verboseLevel, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogThrottle=auditLogThrottle, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicDetected=cardPicDetected, cardNumPorts=cardNumPorts, PYSNMP_MODULE_ID=cadEquipmentMib, portType=portType, cardTestScheduleCommand=cardTestScheduleCommand, auditResult=auditResult, cmResetInfo=cmResetInfo, cmMacAddress=cmMacAddress, cmResetNotification=cmResetNotification, diskDriveNumber=diskDriveNumber, portTxFlowControlDetected=portTxFlowControlDetected, TestTransactionId=TestTransactionId, auditDescription=auditDescription, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditPassedCount=auditPassedCount, cardPicHwVersion=cardPicHwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeNumber=diskVolumeNumber, auditCommand=auditCommand, cmResetStatus=cmResetStatus, cardSubDetected=cardSubDetected, cardCarrierSerialNum=cardCarrierSerialNum, portCurrDsPower=portCurrDsPower, equipmentTraps=equipmentTraps, cardTestId=cardTestId, portMacAddress=portMacAddress, TestSchedule=TestSchedule, remainInDiagMode=remainInDiagMode, cardCPLDVersion=cardCPLDVersion, cardCarrierHwVersion=cardCarrierHwVersion, cardShelfTestIndex=cardShelfTestIndex, auditCardId=auditCardId, portMaxDsPower=portMaxDsPower, portTrapInh=portTrapInh, auditAutoScheduling=auditAutoScheduling, TestResult=TestResult, auditId=auditId, cardDplxStatusChange=cardDplxStatusChange, cardDetected=cardDetected, cmVendor=cmVendor, portDplxStatus=portDplxStatus, cardTrapInh=cardTrapInh, cmIpAddress=cmIpAddress, consoleOutput=consoleOutput, cardTestCommand=cardTestCommand, cmDevice=cmDevice, equipmentAuditGroup=equipmentAuditGroup, equipmentTbl=equipmentTbl, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMibGroup=cadEquipmentMibGroup, portDplxStatusChange=portDplxStatusChange, cardTemperatureHighWarn=cardTemperatureHighWarn, auditFailedCount=auditFailedCount, cardSpareGroupId=cardSpareGroupId)
