...
DiskVolumeUsageLevel, PrimaryState, FirmwareSource, SecondaryState, PortDetectedMode, CardType, ShelfId, UnknownState, EqActionType, FlowControlMode, PortId, DuplexStatus, PortType, AdminState, PicType, CardId, PortMode, CardSubType = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "PrimaryState", "FirmwareSource", "SecondaryState", "PortDetectedMode", "CardType", "ShelfId", "UnknownState", "EqActionType", "FlowControlMode", "PortId", "DuplexStatus", "PortType", "AdminState", "PicType", "CardId", "PortMode", "CardSubType")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestTime=cardTestTime, cardTestTable=cardTestTable, cadEquipmentMibConformance=cadEquipmentMibConformance, shelfNumber=shelfNumber, cardAction=cardAction, cardCPLDVersion=cardCPLDVersion, auditDescription=auditDescription, diskVolumeDiskSize=diskVolumeDiskSize, cmPrimarySID=cmPrimarySID, portType=portType, auditTotalPassedCount=auditTotalPassedCount, portTable=portTable, TestResult=TestResult, portSecStateChange=portSecStateChange, equipmentAuditGroup=equipmentAuditGroup, auditPassedCount=auditPassedCount, cardTable=cardTable, auditShelfId=auditShelfId, cardTemperature=cardTemperature, diskVolumeUsageNotification=diskVolumeUsageNotification, portCardId=portCardId, TestId=TestId, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestCommand=TestCommand, equipmentError=equipmentError, errEventId=errEventId, cardUpTime=cardUpTime, trapCounter=trapCounter, cardAdminState=cardAdminState, portTxFlowControlMode=portTxFlowControlMode, cardTestScheduleCommand=cardTestScheduleCommand, cardFwVersion=cardFwVersion, cardLastResetReason=cardLastResetReason, cmResetReason=cmResetReason, errEvLoggingEnabled=errEvLoggingEnabled, TestScheduleCommand=TestScheduleCommand, cardTestCommand=cardTestCommand, cardSecState=cardSecState, cardTrapInh=cardTrapInh, portId=portId, auditId=auditId, diskVolumeGroup=diskVolumeGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditTable=auditTable, diskVolumeTable=diskVolumeTable, cmResetInfo=cmResetInfo, auditTotalFailedCount=auditTotalFailedCount, shelfSwVersion=shelfSwVersion, cardCarrierFwVersion=cardCarrierFwVersion, portDplxStatus=portDplxStatus, cmResetNotification=cmResetNotification, downstreamPowerLoss=downstreamPowerLoss, equipmentPortGroup=equipmentPortGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestId=cardTestId, fpgaErrorEventTable=fpgaErrorEventTable, cardTestResultDesc=cardTestResultDesc, trapSeverity=trapSeverity, portMode=portMode, errEvRowStatus=errEvRowStatus, shelfName=shelfName, cardTempOverHeatNotification=cardTempOverHeatNotification, cardId=cardId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardBusSpeed=cardBusSpeed, cardPicHwVersion=cardPicHwVersion, auditResult=auditResult, equipmentDiag=equipmentDiag, fpgaErrorEventGroup=fpgaErrorEventGroup, TestTransactionId=TestTransactionId, equipmentCardGroup=equipmentCardGroup, TestSchedule=TestSchedule, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditLogOutput=auditLogOutput, cardTestType=cardTestType, cardTestTransId=cardTestTransId, remainInDiagMode=remainInDiagMode, cardCarrierHwVersion=cardCarrierHwVersion, cardSerialNum=cardSerialNum, cardTestResult=cardTestResult, cadEquipmentMib=cadEquipmentMib, portShelfId=portShelfId, TestType=TestType, cmMacAddress=cmMacAddress, cardSecStateChange=cardSecStateChange, cardTestIndex=cardTestIndex, cardFlashSize=cardFlashSize, portCardSubType=portCardSubType, diskVolumeEntry=diskVolumeEntry, diskDriveNumber=diskDriveNumber, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditLogThrottle=auditLogThrottle, diskVolumeCardId=diskVolumeCardId, cardTemperatureHighError=cardTemperatureHighError, cardTempNoReportNotification=cardTempNoReportNotification, shelfObjects=shelfObjects, portNumber=portNumber, cardPrStateChange=cardPrStateChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardNumPorts=cardNumPorts, diskVolumeId=diskVolumeId, auditTime=auditTime, cardRamSize=cardRamSize, equipmentCardTestGroup=equipmentCardTestGroup, cardLastChangeTime=cardLastChangeTime, portDocsIfIndex=portDocsIfIndex, portMinDsPower=portMinDsPower, diskVolumeName=diskVolumeName, verboseLevel=verboseLevel, cardDplxStatus=cardDplxStatus, cardTestSchedule=cardTestSchedule, auditName=auditName, equipmentTraps=equipmentTraps, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestEntry=cardTestEntry, diskVolumeNumber=diskVolumeNumber, portSecState=portSecState, portBgpId=portBgpId, cardCpuSpeed=cardCpuSpeed, cardType=cardType, cardPrState=cardPrState, auditAutoScheduling=auditAutoScheduling, portPrStateChange=portPrStateChange, auditStatus=auditStatus, auditEntry=auditEntry, cardDetectedChange=cardDetectedChange, portDetectedMode=portDetectedMode, equipmentAudit=equipmentAudit, cardLastBootSource=cardLastBootSource, cardPicDetected=cardPicDetected, cardEntry=cardEntry, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, systemClock=systemClock, consoleOutput=consoleOutput, errEvLogLevel=errEvLogLevel, cardShelfId=cardShelfId, portRxFlowControlMode=portRxFlowControlMode, eqDiagConfig=eqDiagConfig, portAction=portAction, portPrState=portPrState, cmResetClearNotification=cmResetClearNotification, cardShelfTestIndex=cardShelfTestIndex, diskVolumeDriveId=diskVolumeDriveId, portLastChangeTime=portLastChangeTime, cardSubDetected=cardSubDetected, auditCycleCount=auditCycleCount, PYSNMP_MODULE_ID=cadEquipmentMib, systemKey=systemKey, cardDplxStatusChange=cardDplxStatusChange, diskVolumeFileName=diskVolumeFileName, portMaxDsPower=portMaxDsPower, diskVolumeTrapInh=diskVolumeTrapInh, auditFailedCount=auditFailedCount, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeSize=diskVolumeSize, cmIpAddress=cmIpAddress, cardCpuType=cardCpuType, cmDevice=cmDevice, cardDetected=cardDetected, portRxFlowControlDetected=portRxFlowControlDetected, portMacAddress=portMacAddress, portDescription=portDescription, diagTestId=diagTestId, fpgaErrorEventEntry=fpgaErrorEventEntry, portCurrDsPower=portCurrDsPower, cmResetStatus=cmResetStatus, cardName=cardName, equipmentDiagGroup=equipmentDiagGroup, cardHwVersion=cardHwVersion, portMacIfIndex=portMacIfIndex, cardSwVersion=cardSwVersion, cardSpareGroupId=cardSpareGroupId, cardNumber=cardNumber, cardTemperatureHighWarn=cardTemperatureHighWarn, portTxFlowControlDetected=portTxFlowControlDetected, portTrapInh=portTrapInh, cardTestDescription=cardTestDescription, diskVolumeShelfId=diskVolumeShelfId, cmRegistrationNotification=cmRegistrationNotification, auditCommand=auditCommand, cardBootVersion=cardBootVersion, cardLastBootVersion=cardLastBootVersion, cmResetUpTime=cmResetUpTime, systemGeneral=systemGeneral, portDplxStatusChange=portDplxStatusChange, portAdminState=portAdminState, cardTestName=cardTestName, cardFpgaSource=cardFpgaSource, cardSubType=cardSubType, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, cardFwUpdateStatus=cardFwUpdateStatus, cmUChannelID=cmUChannelID, cmVendor=cmVendor, equipmentState=equipmentState, portConnectorId=portConnectorId, portEntry=portEntry, auditCardId=auditCardId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSpareGroupMode=cardSpareGroupMode, portNumChans=portNumChans, cardPicSerialNum=cardPicSerialNum, equipmentTbl=equipmentTbl, equipmentShelfGroup=equipmentShelfGroup)
