...
CardSubType, DiskVolumeUsageLevel, PortId, PicType, CardType, PortType, FirmwareSource, UnknownState, SecondaryState, PortMode, FlowControlMode, PrimaryState, AdminState, ShelfId, EqActionType, CardId, DuplexStatus, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "DiskVolumeUsageLevel", "PortId", "PicType", "CardType", "PortType", "FirmwareSource", "UnknownState", "SecondaryState", "PortMode", "FlowControlMode", "PrimaryState", "AdminState", "ShelfId", "EqActionType", "CardId", "DuplexStatus", "PortDetectedMode")
...
TruthValue, DisplayString, RowStatus, TimeStamp, MacAddress, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DisplayString", "RowStatus", "TimeStamp", "MacAddress", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardDetected=cardDetected, equipmentState=equipmentState, cardSwVersion=cardSwVersion, equipmentAuditGroup=equipmentAuditGroup, shelfSwVersion=shelfSwVersion, cardDplxStatus=cardDplxStatus, cardSpareGroupMode=cardSpareGroupMode, portConnectorId=portConnectorId, cardSubDetected=cardSubDetected, diskVolumeEntry=diskVolumeEntry, portSecStateChange=portSecStateChange, equipmentAudit=equipmentAudit, auditShelfId=auditShelfId, cardTestId=cardTestId, shelfNumber=shelfNumber, cardTestName=cardTestName, cardTestCommand=cardTestCommand, cardTestTime=cardTestTime, errEventId=errEventId, cmDevice=cmDevice, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, TestResult=TestResult, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeId=diskVolumeId, cardFlashSize=cardFlashSize, cardTemperatureHighError=cardTemperatureHighError, fpgaErrorEventTable=fpgaErrorEventTable, cardShelfTestIndex=cardShelfTestIndex, systemGeneralGroup=systemGeneralGroup, equipmentShelfGroup=equipmentShelfGroup, cardCpuSpeed=cardCpuSpeed, diskVolumeShelfId=diskVolumeShelfId, cmPrimarySID=cmPrimarySID, portPrState=portPrState, equipmentTraps=equipmentTraps, cardDetectedChange=cardDetectedChange, cmResetNotification=cmResetNotification, PYSNMP_MODULE_ID=cadEquipmentMib, systemKey=systemKey, cardTestDescription=cardTestDescription, portType=portType, systemClock=systemClock, auditCycleCount=auditCycleCount, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeDriveId=diskVolumeDriveId, cardLastResetReason=cardLastResetReason, cardTestResultDesc=cardTestResultDesc, portDplxStatusChange=portDplxStatusChange, auditFailedCount=auditFailedCount, auditDescription=auditDescription, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentTbl=equipmentTbl, portRxFlowControlDetected=portRxFlowControlDetected, cardPicDetected=cardPicDetected, auditTime=auditTime, diskVolumeTrapInh=diskVolumeTrapInh, auditLogOutput=auditLogOutput, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditLogThrottle=auditLogThrottle, portMode=portMode, cardEntry=cardEntry, portTrapInh=portTrapInh, cardTemperature=cardTemperature, portTxFlowControlMode=portTxFlowControlMode, portMacAddress=portMacAddress, errEvRowStatus=errEvRowStatus, auditCommand=auditCommand, cmResetInfo=cmResetInfo, errEvRecoveryEnabled=errEvRecoveryEnabled, portEntry=portEntry, cardUpTime=cardUpTime, diskVolumeFileName=diskVolumeFileName, equipmentDiag=equipmentDiag, cardTempNoReportNotification=cardTempNoReportNotification, cardAction=cardAction, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, TestScheduleCommand=TestScheduleCommand, auditTotalFailedCount=auditTotalFailedCount, cardTable=cardTable, equipmentCardGroup=equipmentCardGroup, auditId=auditId, cardSecStateChange=cardSecStateChange, cmVendor=cmVendor, cardPicHwVersion=cardPicHwVersion, portRxFlowControlMode=portRxFlowControlMode, portDetectedMode=portDetectedMode, cardType=cardType, auditResult=auditResult, diskVolumeUsageNotification=diskVolumeUsageNotification, cmResetReason=cmResetReason, cardSpareGroupId=cardSpareGroupId, systemGeneral=systemGeneral, fpgaErrorEventEntry=fpgaErrorEventEntry, cmRegistrationNotification=cmRegistrationNotification, cardTestType=cardTestType, cardTestResult=cardTestResult, equipmentDiagGroup=equipmentDiagGroup, equipmentPortGroup=equipmentPortGroup, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCPLDVersion=cardCPLDVersion, portShelfId=portShelfId, cmResetUpTime=cmResetUpTime, TestSchedule=TestSchedule, cardSubType=cardSubType, cardLastChangeTime=cardLastChangeTime, cardTestIndex=cardTestIndex, cardPrStateChange=cardPrStateChange, diskVolumeSize=diskVolumeSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardAdminState=cardAdminState, portPrStateChange=portPrStateChange, cardFwVersion=cardFwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeNumber=diskVolumeNumber, portTxFlowControlDetected=portTxFlowControlDetected, cmMacAddress=cmMacAddress, cardDplxStatusChange=cardDplxStatusChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardBusSpeed=cardBusSpeed, cmIpAddress=cmIpAddress, portCardSubType=portCardSubType, cardTestEntry=cardTestEntry, portBgpId=portBgpId, TestId=TestId, errEvLoggingEnabled=errEvLoggingEnabled, verboseLevel=verboseLevel, diskVolumeName=diskVolumeName, cadEquipmentMib=cadEquipmentMib, cardFwUpdateStatus=cardFwUpdateStatus, portAction=portAction, auditTable=auditTable, cadEquipmentMibConformance=cadEquipmentMibConformance, trapCounter=trapCounter, portNumber=portNumber, cardId=cardId, cardShelfId=cardShelfId, cardCpuType=cardCpuType, cardHwVersion=cardHwVersion, shelfName=shelfName, diskVolumeDiskSize=diskVolumeDiskSize, auditName=auditName, equipmentCardTestGroup=equipmentCardTestGroup, trapSeverity=trapSeverity, portAdminState=portAdminState, portSecState=portSecState, auditEntry=auditEntry, diskVolumeCardId=diskVolumeCardId, auditCardId=auditCardId, cardFpgaSource=cardFpgaSource, cardTrapInh=cardTrapInh, portMacIfIndex=portMacIfIndex, portDplxStatus=portDplxStatus, remainInDiagMode=remainInDiagMode, errEvLogLevel=errEvLogLevel, eqDiagConfig=eqDiagConfig, auditTotalPassedCount=auditTotalPassedCount, TestTransactionId=TestTransactionId, cardSerialNum=cardSerialNum, cardBootVersion=cardBootVersion, equipmentStateGroup=equipmentStateGroup, cardLastBootVersion=cardLastBootVersion, cardTestSchedule=cardTestSchedule, cardCarrierSerialNum=cardCarrierSerialNum, cardCarrierHwVersion=cardCarrierHwVersion, portCardId=portCardId, portCurrDsPower=portCurrDsPower, auditStatus=auditStatus, portId=portId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSecState=cardSecState, cmUChannelID=cmUChannelID, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestType=TestType, portMaxDsPower=portMaxDsPower, shelfObjects=shelfObjects, portMinDsPower=portMinDsPower, equipmentError=equipmentError, cardPicSerialNum=cardPicSerialNum, portTable=portTable, portNumChans=portNumChans, downstreamPowerLoss=downstreamPowerLoss, auditPassedCount=auditPassedCount, portLastChangeTime=portLastChangeTime, cardRamSize=cardRamSize, portDescription=portDescription, cmResetStatus=cmResetStatus, cardTestTransId=cardTestTransId, TestCommand=TestCommand, consoleOutput=consoleOutput, fpgaErrorEventGroup=fpgaErrorEventGroup, cardPrState=cardPrState, cardTestTable=cardTestTable, diskVolumeGroup=diskVolumeGroup, cardName=cardName, cardLastBootSource=cardLastBootSource, diskVolumeTable=diskVolumeTable, diskDriveNumber=diskDriveNumber, portDocsIfIndex=portDocsIfIndex, cardCarrierFwVersion=cardCarrierFwVersion, cmResetClearNotification=cmResetClearNotification, cardNumber=cardNumber, diagTestId=diagTestId)
