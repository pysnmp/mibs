...
CardType, AdminState, FlowControlMode, PicType, PortDetectedMode, UnknownState, CardId, PortId, ShelfId, DiskVolumeUsageLevel, DuplexStatus, FirmwareSource, PrimaryState, PortMode, PortType, EqActionType, CardSubType, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "CardType", "AdminState", "FlowControlMode", "PicType", "PortDetectedMode", "UnknownState", "CardId", "PortId", "ShelfId", "DiskVolumeUsageLevel", "DuplexStatus", "FirmwareSource", "PrimaryState", "PortMode", "PortType", "EqActionType", "CardSubType", "SecondaryState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", fpgaErrorEventGroup=fpgaErrorEventGroup, shelfObjects=shelfObjects, diskVolumeId=diskVolumeId, cardCarrierHwVersion=cardCarrierHwVersion, shelfSwVersion=shelfSwVersion, portMode=portMode, portMaxDsPower=portMaxDsPower, auditTotalPassedCount=auditTotalPassedCount, cardTempNoReportNotification=cardTempNoReportNotification, cmUChannelID=cmUChannelID, portDplxStatus=portDplxStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditResult=auditResult, portTxFlowControlDetected=portTxFlowControlDetected, portNumber=portNumber, equipmentAuditGroup=equipmentAuditGroup, cmResetUpTime=cmResetUpTime, TestCommand=TestCommand, auditName=auditName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portPrStateChange=portPrStateChange, portTxFlowControlMode=portTxFlowControlMode, TestScheduleCommand=TestScheduleCommand, systemClock=systemClock, auditCycleCount=auditCycleCount, cardBusSpeed=cardBusSpeed, cardTestEntry=cardTestEntry, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentPortGroup=equipmentPortGroup, cardTemperatureHighError=cardTemperatureHighError, cardCPLDVersion=cardCPLDVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentDiagGroup=equipmentDiagGroup, equipmentCardTestGroup=equipmentCardTestGroup, cardTestType=cardTestType, cardBootVersion=cardBootVersion, portEntry=portEntry, cardLastBootSource=cardLastBootSource, equipmentStateGroup=equipmentStateGroup, TestSchedule=TestSchedule, portMinDsPower=portMinDsPower, cardSubType=cardSubType, portShelfId=portShelfId, diskVolumeCardId=diskVolumeCardId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmPrimarySID=cmPrimarySID, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmRegistrationNotification=cmRegistrationNotification, cardShelfTestIndex=cardShelfTestIndex, portDplxStatusChange=portDplxStatusChange, cardCpuType=cardCpuType, cmResetStatus=cmResetStatus, portMacIfIndex=portMacIfIndex, cardDetected=cardDetected, cmResetReason=cmResetReason, cardTestScheduleCommand=cardTestScheduleCommand, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditEntry=auditEntry, equipmentState=equipmentState, cardTable=cardTable, TestType=TestType, diskVolumeEntry=diskVolumeEntry, portCardSubType=portCardSubType, diskVolumeTrapInh=diskVolumeTrapInh, auditLogThrottle=auditLogThrottle, cardSecState=cardSecState, cardTestName=cardTestName, portRxFlowControlMode=portRxFlowControlMode, cadEquipmentMibConformance=cadEquipmentMibConformance, portType=portType, shelfNumber=shelfNumber, errEvLoggingEnabled=errEvLoggingEnabled, auditCommand=auditCommand, cardTestResultNotification=cardTestResultNotification, cardNumber=cardNumber, portPrState=portPrState, cardLastResetReason=cardLastResetReason, cardRamSize=cardRamSize, cardTrapInh=cardTrapInh, portDetectedMode=portDetectedMode, auditId=auditId, portSecState=portSecState, cardTestResult=cardTestResult, portNumChans=portNumChans, cardPicSerialNum=cardPicSerialNum, diskVolumeSize=diskVolumeSize, equipmentShelfGroup=equipmentShelfGroup, equipmentError=equipmentError, equipmentCardGroup=equipmentCardGroup, equipmentDiag=equipmentDiag, cardDplxStatusChange=cardDplxStatusChange, cmIpAddress=cmIpAddress, cardTestTransId=cardTestTransId, cardTestDescription=cardTestDescription, diskVolumeShelfId=diskVolumeShelfId, cardTestId=cardTestId, TestId=TestId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCardId=auditCardId, systemGeneral=systemGeneral, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, downstreamPowerLoss=downstreamPowerLoss, diskVolumeGroup=diskVolumeGroup, diskVolumeFileName=diskVolumeFileName, diskVolumeDiskSize=diskVolumeDiskSize, cmResetNotification=cmResetNotification, diskVolumeDriveId=diskVolumeDriveId, cardSecStateChange=cardSecStateChange, cardDetectedChange=cardDetectedChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPicHwVersion=cardPicHwVersion, portId=portId, portCurrDsPower=portCurrDsPower, cardUpTime=cardUpTime, auditLogOutput=auditLogOutput, portDescription=portDescription, systemKey=systemKey, cardTemperatureHighWarn=cardTemperatureHighWarn, portAction=portAction, TestTransactionId=TestTransactionId, consoleOutput=consoleOutput, cardTestTable=cardTestTable, portBgpId=portBgpId, auditTime=auditTime, errEventId=errEventId, errEvLogLevel=errEvLogLevel, trapCounter=trapCounter, cardTestIndex=cardTestIndex, auditTable=auditTable, cardPrStateChange=cardPrStateChange, cardCarrierSerialNum=cardCarrierSerialNum, auditShelfId=auditShelfId, portConnectorId=portConnectorId, cardName=cardName, diagTestId=diagTestId, cardPrState=cardPrState, equipmentAudit=equipmentAudit, PYSNMP_MODULE_ID=cadEquipmentMib, auditDescription=auditDescription, errEvRowStatus=errEvRowStatus, cardShelfId=cardShelfId, remainInDiagMode=remainInDiagMode, cardTestCommand=cardTestCommand, auditStatus=auditStatus, cardAction=cardAction, cardTestTime=cardTestTime, portMacAddress=portMacAddress, equipmentTbl=equipmentTbl, cardTempOverHeatNotification=cardTempOverHeatNotification, cmResetInfo=cmResetInfo, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDocsIfIndex=portDocsIfIndex, fpgaErrorEventTable=fpgaErrorEventTable, cardEntry=cardEntry, cardTestResultDesc=cardTestResultDesc, diskDriveNumber=diskDriveNumber, cardCpuSpeed=cardCpuSpeed, trapSeverity=trapSeverity, cadEquipmentMib=cadEquipmentMib, cardFpgaSource=cardFpgaSource, cardId=cardId, cardFwVersion=cardFwVersion, diskVolumeTable=diskVolumeTable, equipmentTraps=equipmentTraps, cardSpareGroupMode=cardSpareGroupMode, cardLastBootVersion=cardLastBootVersion, verboseLevel=verboseLevel, cardTemperature=cardTemperature, portLastChangeTime=portLastChangeTime, portRxFlowControlDetected=portRxFlowControlDetected, auditPassedCount=auditPassedCount, cmDevice=cmDevice, cardSwVersion=cardSwVersion, cardType=cardType, portCardId=portCardId, diskVolumeUsageLevel=diskVolumeUsageLevel, eqDiagConfig=eqDiagConfig, portSecStateChange=portSecStateChange, cardSpareGroupId=cardSpareGroupId, cardSerialNum=cardSerialNum, cardPicDetected=cardPicDetected, systemGeneralGroup=systemGeneralGroup, cardAdminState=cardAdminState, cardFwUpdateStatus=cardFwUpdateStatus, portAdminState=portAdminState, cardCarrierFwVersion=cardCarrierFwVersion, TestResult=TestResult, portTrapInh=portTrapInh, auditFailedCount=auditFailedCount, cardNumPorts=cardNumPorts, cmMacAddress=cmMacAddress, cmVendor=cmVendor, cmResetClearNotification=cmResetClearNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, cardLastChangeTime=cardLastChangeTime, cardTestSchedule=cardTestSchedule, cardFlashSize=cardFlashSize, portTable=portTable, cardHwVersion=cardHwVersion, shelfName=shelfName, diskVolumeName=diskVolumeName, cardSubDetected=cardSubDetected, cardDplxStatus=cardDplxStatus, diskVolumeNumber=diskVolumeNumber)
