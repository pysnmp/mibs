...
DiskVolumeUsageLevel, PortId, CardType, PortType, ShelfId, EqActionType, AdminState, FirmwareSource, PicType, UnknownState, PortMode, SecondaryState, PortDetectedMode, PrimaryState, CardId, CardSubType, DuplexStatus, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "PortId", "CardType", "PortType", "ShelfId", "EqActionType", "AdminState", "FirmwareSource", "PicType", "UnknownState", "PortMode", "SecondaryState", "PortDetectedMode", "PrimaryState", "CardId", "CardSubType", "DuplexStatus", "FlowControlMode")
...
MacAddress, TruthValue, RowStatus, DateAndTime, DisplayString, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "RowStatus", "DateAndTime", "DisplayString", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portConnectorId=portConnectorId, TestId=TestId, cardTestResult=cardTestResult, cardTestResultDesc=cardTestResultDesc, cmMacAddress=cmMacAddress, cardTempOverHeatNotification=cardTempOverHeatNotification, cmIpAddress=cmIpAddress, cardShelfTestIndex=cardShelfTestIndex, diskVolumeSize=diskVolumeSize, fpgaErrorEventTable=fpgaErrorEventTable, portRxFlowControlDetected=portRxFlowControlDetected, cadEquipmentMib=cadEquipmentMib, downstreamPowerLoss=downstreamPowerLoss, auditCommand=auditCommand, cardAction=cardAction, portAction=portAction, TestSchedule=TestSchedule, portMode=portMode, cardTrapInh=cardTrapInh, diagTestId=diagTestId, cardSpareGroupMode=cardSpareGroupMode, cardFwUpdateStatus=cardFwUpdateStatus, systemGeneral=systemGeneral, equipmentStateGroup=equipmentStateGroup, cardTestEntry=cardTestEntry, systemGeneralGroup=systemGeneralGroup, cmResetUpTime=cmResetUpTime, cardRamSize=cardRamSize, cardName=cardName, TestScheduleCommand=TestScheduleCommand, auditShelfId=auditShelfId, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCpuSpeed=cardCpuSpeed, portCardId=portCardId, portTxFlowControlMode=portTxFlowControlMode, equipmentPortGroup=equipmentPortGroup, cardSecState=cardSecState, cardTestName=cardTestName, equipmentTbl=equipmentTbl, portType=portType, cmVendor=cmVendor, cardSubType=cardSubType, cardSpareGroupId=cardSpareGroupId, diskVolumeNumber=diskVolumeNumber, equipmentDiag=equipmentDiag, auditResult=auditResult, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestScheduleCommand=cardTestScheduleCommand, errEvRowStatus=errEvRowStatus, cardTestDescription=cardTestDescription, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestTable=cardTestTable, auditStatus=auditStatus, trapSeverity=trapSeverity, shelfSwVersion=shelfSwVersion, portNumber=portNumber, cardLastBootVersion=cardLastBootVersion, TestCommand=TestCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicSerialNum=cardPicSerialNum, diskVolumeId=diskVolumeId, cardBusSpeed=cardBusSpeed, TestResult=TestResult, diskVolumeTable=diskVolumeTable, diskVolumeDriveId=diskVolumeDriveId, equipmentShelfGroup=equipmentShelfGroup, portMacIfIndex=portMacIfIndex, cardDetected=cardDetected, errEvLoggingEnabled=errEvLoggingEnabled, cardBootVersion=cardBootVersion, equipmentError=equipmentError, cardEntry=cardEntry, diskDriveNumber=diskDriveNumber, cardDetectedChange=cardDetectedChange, equipmentCardGroup=equipmentCardGroup, diskVolumeTrapInh=diskVolumeTrapInh, portCurrDsPower=portCurrDsPower, equipmentTraps=equipmentTraps, cardPrState=cardPrState, cardTestTime=cardTestTime, cmDevice=cmDevice, equipmentAuditGroup=equipmentAuditGroup, cardFlashSize=cardFlashSize, auditFailedCount=auditFailedCount, portDocsIfIndex=portDocsIfIndex, auditCardId=auditCardId, cardTemperature=cardTemperature, diskVolumeUsageLevel=diskVolumeUsageLevel, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCPLDVersion=cardCPLDVersion, cardDplxStatus=cardDplxStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeFileName=diskVolumeFileName, diskVolumeGroup=diskVolumeGroup, cmResetInfo=cmResetInfo, fpgaErrorEventEntry=fpgaErrorEventEntry, cardFpgaSource=cardFpgaSource, cmRegistrationNotification=cmRegistrationNotification, cardTable=cardTable, cmResetClearNotification=cmResetClearNotification, cardTestId=cardTestId, portBgpId=portBgpId, cardCpuType=cardCpuType, cardSerialNum=cardSerialNum, cardAdminState=cardAdminState, PYSNMP_MODULE_ID=cadEquipmentMib, cardUpTime=cardUpTime, cardPicDetected=cardPicDetected, verboseLevel=verboseLevel, cardId=cardId, cardFwVersion=cardFwVersion, portCardSubType=portCardSubType, cmUChannelID=cmUChannelID, cadEquipmentMibConformance=cadEquipmentMibConformance, portPrState=portPrState, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardNumber=cardNumber, cardShelfId=cardShelfId, cardType=cardType, portId=portId, cardNumPorts=cardNumPorts, cardTemperatureHighError=cardTemperatureHighError, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditTable=auditTable, auditId=auditId, cardPrStateChange=cardPrStateChange, portDplxStatusChange=portDplxStatusChange, diskVolumeDiskSize=diskVolumeDiskSize, consoleOutput=consoleOutput, auditDescription=auditDescription, auditTime=auditTime, cardLastResetReason=cardLastResetReason, cardPicHwVersion=cardPicHwVersion, cmResetStatus=cmResetStatus, cardSecStateChange=cardSecStateChange, TestType=TestType, cardTestType=cardTestType, shelfName=shelfName, equipmentCardTestGroup=equipmentCardTestGroup, cardCarrierHwVersion=cardCarrierHwVersion, eqDiagConfig=eqDiagConfig, portMaxDsPower=portMaxDsPower, trapCounter=trapCounter, auditPassedCount=auditPassedCount, portSecStateChange=portSecStateChange, portSecState=portSecState, auditName=auditName, auditCycleCount=auditCycleCount, cardTestSchedule=cardTestSchedule, shelfNumber=shelfNumber, auditEntry=auditEntry, cmResetReason=cmResetReason, cardTestTransId=cardTestTransId, portPrStateChange=portPrStateChange, remainInDiagMode=remainInDiagMode, cardSwVersion=cardSwVersion, cardTempNoReportNotification=cardTempNoReportNotification, equipmentState=equipmentState, auditAutoScheduling=auditAutoScheduling, cardLastBootSource=cardLastBootSource, cardTestResultNotification=cardTestResultNotification, portDescription=portDescription, diskVolumeName=diskVolumeName, cardTemperatureHighWarn=cardTemperatureHighWarn, systemKey=systemKey, TestTransactionId=TestTransactionId, cmPrimarySID=cmPrimarySID, cardLastChangeTime=cardLastChangeTime, diskVolumeEntry=diskVolumeEntry, auditLogOutput=auditLogOutput, errEventId=errEventId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardHwVersion=cardHwVersion, auditLogThrottle=auditLogThrottle, portShelfId=portShelfId, portTrapInh=portTrapInh, portMacAddress=portMacAddress, portDetectedMode=portDetectedMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, shelfObjects=shelfObjects, cardTestIndex=cardTestIndex, equipmentAudit=equipmentAudit, systemClock=systemClock, portMinDsPower=portMinDsPower, cardSubDetected=cardSubDetected, errEvLogLevel=errEvLogLevel, portTable=portTable, portDplxStatus=portDplxStatus, auditTotalFailedCount=auditTotalFailedCount, portEntry=portEntry, portRxFlowControlMode=portRxFlowControlMode, diskVolumeCardId=diskVolumeCardId, portLastChangeTime=portLastChangeTime, portAdminState=portAdminState, portNumChans=portNumChans, equipmentDiagGroup=equipmentDiagGroup, cardDplxStatusChange=cardDplxStatusChange, auditTotalPassedCount=auditTotalPassedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestCommand=cardTestCommand, cardCarrierSerialNum=cardCarrierSerialNum, cmResetNotification=cmResetNotification)
