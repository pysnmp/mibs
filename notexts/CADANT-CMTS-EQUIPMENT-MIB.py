...
FirmwareSource, FlowControlMode, CardType, CardSubType, PortType, SecondaryState, ShelfId, DiskVolumeUsageLevel, PortMode, DuplexStatus, PicType, UnknownState, CardId, PrimaryState, PortId, EqActionType, PortDetectedMode, AdminState = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "FlowControlMode", "CardType", "CardSubType", "PortType", "SecondaryState", "ShelfId", "DiskVolumeUsageLevel", "PortMode", "DuplexStatus", "PicType", "UnknownState", "CardId", "PrimaryState", "PortId", "EqActionType", "PortDetectedMode", "AdminState")
...
DisplayString, DateAndTime, TimeStamp, TruthValue, RowStatus, TextualConvention, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "DateAndTime", "TimeStamp", "TruthValue", "RowStatus", "TextualConvention", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestTransId=cardTestTransId, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, shelfSwVersion=shelfSwVersion, cardCpuType=cardCpuType, cardPicSerialNum=cardPicSerialNum, cardFpgaSource=cardFpgaSource, TestCommand=TestCommand, cardSubType=cardSubType, portDescription=portDescription, equipmentState=equipmentState, cardDetectedChange=cardDetectedChange, diskVolumeFileName=diskVolumeFileName, portCardId=portCardId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCpuSpeed=cardCpuSpeed, diskVolumeDiskSize=diskVolumeDiskSize, equipmentError=equipmentError, cmResetClearNotification=cmResetClearNotification, cardSpareGroupMode=cardSpareGroupMode, cmRegistrationNotification=cmRegistrationNotification, cardPrStateChange=cardPrStateChange, auditTotalFailedCount=auditTotalFailedCount, cardDplxStatusChange=cardDplxStatusChange, cardTestResultNotification=cardTestResultNotification, cardPicHwVersion=cardPicHwVersion, cardTestEntry=cardTestEntry, portType=portType, auditEntry=auditEntry, portSecStateChange=portSecStateChange, cadEquipmentMib=cadEquipmentMib, cardTestName=cardTestName, cardTempOverHeatNotification=cardTempOverHeatNotification, downstreamPowerLoss=downstreamPowerLoss, consoleOutput=consoleOutput, TestId=TestId, auditId=auditId, remainInDiagMode=remainInDiagMode, cardAdminState=cardAdminState, verboseLevel=verboseLevel, cardTestId=cardTestId, systemClock=systemClock, cardSwVersion=cardSwVersion, cardSpareGroupId=cardSpareGroupId, cmResetInfo=cmResetInfo, cardNumPorts=cardNumPorts, auditPassedCount=auditPassedCount, errEventId=errEventId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTrapInh=cardTrapInh, equipmentDiag=equipmentDiag, cardCarrierFwVersion=cardCarrierFwVersion, cmResetStatus=cmResetStatus, cardTestSchedule=cardTestSchedule, cardLastResetReason=cardLastResetReason, diskVolumeCardId=diskVolumeCardId, portTable=portTable, equipmentStateGroup=equipmentStateGroup, portCurrDsPower=portCurrDsPower, portMinDsPower=portMinDsPower, cardTemperature=cardTemperature, portDplxStatus=portDplxStatus, cardTestCommand=cardTestCommand, cardUpTime=cardUpTime, cardHwVersion=cardHwVersion, diskVolumeName=diskVolumeName, diskVolumeId=diskVolumeId, auditLogOutput=auditLogOutput, errEvRowStatus=errEvRowStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardAction=cardAction, cardLastBootVersion=cardLastBootVersion, cardFwUpdateStatus=cardFwUpdateStatus, trapCounter=trapCounter, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmMacAddress=cmMacAddress, errEvLogLevel=errEvLogLevel, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentDiagGroup=equipmentDiagGroup, cmResetReason=cmResetReason, cadEquipmentMibGroup=cadEquipmentMibGroup, TestSchedule=TestSchedule, cardTestDescription=cardTestDescription, cardFwVersion=cardFwVersion, cardSubDetected=cardSubDetected, cardName=cardName, equipmentCardGroup=equipmentCardGroup, equipmentAudit=equipmentAudit, cardType=cardType, cardShelfId=cardShelfId, cardPicDetected=cardPicDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, portDplxStatusChange=portDplxStatusChange, portMaxDsPower=portMaxDsPower, cmResetNotification=cmResetNotification, portCardSubType=portCardSubType, auditName=auditName, equipmentShelfGroup=equipmentShelfGroup, cardCarrierHwVersion=cardCarrierHwVersion, portAction=portAction, fpgaErrorEventTable=fpgaErrorEventTable, cardEntry=cardEntry, auditFailedCount=auditFailedCount, portTrapInh=portTrapInh, trapSeverity=trapSeverity, systemKey=systemKey, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardFlashSize=cardFlashSize, portShelfId=portShelfId, equipmentCardTestGroup=equipmentCardTestGroup, cardTestTable=cardTestTable, cardCPLDVersion=cardCPLDVersion, portPrStateChange=portPrStateChange, diskVolumeSize=diskVolumeSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, shelfObjects=shelfObjects, portLastChangeTime=portLastChangeTime, diskVolumeShelfId=diskVolumeShelfId, auditCycleCount=auditCycleCount, cardTestTime=cardTestTime, cardTempNoReportNotification=cardTempNoReportNotification, portId=portId, portDetectedMode=portDetectedMode, cmVendor=cmVendor, cmUChannelID=cmUChannelID, portConnectorId=portConnectorId, cmPrimarySID=cmPrimarySID, cardPrState=cardPrState, diskVolumeTable=diskVolumeTable, systemGeneralGroup=systemGeneralGroup, portMode=portMode, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSerialNum=cardSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, PYSNMP_MODULE_ID=cadEquipmentMib, portSecState=portSecState, cardBusSpeed=cardBusSpeed, cardNumber=cardNumber, diskDriveNumber=diskDriveNumber, cardBootVersion=cardBootVersion, auditDescription=auditDescription, TestTransactionId=TestTransactionId, portNumber=portNumber, auditStatus=auditStatus, cmDevice=cmDevice, eqDiagConfig=eqDiagConfig, auditCommand=auditCommand, diskVolumeDriveId=diskVolumeDriveId, diagTestId=diagTestId, portMacIfIndex=portMacIfIndex, portMacAddress=portMacAddress, cardId=cardId, cardTestScheduleCommand=cardTestScheduleCommand, auditTable=auditTable, auditTotalPassedCount=auditTotalPassedCount, diskVolumeEntry=diskVolumeEntry, diskVolumeGroup=diskVolumeGroup, portBgpId=portBgpId, auditLogThrottle=auditLogThrottle, cmResetUpTime=cmResetUpTime, shelfName=shelfName, shelfNumber=shelfNumber, cardCarrierSerialNum=cardCarrierSerialNum, cmIpAddress=cmIpAddress, portAdminState=portAdminState, auditAutoScheduling=auditAutoScheduling, auditCardId=auditCardId, portDocsIfIndex=portDocsIfIndex, TestScheduleCommand=TestScheduleCommand, cardTestType=cardTestType, cardShelfTestIndex=cardShelfTestIndex, auditResult=auditResult, systemGeneral=systemGeneral, cardTable=cardTable, TestType=TestType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portRxFlowControlDetected=portRxFlowControlDetected, cardTemperatureHighError=cardTemperatureHighError, portPrState=portPrState, cardLastBootSource=cardLastBootSource, portRxFlowControlMode=portRxFlowControlMode, auditShelfId=auditShelfId, equipmentPortGroup=equipmentPortGroup, diskVolumeNumber=diskVolumeNumber, cardSecState=cardSecState, TestResult=TestResult, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestIndex=cardTestIndex, cardLastChangeTime=cardLastChangeTime, cardDplxStatus=cardDplxStatus, cardDetected=cardDetected, equipmentTraps=equipmentTraps, portEntry=portEntry, portTxFlowControlMode=portTxFlowControlMode, cardRamSize=cardRamSize, cardTestResultDesc=cardTestResultDesc, cardSecStateChange=cardSecStateChange, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTrapInh=diskVolumeTrapInh, equipmentAuditGroup=equipmentAuditGroup, equipmentTbl=equipmentTbl, errEvLoggingEnabled=errEvLoggingEnabled, auditTime=auditTime, cardTestResult=cardTestResult, portNumChans=portNumChans)
