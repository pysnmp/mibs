...
ShelfId, AdminState, PortId, CardType, FirmwareSource, FlowControlMode, PortDetectedMode, DiskVolumeUsageLevel, PortType, DuplexStatus, UnknownState, CardSubType, PortMode, PicType, CardId, PrimaryState, EqActionType, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "AdminState", "PortId", "CardType", "FirmwareSource", "FlowControlMode", "PortDetectedMode", "DiskVolumeUsageLevel", "PortType", "DuplexStatus", "UnknownState", "CardSubType", "PortMode", "PicType", "CardId", "PrimaryState", "EqActionType", "SecondaryState")
...
TruthValue, MacAddress, DateAndTime, TimeStamp, DisplayString, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "MacAddress", "DateAndTime", "TimeStamp", "DisplayString", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", PYSNMP_MODULE_ID=cadEquipmentMib, cardFwVersion=cardFwVersion, cmResetStatus=cmResetStatus, downstreamPowerLoss=downstreamPowerLoss, cardTestResultDesc=cardTestResultDesc, auditFailedCount=auditFailedCount, TestResult=TestResult, cardCarrierSerialNum=cardCarrierSerialNum, cardPicHwVersion=cardPicHwVersion, cardTestDescription=cardTestDescription, portShelfId=portShelfId, cardTestType=cardTestType, auditTotalPassedCount=auditTotalPassedCount, auditShelfId=auditShelfId, portLastChangeTime=portLastChangeTime, cardTestEntry=cardTestEntry, cardRamSize=cardRamSize, auditCardId=auditCardId, cmMacAddress=cmMacAddress, portDplxStatusChange=portDplxStatusChange, TestSchedule=TestSchedule, cardLastChangeTime=cardLastChangeTime, cardTemperatureHighError=cardTemperatureHighError, cardTestCommand=cardTestCommand, cardFlashSize=cardFlashSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portMode=portMode, portDocsIfIndex=portDocsIfIndex, cardEntry=cardEntry, cardSpareGroupId=cardSpareGroupId, cardTestId=cardTestId, cardTestIndex=cardTestIndex, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeDriveId=diskVolumeDriveId, errEvLoggingEnabled=errEvLoggingEnabled, errEvLogLevel=errEvLogLevel, auditAutoScheduling=auditAutoScheduling, cardTestName=cardTestName, portConnectorId=portConnectorId, systemGeneral=systemGeneral, cardTestTable=cardTestTable, cardCpuType=cardCpuType, cardCarrierFwVersion=cardCarrierFwVersion, portAction=portAction, diskVolumeUsageNotification=diskVolumeUsageNotification, auditPassedCount=auditPassedCount, cardHwVersion=cardHwVersion, cardLastBootSource=cardLastBootSource, portTable=portTable, portCardId=portCardId, cardTestResultNotification=cardTestResultNotification, cardNumber=cardNumber, cardShelfTestIndex=cardShelfTestIndex, portDplxStatus=portDplxStatus, cmDevice=cmDevice, equipmentShelfGroup=equipmentShelfGroup, diskVolumeTrapInh=diskVolumeTrapInh, equipmentDiagGroup=equipmentDiagGroup, cmUChannelID=cmUChannelID, cardPicSerialNum=cardPicSerialNum, cmResetUpTime=cmResetUpTime, cardPicDetected=cardPicDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, consoleOutput=consoleOutput, TestScheduleCommand=TestScheduleCommand, cardPrStateChange=cardPrStateChange, portSecState=portSecState, auditTable=auditTable, portMinDsPower=portMinDsPower, cardDplxStatusChange=cardDplxStatusChange, cardTestTime=cardTestTime, equipmentCardGroup=equipmentCardGroup, auditEntry=auditEntry, portAdminState=portAdminState, verboseLevel=verboseLevel, auditId=auditId, diagTestId=diagTestId, cmResetNotification=cmResetNotification, equipmentDiag=equipmentDiag, cardSecState=cardSecState, diskVolumeId=diskVolumeId, cardDetected=cardDetected, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSerialNum=cardSerialNum, cardDetectedChange=cardDetectedChange, cadEquipmentMib=cadEquipmentMib, cardFpgaSource=cardFpgaSource, portTxFlowControlDetected=portTxFlowControlDetected, auditTime=auditTime, cardSubDetected=cardSubDetected, TestTransactionId=TestTransactionId, auditName=auditName, shelfNumber=shelfNumber, cardType=cardType, portSecStateChange=portSecStateChange, cardCarrierHwVersion=cardCarrierHwVersion, portPrStateChange=portPrStateChange, cmVendor=cmVendor, eqDiagConfig=eqDiagConfig, cardUpTime=cardUpTime, cardCpuSpeed=cardCpuSpeed, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeCardId=diskVolumeCardId, cardPrState=cardPrState, remainInDiagMode=remainInDiagMode, equipmentStateGroup=equipmentStateGroup, cardAction=cardAction, diskVolumeSize=diskVolumeSize, equipmentAuditGroup=equipmentAuditGroup, shelfName=shelfName, portMacAddress=portMacAddress, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditLogOutput=auditLogOutput, diskVolumeShelfId=diskVolumeShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portCurrDsPower=portCurrDsPower, portCardSubType=portCardSubType, cardTemperatureHighWarn=cardTemperatureHighWarn, cardLastResetReason=cardLastResetReason, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditDescription=auditDescription, errEvRowStatus=errEvRowStatus, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, TestId=TestId, cardId=cardId, equipmentTraps=equipmentTraps, equipmentTbl=equipmentTbl, systemGeneralGroup=systemGeneralGroup, systemKey=systemKey, cardName=cardName, equipmentState=equipmentState, portRxFlowControlMode=portRxFlowControlMode, portTrapInh=portTrapInh, portNumChans=portNumChans, portBgpId=portBgpId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardAdminState=cardAdminState, cardTable=cardTable, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestSchedule=cardTestSchedule, shelfSwVersion=shelfSwVersion, diskVolumeEntry=diskVolumeEntry, cmPrimarySID=cmPrimarySID, cardTemperature=cardTemperature, cardDplxStatus=cardDplxStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, shelfObjects=shelfObjects, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeFileName=diskVolumeFileName, cmResetReason=cmResetReason, cardTestResult=cardTestResult, errEventId=errEventId, cardSwVersion=cardSwVersion, systemClock=systemClock, diskVolumeDiskSize=diskVolumeDiskSize, auditTotalFailedCount=auditTotalFailedCount, cardTrapInh=cardTrapInh, cardNumPorts=cardNumPorts, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmRegistrationNotification=cmRegistrationNotification, cardCPLDVersion=cardCPLDVersion, auditCycleCount=auditCycleCount, cardTestTransId=cardTestTransId, cmResetInfo=cmResetInfo, cardTestScheduleCommand=cardTestScheduleCommand, cardLastBootVersion=cardLastBootVersion, portMacIfIndex=portMacIfIndex, fpgaErrorEventTable=fpgaErrorEventTable, cardTempNoReportNotification=cardTempNoReportNotification, portPrState=portPrState, cardFwUpdateStatus=cardFwUpdateStatus, portEntry=portEntry, cardSubType=cardSubType, diskVolumeTable=diskVolumeTable, portId=portId, equipmentAudit=equipmentAudit, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMibCompliance=cadEquipmentMibCompliance, trapCounter=trapCounter, portNumber=portNumber, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardShelfId=cardShelfId, cmIpAddress=cmIpAddress, portTxFlowControlMode=portTxFlowControlMode, equipmentError=equipmentError, cardBusSpeed=cardBusSpeed, TestType=TestType, TestCommand=TestCommand, equipmentPortGroup=equipmentPortGroup, auditLogThrottle=auditLogThrottle, cmResetClearNotification=cmResetClearNotification, auditResult=auditResult, auditStatus=auditStatus, cardSecStateChange=cardSecStateChange, portType=portType, portDescription=portDescription, diskVolumeName=diskVolumeName, diskVolumeNumber=diskVolumeNumber, cardSpareGroupMode=cardSpareGroupMode, cardBootVersion=cardBootVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, diskDriveNumber=diskDriveNumber, trapSeverity=trapSeverity, portDetectedMode=portDetectedMode, auditCommand=auditCommand)
