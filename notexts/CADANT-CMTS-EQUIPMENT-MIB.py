...
EqActionType, PortDetectedMode, CardType, ShelfId, PicType, UnknownState, PrimaryState, PortType, PortId, DiskVolumeUsageLevel, AdminState, SecondaryState, FlowControlMode, FirmwareSource, CardSubType, DuplexStatus, PortMode, CardId = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "PortDetectedMode", "CardType", "ShelfId", "PicType", "UnknownState", "PrimaryState", "PortType", "PortId", "DiskVolumeUsageLevel", "AdminState", "SecondaryState", "FlowControlMode", "FirmwareSource", "CardSubType", "DuplexStatus", "PortMode", "CardId")
...
RowStatus, TextualConvention, DisplayString, MacAddress, TimeStamp, DateAndTime, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "DisplayString", "MacAddress", "TimeStamp", "DateAndTime", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestName=cardTestName, cardTestEntry=cardTestEntry, cardTemperature=cardTemperature, portNumber=portNumber, portTrapInh=portTrapInh, auditTotalPassedCount=auditTotalPassedCount, equipmentCardGroup=equipmentCardGroup, cardBootVersion=cardBootVersion, portDetectedMode=portDetectedMode, diskVolumeUsageLevel=diskVolumeUsageLevel, systemGeneralGroup=systemGeneralGroup, portEntry=portEntry, cardId=cardId, TestScheduleCommand=TestScheduleCommand, trapCounter=trapCounter, cmMacAddress=cmMacAddress, cardNumber=cardNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portCardId=portCardId, cardCpuType=cardCpuType, cardSerialNum=cardSerialNum, portBgpId=portBgpId, cardTestTransId=cardTestTransId, diskVolumeGroup=diskVolumeGroup, TestTransactionId=TestTransactionId, auditDescription=auditDescription, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMode=portMode, diskVolumeTable=diskVolumeTable, fpgaErrorEventEntry=fpgaErrorEventEntry, auditCardId=auditCardId, diskVolumeDriveId=diskVolumeDriveId, auditCycleCount=auditCycleCount, cardTrapInh=cardTrapInh, cmIpAddress=cmIpAddress, cardFpgaSource=cardFpgaSource, cardFwVersion=cardFwVersion, cardTestId=cardTestId, cardAdminState=cardAdminState, cardTestResultDesc=cardTestResultDesc, auditTime=auditTime, portPrStateChange=portPrStateChange, TestType=TestType, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierFwVersion=cardCarrierFwVersion, shelfObjects=shelfObjects, cardTable=cardTable, TestResult=TestResult, diskVolumeTrapInh=diskVolumeTrapInh, consoleOutput=consoleOutput, equipmentCardTestGroup=equipmentCardTestGroup, systemClock=systemClock, cardHwVersion=cardHwVersion, cardFlashSize=cardFlashSize, portMacAddress=portMacAddress, cardName=cardName, auditLogOutput=auditLogOutput, portTxFlowControlDetected=portTxFlowControlDetected, cmPrimarySID=cmPrimarySID, cmResetStatus=cmResetStatus, downstreamPowerLoss=downstreamPowerLoss, cardSwVersion=cardSwVersion, cardTestSchedule=cardTestSchedule, cardType=cardType, cardTestTable=cardTestTable, cmVendor=cmVendor, auditResult=auditResult, cardCpuSpeed=cardCpuSpeed, cardTestCommand=cardTestCommand, errEvLoggingEnabled=errEvLoggingEnabled, cardShelfTestIndex=cardShelfTestIndex, cardSecState=cardSecState, cardDplxStatus=cardDplxStatus, equipmentDiag=equipmentDiag, cardSpareGroupId=cardSpareGroupId, TestCommand=TestCommand, TestId=TestId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeUsageNotification=diskVolumeUsageNotification, diagTestId=diagTestId, cardDplxStatusChange=cardDplxStatusChange, cardBusSpeed=cardBusSpeed, diskVolumeFileName=diskVolumeFileName, cardFwUpdateStatus=cardFwUpdateStatus, cardLastChangeTime=cardLastChangeTime, diskVolumeEntry=diskVolumeEntry, shelfName=shelfName, cardSubDetected=cardSubDetected, portDescription=portDescription, cardPicSerialNum=cardPicSerialNum, portTable=portTable, portMacIfIndex=portMacIfIndex, cmResetClearNotification=cmResetClearNotification, cardLastBootSource=cardLastBootSource, portSecState=portSecState, cardTemperatureHighError=cardTemperatureHighError, auditLogThrottle=auditLogThrottle, portTxFlowControlMode=portTxFlowControlMode, auditPassedCount=auditPassedCount, cardTestResultNotification=cardTestResultNotification, cmUChannelID=cmUChannelID, cmResetNotification=cmResetNotification, portShelfId=portShelfId, portCardSubType=portCardSubType, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCarrierSerialNum=cardCarrierSerialNum, cardCarrierHwVersion=cardCarrierHwVersion, portType=portType, diskDriveNumber=diskDriveNumber, auditTable=auditTable, equipmentPortGroup=equipmentPortGroup, portLastChangeTime=portLastChangeTime, cmResetReason=cmResetReason, cardSecStateChange=cardSecStateChange, equipmentTraps=equipmentTraps, cardPicHwVersion=cardPicHwVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSpareGroupMode=cardSpareGroupMode, auditAutoScheduling=auditAutoScheduling, auditTotalFailedCount=auditTotalFailedCount, portId=portId, errEventId=errEventId, diskVolumeDiskSize=diskVolumeDiskSize, cardTestDescription=cardTestDescription, cmResetUpTime=cmResetUpTime, portSecStateChange=portSecStateChange, equipmentTbl=equipmentTbl, errEvRowStatus=errEvRowStatus, auditShelfId=auditShelfId, diskVolumeCardId=diskVolumeCardId, portDplxStatus=portDplxStatus, portDplxStatusChange=portDplxStatusChange, cmDevice=cmDevice, verboseLevel=verboseLevel, cardTempOverHeatNotification=cardTempOverHeatNotification, remainInDiagMode=remainInDiagMode, systemKey=systemKey, cardLastBootVersion=cardLastBootVersion, portCurrDsPower=portCurrDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentState=equipmentState, cmRegistrationNotification=cmRegistrationNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfSwVersion=shelfSwVersion, portPrState=portPrState, cardAction=cardAction, cadEquipmentMib=cadEquipmentMib, cardEntry=cardEntry, errEvLogLevel=errEvLogLevel, cardLastResetReason=cardLastResetReason, cardTestType=cardTestType, cardRamSize=cardRamSize, eqDiagConfig=eqDiagConfig, auditName=auditName, cardUpTime=cardUpTime, TestSchedule=TestSchedule, auditId=auditId, cmResetInfo=cmResetInfo, cardDetectedChange=cardDetectedChange, equipmentAuditGroup=equipmentAuditGroup, portNumChans=portNumChans, shelfNumber=shelfNumber, auditFailedCount=auditFailedCount, cardTestIndex=cardTestIndex, equipmentError=equipmentError, cardCPLDVersion=cardCPLDVersion, trapSeverity=trapSeverity, portRxFlowControlMode=portRxFlowControlMode, systemGeneral=systemGeneral, portAction=portAction, diskVolumeSize=diskVolumeSize, auditEntry=auditEntry, cardShelfId=cardShelfId, equipmentStateGroup=equipmentStateGroup, equipmentAudit=equipmentAudit, cardPicDetected=cardPicDetected, auditCommand=auditCommand, PYSNMP_MODULE_ID=cadEquipmentMib, portConnectorId=portConnectorId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portRxFlowControlDetected=portRxFlowControlDetected, cardDetected=cardDetected, cardTestScheduleCommand=cardTestScheduleCommand, cardTestTime=cardTestTime, cardPrState=cardPrState, portMinDsPower=portMinDsPower, diskVolumeId=diskVolumeId, cardTestResult=cardTestResult, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeName=diskVolumeName, equipmentDiagGroup=equipmentDiagGroup, portAdminState=portAdminState, diskVolumeShelfId=diskVolumeShelfId, auditStatus=auditStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, cardNumPorts=cardNumPorts, diskVolumeNumber=diskVolumeNumber, cardSubType=cardSubType, equipmentShelfGroup=equipmentShelfGroup, portMaxDsPower=portMaxDsPower, cardTemperatureHighWarn=cardTemperatureHighWarn, portDocsIfIndex=portDocsIfIndex, cardPrStateChange=cardPrStateChange)
