...
ShelfId, CardSubType, DuplexStatus, PrimaryState, UnknownState, PicType, PortType, DiskVolumeUsageLevel, EqActionType, PortMode, SecondaryState, PortDetectedMode, PortId, CardId, CardType, FirmwareSource, AdminState, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "CardSubType", "DuplexStatus", "PrimaryState", "UnknownState", "PicType", "PortType", "DiskVolumeUsageLevel", "EqActionType", "PortMode", "SecondaryState", "PortDetectedMode", "PortId", "CardId", "CardType", "FirmwareSource", "AdminState", "FlowControlMode")
...
TruthValue, MacAddress, DateAndTime, TextualConvention, RowStatus, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "MacAddress", "DateAndTime", "TextualConvention", "RowStatus", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", trapCounter=trapCounter, portDplxStatus=portDplxStatus, diskVolumeId=diskVolumeId, TestScheduleCommand=TestScheduleCommand, systemGeneral=systemGeneral, cardEntry=cardEntry, verboseLevel=verboseLevel, cardTestSchedule=cardTestSchedule, auditAutoScheduling=auditAutoScheduling, portConnectorId=portConnectorId, cmIpAddress=cmIpAddress, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portNumber=portNumber, cardAction=cardAction, diskVolumeSize=diskVolumeSize, cardTestId=cardTestId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, TestCommand=TestCommand, cardFpgaSource=cardFpgaSource, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageNotification=diskVolumeUsageNotification, portMinDsPower=portMinDsPower, cardSerialNum=cardSerialNum, cardName=cardName, cmResetClearNotification=cmResetClearNotification, portPrStateChange=portPrStateChange, portLastChangeTime=portLastChangeTime, cardBusSpeed=cardBusSpeed, eqDiagConfig=eqDiagConfig, diskVolumeGroup=diskVolumeGroup, portAction=portAction, cardId=cardId, TestTransactionId=TestTransactionId, cardPrState=cardPrState, equipmentDiag=equipmentDiag, cardSwVersion=cardSwVersion, auditCommand=auditCommand, cadEquipmentMib=cadEquipmentMib, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portBgpId=portBgpId, diskVolumeDriveId=diskVolumeDriveId, portSecState=portSecState, equipmentAuditGroup=equipmentAuditGroup, systemClock=systemClock, cardShelfTestIndex=cardShelfTestIndex, cardDplxStatusChange=cardDplxStatusChange, errEvLoggingEnabled=errEvLoggingEnabled, cmUChannelID=cmUChannelID, cardLastBootVersion=cardLastBootVersion, portNumChans=portNumChans, cardSubType=cardSubType, cardSpareGroupMode=cardSpareGroupMode, cardPrStateChange=cardPrStateChange, consoleOutput=consoleOutput, cardSubDetected=cardSubDetected, auditId=auditId, portMacIfIndex=portMacIfIndex, diskVolumeFileName=diskVolumeFileName, cmDevice=cmDevice, portEntry=portEntry, diskVolumeTrapInh=diskVolumeTrapInh, portRxFlowControlMode=portRxFlowControlMode, errEventId=errEventId, auditCardId=auditCardId, equipmentPortGroup=equipmentPortGroup, TestType=TestType, cardTestIndex=cardTestIndex, portId=portId, cardTestScheduleCommand=cardTestScheduleCommand, auditTotalFailedCount=auditTotalFailedCount, cmResetReason=cmResetReason, portTrapInh=portTrapInh, diskVolumeDiskSize=diskVolumeDiskSize, cardDetectedChange=cardDetectedChange, equipmentStateGroup=equipmentStateGroup, auditTime=auditTime, diskVolumeEntry=diskVolumeEntry, cmMacAddress=cmMacAddress, equipmentError=equipmentError, cardTestResultNotification=cardTestResultNotification, cardType=cardType, cardBootVersion=cardBootVersion, shelfObjects=shelfObjects, diskVolumeCardId=diskVolumeCardId, cardCPLDVersion=cardCPLDVersion, auditStatus=auditStatus, cardCpuSpeed=cardCpuSpeed, portCardSubType=portCardSubType, equipmentAudit=equipmentAudit, cardSpareGroupId=cardSpareGroupId, cardTestTable=cardTestTable, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCarrierFwVersion=cardCarrierFwVersion, cardTestResultDesc=cardTestResultDesc, cmResetUpTime=cmResetUpTime, portDetectedMode=portDetectedMode, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentTbl=equipmentTbl, portCurrDsPower=portCurrDsPower, equipmentShelfGroup=equipmentShelfGroup, cardTestResult=cardTestResult, diskVolumeName=diskVolumeName, portPrState=portPrState, cardTemperatureHighError=cardTemperatureHighError, cardRamSize=cardRamSize, cardTrapInh=cardTrapInh, TestId=TestId, cmResetNotification=cmResetNotification, trapSeverity=trapSeverity, auditName=auditName, diskVolumeShelfId=diskVolumeShelfId, remainInDiagMode=remainInDiagMode, cardTempOverHeatNotification=cardTempOverHeatNotification, portTable=portTable, cardTestTransId=cardTestTransId, cardTemperature=cardTemperature, portAdminState=portAdminState, cadEquipmentMibGroup=cadEquipmentMibGroup, portDescription=portDescription, cmRegistrationNotification=cmRegistrationNotification, portDocsIfIndex=portDocsIfIndex, cardSecState=cardSecState, PYSNMP_MODULE_ID=cadEquipmentMib, cardTable=cardTable, cardTestName=cardTestName, auditLogThrottle=auditLogThrottle, cardFlashSize=cardFlashSize, errEvRowStatus=errEvRowStatus, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardNumber=cardNumber, equipmentState=equipmentState, cardAdminState=cardAdminState, auditDescription=auditDescription, portRxFlowControlDetected=portRxFlowControlDetected, errEvLogLevel=errEvLogLevel, diagTestId=diagTestId, portTxFlowControlMode=portTxFlowControlMode, auditLogOutput=auditLogOutput, portType=portType, cardFwUpdateStatus=cardFwUpdateStatus, cardTestTime=cardTestTime, cardPicHwVersion=cardPicHwVersion, cardNumPorts=cardNumPorts, cardLastBootSource=cardLastBootSource, auditTable=auditTable, cardCpuType=cardCpuType, auditPassedCount=auditPassedCount, cardLastResetReason=cardLastResetReason, cardDplxStatus=cardDplxStatus, portSecStateChange=portSecStateChange, shelfName=shelfName, cmPrimarySID=cmPrimarySID, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFwVersion=cardFwVersion, cardPicDetected=cardPicDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, TestSchedule=TestSchedule, auditEntry=auditEntry, cmResetInfo=cmResetInfo, auditCycleCount=auditCycleCount, TestResult=TestResult, shelfSwVersion=shelfSwVersion, equipmentCardGroup=equipmentCardGroup, diskVolumeNumber=diskVolumeNumber, cardLastChangeTime=cardLastChangeTime, downstreamPowerLoss=downstreamPowerLoss, systemGeneralGroup=systemGeneralGroup, cmVendor=cmVendor, cardTestCommand=cardTestCommand, auditShelfId=auditShelfId, auditTotalPassedCount=auditTotalPassedCount, equipmentTraps=equipmentTraps, portShelfId=portShelfId, equipmentDiagGroup=equipmentDiagGroup, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageLevel=diskVolumeUsageLevel, auditFailedCount=auditFailedCount, cardCarrierSerialNum=cardCarrierSerialNum, cardTestType=cardTestType, cardTestEntry=cardTestEntry, cardHwVersion=cardHwVersion, cmResetStatus=cmResetStatus, shelfNumber=shelfNumber, fpgaErrorEventTable=fpgaErrorEventTable, diskDriveNumber=diskDriveNumber, auditResult=auditResult, portMode=portMode, systemKey=systemKey, cardTempNoReportNotification=cardTempNoReportNotification, cardShelfId=cardShelfId, cardDetected=cardDetected, diskVolumeTable=diskVolumeTable, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSecStateChange=cardSecStateChange, portMaxDsPower=portMaxDsPower, cardUpTime=cardUpTime, cardTestDescription=cardTestDescription, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portTxFlowControlDetected=portTxFlowControlDetected, cardCarrierHwVersion=cardCarrierHwVersion, portCardId=portCardId, portMacAddress=portMacAddress)
