...
PortId, FlowControlMode, PortDetectedMode, DuplexStatus, PicType, PortType, EqActionType, SecondaryState, UnknownState, CardSubType, CardType, CardId, DiskVolumeUsageLevel, PortMode, ShelfId, FirmwareSource, PrimaryState, AdminState = mibBuilder.importSymbols("CADANT-TC", "PortId", "FlowControlMode", "PortDetectedMode", "DuplexStatus", "PicType", "PortType", "EqActionType", "SecondaryState", "UnknownState", "CardSubType", "CardType", "CardId", "DiskVolumeUsageLevel", "PortMode", "ShelfId", "FirmwareSource", "PrimaryState", "AdminState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portRxFlowControlDetected=portRxFlowControlDetected, portDplxStatusChange=portDplxStatusChange, cardSpareGroupMode=cardSpareGroupMode, equipmentStateGroup=equipmentStateGroup, cardUpTime=cardUpTime, portEntry=portEntry, cardDplxStatusChange=cardDplxStatusChange, cardTestDescription=cardTestDescription, auditPassedCount=auditPassedCount, auditAutoScheduling=auditAutoScheduling, cmVendor=cmVendor, cardTestSchedule=cardTestSchedule, errEvLogLevel=errEvLogLevel, TestScheduleCommand=TestScheduleCommand, cmResetClearNotification=cmResetClearNotification, shelfSwVersion=shelfSwVersion, cardTestId=cardTestId, portSecStateChange=portSecStateChange, cmResetStatus=cmResetStatus, errEvLoggingEnabled=errEvLoggingEnabled, cardCarrierSerialNum=cardCarrierSerialNum, cardCPLDVersion=cardCPLDVersion, portId=portId, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeTable=diskVolumeTable, TestId=TestId, TestSchedule=TestSchedule, portMacIfIndex=portMacIfIndex, cardNumPorts=cardNumPorts, auditTable=auditTable, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditCardId=auditCardId, portCurrDsPower=portCurrDsPower, cardTestTime=cardTestTime, portMacAddress=portMacAddress, diskVolumeCardId=diskVolumeCardId, portCardSubType=portCardSubType, diskVolumeSize=diskVolumeSize, cardTestResultDesc=cardTestResultDesc, cmMacAddress=cmMacAddress, errEvRowStatus=errEvRowStatus, TestTransactionId=TestTransactionId, cadEquipmentMib=cadEquipmentMib, equipmentTbl=equipmentTbl, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditLogOutput=auditLogOutput, portDocsIfIndex=portDocsIfIndex, diskVolumeId=diskVolumeId, PYSNMP_MODULE_ID=cadEquipmentMib, cmIpAddress=cmIpAddress, portPrState=portPrState, diagTestId=diagTestId, TestType=TestType, cardTestResult=cardTestResult, cardSpareGroupId=cardSpareGroupId, portSecState=portSecState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardNumber=cardNumber, cardTempNoReportNotification=cardTempNoReportNotification, cmRegistrationNotification=cmRegistrationNotification, cardType=cardType, cardAction=cardAction, portTxFlowControlDetected=portTxFlowControlDetected, portCardId=portCardId, cardSwVersion=cardSwVersion, equipmentDiagGroup=equipmentDiagGroup, portDplxStatus=portDplxStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardAdminState=cardAdminState, cmUChannelID=cmUChannelID, equipmentCardTestGroup=equipmentCardTestGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardLastBootVersion=cardLastBootVersion, auditTotalFailedCount=auditTotalFailedCount, auditCycleCount=auditCycleCount, portDetectedMode=portDetectedMode, equipmentPortGroup=equipmentPortGroup, cardSubDetected=cardSubDetected, cardLastChangeTime=cardLastChangeTime, cardShelfId=cardShelfId, equipmentDiag=equipmentDiag, auditLogThrottle=auditLogThrottle, cardBootVersion=cardBootVersion, portMode=portMode, fpgaErrorEventTable=fpgaErrorEventTable, cardTrapInh=cardTrapInh, cardTestName=cardTestName, cardFpgaSource=cardFpgaSource, shelfName=shelfName, cardSecState=cardSecState, cardName=cardName, diskVolumeDriveId=diskVolumeDriveId, cmResetUpTime=cmResetUpTime, equipmentAudit=equipmentAudit, portNumber=portNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, trapSeverity=trapSeverity, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeNumber=diskVolumeNumber, cardCpuType=cardCpuType, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierFwVersion=cardCarrierFwVersion, cardTestResultNotification=cardTestResultNotification, cardPrState=cardPrState, diskVolumeGroup=diskVolumeGroup, cardId=cardId, systemKey=systemKey, portLastChangeTime=portLastChangeTime, cardPicHwVersion=cardPicHwVersion, eqDiagConfig=eqDiagConfig, errEvRecoveryEnabled=errEvRecoveryEnabled, auditFailedCount=auditFailedCount, cardCarrierHwVersion=cardCarrierHwVersion, trapCounter=trapCounter, cardFwVersion=cardFwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastResetReason=cardLastResetReason, portType=portType, cardCpuSpeed=cardCpuSpeed, cadEquipmentMibGroup=cadEquipmentMibGroup, portPrStateChange=portPrStateChange, cmPrimarySID=cmPrimarySID, equipmentError=equipmentError, cardSubType=cardSubType, portTable=portTable, auditCommand=auditCommand, equipmentTraps=equipmentTraps, cardTemperature=cardTemperature, consoleOutput=consoleOutput, shelfNumber=shelfNumber, portMinDsPower=portMinDsPower, diskVolumeTrapInh=diskVolumeTrapInh, portAdminState=portAdminState, diskVolumeEntry=diskVolumeEntry, cardShelfTestIndex=cardShelfTestIndex, cmDevice=cmDevice, auditStatus=auditStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetNotification=cmResetNotification, cardLastBootSource=cardLastBootSource, systemClock=systemClock, cardTestEntry=cardTestEntry, cmResetInfo=cmResetInfo, auditName=auditName, portBgpId=portBgpId, diskVolumeDiskSize=diskVolumeDiskSize, portConnectorId=portConnectorId, cardTestTransId=cardTestTransId, cardDplxStatus=cardDplxStatus, cardBusSpeed=cardBusSpeed, cardPrStateChange=cardPrStateChange, diskVolumeUsageLevel=diskVolumeUsageLevel, portTxFlowControlMode=portTxFlowControlMode, errEventId=errEventId, portTrapInh=portTrapInh, auditDescription=auditDescription, equipmentCardGroup=equipmentCardGroup, auditEntry=auditEntry, verboseLevel=verboseLevel, equipmentShelfGroup=equipmentShelfGroup, cardTestIndex=cardTestIndex, auditShelfId=auditShelfId, systemGeneralGroup=systemGeneralGroup, cardTestScheduleCommand=cardTestScheduleCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeShelfId=diskVolumeShelfId, cardTemperatureHighError=cardTemperatureHighError, equipmentState=equipmentState, cardTestType=cardTestType, portShelfId=portShelfId, shelfObjects=shelfObjects, cardRamSize=cardRamSize, downstreamPowerLoss=downstreamPowerLoss, cardTestCommand=cardTestCommand, cardPicDetected=cardPicDetected, auditResult=auditResult, portMaxDsPower=portMaxDsPower, portAction=portAction, TestResult=TestResult, cardHwVersion=cardHwVersion, portNumChans=portNumChans, diskVolumeName=diskVolumeName, cardDetectedChange=cardDetectedChange, TestCommand=TestCommand, cardTable=cardTable, cmResetReason=cmResetReason, auditId=auditId, remainInDiagMode=remainInDiagMode, equipmentAuditGroup=equipmentAuditGroup, auditTime=auditTime, portRxFlowControlMode=portRxFlowControlMode, cardPicSerialNum=cardPicSerialNum, cardFlashSize=cardFlashSize, diskVolumeFileName=diskVolumeFileName, cardSecStateChange=cardSecStateChange, portDescription=portDescription, diskDriveNumber=diskDriveNumber, cardSerialNum=cardSerialNum, cardTestTable=cardTestTable, systemGeneral=systemGeneral, auditTotalPassedCount=auditTotalPassedCount, cardEntry=cardEntry, cardDetected=cardDetected)
