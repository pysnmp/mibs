...
DuplexStatus, DiskVolumeUsageLevel, PortMode, FirmwareSource, PortDetectedMode, PrimaryState, SecondaryState, AdminState, PortId, ShelfId, UnknownState, EqActionType, FlowControlMode, PicType, PortType, CardType, CardSubType, CardId = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "DiskVolumeUsageLevel", "PortMode", "FirmwareSource", "PortDetectedMode", "PrimaryState", "SecondaryState", "AdminState", "PortId", "ShelfId", "UnknownState", "EqActionType", "FlowControlMode", "PicType", "PortType", "CardType", "CardSubType", "CardId")
...
MacAddress, DateAndTime, RowStatus, TextualConvention, DisplayString, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "DateAndTime", "RowStatus", "TextualConvention", "DisplayString", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentAudit=equipmentAudit, auditDescription=auditDescription, cardTemperature=cardTemperature, cardCPLDVersion=cardCPLDVersion, portNumChans=portNumChans, equipmentDiag=equipmentDiag, cardTestTime=cardTestTime, auditStatus=auditStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastBootSource=cardLastBootSource, portMacIfIndex=portMacIfIndex, auditTime=auditTime, cardLastChangeTime=cardLastChangeTime, cardCarrierFwVersion=cardCarrierFwVersion, portMaxDsPower=portMaxDsPower, errEvLogLevel=errEvLogLevel, diskVolumeTrapInh=diskVolumeTrapInh, cardCarrierHwVersion=cardCarrierHwVersion, equipmentError=equipmentError, cardBootVersion=cardBootVersion, portAction=portAction, portDetectedMode=portDetectedMode, portMinDsPower=portMinDsPower, cardTestName=cardTestName, TestTransactionId=TestTransactionId, cardType=cardType, cardNumber=cardNumber, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeGroup=diskVolumeGroup, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardEntry=cardEntry, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeName=diskVolumeName, cardUpTime=cardUpTime, auditCycleCount=auditCycleCount, equipmentPortGroup=equipmentPortGroup, equipmentShelfGroup=equipmentShelfGroup, cardTestCommand=cardTestCommand, TestCommand=TestCommand, cardTestResultNotification=cardTestResultNotification, equipmentAuditGroup=equipmentAuditGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, portSecState=portSecState, cardTestScheduleCommand=cardTestScheduleCommand, portId=portId, consoleOutput=consoleOutput, cardTestSchedule=cardTestSchedule, diskVolumeEntry=diskVolumeEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestEntry=cardTestEntry, cmResetNotification=cmResetNotification, cardDetectedChange=cardDetectedChange, cardSubType=cardSubType, portDplxStatusChange=portDplxStatusChange, cmResetClearNotification=cmResetClearNotification, systemGeneral=systemGeneral, cardTempOverHeatNotification=cardTempOverHeatNotification, diskDriveNumber=diskDriveNumber, diagTestId=diagTestId, downstreamPowerLoss=downstreamPowerLoss, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSecStateChange=cardSecStateChange, eqDiagConfig=eqDiagConfig, cardPrStateChange=cardPrStateChange, portConnectorId=portConnectorId, cardLastResetReason=cardLastResetReason, systemKey=systemKey, auditCommand=auditCommand, diskVolumeCardId=diskVolumeCardId, TestScheduleCommand=TestScheduleCommand, trapCounter=trapCounter, portLastChangeTime=portLastChangeTime, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTotalPassedCount=auditTotalPassedCount, portDescription=portDescription, cardAction=cardAction, auditShelfId=auditShelfId, cardCpuType=cardCpuType, cardTable=cardTable, auditEntry=auditEntry, cardFlashSize=cardFlashSize, cardPicDetected=cardPicDetected, cmUChannelID=cmUChannelID, portEntry=portEntry, diskVolumeDiskSize=diskVolumeDiskSize, cardDplxStatusChange=cardDplxStatusChange, cardPrState=cardPrState, cardTemperatureHighError=cardTemperatureHighError, portPrState=portPrState, cardShelfTestIndex=cardShelfTestIndex, cardTempNoReportNotification=cardTempNoReportNotification, errEvRowStatus=errEvRowStatus, portBgpId=portBgpId, cardSecState=cardSecState, portTrapInh=portTrapInh, diskVolumeNumber=diskVolumeNumber, cmResetStatus=cmResetStatus, auditResult=auditResult, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeFileName=diskVolumeFileName, cardTestTable=cardTestTable, cardSerialNum=cardSerialNum, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestIndex=cardTestIndex, auditLogThrottle=auditLogThrottle, TestSchedule=TestSchedule, cardAdminState=cardAdminState, errEvRecoveryEnabled=errEvRecoveryEnabled, errEvLoggingEnabled=errEvLoggingEnabled, auditTotalFailedCount=auditTotalFailedCount, cardDetected=cardDetected, cardTestDescription=cardTestDescription, portShelfId=portShelfId, cardTrapInh=cardTrapInh, auditLogOutput=auditLogOutput, equipmentStateGroup=equipmentStateGroup, equipmentDiagGroup=equipmentDiagGroup, auditFailedCount=auditFailedCount, equipmentCardTestGroup=equipmentCardTestGroup, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestType=cardTestType, cadEquipmentMib=cadEquipmentMib, cmResetInfo=cmResetInfo, portMacAddress=portMacAddress, cmMacAddress=cmMacAddress, auditCardId=auditCardId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portCardId=portCardId, portRxFlowControlMode=portRxFlowControlMode, equipmentState=equipmentState, equipmentTraps=equipmentTraps, shelfNumber=shelfNumber, remainInDiagMode=remainInDiagMode, TestId=TestId, cardNumPorts=cardNumPorts, portTable=portTable, shelfObjects=shelfObjects, cardFwVersion=cardFwVersion, diskVolumeSize=diskVolumeSize, cmVendor=cmVendor, diskVolumeTable=diskVolumeTable, cardTestResultDesc=cardTestResultDesc, portDplxStatus=portDplxStatus, cardDplxStatus=cardDplxStatus, systemClock=systemClock, fpgaErrorEventTable=fpgaErrorEventTable, portCardSubType=portCardSubType, TestResult=TestResult, cardName=cardName, portType=portType, cmResetUpTime=cmResetUpTime, auditId=auditId, diskVolumeShelfId=diskVolumeShelfId, portRxFlowControlDetected=portRxFlowControlDetected, cardSubDetected=cardSubDetected, equipmentCardGroup=equipmentCardGroup, errEventId=errEventId, portCurrDsPower=portCurrDsPower, cmIpAddress=cmIpAddress, portMode=portMode, cardShelfId=cardShelfId, portSecStateChange=portSecStateChange, cmPrimarySID=cmPrimarySID, cmResetReason=cmResetReason, cardCarrierSerialNum=cardCarrierSerialNum, cardHwVersion=cardHwVersion, auditName=auditName, diskVolumeDriveId=diskVolumeDriveId, cmRegistrationNotification=cmRegistrationNotification, TestType=TestType, portAdminState=portAdminState, cardTestResult=cardTestResult, diskVolumeUsageLevel=diskVolumeUsageLevel, trapSeverity=trapSeverity, portNumber=portNumber, cardSwVersion=cardSwVersion, cardRamSize=cardRamSize, portDocsIfIndex=portDocsIfIndex, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, shelfName=shelfName, cmDevice=cmDevice, cardBusSpeed=cardBusSpeed, equipmentTbl=equipmentTbl, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicHwVersion=cardPicHwVersion, cardTestId=cardTestId, shelfSwVersion=shelfSwVersion, verboseLevel=verboseLevel, cardLastBootVersion=cardLastBootVersion, cardFpgaSource=cardFpgaSource, auditPassedCount=auditPassedCount, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardId=cardId, diskVolumeId=diskVolumeId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPicSerialNum=cardPicSerialNum, cardTestTransId=cardTestTransId, portPrStateChange=portPrStateChange, auditAutoScheduling=auditAutoScheduling, systemGeneralGroup=systemGeneralGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, cardCpuSpeed=cardCpuSpeed, auditTable=auditTable)
