...
CardId, ShelfId, SecondaryState, DuplexStatus, FlowControlMode, PortDetectedMode, PortId, PrimaryState, EqActionType, FirmwareSource, DiskVolumeUsageLevel, AdminState, PicType, UnknownState, CardSubType, CardType, PortType, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardId", "ShelfId", "SecondaryState", "DuplexStatus", "FlowControlMode", "PortDetectedMode", "PortId", "PrimaryState", "EqActionType", "FirmwareSource", "DiskVolumeUsageLevel", "AdminState", "PicType", "UnknownState", "CardSubType", "CardType", "PortType", "PortMode")
...
MacAddress, RowStatus, DisplayString, TruthValue, DateAndTime, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "DisplayString", "TruthValue", "DateAndTime", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portCardSubType=portCardSubType, auditCardId=auditCardId, trapSeverity=trapSeverity, cardPicDetected=cardPicDetected, cmVendor=cmVendor, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSecStateChange=cardSecStateChange, auditLogThrottle=auditLogThrottle, portNumber=portNumber, portDocsIfIndex=portDocsIfIndex, cardTestName=cardTestName, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTemperatureHighError=cardTemperatureHighError, cardTrapInh=cardTrapInh, auditAutoScheduling=auditAutoScheduling, cmResetNotification=cmResetNotification, equipmentShelfGroup=equipmentShelfGroup, cardLastChangeTime=cardLastChangeTime, portDetectedMode=portDetectedMode, portMode=portMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardPicHwVersion=cardPicHwVersion, diskVolumeTable=diskVolumeTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditEntry=auditEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, auditTable=auditTable, cardCarrierFwVersion=cardCarrierFwVersion, equipmentDiagGroup=equipmentDiagGroup, cardNumber=cardNumber, portTrapInh=portTrapInh, cardTempNoReportNotification=cardTempNoReportNotification, portMinDsPower=portMinDsPower, diskVolumeDriveId=diskVolumeDriveId, errEventId=errEventId, cardSpareGroupId=cardSpareGroupId, errEvLogLevel=errEvLogLevel, shelfSwVersion=shelfSwVersion, cardId=cardId, equipmentAudit=equipmentAudit, cmResetReason=cmResetReason, diskVolumeNumber=diskVolumeNumber, cmResetClearNotification=cmResetClearNotification, diskVolumeDiskSize=diskVolumeDiskSize, portLastChangeTime=portLastChangeTime, auditResult=auditResult, diskVolumeShelfId=diskVolumeShelfId, portMaxDsPower=portMaxDsPower, auditPassedCount=auditPassedCount, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, portBgpId=portBgpId, cardTestType=cardTestType, diskVolumeSize=diskVolumeSize, cmResetUpTime=cmResetUpTime, cardFlashSize=cardFlashSize, cardEntry=cardEntry, cardSerialNum=cardSerialNum, equipmentError=equipmentError, systemClock=systemClock, cardHwVersion=cardHwVersion, equipmentCardGroup=equipmentCardGroup, errEvLoggingEnabled=errEvLoggingEnabled, systemKey=systemKey, cardBootVersion=cardBootVersion, portMacIfIndex=portMacIfIndex, eqDiagConfig=eqDiagConfig, cardTestDescription=cardTestDescription, portDplxStatus=portDplxStatus, cardLastBootVersion=cardLastBootVersion, diskVolumeEntry=diskVolumeEntry, cmRegistrationNotification=cmRegistrationNotification, equipmentPortGroup=equipmentPortGroup, TestResult=TestResult, portDescription=portDescription, auditName=auditName, cardPicSerialNum=cardPicSerialNum, cardFpgaSource=cardFpgaSource, cardLastBootSource=cardLastBootSource, cardTestCommand=cardTestCommand, cardTestTime=cardTestTime, cardNumPorts=cardNumPorts, cmIpAddress=cmIpAddress, cardCpuSpeed=cardCpuSpeed, cardTestScheduleCommand=cardTestScheduleCommand, remainInDiagMode=remainInDiagMode, cardSubDetected=cardSubDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditCycleCount=auditCycleCount, auditDescription=auditDescription, equipmentDiag=equipmentDiag, cardCpuType=cardCpuType, cardSubType=cardSubType, cardTemperature=cardTemperature, cardFwVersion=cardFwVersion, cardFwUpdateStatus=cardFwUpdateStatus, cardTestTransId=cardTestTransId, cardTemperatureHighWarn=cardTemperatureHighWarn, TestSchedule=TestSchedule, diskVolumeFileName=diskVolumeFileName, equipmentTbl=equipmentTbl, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portId=portId, TestScheduleCommand=TestScheduleCommand, PYSNMP_MODULE_ID=cadEquipmentMib, shelfObjects=shelfObjects, cardLastResetReason=cardLastResetReason, cmDevice=cmDevice, cardRamSize=cardRamSize, cardUpTime=cardUpTime, cardTestResultNotification=cardTestResultNotification, cardName=cardName, diskVolumeCardId=diskVolumeCardId, cardTempOverHeatNotification=cardTempOverHeatNotification, portNumChans=portNumChans, cardDetectedChange=cardDetectedChange, portRxFlowControlDetected=portRxFlowControlDetected, portTxFlowControlDetected=portTxFlowControlDetected, cardAction=cardAction, TestType=TestType, auditCommand=auditCommand, cardCarrierHwVersion=cardCarrierHwVersion, cmMacAddress=cmMacAddress, diskDriveNumber=diskDriveNumber, errEvRowStatus=errEvRowStatus, cardShelfId=cardShelfId, portSecState=portSecState, auditLogOutput=auditLogOutput, fpgaErrorEventTable=fpgaErrorEventTable, cardTestResultDesc=cardTestResultDesc, cardPrState=cardPrState, cardTestResult=cardTestResult, equipmentTraps=equipmentTraps, portEntry=portEntry, cardShelfTestIndex=cardShelfTestIndex, cardCarrierSerialNum=cardCarrierSerialNum, auditId=auditId, cardTestEntry=cardTestEntry, cmResetStatus=cmResetStatus, cardTestId=cardTestId, cardBusSpeed=cardBusSpeed, auditTotalFailedCount=auditTotalFailedCount, consoleOutput=consoleOutput, auditFailedCount=auditFailedCount, cardDplxStatusChange=cardDplxStatusChange, portShelfId=portShelfId, auditShelfId=auditShelfId, auditTime=auditTime, portTable=portTable, cadEquipmentMib=cadEquipmentMib, portMacAddress=portMacAddress, systemGeneralGroup=systemGeneralGroup, cardTestSchedule=cardTestSchedule, cardSpareGroupMode=cardSpareGroupMode, auditStatus=auditStatus, systemGeneral=systemGeneral, portPrState=portPrState, portCurrDsPower=portCurrDsPower, cadEquipmentMibConformance=cadEquipmentMibConformance, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestIndex=cardTestIndex, shelfName=shelfName, cardSecState=cardSecState, diagTestId=diagTestId, portType=portType, cmUChannelID=cmUChannelID, portTxFlowControlMode=portTxFlowControlMode, diskVolumeName=diskVolumeName, downstreamPowerLoss=downstreamPowerLoss, fpgaErrorEventGroup=fpgaErrorEventGroup, portCardId=portCardId, verboseLevel=verboseLevel, portPrStateChange=portPrStateChange, shelfNumber=shelfNumber, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeGroup=diskVolumeGroup, TestTransactionId=TestTransactionId, cardTestTable=cardTestTable, diskVolumeId=diskVolumeId, TestCommand=TestCommand, cardDplxStatus=cardDplxStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestId=TestId, cardPrStateChange=cardPrStateChange, cardAdminState=cardAdminState, cardTable=cardTable, equipmentStateGroup=equipmentStateGroup, portAdminState=portAdminState, equipmentCardTestGroup=equipmentCardTestGroup, cardCPLDVersion=cardCPLDVersion, portRxFlowControlMode=portRxFlowControlMode, cardDetected=cardDetected, portSecStateChange=portSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentAuditGroup=equipmentAuditGroup, portConnectorId=portConnectorId, trapCounter=trapCounter, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTotalPassedCount=auditTotalPassedCount, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardType=cardType, portAction=portAction, cmPrimarySID=cmPrimarySID, cmResetInfo=cmResetInfo, equipmentState=equipmentState, cardSwVersion=cardSwVersion)
