...
PrimaryState, UnknownState, EqActionType, PortId, SecondaryState, PortType, AdminState, PortDetectedMode, DiskVolumeUsageLevel, FirmwareSource, FlowControlMode, PicType, ShelfId, CardType, CardId, CardSubType, PortMode, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "UnknownState", "EqActionType", "PortId", "SecondaryState", "PortType", "AdminState", "PortDetectedMode", "DiskVolumeUsageLevel", "FirmwareSource", "FlowControlMode", "PicType", "ShelfId", "CardType", "CardId", "CardSubType", "PortMode", "DuplexStatus")
...
DisplayString, TimeStamp, RowStatus, MacAddress, DateAndTime, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "RowStatus", "MacAddress", "DateAndTime", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageNotification=diskVolumeUsageNotification, portCurrDsPower=portCurrDsPower, TestType=TestType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastBootVersion=cardLastBootVersion, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetInfo=cmResetInfo, fpgaErrorEventTable=fpgaErrorEventTable, cardDetectedChange=cardDetectedChange, equipmentStateGroup=equipmentStateGroup, cardPicSerialNum=cardPicSerialNum, cardFwVersion=cardFwVersion, cardTemperature=cardTemperature, auditTotalPassedCount=auditTotalPassedCount, cardBusSpeed=cardBusSpeed, portTxFlowControlDetected=portTxFlowControlDetected, equipmentDiagGroup=equipmentDiagGroup, TestId=TestId, diskVolumeDiskSize=diskVolumeDiskSize, equipmentPortGroup=equipmentPortGroup, portBgpId=portBgpId, cardLastBootSource=cardLastBootSource, portAction=portAction, equipmentCardGroup=equipmentCardGroup, cardEntry=cardEntry, portMacAddress=portMacAddress, diskVolumeId=diskVolumeId, shelfObjects=shelfObjects, cardTestResult=cardTestResult, auditName=auditName, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSerialNum=cardSerialNum, cmPrimarySID=cmPrimarySID, cardUpTime=cardUpTime, systemKey=systemKey, auditCommand=auditCommand, portMaxDsPower=portMaxDsPower, TestTransactionId=TestTransactionId, diskVolumeNumber=diskVolumeNumber, cardHwVersion=cardHwVersion, diskVolumeName=diskVolumeName, cardFpgaSource=cardFpgaSource, cardLastResetReason=cardLastResetReason, cardTempOverHeatNotification=cardTempOverHeatNotification, portTable=portTable, trapCounter=trapCounter, cardSpareGroupMode=cardSpareGroupMode, portRxFlowControlMode=portRxFlowControlMode, cmResetReason=cmResetReason, portSecStateChange=portSecStateChange, portNumChans=portNumChans, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMib=cadEquipmentMib, cardBootVersion=cardBootVersion, auditTable=auditTable, cardTrapInh=cardTrapInh, cardId=cardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, errEvLoggingEnabled=errEvLoggingEnabled, portLastChangeTime=portLastChangeTime, cardCpuSpeed=cardCpuSpeed, TestCommand=TestCommand, equipmentError=equipmentError, portConnectorId=portConnectorId, cardTestType=cardTestType, cmResetStatus=cmResetStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditCycleCount=auditCycleCount, errEventId=errEventId, cardPrState=cardPrState, cardSpareGroupId=cardSpareGroupId, cardTestTime=cardTestTime, cmDevice=cmDevice, TestScheduleCommand=TestScheduleCommand, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentTbl=equipmentTbl, cardSubType=cardSubType, auditEntry=auditEntry, portCardSubType=portCardSubType, cardTestCommand=cardTestCommand, portAdminState=portAdminState, portDocsIfIndex=portDocsIfIndex, diskVolumeCardId=diskVolumeCardId, cardCarrierSerialNum=cardCarrierSerialNum, cardSecState=cardSecState, cardLastChangeTime=cardLastChangeTime, cardPicDetected=cardPicDetected, equipmentDiag=equipmentDiag, cardDplxStatus=cardDplxStatus, cardFwUpdateStatus=cardFwUpdateStatus, cardCpuType=cardCpuType, verboseLevel=verboseLevel, equipmentShelfGroup=equipmentShelfGroup, equipmentTraps=equipmentTraps, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeEntry=diskVolumeEntry, auditLogThrottle=auditLogThrottle, cardTemperatureHighError=cardTemperatureHighError, auditAutoScheduling=auditAutoScheduling, cardTestResultNotification=cardTestResultNotification, auditDescription=auditDescription, portDescription=portDescription, diskVolumeSize=diskVolumeSize, cmResetUpTime=cmResetUpTime, cardTable=cardTable, cardSwVersion=cardSwVersion, cardCarrierHwVersion=cardCarrierHwVersion, portEntry=portEntry, cardPrStateChange=cardPrStateChange, portSecState=portSecState, cardFlashSize=cardFlashSize, diskVolumeTrapInh=diskVolumeTrapInh, cardShelfId=cardShelfId, auditLogOutput=auditLogOutput, portShelfId=portShelfId, portPrState=portPrState, cardName=cardName, cardRamSize=cardRamSize, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, downstreamPowerLoss=downstreamPowerLoss, portId=portId, diskVolumeShelfId=diskVolumeShelfId, cardTestEntry=cardTestEntry, auditId=auditId, cmUChannelID=cmUChannelID, portRxFlowControlDetected=portRxFlowControlDetected, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentState=equipmentState, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicHwVersion=cardPicHwVersion, remainInDiagMode=remainInDiagMode, cardSubDetected=cardSubDetected, equipmentAudit=equipmentAudit, errEvRowStatus=errEvRowStatus, diskVolumeGroup=diskVolumeGroup, trapSeverity=trapSeverity, portNumber=portNumber, diagTestId=diagTestId, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeDriveId=diskVolumeDriveId, portDplxStatus=portDplxStatus, cardShelfTestIndex=cardShelfTestIndex, portMode=portMode, cardTestSchedule=cardTestSchedule, cmIpAddress=cmIpAddress, diskVolumeTable=diskVolumeTable, auditShelfId=auditShelfId, cardNumPorts=cardNumPorts, cardDetected=cardDetected, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestTable=cardTestTable, cmResetClearNotification=cmResetClearNotification, portMinDsPower=portMinDsPower, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmVendor=cmVendor, cmMacAddress=cmMacAddress, cardTestId=cardTestId, consoleOutput=consoleOutput, portType=portType, cardTestName=cardTestName, cardTestDescription=cardTestDescription, auditCardId=auditCardId, systemGeneral=systemGeneral, portTrapInh=portTrapInh, portCardId=portCardId, cardNumber=cardNumber, cardSecStateChange=cardSecStateChange, fpgaErrorEventEntry=fpgaErrorEventEntry, TestSchedule=TestSchedule, systemClock=systemClock, errEvLogLevel=errEvLogLevel, cardCarrierFwVersion=cardCarrierFwVersion, cmRegistrationNotification=cmRegistrationNotification, shelfNumber=shelfNumber, auditResult=auditResult, equipmentCardTestGroup=equipmentCardTestGroup, cardAdminState=cardAdminState, equipmentAuditGroup=equipmentAuditGroup, cardTestTransId=cardTestTransId, shelfSwVersion=shelfSwVersion, auditPassedCount=auditPassedCount, auditFailedCount=auditFailedCount, cardDplxStatusChange=cardDplxStatusChange, portPrStateChange=portPrStateChange, cadEquipmentMibConformance=cadEquipmentMibConformance, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardAction=cardAction, eqDiagConfig=eqDiagConfig, TestResult=TestResult, cardType=cardType, cardCPLDVersion=cardCPLDVersion, portDetectedMode=portDetectedMode, auditTime=auditTime, cmResetNotification=cmResetNotification, cardTestIndex=cardTestIndex, cardTestScheduleCommand=cardTestScheduleCommand, portDplxStatusChange=portDplxStatusChange, diskDriveNumber=diskDriveNumber, systemGeneralGroup=systemGeneralGroup, auditStatus=auditStatus, shelfName=shelfName, portMacIfIndex=portMacIfIndex, auditTotalFailedCount=auditTotalFailedCount, diskVolumeFileName=diskVolumeFileName, cardTestResultDesc=cardTestResultDesc)
