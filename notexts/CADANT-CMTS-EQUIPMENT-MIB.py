...
DuplexStatus, FirmwareSource, PortDetectedMode, PortType, AdminState, PrimaryState, SecondaryState, EqActionType, DiskVolumeUsageLevel, PortId, PicType, FlowControlMode, CardType, ShelfId, CardId, PortMode, CardSubType, UnknownState = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "FirmwareSource", "PortDetectedMode", "PortType", "AdminState", "PrimaryState", "SecondaryState", "EqActionType", "DiskVolumeUsageLevel", "PortId", "PicType", "FlowControlMode", "CardType", "ShelfId", "CardId", "PortMode", "CardSubType", "UnknownState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", errEvLogLevel=errEvLogLevel, auditAutoScheduling=auditAutoScheduling, cardTestScheduleCommand=cardTestScheduleCommand, cardSerialNum=cardSerialNum, equipmentDiag=equipmentDiag, equipmentState=equipmentState, cardCpuType=cardCpuType, cardBootVersion=cardBootVersion, cardTempNoReportNotification=cardTempNoReportNotification, portTxFlowControlDetected=portTxFlowControlDetected, verboseLevel=verboseLevel, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeFileName=diskVolumeFileName, diskVolumeTrapInh=diskVolumeTrapInh, cmPrimarySID=cmPrimarySID, portRxFlowControlDetected=portRxFlowControlDetected, portTxFlowControlMode=portTxFlowControlMode, cmResetUpTime=cmResetUpTime, portDocsIfIndex=portDocsIfIndex, cardSpareGroupId=cardSpareGroupId, portType=portType, portAdminState=portAdminState, shelfNumber=shelfNumber, cardAction=cardAction, cardSwVersion=cardSwVersion, portSecStateChange=portSecStateChange, TestTransactionId=TestTransactionId, cardSubDetected=cardSubDetected, auditName=auditName, cardPicSerialNum=cardPicSerialNum, cardTemperatureHighWarn=cardTemperatureHighWarn, cardEntry=cardEntry, portCardSubType=portCardSubType, diskVolumeName=diskVolumeName, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditTime=auditTime, errEventId=errEventId, equipmentStateGroup=equipmentStateGroup, cardFwUpdateStatus=cardFwUpdateStatus, TestCommand=TestCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestId=TestId, remainInDiagMode=remainInDiagMode, portMacIfIndex=portMacIfIndex, cardTable=cardTable, cardHwVersion=cardHwVersion, cardTemperature=cardTemperature, auditTable=auditTable, portTrapInh=portTrapInh, equipmentError=equipmentError, cardTestTable=cardTestTable, cardTestIndex=cardTestIndex, cardLastBootSource=cardLastBootSource, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, consoleOutput=consoleOutput, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestResultNotification=cardTestResultNotification, auditFailedCount=auditFailedCount, portId=portId, portCurrDsPower=portCurrDsPower, cardTestType=cardTestType, cardSecStateChange=cardSecStateChange, diskDriveNumber=diskDriveNumber, trapCounter=trapCounter, cardDetected=cardDetected, cardUpTime=cardUpTime, cmUChannelID=cmUChannelID, cardNumber=cardNumber, portDescription=portDescription, cardPrState=cardPrState, equipmentTbl=equipmentTbl, cardFpgaSource=cardFpgaSource, portPrStateChange=portPrStateChange, diagTestId=diagTestId, portMacAddress=portMacAddress, auditDescription=auditDescription, cardTemperatureHighError=cardTemperatureHighError, cardTestSchedule=cardTestSchedule, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResultDesc=cardTestResultDesc, cardDetectedChange=cardDetectedChange, systemClock=systemClock, cardSubType=cardSubType, auditCardId=auditCardId, auditLogThrottle=auditLogThrottle, cmRegistrationNotification=cmRegistrationNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentAudit=equipmentAudit, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentPortGroup=equipmentPortGroup, cardShelfId=cardShelfId, TestScheduleCommand=TestScheduleCommand, portEntry=portEntry, auditCommand=auditCommand, portDplxStatusChange=portDplxStatusChange, portLastChangeTime=portLastChangeTime, cardPicHwVersion=cardPicHwVersion, cardNumPorts=cardNumPorts, portPrState=portPrState, equipmentCardGroup=equipmentCardGroup, cmResetReason=cmResetReason, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardId=cardId, cmVendor=cmVendor, cardTestName=cardTestName, cmResetStatus=cmResetStatus, trapSeverity=trapSeverity, diskVolumeDriveId=diskVolumeDriveId, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventTable=fpgaErrorEventTable, cmResetClearNotification=cmResetClearNotification, portSecState=portSecState, cardAdminState=cardAdminState, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeTable=diskVolumeTable, auditCycleCount=auditCycleCount, cardPrStateChange=cardPrStateChange, equipmentShelfGroup=equipmentShelfGroup, portCardId=portCardId, cmResetNotification=cmResetNotification, auditShelfId=auditShelfId, auditStatus=auditStatus, cardCarrierHwVersion=cardCarrierHwVersion, equipmentTraps=equipmentTraps, portMode=portMode, eqDiagConfig=eqDiagConfig, errEvLoggingEnabled=errEvLoggingEnabled, cardLastChangeTime=cardLastChangeTime, portDplxStatus=portDplxStatus, portRxFlowControlMode=portRxFlowControlMode, portNumber=portNumber, shelfName=shelfName, portTable=portTable, cardTestDescription=cardTestDescription, cardCPLDVersion=cardCPLDVersion, diskVolumeShelfId=diskVolumeShelfId, cmDevice=cmDevice, portMinDsPower=portMinDsPower, diskVolumeSize=diskVolumeSize, TestSchedule=TestSchedule, systemKey=systemKey, auditLogOutput=auditLogOutput, fpgaErrorEventGroup=fpgaErrorEventGroup, cadEquipmentMib=cadEquipmentMib, diskVolumeUsageLevel=diskVolumeUsageLevel, systemGeneralGroup=systemGeneralGroup, errEvRowStatus=errEvRowStatus, auditTotalFailedCount=auditTotalFailedCount, shelfObjects=shelfObjects, portConnectorId=portConnectorId, cardTrapInh=cardTrapInh, cardTestCommand=cardTestCommand, portAction=portAction, cardFlashSize=cardFlashSize, downstreamPowerLoss=downstreamPowerLoss, PYSNMP_MODULE_ID=cadEquipmentMib, cardDplxStatus=cardDplxStatus, TestType=TestType, cardName=cardName, auditEntry=auditEntry, cardRamSize=cardRamSize, portMaxDsPower=portMaxDsPower, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestTime=cardTestTime, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeGroup=diskVolumeGroup, cardTestId=cardTestId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeNumber=diskVolumeNumber, shelfSwVersion=shelfSwVersion, cardTestTransId=cardTestTransId, diskVolumeEntry=diskVolumeEntry, cardTestEntry=cardTestEntry, cardType=cardType, cardDplxStatusChange=cardDplxStatusChange, cardPicDetected=cardPicDetected, auditResult=auditResult, cmMacAddress=cmMacAddress, portShelfId=portShelfId, cmIpAddress=cmIpAddress, cardBusSpeed=cardBusSpeed, cardLastResetReason=cardLastResetReason, auditPassedCount=auditPassedCount, cardLastBootVersion=cardLastBootVersion, cardCarrierSerialNum=cardCarrierSerialNum, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSpareGroupMode=cardSpareGroupMode, diskVolumeCardId=diskVolumeCardId, diskVolumeId=diskVolumeId, portDetectedMode=portDetectedMode, TestResult=TestResult, portBgpId=portBgpId, auditTotalPassedCount=auditTotalPassedCount, cardFwVersion=cardFwVersion, cardShelfTestIndex=cardShelfTestIndex, equipmentDiagGroup=equipmentDiagGroup, equipmentAuditGroup=equipmentAuditGroup, portNumChans=portNumChans, cardSecState=cardSecState, auditId=auditId, systemGeneral=systemGeneral, cmResetInfo=cmResetInfo, cardTestResult=cardTestResult, cardCpuSpeed=cardCpuSpeed)
