...
AdminState, PrimaryState, DuplexStatus, PicType, PortType, SecondaryState, PortMode, EqActionType, ShelfId, CardSubType, UnknownState, FirmwareSource, CardId, FlowControlMode, PortId, CardType, PortDetectedMode, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PrimaryState", "DuplexStatus", "PicType", "PortType", "SecondaryState", "PortMode", "EqActionType", "ShelfId", "CardSubType", "UnknownState", "FirmwareSource", "CardId", "FlowControlMode", "PortId", "CardType", "PortDetectedMode", "DiskVolumeUsageLevel")
...
TextualConvention, DisplayString, MacAddress, RowStatus, TruthValue, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "MacAddress", "RowStatus", "TruthValue", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestTransId=cardTestTransId, downstreamPowerLoss=downstreamPowerLoss, portCardSubType=portCardSubType, auditPassedCount=auditPassedCount, portAdminState=portAdminState, cardHwVersion=cardHwVersion, equipmentState=equipmentState, cardPrState=cardPrState, portMacAddress=portMacAddress, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestId=cardTestId, cardBusSpeed=cardBusSpeed, cardSecState=cardSecState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardAdminState=cardAdminState, cardFwUpdateStatus=cardFwUpdateStatus, portDescription=portDescription, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeShelfId=diskVolumeShelfId, auditDescription=auditDescription, diskVolumeSize=diskVolumeSize, cardTempOverHeatNotification=cardTempOverHeatNotification, portNumChans=portNumChans, diskVolumeDriveId=diskVolumeDriveId, equipmentStateGroup=equipmentStateGroup, equipmentTraps=equipmentTraps, portConnectorId=portConnectorId, shelfSwVersion=shelfSwVersion, cmRegistrationNotification=cmRegistrationNotification, TestId=TestId, cardTestCommand=cardTestCommand, equipmentAudit=equipmentAudit, equipmentAuditGroup=equipmentAuditGroup, cardType=cardType, cardFlashSize=cardFlashSize, diskVolumeGroup=diskVolumeGroup, auditStatus=auditStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTempNoReportNotification=cardTempNoReportNotification, cardTestType=cardTestType, cardSerialNum=cardSerialNum, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portBgpId=portBgpId, cardTestResultDesc=cardTestResultDesc, cadEquipmentMib=cadEquipmentMib, cardTestScheduleCommand=cardTestScheduleCommand, portDplxStatusChange=portDplxStatusChange, cardName=cardName, consoleOutput=consoleOutput, cardSpareGroupId=cardSpareGroupId, TestSchedule=TestSchedule, auditTime=auditTime, cardTestEntry=cardTestEntry, portDplxStatus=portDplxStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, cmPrimarySID=cmPrimarySID, shelfNumber=shelfNumber, errEvLoggingEnabled=errEvLoggingEnabled, portPrStateChange=portPrStateChange, auditCommand=auditCommand, portTxFlowControlMode=portTxFlowControlMode, remainInDiagMode=remainInDiagMode, cardDplxStatus=cardDplxStatus, cardTestTime=cardTestTime, auditTable=auditTable, auditShelfId=auditShelfId, TestType=TestType, cmVendor=cmVendor, cmResetReason=cmResetReason, trapCounter=trapCounter, errEvLogLevel=errEvLogLevel, equipmentCardGroup=equipmentCardGroup, cardLastChangeTime=cardLastChangeTime, portSecStateChange=portSecStateChange, TestCommand=TestCommand, cardCarrierHwVersion=cardCarrierHwVersion, cardSubType=cardSubType, fpgaErrorEventGroup=fpgaErrorEventGroup, cardUpTime=cardUpTime, portDocsIfIndex=portDocsIfIndex, portSecState=portSecState, cardTemperatureHighError=cardTemperatureHighError, auditTotalFailedCount=auditTotalFailedCount, TestTransactionId=TestTransactionId, equipmentError=equipmentError, cardId=cardId, eqDiagConfig=eqDiagConfig, cmResetNotification=cmResetNotification, auditId=auditId, cmResetClearNotification=cmResetClearNotification, diskVolumeDiskSize=diskVolumeDiskSize, cmIpAddress=cmIpAddress, cardCPLDVersion=cardCPLDVersion, fpgaErrorEventTable=fpgaErrorEventTable, auditLogThrottle=auditLogThrottle, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardShelfTestIndex=cardShelfTestIndex, portTxFlowControlDetected=portTxFlowControlDetected, cardDetectedChange=cardDetectedChange, portLastChangeTime=portLastChangeTime, equipmentCardTestGroup=equipmentCardTestGroup, cardCpuSpeed=cardCpuSpeed, portCurrDsPower=portCurrDsPower, auditCardId=auditCardId, portType=portType, portNumber=portNumber, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskDriveNumber=diskDriveNumber, cardAction=cardAction, cardFwVersion=cardFwVersion, cardTestDescription=cardTestDescription, cardBootVersion=cardBootVersion, cardTestResult=cardTestResult, cardPicHwVersion=cardPicHwVersion, auditResult=auditResult, cmResetStatus=cmResetStatus, cardNumPorts=cardNumPorts, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portShelfId=portShelfId, auditAutoScheduling=auditAutoScheduling, TestScheduleCommand=TestScheduleCommand, cardTestTable=cardTestTable, cmMacAddress=cmMacAddress, auditCycleCount=auditCycleCount, cardCarrierSerialNum=cardCarrierSerialNum, portCardId=portCardId, cardDetected=cardDetected, cardPicDetected=cardPicDetected, cmUChannelID=cmUChannelID, portRxFlowControlDetected=portRxFlowControlDetected, cardTable=cardTable, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestName=cardTestName, cardTestResultNotification=cardTestResultNotification, cardSpareGroupMode=cardSpareGroupMode, diskVolumeTrapInh=diskVolumeTrapInh, shelfName=shelfName, portId=portId, systemKey=systemKey, shelfObjects=shelfObjects, diskVolumeFileName=diskVolumeFileName, cmDevice=cmDevice, auditName=auditName, auditEntry=auditEntry, cmResetUpTime=cmResetUpTime, cardPrStateChange=cardPrStateChange, cardTemperature=cardTemperature, diagTestId=diagTestId, trapSeverity=trapSeverity, portAction=portAction, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMinDsPower=portMinDsPower, diskVolumeId=diskVolumeId, cmResetInfo=cmResetInfo, systemGeneralGroup=systemGeneralGroup, TestResult=TestResult, portEntry=portEntry, portMaxDsPower=portMaxDsPower, cadEquipmentMibGroup=cadEquipmentMibGroup, portMacIfIndex=portMacIfIndex, cardTestIndex=cardTestIndex, portRxFlowControlMode=portRxFlowControlMode, portTable=portTable, diskVolumeName=diskVolumeName, fpgaErrorEventEntry=fpgaErrorEventEntry, portDetectedMode=portDetectedMode, equipmentPortGroup=equipmentPortGroup, cardDplxStatusChange=cardDplxStatusChange, cardLastBootVersion=cardLastBootVersion, verboseLevel=verboseLevel, equipmentTbl=equipmentTbl, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentDiagGroup=equipmentDiagGroup, diskVolumeNumber=diskVolumeNumber, cardTrapInh=cardTrapInh, cardEntry=cardEntry, portTrapInh=portTrapInh, auditLogOutput=auditLogOutput, cardLastResetReason=cardLastResetReason, cardRamSize=cardRamSize, cardSecStateChange=cardSecStateChange, portPrState=portPrState, cardSwVersion=cardSwVersion, auditTotalPassedCount=auditTotalPassedCount, cardSubDetected=cardSubDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, errEvRowStatus=errEvRowStatus, systemClock=systemClock, cadEquipmentMibConformance=cadEquipmentMibConformance, cardShelfId=cardShelfId, cardCpuType=cardCpuType, equipmentShelfGroup=equipmentShelfGroup, diskVolumeTable=diskVolumeTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCarrierFwVersion=cardCarrierFwVersion, auditFailedCount=auditFailedCount, diskVolumeEntry=diskVolumeEntry, diskVolumeCardId=diskVolumeCardId, cardTestSchedule=cardTestSchedule, cardLastBootSource=cardLastBootSource, cardFpgaSource=cardFpgaSource, portMode=portMode, systemGeneral=systemGeneral, errEventId=errEventId, cardNumber=cardNumber, cardPicSerialNum=cardPicSerialNum, equipmentDiag=equipmentDiag)
