...
DisplayString, MacAddress, TruthValue, TimeStamp, DateAndTime, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "TruthValue", "TimeStamp", "DateAndTime", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portMaxDsPower=portMaxDsPower, auditTable=auditTable, trapSeverity=trapSeverity, portDescription=portDescription, shelfName=shelfName, cardLastBootSource=cardLastBootSource, auditFailedCount=auditFailedCount, cardLastResetReason=cardLastResetReason, cardNumber=cardNumber, cardPrState=cardPrState, portDplxStatusChange=portDplxStatusChange, portDplxStatus=portDplxStatus, diskVolumeGroup=diskVolumeGroup, cardTestResultNotification=cardTestResultNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, cmMacAddress=cmMacAddress, cardTestCommand=cardTestCommand, errEventId=errEventId, cardTestResult=cardTestResult, portCurrDsPower=portCurrDsPower, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentPortGroup=equipmentPortGroup, TestSchedule=TestSchedule, portTable=portTable, cmResetClearNotification=cmResetClearNotification, portTxFlowControlMode=portTxFlowControlMode, portMinDsPower=portMinDsPower, cardTestName=cardTestName, TestCommand=TestCommand, cmResetReason=cmResetReason, portEntry=portEntry, cardCarrierHwVersion=cardCarrierHwVersion, TestScheduleCommand=TestScheduleCommand, portConnectorId=portConnectorId, cardShelfTestIndex=cardShelfTestIndex, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cadEquipmentMibGroup=cadEquipmentMibGroup, PYSNMP_MODULE_ID=cadEquipmentMib, cardFpgaSource=cardFpgaSource, verboseLevel=verboseLevel, auditStatus=auditStatus, cmVendor=cmVendor, diskVolumeId=diskVolumeId, cardSpareGroupMode=cardSpareGroupMode, auditTotalPassedCount=auditTotalPassedCount, portSecStateChange=portSecStateChange, cardCarrierFwVersion=cardCarrierFwVersion, cardTemperatureHighError=cardTemperatureHighError, portNumChans=portNumChans, diskVolumeEntry=diskVolumeEntry, portBgpId=portBgpId, diskVolumeFileName=diskVolumeFileName, cardFlashSize=cardFlashSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portRxFlowControlMode=portRxFlowControlMode, portPrStateChange=portPrStateChange, shelfSwVersion=shelfSwVersion, errEvRowStatus=errEvRowStatus, portMode=portMode, cmIpAddress=cmIpAddress, cardPicHwVersion=cardPicHwVersion, diskVolumeDriveId=diskVolumeDriveId, cmUChannelID=cmUChannelID, remainInDiagMode=remainInDiagMode, cardSerialNum=cardSerialNum, equipmentShelfGroup=equipmentShelfGroup, auditAutoScheduling=auditAutoScheduling, equipmentDiagGroup=equipmentDiagGroup, cardRamSize=cardRamSize, portTxFlowControlDetected=portTxFlowControlDetected, cardTestTable=cardTestTable, equipmentCardGroup=equipmentCardGroup, cardSpareGroupId=cardSpareGroupId, cmResetStatus=cmResetStatus, cardSubDetected=cardSubDetected, auditCommand=auditCommand, cardTestType=cardTestType, cardCPLDVersion=cardCPLDVersion, cardTestIndex=cardTestIndex, equipmentAuditGroup=equipmentAuditGroup, diskVolumeTrapInh=diskVolumeTrapInh, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSubType=cardSubType, portAction=portAction, cadEquipmentMib=cadEquipmentMib, cardTestTime=cardTestTime, diskVolumeNumber=diskVolumeNumber, auditResult=auditResult, auditCardId=auditCardId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, systemClock=systemClock, TestType=TestType, cardLastChangeTime=cardLastChangeTime, equipmentCardTestGroup=equipmentCardTestGroup, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFwUpdateStatus=cardFwUpdateStatus, portNumber=portNumber, cardAdminState=cardAdminState, portLastChangeTime=portLastChangeTime, diskVolumeCardId=diskVolumeCardId, cardTestEntry=cardTestEntry, cardPrStateChange=cardPrStateChange, trapCounter=trapCounter, cardTestDescription=cardTestDescription, equipmentError=equipmentError, cardDetectedChange=cardDetectedChange, cmPrimarySID=cmPrimarySID, cardDplxStatusChange=cardDplxStatusChange, systemGeneralGroup=systemGeneralGroup, equipmentTraps=equipmentTraps, cardUpTime=cardUpTime, cardFwVersion=cardFwVersion, portMacAddress=portMacAddress, shelfObjects=shelfObjects, cardTrapInh=cardTrapInh, portRxFlowControlDetected=portRxFlowControlDetected, cmResetInfo=cmResetInfo, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTable=cardTable, portId=portId, TestTransactionId=TestTransactionId, portMacIfIndex=portMacIfIndex, cmResetNotification=cmResetNotification, cardTempNoReportNotification=cardTempNoReportNotification, cardBootVersion=cardBootVersion, cardTestId=cardTestId, diskVolumeSize=diskVolumeSize, eqDiagConfig=eqDiagConfig, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetUpTime=cmResetUpTime, cmRegistrationNotification=cmRegistrationNotification, cardPicSerialNum=cardPicSerialNum, portType=portType, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeShelfId=diskVolumeShelfId, errEvLoggingEnabled=errEvLoggingEnabled, cardCpuSpeed=cardCpuSpeed, equipmentDiag=equipmentDiag, cmDevice=cmDevice, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardDplxStatus=cardDplxStatus, auditTime=auditTime, cardSecState=cardSecState, portAdminState=portAdminState, cardName=cardName, cardType=cardType, portDetectedMode=portDetectedMode, equipmentStateGroup=equipmentStateGroup, cardLastBootVersion=cardLastBootVersion, cardSwVersion=cardSwVersion, portDocsIfIndex=portDocsIfIndex, portShelfId=portShelfId, equipmentTbl=equipmentTbl, cardShelfId=cardShelfId, cardNumPorts=cardNumPorts, cardHwVersion=cardHwVersion, auditName=auditName, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResultDesc=cardTestResultDesc, cardCpuType=cardCpuType, downstreamPowerLoss=downstreamPowerLoss, systemKey=systemKey, cardDetected=cardDetected, systemGeneral=systemGeneral, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeName=diskVolumeName, diskVolumeUsagePercentage=diskVolumeUsagePercentage, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeDiskSize=diskVolumeDiskSize, portCardId=portCardId, cardId=cardId, diagTestId=diagTestId, diskVolumeTable=diskVolumeTable, cardTestScheduleCommand=cardTestScheduleCommand, auditTotalFailedCount=auditTotalFailedCount, portPrState=portPrState, TestResult=TestResult, consoleOutput=consoleOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentAudit=equipmentAudit, cardTestTransId=cardTestTransId, portCardSubType=portCardSubType, cardAction=cardAction, cardTemperature=cardTemperature, auditId=auditId, cardEntry=cardEntry, auditLogThrottle=auditLogThrottle, shelfNumber=shelfNumber, cardTestSchedule=cardTestSchedule, auditShelfId=auditShelfId, equipmentState=equipmentState, fpgaErrorEventEntry=fpgaErrorEventEntry, errEvLogLevel=errEvLogLevel, auditPassedCount=auditPassedCount, portTrapInh=portTrapInh, auditCycleCount=auditCycleCount, auditEntry=auditEntry, portSecState=portSecState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardBusSpeed=cardBusSpeed, auditLogOutput=auditLogOutput, cardPicDetected=cardPicDetected, cardSecStateChange=cardSecStateChange, diskDriveNumber=diskDriveNumber, TestId=TestId, auditDescription=auditDescription)
