...
PrimaryState, ShelfId, DiskVolumeUsageLevel, CardSubType, UnknownState, EqActionType, PortMode, FlowControlMode, CardId, CardType, PortType, FirmwareSource, SecondaryState, PortId, PortDetectedMode, PicType, AdminState, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "ShelfId", "DiskVolumeUsageLevel", "CardSubType", "UnknownState", "EqActionType", "PortMode", "FlowControlMode", "CardId", "CardType", "PortType", "FirmwareSource", "SecondaryState", "PortId", "PortDetectedMode", "PicType", "AdminState", "DuplexStatus")
...
TimeStamp, MacAddress, RowStatus, DateAndTime, DisplayString, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "MacAddress", "RowStatus", "DateAndTime", "DisplayString", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", systemGeneral=systemGeneral, cardEntry=cardEntry, cardHwVersion=cardHwVersion, portLastChangeTime=portLastChangeTime, portId=portId, fpgaErrorEventGroup=fpgaErrorEventGroup, TestType=TestType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeTrapInh=diskVolumeTrapInh, portNumChans=portNumChans, consoleOutput=consoleOutput, auditDescription=auditDescription, cmResetUpTime=cmResetUpTime, remainInDiagMode=remainInDiagMode, cmResetClearNotification=cmResetClearNotification, cardBootVersion=cardBootVersion, equipmentShelfGroup=equipmentShelfGroup, cardTemperatureHighError=cardTemperatureHighError, portAdminState=portAdminState, cardAdminState=cardAdminState, diskVolumeTable=diskVolumeTable, diskVolumeDiskSize=diskVolumeDiskSize, cardTestCommand=cardTestCommand, cardCpuType=cardCpuType, auditResult=auditResult, diskVolumeCardId=diskVolumeCardId, errEvLogLevel=errEvLogLevel, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cmUChannelID=cmUChannelID, portConnectorId=portConnectorId, cardSecStateChange=cardSecStateChange, portDetectedMode=portDetectedMode, auditCardId=auditCardId, cardTestEntry=cardTestEntry, cardTrapInh=cardTrapInh, cardName=cardName, cardDplxStatusChange=cardDplxStatusChange, auditTable=auditTable, cardLastResetReason=cardLastResetReason, cardSpareGroupId=cardSpareGroupId, diskVolumeEntry=diskVolumeEntry, cardDplxStatus=cardDplxStatus, verboseLevel=verboseLevel, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestTransId=cardTestTransId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetInfo=cmResetInfo, cardSerialNum=cardSerialNum, TestId=TestId, PYSNMP_MODULE_ID=cadEquipmentMib, portMacIfIndex=portMacIfIndex, auditEntry=auditEntry, cardRamSize=cardRamSize, cardTestName=cardTestName, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicSerialNum=cardPicSerialNum, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardBusSpeed=cardBusSpeed, cardTestTime=cardTestTime, cardTestSchedule=cardTestSchedule, diskVolumeId=diskVolumeId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentCardGroup=equipmentCardGroup, cardShelfTestIndex=cardShelfTestIndex, cardCarrierSerialNum=cardCarrierSerialNum, TestScheduleCommand=TestScheduleCommand, portMinDsPower=portMinDsPower, diskVolumeDriveId=diskVolumeDriveId, equipmentAuditGroup=equipmentAuditGroup, equipmentCardTestGroup=equipmentCardTestGroup, cardTestResultDesc=cardTestResultDesc, cardDetected=cardDetected, diskVolumeGroup=diskVolumeGroup, cardAction=cardAction, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDetectedChange=cardDetectedChange, portEntry=portEntry, auditTime=auditTime, equipmentTbl=equipmentTbl, portDescription=portDescription, TestResult=TestResult, portRxFlowControlDetected=portRxFlowControlDetected, portType=portType, portCurrDsPower=portCurrDsPower, diskVolumeNumber=diskVolumeNumber, cardFlashSize=cardFlashSize, portMaxDsPower=portMaxDsPower, errEvLoggingEnabled=errEvLoggingEnabled, portDplxStatusChange=portDplxStatusChange, cardId=cardId, eqDiagConfig=eqDiagConfig, downstreamPowerLoss=downstreamPowerLoss, cardTestResultNotification=cardTestResultNotification, cardTemperature=cardTemperature, cardTestId=cardTestId, auditLogOutput=auditLogOutput, cadEquipmentMib=cadEquipmentMib, diskVolumeFileName=diskVolumeFileName, cmIpAddress=cmIpAddress, equipmentAudit=equipmentAudit, systemGeneralGroup=systemGeneralGroup, cardShelfId=cardShelfId, portRxFlowControlMode=portRxFlowControlMode, portMacAddress=portMacAddress, cardTable=cardTable, equipmentStateGroup=equipmentStateGroup, cardNumber=cardNumber, cardTestIndex=cardTestIndex, cmResetStatus=cmResetStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSpareGroupMode=cardSpareGroupMode, equipmentDiagGroup=equipmentDiagGroup, diagTestId=diagTestId, shelfName=shelfName, auditFailedCount=auditFailedCount, trapCounter=trapCounter, auditTotalFailedCount=auditTotalFailedCount, cardType=cardType, cardFwUpdateStatus=cardFwUpdateStatus, cardCarrierFwVersion=cardCarrierFwVersion, cardTestDescription=cardTestDescription, auditCycleCount=auditCycleCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSubType=cardSubType, portShelfId=portShelfId, cardFwVersion=cardFwVersion, auditTotalPassedCount=auditTotalPassedCount, cardTestScheduleCommand=cardTestScheduleCommand, equipmentPortGroup=equipmentPortGroup, equipmentDiag=equipmentDiag, shelfSwVersion=shelfSwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCPLDVersion=cardCPLDVersion, systemClock=systemClock, portTable=portTable, diskVolumeSize=diskVolumeSize, cardSecState=cardSecState, auditAutoScheduling=auditAutoScheduling, systemKey=systemKey, cardLastChangeTime=cardLastChangeTime, portMode=portMode, cmDevice=cmDevice, equipmentState=equipmentState, equipmentError=equipmentError, cardSwVersion=cardSwVersion, portSecState=portSecState, TestSchedule=TestSchedule, portNumber=portNumber, cardFpgaSource=cardFpgaSource, diskVolumeShelfId=diskVolumeShelfId, cardTestResult=cardTestResult, portBgpId=portBgpId, cardSubDetected=cardSubDetected, cardPrState=cardPrState, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCarrierHwVersion=cardCarrierHwVersion, shelfObjects=shelfObjects, portDocsIfIndex=portDocsIfIndex, auditName=auditName, TestCommand=TestCommand, cardCpuSpeed=cardCpuSpeed, diskVolumeName=diskVolumeName, cardTestTable=cardTestTable, cardTemperatureHighWarn=cardTemperatureHighWarn, fpgaErrorEventTable=fpgaErrorEventTable, trapSeverity=trapSeverity, cardLastBootSource=cardLastBootSource, auditShelfId=auditShelfId, cmMacAddress=cmMacAddress, auditStatus=auditStatus, cardUpTime=cardUpTime, cardTestType=cardTestType, cardPrStateChange=cardPrStateChange, cardNumPorts=cardNumPorts, diskDriveNumber=diskDriveNumber, shelfNumber=shelfNumber, portDplxStatus=portDplxStatus, errEventId=errEventId, portTxFlowControlDetected=portTxFlowControlDetected, auditPassedCount=auditPassedCount, auditId=auditId, cardPicHwVersion=cardPicHwVersion, auditCommand=auditCommand, portPrState=portPrState, portAction=portAction, cardLastBootVersion=cardLastBootVersion, portPrStateChange=portPrStateChange, cmPrimarySID=cmPrimarySID, errEvRowStatus=errEvRowStatus, cmVendor=cmVendor, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetNotification=cmResetNotification, portCardId=portCardId, portCardSubType=portCardSubType, cmResetReason=cmResetReason, cardPicDetected=cardPicDetected, portSecStateChange=portSecStateChange, cmRegistrationNotification=cmRegistrationNotification, portTrapInh=portTrapInh, portTxFlowControlMode=portTxFlowControlMode, equipmentTraps=equipmentTraps, TestTransactionId=TestTransactionId, auditLogThrottle=auditLogThrottle)
