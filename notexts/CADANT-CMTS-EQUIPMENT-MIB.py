...
ShelfId, CardId, FlowControlMode, FirmwareSource, PortMode, PortId, CardSubType, EqActionType, PicType, DiskVolumeUsageLevel, CardType, DuplexStatus, PrimaryState, PortType, UnknownState, PortDetectedMode, AdminState, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "CardId", "FlowControlMode", "FirmwareSource", "PortMode", "PortId", "CardSubType", "EqActionType", "PicType", "DiskVolumeUsageLevel", "CardType", "DuplexStatus", "PrimaryState", "PortType", "UnknownState", "PortDetectedMode", "AdminState", "SecondaryState")
...
RowStatus, DisplayString, TimeStamp, DateAndTime, TextualConvention, TruthValue, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "TimeStamp", "DateAndTime", "TextualConvention", "TruthValue", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardNumber=cardNumber, diskVolumeCardId=diskVolumeCardId, portCardSubType=portCardSubType, cardSecState=cardSecState, cardNumPorts=cardNumPorts, cardFpgaSource=cardFpgaSource, diskVolumeTable=diskVolumeTable, cardDetectedChange=cardDetectedChange, cardTestTime=cardTestTime, cardName=cardName, diskVolumeEntry=diskVolumeEntry, portTable=portTable, equipmentAuditGroup=equipmentAuditGroup, cardHwVersion=cardHwVersion, equipmentStateGroup=equipmentStateGroup, shelfObjects=shelfObjects, cardShelfTestIndex=cardShelfTestIndex, cardAction=cardAction, diskVolumeId=diskVolumeId, cardCPLDVersion=cardCPLDVersion, portNumChans=portNumChans, equipmentPortGroup=equipmentPortGroup, cardTestResultDesc=cardTestResultDesc, TestSchedule=TestSchedule, diskVolumeSize=diskVolumeSize, auditName=auditName, cardTestId=cardTestId, cardSubDetected=cardSubDetected, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTempNoReportNotification=cardTempNoReportNotification, auditCardId=auditCardId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, verboseLevel=verboseLevel, diskVolumeGroup=diskVolumeGroup, cardTestEntry=cardTestEntry, cmVendor=cmVendor, auditStatus=auditStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestResultNotification=cardTestResultNotification, fpgaErrorEventTable=fpgaErrorEventTable, errEvLogLevel=errEvLogLevel, cardBootVersion=cardBootVersion, auditTotalFailedCount=auditTotalFailedCount, TestId=TestId, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResult=cardTestResult, cmResetClearNotification=cmResetClearNotification, cardTable=cardTable, downstreamPowerLoss=downstreamPowerLoss, TestCommand=TestCommand, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuSpeed=cardCpuSpeed, cardSerialNum=cardSerialNum, cardTestSchedule=cardTestSchedule, cardSubType=cardSubType, cardTemperatureHighError=cardTemperatureHighError, consoleOutput=consoleOutput, cmDevice=cmDevice, cardFwVersion=cardFwVersion, cardSpareGroupId=cardSpareGroupId, equipmentError=equipmentError, trapCounter=trapCounter, portAction=portAction, trapSeverity=trapSeverity, portPrStateChange=portPrStateChange, portShelfId=portShelfId, eqDiagConfig=eqDiagConfig, equipmentCardTestGroup=equipmentCardTestGroup, cardSecStateChange=cardSecStateChange, cardLastResetReason=cardLastResetReason, portDocsIfIndex=portDocsIfIndex, portDplxStatus=portDplxStatus, portMacIfIndex=portMacIfIndex, cmResetUpTime=cmResetUpTime, portDplxStatusChange=portDplxStatusChange, TestResult=TestResult, auditId=auditId, TestType=TestType, cardTestCommand=cardTestCommand, portMode=portMode, portSecStateChange=portSecStateChange, portRxFlowControlDetected=portRxFlowControlDetected, systemKey=systemKey, cardLastBootVersion=cardLastBootVersion, portCardId=portCardId, diskDriveNumber=diskDriveNumber, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardGroup=equipmentCardGroup, auditTotalPassedCount=auditTotalPassedCount, cardBusSpeed=cardBusSpeed, cardLastBootSource=cardLastBootSource, portTxFlowControlMode=portTxFlowControlMode, equipmentTraps=equipmentTraps, diskVolumeFileName=diskVolumeFileName, auditEntry=auditEntry, auditDescription=auditDescription, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardRamSize=cardRamSize, equipmentAudit=equipmentAudit, TestTransactionId=TestTransactionId, cardTempOverHeatNotification=cardTempOverHeatNotification, systemGeneral=systemGeneral, shelfSwVersion=shelfSwVersion, portPrState=portPrState, cardCarrierSerialNum=cardCarrierSerialNum, cardTestScheduleCommand=cardTestScheduleCommand, shelfName=shelfName, portDetectedMode=portDetectedMode, auditCycleCount=auditCycleCount, equipmentState=equipmentState, equipmentDiag=equipmentDiag, cmPrimarySID=cmPrimarySID, portLastChangeTime=portLastChangeTime, portMinDsPower=portMinDsPower, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmRegistrationNotification=cmRegistrationNotification, cmUChannelID=cmUChannelID, portConnectorId=portConnectorId, portMaxDsPower=portMaxDsPower, portDescription=portDescription, cardTestIndex=cardTestIndex, systemGeneralGroup=systemGeneralGroup, cardCarrierHwVersion=cardCarrierHwVersion, portEntry=portEntry, cardPicDetected=cardPicDetected, portAdminState=portAdminState, cmResetStatus=cmResetStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, systemClock=systemClock, cardUpTime=cardUpTime, auditFailedCount=auditFailedCount, cardTestType=cardTestType, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestName=cardTestName, auditResult=auditResult, diskVolumeTrapInh=diskVolumeTrapInh, cardDplxStatus=cardDplxStatus, equipmentDiagGroup=equipmentDiagGroup, cmResetReason=cmResetReason, cmMacAddress=cmMacAddress, cardTrapInh=cardTrapInh, errEventId=errEventId, diskVolumeName=diskVolumeName, cardTemperatureHighWarn=cardTemperatureHighWarn, shelfNumber=shelfNumber, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portId=portId, diskVolumeShelfId=diskVolumeShelfId, diskVolumeDriveId=diskVolumeDriveId, diskVolumeNumber=diskVolumeNumber, cardShelfId=cardShelfId, cardPrState=cardPrState, cardAdminState=cardAdminState, portSecState=portSecState, portBgpId=portBgpId, auditCommand=auditCommand, portMacAddress=portMacAddress, portNumber=portNumber, cardPicHwVersion=cardPicHwVersion, cardDplxStatusChange=cardDplxStatusChange, cardLastChangeTime=cardLastChangeTime, auditLogOutput=auditLogOutput, cardPrStateChange=cardPrStateChange, auditLogThrottle=auditLogThrottle, auditPassedCount=auditPassedCount, cardTestDescription=cardTestDescription, diskVolumeDiskSize=diskVolumeDiskSize, cardFwUpdateStatus=cardFwUpdateStatus, cardSpareGroupMode=cardSpareGroupMode, auditAutoScheduling=auditAutoScheduling, portType=portType, cardSwVersion=cardSwVersion, cardEntry=cardEntry, auditTime=auditTime, diskVolumeUsageNotification=diskVolumeUsageNotification, auditShelfId=auditShelfId, cardType=cardType, cardDetected=cardDetected, errEvLoggingEnabled=errEvLoggingEnabled, TestScheduleCommand=TestScheduleCommand, diagTestId=diagTestId, portTrapInh=portTrapInh, portCurrDsPower=portCurrDsPower, cadEquipmentMibGroup=cadEquipmentMibGroup, cardId=cardId, cardPicSerialNum=cardPicSerialNum, cardTestTransId=cardTestTransId, auditTable=auditTable, cmResetInfo=cmResetInfo, cmResetNotification=cmResetNotification, cardTemperature=cardTemperature, equipmentTbl=equipmentTbl, remainInDiagMode=remainInDiagMode, errEvRowStatus=errEvRowStatus, equipmentShelfGroup=equipmentShelfGroup, cmIpAddress=cmIpAddress, cardFlashSize=cardFlashSize, cardTestTable=cardTestTable, cardCpuType=cardCpuType, cardCarrierFwVersion=cardCarrierFwVersion)
