...
PortMode, AdminState, SecondaryState, ShelfId, UnknownState, PortType, CardId, PicType, CardSubType, FlowControlMode, FirmwareSource, CardType, PortDetectedMode, DiskVolumeUsageLevel, EqActionType, PortId, DuplexStatus, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "PortMode", "AdminState", "SecondaryState", "ShelfId", "UnknownState", "PortType", "CardId", "PicType", "CardSubType", "FlowControlMode", "FirmwareSource", "CardType", "PortDetectedMode", "DiskVolumeUsageLevel", "EqActionType", "PortId", "DuplexStatus", "PrimaryState")
...
DisplayString, DateAndTime, TruthValue, TextualConvention, MacAddress, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "DateAndTime", "TruthValue", "TextualConvention", "MacAddress", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portSecStateChange=portSecStateChange, portDescription=portDescription, TestId=TestId, equipmentAudit=equipmentAudit, equipmentError=equipmentError, diskVolumeUsageNotification=diskVolumeUsageNotification, cmUChannelID=cmUChannelID, cadEquipmentMib=cadEquipmentMib, portType=portType, cmResetStatus=cmResetStatus, portMode=portMode, cardSwVersion=cardSwVersion, cardTestIndex=cardTestIndex, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeCardId=diskVolumeCardId, portConnectorId=portConnectorId, diskVolumeId=diskVolumeId, shelfNumber=shelfNumber, cardPicSerialNum=cardPicSerialNum, cardTestTable=cardTestTable, cardBusSpeed=cardBusSpeed, diskVolumeTable=diskVolumeTable, diskVolumeDriveId=diskVolumeDriveId, consoleOutput=consoleOutput, equipmentStateGroup=equipmentStateGroup, equipmentShelfGroup=equipmentShelfGroup, equipmentCardTestGroup=equipmentCardTestGroup, cmMacAddress=cmMacAddress, cmVendor=cmVendor, trapSeverity=trapSeverity, TestTransactionId=TestTransactionId, cardId=cardId, cmResetReason=cmResetReason, equipmentDiagGroup=equipmentDiagGroup, portRxFlowControlMode=portRxFlowControlMode, cmRegistrationNotification=cmRegistrationNotification, cardTemperature=cardTemperature, cardRamSize=cardRamSize, diskVolumeFileName=diskVolumeFileName, equipmentTbl=equipmentTbl, portCardSubType=portCardSubType, cardTestCommand=cardTestCommand, cardBootVersion=cardBootVersion, cardSubDetected=cardSubDetected, cardTestEntry=cardTestEntry, auditStatus=auditStatus, systemClock=systemClock, auditName=auditName, cmDevice=cmDevice, diskDriveNumber=diskDriveNumber, cardSpareGroupId=cardSpareGroupId, cardShelfId=cardShelfId, cardCPLDVersion=cardCPLDVersion, cardDplxStatusChange=cardDplxStatusChange, portPrStateChange=portPrStateChange, cardType=cardType, cardHwVersion=cardHwVersion, portId=portId, cardSpareGroupMode=cardSpareGroupMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, remainInDiagMode=remainInDiagMode, systemGeneralGroup=systemGeneralGroup, auditLogOutput=auditLogOutput, cardPrState=cardPrState, auditCycleCount=auditCycleCount, errEvRowStatus=errEvRowStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCpuType=cardCpuType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portShelfId=portShelfId, cardFlashSize=cardFlashSize, portCardId=portCardId, cmResetUpTime=cmResetUpTime, auditTime=auditTime, cardTestResultDesc=cardTestResultDesc, portTable=portTable, errEvLoggingEnabled=errEvLoggingEnabled, portDplxStatusChange=portDplxStatusChange, shelfObjects=shelfObjects, cadEquipmentMibGroup=cadEquipmentMibGroup, diagTestId=diagTestId, equipmentTraps=equipmentTraps, trapCounter=trapCounter, systemGeneral=systemGeneral, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portMinDsPower=portMinDsPower, auditDescription=auditDescription, auditCommand=auditCommand, portNumChans=portNumChans, equipmentState=equipmentState, verboseLevel=verboseLevel, auditEntry=auditEntry, cardDetected=cardDetected, cardPrStateChange=cardPrStateChange, cardShelfTestIndex=cardShelfTestIndex, cardLastBootVersion=cardLastBootVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, cardCarrierSerialNum=cardCarrierSerialNum, portSecState=portSecState, auditId=auditId, cardPicHwVersion=cardPicHwVersion, cardSubType=cardSubType, portTxFlowControlDetected=portTxFlowControlDetected, TestScheduleCommand=TestScheduleCommand, cmResetInfo=cmResetInfo, cardTestTime=cardTestTime, cardSerialNum=cardSerialNum, cardLastChangeTime=cardLastChangeTime, TestResult=TestResult, equipmentDiag=equipmentDiag, cardEntry=cardEntry, cardDetectedChange=cardDetectedChange, cmResetClearNotification=cmResetClearNotification, diskVolumeGroup=diskVolumeGroup, equipmentCardGroup=equipmentCardGroup, cardPicDetected=cardPicDetected, cardFwVersion=cardFwVersion, cardLastResetReason=cardLastResetReason, cardTemperatureHighError=cardTemperatureHighError, auditShelfId=auditShelfId, cardCpuSpeed=cardCpuSpeed, diskVolumeSize=diskVolumeSize, cardTestResult=cardTestResult, portRxFlowControlDetected=portRxFlowControlDetected, fpgaErrorEventEntry=fpgaErrorEventEntry, auditResult=auditResult, cardCarrierHwVersion=cardCarrierHwVersion, auditLogThrottle=auditLogThrottle, portTxFlowControlMode=portTxFlowControlMode, portDocsIfIndex=portDocsIfIndex, diskVolumeUsageLevel=diskVolumeUsageLevel, portLastChangeTime=portLastChangeTime, downstreamPowerLoss=downstreamPowerLoss, PYSNMP_MODULE_ID=cadEquipmentMib, portMaxDsPower=portMaxDsPower, cardLastBootSource=cardLastBootSource, portAdminState=portAdminState, auditTotalPassedCount=auditTotalPassedCount, cmPrimarySID=cmPrimarySID, portNumber=portNumber, cardTable=cardTable, cardTrapInh=cardTrapInh, cardTestName=cardTestName, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portBgpId=portBgpId, cardSecState=cardSecState, portMacAddress=portMacAddress, auditFailedCount=auditFailedCount, eqDiagConfig=eqDiagConfig, portPrState=portPrState, diskVolumeName=diskVolumeName, cardAdminState=cardAdminState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestResultNotification=cardTestResultNotification, cmResetNotification=cmResetNotification, equipmentAuditGroup=equipmentAuditGroup, portMacIfIndex=portMacIfIndex, cardTestTransId=cardTestTransId, cardNumber=cardNumber, equipmentPortGroup=equipmentPortGroup, cardTestDescription=cardTestDescription, diskVolumeShelfId=diskVolumeShelfId, cardFpgaSource=cardFpgaSource, portCurrDsPower=portCurrDsPower, diskVolumeEntry=diskVolumeEntry, shelfSwVersion=shelfSwVersion, cardDplxStatus=cardDplxStatus, auditPassedCount=auditPassedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, auditAutoScheduling=auditAutoScheduling, errEvLogLevel=errEvLogLevel, cardName=cardName, auditTotalFailedCount=auditTotalFailedCount, TestSchedule=TestSchedule, cardAction=cardAction, diskVolumeNumber=diskVolumeNumber, cardUpTime=cardUpTime, cardSecStateChange=cardSecStateChange, portAction=portAction, portEntry=portEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfName=shelfName, fpgaErrorEventTable=fpgaErrorEventTable, cardNumPorts=cardNumPorts, cardTestSchedule=cardTestSchedule, portTrapInh=portTrapInh, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestType=cardTestType, cardFwUpdateStatus=cardFwUpdateStatus, cmIpAddress=cmIpAddress, cardTestId=cardTestId, TestType=TestType, errEventId=errEventId, auditCardId=auditCardId, systemKey=systemKey, auditTable=auditTable, portDplxStatus=portDplxStatus, diskVolumeDiskSize=diskVolumeDiskSize, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestCommand=TestCommand, portDetectedMode=portDetectedMode)
