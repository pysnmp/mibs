...
PortId, FirmwareSource, CardId, DiskVolumeUsageLevel, FlowControlMode, PicType, EqActionType, SecondaryState, PortDetectedMode, PortMode, UnknownState, CardType, ShelfId, PrimaryState, AdminState, CardSubType, DuplexStatus, PortType = mibBuilder.importSymbols("CADANT-TC", "PortId", "FirmwareSource", "CardId", "DiskVolumeUsageLevel", "FlowControlMode", "PicType", "EqActionType", "SecondaryState", "PortDetectedMode", "PortMode", "UnknownState", "CardType", "ShelfId", "PrimaryState", "AdminState", "CardSubType", "DuplexStatus", "PortType")
...
TextualConvention, RowStatus, TimeStamp, MacAddress, DateAndTime, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "TimeStamp", "MacAddress", "DateAndTime", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastBootSource=cardLastBootSource, portPrStateChange=portPrStateChange, equipmentState=equipmentState, portLastChangeTime=portLastChangeTime, cardFpgaSource=cardFpgaSource, cardAdminState=cardAdminState, portType=portType, auditLogOutput=auditLogOutput, cardSpareGroupId=cardSpareGroupId, equipmentAudit=equipmentAudit, TestCommand=TestCommand, auditCommand=auditCommand, cmResetReason=cmResetReason, diskVolumeName=diskVolumeName, cardCarrierFwVersion=cardCarrierFwVersion, portTxFlowControlMode=portTxFlowControlMode, auditTable=auditTable, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTable=cardTable, diskVolumeNumber=diskVolumeNumber, auditCardId=auditCardId, cardSubDetected=cardSubDetected, errEventId=errEventId, diskVolumeEntry=diskVolumeEntry, cmIpAddress=cmIpAddress, TestResult=TestResult, cardBootVersion=cardBootVersion, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardDetected=cardDetected, cardTestResult=cardTestResult, auditStatus=auditStatus, portDocsIfIndex=portDocsIfIndex, portNumber=portNumber, cardLastChangeTime=cardLastChangeTime, cmResetInfo=cmResetInfo, cardSecStateChange=cardSecStateChange, portSecStateChange=portSecStateChange, cardLastResetReason=cardLastResetReason, verboseLevel=verboseLevel, portAction=portAction, portSecState=portSecState, cardFwUpdateStatus=cardFwUpdateStatus, portCardId=portCardId, cardFlashSize=cardFlashSize, portBgpId=portBgpId, errEvLogLevel=errEvLogLevel, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, PYSNMP_MODULE_ID=cadEquipmentMib, portNumChans=portNumChans, auditFailedCount=auditFailedCount, equipmentDiagGroup=equipmentDiagGroup, diskVolumeTable=diskVolumeTable, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeDiskSize=diskVolumeDiskSize, portMode=portMode, equipmentAuditGroup=equipmentAuditGroup, cardCpuType=cardCpuType, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, diskDriveNumber=diskDriveNumber, cardDplxStatusChange=cardDplxStatusChange, equipmentDiag=equipmentDiag, portDescription=portDescription, cardRamSize=cardRamSize, equipmentShelfGroup=equipmentShelfGroup, cardTestIndex=cardTestIndex, cardPicHwVersion=cardPicHwVersion, fpgaErrorEventTable=fpgaErrorEventTable, cardId=cardId, auditTime=auditTime, portMaxDsPower=portMaxDsPower, diskVolumeSize=diskVolumeSize, auditAutoScheduling=auditAutoScheduling, systemKey=systemKey, errEvRowStatus=errEvRowStatus, auditPassedCount=auditPassedCount, cardShelfTestIndex=cardShelfTestIndex, shelfObjects=shelfObjects, cardNumPorts=cardNumPorts, cardUpTime=cardUpTime, cmResetNotification=cmResetNotification, cardCarrierHwVersion=cardCarrierHwVersion, cardTrapInh=cardTrapInh, diskVolumeId=diskVolumeId, cardTestTransId=cardTestTransId, auditName=auditName, equipmentError=equipmentError, diagTestId=diagTestId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditCycleCount=auditCycleCount, portCurrDsPower=portCurrDsPower, auditTotalFailedCount=auditTotalFailedCount, cardType=cardType, auditDescription=auditDescription, cadEquipmentMibCompliance=cadEquipmentMibCompliance, trapSeverity=trapSeverity, equipmentStateGroup=equipmentStateGroup, cardSecState=cardSecState, diskVolumeShelfId=diskVolumeShelfId, diskVolumeGroup=diskVolumeGroup, cardBusSpeed=cardBusSpeed, TestId=TestId, cardPrState=cardPrState, cardTemperature=cardTemperature, cardTemperatureHighWarn=cardTemperatureHighWarn, portDplxStatusChange=portDplxStatusChange, eqDiagConfig=eqDiagConfig, portConnectorId=portConnectorId, cmPrimarySID=cmPrimarySID, cmResetStatus=cmResetStatus, cardTestName=cardTestName, diskVolumeUsageLevel=diskVolumeUsageLevel, downstreamPowerLoss=downstreamPowerLoss, cardTestTable=cardTestTable, portMacIfIndex=portMacIfIndex, portTable=portTable, cadEquipmentMib=cadEquipmentMib, cardSubType=cardSubType, portAdminState=portAdminState, cardTestTime=cardTestTime, shelfName=shelfName, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeCardId=diskVolumeCardId, cmVendor=cmVendor, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestEntry=cardTestEntry, cardSerialNum=cardSerialNum, cardHwVersion=cardHwVersion, cardTestResultNotification=cardTestResultNotification, diskVolumeDriveId=diskVolumeDriveId, cmDevice=cmDevice, shelfSwVersion=shelfSwVersion, auditShelfId=auditShelfId, portMacAddress=portMacAddress, cardTestDescription=cardTestDescription, trapCounter=trapCounter, cardCarrierSerialNum=cardCarrierSerialNum, portCardSubType=portCardSubType, auditLogThrottle=auditLogThrottle, portTrapInh=portTrapInh, TestScheduleCommand=TestScheduleCommand, cmUChannelID=cmUChannelID, cardNumber=cardNumber, cardEntry=cardEntry, remainInDiagMode=remainInDiagMode, cmRegistrationNotification=cmRegistrationNotification, cardTestSchedule=cardTestSchedule, cardPicDetected=cardPicDetected, portShelfId=portShelfId, cardTestResultDesc=cardTestResultDesc, TestType=TestType, auditId=auditId, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetClearNotification=cmResetClearNotification, equipmentCardGroup=equipmentCardGroup, equipmentCardTestGroup=equipmentCardTestGroup, equipmentTbl=equipmentTbl, systemClock=systemClock, systemGeneral=systemGeneral, shelfNumber=shelfNumber, portDplxStatus=portDplxStatus, cardAction=cardAction, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageNotification=diskVolumeUsageNotification, portDetectedMode=portDetectedMode, fpgaErrorEventGroup=fpgaErrorEventGroup, portId=portId, cardShelfId=cardShelfId, auditResult=auditResult, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardCPLDVersion=cardCPLDVersion, cardLastBootVersion=cardLastBootVersion, cardTestId=cardTestId, cmMacAddress=cmMacAddress, diskVolumeFileName=diskVolumeFileName, cardPrStateChange=cardPrStateChange, cardDplxStatus=cardDplxStatus, equipmentPortGroup=equipmentPortGroup, cardName=cardName, portEntry=portEntry, TestTransactionId=TestTransactionId, cardTempOverHeatNotification=cardTempOverHeatNotification, portPrState=portPrState, portTxFlowControlDetected=portTxFlowControlDetected, cardDetectedChange=cardDetectedChange, cmResetUpTime=cmResetUpTime, cardCpuSpeed=cardCpuSpeed, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemGeneralGroup=systemGeneralGroup, cardTemperatureHighError=cardTemperatureHighError, consoleOutput=consoleOutput, diskVolumeTrapInh=diskVolumeTrapInh, cardTestType=cardTestType, TestSchedule=TestSchedule, cardSwVersion=cardSwVersion, portMinDsPower=portMinDsPower, cardTestCommand=cardTestCommand, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlMode=portRxFlowControlMode, portRxFlowControlDetected=portRxFlowControlDetected, auditEntry=auditEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentTraps=equipmentTraps, cardFwVersion=cardFwVersion, cardSpareGroupMode=cardSpareGroupMode)
