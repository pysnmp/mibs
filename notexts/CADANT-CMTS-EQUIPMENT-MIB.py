...
UnknownState, CardId, DuplexStatus, CardType, PortType, PortMode, ShelfId, FlowControlMode, SecondaryState, FirmwareSource, PicType, AdminState, CardSubType, PrimaryState, DiskVolumeUsageLevel, PortId, PortDetectedMode, EqActionType = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "CardId", "DuplexStatus", "CardType", "PortType", "PortMode", "ShelfId", "FlowControlMode", "SecondaryState", "FirmwareSource", "PicType", "AdminState", "CardSubType", "PrimaryState", "DiskVolumeUsageLevel", "PortId", "PortDetectedMode", "EqActionType")
...
TruthValue, DisplayString, DateAndTime, MacAddress, RowStatus, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DisplayString", "DateAndTime", "MacAddress", "RowStatus", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestType=TestType, PYSNMP_MODULE_ID=cadEquipmentMib, systemClock=systemClock, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestSchedule=TestSchedule, portType=portType, errEvRecoveryEnabled=errEvRecoveryEnabled, cardRamSize=cardRamSize, cardCPLDVersion=cardCPLDVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cardDetectedChange=cardDetectedChange, TestResult=TestResult, diskVolumeTable=diskVolumeTable, auditDescription=auditDescription, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardName=cardName, equipmentError=equipmentError, cmResetNotification=cmResetNotification, portPrStateChange=portPrStateChange, equipmentCardGroup=equipmentCardGroup, portSecState=portSecState, cmUChannelID=cmUChannelID, cardCarrierSerialNum=cardCarrierSerialNum, verboseLevel=verboseLevel, equipmentTbl=equipmentTbl, TestCommand=TestCommand, cardHwVersion=cardHwVersion, equipmentDiag=equipmentDiag, portTrapInh=portTrapInh, equipmentState=equipmentState, cmResetUpTime=cmResetUpTime, cardPicHwVersion=cardPicHwVersion, portCardSubType=portCardSubType, portConnectorId=portConnectorId, cardCarrierHwVersion=cardCarrierHwVersion, cardSerialNum=cardSerialNum, cardCarrierFwVersion=cardCarrierFwVersion, cardTestId=cardTestId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardAdminState=cardAdminState, cardSecState=cardSecState, cardSpareGroupMode=cardSpareGroupMode, portCardId=portCardId, portNumChans=portNumChans, portAdminState=portAdminState, cadEquipmentMibGroup=cadEquipmentMibGroup, systemGeneralGroup=systemGeneralGroup, auditCycleCount=auditCycleCount, TestTransactionId=TestTransactionId, cmMacAddress=cmMacAddress, cardPicSerialNum=cardPicSerialNum, diagTestId=diagTestId, errEvLoggingEnabled=errEvLoggingEnabled, errEvLogLevel=errEvLogLevel, cardTemperatureHighError=cardTemperatureHighError, cardTable=cardTable, diskVolumeSize=diskVolumeSize, diskVolumeCardId=diskVolumeCardId, cardTestIndex=cardTestIndex, cardShelfId=cardShelfId, portMinDsPower=portMinDsPower, cardTestEntry=cardTestEntry, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardNumPorts=cardNumPorts, downstreamPowerLoss=downstreamPowerLoss, portDescription=portDescription, portMacIfIndex=portMacIfIndex, cardFwUpdateStatus=cardFwUpdateStatus, cardPicDetected=cardPicDetected, remainInDiagMode=remainInDiagMode, trapCounter=trapCounter, shelfName=shelfName, diskVolumeShelfId=diskVolumeShelfId, diskVolumeGroup=diskVolumeGroup, portEntry=portEntry, cardTemperature=cardTemperature, portSecStateChange=portSecStateChange, cardSubDetected=cardSubDetected, portTable=portTable, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeDiskSize=diskVolumeDiskSize, portDplxStatusChange=portDplxStatusChange, auditResult=auditResult, cardTestDescription=cardTestDescription, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTotalFailedCount=auditTotalFailedCount, equipmentDiagGroup=equipmentDiagGroup, auditLogThrottle=auditLogThrottle, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portLastChangeTime=portLastChangeTime, diskVolumeNumber=diskVolumeNumber, cardTestResultDesc=cardTestResultDesc, cardTestName=cardTestName, cardPrState=cardPrState, portPrState=portPrState, portTxFlowControlDetected=portTxFlowControlDetected, systemGeneral=systemGeneral, trapSeverity=trapSeverity, cardType=cardType, cardCpuType=cardCpuType, portMode=portMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentAudit=equipmentAudit, errEventId=errEventId, errEvRowStatus=errEvRowStatus, cardFpgaSource=cardFpgaSource, portRxFlowControlMode=portRxFlowControlMode, auditEntry=auditEntry, equipmentAuditGroup=equipmentAuditGroup, equipmentPortGroup=equipmentPortGroup, equipmentShelfGroup=equipmentShelfGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, shelfNumber=shelfNumber, cardFwVersion=cardFwVersion, auditTable=auditTable, portId=portId, cardTestScheduleCommand=cardTestScheduleCommand, cardUpTime=cardUpTime, cardFlashSize=cardFlashSize, portDetectedMode=portDetectedMode, cardTestResult=cardTestResult, auditStatus=auditStatus, cardCpuSpeed=cardCpuSpeed, portBgpId=portBgpId, diskVolumeName=diskVolumeName, cardTestTime=cardTestTime, fpgaErrorEventTable=fpgaErrorEventTable, auditShelfId=auditShelfId, cmPrimarySID=cmPrimarySID, auditFailedCount=auditFailedCount, cadEquipmentMib=cadEquipmentMib, portMaxDsPower=portMaxDsPower, cardPrStateChange=cardPrStateChange, cardSecStateChange=cardSecStateChange, cmResetInfo=cmResetInfo, cardLastBootVersion=cardLastBootVersion, cmIpAddress=cmIpAddress, cardLastChangeTime=cardLastChangeTime, cardSubType=cardSubType, auditPassedCount=auditPassedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, cardAction=cardAction, cardTrapInh=cardTrapInh, cardLastBootSource=cardLastBootSource, auditCardId=auditCardId, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentStateGroup=equipmentStateGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestType=cardTestType, auditLogOutput=auditLogOutput, portRxFlowControlDetected=portRxFlowControlDetected, consoleOutput=consoleOutput, equipmentCardTestGroup=equipmentCardTestGroup, cardLastResetReason=cardLastResetReason, diskVolumeId=diskVolumeId, portCurrDsPower=portCurrDsPower, cmResetReason=cmResetReason, diskVolumeFileName=diskVolumeFileName, cardBusSpeed=cardBusSpeed, auditTime=auditTime, cardTempNoReportNotification=cardTempNoReportNotification, portMacAddress=portMacAddress, cmRegistrationNotification=cmRegistrationNotification, cmDevice=cmDevice, cardTestSchedule=cardTestSchedule, auditAutoScheduling=auditAutoScheduling, shelfObjects=shelfObjects, auditCommand=auditCommand, cardDetected=cardDetected, eqDiagConfig=eqDiagConfig, TestScheduleCommand=TestScheduleCommand, cardTestCommand=cardTestCommand, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardEntry=cardEntry, cardTestResultNotification=cardTestResultNotification, cardId=cardId, portShelfId=portShelfId, cardNumber=cardNumber, cardSwVersion=cardSwVersion, auditName=auditName, portDplxStatus=portDplxStatus, cardBootVersion=cardBootVersion, cardShelfTestIndex=cardShelfTestIndex, cardSpareGroupId=cardSpareGroupId, TestId=TestId, cmResetStatus=cmResetStatus, diskDriveNumber=diskDriveNumber, systemKey=systemKey, cmResetClearNotification=cmResetClearNotification, portAction=portAction, diskVolumeUsageLevel=diskVolumeUsageLevel, auditId=auditId, portDocsIfIndex=portDocsIfIndex, cmVendor=cmVendor, shelfSwVersion=shelfSwVersion, portNumber=portNumber, cardDplxStatus=cardDplxStatus, portTxFlowControlMode=portTxFlowControlMode, cardTestTable=cardTestTable, cardTestTransId=cardTestTransId, auditTotalPassedCount=auditTotalPassedCount, equipmentTraps=equipmentTraps, cardDplxStatusChange=cardDplxStatusChange)
