...
FirmwareSource, DiskVolumeUsageLevel, PortDetectedMode, PrimaryState, SecondaryState, AdminState, PortMode, DuplexStatus, CardType, EqActionType, CardId, CardSubType, PicType, PortType, UnknownState, FlowControlMode, ShelfId, PortId = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "DiskVolumeUsageLevel", "PortDetectedMode", "PrimaryState", "SecondaryState", "AdminState", "PortMode", "DuplexStatus", "CardType", "EqActionType", "CardId", "CardSubType", "PicType", "PortType", "UnknownState", "FlowControlMode", "ShelfId", "PortId")
...
TextualConvention, DateAndTime, DisplayString, TimeStamp, TruthValue, RowStatus, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "DisplayString", "TimeStamp", "TruthValue", "RowStatus", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", consoleOutput=consoleOutput, portSecState=portSecState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetInfo=cmResetInfo, errEvRecoveryEnabled=errEvRecoveryEnabled, auditEntry=auditEntry, cmMacAddress=cmMacAddress, cardHwVersion=cardHwVersion, cardRamSize=cardRamSize, cardTemperatureHighError=cardTemperatureHighError, portSecStateChange=portSecStateChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, eqDiagConfig=eqDiagConfig, cardTestTransId=cardTestTransId, portId=portId, cardPicSerialNum=cardPicSerialNum, portLastChangeTime=portLastChangeTime, portPrState=portPrState, portTable=portTable, portCurrDsPower=portCurrDsPower, auditId=auditId, cardBootVersion=cardBootVersion, cmResetUpTime=cmResetUpTime, systemKey=systemKey, auditStatus=auditStatus, portTrapInh=portTrapInh, cmResetReason=cmResetReason, shelfSwVersion=shelfSwVersion, portEntry=portEntry, cardEntry=cardEntry, portDetectedMode=portDetectedMode, diskVolumeId=diskVolumeId, TestType=TestType, cmDevice=cmDevice, diskVolumeTable=diskVolumeTable, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibGroup=cadEquipmentMibGroup, auditPassedCount=auditPassedCount, cardCPLDVersion=cardCPLDVersion, equipmentTraps=equipmentTraps, portRxFlowControlDetected=portRxFlowControlDetected, portTxFlowControlMode=portTxFlowControlMode, cardAdminState=cardAdminState, auditShelfId=auditShelfId, equipmentDiagGroup=equipmentDiagGroup, cardDetectedChange=cardDetectedChange, diskVolumeEntry=diskVolumeEntry, cardFpgaSource=cardFpgaSource, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPrState=cardPrState, diskVolumeName=diskVolumeName, errEvRowStatus=errEvRowStatus, TestResult=TestResult, cardTestSchedule=cardTestSchedule, cardType=cardType, cardTestType=cardTestType, cardSecStateChange=cardSecStateChange, trapSeverity=trapSeverity, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditLogOutput=auditLogOutput, portDescription=portDescription, remainInDiagMode=remainInDiagMode, cardTestResultDesc=cardTestResultDesc, auditResult=auditResult, diskVolumeNumber=diskVolumeNumber, portConnectorId=portConnectorId, portDplxStatusChange=portDplxStatusChange, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeCardId=diskVolumeCardId, errEvLoggingEnabled=errEvLoggingEnabled, cardTestScheduleCommand=cardTestScheduleCommand, portCardSubType=portCardSubType, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeTrapInh=diskVolumeTrapInh, cardLastResetReason=cardLastResetReason, portDplxStatus=portDplxStatus, auditCycleCount=auditCycleCount, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSubType=cardSubType, diskVolumeDiskSize=diskVolumeDiskSize, diagTestId=diagTestId, cardDetected=cardDetected, equipmentAudit=equipmentAudit, portAction=portAction, portShelfId=portShelfId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardShelfTestIndex=cardShelfTestIndex, auditCardId=auditCardId, cmRegistrationNotification=cmRegistrationNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentCardTestGroup=equipmentCardTestGroup, cardTestName=cardTestName, cardTestResultNotification=cardTestResultNotification, cardSwVersion=cardSwVersion, portType=portType, systemGeneralGroup=systemGeneralGroup, cardDplxStatusChange=cardDplxStatusChange, errEvLogLevel=errEvLogLevel, cardTestResult=cardTestResult, equipmentStateGroup=equipmentStateGroup, cardTestEntry=cardTestEntry, cardTestIndex=cardTestIndex, auditTotalFailedCount=auditTotalFailedCount, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierHwVersion=cardCarrierHwVersion, TestCommand=TestCommand, portMaxDsPower=portMaxDsPower, auditDescription=auditDescription, portMacIfIndex=portMacIfIndex, diskVolumeGroup=diskVolumeGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSpareGroupMode=cardSpareGroupMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentError=equipmentError, cmResetStatus=cmResetStatus, auditTotalPassedCount=auditTotalPassedCount, cardTestId=cardTestId, cardLastChangeTime=cardLastChangeTime, diskVolumeFileName=diskVolumeFileName, equipmentShelfGroup=equipmentShelfGroup, TestScheduleCommand=TestScheduleCommand, cardSerialNum=cardSerialNum, cardSubDetected=cardSubDetected, equipmentCardGroup=equipmentCardGroup, cmVendor=cmVendor, cmResetClearNotification=cmResetClearNotification, cardCpuSpeed=cardCpuSpeed, cardLastBootSource=cardLastBootSource, auditTable=auditTable, cardDplxStatus=cardDplxStatus, cardSecState=cardSecState, cardNumPorts=cardNumPorts, portMode=portMode, diskVolumeDriveId=diskVolumeDriveId, verboseLevel=verboseLevel, portBgpId=portBgpId, cardLastBootVersion=cardLastBootVersion, cmIpAddress=cmIpAddress, diskVolumeUsageNotification=diskVolumeUsageNotification, cardAction=cardAction, systemClock=systemClock, cardCarrierFwVersion=cardCarrierFwVersion, portNumber=portNumber, cadEquipmentMib=cadEquipmentMib, cardCpuType=cardCpuType, portDocsIfIndex=portDocsIfIndex, cardFlashSize=cardFlashSize, auditLogThrottle=auditLogThrottle, equipmentPortGroup=equipmentPortGroup, portNumChans=portNumChans, trapCounter=trapCounter, downstreamPowerLoss=downstreamPowerLoss, errEventId=errEventId, diskDriveNumber=diskDriveNumber, cmPrimarySID=cmPrimarySID, cardShelfId=cardShelfId, portRxFlowControlMode=portRxFlowControlMode, cardTestCommand=cardTestCommand, cardFwUpdateStatus=cardFwUpdateStatus, cmUChannelID=cmUChannelID, equipmentDiag=equipmentDiag, cardName=cardName, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentAuditGroup=equipmentAuditGroup, cardPicDetected=cardPicDetected, portCardId=portCardId, auditFailedCount=auditFailedCount, portMacAddress=portMacAddress, cmResetNotification=cmResetNotification, cardNumber=cardNumber, portAdminState=portAdminState, auditCommand=auditCommand, cardPrStateChange=cardPrStateChange, cardTestDescription=cardTestDescription, auditName=auditName, TestTransactionId=TestTransactionId, equipmentTbl=equipmentTbl, shelfNumber=shelfNumber, cardTemperature=cardTemperature, cardPicHwVersion=cardPicHwVersion, auditTime=auditTime, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestId=TestId, portMinDsPower=portMinDsPower, cardBusSpeed=cardBusSpeed, systemGeneral=systemGeneral, shelfObjects=shelfObjects, cardTrapInh=cardTrapInh, equipmentState=equipmentState, cardFwVersion=cardFwVersion, shelfName=shelfName, cardTable=cardTable, cardSpareGroupId=cardSpareGroupId, cardUpTime=cardUpTime, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeSize=diskVolumeSize, cardCarrierSerialNum=cardCarrierSerialNum, cardTestTime=cardTestTime, cardTempNoReportNotification=cardTempNoReportNotification, cardTestTable=cardTestTable, diskVolumeShelfId=diskVolumeShelfId, portPrStateChange=portPrStateChange, cardId=cardId, TestSchedule=TestSchedule)
