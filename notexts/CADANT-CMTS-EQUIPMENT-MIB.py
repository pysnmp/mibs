...
CardSubType, DiskVolumeUsageLevel, FlowControlMode, PortDetectedMode, PrimaryState, CardId, PortId, DuplexStatus, ShelfId, FirmwareSource, SecondaryState, EqActionType, UnknownState, CardType, PicType, AdminState, PortType, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "DiskVolumeUsageLevel", "FlowControlMode", "PortDetectedMode", "PrimaryState", "CardId", "PortId", "DuplexStatus", "ShelfId", "FirmwareSource", "SecondaryState", "EqActionType", "UnknownState", "CardType", "PicType", "AdminState", "PortType", "PortMode")
...
TextualConvention, RowStatus, TruthValue, DateAndTime, TimeStamp, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "TruthValue", "DateAndTime", "TimeStamp", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTempOverHeatNotification=cardTempOverHeatNotification, portNumber=portNumber, cardFlashSize=cardFlashSize, portDescription=portDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetInfo=cmResetInfo, portId=portId, trapSeverity=trapSeverity, cardShelfId=cardShelfId, fpgaErrorEventTable=fpgaErrorEventTable, errEvRowStatus=errEvRowStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardNumber=cardNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentError=equipmentError, equipmentStateGroup=equipmentStateGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardPicHwVersion=cardPicHwVersion, cardTestIndex=cardTestIndex, portTable=portTable, cardLastChangeTime=cardLastChangeTime, portMacIfIndex=portMacIfIndex, auditDescription=auditDescription, auditPassedCount=auditPassedCount, cardLastResetReason=cardLastResetReason, portMode=portMode, cardTestResult=cardTestResult, cardAdminState=cardAdminState, portCardSubType=portCardSubType, TestId=TestId, portRxFlowControlMode=portRxFlowControlMode, portRxFlowControlDetected=portRxFlowControlDetected, portTrapInh=portTrapInh, equipmentTbl=equipmentTbl, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditShelfId=auditShelfId, cardCpuType=cardCpuType, cardPrStateChange=cardPrStateChange, cardTemperatureHighError=cardTemperatureHighError, cmResetNotification=cmResetNotification, cardTempNoReportNotification=cardTempNoReportNotification, cmResetReason=cmResetReason, equipmentAuditGroup=equipmentAuditGroup, cmDevice=cmDevice, cardSerialNum=cardSerialNum, auditEntry=auditEntry, diskVolumeNumber=diskVolumeNumber, portCardId=portCardId, cardSubType=cardSubType, equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeSize=diskVolumeSize, portConnectorId=portConnectorId, diskDriveNumber=diskDriveNumber, cardTestName=cardTestName, cardPicDetected=cardPicDetected, cardTestTransId=cardTestTransId, shelfNumber=shelfNumber, cardTrapInh=cardTrapInh, cardBusSpeed=cardBusSpeed, cardTestCommand=cardTestCommand, cardPrState=cardPrState, portType=portType, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestResultDesc=cardTestResultDesc, diskVolumeName=diskVolumeName, verboseLevel=verboseLevel, cardTestId=cardTestId, auditId=auditId, portDocsIfIndex=portDocsIfIndex, cardTemperatureHighWarn=cardTemperatureHighWarn, portPrState=portPrState, diskVolumeUsageNotification=diskVolumeUsageNotification, systemClock=systemClock, equipmentAudit=equipmentAudit, TestCommand=TestCommand, portShelfId=portShelfId, equipmentDiagGroup=equipmentDiagGroup, PYSNMP_MODULE_ID=cadEquipmentMib, cardUpTime=cardUpTime, consoleOutput=consoleOutput, downstreamPowerLoss=downstreamPowerLoss, cardFpgaSource=cardFpgaSource, cardDplxStatusChange=cardDplxStatusChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardHwVersion=cardHwVersion, equipmentCardGroup=equipmentCardGroup, shelfSwVersion=shelfSwVersion, auditTime=auditTime, errEventId=errEventId, cmMacAddress=cmMacAddress, trapCounter=trapCounter, portDetectedMode=portDetectedMode, TestScheduleCommand=TestScheduleCommand, cardAction=cardAction, cardTable=cardTable, cardDetected=cardDetected, portMaxDsPower=portMaxDsPower, auditFailedCount=auditFailedCount, portPrStateChange=portPrStateChange, fpgaErrorEventGroup=fpgaErrorEventGroup, portDplxStatus=portDplxStatus, systemKey=systemKey, cardShelfTestIndex=cardShelfTestIndex, systemGeneral=systemGeneral, cardSpareGroupMode=cardSpareGroupMode, cardName=cardName, cmRegistrationNotification=cmRegistrationNotification, cardBootVersion=cardBootVersion, cardLastBootVersion=cardLastBootVersion, cardLastBootSource=cardLastBootSource, auditTotalPassedCount=auditTotalPassedCount, cardTestScheduleCommand=cardTestScheduleCommand, cardPicSerialNum=cardPicSerialNum, cardSwVersion=cardSwVersion, portAction=portAction, cardNumPorts=cardNumPorts, portAdminState=portAdminState, cardSubDetected=cardSubDetected, cardTestSchedule=cardTestSchedule, cardDetectedChange=cardDetectedChange, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSpareGroupId=cardSpareGroupId, cardId=cardId, portEntry=portEntry, portLastChangeTime=portLastChangeTime, cardCarrierHwVersion=cardCarrierHwVersion, portSecState=portSecState, auditCardId=auditCardId, cardCPLDVersion=cardCPLDVersion, equipmentTraps=equipmentTraps, remainInDiagMode=remainInDiagMode, shelfObjects=shelfObjects, cadEquipmentMib=cadEquipmentMib, cardTemperature=cardTemperature, diskVolumeEntry=diskVolumeEntry, diskVolumeTrapInh=diskVolumeTrapInh, cmResetUpTime=cmResetUpTime, portDplxStatusChange=portDplxStatusChange, portMinDsPower=portMinDsPower, auditTable=auditTable, cmIpAddress=cmIpAddress, equipmentShelfGroup=equipmentShelfGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, errEvLoggingEnabled=errEvLoggingEnabled, portCurrDsPower=portCurrDsPower, cmVendor=cmVendor, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeGroup=diskVolumeGroup, auditLogOutput=auditLogOutput, cardEntry=cardEntry, cardTestEntry=cardTestEntry, diskVolumeDriveId=diskVolumeDriveId, diskVolumeFileName=diskVolumeFileName, cardRamSize=cardRamSize, auditTotalFailedCount=auditTotalFailedCount, cmResetStatus=cmResetStatus, cardTestResultNotification=cardTestResultNotification, cardCpuSpeed=cardCpuSpeed, cmUChannelID=cmUChannelID, portNumChans=portNumChans, diskVolumeTable=diskVolumeTable, diskVolumeShelfId=diskVolumeShelfId, cardSecStateChange=cardSecStateChange, cardTestTable=cardTestTable, portSecStateChange=portSecStateChange, systemGeneralGroup=systemGeneralGroup, TestTransactionId=TestTransactionId, equipmentPortGroup=equipmentPortGroup, auditName=auditName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diagTestId=diagTestId, cardFwVersion=cardFwVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestSchedule=TestSchedule, cmResetClearNotification=cmResetClearNotification, errEvLogLevel=errEvLogLevel, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfName=shelfName, cardDplxStatus=cardDplxStatus, cardTestDescription=cardTestDescription, portTxFlowControlDetected=portTxFlowControlDetected, cardTestType=cardTestType, TestResult=TestResult, portTxFlowControlMode=portTxFlowControlMode, eqDiagConfig=eqDiagConfig, TestType=TestType, auditLogThrottle=auditLogThrottle, cardType=cardType, diskVolumeId=diskVolumeId, auditResult=auditResult, portMacAddress=portMacAddress, auditAutoScheduling=auditAutoScheduling, equipmentDiag=equipmentDiag, auditCommand=auditCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeDiskSize=diskVolumeDiskSize, cardCarrierFwVersion=cardCarrierFwVersion, cardSecState=cardSecState, equipmentCardTestGroup=equipmentCardTestGroup, cardTestTime=cardTestTime, cmPrimarySID=cmPrimarySID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portBgpId=portBgpId, auditStatus=auditStatus, auditCycleCount=auditCycleCount)
