...
CardType, ShelfId, UnknownState, CardId, PortMode, PortDetectedMode, DiskVolumeUsageLevel, PicType, PrimaryState, SecondaryState, FlowControlMode, DuplexStatus, AdminState, FirmwareSource, PortId, EqActionType, CardSubType, PortType = mibBuilder.importSymbols("CADANT-TC", "CardType", "ShelfId", "UnknownState", "CardId", "PortMode", "PortDetectedMode", "DiskVolumeUsageLevel", "PicType", "PrimaryState", "SecondaryState", "FlowControlMode", "DuplexStatus", "AdminState", "FirmwareSource", "PortId", "EqActionType", "CardSubType", "PortType")
...
TextualConvention, MacAddress, RowStatus, TruthValue, DisplayString, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "RowStatus", "TruthValue", "DisplayString", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portType=portType, auditDescription=auditDescription, cmResetUpTime=cmResetUpTime, equipmentCardGroup=equipmentCardGroup, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeFileName=diskVolumeFileName, auditLogThrottle=auditLogThrottle, cardDplxStatus=cardDplxStatus, cardTestType=cardTestType, equipmentCardTestGroup=equipmentCardTestGroup, cardPicSerialNum=cardPicSerialNum, equipmentDiagGroup=equipmentDiagGroup, TestResult=TestResult, cardTestTransId=cardTestTransId, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMib=cadEquipmentMib, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTrapInh=cardTrapInh, portRxFlowControlMode=portRxFlowControlMode, systemKey=systemKey, cardName=cardName, consoleOutput=consoleOutput, shelfObjects=shelfObjects, shelfNumber=shelfNumber, portNumChans=portNumChans, cmResetReason=cmResetReason, portPrStateChange=portPrStateChange, diskVolumeId=diskVolumeId, cardAction=cardAction, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, systemGeneral=systemGeneral, fpgaErrorEventGroup=fpgaErrorEventGroup, portMaxDsPower=portMaxDsPower, portDetectedMode=portDetectedMode, errEventId=errEventId, portAdminState=portAdminState, auditEntry=auditEntry, cardTemperature=cardTemperature, cardSubType=cardSubType, portMacIfIndex=portMacIfIndex, cardTestCommand=cardTestCommand, auditTime=auditTime, auditCommand=auditCommand, cmPrimarySID=cmPrimarySID, cardCpuSpeed=cardCpuSpeed, portMinDsPower=portMinDsPower, portCardSubType=portCardSubType, auditId=auditId, cardBootVersion=cardBootVersion, cardTestName=cardTestName, cardTestResultNotification=cardTestResultNotification, diskVolumeName=diskVolumeName, equipmentAuditGroup=equipmentAuditGroup, auditLogOutput=auditLogOutput, diagTestId=diagTestId, cardType=cardType, cardSerialNum=cardSerialNum, diskVolumeTable=diskVolumeTable, cardTestTime=cardTestTime, TestSchedule=TestSchedule, cardDetectedChange=cardDetectedChange, auditName=auditName, trapCounter=trapCounter, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeDiskSize=diskVolumeDiskSize, auditTable=auditTable, portDplxStatusChange=portDplxStatusChange, portTable=portTable, cardTestScheduleCommand=cardTestScheduleCommand, cardTemperatureHighError=cardTemperatureHighError, equipmentStateGroup=equipmentStateGroup, auditCardId=auditCardId, downstreamPowerLoss=downstreamPowerLoss, cardCPLDVersion=cardCPLDVersion, portDocsIfIndex=portDocsIfIndex, cmResetNotification=cmResetNotification, cmResetStatus=cmResetStatus, cardSpareGroupId=cardSpareGroupId, verboseLevel=verboseLevel, portNumber=portNumber, cmMacAddress=cmMacAddress, portCardId=portCardId, cardPicDetected=cardPicDetected, cmDevice=cmDevice, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardFlashSize=cardFlashSize, cmResetClearNotification=cmResetClearNotification, auditCycleCount=auditCycleCount, portConnectorId=portConnectorId, diskVolumeSize=diskVolumeSize, errEvLogLevel=errEvLogLevel, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, systemGeneralGroup=systemGeneralGroup, portTrapInh=portTrapInh, diskVolumeGroup=diskVolumeGroup, equipmentState=equipmentState, diskVolumeUsageLevel=diskVolumeUsageLevel, fpgaErrorEventTable=fpgaErrorEventTable, auditAutoScheduling=auditAutoScheduling, cardId=cardId, cardTestId=cardTestId, equipmentPortGroup=equipmentPortGroup, shelfSwVersion=shelfSwVersion, cardBusSpeed=cardBusSpeed, cardDplxStatusChange=cardDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, portTxFlowControlDetected=portTxFlowControlDetected, cardTable=cardTable, cardLastResetReason=cardLastResetReason, equipmentError=equipmentError, cardCarrierSerialNum=cardCarrierSerialNum, cardLastBootVersion=cardLastBootVersion, diskVolumeEntry=diskVolumeEntry, portSecState=portSecState, cmVendor=cmVendor, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvRowStatus=errEvRowStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, auditResult=auditResult, cardLastChangeTime=cardLastChangeTime, equipmentShelfGroup=equipmentShelfGroup, cardNumber=cardNumber, cardLastBootSource=cardLastBootSource, shelfName=shelfName, auditStatus=auditStatus, equipmentTbl=equipmentTbl, cardTestIndex=cardTestIndex, cardPrState=cardPrState, cardNumPorts=cardNumPorts, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portDplxStatus=portDplxStatus, cardDetected=cardDetected, cardSecState=cardSecState, cardRamSize=cardRamSize, cardFwVersion=cardFwVersion, TestType=TestType, systemClock=systemClock, portCurrDsPower=portCurrDsPower, eqDiagConfig=eqDiagConfig, cardTestDescription=cardTestDescription, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentTraps=equipmentTraps, cardCarrierFwVersion=cardCarrierFwVersion, diskDriveNumber=diskDriveNumber, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSpareGroupMode=cardSpareGroupMode, trapSeverity=trapSeverity, auditTotalPassedCount=auditTotalPassedCount, cardHwVersion=cardHwVersion, auditPassedCount=auditPassedCount, cardSubDetected=cardSubDetected, portMacAddress=portMacAddress, cmRegistrationNotification=cmRegistrationNotification, cardAdminState=cardAdminState, diskVolumeShelfId=diskVolumeShelfId, cardCarrierHwVersion=cardCarrierHwVersion, portBgpId=portBgpId, cardSwVersion=cardSwVersion, cardUpTime=cardUpTime, errEvLoggingEnabled=errEvLoggingEnabled, portSecStateChange=portSecStateChange, cardShelfId=cardShelfId, cardTestTable=cardTestTable, cardTestResult=cardTestResult, cardShelfTestIndex=cardShelfTestIndex, auditFailedCount=auditFailedCount, portPrState=portPrState, portMode=portMode, cardEntry=cardEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, remainInDiagMode=remainInDiagMode, cardTempNoReportNotification=cardTempNoReportNotification, TestCommand=TestCommand, cardPicHwVersion=cardPicHwVersion, portAction=portAction, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestEntry=cardTestEntry, auditShelfId=auditShelfId, portLastChangeTime=portLastChangeTime, cadEquipmentMibConformance=cadEquipmentMibConformance, portShelfId=portShelfId, cmResetInfo=cmResetInfo, diskVolumeDriveId=diskVolumeDriveId, cardFpgaSource=cardFpgaSource, portEntry=portEntry, cmUChannelID=cmUChannelID, cmIpAddress=cmIpAddress, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeCardId=diskVolumeCardId, cardPrStateChange=cardPrStateChange, cardSecStateChange=cardSecStateChange, TestTransactionId=TestTransactionId, portId=portId, diskVolumeTrapInh=diskVolumeTrapInh, auditTotalFailedCount=auditTotalFailedCount, cardTestResultDesc=cardTestResultDesc, TestScheduleCommand=TestScheduleCommand, cardTestSchedule=cardTestSchedule, diskVolumeNumber=diskVolumeNumber, portDescription=portDescription, TestId=TestId, equipmentAudit=equipmentAudit, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portRxFlowControlDetected=portRxFlowControlDetected, equipmentDiag=equipmentDiag, cardCpuType=cardCpuType)
