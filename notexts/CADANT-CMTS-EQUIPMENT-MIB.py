...
CardSubType, FirmwareSource, PortMode, PicType, EqActionType, ShelfId, CardType, DiskVolumeUsageLevel, SecondaryState, UnknownState, CardId, PortType, PrimaryState, FlowControlMode, AdminState, PortId, PortDetectedMode, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "FirmwareSource", "PortMode", "PicType", "EqActionType", "ShelfId", "CardType", "DiskVolumeUsageLevel", "SecondaryState", "UnknownState", "CardId", "PortType", "PrimaryState", "FlowControlMode", "AdminState", "PortId", "PortDetectedMode", "DuplexStatus")
...
TruthValue, TextualConvention, DisplayString, DateAndTime, RowStatus, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TextualConvention", "DisplayString", "DateAndTime", "RowStatus", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardNumber=cardNumber, diskVolumeTrapInh=diskVolumeTrapInh, cardLastChangeTime=cardLastChangeTime, cardSerialNum=cardSerialNum, portDplxStatus=portDplxStatus, consoleOutput=consoleOutput, cardPrState=cardPrState, cardFlashSize=cardFlashSize, auditAutoScheduling=auditAutoScheduling, shelfNumber=shelfNumber, equipmentShelfGroup=equipmentShelfGroup, cardTestResultDesc=cardTestResultDesc, cmUChannelID=cmUChannelID, shelfSwVersion=shelfSwVersion, cardSwVersion=cardSwVersion, portLastChangeTime=portLastChangeTime, cardCarrierSerialNum=cardCarrierSerialNum, cardTestResult=cardTestResult, eqDiagConfig=eqDiagConfig, diskVolumeId=diskVolumeId, diskVolumeSize=diskVolumeSize, TestSchedule=TestSchedule, auditName=auditName, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCPLDVersion=cardCPLDVersion, auditCommand=auditCommand, cardTestTable=cardTestTable, auditStatus=auditStatus, auditTotalPassedCount=auditTotalPassedCount, portDplxStatusChange=portDplxStatusChange, cardAdminState=cardAdminState, auditId=auditId, auditPassedCount=auditPassedCount, cardDplxStatusChange=cardDplxStatusChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portPrState=portPrState, portRxFlowControlMode=portRxFlowControlMode, PYSNMP_MODULE_ID=cadEquipmentMib, cardSpareGroupId=cardSpareGroupId, cmResetClearNotification=cmResetClearNotification, cmResetInfo=cmResetInfo, downstreamPowerLoss=downstreamPowerLoss, portDescription=portDescription, errEvRowStatus=errEvRowStatus, cardTestTime=cardTestTime, cardTemperatureHighWarn=cardTemperatureHighWarn, shelfObjects=shelfObjects, cmMacAddress=cmMacAddress, cardCarrierHwVersion=cardCarrierHwVersion, portBgpId=portBgpId, cardSecStateChange=cardSecStateChange, cardTestName=cardTestName, cardTestIndex=cardTestIndex, equipmentTbl=equipmentTbl, portTrapInh=portTrapInh, shelfName=shelfName, fpgaErrorEventGroup=fpgaErrorEventGroup, portConnectorId=portConnectorId, cardTestTransId=cardTestTransId, cmResetUpTime=cmResetUpTime, cardNumPorts=cardNumPorts, trapSeverity=trapSeverity, auditTime=auditTime, cmVendor=cmVendor, cardTempNoReportNotification=cardTempNoReportNotification, systemClock=systemClock, portAdminState=portAdminState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardBusSpeed=cardBusSpeed, cardCpuType=cardCpuType, portNumChans=portNumChans, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTable=cardTable, cardFwVersion=cardFwVersion, diskVolumeEntry=diskVolumeEntry, cmResetReason=cmResetReason, cardTestScheduleCommand=cardTestScheduleCommand, cardTemperature=cardTemperature, diskVolumeTable=diskVolumeTable, cardTestResultNotification=cardTestResultNotification, equipmentAudit=equipmentAudit, portMacAddress=portMacAddress, cardSecState=cardSecState, portEntry=portEntry, portNumber=portNumber, auditEntry=auditEntry, systemGeneralGroup=systemGeneralGroup, cmResetStatus=cmResetStatus, cardLastBootVersion=cardLastBootVersion, diagTestId=diagTestId, portMaxDsPower=portMaxDsPower, portAction=portAction, verboseLevel=verboseLevel, cmResetNotification=cmResetNotification, cardUpTime=cardUpTime, cardSubDetected=cardSubDetected, auditShelfId=auditShelfId, cadEquipmentMib=cadEquipmentMib, errEventId=errEventId, cardDplxStatus=cardDplxStatus, cardEntry=cardEntry, cardCarrierFwVersion=cardCarrierFwVersion, portSecState=portSecState, portDetectedMode=portDetectedMode, portShelfId=portShelfId, portMinDsPower=portMinDsPower, auditResult=auditResult, auditFailedCount=auditFailedCount, equipmentError=equipmentError, portTable=portTable, systemKey=systemKey, cardPicDetected=cardPicDetected, cardDetectedChange=cardDetectedChange, TestCommand=TestCommand, diskVolumeNumber=diskVolumeNumber, TestTransactionId=TestTransactionId, remainInDiagMode=remainInDiagMode, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardAction=cardAction, cardTemperatureHighError=cardTemperatureHighError, cardTestId=cardTestId, cardTestCommand=cardTestCommand, cardLastBootSource=cardLastBootSource, portTxFlowControlMode=portTxFlowControlMode, equipmentState=equipmentState, cardFwUpdateStatus=cardFwUpdateStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, cardPrStateChange=cardPrStateChange, diskVolumeFileName=diskVolumeFileName, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentCardGroup=equipmentCardGroup, cardPicSerialNum=cardPicSerialNum, portTxFlowControlDetected=portTxFlowControlDetected, equipmentAuditGroup=equipmentAuditGroup, cardRamSize=cardRamSize, cardFpgaSource=cardFpgaSource, portId=portId, auditCycleCount=auditCycleCount, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardName=cardName, cardTestSchedule=cardTestSchedule, TestResult=TestResult, cardTrapInh=cardTrapInh, diskDriveNumber=diskDriveNumber, equipmentDiagGroup=equipmentDiagGroup, equipmentDiag=equipmentDiag, portDocsIfIndex=portDocsIfIndex, equipmentPortGroup=equipmentPortGroup, auditTotalFailedCount=auditTotalFailedCount, portSecStateChange=portSecStateChange, portCardSubType=portCardSubType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestEntry=cardTestEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmIpAddress=cmIpAddress, cmPrimarySID=cmPrimarySID, cardLastResetReason=cardLastResetReason, cardHwVersion=cardHwVersion, TestType=TestType, equipmentTraps=equipmentTraps, cardDetected=cardDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, portType=portType, diskVolumeName=diskVolumeName, portMode=portMode, cardShelfTestIndex=cardShelfTestIndex, cardId=cardId, auditDescription=auditDescription, cardTestDescription=cardTestDescription, TestScheduleCommand=TestScheduleCommand, cmDevice=cmDevice, portPrStateChange=portPrStateChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestId=TestId, systemGeneral=systemGeneral, cardTestType=cardTestType, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSpareGroupMode=cardSpareGroupMode, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeCardId=diskVolumeCardId, cardType=cardType, auditLogOutput=auditLogOutput, cmRegistrationNotification=cmRegistrationNotification, cardSubType=cardSubType, cardBootVersion=cardBootVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, portMacIfIndex=portMacIfIndex, trapCounter=trapCounter, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeDriveId=diskVolumeDriveId, cardPicHwVersion=cardPicHwVersion, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeShelfId=diskVolumeShelfId, errEvLogLevel=errEvLogLevel, equipmentStateGroup=equipmentStateGroup, auditLogThrottle=auditLogThrottle, portCurrDsPower=portCurrDsPower, diskVolumeGroup=diskVolumeGroup, cardShelfId=cardShelfId, auditTable=auditTable, auditCardId=auditCardId, cardCpuSpeed=cardCpuSpeed, portCardId=portCardId)
