...
FlowControlMode, SecondaryState, CardSubType, CardId, CardType, FirmwareSource, PortDetectedMode, PrimaryState, EqActionType, PortType, UnknownState, PicType, DuplexStatus, AdminState, PortMode, DiskVolumeUsageLevel, PortId, ShelfId = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "SecondaryState", "CardSubType", "CardId", "CardType", "FirmwareSource", "PortDetectedMode", "PrimaryState", "EqActionType", "PortType", "UnknownState", "PicType", "DuplexStatus", "AdminState", "PortMode", "DiskVolumeUsageLevel", "PortId", "ShelfId")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPrState=cardPrState, portDetectedMode=portDetectedMode, cmDevice=cmDevice, diskVolumeDriveId=diskVolumeDriveId, auditId=auditId, cmResetClearNotification=cmResetClearNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, auditLogOutput=auditLogOutput, cardTrapInh=cardTrapInh, cardTempNoReportNotification=cardTempNoReportNotification, auditTable=auditTable, cardSwVersion=cardSwVersion, auditCommand=auditCommand, cardType=cardType, cardFpgaSource=cardFpgaSource, portShelfId=portShelfId, auditTotalFailedCount=auditTotalFailedCount, auditCycleCount=auditCycleCount, errEvRowStatus=errEvRowStatus, diskVolumeTrapInh=diskVolumeTrapInh, auditDescription=auditDescription, cmPrimarySID=cmPrimarySID, cardTempOverHeatNotification=cardTempOverHeatNotification, portRxFlowControlMode=portRxFlowControlMode, cmUChannelID=cmUChannelID, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestIndex=cardTestIndex, cmResetReason=cmResetReason, verboseLevel=verboseLevel, fpgaErrorEventEntry=fpgaErrorEventEntry, portMaxDsPower=portMaxDsPower, trapSeverity=trapSeverity, cardShelfId=cardShelfId, cardCpuType=cardCpuType, cardBusSpeed=cardBusSpeed, cardTestTable=cardTestTable, fpgaErrorEventTable=fpgaErrorEventTable, cardUpTime=cardUpTime, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestEntry=cardTestEntry, TestTransactionId=TestTransactionId, auditEntry=auditEntry, cardTestTransId=cardTestTransId, remainInDiagMode=remainInDiagMode, TestType=TestType, systemClock=systemClock, cardDetectedChange=cardDetectedChange, cardDetected=cardDetected, portType=portType, diskVolumeGroup=diskVolumeGroup, diskVolumeCardId=diskVolumeCardId, equipmentTraps=equipmentTraps, cardTestType=cardTestType, portTrapInh=portTrapInh, diskVolumeNumber=diskVolumeNumber, cardHwVersion=cardHwVersion, cardTestTime=cardTestTime, cmIpAddress=cmIpAddress, auditResult=auditResult, cardDplxStatusChange=cardDplxStatusChange, portPrStateChange=portPrStateChange, diskVolumeSize=diskVolumeSize, cardTestDescription=cardTestDescription, equipmentShelfGroup=equipmentShelfGroup, TestScheduleCommand=TestScheduleCommand, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portMode=portMode, systemKey=systemKey, cadEquipmentMib=cadEquipmentMib, auditFailedCount=auditFailedCount, cardShelfTestIndex=cardShelfTestIndex, auditTime=auditTime, shelfSwVersion=shelfSwVersion, portLastChangeTime=portLastChangeTime, cardFwVersion=cardFwVersion, cardFwUpdateStatus=cardFwUpdateStatus, cardSubType=cardSubType, portNumChans=portNumChans, portId=portId, cardCpuSpeed=cardCpuSpeed, errEventId=errEventId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeName=diskVolumeName, cardLastChangeTime=cardLastChangeTime, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardNumber=cardNumber, portMacIfIndex=portMacIfIndex, diskVolumeFileName=diskVolumeFileName, equipmentTbl=equipmentTbl, portMinDsPower=portMinDsPower, portTxFlowControlMode=portTxFlowControlMode, diskVolumeDiskSize=diskVolumeDiskSize, diagTestId=diagTestId, cardTestSchedule=cardTestSchedule, cardAdminState=cardAdminState, cardId=cardId, cardNumPorts=cardNumPorts, shelfObjects=shelfObjects, shelfName=shelfName, cmVendor=cmVendor, cmResetInfo=cmResetInfo, diskVolumeShelfId=diskVolumeShelfId, cardLastBootSource=cardLastBootSource, errEvLoggingEnabled=errEvLoggingEnabled, PYSNMP_MODULE_ID=cadEquipmentMib, cardCarrierFwVersion=cardCarrierFwVersion, cardDplxStatus=cardDplxStatus, cardTemperatureHighError=cardTemperatureHighError, cardRamSize=cardRamSize, cadEquipmentMibGroup=cadEquipmentMibGroup, portPrState=portPrState, auditStatus=auditStatus, cardCPLDVersion=cardCPLDVersion, TestSchedule=TestSchedule, portNumber=portNumber, cardPicDetected=cardPicDetected, cardTestScheduleCommand=cardTestScheduleCommand, cmRegistrationNotification=cmRegistrationNotification, equipmentStateGroup=equipmentStateGroup, TestId=TestId, equipmentDiagGroup=equipmentDiagGroup, equipmentAuditGroup=equipmentAuditGroup, cardSpareGroupId=cardSpareGroupId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetNotification=cmResetNotification, cardSecStateChange=cardSecStateChange, trapCounter=trapCounter, diskVolumeTable=diskVolumeTable, portSecState=portSecState, auditShelfId=auditShelfId, auditTotalPassedCount=auditTotalPassedCount, portSecStateChange=portSecStateChange, portAction=portAction, cardTestId=cardTestId, cardSubDetected=cardSubDetected, cardBootVersion=cardBootVersion, cardLastBootVersion=cardLastBootVersion, equipmentCardGroup=equipmentCardGroup, cardCarrierSerialNum=cardCarrierSerialNum, cardTemperatureHighWarn=cardTemperatureHighWarn, cardPicHwVersion=cardPicHwVersion, cardName=cardName, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, diskDriveNumber=diskDriveNumber, systemGeneralGroup=systemGeneralGroup, equipmentDiag=equipmentDiag, cardLastResetReason=cardLastResetReason, auditName=auditName, portDplxStatus=portDplxStatus, equipmentAudit=equipmentAudit, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portAdminState=portAdminState, auditPassedCount=auditPassedCount, cmResetUpTime=cmResetUpTime, cardPicSerialNum=cardPicSerialNum, portDescription=portDescription, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditAutoScheduling=auditAutoScheduling, portCardId=portCardId, diskVolumeId=diskVolumeId, portDocsIfIndex=portDocsIfIndex, cardAction=cardAction, consoleOutput=consoleOutput, cmResetStatus=cmResetStatus, TestCommand=TestCommand, eqDiagConfig=eqDiagConfig, equipmentPortGroup=equipmentPortGroup, cardEntry=cardEntry, portRxFlowControlDetected=portRxFlowControlDetected, auditLogThrottle=auditLogThrottle, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeEntry=diskVolumeEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeUsageNotification=diskVolumeUsageNotification, portCardSubType=portCardSubType, auditCardId=auditCardId, portMacAddress=portMacAddress, cmMacAddress=cmMacAddress, downstreamPowerLoss=downstreamPowerLoss, equipmentState=equipmentState, cardTable=cardTable, cardTestCommand=cardTestCommand, cardPrStateChange=cardPrStateChange, cardTestResult=cardTestResult, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSerialNum=cardSerialNum, portConnectorId=portConnectorId, systemGeneral=systemGeneral, portTable=portTable, equipmentError=equipmentError, portBgpId=portBgpId, portCurrDsPower=portCurrDsPower, cardSpareGroupMode=cardSpareGroupMode, cardSecState=cardSecState, cardTestName=cardTestName, shelfNumber=shelfNumber, cardTestResultDesc=cardTestResultDesc, errEvLogLevel=errEvLogLevel, cardTestResultNotification=cardTestResultNotification, portDplxStatusChange=portDplxStatusChange, cardFlashSize=cardFlashSize, cardTemperature=cardTemperature, TestResult=TestResult, portEntry=portEntry)
