...
AdminState, FlowControlMode, PortId, UnknownState, PicType, EqActionType, CardType, CardSubType, FirmwareSource, PortType, DuplexStatus, PortMode, CardId, SecondaryState, PortDetectedMode, DiskVolumeUsageLevel, ShelfId, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "AdminState", "FlowControlMode", "PortId", "UnknownState", "PicType", "EqActionType", "CardType", "CardSubType", "FirmwareSource", "PortType", "DuplexStatus", "PortMode", "CardId", "SecondaryState", "PortDetectedMode", "DiskVolumeUsageLevel", "ShelfId", "PrimaryState")
...
MacAddress, DisplayString, TimeStamp, DateAndTime, TruthValue, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "DisplayString", "TimeStamp", "DateAndTime", "TruthValue", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSecState=cardSecState, equipmentStateGroup=equipmentStateGroup, portSecState=portSecState, shelfSwVersion=shelfSwVersion, cmVendor=cmVendor, equipmentDiag=equipmentDiag, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeDriveId=diskVolumeDriveId, cardPicHwVersion=cardPicHwVersion, cardPrState=cardPrState, auditCardId=auditCardId, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeTable=diskVolumeTable, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSubType=cardSubType, auditStatus=auditStatus, auditLogThrottle=auditLogThrottle, diskVolumeNumber=diskVolumeNumber, cardPrStateChange=cardPrStateChange, errEvLogLevel=errEvLogLevel, equipmentShelfGroup=equipmentShelfGroup, auditDescription=auditDescription, cardTemperatureHighError=cardTemperatureHighError, diagTestId=diagTestId, cardTestTable=cardTestTable, cardTestResult=cardTestResult, cardAdminState=cardAdminState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTemperature=cardTemperature, diskVolumeGroup=diskVolumeGroup, cardSwVersion=cardSwVersion, equipmentState=equipmentState, cardName=cardName, portPrState=portPrState, cardPicDetected=cardPicDetected, portCurrDsPower=portCurrDsPower, remainInDiagMode=remainInDiagMode, auditName=auditName, errEvRowStatus=errEvRowStatus, auditFailedCount=auditFailedCount, cmPrimarySID=cmPrimarySID, portMode=portMode, cardCarrierSerialNum=cardCarrierSerialNum, portShelfId=portShelfId, cardTestResultDesc=cardTestResultDesc, cmResetReason=cmResetReason, cadEquipmentMibConformance=cadEquipmentMibConformance, cadEquipmentMib=cadEquipmentMib, eqDiagConfig=eqDiagConfig, TestType=TestType, auditCommand=auditCommand, cardDetectedChange=cardDetectedChange, trapCounter=trapCounter, cardShelfId=cardShelfId, cmResetInfo=cmResetInfo, cmResetClearNotification=cmResetClearNotification, equipmentAuditGroup=equipmentAuditGroup, cardTestTime=cardTestTime, PYSNMP_MODULE_ID=cadEquipmentMib, portMaxDsPower=portMaxDsPower, portNumChans=portNumChans, portLastChangeTime=portLastChangeTime, portAdminState=portAdminState, shelfNumber=shelfNumber, cardTestName=cardTestName, cardEntry=cardEntry, cardTestResultNotification=cardTestResultNotification, cardBusSpeed=cardBusSpeed, diskVolumeEntry=diskVolumeEntry, cardLastResetReason=cardLastResetReason, cardTempOverHeatNotification=cardTempOverHeatNotification, portDescription=portDescription, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetUpTime=cmResetUpTime, portMacAddress=portMacAddress, cardFwUpdateStatus=cardFwUpdateStatus, diskDriveNumber=diskDriveNumber, auditPassedCount=auditPassedCount, cardHwVersion=cardHwVersion, cardSecStateChange=cardSecStateChange, cardSerialNum=cardSerialNum, equipmentAudit=equipmentAudit, portAction=portAction, portMinDsPower=portMinDsPower, cardSpareGroupId=cardSpareGroupId, cardId=cardId, diskVolumeId=diskVolumeId, cmResetNotification=cmResetNotification, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTrapInh=cardTrapInh, cardUpTime=cardUpTime, portSecStateChange=portSecStateChange, cardTestSchedule=cardTestSchedule, cardTestDescription=cardTestDescription, portPrStateChange=portPrStateChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentCardGroup=equipmentCardGroup, equipmentError=equipmentError, cardTestType=cardTestType, cardFwVersion=cardFwVersion, TestCommand=TestCommand, cardFpgaSource=cardFpgaSource, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeDiskSize=diskVolumeDiskSize, cardDplxStatus=cardDplxStatus, auditCycleCount=auditCycleCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, systemClock=systemClock, errEventId=errEventId, portMacIfIndex=portMacIfIndex, cardTestIndex=cardTestIndex, fpgaErrorEventGroup=fpgaErrorEventGroup, auditShelfId=auditShelfId, cardTable=cardTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeName=diskVolumeName, diskVolumeSize=diskVolumeSize, portId=portId, cardTestTransId=cardTestTransId, downstreamPowerLoss=downstreamPowerLoss, TestSchedule=TestSchedule, cardTestId=cardTestId, cardTestScheduleCommand=cardTestScheduleCommand, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentTbl=equipmentTbl, auditTotalPassedCount=auditTotalPassedCount, shelfName=shelfName, cmIpAddress=cmIpAddress, auditId=auditId, cardLastBootSource=cardLastBootSource, portTrapInh=portTrapInh, portDplxStatusChange=portDplxStatusChange, auditLogOutput=auditLogOutput, auditTotalFailedCount=auditTotalFailedCount, cardLastBootVersion=cardLastBootVersion, diskVolumeCardId=diskVolumeCardId, portCardSubType=portCardSubType, cmDevice=cmDevice, auditEntry=auditEntry, cardNumber=cardNumber, cardTestEntry=cardTestEntry, cardTestCommand=cardTestCommand, cmRegistrationNotification=cmRegistrationNotification, equipmentTraps=equipmentTraps, auditTime=auditTime, equipmentDiagGroup=equipmentDiagGroup, cardCarrierHwVersion=cardCarrierHwVersion, equipmentPortGroup=equipmentPortGroup, cardType=cardType, portDplxStatus=portDplxStatus, diskVolumeTrapInh=diskVolumeTrapInh, auditResult=auditResult, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeShelfId=diskVolumeShelfId, cardCPLDVersion=cardCPLDVersion, TestResult=TestResult, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, shelfObjects=shelfObjects, consoleOutput=consoleOutput, diskVolumeFileName=diskVolumeFileName, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, systemGeneralGroup=systemGeneralGroup, cardDetected=cardDetected, portDocsIfIndex=portDocsIfIndex, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardShelfTestIndex=cardShelfTestIndex, errEvLoggingEnabled=errEvLoggingEnabled, cardSpareGroupMode=cardSpareGroupMode, cardCpuSpeed=cardCpuSpeed, cardFlashSize=cardFlashSize, portCardId=portCardId, cardCpuType=cardCpuType, portBgpId=portBgpId, systemKey=systemKey, systemGeneral=systemGeneral, cardRamSize=cardRamSize, auditTable=auditTable, cadEquipmentMibGroup=cadEquipmentMibGroup, cardDplxStatusChange=cardDplxStatusChange, portTxFlowControlMode=portTxFlowControlMode, cmResetStatus=cmResetStatus, cardBootVersion=cardBootVersion, portTxFlowControlDetected=portTxFlowControlDetected, portNumber=portNumber, portEntry=portEntry, cardNumPorts=cardNumPorts, trapSeverity=trapSeverity, TestScheduleCommand=TestScheduleCommand, portDetectedMode=portDetectedMode, TestId=TestId, cardSubDetected=cardSubDetected, portRxFlowControlMode=portRxFlowControlMode, TestTransactionId=TestTransactionId, diskVolumeUsageLevel=diskVolumeUsageLevel, portTable=portTable, cmUChannelID=cmUChannelID, cardAction=cardAction, cmMacAddress=cmMacAddress, cardCarrierFwVersion=cardCarrierFwVersion, cardPicSerialNum=cardPicSerialNum, portType=portType, portConnectorId=portConnectorId, verboseLevel=verboseLevel)
