...
CardSubType, DiskVolumeUsageLevel, FlowControlMode, PortDetectedMode, PortType, DuplexStatus, FirmwareSource, PrimaryState, SecondaryState, ShelfId, UnknownState, PortId, PortMode, AdminState, EqActionType, CardId, PicType, CardType = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "DiskVolumeUsageLevel", "FlowControlMode", "PortDetectedMode", "PortType", "DuplexStatus", "FirmwareSource", "PrimaryState", "SecondaryState", "ShelfId", "UnknownState", "PortId", "PortMode", "AdminState", "EqActionType", "CardId", "PicType", "CardType")
...
TruthValue, DisplayString, MacAddress, RowStatus, DateAndTime, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DisplayString", "MacAddress", "RowStatus", "DateAndTime", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardDetected=cardDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardPicDetected=cardPicDetected, diskVolumeTable=diskVolumeTable, equipmentPortGroup=equipmentPortGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, cardLastBootVersion=cardLastBootVersion, portDplxStatus=portDplxStatus, cardPicHwVersion=cardPicHwVersion, TestCommand=TestCommand, portMacAddress=portMacAddress, cardSpareGroupId=cardSpareGroupId, portNumChans=portNumChans, portCardSubType=portCardSubType, diskVolumeEntry=diskVolumeEntry, cardFwUpdateStatus=cardFwUpdateStatus, TestSchedule=TestSchedule, cardAction=cardAction, cardBootVersion=cardBootVersion, shelfNumber=shelfNumber, cmResetClearNotification=cmResetClearNotification, portTxFlowControlDetected=portTxFlowControlDetected, TestType=TestType, cardTestDescription=cardTestDescription, auditId=auditId, auditCycleCount=auditCycleCount, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, portRxFlowControlMode=portRxFlowControlMode, portPrState=portPrState, cardEntry=cardEntry, cardBusSpeed=cardBusSpeed, cardAdminState=cardAdminState, cardSpareGroupMode=cardSpareGroupMode, remainInDiagMode=remainInDiagMode, cardTestResultDesc=cardTestResultDesc, errEvRowStatus=errEvRowStatus, portLastChangeTime=portLastChangeTime, cardSerialNum=cardSerialNum, portMaxDsPower=portMaxDsPower, TestScheduleCommand=TestScheduleCommand, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCpuSpeed=cardCpuSpeed, auditCommand=auditCommand, systemGeneralGroup=systemGeneralGroup, systemGeneral=systemGeneral, auditPassedCount=auditPassedCount, cardTestName=cardTestName, cardRamSize=cardRamSize, portDocsIfIndex=portDocsIfIndex, diskVolumeShelfId=diskVolumeShelfId, cardTestSchedule=cardTestSchedule, cmResetUpTime=cmResetUpTime, cmIpAddress=cmIpAddress, cardFwVersion=cardFwVersion, diskVolumeSize=diskVolumeSize, cardNumPorts=cardNumPorts, cmResetInfo=cmResetInfo, auditTime=auditTime, cmMacAddress=cmMacAddress, portNumber=portNumber, cadEquipmentMib=cadEquipmentMib, diskVolumeId=diskVolumeId, equipmentDiagGroup=equipmentDiagGroup, portShelfId=portShelfId, cardHwVersion=cardHwVersion, cardCPLDVersion=cardCPLDVersion, equipmentAudit=equipmentAudit, cmPrimarySID=cmPrimarySID, errEvLoggingEnabled=errEvLoggingEnabled, portType=portType, cardTestResultNotification=cardTestResultNotification, cardPrState=cardPrState, auditDescription=auditDescription, equipmentCardGroup=equipmentCardGroup, cardSecStateChange=cardSecStateChange, portAdminState=portAdminState, cardTempNoReportNotification=cardTempNoReportNotification, equipmentTraps=equipmentTraps, cardTemperatureHighError=cardTemperatureHighError, auditTotalFailedCount=auditTotalFailedCount, consoleOutput=consoleOutput, diskVolumeDriveId=diskVolumeDriveId, portTxFlowControlMode=portTxFlowControlMode, cardSubType=cardSubType, portMode=portMode, cmRegistrationNotification=cmRegistrationNotification, cardTestScheduleCommand=cardTestScheduleCommand, shelfObjects=shelfObjects, shelfName=shelfName, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSwVersion=cardSwVersion, cardDetectedChange=cardDetectedChange, cmResetReason=cmResetReason, portEntry=portEntry, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentTbl=equipmentTbl, diskVolumeUsageLevel=diskVolumeUsageLevel, auditFailedCount=auditFailedCount, diagTestId=diagTestId, auditResult=auditResult, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, downstreamPowerLoss=downstreamPowerLoss, systemKey=systemKey, TestId=TestId, cardNumber=cardNumber, cardTestId=cardTestId, cardLastResetReason=cardLastResetReason, diskVolumeTrapInh=diskVolumeTrapInh, cmResetStatus=cmResetStatus, auditCardId=auditCardId, equipmentStateGroup=equipmentStateGroup, cardTestType=cardTestType, portAction=portAction, cardPicSerialNum=cardPicSerialNum, portMinDsPower=portMinDsPower, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetNotification=cmResetNotification, cardTestTable=cardTestTable, auditEntry=auditEntry, cardTestTime=cardTestTime, equipmentError=equipmentError, portDplxStatusChange=portDplxStatusChange, cardTestResult=cardTestResult, cmDevice=cmDevice, fpgaErrorEventGroup=fpgaErrorEventGroup, shelfSwVersion=shelfSwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTrapInh=cardTrapInh, TestResult=TestResult, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeName=diskVolumeName, equipmentCardTestGroup=equipmentCardTestGroup, cardCarrierSerialNum=cardCarrierSerialNum, cardType=cardType, cardId=cardId, cardName=cardName, cardPrStateChange=cardPrStateChange, diskVolumeGroup=diskVolumeGroup, portTrapInh=portTrapInh, cardUpTime=cardUpTime, cardLastChangeTime=cardLastChangeTime, portCardId=portCardId, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTemperature=cardTemperature, auditLogThrottle=auditLogThrottle, cardFpgaSource=cardFpgaSource, cardTestEntry=cardTestEntry, trapSeverity=trapSeverity, cardCarrierHwVersion=cardCarrierHwVersion, equipmentShelfGroup=equipmentShelfGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecStateChange=portSecStateChange, portTable=portTable, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentAuditGroup=equipmentAuditGroup, auditStatus=auditStatus, diskVolumeFileName=diskVolumeFileName, equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, cardDplxStatus=cardDplxStatus, cardShelfId=cardShelfId, errEvRecoveryEnabled=errEvRecoveryEnabled, systemClock=systemClock, portDetectedMode=portDetectedMode, cardDplxStatusChange=cardDplxStatusChange, portDescription=portDescription, auditAutoScheduling=auditAutoScheduling, portConnectorId=portConnectorId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditTable=auditTable, cmVendor=cmVendor, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditShelfId=auditShelfId, portCurrDsPower=portCurrDsPower, diskVolumeNumber=diskVolumeNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardFlashSize=cardFlashSize, eqDiagConfig=eqDiagConfig, cardCpuType=cardCpuType, verboseLevel=verboseLevel, cardTestIndex=cardTestIndex, equipmentDiag=equipmentDiag, cardShelfTestIndex=cardShelfTestIndex, trapCounter=trapCounter, cardTable=cardTable, portId=portId, errEventId=errEventId, portMacIfIndex=portMacIfIndex, cardSubDetected=cardSubDetected, cardSecState=cardSecState, cardLastBootSource=cardLastBootSource, portSecState=portSecState, cmUChannelID=cmUChannelID, TestTransactionId=TestTransactionId, portPrStateChange=portPrStateChange, diskDriveNumber=diskDriveNumber, auditName=auditName, portBgpId=portBgpId, errEvLogLevel=errEvLogLevel, fpgaErrorEventTable=fpgaErrorEventTable, cardTestCommand=cardTestCommand, diskVolumeDiskSize=diskVolumeDiskSize, auditLogOutput=auditLogOutput, cardTestTransId=cardTestTransId)
