...
PicType, FirmwareSource, PortId, SecondaryState, PrimaryState, UnknownState, CardSubType, PortDetectedMode, EqActionType, ShelfId, PortMode, CardId, AdminState, CardType, FlowControlMode, DiskVolumeUsageLevel, PortType, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PicType", "FirmwareSource", "PortId", "SecondaryState", "PrimaryState", "UnknownState", "CardSubType", "PortDetectedMode", "EqActionType", "ShelfId", "PortMode", "CardId", "AdminState", "CardType", "FlowControlMode", "DiskVolumeUsageLevel", "PortType", "DuplexStatus")
...
DisplayString, DateAndTime, TruthValue, TextualConvention, RowStatus, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "DateAndTime", "TruthValue", "TextualConvention", "RowStatus", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portRxFlowControlDetected=portRxFlowControlDetected, cardPicDetected=cardPicDetected, cardTable=cardTable, diskVolumeDiskSize=diskVolumeDiskSize, cardFlashSize=cardFlashSize, cardNumber=cardNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTempNoReportNotification=cardTempNoReportNotification, cardLastResetReason=cardLastResetReason, TestScheduleCommand=TestScheduleCommand, diskVolumeGroup=diskVolumeGroup, cardCarrierHwVersion=cardCarrierHwVersion, cmRegistrationNotification=cmRegistrationNotification, auditName=auditName, portBgpId=portBgpId, auditCycleCount=auditCycleCount, cmVendor=cmVendor, portMinDsPower=portMinDsPower, TestTransactionId=TestTransactionId, TestCommand=TestCommand, equipmentState=equipmentState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardShelfId=cardShelfId, cardTrapInh=cardTrapInh, fpgaErrorEventTable=fpgaErrorEventTable, portAdminState=portAdminState, equipmentPortGroup=equipmentPortGroup, cardTestCommand=cardTestCommand, portTrapInh=portTrapInh, TestSchedule=TestSchedule, shelfNumber=shelfNumber, cardTestSchedule=cardTestSchedule, cmResetStatus=cmResetStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTemperature=cardTemperature, equipmentAuditGroup=equipmentAuditGroup, systemClock=systemClock, auditAutoScheduling=auditAutoScheduling, auditResult=auditResult, cardLastBootVersion=cardLastBootVersion, eqDiagConfig=eqDiagConfig, cardTestTime=cardTestTime, cadEquipmentMibConformance=cadEquipmentMibConformance, cardId=cardId, cardRamSize=cardRamSize, portCardId=portCardId, auditFailedCount=auditFailedCount, shelfObjects=shelfObjects, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastChangeTime=cardLastChangeTime, portPrState=portPrState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditLogOutput=auditLogOutput, cmResetClearNotification=cmResetClearNotification, portMacIfIndex=portMacIfIndex, portTable=portTable, systemKey=systemKey, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeTrapInh=diskVolumeTrapInh, fpgaErrorEventEntry=fpgaErrorEventEntry, cardNumPorts=cardNumPorts, cardFwVersion=cardFwVersion, auditLogThrottle=auditLogThrottle, cardCarrierSerialNum=cardCarrierSerialNum, remainInDiagMode=remainInDiagMode, auditTotalPassedCount=auditTotalPassedCount, cmMacAddress=cmMacAddress, cardSecState=cardSecState, portTxFlowControlMode=portTxFlowControlMode, consoleOutput=consoleOutput, cmDevice=cmDevice, auditShelfId=auditShelfId, portDescription=portDescription, cardAdminState=cardAdminState, downstreamPowerLoss=downstreamPowerLoss, cardTestType=cardTestType, auditTable=auditTable, equipmentAudit=equipmentAudit, cardTestDescription=cardTestDescription, portMode=portMode, shelfName=shelfName, cardUpTime=cardUpTime, portMaxDsPower=portMaxDsPower, portLastChangeTime=portLastChangeTime, diagTestId=diagTestId, portType=portType, portSecState=portSecState, errEvLogLevel=errEvLogLevel, cardCpuType=cardCpuType, equipmentDiagGroup=equipmentDiagGroup, cardTestScheduleCommand=cardTestScheduleCommand, cardEntry=cardEntry, cardLastBootSource=cardLastBootSource, PYSNMP_MODULE_ID=cadEquipmentMib, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibCompliances=cadEquipmentMibCompliances, verboseLevel=verboseLevel, cardSubDetected=cardSubDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, cardDetected=cardDetected, portId=portId, cardPrStateChange=cardPrStateChange, cardHwVersion=cardHwVersion, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardShelfTestIndex=cardShelfTestIndex, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMib=cadEquipmentMib, cardCPLDVersion=cardCPLDVersion, auditTime=auditTime, cardCpuSpeed=cardCpuSpeed, portCardSubType=portCardSubType, diskDriveNumber=diskDriveNumber, cadEquipmentMibGroup=cadEquipmentMibGroup, cardPicHwVersion=cardPicHwVersion, cardTestName=cardTestName, diskVolumeEntry=diskVolumeEntry, portNumber=portNumber, portNumChans=portNumChans, cardSecStateChange=cardSecStateChange, errEvLoggingEnabled=errEvLoggingEnabled, equipmentTbl=equipmentTbl, portDocsIfIndex=portDocsIfIndex, auditEntry=auditEntry, portConnectorId=portConnectorId, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDplxStatus=portDplxStatus, TestResult=TestResult, cardBusSpeed=cardBusSpeed, diskVolumeTable=diskVolumeTable, cardType=cardType, cmIpAddress=cmIpAddress, diskVolumeSize=diskVolumeSize, cardTestResultDesc=cardTestResultDesc, auditCardId=auditCardId, auditCommand=auditCommand, cardName=cardName, cardPicSerialNum=cardPicSerialNum, cardTestTable=cardTestTable, cardTemperatureHighError=cardTemperatureHighError, cardTestId=cardTestId, TestId=TestId, diskVolumeCardId=diskVolumeCardId, auditStatus=auditStatus, shelfSwVersion=shelfSwVersion, cardSpareGroupId=cardSpareGroupId, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResultNotification=cardTestResultNotification, portShelfId=portShelfId, diskVolumeId=diskVolumeId, equipmentTraps=equipmentTraps, cmUChannelID=cmUChannelID, TestType=TestType, systemGeneral=systemGeneral, portAction=portAction, portSecStateChange=portSecStateChange, cardSwVersion=cardSwVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeName=diskVolumeName, errEvRowStatus=errEvRowStatus, portPrStateChange=portPrStateChange, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTotalFailedCount=auditTotalFailedCount, portDetectedMode=portDetectedMode, cmResetUpTime=cmResetUpTime, equipmentCardGroup=equipmentCardGroup, cardTestResult=cardTestResult, cardAction=cardAction, equipmentDiag=equipmentDiag, cmPrimarySID=cmPrimarySID, cardTestTransId=cardTestTransId, cardDplxStatusChange=cardDplxStatusChange, cardBootVersion=cardBootVersion, portMacAddress=portMacAddress, equipmentError=equipmentError, cardTestEntry=cardTestEntry, cardSubType=cardSubType, portDplxStatusChange=portDplxStatusChange, trapSeverity=trapSeverity, trapCounter=trapCounter, diskVolumeNumber=diskVolumeNumber, cmResetNotification=cmResetNotification, cardSerialNum=cardSerialNum, portCurrDsPower=portCurrDsPower, cardDplxStatus=cardDplxStatus, portRxFlowControlMode=portRxFlowControlMode, auditDescription=auditDescription, diskVolumeFileName=diskVolumeFileName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetReason=cmResetReason, equipmentShelfGroup=equipmentShelfGroup, errEventId=errEventId, systemGeneralGroup=systemGeneralGroup, auditPassedCount=auditPassedCount, cmResetInfo=cmResetInfo, cardTestIndex=cardTestIndex, cardFpgaSource=cardFpgaSource, cardDetectedChange=cardDetectedChange, auditId=auditId, cardPrState=cardPrState, portEntry=portEntry, diskVolumeShelfId=diskVolumeShelfId, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentStateGroup=equipmentStateGroup, cardSpareGroupMode=cardSpareGroupMode)
