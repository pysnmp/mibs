...
CardType, DuplexStatus, FlowControlMode, PicType, ShelfId, UnknownState, PrimaryState, AdminState, PortDetectedMode, CardSubType, EqActionType, PortId, CardId, SecondaryState, PortMode, PortType, FirmwareSource, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "CardType", "DuplexStatus", "FlowControlMode", "PicType", "ShelfId", "UnknownState", "PrimaryState", "AdminState", "PortDetectedMode", "CardSubType", "EqActionType", "PortId", "CardId", "SecondaryState", "PortMode", "PortType", "FirmwareSource", "DiskVolumeUsageLevel")
...
RowStatus, DisplayString, MacAddress, TruthValue, TextualConvention, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "MacAddress", "TruthValue", "TextualConvention", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portAction=portAction, cardTestSchedule=cardTestSchedule, equipmentAudit=equipmentAudit, cardTestResultDesc=cardTestResultDesc, auditResult=auditResult, portPrStateChange=portPrStateChange, portNumChans=portNumChans, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSpareGroupMode=cardSpareGroupMode, cmIpAddress=cmIpAddress, auditLogThrottle=auditLogThrottle, cmResetNotification=cmResetNotification, portDetectedMode=portDetectedMode, errEvRowStatus=errEvRowStatus, equipmentAuditGroup=equipmentAuditGroup, systemKey=systemKey, cardId=cardId, portTxFlowControlMode=portTxFlowControlMode, cardTestDescription=cardTestDescription, cadEquipmentMib=cadEquipmentMib, portMode=portMode, portMinDsPower=portMinDsPower, portConnectorId=portConnectorId, diskVolumeNumber=diskVolumeNumber, remainInDiagMode=remainInDiagMode, auditFailedCount=auditFailedCount, auditDescription=auditDescription, portType=portType, shelfNumber=shelfNumber, verboseLevel=verboseLevel, cardLastResetReason=cardLastResetReason, systemGeneral=systemGeneral, diagTestId=diagTestId, auditCardId=auditCardId, cardShelfTestIndex=cardShelfTestIndex, cardHwVersion=cardHwVersion, cardNumPorts=cardNumPorts, cardFlashSize=cardFlashSize, fpgaErrorEventGroup=fpgaErrorEventGroup, portSecStateChange=portSecStateChange, trapSeverity=trapSeverity, diskVolumeFileName=diskVolumeFileName, cardTestType=cardTestType, auditTime=auditTime, cardTestScheduleCommand=cardTestScheduleCommand, portCardId=portCardId, auditTable=auditTable, downstreamPowerLoss=downstreamPowerLoss, cmResetClearNotification=cmResetClearNotification, TestResult=TestResult, portTxFlowControlDetected=portTxFlowControlDetected, cardNumber=cardNumber, equipmentCardGroup=equipmentCardGroup, diskVolumeCardId=diskVolumeCardId, cardBootVersion=cardBootVersion, cardSpareGroupId=cardSpareGroupId, cardTemperatureHighError=cardTemperatureHighError, cardTestResultNotification=cardTestResultNotification, diskVolumeDriveId=diskVolumeDriveId, auditCommand=auditCommand, auditCycleCount=auditCycleCount, diskVolumeName=diskVolumeName, cardTestTime=cardTestTime, cardBusSpeed=cardBusSpeed, auditName=auditName, auditPassedCount=auditPassedCount, cardDetected=cardDetected, cardSwVersion=cardSwVersion, TestTransactionId=TestTransactionId, cardTemperature=cardTemperature, equipmentState=equipmentState, errEvLogLevel=errEvLogLevel, cmResetUpTime=cmResetUpTime, errEvRecoveryEnabled=errEvRecoveryEnabled, portBgpId=portBgpId, equipmentTbl=equipmentTbl, auditId=auditId, cardAction=cardAction, portNumber=portNumber, cmUChannelID=cmUChannelID, shelfName=shelfName, cardType=cardType, portDplxStatusChange=portDplxStatusChange, cardTempOverHeatNotification=cardTempOverHeatNotification, cmPrimarySID=cmPrimarySID, TestType=TestType, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, eqDiagConfig=eqDiagConfig, portCardSubType=portCardSubType, diskVolumeEntry=diskVolumeEntry, TestSchedule=TestSchedule, cardFwUpdateStatus=cardFwUpdateStatus, cardShelfId=cardShelfId, portPrState=portPrState, consoleOutput=consoleOutput, portDplxStatus=portDplxStatus, cardEntry=cardEntry, portDocsIfIndex=portDocsIfIndex, TestScheduleCommand=TestScheduleCommand, portMaxDsPower=portMaxDsPower, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskDriveNumber=diskDriveNumber, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSerialNum=cardSerialNum, shelfObjects=shelfObjects, cardTestCommand=cardTestCommand, systemGeneralGroup=systemGeneralGroup, cardTestTransId=cardTestTransId, cardDplxStatus=cardDplxStatus, auditTotalPassedCount=auditTotalPassedCount, auditAutoScheduling=auditAutoScheduling, diskVolumeGroup=diskVolumeGroup, portTable=portTable, cardName=cardName, cmRegistrationNotification=cmRegistrationNotification, equipmentStateGroup=equipmentStateGroup, cardLastBootSource=cardLastBootSource, portAdminState=portAdminState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardFwVersion=cardFwVersion, cardCarrierHwVersion=cardCarrierHwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeTable=diskVolumeTable, cardSubType=cardSubType, errEvLoggingEnabled=errEvLoggingEnabled, cardPicSerialNum=cardPicSerialNum, systemClock=systemClock, cardTestResult=cardTestResult, TestId=TestId, diskVolumeTrapInh=diskVolumeTrapInh, auditStatus=auditStatus, equipmentPortGroup=equipmentPortGroup, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditEntry=auditEntry, cardLastChangeTime=cardLastChangeTime, cardSubDetected=cardSubDetected, cardCpuType=cardCpuType, diskVolumeUsageLevel=diskVolumeUsageLevel, cmMacAddress=cmMacAddress, equipmentError=equipmentError, cardRamSize=cardRamSize, auditShelfId=auditShelfId, cmVendor=cmVendor, cardCPLDVersion=cardCPLDVersion, portSecState=portSecState, diskVolumeSize=diskVolumeSize, TestCommand=TestCommand, cardFpgaSource=cardFpgaSource, cardCarrierSerialNum=cardCarrierSerialNum, cardPicHwVersion=cardPicHwVersion, portEntry=portEntry, portLastChangeTime=portLastChangeTime, cardLastBootVersion=cardLastBootVersion, diskVolumeShelfId=diskVolumeShelfId, cardTestEntry=cardTestEntry, cardDetectedChange=cardDetectedChange, cmResetReason=cmResetReason, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPrState=cardPrState, cardPrStateChange=cardPrStateChange, cardDplxStatusChange=cardDplxStatusChange, cmResetStatus=cmResetStatus, portRxFlowControlMode=portRxFlowControlMode, fpgaErrorEventEntry=fpgaErrorEventEntry, portCurrDsPower=portCurrDsPower, auditLogOutput=auditLogOutput, cardTestTable=cardTestTable, cardTestName=cardTestName, cardSecState=cardSecState, equipmentCardTestGroup=equipmentCardTestGroup, portMacAddress=portMacAddress, cmResetInfo=cmResetInfo, diskVolumeDiskSize=diskVolumeDiskSize, cardCpuSpeed=cardCpuSpeed, equipmentDiag=equipmentDiag, cardTrapInh=cardTrapInh, cardTemperatureHighWarn=cardTemperatureHighWarn, portMacIfIndex=portMacIfIndex, PYSNMP_MODULE_ID=cadEquipmentMib, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierFwVersion=cardCarrierFwVersion, portId=portId, cadEquipmentMibGroup=cadEquipmentMibGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTable=cardTable, trapCounter=trapCounter, cmDevice=cmDevice, portShelfId=portShelfId, errEventId=errEventId, equipmentTraps=equipmentTraps, cardSecStateChange=cardSecStateChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardUpTime=cardUpTime, cardTempNoReportNotification=cardTempNoReportNotification, cardPicDetected=cardPicDetected, auditTotalFailedCount=auditTotalFailedCount, portTrapInh=portTrapInh, equipmentShelfGroup=equipmentShelfGroup, shelfSwVersion=shelfSwVersion, portDescription=portDescription, cardTestIndex=cardTestIndex, cardTestId=cardTestId, diskVolumeId=diskVolumeId, cardAdminState=cardAdminState, equipmentDiagGroup=equipmentDiagGroup)
