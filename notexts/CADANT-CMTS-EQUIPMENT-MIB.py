...
AdminState, UnknownState, PortType, CardSubType, SecondaryState, CardId, ShelfId, DiskVolumeUsageLevel, PortId, FlowControlMode, DuplexStatus, EqActionType, FirmwareSource, PicType, PrimaryState, CardType, PortMode, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "AdminState", "UnknownState", "PortType", "CardSubType", "SecondaryState", "CardId", "ShelfId", "DiskVolumeUsageLevel", "PortId", "FlowControlMode", "DuplexStatus", "EqActionType", "FirmwareSource", "PicType", "PrimaryState", "CardType", "PortMode", "PortDetectedMode")
...
MacAddress, RowStatus, DisplayString, DateAndTime, TimeStamp, TruthValue, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "DisplayString", "DateAndTime", "TimeStamp", "TruthValue", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeFileName=diskVolumeFileName, verboseLevel=verboseLevel, cardLastBootSource=cardLastBootSource, cardTrapInh=cardTrapInh, errEvRecoveryEnabled=errEvRecoveryEnabled, portTable=portTable, cardCPLDVersion=cardCPLDVersion, portAdminState=portAdminState, cardLastChangeTime=cardLastChangeTime, cmUChannelID=cmUChannelID, cmResetClearNotification=cmResetClearNotification, diagTestId=diagTestId, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestScheduleCommand=cardTestScheduleCommand, cardTestResultDesc=cardTestResultDesc, equipmentAudit=equipmentAudit, TestScheduleCommand=TestScheduleCommand, equipmentPortGroup=equipmentPortGroup, auditTime=auditTime, portRxFlowControlDetected=portRxFlowControlDetected, cardCpuType=cardCpuType, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSpareGroupId=cardSpareGroupId, cardTemperatureHighError=cardTemperatureHighError, portPrState=portPrState, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portType=portType, cardTestResult=cardTestResult, portCardId=portCardId, portConnectorId=portConnectorId, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMib=cadEquipmentMib, errEvRowStatus=errEvRowStatus, shelfName=shelfName, cardTempNoReportNotification=cardTempNoReportNotification, cardTestSchedule=cardTestSchedule, cardFwUpdateStatus=cardFwUpdateStatus, equipmentTraps=equipmentTraps, shelfNumber=shelfNumber, cmResetInfo=cmResetInfo, diskVolumeName=diskVolumeName, auditShelfId=auditShelfId, TestType=TestType, cardShelfTestIndex=cardShelfTestIndex, portDocsIfIndex=portDocsIfIndex, portDetectedMode=portDetectedMode, cardNumPorts=cardNumPorts, cardTestIndex=cardTestIndex, errEventId=errEventId, cadEquipmentMibConformance=cadEquipmentMibConformance, portDplxStatusChange=portDplxStatusChange, equipmentAuditGroup=equipmentAuditGroup, TestTransactionId=TestTransactionId, cardLastBootVersion=cardLastBootVersion, diskVolumeTrapInh=diskVolumeTrapInh, cmResetUpTime=cmResetUpTime, cardTestTable=cardTestTable, cardBootVersion=cardBootVersion, remainInDiagMode=remainInDiagMode, auditStatus=auditStatus, fpgaErrorEventTable=fpgaErrorEventTable, cardPicHwVersion=cardPicHwVersion, cardDetectedChange=cardDetectedChange, portTxFlowControlDetected=portTxFlowControlDetected, portLastChangeTime=portLastChangeTime, cardDplxStatusChange=cardDplxStatusChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardPicSerialNum=cardPicSerialNum, cardTemperature=cardTemperature, cardTestName=cardTestName, diskVolumeGroup=diskVolumeGroup, eqDiagConfig=eqDiagConfig, cardSwVersion=cardSwVersion, cardId=cardId, portNumChans=portNumChans, TestResult=TestResult, auditCycleCount=auditCycleCount, cardTestId=cardTestId, cardPicDetected=cardPicDetected, cardPrState=cardPrState, portAction=portAction, auditId=auditId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditTotalFailedCount=auditTotalFailedCount, portCardSubType=portCardSubType, systemGeneral=systemGeneral, cardType=cardType, cardTestEntry=cardTestEntry, equipmentStateGroup=equipmentStateGroup, cmResetStatus=cmResetStatus, cardTable=cardTable, portTxFlowControlMode=portTxFlowControlMode, diskDriveNumber=diskDriveNumber, cmVendor=cmVendor, diskVolumeUsageNotification=diskVolumeUsageNotification, portMacIfIndex=portMacIfIndex, auditLogOutput=auditLogOutput, auditPassedCount=auditPassedCount, diskVolumeCardId=diskVolumeCardId, downstreamPowerLoss=downstreamPowerLoss, cmResetNotification=cmResetNotification, equipmentError=equipmentError, cardEntry=cardEntry, cmPrimarySID=cmPrimarySID, cardFpgaSource=cardFpgaSource, auditResult=auditResult, auditAutoScheduling=auditAutoScheduling, auditCardId=auditCardId, auditFailedCount=auditFailedCount, diskVolumeShelfId=diskVolumeShelfId, fpgaErrorEventGroup=fpgaErrorEventGroup, trapSeverity=trapSeverity, cardAdminState=cardAdminState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardLastResetReason=cardLastResetReason, cardCarrierSerialNum=cardCarrierSerialNum, cardSubDetected=cardSubDetected, diskVolumeSize=diskVolumeSize, auditTable=auditTable, auditLogThrottle=auditLogThrottle, portBgpId=portBgpId, consoleOutput=consoleOutput, systemClock=systemClock, cardTestDescription=cardTestDescription, cmDevice=cmDevice, equipmentDiag=equipmentDiag, portMacAddress=portMacAddress, equipmentState=equipmentState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, cardSubType=cardSubType, shelfObjects=shelfObjects, cmRegistrationNotification=cmRegistrationNotification, cardShelfId=cardShelfId, equipmentShelfGroup=equipmentShelfGroup, shelfSwVersion=shelfSwVersion, cardFwVersion=cardFwVersion, cardPrStateChange=cardPrStateChange, portSecState=portSecState, PYSNMP_MODULE_ID=cadEquipmentMib, portCurrDsPower=portCurrDsPower, auditCommand=auditCommand, errEvLogLevel=errEvLogLevel, portNumber=portNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, cmIpAddress=cmIpAddress, systemGeneralGroup=systemGeneralGroup, cardTestCommand=cardTestCommand, cmMacAddress=cmMacAddress, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardDplxStatus=cardDplxStatus, cardName=cardName, cardAction=cardAction, diskVolumeEntry=diskVolumeEntry, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeTable=diskVolumeTable, cardTestTransId=cardTestTransId, equipmentCardGroup=equipmentCardGroup, equipmentTbl=equipmentTbl, trapCounter=trapCounter, cardSerialNum=cardSerialNum, TestSchedule=TestSchedule, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeNumber=diskVolumeNumber, cardHwVersion=cardHwVersion, cmResetReason=cmResetReason, portId=portId, portEntry=portEntry, TestCommand=TestCommand, cardSecStateChange=cardSecStateChange, equipmentCardTestGroup=equipmentCardTestGroup, portDescription=portDescription, auditEntry=auditEntry, portPrStateChange=portPrStateChange, cardTestTime=cardTestTime, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portSecStateChange=portSecStateChange, cardDetected=cardDetected, portMinDsPower=portMinDsPower, cardTemperatureHighWarn=cardTemperatureHighWarn, systemKey=systemKey, TestId=TestId, portTrapInh=portTrapInh, cardTestType=cardTestType, cardFlashSize=cardFlashSize, cardNumber=cardNumber, portShelfId=portShelfId, cardUpTime=cardUpTime, portMode=portMode, cardCpuSpeed=cardCpuSpeed, equipmentDiagGroup=equipmentDiagGroup, auditDescription=auditDescription, cardRamSize=cardRamSize, cardSpareGroupMode=cardSpareGroupMode, auditName=auditName, cardTestResultNotification=cardTestResultNotification, portRxFlowControlMode=portRxFlowControlMode, portDplxStatus=portDplxStatus, diskVolumeDriveId=diskVolumeDriveId, cardBusSpeed=cardBusSpeed, cardCarrierFwVersion=cardCarrierFwVersion, cardSecState=cardSecState, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeId=diskVolumeId)
