...
FirmwareSource, SecondaryState, PortMode, PortDetectedMode, PrimaryState, PicType, CardSubType, DiskVolumeUsageLevel, PortType, ShelfId, UnknownState, CardId, EqActionType, AdminState, CardType, DuplexStatus, FlowControlMode, PortId = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "SecondaryState", "PortMode", "PortDetectedMode", "PrimaryState", "PicType", "CardSubType", "DiskVolumeUsageLevel", "PortType", "ShelfId", "UnknownState", "CardId", "EqActionType", "AdminState", "CardType", "DuplexStatus", "FlowControlMode", "PortId")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestId=cardTestId, portEntry=portEntry, auditCommand=auditCommand, diskVolumeName=diskVolumeName, TestCommand=TestCommand, auditTotalPassedCount=auditTotalPassedCount, cardTestTransId=cardTestTransId, consoleOutput=consoleOutput, trapCounter=trapCounter, cardId=cardId, cmDevice=cmDevice, systemKey=systemKey, cardLastChangeTime=cardLastChangeTime, cardTestIndex=cardTestIndex, cmMacAddress=cmMacAddress, cardTestEntry=cardTestEntry, errEventId=errEventId, portDocsIfIndex=portDocsIfIndex, fpgaErrorEventTable=fpgaErrorEventTable, auditCardId=auditCardId, cmResetNotification=cmResetNotification, cardNumPorts=cardNumPorts, TestType=TestType, cardFpgaSource=cardFpgaSource, verboseLevel=verboseLevel, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestTable=cardTestTable, systemGeneralGroup=systemGeneralGroup, cadEquipmentMib=cadEquipmentMib, cardTestDescription=cardTestDescription, diskVolumeDiskSize=diskVolumeDiskSize, cardShelfId=cardShelfId, cardTemperature=cardTemperature, diskVolumeShelfId=diskVolumeShelfId, cardType=cardType, cardShelfTestIndex=cardShelfTestIndex, cardRamSize=cardRamSize, portAdminState=portAdminState, cardEntry=cardEntry, portType=portType, portMacIfIndex=portMacIfIndex, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeEntry=diskVolumeEntry, cardTestCommand=cardTestCommand, cardSecStateChange=cardSecStateChange, cardHwVersion=cardHwVersion, equipmentStateGroup=equipmentStateGroup, cardTrapInh=cardTrapInh, diskVolumeId=diskVolumeId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portPrStateChange=portPrStateChange, portDetectedMode=portDetectedMode, portCardId=portCardId, TestTransactionId=TestTransactionId, systemGeneral=systemGeneral, portCurrDsPower=portCurrDsPower, portTxFlowControlMode=portTxFlowControlMode, cmResetUpTime=cmResetUpTime, diskVolumeFileName=diskVolumeFileName, shelfName=shelfName, portId=portId, portDescription=portDescription, systemClock=systemClock, cardCpuType=cardCpuType, cardNumber=cardNumber, cardUpTime=cardUpTime, portNumChans=portNumChans, auditEntry=auditEntry, auditShelfId=auditShelfId, shelfNumber=shelfNumber, portTrapInh=portTrapInh, equipmentTraps=equipmentTraps, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentDiag=equipmentDiag, auditTable=auditTable, equipmentCardGroup=equipmentCardGroup, cardSubDetected=cardSubDetected, auditName=auditName, diskVolumeDriveId=diskVolumeDriveId, cardTestResult=cardTestResult, portSecState=portSecState, cardFwVersion=cardFwVersion, cmResetClearNotification=cmResetClearNotification, cardSwVersion=cardSwVersion, cardPicSerialNum=cardPicSerialNum, cmPrimarySID=cmPrimarySID, cardTempNoReportNotification=cardTempNoReportNotification, auditLogOutput=auditLogOutput, auditLogThrottle=auditLogThrottle, cardLastBootSource=cardLastBootSource, TestScheduleCommand=TestScheduleCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSecState=cardSecState, auditFailedCount=auditFailedCount, equipmentError=equipmentError, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentAudit=equipmentAudit, auditCycleCount=auditCycleCount, portAction=portAction, cardTable=cardTable, diskVolumeGroup=diskVolumeGroup, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestSchedule=cardTestSchedule, cmResetInfo=cmResetInfo, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portMinDsPower=portMinDsPower, portDplxStatusChange=portDplxStatusChange, equipmentShelfGroup=equipmentShelfGroup, equipmentDiagGroup=equipmentDiagGroup, equipmentTbl=equipmentTbl, portRxFlowControlDetected=portRxFlowControlDetected, cardPicHwVersion=cardPicHwVersion, portPrState=portPrState, errEvLoggingEnabled=errEvLoggingEnabled, portTable=portTable, auditResult=auditResult, cmResetReason=cmResetReason, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeTable=diskVolumeTable, TestSchedule=TestSchedule, portConnectorId=portConnectorId, auditTotalFailedCount=auditTotalFailedCount, portCardSubType=portCardSubType, cardLastResetReason=cardLastResetReason, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestScheduleCommand=cardTestScheduleCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentPortGroup=equipmentPortGroup, cardCarrierFwVersion=cardCarrierFwVersion, cardName=cardName, portBgpId=portBgpId, remainInDiagMode=remainInDiagMode, errEvLogLevel=errEvLogLevel, equipmentState=equipmentState, portDplxStatus=portDplxStatus, cmUChannelID=cmUChannelID, portMode=portMode, equipmentAuditGroup=equipmentAuditGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeNumber=diskVolumeNumber, cardSubType=cardSubType, equipmentCardTestGroup=equipmentCardTestGroup, cardFlashSize=cardFlashSize, diskVolumeCardId=diskVolumeCardId, PYSNMP_MODULE_ID=cadEquipmentMib, portLastChangeTime=portLastChangeTime, cardAction=cardAction, cardPrStateChange=cardPrStateChange, cardTestTime=cardTestTime, diskDriveNumber=diskDriveNumber, cardPrState=cardPrState, cardTestResultDesc=cardTestResultDesc, portRxFlowControlMode=portRxFlowControlMode, cardSerialNum=cardSerialNum, cardBootVersion=cardBootVersion, cardPicDetected=cardPicDetected, errEvRowStatus=errEvRowStatus, diskVolumeSize=diskVolumeSize, portMacAddress=portMacAddress, cardCpuSpeed=cardCpuSpeed, auditStatus=auditStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetStatus=cmResetStatus, shelfObjects=shelfObjects, cardDplxStatus=cardDplxStatus, cardDetectedChange=cardDetectedChange, cardBusSpeed=cardBusSpeed, cardSpareGroupMode=cardSpareGroupMode, portShelfId=portShelfId, auditId=auditId, cadEquipmentMibConformance=cadEquipmentMibConformance, auditTime=auditTime, TestResult=TestResult, portNumber=portNumber, TestId=TestId, cardDetected=cardDetected, eqDiagConfig=eqDiagConfig, auditPassedCount=auditPassedCount, cardCPLDVersion=cardCPLDVersion, diskVolumeTrapInh=diskVolumeTrapInh, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestName=cardTestName, cmIpAddress=cmIpAddress, cmVendor=cmVendor, cardDplxStatusChange=cardDplxStatusChange, portMaxDsPower=portMaxDsPower, portSecStateChange=portSecStateChange, auditDescription=auditDescription, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTempOverHeatNotification=cardTempOverHeatNotification, cmRegistrationNotification=cmRegistrationNotification, cardTestType=cardTestType, cardCarrierHwVersion=cardCarrierHwVersion, cardTemperatureHighError=cardTemperatureHighError, cardAdminState=cardAdminState, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupId=cardSpareGroupId, cardCarrierSerialNum=cardCarrierSerialNum, portTxFlowControlDetected=portTxFlowControlDetected, shelfSwVersion=shelfSwVersion, diagTestId=diagTestId, trapSeverity=trapSeverity, cardTestResultNotification=cardTestResultNotification)
