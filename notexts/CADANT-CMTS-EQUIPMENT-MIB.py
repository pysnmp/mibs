...
FirmwareSource, PortId, CardSubType, CardType, SecondaryState, PortType, PrimaryState, AdminState, PortMode, PortDetectedMode, DiskVolumeUsageLevel, PicType, ShelfId, FlowControlMode, UnknownState, DuplexStatus, EqActionType, CardId = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "PortId", "CardSubType", "CardType", "SecondaryState", "PortType", "PrimaryState", "AdminState", "PortMode", "PortDetectedMode", "DiskVolumeUsageLevel", "PicType", "ShelfId", "FlowControlMode", "UnknownState", "DuplexStatus", "EqActionType", "CardId")
...
MacAddress, RowStatus, TruthValue, TextualConvention, DisplayString, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "TruthValue", "TextualConvention", "DisplayString", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResultDesc=cardTestResultDesc, diskVolumeTable=diskVolumeTable, shelfObjects=shelfObjects, diskVolumeDiskSize=diskVolumeDiskSize, equipmentCardTestGroup=equipmentCardTestGroup, portBgpId=portBgpId, cardTrapInh=cardTrapInh, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmIpAddress=cmIpAddress, cardBusSpeed=cardBusSpeed, cardCarrierSerialNum=cardCarrierSerialNum, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeNumber=diskVolumeNumber, auditFailedCount=auditFailedCount, cardTestIndex=cardTestIndex, cardSpareGroupMode=cardSpareGroupMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskDriveNumber=diskDriveNumber, cardLastBootVersion=cardLastBootVersion, cardCpuType=cardCpuType, portAction=portAction, portCardSubType=portCardSubType, portPrStateChange=portPrStateChange, cardHwVersion=cardHwVersion, cardId=cardId, cardSubType=cardSubType, systemClock=systemClock, errEvRowStatus=errEvRowStatus, equipmentTbl=equipmentTbl, portTrapInh=portTrapInh, cardShelfId=cardShelfId, portId=portId, cardName=cardName, shelfNumber=shelfNumber, cmMacAddress=cmMacAddress, cmResetUpTime=cmResetUpTime, portDocsIfIndex=portDocsIfIndex, diskVolumeId=diskVolumeId, auditEntry=auditEntry, cardRamSize=cardRamSize, cardTestId=cardTestId, auditId=auditId, cardAction=cardAction, cardFlashSize=cardFlashSize, portShelfId=portShelfId, cmResetReason=cmResetReason, auditTime=auditTime, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeDriveId=diskVolumeDriveId, portPrState=portPrState, portCurrDsPower=portCurrDsPower, remainInDiagMode=remainInDiagMode, cardTestTime=cardTestTime, cardTestType=cardTestType, TestType=TestType, portMinDsPower=portMinDsPower, TestTransactionId=TestTransactionId, eqDiagConfig=eqDiagConfig, cardTempOverHeatNotification=cardTempOverHeatNotification, portDetectedMode=portDetectedMode, TestId=TestId, auditTable=auditTable, cardNumber=cardNumber, cardPrState=cardPrState, portCardId=portCardId, auditCommand=auditCommand, portDplxStatusChange=portDplxStatusChange, portDescription=portDescription, cardTestEntry=cardTestEntry, cadEquipmentMib=cadEquipmentMib, cardTestName=cardTestName, cmResetStatus=cmResetStatus, cardSecState=cardSecState, cardPrStateChange=cardPrStateChange, equipmentAuditGroup=equipmentAuditGroup, cardPicHwVersion=cardPicHwVersion, equipmentShelfGroup=equipmentShelfGroup, verboseLevel=verboseLevel, portNumChans=portNumChans, systemGeneral=systemGeneral, portMaxDsPower=portMaxDsPower, consoleOutput=consoleOutput, errEvLoggingEnabled=errEvLoggingEnabled, portMacAddress=portMacAddress, portConnectorId=portConnectorId, cardType=cardType, trapCounter=trapCounter, diskVolumeEntry=diskVolumeEntry, cmResetNotification=cmResetNotification, portMode=portMode, auditResult=auditResult, portSecStateChange=portSecStateChange, portTable=portTable, cardFwVersion=cardFwVersion, cardTestDescription=cardTestDescription, portEntry=portEntry, diskVolumeCardId=diskVolumeCardId, cardNumPorts=cardNumPorts, equipmentTraps=equipmentTraps, equipmentStateGroup=equipmentStateGroup, equipmentDiag=equipmentDiag, cardTestTable=cardTestTable, auditShelfId=auditShelfId, cardTemperatureHighWarn=cardTemperatureHighWarn, auditLogThrottle=auditLogThrottle, equipmentCardGroup=equipmentCardGroup, cardAdminState=cardAdminState, cardTemperatureHighError=cardTemperatureHighError, portAdminState=portAdminState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmPrimarySID=cmPrimarySID, diskVolumeTrapInh=diskVolumeTrapInh, cardTestCommand=cardTestCommand, portTxFlowControlMode=portTxFlowControlMode, diskVolumeSize=diskVolumeSize, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMibConformance=cadEquipmentMibConformance, auditCycleCount=auditCycleCount, equipmentAudit=equipmentAudit, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portSecState=portSecState, auditAutoScheduling=auditAutoScheduling, shelfName=shelfName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCarrierHwVersion=cardCarrierHwVersion, portType=portType, cardShelfTestIndex=cardShelfTestIndex, cardTable=cardTable, diagTestId=diagTestId, cmRegistrationNotification=cmRegistrationNotification, portDplxStatus=portDplxStatus, cardTestResult=cardTestResult, auditPassedCount=auditPassedCount, cardTestScheduleCommand=cardTestScheduleCommand, portNumber=portNumber, auditTotalPassedCount=auditTotalPassedCount, diskVolumeFileName=diskVolumeFileName, cardTestResultNotification=cardTestResultNotification, systemGeneralGroup=systemGeneralGroup, auditName=auditName, cardDetectedChange=cardDetectedChange, portMacIfIndex=portMacIfIndex, errEventId=errEventId, TestCommand=TestCommand, diskVolumeGroup=diskVolumeGroup, cardFwUpdateStatus=cardFwUpdateStatus, TestScheduleCommand=TestScheduleCommand, diskVolumeName=diskVolumeName, auditLogOutput=auditLogOutput, equipmentPortGroup=equipmentPortGroup, cardLastBootSource=cardLastBootSource, cadEquipmentMibGroup=cadEquipmentMibGroup, errEvLogLevel=errEvLogLevel, auditCardId=auditCardId, cardDetected=cardDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentState=equipmentState, diskVolumeUsageLevel=diskVolumeUsageLevel, portLastChangeTime=portLastChangeTime, cmResetClearNotification=cmResetClearNotification, cardDplxStatus=cardDplxStatus, cardBootVersion=cardBootVersion, cardSerialNum=cardSerialNum, cmResetInfo=cmResetInfo, cmUChannelID=cmUChannelID, cardCpuSpeed=cardCpuSpeed, cardPicDetected=cardPicDetected, downstreamPowerLoss=downstreamPowerLoss, cardSecStateChange=cardSecStateChange, shelfSwVersion=shelfSwVersion, cardCarrierFwVersion=cardCarrierFwVersion, cardCPLDVersion=cardCPLDVersion, portRxFlowControlMode=portRxFlowControlMode, auditStatus=auditStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeShelfId=diskVolumeShelfId, cardSpareGroupId=cardSpareGroupId, portTxFlowControlDetected=portTxFlowControlDetected, auditDescription=auditDescription, cardTestSchedule=cardTestSchedule, equipmentError=equipmentError, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, trapSeverity=trapSeverity, TestResult=TestResult, cardDplxStatusChange=cardDplxStatusChange, cardTestTransId=cardTestTransId, cmVendor=cmVendor, cmDevice=cmDevice, TestSchedule=TestSchedule, equipmentDiagGroup=equipmentDiagGroup, cardEntry=cardEntry, cardLastChangeTime=cardLastChangeTime, PYSNMP_MODULE_ID=cadEquipmentMib, cardTempNoReportNotification=cardTempNoReportNotification, cardPicSerialNum=cardPicSerialNum, cardSwVersion=cardSwVersion, systemKey=systemKey, portRxFlowControlDetected=portRxFlowControlDetected, cardTemperature=cardTemperature, auditTotalFailedCount=auditTotalFailedCount, cardFpgaSource=cardFpgaSource, cardSubDetected=cardSubDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, cardUpTime=cardUpTime, cardLastResetReason=cardLastResetReason)
