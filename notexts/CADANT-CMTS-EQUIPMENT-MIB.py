...
PicType, CardSubType, PortDetectedMode, PortId, ShelfId, PortType, SecondaryState, CardType, FlowControlMode, AdminState, PortMode, UnknownState, CardId, EqActionType, DiskVolumeUsageLevel, FirmwareSource, DuplexStatus, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "PicType", "CardSubType", "PortDetectedMode", "PortId", "ShelfId", "PortType", "SecondaryState", "CardType", "FlowControlMode", "AdminState", "PortMode", "UnknownState", "CardId", "EqActionType", "DiskVolumeUsageLevel", "FirmwareSource", "DuplexStatus", "PrimaryState")
...
RowStatus, DateAndTime, TimeStamp, TextualConvention, DisplayString, MacAddress, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "TimeStamp", "TextualConvention", "DisplayString", "MacAddress", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSerialNum=cardSerialNum, portCardSubType=portCardSubType, diskVolumeDiskSize=diskVolumeDiskSize, errEventId=errEventId, systemKey=systemKey, TestScheduleCommand=TestScheduleCommand, equipmentCardTestGroup=equipmentCardTestGroup, cardFwVersion=cardFwVersion, cardSpareGroupMode=cardSpareGroupMode, auditFailedCount=auditFailedCount, trapSeverity=trapSeverity, cardTestSchedule=cardTestSchedule, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetNotification=cmResetNotification, cardBusSpeed=cardBusSpeed, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTemperatureHighError=cardTemperatureHighError, portDocsIfIndex=portDocsIfIndex, diskVolumeTrapInh=diskVolumeTrapInh, portPrState=portPrState, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsageNotification=diskVolumeUsageNotification, systemGeneralGroup=systemGeneralGroup, cmIpAddress=cmIpAddress, portMacIfIndex=portMacIfIndex, cardDplxStatusChange=cardDplxStatusChange, equipmentState=equipmentState, auditAutoScheduling=auditAutoScheduling, cardAdminState=cardAdminState, cardShelfId=cardShelfId, diskVolumeSize=diskVolumeSize, auditShelfId=auditShelfId, cardCarrierSerialNum=cardCarrierSerialNum, cardTestEntry=cardTestEntry, cardTestTime=cardTestTime, cardPrStateChange=cardPrStateChange, verboseLevel=verboseLevel, equipmentTbl=equipmentTbl, cardTemperatureHighWarn=cardTemperatureHighWarn, portId=portId, auditLogOutput=auditLogOutput, auditEntry=auditEntry, auditCycleCount=auditCycleCount, cardLastBootSource=cardLastBootSource, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeName=diskVolumeName, systemGeneral=systemGeneral, portMacAddress=portMacAddress, cardPicSerialNum=cardPicSerialNum, errEvRowStatus=errEvRowStatus, cardTrapInh=cardTrapInh, TestTransactionId=TestTransactionId, consoleOutput=consoleOutput, cardTemperature=cardTemperature, shelfSwVersion=shelfSwVersion, cmVendor=cmVendor, cmResetReason=cmResetReason, cmResetInfo=cmResetInfo, cardTestResultNotification=cardTestResultNotification, equipmentCardGroup=equipmentCardGroup, cardTable=cardTable, diskVolumeTable=diskVolumeTable, shelfNumber=shelfNumber, cardSecStateChange=cardSecStateChange, cardTestTable=cardTestTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardAction=cardAction, portAction=portAction, portDplxStatus=portDplxStatus, TestSchedule=TestSchedule, downstreamPowerLoss=downstreamPowerLoss, cardSecState=cardSecState, portRxFlowControlDetected=portRxFlowControlDetected, shelfObjects=shelfObjects, portTable=portTable, portType=portType, diskVolumeCardId=diskVolumeCardId, auditPassedCount=auditPassedCount, cardLastResetReason=cardLastResetReason, portMinDsPower=portMinDsPower, cardCarrierHwVersion=cardCarrierHwVersion, cardBootVersion=cardBootVersion, portSecState=portSecState, portRxFlowControlMode=portRxFlowControlMode, cardTestTransId=cardTestTransId, remainInDiagMode=remainInDiagMode, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetStatus=cmResetStatus, equipmentTraps=equipmentTraps, cardCpuType=cardCpuType, portMode=portMode, cadEquipmentMib=cadEquipmentMib, errEvLogLevel=errEvLogLevel, cardName=cardName, portDplxStatusChange=portDplxStatusChange, cardTestName=cardTestName, cardTestType=cardTestType, cmMacAddress=cmMacAddress, cardDetectedChange=cardDetectedChange, equipmentShelfGroup=equipmentShelfGroup, cardPicDetected=cardPicDetected, auditCardId=auditCardId, TestCommand=TestCommand, cardSubType=cardSubType, TestType=TestType, cardLastBootVersion=cardLastBootVersion, cmUChannelID=cmUChannelID, portSecStateChange=portSecStateChange, cmResetClearNotification=cmResetClearNotification, portAdminState=portAdminState, cardNumber=cardNumber, cardSwVersion=cardSwVersion, equipmentDiag=equipmentDiag, equipmentAudit=equipmentAudit, cardTestResult=cardTestResult, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portCardId=portCardId, cardPicHwVersion=cardPicHwVersion, portNumChans=portNumChans, systemClock=systemClock, cardTestIndex=cardTestIndex, auditTable=auditTable, diskVolumeFileName=diskVolumeFileName, PYSNMP_MODULE_ID=cadEquipmentMib, TestResult=TestResult, cmRegistrationNotification=cmRegistrationNotification, auditTotalPassedCount=auditTotalPassedCount, cmPrimarySID=cmPrimarySID, cardCarrierFwVersion=cardCarrierFwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfName=shelfName, portEntry=portEntry, cardShelfTestIndex=cardShelfTestIndex, portDescription=portDescription, portShelfId=portShelfId, portBgpId=portBgpId, cardTestId=cardTestId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardId=cardId, auditDescription=auditDescription, diagTestId=diagTestId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestId=TestId, auditTotalFailedCount=auditTotalFailedCount, auditCommand=auditCommand, auditName=auditName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeId=diskVolumeId, portNumber=portNumber, auditStatus=auditStatus, portLastChangeTime=portLastChangeTime, eqDiagConfig=eqDiagConfig, cardCpuSpeed=cardCpuSpeed, cardHwVersion=cardHwVersion, cardTestResultDesc=cardTestResultDesc, cmResetUpTime=cmResetUpTime, trapCounter=trapCounter, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeNumber=diskVolumeNumber, cardFpgaSource=cardFpgaSource, portConnectorId=portConnectorId, equipmentPortGroup=equipmentPortGroup, cardCPLDVersion=cardCPLDVersion, cardLastChangeTime=cardLastChangeTime, diskVolumeShelfId=diskVolumeShelfId, auditLogThrottle=auditLogThrottle, portTxFlowControlMode=portTxFlowControlMode, equipmentStateGroup=equipmentStateGroup, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestDescription=cardTestDescription, cardEntry=cardEntry, portCurrDsPower=portCurrDsPower, cardTestCommand=cardTestCommand, fpgaErrorEventTable=fpgaErrorEventTable, cardType=cardType, diskDriveNumber=diskDriveNumber, cardFwUpdateStatus=cardFwUpdateStatus, portMaxDsPower=portMaxDsPower, portPrStateChange=portPrStateChange, cardFlashSize=cardFlashSize, portTrapInh=portTrapInh, cardSubDetected=cardSubDetected, equipmentDiagGroup=equipmentDiagGroup, cmDevice=cmDevice, cardNumPorts=cardNumPorts, cardRamSize=cardRamSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cadEquipmentMibConformance=cadEquipmentMibConformance, cardDetected=cardDetected, cardTestScheduleCommand=cardTestScheduleCommand, equipmentError=equipmentError, equipmentAuditGroup=equipmentAuditGroup, auditId=auditId, errEvLoggingEnabled=errEvLoggingEnabled, auditResult=auditResult, cardPrState=cardPrState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portDetectedMode=portDetectedMode, cardUpTime=cardUpTime, diskVolumeDriveId=diskVolumeDriveId, diskVolumeGroup=diskVolumeGroup, auditTime=auditTime, portTxFlowControlDetected=portTxFlowControlDetected, cardSpareGroupId=cardSpareGroupId, cardDplxStatus=cardDplxStatus)
