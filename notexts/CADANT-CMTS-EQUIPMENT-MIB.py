...
EqActionType, ShelfId, DuplexStatus, FirmwareSource, DiskVolumeUsageLevel, PicType, PortMode, PortType, PrimaryState, SecondaryState, UnknownState, FlowControlMode, CardSubType, CardId, PortId, PortDetectedMode, AdminState, CardType = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "ShelfId", "DuplexStatus", "FirmwareSource", "DiskVolumeUsageLevel", "PicType", "PortMode", "PortType", "PrimaryState", "SecondaryState", "UnknownState", "FlowControlMode", "CardSubType", "CardId", "PortId", "PortDetectedMode", "AdminState", "CardType")
...
DateAndTime, TruthValue, RowStatus, DisplayString, MacAddress, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TruthValue", "RowStatus", "DisplayString", "MacAddress", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmPrimarySID=cmPrimarySID, cardNumPorts=cardNumPorts, cmVendor=cmVendor, diskVolumeDriveId=diskVolumeDriveId, trapSeverity=trapSeverity, cardHwVersion=cardHwVersion, auditAutoScheduling=auditAutoScheduling, portSecState=portSecState, TestTransactionId=TestTransactionId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardFwUpdateStatus=cardFwUpdateStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, cardLastBootSource=cardLastBootSource, cardRamSize=cardRamSize, auditTime=auditTime, systemGeneral=systemGeneral, portCardId=portCardId, remainInDiagMode=remainInDiagMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTemperatureHighError=cardTemperatureHighError, cardTestId=cardTestId, errEvRowStatus=errEvRowStatus, cardTestScheduleCommand=cardTestScheduleCommand, portRxFlowControlDetected=portRxFlowControlDetected, portDocsIfIndex=portDocsIfIndex, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditCycleCount=auditCycleCount, cardTestSchedule=cardTestSchedule, auditCommand=auditCommand, cmIpAddress=cmIpAddress, shelfNumber=shelfNumber, equipmentStateGroup=equipmentStateGroup, cmResetReason=cmResetReason, cardTrapInh=cardTrapInh, cardType=cardType, equipmentState=equipmentState, TestScheduleCommand=TestScheduleCommand, shelfObjects=shelfObjects, cardFpgaSource=cardFpgaSource, portDplxStatusChange=portDplxStatusChange, portTxFlowControlMode=portTxFlowControlMode, equipmentShelfGroup=equipmentShelfGroup, equipmentTraps=equipmentTraps, diskVolumeFileName=diskVolumeFileName, cardTestIndex=cardTestIndex, diskVolumeName=diskVolumeName, cardSpareGroupId=cardSpareGroupId, cardName=cardName, cmResetNotification=cmResetNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditTotalPassedCount=auditTotalPassedCount, fpgaErrorEventTable=fpgaErrorEventTable, cmDevice=cmDevice, cardCpuType=cardCpuType, diskVolumeGroup=diskVolumeGroup, cardSpareGroupMode=cardSpareGroupMode, cardTestResultNotification=cardTestResultNotification, diskVolumeId=diskVolumeId, diskVolumeDiskSize=diskVolumeDiskSize, trapCounter=trapCounter, verboseLevel=verboseLevel, portTable=portTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestCommand=TestCommand, cardTemperature=cardTemperature, cardFlashSize=cardFlashSize, cardCarrierFwVersion=cardCarrierFwVersion, auditDescription=auditDescription, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeTable=diskVolumeTable, diskVolumeEntry=diskVolumeEntry, cardTestTransId=cardTestTransId, equipmentDiag=equipmentDiag, portAdminState=portAdminState, cardNumber=cardNumber, cardDetectedChange=cardDetectedChange, equipmentCardTestGroup=equipmentCardTestGroup, cardBusSpeed=cardBusSpeed, cardLastBootVersion=cardLastBootVersion, cardSubType=cardSubType, cardTestDescription=cardTestDescription, auditResult=auditResult, portTxFlowControlDetected=portTxFlowControlDetected, TestId=TestId, diskVolumeSize=diskVolumeSize, auditEntry=auditEntry, portMinDsPower=portMinDsPower, diskDriveNumber=diskDriveNumber, cardTestTime=cardTestTime, diskVolumeCardId=diskVolumeCardId, diagTestId=diagTestId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicHwVersion=cardPicHwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSecStateChange=cardSecStateChange, portMacAddress=portMacAddress, eqDiagConfig=eqDiagConfig, shelfSwVersion=shelfSwVersion, cardShelfId=cardShelfId, portDescription=portDescription, auditStatus=auditStatus, equipmentError=equipmentError, portAction=portAction, portBgpId=portBgpId, cmMacAddress=cmMacAddress, cadEquipmentMibGroup=cadEquipmentMibGroup, portShelfId=portShelfId, cardTempNoReportNotification=cardTempNoReportNotification, cmUChannelID=cmUChannelID, cardTable=cardTable, systemKey=systemKey, auditShelfId=auditShelfId, cardAction=cardAction, cardPicDetected=cardPicDetected, auditFailedCount=auditFailedCount, errEventId=errEventId, equipmentTbl=equipmentTbl, equipmentAuditGroup=equipmentAuditGroup, cardAdminState=cardAdminState, cardCarrierSerialNum=cardCarrierSerialNum, cardTestResultDesc=cardTestResultDesc, portLastChangeTime=portLastChangeTime, equipmentPortGroup=equipmentPortGroup, cmResetInfo=cmResetInfo, cardCPLDVersion=cardCPLDVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, portRxFlowControlMode=portRxFlowControlMode, diskVolumeTrapInh=diskVolumeTrapInh, cardPicSerialNum=cardPicSerialNum, portId=portId, auditCardId=auditCardId, TestResult=TestResult, cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMib=cadEquipmentMib, auditId=auditId, cardTestResult=cardTestResult, equipmentDiagGroup=equipmentDiagGroup, cardPrState=cardPrState, cardFwVersion=cardFwVersion, cardCarrierHwVersion=cardCarrierHwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portCardSubType=portCardSubType, cardLastResetReason=cardLastResetReason, cardSecState=cardSecState, consoleOutput=consoleOutput, portTrapInh=portTrapInh, errEvRecoveryEnabled=errEvRecoveryEnabled, cardUpTime=cardUpTime, portDplxStatus=portDplxStatus, cardBootVersion=cardBootVersion, auditLogOutput=auditLogOutput, auditTable=auditTable, fpgaErrorEventEntry=fpgaErrorEventEntry, TestType=TestType, TestSchedule=TestSchedule, cardTestCommand=cardTestCommand, cmResetUpTime=cmResetUpTime, errEvLogLevel=errEvLogLevel, fpgaErrorEventGroup=fpgaErrorEventGroup, auditLogThrottle=auditLogThrottle, portDetectedMode=portDetectedMode, cardDplxStatus=cardDplxStatus, portPrState=portPrState, portMode=portMode, portCurrDsPower=portCurrDsPower, portEntry=portEntry, portMaxDsPower=portMaxDsPower, cmResetStatus=cmResetStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardDplxStatusChange=cardDplxStatusChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portType=portType, portNumChans=portNumChans, cardCpuSpeed=cardCpuSpeed, cardTestEntry=cardTestEntry, portNumber=portNumber, cardLastChangeTime=cardLastChangeTime, portMacIfIndex=portMacIfIndex, diskVolumeShelfId=diskVolumeShelfId, cardId=cardId, portSecStateChange=portSecStateChange, cardTestName=cardTestName, cardDetected=cardDetected, cmRegistrationNotification=cmRegistrationNotification, diskVolumeNumber=diskVolumeNumber, auditTotalFailedCount=auditTotalFailedCount, auditName=auditName, equipmentAudit=equipmentAudit, errEvLoggingEnabled=errEvLoggingEnabled, cardPrStateChange=cardPrStateChange, cardTestType=cardTestType, cmResetClearNotification=cmResetClearNotification, shelfName=shelfName, cardTestTable=cardTestTable, diskVolumeUsageNotification=diskVolumeUsageNotification, systemGeneralGroup=systemGeneralGroup, systemClock=systemClock, cardSwVersion=cardSwVersion, auditPassedCount=auditPassedCount, PYSNMP_MODULE_ID=cadEquipmentMib, portConnectorId=portConnectorId, cardEntry=cardEntry, cardSerialNum=cardSerialNum, equipmentCardGroup=equipmentCardGroup, portPrStateChange=portPrStateChange, cardSubDetected=cardSubDetected, downstreamPowerLoss=downstreamPowerLoss)
