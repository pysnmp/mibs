...
FirmwareSource, ShelfId, CardId, DiskVolumeUsageLevel, EqActionType, DuplexStatus, PortType, PortMode, PicType, PrimaryState, AdminState, CardType, SecondaryState, CardSubType, PortId, UnknownState, FlowControlMode, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "ShelfId", "CardId", "DiskVolumeUsageLevel", "EqActionType", "DuplexStatus", "PortType", "PortMode", "PicType", "PrimaryState", "AdminState", "CardType", "SecondaryState", "CardSubType", "PortId", "UnknownState", "FlowControlMode", "PortDetectedMode")
...
DateAndTime, RowStatus, TruthValue, TimeStamp, DisplayString, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "RowStatus", "TruthValue", "TimeStamp", "DisplayString", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPicDetected=cardPicDetected, cardLastChangeTime=cardLastChangeTime, cardCPLDVersion=cardCPLDVersion, cmDevice=cmDevice, cardName=cardName, portDplxStatus=portDplxStatus, portBgpId=portBgpId, portCardSubType=portCardSubType, cmResetNotification=cmResetNotification, cardAdminState=cardAdminState, cardTestTime=cardTestTime, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwVersion=cardFwVersion, auditStatus=auditStatus, diskVolumeDriveId=diskVolumeDriveId, cardCpuType=cardCpuType, portRxFlowControlMode=portRxFlowControlMode, portSecState=portSecState, portMacIfIndex=portMacIfIndex, cardTestScheduleCommand=cardTestScheduleCommand, cadEquipmentMib=cadEquipmentMib, portCurrDsPower=portCurrDsPower, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestTransId=cardTestTransId, cmRegistrationNotification=cmRegistrationNotification, shelfName=shelfName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditTable=auditTable, diskVolumeShelfId=diskVolumeShelfId, cardTestResultDesc=cardTestResultDesc, cmMacAddress=cmMacAddress, TestCommand=TestCommand, TestTransactionId=TestTransactionId, portPrState=portPrState, cardSubType=cardSubType, auditLogThrottle=auditLogThrottle, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentCardTestGroup=equipmentCardTestGroup, cardSubDetected=cardSubDetected, auditShelfId=auditShelfId, cardTestDescription=cardTestDescription, cardTestType=cardTestType, portLastChangeTime=portLastChangeTime, cardTestSchedule=cardTestSchedule, systemGeneral=systemGeneral, equipmentTraps=equipmentTraps, cmIpAddress=cmIpAddress, cardSecStateChange=cardSecStateChange, diskDriveNumber=diskDriveNumber, cardHwVersion=cardHwVersion, shelfNumber=shelfNumber, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditEntry=auditEntry, equipmentDiag=equipmentDiag, auditId=auditId, systemKey=systemKey, auditLogOutput=auditLogOutput, equipmentCardGroup=equipmentCardGroup, diskVolumeId=diskVolumeId, cmVendor=cmVendor, cardLastBootVersion=cardLastBootVersion, portSecStateChange=portSecStateChange, diskVolumeTable=diskVolumeTable, TestScheduleCommand=TestScheduleCommand, consoleOutput=consoleOutput, cmResetInfo=cmResetInfo, auditTotalFailedCount=auditTotalFailedCount, diskVolumeEntry=diskVolumeEntry, cardTestTable=cardTestTable, errEvRecoveryEnabled=errEvRecoveryEnabled, portNumChans=portNumChans, auditTime=auditTime, cardTestIndex=cardTestIndex, cardDplxStatusChange=cardDplxStatusChange, diskVolumeSize=diskVolumeSize, auditTotalPassedCount=auditTotalPassedCount, cardNumPorts=cardNumPorts, diskVolumeTrapInh=diskVolumeTrapInh, errEvRowStatus=errEvRowStatus, cardCpuSpeed=cardCpuSpeed, cardCarrierSerialNum=cardCarrierSerialNum, portTrapInh=portTrapInh, portDplxStatusChange=portDplxStatusChange, portType=portType, systemClock=systemClock, cardFlashSize=cardFlashSize, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSerialNum=cardSerialNum, auditDescription=auditDescription, cardTestName=cardTestName, cardLastBootSource=cardLastBootSource, cardDetectedChange=cardDetectedChange, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSecState=cardSecState, errEventId=errEventId, trapCounter=trapCounter, cardBootVersion=cardBootVersion, cardBusSpeed=cardBusSpeed, diskVolumeUsageLevel=diskVolumeUsageLevel, TestSchedule=TestSchedule, cardCarrierFwVersion=cardCarrierFwVersion, portMacAddress=portMacAddress, equipmentError=equipmentError, portMaxDsPower=portMaxDsPower, cardNumber=cardNumber, cmResetReason=cmResetReason, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeName=diskVolumeName, auditCommand=auditCommand, cardTestResult=cardTestResult, equipmentState=equipmentState, cadEquipmentMibConformance=cadEquipmentMibConformance, portDescription=portDescription, cardEntry=cardEntry, TestType=TestType, cardTestId=cardTestId, auditCycleCount=auditCycleCount, cardSwVersion=cardSwVersion, diskVolumeFileName=diskVolumeFileName, diskVolumeGroup=diskVolumeGroup, cardShelfId=cardShelfId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTable=cardTable, TestId=TestId, cardPrStateChange=cardPrStateChange, equipmentTbl=equipmentTbl, portConnectorId=portConnectorId, portRxFlowControlDetected=portRxFlowControlDetected, portPrStateChange=portPrStateChange, cardLastResetReason=cardLastResetReason, cardPicHwVersion=cardPicHwVersion, auditCardId=auditCardId, portDetectedMode=portDetectedMode, equipmentAuditGroup=equipmentAuditGroup, cardTrapInh=cardTrapInh, cardTempNoReportNotification=cardTempNoReportNotification, cmResetStatus=cmResetStatus, cmUChannelID=cmUChannelID, cardShelfTestIndex=cardShelfTestIndex, cmResetUpTime=cmResetUpTime, cardRamSize=cardRamSize, diagTestId=diagTestId, downstreamPowerLoss=downstreamPowerLoss, portMode=portMode, cmPrimarySID=cmPrimarySID, systemGeneralGroup=systemGeneralGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardDplxStatus=cardDplxStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentPortGroup=equipmentPortGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardCarrierHwVersion=cardCarrierHwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, shelfObjects=shelfObjects, portId=portId, cardFwUpdateStatus=cardFwUpdateStatus, cardFpgaSource=cardFpgaSource, cardTemperatureHighWarn=cardTemperatureHighWarn, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMibGroup=cadEquipmentMibGroup, errEvLoggingEnabled=errEvLoggingEnabled, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeNumber=diskVolumeNumber, portDocsIfIndex=portDocsIfIndex, portNumber=portNumber, shelfSwVersion=shelfSwVersion, cardSpareGroupId=cardSpareGroupId, portMinDsPower=portMinDsPower, cmResetClearNotification=cmResetClearNotification, errEvLogLevel=errEvLogLevel, cardDetected=cardDetected, auditPassedCount=auditPassedCount, portCardId=portCardId, portAction=portAction, portTable=portTable, equipmentAudit=equipmentAudit, equipmentShelfGroup=equipmentShelfGroup, cardType=cardType, cardTemperature=cardTemperature, eqDiagConfig=eqDiagConfig, auditResult=auditResult, cardPrState=cardPrState, auditAutoScheduling=auditAutoScheduling, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighError=cardTemperatureHighError, portAdminState=portAdminState, auditName=auditName, diskVolumeCardId=diskVolumeCardId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentDiagGroup=equipmentDiagGroup, cardAction=cardAction, portShelfId=portShelfId, TestResult=TestResult, cardTestEntry=cardTestEntry, auditFailedCount=auditFailedCount, equipmentStateGroup=equipmentStateGroup, verboseLevel=verboseLevel, remainInDiagMode=remainInDiagMode, trapSeverity=trapSeverity, cardTestCommand=cardTestCommand, portEntry=portEntry, cardTestResultNotification=cardTestResultNotification, cardId=cardId, cardUpTime=cardUpTime)
