...
PortMode, SecondaryState, DiskVolumeUsageLevel, PrimaryState, CardId, EqActionType, CardType, FlowControlMode, PortDetectedMode, PortType, PicType, UnknownState, CardSubType, DuplexStatus, AdminState, FirmwareSource, PortId, ShelfId = mibBuilder.importSymbols("CADANT-TC", "PortMode", "SecondaryState", "DiskVolumeUsageLevel", "PrimaryState", "CardId", "EqActionType", "CardType", "FlowControlMode", "PortDetectedMode", "PortType", "PicType", "UnknownState", "CardSubType", "DuplexStatus", "AdminState", "FirmwareSource", "PortId", "ShelfId")
...
DateAndTime, RowStatus, TextualConvention, TimeStamp, TruthValue, MacAddress, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "RowStatus", "TextualConvention", "TimeStamp", "TruthValue", "MacAddress", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestName=cardTestName, auditShelfId=auditShelfId, cardRamSize=cardRamSize, cardLastBootSource=cardLastBootSource, cardUpTime=cardUpTime, portCurrDsPower=portCurrDsPower, cardCpuSpeed=cardCpuSpeed, auditCommand=auditCommand, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPrStateChange=cardPrStateChange, auditTime=auditTime, portPrState=portPrState, cadEquipmentMib=cadEquipmentMib, portDplxStatus=portDplxStatus, diskVolumeShelfId=diskVolumeShelfId, errEvLoggingEnabled=errEvLoggingEnabled, cmUChannelID=cmUChannelID, systemClock=systemClock, portCardId=portCardId, cardTrapInh=cardTrapInh, diskDriveNumber=diskDriveNumber, cardTestResultDesc=cardTestResultDesc, diagTestId=diagTestId, cardTestEntry=cardTestEntry, cardTempNoReportNotification=cardTempNoReportNotification, cardSecState=cardSecState, cardFwUpdateStatus=cardFwUpdateStatus, shelfName=shelfName, cardShelfId=cardShelfId, diskVolumeSize=diskVolumeSize, TestType=TestType, portSecStateChange=portSecStateChange, cardPicSerialNum=cardPicSerialNum, auditPassedCount=auditPassedCount, cardHwVersion=cardHwVersion, equipmentAuditGroup=equipmentAuditGroup, diskVolumeTrapInh=diskVolumeTrapInh, equipmentDiag=equipmentDiag, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardId=cardId, auditTotalFailedCount=auditTotalFailedCount, consoleOutput=consoleOutput, eqDiagConfig=eqDiagConfig, portType=portType, cardSerialNum=cardSerialNum, portMaxDsPower=portMaxDsPower, fpgaErrorEventEntry=fpgaErrorEventEntry, cardEntry=cardEntry, cardDplxStatusChange=cardDplxStatusChange, equipmentShelfGroup=equipmentShelfGroup, cardTestDescription=cardTestDescription, equipmentStateGroup=equipmentStateGroup, cardPicHwVersion=cardPicHwVersion, TestTransactionId=TestTransactionId, diskVolumeCardId=diskVolumeCardId, diskVolumeDriveId=diskVolumeDriveId, cmRegistrationNotification=cmRegistrationNotification, TestScheduleCommand=TestScheduleCommand, portTxFlowControlMode=portTxFlowControlMode, cardName=cardName, cardBootVersion=cardBootVersion, auditStatus=auditStatus, TestResult=TestResult, cardNumber=cardNumber, portMode=portMode, systemGeneralGroup=systemGeneralGroup, auditName=auditName, errEvRowStatus=errEvRowStatus, auditLogOutput=auditLogOutput, equipmentCardGroup=equipmentCardGroup, shelfSwVersion=shelfSwVersion, equipmentCardTestGroup=equipmentCardTestGroup, cardTable=cardTable, diskVolumeFileName=diskVolumeFileName, cardDetected=cardDetected, diskVolumeId=diskVolumeId, portTable=portTable, cardPicDetected=cardPicDetected, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeDiskSize=diskVolumeDiskSize, auditLogThrottle=auditLogThrottle, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuType=cardCpuType, portBgpId=portBgpId, auditTotalPassedCount=auditTotalPassedCount, portNumber=portNumber, portLastChangeTime=portLastChangeTime, verboseLevel=verboseLevel, cardTestSchedule=cardTestSchedule, cardTestResult=cardTestResult, TestSchedule=TestSchedule, cardDplxStatus=cardDplxStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestCommand=cardTestCommand, cardSecStateChange=cardSecStateChange, diskVolumeEntry=diskVolumeEntry, portAdminState=portAdminState, portTrapInh=portTrapInh, cardPrState=cardPrState, equipmentTraps=equipmentTraps, equipmentError=equipmentError, cmMacAddress=cmMacAddress, cardLastResetReason=cardLastResetReason, auditCycleCount=auditCycleCount, auditId=auditId, cmResetInfo=cmResetInfo, cmResetNotification=cmResetNotification, portMinDsPower=portMinDsPower, portEntry=portEntry, cardFwVersion=cardFwVersion, diskVolumeNumber=diskVolumeNumber, shelfNumber=shelfNumber, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, shelfObjects=shelfObjects, cardFlashSize=cardFlashSize, TestId=TestId, auditDescription=auditDescription, portRxFlowControlMode=portRxFlowControlMode, portConnectorId=portConnectorId, cardNumPorts=cardNumPorts, cardLastBootVersion=cardLastBootVersion, cmVendor=cmVendor, portDplxStatusChange=portDplxStatusChange, cardLastChangeTime=cardLastChangeTime, diskVolumeName=diskVolumeName, diskVolumeTable=diskVolumeTable, cardTemperature=cardTemperature, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCPLDVersion=cardCPLDVersion, equipmentDiagGroup=equipmentDiagGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentTbl=equipmentTbl, equipmentAudit=equipmentAudit, auditCardId=auditCardId, diskVolumeGroup=diskVolumeGroup, cardTestScheduleCommand=cardTestScheduleCommand, equipmentState=equipmentState, downstreamPowerLoss=downstreamPowerLoss, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestId=cardTestId, cardFpgaSource=cardFpgaSource, cardTestTable=cardTestTable, auditTable=auditTable, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDocsIfIndex=portDocsIfIndex, cardSpareGroupMode=cardSpareGroupMode, portShelfId=portShelfId, portCardSubType=portCardSubType, portRxFlowControlDetected=portRxFlowControlDetected, cardCarrierSerialNum=cardCarrierSerialNum, portNumChans=portNumChans, errEvLogLevel=errEvLogLevel, cardAction=cardAction, portDetectedMode=portDetectedMode, cardShelfTestIndex=cardShelfTestIndex, cmResetClearNotification=cmResetClearNotification, cardSwVersion=cardSwVersion, portDescription=portDescription, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibGroup=cadEquipmentMibGroup, portMacIfIndex=portMacIfIndex, auditFailedCount=auditFailedCount, cardTestResultNotification=cardTestResultNotification, cardBusSpeed=cardBusSpeed, portId=portId, cmResetUpTime=cmResetUpTime, auditResult=auditResult, cardCarrierFwVersion=cardCarrierFwVersion, systemGeneral=systemGeneral, cardType=cardType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmDevice=cmDevice, cardSpareGroupId=cardSpareGroupId, cardAdminState=cardAdminState, cardTemperatureHighError=cardTemperatureHighError, cardTestIndex=cardTestIndex, portAction=portAction, cmResetReason=cmResetReason, cmResetStatus=cmResetStatus, portMacAddress=portMacAddress, equipmentPortGroup=equipmentPortGroup, cardSubDetected=cardSubDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, trapSeverity=trapSeverity, fpgaErrorEventTable=fpgaErrorEventTable, trapCounter=trapCounter, errEvRecoveryEnabled=errEvRecoveryEnabled, systemKey=systemKey, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portSecState=portSecState, cardTestTransId=cardTestTransId, cardSubType=cardSubType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestType=cardTestType, TestCommand=TestCommand, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestTime=cardTestTime, errEventId=errEventId, cardDetectedChange=cardDetectedChange, portPrStateChange=portPrStateChange, remainInDiagMode=remainInDiagMode, auditEntry=auditEntry, cmIpAddress=cmIpAddress, cadEquipmentMibConformance=cadEquipmentMibConformance)
