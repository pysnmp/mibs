...
PicType, SecondaryState, UnknownState, AdminState, DiskVolumeUsageLevel, CardId, PortId, CardType, FlowControlMode, FirmwareSource, PortDetectedMode, ShelfId, EqActionType, CardSubType, PortType, PortMode, PrimaryState, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PicType", "SecondaryState", "UnknownState", "AdminState", "DiskVolumeUsageLevel", "CardId", "PortId", "CardType", "FlowControlMode", "FirmwareSource", "PortDetectedMode", "ShelfId", "EqActionType", "CardSubType", "PortType", "PortMode", "PrimaryState", "DuplexStatus")
...
MacAddress, TextualConvention, DisplayString, RowStatus, DateAndTime, TimeStamp, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TextualConvention", "DisplayString", "RowStatus", "DateAndTime", "TimeStamp", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFpgaSource=cardFpgaSource, equipmentTbl=equipmentTbl, auditCardId=auditCardId, auditPassedCount=auditPassedCount, portDetectedMode=portDetectedMode, cardRamSize=cardRamSize, cardFwUpdateStatus=cardFwUpdateStatus, portTrapInh=portTrapInh, portCardId=portCardId, portCardSubType=portCardSubType, auditShelfId=auditShelfId, errEventId=errEventId, cardFlashSize=cardFlashSize, cardPrStateChange=cardPrStateChange, cardSubDetected=cardSubDetected, cardShelfTestIndex=cardShelfTestIndex, auditResult=auditResult, equipmentStateGroup=equipmentStateGroup, cmPrimarySID=cmPrimarySID, equipmentState=equipmentState, cardTable=cardTable, fpgaErrorEventTable=fpgaErrorEventTable, equipmentTraps=equipmentTraps, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTemperatureHighWarn=cardTemperatureHighWarn, auditTime=auditTime, cardLastBootVersion=cardLastBootVersion, portTable=portTable, remainInDiagMode=remainInDiagMode, consoleOutput=consoleOutput, cardDetected=cardDetected, cadEquipmentMib=cadEquipmentMib, portRxFlowControlDetected=portRxFlowControlDetected, cardTestIndex=cardTestIndex, cardDetectedChange=cardDetectedChange, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cmResetStatus=cmResetStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskDriveNumber=diskDriveNumber, equipmentCardTestGroup=equipmentCardTestGroup, cmMacAddress=cmMacAddress, cardPicSerialNum=cardPicSerialNum, cmDevice=cmDevice, cmUChannelID=cmUChannelID, portDplxStatus=portDplxStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditCycleCount=auditCycleCount, shelfSwVersion=shelfSwVersion, TestSchedule=TestSchedule, portCurrDsPower=portCurrDsPower, portPrState=portPrState, TestTransactionId=TestTransactionId, cmResetUpTime=cmResetUpTime, diskVolumeUsageLevel=diskVolumeUsageLevel, auditTotalPassedCount=auditTotalPassedCount, portLastChangeTime=portLastChangeTime, verboseLevel=verboseLevel, auditFailedCount=auditFailedCount, diskVolumeDiskSize=diskVolumeDiskSize, cardTestCommand=cardTestCommand, auditTable=auditTable, portShelfId=portShelfId, diskVolumeCardId=diskVolumeCardId, cardNumber=cardNumber, cardSwVersion=cardSwVersion, diskVolumeTrapInh=diskVolumeTrapInh, cardNumPorts=cardNumPorts, cardSubType=cardSubType, cardDplxStatus=cardDplxStatus, equipmentShelfGroup=equipmentShelfGroup, systemGeneral=systemGeneral, portAction=portAction, auditName=auditName, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecStateChange=portSecStateChange, cardCpuType=cardCpuType, cardCarrierFwVersion=cardCarrierFwVersion, TestCommand=TestCommand, cardDplxStatusChange=cardDplxStatusChange, auditLogThrottle=auditLogThrottle, shelfObjects=shelfObjects, portTxFlowControlDetected=portTxFlowControlDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, cardAction=cardAction, portMaxDsPower=portMaxDsPower, cardSecStateChange=cardSecStateChange, auditId=auditId, cardTestTransId=cardTestTransId, cardBusSpeed=cardBusSpeed, cardType=cardType, cardEntry=cardEntry, cardBootVersion=cardBootVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfName=shelfName, auditEntry=auditEntry, portDocsIfIndex=portDocsIfIndex, cardTrapInh=cardTrapInh, cardTestEntry=cardTestEntry, portMacAddress=portMacAddress, fpgaErrorEventGroup=fpgaErrorEventGroup, portId=portId, systemClock=systemClock, auditLogOutput=auditLogOutput, portMode=portMode, cardSpareGroupId=cardSpareGroupId, cardShelfId=cardShelfId, cardTestId=cardTestId, equipmentDiagGroup=equipmentDiagGroup, diskVolumeId=diskVolumeId, trapCounter=trapCounter, cmResetInfo=cmResetInfo, cardSerialNum=cardSerialNum, TestResult=TestResult, portMinDsPower=portMinDsPower, portTxFlowControlMode=portTxFlowControlMode, equipmentCardGroup=equipmentCardGroup, cardTestName=cardTestName, cardSecState=cardSecState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditStatus=auditStatus, cardPrState=cardPrState, cardTestType=cardTestType, portDplxStatusChange=portDplxStatusChange, eqDiagConfig=eqDiagConfig, errEvLogLevel=errEvLogLevel, cardLastResetReason=cardLastResetReason, fpgaErrorEventEntry=fpgaErrorEventEntry, portDescription=portDescription, equipmentError=equipmentError, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestTable=cardTestTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portMacIfIndex=portMacIfIndex, equipmentPortGroup=equipmentPortGroup, equipmentAudit=equipmentAudit, auditTotalFailedCount=auditTotalFailedCount, trapSeverity=trapSeverity, portPrStateChange=portPrStateChange, cardTestResultDesc=cardTestResultDesc, diskVolumeNumber=diskVolumeNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTime=cardTestTime, systemGeneralGroup=systemGeneralGroup, cardCarrierHwVersion=cardCarrierHwVersion, cardPicDetected=cardPicDetected, cardFwVersion=cardFwVersion, cardHwVersion=cardHwVersion, cardPicHwVersion=cardPicHwVersion, portBgpId=portBgpId, cardAdminState=cardAdminState, diskVolumeTable=diskVolumeTable, portType=portType, errEvRowStatus=errEvRowStatus, diskVolumeSize=diskVolumeSize, cmVendor=cmVendor, cmRegistrationNotification=cmRegistrationNotification, cardId=cardId, cardUpTime=cardUpTime, cardCpuSpeed=cardCpuSpeed, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighError=cardTemperatureHighError, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portConnectorId=portConnectorId, cardTestSchedule=cardTestSchedule, cardCPLDVersion=cardCPLDVersion, cardTestScheduleCommand=cardTestScheduleCommand, cmResetNotification=cmResetNotification, cardName=cardName, downstreamPowerLoss=downstreamPowerLoss, diskVolumeShelfId=diskVolumeShelfId, cmResetClearNotification=cmResetClearNotification, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeDriveId=diskVolumeDriveId, diskVolumeEntry=diskVolumeEntry, TestId=TestId, TestScheduleCommand=TestScheduleCommand, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentDiag=equipmentDiag, portNumber=portNumber, portEntry=portEntry, portSecState=portSecState, auditAutoScheduling=auditAutoScheduling, portNumChans=portNumChans, cardTemperature=cardTemperature, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeName=diskVolumeName, systemKey=systemKey, cardTestResult=cardTestResult, auditDescription=auditDescription, TestType=TestType, cardTestDescription=cardTestDescription, diagTestId=diagTestId, cardLastChangeTime=cardLastChangeTime, cardLastBootSource=cardLastBootSource, portRxFlowControlMode=portRxFlowControlMode, diskVolumeFileName=diskVolumeFileName, cmIpAddress=cmIpAddress, cadEquipmentMibConformance=cadEquipmentMibConformance, cmResetReason=cmResetReason, cardTestResultNotification=cardTestResultNotification, portAdminState=portAdminState, auditCommand=auditCommand, diskVolumeGroup=diskVolumeGroup, shelfNumber=shelfNumber, cardCarrierSerialNum=cardCarrierSerialNum)
