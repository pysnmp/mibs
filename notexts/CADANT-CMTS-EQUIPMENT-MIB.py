...
PortId, DiskVolumeUsageLevel, ShelfId, EqActionType, PortMode, PicType, FirmwareSource, CardSubType, CardType, DuplexStatus, FlowControlMode, PrimaryState, PortDetectedMode, UnknownState, PortType, SecondaryState, AdminState, CardId = mibBuilder.importSymbols("CADANT-TC", "PortId", "DiskVolumeUsageLevel", "ShelfId", "EqActionType", "PortMode", "PicType", "FirmwareSource", "CardSubType", "CardType", "DuplexStatus", "FlowControlMode", "PrimaryState", "PortDetectedMode", "UnknownState", "PortType", "SecondaryState", "AdminState", "CardId")
...
TextualConvention, DateAndTime, TruthValue, DisplayString, MacAddress, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "TruthValue", "DisplayString", "MacAddress", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portPrState=portPrState, auditStatus=auditStatus, cmResetUpTime=cmResetUpTime, shelfNumber=shelfNumber, diagTestId=diagTestId, cmPrimarySID=cmPrimarySID, diskVolumeDriveId=diskVolumeDriveId, portConnectorId=portConnectorId, portLastChangeTime=portLastChangeTime, TestResult=TestResult, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeGroup=diskVolumeGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeTrapInh=diskVolumeTrapInh, auditFailedCount=auditFailedCount, cardPicSerialNum=cardPicSerialNum, portBgpId=portBgpId, auditCardId=auditCardId, cardTestResultNotification=cardTestResultNotification, cardTestIndex=cardTestIndex, cardLastChangeTime=cardLastChangeTime, cardNumPorts=cardNumPorts, portSecStateChange=portSecStateChange, cardTestType=cardTestType, portEntry=portEntry, portMode=portMode, portMinDsPower=portMinDsPower, cardCarrierHwVersion=cardCarrierHwVersion, portRxFlowControlMode=portRxFlowControlMode, auditCommand=auditCommand, diskDriveNumber=diskDriveNumber, portCurrDsPower=portCurrDsPower, cardTable=cardTable, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeShelfId=diskVolumeShelfId, auditTime=auditTime, equipmentDiagGroup=equipmentDiagGroup, auditTotalFailedCount=auditTotalFailedCount, cardBootVersion=cardBootVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, portDetectedMode=portDetectedMode, shelfSwVersion=shelfSwVersion, errEvLoggingEnabled=errEvLoggingEnabled, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCarrierSerialNum=cardCarrierSerialNum, systemClock=systemClock, verboseLevel=verboseLevel, portNumChans=portNumChans, portNumber=portNumber, cardLastResetReason=cardLastResetReason, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSubDetected=cardSubDetected, cardUpTime=cardUpTime, cardTempNoReportNotification=cardTempNoReportNotification, portCardId=portCardId, auditName=auditName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditCycleCount=auditCycleCount, eqDiagConfig=eqDiagConfig, cardBusSpeed=cardBusSpeed, fpgaErrorEventGroup=fpgaErrorEventGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, portPrStateChange=portPrStateChange, diskVolumeNumber=diskVolumeNumber, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentTraps=equipmentTraps, cardTestName=cardTestName, systemGeneralGroup=systemGeneralGroup, cardTestEntry=cardTestEntry, consoleOutput=consoleOutput, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSecState=cardSecState, portAdminState=portAdminState, auditTable=auditTable, cardDetectedChange=cardDetectedChange, equipmentError=equipmentError, cmResetReason=cmResetReason, portCardSubType=portCardSubType, portId=portId, equipmentShelfGroup=equipmentShelfGroup, cmResetStatus=cmResetStatus, portDescription=portDescription, portDplxStatus=portDplxStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardAdminState=cardAdminState, cardCPLDVersion=cardCPLDVersion, cmVendor=cmVendor, auditLogOutput=auditLogOutput, cardPrStateChange=cardPrStateChange, cardAction=cardAction, cardId=cardId, systemKey=systemKey, cardDplxStatus=cardDplxStatus, cardTestTable=cardTestTable, cmResetInfo=cmResetInfo, portRxFlowControlDetected=portRxFlowControlDetected, equipmentAudit=equipmentAudit, TestType=TestType, trapCounter=trapCounter, auditPassedCount=auditPassedCount, auditAutoScheduling=auditAutoScheduling, auditDescription=auditDescription, diskVolumeSize=diskVolumeSize, cardTestSchedule=cardTestSchedule, cardTestResultDesc=cardTestResultDesc, cardSpareGroupMode=cardSpareGroupMode, downstreamPowerLoss=downstreamPowerLoss, cardDetected=cardDetected, cardSecStateChange=cardSecStateChange, cardFwUpdateStatus=cardFwUpdateStatus, portAction=portAction, auditTotalPassedCount=auditTotalPassedCount, equipmentCardTestGroup=equipmentCardTestGroup, cmResetNotification=cmResetNotification, auditLogThrottle=auditLogThrottle, diskVolumeDiskSize=diskVolumeDiskSize, equipmentStateGroup=equipmentStateGroup, cmUChannelID=cmUChannelID, remainInDiagMode=remainInDiagMode, cmResetClearNotification=cmResetClearNotification, TestTransactionId=TestTransactionId, TestId=TestId, equipmentAuditGroup=equipmentAuditGroup, cardPicHwVersion=cardPicHwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, auditShelfId=auditShelfId, cardTestTime=cardTestTime, cardDplxStatusChange=cardDplxStatusChange, cmRegistrationNotification=cmRegistrationNotification, portMacIfIndex=portMacIfIndex, cardTrapInh=cardTrapInh, cardTestCommand=cardTestCommand, portType=portType, TestSchedule=TestSchedule, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSpareGroupId=cardSpareGroupId, cardPicDetected=cardPicDetected, cardTestTransId=cardTestTransId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardFpgaSource=cardFpgaSource, portTxFlowControlMode=portTxFlowControlMode, diskVolumeCardId=diskVolumeCardId, equipmentDiag=equipmentDiag, cardTestId=cardTestId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardLastBootVersion=cardLastBootVersion, portTrapInh=portTrapInh, equipmentPortGroup=equipmentPortGroup, diskVolumeId=diskVolumeId, PYSNMP_MODULE_ID=cadEquipmentMib, cmIpAddress=cmIpAddress, systemGeneral=systemGeneral, cardCpuType=cardCpuType, auditId=auditId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeName=diskVolumeName, diskVolumeTable=diskVolumeTable, cardName=cardName, cmDevice=cmDevice, portDocsIfIndex=portDocsIfIndex, cardSubType=cardSubType, cardCpuSpeed=cardCpuSpeed, cardRamSize=cardRamSize, errEventId=errEventId, equipmentTbl=equipmentTbl, errEvLogLevel=errEvLogLevel, trapSeverity=trapSeverity, errEvRowStatus=errEvRowStatus, cmMacAddress=cmMacAddress, portDplxStatusChange=portDplxStatusChange, portShelfId=portShelfId, portSecState=portSecState, cardTestResult=cardTestResult, cardEntry=cardEntry, equipmentState=equipmentState, cardFwVersion=cardFwVersion, cardSwVersion=cardSwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cardType=cardType, diskVolumeEntry=diskVolumeEntry, diskVolumeFileName=diskVolumeFileName, shelfObjects=shelfObjects, cadEquipmentMib=cadEquipmentMib, TestScheduleCommand=TestScheduleCommand, cardTestDescription=cardTestDescription, portMacAddress=portMacAddress, portTable=portTable, cardNumber=cardNumber, cardSerialNum=cardSerialNum, cardLastBootSource=cardLastBootSource, cardPrState=cardPrState, cardShelfTestIndex=cardShelfTestIndex, auditResult=auditResult, cardHwVersion=cardHwVersion, cardShelfId=cardShelfId, auditEntry=auditEntry, fpgaErrorEventTable=fpgaErrorEventTable, cardTemperature=cardTemperature, cardFlashSize=cardFlashSize, TestCommand=TestCommand, shelfName=shelfName, portMaxDsPower=portMaxDsPower)
