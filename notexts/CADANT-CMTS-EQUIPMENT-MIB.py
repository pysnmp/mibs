...
SecondaryState, UnknownState, PortType, PrimaryState, CardType, CardId, CardSubType, DuplexStatus, EqActionType, FirmwareSource, PortId, PortMode, DiskVolumeUsageLevel, PortDetectedMode, ShelfId, PicType, AdminState, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "UnknownState", "PortType", "PrimaryState", "CardType", "CardId", "CardSubType", "DuplexStatus", "EqActionType", "FirmwareSource", "PortId", "PortMode", "DiskVolumeUsageLevel", "PortDetectedMode", "ShelfId", "PicType", "AdminState", "FlowControlMode")
...
DateAndTime, TimeStamp, TruthValue, MacAddress, DisplayString, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TimeStamp", "TruthValue", "MacAddress", "DisplayString", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentPortGroup=equipmentPortGroup, cardPrState=cardPrState, TestType=TestType, cardFwVersion=cardFwVersion, cardEntry=cardEntry, portDocsIfIndex=portDocsIfIndex, cardDetectedChange=cardDetectedChange, portShelfId=portShelfId, cardTestResult=cardTestResult, errEvRowStatus=errEvRowStatus, cardTrapInh=cardTrapInh, portRxFlowControlDetected=portRxFlowControlDetected, diskDriveNumber=diskDriveNumber, equipmentDiagGroup=equipmentDiagGroup, cmRegistrationNotification=cmRegistrationNotification, auditEntry=auditEntry, TestId=TestId, cardPicSerialNum=cardPicSerialNum, equipmentTbl=equipmentTbl, cardSecStateChange=cardSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTemperatureHighError=cardTemperatureHighError, cmDevice=cmDevice, cardType=cardType, equipmentShelfGroup=equipmentShelfGroup, auditShelfId=auditShelfId, cardTestEntry=cardTestEntry, cmResetInfo=cmResetInfo, PYSNMP_MODULE_ID=cadEquipmentMib, cardNumPorts=cardNumPorts, portNumber=portNumber, auditFailedCount=auditFailedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeFileName=diskVolumeFileName, cardPrStateChange=cardPrStateChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardName=cardName, cardPicDetected=cardPicDetected, diskVolumeTable=diskVolumeTable, cardBusSpeed=cardBusSpeed, diskVolumeName=diskVolumeName, auditTotalPassedCount=auditTotalPassedCount, diskVolumeDriveId=diskVolumeDriveId, portType=portType, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestDescription=cardTestDescription, TestResult=TestResult, cmIpAddress=cmIpAddress, TestTransactionId=TestTransactionId, cardSwVersion=cardSwVersion, auditTime=auditTime, auditCycleCount=auditCycleCount, cardCarrierHwVersion=cardCarrierHwVersion, portDplxStatusChange=portDplxStatusChange, portDescription=portDescription, portMacIfIndex=portMacIfIndex, cmResetStatus=cmResetStatus, cardId=cardId, auditTable=auditTable, cardTempNoReportNotification=cardTempNoReportNotification, equipmentDiag=equipmentDiag, trapCounter=trapCounter, auditId=auditId, cardSubType=cardSubType, cmResetClearNotification=cmResetClearNotification, cardTestId=cardTestId, cmVendor=cmVendor, cardFwUpdateStatus=cardFwUpdateStatus, cardTestIndex=cardTestIndex, diskVolumeNumber=diskVolumeNumber, cardLastResetReason=cardLastResetReason, portDplxStatus=portDplxStatus, equipmentAuditGroup=equipmentAuditGroup, cardTestType=cardTestType, cadEquipmentMib=cadEquipmentMib, shelfObjects=shelfObjects, cardCpuSpeed=cardCpuSpeed, portTrapInh=portTrapInh, auditResult=auditResult, cardTestTime=cardTestTime, cardHwVersion=cardHwVersion, systemGeneral=systemGeneral, portBgpId=portBgpId, cardTemperature=cardTemperature, portSecStateChange=portSecStateChange, cardTestName=cardTestName, cardShelfId=cardShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvLoggingEnabled=errEvLoggingEnabled, shelfName=shelfName, auditAutoScheduling=auditAutoScheduling, cardSpareGroupId=cardSpareGroupId, cardTestTable=cardTestTable, portMacAddress=portMacAddress, portMinDsPower=portMinDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, fpgaErrorEventTable=fpgaErrorEventTable, TestCommand=TestCommand, auditPassedCount=auditPassedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, eqDiagConfig=eqDiagConfig, diskVolumeId=diskVolumeId, cardTestTransId=cardTestTransId, auditCardId=auditCardId, cmUChannelID=cmUChannelID, cardSecState=cardSecState, cardDplxStatus=cardDplxStatus, portRxFlowControlMode=portRxFlowControlMode, cardTestScheduleCommand=cardTestScheduleCommand, shelfNumber=shelfNumber, cardBootVersion=cardBootVersion, portTable=portTable, errEvRecoveryEnabled=errEvRecoveryEnabled, cardAdminState=cardAdminState, cardCpuType=cardCpuType, cardDplxStatusChange=cardDplxStatusChange, cardLastChangeTime=cardLastChangeTime, equipmentState=equipmentState, cardSpareGroupMode=cardSpareGroupMode, fpgaErrorEventGroup=fpgaErrorEventGroup, auditCommand=auditCommand, portAction=portAction, cardTestResultNotification=cardTestResultNotification, cardSerialNum=cardSerialNum, TestScheduleCommand=TestScheduleCommand, TestSchedule=TestSchedule, systemKey=systemKey, cardLastBootVersion=cardLastBootVersion, portLastChangeTime=portLastChangeTime, diskVolumeShelfId=diskVolumeShelfId, cardTestSchedule=cardTestSchedule, errEventId=errEventId, auditLogThrottle=auditLogThrottle, auditStatus=auditStatus, shelfSwVersion=shelfSwVersion, cardFpgaSource=cardFpgaSource, errEvLogLevel=errEvLogLevel, consoleOutput=consoleOutput, portMaxDsPower=portMaxDsPower, diskVolumeSize=diskVolumeSize, cardFlashSize=cardFlashSize, cardDetected=cardDetected, portCurrDsPower=portCurrDsPower, cmResetReason=cmResetReason, auditTotalFailedCount=auditTotalFailedCount, auditLogOutput=auditLogOutput, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResultDesc=cardTestResultDesc, portPrState=portPrState, portId=portId, portMode=portMode, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, equipmentCardGroup=equipmentCardGroup, portDetectedMode=portDetectedMode, diskVolumeEntry=diskVolumeEntry, verboseLevel=verboseLevel, cardCarrierFwVersion=cardCarrierFwVersion, cmPrimarySID=cmPrimarySID, portCardSubType=portCardSubType, portEntry=portEntry, equipmentStateGroup=equipmentStateGroup, portConnectorId=portConnectorId, equipmentCardTestGroup=equipmentCardTestGroup, portNumChans=portNumChans, cardTable=cardTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portSecState=portSecState, equipmentError=equipmentError, diskVolumeGroup=diskVolumeGroup, portTxFlowControlDetected=portTxFlowControlDetected, portAdminState=portAdminState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentTraps=equipmentTraps, auditDescription=auditDescription, cmResetUpTime=cmResetUpTime, systemClock=systemClock, cardPicHwVersion=cardPicHwVersion, cardTestCommand=cardTestCommand, cardCPLDVersion=cardCPLDVersion, portCardId=portCardId, systemGeneralGroup=systemGeneralGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentAudit=equipmentAudit, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeCardId=diskVolumeCardId, portPrStateChange=portPrStateChange, cardAction=cardAction, cardSubDetected=cardSubDetected, cmResetNotification=cmResetNotification, cardNumber=cardNumber, cardUpTime=cardUpTime, cardRamSize=cardRamSize, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardShelfTestIndex=cardShelfTestIndex, downstreamPowerLoss=downstreamPowerLoss, cmMacAddress=cmMacAddress, cardLastBootSource=cardLastBootSource, diskVolumeUsageLevel=diskVolumeUsageLevel, diagTestId=diagTestId, trapSeverity=trapSeverity, remainInDiagMode=remainInDiagMode, auditName=auditName)
