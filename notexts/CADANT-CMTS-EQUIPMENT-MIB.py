...
FlowControlMode, PortId, PortDetectedMode, PrimaryState, SecondaryState, PortMode, CardType, EqActionType, DuplexStatus, FirmwareSource, ShelfId, PortType, CardSubType, DiskVolumeUsageLevel, PicType, UnknownState, CardId, AdminState = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PortId", "PortDetectedMode", "PrimaryState", "SecondaryState", "PortMode", "CardType", "EqActionType", "DuplexStatus", "FirmwareSource", "ShelfId", "PortType", "CardSubType", "DiskVolumeUsageLevel", "PicType", "UnknownState", "CardId", "AdminState")
...
TimeStamp, TruthValue, DisplayString, RowStatus, DateAndTime, TextualConvention, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "DisplayString", "RowStatus", "DateAndTime", "TextualConvention", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFpgaSource=cardFpgaSource, diskVolumeDiskSize=diskVolumeDiskSize, systemClock=systemClock, auditLogOutput=auditLogOutput, remainInDiagMode=remainInDiagMode, equipmentStateGroup=equipmentStateGroup, TestScheduleCommand=TestScheduleCommand, portDplxStatus=portDplxStatus, portPrState=portPrState, cmMacAddress=cmMacAddress, auditAutoScheduling=auditAutoScheduling, diskVolumeEntry=diskVolumeEntry, cardLastBootSource=cardLastBootSource, portMaxDsPower=portMaxDsPower, cmUChannelID=cmUChannelID, cardNumber=cardNumber, portMode=portMode, auditEntry=auditEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestType=cardTestType, auditShelfId=auditShelfId, cardDplxStatusChange=cardDplxStatusChange, shelfNumber=shelfNumber, errEventId=errEventId, cardAction=cardAction, portSecStateChange=portSecStateChange, cardTable=cardTable, diskDriveNumber=diskDriveNumber, consoleOutput=consoleOutput, shelfSwVersion=shelfSwVersion, diskVolumeGroup=diskVolumeGroup, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeCardId=diskVolumeCardId, cardTestCommand=cardTestCommand, cardTestEntry=cardTestEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestName=cardTestName, equipmentShelfGroup=equipmentShelfGroup, cardSubType=cardSubType, portLastChangeTime=portLastChangeTime, portDetectedMode=portDetectedMode, cardTrapInh=cardTrapInh, equipmentDiag=equipmentDiag, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetNotification=cmResetNotification, cardNumPorts=cardNumPorts, errEvLogLevel=errEvLogLevel, systemGeneralGroup=systemGeneralGroup, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeId=diskVolumeId, cmResetStatus=cmResetStatus, portRxFlowControlDetected=portRxFlowControlDetected, trapCounter=trapCounter, shelfName=shelfName, cmIpAddress=cmIpAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, fpgaErrorEventTable=fpgaErrorEventTable, portType=portType, cmResetUpTime=cmResetUpTime, cardEntry=cardEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCarrierSerialNum=cardCarrierSerialNum, portTable=portTable, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTempOverHeatNotification=cardTempOverHeatNotification, portEntry=portEntry, cardSwVersion=cardSwVersion, cardFwUpdateStatus=cardFwUpdateStatus, cardPrState=cardPrState, cardPicDetected=cardPicDetected, cardLastChangeTime=cardLastChangeTime, cardShelfId=cardShelfId, eqDiagConfig=eqDiagConfig, auditPassedCount=auditPassedCount, auditName=auditName, portPrStateChange=portPrStateChange, portMacAddress=portMacAddress, TestCommand=TestCommand, cardType=cardType, portId=portId, cardTestTransId=cardTestTransId, auditTable=auditTable, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditResult=auditResult, portTxFlowControlMode=portTxFlowControlMode, PYSNMP_MODULE_ID=cadEquipmentMib, cardDetected=cardDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestResultNotification=cardTestResultNotification, trapSeverity=trapSeverity, portTrapInh=portTrapInh, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMib=cadEquipmentMib, cardId=cardId, cardFwVersion=cardFwVersion, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeFileName=diskVolumeFileName, portCardId=portCardId, cardTestIndex=cardTestIndex, diskVolumeShelfId=diskVolumeShelfId, cardTestDescription=cardTestDescription, auditId=auditId, portMinDsPower=portMinDsPower, cadEquipmentMibCompliance=cadEquipmentMibCompliance, downstreamPowerLoss=downstreamPowerLoss, cmPrimarySID=cmPrimarySID, auditStatus=auditStatus, cmResetClearNotification=cmResetClearNotification, TestId=TestId, cardSecStateChange=cardSecStateChange, diskVolumeSize=diskVolumeSize, cardDplxStatus=cardDplxStatus, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTime=cardTestTime, cardBootVersion=cardBootVersion, portRxFlowControlMode=portRxFlowControlMode, auditTotalPassedCount=auditTotalPassedCount, cardCPLDVersion=cardCPLDVersion, equipmentState=equipmentState, cardSpareGroupId=cardSpareGroupId, portConnectorId=portConnectorId, auditCardId=auditCardId, cardShelfTestIndex=cardShelfTestIndex, cardTestResultDesc=cardTestResultDesc, portBgpId=portBgpId, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetInfo=cmResetInfo, cardCpuType=cardCpuType, cmVendor=cmVendor, equipmentCardGroup=equipmentCardGroup, cardTestResult=cardTestResult, equipmentPortGroup=equipmentPortGroup, TestTransactionId=TestTransactionId, auditLogThrottle=auditLogThrottle, auditDescription=auditDescription, equipmentAudit=equipmentAudit, cardSecState=cardSecState, errEvRowStatus=errEvRowStatus, cardTemperature=cardTemperature, auditTime=auditTime, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diagTestId=diagTestId, cardSubDetected=cardSubDetected, diskVolumeDriveId=diskVolumeDriveId, cardHwVersion=cardHwVersion, cardRamSize=cardRamSize, equipmentError=equipmentError, portMacIfIndex=portMacIfIndex, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPicHwVersion=cardPicHwVersion, auditCommand=auditCommand, cmResetReason=cmResetReason, fpgaErrorEventEntry=fpgaErrorEventEntry, cardLastResetReason=cardLastResetReason, systemGeneral=systemGeneral, systemKey=systemKey, cardBusSpeed=cardBusSpeed, cardAdminState=cardAdminState, verboseLevel=verboseLevel, auditCycleCount=auditCycleCount, equipmentAuditGroup=equipmentAuditGroup, equipmentDiagGroup=equipmentDiagGroup, cardSerialNum=cardSerialNum, cardLastBootVersion=cardLastBootVersion, portAction=portAction, diskVolumeNumber=diskVolumeNumber, cardTemperatureHighError=cardTemperatureHighError, cardTestSchedule=cardTestSchedule, cardFlashSize=cardFlashSize, portCardSubType=portCardSubType, cardPrStateChange=cardPrStateChange, portCurrDsPower=portCurrDsPower, cardTestTable=cardTestTable, portShelfId=portShelfId, cardUpTime=cardUpTime, portAdminState=portAdminState, portDescription=portDescription, errEvLoggingEnabled=errEvLoggingEnabled, cmDevice=cmDevice, portNumChans=portNumChans, cardTestId=cardTestId, cardName=cardName, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeName=diskVolumeName, cardTestScheduleCommand=cardTestScheduleCommand, portSecState=portSecState, portDocsIfIndex=portDocsIfIndex, cardSpareGroupMode=cardSpareGroupMode, cardDetectedChange=cardDetectedChange, TestSchedule=TestSchedule, equipmentTraps=equipmentTraps, equipmentTbl=equipmentTbl, auditFailedCount=auditFailedCount, shelfObjects=shelfObjects, cardPicSerialNum=cardPicSerialNum, portNumber=portNumber, TestResult=TestResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeTable=diskVolumeTable, cardCpuSpeed=cardCpuSpeed, cardCarrierFwVersion=cardCarrierFwVersion, TestType=TestType)
