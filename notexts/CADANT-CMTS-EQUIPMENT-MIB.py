...
FirmwareSource, CardType, DiskVolumeUsageLevel, DuplexStatus, PortDetectedMode, PortId, PortType, ShelfId, FlowControlMode, CardSubType, PortMode, CardId, EqActionType, PicType, AdminState, SecondaryState, PrimaryState, UnknownState = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "CardType", "DiskVolumeUsageLevel", "DuplexStatus", "PortDetectedMode", "PortId", "PortType", "ShelfId", "FlowControlMode", "CardSubType", "PortMode", "CardId", "EqActionType", "PicType", "AdminState", "SecondaryState", "PrimaryState", "UnknownState")
...
RowStatus, TimeStamp, MacAddress, DateAndTime, TruthValue, DisplayString, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "MacAddress", "DateAndTime", "TruthValue", "DisplayString", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentPortGroup=equipmentPortGroup, auditTotalPassedCount=auditTotalPassedCount, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeShelfId=diskVolumeShelfId, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, verboseLevel=verboseLevel, cardName=cardName, portTxFlowControlMode=portTxFlowControlMode, cardTemperatureHighError=cardTemperatureHighError, cardTrapInh=cardTrapInh, cardSpareGroupMode=cardSpareGroupMode, TestCommand=TestCommand, portDescription=portDescription, diskDriveNumber=diskDriveNumber, portPrState=portPrState, cardDplxStatus=cardDplxStatus, cardHwVersion=cardHwVersion, cmResetNotification=cmResetNotification, auditCardId=auditCardId, cardNumber=cardNumber, cardTempNoReportNotification=cardTempNoReportNotification, cmIpAddress=cmIpAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, cardBootVersion=cardBootVersion, cardFlashSize=cardFlashSize, auditResult=auditResult, shelfName=shelfName, cardTemperatureHighWarn=cardTemperatureHighWarn, diagTestId=diagTestId, cardCPLDVersion=cardCPLDVersion, cardFwVersion=cardFwVersion, auditFailedCount=auditFailedCount, diskVolumeGroup=diskVolumeGroup, auditEntry=auditEntry, cardDetected=cardDetected, TestTransactionId=TestTransactionId, equipmentTbl=equipmentTbl, diskVolumeFileName=diskVolumeFileName, diskVolumeSize=diskVolumeSize, cardTestSchedule=cardTestSchedule, cardTestTime=cardTestTime, equipmentShelfGroup=equipmentShelfGroup, systemGeneral=systemGeneral, cmResetUpTime=cmResetUpTime, portSecStateChange=portSecStateChange, cardDetectedChange=cardDetectedChange, auditCycleCount=auditCycleCount, portNumber=portNumber, portType=portType, equipmentAudit=equipmentAudit, cmResetReason=cmResetReason, cardSerialNum=cardSerialNum, portId=portId, remainInDiagMode=remainInDiagMode, cardTestName=cardTestName, shelfSwVersion=shelfSwVersion, systemKey=systemKey, portTrapInh=portTrapInh, portShelfId=portShelfId, equipmentAuditGroup=equipmentAuditGroup, equipmentState=equipmentState, diskVolumeId=diskVolumeId, cardTestDescription=cardTestDescription, diskVolumeNumber=diskVolumeNumber, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetStatus=cmResetStatus, cmUChannelID=cmUChannelID, cardLastBootSource=cardLastBootSource, systemGeneralGroup=systemGeneralGroup, diskVolumeDriveId=diskVolumeDriveId, diskVolumeEntry=diskVolumeEntry, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cmVendor=cmVendor, cardTestCommand=cardTestCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTemperature=cardTemperature, cardFpgaSource=cardFpgaSource, cardSwVersion=cardSwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSubDetected=cardSubDetected, cardSpareGroupId=cardSpareGroupId, portBgpId=portBgpId, cardBusSpeed=cardBusSpeed, cardTestIndex=cardTestIndex, portNumChans=portNumChans, auditTable=auditTable, portDplxStatusChange=portDplxStatusChange, portAdminState=portAdminState, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageLevel=diskVolumeUsageLevel, cardCpuType=cardCpuType, cardAction=cardAction, cardNumPorts=cardNumPorts, portPrStateChange=portPrStateChange, cardLastChangeTime=cardLastChangeTime, portDetectedMode=portDetectedMode, cardTestResult=cardTestResult, cardRamSize=cardRamSize, cardPrState=cardPrState, auditPassedCount=auditPassedCount, cardTestId=cardTestId, diskVolumeName=diskVolumeName, portAction=portAction, trapCounter=trapCounter, cardSubType=cardSubType, cardPicDetected=cardPicDetected, cardTestTransId=cardTestTransId, cmDevice=cmDevice, auditTime=auditTime, equipmentDiag=equipmentDiag, auditLogThrottle=auditLogThrottle, portTable=portTable, auditId=auditId, cardShelfId=cardShelfId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentStateGroup=equipmentStateGroup, equipmentDiagGroup=equipmentDiagGroup, cardLastBootVersion=cardLastBootVersion, portRxFlowControlDetected=portRxFlowControlDetected, cmResetInfo=cmResetInfo, cardDplxStatusChange=cardDplxStatusChange, cardPicHwVersion=cardPicHwVersion, systemClock=systemClock, cardCarrierHwVersion=cardCarrierHwVersion, errEvRowStatus=errEvRowStatus, diskVolumeCardId=diskVolumeCardId, equipmentError=equipmentError, cardTestEntry=cardTestEntry, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestScheduleCommand=TestScheduleCommand, TestId=TestId, TestResult=TestResult, diskVolumeTable=diskVolumeTable, trapSeverity=trapSeverity, PYSNMP_MODULE_ID=cadEquipmentMib, cardSecState=cardSecState, portCardSubType=portCardSubType, portEntry=portEntry, portMode=portMode, eqDiagConfig=eqDiagConfig, portCardId=portCardId, cmPrimarySID=cmPrimarySID, consoleOutput=consoleOutput, cmResetClearNotification=cmResetClearNotification, cmRegistrationNotification=cmRegistrationNotification, auditName=auditName, equipmentTraps=equipmentTraps, cardLastResetReason=cardLastResetReason, cardEntry=cardEntry, cardFwUpdateStatus=cardFwUpdateStatus, auditCommand=auditCommand, auditShelfId=auditShelfId, fpgaErrorEventTable=fpgaErrorEventTable, errEvLogLevel=errEvLogLevel, cardTestScheduleCommand=cardTestScheduleCommand, portSecState=portSecState, auditDescription=auditDescription, portMaxDsPower=portMaxDsPower, portRxFlowControlMode=portRxFlowControlMode, portTxFlowControlDetected=portTxFlowControlDetected, auditLogOutput=auditLogOutput, portCurrDsPower=portCurrDsPower, TestSchedule=TestSchedule, portLastChangeTime=portLastChangeTime, portMinDsPower=portMinDsPower, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierFwVersion=cardCarrierFwVersion, portDocsIfIndex=portDocsIfIndex, cardCarrierSerialNum=cardCarrierSerialNum, cardTable=cardTable, portConnectorId=portConnectorId, cmMacAddress=cmMacAddress, portDplxStatus=portDplxStatus, cardType=cardType, cardAdminState=cardAdminState, fpgaErrorEventEntry=fpgaErrorEventEntry, portMacAddress=portMacAddress, portMacIfIndex=portMacIfIndex, cardTestType=cardTestType, cardUpTime=cardUpTime, cardTestResultDesc=cardTestResultDesc, shelfNumber=shelfNumber, cardTestResultNotification=cardTestResultNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeTrapInh=diskVolumeTrapInh, cardCpuSpeed=cardCpuSpeed, TestType=TestType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardId=cardId, cardSecStateChange=cardSecStateChange, errEventId=errEventId, cardPicSerialNum=cardPicSerialNum, cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventGroup=fpgaErrorEventGroup, auditStatus=auditStatus, shelfObjects=shelfObjects, cardTestTable=cardTestTable, equipmentCardGroup=equipmentCardGroup, cardPrStateChange=cardPrStateChange, downstreamPowerLoss=downstreamPowerLoss)
