...
FlowControlMode, PicType, PortType, UnknownState, SecondaryState, FirmwareSource, CardId, PortId, CardType, CardSubType, EqActionType, PrimaryState, DuplexStatus, ShelfId, PortDetectedMode, DiskVolumeUsageLevel, PortMode, AdminState = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PicType", "PortType", "UnknownState", "SecondaryState", "FirmwareSource", "CardId", "PortId", "CardType", "CardSubType", "EqActionType", "PrimaryState", "DuplexStatus", "ShelfId", "PortDetectedMode", "DiskVolumeUsageLevel", "PortMode", "AdminState")
...
RowStatus, TimeStamp, DisplayString, DateAndTime, TextualConvention, MacAddress, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "DisplayString", "DateAndTime", "TextualConvention", "MacAddress", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portId=portId, TestCommand=TestCommand, portCardSubType=portCardSubType, errEvRecoveryEnabled=errEvRecoveryEnabled, cardDplxStatusChange=cardDplxStatusChange, cardFwVersion=cardFwVersion, downstreamPowerLoss=downstreamPowerLoss, equipmentTbl=equipmentTbl, shelfSwVersion=shelfSwVersion, cardEntry=cardEntry, cardCpuSpeed=cardCpuSpeed, cardShelfTestIndex=cardShelfTestIndex, cardUpTime=cardUpTime, equipmentAuditGroup=equipmentAuditGroup, diskVolumeId=diskVolumeId, shelfObjects=shelfObjects, portTrapInh=portTrapInh, cardSecStateChange=cardSecStateChange, cardPicHwVersion=cardPicHwVersion, cardPrStateChange=cardPrStateChange, portDplxStatusChange=portDplxStatusChange, cardCPLDVersion=cardCPLDVersion, portPrStateChange=portPrStateChange, cardFpgaSource=cardFpgaSource, diskVolumeDriveId=diskVolumeDriveId, equipmentError=equipmentError, cardSpareGroupMode=cardSpareGroupMode, auditCardId=auditCardId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, verboseLevel=verboseLevel, auditStatus=auditStatus, portTxFlowControlDetected=portTxFlowControlDetected, cardAction=cardAction, portMaxDsPower=portMaxDsPower, diskVolumeSize=diskVolumeSize, trapSeverity=trapSeverity, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeCardId=diskVolumeCardId, cardHwVersion=cardHwVersion, portDescription=portDescription, cardTestResult=cardTestResult, equipmentAudit=equipmentAudit, cardDetectedChange=cardDetectedChange, auditTable=auditTable, auditCycleCount=auditCycleCount, consoleOutput=consoleOutput, cardAdminState=cardAdminState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardSubDetected=cardSubDetected, shelfNumber=shelfNumber, equipmentState=equipmentState, TestScheduleCommand=TestScheduleCommand, portSecState=portSecState, diskVolumeUsageLevel=diskVolumeUsageLevel, auditPassedCount=auditPassedCount, cardTestScheduleCommand=cardTestScheduleCommand, portTable=portTable, cardTempOverHeatNotification=cardTempOverHeatNotification, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibConformance=cadEquipmentMibConformance, cmVendor=cmVendor, portTxFlowControlMode=portTxFlowControlMode, cardSerialNum=cardSerialNum, cardTestId=cardTestId, portDplxStatus=portDplxStatus, portMinDsPower=portMinDsPower, cardTestResultNotification=cardTestResultNotification, diskVolumeNumber=diskVolumeNumber, auditId=auditId, equipmentDiagGroup=equipmentDiagGroup, portCurrDsPower=portCurrDsPower, diskVolumeShelfId=diskVolumeShelfId, cardTrapInh=cardTrapInh, auditEntry=auditEntry, errEvLogLevel=errEvLogLevel, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portConnectorId=portConnectorId, diagTestId=diagTestId, equipmentTraps=equipmentTraps, systemGeneral=systemGeneral, diskVolumeName=diskVolumeName, portCardId=portCardId, auditLogThrottle=auditLogThrottle, systemKey=systemKey, cardLastBootVersion=cardLastBootVersion, cardTestEntry=cardTestEntry, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditResult=auditResult, cardName=cardName, portNumber=portNumber, portRxFlowControlDetected=portRxFlowControlDetected, cardCarrierHwVersion=cardCarrierHwVersion, cardDplxStatus=cardDplxStatus, PYSNMP_MODULE_ID=cadEquipmentMib, cardSwVersion=cardSwVersion, errEvLoggingEnabled=errEvLoggingEnabled, portDetectedMode=portDetectedMode, cardSubType=cardSubType, cmResetInfo=cmResetInfo, portNumChans=portNumChans, portDocsIfIndex=portDocsIfIndex, equipmentPortGroup=equipmentPortGroup, TestType=TestType, portLastChangeTime=portLastChangeTime, cmUChannelID=cmUChannelID, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetNotification=cmResetNotification, TestSchedule=TestSchedule, portBgpId=portBgpId, cardTestTransId=cardTestTransId, cardPicDetected=cardPicDetected, errEvRowStatus=errEvRowStatus, cardFlashSize=cardFlashSize, cardCpuType=cardCpuType, eqDiagConfig=eqDiagConfig, cmResetClearNotification=cmResetClearNotification, cardSecState=cardSecState, remainInDiagMode=remainInDiagMode, cmResetReason=cmResetReason, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentDiag=equipmentDiag, cardBootVersion=cardBootVersion, trapCounter=trapCounter, systemClock=systemClock, auditName=auditName, cadEquipmentMib=cadEquipmentMib, diskVolumeTrapInh=diskVolumeTrapInh, auditDescription=auditDescription, auditTotalFailedCount=auditTotalFailedCount, cmMacAddress=cmMacAddress, diskDriveNumber=diskDriveNumber, cardDetected=cardDetected, cardType=cardType, cardId=cardId, cardTestSchedule=cardTestSchedule, equipmentCardGroup=equipmentCardGroup, portType=portType, cardTestResultDesc=cardTestResultDesc, auditTotalPassedCount=auditTotalPassedCount, portRxFlowControlMode=portRxFlowControlMode, auditTime=auditTime, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, cmPrimarySID=cmPrimarySID, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTable=cardTable, fpgaErrorEventEntry=fpgaErrorEventEntry, portAdminState=portAdminState, cardLastChangeTime=cardLastChangeTime, TestTransactionId=TestTransactionId, cardLastResetReason=cardLastResetReason, cardSpareGroupId=cardSpareGroupId, cardTestName=cardTestName, cardTestType=cardTestType, cardShelfId=cardShelfId, systemGeneralGroup=systemGeneralGroup, cardPicSerialNum=cardPicSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeGroup=diskVolumeGroup, cardTestTime=cardTestTime, diskVolumeFileName=diskVolumeFileName, cardPrState=cardPrState, TestResult=TestResult, cmDevice=cmDevice, cmRegistrationNotification=cmRegistrationNotification, auditLogOutput=auditLogOutput, fpgaErrorEventTable=fpgaErrorEventTable, cardTestIndex=cardTestIndex, cmResetUpTime=cmResetUpTime, portEntry=portEntry, diskVolumeTable=diskVolumeTable, portShelfId=portShelfId, equipmentShelfGroup=equipmentShelfGroup, cmResetStatus=cmResetStatus, auditFailedCount=auditFailedCount, diskVolumeEntry=diskVolumeEntry, cardTempNoReportNotification=cardTempNoReportNotification, errEventId=errEventId, portAction=portAction, cardTestDescription=cardTestDescription, TestId=TestId, auditCommand=auditCommand, cardBusSpeed=cardBusSpeed, diskVolumeDiskSize=diskVolumeDiskSize, cardTestCommand=cardTestCommand, cardNumPorts=cardNumPorts, cardCarrierFwVersion=cardCarrierFwVersion, portMode=portMode, cardCarrierSerialNum=cardCarrierSerialNum, portSecStateChange=portSecStateChange, equipmentCardTestGroup=equipmentCardTestGroup, portMacIfIndex=portMacIfIndex, cardRamSize=cardRamSize, cardTemperatureHighError=cardTemperatureHighError, cardTemperature=cardTemperature, equipmentStateGroup=equipmentStateGroup, shelfName=shelfName, portMacAddress=portMacAddress, portPrState=portPrState, cardNumber=cardNumber, cardTestTable=cardTestTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardLastBootSource=cardLastBootSource, cmIpAddress=cmIpAddress, cadEquipmentMibGroup=cadEquipmentMibGroup, auditShelfId=auditShelfId)
