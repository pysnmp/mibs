...
PortMode, CardId, SecondaryState, ShelfId, PortId, DuplexStatus, FirmwareSource, FlowControlMode, PicType, PortDetectedMode, AdminState, CardType, CardSubType, DiskVolumeUsageLevel, PortType, UnknownState, PrimaryState, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PortMode", "CardId", "SecondaryState", "ShelfId", "PortId", "DuplexStatus", "FirmwareSource", "FlowControlMode", "PicType", "PortDetectedMode", "AdminState", "CardType", "CardSubType", "DiskVolumeUsageLevel", "PortType", "UnknownState", "PrimaryState", "EqActionType")
...
TimeStamp, TruthValue, MacAddress, RowStatus, TextualConvention, DisplayString, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "MacAddress", "RowStatus", "TextualConvention", "DisplayString", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFwUpdateStatus=cardFwUpdateStatus, cardTestId=cardTestId, cardBusSpeed=cardBusSpeed, portDplxStatusChange=portDplxStatusChange, cardCpuSpeed=cardCpuSpeed, portTable=portTable, portTxFlowControlMode=portTxFlowControlMode, auditDescription=auditDescription, portNumChans=portNumChans, cardTestResultNotification=cardTestResultNotification, cmRegistrationNotification=cmRegistrationNotification, auditShelfId=auditShelfId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestType=cardTestType, cardTemperatureHighError=cardTemperatureHighError, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentDiag=equipmentDiag, cardTestScheduleCommand=cardTestScheduleCommand, cardTempNoReportNotification=cardTempNoReportNotification, portNumber=portNumber, shelfObjects=shelfObjects, cardPicHwVersion=cardPicHwVersion, cardSerialNum=cardSerialNum, cardSecStateChange=cardSecStateChange, portConnectorId=portConnectorId, cardSubType=cardSubType, portMaxDsPower=portMaxDsPower, cardSecState=cardSecState, diskDriveNumber=diskDriveNumber, cardTestIndex=cardTestIndex, TestTransactionId=TestTransactionId, portLastChangeTime=portLastChangeTime, trapCounter=trapCounter, cardUpTime=cardUpTime, cardShelfTestIndex=cardShelfTestIndex, auditLogThrottle=auditLogThrottle, diskVolumeEntry=diskVolumeEntry, diskVolumeName=diskVolumeName, errEventId=errEventId, auditCommand=auditCommand, cardBootVersion=cardBootVersion, portMinDsPower=portMinDsPower, cardType=cardType, cmResetReason=cmResetReason, errEvRowStatus=errEvRowStatus, cardCarrierFwVersion=cardCarrierFwVersion, cardCpuType=cardCpuType, portDescription=portDescription, auditTotalFailedCount=auditTotalFailedCount, fpgaErrorEventEntry=fpgaErrorEventEntry, auditCycleCount=auditCycleCount, cardShelfId=cardShelfId, diskVolumeFileName=diskVolumeFileName, fpgaErrorEventGroup=fpgaErrorEventGroup, portShelfId=portShelfId, shelfNumber=shelfNumber, cardTestDescription=cardTestDescription, auditAutoScheduling=auditAutoScheduling, consoleOutput=consoleOutput, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeCardId=diskVolumeCardId, equipmentTraps=equipmentTraps, auditCardId=auditCardId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestCommand=cardTestCommand, cardTemperature=cardTemperature, cardLastBootVersion=cardLastBootVersion, remainInDiagMode=remainInDiagMode, portAdminState=portAdminState, errEvRecoveryEnabled=errEvRecoveryEnabled, systemKey=systemKey, portPrState=portPrState, cardTestResult=cardTestResult, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestEntry=cardTestEntry, systemGeneral=systemGeneral, TestId=TestId, cardName=cardName, diskVolumeId=diskVolumeId, cardSwVersion=cardSwVersion, portEntry=portEntry, cardPicSerialNum=cardPicSerialNum, auditFailedCount=auditFailedCount, auditEntry=auditEntry, cmResetStatus=cmResetStatus, cardLastResetReason=cardLastResetReason, diskVolumeSize=diskVolumeSize, cmResetUpTime=cmResetUpTime, cardRamSize=cardRamSize, portPrStateChange=portPrStateChange, TestType=TestType, portId=portId, equipmentError=equipmentError, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSpareGroupId=cardSpareGroupId, cardTestTime=cardTestTime, TestResult=TestResult, portDplxStatus=portDplxStatus, shelfSwVersion=shelfSwVersion, eqDiagConfig=eqDiagConfig, equipmentDiagGroup=equipmentDiagGroup, portDetectedMode=portDetectedMode, cmResetInfo=cmResetInfo, portRxFlowControlMode=portRxFlowControlMode, portMode=portMode, cardCPLDVersion=cardCPLDVersion, cardTestName=cardTestName, auditPassedCount=auditPassedCount, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeTable=diskVolumeTable, cmUChannelID=cmUChannelID, cmIpAddress=cmIpAddress, downstreamPowerLoss=downstreamPowerLoss, equipmentShelfGroup=equipmentShelfGroup, cmResetClearNotification=cmResetClearNotification, auditTable=auditTable, equipmentState=equipmentState, cardLastChangeTime=cardLastChangeTime, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeShelfId=diskVolumeShelfId, cardNumPorts=cardNumPorts, cardNumber=cardNumber, portSecState=portSecState, diskVolumeDriveId=diskVolumeDriveId, equipmentStateGroup=equipmentStateGroup, errEvLoggingEnabled=errEvLoggingEnabled, portTxFlowControlDetected=portTxFlowControlDetected, portBgpId=portBgpId, shelfName=shelfName, auditTotalPassedCount=auditTotalPassedCount, portAction=portAction, cmVendor=cmVendor, cmResetNotification=cmResetNotification, cardPicDetected=cardPicDetected, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeTrapInh=diskVolumeTrapInh, portCurrDsPower=portCurrDsPower, equipmentPortGroup=equipmentPortGroup, portCardId=portCardId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSpareGroupMode=cardSpareGroupMode, auditId=auditId, equipmentAudit=equipmentAudit, cardTemperatureHighWarn=cardTemperatureHighWarn, portTrapInh=portTrapInh, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTransId=cardTestTransId, diskVolumeGroup=diskVolumeGroup, equipmentTbl=equipmentTbl, portDocsIfIndex=portDocsIfIndex, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portMacIfIndex=portMacIfIndex, trapSeverity=trapSeverity, systemClock=systemClock, cadEquipmentMib=cadEquipmentMib, diskVolumeDiskSize=diskVolumeDiskSize, equipmentCardGroup=equipmentCardGroup, cardTrapInh=cardTrapInh, cardAdminState=cardAdminState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTempOverHeatNotification=cardTempOverHeatNotification, portSecStateChange=portSecStateChange, cardSubDetected=cardSubDetected, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDplxStatusChange=cardDplxStatusChange, cardAction=cardAction, cardTestResultDesc=cardTestResultDesc, auditTime=auditTime, cardTestTable=cardTestTable, errEvLogLevel=errEvLogLevel, auditResult=auditResult, cardPrStateChange=cardPrStateChange, cardFwVersion=cardFwVersion, cardDetectedChange=cardDetectedChange, cardPrState=cardPrState, cmDevice=cmDevice, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTable=cardTable, auditStatus=auditStatus, cardFlashSize=cardFlashSize, cardFpgaSource=cardFpgaSource, portType=portType, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestCommand=TestCommand, cadEquipmentMibConformance=cadEquipmentMibConformance, cardEntry=cardEntry, TestScheduleCommand=TestScheduleCommand, portMacAddress=portMacAddress, auditLogOutput=auditLogOutput, cardLastBootSource=cardLastBootSource, cmPrimarySID=cmPrimarySID, diskVolumeNumber=diskVolumeNumber, cardDetected=cardDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentAuditGroup=equipmentAuditGroup, cmMacAddress=cmMacAddress, diagTestId=diagTestId, cardId=cardId, auditName=auditName, verboseLevel=verboseLevel, TestSchedule=TestSchedule, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardHwVersion=cardHwVersion, portCardSubType=portCardSubType, cardDplxStatus=cardDplxStatus, cardTestSchedule=cardTestSchedule)
