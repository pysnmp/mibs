...
DiskVolumeUsageLevel, FirmwareSource, CardSubType, EqActionType, PortType, PortMode, CardId, ShelfId, UnknownState, PicType, FlowControlMode, PortId, PrimaryState, AdminState, PortDetectedMode, DuplexStatus, SecondaryState, CardType = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "FirmwareSource", "CardSubType", "EqActionType", "PortType", "PortMode", "CardId", "ShelfId", "UnknownState", "PicType", "FlowControlMode", "PortId", "PrimaryState", "AdminState", "PortDetectedMode", "DuplexStatus", "SecondaryState", "CardType")
...
RowStatus, TruthValue, TextualConvention, TimeStamp, DateAndTime, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TruthValue", "TextualConvention", "TimeStamp", "DateAndTime", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResultDesc=cardTestResultDesc, cardFlashSize=cardFlashSize, cardTestType=cardTestType, cardAction=cardAction, cardCPLDVersion=cardCPLDVersion, portNumber=portNumber, cardTestEntry=cardTestEntry, cardTestScheduleCommand=cardTestScheduleCommand, cardSpareGroupMode=cardSpareGroupMode, downstreamPowerLoss=downstreamPowerLoss, cmVendor=cmVendor, equipmentState=equipmentState, cardUpTime=cardUpTime, errEvLogLevel=errEvLogLevel, cardPicSerialNum=cardPicSerialNum, errEventId=errEventId, portTable=portTable, remainInDiagMode=remainInDiagMode, auditLogOutput=auditLogOutput, cardTable=cardTable, cardDetected=cardDetected, cardTestTime=cardTestTime, auditTime=auditTime, diskDriveNumber=diskDriveNumber, cardBootVersion=cardBootVersion, diskVolumeFileName=diskVolumeFileName, cardSecStateChange=cardSecStateChange, cmResetReason=cmResetReason, auditEntry=auditEntry, cardTestName=cardTestName, equipmentError=equipmentError, cardTemperatureHighWarn=cardTemperatureHighWarn, cardShelfId=cardShelfId, equipmentAudit=equipmentAudit, portRxFlowControlMode=portRxFlowControlMode, cmUChannelID=cmUChannelID, diskVolumeSize=diskVolumeSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, shelfSwVersion=shelfSwVersion, portDescription=portDescription, equipmentCardGroup=equipmentCardGroup, cardTestResult=cardTestResult, auditCardId=auditCardId, cardTempNoReportNotification=cardTempNoReportNotification, cardNumPorts=cardNumPorts, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditTotalPassedCount=auditTotalPassedCount, cmIpAddress=cmIpAddress, portCurrDsPower=portCurrDsPower, TestTransactionId=TestTransactionId, portPrState=portPrState, auditStatus=auditStatus, diskVolumeNumber=diskVolumeNumber, cardPicHwVersion=cardPicHwVersion, diagTestId=diagTestId, equipmentShelfGroup=equipmentShelfGroup, equipmentCardTestGroup=equipmentCardTestGroup, cardTestTransId=cardTestTransId, auditAutoScheduling=auditAutoScheduling, portTrapInh=portTrapInh, portMaxDsPower=portMaxDsPower, cardTrapInh=cardTrapInh, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeId=diskVolumeId, cardSerialNum=cardSerialNum, TestId=TestId, systemKey=systemKey, portCardSubType=portCardSubType, cardTestCommand=cardTestCommand, portAdminState=portAdminState, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestType=TestType, diskVolumeUsageLevel=diskVolumeUsageLevel, cardShelfTestIndex=cardShelfTestIndex, diskVolumeEntry=diskVolumeEntry, systemClock=systemClock, cardTestId=cardTestId, equipmentStateGroup=equipmentStateGroup, shelfObjects=shelfObjects, trapSeverity=trapSeverity, cardPrStateChange=cardPrStateChange, cardRamSize=cardRamSize, cardBusSpeed=cardBusSpeed, cadEquipmentMibGroup=cadEquipmentMibGroup, cardName=cardName, cardDplxStatusChange=cardDplxStatusChange, cardSubDetected=cardSubDetected, equipmentDiag=equipmentDiag, cardCarrierFwVersion=cardCarrierFwVersion, portShelfId=portShelfId, portPrStateChange=portPrStateChange, diskVolumeGroup=diskVolumeGroup, cardId=cardId, cardEntry=cardEntry, portDocsIfIndex=portDocsIfIndex, auditDescription=auditDescription, cardTestResultNotification=cardTestResultNotification, portEntry=portEntry, portAction=portAction, cmPrimarySID=cmPrimarySID, cardCarrierSerialNum=cardCarrierSerialNum, cmDevice=cmDevice, portSecState=portSecState, cardSubType=cardSubType, cardLastBootSource=cardLastBootSource, portType=portType, TestCommand=TestCommand, eqDiagConfig=eqDiagConfig, cmResetUpTime=cmResetUpTime, auditTable=auditTable, portDplxStatusChange=portDplxStatusChange, auditFailedCount=auditFailedCount, cardFwUpdateStatus=cardFwUpdateStatus, portCardId=portCardId, trapCounter=trapCounter, cardDplxStatus=cardDplxStatus, cmResetStatus=cmResetStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, cardLastChangeTime=cardLastChangeTime, auditShelfId=auditShelfId, cardFpgaSource=cardFpgaSource, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTable=diskVolumeTable, portConnectorId=portConnectorId, cardCpuType=cardCpuType, diskVolumeShelfId=diskVolumeShelfId, cardDetectedChange=cardDetectedChange, cardSpareGroupId=cardSpareGroupId, cardTestDescription=cardTestDescription, diskVolumeTrapInh=diskVolumeTrapInh, portRxFlowControlDetected=portRxFlowControlDetected, equipmentTbl=equipmentTbl, auditLogThrottle=auditLogThrottle, diskVolumeDriveId=diskVolumeDriveId, portNumChans=portNumChans, portId=portId, systemGeneralGroup=systemGeneralGroup, auditResult=auditResult, cardType=cardType, cmRegistrationNotification=cmRegistrationNotification, verboseLevel=verboseLevel, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portSecStateChange=portSecStateChange, cardHwVersion=cardHwVersion, diskVolumeDiskSize=diskVolumeDiskSize, cardFwVersion=cardFwVersion, portMinDsPower=portMinDsPower, equipmentTraps=equipmentTraps, portDetectedMode=portDetectedMode, cardPicDetected=cardPicDetected, cardLastResetReason=cardLastResetReason, diskVolumeName=diskVolumeName, cardLastBootVersion=cardLastBootVersion, shelfNumber=shelfNumber, cardNumber=cardNumber, auditCommand=auditCommand, portDplxStatus=portDplxStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestSchedule=cardTestSchedule, auditTotalFailedCount=auditTotalFailedCount, cardSwVersion=cardSwVersion, cardPrState=cardPrState, cmResetNotification=cmResetNotification, auditId=auditId, portLastChangeTime=portLastChangeTime, TestResult=TestResult, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditName=auditName, cardTemperatureHighError=cardTemperatureHighError, PYSNMP_MODULE_ID=cadEquipmentMib, cardAdminState=cardAdminState, fpgaErrorEventTable=fpgaErrorEventTable, errEvLoggingEnabled=errEvLoggingEnabled, equipmentPortGroup=equipmentPortGroup, systemGeneral=systemGeneral, cardTestIndex=cardTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeCardId=diskVolumeCardId, cardCpuSpeed=cardCpuSpeed, portMacAddress=portMacAddress, cardTemperature=cardTemperature, consoleOutput=consoleOutput, auditCycleCount=auditCycleCount, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvRowStatus=errEvRowStatus, portBgpId=portBgpId, portMode=portMode, cardSecState=cardSecState, TestScheduleCommand=TestScheduleCommand, auditPassedCount=auditPassedCount, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetInfo=cmResetInfo, portTxFlowControlDetected=portTxFlowControlDetected, equipmentDiagGroup=equipmentDiagGroup, cardTestTable=cardTestTable, portMacIfIndex=portMacIfIndex, cmResetClearNotification=cmResetClearNotification, cmMacAddress=cmMacAddress, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMib=cadEquipmentMib, TestSchedule=TestSchedule, shelfName=shelfName)
