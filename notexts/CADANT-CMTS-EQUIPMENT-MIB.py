...
PortMode, CardId, PicType, FirmwareSource, ShelfId, DiskVolumeUsageLevel, CardType, SecondaryState, UnknownState, DuplexStatus, PortType, FlowControlMode, AdminState, CardSubType, PortId, PortDetectedMode, PrimaryState, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PortMode", "CardId", "PicType", "FirmwareSource", "ShelfId", "DiskVolumeUsageLevel", "CardType", "SecondaryState", "UnknownState", "DuplexStatus", "PortType", "FlowControlMode", "AdminState", "CardSubType", "PortId", "PortDetectedMode", "PrimaryState", "EqActionType")
...
DisplayString, TruthValue, RowStatus, TimeStamp, MacAddress, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TruthValue", "RowStatus", "TimeStamp", "MacAddress", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cadEquipmentMib=cadEquipmentMib, portDplxStatus=portDplxStatus, diskVolumeName=diskVolumeName, auditPassedCount=auditPassedCount, TestScheduleCommand=TestScheduleCommand, auditAutoScheduling=auditAutoScheduling, cardLastBootVersion=cardLastBootVersion, remainInDiagMode=remainInDiagMode, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, fpgaErrorEventTable=fpgaErrorEventTable, errEventId=errEventId, TestCommand=TestCommand, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portBgpId=portBgpId, shelfNumber=shelfNumber, diskVolumeEntry=diskVolumeEntry, cardSpareGroupId=cardSpareGroupId, portTxFlowControlDetected=portTxFlowControlDetected, auditLogThrottle=auditLogThrottle, auditTotalPassedCount=auditTotalPassedCount, cardTestCommand=cardTestCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestType=TestType, TestSchedule=TestSchedule, cardPicHwVersion=cardPicHwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeDiskSize=diskVolumeDiskSize, portDetectedMode=portDetectedMode, cardPrState=cardPrState, cardTestId=cardTestId, auditCommand=auditCommand, cmResetUpTime=cmResetUpTime, errEvRowStatus=errEvRowStatus, cardAction=cardAction, diskDriveNumber=diskDriveNumber, diagTestId=diagTestId, portMacIfIndex=portMacIfIndex, auditFailedCount=auditFailedCount, cardUpTime=cardUpTime, cardDplxStatusChange=cardDplxStatusChange, cardTestTime=cardTestTime, errEvRecoveryEnabled=errEvRecoveryEnabled, portTable=portTable, cardTrapInh=cardTrapInh, auditName=auditName, cardTable=cardTable, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestIndex=cardTestIndex, verboseLevel=verboseLevel, cmVendor=cmVendor, trapSeverity=trapSeverity, auditCardId=auditCardId, portDescription=portDescription, cardCarrierHwVersion=cardCarrierHwVersion, auditCycleCount=auditCycleCount, portTrapInh=portTrapInh, cardName=cardName, cardTestEntry=cardTestEntry, cmPrimarySID=cmPrimarySID, portMaxDsPower=portMaxDsPower, cardSerialNum=cardSerialNum, cardTestTransId=cardTestTransId, portDplxStatusChange=portDplxStatusChange, cardDetected=cardDetected, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardId=cardId, cardCPLDVersion=cardCPLDVersion, diskVolumeId=diskVolumeId, cardTestType=cardTestType, diskVolumeNumber=diskVolumeNumber, portCardId=portCardId, equipmentError=equipmentError, TestResult=TestResult, equipmentTbl=equipmentTbl, equipmentTraps=equipmentTraps, auditLogOutput=auditLogOutput, portMinDsPower=portMinDsPower, cmResetClearNotification=cmResetClearNotification, cardShelfId=cardShelfId, equipmentShelfGroup=equipmentShelfGroup, cardTestResult=cardTestResult, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardCarrierFwVersion=cardCarrierFwVersion, cardPrStateChange=cardPrStateChange, cardLastBootSource=cardLastBootSource, cardFlashSize=cardFlashSize, shelfObjects=shelfObjects, equipmentCardGroup=equipmentCardGroup, portEntry=portEntry, TestTransactionId=TestTransactionId, auditEntry=auditEntry, equipmentPortGroup=equipmentPortGroup, equipmentState=equipmentState, diskVolumeDriveId=diskVolumeDriveId, equipmentAuditGroup=equipmentAuditGroup, auditTable=auditTable, cardShelfTestIndex=cardShelfTestIndex, auditResult=auditResult, errEvLogLevel=errEvLogLevel, cardSpareGroupMode=cardSpareGroupMode, cardNumber=cardNumber, cardBusSpeed=cardBusSpeed, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeSize=diskVolumeSize, portShelfId=portShelfId, cmResetInfo=cmResetInfo, cardTestScheduleCommand=cardTestScheduleCommand, portRxFlowControlDetected=portRxFlowControlDetected, auditShelfId=auditShelfId, portId=portId, systemGeneral=systemGeneral, cardEntry=cardEntry, shelfName=shelfName, portNumChans=portNumChans, portMacAddress=portMacAddress, cardTestResultDesc=cardTestResultDesc, portNumber=portNumber, portCurrDsPower=portCurrDsPower, cmRegistrationNotification=cmRegistrationNotification, cardDetectedChange=cardDetectedChange, consoleOutput=consoleOutput, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetNotification=cmResetNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCarrierSerialNum=cardCarrierSerialNum, portLastChangeTime=portLastChangeTime, cardSwVersion=cardSwVersion, trapCounter=trapCounter, auditTime=auditTime, equipmentAudit=equipmentAudit, portConnectorId=portConnectorId, portCardSubType=portCardSubType, cmMacAddress=cmMacAddress, cardSubType=cardSubType, portSecState=portSecState, cardPicDetected=cardPicDetected, cardHwVersion=cardHwVersion, cardCpuSpeed=cardCpuSpeed, cardFwUpdateStatus=cardFwUpdateStatus, cardRamSize=cardRamSize, diskVolumeTable=diskVolumeTable, cardTestSchedule=cardTestSchedule, portSecStateChange=portSecStateChange, errEvLoggingEnabled=errEvLoggingEnabled, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeFileName=diskVolumeFileName, cardTestName=cardTestName, auditId=auditId, downstreamPowerLoss=downstreamPowerLoss, portDocsIfIndex=portDocsIfIndex, cmResetReason=cmResetReason, diskVolumeGroup=diskVolumeGroup, portType=portType, cardDplxStatus=cardDplxStatus, diskVolumeShelfId=diskVolumeShelfId, cmIpAddress=cmIpAddress, cardTestResultNotification=cardTestResultNotification, cardTestTable=cardTestTable, portRxFlowControlMode=portRxFlowControlMode, cardCpuType=cardCpuType, diskVolumeTrapInh=diskVolumeTrapInh, cardFpgaSource=cardFpgaSource, cardLastChangeTime=cardLastChangeTime, cmResetStatus=cmResetStatus, auditStatus=auditStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, portMode=portMode, portAction=portAction, cardNumPorts=cardNumPorts, cardAdminState=cardAdminState, cardSubDetected=cardSubDetected, portPrState=portPrState, cardSecState=cardSecState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, PYSNMP_MODULE_ID=cadEquipmentMib, cmDevice=cmDevice, portAdminState=portAdminState, equipmentDiagGroup=equipmentDiagGroup, systemClock=systemClock, portPrStateChange=portPrStateChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTemperatureHighError=cardTemperatureHighError, cardSecStateChange=cardSecStateChange, cardTemperature=cardTemperature, cardBootVersion=cardBootVersion, shelfSwVersion=shelfSwVersion, cardType=cardType, cardLastResetReason=cardLastResetReason, diskVolumeCardId=diskVolumeCardId, eqDiagConfig=eqDiagConfig, cardTestDescription=cardTestDescription, TestId=TestId, cardFwVersion=cardFwVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentDiag=equipmentDiag, systemGeneralGroup=systemGeneralGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, cmUChannelID=cmUChannelID, equipmentStateGroup=equipmentStateGroup, auditDescription=auditDescription, auditTotalFailedCount=auditTotalFailedCount, systemKey=systemKey)
