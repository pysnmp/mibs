...
AdminState, PortId, SecondaryState, EqActionType, CardType, PortDetectedMode, CardId, CardSubType, DuplexStatus, FlowControlMode, PrimaryState, ShelfId, UnknownState, DiskVolumeUsageLevel, PortMode, FirmwareSource, PicType, PortType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PortId", "SecondaryState", "EqActionType", "CardType", "PortDetectedMode", "CardId", "CardSubType", "DuplexStatus", "FlowControlMode", "PrimaryState", "ShelfId", "UnknownState", "DiskVolumeUsageLevel", "PortMode", "FirmwareSource", "PicType", "PortType")
...
TruthValue, RowStatus, DateAndTime, TimeStamp, MacAddress, DisplayString, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "DateAndTime", "TimeStamp", "MacAddress", "DisplayString", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portMode=portMode, cadEquipmentMib=cadEquipmentMib, cardCpuType=cardCpuType, diskVolumeCardId=diskVolumeCardId, fpgaErrorEventTable=fpgaErrorEventTable, cardSubType=cardSubType, cardDetectedChange=cardDetectedChange, cardCarrierHwVersion=cardCarrierHwVersion, verboseLevel=verboseLevel, cardSpareGroupMode=cardSpareGroupMode, portSecStateChange=portSecStateChange, shelfName=shelfName, diskVolumeTrapInh=diskVolumeTrapInh, portDplxStatus=portDplxStatus, cardTestCommand=cardTestCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, portTrapInh=portTrapInh, cardSpareGroupId=cardSpareGroupId, TestResult=TestResult, diskVolumeId=diskVolumeId, shelfSwVersion=shelfSwVersion, portPrStateChange=portPrStateChange, cardPrState=cardPrState, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeFileName=diskVolumeFileName, consoleOutput=consoleOutput, cardHwVersion=cardHwVersion, diagTestId=diagTestId, auditStatus=auditStatus, errEvRowStatus=errEvRowStatus, cardTestResultDesc=cardTestResultDesc, auditLogOutput=auditLogOutput, portEntry=portEntry, cardTestTransId=cardTestTransId, equipmentDiag=equipmentDiag, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestResult=cardTestResult, diskVolumeGroup=diskVolumeGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, auditTotalPassedCount=auditTotalPassedCount, cardFwUpdateStatus=cardFwUpdateStatus, auditName=auditName, portMinDsPower=portMinDsPower, cardCPLDVersion=cardCPLDVersion, portDescription=portDescription, cardTable=cardTable, portDplxStatusChange=portDplxStatusChange, equipmentTbl=equipmentTbl, shelfObjects=shelfObjects, cardSwVersion=cardSwVersion, TestTransactionId=TestTransactionId, cardPicHwVersion=cardPicHwVersion, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDriveId=diskVolumeDriveId, portRxFlowControlDetected=portRxFlowControlDetected, systemGeneralGroup=systemGeneralGroup, errEvLogLevel=errEvLogLevel, diskDriveNumber=diskDriveNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetInfo=cmResetInfo, cardTestTime=cardTestTime, cardTestIndex=cardTestIndex, equipmentCardGroup=equipmentCardGroup, equipmentState=equipmentState, auditCommand=auditCommand, portPrState=portPrState, portConnectorId=portConnectorId, portMacIfIndex=portMacIfIndex, cardBootVersion=cardBootVersion, equipmentAudit=equipmentAudit, cardShelfId=cardShelfId, cardCpuSpeed=cardCpuSpeed, TestId=TestId, portDocsIfIndex=portDocsIfIndex, cardDplxStatus=cardDplxStatus, cardTestId=cardTestId, TestScheduleCommand=TestScheduleCommand, cardSecState=cardSecState, cardBusSpeed=cardBusSpeed, systemGeneral=systemGeneral, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portCardSubType=portCardSubType, cmResetClearNotification=cmResetClearNotification, portCurrDsPower=portCurrDsPower, portLastChangeTime=portLastChangeTime, cmUChannelID=cmUChannelID, cardTestType=cardTestType, cmMacAddress=cmMacAddress, auditCycleCount=auditCycleCount, portCardId=portCardId, cardLastChangeTime=cardLastChangeTime, portRxFlowControlMode=portRxFlowControlMode, errEvRecoveryEnabled=errEvRecoveryEnabled, cmVendor=cmVendor, auditLogThrottle=auditLogThrottle, diskVolumeEntry=diskVolumeEntry, cardSubDetected=cardSubDetected, TestSchedule=TestSchedule, cardTestEntry=cardTestEntry, auditFailedCount=auditFailedCount, TestCommand=TestCommand, cardTestDescription=cardTestDescription, cardTestTable=cardTestTable, trapSeverity=trapSeverity, cardId=cardId, TestType=TestType, cardFpgaSource=cardFpgaSource, auditAutoScheduling=auditAutoScheduling, auditEntry=auditEntry, auditDescription=auditDescription, cmResetReason=cmResetReason, cmResetStatus=cmResetStatus, cardShelfTestIndex=cardShelfTestIndex, portAction=portAction, cardSecStateChange=cardSecStateChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portSecState=portSecState, cardLastResetReason=cardLastResetReason, cardPicDetected=cardPicDetected, cardTestName=cardTestName, equipmentError=equipmentError, diskVolumeUsageNotification=diskVolumeUsageNotification, portTxFlowControlDetected=portTxFlowControlDetected, equipmentShelfGroup=equipmentShelfGroup, cardAction=cardAction, cmRegistrationNotification=cmRegistrationNotification, cmIpAddress=cmIpAddress, equipmentTraps=equipmentTraps, cardTestResultNotification=cardTestResultNotification, cmResetNotification=cmResetNotification, cmDevice=cmDevice, systemKey=systemKey, diskVolumeShelfId=diskVolumeShelfId, errEventId=errEventId, eqDiagConfig=eqDiagConfig, shelfNumber=shelfNumber, portBgpId=portBgpId, auditTable=auditTable, portNumChans=portNumChans, cardName=cardName, equipmentPortGroup=equipmentPortGroup, auditPassedCount=auditPassedCount, auditTotalFailedCount=auditTotalFailedCount, portId=portId, cardNumPorts=cardNumPorts, auditResult=auditResult, remainInDiagMode=remainInDiagMode, equipmentAuditGroup=equipmentAuditGroup, cardPrStateChange=cardPrStateChange, equipmentDiagGroup=equipmentDiagGroup, cardTemperatureHighError=cardTemperatureHighError, diskVolumeSize=diskVolumeSize, portMaxDsPower=portMaxDsPower, cardFwVersion=cardFwVersion, cardTestSchedule=cardTestSchedule, portTable=portTable, auditId=auditId, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cadEquipmentMibGroup=cadEquipmentMibGroup, auditShelfId=auditShelfId, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTempOverHeatNotification=cardTempOverHeatNotification, cardType=cardType, cardSerialNum=cardSerialNum, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCarrierSerialNum=cardCarrierSerialNum, cardPicSerialNum=cardPicSerialNum, diskVolumeName=diskVolumeName, diskVolumeNumber=diskVolumeNumber, portAdminState=portAdminState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTime=auditTime, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTrapInh=cardTrapInh, cardEntry=cardEntry, cardDplxStatusChange=cardDplxStatusChange, diskVolumeDiskSize=diskVolumeDiskSize, cardUpTime=cardUpTime, downstreamPowerLoss=downstreamPowerLoss, cardLastBootSource=cardLastBootSource, systemClock=systemClock, portMacAddress=portMacAddress, diskVolumeTable=diskVolumeTable, cardDetected=cardDetected, cardTemperature=cardTemperature, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditCardId=auditCardId, cardRamSize=cardRamSize, portShelfId=portShelfId, portType=portType, cmPrimarySID=cmPrimarySID, equipmentStateGroup=equipmentStateGroup, portNumber=portNumber, trapCounter=trapCounter, cardAdminState=cardAdminState, cmResetUpTime=cmResetUpTime, cardLastBootVersion=cardLastBootVersion, errEvLoggingEnabled=errEvLoggingEnabled, cardNumber=cardNumber, portDetectedMode=portDetectedMode, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardFlashSize=cardFlashSize)
