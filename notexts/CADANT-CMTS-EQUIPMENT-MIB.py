...
CardId, PortId, FirmwareSource, DiskVolumeUsageLevel, PrimaryState, UnknownState, FlowControlMode, SecondaryState, AdminState, PortDetectedMode, ShelfId, DuplexStatus, CardSubType, CardType, PicType, PortType, EqActionType, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortId", "FirmwareSource", "DiskVolumeUsageLevel", "PrimaryState", "UnknownState", "FlowControlMode", "SecondaryState", "AdminState", "PortDetectedMode", "ShelfId", "DuplexStatus", "CardSubType", "CardType", "PicType", "PortType", "EqActionType", "PortMode")
...
RowStatus, TimeStamp, TruthValue, MacAddress, TextualConvention, DateAndTime, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "TruthValue", "MacAddress", "TextualConvention", "DateAndTime", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTable=cardTable, cardShelfId=cardShelfId, diskVolumeCardId=diskVolumeCardId, auditPassedCount=auditPassedCount, consoleOutput=consoleOutput, equipmentDiagGroup=equipmentDiagGroup, cardHwVersion=cardHwVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, errEvRowStatus=errEvRowStatus, diskVolumeTrapInh=diskVolumeTrapInh, cardSecStateChange=cardSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeTable=diskVolumeTable, cardTestId=cardTestId, portBgpId=portBgpId, cardFpgaSource=cardFpgaSource, cardSecState=cardSecState, diskVolumeDriveId=diskVolumeDriveId, portCardId=portCardId, diskVolumeFileName=diskVolumeFileName, diskVolumeNumber=diskVolumeNumber, portLastChangeTime=portLastChangeTime, trapSeverity=trapSeverity, eqDiagConfig=eqDiagConfig, TestId=TestId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, systemClock=systemClock, cmMacAddress=cmMacAddress, cmResetClearNotification=cmResetClearNotification, cardFwVersion=cardFwVersion, auditTotalPassedCount=auditTotalPassedCount, remainInDiagMode=remainInDiagMode, downstreamPowerLoss=downstreamPowerLoss, cardTestDescription=cardTestDescription, equipmentTbl=equipmentTbl, auditDescription=auditDescription, portMinDsPower=portMinDsPower, shelfName=shelfName, portDescription=portDescription, diskVolumeName=diskVolumeName, auditTime=auditTime, cardAdminState=cardAdminState, cardSpareGroupMode=cardSpareGroupMode, portSecState=portSecState, cardFwUpdateStatus=cardFwUpdateStatus, auditCommand=auditCommand, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentDiag=equipmentDiag, cardTestSchedule=cardTestSchedule, portNumber=portNumber, portRxFlowControlDetected=portRxFlowControlDetected, portMacIfIndex=portMacIfIndex, cardSwVersion=cardSwVersion, cardName=cardName, cardTestResultDesc=cardTestResultDesc, portPrState=portPrState, cardDplxStatusChange=cardDplxStatusChange, cardDplxStatus=cardDplxStatus, cardCpuSpeed=cardCpuSpeed, equipmentCardGroup=equipmentCardGroup, portDetectedMode=portDetectedMode, auditEntry=auditEntry, diskVolumeSize=diskVolumeSize, auditCycleCount=auditCycleCount, portCurrDsPower=portCurrDsPower, TestResult=TestResult, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditId=auditId, errEventId=errEventId, cardNumPorts=cardNumPorts, auditName=auditName, portNumChans=portNumChans, auditTotalFailedCount=auditTotalFailedCount, errEvLogLevel=errEvLogLevel, cardPicDetected=cardPicDetected, portType=portType, cardCPLDVersion=cardCPLDVersion, equipmentStateGroup=equipmentStateGroup, cardBusSpeed=cardBusSpeed, TestCommand=TestCommand, cmVendor=cmVendor, cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditStatus=auditStatus, cardSubDetected=cardSubDetected, cardDetected=cardDetected, cardPrStateChange=cardPrStateChange, cardSubType=cardSubType, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDocsIfIndex=portDocsIfIndex, cadEquipmentMibConformance=cadEquipmentMibConformance, auditFailedCount=auditFailedCount, portRxFlowControlMode=portRxFlowControlMode, cardFlashSize=cardFlashSize, equipmentAudit=equipmentAudit, equipmentState=equipmentState, cardTemperature=cardTemperature, cardTestIndex=cardTestIndex, portTrapInh=portTrapInh, cardTemperatureHighError=cardTemperatureHighError, portTable=portTable, portMaxDsPower=portMaxDsPower, portCardSubType=portCardSubType, cardPicSerialNum=cardPicSerialNum, cardPicHwVersion=cardPicHwVersion, shelfSwVersion=shelfSwVersion, cmResetNotification=cmResetNotification, fpgaErrorEventTable=fpgaErrorEventTable, systemGeneralGroup=systemGeneralGroup, cardAction=cardAction, cardEntry=cardEntry, portTxFlowControlDetected=portTxFlowControlDetected, PYSNMP_MODULE_ID=cadEquipmentMib, cardTempOverHeatNotification=cardTempOverHeatNotification, cmRegistrationNotification=cmRegistrationNotification, TestScheduleCommand=TestScheduleCommand, portId=portId, cardLastChangeTime=cardLastChangeTime, TestType=TestType, diskVolumeEntry=diskVolumeEntry, diskVolumeShelfId=diskVolumeShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, portMode=portMode, cardTrapInh=cardTrapInh, cmDevice=cmDevice, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardLastBootSource=cardLastBootSource, systemKey=systemKey, portDplxStatus=portDplxStatus, cmResetUpTime=cmResetUpTime, cardId=cardId, cardTestTransId=cardTestTransId, portDplxStatusChange=portDplxStatusChange, cmUChannelID=cmUChannelID, portMacAddress=portMacAddress, cardPrState=cardPrState, cardType=cardType, auditShelfId=auditShelfId, cardCarrierHwVersion=cardCarrierHwVersion, diskDriveNumber=diskDriveNumber, cardSerialNum=cardSerialNum, auditAutoScheduling=auditAutoScheduling, shelfObjects=shelfObjects, cardTestName=cardTestName, auditResult=auditResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, auditLogOutput=auditLogOutput, cardUpTime=cardUpTime, portPrStateChange=portPrStateChange, equipmentTraps=equipmentTraps, cadEquipmentMib=cadEquipmentMib, cardTestResult=cardTestResult, TestSchedule=TestSchedule, trapCounter=trapCounter, portTxFlowControlMode=portTxFlowControlMode, diskVolumeId=diskVolumeId, diskVolumeGroup=diskVolumeGroup, auditCardId=auditCardId, errEvLoggingEnabled=errEvLoggingEnabled, cardTempNoReportNotification=cardTempNoReportNotification, cardLastBootVersion=cardLastBootVersion, cardBootVersion=cardBootVersion, systemGeneral=systemGeneral, cardTestTable=cardTestTable, portSecStateChange=portSecStateChange, cmIpAddress=cmIpAddress, cardTestEntry=cardTestEntry, cardCarrierSerialNum=cardCarrierSerialNum, cardTemperatureHighWarn=cardTemperatureHighWarn, portAdminState=portAdminState, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestResultNotification=cardTestResultNotification, TestTransactionId=TestTransactionId, portConnectorId=portConnectorId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardNumber=cardNumber, cmResetReason=cmResetReason, equipmentCardTestGroup=equipmentCardTestGroup, cardTestType=cardTestType, cmResetStatus=cmResetStatus, diskVolumeDiskSize=diskVolumeDiskSize, equipmentShelfGroup=equipmentShelfGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentPortGroup=equipmentPortGroup, cardLastResetReason=cardLastResetReason, auditTable=auditTable, equipmentError=equipmentError, cardDetectedChange=cardDetectedChange, portAction=portAction, portShelfId=portShelfId, equipmentAuditGroup=equipmentAuditGroup, cardTestScheduleCommand=cardTestScheduleCommand, cmPrimarySID=cmPrimarySID, cardTestCommand=cardTestCommand, cardSpareGroupId=cardSpareGroupId, cardRamSize=cardRamSize, cmResetInfo=cmResetInfo, auditLogThrottle=auditLogThrottle, cardCpuType=cardCpuType, portEntry=portEntry, verboseLevel=verboseLevel, shelfNumber=shelfNumber, diagTestId=diagTestId, cardTestTime=cardTestTime)
