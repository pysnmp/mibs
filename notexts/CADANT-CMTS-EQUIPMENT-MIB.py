...
CardType, CardSubType, DiskVolumeUsageLevel, DuplexStatus, SecondaryState, CardId, PortDetectedMode, PrimaryState, UnknownState, FirmwareSource, PortId, ShelfId, AdminState, PortType, PicType, EqActionType, PortMode, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "CardType", "CardSubType", "DiskVolumeUsageLevel", "DuplexStatus", "SecondaryState", "CardId", "PortDetectedMode", "PrimaryState", "UnknownState", "FirmwareSource", "PortId", "ShelfId", "AdminState", "PortType", "PicType", "EqActionType", "PortMode", "FlowControlMode")
...
DateAndTime, TimeStamp, DisplayString, TextualConvention, RowStatus, TruthValue, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TimeStamp", "DisplayString", "TextualConvention", "RowStatus", "TruthValue", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentDiagGroup=equipmentDiagGroup, auditDescription=auditDescription, portType=portType, diskVolumeSize=diskVolumeSize, cardFlashSize=cardFlashSize, shelfNumber=shelfNumber, cardTestName=cardTestName, equipmentShelfGroup=equipmentShelfGroup, cardSwVersion=cardSwVersion, consoleOutput=consoleOutput, cardNumPorts=cardNumPorts, portBgpId=portBgpId, portDetectedMode=portDetectedMode, diskVolumeFileName=diskVolumeFileName, portDescription=portDescription, portAction=portAction, cardLastChangeTime=cardLastChangeTime, cardTempOverHeatNotification=cardTempOverHeatNotification, errEventId=errEventId, equipmentState=equipmentState, cardTestSchedule=cardTestSchedule, cardTestTransId=cardTestTransId, cardId=cardId, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentStateGroup=equipmentStateGroup, portMinDsPower=portMinDsPower, cmResetStatus=cmResetStatus, cardTestScheduleCommand=cardTestScheduleCommand, auditShelfId=auditShelfId, portTrapInh=portTrapInh, systemGeneral=systemGeneral, portCardSubType=portCardSubType, auditCommand=auditCommand, cardLastResetReason=cardLastResetReason, cardDetectedChange=cardDetectedChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditLogThrottle=auditLogThrottle, portMacIfIndex=portMacIfIndex, cardTemperature=cardTemperature, cardLastBootVersion=cardLastBootVersion, auditTime=auditTime, cmRegistrationNotification=cmRegistrationNotification, auditName=auditName, errEvRowStatus=errEvRowStatus, portDplxStatus=portDplxStatus, diskVolumeTable=diskVolumeTable, auditLogOutput=auditLogOutput, auditCycleCount=auditCycleCount, TestType=TestType, diskVolumeTrapInh=diskVolumeTrapInh, cmPrimarySID=cmPrimarySID, cardCarrierFwVersion=cardCarrierFwVersion, cardSubType=cardSubType, diagTestId=diagTestId, cardDetected=cardDetected, eqDiagConfig=eqDiagConfig, cardShelfTestIndex=cardShelfTestIndex, portTxFlowControlMode=portTxFlowControlMode, cmResetUpTime=cmResetUpTime, cmVendor=cmVendor, cardTemperatureHighError=cardTemperatureHighError, portMode=portMode, systemClock=systemClock, auditAutoScheduling=auditAutoScheduling, trapSeverity=trapSeverity, portSecState=portSecState, cardHwVersion=cardHwVersion, portNumChans=portNumChans, cardSecStateChange=cardSecStateChange, auditTable=auditTable, equipmentDiag=equipmentDiag, portRxFlowControlDetected=portRxFlowControlDetected, cardTempNoReportNotification=cardTempNoReportNotification, systemGeneralGroup=systemGeneralGroup, cardSpareGroupId=cardSpareGroupId, cardSubDetected=cardSubDetected, portId=portId, cmUChannelID=cmUChannelID, cardPrStateChange=cardPrStateChange, cardAdminState=cardAdminState, cardRamSize=cardRamSize, equipmentTraps=equipmentTraps, cardCPLDVersion=cardCPLDVersion, cardName=cardName, cmIpAddress=cmIpAddress, equipmentAudit=equipmentAudit, auditFailedCount=auditFailedCount, cardTestId=cardTestId, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeName=diskVolumeName, portMacAddress=portMacAddress, cardUpTime=cardUpTime, diskVolumeEntry=diskVolumeEntry, cardAction=cardAction, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSecState=cardSecState, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeShelfId=diskVolumeShelfId, auditCardId=auditCardId, portTable=portTable, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portPrState=portPrState, auditPassedCount=auditPassedCount, TestCommand=TestCommand, cardDplxStatusChange=cardDplxStatusChange, TestId=TestId, cardSpareGroupMode=cardSpareGroupMode, remainInDiagMode=remainInDiagMode, trapCounter=trapCounter, cardSerialNum=cardSerialNum, equipmentCardTestGroup=equipmentCardTestGroup, portPrStateChange=portPrStateChange, cadEquipmentMib=cadEquipmentMib, diskVolumeGroup=diskVolumeGroup, shelfObjects=shelfObjects, cardType=cardType, cardBootVersion=cardBootVersion, shelfSwVersion=shelfSwVersion, equipmentError=equipmentError, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, fpgaErrorEventTable=fpgaErrorEventTable, shelfName=shelfName, cardTestTime=cardTestTime, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portNumber=portNumber, diskVolumeCardId=diskVolumeCardId, portLastChangeTime=portLastChangeTime, auditTotalPassedCount=auditTotalPassedCount, cmResetInfo=cmResetInfo, diskDriveNumber=diskDriveNumber, auditEntry=auditEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestType=cardTestType, cardBusSpeed=cardBusSpeed, cardCarrierHwVersion=cardCarrierHwVersion, equipmentPortGroup=equipmentPortGroup, cardPrState=cardPrState, verboseLevel=verboseLevel, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestDescription=cardTestDescription, diskVolumeId=diskVolumeId, cmMacAddress=cmMacAddress, equipmentCardGroup=equipmentCardGroup, diskVolumeNumber=diskVolumeNumber, cardShelfId=cardShelfId, equipmentTbl=equipmentTbl, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestCommand=cardTestCommand, auditStatus=auditStatus, cmResetNotification=cmResetNotification, cardCpuType=cardCpuType, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFpgaSource=cardFpgaSource, auditTotalFailedCount=auditTotalFailedCount, cardFwUpdateStatus=cardFwUpdateStatus, cmResetClearNotification=cmResetClearNotification, portShelfId=portShelfId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestTable=cardTestTable, PYSNMP_MODULE_ID=cadEquipmentMib, auditResult=auditResult, portEntry=portEntry, TestSchedule=TestSchedule, auditId=auditId, cardTestIndex=cardTestIndex, cardTestResultDesc=cardTestResultDesc, portCardId=portCardId, cardTrapInh=cardTrapInh, TestResult=TestResult, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvLoggingEnabled=errEvLoggingEnabled, portDplxStatusChange=portDplxStatusChange, downstreamPowerLoss=downstreamPowerLoss, portMaxDsPower=portMaxDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardPicDetected=cardPicDetected, cardTestEntry=cardTestEntry, portDocsIfIndex=portDocsIfIndex, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portSecStateChange=portSecStateChange, portConnectorId=portConnectorId, cardNumber=cardNumber, cardFwVersion=cardFwVersion, portRxFlowControlMode=portRxFlowControlMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResult=cardTestResult, cardCpuSpeed=cardCpuSpeed, errEvLogLevel=errEvLogLevel, diskVolumeDriveId=diskVolumeDriveId, cardPicHwVersion=cardPicHwVersion, cardLastBootSource=cardLastBootSource, diskVolumeUsageLevel=diskVolumeUsageLevel, cardDplxStatus=cardDplxStatus, cmDevice=cmDevice, cardTestResultNotification=cardTestResultNotification, cmResetReason=cmResetReason, cardEntry=cardEntry, portAdminState=portAdminState, systemKey=systemKey, portCurrDsPower=portCurrDsPower, cardTable=cardTable, TestTransactionId=TestTransactionId, cardPicSerialNum=cardPicSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, equipmentAuditGroup=equipmentAuditGroup, TestScheduleCommand=TestScheduleCommand)
