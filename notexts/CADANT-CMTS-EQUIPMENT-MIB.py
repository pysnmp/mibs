...
PortId, PortDetectedMode, PortMode, SecondaryState, CardSubType, CardId, CardType, FlowControlMode, PicType, FirmwareSource, DuplexStatus, EqActionType, PortType, PrimaryState, AdminState, UnknownState, ShelfId, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "PortId", "PortDetectedMode", "PortMode", "SecondaryState", "CardSubType", "CardId", "CardType", "FlowControlMode", "PicType", "FirmwareSource", "DuplexStatus", "EqActionType", "PortType", "PrimaryState", "AdminState", "UnknownState", "ShelfId", "DiskVolumeUsageLevel")
...
DateAndTime, RowStatus, DisplayString, MacAddress, TimeStamp, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "RowStatus", "DisplayString", "MacAddress", "TimeStamp", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBusSpeed=cardBusSpeed, portSecState=portSecState, portDetectedMode=portDetectedMode, cardNumber=cardNumber, cardTestCommand=cardTestCommand, cardCarrierSerialNum=cardCarrierSerialNum, TestCommand=TestCommand, cardFpgaSource=cardFpgaSource, portMaxDsPower=portMaxDsPower, cardTestSchedule=cardTestSchedule, cmDevice=cmDevice, cmResetNotification=cmResetNotification, portShelfId=portShelfId, portCardId=portCardId, cardTestTime=cardTestTime, cadEquipmentMibGroup=cadEquipmentMibGroup, cardPicSerialNum=cardPicSerialNum, cmResetClearNotification=cmResetClearNotification, cardDplxStatusChange=cardDplxStatusChange, diskVolumeName=diskVolumeName, cardDetectedChange=cardDetectedChange, portMode=portMode, auditTotalFailedCount=auditTotalFailedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestDescription=cardTestDescription, cardShelfId=cardShelfId, portMinDsPower=portMinDsPower, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditTable=auditTable, cardTestTransId=cardTestTransId, diskVolumeDriveId=diskVolumeDriveId, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditCommand=auditCommand, portSecStateChange=portSecStateChange, cardRamSize=cardRamSize, cmResetStatus=cmResetStatus, auditDescription=auditDescription, cadEquipmentMib=cadEquipmentMib, cardTestEntry=cardTestEntry, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardDplxStatus=cardDplxStatus, portCardSubType=portCardSubType, fpgaErrorEventTable=fpgaErrorEventTable, cardTestType=cardTestType, portNumber=portNumber, shelfSwVersion=shelfSwVersion, portLastChangeTime=portLastChangeTime, PYSNMP_MODULE_ID=cadEquipmentMib, cardHwVersion=cardHwVersion, cardPicDetected=cardPicDetected, diagTestId=diagTestId, cardAction=cardAction, auditShelfId=auditShelfId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTable=portTable, portTrapInh=portTrapInh, portDocsIfIndex=portDocsIfIndex, cardFwVersion=cardFwVersion, cardSpareGroupMode=cardSpareGroupMode, equipmentPortGroup=equipmentPortGroup, portType=portType, eqDiagConfig=eqDiagConfig, diskVolumeCardId=diskVolumeCardId, cardTestIndex=cardTestIndex, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSpareGroupId=cardSpareGroupId, auditResult=auditResult, cardTestTable=cardTestTable, cardPrStateChange=cardPrStateChange, cardNumPorts=cardNumPorts, cardFlashSize=cardFlashSize, consoleOutput=consoleOutput, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portTxFlowControlMode=portTxFlowControlMode, auditPassedCount=auditPassedCount, equipmentError=equipmentError, diskVolumeSize=diskVolumeSize, diskVolumeTable=diskVolumeTable, cardEntry=cardEntry, cardSecState=cardSecState, equipmentState=equipmentState, diskVolumeFileName=diskVolumeFileName, equipmentCardTestGroup=equipmentCardTestGroup, TestType=TestType, cardPicHwVersion=cardPicHwVersion, portDplxStatusChange=portDplxStatusChange, cardPrState=cardPrState, diskVolumeShelfId=diskVolumeShelfId, errEvRecoveryEnabled=errEvRecoveryEnabled, auditTime=auditTime, downstreamPowerLoss=downstreamPowerLoss, cardDetected=cardDetected, cardTestResultDesc=cardTestResultDesc, equipmentAuditGroup=equipmentAuditGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditAutoScheduling=auditAutoScheduling, cardSubDetected=cardSubDetected, errEvRowStatus=errEvRowStatus, portTxFlowControlDetected=portTxFlowControlDetected, auditName=auditName, cmResetUpTime=cmResetUpTime, cardTestResultNotification=cardTestResultNotification, systemKey=systemKey, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardName=cardName, diskVolumeUsageLevel=diskVolumeUsageLevel, errEventId=errEventId, cardType=cardType, portDescription=portDescription, cmResetReason=cmResetReason, cardCPLDVersion=cardCPLDVersion, cardLastBootVersion=cardLastBootVersion, portMacAddress=portMacAddress, cmVendor=cmVendor, portId=portId, diskVolumeNumber=diskVolumeNumber, cardCarrierFwVersion=cardCarrierFwVersion, portBgpId=portBgpId, equipmentCardGroup=equipmentCardGroup, cardTrapInh=cardTrapInh, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentShelfGroup=equipmentShelfGroup, auditFailedCount=auditFailedCount, diskDriveNumber=diskDriveNumber, auditStatus=auditStatus, equipmentDiag=equipmentDiag, cardUpTime=cardUpTime, equipmentTraps=equipmentTraps, portAdminState=portAdminState, cardTestId=cardTestId, portPrState=portPrState, errEvLogLevel=errEvLogLevel, cardTestResult=cardTestResult, auditEntry=auditEntry, systemClock=systemClock, shelfNumber=shelfNumber, cardSubType=cardSubType, cardLastResetReason=cardLastResetReason, TestId=TestId, cardTemperature=cardTemperature, cardTempNoReportNotification=cardTempNoReportNotification, cmPrimarySID=cmPrimarySID, diskVolumeUsageNotification=diskVolumeUsageNotification, TestResult=TestResult, cardLastBootSource=cardLastBootSource, diskVolumeTrapInh=diskVolumeTrapInh, portCurrDsPower=portCurrDsPower, portAction=portAction, cmIpAddress=cmIpAddress, TestScheduleCommand=TestScheduleCommand, remainInDiagMode=remainInDiagMode, portNumChans=portNumChans, shelfName=shelfName, diskVolumeEntry=diskVolumeEntry, portMacIfIndex=portMacIfIndex, errEvLoggingEnabled=errEvLoggingEnabled, equipmentDiagGroup=equipmentDiagGroup, cardCpuSpeed=cardCpuSpeed, cardBootVersion=cardBootVersion, cardFwUpdateStatus=cardFwUpdateStatus, portDplxStatus=portDplxStatus, diskVolumeDiskSize=diskVolumeDiskSize, auditId=auditId, cardLastChangeTime=cardLastChangeTime, auditTotalPassedCount=auditTotalPassedCount, portConnectorId=portConnectorId, cardShelfTestIndex=cardShelfTestIndex, diskVolumeId=diskVolumeId, cardSwVersion=cardSwVersion, shelfObjects=shelfObjects, trapCounter=trapCounter, auditLogOutput=auditLogOutput, cardTable=cardTable, cmMacAddress=cmMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, portPrStateChange=portPrStateChange, portEntry=portEntry, cardTestName=cardTestName, cmUChannelID=cmUChannelID, equipmentAudit=equipmentAudit, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditCardId=auditCardId, diskVolumeGroup=diskVolumeGroup, cmResetInfo=cmResetInfo, cadEquipmentMibConformance=cadEquipmentMibConformance, TestTransactionId=TestTransactionId, equipmentTbl=equipmentTbl, TestSchedule=TestSchedule, cardCpuType=cardCpuType, systemGeneral=systemGeneral, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSerialNum=cardSerialNum, cardSecStateChange=cardSecStateChange, cmRegistrationNotification=cmRegistrationNotification, cardTemperatureHighError=cardTemperatureHighError, verboseLevel=verboseLevel, auditLogThrottle=auditLogThrottle, systemGeneralGroup=systemGeneralGroup, trapSeverity=trapSeverity, auditCycleCount=auditCycleCount, cardCarrierHwVersion=cardCarrierHwVersion, cardId=cardId, cardAdminState=cardAdminState, portRxFlowControlMode=portRxFlowControlMode)
