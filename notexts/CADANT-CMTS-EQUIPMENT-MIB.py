...
PortDetectedMode, PrimaryState, SecondaryState, EqActionType, CardSubType, AdminState, DuplexStatus, PortType, ShelfId, UnknownState, FlowControlMode, PortMode, FirmwareSource, CardType, PortId, CardId, PicType, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "PrimaryState", "SecondaryState", "EqActionType", "CardSubType", "AdminState", "DuplexStatus", "PortType", "ShelfId", "UnknownState", "FlowControlMode", "PortMode", "FirmwareSource", "CardType", "PortId", "CardId", "PicType", "DiskVolumeUsageLevel")
...
RowStatus, TextualConvention, TruthValue, DisplayString, DateAndTime, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "TruthValue", "DisplayString", "DateAndTime", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portLastChangeTime=portLastChangeTime, fpgaErrorEventTable=fpgaErrorEventTable, cardTestResult=cardTestResult, cardType=cardType, cardTestEntry=cardTestEntry, cadEquipmentMib=cadEquipmentMib, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentDiag=equipmentDiag, cardTestSchedule=cardTestSchedule, auditDescription=auditDescription, cardNumPorts=cardNumPorts, portAction=portAction, cardTestResultDesc=cardTestResultDesc, auditName=auditName, TestSchedule=TestSchedule, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, portTable=portTable, portPrStateChange=portPrStateChange, auditCardId=auditCardId, auditCommand=auditCommand, cmResetClearNotification=cmResetClearNotification, cardTestDescription=cardTestDescription, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditLogThrottle=auditLogThrottle, equipmentTbl=equipmentTbl, cardTestTransId=cardTestTransId, portCardId=portCardId, diagTestId=diagTestId, cardPicSerialNum=cardPicSerialNum, portTrapInh=portTrapInh, cardTestId=cardTestId, auditStatus=auditStatus, eqDiagConfig=eqDiagConfig, portId=portId, portMacAddress=portMacAddress, portDetectedMode=portDetectedMode, cardSecState=cardSecState, diskVolumeId=diskVolumeId, cardShelfId=cardShelfId, cardTestCommand=cardTestCommand, cmResetNotification=cmResetNotification, cardCarrierSerialNum=cardCarrierSerialNum, portMinDsPower=portMinDsPower, downstreamPowerLoss=downstreamPowerLoss, cardLastBootSource=cardLastBootSource, portTxFlowControlDetected=portTxFlowControlDetected, cmDevice=cmDevice, cardShelfTestIndex=cardShelfTestIndex, cardCarrierFwVersion=cardCarrierFwVersion, errEvLoggingEnabled=errEvLoggingEnabled, cardCarrierHwVersion=cardCarrierHwVersion, cmResetStatus=cmResetStatus, cardPicHwVersion=cardPicHwVersion, cardTestTable=cardTestTable, equipmentCardGroup=equipmentCardGroup, TestType=TestType, fpgaErrorEventGroup=fpgaErrorEventGroup, portMode=portMode, equipmentAuditGroup=equipmentAuditGroup, portSecState=portSecState, diskVolumeTable=diskVolumeTable, diskVolumeShelfId=diskVolumeShelfId, auditTable=auditTable, cardNumber=cardNumber, cardSubDetected=cardSubDetected, cmUChannelID=cmUChannelID, cardFlashSize=cardFlashSize, cardDetected=cardDetected, portCurrDsPower=portCurrDsPower, portNumChans=portNumChans, portCardSubType=portCardSubType, cardDplxStatus=cardDplxStatus, cardTrapInh=cardTrapInh, cardPicDetected=cardPicDetected, portShelfId=portShelfId, cmMacAddress=cmMacAddress, cardPrState=cardPrState, cmPrimarySID=cmPrimarySID, cardTestScheduleCommand=cardTestScheduleCommand, errEvLogLevel=errEvLogLevel, equipmentCardTestGroup=equipmentCardTestGroup, cardTempNoReportNotification=cardTempNoReportNotification, equipmentStateGroup=equipmentStateGroup, cmIpAddress=cmIpAddress, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentDiagGroup=equipmentDiagGroup, consoleOutput=consoleOutput, auditFailedCount=auditFailedCount, cmResetReason=cmResetReason, TestTransactionId=TestTransactionId, cardDplxStatusChange=cardDplxStatusChange, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastBootVersion=cardLastBootVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, shelfNumber=shelfNumber, portDplxStatusChange=portDplxStatusChange, portMaxDsPower=portMaxDsPower, cardLastChangeTime=cardLastChangeTime, errEvRecoveryEnabled=errEvRecoveryEnabled, cmRegistrationNotification=cmRegistrationNotification, portConnectorId=portConnectorId, auditTotalPassedCount=auditTotalPassedCount, cardPrStateChange=cardPrStateChange, cardSerialNum=cardSerialNum, cardSubType=cardSubType, diskVolumeGroup=diskVolumeGroup, shelfObjects=shelfObjects, cmVendor=cmVendor, portDplxStatus=portDplxStatus, TestId=TestId, cardAction=cardAction, cardRamSize=cardRamSize, cardTemperature=cardTemperature, portAdminState=portAdminState, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeNumber=diskVolumeNumber, errEventId=errEventId, equipmentState=equipmentState, cardCpuType=cardCpuType, equipmentShelfGroup=equipmentShelfGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeFileName=diskVolumeFileName, portRxFlowControlMode=portRxFlowControlMode, diskVolumeSize=diskVolumeSize, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestScheduleCommand=TestScheduleCommand, systemGeneralGroup=systemGeneralGroup, trapCounter=trapCounter, cardTestName=cardTestName, equipmentAudit=equipmentAudit, cardUpTime=cardUpTime, diskVolumeName=diskVolumeName, cardTemperatureHighError=cardTemperatureHighError, systemKey=systemKey, portDocsIfIndex=portDocsIfIndex, cardSpareGroupId=cardSpareGroupId, portPrState=portPrState, trapSeverity=trapSeverity, cardSwVersion=cardSwVersion, TestCommand=TestCommand, cardFpgaSource=cardFpgaSource, diskVolumeCardId=diskVolumeCardId, cardBootVersion=cardBootVersion, portSecStateChange=portSecStateChange, equipmentError=equipmentError, verboseLevel=verboseLevel, diskDriveNumber=diskDriveNumber, cardTestIndex=cardTestIndex, cardLastResetReason=cardLastResetReason, diskVolumeDiskSize=diskVolumeDiskSize, portType=portType, auditTime=auditTime, PYSNMP_MODULE_ID=cadEquipmentMib, cardId=cardId, portNumber=portNumber, auditLogOutput=auditLogOutput, cardSecStateChange=cardSecStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetInfo=cmResetInfo, cardCpuSpeed=cardCpuSpeed, cardDetectedChange=cardDetectedChange, cardTestResultNotification=cardTestResultNotification, auditPassedCount=auditPassedCount, cardCPLDVersion=cardCPLDVersion, cardAdminState=cardAdminState, diskVolumeDriveId=diskVolumeDriveId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupMode=cardSpareGroupMode, auditId=auditId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardFwUpdateStatus=cardFwUpdateStatus, TestResult=TestResult, cardName=cardName, cardFwVersion=cardFwVersion, portEntry=portEntry, portTxFlowControlMode=portTxFlowControlMode, remainInDiagMode=remainInDiagMode, cardTestTime=cardTestTime, portBgpId=portBgpId, portMacIfIndex=portMacIfIndex, errEvRowStatus=errEvRowStatus, diskVolumeTrapInh=diskVolumeTrapInh, shelfSwVersion=shelfSwVersion, cardTable=cardTable, fpgaErrorEventEntry=fpgaErrorEventEntry, auditCycleCount=auditCycleCount, auditShelfId=auditShelfId, cmResetUpTime=cmResetUpTime, cardHwVersion=cardHwVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestType=cardTestType, shelfName=shelfName, systemGeneral=systemGeneral, portDescription=portDescription, cardBusSpeed=cardBusSpeed, auditResult=auditResult, equipmentTraps=equipmentTraps, auditEntry=auditEntry, systemClock=systemClock, cardEntry=cardEntry, equipmentPortGroup=equipmentPortGroup, auditTotalFailedCount=auditTotalFailedCount, diskVolumeEntry=diskVolumeEntry)
