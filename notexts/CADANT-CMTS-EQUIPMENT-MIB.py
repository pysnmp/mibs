...
PortId, PortType, CardType, DiskVolumeUsageLevel, FirmwareSource, FlowControlMode, PicType, PortMode, PrimaryState, AdminState, ShelfId, EqActionType, UnknownState, SecondaryState, PortDetectedMode, DuplexStatus, CardId, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PortId", "PortType", "CardType", "DiskVolumeUsageLevel", "FirmwareSource", "FlowControlMode", "PicType", "PortMode", "PrimaryState", "AdminState", "ShelfId", "EqActionType", "UnknownState", "SecondaryState", "PortDetectedMode", "DuplexStatus", "CardId", "CardSubType")
...
DisplayString, TruthValue, TextualConvention, RowStatus, MacAddress, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TruthValue", "TextualConvention", "RowStatus", "MacAddress", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardDplxStatus=cardDplxStatus, cardCPLDVersion=cardCPLDVersion, auditEntry=auditEntry, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageLevel=diskVolumeUsageLevel, verboseLevel=verboseLevel, cadEquipmentMibGroup=cadEquipmentMibGroup, cardRamSize=cardRamSize, portTxFlowControlMode=portTxFlowControlMode, diskVolumeEntry=diskVolumeEntry, systemGeneral=systemGeneral, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentTbl=equipmentTbl, cardTrapInh=cardTrapInh, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardDetected=cardDetected, cardName=cardName, cmResetStatus=cmResetStatus, TestResult=TestResult, equipmentTraps=equipmentTraps, portBgpId=portBgpId, cardTemperatureHighError=cardTemperatureHighError, cardTestEntry=cardTestEntry, cardFwVersion=cardFwVersion, auditDescription=auditDescription, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardFpgaSource=cardFpgaSource, auditPassedCount=auditPassedCount, cardSecStateChange=cardSecStateChange, cardTestDescription=cardTestDescription, portDplxStatusChange=portDplxStatusChange, cardShelfId=cardShelfId, portCardId=portCardId, portDocsIfIndex=portDocsIfIndex, auditCycleCount=auditCycleCount, auditTotalFailedCount=auditTotalFailedCount, cardTempNoReportNotification=cardTempNoReportNotification, cardDplxStatusChange=cardDplxStatusChange, shelfName=shelfName, equipmentAudit=equipmentAudit, portCurrDsPower=portCurrDsPower, cardTable=cardTable, TestCommand=TestCommand, auditAutoScheduling=auditAutoScheduling, auditLogOutput=auditLogOutput, consoleOutput=consoleOutput, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestName=cardTestName, cardFwUpdateStatus=cardFwUpdateStatus, cardSecState=cardSecState, diagTestId=diagTestId, portRxFlowControlMode=portRxFlowControlMode, portCardSubType=portCardSubType, cardSpareGroupId=cardSpareGroupId, equipmentDiag=equipmentDiag, auditCardId=auditCardId, cmUChannelID=cmUChannelID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, systemKey=systemKey, portDescription=portDescription, TestId=TestId, TestSchedule=TestSchedule, cardCarrierFwVersion=cardCarrierFwVersion, cardTestCommand=cardTestCommand, equipmentPortGroup=equipmentPortGroup, cardBootVersion=cardBootVersion, cardSwVersion=cardSwVersion, cardLastBootSource=cardLastBootSource, cardDetectedChange=cardDetectedChange, TestTransactionId=TestTransactionId, cardSubDetected=cardSubDetected, equipmentShelfGroup=equipmentShelfGroup, diskVolumeName=diskVolumeName, cardTestId=cardTestId, auditResult=auditResult, shelfObjects=shelfObjects, cardTestSchedule=cardTestSchedule, cardPrState=cardPrState, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeId=diskVolumeId, equipmentDiagGroup=equipmentDiagGroup, cardId=cardId, portEntry=portEntry, equipmentError=equipmentError, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardNumber=cardNumber, diskVolumeNumber=diskVolumeNumber, cardUpTime=cardUpTime, portMacIfIndex=portMacIfIndex, auditTime=auditTime, cardCpuSpeed=cardCpuSpeed, portConnectorId=portConnectorId, cardAction=cardAction, auditId=auditId, diskDriveNumber=diskDriveNumber, cardTestTransId=cardTestTransId, cardSerialNum=cardSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, systemGeneralGroup=systemGeneralGroup, cardTestTime=cardTestTime, cardSubType=cardSubType, cmVendor=cmVendor, equipmentCardTestGroup=equipmentCardTestGroup, cmIpAddress=cmIpAddress, PYSNMP_MODULE_ID=cadEquipmentMib, portShelfId=portShelfId, portMacAddress=portMacAddress, shelfSwVersion=shelfSwVersion, cardPicSerialNum=cardPicSerialNum, portTrapInh=portTrapInh, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditStatus=auditStatus, cardBusSpeed=cardBusSpeed, cardLastBootVersion=cardLastBootVersion, auditTable=auditTable, cardCarrierHwVersion=cardCarrierHwVersion, auditFailedCount=auditFailedCount, portNumChans=portNumChans, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestResultNotification=cardTestResultNotification, portMaxDsPower=portMaxDsPower, diskVolumeDriveId=diskVolumeDriveId, diskVolumeFileName=diskVolumeFileName, portAdminState=portAdminState, diskVolumeDiskSize=diskVolumeDiskSize, cmResetInfo=cmResetInfo, portId=portId, cardShelfTestIndex=cardShelfTestIndex, auditTotalPassedCount=auditTotalPassedCount, cardTemperature=cardTemperature, portTxFlowControlDetected=portTxFlowControlDetected, errEvRowStatus=errEvRowStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, portAction=portAction, portLastChangeTime=portLastChangeTime, diskVolumeShelfId=diskVolumeShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSpareGroupMode=cardSpareGroupMode, cardTestScheduleCommand=cardTestScheduleCommand, systemClock=systemClock, cardEntry=cardEntry, cardTestIndex=cardTestIndex, cmMacAddress=cmMacAddress, errEvLoggingEnabled=errEvLoggingEnabled, cardTestTable=cardTestTable, diskVolumeGroup=diskVolumeGroup, cardLastResetReason=cardLastResetReason, cardTemperatureHighWarn=cardTemperatureHighWarn, downstreamPowerLoss=downstreamPowerLoss, portMode=portMode, errEvLogLevel=errEvLogLevel, cardTestResult=cardTestResult, cardPrStateChange=cardPrStateChange, diskVolumeCardId=diskVolumeCardId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditShelfId=auditShelfId, fpgaErrorEventTable=fpgaErrorEventTable, portSecStateChange=portSecStateChange, eqDiagConfig=eqDiagConfig, auditName=auditName, cardFlashSize=cardFlashSize, trapSeverity=trapSeverity, auditCommand=auditCommand, cmResetUpTime=cmResetUpTime, cardAdminState=cardAdminState, portDetectedMode=portDetectedMode, cardTestType=cardTestType, diskVolumeSize=diskVolumeSize, TestType=TestType, portType=portType, equipmentAuditGroup=equipmentAuditGroup, cardTestResultDesc=cardTestResultDesc, portTable=portTable, cardHwVersion=cardHwVersion, portSecState=portSecState, cadEquipmentMibConformance=cadEquipmentMibConformance, auditLogThrottle=auditLogThrottle, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardPicDetected=cardPicDetected, diskVolumeTrapInh=diskVolumeTrapInh, remainInDiagMode=remainInDiagMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsageNotification=diskVolumeUsageNotification, cmResetNotification=cmResetNotification, cmResetClearNotification=cmResetClearNotification, cardType=cardType, TestScheduleCommand=TestScheduleCommand, cardPicHwVersion=cardPicHwVersion, portDplxStatus=portDplxStatus, cmDevice=cmDevice, errEventId=errEventId, portNumber=portNumber, portPrState=portPrState, shelfNumber=shelfNumber, cmResetReason=cmResetReason, cardNumPorts=cardNumPorts, portMinDsPower=portMinDsPower, cmPrimarySID=cmPrimarySID, cmRegistrationNotification=cmRegistrationNotification, trapCounter=trapCounter, diskVolumeTable=diskVolumeTable, portPrStateChange=portPrStateChange, equipmentCardGroup=equipmentCardGroup, equipmentState=equipmentState, cardCpuType=cardCpuType)
