...
AdminState, FirmwareSource, FlowControlMode, PortDetectedMode, PortType, PortMode, DuplexStatus, PicType, PrimaryState, CardSubType, EqActionType, DiskVolumeUsageLevel, PortId, SecondaryState, ShelfId, CardType, CardId, UnknownState = mibBuilder.importSymbols("CADANT-TC", "AdminState", "FirmwareSource", "FlowControlMode", "PortDetectedMode", "PortType", "PortMode", "DuplexStatus", "PicType", "PrimaryState", "CardSubType", "EqActionType", "DiskVolumeUsageLevel", "PortId", "SecondaryState", "ShelfId", "CardType", "CardId", "UnknownState")
...
TimeStamp, TextualConvention, TruthValue, DateAndTime, DisplayString, MacAddress, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TextualConvention", "TruthValue", "DateAndTime", "DisplayString", "MacAddress", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetReason=cmResetReason, equipmentAudit=equipmentAudit, errEvLogLevel=errEvLogLevel, auditCardId=auditCardId, auditResult=auditResult, auditPassedCount=auditPassedCount, equipmentState=equipmentState, portConnectorId=portConnectorId, cardLastBootVersion=cardLastBootVersion, cardCPLDVersion=cardCPLDVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cardEntry=cardEntry, portNumChans=portNumChans, portDescription=portDescription, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, cardDplxStatus=cardDplxStatus, equipmentCardGroup=equipmentCardGroup, equipmentTraps=equipmentTraps, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portId=portId, portBgpId=portBgpId, shelfName=shelfName, errEventId=errEventId, auditTable=auditTable, cardDetectedChange=cardDetectedChange, portMacAddress=portMacAddress, cadEquipmentMibConformance=cadEquipmentMibConformance, cmUChannelID=cmUChannelID, cardTestEntry=cardTestEntry, cardNumber=cardNumber, equipmentStateGroup=equipmentStateGroup, cardUpTime=cardUpTime, portNumber=portNumber, cmResetStatus=cmResetStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDplxStatus=portDplxStatus, cardTempNoReportNotification=cardTempNoReportNotification, consoleOutput=consoleOutput, cardSwVersion=cardSwVersion, cardAdminState=cardAdminState, portDocsIfIndex=portDocsIfIndex, cardTable=cardTable, downstreamPowerLoss=downstreamPowerLoss, cardTestTime=cardTestTime, auditName=auditName, equipmentAuditGroup=equipmentAuditGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardAction=cardAction, auditId=auditId, cardDetected=cardDetected, diskVolumeDiskSize=diskVolumeDiskSize, cardTestCommand=cardTestCommand, auditTime=auditTime, portShelfId=portShelfId, diskDriveNumber=diskDriveNumber, portTxFlowControlDetected=portTxFlowControlDetected, auditAutoScheduling=auditAutoScheduling, cardTestIndex=cardTestIndex, cardTestDescription=cardTestDescription, trapSeverity=trapSeverity, shelfObjects=shelfObjects, diskVolumeName=diskVolumeName, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeSize=diskVolumeSize, cardHwVersion=cardHwVersion, portCurrDsPower=portCurrDsPower, shelfNumber=shelfNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, eqDiagConfig=eqDiagConfig, cadEquipmentMib=cadEquipmentMib, diskVolumeTable=diskVolumeTable, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, TestType=TestType, equipmentError=equipmentError, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardDplxStatusChange=cardDplxStatusChange, cardSpareGroupId=cardSpareGroupId, cardTemperatureHighError=cardTemperatureHighError, TestResult=TestResult, TestScheduleCommand=TestScheduleCommand, diskVolumeCardId=diskVolumeCardId, cardSubType=cardSubType, cardSerialNum=cardSerialNum, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDetectedMode=portDetectedMode, equipmentDiagGroup=equipmentDiagGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardFwVersion=cardFwVersion, cardLastResetReason=cardLastResetReason, portMaxDsPower=portMaxDsPower, cmResetInfo=cmResetInfo, cmPrimarySID=cmPrimarySID, auditStatus=auditStatus, diskVolumeId=diskVolumeId, portEntry=portEntry, systemKey=systemKey, cardShelfTestIndex=cardShelfTestIndex, portTxFlowControlMode=portTxFlowControlMode, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeGroup=diskVolumeGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentTbl=equipmentTbl, errEvLoggingEnabled=errEvLoggingEnabled, cardPrStateChange=cardPrStateChange, cardPicHwVersion=cardPicHwVersion, cardCarrierHwVersion=cardCarrierHwVersion, cardName=cardName, portAdminState=portAdminState, cardTestResultDesc=cardTestResultDesc, cardPrState=cardPrState, cardRamSize=cardRamSize, diagTestId=diagTestId, portTable=portTable, cardBootVersion=cardBootVersion, portAction=portAction, auditShelfId=auditShelfId, portMacIfIndex=portMacIfIndex, cardSecStateChange=cardSecStateChange, diskVolumeDriveId=diskVolumeDriveId, equipmentShelfGroup=equipmentShelfGroup, cmResetClearNotification=cmResetClearNotification, errEvRowStatus=errEvRowStatus, cmDevice=cmDevice, cmMacAddress=cmMacAddress, cardCpuType=cardCpuType, portMode=portMode, portPrStateChange=portPrStateChange, cardTestSchedule=cardTestSchedule, shelfSwVersion=shelfSwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditCycleCount=auditCycleCount, cardId=cardId, auditTotalFailedCount=auditTotalFailedCount, TestSchedule=TestSchedule, trapCounter=trapCounter, cardFlashSize=cardFlashSize, fpgaErrorEventTable=fpgaErrorEventTable, cardTrapInh=cardTrapInh, cardCpuSpeed=cardCpuSpeed, portSecStateChange=portSecStateChange, cmResetNotification=cmResetNotification, TestTransactionId=TestTransactionId, portCardId=portCardId, equipmentPortGroup=equipmentPortGroup, auditLogOutput=auditLogOutput, diskVolumeNumber=diskVolumeNumber, portMinDsPower=portMinDsPower, cardPicSerialNum=cardPicSerialNum, cardTestTable=cardTestTable, cardType=cardType, auditFailedCount=auditFailedCount, cardTestTransId=cardTestTransId, portCardSubType=portCardSubType, auditEntry=auditEntry, systemGeneralGroup=systemGeneralGroup, auditLogThrottle=auditLogThrottle, cmRegistrationNotification=cmRegistrationNotification, portTrapInh=portTrapInh, diskVolumeFileName=diskVolumeFileName, cardNumPorts=cardNumPorts, verboseLevel=verboseLevel, cardLastChangeTime=cardLastChangeTime, cardSpareGroupMode=cardSpareGroupMode, portPrState=portPrState, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentCardTestGroup=equipmentCardTestGroup, auditCommand=auditCommand, cardPicDetected=cardPicDetected, cardFpgaSource=cardFpgaSource, cmIpAddress=cmIpAddress, diskVolumeEntry=diskVolumeEntry, remainInDiagMode=remainInDiagMode, cardTestResultNotification=cardTestResultNotification, cardLastBootSource=cardLastBootSource, cmVendor=cmVendor, cardSecState=cardSecState, cardTemperature=cardTemperature, portLastChangeTime=portLastChangeTime, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestResult=cardTestResult, TestCommand=TestCommand, cardTempOverHeatNotification=cardTempOverHeatNotification, systemClock=systemClock, cardTestScheduleCommand=cardTestScheduleCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, portSecState=portSecState, cardSubDetected=cardSubDetected, auditTotalPassedCount=auditTotalPassedCount, portDplxStatusChange=portDplxStatusChange, systemGeneral=systemGeneral, portType=portType, auditDescription=auditDescription, cardTestId=cardTestId, cardTestType=cardTestType, cmResetUpTime=cmResetUpTime, cardFwUpdateStatus=cardFwUpdateStatus, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestName=cardTestName, cardBusSpeed=cardBusSpeed, TestId=TestId, equipmentDiag=equipmentDiag, cardShelfId=cardShelfId)
