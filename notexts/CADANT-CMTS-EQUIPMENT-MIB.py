...
CardId, PortDetectedMode, PortType, ShelfId, UnknownState, CardType, DuplexStatus, PicType, PrimaryState, SecondaryState, FlowControlMode, DiskVolumeUsageLevel, EqActionType, PortMode, CardSubType, FirmwareSource, AdminState, PortId = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortDetectedMode", "PortType", "ShelfId", "UnknownState", "CardType", "DuplexStatus", "PicType", "PrimaryState", "SecondaryState", "FlowControlMode", "DiskVolumeUsageLevel", "EqActionType", "PortMode", "CardSubType", "FirmwareSource", "AdminState", "PortId")
...
TimeStamp, DisplayString, RowStatus, MacAddress, TruthValue, TextualConvention, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DisplayString", "RowStatus", "MacAddress", "TruthValue", "TextualConvention", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cadEquipmentMibCompliances=cadEquipmentMibCompliances, remainInDiagMode=remainInDiagMode, TestResult=TestResult, cardShelfId=cardShelfId, consoleOutput=consoleOutput, cardTable=cardTable, cardTestSchedule=cardTestSchedule, cardSecState=cardSecState, auditEntry=auditEntry, portBgpId=portBgpId, cardTestResult=cardTestResult, portId=portId, cardAction=cardAction, equipmentCardTestGroup=equipmentCardTestGroup, TestSchedule=TestSchedule, equipmentTbl=equipmentTbl, portDocsIfIndex=portDocsIfIndex, auditTable=auditTable, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeDiskSize=diskVolumeDiskSize, cmRegistrationNotification=cmRegistrationNotification, equipmentShelfGroup=equipmentShelfGroup, errEventId=errEventId, auditCommand=auditCommand, portSecStateChange=portSecStateChange, portTable=portTable, diskDriveNumber=diskDriveNumber, diagTestId=diagTestId, cmUChannelID=cmUChannelID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, shelfNumber=shelfNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMibConformance=cadEquipmentMibConformance, portConnectorId=portConnectorId, cardSubType=cardSubType, portType=portType, cardLastChangeTime=cardLastChangeTime, cardEntry=cardEntry, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, shelfName=shelfName, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeEntry=diskVolumeEntry, cardPicHwVersion=cardPicHwVersion, auditResult=auditResult, cardDplxStatusChange=cardDplxStatusChange, portCardId=portCardId, equipmentState=equipmentState, cadEquipmentMib=cadEquipmentMib, portAdminState=portAdminState, TestScheduleCommand=TestScheduleCommand, cardDplxStatus=cardDplxStatus, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeFileName=diskVolumeFileName, portAction=portAction, portTxFlowControlMode=portTxFlowControlMode, auditLogThrottle=auditLogThrottle, auditCardId=auditCardId, portSecState=portSecState, auditTime=auditTime, portLastChangeTime=portLastChangeTime, cardLastBootVersion=cardLastBootVersion, diskVolumeName=diskVolumeName, cardSubDetected=cardSubDetected, systemGeneral=systemGeneral, cmVendor=cmVendor, cardTestResultNotification=cardTestResultNotification, equipmentAudit=equipmentAudit, auditTotalFailedCount=auditTotalFailedCount, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvLoggingEnabled=errEvLoggingEnabled, portDplxStatusChange=portDplxStatusChange, cardLastResetReason=cardLastResetReason, fpgaErrorEventGroup=fpgaErrorEventGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardAdminState=cardAdminState, systemKey=systemKey, eqDiagConfig=eqDiagConfig, diskVolumeId=diskVolumeId, cardPicSerialNum=cardPicSerialNum, portMaxDsPower=portMaxDsPower, equipmentStateGroup=equipmentStateGroup, portPrState=portPrState, equipmentError=equipmentError, cmResetInfo=cmResetInfo, fpgaErrorEventEntry=fpgaErrorEventEntry, portDplxStatus=portDplxStatus, cardCarrierSerialNum=cardCarrierSerialNum, portTrapInh=portTrapInh, cardPrStateChange=cardPrStateChange, shelfObjects=shelfObjects, cardCpuSpeed=cardCpuSpeed, diskVolumeTrapInh=diskVolumeTrapInh, cmPrimarySID=cmPrimarySID, cardHwVersion=cardHwVersion, auditFailedCount=auditFailedCount, cmResetClearNotification=cmResetClearNotification, cardTestName=cardTestName, cardTemperatureHighError=cardTemperatureHighError, cardTestScheduleCommand=cardTestScheduleCommand, auditName=auditName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSpareGroupId=cardSpareGroupId, cardSwVersion=cardSwVersion, cardTestTable=cardTestTable, cmResetReason=cmResetReason, auditLogOutput=auditLogOutput, cardFpgaSource=cardFpgaSource, portShelfId=portShelfId, cardTestCommand=cardTestCommand, cardFlashSize=cardFlashSize, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestType=TestType, verboseLevel=verboseLevel, diskVolumeUsageLevel=diskVolumeUsageLevel, cmMacAddress=cmMacAddress, cardBusSpeed=cardBusSpeed, portCurrDsPower=portCurrDsPower, TestId=TestId, cardTrapInh=cardTrapInh, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmDevice=cmDevice, cardDetected=cardDetected, portDetectedMode=portDetectedMode, equipmentDiag=equipmentDiag, diskVolumeCardId=diskVolumeCardId, cardCPLDVersion=cardCPLDVersion, cardSpareGroupMode=cardSpareGroupMode, diskVolumeUsageNotification=diskVolumeUsageNotification, cardDetectedChange=cardDetectedChange, equipmentDiagGroup=equipmentDiagGroup, auditPassedCount=auditPassedCount, portNumChans=portNumChans, TestCommand=TestCommand, auditStatus=auditStatus, cardTestTime=cardTestTime, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicDetected=cardPicDetected, portNumber=portNumber, trapCounter=trapCounter, auditTotalPassedCount=auditTotalPassedCount, portDescription=portDescription, cardNumber=cardNumber, cardId=cardId, cmResetNotification=cmResetNotification, cardRamSize=cardRamSize, systemClock=systemClock, downstreamPowerLoss=downstreamPowerLoss, equipmentCardGroup=equipmentCardGroup, shelfSwVersion=shelfSwVersion, cardSecStateChange=cardSecStateChange, diskVolumeGroup=diskVolumeGroup, portCardSubType=portCardSubType, cardTestId=cardTestId, cardTempNoReportNotification=cardTempNoReportNotification, equipmentTraps=equipmentTraps, equipmentAuditGroup=equipmentAuditGroup, cardFwVersion=cardFwVersion, cmResetStatus=cmResetStatus, PYSNMP_MODULE_ID=cadEquipmentMib, trapSeverity=trapSeverity, errEvLogLevel=errEvLogLevel, auditDescription=auditDescription, diskVolumeShelfId=diskVolumeShelfId, cardTestTransId=cardTestTransId, auditCycleCount=auditCycleCount, portMacIfIndex=portMacIfIndex, portMinDsPower=portMinDsPower, cmIpAddress=cmIpAddress, cardTestResultDesc=cardTestResultDesc, cmResetUpTime=cmResetUpTime, cardLastBootSource=cardLastBootSource, cardBootVersion=cardBootVersion, cardTestType=cardTestType, cardName=cardName, cardShelfTestIndex=cardShelfTestIndex, auditId=auditId, cardSerialNum=cardSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeSize=diskVolumeSize, errEvRowStatus=errEvRowStatus, diskVolumeTable=diskVolumeTable, cardCarrierHwVersion=cardCarrierHwVersion, cardCpuType=cardCpuType, auditShelfId=auditShelfId, cardTestEntry=cardTestEntry, auditAutoScheduling=auditAutoScheduling, equipmentPortGroup=equipmentPortGroup, cardPrState=cardPrState, portPrStateChange=portPrStateChange, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeDriveId=diskVolumeDriveId, cardTestIndex=cardTestIndex, cardType=cardType, cardTemperature=cardTemperature, systemGeneralGroup=systemGeneralGroup, portMode=portMode, cardTestDescription=cardTestDescription, cardUpTime=cardUpTime, cardNumPorts=cardNumPorts, portMacAddress=portMacAddress, portRxFlowControlMode=portRxFlowControlMode, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeNumber=diskVolumeNumber, TestTransactionId=TestTransactionId, portEntry=portEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification)
