...
DuplexStatus, CardType, PortType, SecondaryState, ShelfId, CardSubType, PortId, UnknownState, PortMode, DiskVolumeUsageLevel, EqActionType, FirmwareSource, PortDetectedMode, CardId, PrimaryState, FlowControlMode, AdminState, PicType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "CardType", "PortType", "SecondaryState", "ShelfId", "CardSubType", "PortId", "UnknownState", "PortMode", "DiskVolumeUsageLevel", "EqActionType", "FirmwareSource", "PortDetectedMode", "CardId", "PrimaryState", "FlowControlMode", "AdminState", "PicType")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditName=auditName, auditCardId=auditCardId, portLastChangeTime=portLastChangeTime, cardFwUpdateStatus=cardFwUpdateStatus, PYSNMP_MODULE_ID=cadEquipmentMib, portMacAddress=portMacAddress, equipmentDiagGroup=equipmentDiagGroup, trapSeverity=trapSeverity, cardTrapInh=cardTrapInh, remainInDiagMode=remainInDiagMode, cadEquipmentMibConformance=cadEquipmentMibConformance, portAdminState=portAdminState, auditCommand=auditCommand, cardSwVersion=cardSwVersion, portNumber=portNumber, cardAdminState=cardAdminState, portMaxDsPower=portMaxDsPower, cardAction=cardAction, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetNotification=cmResetNotification, diskVolumeShelfId=diskVolumeShelfId, auditTotalFailedCount=auditTotalFailedCount, cardTemperatureHighWarn=cardTemperatureHighWarn, TestType=TestType, auditResult=auditResult, downstreamPowerLoss=downstreamPowerLoss, cardNumber=cardNumber, cardLastResetReason=cardLastResetReason, diskVolumeUsageLevel=diskVolumeUsageLevel, TestResult=TestResult, cmIpAddress=cmIpAddress, cardSpareGroupMode=cardSpareGroupMode, cardLastBootVersion=cardLastBootVersion, diskVolumeFileName=diskVolumeFileName, errEvLogLevel=errEvLogLevel, cardShelfId=cardShelfId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDplxStatus=portDplxStatus, systemClock=systemClock, cardId=cardId, cardPicSerialNum=cardPicSerialNum, auditPassedCount=auditPassedCount, cardTestCommand=cardTestCommand, cardSpareGroupId=cardSpareGroupId, cardTestTime=cardTestTime, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetClearNotification=cmResetClearNotification, diskDriveNumber=diskDriveNumber, auditDescription=auditDescription, cmDevice=cmDevice, auditTime=auditTime, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemGeneralGroup=systemGeneralGroup, systemGeneral=systemGeneral, cardTestSchedule=cardTestSchedule, cardTestResultDesc=cardTestResultDesc, cardFwVersion=cardFwVersion, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardLastChangeTime=cardLastChangeTime, diskVolumeId=diskVolumeId, equipmentTbl=equipmentTbl, cardPicHwVersion=cardPicHwVersion, portTxFlowControlMode=portTxFlowControlMode, diskVolumeDriveId=diskVolumeDriveId, auditAutoScheduling=auditAutoScheduling, diskVolumeUsagePercentage=diskVolumeUsagePercentage, shelfSwVersion=shelfSwVersion, auditLogOutput=auditLogOutput, cardCpuSpeed=cardCpuSpeed, cardName=cardName, cardTemperature=cardTemperature, diskVolumeSize=diskVolumeSize, equipmentStateGroup=equipmentStateGroup, equipmentShelfGroup=equipmentShelfGroup, auditId=auditId, auditShelfId=auditShelfId, eqDiagConfig=eqDiagConfig, cardTestTransId=cardTestTransId, equipmentError=equipmentError, cardTestIndex=cardTestIndex, verboseLevel=verboseLevel, portType=portType, portTable=portTable, diskVolumeDiskSize=diskVolumeDiskSize, portEntry=portEntry, auditStatus=auditStatus, cmVendor=cmVendor, fpgaErrorEventGroup=fpgaErrorEventGroup, errEventId=errEventId, portCardSubType=portCardSubType, auditLogThrottle=auditLogThrottle, cardPrStateChange=cardPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCPLDVersion=cardCPLDVersion, cardFlashSize=cardFlashSize, diskVolumeTable=diskVolumeTable, cmUChannelID=cmUChannelID, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMib=cadEquipmentMib, cardBusSpeed=cardBusSpeed, cardHwVersion=cardHwVersion, shelfName=shelfName, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentAudit=equipmentAudit, diskVolumeTrapInh=diskVolumeTrapInh, TestScheduleCommand=TestScheduleCommand, portMode=portMode, cardDplxStatus=cardDplxStatus, errEvRowStatus=errEvRowStatus, cardDetectedChange=cardDetectedChange, cardSecStateChange=cardSecStateChange, cardDetected=cardDetected, cmResetUpTime=cmResetUpTime, cardTable=cardTable, trapCounter=trapCounter, portConnectorId=portConnectorId, TestId=TestId, cardLastBootSource=cardLastBootSource, TestTransactionId=TestTransactionId, cardTestResultNotification=cardTestResultNotification, cmPrimarySID=cmPrimarySID, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentState=equipmentState, portPrState=portPrState, diskVolumeUsageNotification=diskVolumeUsageNotification, diagTestId=diagTestId, portDetectedMode=portDetectedMode, portSecStateChange=portSecStateChange, portAction=portAction, portMacIfIndex=portMacIfIndex, cardCpuType=cardCpuType, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResult=cardTestResult, shelfNumber=shelfNumber, diskVolumeCardId=diskVolumeCardId, cardTestScheduleCommand=cardTestScheduleCommand, portDocsIfIndex=portDocsIfIndex, cardType=cardType, cardSubDetected=cardSubDetected, cardDplxStatusChange=cardDplxStatusChange, systemKey=systemKey, TestSchedule=TestSchedule, cardUpTime=cardUpTime, equipmentPortGroup=equipmentPortGroup, diskVolumeGroup=diskVolumeGroup, shelfObjects=shelfObjects, cardPrState=cardPrState, consoleOutput=consoleOutput, cardBootVersion=cardBootVersion, diskVolumeNumber=diskVolumeNumber, portNumChans=portNumChans, cardShelfTestIndex=cardShelfTestIndex, portSecState=portSecState, equipmentCardGroup=equipmentCardGroup, auditFailedCount=auditFailedCount, portId=portId, diskVolumeName=diskVolumeName, equipmentTraps=equipmentTraps, TestCommand=TestCommand, cmMacAddress=cmMacAddress, portRxFlowControlMode=portRxFlowControlMode, auditEntry=auditEntry, cardCarrierSerialNum=cardCarrierSerialNum, cardTestDescription=cardTestDescription, portCurrDsPower=portCurrDsPower, cardSubType=cardSubType, portShelfId=portShelfId, cardCarrierFwVersion=cardCarrierFwVersion, portCardId=portCardId, portRxFlowControlDetected=portRxFlowControlDetected, equipmentAuditGroup=equipmentAuditGroup, portTrapInh=portTrapInh, auditCycleCount=auditCycleCount, cardTestEntry=cardTestEntry, equipmentDiag=equipmentDiag, cmResetInfo=cmResetInfo, cmResetReason=cmResetReason, cardTempNoReportNotification=cardTempNoReportNotification, auditTable=auditTable, cadEquipmentMibGroup=cadEquipmentMibGroup, cardRamSize=cardRamSize, cardCarrierHwVersion=cardCarrierHwVersion, cardSerialNum=cardSerialNum, cardTestType=cardTestType, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDplxStatusChange=portDplxStatusChange, cardTestTable=cardTestTable, cardTestId=cardTestId, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSecState=cardSecState, cardEntry=cardEntry, cardPicDetected=cardPicDetected, cardTestName=cardTestName, fpgaErrorEventTable=fpgaErrorEventTable, portMinDsPower=portMinDsPower, cmResetStatus=cmResetStatus, errEvLoggingEnabled=errEvLoggingEnabled, portBgpId=portBgpId, cardFpgaSource=cardFpgaSource, portDescription=portDescription, equipmentCardTestGroup=equipmentCardTestGroup, portPrStateChange=portPrStateChange)
