...
PrimaryState, CardSubType, CardType, PortDetectedMode, AdminState, ShelfId, PortType, UnknownState, CardId, PicType, SecondaryState, DuplexStatus, EqActionType, PortId, PortMode, FlowControlMode, FirmwareSource, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "CardSubType", "CardType", "PortDetectedMode", "AdminState", "ShelfId", "PortType", "UnknownState", "CardId", "PicType", "SecondaryState", "DuplexStatus", "EqActionType", "PortId", "PortMode", "FlowControlMode", "FirmwareSource", "DiskVolumeUsageLevel")
...
MacAddress, TextualConvention, DisplayString, DateAndTime, TimeStamp, TruthValue, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TextualConvention", "DisplayString", "DateAndTime", "TimeStamp", "TruthValue", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFpgaSource=cardFpgaSource, diskVolumeId=diskVolumeId, portSecState=portSecState, cardFwUpdateStatus=cardFwUpdateStatus, portId=portId, cardPrState=cardPrState, portNumChans=portNumChans, portDocsIfIndex=portDocsIfIndex, portCardId=portCardId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTime=cardTestTime, diskVolumeNumber=diskVolumeNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMib=cadEquipmentMib, cardBusSpeed=cardBusSpeed, portDescription=portDescription, portEntry=portEntry, diskVolumeCardId=diskVolumeCardId, cardTempNoReportNotification=cardTempNoReportNotification, downstreamPowerLoss=downstreamPowerLoss, equipmentState=equipmentState, cardDplxStatusChange=cardDplxStatusChange, cmResetNotification=cmResetNotification, cmMacAddress=cmMacAddress, portMinDsPower=portMinDsPower, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeEntry=diskVolumeEntry, diskVolumeSize=diskVolumeSize, diskDriveNumber=diskDriveNumber, cardPicSerialNum=cardPicSerialNum, errEvLogLevel=errEvLogLevel, cardPicHwVersion=cardPicHwVersion, equipmentDiagGroup=equipmentDiagGroup, auditCardId=auditCardId, portAdminState=portAdminState, cardTestResultDesc=cardTestResultDesc, fpgaErrorEventTable=fpgaErrorEventTable, cardName=cardName, fpgaErrorEventGroup=fpgaErrorEventGroup, shelfNumber=shelfNumber, cmResetInfo=cmResetInfo, portLastChangeTime=portLastChangeTime, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditLogOutput=auditLogOutput, cardDetectedChange=cardDetectedChange, cardLastBootVersion=cardLastBootVersion, cardDetected=cardDetected, cardPrStateChange=cardPrStateChange, diskVolumeTable=diskVolumeTable, cardRamSize=cardRamSize, cardUpTime=cardUpTime, cardPicDetected=cardPicDetected, portPrState=portPrState, portType=portType, cardTemperatureHighError=cardTemperatureHighError, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeGroup=diskVolumeGroup, cardTestDescription=cardTestDescription, cardFwVersion=cardFwVersion, cardTestCommand=cardTestCommand, diskVolumeDriveId=diskVolumeDriveId, auditAutoScheduling=auditAutoScheduling, cardTestSchedule=cardTestSchedule, portPrStateChange=portPrStateChange, portSecStateChange=portSecStateChange, cardTrapInh=cardTrapInh, cardShelfId=cardShelfId, cardDplxStatus=cardDplxStatus, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeShelfId=diskVolumeShelfId, portRxFlowControlMode=portRxFlowControlMode, cardLastResetReason=cardLastResetReason, portTrapInh=portTrapInh, cardAdminState=cardAdminState, errEvRowStatus=errEvRowStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCPLDVersion=cardCPLDVersion, equipmentTbl=equipmentTbl, cardType=cardType, cardTestType=cardTestType, cardCarrierHwVersion=cardCarrierHwVersion, cmResetClearNotification=cmResetClearNotification, cardSwVersion=cardSwVersion, cardCpuSpeed=cardCpuSpeed, portCardSubType=portCardSubType, TestType=TestType, cardFlashSize=cardFlashSize, cardTestIndex=cardTestIndex, shelfSwVersion=shelfSwVersion, systemClock=systemClock, shelfName=shelfName, portDplxStatus=portDplxStatus, diskVolumeFileName=diskVolumeFileName, remainInDiagMode=remainInDiagMode, systemGeneralGroup=systemGeneralGroup, equipmentAuditGroup=equipmentAuditGroup, portMaxDsPower=portMaxDsPower, diskVolumeUsageNotification=diskVolumeUsageNotification, TestTransactionId=TestTransactionId, cardTestName=cardTestName, cardTestResult=cardTestResult, TestId=TestId, auditTime=auditTime, portNumber=portNumber, cardTable=cardTable, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTrapInh=diskVolumeTrapInh, diagTestId=diagTestId, equipmentDiag=equipmentDiag, cardTestTransId=cardTestTransId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portMacIfIndex=portMacIfIndex, auditName=auditName, equipmentTraps=equipmentTraps, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeName=diskVolumeName, TestResult=TestResult, cmUChannelID=cmUChannelID, cmDevice=cmDevice, equipmentShelfGroup=equipmentShelfGroup, auditId=auditId, equipmentStateGroup=equipmentStateGroup, cardEntry=cardEntry, TestSchedule=TestSchedule, TestCommand=TestCommand, cardTestScheduleCommand=cardTestScheduleCommand, equipmentAudit=equipmentAudit, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentCardGroup=equipmentCardGroup, cardBootVersion=cardBootVersion, portMacAddress=portMacAddress, portShelfId=portShelfId, cardShelfTestIndex=cardShelfTestIndex, cardSubType=cardSubType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTotalFailedCount=auditTotalFailedCount, auditCycleCount=auditCycleCount, portDplxStatusChange=portDplxStatusChange, cardNumber=cardNumber, portDetectedMode=portDetectedMode, auditTable=auditTable, errEventId=errEventId, eqDiagConfig=eqDiagConfig, auditDescription=auditDescription, portBgpId=portBgpId, cardSpareGroupMode=cardSpareGroupMode, portTable=portTable, auditShelfId=auditShelfId, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestTable=cardTestTable, auditCommand=auditCommand, cmVendor=cmVendor, portMode=portMode, cmIpAddress=cmIpAddress, systemGeneral=systemGeneral, portTxFlowControlDetected=portTxFlowControlDetected, cmRegistrationNotification=cmRegistrationNotification, cardTestResultNotification=cardTestResultNotification, cardTestEntry=cardTestEntry, diskVolumeDiskSize=diskVolumeDiskSize, portCurrDsPower=portCurrDsPower, TestScheduleCommand=TestScheduleCommand, cardLastChangeTime=cardLastChangeTime, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardNumPorts=cardNumPorts, cardCpuType=cardCpuType, consoleOutput=consoleOutput, auditEntry=auditEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, verboseLevel=verboseLevel, trapSeverity=trapSeverity, cardSecStateChange=cardSecStateChange, auditPassedCount=auditPassedCount, cmResetStatus=cmResetStatus, cardId=cardId, auditResult=auditResult, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardAction=cardAction, cmResetReason=cmResetReason, auditLogThrottle=auditLogThrottle, auditFailedCount=auditFailedCount, auditTotalPassedCount=auditTotalPassedCount, cardSerialNum=cardSerialNum, cardLastBootSource=cardLastBootSource, cardSubDetected=cardSubDetected, equipmentError=equipmentError, cardTemperature=cardTemperature, portRxFlowControlDetected=portRxFlowControlDetected, shelfObjects=shelfObjects, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portAction=portAction, cardHwVersion=cardHwVersion, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCarrierFwVersion=cardCarrierFwVersion, cardTestId=cardTestId, cardSpareGroupId=cardSpareGroupId, cardTempOverHeatNotification=cardTempOverHeatNotification, trapCounter=trapCounter, systemKey=systemKey, cardSecState=cardSecState, auditStatus=auditStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, cmPrimarySID=cmPrimarySID, portConnectorId=portConnectorId, cmResetUpTime=cmResetUpTime)
