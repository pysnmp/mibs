...
DiskVolumeUsageLevel, FlowControlMode, PortDetectedMode, PortType, CardType, PrimaryState, PortMode, DuplexStatus, SecondaryState, UnknownState, CardId, PortId, EqActionType, CardSubType, AdminState, PicType, FirmwareSource, ShelfId = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "FlowControlMode", "PortDetectedMode", "PortType", "CardType", "PrimaryState", "PortMode", "DuplexStatus", "SecondaryState", "UnknownState", "CardId", "PortId", "EqActionType", "CardSubType", "AdminState", "PicType", "FirmwareSource", "ShelfId")
...
TextualConvention, DisplayString, TruthValue, TimeStamp, MacAddress, DateAndTime, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "TruthValue", "TimeStamp", "MacAddress", "DateAndTime", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestScheduleCommand=cardTestScheduleCommand, portShelfId=portShelfId, cardCPLDVersion=cardCPLDVersion, equipmentPortGroup=equipmentPortGroup, cardAdminState=cardAdminState, portPrState=portPrState, cardTrapInh=cardTrapInh, portAction=portAction, cardTestTransId=cardTestTransId, equipmentDiag=equipmentDiag, portPrStateChange=portPrStateChange, equipmentError=equipmentError, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeDiskSize=diskVolumeDiskSize, cardEntry=cardEntry, auditTime=auditTime, portAdminState=portAdminState, portRxFlowControlMode=portRxFlowControlMode, equipmentTraps=equipmentTraps, diskVolumeNumber=diskVolumeNumber, portLastChangeTime=portLastChangeTime, cardId=cardId, auditAutoScheduling=auditAutoScheduling, auditCycleCount=auditCycleCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResultNotification=cardTestResultNotification, portCardId=portCardId, cardAction=cardAction, cmResetClearNotification=cmResetClearNotification, diskVolumeCardId=diskVolumeCardId, equipmentCardGroup=equipmentCardGroup, cardTemperatureHighError=cardTemperatureHighError, auditTotalPassedCount=auditTotalPassedCount, cardLastBootVersion=cardLastBootVersion, cardBusSpeed=cardBusSpeed, cardRamSize=cardRamSize, errEvRecoveryEnabled=errEvRecoveryEnabled, portId=portId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestSchedule=cardTestSchedule, cardTestResultDesc=cardTestResultDesc, portTxFlowControlMode=portTxFlowControlMode, errEvLoggingEnabled=errEvLoggingEnabled, cmResetReason=cmResetReason, cardFpgaSource=cardFpgaSource, diskVolumeUsagePercentage=diskVolumeUsagePercentage, verboseLevel=verboseLevel, diskVolumeUsageNotification=diskVolumeUsageNotification, portDplxStatusChange=portDplxStatusChange, cardFlashSize=cardFlashSize, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeEntry=diskVolumeEntry, cmMacAddress=cmMacAddress, portNumber=portNumber, diskVolumeTable=diskVolumeTable, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentDiagGroup=equipmentDiagGroup, cardTestType=cardTestType, cardLastChangeTime=cardLastChangeTime, auditTable=auditTable, shelfName=shelfName, cmPrimarySID=cmPrimarySID, cardDplxStatusChange=cardDplxStatusChange, portEntry=portEntry, portMode=portMode, TestTransactionId=TestTransactionId, auditTotalFailedCount=auditTotalFailedCount, cmIpAddress=cmIpAddress, cardBootVersion=cardBootVersion, diskVolumeDriveId=diskVolumeDriveId, fpgaErrorEventEntry=fpgaErrorEventEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cardSerialNum=cardSerialNum, portMinDsPower=portMinDsPower, shelfObjects=shelfObjects, cardUpTime=cardUpTime, cardCarrierHwVersion=cardCarrierHwVersion, cardPicDetected=cardPicDetected, equipmentShelfGroup=equipmentShelfGroup, systemGeneralGroup=systemGeneralGroup, cardCarrierSerialNum=cardCarrierSerialNum, equipmentTbl=equipmentTbl, cardHwVersion=cardHwVersion, auditEntry=auditEntry, cardTable=cardTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestCommand=TestCommand, cardName=cardName, cardSubType=cardSubType, cmResetStatus=cmResetStatus, cardDetected=cardDetected, portMacIfIndex=portMacIfIndex, cardTestName=cardTestName, diskVolumeShelfId=diskVolumeShelfId, auditLogOutput=auditLogOutput, diskVolumeName=diskVolumeName, cmResetUpTime=cmResetUpTime, cardType=cardType, cmDevice=cmDevice, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diagTestId=diagTestId, trapSeverity=trapSeverity, portMaxDsPower=portMaxDsPower, portDetectedMode=portDetectedMode, diskVolumeGroup=diskVolumeGroup, portBgpId=portBgpId, downstreamPowerLoss=downstreamPowerLoss, portType=portType, portConnectorId=portConnectorId, cardPicSerialNum=cardPicSerialNum, cardSpareGroupId=cardSpareGroupId, auditStatus=auditStatus, cardPicHwVersion=cardPicHwVersion, cardFwVersion=cardFwVersion, cardShelfTestIndex=cardShelfTestIndex, diskVolumeSize=diskVolumeSize, systemGeneral=systemGeneral, remainInDiagMode=remainInDiagMode, auditName=auditName, cmVendor=cmVendor, TestType=TestType, cmUChannelID=cmUChannelID, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTempNoReportNotification=cardTempNoReportNotification, auditFailedCount=auditFailedCount, auditLogThrottle=auditLogThrottle, portRxFlowControlDetected=portRxFlowControlDetected, consoleOutput=consoleOutput, diskVolumeFileName=diskVolumeFileName, cardLastBootSource=cardLastBootSource, portSecStateChange=portSecStateChange, cadEquipmentMib=cadEquipmentMib, cardTestDescription=cardTestDescription, cardSwVersion=cardSwVersion, trapCounter=trapCounter, cardNumPorts=cardNumPorts, cardFwUpdateStatus=cardFwUpdateStatus, portDocsIfIndex=portDocsIfIndex, cardLastResetReason=cardLastResetReason, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, fpgaErrorEventTable=fpgaErrorEventTable, errEvLogLevel=errEvLogLevel, cardDetectedChange=cardDetectedChange, portTable=portTable, auditId=auditId, cmResetInfo=cmResetInfo, shelfNumber=shelfNumber, cardPrState=cardPrState, diskDriveNumber=diskDriveNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetNotification=cmResetNotification, auditShelfId=auditShelfId, auditDescription=auditDescription, cardSpareGroupMode=cardSpareGroupMode, equipmentCardTestGroup=equipmentCardTestGroup, portDescription=portDescription, cardTestIndex=cardTestIndex, TestScheduleCommand=TestScheduleCommand, TestId=TestId, cardDplxStatus=cardDplxStatus, TestSchedule=TestSchedule, cardSubDetected=cardSubDetected, equipmentAudit=equipmentAudit, cardPrStateChange=cardPrStateChange, portDplxStatus=portDplxStatus, eqDiagConfig=eqDiagConfig, cardTestTable=cardTestTable, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditCommand=auditCommand, equipmentAuditGroup=equipmentAuditGroup, TestResult=TestResult, cardSecStateChange=cardSecStateChange, portCardSubType=portCardSubType, systemKey=systemKey, portNumChans=portNumChans, portMacAddress=portMacAddress, portTrapInh=portTrapInh, equipmentState=equipmentState, auditCardId=auditCardId, cardTestResult=cardTestResult, diskVolumeId=diskVolumeId, portCurrDsPower=portCurrDsPower, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestId=cardTestId, cardNumber=cardNumber, auditPassedCount=auditPassedCount, cmRegistrationNotification=cmRegistrationNotification, cardTestCommand=cardTestCommand, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfSwVersion=shelfSwVersion, errEvRowStatus=errEvRowStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, systemClock=systemClock, cardCpuType=cardCpuType, errEventId=errEventId, portSecState=portSecState, portTxFlowControlDetected=portTxFlowControlDetected, cardTestTime=cardTestTime, cardTestEntry=cardTestEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditResult=auditResult, cardShelfId=cardShelfId, cardSecState=cardSecState, cardCpuSpeed=cardCpuSpeed, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentStateGroup=equipmentStateGroup, cardTemperature=cardTemperature)
