...
PicType, PortType, DiskVolumeUsageLevel, AdminState, EqActionType, PortMode, SecondaryState, CardId, UnknownState, CardSubType, FlowControlMode, PrimaryState, FirmwareSource, DuplexStatus, CardType, PortDetectedMode, ShelfId, PortId = mibBuilder.importSymbols("CADANT-TC", "PicType", "PortType", "DiskVolumeUsageLevel", "AdminState", "EqActionType", "PortMode", "SecondaryState", "CardId", "UnknownState", "CardSubType", "FlowControlMode", "PrimaryState", "FirmwareSource", "DuplexStatus", "CardType", "PortDetectedMode", "ShelfId", "PortId")
...
RowStatus, DisplayString, TextualConvention, TimeStamp, DateAndTime, TruthValue, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "TextualConvention", "TimeStamp", "DateAndTime", "TruthValue", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portDplxStatusChange=portDplxStatusChange, equipmentTbl=equipmentTbl, portTxFlowControlDetected=portTxFlowControlDetected, cmResetReason=cmResetReason, portNumber=portNumber, auditPassedCount=auditPassedCount, cardLastChangeTime=cardLastChangeTime, portDescription=portDescription, equipmentCardGroup=equipmentCardGroup, PYSNMP_MODULE_ID=cadEquipmentMib, downstreamPowerLoss=downstreamPowerLoss, diskVolumeTable=diskVolumeTable, cardFwVersion=cardFwVersion, cardTestName=cardTestName, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmIpAddress=cmIpAddress, cardCarrierFwVersion=cardCarrierFwVersion, TestScheduleCommand=TestScheduleCommand, auditId=auditId, cardAdminState=cardAdminState, cardTestSchedule=cardTestSchedule, cmResetStatus=cmResetStatus, TestType=TestType, cardAction=cardAction, auditName=auditName, cardCpuType=cardCpuType, cardTestResultNotification=cardTestResultNotification, portDetectedMode=portDetectedMode, errEvRecoveryEnabled=errEvRecoveryEnabled, TestSchedule=TestSchedule, cardBootVersion=cardBootVersion, cardName=cardName, errEvRowStatus=errEvRowStatus, cardTestCommand=cardTestCommand, cardSecState=cardSecState, TestResult=TestResult, cardTestDescription=cardTestDescription, diskVolumeShelfId=diskVolumeShelfId, equipmentAudit=equipmentAudit, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDplxStatus=portDplxStatus, portMode=portMode, cardTestType=cardTestType, cmVendor=cmVendor, equipmentPortGroup=equipmentPortGroup, systemGeneral=systemGeneral, diskVolumeNumber=diskVolumeNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentStateGroup=equipmentStateGroup, shelfNumber=shelfNumber, diskVolumeDriveId=diskVolumeDriveId, portLastChangeTime=portLastChangeTime, auditTotalPassedCount=auditTotalPassedCount, portCardSubType=portCardSubType, cmDevice=cmDevice, cardTemperature=cardTemperature, portMaxDsPower=portMaxDsPower, auditLogThrottle=auditLogThrottle, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSecStateChange=cardSecStateChange, cardNumber=cardNumber, TestId=TestId, cardSpareGroupId=cardSpareGroupId, diskVolumeName=diskVolumeName, trapSeverity=trapSeverity, portPrState=portPrState, cmResetInfo=cmResetInfo, cmRegistrationNotification=cmRegistrationNotification, cardShelfId=cardShelfId, cardPrStateChange=cardPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portConnectorId=portConnectorId, TestTransactionId=TestTransactionId, auditCycleCount=auditCycleCount, cadEquipmentMibConformance=cadEquipmentMibConformance, cardType=cardType, portMinDsPower=portMinDsPower, cardTestId=cardTestId, cardTemperatureHighWarn=cardTemperatureHighWarn, auditEntry=auditEntry, auditLogOutput=auditLogOutput, cardFwUpdateStatus=cardFwUpdateStatus, cardTestEntry=cardTestEntry, portMacIfIndex=portMacIfIndex, portSecStateChange=portSecStateChange, diskVolumeFileName=diskVolumeFileName, portId=portId, cardHwVersion=cardHwVersion, cardTestScheduleCommand=cardTestScheduleCommand, shelfObjects=shelfObjects, cardLastBootSource=cardLastBootSource, cardLastBootVersion=cardLastBootVersion, cardLastResetReason=cardLastResetReason, cardPicHwVersion=cardPicHwVersion, cardTempNoReportNotification=cardTempNoReportNotification, portAction=portAction, eqDiagConfig=eqDiagConfig, portTrapInh=portTrapInh, remainInDiagMode=remainInDiagMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeDiskSize=diskVolumeDiskSize, TestCommand=TestCommand, auditTable=auditTable, equipmentError=equipmentError, cardSubType=cardSubType, portRxFlowControlMode=portRxFlowControlMode, diskVolumeGroup=diskVolumeGroup, portSecState=portSecState, cardUpTime=cardUpTime, cardTrapInh=cardTrapInh, consoleOutput=consoleOutput, cmResetClearNotification=cmResetClearNotification, systemGeneralGroup=systemGeneralGroup, portType=portType, equipmentDiagGroup=equipmentDiagGroup, portCardId=portCardId, equipmentAuditGroup=equipmentAuditGroup, cardPicSerialNum=cardPicSerialNum, portDocsIfIndex=portDocsIfIndex, diskVolumeSize=diskVolumeSize, portAdminState=portAdminState, cardSwVersion=cardSwVersion, portBgpId=portBgpId, portPrStateChange=portPrStateChange, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeEntry=diskVolumeEntry, diskVolumeCardId=diskVolumeCardId, cardTemperatureHighError=cardTemperatureHighError, diskDriveNumber=diskDriveNumber, cardShelfTestIndex=cardShelfTestIndex, shelfSwVersion=shelfSwVersion, portTxFlowControlMode=portTxFlowControlMode, equipmentState=equipmentState, diskVolumeTrapInh=diskVolumeTrapInh, cmMacAddress=cmMacAddress, cardSpareGroupMode=cardSpareGroupMode, fpgaErrorEventGroup=fpgaErrorEventGroup, portEntry=portEntry, cardDplxStatus=cardDplxStatus, equipmentCardTestGroup=equipmentCardTestGroup, portCurrDsPower=portCurrDsPower, cardPicDetected=cardPicDetected, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardBusSpeed=cardBusSpeed, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageLevel=diskVolumeUsageLevel, trapCounter=trapCounter, cardFpgaSource=cardFpgaSource, cardCarrierHwVersion=cardCarrierHwVersion, auditStatus=auditStatus, portNumChans=portNumChans, cmResetNotification=cmResetNotification, errEvLogLevel=errEvLogLevel, cmPrimarySID=cmPrimarySID, verboseLevel=verboseLevel, equipmentDiag=equipmentDiag, portShelfId=portShelfId, cadEquipmentMib=cadEquipmentMib, equipmentShelfGroup=equipmentShelfGroup, cardId=cardId, auditFailedCount=auditFailedCount, cardCarrierSerialNum=cardCarrierSerialNum, diagTestId=diagTestId, auditTotalFailedCount=auditTotalFailedCount, cardDplxStatusChange=cardDplxStatusChange, cmResetUpTime=cmResetUpTime, systemKey=systemKey, cardTable=cardTable, cardDetectedChange=cardDetectedChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, systemClock=systemClock, cardRamSize=cardRamSize, cardCpuSpeed=cardCpuSpeed, auditDescription=auditDescription, cardTestIndex=cardTestIndex, cardDetected=cardDetected, cardPrState=cardPrState, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTransId=cardTestTransId, errEventId=errEventId, cardTestTime=cardTestTime, cardNumPorts=cardNumPorts, auditShelfId=auditShelfId, cmUChannelID=cmUChannelID, portTable=portTable, cardTestTable=cardTestTable, auditTime=auditTime, equipmentTraps=equipmentTraps, auditCommand=auditCommand, shelfName=shelfName, auditResult=auditResult, auditCardId=auditCardId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeId=diskVolumeId, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSerialNum=cardSerialNum, portMacAddress=portMacAddress, cardCPLDVersion=cardCPLDVersion, cardSubDetected=cardSubDetected, cardTestResult=cardTestResult, cardEntry=cardEntry, errEvLoggingEnabled=errEvLoggingEnabled, cardFlashSize=cardFlashSize)
