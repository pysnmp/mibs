...
PrimaryState, SecondaryState, ShelfId, CardType, CardId, EqActionType, PicType, AdminState, DuplexStatus, DiskVolumeUsageLevel, PortMode, PortDetectedMode, FlowControlMode, UnknownState, PortId, CardSubType, PortType, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "SecondaryState", "ShelfId", "CardType", "CardId", "EqActionType", "PicType", "AdminState", "DuplexStatus", "DiskVolumeUsageLevel", "PortMode", "PortDetectedMode", "FlowControlMode", "UnknownState", "PortId", "CardSubType", "PortType", "FirmwareSource")
...
DateAndTime, DisplayString, TimeStamp, RowStatus, MacAddress, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "DisplayString", "TimeStamp", "RowStatus", "MacAddress", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTotalFailedCount=auditTotalFailedCount, auditTotalPassedCount=auditTotalPassedCount, diskVolumeTrapInh=diskVolumeTrapInh, cmVendor=cmVendor, cardTrapInh=cardTrapInh, cardTestScheduleCommand=cardTestScheduleCommand, equipmentCardTestGroup=equipmentCardTestGroup, TestCommand=TestCommand, auditCommand=auditCommand, portTable=portTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetReason=cmResetReason, cardTestId=cardTestId, diskVolumeNumber=diskVolumeNumber, systemGeneral=systemGeneral, diskVolumeName=diskVolumeName, auditEntry=auditEntry, cardTestSchedule=cardTestSchedule, fpgaErrorEventTable=fpgaErrorEventTable, cardSecState=cardSecState, portAction=portAction, errEvLoggingEnabled=errEvLoggingEnabled, cardTable=cardTable, cardBootVersion=cardBootVersion, downstreamPowerLoss=downstreamPowerLoss, cardPicHwVersion=cardPicHwVersion, auditResult=auditResult, portDocsIfIndex=portDocsIfIndex, cardTestEntry=cardTestEntry, cardRamSize=cardRamSize, cmUChannelID=cmUChannelID, portShelfId=portShelfId, cardTemperature=cardTemperature, equipmentError=equipmentError, auditCardId=auditCardId, auditTime=auditTime, diskVolumeTable=diskVolumeTable, shelfName=shelfName, portMacIfIndex=portMacIfIndex, cardDplxStatus=cardDplxStatus, trapCounter=trapCounter, cardTestResult=cardTestResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestScheduleCommand=TestScheduleCommand, portCurrDsPower=portCurrDsPower, diskVolumeEntry=diskVolumeEntry, portTrapInh=portTrapInh, cardTempOverHeatNotification=cardTempOverHeatNotification, cmIpAddress=cmIpAddress, portCardSubType=portCardSubType, cardLastChangeTime=cardLastChangeTime, auditStatus=auditStatus, portMinDsPower=portMinDsPower, cardSecStateChange=cardSecStateChange, cmResetInfo=cmResetInfo, equipmentCardGroup=equipmentCardGroup, cardBusSpeed=cardBusSpeed, cardUpTime=cardUpTime, equipmentPortGroup=equipmentPortGroup, auditCycleCount=auditCycleCount, portRxFlowControlDetected=portRxFlowControlDetected, cardShelfTestIndex=cardShelfTestIndex, equipmentAudit=equipmentAudit, cmPrimarySID=cmPrimarySID, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastResetReason=cardLastResetReason, diagTestId=diagTestId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTransId=cardTestTransId, cardCpuSpeed=cardCpuSpeed, cardTempNoReportNotification=cardTempNoReportNotification, cardTestDescription=cardTestDescription, auditLogOutput=auditLogOutput, cardType=cardType, cardTemperatureHighError=cardTemperatureHighError, auditLogThrottle=auditLogThrottle, TestId=TestId, cardNumPorts=cardNumPorts, portDescription=portDescription, cardTestTable=cardTestTable, cardHwVersion=cardHwVersion, diskVolumeId=diskVolumeId, cardTestCommand=cardTestCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, portPrState=portPrState, equipmentTraps=equipmentTraps, portNumChans=portNumChans, trapSeverity=trapSeverity, cardPrState=cardPrState, cardPicSerialNum=cardPicSerialNum, portMaxDsPower=portMaxDsPower, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditId=auditId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardId=cardId, cardPrStateChange=cardPrStateChange, cardCpuType=cardCpuType, portDetectedMode=portDetectedMode, equipmentAuditGroup=equipmentAuditGroup, auditDescription=auditDescription, cmRegistrationNotification=cmRegistrationNotification, equipmentShelfGroup=equipmentShelfGroup, cardCPLDVersion=cardCPLDVersion, TestSchedule=TestSchedule, portSecStateChange=portSecStateChange, cardAdminState=cardAdminState, portTxFlowControlMode=portTxFlowControlMode, shelfSwVersion=shelfSwVersion, portAdminState=portAdminState, errEvLogLevel=errEvLogLevel, cardTestResultDesc=cardTestResultDesc, portRxFlowControlMode=portRxFlowControlMode, diskVolumeCardId=diskVolumeCardId, auditName=auditName, portTxFlowControlDetected=portTxFlowControlDetected, equipmentState=equipmentState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, fpgaErrorEventEntry=fpgaErrorEventEntry, cardFpgaSource=cardFpgaSource, cmResetNotification=cmResetNotification, errEventId=errEventId, cardPicDetected=cardPicDetected, TestTransactionId=TestTransactionId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portNumber=portNumber, portConnectorId=portConnectorId, diskDriveNumber=diskDriveNumber, diskVolumeDriveId=diskVolumeDriveId, cmResetClearNotification=cmResetClearNotification, cardSerialNum=cardSerialNum, portDplxStatusChange=portDplxStatusChange, verboseLevel=verboseLevel, diskVolumeGroup=diskVolumeGroup, cardDetectedChange=cardDetectedChange, auditTable=auditTable, cmDevice=cmDevice, cardTestName=cardTestName, cardSpareGroupId=cardSpareGroupId, remainInDiagMode=remainInDiagMode, cadEquipmentMib=cadEquipmentMib, cardCarrierSerialNum=cardCarrierSerialNum, cardLastBootSource=cardLastBootSource, cmMacAddress=cmMacAddress, equipmentStateGroup=equipmentStateGroup, cardEntry=cardEntry, auditFailedCount=auditFailedCount, cardDplxStatusChange=cardDplxStatusChange, cardTestResultNotification=cardTestResultNotification, cardFwVersion=cardFwVersion, portSecState=portSecState, portBgpId=portBgpId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentDiag=equipmentDiag, equipmentDiagGroup=equipmentDiagGroup, diskVolumeDiskSize=diskVolumeDiskSize, systemKey=systemKey, cardSwVersion=cardSwVersion, portId=portId, portEntry=portEntry, shelfObjects=shelfObjects, portDplxStatus=portDplxStatus, portMacAddress=portMacAddress, cardName=cardName, systemClock=systemClock, auditAutoScheduling=auditAutoScheduling, cardLastBootVersion=cardLastBootVersion, auditPassedCount=auditPassedCount, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, systemGeneralGroup=systemGeneralGroup, cardAction=cardAction, PYSNMP_MODULE_ID=cadEquipmentMib, cardSubType=cardSubType, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFlashSize=cardFlashSize, cmResetStatus=cmResetStatus, cardFwUpdateStatus=cardFwUpdateStatus, cardDetected=cardDetected, portCardId=portCardId, cmResetUpTime=cmResetUpTime, cardNumber=cardNumber, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, auditShelfId=auditShelfId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeFileName=diskVolumeFileName, cardSubDetected=cardSubDetected, diskVolumeSize=diskVolumeSize, cardTestIndex=cardTestIndex, cardTestType=cardTestType, eqDiagConfig=eqDiagConfig, cardTestTime=cardTestTime, errEvRowStatus=errEvRowStatus, portPrStateChange=portPrStateChange, portMode=portMode, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestType=TestType, equipmentTbl=equipmentTbl, cardShelfId=cardShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSpareGroupMode=cardSpareGroupMode, shelfNumber=shelfNumber, portLastChangeTime=portLastChangeTime, consoleOutput=consoleOutput, TestResult=TestResult, portType=portType)
