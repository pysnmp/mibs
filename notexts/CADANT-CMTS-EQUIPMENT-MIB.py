...
PortId, SecondaryState, PortDetectedMode, PicType, ShelfId, UnknownState, DiskVolumeUsageLevel, EqActionType, FirmwareSource, CardType, DuplexStatus, FlowControlMode, CardSubType, PortMode, PortType, PrimaryState, CardId, AdminState = mibBuilder.importSymbols("CADANT-TC", "PortId", "SecondaryState", "PortDetectedMode", "PicType", "ShelfId", "UnknownState", "DiskVolumeUsageLevel", "EqActionType", "FirmwareSource", "CardType", "DuplexStatus", "FlowControlMode", "CardSubType", "PortMode", "PortType", "PrimaryState", "CardId", "AdminState")
...
DisplayString, TruthValue, TextualConvention, TimeStamp, MacAddress, RowStatus, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TruthValue", "TextualConvention", "TimeStamp", "MacAddress", "RowStatus", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", systemClock=systemClock, equipmentAudit=equipmentAudit, shelfSwVersion=shelfSwVersion, cmMacAddress=cmMacAddress, cmPrimarySID=cmPrimarySID, cardFwUpdateStatus=cardFwUpdateStatus, cardTestResult=cardTestResult, cardDplxStatus=cardDplxStatus, cardRamSize=cardRamSize, equipmentError=equipmentError, auditResult=auditResult, cardTempOverHeatNotification=cardTempOverHeatNotification, auditLogThrottle=auditLogThrottle, portDocsIfIndex=portDocsIfIndex, portId=portId, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMibCompliances=cadEquipmentMibCompliances, shelfNumber=shelfNumber, equipmentCardGroup=equipmentCardGroup, cardNumber=cardNumber, portRxFlowControlDetected=portRxFlowControlDetected, diagTestId=diagTestId, equipmentDiag=equipmentDiag, diskVolumeId=diskVolumeId, cardUpTime=cardUpTime, fpgaErrorEventTable=fpgaErrorEventTable, portType=portType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portPrState=portPrState, TestId=TestId, eqDiagConfig=eqDiagConfig, auditTotalFailedCount=auditTotalFailedCount, equipmentTraps=equipmentTraps, auditName=auditName, auditTotalPassedCount=auditTotalPassedCount, cardPrStateChange=cardPrStateChange, equipmentPortGroup=equipmentPortGroup, cardTestCommand=cardTestCommand, cardTestTime=cardTestTime, TestScheduleCommand=TestScheduleCommand, diskVolumeFileName=diskVolumeFileName, portAdminState=portAdminState, equipmentStateGroup=equipmentStateGroup, cardPicSerialNum=cardPicSerialNum, cardEntry=cardEntry, cmResetReason=cmResetReason, cardCpuType=cardCpuType, cadEquipmentMibConformance=cadEquipmentMibConformance, portMode=portMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestType=cardTestType, diskVolumeName=diskVolumeName, portEntry=portEntry, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditDescription=auditDescription, TestCommand=TestCommand, systemGeneralGroup=systemGeneralGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portSecState=portSecState, diskVolumeTable=diskVolumeTable, cardName=cardName, cadEquipmentMib=cadEquipmentMib, cardLastChangeTime=cardLastChangeTime, cardDetected=cardDetected, auditEntry=auditEntry, cardPicHwVersion=cardPicHwVersion, cardCarrierHwVersion=cardCarrierHwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, errEventId=errEventId, errEvRowStatus=errEvRowStatus, cardTable=cardTable, portTrapInh=portTrapInh, diskVolumeShelfId=diskVolumeShelfId, cardHwVersion=cardHwVersion, cardType=cardType, cmResetClearNotification=cmResetClearNotification, cardFpgaSource=cardFpgaSource, cmRegistrationNotification=cmRegistrationNotification, cardPrState=cardPrState, cardTemperatureHighError=cardTemperatureHighError, portTxFlowControlDetected=portTxFlowControlDetected, cardBusSpeed=cardBusSpeed, auditLogOutput=auditLogOutput, systemKey=systemKey, portCurrDsPower=portCurrDsPower, auditTime=auditTime, portDplxStatus=portDplxStatus, cardShelfId=cardShelfId, cardSubDetected=cardSubDetected, portAction=portAction, cardSerialNum=cardSerialNum, cardFlashSize=cardFlashSize, auditShelfId=auditShelfId, cardFwVersion=cardFwVersion, shelfName=shelfName, portNumChans=portNumChans, portTable=portTable, portLastChangeTime=portLastChangeTime, cardTestId=cardTestId, portDetectedMode=portDetectedMode, portConnectorId=portConnectorId, cardTestResultDesc=cardTestResultDesc, auditCommand=auditCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, portMaxDsPower=portMaxDsPower, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestEntry=cardTestEntry, cmDevice=cmDevice, cmResetInfo=cmResetInfo, cardTestIndex=cardTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentTbl=equipmentTbl, cardSpareGroupId=cardSpareGroupId, errEvLogLevel=errEvLogLevel, TestSchedule=TestSchedule, equipmentAuditGroup=equipmentAuditGroup, cardDetectedChange=cardDetectedChange, cardPicDetected=cardPicDetected, portNumber=portNumber, remainInDiagMode=remainInDiagMode, portSecStateChange=portSecStateChange, auditAutoScheduling=auditAutoScheduling, cardSecStateChange=cardSecStateChange, portDplxStatusChange=portDplxStatusChange, cmIpAddress=cmIpAddress, cardAction=cardAction, portMacIfIndex=portMacIfIndex, cmVendor=cmVendor, trapCounter=trapCounter, cardId=cardId, cardAdminState=cardAdminState, cardDplxStatusChange=cardDplxStatusChange, cardBootVersion=cardBootVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSpareGroupMode=cardSpareGroupMode, portDescription=portDescription, cardTestTransId=cardTestTransId, downstreamPowerLoss=downstreamPowerLoss, cardTestScheduleCommand=cardTestScheduleCommand, cmUChannelID=cmUChannelID, auditPassedCount=auditPassedCount, cardNumPorts=cardNumPorts, cardTempNoReportNotification=cardTempNoReportNotification, portShelfId=portShelfId, diskVolumeNumber=diskVolumeNumber, cardSubType=cardSubType, cmResetNotification=cmResetNotification, cmResetStatus=cmResetStatus, auditId=auditId, cmResetUpTime=cmResetUpTime, cardTemperature=cardTemperature, TestResult=TestResult, cardTrapInh=cardTrapInh, cardSecState=cardSecState, equipmentCardTestGroup=equipmentCardTestGroup, portMinDsPower=portMinDsPower, portTxFlowControlMode=portTxFlowControlMode, portMacAddress=portMacAddress, portBgpId=portBgpId, cardTestSchedule=cardTestSchedule, auditTable=auditTable, cardLastResetReason=cardLastResetReason, cardCarrierSerialNum=cardCarrierSerialNum, auditStatus=auditStatus, equipmentDiagGroup=equipmentDiagGroup, cardShelfTestIndex=cardShelfTestIndex, errEvLoggingEnabled=errEvLoggingEnabled, cardTestDescription=cardTestDescription, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfObjects=shelfObjects, auditCycleCount=auditCycleCount, TestType=TestType, equipmentState=equipmentState, consoleOutput=consoleOutput, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestTable=cardTestTable, verboseLevel=verboseLevel, cardCPLDVersion=cardCPLDVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, portCardId=portCardId, diskVolumeSize=diskVolumeSize, diskDriveNumber=diskDriveNumber, cardTestName=cardTestName, diskVolumeDriveId=diskVolumeDriveId, diskVolumeEntry=diskVolumeEntry, diskVolumeCardId=diskVolumeCardId, portCardSubType=portCardSubType, cardCarrierFwVersion=cardCarrierFwVersion, trapSeverity=trapSeverity, auditFailedCount=auditFailedCount, auditCardId=auditCardId, diskVolumeGroup=diskVolumeGroup, cardSwVersion=cardSwVersion, TestTransactionId=TestTransactionId, cardLastBootVersion=cardLastBootVersion, portPrStateChange=portPrStateChange, cardCpuSpeed=cardCpuSpeed, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, systemGeneral=systemGeneral, cardLastBootSource=cardLastBootSource)
