...
AdminState, PicType, CardSubType, CardId, DuplexStatus, FirmwareSource, SecondaryState, UnknownState, PortMode, ShelfId, PortType, PortId, CardType, FlowControlMode, EqActionType, PortDetectedMode, PrimaryState, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PicType", "CardSubType", "CardId", "DuplexStatus", "FirmwareSource", "SecondaryState", "UnknownState", "PortMode", "ShelfId", "PortType", "PortId", "CardType", "FlowControlMode", "EqActionType", "PortDetectedMode", "PrimaryState", "DiskVolumeUsageLevel")
...
TruthValue, TimeStamp, DisplayString, TextualConvention, RowStatus, MacAddress, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TimeStamp", "DisplayString", "TextualConvention", "RowStatus", "MacAddress", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentPortGroup=equipmentPortGroup, diskVolumeCardId=diskVolumeCardId, cardCarrierSerialNum=cardCarrierSerialNum, cardTemperature=cardTemperature, cardLastResetReason=cardLastResetReason, cardUpTime=cardUpTime, cmVendor=cmVendor, cardPrStateChange=cardPrStateChange, portMacAddress=portMacAddress, cardTestEntry=cardTestEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardDplxStatus=cardDplxStatus, auditLogThrottle=auditLogThrottle, auditTotalPassedCount=auditTotalPassedCount, errEvLogLevel=errEvLogLevel, cardTestTable=cardTestTable, TestTransactionId=TestTransactionId, cardLastChangeTime=cardLastChangeTime, portMacIfIndex=portMacIfIndex, cardTestName=cardTestName, cardTestType=cardTestType, portBgpId=portBgpId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditLogOutput=auditLogOutput, cardTestResultNotification=cardTestResultNotification, TestResult=TestResult, portEntry=portEntry, portShelfId=portShelfId, diskVolumeEntry=diskVolumeEntry, cardTestScheduleCommand=cardTestScheduleCommand, cardLastBootSource=cardLastBootSource, cardTestResult=cardTestResult, diskVolumeSize=diskVolumeSize, cardFpgaSource=cardFpgaSource, cardFlashSize=cardFlashSize, diskVolumeId=diskVolumeId, diskVolumeUsageLevel=diskVolumeUsageLevel, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTable=cardTable, portPrState=portPrState, portDescription=portDescription, downstreamPowerLoss=downstreamPowerLoss, cardFwUpdateStatus=cardFwUpdateStatus, cardPicSerialNum=cardPicSerialNum, cmResetUpTime=cmResetUpTime, portMinDsPower=portMinDsPower, equipmentCardGroup=equipmentCardGroup, portTrapInh=portTrapInh, cardTestResultDesc=cardTestResultDesc, cardType=cardType, cardTempNoReportNotification=cardTempNoReportNotification, cardCpuSpeed=cardCpuSpeed, cardNumPorts=cardNumPorts, equipmentError=equipmentError, cardLastBootVersion=cardLastBootVersion, cmUChannelID=cmUChannelID, cardRamSize=cardRamSize, portAdminState=portAdminState, cardDplxStatusChange=cardDplxStatusChange, cardAction=cardAction, portAction=portAction, PYSNMP_MODULE_ID=cadEquipmentMib, cardHwVersion=cardHwVersion, remainInDiagMode=remainInDiagMode, cardSecStateChange=cardSecStateChange, cardCarrierHwVersion=cardCarrierHwVersion, TestSchedule=TestSchedule, verboseLevel=verboseLevel, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfObjects=shelfObjects, cardSubType=cardSubType, portDplxStatus=portDplxStatus, cmIpAddress=cmIpAddress, cardPicDetected=cardPicDetected, portNumChans=portNumChans, cardSubDetected=cardSubDetected, portTxFlowControlMode=portTxFlowControlMode, portDocsIfIndex=portDocsIfIndex, cardTestIndex=cardTestIndex, TestCommand=TestCommand, portConnectorId=portConnectorId, cardShelfTestIndex=cardShelfTestIndex, auditTime=auditTime, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDetectedChange=cardDetectedChange, cmResetClearNotification=cmResetClearNotification, equipmentTraps=equipmentTraps, cardPrState=cardPrState, auditTable=auditTable, portLastChangeTime=portLastChangeTime, cardSwVersion=cardSwVersion, portCardSubType=portCardSubType, auditAutoScheduling=auditAutoScheduling, cardTestTransId=cardTestTransId, cmPrimarySID=cmPrimarySID, portType=portType, errEvLoggingEnabled=errEvLoggingEnabled, auditPassedCount=auditPassedCount, systemKey=systemKey, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibGroup=cadEquipmentMibGroup, auditShelfId=auditShelfId, auditResult=auditResult, cardTestId=cardTestId, cardTemperatureHighError=cardTemperatureHighError, trapCounter=trapCounter, cmMacAddress=cmMacAddress, diagTestId=diagTestId, auditCardId=auditCardId, portSecStateChange=portSecStateChange, portCurrDsPower=portCurrDsPower, cmResetReason=cmResetReason, systemGeneral=systemGeneral, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMib=cadEquipmentMib, cardShelfId=cardShelfId, auditStatus=auditStatus, auditFailedCount=auditFailedCount, auditCommand=auditCommand, equipmentDiag=equipmentDiag, cardFwVersion=cardFwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, trapSeverity=trapSeverity, portRxFlowControlMode=portRxFlowControlMode, errEvRowStatus=errEvRowStatus, cardSerialNum=cardSerialNum, cardCpuType=cardCpuType, cardTemperatureHighWarn=cardTemperatureHighWarn, portMaxDsPower=portMaxDsPower, portId=portId, TestId=TestId, diskVolumeTrapInh=diskVolumeTrapInh, cardTestTime=cardTestTime, diskVolumeName=diskVolumeName, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentAudit=equipmentAudit, cardPicHwVersion=cardPicHwVersion, cmResetInfo=cmResetInfo, shelfNumber=shelfNumber, cardAdminState=cardAdminState, cardSecState=cardSecState, cardId=cardId, diskVolumeNumber=diskVolumeNumber, portRxFlowControlDetected=portRxFlowControlDetected, cardName=cardName, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestCommand=cardTestCommand, cmResetStatus=cmResetStatus, portCardId=portCardId, diskDriveNumber=diskDriveNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portTxFlowControlDetected=portTxFlowControlDetected, auditName=auditName, equipmentTbl=equipmentTbl, diskVolumeFileName=diskVolumeFileName, portTable=portTable, equipmentShelfGroup=equipmentShelfGroup, portSecState=portSecState, auditTotalFailedCount=auditTotalFailedCount, portDetectedMode=portDetectedMode, portMode=portMode, diskVolumeDriveId=diskVolumeDriveId, auditEntry=auditEntry, cardTestDescription=cardTestDescription, cardBusSpeed=cardBusSpeed, cardCarrierFwVersion=cardCarrierFwVersion, shelfSwVersion=shelfSwVersion, cardEntry=cardEntry, eqDiagConfig=eqDiagConfig, cardNumber=cardNumber, auditDescription=auditDescription, cardBootVersion=cardBootVersion, cmResetNotification=cmResetNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditCycleCount=auditCycleCount, equipmentDiagGroup=equipmentDiagGroup, TestType=TestType, portNumber=portNumber, diskVolumeTable=diskVolumeTable, diskVolumeDiskSize=diskVolumeDiskSize, cmDevice=cmDevice, cardCPLDVersion=cardCPLDVersion, auditId=auditId, diskVolumeUsageNotification=diskVolumeUsageNotification, errEventId=errEventId, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestSchedule=cardTestSchedule, equipmentCardTestGroup=equipmentCardTestGroup, equipmentState=equipmentState, cardSpareGroupId=cardSpareGroupId, portPrStateChange=portPrStateChange, cardTrapInh=cardTrapInh, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, consoleOutput=consoleOutput, shelfName=shelfName, equipmentStateGroup=equipmentStateGroup, equipmentAuditGroup=equipmentAuditGroup, diskVolumeShelfId=diskVolumeShelfId, systemClock=systemClock, TestScheduleCommand=TestScheduleCommand, diskVolumeGroup=diskVolumeGroup, cardSpareGroupMode=cardSpareGroupMode, cardDetected=cardDetected)
