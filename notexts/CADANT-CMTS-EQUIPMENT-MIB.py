...
PortMode, DuplexStatus, FirmwareSource, PortDetectedMode, PortId, PortType, PrimaryState, PicType, CardId, AdminState, UnknownState, CardSubType, ShelfId, SecondaryState, DiskVolumeUsageLevel, CardType, FlowControlMode, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PortMode", "DuplexStatus", "FirmwareSource", "PortDetectedMode", "PortId", "PortType", "PrimaryState", "PicType", "CardId", "AdminState", "UnknownState", "CardSubType", "ShelfId", "SecondaryState", "DiskVolumeUsageLevel", "CardType", "FlowControlMode", "EqActionType")
...
TimeStamp, TruthValue, MacAddress, DisplayString, DateAndTime, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "MacAddress", "DisplayString", "DateAndTime", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portType=portType, portAction=portAction, portMode=portMode, diskVolumeName=diskVolumeName, cardTestIndex=cardTestIndex, trapCounter=trapCounter, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, consoleOutput=consoleOutput, errEvRowStatus=errEvRowStatus, portCurrDsPower=portCurrDsPower, diskVolumeDiskSize=diskVolumeDiskSize, shelfNumber=shelfNumber, auditPassedCount=auditPassedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSerialNum=cardSerialNum, cardSecStateChange=cardSecStateChange, cardTestResult=cardTestResult, errEvLoggingEnabled=errEvLoggingEnabled, TestSchedule=TestSchedule, cardTempNoReportNotification=cardTempNoReportNotification, cardPrStateChange=cardPrStateChange, diskVolumeTrapInh=diskVolumeTrapInh, cardTestTime=cardTestTime, auditCycleCount=auditCycleCount, cardType=cardType, equipmentPortGroup=equipmentPortGroup, auditStatus=auditStatus, diskVolumeDriveId=diskVolumeDriveId, portDocsIfIndex=portDocsIfIndex, cardTemperatureHighError=cardTemperatureHighError, TestCommand=TestCommand, auditResult=auditResult, cardNumber=cardNumber, portMaxDsPower=portMaxDsPower, portDescription=portDescription, cardTrapInh=cardTrapInh, cardCarrierFwVersion=cardCarrierFwVersion, cardLastResetReason=cardLastResetReason, cardTable=cardTable, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardNumPorts=cardNumPorts, cardDetected=cardDetected, cardRamSize=cardRamSize, TestScheduleCommand=TestScheduleCommand, cardPicDetected=cardPicDetected, cardTestTable=cardTestTable, diskVolumeGroup=diskVolumeGroup, cardShelfTestIndex=cardShelfTestIndex, cardTestId=cardTestId, portTxFlowControlDetected=portTxFlowControlDetected, cardPicSerialNum=cardPicSerialNum, PYSNMP_MODULE_ID=cadEquipmentMib, portSecStateChange=portSecStateChange, portCardId=portCardId, diskVolumeEntry=diskVolumeEntry, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmDevice=cmDevice, cmResetClearNotification=cmResetClearNotification, cmResetReason=cmResetReason, errEventId=errEventId, cardShelfId=cardShelfId, diskVolumeCardId=diskVolumeCardId, portNumChans=portNumChans, cardCarrierHwVersion=cardCarrierHwVersion, auditLogOutput=auditLogOutput, cardHwVersion=cardHwVersion, equipmentDiag=equipmentDiag, cadEquipmentMibGroup=cadEquipmentMibGroup, cmUChannelID=cmUChannelID, systemClock=systemClock, cardTestScheduleCommand=cardTestScheduleCommand, portId=portId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, downstreamPowerLoss=downstreamPowerLoss, portTable=portTable, auditAutoScheduling=auditAutoScheduling, cardLastBootSource=cardLastBootSource, equipmentAudit=equipmentAudit, TestId=TestId, eqDiagConfig=eqDiagConfig, cardTestType=cardTestType, auditCommand=auditCommand, cardCarrierSerialNum=cardCarrierSerialNum, verboseLevel=verboseLevel, cardLastBootVersion=cardLastBootVersion, cardTestResultDesc=cardTestResultDesc, cardBootVersion=cardBootVersion, cardTestTransId=cardTestTransId, portNumber=portNumber, auditShelfId=auditShelfId, cardFwUpdateStatus=cardFwUpdateStatus, cardTemperature=cardTemperature, cmResetInfo=cmResetInfo, diskVolumeFileName=diskVolumeFileName, portRxFlowControlMode=portRxFlowControlMode, portDplxStatus=portDplxStatus, cardFwVersion=cardFwVersion, cardTestResultNotification=cardTestResultNotification, portAdminState=portAdminState, cardFpgaSource=cardFpgaSource, cardDetectedChange=cardDetectedChange, equipmentShelfGroup=equipmentShelfGroup, shelfObjects=shelfObjects, auditTime=auditTime, portPrState=portPrState, auditFailedCount=auditFailedCount, auditTotalFailedCount=auditTotalFailedCount, auditName=auditName, cardTempOverHeatNotification=cardTempOverHeatNotification, diskDriveNumber=diskDriveNumber, diskVolumeTable=diskVolumeTable, portBgpId=portBgpId, cardSecState=cardSecState, cardEntry=cardEntry, diskVolumeShelfId=diskVolumeShelfId, auditEntry=auditEntry, cmPrimarySID=cmPrimarySID, systemGeneralGroup=systemGeneralGroup, auditCardId=auditCardId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSpareGroupId=cardSpareGroupId, auditTable=auditTable, cmResetStatus=cmResetStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portLastChangeTime=portLastChangeTime, cardCpuType=cardCpuType, portPrStateChange=portPrStateChange, portSecState=portSecState, cardDplxStatus=cardDplxStatus, cmRegistrationNotification=cmRegistrationNotification, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMib=cadEquipmentMib, cardCpuSpeed=cardCpuSpeed, diagTestId=diagTestId, cardPrState=cardPrState, shelfName=shelfName, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTotalPassedCount=auditTotalPassedCount, equipmentState=equipmentState, cardBusSpeed=cardBusSpeed, trapSeverity=trapSeverity, cardPicHwVersion=cardPicHwVersion, equipmentAuditGroup=equipmentAuditGroup, cardTestEntry=cardTestEntry, TestResult=TestResult, TestTransactionId=TestTransactionId, diskVolumeNumber=diskVolumeNumber, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeSize=diskVolumeSize, systemGeneral=systemGeneral, portTrapInh=portTrapInh, portEntry=portEntry, equipmentError=equipmentError, auditDescription=auditDescription, diskVolumeUsageNotification=diskVolumeUsageNotification, cmMacAddress=cmMacAddress, cardAction=cardAction, portTxFlowControlMode=portTxFlowControlMode, cardTestName=cardTestName, cmVendor=cmVendor, cardFlashSize=cardFlashSize, cmIpAddress=cmIpAddress, cardLastChangeTime=cardLastChangeTime, portDetectedMode=portDetectedMode, auditId=auditId, shelfSwVersion=shelfSwVersion, cardSubDetected=cardSubDetected, cardCPLDVersion=cardCPLDVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, cardUpTime=cardUpTime, cmResetUpTime=cmResetUpTime, fpgaErrorEventTable=fpgaErrorEventTable, cardId=cardId, remainInDiagMode=remainInDiagMode, portRxFlowControlDetected=portRxFlowControlDetected, auditLogThrottle=auditLogThrottle, equipmentCardGroup=equipmentCardGroup, cardTestSchedule=cardTestSchedule, diskVolumeId=diskVolumeId, cardTestCommand=cardTestCommand, cardDplxStatusChange=cardDplxStatusChange, cardTestDescription=cardTestDescription, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestType=TestType, cardSubType=cardSubType, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentTraps=equipmentTraps, equipmentStateGroup=equipmentStateGroup, cardAdminState=cardAdminState, portCardSubType=portCardSubType, portMinDsPower=portMinDsPower, portShelfId=portShelfId, equipmentTbl=equipmentTbl, cardName=cardName, cardSpareGroupMode=cardSpareGroupMode, portMacAddress=portMacAddress, systemKey=systemKey, portConnectorId=portConnectorId, portMacIfIndex=portMacIfIndex, portDplxStatusChange=portDplxStatusChange, errEvLogLevel=errEvLogLevel, cardSwVersion=cardSwVersion, cmResetNotification=cmResetNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTemperatureHighWarn=cardTemperatureHighWarn)
