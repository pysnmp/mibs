...
ShelfId, PortId, PortMode, EqActionType, PicType, PortDetectedMode, SecondaryState, CardSubType, DiskVolumeUsageLevel, PortType, CardType, FlowControlMode, AdminState, DuplexStatus, CardId, PrimaryState, UnknownState, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "PortId", "PortMode", "EqActionType", "PicType", "PortDetectedMode", "SecondaryState", "CardSubType", "DiskVolumeUsageLevel", "PortType", "CardType", "FlowControlMode", "AdminState", "DuplexStatus", "CardId", "PrimaryState", "UnknownState", "FirmwareSource")
...
RowStatus, MacAddress, TruthValue, DisplayString, TimeStamp, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "MacAddress", "TruthValue", "DisplayString", "TimeStamp", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeEntry=diskVolumeEntry, shelfName=shelfName, cardEntry=cardEntry, cardTestType=cardTestType, cardFlashSize=cardFlashSize, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardUpTime=cardUpTime, cardTestSchedule=cardTestSchedule, cardLastBootSource=cardLastBootSource, cmResetNotification=cmResetNotification, cardCpuType=cardCpuType, verboseLevel=verboseLevel, equipmentError=equipmentError, cardRamSize=cardRamSize, portCardId=portCardId, auditCardId=auditCardId, cardPrStateChange=cardPrStateChange, cardSubDetected=cardSubDetected, shelfNumber=shelfNumber, cardSwVersion=cardSwVersion, cardAction=cardAction, cardDetected=cardDetected, cardCPLDVersion=cardCPLDVersion, cardId=cardId, diskVolumeSize=diskVolumeSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentShelfGroup=equipmentShelfGroup, systemKey=systemKey, portAdminState=portAdminState, portLastChangeTime=portLastChangeTime, diskVolumeTrapInh=diskVolumeTrapInh, cardTestCommand=cardTestCommand, equipmentStateGroup=equipmentStateGroup, TestId=TestId, TestSchedule=TestSchedule, cardDplxStatus=cardDplxStatus, portDocsIfIndex=portDocsIfIndex, cardTestEntry=cardTestEntry, diskVolumeNumber=diskVolumeNumber, portSecStateChange=portSecStateChange, cardSecState=cardSecState, portDetectedMode=portDetectedMode, cmResetInfo=cmResetInfo, portTxFlowControlMode=portTxFlowControlMode, cmResetStatus=cmResetStatus, systemClock=systemClock, equipmentState=equipmentState, diskVolumeTable=diskVolumeTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditTotalPassedCount=auditTotalPassedCount, systemGeneralGroup=systemGeneralGroup, cmVendor=cmVendor, diskVolumeId=diskVolumeId, cardPicHwVersion=cardPicHwVersion, cmMacAddress=cmMacAddress, cardFwUpdateStatus=cardFwUpdateStatus, trapSeverity=trapSeverity, cardLastBootVersion=cardLastBootVersion, cmResetClearNotification=cmResetClearNotification, portBgpId=portBgpId, errEvLoggingEnabled=errEvLoggingEnabled, cardNumPorts=cardNumPorts, auditTotalFailedCount=auditTotalFailedCount, errEvRowStatus=errEvRowStatus, equipmentTbl=equipmentTbl, portTable=portTable, auditName=auditName, portNumChans=portNumChans, portRxFlowControlDetected=portRxFlowControlDetected, remainInDiagMode=remainInDiagMode, cardLastResetReason=cardLastResetReason, auditFailedCount=auditFailedCount, equipmentAuditGroup=equipmentAuditGroup, auditPassedCount=auditPassedCount, diskVolumeShelfId=diskVolumeShelfId, cardPrState=cardPrState, auditTime=auditTime, cadEquipmentMibConformance=cadEquipmentMibConformance, portDplxStatus=portDplxStatus, cardTestId=cardTestId, equipmentPortGroup=equipmentPortGroup, shelfObjects=shelfObjects, errEventId=errEventId, cardCarrierSerialNum=cardCarrierSerialNum, auditEntry=auditEntry, cmResetReason=cmResetReason, shelfSwVersion=shelfSwVersion, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, downstreamPowerLoss=downstreamPowerLoss, TestTransactionId=TestTransactionId, portDescription=portDescription, auditShelfId=auditShelfId, auditLogThrottle=auditLogThrottle, cardFwVersion=cardFwVersion, cardFpgaSource=cardFpgaSource, portDplxStatusChange=portDplxStatusChange, fpgaErrorEventGroup=fpgaErrorEventGroup, cmRegistrationNotification=cmRegistrationNotification, auditLogOutput=auditLogOutput, cardSpareGroupMode=cardSpareGroupMode, portCardSubType=portCardSubType, portMinDsPower=portMinDsPower, cardTestTable=cardTestTable, cardDplxStatusChange=cardDplxStatusChange, cardLastChangeTime=cardLastChangeTime, cardTestIndex=cardTestIndex, portPrStateChange=portPrStateChange, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCpuSpeed=cardCpuSpeed, cardAdminState=cardAdminState, portRxFlowControlMode=portRxFlowControlMode, cardCarrierHwVersion=cardCarrierHwVersion, portAction=portAction, portCurrDsPower=portCurrDsPower, portId=portId, cardSecStateChange=cardSecStateChange, TestType=TestType, auditTable=auditTable, cardTestTime=cardTestTime, cardTestResult=cardTestResult, auditStatus=auditStatus, auditResult=auditResult, portEntry=portEntry, cmResetUpTime=cmResetUpTime, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSerialNum=cardSerialNum, cardType=cardType, cardTable=cardTable, portMacIfIndex=portMacIfIndex, auditAutoScheduling=auditAutoScheduling, cardTempOverHeatNotification=cardTempOverHeatNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, auditId=auditId, portShelfId=portShelfId, errEvLogLevel=errEvLogLevel, auditCycleCount=auditCycleCount, equipmentDiag=equipmentDiag, cmPrimarySID=cmPrimarySID, equipmentDiagGroup=equipmentDiagGroup, diskVolumeName=diskVolumeName, TestResult=TestResult, cardTrapInh=cardTrapInh, equipmentAudit=equipmentAudit, cardPicSerialNum=cardPicSerialNum, portTrapInh=portTrapInh, cardTestTransId=cardTestTransId, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResultNotification=cardTestResultNotification, systemGeneral=systemGeneral, cardTemperature=cardTemperature, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTempNoReportNotification=cardTempNoReportNotification, cmIpAddress=cmIpAddress, diskDriveNumber=diskDriveNumber, portMacAddress=portMacAddress, portNumber=portNumber, trapCounter=trapCounter, equipmentCardTestGroup=equipmentCardTestGroup, cardBootVersion=cardBootVersion, portMode=portMode, TestCommand=TestCommand, cardShelfTestIndex=cardShelfTestIndex, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeGroup=diskVolumeGroup, cardShelfId=cardShelfId, cardTestName=cardTestName, cadEquipmentMib=cadEquipmentMib, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventTable=fpgaErrorEventTable, equipmentTraps=equipmentTraps, portConnectorId=portConnectorId, cardBusSpeed=cardBusSpeed, cmDevice=cmDevice, diskVolumeFileName=diskVolumeFileName, cardNumber=cardNumber, portSecState=portSecState, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeCardId=diskVolumeCardId, cardPicDetected=cardPicDetected, cardTestScheduleCommand=cardTestScheduleCommand, cmUChannelID=cmUChannelID, cardDetectedChange=cardDetectedChange, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageNotification=diskVolumeUsageNotification, portPrState=portPrState, cardHwVersion=cardHwVersion, portType=portType, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTemperatureHighError=cardTemperatureHighError, cardSubType=cardSubType, diagTestId=diagTestId, auditDescription=auditDescription, TestScheduleCommand=TestScheduleCommand, cardName=cardName, cardCarrierFwVersion=cardCarrierFwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestResultDesc=cardTestResultDesc, portMaxDsPower=portMaxDsPower, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestDescription=cardTestDescription, consoleOutput=consoleOutput, auditCommand=auditCommand, eqDiagConfig=eqDiagConfig, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold)
