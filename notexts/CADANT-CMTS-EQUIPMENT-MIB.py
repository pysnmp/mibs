...
PortType, ShelfId, FirmwareSource, CardType, PortMode, UnknownState, PicType, EqActionType, DiskVolumeUsageLevel, DuplexStatus, PrimaryState, AdminState, CardId, CardSubType, SecondaryState, PortId, FlowControlMode, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "PortType", "ShelfId", "FirmwareSource", "CardType", "PortMode", "UnknownState", "PicType", "EqActionType", "DiskVolumeUsageLevel", "DuplexStatus", "PrimaryState", "AdminState", "CardId", "CardSubType", "SecondaryState", "PortId", "FlowControlMode", "PortDetectedMode")
...
RowStatus, DisplayString, TimeStamp, TruthValue, MacAddress, TextualConvention, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "TimeStamp", "TruthValue", "MacAddress", "TextualConvention", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cadEquipmentMib=cadEquipmentMib, portTable=portTable, cmResetInfo=cmResetInfo, equipmentState=equipmentState, errEvLogLevel=errEvLogLevel, portId=portId, portPrState=portPrState, cardFlashSize=cardFlashSize, portAction=portAction, errEvRecoveryEnabled=errEvRecoveryEnabled, cardLastResetReason=cardLastResetReason, auditAutoScheduling=auditAutoScheduling, cardTestDescription=cardTestDescription, diskVolumeCardId=diskVolumeCardId, portNumber=portNumber, portMacAddress=portMacAddress, cardAction=cardAction, downstreamPowerLoss=downstreamPowerLoss, equipmentDiagGroup=equipmentDiagGroup, diskVolumeShelfId=diskVolumeShelfId, auditTime=auditTime, cardTestTransId=cardTestTransId, auditShelfId=auditShelfId, diskVolumeEntry=diskVolumeEntry, cardRamSize=cardRamSize, diskVolumeId=diskVolumeId, cmResetClearNotification=cmResetClearNotification, portCardId=portCardId, diskVolumeDriveId=diskVolumeDriveId, shelfSwVersion=shelfSwVersion, cardTestName=cardTestName, cardTestResult=cardTestResult, cmRegistrationNotification=cmRegistrationNotification, cardTestTime=cardTestTime, portEntry=portEntry, cmResetStatus=cmResetStatus, cardCPLDVersion=cardCPLDVersion, cardCarrierFwVersion=cardCarrierFwVersion, equipmentDiag=equipmentDiag, cardFwVersion=cardFwVersion, systemGeneralGroup=systemGeneralGroup, TestResult=TestResult, cardAdminState=cardAdminState, cardTestResultDesc=cardTestResultDesc, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfObjects=shelfObjects, cardTemperatureHighWarn=cardTemperatureHighWarn, portSecStateChange=portSecStateChange, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSubDetected=cardSubDetected, auditTotalFailedCount=auditTotalFailedCount, cardId=cardId, TestId=TestId, diskVolumeNumber=diskVolumeNumber, auditName=auditName, auditTable=auditTable, cardTestEntry=cardTestEntry, auditCycleCount=auditCycleCount, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardHwVersion=cardHwVersion, cardShelfTestIndex=cardShelfTestIndex, TestScheduleCommand=TestScheduleCommand, cardDetected=cardDetected, portCurrDsPower=portCurrDsPower, portMaxDsPower=portMaxDsPower, portMinDsPower=portMinDsPower, systemKey=systemKey, diskVolumeFileName=diskVolumeFileName, portTrapInh=portTrapInh, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentCardTestGroup=equipmentCardTestGroup, cardNumber=cardNumber, cardCpuType=cardCpuType, cardTestId=cardTestId, portSecState=portSecState, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTemperature=cardTemperature, auditCardId=auditCardId, eqDiagConfig=eqDiagConfig, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMode=portMode, cardBootVersion=cardBootVersion, TestTransactionId=TestTransactionId, cardTrapInh=cardTrapInh, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeSize=diskVolumeSize, cardEntry=cardEntry, diskVolumeName=diskVolumeName, fpgaErrorEventTable=fpgaErrorEventTable, auditDescription=auditDescription, portDescription=portDescription, portBgpId=portBgpId, systemGeneral=systemGeneral, cardCpuSpeed=cardCpuSpeed, equipmentShelfGroup=equipmentShelfGroup, cardUpTime=cardUpTime, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeGroup=diskVolumeGroup, cmResetReason=cmResetReason, cmPrimarySID=cmPrimarySID, cmMacAddress=cmMacAddress, cardCarrierSerialNum=cardCarrierSerialNum, portType=portType, diskVolumeTrapInh=diskVolumeTrapInh, cardSwVersion=cardSwVersion, portTxFlowControlMode=portTxFlowControlMode, portRxFlowControlDetected=portRxFlowControlDetected, cardSecState=cardSecState, portDocsIfIndex=portDocsIfIndex, auditTotalPassedCount=auditTotalPassedCount, cardLastBootVersion=cardLastBootVersion, diskVolumeDiskSize=diskVolumeDiskSize, cardDplxStatusChange=cardDplxStatusChange, portRxFlowControlMode=portRxFlowControlMode, auditFailedCount=auditFailedCount, portDetectedMode=portDetectedMode, cardBusSpeed=cardBusSpeed, portConnectorId=portConnectorId, auditLogOutput=auditLogOutput, auditLogThrottle=auditLogThrottle, equipmentTbl=equipmentTbl, cardTestType=cardTestType, cardPicHwVersion=cardPicHwVersion, cmResetNotification=cmResetNotification, cardTestSchedule=cardTestSchedule, shelfName=shelfName, cardDetectedChange=cardDetectedChange, auditCommand=auditCommand, cardNumPorts=cardNumPorts, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDplxStatus=cardDplxStatus, cardPrState=cardPrState, equipmentAudit=equipmentAudit, cmIpAddress=cmIpAddress, cardCarrierHwVersion=cardCarrierHwVersion, diagTestId=diagTestId, equipmentCardGroup=equipmentCardGroup, cardSubType=cardSubType, auditId=auditId, equipmentPortGroup=equipmentPortGroup, auditResult=auditResult, equipmentError=equipmentError, TestCommand=TestCommand, cardTestTable=cardTestTable, auditStatus=auditStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentStateGroup=equipmentStateGroup, cardPrStateChange=cardPrStateChange, remainInDiagMode=remainInDiagMode, cardTestResultNotification=cardTestResultNotification, systemClock=systemClock, cardTable=cardTable, cardFpgaSource=cardFpgaSource, cardSerialNum=cardSerialNum, TestType=TestType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, verboseLevel=verboseLevel, cardTempNoReportNotification=cardTempNoReportNotification, portLastChangeTime=portLastChangeTime, portNumChans=portNumChans, shelfNumber=shelfNumber, cardTestIndex=cardTestIndex, cardSecStateChange=cardSecStateChange, cardName=cardName, trapSeverity=trapSeverity, cardPicSerialNum=cardPicSerialNum, cmResetUpTime=cmResetUpTime, portAdminState=portAdminState, portCardSubType=portCardSubType, auditPassedCount=auditPassedCount, errEvLoggingEnabled=errEvLoggingEnabled, cardShelfId=cardShelfId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSpareGroupMode=cardSpareGroupMode, trapCounter=trapCounter, auditEntry=auditEntry, errEventId=errEventId, cmUChannelID=cmUChannelID, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portMacIfIndex=portMacIfIndex, portDplxStatus=portDplxStatus, diskVolumeTable=diskVolumeTable, cardLastBootSource=cardLastBootSource, portShelfId=portShelfId, cmDevice=cmDevice, TestSchedule=TestSchedule, cardPicDetected=cardPicDetected, equipmentAuditGroup=equipmentAuditGroup, cardLastChangeTime=cardLastChangeTime, diskDriveNumber=diskDriveNumber, cardTestCommand=cardTestCommand, portPrStateChange=portPrStateChange, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, consoleOutput=consoleOutput, cardTemperatureHighError=cardTemperatureHighError, cmVendor=cmVendor, fpgaErrorEventEntry=fpgaErrorEventEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cardSpareGroupId=cardSpareGroupId, equipmentTraps=equipmentTraps, errEvRowStatus=errEvRowStatus, cardType=cardType, portTxFlowControlDetected=portTxFlowControlDetected)
