...
CardType, FirmwareSource, PortType, SecondaryState, UnknownState, PortMode, CardId, PortId, DuplexStatus, PortDetectedMode, PrimaryState, PicType, CardSubType, FlowControlMode, AdminState, DiskVolumeUsageLevel, EqActionType, ShelfId = mibBuilder.importSymbols("CADANT-TC", "CardType", "FirmwareSource", "PortType", "SecondaryState", "UnknownState", "PortMode", "CardId", "PortId", "DuplexStatus", "PortDetectedMode", "PrimaryState", "PicType", "CardSubType", "FlowControlMode", "AdminState", "DiskVolumeUsageLevel", "EqActionType", "ShelfId")
...
RowStatus, TextualConvention, DateAndTime, TruthValue, MacAddress, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "DateAndTime", "TruthValue", "MacAddress", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfObjects=shelfObjects, cardAdminState=cardAdminState, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestName=cardTestName, diskVolumeNumber=diskVolumeNumber, cardCpuType=cardCpuType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmRegistrationNotification=cmRegistrationNotification, diskVolumeDiskSize=diskVolumeDiskSize, portDetectedMode=portDetectedMode, auditResult=auditResult, cardFwVersion=cardFwVersion, diskVolumeId=diskVolumeId, TestResult=TestResult, downstreamPowerLoss=downstreamPowerLoss, TestTransactionId=TestTransactionId, portMode=portMode, cardTestTable=cardTestTable, portDocsIfIndex=portDocsIfIndex, cmResetReason=cmResetReason, auditStatus=auditStatus, cmIpAddress=cmIpAddress, diskVolumeShelfId=diskVolumeShelfId, equipmentDiagGroup=equipmentDiagGroup, cardShelfTestIndex=cardShelfTestIndex, cmVendor=cmVendor, diskVolumeDriveId=diskVolumeDriveId, portCurrDsPower=portCurrDsPower, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestSchedule=TestSchedule, auditCycleCount=auditCycleCount, portDplxStatusChange=portDplxStatusChange, portShelfId=portShelfId, fpgaErrorEventGroup=fpgaErrorEventGroup, auditName=auditName, portNumber=portNumber, portMaxDsPower=portMaxDsPower, cardDetected=cardDetected, diskDriveNumber=diskDriveNumber, diskVolumeName=diskVolumeName, portAdminState=portAdminState, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeGroup=diskVolumeGroup, portId=portId, portRxFlowControlMode=portRxFlowControlMode, cardTemperatureHighError=cardTemperatureHighError, auditTotalPassedCount=auditTotalPassedCount, cardCarrierHwVersion=cardCarrierHwVersion, TestType=TestType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardBusSpeed=cardBusSpeed, cadEquipmentMibCompliance=cadEquipmentMibCompliance, verboseLevel=verboseLevel, cardAction=cardAction, cardLastChangeTime=cardLastChangeTime, portMinDsPower=portMinDsPower, remainInDiagMode=remainInDiagMode, cardTestId=cardTestId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardDplxStatusChange=cardDplxStatusChange, equipmentShelfGroup=equipmentShelfGroup, cardUpTime=cardUpTime, cardName=cardName, TestId=TestId, portConnectorId=portConnectorId, equipmentCardTestGroup=equipmentCardTestGroup, cardLastBootSource=cardLastBootSource, shelfNumber=shelfNumber, auditTable=auditTable, cardTemperature=cardTemperature, diskVolumeEntry=diskVolumeEntry, errEventId=errEventId, cardPicDetected=cardPicDetected, cardSecState=cardSecState, cardType=cardType, portMacAddress=portMacAddress, diskVolumeTable=diskVolumeTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentState=equipmentState, cardTestResultNotification=cardTestResultNotification, cardSwVersion=cardSwVersion, systemGeneralGroup=systemGeneralGroup, auditCommand=auditCommand, cardCPLDVersion=cardCPLDVersion, diskVolumeCardId=diskVolumeCardId, cmResetUpTime=cmResetUpTime, portCardSubType=portCardSubType, equipmentDiag=equipmentDiag, cardTestIndex=cardTestIndex, portEntry=portEntry, cardEntry=cardEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, cardPrStateChange=cardPrStateChange, cardTestEntry=cardTestEntry, cmMacAddress=cmMacAddress, cardRamSize=cardRamSize, portMacIfIndex=portMacIfIndex, PYSNMP_MODULE_ID=cadEquipmentMib, portTxFlowControlDetected=portTxFlowControlDetected, equipmentAudit=equipmentAudit, auditShelfId=auditShelfId, errEvLogLevel=errEvLogLevel, consoleOutput=consoleOutput, cardHwVersion=cardHwVersion, equipmentStateGroup=equipmentStateGroup, equipmentTbl=equipmentTbl, portRxFlowControlDetected=portRxFlowControlDetected, systemGeneral=systemGeneral, diskVolumeTrapInh=diskVolumeTrapInh, cardSerialNum=cardSerialNum, auditLogOutput=auditLogOutput, cardCpuSpeed=cardCpuSpeed, cardTestTransId=cardTestTransId, cardTestSchedule=cardTestSchedule, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, errEvRowStatus=errEvRowStatus, cadEquipmentMib=cadEquipmentMib, portSecStateChange=portSecStateChange, portLastChangeTime=portLastChangeTime, cardTrapInh=cardTrapInh, cardSubDetected=cardSubDetected, systemClock=systemClock, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardNumPorts=cardNumPorts, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSecStateChange=cardSecStateChange, auditTotalFailedCount=auditTotalFailedCount, cmResetStatus=cmResetStatus, equipmentAuditGroup=equipmentAuditGroup, cardTestCommand=cardTestCommand, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeSize=diskVolumeSize, cmDevice=cmDevice, eqDiagConfig=eqDiagConfig, auditPassedCount=auditPassedCount, cardTestDescription=cardTestDescription, auditFailedCount=auditFailedCount, cardCarrierSerialNum=cardCarrierSerialNum, cardPicHwVersion=cardPicHwVersion, shelfName=shelfName, shelfSwVersion=shelfSwVersion, portTrapInh=portTrapInh, auditEntry=auditEntry, auditTime=auditTime, trapSeverity=trapSeverity, errEvLoggingEnabled=errEvLoggingEnabled, cardFpgaSource=cardFpgaSource, cardDetectedChange=cardDetectedChange, cardTempNoReportNotification=cardTempNoReportNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, TestCommand=TestCommand, portNumChans=portNumChans, cmUChannelID=cmUChannelID, diagTestId=diagTestId, auditDescription=auditDescription, cmResetNotification=cmResetNotification, cardBootVersion=cardBootVersion, cardFlashSize=cardFlashSize, cmResetInfo=cmResetInfo, equipmentCardGroup=equipmentCardGroup, TestScheduleCommand=TestScheduleCommand, cardTestResult=cardTestResult, portPrState=portPrState, auditCardId=auditCardId, cmPrimarySID=cmPrimarySID, systemKey=systemKey, diskVolumeUsageLevel=diskVolumeUsageLevel, cardDplxStatus=cardDplxStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, portType=portType, equipmentError=equipmentError, portDplxStatus=portDplxStatus, portSecState=portSecState, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeFileName=diskVolumeFileName, cardSpareGroupId=cardSpareGroupId, portTable=portTable, cardLastBootVersion=cardLastBootVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardNumber=cardNumber, cardLastResetReason=cardLastResetReason, portBgpId=portBgpId, portPrStateChange=portPrStateChange, cardSpareGroupMode=cardSpareGroupMode, portTxFlowControlMode=portTxFlowControlMode, cardTestResultDesc=cardTestResultDesc, auditLogThrottle=auditLogThrottle, auditId=auditId, equipmentTraps=equipmentTraps, trapCounter=trapCounter, cardTable=cardTable, portDescription=portDescription, portCardId=portCardId, auditAutoScheduling=auditAutoScheduling, fpgaErrorEventEntry=fpgaErrorEventEntry, cmResetClearNotification=cmResetClearNotification, cardId=cardId, cardTestType=cardTestType, cardSubType=cardSubType, cardPicSerialNum=cardPicSerialNum, portAction=portAction, cardPrState=cardPrState, cardTestTime=cardTestTime, cardShelfId=cardShelfId, equipmentPortGroup=equipmentPortGroup, cardFwUpdateStatus=cardFwUpdateStatus)
