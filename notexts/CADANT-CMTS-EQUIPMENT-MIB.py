...
DuplexStatus, UnknownState, AdminState, FlowControlMode, PortMode, PortDetectedMode, PrimaryState, CardSubType, FirmwareSource, SecondaryState, CardId, EqActionType, PortType, PicType, CardType, DiskVolumeUsageLevel, ShelfId, PortId = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "UnknownState", "AdminState", "FlowControlMode", "PortMode", "PortDetectedMode", "PrimaryState", "CardSubType", "FirmwareSource", "SecondaryState", "CardId", "EqActionType", "PortType", "PicType", "CardType", "DiskVolumeUsageLevel", "ShelfId", "PortId")
...
RowStatus, TimeStamp, TextualConvention, DisplayString, MacAddress, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "TextualConvention", "DisplayString", "MacAddress", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeDriveId=diskVolumeDriveId, portMacIfIndex=portMacIfIndex, portAdminState=portAdminState, cardName=cardName, portCurrDsPower=portCurrDsPower, auditAutoScheduling=auditAutoScheduling, diskVolumeTable=diskVolumeTable, cardSubType=cardSubType, eqDiagConfig=eqDiagConfig, portDplxStatus=portDplxStatus, cardDetected=cardDetected, cardTestCommand=cardTestCommand, portNumChans=portNumChans, cardTestScheduleCommand=cardTestScheduleCommand, errEvLoggingEnabled=errEvLoggingEnabled, portTxFlowControlMode=portTxFlowControlMode, auditPassedCount=auditPassedCount, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestId=cardTestId, diskVolumeDiskSize=diskVolumeDiskSize, cardShelfTestIndex=cardShelfTestIndex, cardTestSchedule=cardTestSchedule, systemClock=systemClock, TestSchedule=TestSchedule, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetNotification=cmResetNotification, cardSecStateChange=cardSecStateChange, portTable=portTable, portDocsIfIndex=portDocsIfIndex, cardCpuType=cardCpuType, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentAudit=equipmentAudit, cardCPLDVersion=cardCPLDVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, cardLastBootSource=cardLastBootSource, cardNumber=cardNumber, cardDplxStatus=cardDplxStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, TestTransactionId=TestTransactionId, portMacAddress=portMacAddress, portDescription=portDescription, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, downstreamPowerLoss=downstreamPowerLoss, cardPicHwVersion=cardPicHwVersion, cardId=cardId, diskVolumeSize=diskVolumeSize, diskVolumeEntry=diskVolumeEntry, auditCommand=auditCommand, cardBusSpeed=cardBusSpeed, portBgpId=portBgpId, TestScheduleCommand=TestScheduleCommand, cardFwVersion=cardFwVersion, portCardSubType=portCardSubType, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentAuditGroup=equipmentAuditGroup, auditCardId=auditCardId, cadEquipmentMibConformance=cadEquipmentMibConformance, portDetectedMode=portDetectedMode, cardPrState=cardPrState, cardFpgaSource=cardFpgaSource, portPrState=portPrState, cardSpareGroupMode=cardSpareGroupMode, equipmentDiag=equipmentDiag, auditResult=auditResult, portMode=portMode, systemGeneralGroup=systemGeneralGroup, cardAction=cardAction, verboseLevel=verboseLevel, cardTempOverHeatNotification=cardTempOverHeatNotification, portEntry=portEntry, cardSpareGroupId=cardSpareGroupId, auditTable=auditTable, equipmentError=equipmentError, cardTestDescription=cardTestDescription, portDplxStatusChange=portDplxStatusChange, cardLastChangeTime=cardLastChangeTime, equipmentState=equipmentState, cardSerialNum=cardSerialNum, cardPicDetected=cardPicDetected, cardTestTime=cardTestTime, cmVendor=cmVendor, equipmentTbl=equipmentTbl, cmPrimarySID=cmPrimarySID, cadEquipmentMib=cadEquipmentMib, portCardId=portCardId, auditEntry=auditEntry, portMinDsPower=portMinDsPower, cardTemperatureHighError=cardTemperatureHighError, portRxFlowControlDetected=portRxFlowControlDetected, portConnectorId=portConnectorId, errEventId=errEventId, cardCarrierSerialNum=cardCarrierSerialNum, portMaxDsPower=portMaxDsPower, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSwVersion=cardSwVersion, portPrStateChange=portPrStateChange, diskVolumeTrapInh=diskVolumeTrapInh, portId=portId, cardTestTable=cardTestTable, cardTestResultNotification=cardTestResultNotification, cmUChannelID=cmUChannelID, auditDescription=auditDescription, cmDevice=cmDevice, auditLogOutput=auditLogOutput, diskVolumeCardId=diskVolumeCardId, TestType=TestType, shelfNumber=shelfNumber, cardPrStateChange=cardPrStateChange, cardBootVersion=cardBootVersion, cardFwUpdateStatus=cardFwUpdateStatus, cmResetStatus=cmResetStatus, TestId=TestId, cardCarrierHwVersion=cardCarrierHwVersion, auditCycleCount=auditCycleCount, cardDetectedChange=cardDetectedChange, cardLastBootVersion=cardLastBootVersion, cardEntry=cardEntry, equipmentCardTestGroup=equipmentCardTestGroup, shelfName=shelfName, cardShelfId=cardShelfId, portAction=portAction, cmResetInfo=cmResetInfo, diskVolumeNumber=diskVolumeNumber, equipmentCardGroup=equipmentCardGroup, equipmentStateGroup=equipmentStateGroup, consoleOutput=consoleOutput, auditStatus=auditStatus, trapSeverity=trapSeverity, auditId=auditId, auditShelfId=auditShelfId, cardAdminState=cardAdminState, cardSecState=cardSecState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemGeneral=systemGeneral, cardTestEntry=cardTestEntry, trapCounter=trapCounter, cardTemperature=cardTemperature, portShelfId=portShelfId, diskVolumeShelfId=diskVolumeShelfId, portSecState=portSecState, cardRamSize=cardRamSize, cardSubDetected=cardSubDetected, diskVolumeFileName=diskVolumeFileName, cardLastResetReason=cardLastResetReason, cmResetReason=cmResetReason, auditFailedCount=auditFailedCount, errEvRowStatus=errEvRowStatus, portNumber=portNumber, portType=portType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTable=cardTable, auditTotalPassedCount=auditTotalPassedCount, diskVolumeId=diskVolumeId, cardTestType=cardTestType, cmMacAddress=cmMacAddress, cmResetClearNotification=cmResetClearNotification, cardTestIndex=cardTestIndex, shelfSwVersion=shelfSwVersion, diagTestId=diagTestId, cardFlashSize=cardFlashSize, portTrapInh=portTrapInh, fpgaErrorEventEntry=fpgaErrorEventEntry, cmIpAddress=cmIpAddress, portSecStateChange=portSecStateChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentTraps=equipmentTraps, cardTrapInh=cardTrapInh, cardTempNoReportNotification=cardTempNoReportNotification, cardDplxStatusChange=cardDplxStatusChange, cardUpTime=cardUpTime, auditLogThrottle=auditLogThrottle, auditTime=auditTime, portRxFlowControlMode=portRxFlowControlMode, errEvLogLevel=errEvLogLevel, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResultDesc=cardTestResultDesc, systemKey=systemKey, diskVolumeGroup=diskVolumeGroup, equipmentPortGroup=equipmentPortGroup, cardTestName=cardTestName, diskDriveNumber=diskDriveNumber, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardNumPorts=cardNumPorts, shelfObjects=shelfObjects, cardCpuSpeed=cardCpuSpeed, diskVolumeName=diskVolumeName, cardHwVersion=cardHwVersion, equipmentShelfGroup=equipmentShelfGroup, portLastChangeTime=portLastChangeTime, remainInDiagMode=remainInDiagMode, TestCommand=TestCommand, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardType=cardType, cardPicSerialNum=cardPicSerialNum, auditName=auditName, cmRegistrationNotification=cmRegistrationNotification, cardTestTransId=cardTestTransId, cmResetUpTime=cmResetUpTime, TestResult=TestResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResult=cardTestResult, cardCarrierFwVersion=cardCarrierFwVersion, fpgaErrorEventTable=fpgaErrorEventTable, equipmentDiagGroup=equipmentDiagGroup)
