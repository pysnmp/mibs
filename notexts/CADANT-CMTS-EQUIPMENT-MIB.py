...
PortType, FlowControlMode, CardId, PortId, FirmwareSource, ShelfId, CardSubType, PortMode, UnknownState, CardType, PrimaryState, AdminState, DiskVolumeUsageLevel, SecondaryState, PortDetectedMode, EqActionType, DuplexStatus, PicType = mibBuilder.importSymbols("CADANT-TC", "PortType", "FlowControlMode", "CardId", "PortId", "FirmwareSource", "ShelfId", "CardSubType", "PortMode", "UnknownState", "CardType", "PrimaryState", "AdminState", "DiskVolumeUsageLevel", "SecondaryState", "PortDetectedMode", "EqActionType", "DuplexStatus", "PicType")
...
TimeStamp, DateAndTime, DisplayString, TruthValue, RowStatus, TextualConvention, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DateAndTime", "DisplayString", "TruthValue", "RowStatus", "TextualConvention", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditId=auditId, errEvRowStatus=errEvRowStatus, errEvLogLevel=errEvLogLevel, cardTempNoReportNotification=cardTempNoReportNotification, cardFwUpdateStatus=cardFwUpdateStatus, cardShelfTestIndex=cardShelfTestIndex, portLastChangeTime=portLastChangeTime, cardLastChangeTime=cardLastChangeTime, diskVolumeName=diskVolumeName, cardName=cardName, auditCommand=auditCommand, cardBootVersion=cardBootVersion, errEventId=errEventId, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCarrierFwVersion=cardCarrierFwVersion, remainInDiagMode=remainInDiagMode, shelfName=shelfName, portDescription=portDescription, portAction=portAction, cardFwVersion=cardFwVersion, diagTestId=diagTestId, cmVendor=cmVendor, cardLastResetReason=cardLastResetReason, TestSchedule=TestSchedule, equipmentTraps=equipmentTraps, cardTemperature=cardTemperature, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, portRxFlowControlMode=portRxFlowControlMode, cardCPLDVersion=cardCPLDVersion, diskDriveNumber=diskDriveNumber, cardTestDescription=cardTestDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentPortGroup=equipmentPortGroup, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, shelfNumber=shelfNumber, portDplxStatus=portDplxStatus, cardTestTime=cardTestTime, portMinDsPower=portMinDsPower, cardTestId=cardTestId, portRxFlowControlDetected=portRxFlowControlDetected, cardSpareGroupMode=cardSpareGroupMode, diskVolumeCardId=diskVolumeCardId, cmIpAddress=cmIpAddress, cardCarrierHwVersion=cardCarrierHwVersion, cardTemperatureHighError=cardTemperatureHighError, cardCpuSpeed=cardCpuSpeed, portTable=portTable, portId=portId, cardTable=cardTable, cardTestType=cardTestType, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResult=cardTestResult, cardDplxStatusChange=cardDplxStatusChange, auditEntry=auditEntry, portConnectorId=portConnectorId, auditAutoScheduling=auditAutoScheduling, cardSerialNum=cardSerialNum, portShelfId=portShelfId, equipmentDiagGroup=equipmentDiagGroup, cardPicDetected=cardPicDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, TestScheduleCommand=TestScheduleCommand, auditTotalPassedCount=auditTotalPassedCount, auditLogOutput=auditLogOutput, cmMacAddress=cmMacAddress, cmResetStatus=cmResetStatus, cardDetectedChange=cardDetectedChange, portTrapInh=portTrapInh, cmDevice=cmDevice, portPrStateChange=portPrStateChange, portSecStateChange=portSecStateChange, portNumber=portNumber, cardRamSize=cardRamSize, PYSNMP_MODULE_ID=cadEquipmentMib, systemGeneral=systemGeneral, auditDescription=auditDescription, cardTemperatureHighWarn=cardTemperatureHighWarn, cardDetected=cardDetected, systemGeneralGroup=systemGeneralGroup, cardTestSchedule=cardTestSchedule, TestType=TestType, portPrState=portPrState, cmPrimarySID=cmPrimarySID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentAuditGroup=equipmentAuditGroup, cardLastBootVersion=cardLastBootVersion, diskVolumeTrapInh=diskVolumeTrapInh, cardTestEntry=cardTestEntry, equipmentDiag=equipmentDiag, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetUpTime=cmResetUpTime, trapSeverity=trapSeverity, cardTestTransId=cardTestTransId, cardTestName=cardTestName, portMode=portMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeNumber=diskVolumeNumber, auditPassedCount=auditPassedCount, portCardSubType=portCardSubType, cardId=cardId, diskVolumeGroup=diskVolumeGroup, cmResetNotification=cmResetNotification, cardPicSerialNum=cardPicSerialNum, portMaxDsPower=portMaxDsPower, portType=portType, cardFlashSize=cardFlashSize, diskVolumeId=diskVolumeId, cardTestTable=cardTestTable, cardType=cardType, auditName=auditName, cardBusSpeed=cardBusSpeed, cardTestResultNotification=cardTestResultNotification, portMacIfIndex=portMacIfIndex, auditStatus=auditStatus, TestId=TestId, auditCardId=auditCardId, portNumChans=portNumChans, diskVolumeEntry=diskVolumeEntry, equipmentShelfGroup=equipmentShelfGroup, cmResetClearNotification=cmResetClearNotification, auditLogThrottle=auditLogThrottle, cardSecState=cardSecState, cardPrStateChange=cardPrStateChange, cardSwVersion=cardSwVersion, cardDplxStatus=cardDplxStatus, diskVolumeTable=diskVolumeTable, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierSerialNum=cardCarrierSerialNum, cadEquipmentMib=cadEquipmentMib, trapCounter=trapCounter, cardLastBootSource=cardLastBootSource, cadEquipmentMibCompliance=cadEquipmentMibCompliance, downstreamPowerLoss=downstreamPowerLoss, cmUChannelID=cmUChannelID, shelfObjects=shelfObjects, portDocsIfIndex=portDocsIfIndex, cardSubType=cardSubType, portMacAddress=portMacAddress, portCurrDsPower=portCurrDsPower, cardTestResultDesc=cardTestResultDesc, portBgpId=portBgpId, cardFpgaSource=cardFpgaSource, errEvLoggingEnabled=errEvLoggingEnabled, auditTime=auditTime, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTrapInh=cardTrapInh, verboseLevel=verboseLevel, cardAdminState=cardAdminState, cardPrState=cardPrState, cardTestIndex=cardTestIndex, cardNumber=cardNumber, equipmentState=equipmentState, auditShelfId=auditShelfId, equipmentTbl=equipmentTbl, portAdminState=portAdminState, portDetectedMode=portDetectedMode, TestTransactionId=TestTransactionId, diskVolumeFileName=diskVolumeFileName, equipmentAudit=equipmentAudit, cardHwVersion=cardHwVersion, systemKey=systemKey, portDplxStatusChange=portDplxStatusChange, equipmentStateGroup=equipmentStateGroup, eqDiagConfig=eqDiagConfig, cmResetInfo=cmResetInfo, equipmentError=equipmentError, auditTotalFailedCount=auditTotalFailedCount, cardTestScheduleCommand=cardTestScheduleCommand, auditFailedCount=auditFailedCount, portSecState=portSecState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardUpTime=cardUpTime, shelfSwVersion=shelfSwVersion, cardPicHwVersion=cardPicHwVersion, diskVolumeShelfId=diskVolumeShelfId, cardCpuType=cardCpuType, equipmentCardGroup=equipmentCardGroup, TestResult=TestResult, cardEntry=cardEntry, cardNumPorts=cardNumPorts, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSpareGroupId=cardSpareGroupId, auditCycleCount=auditCycleCount, cmResetReason=cmResetReason, auditResult=auditResult, diskVolumeSize=diskVolumeSize, portTxFlowControlMode=portTxFlowControlMode, auditTable=auditTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, systemClock=systemClock, portCardId=portCardId, consoleOutput=consoleOutput, cardTestCommand=cardTestCommand, cmRegistrationNotification=cmRegistrationNotification, diskVolumeDriveId=diskVolumeDriveId, portEntry=portEntry, cardSecStateChange=cardSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, cardAction=cardAction, errEvRecoveryEnabled=errEvRecoveryEnabled, cardShelfId=cardShelfId, cardSubDetected=cardSubDetected, TestCommand=TestCommand)
