...
SecondaryState, CardId, EqActionType, PortId, PortMode, PortType, AdminState, UnknownState, ShelfId, PrimaryState, CardType, FirmwareSource, DiskVolumeUsageLevel, PicType, FlowControlMode, PortDetectedMode, CardSubType, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "CardId", "EqActionType", "PortId", "PortMode", "PortType", "AdminState", "UnknownState", "ShelfId", "PrimaryState", "CardType", "FirmwareSource", "DiskVolumeUsageLevel", "PicType", "FlowControlMode", "PortDetectedMode", "CardSubType", "DuplexStatus")
...
TextualConvention, TruthValue, DateAndTime, RowStatus, TimeStamp, MacAddress, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "DateAndTime", "RowStatus", "TimeStamp", "MacAddress", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portPrState=portPrState, portMaxDsPower=portMaxDsPower, auditCardId=auditCardId, cardTemperature=cardTemperature, cardShelfId=cardShelfId, auditStatus=auditStatus, cardShelfTestIndex=cardShelfTestIndex, diskVolumeFileName=diskVolumeFileName, portType=portType, auditPassedCount=auditPassedCount, portRxFlowControlDetected=portRxFlowControlDetected, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeTrapInh=diskVolumeTrapInh, cardBusSpeed=cardBusSpeed, cardLastBootSource=cardLastBootSource, cardLastChangeTime=cardLastChangeTime, cardSubDetected=cardSubDetected, cardAdminState=cardAdminState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portRxFlowControlMode=portRxFlowControlMode, diskVolumeTable=diskVolumeTable, auditTime=auditTime, auditTotalFailedCount=auditTotalFailedCount, cardLastBootVersion=cardLastBootVersion, auditId=auditId, portCardSubType=portCardSubType, diskVolumeCardId=diskVolumeCardId, portShelfId=portShelfId, cardSwVersion=cardSwVersion, cmMacAddress=cmMacAddress, cardTemperatureHighError=cardTemperatureHighError, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portNumChans=portNumChans, auditAutoScheduling=auditAutoScheduling, cardPicDetected=cardPicDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, auditName=auditName, portMacAddress=portMacAddress, fpgaErrorEventTable=fpgaErrorEventTable, TestId=TestId, cardFpgaSource=cardFpgaSource, diskVolumeSize=diskVolumeSize, auditDescription=auditDescription, cmUChannelID=cmUChannelID, equipmentShelfGroup=equipmentShelfGroup, portSecState=portSecState, TestResult=TestResult, systemGeneralGroup=systemGeneralGroup, cardTrapInh=cardTrapInh, cadEquipmentMibConformance=cadEquipmentMibConformance, cardType=cardType, cardPrStateChange=cardPrStateChange, eqDiagConfig=eqDiagConfig, cardAction=cardAction, TestSchedule=TestSchedule, remainInDiagMode=remainInDiagMode, diskVolumeNumber=diskVolumeNumber, cardHwVersion=cardHwVersion, auditShelfId=auditShelfId, cardName=cardName, errEvLogLevel=errEvLogLevel, portBgpId=portBgpId, equipmentAuditGroup=equipmentAuditGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentStateGroup=equipmentStateGroup, cmResetClearNotification=cmResetClearNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentCardTestGroup=equipmentCardTestGroup, portTable=portTable, cardLastResetReason=cardLastResetReason, equipmentError=equipmentError, equipmentState=equipmentState, cardTestType=cardTestType, cardSecStateChange=cardSecStateChange, shelfNumber=shelfNumber, cardTestScheduleCommand=cardTestScheduleCommand, auditCommand=auditCommand, cmResetReason=cmResetReason, portMinDsPower=portMinDsPower, cmResetStatus=cmResetStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDocsIfIndex=portDocsIfIndex, cardUpTime=cardUpTime, cardTempNoReportNotification=cardTempNoReportNotification, portMacIfIndex=portMacIfIndex, portMode=portMode, diskVolumeDiskSize=diskVolumeDiskSize, cardTestIndex=cardTestIndex, auditLogThrottle=auditLogThrottle, cmPrimarySID=cmPrimarySID, portTrapInh=portTrapInh, portEntry=portEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portCardId=portCardId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentDiagGroup=equipmentDiagGroup, portAction=portAction, cmDevice=cmDevice, cmResetUpTime=cmResetUpTime, errEventId=errEventId, cardPrState=cardPrState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTxFlowControlMode=portTxFlowControlMode, shelfName=shelfName, TestTransactionId=TestTransactionId, cardTestId=cardTestId, cardNumber=cardNumber, cardNumPorts=cardNumPorts, cardSecState=cardSecState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDescription=portDescription, auditTable=auditTable, cardCarrierSerialNum=cardCarrierSerialNum, cmResetInfo=cmResetInfo, diskVolumeName=diskVolumeName, cardTemperatureHighWarn=cardTemperatureHighWarn, diskDriveNumber=diskDriveNumber, systemGeneral=systemGeneral, portPrStateChange=portPrStateChange, diskVolumeEntry=diskVolumeEntry, cadEquipmentMib=cadEquipmentMib, errEvRowStatus=errEvRowStatus, equipmentAudit=equipmentAudit, portLastChangeTime=portLastChangeTime, cardTestTable=cardTestTable, portDplxStatus=portDplxStatus, portSecStateChange=portSecStateChange, equipmentDiag=equipmentDiag, diagTestId=diagTestId, shelfObjects=shelfObjects, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfSwVersion=shelfSwVersion, cardTestName=cardTestName, auditFailedCount=auditFailedCount, trapSeverity=trapSeverity, systemClock=systemClock, cardSpareGroupMode=cardSpareGroupMode, cardTestDescription=cardTestDescription, diskVolumeGroup=diskVolumeGroup, cardTestResult=cardTestResult, diskVolumeId=diskVolumeId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditLogOutput=auditLogOutput, cmIpAddress=cmIpAddress, cmResetNotification=cmResetNotification, cmRegistrationNotification=cmRegistrationNotification, cardTestCommand=cardTestCommand, portNumber=portNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, downstreamPowerLoss=downstreamPowerLoss, TestType=TestType, equipmentTbl=equipmentTbl, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuSpeed=cardCpuSpeed, cardPicSerialNum=cardPicSerialNum, cardTestTime=cardTestTime, cardDplxStatusChange=cardDplxStatusChange, diskVolumeDriveId=diskVolumeDriveId, cardSpareGroupId=cardSpareGroupId, cardFwUpdateStatus=cardFwUpdateStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, auditCycleCount=auditCycleCount, cardEntry=cardEntry, errEvLoggingEnabled=errEvLoggingEnabled, cardTempOverHeatNotification=cardTempOverHeatNotification, cardBootVersion=cardBootVersion, cardCarrierHwVersion=cardCarrierHwVersion, portConnectorId=portConnectorId, portCurrDsPower=portCurrDsPower, verboseLevel=verboseLevel, auditResult=auditResult, cardDetected=cardDetected, cmVendor=cmVendor, trapCounter=trapCounter, TestCommand=TestCommand, cardFwVersion=cardFwVersion, cardRamSize=cardRamSize, portId=portId, portDetectedMode=portDetectedMode, cardTestResultDesc=cardTestResultDesc, equipmentTraps=equipmentTraps, cardTestEntry=cardTestEntry, cardTestResultNotification=cardTestResultNotification, cardPicHwVersion=cardPicHwVersion, diskVolumeShelfId=diskVolumeShelfId, portAdminState=portAdminState, systemKey=systemKey, cardCpuType=cardCpuType, cardId=cardId, TestScheduleCommand=TestScheduleCommand, cardDetectedChange=cardDetectedChange, cardTestSchedule=cardTestSchedule, consoleOutput=consoleOutput, cardSubType=cardSubType, cardCPLDVersion=cardCPLDVersion, portTxFlowControlDetected=portTxFlowControlDetected, cardTestTransId=cardTestTransId, equipmentCardGroup=equipmentCardGroup, equipmentPortGroup=equipmentPortGroup, auditEntry=auditEntry, cardFlashSize=cardFlashSize, cardSerialNum=cardSerialNum, auditTotalPassedCount=auditTotalPassedCount, portDplxStatusChange=portDplxStatusChange, cardDplxStatus=cardDplxStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTable=cardTable)
