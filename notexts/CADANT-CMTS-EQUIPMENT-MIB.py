...
DiskVolumeUsageLevel, DuplexStatus, AdminState, PortMode, PortType, CardType, SecondaryState, CardId, UnknownState, PortId, EqActionType, FlowControlMode, PicType, CardSubType, ShelfId, PrimaryState, PortDetectedMode, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "DuplexStatus", "AdminState", "PortMode", "PortType", "CardType", "SecondaryState", "CardId", "UnknownState", "PortId", "EqActionType", "FlowControlMode", "PicType", "CardSubType", "ShelfId", "PrimaryState", "PortDetectedMode", "FirmwareSource")
...
DateAndTime, TimeStamp, DisplayString, TruthValue, MacAddress, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TimeStamp", "DisplayString", "TruthValue", "MacAddress", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentTbl=equipmentTbl, cardSerialNum=cardSerialNum, eqDiagConfig=eqDiagConfig, cardPrStateChange=cardPrStateChange, shelfNumber=shelfNumber, cmUChannelID=cmUChannelID, portBgpId=portBgpId, cardTestId=cardTestId, auditPassedCount=auditPassedCount, cardTemperatureHighWarn=cardTemperatureHighWarn, portAction=portAction, auditDescription=auditDescription, cardType=cardType, cmResetUpTime=cmResetUpTime, portDescription=portDescription, auditResult=auditResult, cardLastChangeTime=cardLastChangeTime, TestId=TestId, portType=portType, cardTestScheduleCommand=cardTestScheduleCommand, systemGeneralGroup=systemGeneralGroup, cardAction=cardAction, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentCardGroup=equipmentCardGroup, TestScheduleCommand=TestScheduleCommand, cardPrState=cardPrState, fpgaErrorEventGroup=fpgaErrorEventGroup, cmIpAddress=cmIpAddress, cardPicSerialNum=cardPicSerialNum, downstreamPowerLoss=downstreamPowerLoss, cardPicHwVersion=cardPicHwVersion, cardNumPorts=cardNumPorts, portDetectedMode=portDetectedMode, cmVendor=cmVendor, diskVolumeUsageNotification=diskVolumeUsageNotification, systemGeneral=systemGeneral, auditId=auditId, equipmentState=equipmentState, cardTestCommand=cardTestCommand, cardCPLDVersion=cardCPLDVersion, cardEntry=cardEntry, diskVolumeDriveId=diskVolumeDriveId, cardTable=cardTable, auditTable=auditTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmDevice=cmDevice, auditFailedCount=auditFailedCount, cardFwVersion=cardFwVersion, portDplxStatus=portDplxStatus, diskDriveNumber=diskDriveNumber, diskVolumeShelfId=diskVolumeShelfId, portPrStateChange=portPrStateChange, equipmentShelfGroup=equipmentShelfGroup, diskVolumeName=diskVolumeName, auditLogOutput=auditLogOutput, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentTraps=equipmentTraps, cardDplxStatus=cardDplxStatus, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCarrierFwVersion=cardCarrierFwVersion, systemKey=systemKey, cmResetStatus=cmResetStatus, cardSpareGroupMode=cardSpareGroupMode, portTrapInh=portTrapInh, portMinDsPower=portMinDsPower, portTxFlowControlMode=portTxFlowControlMode, cardTestEntry=cardTestEntry, cardSpareGroupId=cardSpareGroupId, cardTestResult=cardTestResult, diagTestId=diagTestId, auditShelfId=auditShelfId, diskVolumeId=diskVolumeId, TestResult=TestResult, cardTemperatureHighError=cardTemperatureHighError, cardFpgaSource=cardFpgaSource, TestCommand=TestCommand, cardShelfTestIndex=cardShelfTestIndex, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCardId=auditCardId, cardPicDetected=cardPicDetected, cadEquipmentMib=cadEquipmentMib, shelfSwVersion=shelfSwVersion, diskVolumeNumber=diskVolumeNumber, cmRegistrationNotification=cmRegistrationNotification, cardSubDetected=cardSubDetected, trapCounter=trapCounter, diskVolumeSize=diskVolumeSize, cardName=cardName, cardTrapInh=cardTrapInh, verboseLevel=verboseLevel, auditAutoScheduling=auditAutoScheduling, cmResetNotification=cmResetNotification, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditTotalFailedCount=auditTotalFailedCount, portMacAddress=portMacAddress, cardTempOverHeatNotification=cardTempOverHeatNotification, cardBootVersion=cardBootVersion, cmResetClearNotification=cmResetClearNotification, cardDetectedChange=cardDetectedChange, diskVolumeFileName=diskVolumeFileName, auditCycleCount=auditCycleCount, cardTestResultNotification=cardTestResultNotification, auditStatus=auditStatus, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestTime=cardTestTime, auditLogThrottle=auditLogThrottle, errEvRecoveryEnabled=errEvRecoveryEnabled, portSecStateChange=portSecStateChange, portNumChans=portNumChans, shelfObjects=shelfObjects, errEvLoggingEnabled=errEvLoggingEnabled, cardUpTime=cardUpTime, TestSchedule=TestSchedule, portLastChangeTime=portLastChangeTime, equipmentAudit=equipmentAudit, diskVolumeTable=diskVolumeTable, diskVolumeCardId=diskVolumeCardId, consoleOutput=consoleOutput, errEvRowStatus=errEvRowStatus, cardSecStateChange=cardSecStateChange, cardTestName=cardTestName, portConnectorId=portConnectorId, portPrState=portPrState, cardFwUpdateStatus=cardFwUpdateStatus, cardCarrierHwVersion=cardCarrierHwVersion, cardHwVersion=cardHwVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, fpgaErrorEventTable=fpgaErrorEventTable, equipmentError=equipmentError, cardTemperature=cardTemperature, cardFlashSize=cardFlashSize, shelfName=shelfName, cardBusSpeed=cardBusSpeed, systemClock=systemClock, cardSubType=cardSubType, portCurrDsPower=portCurrDsPower, portMaxDsPower=portMaxDsPower, cardCpuSpeed=cardCpuSpeed, portTxFlowControlDetected=portTxFlowControlDetected, TestType=TestType, cardTestResultDesc=cardTestResultDesc, cardRamSize=cardRamSize, diskVolumeTrapInh=diskVolumeTrapInh, portDocsIfIndex=portDocsIfIndex, portCardId=portCardId, errEventId=errEventId, cardDetected=cardDetected, diskVolumeDiskSize=diskVolumeDiskSize, cardTestTable=cardTestTable, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastBootSource=cardLastBootSource, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetInfo=cmResetInfo, equipmentPortGroup=equipmentPortGroup, cardDplxStatusChange=cardDplxStatusChange, portCardSubType=portCardSubType, equipmentDiag=equipmentDiag, cardTestType=cardTestType, portTable=portTable, portAdminState=portAdminState, auditName=auditName, cmPrimarySID=cmPrimarySID, cmResetReason=cmResetReason, cardTestTransId=cardTestTransId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestSchedule=cardTestSchedule, equipmentAuditGroup=equipmentAuditGroup, equipmentCardTestGroup=equipmentCardTestGroup, equipmentStateGroup=equipmentStateGroup, TestTransactionId=TestTransactionId, cardSwVersion=cardSwVersion, cardCpuType=cardCpuType, cardTestIndex=cardTestIndex, cmMacAddress=cmMacAddress, errEvLogLevel=errEvLogLevel, portRxFlowControlDetected=portRxFlowControlDetected, auditEntry=auditEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portNumber=portNumber, portId=portId, portRxFlowControlMode=portRxFlowControlMode, portMacIfIndex=portMacIfIndex, trapSeverity=trapSeverity, cardShelfId=cardShelfId, cardLastBootVersion=cardLastBootVersion, cardSecState=cardSecState, cardNumber=cardNumber, remainInDiagMode=remainInDiagMode, portSecState=portSecState, cardId=cardId, cadEquipmentMibConformance=cadEquipmentMibConformance, portDplxStatusChange=portDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, cardAdminState=cardAdminState, portEntry=portEntry, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeEntry=diskVolumeEntry, cardLastResetReason=cardLastResetReason, portShelfId=portShelfId, auditTime=auditTime, auditCommand=auditCommand, portMode=portMode, cardTestDescription=cardTestDescription)
