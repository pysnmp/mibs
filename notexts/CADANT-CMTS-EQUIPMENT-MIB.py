...
FlowControlMode, AdminState, PrimaryState, UnknownState, PortMode, EqActionType, CardSubType, FirmwareSource, PortDetectedMode, ShelfId, PortId, PicType, SecondaryState, CardType, DuplexStatus, DiskVolumeUsageLevel, PortType, CardId = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "AdminState", "PrimaryState", "UnknownState", "PortMode", "EqActionType", "CardSubType", "FirmwareSource", "PortDetectedMode", "ShelfId", "PortId", "PicType", "SecondaryState", "CardType", "DuplexStatus", "DiskVolumeUsageLevel", "PortType", "CardId")
...
TruthValue, RowStatus, TextualConvention, MacAddress, DateAndTime, DisplayString, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "TextualConvention", "MacAddress", "DateAndTime", "DisplayString", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditCycleCount=auditCycleCount, auditTime=auditTime, diskVolumeGroup=diskVolumeGroup, auditTotalFailedCount=auditTotalFailedCount, auditCommand=auditCommand, cardSubDetected=cardSubDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentTraps=equipmentTraps, cardLastBootVersion=cardLastBootVersion, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResultDesc=cardTestResultDesc, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, portAdminState=portAdminState, cadEquipmentMib=cadEquipmentMib, portMode=portMode, portDetectedMode=portDetectedMode, cardTestDescription=cardTestDescription, cardTestTime=cardTestTime, auditName=auditName, equipmentState=equipmentState, cadEquipmentMibGroup=cadEquipmentMibGroup, auditShelfId=auditShelfId, cmIpAddress=cmIpAddress, cardTempNoReportNotification=cardTempNoReportNotification, systemKey=systemKey, cardPrStateChange=cardPrStateChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTable=cardTestTable, PYSNMP_MODULE_ID=cadEquipmentMib, cardSecState=cardSecState, cmUChannelID=cmUChannelID, cmDevice=cmDevice, cardCpuSpeed=cardCpuSpeed, cardTestName=cardTestName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardBootVersion=cardBootVersion, equipmentAudit=equipmentAudit, cardDetectedChange=cardDetectedChange, shelfNumber=shelfNumber, cardTestCommand=cardTestCommand, errEvRowStatus=errEvRowStatus, cmResetNotification=cmResetNotification, systemGeneralGroup=systemGeneralGroup, TestCommand=TestCommand, diagTestId=diagTestId, cardTestType=cardTestType, diskVolumeCardId=diskVolumeCardId, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portCardSubType=portCardSubType, portSecStateChange=portSecStateChange, cardDplxStatusChange=cardDplxStatusChange, portTrapInh=portTrapInh, cardDetected=cardDetected, auditTotalPassedCount=auditTotalPassedCount, cmResetStatus=cmResetStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, portTxFlowControlMode=portTxFlowControlMode, portRxFlowControlMode=portRxFlowControlMode, cardNumber=cardNumber, cardTestTransId=cardTestTransId, cardPicSerialNum=cardPicSerialNum, auditTable=auditTable, cardFwUpdateStatus=cardFwUpdateStatus, cardTestResult=cardTestResult, cardCpuType=cardCpuType, cardCPLDVersion=cardCPLDVersion, portPrState=portPrState, TestResult=TestResult, shelfName=shelfName, equipmentDiag=equipmentDiag, downstreamPowerLoss=downstreamPowerLoss, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeTable=diskVolumeTable, cardLastResetReason=cardLastResetReason, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestId=TestId, diskDriveNumber=diskDriveNumber, cmResetReason=cmResetReason, diskVolumeFileName=diskVolumeFileName, cardTestIndex=cardTestIndex, auditLogOutput=auditLogOutput, equipmentTbl=equipmentTbl, portConnectorId=portConnectorId, portPrStateChange=portPrStateChange, cardTestResultNotification=cardTestResultNotification, auditEntry=auditEntry, cardFlashSize=cardFlashSize, cardCarrierSerialNum=cardCarrierSerialNum, cardCarrierHwVersion=cardCarrierHwVersion, eqDiagConfig=eqDiagConfig, errEvRecoveryEnabled=errEvRecoveryEnabled, portNumber=portNumber, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentCardGroup=equipmentCardGroup, errEvLogLevel=errEvLogLevel, cardTable=cardTable, trapSeverity=trapSeverity, diskVolumeSize=diskVolumeSize, portTable=portTable, auditResult=auditResult, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeEntry=diskVolumeEntry, TestScheduleCommand=TestScheduleCommand, fpgaErrorEventTable=fpgaErrorEventTable, TestType=TestType, portRxFlowControlDetected=portRxFlowControlDetected, cmRegistrationNotification=cmRegistrationNotification, cardId=cardId, cardShelfId=cardShelfId, TestTransactionId=TestTransactionId, diskVolumeShelfId=diskVolumeShelfId, cardSecStateChange=cardSecStateChange, auditAutoScheduling=auditAutoScheduling, cardAction=cardAction, cardPicHwVersion=cardPicHwVersion, auditDescription=auditDescription, cmResetInfo=cmResetInfo, systemGeneral=systemGeneral, portMacAddress=portMacAddress, auditLogThrottle=auditLogThrottle, auditStatus=auditStatus, cardSpareGroupMode=cardSpareGroupMode, cardTestId=cardTestId, portType=portType, diskVolumeId=diskVolumeId, portTxFlowControlDetected=portTxFlowControlDetected, trapCounter=trapCounter, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMacIfIndex=portMacIfIndex, equipmentPortGroup=equipmentPortGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentShelfGroup=equipmentShelfGroup, cardTemperature=cardTemperature, cmPrimarySID=cmPrimarySID, portMinDsPower=portMinDsPower, portCardId=portCardId, diskVolumeNumber=diskVolumeNumber, cardPrState=cardPrState, cardCarrierFwVersion=cardCarrierFwVersion, cardEntry=cardEntry, portDplxStatus=portDplxStatus, cardShelfTestIndex=cardShelfTestIndex, cardNumPorts=cardNumPorts, portMaxDsPower=portMaxDsPower, verboseLevel=verboseLevel, cardHwVersion=cardHwVersion, cardTemperatureHighError=cardTemperatureHighError, cardSpareGroupId=cardSpareGroupId, cardRamSize=cardRamSize, diskVolumeTrapInh=diskVolumeTrapInh, auditCardId=auditCardId, shelfObjects=shelfObjects, cardSerialNum=cardSerialNum, auditId=auditId, equipmentError=equipmentError, equipmentDiagGroup=equipmentDiagGroup, portBgpId=portBgpId, portLastChangeTime=portLastChangeTime, auditFailedCount=auditFailedCount, portAction=portAction, shelfSwVersion=shelfSwVersion, cardBusSpeed=cardBusSpeed, portShelfId=portShelfId, portDescription=portDescription, cardTestEntry=cardTestEntry, TestSchedule=TestSchedule, cmMacAddress=cmMacAddress, remainInDiagMode=remainInDiagMode, cardDplxStatus=cardDplxStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portSecState=portSecState, auditPassedCount=auditPassedCount, cardName=cardName, cardUpTime=cardUpTime, cardAdminState=cardAdminState, equipmentStateGroup=equipmentStateGroup, cardPicDetected=cardPicDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, cardType=cardType, cardTrapInh=cardTrapInh, portId=portId, portNumChans=portNumChans, portCurrDsPower=portCurrDsPower, diskVolumeDriveId=diskVolumeDriveId, cmResetUpTime=cmResetUpTime, errEventId=errEventId, cmVendor=cmVendor, diskVolumeName=diskVolumeName, cardSubType=cardSubType, equipmentCardTestGroup=equipmentCardTestGroup, cardSwVersion=cardSwVersion, consoleOutput=consoleOutput, systemClock=systemClock, cardFpgaSource=cardFpgaSource, equipmentAuditGroup=equipmentAuditGroup, cardTestSchedule=cardTestSchedule, portDocsIfIndex=portDocsIfIndex, cardLastBootSource=cardLastBootSource, portEntry=portEntry, cmResetClearNotification=cmResetClearNotification, cardFwVersion=cardFwVersion)
