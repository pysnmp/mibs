...
EqActionType, PrimaryState, PortType, PortDetectedMode, CardType, PortMode, ShelfId, PicType, FirmwareSource, CardSubType, PortId, DuplexStatus, SecondaryState, CardId, DiskVolumeUsageLevel, AdminState, FlowControlMode, UnknownState = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "PrimaryState", "PortType", "PortDetectedMode", "CardType", "PortMode", "ShelfId", "PicType", "FirmwareSource", "CardSubType", "PortId", "DuplexStatus", "SecondaryState", "CardId", "DiskVolumeUsageLevel", "AdminState", "FlowControlMode", "UnknownState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCpuType=cardCpuType, portLastChangeTime=portLastChangeTime, portBgpId=portBgpId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSecState=cardSecState, cmMacAddress=cmMacAddress, equipmentTraps=equipmentTraps, cardDetectedChange=cardDetectedChange, cardEntry=cardEntry, auditName=auditName, cmResetStatus=cmResetStatus, errEvLoggingEnabled=errEvLoggingEnabled, cardShelfId=cardShelfId, cardType=cardType, cardTestType=cardTestType, cardBusSpeed=cardBusSpeed, portDplxStatus=portDplxStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfObjects=shelfObjects, portAction=portAction, shelfNumber=shelfNumber, auditCardId=auditCardId, portCardSubType=portCardSubType, fpgaErrorEventEntry=fpgaErrorEventEntry, errEvRowStatus=errEvRowStatus, cardTestDescription=cardTestDescription, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSecStateChange=cardSecStateChange, PYSNMP_MODULE_ID=cadEquipmentMib, systemKey=systemKey, cmPrimarySID=cmPrimarySID, cardLastBootSource=cardLastBootSource, errEventId=errEventId, portNumChans=portNumChans, portTxFlowControlDetected=portTxFlowControlDetected, portMinDsPower=portMinDsPower, TestId=TestId, auditCycleCount=auditCycleCount, cardTestTransId=cardTestTransId, auditEntry=auditEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCarrierFwVersion=cardCarrierFwVersion, cardSwVersion=cardSwVersion, cardTemperature=cardTemperature, diskVolumeEntry=diskVolumeEntry, cardShelfTestIndex=cardShelfTestIndex, portDocsIfIndex=portDocsIfIndex, cardTestIndex=cardTestIndex, cardHwVersion=cardHwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTemperatureHighError=cardTemperatureHighError, cardTestName=cardTestName, cardTestResult=cardTestResult, downstreamPowerLoss=downstreamPowerLoss, cardSubDetected=cardSubDetected, portSecStateChange=portSecStateChange, errEvLogLevel=errEvLogLevel, cardSubType=cardSubType, cardPrStateChange=cardPrStateChange, equipmentDiagGroup=equipmentDiagGroup, auditFailedCount=auditFailedCount, cardLastChangeTime=cardLastChangeTime, portTxFlowControlMode=portTxFlowControlMode, cardTestCommand=cardTestCommand, cardCarrierSerialNum=cardCarrierSerialNum, cardRamSize=cardRamSize, portDplxStatusChange=portDplxStatusChange, portType=portType, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeCardId=diskVolumeCardId, diskVolumeSize=diskVolumeSize, diskVolumeDiskSize=diskVolumeDiskSize, portPrStateChange=portPrStateChange, equipmentCardTestGroup=equipmentCardTestGroup, portTrapInh=portTrapInh, equipmentDiag=equipmentDiag, equipmentTbl=equipmentTbl, auditTable=auditTable, cardCpuSpeed=cardCpuSpeed, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardDetected=cardDetected, cardCPLDVersion=cardCPLDVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, trapCounter=trapCounter, portDescription=portDescription, portRxFlowControlDetected=portRxFlowControlDetected, portTable=portTable, cardNumPorts=cardNumPorts, portConnectorId=portConnectorId, cardTempNoReportNotification=cardTempNoReportNotification, TestScheduleCommand=TestScheduleCommand, cardLastBootVersion=cardLastBootVersion, equipmentAudit=equipmentAudit, cmVendor=cmVendor, portId=portId, cardTestResultNotification=cardTestResultNotification, TestCommand=TestCommand, portMacIfIndex=portMacIfIndex, cardNumber=cardNumber, portDetectedMode=portDetectedMode, portAdminState=portAdminState, remainInDiagMode=remainInDiagMode, auditLogThrottle=auditLogThrottle, cardSpareGroupId=cardSpareGroupId, equipmentCardGroup=equipmentCardGroup, cardSerialNum=cardSerialNum, cardPicSerialNum=cardPicSerialNum, cardPicDetected=cardPicDetected, diskVolumeGroup=diskVolumeGroup, cmUChannelID=cmUChannelID, eqDiagConfig=eqDiagConfig, equipmentShelfGroup=equipmentShelfGroup, cardDplxStatusChange=cardDplxStatusChange, cardSpareGroupMode=cardSpareGroupMode, cardTestTime=cardTestTime, cmDevice=cmDevice, cardPrState=cardPrState, portSecState=portSecState, equipmentPortGroup=equipmentPortGroup, TestResult=TestResult, portCurrDsPower=portCurrDsPower, portNumber=portNumber, diskVolumeId=diskVolumeId, cmIpAddress=cmIpAddress, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditAutoScheduling=auditAutoScheduling, auditStatus=auditStatus, shelfName=shelfName, diskVolumeTable=diskVolumeTable, cardPicHwVersion=cardPicHwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditDescription=auditDescription, fpgaErrorEventTable=fpgaErrorEventTable, cmResetNotification=cmResetNotification, cardAdminState=cardAdminState, cardAction=cardAction, portMode=portMode, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTable=cardTable, cardLastResetReason=cardLastResetReason, auditTotalFailedCount=auditTotalFailedCount, equipmentAuditGroup=equipmentAuditGroup, auditPassedCount=auditPassedCount, cardId=cardId, cmResetUpTime=cmResetUpTime, cmResetClearNotification=cmResetClearNotification, verboseLevel=verboseLevel, cardFwVersion=cardFwVersion, systemGeneralGroup=systemGeneralGroup, TestType=TestType, TestTransactionId=TestTransactionId, diskVolumeName=diskVolumeName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmRegistrationNotification=cmRegistrationNotification, portMaxDsPower=portMaxDsPower, portEntry=portEntry, cardTrapInh=cardTrapInh, diskDriveNumber=diskDriveNumber, cardFlashSize=cardFlashSize, diskVolumeDriveId=diskVolumeDriveId, equipmentState=equipmentState, auditTotalPassedCount=auditTotalPassedCount, cardTestTable=cardTestTable, cadEquipmentMibConformance=cadEquipmentMibConformance, auditTime=auditTime, equipmentError=equipmentError, cardTestSchedule=cardTestSchedule, portCardId=portCardId, systemGeneral=systemGeneral, auditResult=auditResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeNumber=diskVolumeNumber, cmResetReason=cmResetReason, trapSeverity=trapSeverity, portPrState=portPrState, cardBootVersion=cardBootVersion, auditCommand=auditCommand, auditShelfId=auditShelfId, portRxFlowControlMode=portRxFlowControlMode, fpgaErrorEventGroup=fpgaErrorEventGroup, TestSchedule=TestSchedule, cardName=cardName, portMacAddress=portMacAddress, cardDplxStatus=cardDplxStatus, cardUpTime=cardUpTime, systemClock=systemClock, cardCarrierHwVersion=cardCarrierHwVersion, equipmentStateGroup=equipmentStateGroup, cardTestEntry=cardTestEntry, portShelfId=portShelfId, diagTestId=diagTestId, diskVolumeUsageLevel=diskVolumeUsageLevel, auditLogOutput=auditLogOutput, consoleOutput=consoleOutput, cardFpgaSource=cardFpgaSource, cmResetInfo=cmResetInfo, diskVolumeTrapInh=diskVolumeTrapInh, cardTestId=cardTestId, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeFileName=diskVolumeFileName, auditId=auditId, shelfSwVersion=shelfSwVersion, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMib=cadEquipmentMib, cardTestResultDesc=cardTestResultDesc)
