...
UnknownState, CardSubType, AdminState, CardId, DuplexStatus, PortMode, PrimaryState, PortDetectedMode, FirmwareSource, DiskVolumeUsageLevel, PicType, ShelfId, EqActionType, PortType, SecondaryState, FlowControlMode, CardType, PortId = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "CardSubType", "AdminState", "CardId", "DuplexStatus", "PortMode", "PrimaryState", "PortDetectedMode", "FirmwareSource", "DiskVolumeUsageLevel", "PicType", "ShelfId", "EqActionType", "PortType", "SecondaryState", "FlowControlMode", "CardType", "PortId")
...
TextualConvention, RowStatus, TimeStamp, DateAndTime, TruthValue, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "TimeStamp", "DateAndTime", "TruthValue", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfSwVersion=shelfSwVersion, cardFpgaSource=cardFpgaSource, cardTestIndex=cardTestIndex, cardTestSchedule=cardTestSchedule, auditCommand=auditCommand, systemGeneral=systemGeneral, cardCpuType=cardCpuType, cardAdminState=cardAdminState, diskVolumeFileName=diskVolumeFileName, cardShelfTestIndex=cardShelfTestIndex, equipmentTbl=equipmentTbl, cardCpuSpeed=cardCpuSpeed, cardPrStateChange=cardPrStateChange, portConnectorId=portConnectorId, diskVolumeDriveId=diskVolumeDriveId, diskVolumeId=diskVolumeId, verboseLevel=verboseLevel, cardTestTable=cardTestTable, equipmentState=equipmentState, auditDescription=auditDescription, cardShelfId=cardShelfId, equipmentAuditGroup=equipmentAuditGroup, consoleOutput=consoleOutput, cardPicSerialNum=cardPicSerialNum, errEvRowStatus=errEvRowStatus, cardTempNoReportNotification=cardTempNoReportNotification, auditTotalPassedCount=auditTotalPassedCount, TestResult=TestResult, cmIpAddress=cmIpAddress, auditName=auditName, diskVolumeTable=diskVolumeTable, cardTestType=cardTestType, equipmentCardGroup=equipmentCardGroup, cardSerialNum=cardSerialNum, TestSchedule=TestSchedule, cardTestTime=cardTestTime, cardUpTime=cardUpTime, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardFwVersion=cardFwVersion, cardDetected=cardDetected, shelfObjects=shelfObjects, errEventId=errEventId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMib=cadEquipmentMib, cardSecStateChange=cardSecStateChange, auditEntry=auditEntry, cmUChannelID=cmUChannelID, equipmentError=equipmentError, cardTestName=cardTestName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestTransactionId=TestTransactionId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeGroup=diskVolumeGroup, cardSubType=cardSubType, auditTable=auditTable, portType=portType, portLastChangeTime=portLastChangeTime, portTable=portTable, portShelfId=portShelfId, PYSNMP_MODULE_ID=cadEquipmentMib, TestScheduleCommand=TestScheduleCommand, TestCommand=TestCommand, portSecStateChange=portSecStateChange, portTrapInh=portTrapInh, cardTestEntry=cardTestEntry, cardEntry=cardEntry, diagTestId=diagTestId, cardCarrierSerialNum=cardCarrierSerialNum, portDetectedMode=portDetectedMode, diskVolumeShelfId=diskVolumeShelfId, cardTempOverHeatNotification=cardTempOverHeatNotification, portAction=portAction, portNumChans=portNumChans, trapSeverity=trapSeverity, equipmentShelfGroup=equipmentShelfGroup, shelfName=shelfName, auditLogOutput=auditLogOutput, portCurrDsPower=portCurrDsPower, cardTrapInh=cardTrapInh, portDescription=portDescription, cardCarrierHwVersion=cardCarrierHwVersion, cardAction=cardAction, remainInDiagMode=remainInDiagMode, cardCPLDVersion=cardCPLDVersion, portRxFlowControlMode=portRxFlowControlMode, equipmentPortGroup=equipmentPortGroup, cardSwVersion=cardSwVersion, cardFwUpdateStatus=cardFwUpdateStatus, portCardId=portCardId, cmResetStatus=cmResetStatus, systemGeneralGroup=systemGeneralGroup, diskVolumeDiskSize=diskVolumeDiskSize, portId=portId, cmPrimarySID=cmPrimarySID, cardFlashSize=cardFlashSize, TestType=TestType, cardSecState=cardSecState, cardTemperatureHighWarn=cardTemperatureHighWarn, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portPrState=portPrState, fpgaErrorEventTable=fpgaErrorEventTable, systemClock=systemClock, cardTestId=cardTestId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardRamSize=cardRamSize, cardTemperatureHighError=cardTemperatureHighError, cardLastBootVersion=cardLastBootVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditPassedCount=auditPassedCount, cardTestResultDesc=cardTestResultDesc, portMacIfIndex=portMacIfIndex, diskVolumeNumber=diskVolumeNumber, portNumber=portNumber, shelfNumber=shelfNumber, cmResetNotification=cmResetNotification, diskVolumeTrapInh=diskVolumeTrapInh, cardTestDescription=cardTestDescription, portRxFlowControlDetected=portRxFlowControlDetected, cardCarrierFwVersion=cardCarrierFwVersion, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeEntry=diskVolumeEntry, equipmentAudit=equipmentAudit, cardNumber=cardNumber, cardTestScheduleCommand=cardTestScheduleCommand, equipmentDiag=equipmentDiag, auditTotalFailedCount=auditTotalFailedCount, auditCardId=auditCardId, cardBootVersion=cardBootVersion, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, auditFailedCount=auditFailedCount, cardDplxStatus=cardDplxStatus, cardName=cardName, diskVolumeUsageLevel=diskVolumeUsageLevel, auditCycleCount=auditCycleCount, portBgpId=portBgpId, auditShelfId=auditShelfId, cmVendor=cmVendor, errEvLogLevel=errEvLogLevel, diskDriveNumber=diskDriveNumber, systemKey=systemKey, portCardSubType=portCardSubType, cmResetClearNotification=cmResetClearNotification, auditLogThrottle=auditLogThrottle, cmResetUpTime=cmResetUpTime, portAdminState=portAdminState, auditAutoScheduling=auditAutoScheduling, cardDplxStatusChange=cardDplxStatusChange, equipmentTraps=equipmentTraps, diskVolumeCardId=diskVolumeCardId, diskVolumeUsageNotification=diskVolumeUsageNotification, portMacAddress=portMacAddress, cardTemperature=cardTemperature, TestId=TestId, cardSpareGroupId=cardSpareGroupId, diskVolumeSize=diskVolumeSize, equipmentStateGroup=equipmentStateGroup, cardTestResult=cardTestResult, auditTime=auditTime, eqDiagConfig=eqDiagConfig, portDplxStatusChange=portDplxStatusChange, trapCounter=trapCounter, cadEquipmentMibGroup=cadEquipmentMibGroup, cmMacAddress=cmMacAddress, diskVolumeName=diskVolumeName, cardLastBootSource=cardLastBootSource, portMode=portMode, cardTestResultNotification=cardTestResultNotification, cadEquipmentMibConformance=cadEquipmentMibConformance, cardType=cardType, cmDevice=cmDevice, downstreamPowerLoss=downstreamPowerLoss, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTable=cardTable, portDplxStatus=portDplxStatus, cardNumPorts=cardNumPorts, cardTestCommand=cardTestCommand, cardLastResetReason=cardLastResetReason, cmResetReason=cmResetReason, portDocsIfIndex=portDocsIfIndex, cardPicHwVersion=cardPicHwVersion, auditStatus=auditStatus, portPrStateChange=portPrStateChange, cardId=cardId, auditResult=auditResult, cardPrState=cardPrState, cardDetectedChange=cardDetectedChange, cardPicDetected=cardPicDetected, cardSpareGroupMode=cardSpareGroupMode, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentCardTestGroup=equipmentCardTestGroup, portEntry=portEntry, cardBusSpeed=cardBusSpeed, portSecState=portSecState, portMinDsPower=portMinDsPower, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestTransId=cardTestTransId, cmRegistrationNotification=cmRegistrationNotification, equipmentDiagGroup=equipmentDiagGroup, auditId=auditId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardHwVersion=cardHwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSubDetected=cardSubDetected, cmResetInfo=cmResetInfo)
