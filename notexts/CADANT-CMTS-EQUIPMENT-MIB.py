...
PrimaryState, DuplexStatus, PortType, SecondaryState, FirmwareSource, PicType, DiskVolumeUsageLevel, FlowControlMode, CardSubType, PortMode, CardId, CardType, EqActionType, PortId, UnknownState, AdminState, ShelfId, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "DuplexStatus", "PortType", "SecondaryState", "FirmwareSource", "PicType", "DiskVolumeUsageLevel", "FlowControlMode", "CardSubType", "PortMode", "CardId", "CardType", "EqActionType", "PortId", "UnknownState", "AdminState", "ShelfId", "PortDetectedMode")
...
RowStatus, TimeStamp, DateAndTime, TextualConvention, DisplayString, TruthValue, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "DateAndTime", "TextualConvention", "DisplayString", "TruthValue", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageNotification=diskVolumeUsageNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, TestScheduleCommand=TestScheduleCommand, systemGeneral=systemGeneral, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeCardId=diskVolumeCardId, equipmentError=equipmentError, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentShelfGroup=equipmentShelfGroup, PYSNMP_MODULE_ID=cadEquipmentMib, auditId=auditId, cmResetUpTime=cmResetUpTime, cardId=cardId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardEntry=cardEntry, fpgaErrorEventEntry=fpgaErrorEventEntry, shelfSwVersion=shelfSwVersion, portBgpId=portBgpId, portTable=portTable, cardLastChangeTime=cardLastChangeTime, cardFwVersion=cardFwVersion, auditTime=auditTime, TestId=TestId, portCardId=portCardId, cardTrapInh=cardTrapInh, cardRamSize=cardRamSize, auditAutoScheduling=auditAutoScheduling, cardPicHwVersion=cardPicHwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSubType=cardSubType, equipmentDiag=equipmentDiag, diskVolumeSize=diskVolumeSize, cardTable=cardTable, errEventId=errEventId, auditEntry=auditEntry, auditTable=auditTable, cardPicSerialNum=cardPicSerialNum, TestSchedule=TestSchedule, auditStatus=auditStatus, equipmentCardTestGroup=equipmentCardTestGroup, equipmentState=equipmentState, auditResult=auditResult, portLastChangeTime=portLastChangeTime, cmVendor=cmVendor, portNumChans=portNumChans, portType=portType, diskVolumeGroup=diskVolumeGroup, auditShelfId=auditShelfId, equipmentStateGroup=equipmentStateGroup, cmResetNotification=cmResetNotification, diskVolumeDiskSize=diskVolumeDiskSize, systemKey=systemKey, cardTestIndex=cardTestIndex, portNumber=portNumber, cardUpTime=cardUpTime, cardSwVersion=cardSwVersion, equipmentDiagGroup=equipmentDiagGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestTime=cardTestTime, cardDetectedChange=cardDetectedChange, portMode=portMode, cardNumber=cardNumber, cardTestResult=cardTestResult, systemGeneralGroup=systemGeneralGroup, equipmentCardGroup=equipmentCardGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardTestSchedule=cardTestSchedule, cardBootVersion=cardBootVersion, cardPrStateChange=cardPrStateChange, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, consoleOutput=consoleOutput, auditCommand=auditCommand, auditCardId=auditCardId, cardDetected=cardDetected, auditFailedCount=auditFailedCount, cardSecState=cardSecState, cardTestId=cardTestId, auditLogOutput=auditLogOutput, cardTemperature=cardTemperature, shelfName=shelfName, cardHwVersion=cardHwVersion, cardFlashSize=cardFlashSize, systemClock=systemClock, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeEntry=diskVolumeEntry, cardShelfId=cardShelfId, portCurrDsPower=portCurrDsPower, equipmentTbl=equipmentTbl, cardPicDetected=cardPicDetected, cardFpgaSource=cardFpgaSource, cardTestResultNotification=cardTestResultNotification, diskVolumeTable=diskVolumeTable, auditDescription=auditDescription, auditTotalFailedCount=auditTotalFailedCount, errEvRowStatus=errEvRowStatus, diskDriveNumber=diskDriveNumber, TestResult=TestResult, diskVolumeDriveId=diskVolumeDriveId, portDplxStatusChange=portDplxStatusChange, portAction=portAction, cardTestTransId=cardTestTransId, cmUChannelID=cmUChannelID, cadEquipmentMibConformance=cadEquipmentMibConformance, cardAction=cardAction, portMaxDsPower=portMaxDsPower, auditLogThrottle=auditLogThrottle, auditPassedCount=auditPassedCount, cardTestTable=cardTestTable, portPrStateChange=portPrStateChange, diskVolumeNumber=diskVolumeNumber, cardAdminState=cardAdminState, cmResetStatus=cmResetStatus, downstreamPowerLoss=downstreamPowerLoss, portDetectedMode=portDetectedMode, fpgaErrorEventGroup=fpgaErrorEventGroup, portMacAddress=portMacAddress, cardType=cardType, TestType=TestType, cardCpuType=cardCpuType, portEntry=portEntry, portMacIfIndex=portMacIfIndex, equipmentTraps=equipmentTraps, trapCounter=trapCounter, cardCarrierFwVersion=cardCarrierFwVersion, cardTestDescription=cardTestDescription, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeId=diskVolumeId, cardTestEntry=cardTestEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSerialNum=cardSerialNum, cardLastBootSource=cardLastBootSource, cmMacAddress=cmMacAddress, TestTransactionId=TestTransactionId, trapSeverity=trapSeverity, cardTemperatureHighError=cardTemperatureHighError, cmDevice=cmDevice, portTrapInh=portTrapInh, diskVolumeShelfId=diskVolumeShelfId, auditTotalPassedCount=auditTotalPassedCount, cmIpAddress=cmIpAddress, cardSecStateChange=cardSecStateChange, cardBusSpeed=cardBusSpeed, cardNumPorts=cardNumPorts, diskVolumeFileName=diskVolumeFileName, eqDiagConfig=eqDiagConfig, shelfObjects=shelfObjects, diagTestId=diagTestId, portAdminState=portAdminState, cardName=cardName, errEvLogLevel=errEvLogLevel, portSecState=portSecState, cadEquipmentMib=cadEquipmentMib, portTxFlowControlDetected=portTxFlowControlDetected, cardPrState=cardPrState, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portShelfId=portShelfId, portDescription=portDescription, portSecStateChange=portSecStateChange, equipmentAudit=equipmentAudit, portDplxStatus=portDplxStatus, cardTestType=cardTestType, cmResetClearNotification=cmResetClearNotification, cardCarrierSerialNum=cardCarrierSerialNum, cardLastBootVersion=cardLastBootVersion, portRxFlowControlMode=portRxFlowControlMode, cardTestName=cardTestName, portConnectorId=portConnectorId, cardShelfTestIndex=cardShelfTestIndex, cmResetInfo=cmResetInfo, portCardSubType=portCardSubType, errEvLoggingEnabled=errEvLoggingEnabled, cmRegistrationNotification=cmRegistrationNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, portPrState=portPrState, cmPrimarySID=cmPrimarySID, portTxFlowControlMode=portTxFlowControlMode, cardDplxStatus=cardDplxStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portId=portId, equipmentPortGroup=equipmentPortGroup, auditCycleCount=auditCycleCount, cardCarrierHwVersion=cardCarrierHwVersion, cardSpareGroupMode=cardSpareGroupMode, TestCommand=TestCommand, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeName=diskVolumeName, cardTestCommand=cardTestCommand, cardSubDetected=cardSubDetected, cardDplxStatusChange=cardDplxStatusChange, equipmentAuditGroup=equipmentAuditGroup, fpgaErrorEventTable=fpgaErrorEventTable, cardCpuSpeed=cardCpuSpeed, auditName=auditName, portMinDsPower=portMinDsPower, cardTestResultDesc=cardTestResultDesc, cmResetReason=cmResetReason, cardSpareGroupId=cardSpareGroupId, remainInDiagMode=remainInDiagMode, portDocsIfIndex=portDocsIfIndex, verboseLevel=verboseLevel, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardLastResetReason=cardLastResetReason, shelfNumber=shelfNumber, cardCPLDVersion=cardCPLDVersion, cardTestScheduleCommand=cardTestScheduleCommand)
