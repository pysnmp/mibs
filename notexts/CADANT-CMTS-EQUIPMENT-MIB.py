...
DiskVolumeUsageLevel, PicType, ShelfId, CardType, PortType, UnknownState, CardId, CardSubType, AdminState, PrimaryState, SecondaryState, PortDetectedMode, DuplexStatus, PortMode, PortId, FlowControlMode, FirmwareSource, EqActionType = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "PicType", "ShelfId", "CardType", "PortType", "UnknownState", "CardId", "CardSubType", "AdminState", "PrimaryState", "SecondaryState", "PortDetectedMode", "DuplexStatus", "PortMode", "PortId", "FlowControlMode", "FirmwareSource", "EqActionType")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portMode=portMode, cardDplxStatus=cardDplxStatus, cardTestTable=cardTestTable, cardDetectedChange=cardDetectedChange, cardSpareGroupMode=cardSpareGroupMode, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, portCardSubType=portCardSubType, TestResult=TestResult, cardAdminState=cardAdminState, trapSeverity=trapSeverity, cadEquipmentMibGroup=cadEquipmentMibGroup, portBgpId=portBgpId, equipmentError=equipmentError, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentDiagGroup=equipmentDiagGroup, cardTestTransId=cardTestTransId, equipmentCardGroup=equipmentCardGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestId=TestId, diskVolumeFileName=diskVolumeFileName, cardSwVersion=cardSwVersion, cardFwUpdateStatus=cardFwUpdateStatus, portDetectedMode=portDetectedMode, equipmentAudit=equipmentAudit, portDplxStatusChange=portDplxStatusChange, cardCpuSpeed=cardCpuSpeed, cardTestTime=cardTestTime, cardPrStateChange=cardPrStateChange, cardLastResetReason=cardLastResetReason, auditLogThrottle=auditLogThrottle, auditEntry=auditEntry, auditStatus=auditStatus, cardPicSerialNum=cardPicSerialNum, cardCPLDVersion=cardCPLDVersion, errEvRowStatus=errEvRowStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditDescription=auditDescription, errEventId=errEventId, cardSerialNum=cardSerialNum, consoleOutput=consoleOutput, cardShelfId=cardShelfId, auditTotalPassedCount=auditTotalPassedCount, cardLastChangeTime=cardLastChangeTime, equipmentCardTestGroup=equipmentCardTestGroup, cardNumPorts=cardNumPorts, cardPrState=cardPrState, cardFlashSize=cardFlashSize, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portTxFlowControlMode=portTxFlowControlMode, cardTestName=cardTestName, auditCycleCount=auditCycleCount, TestSchedule=TestSchedule, portRxFlowControlMode=portRxFlowControlMode, cmUChannelID=cmUChannelID, trapCounter=trapCounter, equipmentState=equipmentState, auditLogOutput=auditLogOutput, portSecStateChange=portSecStateChange, cardTrapInh=cardTrapInh, cardEntry=cardEntry, cardTestSchedule=cardTestSchedule, equipmentStateGroup=equipmentStateGroup, portDescription=portDescription, auditTotalFailedCount=auditTotalFailedCount, cardFwVersion=cardFwVersion, cardTestScheduleCommand=cardTestScheduleCommand, fpgaErrorEventTable=fpgaErrorEventTable, portTable=portTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, PYSNMP_MODULE_ID=cadEquipmentMib, shelfNumber=shelfNumber, equipmentTbl=equipmentTbl, diskVolumeTrapInh=diskVolumeTrapInh, downstreamPowerLoss=downstreamPowerLoss, portType=portType, cardBusSpeed=cardBusSpeed, diskVolumeUsageLevel=diskVolumeUsageLevel, portId=portId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardAction=cardAction, portMaxDsPower=portMaxDsPower, cmResetUpTime=cmResetUpTime, fpgaErrorEventEntry=fpgaErrorEventEntry, cardName=cardName, diskVolumeEntry=diskVolumeEntry, cardSubType=cardSubType, portShelfId=portShelfId, portAction=portAction, cmDevice=cmDevice, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestResultNotification=cardTestResultNotification, auditFailedCount=auditFailedCount, cadEquipmentMib=cadEquipmentMib, shelfName=shelfName, diskVolumeNumber=diskVolumeNumber, cardDplxStatusChange=cardDplxStatusChange, TestType=TestType, auditTable=auditTable, cardTestType=cardTestType, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeTable=diskVolumeTable, equipmentDiag=equipmentDiag, portSecState=portSecState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditPassedCount=auditPassedCount, cardUpTime=cardUpTime, cardSubDetected=cardSubDetected, cardTestIndex=cardTestIndex, portPrState=portPrState, systemGeneralGroup=systemGeneralGroup, cmResetNotification=cmResetNotification, eqDiagConfig=eqDiagConfig, cardTestResult=cardTestResult, auditAutoScheduling=auditAutoScheduling, verboseLevel=verboseLevel, auditTime=auditTime, TestScheduleCommand=TestScheduleCommand, cmVendor=cmVendor, cardTempNoReportNotification=cardTempNoReportNotification, TestCommand=TestCommand, shelfSwVersion=shelfSwVersion, cardRamSize=cardRamSize, diskVolumeId=diskVolumeId, portAdminState=portAdminState, auditCommand=auditCommand, diskVolumeUsageNotification=diskVolumeUsageNotification, cmResetStatus=cmResetStatus, cmResetClearNotification=cmResetClearNotification, auditShelfId=auditShelfId, cardLastBootSource=cardLastBootSource, cardDetected=cardDetected, cardTestDescription=cardTestDescription, portConnectorId=portConnectorId, cardId=cardId, cardTestId=cardTestId, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeGroup=diskVolumeGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, cardBootVersion=cardBootVersion, portPrStateChange=portPrStateChange, portNumber=portNumber, cardShelfTestIndex=cardShelfTestIndex, portEntry=portEntry, cardType=cardType, cardCpuType=cardCpuType, portCurrDsPower=portCurrDsPower, cmResetReason=cmResetReason, diskVolumeSize=diskVolumeSize, diagTestId=diagTestId, equipmentAuditGroup=equipmentAuditGroup, cardTestEntry=cardTestEntry, cardSecStateChange=cardSecStateChange, cardTemperature=cardTemperature, diskDriveNumber=diskDriveNumber, portLastChangeTime=portLastChangeTime, portMacIfIndex=portMacIfIndex, equipmentPortGroup=equipmentPortGroup, cardSpareGroupId=cardSpareGroupId, portCardId=portCardId, cardNumber=cardNumber, diskVolumeShelfId=diskVolumeShelfId, auditCardId=auditCardId, cardCarrierHwVersion=cardCarrierHwVersion, portDocsIfIndex=portDocsIfIndex, equipmentShelfGroup=equipmentShelfGroup, systemGeneral=systemGeneral, systemClock=systemClock, cardTable=cardTable, cardFpgaSource=cardFpgaSource, fpgaErrorEventGroup=fpgaErrorEventGroup, portTrapInh=portTrapInh, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeCardId=diskVolumeCardId, cmResetInfo=cmResetInfo, cardLastBootVersion=cardLastBootVersion, TestTransactionId=TestTransactionId, cardPicHwVersion=cardPicHwVersion, cardPicDetected=cardPicDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, remainInDiagMode=remainInDiagMode, cardHwVersion=cardHwVersion, portMinDsPower=portMinDsPower, auditResult=auditResult, portNumChans=portNumChans, cmPrimarySID=cmPrimarySID, cmMacAddress=cmMacAddress, auditName=auditName, diskVolumeName=diskVolumeName, cardTemperatureHighError=cardTemperatureHighError, auditId=auditId, cardSecState=cardSecState, errEvLogLevel=errEvLogLevel, portDplxStatus=portDplxStatus, equipmentTraps=equipmentTraps, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResultDesc=cardTestResultDesc, shelfObjects=shelfObjects, portMacAddress=portMacAddress, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeDriveId=diskVolumeDriveId, cardTestCommand=cardTestCommand, cmIpAddress=cmIpAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, systemKey=systemKey)
