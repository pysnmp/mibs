...
CardSubType, FirmwareSource, DiskVolumeUsageLevel, PortType, EqActionType, PicType, SecondaryState, PortMode, PrimaryState, DuplexStatus, PortDetectedMode, CardType, UnknownState, AdminState, ShelfId, PortId, CardId, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "FirmwareSource", "DiskVolumeUsageLevel", "PortType", "EqActionType", "PicType", "SecondaryState", "PortMode", "PrimaryState", "DuplexStatus", "PortDetectedMode", "CardType", "UnknownState", "AdminState", "ShelfId", "PortId", "CardId", "FlowControlMode")
...
TruthValue, TextualConvention, RowStatus, DateAndTime, MacAddress, DisplayString, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TextualConvention", "RowStatus", "DateAndTime", "MacAddress", "DisplayString", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeFileName=diskVolumeFileName, TestResult=TestResult, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portNumber=portNumber, shelfObjects=shelfObjects, auditStatus=auditStatus, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMibConformance=cadEquipmentMibConformance, auditName=auditName, cmMacAddress=cmMacAddress, cardId=cardId, cardAdminState=cardAdminState, cardTempNoReportNotification=cardTempNoReportNotification, errEvLoggingEnabled=errEvLoggingEnabled, systemClock=systemClock, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvRowStatus=errEvRowStatus, cardShelfTestIndex=cardShelfTestIndex, cardFpgaSource=cardFpgaSource, cardTestCommand=cardTestCommand, cardTestEntry=cardTestEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portId=portId, portDplxStatusChange=portDplxStatusChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSecState=cardSecState, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResult=cardTestResult, cardFwVersion=cardFwVersion, diskVolumeDriveId=diskVolumeDriveId, cmVendor=cmVendor, cardBusSpeed=cardBusSpeed, portMacIfIndex=portMacIfIndex, diskVolumeId=diskVolumeId, cmResetStatus=cmResetStatus, portTxFlowControlDetected=portTxFlowControlDetected, portTxFlowControlMode=portTxFlowControlMode, equipmentState=equipmentState, diskVolumeUsageNotification=diskVolumeUsageNotification, auditId=auditId, cardHwVersion=cardHwVersion, cardNumber=cardNumber, cardFlashSize=cardFlashSize, cardBootVersion=cardBootVersion, portMode=portMode, cardShelfId=cardShelfId, cardTestType=cardTestType, portSecStateChange=portSecStateChange, diskVolumeName=diskVolumeName, cardDetectedChange=cardDetectedChange, diskVolumeEntry=diskVolumeEntry, portEntry=portEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeTable=diskVolumeTable, auditDescription=auditDescription, cadEquipmentMib=cadEquipmentMib, cardTestTable=cardTestTable, diskVolumeGroup=diskVolumeGroup, cardTrapInh=cardTrapInh, systemGeneralGroup=systemGeneralGroup, diskVolumeCardId=diskVolumeCardId, portPrStateChange=portPrStateChange, portCardId=portCardId, auditTotalFailedCount=auditTotalFailedCount, diskDriveNumber=diskDriveNumber, cardCpuSpeed=cardCpuSpeed, verboseLevel=verboseLevel, cmResetInfo=cmResetInfo, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestCommand=TestCommand, cardRamSize=cardRamSize, equipmentTbl=equipmentTbl, auditEntry=auditEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentCardGroup=equipmentCardGroup, cardAction=cardAction, TestScheduleCommand=TestScheduleCommand, auditPassedCount=auditPassedCount, cmResetReason=cmResetReason, cardCpuType=cardCpuType, diskVolumeNumber=diskVolumeNumber, equipmentAudit=equipmentAudit, cardTestResultNotification=cardTestResultNotification, shelfName=shelfName, cardCarrierHwVersion=cardCarrierHwVersion, cardTestTime=cardTestTime, cmUChannelID=cmUChannelID, portMacAddress=portMacAddress, cardDplxStatusChange=cardDplxStatusChange, cardTestName=cardTestName, cardPicSerialNum=cardPicSerialNum, diskVolumeShelfId=diskVolumeShelfId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, remainInDiagMode=remainInDiagMode, auditResult=auditResult, portTrapInh=portTrapInh, cmDevice=cmDevice, equipmentTraps=equipmentTraps, downstreamPowerLoss=downstreamPowerLoss, equipmentStateGroup=equipmentStateGroup, portMaxDsPower=portMaxDsPower, cardSubType=cardSubType, cardLastResetReason=cardLastResetReason, auditCycleCount=auditCycleCount, cardPrStateChange=cardPrStateChange, portDescription=portDescription, equipmentDiag=equipmentDiag, cardTestId=cardTestId, portNumChans=portNumChans, cardFwUpdateStatus=cardFwUpdateStatus, diagTestId=diagTestId, cardTestDescription=cardTestDescription, equipmentDiagGroup=equipmentDiagGroup, portTable=portTable, portAdminState=portAdminState, portSecState=portSecState, shelfNumber=shelfNumber, equipmentCardTestGroup=equipmentCardTestGroup, cardLastBootVersion=cardLastBootVersion, cardTestTransId=cardTestTransId, diskVolumeTrapInh=diskVolumeTrapInh, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSecStateChange=cardSecStateChange, auditCommand=auditCommand, cardPicHwVersion=cardPicHwVersion, cardSerialNum=cardSerialNum, portConnectorId=portConnectorId, equipmentPortGroup=equipmentPortGroup, shelfSwVersion=shelfSwVersion, auditLogOutput=auditLogOutput, TestTransactionId=TestTransactionId, TestSchedule=TestSchedule, cardName=cardName, diskVolumeSize=diskVolumeSize, cardCPLDVersion=cardCPLDVersion, auditAutoScheduling=auditAutoScheduling, portAction=portAction, auditShelfId=auditShelfId, TestId=TestId, cardTemperature=cardTemperature, systemGeneral=systemGeneral, cardDetected=cardDetected, cardSubDetected=cardSubDetected, auditFailedCount=auditFailedCount, fpgaErrorEventTable=fpgaErrorEventTable, trapSeverity=trapSeverity, cardSpareGroupMode=cardSpareGroupMode, portShelfId=portShelfId, auditTotalPassedCount=auditTotalPassedCount, portRxFlowControlDetected=portRxFlowControlDetected, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentError=equipmentError, cmPrimarySID=cmPrimarySID, cardEntry=cardEntry, cardTestSchedule=cardTestSchedule, eqDiagConfig=eqDiagConfig, cardUpTime=cardUpTime, cardTemperatureHighWarn=cardTemperatureHighWarn, cardPicDetected=cardPicDetected, portDplxStatus=portDplxStatus, systemKey=systemKey, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTable=cardTable, cardNumPorts=cardNumPorts, portDocsIfIndex=portDocsIfIndex, cardTestResultDesc=cardTestResultDesc, cmResetNotification=cmResetNotification, cardCarrierSerialNum=cardCarrierSerialNum, cardTestIndex=cardTestIndex, consoleOutput=consoleOutput, auditTable=auditTable, cardSwVersion=cardSwVersion, portType=portType, cardSpareGroupId=cardSpareGroupId, portDetectedMode=portDetectedMode, portCurrDsPower=portCurrDsPower, trapCounter=trapCounter, errEventId=errEventId, cmResetClearNotification=cmResetClearNotification, cmIpAddress=cmIpAddress, equipmentAuditGroup=equipmentAuditGroup, auditCardId=auditCardId, cardCarrierFwVersion=cardCarrierFwVersion, TestType=TestType, portPrState=portPrState, cardPrState=cardPrState, portLastChangeTime=portLastChangeTime, cardTestScheduleCommand=cardTestScheduleCommand, cardDplxStatus=cardDplxStatus, cardLastChangeTime=cardLastChangeTime, cmRegistrationNotification=cmRegistrationNotification, cmResetUpTime=cmResetUpTime, cardLastBootSource=cardLastBootSource, portCardSubType=portCardSubType, portRxFlowControlMode=portRxFlowControlMode, portMinDsPower=portMinDsPower, auditLogThrottle=auditLogThrottle, fpgaErrorEventEntry=fpgaErrorEventEntry, portBgpId=portBgpId, errEvLogLevel=errEvLogLevel, cardType=cardType, auditTime=auditTime, cardTemperatureHighError=cardTemperatureHighError)
