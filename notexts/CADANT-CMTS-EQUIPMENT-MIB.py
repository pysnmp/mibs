...
AdminState, CardSubType, EqActionType, PortType, SecondaryState, CardId, FlowControlMode, PortMode, PrimaryState, PortDetectedMode, UnknownState, CardType, PicType, DuplexStatus, FirmwareSource, DiskVolumeUsageLevel, PortId, ShelfId = mibBuilder.importSymbols("CADANT-TC", "AdminState", "CardSubType", "EqActionType", "PortType", "SecondaryState", "CardId", "FlowControlMode", "PortMode", "PrimaryState", "PortDetectedMode", "UnknownState", "CardType", "PicType", "DuplexStatus", "FirmwareSource", "DiskVolumeUsageLevel", "PortId", "ShelfId")
...
MacAddress, TruthValue, RowStatus, TimeStamp, TextualConvention, DisplayString, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "RowStatus", "TimeStamp", "TextualConvention", "DisplayString", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestIndex=cardTestIndex, cardSpareGroupMode=cardSpareGroupMode, cardCarrierHwVersion=cardCarrierHwVersion, cardTestDescription=cardTestDescription, auditLogOutput=auditLogOutput, auditFailedCount=auditFailedCount, diskVolumeGroup=diskVolumeGroup, portMacIfIndex=portMacIfIndex, cardBootVersion=cardBootVersion, diskVolumeDriveId=diskVolumeDriveId, cardTestId=cardTestId, cmVendor=cmVendor, cardLastBootSource=cardLastBootSource, cmResetNotification=cmResetNotification, cardFlashSize=cardFlashSize, portId=portId, portType=portType, cardAction=cardAction, diskVolumeId=diskVolumeId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, verboseLevel=verboseLevel, TestScheduleCommand=TestScheduleCommand, portShelfId=portShelfId, cmResetInfo=cmResetInfo, portTable=portTable, portDescription=portDescription, cardNumber=cardNumber, cardLastResetReason=cardLastResetReason, consoleOutput=consoleOutput, portSecStateChange=portSecStateChange, TestResult=TestResult, portCardId=portCardId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditPassedCount=auditPassedCount, errEventId=errEventId, cardFwUpdateStatus=cardFwUpdateStatus, portDocsIfIndex=portDocsIfIndex, auditCommand=auditCommand, auditShelfId=auditShelfId, cardLastChangeTime=cardLastChangeTime, cardTestSchedule=cardTestSchedule, equipmentDiag=equipmentDiag, cardPicDetected=cardPicDetected, cardLastBootVersion=cardLastBootVersion, cmIpAddress=cmIpAddress, auditDescription=auditDescription, auditEntry=auditEntry, cardUpTime=cardUpTime, cardId=cardId, portPrStateChange=portPrStateChange, cardName=cardName, portAdminState=portAdminState, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, eqDiagConfig=eqDiagConfig, errEvRowStatus=errEvRowStatus, equipmentCardGroup=equipmentCardGroup, cmResetUpTime=cmResetUpTime, cardSwVersion=cardSwVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfNumber=shelfNumber, cardDplxStatusChange=cardDplxStatusChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditLogThrottle=auditLogThrottle, cmResetStatus=cmResetStatus, cardTemperature=cardTemperature, portNumChans=portNumChans, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentState=equipmentState, auditTotalFailedCount=auditTotalFailedCount, cardSecState=cardSecState, shelfSwVersion=shelfSwVersion, cardShelfTestIndex=cardShelfTestIndex, cardSpareGroupId=cardSpareGroupId, cardAdminState=cardAdminState, shelfObjects=shelfObjects, cardCpuType=cardCpuType, TestSchedule=TestSchedule, trapSeverity=trapSeverity, systemKey=systemKey, auditTime=auditTime, portPrState=portPrState, diskVolumeUsageLevel=diskVolumeUsageLevel, shelfName=shelfName, diskVolumeEntry=diskVolumeEntry, diskVolumeShelfId=diskVolumeShelfId, cardPicSerialNum=cardPicSerialNum, errEvLogLevel=errEvLogLevel, portConnectorId=portConnectorId, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetClearNotification=cmResetClearNotification, cardEntry=cardEntry, equipmentAudit=equipmentAudit, cardTestType=cardTestType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardHwVersion=cardHwVersion, TestType=TestType, cardTable=cardTable, diskVolumeSize=diskVolumeSize, cmDevice=cmDevice, cardTestCommand=cardTestCommand, systemGeneralGroup=systemGeneralGroup, portMinDsPower=portMinDsPower, cardBusSpeed=cardBusSpeed, cardDplxStatus=cardDplxStatus, cardTestTable=cardTestTable, cmUChannelID=cmUChannelID, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeTrapInh=diskVolumeTrapInh, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSecStateChange=cardSecStateChange, cardFwVersion=cardFwVersion, portDplxStatusChange=portDplxStatusChange, cmRegistrationNotification=cmRegistrationNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTrapInh=cardTrapInh, portEntry=portEntry, cardCpuSpeed=cardCpuSpeed, cardTestTime=cardTestTime, auditCardId=auditCardId, diskVolumeDiskSize=diskVolumeDiskSize, trapCounter=trapCounter, diagTestId=diagTestId, equipmentDiagGroup=equipmentDiagGroup, cardTestResult=cardTestResult, portCardSubType=portCardSubType, portDetectedMode=portDetectedMode, portMacAddress=portMacAddress, portMaxDsPower=portMaxDsPower, auditResult=auditResult, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentAuditGroup=equipmentAuditGroup, cardTestName=cardTestName, portBgpId=portBgpId, cardTemperatureHighError=cardTemperatureHighError, auditStatus=auditStatus, remainInDiagMode=remainInDiagMode, cardSubDetected=cardSubDetected, TestId=TestId, portRxFlowControlMode=portRxFlowControlMode, equipmentCardTestGroup=equipmentCardTestGroup, portAction=portAction, cardTestResultDesc=cardTestResultDesc, cadEquipmentMib=cadEquipmentMib, cardTestTransId=cardTestTransId, portNumber=portNumber, auditName=auditName, equipmentStateGroup=equipmentStateGroup, cardPrState=cardPrState, portMode=portMode, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeTable=diskVolumeTable, cardTestScheduleCommand=cardTestScheduleCommand, cardCarrierSerialNum=cardCarrierSerialNum, portSecState=portSecState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentTraps=equipmentTraps, cardCPLDVersion=cardCPLDVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSubType=cardSubType, equipmentTbl=equipmentTbl, cardDetected=cardDetected, cardPicHwVersion=cardPicHwVersion, diskVolumeCardId=diskVolumeCardId, auditTotalPassedCount=auditTotalPassedCount, portLastChangeTime=portLastChangeTime, portRxFlowControlDetected=portRxFlowControlDetected, auditTable=auditTable, fpgaErrorEventEntry=fpgaErrorEventEntry, cardDetectedChange=cardDetectedChange, cardTestResultNotification=cardTestResultNotification, equipmentShelfGroup=equipmentShelfGroup, systemClock=systemClock, cmPrimarySID=cmPrimarySID, cmMacAddress=cmMacAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDplxStatus=portDplxStatus, equipmentError=equipmentError, cardTemperatureHighWarn=cardTemperatureHighWarn, TestCommand=TestCommand, errEvLoggingEnabled=errEvLoggingEnabled, equipmentPortGroup=equipmentPortGroup, cardFpgaSource=cardFpgaSource, cmResetReason=cmResetReason, diskVolumeName=diskVolumeName, downstreamPowerLoss=downstreamPowerLoss, cardShelfId=cardShelfId, diskVolumeFileName=diskVolumeFileName, diskVolumeUsageNotification=diskVolumeUsageNotification, portTrapInh=portTrapInh, cardRamSize=cardRamSize, cardTestEntry=cardTestEntry, diskVolumeNumber=diskVolumeNumber, TestTransactionId=TestTransactionId, fpgaErrorEventTable=fpgaErrorEventTable, portCurrDsPower=portCurrDsPower, cardTempNoReportNotification=cardTempNoReportNotification, auditAutoScheduling=auditAutoScheduling, auditCycleCount=auditCycleCount, diskDriveNumber=diskDriveNumber, auditId=auditId, cardSerialNum=cardSerialNum, cardNumPorts=cardNumPorts, portTxFlowControlMode=portTxFlowControlMode, cardPrStateChange=cardPrStateChange, systemGeneral=systemGeneral, cardType=cardType)
