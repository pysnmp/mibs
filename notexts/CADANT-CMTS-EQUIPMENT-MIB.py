...
PortId, DuplexStatus, PortDetectedMode, FlowControlMode, CardId, EqActionType, CardType, FirmwareSource, PicType, DiskVolumeUsageLevel, PortType, PortMode, PrimaryState, SecondaryState, AdminState, ShelfId, UnknownState, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PortId", "DuplexStatus", "PortDetectedMode", "FlowControlMode", "CardId", "EqActionType", "CardType", "FirmwareSource", "PicType", "DiskVolumeUsageLevel", "PortType", "PortMode", "PrimaryState", "SecondaryState", "AdminState", "ShelfId", "UnknownState", "CardSubType")
...
TimeStamp, TextualConvention, TruthValue, DisplayString, MacAddress, DateAndTime, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TextualConvention", "TruthValue", "DisplayString", "MacAddress", "DateAndTime", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentState=equipmentState, portBgpId=portBgpId, fpgaErrorEventGroup=fpgaErrorEventGroup, portMacIfIndex=portMacIfIndex, portPrState=portPrState, portMode=portMode, portNumChans=portNumChans, cmIpAddress=cmIpAddress, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMib=cadEquipmentMib, cardPrState=cardPrState, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeSize=diskVolumeSize, cardTemperature=cardTemperature, auditStatus=auditStatus, equipmentTraps=equipmentTraps, TestScheduleCommand=TestScheduleCommand, auditCycleCount=auditCycleCount, portMinDsPower=portMinDsPower, diagTestId=diagTestId, diskDriveNumber=diskDriveNumber, portCurrDsPower=portCurrDsPower, cmVendor=cmVendor, cardAction=cardAction, equipmentAudit=equipmentAudit, errEventId=errEventId, TestTransactionId=TestTransactionId, shelfName=shelfName, auditTotalFailedCount=auditTotalFailedCount, cmUChannelID=cmUChannelID, cardAdminState=cardAdminState, systemClock=systemClock, cardDplxStatusChange=cardDplxStatusChange, cardSwVersion=cardSwVersion, equipmentCardGroup=equipmentCardGroup, cardTestCommand=cardTestCommand, cadEquipmentMibConformance=cadEquipmentMibConformance, cardType=cardType, cardNumber=cardNumber, cardCpuSpeed=cardCpuSpeed, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeNumber=diskVolumeNumber, cardFlashSize=cardFlashSize, cardTestSchedule=cardTestSchedule, errEvRowStatus=errEvRowStatus, cmResetNotification=cmResetNotification, cardSecState=cardSecState, auditCardId=auditCardId, cardCarrierHwVersion=cardCarrierHwVersion, portDescription=portDescription, diskVolumeGroup=diskVolumeGroup, equipmentError=equipmentError, cardLastChangeTime=cardLastChangeTime, cardTestDescription=cardTestDescription, portDplxStatus=portDplxStatus, portShelfId=portShelfId, portNumber=portNumber, auditTable=auditTable, auditCommand=auditCommand, cardDetectedChange=cardDetectedChange, portDetectedMode=portDetectedMode, cmResetUpTime=cmResetUpTime, cardPrStateChange=cardPrStateChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeTable=diskVolumeTable, cardBootVersion=cardBootVersion, auditTime=auditTime, cmRegistrationNotification=cmRegistrationNotification, remainInDiagMode=remainInDiagMode, auditTotalPassedCount=auditTotalPassedCount, portTrapInh=portTrapInh, cardTestEntry=cardTestEntry, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeTrapInh=diskVolumeTrapInh, equipmentCardTestGroup=equipmentCardTestGroup, equipmentDiag=equipmentDiag, cardTestResult=cardTestResult, errEvRecoveryEnabled=errEvRecoveryEnabled, auditDescription=auditDescription, cardPicHwVersion=cardPicHwVersion, cardCarrierFwVersion=cardCarrierFwVersion, cardFwUpdateStatus=cardFwUpdateStatus, portTable=portTable, auditShelfId=auditShelfId, equipmentTbl=equipmentTbl, auditLogThrottle=auditLogThrottle, diskVolumeId=diskVolumeId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeFileName=diskVolumeFileName, cardSubDetected=cardSubDetected, auditPassedCount=auditPassedCount, cardPicDetected=cardPicDetected, cardSpareGroupId=cardSpareGroupId, portRxFlowControlDetected=portRxFlowControlDetected, auditEntry=auditEntry, consoleOutput=consoleOutput, cmResetStatus=cmResetStatus, cardUpTime=cardUpTime, trapCounter=trapCounter, cardHwVersion=cardHwVersion, cardLastBootSource=cardLastBootSource, diskVolumeName=diskVolumeName, auditResult=auditResult, portTxFlowControlMode=portTxFlowControlMode, cardFwVersion=cardFwVersion, cardNumPorts=cardNumPorts, diskVolumeShelfId=diskVolumeShelfId, cardTestName=cardTestName, shelfNumber=shelfNumber, verboseLevel=verboseLevel, cardSpareGroupMode=cardSpareGroupMode, portSecStateChange=portSecStateChange, cardBusSpeed=cardBusSpeed, cardTestIndex=cardTestIndex, cardPicSerialNum=cardPicSerialNum, cmResetInfo=cmResetInfo, portDplxStatusChange=portDplxStatusChange, diskVolumeDiskSize=diskVolumeDiskSize, cardTestScheduleCommand=cardTestScheduleCommand, cardSecStateChange=cardSecStateChange, diskVolumeEntry=diskVolumeEntry, portType=portType, trapSeverity=trapSeverity, cardCarrierSerialNum=cardCarrierSerialNum, cardSubType=cardSubType, equipmentShelfGroup=equipmentShelfGroup, cardId=cardId, cmResetClearNotification=cmResetClearNotification, cardCPLDVersion=cardCPLDVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestTime=cardTestTime, shelfSwVersion=shelfSwVersion, cardTestId=cardTestId, cardShelfId=cardShelfId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardDetected=cardDetected, portDocsIfIndex=portDocsIfIndex, cardTestTransId=cardTestTransId, cardTestResultNotification=cardTestResultNotification, TestResult=TestResult, cardFpgaSource=cardFpgaSource, cardTestResultDesc=cardTestResultDesc, TestId=TestId, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestTable=cardTestTable, portSecState=portSecState, cardTempNoReportNotification=cardTempNoReportNotification, equipmentAuditGroup=equipmentAuditGroup, equipmentPortGroup=equipmentPortGroup, portMaxDsPower=portMaxDsPower, portLastChangeTime=portLastChangeTime, diskVolumeUsageLevel=diskVolumeUsageLevel, portConnectorId=portConnectorId, portCardSubType=portCardSubType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardDplxStatus=cardDplxStatus, systemGeneral=systemGeneral, auditName=auditName, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmMacAddress=cmMacAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvLoggingEnabled=errEvLoggingEnabled, portTxFlowControlDetected=portTxFlowControlDetected, cardCpuType=cardCpuType, cardTestType=cardTestType, portAction=portAction, portEntry=portEntry, shelfObjects=shelfObjects, cardSerialNum=cardSerialNum, cardTable=cardTable, auditFailedCount=auditFailedCount, TestSchedule=TestSchedule, systemKey=systemKey, portPrStateChange=portPrStateChange, TestCommand=TestCommand, cardLastResetReason=cardLastResetReason, portRxFlowControlMode=portRxFlowControlMode, cardTrapInh=cardTrapInh, cardRamSize=cardRamSize, portCardId=portCardId, portId=portId, diskVolumeCardId=diskVolumeCardId, cmPrimarySID=cmPrimarySID, cardLastBootVersion=cardLastBootVersion, cmDevice=cmDevice, eqDiagConfig=eqDiagConfig, portAdminState=portAdminState, cardTemperatureHighError=cardTemperatureHighError, auditAutoScheduling=auditAutoScheduling, systemGeneralGroup=systemGeneralGroup, errEvLogLevel=errEvLogLevel, cmResetReason=cmResetReason, cardTempOverHeatNotification=cardTempOverHeatNotification, cardEntry=cardEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, portMacAddress=portMacAddress, auditLogOutput=auditLogOutput, fpgaErrorEventTable=fpgaErrorEventTable, cardName=cardName, equipmentDiagGroup=equipmentDiagGroup, TestType=TestType, equipmentStateGroup=equipmentStateGroup, cardShelfTestIndex=cardShelfTestIndex, auditId=auditId)
