...
PortId, PortDetectedMode, CardId, PortMode, DuplexStatus, CardSubType, PicType, UnknownState, ShelfId, EqActionType, SecondaryState, CardType, FirmwareSource, FlowControlMode, DiskVolumeUsageLevel, PortType, AdminState, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "PortId", "PortDetectedMode", "CardId", "PortMode", "DuplexStatus", "CardSubType", "PicType", "UnknownState", "ShelfId", "EqActionType", "SecondaryState", "CardType", "FirmwareSource", "FlowControlMode", "DiskVolumeUsageLevel", "PortType", "AdminState", "PrimaryState")
...
TruthValue, DisplayString, TimeStamp, MacAddress, DateAndTime, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DisplayString", "TimeStamp", "MacAddress", "DateAndTime", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTrapInh=cardTrapInh, portDplxStatusChange=portDplxStatusChange, diskVolumeId=diskVolumeId, auditTime=auditTime, cmVendor=cmVendor, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portLastChangeTime=portLastChangeTime, cardTestTransId=cardTestTransId, cardNumPorts=cardNumPorts, auditCommand=auditCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, fpgaErrorEventTable=fpgaErrorEventTable, auditTable=auditTable, cardSecState=cardSecState, diskVolumeFileName=diskVolumeFileName, cmResetInfo=cmResetInfo, portCardId=portCardId, portRxFlowControlDetected=portRxFlowControlDetected, equipmentAudit=equipmentAudit, cmIpAddress=cmIpAddress, equipmentCardTestGroup=equipmentCardTestGroup, verboseLevel=verboseLevel, cardTestCommand=cardTestCommand, cardFwUpdateStatus=cardFwUpdateStatus, cardAction=cardAction, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPrState=cardPrState, diskVolumeCardId=diskVolumeCardId, cardTestScheduleCommand=cardTestScheduleCommand, auditId=auditId, cardSwVersion=cardSwVersion, portCardSubType=portCardSubType, diskVolumeUsageLevel=diskVolumeUsageLevel, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeSize=diskVolumeSize, cardTemperature=cardTemperature, remainInDiagMode=remainInDiagMode, portNumber=portNumber, cardLastBootVersion=cardLastBootVersion, portRxFlowControlMode=portRxFlowControlMode, auditAutoScheduling=auditAutoScheduling, cardTestResult=cardTestResult, consoleOutput=consoleOutput, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvLoggingEnabled=errEvLoggingEnabled, auditPassedCount=auditPassedCount, TestType=TestType, cardCPLDVersion=cardCPLDVersion, cardSubDetected=cardSubDetected, cardShelfId=cardShelfId, cardDetected=cardDetected, portAdminState=portAdminState, portMacIfIndex=portMacIfIndex, cardTestDescription=cardTestDescription, portDplxStatus=portDplxStatus, cardName=cardName, auditEntry=auditEntry, cmRegistrationNotification=cmRegistrationNotification, auditDescription=auditDescription, cardCarrierHwVersion=cardCarrierHwVersion, portDetectedMode=portDetectedMode, cardTestId=cardTestId, auditResult=auditResult, cardBusSpeed=cardBusSpeed, cmResetClearNotification=cmResetClearNotification, diskVolumeShelfId=diskVolumeShelfId, cardRamSize=cardRamSize, cardEntry=cardEntry, diskVolumeTable=diskVolumeTable, diskVolumeEntry=diskVolumeEntry, portDocsIfIndex=portDocsIfIndex, cardTestTable=cardTestTable, equipmentStateGroup=equipmentStateGroup, cardFpgaSource=cardFpgaSource, portPrStateChange=portPrStateChange, portAction=portAction, cmResetReason=cmResetReason, errEventId=errEventId, auditTotalFailedCount=auditTotalFailedCount, auditShelfId=auditShelfId, TestScheduleCommand=TestScheduleCommand, cmResetNotification=cmResetNotification, cardFlashSize=cardFlashSize, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentCardGroup=equipmentCardGroup, portMacAddress=portMacAddress, auditCycleCount=auditCycleCount, equipmentTraps=equipmentTraps, cardTestName=cardTestName, portMinDsPower=portMinDsPower, eqDiagConfig=eqDiagConfig, cardTemperatureHighError=cardTemperatureHighError, cmMacAddress=cmMacAddress, diagTestId=diagTestId, cardTestEntry=cardTestEntry, cardLastResetReason=cardLastResetReason, portDescription=portDescription, trapSeverity=trapSeverity, TestId=TestId, equipmentTbl=equipmentTbl, diskVolumeDiskSize=diskVolumeDiskSize, errEvLogLevel=errEvLogLevel, cardSpareGroupId=cardSpareGroupId, cadEquipmentMib=cadEquipmentMib, equipmentPortGroup=equipmentPortGroup, systemGeneralGroup=systemGeneralGroup, cardTable=cardTable, portTxFlowControlMode=portTxFlowControlMode, cardTestResultDesc=cardTestResultDesc, downstreamPowerLoss=downstreamPowerLoss, cardCpuType=cardCpuType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemGeneral=systemGeneral, cardSerialNum=cardSerialNum, auditLogThrottle=auditLogThrottle, portMaxDsPower=portMaxDsPower, shelfSwVersion=shelfSwVersion, cardShelfTestIndex=cardShelfTestIndex, TestTransactionId=TestTransactionId, portEntry=portEntry, systemKey=systemKey, cardPicDetected=cardPicDetected, auditLogOutput=auditLogOutput, cmDevice=cmDevice, cardSecStateChange=cardSecStateChange, auditStatus=auditStatus, diskDriveNumber=diskDriveNumber, cardId=cardId, diskVolumeNumber=diskVolumeNumber, cardTestIndex=cardTestIndex, TestCommand=TestCommand, errEvRowStatus=errEvRowStatus, cardSubType=cardSubType, portMode=portMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastChangeTime=cardLastChangeTime, TestSchedule=TestSchedule, portNumChans=portNumChans, cardDetectedChange=cardDetectedChange, auditFailedCount=auditFailedCount, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, trapCounter=trapCounter, portTrapInh=portTrapInh, cmResetUpTime=cmResetUpTime, equipmentShelfGroup=equipmentShelfGroup, portShelfId=portShelfId, portPrState=portPrState, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeDriveId=diskVolumeDriveId, cardTestType=cardTestType, systemClock=systemClock, cardFwVersion=cardFwVersion, shelfName=shelfName, cardAdminState=cardAdminState, cardPicSerialNum=cardPicSerialNum, portTable=portTable, cadEquipmentMibConformance=cadEquipmentMibConformance, cardLastBootSource=cardLastBootSource, cmPrimarySID=cmPrimarySID, cardDplxStatusChange=cardDplxStatusChange, cardUpTime=cardUpTime, cardTestSchedule=cardTestSchedule, equipmentDiag=equipmentDiag, cardCpuSpeed=cardCpuSpeed, diskVolumeName=diskVolumeName, equipmentAuditGroup=equipmentAuditGroup, portCurrDsPower=portCurrDsPower, cardTemperatureHighWarn=cardTemperatureHighWarn, cardPicHwVersion=cardPicHwVersion, cardType=cardType, cardBootVersion=cardBootVersion, shelfObjects=shelfObjects, cardDplxStatus=cardDplxStatus, portType=portType, diskVolumeGroup=diskVolumeGroup, cardPrStateChange=cardPrStateChange, cardHwVersion=cardHwVersion, cardSpareGroupMode=cardSpareGroupMode, equipmentState=equipmentState, portConnectorId=portConnectorId, equipmentError=equipmentError, errEvRecoveryEnabled=errEvRecoveryEnabled, TestResult=TestResult, shelfNumber=shelfNumber, portSecState=portSecState, auditCardId=auditCardId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, cardNumber=cardNumber, cardCarrierSerialNum=cardCarrierSerialNum, portBgpId=portBgpId, auditName=auditName, portSecStateChange=portSecStateChange, cardTempNoReportNotification=cardTempNoReportNotification, cardTestResultNotification=cardTestResultNotification, cmUChannelID=cmUChannelID, equipmentDiagGroup=equipmentDiagGroup, cmResetStatus=cmResetStatus, auditTotalPassedCount=auditTotalPassedCount, PYSNMP_MODULE_ID=cadEquipmentMib, portId=portId, cardTestTime=cardTestTime)
