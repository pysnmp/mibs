...
SecondaryState, PicType, PrimaryState, DiskVolumeUsageLevel, EqActionType, UnknownState, CardSubType, FirmwareSource, PortType, CardId, ShelfId, FlowControlMode, PortMode, CardType, DuplexStatus, PortDetectedMode, PortId, AdminState = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "PicType", "PrimaryState", "DiskVolumeUsageLevel", "EqActionType", "UnknownState", "CardSubType", "FirmwareSource", "PortType", "CardId", "ShelfId", "FlowControlMode", "PortMode", "CardType", "DuplexStatus", "PortDetectedMode", "PortId", "AdminState")
...
DateAndTime, DisplayString, TimeStamp, RowStatus, TruthValue, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "DisplayString", "TimeStamp", "RowStatus", "TruthValue", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentState=equipmentState, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardRamSize=cardRamSize, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, systemClock=systemClock, cmVendor=cmVendor, cardTemperatureHighError=cardTemperatureHighError, PYSNMP_MODULE_ID=cadEquipmentMib, shelfObjects=shelfObjects, portDplxStatus=portDplxStatus, cardId=cardId, portCardSubType=portCardSubType, TestResult=TestResult, TestTransactionId=TestTransactionId, auditEntry=auditEntry, systemGeneral=systemGeneral, equipmentStateGroup=equipmentStateGroup, portShelfId=portShelfId, cmResetUpTime=cmResetUpTime, cardCarrierSerialNum=cardCarrierSerialNum, cardType=cardType, cardDetected=cardDetected, cardHwVersion=cardHwVersion, portAction=portAction, auditStatus=auditStatus, auditTime=auditTime, portPrState=portPrState, portConnectorId=portConnectorId, remainInDiagMode=remainInDiagMode, cmMacAddress=cmMacAddress, portPrStateChange=portPrStateChange, cardCarrierHwVersion=cardCarrierHwVersion, auditTable=auditTable, cardNumPorts=cardNumPorts, portRxFlowControlMode=portRxFlowControlMode, portNumber=portNumber, portMacIfIndex=portMacIfIndex, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, eqDiagConfig=eqDiagConfig, cardTestResultNotification=cardTestResultNotification, cardSubType=cardSubType, portTrapInh=portTrapInh, portType=portType, portSecStateChange=portSecStateChange, cardSecStateChange=cardSecStateChange, cardTestEntry=cardTestEntry, cardDplxStatus=cardDplxStatus, diskVolumeShelfId=diskVolumeShelfId, cmResetInfo=cmResetInfo, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMib=cadEquipmentMib, cardBootVersion=cardBootVersion, diskVolumeGroup=diskVolumeGroup, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetStatus=cmResetStatus, equipmentDiag=equipmentDiag, cardNumber=cardNumber, cardPicDetected=cardPicDetected, shelfSwVersion=shelfSwVersion, cardPicHwVersion=cardPicHwVersion, cardTestTransId=cardTestTransId, cardTestIndex=cardTestIndex, cardShelfTestIndex=cardShelfTestIndex, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestId=cardTestId, diskVolumeDriveId=diskVolumeDriveId, auditFailedCount=auditFailedCount, cardPrStateChange=cardPrStateChange, equipmentAudit=equipmentAudit, portId=portId, diskVolumeTrapInh=diskVolumeTrapInh, diagTestId=diagTestId, fpgaErrorEventGroup=fpgaErrorEventGroup, auditCardId=auditCardId, cardSerialNum=cardSerialNum, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeCardId=diskVolumeCardId, diskVolumeSize=diskVolumeSize, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditDescription=auditDescription, cardFpgaSource=cardFpgaSource, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardFwVersion=cardFwVersion, portDplxStatusChange=portDplxStatusChange, diskVolumeTable=diskVolumeTable, cardTestScheduleCommand=cardTestScheduleCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, fpgaErrorEventEntry=fpgaErrorEventEntry, auditShelfId=auditShelfId, cardTempNoReportNotification=cardTempNoReportNotification, cardName=cardName, diskVolumeNumber=diskVolumeNumber, fpgaErrorEventTable=fpgaErrorEventTable, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeName=diskVolumeName, cardTestCommand=cardTestCommand, portRxFlowControlDetected=portRxFlowControlDetected, cardLastBootSource=cardLastBootSource, cardLastBootVersion=cardLastBootVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetClearNotification=cmResetClearNotification, equipmentTraps=equipmentTraps, cmResetNotification=cmResetNotification, equipmentAuditGroup=equipmentAuditGroup, equipmentCardGroup=equipmentCardGroup, cardShelfId=cardShelfId, equipmentCardTestGroup=equipmentCardTestGroup, portAdminState=portAdminState, cardTrapInh=cardTrapInh, portBgpId=portBgpId, diskVolumeFileName=diskVolumeFileName, cardSpareGroupId=cardSpareGroupId, cmUChannelID=cmUChannelID, cardCpuSpeed=cardCpuSpeed, cmRegistrationNotification=cmRegistrationNotification, cardFwUpdateStatus=cardFwUpdateStatus, cardAdminState=cardAdminState, cardDetectedChange=cardDetectedChange, auditName=auditName, cardLastChangeTime=cardLastChangeTime, cmPrimarySID=cmPrimarySID, cardTestName=cardTestName, verboseLevel=verboseLevel, downstreamPowerLoss=downstreamPowerLoss, cardTestTime=cardTestTime, cardTestTable=cardTestTable, trapCounter=trapCounter, shelfNumber=shelfNumber, cardSubDetected=cardSubDetected, cadEquipmentMibGroup=cadEquipmentMibGroup, portDocsIfIndex=portDocsIfIndex, portLastChangeTime=portLastChangeTime, auditAutoScheduling=auditAutoScheduling, cmDevice=cmDevice, cardTestResult=cardTestResult, auditLogOutput=auditLogOutput, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeEntry=diskVolumeEntry, equipmentShelfGroup=equipmentShelfGroup, equipmentDiagGroup=equipmentDiagGroup, cardTestType=cardTestType, cardCarrierFwVersion=cardCarrierFwVersion, portNumChans=portNumChans, auditTotalFailedCount=auditTotalFailedCount, auditId=auditId, auditResult=auditResult, consoleOutput=consoleOutput, cardSpareGroupMode=cardSpareGroupMode, diskVolumeUsageLevel=diskVolumeUsageLevel, trapSeverity=trapSeverity, cmIpAddress=cmIpAddress, cardTempOverHeatNotification=cardTempOverHeatNotification, diskDriveNumber=diskDriveNumber, cardSecState=cardSecState, cardEntry=cardEntry, portDescription=portDescription, errEventId=errEventId, auditLogThrottle=auditLogThrottle, portMode=portMode, errEvRowStatus=errEvRowStatus, cardCPLDVersion=cardCPLDVersion, cardTemperature=cardTemperature, auditCommand=auditCommand, portMaxDsPower=portMaxDsPower, cmResetReason=cmResetReason, TestCommand=TestCommand, equipmentError=equipmentError, TestScheduleCommand=TestScheduleCommand, shelfName=shelfName, cardTestResultDesc=cardTestResultDesc, cardSwVersion=cardSwVersion, auditPassedCount=auditPassedCount, errEvLogLevel=errEvLogLevel, portSecState=portSecState, cardLastResetReason=cardLastResetReason, TestSchedule=TestSchedule, TestType=TestType, cardPrState=cardPrState, portTable=portTable, cardTestSchedule=cardTestSchedule, auditTotalPassedCount=auditTotalPassedCount, portMinDsPower=portMinDsPower, cardCpuType=cardCpuType, portMacAddress=portMacAddress, cardDplxStatusChange=cardDplxStatusChange, portCurrDsPower=portCurrDsPower, TestId=TestId, equipmentTbl=equipmentTbl, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, systemGeneralGroup=systemGeneralGroup, systemKey=systemKey, cardBusSpeed=cardBusSpeed, cardUpTime=cardUpTime, cardTestDescription=cardTestDescription, equipmentPortGroup=equipmentPortGroup, portEntry=portEntry, auditCycleCount=auditCycleCount, cadEquipmentMibConformance=cadEquipmentMibConformance, cardFlashSize=cardFlashSize, cardAction=cardAction, portDetectedMode=portDetectedMode, cardTable=cardTable, portCardId=portCardId, diskVolumeId=diskVolumeId)
