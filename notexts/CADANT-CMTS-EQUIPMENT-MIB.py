...
FirmwareSource, DiskVolumeUsageLevel, FlowControlMode, PicType, PortType, PrimaryState, EqActionType, PortId, ShelfId, CardSubType, PortDetectedMode, CardType, SecondaryState, AdminState, PortMode, UnknownState, CardId, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "DiskVolumeUsageLevel", "FlowControlMode", "PicType", "PortType", "PrimaryState", "EqActionType", "PortId", "ShelfId", "CardSubType", "PortDetectedMode", "CardType", "SecondaryState", "AdminState", "PortMode", "UnknownState", "CardId", "DuplexStatus")
...
MacAddress, TimeStamp, DateAndTime, TextualConvention, RowStatus, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TimeStamp", "DateAndTime", "TextualConvention", "RowStatus", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentStateGroup=equipmentStateGroup, auditCycleCount=auditCycleCount, auditCardId=auditCardId, cardSerialNum=cardSerialNum, portMode=portMode, portPrStateChange=portPrStateChange, cardFpgaSource=cardFpgaSource, auditCommand=auditCommand, cardSubType=cardSubType, TestSchedule=TestSchedule, cardLastBootVersion=cardLastBootVersion, shelfNumber=shelfNumber, cardTestTime=cardTestTime, auditStatus=auditStatus, remainInDiagMode=remainInDiagMode, auditLogOutput=auditLogOutput, auditName=auditName, equipmentTbl=equipmentTbl, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestSchedule=cardTestSchedule, cmVendor=cmVendor, cardTestResultDesc=cardTestResultDesc, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardPicDetected=cardPicDetected, systemClock=systemClock, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeFileName=diskVolumeFileName, equipmentCardTestGroup=equipmentCardTestGroup, cardSecStateChange=cardSecStateChange, cmResetReason=cmResetReason, cmUChannelID=cmUChannelID, cmResetInfo=cmResetInfo, cardTestTransId=cardTestTransId, cardName=cardName, portNumChans=portNumChans, cardTempNoReportNotification=cardTempNoReportNotification, auditFailedCount=auditFailedCount, portShelfId=portShelfId, TestScheduleCommand=TestScheduleCommand, cardDplxStatusChange=cardDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, cardLastBootSource=cardLastBootSource, cardSubDetected=cardSubDetected, cadEquipmentMib=cadEquipmentMib, cardSpareGroupId=cardSpareGroupId, cardShelfId=cardShelfId, cardTestName=cardTestName, cardPrStateChange=cardPrStateChange, cardCpuSpeed=cardCpuSpeed, cardTestDescription=cardTestDescription, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeCardId=diskVolumeCardId, cardSecState=cardSecState, equipmentPortGroup=equipmentPortGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, portMacIfIndex=portMacIfIndex, cmMacAddress=cmMacAddress, cadEquipmentMibCompliance=cadEquipmentMibCompliance, shelfObjects=shelfObjects, shelfName=shelfName, portSecStateChange=portSecStateChange, cmPrimarySID=cmPrimarySID, diskVolumeNumber=diskVolumeNumber, TestType=TestType, errEvLogLevel=errEvLogLevel, auditTotalPassedCount=auditTotalPassedCount, portCardId=portCardId, errEvLoggingEnabled=errEvLoggingEnabled, cmDevice=cmDevice, cardSwVersion=cardSwVersion, auditDescription=auditDescription, cardTestScheduleCommand=cardTestScheduleCommand, cardFwUpdateStatus=cardFwUpdateStatus, portTable=portTable, portRxFlowControlMode=portRxFlowControlMode, cardShelfTestIndex=cardShelfTestIndex, cmResetUpTime=cmResetUpTime, cardNumPorts=cardNumPorts, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portPrState=portPrState, systemGeneralGroup=systemGeneralGroup, portMinDsPower=portMinDsPower, consoleOutput=consoleOutput, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeName=diskVolumeName, cardFlashSize=cardFlashSize, auditShelfId=auditShelfId, cardDplxStatus=cardDplxStatus, cardCarrierSerialNum=cardCarrierSerialNum, shelfSwVersion=shelfSwVersion, portCardSubType=portCardSubType, equipmentState=equipmentState, cardLastChangeTime=cardLastChangeTime, diagTestId=diagTestId, cmResetClearNotification=cmResetClearNotification, trapSeverity=trapSeverity, cardSpareGroupMode=cardSpareGroupMode, cardEntry=cardEntry, auditResult=auditResult, cmResetStatus=cmResetStatus, cardFwVersion=cardFwVersion, equipmentAuditGroup=equipmentAuditGroup, cardRamSize=cardRamSize, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentError=equipmentError, cardType=cardType, auditTable=auditTable, cardCarrierHwVersion=cardCarrierHwVersion, cardPrState=cardPrState, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTemperatureHighError=cardTemperatureHighError, portTrapInh=portTrapInh, cardHwVersion=cardHwVersion, portMaxDsPower=portMaxDsPower, cardBootVersion=cardBootVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmIpAddress=cmIpAddress, cardDetected=cardDetected, portCurrDsPower=portCurrDsPower, portDplxStatus=portDplxStatus, cardTemperature=cardTemperature, portConnectorId=portConnectorId, diskVolumeGroup=diskVolumeGroup, equipmentDiagGroup=equipmentDiagGroup, cardCPLDVersion=cardCPLDVersion, auditLogThrottle=auditLogThrottle, diskVolumeId=diskVolumeId, portRxFlowControlDetected=portRxFlowControlDetected, auditTotalFailedCount=auditTotalFailedCount, cmRegistrationNotification=cmRegistrationNotification, errEvRowStatus=errEvRowStatus, cardId=cardId, cardTable=cardTable, portEntry=portEntry, cardLastResetReason=cardLastResetReason, cardTrapInh=cardTrapInh, portBgpId=portBgpId, cardTestEntry=cardTestEntry, portNumber=portNumber, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibGroup=cadEquipmentMibGroup, portDplxStatusChange=portDplxStatusChange, portTxFlowControlDetected=portTxFlowControlDetected, cardAdminState=cardAdminState, cardNumber=cardNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestCommand=TestCommand, cardTestIndex=cardTestIndex, auditAutoScheduling=auditAutoScheduling, portId=portId, cardDetectedChange=cardDetectedChange, fpgaErrorEventTable=fpgaErrorEventTable, TestResult=TestResult, diskVolumeUsageLevel=diskVolumeUsageLevel, portDescription=portDescription, errEventId=errEventId, cardTestCommand=cardTestCommand, diskVolumeDriveId=diskVolumeDriveId, equipmentCardGroup=equipmentCardGroup, portLastChangeTime=portLastChangeTime, auditId=auditId, cardAction=cardAction, auditEntry=auditEntry, eqDiagConfig=eqDiagConfig, fpgaErrorEventGroup=fpgaErrorEventGroup, downstreamPowerLoss=downstreamPowerLoss, auditPassedCount=auditPassedCount, portDetectedMode=portDetectedMode, systemGeneral=systemGeneral, portType=portType, diskVolumeSize=diskVolumeSize, verboseLevel=verboseLevel, cardBusSpeed=cardBusSpeed, cardCarrierFwVersion=cardCarrierFwVersion, equipmentTraps=equipmentTraps, systemKey=systemKey, cardTestTable=cardTestTable, auditTime=auditTime, cardTestResultNotification=cardTestResultNotification, diskVolumeEntry=diskVolumeEntry, portDocsIfIndex=portDocsIfIndex, cardTestType=cardTestType, portAction=portAction, portSecState=portSecState, cmResetNotification=cmResetNotification, cardUpTime=cardUpTime, trapCounter=trapCounter, portMacAddress=portMacAddress, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestResult=cardTestResult, diskVolumeShelfId=diskVolumeShelfId, cardTestId=cardTestId, equipmentAudit=equipmentAudit, diskDriveNumber=diskDriveNumber, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portTxFlowControlMode=portTxFlowControlMode, portAdminState=portAdminState, TestId=TestId, diskVolumeTable=diskVolumeTable, equipmentDiag=equipmentDiag, cardPicHwVersion=cardPicHwVersion, equipmentShelfGroup=equipmentShelfGroup, cardCpuType=cardCpuType, TestTransactionId=TestTransactionId, cardPicSerialNum=cardPicSerialNum)
