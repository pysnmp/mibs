...
PrimaryState, PortId, PortMode, AdminState, FirmwareSource, DiskVolumeUsageLevel, ShelfId, PortType, PortDetectedMode, SecondaryState, CardSubType, EqActionType, DuplexStatus, FlowControlMode, PicType, UnknownState, CardType, CardId = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "PortId", "PortMode", "AdminState", "FirmwareSource", "DiskVolumeUsageLevel", "ShelfId", "PortType", "PortDetectedMode", "SecondaryState", "CardSubType", "EqActionType", "DuplexStatus", "FlowControlMode", "PicType", "UnknownState", "CardType", "CardId")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", remainInDiagMode=remainInDiagMode, portDplxStatusChange=portDplxStatusChange, portNumChans=portNumChans, auditStatus=auditStatus, portTxFlowControlMode=portTxFlowControlMode, auditTable=auditTable, cardLastBootSource=cardLastBootSource, cardCarrierSerialNum=cardCarrierSerialNum, portRxFlowControlMode=portRxFlowControlMode, trapCounter=trapCounter, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeName=diskVolumeName, portTxFlowControlDetected=portTxFlowControlDetected, TestResult=TestResult, cardPicDetected=cardPicDetected, cardRamSize=cardRamSize, portConnectorId=portConnectorId, portDescription=portDescription, cardBootVersion=cardBootVersion, portAdminState=portAdminState, cardTestResultDesc=cardTestResultDesc, TestType=TestType, cardSubType=cardSubType, shelfName=shelfName, cmMacAddress=cmMacAddress, portPrState=portPrState, cardId=cardId, cmIpAddress=cmIpAddress, portLastChangeTime=portLastChangeTime, cardLastBootVersion=cardLastBootVersion, portCurrDsPower=portCurrDsPower, auditTotalPassedCount=auditTotalPassedCount, diskDriveNumber=diskDriveNumber, cardBusSpeed=cardBusSpeed, auditLogThrottle=auditLogThrottle, verboseLevel=verboseLevel, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCpuSpeed=cardCpuSpeed, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestId=TestId, portTrapInh=portTrapInh, equipmentTraps=equipmentTraps, portShelfId=portShelfId, cardTestTime=cardTestTime, auditCycleCount=auditCycleCount, cardTestDescription=cardTestDescription, portMode=portMode, portCardId=portCardId, portSecState=portSecState, portBgpId=portBgpId, portRxFlowControlDetected=portRxFlowControlDetected, fpgaErrorEventTable=fpgaErrorEventTable, systemGeneral=systemGeneral, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetClearNotification=cmResetClearNotification, portMacIfIndex=portMacIfIndex, cardTestResultNotification=cardTestResultNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardNumber=cardNumber, diskVolumeCardId=diskVolumeCardId, auditCardId=auditCardId, equipmentTbl=equipmentTbl, cardSecStateChange=cardSecStateChange, equipmentState=equipmentState, cardShelfId=cardShelfId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTable=cardTable, diskVolumeGroup=diskVolumeGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmVendor=cmVendor, equipmentAuditGroup=equipmentAuditGroup, portId=portId, portNumber=portNumber, equipmentError=equipmentError, diskVolumeTable=diskVolumeTable, equipmentCardGroup=equipmentCardGroup, cmUChannelID=cmUChannelID, systemKey=systemKey, diskVolumeSize=diskVolumeSize, diskVolumeUsageLevel=diskVolumeUsageLevel, trapSeverity=trapSeverity, cmRegistrationNotification=cmRegistrationNotification, cardSecState=cardSecState, cardPicSerialNum=cardPicSerialNum, auditDescription=auditDescription, systemClock=systemClock, cardFwVersion=cardFwVersion, auditLogOutput=auditLogOutput, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetNotification=cmResetNotification, equipmentShelfGroup=equipmentShelfGroup, TestCommand=TestCommand, errEventId=errEventId, portType=portType, cardTempNoReportNotification=cardTempNoReportNotification, cardTemperature=cardTemperature, auditEntry=auditEntry, diskVolumeDriveId=diskVolumeDriveId, cmPrimarySID=cmPrimarySID, cmDevice=cmDevice, cardHwVersion=cardHwVersion, errEvLoggingEnabled=errEvLoggingEnabled, cardCpuType=cardCpuType, equipmentDiag=equipmentDiag, consoleOutput=consoleOutput, auditFailedCount=auditFailedCount, cardFlashSize=cardFlashSize, portSecStateChange=portSecStateChange, errEvRowStatus=errEvRowStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSpareGroupId=cardSpareGroupId, cardTestId=cardTestId, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDplxStatus=portDplxStatus, auditPassedCount=auditPassedCount, cardFpgaSource=cardFpgaSource, equipmentAudit=equipmentAudit, cardDetectedChange=cardDetectedChange, cardShelfTestIndex=cardShelfTestIndex, cardCarrierFwVersion=cardCarrierFwVersion, cardTestIndex=cardTestIndex, errEvLogLevel=errEvLogLevel, downstreamPowerLoss=downstreamPowerLoss, cardLastResetReason=cardLastResetReason, diskVolumeEntry=diskVolumeEntry, cardTestResult=cardTestResult, cmResetUpTime=cmResetUpTime, auditCommand=auditCommand, portCardSubType=portCardSubType, portMaxDsPower=portMaxDsPower, shelfObjects=shelfObjects, cardSwVersion=cardSwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeDiskSize=diskVolumeDiskSize, cardSubDetected=cardSubDetected, cardDetected=cardDetected, cardTestTable=cardTestTable, cardTrapInh=cardTrapInh, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetStatus=cmResetStatus, auditTime=auditTime, portMacAddress=portMacAddress, systemGeneralGroup=systemGeneralGroup, cardPrStateChange=cardPrStateChange, cardCarrierHwVersion=cardCarrierHwVersion, cardTestTransId=cardTestTransId, cmResetReason=cmResetReason, shelfSwVersion=shelfSwVersion, diagTestId=diagTestId, portTable=portTable, portPrStateChange=portPrStateChange, auditId=auditId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPrState=cardPrState, TestTransactionId=TestTransactionId, diskVolumeId=diskVolumeId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, eqDiagConfig=eqDiagConfig, cardUpTime=cardUpTime, cardSpareGroupMode=cardSpareGroupMode, portDetectedMode=portDetectedMode, auditShelfId=auditShelfId, cardDplxStatus=cardDplxStatus, cardNumPorts=cardNumPorts, cmResetInfo=cmResetInfo, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastChangeTime=cardLastChangeTime, cardTestCommand=cardTestCommand, cardTemperatureHighError=cardTemperatureHighError, diskVolumeFileName=diskVolumeFileName, portEntry=portEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cardAction=cardAction, TestSchedule=TestSchedule, TestScheduleCommand=TestScheduleCommand, cardEntry=cardEntry, cardDplxStatusChange=cardDplxStatusChange, cardTestEntry=cardTestEntry, cardAdminState=cardAdminState, cardTestName=cardTestName, portAction=portAction, cardTestType=cardTestType, auditTotalFailedCount=auditTotalFailedCount, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeNumber=diskVolumeNumber, cardSerialNum=cardSerialNum, cardName=cardName, cardCPLDVersion=cardCPLDVersion, cardPicHwVersion=cardPicHwVersion, shelfNumber=shelfNumber, portMinDsPower=portMinDsPower, cardTestSchedule=cardTestSchedule, auditName=auditName, portDocsIfIndex=portDocsIfIndex, auditAutoScheduling=auditAutoScheduling, auditResult=auditResult, diskVolumeShelfId=diskVolumeShelfId, cardType=cardType, equipmentStateGroup=equipmentStateGroup)
