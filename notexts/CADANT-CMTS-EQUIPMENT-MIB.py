...
EqActionType, FirmwareSource, PrimaryState, ShelfId, DiskVolumeUsageLevel, CardId, PicType, AdminState, PortDetectedMode, PortType, SecondaryState, FlowControlMode, CardType, PortId, DuplexStatus, UnknownState, CardSubType, PortMode = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "FirmwareSource", "PrimaryState", "ShelfId", "DiskVolumeUsageLevel", "CardId", "PicType", "AdminState", "PortDetectedMode", "PortType", "SecondaryState", "FlowControlMode", "CardType", "PortId", "DuplexStatus", "UnknownState", "CardSubType", "PortMode")
...
MacAddress, TimeStamp, TextualConvention, TruthValue, RowStatus, DisplayString, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TimeStamp", "TextualConvention", "TruthValue", "RowStatus", "DisplayString", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portNumChans=portNumChans, portMacAddress=portMacAddress, diskVolumeTrapInh=diskVolumeTrapInh, cardTestResult=cardTestResult, portShelfId=portShelfId, portLastChangeTime=portLastChangeTime, cardSwVersion=cardSwVersion, cardAction=cardAction, auditLogThrottle=auditLogThrottle, cardBootVersion=cardBootVersion, cardHwVersion=cardHwVersion, portConnectorId=portConnectorId, portRxFlowControlDetected=portRxFlowControlDetected, cardDetected=cardDetected, equipmentDiag=equipmentDiag, cardCarrierSerialNum=cardCarrierSerialNum, cardFwVersion=cardFwVersion, cardDplxStatusChange=cardDplxStatusChange, cardLastBootSource=cardLastBootSource, eqDiagConfig=eqDiagConfig, diskDriveNumber=diskDriveNumber, TestSchedule=TestSchedule, cardCPLDVersion=cardCPLDVersion, shelfNumber=shelfNumber, shelfObjects=shelfObjects, equipmentAudit=equipmentAudit, cardCpuSpeed=cardCpuSpeed, cardCpuType=cardCpuType, trapSeverity=trapSeverity, auditTime=auditTime, equipmentTraps=equipmentTraps, cardFwUpdateStatus=cardFwUpdateStatus, TestType=TestType, cardFpgaSource=cardFpgaSource, equipmentCardTestGroup=equipmentCardTestGroup, cardTemperatureHighError=cardTemperatureHighError, auditLogOutput=auditLogOutput, auditShelfId=auditShelfId, portDocsIfIndex=portDocsIfIndex, cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventTable=fpgaErrorEventTable, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTrapInh=cardTrapInh, cardDetectedChange=cardDetectedChange, systemGeneral=systemGeneral, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeUsageLevel=diskVolumeUsageLevel, cardPrStateChange=cardPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portTrapInh=portTrapInh, cardId=cardId, cardTestTransId=cardTestTransId, cardPicDetected=cardPicDetected, cardNumber=cardNumber, diskVolumeUsageNotification=diskVolumeUsageNotification, auditTotalFailedCount=auditTotalFailedCount, diagTestId=diagTestId, cmIpAddress=cmIpAddress, portDplxStatus=portDplxStatus, cardShelfId=cardShelfId, cardUpTime=cardUpTime, errEvRowStatus=errEvRowStatus, systemGeneralGroup=systemGeneralGroup, portEntry=portEntry, cmResetUpTime=cmResetUpTime, cardTestDescription=cardTestDescription, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecStateChange=portSecStateChange, auditCardId=auditCardId, cardTable=cardTable, cardType=cardType, consoleOutput=consoleOutput, cardTestIndex=cardTestIndex, cardCarrierFwVersion=cardCarrierFwVersion, portMacIfIndex=portMacIfIndex, cardTestTable=cardTestTable, auditCommand=auditCommand, cmUChannelID=cmUChannelID, cardTemperature=cardTemperature, errEventId=errEventId, portCardId=portCardId, cardTestName=cardTestName, cardEntry=cardEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCycleCount=auditCycleCount, auditId=auditId, portTxFlowControlMode=portTxFlowControlMode, portId=portId, equipmentPortGroup=equipmentPortGroup, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeFileName=diskVolumeFileName, systemClock=systemClock, PYSNMP_MODULE_ID=cadEquipmentMib, cmDevice=cmDevice, portDplxStatusChange=portDplxStatusChange, equipmentShelfGroup=equipmentShelfGroup, cardTempNoReportNotification=cardTempNoReportNotification, errEvLogLevel=errEvLogLevel, trapCounter=trapCounter, diskVolumeId=diskVolumeId, cmResetClearNotification=cmResetClearNotification, diskVolumeDriveId=diskVolumeDriveId, cardTestTime=cardTestTime, portBgpId=portBgpId, auditFailedCount=auditFailedCount, equipmentCardGroup=equipmentCardGroup, shelfName=shelfName, TestTransactionId=TestTransactionId, equipmentState=equipmentState, portRxFlowControlMode=portRxFlowControlMode, cmResetReason=cmResetReason, cardAdminState=cardAdminState, cardDplxStatus=cardDplxStatus, verboseLevel=verboseLevel, cardTestResultDesc=cardTestResultDesc, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSerialNum=cardSerialNum, fpgaErrorEventGroup=fpgaErrorEventGroup, systemKey=systemKey, cardPrState=cardPrState, cadEquipmentMib=cadEquipmentMib, portCurrDsPower=portCurrDsPower, portSecState=portSecState, cardTestSchedule=cardTestSchedule, auditAutoScheduling=auditAutoScheduling, cardBusSpeed=cardBusSpeed, cardRamSize=cardRamSize, cmVendor=cmVendor, cmResetInfo=cmResetInfo, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentStateGroup=equipmentStateGroup, diskVolumeNumber=diskVolumeNumber, cardSpareGroupMode=cardSpareGroupMode, cardLastResetReason=cardLastResetReason, portAdminState=portAdminState, cardTestId=cardTestId, portDetectedMode=portDetectedMode, portAction=portAction, cardLastChangeTime=cardLastChangeTime, diskVolumeGroup=diskVolumeGroup, portCardSubType=portCardSubType, portTable=portTable, cardName=cardName, auditTable=auditTable, diskVolumeCardId=diskVolumeCardId, cardTemperatureHighWarn=cardTemperatureHighWarn, portTxFlowControlDetected=portTxFlowControlDetected, cmResetStatus=cmResetStatus, portPrStateChange=portPrStateChange, TestId=TestId, cmRegistrationNotification=cmRegistrationNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, portMaxDsPower=portMaxDsPower, downstreamPowerLoss=downstreamPowerLoss, cardSecState=cardSecState, cardPicHwVersion=cardPicHwVersion, cmPrimarySID=cmPrimarySID, cardSecStateChange=cardSecStateChange, shelfSwVersion=shelfSwVersion, cmMacAddress=cmMacAddress, portNumber=portNumber, portPrState=portPrState, cardSubDetected=cardSubDetected, auditEntry=auditEntry, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestResult=TestResult, equipmentTbl=equipmentTbl, auditPassedCount=auditPassedCount, cardNumPorts=cardNumPorts, auditDescription=auditDescription, cardSubType=cardSubType, portMinDsPower=portMinDsPower, TestScheduleCommand=TestScheduleCommand, cardFlashSize=cardFlashSize, auditName=auditName, diskVolumeEntry=diskVolumeEntry, cardPicSerialNum=cardPicSerialNum, diskVolumeTable=diskVolumeTable, cardTempOverHeatNotification=cardTempOverHeatNotification, remainInDiagMode=remainInDiagMode, auditTotalPassedCount=auditTotalPassedCount, cmResetNotification=cmResetNotification, cardSpareGroupId=cardSpareGroupId, portType=portType, cardTestType=cardTestType, cardTestCommand=cardTestCommand, auditResult=auditResult, diskVolumeShelfId=diskVolumeShelfId, equipmentError=equipmentError, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeName=diskVolumeName, TestCommand=TestCommand, diskVolumeSize=diskVolumeSize, portDescription=portDescription, cardTestEntry=cardTestEntry, auditStatus=auditStatus, cardTestScheduleCommand=cardTestScheduleCommand, portMode=portMode)
