...
EqActionType, CardSubType, PortId, FirmwareSource, FlowControlMode, PrimaryState, PicType, UnknownState, DiskVolumeUsageLevel, CardId, PortDetectedMode, DuplexStatus, SecondaryState, CardType, PortType, PortMode, ShelfId, AdminState = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "CardSubType", "PortId", "FirmwareSource", "FlowControlMode", "PrimaryState", "PicType", "UnknownState", "DiskVolumeUsageLevel", "CardId", "PortDetectedMode", "DuplexStatus", "SecondaryState", "CardType", "PortType", "PortMode", "ShelfId", "AdminState")
...
MacAddress, RowStatus, TruthValue, TextualConvention, DisplayString, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "TruthValue", "TextualConvention", "DisplayString", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentTraps=equipmentTraps, cardEntry=cardEntry, TestSchedule=TestSchedule, portMacAddress=portMacAddress, auditEntry=auditEntry, TestType=TestType, diskVolumeName=diskVolumeName, TestTransactionId=TestTransactionId, cardLastResetReason=cardLastResetReason, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestId=cardTestId, cardAction=cardAction, portDocsIfIndex=portDocsIfIndex, equipmentAudit=equipmentAudit, portCurrDsPower=portCurrDsPower, cmVendor=cmVendor, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeTrapInh=diskVolumeTrapInh, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeCardId=diskVolumeCardId, cmResetUpTime=cmResetUpTime, diskVolumeUsageLevel=diskVolumeUsageLevel, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeGroup=diskVolumeGroup, equipmentDiag=equipmentDiag, cardLastBootSource=cardLastBootSource, cmResetInfo=cmResetInfo, portAdminState=portAdminState, cardTestSchedule=cardTestSchedule, portTrapInh=portTrapInh, diagTestId=diagTestId, portPrState=portPrState, cardTestScheduleCommand=cardTestScheduleCommand, cardTestResultDesc=cardTestResultDesc, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardNumPorts=cardNumPorts, systemGeneral=systemGeneral, cardFwVersion=cardFwVersion, cardUpTime=cardUpTime, equipmentStateGroup=equipmentStateGroup, systemKey=systemKey, errEventId=errEventId, shelfSwVersion=shelfSwVersion, portBgpId=portBgpId, cardTestName=cardTestName, portTxFlowControlMode=portTxFlowControlMode, cardCarrierSerialNum=cardCarrierSerialNum, cardTestResult=cardTestResult, cardFpgaSource=cardFpgaSource, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetNotification=cmResetNotification, diskVolumeSize=diskVolumeSize, cardPicHwVersion=cardPicHwVersion, portId=portId, verboseLevel=verboseLevel, auditName=auditName, cmDevice=cmDevice, cardBusSpeed=cardBusSpeed, fpgaErrorEventTable=fpgaErrorEventTable, cardFwUpdateStatus=cardFwUpdateStatus, portDetectedMode=portDetectedMode, auditLogOutput=auditLogOutput, errEvRowStatus=errEvRowStatus, portSecStateChange=portSecStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCPLDVersion=cardCPLDVersion, portConnectorId=portConnectorId, cardHwVersion=cardHwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastBootVersion=cardLastBootVersion, cmResetClearNotification=cmResetClearNotification, cardRamSize=cardRamSize, TestCommand=TestCommand, cardSubDetected=cardSubDetected, cmResetReason=cmResetReason, portMacIfIndex=portMacIfIndex, cardPicDetected=cardPicDetected, TestResult=TestResult, portLastChangeTime=portLastChangeTime, cardType=cardType, cardSerialNum=cardSerialNum, diskVolumeNumber=diskVolumeNumber, remainInDiagMode=remainInDiagMode, cardAdminState=cardAdminState, diskVolumeEntry=diskVolumeEntry, cardLastChangeTime=cardLastChangeTime, cardDplxStatusChange=cardDplxStatusChange, portDplxStatus=portDplxStatus, consoleOutput=consoleOutput, shelfNumber=shelfNumber, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestTime=cardTestTime, auditTable=auditTable, equipmentError=equipmentError, portShelfId=portShelfId, cardTable=cardTable, auditTime=auditTime, systemClock=systemClock, portTable=portTable, equipmentPortGroup=equipmentPortGroup, auditFailedCount=auditFailedCount, portMinDsPower=portMinDsPower, portRxFlowControlMode=portRxFlowControlMode, cardSecState=cardSecState, diskVolumeFileName=diskVolumeFileName, auditCommand=auditCommand, cardTestType=cardTestType, auditAutoScheduling=auditAutoScheduling, portAction=portAction, cardTestIndex=cardTestIndex, portNumber=portNumber, portEntry=portEntry, cardShelfTestIndex=cardShelfTestIndex, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPrStateChange=cardPrStateChange, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentAuditGroup=equipmentAuditGroup, eqDiagConfig=eqDiagConfig, cardTempNoReportNotification=cardTempNoReportNotification, cmRegistrationNotification=cmRegistrationNotification, cardTestDescription=cardTestDescription, errEvLogLevel=errEvLogLevel, auditTotalPassedCount=auditTotalPassedCount, shelfObjects=shelfObjects, cardTestTransId=cardTestTransId, diskDriveNumber=diskDriveNumber, cardTemperature=cardTemperature, diskVolumeShelfId=diskVolumeShelfId, auditId=auditId, portMode=portMode, errEvRecoveryEnabled=errEvRecoveryEnabled, cardDetectedChange=cardDetectedChange, cardPicSerialNum=cardPicSerialNum, downstreamPowerLoss=downstreamPowerLoss, diskVolumeDiskSize=diskVolumeDiskSize, equipmentState=equipmentState, equipmentDiagGroup=equipmentDiagGroup, cardTestResultNotification=cardTestResultNotification, cardTrapInh=cardTrapInh, auditStatus=auditStatus, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, shelfName=shelfName, portCardId=portCardId, cmPrimarySID=cmPrimarySID, cardNumber=cardNumber, trapCounter=trapCounter, auditCardId=auditCardId, cmIpAddress=cmIpAddress, cardTemperatureHighWarn=cardTemperatureHighWarn, systemGeneralGroup=systemGeneralGroup, cardDplxStatus=cardDplxStatus, portMaxDsPower=portMaxDsPower, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetStatus=cmResetStatus, equipmentTbl=equipmentTbl, cardSpareGroupMode=cardSpareGroupMode, cardDetected=cardDetected, cardCpuType=cardCpuType, cardTestTable=cardTestTable, errEvLoggingEnabled=errEvLoggingEnabled, portPrStateChange=portPrStateChange, portNumChans=portNumChans, portType=portType, cardTemperatureHighError=cardTemperatureHighError, auditShelfId=auditShelfId, cardPrState=cardPrState, cardId=cardId, cadEquipmentMib=cadEquipmentMib, cardSpareGroupId=cardSpareGroupId, cardShelfId=cardShelfId, cardName=cardName, portDescription=portDescription, cardFlashSize=cardFlashSize, auditCycleCount=auditCycleCount, cardTestEntry=cardTestEntry, cmUChannelID=cmUChannelID, TestScheduleCommand=TestScheduleCommand, diskVolumeId=diskVolumeId, equipmentShelfGroup=equipmentShelfGroup, trapSeverity=trapSeverity, cardSecStateChange=cardSecStateChange, cmMacAddress=cmMacAddress, auditLogThrottle=auditLogThrottle, diskVolumeTable=diskVolumeTable, cardSubType=cardSubType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCpuSpeed=cardCpuSpeed, auditDescription=auditDescription, TestId=TestId, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeDriveId=diskVolumeDriveId, PYSNMP_MODULE_ID=cadEquipmentMib, cardSwVersion=cardSwVersion, cardBootVersion=cardBootVersion, cardTestCommand=cardTestCommand, auditPassedCount=auditPassedCount, portSecState=portSecState, portCardSubType=portCardSubType, auditResult=auditResult, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification)
