...
EqActionType, CardId, AdminState, DuplexStatus, ShelfId, FirmwareSource, PortDetectedMode, PrimaryState, PortMode, SecondaryState, PicType, UnknownState, CardType, PortId, PortType, FlowControlMode, DiskVolumeUsageLevel, CardSubType = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "CardId", "AdminState", "DuplexStatus", "ShelfId", "FirmwareSource", "PortDetectedMode", "PrimaryState", "PortMode", "SecondaryState", "PicType", "UnknownState", "CardType", "PortId", "PortType", "FlowControlMode", "DiskVolumeUsageLevel", "CardSubType")
...
TextualConvention, DateAndTime, TimeStamp, DisplayString, TruthValue, RowStatus, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "TimeStamp", "DisplayString", "TruthValue", "RowStatus", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResultNotification=cardTestResultNotification, cmResetReason=cmResetReason, cardTemperatureHighError=cardTemperatureHighError, TestId=TestId, portTable=portTable, TestCommand=TestCommand, portNumChans=portNumChans, cardName=cardName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDetectedMode=portDetectedMode, cardTestCommand=cardTestCommand, cmResetInfo=cmResetInfo, cmResetUpTime=cmResetUpTime, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardShelfId=cardShelfId, cardPicDetected=cardPicDetected, cardSecStateChange=cardSecStateChange, cardAdminState=cardAdminState, portPrState=portPrState, cardTestDescription=cardTestDescription, diskVolumeUsageNotification=diskVolumeUsageNotification, portMacAddress=portMacAddress, remainInDiagMode=remainInDiagMode, auditTime=auditTime, portCurrDsPower=portCurrDsPower, portAction=portAction, diskVolumeUsageLevel=diskVolumeUsageLevel, portBgpId=portBgpId, cmPrimarySID=cmPrimarySID, cardSecState=cardSecState, cardRamSize=cardRamSize, portType=portType, cardFlashSize=cardFlashSize, equipmentTbl=equipmentTbl, cardDetected=cardDetected, cardPrState=cardPrState, auditTotalFailedCount=auditTotalFailedCount, auditId=auditId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTxFlowControlDetected=portTxFlowControlDetected, portCardId=portCardId, cardSubType=cardSubType, cardCarrierHwVersion=cardCarrierHwVersion, TestResult=TestResult, cardTestTransId=cardTestTransId, portDescription=portDescription, downstreamPowerLoss=downstreamPowerLoss, cardTestResultDesc=cardTestResultDesc, shelfName=shelfName, cardBusSpeed=cardBusSpeed, cmResetNotification=cmResetNotification, equipmentTraps=equipmentTraps, diskVolumeFileName=diskVolumeFileName, diskVolumeGroup=diskVolumeGroup, cardEntry=cardEntry, cardTemperature=cardTemperature, cardType=cardType, cardAction=cardAction, cardPicHwVersion=cardPicHwVersion, diskVolumeTrapInh=diskVolumeTrapInh, equipmentPortGroup=equipmentPortGroup, cardLastChangeTime=cardLastChangeTime, diagTestId=diagTestId, portShelfId=portShelfId, shelfObjects=shelfObjects, portDplxStatusChange=portDplxStatusChange, systemGeneralGroup=systemGeneralGroup, portDocsIfIndex=portDocsIfIndex, cardCarrierFwVersion=cardCarrierFwVersion, portLastChangeTime=portLastChangeTime, diskVolumeId=diskVolumeId, equipmentState=equipmentState, fpgaErrorEventEntry=fpgaErrorEventEntry, cardBootVersion=cardBootVersion, eqDiagConfig=eqDiagConfig, cardHwVersion=cardHwVersion, equipmentAuditGroup=equipmentAuditGroup, TestTransactionId=TestTransactionId, equipmentAudit=equipmentAudit, cardNumber=cardNumber, errEvLoggingEnabled=errEvLoggingEnabled, consoleOutput=consoleOutput, cardCpuType=cardCpuType, diskDriveNumber=diskDriveNumber, fpgaErrorEventTable=fpgaErrorEventTable, shelfNumber=shelfNumber, cardLastResetReason=cardLastResetReason, portTxFlowControlMode=portTxFlowControlMode, portMacIfIndex=portMacIfIndex, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portTrapInh=portTrapInh, systemKey=systemKey, portDplxStatus=portDplxStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardCPLDVersion=cardCPLDVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeNumber=diskVolumeNumber, cardLastBootSource=cardLastBootSource, cardTestName=cardTestName, cardDplxStatusChange=cardDplxStatusChange, portMinDsPower=portMinDsPower, diskVolumeTable=diskVolumeTable, errEvLogLevel=errEvLogLevel, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSpareGroupId=cardSpareGroupId, cardDplxStatus=cardDplxStatus, equipmentDiagGroup=equipmentDiagGroup, shelfSwVersion=shelfSwVersion, portMode=portMode, portSecStateChange=portSecStateChange, cmResetStatus=cmResetStatus, diskVolumeDriveId=diskVolumeDriveId, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSubDetected=cardSubDetected, cardTrapInh=cardTrapInh, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSpareGroupMode=cardSpareGroupMode, cmResetClearNotification=cmResetClearNotification, cmIpAddress=cmIpAddress, portCardSubType=portCardSubType, portConnectorId=portConnectorId, equipmentShelfGroup=equipmentShelfGroup, auditResult=auditResult, cardFpgaSource=cardFpgaSource, equipmentCardTestGroup=equipmentCardTestGroup, portId=portId, diskVolumeCardId=diskVolumeCardId, cardFwVersion=cardFwVersion, cardTestResult=cardTestResult, cardTemperatureHighWarn=cardTemperatureHighWarn, auditPassedCount=auditPassedCount, auditName=auditName, PYSNMP_MODULE_ID=cadEquipmentMib, trapCounter=trapCounter, equipmentError=equipmentError, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSerialNum=cardSerialNum, diskVolumeSize=diskVolumeSize, cardPrStateChange=cardPrStateChange, auditCardId=auditCardId, fpgaErrorEventGroup=fpgaErrorEventGroup, trapSeverity=trapSeverity, auditLogOutput=auditLogOutput, auditCycleCount=auditCycleCount, portAdminState=portAdminState, cardTestScheduleCommand=cardTestScheduleCommand, cmMacAddress=cmMacAddress, diskVolumeDiskSize=diskVolumeDiskSize, cardTestType=cardTestType, cardLastBootVersion=cardLastBootVersion, auditCommand=auditCommand, portMaxDsPower=portMaxDsPower, cmDevice=cmDevice, cardUpTime=cardUpTime, cardCpuSpeed=cardCpuSpeed, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeShelfId=diskVolumeShelfId, errEventId=errEventId, auditDescription=auditDescription, cardFwUpdateStatus=cardFwUpdateStatus, verboseLevel=verboseLevel, cardTestTable=cardTestTable, TestType=TestType, diskVolumeName=diskVolumeName, portRxFlowControlMode=portRxFlowControlMode, portPrStateChange=portPrStateChange, cardTestIndex=cardTestIndex, systemGeneral=systemGeneral, cmRegistrationNotification=cmRegistrationNotification, TestScheduleCommand=TestScheduleCommand, auditFailedCount=auditFailedCount, auditLogThrottle=auditLogThrottle, cardShelfTestIndex=cardShelfTestIndex, cardSwVersion=cardSwVersion, cardNumPorts=cardNumPorts, equipmentCardGroup=equipmentCardGroup, equipmentStateGroup=equipmentStateGroup, auditStatus=auditStatus, cmVendor=cmVendor, auditTotalPassedCount=auditTotalPassedCount, cardTempNoReportNotification=cardTempNoReportNotification, cmUChannelID=cmUChannelID, cardId=cardId, cadEquipmentMib=cadEquipmentMib, TestSchedule=TestSchedule, cardTestEntry=cardTestEntry, auditTable=auditTable, cardTable=cardTable, portNumber=portNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, errEvRowStatus=errEvRowStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditEntry=auditEntry, equipmentDiag=equipmentDiag, cardDetectedChange=cardDetectedChange, cardPicSerialNum=cardPicSerialNum, cardTestTime=cardTestTime, auditAutoScheduling=auditAutoScheduling, portEntry=portEntry, auditShelfId=auditShelfId, cardTestSchedule=cardTestSchedule, portSecState=portSecState, cardTestId=cardTestId, diskVolumeEntry=diskVolumeEntry, systemClock=systemClock, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification)
