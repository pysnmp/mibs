...
PortId, PortMode, PicType, CardSubType, CardType, EqActionType, PortType, CardId, SecondaryState, DuplexStatus, UnknownState, PortDetectedMode, ShelfId, FlowControlMode, FirmwareSource, AdminState, DiskVolumeUsageLevel, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "PortId", "PortMode", "PicType", "CardSubType", "CardType", "EqActionType", "PortType", "CardId", "SecondaryState", "DuplexStatus", "UnknownState", "PortDetectedMode", "ShelfId", "FlowControlMode", "FirmwareSource", "AdminState", "DiskVolumeUsageLevel", "PrimaryState")
...
RowStatus, DateAndTime, DisplayString, TruthValue, MacAddress, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "DisplayString", "TruthValue", "MacAddress", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSecStateChange=cardSecStateChange, cardCarrierFwVersion=cardCarrierFwVersion, cmDevice=cmDevice, cardTestResultDesc=cardTestResultDesc, cardShelfId=cardShelfId, cardShelfTestIndex=cardShelfTestIndex, cardTestType=cardTestType, portDescription=portDescription, auditTotalPassedCount=auditTotalPassedCount, cmResetNotification=cmResetNotification, equipmentShelfGroup=equipmentShelfGroup, equipmentState=equipmentState, auditLogOutput=auditLogOutput, cardPicHwVersion=cardPicHwVersion, auditCardId=auditCardId, consoleOutput=consoleOutput, fpgaErrorEventEntry=fpgaErrorEventEntry, errEvLogLevel=errEvLogLevel, cardPicDetected=cardPicDetected, cardType=cardType, diskVolumeEntry=diskVolumeEntry, diskVolumeDriveId=diskVolumeDriveId, portDetectedMode=portDetectedMode, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeId=diskVolumeId, portMinDsPower=portMinDsPower, downstreamPowerLoss=downstreamPowerLoss, equipmentAudit=equipmentAudit, fpgaErrorEventTable=fpgaErrorEventTable, portAdminState=portAdminState, cardLastBootSource=cardLastBootSource, cardDplxStatusChange=cardDplxStatusChange, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentStateGroup=equipmentStateGroup, portNumber=portNumber, auditPassedCount=auditPassedCount, cardTestCommand=cardTestCommand, portCardSubType=portCardSubType, cardTemperature=cardTemperature, cardTestTime=cardTestTime, portMode=portMode, cadEquipmentMibConformance=cadEquipmentMibConformance, portMacAddress=portMacAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestResultNotification=cardTestResultNotification, diskVolumeSize=diskVolumeSize, diskVolumeTrapInh=diskVolumeTrapInh, cardUpTime=cardUpTime, cmResetStatus=cmResetStatus, auditCommand=auditCommand, errEvRowStatus=errEvRowStatus, auditResult=auditResult, cardTempOverHeatNotification=cardTempOverHeatNotification, cardAdminState=cardAdminState, cardTestName=cardTestName, cardPrStateChange=cardPrStateChange, cardTestTable=cardTestTable, cardTestResult=cardTestResult, cardTestTransId=cardTestTransId, auditFailedCount=auditFailedCount, shelfNumber=shelfNumber, portShelfId=portShelfId, equipmentDiagGroup=equipmentDiagGroup, PYSNMP_MODULE_ID=cadEquipmentMib, TestTransactionId=TestTransactionId, cardSubType=cardSubType, auditCycleCount=auditCycleCount, cardSecState=cardSecState, trapSeverity=trapSeverity, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFlashSize=cardFlashSize, cardBootVersion=cardBootVersion, cardFpgaSource=cardFpgaSource, cardTestEntry=cardTestEntry, cadEquipmentMib=cadEquipmentMib, cardPicSerialNum=cardPicSerialNum, portRxFlowControlMode=portRxFlowControlMode, cardPrState=cardPrState, diskVolumeShelfId=diskVolumeShelfId, cardCpuType=cardCpuType, TestId=TestId, TestScheduleCommand=TestScheduleCommand, portId=portId, equipmentTraps=equipmentTraps, cardSwVersion=cardSwVersion, cmResetClearNotification=cmResetClearNotification, cardCpuSpeed=cardCpuSpeed, auditShelfId=auditShelfId, eqDiagConfig=eqDiagConfig, systemClock=systemClock, cardNumPorts=cardNumPorts, portNumChans=portNumChans, diskVolumeGroup=diskVolumeGroup, portTrapInh=portTrapInh, auditTable=auditTable, portPrStateChange=portPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCarrierSerialNum=cardCarrierSerialNum, cmResetReason=cmResetReason, systemGeneral=systemGeneral, equipmentDiag=equipmentDiag, trapCounter=trapCounter, errEventId=errEventId, portLastChangeTime=portLastChangeTime, cmResetInfo=cmResetInfo, cardEntry=cardEntry, TestCommand=TestCommand, cardLastChangeTime=cardLastChangeTime, portBgpId=portBgpId, verboseLevel=verboseLevel, equipmentPortGroup=equipmentPortGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTemperatureHighError=cardTemperatureHighError, cardDetectedChange=cardDetectedChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardHwVersion=cardHwVersion, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeNumber=diskVolumeNumber, equipmentAuditGroup=equipmentAuditGroup, portDplxStatusChange=portDplxStatusChange, equipmentError=equipmentError, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardLastResetReason=cardLastResetReason, shelfObjects=shelfObjects, diskVolumeCardId=diskVolumeCardId, auditId=auditId, errEvLoggingEnabled=errEvLoggingEnabled, auditTotalFailedCount=auditTotalFailedCount, diskVolumeName=diskVolumeName, diskVolumeUsageNotification=diskVolumeUsageNotification, portSecStateChange=portSecStateChange, portCardId=portCardId, portTxFlowControlMode=portTxFlowControlMode, remainInDiagMode=remainInDiagMode, auditName=auditName, portAction=portAction, portMacIfIndex=portMacIfIndex, cardDplxStatus=cardDplxStatus, cmResetUpTime=cmResetUpTime, cardFwUpdateStatus=cardFwUpdateStatus, TestType=TestType, cardLastBootVersion=cardLastBootVersion, cardTestSchedule=cardTestSchedule, portDocsIfIndex=portDocsIfIndex, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cadEquipmentMibGroup=cadEquipmentMibGroup, diagTestId=diagTestId, portType=portType, cardCPLDVersion=cardCPLDVersion, cardSpareGroupMode=cardSpareGroupMode, cardRamSize=cardRamSize, shelfName=shelfName, auditAutoScheduling=auditAutoScheduling, cardId=cardId, portSecState=portSecState, cardSpareGroupId=cardSpareGroupId, cardTestIndex=cardTestIndex, cardName=cardName, portConnectorId=portConnectorId, cardTestId=cardTestId, portTxFlowControlDetected=portTxFlowControlDetected, cardTestDescription=cardTestDescription, portPrState=portPrState, portRxFlowControlDetected=portRxFlowControlDetected, auditDescription=auditDescription, cardSerialNum=cardSerialNum, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmRegistrationNotification=cmRegistrationNotification, equipmentTbl=equipmentTbl, portMaxDsPower=portMaxDsPower, cmVendor=cmVendor, portCurrDsPower=portCurrDsPower, portDplxStatus=portDplxStatus, cmMacAddress=cmMacAddress, diskVolumeFileName=diskVolumeFileName, cardTemperatureHighWarn=cardTemperatureHighWarn, cardFwVersion=cardFwVersion, auditTime=auditTime, cardSubDetected=cardSubDetected, equipmentCardGroup=equipmentCardGroup, portEntry=portEntry, cardTable=cardTable, auditStatus=auditStatus, cmUChannelID=cmUChannelID, equipmentCardTestGroup=equipmentCardTestGroup, cardTrapInh=cardTrapInh, systemKey=systemKey, TestSchedule=TestSchedule, systemGeneralGroup=systemGeneralGroup, auditEntry=auditEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestResult=TestResult, diskVolumeTable=diskVolumeTable, cardDetected=cardDetected, portTable=portTable, cmPrimarySID=cmPrimarySID, shelfSwVersion=shelfSwVersion, cardNumber=cardNumber, diskDriveNumber=diskDriveNumber, auditLogThrottle=auditLogThrottle, cmIpAddress=cmIpAddress, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardBusSpeed=cardBusSpeed, cardAction=cardAction)
