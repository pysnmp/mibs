...
PicType, PortDetectedMode, CardType, AdminState, UnknownState, PrimaryState, PortMode, FirmwareSource, FlowControlMode, PortId, PortType, SecondaryState, DiskVolumeUsageLevel, CardId, ShelfId, DuplexStatus, EqActionType, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PicType", "PortDetectedMode", "CardType", "AdminState", "UnknownState", "PrimaryState", "PortMode", "FirmwareSource", "FlowControlMode", "PortId", "PortType", "SecondaryState", "DiskVolumeUsageLevel", "CardId", "ShelfId", "DuplexStatus", "EqActionType", "CardSubType")
...
DisplayString, TextualConvention, RowStatus, DateAndTime, TimeStamp, MacAddress, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TextualConvention", "RowStatus", "DateAndTime", "TimeStamp", "MacAddress", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfObjects=shelfObjects, shelfSwVersion=shelfSwVersion, diskVolumeName=diskVolumeName, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibConformance=cadEquipmentMibConformance, cardPrState=cardPrState, cardPicHwVersion=cardPicHwVersion, systemClock=systemClock, cardFwVersion=cardFwVersion, equipmentTbl=equipmentTbl, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFpgaSource=cardFpgaSource, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTemperatureHighError=cardTemperatureHighError, TestTransactionId=TestTransactionId, TestType=TestType, equipmentDiag=equipmentDiag, diskVolumeDiskSize=diskVolumeDiskSize, remainInDiagMode=remainInDiagMode, portDplxStatusChange=portDplxStatusChange, equipmentError=equipmentError, cardLastBootVersion=cardLastBootVersion, errEvRowStatus=errEvRowStatus, diskVolumeTable=diskVolumeTable, cardCarrierFwVersion=cardCarrierFwVersion, portCurrDsPower=portCurrDsPower, cmDevice=cmDevice, cardTestCommand=cardTestCommand, diskVolumeTrapInh=diskVolumeTrapInh, auditLogOutput=auditLogOutput, cmResetClearNotification=cmResetClearNotification, cardDplxStatus=cardDplxStatus, diagTestId=diagTestId, cardTestName=cardTestName, cardDplxStatusChange=cardDplxStatusChange, diskVolumeFileName=diskVolumeFileName, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditCycleCount=auditCycleCount, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeCardId=diskVolumeCardId, cardSubType=cardSubType, cardPrStateChange=cardPrStateChange, cardSecState=cardSecState, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentAuditGroup=equipmentAuditGroup, downstreamPowerLoss=downstreamPowerLoss, cardFwUpdateStatus=cardFwUpdateStatus, cardTestId=cardTestId, auditPassedCount=auditPassedCount, trapCounter=trapCounter, cardSecStateChange=cardSecStateChange, portPrState=portPrState, portShelfId=portShelfId, cardTestResultNotification=cardTestResultNotification, cardNumPorts=cardNumPorts, portMinDsPower=portMinDsPower, cardUpTime=cardUpTime, portAdminState=portAdminState, portTxFlowControlMode=portTxFlowControlMode, auditName=auditName, cardTestTime=cardTestTime, auditEntry=auditEntry, cardType=cardType, cardTestResult=cardTestResult, portId=portId, systemKey=systemKey, cmIpAddress=cmIpAddress, equipmentShelfGroup=equipmentShelfGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardTestType=cardTestType, cmResetReason=cmResetReason, cardTestResultDesc=cardTestResultDesc, cardPicDetected=cardPicDetected, diskVolumeSize=diskVolumeSize, diskVolumeEntry=diskVolumeEntry, auditId=auditId, equipmentCardTestGroup=equipmentCardTestGroup, equipmentStateGroup=equipmentStateGroup, portEntry=portEntry, diskVolumeDriveId=diskVolumeDriveId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portDplxStatus=portDplxStatus, auditTime=auditTime, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicSerialNum=cardPicSerialNum, cmResetInfo=cmResetInfo, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestTable=cardTestTable, TestId=TestId, cardDetected=cardDetected, cardTable=cardTable, cardId=cardId, auditTable=auditTable, cardTestSchedule=cardTestSchedule, shelfName=shelfName, cardTestScheduleCommand=cardTestScheduleCommand, auditCardId=auditCardId, TestSchedule=TestSchedule, portCardSubType=portCardSubType, portNumber=portNumber, diskVolumeGroup=diskVolumeGroup, portCardId=portCardId, cardCpuType=cardCpuType, verboseLevel=verboseLevel, auditLogThrottle=auditLogThrottle, portTable=portTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardShelfId=cardShelfId, cardSwVersion=cardSwVersion, portMode=portMode, cardTestIndex=cardTestIndex, cardTemperature=cardTemperature, cardTestDescription=cardTestDescription, auditTotalPassedCount=auditTotalPassedCount, shelfNumber=shelfNumber, cmResetStatus=cmResetStatus, portMacIfIndex=portMacIfIndex, cardTestTransId=cardTestTransId, cardDetectedChange=cardDetectedChange, cardSpareGroupMode=cardSpareGroupMode, auditTotalFailedCount=auditTotalFailedCount, cmResetUpTime=cmResetUpTime, portBgpId=portBgpId, portDetectedMode=portDetectedMode, cardNumber=cardNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastResetReason=cardLastResetReason, systemGeneralGroup=systemGeneralGroup, cmPrimarySID=cmPrimarySID, cardAction=cardAction, portRxFlowControlDetected=portRxFlowControlDetected, portTrapInh=portTrapInh, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCarrierHwVersion=cardCarrierHwVersion, auditDescription=auditDescription, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, trapSeverity=trapSeverity, cardEntry=cardEntry, cardHwVersion=cardHwVersion, cardSpareGroupId=cardSpareGroupId, portLastChangeTime=portLastChangeTime, portMacAddress=portMacAddress, eqDiagConfig=eqDiagConfig, systemGeneral=systemGeneral, portDescription=portDescription, cmMacAddress=cmMacAddress, fpgaErrorEventGroup=fpgaErrorEventGroup, diskDriveNumber=diskDriveNumber, cardAdminState=cardAdminState, errEventId=errEventId, cardSubDetected=cardSubDetected, equipmentCardGroup=equipmentCardGroup, cardCPLDVersion=cardCPLDVersion, portNumChans=portNumChans, cmRegistrationNotification=cmRegistrationNotification, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentState=equipmentState, auditFailedCount=auditFailedCount, TestResult=TestResult, portAction=portAction, diskVolumeId=diskVolumeId, cardCarrierSerialNum=cardCarrierSerialNum, errEvLogLevel=errEvLogLevel, portRxFlowControlMode=portRxFlowControlMode, consoleOutput=consoleOutput, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentPortGroup=equipmentPortGroup, cardBootVersion=cardBootVersion, cardSerialNum=cardSerialNum, cmUChannelID=cmUChannelID, portSecStateChange=portSecStateChange, fpgaErrorEventTable=fpgaErrorEventTable, portPrStateChange=portPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardBusSpeed=cardBusSpeed, diskVolumeUsageNotification=diskVolumeUsageNotification, cmVendor=cmVendor, equipmentAudit=equipmentAudit, cardLastChangeTime=cardLastChangeTime, TestCommand=TestCommand, portMaxDsPower=portMaxDsPower, diskVolumeShelfId=diskVolumeShelfId, auditResult=auditResult, cardLastBootSource=cardLastBootSource, TestScheduleCommand=TestScheduleCommand, portDocsIfIndex=portDocsIfIndex, cardTestEntry=cardTestEntry, cardName=cardName, cadEquipmentMib=cadEquipmentMib, auditShelfId=auditShelfId, cardCpuSpeed=cardCpuSpeed, cardRamSize=cardRamSize, cmResetNotification=cmResetNotification, equipmentDiagGroup=equipmentDiagGroup, portSecState=portSecState, cardTrapInh=cardTrapInh, portType=portType, diskVolumeNumber=diskVolumeNumber, cardFlashSize=cardFlashSize, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditCommand=auditCommand, auditStatus=auditStatus, cardShelfTestIndex=cardShelfTestIndex, equipmentTraps=equipmentTraps, portConnectorId=portConnectorId)
