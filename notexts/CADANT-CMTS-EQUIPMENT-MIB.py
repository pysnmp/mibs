...
FlowControlMode, SecondaryState, DiskVolumeUsageLevel, CardId, CardType, PicType, PortType, ShelfId, PortMode, UnknownState, CardSubType, DuplexStatus, FirmwareSource, PortDetectedMode, EqActionType, PortId, AdminState, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "SecondaryState", "DiskVolumeUsageLevel", "CardId", "CardType", "PicType", "PortType", "ShelfId", "PortMode", "UnknownState", "CardSubType", "DuplexStatus", "FirmwareSource", "PortDetectedMode", "EqActionType", "PortId", "AdminState", "PrimaryState")
...
RowStatus, TimeStamp, TextualConvention, DateAndTime, TruthValue, MacAddress, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "TextualConvention", "DateAndTime", "TruthValue", "MacAddress", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmResetReason=cmResetReason, diskVolumeName=diskVolumeName, cardTestIndex=cardTestIndex, portCurrDsPower=portCurrDsPower, cardSerialNum=cardSerialNum, cardPicDetected=cardPicDetected, portTxFlowControlMode=portTxFlowControlMode, cardDplxStatus=cardDplxStatus, cardSubDetected=cardSubDetected, diskVolumeId=diskVolumeId, cardSecState=cardSecState, portNumber=portNumber, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastChangeTime=cardLastChangeTime, cardSubType=cardSubType, auditCommand=auditCommand, equipmentTraps=equipmentTraps, auditDescription=auditDescription, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentDiagGroup=equipmentDiagGroup, portTrapInh=portTrapInh, equipmentCardGroup=equipmentCardGroup, cadEquipmentMib=cadEquipmentMib, portMaxDsPower=portMaxDsPower, diskVolumeFileName=diskVolumeFileName, cardCarrierFwVersion=cardCarrierFwVersion, portPrState=portPrState, cardTemperatureHighError=cardTemperatureHighError, diskVolumeCardId=diskVolumeCardId, cardTestCommand=cardTestCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, verboseLevel=verboseLevel, cardUpTime=cardUpTime, cmResetClearNotification=cmResetClearNotification, auditCardId=auditCardId, portBgpId=portBgpId, auditStatus=auditStatus, cardTestScheduleCommand=cardTestScheduleCommand, cardLastBootSource=cardLastBootSource, shelfObjects=shelfObjects, equipmentAudit=equipmentAudit, portDetectedMode=portDetectedMode, cardTestId=cardTestId, shelfNumber=shelfNumber, shelfSwVersion=shelfSwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeEntry=diskVolumeEntry, remainInDiagMode=remainInDiagMode, cardFwVersion=cardFwVersion, cardSwVersion=cardSwVersion, TestType=TestType, cardLastResetReason=cardLastResetReason, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeSize=diskVolumeSize, cardLastBootVersion=cardLastBootVersion, cardTestType=cardTestType, diskVolumeShelfId=diskVolumeShelfId, cardTempNoReportNotification=cardTempNoReportNotification, portDescription=portDescription, cmResetStatus=cmResetStatus, diskVolumeNumber=diskVolumeNumber, auditShelfId=auditShelfId, trapCounter=trapCounter, portId=portId, cardAdminState=cardAdminState, systemGeneralGroup=systemGeneralGroup, portCardSubType=portCardSubType, auditAutoScheduling=auditAutoScheduling, cmPrimarySID=cmPrimarySID, cardBootVersion=cardBootVersion, cardCpuType=cardCpuType, portDplxStatus=portDplxStatus, cardCpuSpeed=cardCpuSpeed, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEvRowStatus=errEvRowStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portCardId=portCardId, cardAction=cardAction, equipmentCardTestGroup=equipmentCardTestGroup, TestTransactionId=TestTransactionId, auditTotalFailedCount=auditTotalFailedCount, cmDevice=cmDevice, cmResetUpTime=cmResetUpTime, TestResult=TestResult, portAction=portAction, auditEntry=auditEntry, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestResultDesc=cardTestResultDesc, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSecStateChange=cardSecStateChange, portSecStateChange=portSecStateChange, diskVolumeDiskSize=diskVolumeDiskSize, cardPrStateChange=cardPrStateChange, portSecState=portSecState, errEvRecoveryEnabled=errEvRecoveryEnabled, trapSeverity=trapSeverity, cardPicSerialNum=cardPicSerialNum, cardId=cardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetNotification=cmResetNotification, cardDetectedChange=cardDetectedChange, cardSpareGroupMode=cardSpareGroupMode, portMacAddress=portMacAddress, cardPicHwVersion=cardPicHwVersion, auditTotalPassedCount=auditTotalPassedCount, portNumChans=portNumChans, portConnectorId=portConnectorId, portDplxStatusChange=portDplxStatusChange, cardTestName=cardTestName, cardSpareGroupId=cardSpareGroupId, equipmentPortGroup=equipmentPortGroup, diskDriveNumber=diskDriveNumber, cardFpgaSource=cardFpgaSource, cadEquipmentMibCompliance=cadEquipmentMibCompliance, systemKey=systemKey, auditName=auditName, auditFailedCount=auditFailedCount, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeTable=diskVolumeTable, cardTestSchedule=cardTestSchedule, portMinDsPower=portMinDsPower, cardShelfId=cardShelfId, cardDetected=cardDetected, cardCPLDVersion=cardCPLDVersion, portAdminState=portAdminState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestId=TestId, cardName=cardName, portRxFlowControlMode=portRxFlowControlMode, cmResetInfo=cmResetInfo, cardTemperature=cardTemperature, equipmentAuditGroup=equipmentAuditGroup, cmMacAddress=cmMacAddress, shelfName=shelfName, cardHwVersion=cardHwVersion, auditLogThrottle=auditLogThrottle, cardNumber=cardNumber, cardTrapInh=cardTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResultNotification=cardTestResultNotification, auditCycleCount=auditCycleCount, cardNumPorts=cardNumPorts, systemClock=systemClock, consoleOutput=consoleOutput, cmUChannelID=cmUChannelID, equipmentError=equipmentError, portMode=portMode, fpgaErrorEventGroup=fpgaErrorEventGroup, portEntry=portEntry, portType=portType, portMacIfIndex=portMacIfIndex, cardTestResult=cardTestResult, portLastChangeTime=portLastChangeTime, cardTestDescription=cardTestDescription, TestScheduleCommand=TestScheduleCommand, equipmentStateGroup=equipmentStateGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardEntry=cardEntry, systemGeneral=systemGeneral, cardBusSpeed=cardBusSpeed, auditLogOutput=auditLogOutput, TestCommand=TestCommand, cardPrState=cardPrState, cmIpAddress=cmIpAddress, portPrStateChange=portPrStateChange, auditTable=auditTable, auditTime=auditTime, downstreamPowerLoss=downstreamPowerLoss, cardTestEntry=cardTestEntry, diskVolumeGroup=diskVolumeGroup, cardShelfTestIndex=cardShelfTestIndex, portTable=portTable, diagTestId=diagTestId, auditPassedCount=auditPassedCount, cardTable=cardTable, equipmentState=equipmentState, eqDiagConfig=eqDiagConfig, cmVendor=cmVendor, cardDplxStatusChange=cardDplxStatusChange, auditResult=auditResult, equipmentTbl=equipmentTbl, cardRamSize=cardRamSize, cardFlashSize=cardFlashSize, errEventId=errEventId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestSchedule=TestSchedule, cardType=cardType, portShelfId=portShelfId, portDocsIfIndex=portDocsIfIndex, equipmentDiag=equipmentDiag, errEvLoggingEnabled=errEvLoggingEnabled, auditId=auditId, cmRegistrationNotification=cmRegistrationNotification, portTxFlowControlDetected=portTxFlowControlDetected, cardTestTable=cardTestTable, errEvLogLevel=errEvLogLevel, equipmentShelfGroup=equipmentShelfGroup, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestTime=cardTestTime, diskVolumeDriveId=diskVolumeDriveId, cardFwUpdateStatus=cardFwUpdateStatus, cardTestTransId=cardTestTransId, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile)
