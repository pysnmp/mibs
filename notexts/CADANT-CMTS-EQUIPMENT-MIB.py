...
FlowControlMode, PicType, SecondaryState, UnknownState, DiskVolumeUsageLevel, CardSubType, PortType, PortDetectedMode, PortId, EqActionType, AdminState, CardType, PortMode, DuplexStatus, ShelfId, CardId, FirmwareSource, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PicType", "SecondaryState", "UnknownState", "DiskVolumeUsageLevel", "CardSubType", "PortType", "PortDetectedMode", "PortId", "EqActionType", "AdminState", "CardType", "PortMode", "DuplexStatus", "ShelfId", "CardId", "FirmwareSource", "PrimaryState")
...
TimeStamp, RowStatus, TruthValue, DisplayString, MacAddress, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "RowStatus", "TruthValue", "DisplayString", "MacAddress", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portEntry=portEntry, cardTestEntry=cardTestEntry, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSubDetected=cardSubDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResult=cardTestResult, portTrapInh=portTrapInh, shelfNumber=shelfNumber, diskVolumeFileName=diskVolumeFileName, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentTraps=equipmentTraps, portAction=portAction, diskVolumeCardId=diskVolumeCardId, portCardId=portCardId, cardShelfId=cardShelfId, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardDplxStatus=cardDplxStatus, TestTransactionId=TestTransactionId, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetReason=cmResetReason, cmIpAddress=cmIpAddress, auditId=auditId, auditResult=auditResult, diskVolumeId=diskVolumeId, portShelfId=portShelfId, shelfSwVersion=shelfSwVersion, cardBootVersion=cardBootVersion, cmResetStatus=cmResetStatus, portAdminState=portAdminState, cardTestType=cardTestType, systemGeneral=systemGeneral, auditTotalPassedCount=auditTotalPassedCount, TestCommand=TestCommand, cardSerialNum=cardSerialNum, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestResultDesc=cardTestResultDesc, cardTestCommand=cardTestCommand, auditTime=auditTime, verboseLevel=verboseLevel, diskVolumeName=diskVolumeName, errEvLogLevel=errEvLogLevel, cardLastResetReason=cardLastResetReason, cardTemperature=cardTemperature, portType=portType, cardNumber=cardNumber, errEvRowStatus=errEvRowStatus, cardAction=cardAction, portNumChans=portNumChans, cardUpTime=cardUpTime, portNumber=portNumber, cmDevice=cmDevice, cardDetected=cardDetected, portDocsIfIndex=portDocsIfIndex, portBgpId=portBgpId, portDetectedMode=portDetectedMode, cardSecStateChange=cardSecStateChange, auditCommand=auditCommand, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, systemKey=systemKey, equipmentCardTestGroup=equipmentCardTestGroup, cardType=cardType, cmResetClearNotification=cmResetClearNotification, equipmentAuditGroup=equipmentAuditGroup, trapSeverity=trapSeverity, cardFpgaSource=cardFpgaSource, portCurrDsPower=portCurrDsPower, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTable=diskVolumeTable, cardTestName=cardTestName, auditAutoScheduling=auditAutoScheduling, diskVolumeGroup=diskVolumeGroup, diskDriveNumber=diskDriveNumber, cardCarrierHwVersion=cardCarrierHwVersion, cardSecState=cardSecState, cardCarrierSerialNum=cardCarrierSerialNum, cardNumPorts=cardNumPorts, portMacAddress=portMacAddress, fpgaErrorEventEntry=fpgaErrorEventEntry, systemGeneralGroup=systemGeneralGroup, diagTestId=diagTestId, equipmentStateGroup=equipmentStateGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardName=cardName, cardDetectedChange=cardDetectedChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmVendor=cmVendor, cardLastChangeTime=cardLastChangeTime, cardSpareGroupMode=cardSpareGroupMode, diskVolumeTrapInh=diskVolumeTrapInh, cardCpuSpeed=cardCpuSpeed, auditStatus=auditStatus, PYSNMP_MODULE_ID=cadEquipmentMib, fpgaErrorEventTable=fpgaErrorEventTable, equipmentAudit=equipmentAudit, cardCPLDVersion=cardCPLDVersion, shelfObjects=shelfObjects, equipmentError=equipmentError, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditLogThrottle=auditLogThrottle, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestType=TestType, diskVolumeUsageNotification=diskVolumeUsageNotification, errEventId=errEventId, cmRegistrationNotification=cmRegistrationNotification, eqDiagConfig=eqDiagConfig, downstreamPowerLoss=downstreamPowerLoss, auditPassedCount=auditPassedCount, cmPrimarySID=cmPrimarySID, portConnectorId=portConnectorId, TestId=TestId, cardTestIndex=cardTestIndex, TestResult=TestResult, cmUChannelID=cmUChannelID, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentTbl=equipmentTbl, equipmentShelfGroup=equipmentShelfGroup, equipmentCardGroup=equipmentCardGroup, auditCycleCount=auditCycleCount, cardPicSerialNum=cardPicSerialNum, portTxFlowControlDetected=portTxFlowControlDetected, cardId=cardId, auditFailedCount=auditFailedCount, diskVolumeSize=diskVolumeSize, auditLogOutput=auditLogOutput, portSecState=portSecState, portMode=portMode, cmResetInfo=cmResetInfo, portTable=portTable, cardAdminState=cardAdminState, portMinDsPower=portMinDsPower, cardTestTable=cardTestTable, cardShelfTestIndex=cardShelfTestIndex, auditDescription=auditDescription, auditEntry=auditEntry, cardFlashSize=cardFlashSize, portMacIfIndex=portMacIfIndex, cardPrStateChange=cardPrStateChange, systemClock=systemClock, cardFwUpdateStatus=cardFwUpdateStatus, cardPicDetected=cardPicDetected, portLastChangeTime=portLastChangeTime, cardTestId=cardTestId, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeShelfId=diskVolumeShelfId, portRxFlowControlMode=portRxFlowControlMode, diskVolumeEntry=diskVolumeEntry, cardTestScheduleCommand=cardTestScheduleCommand, cardDplxStatusChange=cardDplxStatusChange, cardEntry=cardEntry, portDplxStatusChange=portDplxStatusChange, cardSubType=cardSubType, portCardSubType=portCardSubType, cardTestSchedule=cardTestSchedule, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeDiskSize=diskVolumeDiskSize, cmResetNotification=cmResetNotification, TestSchedule=TestSchedule, cardSwVersion=cardSwVersion, cardLastBootSource=cardLastBootSource, diskVolumeDriveId=diskVolumeDriveId, cardTrapInh=cardTrapInh, cmResetUpTime=cmResetUpTime, portSecStateChange=portSecStateChange, cardTestResultNotification=cardTestResultNotification, cmMacAddress=cmMacAddress, diskVolumeNumber=diskVolumeNumber, portId=portId, cardTestTransId=cardTestTransId, cardTemperatureHighError=cardTemperatureHighError, cardTable=cardTable, equipmentDiag=equipmentDiag, portMaxDsPower=portMaxDsPower, cadEquipmentMib=cadEquipmentMib, portPrState=portPrState, auditTable=auditTable, equipmentPortGroup=equipmentPortGroup, equipmentState=equipmentState, errEvLoggingEnabled=errEvLoggingEnabled, remainInDiagMode=remainInDiagMode, cardTestTime=cardTestTime, cadEquipmentMibGroup=cadEquipmentMibGroup, auditCardId=auditCardId, cardTestDescription=cardTestDescription, auditShelfId=auditShelfId, portPrStateChange=portPrStateChange, equipmentDiagGroup=equipmentDiagGroup, TestScheduleCommand=TestScheduleCommand, shelfName=shelfName, cardBusSpeed=cardBusSpeed, cardCarrierFwVersion=cardCarrierFwVersion, cardPicHwVersion=cardPicHwVersion, cardFwVersion=cardFwVersion, cardPrState=cardPrState, trapCounter=trapCounter, cardLastBootVersion=cardLastBootVersion, cardCpuType=cardCpuType, cardHwVersion=cardHwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, portDplxStatus=portDplxStatus, auditName=auditName, consoleOutput=consoleOutput, cardRamSize=cardRamSize, portDescription=portDescription, auditTotalFailedCount=auditTotalFailedCount, cardSpareGroupId=cardSpareGroupId)
