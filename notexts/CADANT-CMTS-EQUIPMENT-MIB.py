...
PortDetectedMode, PortType, DuplexStatus, PortId, PortMode, FlowControlMode, ShelfId, CardSubType, CardType, CardId, EqActionType, DiskVolumeUsageLevel, AdminState, FirmwareSource, PrimaryState, SecondaryState, UnknownState, PicType = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "PortType", "DuplexStatus", "PortId", "PortMode", "FlowControlMode", "ShelfId", "CardSubType", "CardType", "CardId", "EqActionType", "DiskVolumeUsageLevel", "AdminState", "FirmwareSource", "PrimaryState", "SecondaryState", "UnknownState", "PicType")
...
TextualConvention, MacAddress, DisplayString, DateAndTime, RowStatus, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "DisplayString", "DateAndTime", "RowStatus", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditLogThrottle=auditLogThrottle, cardTestTransId=cardTestTransId, cardTrapInh=cardTrapInh, diskVolumeDriveId=diskVolumeDriveId, cardTestTime=cardTestTime, cardDetected=cardDetected, cardTestScheduleCommand=cardTestScheduleCommand, auditPassedCount=auditPassedCount, cardPrStateChange=cardPrStateChange, portType=portType, portBgpId=portBgpId, portDplxStatusChange=portDplxStatusChange, portNumChans=portNumChans, diskVolumeFileName=diskVolumeFileName, cmResetClearNotification=cmResetClearNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, cardHwVersion=cardHwVersion, cardTestResultNotification=cardTestResultNotification, systemClock=systemClock, cardFwUpdateStatus=cardFwUpdateStatus, cardBusSpeed=cardBusSpeed, TestResult=TestResult, cardSerialNum=cardSerialNum, cardTestDescription=cardTestDescription, TestScheduleCommand=TestScheduleCommand, fpgaErrorEventGroup=fpgaErrorEventGroup, auditTable=auditTable, errEvRecoveryEnabled=errEvRecoveryEnabled, auditEntry=auditEntry, cardCarrierFwVersion=cardCarrierFwVersion, cardEntry=cardEntry, cardCarrierHwVersion=cardCarrierHwVersion, cardDplxStatusChange=cardDplxStatusChange, portTable=portTable, systemKey=systemKey, cardTemperatureHighError=cardTemperatureHighError, equipmentCardTestGroup=equipmentCardTestGroup, verboseLevel=verboseLevel, diskDriveNumber=diskDriveNumber, downstreamPowerLoss=downstreamPowerLoss, equipmentDiag=equipmentDiag, portDocsIfIndex=portDocsIfIndex, shelfName=shelfName, portCardId=portCardId, portSecState=portSecState, cardTemperature=cardTemperature, auditStatus=auditStatus, cardNumber=cardNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, portTrapInh=portTrapInh, cadEquipmentMib=cadEquipmentMib, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentError=equipmentError, cmResetUpTime=cmResetUpTime, cardTestResult=cardTestResult, cardShelfId=cardShelfId, portCurrDsPower=portCurrDsPower, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSubType=cardSubType, cardTestResultDesc=cardTestResultDesc, auditLogOutput=auditLogOutput, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMibConformance=cadEquipmentMibConformance, auditTotalPassedCount=auditTotalPassedCount, portLastChangeTime=portLastChangeTime, portId=portId, cardShelfTestIndex=cardShelfTestIndex, cardType=cardType, cmIpAddress=cmIpAddress, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDetectedMode=portDetectedMode, portEntry=portEntry, equipmentShelfGroup=equipmentShelfGroup, cardRamSize=cardRamSize, auditCardId=auditCardId, trapSeverity=trapSeverity, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portMode=portMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditFailedCount=auditFailedCount, diskVolumeGroup=diskVolumeGroup, equipmentState=equipmentState, cardNumPorts=cardNumPorts, cmUChannelID=cmUChannelID, cardBootVersion=cardBootVersion, equipmentStateGroup=equipmentStateGroup, equipmentPortGroup=equipmentPortGroup, cmPrimarySID=cmPrimarySID, cardSpareGroupId=cardSpareGroupId, diskVolumeTable=diskVolumeTable, shelfObjects=shelfObjects, cardTestName=cardTestName, portPrState=portPrState, portAdminState=portAdminState, diskVolumeId=diskVolumeId, diskVolumeName=diskVolumeName, cardTable=cardTable, portDescription=portDescription, eqDiagConfig=eqDiagConfig, portTxFlowControlMode=portTxFlowControlMode, auditDescription=auditDescription, diskVolumeSize=diskVolumeSize, cmResetReason=cmResetReason, portSecStateChange=portSecStateChange, cardTestEntry=cardTestEntry, cardSecStateChange=cardSecStateChange, portMaxDsPower=portMaxDsPower, diskVolumeEntry=diskVolumeEntry, systemGeneral=systemGeneral, cardDetectedChange=cardDetectedChange, cardPrState=cardPrState, shelfNumber=shelfNumber, cardName=cardName, auditTime=auditTime, cardLastBootSource=cardLastBootSource, portDplxStatus=portDplxStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestTransactionId=TestTransactionId, auditId=auditId, portMinDsPower=portMinDsPower, equipmentAudit=equipmentAudit, cardSubDetected=cardSubDetected, cardPicHwVersion=cardPicHwVersion, cardLastBootVersion=cardLastBootVersion, consoleOutput=consoleOutput, trapCounter=trapCounter, TestSchedule=TestSchedule, cardSecState=cardSecState, portRxFlowControlDetected=portRxFlowControlDetected, cmResetInfo=cmResetInfo, portConnectorId=portConnectorId, cmMacAddress=cmMacAddress, TestId=TestId, equipmentTraps=equipmentTraps, portMacAddress=portMacAddress, auditCommand=auditCommand, errEvLogLevel=errEvLogLevel, cardCpuSpeed=cardCpuSpeed, cardCarrierSerialNum=cardCarrierSerialNum, cardPicDetected=cardPicDetected, portMacIfIndex=portMacIfIndex, cardTempNoReportNotification=cardTempNoReportNotification, portNumber=portNumber, cardTestSchedule=cardTestSchedule, cmVendor=cmVendor, cardTestId=cardTestId, diskVolumeDiskSize=diskVolumeDiskSize, cardFwVersion=cardFwVersion, cardSwVersion=cardSwVersion, portRxFlowControlMode=portRxFlowControlMode, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeShelfId=diskVolumeShelfId, equipmentAuditGroup=equipmentAuditGroup, diskVolumeCardId=diskVolumeCardId, cardTemperatureHighWarn=cardTemperatureHighWarn, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCPLDVersion=cardCPLDVersion, auditName=auditName, auditTotalFailedCount=auditTotalFailedCount, portTxFlowControlDetected=portTxFlowControlDetected, cardFlashSize=cardFlashSize, cardAction=cardAction, diskVolumeTrapInh=diskVolumeTrapInh, cardTestIndex=cardTestIndex, errEvLoggingEnabled=errEvLoggingEnabled, cardDplxStatus=cardDplxStatus, auditResult=auditResult, cardTestType=cardTestType, cardLastResetReason=cardLastResetReason, auditAutoScheduling=auditAutoScheduling, cardUpTime=cardUpTime, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portAction=portAction, systemGeneralGroup=systemGeneralGroup, equipmentCardGroup=equipmentCardGroup, auditCycleCount=auditCycleCount, cardFpgaSource=cardFpgaSource, shelfSwVersion=shelfSwVersion, TestType=TestType, cmResetNotification=cmResetNotification, cmResetStatus=cmResetStatus, cardLastChangeTime=cardLastChangeTime, auditShelfId=auditShelfId, diagTestId=diagTestId, TestCommand=TestCommand, equipmentDiagGroup=equipmentDiagGroup, cmRegistrationNotification=cmRegistrationNotification, cardSpareGroupMode=cardSpareGroupMode, remainInDiagMode=remainInDiagMode, portPrStateChange=portPrStateChange, equipmentTbl=equipmentTbl, cardPicSerialNum=cardPicSerialNum, cardAdminState=cardAdminState, diskVolumeNumber=diskVolumeNumber, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuType=cardCpuType, cmDevice=cmDevice, portCardSubType=portCardSubType, cardId=cardId, portShelfId=portShelfId, errEventId=errEventId, errEvRowStatus=errEvRowStatus, cardTestTable=cardTestTable, cardTestCommand=cardTestCommand)
