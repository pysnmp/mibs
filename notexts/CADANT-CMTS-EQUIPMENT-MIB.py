...
PortId, DiskVolumeUsageLevel, CardType, EqActionType, PortMode, PrimaryState, PortType, AdminState, PortDetectedMode, PicType, FlowControlMode, DuplexStatus, SecondaryState, ShelfId, CardId, UnknownState, CardSubType, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "PortId", "DiskVolumeUsageLevel", "CardType", "EqActionType", "PortMode", "PrimaryState", "PortType", "AdminState", "PortDetectedMode", "PicType", "FlowControlMode", "DuplexStatus", "SecondaryState", "ShelfId", "CardId", "UnknownState", "CardSubType", "FirmwareSource")
...
DateAndTime, RowStatus, TruthValue, DisplayString, MacAddress, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "RowStatus", "TruthValue", "DisplayString", "MacAddress", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCarrierSerialNum=cardCarrierSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, auditAutoScheduling=auditAutoScheduling, cardSerialNum=cardSerialNum, portConnectorId=portConnectorId, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecState=portSecState, equipmentAudit=equipmentAudit, portLastChangeTime=portLastChangeTime, portTxFlowControlMode=portTxFlowControlMode, diskVolumeShelfId=diskVolumeShelfId, portTrapInh=portTrapInh, cardFpgaSource=cardFpgaSource, portCurrDsPower=portCurrDsPower, portMinDsPower=portMinDsPower, fpgaErrorEventTable=fpgaErrorEventTable, errEvLoggingEnabled=errEvLoggingEnabled, cardTestScheduleCommand=cardTestScheduleCommand, cardBootVersion=cardBootVersion, auditCardId=auditCardId, equipmentDiag=equipmentDiag, cardShelfTestIndex=cardShelfTestIndex, portCardSubType=portCardSubType, diskVolumeFileName=diskVolumeFileName, portPrState=portPrState, cardBusSpeed=cardBusSpeed, auditDescription=auditDescription, cardFlashSize=cardFlashSize, cmMacAddress=cmMacAddress, equipmentTbl=equipmentTbl, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetInfo=cmResetInfo, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeEntry=diskVolumeEntry, cardTestCommand=cardTestCommand, cardAdminState=cardAdminState, portCardId=portCardId, shelfObjects=shelfObjects, portDescription=portDescription, cardDplxStatusChange=cardDplxStatusChange, diagTestId=diagTestId, cardLastBootVersion=cardLastBootVersion, portAdminState=portAdminState, cardTestResult=cardTestResult, cardPicSerialNum=cardPicSerialNum, cardLastResetReason=cardLastResetReason, portMode=portMode, portDetectedMode=portDetectedMode, downstreamPowerLoss=downstreamPowerLoss, shelfName=shelfName, cardFwVersion=cardFwVersion, diskVolumeTrapInh=diskVolumeTrapInh, cardTestIndex=cardTestIndex, cardUpTime=cardUpTime, auditTable=auditTable, auditCommand=auditCommand, portTable=portTable, TestResult=TestResult, auditTotalFailedCount=auditTotalFailedCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portMaxDsPower=portMaxDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditShelfId=auditShelfId, TestTransactionId=TestTransactionId, diskVolumeDriveId=diskVolumeDriveId, verboseLevel=verboseLevel, equipmentTraps=equipmentTraps, cardId=cardId, cardTestTransId=cardTestTransId, portType=portType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeNumber=diskVolumeNumber, portMacAddress=portMacAddress, equipmentPortGroup=equipmentPortGroup, portAction=portAction, auditPassedCount=auditPassedCount, portSecStateChange=portSecStateChange, portTxFlowControlDetected=portTxFlowControlDetected, cardSpareGroupMode=cardSpareGroupMode, auditTotalPassedCount=auditTotalPassedCount, fpgaErrorEventEntry=fpgaErrorEventEntry, cardShelfId=cardShelfId, cardSpareGroupId=cardSpareGroupId, cardTemperatureHighError=cardTemperatureHighError, trapSeverity=trapSeverity, auditTime=auditTime, portRxFlowControlMode=portRxFlowControlMode, cardTrapInh=cardTrapInh, cardTestName=cardTestName, shelfNumber=shelfNumber, portNumChans=portNumChans, auditEntry=auditEntry, auditId=auditId, equipmentError=equipmentError, errEvRowStatus=errEvRowStatus, cardDplxStatus=cardDplxStatus, cmUChannelID=cmUChannelID, auditName=auditName, auditStatus=auditStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, shelfSwVersion=shelfSwVersion, errEventId=errEventId, cardTestDescription=cardTestDescription, portId=portId, systemGeneralGroup=systemGeneralGroup, TestId=TestId, remainInDiagMode=remainInDiagMode, cardPrState=cardPrState, portDplxStatus=portDplxStatus, equipmentCardGroup=equipmentCardGroup, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeGroup=diskVolumeGroup, cardSecStateChange=cardSecStateChange, auditLogThrottle=auditLogThrottle, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuType=cardCpuType, cardCPLDVersion=cardCPLDVersion, portShelfId=portShelfId, cardTestSchedule=cardTestSchedule, diskDriveNumber=diskDriveNumber, portPrStateChange=portPrStateChange, cadEquipmentMib=cadEquipmentMib, cardTestTable=cardTestTable, errEvRecoveryEnabled=errEvRecoveryEnabled, cardNumber=cardNumber, eqDiagConfig=eqDiagConfig, cardSecState=cardSecState, cardNumPorts=cardNumPorts, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardAction=cardAction, diskVolumeId=diskVolumeId, auditResult=auditResult, cardSubDetected=cardSubDetected, cardName=cardName, trapCounter=trapCounter, portDplxStatusChange=portDplxStatusChange, systemClock=systemClock, equipmentState=equipmentState, cardTestType=cardTestType, cmResetClearNotification=cmResetClearNotification, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, cmIpAddress=cmIpAddress, cmVendor=cmVendor, cardLastChangeTime=cardLastChangeTime, cardHwVersion=cardHwVersion, consoleOutput=consoleOutput, cardEntry=cardEntry, auditLogOutput=auditLogOutput, cardSwVersion=cardSwVersion, cmResetNotification=cmResetNotification, diskVolumeTable=diskVolumeTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentShelfGroup=equipmentShelfGroup, diskVolumeName=diskVolumeName, cardDetectedChange=cardDetectedChange, portDocsIfIndex=portDocsIfIndex, cardTempNoReportNotification=cardTempNoReportNotification, equipmentAuditGroup=equipmentAuditGroup, systemKey=systemKey, cardSubType=cardSubType, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cmResetUpTime=cmResetUpTime, errEvLogLevel=errEvLogLevel, diskVolumeCardId=diskVolumeCardId, cardType=cardType, cardTemperature=cardTemperature, cardPrStateChange=cardPrStateChange, cardTestResultNotification=cardTestResultNotification, diskVolumeSize=diskVolumeSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeDiskSize=diskVolumeDiskSize, auditCycleCount=auditCycleCount, cardTemperatureHighWarn=cardTemperatureHighWarn, cardRamSize=cardRamSize, portBgpId=portBgpId, cmPrimarySID=cmPrimarySID, cmRegistrationNotification=cmRegistrationNotification, cardTestId=cardTestId, cardTestEntry=cardTestEntry, portEntry=portEntry, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmDevice=cmDevice, TestCommand=TestCommand, cardTable=cardTable, portMacIfIndex=portMacIfIndex, portNumber=portNumber, cardLastBootSource=cardLastBootSource, equipmentCardTestGroup=equipmentCardTestGroup, cardTestTime=cardTestTime, TestScheduleCommand=TestScheduleCommand, cardPicHwVersion=cardPicHwVersion, TestSchedule=TestSchedule, auditFailedCount=auditFailedCount, cmResetReason=cmResetReason, systemGeneral=systemGeneral, cardDetected=cardDetected, cardCarrierHwVersion=cardCarrierHwVersion, cardTestResultDesc=cardTestResultDesc, equipmentStateGroup=equipmentStateGroup, cardCpuSpeed=cardCpuSpeed, cardPicDetected=cardPicDetected, TestType=TestType, cmResetStatus=cmResetStatus)
