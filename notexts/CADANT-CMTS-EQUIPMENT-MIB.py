...
UnknownState, ShelfId, FlowControlMode, SecondaryState, PrimaryState, CardType, PortDetectedMode, EqActionType, PicType, CardId, PortId, DuplexStatus, PortMode, FirmwareSource, PortType, AdminState, CardSubType, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "ShelfId", "FlowControlMode", "SecondaryState", "PrimaryState", "CardType", "PortDetectedMode", "EqActionType", "PicType", "CardId", "PortId", "DuplexStatus", "PortMode", "FirmwareSource", "PortType", "AdminState", "CardSubType", "DiskVolumeUsageLevel")
...
TextualConvention, DisplayString, DateAndTime, RowStatus, MacAddress, TimeStamp, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "DateAndTime", "RowStatus", "MacAddress", "TimeStamp", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardHwVersion=cardHwVersion, auditLogThrottle=auditLogThrottle, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardAdminState=cardAdminState, portAction=portAction, portDplxStatusChange=portDplxStatusChange, cardType=cardType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portPrState=portPrState, portTrapInh=portTrapInh, shelfName=shelfName, portNumber=portNumber, cardPicDetected=cardPicDetected, diskVolumeSize=diskVolumeSize, cadEquipmentMib=cadEquipmentMib, cardDplxStatus=cardDplxStatus, portMacAddress=portMacAddress, auditShelfId=auditShelfId, cardSpareGroupId=cardSpareGroupId, TestCommand=TestCommand, systemKey=systemKey, equipmentTraps=equipmentTraps, cardSubDetected=cardSubDetected, cardShelfTestIndex=cardShelfTestIndex, cmRegistrationNotification=cmRegistrationNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestScheduleCommand=cardTestScheduleCommand, TestId=TestId, consoleOutput=consoleOutput, cmMacAddress=cmMacAddress, equipmentShelfGroup=equipmentShelfGroup, shelfNumber=shelfNumber, cardTestDescription=cardTestDescription, diskVolumeUsageNotification=diskVolumeUsageNotification, diskDriveNumber=diskDriveNumber, cardSubType=cardSubType, auditCardId=auditCardId, portId=portId, shelfSwVersion=shelfSwVersion, cardLastChangeTime=cardLastChangeTime, equipmentAuditGroup=equipmentAuditGroup, systemGeneral=systemGeneral, cardDetected=cardDetected, cardLastBootVersion=cardLastBootVersion, equipmentCardTestGroup=equipmentCardTestGroup, portBgpId=portBgpId, cardEntry=cardEntry, cardTemperatureHighError=cardTemperatureHighError, cmVendor=cmVendor, portMode=portMode, portMaxDsPower=portMaxDsPower, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestSchedule=cardTestSchedule, cmResetReason=cmResetReason, TestType=TestType, cardTestResult=cardTestResult, diskVolumeGroup=diskVolumeGroup, auditTime=auditTime, auditPassedCount=auditPassedCount, cardLastBootSource=cardLastBootSource, cardTempNoReportNotification=cardTempNoReportNotification, TestResult=TestResult, PYSNMP_MODULE_ID=cadEquipmentMib, cardDetectedChange=cardDetectedChange, cardTemperatureHighWarn=cardTemperatureHighWarn, portAdminState=portAdminState, equipmentDiag=equipmentDiag, shelfObjects=shelfObjects, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portSecStateChange=portSecStateChange, cmIpAddress=cmIpAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portCardSubType=portCardSubType, portCurrDsPower=portCurrDsPower, equipmentState=equipmentState, cardTable=cardTable, cardId=cardId, cardTemperature=cardTemperature, portRxFlowControlMode=portRxFlowControlMode, cardCarrierHwVersion=cardCarrierHwVersion, portCardId=portCardId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portEntry=portEntry, cardCPLDVersion=cardCPLDVersion, cardBusSpeed=cardBusSpeed, portTable=portTable, remainInDiagMode=remainInDiagMode, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeShelfId=diskVolumeShelfId, auditDescription=auditDescription, fpgaErrorEventTable=fpgaErrorEventTable, cardAction=cardAction, portMacIfIndex=portMacIfIndex, verboseLevel=verboseLevel, portDescription=portDescription, auditName=auditName, cardTrapInh=cardTrapInh, diskVolumeCardId=diskVolumeCardId, auditResult=auditResult, cmResetClearNotification=cmResetClearNotification, cardName=cardName, portRxFlowControlDetected=portRxFlowControlDetected, auditFailedCount=auditFailedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicSerialNum=cardPicSerialNum, cardUpTime=cardUpTime, cardRamSize=cardRamSize, cardPicHwVersion=cardPicHwVersion, auditAutoScheduling=auditAutoScheduling, auditLogOutput=auditLogOutput, trapCounter=trapCounter, TestTransactionId=TestTransactionId, cmUChannelID=cmUChannelID, cardPrStateChange=cardPrStateChange, cardTestTransId=cardTestTransId, diskVolumeDiskSize=diskVolumeDiskSize, cardTestType=cardTestType, portLastChangeTime=portLastChangeTime, diskVolumeTable=diskVolumeTable, equipmentAudit=equipmentAudit, cardSwVersion=cardSwVersion, diskVolumeTrapInh=diskVolumeTrapInh, systemClock=systemClock, cardTestTime=cardTestTime, diskVolumeName=diskVolumeName, auditCommand=auditCommand, errEvRowStatus=errEvRowStatus, eqDiagConfig=eqDiagConfig, cardPrState=cardPrState, diskVolumeId=diskVolumeId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardDplxStatusChange=cardDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentStateGroup=equipmentStateGroup, cardSpareGroupMode=cardSpareGroupMode, portTxFlowControlMode=portTxFlowControlMode, cardFwVersion=cardFwVersion, cardBootVersion=cardBootVersion, cardCarrierFwVersion=cardCarrierFwVersion, auditId=auditId, auditTotalFailedCount=auditTotalFailedCount, portShelfId=portShelfId, downstreamPowerLoss=downstreamPowerLoss, diskVolumeDriveId=diskVolumeDriveId, portPrStateChange=portPrStateChange, cardNumPorts=cardNumPorts, cardLastResetReason=cardLastResetReason, portMinDsPower=portMinDsPower, errEvLogLevel=errEvLogLevel, diskVolumeUsageLevel=diskVolumeUsageLevel, cardCpuSpeed=cardCpuSpeed, cardTestId=cardTestId, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestCommand=cardTestCommand, cardTestResultDesc=cardTestResultDesc, auditTotalPassedCount=auditTotalPassedCount, equipmentTbl=equipmentTbl, cardFlashSize=cardFlashSize, systemGeneralGroup=systemGeneralGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardSecStateChange=cardSecStateChange, portTxFlowControlDetected=portTxFlowControlDetected, cardTestTable=cardTestTable, cardCarrierSerialNum=cardCarrierSerialNum, portDocsIfIndex=portDocsIfIndex, cardTestIndex=cardTestIndex, cardTestName=cardTestName, diskVolumeFileName=diskVolumeFileName, cardFpgaSource=cardFpgaSource, diskVolumeEntry=diskVolumeEntry, auditTable=auditTable, auditStatus=auditStatus, cardTestResultNotification=cardTestResultNotification, portConnectorId=portConnectorId, cardSecState=cardSecState, equipmentPortGroup=equipmentPortGroup, cardTestEntry=cardTestEntry, auditEntry=auditEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetStatus=cmResetStatus, cardCpuType=cardCpuType, cmResetUpTime=cmResetUpTime, errEvRecoveryEnabled=errEvRecoveryEnabled, trapSeverity=trapSeverity, cmDevice=cmDevice, cmResetInfo=cmResetInfo, equipmentError=equipmentError, diskVolumeNumber=diskVolumeNumber, cmPrimarySID=cmPrimarySID, portSecState=portSecState, equipmentCardGroup=equipmentCardGroup, cardShelfId=cardShelfId, TestSchedule=TestSchedule, cardNumber=cardNumber, cardSerialNum=cardSerialNum, portType=portType, portDplxStatus=portDplxStatus, auditCycleCount=auditCycleCount, errEventId=errEventId, portNumChans=portNumChans, cmResetNotification=cmResetNotification, TestScheduleCommand=TestScheduleCommand, diagTestId=diagTestId, portDetectedMode=portDetectedMode)
