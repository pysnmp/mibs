...
PicType, PortDetectedMode, PortType, PortId, PortMode, ShelfId, FlowControlMode, CardId, FirmwareSource, AdminState, DuplexStatus, EqActionType, PrimaryState, UnknownState, CardSubType, DiskVolumeUsageLevel, CardType, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "PicType", "PortDetectedMode", "PortType", "PortId", "PortMode", "ShelfId", "FlowControlMode", "CardId", "FirmwareSource", "AdminState", "DuplexStatus", "EqActionType", "PrimaryState", "UnknownState", "CardSubType", "DiskVolumeUsageLevel", "CardType", "SecondaryState")
...
DateAndTime, DisplayString, TextualConvention, MacAddress, TruthValue, TimeStamp, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "DisplayString", "TextualConvention", "MacAddress", "TruthValue", "TimeStamp", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", downstreamPowerLoss=downstreamPowerLoss, cmPrimarySID=cmPrimarySID, cardShelfId=cardShelfId, cardPicDetected=cardPicDetected, diagTestId=diagTestId, cardSubDetected=cardSubDetected, TestResult=TestResult, errEvLogLevel=errEvLogLevel, portPrStateChange=portPrStateChange, portSecStateChange=portSecStateChange, errEvRowStatus=errEvRowStatus, cardDplxStatusChange=cardDplxStatusChange, cardSecState=cardSecState, TestCommand=TestCommand, diskDriveNumber=diskDriveNumber, equipmentCardTestGroup=equipmentCardTestGroup, portMaxDsPower=portMaxDsPower, cardName=cardName, TestId=TestId, portPrState=portPrState, cardBusSpeed=cardBusSpeed, diskVolumeSize=diskVolumeSize, auditStatus=auditStatus, portMode=portMode, cmRegistrationNotification=cmRegistrationNotification, cardSpareGroupId=cardSpareGroupId, cardTestDescription=cardTestDescription, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardFwVersion=cardFwVersion, cardTestResult=cardTestResult, cardPicSerialNum=cardPicSerialNum, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portTxFlowControlDetected=portTxFlowControlDetected, cmIpAddress=cmIpAddress, cardTestTime=cardTestTime, auditId=auditId, shelfNumber=shelfNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCarrierFwVersion=cardCarrierFwVersion, cardSubType=cardSubType, portId=portId, auditDescription=auditDescription, cardTrapInh=cardTrapInh, cardTestResultNotification=cardTestResultNotification, cardDetectedChange=cardDetectedChange, cardTestIndex=cardTestIndex, cardCpuType=cardCpuType, trapSeverity=trapSeverity, TestSchedule=TestSchedule, cardFwUpdateStatus=cardFwUpdateStatus, remainInDiagMode=remainInDiagMode, cardRamSize=cardRamSize, portLastChangeTime=portLastChangeTime, cardPicHwVersion=cardPicHwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, systemClock=systemClock, portConnectorId=portConnectorId, portMinDsPower=portMinDsPower, verboseLevel=verboseLevel, errEventId=errEventId, portAction=portAction, equipmentDiag=equipmentDiag, cardTestName=cardTestName, portMacIfIndex=portMacIfIndex, portCardId=portCardId, portEntry=portEntry, cardDetected=cardDetected, auditCardId=auditCardId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardId=cardId, portDescription=portDescription, cardTestId=cardTestId, cardHwVersion=cardHwVersion, TestScheduleCommand=TestScheduleCommand, cardAction=cardAction, auditCommand=auditCommand, portTxFlowControlMode=portTxFlowControlMode, cardDplxStatus=cardDplxStatus, equipmentAuditGroup=equipmentAuditGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastBootVersion=cardLastBootVersion, auditEntry=auditEntry, cardPrStateChange=cardPrStateChange, portDplxStatusChange=portDplxStatusChange, cardLastBootSource=cardLastBootSource, cmResetNotification=cmResetNotification, diskVolumeTable=diskVolumeTable, cardTemperature=cardTemperature, auditLogThrottle=auditLogThrottle, portSecState=portSecState, diskVolumeEntry=diskVolumeEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentShelfGroup=equipmentShelfGroup, cardCarrierHwVersion=cardCarrierHwVersion, cmResetClearNotification=cmResetClearNotification, auditResult=auditResult, cardTestResultDesc=cardTestResultDesc, cardTestScheduleCommand=cardTestScheduleCommand, equipmentCardGroup=equipmentCardGroup, portDetectedMode=portDetectedMode, auditLogOutput=auditLogOutput, cardBootVersion=cardBootVersion, diskVolumeNumber=diskVolumeNumber, auditShelfId=auditShelfId, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeName=diskVolumeName, portShelfId=portShelfId, cmVendor=cmVendor, portMacAddress=portMacAddress, cardTestTransId=cardTestTransId, equipmentPortGroup=equipmentPortGroup, auditTime=auditTime, cadEquipmentMibGroup=cadEquipmentMibGroup, cmMacAddress=cmMacAddress, cardEntry=cardEntry, cardLastResetReason=cardLastResetReason, equipmentDiagGroup=equipmentDiagGroup, systemGeneralGroup=systemGeneralGroup, cardFpgaSource=cardFpgaSource, cardCpuSpeed=cardCpuSpeed, portNumber=portNumber, cmResetStatus=cmResetStatus, cardTestType=cardTestType, cmResetReason=cmResetReason, cardTestTable=cardTestTable, portType=portType, auditFailedCount=auditFailedCount, equipmentTbl=equipmentTbl, diskVolumeGroup=diskVolumeGroup, cardSerialNum=cardSerialNum, equipmentAudit=equipmentAudit, cardNumPorts=cardNumPorts, portNumChans=portNumChans, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, eqDiagConfig=eqDiagConfig, auditCycleCount=auditCycleCount, portCurrDsPower=portCurrDsPower, TestTransactionId=TestTransactionId, auditTotalPassedCount=auditTotalPassedCount, cardLastChangeTime=cardLastChangeTime, cardShelfTestIndex=cardShelfTestIndex, cardSpareGroupMode=cardSpareGroupMode, cardTestCommand=cardTestCommand, diskVolumeFileName=diskVolumeFileName, cardTable=cardTable, portDocsIfIndex=portDocsIfIndex, cardPrState=cardPrState, equipmentError=equipmentError, systemGeneral=systemGeneral, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDplxStatus=portDplxStatus, auditName=auditName, cmDevice=cmDevice, cmResetInfo=cmResetInfo, equipmentTraps=equipmentTraps, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSecStateChange=cardSecStateChange, cardTestEntry=cardTestEntry, auditAutoScheduling=auditAutoScheduling, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeShelfId=diskVolumeShelfId, equipmentStateGroup=equipmentStateGroup, cardCarrierSerialNum=cardCarrierSerialNum, shelfSwVersion=shelfSwVersion, cardUpTime=cardUpTime, shelfObjects=shelfObjects, portAdminState=portAdminState, diskVolumeId=diskVolumeId, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsagePercentage=diskVolumeUsagePercentage, errEvLoggingEnabled=errEvLoggingEnabled, fpgaErrorEventEntry=fpgaErrorEventEntry, portBgpId=portBgpId, cmResetUpTime=cmResetUpTime, diskVolumeCardId=diskVolumeCardId, systemKey=systemKey, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentState=equipmentState, auditTable=auditTable, cardCPLDVersion=cardCPLDVersion, portTable=portTable, portCardSubType=portCardSubType, auditPassedCount=auditPassedCount, portRxFlowControlDetected=portRxFlowControlDetected, cardNumber=cardNumber, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardType=cardType, TestType=TestType, consoleOutput=consoleOutput, cardFlashSize=cardFlashSize, shelfName=shelfName, cardSwVersion=cardSwVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTemperatureHighError=cardTemperatureHighError, auditTotalFailedCount=auditTotalFailedCount, trapCounter=trapCounter, cmUChannelID=cmUChannelID, cardTestSchedule=cardTestSchedule, cardAdminState=cardAdminState, errEvRecoveryEnabled=errEvRecoveryEnabled, portTrapInh=portTrapInh)
