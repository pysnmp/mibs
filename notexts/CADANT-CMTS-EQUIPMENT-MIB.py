...
DuplexStatus, PortMode, PortType, PicType, AdminState, FlowControlMode, CardType, ShelfId, PortDetectedMode, SecondaryState, UnknownState, DiskVolumeUsageLevel, PortId, CardId, EqActionType, CardSubType, PrimaryState, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortMode", "PortType", "PicType", "AdminState", "FlowControlMode", "CardType", "ShelfId", "PortDetectedMode", "SecondaryState", "UnknownState", "DiskVolumeUsageLevel", "PortId", "CardId", "EqActionType", "CardSubType", "PrimaryState", "FirmwareSource")
...
TextualConvention, MacAddress, RowStatus, DateAndTime, TimeStamp, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "RowStatus", "DateAndTime", "TimeStamp", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardType=cardType, cardTestId=cardTestId, auditTime=auditTime, cmDevice=cmDevice, cardTestTransId=cardTestTransId, cadEquipmentMib=cadEquipmentMib, shelfNumber=shelfNumber, diskVolumeShelfId=diskVolumeShelfId, portDplxStatusChange=portDplxStatusChange, equipmentError=equipmentError, errEvLogLevel=errEvLogLevel, cardTable=cardTable, cardCpuType=cardCpuType, cmIpAddress=cmIpAddress, cardSwVersion=cardSwVersion, cardTempNoReportNotification=cardTempNoReportNotification, auditAutoScheduling=auditAutoScheduling, cardNumPorts=cardNumPorts, portDetectedMode=portDetectedMode, portMacIfIndex=portMacIfIndex, cmResetUpTime=cmResetUpTime, diskVolumeEntry=diskVolumeEntry, cardHwVersion=cardHwVersion, errEvRowStatus=errEvRowStatus, cardLastBootSource=cardLastBootSource, portTable=portTable, cardSpareGroupMode=cardSpareGroupMode, diskVolumeTable=diskVolumeTable, cmVendor=cmVendor, diskVolumeGroup=diskVolumeGroup, trapSeverity=trapSeverity, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTrapInh=cardTrapInh, cmResetStatus=cmResetStatus, cardTestScheduleCommand=cardTestScheduleCommand, verboseLevel=verboseLevel, cardUpTime=cardUpTime, equipmentTbl=equipmentTbl, equipmentCardTestGroup=equipmentCardTestGroup, cardTestCommand=cardTestCommand, diskVolumeCardId=diskVolumeCardId, TestId=TestId, portSecStateChange=portSecStateChange, auditCardId=auditCardId, trapCounter=trapCounter, portAction=portAction, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicSerialNum=cardPicSerialNum, auditLogThrottle=auditLogThrottle, equipmentShelfGroup=equipmentShelfGroup, diskDriveNumber=diskDriveNumber, downstreamPowerLoss=downstreamPowerLoss, auditShelfId=auditShelfId, cmRegistrationNotification=cmRegistrationNotification, systemGeneralGroup=systemGeneralGroup, cardLastBootVersion=cardLastBootVersion, portMinDsPower=portMinDsPower, shelfObjects=shelfObjects, cardSubDetected=cardSubDetected, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portTxFlowControlDetected=portTxFlowControlDetected, cardFwUpdateStatus=cardFwUpdateStatus, auditName=auditName, cadEquipmentMibGroup=cadEquipmentMibGroup, portCardId=portCardId, cardNumber=cardNumber, diskVolumeSize=diskVolumeSize, cardTestResultNotification=cardTestResultNotification, cmResetNotification=cmResetNotification, cardPicHwVersion=cardPicHwVersion, cardLastChangeTime=cardLastChangeTime, equipmentTraps=equipmentTraps, cmMacAddress=cmMacAddress, diskVolumeNumber=diskVolumeNumber, cardTestName=cardTestName, cardName=cardName, portMode=portMode, cardCarrierSerialNum=cardCarrierSerialNum, cardDplxStatusChange=cardDplxStatusChange, equipmentStateGroup=equipmentStateGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCarrierFwVersion=cardCarrierFwVersion, cardTestEntry=cardTestEntry, cardSerialNum=cardSerialNum, diskVolumeName=diskVolumeName, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, fpgaErrorEventTable=fpgaErrorEventTable, cardSecState=cardSecState, portPrStateChange=portPrStateChange, systemKey=systemKey, cardEntry=cardEntry, cardPrState=cardPrState, portRxFlowControlDetected=portRxFlowControlDetected, equipmentDiagGroup=equipmentDiagGroup, portDplxStatus=portDplxStatus, cardFlashSize=cardFlashSize, cardTestDescription=cardTestDescription, TestSchedule=TestSchedule, diskVolumeDriveId=diskVolumeDriveId, diagTestId=diagTestId, portType=portType, auditId=auditId, cardSpareGroupId=cardSpareGroupId, cardBootVersion=cardBootVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCarrierHwVersion=cardCarrierHwVersion, cardAction=cardAction, auditPassedCount=auditPassedCount, cardTestType=cardTestType, auditTotalPassedCount=auditTotalPassedCount, portId=portId, cardBusSpeed=cardBusSpeed, cardPrStateChange=cardPrStateChange, cardCPLDVersion=cardCPLDVersion, portAdminState=portAdminState, cardTemperature=cardTemperature, portBgpId=portBgpId, portNumChans=portNumChans, diskVolumeId=diskVolumeId, cardCpuSpeed=cardCpuSpeed, cardDplxStatus=cardDplxStatus, cardDetected=cardDetected, portMaxDsPower=portMaxDsPower, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentState=equipmentState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmPrimarySID=cmPrimarySID, cardTestResult=cardTestResult, portDescription=portDescription, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeFileName=diskVolumeFileName, equipmentDiag=equipmentDiag, portShelfId=portShelfId, TestTransactionId=TestTransactionId, cmUChannelID=cmUChannelID, auditLogOutput=auditLogOutput, auditCycleCount=auditCycleCount, TestType=TestType, cmResetReason=cmResetReason, portTxFlowControlMode=portTxFlowControlMode, errEvRecoveryEnabled=errEvRecoveryEnabled, systemClock=systemClock, equipmentPortGroup=equipmentPortGroup, equipmentAudit=equipmentAudit, portLastChangeTime=portLastChangeTime, shelfName=shelfName, errEventId=errEventId, eqDiagConfig=eqDiagConfig, cmResetInfo=cmResetInfo, cardSecStateChange=cardSecStateChange, portCardSubType=portCardSubType, portEntry=portEntry, portCurrDsPower=portCurrDsPower, auditTable=auditTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, consoleOutput=consoleOutput, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, remainInDiagMode=remainInDiagMode, diskVolumeUsageLevel=diskVolumeUsageLevel, cardLastResetReason=cardLastResetReason, auditEntry=auditEntry, auditCommand=auditCommand, auditTotalFailedCount=auditTotalFailedCount, cardRamSize=cardRamSize, portSecState=portSecState, cardTestResultDesc=cardTestResultDesc, cmResetClearNotification=cmResetClearNotification, auditDescription=auditDescription, equipmentCardGroup=equipmentCardGroup, systemGeneral=systemGeneral, PYSNMP_MODULE_ID=cadEquipmentMib, cardShelfId=cardShelfId, TestScheduleCommand=TestScheduleCommand, auditStatus=auditStatus, cardTemperatureHighError=cardTemperatureHighError, TestResult=TestResult, cardFpgaSource=cardFpgaSource, portMacAddress=portMacAddress, cardFwVersion=cardFwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestSchedule=cardTestSchedule, cardPicDetected=cardPicDetected, cardTestTime=cardTestTime, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeDiskSize=diskVolumeDiskSize, cardTestIndex=cardTestIndex, diskVolumeUsageNotification=diskVolumeUsageNotification, cardDetectedChange=cardDetectedChange, auditFailedCount=auditFailedCount, auditResult=auditResult, portNumber=portNumber, portPrState=portPrState, portDocsIfIndex=portDocsIfIndex, cardShelfTestIndex=cardShelfTestIndex, cardAdminState=cardAdminState, shelfSwVersion=shelfSwVersion, cardId=cardId, cardTestTable=cardTestTable, TestCommand=TestCommand, diskVolumeTrapInh=diskVolumeTrapInh, portConnectorId=portConnectorId, portTrapInh=portTrapInh, cardSubType=cardSubType, fpgaErrorEventGroup=fpgaErrorEventGroup)
