...
DuplexStatus, PortDetectedMode, ShelfId, UnknownState, PicType, CardType, AdminState, FirmwareSource, CardSubType, PortId, PortMode, EqActionType, PortType, CardId, PrimaryState, FlowControlMode, SecondaryState, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortDetectedMode", "ShelfId", "UnknownState", "PicType", "CardType", "AdminState", "FirmwareSource", "CardSubType", "PortId", "PortMode", "EqActionType", "PortType", "CardId", "PrimaryState", "FlowControlMode", "SecondaryState", "DiskVolumeUsageLevel")
...
DateAndTime, TextualConvention, RowStatus, DisplayString, TimeStamp, TruthValue, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TextualConvention", "RowStatus", "DisplayString", "TimeStamp", "TruthValue", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageLevel=diskVolumeUsageLevel, portLastChangeTime=portLastChangeTime, auditResult=auditResult, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardShelfTestIndex=cardShelfTestIndex, cardDetected=cardDetected, diskVolumeDriveId=diskVolumeDriveId, errEvRowStatus=errEvRowStatus, cardNumber=cardNumber, cardType=cardType, cardSecState=cardSecState, PYSNMP_MODULE_ID=cadEquipmentMib, auditLogOutput=auditLogOutput, shelfSwVersion=shelfSwVersion, portBgpId=portBgpId, equipmentShelfGroup=equipmentShelfGroup, errEventId=errEventId, portDplxStatus=portDplxStatus, diskVolumeId=diskVolumeId, portTable=portTable, cardCarrierSerialNum=cardCarrierSerialNum, cardPicHwVersion=cardPicHwVersion, equipmentStateGroup=equipmentStateGroup, cardBootVersion=cardBootVersion, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResultDesc=cardTestResultDesc, portTrapInh=portTrapInh, diskVolumeDiskSize=diskVolumeDiskSize, portDplxStatusChange=portDplxStatusChange, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeTrapInh=diskVolumeTrapInh, auditTotalPassedCount=auditTotalPassedCount, systemGeneralGroup=systemGeneralGroup, shelfObjects=shelfObjects, cardName=cardName, auditId=auditId, cardFpgaSource=cardFpgaSource, auditDescription=auditDescription, cardAdminState=cardAdminState, auditTime=auditTime, cardSwVersion=cardSwVersion, portRxFlowControlMode=portRxFlowControlMode, errEvLogLevel=errEvLogLevel, cmResetNotification=cmResetNotification, cardFlashSize=cardFlashSize, systemClock=systemClock, cardSpareGroupMode=cardSpareGroupMode, cardBusSpeed=cardBusSpeed, cardTemperature=cardTemperature, cardTestResult=cardTestResult, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditPassedCount=auditPassedCount, cardTestIndex=cardTestIndex, portSecState=portSecState, cardNumPorts=cardNumPorts, portId=portId, diskVolumeGroup=diskVolumeGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, portAdminState=portAdminState, cardTestScheduleCommand=cardTestScheduleCommand, diskDriveNumber=diskDriveNumber, cmRegistrationNotification=cmRegistrationNotification, cardHwVersion=cardHwVersion, equipmentCardGroup=equipmentCardGroup, shelfName=shelfName, consoleOutput=consoleOutput, cardTemperatureHighError=cardTemperatureHighError, diskVolumeShelfId=diskVolumeShelfId, portType=portType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditTotalFailedCount=auditTotalFailedCount, auditName=auditName, cardFwUpdateStatus=cardFwUpdateStatus, shelfNumber=shelfNumber, cardTestType=cardTestType, cardTestId=cardTestId, trapSeverity=trapSeverity, auditFailedCount=auditFailedCount, cmResetClearNotification=cmResetClearNotification, cardTestEntry=cardTestEntry, auditCommand=auditCommand, diskVolumeUsageNotification=diskVolumeUsageNotification, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeTable=diskVolumeTable, cmUChannelID=cmUChannelID, TestId=TestId, cardPicDetected=cardPicDetected, equipmentDiag=equipmentDiag, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portDetectedMode=portDetectedMode, cardTestTime=cardTestTime, cardLastBootVersion=cardLastBootVersion, portDocsIfIndex=portDocsIfIndex, auditAutoScheduling=auditAutoScheduling, cardDplxStatus=cardDplxStatus, portTxFlowControlMode=portTxFlowControlMode, cmVendor=cmVendor, equipmentTbl=equipmentTbl, TestSchedule=TestSchedule, cardCpuSpeed=cardCpuSpeed, auditEntry=auditEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, portMode=portMode, portEntry=portEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetReason=cmResetReason, portConnectorId=portConnectorId, cmMacAddress=cmMacAddress, auditCycleCount=auditCycleCount, downstreamPowerLoss=downstreamPowerLoss, cardLastResetReason=cardLastResetReason, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentError=equipmentError, cardPicSerialNum=cardPicSerialNum, cardRamSize=cardRamSize, cardPrState=cardPrState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, systemKey=systemKey, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeCardId=diskVolumeCardId, cardCarrierFwVersion=cardCarrierFwVersion, cardSubType=cardSubType, cmResetStatus=cmResetStatus, cardTempNoReportNotification=cardTempNoReportNotification, TestTransactionId=TestTransactionId, diskVolumeNumber=diskVolumeNumber, equipmentPortGroup=equipmentPortGroup, cardUpTime=cardUpTime, diagTestId=diagTestId, portPrState=portPrState, portCardSubType=portCardSubType, portNumChans=portNumChans, equipmentAuditGroup=equipmentAuditGroup, cmDevice=cmDevice, cardTable=cardTable, TestCommand=TestCommand, cardTestTransId=cardTestTransId, cardSpareGroupId=cardSpareGroupId, portSecStateChange=portSecStateChange, diskVolumeEntry=diskVolumeEntry, TestResult=TestResult, cardCpuType=cardCpuType, equipmentDiagGroup=equipmentDiagGroup, cardDplxStatusChange=cardDplxStatusChange, portCardId=portCardId, cardTestName=cardTestName, cardTestTable=cardTestTable, trapCounter=trapCounter, cadEquipmentMib=cadEquipmentMib, auditStatus=auditStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portPrStateChange=portPrStateChange, cardPrStateChange=cardPrStateChange, cardTrapInh=cardTrapInh, cardTestSchedule=cardTestSchedule, cardLastBootSource=cardLastBootSource, cardCPLDVersion=cardCPLDVersion, cardCarrierHwVersion=cardCarrierHwVersion, cardTestCommand=cardTestCommand, cadEquipmentMibConformance=cadEquipmentMibConformance, TestScheduleCommand=TestScheduleCommand, diskVolumeSize=diskVolumeSize, cmIpAddress=cmIpAddress, cmResetInfo=cmResetInfo, cardDetectedChange=cardDetectedChange, portMaxDsPower=portMaxDsPower, cmResetUpTime=cmResetUpTime, fpgaErrorEventGroup=fpgaErrorEventGroup, cardShelfId=cardShelfId, cardTestDescription=cardTestDescription, portMinDsPower=portMinDsPower, TestType=TestType, equipmentAudit=equipmentAudit, cardSubDetected=cardSubDetected, diskVolumeName=diskVolumeName, portDescription=portDescription, cardId=cardId, cardEntry=cardEntry, cardAction=cardAction, auditCardId=auditCardId, errEvLoggingEnabled=errEvLoggingEnabled, equipmentTraps=equipmentTraps, auditLogThrottle=auditLogThrottle, verboseLevel=verboseLevel, auditShelfId=auditShelfId, equipmentState=equipmentState, portMacIfIndex=portMacIfIndex, cmPrimarySID=cmPrimarySID, cardSerialNum=cardSerialNum, portShelfId=portShelfId, remainInDiagMode=remainInDiagMode, portMacAddress=portMacAddress, portAction=portAction, cardSecStateChange=cardSecStateChange, portNumber=portNumber, systemGeneral=systemGeneral, cardTestResultNotification=cardTestResultNotification, cardFwVersion=cardFwVersion, portCurrDsPower=portCurrDsPower, cardLastChangeTime=cardLastChangeTime, cardTemperatureHighWarn=cardTemperatureHighWarn, eqDiagConfig=eqDiagConfig, diskVolumeFileName=diskVolumeFileName, auditTable=auditTable, errEvRecoveryEnabled=errEvRecoveryEnabled)
