...
PicType, DiskVolumeUsageLevel, CardId, FirmwareSource, CardType, PortDetectedMode, PrimaryState, CardSubType, FlowControlMode, PortMode, AdminState, UnknownState, ShelfId, DuplexStatus, PortId, EqActionType, SecondaryState, PortType = mibBuilder.importSymbols("CADANT-TC", "PicType", "DiskVolumeUsageLevel", "CardId", "FirmwareSource", "CardType", "PortDetectedMode", "PrimaryState", "CardSubType", "FlowControlMode", "PortMode", "AdminState", "UnknownState", "ShelfId", "DuplexStatus", "PortId", "EqActionType", "SecondaryState", "PortType")
...
DisplayString, MacAddress, TimeStamp, DateAndTime, RowStatus, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "TimeStamp", "DateAndTime", "RowStatus", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardType=cardType, cardFwUpdateStatus=cardFwUpdateStatus, trapCounter=trapCounter, portCardSubType=portCardSubType, shelfName=shelfName, cmPrimarySID=cmPrimarySID, portPrStateChange=portPrStateChange, cardCarrierFwVersion=cardCarrierFwVersion, portTxFlowControlDetected=portTxFlowControlDetected, systemGeneralGroup=systemGeneralGroup, equipmentStateGroup=equipmentStateGroup, portShelfId=portShelfId, portRxFlowControlMode=portRxFlowControlMode, portType=portType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portTable=portTable, TestId=TestId, cardTestScheduleCommand=cardTestScheduleCommand, portMode=portMode, cardTrapInh=cardTrapInh, cardDetected=cardDetected, TestScheduleCommand=TestScheduleCommand, auditTable=auditTable, portSecStateChange=portSecStateChange, portDocsIfIndex=portDocsIfIndex, auditPassedCount=auditPassedCount, cardTestDescription=cardTestDescription, auditDescription=auditDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmVendor=cmVendor, cadEquipmentMibCompliance=cadEquipmentMibCompliance, fpgaErrorEventGroup=fpgaErrorEventGroup, portMaxDsPower=portMaxDsPower, auditLogThrottle=auditLogThrottle, diskVolumeSize=diskVolumeSize, equipmentShelfGroup=equipmentShelfGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, portTrapInh=portTrapInh, cardShelfId=cardShelfId, cardBootVersion=cardBootVersion, cmResetReason=cmResetReason, auditCardId=auditCardId, cmDevice=cmDevice, cmRegistrationNotification=cmRegistrationNotification, cardCPLDVersion=cardCPLDVersion, equipmentCardTestGroup=equipmentCardTestGroup, portNumber=portNumber, diskVolumeFileName=diskVolumeFileName, diskVolumeDiskSize=diskVolumeDiskSize, cardRamSize=cardRamSize, equipmentTbl=equipmentTbl, cardLastResetReason=cardLastResetReason, cardTestCommand=cardTestCommand, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentDiagGroup=equipmentDiagGroup, portMacAddress=portMacAddress, portPrState=portPrState, portNumChans=portNumChans, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeEntry=diskVolumeEntry, portMinDsPower=portMinDsPower, auditShelfId=auditShelfId, portId=portId, portDplxStatus=portDplxStatus, cmMacAddress=cmMacAddress, equipmentAudit=equipmentAudit, cardTestResult=cardTestResult, diskVolumeTrapInh=diskVolumeTrapInh, portLastChangeTime=portLastChangeTime, shelfNumber=shelfNumber, equipmentTraps=equipmentTraps, auditId=auditId, portDescription=portDescription, portCurrDsPower=portCurrDsPower, TestSchedule=TestSchedule, eqDiagConfig=eqDiagConfig, cmUChannelID=cmUChannelID, portEntry=portEntry, cardFwVersion=cardFwVersion, cardLastBootVersion=cardLastBootVersion, equipmentDiag=equipmentDiag, TestCommand=TestCommand, cardSecStateChange=cardSecStateChange, cardSubType=cardSubType, cardNumPorts=cardNumPorts, cmResetClearNotification=cmResetClearNotification, cardTestEntry=cardTestEntry, portBgpId=portBgpId, equipmentCardGroup=equipmentCardGroup, errEventId=errEventId, diskVolumeId=diskVolumeId, cardHwVersion=cardHwVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetInfo=cmResetInfo, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageNotification=diskVolumeUsageNotification, cmResetStatus=cmResetStatus, cardSpareGroupMode=cardSpareGroupMode, cardCarrierHwVersion=cardCarrierHwVersion, cardTestIndex=cardTestIndex, cardTestSchedule=cardTestSchedule, portAdminState=portAdminState, cardName=cardName, systemClock=systemClock, portTxFlowControlMode=portTxFlowControlMode, cardTestTable=cardTestTable, trapSeverity=trapSeverity, auditName=auditName, PYSNMP_MODULE_ID=cadEquipmentMib, portMacIfIndex=portMacIfIndex, shelfSwVersion=shelfSwVersion, shelfObjects=shelfObjects, systemKey=systemKey, systemGeneral=systemGeneral, diskVolumeTable=diskVolumeTable, portAction=portAction, cardBusSpeed=cardBusSpeed, cardFlashSize=cardFlashSize, cardTestType=cardTestType, cardTestResultNotification=cardTestResultNotification, cardTemperature=cardTemperature, diskVolumeCardId=diskVolumeCardId, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentState=equipmentState, cardFpgaSource=cardFpgaSource, auditCommand=auditCommand, portDplxStatusChange=portDplxStatusChange, cardSubDetected=cardSubDetected, cardUpTime=cardUpTime, cardId=cardId, cmIpAddress=cmIpAddress, cardTestId=cardTestId, cardCarrierSerialNum=cardCarrierSerialNum, equipmentError=equipmentError, cardCpuType=cardCpuType, cardLastBootSource=cardLastBootSource, portSecState=portSecState, cardAdminState=cardAdminState, cardPrState=cardPrState, consoleOutput=consoleOutput, diagTestId=diagTestId, cardTestTime=cardTestTime, cardPicHwVersion=cardPicHwVersion, diskVolumeName=diskVolumeName, auditEntry=auditEntry, diskVolumeGroup=diskVolumeGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTemperatureHighError=cardTemperatureHighError, diskDriveNumber=diskDriveNumber, cardLastChangeTime=cardLastChangeTime, auditStatus=auditStatus, fpgaErrorEventTable=fpgaErrorEventTable, cardSwVersion=cardSwVersion, TestType=TestType, auditAutoScheduling=auditAutoScheduling, cardDplxStatus=cardDplxStatus, cardDetectedChange=cardDetectedChange, cardSecState=cardSecState, auditTime=auditTime, cardDplxStatusChange=cardDplxStatusChange, downstreamPowerLoss=downstreamPowerLoss, equipmentPortGroup=equipmentPortGroup, cardAction=cardAction, cardNumber=cardNumber, cadEquipmentMibGroup=cadEquipmentMibGroup, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageLevel=diskVolumeUsageLevel, errEvRowStatus=errEvRowStatus, portDetectedMode=portDetectedMode, cardShelfTestIndex=cardShelfTestIndex, auditResult=auditResult, auditLogOutput=auditLogOutput, portRxFlowControlDetected=portRxFlowControlDetected, cardPicSerialNum=cardPicSerialNum, cardTestTransId=cardTestTransId, TestTransactionId=TestTransactionId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditTotalPassedCount=auditTotalPassedCount, portConnectorId=portConnectorId, verboseLevel=verboseLevel, cardEntry=cardEntry, cardTestName=cardTestName, diskVolumeDriveId=diskVolumeDriveId, portCardId=portCardId, diskVolumeShelfId=diskVolumeShelfId, cardCpuSpeed=cardCpuSpeed, TestResult=TestResult, cmResetNotification=cmResetNotification, cardPrStateChange=cardPrStateChange, cardPicDetected=cardPicDetected, remainInDiagMode=remainInDiagMode, cmResetUpTime=cmResetUpTime, auditCycleCount=auditCycleCount, diskVolumeNumber=diskVolumeNumber, errEvLogLevel=errEvLogLevel, equipmentAuditGroup=equipmentAuditGroup, auditFailedCount=auditFailedCount, cardSerialNum=cardSerialNum, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSpareGroupId=cardSpareGroupId, cardTable=cardTable, cadEquipmentMib=cadEquipmentMib)
