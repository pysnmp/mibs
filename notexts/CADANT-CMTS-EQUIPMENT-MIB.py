...
SecondaryState, FlowControlMode, PortMode, ShelfId, UnknownState, CardSubType, PortId, CardType, DiskVolumeUsageLevel, EqActionType, PicType, PortDetectedMode, DuplexStatus, PortType, PrimaryState, FirmwareSource, CardId, AdminState = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "FlowControlMode", "PortMode", "ShelfId", "UnknownState", "CardSubType", "PortId", "CardType", "DiskVolumeUsageLevel", "EqActionType", "PicType", "PortDetectedMode", "DuplexStatus", "PortType", "PrimaryState", "FirmwareSource", "CardId", "AdminState")
...
RowStatus, DateAndTime, TruthValue, DisplayString, TextualConvention, TimeStamp, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "TruthValue", "DisplayString", "TextualConvention", "TimeStamp", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portMode=portMode, cardTemperature=cardTemperature, shelfNumber=shelfNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardUpTime=cardUpTime, diagTestId=diagTestId, auditName=auditName, equipmentPortGroup=equipmentPortGroup, auditId=auditId, cardTestIndex=cardTestIndex, portDocsIfIndex=portDocsIfIndex, diskVolumeFileName=diskVolumeFileName, auditFailedCount=auditFailedCount, cmUChannelID=cmUChannelID, cardPrStateChange=cardPrStateChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTotalPassedCount=auditTotalPassedCount, downstreamPowerLoss=downstreamPowerLoss, portShelfId=portShelfId, cardId=cardId, shelfName=shelfName, portPrState=portPrState, portDescription=portDescription, portTxFlowControlMode=portTxFlowControlMode, portId=portId, trapSeverity=trapSeverity, cardSubDetected=cardSubDetected, portCurrDsPower=portCurrDsPower, cardFlashSize=cardFlashSize, cmResetReason=cmResetReason, systemGeneralGroup=systemGeneralGroup, cardAdminState=cardAdminState, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestId=cardTestId, cmResetClearNotification=cmResetClearNotification, cardTestTable=cardTestTable, cardSwVersion=cardSwVersion, cardDplxStatus=cardDplxStatus, cardPicHwVersion=cardPicHwVersion, systemGeneral=systemGeneral, cardSecState=cardSecState, cardDplxStatusChange=cardDplxStatusChange, cardSpareGroupMode=cardSpareGroupMode, portTable=portTable, cardLastBootSource=cardLastBootSource, errEvLogLevel=errEvLogLevel, cmResetStatus=cmResetStatus, cardFpgaSource=cardFpgaSource, portMacIfIndex=portMacIfIndex, shelfSwVersion=shelfSwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, cmRegistrationNotification=cmRegistrationNotification, cardPicSerialNum=cardPicSerialNum, cardType=cardType, cardTestResultNotification=cardTestResultNotification, cardLastBootVersion=cardLastBootVersion, portAction=portAction, cardPicDetected=cardPicDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeTrapInh=diskVolumeTrapInh, cardFwUpdateStatus=cardFwUpdateStatus, cardCarrierSerialNum=cardCarrierSerialNum, TestId=TestId, cardSubType=cardSubType, cardEntry=cardEntry, portDplxStatus=portDplxStatus, cardTestType=cardTestType, portDetectedMode=portDetectedMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, trapCounter=trapCounter, cardCpuType=cardCpuType, cardCPLDVersion=cardCPLDVersion, cardTestEntry=cardTestEntry, portType=portType, equipmentAudit=equipmentAudit, cardSpareGroupId=cardSpareGroupId, cmPrimarySID=cmPrimarySID, cardSerialNum=cardSerialNum, equipmentStateGroup=equipmentStateGroup, cardName=cardName, errEvRecoveryEnabled=errEvRecoveryEnabled, auditCycleCount=auditCycleCount, auditStatus=auditStatus, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portSecState=portSecState, cardLastChangeTime=cardLastChangeTime, consoleOutput=consoleOutput, auditDescription=auditDescription, cardBootVersion=cardBootVersion, portBgpId=portBgpId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestTime=cardTestTime, diskVolumeNumber=diskVolumeNumber, portEntry=portEntry, portCardId=portCardId, portMaxDsPower=portMaxDsPower, portSecStateChange=portSecStateChange, TestCommand=TestCommand, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeDriveId=diskVolumeDriveId, cardTestResult=cardTestResult, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cadEquipmentMibGroup=cadEquipmentMibGroup, auditCardId=auditCardId, cardTestName=cardTestName, auditCommand=auditCommand, cardShelfId=cardShelfId, auditResult=auditResult, auditTotalFailedCount=auditTotalFailedCount, portMacAddress=portMacAddress, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDiskSize=diskVolumeDiskSize, systemClock=systemClock, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTemperatureHighError=cardTemperatureHighError, portConnectorId=portConnectorId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestType=TestType, equipmentTbl=equipmentTbl, cardHwVersion=cardHwVersion, portTrapInh=portTrapInh, auditAutoScheduling=auditAutoScheduling, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditLogThrottle=auditLogThrottle, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwVersion=cardFwVersion, diskVolumeShelfId=diskVolumeShelfId, cardCarrierFwVersion=cardCarrierFwVersion, cardNumPorts=cardNumPorts, auditPassedCount=auditPassedCount, cmIpAddress=cmIpAddress, portNumChans=portNumChans, cardDetectedChange=cardDetectedChange, auditTime=auditTime, cardDetected=cardDetected, errEvRowStatus=errEvRowStatus, TestResult=TestResult, TestTransactionId=TestTransactionId, cmResetUpTime=cmResetUpTime, auditShelfId=auditShelfId, portNumber=portNumber, diskVolumeEntry=diskVolumeEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cmMacAddress=cmMacAddress, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeId=diskVolumeId, remainInDiagMode=remainInDiagMode, equipmentState=equipmentState, cardShelfTestIndex=cardShelfTestIndex, portPrStateChange=portPrStateChange, equipmentDiagGroup=equipmentDiagGroup, portMinDsPower=portMinDsPower, equipmentShelfGroup=equipmentShelfGroup, auditEntry=auditEntry, portLastChangeTime=portLastChangeTime, cardRamSize=cardRamSize, TestSchedule=TestSchedule, cardLastResetReason=cardLastResetReason, equipmentTraps=equipmentTraps, shelfObjects=shelfObjects, cardTrapInh=cardTrapInh, diskVolumeCardId=diskVolumeCardId, equipmentError=equipmentError, cardTable=cardTable, auditTable=auditTable, diskVolumeName=diskVolumeName, portTxFlowControlDetected=portTxFlowControlDetected, equipmentDiag=equipmentDiag, cmDevice=cmDevice, verboseLevel=verboseLevel, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cardNumber=cardNumber, diskVolumeTable=diskVolumeTable, cardTestSchedule=cardTestSchedule, cmResetNotification=cmResetNotification, diskVolumeSize=diskVolumeSize, cardTestDescription=cardTestDescription, portAdminState=portAdminState, eqDiagConfig=eqDiagConfig, cardTestCommand=cardTestCommand, TestScheduleCommand=TestScheduleCommand, cardAction=cardAction, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlDetected=portRxFlowControlDetected, portRxFlowControlMode=portRxFlowControlMode, cardTestTransId=cardTestTransId, cardSecStateChange=cardSecStateChange, cardCpuSpeed=cardCpuSpeed, cardBusSpeed=cardBusSpeed, cadEquipmentMib=cadEquipmentMib, cmVendor=cmVendor, equipmentAuditGroup=equipmentAuditGroup, errEventId=errEventId, cmResetInfo=cmResetInfo, diskVolumeUsageNotification=diskVolumeUsageNotification, auditLogOutput=auditLogOutput, systemKey=systemKey, cardTestScheduleCommand=cardTestScheduleCommand, cardPrState=cardPrState, portCardSubType=portCardSubType, cardTestResultDesc=cardTestResultDesc, cardCarrierHwVersion=cardCarrierHwVersion, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeGroup=diskVolumeGroup, diskDriveNumber=diskDriveNumber)
