...
PortDetectedMode, EqActionType, DiskVolumeUsageLevel, UnknownState, CardType, SecondaryState, PortId, CardSubType, FlowControlMode, PortMode, CardId, PrimaryState, FirmwareSource, ShelfId, PicType, PortType, AdminState, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "EqActionType", "DiskVolumeUsageLevel", "UnknownState", "CardType", "SecondaryState", "PortId", "CardSubType", "FlowControlMode", "PortMode", "CardId", "PrimaryState", "FirmwareSource", "ShelfId", "PicType", "PortType", "AdminState", "DuplexStatus")
...
RowStatus, DisplayString, TruthValue, DateAndTime, TimeStamp, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "TruthValue", "DateAndTime", "TimeStamp", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageLevel=diskVolumeUsageLevel, auditCardId=auditCardId, cardName=cardName, auditName=auditName, cardDetectedChange=cardDetectedChange, diskVolumeEntry=diskVolumeEntry, cardDetected=cardDetected, portMinDsPower=portMinDsPower, cardTestResultDesc=cardTestResultDesc, cardTrapInh=cardTrapInh, portMaxDsPower=portMaxDsPower, systemClock=systemClock, cardPicHwVersion=cardPicHwVersion, portMacIfIndex=portMacIfIndex, systemKey=systemKey, auditTotalPassedCount=auditTotalPassedCount, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentAudit=equipmentAudit, TestResult=TestResult, TestTransactionId=TestTransactionId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmIpAddress=cmIpAddress, cardTestId=cardTestId, auditCommand=auditCommand, cardLastResetReason=cardLastResetReason, cmRegistrationNotification=cmRegistrationNotification, portSecState=portSecState, cardEntry=cardEntry, portTable=portTable, cmResetStatus=cmResetStatus, cardSpareGroupMode=cardSpareGroupMode, cardFpgaSource=cardFpgaSource, cardTestResultNotification=cardTestResultNotification, TestType=TestType, diskVolumeDiskSize=diskVolumeDiskSize, errEvLogLevel=errEvLogLevel, cmResetUpTime=cmResetUpTime, auditLogThrottle=auditLogThrottle, cardFlashSize=cardFlashSize, cadEquipmentMibGroup=cadEquipmentMibGroup, portType=portType, diskVolumeNumber=diskVolumeNumber, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cardAction=cardAction, cardSecStateChange=cardSecStateChange, cardCPLDVersion=cardCPLDVersion, cardSerialNum=cardSerialNum, shelfObjects=shelfObjects, equipmentTraps=equipmentTraps, cadEquipmentMib=cadEquipmentMib, cmResetNotification=cmResetNotification, portTxFlowControlDetected=portTxFlowControlDetected, portNumber=portNumber, cardFwUpdateStatus=cardFwUpdateStatus, cardUpTime=cardUpTime, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetInfo=cmResetInfo, cardTestName=cardTestName, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestDescription=cardTestDescription, diskVolumeShelfId=diskVolumeShelfId, auditShelfId=auditShelfId, diskVolumeTrapInh=diskVolumeTrapInh, cardTestEntry=cardTestEntry, equipmentDiag=equipmentDiag, equipmentTbl=equipmentTbl, portDetectedMode=portDetectedMode, cardDplxStatus=cardDplxStatus, cardTestCommand=cardTestCommand, remainInDiagMode=remainInDiagMode, portNumChans=portNumChans, trapCounter=trapCounter, eqDiagConfig=eqDiagConfig, equipmentError=equipmentError, portLastChangeTime=portLastChangeTime, portEntry=portEntry, TestId=TestId, shelfSwVersion=shelfSwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, portRxFlowControlDetected=portRxFlowControlDetected, TestCommand=TestCommand, portDplxStatus=portDplxStatus, auditLogOutput=auditLogOutput, diskVolumeGroup=diskVolumeGroup, auditEntry=auditEntry, equipmentCardTestGroup=equipmentCardTestGroup, portDplxStatusChange=portDplxStatusChange, diagTestId=diagTestId, portPrState=portPrState, TestSchedule=TestSchedule, cardFwVersion=cardFwVersion, cardCarrierHwVersion=cardCarrierHwVersion, portBgpId=portBgpId, auditPassedCount=auditPassedCount, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsageNotification=diskVolumeUsageNotification, cardLastBootVersion=cardLastBootVersion, portConnectorId=portConnectorId, diskVolumeDriveId=diskVolumeDriveId, cardTable=cardTable, portCardId=portCardId, auditFailedCount=auditFailedCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portRxFlowControlMode=portRxFlowControlMode, cardPrState=cardPrState, cardBootVersion=cardBootVersion, auditDescription=auditDescription, cardLastBootSource=cardLastBootSource, portMode=portMode, portCardSubType=portCardSubType, portDescription=portDescription, cardSpareGroupId=cardSpareGroupId, equipmentShelfGroup=equipmentShelfGroup, cardCarrierSerialNum=cardCarrierSerialNum, cardTemperature=cardTemperature, cardNumber=cardNumber, verboseLevel=verboseLevel, cmDevice=cmDevice, portTrapInh=portTrapInh, cardRamSize=cardRamSize, cardCpuType=cardCpuType, cardAdminState=cardAdminState, cardTestSchedule=cardTestSchedule, cardPicDetected=cardPicDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentPortGroup=equipmentPortGroup, auditCycleCount=auditCycleCount, cardTestResult=cardTestResult, shelfNumber=shelfNumber, auditTime=auditTime, portId=portId, diskVolumeName=diskVolumeName, cardSwVersion=cardSwVersion, equipmentStateGroup=equipmentStateGroup, auditStatus=auditStatus, cardSubType=cardSubType, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetReason=cmResetReason, portPrStateChange=portPrStateChange, equipmentState=equipmentState, cardHwVersion=cardHwVersion, cardTemperatureHighError=cardTemperatureHighError, cmUChannelID=cmUChannelID, cmMacAddress=cmMacAddress, equipmentAuditGroup=equipmentAuditGroup, cardTestTransId=cardTestTransId, cardSecState=cardSecState, cardSubDetected=cardSubDetected, systemGeneralGroup=systemGeneralGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCarrierFwVersion=cardCarrierFwVersion, TestScheduleCommand=TestScheduleCommand, cardType=cardType, cardShelfId=cardShelfId, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeSize=diskVolumeSize, portAdminState=portAdminState, portShelfId=portShelfId, diskVolumeFileName=diskVolumeFileName, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestIndex=cardTestIndex, auditId=auditId, portSecStateChange=portSecStateChange, auditTable=auditTable, diskDriveNumber=diskDriveNumber, diskVolumeTable=diskVolumeTable, errEvLoggingEnabled=errEvLoggingEnabled, cardPrStateChange=cardPrStateChange, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, shelfName=shelfName, downstreamPowerLoss=downstreamPowerLoss, cardBusSpeed=cardBusSpeed, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastChangeTime=cardLastChangeTime, cmVendor=cmVendor, portDocsIfIndex=portDocsIfIndex, portMacAddress=portMacAddress, cardTestTable=cardTestTable, cmResetClearNotification=cmResetClearNotification, diskVolumeCardId=diskVolumeCardId, diskVolumeId=diskVolumeId, auditAutoScheduling=auditAutoScheduling, cardDplxStatusChange=cardDplxStatusChange, auditResult=auditResult, errEventId=errEventId, cardCpuSpeed=cardCpuSpeed, cardTestScheduleCommand=cardTestScheduleCommand, portTxFlowControlMode=portTxFlowControlMode, portCurrDsPower=portCurrDsPower, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, consoleOutput=consoleOutput, portAction=portAction, cmPrimarySID=cmPrimarySID, systemGeneral=systemGeneral, cardPicSerialNum=cardPicSerialNum, cardNumPorts=cardNumPorts, equipmentDiagGroup=equipmentDiagGroup, trapSeverity=trapSeverity, cardShelfTestIndex=cardShelfTestIndex, cardTestType=cardTestType, cardTestTime=cardTestTime, cardId=cardId, errEvRowStatus=errEvRowStatus)
