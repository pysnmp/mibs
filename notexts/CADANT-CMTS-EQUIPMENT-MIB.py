...
CardId, PrimaryState, PicType, EqActionType, CardType, PortMode, UnknownState, DiskVolumeUsageLevel, AdminState, PortDetectedMode, FirmwareSource, PortType, PortId, DuplexStatus, ShelfId, FlowControlMode, CardSubType, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "CardId", "PrimaryState", "PicType", "EqActionType", "CardType", "PortMode", "UnknownState", "DiskVolumeUsageLevel", "AdminState", "PortDetectedMode", "FirmwareSource", "PortType", "PortId", "DuplexStatus", "ShelfId", "FlowControlMode", "CardSubType", "SecondaryState")
...
RowStatus, TextualConvention, TruthValue, DateAndTime, TimeStamp, MacAddress, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "TruthValue", "DateAndTime", "TimeStamp", "MacAddress", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portCardSubType=portCardSubType, cardFpgaSource=cardFpgaSource, equipmentDiag=equipmentDiag, equipmentShelfGroup=equipmentShelfGroup, portMacIfIndex=portMacIfIndex, auditEntry=auditEntry, cmVendor=cmVendor, portPrStateChange=portPrStateChange, auditAutoScheduling=auditAutoScheduling, portConnectorId=portConnectorId, cadEquipmentMib=cadEquipmentMib, cardTemperatureHighError=cardTemperatureHighError, cardCpuSpeed=cardCpuSpeed, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, fpgaErrorEventEntry=fpgaErrorEventEntry, cardAction=cardAction, cardSwVersion=cardSwVersion, equipmentTraps=equipmentTraps, cadEquipmentMibConformance=cadEquipmentMibConformance, auditName=auditName, equipmentPortGroup=equipmentPortGroup, cmUChannelID=cmUChannelID, portId=portId, portDplxStatus=portDplxStatus, cardSerialNum=cardSerialNum, cmIpAddress=cmIpAddress, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTemperatureHighWarn=cardTemperatureHighWarn, TestSchedule=TestSchedule, systemClock=systemClock, portLastChangeTime=portLastChangeTime, cardBusSpeed=cardBusSpeed, cardTestTable=cardTestTable, cmDevice=cmDevice, cardTestScheduleCommand=cardTestScheduleCommand, cardType=cardType, cardTestResultDesc=cardTestResultDesc, cardTestIndex=cardTestIndex, shelfObjects=shelfObjects, cardTestTransId=cardTestTransId, cardCarrierSerialNum=cardCarrierSerialNum, TestResult=TestResult, diskVolumeName=diskVolumeName, verboseLevel=verboseLevel, cardDplxStatus=cardDplxStatus, auditFailedCount=auditFailedCount, systemGeneralGroup=systemGeneralGroup, shelfName=shelfName, portTxFlowControlDetected=portTxFlowControlDetected, errEvLogLevel=errEvLogLevel, cmResetInfo=cmResetInfo, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentState=equipmentState, cardFwUpdateStatus=cardFwUpdateStatus, TestCommand=TestCommand, cardPicSerialNum=cardPicSerialNum, diagTestId=diagTestId, equipmentCardTestGroup=equipmentCardTestGroup, auditDescription=auditDescription, cardHwVersion=cardHwVersion, cardTempNoReportNotification=cardTempNoReportNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmRegistrationNotification=cmRegistrationNotification, portSecStateChange=portSecStateChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentAudit=equipmentAudit, cardTable=cardTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portMinDsPower=portMinDsPower, auditTime=auditTime, cardTrapInh=cardTrapInh, diskVolumeUsageLevel=diskVolumeUsageLevel, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestId=TestId, TestScheduleCommand=TestScheduleCommand, TestTransactionId=TestTransactionId, diskVolumeNumber=diskVolumeNumber, cardTestResultNotification=cardTestResultNotification, portDescription=portDescription, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentTbl=equipmentTbl, portSecState=portSecState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardBootVersion=cardBootVersion, equipmentAuditGroup=equipmentAuditGroup, auditLogOutput=auditLogOutput, portPrState=portPrState, cardShelfId=cardShelfId, portTxFlowControlMode=portTxFlowControlMode, errEvRowStatus=errEvRowStatus, cardFwVersion=cardFwVersion, trapSeverity=trapSeverity, cardCpuType=cardCpuType, portAction=portAction, cardTestCommand=cardTestCommand, cardTestName=cardTestName, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetNotification=cmResetNotification, cmMacAddress=cmMacAddress, diskVolumeShelfId=diskVolumeShelfId, auditCardId=auditCardId, portCurrDsPower=portCurrDsPower, cardFlashSize=cardFlashSize, fpgaErrorEventGroup=fpgaErrorEventGroup, downstreamPowerLoss=downstreamPowerLoss, cardSecState=cardSecState, cardDetectedChange=cardDetectedChange, cmResetStatus=cmResetStatus, cardPrStateChange=cardPrStateChange, fpgaErrorEventTable=fpgaErrorEventTable, portType=portType, cardTestId=cardTestId, cardDetected=cardDetected, auditTotalPassedCount=auditTotalPassedCount, cardSubType=cardSubType, auditPassedCount=auditPassedCount, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeGroup=diskVolumeGroup, cardPicHwVersion=cardPicHwVersion, remainInDiagMode=remainInDiagMode, cadEquipmentMibGroup=cadEquipmentMibGroup, portNumChans=portNumChans, diskVolumeId=diskVolumeId, cardId=cardId, cardNumber=cardNumber, diskVolumeTable=diskVolumeTable, consoleOutput=consoleOutput, shelfNumber=shelfNumber, cardTestType=cardTestType, errEventId=errEventId, equipmentCardGroup=equipmentCardGroup, portTable=portTable, portAdminState=portAdminState, cardRamSize=cardRamSize, portBgpId=portBgpId, diskVolumeCardId=diskVolumeCardId, auditId=auditId, portMaxDsPower=portMaxDsPower, equipmentDiagGroup=equipmentDiagGroup, auditCycleCount=auditCycleCount, cardDplxStatusChange=cardDplxStatusChange, auditLogThrottle=auditLogThrottle, portDetectedMode=portDetectedMode, auditResult=auditResult, diskVolumeDiskSize=diskVolumeDiskSize, portTrapInh=portTrapInh, cardUpTime=cardUpTime, portShelfId=portShelfId, cardTestEntry=cardTestEntry, portDocsIfIndex=portDocsIfIndex, portEntry=portEntry, cardSecStateChange=cardSecStateChange, cardPicDetected=cardPicDetected, shelfSwVersion=shelfSwVersion, cardSpareGroupId=cardSpareGroupId, cardTestTime=cardTestTime, cardEntry=cardEntry, cardTestResult=cardTestResult, cardShelfTestIndex=cardShelfTestIndex, auditStatus=auditStatus, systemGeneral=systemGeneral, diskDriveNumber=diskDriveNumber, cardCPLDVersion=cardCPLDVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetReason=cmResetReason, portMacAddress=portMacAddress, cardLastChangeTime=cardLastChangeTime, auditTotalFailedCount=auditTotalFailedCount, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTrapInh=diskVolumeTrapInh, portMode=portMode, auditTable=auditTable, cmResetClearNotification=cmResetClearNotification, eqDiagConfig=eqDiagConfig, cmResetUpTime=cmResetUpTime, auditCommand=auditCommand, cardLastResetReason=cardLastResetReason, diskVolumeUsageNotification=diskVolumeUsageNotification, portDplxStatusChange=portDplxStatusChange, trapCounter=trapCounter, cmPrimarySID=cmPrimarySID, cardSubDetected=cardSubDetected, cardName=cardName, systemKey=systemKey, portRxFlowControlMode=portRxFlowControlMode, cardTestDescription=cardTestDescription, cardTestSchedule=cardTestSchedule, equipmentStateGroup=equipmentStateGroup, portNumber=portNumber, cardNumPorts=cardNumPorts, cardAdminState=cardAdminState, cardTemperature=cardTemperature, cardLastBootVersion=cardLastBootVersion, cardSpareGroupMode=cardSpareGroupMode, equipmentError=equipmentError, cardCarrierFwVersion=cardCarrierFwVersion, portRxFlowControlDetected=portRxFlowControlDetected, cardPrState=cardPrState, diskVolumeDriveId=diskVolumeDriveId, auditShelfId=auditShelfId, portCardId=portCardId, diskVolumeFileName=diskVolumeFileName, cardCarrierHwVersion=cardCarrierHwVersion, cardLastBootSource=cardLastBootSource, diskVolumeSize=diskVolumeSize, diskVolumeEntry=diskVolumeEntry, TestType=TestType)
