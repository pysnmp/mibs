...
AdminState, PicType, PortDetectedMode, CardId, UnknownState, SecondaryState, PortId, FirmwareSource, DiskVolumeUsageLevel, CardType, EqActionType, DuplexStatus, PortType, PrimaryState, FlowControlMode, PortMode, ShelfId, CardSubType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PicType", "PortDetectedMode", "CardId", "UnknownState", "SecondaryState", "PortId", "FirmwareSource", "DiskVolumeUsageLevel", "CardType", "EqActionType", "DuplexStatus", "PortType", "PrimaryState", "FlowControlMode", "PortMode", "ShelfId", "CardSubType")
...
RowStatus, TimeStamp, TruthValue, DisplayString, DateAndTime, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "TruthValue", "DisplayString", "DateAndTime", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPicHwVersion=cardPicHwVersion, portCardSubType=portCardSubType, trapSeverity=trapSeverity, shelfNumber=shelfNumber, equipmentState=equipmentState, cardCpuSpeed=cardCpuSpeed, cardName=cardName, cmUChannelID=cmUChannelID, cardTestResult=cardTestResult, equipmentStateGroup=equipmentStateGroup, diskVolumeTable=diskVolumeTable, cardTestEntry=cardTestEntry, diskVolumeGroup=diskVolumeGroup, portNumChans=portNumChans, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetInfo=cmResetInfo, equipmentDiagGroup=equipmentDiagGroup, portTrapInh=portTrapInh, cardTempNoReportNotification=cardTempNoReportNotification, TestTransactionId=TestTransactionId, auditShelfId=auditShelfId, diskVolumeCardId=diskVolumeCardId, portMacAddress=portMacAddress, auditCycleCount=auditCycleCount, equipmentError=equipmentError, systemClock=systemClock, cardTable=cardTable, errEvLoggingEnabled=errEvLoggingEnabled, diskDriveNumber=diskDriveNumber, cardNumber=cardNumber, cardTestType=cardTestType, portDplxStatusChange=portDplxStatusChange, TestResult=TestResult, portNumber=portNumber, cardEntry=cardEntry, downstreamPowerLoss=downstreamPowerLoss, trapCounter=trapCounter, cardSubDetected=cardSubDetected, equipmentPortGroup=equipmentPortGroup, diskVolumeShelfId=diskVolumeShelfId, cardBootVersion=cardBootVersion, diskVolumeNumber=diskVolumeNumber, TestId=TestId, cmVendor=cmVendor, portDescription=portDescription, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetUpTime=cmResetUpTime, cardDetected=cardDetected, portLastChangeTime=portLastChangeTime, shelfSwVersion=shelfSwVersion, cardSpareGroupMode=cardSpareGroupMode, diskVolumeEntry=diskVolumeEntry, cardId=cardId, cardFwUpdateStatus=cardFwUpdateStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTempOverHeatNotification=cardTempOverHeatNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTxFlowControlDetected=portTxFlowControlDetected, equipmentShelfGroup=equipmentShelfGroup, cmResetClearNotification=cmResetClearNotification, portEntry=portEntry, cardSerialNum=cardSerialNum, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditLogThrottle=auditLogThrottle, cardTestTable=cardTestTable, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditName=auditName, cmResetStatus=cmResetStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, portDetectedMode=portDetectedMode, portPrStateChange=portPrStateChange, cardSubType=cardSubType, cardTestScheduleCommand=cardTestScheduleCommand, cardType=cardType, equipmentCardGroup=equipmentCardGroup, diskVolumeDiskSize=diskVolumeDiskSize, systemGeneral=systemGeneral, TestScheduleCommand=TestScheduleCommand, cardCarrierHwVersion=cardCarrierHwVersion, portConnectorId=portConnectorId, diskVolumeTrapInh=diskVolumeTrapInh, TestType=TestType, shelfObjects=shelfObjects, cadEquipmentMib=cadEquipmentMib, cardPicDetected=cardPicDetected, cardTemperatureHighError=cardTemperatureHighError, cardPrState=cardPrState, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeFileName=diskVolumeFileName, errEventId=errEventId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portId=portId, cmResetNotification=cmResetNotification, portAdminState=portAdminState, errEvLogLevel=errEvLogLevel, cmMacAddress=cmMacAddress, cmIpAddress=cmIpAddress, auditCommand=auditCommand, cardBusSpeed=cardBusSpeed, shelfName=shelfName, auditDescription=auditDescription, portRxFlowControlMode=portRxFlowControlMode, eqDiagConfig=eqDiagConfig, fpgaErrorEventEntry=fpgaErrorEventEntry, portPrState=portPrState, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentAudit=equipmentAudit, portMaxDsPower=portMaxDsPower, cardSecState=cardSecState, portSecStateChange=portSecStateChange, cardHwVersion=cardHwVersion, cardCPLDVersion=cardCPLDVersion, cardTestId=cardTestId, cardLastBootVersion=cardLastBootVersion, cardLastResetReason=cardLastResetReason, systemGeneralGroup=systemGeneralGroup, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeId=diskVolumeId, equipmentTbl=equipmentTbl, cardDplxStatus=cardDplxStatus, diskVolumeDriveId=diskVolumeDriveId, auditTotalPassedCount=auditTotalPassedCount, cardPrStateChange=cardPrStateChange, portTable=portTable, cardTestSchedule=cardTestSchedule, cardSwVersion=cardSwVersion, cardFlashSize=cardFlashSize, cardUpTime=cardUpTime, portDocsIfIndex=portDocsIfIndex, auditStatus=auditStatus, portAction=portAction, cardTestResultNotification=cardTestResultNotification, auditId=auditId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvRowStatus=errEvRowStatus, cmPrimarySID=cmPrimarySID, cmResetReason=cmResetReason, cardShelfId=cardShelfId, portMode=portMode, equipmentDiag=equipmentDiag, portTxFlowControlMode=portTxFlowControlMode, portMacIfIndex=portMacIfIndex, cardPicSerialNum=cardPicSerialNum, cardTemperature=cardTemperature, cardTrapInh=cardTrapInh, cmDevice=cmDevice, cardFwVersion=cardFwVersion, systemKey=systemKey, portSecState=portSecState, diskVolumeName=diskVolumeName, cardShelfTestIndex=cardShelfTestIndex, auditFailedCount=auditFailedCount, TestSchedule=TestSchedule, fpgaErrorEventTable=fpgaErrorEventTable, auditAutoScheduling=auditAutoScheduling, cardAdminState=cardAdminState, cardTestDescription=cardTestDescription, portMinDsPower=portMinDsPower, cardFpgaSource=cardFpgaSource, cardTestTransId=cardTestTransId, equipmentTraps=equipmentTraps, cardNumPorts=cardNumPorts, cardLastChangeTime=cardLastChangeTime, cardTestResultDesc=cardTestResultDesc, cardTestTime=cardTestTime, auditTime=auditTime, cmRegistrationNotification=cmRegistrationNotification, cardCarrierSerialNum=cardCarrierSerialNum, fpgaErrorEventGroup=fpgaErrorEventGroup, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, remainInDiagMode=remainInDiagMode, auditEntry=auditEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, cardRamSize=cardRamSize, cardSpareGroupId=cardSpareGroupId, portBgpId=portBgpId, auditTable=auditTable, cardDetectedChange=cardDetectedChange, diskVolumeSize=diskVolumeSize, portShelfId=portShelfId, auditPassedCount=auditPassedCount, TestCommand=TestCommand, cardTestIndex=cardTestIndex, consoleOutput=consoleOutput, auditCardId=auditCardId, cardDplxStatusChange=cardDplxStatusChange, verboseLevel=verboseLevel, auditResult=auditResult, PYSNMP_MODULE_ID=cadEquipmentMib, cardSecStateChange=cardSecStateChange, portCurrDsPower=portCurrDsPower, cardTestCommand=cardTestCommand, portType=portType, cardTestName=cardTestName, auditLogOutput=auditLogOutput, cardCpuType=cardCpuType, auditTotalFailedCount=auditTotalFailedCount, equipmentAuditGroup=equipmentAuditGroup, cardAction=cardAction, portCardId=portCardId, portDplxStatus=portDplxStatus, cardLastBootSource=cardLastBootSource, diagTestId=diagTestId)
