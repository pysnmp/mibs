...
FlowControlMode, PortType, CardSubType, FirmwareSource, ShelfId, PortDetectedMode, UnknownState, CardId, AdminState, CardType, DuplexStatus, PortMode, PrimaryState, DiskVolumeUsageLevel, PicType, PortId, SecondaryState, EqActionType = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PortType", "CardSubType", "FirmwareSource", "ShelfId", "PortDetectedMode", "UnknownState", "CardId", "AdminState", "CardType", "DuplexStatus", "PortMode", "PrimaryState", "DiskVolumeUsageLevel", "PicType", "PortId", "SecondaryState", "EqActionType")
...
RowStatus, MacAddress, TruthValue, DateAndTime, DisplayString, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "MacAddress", "TruthValue", "DateAndTime", "DisplayString", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", errEvRecoveryEnabled=errEvRecoveryEnabled, cardSecStateChange=cardSecStateChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTable=cardTable, cardTestDescription=cardTestDescription, cardName=cardName, cardTemperature=cardTemperature, portAdminState=portAdminState, errEvRowStatus=errEvRowStatus, cardTestId=cardTestId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetInfo=cmResetInfo, diskVolumeCardId=diskVolumeCardId, cmPrimarySID=cmPrimarySID, equipmentShelfGroup=equipmentShelfGroup, portTxFlowControlMode=portTxFlowControlMode, cardFwUpdateStatus=cardFwUpdateStatus, auditId=auditId, shelfSwVersion=shelfSwVersion, equipmentTbl=equipmentTbl, portConnectorId=portConnectorId, diskVolumeTrapInh=diskVolumeTrapInh, cardSpareGroupId=cardSpareGroupId, cardSecState=cardSecState, auditEntry=auditEntry, portType=portType, diskVolumeShelfId=diskVolumeShelfId, trapSeverity=trapSeverity, portDescription=portDescription, cardSwVersion=cardSwVersion, systemKey=systemKey, cadEquipmentMib=cadEquipmentMib, cardAdminState=cardAdminState, cardTestScheduleCommand=cardTestScheduleCommand, portId=portId, cardBusSpeed=cardBusSpeed, portMinDsPower=portMinDsPower, fpgaErrorEventEntry=fpgaErrorEventEntry, auditName=auditName, cardCPLDVersion=cardCPLDVersion, cardTestResultNotification=cardTestResultNotification, TestCommand=TestCommand, auditLogOutput=auditLogOutput, cardCarrierFwVersion=cardCarrierFwVersion, cardNumPorts=cardNumPorts, cardTempNoReportNotification=cardTempNoReportNotification, portDocsIfIndex=portDocsIfIndex, cardTestResult=cardTestResult, TestScheduleCommand=TestScheduleCommand, cardTrapInh=cardTrapInh, verboseLevel=verboseLevel, portRxFlowControlDetected=portRxFlowControlDetected, errEvLogLevel=errEvLogLevel, auditFailedCount=auditFailedCount, cardLastResetReason=cardLastResetReason, cardEntry=cardEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTable=cardTestTable, cmRegistrationNotification=cmRegistrationNotification, equipmentTraps=equipmentTraps, portPrState=portPrState, cardCpuSpeed=cardCpuSpeed, cardRamSize=cardRamSize, auditStatus=auditStatus, cardPicHwVersion=cardPicHwVersion, diskVolumeTable=diskVolumeTable, cmUChannelID=cmUChannelID, cardPrStateChange=cardPrStateChange, cardDetectedChange=cardDetectedChange, portTable=portTable, portCurrDsPower=portCurrDsPower, cardPrState=cardPrState, cardBootVersion=cardBootVersion, cardNumber=cardNumber, eqDiagConfig=eqDiagConfig, portLastChangeTime=portLastChangeTime, diskVolumeFileName=diskVolumeFileName, cardTestName=cardTestName, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageNotification=diskVolumeUsageNotification, portCardSubType=portCardSubType, auditTime=auditTime, cardTemperatureHighWarn=cardTemperatureHighWarn, portAction=portAction, cardCarrierHwVersion=cardCarrierHwVersion, portRxFlowControlMode=portRxFlowControlMode, portShelfId=portShelfId, portTxFlowControlDetected=portTxFlowControlDetected, auditTotalPassedCount=auditTotalPassedCount, systemClock=systemClock, portSecStateChange=portSecStateChange, cardHwVersion=cardHwVersion, cmVendor=cmVendor, shelfObjects=shelfObjects, shelfName=shelfName, cardTemperatureHighError=cardTemperatureHighError, cardLastChangeTime=cardLastChangeTime, equipmentError=equipmentError, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestEntry=cardTestEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetNotification=cmResetNotification, diskVolumeEntry=diskVolumeEntry, portMaxDsPower=portMaxDsPower, portDplxStatus=portDplxStatus, portMacIfIndex=portMacIfIndex, cardPicSerialNum=cardPicSerialNum, cardId=cardId, consoleOutput=consoleOutput, trapCounter=trapCounter, auditCycleCount=auditCycleCount, cmResetStatus=cmResetStatus, cardPicDetected=cardPicDetected, diskVolumeNumber=diskVolumeNumber, equipmentStateGroup=equipmentStateGroup, cardTestCommand=cardTestCommand, diskVolumeId=diskVolumeId, downstreamPowerLoss=downstreamPowerLoss, diskVolumeName=diskVolumeName, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSubDetected=cardSubDetected, cardFpgaSource=cardFpgaSource, cmResetUpTime=cmResetUpTime, equipmentDiagGroup=equipmentDiagGroup, portNumChans=portNumChans, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diagTestId=diagTestId, cardAction=cardAction, cardTestTransId=cardTestTransId, portBgpId=portBgpId, cardLastBootSource=cardLastBootSource, diskVolumeDiskSize=diskVolumeDiskSize, cmMacAddress=cmMacAddress, portMode=portMode, portDetectedMode=portDetectedMode, equipmentCardTestGroup=equipmentCardTestGroup, shelfNumber=shelfNumber, auditCommand=auditCommand, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestSchedule=cardTestSchedule, portNumber=portNumber, cardCpuType=cardCpuType, cmDevice=cmDevice, TestId=TestId, auditCardId=auditCardId, cardShelfTestIndex=cardShelfTestIndex, cardTestResultDesc=cardTestResultDesc, systemGeneral=systemGeneral, equipmentAudit=equipmentAudit, cmResetClearNotification=cmResetClearNotification, portTrapInh=portTrapInh, auditLogThrottle=auditLogThrottle, portCardId=portCardId, fpgaErrorEventTable=fpgaErrorEventTable, cardTestTime=cardTestTime, cadEquipmentMibGroup=cadEquipmentMibGroup, TestType=TestType, cardTestType=cardTestType, errEventId=errEventId, systemGeneralGroup=systemGeneralGroup, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentAuditGroup=equipmentAuditGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditDescription=auditDescription, errEvLoggingEnabled=errEvLoggingEnabled, equipmentDiag=equipmentDiag, diskVolumeGroup=diskVolumeGroup, portSecState=portSecState, cardSerialNum=cardSerialNum, fpgaErrorEventGroup=fpgaErrorEventGroup, remainInDiagMode=remainInDiagMode, TestSchedule=TestSchedule, portMacAddress=portMacAddress, cmResetReason=cmResetReason, TestResult=TestResult, equipmentCardGroup=equipmentCardGroup, cardCarrierSerialNum=cardCarrierSerialNum, auditResult=auditResult, TestTransactionId=TestTransactionId, cardUpTime=cardUpTime, cardShelfId=cardShelfId, diskVolumeDriveId=diskVolumeDriveId, cardTestIndex=cardTestIndex, cardSpareGroupMode=cardSpareGroupMode, auditShelfId=auditShelfId, portPrStateChange=portPrStateChange, cmIpAddress=cmIpAddress, auditTable=auditTable, diskDriveNumber=diskDriveNumber, equipmentState=equipmentState, cardDplxStatusChange=cardDplxStatusChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardType=cardType, cardLastBootVersion=cardLastBootVersion, cardDetected=cardDetected, portEntry=portEntry, cardFlashSize=cardFlashSize, cardDplxStatus=cardDplxStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeSize=diskVolumeSize, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditPassedCount=auditPassedCount, cardSubType=cardSubType, cardFwVersion=cardFwVersion, auditAutoScheduling=auditAutoScheduling)
