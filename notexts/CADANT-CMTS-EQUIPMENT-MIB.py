...
PortType, FlowControlMode, PortDetectedMode, PrimaryState, PortId, SecondaryState, FirmwareSource, CardId, DiskVolumeUsageLevel, DuplexStatus, PortMode, CardType, AdminState, UnknownState, CardSubType, EqActionType, PicType, ShelfId = mibBuilder.importSymbols("CADANT-TC", "PortType", "FlowControlMode", "PortDetectedMode", "PrimaryState", "PortId", "SecondaryState", "FirmwareSource", "CardId", "DiskVolumeUsageLevel", "DuplexStatus", "PortMode", "CardType", "AdminState", "UnknownState", "CardSubType", "EqActionType", "PicType", "ShelfId")
...
RowStatus, TruthValue, TextualConvention, TimeStamp, MacAddress, DisplayString, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TruthValue", "TextualConvention", "TimeStamp", "MacAddress", "DisplayString", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentAudit=equipmentAudit, portTxFlowControlMode=portTxFlowControlMode, portNumber=portNumber, auditStatus=auditStatus, diskVolumeNumber=diskVolumeNumber, cardUpTime=cardUpTime, auditTotalFailedCount=auditTotalFailedCount, TestId=TestId, portBgpId=portBgpId, cardTestDescription=cardTestDescription, portConnectorId=portConnectorId, diskVolumeShelfId=diskVolumeShelfId, cardCPLDVersion=cardCPLDVersion, auditName=auditName, cmDevice=cmDevice, cmMacAddress=cmMacAddress, cardTempOverHeatNotification=cardTempOverHeatNotification, auditTable=auditTable, cardSecStateChange=cardSecStateChange, portMacIfIndex=portMacIfIndex, cardSerialNum=cardSerialNum, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageLevel=diskVolumeUsageLevel, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, auditAutoScheduling=auditAutoScheduling, cardPicDetected=cardPicDetected, diskDriveNumber=diskDriveNumber, portSecState=portSecState, auditResult=auditResult, cardTable=cardTable, cardTestType=cardTestType, cardId=cardId, consoleOutput=consoleOutput, equipmentTraps=equipmentTraps, portId=portId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardType=cardType, portShelfId=portShelfId, systemGeneral=systemGeneral, equipmentAuditGroup=equipmentAuditGroup, portPrStateChange=portPrStateChange, auditEntry=auditEntry, cardNumPorts=cardNumPorts, auditLogOutput=auditLogOutput, diagTestId=diagTestId, TestTransactionId=TestTransactionId, portDocsIfIndex=portDocsIfIndex, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portCardSubType=portCardSubType, cardEntry=cardEntry, cmResetInfo=cmResetInfo, diskVolumeGroup=diskVolumeGroup, downstreamPowerLoss=downstreamPowerLoss, cardTestSchedule=cardTestSchedule, equipmentState=equipmentState, equipmentTbl=equipmentTbl, cardLastResetReason=cardLastResetReason, errEvRowStatus=errEvRowStatus, cardDplxStatusChange=cardDplxStatusChange, equipmentCardTestGroup=equipmentCardTestGroup, portTrapInh=portTrapInh, cardTestResult=cardTestResult, systemClock=systemClock, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardFpgaSource=cardFpgaSource, portType=portType, auditCycleCount=auditCycleCount, auditCardId=auditCardId, cardTestCommand=cardTestCommand, cardPrState=cardPrState, portMode=portMode, equipmentShelfGroup=equipmentShelfGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDetectedMode=portDetectedMode, portEntry=portEntry, diskVolumeDriveId=diskVolumeDriveId, cardShelfId=cardShelfId, diskVolumeEntry=diskVolumeEntry, portLastChangeTime=portLastChangeTime, cardTestId=cardTestId, systemGeneralGroup=systemGeneralGroup, cmResetUpTime=cmResetUpTime, portNumChans=portNumChans, cmIpAddress=cmIpAddress, portSecStateChange=portSecStateChange, cardLastChangeTime=cardLastChangeTime, diskVolumeName=diskVolumeName, cardTemperature=cardTemperature, auditTotalPassedCount=auditTotalPassedCount, cardFlashSize=cardFlashSize, cardLastBootSource=cardLastBootSource, diskVolumeFileName=diskVolumeFileName, auditDescription=auditDescription, auditCommand=auditCommand, cardTrapInh=cardTrapInh, cardTestResultDesc=cardTestResultDesc, TestScheduleCommand=TestScheduleCommand, portDplxStatus=portDplxStatus, cardCpuSpeed=cardCpuSpeed, cardSpareGroupId=cardSpareGroupId, verboseLevel=verboseLevel, cardTestIndex=cardTestIndex, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmUChannelID=cmUChannelID, cadEquipmentMib=cadEquipmentMib, cardSubType=cardSubType, cardCarrierHwVersion=cardCarrierHwVersion, portPrState=portPrState, diskVolumeCardId=diskVolumeCardId, errEventId=errEventId, diskVolumeSize=diskVolumeSize, cardLastBootVersion=cardLastBootVersion, shelfSwVersion=shelfSwVersion, TestSchedule=TestSchedule, systemKey=systemKey, cardTestEntry=cardTestEntry, cardTestResultNotification=cardTestResultNotification, TestType=TestType, cardSwVersion=cardSwVersion, cardRamSize=cardRamSize, errEvRecoveryEnabled=errEvRecoveryEnabled, cardDetectedChange=cardDetectedChange, trapCounter=trapCounter, portDplxStatusChange=portDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, trapSeverity=trapSeverity, cardCarrierSerialNum=cardCarrierSerialNum, remainInDiagMode=remainInDiagMode, auditPassedCount=auditPassedCount, shelfNumber=shelfNumber, portCardId=portCardId, portRxFlowControlDetected=portRxFlowControlDetected, cardFwUpdateStatus=cardFwUpdateStatus, portAction=portAction, cardSecState=cardSecState, cmResetReason=cmResetReason, equipmentStateGroup=equipmentStateGroup, portAdminState=portAdminState, cardNumber=cardNumber, cmPrimarySID=cmPrimarySID, equipmentPortGroup=equipmentPortGroup, cmVendor=cmVendor, shelfName=shelfName, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvLoggingEnabled=errEvLoggingEnabled, portMinDsPower=portMinDsPower, cardDetected=cardDetected, errEvLogLevel=errEvLogLevel, TestCommand=TestCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentDiagGroup=equipmentDiagGroup, cardTestTable=cardTestTable, auditLogThrottle=auditLogThrottle, shelfObjects=shelfObjects, diskVolumeDiskSize=diskVolumeDiskSize, cardCarrierFwVersion=cardCarrierFwVersion, cardTestTransId=cardTestTransId, cardPrStateChange=cardPrStateChange, diskVolumeTable=diskVolumeTable, cardHwVersion=cardHwVersion, cardPicSerialNum=cardPicSerialNum, portMacAddress=portMacAddress, cardSubDetected=cardSubDetected, cmResetClearNotification=cmResetClearNotification, fpgaErrorEventTable=fpgaErrorEventTable, eqDiagConfig=eqDiagConfig, cardTestName=cardTestName, cmRegistrationNotification=cmRegistrationNotification, cardSpareGroupMode=cardSpareGroupMode, cardTempNoReportNotification=cardTempNoReportNotification, cmResetStatus=cmResetStatus, cardDplxStatus=cardDplxStatus, equipmentError=equipmentError, portTable=portTable, equipmentCardGroup=equipmentCardGroup, cardTestTime=cardTestTime, portCurrDsPower=portCurrDsPower, cardFwVersion=cardFwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, auditShelfId=auditShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeId=diskVolumeId, cmResetNotification=cmResetNotification, cardBootVersion=cardBootVersion, cardBusSpeed=cardBusSpeed, equipmentDiag=equipmentDiag, cardTestScheduleCommand=cardTestScheduleCommand, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditFailedCount=auditFailedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, cardName=cardName, cardAction=cardAction, portDescription=portDescription, cardCpuType=cardCpuType, cardShelfTestIndex=cardShelfTestIndex, auditTime=auditTime, portMaxDsPower=portMaxDsPower, cardAdminState=cardAdminState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditId=auditId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPicHwVersion=cardPicHwVersion, diskVolumeTrapInh=diskVolumeTrapInh, TestResult=TestResult)
