...
FlowControlMode, DiskVolumeUsageLevel, PortType, UnknownState, AdminState, PortId, PortMode, EqActionType, SecondaryState, FirmwareSource, ShelfId, DuplexStatus, PortDetectedMode, CardType, PicType, PrimaryState, CardId, CardSubType = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "DiskVolumeUsageLevel", "PortType", "UnknownState", "AdminState", "PortId", "PortMode", "EqActionType", "SecondaryState", "FirmwareSource", "ShelfId", "DuplexStatus", "PortDetectedMode", "CardType", "PicType", "PrimaryState", "CardId", "CardSubType")
...
TruthValue, RowStatus, MacAddress, TimeStamp, DisplayString, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "MacAddress", "TimeStamp", "DisplayString", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTotalPassedCount=auditTotalPassedCount, remainInDiagMode=remainInDiagMode, cardSerialNum=cardSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, portCardSubType=portCardSubType, diskVolumeUsageLevel=diskVolumeUsageLevel, shelfNumber=shelfNumber, cardLastBootVersion=cardLastBootVersion, cardSubDetected=cardSubDetected, systemGeneral=systemGeneral, cardTempNoReportNotification=cardTempNoReportNotification, equipmentCardTestGroup=equipmentCardTestGroup, cardFpgaSource=cardFpgaSource, diskVolumeEntry=diskVolumeEntry, eqDiagConfig=eqDiagConfig, shelfName=shelfName, cardUpTime=cardUpTime, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfObjects=shelfObjects, portMaxDsPower=portMaxDsPower, portDocsIfIndex=portDocsIfIndex, portNumber=portNumber, TestTransactionId=TestTransactionId, cardShelfId=cardShelfId, cardTestSchedule=cardTestSchedule, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetInfo=cmResetInfo, cardSecStateChange=cardSecStateChange, auditId=auditId, cardFlashSize=cardFlashSize, cardTrapInh=cardTrapInh, portId=portId, cardShelfTestIndex=cardShelfTestIndex, portAction=portAction, cardHwVersion=cardHwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, fpgaErrorEventEntry=fpgaErrorEventEntry, cmResetClearNotification=cmResetClearNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeGroup=diskVolumeGroup, downstreamPowerLoss=downstreamPowerLoss, equipmentCardGroup=equipmentCardGroup, auditPassedCount=auditPassedCount, cardDplxStatus=cardDplxStatus, portDescription=portDescription, equipmentPortGroup=equipmentPortGroup, auditCycleCount=auditCycleCount, cardEntry=cardEntry, cardSpareGroupId=cardSpareGroupId, portMode=portMode, auditDescription=auditDescription, shelfSwVersion=shelfSwVersion, cardLastResetReason=cardLastResetReason, equipmentAuditGroup=equipmentAuditGroup, cardAdminState=cardAdminState, cardTestIndex=cardTestIndex, cardDetectedChange=cardDetectedChange, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, verboseLevel=verboseLevel, trapSeverity=trapSeverity, diskDriveNumber=diskDriveNumber, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cadEquipmentMibConformance=cadEquipmentMibConformance, cmResetReason=cmResetReason, cmVendor=cmVendor, portBgpId=portBgpId, cmResetNotification=cmResetNotification, cardPrStateChange=cardPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmUChannelID=cmUChannelID, TestResult=TestResult, consoleOutput=consoleOutput, cardTemperatureHighError=cardTemperatureHighError, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeDriveId=diskVolumeDriveId, cardPrState=cardPrState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCPLDVersion=cardCPLDVersion, diskVolumeId=diskVolumeId, cardRamSize=cardRamSize, cardAction=cardAction, TestCommand=TestCommand, cardCarrierFwVersion=cardCarrierFwVersion, portType=portType, cardPicHwVersion=cardPicHwVersion, cardSpareGroupMode=cardSpareGroupMode, portShelfId=portShelfId, cardTable=cardTable, portDplxStatus=portDplxStatus, equipmentTbl=equipmentTbl, cardFwVersion=cardFwVersion, cardTestType=cardTestType, cadEquipmentMibGroup=cadEquipmentMibGroup, portRxFlowControlMode=portRxFlowControlMode, diskVolumeName=diskVolumeName, auditAutoScheduling=auditAutoScheduling, cardId=cardId, auditName=auditName, auditTime=auditTime, cmResetUpTime=cmResetUpTime, portMacIfIndex=portMacIfIndex, portTxFlowControlMode=portTxFlowControlMode, equipmentState=equipmentState, cardName=cardName, diskVolumeNumber=diskVolumeNumber, portEntry=portEntry, cardTestId=cardTestId, errEvLogLevel=errEvLogLevel, portDplxStatusChange=portDplxStatusChange, equipmentError=equipmentError, cardTestDescription=cardTestDescription, TestType=TestType, portLastChangeTime=portLastChangeTime, diskVolumeCardId=diskVolumeCardId, errEvRowStatus=errEvRowStatus, cardTestEntry=cardTestEntry, diagTestId=diagTestId, cardTestResult=cardTestResult, TestScheduleCommand=TestScheduleCommand, cardTestName=cardTestName, cardTestResultDesc=cardTestResultDesc, TestSchedule=TestSchedule, PYSNMP_MODULE_ID=cadEquipmentMib, auditFailedCount=auditFailedCount, cmResetStatus=cmResetStatus, cmRegistrationNotification=cmRegistrationNotification, errEventId=errEventId, diskVolumeShelfId=diskVolumeShelfId, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvLoggingEnabled=errEvLoggingEnabled, cardPicDetected=cardPicDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentDiagGroup=equipmentDiagGroup, portPrState=portPrState, cardCarrierSerialNum=cardCarrierSerialNum, portNumChans=portNumChans, cardSecState=cardSecState, equipmentShelfGroup=equipmentShelfGroup, cardTestTable=cardTestTable, portCurrDsPower=portCurrDsPower, cardBusSpeed=cardBusSpeed, auditCardId=auditCardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portPrStateChange=portPrStateChange, cardTemperature=cardTemperature, auditTable=auditTable, auditLogOutput=auditLogOutput, auditResult=auditResult, portConnectorId=portConnectorId, trapCounter=trapCounter, portSecStateChange=portSecStateChange, portTable=portTable, diskVolumeFileName=diskVolumeFileName, equipmentAudit=equipmentAudit, diskVolumeSize=diskVolumeSize, systemClock=systemClock, cardCpuType=cardCpuType, cardSwVersion=cardSwVersion, diskVolumeTrapInh=diskVolumeTrapInh, portCardId=portCardId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDetectedMode=portDetectedMode, auditShelfId=auditShelfId, auditTotalFailedCount=auditTotalFailedCount, auditCommand=auditCommand, portMinDsPower=portMinDsPower, cardNumber=cardNumber, systemKey=systemKey, cardLastBootSource=cardLastBootSource, portMacAddress=portMacAddress, cmDevice=cmDevice, TestId=TestId, portTxFlowControlDetected=portTxFlowControlDetected, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMib=cadEquipmentMib, cardType=cardType, cardTestTime=cardTestTime, cardDplxStatusChange=cardDplxStatusChange, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResultNotification=cardTestResultNotification, equipmentDiag=equipmentDiag, cardLastChangeTime=cardLastChangeTime, portAdminState=portAdminState, cardTestScheduleCommand=cardTestScheduleCommand, cardDetected=cardDetected, cardCpuSpeed=cardCpuSpeed, cardBootVersion=cardBootVersion, cardTestTransId=cardTestTransId, cmIpAddress=cmIpAddress, systemGeneralGroup=systemGeneralGroup, portSecState=portSecState, equipmentTraps=equipmentTraps, auditLogThrottle=auditLogThrottle, auditStatus=auditStatus, cardSubType=cardSubType, cardPicSerialNum=cardPicSerialNum, fpgaErrorEventTable=fpgaErrorEventTable, portTrapInh=portTrapInh, diskVolumeTable=diskVolumeTable, auditEntry=auditEntry, equipmentStateGroup=equipmentStateGroup, cardTestCommand=cardTestCommand, cardNumPorts=cardNumPorts, cmMacAddress=cmMacAddress)
