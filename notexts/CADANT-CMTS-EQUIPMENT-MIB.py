...
FlowControlMode, PortId, DiskVolumeUsageLevel, PortMode, CardSubType, CardType, PortType, PrimaryState, ShelfId, AdminState, PortDetectedMode, CardId, DuplexStatus, UnknownState, EqActionType, PicType, FirmwareSource, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PortId", "DiskVolumeUsageLevel", "PortMode", "CardSubType", "CardType", "PortType", "PrimaryState", "ShelfId", "AdminState", "PortDetectedMode", "CardId", "DuplexStatus", "UnknownState", "EqActionType", "PicType", "FirmwareSource", "SecondaryState")
...
TextualConvention, TimeStamp, DisplayString, RowStatus, MacAddress, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TimeStamp", "DisplayString", "RowStatus", "MacAddress", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardShelfId=cardShelfId, cardTestResultNotification=cardTestResultNotification, equipmentTbl=equipmentTbl, cadEquipmentMibCompliance=cadEquipmentMibCompliance, systemGeneral=systemGeneral, portCurrDsPower=portCurrDsPower, cardSerialNum=cardSerialNum, TestId=TestId, cardSecState=cardSecState, cardId=cardId, portMode=portMode, auditFailedCount=auditFailedCount, portSecStateChange=portSecStateChange, cardNumPorts=cardNumPorts, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPicSerialNum=cardPicSerialNum, cmResetClearNotification=cmResetClearNotification, cardDetectedChange=cardDetectedChange, cardPrState=cardPrState, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestType=cardTestType, auditResult=auditResult, portRxFlowControlMode=portRxFlowControlMode, equipmentDiag=equipmentDiag, auditCommand=auditCommand, auditCycleCount=auditCycleCount, auditLogOutput=auditLogOutput, portLastChangeTime=portLastChangeTime, cardTestTransId=cardTestTransId, auditTable=auditTable, portDplxStatusChange=portDplxStatusChange, cmResetStatus=cmResetStatus, diagTestId=diagTestId, cardSubType=cardSubType, cardTemperatureHighError=cardTemperatureHighError, diskVolumeGroup=diskVolumeGroup, cardDplxStatus=cardDplxStatus, cardUpTime=cardUpTime, cardTemperature=cardTemperature, cardTestCommand=cardTestCommand, cardTestId=cardTestId, cardFwUpdateStatus=cardFwUpdateStatus, cardRamSize=cardRamSize, errEvRecoveryEnabled=errEvRecoveryEnabled, auditName=auditName, equipmentPortGroup=equipmentPortGroup, diskVolumeNumber=diskVolumeNumber, portMaxDsPower=portMaxDsPower, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditId=auditId, cardCarrierSerialNum=cardCarrierSerialNum, auditTotalPassedCount=auditTotalPassedCount, errEventId=errEventId, cardSpareGroupMode=cardSpareGroupMode, cardHwVersion=cardHwVersion, equipmentError=equipmentError, cardCpuType=cardCpuType, portShelfId=portShelfId, eqDiagConfig=eqDiagConfig, portMacAddress=portMacAddress, cardPicDetected=cardPicDetected, auditTime=auditTime, diskVolumeSize=diskVolumeSize, cardName=cardName, portCardSubType=portCardSubType, diskVolumeName=diskVolumeName, cardTestName=cardTestName, cmPrimarySID=cmPrimarySID, errEvLogLevel=errEvLogLevel, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentTraps=equipmentTraps, cardBusSpeed=cardBusSpeed, cardLastBootVersion=cardLastBootVersion, portCardId=portCardId, cmResetInfo=cmResetInfo, cadEquipmentMib=cadEquipmentMib, cmVendor=cmVendor, diskVolumeDiskSize=diskVolumeDiskSize, auditEntry=auditEntry, portBgpId=portBgpId, portMacIfIndex=portMacIfIndex, verboseLevel=verboseLevel, trapSeverity=trapSeverity, portSecState=portSecState, portDescription=portDescription, cmResetReason=cmResetReason, portTable=portTable, TestCommand=TestCommand, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, portDetectedMode=portDetectedMode, cardTestDescription=cardTestDescription, cmIpAddress=cmIpAddress, cardAction=cardAction, portId=portId, cmResetUpTime=cmResetUpTime, cardTestTime=cardTestTime, cmRegistrationNotification=cmRegistrationNotification, portDplxStatus=portDplxStatus, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portTxFlowControlMode=portTxFlowControlMode, errEvRowStatus=errEvRowStatus, portAdminState=portAdminState, auditShelfId=auditShelfId, trapCounter=trapCounter, cardTestResultDesc=cardTestResultDesc, diskVolumeFileName=diskVolumeFileName, auditTotalFailedCount=auditTotalFailedCount, cardTestResult=cardTestResult, TestScheduleCommand=TestScheduleCommand, diskVolumeId=diskVolumeId, portTxFlowControlDetected=portTxFlowControlDetected, cardTable=cardTable, systemKey=systemKey, shelfObjects=shelfObjects, cardLastBootSource=cardLastBootSource, TestSchedule=TestSchedule, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeTable=diskVolumeTable, cmUChannelID=cmUChannelID, PYSNMP_MODULE_ID=cadEquipmentMib, cardTempOverHeatNotification=cardTempOverHeatNotification, cardFlashSize=cardFlashSize, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, errEvLoggingEnabled=errEvLoggingEnabled, cardTestEntry=cardTestEntry, cardSwVersion=cardSwVersion, portNumChans=portNumChans, cardAdminState=cardAdminState, cardNumber=cardNumber, diskVolumeTrapInh=diskVolumeTrapInh, cardTempNoReportNotification=cardTempNoReportNotification, diskDriveNumber=diskDriveNumber, cardSpareGroupId=cardSpareGroupId, cardEntry=cardEntry, cardTestTable=cardTestTable, consoleOutput=consoleOutput, cardShelfTestIndex=cardShelfTestIndex, equipmentShelfGroup=equipmentShelfGroup, auditDescription=auditDescription, equipmentDiagGroup=equipmentDiagGroup, shelfName=shelfName, cardType=cardType, portEntry=portEntry, cardDetected=cardDetected, cardPicHwVersion=cardPicHwVersion, shelfNumber=shelfNumber, cardSubDetected=cardSubDetected, portAction=portAction, cardLastResetReason=cardLastResetReason, auditPassedCount=auditPassedCount, equipmentAudit=equipmentAudit, diskVolumeCardId=diskVolumeCardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardDplxStatusChange=cardDplxStatusChange, auditLogThrottle=auditLogThrottle, equipmentState=equipmentState, cardTrapInh=cardTrapInh, portTrapInh=portTrapInh, cardFwVersion=cardFwVersion, remainInDiagMode=remainInDiagMode, cardTestSchedule=cardTestSchedule, downstreamPowerLoss=downstreamPowerLoss, portType=portType, fpgaErrorEventEntry=fpgaErrorEventEntry, cmResetNotification=cmResetNotification, equipmentCardGroup=equipmentCardGroup, cardCPLDVersion=cardCPLDVersion, portPrState=portPrState, TestType=TestType, auditAutoScheduling=auditAutoScheduling, portConnectorId=portConnectorId, cmMacAddress=cmMacAddress, portMinDsPower=portMinDsPower, diskVolumeEntry=diskVolumeEntry, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsageLevel=diskVolumeUsageLevel, cardBootVersion=cardBootVersion, TestResult=TestResult, TestTransactionId=TestTransactionId, diskVolumeDriveId=diskVolumeDriveId, cardCarrierHwVersion=cardCarrierHwVersion, cardTestScheduleCommand=cardTestScheduleCommand, cardPrStateChange=cardPrStateChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCardId=auditCardId, cmDevice=cmDevice, portPrStateChange=portPrStateChange, portNumber=portNumber, diskVolumeShelfId=diskVolumeShelfId, cardCpuSpeed=cardCpuSpeed, cardFpgaSource=cardFpgaSource, cardSecStateChange=cardSecStateChange, cardTestIndex=cardTestIndex, equipmentStateGroup=equipmentStateGroup, shelfSwVersion=shelfSwVersion, portDocsIfIndex=portDocsIfIndex, equipmentAuditGroup=equipmentAuditGroup, auditStatus=auditStatus, cardLastChangeTime=cardLastChangeTime, systemClock=systemClock)
