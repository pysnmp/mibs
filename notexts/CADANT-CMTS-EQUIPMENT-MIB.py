...
SecondaryState, DuplexStatus, ShelfId, PortMode, PortType, UnknownState, CardId, CardType, PortDetectedMode, EqActionType, CardSubType, FlowControlMode, AdminState, PicType, FirmwareSource, PrimaryState, PortId, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "DuplexStatus", "ShelfId", "PortMode", "PortType", "UnknownState", "CardId", "CardType", "PortDetectedMode", "EqActionType", "CardSubType", "FlowControlMode", "AdminState", "PicType", "FirmwareSource", "PrimaryState", "PortId", "DiskVolumeUsageLevel")
...
TimeStamp, TruthValue, DateAndTime, MacAddress, TextualConvention, DisplayString, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "DateAndTime", "MacAddress", "TextualConvention", "DisplayString", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmVendor=cmVendor, cardUpTime=cardUpTime, portPrState=portPrState, cmIpAddress=cmIpAddress, downstreamPowerLoss=downstreamPowerLoss, equipmentAuditGroup=equipmentAuditGroup, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestSchedule=cardTestSchedule, portMacIfIndex=portMacIfIndex, portType=portType, cmResetNotification=cmResetNotification, portNumChans=portNumChans, auditLogOutput=auditLogOutput, cardTestTime=cardTestTime, portCurrDsPower=portCurrDsPower, cardBusSpeed=cardBusSpeed, TestScheduleCommand=TestScheduleCommand, portLastChangeTime=portLastChangeTime, systemGeneral=systemGeneral, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardEntry=cardEntry, auditTotalPassedCount=auditTotalPassedCount, diskVolumeNumber=diskVolumeNumber, portMacAddress=portMacAddress, cardSecStateChange=cardSecStateChange, cardRamSize=cardRamSize, portCardId=portCardId, auditName=auditName, portMaxDsPower=portMaxDsPower, cardSubType=cardSubType, cardTestId=cardTestId, auditPassedCount=auditPassedCount, cardFwVersion=cardFwVersion, trapCounter=trapCounter, equipmentTraps=equipmentTraps, cardSpareGroupMode=cardSpareGroupMode, auditDescription=auditDescription, diskVolumeFileName=diskVolumeFileName, cardTrapInh=cardTrapInh, cardNumPorts=cardNumPorts, verboseLevel=verboseLevel, cardCPLDVersion=cardCPLDVersion, diagTestId=diagTestId, cardDetectedChange=cardDetectedChange, cardCpuType=cardCpuType, cmResetUpTime=cmResetUpTime, cardTestTable=cardTestTable, auditTime=auditTime, equipmentCardGroup=equipmentCardGroup, cardLastChangeTime=cardLastChangeTime, portNumber=portNumber, cardTestResultNotification=cardTestResultNotification, auditStatus=auditStatus, cardDplxStatusChange=cardDplxStatusChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDplxStatus=portDplxStatus, TestType=TestType, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibConformance=cadEquipmentMibConformance, portSecStateChange=portSecStateChange, auditCommand=auditCommand, equipmentTbl=equipmentTbl, shelfSwVersion=shelfSwVersion, portEntry=portEntry, portShelfId=portShelfId, portRxFlowControlDetected=portRxFlowControlDetected, cardPrStateChange=cardPrStateChange, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeGroup=diskVolumeGroup, equipmentShelfGroup=equipmentShelfGroup, diskDriveNumber=diskDriveNumber, equipmentDiagGroup=equipmentDiagGroup, TestResult=TestResult, auditTotalFailedCount=auditTotalFailedCount, cmPrimarySID=cmPrimarySID, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portMode=portMode, cardTable=cardTable, portPrStateChange=portPrStateChange, portDocsIfIndex=portDocsIfIndex, cardSecState=cardSecState, shelfObjects=shelfObjects, cardPicHwVersion=cardPicHwVersion, shelfNumber=shelfNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSubDetected=cardSubDetected, fpgaErrorEventGroup=fpgaErrorEventGroup, portAction=portAction, cmResetInfo=cmResetInfo, cardTestResult=cardTestResult, diskVolumeSize=diskVolumeSize, cardTempNoReportNotification=cardTempNoReportNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, shelfName=shelfName, auditShelfId=auditShelfId, cardTestCommand=cardTestCommand, diskVolumeShelfId=diskVolumeShelfId, cardCarrierHwVersion=cardCarrierHwVersion, systemKey=systemKey, eqDiagConfig=eqDiagConfig, cardAdminState=cardAdminState, equipmentAudit=equipmentAudit, fpgaErrorEventTable=fpgaErrorEventTable, errEvLogLevel=errEvLogLevel, errEventId=errEventId, TestTransactionId=TestTransactionId, diskVolumeDriveId=diskVolumeDriveId, cardLastBootVersion=cardLastBootVersion, auditLogThrottle=auditLogThrottle, cardTestScheduleCommand=cardTestScheduleCommand, cmRegistrationNotification=cmRegistrationNotification, trapSeverity=trapSeverity, cmDevice=cmDevice, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPrState=cardPrState, diskVolumeTable=diskVolumeTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCpuSpeed=cardCpuSpeed, cardTestEntry=cardTestEntry, cardNumber=cardNumber, cardShelfId=cardShelfId, portDescription=portDescription, errEvRowStatus=errEvRowStatus, cardTestTransId=cardTestTransId, equipmentPortGroup=equipmentPortGroup, TestId=TestId, diskVolumeUsageLevel=diskVolumeUsageLevel, consoleOutput=consoleOutput, cardFpgaSource=cardFpgaSource, cardTestDescription=cardTestDescription, cardId=cardId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicDetected=cardPicDetected, auditEntry=auditEntry, equipmentError=equipmentError, diskVolumeName=diskVolumeName, cmResetStatus=cmResetStatus, cardTemperatureHighError=cardTemperatureHighError, auditTable=auditTable, cardAction=cardAction, auditFailedCount=auditFailedCount, diskVolumeDiskSize=diskVolumeDiskSize, cardLastResetReason=cardLastResetReason, equipmentDiag=equipmentDiag, cmResetClearNotification=cmResetClearNotification, cardSerialNum=cardSerialNum, portBgpId=portBgpId, diskVolumeCardId=diskVolumeCardId, errEvLoggingEnabled=errEvLoggingEnabled, portDplxStatusChange=portDplxStatusChange, diskVolumeEntry=diskVolumeEntry, auditResult=auditResult, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeId=diskVolumeId, cardTestResultDesc=cardTestResultDesc, cardLastBootSource=cardLastBootSource, systemGeneralGroup=systemGeneralGroup, TestCommand=TestCommand, cardCarrierFwVersion=cardCarrierFwVersion, portSecState=portSecState, cardShelfTestIndex=cardShelfTestIndex, cardFlashSize=cardFlashSize, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cadEquipmentMib=cadEquipmentMib, portTxFlowControlDetected=portTxFlowControlDetected, equipmentStateGroup=equipmentStateGroup, systemClock=systemClock, cardDetected=cardDetected, auditId=auditId, portId=portId, cardDplxStatus=cardDplxStatus, auditCardId=auditCardId, cardSpareGroupId=cardSpareGroupId, cardCarrierSerialNum=cardCarrierSerialNum, portAdminState=portAdminState, portTrapInh=portTrapInh, cardFwUpdateStatus=cardFwUpdateStatus, portDetectedMode=portDetectedMode, cardName=cardName, equipmentState=equipmentState, cmMacAddress=cmMacAddress, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetReason=cmResetReason, auditCycleCount=auditCycleCount, portTable=portTable, portConnectorId=portConnectorId, cardBootVersion=cardBootVersion, cmUChannelID=cmUChannelID, cardTemperature=cardTemperature, cardTestName=cardTestName, TestSchedule=TestSchedule, cardSwVersion=cardSwVersion, portCardSubType=portCardSubType, cardTestType=cardTestType, portRxFlowControlMode=portRxFlowControlMode, cardHwVersion=cardHwVersion, cardType=cardType, remainInDiagMode=remainInDiagMode, cardTestIndex=cardTestIndex, portMinDsPower=portMinDsPower, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, auditAutoScheduling=auditAutoScheduling)
