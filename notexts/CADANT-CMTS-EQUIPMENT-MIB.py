...
PrimaryState, PortMode, UnknownState, DuplexStatus, AdminState, FirmwareSource, SecondaryState, DiskVolumeUsageLevel, ShelfId, PortDetectedMode, CardSubType, FlowControlMode, CardType, CardId, PortType, PortId, EqActionType, PicType = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "PortMode", "UnknownState", "DuplexStatus", "AdminState", "FirmwareSource", "SecondaryState", "DiskVolumeUsageLevel", "ShelfId", "PortDetectedMode", "CardSubType", "FlowControlMode", "CardType", "CardId", "PortType", "PortId", "EqActionType", "PicType")
...
TruthValue, RowStatus, TextualConvention, DateAndTime, MacAddress, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "TextualConvention", "DateAndTime", "MacAddress", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardUpTime=cardUpTime, cmResetStatus=cmResetStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portRxFlowControlMode=portRxFlowControlMode, auditCommand=auditCommand, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmPrimarySID=cmPrimarySID, cardShelfId=cardShelfId, TestType=TestType, portCardId=portCardId, diskVolumeDiskSize=diskVolumeDiskSize, portMacAddress=portMacAddress, equipmentError=equipmentError, cardFpgaSource=cardFpgaSource, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTempNoReportNotification=cardTempNoReportNotification, cardTemperature=cardTemperature, portNumber=portNumber, trapSeverity=trapSeverity, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditName=auditName, cardNumber=cardNumber, equipmentShelfGroup=equipmentShelfGroup, cardTestScheduleCommand=cardTestScheduleCommand, cardTestSchedule=cardTestSchedule, cardTestTime=cardTestTime, cardCarrierSerialNum=cardCarrierSerialNum, TestSchedule=TestSchedule, cardFwUpdateStatus=cardFwUpdateStatus, cardTestResultDesc=cardTestResultDesc, portDocsIfIndex=portDocsIfIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestIndex=cardTestIndex, equipmentPortGroup=equipmentPortGroup, cardLastBootVersion=cardLastBootVersion, systemClock=systemClock, cmVendor=cmVendor, cardSpareGroupId=cardSpareGroupId, auditFailedCount=auditFailedCount, cardNumPorts=cardNumPorts, cardTable=cardTable, equipmentDiag=equipmentDiag, cardType=cardType, equipmentTbl=equipmentTbl, equipmentState=equipmentState, equipmentCardGroup=equipmentCardGroup, auditId=auditId, portLastChangeTime=portLastChangeTime, cardSwVersion=cardSwVersion, portId=portId, portTable=portTable, systemGeneralGroup=systemGeneralGroup, portAction=portAction, systemKey=systemKey, cardCPLDVersion=cardCPLDVersion, cardSpareGroupMode=cardSpareGroupMode, cadEquipmentMibCompliance=cadEquipmentMibCompliance, errEventId=errEventId, cardSecStateChange=cardSecStateChange, eqDiagConfig=eqDiagConfig, remainInDiagMode=remainInDiagMode, cardLastResetReason=cardLastResetReason, diskVolumeGroup=diskVolumeGroup, shelfNumber=shelfNumber, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTemperatureHighWarn=cardTemperatureHighWarn, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierFwVersion=cardCarrierFwVersion, portTxFlowControlMode=portTxFlowControlMode, portPrState=portPrState, TestTransactionId=TestTransactionId, auditDescription=auditDescription, cmRegistrationNotification=cmRegistrationNotification, equipmentStateGroup=equipmentStateGroup, diskVolumeShelfId=diskVolumeShelfId, auditLogThrottle=auditLogThrottle, auditStatus=auditStatus, TestCommand=TestCommand, trapCounter=trapCounter, cardPicHwVersion=cardPicHwVersion, cardLastBootSource=cardLastBootSource, diskVolumeUsageLevel=diskVolumeUsageLevel, verboseLevel=verboseLevel, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestId=cardTestId, portAdminState=portAdminState, errEvLoggingEnabled=errEvLoggingEnabled, equipmentTraps=equipmentTraps, diskDriveNumber=diskDriveNumber, cardCarrierHwVersion=cardCarrierHwVersion, equipmentAuditGroup=equipmentAuditGroup, portNumChans=portNumChans, portDplxStatusChange=portDplxStatusChange, errEvRowStatus=errEvRowStatus, cardTestDescription=cardTestDescription, cardSerialNum=cardSerialNum, cardFwVersion=cardFwVersion, auditShelfId=auditShelfId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPrStateChange=cardPrStateChange, equipmentDiagGroup=equipmentDiagGroup, auditCycleCount=auditCycleCount, portMode=portMode, auditResult=auditResult, portCurrDsPower=portCurrDsPower, cardId=cardId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portShelfId=portShelfId, portDetectedMode=portDetectedMode, portSecState=portSecState, TestResult=TestResult, cardLastChangeTime=cardLastChangeTime, portDplxStatus=portDplxStatus, diskVolumeDriveId=diskVolumeDriveId, cardCpuSpeed=cardCpuSpeed, cardFlashSize=cardFlashSize, portEntry=portEntry, cardEntry=cardEntry, cardDplxStatus=cardDplxStatus, cardDetectedChange=cardDetectedChange, portRxFlowControlDetected=portRxFlowControlDetected, cardName=cardName, cardSecState=cardSecState, auditTotalPassedCount=auditTotalPassedCount, portTxFlowControlDetected=portTxFlowControlDetected, cardPicDetected=cardPicDetected, cardTemperatureHighError=cardTemperatureHighError, systemGeneral=systemGeneral, cardSubType=cardSubType, cardDetected=cardDetected, cmResetClearNotification=cmResetClearNotification, PYSNMP_MODULE_ID=cadEquipmentMib, cardDplxStatusChange=cardDplxStatusChange, cardTestType=cardTestType, diskVolumeTrapInh=diskVolumeTrapInh, shelfObjects=shelfObjects, diskVolumeTable=diskVolumeTable, errEvLogLevel=errEvLogLevel, diskVolumeFileName=diskVolumeFileName, cardTestEntry=cardTestEntry, equipmentAudit=equipmentAudit, cardTestCommand=cardTestCommand, auditPassedCount=auditPassedCount, downstreamPowerLoss=downstreamPowerLoss, diskVolumeCardId=diskVolumeCardId, cmMacAddress=cmMacAddress, diskVolumeNumber=diskVolumeNumber, shelfName=shelfName, cardPicSerialNum=cardPicSerialNum, TestId=TestId, consoleOutput=consoleOutput, portDescription=portDescription, cmResetInfo=cmResetInfo, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeName=diskVolumeName, auditTable=auditTable, cardTestName=cardTestName, auditEntry=auditEntry, cardTrapInh=cardTrapInh, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardRamSize=cardRamSize, portType=portType, auditTotalFailedCount=auditTotalFailedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPrState=cardPrState, cadEquipmentMib=cadEquipmentMib, portPrStateChange=portPrStateChange, cardTestResult=cardTestResult, auditCardId=auditCardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetReason=cmResetReason, cardTestResultNotification=cardTestResultNotification, auditLogOutput=auditLogOutput, portSecStateChange=portSecStateChange, cardShelfTestIndex=cardShelfTestIndex, cmDevice=cmDevice, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetUpTime=cmResetUpTime, cmResetNotification=cmResetNotification, portBgpId=portBgpId, cardAction=cardAction, portMacIfIndex=portMacIfIndex, cardBootVersion=cardBootVersion, TestScheduleCommand=TestScheduleCommand, shelfSwVersion=shelfSwVersion, cardAdminState=cardAdminState, diskVolumeEntry=diskVolumeEntry, auditTime=auditTime, portMaxDsPower=portMaxDsPower, cardHwVersion=cardHwVersion, cardSubDetected=cardSubDetected, portConnectorId=portConnectorId, cardCpuType=cardCpuType, auditAutoScheduling=auditAutoScheduling, cardBusSpeed=cardBusSpeed, portMinDsPower=portMinDsPower, portCardSubType=portCardSubType, diskVolumeId=diskVolumeId, diagTestId=diagTestId, cmUChannelID=cmUChannelID, portTrapInh=portTrapInh, cardTestTable=cardTestTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeSize=diskVolumeSize, cardTestTransId=cardTestTransId, cmIpAddress=cmIpAddress)
