...
PortId, DiskVolumeUsageLevel, CardType, PortDetectedMode, CardSubType, FlowControlMode, PortMode, PrimaryState, ShelfId, FirmwareSource, PortType, SecondaryState, CardId, AdminState, PicType, DuplexStatus, EqActionType, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PortId", "DiskVolumeUsageLevel", "CardType", "PortDetectedMode", "CardSubType", "FlowControlMode", "PortMode", "PrimaryState", "ShelfId", "FirmwareSource", "PortType", "SecondaryState", "CardId", "AdminState", "PicType", "DuplexStatus", "EqActionType", "UnknownState")
...
DateAndTime, MacAddress, TextualConvention, RowStatus, DisplayString, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "MacAddress", "TextualConvention", "RowStatus", "DisplayString", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentState=equipmentState, cardSerialNum=cardSerialNum, shelfNumber=shelfNumber, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeTrapInh=diskVolumeTrapInh, cardTable=cardTable, cardCpuSpeed=cardCpuSpeed, portPrState=portPrState, cardTestTime=cardTestTime, cmResetClearNotification=cmResetClearNotification, diskVolumeName=diskVolumeName, portSecStateChange=portSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestId=cardTestId, TestCommand=TestCommand, auditId=auditId, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDetectedMode=portDetectedMode, cardLastBootSource=cardLastBootSource, systemKey=systemKey, auditDescription=auditDescription, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTotalFailedCount=auditTotalFailedCount, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentStateGroup=equipmentStateGroup, equipmentTraps=equipmentTraps, equipmentDiag=equipmentDiag, systemGeneralGroup=systemGeneralGroup, cardSpareGroupId=cardSpareGroupId, cardNumPorts=cardNumPorts, cmRegistrationNotification=cmRegistrationNotification, cardTempNoReportNotification=cardTempNoReportNotification, downstreamPowerLoss=downstreamPowerLoss, diskDriveNumber=diskDriveNumber, cardCarrierHwVersion=cardCarrierHwVersion, cardFlashSize=cardFlashSize, diskVolumeId=diskVolumeId, TestSchedule=TestSchedule, cmResetNotification=cmResetNotification, equipmentPortGroup=equipmentPortGroup, equipmentAuditGroup=equipmentAuditGroup, portDplxStatus=portDplxStatus, errEvLoggingEnabled=errEvLoggingEnabled, cardCarrierSerialNum=cardCarrierSerialNum, equipmentError=equipmentError, cardTemperatureHighError=cardTemperatureHighError, cmIpAddress=cmIpAddress, cardEntry=cardEntry, diskVolumeNumber=diskVolumeNumber, portAction=portAction, cardAction=cardAction, auditCommand=auditCommand, cardCPLDVersion=cardCPLDVersion, cardType=cardType, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeTable=diskVolumeTable, cmDevice=cmDevice, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCpuType=cardCpuType, portMinDsPower=portMinDsPower, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cadEquipmentMibConformance=cadEquipmentMibConformance, consoleOutput=consoleOutput, portLastChangeTime=portLastChangeTime, cmResetReason=cmResetReason, portBgpId=portBgpId, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestSchedule=cardTestSchedule, equipmentShelfGroup=equipmentShelfGroup, diskVolumeFileName=diskVolumeFileName, cardHwVersion=cardHwVersion, cardDetected=cardDetected, cmMacAddress=cmMacAddress, remainInDiagMode=remainInDiagMode, cardFwVersion=cardFwVersion, cardTestTable=cardTestTable, auditPassedCount=auditPassedCount, cardSwVersion=cardSwVersion, cardCarrierFwVersion=cardCarrierFwVersion, portShelfId=portShelfId, cardTestName=cardTestName, cardTestType=cardTestType, errEvRowStatus=errEvRowStatus, equipmentAudit=equipmentAudit, cadEquipmentMib=cadEquipmentMib, portSecState=portSecState, auditShelfId=auditShelfId, cardDplxStatus=cardDplxStatus, shelfSwVersion=shelfSwVersion, verboseLevel=verboseLevel, portTxFlowControlDetected=portTxFlowControlDetected, cardSubDetected=cardSubDetected, cardBootVersion=cardBootVersion, portTrapInh=portTrapInh, portNumber=portNumber, portRxFlowControlDetected=portRxFlowControlDetected, errEvLogLevel=errEvLogLevel, eqDiagConfig=eqDiagConfig, cardTestResultDesc=cardTestResultDesc, auditResult=auditResult, TestId=TestId, portMode=portMode, equipmentTbl=equipmentTbl, cardTestTransId=cardTestTransId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFpgaSource=cardFpgaSource, cardTestScheduleCommand=cardTestScheduleCommand, auditLogOutput=auditLogOutput, cardTrapInh=cardTrapInh, errEventId=errEventId, portAdminState=portAdminState, portMacAddress=portMacAddress, portTable=portTable, auditTotalPassedCount=auditTotalPassedCount, cardDplxStatusChange=cardDplxStatusChange, cmResetUpTime=cmResetUpTime, cardAdminState=cardAdminState, portDocsIfIndex=portDocsIfIndex, trapCounter=trapCounter, auditCardId=auditCardId, cardTestResult=cardTestResult, auditCycleCount=auditCycleCount, cardSecState=cardSecState, TestTransactionId=TestTransactionId, cardTemperature=cardTemperature, cmResetStatus=cmResetStatus, cardPrState=cardPrState, cardSecStateChange=cardSecStateChange, cardTestEntry=cardTestEntry, cardPicDetected=cardPicDetected, cmVendor=cmVendor, diskVolumeSize=diskVolumeSize, cardDetectedChange=cardDetectedChange, portDplxStatusChange=portDplxStatusChange, TestType=TestType, diskVolumeShelfId=diskVolumeShelfId, auditEntry=auditEntry, shelfObjects=shelfObjects, cardShelfTestIndex=cardShelfTestIndex, cardId=cardId, cardFwUpdateStatus=cardFwUpdateStatus, equipmentDiagGroup=equipmentDiagGroup, cardBusSpeed=cardBusSpeed, cardShelfId=cardShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditAutoScheduling=auditAutoScheduling, cardSubType=cardSubType, cardTestIndex=cardTestIndex, portPrStateChange=portPrStateChange, cardNumber=cardNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, systemClock=systemClock, diskVolumeGroup=diskVolumeGroup, portCurrDsPower=portCurrDsPower, auditName=auditName, cardLastChangeTime=cardLastChangeTime, cardPicHwVersion=cardPicHwVersion, portMacIfIndex=portMacIfIndex, trapSeverity=trapSeverity, cardSpareGroupMode=cardSpareGroupMode, portEntry=portEntry, portCardSubType=portCardSubType, TestScheduleCommand=TestScheduleCommand, diagTestId=diagTestId, portId=portId, portRxFlowControlMode=portRxFlowControlMode, portNumChans=portNumChans, diskVolumeUsageNotification=diskVolumeUsageNotification, cardLastBootVersion=cardLastBootVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestDescription=cardTestDescription, equipmentCardGroup=equipmentCardGroup, auditTime=auditTime, portConnectorId=portConnectorId, auditStatus=auditStatus, diskVolumeEntry=diskVolumeEntry, cmResetInfo=cmResetInfo, cardTestCommand=cardTestCommand, auditFailedCount=auditFailedCount, cardUpTime=cardUpTime, TestResult=TestResult, cardTemperatureHighWarn=cardTemperatureHighWarn, cardRamSize=cardRamSize, auditLogThrottle=auditLogThrottle, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfName=shelfName, portCardId=portCardId, cmPrimarySID=cmPrimarySID, portDescription=portDescription, fpgaErrorEventEntry=fpgaErrorEventEntry, portType=portType, cardName=cardName, cmUChannelID=cmUChannelID, systemGeneral=systemGeneral, diskVolumeDiskSize=diskVolumeDiskSize, portTxFlowControlMode=portTxFlowControlMode, auditTable=auditTable, fpgaErrorEventGroup=fpgaErrorEventGroup, cardPicSerialNum=cardPicSerialNum, cardLastResetReason=cardLastResetReason, portMaxDsPower=portMaxDsPower, cardPrStateChange=cardPrStateChange, cardTestResultNotification=cardTestResultNotification, diskVolumeCardId=diskVolumeCardId)
