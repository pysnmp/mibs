...
PortMode, FlowControlMode, PrimaryState, AdminState, DiskVolumeUsageLevel, CardType, CardId, PortType, ShelfId, PicType, UnknownState, FirmwareSource, CardSubType, PortDetectedMode, EqActionType, DuplexStatus, PortId, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "PortMode", "FlowControlMode", "PrimaryState", "AdminState", "DiskVolumeUsageLevel", "CardType", "CardId", "PortType", "ShelfId", "PicType", "UnknownState", "FirmwareSource", "CardSubType", "PortDetectedMode", "EqActionType", "DuplexStatus", "PortId", "SecondaryState")
...
DisplayString, TimeStamp, DateAndTime, TruthValue, MacAddress, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "DateAndTime", "TruthValue", "MacAddress", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portCardSubType=portCardSubType, portEntry=portEntry, cardShelfId=cardShelfId, cardTestName=cardTestName, portRxFlowControlDetected=portRxFlowControlDetected, errEvLogLevel=errEvLogLevel, cardCarrierFwVersion=cardCarrierFwVersion, cardTestIndex=cardTestIndex, auditResult=auditResult, portTable=portTable, auditTime=auditTime, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestResultNotification=cardTestResultNotification, equipmentShelfGroup=equipmentShelfGroup, portAction=portAction, auditAutoScheduling=auditAutoScheduling, equipmentDiagGroup=equipmentDiagGroup, shelfSwVersion=shelfSwVersion, TestResult=TestResult, cardFlashSize=cardFlashSize, cardTempNoReportNotification=cardTempNoReportNotification, portSecStateChange=portSecStateChange, downstreamPowerLoss=downstreamPowerLoss, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentState=equipmentState, cardTrapInh=cardTrapInh, auditCycleCount=auditCycleCount, errEvLoggingEnabled=errEvLoggingEnabled, cardTable=cardTable, cmResetUpTime=cmResetUpTime, cmIpAddress=cmIpAddress, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeGroup=diskVolumeGroup, portDescription=portDescription, auditId=auditId, equipmentAuditGroup=equipmentAuditGroup, cardSecStateChange=cardSecStateChange, TestType=TestType, cardFpgaSource=cardFpgaSource, cardTestResult=cardTestResult, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentTraps=equipmentTraps, auditTable=auditTable, auditCommand=auditCommand, cardPrState=cardPrState, cardType=cardType, equipmentStateGroup=equipmentStateGroup, cardEntry=cardEntry, portId=portId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, errEvRowStatus=errEvRowStatus, cmResetClearNotification=cmResetClearNotification, cardLastBootVersion=cardLastBootVersion, cardTestId=cardTestId, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardNumber=cardNumber, cardSpareGroupMode=cardSpareGroupMode, portAdminState=portAdminState, portMinDsPower=portMinDsPower, diskVolumeTable=diskVolumeTable, portDocsIfIndex=portDocsIfIndex, cardName=cardName, portMode=portMode, cardTestResultDesc=cardTestResultDesc, cardSecState=cardSecState, equipmentPortGroup=equipmentPortGroup, cardBusSpeed=cardBusSpeed, cardFwUpdateStatus=cardFwUpdateStatus, cardLastResetReason=cardLastResetReason, cardCarrierHwVersion=cardCarrierHwVersion, cardAction=cardAction, eqDiagConfig=eqDiagConfig, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmRegistrationNotification=cmRegistrationNotification, portTrapInh=portTrapInh, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardHwVersion=cardHwVersion, cardDplxStatus=cardDplxStatus, cardCpuSpeed=cardCpuSpeed, shelfName=shelfName, TestId=TestId, portTxFlowControlMode=portTxFlowControlMode, portCurrDsPower=portCurrDsPower, portTxFlowControlDetected=portTxFlowControlDetected, auditName=auditName, diskVolumeTrapInh=diskVolumeTrapInh, cardUpTime=cardUpTime, TestSchedule=TestSchedule, portNumber=portNumber, cmUChannelID=cmUChannelID, shelfNumber=shelfNumber, auditPassedCount=auditPassedCount, diskDriveNumber=diskDriveNumber, cardDplxStatusChange=cardDplxStatusChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portConnectorId=portConnectorId, diskVolumeDiskSize=diskVolumeDiskSize, trapSeverity=trapSeverity, diskVolumeCardId=diskVolumeCardId, cardDetected=cardDetected, equipmentAudit=equipmentAudit, diskVolumeId=diskVolumeId, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeNumber=diskVolumeNumber, cardLastChangeTime=cardLastChangeTime, errEventId=errEventId, portShelfId=portShelfId, cmResetNotification=cmResetNotification, cardTestTable=cardTestTable, cardRamSize=cardRamSize, portPrStateChange=portPrStateChange, cardShelfTestIndex=cardShelfTestIndex, equipmentError=equipmentError, verboseLevel=verboseLevel, cardCarrierSerialNum=cardCarrierSerialNum, trapCounter=trapCounter, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditFailedCount=auditFailedCount, cardTestCommand=cardTestCommand, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetReason=cmResetReason, TestCommand=TestCommand, cardTestScheduleCommand=cardTestScheduleCommand, cadEquipmentMib=cadEquipmentMib, portRxFlowControlMode=portRxFlowControlMode, cardCPLDVersion=cardCPLDVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeFileName=diskVolumeFileName, cardSerialNum=cardSerialNum, auditLogThrottle=auditLogThrottle, portSecState=portSecState, auditStatus=auditStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, portPrState=portPrState, diskVolumeName=diskVolumeName, cmResetInfo=cmResetInfo, equipmentCardTestGroup=equipmentCardTestGroup, cardFwVersion=cardFwVersion, portDetectedMode=portDetectedMode, cardSubDetected=cardSubDetected, cardSpareGroupId=cardSpareGroupId, systemKey=systemKey, portType=portType, cardDetectedChange=cardDetectedChange, cardPicHwVersion=cardPicHwVersion, portNumChans=portNumChans, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardCpuType=cardCpuType, cardTestSchedule=cardTestSchedule, cardId=cardId, auditLogOutput=auditLogOutput, consoleOutput=consoleOutput, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditShelfId=auditShelfId, auditTotalPassedCount=auditTotalPassedCount, cmMacAddress=cmMacAddress, diskVolumeEntry=diskVolumeEntry, cardTestTime=cardTestTime, cardLastBootSource=cardLastBootSource, cardBootVersion=cardBootVersion, portMacAddress=portMacAddress, equipmentTbl=equipmentTbl, portBgpId=portBgpId, equipmentCardGroup=equipmentCardGroup, diagTestId=diagTestId, errEvRecoveryEnabled=errEvRecoveryEnabled, auditEntry=auditEntry, cardTemperature=cardTemperature, cardTestEntry=cardTestEntry, cardTestTransId=cardTestTransId, cardTestType=cardTestType, cmVendor=cmVendor, TestTransactionId=TestTransactionId, diskVolumeShelfId=diskVolumeShelfId, equipmentDiag=equipmentDiag, auditCardId=auditCardId, portMacIfIndex=portMacIfIndex, cardPicSerialNum=cardPicSerialNum, diskVolumeDriveId=diskVolumeDriveId, cmResetStatus=cmResetStatus, cardPrStateChange=cardPrStateChange, cardTemperatureHighError=cardTemperatureHighError, cardSwVersion=cardSwVersion, cardNumPorts=cardNumPorts, TestScheduleCommand=TestScheduleCommand, cardAdminState=cardAdminState, systemClock=systemClock, shelfObjects=shelfObjects, cardSubType=cardSubType, remainInDiagMode=remainInDiagMode, cmDevice=cmDevice, systemGeneral=systemGeneral, auditTotalFailedCount=auditTotalFailedCount, portDplxStatus=portDplxStatus, cardTestDescription=cardTestDescription, portLastChangeTime=portLastChangeTime, portMaxDsPower=portMaxDsPower, portCardId=portCardId, auditDescription=auditDescription, cardPicDetected=cardPicDetected, diskVolumeSize=diskVolumeSize)
