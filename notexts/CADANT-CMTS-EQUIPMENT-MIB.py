...
FirmwareSource, PortMode, CardSubType, DiskVolumeUsageLevel, PortId, PortDetectedMode, ShelfId, EqActionType, PrimaryState, PicType, AdminState, UnknownState, CardType, SecondaryState, CardId, PortType, FlowControlMode, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "PortMode", "CardSubType", "DiskVolumeUsageLevel", "PortId", "PortDetectedMode", "ShelfId", "EqActionType", "PrimaryState", "PicType", "AdminState", "UnknownState", "CardType", "SecondaryState", "CardId", "PortType", "FlowControlMode", "DuplexStatus")
...
RowStatus, DisplayString, TextualConvention, TruthValue, MacAddress, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "TextualConvention", "TruthValue", "MacAddress", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditEntry=auditEntry, equipmentError=equipmentError, portTrapInh=portTrapInh, cardTestName=cardTestName, cardLastChangeTime=cardLastChangeTime, equipmentStateGroup=equipmentStateGroup, cmIpAddress=cmIpAddress, diskVolumeSize=diskVolumeSize, TestType=TestType, portDescription=portDescription, diskVolumeTrapInh=diskVolumeTrapInh, cardTestIndex=cardTestIndex, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeShelfId=diskVolumeShelfId, auditName=auditName, cardCpuSpeed=cardCpuSpeed, portMacIfIndex=portMacIfIndex, cadEquipmentMib=cadEquipmentMib, cardEntry=cardEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, portAdminState=portAdminState, shelfSwVersion=shelfSwVersion, auditAutoScheduling=auditAutoScheduling, portLastChangeTime=portLastChangeTime, cardSerialNum=cardSerialNum, diskVolumeFileName=diskVolumeFileName, verboseLevel=verboseLevel, cardDetected=cardDetected, cardTestSchedule=cardTestSchedule, shelfName=shelfName, auditTotalPassedCount=auditTotalPassedCount, cardBootVersion=cardBootVersion, auditLogOutput=auditLogOutput, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardLastBootSource=cardLastBootSource, cardSpareGroupMode=cardSpareGroupMode, equipmentAudit=equipmentAudit, cardDplxStatusChange=cardDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestTransactionId=TestTransactionId, cardTestType=cardTestType, cardPrStateChange=cardPrStateChange, portTxFlowControlDetected=portTxFlowControlDetected, equipmentTraps=equipmentTraps, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResultNotification=cardTestResultNotification, equipmentState=equipmentState, cardTestCommand=cardTestCommand, portMaxDsPower=portMaxDsPower, cardPicDetected=cardPicDetected, cardTempNoReportNotification=cardTempNoReportNotification, auditPassedCount=auditPassedCount, cardAdminState=cardAdminState, cmUChannelID=cmUChannelID, diskVolumeGroup=diskVolumeGroup, cardFwUpdateStatus=cardFwUpdateStatus, TestScheduleCommand=TestScheduleCommand, trapCounter=trapCounter, portSecState=portSecState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portPrStateChange=portPrStateChange, cardTestEntry=cardTestEntry, portPrState=portPrState, diskVolumeId=diskVolumeId, shelfNumber=shelfNumber, equipmentDiagGroup=equipmentDiagGroup, errEventId=errEventId, cardFwVersion=cardFwVersion, cardLastResetReason=cardLastResetReason, portRxFlowControlMode=portRxFlowControlMode, systemGeneralGroup=systemGeneralGroup, auditTotalFailedCount=auditTotalFailedCount, auditCardId=auditCardId, diagTestId=diagTestId, portType=portType, errEvLogLevel=errEvLogLevel, cardCPLDVersion=cardCPLDVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, trapSeverity=trapSeverity, equipmentTbl=equipmentTbl, fpgaErrorEventTable=fpgaErrorEventTable, cardFlashSize=cardFlashSize, systemGeneral=systemGeneral, diskVolumeUsageNotification=diskVolumeUsageNotification, cardAction=cardAction, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentCardGroup=equipmentCardGroup, cardName=cardName, cardPicSerialNum=cardPicSerialNum, portNumChans=portNumChans, errEvLoggingEnabled=errEvLoggingEnabled, cardHwVersion=cardHwVersion, cardTable=cardTable, eqDiagConfig=eqDiagConfig, cardPrState=cardPrState, cardTrapInh=cardTrapInh, cardCarrierFwVersion=cardCarrierFwVersion, cmDevice=cmDevice, downstreamPowerLoss=downstreamPowerLoss, auditCycleCount=auditCycleCount, errEvRowStatus=errEvRowStatus, cardSubDetected=cardSubDetected, cmPrimarySID=cmPrimarySID, diskVolumeDiskSize=diskVolumeDiskSize, equipmentDiag=equipmentDiag, cardNumber=cardNumber, portSecStateChange=portSecStateChange, cardTestResult=cardTestResult, cardFpgaSource=cardFpgaSource, auditTime=auditTime, diskVolumeName=diskVolumeName, diskVolumeTable=diskVolumeTable, portTxFlowControlMode=portTxFlowControlMode, cmRegistrationNotification=cmRegistrationNotification, cmResetStatus=cmResetStatus, cardShelfTestIndex=cardShelfTestIndex, auditTable=auditTable, cardRamSize=cardRamSize, cardTestId=cardTestId, TestId=TestId, shelfObjects=shelfObjects, cmResetReason=cmResetReason, portRxFlowControlDetected=portRxFlowControlDetected, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTemperatureHighError=cardTemperatureHighError, cardId=cardId, portId=portId, cardDplxStatus=cardDplxStatus, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmVendor=cmVendor, cmResetInfo=cmResetInfo, equipmentAuditGroup=equipmentAuditGroup, portAction=portAction, cardType=cardType, diskVolumeNumber=diskVolumeNumber, cardSubType=cardSubType, cardSecState=cardSecState, auditResult=auditResult, portEntry=portEntry, cardShelfId=cardShelfId, portShelfId=portShelfId, diskDriveNumber=diskDriveNumber, cardCarrierHwVersion=cardCarrierHwVersion, auditLogThrottle=auditLogThrottle, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetUpTime=cmResetUpTime, TestCommand=TestCommand, cardLastBootVersion=cardLastBootVersion, cardPicHwVersion=cardPicHwVersion, cardTestDescription=cardTestDescription, portTable=portTable, diskVolumeDriveId=diskVolumeDriveId, auditShelfId=auditShelfId, portBgpId=portBgpId, TestSchedule=TestSchedule, auditStatus=auditStatus, auditFailedCount=auditFailedCount, diskVolumeCardId=diskVolumeCardId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardDetectedChange=cardDetectedChange, cardSwVersion=cardSwVersion, consoleOutput=consoleOutput, cmResetClearNotification=cmResetClearNotification, cardBusSpeed=cardBusSpeed, cardUpTime=cardUpTime, portCardId=portCardId, portMacAddress=portMacAddress, portMode=portMode, portCurrDsPower=portCurrDsPower, cardTestTime=cardTestTime, remainInDiagMode=remainInDiagMode, cardCpuType=cardCpuType, portNumber=portNumber, equipmentShelfGroup=equipmentShelfGroup, cardTestTransId=cardTestTransId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCarrierSerialNum=cardCarrierSerialNum, cmResetNotification=cmResetNotification, auditCommand=auditCommand, portConnectorId=portConnectorId, cardTestTable=cardTestTable, portDocsIfIndex=portDocsIfIndex, auditDescription=auditDescription, cardTemperature=cardTemperature, portDplxStatusChange=portDplxStatusChange, cardSpareGroupId=cardSpareGroupId, cardTestResultDesc=cardTestResultDesc, portDplxStatus=portDplxStatus, cmMacAddress=cmMacAddress, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeEntry=diskVolumeEntry, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, portMinDsPower=portMinDsPower, portDetectedMode=portDetectedMode, equipmentCardTestGroup=equipmentCardTestGroup, TestResult=TestResult, cardSecStateChange=cardSecStateChange, auditId=auditId, portCardSubType=portCardSubType, systemKey=systemKey, systemClock=systemClock)
