...
FirmwareSource, PortId, PortDetectedMode, SecondaryState, CardType, PortType, EqActionType, ShelfId, AdminState, DuplexStatus, PicType, PortMode, CardSubType, PrimaryState, FlowControlMode, DiskVolumeUsageLevel, UnknownState, CardId = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "PortId", "PortDetectedMode", "SecondaryState", "CardType", "PortType", "EqActionType", "ShelfId", "AdminState", "DuplexStatus", "PicType", "PortMode", "CardSubType", "PrimaryState", "FlowControlMode", "DiskVolumeUsageLevel", "UnknownState", "CardId")
...
MacAddress, TruthValue, RowStatus, DisplayString, TimeStamp, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "RowStatus", "DisplayString", "TimeStamp", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portCardSubType=portCardSubType, shelfName=shelfName, equipmentTraps=equipmentTraps, cardTestType=cardTestType, portConnectorId=portConnectorId, cmMacAddress=cmMacAddress, cmUChannelID=cmUChannelID, systemClock=systemClock, equipmentStateGroup=equipmentStateGroup, cardFpgaSource=cardFpgaSource, cmResetClearNotification=cmResetClearNotification, cardSubDetected=cardSubDetected, portMacAddress=portMacAddress, cardName=cardName, cardTestResult=cardTestResult, eqDiagConfig=eqDiagConfig, cardTable=cardTable, cardSerialNum=cardSerialNum, portEntry=portEntry, consoleOutput=consoleOutput, TestCommand=TestCommand, portTxFlowControlMode=portTxFlowControlMode, cmIpAddress=cmIpAddress, equipmentAudit=equipmentAudit, shelfObjects=shelfObjects, cardUpTime=cardUpTime, cardTemperatureHighWarn=cardTemperatureHighWarn, portDocsIfIndex=portDocsIfIndex, cmResetInfo=cmResetInfo, errEvLogLevel=errEvLogLevel, portBgpId=portBgpId, equipmentTbl=equipmentTbl, cardFwVersion=cardFwVersion, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardPicDetected=cardPicDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, cmResetNotification=cmResetNotification, auditCardId=auditCardId, cardTestDescription=cardTestDescription, cardHwVersion=cardHwVersion, cardPicSerialNum=cardPicSerialNum, cardTestIndex=cardTestIndex, systemGeneral=systemGeneral, cmPrimarySID=cmPrimarySID, cardBootVersion=cardBootVersion, cardDetected=cardDetected, portMinDsPower=portMinDsPower, cadEquipmentMib=cadEquipmentMib, diskVolumeShelfId=diskVolumeShelfId, shelfSwVersion=shelfSwVersion, cardTestSchedule=cardTestSchedule, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardFwUpdateStatus=cardFwUpdateStatus, cardTestId=cardTestId, trapSeverity=trapSeverity, errEventId=errEventId, diskDriveNumber=diskDriveNumber, cmDevice=cmDevice, equipmentCardGroup=equipmentCardGroup, auditName=auditName, cardCarrierHwVersion=cardCarrierHwVersion, TestType=TestType, auditDescription=auditDescription, cardRamSize=cardRamSize, equipmentDiagGroup=equipmentDiagGroup, auditId=auditId, cardLastBootSource=cardLastBootSource, diskVolumeId=diskVolumeId, cardLastChangeTime=cardLastChangeTime, equipmentAuditGroup=equipmentAuditGroup, portMaxDsPower=portMaxDsPower, systemKey=systemKey, cardPicHwVersion=cardPicHwVersion, verboseLevel=verboseLevel, errEvLoggingEnabled=errEvLoggingEnabled, portDplxStatus=portDplxStatus, portSecState=portSecState, equipmentPortGroup=equipmentPortGroup, auditAutoScheduling=auditAutoScheduling, equipmentDiag=equipmentDiag, cadEquipmentMibConformance=cadEquipmentMibConformance, cardPrState=cardPrState, portAction=portAction, cardSubType=cardSubType, diskVolumeNumber=diskVolumeNumber, cardLastBootVersion=cardLastBootVersion, auditPassedCount=auditPassedCount, TestSchedule=TestSchedule, cardNumPorts=cardNumPorts, cardTemperatureHighError=cardTemperatureHighError, portLastChangeTime=portLastChangeTime, TestResult=TestResult, cardDplxStatusChange=cardDplxStatusChange, diskVolumeSize=diskVolumeSize, portRxFlowControlMode=portRxFlowControlMode, portTable=portTable, cmResetStatus=cmResetStatus, TestId=TestId, diskVolumeDriveId=diskVolumeDriveId, cardCarrierSerialNum=cardCarrierSerialNum, cmVendor=cmVendor, cardSwVersion=cardSwVersion, cardTestCommand=cardTestCommand, auditEntry=auditEntry, cardLastResetReason=cardLastResetReason, equipmentState=equipmentState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardEntry=cardEntry, cardTemperature=cardTemperature, auditTotalFailedCount=auditTotalFailedCount, portShelfId=portShelfId, equipmentError=equipmentError, cardPrStateChange=cardPrStateChange, portDescription=portDescription, auditFailedCount=auditFailedCount, errEvRowStatus=errEvRowStatus, diskVolumeDiskSize=diskVolumeDiskSize, portType=portType, cardTestTime=cardTestTime, fpgaErrorEventEntry=fpgaErrorEventEntry, auditTime=auditTime, portCurrDsPower=portCurrDsPower, diskVolumeGroup=diskVolumeGroup, portMacIfIndex=portMacIfIndex, diskVolumeTable=diskVolumeTable, auditTable=auditTable, cardTestTable=cardTestTable, cardId=cardId, diskVolumeFileName=diskVolumeFileName, cardCarrierFwVersion=cardCarrierFwVersion, auditLogOutput=auditLogOutput, shelfNumber=shelfNumber, portNumChans=portNumChans, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diagTestId=diagTestId, downstreamPowerLoss=downstreamPowerLoss, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResultDesc=cardTestResultDesc, portTrapInh=portTrapInh, cardSecStateChange=cardSecStateChange, systemGeneralGroup=systemGeneralGroup, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentCardTestGroup=equipmentCardTestGroup, PYSNMP_MODULE_ID=cadEquipmentMib, cardAction=cardAction, cardCpuType=cardCpuType, cmResetUpTime=cmResetUpTime, cardDetectedChange=cardDetectedChange, diskVolumeCardId=diskVolumeCardId, cardType=cardType, portNumber=portNumber, cardSpareGroupMode=cardSpareGroupMode, portDplxStatusChange=portDplxStatusChange, cardSpareGroupId=cardSpareGroupId, cardNumber=cardNumber, auditShelfId=auditShelfId, cardShelfId=cardShelfId, cardCPLDVersion=cardCPLDVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeEntry=diskVolumeEntry, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardBusSpeed=cardBusSpeed, auditResult=auditResult, portDetectedMode=portDetectedMode, portAdminState=portAdminState, cardTrapInh=cardTrapInh, cardFlashSize=cardFlashSize, fpgaErrorEventGroup=fpgaErrorEventGroup, cardShelfTestIndex=cardShelfTestIndex, TestScheduleCommand=TestScheduleCommand, portMode=portMode, cardDplxStatus=cardDplxStatus, remainInDiagMode=remainInDiagMode, TestTransactionId=TestTransactionId, cardAdminState=cardAdminState, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCpuSpeed=cardCpuSpeed, auditTotalPassedCount=auditTotalPassedCount, cmResetReason=cmResetReason, equipmentShelfGroup=equipmentShelfGroup, cmRegistrationNotification=cmRegistrationNotification, auditCommand=auditCommand, portSecStateChange=portSecStateChange, portPrStateChange=portPrStateChange, cardTestTransId=cardTestTransId, trapCounter=trapCounter, auditCycleCount=auditCycleCount, cardSecState=cardSecState, portPrState=portPrState, portCardId=portCardId, diskVolumeName=diskVolumeName, cardTestEntry=cardTestEntry, auditStatus=auditStatus, cardTempNoReportNotification=cardTempNoReportNotification, cardTestName=cardTestName, portId=portId, auditLogThrottle=auditLogThrottle)
