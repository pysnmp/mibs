...
SecondaryState, DuplexStatus, PortId, EqActionType, PortDetectedMode, PrimaryState, CardType, PicType, UnknownState, CardId, FirmwareSource, DiskVolumeUsageLevel, CardSubType, PortType, AdminState, ShelfId, FlowControlMode, PortMode = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "DuplexStatus", "PortId", "EqActionType", "PortDetectedMode", "PrimaryState", "CardType", "PicType", "UnknownState", "CardId", "FirmwareSource", "DiskVolumeUsageLevel", "CardSubType", "PortType", "AdminState", "ShelfId", "FlowControlMode", "PortMode")
...
MacAddress, TextualConvention, DisplayString, TimeStamp, RowStatus, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TextualConvention", "DisplayString", "TimeStamp", "RowStatus", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResultDesc=cardTestResultDesc, cardEntry=cardEntry, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditEntry=auditEntry, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditCardId=auditCardId, portBgpId=portBgpId, equipmentDiagGroup=equipmentDiagGroup, PYSNMP_MODULE_ID=cadEquipmentMib, shelfName=shelfName, cardFwVersion=cardFwVersion, cmResetClearNotification=cmResetClearNotification, remainInDiagMode=remainInDiagMode, portDplxStatusChange=portDplxStatusChange, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentPortGroup=equipmentPortGroup, auditLogThrottle=auditLogThrottle, cmResetNotification=cmResetNotification, equipmentTraps=equipmentTraps, fpgaErrorEventGroup=fpgaErrorEventGroup, portSecState=portSecState, portMacIfIndex=portMacIfIndex, equipmentState=equipmentState, systemGeneralGroup=systemGeneralGroup, TestTransactionId=TestTransactionId, cardDplxStatus=cardDplxStatus, cardFpgaSource=cardFpgaSource, diskVolumeShelfId=diskVolumeShelfId, cardTestResult=cardTestResult, auditCommand=auditCommand, errEventId=errEventId, cardLastBootSource=cardLastBootSource, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portCurrDsPower=portCurrDsPower, cmUChannelID=cmUChannelID, portMode=portMode, portId=portId, systemGeneral=systemGeneral, auditAutoScheduling=auditAutoScheduling, cardRamSize=cardRamSize, downstreamPowerLoss=downstreamPowerLoss, cardPicDetected=cardPicDetected, portMinDsPower=portMinDsPower, auditTable=auditTable, cardSecStateChange=cardSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSwVersion=cardSwVersion, cardShelfId=cardShelfId, cardSerialNum=cardSerialNum, cardAction=cardAction, cardCPLDVersion=cardCPLDVersion, cardFlashSize=cardFlashSize, cardNumPorts=cardNumPorts, diskVolumeSize=diskVolumeSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestType=cardTestType, portAction=portAction, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditLogOutput=auditLogOutput, cardCarrierSerialNum=cardCarrierSerialNum, auditShelfId=auditShelfId, cardTempNoReportNotification=cardTempNoReportNotification, portLastChangeTime=portLastChangeTime, cardDplxStatusChange=cardDplxStatusChange, auditDescription=auditDescription, cardSubType=cardSubType, portPrState=portPrState, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPrStateChange=cardPrStateChange, TestResult=TestResult, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestIndex=cardTestIndex, cardTestId=cardTestId, cardBootVersion=cardBootVersion, cardPicHwVersion=cardPicHwVersion, portRxFlowControlMode=portRxFlowControlMode, portConnectorId=portConnectorId, diskVolumeName=diskVolumeName, cardTestResultNotification=cardTestResultNotification, cardDetected=cardDetected, portTxFlowControlDetected=portTxFlowControlDetected, TestId=TestId, cardTrapInh=cardTrapInh, shelfSwVersion=shelfSwVersion, portDescription=portDescription, portMaxDsPower=portMaxDsPower, trapCounter=trapCounter, portShelfId=portShelfId, cardCpuType=cardCpuType, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventEntry=fpgaErrorEventEntry, cardType=cardType, cardSubDetected=cardSubDetected, portCardSubType=portCardSubType, cmDevice=cmDevice, cmMacAddress=cmMacAddress, cmRegistrationNotification=cmRegistrationNotification, fpgaErrorEventTable=fpgaErrorEventTable, cardTestTable=cardTestTable, consoleOutput=consoleOutput, portMacAddress=portMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, auditCycleCount=auditCycleCount, diskVolumeGroup=diskVolumeGroup, portDplxStatus=portDplxStatus, cardName=cardName, equipmentShelfGroup=equipmentShelfGroup, cardSecState=cardSecState, equipmentTbl=equipmentTbl, cardId=cardId, TestType=TestType, trapSeverity=trapSeverity, cardTemperature=cardTemperature, errEvRowStatus=errEvRowStatus, cardFwUpdateStatus=cardFwUpdateStatus, diagTestId=diagTestId, portType=portType, cardCarrierFwVersion=cardCarrierFwVersion, auditName=auditName, errEvLoggingEnabled=errEvLoggingEnabled, cardCpuSpeed=cardCpuSpeed, diskVolumeFileName=diskVolumeFileName, portDetectedMode=portDetectedMode, cardSpareGroupMode=cardSpareGroupMode, cardTable=cardTable, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTemperatureHighError=cardTemperatureHighError, portTable=portTable, auditTime=auditTime, shelfNumber=shelfNumber, portNumber=portNumber, verboseLevel=verboseLevel, cardTestDescription=cardTestDescription, eqDiagConfig=eqDiagConfig, portNumChans=portNumChans, auditId=auditId, auditTotalPassedCount=auditTotalPassedCount, cmResetUpTime=cmResetUpTime, diskVolumeDriveId=diskVolumeDriveId, cardTestTime=cardTestTime, portTxFlowControlMode=portTxFlowControlMode, cmIpAddress=cmIpAddress, cardDetectedChange=cardDetectedChange, cardLastChangeTime=cardLastChangeTime, cardTestEntry=cardTestEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, cardLastBootVersion=cardLastBootVersion, auditFailedCount=auditFailedCount, equipmentError=equipmentError, errEvLogLevel=errEvLogLevel, auditTotalFailedCount=auditTotalFailedCount, cardBusSpeed=cardBusSpeed, portPrStateChange=portPrStateChange, diskVolumeId=diskVolumeId, equipmentAuditGroup=equipmentAuditGroup, equipmentStateGroup=equipmentStateGroup, cardCarrierHwVersion=cardCarrierHwVersion, portCardId=portCardId, cardTestSchedule=cardTestSchedule, portDocsIfIndex=portDocsIfIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, portAdminState=portAdminState, cardLastResetReason=cardLastResetReason, cmPrimarySID=cmPrimarySID, cardUpTime=cardUpTime, portTrapInh=portTrapInh, cardTestCommand=cardTestCommand, cardHwVersion=cardHwVersion, portEntry=portEntry, TestCommand=TestCommand, equipmentAudit=equipmentAudit, auditStatus=auditStatus, portSecStateChange=portSecStateChange, TestSchedule=TestSchedule, systemClock=systemClock, cmResetStatus=cmResetStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, TestScheduleCommand=TestScheduleCommand, cardShelfTestIndex=cardShelfTestIndex, cmResetInfo=cmResetInfo, equipmentDiag=equipmentDiag, cmResetReason=cmResetReason, cardPrState=cardPrState, cardAdminState=cardAdminState, diskVolumeNumber=diskVolumeNumber, systemKey=systemKey, auditResult=auditResult, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeTable=diskVolumeTable, cmVendor=cmVendor, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeCardId=diskVolumeCardId, equipmentCardGroup=equipmentCardGroup, cardNumber=cardNumber, cardTestTransId=cardTestTransId, diskVolumeTrapInh=diskVolumeTrapInh, auditPassedCount=auditPassedCount, shelfObjects=shelfObjects, cardSpareGroupId=cardSpareGroupId, diskDriveNumber=diskDriveNumber, cardTestName=cardTestName)
