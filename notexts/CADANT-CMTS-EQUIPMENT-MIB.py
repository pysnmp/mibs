...
CardId, PortId, ShelfId, PortDetectedMode, PortMode, SecondaryState, CardType, UnknownState, DiskVolumeUsageLevel, EqActionType, CardSubType, FirmwareSource, FlowControlMode, AdminState, PortType, PicType, PrimaryState, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortId", "ShelfId", "PortDetectedMode", "PortMode", "SecondaryState", "CardType", "UnknownState", "DiskVolumeUsageLevel", "EqActionType", "CardSubType", "FirmwareSource", "FlowControlMode", "AdminState", "PortType", "PicType", "PrimaryState", "DuplexStatus")
...
TimeStamp, RowStatus, TextualConvention, TruthValue, MacAddress, DisplayString, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "RowStatus", "TextualConvention", "TruthValue", "MacAddress", "DisplayString", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", eqDiagConfig=eqDiagConfig, portRxFlowControlMode=portRxFlowControlMode, cmResetNotification=cmResetNotification, portNumChans=portNumChans, portMacAddress=portMacAddress, auditResult=auditResult, cardDplxStatusChange=cardDplxStatusChange, errEventId=errEventId, equipmentTbl=equipmentTbl, portConnectorId=portConnectorId, cardTestDescription=cardTestDescription, cardSubType=cardSubType, portPrStateChange=portPrStateChange, cardDetected=cardDetected, equipmentAudit=equipmentAudit, equipmentError=equipmentError, cardPrStateChange=cardPrStateChange, cardTestIndex=cardTestIndex, equipmentCardGroup=equipmentCardGroup, portNumber=portNumber, cardFwVersion=cardFwVersion, cardType=cardType, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeGroup=diskVolumeGroup, cmMacAddress=cmMacAddress, portSecState=portSecState, cmDevice=cmDevice, portMaxDsPower=portMaxDsPower, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestSchedule=TestSchedule, portMode=portMode, diagTestId=diagTestId, cardTestTransId=cardTestTransId, downstreamPowerLoss=downstreamPowerLoss, cardLastBootSource=cardLastBootSource, cardTestType=cardTestType, portAdminState=portAdminState, cardDplxStatus=cardDplxStatus, cardPicHwVersion=cardPicHwVersion, diskVolumeName=diskVolumeName, auditAutoScheduling=auditAutoScheduling, cardTestResultDesc=cardTestResultDesc, equipmentCardTestGroup=equipmentCardTestGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, TestId=TestId, auditCommand=auditCommand, diskDriveNumber=diskDriveNumber, portDetectedMode=portDetectedMode, diskVolumeId=diskVolumeId, auditLogOutput=auditLogOutput, portId=portId, TestCommand=TestCommand, auditCycleCount=auditCycleCount, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentState=equipmentState, cardEntry=cardEntry, TestTransactionId=TestTransactionId, cardTestName=cardTestName, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeCardId=diskVolumeCardId, portDescription=portDescription, cardTestTable=cardTestTable, auditCardId=auditCardId, auditTotalPassedCount=auditTotalPassedCount, shelfNumber=shelfNumber, auditTime=auditTime, diskVolumeSize=diskVolumeSize, cadEquipmentMibConformance=cadEquipmentMibConformance, portMinDsPower=portMinDsPower, TestResult=TestResult, auditStatus=auditStatus, cardCPLDVersion=cardCPLDVersion, cardFpgaSource=cardFpgaSource, cardPrState=cardPrState, cardAction=cardAction, diskVolumeEntry=diskVolumeEntry, errEvRowStatus=errEvRowStatus, auditFailedCount=auditFailedCount, equipmentTraps=equipmentTraps, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetClearNotification=cmResetClearNotification, cardNumPorts=cardNumPorts, cadEquipmentMib=cadEquipmentMib, cardDetectedChange=cardDetectedChange, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, verboseLevel=verboseLevel, cardTestEntry=cardTestEntry, cardSwVersion=cardSwVersion, cardSerialNum=cardSerialNum, cardTemperatureHighError=cardTemperatureHighError, shelfSwVersion=shelfSwVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardBusSpeed=cardBusSpeed, systemKey=systemKey, portTable=portTable, auditEntry=auditEntry, diskVolumeDriveId=diskVolumeDriveId, cardCpuSpeed=cardCpuSpeed, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestCommand=cardTestCommand, shelfObjects=shelfObjects, cmResetStatus=cmResetStatus, systemClock=systemClock, cardName=cardName, auditDescription=auditDescription, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, TestScheduleCommand=TestScheduleCommand, cardNumber=cardNumber, cardTestTime=cardTestTime, cmVendor=cmVendor, errEvLogLevel=errEvLogLevel, cardTempNoReportNotification=cardTempNoReportNotification, fpgaErrorEventTable=fpgaErrorEventTable, portShelfId=portShelfId, consoleOutput=consoleOutput, cardLastChangeTime=cardLastChangeTime, cardFlashSize=cardFlashSize, portEntry=portEntry, trapSeverity=trapSeverity, cardSecState=cardSecState, equipmentDiag=equipmentDiag, cmResetUpTime=cmResetUpTime, portAction=portAction, portLastChangeTime=portLastChangeTime, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSubDetected=cardSubDetected, shelfName=shelfName, cardTestSchedule=cardTestSchedule, equipmentDiagGroup=equipmentDiagGroup, cardSecStateChange=cardSecStateChange, diskVolumeDiskSize=diskVolumeDiskSize, cardTable=cardTable, cardTemperature=cardTemperature, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeNumber=diskVolumeNumber, TestType=TestType, auditId=auditId, portPrState=portPrState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portType=portType, diskVolumeShelfId=diskVolumeShelfId, systemGeneralGroup=systemGeneralGroup, cardSpareGroupId=cardSpareGroupId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeTable=diskVolumeTable, cardId=cardId, cardHwVersion=cardHwVersion, portDocsIfIndex=portDocsIfIndex, cmPrimarySID=cmPrimarySID, cardTestResult=cardTestResult, portTrapInh=portTrapInh, cardAdminState=cardAdminState, cardFwUpdateStatus=cardFwUpdateStatus, cardCpuType=cardCpuType, cardPicDetected=cardPicDetected, equipmentShelfGroup=equipmentShelfGroup, cmResetReason=cmResetReason, fpgaErrorEventGroup=fpgaErrorEventGroup, cardBootVersion=cardBootVersion, diskVolumeFileName=diskVolumeFileName, cardUpTime=cardUpTime, portCardSubType=portCardSubType, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetInfo=cmResetInfo, cardTrapInh=cardTrapInh, auditPassedCount=auditPassedCount, remainInDiagMode=remainInDiagMode, cardTemperatureHighWarn=cardTemperatureHighWarn, portCardId=portCardId, diskVolumeTrapInh=diskVolumeTrapInh, portCurrDsPower=portCurrDsPower, equipmentPortGroup=equipmentPortGroup, auditName=auditName, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, trapCounter=trapCounter, equipmentAuditGroup=equipmentAuditGroup, portBgpId=portBgpId, portTxFlowControlMode=portTxFlowControlMode, portMacIfIndex=portMacIfIndex, auditShelfId=auditShelfId, systemGeneral=systemGeneral, cardShelfTestIndex=cardShelfTestIndex, cardSpareGroupMode=cardSpareGroupMode, cmIpAddress=cmIpAddress, cardCarrierSerialNum=cardCarrierSerialNum, auditLogThrottle=auditLogThrottle, cardShelfId=cardShelfId, portSecStateChange=portSecStateChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicSerialNum=cardPicSerialNum, auditTable=auditTable, cardLastResetReason=cardLastResetReason, cardTestId=cardTestId, cmUChannelID=cmUChannelID, equipmentStateGroup=equipmentStateGroup, cardTestResultNotification=cardTestResultNotification, cardRamSize=cardRamSize, portDplxStatus=portDplxStatus, portDplxStatusChange=portDplxStatusChange, auditTotalFailedCount=auditTotalFailedCount, errEvLoggingEnabled=errEvLoggingEnabled)
