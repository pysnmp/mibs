...
ShelfId, CardSubType, PortMode, PortDetectedMode, SecondaryState, FirmwareSource, EqActionType, PortId, DiskVolumeUsageLevel, AdminState, PortType, UnknownState, CardId, PicType, CardType, FlowControlMode, PrimaryState, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "CardSubType", "PortMode", "PortDetectedMode", "SecondaryState", "FirmwareSource", "EqActionType", "PortId", "DiskVolumeUsageLevel", "AdminState", "PortType", "UnknownState", "CardId", "PicType", "CardType", "FlowControlMode", "PrimaryState", "DuplexStatus")
...
DateAndTime, DisplayString, RowStatus, TruthValue, TextualConvention, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "DisplayString", "RowStatus", "TruthValue", "TextualConvention", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestScheduleCommand=TestScheduleCommand, cardAdminState=cardAdminState, portId=portId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditFailedCount=auditFailedCount, cardDplxStatus=cardDplxStatus, cardLastChangeTime=cardLastChangeTime, portEntry=portEntry, cardSpareGroupId=cardSpareGroupId, cardTestEntry=cardTestEntry, auditCommand=auditCommand, cardFpgaSource=cardFpgaSource, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentState=equipmentState, cardSpareGroupMode=cardSpareGroupMode, equipmentError=equipmentError, cardTempNoReportNotification=cardTempNoReportNotification, portNumber=portNumber, portBgpId=portBgpId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCarrierSerialNum=cardCarrierSerialNum, cmResetUpTime=cmResetUpTime, auditName=auditName, cardTrapInh=cardTrapInh, cardTestId=cardTestId, cardLastBootSource=cardLastBootSource, portCurrDsPower=portCurrDsPower, verboseLevel=verboseLevel, cmIpAddress=cmIpAddress, cardTable=cardTable, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResult=cardTestResult, cmVendor=cmVendor, auditTime=auditTime, cardShelfTestIndex=cardShelfTestIndex, equipmentAuditGroup=equipmentAuditGroup, TestResult=TestResult, errEvRowStatus=errEvRowStatus, TestType=TestType, auditLogOutput=auditLogOutput, portMinDsPower=portMinDsPower, portAdminState=portAdminState, errEventId=errEventId, portCardId=portCardId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestDescription=cardTestDescription, equipmentDiag=equipmentDiag, cardLastBootVersion=cardLastBootVersion, auditShelfId=auditShelfId, cardTestType=cardTestType, auditTotalFailedCount=auditTotalFailedCount, systemGeneral=systemGeneral, cardTemperatureHighError=cardTemperatureHighError, cardCpuSpeed=cardCpuSpeed, cardFwVersion=cardFwVersion, equipmentCardTestGroup=equipmentCardTestGroup, auditPassedCount=auditPassedCount, cardTestScheduleCommand=cardTestScheduleCommand, cardTemperature=cardTemperature, systemKey=systemKey, equipmentStateGroup=equipmentStateGroup, equipmentTraps=equipmentTraps, TestId=TestId, equipmentCardGroup=equipmentCardGroup, cardUpTime=cardUpTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmUChannelID=cmUChannelID, portAction=portAction, portMacIfIndex=portMacIfIndex, cardTestName=cardTestName, equipmentShelfGroup=equipmentShelfGroup, auditResult=auditResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portPrStateChange=portPrStateChange, cmRegistrationNotification=cmRegistrationNotification, equipmentDiagGroup=equipmentDiagGroup, cardTestResultDesc=cardTestResultDesc, cardName=cardName, cardDplxStatusChange=cardDplxStatusChange, trapSeverity=trapSeverity, portMaxDsPower=portMaxDsPower, auditAutoScheduling=auditAutoScheduling, diskVolumeFileName=diskVolumeFileName, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portShelfId=portShelfId, diskVolumeCardId=diskVolumeCardId, cardCPLDVersion=cardCPLDVersion, cardPrStateChange=cardPrStateChange, cardDetected=cardDetected, portRxFlowControlMode=portRxFlowControlMode, cmPrimarySID=cmPrimarySID, cadEquipmentMibConformance=cadEquipmentMibConformance, cardCarrierFwVersion=cardCarrierFwVersion, cardCpuType=cardCpuType, portConnectorId=portConnectorId, cardTestTable=cardTestTable, cardFlashSize=cardFlashSize, auditEntry=auditEntry, auditCardId=auditCardId, diskVolumeNumber=diskVolumeNumber, cadEquipmentMib=cadEquipmentMib, portSecStateChange=portSecStateChange, portMacAddress=portMacAddress, TestCommand=TestCommand, systemClock=systemClock, cardRamSize=cardRamSize, auditId=auditId, cardSecStateChange=cardSecStateChange, cardSwVersion=cardSwVersion, equipmentPortGroup=equipmentPortGroup, cardSecState=cardSecState, diskVolumeEntry=diskVolumeEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetReason=cmResetReason, cardNumPorts=cardNumPorts, portTrapInh=portTrapInh, cardId=cardId, cardHwVersion=cardHwVersion, cmResetNotification=cmResetNotification, portType=portType, portTable=portTable, auditDescription=auditDescription, diskVolumeGroup=diskVolumeGroup, portDetectedMode=portDetectedMode, cardBootVersion=cardBootVersion, cardPicHwVersion=cardPicHwVersion, cardSubDetected=cardSubDetected, portTxFlowControlDetected=portTxFlowControlDetected, portRxFlowControlDetected=portRxFlowControlDetected, shelfName=shelfName, systemGeneralGroup=systemGeneralGroup, cardBusSpeed=cardBusSpeed, shelfNumber=shelfNumber, remainInDiagMode=remainInDiagMode, diskVolumeId=diskVolumeId, portDplxStatusChange=portDplxStatusChange, diskVolumeTable=diskVolumeTable, eqDiagConfig=eqDiagConfig, diskVolumeUsageLevel=diskVolumeUsageLevel, errEvLogLevel=errEvLogLevel, portMode=portMode, portLastChangeTime=portLastChangeTime, cardCarrierHwVersion=cardCarrierHwVersion, downstreamPowerLoss=downstreamPowerLoss, cardTestIndex=cardTestIndex, cardDetectedChange=cardDetectedChange, cmResetClearNotification=cmResetClearNotification, cardPicSerialNum=cardPicSerialNum, diskVolumeName=diskVolumeName, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeDiskSize=diskVolumeDiskSize, portPrState=portPrState, cardPrState=cardPrState, portCardSubType=portCardSubType, cardTestTransId=cardTestTransId, cmDevice=cmDevice, cmResetStatus=cmResetStatus, auditLogThrottle=auditLogThrottle, diskVolumeSize=diskVolumeSize, cardTestResultNotification=cardTestResultNotification, portSecState=portSecState, diagTestId=diagTestId, portTxFlowControlMode=portTxFlowControlMode, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetInfo=cmResetInfo, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSubType=cardSubType, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTrapInh=diskVolumeTrapInh, cardAction=cardAction, portDplxStatus=portDplxStatus, equipmentAudit=equipmentAudit, cardTestCommand=cardTestCommand, cardEntry=cardEntry, equipmentTbl=equipmentTbl, shelfObjects=shelfObjects, cardTempOverHeatNotification=cardTempOverHeatNotification, auditTable=auditTable, TestSchedule=TestSchedule, cardTestSchedule=cardTestSchedule, portDescription=portDescription, diskDriveNumber=diskDriveNumber, TestTransactionId=TestTransactionId, cardSerialNum=cardSerialNum, diskVolumeShelfId=diskVolumeShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, portDocsIfIndex=portDocsIfIndex, portNumChans=portNumChans, cardLastResetReason=cardLastResetReason, diskVolumeDriveId=diskVolumeDriveId, cardNumber=cardNumber, shelfSwVersion=shelfSwVersion, auditCycleCount=auditCycleCount, cardTestTime=cardTestTime, auditStatus=auditStatus, trapCounter=trapCounter, cardShelfId=cardShelfId, auditTotalPassedCount=auditTotalPassedCount, cmMacAddress=cmMacAddress, consoleOutput=consoleOutput, cardType=cardType, cardPicDetected=cardPicDetected, fpgaErrorEventTable=fpgaErrorEventTable)
