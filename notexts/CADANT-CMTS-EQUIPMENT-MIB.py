...
CardId, PortMode, PicType, PortId, UnknownState, SecondaryState, FlowControlMode, PortType, DuplexStatus, DiskVolumeUsageLevel, CardSubType, PrimaryState, EqActionType, ShelfId, FirmwareSource, PortDetectedMode, CardType, AdminState = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortMode", "PicType", "PortId", "UnknownState", "SecondaryState", "FlowControlMode", "PortType", "DuplexStatus", "DiskVolumeUsageLevel", "CardSubType", "PrimaryState", "EqActionType", "ShelfId", "FirmwareSource", "PortDetectedMode", "CardType", "AdminState")
...
TimeStamp, TruthValue, RowStatus, TextualConvention, MacAddress, DisplayString, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "RowStatus", "TextualConvention", "MacAddress", "DisplayString", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditResult=auditResult, cadEquipmentMib=cadEquipmentMib, cardName=cardName, cardPicDetected=cardPicDetected, cardId=cardId, portDescription=portDescription, diskVolumeDriveId=diskVolumeDriveId, cardTestTable=cardTestTable, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDplxStatusChange=portDplxStatusChange, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmUChannelID=cmUChannelID, diskVolumeGroup=diskVolumeGroup, remainInDiagMode=remainInDiagMode, cardEntry=cardEntry, portShelfId=portShelfId, cardCarrierSerialNum=cardCarrierSerialNum, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentCardGroup=equipmentCardGroup, cmResetUpTime=cmResetUpTime, cardCarrierFwVersion=cardCarrierFwVersion, cardTestType=cardTestType, TestScheduleCommand=TestScheduleCommand, diskVolumeNumber=diskVolumeNumber, cardTable=cardTable, diskVolumeShelfId=diskVolumeShelfId, portDplxStatus=portDplxStatus, cardSubType=cardSubType, portMinDsPower=portMinDsPower, auditTime=auditTime, cardSpareGroupMode=cardSpareGroupMode, cardCPLDVersion=cardCPLDVersion, cardTemperature=cardTemperature, auditFailedCount=auditFailedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeId=diskVolumeId, diskVolumeName=diskVolumeName, equipmentState=equipmentState, cardFpgaSource=cardFpgaSource, cardCarrierHwVersion=cardCarrierHwVersion, verboseLevel=verboseLevel, cardDetectedChange=cardDetectedChange, cardBootVersion=cardBootVersion, portPrStateChange=portPrStateChange, cardTestEntry=cardTestEntry, auditShelfId=auditShelfId, cardAction=cardAction, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDiskSize=diskVolumeDiskSize, portRxFlowControlDetected=portRxFlowControlDetected, errEvLoggingEnabled=errEvLoggingEnabled, cardCpuSpeed=cardCpuSpeed, portType=portType, cmResetNotification=cmResetNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestScheduleCommand=cardTestScheduleCommand, cardType=cardType, cardTestCommand=cardTestCommand, portTxFlowControlMode=portTxFlowControlMode, auditName=auditName, cardRamSize=cardRamSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetInfo=cmResetInfo, portCardSubType=portCardSubType, errEvRowStatus=errEvRowStatus, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, cardPicHwVersion=cardPicHwVersion, cardSecStateChange=cardSecStateChange, equipmentAudit=equipmentAudit, auditLogOutput=auditLogOutput, downstreamPowerLoss=downstreamPowerLoss, portEntry=portEntry, systemGeneralGroup=systemGeneralGroup, portNumChans=portNumChans, diskVolumeUsageNotification=diskVolumeUsageNotification, auditTotalFailedCount=auditTotalFailedCount, fpgaErrorEventTable=fpgaErrorEventTable, portAction=portAction, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDplxStatusChange=cardDplxStatusChange, equipmentAuditGroup=equipmentAuditGroup, trapSeverity=trapSeverity, auditId=auditId, cardPrStateChange=cardPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, portTable=portTable, cardSwVersion=cardSwVersion, eqDiagConfig=eqDiagConfig, auditStatus=auditStatus, TestId=TestId, auditCardId=auditCardId, cardTemperatureHighError=cardTemperatureHighError, portId=portId, auditLogThrottle=auditLogThrottle, cardTestResultDesc=cardTestResultDesc, equipmentTraps=equipmentTraps, auditDescription=auditDescription, auditEntry=auditEntry, diskVolumeTable=diskVolumeTable, diskVolumeSize=diskVolumeSize, cardHwVersion=cardHwVersion, auditCycleCount=auditCycleCount, portBgpId=portBgpId, portCurrDsPower=portCurrDsPower, cardNumPorts=cardNumPorts, diskVolumeTrapInh=diskVolumeTrapInh, portPrState=portPrState, cardTempNoReportNotification=cardTempNoReportNotification, systemKey=systemKey, equipmentError=equipmentError, cmDevice=cmDevice, cardDetected=cardDetected, TestType=TestType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestSchedule=cardTestSchedule, cardBusSpeed=cardBusSpeed, portDocsIfIndex=portDocsIfIndex, portMode=portMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmPrimarySID=cmPrimarySID, cmResetClearNotification=cmResetClearNotification, cardTrapInh=cardTrapInh, cardNumber=cardNumber, cardTestId=cardTestId, diskVolumeFileName=diskVolumeFileName, cardLastResetReason=cardLastResetReason, cardTestTime=cardTestTime, TestResult=TestResult, auditTable=auditTable, TestCommand=TestCommand, portCardId=portCardId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portAdminState=portAdminState, portTxFlowControlDetected=portTxFlowControlDetected, systemClock=systemClock, cardPicSerialNum=cardPicSerialNum, auditTotalPassedCount=auditTotalPassedCount, portNumber=portNumber, portLastChangeTime=portLastChangeTime, shelfName=shelfName, cardTestName=cardTestName, cardLastBootVersion=cardLastBootVersion, shelfSwVersion=shelfSwVersion, cardPrState=cardPrState, cardTestTransId=cardTestTransId, cardUpTime=cardUpTime, consoleOutput=consoleOutput, trapCounter=trapCounter, portRxFlowControlMode=portRxFlowControlMode, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmRegistrationNotification=cmRegistrationNotification, cardTestDescription=cardTestDescription, diskDriveNumber=diskDriveNumber, cardLastBootSource=cardLastBootSource, portConnectorId=portConnectorId, cardSubDetected=cardSubDetected, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentDiag=equipmentDiag, errEvRecoveryEnabled=errEvRecoveryEnabled, TestSchedule=TestSchedule, equipmentStateGroup=equipmentStateGroup, cardSecState=cardSecState, cardTestIndex=cardTestIndex, cardLastChangeTime=cardLastChangeTime, errEventId=errEventId, portSecStateChange=portSecStateChange, cardDplxStatus=cardDplxStatus, portDetectedMode=portDetectedMode, portMaxDsPower=portMaxDsPower, cardSpareGroupId=cardSpareGroupId, errEvLogLevel=errEvLogLevel, cardTestResult=cardTestResult, portTrapInh=portTrapInh, equipmentTbl=equipmentTbl, cardAdminState=cardAdminState, auditCommand=auditCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, cmIpAddress=cmIpAddress, TestTransactionId=TestTransactionId, cmResetStatus=cmResetStatus, shelfObjects=shelfObjects, cardFlashSize=cardFlashSize, systemGeneral=systemGeneral, diskVolumeEntry=diskVolumeEntry, cmVendor=cmVendor, auditPassedCount=auditPassedCount, cardFwVersion=cardFwVersion, auditAutoScheduling=auditAutoScheduling, portMacAddress=portMacAddress, cardSerialNum=cardSerialNum, cardTestResultNotification=cardTestResultNotification, cmMacAddress=cmMacAddress, diagTestId=diagTestId, shelfNumber=shelfNumber, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuType=cardCpuType, cmResetReason=cmResetReason, diskVolumeCardId=diskVolumeCardId, cardShelfId=cardShelfId, portSecState=portSecState, portMacIfIndex=portMacIfIndex, cardShelfTestIndex=cardShelfTestIndex)
