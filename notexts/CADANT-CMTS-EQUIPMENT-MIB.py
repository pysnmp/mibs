...
CardId, PortId, AdminState, CardType, UnknownState, PrimaryState, DuplexStatus, ShelfId, EqActionType, DiskVolumeUsageLevel, PortMode, FlowControlMode, PortDetectedMode, PortType, CardSubType, SecondaryState, PicType, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortId", "AdminState", "CardType", "UnknownState", "PrimaryState", "DuplexStatus", "ShelfId", "EqActionType", "DiskVolumeUsageLevel", "PortMode", "FlowControlMode", "PortDetectedMode", "PortType", "CardSubType", "SecondaryState", "PicType", "FirmwareSource")
...
MacAddress, RowStatus, DateAndTime, TimeStamp, TextualConvention, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "DateAndTime", "TimeStamp", "TextualConvention", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentTraps=equipmentTraps, equipmentCardGroup=equipmentCardGroup, TestResult=TestResult, downstreamPowerLoss=downstreamPowerLoss, cardCarrierFwVersion=cardCarrierFwVersion, portId=portId, diskDriveNumber=diskDriveNumber, cmIpAddress=cmIpAddress, cardLastBootSource=cardLastBootSource, portRxFlowControlDetected=portRxFlowControlDetected, auditEntry=auditEntry, equipmentPortGroup=equipmentPortGroup, auditAutoScheduling=auditAutoScheduling, diskVolumeCardId=diskVolumeCardId, cardTemperature=cardTemperature, systemGeneral=systemGeneral, cmResetNotification=cmResetNotification, cardCpuType=cardCpuType, diskVolumeTable=diskVolumeTable, diskVolumeNumber=diskVolumeNumber, diskVolumeId=diskVolumeId, cardTestCommand=cardTestCommand, equipmentAudit=equipmentAudit, cardDplxStatusChange=cardDplxStatusChange, cardUpTime=cardUpTime, cadEquipmentMibGroup=cadEquipmentMibGroup, TestId=TestId, auditCommand=auditCommand, trapSeverity=trapSeverity, shelfObjects=shelfObjects, cardShelfId=cardShelfId, portMacIfIndex=portMacIfIndex, cmResetUpTime=cmResetUpTime, auditTotalFailedCount=auditTotalFailedCount, TestTransactionId=TestTransactionId, equipmentTbl=equipmentTbl, cardType=cardType, cardCPLDVersion=cardCPLDVersion, portPrStateChange=portPrStateChange, diskVolumeShelfId=diskVolumeShelfId, auditStatus=auditStatus, trapCounter=trapCounter, cardSubDetected=cardSubDetected, portRxFlowControlMode=portRxFlowControlMode, portDplxStatus=portDplxStatus, consoleOutput=consoleOutput, cardSerialNum=cardSerialNum, cardSecStateChange=cardSecStateChange, cardTrapInh=cardTrapInh, equipmentStateGroup=equipmentStateGroup, auditTotalPassedCount=auditTotalPassedCount, cmResetClearNotification=cmResetClearNotification, auditPassedCount=auditPassedCount, cardBootVersion=cardBootVersion, cardFlashSize=cardFlashSize, cardFpgaSource=cardFpgaSource, shelfNumber=shelfNumber, cmResetReason=cmResetReason, cardName=cardName, cadEquipmentMibConformance=cadEquipmentMibConformance, systemKey=systemKey, shelfName=shelfName, portMode=portMode, cardTestName=cardTestName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditLogThrottle=auditLogThrottle, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSwVersion=cardSwVersion, cardDetected=cardDetected, cardBusSpeed=cardBusSpeed, errEvLoggingEnabled=errEvLoggingEnabled, cardTestDescription=cardTestDescription, portTxFlowControlDetected=portTxFlowControlDetected, systemGeneralGroup=systemGeneralGroup, PYSNMP_MODULE_ID=cadEquipmentMib, portSecStateChange=portSecStateChange, cardTestResultNotification=cardTestResultNotification, cardTemperatureHighError=cardTemperatureHighError, portType=portType, cardCarrierHwVersion=cardCarrierHwVersion, portNumChans=portNumChans, portBgpId=portBgpId, cmPrimarySID=cmPrimarySID, cardTempOverHeatNotification=cardTempOverHeatNotification, cardDetectedChange=cardDetectedChange, portMaxDsPower=portMaxDsPower, cmResetStatus=cmResetStatus, cardId=cardId, errEvRowStatus=errEvRowStatus, portLastChangeTime=portLastChangeTime, portSecState=portSecState, eqDiagConfig=eqDiagConfig, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeFileName=diskVolumeFileName, diskVolumeEntry=diskVolumeEntry, auditCycleCount=auditCycleCount, remainInDiagMode=remainInDiagMode, cardTestScheduleCommand=cardTestScheduleCommand, cardTable=cardTable, cardCarrierSerialNum=cardCarrierSerialNum, cardFwUpdateStatus=cardFwUpdateStatus, cmUChannelID=cmUChannelID, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPrState=cardPrState, cardTemperatureHighWarn=cardTemperatureHighWarn, cardShelfTestIndex=cardShelfTestIndex, diskVolumeDiskSize=diskVolumeDiskSize, auditCardId=auditCardId, cardLastResetReason=cardLastResetReason, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestId=cardTestId, portConnectorId=portConnectorId, portShelfId=portShelfId, cardLastChangeTime=cardLastChangeTime, cardSpareGroupMode=cardSpareGroupMode, portMinDsPower=portMinDsPower, cardHwVersion=cardHwVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeName=diskVolumeName, systemClock=systemClock, portDocsIfIndex=portDocsIfIndex, equipmentDiagGroup=equipmentDiagGroup, portCurrDsPower=portCurrDsPower, diskVolumeGroup=diskVolumeGroup, cardNumber=cardNumber, portAdminState=portAdminState, errEvLogLevel=errEvLogLevel, portNumber=portNumber, equipmentDiag=equipmentDiag, portPrState=portPrState, verboseLevel=verboseLevel, cardPicDetected=cardPicDetected, auditId=auditId, TestCommand=TestCommand, cardTestTransId=cardTestTransId, cardLastBootVersion=cardLastBootVersion, cadEquipmentMib=cadEquipmentMib, cardSubType=cardSubType, cardAdminState=cardAdminState, diskVolumeDriveId=diskVolumeDriveId, diskVolumeSize=diskVolumeSize, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardPicHwVersion=cardPicHwVersion, portTxFlowControlMode=portTxFlowControlMode, cmVendor=cmVendor, cmDevice=cmDevice, portTrapInh=portTrapInh, fpgaErrorEventEntry=fpgaErrorEventEntry, cardAction=cardAction, cardDplxStatus=cardDplxStatus, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditLogOutput=auditLogOutput, auditTime=auditTime, portDetectedMode=portDetectedMode, cardTestEntry=cardTestEntry, cardPrStateChange=cardPrStateChange, cardTestTable=cardTestTable, auditTable=auditTable, cardFwVersion=cardFwVersion, portAction=portAction, auditShelfId=auditShelfId, portEntry=portEntry, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmRegistrationNotification=cmRegistrationNotification, errEventId=errEventId, equipmentCardTestGroup=equipmentCardTestGroup, TestType=TestType, cardTestTime=cardTestTime, TestScheduleCommand=TestScheduleCommand, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmMacAddress=cmMacAddress, equipmentState=equipmentState, cmResetInfo=cmResetInfo, portDplxStatusChange=portDplxStatusChange, portCardId=portCardId, cardPicSerialNum=cardPicSerialNum, auditDescription=auditDescription, equipmentError=equipmentError, diagTestId=diagTestId, shelfSwVersion=shelfSwVersion, cardCpuSpeed=cardCpuSpeed, cardTestResultDesc=cardTestResultDesc, fpgaErrorEventTable=fpgaErrorEventTable, cardRamSize=cardRamSize, fpgaErrorEventGroup=fpgaErrorEventGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardNumPorts=cardNumPorts, auditFailedCount=auditFailedCount, cardTestType=cardTestType, portDescription=portDescription, cardTestIndex=cardTestIndex, cardTempNoReportNotification=cardTempNoReportNotification, cardEntry=cardEntry, auditResult=auditResult, cardSpareGroupId=cardSpareGroupId, cardTestSchedule=cardTestSchedule, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSecState=cardSecState, equipmentAuditGroup=equipmentAuditGroup, TestSchedule=TestSchedule, portMacAddress=portMacAddress, cardTestResult=cardTestResult, auditName=auditName, portTable=portTable, portCardSubType=portCardSubType)
