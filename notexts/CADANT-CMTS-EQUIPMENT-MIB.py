...
AdminState, PicType, ShelfId, PortType, PrimaryState, FlowControlMode, EqActionType, DuplexStatus, PortDetectedMode, CardSubType, CardType, UnknownState, PortMode, SecondaryState, CardId, FirmwareSource, PortId, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PicType", "ShelfId", "PortType", "PrimaryState", "FlowControlMode", "EqActionType", "DuplexStatus", "PortDetectedMode", "CardSubType", "CardType", "UnknownState", "PortMode", "SecondaryState", "CardId", "FirmwareSource", "PortId", "DiskVolumeUsageLevel")
...
TextualConvention, MacAddress, TruthValue, TimeStamp, DisplayString, DateAndTime, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "TruthValue", "TimeStamp", "DisplayString", "DateAndTime", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardType=cardType, cardUpTime=cardUpTime, cardTestCommand=cardTestCommand, cardShelfTestIndex=cardShelfTestIndex, cmUChannelID=cmUChannelID, cardNumPorts=cardNumPorts, diskVolumeTrapInh=diskVolumeTrapInh, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFlashSize=cardFlashSize, cardTestSchedule=cardTestSchedule, eqDiagConfig=eqDiagConfig, equipmentError=equipmentError, cardSubDetected=cardSubDetected, auditLogOutput=auditLogOutput, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeFileName=diskVolumeFileName, cardBusSpeed=cardBusSpeed, cardTestId=cardTestId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPicDetected=cardPicDetected, portEntry=portEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSpareGroupId=cardSpareGroupId, diagTestId=diagTestId, diskVolumeGroup=diskVolumeGroup, portCardId=portCardId, equipmentShelfGroup=equipmentShelfGroup, cardTemperatureHighError=cardTemperatureHighError, auditEntry=auditEntry, portTable=portTable, portDocsIfIndex=portDocsIfIndex, errEventId=errEventId, portSecStateChange=portSecStateChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestTransactionId=TestTransactionId, auditLogThrottle=auditLogThrottle, auditFailedCount=auditFailedCount, cardAction=cardAction, fpgaErrorEventTable=fpgaErrorEventTable, cardSecStateChange=cardSecStateChange, TestType=TestType, TestScheduleCommand=TestScheduleCommand, portRxFlowControlMode=portRxFlowControlMode, portDescription=portDescription, auditTotalPassedCount=auditTotalPassedCount, shelfSwVersion=shelfSwVersion, portPrState=portPrState, cardLastResetReason=cardLastResetReason, errEvLoggingEnabled=errEvLoggingEnabled, cardTestScheduleCommand=cardTestScheduleCommand, cardFwVersion=cardFwVersion, portDplxStatusChange=portDplxStatusChange, auditTable=auditTable, portNumChans=portNumChans, cardId=cardId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCarrierSerialNum=cardCarrierSerialNum, verboseLevel=verboseLevel, shelfNumber=shelfNumber, equipmentTbl=equipmentTbl, cardLastBootVersion=cardLastBootVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTxFlowControlDetected=portTxFlowControlDetected, cardCarrierFwVersion=cardCarrierFwVersion, cardSwVersion=cardSwVersion, cardSecState=cardSecState, cardTestType=cardTestType, cardEntry=cardEntry, cardPrState=cardPrState, cardSpareGroupMode=cardSpareGroupMode, shelfName=shelfName, cmVendor=cmVendor, diskDriveNumber=diskDriveNumber, portShelfId=portShelfId, cadEquipmentMib=cadEquipmentMib, cardPicHwVersion=cardPicHwVersion, remainInDiagMode=remainInDiagMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestName=cardTestName, cmResetInfo=cmResetInfo, equipmentState=equipmentState, cardCpuType=cardCpuType, auditCycleCount=auditCycleCount, downstreamPowerLoss=downstreamPowerLoss, auditPassedCount=auditPassedCount, auditStatus=auditStatus, portLastChangeTime=portLastChangeTime, trapSeverity=trapSeverity, diskVolumeId=diskVolumeId, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeNumber=diskVolumeNumber, errEvRowStatus=errEvRowStatus, cmResetClearNotification=cmResetClearNotification, cardTable=cardTable, diskVolumeSize=diskVolumeSize, auditTotalFailedCount=auditTotalFailedCount, cardTestResultDesc=cardTestResultDesc, cmMacAddress=cmMacAddress, cardCPLDVersion=cardCPLDVersion, PYSNMP_MODULE_ID=cadEquipmentMib, trapCounter=trapCounter, cardSerialNum=cardSerialNum, cardTestResult=cardTestResult, cardTemperatureHighWarn=cardTemperatureHighWarn, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibGroup=cadEquipmentMibGroup, cardDetected=cardDetected, systemKey=systemKey, cardDplxStatus=cardDplxStatus, diskVolumeCardId=diskVolumeCardId, auditCardId=auditCardId, portMaxDsPower=portMaxDsPower, TestResult=TestResult, equipmentAuditGroup=equipmentAuditGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardTrapInh=cardTrapInh, portMinDsPower=portMinDsPower, shelfObjects=shelfObjects, cardTempNoReportNotification=cardTempNoReportNotification, cmResetStatus=cmResetStatus, cardLastChangeTime=cardLastChangeTime, cardCarrierHwVersion=cardCarrierHwVersion, portType=portType, cardTestIndex=cardTestIndex, systemClock=systemClock, cardDplxStatusChange=cardDplxStatusChange, diskVolumeShelfId=diskVolumeShelfId, cardTestEntry=cardTestEntry, portMacIfIndex=portMacIfIndex, auditId=auditId, portCurrDsPower=portCurrDsPower, cmPrimarySID=cmPrimarySID, cardHwVersion=cardHwVersion, equipmentTraps=equipmentTraps, fpgaErrorEventEntry=fpgaErrorEventEntry, cardBootVersion=cardBootVersion, portAction=portAction, portSecState=portSecState, auditResult=auditResult, consoleOutput=consoleOutput, cardTestTime=cardTestTime, portConnectorId=portConnectorId, diskVolumeEntry=diskVolumeEntry, cmRegistrationNotification=cmRegistrationNotification, systemGeneral=systemGeneral, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portAdminState=portAdminState, cardTestDescription=cardTestDescription, equipmentCardGroup=equipmentCardGroup, cardAdminState=cardAdminState, equipmentDiag=equipmentDiag, cardDetectedChange=cardDetectedChange, cardShelfId=cardShelfId, portId=portId, portTrapInh=portTrapInh, cardSubType=cardSubType, portPrStateChange=portPrStateChange, TestSchedule=TestSchedule, portMacAddress=portMacAddress, diskVolumeDriveId=diskVolumeDriveId, cmResetNotification=cmResetNotification, equipmentStateGroup=equipmentStateGroup, portBgpId=portBgpId, portDetectedMode=portDetectedMode, auditAutoScheduling=auditAutoScheduling, cardFwUpdateStatus=cardFwUpdateStatus, portTxFlowControlMode=portTxFlowControlMode, equipmentPortGroup=equipmentPortGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestResultNotification=cardTestResultNotification, cardFpgaSource=cardFpgaSource, cardCpuSpeed=cardCpuSpeed, portCardSubType=portCardSubType, cmDevice=cmDevice, fpgaErrorEventGroup=fpgaErrorEventGroup, portMode=portMode, portDplxStatus=portDplxStatus, cardName=cardName, cardRamSize=cardRamSize, TestId=TestId, auditShelfId=auditShelfId, cmIpAddress=cmIpAddress, auditDescription=auditDescription, errEvLogLevel=errEvLogLevel, equipmentAudit=equipmentAudit, cardTestTable=cardTestTable, equipmentDiagGroup=equipmentDiagGroup, auditName=auditName, cardNumber=cardNumber, cmResetUpTime=cmResetUpTime, cardTemperature=cardTemperature, TestCommand=TestCommand, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetReason=cmResetReason, diskVolumeTable=diskVolumeTable, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeName=diskVolumeName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portNumber=portNumber, auditCommand=auditCommand, cardPrStateChange=cardPrStateChange, cardLastBootSource=cardLastBootSource, cadEquipmentMibConformance=cadEquipmentMibConformance, systemGeneralGroup=systemGeneralGroup, auditTime=auditTime, cardTestTransId=cardTestTransId)
