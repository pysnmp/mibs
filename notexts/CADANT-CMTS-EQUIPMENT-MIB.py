...
CardId, UnknownState, FlowControlMode, DuplexStatus, PortMode, CardSubType, FirmwareSource, DiskVolumeUsageLevel, CardType, AdminState, PortDetectedMode, PortType, EqActionType, SecondaryState, PrimaryState, PicType, ShelfId, PortId = mibBuilder.importSymbols("CADANT-TC", "CardId", "UnknownState", "FlowControlMode", "DuplexStatus", "PortMode", "CardSubType", "FirmwareSource", "DiskVolumeUsageLevel", "CardType", "AdminState", "PortDetectedMode", "PortType", "EqActionType", "SecondaryState", "PrimaryState", "PicType", "ShelfId", "PortId")
...
TimeStamp, DisplayString, RowStatus, DateAndTime, TruthValue, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DisplayString", "RowStatus", "DateAndTime", "TruthValue", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestType=TestType, equipmentPortGroup=equipmentPortGroup, cardSecState=cardSecState, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetInfo=cmResetInfo, fpgaErrorEventEntry=fpgaErrorEventEntry, portNumChans=portNumChans, cardBusSpeed=cardBusSpeed, cmResetUpTime=cmResetUpTime, TestId=TestId, cardDplxStatus=cardDplxStatus, equipmentDiagGroup=equipmentDiagGroup, cardLastResetReason=cardLastResetReason, portSecState=portSecState, equipmentAudit=equipmentAudit, cardPrState=cardPrState, cardEntry=cardEntry, shelfObjects=shelfObjects, portPrState=portPrState, cadEquipmentMib=cadEquipmentMib, cmResetStatus=cmResetStatus, portCardSubType=portCardSubType, cardCpuSpeed=cardCpuSpeed, cardPicHwVersion=cardPicHwVersion, consoleOutput=consoleOutput, equipmentError=equipmentError, cardTestResultDesc=cardTestResultDesc, cardTrapInh=cardTrapInh, cardTempOverHeatNotification=cardTempOverHeatNotification, cardDetectedChange=cardDetectedChange, cmRegistrationNotification=cmRegistrationNotification, diskVolumeName=diskVolumeName, cardNumPorts=cardNumPorts, cardTestResultNotification=cardTestResultNotification, downstreamPowerLoss=downstreamPowerLoss, cardCPLDVersion=cardCPLDVersion, verboseLevel=verboseLevel, portCardId=portCardId, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardBootVersion=cardBootVersion, errEvRowStatus=errEvRowStatus, remainInDiagMode=remainInDiagMode, trapSeverity=trapSeverity, auditLogThrottle=auditLogThrottle, cardFwVersion=cardFwVersion, cardTemperature=cardTemperature, cardFpgaSource=cardFpgaSource, fpgaErrorEventTable=fpgaErrorEventTable, systemClock=systemClock, cardPicDetected=cardPicDetected, eqDiagConfig=eqDiagConfig, portPrStateChange=portPrStateChange, portBgpId=portBgpId, diskVolumeUsageLevel=diskVolumeUsageLevel, errEvLogLevel=errEvLogLevel, portAdminState=portAdminState, portMaxDsPower=portMaxDsPower, diskVolumeTrapInh=diskVolumeTrapInh, auditDescription=auditDescription, errEvRecoveryEnabled=errEvRecoveryEnabled, auditFailedCount=auditFailedCount, portConnectorId=portConnectorId, equipmentCardGroup=equipmentCardGroup, diskDriveNumber=diskDriveNumber, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestId=cardTestId, diagTestId=diagTestId, auditTotalFailedCount=auditTotalFailedCount, auditCardId=auditCardId, cmIpAddress=cmIpAddress, cardRamSize=cardRamSize, cardId=cardId, cardFwUpdateStatus=cardFwUpdateStatus, portMacAddress=portMacAddress, TestCommand=TestCommand, cardTestTransId=cardTestTransId, cardTestCommand=cardTestCommand, TestScheduleCommand=TestScheduleCommand, equipmentTraps=equipmentTraps, cardLastBootVersion=cardLastBootVersion, TestTransactionId=TestTransactionId, cardTestSchedule=cardTestSchedule, portTxFlowControlMode=portTxFlowControlMode, portMacIfIndex=portMacIfIndex, auditPassedCount=auditPassedCount, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCarrierHwVersion=cardCarrierHwVersion, portId=portId, auditName=auditName, diskVolumeGroup=diskVolumeGroup, diskVolumeDriveId=diskVolumeDriveId, cmDevice=cmDevice, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardShelfTestIndex=cardShelfTestIndex, cardCarrierSerialNum=cardCarrierSerialNum, cardUpTime=cardUpTime, cmUChannelID=cmUChannelID, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetReason=cmResetReason, auditShelfId=auditShelfId, cardTestType=cardTestType, diskVolumeNumber=diskVolumeNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentTbl=equipmentTbl, cardFlashSize=cardFlashSize, portType=portType, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestIndex=cardTestIndex, portDescription=portDescription, portNumber=portNumber, portTable=portTable, auditTotalPassedCount=auditTotalPassedCount, cardShelfId=cardShelfId, systemKey=systemKey, portRxFlowControlDetected=portRxFlowControlDetected, portDplxStatus=portDplxStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, portAction=portAction, cardTestName=cardTestName, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentDiag=equipmentDiag, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfNumber=shelfNumber, TestSchedule=TestSchedule, cardTestDescription=cardTestDescription, cmVendor=cmVendor, diskVolumeTable=diskVolumeTable, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDocsIfIndex=portDocsIfIndex, portDetectedMode=portDetectedMode, diskVolumeId=diskVolumeId, cardSpareGroupMode=cardSpareGroupMode, portDplxStatusChange=portDplxStatusChange, cardSecStateChange=cardSecStateChange, cardAdminState=cardAdminState, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetNotification=cmResetNotification, cmPrimarySID=cmPrimarySID, auditCommand=auditCommand, cardDetected=cardDetected, cardPrStateChange=cardPrStateChange, cardHwVersion=cardHwVersion, cardTable=cardTable, cardTestEntry=cardTestEntry, cardName=cardName, portShelfId=portShelfId, auditAutoScheduling=auditAutoScheduling, diskVolumeEntry=diskVolumeEntry, diskVolumeCardId=diskVolumeCardId, cardTestScheduleCommand=cardTestScheduleCommand, cardSubDetected=cardSubDetected, equipmentStateGroup=equipmentStateGroup, diskVolumeSize=diskVolumeSize, errEvLoggingEnabled=errEvLoggingEnabled, portMinDsPower=portMinDsPower, auditCycleCount=auditCycleCount, cardLastChangeTime=cardLastChangeTime, auditTable=auditTable, portMode=portMode, portRxFlowControlMode=portRxFlowControlMode, cardDplxStatusChange=cardDplxStatusChange, cardType=cardType, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTemperatureHighError=cardTemperatureHighError, auditEntry=auditEntry, portSecStateChange=portSecStateChange, portLastChangeTime=portLastChangeTime, portEntry=portEntry, portCurrDsPower=portCurrDsPower, cardSerialNum=cardSerialNum, auditResult=auditResult, equipmentState=equipmentState, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditLogOutput=auditLogOutput, auditId=auditId, systemGeneralGroup=systemGeneralGroup, portTrapInh=portTrapInh, diskVolumeShelfId=diskVolumeShelfId, cardSubType=cardSubType, cardTempNoReportNotification=cardTempNoReportNotification, cardTestTable=cardTestTable, auditTime=auditTime, diskVolumeFileName=diskVolumeFileName, shelfSwVersion=shelfSwVersion, cardNumber=cardNumber, shelfName=shelfName, auditStatus=auditStatus, errEventId=errEventId, trapCounter=trapCounter, cardSwVersion=cardSwVersion, TestResult=TestResult, systemGeneral=systemGeneral, cardLastBootSource=cardLastBootSource, cmMacAddress=cmMacAddress, equipmentShelfGroup=equipmentShelfGroup, equipmentAuditGroup=equipmentAuditGroup, cardTestResult=cardTestResult, cardAction=cardAction, cmResetClearNotification=cmResetClearNotification, cardTestTime=cardTestTime, cardCpuType=cardCpuType)
