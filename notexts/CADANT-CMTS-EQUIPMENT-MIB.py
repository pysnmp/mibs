...
PortType, CardId, ShelfId, PortId, FirmwareSource, FlowControlMode, PicType, PortMode, AdminState, DuplexStatus, DiskVolumeUsageLevel, SecondaryState, EqActionType, PrimaryState, UnknownState, PortDetectedMode, CardSubType, CardType = mibBuilder.importSymbols("CADANT-TC", "PortType", "CardId", "ShelfId", "PortId", "FirmwareSource", "FlowControlMode", "PicType", "PortMode", "AdminState", "DuplexStatus", "DiskVolumeUsageLevel", "SecondaryState", "EqActionType", "PrimaryState", "UnknownState", "PortDetectedMode", "CardSubType", "CardType")
...
DateAndTime, TruthValue, TextualConvention, RowStatus, MacAddress, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TruthValue", "TextualConvention", "RowStatus", "MacAddress", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeTable=diskVolumeTable, portId=portId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, downstreamPowerLoss=downstreamPowerLoss, portSecState=portSecState, cardPicDetected=cardPicDetected, cardShelfId=cardShelfId, consoleOutput=consoleOutput, cardTestName=cardTestName, equipmentAuditGroup=equipmentAuditGroup, cardDplxStatusChange=cardDplxStatusChange, cardTestEntry=cardTestEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSecState=cardSecState, cardTestIndex=cardTestIndex, cardFwVersion=cardFwVersion, shelfSwVersion=shelfSwVersion, auditPassedCount=auditPassedCount, portMode=portMode, auditId=auditId, errEvRecoveryEnabled=errEvRecoveryEnabled, systemClock=systemClock, trapSeverity=trapSeverity, portMinDsPower=portMinDsPower, auditLogOutput=auditLogOutput, cardPicHwVersion=cardPicHwVersion, cardAdminState=cardAdminState, cardTestResult=cardTestResult, cardTestType=cardTestType, cardTestTransId=cardTestTransId, fpgaErrorEventTable=fpgaErrorEventTable, auditStatus=auditStatus, equipmentDiag=equipmentDiag, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentState=equipmentState, shelfObjects=shelfObjects, cardCpuType=cardCpuType, remainInDiagMode=remainInDiagMode, equipmentCardTestGroup=equipmentCardTestGroup, cardPrStateChange=cardPrStateChange, cardSecStateChange=cardSecStateChange, equipmentAudit=equipmentAudit, diskVolumeName=diskVolumeName, portLastChangeTime=portLastChangeTime, portBgpId=portBgpId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portTrapInh=portTrapInh, cmRegistrationNotification=cmRegistrationNotification, cmMacAddress=cmMacAddress, cardFpgaSource=cardFpgaSource, cardTestCommand=cardTestCommand, portPrStateChange=portPrStateChange, equipmentDiagGroup=equipmentDiagGroup, cardCarrierHwVersion=cardCarrierHwVersion, portMaxDsPower=portMaxDsPower, TestTransactionId=TestTransactionId, auditTable=auditTable, portTxFlowControlMode=portTxFlowControlMode, auditCardId=auditCardId, cardTestId=cardTestId, cardTemperatureHighWarn=cardTemperatureHighWarn, portCurrDsPower=portCurrDsPower, cardTestResultNotification=cardTestResultNotification, diskVolumeDriveId=diskVolumeDriveId, diskVolumeDiskSize=diskVolumeDiskSize, cardBusSpeed=cardBusSpeed, portDocsIfIndex=portDocsIfIndex, portAction=portAction, auditResult=auditResult, cardTestTable=cardTestTable, cardSpareGroupId=cardSpareGroupId, cardSubType=cardSubType, eqDiagConfig=eqDiagConfig, auditName=auditName, portSecStateChange=portSecStateChange, cardFlashSize=cardFlashSize, TestId=TestId, cardDetectedChange=cardDetectedChange, cardTable=cardTable, diskVolumeCardId=diskVolumeCardId, shelfNumber=shelfNumber, cardSerialNum=cardSerialNum, portDplxStatus=portDplxStatus, cmUChannelID=cmUChannelID, TestSchedule=TestSchedule, diskVolumeEntry=diskVolumeEntry, verboseLevel=verboseLevel, cmVendor=cmVendor, cardTempOverHeatNotification=cardTempOverHeatNotification, portAdminState=portAdminState, portDplxStatusChange=portDplxStatusChange, errEvLoggingEnabled=errEvLoggingEnabled, cardCarrierFwVersion=cardCarrierFwVersion, cardEntry=cardEntry, equipmentPortGroup=equipmentPortGroup, cardDetected=cardDetected, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestType=TestType, auditFailedCount=auditFailedCount, portDescription=portDescription, cardAction=cardAction, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetNotification=cmResetNotification, cadEquipmentMib=cadEquipmentMib, diskVolumeGroup=diskVolumeGroup, TestCommand=TestCommand, auditShelfId=auditShelfId, equipmentTraps=equipmentTraps, cardSwVersion=cardSwVersion, auditDescription=auditDescription, diskVolumeSize=diskVolumeSize, cardTestTime=cardTestTime, fpgaErrorEventEntry=fpgaErrorEventEntry, systemGeneralGroup=systemGeneralGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestScheduleCommand=TestScheduleCommand, cardLastChangeTime=cardLastChangeTime, systemKey=systemKey, equipmentError=equipmentError, cardId=cardId, auditTotalPassedCount=auditTotalPassedCount, equipmentStateGroup=equipmentStateGroup, cardCPLDVersion=cardCPLDVersion, equipmentTbl=equipmentTbl, portEntry=portEntry, diskVolumeId=diskVolumeId, portNumber=portNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, cardName=cardName, diskVolumeFileName=diskVolumeFileName, cmResetInfo=cmResetInfo, cardNumber=cardNumber, cardSpareGroupMode=cardSpareGroupMode, shelfName=shelfName, cmResetUpTime=cmResetUpTime, cardLastBootSource=cardLastBootSource, cardTestSchedule=cardTestSchedule, portMacIfIndex=portMacIfIndex, cardTestDescription=cardTestDescription, auditTotalFailedCount=auditTotalFailedCount, cardTestScheduleCommand=cardTestScheduleCommand, auditLogThrottle=auditLogThrottle, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCpuSpeed=cardCpuSpeed, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastBootVersion=cardLastBootVersion, cardNumPorts=cardNumPorts, cmResetStatus=cmResetStatus, portNumChans=portNumChans, cmIpAddress=cmIpAddress, cardLastResetReason=cardLastResetReason, cardTempNoReportNotification=cardTempNoReportNotification, cardTemperatureHighError=cardTemperatureHighError, auditCommand=auditCommand, portCardId=portCardId, portType=portType, cmPrimarySID=cmPrimarySID, cardTrapInh=cardTrapInh, cardTemperature=cardTemperature, cardBootVersion=cardBootVersion, portRxFlowControlMode=portRxFlowControlMode, errEvLogLevel=errEvLogLevel, cmDevice=cmDevice, diskVolumeShelfId=diskVolumeShelfId, portTxFlowControlDetected=portTxFlowControlDetected, portShelfId=portShelfId, portMacAddress=portMacAddress, auditCycleCount=auditCycleCount, portConnectorId=portConnectorId, equipmentShelfGroup=equipmentShelfGroup, cardUpTime=cardUpTime, cardHwVersion=cardHwVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskDriveNumber=diskDriveNumber, cadEquipmentMibConformance=cadEquipmentMibConformance, cardType=cardType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portPrState=portPrState, cardRamSize=cardRamSize, cardPicSerialNum=cardPicSerialNum, portDetectedMode=portDetectedMode, cardDplxStatus=cardDplxStatus, portCardSubType=portCardSubType, diagTestId=diagTestId, cmResetClearNotification=cmResetClearNotification, cardShelfTestIndex=cardShelfTestIndex, PYSNMP_MODULE_ID=cadEquipmentMib, trapCounter=trapCounter, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeTrapInh=diskVolumeTrapInh, portTable=portTable, cmResetReason=cmResetReason, portRxFlowControlDetected=portRxFlowControlDetected, equipmentCardGroup=equipmentCardGroup, diskVolumeNumber=diskVolumeNumber, cardFwUpdateStatus=cardFwUpdateStatus, errEventId=errEventId, errEvRowStatus=errEvRowStatus, auditEntry=auditEntry, cardPrState=cardPrState, TestResult=TestResult, cardTestResultDesc=cardTestResultDesc, auditAutoScheduling=auditAutoScheduling, cardSubDetected=cardSubDetected, systemGeneral=systemGeneral, auditTime=auditTime)
