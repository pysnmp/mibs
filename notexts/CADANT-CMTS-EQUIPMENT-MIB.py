...
CardType, DuplexStatus, PortId, SecondaryState, UnknownState, DiskVolumeUsageLevel, EqActionType, PortType, CardSubType, ShelfId, PortMode, CardId, FirmwareSource, FlowControlMode, PicType, PortDetectedMode, PrimaryState, AdminState = mibBuilder.importSymbols("CADANT-TC", "CardType", "DuplexStatus", "PortId", "SecondaryState", "UnknownState", "DiskVolumeUsageLevel", "EqActionType", "PortType", "CardSubType", "ShelfId", "PortMode", "CardId", "FirmwareSource", "FlowControlMode", "PicType", "PortDetectedMode", "PrimaryState", "AdminState")
...
RowStatus, TruthValue, TextualConvention, TimeStamp, DisplayString, MacAddress, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TruthValue", "TextualConvention", "TimeStamp", "DisplayString", "MacAddress", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSerialNum=cardSerialNum, cardSpareGroupMode=cardSpareGroupMode, auditDescription=auditDescription, cardNumPorts=cardNumPorts, portAction=portAction, diskVolumeDiskSize=diskVolumeDiskSize, portDplxStatusChange=portDplxStatusChange, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskDriveNumber=diskDriveNumber, cardPrStateChange=cardPrStateChange, cmDevice=cmDevice, portConnectorId=portConnectorId, cmResetUpTime=cmResetUpTime, portRxFlowControlMode=portRxFlowControlMode, diskVolumeGroup=diskVolumeGroup, cardSpareGroupId=cardSpareGroupId, cardTempOverHeatNotification=cardTempOverHeatNotification, portAdminState=portAdminState, cardTemperature=cardTemperature, portBgpId=portBgpId, cardTestEntry=cardTestEntry, shelfSwVersion=shelfSwVersion, cardTestResult=cardTestResult, cmResetNotification=cmResetNotification, cardSecState=cardSecState, portCardId=portCardId, portSecState=portSecState, cardFpgaSource=cardFpgaSource, cmUChannelID=cmUChannelID, portPrState=portPrState, cmVendor=cmVendor, systemGeneralGroup=systemGeneralGroup, auditCardId=auditCardId, TestId=TestId, cardType=cardType, cmMacAddress=cmMacAddress, cardCarrierSerialNum=cardCarrierSerialNum, auditCycleCount=auditCycleCount, errEvRowStatus=errEvRowStatus, shelfObjects=shelfObjects, cardRamSize=cardRamSize, systemClock=systemClock, portDplxStatus=portDplxStatus, cmResetClearNotification=cmResetClearNotification, cardBusSpeed=cardBusSpeed, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestCommand=cardTestCommand, remainInDiagMode=remainInDiagMode, portTxFlowControlMode=portTxFlowControlMode, auditFailedCount=auditFailedCount, portPrStateChange=portPrStateChange, cardCPLDVersion=cardCPLDVersion, systemKey=systemKey, cardTestTable=cardTestTable, auditTime=auditTime, portSecStateChange=portSecStateChange, cardTemperatureHighWarn=cardTemperatureHighWarn, cardLastChangeTime=cardLastChangeTime, diskVolumeNumber=diskVolumeNumber, cardPicDetected=cardPicDetected, cmIpAddress=cmIpAddress, diskVolumeDriveId=diskVolumeDriveId, cardTestType=cardTestType, cardCarrierHwVersion=cardCarrierHwVersion, portId=portId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portDetectedMode=portDetectedMode, cardTestResultNotification=cardTestResultNotification, diskVolumeShelfId=diskVolumeShelfId, verboseLevel=verboseLevel, portCurrDsPower=portCurrDsPower, cardFwVersion=cardFwVersion, portLastChangeTime=portLastChangeTime, shelfName=shelfName, cardSecStateChange=cardSecStateChange, cardLastBootSource=cardLastBootSource, cardFwUpdateStatus=cardFwUpdateStatus, cardTempNoReportNotification=cardTempNoReportNotification, equipmentShelfGroup=equipmentShelfGroup, equipmentCardTestGroup=equipmentCardTestGroup, systemGeneral=systemGeneral, TestCommand=TestCommand, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeEntry=diskVolumeEntry, cardTestTransId=cardTestTransId, cardTrapInh=cardTrapInh, portEntry=portEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, portTrapInh=portTrapInh, portDocsIfIndex=portDocsIfIndex, cardEntry=cardEntry, portMinDsPower=portMinDsPower, cardTemperatureHighError=cardTemperatureHighError, TestResult=TestResult, equipmentError=equipmentError, shelfNumber=shelfNumber, equipmentTraps=equipmentTraps, downstreamPowerLoss=downstreamPowerLoss, equipmentStateGroup=equipmentStateGroup, cardSwVersion=cardSwVersion, cmResetReason=cmResetReason, trapCounter=trapCounter, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestResultDesc=cardTestResultDesc, cardSubDetected=cardSubDetected, cardTestName=cardTestName, cardDetectedChange=cardDetectedChange, cardPicHwVersion=cardPicHwVersion, auditTotalPassedCount=auditTotalPassedCount, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentDiagGroup=equipmentDiagGroup, cardBootVersion=cardBootVersion, cardName=cardName, portType=portType, portShelfId=portShelfId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCommand=auditCommand, consoleOutput=consoleOutput, portMode=portMode, cardSubType=cardSubType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeTrapInh=diskVolumeTrapInh, cardCpuSpeed=cardCpuSpeed, cardTable=cardTable, cardCpuType=cardCpuType, auditTotalFailedCount=auditTotalFailedCount, cardFlashSize=cardFlashSize, portNumber=portNumber, cardNumber=cardNumber, portCardSubType=portCardSubType, equipmentDiag=equipmentDiag, auditPassedCount=auditPassedCount, errEvLogLevel=errEvLogLevel, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDplxStatus=cardDplxStatus, cmResetInfo=cmResetInfo, PYSNMP_MODULE_ID=cadEquipmentMib, portNumChans=portNumChans, cardTestDescription=cardTestDescription, diskVolumeCardId=diskVolumeCardId, diskVolumeName=diskVolumeName, auditResult=auditResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditLogThrottle=auditLogThrottle, portMaxDsPower=portMaxDsPower, auditShelfId=auditShelfId, cardLastResetReason=cardLastResetReason, auditTable=auditTable, auditStatus=auditStatus, auditId=auditId, errEventId=errEventId, cardTestScheduleCommand=cardTestScheduleCommand, cardAdminState=cardAdminState, cardTestTime=cardTestTime, cardId=cardId, TestSchedule=TestSchedule, cadEquipmentMib=cadEquipmentMib, portTable=portTable, diskVolumeFileName=diskVolumeFileName, portDescription=portDescription, cardDplxStatusChange=cardDplxStatusChange, TestScheduleCommand=TestScheduleCommand, portMacAddress=portMacAddress, cardDetected=cardDetected, equipmentCardGroup=equipmentCardGroup, cardShelfTestIndex=cardShelfTestIndex, TestType=TestType, cardTestIndex=cardTestIndex, auditAutoScheduling=auditAutoScheduling, equipmentState=equipmentState, diskVolumeId=diskVolumeId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeSize=diskVolumeSize, cardPrState=cardPrState, cmResetStatus=cmResetStatus, equipmentPortGroup=equipmentPortGroup, auditLogOutput=auditLogOutput, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCarrierFwVersion=cardCarrierFwVersion, auditEntry=auditEntry, equipmentAuditGroup=equipmentAuditGroup, cardUpTime=cardUpTime, cardTestSchedule=cardTestSchedule, diagTestId=diagTestId, trapSeverity=trapSeverity, TestTransactionId=TestTransactionId, cmPrimarySID=cmPrimarySID, auditName=auditName, cadEquipmentMibGroup=cadEquipmentMibGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardAction=cardAction, cardPicSerialNum=cardPicSerialNum, portMacIfIndex=portMacIfIndex, equipmentTbl=equipmentTbl, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestId=cardTestId, cardLastBootVersion=cardLastBootVersion, eqDiagConfig=eqDiagConfig, cardShelfId=cardShelfId, equipmentAudit=equipmentAudit, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeTable=diskVolumeTable, fpgaErrorEventTable=fpgaErrorEventTable, cardHwVersion=cardHwVersion)
