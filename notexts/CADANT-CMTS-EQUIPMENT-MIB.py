...
UnknownState, EqActionType, PicType, PrimaryState, CardId, PortId, PortMode, CardSubType, PortType, SecondaryState, ShelfId, DiskVolumeUsageLevel, AdminState, PortDetectedMode, DuplexStatus, FirmwareSource, FlowControlMode, CardType = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "EqActionType", "PicType", "PrimaryState", "CardId", "PortId", "PortMode", "CardSubType", "PortType", "SecondaryState", "ShelfId", "DiskVolumeUsageLevel", "AdminState", "PortDetectedMode", "DuplexStatus", "FirmwareSource", "FlowControlMode", "CardType")
...
TruthValue, DateAndTime, TextualConvention, DisplayString, TimeStamp, MacAddress, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "TextualConvention", "DisplayString", "TimeStamp", "MacAddress", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeName=diskVolumeName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeCardId=diskVolumeCardId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeEntry=diskVolumeEntry, cardPicDetected=cardPicDetected, cardBusSpeed=cardBusSpeed, diskVolumeNumber=diskVolumeNumber, cardName=cardName, cardLastBootSource=cardLastBootSource, auditPassedCount=auditPassedCount, cmResetInfo=cmResetInfo, cardTestDescription=cardTestDescription, cardFpgaSource=cardFpgaSource, systemGeneralGroup=systemGeneralGroup, diskVolumeTrapInh=diskVolumeTrapInh, auditName=auditName, systemGeneral=systemGeneral, diskVolumeDiskSize=diskVolumeDiskSize, portId=portId, equipmentState=equipmentState, portTxFlowControlMode=portTxFlowControlMode, cardPicHwVersion=cardPicHwVersion, diskVolumeGroup=diskVolumeGroup, remainInDiagMode=remainInDiagMode, TestId=TestId, portMinDsPower=portMinDsPower, auditCycleCount=auditCycleCount, portDescription=portDescription, consoleOutput=consoleOutput, cardFlashSize=cardFlashSize, portDetectedMode=portDetectedMode, cardPrStateChange=cardPrStateChange, cardCpuType=cardCpuType, portConnectorId=portConnectorId, diskVolumeUsageLevel=diskVolumeUsageLevel, systemKey=systemKey, equipmentDiagGroup=equipmentDiagGroup, TestCommand=TestCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, portCardId=portCardId, cardTestIndex=cardTestIndex, cardShelfTestIndex=cardShelfTestIndex, cardSerialNum=cardSerialNum, cardTestType=cardTestType, cmPrimarySID=cmPrimarySID, equipmentPortGroup=equipmentPortGroup, equipmentCardTestGroup=equipmentCardTestGroup, cardTestResultDesc=cardTestResultDesc, errEventId=errEventId, cardCpuSpeed=cardCpuSpeed, downstreamPowerLoss=downstreamPowerLoss, cardDplxStatusChange=cardDplxStatusChange, cardEntry=cardEntry, auditTable=auditTable, shelfNumber=shelfNumber, cardSwVersion=cardSwVersion, auditCardId=auditCardId, portTrapInh=portTrapInh, cmResetNotification=cmResetNotification, equipmentTraps=equipmentTraps, cardTemperatureHighError=cardTemperatureHighError, errEvRowStatus=errEvRowStatus, cmRegistrationNotification=cmRegistrationNotification, cardTestTime=cardTestTime, TestSchedule=TestSchedule, shelfSwVersion=shelfSwVersion, cardHwVersion=cardHwVersion, cardFwUpdateStatus=cardFwUpdateStatus, portShelfId=portShelfId, portAction=portAction, cmDevice=cmDevice, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSpareGroupId=cardSpareGroupId, errEvLogLevel=errEvLogLevel, portAdminState=portAdminState, verboseLevel=verboseLevel, auditResult=auditResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetClearNotification=cmResetClearNotification, portLastChangeTime=portLastChangeTime, cardTable=cardTable, portSecStateChange=portSecStateChange, cardTestScheduleCommand=cardTestScheduleCommand, portDocsIfIndex=portDocsIfIndex, cmVendor=cmVendor, cardCPLDVersion=cardCPLDVersion, trapCounter=trapCounter, eqDiagConfig=eqDiagConfig, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, cardPrState=cardPrState, equipmentError=equipmentError, portRxFlowControlMode=portRxFlowControlMode, cmResetReason=cmResetReason, portCardSubType=portCardSubType, cadEquipmentMibConformance=cadEquipmentMibConformance, cardAction=cardAction, diskVolumeDriveId=diskVolumeDriveId, cardTestSchedule=cardTestSchedule, auditStatus=auditStatus, cardRamSize=cardRamSize, cardLastBootVersion=cardLastBootVersion, portSecState=portSecState, diskVolumeUsageNotification=diskVolumeUsageNotification, portTable=portTable, cardTestResult=cardTestResult, portTxFlowControlDetected=portTxFlowControlDetected, cardAdminState=cardAdminState, cardLastChangeTime=cardLastChangeTime, cardTestEntry=cardTestEntry, cardTestName=cardTestName, cadEquipmentMibGroup=cadEquipmentMibGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeId=diskVolumeId, cardBootVersion=cardBootVersion, cardType=cardType, cardTempNoReportNotification=cardTempNoReportNotification, portMacAddress=portMacAddress, auditEntry=auditEntry, auditCommand=auditCommand, cardCarrierFwVersion=cardCarrierFwVersion, equipmentAuditGroup=equipmentAuditGroup, PYSNMP_MODULE_ID=cadEquipmentMib, cardTemperature=cardTemperature, cardTestTable=cardTestTable, errEvLoggingEnabled=errEvLoggingEnabled, cardTestTransId=cardTestTransId, diagTestId=diagTestId, TestTransactionId=TestTransactionId, cardSubDetected=cardSubDetected, portEntry=portEntry, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditTotalFailedCount=auditTotalFailedCount, equipmentStateGroup=equipmentStateGroup, cmIpAddress=cmIpAddress, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDplxStatusChange=portDplxStatusChange, auditId=auditId, diskVolumeSize=diskVolumeSize, cardDetected=cardDetected, portCurrDsPower=portCurrDsPower, diskDriveNumber=diskDriveNumber, TestScheduleCommand=TestScheduleCommand, cardPicSerialNum=cardPicSerialNum, equipmentTbl=equipmentTbl, auditLogOutput=auditLogOutput, cardShelfId=cardShelfId, portMacIfIndex=portMacIfIndex, cardTestResultNotification=cardTestResultNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentDiag=equipmentDiag, cardSecState=cardSecState, cardId=cardId, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierSerialNum=cardCarrierSerialNum, portDplxStatus=portDplxStatus, portType=portType, cmUChannelID=cmUChannelID, diskVolumeFileName=diskVolumeFileName, shelfObjects=shelfObjects, cardFwVersion=cardFwVersion, diskVolumeShelfId=diskVolumeShelfId, cardCarrierHwVersion=cardCarrierHwVersion, cmResetUpTime=cmResetUpTime, errEvRecoveryEnabled=errEvRecoveryEnabled, fpgaErrorEventGroup=fpgaErrorEventGroup, auditLogThrottle=auditLogThrottle, portNumChans=portNumChans, TestResult=TestResult, cardTrapInh=cardTrapInh, portRxFlowControlDetected=portRxFlowControlDetected, auditTime=auditTime, portPrState=portPrState, cadEquipmentMib=cadEquipmentMib, shelfName=shelfName, cardNumPorts=cardNumPorts, trapSeverity=trapSeverity, cardTestId=cardTestId, cmResetStatus=cmResetStatus, auditTotalPassedCount=auditTotalPassedCount, auditAutoScheduling=auditAutoScheduling, systemClock=systemClock, equipmentCardGroup=equipmentCardGroup, equipmentAudit=equipmentAudit, portBgpId=portBgpId, auditDescription=auditDescription, portMode=portMode, cardSubType=cardSubType, cardLastResetReason=cardLastResetReason, TestType=TestType, cardNumber=cardNumber, auditShelfId=auditShelfId, auditFailedCount=auditFailedCount, cardDetectedChange=cardDetectedChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardDplxStatus=cardDplxStatus, cardTestCommand=cardTestCommand, portPrStateChange=portPrStateChange, cardUpTime=cardUpTime, cmMacAddress=cmMacAddress, equipmentShelfGroup=equipmentShelfGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSecStateChange=cardSecStateChange, diskVolumeTable=diskVolumeTable, portNumber=portNumber, cardSpareGroupMode=cardSpareGroupMode)
