...
DiskVolumeUsageLevel, ShelfId, DuplexStatus, PortType, EqActionType, SecondaryState, PrimaryState, PortId, CardSubType, UnknownState, AdminState, CardId, FirmwareSource, PicType, FlowControlMode, CardType, PortMode, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "ShelfId", "DuplexStatus", "PortType", "EqActionType", "SecondaryState", "PrimaryState", "PortId", "CardSubType", "UnknownState", "AdminState", "CardId", "FirmwareSource", "PicType", "FlowControlMode", "CardType", "PortMode", "PortDetectedMode")
...
TextualConvention, TimeStamp, DateAndTime, TruthValue, RowStatus, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TimeStamp", "DateAndTime", "TruthValue", "RowStatus", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentState=equipmentState, cmResetInfo=cmResetInfo, auditName=auditName, cardCpuSpeed=cardCpuSpeed, portDocsIfIndex=portDocsIfIndex, cardTestEntry=cardTestEntry, diskVolumeUsageNotification=diskVolumeUsageNotification, systemGeneral=systemGeneral, equipmentAudit=equipmentAudit, cardFlashSize=cardFlashSize, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetClearNotification=cmResetClearNotification, cardBootVersion=cardBootVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portNumber=portNumber, TestScheduleCommand=TestScheduleCommand, eqDiagConfig=eqDiagConfig, cmDevice=cmDevice, equipmentCardGroup=equipmentCardGroup, portMode=portMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditCardId=auditCardId, auditAutoScheduling=auditAutoScheduling, cardTestTable=cardTestTable, auditEntry=auditEntry, portMacAddress=portMacAddress, cardTestType=cardTestType, diskVolumeGroup=diskVolumeGroup, diskVolumeCardId=diskVolumeCardId, cardShelfTestIndex=cardShelfTestIndex, systemClock=systemClock, portType=portType, errEvRowStatus=errEvRowStatus, equipmentDiagGroup=equipmentDiagGroup, portLastChangeTime=portLastChangeTime, auditShelfId=auditShelfId, cardFpgaSource=cardFpgaSource, systemKey=systemKey, shelfNumber=shelfNumber, cardNumPorts=cardNumPorts, cardSecStateChange=cardSecStateChange, shelfSwVersion=shelfSwVersion, systemGeneralGroup=systemGeneralGroup, auditFailedCount=auditFailedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, portPrStateChange=portPrStateChange, cardId=cardId, TestId=TestId, cardTestTransId=cardTestTransId, errEvLogLevel=errEvLogLevel, cardLastChangeTime=cardLastChangeTime, portConnectorId=portConnectorId, auditLogThrottle=auditLogThrottle, equipmentDiag=equipmentDiag, cardTestId=cardTestId, portSecState=portSecState, auditCycleCount=auditCycleCount, equipmentAuditGroup=equipmentAuditGroup, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResultDesc=cardTestResultDesc, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMib=cadEquipmentMib, cardCPLDVersion=cardCPLDVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, auditResult=auditResult, portAdminState=portAdminState, cardTestName=cardTestName, cardSpareGroupMode=cardSpareGroupMode, equipmentStateGroup=equipmentStateGroup, cardTempNoReportNotification=cardTempNoReportNotification, cardSubDetected=cardSubDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portId=portId, cardPicHwVersion=cardPicHwVersion, TestSchedule=TestSchedule, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeEntry=diskVolumeEntry, verboseLevel=verboseLevel, cardTestIndex=cardTestIndex, diskVolumeName=diskVolumeName, diskVolumeFileName=diskVolumeFileName, cardDetectedChange=cardDetectedChange, cardDplxStatusChange=cardDplxStatusChange, diskVolumeSize=diskVolumeSize, trapSeverity=trapSeverity, auditDescription=auditDescription, cardSpareGroupId=cardSpareGroupId, cardLastBootSource=cardLastBootSource, portBgpId=portBgpId, cmResetReason=cmResetReason, portAction=portAction, diskVolumeId=diskVolumeId, cmResetStatus=cmResetStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, fpgaErrorEventTable=fpgaErrorEventTable, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastBootVersion=cardLastBootVersion, cardSerialNum=cardSerialNum, portTrapInh=portTrapInh, cardTestSchedule=cardTestSchedule, cardAction=cardAction, cardAdminState=cardAdminState, errEventId=errEventId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEvLoggingEnabled=errEvLoggingEnabled, cardTemperatureHighWarn=cardTemperatureHighWarn, portTxFlowControlDetected=portTxFlowControlDetected, portShelfId=portShelfId, diagTestId=diagTestId, cardTestScheduleCommand=cardTestScheduleCommand, portTable=portTable, cmResetUpTime=cmResetUpTime, cardSwVersion=cardSwVersion, cardFwVersion=cardFwVersion, cardTestResult=cardTestResult, diskVolumeNumber=diskVolumeNumber, portRxFlowControlMode=portRxFlowControlMode, portCardId=portCardId, portDplxStatus=portDplxStatus, portMaxDsPower=portMaxDsPower, cardTemperature=cardTemperature, portDetectedMode=portDetectedMode, portCardSubType=portCardSubType, cardEntry=cardEntry, auditTable=auditTable, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentPortGroup=equipmentPortGroup, cmIpAddress=cmIpAddress, cardPrStateChange=cardPrStateChange, cardTrapInh=cardTrapInh, diskVolumeDriveId=diskVolumeDriveId, portCurrDsPower=portCurrDsPower, cmRegistrationNotification=cmRegistrationNotification, shelfName=shelfName, cardNumber=cardNumber, cardTemperatureHighError=cardTemperatureHighError, cardTestDescription=cardTestDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardType=cardType, diskVolumeTable=diskVolumeTable, remainInDiagMode=remainInDiagMode, TestCommand=TestCommand, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentTraps=equipmentTraps, auditPassedCount=auditPassedCount, cmPrimarySID=cmPrimarySID, equipmentError=equipmentError, cardFwUpdateStatus=cardFwUpdateStatus, cardTable=cardTable, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardName=cardName, auditTotalFailedCount=auditTotalFailedCount, equipmentTbl=equipmentTbl, portTxFlowControlMode=portTxFlowControlMode, cardTestResultNotification=cardTestResultNotification, cardRamSize=cardRamSize, portPrState=portPrState, auditCommand=auditCommand, auditStatus=auditStatus, TestType=TestType, cardBusSpeed=cardBusSpeed, auditTime=auditTime, cardPrState=cardPrState, cardPicSerialNum=cardPicSerialNum, cardSecState=cardSecState, cardLastResetReason=cardLastResetReason, cardCarrierFwVersion=cardCarrierFwVersion, equipmentShelfGroup=equipmentShelfGroup, diskDriveNumber=diskDriveNumber, cmMacAddress=cmMacAddress, portSecStateChange=portSecStateChange, cmResetNotification=cmResetNotification, cardDetected=cardDetected, TestResult=TestResult, cardShelfId=cardShelfId, portEntry=portEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cardUpTime=cardUpTime, auditLogOutput=auditLogOutput, errEvRecoveryEnabled=errEvRecoveryEnabled, portMinDsPower=portMinDsPower, cmVendor=cmVendor, portNumChans=portNumChans, TestTransactionId=TestTransactionId, cardPicDetected=cardPicDetected, portRxFlowControlDetected=portRxFlowControlDetected, shelfObjects=shelfObjects, portDplxStatusChange=portDplxStatusChange, cardCpuType=cardCpuType, portMacIfIndex=portMacIfIndex, trapCounter=trapCounter, cardTestTime=cardTestTime, auditTotalPassedCount=auditTotalPassedCount, cardSubType=cardSubType, cardCarrierHwVersion=cardCarrierHwVersion, auditId=auditId, cmUChannelID=cmUChannelID, cardTestCommand=cardTestCommand, consoleOutput=consoleOutput, cardDplxStatus=cardDplxStatus, portDescription=portDescription, cardHwVersion=cardHwVersion, equipmentCardTestGroup=equipmentCardTestGroup)
