...
FlowControlMode, PortId, AdminState, DuplexStatus, PortDetectedMode, CardType, PicType, CardSubType, PortMode, ShelfId, CardId, PrimaryState, FirmwareSource, PortType, EqActionType, SecondaryState, UnknownState, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PortId", "AdminState", "DuplexStatus", "PortDetectedMode", "CardType", "PicType", "CardSubType", "PortMode", "ShelfId", "CardId", "PrimaryState", "FirmwareSource", "PortType", "EqActionType", "SecondaryState", "UnknownState", "DiskVolumeUsageLevel")
...
MacAddress, DisplayString, DateAndTime, TimeStamp, RowStatus, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "DisplayString", "DateAndTime", "TimeStamp", "RowStatus", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardRamSize=cardRamSize, cadEquipmentMibConformance=cadEquipmentMibConformance, portDplxStatusChange=portDplxStatusChange, portAdminState=portAdminState, cardAdminState=cardAdminState, portPrState=portPrState, cardShelfTestIndex=cardShelfTestIndex, TestScheduleCommand=TestScheduleCommand, equipmentCardTestGroup=equipmentCardTestGroup, portCurrDsPower=portCurrDsPower, cmVendor=cmVendor, auditTotalFailedCount=auditTotalFailedCount, portPrStateChange=portPrStateChange, cardCpuSpeed=cardCpuSpeed, equipmentTraps=equipmentTraps, cardLastBootSource=cardLastBootSource, cardTestResultNotification=cardTestResultNotification, errEventId=errEventId, cardTable=cardTable, portCardId=portCardId, portMacIfIndex=portMacIfIndex, errEvLogLevel=errEvLogLevel, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentError=equipmentError, cardDetected=cardDetected, auditResult=auditResult, cardTestTable=cardTestTable, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portBgpId=portBgpId, diskVolumeShelfId=diskVolumeShelfId, diskVolumeName=diskVolumeName, cardDetectedChange=cardDetectedChange, portNumber=portNumber, auditTotalPassedCount=auditTotalPassedCount, equipmentTbl=equipmentTbl, cardName=cardName, cadEquipmentMib=cadEquipmentMib, cardDplxStatusChange=cardDplxStatusChange, portDescription=portDescription, diskVolumeFileName=diskVolumeFileName, cmResetNotification=cmResetNotification, TestType=TestType, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeDriveId=diskVolumeDriveId, equipmentAuditGroup=equipmentAuditGroup, cmPrimarySID=cmPrimarySID, cardType=cardType, trapSeverity=trapSeverity, cardSecState=cardSecState, portDocsIfIndex=portDocsIfIndex, cardFwUpdateStatus=cardFwUpdateStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditShelfId=auditShelfId, cardTempNoReportNotification=cardTempNoReportNotification, cardSpareGroupId=cardSpareGroupId, cmDevice=cmDevice, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageLevel=diskVolumeUsageLevel, portDetectedMode=portDetectedMode, cardTestTransId=cardTestTransId, diskVolumeSize=diskVolumeSize, auditStatus=auditStatus, cmResetClearNotification=cmResetClearNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeId=diskVolumeId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmMacAddress=cmMacAddress, auditTime=auditTime, cardCPLDVersion=cardCPLDVersion, cardSpareGroupMode=cardSpareGroupMode, portCardSubType=portCardSubType, auditFailedCount=auditFailedCount, auditCycleCount=auditCycleCount, cardCarrierFwVersion=cardCarrierFwVersion, cardPicHwVersion=cardPicHwVersion, portType=portType, errEvRowStatus=errEvRowStatus, cardTestDescription=cardTestDescription, cardPrState=cardPrState, portTrapInh=portTrapInh, cardNumber=cardNumber, cardTemperature=cardTemperature, auditPassedCount=auditPassedCount, cardTestName=cardTestName, errEvLoggingEnabled=errEvLoggingEnabled, cardTestEntry=cardTestEntry, TestCommand=TestCommand, systemGeneral=systemGeneral, remainInDiagMode=remainInDiagMode, eqDiagConfig=eqDiagConfig, cardTestType=cardTestType, downstreamPowerLoss=downstreamPowerLoss, cardFlashSize=cardFlashSize, trapCounter=trapCounter, systemKey=systemKey, shelfObjects=shelfObjects, cardFpgaSource=cardFpgaSource, diskVolumeEntry=diskVolumeEntry, cardPrStateChange=cardPrStateChange, equipmentShelfGroup=equipmentShelfGroup, cardAction=cardAction, cardTrapInh=cardTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, TestSchedule=TestSchedule, equipmentState=equipmentState, cardId=cardId, cardNumPorts=cardNumPorts, portEntry=portEntry, auditLogThrottle=auditLogThrottle, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetUpTime=cmResetUpTime, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, portSecStateChange=portSecStateChange, portSecState=portSecState, cardEntry=cardEntry, systemClock=systemClock, diskDriveNumber=diskDriveNumber, cardLastChangeTime=cardLastChangeTime, portConnectorId=portConnectorId, portAction=portAction, cmUChannelID=cmUChannelID, cardTempOverHeatNotification=cardTempOverHeatNotification, cmIpAddress=cmIpAddress, portMode=portMode, cardLastResetReason=cardLastResetReason, cardSwVersion=cardSwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, diagTestId=diagTestId, cardTestSchedule=cardTestSchedule, equipmentCardGroup=equipmentCardGroup, portId=portId, auditTable=auditTable, verboseLevel=verboseLevel, cardTestId=cardTestId, auditAutoScheduling=auditAutoScheduling, cardTemperatureHighError=cardTemperatureHighError, fpgaErrorEventTable=fpgaErrorEventTable, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCpuType=cardCpuType, cardSubDetected=cardSubDetected, auditLogOutput=auditLogOutput, portNumChans=portNumChans, portDplxStatus=portDplxStatus, equipmentDiag=equipmentDiag, cardBootVersion=cardBootVersion, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSubType=cardSubType, cardBusSpeed=cardBusSpeed, diskVolumeTable=diskVolumeTable, cardTestScheduleCommand=cardTestScheduleCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, consoleOutput=consoleOutput, portMinDsPower=portMinDsPower, TestResult=TestResult, shelfNumber=shelfNumber, equipmentStateGroup=equipmentStateGroup, cardShelfId=cardShelfId, systemGeneralGroup=systemGeneralGroup, diskVolumeNumber=diskVolumeNumber, cardTestCommand=cardTestCommand, cmResetReason=cmResetReason, cmResetInfo=cmResetInfo, auditDescription=auditDescription, auditCommand=auditCommand, diskVolumeCardId=diskVolumeCardId, portTable=portTable, cardSerialNum=cardSerialNum, cardTestIndex=cardTestIndex, cardTestResult=cardTestResult, auditName=auditName, shelfName=shelfName, cardCarrierHwVersion=cardCarrierHwVersion, TestTransactionId=TestTransactionId, shelfSwVersion=shelfSwVersion, portMaxDsPower=portMaxDsPower, cardPicDetected=cardPicDetected, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestTime=cardTestTime, portRxFlowControlMode=portRxFlowControlMode, cardTestResultDesc=cardTestResultDesc, cmRegistrationNotification=cmRegistrationNotification, diskVolumeGroup=diskVolumeGroup, equipmentAudit=equipmentAudit, portTxFlowControlDetected=portTxFlowControlDetected, auditCardId=auditCardId, cardFwVersion=cardFwVersion, cardSecStateChange=cardSecStateChange, TestId=TestId, portLastChangeTime=portLastChangeTime, cardCarrierSerialNum=cardCarrierSerialNum, cardHwVersion=cardHwVersion, cardUpTime=cardUpTime, portShelfId=portShelfId, portMacAddress=portMacAddress, equipmentDiagGroup=equipmentDiagGroup, cardLastBootVersion=cardLastBootVersion, equipmentPortGroup=equipmentPortGroup, auditEntry=auditEntry, auditId=auditId, cmResetStatus=cmResetStatus, cardDplxStatus=cardDplxStatus)
