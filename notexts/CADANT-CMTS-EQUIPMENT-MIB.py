...
EqActionType, AdminState, PortDetectedMode, PortType, SecondaryState, DuplexStatus, ShelfId, PortMode, PrimaryState, UnknownState, CardType, CardId, CardSubType, FlowControlMode, PortId, FirmwareSource, DiskVolumeUsageLevel, PicType = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "AdminState", "PortDetectedMode", "PortType", "SecondaryState", "DuplexStatus", "ShelfId", "PortMode", "PrimaryState", "UnknownState", "CardType", "CardId", "CardSubType", "FlowControlMode", "PortId", "FirmwareSource", "DiskVolumeUsageLevel", "PicType")
...
MacAddress, TextualConvention, DisplayString, TimeStamp, DateAndTime, RowStatus, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TextualConvention", "DisplayString", "TimeStamp", "DateAndTime", "RowStatus", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestType=cardTestType, auditName=auditName, cardDplxStatus=cardDplxStatus, equipmentState=equipmentState, cardSubDetected=cardSubDetected, cardPicSerialNum=cardPicSerialNum, portDocsIfIndex=portDocsIfIndex, errEvRowStatus=errEvRowStatus, cardAdminState=cardAdminState, cardTestId=cardTestId, errEvLogLevel=errEvLogLevel, cardTestTime=cardTestTime, cmRegistrationNotification=cmRegistrationNotification, portMaxDsPower=portMaxDsPower, auditCardId=auditCardId, diskVolumeNumber=diskVolumeNumber, cardHwVersion=cardHwVersion, cmResetNotification=cmResetNotification, cardShelfTestIndex=cardShelfTestIndex, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeSize=diskVolumeSize, TestCommand=TestCommand, equipmentTraps=equipmentTraps, TestTransactionId=TestTransactionId, portAdminState=portAdminState, cmResetInfo=cmResetInfo, portId=portId, systemClock=systemClock, diskVolumeTrapInh=diskVolumeTrapInh, cardCpuType=cardCpuType, portRxFlowControlMode=portRxFlowControlMode, auditPassedCount=auditPassedCount, cardFwUpdateStatus=cardFwUpdateStatus, cmUChannelID=cmUChannelID, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, errEvLoggingEnabled=errEvLoggingEnabled, cardLastBootSource=cardLastBootSource, cardPicHwVersion=cardPicHwVersion, portMode=portMode, diskVolumeDriveId=diskVolumeDriveId, portType=portType, systemGeneral=systemGeneral, portShelfId=portShelfId, diskVolumeShelfId=diskVolumeShelfId, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibConformance=cadEquipmentMibConformance, cardDplxStatusChange=cardDplxStatusChange, cardTestName=cardTestName, cardDetected=cardDetected, portMinDsPower=portMinDsPower, portLastChangeTime=portLastChangeTime, equipmentStateGroup=equipmentStateGroup, TestScheduleCommand=TestScheduleCommand, TestType=TestType, portBgpId=portBgpId, cardSerialNum=cardSerialNum, cardFwVersion=cardFwVersion, cardTestResultDesc=cardTestResultDesc, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portCardSubType=portCardSubType, shelfNumber=shelfNumber, diskVolumeTable=diskVolumeTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestScheduleCommand=cardTestScheduleCommand, shelfSwVersion=shelfSwVersion, cardSwVersion=cardSwVersion, cmResetUpTime=cmResetUpTime, auditTime=auditTime, equipmentAuditGroup=equipmentAuditGroup, auditResult=auditResult, cmDevice=cmDevice, equipmentDiagGroup=equipmentDiagGroup, cardTestCommand=cardTestCommand, portTrapInh=portTrapInh, cardCarrierSerialNum=cardCarrierSerialNum, cardTestResult=cardTestResult, shelfObjects=shelfObjects, equipmentShelfGroup=equipmentShelfGroup, cardSubType=cardSubType, cmIpAddress=cmIpAddress, auditStatus=auditStatus, TestSchedule=TestSchedule, cadEquipmentMibGroup=cadEquipmentMibGroup, portPrState=portPrState, cardTempNoReportNotification=cardTempNoReportNotification, cardRamSize=cardRamSize, cardTrapInh=cardTrapInh, auditCycleCount=auditCycleCount, cardAction=cardAction, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portEntry=portEntry, cardTestIndex=cardTestIndex, cardCPLDVersion=cardCPLDVersion, cardLastChangeTime=cardLastChangeTime, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSecState=cardSecState, portCurrDsPower=portCurrDsPower, cardDetectedChange=cardDetectedChange, equipmentError=equipmentError, errEventId=errEventId, cardBootVersion=cardBootVersion, cardTestTable=cardTestTable, cardFpgaSource=cardFpgaSource, cardLastBootVersion=cardLastBootVersion, portSecStateChange=portSecStateChange, portCardId=portCardId, cardTestDescription=cardTestDescription, errEvRecoveryEnabled=errEvRecoveryEnabled, auditTable=auditTable, cardUpTime=cardUpTime, equipmentTbl=equipmentTbl, cardSpareGroupId=cardSpareGroupId, auditLogOutput=auditLogOutput, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeEntry=diskVolumeEntry, portDplxStatusChange=portDplxStatusChange, portSecState=portSecState, portAction=portAction, diskVolumeFileName=diskVolumeFileName, auditDescription=auditDescription, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfName=shelfName, cardFlashSize=cardFlashSize, diskDriveNumber=diskDriveNumber, auditEntry=auditEntry, portPrStateChange=portPrStateChange, cardName=cardName, cardTable=cardTable, cmPrimarySID=cmPrimarySID, cardId=cardId, systemGeneralGroup=systemGeneralGroup, equipmentCardGroup=equipmentCardGroup, equipmentPortGroup=equipmentPortGroup, systemKey=systemKey, diskVolumeCardId=diskVolumeCardId, cardCarrierHwVersion=cardCarrierHwVersion, portTable=portTable, cmResetReason=cmResetReason, remainInDiagMode=remainInDiagMode, auditId=auditId, portNumChans=portNumChans, cmResetStatus=cmResetStatus, cardLastResetReason=cardLastResetReason, portTxFlowControlMode=portTxFlowControlMode, auditFailedCount=auditFailedCount, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditTotalPassedCount=auditTotalPassedCount, cmVendor=cmVendor, diskVolumeName=diskVolumeName, cardTestEntry=cardTestEntry, eqDiagConfig=eqDiagConfig, portNumber=portNumber, cardPrState=cardPrState, portMacIfIndex=portMacIfIndex, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDiskSize=diskVolumeDiskSize, cmMacAddress=cmMacAddress, PYSNMP_MODULE_ID=cadEquipmentMib, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMibCompliances=cadEquipmentMibCompliances, downstreamPowerLoss=downstreamPowerLoss, cardPicDetected=cardPicDetected, portTxFlowControlDetected=portTxFlowControlDetected, TestId=TestId, fpgaErrorEventTable=fpgaErrorEventTable, equipmentAudit=equipmentAudit, cardTestSchedule=cardTestSchedule, portRxFlowControlDetected=portRxFlowControlDetected, trapSeverity=trapSeverity, TestResult=TestResult, portConnectorId=portConnectorId, cardTemperatureHighWarn=cardTemperatureHighWarn, diagTestId=diagTestId, auditShelfId=auditShelfId, cmResetClearNotification=cmResetClearNotification, trapCounter=trapCounter, cardShelfId=cardShelfId, cardSpareGroupMode=cardSpareGroupMode, cardNumber=cardNumber, cardTemperature=cardTemperature, portMacAddress=portMacAddress, diskVolumeId=diskVolumeId, equipmentDiag=equipmentDiag, consoleOutput=consoleOutput, cardTestTransId=cardTestTransId, cardCarrierFwVersion=cardCarrierFwVersion, cardTestResultNotification=cardTestResultNotification, cardType=cardType, auditCommand=auditCommand, cadEquipmentMib=cadEquipmentMib, portDetectedMode=portDetectedMode, cardEntry=cardEntry, portDplxStatus=portDplxStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, verboseLevel=verboseLevel, cardBusSpeed=cardBusSpeed, cardSecStateChange=cardSecStateChange, diskVolumeGroup=diskVolumeGroup, auditTotalFailedCount=auditTotalFailedCount, auditLogThrottle=auditLogThrottle, cardCpuSpeed=cardCpuSpeed, portDescription=portDescription, diskVolumeUsageNotification=diskVolumeUsageNotification, cardNumPorts=cardNumPorts, cardPrStateChange=cardPrStateChange)
