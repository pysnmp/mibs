...
CardSubType, EqActionType, UnknownState, PicType, PortId, DiskVolumeUsageLevel, PortMode, FlowControlMode, PortType, PrimaryState, SecondaryState, AdminState, FirmwareSource, ShelfId, CardType, DuplexStatus, PortDetectedMode, CardId = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "EqActionType", "UnknownState", "PicType", "PortId", "DiskVolumeUsageLevel", "PortMode", "FlowControlMode", "PortType", "PrimaryState", "SecondaryState", "AdminState", "FirmwareSource", "ShelfId", "CardType", "DuplexStatus", "PortDetectedMode", "CardId")
...
TextualConvention, MacAddress, RowStatus, TimeStamp, TruthValue, DisplayString, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "RowStatus", "TimeStamp", "TruthValue", "DisplayString", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTable=cardTable, cardCpuSpeed=cardCpuSpeed, portSecState=portSecState, cardShelfTestIndex=cardShelfTestIndex, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSpareGroupId=cardSpareGroupId, diskVolumeDriveId=diskVolumeDriveId, systemGeneral=systemGeneral, diskDriveNumber=diskDriveNumber, shelfNumber=shelfNumber, portConnectorId=portConnectorId, equipmentCardGroup=equipmentCardGroup, cardTestCommand=cardTestCommand, cadEquipmentMib=cadEquipmentMib, portNumChans=portNumChans, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestSchedule=cardTestSchedule, equipmentAudit=equipmentAudit, diskVolumeUsageLevel=diskVolumeUsageLevel, auditTotalFailedCount=auditTotalFailedCount, portMinDsPower=portMinDsPower, portSecStateChange=portSecStateChange, cardCarrierFwVersion=cardCarrierFwVersion, auditCardId=auditCardId, diskVolumeSize=diskVolumeSize, auditLogThrottle=auditLogThrottle, auditEntry=auditEntry, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditResult=auditResult, cardBusSpeed=cardBusSpeed, cardTestResultNotification=cardTestResultNotification, equipmentDiag=equipmentDiag, cadEquipmentMibGroup=cadEquipmentMibGroup, portCurrDsPower=portCurrDsPower, diskVolumeEntry=diskVolumeEntry, cardTestResultDesc=cardTestResultDesc, diskVolumeGroup=diskVolumeGroup, cardEntry=cardEntry, cardAction=cardAction, cardTempNoReportNotification=cardTempNoReportNotification, auditDescription=auditDescription, cardLastBootVersion=cardLastBootVersion, cardPicSerialNum=cardPicSerialNum, portTable=portTable, portTxFlowControlMode=portTxFlowControlMode, portTrapInh=portTrapInh, diskVolumeUsagePercentage=diskVolumeUsagePercentage, errEvLogLevel=errEvLogLevel, auditFailedCount=auditFailedCount, TestType=TestType, portMacIfIndex=portMacIfIndex, diskVolumeName=diskVolumeName, cmResetInfo=cmResetInfo, portLastChangeTime=portLastChangeTime, cmResetNotification=cmResetNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardNumPorts=cardNumPorts, equipmentShelfGroup=equipmentShelfGroup, portType=portType, cardFlashSize=cardFlashSize, cardFpgaSource=cardFpgaSource, equipmentAuditGroup=equipmentAuditGroup, cmIpAddress=cmIpAddress, cardName=cardName, cardRamSize=cardRamSize, auditCycleCount=auditCycleCount, auditId=auditId, diagTestId=diagTestId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentStateGroup=equipmentStateGroup, portRxFlowControlMode=portRxFlowControlMode, diskVolumeId=diskVolumeId, cardId=cardId, fpgaErrorEventGroup=fpgaErrorEventGroup, portShelfId=portShelfId, cmVendor=cmVendor, portMacAddress=portMacAddress, diskVolumeUsageNotification=diskVolumeUsageNotification, cardNumber=cardNumber, cardSubType=cardSubType, equipmentTraps=equipmentTraps, equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, portCardId=portCardId, portEntry=portEntry, cardDetectedChange=cardDetectedChange, auditTime=auditTime, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmRegistrationNotification=cmRegistrationNotification, cardTestTime=cardTestTime, cmUChannelID=cmUChannelID, shelfSwVersion=shelfSwVersion, cardTrapInh=cardTrapInh, fpgaErrorEventEntry=fpgaErrorEventEntry, cmResetReason=cmResetReason, diskVolumeDiskSize=diskVolumeDiskSize, cmResetUpTime=cmResetUpTime, cardType=cardType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSerialNum=cardSerialNum, portAction=portAction, eqDiagConfig=eqDiagConfig, cardTestName=cardTestName, cardTemperatureHighError=cardTemperatureHighError, cardShelfId=cardShelfId, portDocsIfIndex=portDocsIfIndex, cardTestId=cardTestId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSecStateChange=cardSecStateChange, systemClock=systemClock, auditAutoScheduling=auditAutoScheduling, verboseLevel=verboseLevel, cardPrState=cardPrState, TestId=TestId, cardTestDescription=cardTestDescription, auditShelfId=auditShelfId, auditCommand=auditCommand, cardTestEntry=cardTestEntry, equipmentPortGroup=equipmentPortGroup, cardLastBootSource=cardLastBootSource, diskVolumeNumber=diskVolumeNumber, cardTestTransId=cardTestTransId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCarrierHwVersion=cardCarrierHwVersion, cardAdminState=cardAdminState, auditName=auditName, errEventId=errEventId, equipmentCardTestGroup=equipmentCardTestGroup, portMode=portMode, cardSecState=cardSecState, diskVolumeFileName=diskVolumeFileName, cardTestTable=cardTestTable, TestTransactionId=TestTransactionId, cardCarrierSerialNum=cardCarrierSerialNum, cardCpuType=cardCpuType, diskVolumeShelfId=diskVolumeShelfId, TestScheduleCommand=TestScheduleCommand, remainInDiagMode=remainInDiagMode, cardTestScheduleCommand=cardTestScheduleCommand, portDplxStatus=portDplxStatus, cardCPLDVersion=cardCPLDVersion, cmResetClearNotification=cmResetClearNotification, cardFwVersion=cardFwVersion, portPrState=portPrState, portDplxStatusChange=portDplxStatusChange, cmDevice=cmDevice, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSwVersion=cardSwVersion, auditTable=auditTable, TestResult=TestResult, cardFwUpdateStatus=cardFwUpdateStatus, portNumber=portNumber, equipmentError=equipmentError, shelfName=shelfName, diskVolumeTable=diskVolumeTable, portTxFlowControlDetected=portTxFlowControlDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, trapCounter=trapCounter, portAdminState=portAdminState, cardBootVersion=cardBootVersion, TestCommand=TestCommand, cardDetected=cardDetected, cardPicDetected=cardPicDetected, portRxFlowControlDetected=portRxFlowControlDetected, cmPrimarySID=cmPrimarySID, auditPassedCount=auditPassedCount, systemKey=systemKey, cardLastChangeTime=cardLastChangeTime, cardTestIndex=cardTestIndex, errEvLoggingEnabled=errEvLoggingEnabled, cardHwVersion=cardHwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, consoleOutput=consoleOutput, cardSpareGroupMode=cardSpareGroupMode, cardTemperature=cardTemperature, portMaxDsPower=portMaxDsPower, equipmentDiagGroup=equipmentDiagGroup, cardTestResult=cardTestResult, cardTestType=cardTestType, cardDplxStatusChange=cardDplxStatusChange, diskVolumeTrapInh=diskVolumeTrapInh, portDetectedMode=portDetectedMode, auditTotalPassedCount=auditTotalPassedCount, cardSubDetected=cardSubDetected, TestSchedule=TestSchedule, cardDplxStatus=cardDplxStatus, portId=portId, auditStatus=auditStatus, cmMacAddress=cmMacAddress, portDescription=portDescription, equipmentTbl=equipmentTbl, portCardSubType=portCardSubType, errEvRowStatus=errEvRowStatus, auditLogOutput=auditLogOutput, cardPrStateChange=cardPrStateChange, downstreamPowerLoss=downstreamPowerLoss, portBgpId=portBgpId, shelfObjects=shelfObjects, cardUpTime=cardUpTime, cardLastResetReason=cardLastResetReason, cmResetStatus=cmResetStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portPrStateChange=portPrStateChange, trapSeverity=trapSeverity, systemGeneralGroup=systemGeneralGroup, cardPicHwVersion=cardPicHwVersion)
