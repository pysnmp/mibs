...
DiskVolumeUsageLevel, DuplexStatus, PicType, AdminState, PortDetectedMode, UnknownState, FirmwareSource, CardSubType, PrimaryState, CardType, PortId, PortType, ShelfId, EqActionType, CardId, FlowControlMode, PortMode, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "DuplexStatus", "PicType", "AdminState", "PortDetectedMode", "UnknownState", "FirmwareSource", "CardSubType", "PrimaryState", "CardType", "PortId", "PortType", "ShelfId", "EqActionType", "CardId", "FlowControlMode", "PortMode", "SecondaryState")
...
MacAddress, TruthValue, DateAndTime, DisplayString, TimeStamp, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "DateAndTime", "DisplayString", "TimeStamp", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeDiskSize=diskVolumeDiskSize, auditAutoScheduling=auditAutoScheduling, errEventId=errEventId, cardDplxStatus=cardDplxStatus, cardNumPorts=cardNumPorts, portRxFlowControlDetected=portRxFlowControlDetected, cardTestEntry=cardTestEntry, cardSpareGroupMode=cardSpareGroupMode, portSecState=portSecState, equipmentTbl=equipmentTbl, trapCounter=trapCounter, portMaxDsPower=portMaxDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestCommand=TestCommand, shelfName=shelfName, equipmentState=equipmentState, diskVolumeSize=diskVolumeSize, cardLastResetReason=cardLastResetReason, auditCardId=auditCardId, auditId=auditId, diskVolumeShelfId=diskVolumeShelfId, downstreamPowerLoss=downstreamPowerLoss, portTxFlowControlMode=portTxFlowControlMode, diskVolumeGroup=diskVolumeGroup, auditFailedCount=auditFailedCount, equipmentTraps=equipmentTraps, cardTestTime=cardTestTime, systemGeneralGroup=systemGeneralGroup, shelfSwVersion=shelfSwVersion, cardSwVersion=cardSwVersion, cardTestScheduleCommand=cardTestScheduleCommand, cmResetReason=cmResetReason, auditStatus=auditStatus, equipmentError=equipmentError, cardTestId=cardTestId, cardRamSize=cardRamSize, cardTestTransId=cardTestTransId, equipmentAuditGroup=equipmentAuditGroup, cardPrStateChange=cardPrStateChange, systemKey=systemKey, diskVolumeId=diskVolumeId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardDplxStatusChange=cardDplxStatusChange, cardTemperatureHighError=cardTemperatureHighError, cmMacAddress=cmMacAddress, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardFpgaSource=cardFpgaSource, cardAction=cardAction, cardTestResultDesc=cardTestResultDesc, cardTable=cardTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSubType=cardSubType, cardTestName=cardTestName, cmResetUpTime=cmResetUpTime, cardCarrierHwVersion=cardCarrierHwVersion, cardCPLDVersion=cardCPLDVersion, cardDetected=cardDetected, cardShelfId=cardShelfId, cardTemperature=cardTemperature, diskVolumeEntry=diskVolumeEntry, cmResetNotification=cmResetNotification, errEvRowStatus=errEvRowStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portMinDsPower=portMinDsPower, cmIpAddress=cmIpAddress, cmUChannelID=cmUChannelID, cardSerialNum=cardSerialNum, cardEntry=cardEntry, remainInDiagMode=remainInDiagMode, errEvLogLevel=errEvLogLevel, cardSubDetected=cardSubDetected, TestResult=TestResult, diskVolumeName=diskVolumeName, shelfObjects=shelfObjects, TestType=TestType, cardTrapInh=cardTrapInh, cardCarrierSerialNum=cardCarrierSerialNum, cardDetectedChange=cardDetectedChange, cardLastBootSource=cardLastBootSource, diskVolumeCardId=diskVolumeCardId, consoleOutput=consoleOutput, portDplxStatusChange=portDplxStatusChange, eqDiagConfig=eqDiagConfig, cardPicHwVersion=cardPicHwVersion, portId=portId, auditCycleCount=auditCycleCount, portTable=portTable, cardTestIndex=cardTestIndex, auditPassedCount=auditPassedCount, diskVolumeTrapInh=diskVolumeTrapInh, cmPrimarySID=cmPrimarySID, TestSchedule=TestSchedule, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardBootVersion=cardBootVersion, auditShelfId=auditShelfId, cardTestTable=cardTestTable, cardPicSerialNum=cardPicSerialNum, portNumber=portNumber, diskVolumeTable=diskVolumeTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestCommand=cardTestCommand, PYSNMP_MODULE_ID=cadEquipmentMib, cardLastBootVersion=cardLastBootVersion, portTxFlowControlDetected=portTxFlowControlDetected, equipmentShelfGroup=equipmentShelfGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, portDplxStatus=portDplxStatus, equipmentCardGroup=equipmentCardGroup, cardNumber=cardNumber, portNumChans=portNumChans, equipmentStateGroup=equipmentStateGroup, cardUpTime=cardUpTime, cardTestResult=cardTestResult, systemGeneral=systemGeneral, portCardId=portCardId, errEvLoggingEnabled=errEvLoggingEnabled, TestId=TestId, portPrStateChange=portPrStateChange, auditTotalFailedCount=auditTotalFailedCount, cardTempNoReportNotification=cardTempNoReportNotification, portDetectedMode=portDetectedMode, cardSecState=cardSecState, cadEquipmentMib=cadEquipmentMib, auditResult=auditResult, portRxFlowControlMode=portRxFlowControlMode, cardTestDescription=cardTestDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskDriveNumber=diskDriveNumber, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSecStateChange=cardSecStateChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentDiag=equipmentDiag, fpgaErrorEventEntry=fpgaErrorEventEntry, portBgpId=portBgpId, portType=portType, cardFlashSize=cardFlashSize, equipmentDiagGroup=equipmentDiagGroup, equipmentAudit=equipmentAudit, cardAdminState=cardAdminState, diskVolumeFileName=diskVolumeFileName, portMacAddress=portMacAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, portLastChangeTime=portLastChangeTime, cmResetInfo=cmResetInfo, portDescription=portDescription, portEntry=portEntry, cardPrState=cardPrState, auditTotalPassedCount=auditTotalPassedCount, portAction=portAction, trapSeverity=trapSeverity, equipmentPortGroup=equipmentPortGroup, cardBusSpeed=cardBusSpeed, cardCarrierFwVersion=cardCarrierFwVersion, cardId=cardId, cardCpuType=cardCpuType, cmRegistrationNotification=cmRegistrationNotification, cardTestResultNotification=cardTestResultNotification, auditEntry=auditEntry, fpgaErrorEventTable=fpgaErrorEventTable, portCardSubType=portCardSubType, cardTestType=cardTestType, auditName=auditName, auditCommand=auditCommand, portDocsIfIndex=portDocsIfIndex, cardType=cardType, systemClock=systemClock, portConnectorId=portConnectorId, cmVendor=cmVendor, portMode=portMode, portAdminState=portAdminState, cardLastChangeTime=cardLastChangeTime, cardCpuSpeed=cardCpuSpeed, cardTempOverHeatNotification=cardTempOverHeatNotification, cardFwVersion=cardFwVersion, auditLogThrottle=auditLogThrottle, diagTestId=diagTestId, cmResetStatus=cmResetStatus, cardPicDetected=cardPicDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeDriveId=diskVolumeDriveId, auditTime=auditTime, cmDevice=cmDevice, portCurrDsPower=portCurrDsPower, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portSecStateChange=portSecStateChange, cardHwVersion=cardHwVersion, TestTransactionId=TestTransactionId, portMacIfIndex=portMacIfIndex, verboseLevel=verboseLevel, TestScheduleCommand=TestScheduleCommand, cmResetClearNotification=cmResetClearNotification, cardName=cardName, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeNumber=diskVolumeNumber, auditDescription=auditDescription, portTrapInh=portTrapInh, auditTable=auditTable, equipmentCardTestGroup=equipmentCardTestGroup, portPrState=portPrState, cardTestSchedule=cardTestSchedule, cardShelfTestIndex=cardShelfTestIndex, shelfNumber=shelfNumber, portShelfId=portShelfId, auditLogOutput=auditLogOutput, cardSpareGroupId=cardSpareGroupId)
