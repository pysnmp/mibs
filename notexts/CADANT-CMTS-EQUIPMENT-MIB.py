...
PortMode, FlowControlMode, PortType, ShelfId, PicType, SecondaryState, DiskVolumeUsageLevel, FirmwareSource, CardSubType, AdminState, EqActionType, CardId, PortId, CardType, PrimaryState, PortDetectedMode, DuplexStatus, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PortMode", "FlowControlMode", "PortType", "ShelfId", "PicType", "SecondaryState", "DiskVolumeUsageLevel", "FirmwareSource", "CardSubType", "AdminState", "EqActionType", "CardId", "PortId", "CardType", "PrimaryState", "PortDetectedMode", "DuplexStatus", "UnknownState")
...
TruthValue, TimeStamp, DisplayString, RowStatus, MacAddress, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TimeStamp", "DisplayString", "RowStatus", "MacAddress", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmUChannelID=cmUChannelID, cardFwVersion=cardFwVersion, equipmentError=equipmentError, cardEntry=cardEntry, equipmentTbl=equipmentTbl, cardShelfId=cardShelfId, systemKey=systemKey, diskVolumeNumber=diskVolumeNumber, portDescription=portDescription, cardShelfTestIndex=cardShelfTestIndex, TestTransactionId=TestTransactionId, cardSubType=cardSubType, TestScheduleCommand=TestScheduleCommand, cardBusSpeed=cardBusSpeed, cmResetNotification=cmResetNotification, portDplxStatusChange=portDplxStatusChange, portCurrDsPower=portCurrDsPower, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, fpgaErrorEventTable=fpgaErrorEventTable, TestCommand=TestCommand, portTable=portTable, portMaxDsPower=portMaxDsPower, errEventId=errEventId, cmIpAddress=cmIpAddress, cmRegistrationNotification=cmRegistrationNotification, cardPicHwVersion=cardPicHwVersion, cmResetStatus=cmResetStatus, eqDiagConfig=eqDiagConfig, cardTestName=cardTestName, TestType=TestType, cardAction=cardAction, cardCarrierSerialNum=cardCarrierSerialNum, portRxFlowControlMode=portRxFlowControlMode, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentDiagGroup=equipmentDiagGroup, diskVolumeTrapInh=diskVolumeTrapInh, auditResult=auditResult, portType=portType, portAdminState=portAdminState, equipmentDiag=equipmentDiag, cardCpuType=cardCpuType, auditTime=auditTime, portNumber=portNumber, cmVendor=cmVendor, diskVolumeCardId=diskVolumeCardId, TestResult=TestResult, cardFlashSize=cardFlashSize, portNumChans=portNumChans, auditTotalPassedCount=auditTotalPassedCount, cardNumPorts=cardNumPorts, cardPicSerialNum=cardPicSerialNum, cardUpTime=cardUpTime, cardTable=cardTable, cardCarrierFwVersion=cardCarrierFwVersion, portPrState=portPrState, portId=portId, shelfSwVersion=shelfSwVersion, portAction=portAction, cardTestEntry=cardTestEntry, equipmentCardTestGroup=equipmentCardTestGroup, equipmentAuditGroup=equipmentAuditGroup, auditCommand=auditCommand, cardId=cardId, errEvLoggingEnabled=errEvLoggingEnabled, equipmentState=equipmentState, auditFailedCount=auditFailedCount, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, errEvLogLevel=errEvLogLevel, portEntry=portEntry, errEvRowStatus=errEvRowStatus, portSecState=portSecState, auditName=auditName, cardTestResult=cardTestResult, auditId=auditId, diskVolumeId=diskVolumeId, portPrStateChange=portPrStateChange, cardHwVersion=cardHwVersion, portBgpId=portBgpId, cmResetInfo=cmResetInfo, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardName=cardName, auditCycleCount=auditCycleCount, diskVolumeShelfId=diskVolumeShelfId, cardSpareGroupId=cardSpareGroupId, cardTestTransId=cardTestTransId, trapCounter=trapCounter, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMib=cadEquipmentMib, cardSwVersion=cardSwVersion, portDplxStatus=portDplxStatus, portMacAddress=portMacAddress, cmResetReason=cmResetReason, shelfObjects=shelfObjects, shelfNumber=shelfNumber, auditDescription=auditDescription, portConnectorId=portConnectorId, cardTestTime=cardTestTime, cardTempNoReportNotification=cardTempNoReportNotification, cardSecState=cardSecState, cardSerialNum=cardSerialNum, equipmentStateGroup=equipmentStateGroup, equipmentPortGroup=equipmentPortGroup, trapSeverity=trapSeverity, diskDriveNumber=diskDriveNumber, diskVolumeUsageNotification=diskVolumeUsageNotification, auditEntry=auditEntry, auditLogThrottle=auditLogThrottle, auditTotalFailedCount=auditTotalFailedCount, cardPrStateChange=cardPrStateChange, auditLogOutput=auditLogOutput, cardDplxStatusChange=cardDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, portLastChangeTime=portLastChangeTime, cardFpgaSource=cardFpgaSource, auditTable=auditTable, cmMacAddress=cmMacAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardNumber=cardNumber, portMode=portMode, cardTestResultDesc=cardTestResultDesc, cardTestId=cardTestId, cardDetected=cardDetected, TestSchedule=TestSchedule, cardTemperatureHighError=cardTemperatureHighError, fpgaErrorEventGroup=fpgaErrorEventGroup, cardType=cardType, diagTestId=diagTestId, portCardId=portCardId, portDetectedMode=portDetectedMode, auditShelfId=auditShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardDetectedChange=cardDetectedChange, diskVolumeGroup=diskVolumeGroup, cardLastBootVersion=cardLastBootVersion, downstreamPowerLoss=downstreamPowerLoss, portMinDsPower=portMinDsPower, cardTestIndex=cardTestIndex, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCarrierHwVersion=cardCarrierHwVersion, cardPicDetected=cardPicDetected, cmDevice=cmDevice, verboseLevel=verboseLevel, cadEquipmentMibConformance=cadEquipmentMibConformance, portRxFlowControlDetected=portRxFlowControlDetected, equipmentCardGroup=equipmentCardGroup, portSecStateChange=portSecStateChange, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestTable=cardTestTable, TestId=TestId, cardSecStateChange=cardSecStateChange, cardBootVersion=cardBootVersion, systemClock=systemClock, diskVolumeDriveId=diskVolumeDriveId, portTxFlowControlMode=portTxFlowControlMode, cardTestScheduleCommand=cardTestScheduleCommand, cmResetClearNotification=cmResetClearNotification, systemGeneralGroup=systemGeneralGroup, remainInDiagMode=remainInDiagMode, cardTemperatureHighWarn=cardTemperatureHighWarn, portTxFlowControlDetected=portTxFlowControlDetected, cmPrimarySID=cmPrimarySID, cmResetUpTime=cmResetUpTime, cardTestType=cardTestType, cardFwUpdateStatus=cardFwUpdateStatus, cardPrState=cardPrState, fpgaErrorEventEntry=fpgaErrorEventEntry, shelfName=shelfName, cardTemperature=cardTemperature, diskVolumeTable=diskVolumeTable, portMacIfIndex=portMacIfIndex, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentShelfGroup=equipmentShelfGroup, cardLastResetReason=cardLastResetReason, portDocsIfIndex=portDocsIfIndex, diskVolumeName=diskVolumeName, systemGeneral=systemGeneral, cardTestDescription=cardTestDescription, cardTestResultNotification=cardTestResultNotification, cardTestSchedule=cardTestSchedule, portTrapInh=portTrapInh, cardLastBootSource=cardLastBootSource, diskVolumeSize=diskVolumeSize, cardCpuSpeed=cardCpuSpeed, cardLastChangeTime=cardLastChangeTime, cardTestCommand=cardTestCommand, cardTrapInh=cardTrapInh, equipmentTraps=equipmentTraps, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSubDetected=cardSubDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, auditAutoScheduling=auditAutoScheduling, cardAdminState=cardAdminState, diskVolumeFileName=diskVolumeFileName, portShelfId=portShelfId, auditCardId=auditCardId, cardDplxStatus=cardDplxStatus, equipmentAudit=equipmentAudit, auditStatus=auditStatus, cardCPLDVersion=cardCPLDVersion, cardSpareGroupMode=cardSpareGroupMode, consoleOutput=consoleOutput, cardRamSize=cardRamSize, auditPassedCount=auditPassedCount, diskVolumeDiskSize=diskVolumeDiskSize, portCardSubType=portCardSubType)
