...
PicType, DiskVolumeUsageLevel, PortMode, SecondaryState, CardId, FirmwareSource, PortId, CardType, DuplexStatus, FlowControlMode, EqActionType, PrimaryState, PortType, AdminState, ShelfId, PortDetectedMode, CardSubType, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PicType", "DiskVolumeUsageLevel", "PortMode", "SecondaryState", "CardId", "FirmwareSource", "PortId", "CardType", "DuplexStatus", "FlowControlMode", "EqActionType", "PrimaryState", "PortType", "AdminState", "ShelfId", "PortDetectedMode", "CardSubType", "UnknownState")
...
TruthValue, DateAndTime, TextualConvention, MacAddress, RowStatus, DisplayString, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "TextualConvention", "MacAddress", "RowStatus", "DisplayString", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFlashSize=cardFlashSize, shelfSwVersion=shelfSwVersion, cardSpareGroupMode=cardSpareGroupMode, cardTestSchedule=cardTestSchedule, cardBootVersion=cardBootVersion, diagTestId=diagTestId, cardEntry=cardEntry, cardFpgaSource=cardFpgaSource, auditLogThrottle=auditLogThrottle, portCardId=portCardId, portPrState=portPrState, cardTestTable=cardTestTable, cardFwVersion=cardFwVersion, cmResetUpTime=cmResetUpTime, auditLogOutput=auditLogOutput, portTrapInh=portTrapInh, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicHwVersion=cardPicHwVersion, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeDriveId=diskVolumeDriveId, cardTestType=cardTestType, auditAutoScheduling=auditAutoScheduling, cardDplxStatus=cardDplxStatus, equipmentError=equipmentError, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeEntry=diskVolumeEntry, shelfName=shelfName, cardAdminState=cardAdminState, cardCpuSpeed=cardCpuSpeed, portNumber=portNumber, portAdminState=portAdminState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMinDsPower=portMinDsPower, auditName=auditName, cmPrimarySID=cmPrimarySID, cardDetected=cardDetected, TestCommand=TestCommand, equipmentCardTestGroup=equipmentCardTestGroup, cardShelfTestIndex=cardShelfTestIndex, systemGeneral=systemGeneral, diskVolumeGroup=diskVolumeGroup, portId=portId, PYSNMP_MODULE_ID=cadEquipmentMib, TestSchedule=TestSchedule, cardDplxStatusChange=cardDplxStatusChange, portMacIfIndex=portMacIfIndex, systemKey=systemKey, equipmentPortGroup=equipmentPortGroup, portDplxStatus=portDplxStatus, cardLastBootSource=cardLastBootSource, diskVolumeTable=diskVolumeTable, cardAction=cardAction, systemClock=systemClock, cardDetectedChange=cardDetectedChange, diskVolumeSize=diskVolumeSize, cardTestResultDesc=cardTestResultDesc, cmDevice=cmDevice, cardTestIndex=cardTestIndex, portLastChangeTime=portLastChangeTime, cadEquipmentMib=cadEquipmentMib, cardSecState=cardSecState, cardNumPorts=cardNumPorts, verboseLevel=verboseLevel, cardSecStateChange=cardSecStateChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCPLDVersion=cardCPLDVersion, diskVolumeName=diskVolumeName, cardCarrierHwVersion=cardCarrierHwVersion, cardTestName=cardTestName, cardNumber=cardNumber, cardPrState=cardPrState, cmResetReason=cmResetReason, portDocsIfIndex=portDocsIfIndex, errEvRowStatus=errEvRowStatus, cmVendor=cmVendor, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditId=auditId, fpgaErrorEventTable=fpgaErrorEventTable, portRxFlowControlMode=portRxFlowControlMode, fpgaErrorEventGroup=fpgaErrorEventGroup, cmUChannelID=cmUChannelID, portDplxStatusChange=portDplxStatusChange, cardShelfId=cardShelfId, cardSerialNum=cardSerialNum, cardSwVersion=cardSwVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestTransId=cardTestTransId, cardType=cardType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditStatus=auditStatus, TestTransactionId=TestTransactionId, consoleOutput=consoleOutput, cardTestScheduleCommand=cardTestScheduleCommand, auditTotalFailedCount=auditTotalFailedCount, portTxFlowControlMode=portTxFlowControlMode, cardTestResultNotification=cardTestResultNotification, equipmentState=equipmentState, portTable=portTable, portType=portType, diskVolumeCardId=diskVolumeCardId, cmRegistrationNotification=cmRegistrationNotification, diskDriveNumber=diskDriveNumber, shelfObjects=shelfObjects, portCardSubType=portCardSubType, portDetectedMode=portDetectedMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portSecStateChange=portSecStateChange, portPrStateChange=portPrStateChange, cardPicDetected=cardPicDetected, cardTrapInh=cardTrapInh, portEntry=portEntry, cardPrStateChange=cardPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentCardGroup=equipmentCardGroup, equipmentAuditGroup=equipmentAuditGroup, cardTestEntry=cardTestEntry, auditTable=auditTable, errEventId=errEventId, cardRamSize=cardRamSize, cardSpareGroupId=cardSpareGroupId, portMode=portMode, auditEntry=auditEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentDiag=equipmentDiag, cardId=cardId, cmResetNotification=cmResetNotification, cardCpuType=cardCpuType, portBgpId=portBgpId, cardPicSerialNum=cardPicSerialNum, diskVolumeNumber=diskVolumeNumber, cardTestCommand=cardTestCommand, cardName=cardName, portCurrDsPower=portCurrDsPower, portConnectorId=portConnectorId, portAction=portAction, cmResetStatus=cmResetStatus, equipmentDiagGroup=equipmentDiagGroup, diskVolumeShelfId=diskVolumeShelfId, equipmentAudit=equipmentAudit, remainInDiagMode=remainInDiagMode, auditDescription=auditDescription, cardCarrierSerialNum=cardCarrierSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, cardTemperature=cardTemperature, cardLastChangeTime=cardLastChangeTime, cardSubType=cardSubType, diskVolumeDiskSize=diskVolumeDiskSize, cardUpTime=cardUpTime, diskVolumeUsageLevel=diskVolumeUsageLevel, cardLastResetReason=cardLastResetReason, auditCardId=auditCardId, cardHwVersion=cardHwVersion, auditPassedCount=auditPassedCount, diskVolumeId=diskVolumeId, cardTemperatureHighError=cardTemperatureHighError, cardBusSpeed=cardBusSpeed, diskVolumeFileName=diskVolumeFileName, cardTestResult=cardTestResult, cardTable=cardTable, portMaxDsPower=portMaxDsPower, equipmentStateGroup=equipmentStateGroup, auditCycleCount=auditCycleCount, cardTemperatureHighWarn=cardTemperatureHighWarn, TestScheduleCommand=TestScheduleCommand, trapCounter=trapCounter, equipmentTraps=equipmentTraps, equipmentShelfGroup=equipmentShelfGroup, cmIpAddress=cmIpAddress, downstreamPowerLoss=downstreamPowerLoss, auditTime=auditTime, auditCommand=auditCommand, portNumChans=portNumChans, systemGeneralGroup=systemGeneralGroup, auditShelfId=auditShelfId, cardLastBootVersion=cardLastBootVersion, portSecState=portSecState, eqDiagConfig=eqDiagConfig, diskVolumeUsageNotification=diskVolumeUsageNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, portShelfId=portShelfId, errEvLoggingEnabled=errEvLoggingEnabled, cardSubDetected=cardSubDetected, cmMacAddress=cmMacAddress, cardFwUpdateStatus=cardFwUpdateStatus, cardTestDescription=cardTestDescription, cmResetInfo=cmResetInfo, diskVolumeTrapInh=diskVolumeTrapInh, portMacAddress=portMacAddress, shelfNumber=shelfNumber, TestResult=TestResult, trapSeverity=trapSeverity, cardTestId=cardTestId, auditTotalPassedCount=auditTotalPassedCount, equipmentTbl=equipmentTbl, TestType=TestType, cardTestTime=cardTestTime, TestId=TestId, auditResult=auditResult, errEvLogLevel=errEvLogLevel, auditFailedCount=auditFailedCount, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetClearNotification=cmResetClearNotification, portTxFlowControlDetected=portTxFlowControlDetected, portDescription=portDescription, cadEquipmentMibConformance=cadEquipmentMibConformance)
