...
FirmwareSource, PicType, CardSubType, ShelfId, PortId, UnknownState, PrimaryState, FlowControlMode, DiskVolumeUsageLevel, PortType, CardId, PortMode, EqActionType, AdminState, DuplexStatus, PortDetectedMode, SecondaryState, CardType = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "PicType", "CardSubType", "ShelfId", "PortId", "UnknownState", "PrimaryState", "FlowControlMode", "DiskVolumeUsageLevel", "PortType", "CardId", "PortMode", "EqActionType", "AdminState", "DuplexStatus", "PortDetectedMode", "SecondaryState", "CardType")
...
TruthValue, TextualConvention, TimeStamp, DisplayString, RowStatus, MacAddress, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TextualConvention", "TimeStamp", "DisplayString", "RowStatus", "MacAddress", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portDplxStatusChange=portDplxStatusChange, portRxFlowControlDetected=portRxFlowControlDetected, equipmentTbl=equipmentTbl, cardTestIndex=cardTestIndex, portNumChans=portNumChans, portNumber=portNumber, cardHwVersion=cardHwVersion, cardCpuType=cardCpuType, auditAutoScheduling=auditAutoScheduling, cardTrapInh=cardTrapInh, auditLogOutput=auditLogOutput, portRxFlowControlMode=portRxFlowControlMode, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, auditStatus=auditStatus, TestId=TestId, cadEquipmentMibGroup=cadEquipmentMibGroup, portAction=portAction, auditName=auditName, auditShelfId=auditShelfId, portSecStateChange=portSecStateChange, diskVolumeDiskSize=diskVolumeDiskSize, auditCycleCount=auditCycleCount, cardTestTable=cardTestTable, cardTestDescription=cardTestDescription, cardTestScheduleCommand=cardTestScheduleCommand, cardTestTime=cardTestTime, auditTotalPassedCount=auditTotalPassedCount, systemClock=systemClock, cardTemperature=cardTemperature, shelfSwVersion=shelfSwVersion, cardShelfId=cardShelfId, TestSchedule=TestSchedule, cardSubDetected=cardSubDetected, portSecState=portSecState, cardTestName=cardTestName, auditLogThrottle=auditLogThrottle, cardTestTransId=cardTestTransId, diskVolumeTable=diskVolumeTable, cmResetReason=cmResetReason, systemGeneral=systemGeneral, cadEquipmentMibConformance=cadEquipmentMibConformance, cardAdminState=cardAdminState, portTable=portTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvRowStatus=errEvRowStatus, cardUpTime=cardUpTime, cardPicDetected=cardPicDetected, portDescription=portDescription, auditCardId=auditCardId, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentAudit=equipmentAudit, portId=portId, TestResult=TestResult, portDocsIfIndex=portDocsIfIndex, diskVolumeFileName=diskVolumeFileName, equipmentCardTestGroup=equipmentCardTestGroup, cardAction=cardAction, diskVolumeUsageLevel=diskVolumeUsageLevel, portCardSubType=portCardSubType, portBgpId=portBgpId, portMacIfIndex=portMacIfIndex, cardLastResetReason=cardLastResetReason, portMinDsPower=portMinDsPower, equipmentState=equipmentState, fpgaErrorEventTable=fpgaErrorEventTable, cardSpareGroupId=cardSpareGroupId, consoleOutput=consoleOutput, cardPicHwVersion=cardPicHwVersion, errEvLogLevel=errEvLogLevel, diskVolumeGroup=diskVolumeGroup, portType=portType, portTxFlowControlMode=portTxFlowControlMode, cardTestType=cardTestType, auditResult=auditResult, TestScheduleCommand=TestScheduleCommand, cardTemperatureHighError=cardTemperatureHighError, diskVolumeSize=diskVolumeSize, auditDescription=auditDescription, equipmentError=equipmentError, cardBootVersion=cardBootVersion, cardDetectedChange=cardDetectedChange, cmResetInfo=cmResetInfo, trapCounter=trapCounter, cardFwVersion=cardFwVersion, equipmentTraps=equipmentTraps, portAdminState=portAdminState, cardLastChangeTime=cardLastChangeTime, auditCommand=auditCommand, auditTotalFailedCount=auditTotalFailedCount, portPrStateChange=portPrStateChange, equipmentStateGroup=equipmentStateGroup, cmRegistrationNotification=cmRegistrationNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, portTrapInh=portTrapInh, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardDetected=cardDetected, auditEntry=auditEntry, diskDriveNumber=diskDriveNumber, eqDiagConfig=eqDiagConfig, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestCommand=cardTestCommand, portDplxStatus=portDplxStatus, cardTestResultDesc=cardTestResultDesc, errEventId=errEventId, cardNumPorts=cardNumPorts, cardTestSchedule=cardTestSchedule, cardLastBootVersion=cardLastBootVersion, auditPassedCount=auditPassedCount, cmResetClearNotification=cmResetClearNotification, cmIpAddress=cmIpAddress, cardSwVersion=cardSwVersion, cardBusSpeed=cardBusSpeed, cardCarrierHwVersion=cardCarrierHwVersion, cardType=cardType, cardCarrierSerialNum=cardCarrierSerialNum, trapSeverity=trapSeverity, shelfObjects=shelfObjects, remainInDiagMode=remainInDiagMode, cardEntry=cardEntry, cardSecState=cardSecState, errEvLoggingEnabled=errEvLoggingEnabled, TestCommand=TestCommand, TestTransactionId=TestTransactionId, auditId=auditId, cardTestResultNotification=cardTestResultNotification, cardPicSerialNum=cardPicSerialNum, auditFailedCount=auditFailedCount, equipmentDiag=equipmentDiag, equipmentCardGroup=equipmentCardGroup, portPrState=portPrState, diskVolumeTrapInh=diskVolumeTrapInh, cardLastBootSource=cardLastBootSource, cmResetStatus=cmResetStatus, cmVendor=cmVendor, cardTable=cardTable, cardPrState=cardPrState, cadEquipmentMib=cadEquipmentMib, cmResetUpTime=cmResetUpTime, equipmentPortGroup=equipmentPortGroup, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeId=diskVolumeId, portMode=portMode, cardSecStateChange=cardSecStateChange, diskVolumeShelfId=diskVolumeShelfId, auditTime=auditTime, cardId=cardId, cardSpareGroupMode=cardSpareGroupMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portEntry=portEntry, diskVolumeNumber=diskVolumeNumber, TestType=TestType, portCardId=portCardId, equipmentDiagGroup=equipmentDiagGroup, verboseLevel=verboseLevel, shelfNumber=shelfNumber, cmPrimarySID=cmPrimarySID, diskVolumeDriveId=diskVolumeDriveId, auditTable=auditTable, cardFpgaSource=cardFpgaSource, cardSubType=cardSubType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmUChannelID=cmUChannelID, cardPrStateChange=cardPrStateChange, equipmentShelfGroup=equipmentShelfGroup, portConnectorId=portConnectorId, diagTestId=diagTestId, cardTestId=cardTestId, cardName=cardName, portDetectedMode=portDetectedMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeName=diskVolumeName, cardTestEntry=cardTestEntry, cardDplxStatusChange=cardDplxStatusChange, cardFwUpdateStatus=cardFwUpdateStatus, cardRamSize=cardRamSize, diskVolumeCardId=diskVolumeCardId, cardDplxStatus=cardDplxStatus, equipmentAuditGroup=equipmentAuditGroup, cardCPLDVersion=cardCPLDVersion, cmResetNotification=cmResetNotification, downstreamPowerLoss=downstreamPowerLoss, cardFlashSize=cardFlashSize, cardCpuSpeed=cardCpuSpeed, cardNumber=cardNumber, cardTempNoReportNotification=cardTempNoReportNotification, portCurrDsPower=portCurrDsPower, systemKey=systemKey, cardSerialNum=cardSerialNum, cardTestResult=cardTestResult, shelfName=shelfName, portLastChangeTime=portLastChangeTime, portShelfId=portShelfId, portTxFlowControlDetected=portTxFlowControlDetected, cmMacAddress=cmMacAddress, portMaxDsPower=portMaxDsPower, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, cardShelfTestIndex=cardShelfTestIndex, portMacAddress=portMacAddress, diskVolumeEntry=diskVolumeEntry, cmDevice=cmDevice, cardCarrierFwVersion=cardCarrierFwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibCompliances=cadEquipmentMibCompliances)
