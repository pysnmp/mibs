...
CardId, PortType, UnknownState, AdminState, DuplexStatus, CardSubType, FirmwareSource, PortDetectedMode, PrimaryState, DiskVolumeUsageLevel, PortMode, PicType, FlowControlMode, SecondaryState, ShelfId, CardType, EqActionType, PortId = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortType", "UnknownState", "AdminState", "DuplexStatus", "CardSubType", "FirmwareSource", "PortDetectedMode", "PrimaryState", "DiskVolumeUsageLevel", "PortMode", "PicType", "FlowControlMode", "SecondaryState", "ShelfId", "CardType", "EqActionType", "PortId")
...
MacAddress, TextualConvention, RowStatus, DisplayString, DateAndTime, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TextualConvention", "RowStatus", "DisplayString", "DateAndTime", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeTable=diskVolumeTable, cardSecStateChange=cardSecStateChange, diskVolumeId=diskVolumeId, cardTestScheduleCommand=cardTestScheduleCommand, diskDriveNumber=diskDriveNumber, portDplxStatusChange=portDplxStatusChange, cmIpAddress=cmIpAddress, portAction=portAction, cardTestSchedule=cardTestSchedule, cmMacAddress=cmMacAddress, cardLastBootVersion=cardLastBootVersion, cardSpareGroupMode=cardSpareGroupMode, cardTestCommand=cardTestCommand, cardSerialNum=cardSerialNum, cardShelfId=cardShelfId, portMinDsPower=portMinDsPower, trapSeverity=trapSeverity, cardTempNoReportNotification=cardTempNoReportNotification, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardRamSize=cardRamSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditCardId=auditCardId, cardTemperatureHighWarn=cardTemperatureHighWarn, systemKey=systemKey, auditAutoScheduling=auditAutoScheduling, cardFlashSize=cardFlashSize, auditName=auditName, cardDetectedChange=cardDetectedChange, cardFpgaSource=cardFpgaSource, auditTotalPassedCount=auditTotalPassedCount, shelfSwVersion=shelfSwVersion, cardTestDescription=cardTestDescription, portDocsIfIndex=portDocsIfIndex, cardEntry=cardEntry, portMacIfIndex=portMacIfIndex, cardCPLDVersion=cardCPLDVersion, diskVolumeFileName=diskVolumeFileName, equipmentShelfGroup=equipmentShelfGroup, equipmentState=equipmentState, cardSubDetected=cardSubDetected, cardNumPorts=cardNumPorts, cardTrapInh=cardTrapInh, cardLastBootSource=cardLastBootSource, errEvLoggingEnabled=errEvLoggingEnabled, cmVendor=cmVendor, cadEquipmentMibConformance=cadEquipmentMibConformance, cmRegistrationNotification=cmRegistrationNotification, errEventId=errEventId, TestId=TestId, auditFailedCount=auditFailedCount, TestSchedule=TestSchedule, equipmentStateGroup=equipmentStateGroup, shelfNumber=shelfNumber, equipmentAuditGroup=equipmentAuditGroup, portBgpId=portBgpId, cardId=cardId, cardTestResultDesc=cardTestResultDesc, cardNumber=cardNumber, cardCpuSpeed=cardCpuSpeed, downstreamPowerLoss=downstreamPowerLoss, cmPrimarySID=cmPrimarySID, cardTestEntry=cardTestEntry, cardPrStateChange=cardPrStateChange, cardUpTime=cardUpTime, portPrState=portPrState, cardBootVersion=cardBootVersion, equipmentAudit=equipmentAudit, equipmentTraps=equipmentTraps, cardTestResultNotification=cardTestResultNotification, cardCpuType=cardCpuType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portSecStateChange=portSecStateChange, portCardId=portCardId, TestResult=TestResult, cardAdminState=cardAdminState, cardSpareGroupId=cardSpareGroupId, remainInDiagMode=remainInDiagMode, cardSubType=cardSubType, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetStatus=cmResetStatus, portNumChans=portNumChans, auditLogThrottle=auditLogThrottle, diskVolumeShelfId=diskVolumeShelfId, TestCommand=TestCommand, errEvRowStatus=errEvRowStatus, portEntry=portEntry, cardCarrierHwVersion=cardCarrierHwVersion, portType=portType, cardPicHwVersion=cardPicHwVersion, portAdminState=portAdminState, fpgaErrorEventGroup=fpgaErrorEventGroup, fpgaErrorEventTable=fpgaErrorEventTable, cardType=cardType, auditTime=auditTime, cardTestName=cardTestName, cardHwVersion=cardHwVersion, equipmentPortGroup=equipmentPortGroup, TestType=TestType, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeSize=diskVolumeSize, cmResetInfo=cmResetInfo, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeDriveId=diskVolumeDriveId, diskVolumeEntry=diskVolumeEntry, portLastChangeTime=portLastChangeTime, diskVolumeUsageNotification=diskVolumeUsageNotification, portDetectedMode=portDetectedMode, portTable=portTable, cardTestTime=cardTestTime, cardCarrierSerialNum=cardCarrierSerialNum, trapCounter=trapCounter, consoleOutput=consoleOutput, fpgaErrorEventEntry=fpgaErrorEventEntry, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentDiagGroup=equipmentDiagGroup, diskVolumeCardId=diskVolumeCardId, systemGeneral=systemGeneral, cardTestId=cardTestId, verboseLevel=verboseLevel, cardTestIndex=cardTestIndex, shelfName=shelfName, diskVolumeUsageLevel=diskVolumeUsageLevel, portMaxDsPower=portMaxDsPower, cardTestTransId=cardTestTransId, portDplxStatus=portDplxStatus, auditId=auditId, cmResetReason=cmResetReason, portRxFlowControlDetected=portRxFlowControlDetected, auditPassedCount=auditPassedCount, cardBusSpeed=cardBusSpeed, cardAction=cardAction, cmResetUpTime=cmResetUpTime, portShelfId=portShelfId, portRxFlowControlMode=portRxFlowControlMode, auditCycleCount=auditCycleCount, cardDetected=cardDetected, diagTestId=diagTestId, systemClock=systemClock, cardPicDetected=cardPicDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardFwUpdateStatus=cardFwUpdateStatus, portConnectorId=portConnectorId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMib=cadEquipmentMib, cardName=cardName, cardSecState=cardSecState, portMacAddress=portMacAddress, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeNumber=diskVolumeNumber, eqDiagConfig=eqDiagConfig, cmDevice=cmDevice, cmUChannelID=cmUChannelID, diskVolumeGroup=diskVolumeGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cmResetNotification=cmResetNotification, cardTestResult=cardTestResult, equipmentDiag=equipmentDiag, cardDplxStatusChange=cardDplxStatusChange, portDescription=portDescription, portTxFlowControlMode=portTxFlowControlMode, diskVolumeName=diskVolumeName, portNumber=portNumber, portPrStateChange=portPrStateChange, auditDescription=auditDescription, portTrapInh=portTrapInh, cardPicSerialNum=cardPicSerialNum, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestScheduleCommand=TestScheduleCommand, auditEntry=auditEntry, portId=portId, portMode=portMode, cmResetClearNotification=cmResetClearNotification, portSecState=portSecState, cardTemperatureHighError=cardTemperatureHighError, diskVolumeDiskSize=diskVolumeDiskSize, cardShelfTestIndex=cardShelfTestIndex, cardTestTable=cardTestTable, TestTransactionId=TestTransactionId, shelfObjects=shelfObjects, cardLastChangeTime=cardLastChangeTime, cardCarrierFwVersion=cardCarrierFwVersion, portCardSubType=portCardSubType, auditLogOutput=auditLogOutput, equipmentError=equipmentError, portCurrDsPower=portCurrDsPower, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPrState=cardPrState, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditCommand=auditCommand, cardDplxStatus=cardDplxStatus, systemGeneralGroup=systemGeneralGroup, cardTestType=cardTestType, diskVolumeTrapInh=diskVolumeTrapInh, auditStatus=auditStatus, auditShelfId=auditShelfId, auditResult=auditResult, equipmentCardGroup=equipmentCardGroup, cardFwVersion=cardFwVersion, auditTable=auditTable, equipmentTbl=equipmentTbl, cardSwVersion=cardSwVersion, cardLastResetReason=cardLastResetReason, cardTable=cardTable, cardTemperature=cardTemperature, errEvLogLevel=errEvLogLevel)
