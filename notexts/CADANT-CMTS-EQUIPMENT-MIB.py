...
DuplexStatus, PortMode, SecondaryState, CardId, DiskVolumeUsageLevel, PortDetectedMode, PicType, FirmwareSource, PortType, UnknownState, PrimaryState, AdminState, PortId, CardType, ShelfId, FlowControlMode, CardSubType, EqActionType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortMode", "SecondaryState", "CardId", "DiskVolumeUsageLevel", "PortDetectedMode", "PicType", "FirmwareSource", "PortType", "UnknownState", "PrimaryState", "AdminState", "PortId", "CardType", "ShelfId", "FlowControlMode", "CardSubType", "EqActionType")
...
TimeStamp, TruthValue, TextualConvention, DateAndTime, RowStatus, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "TextualConvention", "DateAndTime", "RowStatus", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portAction=portAction, cardSecState=cardSecState, portMode=portMode, cardTestCommand=cardTestCommand, cardPrStateChange=cardPrStateChange, diskVolumeEntry=diskVolumeEntry, portDplxStatus=portDplxStatus, cardDetectedChange=cardDetectedChange, cmResetInfo=cmResetInfo, diskVolumeDriveId=diskVolumeDriveId, fpgaErrorEventTable=fpgaErrorEventTable, cardTestTable=cardTestTable, TestSchedule=TestSchedule, diskVolumeFileName=diskVolumeFileName, cardBootVersion=cardBootVersion, TestScheduleCommand=TestScheduleCommand, cmVendor=cmVendor, auditDescription=auditDescription, equipmentDiagGroup=equipmentDiagGroup, cardPicHwVersion=cardPicHwVersion, auditCommand=auditCommand, portNumber=portNumber, portType=portType, equipmentState=equipmentState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portTxFlowControlDetected=portTxFlowControlDetected, auditTotalPassedCount=auditTotalPassedCount, cmMacAddress=cmMacAddress, equipmentAuditGroup=equipmentAuditGroup, cardTestTransId=cardTestTransId, portCurrDsPower=portCurrDsPower, portCardSubType=portCardSubType, cardCarrierFwVersion=cardCarrierFwVersion, errEvRowStatus=errEvRowStatus, TestId=TestId, trapSeverity=trapSeverity, auditId=auditId, errEvLogLevel=errEvLogLevel, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, shelfNumber=shelfNumber, cardFwUpdateStatus=cardFwUpdateStatus, cardCarrierHwVersion=cardCarrierHwVersion, cardTestResultNotification=cardTestResultNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, portMacAddress=portMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, cmIpAddress=cmIpAddress, diskVolumeId=diskVolumeId, cardFwVersion=cardFwVersion, cardPrState=cardPrState, portId=portId, portAdminState=portAdminState, diskVolumeShelfId=diskVolumeShelfId, cardTestType=cardTestType, portDplxStatusChange=portDplxStatusChange, auditTotalFailedCount=auditTotalFailedCount, equipmentAudit=equipmentAudit, portPrStateChange=portPrStateChange, systemGeneralGroup=systemGeneralGroup, cardLastChangeTime=cardLastChangeTime, cardRamSize=cardRamSize, cardCpuSpeed=cardCpuSpeed, diskVolumeGroup=diskVolumeGroup, systemGeneral=systemGeneral, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeTrapInh=diskVolumeTrapInh, errEvLoggingEnabled=errEvLoggingEnabled, cardTrapInh=cardTrapInh, cardCpuType=cardCpuType, cardCPLDVersion=cardCPLDVersion, auditStatus=auditStatus, cardTestTime=cardTestTime, cmRegistrationNotification=cmRegistrationNotification, cardTemperatureHighError=cardTemperatureHighError, portTxFlowControlMode=portTxFlowControlMode, cardLastBootVersion=cardLastBootVersion, remainInDiagMode=remainInDiagMode, cardTestResult=cardTestResult, portDocsIfIndex=portDocsIfIndex, shelfName=shelfName, cardNumPorts=cardNumPorts, portBgpId=portBgpId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, systemKey=systemKey, diskVolumeNumber=diskVolumeNumber, cmResetReason=cmResetReason, portLastChangeTime=portLastChangeTime, auditTime=auditTime, portConnectorId=portConnectorId, portRxFlowControlMode=portRxFlowControlMode, cardEntry=cardEntry, cardSubDetected=cardSubDetected, cardCarrierSerialNum=cardCarrierSerialNum, cmDevice=cmDevice, cardDplxStatusChange=cardDplxStatusChange, portSecStateChange=portSecStateChange, cardFpgaSource=cardFpgaSource, cardNumber=cardNumber, trapCounter=trapCounter, cardTable=cardTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestId=cardTestId, cardFlashSize=cardFlashSize, fpgaErrorEventEntry=fpgaErrorEventEntry, cmUChannelID=cmUChannelID, consoleOutput=consoleOutput, cardId=cardId, cardTestName=cardTestName, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMib=cadEquipmentMib, cardPicDetected=cardPicDetected, diskVolumeCardId=diskVolumeCardId, auditResult=auditResult, equipmentTbl=equipmentTbl, portPrState=portPrState, auditTable=auditTable, cardTemperature=cardTemperature, equipmentCardTestGroup=equipmentCardTestGroup, equipmentShelfGroup=equipmentShelfGroup, cardSpareGroupId=cardSpareGroupId, auditName=auditName, portCardId=portCardId, equipmentStateGroup=equipmentStateGroup, auditCardId=auditCardId, auditAutoScheduling=auditAutoScheduling, cardTestResultDesc=cardTestResultDesc, auditLogOutput=auditLogOutput, cardTestDescription=cardTestDescription, cardSerialNum=cardSerialNum, cardShelfId=cardShelfId, shelfSwVersion=shelfSwVersion, cardUpTime=cardUpTime, cardType=cardType, eqDiagConfig=eqDiagConfig, portDescription=portDescription, TestResult=TestResult, diskVolumeSize=diskVolumeSize, cardName=cardName, auditFailedCount=auditFailedCount, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardAction=cardAction, portDetectedMode=portDetectedMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portMacIfIndex=portMacIfIndex, auditEntry=auditEntry, cardTestIndex=cardTestIndex, systemClock=systemClock, cardDetected=cardDetected, TestCommand=TestCommand, cardHwVersion=cardHwVersion, cmResetUpTime=cmResetUpTime, cardSpareGroupMode=cardSpareGroupMode, cmResetNotification=cmResetNotification, equipmentTraps=equipmentTraps, fpgaErrorEventGroup=fpgaErrorEventGroup, cardAdminState=cardAdminState, cmPrimarySID=cmPrimarySID, diskVolumeUsageNotification=diskVolumeUsageNotification, cardBusSpeed=cardBusSpeed, cardTestEntry=cardTestEntry, portMinDsPower=portMinDsPower, TestTransactionId=TestTransactionId, auditPassedCount=auditPassedCount, portSecState=portSecState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTempNoReportNotification=cardTempNoReportNotification, cardLastBootSource=cardLastBootSource, portEntry=portEntry, portTrapInh=portTrapInh, cardTestScheduleCommand=cardTestScheduleCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestSchedule=cardTestSchedule, auditShelfId=auditShelfId, cardLastResetReason=cardLastResetReason, cardSubType=cardSubType, cardSecStateChange=cardSecStateChange, cmResetClearNotification=cmResetClearNotification, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeTable=diskVolumeTable, diskDriveNumber=diskDriveNumber, errEventId=errEventId, auditLogThrottle=auditLogThrottle, portNumChans=portNumChans, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeName=diskVolumeName, portShelfId=portShelfId, downstreamPowerLoss=downstreamPowerLoss, equipmentError=equipmentError, cardDplxStatus=cardDplxStatus, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, portMaxDsPower=portMaxDsPower, equipmentDiag=equipmentDiag, cmResetStatus=cmResetStatus, auditCycleCount=auditCycleCount, equipmentCardGroup=equipmentCardGroup, shelfObjects=shelfObjects, cardShelfTestIndex=cardShelfTestIndex, verboseLevel=verboseLevel, PYSNMP_MODULE_ID=cadEquipmentMib, cardSwVersion=cardSwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portTable=portTable, TestType=TestType, diagTestId=diagTestId)
