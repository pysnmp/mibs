...
AdminState, PortType, CardId, FlowControlMode, FirmwareSource, PrimaryState, CardType, ShelfId, SecondaryState, PortDetectedMode, PicType, DiskVolumeUsageLevel, PortId, PortMode, EqActionType, CardSubType, UnknownState, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PortType", "CardId", "FlowControlMode", "FirmwareSource", "PrimaryState", "CardType", "ShelfId", "SecondaryState", "PortDetectedMode", "PicType", "DiskVolumeUsageLevel", "PortId", "PortMode", "EqActionType", "CardSubType", "UnknownState", "DuplexStatus")
...
RowStatus, TimeStamp, DisplayString, DateAndTime, MacAddress, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "DisplayString", "DateAndTime", "MacAddress", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmResetNotification=cmResetNotification, cardPrStateChange=cardPrStateChange, auditDescription=auditDescription, portDocsIfIndex=portDocsIfIndex, errEvLogLevel=errEvLogLevel, diskVolumeCardId=diskVolumeCardId, cardPicDetected=cardPicDetected, cardTestCommand=cardTestCommand, cardNumber=cardNumber, cardSecState=cardSecState, cardBootVersion=cardBootVersion, errEventId=errEventId, cardCPLDVersion=cardCPLDVersion, consoleOutput=consoleOutput, cardPicHwVersion=cardPicHwVersion, auditTable=auditTable, portPrStateChange=portPrStateChange, auditLogOutput=auditLogOutput, equipmentStateGroup=equipmentStateGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, verboseLevel=verboseLevel, portPrState=portPrState, cardTestResultNotification=cardTestResultNotification, cardShelfId=cardShelfId, cardFwVersion=cardFwVersion, cmPrimarySID=cmPrimarySID, cardAdminState=cardAdminState, auditTime=auditTime, equipmentDiag=equipmentDiag, TestId=TestId, systemClock=systemClock, cardDplxStatusChange=cardDplxStatusChange, cmResetInfo=cmResetInfo, portCardId=portCardId, portCurrDsPower=portCurrDsPower, systemGeneral=systemGeneral, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeEntry=diskVolumeEntry, cardTestIndex=cardTestIndex, cardTestDescription=cardTestDescription, portConnectorId=portConnectorId, cmResetUpTime=cmResetUpTime, cadEquipmentMibConformance=cadEquipmentMibConformance, auditResult=auditResult, cmResetClearNotification=cmResetClearNotification, equipmentTbl=equipmentTbl, cardTemperatureHighError=cardTemperatureHighError, cardSpareGroupId=cardSpareGroupId, portTxFlowControlMode=portTxFlowControlMode, TestSchedule=TestSchedule, auditEntry=auditEntry, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmDevice=cmDevice, equipmentPortGroup=equipmentPortGroup, diskVolumeDiskSize=diskVolumeDiskSize, cardRamSize=cardRamSize, cardDetected=cardDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmUChannelID=cmUChannelID, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTrapInh=diskVolumeTrapInh, cardLastBootVersion=cardLastBootVersion, cardCarrierSerialNum=cardCarrierSerialNum, portDplxStatus=portDplxStatus, cardTempNoReportNotification=cardTempNoReportNotification, cardDplxStatus=cardDplxStatus, cmIpAddress=cmIpAddress, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeTable=diskVolumeTable, portDetectedMode=portDetectedMode, trapCounter=trapCounter, cardTestResultDesc=cardTestResultDesc, downstreamPowerLoss=downstreamPowerLoss, cardUpTime=cardUpTime, cardTemperature=cardTemperature, cardCpuSpeed=cardCpuSpeed, cardPicSerialNum=cardPicSerialNum, cardTestSchedule=cardTestSchedule, cmVendor=cmVendor, cmResetReason=cmResetReason, diskVolumeGroup=diskVolumeGroup, portRxFlowControlMode=portRxFlowControlMode, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMode=portMode, cardFpgaSource=cardFpgaSource, cardCpuType=cardCpuType, auditName=auditName, cardLastBootSource=cardLastBootSource, portAdminState=portAdminState, cardCarrierHwVersion=cardCarrierHwVersion, portSecStateChange=portSecStateChange, diskVolumeFileName=diskVolumeFileName, portCardSubType=portCardSubType, portTrapInh=portTrapInh, cardTestTable=cardTestTable, cardFwUpdateStatus=cardFwUpdateStatus, errEvLoggingEnabled=errEvLoggingEnabled, cardTestTransId=cardTestTransId, portSecState=portSecState, portTxFlowControlDetected=portTxFlowControlDetected, portTable=portTable, diskVolumeSize=diskVolumeSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMaxDsPower=portMaxDsPower, cardTestScheduleCommand=cardTestScheduleCommand, cardTestResult=cardTestResult, portEntry=portEntry, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portAction=portAction, cardId=cardId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardName=cardName, diskVolumeName=diskVolumeName, auditCommand=auditCommand, cmRegistrationNotification=cmRegistrationNotification, TestResult=TestResult, cardTable=cardTable, cardType=cardType, portShelfId=portShelfId, fpgaErrorEventGroup=fpgaErrorEventGroup, auditTotalFailedCount=auditTotalFailedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portNumber=portNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, auditId=auditId, auditPassedCount=auditPassedCount, equipmentTraps=equipmentTraps, cardLastResetReason=cardLastResetReason, portType=portType, TestScheduleCommand=TestScheduleCommand, portNumChans=portNumChans, cardTestName=cardTestName, cardPrState=cardPrState, shelfSwVersion=shelfSwVersion, auditTotalPassedCount=auditTotalPassedCount, cardSubDetected=cardSubDetected, cardAction=cardAction, cardSpareGroupMode=cardSpareGroupMode, equipmentCardGroup=equipmentCardGroup, equipmentState=equipmentState, diskVolumeId=diskVolumeId, cardTestType=cardTestType, diskVolumeNumber=diskVolumeNumber, portId=portId, cardSwVersion=cardSwVersion, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageNotification=diskVolumeUsageNotification, diskDriveNumber=diskDriveNumber, portRxFlowControlDetected=portRxFlowControlDetected, auditCardId=auditCardId, cardSubType=cardSubType, equipmentAuditGroup=equipmentAuditGroup, portDplxStatusChange=portDplxStatusChange, auditAutoScheduling=auditAutoScheduling, shelfName=shelfName, cardTrapInh=cardTrapInh, equipmentCardTestGroup=equipmentCardTestGroup, TestTransactionId=TestTransactionId, cardShelfTestIndex=cardShelfTestIndex, cardFlashSize=cardFlashSize, remainInDiagMode=remainInDiagMode, cardTestId=cardTestId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, eqDiagConfig=eqDiagConfig, cadEquipmentMibCompliance=cadEquipmentMibCompliance, fpgaErrorEventTable=fpgaErrorEventTable, cardLastChangeTime=cardLastChangeTime, portMinDsPower=portMinDsPower, auditFailedCount=auditFailedCount, shelfNumber=shelfNumber, equipmentDiagGroup=equipmentDiagGroup, cardNumPorts=cardNumPorts, equipmentError=equipmentError, errEvRowStatus=errEvRowStatus, portLastChangeTime=portLastChangeTime, cardEntry=cardEntry, auditCycleCount=auditCycleCount, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeShelfId=diskVolumeShelfId, cmMacAddress=cmMacAddress, cmResetStatus=cmResetStatus, shelfObjects=shelfObjects, portBgpId=portBgpId, cardSecStateChange=cardSecStateChange, auditLogThrottle=auditLogThrottle, cardTestTime=cardTestTime, portDescription=portDescription, auditStatus=auditStatus, trapSeverity=trapSeverity, TestCommand=TestCommand, cardHwVersion=cardHwVersion, systemKey=systemKey, portMacAddress=portMacAddress, cardTestEntry=cardTestEntry, TestType=TestType, portMacIfIndex=portMacIfIndex, cardTemperatureHighWarn=cardTemperatureHighWarn, diagTestId=diagTestId, cadEquipmentMib=cadEquipmentMib, cardSerialNum=cardSerialNum, auditShelfId=auditShelfId, equipmentAudit=equipmentAudit, equipmentShelfGroup=equipmentShelfGroup, cardDetectedChange=cardDetectedChange, cardBusSpeed=cardBusSpeed, systemGeneralGroup=systemGeneralGroup)
