...
UnknownState, PortDetectedMode, PortType, CardSubType, ShelfId, CardId, PrimaryState, DiskVolumeUsageLevel, SecondaryState, CardType, EqActionType, AdminState, DuplexStatus, PortMode, PicType, PortId, FirmwareSource, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "PortDetectedMode", "PortType", "CardSubType", "ShelfId", "CardId", "PrimaryState", "DiskVolumeUsageLevel", "SecondaryState", "CardType", "EqActionType", "AdminState", "DuplexStatus", "PortMode", "PicType", "PortId", "FirmwareSource", "FlowControlMode")
...
RowStatus, TimeStamp, DateAndTime, DisplayString, TruthValue, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "DateAndTime", "DisplayString", "TruthValue", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardAction=cardAction, portMaxDsPower=portMaxDsPower, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTempOverHeatNotification=cardTempOverHeatNotification, TestSchedule=TestSchedule, cardPicDetected=cardPicDetected, diskVolumeEntry=diskVolumeEntry, cmIpAddress=cmIpAddress, cardSecStateChange=cardSecStateChange, cardPrState=cardPrState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portId=portId, cardTempNoReportNotification=cardTempNoReportNotification, cardSecState=cardSecState, cardCarrierFwVersion=cardCarrierFwVersion, portSecStateChange=portSecStateChange, auditAutoScheduling=auditAutoScheduling, systemKey=systemKey, cardFwVersion=cardFwVersion, portMacAddress=portMacAddress, cmMacAddress=cmMacAddress, portMinDsPower=portMinDsPower, TestScheduleCommand=TestScheduleCommand, cardSerialNum=cardSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, portTxFlowControlMode=portTxFlowControlMode, equipmentAudit=equipmentAudit, cardTestResultNotification=cardTestResultNotification, equipmentAuditGroup=equipmentAuditGroup, cardTestTransId=cardTestTransId, diskVolumeDiskSize=diskVolumeDiskSize, cardTestType=cardTestType, cardTestResult=cardTestResult, cardTestTime=cardTestTime, cardTrapInh=cardTrapInh, cmDevice=cmDevice, cardSpareGroupId=cardSpareGroupId, fpgaErrorEventEntry=fpgaErrorEventEntry, portLastChangeTime=portLastChangeTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, trapCounter=trapCounter, cardTemperature=cardTemperature, cardBusSpeed=cardBusSpeed, systemClock=systemClock, auditTable=auditTable, cmUChannelID=cmUChannelID, portType=portType, equipmentCardGroup=equipmentCardGroup, cardFwUpdateStatus=cardFwUpdateStatus, auditTotalFailedCount=auditTotalFailedCount, cardType=cardType, shelfNumber=shelfNumber, portTable=portTable, cardAdminState=cardAdminState, portRxFlowControlMode=portRxFlowControlMode, diskDriveNumber=diskDriveNumber, cardSwVersion=cardSwVersion, cadEquipmentMib=cadEquipmentMib, cardTestTable=cardTestTable, remainInDiagMode=remainInDiagMode, errEvRowStatus=errEvRowStatus, diskVolumeShelfId=diskVolumeShelfId, cardPrStateChange=cardPrStateChange, shelfSwVersion=shelfSwVersion, equipmentDiagGroup=equipmentDiagGroup, cardCpuSpeed=cardCpuSpeed, cardEntry=cardEntry, downstreamPowerLoss=downstreamPowerLoss, TestId=TestId, cardLastResetReason=cardLastResetReason, equipmentStateGroup=equipmentStateGroup, systemGeneralGroup=systemGeneralGroup, shelfName=shelfName, cardDetectedChange=cardDetectedChange, portSecState=portSecState, cmRegistrationNotification=cmRegistrationNotification, portPrState=portPrState, equipmentTraps=equipmentTraps, cardNumber=cardNumber, portMode=portMode, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibConformance=cadEquipmentMibConformance, portAction=portAction, cardTestId=cardTestId, cardUpTime=cardUpTime, TestResult=TestResult, diskVolumeTrapInh=diskVolumeTrapInh, auditPassedCount=auditPassedCount, cardCarrierSerialNum=cardCarrierSerialNum, verboseLevel=verboseLevel, portDocsIfIndex=portDocsIfIndex, diskVolumeSize=diskVolumeSize, TestType=TestType, cardShelfId=cardShelfId, equipmentState=equipmentState, systemGeneral=systemGeneral, auditCommand=auditCommand, cardTestEntry=cardTestEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditName=auditName, auditEntry=auditEntry, errEvRecoveryEnabled=errEvRecoveryEnabled, consoleOutput=consoleOutput, cmResetReason=cmResetReason, cmVendor=cmVendor, cardName=cardName, portPrStateChange=portPrStateChange, cardDetected=cardDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeGroup=diskVolumeGroup, cmResetNotification=cmResetNotification, diagTestId=diagTestId, cardTestName=cardTestName, portEntry=portEntry, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardHwVersion=cardHwVersion, diskVolumeName=diskVolumeName, portNumber=portNumber, errEventId=errEventId, auditFailedCount=auditFailedCount, cardTestScheduleCommand=cardTestScheduleCommand, fpgaErrorEventTable=fpgaErrorEventTable, cardId=cardId, cardBootVersion=cardBootVersion, PYSNMP_MODULE_ID=cadEquipmentMib, shelfObjects=shelfObjects, cardTemperatureHighWarn=cardTemperatureHighWarn, cardDplxStatusChange=cardDplxStatusChange, portShelfId=portShelfId, trapSeverity=trapSeverity, portTxFlowControlDetected=portTxFlowControlDetected, portMacIfIndex=portMacIfIndex, eqDiagConfig=eqDiagConfig, cardTestCommand=cardTestCommand, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetClearNotification=cmResetClearNotification, auditShelfId=auditShelfId, auditId=auditId, diskVolumeNumber=diskVolumeNumber, portBgpId=portBgpId, cardPicHwVersion=cardPicHwVersion, portCardSubType=portCardSubType, diskVolumeDriveId=diskVolumeDriveId, portDetectedMode=portDetectedMode, cardTestDescription=cardTestDescription, diskVolumeCardId=diskVolumeCardId, equipmentError=equipmentError, errEvLogLevel=errEvLogLevel, cardSubType=cardSubType, cardRamSize=cardRamSize, auditCardId=auditCardId, TestTransactionId=TestTransactionId, cardLastBootSource=cardLastBootSource, cardTestResultDesc=cardTestResultDesc, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentTbl=equipmentTbl, cardLastBootVersion=cardLastBootVersion, cardTable=cardTable, diskVolumeFileName=diskVolumeFileName, cardCpuType=cardCpuType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardLastChangeTime=cardLastChangeTime, cardFpgaSource=cardFpgaSource, auditTime=auditTime, equipmentShelfGroup=equipmentShelfGroup, auditTotalPassedCount=auditTotalPassedCount, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeId=diskVolumeId, cardPicSerialNum=cardPicSerialNum, cardDplxStatus=cardDplxStatus, cardCPLDVersion=cardCPLDVersion, portAdminState=portAdminState, equipmentDiag=equipmentDiag, portNumChans=portNumChans, cardTemperatureHighError=cardTemperatureHighError, auditCycleCount=auditCycleCount, portCardId=portCardId, portCurrDsPower=portCurrDsPower, cmResetStatus=cmResetStatus, cardSpareGroupMode=cardSpareGroupMode, cardTestIndex=cardTestIndex, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDescription=portDescription, diskVolumeTable=diskVolumeTable, auditLogOutput=auditLogOutput, cardFlashSize=cardFlashSize, cmResetUpTime=cmResetUpTime, equipmentPortGroup=equipmentPortGroup, cmPrimarySID=cmPrimarySID, auditResult=auditResult, cmResetInfo=cmResetInfo, TestCommand=TestCommand, cardCarrierHwVersion=cardCarrierHwVersion, equipmentCardTestGroup=equipmentCardTestGroup, portTrapInh=portTrapInh, portDplxStatus=portDplxStatus, portConnectorId=portConnectorId, cardShelfTestIndex=cardShelfTestIndex, auditDescription=auditDescription, auditLogThrottle=auditLogThrottle, cardSubDetected=cardSubDetected, cardTestSchedule=cardTestSchedule, auditStatus=auditStatus, diskVolumeUsageNotification=diskVolumeUsageNotification)
