...
PortMode, SecondaryState, EqActionType, DiskVolumeUsageLevel, FlowControlMode, DuplexStatus, PicType, PrimaryState, AdminState, UnknownState, CardType, FirmwareSource, PortDetectedMode, CardSubType, CardId, ShelfId, PortId, PortType = mibBuilder.importSymbols("CADANT-TC", "PortMode", "SecondaryState", "EqActionType", "DiskVolumeUsageLevel", "FlowControlMode", "DuplexStatus", "PicType", "PrimaryState", "AdminState", "UnknownState", "CardType", "FirmwareSource", "PortDetectedMode", "CardSubType", "CardId", "ShelfId", "PortId", "PortType")
...
DateAndTime, TimeStamp, TruthValue, MacAddress, DisplayString, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TimeStamp", "TruthValue", "MacAddress", "DisplayString", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTxFlowControlDetected=portTxFlowControlDetected, equipmentPortGroup=equipmentPortGroup, cardFwVersion=cardFwVersion, TestScheduleCommand=TestScheduleCommand, portTxFlowControlMode=portTxFlowControlMode, cardBusSpeed=cardBusSpeed, cmRegistrationNotification=cmRegistrationNotification, cardAdminState=cardAdminState, cardTable=cardTable, cardSecState=cardSecState, cmIpAddress=cmIpAddress, auditStatus=auditStatus, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, shelfName=shelfName, errEventId=errEventId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardDetectedChange=cardDetectedChange, cardPicSerialNum=cardPicSerialNum, diskVolumeFileName=diskVolumeFileName, portMacAddress=portMacAddress, cardTestSchedule=cardTestSchedule, cardSpareGroupId=cardSpareGroupId, portLastChangeTime=portLastChangeTime, cmResetInfo=cmResetInfo, portDplxStatusChange=portDplxStatusChange, cardTestDescription=cardTestDescription, cardCPLDVersion=cardCPLDVersion, equipmentTraps=equipmentTraps, auditLogOutput=auditLogOutput, equipmentTbl=equipmentTbl, cardCarrierHwVersion=cardCarrierHwVersion, cardTrapInh=cardTrapInh, diskVolumeCardId=diskVolumeCardId, portCardId=portCardId, cardPicDetected=cardPicDetected, eqDiagConfig=eqDiagConfig, cardTestEntry=cardTestEntry, cardNumPorts=cardNumPorts, portRxFlowControlMode=portRxFlowControlMode, equipmentShelfGroup=equipmentShelfGroup, cardTestTable=cardTestTable, auditPassedCount=auditPassedCount, portDplxStatus=portDplxStatus, diskVolumeEntry=diskVolumeEntry, cardPrStateChange=cardPrStateChange, equipmentStateGroup=equipmentStateGroup, cardId=cardId, auditFailedCount=auditFailedCount, cardSecStateChange=cardSecStateChange, diskVolumeName=diskVolumeName, cmResetNotification=cmResetNotification, cadEquipmentMib=cadEquipmentMib, diskVolumeShelfId=diskVolumeShelfId, portMinDsPower=portMinDsPower, cardShelfId=cardShelfId, diskDriveNumber=diskDriveNumber, auditEntry=auditEntry, cardCpuSpeed=cardCpuSpeed, equipmentError=equipmentError, systemGeneralGroup=systemGeneralGroup, portAdminState=portAdminState, portType=portType, cardAction=cardAction, portBgpId=portBgpId, cardTestId=cardTestId, errEvLogLevel=errEvLogLevel, cardLastBootSource=cardLastBootSource, portEntry=portEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, trapCounter=trapCounter, diskVolumeNumber=diskVolumeNumber, auditLogThrottle=auditLogThrottle, cardTestResultNotification=cardTestResultNotification, cardNumber=cardNumber, TestId=TestId, cardTempOverHeatNotification=cardTempOverHeatNotification, portShelfId=portShelfId, diagTestId=diagTestId, shelfNumber=shelfNumber, portNumber=portNumber, cardTestResult=cardTestResult, cardTestTransId=cardTestTransId, PYSNMP_MODULE_ID=cadEquipmentMib, cardHwVersion=cardHwVersion, cardSubDetected=cardSubDetected, TestSchedule=TestSchedule, cardLastChangeTime=cardLastChangeTime, downstreamPowerLoss=downstreamPowerLoss, TestCommand=TestCommand, cardTestCommand=cardTestCommand, TestTransactionId=TestTransactionId, auditTotalPassedCount=auditTotalPassedCount, portDescription=portDescription, diskVolumeDriveId=diskVolumeDriveId, equipmentState=equipmentState, errEvRecoveryEnabled=errEvRecoveryEnabled, fpgaErrorEventEntry=fpgaErrorEventEntry, cardName=cardName, portCardSubType=portCardSubType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portMacIfIndex=portMacIfIndex, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPrState=cardPrState, cardSubType=cardSubType, portSecStateChange=portSecStateChange, cardFlashSize=cardFlashSize, cmUChannelID=cmUChannelID, cardDplxStatus=cardDplxStatus, portRxFlowControlDetected=portRxFlowControlDetected, equipmentCardGroup=equipmentCardGroup, auditAutoScheduling=auditAutoScheduling, cmMacAddress=cmMacAddress, cmPrimarySID=cmPrimarySID, systemKey=systemKey, TestType=TestType, cardTemperature=cardTemperature, portNumChans=portNumChans, auditTime=auditTime, cardTestType=cardTestType, cmDevice=cmDevice, equipmentCardTestGroup=equipmentCardTestGroup, cardTestResultDesc=cardTestResultDesc, auditShelfId=auditShelfId, auditCycleCount=auditCycleCount, cmResetUpTime=cmResetUpTime, diskVolumeTable=diskVolumeTable, cardFpgaSource=cardFpgaSource, equipmentAudit=equipmentAudit, auditCommand=auditCommand, verboseLevel=verboseLevel, cardTemperatureHighError=cardTemperatureHighError, cardShelfTestIndex=cardShelfTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, remainInDiagMode=remainInDiagMode, cardType=cardType, errEvLoggingEnabled=errEvLoggingEnabled, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDetectedMode=portDetectedMode, shelfSwVersion=shelfSwVersion, diskVolumeDiskSize=diskVolumeDiskSize, consoleOutput=consoleOutput, auditTotalFailedCount=auditTotalFailedCount, cmResetClearNotification=cmResetClearNotification, cardTestTime=cardTestTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, portId=portId, auditDescription=auditDescription, systemGeneral=systemGeneral, errEvRowStatus=errEvRowStatus, diskVolumeSize=diskVolumeSize, portPrStateChange=portPrStateChange, diskVolumeTrapInh=diskVolumeTrapInh, cardTempNoReportNotification=cardTempNoReportNotification, cardRamSize=cardRamSize, portPrState=portPrState, cardPicHwVersion=cardPicHwVersion, portTrapInh=portTrapInh, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditName=auditName, cmResetReason=cmResetReason, equipmentDiag=equipmentDiag, cardSwVersion=cardSwVersion, cardFwUpdateStatus=cardFwUpdateStatus, equipmentAuditGroup=equipmentAuditGroup, cardCarrierSerialNum=cardCarrierSerialNum, portMode=portMode, cmResetStatus=cmResetStatus, cardLastBootVersion=cardLastBootVersion, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeUsageNotification=diskVolumeUsageNotification, TestResult=TestResult, cardUpTime=cardUpTime, auditTable=auditTable, cardBootVersion=cardBootVersion, diskVolumeGroup=diskVolumeGroup, cardEntry=cardEntry, auditResult=auditResult, cardLastResetReason=cardLastResetReason, portTable=portTable, auditCardId=auditCardId, equipmentDiagGroup=equipmentDiagGroup, fpgaErrorEventTable=fpgaErrorEventTable, cardTestIndex=cardTestIndex, cardSerialNum=cardSerialNum, cardTestName=cardTestName, portConnectorId=portConnectorId, trapSeverity=trapSeverity, cardDplxStatusChange=cardDplxStatusChange, cardDetected=cardDetected, cardTemperatureHighWarn=cardTemperatureHighWarn, portMaxDsPower=portMaxDsPower, systemClock=systemClock, cardCarrierFwVersion=cardCarrierFwVersion, auditId=auditId, cadEquipmentMibConformance=cadEquipmentMibConformance, portAction=portAction, diskVolumeUsageLevel=diskVolumeUsageLevel, portCurrDsPower=portCurrDsPower, cardSpareGroupMode=cardSpareGroupMode, portDocsIfIndex=portDocsIfIndex, diskVolumeId=diskVolumeId, shelfObjects=shelfObjects, portSecState=portSecState, cmVendor=cmVendor, cardCpuType=cardCpuType)
