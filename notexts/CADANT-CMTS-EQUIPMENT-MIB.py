...
CardId, FlowControlMode, DuplexStatus, PortMode, DiskVolumeUsageLevel, PortDetectedMode, EqActionType, PicType, PortType, UnknownState, CardType, FirmwareSource, PrimaryState, SecondaryState, CardSubType, ShelfId, PortId, AdminState = mibBuilder.importSymbols("CADANT-TC", "CardId", "FlowControlMode", "DuplexStatus", "PortMode", "DiskVolumeUsageLevel", "PortDetectedMode", "EqActionType", "PicType", "PortType", "UnknownState", "CardType", "FirmwareSource", "PrimaryState", "SecondaryState", "CardSubType", "ShelfId", "PortId", "AdminState")
...
DisplayString, DateAndTime, RowStatus, TruthValue, MacAddress, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "DateAndTime", "RowStatus", "TruthValue", "MacAddress", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portCurrDsPower=portCurrDsPower, portMinDsPower=portMinDsPower, equipmentAuditGroup=equipmentAuditGroup, portSecStateChange=portSecStateChange, shelfSwVersion=shelfSwVersion, cadEquipmentMib=cadEquipmentMib, errEvLogLevel=errEvLogLevel, cardSerialNum=cardSerialNum, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditTotalPassedCount=auditTotalPassedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSpareGroupMode=cardSpareGroupMode, systemClock=systemClock, PYSNMP_MODULE_ID=cadEquipmentMib, cardBusSpeed=cardBusSpeed, auditLogOutput=auditLogOutput, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portEntry=portEntry, cardTestResult=cardTestResult, equipmentTraps=equipmentTraps, cardTemperatureHighWarn=cardTemperatureHighWarn, cardAction=cardAction, TestCommand=TestCommand, errEventId=errEventId, cardEntry=cardEntry, diskDriveNumber=diskDriveNumber, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, downstreamPowerLoss=downstreamPowerLoss, eqDiagConfig=eqDiagConfig, auditFailedCount=auditFailedCount, cmVendor=cmVendor, portCardId=portCardId, portNumber=portNumber, cardPrStateChange=cardPrStateChange, TestType=TestType, equipmentDiagGroup=equipmentDiagGroup, portMaxDsPower=portMaxDsPower, equipmentDiag=equipmentDiag, cardFpgaSource=cardFpgaSource, cardNumPorts=cardNumPorts, cardBootVersion=cardBootVersion, portTrapInh=portTrapInh, cardCarrierFwVersion=cardCarrierFwVersion, equipmentShelfGroup=equipmentShelfGroup, consoleOutput=consoleOutput, diskVolumeCardId=diskVolumeCardId, auditTable=auditTable, cardLastResetReason=cardLastResetReason, auditPassedCount=auditPassedCount, systemKey=systemKey, portMacIfIndex=portMacIfIndex, shelfName=shelfName, portAdminState=portAdminState, TestResult=TestResult, portDetectedMode=portDetectedMode, auditCycleCount=auditCycleCount, cardSubDetected=cardSubDetected, portId=portId, cardPicSerialNum=cardPicSerialNum, portShelfId=portShelfId, auditShelfId=auditShelfId, trapSeverity=trapSeverity, diskVolumeName=diskVolumeName, diskVolumeNumber=diskVolumeNumber, cmRegistrationNotification=cmRegistrationNotification, cardDplxStatus=cardDplxStatus, portLastChangeTime=portLastChangeTime, portNumChans=portNumChans, cardSecStateChange=cardSecStateChange, cardTempNoReportNotification=cardTempNoReportNotification, auditStatus=auditStatus, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portSecState=portSecState, auditId=auditId, cmMacAddress=cmMacAddress, equipmentError=equipmentError, cardTestId=cardTestId, cmResetReason=cmResetReason, diskVolumeGroup=diskVolumeGroup, cardCPLDVersion=cardCPLDVersion, cardTestTable=cardTestTable, cardUpTime=cardUpTime, shelfObjects=shelfObjects, cmResetUpTime=cmResetUpTime, cardTestResultNotification=cardTestResultNotification, cardNumber=cardNumber, cardTestTransId=cardTestTransId, equipmentPortGroup=equipmentPortGroup, portTxFlowControlMode=portTxFlowControlMode, auditTotalFailedCount=auditTotalFailedCount, errEvLoggingEnabled=errEvLoggingEnabled, TestScheduleCommand=TestScheduleCommand, cardTable=cardTable, cmResetNotification=cmResetNotification, cardTestResultDesc=cardTestResultDesc, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardTestGroup=equipmentCardTestGroup, portDplxStatusChange=portDplxStatusChange, auditName=auditName, portAction=portAction, cardTestCommand=cardTestCommand, portMacAddress=portMacAddress, equipmentTbl=equipmentTbl, equipmentState=equipmentState, cardAdminState=cardAdminState, verboseLevel=verboseLevel, portMode=portMode, cardTempOverHeatNotification=cardTempOverHeatNotification, portRxFlowControlMode=portRxFlowControlMode, auditLogThrottle=auditLogThrottle, cmUChannelID=cmUChannelID, cmResetInfo=cmResetInfo, cardLastChangeTime=cardLastChangeTime, errEvRecoveryEnabled=errEvRecoveryEnabled, cardShelfId=cardShelfId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCarrierHwVersion=cardCarrierHwVersion, TestId=TestId, cardTestType=cardTestType, cardTestDescription=cardTestDescription, portDplxStatus=portDplxStatus, portTable=portTable, cardFwVersion=cardFwVersion, cardTemperature=cardTemperature, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFlashSize=cardFlashSize, shelfNumber=shelfNumber, cardDetected=cardDetected, cardCpuSpeed=cardCpuSpeed, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicHwVersion=cardPicHwVersion, cardId=cardId, systemGeneral=systemGeneral, diskVolumeFileName=diskVolumeFileName, portBgpId=portBgpId, cardTestSchedule=cardTestSchedule, diskVolumeUsageLevel=diskVolumeUsageLevel, portCardSubType=portCardSubType, diskVolumeTrapInh=diskVolumeTrapInh, cardTestIndex=cardTestIndex, portDescription=portDescription, cmPrimarySID=cmPrimarySID, diskVolumeDiskSize=diskVolumeDiskSize, cardTestScheduleCommand=cardTestScheduleCommand, portConnectorId=portConnectorId, auditDescription=auditDescription, TestSchedule=TestSchedule, cmResetClearNotification=cmResetClearNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portType=portType, cardCpuType=cardCpuType, diskVolumeId=diskVolumeId, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeUsageNotification=diskVolumeUsageNotification, trapCounter=trapCounter, cardLastBootVersion=cardLastBootVersion, cardSwVersion=cardSwVersion, diskVolumeDriveId=diskVolumeDriveId, cardTemperatureHighError=cardTemperatureHighError, portPrState=portPrState, portDocsIfIndex=portDocsIfIndex, cardShelfTestIndex=cardShelfTestIndex, portRxFlowControlDetected=portRxFlowControlDetected, cardTrapInh=cardTrapInh, cardTestName=cardTestName, cardRamSize=cardRamSize, systemGeneralGroup=systemGeneralGroup, cardTestTime=cardTestTime, cardHwVersion=cardHwVersion, cardLastBootSource=cardLastBootSource, diagTestId=diagTestId, cardDetectedChange=cardDetectedChange, cardSpareGroupId=cardSpareGroupId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardName=cardName, diskVolumeSize=diskVolumeSize, auditCommand=auditCommand, auditResult=auditResult, equipmentStateGroup=equipmentStateGroup, equipmentCardGroup=equipmentCardGroup, auditTime=auditTime, cmIpAddress=cmIpAddress, cmDevice=cmDevice, equipmentAudit=equipmentAudit, cardPrState=cardPrState, diskVolumeTable=diskVolumeTable, auditAutoScheduling=auditAutoScheduling, auditEntry=auditEntry, cmResetStatus=cmResetStatus, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeShelfId=diskVolumeShelfId, cardDplxStatusChange=cardDplxStatusChange, cardTestEntry=cardTestEntry, cardCarrierSerialNum=cardCarrierSerialNum, cardSecState=cardSecState, TestTransactionId=TestTransactionId, cardType=cardType, diskVolumeEntry=diskVolumeEntry, remainInDiagMode=remainInDiagMode, errEvRowStatus=errEvRowStatus, cardPicDetected=cardPicDetected, cardSubType=cardSubType, portPrStateChange=portPrStateChange, auditCardId=auditCardId)
