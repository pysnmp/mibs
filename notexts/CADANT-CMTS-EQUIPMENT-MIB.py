...
AdminState, PortDetectedMode, DuplexStatus, PortId, PrimaryState, FirmwareSource, CardSubType, PortMode, CardId, PortType, ShelfId, UnknownState, FlowControlMode, CardType, PicType, SecondaryState, DiskVolumeUsageLevel, EqActionType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PortDetectedMode", "DuplexStatus", "PortId", "PrimaryState", "FirmwareSource", "CardSubType", "PortMode", "CardId", "PortType", "ShelfId", "UnknownState", "FlowControlMode", "CardType", "PicType", "SecondaryState", "DiskVolumeUsageLevel", "EqActionType")
...
TextualConvention, RowStatus, MacAddress, DisplayString, TruthValue, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "MacAddress", "DisplayString", "TruthValue", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeGroup=diskVolumeGroup, TestSchedule=TestSchedule, errEvRowStatus=errEvRowStatus, cmResetClearNotification=cmResetClearNotification, auditTime=auditTime, cardName=cardName, portTable=portTable, TestType=TestType, cardHwVersion=cardHwVersion, cardNumber=cardNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, portMinDsPower=portMinDsPower, cardDetectedChange=cardDetectedChange, equipmentDiagGroup=equipmentDiagGroup, cardTable=cardTable, cardId=cardId, equipmentState=equipmentState, cardCarrierSerialNum=cardCarrierSerialNum, cardTempOverHeatNotification=cardTempOverHeatNotification, TestResult=TestResult, cardPrState=cardPrState, remainInDiagMode=remainInDiagMode, auditTable=auditTable, portDplxStatus=portDplxStatus, portCurrDsPower=portCurrDsPower, equipmentCardGroup=equipmentCardGroup, equipmentError=equipmentError, trapCounter=trapCounter, auditName=auditName, equipmentCardTestGroup=equipmentCardTestGroup, trapSeverity=trapSeverity, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portAdminState=portAdminState, cardTestEntry=cardTestEntry, auditCycleCount=auditCycleCount, auditAutoScheduling=auditAutoScheduling, cardTestScheduleCommand=cardTestScheduleCommand, cardShelfId=cardShelfId, diskVolumeDiskSize=diskVolumeDiskSize, consoleOutput=consoleOutput, cardAction=cardAction, cardFwVersion=cardFwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, portCardId=portCardId, verboseLevel=verboseLevel, cardLastChangeTime=cardLastChangeTime, portMacIfIndex=portMacIfIndex, eqDiagConfig=eqDiagConfig, cardSpareGroupMode=cardSpareGroupMode, cmDevice=cmDevice, equipmentTraps=equipmentTraps, portDetectedMode=portDetectedMode, cmPrimarySID=cmPrimarySID, diskVolumeTrapInh=diskVolumeTrapInh, portBgpId=portBgpId, cardEntry=cardEntry, cardSerialNum=cardSerialNum, auditFailedCount=auditFailedCount, cardTemperature=cardTemperature, portTxFlowControlDetected=portTxFlowControlDetected, auditEntry=auditEntry, portLastChangeTime=portLastChangeTime, portRxFlowControlDetected=portRxFlowControlDetected, cmRegistrationNotification=cmRegistrationNotification, cardTestIndex=cardTestIndex, equipmentPortGroup=equipmentPortGroup, equipmentDiag=equipmentDiag, cardDplxStatus=cardDplxStatus, cardPicDetected=cardPicDetected, cardTestResult=cardTestResult, auditPassedCount=auditPassedCount, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmMacAddress=cmMacAddress, diskVolumeTable=diskVolumeTable, equipmentAudit=equipmentAudit, equipmentShelfGroup=equipmentShelfGroup, systemClock=systemClock, cardDetected=cardDetected, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetReason=cmResetReason, auditStatus=auditStatus, cardTestResultDesc=cardTestResultDesc, portCardSubType=portCardSubType, errEventId=errEventId, cardFwUpdateStatus=cardFwUpdateStatus, cardNumPorts=cardNumPorts, portDplxStatusChange=portDplxStatusChange, portNumChans=portNumChans, systemGeneralGroup=systemGeneralGroup, cardUpTime=cardUpTime, shelfSwVersion=shelfSwVersion, portSecStateChange=portSecStateChange, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestId=cardTestId, errEvLogLevel=errEvLogLevel, systemGeneral=systemGeneral, cardSubType=cardSubType, auditLogThrottle=auditLogThrottle, cardTestSchedule=cardTestSchedule, fpgaErrorEventGroup=fpgaErrorEventGroup, portConnectorId=portConnectorId, diskVolumeShelfId=diskVolumeShelfId, portDocsIfIndex=portDocsIfIndex, auditLogOutput=auditLogOutput, cmIpAddress=cmIpAddress, shelfName=shelfName, cardTestResultNotification=cardTestResultNotification, equipmentStateGroup=equipmentStateGroup, cardTestTransId=cardTestTransId, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetInfo=cmResetInfo, TestScheduleCommand=TestScheduleCommand, cardType=cardType, portPrStateChange=portPrStateChange, shelfNumber=shelfNumber, portTrapInh=portTrapInh, cmUChannelID=cmUChannelID, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditCardId=auditCardId, cardDplxStatusChange=cardDplxStatusChange, cardCPLDVersion=cardCPLDVersion, diskVolumeSize=diskVolumeSize, portDescription=portDescription, cardBusSpeed=cardBusSpeed, cardTestDescription=cardTestDescription, cardRamSize=cardRamSize, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentTbl=equipmentTbl, cardTestTime=cardTestTime, shelfObjects=shelfObjects, cardLastResetReason=cardLastResetReason, cardPicSerialNum=cardPicSerialNum, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portId=portId, cadEquipmentMib=cadEquipmentMib, diskVolumeName=diskVolumeName, cardFlashSize=cardFlashSize, portEntry=portEntry, systemKey=systemKey, cardCarrierHwVersion=cardCarrierHwVersion, cardPrStateChange=cardPrStateChange, diskVolumeEntry=diskVolumeEntry, diskDriveNumber=diskDriveNumber, cardSecStateChange=cardSecStateChange, fpgaErrorEventTable=fpgaErrorEventTable, cardLastBootSource=cardLastBootSource, diskVolumeId=diskVolumeId, cardSecState=cardSecState, TestId=TestId, cardLastBootVersion=cardLastBootVersion, auditId=auditId, portShelfId=portShelfId, cardTestType=cardTestType, errEvLoggingEnabled=errEvLoggingEnabled, cardSwVersion=cardSwVersion, auditResult=auditResult, portAction=portAction, cardCpuType=cardCpuType, diskVolumeCardId=diskVolumeCardId, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeDriveId=diskVolumeDriveId, auditCommand=auditCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetUpTime=cmResetUpTime, downstreamPowerLoss=downstreamPowerLoss, portSecState=portSecState, cardAdminState=cardAdminState, portRxFlowControlMode=portRxFlowControlMode, cardCarrierFwVersion=cardCarrierFwVersion, portNumber=portNumber, portMacAddress=portMacAddress, auditShelfId=auditShelfId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardBootVersion=cardBootVersion, cardPicHwVersion=cardPicHwVersion, cardShelfTestIndex=cardShelfTestIndex, cardSubDetected=cardSubDetected, diskVolumeNumber=diskVolumeNumber, cardSpareGroupId=cardSpareGroupId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestName=cardTestName, auditDescription=auditDescription, cardTrapInh=cardTrapInh, portType=portType, equipmentAuditGroup=equipmentAuditGroup, portMaxDsPower=portMaxDsPower, auditTotalPassedCount=auditTotalPassedCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestTransactionId=TestTransactionId, cmVendor=cmVendor, portTxFlowControlMode=portTxFlowControlMode, cardFpgaSource=cardFpgaSource, cmResetNotification=cmResetNotification, portMode=portMode, diskVolumeFileName=diskVolumeFileName, diagTestId=diagTestId, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestCommand=cardTestCommand, portPrState=portPrState, cmResetStatus=cmResetStatus, TestCommand=TestCommand, cardTestTable=cardTestTable, cardCpuSpeed=cardCpuSpeed)
