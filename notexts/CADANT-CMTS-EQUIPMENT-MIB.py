...
FirmwareSource, UnknownState, SecondaryState, CardId, PrimaryState, AdminState, FlowControlMode, DiskVolumeUsageLevel, PicType, PortDetectedMode, PortType, PortMode, ShelfId, CardType, CardSubType, EqActionType, DuplexStatus, PortId = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "UnknownState", "SecondaryState", "CardId", "PrimaryState", "AdminState", "FlowControlMode", "DiskVolumeUsageLevel", "PicType", "PortDetectedMode", "PortType", "PortMode", "ShelfId", "CardType", "CardSubType", "EqActionType", "DuplexStatus", "PortId")
...
TextualConvention, TruthValue, DisplayString, RowStatus, MacAddress, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "DisplayString", "RowStatus", "MacAddress", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResultDesc=cardTestResultDesc, cmDevice=cmDevice, equipmentAuditGroup=equipmentAuditGroup, equipmentStateGroup=equipmentStateGroup, cardTestResultNotification=cardTestResultNotification, portAdminState=portAdminState, cardId=cardId, cardTestTime=cardTestTime, auditLogOutput=auditLogOutput, errEventId=errEventId, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentState=equipmentState, cardShelfId=cardShelfId, cmMacAddress=cmMacAddress, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSubType=cardSubType, TestCommand=TestCommand, systemClock=systemClock, diskVolumeTable=diskVolumeTable, fpgaErrorEventEntry=fpgaErrorEventEntry, portTxFlowControlDetected=portTxFlowControlDetected, portCurrDsPower=portCurrDsPower, portTable=portTable, cmResetInfo=cmResetInfo, equipmentCardTestGroup=equipmentCardTestGroup, cardBootVersion=cardBootVersion, portConnectorId=portConnectorId, auditTotalFailedCount=auditTotalFailedCount, TestTransactionId=TestTransactionId, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentTraps=equipmentTraps, cardTestResult=cardTestResult, cardTestIndex=cardTestIndex, cmResetReason=cmResetReason, cardBusSpeed=cardBusSpeed, portCardSubType=portCardSubType, shelfObjects=shelfObjects, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditCycleCount=auditCycleCount, cardTestTransId=cardTestTransId, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestDescription=cardTestDescription, auditId=auditId, cmRegistrationNotification=cmRegistrationNotification, cardTable=cardTable, portAction=portAction, cardLastChangeTime=cardLastChangeTime, portDescription=portDescription, auditPassedCount=auditPassedCount, cardCarrierHwVersion=cardCarrierHwVersion, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeFileName=diskVolumeFileName, diskVolumeTrapInh=diskVolumeTrapInh, cardTemperature=cardTemperature, portShelfId=portShelfId, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentCardGroup=equipmentCardGroup, cardType=cardType, equipmentPortGroup=equipmentPortGroup, cardSerialNum=cardSerialNum, portTxFlowControlMode=portTxFlowControlMode, auditDescription=auditDescription, equipmentDiagGroup=equipmentDiagGroup, cardPicHwVersion=cardPicHwVersion, trapCounter=trapCounter, downstreamPowerLoss=downstreamPowerLoss, portEntry=portEntry, portType=portType, portTrapInh=portTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, auditName=auditName, auditFailedCount=auditFailedCount, TestType=TestType, cmResetUpTime=cmResetUpTime, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeId=diskVolumeId, auditTable=auditTable, cardTemperatureHighError=cardTemperatureHighError, cardTestTable=cardTestTable, diskVolumeGroup=diskVolumeGroup, trapSeverity=trapSeverity, cardUpTime=cardUpTime, portMode=portMode, auditShelfId=auditShelfId, cardTrapInh=cardTrapInh, cardAction=cardAction, portSecState=portSecState, diskVolumeNumber=diskVolumeNumber, portPrState=portPrState, diskVolumeSize=diskVolumeSize, TestScheduleCommand=TestScheduleCommand, TestId=TestId, portMacAddress=portMacAddress, portNumber=portNumber, cmResetClearNotification=cmResetClearNotification, cardNumPorts=cardNumPorts, cmIpAddress=cmIpAddress, cardEntry=cardEntry, cardSwVersion=cardSwVersion, shelfSwVersion=shelfSwVersion, auditTotalPassedCount=auditTotalPassedCount, equipmentShelfGroup=equipmentShelfGroup, cardTestName=cardTestName, cardDplxStatusChange=cardDplxStatusChange, errEvRowStatus=errEvRowStatus, portMaxDsPower=portMaxDsPower, eqDiagConfig=eqDiagConfig, auditCardId=auditCardId, cardDplxStatus=cardDplxStatus, equipmentError=equipmentError, auditResult=auditResult, cmResetStatus=cmResetStatus, cmUChannelID=cmUChannelID, cardTestCommand=cardTestCommand, cardFwVersion=cardFwVersion, cardNumber=cardNumber, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portCardId=portCardId, diskVolumeDriveId=diskVolumeDriveId, TestResult=TestResult, cardDetected=cardDetected, cardFwUpdateStatus=cardFwUpdateStatus, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeEntry=diskVolumeEntry, cardSubDetected=cardSubDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardName=cardName, cardLastBootVersion=cardLastBootVersion, diagTestId=diagTestId, auditTime=auditTime, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDetectedMode=portDetectedMode, cardLastResetReason=cardLastResetReason, portDplxStatus=portDplxStatus, verboseLevel=verboseLevel, auditCommand=auditCommand, auditAutoScheduling=auditAutoScheduling, portMacIfIndex=portMacIfIndex, cardSecState=cardSecState, portRxFlowControlMode=portRxFlowControlMode, cardCarrierFwVersion=cardCarrierFwVersion, portDplxStatusChange=portDplxStatusChange, cardTestScheduleCommand=cardTestScheduleCommand, cardLastBootSource=cardLastBootSource, diskVolumeDiskSize=diskVolumeDiskSize, systemGeneralGroup=systemGeneralGroup, cardFlashSize=cardFlashSize, cardSpareGroupMode=cardSpareGroupMode, equipmentTbl=equipmentTbl, cardPicSerialNum=cardPicSerialNum, portNumChans=portNumChans, portMinDsPower=portMinDsPower, cardTestId=cardTestId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardFpgaSource=cardFpgaSource, cadEquipmentMib=cadEquipmentMib, cardSecStateChange=cardSecStateChange, cmResetNotification=cmResetNotification, shelfName=shelfName, cardTestSchedule=cardTestSchedule, equipmentAudit=equipmentAudit, portBgpId=portBgpId, diskVolumeCardId=diskVolumeCardId, cardTestEntry=cardTestEntry, cardTestType=cardTestType, auditLogThrottle=auditLogThrottle, systemGeneral=systemGeneral, cardRamSize=cardRamSize, cadEquipmentMibConformance=cadEquipmentMibConformance, cardPrStateChange=cardPrStateChange, cardPicDetected=cardPicDetected, cardHwVersion=cardHwVersion, cardDetectedChange=cardDetectedChange, portPrStateChange=portPrStateChange, cmPrimarySID=cmPrimarySID, portId=portId, errEvRecoveryEnabled=errEvRecoveryEnabled, errEvLogLevel=errEvLogLevel, cardCpuSpeed=cardCpuSpeed, cardAdminState=cardAdminState, cardCpuType=cardCpuType, cardCarrierSerialNum=cardCarrierSerialNum, auditStatus=auditStatus, portSecStateChange=portSecStateChange, diskDriveNumber=diskDriveNumber, equipmentDiag=equipmentDiag, TestSchedule=TestSchedule, cardCPLDVersion=cardCPLDVersion, shelfNumber=shelfNumber, diskVolumeShelfId=diskVolumeShelfId, remainInDiagMode=remainInDiagMode, cardPrState=cardPrState, systemKey=systemKey, cardTemperatureHighWarn=cardTemperatureHighWarn, cardShelfTestIndex=cardShelfTestIndex, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portLastChangeTime=portLastChangeTime, auditEntry=auditEntry, diskVolumeName=diskVolumeName, consoleOutput=consoleOutput, cmVendor=cmVendor, errEvLoggingEnabled=errEvLoggingEnabled, portDocsIfIndex=portDocsIfIndex)
