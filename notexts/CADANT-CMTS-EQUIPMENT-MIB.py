...
PicType, FirmwareSource, PortId, SecondaryState, AdminState, FlowControlMode, DuplexStatus, UnknownState, CardType, PortType, DiskVolumeUsageLevel, CardSubType, PortMode, EqActionType, CardId, PrimaryState, PortDetectedMode, ShelfId = mibBuilder.importSymbols("CADANT-TC", "PicType", "FirmwareSource", "PortId", "SecondaryState", "AdminState", "FlowControlMode", "DuplexStatus", "UnknownState", "CardType", "PortType", "DiskVolumeUsageLevel", "CardSubType", "PortMode", "EqActionType", "CardId", "PrimaryState", "PortDetectedMode", "ShelfId")
...
RowStatus, DisplayString, TimeStamp, TruthValue, DateAndTime, TextualConvention, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "TimeStamp", "TruthValue", "DateAndTime", "TextualConvention", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", trapSeverity=trapSeverity, equipmentState=equipmentState, auditLogOutput=auditLogOutput, errEvLogLevel=errEvLogLevel, cadEquipmentMibGroup=cadEquipmentMibGroup, cardFpgaSource=cardFpgaSource, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSpareGroupId=cardSpareGroupId, portPrState=portPrState, cardTestResultDesc=cardTestResultDesc, fpgaErrorEventEntry=fpgaErrorEventEntry, TestId=TestId, diskVolumeNumber=diskVolumeNumber, portId=portId, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditShelfId=auditShelfId, cardCpuType=cardCpuType, equipmentTbl=equipmentTbl, cadEquipmentMibConformance=cadEquipmentMibConformance, portDetectedMode=portDetectedMode, TestSchedule=TestSchedule, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMib=cadEquipmentMib, cardLastBootSource=cardLastBootSource, cardTemperatureHighWarn=cardTemperatureHighWarn, cardFlashSize=cardFlashSize, cardLastResetReason=cardLastResetReason, portEntry=portEntry, portDplxStatus=portDplxStatus, portConnectorId=portConnectorId, auditCommand=auditCommand, errEventId=errEventId, cardCpuSpeed=cardCpuSpeed, cardShelfId=cardShelfId, diskVolumeGroup=diskVolumeGroup, diskDriveNumber=diskDriveNumber, cardTestName=cardTestName, portMinDsPower=portMinDsPower, eqDiagConfig=eqDiagConfig, systemClock=systemClock, cmResetInfo=cmResetInfo, TestResult=TestResult, cardCPLDVersion=cardCPLDVersion, portLastChangeTime=portLastChangeTime, cardTestTime=cardTestTime, cardFwVersion=cardFwVersion, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeTable=diskVolumeTable, cardUpTime=cardUpTime, shelfSwVersion=shelfSwVersion, diskVolumeId=diskVolumeId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestScheduleCommand=cardTestScheduleCommand, cardDetectedChange=cardDetectedChange, cmResetClearNotification=cmResetClearNotification, cardBootVersion=cardBootVersion, cardPicHwVersion=cardPicHwVersion, diagTestId=diagTestId, cardHwVersion=cardHwVersion, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeFileName=diskVolumeFileName, portSecStateChange=portSecStateChange, cardTestCommand=cardTestCommand, portType=portType, auditId=auditId, shelfObjects=shelfObjects, cardSwVersion=cardSwVersion, errEvLoggingEnabled=errEvLoggingEnabled, cardPrStateChange=cardPrStateChange, portSecState=portSecState, auditStatus=auditStatus, downstreamPowerLoss=downstreamPowerLoss, fpgaErrorEventTable=fpgaErrorEventTable, cardSerialNum=cardSerialNum, equipmentStateGroup=equipmentStateGroup, portMaxDsPower=portMaxDsPower, cardLastBootVersion=cardLastBootVersion, cmVendor=cmVendor, cardNumber=cardNumber, cardPicDetected=cardPicDetected, portMacIfIndex=portMacIfIndex, portCardId=portCardId, diskVolumeTrapInh=diskVolumeTrapInh, cardDplxStatus=cardDplxStatus, cmMacAddress=cmMacAddress, equipmentShelfGroup=equipmentShelfGroup, systemKey=systemKey, auditCycleCount=auditCycleCount, cardTestDescription=cardTestDescription, shelfName=shelfName, cardCarrierHwVersion=cardCarrierHwVersion, auditAutoScheduling=auditAutoScheduling, equipmentError=equipmentError, cardTestTransId=cardTestTransId, auditDescription=auditDescription, equipmentCardGroup=equipmentCardGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTrapInh=cardTrapInh, equipmentAuditGroup=equipmentAuditGroup, portCardSubType=portCardSubType, cardBusSpeed=cardBusSpeed, cardTemperatureHighError=cardTemperatureHighError, portDocsIfIndex=portDocsIfIndex, cardTestSchedule=cardTestSchedule, verboseLevel=verboseLevel, cardTestType=cardTestType, cmPrimarySID=cmPrimarySID, portTrapInh=portTrapInh, cardTestId=cardTestId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditTable=auditTable, auditFailedCount=auditFailedCount, cardName=cardName, auditLogThrottle=auditLogThrottle, fpgaErrorEventGroup=fpgaErrorEventGroup, cardPicSerialNum=cardPicSerialNum, equipmentPortGroup=equipmentPortGroup, cardAdminState=cardAdminState, cardSubDetected=cardSubDetected, PYSNMP_MODULE_ID=cadEquipmentMib, remainInDiagMode=remainInDiagMode, equipmentDiagGroup=equipmentDiagGroup, cardSpareGroupMode=cardSpareGroupMode, cardShelfTestIndex=cardShelfTestIndex, cardTestTable=cardTestTable, cardPrState=cardPrState, cmResetStatus=cmResetStatus, portTxFlowControlDetected=portTxFlowControlDetected, cmUChannelID=cmUChannelID, cardId=cardId, shelfNumber=shelfNumber, cardTemperature=cardTemperature, auditTime=auditTime, auditPassedCount=auditPassedCount, systemGeneralGroup=systemGeneralGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portBgpId=portBgpId, auditTotalPassedCount=auditTotalPassedCount, cardSecState=cardSecState, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSubType=cardSubType, portCurrDsPower=portCurrDsPower, trapCounter=trapCounter, portDplxStatusChange=portDplxStatusChange, portRxFlowControlDetected=portRxFlowControlDetected, cmIpAddress=cmIpAddress, cardTestResult=cardTestResult, cmResetUpTime=cmResetUpTime, TestType=TestType, cardDetected=cardDetected, cardTable=cardTable, cardAction=cardAction, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmDevice=cmDevice, cardDplxStatusChange=cardDplxStatusChange, cardType=cardType, portNumChans=portNumChans, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentCardTestGroup=equipmentCardTestGroup, portMacAddress=portMacAddress, cardTestIndex=cardTestIndex, cardSecStateChange=cardSecStateChange, errEvRowStatus=errEvRowStatus, cardLastChangeTime=cardLastChangeTime, portNumber=portNumber, diskVolumeDriveId=diskVolumeDriveId, cardTestResultNotification=cardTestResultNotification, auditResult=auditResult, portDescription=portDescription, diskVolumeName=diskVolumeName, diskVolumeCardId=diskVolumeCardId, cardRamSize=cardRamSize, portAction=portAction, TestTransactionId=TestTransactionId, portShelfId=portShelfId, consoleOutput=consoleOutput, cardNumPorts=cardNumPorts, portRxFlowControlMode=portRxFlowControlMode, auditEntry=auditEntry, equipmentAudit=equipmentAudit, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeSize=diskVolumeSize, portPrStateChange=portPrStateChange, portTable=portTable, cmResetReason=cmResetReason, cardEntry=cardEntry, diskVolumeShelfId=diskVolumeShelfId, portAdminState=portAdminState, TestScheduleCommand=TestScheduleCommand, cardTestEntry=cardTestEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, portMode=portMode, TestCommand=TestCommand, equipmentDiag=equipmentDiag, auditCardId=auditCardId, systemGeneral=systemGeneral, cmResetNotification=cmResetNotification, cardCarrierFwVersion=cardCarrierFwVersion, auditName=auditName, equipmentTraps=equipmentTraps, diskVolumeEntry=diskVolumeEntry, cmRegistrationNotification=cmRegistrationNotification)
