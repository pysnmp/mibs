...
FlowControlMode, AdminState, PrimaryState, CardSubType, PortMode, DuplexStatus, PortDetectedMode, ShelfId, PicType, FirmwareSource, EqActionType, SecondaryState, PortType, CardType, UnknownState, DiskVolumeUsageLevel, CardId, PortId = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "AdminState", "PrimaryState", "CardSubType", "PortMode", "DuplexStatus", "PortDetectedMode", "ShelfId", "PicType", "FirmwareSource", "EqActionType", "SecondaryState", "PortType", "CardType", "UnknownState", "DiskVolumeUsageLevel", "CardId", "PortId")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSwVersion=cardSwVersion, equipmentAudit=equipmentAudit, TestId=TestId, cardNumber=cardNumber, equipmentState=equipmentState, cardAdminState=cardAdminState, portTrapInh=portTrapInh, auditTable=auditTable, cardTestDescription=cardTestDescription, cardDetectedChange=cardDetectedChange, auditTotalFailedCount=auditTotalFailedCount, equipmentDiagGroup=equipmentDiagGroup, systemGeneralGroup=systemGeneralGroup, PYSNMP_MODULE_ID=cadEquipmentMib, portDetectedMode=portDetectedMode, auditPassedCount=auditPassedCount, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardBusSpeed=cardBusSpeed, diskVolumeFileName=diskVolumeFileName, cardTestType=cardTestType, diskVolumeName=diskVolumeName, cardSerialNum=cardSerialNum, portConnectorId=portConnectorId, auditLogThrottle=auditLogThrottle, cadEquipmentMib=cadEquipmentMib, portRxFlowControlDetected=portRxFlowControlDetected, equipmentTbl=equipmentTbl, equipmentPortGroup=equipmentPortGroup, auditCommand=auditCommand, cardTestResultDesc=cardTestResultDesc, cmVendor=cmVendor, cardTemperatureHighError=cardTemperatureHighError, cardLastBootSource=cardLastBootSource, equipmentShelfGroup=equipmentShelfGroup, auditCycleCount=auditCycleCount, diagTestId=diagTestId, portBgpId=portBgpId, portMacIfIndex=portMacIfIndex, cmRegistrationNotification=cmRegistrationNotification, shelfSwVersion=shelfSwVersion, TestTransactionId=TestTransactionId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardLastResetReason=cardLastResetReason, diskVolumeSize=diskVolumeSize, TestScheduleCommand=TestScheduleCommand, cardCpuSpeed=cardCpuSpeed, portDescription=portDescription, cardPicSerialNum=cardPicSerialNum, cardCarrierFwVersion=cardCarrierFwVersion, portShelfId=portShelfId, errEvLoggingEnabled=errEvLoggingEnabled, cardAction=cardAction, cmResetUpTime=cmResetUpTime, cardDetected=cardDetected, cardCarrierHwVersion=cardCarrierHwVersion, TestSchedule=TestSchedule, cardSecStateChange=cardSecStateChange, cardSpareGroupId=cardSpareGroupId, cardTestResultNotification=cardTestResultNotification, eqDiagConfig=eqDiagConfig, fpgaErrorEventEntry=fpgaErrorEventEntry, auditDescription=auditDescription, errEvRowStatus=errEvRowStatus, errEvLogLevel=errEvLogLevel, cardTemperature=cardTemperature, cardPrStateChange=cardPrStateChange, systemClock=systemClock, cmIpAddress=cmIpAddress, cardShelfTestIndex=cardShelfTestIndex, portId=portId, shelfName=shelfName, portMaxDsPower=portMaxDsPower, cardSubType=cardSubType, cardTrapInh=cardTrapInh, cmResetStatus=cmResetStatus, TestCommand=TestCommand, cmResetInfo=cmResetInfo, portLastChangeTime=portLastChangeTime, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTempNoReportNotification=cardTempNoReportNotification, cardTable=cardTable, portTxFlowControlMode=portTxFlowControlMode, cardTestId=cardTestId, auditLogOutput=auditLogOutput, equipmentTraps=equipmentTraps, fpgaErrorEventGroup=fpgaErrorEventGroup, downstreamPowerLoss=downstreamPowerLoss, cardFpgaSource=cardFpgaSource, portDplxStatusChange=portDplxStatusChange, cardSecState=cardSecState, portDocsIfIndex=portDocsIfIndex, portTxFlowControlDetected=portTxFlowControlDetected, equipmentDiag=equipmentDiag, auditFailedCount=auditFailedCount, portMacAddress=portMacAddress, diskVolumeDriveId=diskVolumeDriveId, cardName=cardName, cardTestSchedule=cardTestSchedule, cardId=cardId, cardShelfId=cardShelfId, cmResetNotification=cmResetNotification, cardTestTransId=cardTestTransId, cardTemperatureHighWarn=cardTemperatureHighWarn, auditCardId=auditCardId, cardTestResult=cardTestResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portAction=portAction, shelfObjects=shelfObjects, diskVolumeGroup=diskVolumeGroup, diskVolumeDiskSize=diskVolumeDiskSize, portPrState=portPrState, cardTestScheduleCommand=cardTestScheduleCommand, systemGeneral=systemGeneral, cardCPLDVersion=cardCPLDVersion, cardDplxStatusChange=cardDplxStatusChange, portMinDsPower=portMinDsPower, cmResetReason=cmResetReason, cmMacAddress=cmMacAddress, diskVolumeTrapInh=diskVolumeTrapInh, portEntry=portEntry, cardCpuType=cardCpuType, cardSubDetected=cardSubDetected, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSpareGroupMode=cardSpareGroupMode, errEventId=errEventId, diskDriveNumber=diskDriveNumber, equipmentAuditGroup=equipmentAuditGroup, cardDplxStatus=cardDplxStatus, cardTestIndex=cardTestIndex, cmDevice=cmDevice, cardRamSize=cardRamSize, cardType=cardType, auditStatus=auditStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portCurrDsPower=portCurrDsPower, portCardId=portCardId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, systemKey=systemKey, cmUChannelID=cmUChannelID, diskVolumeUsageLevel=diskVolumeUsageLevel, fpgaErrorEventTable=fpgaErrorEventTable, cardPicHwVersion=cardPicHwVersion, portDplxStatus=portDplxStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmPrimarySID=cmPrimarySID, auditShelfId=auditShelfId, portSecStateChange=portSecStateChange, trapSeverity=trapSeverity, TestResult=TestResult, portTable=portTable, auditAutoScheduling=auditAutoScheduling, cardCarrierSerialNum=cardCarrierSerialNum, cardTestTable=cardTestTable, portAdminState=portAdminState, cardFwVersion=cardFwVersion, cardTestCommand=cardTestCommand, cardNumPorts=cardNumPorts, TestType=TestType, diskVolumeEntry=diskVolumeEntry, portSecState=portSecState, cardBootVersion=cardBootVersion, portMode=portMode, diskVolumeCardId=diskVolumeCardId, verboseLevel=verboseLevel, auditId=auditId, diskVolumeShelfId=diskVolumeShelfId, diskVolumeUsageNotification=diskVolumeUsageNotification, portNumber=portNumber, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeNumber=diskVolumeNumber, portNumChans=portNumChans, auditName=auditName, cardFlashSize=cardFlashSize, cmResetClearNotification=cmResetClearNotification, portCardSubType=portCardSubType, diskVolumeTable=diskVolumeTable, consoleOutput=consoleOutput, cardTestEntry=cardTestEntry, auditTotalPassedCount=auditTotalPassedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicDetected=cardPicDetected, cardHwVersion=cardHwVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, portPrStateChange=portPrStateChange, equipmentStateGroup=equipmentStateGroup, cardEntry=cardEntry, cardUpTime=cardUpTime, diskVolumeId=diskVolumeId, cardLastChangeTime=cardLastChangeTime, cadEquipmentMibConformance=cadEquipmentMibConformance, portRxFlowControlMode=portRxFlowControlMode, trapCounter=trapCounter, auditEntry=auditEntry, auditResult=auditResult, auditTime=auditTime, cardTestName=cardTestName, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfNumber=shelfNumber, portType=portType, equipmentCardGroup=equipmentCardGroup, equipmentCardTestGroup=equipmentCardTestGroup, remainInDiagMode=remainInDiagMode, cardTestTime=cardTestTime, equipmentError=equipmentError, cardPrState=cardPrState, cardLastBootVersion=cardLastBootVersion)
