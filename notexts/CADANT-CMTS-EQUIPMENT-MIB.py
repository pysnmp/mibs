...
CardType, PortType, UnknownState, PortDetectedMode, EqActionType, PrimaryState, CardId, PicType, PortMode, PortId, ShelfId, AdminState, FlowControlMode, SecondaryState, DiskVolumeUsageLevel, CardSubType, FirmwareSource, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortType", "UnknownState", "PortDetectedMode", "EqActionType", "PrimaryState", "CardId", "PicType", "PortMode", "PortId", "ShelfId", "AdminState", "FlowControlMode", "SecondaryState", "DiskVolumeUsageLevel", "CardSubType", "FirmwareSource", "DuplexStatus")
...
RowStatus, DisplayString, MacAddress, TruthValue, DateAndTime, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "MacAddress", "TruthValue", "DateAndTime", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfName=shelfName, equipmentDiag=equipmentDiag, cardBootVersion=cardBootVersion, TestCommand=TestCommand, portSecState=portSecState, portId=portId, cmResetNotification=cmResetNotification, portShelfId=portShelfId, portTrapInh=portTrapInh, diskVolumeCardId=diskVolumeCardId, cardTestResultDesc=cardTestResultDesc, cardSecStateChange=cardSecStateChange, cardNumPorts=cardNumPorts, cardEntry=cardEntry, cardBusSpeed=cardBusSpeed, cmMacAddress=cmMacAddress, portDplxStatusChange=portDplxStatusChange, cardTestEntry=cardTestEntry, TestTransactionId=TestTransactionId, portPrStateChange=portPrStateChange, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeFileName=diskVolumeFileName, diskVolumeDriveId=diskVolumeDriveId, diagTestId=diagTestId, cardId=cardId, portLastChangeTime=portLastChangeTime, cmPrimarySID=cmPrimarySID, equipmentTbl=equipmentTbl, cmResetClearNotification=cmResetClearNotification, cmResetStatus=cmResetStatus, cardShelfId=cardShelfId, cardAction=cardAction, cardSpareGroupMode=cardSpareGroupMode, trapCounter=trapCounter, diskDriveNumber=diskDriveNumber, diskVolumeTrapInh=diskVolumeTrapInh, cmResetInfo=cmResetInfo, errEvRowStatus=errEvRowStatus, cardDplxStatus=cardDplxStatus, cmDevice=cmDevice, equipmentAuditGroup=equipmentAuditGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCpuSpeed=cardCpuSpeed, diskVolumeUsageLevel=diskVolumeUsageLevel, cadEquipmentMib=cadEquipmentMib, portAdminState=portAdminState, diskVolumeGroup=diskVolumeGroup, portType=portType, auditCommand=auditCommand, cmResetReason=cmResetReason, portTxFlowControlDetected=portTxFlowControlDetected, cardFlashSize=cardFlashSize, cardTestTransId=cardTestTransId, remainInDiagMode=remainInDiagMode, auditLogOutput=auditLogOutput, auditResult=auditResult, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, shelfSwVersion=shelfSwVersion, cardType=cardType, errEvLogLevel=errEvLogLevel, auditFailedCount=auditFailedCount, cardSubDetected=cardSubDetected, trapSeverity=trapSeverity, cardTestDescription=cardTestDescription, auditStatus=auditStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, TestType=TestType, cardLastBootSource=cardLastBootSource, equipmentCardGroup=equipmentCardGroup, cardDplxStatusChange=cardDplxStatusChange, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPicHwVersion=cardPicHwVersion, consoleOutput=consoleOutput, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditTotalPassedCount=auditTotalPassedCount, systemClock=systemClock, cardTrapInh=cardTrapInh, cmVendor=cmVendor, TestSchedule=TestSchedule, equipmentTraps=equipmentTraps, equipmentStateGroup=equipmentStateGroup, cardSubType=cardSubType, cardShelfTestIndex=cardShelfTestIndex, portCardSubType=portCardSubType, cardHwVersion=cardHwVersion, cardTestName=cardTestName, cardNumber=cardNumber, portEntry=portEntry, auditPassedCount=auditPassedCount, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestIndex=cardTestIndex, cardTestCommand=cardTestCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResult=cardTestResult, diskVolumeId=diskVolumeId, auditTotalFailedCount=auditTotalFailedCount, portMaxDsPower=portMaxDsPower, cardTestSchedule=cardTestSchedule, diskVolumeEntry=diskVolumeEntry, TestResult=TestResult, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeDiskSize=diskVolumeDiskSize, equipmentAudit=equipmentAudit, cardAdminState=cardAdminState, verboseLevel=verboseLevel, auditEntry=auditEntry, shelfNumber=shelfNumber, cardCPLDVersion=cardCPLDVersion, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeShelfId=diskVolumeShelfId, portTable=portTable, cardLastResetReason=cardLastResetReason, cardLastBootVersion=cardLastBootVersion, cardPrState=cardPrState, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeName=diskVolumeName, equipmentPortGroup=equipmentPortGroup, cardSpareGroupId=cardSpareGroupId, cardUpTime=cardUpTime, portDetectedMode=portDetectedMode, cadEquipmentMibConformance=cadEquipmentMibConformance, systemKey=systemKey, auditCardId=auditCardId, auditDescription=auditDescription, portPrState=portPrState, cardTestTime=cardTestTime, diskVolumeTable=diskVolumeTable, portNumChans=portNumChans, cardSecState=cardSecState, portBgpId=portBgpId, auditId=auditId, portDplxStatus=portDplxStatus, cardPicDetected=cardPicDetected, systemGeneral=systemGeneral, cardTestResultNotification=cardTestResultNotification, cardFwUpdateStatus=cardFwUpdateStatus, cardTestTable=cardTestTable, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, eqDiagConfig=eqDiagConfig, diskVolumeNumber=diskVolumeNumber, portNumber=portNumber, cardName=cardName, cardDetectedChange=cardDetectedChange, portMacIfIndex=portMacIfIndex, errEventId=errEventId, cardFpgaSource=cardFpgaSource, cardTemperature=cardTemperature, portRxFlowControlMode=portRxFlowControlMode, cmRegistrationNotification=cmRegistrationNotification, cardCpuType=cardCpuType, portCardId=portCardId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmUChannelID=cmUChannelID, cardCarrierHwVersion=cardCarrierHwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestType=cardTestType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCarrierSerialNum=cardCarrierSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditName=auditName, errEvRecoveryEnabled=errEvRecoveryEnabled, portMacAddress=portMacAddress, auditLogThrottle=auditLogThrottle, cardTable=cardTable, portDescription=portDescription, cardTemperatureHighError=cardTemperatureHighError, auditCycleCount=auditCycleCount, cmIpAddress=cmIpAddress, TestId=TestId, shelfObjects=shelfObjects, portCurrDsPower=portCurrDsPower, equipmentState=equipmentState, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageNotification=diskVolumeUsageNotification, downstreamPowerLoss=downstreamPowerLoss, auditShelfId=auditShelfId, portDocsIfIndex=portDocsIfIndex, diskVolumeSize=diskVolumeSize, cardTestId=cardTestId, equipmentShelfGroup=equipmentShelfGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, portConnectorId=portConnectorId, cardSerialNum=cardSerialNum, cmResetUpTime=cmResetUpTime, cardRamSize=cardRamSize, cardTempNoReportNotification=cardTempNoReportNotification, cardPicSerialNum=cardPicSerialNum, portMinDsPower=portMinDsPower, cardDetected=cardDetected, portAction=portAction, auditTime=auditTime, fpgaErrorEventEntry=fpgaErrorEventEntry, errEvLoggingEnabled=errEvLoggingEnabled, cardFwVersion=cardFwVersion, portSecStateChange=portSecStateChange, cardSwVersion=cardSwVersion, systemGeneralGroup=systemGeneralGroup, portTxFlowControlMode=portTxFlowControlMode, equipmentDiagGroup=equipmentDiagGroup, TestScheduleCommand=TestScheduleCommand, equipmentError=equipmentError, cardPrStateChange=cardPrStateChange, auditTable=auditTable, portMode=portMode)
