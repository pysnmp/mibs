...
CardType, FirmwareSource, PortType, DuplexStatus, PortDetectedMode, CardId, EqActionType, ShelfId, UnknownState, FlowControlMode, SecondaryState, PortMode, CardSubType, PrimaryState, AdminState, DiskVolumeUsageLevel, PortId, PicType = mibBuilder.importSymbols("CADANT-TC", "CardType", "FirmwareSource", "PortType", "DuplexStatus", "PortDetectedMode", "CardId", "EqActionType", "ShelfId", "UnknownState", "FlowControlMode", "SecondaryState", "PortMode", "CardSubType", "PrimaryState", "AdminState", "DiskVolumeUsageLevel", "PortId", "PicType")
...
TimeStamp, MacAddress, RowStatus, DisplayString, TruthValue, TextualConvention, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "MacAddress", "RowStatus", "DisplayString", "TruthValue", "TextualConvention", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCpuSpeed=cardCpuSpeed, auditPassedCount=auditPassedCount, cardLastResetReason=cardLastResetReason, cardName=cardName, cadEquipmentMibGroup=cadEquipmentMibGroup, cardFpgaSource=cardFpgaSource, shelfName=shelfName, auditTotalPassedCount=auditTotalPassedCount, cardShelfTestIndex=cardShelfTestIndex, auditCardId=auditCardId, portCurrDsPower=portCurrDsPower, cardTestResultDesc=cardTestResultDesc, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeDriveId=diskVolumeDriveId, portCardSubType=portCardSubType, systemClock=systemClock, cmRegistrationNotification=cmRegistrationNotification, cardPicHwVersion=cardPicHwVersion, systemGeneral=systemGeneral, cadEquipmentMibConformance=cadEquipmentMibConformance, portCardId=portCardId, auditId=auditId, cmResetInfo=cmResetInfo, cardTestTable=cardTestTable, auditEntry=auditEntry, portTrapInh=portTrapInh, cardBootVersion=cardBootVersion, equipmentError=equipmentError, cardTempNoReportNotification=cardTempNoReportNotification, cardTestDescription=cardTestDescription, shelfNumber=shelfNumber, portLastChangeTime=portLastChangeTime, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentStateGroup=equipmentStateGroup, cardRamSize=cardRamSize, cmResetReason=cmResetReason, portDocsIfIndex=portDocsIfIndex, cardTemperatureHighWarn=cardTemperatureHighWarn, auditTime=auditTime, cardShelfId=cardShelfId, portBgpId=portBgpId, cardTestName=cardTestName, cardTestSchedule=cardTestSchedule, cardSwVersion=cardSwVersion, cmDevice=cmDevice, cardDetectedChange=cardDetectedChange, cardPrStateChange=cardPrStateChange, equipmentTbl=equipmentTbl, cardLastBootSource=cardLastBootSource, cardTestCommand=cardTestCommand, equipmentAuditGroup=equipmentAuditGroup, cardSpareGroupMode=cardSpareGroupMode, TestId=TestId, cardSpareGroupId=cardSpareGroupId, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmPrimarySID=cmPrimarySID, cardBusSpeed=cardBusSpeed, cmVendor=cmVendor, cardId=cardId, portMinDsPower=portMinDsPower, cadEquipmentMib=cadEquipmentMib, cardTestResult=cardTestResult, portPrStateChange=portPrStateChange, portType=portType, cardSubType=cardSubType, equipmentAudit=equipmentAudit, errEvRecoveryEnabled=errEvRecoveryEnabled, portMode=portMode, portRxFlowControlDetected=portRxFlowControlDetected, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cmUChannelID=cmUChannelID, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portSecStateChange=portSecStateChange, portMacAddress=portMacAddress, auditDescription=auditDescription, cardTestType=cardTestType, errEvRowStatus=errEvRowStatus, cmIpAddress=cmIpAddress, cardNumber=cardNumber, auditFailedCount=auditFailedCount, errEventId=errEventId, portNumber=portNumber, diskDriveNumber=diskDriveNumber, eqDiagConfig=eqDiagConfig, portMacIfIndex=portMacIfIndex, cardCPLDVersion=cardCPLDVersion, portDplxStatusChange=portDplxStatusChange, cardEntry=cardEntry, TestTransactionId=TestTransactionId, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuType=cardCpuType, auditStatus=auditStatus, diskVolumeShelfId=diskVolumeShelfId, cmResetClearNotification=cmResetClearNotification, consoleOutput=consoleOutput, diskVolumeFileName=diskVolumeFileName, portTxFlowControlMode=portTxFlowControlMode, cardSerialNum=cardSerialNum, cmResetUpTime=cmResetUpTime, TestType=TestType, equipmentState=equipmentState, diagTestId=diagTestId, cardTestIndex=cardTestIndex, cmMacAddress=cmMacAddress, cardTestEntry=cardTestEntry, shelfSwVersion=shelfSwVersion, portEntry=portEntry, portTxFlowControlDetected=portTxFlowControlDetected, auditCommand=auditCommand, fpgaErrorEventTable=fpgaErrorEventTable, portAction=portAction, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSecStateChange=cardSecStateChange, cardSubDetected=cardSubDetected, diskVolumeNumber=diskVolumeNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTemperatureHighError=cardTemperatureHighError, portConnectorId=portConnectorId, portPrState=portPrState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentDiagGroup=equipmentDiagGroup, cardTestTime=cardTestTime, shelfObjects=shelfObjects, cardCarrierFwVersion=cardCarrierFwVersion, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeName=diskVolumeName, cardPrState=cardPrState, cardType=cardType, cardPicSerialNum=cardPicSerialNum, errEvLogLevel=errEvLogLevel, cardTestId=cardTestId, auditTotalFailedCount=auditTotalFailedCount, portDescription=portDescription, cardHwVersion=cardHwVersion, cardCarrierSerialNum=cardCarrierSerialNum, portSecState=portSecState, portTable=portTable, cmResetStatus=cmResetStatus, cardAdminState=cardAdminState, cardDplxStatus=cardDplxStatus, cardPicDetected=cardPicDetected, auditLogThrottle=auditLogThrottle, cardAction=cardAction, cardTestScheduleCommand=cardTestScheduleCommand, equipmentTraps=equipmentTraps, cardTestResultNotification=cardTestResultNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, trapCounter=trapCounter, auditShelfId=auditShelfId, remainInDiagMode=remainInDiagMode, equipmentCardTestGroup=equipmentCardTestGroup, portId=portId, trapSeverity=trapSeverity, systemKey=systemKey, auditCycleCount=auditCycleCount, cardTable=cardTable, portRxFlowControlMode=portRxFlowControlMode, diskVolumeId=diskVolumeId, cardTemperature=cardTemperature, TestSchedule=TestSchedule, portMaxDsPower=portMaxDsPower, diskVolumeEntry=diskVolumeEntry, cardFlashSize=cardFlashSize, auditTable=auditTable, portShelfId=portShelfId, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeDiskSize=diskVolumeDiskSize, cardSecState=cardSecState, cardTestTransId=cardTestTransId, diskVolumeGroup=diskVolumeGroup, equipmentShelfGroup=equipmentShelfGroup, equipmentDiag=equipmentDiag, auditName=auditName, cardNumPorts=cardNumPorts, auditResult=auditResult, cardLastBootVersion=cardLastBootVersion, cardUpTime=cardUpTime, TestCommand=TestCommand, diskVolumeCardId=diskVolumeCardId, equipmentPortGroup=equipmentPortGroup, portDplxStatus=portDplxStatus, cmResetNotification=cmResetNotification, portAdminState=portAdminState, cardTrapInh=cardTrapInh, verboseLevel=verboseLevel, diskVolumeSize=diskVolumeSize, cardDplxStatusChange=cardDplxStatusChange, TestScheduleCommand=TestScheduleCommand, cardFwVersion=cardFwVersion, auditAutoScheduling=auditAutoScheduling, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeTable=diskVolumeTable, diskVolumeTrapInh=diskVolumeTrapInh, auditLogOutput=auditLogOutput, portNumChans=portNumChans, TestResult=TestResult, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardDetected=cardDetected, portDetectedMode=portDetectedMode)
