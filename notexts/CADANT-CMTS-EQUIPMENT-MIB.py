...
EqActionType, PortType, ShelfId, FlowControlMode, FirmwareSource, CardId, PortMode, PortDetectedMode, DuplexStatus, SecondaryState, UnknownState, PicType, CardType, CardSubType, DiskVolumeUsageLevel, PortId, PrimaryState, AdminState = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "PortType", "ShelfId", "FlowControlMode", "FirmwareSource", "CardId", "PortMode", "PortDetectedMode", "DuplexStatus", "SecondaryState", "UnknownState", "PicType", "CardType", "CardSubType", "DiskVolumeUsageLevel", "PortId", "PrimaryState", "AdminState")
...
TextualConvention, MacAddress, TruthValue, RowStatus, DisplayString, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "TruthValue", "RowStatus", "DisplayString", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCpuSpeed=cardCpuSpeed, cardPicSerialNum=cardPicSerialNum, portPrState=portPrState, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmVendor=cmVendor, portDetectedMode=portDetectedMode, auditTable=auditTable, systemKey=systemKey, trapCounter=trapCounter, cardTestCommand=cardTestCommand, cardTestResultNotification=cardTestResultNotification, shelfName=shelfName, remainInDiagMode=remainInDiagMode, cadEquipmentMib=cadEquipmentMib, errEventId=errEventId, cardNumPorts=cardNumPorts, portLastChangeTime=portLastChangeTime, cmMacAddress=cmMacAddress, cardUpTime=cardUpTime, cardCarrierFwVersion=cardCarrierFwVersion, cardSubDetected=cardSubDetected, auditShelfId=auditShelfId, portDocsIfIndex=portDocsIfIndex, cardBootVersion=cardBootVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, systemClock=systemClock, cardSpareGroupMode=cardSpareGroupMode, diskVolumeShelfId=diskVolumeShelfId, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeNumber=diskVolumeNumber, cardDplxStatus=cardDplxStatus, cardHwVersion=cardHwVersion, portNumChans=portNumChans, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSerialNum=cardSerialNum, portRxFlowControlMode=portRxFlowControlMode, equipmentStateGroup=equipmentStateGroup, portTxFlowControlMode=portTxFlowControlMode, equipmentState=equipmentState, equipmentError=equipmentError, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestCommand=TestCommand, portConnectorId=portConnectorId, cardFlashSize=cardFlashSize, TestType=TestType, cardRamSize=cardRamSize, TestSchedule=TestSchedule, equipmentDiagGroup=equipmentDiagGroup, cardFpgaSource=cardFpgaSource, auditCardId=auditCardId, auditStatus=auditStatus, diskVolumeDriveId=diskVolumeDriveId, cardTestDescription=cardTestDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditPassedCount=auditPassedCount, portShelfId=portShelfId, auditDescription=auditDescription, cardName=cardName, auditLogOutput=auditLogOutput, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMibGroup=cadEquipmentMibGroup, cmPrimarySID=cmPrimarySID, cardBusSpeed=cardBusSpeed, portNumber=portNumber, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeFileName=diskVolumeFileName, cardAction=cardAction, cardCarrierHwVersion=cardCarrierHwVersion, portSecStateChange=portSecStateChange, cardLastResetReason=cardLastResetReason, diskVolumeCardId=diskVolumeCardId, cardTestId=cardTestId, portTxFlowControlDetected=portTxFlowControlDetected, cardSubType=cardSubType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfSwVersion=shelfSwVersion, shelfNumber=shelfNumber, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestTransId=cardTestTransId, systemGeneralGroup=systemGeneralGroup, portSecState=portSecState, portMode=portMode, portAdminState=portAdminState, diskVolumeEntry=diskVolumeEntry, portTable=portTable, diskVolumeUsageLevel=diskVolumeUsageLevel, portMinDsPower=portMinDsPower, cardTestType=cardTestType, cmIpAddress=cmIpAddress, cmResetNotification=cmResetNotification, cardTestTime=cardTestTime, TestResult=TestResult, cardTemperatureHighError=cardTemperatureHighError, portTrapInh=portTrapInh, cardTestName=cardTestName, cardDplxStatusChange=cardDplxStatusChange, cardSecStateChange=cardSecStateChange, auditCycleCount=auditCycleCount, cmResetInfo=cmResetInfo, TestScheduleCommand=TestScheduleCommand, diskVolumeId=diskVolumeId, auditId=auditId, equipmentCardTestGroup=equipmentCardTestGroup, portMaxDsPower=portMaxDsPower, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastChangeTime=cardLastChangeTime, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlDetected=portRxFlowControlDetected, cardTrapInh=cardTrapInh, shelfObjects=shelfObjects, cardSwVersion=cardSwVersion, auditResult=auditResult, auditTotalFailedCount=auditTotalFailedCount, downstreamPowerLoss=downstreamPowerLoss, errEvLogLevel=errEvLogLevel, cardFwUpdateStatus=cardFwUpdateStatus, cardTestEntry=cardTestEntry, cardFwVersion=cardFwVersion, auditTotalPassedCount=auditTotalPassedCount, cmDevice=cmDevice, auditEntry=auditEntry, portMacIfIndex=portMacIfIndex, cmResetReason=cmResetReason, cardShelfTestIndex=cardShelfTestIndex, equipmentTraps=equipmentTraps, cmResetStatus=cmResetStatus, cardType=cardType, systemGeneral=systemGeneral, auditFailedCount=auditFailedCount, portBgpId=portBgpId, cardTestTable=cardTestTable, errEvRowStatus=errEvRowStatus, auditLogThrottle=auditLogThrottle, cardDetected=cardDetected, portEntry=portEntry, errEvRecoveryEnabled=errEvRecoveryEnabled, consoleOutput=consoleOutput, auditTime=auditTime, cardTemperature=cardTemperature, diagTestId=diagTestId, cardCPLDVersion=cardCPLDVersion, portCardId=portCardId, auditAutoScheduling=auditAutoScheduling, cardPrStateChange=cardPrStateChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, trapSeverity=trapSeverity, portDplxStatus=portDplxStatus, equipmentAudit=equipmentAudit, portMacAddress=portMacAddress, portCardSubType=portCardSubType, cardDetectedChange=cardDetectedChange, diskVolumeName=diskVolumeName, cardPrState=cardPrState, cardSecState=cardSecState, diskVolumeUsageNotification=diskVolumeUsageNotification, TestTransactionId=TestTransactionId, cardNumber=cardNumber, cardLastBootSource=cardLastBootSource, equipmentTbl=equipmentTbl, cardSpareGroupId=cardSpareGroupId, cardTestSchedule=cardTestSchedule, equipmentCardGroup=equipmentCardGroup, equipmentDiag=equipmentDiag, TestId=TestId, portDplxStatusChange=portDplxStatusChange, auditCommand=auditCommand, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTempNoReportNotification=cardTempNoReportNotification, cardLastBootVersion=cardLastBootVersion, portAction=portAction, equipmentPortGroup=equipmentPortGroup, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeTable=diskVolumeTable, cardTable=cardTable, fpgaErrorEventEntry=fpgaErrorEventEntry, cardShelfId=cardShelfId, diskDriveNumber=diskDriveNumber, portDescription=portDescription, cardCarrierSerialNum=cardCarrierSerialNum, equipmentShelfGroup=equipmentShelfGroup, cardTestIndex=cardTestIndex, cardId=cardId, cardTestScheduleCommand=cardTestScheduleCommand, cmResetUpTime=cmResetUpTime, portCurrDsPower=portCurrDsPower, eqDiagConfig=eqDiagConfig, cardPicHwVersion=cardPicHwVersion, cardTestResult=cardTestResult, diskVolumeGroup=diskVolumeGroup, cardTestResultDesc=cardTestResultDesc, verboseLevel=verboseLevel, cmUChannelID=cmUChannelID, cardCpuType=cardCpuType, cmResetClearNotification=cmResetClearNotification, cardEntry=cardEntry, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentAuditGroup=equipmentAuditGroup, cardPicDetected=cardPicDetected, auditName=auditName, portPrStateChange=portPrStateChange, cardAdminState=cardAdminState, portType=portType, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeSize=diskVolumeSize, portId=portId)
