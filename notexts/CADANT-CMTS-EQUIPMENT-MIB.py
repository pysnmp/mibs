...
PortId, PrimaryState, FlowControlMode, PortMode, EqActionType, FirmwareSource, PortDetectedMode, CardSubType, AdminState, PicType, SecondaryState, CardId, DiskVolumeUsageLevel, DuplexStatus, CardType, UnknownState, ShelfId, PortType = mibBuilder.importSymbols("CADANT-TC", "PortId", "PrimaryState", "FlowControlMode", "PortMode", "EqActionType", "FirmwareSource", "PortDetectedMode", "CardSubType", "AdminState", "PicType", "SecondaryState", "CardId", "DiskVolumeUsageLevel", "DuplexStatus", "CardType", "UnknownState", "ShelfId", "PortType")
...
DateAndTime, DisplayString, TruthValue, TimeStamp, TextualConvention, RowStatus, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "DisplayString", "TruthValue", "TimeStamp", "TextualConvention", "RowStatus", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCarrierHwVersion=cardCarrierHwVersion, portBgpId=portBgpId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupId=cardSpareGroupId, diskVolumeFileName=diskVolumeFileName, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardLastResetReason=cardLastResetReason, diskDriveNumber=diskDriveNumber, eqDiagConfig=eqDiagConfig, cardFwVersion=cardFwVersion, portTxFlowControlMode=portTxFlowControlMode, cardTestCommand=cardTestCommand, cmVendor=cmVendor, cmResetUpTime=cmResetUpTime, cardTemperature=cardTemperature, cadEquipmentMibGroup=cadEquipmentMibGroup, verboseLevel=verboseLevel, TestSchedule=TestSchedule, diskVolumeTable=diskVolumeTable, diskVolumeTrapInh=diskVolumeTrapInh, cardTestScheduleCommand=cardTestScheduleCommand, cmIpAddress=cmIpAddress, cardTestTable=cardTestTable, cardTemperatureHighError=cardTemperatureHighError, PYSNMP_MODULE_ID=cadEquipmentMib, cardFlashSize=cardFlashSize, portSecStateChange=portSecStateChange, portMaxDsPower=portMaxDsPower, cardTestResultNotification=cardTestResultNotification, auditLogThrottle=auditLogThrottle, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentState=equipmentState, cardAdminState=cardAdminState, diskVolumeId=diskVolumeId, equipmentAudit=equipmentAudit, auditCommand=auditCommand, errEventId=errEventId, cardSerialNum=cardSerialNum, portPrStateChange=portPrStateChange, portSecState=portSecState, cardTestType=cardTestType, fpgaErrorEventGroup=fpgaErrorEventGroup, cardDetectedChange=cardDetectedChange, portCardSubType=portCardSubType, diagTestId=diagTestId, portId=portId, auditTotalFailedCount=auditTotalFailedCount, diskVolumeEntry=diskVolumeEntry, cardSecState=cardSecState, cardTestDescription=cardTestDescription, cardShelfTestIndex=cardShelfTestIndex, auditLogOutput=auditLogOutput, cardEntry=cardEntry, cardNumber=cardNumber, portMacAddress=portMacAddress, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestEntry=cardTestEntry, cardPrStateChange=cardPrStateChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, systemKey=systemKey, remainInDiagMode=remainInDiagMode, diskVolumeName=diskVolumeName, TestTransactionId=TestTransactionId, auditTable=auditTable, cardBootVersion=cardBootVersion, cardSpareGroupMode=cardSpareGroupMode, cardSubType=cardSubType, auditPassedCount=auditPassedCount, cardShelfId=cardShelfId, cardCarrierFwVersion=cardCarrierFwVersion, cardDplxStatusChange=cardDplxStatusChange, auditEntry=auditEntry, cardName=cardName, cardCpuType=cardCpuType, cardTestIndex=cardTestIndex, cardTempNoReportNotification=cardTempNoReportNotification, auditResult=auditResult, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestCommand=TestCommand, trapSeverity=trapSeverity, cardAction=cardAction, auditCardId=auditCardId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmDevice=cmDevice, portPrState=portPrState, cardBusSpeed=cardBusSpeed, cmPrimarySID=cmPrimarySID, errEvLoggingEnabled=errEvLoggingEnabled, errEvRowStatus=errEvRowStatus, portLastChangeTime=portLastChangeTime, cardSubDetected=cardSubDetected, equipmentTraps=equipmentTraps, portDetectedMode=portDetectedMode, cardTestResultDesc=cardTestResultDesc, cardCPLDVersion=cardCPLDVersion, cardType=cardType, portShelfId=portShelfId, portCurrDsPower=portCurrDsPower, cmRegistrationNotification=cmRegistrationNotification, portConnectorId=portConnectorId, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeNumber=diskVolumeNumber, TestResult=TestResult, cardTable=cardTable, cardNumPorts=cardNumPorts, shelfName=shelfName, downstreamPowerLoss=downstreamPowerLoss, cardHwVersion=cardHwVersion, portNumber=portNumber, cardTrapInh=cardTrapInh, portEntry=portEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmMacAddress=cmMacAddress, cardLastChangeTime=cardLastChangeTime, cmResetStatus=cmResetStatus, cardTestTransId=cardTestTransId, equipmentError=equipmentError, equipmentAuditGroup=equipmentAuditGroup, portMacIfIndex=portMacIfIndex, auditAutoScheduling=auditAutoScheduling, diskVolumeCardId=diskVolumeCardId, cardPrState=cardPrState, portNumChans=portNumChans, cmResetClearNotification=cmResetClearNotification, systemGeneralGroup=systemGeneralGroup, systemClock=systemClock, equipmentStateGroup=equipmentStateGroup, trapCounter=trapCounter, portMode=portMode, portTable=portTable, cardUpTime=cardUpTime, cardTestResult=cardTestResult, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsageLevel=diskVolumeUsageLevel, auditName=auditName, errEvLogLevel=errEvLogLevel, equipmentDiag=equipmentDiag, portCardId=portCardId, equipmentCardGroup=equipmentCardGroup, diskVolumeDriveId=diskVolumeDriveId, cardSecStateChange=cardSecStateChange, equipmentDiagGroup=equipmentDiagGroup, cardTestSchedule=cardTestSchedule, cmResetInfo=cmResetInfo, cardLastBootVersion=cardLastBootVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetNotification=cmResetNotification, cardCpuSpeed=cardCpuSpeed, cardFpgaSource=cardFpgaSource, cardCarrierSerialNum=cardCarrierSerialNum, portDplxStatus=portDplxStatus, cardTestName=cardTestName, portDescription=portDescription, auditId=auditId, auditDescription=auditDescription, cardId=cardId, TestId=TestId, portDocsIfIndex=portDocsIfIndex, auditFailedCount=auditFailedCount, cardTestId=cardTestId, cardDetected=cardDetected, cardSwVersion=cardSwVersion, TestScheduleCommand=TestScheduleCommand, portTxFlowControlDetected=portTxFlowControlDetected, shelfNumber=shelfNumber, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeGroup=diskVolumeGroup, portRxFlowControlDetected=portRxFlowControlDetected, systemGeneral=systemGeneral, cardDplxStatus=cardDplxStatus, consoleOutput=consoleOutput, diskVolumeSize=diskVolumeSize, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentTbl=equipmentTbl, diskVolumeDiskSize=diskVolumeDiskSize, auditShelfId=auditShelfId, cadEquipmentMib=cadEquipmentMib, cmUChannelID=cmUChannelID, portAdminState=portAdminState, portAction=portAction, portMinDsPower=portMinDsPower, equipmentShelfGroup=equipmentShelfGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicHwVersion=cardPicHwVersion, auditStatus=auditStatus, equipmentPortGroup=equipmentPortGroup, cardPicDetected=cardPicDetected, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeUsageNotification=diskVolumeUsageNotification, auditTime=auditTime, equipmentCardTestGroup=equipmentCardTestGroup, portTrapInh=portTrapInh, auditCycleCount=auditCycleCount, portRxFlowControlMode=portRxFlowControlMode, cardRamSize=cardRamSize, portType=portType, shelfSwVersion=shelfSwVersion, cardTestTime=cardTestTime, shelfObjects=shelfObjects, TestType=TestType, cardLastBootSource=cardLastBootSource, portDplxStatusChange=portDplxStatusChange, cmResetReason=cmResetReason)
