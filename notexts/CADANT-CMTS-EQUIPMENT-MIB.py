...
PortId, EqActionType, FlowControlMode, AdminState, PortType, UnknownState, PortDetectedMode, PicType, ShelfId, DuplexStatus, SecondaryState, PortMode, CardSubType, PrimaryState, CardId, CardType, DiskVolumeUsageLevel, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "PortId", "EqActionType", "FlowControlMode", "AdminState", "PortType", "UnknownState", "PortDetectedMode", "PicType", "ShelfId", "DuplexStatus", "SecondaryState", "PortMode", "CardSubType", "PrimaryState", "CardId", "CardType", "DiskVolumeUsageLevel", "FirmwareSource")
...
TruthValue, TextualConvention, DateAndTime, RowStatus, MacAddress, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TextualConvention", "DateAndTime", "RowStatus", "MacAddress", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestEntry=cardTestEntry, equipmentError=equipmentError, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentPortGroup=equipmentPortGroup, fpgaErrorEventTable=fpgaErrorEventTable, cardPrState=cardPrState, cardTestScheduleCommand=cardTestScheduleCommand, equipmentTbl=equipmentTbl, portShelfId=portShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPrStateChange=cardPrStateChange, shelfNumber=shelfNumber, eqDiagConfig=eqDiagConfig, portMacIfIndex=portMacIfIndex, cardPicHwVersion=cardPicHwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeDriveId=diskVolumeDriveId, cardDplxStatusChange=cardDplxStatusChange, cardUpTime=cardUpTime, portPrState=portPrState, cardTestSchedule=cardTestSchedule, cardTestResultNotification=cardTestResultNotification, cardNumPorts=cardNumPorts, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardSwVersion=cardSwVersion, diskVolumeCardId=diskVolumeCardId, TestTransactionId=TestTransactionId, portLastChangeTime=portLastChangeTime, cardTestTime=cardTestTime, systemClock=systemClock, diskVolumeFileName=diskVolumeFileName, cmUChannelID=cmUChannelID, cmResetNotification=cmResetNotification, auditCommand=auditCommand, equipmentShelfGroup=equipmentShelfGroup, errEvLogLevel=errEvLogLevel, cardTestResult=cardTestResult, cardTrapInh=cardTrapInh, errEvRecoveryEnabled=errEvRecoveryEnabled, cardBootVersion=cardBootVersion, portMacAddress=portMacAddress, auditStatus=auditStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardDplxStatus=cardDplxStatus, equipmentAudit=equipmentAudit, cardShelfId=cardShelfId, errEvRowStatus=errEvRowStatus, cmMacAddress=cmMacAddress, portMode=portMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestType=cardTestType, cardRamSize=cardRamSize, diskVolumeTable=diskVolumeTable, diskVolumeDiskSize=diskVolumeDiskSize, portEntry=portEntry, cmDevice=cmDevice, diagTestId=diagTestId, cardFwUpdateStatus=cardFwUpdateStatus, TestResult=TestResult, portRxFlowControlMode=portRxFlowControlMode, cardAction=cardAction, portBgpId=portBgpId, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlDetected=portRxFlowControlDetected, cardLastBootVersion=cardLastBootVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, TestScheduleCommand=TestScheduleCommand, auditTotalPassedCount=auditTotalPassedCount, portId=portId, cardLastResetReason=cardLastResetReason, shelfName=shelfName, cardTestDescription=cardTestDescription, cardSecState=cardSecState, portTable=portTable, equipmentDiag=equipmentDiag, diskVolumeId=diskVolumeId, cmVendor=cmVendor, trapSeverity=trapSeverity, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeEntry=diskVolumeEntry, consoleOutput=consoleOutput, portSecState=portSecState, errEventId=errEventId, cardPicDetected=cardPicDetected, portType=portType, equipmentState=equipmentState, diskVolumeShelfId=diskVolumeShelfId, portPrStateChange=portPrStateChange, systemGeneral=systemGeneral, portDplxStatus=portDplxStatus, cmRegistrationNotification=cmRegistrationNotification, auditResult=auditResult, equipmentTraps=equipmentTraps, equipmentCardGroup=equipmentCardGroup, cardTable=cardTable, cardTemperatureHighError=cardTemperatureHighError, shelfSwVersion=shelfSwVersion, trapCounter=trapCounter, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTemperatureHighWarn=cardTemperatureHighWarn, auditId=auditId, cardTempNoReportNotification=cardTempNoReportNotification, portConnectorId=portConnectorId, remainInDiagMode=remainInDiagMode, portMinDsPower=portMinDsPower, auditAutoScheduling=auditAutoScheduling, portAction=portAction, portSecStateChange=portSecStateChange, cardName=cardName, cardCPLDVersion=cardCPLDVersion, TestCommand=TestCommand, TestId=TestId, portAdminState=portAdminState, auditName=auditName, cardSpareGroupMode=cardSpareGroupMode, cmResetClearNotification=cmResetClearNotification, cardCpuSpeed=cardCpuSpeed, cardType=cardType, cardDetectedChange=cardDetectedChange, equipmentDiagGroup=equipmentDiagGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetReason=cmResetReason, cardTestCommand=cardTestCommand, portDplxStatusChange=portDplxStatusChange, auditEntry=auditEntry, portDescription=portDescription, auditCycleCount=auditCycleCount, cardDetected=cardDetected, diskVolumeTrapInh=diskVolumeTrapInh, TestType=TestType, auditFailedCount=auditFailedCount, cmIpAddress=cmIpAddress, cardCpuType=cardCpuType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeSize=diskVolumeSize, auditLogOutput=auditLogOutput, cardAdminState=cardAdminState, cardEntry=cardEntry, cardId=cardId, cardFwVersion=cardFwVersion, verboseLevel=verboseLevel, auditTable=auditTable, portTxFlowControlDetected=portTxFlowControlDetected, cardFpgaSource=cardFpgaSource, cardTestTable=cardTestTable, equipmentAuditGroup=equipmentAuditGroup, cardTestIndex=cardTestIndex, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestTransId=cardTestTransId, cardNumber=cardNumber, portTrapInh=portTrapInh, auditTotalFailedCount=auditTotalFailedCount, portMaxDsPower=portMaxDsPower, diskVolumeGroup=diskVolumeGroup, portDocsIfIndex=portDocsIfIndex, auditPassedCount=auditPassedCount, portCardSubType=portCardSubType, cardTestResultDesc=cardTestResultDesc, cardSubDetected=cardSubDetected, cardCarrierSerialNum=cardCarrierSerialNum, TestSchedule=TestSchedule, cardShelfTestIndex=cardShelfTestIndex, auditShelfId=auditShelfId, cadEquipmentMib=cadEquipmentMib, diskVolumeName=diskVolumeName, systemKey=systemKey, portDetectedMode=portDetectedMode, cmResetStatus=cmResetStatus, cardLastChangeTime=cardLastChangeTime, cardSpareGroupId=cardSpareGroupId, cardHwVersion=cardHwVersion, portCardId=portCardId, portCurrDsPower=portCurrDsPower, cardPicSerialNum=cardPicSerialNum, systemGeneralGroup=systemGeneralGroup, cardSerialNum=cardSerialNum, diskVolumeNumber=diskVolumeNumber, portNumber=portNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, diskDriveNumber=diskDriveNumber, auditLogThrottle=auditLogThrottle, auditCardId=auditCardId, portNumChans=portNumChans, cadEquipmentMibGroup=cadEquipmentMibGroup, auditTime=auditTime, cardSubType=cardSubType, cardTestId=cardTestId, cmResetUpTime=cmResetUpTime, cardCarrierHwVersion=cardCarrierHwVersion, cardLastBootSource=cardLastBootSource, cmPrimarySID=cmPrimarySID, cmResetInfo=cmResetInfo, shelfObjects=shelfObjects, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardBusSpeed=cardBusSpeed, equipmentStateGroup=equipmentStateGroup, portTxFlowControlMode=portTxFlowControlMode, cardTemperature=cardTemperature, cardSecStateChange=cardSecStateChange, cardFlashSize=cardFlashSize, equipmentCardTestGroup=equipmentCardTestGroup, cardTestName=cardTestName, auditDescription=auditDescription, downstreamPowerLoss=downstreamPowerLoss)
