...
ShelfId, AdminState, DiskVolumeUsageLevel, CardSubType, EqActionType, FirmwareSource, PicType, PortMode, CardId, DuplexStatus, FlowControlMode, PrimaryState, PortType, SecondaryState, UnknownState, PortId, CardType, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "AdminState", "DiskVolumeUsageLevel", "CardSubType", "EqActionType", "FirmwareSource", "PicType", "PortMode", "CardId", "DuplexStatus", "FlowControlMode", "PrimaryState", "PortType", "SecondaryState", "UnknownState", "PortId", "CardType", "PortDetectedMode")
...
TextualConvention, MacAddress, DisplayString, TruthValue, TimeStamp, DateAndTime, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "DisplayString", "TruthValue", "TimeStamp", "DateAndTime", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portSecState=portSecState, TestTransactionId=TestTransactionId, diagTestId=diagTestId, equipmentError=equipmentError, portCardId=portCardId, portLastChangeTime=portLastChangeTime, equipmentAudit=equipmentAudit, portEntry=portEntry, TestScheduleCommand=TestScheduleCommand, cardId=cardId, auditStatus=auditStatus, cardAdminState=cardAdminState, cardLastResetReason=cardLastResetReason, auditId=auditId, cardTestSchedule=cardTestSchedule, equipmentCardTestGroup=equipmentCardTestGroup, cmMacAddress=cmMacAddress, portDplxStatus=portDplxStatus, portPrState=portPrState, cmResetNotification=cmResetNotification, cardTestDescription=cardTestDescription, cardPicDetected=cardPicDetected, auditTable=auditTable, diskVolumeEntry=diskVolumeEntry, fpgaErrorEventTable=fpgaErrorEventTable, equipmentTbl=equipmentTbl, errEvRecoveryEnabled=errEvRecoveryEnabled, auditName=auditName, auditTotalPassedCount=auditTotalPassedCount, cardSecState=cardSecState, portAdminState=portAdminState, equipmentCardGroup=equipmentCardGroup, equipmentShelfGroup=equipmentShelfGroup, trapSeverity=trapSeverity, cardSubDetected=cardSubDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, cardDetectedChange=cardDetectedChange, cadEquipmentMib=cadEquipmentMib, cardAction=cardAction, auditAutoScheduling=auditAutoScheduling, portMacIfIndex=portMacIfIndex, auditResult=auditResult, diskVolumeTable=diskVolumeTable, auditShelfId=auditShelfId, downstreamPowerLoss=downstreamPowerLoss, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestIndex=cardTestIndex, cmResetStatus=cmResetStatus, cmRegistrationNotification=cmRegistrationNotification, errEvRowStatus=errEvRowStatus, diskVolumeShelfId=diskVolumeShelfId, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentPortGroup=equipmentPortGroup, portMinDsPower=portMinDsPower, cardShelfTestIndex=cardShelfTestIndex, cardNumPorts=cardNumPorts, portTxFlowControlMode=portTxFlowControlMode, portMode=portMode, shelfName=shelfName, trapCounter=trapCounter, cardSwVersion=cardSwVersion, cardUpTime=cardUpTime, auditEntry=auditEntry, portDescription=portDescription, cardSerialNum=cardSerialNum, portBgpId=portBgpId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardBusSpeed=cardBusSpeed, auditTime=auditTime, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestScheduleCommand=cardTestScheduleCommand, cardCpuSpeed=cardCpuSpeed, cardTestResult=cardTestResult, systemKey=systemKey, portTrapInh=portTrapInh, errEvLogLevel=errEvLogLevel, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardPicSerialNum=cardPicSerialNum, cmResetInfo=cmResetInfo, cardType=cardType, errEvLoggingEnabled=errEvLoggingEnabled, cardTemperatureHighError=cardTemperatureHighError, cardEntry=cardEntry, diskVolumeName=diskVolumeName, diskVolumeDiskSize=diskVolumeDiskSize, cardSecStateChange=cardSecStateChange, cardDplxStatus=cardDplxStatus, shelfObjects=shelfObjects, cardDplxStatusChange=cardDplxStatusChange, portShelfId=portShelfId, portNumChans=portNumChans, equipmentStateGroup=equipmentStateGroup, cardTable=cardTable, cardNumber=cardNumber, equipmentTraps=equipmentTraps, diskVolumeFileName=diskVolumeFileName, equipmentDiag=equipmentDiag, verboseLevel=verboseLevel, portCurrDsPower=portCurrDsPower, auditCommand=auditCommand, cardCpuType=cardCpuType, TestResult=TestResult, portPrStateChange=portPrStateChange, cardHwVersion=cardHwVersion, cardLastBootSource=cardLastBootSource, errEventId=errEventId, portAction=portAction, systemGeneral=systemGeneral, auditCycleCount=auditCycleCount, cardTestTime=cardTestTime, portType=portType, auditLogThrottle=auditLogThrottle, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPrState=cardPrState, TestType=TestType, cardCPLDVersion=cardCPLDVersion, cardShelfId=cardShelfId, portMacAddress=portMacAddress, cardSpareGroupId=cardSpareGroupId, portDocsIfIndex=portDocsIfIndex, cmUChannelID=cmUChannelID, cmIpAddress=cmIpAddress, equipmentAuditGroup=equipmentAuditGroup, auditCardId=auditCardId, cardTempNoReportNotification=cardTempNoReportNotification, systemClock=systemClock, auditFailedCount=auditFailedCount, cardName=cardName, cardTestName=cardTestName, cardTestCommand=cardTestCommand, diskVolumeTrapInh=diskVolumeTrapInh, cmResetUpTime=cmResetUpTime, portSecStateChange=portSecStateChange, consoleOutput=consoleOutput, systemGeneralGroup=systemGeneralGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetClearNotification=cmResetClearNotification, equipmentDiagGroup=equipmentDiagGroup, auditPassedCount=auditPassedCount, cardFwVersion=cardFwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestType=cardTestType, cardTestEntry=cardTestEntry, portRxFlowControlMode=portRxFlowControlMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeGroup=diskVolumeGroup, remainInDiagMode=remainInDiagMode, cadEquipmentMibGroup=cadEquipmentMibGroup, portNumber=portNumber, cardFlashSize=cardFlashSize, cardTestTransId=cardTestTransId, shelfNumber=shelfNumber, cardCarrierSerialNum=cardCarrierSerialNum, cardSubType=cardSubType, diskVolumeSize=diskVolumeSize, cmVendor=cmVendor, equipmentState=equipmentState, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestResultDesc=cardTestResultDesc, cmPrimarySID=cmPrimarySID, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestCommand=TestCommand, portId=portId, diskVolumeNumber=diskVolumeNumber, diskVolumeId=diskVolumeId, cardRamSize=cardRamSize, cardDetected=cardDetected, cardTestId=cardTestId, cardPicHwVersion=cardPicHwVersion, shelfSwVersion=shelfSwVersion, diskVolumeCardId=diskVolumeCardId, TestId=TestId, auditTotalFailedCount=auditTotalFailedCount, cardBootVersion=cardBootVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSpareGroupMode=cardSpareGroupMode, portCardSubType=portCardSubType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastBootVersion=cardLastBootVersion, eqDiagConfig=eqDiagConfig, portDplxStatusChange=portDplxStatusChange, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskDriveNumber=diskDriveNumber, cardCarrierFwVersion=cardCarrierFwVersion, cardFpgaSource=cardFpgaSource, cmResetReason=cmResetReason, cardTrapInh=cardTrapInh, portRxFlowControlDetected=portRxFlowControlDetected, auditDescription=auditDescription, diskVolumeDriveId=diskVolumeDriveId, cardTemperature=cardTemperature, cardTestResultNotification=cardTestResultNotification, portDetectedMode=portDetectedMode, cardPrStateChange=cardPrStateChange, portMaxDsPower=portMaxDsPower, portTxFlowControlDetected=portTxFlowControlDetected, cmDevice=cmDevice, auditLogOutput=auditLogOutput, cardLastChangeTime=cardLastChangeTime, TestSchedule=TestSchedule, portTable=portTable, portConnectorId=portConnectorId, cardTestTable=cardTestTable)
