...
CardType, PortDetectedMode, AdminState, FirmwareSource, UnknownState, SecondaryState, CardSubType, PicType, ShelfId, PortMode, PrimaryState, PortType, DuplexStatus, FlowControlMode, DiskVolumeUsageLevel, PortId, CardId, EqActionType = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortDetectedMode", "AdminState", "FirmwareSource", "UnknownState", "SecondaryState", "CardSubType", "PicType", "ShelfId", "PortMode", "PrimaryState", "PortType", "DuplexStatus", "FlowControlMode", "DiskVolumeUsageLevel", "PortId", "CardId", "EqActionType")
...
TextualConvention, RowStatus, TimeStamp, DisplayString, DateAndTime, TruthValue, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "TimeStamp", "DisplayString", "DateAndTime", "TruthValue", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageNotification=diskVolumeUsageNotification, portShelfId=portShelfId, portMacAddress=portMacAddress, portAction=portAction, fpgaErrorEventGroup=fpgaErrorEventGroup, systemKey=systemKey, diskVolumeCardId=diskVolumeCardId, equipmentAudit=equipmentAudit, cmResetReason=cmResetReason, diskVolumeSize=diskVolumeSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditStatus=auditStatus, diskVolumeGroup=diskVolumeGroup, cardLastResetReason=cardLastResetReason, systemGeneral=systemGeneral, auditLogThrottle=auditLogThrottle, equipmentAuditGroup=equipmentAuditGroup, cardTable=cardTable, systemGeneralGroup=systemGeneralGroup, cardTestEntry=cardTestEntry, auditCommand=auditCommand, portDetectedMode=portDetectedMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditId=auditId, portAdminState=portAdminState, remainInDiagMode=remainInDiagMode, errEvLogLevel=errEvLogLevel, cardDetected=cardDetected, cardPicDetected=cardPicDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, cardType=cardType, diskVolumeDiskSize=diskVolumeDiskSize, TestScheduleCommand=TestScheduleCommand, diskVolumeEntry=diskVolumeEntry, portRxFlowControlMode=portRxFlowControlMode, cardTemperatureHighWarn=cardTemperatureHighWarn, auditCycleCount=auditCycleCount, cardSpareGroupId=cardSpareGroupId, cardDplxStatus=cardDplxStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentTraps=equipmentTraps, cardTestDescription=cardTestDescription, cmResetInfo=cmResetInfo, portType=portType, cardCpuSpeed=cardCpuSpeed, cmVendor=cmVendor, shelfObjects=shelfObjects, cardCarrierHwVersion=cardCarrierHwVersion, auditShelfId=auditShelfId, cardRamSize=cardRamSize, cmPrimarySID=cmPrimarySID, equipmentCardGroup=equipmentCardGroup, cardTestIndex=cardTestIndex, cardFwVersion=cardFwVersion, diskVolumeNumber=diskVolumeNumber, auditFailedCount=auditFailedCount, portBgpId=portBgpId, auditPassedCount=auditPassedCount, cardEntry=cardEntry, TestCommand=TestCommand, auditCardId=auditCardId, diskVolumeFileName=diskVolumeFileName, TestSchedule=TestSchedule, shelfName=shelfName, TestResult=TestResult, portDplxStatusChange=portDplxStatusChange, eqDiagConfig=eqDiagConfig, equipmentShelfGroup=equipmentShelfGroup, cmResetStatus=cmResetStatus, errEventId=errEventId, trapCounter=trapCounter, portDescription=portDescription, equipmentDiag=equipmentDiag, auditTime=auditTime, cardNumber=cardNumber, cardId=cardId, cardAction=cardAction, cadEquipmentMib=cadEquipmentMib, cardSubDetected=cardSubDetected, cardUpTime=cardUpTime, equipmentStateGroup=equipmentStateGroup, cmResetNotification=cmResetNotification, fpgaErrorEventEntry=fpgaErrorEventEntry, cmDevice=cmDevice, portTable=portTable, equipmentTbl=equipmentTbl, cardDplxStatusChange=cardDplxStatusChange, auditTotalFailedCount=auditTotalFailedCount, diskVolumeTable=diskVolumeTable, cardTestType=cardTestType, errEvLoggingEnabled=errEvLoggingEnabled, cmMacAddress=cmMacAddress, cardDetectedChange=cardDetectedChange, cardSecStateChange=cardSecStateChange, cardCPLDVersion=cardCPLDVersion, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestSchedule=cardTestSchedule, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTime=cardTestTime, systemClock=systemClock, cardAdminState=cardAdminState, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditResult=auditResult, portEntry=portEntry, cardPrState=cardPrState, cardSpareGroupMode=cardSpareGroupMode, cardTestScheduleCommand=cardTestScheduleCommand, diskDriveNumber=diskDriveNumber, diskVolumeName=diskVolumeName, cardTestId=cardTestId, TestTransactionId=TestTransactionId, cmIpAddress=cmIpAddress, portSecStateChange=portSecStateChange, TestId=TestId, cardName=cardName, equipmentCardTestGroup=equipmentCardTestGroup, cardLastBootVersion=cardLastBootVersion, cardFlashSize=cardFlashSize, portNumber=portNumber, cadEquipmentMibGroup=cadEquipmentMibGroup, portPrStateChange=portPrStateChange, cardTestResultNotification=cardTestResultNotification, PYSNMP_MODULE_ID=cadEquipmentMib, verboseLevel=verboseLevel, cmResetClearNotification=cmResetClearNotification, portPrState=portPrState, portDocsIfIndex=portDocsIfIndex, equipmentPortGroup=equipmentPortGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardSecState=cardSecState, cardTemperatureHighError=cardTemperatureHighError, portLastChangeTime=portLastChangeTime, portMaxDsPower=portMaxDsPower, errEvRowStatus=errEvRowStatus, cardCpuType=cardCpuType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditTable=auditTable, cmRegistrationNotification=cmRegistrationNotification, cardLastBootSource=cardLastBootSource, cardTestResultDesc=cardTestResultDesc, cardShelfTestIndex=cardShelfTestIndex, cardSerialNum=cardSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeTrapInh=diskVolumeTrapInh, portCurrDsPower=portCurrDsPower, portCardSubType=portCardSubType, auditTotalPassedCount=auditTotalPassedCount, trapSeverity=trapSeverity, diskVolumeShelfId=diskVolumeShelfId, cmResetUpTime=cmResetUpTime, consoleOutput=consoleOutput, cardTempNoReportNotification=cardTempNoReportNotification, portNumChans=portNumChans, cardBootVersion=cardBootVersion, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeDriveId=diskVolumeDriveId, auditAutoScheduling=auditAutoScheduling, portCardId=portCardId, cardTestTable=cardTestTable, equipmentState=equipmentState, cardTemperature=cardTemperature, cardTestTransId=cardTestTransId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardPicSerialNum=cardPicSerialNum, equipmentDiagGroup=equipmentDiagGroup, TestType=TestType, portTxFlowControlDetected=portTxFlowControlDetected, portMacIfIndex=portMacIfIndex, shelfNumber=shelfNumber, shelfSwVersion=shelfSwVersion, auditName=auditName, equipmentError=equipmentError, portTxFlowControlMode=portTxFlowControlMode, portSecState=portSecState, cardTrapInh=cardTrapInh, cardCarrierSerialNum=cardCarrierSerialNum, cardPicHwVersion=cardPicHwVersion, cardPrStateChange=cardPrStateChange, cardBusSpeed=cardBusSpeed, auditLogOutput=auditLogOutput, diskVolumeId=diskVolumeId, cardFpgaSource=cardFpgaSource, cardLastChangeTime=cardLastChangeTime, cmUChannelID=cmUChannelID, portDplxStatus=portDplxStatus, cardTestName=cardTestName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, downstreamPowerLoss=downstreamPowerLoss, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResult=cardTestResult, diagTestId=diagTestId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardHwVersion=cardHwVersion, cardSwVersion=cardSwVersion, portId=portId, cardSubType=cardSubType, cardCarrierFwVersion=cardCarrierFwVersion, portMode=portMode, portConnectorId=portConnectorId, auditEntry=auditEntry, cardTestCommand=cardTestCommand, auditDescription=auditDescription, portMinDsPower=portMinDsPower, portTrapInh=portTrapInh, cardShelfId=cardShelfId)
