...
AdminState, CardType, PortId, FirmwareSource, ShelfId, FlowControlMode, PortMode, UnknownState, PrimaryState, CardId, PortType, DuplexStatus, PicType, CardSubType, EqActionType, SecondaryState, DiskVolumeUsageLevel, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "AdminState", "CardType", "PortId", "FirmwareSource", "ShelfId", "FlowControlMode", "PortMode", "UnknownState", "PrimaryState", "CardId", "PortType", "DuplexStatus", "PicType", "CardSubType", "EqActionType", "SecondaryState", "DiskVolumeUsageLevel", "PortDetectedMode")
...
TextualConvention, RowStatus, MacAddress, DateAndTime, DisplayString, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "MacAddress", "DateAndTime", "DisplayString", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentDiagGroup=equipmentDiagGroup, portMaxDsPower=portMaxDsPower, equipmentError=equipmentError, cardSpareGroupId=cardSpareGroupId, auditLogThrottle=auditLogThrottle, cardTestResultDesc=cardTestResultDesc, shelfObjects=shelfObjects, portPrState=portPrState, cardBootVersion=cardBootVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, errEvLogLevel=errEvLogLevel, cardTestCommand=cardTestCommand, portSecStateChange=portSecStateChange, trapSeverity=trapSeverity, portDocsIfIndex=portDocsIfIndex, portMacAddress=portMacAddress, cmIpAddress=cmIpAddress, portType=portType, equipmentAudit=equipmentAudit, portNumChans=portNumChans, cardPrStateChange=cardPrStateChange, auditTotalFailedCount=auditTotalFailedCount, TestScheduleCommand=TestScheduleCommand, cardTempOverHeatNotification=cardTempOverHeatNotification, portSecState=portSecState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, shelfSwVersion=shelfSwVersion, equipmentShelfGroup=equipmentShelfGroup, cardCPLDVersion=cardCPLDVersion, cmUChannelID=cmUChannelID, portDplxStatus=portDplxStatus, cardCpuSpeed=cardCpuSpeed, cardRamSize=cardRamSize, cardShelfTestIndex=cardShelfTestIndex, auditResult=auditResult, cardLastChangeTime=cardLastChangeTime, portCurrDsPower=portCurrDsPower, diskVolumeDiskSize=diskVolumeDiskSize, errEvLoggingEnabled=errEvLoggingEnabled, portDetectedMode=portDetectedMode, equipmentStateGroup=equipmentStateGroup, diskVolumeName=diskVolumeName, cardAdminState=cardAdminState, TestType=TestType, cardDplxStatusChange=cardDplxStatusChange, cardSecStateChange=cardSecStateChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTemperature=cardTemperature, cadEquipmentMib=cadEquipmentMib, cardId=cardId, portShelfId=portShelfId, equipmentTbl=equipmentTbl, cardBusSpeed=cardBusSpeed, cardTestType=cardTestType, equipmentPortGroup=equipmentPortGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSubDetected=cardSubDetected, auditDescription=auditDescription, cmDevice=cmDevice, portAction=portAction, auditStatus=auditStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, systemGeneral=systemGeneral, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentTraps=equipmentTraps, cardNumber=cardNumber, auditName=auditName, errEvRowStatus=errEvRowStatus, TestTransactionId=TestTransactionId, portDplxStatusChange=portDplxStatusChange, diskDriveNumber=diskDriveNumber, diskVolumeDriveId=diskVolumeDriveId, portMode=portMode, verboseLevel=verboseLevel, portMinDsPower=portMinDsPower, portAdminState=portAdminState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diagTestId=diagTestId, cardSpareGroupMode=cardSpareGroupMode, cardTestTime=cardTestTime, TestCommand=TestCommand, auditCommand=auditCommand, cmMacAddress=cmMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, portTable=portTable, diskVolumeEntry=diskVolumeEntry, eqDiagConfig=eqDiagConfig, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardPicDetected=cardPicDetected, cardUpTime=cardUpTime, portTxFlowControlMode=portTxFlowControlMode, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeSize=diskVolumeSize, cardTestSchedule=cardTestSchedule, cardSubType=cardSubType, cardTestEntry=cardTestEntry, cmResetReason=cmResetReason, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeShelfId=diskVolumeShelfId, cardDetected=cardDetected, TestId=TestId, cmResetNotification=cmResetNotification, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeNumber=diskVolumeNumber, cardTempNoReportNotification=cardTempNoReportNotification, auditTime=auditTime, portTrapInh=portTrapInh, cardHwVersion=cardHwVersion, cardPrState=cardPrState, auditAutoScheduling=auditAutoScheduling, systemGeneralGroup=systemGeneralGroup, portCardSubType=portCardSubType, portEntry=portEntry, cardTestTransId=cardTestTransId, consoleOutput=consoleOutput, systemKey=systemKey, equipmentState=equipmentState, PYSNMP_MODULE_ID=cadEquipmentMib, cardFwUpdateStatus=cardFwUpdateStatus, portConnectorId=portConnectorId, auditLogOutput=auditLogOutput, fpgaErrorEventGroup=fpgaErrorEventGroup, auditId=auditId, cardFlashSize=cardFlashSize, cardTable=cardTable, cardTemperatureHighError=cardTemperatureHighError, portLastChangeTime=portLastChangeTime, TestResult=TestResult, portRxFlowControlMode=portRxFlowControlMode, portBgpId=portBgpId, cardLastResetReason=cardLastResetReason, cardTestName=cardTestName, cardPicHwVersion=cardPicHwVersion, portPrStateChange=portPrStateChange, auditEntry=auditEntry, auditTable=auditTable, equipmentCardGroup=equipmentCardGroup, equipmentCardTestGroup=equipmentCardTestGroup, cardCpuType=cardCpuType, cardSecState=cardSecState, equipmentDiag=equipmentDiag, cardTestResult=cardTestResult, cardTestResultNotification=cardTestResultNotification, cardFwVersion=cardFwVersion, cmVendor=cmVendor, cardCarrierHwVersion=cardCarrierHwVersion, cardShelfId=cardShelfId, remainInDiagMode=remainInDiagMode, cardDplxStatus=cardDplxStatus, cardTestId=cardTestId, diskVolumeCardId=diskVolumeCardId, errEventId=errEventId, cardCarrierFwVersion=cardCarrierFwVersion, cardSerialNum=cardSerialNum, portId=portId, auditShelfId=auditShelfId, cardTestScheduleCommand=cardTestScheduleCommand, cardTestTable=cardTestTable, cadEquipmentMibGroup=cadEquipmentMibGroup, auditFailedCount=auditFailedCount, cardFpgaSource=cardFpgaSource, cardTrapInh=cardTrapInh, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetInfo=cmResetInfo, diskVolumeId=diskVolumeId, portDescription=portDescription, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTable=diskVolumeTable, auditCardId=auditCardId, auditPassedCount=auditPassedCount, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, systemClock=systemClock, cardNumPorts=cardNumPorts, portNumber=portNumber, portTxFlowControlDetected=portTxFlowControlDetected, cardTestIndex=cardTestIndex, cardTestDescription=cardTestDescription, equipmentAuditGroup=equipmentAuditGroup, cmResetClearNotification=cmResetClearNotification, cardType=cardType, TestSchedule=TestSchedule, auditCycleCount=auditCycleCount, shelfNumber=shelfNumber, cardPicSerialNum=cardPicSerialNum, downstreamPowerLoss=downstreamPowerLoss, trapCounter=trapCounter, cardEntry=cardEntry, auditTotalPassedCount=auditTotalPassedCount, cardSwVersion=cardSwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardLastBootSource=cardLastBootSource, cardAction=cardAction, shelfName=shelfName, cardCarrierSerialNum=cardCarrierSerialNum, cardDetectedChange=cardDetectedChange, diskVolumeTrapInh=diskVolumeTrapInh, cmResetStatus=cmResetStatus, portMacIfIndex=portMacIfIndex, cmRegistrationNotification=cmRegistrationNotification, cardName=cardName, portCardId=portCardId, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeFileName=diskVolumeFileName, cmResetUpTime=cmResetUpTime, cardLastBootVersion=cardLastBootVersion)
