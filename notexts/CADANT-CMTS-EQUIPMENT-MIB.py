...
CardType, UnknownState, EqActionType, SecondaryState, PortMode, PortId, AdminState, DiskVolumeUsageLevel, PrimaryState, FlowControlMode, ShelfId, DuplexStatus, CardId, PortType, FirmwareSource, CardSubType, PortDetectedMode, PicType = mibBuilder.importSymbols("CADANT-TC", "CardType", "UnknownState", "EqActionType", "SecondaryState", "PortMode", "PortId", "AdminState", "DiskVolumeUsageLevel", "PrimaryState", "FlowControlMode", "ShelfId", "DuplexStatus", "CardId", "PortType", "FirmwareSource", "CardSubType", "PortDetectedMode", "PicType")
...
TextualConvention, DateAndTime, DisplayString, TruthValue, RowStatus, TimeStamp, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "DisplayString", "TruthValue", "RowStatus", "TimeStamp", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageLevel=diskVolumeUsageLevel, fpgaErrorEventTable=fpgaErrorEventTable, cardCpuSpeed=cardCpuSpeed, portMacIfIndex=portMacIfIndex, cardShelfTestIndex=cardShelfTestIndex, cmResetUpTime=cmResetUpTime, auditLogThrottle=auditLogThrottle, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portType=portType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardEntry=cardEntry, portAction=portAction, cmResetStatus=cmResetStatus, cardCpuType=cardCpuType, errEvRowStatus=errEvRowStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMib=cadEquipmentMib, equipmentCardGroup=equipmentCardGroup, downstreamPowerLoss=downstreamPowerLoss, cardId=cardId, cardTestEntry=cardTestEntry, cardTempNoReportNotification=cardTempNoReportNotification, cardSerialNum=cardSerialNum, equipmentAuditGroup=equipmentAuditGroup, cardSecState=cardSecState, portPrState=portPrState, cardSwVersion=cardSwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardFwUpdateStatus=cardFwUpdateStatus, portAdminState=portAdminState, portDocsIfIndex=portDocsIfIndex, diskVolumeUsageNotification=diskVolumeUsageNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardPicSerialNum=cardPicSerialNum, TestId=TestId, cardTable=cardTable, diagTestId=diagTestId, cardTestTransId=cardTestTransId, portDetectedMode=portDetectedMode, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditResult=auditResult, TestSchedule=TestSchedule, auditFailedCount=auditFailedCount, cardLastChangeTime=cardLastChangeTime, diskVolumeCardId=diskVolumeCardId, equipmentAudit=equipmentAudit, systemGeneral=systemGeneral, cmPrimarySID=cmPrimarySID, cardTrapInh=cardTrapInh, cardType=cardType, cardTestResultDesc=cardTestResultDesc, cardTestResult=cardTestResult, cardSpareGroupMode=cardSpareGroupMode, cardNumPorts=cardNumPorts, portTxFlowControlDetected=portTxFlowControlDetected, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetInfo=cmResetInfo, TestTransactionId=TestTransactionId, cardSecStateChange=cardSecStateChange, cmMacAddress=cmMacAddress, cardLastBootSource=cardLastBootSource, cardDetectedChange=cardDetectedChange, cardCarrierSerialNum=cardCarrierSerialNum, cardPicDetected=cardPicDetected, auditPassedCount=auditPassedCount, cardPrState=cardPrState, equipmentDiag=equipmentDiag, errEvRecoveryEnabled=errEvRecoveryEnabled, cardBusSpeed=cardBusSpeed, cardTemperature=cardTemperature, diskVolumeName=diskVolumeName, shelfSwVersion=shelfSwVersion, auditLogOutput=auditLogOutput, errEventId=errEventId, cardLastResetReason=cardLastResetReason, auditTotalPassedCount=auditTotalPassedCount, cardCarrierFwVersion=cardCarrierFwVersion, remainInDiagMode=remainInDiagMode, cardSpareGroupId=cardSpareGroupId, cardCarrierHwVersion=cardCarrierHwVersion, auditTotalFailedCount=auditTotalFailedCount, portTxFlowControlMode=portTxFlowControlMode, equipmentError=equipmentError, cmUChannelID=cmUChannelID, equipmentTraps=equipmentTraps, cardAction=cardAction, diskVolumeSize=diskVolumeSize, equipmentPortGroup=equipmentPortGroup, cardFpgaSource=cardFpgaSource, cmResetReason=cmResetReason, auditName=auditName, cardName=cardName, cardTestType=cardTestType, cardDplxStatusChange=cardDplxStatusChange, portDplxStatus=portDplxStatus, diskVolumeTrapInh=diskVolumeTrapInh, portMacAddress=portMacAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDescription=portDescription, portBgpId=portBgpId, portMinDsPower=portMinDsPower, auditCycleCount=auditCycleCount, portLastChangeTime=portLastChangeTime, portCardSubType=portCardSubType, cardTestDescription=cardTestDescription, cardTestCommand=cardTestCommand, auditTime=auditTime, cmResetNotification=cmResetNotification, portMode=portMode, shelfObjects=shelfObjects, auditCardId=auditCardId, portSecState=portSecState, cardPrStateChange=cardPrStateChange, cardUpTime=cardUpTime, systemGeneralGroup=systemGeneralGroup, portConnectorId=portConnectorId, equipmentTbl=equipmentTbl, errEvLogLevel=errEvLogLevel, diskVolumeNumber=diskVolumeNumber, auditId=auditId, cmRegistrationNotification=cmRegistrationNotification, auditStatus=auditStatus, cardHwVersion=cardHwVersion, portRxFlowControlMode=portRxFlowControlMode, portShelfId=portShelfId, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentDiagGroup=equipmentDiagGroup, cardFwVersion=cardFwVersion, consoleOutput=consoleOutput, systemClock=systemClock, verboseLevel=verboseLevel, cmVendor=cmVendor, cardPicHwVersion=cardPicHwVersion, auditCommand=auditCommand, portTrapInh=portTrapInh, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestId=cardTestId, cardShelfId=cardShelfId, systemKey=systemKey, equipmentStateGroup=equipmentStateGroup, eqDiagConfig=eqDiagConfig, diskVolumeFileName=diskVolumeFileName, equipmentShelfGroup=equipmentShelfGroup, equipmentState=equipmentState, diskVolumeId=diskVolumeId, portEntry=portEntry, cardTestTime=cardTestTime, cardDplxStatus=cardDplxStatus, cmDevice=cmDevice, trapCounter=trapCounter, cardDetected=cardDetected, cardSubDetected=cardSubDetected, auditTable=auditTable, auditShelfId=auditShelfId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cardAdminState=cardAdminState, cardSubType=cardSubType, cardTestResultNotification=cardTestResultNotification, shelfNumber=shelfNumber, cardRamSize=cardRamSize, fpgaErrorEventEntry=fpgaErrorEventEntry, TestCommand=TestCommand, diskVolumeEntry=diskVolumeEntry, shelfName=shelfName, trapSeverity=trapSeverity, portTable=portTable, cmResetClearNotification=cmResetClearNotification, portMaxDsPower=portMaxDsPower, TestScheduleCommand=TestScheduleCommand, cardBootVersion=cardBootVersion, cardTemperatureHighError=cardTemperatureHighError, portCurrDsPower=portCurrDsPower, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestResult=TestResult, cardTestIndex=cardTestIndex, errEvLoggingEnabled=errEvLoggingEnabled, cardTestSchedule=cardTestSchedule, diskVolumeDriveId=diskVolumeDriveId, cardTestTable=cardTestTable, portNumChans=portNumChans, diskVolumeGroup=diskVolumeGroup, auditDescription=auditDescription, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeShelfId=diskVolumeShelfId, cardNumber=cardNumber, portPrStateChange=portPrStateChange, diskVolumeTable=diskVolumeTable, auditEntry=auditEntry, cardTestName=cardTestName, cardLastBootVersion=cardLastBootVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portNumber=portNumber, auditAutoScheduling=auditAutoScheduling, portDplxStatusChange=portDplxStatusChange, portRxFlowControlDetected=portRxFlowControlDetected, cmIpAddress=cmIpAddress, TestType=TestType, diskDriveNumber=diskDriveNumber, diskVolumeDiskSize=diskVolumeDiskSize, cardFlashSize=cardFlashSize, equipmentCardTestGroup=equipmentCardTestGroup, cardTestScheduleCommand=cardTestScheduleCommand, cardCPLDVersion=cardCPLDVersion, portId=portId, portSecStateChange=portSecStateChange, portCardId=portCardId)
