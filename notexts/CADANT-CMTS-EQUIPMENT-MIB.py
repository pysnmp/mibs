...
PortType, CardId, FlowControlMode, PicType, SecondaryState, CardSubType, PortId, EqActionType, UnknownState, AdminState, FirmwareSource, PortMode, PortDetectedMode, CardType, DuplexStatus, PrimaryState, ShelfId, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "PortType", "CardId", "FlowControlMode", "PicType", "SecondaryState", "CardSubType", "PortId", "EqActionType", "UnknownState", "AdminState", "FirmwareSource", "PortMode", "PortDetectedMode", "CardType", "DuplexStatus", "PrimaryState", "ShelfId", "DiskVolumeUsageLevel")
...
DisplayString, RowStatus, MacAddress, TextualConvention, TimeStamp, DateAndTime, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "RowStatus", "MacAddress", "TextualConvention", "TimeStamp", "DateAndTime", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBusSpeed=cardBusSpeed, portPrStateChange=portPrStateChange, equipmentTbl=equipmentTbl, cardCarrierHwVersion=cardCarrierHwVersion, cardName=cardName, eqDiagConfig=eqDiagConfig, portMode=portMode, cardTestSchedule=cardTestSchedule, auditCommand=auditCommand, errEvLogLevel=errEvLogLevel, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSecState=cardSecState, portNumber=portNumber, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portTrapInh=portTrapInh, portCurrDsPower=portCurrDsPower, equipmentAuditGroup=equipmentAuditGroup, cardTestEntry=cardTestEntry, portEntry=portEntry, cardHwVersion=cardHwVersion, shelfNumber=shelfNumber, portDocsIfIndex=portDocsIfIndex, diskVolumeSize=diskVolumeSize, trapSeverity=trapSeverity, cmPrimarySID=cmPrimarySID, cardBootVersion=cardBootVersion, systemGeneral=systemGeneral, auditName=auditName, cardTestType=cardTestType, cardEntry=cardEntry, auditTotalFailedCount=auditTotalFailedCount, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetNotification=cmResetNotification, cardTestTransId=cardTestTransId, systemKey=systemKey, cardDetected=cardDetected, diskVolumeShelfId=diskVolumeShelfId, cardFpgaSource=cardFpgaSource, portAdminState=portAdminState, shelfName=shelfName, cardLastChangeTime=cardLastChangeTime, cardCarrierFwVersion=cardCarrierFwVersion, auditTable=auditTable, cardTempNoReportNotification=cardTempNoReportNotification, cardLastBootSource=cardLastBootSource, portDetectedMode=portDetectedMode, portNumChans=portNumChans, TestSchedule=TestSchedule, equipmentDiag=equipmentDiag, cardSecStateChange=cardSecStateChange, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResult=cardTestResult, portSecStateChange=portSecStateChange, cardTestScheduleCommand=cardTestScheduleCommand, auditCardId=auditCardId, TestCommand=TestCommand, cardAdminState=cardAdminState, diskVolumeDriveId=diskVolumeDriveId, cardSpareGroupMode=cardSpareGroupMode, cardPrStateChange=cardPrStateChange, remainInDiagMode=remainInDiagMode, cardPicDetected=cardPicDetected, cardDetectedChange=cardDetectedChange, cadEquipmentMibGroup=cadEquipmentMibGroup, portCardId=portCardId, consoleOutput=consoleOutput, errEvLoggingEnabled=errEvLoggingEnabled, auditStatus=auditStatus, cardCpuSpeed=cardCpuSpeed, cardTemperatureHighError=cardTemperatureHighError, diskVolumeGroup=diskVolumeGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, diskDriveNumber=diskDriveNumber, portMinDsPower=portMinDsPower, cmIpAddress=cmIpAddress, portTable=portTable, cmMacAddress=cmMacAddress, portId=portId, cardTestName=cardTestName, TestType=TestType, cardTestCommand=cardTestCommand, diskVolumeUsagePercentage=diskVolumeUsagePercentage, verboseLevel=verboseLevel, TestResult=TestResult, TestTransactionId=TestTransactionId, equipmentState=equipmentState, portShelfId=portShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentDiagGroup=equipmentDiagGroup, auditTime=auditTime, diskVolumeId=diskVolumeId, cardId=cardId, cardCpuType=cardCpuType, cardTestDescription=cardTestDescription, diskVolumeName=diskVolumeName, cmResetStatus=cmResetStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, cardAction=cardAction, cardNumPorts=cardNumPorts, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, shelfObjects=shelfObjects, auditLogThrottle=auditLogThrottle, equipmentCardTestGroup=equipmentCardTestGroup, cardTestResultDesc=cardTestResultDesc, cmResetInfo=cmResetInfo, portConnectorId=portConnectorId, cardShelfId=cardShelfId, cardTestTable=cardTestTable, cmUChannelID=cmUChannelID, trapCounter=trapCounter, portDescription=portDescription, cardRamSize=cardRamSize, cardCPLDVersion=cardCPLDVersion, downstreamPowerLoss=downstreamPowerLoss, portMaxDsPower=portMaxDsPower, portMacIfIndex=portMacIfIndex, cardPicSerialNum=cardPicSerialNum, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardNumber=cardNumber, auditEntry=auditEntry, portPrState=portPrState, diskVolumeTable=diskVolumeTable, auditCycleCount=auditCycleCount, cmRegistrationNotification=cmRegistrationNotification, cardTestId=cardTestId, cardPrState=cardPrState, auditResult=auditResult, fpgaErrorEventTable=fpgaErrorEventTable, cmDevice=cmDevice, portType=portType, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cmVendor=cmVendor, equipmentShelfGroup=equipmentShelfGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentStateGroup=equipmentStateGroup, cardCarrierSerialNum=cardCarrierSerialNum, cardPicHwVersion=cardPicHwVersion, portSecState=portSecState, cardTrapInh=cardTrapInh, cardFlashSize=cardFlashSize, errEvRecoveryEnabled=errEvRecoveryEnabled, portMacAddress=portMacAddress, cardDplxStatusChange=cardDplxStatusChange, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeCardId=diskVolumeCardId, auditAutoScheduling=auditAutoScheduling, diskVolumeTrapInh=diskVolumeTrapInh, auditId=auditId, auditPassedCount=auditPassedCount, cmResetUpTime=cmResetUpTime, cmResetClearNotification=cmResetClearNotification, portDplxStatus=portDplxStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMib=cadEquipmentMib, equipmentAudit=equipmentAudit, cardTemperature=cardTemperature, cmResetReason=cmResetReason, cardSubDetected=cardSubDetected, auditFailedCount=auditFailedCount, diskVolumeNumber=diskVolumeNumber, cardUpTime=cardUpTime, cardSerialNum=cardSerialNum, equipmentError=equipmentError, errEventId=errEventId, portTxFlowControlDetected=portTxFlowControlDetected, TestId=TestId, portBgpId=portBgpId, auditTotalPassedCount=auditTotalPassedCount, portRxFlowControlMode=portRxFlowControlMode, PYSNMP_MODULE_ID=cadEquipmentMib, systemClock=systemClock, portLastChangeTime=portLastChangeTime, equipmentCardGroup=equipmentCardGroup, auditShelfId=auditShelfId, cardShelfTestIndex=cardShelfTestIndex, errEvRowStatus=errEvRowStatus, shelfSwVersion=shelfSwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentTraps=equipmentTraps, cardFwVersion=cardFwVersion, cardTestIndex=cardTestIndex, systemGeneralGroup=systemGeneralGroup, cardSubType=cardSubType, cardSpareGroupId=cardSpareGroupId, diskVolumeDiskSize=diskVolumeDiskSize, portCardSubType=portCardSubType, cardLastBootVersion=cardLastBootVersion, cardLastResetReason=cardLastResetReason, portTxFlowControlMode=portTxFlowControlMode, cardType=cardType, cardSwVersion=cardSwVersion, TestScheduleCommand=TestScheduleCommand, cardTable=cardTable, portAction=portAction, cardTestTime=cardTestTime, diagTestId=diagTestId, auditDescription=auditDescription, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeFileName=diskVolumeFileName, cardDplxStatus=cardDplxStatus, portDplxStatusChange=portDplxStatusChange, auditLogOutput=auditLogOutput, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification)
