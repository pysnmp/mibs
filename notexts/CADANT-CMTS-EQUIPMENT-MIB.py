...
DuplexStatus, FirmwareSource, PrimaryState, ShelfId, PicType, PortId, PortType, AdminState, PortMode, DiskVolumeUsageLevel, FlowControlMode, EqActionType, SecondaryState, PortDetectedMode, CardType, UnknownState, CardSubType, CardId = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "FirmwareSource", "PrimaryState", "ShelfId", "PicType", "PortId", "PortType", "AdminState", "PortMode", "DiskVolumeUsageLevel", "FlowControlMode", "EqActionType", "SecondaryState", "PortDetectedMode", "CardType", "UnknownState", "CardSubType", "CardId")
...
TextualConvention, TruthValue, TimeStamp, MacAddress, DateAndTime, RowStatus, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "TimeStamp", "MacAddress", "DateAndTime", "RowStatus", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskDriveNumber=diskDriveNumber, cmMacAddress=cmMacAddress, portTxFlowControlDetected=portTxFlowControlDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, cardNumber=cardNumber, cardTable=cardTable, cardTestType=cardTestType, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetClearNotification=cmResetClearNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditTotalFailedCount=auditTotalFailedCount, portRxFlowControlDetected=portRxFlowControlDetected, cardSpareGroupMode=cardSpareGroupMode, systemGeneral=systemGeneral, auditPassedCount=auditPassedCount, cardAdminState=cardAdminState, cmResetUpTime=cmResetUpTime, cardRamSize=cardRamSize, equipmentState=equipmentState, equipmentAudit=equipmentAudit, cardSecStateChange=cardSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestId=cardTestId, auditId=auditId, cmResetNotification=cmResetNotification, auditStatus=auditStatus, fpgaErrorEventTable=fpgaErrorEventTable, cardPicHwVersion=cardPicHwVersion, trapSeverity=trapSeverity, cardTestEntry=cardTestEntry, equipmentStateGroup=equipmentStateGroup, trapCounter=trapCounter, cardBootVersion=cardBootVersion, cardDetected=cardDetected, cardCarrierFwVersion=cardCarrierFwVersion, portEntry=portEntry, cardCPLDVersion=cardCPLDVersion, auditTime=auditTime, cardTestSchedule=cardTestSchedule, portDetectedMode=portDetectedMode, diskVolumeId=diskVolumeId, portMacIfIndex=portMacIfIndex, cardName=cardName, cardType=cardType, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfObjects=shelfObjects, portPrState=portPrState, diskVolumeFileName=diskVolumeFileName, equipmentTraps=equipmentTraps, portMacAddress=portMacAddress, cardTestResult=cardTestResult, portTable=portTable, eqDiagConfig=eqDiagConfig, diskVolumeUsageLevel=diskVolumeUsageLevel, cardFwUpdateStatus=cardFwUpdateStatus, portNumber=portNumber, cardFpgaSource=cardFpgaSource, cadEquipmentMibConformance=cadEquipmentMibConformance, auditTable=auditTable, cmResetInfo=cmResetInfo, cardCarrierHwVersion=cardCarrierHwVersion, equipmentError=equipmentError, portTxFlowControlMode=portTxFlowControlMode, verboseLevel=verboseLevel, shelfNumber=shelfNumber, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditDescription=auditDescription, TestResult=TestResult, diskVolumeCardId=diskVolumeCardId, cardTestTransId=cardTestTransId, errEvRowStatus=errEvRowStatus, diagTestId=diagTestId, cardPicSerialNum=cardPicSerialNum, diskVolumeTrapInh=diskVolumeTrapInh, cardEntry=cardEntry, cardDetectedChange=cardDetectedChange, cardBusSpeed=cardBusSpeed, portAdminState=portAdminState, shelfSwVersion=shelfSwVersion, cardTemperatureHighError=cardTemperatureHighError, TestSchedule=TestSchedule, cardCpuType=cardCpuType, portSecState=portSecState, cmResetStatus=cmResetStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogThrottle=auditLogThrottle, auditShelfId=auditShelfId, errEventId=errEventId, diskVolumeGroup=diskVolumeGroup, cardSubType=cardSubType, diskVolumeShelfId=diskVolumeShelfId, cardCarrierSerialNum=cardCarrierSerialNum, auditCommand=auditCommand, auditCycleCount=auditCycleCount, consoleOutput=consoleOutput, cmDevice=cmDevice, portCardSubType=portCardSubType, cardHwVersion=cardHwVersion, cardId=cardId, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portRxFlowControlMode=portRxFlowControlMode, cardShelfId=cardShelfId, cmVendor=cmVendor, portCurrDsPower=portCurrDsPower, portAction=portAction, cardSwVersion=cardSwVersion, portDocsIfIndex=portDocsIfIndex, portDplxStatusChange=portDplxStatusChange, portNumChans=portNumChans, diskVolumeDriveId=diskVolumeDriveId, errEvRecoveryEnabled=errEvRecoveryEnabled, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPicDetected=cardPicDetected, equipmentShelfGroup=equipmentShelfGroup, systemClock=systemClock, cardSecState=cardSecState, cardSpareGroupId=cardSpareGroupId, downstreamPowerLoss=downstreamPowerLoss, equipmentPortGroup=equipmentPortGroup, cardTestScheduleCommand=cardTestScheduleCommand, cardSerialNum=cardSerialNum, auditLogOutput=auditLogOutput, TestCommand=TestCommand, portDplxStatus=portDplxStatus, cardLastChangeTime=cardLastChangeTime, remainInDiagMode=remainInDiagMode, portShelfId=portShelfId, portId=portId, portMinDsPower=portMinDsPower, diskVolumeNumber=diskVolumeNumber, equipmentTbl=equipmentTbl, cardTemperature=cardTemperature, cardTestTime=cardTestTime, auditEntry=auditEntry, equipmentCardGroup=equipmentCardGroup, cardDplxStatusChange=cardDplxStatusChange, cardPrState=cardPrState, auditFailedCount=auditFailedCount, portLastChangeTime=portLastChangeTime, cmResetReason=cmResetReason, cardTestIndex=cardTestIndex, equipmentCardTestGroup=equipmentCardTestGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, portDescription=portDescription, cmUChannelID=cmUChannelID, cardUpTime=cardUpTime, portMaxDsPower=portMaxDsPower, cardNumPorts=cardNumPorts, TestType=TestType, portMode=portMode, systemKey=systemKey, cardDplxStatus=cardDplxStatus, cardShelfTestIndex=cardShelfTestIndex, diskVolumeEntry=diskVolumeEntry, cmIpAddress=cmIpAddress, diskVolumeDiskSize=diskVolumeDiskSize, shelfName=shelfName, diskVolumeSize=diskVolumeSize, auditCardId=auditCardId, equipmentAuditGroup=equipmentAuditGroup, cardLastBootVersion=cardLastBootVersion, cardTestDescription=cardTestDescription, auditResult=auditResult, cardTestTable=cardTestTable, portPrStateChange=portPrStateChange, cardFwVersion=cardFwVersion, cardLastBootSource=cardLastBootSource, portType=portType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResultDesc=cardTestResultDesc, cardLastResetReason=cardLastResetReason, portSecStateChange=portSecStateChange, cardTestResultNotification=cardTestResultNotification, portBgpId=portBgpId, auditName=auditName, errEvLogLevel=errEvLogLevel, cardTestCommand=cardTestCommand, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestTransactionId=TestTransactionId, equipmentDiag=equipmentDiag, cardAction=cardAction, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmRegistrationNotification=cmRegistrationNotification, TestScheduleCommand=TestScheduleCommand, cadEquipmentMib=cadEquipmentMib, diskVolumeTable=diskVolumeTable, portConnectorId=portConnectorId, diskVolumeName=diskVolumeName, cardTestName=cardTestName, cmPrimarySID=cmPrimarySID, systemGeneralGroup=systemGeneralGroup, equipmentDiagGroup=equipmentDiagGroup, TestId=TestId, cardCpuSpeed=cardCpuSpeed, cardFlashSize=cardFlashSize, cardPrStateChange=cardPrStateChange, portCardId=portCardId, cardSubDetected=cardSubDetected, portTrapInh=portTrapInh, cardTrapInh=cardTrapInh)
