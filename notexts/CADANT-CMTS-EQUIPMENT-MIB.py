...
DuplexStatus, AdminState, CardType, SecondaryState, PortMode, CardId, DiskVolumeUsageLevel, PrimaryState, FirmwareSource, PicType, PortType, FlowControlMode, UnknownState, ShelfId, PortId, EqActionType, CardSubType, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "AdminState", "CardType", "SecondaryState", "PortMode", "CardId", "DiskVolumeUsageLevel", "PrimaryState", "FirmwareSource", "PicType", "PortType", "FlowControlMode", "UnknownState", "ShelfId", "PortId", "EqActionType", "CardSubType", "PortDetectedMode")
...
TruthValue, TextualConvention, RowStatus, TimeStamp, DateAndTime, MacAddress, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TextualConvention", "RowStatus", "TimeStamp", "DateAndTime", "MacAddress", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestSchedule=TestSchedule, cardTestType=cardTestType, cardSecState=cardSecState, auditLogThrottle=auditLogThrottle, cardBusSpeed=cardBusSpeed, cardCarrierFwVersion=cardCarrierFwVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, diskDriveNumber=diskDriveNumber, portSecState=portSecState, errEvLoggingEnabled=errEvLoggingEnabled, portNumber=portNumber, fpgaErrorEventTable=fpgaErrorEventTable, cardDetectedChange=cardDetectedChange, cardName=cardName, cardTable=cardTable, portTable=portTable, cardTestName=cardTestName, errEvRowStatus=errEvRowStatus, cardSecStateChange=cardSecStateChange, auditCardId=auditCardId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portMinDsPower=portMinDsPower, portCardSubType=portCardSubType, portMacIfIndex=portMacIfIndex, cardAction=cardAction, cardPicSerialNum=cardPicSerialNum, cardTestId=cardTestId, cmPrimarySID=cmPrimarySID, cardLastBootSource=cardLastBootSource, diskVolumeNumber=diskVolumeNumber, cardTestTransId=cardTestTransId, portEntry=portEntry, errEventId=errEventId, auditFailedCount=auditFailedCount, cmDevice=cmDevice, errEvLogLevel=errEvLogLevel, trapCounter=trapCounter, auditAutoScheduling=auditAutoScheduling, cardSerialNum=cardSerialNum, cardRamSize=cardRamSize, cardTestCommand=cardTestCommand, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDocsIfIndex=portDocsIfIndex, cardTestSchedule=cardTestSchedule, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSubDetected=cardSubDetected, equipmentStateGroup=equipmentStateGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portCurrDsPower=portCurrDsPower, cardPrState=cardPrState, diskVolumeFileName=diskVolumeFileName, cardSpareGroupId=cardSpareGroupId, cardCpuType=cardCpuType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardPicHwVersion=cardPicHwVersion, cardSwVersion=cardSwVersion, auditShelfId=auditShelfId, cardTestEntry=cardTestEntry, cardHwVersion=cardHwVersion, diskVolumeTable=diskVolumeTable, cmIpAddress=cmIpAddress, portMode=portMode, auditTotalFailedCount=auditTotalFailedCount, verboseLevel=verboseLevel, remainInDiagMode=remainInDiagMode, portNumChans=portNumChans, cmResetClearNotification=cmResetClearNotification, cardBootVersion=cardBootVersion, cmUChannelID=cmUChannelID, equipmentTraps=equipmentTraps, cardCarrierHwVersion=cardCarrierHwVersion, cardNumPorts=cardNumPorts, cmResetStatus=cmResetStatus, diskVolumeDiskSize=diskVolumeDiskSize, portType=portType, portAction=portAction, cardTestResult=cardTestResult, cardPicDetected=cardPicDetected, auditTime=auditTime, TestId=TestId, portAdminState=portAdminState, diskVolumeCardId=diskVolumeCardId, diskVolumeSize=diskVolumeSize, systemGeneral=systemGeneral, diskVolumeShelfId=diskVolumeShelfId, TestType=TestType, cardType=cardType, equipmentShelfGroup=equipmentShelfGroup, cmMacAddress=cmMacAddress, portTxFlowControlMode=portTxFlowControlMode, diskVolumeDriveId=diskVolumeDriveId, cardFwVersion=cardFwVersion, cardDplxStatusChange=cardDplxStatusChange, portSecStateChange=portSecStateChange, shelfNumber=shelfNumber, TestScheduleCommand=TestScheduleCommand, auditCommand=auditCommand, equipmentDiag=equipmentDiag, cmResetNotification=cmResetNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeTrapInh=diskVolumeTrapInh, errEvRecoveryEnabled=errEvRecoveryEnabled, auditCycleCount=auditCycleCount, cardLastResetReason=cardLastResetReason, portRxFlowControlMode=portRxFlowControlMode, cardTemperature=cardTemperature, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, auditTable=auditTable, systemGeneralGroup=systemGeneralGroup, cmResetUpTime=cmResetUpTime, equipmentAuditGroup=equipmentAuditGroup, cardShelfTestIndex=cardShelfTestIndex, portDplxStatus=portDplxStatus, diskVolumeId=diskVolumeId, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentTbl=equipmentTbl, shelfName=shelfName, systemClock=systemClock, portDescription=portDescription, cardTempNoReportNotification=cardTempNoReportNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentError=equipmentError, TestCommand=TestCommand, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentPortGroup=equipmentPortGroup, cardShelfId=cardShelfId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, PYSNMP_MODULE_ID=cadEquipmentMib, cadEquipmentMibGroup=cadEquipmentMibGroup, portDetectedMode=portDetectedMode, equipmentAudit=equipmentAudit, equipmentCardGroup=equipmentCardGroup, cardCPLDVersion=cardCPLDVersion, cardPrStateChange=cardPrStateChange, portLastChangeTime=portLastChangeTime, cmVendor=cmVendor, auditResult=auditResult, cardTestDescription=cardTestDescription, portBgpId=portBgpId, equipmentCardTestGroup=equipmentCardTestGroup, consoleOutput=consoleOutput, diskVolumeEntry=diskVolumeEntry, cardLastBootVersion=cardLastBootVersion, cardDetected=cardDetected, cardEntry=cardEntry, shelfObjects=shelfObjects, auditTotalPassedCount=auditTotalPassedCount, portShelfId=portShelfId, cardTestIndex=cardTestIndex, cardCpuSpeed=cardCpuSpeed, cardFpgaSource=cardFpgaSource, trapSeverity=trapSeverity, auditLogOutput=auditLogOutput, auditId=auditId, cardFlashSize=cardFlashSize, cardUpTime=cardUpTime, systemKey=systemKey, portPrState=portPrState, cardDplxStatus=cardDplxStatus, TestResult=TestResult, auditDescription=auditDescription, cardTestResultDesc=cardTestResultDesc, cardTemperatureHighError=cardTemperatureHighError, cardTrapInh=cardTrapInh, cardCarrierSerialNum=cardCarrierSerialNum, cardId=cardId, cardAdminState=cardAdminState, cardFwUpdateStatus=cardFwUpdateStatus, eqDiagConfig=eqDiagConfig, portCardId=portCardId, equipmentDiagGroup=equipmentDiagGroup, auditEntry=auditEntry, portTxFlowControlDetected=portTxFlowControlDetected, shelfSwVersion=shelfSwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSpareGroupMode=cardSpareGroupMode, diskVolumeName=diskVolumeName, cardTestResultNotification=cardTestResultNotification, diskVolumeGroup=diskVolumeGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, portMacAddress=portMacAddress, auditPassedCount=auditPassedCount, cmResetInfo=cmResetInfo, portDplxStatusChange=portDplxStatusChange, auditStatus=auditStatus, TestTransactionId=TestTransactionId, portTrapInh=portTrapInh, cardTestScheduleCommand=cardTestScheduleCommand, cmResetReason=cmResetReason, cardTempOverHeatNotification=cardTempOverHeatNotification, portConnectorId=portConnectorId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSubType=cardSubType, equipmentState=equipmentState, portPrStateChange=portPrStateChange, diagTestId=diagTestId, auditName=auditName, cadEquipmentMib=cadEquipmentMib, cardTestTime=cardTestTime, portId=portId, cardNumber=cardNumber, portMaxDsPower=portMaxDsPower, cardTestTable=cardTestTable)
