...
PrimaryState, ShelfId, DiskVolumeUsageLevel, CardType, DuplexStatus, PicType, PortType, CardSubType, UnknownState, PortDetectedMode, SecondaryState, PortId, EqActionType, AdminState, FlowControlMode, PortMode, CardId, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "ShelfId", "DiskVolumeUsageLevel", "CardType", "DuplexStatus", "PicType", "PortType", "CardSubType", "UnknownState", "PortDetectedMode", "SecondaryState", "PortId", "EqActionType", "AdminState", "FlowControlMode", "PortMode", "CardId", "FirmwareSource")
...
TextualConvention, DisplayString, TruthValue, TimeStamp, RowStatus, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "TruthValue", "TimeStamp", "RowStatus", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSubType=cardSubType, portTxFlowControlDetected=portTxFlowControlDetected, cardDplxStatus=cardDplxStatus, auditTime=auditTime, cmPrimarySID=cmPrimarySID, auditDescription=auditDescription, cadEquipmentMibCompliance=cadEquipmentMibCompliance, trapCounter=trapCounter, cardTestTable=cardTestTable, equipmentTraps=equipmentTraps, diskVolumeName=diskVolumeName, cardNumber=cardNumber, cardSpareGroupId=cardSpareGroupId, cardTestTransId=cardTestTransId, cardDetectedChange=cardDetectedChange, cardTestResultNotification=cardTestResultNotification, cardAction=cardAction, cardId=cardId, diskVolumeSize=diskVolumeSize, equipmentCardTestGroup=equipmentCardTestGroup, cmResetUpTime=cmResetUpTime, TestId=TestId, portCardSubType=portCardSubType, cardTestTime=cardTestTime, cardEntry=cardEntry, shelfNumber=shelfNumber, cardSerialNum=cardSerialNum, portRxFlowControlMode=portRxFlowControlMode, equipmentAuditGroup=equipmentAuditGroup, cmRegistrationNotification=cmRegistrationNotification, portNumChans=portNumChans, diskVolumeDriveId=diskVolumeDriveId, diskVolumeId=diskVolumeId, equipmentShelfGroup=equipmentShelfGroup, cardLastBootSource=cardLastBootSource, portType=portType, portDescription=portDescription, shelfSwVersion=shelfSwVersion, auditAutoScheduling=auditAutoScheduling, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMib=cadEquipmentMib, equipmentTbl=equipmentTbl, portTrapInh=portTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCarrierHwVersion=cardCarrierHwVersion, cmDevice=cmDevice, TestScheduleCommand=TestScheduleCommand, portCardId=portCardId, auditLogOutput=auditLogOutput, auditCommand=auditCommand, equipmentState=equipmentState, diskVolumeTrapInh=diskVolumeTrapInh, cardBusSpeed=cardBusSpeed, diskVolumeUsageNotification=diskVolumeUsageNotification, errEvRowStatus=errEvRowStatus, cardTestType=cardTestType, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portMinDsPower=portMinDsPower, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portDplxStatus=portDplxStatus, cardFlashSize=cardFlashSize, cardTestSchedule=cardTestSchedule, auditResult=auditResult, cmUChannelID=cmUChannelID, cardTable=cardTable, cardPrStateChange=cardPrStateChange, portDetectedMode=portDetectedMode, equipmentPortGroup=equipmentPortGroup, portPrStateChange=portPrStateChange, equipmentDiagGroup=equipmentDiagGroup, TestType=TestType, auditTotalFailedCount=auditTotalFailedCount, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentCardGroup=equipmentCardGroup, cardCpuSpeed=cardCpuSpeed, portAction=portAction, portNumber=portNumber, cardTestEntry=cardTestEntry, portDocsIfIndex=portDocsIfIndex, cardTestIndex=cardTestIndex, cardCPLDVersion=cardCPLDVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, auditEntry=auditEntry, consoleOutput=consoleOutput, cardName=cardName, cardTrapInh=cardTrapInh, auditName=auditName, systemGeneral=systemGeneral, cardShelfId=cardShelfId, cardHwVersion=cardHwVersion, eqDiagConfig=eqDiagConfig, cardLastChangeTime=cardLastChangeTime, cmResetInfo=cmResetInfo, cardSecStateChange=cardSecStateChange, cardPicSerialNum=cardPicSerialNum, diskVolumeGroup=diskVolumeGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmMacAddress=cmMacAddress, cardAdminState=cardAdminState, cmResetStatus=cmResetStatus, portLastChangeTime=portLastChangeTime, auditPassedCount=auditPassedCount, auditShelfId=auditShelfId, portConnectorId=portConnectorId, remainInDiagMode=remainInDiagMode, downstreamPowerLoss=downstreamPowerLoss, portMode=portMode, cardTestCommand=cardTestCommand, diskVolumeTable=diskVolumeTable, cmResetNotification=cmResetNotification, TestSchedule=TestSchedule, cardFwVersion=cardFwVersion, cardShelfTestIndex=cardShelfTestIndex, cardPicDetected=cardPicDetected, cardTestDescription=cardTestDescription, diskDriveNumber=diskDriveNumber, cardCarrierSerialNum=cardCarrierSerialNum, auditFailedCount=auditFailedCount, portCurrDsPower=portCurrDsPower, systemKey=systemKey, cadEquipmentMibConformance=cadEquipmentMibConformance, portId=portId, errEvLoggingEnabled=errEvLoggingEnabled, cardDplxStatusChange=cardDplxStatusChange, diskVolumeCardId=diskVolumeCardId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestId=cardTestId, portSecStateChange=portSecStateChange, cardCpuType=cardCpuType, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetReason=cmResetReason, cardTempNoReportNotification=cardTempNoReportNotification, cardSpareGroupMode=cardSpareGroupMode, equipmentError=equipmentError, cardUpTime=cardUpTime, portRxFlowControlDetected=portRxFlowControlDetected, cardTestName=cardTestName, cardFwUpdateStatus=cardFwUpdateStatus, portAdminState=portAdminState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestCommand=TestCommand, portEntry=portEntry, diskVolumeFileName=diskVolumeFileName, TestResult=TestResult, cardTemperature=cardTemperature, equipmentStateGroup=equipmentStateGroup, cardType=cardType, auditStatus=auditStatus, verboseLevel=verboseLevel, cardFpgaSource=cardFpgaSource, trapSeverity=trapSeverity, portMacIfIndex=portMacIfIndex, cardCarrierFwVersion=cardCarrierFwVersion, cardPicHwVersion=cardPicHwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, cardRamSize=cardRamSize, fpgaErrorEventTable=fpgaErrorEventTable, cardLastBootVersion=cardLastBootVersion, diskVolumeShelfId=diskVolumeShelfId, portShelfId=portShelfId, cardSubDetected=cardSubDetected, errEventId=errEventId, auditTotalPassedCount=auditTotalPassedCount, systemClock=systemClock, cardTestResultDesc=cardTestResultDesc, auditCycleCount=auditCycleCount, cardSecState=cardSecState, cardTestScheduleCommand=cardTestScheduleCommand, diagTestId=diagTestId, auditLogThrottle=auditLogThrottle, cmResetClearNotification=cmResetClearNotification, systemGeneralGroup=systemGeneralGroup, cmVendor=cmVendor, auditId=auditId, shelfObjects=shelfObjects, equipmentAudit=equipmentAudit, cardNumPorts=cardNumPorts, auditTable=auditTable, portMacAddress=portMacAddress, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTemperatureHighError=cardTemperatureHighError, errEvLogLevel=errEvLogLevel, fpgaErrorEventEntry=fpgaErrorEventEntry, auditCardId=auditCardId, cmIpAddress=cmIpAddress, diskVolumeNumber=diskVolumeNumber, cardBootVersion=cardBootVersion, diskVolumeEntry=diskVolumeEntry, portTable=portTable, shelfName=shelfName, cardSwVersion=cardSwVersion, cardTestResult=cardTestResult, portMaxDsPower=portMaxDsPower, cardLastResetReason=cardLastResetReason, TestTransactionId=TestTransactionId, portSecState=portSecState, equipmentDiag=equipmentDiag, portPrState=portPrState, portDplxStatusChange=portDplxStatusChange, portBgpId=portBgpId, cardPrState=cardPrState, cardDetected=cardDetected)
