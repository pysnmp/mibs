...
DuplexStatus, DiskVolumeUsageLevel, CardId, FlowControlMode, PicType, CardSubType, PortDetectedMode, PortId, SecondaryState, ShelfId, CardType, EqActionType, PrimaryState, UnknownState, FirmwareSource, PortType, PortMode, AdminState = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "DiskVolumeUsageLevel", "CardId", "FlowControlMode", "PicType", "CardSubType", "PortDetectedMode", "PortId", "SecondaryState", "ShelfId", "CardType", "EqActionType", "PrimaryState", "UnknownState", "FirmwareSource", "PortType", "PortMode", "AdminState")
...
MacAddress, TextualConvention, DateAndTime, TruthValue, TimeStamp, RowStatus, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TextualConvention", "DateAndTime", "TruthValue", "TimeStamp", "RowStatus", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestEntry=cardTestEntry, cardSpareGroupMode=cardSpareGroupMode, portCardSubType=portCardSubType, diskVolumeShelfId=diskVolumeShelfId, cmPrimarySID=cmPrimarySID, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetNotification=cmResetNotification, cardCpuSpeed=cardCpuSpeed, portPrState=portPrState, cmVendor=cmVendor, errEvLoggingEnabled=errEvLoggingEnabled, equipmentState=equipmentState, diskVolumeGroup=diskVolumeGroup, auditCommand=auditCommand, cadEquipmentMibGroup=cadEquipmentMibGroup, errEvLogLevel=errEvLogLevel, downstreamPowerLoss=downstreamPowerLoss, portCurrDsPower=portCurrDsPower, diskVolumeNumber=diskVolumeNumber, cardTestDescription=cardTestDescription, remainInDiagMode=remainInDiagMode, auditId=auditId, cardTemperatureHighError=cardTemperatureHighError, cardTestTable=cardTestTable, TestResult=TestResult, cardTemperatureHighWarn=cardTemperatureHighWarn, auditLogOutput=auditLogOutput, portEntry=portEntry, cmDevice=cmDevice, cardLastBootSource=cardLastBootSource, equipmentError=equipmentError, diskVolumeFileName=diskVolumeFileName, cardDetected=cardDetected, equipmentDiagGroup=equipmentDiagGroup, errEvRowStatus=errEvRowStatus, diskVolumeTrapInh=diskVolumeTrapInh, auditDescription=auditDescription, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestSchedule=cardTestSchedule, auditPassedCount=auditPassedCount, portCardId=portCardId, consoleOutput=consoleOutput, cardTestResult=cardTestResult, cardRamSize=cardRamSize, portAdminState=portAdminState, TestCommand=TestCommand, auditStatus=auditStatus, cardUpTime=cardUpTime, auditResult=auditResult, systemGeneralGroup=systemGeneralGroup, cardName=cardName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portType=portType, TestId=TestId, cardId=cardId, portShelfId=portShelfId, portNumber=portNumber, cardTestTransId=cardTestTransId, errEventId=errEventId, cardDetectedChange=cardDetectedChange, portMacIfIndex=portMacIfIndex, cardTable=cardTable, cardFwUpdateStatus=cardFwUpdateStatus, cardSubDetected=cardSubDetected, portDetectedMode=portDetectedMode, portMode=portMode, auditFailedCount=auditFailedCount, TestScheduleCommand=TestScheduleCommand, cardTestCommand=cardTestCommand, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSerialNum=cardSerialNum, cardFpgaSource=cardFpgaSource, cardPicDetected=cardPicDetected, diskVolumeTable=diskVolumeTable, equipmentDiag=equipmentDiag, auditCycleCount=auditCycleCount, portPrStateChange=portPrStateChange, diskVolumeEntry=diskVolumeEntry, portDescription=portDescription, diskVolumeName=diskVolumeName, fpgaErrorEventEntry=fpgaErrorEventEntry, TestTransactionId=TestTransactionId, auditAutoScheduling=auditAutoScheduling, cardTempOverHeatNotification=cardTempOverHeatNotification, cardShelfTestIndex=cardShelfTestIndex, cardCPLDVersion=cardCPLDVersion, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPrState=cardPrState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeDiskSize=diskVolumeDiskSize, cardPrStateChange=cardPrStateChange, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventTable=fpgaErrorEventTable, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMib=cadEquipmentMib, equipmentTraps=equipmentTraps, portDplxStatusChange=portDplxStatusChange, portSecStateChange=portSecStateChange, cardSubType=cardSubType, cardFwVersion=cardFwVersion, auditTime=auditTime, eqDiagConfig=eqDiagConfig, cmMacAddress=cmMacAddress, portMaxDsPower=portMaxDsPower, cmIpAddress=cmIpAddress, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCarrierHwVersion=cardCarrierHwVersion, diagTestId=diagTestId, portTxFlowControlMode=portTxFlowControlMode, cardTestResultDesc=cardTestResultDesc, portNumChans=portNumChans, equipmentPortGroup=equipmentPortGroup, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardBusSpeed=cardBusSpeed, cardFlashSize=cardFlashSize, cardSecState=cardSecState, cardAction=cardAction, portBgpId=portBgpId, cardTempNoReportNotification=cardTempNoReportNotification, cardTestTime=cardTestTime, auditTotalPassedCount=auditTotalPassedCount, cardTemperature=cardTemperature, portConnectorId=portConnectorId, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestResultNotification=cardTestResultNotification, cmResetReason=cmResetReason, TestType=TestType, diskVolumeSize=diskVolumeSize, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, trapSeverity=trapSeverity, cardPicSerialNum=cardPicSerialNum, shelfObjects=shelfObjects, cardSwVersion=cardSwVersion, cardAdminState=cardAdminState, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeDriveId=diskVolumeDriveId, cardType=cardType, portTable=portTable, auditName=auditName, cardLastBootVersion=cardLastBootVersion, portDplxStatus=portDplxStatus, auditShelfId=auditShelfId, equipmentStateGroup=equipmentStateGroup, cardHwVersion=cardHwVersion, cardTestId=cardTestId, cardShelfId=cardShelfId, portSecState=portSecState, auditCardId=auditCardId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentTbl=equipmentTbl, cardTestType=cardTestType, shelfName=shelfName, cardTestScheduleCommand=cardTestScheduleCommand, cmResetClearNotification=cmResetClearNotification, equipmentCardGroup=equipmentCardGroup, cardCpuType=cardCpuType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmUChannelID=cmUChannelID, portMinDsPower=portMinDsPower, cardDplxStatusChange=cardDplxStatusChange, fpgaErrorEventGroup=fpgaErrorEventGroup, cardDplxStatus=cardDplxStatus, shelfNumber=shelfNumber, diskVolumeId=diskVolumeId, systemGeneral=systemGeneral, cardNumPorts=cardNumPorts, portMacAddress=portMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, equipmentAudit=equipmentAudit, cardSecStateChange=cardSecStateChange, portId=portId, portAction=portAction, diskDriveNumber=diskDriveNumber, cmResetInfo=cmResetInfo, cmRegistrationNotification=cmRegistrationNotification, auditTable=auditTable, cardTrapInh=cardTrapInh, cardTestName=cardTestName, cardNumber=cardNumber, portLastChangeTime=portLastChangeTime, shelfSwVersion=shelfSwVersion, auditLogThrottle=auditLogThrottle, diskVolumeCardId=diskVolumeCardId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardBootVersion=cardBootVersion, cardTestIndex=cardTestIndex, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetUpTime=cmResetUpTime, cardLastChangeTime=cardLastChangeTime, trapCounter=trapCounter, portTrapInh=portTrapInh, systemKey=systemKey, cardPicHwVersion=cardPicHwVersion, portDocsIfIndex=portDocsIfIndex, cardEntry=cardEntry, cardLastResetReason=cardLastResetReason, auditEntry=auditEntry, cmResetStatus=cmResetStatus, verboseLevel=verboseLevel, TestSchedule=TestSchedule, systemClock=systemClock, cardCarrierFwVersion=cardCarrierFwVersion, portRxFlowControlMode=portRxFlowControlMode)
