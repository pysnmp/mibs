...
CardId, PortDetectedMode, CardType, FlowControlMode, DuplexStatus, SecondaryState, PortId, EqActionType, PicType, ShelfId, PrimaryState, FirmwareSource, UnknownState, CardSubType, DiskVolumeUsageLevel, PortType, PortMode, AdminState = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortDetectedMode", "CardType", "FlowControlMode", "DuplexStatus", "SecondaryState", "PortId", "EqActionType", "PicType", "ShelfId", "PrimaryState", "FirmwareSource", "UnknownState", "CardSubType", "DiskVolumeUsageLevel", "PortType", "PortMode", "AdminState")
...
TextualConvention, TruthValue, RowStatus, DisplayString, TimeStamp, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "RowStatus", "DisplayString", "TimeStamp", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portMinDsPower=portMinDsPower, equipmentAuditGroup=equipmentAuditGroup, portCardId=portCardId, cardSecState=cardSecState, equipmentAudit=equipmentAudit, auditCycleCount=auditCycleCount, auditTotalPassedCount=auditTotalPassedCount, systemGeneralGroup=systemGeneralGroup, PYSNMP_MODULE_ID=cadEquipmentMib, cardCarrierSerialNum=cardCarrierSerialNum, cardRamSize=cardRamSize, portMaxDsPower=portMaxDsPower, cardTestName=cardTestName, auditCardId=auditCardId, cardTestId=cardTestId, diskVolumeGroup=diskVolumeGroup, diskVolumeName=diskVolumeName, portTxFlowControlMode=portTxFlowControlMode, diskVolumeDiskSize=diskVolumeDiskSize, auditLogOutput=auditLogOutput, auditName=auditName, portDescription=portDescription, trapCounter=trapCounter, cardBootVersion=cardBootVersion, TestScheduleCommand=TestScheduleCommand, shelfSwVersion=shelfSwVersion, portSecStateChange=portSecStateChange, equipmentShelfGroup=equipmentShelfGroup, cardTestEntry=cardTestEntry, auditTable=auditTable, cardTestCommand=cardTestCommand, diskVolumeNumber=diskVolumeNumber, portLastChangeTime=portLastChangeTime, TestId=TestId, portNumber=portNumber, cmResetUpTime=cmResetUpTime, cardDetected=cardDetected, auditPassedCount=auditPassedCount, shelfName=shelfName, portDocsIfIndex=portDocsIfIndex, shelfNumber=shelfNumber, portType=portType, diskVolumeSize=diskVolumeSize, portAction=portAction, cardDplxStatus=cardDplxStatus, shelfObjects=shelfObjects, cmResetInfo=cmResetInfo, equipmentState=equipmentState, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibConformance=cadEquipmentMibConformance, downstreamPowerLoss=downstreamPowerLoss, systemClock=systemClock, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portMacAddress=portMacAddress, portNumChans=portNumChans, cardId=cardId, portTxFlowControlDetected=portTxFlowControlDetected, cardFpgaSource=cardFpgaSource, auditId=auditId, portDplxStatusChange=portDplxStatusChange, cardDetectedChange=cardDetectedChange, cmRegistrationNotification=cmRegistrationNotification, equipmentPortGroup=equipmentPortGroup, portEntry=portEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestScheduleCommand=cardTestScheduleCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, cmUChannelID=cmUChannelID, errEvLogLevel=errEvLogLevel, cardType=cardType, cardTable=cardTable, cardTestIndex=cardTestIndex, cardSwVersion=cardSwVersion, diskVolumeDriveId=diskVolumeDriveId, cardLastBootVersion=cardLastBootVersion, cardHwVersion=cardHwVersion, portMode=portMode, cardPicSerialNum=cardPicSerialNum, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentCardGroup=equipmentCardGroup, errEvLoggingEnabled=errEvLoggingEnabled, remainInDiagMode=remainInDiagMode, TestType=TestType, equipmentTbl=equipmentTbl, cardName=cardName, cardTestType=cardTestType, errEvRecoveryEnabled=errEvRecoveryEnabled, cardShelfId=cardShelfId, cardSerialNum=cardSerialNum, cardTestResultDesc=cardTestResultDesc, cardCpuType=cardCpuType, auditTotalFailedCount=auditTotalFailedCount, cardSubDetected=cardSubDetected, portMacIfIndex=portMacIfIndex, diskVolumeCardId=diskVolumeCardId, auditLogThrottle=auditLogThrottle, cmPrimarySID=cmPrimarySID, portBgpId=portBgpId, portRxFlowControlDetected=portRxFlowControlDetected, cmResetClearNotification=cmResetClearNotification, auditTime=auditTime, cmVendor=cmVendor, portConnectorId=portConnectorId, cardTestDescription=cardTestDescription, diskVolumeFileName=diskVolumeFileName, cmMacAddress=cmMacAddress, portRxFlowControlMode=portRxFlowControlMode, cardSpareGroupId=cardSpareGroupId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskDriveNumber=diskDriveNumber, cardTemperatureHighError=cardTemperatureHighError, equipmentTraps=equipmentTraps, portDetectedMode=portDetectedMode, equipmentCardTestGroup=equipmentCardTestGroup, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentDiag=equipmentDiag, fpgaErrorEventTable=fpgaErrorEventTable, cardNumber=cardNumber, cardTemperature=cardTemperature, cardPrStateChange=cardPrStateChange, cardLastResetReason=cardLastResetReason, cardPicHwVersion=cardPicHwVersion, systemKey=systemKey, diskVolumeUsageNotification=diskVolumeUsageNotification, errEventId=errEventId, portShelfId=portShelfId, cardCPLDVersion=cardCPLDVersion, diskVolumeTable=diskVolumeTable, cardTestResult=cardTestResult, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portPrStateChange=portPrStateChange, cardTestResultNotification=cardTestResultNotification, cmResetNotification=cmResetNotification, cardEntry=cardEntry, eqDiagConfig=eqDiagConfig, portTrapInh=portTrapInh, cardShelfTestIndex=cardShelfTestIndex, cardFwVersion=cardFwVersion, cardTrapInh=cardTrapInh, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmIpAddress=cmIpAddress, errEvRowStatus=errEvRowStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, cardNumPorts=cardNumPorts, cardLastChangeTime=cardLastChangeTime, cardBusSpeed=cardBusSpeed, diskVolumeTrapInh=diskVolumeTrapInh, cardTestSchedule=cardTestSchedule, cardLastBootSource=cardLastBootSource, diskVolumeEntry=diskVolumeEntry, cardTempNoReportNotification=cardTempNoReportNotification, cardPrState=cardPrState, cardCarrierHwVersion=cardCarrierHwVersion, auditDescription=auditDescription, cardSubType=cardSubType, cardTestTable=cardTestTable, cmResetStatus=cmResetStatus, portSecState=portSecState, auditFailedCount=auditFailedCount, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardDplxStatusChange=cardDplxStatusChange, TestTransactionId=TestTransactionId, portAdminState=portAdminState, cardCarrierFwVersion=cardCarrierFwVersion, systemGeneral=systemGeneral, fpgaErrorEventGroup=fpgaErrorEventGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, consoleOutput=consoleOutput, cmResetReason=cmResetReason, auditEntry=auditEntry, equipmentStateGroup=equipmentStateGroup, portDplxStatus=portDplxStatus, portCurrDsPower=portCurrDsPower, cardTestTransId=cardTestTransId, diskVolumeShelfId=diskVolumeShelfId, cardTemperatureHighWarn=cardTemperatureHighWarn, portId=portId, diskVolumeUsageLevel=diskVolumeUsageLevel, TestSchedule=TestSchedule, cardAction=cardAction, verboseLevel=verboseLevel, cardTestTime=cardTestTime, TestResult=TestResult, portCardSubType=portCardSubType, auditCommand=auditCommand, cardSecStateChange=cardSecStateChange, auditStatus=auditStatus, cardAdminState=cardAdminState, diagTestId=diagTestId, TestCommand=TestCommand, diskVolumeId=diskVolumeId, cardCpuSpeed=cardCpuSpeed, auditResult=auditResult, equipmentDiagGroup=equipmentDiagGroup, portTable=portTable, trapSeverity=trapSeverity, auditShelfId=auditShelfId, cmDevice=cmDevice, cardFlashSize=cardFlashSize, cadEquipmentMib=cadEquipmentMib, cardUpTime=cardUpTime, portPrState=portPrState, equipmentError=equipmentError, cardSpareGroupMode=cardSpareGroupMode, cardPicDetected=cardPicDetected)
