...
AdminState, FirmwareSource, PicType, PortType, SecondaryState, UnknownState, DiskVolumeUsageLevel, FlowControlMode, CardSubType, PortMode, DuplexStatus, PortDetectedMode, ShelfId, PortId, CardType, PrimaryState, EqActionType, CardId = mibBuilder.importSymbols("CADANT-TC", "AdminState", "FirmwareSource", "PicType", "PortType", "SecondaryState", "UnknownState", "DiskVolumeUsageLevel", "FlowControlMode", "CardSubType", "PortMode", "DuplexStatus", "PortDetectedMode", "ShelfId", "PortId", "CardType", "PrimaryState", "EqActionType", "CardId")
...
TextualConvention, DateAndTime, MacAddress, RowStatus, DisplayString, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "MacAddress", "RowStatus", "DisplayString", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSubDetected=cardSubDetected, equipmentTbl=equipmentTbl, cardDetected=cardDetected, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetClearNotification=cmResetClearNotification, diskVolumeDiskSize=diskVolumeDiskSize, diskDriveNumber=diskDriveNumber, cardType=cardType, auditShelfId=auditShelfId, portPrStateChange=portPrStateChange, equipmentState=equipmentState, cardShelfId=cardShelfId, auditId=auditId, fpgaErrorEventTable=fpgaErrorEventTable, cardSerialNum=cardSerialNum, cardLastChangeTime=cardLastChangeTime, cardSwVersion=cardSwVersion, errEvLogLevel=errEvLogLevel, portSecState=portSecState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSecStateChange=cardSecStateChange, cardCarrierHwVersion=cardCarrierHwVersion, equipmentAuditGroup=equipmentAuditGroup, trapCounter=trapCounter, portShelfId=portShelfId, cardHwVersion=cardHwVersion, portTxFlowControlDetected=portTxFlowControlDetected, remainInDiagMode=remainInDiagMode, TestScheduleCommand=TestScheduleCommand, equipmentDiag=equipmentDiag, cardDplxStatus=cardDplxStatus, cardCPLDVersion=cardCPLDVersion, cardSpareGroupMode=cardSpareGroupMode, portType=portType, diskVolumeTrapInh=diskVolumeTrapInh, shelfName=shelfName, diskVolumeName=diskVolumeName, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, TestResult=TestResult, portMode=portMode, cardTestResultDesc=cardTestResultDesc, auditAutoScheduling=auditAutoScheduling, auditEntry=auditEntry, cardTrapInh=cardTrapInh, cmResetNotification=cmResetNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portId=portId, portConnectorId=portConnectorId, cardTestSchedule=cardTestSchedule, auditResult=auditResult, diskVolumeSize=diskVolumeSize, errEventId=errEventId, cardLastBootSource=cardLastBootSource, cmResetStatus=cmResetStatus, equipmentCardGroup=equipmentCardGroup, portSecStateChange=portSecStateChange, diskVolumeGroup=diskVolumeGroup, cmDevice=cmDevice, shelfObjects=shelfObjects, cardTestIndex=cardTestIndex, cardTemperature=cardTemperature, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeNumber=diskVolumeNumber, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditLogOutput=auditLogOutput, cmVendor=cmVendor, portPrState=portPrState, cardTestTime=cardTestTime, auditFailedCount=auditFailedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, cardAdminState=cardAdminState, equipmentDiagGroup=equipmentDiagGroup, cardSecState=cardSecState, TestSchedule=TestSchedule, cmIpAddress=cmIpAddress, cardTemperatureHighWarn=cardTemperatureHighWarn, diagTestId=diagTestId, portMacIfIndex=portMacIfIndex, TestTransactionId=TestTransactionId, cardTestScheduleCommand=cardTestScheduleCommand, portAction=portAction, cardRamSize=cardRamSize, cardFwUpdateStatus=cardFwUpdateStatus, auditTotalFailedCount=auditTotalFailedCount, cardLastBootVersion=cardLastBootVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portMacAddress=portMacAddress, portEntry=portEntry, diskVolumeDriveId=diskVolumeDriveId, cardBusSpeed=cardBusSpeed, cardTestEntry=cardTestEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, auditCardId=auditCardId, portDplxStatus=portDplxStatus, diskVolumeEntry=diskVolumeEntry, cardTestName=cardTestName, portNumChans=portNumChans, cardPicSerialNum=cardPicSerialNum, cmPrimarySID=cmPrimarySID, portNumber=portNumber, cardTempOverHeatNotification=cardTempOverHeatNotification, cardFwVersion=cardFwVersion, portDetectedMode=portDetectedMode, cardTable=cardTable, cardPicDetected=cardPicDetected, cardCpuType=cardCpuType, systemClock=systemClock, cardUpTime=cardUpTime, portAdminState=portAdminState, auditStatus=auditStatus, equipmentStateGroup=equipmentStateGroup, equipmentError=equipmentError, cardBootVersion=cardBootVersion, portTxFlowControlMode=portTxFlowControlMode, auditLogThrottle=auditLogThrottle, cardTestId=cardTestId, cardId=cardId, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentCardTestGroup=equipmentCardTestGroup, cardCarrierSerialNum=cardCarrierSerialNum, portTable=portTable, systemGeneralGroup=systemGeneralGroup, cardName=cardName, cmMacAddress=cmMacAddress, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portRxFlowControlDetected=portRxFlowControlDetected, TestType=TestType, systemGeneral=systemGeneral, auditName=auditName, cardCarrierFwVersion=cardCarrierFwVersion, cardDplxStatusChange=cardDplxStatusChange, cardSubType=cardSubType, portMinDsPower=portMinDsPower, portCurrDsPower=portCurrDsPower, auditPassedCount=auditPassedCount, cardTestDescription=cardTestDescription, cardNumPorts=cardNumPorts, errEvRowStatus=errEvRowStatus, portRxFlowControlMode=portRxFlowControlMode, auditTable=auditTable, cardPrStateChange=cardPrStateChange, cmUChannelID=cmUChannelID, cadEquipmentMib=cadEquipmentMib, portCardId=portCardId, cardTempNoReportNotification=cardTempNoReportNotification, portTrapInh=portTrapInh, portDescription=portDescription, cardTestResult=cardTestResult, auditTotalPassedCount=auditTotalPassedCount, cmResetInfo=cmResetInfo, cmResetUpTime=cmResetUpTime, cardEntry=cardEntry, cardTemperatureHighError=cardTemperatureHighError, eqDiagConfig=eqDiagConfig, cardLastResetReason=cardLastResetReason, auditCycleCount=auditCycleCount, cardTestCommand=cardTestCommand, cardTestType=cardTestType, cardTestTable=cardTestTable, equipmentTraps=equipmentTraps, portMaxDsPower=portMaxDsPower, errEvLoggingEnabled=errEvLoggingEnabled, auditCommand=auditCommand, cadEquipmentMibGroup=cadEquipmentMibGroup, portDplxStatusChange=portDplxStatusChange, equipmentShelfGroup=equipmentShelfGroup, cardPicHwVersion=cardPicHwVersion, shelfSwVersion=shelfSwVersion, cardDetectedChange=cardDetectedChange, shelfNumber=shelfNumber, portDocsIfIndex=portDocsIfIndex, diskVolumeShelfId=diskVolumeShelfId, TestId=TestId, cardShelfTestIndex=cardShelfTestIndex, cardAction=cardAction, auditDescription=auditDescription, TestCommand=TestCommand, portBgpId=portBgpId, diskVolumeFileName=diskVolumeFileName, systemKey=systemKey, cmResetReason=cmResetReason, auditTime=auditTime, cardFpgaSource=cardFpgaSource, cardNumber=cardNumber, portLastChangeTime=portLastChangeTime, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, downstreamPowerLoss=downstreamPowerLoss, portCardSubType=portCardSubType, consoleOutput=consoleOutput, diskVolumeCardId=diskVolumeCardId, diskVolumeTable=diskVolumeTable, equipmentAudit=equipmentAudit, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestTransId=cardTestTransId, trapSeverity=trapSeverity, cardSpareGroupId=cardSpareGroupId, cardFlashSize=cardFlashSize, equipmentPortGroup=equipmentPortGroup, cardCpuSpeed=cardCpuSpeed, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPrState=cardPrState, verboseLevel=verboseLevel, diskVolumeId=diskVolumeId)
