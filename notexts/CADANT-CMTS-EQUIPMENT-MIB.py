...
AdminState, PrimaryState, UnknownState, CardId, CardSubType, DuplexStatus, FlowControlMode, DiskVolumeUsageLevel, SecondaryState, CardType, PortMode, FirmwareSource, PortDetectedMode, ShelfId, EqActionType, PicType, PortType, PortId = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PrimaryState", "UnknownState", "CardId", "CardSubType", "DuplexStatus", "FlowControlMode", "DiskVolumeUsageLevel", "SecondaryState", "CardType", "PortMode", "FirmwareSource", "PortDetectedMode", "ShelfId", "EqActionType", "PicType", "PortType", "PortId")
...
TextualConvention, DateAndTime, DisplayString, TimeStamp, RowStatus, MacAddress, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "DisplayString", "TimeStamp", "RowStatus", "MacAddress", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCpuType=cardCpuType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portRxFlowControlDetected=portRxFlowControlDetected, cardFwUpdateStatus=cardFwUpdateStatus, portPrState=portPrState, diskVolumeUsageLevel=diskVolumeUsageLevel, auditTime=auditTime, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestName=cardTestName, portId=portId, portSecState=portSecState, cardTestId=cardTestId, portTxFlowControlMode=portTxFlowControlMode, fpgaErrorEventTable=fpgaErrorEventTable, diskDriveNumber=diskDriveNumber, cmResetNotification=cmResetNotification, equipmentState=equipmentState, auditTotalFailedCount=auditTotalFailedCount, diskVolumeSize=diskVolumeSize, cmUChannelID=cmUChannelID, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestIndex=cardTestIndex, diskVolumeNumber=diskVolumeNumber, cardName=cardName, portAdminState=portAdminState, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portTrapInh=portTrapInh, diskVolumeTable=diskVolumeTable, portDocsIfIndex=portDocsIfIndex, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentTbl=equipmentTbl, TestSchedule=TestSchedule, portCurrDsPower=portCurrDsPower, portEntry=portEntry, portDescription=portDescription, fpgaErrorEventGroup=fpgaErrorEventGroup, portMacIfIndex=portMacIfIndex, cardSwVersion=cardSwVersion, auditId=auditId, cmResetStatus=cmResetStatus, auditCommand=auditCommand, cardTestEntry=cardTestEntry, cardTestCommand=cardTestCommand, portTable=portTable, remainInDiagMode=remainInDiagMode, cardSecState=cardSecState, cardDetectedChange=cardDetectedChange, TestTransactionId=TestTransactionId, cardFwVersion=cardFwVersion, cardShelfId=cardShelfId, cmPrimarySID=cmPrimarySID, cardRamSize=cardRamSize, portCardId=portCardId, portPrStateChange=portPrStateChange, errEvRecoveryEnabled=errEvRecoveryEnabled, cardId=cardId, diskVolumeUsageNotification=diskVolumeUsageNotification, portType=portType, diskVolumeCardId=diskVolumeCardId, diskVolumeId=diskVolumeId, cardCarrierHwVersion=cardCarrierHwVersion, downstreamPowerLoss=downstreamPowerLoss, cardTestTransId=cardTestTransId, portTxFlowControlDetected=portTxFlowControlDetected, cmRegistrationNotification=cmRegistrationNotification, cardLastChangeTime=cardLastChangeTime, cardSubType=cardSubType, cardLastBootVersion=cardLastBootVersion, cardPicSerialNum=cardPicSerialNum, diagTestId=diagTestId, cardTestScheduleCommand=cardTestScheduleCommand, cardTestResult=cardTestResult, portNumChans=portNumChans, cmResetClearNotification=cmResetClearNotification, TestId=TestId, cardSpareGroupId=cardSpareGroupId, cardNumPorts=cardNumPorts, cmMacAddress=cmMacAddress, diskVolumeDriveId=diskVolumeDriveId, auditName=auditName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResultDesc=cardTestResultDesc, eqDiagConfig=eqDiagConfig, cmVendor=cmVendor, cardHwVersion=cardHwVersion, portMaxDsPower=portMaxDsPower, equipmentAuditGroup=equipmentAuditGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, auditLogThrottle=auditLogThrottle, equipmentAudit=equipmentAudit, errEvRowStatus=errEvRowStatus, cardBootVersion=cardBootVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardAction=cardAction, diskVolumeTrapInh=diskVolumeTrapInh, cardTestSchedule=cardTestSchedule, systemClock=systemClock, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portNumber=portNumber, auditCardId=auditCardId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portMode=portMode, errEvLoggingEnabled=errEvLoggingEnabled, portLastChangeTime=portLastChangeTime, auditFailedCount=auditFailedCount, auditLogOutput=auditLogOutput, auditCycleCount=auditCycleCount, auditShelfId=auditShelfId, cardCarrierSerialNum=cardCarrierSerialNum, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDplxStatusChange=portDplxStatusChange, cardSecStateChange=cardSecStateChange, cadEquipmentMib=cadEquipmentMib, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeDiskSize=diskVolumeDiskSize, equipmentCardTestGroup=equipmentCardTestGroup, cardTestTime=cardTestTime, auditTable=auditTable, cardType=cardType, cardCpuSpeed=cardCpuSpeed, cmDevice=cmDevice, cardTemperature=cardTemperature, auditEntry=auditEntry, auditTotalPassedCount=auditTotalPassedCount, trapCounter=trapCounter, portDetectedMode=portDetectedMode, portBgpId=portBgpId, equipmentDiag=equipmentDiag, diskVolumeShelfId=diskVolumeShelfId, cardTestTable=cardTestTable, cardTestResultNotification=cardTestResultNotification, cardFlashSize=cardFlashSize, systemKey=systemKey, TestResult=TestResult, equipmentShelfGroup=equipmentShelfGroup, equipmentError=equipmentError, cardTestDescription=cardTestDescription, cardLastBootSource=cardLastBootSource, equipmentStateGroup=equipmentStateGroup, portMinDsPower=portMinDsPower, auditAutoScheduling=auditAutoScheduling, verboseLevel=verboseLevel, errEvLogLevel=errEvLogLevel, auditResult=auditResult, portMacAddress=portMacAddress, cmResetReason=cmResetReason, cardPicDetected=cardPicDetected, portConnectorId=portConnectorId, cardShelfTestIndex=cardShelfTestIndex, cardDplxStatus=cardDplxStatus, cardTestType=cardTestType, TestCommand=TestCommand, cmResetInfo=cmResetInfo, cardDetected=cardDetected, equipmentCardGroup=equipmentCardGroup, cardLastResetReason=cardLastResetReason, cardSubDetected=cardSubDetected, errEventId=errEventId, cardSpareGroupMode=cardSpareGroupMode, consoleOutput=consoleOutput, cardDplxStatusChange=cardDplxStatusChange, cardCPLDVersion=cardCPLDVersion, systemGeneralGroup=systemGeneralGroup, diskVolumeEntry=diskVolumeEntry, cardBusSpeed=cardBusSpeed, cardFpgaSource=cardFpgaSource, systemGeneral=systemGeneral, auditDescription=auditDescription, cmResetUpTime=cmResetUpTime, cardTempNoReportNotification=cardTempNoReportNotification, portAction=portAction, cardPicHwVersion=cardPicHwVersion, TestType=TestType, equipmentTraps=equipmentTraps, shelfName=shelfName, cardCarrierFwVersion=cardCarrierFwVersion, cardTrapInh=cardTrapInh, portDplxStatus=portDplxStatus, auditStatus=auditStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfObjects=shelfObjects, portSecStateChange=portSecStateChange, equipmentDiagGroup=equipmentDiagGroup, cardUpTime=cardUpTime, TestScheduleCommand=TestScheduleCommand, trapSeverity=trapSeverity, diskVolumeName=diskVolumeName, diskVolumeFileName=diskVolumeFileName, equipmentPortGroup=equipmentPortGroup, cardPrState=cardPrState, diskVolumeGroup=diskVolumeGroup, auditPassedCount=auditPassedCount, cardNumber=cardNumber, cadEquipmentMibConformance=cadEquipmentMibConformance, portCardSubType=portCardSubType, cmIpAddress=cmIpAddress, shelfSwVersion=shelfSwVersion, portShelfId=portShelfId, cardAdminState=cardAdminState, cardEntry=cardEntry, cardSerialNum=cardSerialNum, cardTemperatureHighError=cardTemperatureHighError, shelfNumber=shelfNumber, cardPrStateChange=cardPrStateChange, cardTable=cardTable)
