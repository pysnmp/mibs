...
PicType, ShelfId, CardType, SecondaryState, PortType, FirmwareSource, EqActionType, PortId, PortDetectedMode, PrimaryState, DiskVolumeUsageLevel, AdminState, DuplexStatus, UnknownState, CardSubType, PortMode, CardId, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "PicType", "ShelfId", "CardType", "SecondaryState", "PortType", "FirmwareSource", "EqActionType", "PortId", "PortDetectedMode", "PrimaryState", "DiskVolumeUsageLevel", "AdminState", "DuplexStatus", "UnknownState", "CardSubType", "PortMode", "CardId", "FlowControlMode")
...
TruthValue, RowStatus, TextualConvention, DisplayString, MacAddress, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "TextualConvention", "DisplayString", "MacAddress", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portPrState=portPrState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCPLDVersion=cardCPLDVersion, cardShelfTestIndex=cardShelfTestIndex, TestCommand=TestCommand, fpgaErrorEventTable=fpgaErrorEventTable, portDescription=portDescription, equipmentAudit=equipmentAudit, equipmentShelfGroup=equipmentShelfGroup, portShelfId=portShelfId, cardNumPorts=cardNumPorts, portPrStateChange=portPrStateChange, equipmentTraps=equipmentTraps, diskVolumeGroup=diskVolumeGroup, cardFpgaSource=cardFpgaSource, cmMacAddress=cmMacAddress, cardTestDescription=cardTestDescription, portTxFlowControlDetected=portTxFlowControlDetected, cardSpareGroupMode=cardSpareGroupMode, portMode=portMode, diskVolumeEntry=diskVolumeEntry, errEvLoggingEnabled=errEvLoggingEnabled, cardLastBootSource=cardLastBootSource, systemGeneralGroup=systemGeneralGroup, diagTestId=diagTestId, cmRegistrationNotification=cmRegistrationNotification, cardSecStateChange=cardSecStateChange, cardName=cardName, cardSerialNum=cardSerialNum, cardDplxStatusChange=cardDplxStatusChange, cardTestTransId=cardTestTransId, cardPicSerialNum=cardPicSerialNum, portTable=portTable, cardTestResult=cardTestResult, auditEntry=auditEntry, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeName=diskVolumeName, portLastChangeTime=portLastChangeTime, cmResetUpTime=cmResetUpTime, auditLogThrottle=auditLogThrottle, auditCommand=auditCommand, cardPicDetected=cardPicDetected, diskDriveNumber=diskDriveNumber, diskVolumeNumber=diskVolumeNumber, portAdminState=portAdminState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTemperature=cardTemperature, diskVolumeId=diskVolumeId, consoleOutput=consoleOutput, cadEquipmentMibConformance=cadEquipmentMibConformance, cardNumber=cardNumber, shelfNumber=shelfNumber, auditId=auditId, trapSeverity=trapSeverity, cardDplxStatus=cardDplxStatus, diskVolumeFileName=diskVolumeFileName, cardTestResultNotification=cardTestResultNotification, portId=portId, cardTrapInh=cardTrapInh, auditCardId=auditCardId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFwUpdateStatus=cardFwUpdateStatus, auditName=auditName, portRxFlowControlMode=portRxFlowControlMode, eqDiagConfig=eqDiagConfig, cardTestSchedule=cardTestSchedule, portTxFlowControlMode=portTxFlowControlMode, cardSubDetected=cardSubDetected, trapCounter=trapCounter, cardCarrierFwVersion=cardCarrierFwVersion, portCardId=portCardId, portBgpId=portBgpId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portNumber=portNumber, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetReason=cmResetReason, cardPrStateChange=cardPrStateChange, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetNotification=cmResetNotification, cardDetected=cardDetected, portDplxStatus=portDplxStatus, portNumChans=portNumChans, portDetectedMode=portDetectedMode, equipmentAuditGroup=equipmentAuditGroup, TestSchedule=TestSchedule, cardTestEntry=cardTestEntry, diskVolumeUsageNotification=diskVolumeUsageNotification, TestType=TestType, cardSpareGroupId=cardSpareGroupId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portType=portType, portSecStateChange=portSecStateChange, equipmentPortGroup=equipmentPortGroup, equipmentError=equipmentError, TestId=TestId, cardHwVersion=cardHwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, cardTemperatureHighError=cardTemperatureHighError, auditStatus=auditStatus, diskVolumeDriveId=diskVolumeDriveId, cardTestResultDesc=cardTestResultDesc, auditTable=auditTable, auditTotalPassedCount=auditTotalPassedCount, cmResetClearNotification=cmResetClearNotification, cardLastResetReason=cardLastResetReason, diskVolumeTrapInh=diskVolumeTrapInh, equipmentDiagGroup=equipmentDiagGroup, cardCarrierHwVersion=cardCarrierHwVersion, cardCpuType=cardCpuType, cardTempOverHeatNotification=cardTempOverHeatNotification, portCurrDsPower=portCurrDsPower, cardSecState=cardSecState, fpgaErrorEventEntry=fpgaErrorEventEntry, systemClock=systemClock, TestResult=TestResult, portMacIfIndex=portMacIfIndex, cardFlashSize=cardFlashSize, equipmentTbl=equipmentTbl, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portDplxStatusChange=portDplxStatusChange, systemKey=systemKey, portAction=portAction, cardLastBootVersion=cardLastBootVersion, portDocsIfIndex=portDocsIfIndex, TestScheduleCommand=TestScheduleCommand, diskVolumeSize=diskVolumeSize, diskVolumeTable=diskVolumeTable, cardFwVersion=cardFwVersion, cadEquipmentMib=cadEquipmentMib, cardTestType=cardTestType, cardTestTime=cardTestTime, equipmentCardGroup=equipmentCardGroup, cardTestId=cardTestId, cardShelfId=cardShelfId, diskVolumeCardId=diskVolumeCardId, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierSerialNum=cardCarrierSerialNum, downstreamPowerLoss=downstreamPowerLoss, portTrapInh=portTrapInh, equipmentDiag=equipmentDiag, cmPrimarySID=cmPrimarySID, cardTestScheduleCommand=cardTestScheduleCommand, auditFailedCount=auditFailedCount, cardId=cardId, cardTestCommand=cardTestCommand, portSecState=portSecState, auditPassedCount=auditPassedCount, portMinDsPower=portMinDsPower, portConnectorId=portConnectorId, auditResult=auditResult, errEventId=errEventId, cardBootVersion=cardBootVersion, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeDiskSize=diskVolumeDiskSize, cardTestTable=cardTestTable, auditCycleCount=auditCycleCount, cardPicHwVersion=cardPicHwVersion, cardSwVersion=cardSwVersion, cmIpAddress=cmIpAddress, auditLogOutput=auditLogOutput, shelfObjects=shelfObjects, equipmentState=equipmentState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portMaxDsPower=portMaxDsPower, portMacAddress=portMacAddress, cmVendor=cmVendor, cardBusSpeed=cardBusSpeed, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardEntry=cardEntry, shelfName=shelfName, cardCpuSpeed=cardCpuSpeed, auditTime=auditTime, TestTransactionId=TestTransactionId, cmDevice=cmDevice, errEvRowStatus=errEvRowStatus, cardRamSize=cardRamSize, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTable=cardTable, shelfSwVersion=shelfSwVersion, systemGeneral=systemGeneral, cardAction=cardAction, portCardSubType=portCardSubType, verboseLevel=verboseLevel, cmResetInfo=cmResetInfo, cmUChannelID=cmUChannelID, cmResetStatus=cmResetStatus, auditDescription=auditDescription, cardUpTime=cardUpTime, cardTestName=cardTestName, cardPrState=cardPrState, cardType=cardType, cardTestIndex=cardTestIndex, portRxFlowControlDetected=portRxFlowControlDetected, cardDetectedChange=cardDetectedChange, diskVolumeShelfId=diskVolumeShelfId, auditShelfId=auditShelfId, diskVolumeUsageLevel=diskVolumeUsageLevel, remainInDiagMode=remainInDiagMode, errEvLogLevel=errEvLogLevel, auditAutoScheduling=auditAutoScheduling, portEntry=portEntry, cardAdminState=cardAdminState, cardSubType=cardSubType)
