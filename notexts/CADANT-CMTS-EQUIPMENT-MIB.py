...
UnknownState, DuplexStatus, CardId, PortType, SecondaryState, FirmwareSource, EqActionType, PortMode, ShelfId, CardSubType, PicType, FlowControlMode, PortId, PortDetectedMode, AdminState, PrimaryState, DiskVolumeUsageLevel, CardType = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "DuplexStatus", "CardId", "PortType", "SecondaryState", "FirmwareSource", "EqActionType", "PortMode", "ShelfId", "CardSubType", "PicType", "FlowControlMode", "PortId", "PortDetectedMode", "AdminState", "PrimaryState", "DiskVolumeUsageLevel", "CardType")
...
TextualConvention, DisplayString, DateAndTime, TimeStamp, TruthValue, RowStatus, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "DateAndTime", "TimeStamp", "TruthValue", "RowStatus", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portCurrDsPower=portCurrDsPower, portTrapInh=portTrapInh, auditStatus=auditStatus, cadEquipmentMib=cadEquipmentMib, cardTemperature=cardTemperature, auditTime=auditTime, equipmentTbl=equipmentTbl, cardTestCommand=cardTestCommand, portMacAddress=portMacAddress, portDplxStatusChange=portDplxStatusChange, systemKey=systemKey, consoleOutput=consoleOutput, portMacIfIndex=portMacIfIndex, cardTestSchedule=cardTestSchedule, cmResetNotification=cmResetNotification, cardNumber=cardNumber, cmUChannelID=cmUChannelID, cardTable=cardTable, verboseLevel=verboseLevel, equipmentCardGroup=equipmentCardGroup, cardTestIndex=cardTestIndex, cardTestResult=cardTestResult, TestScheduleCommand=TestScheduleCommand, portDescription=portDescription, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, remainInDiagMode=remainInDiagMode, cardTestResultNotification=cardTestResultNotification, cardLastBootSource=cardLastBootSource, cardCarrierSerialNum=cardCarrierSerialNum, cardFlashSize=cardFlashSize, cmDevice=cmDevice, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardBusSpeed=cardBusSpeed, cardLastResetReason=cardLastResetReason, equipmentDiag=equipmentDiag, cmResetStatus=cmResetStatus, eqDiagConfig=eqDiagConfig, cardPicHwVersion=cardPicHwVersion, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portNumChans=portNumChans, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeTrapInh=diskVolumeTrapInh, cardTestTransId=cardTestTransId, cardTestEntry=cardTestEntry, diskVolumeName=diskVolumeName, cmResetClearNotification=cmResetClearNotification, diskVolumeShelfId=diskVolumeShelfId, portId=portId, portNumber=portNumber, equipmentShelfGroup=equipmentShelfGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cardDetectedChange=cardDetectedChange, cardPicDetected=cardPicDetected, equipmentTraps=equipmentTraps, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeSize=diskVolumeSize, shelfObjects=shelfObjects, cardTestName=cardTestName, auditLogOutput=auditLogOutput, cardTemperatureHighError=cardTemperatureHighError, equipmentState=equipmentState, portRxFlowControlDetected=portRxFlowControlDetected, portAction=portAction, errEvRowStatus=errEvRowStatus, diskDriveNumber=diskDriveNumber, cardAction=cardAction, auditLogThrottle=auditLogThrottle, cardTestScheduleCommand=cardTestScheduleCommand, cardUpTime=cardUpTime, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmPrimarySID=cmPrimarySID, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentPortGroup=equipmentPortGroup, diagTestId=diagTestId, portEntry=portEntry, auditCardId=auditCardId, systemGeneralGroup=systemGeneralGroup, TestCommand=TestCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicSerialNum=cardPicSerialNum, cardSecStateChange=cardSecStateChange, cardPrState=cardPrState, diskVolumeFileName=diskVolumeFileName, cardTestType=cardTestType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFwVersion=cardFwVersion, cardAdminState=cardAdminState, cardRamSize=cardRamSize, portShelfId=portShelfId, portSecState=portSecState, diskVolumeGroup=diskVolumeGroup, cardSubType=cardSubType, equipmentError=equipmentError, portSecStateChange=portSecStateChange, cardEntry=cardEntry, cardId=cardId, cardDetected=cardDetected, equipmentAudit=equipmentAudit, TestResult=TestResult, cardSpareGroupMode=cardSpareGroupMode, portCardId=portCardId, cardTestResultDesc=cardTestResultDesc, errEvLogLevel=errEvLogLevel, portCardSubType=portCardSubType, cardTestDescription=cardTestDescription, cardType=cardType, cmMacAddress=cmMacAddress, portDetectedMode=portDetectedMode, cardSecState=cardSecState, TestId=TestId, TestTransactionId=TestTransactionId, cmResetReason=cmResetReason, cardBootVersion=cardBootVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditShelfId=auditShelfId, cardName=cardName, cmResetUpTime=cmResetUpTime, cardTrapInh=cardTrapInh, errEvLoggingEnabled=errEvLoggingEnabled, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeDiskSize=diskVolumeDiskSize, downstreamPowerLoss=downstreamPowerLoss, shelfSwVersion=shelfSwVersion, auditEntry=auditEntry, portDocsIfIndex=portDocsIfIndex, cardSwVersion=cardSwVersion, diskVolumeDriveId=diskVolumeDriveId, cardFpgaSource=cardFpgaSource, auditDescription=auditDescription, portBgpId=portBgpId, auditCommand=auditCommand, errEventId=errEventId, cmResetInfo=cmResetInfo, portDplxStatus=portDplxStatus, cardSpareGroupId=cardSpareGroupId, cardCPLDVersion=cardCPLDVersion, cmVendor=cmVendor, auditTotalPassedCount=auditTotalPassedCount, equipmentStateGroup=equipmentStateGroup, cardNumPorts=cardNumPorts, auditPassedCount=auditPassedCount, PYSNMP_MODULE_ID=cadEquipmentMib, auditCycleCount=auditCycleCount, portConnectorId=portConnectorId, auditAutoScheduling=auditAutoScheduling, auditTable=auditTable, auditFailedCount=auditFailedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCarrierHwVersion=cardCarrierHwVersion, cardCarrierFwVersion=cardCarrierFwVersion, trapSeverity=trapSeverity, shelfName=shelfName, cardCpuSpeed=cardCpuSpeed, cardTestTable=cardTestTable, portTable=portTable, TestSchedule=TestSchedule, cardLastChangeTime=cardLastChangeTime, cardHwVersion=cardHwVersion, diskVolumeEntry=diskVolumeEntry, diskVolumeNumber=diskVolumeNumber, cmIpAddress=cmIpAddress, equipmentCardTestGroup=equipmentCardTestGroup, trapCounter=trapCounter, diskVolumeTable=diskVolumeTable, auditName=auditName, cardDplxStatusChange=cardDplxStatusChange, portLastChangeTime=portLastChangeTime, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeId=diskVolumeId, diskVolumeUsageNotification=diskVolumeUsageNotification, fpgaErrorEventEntry=fpgaErrorEventEntry, portMode=portMode, portRxFlowControlMode=portRxFlowControlMode, cadEquipmentMibCompliance=cadEquipmentMibCompliance, shelfNumber=shelfNumber, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditId=auditId, portPrState=portPrState, cardTestId=cardTestId, cardCpuType=cardCpuType, cardTemperatureHighWarn=cardTemperatureHighWarn, systemClock=systemClock, cardSubDetected=cardSubDetected, portMinDsPower=portMinDsPower, portMaxDsPower=portMaxDsPower, portAdminState=portAdminState, auditResult=auditResult, fpgaErrorEventTable=fpgaErrorEventTable, TestType=TestType, cardTestTime=cardTestTime, portType=portType, cardShelfTestIndex=cardShelfTestIndex, portPrStateChange=portPrStateChange, systemGeneral=systemGeneral, cmRegistrationNotification=cmRegistrationNotification, cardShelfId=cardShelfId, diskVolumeCardId=diskVolumeCardId, cardPrStateChange=cardPrStateChange, equipmentDiagGroup=equipmentDiagGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentAuditGroup=equipmentAuditGroup, cardLastBootVersion=cardLastBootVersion, cardSerialNum=cardSerialNum, cardDplxStatus=cardDplxStatus)
