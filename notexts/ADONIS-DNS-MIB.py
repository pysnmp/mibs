...
enterprises, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, Counter64, Counter32, ModuleIdentity, NotificationType, ObjectIdentity, MibIdentifier, IpAddress, TimeTicks, Unsigned32, Integer32, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "Counter64", "Counter32", "ModuleIdentity", "NotificationType", "ObjectIdentity", "MibIdentifier", "IpAddress", "TimeTicks", "Unsigned32", "Integer32", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpConfig=dhcpConfig, trapReplicationFailure=trapReplicationFailure, tftpDaemonRunning=tftpDaemonRunning, dns=dns, dhcpPoolSize=dhcpPoolSize, dnsStats=dnsStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, systemState=systemState, dhcpLeaseTable=dhcpLeaseTable, ha=ha, trapHA=trapHA, trapTFTPDaemon=trapTFTPDaemon, haService=haService, adonis=adonis, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonRunning=dnsDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, system=system, dhcpPoolStartIP=dhcpPoolStartIP, commandServer=commandServer, trapCommandServerDaemon=trapCommandServerDaemon, adonisTraps=adonisTraps, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetUsed=dhcpSubnetUsed, commandServerDaemon=commandServerDaemon, trapDNS=trapDNS, dhcpPoolUsed=dhcpPoolUsed, dnsStatsReferral=dnsStatsReferral, lcd=lcd, commandServerDaemonRunning=commandServerDaemonRunning, trapDHCP=trapDHCP, adonisObjects=adonisObjects, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, PYSNMP_MODULE_ID=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftp=tftp, haReplicationBinding=haReplicationBinding, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFailOverState=dhcpFailOverState, lcdDaemon=lcdDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, licenseValid=licenseValid, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsFailure=dnsStatsFailure, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcp=dhcp, dnsStatsSuccess=dnsStatsSuccess, trapSystemDaemon=trapSystemDaemon, haServiceNodeType=haServiceNodeType, dhcpPoolAlert=dhcpPoolAlert, trapReplication=trapReplication, dnsDaemon=dnsDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dhcpIP=dhcpIP, systemDaemon=systemDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapCommandServer=trapCommandServer, trapTFTP=trapTFTP, dhcpStats=dhcpStats, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDNSDaemon=trapDNSDaemon, tftpDaemon=tftpDaemon, licenseExpiry=licenseExpiry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceRunning=haServiceRunning, dhcpPoolTable=dhcpPoolTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetSize=dhcpSubnetSize, trapHAServiceFailOver=trapHAServiceFailOver, trapSystem=trapSystem)
