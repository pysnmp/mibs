...
TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, MibIdentifier, Counter64, ModuleIdentity, Gauge32, Unsigned32, IpAddress, enterprises, Bits, NotificationType, iso, Counter32, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "MibIdentifier", "Counter64", "ModuleIdentity", "Gauge32", "Unsigned32", "IpAddress", "enterprises", "Bits", "NotificationType", "iso", "Counter32", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonisTraps=adonisTraps, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetEntry=dhcpSubnetEntry, trapDHCPDaemon=trapDHCPDaemon, licenseExpiry=licenseExpiry, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolUsed=dhcpPoolUsed, trapTFTPDaemon=trapTFTPDaemon, dhcpIP=dhcpIP, tftp=tftp, dhcpFixedIP=dhcpFixedIP, dhcpConfig=dhcpConfig, trapSystemDaemon=trapSystemDaemon, dhcpPoolTable=dhcpPoolTable, dnsStats=dnsStats, dhcp=dhcp, systemDaemon=systemDaemon, trapReplicationFailure=trapReplicationFailure, trapSystem=trapSystem, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServerDaemon=commandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTable=dhcpLeaseTable, trapHAServiceFailOver=trapHAServiceFailOver, licenseValid=licenseValid, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemon=dnsDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceNodeType=haServiceNodeType, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, trapCommandServer=trapCommandServer, commandServer=commandServer, dhcpDaemon=dhcpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsRecursion=dnsStatsRecursion, lcdDaemon=lcdDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTP=trapTFTP, trapReplication=trapReplication, dhcpFailOverState=dhcpFailOverState, trapDNS=trapDNS, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsReferral=dnsStatsReferral, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haService=haService, dnsStatsSuccess=dnsStatsSuccess, systemState=systemState, dhcpStats=dhcpStats, adonisObjects=adonisObjects, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonRunning=dnsDaemonRunning, trapDNSDaemon=trapDNSDaemon, adonis=adonis, trapDHCP=trapDHCP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSize=dhcpPoolSize, dnsStatsNXDomain=dnsStatsNXDomain, system=system, PYSNMP_MODULE_ID=adonis, dhcpLeaseBindState=dhcpLeaseBindState, trapHA=trapHA, lcd=lcd, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseHostname=dhcpLeaseHostname, dns=dns, haReplicationBinding=haReplicationBinding, ha=ha, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServerDaemonRunning=commandServerDaemonRunning, haServiceRunning=haServiceRunning, tftpDaemon=tftpDaemon, dnsStatsFailure=dnsStatsFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess)
