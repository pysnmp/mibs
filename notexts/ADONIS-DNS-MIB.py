...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpConfig=dhcpConfig, trapTFTP=trapTFTP, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonRunning=dnsDaemonRunning, haReplicationBinding=haReplicationBinding, commandServerDaemonRunning=commandServerDaemonRunning, PYSNMP_MODULE_ID=adonis, dhcpIP=dhcpIP, dhcpSubnetSize=dhcpSubnetSize, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsSuccess=dnsStatsSuccess, dnsStatsFailure=dnsStatsFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemon=tftpDaemon, trapCommandServer=trapCommandServer, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonis=adonis, ha=ha, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetEntry=dhcpSubnetEntry, trapDHCP=trapDHCP, dhcp=dhcp, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplicationFailure=trapReplicationFailure, trapSystem=trapSystem, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonRunning=dhcpDaemonRunning, trapDNS=trapDNS, systemState=systemState, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemon=dhcpDaemon, dhcpSubnetIP=dhcpSubnetIP, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, commandServerDaemon=commandServerDaemon, dhcpLeaseEntry=dhcpLeaseEntry, adonisObjects=adonisObjects, trapReplication=trapReplication, dns=dns, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystemDaemon=trapSystemDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapCommandServerDaemon=trapCommandServerDaemon, dnsStats=dnsStats, dhcpFailOverState=dhcpFailOverState, dhcpFixedIP=dhcpFixedIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, system=system, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, commandServer=commandServer, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolUsed=dhcpPoolUsed, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSize=dhcpPoolSize, dnsStatsReferral=dnsStatsReferral, dhcpLeaseStartTime=dhcpLeaseStartTime, lcdDaemon=lcdDaemon, lcd=lcd, dhcpStats=dhcpStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemDaemon=systemDaemon, haService=haService, tftp=tftp, haServiceNodeType=haServiceNodeType, licenseValid=licenseValid, dhcpPoolTable=dhcpPoolTable, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEndIP=dhcpPoolEndIP, trapDNSDaemon=trapDNSDaemon, dnsDaemon=dnsDaemon, trapHA=trapHA, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapTFTPDaemon=trapTFTPDaemon, adonisTraps=adonisTraps, tftpDaemonRunning=tftpDaemonRunning, haServiceRunning=haServiceRunning)
