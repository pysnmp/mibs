...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplicationFailure=trapReplicationFailure, dhcpPoolTable=dhcpPoolTable, trapCommandServer=trapCommandServer, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemon=dnsDaemon, haServiceNodeType=haServiceNodeType, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, dhcpIP=dhcpIP, trapSystemDaemon=trapSystemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServerDaemon=trapCommandServerDaemon, trapDHCP=trapDHCP, haServiceRunning=haServiceRunning, trapTFTP=trapTFTP, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemon=commandServerDaemon, dhcpFixedIP=dhcpFixedIP, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStats=dnsStats, haReplicationBinding=haReplicationBinding, trapHA=trapHA, trapDNSDaemon=trapDNSDaemon, haService=haService, PYSNMP_MODULE_ID=adonis, trapTFTPDaemon=trapTFTPDaemon, trapDNS=trapDNS, dns=dns, dhcp=dhcp, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonisObjects=adonisObjects, dnsStatsReferral=dnsStatsReferral, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetAlert=dhcpSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFailOverState=dhcpFailOverState, dnsStatsFailure=dnsStatsFailure, trapReplication=trapReplication, dhcpConfig=dhcpConfig, licenseExpiry=licenseExpiry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, ha=ha, adonisTraps=adonisTraps, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsRecursion=dnsStatsRecursion, system=system, trapSystem=trapSystem, dhcpLeaseHostname=dhcpLeaseHostname, adonis=adonis, systemDaemon=systemDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsSuccess=dnsStatsSuccess, tftp=tftp, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetUsed=dhcpSubnetUsed, licenseValid=licenseValid, dhcpStats=dhcpStats, dhcpLeaseEntry=dhcpLeaseEntry, tftpDaemon=tftpDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseStartTime=dhcpLeaseStartTime, systemState=systemState, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetSize=dhcpSubnetSize, lcdDaemon=lcdDaemon, commandServer=commandServer, dhcpPoolSize=dhcpPoolSize, dhcpDaemonRunning=dhcpDaemonRunning, lcd=lcd, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemon=dhcpDaemon)
