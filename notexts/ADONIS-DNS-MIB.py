...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolTable=dhcpPoolTable, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolAlert=dhcpPoolAlert, trapDHCPDaemon=trapDHCPDaemon, PYSNMP_MODULE_ID=adonis, dnsDaemon=dnsDaemon, dnsStatsReferral=dnsStatsReferral, dhcpPoolEndIP=dhcpPoolEndIP, dnsStats=dnsStats, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonRunning=dnsDaemonRunning, trapSystemDaemon=trapSystemDaemon, dhcpPoolSize=dhcpPoolSize, haService=haService, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsRecursion=dnsStatsRecursion, trapTFTP=trapTFTP, systemState=systemState, dhcpPoolEntry=dhcpPoolEntry, dhcpStats=dhcpStats, adonisTraps=adonisTraps, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, tftp=tftp, lcdDaemon=lcdDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, adonis=adonis, dnsStatsSuccess=dnsStatsSuccess, adonisObjects=adonisObjects, commandServerDaemon=commandServerDaemon, trapTFTPDaemon=trapTFTPDaemon, tftpDaemon=tftpDaemon, haServiceNodeType=haServiceNodeType, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServer=commandServer, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapSystem=trapSystem, dhcpIP=dhcpIP, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haReplicationBinding=haReplicationBinding, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseBindState=dhcpLeaseBindState, system=system, dhcp=dhcp, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetSize=dhcpSubnetSize, dns=dns, systemDaemon=systemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, licenseExpiry=licenseExpiry, dhcpFixedIP=dhcpFixedIP, licenseValid=licenseValid, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcd=lcd, dhcpConfig=dhcpConfig, trapHA=trapHA, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, trapReplication=trapReplication, trapCommandServer=trapCommandServer, dhcpDaemonRunning=dhcpDaemonRunning, trapDNSDaemon=trapDNSDaemon, ha=ha, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFailOverState=dhcpFailOverState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemon=dhcpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetMask=dhcpSubnetMask, trapDHCP=trapDHCP, dnsStatsFailure=dnsStatsFailure, haServiceRunning=haServiceRunning, trapReplicationFailure=trapReplicationFailure, trapDNS=trapDNS)
