...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTP=trapTFTP, haService=haService, dhcpFailOverState=dhcpFailOverState, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPTable=dhcpFixedIPTable, dns=dns, dhcpDaemon=dhcpDaemon, dhcpConfig=dhcpConfig, dhcpIP=dhcpIP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolTable=dhcpPoolTable, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolUsed=dhcpPoolUsed, dnsStats=dnsStats, tftpDaemonRunning=tftpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcp=dhcp, trapSystemDaemon=trapSystemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsSuccess=dnsStatsSuccess, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapTFTPDaemon=trapTFTPDaemon, haReplicationBinding=haReplicationBinding, adonisObjects=adonisObjects, adonisTraps=adonisTraps, dhcpLeaseHostname=dhcpLeaseHostname, dhcpStats=dhcpStats, dnsStatsNXRRSet=dnsStatsNXRRSet, systemDaemon=systemDaemon, adonis=adonis, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, tftpDaemon=tftpDaemon, commandServer=commandServer, dhcpSubnetTable=dhcpSubnetTable, trapDHCP=trapDHCP, trapReplication=trapReplication, system=system, trapCommandServer=trapCommandServer, dhcpPoolSubnetIP=dhcpPoolSubnetIP, PYSNMP_MODULE_ID=adonis, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, trapDNS=trapDNS, dhcpPoolAlert=dhcpPoolAlert, trapHA=trapHA, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcdDaemon=lcdDaemon, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolEntry=dhcpPoolEntry, trapSystem=trapSystem, dnsDaemonRunning=dnsDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dnsDaemon=dnsDaemon, dhcpPoolSize=dhcpPoolSize, commandServerDaemon=commandServerDaemon, trapDNSDaemon=trapDNSDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcd=lcd, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, ha=ha, systemState=systemState, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEndIP=dhcpPoolEndIP, haServiceRunning=haServiceRunning, licenseValid=licenseValid, licenseExpiry=licenseExpiry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, trapReplicationFailure=trapReplicationFailure, tftp=tftp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIP=dhcpFixedIP, dnsStatsFailure=dnsStatsFailure)
