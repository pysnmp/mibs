...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServer=commandServer, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, tftpDaemon=tftpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseTable=dhcpLeaseTable, system=system, PYSNMP_MODULE_ID=adonis, trapDNS=trapDNS, trapDHCP=trapDHCP, dnsDaemonRunning=dnsDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, tftpDaemonRunning=tftpDaemonRunning, commandServerDaemon=commandServerDaemon, haReplicationBinding=haReplicationBinding, dhcpSubnetMask=dhcpSubnetMask, dnsStats=dnsStats, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplication=trapReplication, ha=ha, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haService=haService, dhcpIP=dhcpIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcp=dhcp, dhcpStats=dhcpStats, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolAlert=dhcpPoolAlert, adonisObjects=adonisObjects, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonRunning=dhcpDaemonRunning, licenseValid=licenseValid, trapCommandServer=trapCommandServer, trapTFTP=trapTFTP, dnsStatsSuccess=dnsStatsSuccess, systemState=systemState, lcdDaemon=lcdDaemon, trapHA=trapHA, dnsStatsReferral=dnsStatsReferral, trapHAServiceFailOver=trapHAServiceFailOver, trapCommandServerDaemon=trapCommandServerDaemon, adonis=adonis, dhcpSubnetAlert=dhcpSubnetAlert, tftp=tftp, dhcpDaemon=dhcpDaemon, dhcpPoolSize=dhcpPoolSize, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dns=dns, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, systemDaemon=systemDaemon, dhcpConfig=dhcpConfig, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIP=dhcpFixedIP, dhcpSubnetUsed=dhcpSubnetUsed, lcd=lcd, dnsDaemon=dnsDaemon, trapSystem=trapSystem, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dhcpFailOverState=dhcpFailOverState, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDHCPDaemon=trapDHCPDaemon, adonisTraps=adonisTraps, dhcpPoolTable=dhcpPoolTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseExpiry=licenseExpiry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceRunning=haServiceRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNSDaemon=trapDNSDaemon, dhcpPoolUsed=dhcpPoolUsed, trapReplicationFailure=trapReplicationFailure, haServiceNodeType=haServiceNodeType, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSubnetIP=dhcpPoolSubnetIP)
