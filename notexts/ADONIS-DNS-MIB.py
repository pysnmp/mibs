...
enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, Gauge32, Bits, TimeTicks, ObjectIdentity, Counter32, IpAddress, Integer32, Unsigned32, ModuleIdentity, NotificationType, MibIdentifier, iso = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "Gauge32", "Bits", "TimeTicks", "ObjectIdentity", "Counter32", "IpAddress", "Integer32", "Unsigned32", "ModuleIdentity", "NotificationType", "MibIdentifier", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolAlert=dhcpPoolAlert, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, haService=haService, dnsStatsReferral=dnsStatsReferral, trapReplication=trapReplication, haReplicationBinding=haReplicationBinding, dnsDaemonRunning=dnsDaemonRunning, tftp=tftp, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haServiceNodeType=haServiceNodeType, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, licenseValid=licenseValid, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSubnetIP=dhcpPoolSubnetIP, system=system, trapCommandServerDaemon=trapCommandServerDaemon, commandServer=commandServer, tftpDaemonRunning=tftpDaemonRunning, haServiceRunning=haServiceRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsFailure=dnsStatsFailure, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolSize=dhcpPoolSize, trapDHCPDaemon=trapDHCPDaemon, adonisObjects=adonisObjects, trapCommandServer=trapCommandServer, dhcpStats=dhcpStats, trapHA=trapHA, trapDNSDaemon=trapDNSDaemon, systemState=systemState, trapTFTPDaemon=trapTFTPDaemon, trapReplicationFailure=trapReplicationFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, licenseExpiry=licenseExpiry, dnsStats=dnsStats, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTable=dhcpLeaseTable, lcd=lcd, dhcpIP=dhcpIP, PYSNMP_MODULE_ID=adonis, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapSystemDaemon=trapSystemDaemon, ha=ha, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEntry=dhcpPoolEntry, trapDHCP=trapDHCP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTP=trapTFTP, lcdDaemon=lcdDaemon, trapDNS=trapDNS, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisTraps=adonisTraps, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemon=dnsDaemon, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcp=dhcp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapSystem=trapSystem, dhcpLeaseHostname=dhcpLeaseHostname, tftpDaemon=tftpDaemon, systemDaemon=systemDaemon, dhcpPoolTable=dhcpPoolTable, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFixedIP=dhcpFixedIP, commandServerDaemon=commandServerDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFailOverState=dhcpFailOverState, adonis=adonis, dhcpDaemon=dhcpDaemon, dhcpConfig=dhcpConfig, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseEntry=dhcpLeaseEntry)
