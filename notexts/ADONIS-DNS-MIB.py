...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseTable=dhcpLeaseTable, ha=ha, dhcp=dhcp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemon=dnsDaemon, trapSystem=trapSystem, dns=dns, commandServer=commandServer, dhcpLeaseEntry=dhcpLeaseEntry, systemDaemon=systemDaemon, licenseExpiry=licenseExpiry, commandServerDaemon=commandServerDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCPDaemon=trapDHCPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpStats=dhcpStats, adonis=adonis, trapReplication=trapReplication, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonRunning=dhcpDaemonRunning, trapReplicationFailure=trapReplicationFailure, dhcpPoolEntry=dhcpPoolEntry, trapSystemDaemon=trapSystemDaemon, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTP=trapTFTP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsFailure=dnsStatsFailure, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSize=dhcpPoolSize, dhcpPoolEndIP=dhcpPoolEndIP, trapCommandServer=trapCommandServer, dhcpDaemon=dhcpDaemon, lcd=lcd, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolStartIP=dhcpPoolStartIP, system=system, haService=haService, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemon=tftpDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsSuccess=dnsStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, dnsStats=dnsStats, trapDHCP=trapDHCP, dhcpFixedIPTable=dhcpFixedIPTable, licenseValid=licenseValid, adonisObjects=adonisObjects, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceNodeType=haServiceNodeType, adonisTraps=adonisTraps, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, dhcpConfig=dhcpConfig, dhcpSubnetTable=dhcpSubnetTable, tftp=tftp, trapDNS=trapDNS, systemState=systemState, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseEndTime=dhcpLeaseEndTime, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFailOverState=dhcpFailOverState, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolTable=dhcpPoolTable, trapHAServiceFailOver=trapHAServiceFailOver, lcdDaemon=lcdDaemon, dhcpSubnetIP=dhcpSubnetIP, trapDNSDaemon=trapDNSDaemon, haReplicationBinding=haReplicationBinding, dhcpFixedIP=dhcpFixedIP, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haServiceRunning=haServiceRunning)
