...
Counter32, Unsigned32, NotificationType, Counter64, ModuleIdentity, enterprises, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, iso, Bits, TimeTicks, MibIdentifier, ObjectIdentity, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "Unsigned32", "NotificationType", "Counter64", "ModuleIdentity", "enterprises", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "iso", "Bits", "TimeTicks", "MibIdentifier", "ObjectIdentity", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcp=dhcp, ha=ha, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemon=dnsDaemon, dhcpPoolEntry=dhcpPoolEntry, system=system, dnsStats=dnsStats, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haServiceNodeType=haServiceNodeType, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, adonisObjects=adonisObjects, adonis=adonis, lcd=lcd, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonRunning=dnsDaemonRunning, dnsStatsFailure=dnsStatsFailure, dhcpPoolAlert=dhcpPoolAlert, trapDHCP=trapDHCP, dhcpFailOverState=dhcpFailOverState, trapSystemDaemon=trapSystemDaemon, trapTFTPDaemon=trapTFTPDaemon, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, dhcpStats=dhcpStats, dhcpLeaseHostname=dhcpLeaseHostname, trapDNSDaemon=trapDNSDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, dnsStatsSuccess=dnsStatsSuccess, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolSize=dhcpPoolSize, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, systemState=systemState, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, licenseValid=licenseValid, tftp=tftp, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplication=trapReplication, haReplicationBinding=haReplicationBinding, dns=dns, trapCommandServer=trapCommandServer, trapReplicationFailure=trapReplicationFailure, dhcpSubnetIP=dhcpSubnetIP, dhcpFixedIP=dhcpFixedIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsReferral=dnsStatsReferral, trapSystem=trapSystem, commandServer=commandServer, adonisTraps=adonisTraps, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemon=commandServerDaemon, haService=haService, trapDNS=trapDNS, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, systemDaemon=systemDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolTable=dhcpPoolTable, dhcpDaemon=dhcpDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpConfig=dhcpConfig, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolUsed=dhcpPoolUsed, licenseExpiry=licenseExpiry, lcdDaemon=lcdDaemon, haServiceRunning=haServiceRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred)
