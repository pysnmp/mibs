...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapCommandServer=trapCommandServer, dnsStatsReferral=dnsStatsReferral, dns=dns, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolStartIP=dhcpPoolStartIP, trapDNSDaemon=trapDNSDaemon, lcd=lcd, dhcpPoolUsed=dhcpPoolUsed, system=system, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonRunning=dnsDaemonRunning, dnsStatsFailure=dnsStatsFailure, trapSystemDaemon=trapSystemDaemon, trapDNS=trapDNS, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTP=trapTFTP, dnsStatsSuccess=dnsStatsSuccess, dhcpStats=dhcpStats, adonis=adonis, dnsDaemon=dnsDaemon, systemDaemon=systemDaemon, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, licenseExpiry=licenseExpiry, tftp=tftp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSize=dhcpPoolSize, dnsStatsNXDomain=dnsStatsNXDomain, ha=ha, dhcpFixedIP=dhcpFixedIP, trapDHCPDaemon=trapDHCPDaemon, dhcpConfig=dhcpConfig, dhcpLeaseStartTime=dhcpLeaseStartTime, commandServer=commandServer, haServiceRunning=haServiceRunning, dhcp=dhcp, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, haServiceNodeType=haServiceNodeType, dhcpLeaseTable=dhcpLeaseTable, dhcpIP=dhcpIP, systemState=systemState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStats=dnsStats, PYSNMP_MODULE_ID=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolAlert=dhcpPoolAlert, haReplicationBinding=haReplicationBinding, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapHA=trapHA, dhcpSubnetTable=dhcpSubnetTable, tftpDaemon=tftpDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCP=trapDHCP, dhcpPoolEndIP=dhcpPoolEndIP, trapHAServiceFailOver=trapHAServiceFailOver, trapReplicationFailure=trapReplicationFailure, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemon=dhcpDaemon, dhcpFixedIPTable=dhcpFixedIPTable, haService=haService, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseValid=licenseValid, lcdDaemon=lcdDaemon, commandServerDaemon=commandServerDaemon, trapReplication=trapReplication, adonisTraps=adonisTraps, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolTable=dhcpPoolTable, tftpDaemonRunning=tftpDaemonRunning, trapSystem=trapSystem, dhcpPoolEntry=dhcpPoolEntry, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseBindState=dhcpLeaseBindState, adonisObjects=adonisObjects, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemonRunning=dhcpDaemonRunning)
