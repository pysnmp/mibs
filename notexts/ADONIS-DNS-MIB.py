...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, systemState=systemState, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, lcd=lcd, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFailOverState=dhcpFailOverState, trapHA=trapHA, haService=haService, trapTFTP=trapTFTP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServer=commandServer, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHostname=dhcpLeaseHostname, trapReplicationFailure=trapReplicationFailure, dhcpIP=dhcpIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNS=trapDNS, systemDaemon=systemDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemon=dhcpDaemon, trapReplication=trapReplication, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haServiceNodeType=haServiceNodeType, dhcpSubnetSize=dhcpSubnetSize, ha=ha, trapDHCP=trapDHCP, tftp=tftp, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisObjects=adonisObjects, dhcp=dhcp, licenseValid=licenseValid, dhcpDaemonRunning=dhcpDaemonRunning, dhcpConfig=dhcpConfig, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolStartIP=dhcpPoolStartIP, PYSNMP_MODULE_ID=adonis, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystemDaemon=trapSystemDaemon, licenseExpiry=licenseExpiry, trapSystem=trapSystem, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, adonisTraps=adonisTraps, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haReplicationBinding=haReplicationBinding, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetTable=dhcpSubnetTable, adonis=adonis, system=system, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSize=dhcpPoolSize, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIPTable=dhcpFixedIPTable, commandServerDaemon=commandServerDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolUsed=dhcpPoolUsed, trapCommandServer=trapCommandServer, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolTable=dhcpPoolTable, trapDHCPDaemon=trapDHCPDaemon, dns=dns, trapDNSDaemon=trapDNSDaemon, tftpDaemon=tftpDaemon, dhcpStats=dhcpStats, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXDomain=dnsStatsNXDomain, haServiceRunning=haServiceRunning, dnsStatsReferral=dnsStatsReferral, dnsDaemon=dnsDaemon, dnsStats=dnsStats, lcdDaemon=lcdDaemon, trapHAServiceFailOver=trapHAServiceFailOver)
