...
TimeTicks, Counter32, Counter64, enterprises, Integer32, ModuleIdentity, Gauge32, Bits, NotificationType, Unsigned32, MibIdentifier, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, iso = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Counter32", "Counter64", "enterprises", "Integer32", "ModuleIdentity", "Gauge32", "Bits", "NotificationType", "Unsigned32", "MibIdentifier", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetSize=dhcpSubnetSize, dhcp=dhcp, adonis=adonis, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolSize=dhcpPoolSize, haService=haService, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, dhcpStats=dhcpStats, tftpDaemon=tftpDaemon, dhcpLeaseHostname=dhcpLeaseHostname, ha=ha, commandServer=commandServer, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, systemDaemon=systemDaemon, dhcpIP=dhcpIP, trapReplication=trapReplication, dhcpPoolUsed=dhcpPoolUsed, system=system, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIP=dhcpFixedIP, trapReplicationFailure=trapReplicationFailure, trapDHCP=trapDHCP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsRecursion=dnsStatsRecursion, trapDHCPDaemon=trapDHCPDaemon, dhcpDaemon=dhcpDaemon, trapHA=trapHA, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, dnsStats=dnsStats, lcd=lcd, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTPDaemon=trapTFTPDaemon, adonisObjects=adonisObjects, dhcpLeaseStartTime=dhcpLeaseStartTime, trapCommandServer=trapCommandServer, haServiceNodeType=haServiceNodeType, tftpDaemonRunning=tftpDaemonRunning, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemon=dnsDaemon, dnsStatsReferral=dnsStatsReferral, haReplicationBinding=haReplicationBinding, dhcpSubnetAlert=dhcpSubnetAlert, licenseValid=licenseValid, trapDNS=trapDNS, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisTraps=adonisTraps, dhcpSubnetIP=dhcpSubnetIP, systemState=systemState, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetEntry=dhcpSubnetEntry, trapDNSDaemon=trapDNSDaemon, dns=dns, dhcpFailOverState=dhcpFailOverState, trapCommandServerDaemon=trapCommandServerDaemon, dhcpConfig=dhcpConfig, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystem=trapSystem, dhcpSubnetTable=dhcpSubnetTable, haServiceRunning=haServiceRunning, dnsStatsFailure=dnsStatsFailure, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemonRunning=commandServerDaemonRunning, PYSNMP_MODULE_ID=adonis, dhcpSubnetUsed=dhcpSubnetUsed, commandServerDaemon=commandServerDaemon, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftp=tftp, dnsStatsSuccess=dnsStatsSuccess)
