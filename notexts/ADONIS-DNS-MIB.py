...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemon=dhcpDaemon, dnsStatsReferral=dnsStatsReferral, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, systemState=systemState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, licenseValid=licenseValid, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcp=dhcp, trapCommandServer=trapCommandServer, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetTable=dhcpSubnetTable, dnsDaemon=dnsDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTP=trapTFTP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, tftp=tftp, trapDHCP=trapDHCP, dnsStatsRecursion=dnsStatsRecursion, dhcpFailOverState=dhcpFailOverState, lcd=lcd, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapReplication=trapReplication, dhcpPoolTable=dhcpPoolTable, dhcpPoolAlert=dhcpPoolAlert, adonisObjects=adonisObjects, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, haServiceRunning=haServiceRunning, haReplicationBinding=haReplicationBinding, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapReplicationFailure=trapReplicationFailure, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapSystem=trapSystem, dhcpIP=dhcpIP, dhcpSubnetMask=dhcpSubnetMask, trapTFTPDaemon=trapTFTPDaemon, tftpDaemon=tftpDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpConfig=dhcpConfig, trapHAServiceFailOver=trapHAServiceFailOver, ha=ha, commandServer=commandServer, dnsStats=dnsStats, adonis=adonis, systemDaemon=systemDaemon, adonisTraps=adonisTraps, dhcpSubnetAlert=dhcpSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, trapHA=trapHA, dhcpLeaseEntry=dhcpLeaseEntry, lcdDaemon=lcdDaemon, commandServerDaemon=commandServerDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, licenseExpiry=licenseExpiry, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, system=system, dns=dns, dhcpPoolEndIP=dhcpPoolEndIP, haService=haService, dnsStatsFailure=dnsStatsFailure, dhcpPoolUsed=dhcpPoolUsed, trapDNSDaemon=trapDNSDaemon, dhcpPoolStartIP=dhcpPoolStartIP, trapDNS=trapDNS, PYSNMP_MODULE_ID=adonis, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetIP=dhcpSubnetIP, haServiceNodeType=haServiceNodeType, dhcpPoolSize=dhcpPoolSize)
