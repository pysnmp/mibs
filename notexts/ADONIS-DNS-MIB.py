...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplicationFailure=trapReplicationFailure, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCP=trapDHCP, lcdDaemon=lcdDaemon, trapDHCPDaemon=trapDHCPDaemon, system=system, dhcp=dhcp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, systemDaemon=systemDaemon, dhcpLeaseBindState=dhcpLeaseBindState, lcd=lcd, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetSize=dhcpSubnetSize, tftp=tftp, dhcpPoolEntry=dhcpPoolEntry, haServiceRunning=haServiceRunning, trapDNSDaemon=trapDNSDaemon, haServiceNodeType=haServiceNodeType, commandServer=commandServer, trapTFTP=trapTFTP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemonRunning=commandServerDaemonRunning, trapSystem=trapSystem, dhcpStats=dhcpStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonisTraps=adonisTraps, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseTable=dhcpLeaseTable, trapCommandServerDaemon=trapCommandServerDaemon, trapCommandServer=trapCommandServer, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, licenseValid=licenseValid, adonisObjects=adonisObjects, dnsDaemon=dnsDaemon, dhcpPoolSize=dhcpPoolSize, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsSuccess=dnsStatsSuccess, systemState=systemState, adonis=adonis, dhcpConfig=dhcpConfig, dhcpSubnetTable=dhcpSubnetTable, haService=haService, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStats=dnsStats, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDNS=trapDNS, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, ha=ha, trapSystemDaemon=trapSystemDaemon, trapHA=trapHA, dnsStatsReferral=dnsStatsReferral, trapTFTPDaemon=trapTFTPDaemon, trapReplication=trapReplication, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemon=dhcpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseEntry=dhcpLeaseEntry, dns=dns, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, PYSNMP_MODULE_ID=adonis, dnsStatsFailure=dnsStatsFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpIP=dhcpIP, dhcpSubnetUsed=dhcpSubnetUsed, commandServerDaemon=commandServerDaemon, tftpDaemonRunning=tftpDaemonRunning, licenseExpiry=licenseExpiry, dhcpFailOverState=dhcpFailOverState, tftpDaemon=tftpDaemon, haReplicationBinding=haReplicationBinding, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPTable=dhcpFixedIPTable)
