...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetMask=dhcpSubnetMask, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapCommandServer=trapCommandServer, lcd=lcd, haServiceRunning=haServiceRunning, dhcpDaemon=dhcpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, dhcpPoolEndIP=dhcpPoolEndIP, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, adonisTraps=adonisTraps, tftpDaemon=tftpDaemon, dhcp=dhcp, dhcpPoolTable=dhcpPoolTable, tftpDaemonRunning=tftpDaemonRunning, system=system, dnsStatsReferral=dnsStatsReferral, dhcpStats=dhcpStats, dhcpFixedIP=dhcpFixedIP, dhcpSubnetSize=dhcpSubnetSize, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseHostname=dhcpLeaseHostname, trapDHCP=trapDHCP, ha=ha, trapReplication=trapReplication, dhcpConfig=dhcpConfig, dhcpLeaseStartTime=dhcpLeaseStartTime, commandServer=commandServer, dnsStatsFailure=dnsStatsFailure, dhcpSubnetEntry=dhcpSubnetEntry, dnsStats=dnsStats, systemDaemon=systemDaemon, systemState=systemState, trapSystemDaemon=trapSystemDaemon, trapReplicationFailure=trapReplicationFailure, trapTFTPDaemon=trapTFTPDaemon, dnsStatsNXDomain=dnsStatsNXDomain, adonisObjects=adonisObjects, dhcpFailOverState=dhcpFailOverState, trapDNS=trapDNS, dhcpDaemonRunning=dhcpDaemonRunning, haServiceNodeType=haServiceNodeType, trapDHCPDaemon=trapDHCPDaemon, lcdDaemon=lcdDaemon, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemon=commandServerDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTP=trapTFTP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftp=tftp, dhcpIP=dhcpIP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseTable=dhcpLeaseTable, dnsStatsNXRRSet=dnsStatsNXRRSet, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolSize=dhcpPoolSize, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dns=dns, dhcpLeaseEntry=dhcpLeaseEntry, haService=haService, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolUsed=dhcpPoolUsed, dnsDaemon=dnsDaemon, licenseValid=licenseValid, licenseExpiry=licenseExpiry, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPTable=dhcpFixedIPTable, trapSystem=trapSystem, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonis=adonis, dnsDaemonRunning=dnsDaemonRunning)
