...
enterprises, ObjectIdentity, Counter32, Integer32, ModuleIdentity, iso, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, MibIdentifier, Unsigned32, Bits, Counter64, IpAddress, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "ObjectIdentity", "Counter32", "Integer32", "ModuleIdentity", "iso", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "MibIdentifier", "Unsigned32", "Bits", "Counter64", "IpAddress", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpStats=dhcpStats, haServiceRunning=haServiceRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, adonisTraps=adonisTraps, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEntry=dhcpLeaseEntry, licenseValid=licenseValid, licenseExpiry=licenseExpiry, trapDHCPDaemon=trapDHCPDaemon, tftpDaemon=tftpDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsStatsReferral=dnsStatsReferral, trapReplication=trapReplication, dnsStatsFailure=dnsStatsFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcp=dhcp, trapSystem=trapSystem, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCP=trapDHCP, commandServerDaemon=commandServerDaemon, trapDNS=trapDNS, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseTable=dhcpLeaseTable, dhcpConfig=dhcpConfig, dhcpIP=dhcpIP, commandServer=commandServer, dhcpDaemonRunning=dhcpDaemonRunning, haServiceNodeType=haServiceNodeType, dhcpPoolEntry=dhcpPoolEntry, adonisObjects=adonisObjects, dhcpPoolSize=dhcpPoolSize, trapSystemDaemon=trapSystemDaemon, system=system, dhcpSubnetMask=dhcpSubnetMask, dnsStats=dnsStats, haReplicationBinding=haReplicationBinding, dnsDaemon=dnsDaemon, ha=ha, dhcpFixedIP=dhcpFixedIP, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsNXDomain=dnsStatsNXDomain, haService=haService, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemon=dhcpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapTFTPDaemon=trapTFTPDaemon, trapReplicationFailure=trapReplicationFailure, trapDNSDaemon=trapDNSDaemon, systemDaemon=systemDaemon, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolTable=dhcpPoolTable, dhcpSubnetUsed=dhcpSubnetUsed, dns=dns, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapCommandServerDaemon=trapCommandServerDaemon, trapTFTP=trapTFTP, tftp=tftp, commandServerDaemonRunning=commandServerDaemonRunning, adonis=adonis, trapHA=trapHA, dhcpFailOverState=dhcpFailOverState, dnsStatsNXRRSet=dnsStatsNXRRSet, systemState=systemState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapCommandServer=trapCommandServer, PYSNMP_MODULE_ID=adonis)
