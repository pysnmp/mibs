...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcdDaemon=lcdDaemon, PYSNMP_MODULE_ID=adonis, trapReplicationFailure=trapReplicationFailure, dnsStatsFailure=dnsStatsFailure, trapSystem=trapSystem, dhcpFixedIPTable=dhcpFixedIPTable, licenseValid=licenseValid, dhcp=dhcp, trapDHCP=trapDHCP, trapTFTP=trapTFTP, dnsDaemon=dnsDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolStartIP=dhcpPoolStartIP, trapSystemDaemon=trapSystemDaemon, commandServerDaemon=commandServerDaemon, dhcpConfig=dhcpConfig, ha=ha, dhcpFailOverState=dhcpFailOverState, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolSize=dhcpPoolSize, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseTable=dhcpLeaseTable, tftp=tftp, lcd=lcd, tftpDaemon=tftpDaemon, haServiceRunning=haServiceRunning, dhcpSubnetSize=dhcpSubnetSize, trapCommandServerDaemon=trapCommandServerDaemon, trapReplication=trapReplication, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsRecursion=dnsStatsRecursion, adonis=adonis, dhcpDaemon=dhcpDaemon, dhcpIP=dhcpIP, dnsStatsReferral=dnsStatsReferral, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseEntry=dhcpLeaseEntry, adonisObjects=adonisObjects, haServiceNodeType=haServiceNodeType, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, system=system, commandServer=commandServer, trapDHCPDaemon=trapDHCPDaemon, systemState=systemState, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, systemDaemon=systemDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNS=trapDNS, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIP=dhcpFixedIP, haReplicationBinding=haReplicationBinding, dhcpPoolUsed=dhcpPoolUsed, dns=dns, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapHA=trapHA, dnsStatsSuccess=dnsStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolEntry=dhcpPoolEntry, trapDNSDaemon=trapDNSDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEndTime=dhcpLeaseEndTime, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolTable=dhcpPoolTable, dnsDaemonRunning=dnsDaemonRunning, haService=haService, adonisTraps=adonisTraps, dhcpStats=dhcpStats, licenseExpiry=licenseExpiry, dnsStats=dnsStats)
