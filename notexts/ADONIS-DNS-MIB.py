...
iso, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, MibIdentifier, NotificationType, enterprises, Bits, TimeTicks, ModuleIdentity, Counter32, ObjectIdentity, Counter64, Unsigned32, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "MibIdentifier", "NotificationType", "enterprises", "Bits", "TimeTicks", "ModuleIdentity", "Counter32", "ObjectIdentity", "Counter64", "Unsigned32", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", licenseExpiry=licenseExpiry, trapReplicationFailure=trapReplicationFailure, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dns=dns, trapTFTPDaemon=trapTFTPDaemon, lcd=lcd, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapTFTP=trapTFTP, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, PYSNMP_MODULE_ID=adonis, lcdDaemon=lcdDaemon, trapSystem=trapSystem, dhcp=dhcp, trapDNS=trapDNS, dhcpIP=dhcpIP, dhcpFailOverState=dhcpFailOverState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolEndIP=dhcpPoolEndIP, ha=ha, tftpDaemon=tftpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDHCP=trapDHCP, adonis=adonis, dnsDaemon=dnsDaemon, dhcpConfig=dhcpConfig, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetMask=dhcpSubnetMask, haServiceRunning=haServiceRunning, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolStartIP=dhcpPoolStartIP, trapSystemDaemon=trapSystemDaemon, trapHA=trapHA, commandServerDaemon=commandServerDaemon, commandServer=commandServer, dhcpSubnetSize=dhcpSubnetSize, dnsStatsReferral=dnsStatsReferral, tftp=tftp, dhcpStats=dhcpStats, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDHCPDaemon=trapDHCPDaemon, dnsStatsFailure=dnsStatsFailure, systemDaemon=systemDaemon, dnsStatsSuccess=dnsStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, haServiceNodeType=haServiceNodeType, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftpDaemonRunning=tftpDaemonRunning, trapReplication=trapReplication, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEndTime=dhcpLeaseEndTime, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, licenseValid=licenseValid, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSize=dhcpPoolSize, haReplicationBinding=haReplicationBinding, dhcpPoolEntry=dhcpPoolEntry, dnsStats=dnsStats, trapCommandServerDaemon=trapCommandServerDaemon, systemState=systemState, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIP=dhcpFixedIP, adonisTraps=adonisTraps, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisObjects=adonisObjects, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemon=dhcpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, system=system)
