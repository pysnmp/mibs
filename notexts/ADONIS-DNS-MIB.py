...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFailOverState=dhcpFailOverState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dhcpLeaseHostname=dhcpLeaseHostname, commandServer=commandServer, trapSystemDaemon=trapSystemDaemon, adonisObjects=adonisObjects, dhcpLeaseTable=dhcpLeaseTable, lcdDaemon=lcdDaemon, dhcpPoolTable=dhcpPoolTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, PYSNMP_MODULE_ID=adonis, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpIP=dhcpIP, commandServerDaemonRunning=commandServerDaemonRunning, haService=haService, tftpDaemon=tftpDaemon, haReplicationBinding=haReplicationBinding, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetIP=dhcpSubnetIP, dhcpFixedIPTable=dhcpFixedIPTable, commandServerDaemon=commandServerDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplication=trapReplication, dnsStatsReferral=dnsStatsReferral, adonisTraps=adonisTraps, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStats=dnsStats, dhcpSubnetEntry=dhcpSubnetEntry, systemState=systemState, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplicationFailure=trapReplicationFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDHCP=trapDHCP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolUsed=dhcpPoolUsed, ha=ha, dhcpDaemon=dhcpDaemon, system=system, dnsDaemon=dnsDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceRunning=haServiceRunning, dhcpStats=dhcpStats, trapCommandServer=trapCommandServer, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIP=dhcpFixedIP, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, adonis=adonis, dhcpPoolSize=dhcpPoolSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapSystem=trapSystem, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolEndIP=dhcpPoolEndIP, lcd=lcd, trapDNS=trapDNS, dhcpSubnetTable=dhcpSubnetTable, licenseExpiry=licenseExpiry, licenseValid=licenseValid, systemDaemon=systemDaemon, dnsStatsFailure=dnsStatsFailure, dhcpSubnetMask=dhcpSubnetMask, dhcp=dhcp, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPEntry=dhcpFixedIPEntry, trapHAServiceFailOver=trapHAServiceFailOver, tftp=tftp, dns=dns, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpConfig=dhcpConfig, dhcpPoolEntry=dhcpPoolEntry)
