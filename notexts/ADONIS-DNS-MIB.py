...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", licenseValid=licenseValid, dnsDaemon=dnsDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsStats=dnsStats, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetEntry=dhcpSubnetEntry, haServiceNodeType=haServiceNodeType, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, adonisObjects=adonisObjects, PYSNMP_MODULE_ID=adonis, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetIP=dhcpSubnetIP, dnsStatsFailure=dnsStatsFailure, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolStartIP=dhcpPoolStartIP, haService=haService, dhcpPoolUsed=dhcpPoolUsed, dhcpIP=dhcpIP, systemState=systemState, dnsStatsReferral=dnsStatsReferral, dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcdDaemon=lcdDaemon, dhcpDaemonRunning=dhcpDaemonRunning, adonisTraps=adonisTraps, dhcpFixedIP=dhcpFixedIP, dhcp=dhcp, trapDNS=trapDNS, haReplicationBinding=haReplicationBinding, dhcpSubnetMask=dhcpSubnetMask, dhcpStats=dhcpStats, trapSystem=trapSystem, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsSuccess=dnsStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, systemDaemon=systemDaemon, dnsStatsRecursion=dnsStatsRecursion, trapHA=trapHA, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonRunning=dnsDaemonRunning, tftp=tftp, dhcpSubnetSize=dhcpSubnetSize, licenseExpiry=licenseExpiry, trapCommandServer=trapCommandServer, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemon=tftpDaemon, dhcpSubnetUsed=dhcpSubnetUsed, ha=ha, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapReplication=trapReplication, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFailOverState=dhcpFailOverState, haServiceRunning=haServiceRunning, trapSystemDaemon=trapSystemDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseHostname=dhcpLeaseHostname, lcd=lcd, commandServer=commandServer, dhcpDaemon=dhcpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, tftpDaemonRunning=tftpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapReplicationFailure=trapReplicationFailure, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolEntry=dhcpPoolEntry, dns=dns, adonis=adonis, system=system, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolSize=dhcpPoolSize, trapTFTP=trapTFTP, commandServerDaemon=commandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, trapDHCP=trapDHCP, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsNXDomain=dnsStatsNXDomain, trapDNSDaemon=trapDNSDaemon, dhcpConfig=dhcpConfig, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure)
