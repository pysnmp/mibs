...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpConfig=dhcpConfig, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemon=commandServerDaemon, lcdDaemon=lcdDaemon, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, haServiceRunning=haServiceRunning, trapCommandServer=trapCommandServer, PYSNMP_MODULE_ID=adonis, dhcp=dhcp, tftp=tftp, dhcpPoolStartIP=dhcpPoolStartIP, trapSystemDaemon=trapSystemDaemon, dnsDaemon=dnsDaemon, commandServer=commandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetMask=dhcpSubnetMask, commandServerDaemonRunning=commandServerDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTP=trapTFTP, tftpDaemon=tftpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, trapDNSDaemon=trapDNSDaemon, haServiceNodeType=haServiceNodeType, dhcpFailOverState=dhcpFailOverState, dhcpDaemon=dhcpDaemon, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplication=trapReplication, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, trapDNS=trapDNS, adonisTraps=adonisTraps, dhcpLeaseTable=dhcpLeaseTable, trapCommandServerDaemon=trapCommandServerDaemon, haReplicationBinding=haReplicationBinding, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTPDaemon=trapTFTPDaemon, dhcpIP=dhcpIP, licenseValid=licenseValid, dnsStats=dnsStats, dhcpPoolSize=dhcpPoolSize, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonis=adonis, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonRunning=dhcpDaemonRunning, haService=haService, dnsStatsNXDomain=dnsStatsNXDomain, ha=ha, lcd=lcd, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapHA=trapHA, dnsStatsReferral=dnsStatsReferral, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCPDaemon=trapDHCPDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemState=systemState, trapSystem=trapSystem, dhcpPoolAlert=dhcpPoolAlert, adonisObjects=adonisObjects, trapReplicationFailure=trapReplicationFailure, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolTable=dhcpPoolTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, licenseExpiry=licenseExpiry, dhcpStats=dhcpStats, trapDHCP=trapDHCP, systemDaemon=systemDaemon, system=system, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEntry=dhcpPoolEntry, dnsStatsFailure=dnsStatsFailure, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseHostname=dhcpLeaseHostname)
