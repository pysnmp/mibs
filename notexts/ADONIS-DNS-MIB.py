...
Bits, Counter64, IpAddress, ModuleIdentity, Counter32, TimeTicks, iso, ObjectIdentity, NotificationType, Gauge32, Integer32, enterprises, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Counter64", "IpAddress", "ModuleIdentity", "Counter32", "TimeTicks", "iso", "ObjectIdentity", "NotificationType", "Gauge32", "Integer32", "enterprises", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemonRunning=tftpDaemonRunning, dhcpConfig=dhcpConfig, adonisTraps=adonisTraps, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpStats=dhcpStats, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, lcdDaemon=lcdDaemon, trapDHCPDaemon=trapDHCPDaemon, haServiceNodeType=haServiceNodeType, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemonRunning=commandServerDaemonRunning, dnsStats=dnsStats, dnsStatsReferral=dnsStatsReferral, dhcpSubnetSize=dhcpSubnetSize, licenseExpiry=licenseExpiry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsNXRRSet=dnsStatsNXRRSet, systemState=systemState, dhcp=dhcp, dhcpFixedIP=dhcpFixedIP, dhcpLeaseEntry=dhcpLeaseEntry, trapDNSDaemon=trapDNSDaemon, dhcpFixedIPTable=dhcpFixedIPTable, haReplicationBinding=haReplicationBinding, ha=ha, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, trapSystem=trapSystem, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapHAServiceFailOver=trapHAServiceFailOver, trapSystemDaemon=trapSystemDaemon, dhcpSubnetMask=dhcpSubnetMask, system=system, licenseValid=licenseValid, commandServerDaemon=commandServerDaemon, trapCommandServer=trapCommandServer, dhcpPoolTable=dhcpPoolTable, dnsDaemon=dnsDaemon, dhcpIP=dhcpIP, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsSuccess=dnsStatsSuccess, trapDHCP=trapDHCP, trapReplication=trapReplication, dhcpPoolEndIP=dhcpPoolEndIP, adonis=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, haService=haService, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTPDaemon=trapTFTPDaemon, tftp=tftp, systemDaemon=systemDaemon, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonRunning=dhcpDaemonRunning, trapHA=trapHA, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSize=dhcpPoolSize, dhcpLeaseHostname=dhcpLeaseHostname, PYSNMP_MODULE_ID=adonis, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsRecursion=dnsStatsRecursion, haServiceRunning=haServiceRunning, dnsStatsFailure=dnsStatsFailure, dhcpPoolUsed=dhcpPoolUsed, dns=dns, dhcpLeaseEndTime=dhcpLeaseEndTime, adonisObjects=adonisObjects, lcd=lcd, dhcpFailOverState=dhcpFailOverState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplicationFailure=trapReplicationFailure, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftpDaemon=tftpDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemon=dhcpDaemon, commandServer=commandServer, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetTable=dhcpSubnetTable)
