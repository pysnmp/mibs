...
MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Counter64, enterprises, Bits, ModuleIdentity, ObjectIdentity, MibIdentifier, iso, NotificationType, IpAddress, Unsigned32, Gauge32, TimeTicks, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Counter64", "enterprises", "Bits", "ModuleIdentity", "ObjectIdentity", "MibIdentifier", "iso", "NotificationType", "IpAddress", "Unsigned32", "Gauge32", "TimeTicks", "Counter32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStats=dnsStats, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPTable=dhcpFixedIPTable, systemDaemon=systemDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolSize=dhcpPoolSize, trapDHCP=trapDHCP, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTPDaemon=trapTFTPDaemon, dhcp=dhcp, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetSize=dhcpSubnetSize, trapDNS=trapDNS, dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcd=lcd, haReplicationBinding=haReplicationBinding, dhcpPoolEntry=dhcpPoolEntry, haService=haService, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, trapSystem=trapSystem, trapReplication=trapReplication, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHostname=dhcpLeaseHostname, licenseValid=licenseValid, lcdDaemon=lcdDaemon, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, dhcpIP=dhcpIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolTable=dhcpPoolTable, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemon=dhcpDaemon, systemState=systemState, adonis=adonis, tftpDaemon=tftpDaemon, haServiceRunning=haServiceRunning, trapSystemDaemon=trapSystemDaemon, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, trapTFTP=trapTFTP, dhcpDaemonRunning=dhcpDaemonRunning, commandServer=commandServer, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseExpiry=licenseExpiry, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemonRunning=tftpDaemonRunning, ha=ha, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFailOverState=dhcpFailOverState, tftp=tftp, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseEntry=dhcpLeaseEntry, trapDNSDaemon=trapDNSDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetIP=dhcpSubnetIP, trapCommandServerDaemon=trapCommandServerDaemon, adonisTraps=adonisTraps, dnsStatsFailure=dnsStatsFailure, trapReplicationFailure=trapReplicationFailure, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemon=dnsDaemon, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, system=system, dns=dns, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, dhcpConfig=dhcpConfig, dnsStatsReferral=dnsStatsReferral, haServiceNodeType=haServiceNodeType, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpStats=dhcpStats, commandServerDaemon=commandServerDaemon)
