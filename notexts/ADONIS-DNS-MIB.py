...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetSize=dhcpSubnetSize, trapDNS=trapDNS, haServiceRunning=haServiceRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHostname=dhcpLeaseHostname, trapHA=trapHA, dhcpSubnetEntry=dhcpSubnetEntry, systemDaemon=systemDaemon, dhcpLeaseTable=dhcpLeaseTable, commandServer=commandServer, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetIP=dhcpSubnetIP, dnsStats=dnsStats, dhcpStats=dhcpStats, system=system, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, PYSNMP_MODULE_ID=adonis, haReplicationBinding=haReplicationBinding, tftpDaemon=tftpDaemon, dns=dns, dhcpLeaseEndTime=dhcpLeaseEndTime, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsFailure=dnsStatsFailure, dnsDaemon=dnsDaemon, trapDHCPDaemon=trapDHCPDaemon, lcdDaemon=lcdDaemon, trapSystem=trapSystem, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetAlert=dhcpSubnetAlert, dhcpConfig=dhcpConfig, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, licenseExpiry=licenseExpiry, dhcpPoolAlert=dhcpPoolAlert, trapReplicationFailure=trapReplicationFailure, commandServerDaemonRunning=commandServerDaemonRunning, adonis=adonis, adonisObjects=adonisObjects, trapReplication=trapReplication, adonisTraps=adonisTraps, dhcpPoolSize=dhcpPoolSize, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapCommandServerDaemon=trapCommandServerDaemon, dhcp=dhcp, dhcpIP=dhcpIP, dhcpPoolEndIP=dhcpPoolEndIP, haService=haService, trapDHCP=trapDHCP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, licenseValid=licenseValid, lcd=lcd, trapTFTP=trapTFTP, trapCommandServer=trapCommandServer, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPEntry=dhcpFixedIPEntry, tftpDaemonRunning=tftpDaemonRunning, ha=ha, systemState=systemState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEntry=dhcpPoolEntry, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNSDaemon=trapDNSDaemon, haServiceNodeType=haServiceNodeType, trapSystemDaemon=trapSystemDaemon, dhcpFailOverState=dhcpFailOverState, dhcpPoolTable=dhcpPoolTable, dnsStatsReferral=dnsStatsReferral, tftp=tftp, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemon=dhcpDaemon)
