...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpStats=dhcpStats, trapDNSDaemon=trapDNSDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonisTraps=adonisTraps, haServiceRunning=haServiceRunning, dhcpSubnetSize=dhcpSubnetSize, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseEntry=dhcpLeaseEntry, dnsStats=dnsStats, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSize=dhcpPoolSize, trapCommandServerDaemon=trapCommandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonRunning=dhcpDaemonRunning, trapReplication=trapReplication, dhcpLeaseEndTime=dhcpLeaseEndTime, system=system, lcdDaemon=lcdDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dns=dns, dhcpFailOverState=dhcpFailOverState, dhcpFixedIP=dhcpFixedIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetEntry=dhcpSubnetEntry, dhcpIP=dhcpIP, haReplicationBinding=haReplicationBinding, trapDNS=trapDNS, dhcpPoolStartIP=dhcpPoolStartIP, systemState=systemState, dhcpConfig=dhcpConfig, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolUsed=dhcpPoolUsed, haService=haService, dhcpDaemon=dhcpDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftpDaemon=tftpDaemon, dhcpSubnetMask=dhcpSubnetMask, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolTable=dhcpPoolTable, dhcp=dhcp, trapDHCP=trapDHCP, dhcpFixedIPTable=dhcpFixedIPTable, trapSystemDaemon=trapSystemDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEntry=dhcpPoolEntry, trapCommandServer=trapCommandServer, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcd=lcd, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, adonisObjects=adonisObjects, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEndIP=dhcpPoolEndIP, PYSNMP_MODULE_ID=adonis, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, ha=ha, commandServer=commandServer, trapSystem=trapSystem, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsSuccess=dnsStatsSuccess, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsFailure=dnsStatsFailure, trapReplicationFailure=trapReplicationFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonRunning=dnsDaemonRunning, haServiceNodeType=haServiceNodeType, trapDHCPDaemon=trapDHCPDaemon, systemDaemon=systemDaemon, licenseExpiry=licenseExpiry, adonis=adonis, tftp=tftp, trapHA=trapHA, dnsDaemon=dnsDaemon, trapTFTPDaemon=trapTFTPDaemon, trapTFTP=trapTFTP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, licenseValid=licenseValid, tftpDaemonRunning=tftpDaemonRunning)
