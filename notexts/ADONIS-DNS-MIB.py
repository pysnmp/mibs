...
NotificationType, TimeTicks, IpAddress, Counter32, ObjectIdentity, iso, Unsigned32, Integer32, enterprises, MibIdentifier, ModuleIdentity, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "TimeTicks", "IpAddress", "Counter32", "ObjectIdentity", "iso", "Unsigned32", "Integer32", "enterprises", "MibIdentifier", "ModuleIdentity", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemon=dhcpDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolAlert=dhcpPoolAlert, tftpDaemon=tftpDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEntry=dhcpLeaseEntry, tftp=tftp, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNSDaemon=trapDNSDaemon, trapDHCPDaemon=trapDHCPDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapReplicationFailure=trapReplicationFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystem=trapSystem, dhcpStats=dhcpStats, dhcpConfig=dhcpConfig, trapCommandServer=trapCommandServer, ha=ha, systemDaemon=systemDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetMask=dhcpSubnetMask, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemon=commandServerDaemon, dhcpFixedIP=dhcpFixedIP, licenseExpiry=licenseExpiry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haService=haService, lcdDaemon=lcdDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseHostname=dhcpLeaseHostname, commandServerDaemonRunning=commandServerDaemonRunning, dhcp=dhcp, PYSNMP_MODULE_ID=adonis, dnsStatsReferral=dnsStatsReferral, dhcpLeaseEndTime=dhcpLeaseEndTime, lcd=lcd, trapTFTP=trapTFTP, haReplicationBinding=haReplicationBinding, dhcpFailOverState=dhcpFailOverState, dhcpDaemonRunning=dhcpDaemonRunning, trapSystemDaemon=trapSystemDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, adonisTraps=adonisTraps, trapHA=trapHA, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDNS=trapDNS, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dns=dns, dnsStats=dnsStats, dhcpLeaseBindState=dhcpLeaseBindState, trapDHCP=trapDHCP, dhcpSubnetUsed=dhcpSubnetUsed, trapHAServiceFailOver=trapHAServiceFailOver, haServiceRunning=haServiceRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemon=dnsDaemon, dnsStatsFailure=dnsStatsFailure, haServiceNodeType=haServiceNodeType, dhcpPoolUsed=dhcpPoolUsed, commandServer=commandServer, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, licenseValid=licenseValid, systemState=systemState, dhcpIP=dhcpIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolSize=dhcpPoolSize, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsSuccess=dnsStatsSuccess, trapReplication=trapReplication, adonis=adonis, dhcpLeaseTable=dhcpLeaseTable, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetEntry=dhcpSubnetEntry, adonisObjects=adonisObjects, dhcpPoolTable=dhcpPoolTable, system=system, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp)
