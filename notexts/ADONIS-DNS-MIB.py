...
ObjectIdentity, Unsigned32, MibIdentifier, Counter64, IpAddress, Bits, iso, Integer32, Counter32, ModuleIdentity, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Gauge32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Unsigned32", "MibIdentifier", "Counter64", "IpAddress", "Bits", "iso", "Integer32", "Counter32", "ModuleIdentity", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Gauge32", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemState=systemState, trapTFTP=trapTFTP, trapReplicationFailure=trapReplicationFailure, trapSystemDaemon=trapSystemDaemon, trapTFTPDaemon=trapTFTPDaemon, haServiceRunning=haServiceRunning, haReplicationBinding=haReplicationBinding, trapDNSDaemon=trapDNSDaemon, haServiceNodeType=haServiceNodeType, dnsStats=dnsStats, commandServer=commandServer, haService=haService, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, ha=ha, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpStats=dhcpStats, dhcp=dhcp, adonisTraps=adonisTraps, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHostname=dhcpLeaseHostname, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFailOverState=dhcpFailOverState, lcdDaemon=lcdDaemon, systemDaemon=systemDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapHA=trapHA, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXDomain=dnsStatsNXDomain, tftp=tftp, dhcpConfig=dhcpConfig, adonis=adonis, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServer=trapCommandServer, tftpDaemonRunning=tftpDaemonRunning, lcd=lcd, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetAlert=dhcpSubnetAlert, trapSystem=trapSystem, dhcpPoolSize=dhcpPoolSize, dhcpSubnetUsed=dhcpSubnetUsed, licenseExpiry=licenseExpiry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseValid=licenseValid, dns=dns, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCP=trapDHCP, dhcpSubnetMask=dhcpSubnetMask, dhcpIP=dhcpIP, trapDHCPDaemon=trapDHCPDaemon, dnsDaemon=dnsDaemon, dnsStatsReferral=dnsStatsReferral, dhcpPoolEntry=dhcpPoolEntry, tftpDaemon=tftpDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisObjects=adonisObjects, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonRunning=dnsDaemonRunning, dnsStatsFailure=dnsStatsFailure, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetIP=dhcpSubnetIP, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemon=dhcpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapReplication=trapReplication, trapDNS=trapDNS, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolTable=dhcpPoolTable, dhcpFixedIP=dhcpFixedIP, system=system, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, PYSNMP_MODULE_ID=adonis, trapCommandServerDaemon=trapCommandServerDaemon)
