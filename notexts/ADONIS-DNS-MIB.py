...
ObjectIdentity, Gauge32, MibIdentifier, Integer32, enterprises, Counter64, Bits, TimeTicks, IpAddress, NotificationType, Counter32, Unsigned32, iso, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Gauge32", "MibIdentifier", "Integer32", "enterprises", "Counter64", "Bits", "TimeTicks", "IpAddress", "NotificationType", "Counter32", "Unsigned32", "iso", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonisObjects=adonisObjects, dhcpPoolSize=dhcpPoolSize, tftpDaemonRunning=tftpDaemonRunning, trapTFTP=trapTFTP, trapCommandServer=trapCommandServer, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcp=dhcp, dhcpLeaseHostname=dhcpLeaseHostname, dnsStats=dnsStats, trapDNSDaemon=trapDNSDaemon, trapHAServiceFailOver=trapHAServiceFailOver, trapHA=trapHA, trapTFTPDaemon=trapTFTPDaemon, dnsStatsNXDomain=dnsStatsNXDomain, adonisTraps=adonisTraps, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetTable=dhcpSubnetTable, commandServer=commandServer, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemon=dhcpDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseExpiry=licenseExpiry, PYSNMP_MODULE_ID=adonis, dhcpStats=dhcpStats, dhcpLeaseEntry=dhcpLeaseEntry, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemon=dnsDaemon, trapDHCP=trapDHCP, adonis=adonis, dhcpIP=dhcpIP, ha=ha, dns=dns, trapDHCPDaemon=trapDHCPDaemon, haReplicationBinding=haReplicationBinding, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcdDaemon=lcdDaemon, systemDaemon=systemDaemon, dnsStatsFailure=dnsStatsFailure, dhcpFixedIP=dhcpFixedIP, dhcpPoolTable=dhcpPoolTable, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonRunning=dnsDaemonRunning, system=system, trapSystem=trapSystem, lcd=lcd, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, tftpDaemon=tftpDaemon, haService=haService, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, systemState=systemState, trapDNS=trapDNS, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFailOverState=dhcpFailOverState, dhcpConfig=dhcpConfig, haServiceRunning=haServiceRunning, commandServerDaemon=commandServerDaemon, haServiceNodeType=haServiceNodeType, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftp=tftp, licenseValid=licenseValid, dnsStatsReferral=dnsStatsReferral, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, trapReplicationFailure=trapReplicationFailure, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolEndIP=dhcpPoolEndIP, trapReplication=trapReplication)
