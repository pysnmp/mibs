...
ModuleIdentity, Gauge32, iso, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, enterprises, Counter32, IpAddress, Unsigned32, Integer32, Bits, TimeTicks, NotificationType, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Gauge32", "iso", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "enterprises", "Counter32", "IpAddress", "Unsigned32", "Integer32", "Bits", "TimeTicks", "NotificationType", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetMask=dhcpSubnetMask, trapDNSDaemon=trapDNSDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapSystemDaemon=trapSystemDaemon, lcdDaemon=lcdDaemon, dhcpFixedIP=dhcpFixedIP, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemon=commandServerDaemon, dhcpStats=dhcpStats, dnsDaemon=dnsDaemon, PYSNMP_MODULE_ID=adonis, dhcpSubnetUsed=dhcpSubnetUsed, adonisObjects=adonisObjects, licenseValid=licenseValid, systemState=systemState, dns=dns, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, lcd=lcd, dhcpFailOverState=dhcpFailOverState, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolAlert=dhcpPoolAlert, trapCommandServer=trapCommandServer, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCP=trapDHCP, system=system, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsRecursion=dnsStatsRecursion, tftp=tftp, dhcpPoolStartIP=dhcpPoolStartIP, trapReplication=trapReplication, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolTable=dhcpPoolTable, dhcpSubnetTable=dhcpSubnetTable, haServiceRunning=haServiceRunning, adonisTraps=adonisTraps, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNS=trapDNS, trapDHCPDaemon=trapDHCPDaemon, haService=haService, dhcpIP=dhcpIP, tftpDaemon=tftpDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapTFTP=trapTFTP, dhcpPoolEntry=dhcpPoolEntry, dhcp=dhcp, trapReplicationFailure=trapReplicationFailure, haServiceNodeType=haServiceNodeType, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapHAServiceFailOver=trapHAServiceFailOver, haReplicationBinding=haReplicationBinding, dhcpLeaseBindState=dhcpLeaseBindState, licenseExpiry=licenseExpiry, dnsStatsSuccess=dnsStatsSuccess, trapHA=trapHA, dhcpDaemonRunning=dhcpDaemonRunning, dhcpConfig=dhcpConfig, dnsStats=dnsStats, ha=ha, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsReferral=dnsStatsReferral, adonis=adonis, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSize=dhcpPoolSize, dhcpDaemon=dhcpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIPEntry=dhcpFixedIPEntry, systemDaemon=systemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapSystem=trapSystem, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsFailure=dnsStatsFailure, commandServer=commandServer, dhcpFixedIPTable=dhcpFixedIPTable)
