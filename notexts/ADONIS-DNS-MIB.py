...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", lcd=lcd, dhcpDaemonRunning=dhcpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolAlert=dhcpPoolAlert, PYSNMP_MODULE_ID=adonis, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIP=dhcpFixedIP, dnsStatsReferral=dnsStatsReferral, dnsDaemonRunning=dnsDaemonRunning, dhcpStats=dhcpStats, adonisObjects=adonisObjects, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dhcpFailOverState=dhcpFailOverState, dhcpPoolEntry=dhcpPoolEntry, haServiceRunning=haServiceRunning, trapSystemDaemon=trapSystemDaemon, trapDHCPDaemon=trapDHCPDaemon, trapCommandServer=trapCommandServer, systemState=systemState, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsNXRRSet=dnsStatsNXRRSet, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetMask=dhcpSubnetMask, trapReplicationFailure=trapReplicationFailure, trapDNSDaemon=trapDNSDaemon, dhcpIP=dhcpIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, trapReplication=trapReplication, dhcpPoolTable=dhcpPoolTable, dhcpPoolEndIP=dhcpPoolEndIP, haServiceNodeType=haServiceNodeType, adonisTraps=adonisTraps, licenseExpiry=licenseExpiry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStats=dnsStats, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDHCP=trapDHCP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolSubnetIP=dhcpPoolSubnetIP, system=system, systemDaemon=systemDaemon, dhcpDaemon=dhcpDaemon, licenseValid=licenseValid, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSize=dhcpPoolSize, commandServerDaemon=commandServerDaemon, haService=haService, dhcpConfig=dhcpConfig, trapSystem=trapSystem, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapTFTPDaemon=trapTFTPDaemon, dnsStatsFailure=dnsStatsFailure, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemon=dnsDaemon, ha=ha, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseHostname=dhcpLeaseHostname, tftp=tftp, trapHA=trapHA, commandServer=commandServer, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetEntry=dhcpSubnetEntry, adonis=adonis, trapTFTP=trapTFTP, dhcp=dhcp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftpDaemon=tftpDaemon, haReplicationBinding=haReplicationBinding, dhcpLeaseStartTime=dhcpLeaseStartTime, dns=dns, dhcpPoolUsed=dhcpPoolUsed, dnsStatsSuccess=dnsStatsSuccess)
