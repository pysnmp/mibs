...
ModuleIdentity, TimeTicks, Integer32, Unsigned32, Counter64, MibIdentifier, Gauge32, IpAddress, ObjectIdentity, Bits, Counter32, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "TimeTicks", "Integer32", "Unsigned32", "Counter64", "MibIdentifier", "Gauge32", "IpAddress", "ObjectIdentity", "Bits", "Counter32", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFixedIP=dhcpFixedIP, trapDNSDaemon=trapDNSDaemon, commandServerDaemon=commandServerDaemon, trapHAServiceFailOver=trapHAServiceFailOver, haService=haService, licenseExpiry=licenseExpiry, dhcp=dhcp, dhcpPoolUsed=dhcpPoolUsed, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, lcd=lcd, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemon=dnsDaemon, trapCommandServerDaemon=trapCommandServerDaemon, adonisTraps=adonisTraps, tftp=tftp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsRecursion=dnsStatsRecursion, trapCommandServer=trapCommandServer, trapHA=trapHA, lcdDaemon=lcdDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNS=trapDNS, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplication=trapReplication, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsFailure=dnsStatsFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTP=trapTFTP, dhcpPoolTable=dhcpPoolTable, licenseValid=licenseValid, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemon=dhcpDaemon, dhcpStats=dhcpStats, dns=dns, dhcpDaemonRunning=dhcpDaemonRunning, system=system, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haServiceRunning=haServiceRunning, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplicationFailure=trapReplicationFailure, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetAlert=dhcpSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, haReplicationBinding=haReplicationBinding, trapTFTPDaemon=trapTFTPDaemon, dnsStats=dnsStats, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceNodeType=haServiceNodeType, adonisObjects=adonisObjects, tftpDaemon=tftpDaemon, commandServer=commandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, systemState=systemState, systemDaemon=systemDaemon, dhcpIP=dhcpIP, trapSystemDaemon=trapSystemDaemon, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolEntry=dhcpPoolEntry, adonis=adonis, trapDHCP=trapDHCP, dhcpSubnetIP=dhcpSubnetIP, dhcpConfig=dhcpConfig, dhcpFailOverState=dhcpFailOverState, dhcpSubnetEntry=dhcpSubnetEntry, trapDHCPDaemon=trapDHCPDaemon, trapSystem=trapSystem, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsReferral=dnsStatsReferral, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetTable=dhcpSubnetTable, ha=ha)
