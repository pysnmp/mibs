...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonRunning=dhcpDaemonRunning, dhcpIP=dhcpIP, trapCommandServer=trapCommandServer, dhcpFailOverState=dhcpFailOverState, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonisObjects=adonisObjects, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haReplicationBinding=haReplicationBinding, dnsDaemon=dnsDaemon, dhcpPoolUsed=dhcpPoolUsed, trapReplication=trapReplication, dnsStatsRecursion=dnsStatsRecursion, adonis=adonis, trapSystem=trapSystem, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolSize=dhcpPoolSize, dhcpPoolStartIP=dhcpPoolStartIP, dnsStats=dnsStats, trapDNSDaemon=trapDNSDaemon, dhcpSubnetEntry=dhcpSubnetEntry, systemDaemon=systemDaemon, trapSystemDaemon=trapSystemDaemon, adonisTraps=adonisTraps, dhcpStats=dhcpStats, haServiceRunning=haServiceRunning, haServiceNodeType=haServiceNodeType, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, lcdDaemon=lcdDaemon, trapDHCP=trapDHCP, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonRunning=dnsDaemonRunning, dhcpConfig=dhcpConfig, tftp=tftp, commandServer=commandServer, trapDNS=trapDNS, tftpDaemon=tftpDaemon, dnsStatsReferral=dnsStatsReferral, lcd=lcd, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetAlert=dhcpSubnetAlert, dhcpFixedIP=dhcpFixedIP, dnsStatsFailure=dnsStatsFailure, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcp=dhcp, licenseExpiry=licenseExpiry, trapHAServiceFailOver=trapHAServiceFailOver, PYSNMP_MODULE_ID=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemon=dhcpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapReplicationFailure=trapReplicationFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolAlert=dhcpPoolAlert, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, commandServerDaemonRunning=commandServerDaemonRunning, haService=haService, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolTable=dhcpPoolTable, licenseValid=licenseValid, ha=ha, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetUsed=dhcpSubnetUsed, dns=dns, systemState=systemState, commandServerDaemon=commandServerDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetIP=dhcpSubnetIP, trapTFTP=trapTFTP, dhcpFixedIPEntry=dhcpFixedIPEntry, trapHA=trapHA, system=system, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseStartTime=dhcpLeaseStartTime)
