...
enterprises, Gauge32, Counter64, Bits, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, iso, Counter32, Integer32, ModuleIdentity, ObjectIdentity, TimeTicks, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Gauge32", "Counter64", "Bits", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "iso", "Counter32", "Integer32", "ModuleIdentity", "ObjectIdentity", "TimeTicks", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemDaemon=systemDaemon, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsFailure=dnsStatsFailure, haReplicationBinding=haReplicationBinding, systemState=systemState, dhcpPoolUsed=dhcpPoolUsed, dhcpIP=dhcpIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSize=dhcpPoolSize, dnsDaemon=dnsDaemon, adonisObjects=adonisObjects, dnsStats=dnsStats, licenseExpiry=licenseExpiry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCP=trapDHCP, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpFailOverState=dhcpFailOverState, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServer=trapCommandServer, trapDNSDaemon=trapDNSDaemon, dhcpSubnetEntry=dhcpSubnetEntry, trapReplicationFailure=trapReplicationFailure, dhcp=dhcp, tftp=tftp, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemonRunning=commandServerDaemonRunning, licenseValid=licenseValid, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftpDaemon=tftpDaemon, PYSNMP_MODULE_ID=adonis, adonisTraps=adonisTraps, trapSystemDaemon=trapSystemDaemon, dnsStatsRecursion=dnsStatsRecursion, trapTFTP=trapTFTP, lcd=lcd, commandServer=commandServer, lcdDaemon=lcdDaemon, haServiceNodeType=haServiceNodeType, dnsStatsSuccess=dnsStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, trapReplication=trapReplication, haService=haService, tftpDaemonRunning=tftpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemon=dhcpDaemon, commandServerDaemon=commandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseHostname=dhcpLeaseHostname, trapHA=trapHA, dhcpSubnetAlert=dhcpSubnetAlert, dns=dns, system=system, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseEntry=dhcpLeaseEntry, haServiceRunning=haServiceRunning, ha=ha, dhcpStats=dhcpStats, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPTable=dhcpFixedIPTable, trapSystem=trapSystem, dhcpConfig=dhcpConfig, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolTable=dhcpPoolTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsReferral=dnsStatsReferral, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, adonis=adonis, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolStartIP=dhcpPoolStartIP)
