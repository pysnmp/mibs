...
ModuleIdentity, Counter64, TimeTicks, Gauge32, IpAddress, ObjectIdentity, Integer32, Unsigned32, iso, NotificationType, MibIdentifier, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Counter64", "TimeTicks", "Gauge32", "IpAddress", "ObjectIdentity", "Integer32", "Unsigned32", "iso", "NotificationType", "MibIdentifier", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsSuccess=dnsStatsSuccess, tftp=tftp, dhcpFixedIP=dhcpFixedIP, dhcpPoolUsed=dhcpPoolUsed, trapDNSDaemon=trapDNSDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEntry=dhcpLeaseEntry, lcdDaemon=lcdDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemon=dnsDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServer=commandServer, dns=dns, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, commandServerDaemon=commandServerDaemon, adonis=adonis, haService=haService, systemDaemon=systemDaemon, trapCommandServerDaemon=trapCommandServerDaemon, haReplicationBinding=haReplicationBinding, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, dnsStatsRecursion=dnsStatsRecursion, systemState=systemState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsReferral=dnsStatsReferral, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTP=trapTFTP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolEntry=dhcpPoolEntry, dhcp=dhcp, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpStats=dhcpStats, licenseExpiry=licenseExpiry, system=system, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStats=dnsStats, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetTable=dhcpSubnetTable, trapSystemDaemon=trapSystemDaemon, dhcpPoolTable=dhcpPoolTable, trapReplicationFailure=trapReplicationFailure, ha=ha, lcd=lcd, PYSNMP_MODULE_ID=adonis, dhcpLeaseStartTime=dhcpLeaseStartTime, adonisObjects=adonisObjects, trapDHCP=trapDHCP, haServiceRunning=haServiceRunning, dhcpIP=dhcpIP, dhcpDaemon=dhcpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceNodeType=haServiceNodeType, adonisTraps=adonisTraps, dhcpFixedIPEntry=dhcpFixedIPEntry, trapCommandServer=trapCommandServer, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSize=dhcpPoolSize, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsFailure=dnsStatsFailure, dhcpFixedIPTable=dhcpFixedIPTable, licenseValid=licenseValid, dhcpFailOverState=dhcpFailOverState, trapSystem=trapSystem, dhcpPoolStartIP=dhcpPoolStartIP, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetEntry=dhcpSubnetEntry, trapDNS=trapDNS, dhcpConfig=dhcpConfig, trapHA=trapHA, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsNXDomain=dnsStatsNXDomain, tftpDaemon=tftpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftpDaemonRunning=tftpDaemonRunning, trapReplication=trapReplication)
