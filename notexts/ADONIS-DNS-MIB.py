...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplication=trapReplication, dhcpConfig=dhcpConfig, PYSNMP_MODULE_ID=adonis, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, dnsDaemon=dnsDaemon, haServiceNodeType=haServiceNodeType, dhcpDaemon=dhcpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolSize=dhcpPoolSize, dnsStatsReferral=dnsStatsReferral, systemState=systemState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, adonisTraps=adonisTraps, trapHA=trapHA, dnsStatsNXDomain=dnsStatsNXDomain, licenseExpiry=licenseExpiry, dhcpSubnetEntry=dhcpSubnetEntry, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetSize=dhcpSubnetSize, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolTable=dhcpPoolTable, dnsStatsFailure=dnsStatsFailure, dnsStats=dnsStats, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNSDaemon=trapDNSDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCPDaemon=trapDHCPDaemon, dhcpFailOverState=dhcpFailOverState, trapReplicationFailure=trapReplicationFailure, trapDNS=trapDNS, dhcpFixedIPTable=dhcpFixedIPTable, systemDaemon=systemDaemon, dns=dns, dnsStatsRecursion=dnsStatsRecursion, system=system, haServiceRunning=haServiceRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServer=trapCommandServer, tftp=tftp, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemon=tftpDaemon, trapSystem=trapSystem, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonisObjects=adonisObjects, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseEntry=dhcpLeaseEntry, lcdDaemon=lcdDaemon, lcd=lcd, commandServer=commandServer, haReplicationBinding=haReplicationBinding, dhcpPoolStartIP=dhcpPoolStartIP, trapDHCP=trapDHCP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTP=trapTFTP, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetMask=dhcpSubnetMask, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, ha=ha, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, licenseValid=licenseValid, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIP=dhcpFixedIP, dhcp=dhcp, dhcpSubnetUsed=dhcpSubnetUsed, adonis=adonis, dhcpLeaseTable=dhcpLeaseTable, dhcpIP=dhcpIP, dhcpStats=dhcpStats, commandServerDaemon=commandServerDaemon)
