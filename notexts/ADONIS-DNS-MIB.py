...
Integer32, IpAddress, NotificationType, Unsigned32, Bits, enterprises, Gauge32, ModuleIdentity, MibIdentifier, Counter64, iso, TimeTicks, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "IpAddress", "NotificationType", "Unsigned32", "Bits", "enterprises", "Gauge32", "ModuleIdentity", "MibIdentifier", "Counter64", "iso", "TimeTicks", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServerDaemon=commandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolUsed=dhcpPoolUsed, trapTFTP=trapTFTP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpConfig=dhcpConfig, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemon=dnsDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapSystemDaemon=trapSystemDaemon, adonisObjects=adonisObjects, dhcpSubnetSize=dhcpSubnetSize, trapDNS=trapDNS, commandServer=commandServer, dhcpIP=dhcpIP, dhcpPoolAlert=dhcpPoolAlert, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpStats=dhcpStats, dhcpDaemonRunning=dhcpDaemonRunning, licenseValid=licenseValid, trapDNSDaemon=trapDNSDaemon, dhcp=dhcp, adonisTraps=adonisTraps, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStats=dnsStats, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, systemState=systemState, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseEntry=dhcpLeaseEntry, dns=dns, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsSuccess=dnsStatsSuccess, lcdDaemon=lcdDaemon, lcd=lcd, dhcpLeaseTable=dhcpLeaseTable, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolTable=dhcpPoolTable, system=system, dhcpSubnetTable=dhcpSubnetTable, ha=ha, dhcpPoolSize=dhcpPoolSize, dhcpFixedIP=dhcpFixedIP, dhcpDaemon=dhcpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIPTable=dhcpFixedIPTable, trapTFTPDaemon=trapTFTPDaemon, adonis=adonis, dnsStatsReferral=dnsStatsReferral, dhcpSubnetEntry=dhcpSubnetEntry, trapReplication=trapReplication, systemDaemon=systemDaemon, dhcpSubnetIP=dhcpSubnetIP, trapCommandServer=trapCommandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFailOverState=dhcpFailOverState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsFailure=dnsStatsFailure, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolEntry=dhcpPoolEntry, licenseExpiry=licenseExpiry, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haServiceNodeType=haServiceNodeType, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHA=trapHA, PYSNMP_MODULE_ID=adonis, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceRunning=haServiceRunning, trapSystem=trapSystem, haService=haService, dhcpLeaseEndTime=dhcpLeaseEndTime, haReplicationBinding=haReplicationBinding, dnsDaemonRunning=dnsDaemonRunning, tftp=tftp, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftpDaemon=tftpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDHCP=trapDHCP, commandServerDaemonRunning=commandServerDaemonRunning, trapReplicationFailure=trapReplicationFailure, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolStartIP=dhcpPoolStartIP)
