...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDHCPDaemon=trapDHCPDaemon, tftp=tftp, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonRunning=dnsDaemonRunning, licenseValid=licenseValid, dhcpConfig=dhcpConfig, dnsStats=dnsStats, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDNSDaemon=trapDNSDaemon, dnsStatsReferral=dnsStatsReferral, dhcpPoolUsed=dhcpPoolUsed, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDHCP=trapDHCP, commandServerDaemonRunning=commandServerDaemonRunning, PYSNMP_MODULE_ID=adonis, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHostname=dhcpLeaseHostname, trapSystem=trapSystem, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseBindState=dhcpLeaseBindState, trapReplication=trapReplication, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsFailure=dnsStatsFailure, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolAlert=dhcpPoolAlert, tftpDaemon=tftpDaemon, dhcpFailOverState=dhcpFailOverState, dhcpPoolSize=dhcpPoolSize, dhcpStats=dhcpStats, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftpDaemonRunning=tftpDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemon=dhcpDaemon, trapDNS=trapDNS, dhcpLeaseEntry=dhcpLeaseEntry, trapReplicationFailure=trapReplicationFailure, trapTFTP=trapTFTP, haServiceNodeType=haServiceNodeType, dhcp=dhcp, haService=haService, dhcpFixedIPEntry=dhcpFixedIPEntry, systemState=systemState, dhcpPoolTable=dhcpPoolTable, haReplicationBinding=haReplicationBinding, dns=dns, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetMask=dhcpSubnetMask, adonisTraps=adonisTraps, dhcpFixedIP=dhcpFixedIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, system=system, dhcpPoolStartIP=dhcpPoolStartIP, lcdDaemon=lcdDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServer=trapCommandServer, dhcpSubnetSize=dhcpSubnetSize, dnsDaemon=dnsDaemon, trapHAServiceFailOver=trapHAServiceFailOver, commandServer=commandServer, lcd=lcd, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEndTime=dhcpLeaseEndTime, trapSystemDaemon=trapSystemDaemon, ha=ha, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonis=adonis, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXDomain=dnsStatsNXDomain, systemDaemon=systemDaemon, dhcpIP=dhcpIP, haServiceRunning=haServiceRunning, trapHA=trapHA, commandServerDaemon=commandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, licenseExpiry=licenseExpiry, adonisObjects=adonisObjects)
