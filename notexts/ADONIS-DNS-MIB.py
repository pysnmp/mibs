...
enterprises, MibIdentifier, Integer32, iso, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, TimeTicks, Counter32, Unsigned32, Counter64, ModuleIdentity, Bits, IpAddress, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "MibIdentifier", "Integer32", "iso", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "TimeTicks", "Counter32", "Unsigned32", "Counter64", "ModuleIdentity", "Bits", "IpAddress", "ObjectIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolSize=dhcpPoolSize, dhcpLeaseBindState=dhcpLeaseBindState, ha=ha, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, adonis=adonis, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIP=dhcpFixedIP, dhcpPoolTable=dhcpPoolTable, haService=haService, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetSize=dhcpSubnetSize, trapCommandServer=trapCommandServer, tftpDaemonRunning=tftpDaemonRunning, dhcpStats=dhcpStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolUsed=dhcpPoolUsed, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFailOverState=dhcpFailOverState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseExpiry=licenseExpiry, systemState=systemState, dnsStatsReferral=dnsStatsReferral, dnsDaemonRunning=dnsDaemonRunning, dnsDaemon=dnsDaemon, dns=dns, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServer=commandServer, dnsStats=dnsStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceNodeType=haServiceNodeType, dhcpLeaseEntry=dhcpLeaseEntry, dhcpIP=dhcpIP, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTPDaemon=trapTFTPDaemon, dhcp=dhcp, system=system, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystemDaemon=trapSystemDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsSuccess=dnsStatsSuccess, adonisTraps=adonisTraps, commandServerDaemon=commandServerDaemon, PYSNMP_MODULE_ID=adonis, tftpDaemon=tftpDaemon, trapHA=trapHA, dhcpPoolEntry=dhcpPoolEntry, trapSystem=trapSystem, dhcpSubnetTable=dhcpSubnetTable, haServiceRunning=haServiceRunning, haReplicationBinding=haReplicationBinding, tftp=tftp, trapDHCP=trapDHCP, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, systemDaemon=systemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, trapDNS=trapDNS, dhcpDaemon=dhcpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTP=trapTFTP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetMask=dhcpSubnetMask, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetEntry=dhcpSubnetEntry, licenseValid=licenseValid, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapReplication=trapReplication, dhcpPoolAlert=dhcpPoolAlert, lcdDaemon=lcdDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisObjects=adonisObjects, dnsStatsFailure=dnsStatsFailure, dhcpLeaseTable=dhcpLeaseTable, lcd=lcd, trapDNSDaemon=trapDNSDaemon)
