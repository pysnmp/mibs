...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dns=dns, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolSize=dhcpPoolSize, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDHCPDaemon=trapDHCPDaemon, haService=haService, trapCommandServer=trapCommandServer, licenseValid=licenseValid, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsSuccess=dnsStatsSuccess, adonisTraps=adonisTraps, trapTFTP=trapTFTP, lcdDaemon=lcdDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolAlert=dhcpPoolAlert, systemDaemon=systemDaemon, trapReplication=trapReplication, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemon=dhcpDaemon, dhcpConfig=dhcpConfig, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, commandServer=commandServer, ha=ha, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStats=dnsStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, systemState=systemState, trapReplicationFailure=trapReplicationFailure, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHostname=dhcpLeaseHostname, commandServerDaemon=commandServerDaemon, dnsStatsReferral=dnsStatsReferral, haServiceNodeType=haServiceNodeType, haServiceRunning=haServiceRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpIP=dhcpIP, dhcpDaemonRunning=dhcpDaemonRunning, trapHA=trapHA, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, dhcpStats=dhcpStats, dhcp=dhcp, dnsStatsFailure=dnsStatsFailure, dnsDaemon=dnsDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetTable=dhcpSubnetTable, tftpDaemon=tftpDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEntry=dhcpPoolEntry, commandServerDaemonRunning=commandServerDaemonRunning, lcd=lcd, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapSystemDaemon=trapSystemDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, system=system, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, trapSystem=trapSystem, licenseExpiry=licenseExpiry, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolUsed=dhcpPoolUsed, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIP=dhcpFixedIP, trapDNSDaemon=trapDNSDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXDomain=dnsStatsNXDomain, tftp=tftp, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, adonis=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetMask=dhcpSubnetMask, PYSNMP_MODULE_ID=adonis, dhcpFailOverState=dhcpFailOverState, adonisObjects=adonisObjects, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolStartIP=dhcpPoolStartIP)
