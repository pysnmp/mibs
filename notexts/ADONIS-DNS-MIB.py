...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolSubnetIP=dhcpPoolSubnetIP, tftp=tftp, trapReplication=trapReplication, dhcpFixedIPTable=dhcpFixedIPTable, licenseExpiry=licenseExpiry, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsRecursion=dnsStatsRecursion, haServiceNodeType=haServiceNodeType, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcp=dhcp, haReplicationBinding=haReplicationBinding, lcd=lcd, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapSystem=trapSystem, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, system=system, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetMask=dhcpSubnetMask, trapSystemDaemon=trapSystemDaemon, commandServerDaemonRunning=commandServerDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, tftpDaemonRunning=tftpDaemonRunning, trapTFTP=trapTFTP, adonisTraps=adonisTraps, adonisObjects=adonisObjects, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSize=dhcpPoolSize, systemDaemon=systemDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsFailure=dnsStatsFailure, trapHA=trapHA, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServer=commandServer, adonis=adonis, dhcpPoolTable=dhcpPoolTable, trapTFTPDaemon=trapTFTPDaemon, dnsDaemon=dnsDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetTable=dhcpSubnetTable, trapDNSDaemon=trapDNSDaemon, trapReplicationFailure=trapReplicationFailure, dhcpSubnetEntry=dhcpSubnetEntry, ha=ha, dhcpSubnetAlert=dhcpSubnetAlert, dhcpConfig=dhcpConfig, systemState=systemState, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, dnsStatsReferral=dnsStatsReferral, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCP=trapDHCP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpIP=dhcpIP, commandServerDaemon=commandServerDaemon, PYSNMP_MODULE_ID=adonis, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolStartIP=dhcpPoolStartIP, licenseValid=licenseValid, dnsStats=dnsStats, dhcpDaemon=dhcpDaemon, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapCommandServer=trapCommandServer, trapDNS=trapDNS, haServiceRunning=haServiceRunning, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseEndTime=dhcpLeaseEndTime, haService=haService, dhcpStats=dhcpStats, lcdDaemon=lcdDaemon, dhcpPoolEntry=dhcpPoolEntry, dns=dns, dhcpLeaseBindState=dhcpLeaseBindState, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonRunning=dhcpDaemonRunning, tftpDaemon=tftpDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIP=dhcpFixedIP)
