...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseEndTime=dhcpLeaseEndTime, system=system, trapHAServiceFailOver=trapHAServiceFailOver, licenseValid=licenseValid, dhcp=dhcp, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHostname=dhcpLeaseHostname, lcdDaemon=lcdDaemon, haServiceRunning=haServiceRunning, dhcpPoolEntry=dhcpPoolEntry, dnsStatsFailure=dnsStatsFailure, systemDaemon=systemDaemon, adonis=adonis, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetSize=dhcpSubnetSize, dnsDaemon=dnsDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemon=dhcpDaemon, trapSystem=trapSystem, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapReplication=trapReplication, lcd=lcd, dns=dns, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFailOverState=dhcpFailOverState, dnsStatsSuccess=dnsStatsSuccess, ha=ha, dhcpStats=dhcpStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolUsed=dhcpPoolUsed, haService=haService, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsRecursion=dnsStatsRecursion, dhcpConfig=dhcpConfig, trapDNS=trapDNS, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetUsed=dhcpSubnetUsed, haServiceNodeType=haServiceNodeType, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServer=trapCommandServer, trapDHCPDaemon=trapDHCPDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpIP=dhcpIP, commandServerDaemon=commandServerDaemon, adonisObjects=adonisObjects, commandServer=commandServer, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, tftpDaemon=tftpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, haReplicationBinding=haReplicationBinding, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolTable=dhcpPoolTable, PYSNMP_MODULE_ID=adonis, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, systemState=systemState, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystemDaemon=trapSystemDaemon, trapCommandServerDaemon=trapCommandServerDaemon, tftp=tftp, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, trapReplicationFailure=trapReplicationFailure, trapDHCP=trapDHCP, trapHA=trapHA, trapTFTP=trapTFTP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIPTable=dhcpFixedIPTable, licenseExpiry=licenseExpiry, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonRunning=dnsDaemonRunning, trapDNSDaemon=trapDNSDaemon, dnsStats=dnsStats, adonisTraps=adonisTraps, dnsStatsNXDomain=dnsStatsNXDomain)
