...
iso, MibIdentifier, Counter32, Unsigned32, ObjectIdentity, Gauge32, TimeTicks, Bits, Counter64, NotificationType, IpAddress, enterprises, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "MibIdentifier", "Counter32", "Unsigned32", "ObjectIdentity", "Gauge32", "TimeTicks", "Bits", "Counter64", "NotificationType", "IpAddress", "enterprises", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsReferral=dnsStatsReferral, systemState=systemState, trapSystem=trapSystem, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, lcdDaemon=lcdDaemon, haServiceRunning=haServiceRunning, licenseValid=licenseValid, trapTFTP=trapTFTP, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServer=trapCommandServer, dhcpPoolSize=dhcpPoolSize, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseExpiry=licenseExpiry, dnsDaemon=dnsDaemon, dhcpLeaseTable=dhcpLeaseTable, trapDHCP=trapDHCP, dnsStats=dnsStats, adonisTraps=adonisTraps, system=system, trapHA=trapHA, dhcpFixedIP=dhcpFixedIP, tftp=tftp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolTable=dhcpPoolTable, haServiceNodeType=haServiceNodeType, haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemon=dhcpDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseStartTime=dhcpLeaseStartTime, haService=haService, trapReplicationFailure=trapReplicationFailure, adonisObjects=adonisObjects, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplication=trapReplication, dhcpStats=dhcpStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpConfig=dhcpConfig, PYSNMP_MODULE_ID=adonis, commandServer=commandServer, systemDaemon=systemDaemon, dhcpIP=dhcpIP, dhcpSubnetSize=dhcpSubnetSize, dnsStatsFailure=dnsStatsFailure, dhcpPoolUsed=dhcpPoolUsed, trapDNS=trapDNS, dhcpSubnetUsed=dhcpSubnetUsed, adonis=adonis, dhcpLeaseBindState=dhcpLeaseBindState, trapDHCPDaemon=trapDHCPDaemon, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXDomain=dnsStatsNXDomain, dhcp=dhcp, trapSystemDaemon=trapSystemDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseEntry=dhcpLeaseEntry, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFailOverState=dhcpFailOverState, trapTFTPDaemon=trapTFTPDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPEntry=dhcpFixedIPEntry, dns=dns, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, ha=ha, commandServerDaemon=commandServerDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, lcd=lcd, tftpDaemon=tftpDaemon, dhcpLeaseHostname=dhcpLeaseHostname)
