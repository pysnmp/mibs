...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemon=dhcpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, trapDHCP=trapDHCP, commandServerDaemon=commandServerDaemon, dhcpPoolEntry=dhcpPoolEntry, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpFailOverState=dhcpFailOverState, haReplicationBinding=haReplicationBinding, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemDaemon=systemDaemon, trapTFTPDaemon=trapTFTPDaemon, lcd=lcd, trapTFTP=trapTFTP, haServiceRunning=haServiceRunning, trapDNS=trapDNS, adonisTraps=adonisTraps, ha=ha, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, haServiceNodeType=haServiceNodeType, commandServer=commandServer, dhcpSubnetIP=dhcpSubnetIP, adonis=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapCommandServer=trapCommandServer, tftpDaemon=tftpDaemon, dhcpPoolSize=dhcpPoolSize, tftp=tftp, dhcpLeaseHostname=dhcpLeaseHostname, dhcp=dhcp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapSystem=trapSystem, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetMask=dhcpSubnetMask, dnsStats=dnsStats, dhcpIP=dhcpIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServerDaemon=trapCommandServerDaemon, licenseExpiry=licenseExpiry, trapReplicationFailure=trapReplicationFailure, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetUsed=dhcpSubnetUsed, lcdDaemon=lcdDaemon, dnsStatsReferral=dnsStatsReferral, dhcpFixedIP=dhcpFixedIP, dnsDaemon=dnsDaemon, haService=haService, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEntry=dhcpLeaseEntry, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplication=trapReplication, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, licenseValid=licenseValid, PYSNMP_MODULE_ID=adonis, system=system, dhcpPoolTable=dhcpPoolTable, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, tftpDaemonRunning=tftpDaemonRunning, trapHA=trapHA, dnsStatsFailure=dnsStatsFailure, systemState=systemState, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisObjects=adonisObjects, trapDNSDaemon=trapDNSDaemon, dhcpSubnetSize=dhcpSubnetSize, dns=dns, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpStats=dhcpStats)
