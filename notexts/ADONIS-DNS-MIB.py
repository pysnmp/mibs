...
iso, ObjectIdentity, IpAddress, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Gauge32, TimeTicks, MibIdentifier, NotificationType, enterprises, Counter32, Counter64, Bits, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "ObjectIdentity", "IpAddress", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Gauge32", "TimeTicks", "MibIdentifier", "NotificationType", "enterprises", "Counter32", "Counter64", "Bits", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolUsed=dhcpPoolUsed, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseEndTime=dhcpLeaseEndTime, trapSystemDaemon=trapSystemDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolEntry=dhcpPoolEntry, system=system, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, licenseExpiry=licenseExpiry, dhcpFailOverState=dhcpFailOverState, dnsStats=dnsStats, dnsStatsSuccess=dnsStatsSuccess, trapTFTP=trapTFTP, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolStartIP=dhcpPoolStartIP, systemState=systemState, trapReplicationFailure=trapReplicationFailure, dns=dns, dhcpLeaseStartTime=dhcpLeaseStartTime, PYSNMP_MODULE_ID=adonis, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceNodeType=haServiceNodeType, tftpDaemonRunning=tftpDaemonRunning, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haReplicationBinding=haReplicationBinding, tftpDaemon=tftpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, trapSystem=trapSystem, dnsDaemon=dnsDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpIP=dhcpIP, dhcpDaemonRunning=dhcpDaemonRunning, trapHA=trapHA, dnsStatsReferral=dnsStatsReferral, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolSize=dhcpPoolSize, tftp=tftp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplication=trapReplication, dhcpLeaseHostname=dhcpLeaseHostname, ha=ha, dhcpConfig=dhcpConfig, haServiceRunning=haServiceRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisTraps=adonisTraps, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolTable=dhcpPoolTable, haService=haService, dhcpDaemon=dhcpDaemon, dhcpSubnetTable=dhcpSubnetTable, adonis=adonis, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFixedIPTable=dhcpFixedIPTable, trapHAServiceFailOver=trapHAServiceFailOver, dhcpStats=dhcpStats, licenseValid=licenseValid, trapCommandServer=trapCommandServer, dnsStatsFailure=dnsStatsFailure, lcd=lcd, dhcpFixedIPEntry=dhcpFixedIPEntry, lcdDaemon=lcdDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolAlert=dhcpPoolAlert, trapDNSDaemon=trapDNSDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcp=dhcp, trapDHCP=trapDHCP, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNS=trapDNS, dhcpFixedIP=dhcpFixedIP, commandServer=commandServer, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetSize=dhcpSubnetSize, systemDaemon=systemDaemon, dhcpLeaseTable=dhcpLeaseTable, adonisObjects=adonisObjects)
