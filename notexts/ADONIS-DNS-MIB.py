...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetIP=dhcpSubnetIP, systemState=systemState, dhcpStats=dhcpStats, licenseExpiry=licenseExpiry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServer=commandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsReferral=dnsStatsReferral, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemon=dhcpDaemon, trapDHCP=trapDHCP, haServiceNodeType=haServiceNodeType, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTPDaemon=trapTFTPDaemon, dhcp=dhcp, dhcpSubnetAlert=dhcpSubnetAlert, dhcpIP=dhcpIP, tftp=tftp, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServerDaemon=commandServerDaemon, trapSystem=trapSystem, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonRunning=dnsDaemonRunning, dnsStatsFailure=dnsStatsFailure, PYSNMP_MODULE_ID=adonis, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStats=dnsStats, adonisTraps=adonisTraps, dhcpPoolTable=dhcpPoolTable, dhcpSubnetSize=dhcpSubnetSize, haServiceRunning=haServiceRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDNS=trapDNS, dhcpPoolUsed=dhcpPoolUsed, system=system, dnsDaemon=dnsDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, ha=ha, dhcpConfig=dhcpConfig, dhcpPoolAlert=dhcpPoolAlert, tftpDaemonRunning=tftpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, licenseValid=licenseValid, haService=haService, adonis=adonis, lcd=lcd, trapReplication=trapReplication, dhcpSubnetMask=dhcpSubnetMask, dnsStatsRecursion=dnsStatsRecursion, lcdDaemon=lcdDaemon, trapHAServiceFailOver=trapHAServiceFailOver, haReplicationBinding=haReplicationBinding, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolStartIP=dhcpPoolStartIP, tftpDaemon=tftpDaemon, trapHA=trapHA, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapReplicationFailure=trapReplicationFailure, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServer=trapCommandServer, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIP=dhcpFixedIP, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFailOverState=dhcpFailOverState, dhcpFixedIPEntry=dhcpFixedIPEntry, adonisObjects=adonisObjects, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsNXDomain=dnsStatsNXDomain, trapTFTP=trapTFTP, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDHCPDaemon=trapDHCPDaemon, systemDaemon=systemDaemon, dns=dns)
