...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsFailure=dnsStatsFailure, trapSystem=trapSystem, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, dhcpFixedIP=dhcpFixedIP, trapHAServiceFailOver=trapHAServiceFailOver, dhcpConfig=dhcpConfig, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapCommandServerDaemon=trapCommandServerDaemon, dhcpStats=dhcpStats, adonisTraps=adonisTraps, dhcpSubnetIP=dhcpSubnetIP, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemon=commandServerDaemon, dhcpFixedIPTable=dhcpFixedIPTable, systemState=systemState, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, haService=haService, dhcpPoolTable=dhcpPoolTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDHCP=trapDHCP, PYSNMP_MODULE_ID=adonis, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolUsed=dhcpPoolUsed, trapSystemDaemon=trapSystemDaemon, dhcpPoolSize=dhcpPoolSize, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsReferral=dnsStatsReferral, adonisObjects=adonisObjects, dhcpIP=dhcpIP, haServiceRunning=haServiceRunning, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, tftp=tftp, adonis=adonis, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemon=dhcpDaemon, licenseValid=licenseValid, dhcpFailOverState=dhcpFailOverState, dhcpPoolAlert=dhcpPoolAlert, trapReplication=trapReplication, trapReplicationFailure=trapReplicationFailure, dnsDaemon=dnsDaemon, ha=ha, dhcp=dhcp, trapDNS=trapDNS, dns=dns, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, systemDaemon=systemDaemon, trapHA=trapHA, haReplicationBinding=haReplicationBinding, dnsStats=dnsStats, trapTFTP=trapTFTP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetUsed=dhcpSubnetUsed, commandServer=commandServer, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, lcdDaemon=lcdDaemon, dhcpLeaseBindState=dhcpLeaseBindState, haServiceNodeType=haServiceNodeType, lcd=lcd, dnsStatsRecursion=dnsStatsRecursion, system=system, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, trapCommandServer=trapCommandServer, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCPDaemon=trapDHCPDaemon)
