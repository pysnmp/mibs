...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, trapCommandServerDaemon=trapCommandServerDaemon, haServiceNodeType=haServiceNodeType, ha=ha, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, tftp=tftp, dns=dns, dhcpSubnetEntry=dhcpSubnetEntry, trapReplicationFailure=trapReplicationFailure, system=system, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemonRunning=commandServerDaemonRunning, licenseExpiry=licenseExpiry, dnsStatsNXRRSet=dnsStatsNXRRSet, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemon=dnsDaemon, adonisTraps=adonisTraps, trapTFTP=trapTFTP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServer=commandServer, dhcpLeaseTable=dhcpLeaseTable, trapDNSDaemon=trapDNSDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolTable=dhcpPoolTable, trapDHCP=trapDHCP, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, trapDNS=trapDNS, haService=haService, commandServerDaemon=commandServerDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, dnsStatsRecursion=dnsStatsRecursion, dnsStatsReferral=dnsStatsReferral, trapSystem=trapSystem, dhcpFixedIP=dhcpFixedIP, licenseValid=licenseValid, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haReplicationBinding=haReplicationBinding, dhcp=dhcp, adonis=adonis, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonRunning=dnsDaemonRunning, systemDaemon=systemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetMask=dhcpSubnetMask, trapReplication=trapReplication, trapCommandServer=trapCommandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHA=trapHA, dhcpConfig=dhcpConfig, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisObjects=adonisObjects, dhcpPoolEntry=dhcpPoolEntry, dnsStatsFailure=dnsStatsFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonRunning=dhcpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dhcpStats=dhcpStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolSize=dhcpPoolSize, haServiceRunning=haServiceRunning, lcdDaemon=lcdDaemon, dhcpPoolEndIP=dhcpPoolEndIP, systemState=systemState, trapSystemDaemon=trapSystemDaemon, dnsStats=dnsStats, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseBindState=dhcpLeaseBindState, trapTFTPDaemon=trapTFTPDaemon, lcd=lcd, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemon=dhcpDaemon, dhcpLeaseHostname=dhcpLeaseHostname, tftpDaemon=tftpDaemon)
