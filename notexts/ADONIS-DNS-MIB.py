...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", licenseValid=licenseValid, dhcpIP=dhcpIP, dhcpFailOverState=dhcpFailOverState, systemDaemon=systemDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseHostname=dhcpLeaseHostname, haService=haService, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolUsed=dhcpPoolUsed, lcdDaemon=lcdDaemon, trapSystem=trapSystem, dhcpPoolEntry=dhcpPoolEntry, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStats=dnsStats, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapCommandServerDaemon=trapCommandServerDaemon, system=system, commandServerDaemon=commandServerDaemon, dhcpFixedIP=dhcpFixedIP, tftp=tftp, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, haServiceRunning=haServiceRunning, dnsStatsReferral=dnsStatsReferral, trapCommandServer=trapCommandServer, commandServer=commandServer, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpConfig=dhcpConfig, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, adonisTraps=adonisTraps, trapReplicationFailure=trapReplicationFailure, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftpDaemon=tftpDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, trapSystemDaemon=trapSystemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemon=dnsDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSize=dhcpPoolSize, trapHAServiceFailOver=trapHAServiceFailOver, haReplicationBinding=haReplicationBinding, dnsDaemonRunning=dnsDaemonRunning, haServiceNodeType=haServiceNodeType, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolAlert=dhcpPoolAlert, trapReplication=trapReplication, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonis=adonis, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemon=dhcpDaemon, systemState=systemState, trapTFTPDaemon=trapTFTPDaemon, ha=ha, dhcpPoolTable=dhcpPoolTable, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsFailure=dnsStatsFailure, trapTFTP=trapTFTP, licenseExpiry=licenseExpiry, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, PYSNMP_MODULE_ID=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dns=dns, dnsStatsNXDomain=dnsStatsNXDomain, trapDHCP=trapDHCP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDNSDaemon=trapDNSDaemon, lcd=lcd, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapHA=trapHA, dhcpStats=dhcpStats, trapDNS=trapDNS, dhcp=dhcp)
