...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemState=systemState, licenseValid=licenseValid, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStats=dnsStats, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolAlert=dhcpPoolAlert, trapReplication=trapReplication, trapSystemDaemon=trapSystemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemon=dnsDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, systemDaemon=systemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, trapDNS=trapDNS, trapDHCP=trapDHCP, tftpDaemonRunning=tftpDaemonRunning, trapHA=trapHA, adonisObjects=adonisObjects, lcdDaemon=lcdDaemon, dhcpDaemon=dhcpDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, adonis=adonis, haReplicationBinding=haReplicationBinding, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSize=dhcpPoolSize, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIP=dhcpFixedIP, dhcpPoolEndIP=dhcpPoolEndIP, adonisTraps=adonisTraps, commandServer=commandServer, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTable=dhcpLeaseTable, dhcpConfig=dhcpConfig, system=system, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDHCPDaemon=trapDHCPDaemon, tftp=tftp, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServer=trapCommandServer, trapTFTPDaemon=trapTFTPDaemon, tftpDaemon=tftpDaemon, haServiceRunning=haServiceRunning, dhcpPoolTable=dhcpPoolTable, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseEndTime=dhcpLeaseEndTime, dns=dns, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTP=trapTFTP, dhcpIP=dhcpIP, haServiceNodeType=haServiceNodeType, dhcpSubnetAlert=dhcpSubnetAlert, commandServerDaemon=commandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, PYSNMP_MODULE_ID=adonis, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsFailure=dnsStatsFailure, dhcpDaemonRunning=dhcpDaemonRunning, trapDNSDaemon=trapDNSDaemon, dhcpFailOverState=dhcpFailOverState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcd=lcd, dhcp=dhcp, haService=haService, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseBindState=dhcpLeaseBindState, trapSystem=trapSystem, dnsStatsReferral=dnsStatsReferral, dhcpStats=dhcpStats, ha=ha)
