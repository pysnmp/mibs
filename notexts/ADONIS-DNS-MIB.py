...
MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, TimeTicks, Counter32, Integer32, IpAddress, ModuleIdentity, NotificationType, Bits, enterprises, Gauge32, iso, Unsigned32, Counter64, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "TimeTicks", "Counter32", "Integer32", "IpAddress", "ModuleIdentity", "NotificationType", "Bits", "enterprises", "Gauge32", "iso", "Unsigned32", "Counter64", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", licenseExpiry=licenseExpiry, adonis=adonis, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dns=dns, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseEntry=dhcpLeaseEntry, trapSystem=trapSystem, tftpDaemonRunning=tftpDaemonRunning, trapDNSDaemon=trapDNSDaemon, dhcpIP=dhcpIP, dhcpFixedIPEntry=dhcpFixedIPEntry, trapCommandServer=trapCommandServer, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haService=haService, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTable=dhcpLeaseTable, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolAlert=dhcpPoolAlert, trapDNS=trapDNS, commandServerDaemon=commandServerDaemon, trapHA=trapHA, trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonisObjects=adonisObjects, dhcpPoolStartIP=dhcpPoolStartIP, lcd=lcd, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsReferral=dnsStatsReferral, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsFailure=dnsStatsFailure, haReplicationBinding=haReplicationBinding, systemState=systemState, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemon=dhcpDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIP=dhcpFixedIP, trapReplication=trapReplication, haServiceRunning=haServiceRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFailOverState=dhcpFailOverState, systemDaemon=systemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonRunning=dhcpDaemonRunning, commandServer=commandServer, tftpDaemon=tftpDaemon, adonisTraps=adonisTraps, PYSNMP_MODULE_ID=adonis, trapReplicationFailure=trapReplicationFailure, dhcpConfig=dhcpConfig, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetSize=dhcpSubnetSize, lcdDaemon=lcdDaemon, dhcpSubnetIP=dhcpSubnetIP, tftp=tftp, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, system=system, dnsStats=dnsStats, trapDHCP=trapDHCP, dhcp=dhcp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolTable=dhcpPoolTable, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolSize=dhcpPoolSize, dhcpPoolUsed=dhcpPoolUsed, licenseValid=licenseValid, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemon=dnsDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, ha=ha)
