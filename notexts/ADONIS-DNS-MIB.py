...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolSize=dhcpPoolSize, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemon=dnsDaemon, dhcpPoolUsed=dhcpPoolUsed, adonis=adonis, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplication=trapReplication, dhcpLeaseHostname=dhcpLeaseHostname, adonisTraps=adonisTraps, dnsStatsNXDomain=dnsStatsNXDomain, dhcpConfig=dhcpConfig, dhcpSubnetTable=dhcpSubnetTable, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetIP=dhcpSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIP=dhcpFixedIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapSystem=trapSystem, trapSystemDaemon=trapSystemDaemon, trapHA=trapHA, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapCommandServer=trapCommandServer, ha=ha, PYSNMP_MODULE_ID=adonis, lcd=lcd, system=system, trapReplicationFailure=trapReplicationFailure, dhcpFailOverState=dhcpFailOverState, lcdDaemon=lcdDaemon, dnsStats=dnsStats, dhcpSubnetEntry=dhcpSubnetEntry, dns=dns, tftpDaemon=tftpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpStats=dhcpStats, licenseValid=licenseValid, dhcpIP=dhcpIP, tftp=tftp, dhcpSubnetSize=dhcpSubnetSize, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseBindState=dhcpLeaseBindState, commandServer=commandServer, dhcpDaemon=dhcpDaemon, trapDNS=trapDNS, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetMask=dhcpSubnetMask, commandServerDaemon=commandServerDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolAlert=dhcpPoolAlert, trapDNSDaemon=trapDNSDaemon, licenseExpiry=licenseExpiry, systemState=systemState, dnsStatsReferral=dnsStatsReferral, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, haReplicationBinding=haReplicationBinding, dnsStatsFailure=dnsStatsFailure, dnsStatsRecursion=dnsStatsRecursion, systemDaemon=systemDaemon, haServiceRunning=haServiceRunning, dhcpPoolTable=dhcpPoolTable, haServiceNodeType=haServiceNodeType, trapDHCP=trapDHCP, trapTFTP=trapTFTP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseEntry=dhcpLeaseEntry, dhcp=dhcp, dhcpFixedIPTable=dhcpFixedIPTable, trapDHCPDaemon=trapDHCPDaemon, adonisObjects=adonisObjects, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haService=haService, dnsDaemonRunning=dnsDaemonRunning)
