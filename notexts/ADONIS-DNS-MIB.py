...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonRunning=dhcpDaemonRunning, system=system, systemState=systemState, licenseExpiry=licenseExpiry, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTP=trapTFTP, commandServerDaemon=commandServerDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetSize=dhcpSubnetSize, dhcpStats=dhcpStats, dhcpSubnetIP=dhcpSubnetIP, tftp=tftp, dhcpIP=dhcpIP, dhcpDaemon=dhcpDaemon, dhcpPoolUsed=dhcpPoolUsed, trapSystem=trapSystem, dhcpPoolEndIP=dhcpPoolEndIP, dhcp=dhcp, haServiceNodeType=haServiceNodeType, lcd=lcd, trapDNSDaemon=trapDNSDaemon, PYSNMP_MODULE_ID=adonis, trapCommandServerDaemon=trapCommandServerDaemon, trapReplicationFailure=trapReplicationFailure, commandServer=commandServer, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFailOverState=dhcpFailOverState, dhcpFixedIPTable=dhcpFixedIPTable, dhcpConfig=dhcpConfig, licenseValid=licenseValid, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonisObjects=adonisObjects, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetUsed=dhcpSubnetUsed, trapSystemDaemon=trapSystemDaemon, tftpDaemon=tftpDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, adonis=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetTable=dhcpSubnetTable, trapDHCPDaemon=trapDHCPDaemon, haService=haService, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseBindState=dhcpLeaseBindState, haReplicationBinding=haReplicationBinding, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemon=dnsDaemon, trapDNS=trapDNS, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, tftpDaemonRunning=tftpDaemonRunning, adonisTraps=adonisTraps, dnsStatsRecursion=dnsStatsRecursion, dnsStatsFailure=dnsStatsFailure, haServiceRunning=haServiceRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, systemDaemon=systemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsSuccess=dnsStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, trapHA=trapHA, lcdDaemon=lcdDaemon, dhcpPoolSize=dhcpPoolSize, dns=dns, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolTable=dhcpPoolTable, dnsStats=dnsStats, trapReplication=trapReplication, dhcpLeaseEndTime=dhcpLeaseEndTime, ha=ha, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServer=trapCommandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIP=dhcpFixedIP, dhcpLeaseHostname=dhcpLeaseHostname, trapDHCP=trapDHCP)
