...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFixedIP=dhcpFixedIP, haReplicationBinding=haReplicationBinding, adonisObjects=adonisObjects, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseHostname=dhcpLeaseHostname, trapTFTP=trapTFTP, trapCommandServer=trapCommandServer, trapSystem=trapSystem, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolTable=dhcpPoolTable, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonRunning=dhcpDaemonRunning, licenseValid=licenseValid, trapSystemDaemon=trapSystemDaemon, systemState=systemState, dhcpLeaseBindState=dhcpLeaseBindState, dns=dns, adonis=adonis, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsFailure=dnsStatsFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsReferral=dnsStatsReferral, haServiceNodeType=haServiceNodeType, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, tftp=tftp, dhcpLeaseEntry=dhcpLeaseEntry, trapHA=trapHA, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemon=dhcpDaemon, dhcpPoolUsed=dhcpPoolUsed, adonisTraps=adonisTraps, trapDNSDaemon=trapDNSDaemon, ha=ha, haService=haService, lcd=lcd, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, systemDaemon=systemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpFailOverState=dhcpFailOverState, trapReplication=trapReplication, trapReplicationFailure=trapReplicationFailure, trapDHCPDaemon=trapDHCPDaemon, trapDNS=trapDNS, dhcp=dhcp, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, dhcpLeaseEndTime=dhcpLeaseEndTime, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetIP=dhcpSubnetIP, dnsDaemon=dnsDaemon, dhcpConfig=dhcpConfig, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, system=system, dhcpStats=dhcpStats, commandServerDaemon=commandServerDaemon, tftpDaemonRunning=tftpDaemonRunning, commandServer=commandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetTable=dhcpSubnetTable, tftpDaemon=tftpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetAlert=dhcpSubnetAlert, PYSNMP_MODULE_ID=adonis, trapHAServiceFailOver=trapHAServiceFailOver, lcdDaemon=lcdDaemon, licenseExpiry=licenseExpiry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolSize=dhcpPoolSize, dnsStats=dnsStats, haServiceRunning=haServiceRunning, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCP=trapDHCP, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEntry=dhcpPoolEntry)
