...
ObjectIdentity, ModuleIdentity, Bits, iso, Counter64, TimeTicks, NotificationType, Integer32, Gauge32, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Unsigned32, enterprises, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "ModuleIdentity", "Bits", "iso", "Counter64", "TimeTicks", "NotificationType", "Integer32", "Gauge32", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Unsigned32", "enterprises", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFixedIP=dhcpFixedIP, system=system, dhcpLeaseEntry=dhcpLeaseEntry, dns=dns, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, haReplicationBinding=haReplicationBinding, trapTFTP=trapTFTP, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNSDaemon=trapDNSDaemon, dnsDaemon=dnsDaemon, trapDHCP=trapDHCP, commandServerDaemon=commandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDHCPDaemon=trapDHCPDaemon, licenseValid=licenseValid, systemDaemon=systemDaemon, dhcpLeaseBindState=dhcpLeaseBindState, systemState=systemState, dhcpSubnetIP=dhcpSubnetIP, trapHAServiceFailOver=trapHAServiceFailOver, lcdDaemon=lcdDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceNodeType=haServiceNodeType, tftpDaemon=tftpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, trapDNS=trapDNS, dhcpPoolUsed=dhcpPoolUsed, dnsStats=dnsStats, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpIP=dhcpIP, dnsDaemonRunning=dnsDaemonRunning, dhcpFailOverState=dhcpFailOverState, licenseExpiry=licenseExpiry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonis=adonis, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseHostname=dhcpLeaseHostname, ha=ha, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIPTable=dhcpFixedIPTable, trapSystemDaemon=trapSystemDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpStats=dhcpStats, trapTFTPDaemon=trapTFTPDaemon, adonisTraps=adonisTraps, trapCommandServerDaemon=trapCommandServerDaemon, trapHA=trapHA, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsFailure=dnsStatsFailure, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSize=dhcpPoolSize, dhcpPoolEntry=dhcpPoolEntry, adonisObjects=adonisObjects, dnsStatsReferral=dnsStatsReferral, dhcp=dhcp, trapReplicationFailure=trapReplicationFailure, haServiceRunning=haServiceRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolAlert=dhcpPoolAlert, haService=haService, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapReplication=trapReplication, dnsStatsRecursion=dnsStatsRecursion, commandServer=commandServer, dhcpPoolTable=dhcpPoolTable, trapSystem=trapSystem, PYSNMP_MODULE_ID=adonis, trapCommandServer=trapCommandServer, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpConfig=dhcpConfig, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEndIP=dhcpPoolEndIP, tftp=tftp, lcd=lcd, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess)
