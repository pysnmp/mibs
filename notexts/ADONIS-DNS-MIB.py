...
MibIdentifier, Bits, Gauge32, Counter64, IpAddress, iso, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, Unsigned32, TimeTicks, NotificationType, enterprises, Counter32, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Bits", "Gauge32", "Counter64", "IpAddress", "iso", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "Unsigned32", "TimeTicks", "NotificationType", "enterprises", "Counter32", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStats=dnsStats, dhcp=dhcp, commandServerDaemon=commandServerDaemon, lcd=lcd, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNS=trapDNS, dhcpFixedIP=dhcpFixedIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftpDaemon=tftpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpConfig=dhcpConfig, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, dhcpPoolTable=dhcpPoolTable, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolSize=dhcpPoolSize, trapHA=trapHA, licenseValid=licenseValid, trapHAServiceFailOver=trapHAServiceFailOver, trapReplication=trapReplication, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haReplicationBinding=haReplicationBinding, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEntry=dhcpPoolEntry, lcdDaemon=lcdDaemon, dhcpSubnetTable=dhcpSubnetTable, systemState=systemState, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, adonisObjects=adonisObjects, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, tftp=tftp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, trapDNSDaemon=trapDNSDaemon, dhcpSubnetIP=dhcpSubnetIP, trapDHCP=trapDHCP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsFailure=dnsStatsFailure, trapCommandServer=trapCommandServer, trapTFTP=trapTFTP, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDHCPDaemon=trapDHCPDaemon, licenseExpiry=licenseExpiry, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapTFTPDaemon=trapTFTPDaemon, dhcpFailOverState=dhcpFailOverState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceRunning=haServiceRunning, tftpDaemonRunning=tftpDaemonRunning, trapReplicationFailure=trapReplicationFailure, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseHostname=dhcpLeaseHostname, ha=ha, trapSystem=trapSystem, dhcpLeaseEntry=dhcpLeaseEntry, system=system, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEndIP=dhcpPoolEndIP, dhcpIP=dhcpIP, adonisTraps=adonisTraps, adonis=adonis, PYSNMP_MODULE_ID=adonis, dnsStatsReferral=dnsStatsReferral, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, systemDaemon=systemDaemon, haServiceNodeType=haServiceNodeType, dnsDaemon=dnsDaemon, dns=dns, commandServer=commandServer, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemon=dhcpDaemon)
