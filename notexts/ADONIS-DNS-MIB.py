...
MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, ObjectIdentity, IpAddress, Counter64, TimeTicks, Integer32, iso, Gauge32, NotificationType, Counter32, enterprises, Unsigned32, ModuleIdentity, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "ObjectIdentity", "IpAddress", "Counter64", "TimeTicks", "Integer32", "iso", "Gauge32", "NotificationType", "Counter32", "enterprises", "Unsigned32", "ModuleIdentity", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFailOverState=dhcpFailOverState, adonis=adonis, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolSize=dhcpPoolSize, lcdDaemon=lcdDaemon, adonisTraps=adonisTraps, dhcpStats=dhcpStats, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haServiceRunning=haServiceRunning, dhcpFixedIPTable=dhcpFixedIPTable, trapHA=trapHA, dhcpLeaseBindState=dhcpLeaseBindState, haReplicationBinding=haReplicationBinding, lcd=lcd, dhcpFixedIP=dhcpFixedIP, commandServer=commandServer, licenseExpiry=licenseExpiry, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapHAServiceFailOver=trapHAServiceFailOver, licenseValid=licenseValid, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, system=system, dhcpPoolStartIP=dhcpPoolStartIP, dhcpIP=dhcpIP, dhcpConfig=dhcpConfig, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemon=dnsDaemon, dhcpPoolTable=dhcpPoolTable, trapSystem=trapSystem, trapCommandServer=trapCommandServer, trapDHCP=trapDHCP, trapReplicationFailure=trapReplicationFailure, dhcpPoolUsed=dhcpPoolUsed, haService=haService, dhcpFixedIPEntry=dhcpFixedIPEntry, trapSystemDaemon=trapSystemDaemon, systemDaemon=systemDaemon, ha=ha, dns=dns, haServiceNodeType=haServiceNodeType, dhcpLeaseTable=dhcpLeaseTable, tftp=tftp, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolAlert=dhcpPoolAlert, trapDNSDaemon=trapDNSDaemon, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemon=commandServerDaemon, dhcpSubnetIP=dhcpSubnetIP, PYSNMP_MODULE_ID=adonis, dnsStatsFailure=dnsStatsFailure, dhcpSubnetUsed=dhcpSubnetUsed, dhcp=dhcp, dhcpSubnetTable=dhcpSubnetTable, dnsStatsReferral=dnsStatsReferral, trapTFTP=trapTFTP, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCPDaemon=trapDHCPDaemon, systemState=systemState, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNS=trapDNS, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisObjects=adonisObjects, dhcpSubnetAlert=dhcpSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, trapReplication=trapReplication, dnsStats=dnsStats, dhcpSubnetMask=dhcpSubnetMask, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemon=dhcpDaemon, tftpDaemon=tftpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert)
