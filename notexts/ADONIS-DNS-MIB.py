...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisTraps=adonisTraps, licenseExpiry=licenseExpiry, lcdDaemon=lcdDaemon, dhcpLeaseHostname=dhcpLeaseHostname, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpStats=dhcpStats, tftp=tftp, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemon=dhcpDaemon, adonis=adonis, trapCommandServerDaemon=trapCommandServerDaemon, dhcp=dhcp, dnsDaemon=dnsDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEndTime=dhcpLeaseEndTime, systemDaemon=systemDaemon, dhcpPoolSize=dhcpPoolSize, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolEndIP=dhcpPoolEndIP, tftpDaemonRunning=tftpDaemonRunning, dhcpIP=dhcpIP, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServer=trapCommandServer, dhcpPoolEntry=dhcpPoolEntry, haService=haService, commandServer=commandServer, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, system=system, dnsStats=dnsStats, PYSNMP_MODULE_ID=adonis, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolUsed=dhcpPoolUsed, haReplicationBinding=haReplicationBinding, trapReplication=trapReplication, trapReplicationFailure=trapReplicationFailure, trapHA=trapHA, haServiceRunning=haServiceRunning, dhcpFixedIP=dhcpFixedIP, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolStartIP=dhcpPoolStartIP, dns=dns, trapDNS=trapDNS, dnsStatsReferral=dnsStatsReferral, licenseValid=licenseValid, trapSystemDaemon=trapSystemDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIPEntry=dhcpFixedIPEntry, trapHAServiceFailOver=trapHAServiceFailOver, trapSystem=trapSystem, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServerDaemon=commandServerDaemon, dhcpConfig=dhcpConfig, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetAlert=dhcpSubnetAlert, haServiceNodeType=haServiceNodeType, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPTable=dhcpFixedIPTable, trapTFTPDaemon=trapTFTPDaemon, trapTFTP=trapTFTP, dhcpSubnetSize=dhcpSubnetSize, systemState=systemState, dhcpFailOverState=dhcpFailOverState, ha=ha, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDHCPDaemon=trapDHCPDaemon, trapDHCP=trapDHCP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonisObjects=adonisObjects, dnsStatsFailure=dnsStatsFailure, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemon=tftpDaemon, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonRunning=dnsDaemonRunning, lcd=lcd)
