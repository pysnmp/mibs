...
enterprises, IpAddress, Gauge32, ModuleIdentity, NotificationType, iso, Counter32, ObjectIdentity, MibIdentifier, TimeTicks, Counter64, Integer32, Unsigned32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "IpAddress", "Gauge32", "ModuleIdentity", "NotificationType", "iso", "Counter32", "ObjectIdentity", "MibIdentifier", "TimeTicks", "Counter64", "Integer32", "Unsigned32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haServiceNodeType=haServiceNodeType, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIPEntry=dhcpFixedIPEntry, dns=dns, trapTFTP=trapTFTP, dhcpPoolSize=dhcpPoolSize, systemState=systemState, dnsDaemon=dnsDaemon, dhcpLeaseEntry=dhcpLeaseEntry, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haReplicationBinding=haReplicationBinding, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIP=dhcpFixedIP, dhcpIP=dhcpIP, dnsStats=dnsStats, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseBindState=dhcpLeaseBindState, adonis=adonis, dhcpDaemonRunning=dhcpDaemonRunning, PYSNMP_MODULE_ID=adonis, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsReferral=dnsStatsReferral, trapSystem=trapSystem, haServiceRunning=haServiceRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, systemDaemon=systemDaemon, haService=haService, lcd=lcd, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFailOverState=dhcpFailOverState, trapHA=trapHA, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolTable=dhcpPoolTable, tftp=tftp, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, adonisObjects=adonisObjects, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetSize=dhcpSubnetSize, trapSystemDaemon=trapSystemDaemon, adonisTraps=adonisTraps, dhcpSubnetIP=dhcpSubnetIP, dhcpStats=dhcpStats, dhcpDaemon=dhcpDaemon, commandServer=commandServer, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolAlert=dhcpPoolAlert, trapHAServiceFailOver=trapHAServiceFailOver, dhcpConfig=dhcpConfig, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNS=trapDNS, ha=ha, trapDHCP=trapDHCP, trapCommandServer=trapCommandServer, licenseValid=licenseValid, trapDHCPDaemon=trapDHCPDaemon, tftpDaemon=tftpDaemon, system=system, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseTable=dhcpLeaseTable, trapCommandServerDaemon=trapCommandServerDaemon, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, trapReplication=trapReplication, trapDNSDaemon=trapDNSDaemon, trapReplicationFailure=trapReplicationFailure, dhcpLeaseEndTime=dhcpLeaseEndTime, tftpDaemonRunning=tftpDaemonRunning, dhcp=dhcp, lcdDaemon=lcdDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseHostname=dhcpLeaseHostname)
