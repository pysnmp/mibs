...
MibIdentifier, ObjectIdentity, NotificationType, Integer32, iso, Unsigned32, enterprises, Gauge32, ModuleIdentity, Bits, Counter64, Counter32, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "ObjectIdentity", "NotificationType", "Integer32", "iso", "Unsigned32", "enterprises", "Gauge32", "ModuleIdentity", "Bits", "Counter64", "Counter32", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolStartIP=dhcpPoolStartIP, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, licenseExpiry=licenseExpiry, trapHAServiceFailOver=trapHAServiceFailOver, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftp=tftp, trapSystem=trapSystem, ha=ha, dhcpLeaseTable=dhcpLeaseTable, trapDNS=trapDNS, system=system, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsFailure=dnsStatsFailure, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPTable=dhcpFixedIPTable, haService=haService, trapTFTPDaemon=trapTFTPDaemon, dhcpConfig=dhcpConfig, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHostname=dhcpLeaseHostname, licenseValid=licenseValid, trapCommandServerDaemon=trapCommandServerDaemon, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFixedIP=dhcpFixedIP, commandServer=commandServer, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServerDaemon=commandServerDaemon, adonisTraps=adonisTraps, trapTFTP=trapTFTP, dhcpDaemon=dhcpDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseStartTime=dhcpLeaseStartTime, lcd=lcd, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsReferral=dnsStatsReferral, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolTable=dhcpPoolTable, adonisObjects=adonisObjects, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonRunning=dnsDaemonRunning, trapHA=trapHA, trapSystemDaemon=trapSystemDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetTable=dhcpSubnetTable, dnsStats=dnsStats, adonis=adonis, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolAlert=dhcpPoolAlert, PYSNMP_MODULE_ID=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, systemDaemon=systemDaemon, dnsDaemon=dnsDaemon, trapDNSDaemon=trapDNSDaemon, dnsStatsSuccess=dnsStatsSuccess, trapReplication=trapReplication, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcp=dhcp, dhcpSubnetAlert=dhcpSubnetAlert, trapReplicationFailure=trapReplicationFailure, haServiceRunning=haServiceRunning, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetIP=dhcpSubnetIP, dhcpFailOverState=dhcpFailOverState, dhcpStats=dhcpStats, dhcpPoolSize=dhcpPoolSize, tftpDaemon=tftpDaemon, trapDHCP=trapDHCP, lcdDaemon=lcdDaemon, dns=dns, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpIP=dhcpIP, systemState=systemState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolEntry=dhcpPoolEntry, trapDHCPDaemon=trapDHCPDaemon)
