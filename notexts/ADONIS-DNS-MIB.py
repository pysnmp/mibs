...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsReferral=dnsStatsReferral, dhcpPoolTable=dhcpPoolTable, dhcpStats=dhcpStats, dnsStatsNXDomain=dnsStatsNXDomain, adonis=adonis, haServiceNodeType=haServiceNodeType, tftp=tftp, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapHA=trapHA, adonisObjects=adonisObjects, dnsStatsFailure=dnsStatsFailure, dhcpFixedIP=dhcpFixedIP, adonisTraps=adonisTraps, trapTFTP=trapTFTP, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolAlert=dhcpPoolAlert, dhcpConfig=dhcpConfig, dnsStats=dnsStats, dns=dns, ha=ha, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystem=trapSystem, dhcpFailOverState=dhcpFailOverState, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNS=trapDNS, dhcpSubnetAlert=dhcpSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemon=dnsDaemon, dhcpDaemon=dhcpDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsNXRRSet=dnsStatsNXRRSet, trapCommandServer=trapCommandServer, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolSize=dhcpPoolSize, haServiceRunning=haServiceRunning, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haService=haService, PYSNMP_MODULE_ID=adonis, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseTable=dhcpLeaseTable, trapTFTPDaemon=trapTFTPDaemon, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpIP=dhcpIP, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemDaemon=systemDaemon, trapReplication=trapReplication, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemon=commandServerDaemon, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapSystemDaemon=trapSystemDaemon, systemState=systemState, tftpDaemon=tftpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolUsed=dhcpPoolUsed, system=system, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServer=commandServer, lcdDaemon=lcdDaemon, lcd=lcd, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseBindState=dhcpLeaseBindState, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCPDaemon=trapDHCPDaemon, licenseValid=licenseValid, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNSDaemon=trapDNSDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dhcp=dhcp)
