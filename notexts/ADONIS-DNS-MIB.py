...
enterprises, ModuleIdentity, Integer32, Counter64, Bits, Counter32, ObjectIdentity, Unsigned32, IpAddress, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, MibIdentifier, TimeTicks, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "ModuleIdentity", "Integer32", "Counter64", "Bits", "Counter32", "ObjectIdentity", "Unsigned32", "IpAddress", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "MibIdentifier", "TimeTicks", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haServiceNodeType=haServiceNodeType, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemon=commandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, trapDNS=trapDNS, ha=ha, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, trapSystem=trapSystem, dhcpConfig=dhcpConfig, dhcpLeaseHostname=dhcpLeaseHostname, adonisObjects=adonisObjects, trapTFTP=trapTFTP, adonis=adonis, dhcpFixedIPTable=dhcpFixedIPTable, trapHAServiceFailOver=trapHAServiceFailOver, tftpDaemon=tftpDaemon, dhcpFailOverState=dhcpFailOverState, dhcpPoolEntry=dhcpPoolEntry, tftp=tftp, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServer=commandServer, dhcpSubnetSize=dhcpSubnetSize, dnsStatsFailure=dnsStatsFailure, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStats=dnsStats, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolTable=dhcpPoolTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolStartIP=dhcpPoolStartIP, haReplicationBinding=haReplicationBinding, dhcpPoolUsed=dhcpPoolUsed, lcd=lcd, dnsStatsReferral=dnsStatsReferral, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapCommandServerDaemon=trapCommandServerDaemon, trapDNSDaemon=trapDNSDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, systemDaemon=systemDaemon, trapSystemDaemon=trapSystemDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemon=dhcpDaemon, dhcpFixedIP=dhcpFixedIP, dhcpLeaseTable=dhcpLeaseTable, trapCommandServer=trapCommandServer, haServiceRunning=haServiceRunning, system=system, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDHCP=trapDHCP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, haService=haService, PYSNMP_MODULE_ID=adonis, dhcpIP=dhcpIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemState=systemState, adonisTraps=adonisTraps, dhcp=dhcp, lcdDaemon=lcdDaemon, dhcpSubnetEntry=dhcpSubnetEntry, trapHA=trapHA, dnsDaemon=dnsDaemon, dhcpLeaseBindState=dhcpLeaseBindState, licenseExpiry=licenseExpiry, dhcpStats=dhcpStats, trapReplication=trapReplication, dns=dns, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, licenseValid=licenseValid, trapReplicationFailure=trapReplicationFailure, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolSize=dhcpPoolSize, dhcpLeaseEntry=dhcpLeaseEntry)
