...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsFailure=dnsStatsFailure, trapSystem=trapSystem, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsReferral=dnsStatsReferral, haService=haService, dhcp=dhcp, dnsStatsNXRRSet=dnsStatsNXRRSet, tftpDaemon=tftpDaemon, dhcpStats=dhcpStats, dnsDaemon=dnsDaemon, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, lcd=lcd, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapReplication=trapReplication, adonisTraps=adonisTraps, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStats=dnsStats, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemon=dhcpDaemon, systemDaemon=systemDaemon, haReplicationBinding=haReplicationBinding, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, trapHA=trapHA, trapDNS=trapDNS, dhcpPoolUsed=dhcpPoolUsed, dns=dns, dhcpConfig=dhcpConfig, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetUsed=dhcpSubnetUsed, trapDNSDaemon=trapDNSDaemon, commandServerDaemon=commandServerDaemon, system=system, PYSNMP_MODULE_ID=adonis, trapCommandServer=trapCommandServer, dhcpPoolSize=dhcpPoolSize, dhcpFixedIP=dhcpFixedIP, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCP=trapDHCP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetTable=dhcpSubnetTable, trapReplicationFailure=trapReplicationFailure, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolTable=dhcpPoolTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonRunning=dnsDaemonRunning, licenseExpiry=licenseExpiry, dhcpLeaseTable=dhcpLeaseTable, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsRecursion=dnsStatsRecursion, trapTFTPDaemon=trapTFTPDaemon, adonis=adonis, haServiceNodeType=haServiceNodeType, licenseValid=licenseValid, dhcpSubnetAlert=dhcpSubnetAlert, lcdDaemon=lcdDaemon, adonisObjects=adonisObjects, commandServer=commandServer, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPTable=dhcpFixedIPTable, tftp=tftp, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpIP=dhcpIP, haServiceRunning=haServiceRunning, trapSystemDaemon=trapSystemDaemon, tftpDaemonRunning=tftpDaemonRunning, systemState=systemState, dnsStatsSuccess=dnsStatsSuccess, ha=ha)
