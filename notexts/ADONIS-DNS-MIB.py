...
NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, TimeTicks, MibIdentifier, IpAddress, Integer32, Bits, ModuleIdentity, ObjectIdentity, Counter64, iso, Counter32, enterprises, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "TimeTicks", "MibIdentifier", "IpAddress", "Integer32", "Bits", "ModuleIdentity", "ObjectIdentity", "Counter64", "iso", "Counter32", "enterprises", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapSystem=trapSystem, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDNS=trapDNS, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIPTable=dhcpFixedIPTable, system=system, commandServerDaemon=commandServerDaemon, commandServer=commandServer, dhcpStats=dhcpStats, trapDHCP=trapDHCP, dnsStatsSuccess=dnsStatsSuccess, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemon=dhcpDaemon, trapCommandServerDaemon=trapCommandServerDaemon, trapReplicationFailure=trapReplicationFailure, dnsStats=dnsStats, adonisTraps=adonisTraps, dhcpSubnetAlert=dhcpSubnetAlert, systemState=systemState, dhcpSubnetMask=dhcpSubnetMask, dnsStatsFailure=dnsStatsFailure, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemon=dnsDaemon, dhcpPoolEntry=dhcpPoolEntry, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haServiceNodeType=haServiceNodeType, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsRecursion=dnsStatsRecursion, licenseExpiry=licenseExpiry, dhcpFailOverState=dhcpFailOverState, tftp=tftp, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, lcd=lcd, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCPDaemon=trapDHCPDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, PYSNMP_MODULE_ID=adonis, dhcpFixedIP=dhcpFixedIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, ha=ha, haReplicationBinding=haReplicationBinding, systemDaemon=systemDaemon, dns=dns, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetIP=dhcpSubnetIP, dnsStatsReferral=dnsStatsReferral, dhcp=dhcp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftpDaemon=tftpDaemon, dhcpSubnetTable=dhcpSubnetTable, trapSystemDaemon=trapSystemDaemon, trapHA=trapHA, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haServiceRunning=haServiceRunning, dhcpIP=dhcpIP, adonisObjects=adonisObjects, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpConfig=dhcpConfig, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolSize=dhcpPoolSize, dhcpLeaseEndTime=dhcpLeaseEndTime, trapHAServiceFailOver=trapHAServiceFailOver, trapReplication=trapReplication, trapCommandServer=trapCommandServer, lcdDaemon=lcdDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haService=haService, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolTable=dhcpPoolTable, dhcpSubnetSize=dhcpSubnetSize, adonis=adonis, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetEntry=dhcpSubnetEntry, licenseValid=licenseValid, trapTFTP=trapTFTP, dhcpPoolUsed=dhcpPoolUsed)
