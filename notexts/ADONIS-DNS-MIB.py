...
enterprises, Gauge32, iso, IpAddress, Unsigned32, ObjectIdentity, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, MibIdentifier, Bits, ModuleIdentity, Counter32, TimeTicks, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Gauge32", "iso", "IpAddress", "Unsigned32", "ObjectIdentity", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "MibIdentifier", "Bits", "ModuleIdentity", "Counter32", "TimeTicks", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, system=system, dnsStatsRecursion=dnsStatsRecursion, trapCommandServer=trapCommandServer, adonisTraps=adonisTraps, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, licenseValid=licenseValid, licenseExpiry=licenseExpiry, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolUsed=dhcpPoolUsed, trapDHCP=trapDHCP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpStats=dhcpStats, dhcpPoolTable=dhcpPoolTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemon=dnsDaemon, haServiceRunning=haServiceRunning, dhcpIP=dhcpIP, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStats=dnsStats, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHostname=dhcpLeaseHostname, dhcpConfig=dhcpConfig, dns=dns, PYSNMP_MODULE_ID=adonis, commandServerDaemon=commandServerDaemon, haServiceNodeType=haServiceNodeType, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapCommandServerDaemon=trapCommandServerDaemon, dhcp=dhcp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystem=trapSystem, dhcpPoolStartIP=dhcpPoolStartIP, adonis=adonis, haService=haService, commandServer=commandServer, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIPEntry=dhcpFixedIPEntry, trapHA=trapHA, dhcpPoolAlert=dhcpPoolAlert, dhcpFailOverState=dhcpFailOverState, dnsStatsReferral=dnsStatsReferral, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIP=dhcpFixedIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemon=dhcpDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTP=trapTFTP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetSize=dhcpSubnetSize, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemon=tftpDaemon, lcd=lcd, trapDHCPDaemon=trapDHCPDaemon, haReplicationBinding=haReplicationBinding, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolEndIP=dhcpPoolEndIP, trapDNSDaemon=trapDNSDaemon, adonisObjects=adonisObjects, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapReplication=trapReplication, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, systemDaemon=systemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplicationFailure=trapReplicationFailure, lcdDaemon=lcdDaemon, ha=ha, trapSystemDaemon=trapSystemDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSize=dhcpPoolSize, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsFailure=dnsStatsFailure, systemState=systemState, tftp=tftp, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNS=trapDNS)
