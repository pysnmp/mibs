...
ObjectIdentity, Gauge32, Counter64, IpAddress, ModuleIdentity, Counter32, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, TimeTicks, MibIdentifier, NotificationType, Integer32, Unsigned32, iso = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Gauge32", "Counter64", "IpAddress", "ModuleIdentity", "Counter32", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "TimeTicks", "MibIdentifier", "NotificationType", "Integer32", "Unsigned32", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServer=trapCommandServer, dhcpPoolEntry=dhcpPoolEntry, adonisObjects=adonisObjects, system=system, licenseValid=licenseValid, dhcpConfig=dhcpConfig, haServiceNodeType=haServiceNodeType, commandServer=commandServer, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemon=dnsDaemon, adonisTraps=adonisTraps, PYSNMP_MODULE_ID=adonis, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcp=dhcp, licenseExpiry=licenseExpiry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEndIP=dhcpPoolEndIP, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsNXDomain=dnsStatsNXDomain, trapTFTPDaemon=trapTFTPDaemon, adonis=adonis, dhcpSubnetMask=dhcpSubnetMask, lcd=lcd, dhcpFixedIP=dhcpFixedIP, dns=dns, haService=haService, systemState=systemState, dhcpPoolTable=dhcpPoolTable, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolSize=dhcpPoolSize, trapHA=trapHA, trapDNSDaemon=trapDNSDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetIP=dhcpSubnetIP, dhcpFailOverState=dhcpFailOverState, dnsStatsFailure=dnsStatsFailure, lcdDaemon=lcdDaemon, dnsStats=dnsStats, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolStartIP=dhcpPoolStartIP, trapSystemDaemon=trapSystemDaemon, ha=ha, dhcpLeaseEndTime=dhcpLeaseEndTime, trapSystem=trapSystem, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIPTable=dhcpFixedIPTable, commandServerDaemon=commandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, haReplicationBinding=haReplicationBinding, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, dnsStatsReferral=dnsStatsReferral, trapReplication=trapReplication, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpIP=dhcpIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapReplicationFailure=trapReplicationFailure, trapTFTP=trapTFTP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemon=dhcpDaemon, trapDHCP=trapDHCP, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDHCPDaemon=trapDHCPDaemon, dnsStatsSuccess=dnsStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, trapDNS=trapDNS, haServiceRunning=haServiceRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsRecursion=dnsStatsRecursion, tftp=tftp, dhcpSubnetUsed=dhcpSubnetUsed, dhcpStats=dhcpStats, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonRunning=dnsDaemonRunning)
