...
ModuleIdentity, Bits, ObjectIdentity, enterprises, Counter32, Integer32, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, TimeTicks, iso, Unsigned32, MibIdentifier, Counter64, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Bits", "ObjectIdentity", "enterprises", "Counter32", "Integer32", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "TimeTicks", "iso", "Unsigned32", "MibIdentifier", "Counter64", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpConfig=dhcpConfig, tftp=tftp, dhcpFixedIP=dhcpFixedIP, dhcpLeaseTable=dhcpLeaseTable, trapTFTP=trapTFTP, dnsStats=dnsStats, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolSize=dhcpPoolSize, trapReplication=trapReplication, trapCommandServer=trapCommandServer, dhcpDaemon=dhcpDaemon, PYSNMP_MODULE_ID=adonis, dhcpPoolTable=dhcpPoolTable, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetSize=dhcpSubnetSize, dhcpStats=dhcpStats, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonis=adonis, system=system, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapSystem=trapSystem, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsFailure=dnsStatsFailure, lcdDaemon=lcdDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, haServiceNodeType=haServiceNodeType, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemon=dnsDaemon, haReplicationBinding=haReplicationBinding, trapDNSDaemon=trapDNSDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsReferral=dnsStatsReferral, dhcpFailOverState=dhcpFailOverState, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetUsed=dhcpSubnetUsed, commandServer=commandServer, trapReplicationFailure=trapReplicationFailure, trapDNS=trapDNS, commandServerDaemon=commandServerDaemon, licenseValid=licenseValid, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dns=dns, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHA=trapHA, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, trapCommandServerDaemon=trapCommandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftpDaemon=tftpDaemon, dhcpSubnetMask=dhcpSubnetMask, systemState=systemState, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEntry=dhcpLeaseEntry, adonisObjects=adonisObjects, adonisTraps=adonisTraps, lcd=lcd, dnsStatsSuccess=dnsStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, dhcp=dhcp, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsNXDomain=dnsStatsNXDomain, haServiceRunning=haServiceRunning, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, ha=ha, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetAlert=dhcpSubnetAlert, licenseExpiry=licenseExpiry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCP=trapDHCP, dhcpIP=dhcpIP, dhcpPoolAlert=dhcpPoolAlert, systemDaemon=systemDaemon, haService=haService)
