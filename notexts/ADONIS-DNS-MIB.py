...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsFailure=dnsStatsFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolUsed=dhcpPoolUsed, trapTFTP=trapTFTP, systemState=systemState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCP=trapDHCP, trapCommandServerDaemon=trapCommandServerDaemon, trapSystem=trapSystem, dhcpLeaseEntry=dhcpLeaseEntry, haServiceRunning=haServiceRunning, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, tftp=tftp, haServiceNodeType=haServiceNodeType, lcd=lcd, dhcpPoolAlert=dhcpPoolAlert, dhcpFixedIP=dhcpFixedIP, trapDHCPDaemon=trapDHCPDaemon, systemDaemon=systemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIPTable=dhcpFixedIPTable, adonisObjects=adonisObjects, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemon=dnsDaemon, trapSystemDaemon=trapSystemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcp=dhcp, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapReplication=trapReplication, dhcpLeaseTable=dhcpLeaseTable, dhcpFailOverState=dhcpFailOverState, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, tftpDaemon=tftpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpStats=dhcpStats, PYSNMP_MODULE_ID=adonis, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTPDaemon=trapTFTPDaemon, ha=ha, dhcpPoolStartIP=dhcpPoolStartIP, haReplicationBinding=haReplicationBinding, haService=haService, adonis=adonis, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNS=trapDNS, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, system=system, commandServerDaemon=commandServerDaemon, dhcpSubnetIP=dhcpSubnetIP, trapReplicationFailure=trapReplicationFailure, dhcpPoolEntry=dhcpPoolEntry, tftpDaemonRunning=tftpDaemonRunning, adonisTraps=adonisTraps, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseValid=licenseValid, dhcpConfig=dhcpConfig, licenseExpiry=licenseExpiry, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolSize=dhcpPoolSize, dhcpSubnetEntry=dhcpSubnetEntry, trapDNSDaemon=trapDNSDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, lcdDaemon=lcdDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpIP=dhcpIP, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemon=dhcpDaemon, dns=dns, dhcpSubnetTable=dhcpSubnetTable, trapHA=trapHA, dnsStatsReferral=dnsStatsReferral, dnsStats=dnsStats, commandServer=commandServer, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapCommandServer=trapCommandServer, dhcpPoolTable=dhcpPoolTable)
