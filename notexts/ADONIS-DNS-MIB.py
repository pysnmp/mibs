...
MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Integer32, NotificationType, IpAddress, enterprises, Unsigned32, ObjectIdentity, Bits, Gauge32, Counter32, Counter64, iso, ModuleIdentity, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Integer32", "NotificationType", "IpAddress", "enterprises", "Unsigned32", "ObjectIdentity", "Bits", "Gauge32", "Counter32", "Counter64", "iso", "ModuleIdentity", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, lcdDaemon=lcdDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisTraps=adonisTraps, dhcpDaemonRunning=dhcpDaemonRunning, tftp=tftp, dhcpPoolUsed=dhcpPoolUsed, commandServer=commandServer, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolEntry=dhcpPoolEntry, dhcp=dhcp, dhcpSubnetAlert=dhcpSubnetAlert, haServiceNodeType=haServiceNodeType, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCP=trapDHCP, commandServerDaemon=commandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNSDaemon=trapDNSDaemon, tftpDaemon=tftpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, dnsStatsFailure=dnsStatsFailure, trapHA=trapHA, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFailOverState=dhcpFailOverState, trapReplication=trapReplication, adonis=adonis, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTable=dhcpLeaseTable, dnsStats=dnsStats, dnsStatsRecursion=dnsStatsRecursion, adonisObjects=adonisObjects, dhcpFixedIP=dhcpFixedIP, systemState=systemState, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolAlert=dhcpPoolAlert, systemDaemon=systemDaemon, haServiceRunning=haServiceRunning, trapDNS=trapDNS, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dnsStatsReferral=dnsStatsReferral, dhcpPoolSize=dhcpPoolSize, trapHAServiceFailOver=trapHAServiceFailOver, haService=haService, trapCommandServer=trapCommandServer, dns=dns, ha=ha, dhcpPoolTable=dhcpPoolTable, haReplicationBinding=haReplicationBinding, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpConfig=dhcpConfig, dhcpStats=dhcpStats, dhcpIP=dhcpIP, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapSystem=trapSystem, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetMask=dhcpSubnetMask, trapSystemDaemon=trapSystemDaemon, trapTFTP=trapTFTP, system=system, licenseValid=licenseValid, trapTFTPDaemon=trapTFTPDaemon, dnsDaemon=dnsDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseExpiry=licenseExpiry, dhcpSubnetTable=dhcpSubnetTable, lcd=lcd, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemon=dhcpDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDHCPDaemon=trapDHCPDaemon)
