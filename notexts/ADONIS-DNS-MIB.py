...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemonRunning=tftpDaemonRunning, dhcpDaemon=dhcpDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, ha=ha, trapDHCPDaemon=trapDHCPDaemon, adonisTraps=adonisTraps, dnsStats=dnsStats, dhcpPoolUsed=dhcpPoolUsed, dhcp=dhcp, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolEndIP=dhcpPoolEndIP, licenseValid=licenseValid, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCP=trapDHCP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseBindState=dhcpLeaseBindState, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolTable=dhcpPoolTable, dhcpFixedIP=dhcpFixedIP, trapCommandServer=trapCommandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSize=dhcpPoolSize, trapTFTPDaemon=trapTFTPDaemon, adonis=adonis, dhcpSubnetMask=dhcpSubnetMask, trapTFTP=trapTFTP, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpStats=dhcpStats, trapReplication=trapReplication, dnsStatsSuccess=dnsStatsSuccess, lcd=lcd, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHA=trapHA, trapReplicationFailure=trapReplicationFailure, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, PYSNMP_MODULE_ID=adonis, tftpDaemon=tftpDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetAlert=dhcpSubnetAlert, haReplicationBinding=haReplicationBinding, dhcpIP=dhcpIP, systemDaemon=systemDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEntry=dhcpPoolEntry, lcdDaemon=lcdDaemon, haServiceNodeType=haServiceNodeType, trapDNSDaemon=trapDNSDaemon, dhcpFailOverState=dhcpFailOverState, haServiceRunning=haServiceRunning, dhcpPoolStartIP=dhcpPoolStartIP, adonisObjects=adonisObjects, dns=dns, commandServer=commandServer, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetEntry=dhcpSubnetEntry, systemState=systemState, dnsDaemon=dnsDaemon, licenseExpiry=licenseExpiry, dhcpConfig=dhcpConfig, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapSystem=trapSystem, haService=haService, dnsStatsNXDomain=dnsStatsNXDomain, tftp=tftp, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsReferral=dnsStatsReferral, dnsDaemonRunning=dnsDaemonRunning, system=system, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemon=commandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsFailure=dnsStatsFailure, dhcpSubnetIP=dhcpSubnetIP, trapSystemDaemon=trapSystemDaemon, trapDNS=trapDNS, dhcpSubnetTable=dhcpSubnetTable)
