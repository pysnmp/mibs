...
MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, iso, TimeTicks, ObjectIdentity, Integer32, Counter32, enterprises, Gauge32, Bits, ModuleIdentity, Unsigned32, MibIdentifier, NotificationType, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "iso", "TimeTicks", "ObjectIdentity", "Integer32", "Counter32", "enterprises", "Gauge32", "Bits", "ModuleIdentity", "Unsigned32", "MibIdentifier", "NotificationType", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpConfig=dhcpConfig, dnsDaemon=dnsDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStats=dnsStats, haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, trapHAServiceFailOver=trapHAServiceFailOver, lcd=lcd, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsFailure=dnsStatsFailure, adonisObjects=adonisObjects, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEntry=dhcpPoolEntry, trapDNS=trapDNS, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFailOverState=dhcpFailOverState, commandServerDaemon=commandServerDaemon, dhcpSubnetSize=dhcpSubnetSize, ha=ha, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haServiceRunning=haServiceRunning, trapDNSDaemon=trapDNSDaemon, trapSystem=trapSystem, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolAlert=dhcpPoolAlert, lcdDaemon=lcdDaemon, dhcpSubnetTable=dhcpSubnetTable, systemDaemon=systemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcp=dhcp, systemState=systemState, adonisTraps=adonisTraps, dhcpLeaseBindState=dhcpLeaseBindState, licenseValid=licenseValid, dnsStatsNXDomain=dnsStatsNXDomain, trapTFTPDaemon=trapTFTPDaemon, dnsStatsRecursion=dnsStatsRecursion, PYSNMP_MODULE_ID=adonis, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSize=dhcpPoolSize, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haServiceNodeType=haServiceNodeType, trapCommandServer=trapCommandServer, trapDHCP=trapDHCP, dhcpPoolEndIP=dhcpPoolEndIP, adonis=adonis, trapReplicationFailure=trapReplicationFailure, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolStartIP=dhcpPoolStartIP, tftpDaemonRunning=tftpDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haService=haService, tftpDaemon=tftpDaemon, dhcpStats=dhcpStats, dhcpIP=dhcpIP, tftp=tftp, trapSystemDaemon=trapSystemDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIP=dhcpFixedIP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, system=system, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServer=commandServer, dhcpDaemon=dhcpDaemon, dns=dns, trapReplication=trapReplication, trapHA=trapHA, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolTable=dhcpPoolTable, licenseExpiry=licenseExpiry, trapTFTP=trapTFTP, dhcpLeaseHostname=dhcpLeaseHostname)
