...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapHA=trapHA, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIP=dhcpFixedIP, tftp=tftp, trapDNS=trapDNS, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, dns=dns, dhcpIP=dhcpIP, systemState=systemState, licenseExpiry=licenseExpiry, dhcpLeaseEndTime=dhcpLeaseEndTime, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, adonis=adonis, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetMask=dhcpSubnetMask, dnsDaemon=dnsDaemon, lcd=lcd, commandServer=commandServer, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haServiceNodeType=haServiceNodeType, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonRunning=dnsDaemonRunning, trapDHCP=trapDHCP, dnsStatsReferral=dnsStatsReferral, haServiceRunning=haServiceRunning, dhcpPoolSize=dhcpPoolSize, trapCommandServer=trapCommandServer, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonisObjects=adonisObjects, tftpDaemonRunning=tftpDaemonRunning, dnsStatsFailure=dnsStatsFailure, dnsStatsRecursion=dnsStatsRecursion, ha=ha, dhcpLeaseHostname=dhcpLeaseHostname, systemDaemon=systemDaemon, trapReplication=trapReplication, adonisTraps=adonisTraps, licenseValid=licenseValid, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseBindState=dhcpLeaseBindState, system=system, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolEntry=dhcpPoolEntry, commandServerDaemon=commandServerDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolTable=dhcpPoolTable, trapTFTPDaemon=trapTFTPDaemon, dnsStats=dnsStats, dhcpSubnetEntry=dhcpSubnetEntry, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolUsed=dhcpPoolUsed, haReplicationBinding=haReplicationBinding, trapReplicationFailure=trapReplicationFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapTFTP=trapTFTP, PYSNMP_MODULE_ID=adonis, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsSuccess=dnsStatsSuccess, trapHAServiceFailOver=trapHAServiceFailOver, dhcp=dhcp, trapSystem=trapSystem, trapDHCPDaemon=trapDHCPDaemon, trapCommandServerDaemon=trapCommandServerDaemon, trapDNSDaemon=trapDNSDaemon, dhcpConfig=dhcpConfig, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, tftpDaemon=tftpDaemon, dhcpDaemon=dhcpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcdDaemon=lcdDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress)
