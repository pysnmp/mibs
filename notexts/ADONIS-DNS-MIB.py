...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsSuccess=dnsStatsSuccess, haServiceNodeType=haServiceNodeType, haServiceRunning=haServiceRunning, dhcp=dhcp, trapDHCPDaemon=trapDHCPDaemon, dnsStatsReferral=dnsStatsReferral, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIP=dhcpFixedIP, trapCommandServer=trapCommandServer, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemon=dnsDaemon, commandServerDaemon=commandServerDaemon, dhcpLeaseHostname=dhcpLeaseHostname, adonis=adonis, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, trapHAServiceFailOver=trapHAServiceFailOver, dhcpStats=dhcpStats, dhcpFixedIPEntry=dhcpFixedIPEntry, lcd=lcd, trapSystemDaemon=trapSystemDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEndTime=dhcpLeaseEndTime, haService=haService, dnsStats=dnsStats, systemDaemon=systemDaemon, adonisTraps=adonisTraps, trapTFTPDaemon=trapTFTPDaemon, trapReplicationFailure=trapReplicationFailure, dhcpDaemonRunning=dhcpDaemonRunning, trapDHCP=trapDHCP, lcdDaemon=lcdDaemon, adonisObjects=adonisObjects, dhcpPoolEntry=dhcpPoolEntry, trapSystem=trapSystem, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNSDaemon=trapDNSDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolTable=dhcpPoolTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEntry=dhcpLeaseEntry, haReplicationBinding=haReplicationBinding, trapHA=trapHA, PYSNMP_MODULE_ID=adonis, licenseValid=licenseValid, commandServer=commandServer, dhcpPoolAlert=dhcpPoolAlert, trapTFTP=trapTFTP, ha=ha, dhcpFixedIPTable=dhcpFixedIPTable, trapReplication=trapReplication, dhcpConfig=dhcpConfig, tftp=tftp, systemState=systemState, dhcpSubnetTable=dhcpSubnetTable, dnsStatsFailure=dnsStatsFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolUsed=dhcpPoolUsed, licenseExpiry=licenseExpiry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNS=trapDNS, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemon=dhcpDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, tftpDaemon=tftpDaemon, system=system, dns=dns, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonRunning=dnsDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain)
