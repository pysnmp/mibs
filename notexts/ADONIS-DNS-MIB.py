...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolSubnetIP=dhcpPoolSubnetIP, tftpDaemon=tftpDaemon, dhcpIP=dhcpIP, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisObjects=adonisObjects, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetSize=dhcpSubnetSize, lcdDaemon=lcdDaemon, licenseExpiry=licenseExpiry, trapReplicationFailure=trapReplicationFailure, dhcpDaemon=dhcpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonis=adonis, dhcpFixedIP=dhcpFixedIP, dnsStatsFailure=dnsStatsFailure, dhcpSubnetEntry=dhcpSubnetEntry, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, system=system, commandServer=commandServer, trapDNSDaemon=trapDNSDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, dnsStatsSuccess=dnsStatsSuccess, dnsStats=dnsStats, lcd=lcd, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisTraps=adonisTraps, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, ha=ha, trapDHCP=trapDHCP, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolStartIP=dhcpPoolStartIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFailOverState=dhcpFailOverState, trapHA=trapHA, dhcpLeaseEntry=dhcpLeaseEntry, systemState=systemState, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, trapSystem=trapSystem, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCPDaemon=trapDHCPDaemon, tftpDaemonRunning=tftpDaemonRunning, systemDaemon=systemDaemon, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolSize=dhcpPoolSize, dhcpStats=dhcpStats, dnsStatsReferral=dnsStatsReferral, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonRunning=dnsDaemonRunning, dns=dns, tftp=tftp, dnsStatsRecursion=dnsStatsRecursion, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapCommandServer=trapCommandServer, trapSystemDaemon=trapSystemDaemon, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemon=dnsDaemon, PYSNMP_MODULE_ID=adonis, trapTFTP=trapTFTP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolTable=dhcpPoolTable, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTPDaemon=trapTFTPDaemon, haServiceNodeType=haServiceNodeType, haServiceRunning=haServiceRunning, dhcpLeaseTable=dhcpLeaseTable, trapReplication=trapReplication, dhcp=dhcp, dhcpFixedIPTable=dhcpFixedIPTable, licenseValid=licenseValid, haReplicationBinding=haReplicationBinding, dhcpConfig=dhcpConfig, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServerDaemon=commandServerDaemon)
