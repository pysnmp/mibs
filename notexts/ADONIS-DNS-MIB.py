...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolStartIP=dhcpPoolStartIP, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonRunning=dnsDaemonRunning, trapDNS=trapDNS, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsRecursion=dnsStatsRecursion, trapTFTPDaemon=trapTFTPDaemon, dhcpFailOverState=dhcpFailOverState, lcd=lcd, dnsStatsFailure=dnsStatsFailure, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetMask=dhcpSubnetMask, licenseExpiry=licenseExpiry, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTable=dhcpLeaseTable, dnsStatsReferral=dnsStatsReferral, dhcpIP=dhcpIP, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemon=commandServerDaemon, dhcpPoolSize=dhcpPoolSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haService=haService, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcdDaemon=lcdDaemon, adonis=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseValid=licenseValid, trapReplicationFailure=trapReplicationFailure, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTP=trapTFTP, tftp=tftp, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolTable=dhcpPoolTable, trapDHCPDaemon=trapDHCPDaemon, dnsStatsSuccess=dnsStatsSuccess, adonisTraps=adonisTraps, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, trapSystem=trapSystem, dhcp=dhcp, trapSystemDaemon=trapSystemDaemon, PYSNMP_MODULE_ID=adonis, systemState=systemState, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonRunning=dhcpDaemonRunning, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemDaemon=systemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIPEntry=dhcpFixedIPEntry, ha=ha, dhcpPoolEntry=dhcpPoolEntry, dhcpStats=dhcpStats, haServiceRunning=haServiceRunning, haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, dns=dns, trapReplication=trapReplication, dnsStats=dnsStats, dhcpFixedIP=dhcpFixedIP, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemon=dnsDaemon, tftpDaemon=tftpDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapCommandServer=trapCommandServer, commandServer=commandServer, system=system, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemon=dhcpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDHCP=trapDHCP, haServiceNodeType=haServiceNodeType, dhcpConfig=dhcpConfig)
