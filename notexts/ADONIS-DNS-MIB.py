...
Bits, IpAddress, Gauge32, enterprises, Unsigned32, Counter32, iso, TimeTicks, Counter64, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, NotificationType, MibIdentifier, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "IpAddress", "Gauge32", "enterprises", "Unsigned32", "Counter32", "iso", "TimeTicks", "Counter64", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "NotificationType", "MibIdentifier", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDHCPDaemon=trapDHCPDaemon, trapSystemDaemon=trapSystemDaemon, dnsDaemon=dnsDaemon, lcd=lcd, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemon=dhcpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStats=dnsStats, dhcpPoolUsed=dhcpPoolUsed, dhcp=dhcp, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFailOverState=dhcpFailOverState, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetAlert=dhcpSubnetAlert, dhcpStats=dhcpStats, dhcpSubnetIP=dhcpSubnetIP, dhcpConfig=dhcpConfig, licenseExpiry=licenseExpiry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, PYSNMP_MODULE_ID=adonis, trapDHCP=trapDHCP, haReplicationBinding=haReplicationBinding, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapReplication=trapReplication, trapCommandServerDaemon=trapCommandServerDaemon, dns=dns, trapReplicationFailure=trapReplicationFailure, dhcpSubnetSize=dhcpSubnetSize, dnsStatsFailure=dnsStatsFailure, dnsStatsReferral=dnsStatsReferral, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpIP=dhcpIP, trapHA=trapHA, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonRunning=dhcpDaemonRunning, lcdDaemon=lcdDaemon, commandServerDaemon=commandServerDaemon, systemDaemon=systemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceNodeType=haServiceNodeType, adonis=adonis, commandServer=commandServer, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolEntry=dhcpPoolEntry, trapSystem=trapSystem, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNS=trapDNS, dhcpPoolStartIP=dhcpPoolStartIP, haService=haService, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, adonisObjects=adonisObjects, dhcpFixedIP=dhcpFixedIP, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolSize=dhcpPoolSize, haServiceRunning=haServiceRunning, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, trapCommandServer=trapCommandServer, ha=ha, dnsStatsSuccess=dnsStatsSuccess, trapTFTP=trapTFTP, systemState=systemState, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsRecursion=dnsStatsRecursion, system=system, tftp=tftp, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFixedIPTable=dhcpFixedIPTable, licenseValid=licenseValid, adonisTraps=adonisTraps, tftpDaemon=tftpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonRunning=dnsDaemonRunning)
