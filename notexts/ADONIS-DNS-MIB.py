...
ObjectIdentity, iso, IpAddress, Bits, Counter32, enterprises, Counter64, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Gauge32, Integer32, MibIdentifier, ModuleIdentity, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "iso", "IpAddress", "Bits", "Counter32", "enterprises", "Counter64", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Gauge32", "Integer32", "MibIdentifier", "ModuleIdentity", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftp=tftp, haServiceRunning=haServiceRunning, licenseExpiry=licenseExpiry, dhcpPoolEntry=dhcpPoolEntry, dhcpStats=dhcpStats, lcd=lcd, dnsStats=dnsStats, dhcpSubnetIP=dhcpSubnetIP, trapHA=trapHA, systemDaemon=systemDaemon, PYSNMP_MODULE_ID=adonis, dhcpSubnetEntry=dhcpSubnetEntry, haService=haService, dnsStatsRecursion=dnsStatsRecursion, dhcpIP=dhcpIP, ha=ha, dhcpPoolStartIP=dhcpPoolStartIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpConfig=dhcpConfig, haReplicationBinding=haReplicationBinding, licenseValid=licenseValid, haServiceNodeType=haServiceNodeType, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCPDaemon=trapDHCPDaemon, tftpDaemon=tftpDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTable=dhcpLeaseTable, system=system, dhcpPoolSize=dhcpPoolSize, trapCommandServer=trapCommandServer, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, systemState=systemState, adonis=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetSize=dhcpSubnetSize, trapDNS=trapDNS, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseHostname=dhcpLeaseHostname, trapCommandServerDaemon=trapCommandServerDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNSDaemon=trapDNSDaemon, dhcp=dhcp, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemon=dhcpDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFixedIP=dhcpFixedIP, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolTable=dhcpPoolTable, dhcpFailOverState=dhcpFailOverState, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplication=trapReplication, trapDHCP=trapDHCP, adonisTraps=adonisTraps, dnsStatsReferral=dnsStatsReferral, commandServerDaemon=commandServerDaemon, dnsDaemon=dnsDaemon, trapTFTP=trapTFTP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTPDaemon=trapTFTPDaemon, trapSystem=trapSystem, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsFailure=dnsStatsFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplicationFailure=trapReplicationFailure, adonisObjects=adonisObjects, dhcpDaemonRunning=dhcpDaemonRunning, lcdDaemon=lcdDaemon, commandServer=commandServer)
