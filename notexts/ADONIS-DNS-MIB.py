...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolSize=dhcpPoolSize, trapHA=trapHA, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsNXDomain=dnsStatsNXDomain, systemState=systemState, licenseValid=licenseValid, trapDNS=trapDNS, adonisObjects=adonisObjects, dhcpPoolTable=dhcpPoolTable, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, commandServerDaemon=commandServerDaemon, trapReplicationFailure=trapReplicationFailure, dhcpStats=dhcpStats, trapDHCPDaemon=trapDHCPDaemon, dnsDaemon=dnsDaemon, dhcpFixedIP=dhcpFixedIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, tftp=tftp, adonisTraps=adonisTraps, dhcp=dhcp, dhcpSubnetIP=dhcpSubnetIP, dhcpFailOverState=dhcpFailOverState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTable=dhcpLeaseTable, dhcpConfig=dhcpConfig, adonis=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServer=commandServer, systemDaemon=systemDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, PYSNMP_MODULE_ID=adonis, trapCommandServer=trapCommandServer, dns=dns, trapTFTP=trapTFTP, trapDHCP=trapDHCP, haService=haService, trapSystemDaemon=trapSystemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolUsed=dhcpPoolUsed, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, licenseExpiry=licenseExpiry, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemonRunning=tftpDaemonRunning, dnsStatsFailure=dnsStatsFailure, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolAlert=dhcpPoolAlert, dnsStatsReferral=dnsStatsReferral, haServiceRunning=haServiceRunning, lcdDaemon=lcdDaemon, trapSystem=trapSystem, dhcpIP=dhcpIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEntry=dhcpPoolEntry, lcd=lcd, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceNodeType=haServiceNodeType, dnsStats=dnsStats, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemon=dhcpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetSize=dhcpSubnetSize, commandServerDaemonRunning=commandServerDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplication=trapReplication, ha=ha, dhcpLeaseEntry=dhcpLeaseEntry, trapDNSDaemon=trapDNSDaemon, tftpDaemon=tftpDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolStartIP=dhcpPoolStartIP, system=system)
