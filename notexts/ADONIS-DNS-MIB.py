...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplicationFailure=trapReplicationFailure, tftpDaemon=tftpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemon=dhcpDaemon, trapDNS=trapDNS, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetAlert=dhcpSubnetAlert, dhcp=dhcp, dhcpConfig=dhcpConfig, dns=dns, dhcpIP=dhcpIP, dnsStats=dnsStats, dnsStatsFailure=dnsStatsFailure, trapCommandServer=trapCommandServer, dhcpLeaseEndTime=dhcpLeaseEndTime, lcd=lcd, trapReplication=trapReplication, licenseValid=licenseValid, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolTable=dhcpPoolTable, trapCommandServerDaemon=trapCommandServerDaemon, systemState=systemState, tftpDaemonRunning=tftpDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemon=commandServerDaemon, ha=ha, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolAlert=dhcpPoolAlert, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, haServiceNodeType=haServiceNodeType, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSize=dhcpPoolSize, dhcpPoolEntry=dhcpPoolEntry, haService=haService, dnsStatsRecursion=dnsStatsRecursion, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIP=dhcpFixedIP, adonis=adonis, trapTFTP=trapTFTP, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, adonisObjects=adonisObjects, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, haReplicationBinding=haReplicationBinding, commandServer=commandServer, trapSystem=trapSystem, dhcpLeaseEntry=dhcpLeaseEntry, system=system, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseBindState=dhcpLeaseBindState, PYSNMP_MODULE_ID=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHostname=dhcpLeaseHostname, tftp=tftp, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisTraps=adonisTraps, licenseExpiry=licenseExpiry, haServiceRunning=haServiceRunning, lcdDaemon=lcdDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpFailOverState=dhcpFailOverState, systemDaemon=systemDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonRunning=dnsDaemonRunning, dnsDaemon=dnsDaemon, trapDHCP=trapDHCP)
