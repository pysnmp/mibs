...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonRunning=dhcpDaemonRunning, haReplicationBinding=haReplicationBinding, dhcpPoolSize=dhcpPoolSize, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, dhcpLeaseEntry=dhcpLeaseEntry, system=system, tftp=tftp, tftpDaemonRunning=tftpDaemonRunning, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsReferral=dnsStatsReferral, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCP=trapDHCP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsFailure=dnsStatsFailure, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolStartIP=dhcpPoolStartIP, dhcp=dhcp, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseHostname=dhcpLeaseHostname, adonisTraps=adonisTraps, ha=ha, dhcpFixedIPEntry=dhcpFixedIPEntry, trapSystemDaemon=trapSystemDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpIP=dhcpIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceRunning=haServiceRunning, licenseValid=licenseValid, dhcpConfig=dhcpConfig, dhcpPoolTable=dhcpPoolTable, dhcpPoolEntry=dhcpPoolEntry, trapTFTP=trapTFTP, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseBindState=dhcpLeaseBindState, trapDNSDaemon=trapDNSDaemon, trapDNS=trapDNS, systemDaemon=systemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServer=commandServer, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapHAServiceFailOver=trapHAServiceFailOver, systemState=systemState, dhcpLeaseTable=dhcpLeaseTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTPDaemon=trapTFTPDaemon, dnsStatsSuccess=dnsStatsSuccess, dnsStats=dnsStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolAlert=dhcpPoolAlert, trapReplication=trapReplication, PYSNMP_MODULE_ID=adonis, lcdDaemon=lcdDaemon, lcd=lcd, dhcpFailOverState=dhcpFailOverState, trapCommandServer=trapCommandServer, dhcpFixedIP=dhcpFixedIP, adonis=adonis, tftpDaemon=tftpDaemon, dns=dns, dnsStatsNXDomain=dnsStatsNXDomain, haServiceNodeType=haServiceNodeType, adonisObjects=adonisObjects, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServerDaemon=commandServerDaemon, trapSystem=trapSystem, dhcpSubnetAlert=dhcpSubnetAlert, licenseExpiry=licenseExpiry, dnsDaemonRunning=dnsDaemonRunning, dhcpStats=dhcpStats, dhcpDaemon=dhcpDaemon, dhcpPoolUsed=dhcpPoolUsed, dnsDaemon=dnsDaemon, dnsStatsRecursion=dnsStatsRecursion)
