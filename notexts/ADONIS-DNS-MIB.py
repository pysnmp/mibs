...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemon=dhcpDaemon, licenseValid=licenseValid, adonis=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStats=dnsStats, trapSystemDaemon=trapSystemDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetAlert=dhcpSubnetAlert, adonisTraps=adonisTraps, dnsDaemon=dnsDaemon, trapReplicationFailure=trapReplicationFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, systemState=systemState, dhcpPoolTable=dhcpPoolTable, dnsStatsReferral=dnsStatsReferral, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, lcdDaemon=lcdDaemon, dhcpLeaseEntry=dhcpLeaseEntry, trapReplication=trapReplication, dhcpSubnetSize=dhcpSubnetSize, lcd=lcd, dhcpSubnetMask=dhcpSubnetMask, dns=dns, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemon=commandServerDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsFailure=dnsStatsFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, tftp=tftp, dhcpFailOverState=dhcpFailOverState, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseHostname=dhcpLeaseHostname, tftpDaemon=tftpDaemon, dhcpPoolSize=dhcpPoolSize, PYSNMP_MODULE_ID=adonis, trapDHCPDaemon=trapDHCPDaemon, trapDNS=trapDNS, dhcpPoolAlert=dhcpPoolAlert, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTable=dhcpLeaseTable, dhcpConfig=dhcpConfig, ha=ha, dhcpDaemonRunning=dhcpDaemonRunning, system=system, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDNSDaemon=trapDNSDaemon, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisObjects=adonisObjects, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemonRunning=tftpDaemonRunning, haServiceNodeType=haServiceNodeType, dnsDaemonRunning=dnsDaemonRunning, systemDaemon=systemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpIP=dhcpIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseEndTime=dhcpLeaseEndTime, haServiceRunning=haServiceRunning, trapTFTP=trapTFTP, trapHA=trapHA, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolUsed=dhcpPoolUsed, haService=haService, dhcpLeaseBindState=dhcpLeaseBindState, licenseExpiry=licenseExpiry, trapTFTPDaemon=trapTFTPDaemon, haReplicationBinding=haReplicationBinding, dhcp=dhcp, dnsStatsRecursion=dnsStatsRecursion, trapDHCP=trapDHCP, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServer=trapCommandServer, trapSystem=trapSystem, dhcpFixedIP=dhcpFixedIP, commandServer=commandServer, dnsStatsNXDomain=dnsStatsNXDomain)
