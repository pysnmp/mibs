...
iso, Bits, Counter32, ObjectIdentity, MibIdentifier, IpAddress, Gauge32, Counter64, enterprises, Integer32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Unsigned32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Bits", "Counter32", "ObjectIdentity", "MibIdentifier", "IpAddress", "Gauge32", "Counter64", "enterprises", "Integer32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Unsigned32", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolEndIP=dhcpPoolEndIP, trapHA=trapHA, trapTFTP=trapTFTP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, lcdDaemon=lcdDaemon, adonis=adonis, dhcpLeaseTable=dhcpLeaseTable, adonisObjects=adonisObjects, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseEndTime=dhcpLeaseEndTime, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseHostname=dhcpLeaseHostname, systemDaemon=systemDaemon, dhcpLeaseEntry=dhcpLeaseEntry, trapSystemDaemon=trapSystemDaemon, dhcp=dhcp, trapDHCP=trapDHCP, dhcpIP=dhcpIP, tftpDaemon=tftpDaemon, trapCommandServer=trapCommandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolTable=dhcpPoolTable, dhcpPoolStartIP=dhcpPoolStartIP, haServiceRunning=haServiceRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolSize=dhcpPoolSize, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFailOverState=dhcpFailOverState, tftp=tftp, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsReferral=dnsStatsReferral, ha=ha, system=system, dnsStatsSuccess=dnsStatsSuccess, PYSNMP_MODULE_ID=adonis, adonisTraps=adonisTraps, dhcpStats=dhcpStats, dnsStats=dnsStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSubnetIP=dhcpPoolSubnetIP, systemState=systemState, trapCommandServerDaemon=trapCommandServerDaemon, trapDNS=trapDNS, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetAlert=dhcpSubnetAlert, trapDNSDaemon=trapDNSDaemon, commandServerDaemon=commandServerDaemon, dhcpFixedIPTable=dhcpFixedIPTable, haServiceNodeType=haServiceNodeType, dhcpDaemon=dhcpDaemon, licenseValid=licenseValid, dhcpSubnetIP=dhcpSubnetIP, dnsDaemon=dnsDaemon, trapSystem=trapSystem, dnsStatsFailure=dnsStatsFailure, dns=dns, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetEntry=dhcpSubnetEntry, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonRunning=dhcpDaemonRunning, licenseExpiry=licenseExpiry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIP=dhcpFixedIP, trapHAServiceFailOver=trapHAServiceFailOver, trapReplicationFailure=trapReplicationFailure, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolEntry=dhcpPoolEntry, haService=haService, tftpDaemonRunning=tftpDaemonRunning, trapReplication=trapReplication, commandServer=commandServer, haReplicationBinding=haReplicationBinding, dhcpConfig=dhcpConfig)
