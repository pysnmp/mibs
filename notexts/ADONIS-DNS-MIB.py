...
Gauge32, ModuleIdentity, TimeTicks, NotificationType, enterprises, ObjectIdentity, IpAddress, Unsigned32, MibIdentifier, Counter32, Counter64, Integer32, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "ModuleIdentity", "TimeTicks", "NotificationType", "enterprises", "ObjectIdentity", "IpAddress", "Unsigned32", "MibIdentifier", "Counter32", "Counter64", "Integer32", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapHAServiceFailOver=trapHAServiceFailOver, tftp=tftp, dhcpFixedIPEntry=dhcpFixedIPEntry, haService=haService, dhcpSubnetMask=dhcpSubnetMask, lcdDaemon=lcdDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonisObjects=adonisObjects, adonisTraps=adonisTraps, trapSystem=trapSystem, commandServer=commandServer, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemon=tftpDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCPDaemon=trapDHCPDaemon, systemDaemon=systemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemon=dhcpDaemon, dnsStatsSuccess=dnsStatsSuccess, dnsStatsFailure=dnsStatsFailure, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseExpiry=licenseExpiry, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapReplication=trapReplication, trapDHCP=trapDHCP, dhcpFailOverState=dhcpFailOverState, haReplicationBinding=haReplicationBinding, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHostname=dhcpLeaseHostname, licenseValid=licenseValid, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseEntry=dhcpLeaseEntry, dnsStats=dnsStats, dnsStatsReferral=dnsStatsReferral, trapDNS=trapDNS, dhcpIP=dhcpIP, trapCommandServer=trapCommandServer, dhcpPoolTable=dhcpPoolTable, dns=dns, dhcpConfig=dhcpConfig, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, ha=ha, systemState=systemState, haServiceNodeType=haServiceNodeType, trapTFTP=trapTFTP, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolSize=dhcpPoolSize, PYSNMP_MODULE_ID=adonis, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, system=system, dhcpStats=dhcpStats, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize, haServiceRunning=haServiceRunning, commandServerDaemon=commandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, adonis=adonis, dnsDaemonRunning=dnsDaemonRunning, trapReplicationFailure=trapReplicationFailure, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetTable=dhcpSubnetTable, dnsStatsRecursion=dnsStatsRecursion, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseTable=dhcpLeaseTable, dhcp=dhcp, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, dnsDaemon=dnsDaemon, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState)
