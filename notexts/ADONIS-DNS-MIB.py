...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetUsed=dhcpSubnetUsed, lcd=lcd, trapReplication=trapReplication, system=system, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetEntry=dhcpSubnetEntry, haServiceRunning=haServiceRunning, tftpDaemonRunning=tftpDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServer=trapCommandServer, trapDNS=trapDNS, haService=haService, systemDaemon=systemDaemon, trapCommandServerDaemon=trapCommandServerDaemon, haServiceNodeType=haServiceNodeType, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIP=dhcpFixedIP, trapTFTP=trapTFTP, tftpDaemon=tftpDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsFailure=dnsStatsFailure, tftp=tftp, commandServerDaemon=commandServerDaemon, dns=dns, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseExpiry=licenseExpiry, dhcpConfig=dhcpConfig, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapReplicationFailure=trapReplicationFailure, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, dhcp=dhcp, lcdDaemon=lcdDaemon, dnsStatsRecursion=dnsStatsRecursion, PYSNMP_MODULE_ID=adonis, trapDNSDaemon=trapDNSDaemon, haReplicationBinding=haReplicationBinding, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, adonis=adonis, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsReferral=dnsStatsReferral, dhcpStats=dhcpStats, adonisObjects=adonisObjects, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemon=dnsDaemon, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEntry=dhcpLeaseEntry, trapSystemDaemon=trapSystemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetIP=dhcpSubnetIP, dnsStats=dnsStats, commandServer=commandServer, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolTable=dhcpPoolTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, licenseValid=licenseValid, dhcpIP=dhcpIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolAlert=dhcpPoolAlert, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, ha=ha, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapSystem=trapSystem, trapHA=trapHA, dhcpDaemon=dhcpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisTraps=adonisTraps, trapDHCPDaemon=trapDHCPDaemon, systemState=systemState, dhcpSubnetMask=dhcpSubnetMask, trapDHCP=trapDHCP, dnsStatsSuccess=dnsStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolSize=dhcpPoolSize)
