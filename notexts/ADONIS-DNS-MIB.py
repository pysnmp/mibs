...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolAlert=dhcpPoolAlert, tftpDaemonRunning=tftpDaemonRunning, adonisObjects=adonisObjects, dns=dns, dnsDaemon=dnsDaemon, haService=haService, trapSystem=trapSystem, dhcpSubnetTable=dhcpSubnetTable, trapReplication=trapReplication, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, haServiceRunning=haServiceRunning, trapDHCPDaemon=trapDHCPDaemon, dhcp=dhcp, licenseExpiry=licenseExpiry, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsNXDomain=dnsStatsNXDomain, trapReplicationFailure=trapReplicationFailure, lcd=lcd, tftp=tftp, adonis=adonis, dhcpDaemon=dhcpDaemon, dhcpLeaseTable=dhcpLeaseTable, dnsStatsNXRRSet=dnsStatsNXRRSet, haReplicationBinding=haReplicationBinding, PYSNMP_MODULE_ID=adonis, systemState=systemState, licenseValid=licenseValid, trapCommandServerDaemon=trapCommandServerDaemon, trapDHCP=trapDHCP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseBindState=dhcpLeaseBindState, trapSystemDaemon=trapSystemDaemon, lcdDaemon=lcdDaemon, dnsStatsRecursion=dnsStatsRecursion, tftpDaemon=tftpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpConfig=dhcpConfig, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSize=dhcpPoolSize, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsReferral=dnsStatsReferral, dhcpSubnetIP=dhcpSubnetIP, trapDNS=trapDNS, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDNSDaemon=trapDNSDaemon, ha=ha, haServiceNodeType=haServiceNodeType, dnsStats=dnsStats, commandServerDaemonRunning=commandServerDaemonRunning, system=system, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpStats=dhcpStats, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonRunning=dnsDaemonRunning, trapTFTP=trapTFTP, dhcpFailOverState=dhcpFailOverState, systemDaemon=systemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsFailure=dnsStatsFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseStartTime=dhcpLeaseStartTime, trapCommandServer=trapCommandServer, adonisTraps=adonisTraps, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolUsed=dhcpPoolUsed, dhcpIP=dhcpIP, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolTable=dhcpPoolTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapHA=trapHA, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFixedIP=dhcpFixedIP, commandServer=commandServer)
