...
MibIdentifier, ObjectIdentity, Integer32, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, TimeTicks, Gauge32, ModuleIdentity, NotificationType, IpAddress, iso, Bits, Unsigned32, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "ObjectIdentity", "Integer32", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "TimeTicks", "Gauge32", "ModuleIdentity", "NotificationType", "IpAddress", "iso", "Bits", "Unsigned32", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemState=systemState, tftpDaemon=tftpDaemon, trapTFTPDaemon=trapTFTPDaemon, dns=dns, dnsStats=dnsStats, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetSize=dhcpSubnetSize, trapDHCP=trapDHCP, systemDaemon=systemDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetIP=dhcpSubnetIP, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetEntry=dhcpSubnetEntry, adonis=adonis, tftpDaemonRunning=tftpDaemonRunning, tftp=tftp, haServiceRunning=haServiceRunning, dhcpSubnetUsed=dhcpSubnetUsed, dhcpConfig=dhcpConfig, licenseValid=licenseValid, licenseExpiry=licenseExpiry, PYSNMP_MODULE_ID=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, trapCommandServerDaemon=trapCommandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisTraps=adonisTraps, ha=ha, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemonRunning=commandServerDaemonRunning, dhcpIP=dhcpIP, dnsDaemon=dnsDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolTable=dhcpPoolTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcp=dhcp, dnsStatsSuccess=dnsStatsSuccess, trapTFTP=trapTFTP, haServiceNodeType=haServiceNodeType, trapCommandServer=trapCommandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNS=trapDNS, dhcpFailOverState=dhcpFailOverState, trapReplicationFailure=trapReplicationFailure, dhcpLeaseBindState=dhcpLeaseBindState, trapSystem=trapSystem, dhcpDaemon=dhcpDaemon, haReplicationBinding=haReplicationBinding, lcd=lcd, commandServerDaemon=commandServerDaemon, system=system, dhcpFixedIP=dhcpFixedIP, dnsStatsReferral=dnsStatsReferral, trapSystemDaemon=trapSystemDaemon, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHostname=dhcpLeaseHostname, commandServer=commandServer, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapHA=trapHA, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSize=dhcpPoolSize, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpStats=dhcpStats, trapReplication=trapReplication, dnsStatsFailure=dnsStatsFailure, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXDomain=dnsStatsNXDomain, haService=haService, trapHAServiceFailOver=trapHAServiceFailOver, lcdDaemon=lcdDaemon, adonisObjects=adonisObjects, dhcpLeaseTable=dhcpLeaseTable)
