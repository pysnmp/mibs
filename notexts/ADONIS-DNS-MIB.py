...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dhcpStats=dhcpStats, tftp=tftp, trapSystem=trapSystem, licenseExpiry=licenseExpiry, dhcpPoolUsed=dhcpPoolUsed, trapReplication=trapReplication, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapReplicationFailure=trapReplicationFailure, dhcpPoolTable=dhcpPoolTable, haService=haService, dhcpFailOverState=dhcpFailOverState, systemDaemon=systemDaemon, dhcpSubnetSize=dhcpSubnetSize, lcdDaemon=lcdDaemon, dnsStatsReferral=dnsStatsReferral, tftpDaemon=tftpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonisTraps=adonisTraps, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemonRunning=tftpDaemonRunning, dhcp=dhcp, dnsStats=dnsStats, commandServer=commandServer, lcd=lcd, licenseValid=licenseValid, dhcpPoolSize=dhcpPoolSize, trapCommandServer=trapCommandServer, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIP=dhcpFixedIP, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTPDaemon=trapTFTPDaemon, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetIP=dhcpSubnetIP, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseHostname=dhcpLeaseHostname, adonisObjects=adonisObjects, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServerDaemon=commandServerDaemon, trapTFTP=trapTFTP, trapDNS=trapDNS, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXDomain=dnsStatsNXDomain, haServiceNodeType=haServiceNodeType, dhcpSubnetTable=dhcpSubnetTable, haServiceRunning=haServiceRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDHCP=trapDHCP, dhcpSubnetAlert=dhcpSubnetAlert, PYSNMP_MODULE_ID=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpIP=dhcpIP, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haReplicationBinding=haReplicationBinding, systemState=systemState, dhcpConfig=dhcpConfig, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemon=dnsDaemon, system=system, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemon=dhcpDaemon, dnsStatsFailure=dnsStatsFailure, dhcpPoolAlert=dhcpPoolAlert, adonis=adonis, dhcpSubnetMask=dhcpSubnetMask, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseBindState=dhcpLeaseBindState, ha=ha, dhcpFixedIPEntry=dhcpFixedIPEntry, dns=dns, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonRunning=dnsDaemonRunning)
