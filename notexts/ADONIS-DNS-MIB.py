...
Counter32, Integer32, Bits, NotificationType, TimeTicks, ModuleIdentity, Counter64, iso, ObjectIdentity, Unsigned32, MibIdentifier, enterprises, IpAddress, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "Integer32", "Bits", "NotificationType", "TimeTicks", "ModuleIdentity", "Counter64", "iso", "ObjectIdentity", "Unsigned32", "MibIdentifier", "enterprises", "IpAddress", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetTable=dhcpSubnetTable, PYSNMP_MODULE_ID=adonis, dhcpLeaseBindState=dhcpLeaseBindState, haReplicationBinding=haReplicationBinding, dhcpPoolTable=dhcpPoolTable, trapDNS=trapDNS, dhcpFixedIP=dhcpFixedIP, trapReplication=trapReplication, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetSize=dhcpSubnetSize, trapSystemDaemon=trapSystemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetIP=dhcpSubnetIP, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemon=commandServerDaemon, dhcpDaemon=dhcpDaemon, dhcpFailOverState=dhcpFailOverState, haService=haService, dnsStatsSuccess=dnsStatsSuccess, haServiceNodeType=haServiceNodeType, systemState=systemState, dnsStats=dnsStats, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEntry=dhcpPoolEntry, lcd=lcd, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, tftpDaemon=tftpDaemon, lcdDaemon=lcdDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, trapReplicationFailure=trapReplicationFailure, system=system, dhcpSubnetUsed=dhcpSubnetUsed, trapSystem=trapSystem, licenseExpiry=licenseExpiry, dhcp=dhcp, dhcpFixedIPEntry=dhcpFixedIPEntry, tftp=tftp, adonis=adonis, adonisTraps=adonisTraps, dhcpConfig=dhcpConfig, trapHA=trapHA, dhcpIP=dhcpIP, trapDNSDaemon=trapDNSDaemon, systemDaemon=systemDaemon, dhcpLeaseHostname=dhcpLeaseHostname, trapDHCP=trapDHCP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolAlert=dhcpPoolAlert, dns=dns, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, trapTFTPDaemon=trapTFTPDaemon, dnsStatsFailure=dnsStatsFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, commandServer=commandServer, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetAlert=dhcpSubnetAlert, licenseValid=licenseValid, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpStats=dhcpStats, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsReferral=dnsStatsReferral, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemon=dnsDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, ha=ha, adonisObjects=adonisObjects, trapCommandServer=trapCommandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonRunning=dhcpDaemonRunning, haServiceRunning=haServiceRunning)
