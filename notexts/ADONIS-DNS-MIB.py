...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNSDaemon=trapDNSDaemon, dhcpConfig=dhcpConfig, tftp=tftp, dnsStats=dnsStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, lcd=lcd, trapCommandServer=trapCommandServer, dnsDaemonRunning=dnsDaemonRunning, trapSystem=trapSystem, trapSystemDaemon=trapSystemDaemon, haReplicationBinding=haReplicationBinding, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolUsed=dhcpPoolUsed, tftpDaemon=tftpDaemon, trapReplication=trapReplication, commandServer=commandServer, trapTFTP=trapTFTP, dhcpSubnetIP=dhcpSubnetIP, systemDaemon=systemDaemon, dhcpStats=dhcpStats, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dnsDaemon=dnsDaemon, dhcpSubnetMask=dhcpSubnetMask, trapHAServiceFailOver=trapHAServiceFailOver, haServiceRunning=haServiceRunning, dhcpIP=dhcpIP, adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, dhcpPoolStartIP=dhcpPoolStartIP, licenseExpiry=licenseExpiry, adonisObjects=adonisObjects, dhcpPoolEndIP=dhcpPoolEndIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemon=dhcpDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcp=dhcp, dhcpFixedIPTable=dhcpFixedIPTable, ha=ha, lcdDaemon=lcdDaemon, haServiceNodeType=haServiceNodeType, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseValid=licenseValid, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDHCP=trapDHCP, dhcpPoolTable=dhcpPoolTable, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, system=system, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapTFTPDaemon=trapTFTPDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEntry=dhcpPoolEntry, trapReplicationFailure=trapReplicationFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemon=commandServerDaemon, dnsStatsFailure=dnsStatsFailure, PYSNMP_MODULE_ID=adonis, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFailOverState=dhcpFailOverState, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, adonis=adonis, dhcpFixedIP=dhcpFixedIP, dnsStatsRecursion=dnsStatsRecursion, systemState=systemState, trapDNS=trapDNS, dhcpSubnetSize=dhcpSubnetSize, trapHA=trapHA, dns=dns)
