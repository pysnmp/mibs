...
MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Counter64, MibIdentifier, iso, Unsigned32, Integer32, NotificationType, Gauge32, IpAddress, TimeTicks, ObjectIdentity, ModuleIdentity, Bits, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Counter64", "MibIdentifier", "iso", "Unsigned32", "Integer32", "NotificationType", "Gauge32", "IpAddress", "TimeTicks", "ObjectIdentity", "ModuleIdentity", "Bits", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcd=lcd, dnsStatsRecursion=dnsStatsRecursion, PYSNMP_MODULE_ID=adonis, dhcpLeaseStartTime=dhcpLeaseStartTime, ha=ha, trapReplication=trapReplication, commandServer=commandServer, system=system, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemon=dhcpDaemon, tftp=tftp, haReplicationBinding=haReplicationBinding, dhcpSubnetIP=dhcpSubnetIP, haServiceRunning=haServiceRunning, dhcpPoolUsed=dhcpPoolUsed, licenseValid=licenseValid, dns=dns, dhcpFixedIPEntry=dhcpFixedIPEntry, trapReplicationFailure=trapReplicationFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolTable=dhcpPoolTable, tftpDaemon=tftpDaemon, trapCommandServer=trapCommandServer, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServerDaemon=trapCommandServerDaemon, licenseExpiry=licenseExpiry, dhcpPoolEndIP=dhcpPoolEndIP, haService=haService, commandServerDaemon=commandServerDaemon, adonisObjects=adonisObjects, adonisTraps=adonisTraps, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTable=dhcpLeaseTable, dnsStatsFailure=dnsStatsFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetTable=dhcpSubnetTable, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIP=dhcpFixedIP, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEntry=dhcpLeaseEntry, tftpDaemonRunning=tftpDaemonRunning, trapDHCP=trapDHCP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, systemDaemon=systemDaemon, dnsStats=dnsStats, dhcpPoolAlert=dhcpPoolAlert, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsReferral=dnsStatsReferral, commandServerDaemonRunning=commandServerDaemonRunning, lcdDaemon=lcdDaemon, trapTFTP=trapTFTP, dhcpStats=dhcpStats, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHA=trapHA, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpConfig=dhcpConfig, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetEntry=dhcpSubnetEntry, dhcp=dhcp, trapDNSDaemon=trapDNSDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetUsed=dhcpSubnetUsed, systemState=systemState, trapHAServiceFailOver=trapHAServiceFailOver, trapDNS=trapDNS, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFailOverState=dhcpFailOverState, dnsDaemon=dnsDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpIP=dhcpIP, haServiceNodeType=haServiceNodeType, adonis=adonis, dnsDaemonRunning=dnsDaemonRunning, trapSystem=trapSystem, dhcpPoolSize=dhcpPoolSize, trapSystemDaemon=trapSystemDaemon)
