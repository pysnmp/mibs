...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetAlert=dhcpSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolEndIP=dhcpPoolEndIP, adonis=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemon=dhcpDaemon, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, tftpDaemon=tftpDaemon, dhcpSubnetSize=dhcpSubnetSize, dnsStatsReferral=dnsStatsReferral, dnsStatsSuccess=dnsStatsSuccess, dnsStatsFailure=dnsStatsFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisTraps=adonisTraps, dhcp=dhcp, dhcpPoolTable=dhcpPoolTable, system=system, haServiceNodeType=haServiceNodeType, PYSNMP_MODULE_ID=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, commandServer=commandServer, systemState=systemState, lcdDaemon=lcdDaemon, trapHA=trapHA, tftp=tftp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCPDaemon=trapDHCPDaemon, dnsDaemon=dnsDaemon, dhcpFixedIP=dhcpFixedIP, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseTable=dhcpLeaseTable, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemon=commandServerDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetIP=dhcpSubnetIP, haServiceRunning=haServiceRunning, ha=ha, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, licenseValid=licenseValid, haReplicationBinding=haReplicationBinding, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSize=dhcpPoolSize, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseExpiry=licenseExpiry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStats=dnsStats, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetMask=dhcpSubnetMask, trapDNS=trapDNS, trapDNSDaemon=trapDNSDaemon, trapReplicationFailure=trapReplicationFailure, commandServerDaemonRunning=commandServerDaemonRunning, haService=haService, dhcpConfig=dhcpConfig, dhcpStats=dhcpStats, trapTFTP=trapTFTP, trapSystem=trapSystem, lcd=lcd, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetTable=dhcpSubnetTable, trapReplication=trapReplication, adonisObjects=adonisObjects, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, dns=dns, trapCommandServer=trapCommandServer, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, systemDaemon=systemDaemon, trapDHCP=trapDHCP, dhcpPoolEntry=dhcpPoolEntry)
