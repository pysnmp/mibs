...
Bits, Integer32, Unsigned32, TimeTicks, enterprises, Counter32, ModuleIdentity, iso, MibIdentifier, NotificationType, Gauge32, ObjectIdentity, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Integer32", "Unsigned32", "TimeTicks", "enterprises", "Counter32", "ModuleIdentity", "iso", "MibIdentifier", "NotificationType", "Gauge32", "ObjectIdentity", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonisTraps=adonisTraps, trapCommandServer=trapCommandServer, trapDNS=trapDNS, trapDNSDaemon=trapDNSDaemon, dhcpFailOverState=dhcpFailOverState, dhcpPoolTable=dhcpPoolTable, dhcpDaemon=dhcpDaemon, ha=ha, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIP=dhcpFixedIP, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsFailure=dnsStatsFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftp=tftp, dnsDaemon=dnsDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcp=dhcp, dhcpLeaseEndTime=dhcpLeaseEndTime, adonis=adonis, licenseValid=licenseValid, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapCommandServerDaemon=trapCommandServerDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemon=commandServerDaemon, lcd=lcd, haServiceNodeType=haServiceNodeType, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsRecursion=dnsStatsRecursion, dhcpIP=dhcpIP, licenseExpiry=licenseExpiry, system=system, trapSystem=trapSystem, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpStats=dhcpStats, trapSystemDaemon=trapSystemDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFixedIPEntry=dhcpFixedIPEntry, trapHA=trapHA, dhcpPoolUsed=dhcpPoolUsed, trapTFTP=trapTFTP, haService=haService, dnsStatsReferral=dnsStatsReferral, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapReplicationFailure=trapReplicationFailure, adonisObjects=adonisObjects, trapDHCP=trapDHCP, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dnsStats=dnsStats, dhcpConfig=dhcpConfig, dhcpLeaseHostname=dhcpLeaseHostname, systemState=systemState, PYSNMP_MODULE_ID=adonis, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolAlert=dhcpPoolAlert, lcdDaemon=lcdDaemon, tftpDaemon=tftpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetIP=dhcpSubnetIP, haServiceRunning=haServiceRunning, dns=dns, systemDaemon=systemDaemon, trapReplication=trapReplication, commandServer=commandServer, haReplicationBinding=haReplicationBinding, dhcpPoolSize=dhcpPoolSize)
