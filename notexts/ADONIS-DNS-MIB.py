...
ObjectIdentity, MibIdentifier, enterprises, NotificationType, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, Counter32, Counter64, iso, Bits, Gauge32, Integer32, IpAddress, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "MibIdentifier", "enterprises", "NotificationType", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "Counter32", "Counter64", "iso", "Bits", "Gauge32", "Integer32", "IpAddress", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonisTraps=adonisTraps, ha=ha, dhcpPoolEntry=dhcpPoolEntry, trapReplicationFailure=trapReplicationFailure, dhcpStats=dhcpStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNS=trapDNS, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, haServiceNodeType=haServiceNodeType, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXRRSet=dnsStatsNXRRSet, licenseExpiry=licenseExpiry, dhcpConfig=dhcpConfig, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolSize=dhcpPoolSize, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIP=dhcpFixedIP, adonis=adonis, commandServerDaemon=commandServerDaemon, PYSNMP_MODULE_ID=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, trapSystem=trapSystem, dnsStatsSuccess=dnsStatsSuccess, dnsDaemon=dnsDaemon, adonisObjects=adonisObjects, dnsStatsRecursion=dnsStatsRecursion, tftpDaemon=tftpDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpIP=dhcpIP, dns=dns, lcdDaemon=lcdDaemon, dhcpFailOverState=dhcpFailOverState, commandServer=commandServer, dhcpPoolTable=dhcpPoolTable, trapSystemDaemon=trapSystemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseEntry=dhcpLeaseEntry, trapTFTPDaemon=trapTFTPDaemon, trapDHCP=trapDHCP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsReferral=dnsStatsReferral, trapHA=trapHA, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemon=dhcpDaemon, systemDaemon=systemDaemon, trapReplication=trapReplication, systemState=systemState, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceRunning=haServiceRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStats=dnsStats, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, licenseValid=licenseValid, dhcpSubnetSize=dhcpSubnetSize, trapDNSDaemon=trapDNSDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseTable=dhcpLeaseTable, dnsStatsFailure=dnsStatsFailure, dhcpSubnetEntry=dhcpSubnetEntry, system=system, trapTFTP=trapTFTP, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, lcd=lcd, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetAlert=dhcpSubnetAlert, dhcp=dhcp, dhcpPoolUsed=dhcpPoolUsed, trapCommandServer=trapCommandServer, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetMask=dhcpSubnetMask, tftp=tftp, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseHostname=dhcpLeaseHostname, trapDHCPDaemon=trapDHCPDaemon)
