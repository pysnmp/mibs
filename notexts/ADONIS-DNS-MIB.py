...
Counter64, NotificationType, Integer32, iso, IpAddress, Counter32, ObjectIdentity, TimeTicks, Unsigned32, enterprises, ModuleIdentity, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Counter64", "NotificationType", "Integer32", "iso", "IpAddress", "Counter32", "ObjectIdentity", "TimeTicks", "Unsigned32", "enterprises", "ModuleIdentity", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemDaemon=systemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceRunning=haServiceRunning, dnsDaemon=dnsDaemon, licenseExpiry=licenseExpiry, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServer=commandServer, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapCommandServer=trapCommandServer, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseBindState=dhcpLeaseBindState, systemState=systemState, commandServerDaemon=commandServerDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haService=haService, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseHostname=dhcpLeaseHostname, tftp=tftp, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetIP=dhcpSubnetIP, dnsStatsFailure=dnsStatsFailure, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolTable=dhcpPoolTable, adonis=adonis, adonisTraps=adonisTraps, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetMask=dhcpSubnetMask, trapTFTPDaemon=trapTFTPDaemon, haReplicationBinding=haReplicationBinding, ha=ha, trapReplication=trapReplication, dhcpFailOverState=dhcpFailOverState, trapTFTP=trapTFTP, adonisObjects=adonisObjects, dhcpPoolEndIP=dhcpPoolEndIP, trapCommandServerDaemon=trapCommandServerDaemon, system=system, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNS=trapDNS, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpConfig=dhcpConfig, dhcpPoolUsed=dhcpPoolUsed, haServiceNodeType=haServiceNodeType, tftpDaemon=tftpDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEntry=dhcpLeaseEntry, licenseValid=licenseValid, dns=dns, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemon=dhcpDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dhcpIP=dhcpIP, lcdDaemon=lcdDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsReferral=dnsStatsReferral, trapDHCP=trapDHCP, commandServerDaemonRunning=commandServerDaemonRunning, trapSystemDaemon=trapSystemDaemon, lcd=lcd, dhcpDaemonRunning=dhcpDaemonRunning, trapSystem=trapSystem, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCPDaemon=trapDHCPDaemon, dhcpStats=dhcpStats, trapHA=trapHA, trapReplicationFailure=trapReplicationFailure, dhcpPoolEntry=dhcpPoolEntry, dhcp=dhcp, dnsStats=dnsStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNSDaemon=trapDNSDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonDebugLevel=dnsDaemonDebugLevel, PYSNMP_MODULE_ID=adonis, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIP=dhcpFixedIP)
