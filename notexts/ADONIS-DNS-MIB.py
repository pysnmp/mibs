...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTPDaemon=trapTFTPDaemon, dnsStats=dnsStats, dhcpSubnetTable=dhcpSubnetTable, trapCommandServer=trapCommandServer, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, ha=ha, haServiceNodeType=haServiceNodeType, dhcpFixedIP=dhcpFixedIP, trapSystem=trapSystem, dhcpFailOverState=dhcpFailOverState, adonis=adonis, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcp=dhcp, lcdDaemon=lcdDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemon=commandServerDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEntry=dhcpPoolEntry, tftpDaemon=tftpDaemon, dnsStatsRecursion=dnsStatsRecursion, trapReplicationFailure=trapReplicationFailure, tftp=tftp, trapDNSDaemon=trapDNSDaemon, dhcpPoolSize=dhcpPoolSize, dnsDaemon=dnsDaemon, trapDNS=trapDNS, trapSystemDaemon=trapSystemDaemon, systemState=systemState, PYSNMP_MODULE_ID=adonis, system=system, commandServerDaemonRunning=commandServerDaemonRunning, dns=dns, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseEntry=dhcpLeaseEntry, adonisTraps=adonisTraps, systemDaemon=systemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpStats=dhcpStats, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolAlert=dhcpPoolAlert, commandServer=commandServer, dhcpDaemonRunning=dhcpDaemonRunning, haService=haService, trapHA=trapHA, trapDHCPDaemon=trapDHCPDaemon, dhcpConfig=dhcpConfig, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolTable=dhcpPoolTable, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTP=trapTFTP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, haReplicationBinding=haReplicationBinding, dnsStatsFailure=dnsStatsFailure, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolUsed=dhcpPoolUsed, dnsStatsReferral=dnsStatsReferral, trapReplication=trapReplication, licenseExpiry=licenseExpiry, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEndIP=dhcpPoolEndIP, trapHAServiceFailOver=trapHAServiceFailOver, adonisObjects=adonisObjects, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemon=dhcpDaemon, haServiceRunning=haServiceRunning, licenseValid=licenseValid, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpIP=dhcpIP, lcd=lcd, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDHCP=trapDHCP)
