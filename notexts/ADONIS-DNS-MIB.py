...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsFailure=dnsStatsFailure, commandServerDaemonRunning=commandServerDaemonRunning, dhcp=dhcp, trapDNS=trapDNS, trapDHCP=trapDHCP, dhcpPoolEndIP=dhcpPoolEndIP, adonis=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, ha=ha, trapDNSDaemon=trapDNSDaemon, licenseValid=licenseValid, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetUsed=dhcpSubnetUsed, commandServer=commandServer, tftpDaemonRunning=tftpDaemonRunning, dhcpStats=dhcpStats, dhcpSubnetMask=dhcpSubnetMask, systemState=systemState, dhcpPoolSize=dhcpPoolSize, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetAlert=dhcpSubnetAlert, haService=haService, dhcpPoolEntry=dhcpPoolEntry, lcdDaemon=lcdDaemon, trapCommandServerDaemon=trapCommandServerDaemon, licenseExpiry=licenseExpiry, dhcpConfig=dhcpConfig, trapTFTP=trapTFTP, haReplicationBinding=haReplicationBinding, trapTFTPDaemon=trapTFTPDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpIP=dhcpIP, PYSNMP_MODULE_ID=adonis, trapHAServiceFailOver=trapHAServiceFailOver, trapReplicationFailure=trapReplicationFailure, dhcpPoolStartIP=dhcpPoolStartIP, system=system, trapReplication=trapReplication, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemon=dhcpDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIPEntry=dhcpFixedIPEntry, trapSystem=trapSystem, haServiceNodeType=haServiceNodeType, tftp=tftp, commandServerDaemon=commandServerDaemon, dnsDaemon=dnsDaemon, adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetIP=dhcpSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolAlert=dhcpPoolAlert, dhcpFailOverState=dhcpFailOverState, tftpDaemon=tftpDaemon, haServiceRunning=haServiceRunning, lcd=lcd, trapSystemDaemon=trapSystemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dns=dns, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonRunning=dnsDaemonRunning, systemDaemon=systemDaemon, trapHA=trapHA, trapCommandServer=trapCommandServer, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsRecursion=dnsStatsRecursion, adonisObjects=adonisObjects, dnsStats=dnsStats, dnsStatsReferral=dnsStatsReferral, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolTable=dhcpPoolTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP)
