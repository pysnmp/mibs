...
TimeTicks, Counter32, Counter64, IpAddress, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, ModuleIdentity, Bits, ObjectIdentity, Unsigned32, MibIdentifier, enterprises, Integer32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Counter32", "Counter64", "IpAddress", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "ModuleIdentity", "Bits", "ObjectIdentity", "Unsigned32", "MibIdentifier", "enterprises", "Integer32", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haServiceRunning=haServiceRunning, trapReplicationFailure=trapReplicationFailure, dhcpFixedIPTable=dhcpFixedIPTable, adonisTraps=adonisTraps, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsFailure=dnsStatsFailure, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dns=dns, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseBindState=dhcpLeaseBindState, systemDaemon=systemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, dhcpFailOverState=dhcpFailOverState, ha=ha, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsNXRRSet=dnsStatsNXRRSet, licenseValid=licenseValid, trapSystemDaemon=trapSystemDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolSize=dhcpPoolSize, haReplicationBinding=haReplicationBinding, lcd=lcd, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, haServiceNodeType=haServiceNodeType, dnsStatsSuccess=dnsStatsSuccess, licenseExpiry=licenseExpiry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetSize=dhcpSubnetSize, dhcpStats=dhcpStats, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNSDaemon=trapDNSDaemon, trapTFTP=trapTFTP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, tftpDaemon=tftpDaemon, dhcpIP=dhcpIP, trapDHCPDaemon=trapDHCPDaemon, commandServer=commandServer, dnsStatsRecursion=dnsStatsRecursion, PYSNMP_MODULE_ID=adonis, dnsStats=dnsStats, systemState=systemState, adonisObjects=adonisObjects, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemon=dnsDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIPEntry=dhcpFixedIPEntry, system=system, dhcpPoolUsed=dhcpPoolUsed, lcdDaemon=lcdDaemon, dhcpDaemon=dhcpDaemon, dhcpLeaseTable=dhcpLeaseTable, trapDNS=trapDNS, trapHA=trapHA, commandServerDaemonRunning=commandServerDaemonRunning, trapCommandServer=trapCommandServer, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolAlert=dhcpPoolAlert, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsReferral=dnsStatsReferral, dhcpSubnetMask=dhcpSubnetMask, trapCommandServerDaemon=trapCommandServerDaemon, trapSystem=trapSystem, trapDHCP=trapDHCP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftp=tftp, dhcpSubnetUsed=dhcpSubnetUsed, haService=haService, adonis=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonRunning=dnsDaemonRunning, trapReplication=trapReplication, commandServerDaemon=commandServerDaemon, dhcpConfig=dhcpConfig, dhcp=dhcp)
