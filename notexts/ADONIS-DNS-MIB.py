...
Gauge32, ModuleIdentity, Bits, MibIdentifier, Unsigned32, Counter64, enterprises, Integer32, iso, ObjectIdentity, IpAddress, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "ModuleIdentity", "Bits", "MibIdentifier", "Unsigned32", "Counter64", "enterprises", "Integer32", "iso", "ObjectIdentity", "IpAddress", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", lcdDaemon=lcdDaemon, haServiceRunning=haServiceRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNS=trapDNS, trapSystemDaemon=trapSystemDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsSuccess=dnsStatsSuccess, dnsDaemon=dnsDaemon, dhcpFailOverState=dhcpFailOverState, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetTable=dhcpSubnetTable, trapSystem=trapSystem, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetUsed=dhcpSubnetUsed, ha=ha, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapReplication=trapReplication, PYSNMP_MODULE_ID=adonis, adonis=adonis, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemon=dhcpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIPTable=dhcpFixedIPTable, haReplicationBinding=haReplicationBinding, dhcpStats=dhcpStats, dhcpDaemonRunning=dhcpDaemonRunning, tftpDaemon=tftpDaemon, systemDaemon=systemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, trapDNSDaemon=trapDNSDaemon, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemonRunning=commandServerDaemonRunning, lcd=lcd, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, adonisTraps=adonisTraps, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolUsed=dhcpPoolUsed, adonisObjects=adonisObjects, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolSize=dhcpPoolSize, licenseValid=licenseValid, dnsStats=dnsStats, haServiceNodeType=haServiceNodeType, dnsStatsReferral=dnsStatsReferral, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, tftp=tftp, trapCommandServer=trapCommandServer, licenseExpiry=licenseExpiry, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, commandServer=commandServer, tftpDaemonRunning=tftpDaemonRunning, dhcp=dhcp, commandServerDaemon=commandServerDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseHostname=dhcpLeaseHostname, trapHA=trapHA, trapDHCP=trapDHCP, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTP=trapTFTP, dhcpFixedIP=dhcpFixedIP, trapReplicationFailure=trapReplicationFailure, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsFailure=dnsStatsFailure, system=system, dns=dns, dhcpIP=dhcpIP, dhcpSubnetMask=dhcpSubnetMask, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEntry=dhcpPoolEntry, systemState=systemState, haService=haService, dhcpPoolTable=dhcpPoolTable, trapCommandServerDaemon=trapCommandServerDaemon, dhcpConfig=dhcpConfig)
