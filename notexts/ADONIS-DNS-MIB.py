...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapCommandServer=trapCommandServer, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolSize=dhcpPoolSize, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsSuccess=dnsStatsSuccess, tftp=tftp, lcdDaemon=lcdDaemon, dnsDaemon=dnsDaemon, dns=dns, dhcpFixedIP=dhcpFixedIP, dnsStats=dnsStats, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haService=haService, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsFailure=dnsStatsFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPEntry=dhcpFixedIPEntry, haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, lcd=lcd, dhcpPoolUsed=dhcpPoolUsed, commandServer=commandServer, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseBindState=dhcpLeaseBindState, haServiceRunning=haServiceRunning, trapDNS=trapDNS, tftpDaemonRunning=tftpDaemonRunning, systemState=systemState, trapDHCPDaemon=trapDHCPDaemon, commandServerDaemon=commandServerDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, ha=ha, trapSystemDaemon=trapSystemDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisTraps=adonisTraps, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, trapSystem=trapSystem, trapHA=trapHA, haServiceNodeType=haServiceNodeType, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHostname=dhcpLeaseHostname, trapReplicationFailure=trapReplicationFailure, trapDNSDaemon=trapDNSDaemon, dnsStatsReferral=dnsStatsReferral, dhcpDaemon=dhcpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpStats=dhcpStats, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpIP=dhcpIP, dhcpSubnetTable=dhcpSubnetTable, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpConfig=dhcpConfig, dhcp=dhcp, licenseExpiry=licenseExpiry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonRunning=dhcpDaemonRunning, systemDaemon=systemDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, adonis=adonis, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFailOverState=dhcpFailOverState, trapDHCP=trapDHCP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, adonisObjects=adonisObjects, trapReplication=trapReplication, PYSNMP_MODULE_ID=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseValid=licenseValid, trapHAServiceFailOver=trapHAServiceFailOver, system=system, trapTFTP=trapTFTP, tftpDaemon=tftpDaemon, dhcpPoolTable=dhcpPoolTable)
