...
ModuleIdentity, ObjectIdentity, iso, enterprises, Counter64, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Unsigned32, Gauge32, NotificationType, Counter32, MibIdentifier, Bits, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "ObjectIdentity", "iso", "enterprises", "Counter64", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Unsigned32", "Gauge32", "NotificationType", "Counter32", "MibIdentifier", "Bits", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSize=dhcpPoolSize, adonisTraps=adonisTraps, commandServerDaemonRunning=commandServerDaemonRunning, licenseValid=licenseValid, haService=haService, PYSNMP_MODULE_ID=adonis, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetIP=dhcpSubnetIP, tftpDaemonRunning=tftpDaemonRunning, dhcp=dhcp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServer=commandServer, tftpDaemon=tftpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dnsStats=dnsStats, lcd=lcd, lcdDaemon=lcdDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFixedIPEntry=dhcpFixedIPEntry, trapHA=trapHA, dhcpConfig=dhcpConfig, trapCommandServerDaemon=trapCommandServerDaemon, trapSystemDaemon=trapSystemDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, system=system, dhcpFixedIP=dhcpFixedIP, trapDHCP=trapDHCP, dhcpSubnetAlert=dhcpSubnetAlert, trapDNSDaemon=trapDNSDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dns=dns, dhcpFailOverState=dhcpFailOverState, dhcpPoolUsed=dhcpPoolUsed, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsNXRRSet=dnsStatsNXRRSet, haServiceNodeType=haServiceNodeType, trapDNS=trapDNS, dnsDaemon=dnsDaemon, trapReplication=trapReplication, trapSystem=trapSystem, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXDomain=dnsStatsNXDomain, systemState=systemState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolStartIP=dhcpPoolStartIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftp=tftp, dhcpPoolEntry=dhcpPoolEntry, adonis=adonis, haServiceRunning=haServiceRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolAlert=dhcpPoolAlert, dnsStatsFailure=dnsStatsFailure, dhcpIP=dhcpIP, dnsStatsReferral=dnsStatsReferral, trapReplicationFailure=trapReplicationFailure, trapCommandServer=trapCommandServer, dhcpDaemonRunning=dhcpDaemonRunning, haReplicationBinding=haReplicationBinding, adonisObjects=adonisObjects, dhcpPoolTable=dhcpPoolTable, dhcpDaemon=dhcpDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpLeaseBindState=dhcpLeaseBindState, licenseExpiry=licenseExpiry, dhcpStats=dhcpStats, systemDaemon=systemDaemon, trapDHCPDaemon=trapDHCPDaemon, ha=ha, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseHostname=dhcpLeaseHostname)
