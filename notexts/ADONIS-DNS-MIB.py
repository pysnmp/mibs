...
Gauge32, NotificationType, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, MibIdentifier, Unsigned32, ObjectIdentity, Counter64, Counter32, iso, ModuleIdentity, enterprises, TimeTicks, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "NotificationType", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "MibIdentifier", "Unsigned32", "ObjectIdentity", "Counter64", "Counter32", "iso", "ModuleIdentity", "enterprises", "TimeTicks", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsReferral=dnsStatsReferral, dhcpFailOverState=dhcpFailOverState, dhcpStats=dhcpStats, dhcpLeaseBindState=dhcpLeaseBindState, adonis=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseEntry=dhcpLeaseEntry, ha=ha, dhcpConfig=dhcpConfig, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemon=commandServerDaemon, dhcpIP=dhcpIP, trapReplication=trapReplication, licenseExpiry=licenseExpiry, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolAlert=dhcpPoolAlert, dnsStatsFailure=dnsStatsFailure, tftpDaemonRunning=tftpDaemonRunning, systemState=systemState, dhcpSubnetAlert=dhcpSubnetAlert, adonisObjects=adonisObjects, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, PYSNMP_MODULE_ID=adonis, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseEndTime=dhcpLeaseEndTime, haService=haService, dnsDaemon=dnsDaemon, haReplicationBinding=haReplicationBinding, dnsDaemonRunning=dnsDaemonRunning, dhcp=dhcp, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemon=dhcpDaemon, tftpDaemon=tftpDaemon, adonisTraps=adonisTraps, commandServer=commandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolUsed=dhcpPoolUsed, licenseValid=licenseValid, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolStartIP=dhcpPoolStartIP, dns=dns, trapCommandServer=trapCommandServer, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIP=dhcpFixedIP, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize, dnsStats=dnsStats, trapDNSDaemon=trapDNSDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, haServiceRunning=haServiceRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPTable=dhcpFixedIPTable, trapSystem=trapSystem, haServiceNodeType=haServiceNodeType, trapHA=trapHA, dhcpPoolTable=dhcpPoolTable, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemDaemon=systemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, system=system, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCP=trapDHCP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, trapTFTP=trapTFTP, tftp=tftp, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolSize=dhcpPoolSize, dhcpPoolEndIP=dhcpPoolEndIP, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapCommandServerDaemon=trapCommandServerDaemon, trapReplicationFailure=trapReplicationFailure, trapDNS=trapDNS, trapSystemDaemon=trapSystemDaemon, dhcpLeaseHostname=dhcpLeaseHostname)
