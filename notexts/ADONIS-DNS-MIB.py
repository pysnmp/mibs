...
ModuleIdentity, Bits, MibIdentifier, Unsigned32, IpAddress, Gauge32, Counter32, ObjectIdentity, Integer32, Counter64, NotificationType, iso, enterprises, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Bits", "MibIdentifier", "Unsigned32", "IpAddress", "Gauge32", "Counter32", "ObjectIdentity", "Integer32", "Counter64", "NotificationType", "iso", "enterprises", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemon=dnsDaemon, commandServerDaemon=commandServerDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dns=dns, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIP=dhcpFixedIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcp=dhcp, trapDHCPDaemon=trapDHCPDaemon, trapSystem=trapSystem, dhcpConfig=dhcpConfig, adonisObjects=adonisObjects, dhcpDaemon=dhcpDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseTable=dhcpLeaseTable, lcdDaemon=lcdDaemon, systemDaemon=systemDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetMask=dhcpSubnetMask, PYSNMP_MODULE_ID=adonis, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetUsed=dhcpSubnetUsed, licenseExpiry=licenseExpiry, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemState=systemState, dhcpIP=dhcpIP, haService=haService, dhcpPoolEndIP=dhcpPoolEndIP, trapReplication=trapReplication, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetIP=dhcpSubnetIP, trapDNSDaemon=trapDNSDaemon, licenseValid=licenseValid, ha=ha, trapCommandServer=trapCommandServer, dhcpFailOverState=dhcpFailOverState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEntry=dhcpPoolEntry, dnsStatsFailure=dnsStatsFailure, commandServerDaemonRunning=commandServerDaemonRunning, trapDHCP=trapDHCP, dnsStatsRecursion=dnsStatsRecursion, commandServer=commandServer, dnsStats=dnsStats, haReplicationBinding=haReplicationBinding, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisTraps=adonisTraps, haServiceRunning=haServiceRunning, tftp=tftp, trapReplicationFailure=trapReplicationFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, trapSystemDaemon=trapSystemDaemon, system=system, dnsStatsSuccess=dnsStatsSuccess, dnsStatsReferral=dnsStatsReferral, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseEndTime=dhcpLeaseEndTime, haServiceNodeType=haServiceNodeType, trapHA=trapHA, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, adonis=adonis, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpStats=dhcpStats, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDNS=trapDNS, tftpDaemon=tftpDaemon, trapTFTP=trapTFTP, dhcpPoolTable=dhcpPoolTable, trapCommandServerDaemon=trapCommandServerDaemon, lcd=lcd, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetAlert=dhcpSubnetAlert)
