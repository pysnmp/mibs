...
enterprises, ObjectIdentity, MibIdentifier, TimeTicks, NotificationType, Unsigned32, Counter32, iso, Integer32, Bits, Gauge32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "ObjectIdentity", "MibIdentifier", "TimeTicks", "NotificationType", "Unsigned32", "Counter32", "iso", "Integer32", "Bits", "Gauge32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsReferral=dnsStatsReferral, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemon=dnsDaemon, tftpDaemon=tftpDaemon, haServiceRunning=haServiceRunning, trapSystem=trapSystem, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolUsed=dhcpPoolUsed, haReplicationBinding=haReplicationBinding, dhcpStats=dhcpStats, trapDHCP=trapDHCP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolSize=dhcpPoolSize, trapHA=trapHA, dhcpLeaseEntry=dhcpLeaseEntry, dns=dns, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplication=trapReplication, adonis=adonis, dhcpDaemon=dhcpDaemon, lcdDaemon=lcdDaemon, licenseValid=licenseValid, adonisObjects=adonisObjects, haServiceNodeType=haServiceNodeType, dhcpFailOverState=dhcpFailOverState, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEndIP=dhcpPoolEndIP, systemState=systemState, haService=haService, dhcpFixedIP=dhcpFixedIP, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetSize=dhcpSubnetSize, trapCommandServer=trapCommandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNSDaemon=trapDNSDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolTable=dhcpPoolTable, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDHCPDaemon=trapDHCPDaemon, tftp=tftp, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNS=trapDNS, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpFixedIPTable=dhcpFixedIPTable, ha=ha, dhcpConfig=dhcpConfig, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, trapSystemDaemon=trapSystemDaemon, licenseExpiry=licenseExpiry, trapTFTP=trapTFTP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapReplicationFailure=trapReplicationFailure, lcd=lcd, systemDaemon=systemDaemon, dhcpIP=dhcpIP, PYSNMP_MODULE_ID=adonis, commandServer=commandServer, dhcp=dhcp, dnsStatsFailure=dnsStatsFailure, system=system, adonisTraps=adonisTraps, dnsStats=dnsStats)
