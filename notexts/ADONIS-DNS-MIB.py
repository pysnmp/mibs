...
MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, iso, Counter64, MibIdentifier, IpAddress, Counter32, TimeTicks, Integer32, NotificationType, ModuleIdentity, Gauge32, Unsigned32, ObjectIdentity, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "iso", "Counter64", "MibIdentifier", "IpAddress", "Counter32", "TimeTicks", "Integer32", "NotificationType", "ModuleIdentity", "Gauge32", "Unsigned32", "ObjectIdentity", "Bits")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemon=dnsDaemon, dhcpFixedIPTable=dhcpFixedIPTable, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetEntry=dhcpSubnetEntry, dns=dns, licenseExpiry=licenseExpiry, dhcpFixedIP=dhcpFixedIP, tftp=tftp, dhcpConfig=dhcpConfig, lcd=lcd, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, lcdDaemon=lcdDaemon, dhcpStats=dhcpStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetIP=dhcpSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCPDaemon=trapDHCPDaemon, tftpDaemonRunning=tftpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haServiceRunning=haServiceRunning, trapDNS=trapDNS, adonis=adonis, dhcpIP=dhcpIP, licenseValid=licenseValid, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dnsStatsFailure=dnsStatsFailure, dnsStatsReferral=dnsStatsReferral, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolSize=dhcpPoolSize, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsRecursion=dnsStatsRecursion, trapTFTP=trapTFTP, adonisTraps=adonisTraps, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolTable=dhcpPoolTable, dhcpPoolAlert=dhcpPoolAlert, trapHA=trapHA, systemDaemon=systemDaemon, systemState=systemState, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEndIP=dhcpPoolEndIP, commandServer=commandServer, dhcpLeaseTable=dhcpLeaseTable, dhcpFailOverState=dhcpFailOverState, trapCommandServer=trapCommandServer, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseHostname=dhcpLeaseHostname, trapSystem=trapSystem, trapDHCP=trapDHCP, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, system=system, dnsStatsNXDomain=dnsStatsNXDomain, haService=haService, PYSNMP_MODULE_ID=adonis, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemon=dhcpDaemon, trapDNSDaemon=trapDNSDaemon, trapReplication=trapReplication, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, haReplicationBinding=haReplicationBinding, tftpDaemon=tftpDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsStats=dnsStats, dhcp=dhcp, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseEndTime=dhcpLeaseEndTime, adonisObjects=adonisObjects, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, ha=ha, commandServerDaemon=commandServerDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapReplicationFailure=trapReplicationFailure)
