...
enterprises, ObjectIdentity, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Bits, Counter32, IpAddress, ModuleIdentity, iso, TimeTicks, Unsigned32, Gauge32, Counter64, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "ObjectIdentity", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Bits", "Counter32", "IpAddress", "ModuleIdentity", "iso", "TimeTicks", "Unsigned32", "Gauge32", "Counter64", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haReplicationBinding=haReplicationBinding, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapHA=trapHA, dhcpConfig=dhcpConfig, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStats=dnsStats, lcdDaemon=lcdDaemon, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonisObjects=adonisObjects, dhcpSubnetEntry=dhcpSubnetEntry, trapHAServiceFailOver=trapHAServiceFailOver, licenseExpiry=licenseExpiry, trapDHCP=trapDHCP, trapDHCPDaemon=trapDHCPDaemon, trapReplication=trapReplication, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemon=dnsDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIPEntry=dhcpFixedIPEntry, haService=haService, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsSuccess=dnsStatsSuccess, trapCommandServerDaemon=trapCommandServerDaemon, licenseValid=licenseValid, dhcpDaemon=dhcpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTPDaemon=trapTFTPDaemon, tftpDaemon=tftpDaemon, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemDaemon=systemDaemon, haServiceRunning=haServiceRunning, dhcpStats=dhcpStats, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsFailure=dnsStatsFailure, ha=ha, trapTFTP=trapTFTP, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonRunning=dnsDaemonRunning, systemState=systemState, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServer=commandServer, dhcpPoolUsed=dhcpPoolUsed, trapReplicationFailure=trapReplicationFailure, tftp=tftp, dhcpPoolEndIP=dhcpPoolEndIP, trapSystem=trapSystem, trapDNS=trapDNS, dns=dns, dhcpPoolStartIP=dhcpPoolStartIP, lcd=lcd, haServiceNodeType=haServiceNodeType, trapSystemDaemon=trapSystemDaemon, adonisTraps=adonisTraps, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, adonis=adonis, system=system, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsReferral=dnsStatsReferral, dhcpSubnetTable=dhcpSubnetTable, dhcpIP=dhcpIP, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolTable=dhcpPoolTable, dhcp=dhcp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, PYSNMP_MODULE_ID=adonis, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIP=dhcpFixedIP, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolAlert=dhcpPoolAlert, trapDNSDaemon=trapDNSDaemon, commandServerDaemon=commandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFailOverState=dhcpFailOverState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServer=trapCommandServer, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftpDaemonRunning=tftpDaemonRunning)
