...
Counter64, Integer32, ObjectIdentity, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, Bits, Gauge32, ModuleIdentity, iso, Unsigned32, TimeTicks, Counter32, IpAddress, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Counter64", "Integer32", "ObjectIdentity", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "Bits", "Gauge32", "ModuleIdentity", "iso", "Unsigned32", "TimeTicks", "Counter32", "IpAddress", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFailOverState=dhcpFailOverState, dnsDaemonRunning=dnsDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIP=dhcpFixedIP, trapCommandServer=trapCommandServer, lcd=lcd, lcdDaemon=lcdDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseBindState=dhcpLeaseBindState, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapHA=trapHA, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpIP=dhcpIP, trapTFTP=trapTFTP, commandServerDaemon=commandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcp=dhcp, ha=ha, dhcpSubnetSize=dhcpSubnetSize, haServiceRunning=haServiceRunning, dhcpPoolEndIP=dhcpPoolEndIP, adonisTraps=adonisTraps, dnsStatsReferral=dnsStatsReferral, licenseExpiry=licenseExpiry, trapDNSDaemon=trapDNSDaemon, trapHAServiceFailOver=trapHAServiceFailOver, PYSNMP_MODULE_ID=adonis, dhcpPoolSize=dhcpPoolSize, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServer=commandServer, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseTable=dhcpLeaseTable, haServiceNodeType=haServiceNodeType, haReplicationBinding=haReplicationBinding, dns=dns, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetMask=dhcpSubnetMask, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, dnsStats=dnsStats, adonisObjects=adonisObjects, systemDaemon=systemDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dnsDaemon=dnsDaemon, dhcpStats=dhcpStats, trapDNS=trapDNS, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolStartIP=dhcpPoolStartIP, trapDHCP=trapDHCP, tftp=tftp, dhcpSubnetEntry=dhcpSubnetEntry, adonis=adonis, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, systemState=systemState, dnsStatsRecursion=dnsStatsRecursion, trapDHCPDaemon=trapDHCPDaemon, trapTFTPDaemon=trapTFTPDaemon, trapReplicationFailure=trapReplicationFailure, haService=haService, trapSystemDaemon=trapSystemDaemon, dhcpPoolTable=dhcpPoolTable, tftpDaemonRunning=tftpDaemonRunning, dnsStatsFailure=dnsStatsFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseValid=licenseValid, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, system=system, trapReplication=trapReplication, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemon=dhcpDaemon, trapSystem=trapSystem, dhcpConfig=dhcpConfig)
