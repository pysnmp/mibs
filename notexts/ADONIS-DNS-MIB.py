...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, dnsStatsFailure=dnsStatsFailure, licenseValid=licenseValid, haServiceRunning=haServiceRunning, dhcpSubnetIP=dhcpSubnetIP, dnsStats=dnsStats, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemonRunning=commandServerDaemonRunning, ha=ha, trapReplicationFailure=trapReplicationFailure, trapDNS=trapDNS, dnsDaemonRunning=dnsDaemonRunning, PYSNMP_MODULE_ID=adonis, commandServerDaemon=commandServerDaemon, adonisTraps=adonisTraps, dhcpDaemon=dhcpDaemon, dns=dns, dhcpFailOverState=dhcpFailOverState, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEntry=dhcpPoolEntry, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, system=system, dhcpConfig=dhcpConfig, adonis=adonis, trapDHCP=trapDHCP, adonisObjects=adonisObjects, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapSystemDaemon=trapSystemDaemon, trapSystem=trapSystem, lcdDaemon=lcdDaemon, dnsStatsReferral=dnsStatsReferral, haService=haService, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolEndIP=dhcpPoolEndIP, systemState=systemState, dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, haReplicationBinding=haReplicationBinding, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolTable=dhcpPoolTable, trapTFTP=trapTFTP, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolStartIP=dhcpPoolStartIP, dhcp=dhcp, tftp=tftp, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIP=dhcpFixedIP, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haServiceNodeType=haServiceNodeType, trapCommandServer=trapCommandServer, trapReplication=trapReplication, tftpDaemon=tftpDaemon, licenseExpiry=licenseExpiry, trapDHCPDaemon=trapDHCPDaemon, dhcpIP=dhcpIP, systemDaemon=systemDaemon, dnsDaemon=dnsDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServer=commandServer, dhcpPoolSize=dhcpPoolSize, lcd=lcd, dhcpPoolAlert=dhcpPoolAlert)
