...
Integer32, Bits, Counter32, ModuleIdentity, Gauge32, NotificationType, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, TimeTicks, ObjectIdentity, Unsigned32, MibIdentifier, enterprises, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Bits", "Counter32", "ModuleIdentity", "Gauge32", "NotificationType", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "TimeTicks", "ObjectIdentity", "Unsigned32", "MibIdentifier", "enterprises", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFailOverState=dhcpFailOverState, dhcpPoolEndIP=dhcpPoolEndIP, adonisTraps=adonisTraps, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, tftpDaemonRunning=tftpDaemonRunning, adonis=adonis, trapDHCP=trapDHCP, adonisObjects=adonisObjects, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, PYSNMP_MODULE_ID=adonis, dhcpSubnetEntry=dhcpSubnetEntry, trapDNSDaemon=trapDNSDaemon, dnsStatsRecursion=dnsStatsRecursion, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseEntry=dhcpLeaseEntry, haServiceRunning=haServiceRunning, trapSystem=trapSystem, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetTable=dhcpSubnetTable, tftpDaemon=tftpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, trapDNS=trapDNS, systemState=systemState, dhcpFixedIPTable=dhcpFixedIPTable, ha=ha, dhcpPoolTable=dhcpPoolTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIP=dhcpFixedIP, licenseExpiry=licenseExpiry, dhcpStats=dhcpStats, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSize=dhcpPoolSize, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpConfig=dhcpConfig, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapReplicationFailure=trapReplicationFailure, commandServer=commandServer, dhcpSubnetIP=dhcpSubnetIP, haServiceNodeType=haServiceNodeType, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dns=dns, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEntry=dhcpPoolEntry, system=system, trapReplication=trapReplication, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStats=dnsStats, dhcpLeaseHostname=dhcpLeaseHostname, trapHA=trapHA, licenseValid=licenseValid, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapTFTP=trapTFTP, dhcpDaemon=dhcpDaemon, dnsStatsSuccess=dnsStatsSuccess, systemDaemon=systemDaemon, dhcpIP=dhcpIP, dhcpSubnetUsed=dhcpSubnetUsed, haService=haService, commandServerDaemon=commandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, haReplicationBinding=haReplicationBinding, trapCommandServer=trapCommandServer, dhcpSubnetAlert=dhcpSubnetAlert, lcdDaemon=lcdDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolAlert=dhcpPoolAlert, lcd=lcd, tftp=tftp, trapDHCPDaemon=trapDHCPDaemon, trapSystemDaemon=trapSystemDaemon, dhcp=dhcp, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemon=dnsDaemon, dnsStatsFailure=dnsStatsFailure, dhcpSubnetMask=dhcpSubnetMask)
