...
MibIdentifier, Counter64, Integer32, Gauge32, iso, TimeTicks, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, Unsigned32, enterprises, Bits, Counter32, IpAddress, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Counter64", "Integer32", "Gauge32", "iso", "TimeTicks", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "Unsigned32", "enterprises", "Bits", "Counter32", "IpAddress", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolTable=dhcpPoolTable, dhcpSubnetIP=dhcpSubnetIP, trapSystemDaemon=trapSystemDaemon, trapReplication=trapReplication, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNSDaemon=trapDNSDaemon, dhcpPoolAlert=dhcpPoolAlert, ha=ha, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNS=trapDNS, trapHAServiceFailOver=trapHAServiceFailOver, haService=haService, dnsStatsReferral=dnsStatsReferral, dnsStatsFailure=dnsStatsFailure, dhcpLeaseEndTime=dhcpLeaseEndTime, trapCommandServer=trapCommandServer, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServer=commandServer, trapReplicationFailure=trapReplicationFailure, dhcpIP=dhcpIP, trapTFTPDaemon=trapTFTPDaemon, lcdDaemon=lcdDaemon, dnsStatsNXDomain=dnsStatsNXDomain, PYSNMP_MODULE_ID=adonis, lcd=lcd, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, systemDaemon=systemDaemon, licenseValid=licenseValid, dhcp=dhcp, dns=dns, trapTFTP=trapTFTP, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIP=dhcpFixedIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFailOverState=dhcpFailOverState, trapSystem=trapSystem, dhcpPoolEndIP=dhcpPoolEndIP, adonisTraps=adonisTraps, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemon=dhcpDaemon, adonisObjects=adonisObjects, dnsStatsSuccess=dnsStatsSuccess, adonis=adonis, dhcpLeaseStartTime=dhcpLeaseStartTime, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, systemState=systemState, trapHA=trapHA, licenseExpiry=licenseExpiry, tftp=tftp, tftpDaemon=tftpDaemon, trapDHCP=trapDHCP, dnsDaemon=dnsDaemon, dnsStats=dnsStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceNodeType=haServiceNodeType, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetMask=dhcpSubnetMask, trapCommandServerDaemon=trapCommandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, haReplicationBinding=haReplicationBinding, system=system, commandServerDaemon=commandServerDaemon, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonRunning=dhcpDaemonRunning, haServiceRunning=haServiceRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpStats=dhcpStats, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolUsed=dhcpPoolUsed, dhcpConfig=dhcpConfig, dhcpSubnetUsed=dhcpSubnetUsed)
