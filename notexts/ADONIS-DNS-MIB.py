...
ModuleIdentity, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, ObjectIdentity, Integer32, Bits, IpAddress, Counter64, NotificationType, TimeTicks, enterprises, MibIdentifier, Gauge32, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "ObjectIdentity", "Integer32", "Bits", "IpAddress", "Counter64", "NotificationType", "TimeTicks", "enterprises", "MibIdentifier", "Gauge32", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEntry=dhcpLeaseEntry, dhcpStats=dhcpStats, trapDHCP=trapDHCP, tftp=tftp, systemState=systemState, dnsStatsReferral=dnsStatsReferral, ha=ha, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpIP=dhcpIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServer=commandServer, dhcpDaemonRunning=dhcpDaemonRunning, lcdDaemon=lcdDaemon, trapDHCPDaemon=trapDHCPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseTable=dhcpLeaseTable, trapCommandServer=trapCommandServer, PYSNMP_MODULE_ID=adonis, trapSystem=trapSystem, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, systemDaemon=systemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetSize=dhcpSubnetSize, dnsStatsFailure=dnsStatsFailure, lcd=lcd, dhcp=dhcp, dnsStatsRecursion=dnsStatsRecursion, licenseValid=licenseValid, dhcpSubnetEntry=dhcpSubnetEntry, haServiceRunning=haServiceRunning, dnsStatsNXDomain=dnsStatsNXDomain, licenseExpiry=licenseExpiry, dhcpPoolSize=dhcpPoolSize, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolStartIP=dhcpPoolStartIP, trapSystemDaemon=trapSystemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, haReplicationBinding=haReplicationBinding, dns=dns, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolUsed=dhcpPoolUsed, trapReplicationFailure=trapReplicationFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsSuccess=dnsStatsSuccess, dnsStats=dnsStats, haServiceNodeType=haServiceNodeType, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpConfig=dhcpConfig, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemon=dnsDaemon, dhcpPoolTable=dhcpPoolTable, dhcpPoolAlert=dhcpPoolAlert, tftpDaemonRunning=tftpDaemonRunning, system=system, dhcpFailOverState=dhcpFailOverState, dhcpPoolEntry=dhcpPoolEntry, trapReplication=trapReplication, tftpDaemon=tftpDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNSDaemon=trapDNSDaemon, haService=haService, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHostname=dhcpLeaseHostname, commandServerDaemon=commandServerDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonis=adonis, trapTFTP=trapTFTP, dhcpDaemon=dhcpDaemon, trapHA=trapHA, dhcpSubnetUsed=dhcpSubnetUsed, adonisTraps=adonisTraps, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonisObjects=adonisObjects, dhcpFixedIP=dhcpFixedIP, trapDNS=trapDNS, dhcpPoolEndIP=dhcpPoolEndIP)
