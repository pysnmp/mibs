...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapCommandServer=trapCommandServer, lcdDaemon=lcdDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServer=commandServer, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, adonisObjects=adonisObjects, tftp=tftp, dhcpPoolSize=dhcpPoolSize, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpConfig=dhcpConfig, trapDHCP=trapDHCP, trapTFTPDaemon=trapTFTPDaemon, lcd=lcd, dhcpFixedIP=dhcpFixedIP, trapReplicationFailure=trapReplicationFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, systemState=systemState, dhcpPoolEntry=dhcpPoolEntry, systemDaemon=systemDaemon, trapTFTP=trapTFTP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcp=dhcp, haServiceRunning=haServiceRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapSystem=trapSystem, adonis=adonis, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemon=tftpDaemon, trapDNS=trapDNS, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, dns=dns, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCPDaemon=trapDHCPDaemon, adonisTraps=adonisTraps, dnsStatsReferral=dnsStatsReferral, haServiceNodeType=haServiceNodeType, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetTable=dhcpSubnetTable, dnsDaemon=dnsDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseBindState=dhcpLeaseBindState, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFailOverState=dhcpFailOverState, trapSystemDaemon=trapSystemDaemon, system=system, dhcpStats=dhcpStats, dnsStats=dnsStats, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolUsed=dhcpPoolUsed, trapDNSDaemon=trapDNSDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolTable=dhcpPoolTable, trapHA=trapHA, haReplicationBinding=haReplicationBinding, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsRecursion=dnsStatsRecursion, trapReplication=trapReplication, PYSNMP_MODULE_ID=adonis, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPTable=dhcpFixedIPTable, dhcpIP=dhcpIP, dhcpSubnetMask=dhcpSubnetMask, licenseValid=licenseValid, dnsStatsFailure=dnsStatsFailure, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpDaemon=dhcpDaemon, licenseExpiry=licenseExpiry, ha=ha, dnsStatsNXDomain=dnsStatsNXDomain)
