...
TimeTicks, IpAddress, Gauge32, enterprises, ModuleIdentity, Integer32, Counter64, Bits, Counter32, ObjectIdentity, Unsigned32, NotificationType, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, iso = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "IpAddress", "Gauge32", "enterprises", "ModuleIdentity", "Integer32", "Counter64", "Bits", "Counter32", "ObjectIdentity", "Unsigned32", "NotificationType", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDHCPDaemon=trapDHCPDaemon, trapDHCP=trapDHCP, dhcpPoolSize=dhcpPoolSize, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTable=dhcpLeaseTable, trapDNSDaemon=trapDNSDaemon, dnsStatsFailure=dnsStatsFailure, commandServer=commandServer, dhcpLeaseHostname=dhcpLeaseHostname, haServiceRunning=haServiceRunning, trapReplicationFailure=trapReplicationFailure, PYSNMP_MODULE_ID=adonis, dnsDaemon=dnsDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolTable=dhcpPoolTable, adonisObjects=adonisObjects, dhcpPoolAlert=dhcpPoolAlert, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftp=tftp, haService=haService, systemDaemon=systemDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, commandServerDaemonRunning=commandServerDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetSize=dhcpSubnetSize, dhcpConfig=dhcpConfig, dhcpIP=dhcpIP, haReplicationBinding=haReplicationBinding, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemon=dhcpDaemon, lcd=lcd, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEndTime=dhcpLeaseEndTime, trapSystem=trapSystem, dhcpFixedIP=dhcpFixedIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetAlert=dhcpSubnetAlert, trapDNS=trapDNS, trapReplication=trapReplication, dhcpPoolEntry=dhcpPoolEntry, dhcpStats=dhcpStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseBindState=dhcpLeaseBindState, licenseExpiry=licenseExpiry, trapSystemDaemon=trapSystemDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseValid=licenseValid, dhcpFixedIPEntry=dhcpFixedIPEntry, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServer=trapCommandServer, dhcpPoolUsed=dhcpPoolUsed, lcdDaemon=lcdDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTP=trapTFTP, dnsStats=dnsStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetTable=dhcpSubnetTable, adonis=adonis, dnsStatsRecursion=dnsStatsRecursion, ha=ha, trapHA=trapHA, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, systemState=systemState, dhcpFailOverState=dhcpFailOverState, dhcpSubnetIP=dhcpSubnetIP, adonisTraps=adonisTraps, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcp=dhcp, dns=dns, haServiceNodeType=haServiceNodeType, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsReferral=dnsStatsReferral, system=system, dnsDaemonRunning=dnsDaemonRunning, tftpDaemon=tftpDaemon)
