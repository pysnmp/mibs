...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpConfig=dhcpConfig, adonisTraps=adonisTraps, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolTable=dhcpPoolTable, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, dhcpIP=dhcpIP, ha=ha, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIPEntry=dhcpFixedIPEntry, trapTFTPDaemon=trapTFTPDaemon, dnsStatsReferral=dnsStatsReferral, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcdDaemon=lcdDaemon, trapReplicationFailure=trapReplicationFailure, dhcp=dhcp, dhcpLeaseTable=dhcpLeaseTable, dhcpFixedIP=dhcpFixedIP, dhcpPoolStartIP=dhcpPoolStartIP, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSize=dhcpPoolSize, lcd=lcd, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haServiceNodeType=haServiceNodeType, dhcpDaemonRunning=dhcpDaemonRunning, trapDNSDaemon=trapDNSDaemon, commandServer=commandServer, trapDHCPDaemon=trapDHCPDaemon, systemDaemon=systemDaemon, dnsDaemon=dnsDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetMask=dhcpSubnetMask, trapDNS=trapDNS, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftp=tftp, systemState=systemState, dhcpSubnetTable=dhcpSubnetTable, licenseValid=licenseValid, tftpDaemon=tftpDaemon, haReplicationBinding=haReplicationBinding, trapSystem=trapSystem, haServiceRunning=haServiceRunning, adonis=adonis, trapSystemDaemon=trapSystemDaemon, dnsStats=dnsStats, trapTFTP=trapTFTP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemon=dhcpDaemon, dhcpStats=dhcpStats, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXRRSet=dnsStatsNXRRSet, licenseExpiry=licenseExpiry, haService=haService, dhcpFailOverState=dhcpFailOverState, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, trapDHCP=trapDHCP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEndTime=dhcpLeaseEndTime, system=system, dhcpSubnetSize=dhcpSubnetSize, dnsStatsFailure=dnsStatsFailure, trapHAServiceFailOver=trapHAServiceFailOver, trapReplication=trapReplication, dnsStatsRecursion=dnsStatsRecursion, dns=dns, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred)
