...
ObjectIdentity, Counter64, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, ModuleIdentity, Unsigned32, Gauge32, iso, Counter32, Bits, NotificationType, IpAddress, MibIdentifier, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Counter64", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "ModuleIdentity", "Unsigned32", "Gauge32", "iso", "Counter32", "Bits", "NotificationType", "IpAddress", "MibIdentifier", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolUsed=dhcpPoolUsed, trapCommandServerDaemon=trapCommandServerDaemon, tftpDaemon=tftpDaemon, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, PYSNMP_MODULE_ID=adonis, dhcpPoolAlert=dhcpPoolAlert, commandServerDaemonRunning=commandServerDaemonRunning, trapSystem=trapSystem, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemon=dhcpDaemon, dnsStatsFailure=dnsStatsFailure, dhcpSubnetEntry=dhcpSubnetEntry, trapSystemDaemon=trapSystemDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseBindState=dhcpLeaseBindState, systemState=systemState, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, system=system, dhcpSubnetIP=dhcpSubnetIP, adonis=adonis, systemDaemon=systemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpConfig=dhcpConfig, dhcpPoolTable=dhcpPoolTable, licenseValid=licenseValid, dnsStatsNXDomain=dnsStatsNXDomain, adonisTraps=adonisTraps, haServiceRunning=haServiceRunning, dhcpFixedIPTable=dhcpFixedIPTable, trapReplicationFailure=trapReplicationFailure, dnsStatsRecursion=dnsStatsRecursion, lcdDaemon=lcdDaemon, trapTFTP=trapTFTP, dhcp=dhcp, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpStats=dhcpStats, dns=dns, dhcpPoolSize=dhcpPoolSize, commandServerDaemon=commandServerDaemon, dnsStatsSuccess=dnsStatsSuccess, dnsDaemon=dnsDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpIP=dhcpIP, commandServer=commandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, tftp=tftp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolEndIP=dhcpPoolEndIP, trapHAServiceFailOver=trapHAServiceFailOver, adonisObjects=adonisObjects, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIP=dhcpFixedIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsReferral=dnsStatsReferral, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolStartIP=dhcpPoolStartIP, licenseExpiry=licenseExpiry, dhcpSubnetMask=dhcpSubnetMask, haService=haService, ha=ha, dhcpSubnetTable=dhcpSubnetTable, lcd=lcd, haServiceNodeType=haServiceNodeType, dhcpSubnetUsed=dhcpSubnetUsed, trapDNSDaemon=trapDNSDaemon, dnsStats=dnsStats, trapHA=trapHA, trapCommandServer=trapCommandServer, trapDNS=trapDNS, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseEntry=dhcpLeaseEntry, trapReplication=trapReplication, dnsDaemonRunning=dnsDaemonRunning)
