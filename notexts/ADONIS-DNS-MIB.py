...
Integer32, iso, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, ObjectIdentity, Unsigned32, Counter32, ModuleIdentity, enterprises, TimeTicks, Counter64, Bits, NotificationType, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "iso", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "ObjectIdentity", "Unsigned32", "Counter32", "ModuleIdentity", "enterprises", "TimeTicks", "Counter64", "Bits", "NotificationType", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolEndIP=dhcpPoolEndIP, trapSystemDaemon=trapSystemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIPTable=dhcpFixedIPTable, adonisObjects=adonisObjects, dhcpSubnetSize=dhcpSubnetSize, haReplicationBinding=haReplicationBinding, lcdDaemon=lcdDaemon, haServiceRunning=haServiceRunning, dnsStatsSuccess=dnsStatsSuccess, lcd=lcd, tftp=tftp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, licenseExpiry=licenseExpiry, dhcpDaemon=dhcpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, systemDaemon=systemDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFailOverState=dhcpFailOverState, dnsDaemonRunning=dnsDaemonRunning, PYSNMP_MODULE_ID=adonis, dhcpLeaseBindState=dhcpLeaseBindState, systemState=systemState, dhcpPoolTable=dhcpPoolTable, dhcpStats=dhcpStats, trapReplication=trapReplication, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetUsed=dhcpSubnetUsed, licenseValid=licenseValid, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haServiceNodeType=haServiceNodeType, dhcpPoolAlert=dhcpPoolAlert, trapHAServiceFailOver=trapHAServiceFailOver, system=system, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSize=dhcpPoolSize, trapDHCPDaemon=trapDHCPDaemon, dhcpConfig=dhcpConfig, dhcpFixedIP=dhcpFixedIP, adonis=adonis, trapCommandServer=trapCommandServer, dhcpSubnetTable=dhcpSubnetTable, dns=dns, dhcpFixedIPEntry=dhcpFixedIPEntry, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemon=dnsDaemon, dhcpLeaseTable=dhcpLeaseTable, ha=ha, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolStartIP=dhcpPoolStartIP, dhcpDaemonRunning=dhcpDaemonRunning, trapSystem=trapSystem, dhcpIP=dhcpIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetMask=dhcpSubnetMask, tftpDaemon=tftpDaemon, haService=haService, dnsStats=dnsStats, trapDHCP=trapDHCP, commandServerDaemon=commandServerDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsNXDomain=dnsStatsNXDomain, adonisTraps=adonisTraps, dnsStatsFailure=dnsStatsFailure, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTP=trapTFTP, trapHA=trapHA, dhcpSubnetEntry=dhcpSubnetEntry, trapDNS=trapDNS, dnsStatsReferral=dnsStatsReferral, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcp=dhcp, trapReplicationFailure=trapReplicationFailure, commandServer=commandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolEntry=dhcpPoolEntry)
