...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetUsed=dhcpSubnetUsed, licenseValid=licenseValid, dhcpDaemonRunning=dhcpDaemonRunning, dhcpStats=dhcpStats, dhcpLeaseStartTime=dhcpLeaseStartTime, tftpDaemon=tftpDaemon, dhcpPoolEndIP=dhcpPoolEndIP, trapHAServiceFailOver=trapHAServiceFailOver, haService=haService, trapSystem=trapSystem, trapReplicationFailure=trapReplicationFailure, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolTable=dhcpPoolTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapSystemDaemon=trapSystemDaemon, dhcpPoolSize=dhcpPoolSize, dhcpFailOverState=dhcpFailOverState, dhcpFixedIPEntry=dhcpFixedIPEntry, adonisObjects=adonisObjects, dnsDaemon=dnsDaemon, trapCommandServer=trapCommandServer, ha=ha, trapDHCPDaemon=trapDHCPDaemon, systemState=systemState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseHostname=dhcpLeaseHostname, licenseExpiry=licenseExpiry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEntry=dhcpPoolEntry, commandServer=commandServer, trapHA=trapHA, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemonRunning=commandServerDaemonRunning, dnsStats=dnsStats, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, dhcpPoolStartIP=dhcpPoolStartIP, dns=dns, adonis=adonis, trapDNSDaemon=trapDNSDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcd=lcd, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplication=trapReplication, systemDaemon=systemDaemon, haReplicationBinding=haReplicationBinding, haServiceNodeType=haServiceNodeType, trapDHCP=trapDHCP, lcdDaemon=lcdDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haServiceRunning=haServiceRunning, dhcpSubnetMask=dhcpSubnetMask, dhcp=dhcp, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPTable=dhcpFixedIPTable, system=system, dnsStatsRecursion=dnsStatsRecursion, dnsStatsFailure=dnsStatsFailure, adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, trapTFTP=trapTFTP, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, tftp=tftp, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemon=dhcpDaemon, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDNS=trapDNS, PYSNMP_MODULE_ID=adonis, dhcpFixedIP=dhcpFixedIP, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsReferral=dnsStatsReferral, commandServerDaemon=commandServerDaemon, dhcpConfig=dhcpConfig, dhcpSubnetIP=dhcpSubnetIP, trapCommandServerDaemon=trapCommandServerDaemon)
