...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNS=trapDNS, licenseValid=licenseValid, dhcpSubnetSize=dhcpSubnetSize, trapSystem=trapSystem, dhcp=dhcp, commandServerDaemonRunning=commandServerDaemonRunning, tftp=tftp, trapTFTP=trapTFTP, dnsStatsRecursion=dnsStatsRecursion, trapDHCP=trapDHCP, dhcpStats=dhcpStats, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonRunning=dhcpDaemonRunning, haService=haService, dhcpSubnetUsed=dhcpSubnetUsed, adonisTraps=adonisTraps, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemon=dnsDaemon, adonis=adonis, PYSNMP_MODULE_ID=adonis, lcd=lcd, dhcpFailOverState=dhcpFailOverState, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolTable=dhcpPoolTable, dnsStatsFailure=dnsStatsFailure, dhcpSubnetEntry=dhcpSubnetEntry, trapHA=trapHA, dhcpFixedIP=dhcpFixedIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetMask=dhcpSubnetMask, adonisObjects=adonisObjects, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNSDaemon=trapDNSDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonRunning=dnsDaemonRunning, haServiceNodeType=haServiceNodeType, dhcpPoolSize=dhcpPoolSize, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, systemState=systemState, ha=ha, tftpDaemon=tftpDaemon, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, systemDaemon=systemDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetTable=dhcpSubnetTable, trapHAServiceFailOver=trapHAServiceFailOver, dns=dns, dhcpLeaseTable=dhcpLeaseTable, trapTFTPDaemon=trapTFTPDaemon, commandServer=commandServer, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolUsed=dhcpPoolUsed, trapCommandServer=trapCommandServer, dhcpPoolEntry=dhcpPoolEntry, dnsStatsSuccess=dnsStatsSuccess, lcdDaemon=lcdDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, system=system, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapReplicationFailure=trapReplicationFailure, dhcpDaemon=dhcpDaemon, commandServerDaemon=commandServerDaemon, dhcpConfig=dhcpConfig, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapReplication=trapReplication, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, licenseExpiry=licenseExpiry, haServiceRunning=haServiceRunning, dnsStats=dnsStats, trapCommandServerDaemon=trapCommandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEntry=dhcpLeaseEntry, dhcpIP=dhcpIP, dnsStatsReferral=dnsStatsReferral, dhcpLeaseStartTime=dhcpLeaseStartTime, haReplicationBinding=haReplicationBinding, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert)
