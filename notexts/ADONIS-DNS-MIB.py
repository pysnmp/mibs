...
ModuleIdentity, Counter32, Gauge32, Unsigned32, Bits, IpAddress, Counter64, enterprises, MibIdentifier, TimeTicks, ObjectIdentity, NotificationType, iso, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Counter32", "Gauge32", "Unsigned32", "Bits", "IpAddress", "Counter64", "enterprises", "MibIdentifier", "TimeTicks", "ObjectIdentity", "NotificationType", "iso", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisTraps=adonisTraps, trapSystemDaemon=trapSystemDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSize=dhcpPoolSize, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpIP=dhcpIP, dhcpStats=dhcpStats, adonis=adonis, dhcpPoolAlert=dhcpPoolAlert, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServer=trapCommandServer, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, PYSNMP_MODULE_ID=adonis, licenseValid=licenseValid, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEntry=dhcpLeaseEntry, lcdDaemon=lcdDaemon, dhcpPoolStartIP=dhcpPoolStartIP, trapReplication=trapReplication, dhcpSubnetIP=dhcpSubnetIP, dhcp=dhcp, systemState=systemState, dnsDaemon=dnsDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServer=commandServer, haServiceRunning=haServiceRunning, dhcpFailOverState=dhcpFailOverState, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEntry=dhcpPoolEntry, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, haReplicationBinding=haReplicationBinding, trapDNS=trapDNS, adonisObjects=adonisObjects, dhcpPoolUsed=dhcpPoolUsed, licenseExpiry=licenseExpiry, dhcpConfig=dhcpConfig, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsReferral=dnsStatsReferral, trapSystem=trapSystem, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, commandServerDaemon=commandServerDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIP=dhcpFixedIP, dnsDaemonRunning=dnsDaemonRunning, dnsStatsFailure=dnsStatsFailure, haService=haService, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTP=trapTFTP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseEndTime=dhcpLeaseEndTime, system=system, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolTable=dhcpPoolTable, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, tftpDaemonRunning=tftpDaemonRunning, ha=ha, dhcpPoolEndIP=dhcpPoolEndIP, tftp=tftp, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsSuccess=dnsStatsSuccess, trapDNSDaemon=trapDNSDaemon, lcd=lcd, dnsStats=dnsStats, dhcpSubnetSize=dhcpSubnetSize, trapDHCP=trapDHCP, tftpDaemon=tftpDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTPDaemon=trapTFTPDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceNodeType=haServiceNodeType, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, systemDaemon=systemDaemon)
