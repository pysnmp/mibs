...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapSystemDaemon=trapSystemDaemon, dns=dns, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFixedIP=dhcpFixedIP, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetUsed=dhcpSubnetUsed, trapSystem=trapSystem, dhcp=dhcp, adonisTraps=adonisTraps, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemon=commandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolTable=dhcpPoolTable, haServiceNodeType=haServiceNodeType, dhcpDaemon=dhcpDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haService=haService, trapDNS=trapDNS, trapCommandServer=trapCommandServer, trapTFTP=trapTFTP, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolSize=dhcpPoolSize, dnsStats=dnsStats, dhcpFailOverState=dhcpFailOverState, dnsDaemon=dnsDaemon, PYSNMP_MODULE_ID=adonis, dhcpIP=dhcpIP, licenseValid=licenseValid, trapDNSDaemon=trapDNSDaemon, tftpDaemon=tftpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetAlert=dhcpSubnetAlert, trapReplicationFailure=trapReplicationFailure, trapReplication=trapReplication, adonisObjects=adonisObjects, lcd=lcd, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsFailure=dnsStatsFailure, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEntry=dhcpPoolEntry, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsReferral=dnsStatsReferral, dhcpStats=dhcpStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, systemState=systemState, tftp=tftp, haServiceRunning=haServiceRunning, system=system, trapHA=trapHA, dnsStatsNXRRSet=dnsStatsNXRRSet, adonis=adonis, commandServer=commandServer, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, haReplicationBinding=haReplicationBinding, commandServerDaemonRunning=commandServerDaemonRunning, lcdDaemon=lcdDaemon, systemDaemon=systemDaemon, trapDHCP=trapDHCP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseTable=dhcpLeaseTable, dhcpConfig=dhcpConfig, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonDebugLevel=dnsDaemonDebugLevel, ha=ha, licenseExpiry=licenseExpiry, dnsStatsNXDomain=dnsStatsNXDomain)
