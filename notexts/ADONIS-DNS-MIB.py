...
Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Unsigned32, Counter64, NotificationType, ModuleIdentity, MibIdentifier, Counter32, IpAddress, Bits, TimeTicks, ObjectIdentity, iso, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Unsigned32", "Counter64", "NotificationType", "ModuleIdentity", "MibIdentifier", "Counter32", "IpAddress", "Bits", "TimeTicks", "ObjectIdentity", "iso", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemDaemon=systemDaemon, lcd=lcd, dnsStats=dnsStats, trapTFTPDaemon=trapTFTPDaemon, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapReplication=trapReplication, dhcpPoolSize=dhcpPoolSize, dhcpFixedIPEntry=dhcpFixedIPEntry, trapSystemDaemon=trapSystemDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIP=dhcpFixedIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDHCPDaemon=trapDHCPDaemon, licenseValid=licenseValid, dnsStatsSuccess=dnsStatsSuccess, trapTFTP=trapTFTP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolStartIP=dhcpPoolStartIP, dhcpConfig=dhcpConfig, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServerDaemonRunning=commandServerDaemonRunning, PYSNMP_MODULE_ID=adonis, dnsDaemon=dnsDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceRunning=haServiceRunning, adonis=adonis, dnsStatsReferral=dnsStatsReferral, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolAlert=dhcpPoolAlert, haReplicationBinding=haReplicationBinding, commandServerDaemon=commandServerDaemon, ha=ha, trapDNSDaemon=trapDNSDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftp=tftp, systemState=systemState, tftpDaemon=tftpDaemon, trapDHCP=trapDHCP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemon=dhcpDaemon, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpStats=dhcpStats, adonisTraps=adonisTraps, adonisObjects=adonisObjects, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetAlert=dhcpSubnetAlert, trapDNS=trapDNS, trapSystem=trapSystem, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEntry=dhcpPoolEntry, dns=dns, lcdDaemon=lcdDaemon, dhcpSubnetSize=dhcpSubnetSize, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolUsed=dhcpPoolUsed, haService=haService, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseBindState=dhcpLeaseBindState, commandServer=commandServer, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonRunning=dhcpDaemonRunning, dhcp=dhcp, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolTable=dhcpPoolTable, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServer=trapCommandServer, licenseExpiry=licenseExpiry, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetTable=dhcpSubnetTable, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, system=system, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsRecursion=dnsStatsRecursion, dnsStatsFailure=dnsStatsFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress)
