...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapHA=trapHA, commandServerDaemonRunning=commandServerDaemonRunning, licenseExpiry=licenseExpiry, haReplicationBinding=haReplicationBinding, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNS=trapDNS, trapSystem=trapSystem, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, lcd=lcd, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolEntry=dhcpPoolEntry, dnsDaemon=dnsDaemon, dhcpDaemon=dhcpDaemon, adonis=adonis, commandServerDaemon=commandServerDaemon, systemDaemon=systemDaemon, dhcpSubnetAlert=dhcpSubnetAlert, tftpDaemon=tftpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolUsed=dhcpPoolUsed, trapHAServiceFailOver=trapHAServiceFailOver, systemState=systemState, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetUsed=dhcpSubnetUsed, dns=dns, trapDHCPDaemon=trapDHCPDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonRunning=dhcpDaemonRunning, haServiceRunning=haServiceRunning, lcdDaemon=lcdDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, haServiceNodeType=haServiceNodeType, dhcpStats=dhcpStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsFailure=dnsStatsFailure, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseTable=dhcpLeaseTable, adonisTraps=adonisTraps, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonRunning=dnsDaemonRunning, trapReplication=trapReplication, dhcp=dhcp, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapSystemDaemon=trapSystemDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolSize=dhcpPoolSize, dhcpConfig=dhcpConfig, adonisObjects=adonisObjects, commandServer=commandServer, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapTFTP=trapTFTP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolTable=dhcpPoolTable, licenseValid=licenseValid, trapDHCP=trapDHCP, trapReplicationFailure=trapReplicationFailure, trapTFTPDaemon=trapTFTPDaemon, trapDNSDaemon=trapDNSDaemon, trapCommandServer=trapCommandServer, dnsStats=dnsStats, dhcpFixedIP=dhcpFixedIP, dnsStatsRecursion=dnsStatsRecursion, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsReferral=dnsStatsReferral, system=system, ha=ha, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpIP=dhcpIP, tftp=tftp, dhcpFailOverState=dhcpFailOverState)
