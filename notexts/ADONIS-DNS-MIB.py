...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsReferral=dnsStatsReferral, dhcpSubnetUsed=dhcpSubnetUsed, licenseValid=licenseValid, trapTFTP=trapTFTP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcp=dhcp, trapReplication=trapReplication, trapDHCP=trapDHCP, tftpDaemon=tftpDaemon, PYSNMP_MODULE_ID=adonis, dhcpFixedIP=dhcpFixedIP, adonisTraps=adonisTraps, dhcpLeaseTable=dhcpLeaseTable, dnsStatsFailure=dnsStatsFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemon=commandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, lcd=lcd, haServiceNodeType=haServiceNodeType, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, systemDaemon=systemDaemon, systemState=systemState, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpStats=dhcpStats, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, trapCommandServer=trapCommandServer, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, lcdDaemon=lcdDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, haReplicationBinding=haReplicationBinding, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, ha=ha, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolAlert=dhcpPoolAlert, dhcpIP=dhcpIP, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServer=commandServer, dhcpConfig=dhcpConfig, trapCommandServerDaemon=trapCommandServerDaemon, adonisObjects=adonisObjects, system=system, dnsDaemonDebugLevel=dnsDaemonDebugLevel, licenseExpiry=licenseExpiry, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemon=dhcpDaemon, haService=haService, dhcpPoolTable=dhcpPoolTable, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemon=dnsDaemon, tftp=tftp, dnsStatsRecursion=dnsStatsRecursion, haServiceRunning=haServiceRunning, dhcpPoolSize=dhcpPoolSize, trapReplicationFailure=trapReplicationFailure, dhcpPoolStartIP=dhcpPoolStartIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStats=dnsStats, dhcpSubnetAlert=dhcpSubnetAlert, trapHA=trapHA, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNSDaemon=trapDNSDaemon, dhcpSubnetMask=dhcpSubnetMask, dns=dns, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, trapSystem=trapSystem, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDNS=trapDNS, tftpDaemonRunning=tftpDaemonRunning, adonis=adonis, trapHAServiceFailOver=trapHAServiceFailOver)
