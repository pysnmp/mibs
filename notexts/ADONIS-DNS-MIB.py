...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapHAServiceFailOver=trapHAServiceFailOver, system=system, dhcpSubnetIP=dhcpSubnetIP, trapReplicationFailure=trapReplicationFailure, dnsStatsFailure=dnsStatsFailure, dhcpSubnetTable=dhcpSubnetTable, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dns=dns, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseExpiry=licenseExpiry, trapCommandServerDaemon=trapCommandServerDaemon, commandServer=commandServer, adonisTraps=adonisTraps, dhcp=dhcp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftp=tftp, adonis=adonis, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetMask=dhcpSubnetMask, trapSystem=trapSystem, haServiceNodeType=haServiceNodeType, dnsDaemon=dnsDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNS=trapDNS, dhcpIP=dhcpIP, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemon=tftpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTable=dhcpLeaseTable, haServiceRunning=haServiceRunning, trapReplication=trapReplication, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, licenseValid=licenseValid, dhcpFixedIP=dhcpFixedIP, dhcpFailOverState=dhcpFailOverState, trapSystemDaemon=trapSystemDaemon, ha=ha, dhcpStats=dhcpStats, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemon=commandServerDaemon, dhcpSubnetAlert=dhcpSubnetAlert, lcdDaemon=lcdDaemon, dhcpPoolEntry=dhcpPoolEntry, adonisObjects=adonisObjects, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSize=dhcpPoolSize, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStats=dnsStats, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemState=systemState, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsReferral=dnsStatsReferral, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolTable=dhcpPoolTable, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonRunning=dnsDaemonRunning, lcd=lcd, systemDaemon=systemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haService=haService, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemon=dhcpDaemon, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTP=trapTFTP, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, trapDHCP=trapDHCP, trapHA=trapHA, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServer=trapCommandServer, dhcpDaemonRunning=dhcpDaemonRunning, dhcpConfig=dhcpConfig, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCPDaemon=trapDHCPDaemon, dnsStatsSuccess=dnsStatsSuccess)
