...
Gauge32, enterprises, IpAddress, NotificationType, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Integer32, ObjectIdentity, Counter64, Bits, MibIdentifier, ModuleIdentity, iso, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "enterprises", "IpAddress", "NotificationType", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Integer32", "ObjectIdentity", "Counter64", "Bits", "MibIdentifier", "ModuleIdentity", "iso", "Counter32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCP=trapDHCP, dnsStatsReferral=dnsStatsReferral, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseEntry=dhcpLeaseEntry, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolEndIP=dhcpPoolEndIP, lcd=lcd, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, systemState=systemState, dhcpDaemonRunning=dhcpDaemonRunning, system=system, adonis=adonis, trapDHCPDaemon=trapDHCPDaemon, dhcp=dhcp, dhcpFixedIP=dhcpFixedIP, dhcpStats=dhcpStats, haService=haService, licenseExpiry=licenseExpiry, lcdDaemon=lcdDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, tftpDaemon=tftpDaemon, dhcpSubnetSize=dhcpSubnetSize, tftpDaemonRunning=tftpDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEntry=dhcpPoolEntry, tftp=tftp, adonisObjects=adonisObjects, licenseValid=licenseValid, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolSize=dhcpPoolSize, trapTFTPDaemon=trapTFTPDaemon, trapHA=trapHA, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNSDaemon=trapDNSDaemon, trapReplicationFailure=trapReplicationFailure, dns=dns, dhcpPoolTable=dhcpPoolTable, dnsStatsFailure=dnsStatsFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFailOverState=dhcpFailOverState, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, ha=ha, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseTable=dhcpLeaseTable, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolUsed=dhcpPoolUsed, dnsDaemon=dnsDaemon, dnsStats=dnsStats, dhcpLeaseBindState=dhcpLeaseBindState, trapHAServiceFailOver=trapHAServiceFailOver, dhcpIP=dhcpIP, haServiceRunning=haServiceRunning, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, haReplicationBinding=haReplicationBinding, dhcpDaemon=dhcpDaemon, trapReplication=trapReplication, trapSystem=trapSystem, systemDaemon=systemDaemon, dhcpConfig=dhcpConfig, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDNS=trapDNS, trapSystemDaemon=trapSystemDaemon, trapCommandServer=trapCommandServer, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, PYSNMP_MODULE_ID=adonis, commandServer=commandServer, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapTFTP=trapTFTP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisTraps=adonisTraps)
