...
Bits, Unsigned32, IpAddress, iso, Counter32, Counter64, ModuleIdentity, Integer32, enterprises, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, ObjectIdentity, MibIdentifier, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Unsigned32", "IpAddress", "iso", "Counter32", "Counter64", "ModuleIdentity", "Integer32", "enterprises", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "ObjectIdentity", "MibIdentifier", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemon=tftpDaemon, tftp=tftp, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFailOverState=dhcpFailOverState, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsReferral=dnsStatsReferral, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCP=trapDHCP, dhcpSubnetUsed=dhcpSubnetUsed, adonis=adonis, trapHA=trapHA, licenseValid=licenseValid, dhcpPoolAlert=dhcpPoolAlert, dnsStatsFailure=dnsStatsFailure, dhcpPoolSize=dhcpPoolSize, commandServerDaemon=commandServerDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServer=trapCommandServer, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolTable=dhcpPoolTable, dhcpPoolEntry=dhcpPoolEntry, trapDNS=trapDNS, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetEntry=dhcpSubnetEntry, dnsStats=dnsStats, dhcpFixedIPEntry=dhcpFixedIPEntry, tftpDaemonRunning=tftpDaemonRunning, dhcpIP=dhcpIP, system=system, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haServiceRunning=haServiceRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haReplicationBinding=haReplicationBinding, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dns=dns, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsSuccess=dnsStatsSuccess, systemDaemon=systemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, PYSNMP_MODULE_ID=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, trapSystem=trapSystem, licenseExpiry=licenseExpiry, dhcpFixedIP=dhcpFixedIP, dhcpConfig=dhcpConfig, dhcpSubnetAlert=dhcpSubnetAlert, dhcpStats=dhcpStats, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNSDaemon=trapDNSDaemon, systemState=systemState, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapTFTPDaemon=trapTFTPDaemon, lcd=lcd, dhcpDaemon=dhcpDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseTable=dhcpLeaseTable, dnsDaemon=dnsDaemon, ha=ha, dhcp=dhcp, commandServer=commandServer, dhcpSubnetMask=dhcpSubnetMask, trapReplicationFailure=trapReplicationFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, trapCommandServerDaemon=trapCommandServerDaemon, haService=haService, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceNodeType=haServiceNodeType, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonisObjects=adonisObjects, trapTFTP=trapTFTP, trapSystemDaemon=trapSystemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsRecursion=dnsStatsRecursion, trapReplication=trapReplication, adonisTraps=adonisTraps, lcdDaemon=lcdDaemon)
