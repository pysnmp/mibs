...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolEndIP=dhcpPoolEndIP, dhcp=dhcp, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, trapDHCP=trapDHCP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsFailure=dnsStatsFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetUsed=dhcpSubnetUsed, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetTable=dhcpSubnetTable, system=system, dhcpStats=dhcpStats, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsReferral=dnsStatsReferral, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisTraps=adonisTraps, dhcpSubnetEntry=dhcpSubnetEntry, dnsStats=dnsStats, dhcpFailOverState=dhcpFailOverState, dnsDaemon=dnsDaemon, dhcpIP=dhcpIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapReplicationFailure=trapReplicationFailure, dhcpPoolEntry=dhcpPoolEntry, commandServerDaemon=commandServerDaemon, commandServer=commandServer, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, licenseValid=licenseValid, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseStartTime=dhcpLeaseStartTime, trapReplication=trapReplication, dhcpFixedIP=dhcpFixedIP, adonis=adonis, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetAlert=dhcpSubnetAlert, trapSystem=trapSystem, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTable=dhcpLeaseTable, haServiceRunning=haServiceRunning, trapDNS=trapDNS, adonisObjects=adonisObjects, dhcpDaemon=dhcpDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsSuccess=dnsStatsSuccess, dhcpConfig=dhcpConfig, trapCommandServerDaemon=trapCommandServerDaemon, trapHA=trapHA, systemState=systemState, trapTFTP=trapTFTP, systemDaemon=systemDaemon, dns=dns, tftp=tftp, trapCommandServer=trapCommandServer, trapDHCPDaemon=trapDHCPDaemon, tftpDaemon=tftpDaemon, lcd=lcd, commandServerDaemonRunning=commandServerDaemonRunning, ha=ha, trapSystemDaemon=trapSystemDaemon, trapDNSDaemon=trapDNSDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, PYSNMP_MODULE_ID=adonis, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, haService=haService, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseExpiry=licenseExpiry, dhcpPoolSize=dhcpPoolSize, dhcpPoolAlert=dhcpPoolAlert, lcdDaemon=lcdDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetSize=dhcpSubnetSize, haServiceNodeType=haServiceNodeType, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonRunning=dhcpDaemonRunning, haReplicationBinding=haReplicationBinding)
