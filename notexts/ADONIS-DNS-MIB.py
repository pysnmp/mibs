...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFixedIP=dhcpFixedIP, dhcpConfig=dhcpConfig, dhcpPoolUsed=dhcpPoolUsed, adonis=adonis, dhcpLeaseHostname=dhcpLeaseHostname, haService=haService, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapHA=trapHA, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemon=dhcpDaemon, dhcpPoolSize=dhcpPoolSize, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsFailure=dnsStatsFailure, dhcpLeaseEntry=dhcpLeaseEntry, commandServer=commandServer, trapReplicationFailure=trapReplicationFailure, trapSystem=trapSystem, dnsDaemonRunning=dnsDaemonRunning, tftp=tftp, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolTable=dhcpPoolTable, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemon=dnsDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, ha=ha, haServiceRunning=haServiceRunning, trapDNS=trapDNS, commandServerDaemon=commandServerDaemon, tftpDaemon=tftpDaemon, systemState=systemState, licenseExpiry=licenseExpiry, licenseValid=licenseValid, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapCommandServer=trapCommandServer, haServiceNodeType=haServiceNodeType, dhcpPoolEntry=dhcpPoolEntry, dnsStats=dnsStats, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcp=dhcp, dhcpPoolStartIP=dhcpPoolStartIP, trapDNSDaemon=trapDNSDaemon, trapSystemDaemon=trapSystemDaemon, trapReplication=trapReplication, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsReferral=dnsStatsReferral, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonRunning=dhcpDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, adonisTraps=adonisTraps, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetSize=dhcpSubnetSize, lcd=lcd, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetMask=dhcpSubnetMask, system=system, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseTable=dhcpLeaseTable, trapTFTPDaemon=trapTFTPDaemon, adonisObjects=adonisObjects, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpStats=dhcpStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFailOverState=dhcpFailOverState, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, haReplicationBinding=haReplicationBinding, dns=dns, systemDaemon=systemDaemon, lcdDaemon=lcdDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDHCP=trapDHCP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftpDaemonRunning=tftpDaemonRunning, trapTFTP=trapTFTP, PYSNMP_MODULE_ID=adonis, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, dhcpPoolAlert=dhcpPoolAlert)
