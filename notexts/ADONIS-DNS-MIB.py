...
Bits, NotificationType, iso, TimeTicks, ObjectIdentity, MibIdentifier, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, IpAddress, Integer32, enterprises, Counter64, Unsigned32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "NotificationType", "iso", "TimeTicks", "ObjectIdentity", "MibIdentifier", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "IpAddress", "Integer32", "enterprises", "Counter64", "Unsigned32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapSystemDaemon=trapSystemDaemon, dhcpPoolTable=dhcpPoolTable, dhcpFailOverState=dhcpFailOverState, systemState=systemState, dhcpDaemon=dhcpDaemon, trapTFTPDaemon=trapTFTPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, trapCommandServerDaemon=trapCommandServerDaemon, tftp=tftp, trapHA=trapHA, adonisObjects=adonisObjects, dhcpLeaseBindState=dhcpLeaseBindState, dnsStats=dnsStats, dhcpPoolEndIP=dhcpPoolEndIP, commandServer=commandServer, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSize=dhcpPoolSize, adonisTraps=adonisTraps, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, trapReplication=trapReplication, haServiceNodeType=haServiceNodeType, dhcpPoolUsed=dhcpPoolUsed, trapCommandServer=trapCommandServer, adonis=adonis, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIPTable=dhcpFixedIPTable, commandServerDaemon=commandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonRunning=dnsDaemonRunning, haServiceRunning=haServiceRunning, PYSNMP_MODULE_ID=adonis, dhcpLeaseHostname=dhcpLeaseHostname, trapTFTP=trapTFTP, dhcpSubnetIP=dhcpSubnetIP, lcd=lcd, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, lcdDaemon=lcdDaemon, dnsStatsFailure=dnsStatsFailure, dnsStatsRecursion=dnsStatsRecursion, haReplicationBinding=haReplicationBinding, dhcpPoolSubnetIP=dhcpPoolSubnetIP, system=system, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNS=trapDNS, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsNXDomain=dnsStatsNXDomain, dhcp=dhcp, dhcpConfig=dhcpConfig, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpIP=dhcpIP, dnsStatsReferral=dnsStatsReferral, tftpDaemonRunning=tftpDaemonRunning, licenseValid=licenseValid, licenseExpiry=licenseExpiry, dhcpStats=dhcpStats, haService=haService, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFixedIP=dhcpFixedIP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCPDaemon=trapDHCPDaemon, ha=ha, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dns=dns, dhcpSubnetAlert=dhcpSubnetAlert, trapSystem=trapSystem, trapReplicationFailure=trapReplicationFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDHCP=trapDHCP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNSDaemon=trapDNSDaemon, dnsDaemon=dnsDaemon, tftpDaemon=tftpDaemon)
