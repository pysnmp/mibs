...
Integer32, NotificationType, iso, ObjectIdentity, TimeTicks, enterprises, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, Counter64, MibIdentifier, Counter32, Bits, Gauge32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "NotificationType", "iso", "ObjectIdentity", "TimeTicks", "enterprises", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "Counter64", "MibIdentifier", "Counter32", "Bits", "Gauge32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpStats=dhcpStats, system=system, dhcpPoolSize=dhcpPoolSize, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemon=dhcpDaemon, trapCommandServer=trapCommandServer, trapSystem=trapSystem, dhcpFixedIP=dhcpFixedIP, dns=dns, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, ha=ha, adonis=adonis, dhcpSubnetTable=dhcpSubnetTable, dnsStatsRecursion=dnsStatsRecursion, dhcpFailOverState=dhcpFailOverState, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseBindState=dhcpLeaseBindState, lcdDaemon=lcdDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpConfig=dhcpConfig, licenseExpiry=licenseExpiry, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolStartIP=dhcpPoolStartIP, tftp=tftp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsFailure=dnsStatsFailure, trapDNS=trapDNS, trapDHCP=trapDHCP, haService=haService, trapReplicationFailure=trapReplicationFailure, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, haReplicationBinding=haReplicationBinding, trapHAServiceFailOver=trapHAServiceFailOver, haServiceRunning=haServiceRunning, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEntry=dhcpPoolEntry, dnsStats=dnsStats, dhcpSubnetAlert=dhcpSubnetAlert, lcd=lcd, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemDaemon=systemDaemon, dnsDaemonRunning=dnsDaemonRunning, trapReplication=trapReplication, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpIP=dhcpIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDNSDaemon=trapDNSDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsReferral=dnsStatsReferral, PYSNMP_MODULE_ID=adonis, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcp=dhcp, licenseValid=licenseValid, dhcpPoolTable=dhcpPoolTable, dhcpSubnetMask=dhcpSubnetMask, adonisTraps=adonisTraps, adonisObjects=adonisObjects, tftpDaemon=tftpDaemon, systemState=systemState, trapTFTPDaemon=trapTFTPDaemon, dnsDaemon=dnsDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haServiceNodeType=haServiceNodeType, trapDHCPDaemon=trapDHCPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServer=commandServer, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTP=trapTFTP, commandServerDaemon=commandServerDaemon, trapSystemDaemon=trapSystemDaemon, dnsStatsNXDomain=dnsStatsNXDomain)
