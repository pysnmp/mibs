...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpConfig=dhcpConfig, trapDHCPDaemon=trapDHCPDaemon, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemon=commandServerDaemon, dhcpFailOverState=dhcpFailOverState, commandServer=commandServer, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseTable=dhcpLeaseTable, ha=ha, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSize=dhcpPoolSize, haServiceNodeType=haServiceNodeType, dhcpLeaseHostname=dhcpLeaseHostname, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpIP=dhcpIP, adonisTraps=adonisTraps, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haReplicationBinding=haReplicationBinding, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolEndIP=dhcpPoolEndIP, adonis=adonis, systemState=systemState, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTP=trapTFTP, dhcpSubnetAlert=dhcpSubnetAlert, lcd=lcd, adonisObjects=adonisObjects, tftp=tftp, dnsDaemonRunning=dnsDaemonRunning, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, trapReplication=trapReplication, dnsStatsReferral=dnsStatsReferral, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapReplicationFailure=trapReplicationFailure, dns=dns, licenseValid=licenseValid, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemon=dhcpDaemon, systemDaemon=systemDaemon, dhcpPoolTable=dhcpPoolTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStats=dnsStats, dhcpPoolAlert=dhcpPoolAlert, haServiceRunning=haServiceRunning, tftpDaemonRunning=tftpDaemonRunning, dhcp=dhcp, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemon=dnsDaemon, dhcpStats=dhcpStats, trapDHCP=trapDHCP, licenseExpiry=licenseExpiry, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystemDaemon=trapSystemDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNS=trapDNS, dhcpLeaseEntry=dhcpLeaseEntry, trapCommandServerDaemon=trapCommandServerDaemon, trapSystem=trapSystem, dhcpSubnetSize=dhcpSubnetSize, system=system, dnsStatsFailure=dnsStatsFailure, haService=haService, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServer=trapCommandServer, dhcpSubnetMask=dhcpSubnetMask, lcdDaemon=lcdDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIP=dhcpFixedIP, dnsStatsRecursion=dnsStatsRecursion)
