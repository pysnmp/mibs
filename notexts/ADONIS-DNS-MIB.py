...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTP=trapTFTP, tftp=tftp, dnsStats=dnsStats, dhcpFixedIPEntry=dhcpFixedIPEntry, tftpDaemonRunning=tftpDaemonRunning, trapSystemDaemon=trapSystemDaemon, trapDHCP=trapDHCP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseHostname=dhcpLeaseHostname, haServiceRunning=haServiceRunning, dhcpStats=dhcpStats, licenseValid=licenseValid, lcd=lcd, dns=dns, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, commandServerDaemon=commandServerDaemon, dhcpIP=dhcpIP, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, PYSNMP_MODULE_ID=adonis, haReplicationBinding=haReplicationBinding, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolEntry=dhcpPoolEntry, dnsDaemon=dnsDaemon, dhcpDaemon=dhcpDaemon, ha=ha, dhcpSubnetEntry=dhcpSubnetEntry, commandServer=commandServer, dhcpFailOverState=dhcpFailOverState, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIP=dhcpFixedIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapSystem=trapSystem, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, licenseExpiry=licenseExpiry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, tftpDaemon=tftpDaemon, dhcpPoolAlert=dhcpPoolAlert, dnsStatsReferral=dnsStatsReferral, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetAlert=dhcpSubnetAlert, haService=haService, haServiceNodeType=haServiceNodeType, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, dhcp=dhcp, dhcpLeaseTable=dhcpLeaseTable, trapHA=trapHA, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapReplicationFailure=trapReplicationFailure, dhcpFixedIPTable=dhcpFixedIPTable, dhcpConfig=dhcpConfig, systemDaemon=systemDaemon, trapCommandServer=trapCommandServer, dhcpPoolTable=dhcpPoolTable, trapTFTPDaemon=trapTFTPDaemon, trapDHCPDaemon=trapDHCPDaemon, system=system, adonisObjects=adonisObjects, systemState=systemState, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseEntry=dhcpLeaseEntry, lcdDaemon=lcdDaemon, dnsStatsFailure=dnsStatsFailure, adonisTraps=adonisTraps, dhcpSubnetUsed=dhcpSubnetUsed, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsRecursion=dnsStatsRecursion, trapDNS=trapDNS, dhcpPoolSize=dhcpPoolSize, dhcpLeaseBindState=dhcpLeaseBindState, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, dnsStatsSuccess=dnsStatsSuccess, adonis=adonis, trapReplication=trapReplication, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones)
