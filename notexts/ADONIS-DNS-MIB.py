...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, adonisObjects=adonisObjects, haServiceNodeType=haServiceNodeType, trapHA=trapHA, dnsStatsFailure=dnsStatsFailure, lcdDaemon=lcdDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsReferral=dnsStatsReferral, systemDaemon=systemDaemon, dhcpSubnetTable=dhcpSubnetTable, trapTFTP=trapTFTP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseStartTime=dhcpLeaseStartTime, dns=dns, commandServer=commandServer, dhcpPoolTable=dhcpPoolTable, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemon=dhcpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, trapSystemDaemon=trapSystemDaemon, trapDHCPDaemon=trapDHCPDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseBindState=dhcpLeaseBindState, licenseValid=licenseValid, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, trapDNS=trapDNS, trapCommandServer=trapCommandServer, dhcpStats=dhcpStats, trapSystem=trapSystem, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, system=system, dnsDaemon=dnsDaemon, dhcpPoolSize=dhcpPoolSize, haService=haService, dhcpPoolAlert=dhcpPoolAlert, dhcp=dhcp, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, systemState=systemState, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetIP=dhcpSubnetIP, tftpDaemon=tftpDaemon, dhcpFixedIP=dhcpFixedIP, adonis=adonis, tftp=tftp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, ha=ha, haReplicationBinding=haReplicationBinding, dnsStats=dnsStats, lcd=lcd, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisTraps=adonisTraps, PYSNMP_MODULE_ID=adonis, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFailOverState=dhcpFailOverState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServerDaemon=commandServerDaemon, trapReplication=trapReplication, dhcpLeaseTable=dhcpLeaseTable, licenseExpiry=licenseExpiry, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, dhcpIP=dhcpIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDHCP=trapDHCP, haServiceRunning=haServiceRunning, trapDNSDaemon=trapDNSDaemon, trapCommandServerDaemon=trapCommandServerDaemon, trapReplicationFailure=trapReplicationFailure)
