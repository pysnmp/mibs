...
Bits, MibIdentifier, ModuleIdentity, TimeTicks, Counter64, Counter32, iso, ObjectIdentity, NotificationType, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Unsigned32, enterprises, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "MibIdentifier", "ModuleIdentity", "TimeTicks", "Counter64", "Counter32", "iso", "ObjectIdentity", "NotificationType", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Unsigned32", "enterprises", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haReplicationBinding=haReplicationBinding, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpConfig=dhcpConfig, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolTable=dhcpPoolTable, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTable=dhcpLeaseTable, trapHA=trapHA, dhcpSubnetAlert=dhcpSubnetAlert, PYSNMP_MODULE_ID=adonis, dhcpPoolSize=dhcpPoolSize, dhcpStats=dhcpStats, haServiceNodeType=haServiceNodeType, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemon=dhcpDaemon, trapDNS=trapDNS, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsRecursion=dnsStatsRecursion, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsSuccess=dnsStatsSuccess, dns=dns, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, tftpDaemon=tftpDaemon, adonis=adonis, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, trapCommandServer=trapCommandServer, systemDaemon=systemDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDHCP=trapDHCP, dnsDaemon=dnsDaemon, commandServer=commandServer, trapSystemDaemon=trapSystemDaemon, ha=ha, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemState=systemState, system=system, dhcpPoolAlert=dhcpPoolAlert, lcdDaemon=lcdDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDNSDaemon=trapDNSDaemon, dnsStatsFailure=dnsStatsFailure, adonisObjects=adonisObjects, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSubnetIP=dhcpPoolSubnetIP, licenseValid=licenseValid, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpIP=dhcpIP, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapReplicationFailure=trapReplicationFailure, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIP=dhcpFixedIP, commandServerDaemon=commandServerDaemon, dhcp=dhcp, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEndTime=dhcpLeaseEndTime, trapHAServiceFailOver=trapHAServiceFailOver, lcd=lcd, dhcpFailOverState=dhcpFailOverState, haService=haService, trapSystem=trapSystem, trapReplication=trapReplication, tftp=tftp, haServiceRunning=haServiceRunning, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTP=trapTFTP, licenseExpiry=licenseExpiry, commandServerDaemonRunning=commandServerDaemonRunning, adonisTraps=adonisTraps, dnsStatsReferral=dnsStatsReferral, dhcpSubnetIP=dhcpSubnetIP, dnsStats=dnsStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress)
