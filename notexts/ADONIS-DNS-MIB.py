...
ObjectIdentity, Integer32, ModuleIdentity, Bits, enterprises, TimeTicks, Counter32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Unsigned32, Counter64, Gauge32, MibIdentifier, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Integer32", "ModuleIdentity", "Bits", "enterprises", "TimeTicks", "Counter32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Unsigned32", "Counter64", "Gauge32", "MibIdentifier", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcp=dhcp, dnsStatsSuccess=dnsStatsSuccess, tftp=tftp, dhcpSubnetMask=dhcpSubnetMask, dnsStatsNXRRSet=dnsStatsNXRRSet, ha=ha, system=system, dnsStatsReferral=dnsStatsReferral, trapDHCPDaemon=trapDHCPDaemon, trapDHCP=trapDHCP, tftpDaemonRunning=tftpDaemonRunning, trapDNS=trapDNS, tftpDaemon=tftpDaemon, trapTFTP=trapTFTP, licenseExpiry=licenseExpiry, trapSystem=trapSystem, trapDNSDaemon=trapDNSDaemon, dhcpSubnetUsed=dhcpSubnetUsed, trapReplicationFailure=trapReplicationFailure, dnsStatsNXDomain=dnsStatsNXDomain, dns=dns, trapCommandServerDaemon=trapCommandServerDaemon, commandServerDaemonRunning=commandServerDaemonRunning, haReplicationBinding=haReplicationBinding, systemState=systemState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, PYSNMP_MODULE_ID=adonis, adonisObjects=adonisObjects, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpStats=dhcpStats, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolSize=dhcpPoolSize, dnsStats=dnsStats, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpIP=dhcpIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, trapCommandServer=trapCommandServer, dhcpFailOverState=dhcpFailOverState, dhcpLeaseHostname=dhcpLeaseHostname, haServiceRunning=haServiceRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, licenseValid=licenseValid, lcd=lcd, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, systemDaemon=systemDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetSize=dhcpSubnetSize, dnsDaemon=dnsDaemon, commandServer=commandServer, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTable=dhcpLeaseTable, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, haService=haService, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapHAServiceFailOver=trapHAServiceFailOver, commandServerDaemon=commandServerDaemon, dhcpPoolTable=dhcpPoolTable, adonisTraps=adonisTraps, haServiceNodeType=haServiceNodeType, adonis=adonis, trapSystemDaemon=trapSystemDaemon, lcdDaemon=lcdDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsFailure=dnsStatsFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, trapReplication=trapReplication, dhcpSubnetAlert=dhcpSubnetAlert, trapHA=trapHA, dhcpDaemon=dhcpDaemon, dhcpConfig=dhcpConfig, dhcpPoolAlert=dhcpPoolAlert)
