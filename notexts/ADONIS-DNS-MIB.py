...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemon=dnsDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapCommandServer=trapCommandServer, dhcpStats=dhcpStats, tftpDaemonRunning=tftpDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetMask=dhcpSubnetMask, commandServer=commandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolStartIP=dhcpPoolStartIP, commandServerDaemon=commandServerDaemon, lcdDaemon=lcdDaemon, trapSystemDaemon=trapSystemDaemon, trapDNS=trapDNS, adonisObjects=adonisObjects, dns=dns, adonis=adonis, trapHA=trapHA, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHostname=dhcpLeaseHostname, trapTFTP=trapTFTP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsReferral=dnsStatsReferral, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIP=dhcpFixedIP, dhcpPoolAlert=dhcpPoolAlert, dnsStatsFailure=dnsStatsFailure, system=system, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPTable=dhcpFixedIPTable, adonisTraps=adonisTraps, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolTable=dhcpPoolTable, haReplicationBinding=haReplicationBinding, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpConfig=dhcpConfig, dhcpSubnetSize=dhcpSubnetSize, trapDHCP=trapDHCP, dhcpIP=dhcpIP, dhcpDaemon=dhcpDaemon, tftpDaemon=tftpDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetUsed=dhcpSubnetUsed, haServiceRunning=haServiceRunning, tftp=tftp, licenseExpiry=licenseExpiry, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonRunning=dnsDaemonRunning, systemDaemon=systemDaemon, dhcpLeaseTable=dhcpLeaseTable, trapSystem=trapSystem, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapHAServiceFailOver=trapHAServiceFailOver, dnsStats=dnsStats, dhcpSubnetTable=dhcpSubnetTable, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSize=dhcpPoolSize, lcd=lcd, dhcpFailOverState=dhcpFailOverState, trapCommandServerDaemon=trapCommandServerDaemon, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemState=systemState, haServiceNodeType=haServiceNodeType, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, ha=ha, dhcp=dhcp, licenseValid=licenseValid, trapReplication=trapReplication, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEntry=dhcpPoolEntry, haService=haService, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseStartTime=dhcpLeaseStartTime)
