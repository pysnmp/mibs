...
MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, iso, ModuleIdentity, Bits, Counter64, Counter32, ObjectIdentity, Unsigned32, TimeTicks, MibIdentifier, NotificationType, Integer32, enterprises, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "iso", "ModuleIdentity", "Bits", "Counter64", "Counter32", "ObjectIdentity", "Unsigned32", "TimeTicks", "MibIdentifier", "NotificationType", "Integer32", "enterprises", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplicationFailure=trapReplicationFailure, dhcpFailOverState=dhcpFailOverState, dhcpSubnetMask=dhcpSubnetMask, haServiceRunning=haServiceRunning, licenseValid=licenseValid, lcdDaemon=lcdDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, licenseExpiry=licenseExpiry, dhcpLeaseStartTime=dhcpLeaseStartTime, systemState=systemState, dhcpSubnetUsed=dhcpSubnetUsed, commandServer=commandServer, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, system=system, ha=ha, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemon=tftpDaemon, adonis=adonis, lcd=lcd, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapSystemDaemon=trapSystemDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonRunning=dhcpDaemonRunning, dhcpConfig=dhcpConfig, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemon=dhcpDaemon, dnsStatsReferral=dnsStatsReferral, trapHA=trapHA, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapReplication=trapReplication, dhcpPoolAlert=dhcpPoolAlert, trapDHCP=trapDHCP, dhcpLeaseBindState=dhcpLeaseBindState, tftp=tftp, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsFailure=dnsStatsFailure, adonisTraps=adonisTraps, dnsStatsNXDomain=dnsStatsNXDomain, trapSystem=trapSystem, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpFixedIP=dhcpFixedIP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolUsed=dhcpPoolUsed, haService=haService, dns=dns, haServiceNodeType=haServiceNodeType, dhcpIP=dhcpIP, trapTFTPDaemon=trapTFTPDaemon, dhcpStats=dhcpStats, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolTable=dhcpPoolTable, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemDaemon=systemDaemon, adonisObjects=adonisObjects, commandServerDaemon=commandServerDaemon, dnsDaemon=dnsDaemon, trapCommandServer=trapCommandServer, haReplicationBinding=haReplicationBinding, tftpDaemonRunning=tftpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIPTable=dhcpFixedIPTable, trapDNS=trapDNS, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSize=dhcpPoolSize, dhcpSubnetTable=dhcpSubnetTable, dnsStats=dnsStats, dnsStatsSuccess=dnsStatsSuccess, trapTFTP=trapTFTP, dhcpLeaseTable=dhcpLeaseTable, dhcp=dhcp, trapDNSDaemon=trapDNSDaemon)
