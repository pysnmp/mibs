...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolTable=dhcpPoolTable, dnsStatsFailure=dnsStatsFailure, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemon=tftpDaemon, adonisObjects=adonisObjects, dhcpDaemon=dhcpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsReferral=dnsStatsReferral, haServiceNodeType=haServiceNodeType, dhcpSubnetIP=dhcpSubnetIP, dhcp=dhcp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpIP=dhcpIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, ha=ha, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseHostname=dhcpLeaseHostname, system=system, dhcpFailOverState=dhcpFailOverState, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, licenseValid=licenseValid, commandServer=commandServer, tftpDaemonRunning=tftpDaemonRunning, systemState=systemState, adonis=adonis, trapHAServiceFailOver=trapHAServiceFailOver, haReplicationBinding=haReplicationBinding, dhcpLeaseEntry=dhcpLeaseEntry, trapHA=trapHA, dnsDaemon=dnsDaemon, trapCommandServerDaemon=trapCommandServerDaemon, haServiceRunning=haServiceRunning, trapReplication=trapReplication, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseEndTime=dhcpLeaseEndTime, lcdDaemon=lcdDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIP=dhcpFixedIP, PYSNMP_MODULE_ID=adonis, systemDaemon=systemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXDomain=dnsStatsNXDomain, dhcpConfig=dhcpConfig, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystem=trapSystem, dhcpSubnetMask=dhcpSubnetMask, trapDHCP=trapDHCP, dhcpSubnetSize=dhcpSubnetSize, dhcpStats=dhcpStats, trapDHCPDaemon=trapDHCPDaemon, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServerDaemon=commandServerDaemon, dnsStats=dnsStats, dhcpLeaseTable=dhcpLeaseTable, licenseExpiry=licenseExpiry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisTraps=adonisTraps, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftp=tftp, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, trapSystemDaemon=trapSystemDaemon, haService=haService, dhcpPoolSize=dhcpPoolSize, trapCommandServer=trapCommandServer, dnsStatsRecursion=dnsStatsRecursion, trapReplicationFailure=trapReplicationFailure, lcd=lcd, trapDNS=trapDNS, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTP=trapTFTP)
