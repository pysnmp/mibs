...
MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, TimeTicks, IpAddress, NotificationType, iso, Gauge32, Unsigned32, enterprises, Counter32, Bits, MibIdentifier, ObjectIdentity, Counter64, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "TimeTicks", "IpAddress", "NotificationType", "iso", "Gauge32", "Unsigned32", "enterprises", "Counter32", "Bits", "MibIdentifier", "ObjectIdentity", "Counter64", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetUsed=dhcpSubnetUsed, trapReplicationFailure=trapReplicationFailure, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemon=dnsDaemon, trapSystemDaemon=trapSystemDaemon, haServiceRunning=haServiceRunning, trapHA=trapHA, trapSystem=trapSystem, commandServer=commandServer, dnsDaemonRunning=dnsDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNS=trapDNS, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolAlert=dhcpPoolAlert, trapCommandServer=trapCommandServer, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemon=dhcpDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolTable=dhcpPoolTable, tftpDaemon=tftpDaemon, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, trapTFTP=trapTFTP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplication=trapReplication, systemState=systemState, dhcpPoolEntry=dhcpPoolEntry, haReplicationBinding=haReplicationBinding, dhcpSubnetEntry=dhcpSubnetEntry, dhcp=dhcp, dhcpFailOverState=dhcpFailOverState, trapDHCP=trapDHCP, dnsStats=dnsStats, trapCommandServerDaemon=trapCommandServerDaemon, licenseValid=licenseValid, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIPTable=dhcpFixedIPTable, system=system, ha=ha, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetMask=dhcpSubnetMask, dns=dns, dnsStatsNXDomain=dnsStatsNXDomain, lcdDaemon=lcdDaemon, dhcpConfig=dhcpConfig, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemDaemon=systemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsFailure=dnsStatsFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpIP=dhcpIP, haServiceNodeType=haServiceNodeType, dhcpLeaseHostname=dhcpLeaseHostname, commandServerDaemon=commandServerDaemon, tftp=tftp, licenseExpiry=licenseExpiry, adonisObjects=adonisObjects, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haService=haService, tftpDaemonRunning=tftpDaemonRunning, lcd=lcd, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSize=dhcpPoolSize, dhcpStats=dhcpStats, adonis=adonis, PYSNMP_MODULE_ID=adonis, adonisTraps=adonisTraps)
