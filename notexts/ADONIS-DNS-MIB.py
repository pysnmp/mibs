...
NotificationType, TimeTicks, enterprises, Counter64, Bits, IpAddress, Integer32, Counter32, ModuleIdentity, ObjectIdentity, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Unsigned32, iso = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "TimeTicks", "enterprises", "Counter64", "Bits", "IpAddress", "Integer32", "Counter32", "ModuleIdentity", "ObjectIdentity", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Unsigned32", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonRunning=dhcpDaemonRunning, haReplicationBinding=haReplicationBinding, dhcpDaemon=dhcpDaemon, dnsDaemonRunning=dnsDaemonRunning, trapSystem=trapSystem, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServerDaemon=commandServerDaemon, commandServer=commandServer, dhcpIP=dhcpIP, systemState=systemState, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseValid=licenseValid, dhcpPoolUsed=dhcpPoolUsed, dnsStats=dnsStats, trapDHCPDaemon=trapDHCPDaemon, haService=haService, dhcpSubnetAlert=dhcpSubnetAlert, adonisTraps=adonisTraps, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, licenseExpiry=licenseExpiry, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemon=dnsDaemon, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, system=system, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsNXDomain=dnsStatsNXDomain, trapDNS=trapDNS, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapReplication=trapReplication, dns=dns, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, dhcpSubnetSize=dhcpSubnetSize, lcd=lcd, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, ha=ha, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, systemDaemon=systemDaemon, lcdDaemon=lcdDaemon, tftpDaemonRunning=tftpDaemonRunning, tftpDaemon=tftpDaemon, dhcpSubnetTable=dhcpSubnetTable, adonisObjects=adonisObjects, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIP=dhcpFixedIP, dhcpPoolTable=dhcpPoolTable, dhcpFailOverState=dhcpFailOverState, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpConfig=dhcpConfig, dhcpPoolStartIP=dhcpPoolStartIP, dhcp=dhcp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetIP=dhcpSubnetIP, dnsStatsRecursion=dnsStatsRecursion, tftp=tftp, dhcpLeaseEntry=dhcpLeaseEntry, trapDNSDaemon=trapDNSDaemon, trapTFTPDaemon=trapTFTPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, adonis=adonis, trapDHCP=trapDHCP, dhcpLeaseTable=dhcpLeaseTable, haServiceRunning=haServiceRunning, trapCommandServer=trapCommandServer, dhcpPoolEntry=dhcpPoolEntry, trapReplicationFailure=trapReplicationFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTP=trapTFTP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsReferral=dnsStatsReferral)
