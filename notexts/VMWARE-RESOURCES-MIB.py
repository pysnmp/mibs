...
# ASN.1 source VMWARE-RESOURCES-MIB
# Source digest sha256:205396036b9489ff3d325749a8100b037f8d6334e7ed8ce707faaff248f7045b
...
if mibBuilder.loadTexts: vmwResourcesMIB.setLastUpdated('2008-10-15 00:00')
...
vmwHostBusAdapterTable = MibTable((1, 3, 6, 1, 4, 1, 6876, 3, 5, 2), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: vmwHostBusAdapterTable.setStatus('current')
vmwHostBusAdapterEntry = MibTableRow((1, 3, 6, 1, 4, 1, 6876, 3, 5, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "VMWARE-RESOURCES-MIB", "vmwHostBusAdapterIndex"))
if mibBuilder.loadTexts: vmwHostBusAdapterEntry.setStatus('current')
vmwHostBusAdapterIndex = MibTableColumn((1, 3, 6, 1, 4, 1, 6876, 3, 5, 2, 1, 1), Integer32().subtype(subtypeSpec=ValueRangeConstraint(0, 1023))).setMaxAccess("notaccessible")
...
mibBuilder.exportSymbols("VMWARE-RESOURCES-MIB", PYSNMP_MODULE_ID=vmwResourcesMIB, vmwCPU=vmwCPU, vmwHbaBusNumber=vmwHbaBusNumber, vmwHbaDeviceName=vmwHbaDeviceName, vmwHbaDriverName=vmwHbaDriverName, vmwHbaModelName=vmwHbaModelName, vmwHbaPci=vmwHbaPci, vmwHbaStatus=vmwHbaStatus, vmwHostBusAdapterEntry=vmwHostBusAdapterEntry, vmwHostBusAdapterIndex=vmwHostBusAdapterIndex, vmwHostBusAdapterNumber=vmwHostBusAdapterNumber, vmwHostBusAdapterTable=vmwHostBusAdapterTable, vmwMemAvail=vmwMemAvail, vmwMemCOS=vmwMemCOS, vmwMemSize=vmwMemSize, vmwMemory=vmwMemory, vmwNumCPUs=vmwNumCPUs, vmwResMIBGroups=vmwResMIBGroups, vmwResourceGroup=vmwResourceGroup, vmwResourceMIBCompliance=vmwResourceMIBCompliance, vmwResourceMIBCompliances=vmwResourceMIBCompliances, vmwResourceMIBConformance=vmwResourceMIBConformance, vmwResourcesMIB=vmwResourcesMIB, vmwStorage=vmwStorage)
