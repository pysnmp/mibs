...
mibBuilder.exportSymbols("ICOTERA-I6400-SERIES", curMetrReordered=curMetrReordered, histMetrKbps=histMetrKbps, histMetrGroupAddr=histMetrGroupAddr, curMetrStopTimestamp=curMetrStopTimestamp, curMetrGroupAddr=curMetrGroupAddr, curMetrJitter=curMetrJitter, histMetrTotalMaxDelta=histMetrTotalMaxDelta, curMetrSourceAddr=curMetrSourceAddr, curMetrAvgDelta=curMetrAvgDelta, histMetrTotalPackets=histMetrTotalPackets, curMetrLost=curMetrLost, ictMcastAnalyzer=ictMcastAnalyzer, histGroupAddr=histGroupAddr, histMetrStopTimestamp=histMetrStopTimestamp, curMetrTotalPackets=curMetrTotalPackets, curMetrStartTimestamp=curMetrStartTimestamp, icotera=icotera, histMetrReordered=histMetrReordered, histMetrAvgLostPps=histMetrAvgLostPps, ictMcastAnalyzerCurrentList=ictMcastAnalyzerCurrentList, catvModulePowerLevel=catvModulePowerLevel, transceiverDiagCapable=transceiverDiagCapable, upgStatus=upgStatus, histMetrTotalSkips=histMetrTotalSkips, catvModuleLowSignal=catvModuleLowSignal, transceiverDdmTxPower=transceiverDdmTxPower, ictFacRstMib=ictFacRstMib, curMetrPps=curMetrPps, histGroupIndex=histGroupIndex, histMetrTotalBytes=histMetrTotalBytes, ictCfgUpdate=ictCfgUpdate, cfgExecute=cfgExecute, ictMcastAnalyzerHistory=ictMcastAnalyzerHistory, histMetrStartTimestamp=histMetrStartTimestamp, histMetrTotalDiscontinuities=histMetrTotalDiscontinuities, transceiverDdmRxPower=transceiverDdmRxPower, transceiverTransceiverType=transceiverTransceiverType, curMetrSkips=curMetrSkips, curMetrDiscontinuities=curMetrDiscontinuities, curMetrTotalMaxDelta=curMetrTotalMaxDelta, transceiverDdmVoltage=transceiverDdmVoltage, curMetrSrcPort=curMetrSrcPort, ictMcastAnalyzerHistoryList=ictMcastAnalyzerHistoryList, catvModuleFilter=catvModuleFilter, transceiverDdmTxBiasCurrent=transceiverDdmTxBiasCurrent, histMetrAvgKbps=histMetrAvgKbps, IctDelta=IctDelta, ictIGW1k=ictIGW1k, histMetrLost=histMetrLost, ictMgmt=ictMgmt, ictServicesMibs=ictServicesMibs, curMetrAvgKbps=curMetrAvgKbps, ictMgmtMib=ictMgmtMib, performFactoryReset=performFactoryReset, ictServices=ictServices, curMetrMemberPorts=curMetrMemberPorts, transceiverLaserWavelength=transceiverLaserWavelength, catvModuleRflevel=catvModuleRflevel, catvModuleSignalDetected=catvModuleSignalDetected, histMetrSourceAddr=histMetrSourceAddr, curGroupIndex=curGroupIndex, IctJitter=IctJitter, histMetrStreamType=histMetrStreamType, histMetrAvgPps=histMetrAvgPps, curMetrTotalReordered=curMetrTotalReordered, historyMetricsEntry=historyMetricsEntry, transceiverConnectorType=transceiverConnectorType, ictMcastAnalyzerCurrent=ictMcastAnalyzerCurrent, curMetrKbps=curMetrKbps, transceiverLinkLength=transceiverLinkLength, currentMetricsEntry=currentMetricsEntry, upgUrl=upgUrl, ictMcastAnalyzerCurrentMetrics=ictMcastAnalyzerCurrentMetrics, ictReset=ictReset, ictReboot=ictReboot, histMetrAvgDelta=histMetrAvgDelta, histMetrTotalAvgDelta=histMetrTotalAvgDelta, histMetrMemberPorts=histMetrMemberPorts, cfgStatus=cfgStatus, ictTransceiver=ictTransceiver, histMetrPps=histMetrPps, cfgTftpPath=cfgTftpPath, curMetrAvgPps=curMetrAvgPps, curMetrTotalSkips=curMetrTotalSkips, performCpeReboot=performCpeReboot, curMetrTotalDiscontinuities=curMetrTotalDiscontinuities, curMetrAvgLostPps=curMetrAvgLostPps, histMetrDstPort=histMetrDstPort, curMetrStreamType=curMetrStreamType, histMetrMaxDelta=histMetrMaxDelta, catvModuleAdminStatus=catvModuleAdminStatus, curMetrTotalLost=curMetrTotalLost, curMetrTotalBytes=curMetrTotalBytes, histMetrSkips=histMetrSkips, curGroupAddr=curGroupAddr, histMetrTotalReordered=histMetrTotalReordered, currentListEntry=currentListEntry, curMetrDstPort=curMetrDstPort, ictFacRst=ictFacRst, histMetrJitter=histMetrJitter, transceiverDdmTemperature=transceiverDdmTemperature, curMetrMaxDelta=curMetrMaxDelta, histMetrTotalLost=histMetrTotalLost, curMetrTotalAvgDelta=curMetrTotalAvgDelta, histMetrDiscontinuities=histMetrDiscontinuities, transceiverEthernetCompliance=transceiverEthernetCompliance, IctTimeStamp=IctTimeStamp, historyListEntry=historyListEntry, ictMcastAnalyzerHistoryMetrics=ictMcastAnalyzerHistoryMetrics, histMetrSrcPort=histMetrSrcPort, ictFwUpg=ictFwUpg, ictCatv=ictCatv, IctPortList=IctPortList, PYSNMP_MODULE_ID=icotera, upgExecute=upgExecute)
