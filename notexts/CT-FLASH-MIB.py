...
# ASN.1 source CT-FLASH-MIB
# Source digest sha256:fc100325f7a2e7bcd654224e2587b7d4f8256ff43310f2f6a76e6a140c7901c8
...
flashVolumeStatusTable = MibTable((1, 3, 6, 1, 4, 1, 52, 4, 1, 5, 10, 1, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: flashVolumeStatusTable.setStatus('mandatory')
flashVolumeStatusEntry = MibTableRow((1, 3, 6, 1, 4, 1, 52, 4, 1, 5, 10, 1, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CT-FLASH-MIB", "flashVolume"))
...
flashFileTable = MibTable((1, 3, 6, 1, 4, 1, 52, 4, 1, 5, 10, 2, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: flashFileTable.setStatus('mandatory')
flashFileEntry = MibTableRow((1, 3, 6, 1, 4, 1, 52, 4, 1, 5, 10, 2, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CT-FLASH-MIB", "flashVolume"), (0, "CT-FLASH-MIB", "flashFileID"))
...
mibBuilder.exportSymbols("CT-FLASH-MIB", flashBlockCount=flashBlockCount, flashCmd=flashCmd, flashCmdError=flashCmdError, flashCmdFile=flashCmdFile, flashCmdNetAddress=flashCmdNetAddress, flashCmdOperation=flashCmdOperation, flashCmdPath=flashCmdPath, flashCmdSize=flashCmdSize, flashCmdStatus=flashCmdStatus, flashCmdType=flashCmdType, flashCmdVersion=flashCmdVersion, flashCmdVolume=flashCmdVolume, flashFile=flashFile, flashFileEntry=flashFileEntry, flashFileID=flashFileID, flashFileSize=flashFileSize, flashFileTable=flashFileTable, flashFileType=flashFileType, flashFileVersion=flashFileVersion, flashFilename=flashFilename, flashStatus=flashStatus, flashVolFiles=flashVolFiles, flashVolSpace=flashVolSpace, flashVolume=flashVolume, flashVolumeStatusEntry=flashVolumeStatusEntry, flashVolumeStatusTable=flashVolumeStatusTable)
