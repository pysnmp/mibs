...
# ASN.1 source T3610-MIB
# Source digest sha256:803c681fd2c47dd64eb40f00b86aac04f11b32cf90fa8d36609cacac21ad4022
...
historyTable = MibTable((1, 3, 6, 1, 4, 1, 22626, 1, 2, 6, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: historyTable.setStatus('mandatory')
historyEntry = MibTableRow((1, 3, 6, 1, 4, 1, 22626, 1, 2, 6, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "T3610-MIB", "histTemp"))
...
mibBuilder.exportSymbols("T3610-MIB", DisplayString=DisplayString, comet=comet, compVal=compVal, compValAlarm=compValAlarm, compValAlarmInt=compValAlarmInt, compValDelayInt=compValDelayInt, compValHighInt=compValHighInt, compValHystInt=compValHystInt, compValInt=compValInt, compValLowInt=compValLowInt, compValMax=compValMax, compValMin=compValMin, compValUnit=compValUnit, deviceType=deviceType, histCompVal=histCompVal, histHum=histHum, histTemp=histTemp, historyEntry=historyEntry, historyTable=historyTable, hum=hum, humAlarm=humAlarm, humAlarmInt=humAlarmInt, humDelayInt=humDelayInt, humHighInt=humHighInt, humHystInt=humHystInt, humInt=humInt, humLowInt=humLowInt, humMax=humMax, humMin=humMin, humUnit=humUnit, messageString=messageString, products=products, pysmi_global=pysmi_global, sensorName=sensorName, serialNumber=serialNumber, settings=settings, t3610=t3610, tables=tables, temp=temp, tempAlarm=tempAlarm, tempAlarmInt=tempAlarmInt, tempDelayInt=tempDelayInt, tempHighInt=tempHighInt, tempHystInt=tempHystInt, tempInt=tempInt, tempLowInt=tempLowInt, tempMax=tempMax, tempMin=tempMin, tempUnit=tempUnit, trapCompValClrAlarm=trapCompValClrAlarm, trapCompValError=trapCompValError, trapCompValHighAlarm=trapCompValHighAlarm, trapCompValLowAlarm=trapCompValLowAlarm, trapEmailErrAuth=trapEmailErrAuth, trapEmailErrDNS=trapEmailErrDNS, trapEmailErrLogin=trapEmailErrLogin, trapEmailErrSocket=trapEmailErrSocket, trapEmailErrSome=trapEmailErrSome, trapHumClrAlarm=trapHumClrAlarm, trapHumError=trapHumError, trapHumHighAlarm=trapHumHighAlarm, trapHumLowAlarm=trapHumLowAlarm, trapNTPError=trapNTPError, trapSOAPErrDNS=trapSOAPErrDNS, trapSOAPErrDelivery=trapSOAPErrDelivery, trapSOAPErrFile=trapSOAPErrFile, trapSOAPErrSocket=trapSOAPErrSocket, trapTempClrAlarm=trapTempClrAlarm, trapTempError=trapTempError, trapTempHighAlarm=trapTempHighAlarm, trapTempLowAlarm=trapTempLowAlarm, trapTest=trapTest, traps=traps, values=values, valuesInt=valuesInt)
