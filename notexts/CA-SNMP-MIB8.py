...
ModuleIdentity, iso, Opaque, Gauge32, Bits, NotificationType, Integer32, enterprises, TimeTicks, Unsigned32, ObjectIdentity, Counter64, MibIdentifier, IpAddress, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "iso", "Opaque", "Gauge32", "Bits", "NotificationType", "Integer32", "enterprises", "TimeTicks", "Unsigned32", "ObjectIdentity", "Counter64", "MibIdentifier", "IpAddress", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsProtocol=vsProtocol, totalSucc=totalSucc, requestsToHttps=requestsToHttps, logHistTableMaxLength=logHistTableMaxLength, clusterVirAddress=clusterVirAddress, peakSentBytesPerSec=peakSentBytesPerSec, succLastDay=succLastDay, cacheSkip=cacheSkip, totalIPBytesIn=totalIPBytesIn, vsHeaderHits=vsHeaderHits, ctcpConnTable=ctcpConnTable, ipBytesIn=ipBytesIn, clusterNum=clusterNum, memory=memory, fanspeed=fanspeed, slbGeneral=slbGeneral, clientEstabConn=clientEstabConn, serverEstabConn=serverEstabConn, caTraps=caTraps, hcRSCount=hcRSCount, hcIndex=hcIndex, infInUnknownProtos=infInUnknownProtos, compression=compression, ipStatsEntry=ipStatsEntry, hcName=hcName, ctcpInErrs=ctcpInErrs, gpStatsEntry=gpStatsEntry, ctcpConnRemAddr=ctcpConnRemAddr, requestsReceived=requestsReceived, virtualPort=virtualPort, systemInfo=systemInfo, slbStats=slbStats, virtualAddr=virtualAddr, reqPeakHour=reqPeakHour, sysMemoryUtilization=sysMemoryUtilization, infInMcastPkts=infInMcastPkts, infInUcastPkts=infInUcastPkts, sslStats=sslStats, sslEntry=sslEntry, vsIpAddress=vsIpAddress, vsHashURLHits=vsHashURLHits, realServerID=realServerID, cacheMissWRespNo=cacheMissWRespNo, msgText=msgText, ipStatsTable=ipStatsTable, infTotalInOctets=infTotalInOctets, clusterVirState=clusterVirState, ctcpOutSegs=ctcpOutSegs, infAddress=infAddress, powerstate=powerstate, headRequests=headRequests, ipPktsOut=ipPktsOut, clusterVirInterval=clusterVirInterval, hcAddress=hcAddress, cacheMissWNoncacheReq=cacheMissWNoncacheReq, groupIndex=groupIndex, infIpv6AddressType=infIpv6AddressType, vhostNum=vhostNum, maxCluster=maxCluster, vsConnCnt=vsConnCnt, succLastSec5=succLastSec5, resumableSess=resumableSess, succPeakMin=succPeakMin, logMaxSeverity=logMaxSeverity, vsEntry=vsEntry, healthCheck=healthCheck, clusterVirAddressType=clusterVirAddressType, virtServerID=virtServerID, ctcpInSegs=ctcpInSegs, hcStats=hcStats, caStart=caStart, vsStatsEntry=vsStatsEntry, connsPerSec=connsPerSec, rsIpAddress=rsIpAddress, clusterVirIndex=clusterVirIndex, clusterVirAuthType=clusterVirAuthType, serialNumber=serialNumber, rsStatsTable=rsStatsTable, ctcpConnLocalAddrType=ctcpConnLocalAddrType, reqLastMin=reqLastMin, ipAddr=ipAddr, responsesToHttps=responsesToHttps, vsRCookieHits=vsRCookieHits, logNotificationsEnabled=logNotificationsEnabled, infInNUcastPkts=infInNUcastPkts, gpIndex=gpIndex, caShutdown=caShutdown, ctcpOutRsts=ctcpOutRsts, vsTable=vsTable, vsCacheHits=vsCacheHits, vipStats=vipStats, failLastSec5=failLastSec5, hcStatsTable=hcStatsTable, vsRedirectHits=vsRedirectHits, gpEntry=gpEntry, vsURLHits=vsURLHits, failLastHour=failLastHour, gpMetrics=gpMetrics, realStats=realStats, rsMaxConn=rsMaxConn, infOutErrors=infOutErrors, infTable=infTable, realStatus=realStatus, infIpv6Address=infIpv6Address, missSess=missSess, logHistoryTable=logHistoryTable, gpTotalHits=gpTotalHits, ctcpConnRemPort=ctcpConnRemPort, sslGeneral=sslGeneral, vsPort=vsPort, responsesOnRegex=responsesOnRegex, totalRequestedConns=totalRequestedConns, groupStats=groupStats, vsIndex=vsIndex, licenseRemainingDays=licenseRemainingDays, infInDiscards=infInDiscards, PYSNMP_MODULE_ID=arrayNetworks, infIpv4AddressType=infIpv4AddressType, realIndex=realIndex, acceptedConns=acceptedConns, failLastMin=failLastMin, totalOpenSSLConns=totalOpenSSLConns, openSSLConns=openSSLConns, monitor=monitor, rsTable=rsTable, hcCause=hcCause, vsQosURLHits=vsQosURLHits, gpTable=gpTable, gpStatsTable=gpStatsTable, clusterVirAuthPasswd=clusterVirAuthPasswd, vsMaxConn=vsMaxConn, virtualAddress=virtualAddress, clusterVirAddr=clusterVirAddr, vsRegexHits=vsRegexHits, startTime=startTime, succPeakHour=succPeakHour, rsConnCnt=rsConnCnt, vrrp=vrrp, vsQosBodyHits=vsQosBodyHits, reqLastDay=reqLastDay, vsIpAddressType=vsIpAddressType, totalBytesRcvd=totalBytesRcvd, reqLastHour=reqLastHour, infInBandwidth=infInBandwidth, clusterVirPreempt=clusterVirPreempt, clusterVirPriority=clusterVirPriority, infOutBandwidth=infOutBandwidth, sysSwapUsed=sysSwapUsed, ipPktsIn=ipPktsIn, vsQosCookieHits=vsQosCookieHits, vrrpEntry=vrrpEntry, reqLastSec5=reqLastSec5, infInBcastPkts=infInBcastPkts, hitsReplyWPreFailed=hitsReplyWPreFailed, totalFail=totalFail, caSyslogTrap=caSyslogTrap, reqPeakSec5=reqPeakSec5, vsID=vsID, infDescr=infDescr, Float=Float, sslMIB=sslMIB, vsQosNetworkHits=vsQosNetworkHits, totalIPPktsIn=totalIPPktsIn, requestsToUrl=requestsToUrl, reqPeakDay=reqPeakDay, hitsReplyWNotModified=hitsReplyWNotModified, ctcpConnRemAddress=ctcpConnRemAddress, ctcpConnRemAddrType=ctcpConnRemAddrType, hcNumDowns=hcNumDowns, hitsReply=hitsReply, connectionsPerSec=connectionsPerSec, realAddress=realAddress, hcStatsEntry=hcStatsEntry, logNotificationsSent=logNotificationsSent, hcNumUps=hcNumUps, index=index, ctcpPassiveOpens=ctcpPassiveOpens, cacheMissWNewEntry=cacheMissWNewEntry, ctcpConnState=ctcpConnState, vsStatsTable=vsStatsTable, ctcpConnEntry=ctcpConnEntry, infOperStatus=infOperStatus, hcPort=hcPort, ctcpConnLocalAddr=ctcpConnLocalAddr, vsDefaultHits=vsDefaultHits, vrrpTable=vrrpTable, infNumber=infNumber, SyslogSeverity=SyslogSeverity, logBasic=logBasic, infIndex=infIndex, hcAddressType=hcAddressType, infOutNUcastPkts=infOutNUcastPkts, clusterVrrp=clusterVrrp, vsStaticHits=vsStaticHits, hitRevalidate=hitRevalidate, cacheBasicStats=cacheBasicStats, rcvdBytesPerSec=rcvdBytesPerSec, succPeakDay=succPeakDay, sslTable=sslTable, vsIpAddr=vsIpAddr, reqPeakMin=reqPeakMin, ipAddress=ipAddress, rsStatsEntry=rsStatsEntry, cacheHitRatio=cacheHitRatio, rsIpAddr=rsIpAddr, infOutUcastPkts=infOutUcastPkts, reqPeakSec=reqPeakSec, sdns=sdns, syslogTrap=syslogTrap, ctcpRetransSegs=ctcpRetransSegs, infTotalOutOctets=infTotalOutOctets, hcConnAttempt=hcConnAttempt, cputemp=cputemp, realAddr=realAddr, ctcpConnLocalPort=ctcpConnLocalPort, ctcpIndex=ctcpIndex, vsBackupHits=vsBackupHits, rsStatus=rsStatus, ctcpConnLocalAddress=ctcpConnLocalAddress, infIpv4Address=infIpv4Address, sysSwapCapacity=sysSwapCapacity, rsTotalHits=rsTotalHits, severity=severity, hostName=hostName, realID=realID, vsHostnameHits=vsHostnameHits, rsProtocol=rsProtocol, succLastMin=succLastMin, requestsPerSec=requestsPerSec, arrayNetworks=arrayNetworks, requestedConns=requestedConns, caSyslog=caSyslog, vsQosClientPortHits=vsQosClientPortHits, clickTcp=clickTcp, succLastSec=succLastSec, virtualAddressType=virtualAddressType, rsIpAddressType=rsIpAddressType, infOutMcastPkts=infOutMcastPkts, clusterId=clusterId, purgeRequests=purgeRequests, ctcpAttemptFails=ctcpAttemptFails, vsCount=vsCount, succPeakSec5=succPeakSec5, virtualIndex=virtualIndex, totalReq=totalReq, ctcpCurrEstab=ctcpCurrEstab, vsPerstntCookieHits=vsPerstntCookieHits, ctcpEstabResets=ctcpEstabResets, ipIndex=ipIndex, rsCntOfReq=rsCntOfReq, peakRcvdBytesPerSec=peakRcvdBytesPerSec, sentBytesPerSec=sentBytesPerSec, ipBytesOut=ipBytesOut, virtualStats=virtualStats)
mibBuilder.exportSymbols("CA-SNMP-MIB8", logHistoryEntry=logHistoryEntry, infEntry=infEntry, postRequests=postRequests, totalIPBytesOut=totalIPBytesOut, totalAcceptedConns=totalAcceptedConns, infInOctets=infInOctets, hcStatus=hcStatus, failLastSec=failLastSec, logHistory=logHistory, rsCount=rsCount, clusterVirIfname=clusterVirIfname, vsPerstntURLHits=vsPerstntURLHits, reqLastSec=reqLastSec, currentTime=currentTime, gpID=gpID, slbMIB=slbMIB, failLastDay=failLastDay, sysMemory=sysMemory, getRequests=getRequests, ctcpActiveOpens=ctcpActiveOpens, rsID=rsID, realPort=realPort, resumedSess=resumedSess, infOutOctets=infOutOctets, hcConnSuccess=hcConnSuccess, realAddressType=realAddressType, sslIndex=sslIndex, infOutBcastPkts=infOutBcastPkts, groupID=groupID, ifTraffic=ifTraffic, groupCount=groupCount, vsICookieHits=vsICookieHits, rsAvgRespTime=rsAvgRespTime, totalIPPktsOut=totalIPPktsOut, rsPort=rsPort, succLastHour=succLastHour, vipStatus=vipStatus, cacheStatus=cacheStatus, rsEntry=rsEntry, rsIndex=rsIndex, totalBytesSent=totalBytesSent, cpuUtilization=cpuUtilization, infInErrors=infInErrors, groupCurCfg=groupCurCfg, revProxyCache=revProxyCache, requestsOnRegex=requestsOnRegex, succPeakSec=succPeakSec, hcAddr=hcAddr, vhostName=vhostName, performance=performance, ipAddrType=ipAddrType, hcConnFail=hcConnFail, realServer=realServer, activeTransac=activeTransac, virtualServer=virtualServer)
