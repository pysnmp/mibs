...
Counter32, ModuleIdentity, ObjectIdentity, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, Unsigned32, enterprises, Integer32, Opaque, IpAddress, TimeTicks, NotificationType, Bits, iso, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "ModuleIdentity", "ObjectIdentity", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "Unsigned32", "enterprises", "Integer32", "Opaque", "IpAddress", "TimeTicks", "NotificationType", "Bits", "iso", "Gauge32")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", totalFail=totalFail, ctcpConnLocalPort=ctcpConnLocalPort, infInBandwidth=infInBandwidth, vsHeaderHits=vsHeaderHits, vsConnCnt=vsConnCnt, succPeakSec=succPeakSec, cacheStatus=cacheStatus, monitor=monitor, revProxyCache=revProxyCache, sysSwapUsed=sysSwapUsed, headRequests=headRequests, cacheMissWNewEntry=cacheMissWNewEntry, rsCount=rsCount, vrrpTable=vrrpTable, hitsReply=hitsReply, ctcpConnEntry=ctcpConnEntry, cacheMissWRespNo=cacheMissWRespNo, hcNumUps=hcNumUps, logNotificationsSent=logNotificationsSent, sentBytesPerSec=sentBytesPerSec, realID=realID, ctcpConnRemAddr=ctcpConnRemAddr, hcStats=hcStats, vsRCookieHits=vsRCookieHits, infInErrors=infInErrors, vsCacheHits=vsCacheHits, vsQosCookieHits=vsQosCookieHits, cacheSkip=cacheSkip, clusterVirAuthPasswd=clusterVirAuthPasswd, logBasic=logBasic, clusterVirState=clusterVirState, gpID=gpID, hcPort=hcPort, systemInfo=systemInfo, connsPerSec=connsPerSec, startTime=startTime, infOutUcastPkts=infOutUcastPkts, clickTcp=clickTcp, severity=severity, virtualAddressType=virtualAddressType, reqPeakDay=reqPeakDay, ctcpConnTable=ctcpConnTable, acceptedConns=acceptedConns, hcAddr=hcAddr, vhostNum=vhostNum, hcName=hcName, vrrpEntry=vrrpEntry, sdns=sdns, sslStats=sslStats, sslMIB=sslMIB, ctcpIndex=ctcpIndex, vsPort=vsPort, failLastMin=failLastMin, currentTime=currentTime, gpTable=gpTable, hcStatsEntry=hcStatsEntry, totalBytesSent=totalBytesSent, hcConnAttempt=hcConnAttempt, infTotalInOctets=infTotalInOctets, infInDiscards=infInDiscards, hitsReplyWNotModified=hitsReplyWNotModified, requestsToUrl=requestsToUrl, fanspeed=fanspeed, vrrp=vrrp, rsPort=rsPort, infInOctets=infInOctets, clusterVirPreempt=clusterVirPreempt, rsStatus=rsStatus, realIndex=realIndex, virtualIndex=virtualIndex, vsICookieHits=vsICookieHits, slbGeneral=slbGeneral, totalSucc=totalSucc, realAddress=realAddress, vhostName=vhostName, serialNumber=serialNumber, rsMaxConn=rsMaxConn, groupCount=groupCount, resumedSess=resumedSess, ctcpInErrs=ctcpInErrs, vsBackupHits=vsBackupHits, clusterVirAddress=clusterVirAddress, succPeakMin=succPeakMin, totalIPPktsIn=totalIPPktsIn, infOperStatus=infOperStatus, hcCause=hcCause, ipIndex=ipIndex, requestedConns=requestedConns, infEntry=infEntry, rsStatsEntry=rsStatsEntry, failLastDay=failLastDay, openSSLConns=openSSLConns, SyslogSeverity=SyslogSeverity, ipAddrType=ipAddrType, ifTraffic=ifTraffic, clusterVirAuthType=clusterVirAuthType, ctcpConnLocalAddrType=ctcpConnLocalAddrType, cacheBasicStats=cacheBasicStats, cpuUtilization=cpuUtilization, rsIpAddress=rsIpAddress, vsQosNetworkHits=vsQosNetworkHits, infNumber=infNumber, memory=memory, responsesToHttps=responsesToHttps, ctcpRetransSegs=ctcpRetransSegs, vsTable=vsTable, realStatus=realStatus, virtServerID=virtServerID, succPeakHour=succPeakHour, sslGeneral=sslGeneral, failLastSec5=failLastSec5, caSyslogTrap=caSyslogTrap, logMaxSeverity=logMaxSeverity, infInNUcastPkts=infInNUcastPkts, vsRedirectHits=vsRedirectHits, postRequests=postRequests, gpMetrics=gpMetrics, reqPeakSec5=reqPeakSec5, ipStatsEntry=ipStatsEntry, vsHashURLHits=vsHashURLHits, rcvdBytesPerSec=rcvdBytesPerSec, succPeakSec5=succPeakSec5, failLastSec=failLastSec, hitRevalidate=hitRevalidate, ctcpCurrEstab=ctcpCurrEstab, ctcpConnRemPort=ctcpConnRemPort, rsID=rsID, infOutErrors=infOutErrors, reqPeakMin=reqPeakMin, hcAddress=hcAddress, vsEntry=vsEntry, rsConnCnt=rsConnCnt, clusterNum=clusterNum, gpIndex=gpIndex, infTable=infTable, performance=performance, clientEstabConn=clientEstabConn, clusterVrrp=clusterVrrp, ctcpConnRemAddrType=ctcpConnRemAddrType, rsTable=rsTable, succLastMin=succLastMin, hcNumDowns=hcNumDowns, peakRcvdBytesPerSec=peakRcvdBytesPerSec, sysMemoryUtilization=sysMemoryUtilization, caStart=caStart, caShutdown=caShutdown, rsTotalHits=rsTotalHits, logHistoryTable=logHistoryTable, requestsOnRegex=requestsOnRegex, vsMaxConn=vsMaxConn, infIpv6Address=infIpv6Address, caTraps=caTraps, ctcpOutRsts=ctcpOutRsts, rsIpAddr=rsIpAddr, logNotificationsEnabled=logNotificationsEnabled, vsIndex=vsIndex, maxCluster=maxCluster, requestsToHttps=requestsToHttps, realStats=realStats, cputemp=cputemp, vsIpAddressType=vsIpAddressType, vsID=vsID, vsURLHits=vsURLHits, realServerID=realServerID, hitsReplyWPreFailed=hitsReplyWPreFailed, gpTotalHits=gpTotalHits, infInUcastPkts=infInUcastPkts, rsStatsTable=rsStatsTable, gpEntry=gpEntry, logHistoryEntry=logHistoryEntry, virtualAddr=virtualAddr, ctcpConnLocalAddress=ctcpConnLocalAddress, requestsPerSec=requestsPerSec, hcStatsTable=hcStatsTable, ipAddress=ipAddress, missSess=missSess, ipPktsOut=ipPktsOut, responsesOnRegex=responsesOnRegex, ctcpEstabResets=ctcpEstabResets, clusterId=clusterId, ipBytesOut=ipBytesOut, gpStatsTable=gpStatsTable, realPort=realPort, vsQosClientPortHits=vsQosClientPortHits, realAddressType=realAddressType, ctcpActiveOpens=ctcpActiveOpens, sslTable=sslTable, hcConnFail=hcConnFail, vsQosBodyHits=vsQosBodyHits, totalAcceptedConns=totalAcceptedConns, ipStatsTable=ipStatsTable, vsQosURLHits=vsQosURLHits, hcConnSuccess=hcConnSuccess, sysMemory=sysMemory, reqPeakHour=reqPeakHour, vsIpAddress=vsIpAddress, infTotalOutOctets=infTotalOutOctets, hcAddressType=hcAddressType, slbMIB=slbMIB, connectionsPerSec=connectionsPerSec, ctcpOutSegs=ctcpOutSegs, rsCntOfReq=rsCntOfReq, reqLastDay=reqLastDay, hcIndex=hcIndex, infIpv4AddressType=infIpv4AddressType, vsStatsEntry=vsStatsEntry, totalOpenSSLConns=totalOpenSSLConns, clusterVirInterval=clusterVirInterval, rsIpAddressType=rsIpAddressType, vsRegexHits=vsRegexHits, infOutOctets=infOutOctets, cacheMissWNoncacheReq=cacheMissWNoncacheReq, ctcpAttemptFails=ctcpAttemptFails, PYSNMP_MODULE_ID=arrayNetworks, sysSwapCapacity=sysSwapCapacity, ipPktsIn=ipPktsIn, caSyslog=caSyslog, arrayNetworks=arrayNetworks, clusterVirAddressType=clusterVirAddressType, gpStatsEntry=gpStatsEntry, rsIndex=rsIndex, infDescr=infDescr, groupCurCfg=groupCurCfg, realServer=realServer, infInMcastPkts=infInMcastPkts, logHistTableMaxLength=logHistTableMaxLength, healthCheck=healthCheck, infInBcastPkts=infInBcastPkts, vsPerstntURLHits=vsPerstntURLHits, infIpv6AddressType=infIpv6AddressType, activeTransac=activeTransac, ctcpInSegs=ctcpInSegs, vsIpAddr=vsIpAddr, groupIndex=groupIndex, msgText=msgText, clusterVirIndex=clusterVirIndex, Float=Float, infIndex=infIndex, infOutMcastPkts=infOutMcastPkts, vsPerstntCookieHits=vsPerstntCookieHits, licenseRemainingDays=licenseRemainingDays, succPeakDay=succPeakDay, purgeRequests=purgeRequests, slbStats=slbStats, virtualStats=virtualStats, serverEstabConn=serverEstabConn, ctcpConnLocalAddr=ctcpConnLocalAddr, ctcpConnRemAddress=ctcpConnRemAddress, succLastDay=succLastDay, totalIPBytesIn=totalIPBytesIn, ctcpPassiveOpens=ctcpPassiveOpens, index=index, syslogTrap=syslogTrap, clusterVirAddr=clusterVirAddr)
mibBuilder.exportSymbols("CA-SNMP-MIB8", infAddress=infAddress, totalBytesRcvd=totalBytesRcvd, totalRequestedConns=totalRequestedConns, compression=compression, vsProtocol=vsProtocol, logHistory=logHistory, powerstate=powerstate, vipStatus=vipStatus, cacheHitRatio=cacheHitRatio, groupStats=groupStats, vsHostnameHits=vsHostnameHits, sslIndex=sslIndex, totalIPPktsOut=totalIPPktsOut, reqLastSec=reqLastSec, peakSentBytesPerSec=peakSentBytesPerSec, succLastHour=succLastHour, infOutBcastPkts=infOutBcastPkts, virtualPort=virtualPort, clusterVirPriority=clusterVirPriority, infInUnknownProtos=infInUnknownProtos, infOutBandwidth=infOutBandwidth, virtualAddress=virtualAddress, sslEntry=sslEntry, reqPeakSec=reqPeakSec, rsProtocol=rsProtocol, virtualServer=virtualServer, vsDefaultHits=vsDefaultHits, rsEntry=rsEntry, ctcpConnState=ctcpConnState, succLastSec5=succLastSec5, reqLastHour=reqLastHour, vipStats=vipStats, requestsReceived=requestsReceived, hcRSCount=hcRSCount, reqLastSec5=reqLastSec5, totalIPBytesOut=totalIPBytesOut, vsCount=vsCount, realAddr=realAddr, totalReq=totalReq, ipBytesIn=ipBytesIn, infOutNUcastPkts=infOutNUcastPkts, vsStaticHits=vsStaticHits, groupID=groupID, getRequests=getRequests, resumableSess=resumableSess, failLastHour=failLastHour, succLastSec=succLastSec, clusterVirIfname=clusterVirIfname, hostName=hostName, vsStatsTable=vsStatsTable, rsAvgRespTime=rsAvgRespTime, hcStatus=hcStatus, ipAddr=ipAddr, infIpv4Address=infIpv4Address, reqLastMin=reqLastMin)
