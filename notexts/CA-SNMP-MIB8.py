...
Integer32, ModuleIdentity, Counter32, Bits, Opaque, Gauge32, iso, IpAddress, TimeTicks, Counter64, ObjectIdentity, Unsigned32, enterprises, NotificationType, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "ModuleIdentity", "Counter32", "Bits", "Opaque", "Gauge32", "iso", "IpAddress", "TimeTicks", "Counter64", "ObjectIdentity", "Unsigned32", "enterprises", "NotificationType", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsMaxConn=vsMaxConn, rsTotalHits=rsTotalHits, hitRevalidate=hitRevalidate, ctcpOutSegs=ctcpOutSegs, rsPort=rsPort, reqPeakHour=reqPeakHour, virtualPort=virtualPort, hcNumUps=hcNumUps, rsAvgRespTime=rsAvgRespTime, hcNumDowns=hcNumDowns, realAddr=realAddr, totalBytesSent=totalBytesSent, msgText=msgText, rsMaxConn=rsMaxConn, requestsReceived=requestsReceived, hcPort=hcPort, slbMIB=slbMIB, cacheSkip=cacheSkip, ctcpConnState=ctcpConnState, hcAddress=hcAddress, hostName=hostName, totalIPPktsIn=totalIPPktsIn, reqLastDay=reqLastDay, slbGeneral=slbGeneral, headRequests=headRequests, hcStatsEntry=hcStatsEntry, ipStatsTable=ipStatsTable, realPort=realPort, index=index, getRequests=getRequests, slbStats=slbStats, rsStatsEntry=rsStatsEntry, vsIpAddress=vsIpAddress, totalIPPktsOut=totalIPPktsOut, vsIpAddressType=vsIpAddressType, ctcpConnRemAddress=ctcpConnRemAddress, rsConnCnt=rsConnCnt, activeTransac=activeTransac, failLastMin=failLastMin, sslIndex=sslIndex, infInDiscards=infInDiscards, hcConnFail=hcConnFail, caShutdown=caShutdown, vrrp=vrrp, rcvdBytesPerSec=rcvdBytesPerSec, ctcpConnRemAddr=ctcpConnRemAddr, virtualStats=virtualStats, reqPeakMin=reqPeakMin, hcStatus=hcStatus, infOutOctets=infOutOctets, clickTcp=clickTcp, totalAcceptedConns=totalAcceptedConns, caStart=caStart, memory=memory, ipPktsIn=ipPktsIn, infIpv4AddressType=infIpv4AddressType, sslEntry=sslEntry, logNotificationsEnabled=logNotificationsEnabled, connectionsPerSec=connectionsPerSec, hitsReply=hitsReply, reqLastSec=reqLastSec, hcStatsTable=hcStatsTable, cacheBasicStats=cacheBasicStats, rsProtocol=rsProtocol, reqPeakDay=reqPeakDay, rsID=rsID, realServerID=realServerID, healthCheck=healthCheck, logNotificationsSent=logNotificationsSent, currentTime=currentTime, infIpv4Address=infIpv4Address, postRequests=postRequests, virtServerID=virtServerID, requestsToUrl=requestsToUrl, virtualServer=virtualServer, ifTraffic=ifTraffic, ipBytesOut=ipBytesOut, succPeakSec=succPeakSec, infOutBandwidth=infOutBandwidth, rsEntry=rsEntry, totalFail=totalFail, responsesToHttps=responsesToHttps, groupCount=groupCount, compression=compression, vipStatus=vipStatus, infEntry=infEntry, infTotalInOctets=infTotalInOctets, groupCurCfg=groupCurCfg, vrrpEntry=vrrpEntry, logHistoryEntry=logHistoryEntry, rsIpAddressType=rsIpAddressType, vsStatsEntry=vsStatsEntry, totalOpenSSLConns=totalOpenSSLConns, totalRequestedConns=totalRequestedConns, gpStatsTable=gpStatsTable, maxCluster=maxCluster, gpIndex=gpIndex, clusterVrrp=clusterVrrp, reqLastSec5=reqLastSec5, infAddress=infAddress, infTable=infTable, requestsOnRegex=requestsOnRegex, reqLastHour=reqLastHour, startTime=startTime, logMaxSeverity=logMaxSeverity, groupID=groupID, hitsReplyWNotModified=hitsReplyWNotModified, sslMIB=sslMIB, infOutUcastPkts=infOutUcastPkts, virtualAddressType=virtualAddressType, infDescr=infDescr, systemInfo=systemInfo, ctcpRetransSegs=ctcpRetransSegs, ipAddress=ipAddress, SyslogSeverity=SyslogSeverity, ipIndex=ipIndex, peakRcvdBytesPerSec=peakRcvdBytesPerSec, vsEntry=vsEntry, vsPort=vsPort, ctcpInErrs=ctcpInErrs, vsDefaultHits=vsDefaultHits, purgeRequests=purgeRequests, failLastSec5=failLastSec5, rsIpAddr=rsIpAddr, infOutErrors=infOutErrors, vsHostnameHits=vsHostnameHits, vsPerstntCookieHits=vsPerstntCookieHits, realID=realID, gpTable=gpTable, responsesOnRegex=responsesOnRegex, vsCount=vsCount, clusterVirIndex=clusterVirIndex, totalSucc=totalSucc, rsIndex=rsIndex, succLastMin=succLastMin, requestsToHttps=requestsToHttps, cacheMissWNoncacheReq=cacheMissWNoncacheReq, hcName=hcName, hcConnAttempt=hcConnAttempt, rsStatsTable=rsStatsTable, gpTotalHits=gpTotalHits, monitor=monitor, ctcpConnLocalAddrType=ctcpConnLocalAddrType, sysMemoryUtilization=sysMemoryUtilization, logBasic=logBasic, vsRedirectHits=vsRedirectHits, succLastHour=succLastHour, vsHashURLHits=vsHashURLHits, clusterVirPreempt=clusterVirPreempt, ipPktsOut=ipPktsOut, infIpv6Address=infIpv6Address, vsTable=vsTable, infIndex=infIndex, succPeakDay=succPeakDay, caTraps=caTraps, hcIndex=hcIndex, sentBytesPerSec=sentBytesPerSec, hcStats=hcStats, ctcpConnEntry=ctcpConnEntry, gpEntry=gpEntry, syslogTrap=syslogTrap, vsCacheHits=vsCacheHits, infInBcastPkts=infInBcastPkts, vsPerstntURLHits=vsPerstntURLHits, succPeakHour=succPeakHour, resumedSess=resumedSess, arrayNetworks=arrayNetworks, vsStatsTable=vsStatsTable, totalReq=totalReq, succLastDay=succLastDay, virtualAddr=virtualAddr, infNumber=infNumber, infOutNUcastPkts=infOutNUcastPkts, infInNUcastPkts=infInNUcastPkts, rsStatus=rsStatus, ctcpConnRemAddrType=ctcpConnRemAddrType, vhostName=vhostName, infOutMcastPkts=infOutMcastPkts, infIpv6AddressType=infIpv6AddressType, vrrpTable=vrrpTable, logHistoryTable=logHistoryTable, ctcpConnLocalAddr=ctcpConnLocalAddr, infInOctets=infInOctets, succPeakMin=succPeakMin, groupIndex=groupIndex, ipBytesIn=ipBytesIn, Float=Float, vsIndex=vsIndex, rsTable=rsTable, virtualAddress=virtualAddress, licenseRemainingDays=licenseRemainingDays, virtualIndex=virtualIndex, logHistory=logHistory, rsIpAddress=rsIpAddress, sdns=sdns, revProxyCache=revProxyCache, vsQosNetworkHits=vsQosNetworkHits, clusterVirPriority=clusterVirPriority, requestedConns=requestedConns, ipStatsEntry=ipStatsEntry, realStats=realStats, vsQosURLHits=vsQosURLHits, infInUcastPkts=infInUcastPkts, clusterVirAddressType=clusterVirAddressType, hcAddressType=hcAddressType, missSess=missSess, cpuUtilization=cpuUtilization, connsPerSec=connsPerSec, clusterId=clusterId, rsCntOfReq=rsCntOfReq, hcConnSuccess=hcConnSuccess, clusterNum=clusterNum, requestsPerSec=requestsPerSec, realServer=realServer, sysMemory=sysMemory, failLastSec=failLastSec, infInBandwidth=infInBandwidth, infInMcastPkts=infInMcastPkts, vsICookieHits=vsICookieHits, groupStats=groupStats, hcRSCount=hcRSCount, vsQosCookieHits=vsQosCookieHits, rsCount=rsCount, PYSNMP_MODULE_ID=arrayNetworks, vsBackupHits=vsBackupHits, vsQosClientPortHits=vsQosClientPortHits, reqPeakSec=reqPeakSec, clusterVirState=clusterVirState, cacheHitRatio=cacheHitRatio, acceptedConns=acceptedConns, infOutBcastPkts=infOutBcastPkts, vsConnCnt=vsConnCnt, infOperStatus=infOperStatus, realAddressType=realAddressType, ctcpOutRsts=ctcpOutRsts, vhostNum=vhostNum, resumableSess=resumableSess, openSSLConns=openSSLConns, serverEstabConn=serverEstabConn, ipAddr=ipAddr, vsURLHits=vsURLHits, powerstate=powerstate, ctcpConnLocalPort=ctcpConnLocalPort, clusterVirIfname=clusterVirIfname, cacheStatus=cacheStatus, sslStats=sslStats, ctcpConnTable=ctcpConnTable, vsQosBodyHits=vsQosBodyHits, reqLastMin=reqLastMin, realStatus=realStatus, vsRegexHits=vsRegexHits, failLastDay=failLastDay, sslGeneral=sslGeneral, logHistTableMaxLength=logHistTableMaxLength)
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsProtocol=vsProtocol, infInErrors=infInErrors, clusterVirAuthType=clusterVirAuthType, vsID=vsID, failLastHour=failLastHour, ctcpConnLocalAddress=ctcpConnLocalAddress, severity=severity, ctcpEstabResets=ctcpEstabResets, gpStatsEntry=gpStatsEntry, gpID=gpID, vipStats=vipStats, caSyslog=caSyslog, clusterVirAddr=clusterVirAddr, cacheMissWRespNo=cacheMissWRespNo, ipAddrType=ipAddrType, succLastSec=succLastSec, vsStaticHits=vsStaticHits, vsHeaderHits=vsHeaderHits, clusterVirAddress=clusterVirAddress, hitsReplyWPreFailed=hitsReplyWPreFailed, ctcpActiveOpens=ctcpActiveOpens, clusterVirAuthPasswd=clusterVirAuthPasswd, peakSentBytesPerSec=peakSentBytesPerSec, gpMetrics=gpMetrics, ctcpAttemptFails=ctcpAttemptFails, hcAddr=hcAddr, totalBytesRcvd=totalBytesRcvd, sslTable=sslTable, clusterVirInterval=clusterVirInterval, vsIpAddr=vsIpAddr, reqPeakSec5=reqPeakSec5, vsRCookieHits=vsRCookieHits, cacheMissWNewEntry=cacheMissWNewEntry, infTotalOutOctets=infTotalOutOctets, totalIPBytesOut=totalIPBytesOut, fanspeed=fanspeed, ctcpInSegs=ctcpInSegs, ctcpIndex=ctcpIndex, caSyslogTrap=caSyslogTrap, clientEstabConn=clientEstabConn, totalIPBytesIn=totalIPBytesIn, hcCause=hcCause, sysSwapUsed=sysSwapUsed, ctcpCurrEstab=ctcpCurrEstab, infInUnknownProtos=infInUnknownProtos, succPeakSec5=succPeakSec5, ctcpConnRemPort=ctcpConnRemPort, succLastSec5=succLastSec5, realAddress=realAddress, sysSwapCapacity=sysSwapCapacity, serialNumber=serialNumber, performance=performance, realIndex=realIndex, ctcpPassiveOpens=ctcpPassiveOpens, cputemp=cputemp)
