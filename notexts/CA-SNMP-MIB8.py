...
ModuleIdentity, Gauge32, ObjectIdentity, IpAddress, TimeTicks, MibIdentifier, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Bits, Counter64, NotificationType, iso, enterprises, Opaque, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Gauge32", "ObjectIdentity", "IpAddress", "TimeTicks", "MibIdentifier", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Bits", "Counter64", "NotificationType", "iso", "enterprises", "Opaque", "Unsigned32")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", vrrpTable=vrrpTable, clusterVirInterval=clusterVirInterval, totalSucc=totalSucc, infOperStatus=infOperStatus, ipPktsIn=ipPktsIn, cacheMissWRespNo=cacheMissWRespNo, virtualStats=virtualStats, missSess=missSess, totalBytesRcvd=totalBytesRcvd, hitRevalidate=hitRevalidate, virtualServer=virtualServer, ctcpOutRsts=ctcpOutRsts, reqPeakDay=reqPeakDay, hcIndex=hcIndex, infInUcastPkts=infInUcastPkts, ctcpConnLocalAddress=ctcpConnLocalAddress, vsRedirectHits=vsRedirectHits, rsConnCnt=rsConnCnt, fanspeed=fanspeed, clusterVirAuthPasswd=clusterVirAuthPasswd, reqLastSec5=reqLastSec5, vsRegexHits=vsRegexHits, PYSNMP_MODULE_ID=arrayNetworks, totalFail=totalFail, clusterVirIfname=clusterVirIfname, infInErrors=infInErrors, clusterVirAuthType=clusterVirAuthType, vsICookieHits=vsICookieHits, vsEntry=vsEntry, vsPort=vsPort, vsStaticHits=vsStaticHits, failLastMin=failLastMin, ctcpInErrs=ctcpInErrs, ipAddr=ipAddr, memory=memory, ipStatsEntry=ipStatsEntry, infEntry=infEntry, clusterVirAddressType=clusterVirAddressType, totalIPPktsIn=totalIPPktsIn, ipIndex=ipIndex, hcRSCount=hcRSCount, infInBandwidth=infInBandwidth, ctcpConnTable=ctcpConnTable, succLastSec5=succLastSec5, vsHashURLHits=vsHashURLHits, realIndex=realIndex, infInMcastPkts=infInMcastPkts, realStats=realStats, rsCount=rsCount, hcConnSuccess=hcConnSuccess, vsQosURLHits=vsQosURLHits, licenseRemainingDays=licenseRemainingDays, vsQosClientPortHits=vsQosClientPortHits, requestsToHttps=requestsToHttps, vsTable=vsTable, postRequests=postRequests, totalIPBytesOut=totalIPBytesOut, realPort=realPort, caTraps=caTraps, succLastMin=succLastMin, resumableSess=resumableSess, vrrpEntry=vrrpEntry, realAddressType=realAddressType, ctcpConnState=ctcpConnState, groupStats=groupStats, ctcpIndex=ctcpIndex, vsQosNetworkHits=vsQosNetworkHits, clusterVirIndex=clusterVirIndex, Float=Float, infIpv6AddressType=infIpv6AddressType, reqLastHour=reqLastHour, peakRcvdBytesPerSec=peakRcvdBytesPerSec, logHistoryTable=logHistoryTable, totalIPBytesIn=totalIPBytesIn, hcName=hcName, clientEstabConn=clientEstabConn, rsIpAddressType=rsIpAddressType, reqPeakSec5=reqPeakSec5, failLastSec5=failLastSec5, ctcpConnLocalAddrType=ctcpConnLocalAddrType, requestsOnRegex=requestsOnRegex, rsStatsEntry=rsStatsEntry, ctcpRetransSegs=ctcpRetransSegs, infTable=infTable, hcStatsTable=hcStatsTable, logNotificationsSent=logNotificationsSent, sslIndex=sslIndex, openSSLConns=openSSLConns, reqLastSec=reqLastSec, succLastHour=succLastHour, syslogTrap=syslogTrap, virtualAddr=virtualAddr, infOutOctets=infOutOctets, infIpv6Address=infIpv6Address, infOutUcastPkts=infOutUcastPkts, gpStatsEntry=gpStatsEntry, getRequests=getRequests, hitsReply=hitsReply, cputemp=cputemp, ctcpConnEntry=ctcpConnEntry, purgeRequests=purgeRequests, hcConnFail=hcConnFail, virtualAddressType=virtualAddressType, caShutdown=caShutdown, succPeakSec=succPeakSec, ipBytesIn=ipBytesIn, infDescr=infDescr, hcNumDowns=hcNumDowns, vhostNum=vhostNum, realStatus=realStatus, sdns=sdns, startTime=startTime, slbGeneral=slbGeneral, ctcpAttemptFails=ctcpAttemptFails, groupCount=groupCount, infTotalInOctets=infTotalInOctets, vsIpAddr=vsIpAddr, resumedSess=resumedSess, responsesOnRegex=responsesOnRegex, performance=performance, clusterVirPreempt=clusterVirPreempt, logHistTableMaxLength=logHistTableMaxLength, realServerID=realServerID, infOutBcastPkts=infOutBcastPkts, caSyslog=caSyslog, requestsReceived=requestsReceived, reqPeakHour=reqPeakHour, vrrp=vrrp, monitor=monitor, rcvdBytesPerSec=rcvdBytesPerSec, clusterVirPriority=clusterVirPriority, realServer=realServer, totalAcceptedConns=totalAcceptedConns, ctcpInSegs=ctcpInSegs, vsProtocol=vsProtocol, requestedConns=requestedConns, infIpv4AddressType=infIpv4AddressType, clusterId=clusterId, vipStatus=vipStatus, sslMIB=sslMIB, gpTable=gpTable, headRequests=headRequests, caSyslogTrap=caSyslogTrap, ctcpCurrEstab=ctcpCurrEstab, ctcpPassiveOpens=ctcpPassiveOpens, virtualAddress=virtualAddress, gpEntry=gpEntry, infIpv4Address=infIpv4Address, serverEstabConn=serverEstabConn, vsQosCookieHits=vsQosCookieHits, vsID=vsID, totalBytesSent=totalBytesSent, ipStatsTable=ipStatsTable, gpMetrics=gpMetrics, hcAddr=hcAddr, vsCount=vsCount, cacheSkip=cacheSkip, rsID=rsID, vsURLHits=vsURLHits, serialNumber=serialNumber, virtServerID=virtServerID, infInUnknownProtos=infInUnknownProtos, rsPort=rsPort, rsEntry=rsEntry, severity=severity, reqLastMin=reqLastMin, rsIpAddr=rsIpAddr, infNumber=infNumber, clusterVirAddress=clusterVirAddress, ctcpActiveOpens=ctcpActiveOpens, rsStatsTable=rsStatsTable, rsMaxConn=rsMaxConn, powerstate=powerstate, connectionsPerSec=connectionsPerSec, sysMemory=sysMemory, failLastSec=failLastSec, caStart=caStart, hcCause=hcCause, slbStats=slbStats, ifTraffic=ifTraffic, slbMIB=slbMIB, realAddress=realAddress, succLastDay=succLastDay, sysMemoryUtilization=sysMemoryUtilization, cacheMissWNewEntry=cacheMissWNewEntry, hitsReplyWNotModified=hitsReplyWNotModified, logBasic=logBasic, vhostName=vhostName, vsBackupHits=vsBackupHits, index=index, peakSentBytesPerSec=peakSentBytesPerSec, gpID=gpID, groupIndex=groupIndex, succPeakSec5=succPeakSec5, totalIPPktsOut=totalIPPktsOut, ctcpEstabResets=ctcpEstabResets, groupCurCfg=groupCurCfg, sslGeneral=sslGeneral, maxCluster=maxCluster, connsPerSec=connsPerSec, failLastDay=failLastDay, rsProtocol=rsProtocol, logHistoryEntry=logHistoryEntry, logNotificationsEnabled=logNotificationsEnabled, sentBytesPerSec=sentBytesPerSec, systemInfo=systemInfo, msgText=msgText, infAddress=infAddress, infInDiscards=infInDiscards, infInBcastPkts=infInBcastPkts, hcStatsEntry=hcStatsEntry, vsHostnameHits=vsHostnameHits, rsAvgRespTime=rsAvgRespTime, infOutErrors=infOutErrors, sslEntry=sslEntry, logMaxSeverity=logMaxSeverity, vsPerstntURLHits=vsPerstntURLHits, infTotalOutOctets=infTotalOutOctets, ipBytesOut=ipBytesOut, virtualIndex=virtualIndex, vsDefaultHits=vsDefaultHits, acceptedConns=acceptedConns, rsIndex=rsIndex, virtualPort=virtualPort, rsTotalHits=rsTotalHits, logHistory=logHistory, vsConnCnt=vsConnCnt, ipAddress=ipAddress, ctcpConnLocalPort=ctcpConnLocalPort, clusterVrrp=clusterVrrp, revProxyCache=revProxyCache, sysSwapUsed=sysSwapUsed, requestsPerSec=requestsPerSec, vsIpAddress=vsIpAddress, realID=realID, vsIndex=vsIndex, reqPeakMin=reqPeakMin, hostName=hostName, ctcpConnRemAddrType=ctcpConnRemAddrType, requestsToUrl=requestsToUrl, hitsReplyWPreFailed=hitsReplyWPreFailed, ctcpConnRemAddress=ctcpConnRemAddress, ctcpConnLocalAddr=ctcpConnLocalAddr, succLastSec=succLastSec, hcStats=hcStats, hcPort=hcPort, healthCheck=healthCheck, infInNUcastPkts=infInNUcastPkts, clusterVirState=clusterVirState, reqLastDay=reqLastDay, totalOpenSSLConns=totalOpenSSLConns, sysSwapCapacity=sysSwapCapacity, gpStatsTable=gpStatsTable, vsPerstntCookieHits=vsPerstntCookieHits)
mibBuilder.exportSymbols("CA-SNMP-MIB8", gpTotalHits=gpTotalHits, responsesToHttps=responsesToHttps, clusterNum=clusterNum, ctcpConnRemPort=ctcpConnRemPort, vsRCookieHits=vsRCookieHits, gpIndex=gpIndex, infInOctets=infInOctets, infOutBandwidth=infOutBandwidth, cacheBasicStats=cacheBasicStats, cacheMissWNoncacheReq=cacheMissWNoncacheReq, reqPeakSec=reqPeakSec, totalRequestedConns=totalRequestedConns, SyslogSeverity=SyslogSeverity, cacheStatus=cacheStatus, rsStatus=rsStatus, sslStats=sslStats, hcAddressType=hcAddressType, vsMaxConn=vsMaxConn, vsStatsTable=vsStatsTable, succPeakDay=succPeakDay, groupID=groupID, vsCacheHits=vsCacheHits, clusterVirAddr=clusterVirAddr, rsCntOfReq=rsCntOfReq, ipPktsOut=ipPktsOut, clickTcp=clickTcp, cacheHitRatio=cacheHitRatio, compression=compression, vsQosBodyHits=vsQosBodyHits, infIndex=infIndex, vsHeaderHits=vsHeaderHits, ipAddrType=ipAddrType, hcConnAttempt=hcConnAttempt, ctcpOutSegs=ctcpOutSegs, infOutNUcastPkts=infOutNUcastPkts, vipStats=vipStats, arrayNetworks=arrayNetworks, hcAddress=hcAddress, totalReq=totalReq, failLastHour=failLastHour, sslTable=sslTable, realAddr=realAddr, activeTransac=activeTransac, succPeakMin=succPeakMin, rsIpAddress=rsIpAddress, rsTable=rsTable, hcNumUps=hcNumUps, ctcpConnRemAddr=ctcpConnRemAddr, succPeakHour=succPeakHour, vsStatsEntry=vsStatsEntry, hcStatus=hcStatus, cpuUtilization=cpuUtilization, vsIpAddressType=vsIpAddressType, infOutMcastPkts=infOutMcastPkts, currentTime=currentTime)
