...
ModuleIdentity, Gauge32, Bits, iso, Counter32, enterprises, Opaque, NotificationType, MibIdentifier, TimeTicks, Unsigned32, Counter64, Integer32, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Gauge32", "Bits", "iso", "Counter32", "enterprises", "Opaque", "NotificationType", "MibIdentifier", "TimeTicks", "Unsigned32", "Counter64", "Integer32", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", ctcpIndex=ctcpIndex, ipBytesOut=ipBytesOut, powerstate=powerstate, hcNumDowns=hcNumDowns, performance=performance, succLastMin=succLastMin, vsIpAddr=vsIpAddr, ctcpActiveOpens=ctcpActiveOpens, infInErrors=infInErrors, reqPeakHour=reqPeakHour, rsIpAddr=rsIpAddr, rsStatus=rsStatus, responsesOnRegex=responsesOnRegex, sslEntry=sslEntry, clusterVrrp=clusterVrrp, totalReq=totalReq, hcStats=hcStats, realAddr=realAddr, reqPeakMin=reqPeakMin, ctcpConnRemAddrType=ctcpConnRemAddrType, serverEstabConn=serverEstabConn, clusterVirAuthType=clusterVirAuthType, groupStats=groupStats, hcRSCount=hcRSCount, infOutBandwidth=infOutBandwidth, hcPort=hcPort, hitsReplyWPreFailed=hitsReplyWPreFailed, sslTable=sslTable, logHistTableMaxLength=logHistTableMaxLength, realStatus=realStatus, virtualAddress=virtualAddress, infNumber=infNumber, fanspeed=fanspeed, infInMcastPkts=infInMcastPkts, infInBcastPkts=infInBcastPkts, totalAcceptedConns=totalAcceptedConns, vsIpAddress=vsIpAddress, groupID=groupID, vrrpTable=vrrpTable, sslGeneral=sslGeneral, gpMetrics=gpMetrics, ipIndex=ipIndex, systemInfo=systemInfo, infIpv4Address=infIpv4Address, realAddress=realAddress, vipStats=vipStats, gpStatsTable=gpStatsTable, infIpv4AddressType=infIpv4AddressType, ctcpInSegs=ctcpInSegs, realServer=realServer, gpID=gpID, infOutOctets=infOutOctets, succPeakDay=succPeakDay, vsBackupHits=vsBackupHits, infTotalInOctets=infTotalInOctets, vrrp=vrrp, hcAddressType=hcAddressType, infInUcastPkts=infInUcastPkts, connsPerSec=connsPerSec, vsPerstntCookieHits=vsPerstntCookieHits, logHistory=logHistory, virtualPort=virtualPort, rsIndex=rsIndex, reqPeakSec5=reqPeakSec5, serialNumber=serialNumber, clusterVirIfname=clusterVirIfname, hostName=hostName, rsID=rsID, vsRedirectHits=vsRedirectHits, rsStatsTable=rsStatsTable, ipStatsTable=ipStatsTable, Float=Float, severity=severity, hcAddr=hcAddr, gpIndex=gpIndex, hcAddress=hcAddress, ctcpCurrEstab=ctcpCurrEstab, caTraps=caTraps, headRequests=headRequests, realStats=realStats, hcStatsEntry=hcStatsEntry, infInNUcastPkts=infInNUcastPkts, realID=realID, totalIPPktsOut=totalIPPktsOut, ctcpConnEntry=ctcpConnEntry, ipAddr=ipAddr, requestsOnRegex=requestsOnRegex, vsTable=vsTable, postRequests=postRequests, logMaxSeverity=logMaxSeverity, clusterVirAddress=clusterVirAddress, rsConnCnt=rsConnCnt, clusterVirAuthPasswd=clusterVirAuthPasswd, succLastDay=succLastDay, succLastSec5=succLastSec5, failLastDay=failLastDay, clusterVirAddressType=clusterVirAddressType, totalRequestedConns=totalRequestedConns, logHistoryEntry=logHistoryEntry, infIpv6AddressType=infIpv6AddressType, infAddress=infAddress, clusterNum=clusterNum, connectionsPerSec=connectionsPerSec, reqLastHour=reqLastHour, requestedConns=requestedConns, sysSwapCapacity=sysSwapCapacity, clusterId=clusterId, sysMemory=sysMemory, vsQosClientPortHits=vsQosClientPortHits, sentBytesPerSec=sentBytesPerSec, reqLastDay=reqLastDay, PYSNMP_MODULE_ID=arrayNetworks, clusterVirInterval=clusterVirInterval, infInDiscards=infInDiscards, cacheMissWNewEntry=cacheMissWNewEntry, infInUnknownProtos=infInUnknownProtos, requestsToUrl=requestsToUrl, slbStats=slbStats, purgeRequests=purgeRequests, rsMaxConn=rsMaxConn, totalOpenSSLConns=totalOpenSSLConns, ctcpConnRemAddress=ctcpConnRemAddress, caShutdown=caShutdown, vsPerstntURLHits=vsPerstntURLHits, realServerID=realServerID, failLastHour=failLastHour, ipPktsIn=ipPktsIn, infOutMcastPkts=infOutMcastPkts, vsRCookieHits=vsRCookieHits, logBasic=logBasic, rsAvgRespTime=rsAvgRespTime, monitor=monitor, ctcpConnState=ctcpConnState, sysSwapUsed=sysSwapUsed, gpEntry=gpEntry, ipBytesIn=ipBytesIn, vsQosURLHits=vsQosURLHits, groupCount=groupCount, groupCurCfg=groupCurCfg, hitsReply=hitsReply, ctcpConnLocalAddr=ctcpConnLocalAddr, totalIPBytesOut=totalIPBytesOut, ipAddress=ipAddress, reqLastSec5=reqLastSec5, startTime=startTime, slbGeneral=slbGeneral, infOperStatus=infOperStatus, infInOctets=infInOctets, virtualStats=virtualStats, vsHashURLHits=vsHashURLHits, infOutBcastPkts=infOutBcastPkts, vsIndex=vsIndex, rsTotalHits=rsTotalHits, hcNumUps=hcNumUps, infTotalOutOctets=infTotalOutOctets, hcConnAttempt=hcConnAttempt, vsQosNetworkHits=vsQosNetworkHits, ctcpRetransSegs=ctcpRetransSegs, succLastSec=succLastSec, failLastSec5=failLastSec5, msgText=msgText, vsURLHits=vsURLHits, revProxyCache=revProxyCache, ctcpOutSegs=ctcpOutSegs, realAddressType=realAddressType, gpTable=gpTable, vsICookieHits=vsICookieHits, hitRevalidate=hitRevalidate, requestsToHttps=requestsToHttps, cacheBasicStats=cacheBasicStats, peakSentBytesPerSec=peakSentBytesPerSec, currentTime=currentTime, vsHeaderHits=vsHeaderHits, clusterVirAddr=clusterVirAddr, virtualIndex=virtualIndex, ctcpConnRemPort=ctcpConnRemPort, reqLastMin=reqLastMin, vhostNum=vhostNum, vsConnCnt=vsConnCnt, virtServerID=virtServerID, infEntry=infEntry, realPort=realPort, vsStatsEntry=vsStatsEntry, clusterVirPriority=clusterVirPriority, sysMemoryUtilization=sysMemoryUtilization, slbMIB=slbMIB, ctcpConnTable=ctcpConnTable, rsIpAddressType=rsIpAddressType, vsHostnameHits=vsHostnameHits, cacheStatus=cacheStatus, reqPeakSec=reqPeakSec, ipStatsEntry=ipStatsEntry, totalIPPktsIn=totalIPPktsIn, infOutErrors=infOutErrors, ctcpConnRemAddr=ctcpConnRemAddr, clickTcp=clickTcp, ctcpPassiveOpens=ctcpPassiveOpens, hcConnFail=hcConnFail, ctcpConnLocalAddrType=ctcpConnLocalAddrType, requestsReceived=requestsReceived, resumedSess=resumedSess, succPeakSec5=succPeakSec5, clusterVirIndex=clusterVirIndex, maxCluster=maxCluster, succPeakMin=succPeakMin, caSyslog=caSyslog, totalIPBytesIn=totalIPBytesIn, ctcpOutRsts=ctcpOutRsts, responsesToHttps=responsesToHttps, arrayNetworks=arrayNetworks, groupIndex=groupIndex, peakRcvdBytesPerSec=peakRcvdBytesPerSec, hcCause=hcCause, cacheHitRatio=cacheHitRatio, realIndex=realIndex, vsEntry=vsEntry, clientEstabConn=clientEstabConn, vsCount=vsCount, ipPktsOut=ipPktsOut, healthCheck=healthCheck, cacheMissWRespNo=cacheMissWRespNo, SyslogSeverity=SyslogSeverity, vsStatsTable=vsStatsTable, vsMaxConn=vsMaxConn, vsStaticHits=vsStaticHits, cpuUtilization=cpuUtilization, rsStatsEntry=rsStatsEntry, failLastMin=failLastMin, succLastHour=succLastHour, ctcpEstabResets=ctcpEstabResets, openSSLConns=openSSLConns, sdns=sdns, vsQosCookieHits=vsQosCookieHits, reqPeakDay=reqPeakDay, hcName=hcName, index=index, requestsPerSec=requestsPerSec, vsProtocol=vsProtocol, infTable=infTable, ctcpAttemptFails=ctcpAttemptFails, cacheSkip=cacheSkip, hcIndex=hcIndex, hitsReplyWNotModified=hitsReplyWNotModified, rsEntry=rsEntry, acceptedConns=acceptedConns, clusterVirPreempt=clusterVirPreempt, rsIpAddress=rsIpAddress, hcConnSuccess=hcConnSuccess, totalBytesRcvd=totalBytesRcvd, caSyslogTrap=caSyslogTrap, vsRegexHits=vsRegexHits, logNotificationsEnabled=logNotificationsEnabled, rsCntOfReq=rsCntOfReq, ctcpConnLocalPort=ctcpConnLocalPort)
mibBuilder.exportSymbols("CA-SNMP-MIB8", ipAddrType=ipAddrType, ctcpInErrs=ctcpInErrs, virtualAddressType=virtualAddressType, vrrpEntry=vrrpEntry, rsProtocol=rsProtocol, vsPort=vsPort, totalSucc=totalSucc, rsCount=rsCount, reqLastSec=reqLastSec, logNotificationsSent=logNotificationsSent, totalBytesSent=totalBytesSent, infInBandwidth=infInBandwidth, hcStatus=hcStatus, sslIndex=sslIndex, infOutUcastPkts=infOutUcastPkts, missSess=missSess, ctcpConnLocalAddress=ctcpConnLocalAddress, cputemp=cputemp, totalFail=totalFail, rcvdBytesPerSec=rcvdBytesPerSec, infIndex=infIndex, vsQosBodyHits=vsQosBodyHits, infOutNUcastPkts=infOutNUcastPkts, logHistoryTable=logHistoryTable, cacheMissWNoncacheReq=cacheMissWNoncacheReq, vsDefaultHits=vsDefaultHits, hcStatsTable=hcStatsTable, infDescr=infDescr, vsCacheHits=vsCacheHits, rsTable=rsTable, succPeakHour=succPeakHour, vipStatus=vipStatus, ifTraffic=ifTraffic, resumableSess=resumableSess, rsPort=rsPort, memory=memory, syslogTrap=syslogTrap, virtualServer=virtualServer, succPeakSec=succPeakSec, licenseRemainingDays=licenseRemainingDays, sslMIB=sslMIB, sslStats=sslStats, gpStatsEntry=gpStatsEntry, virtualAddr=virtualAddr, compression=compression, failLastSec=failLastSec, gpTotalHits=gpTotalHits, infIpv6Address=infIpv6Address, vsIpAddressType=vsIpAddressType, caStart=caStart, getRequests=getRequests, clusterVirState=clusterVirState, vhostName=vhostName, activeTransac=activeTransac, vsID=vsID)
