...
ModuleIdentity, Opaque, IpAddress, iso, Counter32, NotificationType, TimeTicks, ObjectIdentity, Counter64, MibIdentifier, Gauge32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, enterprises, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Opaque", "IpAddress", "iso", "Counter32", "NotificationType", "TimeTicks", "ObjectIdentity", "Counter64", "MibIdentifier", "Gauge32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "enterprises", "Unsigned32")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", startTime=startTime, virtualIndex=virtualIndex, SyslogSeverity=SyslogSeverity, gpTotalHits=gpTotalHits, hitRevalidate=hitRevalidate, realServer=realServer, hcConnSuccess=hcConnSuccess, clusterVirInterval=clusterVirInterval, cpuUtilization=cpuUtilization, groupCount=groupCount, vsHeaderHits=vsHeaderHits, clusterVirAddr=clusterVirAddr, realAddr=realAddr, hcConnAttempt=hcConnAttempt, reqLastDay=reqLastDay, succPeakMin=succPeakMin, vsConnCnt=vsConnCnt, infTotalOutOctets=infTotalOutOctets, vsQosClientPortHits=vsQosClientPortHits, ctcpConnLocalAddr=ctcpConnLocalAddr, infDescr=infDescr, clusterId=clusterId, vsPerstntURLHits=vsPerstntURLHits, rsStatus=rsStatus, realPort=realPort, vsIpAddr=vsIpAddr, virtualAddressType=virtualAddressType, cacheMissWRespNo=cacheMissWRespNo, monitor=monitor, vsQosCookieHits=vsQosCookieHits, infTable=infTable, requestsOnRegex=requestsOnRegex, reqPeakSec=reqPeakSec, vipStatus=vipStatus, resumableSess=resumableSess, infNumber=infNumber, ctcpRetransSegs=ctcpRetransSegs, virtualAddr=virtualAddr, requestsToHttps=requestsToHttps, cacheMissWNewEntry=cacheMissWNewEntry, groupIndex=groupIndex, infIpv6AddressType=infIpv6AddressType, peakSentBytesPerSec=peakSentBytesPerSec, clusterVrrp=clusterVrrp, currentTime=currentTime, sslIndex=sslIndex, reqLastMin=reqLastMin, peakRcvdBytesPerSec=peakRcvdBytesPerSec, ctcpConnEntry=ctcpConnEntry, ctcpConnTable=ctcpConnTable, vsQosNetworkHits=vsQosNetworkHits, totalOpenSSLConns=totalOpenSSLConns, headRequests=headRequests, ipPktsOut=ipPktsOut, activeTransac=activeTransac, infOutNUcastPkts=infOutNUcastPkts, openSSLConns=openSSLConns, ctcpInErrs=ctcpInErrs, cputemp=cputemp, hcPort=hcPort, infOperStatus=infOperStatus, sdns=sdns, totalIPPktsIn=totalIPPktsIn, slbGeneral=slbGeneral, infOutUcastPkts=infOutUcastPkts, infInUcastPkts=infInUcastPkts, succLastMin=succLastMin, realIndex=realIndex, memory=memory, hcAddressType=hcAddressType, ctcpConnLocalAddress=ctcpConnLocalAddress, vsProtocol=vsProtocol, vsRCookieHits=vsRCookieHits, infOutBandwidth=infOutBandwidth, hcCause=hcCause, clusterVirAddressType=clusterVirAddressType, logNotificationsSent=logNotificationsSent, caStart=caStart, succLastDay=succLastDay, vsStatsEntry=vsStatsEntry, hcAddr=hcAddr, sslGeneral=sslGeneral, infIndex=infIndex, ipStatsEntry=ipStatsEntry, succPeakHour=succPeakHour, caSyslogTrap=caSyslogTrap, arrayNetworks=arrayNetworks, rsID=rsID, logHistoryEntry=logHistoryEntry, vsHostnameHits=vsHostnameHits, virtualServer=virtualServer, caShutdown=caShutdown, gpID=gpID, totalSucc=totalSucc, ctcpIndex=ctcpIndex, logBasic=logBasic, resumedSess=resumedSess, ctcpConnRemAddrType=ctcpConnRemAddrType, ipAddrType=ipAddrType, vrrpEntry=vrrpEntry, serialNumber=serialNumber, vrrp=vrrp, ctcpActiveOpens=ctcpActiveOpens, gpStatsEntry=gpStatsEntry, acceptedConns=acceptedConns, groupCurCfg=groupCurCfg, vsRedirectHits=vsRedirectHits, hcNumDowns=hcNumDowns, sslEntry=sslEntry, realID=realID, hcStatus=hcStatus, failLastMin=failLastMin, hcRSCount=hcRSCount, PYSNMP_MODULE_ID=arrayNetworks, ctcpAttemptFails=ctcpAttemptFails, index=index, failLastDay=failLastDay, rsIndex=rsIndex, requestsReceived=requestsReceived, succLastHour=succLastHour, clusterVirAuthPasswd=clusterVirAuthPasswd, groupStats=groupStats, sysMemory=sysMemory, succPeakSec=succPeakSec, ctcpConnRemPort=ctcpConnRemPort, rsIpAddressType=rsIpAddressType, cacheSkip=cacheSkip, clickTcp=clickTcp, ctcpPassiveOpens=ctcpPassiveOpens, ctcpCurrEstab=ctcpCurrEstab, failLastSec=failLastSec, rsAvgRespTime=rsAvgRespTime, vsHashURLHits=vsHashURLHits, ipPktsIn=ipPktsIn, clusterVirAddress=clusterVirAddress, ipIndex=ipIndex, infOutMcastPkts=infOutMcastPkts, ipStatsTable=ipStatsTable, vsQosURLHits=vsQosURLHits, infInNUcastPkts=infInNUcastPkts, gpMetrics=gpMetrics, infInMcastPkts=infInMcastPkts, serverEstabConn=serverEstabConn, clientEstabConn=clientEstabConn, vsURLHits=vsURLHits, rsEntry=rsEntry, Float=Float, getRequests=getRequests, infInBandwidth=infInBandwidth, hostName=hostName, totalBytesRcvd=totalBytesRcvd, clusterVirIfname=clusterVirIfname, caSyslog=caSyslog, infIpv4Address=infIpv4Address, requestsToUrl=requestsToUrl, ctcpOutRsts=ctcpOutRsts, ctcpConnRemAddr=ctcpConnRemAddr, vsCount=vsCount, vsIpAddress=vsIpAddress, rsIpAddr=rsIpAddr, reqLastSec5=reqLastSec5, infAddress=infAddress, hcNumUps=hcNumUps, reqPeakSec5=reqPeakSec5, vhostNum=vhostNum, responsesToHttps=responsesToHttps, ctcpEstabResets=ctcpEstabResets, clusterVirAuthType=clusterVirAuthType, connsPerSec=connsPerSec, virtualAddress=virtualAddress, licenseRemainingDays=licenseRemainingDays, sslStats=sslStats, failLastSec5=failLastSec5, hcConnFail=hcConnFail, hcIndex=hcIndex, sysSwapCapacity=sysSwapCapacity, hcAddress=hcAddress, hitsReplyWPreFailed=hitsReplyWPreFailed, rcvdBytesPerSec=rcvdBytesPerSec, maxCluster=maxCluster, succLastSec=succLastSec, logHistoryTable=logHistoryTable, severity=severity, infTotalInOctets=infTotalInOctets, failLastHour=failLastHour, rsProtocol=rsProtocol, requestedConns=requestedConns, reqPeakMin=reqPeakMin, ctcpConnLocalAddrType=ctcpConnLocalAddrType, rsTotalHits=rsTotalHits, clusterVirState=clusterVirState, logHistory=logHistory, infInErrors=infInErrors, clusterVirIndex=clusterVirIndex, hcStatsTable=hcStatsTable, vsCacheHits=vsCacheHits, succLastSec5=succLastSec5, powerstate=powerstate, rsStatsTable=rsStatsTable, vsIpAddressType=vsIpAddressType, virtServerID=virtServerID, totalRequestedConns=totalRequestedConns, vsMaxConn=vsMaxConn, slbMIB=slbMIB, vsQosBodyHits=vsQosBodyHits, infInUnknownProtos=infInUnknownProtos, infInDiscards=infInDiscards, postRequests=postRequests, logMaxSeverity=logMaxSeverity, realStatus=realStatus, infIpv4AddressType=infIpv4AddressType, hitsReplyWNotModified=hitsReplyWNotModified, reqPeakDay=reqPeakDay, realStats=realStats, reqLastSec=reqLastSec, ipAddr=ipAddr, logNotificationsEnabled=logNotificationsEnabled, vsPerstntCookieHits=vsPerstntCookieHits, sslMIB=sslMIB, infOutBcastPkts=infOutBcastPkts, ctcpConnState=ctcpConnState, reqLastHour=reqLastHour, totalAcceptedConns=totalAcceptedConns, sslTable=sslTable, realAddressType=realAddressType, rsTable=rsTable, realAddress=realAddress, vsIndex=vsIndex, infIpv6Address=infIpv6Address, gpTable=gpTable, rsMaxConn=rsMaxConn, hitsReply=hitsReply, caTraps=caTraps, totalReq=totalReq, gpIndex=gpIndex, ctcpConnRemAddress=ctcpConnRemAddress, infOutOctets=infOutOctets, rsConnCnt=rsConnCnt, hcStatsEntry=hcStatsEntry, vsBackupHits=vsBackupHits, cacheHitRatio=cacheHitRatio, requestsPerSec=requestsPerSec, responsesOnRegex=responsesOnRegex, virtualPort=virtualPort, gpEntry=gpEntry, clusterVirPriority=clusterVirPriority, logHistTableMaxLength=logHistTableMaxLength, cacheStatus=cacheStatus, vhostName=vhostName, vsDefaultHits=vsDefaultHits, sysMemoryUtilization=sysMemoryUtilization, vsICookieHits=vsICookieHits, ctcpOutSegs=ctcpOutSegs, succPeakSec5=succPeakSec5)
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsTable=vsTable, sysSwapUsed=sysSwapUsed, compression=compression, slbStats=slbStats, groupID=groupID, cacheBasicStats=cacheBasicStats, sentBytesPerSec=sentBytesPerSec, ctcpInSegs=ctcpInSegs, reqPeakHour=reqPeakHour, cacheMissWNoncacheReq=cacheMissWNoncacheReq, vsPort=vsPort, ipBytesIn=ipBytesIn, infInOctets=infInOctets, clusterVirPreempt=clusterVirPreempt, totalIPPktsOut=totalIPPktsOut, succPeakDay=succPeakDay, vsEntry=vsEntry, infInBcastPkts=infInBcastPkts, realServerID=realServerID, virtualStats=virtualStats, rsIpAddress=rsIpAddress, infEntry=infEntry, systemInfo=systemInfo, revProxyCache=revProxyCache, fanspeed=fanspeed, hcStats=hcStats, totalFail=totalFail, totalBytesSent=totalBytesSent, ctcpConnLocalPort=ctcpConnLocalPort, totalIPBytesIn=totalIPBytesIn, connectionsPerSec=connectionsPerSec, infOutErrors=infOutErrors, rsPort=rsPort, performance=performance, syslogTrap=syslogTrap, vsStatsTable=vsStatsTable, ipBytesOut=ipBytesOut, rsCount=rsCount, vsStaticHits=vsStaticHits, vsRegexHits=vsRegexHits, missSess=missSess, vrrpTable=vrrpTable, healthCheck=healthCheck, rsCntOfReq=rsCntOfReq, clusterNum=clusterNum, purgeRequests=purgeRequests, ifTraffic=ifTraffic, rsStatsEntry=rsStatsEntry, gpStatsTable=gpStatsTable, totalIPBytesOut=totalIPBytesOut, vsID=vsID, hcName=hcName, ipAddress=ipAddress, vipStats=vipStats, msgText=msgText)
