...
Bits, Integer32, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, MibIdentifier, Counter32, NotificationType, Gauge32, Opaque, IpAddress, enterprises, Counter64, TimeTicks, Unsigned32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Integer32", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "MibIdentifier", "Counter32", "NotificationType", "Gauge32", "Opaque", "IpAddress", "enterprises", "Counter64", "TimeTicks", "Unsigned32", "ModuleIdentity")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", slbGeneral=slbGeneral, rsIpAddress=rsIpAddress, severity=severity, succPeakDay=succPeakDay, succLastMin=succLastMin, sslEntry=sslEntry, rsMaxConn=rsMaxConn, infAddress=infAddress, hitsReplyWPreFailed=hitsReplyWPreFailed, gpStatsEntry=gpStatsEntry, succLastSec=succLastSec, infInUnknownProtos=infInUnknownProtos, vsQosNetworkHits=vsQosNetworkHits, vsEntry=vsEntry, realAddressType=realAddressType, syslogTrap=syslogTrap, failLastDay=failLastDay, ipAddress=ipAddress, totalReq=totalReq, virtualServer=virtualServer, sslStats=sslStats, vsQosURLHits=vsQosURLHits, realStats=realStats, hcConnFail=hcConnFail, reqPeakMin=reqPeakMin, serialNumber=serialNumber, succPeakSec5=succPeakSec5, logHistoryEntry=logHistoryEntry, responsesOnRegex=responsesOnRegex, sysMemory=sysMemory, ctcpConnLocalAddr=ctcpConnLocalAddr, vsIpAddr=vsIpAddr, virtualAddr=virtualAddr, realServerID=realServerID, vsPort=vsPort, hcAddress=hcAddress, hcStatsTable=hcStatsTable, cpuUtilization=cpuUtilization, vipStats=vipStats, gpTotalHits=gpTotalHits, reqLastHour=reqLastHour, msgText=msgText, slbMIB=slbMIB, PYSNMP_MODULE_ID=arrayNetworks, rsEntry=rsEntry, rsStatsEntry=rsStatsEntry, hcConnAttempt=hcConnAttempt, cacheStatus=cacheStatus, gpMetrics=gpMetrics, rsIndex=rsIndex, ctcpOutRsts=ctcpOutRsts, SyslogSeverity=SyslogSeverity, hcStats=hcStats, reqPeakDay=reqPeakDay, clusterVirAuthType=clusterVirAuthType, virtualAddressType=virtualAddressType, totalBytesRcvd=totalBytesRcvd, serverEstabConn=serverEstabConn, infIpv4AddressType=infIpv4AddressType, caStart=caStart, hcCause=hcCause, memory=memory, rsStatsTable=rsStatsTable, peakRcvdBytesPerSec=peakRcvdBytesPerSec, logHistoryTable=logHistoryTable, succPeakHour=succPeakHour, requestsToUrl=requestsToUrl, reqPeakSec=reqPeakSec, infEntry=infEntry, ctcpActiveOpens=ctcpActiveOpens, vsIpAddress=vsIpAddress, purgeRequests=purgeRequests, hcNumDowns=hcNumDowns, rsProtocol=rsProtocol, powerstate=powerstate, succLastDay=succLastDay, ctcpCurrEstab=ctcpCurrEstab, Float=Float, sslMIB=sslMIB, infInOctets=infInOctets, infOutBandwidth=infOutBandwidth, ctcpConnState=ctcpConnState, vsHashURLHits=vsHashURLHits, infInUcastPkts=infInUcastPkts, infTotalInOctets=infTotalInOctets, logMaxSeverity=logMaxSeverity, realAddr=realAddr, ipAddr=ipAddr, infInNUcastPkts=infInNUcastPkts, ctcpIndex=ctcpIndex, hcStatsEntry=hcStatsEntry, ipBytesOut=ipBytesOut, hcPort=hcPort, infIpv6Address=infIpv6Address, arrayNetworks=arrayNetworks, rsConnCnt=rsConnCnt, vsURLHits=vsURLHits, realPort=realPort, hcConnSuccess=hcConnSuccess, vsQosBodyHits=vsQosBodyHits, responsesToHttps=responsesToHttps, hostName=hostName, failLastSec=failLastSec, slbStats=slbStats, vsHeaderHits=vsHeaderHits, hcRSCount=hcRSCount, totalFail=totalFail, logNotificationsSent=logNotificationsSent, hitsReply=hitsReply, rsTable=rsTable, infInBcastPkts=infInBcastPkts, infOutMcastPkts=infOutMcastPkts, infInDiscards=infInDiscards, gpTable=gpTable, vsConnCnt=vsConnCnt, acceptedConns=acceptedConns, realID=realID, infOutBcastPkts=infOutBcastPkts, failLastSec5=failLastSec5, virtualIndex=virtualIndex, totalIPPktsIn=totalIPPktsIn, ctcpConnRemAddress=ctcpConnRemAddress, ipAddrType=ipAddrType, rsAvgRespTime=rsAvgRespTime, vsStatsTable=vsStatsTable, cacheMissWNoncacheReq=cacheMissWNoncacheReq, virtualAddress=virtualAddress, infOutOctets=infOutOctets, sslIndex=sslIndex, cacheBasicStats=cacheBasicStats, totalBytesSent=totalBytesSent, logHistTableMaxLength=logHistTableMaxLength, connsPerSec=connsPerSec, rcvdBytesPerSec=rcvdBytesPerSec, clusterVirInterval=clusterVirInterval, sslTable=sslTable, rsTotalHits=rsTotalHits, infOutUcastPkts=infOutUcastPkts, caSyslog=caSyslog, ipStatsTable=ipStatsTable, activeTransac=activeTransac, hitRevalidate=hitRevalidate, reqLastDay=reqLastDay, vsMaxConn=vsMaxConn, failLastMin=failLastMin, performance=performance, ctcpRetransSegs=ctcpRetransSegs, ipPktsIn=ipPktsIn, caSyslogTrap=caSyslogTrap, requestedConns=requestedConns, cputemp=cputemp, clientEstabConn=clientEstabConn, infOperStatus=infOperStatus, sdns=sdns, postRequests=postRequests, clusterVirState=clusterVirState, headRequests=headRequests, vrrpTable=vrrpTable, totalIPBytesIn=totalIPBytesIn, sslGeneral=sslGeneral, clusterVirAddr=clusterVirAddr, realServer=realServer, currentTime=currentTime, gpID=gpID, ctcpEstabResets=ctcpEstabResets, startTime=startTime, cacheMissWRespNo=cacheMissWRespNo, sentBytesPerSec=sentBytesPerSec, hitsReplyWNotModified=hitsReplyWNotModified, gpStatsTable=gpStatsTable, vsIndex=vsIndex, clusterId=clusterId, totalIPPktsOut=totalIPPktsOut, vsRCookieHits=vsRCookieHits, hcIndex=hcIndex, rsIpAddr=rsIpAddr, ctcpConnTable=ctcpConnTable, infTable=infTable, maxCluster=maxCluster, rsPort=rsPort, caShutdown=caShutdown, resumableSess=resumableSess, cacheHitRatio=cacheHitRatio, groupIndex=groupIndex, reqLastMin=reqLastMin, groupCount=groupCount, failLastHour=failLastHour, vsPerstntURLHits=vsPerstntURLHits, rsID=rsID, succLastSec5=succLastSec5, gpIndex=gpIndex, sysMemoryUtilization=sysMemoryUtilization, vhostNum=vhostNum, ifTraffic=ifTraffic, vsRedirectHits=vsRedirectHits, succPeakMin=succPeakMin, clusterVrrp=clusterVrrp, requestsPerSec=requestsPerSec, ctcpConnRemAddr=ctcpConnRemAddr, infIndex=infIndex, vsStatsEntry=vsStatsEntry, reqPeakHour=reqPeakHour, sysSwapCapacity=sysSwapCapacity, requestsReceived=requestsReceived, openSSLConns=openSSLConns, systemInfo=systemInfo, totalRequestedConns=totalRequestedConns, resumedSess=resumedSess, connectionsPerSec=connectionsPerSec, vsProtocol=vsProtocol, realIndex=realIndex, ctcpConnRemAddrType=ctcpConnRemAddrType, infDescr=infDescr, ctcpOutSegs=ctcpOutSegs, sysSwapUsed=sysSwapUsed, reqPeakSec5=reqPeakSec5, clickTcp=clickTcp, vsDefaultHits=vsDefaultHits, vsIpAddressType=vsIpAddressType, infInMcastPkts=infInMcastPkts, hcStatus=hcStatus, index=index, infInErrors=infInErrors, clusterVirIndex=clusterVirIndex, logNotificationsEnabled=logNotificationsEnabled, ipBytesIn=ipBytesIn, healthCheck=healthCheck, vsHostnameHits=vsHostnameHits, realStatus=realStatus, groupStats=groupStats, clusterVirAddress=clusterVirAddress, peakSentBytesPerSec=peakSentBytesPerSec, ctcpConnLocalAddrType=ctcpConnLocalAddrType, requestsToHttps=requestsToHttps, ipPktsOut=ipPktsOut, infInBandwidth=infInBandwidth, ipStatsEntry=ipStatsEntry, ctcpPassiveOpens=ctcpPassiveOpens, infOutErrors=infOutErrors, clusterVirAuthPasswd=clusterVirAuthPasswd, vsQosCookieHits=vsQosCookieHits, vsTable=vsTable, ctcpConnLocalPort=ctcpConnLocalPort, hcNumUps=hcNumUps, infIpv4Address=infIpv4Address, totalOpenSSLConns=totalOpenSSLConns, clusterNum=clusterNum, clusterVirAddressType=clusterVirAddressType, infOutNUcastPkts=infOutNUcastPkts, infNumber=infNumber, logHistory=logHistory, ctcpConnLocalAddress=ctcpConnLocalAddress, succLastHour=succLastHour, vsStaticHits=vsStaticHits)
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsICookieHits=vsICookieHits, totalSucc=totalSucc, vsID=vsID, infTotalOutOctets=infTotalOutOctets, rsStatus=rsStatus, getRequests=getRequests, vipStatus=vipStatus, totalIPBytesOut=totalIPBytesOut, rsCntOfReq=rsCntOfReq, caTraps=caTraps, licenseRemainingDays=licenseRemainingDays, monitor=monitor, vsRegexHits=vsRegexHits, succPeakSec=succPeakSec, fanspeed=fanspeed, vsCacheHits=vsCacheHits, ipIndex=ipIndex, vsBackupHits=vsBackupHits, totalAcceptedConns=totalAcceptedConns, clusterVirPreempt=clusterVirPreempt, vsPerstntCookieHits=vsPerstntCookieHits, hcAddressType=hcAddressType, ctcpInSegs=ctcpInSegs, revProxyCache=revProxyCache, vrrpEntry=vrrpEntry, rsCount=rsCount, cacheSkip=cacheSkip, rsIpAddressType=rsIpAddressType, vrrp=vrrp, ctcpAttemptFails=ctcpAttemptFails, ctcpConnRemPort=ctcpConnRemPort, groupID=groupID, gpEntry=gpEntry, reqLastSec=reqLastSec, missSess=missSess, cacheMissWNewEntry=cacheMissWNewEntry, vsCount=vsCount, vsQosClientPortHits=vsQosClientPortHits, virtualStats=virtualStats, groupCurCfg=groupCurCfg, infIpv6AddressType=infIpv6AddressType, reqLastSec5=reqLastSec5, virtServerID=virtServerID, ctcpInErrs=ctcpInErrs, hcName=hcName, clusterVirPriority=clusterVirPriority, virtualPort=virtualPort, vhostName=vhostName, ctcpConnEntry=ctcpConnEntry, logBasic=logBasic, clusterVirIfname=clusterVirIfname, requestsOnRegex=requestsOnRegex, realAddress=realAddress, hcAddr=hcAddr, compression=compression)
