...
enterprises, Counter64, TimeTicks, Opaque, Gauge32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Unsigned32, Counter32, Integer32, ModuleIdentity, Bits, MibIdentifier, ObjectIdentity, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Counter64", "TimeTicks", "Opaque", "Gauge32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Unsigned32", "Counter32", "Integer32", "ModuleIdentity", "Bits", "MibIdentifier", "ObjectIdentity", "IpAddress")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsTable=vsTable, vsCacheHits=vsCacheHits, caStart=caStart, hcStatsEntry=hcStatsEntry, reqPeakHour=reqPeakHour, totalAcceptedConns=totalAcceptedConns, headRequests=headRequests, vsID=vsID, hcStatsTable=hcStatsTable, realID=realID, vhostName=vhostName, ctcpRetransSegs=ctcpRetransSegs, vsHashURLHits=vsHashURLHits, compression=compression, hcConnFail=hcConnFail, infOutOctets=infOutOctets, virtualServer=virtualServer, hcStats=hcStats, ctcpConnRemAddrType=ctcpConnRemAddrType, ctcpOutRsts=ctcpOutRsts, rsMaxConn=rsMaxConn, clusterNum=clusterNum, groupStats=groupStats, ctcpConnLocalAddr=ctcpConnLocalAddr, ifTraffic=ifTraffic, revProxyCache=revProxyCache, caShutdown=caShutdown, rcvdBytesPerSec=rcvdBytesPerSec, rsStatsEntry=rsStatsEntry, hitsReplyWNotModified=hitsReplyWNotModified, rsProtocol=rsProtocol, logHistoryEntry=logHistoryEntry, ipPktsOut=ipPktsOut, logMaxSeverity=logMaxSeverity, hitRevalidate=hitRevalidate, vsBackupHits=vsBackupHits, failLastSec5=failLastSec5, hcAddressType=hcAddressType, virtualStats=virtualStats, ctcpInSegs=ctcpInSegs, reqLastSec=reqLastSec, vsPort=vsPort, totalIPBytesIn=totalIPBytesIn, sysSwapCapacity=sysSwapCapacity, vsRedirectHits=vsRedirectHits, virtualPort=virtualPort, vrrpEntry=vrrpEntry, infInBcastPkts=infInBcastPkts, requestedConns=requestedConns, cpuUtilization=cpuUtilization, gpIndex=gpIndex, cacheHitRatio=cacheHitRatio, infIpv6Address=infIpv6Address, realAddressType=realAddressType, activeTransac=activeTransac, clusterVirPriority=clusterVirPriority, clickTcp=clickTcp, caTraps=caTraps, vsIpAddress=vsIpAddress, clusterId=clusterId, vsIpAddressType=vsIpAddressType, gpTotalHits=gpTotalHits, succPeakSec=succPeakSec, rsIpAddressType=rsIpAddressType, sysMemory=sysMemory, infOutMcastPkts=infOutMcastPkts, resumableSess=resumableSess, vsRCookieHits=vsRCookieHits, groupCurCfg=groupCurCfg, ipAddr=ipAddr, rsEntry=rsEntry, rsPort=rsPort, vsStatsTable=vsStatsTable, gpStatsEntry=gpStatsEntry, infInNUcastPkts=infInNUcastPkts, reqPeakSec5=reqPeakSec5, arrayNetworks=arrayNetworks, infInMcastPkts=infInMcastPkts, hitsReply=hitsReply, PYSNMP_MODULE_ID=arrayNetworks, totalRequestedConns=totalRequestedConns, missSess=missSess, rsCntOfReq=rsCntOfReq, vsConnCnt=vsConnCnt, logHistTableMaxLength=logHistTableMaxLength, vrrpTable=vrrpTable, vsRegexHits=vsRegexHits, infOutErrors=infOutErrors, reqLastSec5=reqLastSec5, realAddr=realAddr, virtServerID=virtServerID, vsStaticHits=vsStaticHits, rsTable=rsTable, vsIpAddr=vsIpAddr, rsCount=rsCount, rsStatsTable=rsStatsTable, ipAddrType=ipAddrType, vsMaxConn=vsMaxConn, infDescr=infDescr, sentBytesPerSec=sentBytesPerSec, vipStats=vipStats, fanspeed=fanspeed, responsesOnRegex=responsesOnRegex, vsEntry=vsEntry, clusterVirAddressType=clusterVirAddressType, vsDefaultHits=vsDefaultHits, clusterVirState=clusterVirState, totalFail=totalFail, groupID=groupID, slbStats=slbStats, maxCluster=maxCluster, realServer=realServer, cputemp=cputemp, vsQosNetworkHits=vsQosNetworkHits, openSSLConns=openSSLConns, infOutNUcastPkts=infOutNUcastPkts, requestsPerSec=requestsPerSec, reqPeakSec=reqPeakSec, ipIndex=ipIndex, caSyslog=caSyslog, ipPktsIn=ipPktsIn, clusterVirPreempt=clusterVirPreempt, peakSentBytesPerSec=peakSentBytesPerSec, hcConnAttempt=hcConnAttempt, infIndex=infIndex, caSyslogTrap=caSyslogTrap, succPeakHour=succPeakHour, responsesToHttps=responsesToHttps, vsProtocol=vsProtocol, vsIndex=vsIndex, sslTable=sslTable, totalReq=totalReq, infInDiscards=infInDiscards, severity=severity, logHistoryTable=logHistoryTable, vhostNum=vhostNum, hcAddr=hcAddr, ipStatsEntry=ipStatsEntry, slbMIB=slbMIB, succLastSec=succLastSec, rsIndex=rsIndex, serialNumber=serialNumber, vsQosBodyHits=vsQosBodyHits, hcIndex=hcIndex, hitsReplyWPreFailed=hitsReplyWPreFailed, gpStatsTable=gpStatsTable, hcNumUps=hcNumUps, cacheMissWRespNo=cacheMissWRespNo, failLastMin=failLastMin, Float=Float, infOutUcastPkts=infOutUcastPkts, realAddress=realAddress, clientEstabConn=clientEstabConn, infIpv4AddressType=infIpv4AddressType, clusterVirAddress=clusterVirAddress, totalBytesRcvd=totalBytesRcvd, cacheMissWNewEntry=cacheMissWNewEntry, infEntry=infEntry, hcPort=hcPort, totalOpenSSLConns=totalOpenSSLConns, succLastMin=succLastMin, infOutBandwidth=infOutBandwidth, vipStatus=vipStatus, virtualAddr=virtualAddr, realServerID=realServerID, hcRSCount=hcRSCount, sslStats=sslStats, ctcpConnLocalPort=ctcpConnLocalPort, totalIPPktsIn=totalIPPktsIn, reqLastMin=reqLastMin, ctcpCurrEstab=ctcpCurrEstab, vrrp=vrrp, clusterVirIndex=clusterVirIndex, gpEntry=gpEntry, rsStatus=rsStatus, purgeRequests=purgeRequests, cacheSkip=cacheSkip, vsQosCookieHits=vsQosCookieHits, msgText=msgText, ctcpConnLocalAddrType=ctcpConnLocalAddrType, infIpv6AddressType=infIpv6AddressType, ctcpEstabResets=ctcpEstabResets, totalIPPktsOut=totalIPPktsOut, failLastSec=failLastSec, realStats=realStats, reqPeakDay=reqPeakDay, infInBandwidth=infInBandwidth, clusterVirIfname=clusterVirIfname, infTotalOutOctets=infTotalOutOctets, rsConnCnt=rsConnCnt, totalSucc=totalSucc, index=index, ctcpConnRemAddress=ctcpConnRemAddress, ctcpConnTable=ctcpConnTable, ctcpConnRemAddr=ctcpConnRemAddr, sslMIB=sslMIB, succLastDay=succLastDay, hcConnSuccess=hcConnSuccess, vsStatsEntry=vsStatsEntry, monitor=monitor, sslEntry=sslEntry, infTotalInOctets=infTotalInOctets, vsCount=vsCount, startTime=startTime, ipBytesOut=ipBytesOut, resumedSess=resumedSess, logBasic=logBasic, postRequests=postRequests, infInUcastPkts=infInUcastPkts, rsID=rsID, infOperStatus=infOperStatus, realPort=realPort, healthCheck=healthCheck, sslIndex=sslIndex, cacheBasicStats=cacheBasicStats, clusterVirAuthType=clusterVirAuthType, ctcpConnState=ctcpConnState, SyslogSeverity=SyslogSeverity, requestsOnRegex=requestsOnRegex, succPeakMin=succPeakMin, sysMemoryUtilization=sysMemoryUtilization, ipAddress=ipAddress, ctcpIndex=ctcpIndex, systemInfo=systemInfo, memory=memory, totalBytesSent=totalBytesSent, totalIPBytesOut=totalIPBytesOut, connectionsPerSec=connectionsPerSec, infNumber=infNumber, ipBytesIn=ipBytesIn, groupIndex=groupIndex, ctcpConnRemPort=ctcpConnRemPort, ipStatsTable=ipStatsTable, virtualAddress=virtualAddress, succLastSec5=succLastSec5, hcCause=hcCause, rsTotalHits=rsTotalHits, vsPerstntURLHits=vsPerstntURLHits, infInOctets=infInOctets, vsPerstntCookieHits=vsPerstntCookieHits, infTable=infTable, performance=performance, hcName=hcName, reqLastDay=reqLastDay, failLastHour=failLastHour, vsICookieHits=vsICookieHits, reqPeakMin=reqPeakMin, ctcpInErrs=ctcpInErrs, realIndex=realIndex, hostName=hostName, clusterVirAddr=clusterVirAddr, sysSwapUsed=sysSwapUsed, ctcpConnEntry=ctcpConnEntry, gpMetrics=gpMetrics)
mibBuilder.exportSymbols("CA-SNMP-MIB8", connsPerSec=connsPerSec, virtualAddressType=virtualAddressType, serverEstabConn=serverEstabConn, sslGeneral=sslGeneral, peakRcvdBytesPerSec=peakRcvdBytesPerSec, infOutBcastPkts=infOutBcastPkts, rsAvgRespTime=rsAvgRespTime, licenseRemainingDays=licenseRemainingDays, succPeakSec5=succPeakSec5, infIpv4Address=infIpv4Address, vsHeaderHits=vsHeaderHits, failLastDay=failLastDay, acceptedConns=acceptedConns, ctcpConnLocalAddress=ctcpConnLocalAddress, sdns=sdns, requestsReceived=requestsReceived, logNotificationsEnabled=logNotificationsEnabled, vsHostnameHits=vsHostnameHits, clusterVrrp=clusterVrrp, hcStatus=hcStatus, groupCount=groupCount, cacheMissWNoncacheReq=cacheMissWNoncacheReq, realStatus=realStatus, gpID=gpID, ctcpActiveOpens=ctcpActiveOpens, vsQosURLHits=vsQosURLHits, reqLastHour=reqLastHour, infInUnknownProtos=infInUnknownProtos, rsIpAddr=rsIpAddr, succLastHour=succLastHour, getRequests=getRequests, gpTable=gpTable, hcNumDowns=hcNumDowns, requestsToUrl=requestsToUrl, logHistory=logHistory, rsIpAddress=rsIpAddress, clusterVirInterval=clusterVirInterval, infAddress=infAddress, ctcpAttemptFails=ctcpAttemptFails, clusterVirAuthPasswd=clusterVirAuthPasswd, virtualIndex=virtualIndex, cacheStatus=cacheStatus, ctcpOutSegs=ctcpOutSegs, hcAddress=hcAddress, logNotificationsSent=logNotificationsSent, powerstate=powerstate, syslogTrap=syslogTrap, succPeakDay=succPeakDay, infInErrors=infInErrors, vsQosClientPortHits=vsQosClientPortHits, requestsToHttps=requestsToHttps, slbGeneral=slbGeneral, ctcpPassiveOpens=ctcpPassiveOpens, vsURLHits=vsURLHits, currentTime=currentTime)
