...
Counter64, Unsigned32, Integer32, IpAddress, Gauge32, Bits, MibIdentifier, iso, ModuleIdentity, Opaque, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Counter32, TimeTicks, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "Counter64", "Unsigned32", "Integer32", "IpAddress", "Gauge32", "Bits", "MibIdentifier", "iso", "ModuleIdentity", "Opaque", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Counter32", "TimeTicks", "enterprises")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", virtualAddress=virtualAddress, hitsReply=hitsReply, clusterNum=clusterNum, infInNUcastPkts=infInNUcastPkts, realID=realID, hostName=hostName, ctcpConnRemAddr=ctcpConnRemAddr, startTime=startTime, cacheMissWNewEntry=cacheMissWNewEntry, virtServerID=virtServerID, vrrpTable=vrrpTable, cacheMissWRespNo=cacheMissWRespNo, virtualServer=virtualServer, vipStats=vipStats, hitsReplyWPreFailed=hitsReplyWPreFailed, vsCacheHits=vsCacheHits, vsBackupHits=vsBackupHits, rsConnCnt=rsConnCnt, ipPktsOut=ipPktsOut, totalSucc=totalSucc, postRequests=postRequests, realServerID=realServerID, connsPerSec=connsPerSec, infDescr=infDescr, infOutErrors=infOutErrors, groupCount=groupCount, rsID=rsID, sysSwapCapacity=sysSwapCapacity, clusterVirIfname=clusterVirIfname, requestsToHttps=requestsToHttps, succPeakHour=succPeakHour, ctcpConnTable=ctcpConnTable, licenseRemainingDays=licenseRemainingDays, caTraps=caTraps, rsStatsEntry=rsStatsEntry, infTotalInOctets=infTotalInOctets, ctcpActiveOpens=ctcpActiveOpens, rsIpAddress=rsIpAddress, ctcpConnLocalAddr=ctcpConnLocalAddr, failLastSec5=failLastSec5, infTotalOutOctets=infTotalOutOctets, succLastSec5=succLastSec5, powerstate=powerstate, healthCheck=healthCheck, serverEstabConn=serverEstabConn, succPeakSec5=succPeakSec5, logMaxSeverity=logMaxSeverity, clusterVirAuthPasswd=clusterVirAuthPasswd, clusterVirAddressType=clusterVirAddressType, rsIpAddressType=rsIpAddressType, infNumber=infNumber, ctcpConnEntry=ctcpConnEntry, groupStats=groupStats, ctcpConnLocalAddrType=ctcpConnLocalAddrType, caStart=caStart, slbMIB=slbMIB, vsPort=vsPort, gpTotalHits=gpTotalHits, rsEntry=rsEntry, ifTraffic=ifTraffic, cpuUtilization=cpuUtilization, totalAcceptedConns=totalAcceptedConns, ctcpInErrs=ctcpInErrs, memory=memory, cacheSkip=cacheSkip, ctcpIndex=ctcpIndex, cputemp=cputemp, clusterVirIndex=clusterVirIndex, succLastDay=succLastDay, vsStatsTable=vsStatsTable, sysMemoryUtilization=sysMemoryUtilization, hcStatus=hcStatus, rsMaxConn=rsMaxConn, ctcpConnLocalPort=ctcpConnLocalPort, sysSwapUsed=sysSwapUsed, infIpv6Address=infIpv6Address, realAddressType=realAddressType, ctcpEstabResets=ctcpEstabResets, vsHeaderHits=vsHeaderHits, requestedConns=requestedConns, sdns=sdns, infInUnknownProtos=infInUnknownProtos, purgeRequests=purgeRequests, succLastHour=succLastHour, infInDiscards=infInDiscards, requestsReceived=requestsReceived, compression=compression, virtualStats=virtualStats, hcStats=hcStats, rsProtocol=rsProtocol, vsDefaultHits=vsDefaultHits, reqLastHour=reqLastHour, sslMIB=sslMIB, rsAvgRespTime=rsAvgRespTime, rsIndex=rsIndex, peakSentBytesPerSec=peakSentBytesPerSec, logNotificationsSent=logNotificationsSent, rsCount=rsCount, realIndex=realIndex, totalIPBytesOut=totalIPBytesOut, reqPeakDay=reqPeakDay, ctcpConnRemAddress=ctcpConnRemAddress, vsIpAddr=vsIpAddr, totalBytesSent=totalBytesSent, msgText=msgText, openSSLConns=openSSLConns, infOperStatus=infOperStatus, infInOctets=infInOctets, totalIPPktsOut=totalIPPktsOut, clickTcp=clickTcp, vsRegexHits=vsRegexHits, caShutdown=caShutdown, reqPeakSec=reqPeakSec, totalOpenSSLConns=totalOpenSSLConns, hcConnFail=hcConnFail, hcConnSuccess=hcConnSuccess, gpTable=gpTable, totalFail=totalFail, ctcpConnRemPort=ctcpConnRemPort, clusterId=clusterId, ipPktsIn=ipPktsIn, hcAddr=hcAddr, gpMetrics=gpMetrics, gpEntry=gpEntry, clusterVirState=clusterVirState, ipIndex=ipIndex, logHistory=logHistory, rsPort=rsPort, clusterVirAddr=clusterVirAddr, vsIpAddress=vsIpAddress, succPeakDay=succPeakDay, ctcpPassiveOpens=ctcpPassiveOpens, infInMcastPkts=infInMcastPkts, Float=Float, slbGeneral=slbGeneral, rsTable=rsTable, failLastMin=failLastMin, vsQosNetworkHits=vsQosNetworkHits, rsCntOfReq=rsCntOfReq, ctcpInSegs=ctcpInSegs, arrayNetworks=arrayNetworks, hcStatsEntry=hcStatsEntry, ctcpConnLocalAddress=ctcpConnLocalAddress, requestsPerSec=requestsPerSec, totalIPPktsIn=totalIPPktsIn, rsIpAddr=rsIpAddr, sentBytesPerSec=sentBytesPerSec, hcCause=hcCause, missSess=missSess, clusterVirPreempt=clusterVirPreempt, infEntry=infEntry, vsRCookieHits=vsRCookieHits, responsesToHttps=responsesToHttps, virtualAddr=virtualAddr, vsStatsEntry=vsStatsEntry, hcRSCount=hcRSCount, ipAddress=ipAddress, fanspeed=fanspeed, vsQosClientPortHits=vsQosClientPortHits, headRequests=headRequests, reqLastSec=reqLastSec, cacheHitRatio=cacheHitRatio, vsProtocol=vsProtocol, vsHostnameHits=vsHostnameHits, ipAddrType=ipAddrType, vsQosCookieHits=vsQosCookieHits, totalRequestedConns=totalRequestedConns, SyslogSeverity=SyslogSeverity, sslStats=sslStats, infInBandwidth=infInBandwidth, severity=severity, vsRedirectHits=vsRedirectHits, connectionsPerSec=connectionsPerSec, infIpv6AddressType=infIpv6AddressType, cacheBasicStats=cacheBasicStats, performance=performance, ipStatsEntry=ipStatsEntry, infOutOctets=infOutOctets, realAddress=realAddress, rsStatsTable=rsStatsTable, ipStatsTable=ipStatsTable, sslIndex=sslIndex, sslTable=sslTable, vsPerstntURLHits=vsPerstntURLHits, failLastDay=failLastDay, vsPerstntCookieHits=vsPerstntCookieHits, infInErrors=infInErrors, hcName=hcName, groupIndex=groupIndex, acceptedConns=acceptedConns, infIpv4AddressType=infIpv4AddressType, systemInfo=systemInfo, reqLastDay=reqLastDay, sslEntry=sslEntry, totalIPBytesIn=totalIPBytesIn, activeTransac=activeTransac, realStatus=realStatus, clusterVirInterval=clusterVirInterval, hcAddress=hcAddress, virtualAddressType=virtualAddressType, peakRcvdBytesPerSec=peakRcvdBytesPerSec, infOutMcastPkts=infOutMcastPkts, vsIpAddressType=vsIpAddressType, reqLastMin=reqLastMin, logBasic=logBasic, getRequests=getRequests, rsStatus=rsStatus, clusterVrrp=clusterVrrp, infOutNUcastPkts=infOutNUcastPkts, hcPort=hcPort, vsTable=vsTable, hcNumUps=hcNumUps, infInUcastPkts=infInUcastPkts, hcNumDowns=hcNumDowns, groupCurCfg=groupCurCfg, vsConnCnt=vsConnCnt, reqPeakMin=reqPeakMin, logHistoryEntry=logHistoryEntry, gpIndex=gpIndex, ctcpOutSegs=ctcpOutSegs, ctcpConnRemAddrType=ctcpConnRemAddrType, caSyslog=caSyslog, vsStaticHits=vsStaticHits, hcAddressType=hcAddressType, logNotificationsEnabled=logNotificationsEnabled, monitor=monitor, clusterVirPriority=clusterVirPriority, hcConnAttempt=hcConnAttempt, index=index, infOutBandwidth=infOutBandwidth, ipBytesIn=ipBytesIn, hcIndex=hcIndex, groupID=groupID, resumedSess=resumedSess, infOutBcastPkts=infOutBcastPkts, ctcpConnState=ctcpConnState, succPeakMin=succPeakMin, hitRevalidate=hitRevalidate, rsTotalHits=rsTotalHits, vsQosBodyHits=vsQosBodyHits, vsMaxConn=vsMaxConn, vsIndex=vsIndex, succLastMin=succLastMin, requestsOnRegex=requestsOnRegex, vrrp=vrrp, clientEstabConn=clientEstabConn, vsURLHits=vsURLHits, vsQosURLHits=vsQosURLHits, totalReq=totalReq, reqPeakSec5=reqPeakSec5, gpStatsEntry=gpStatsEntry, realStats=realStats, virtualIndex=virtualIndex, totalBytesRcvd=totalBytesRcvd)
mibBuilder.exportSymbols("CA-SNMP-MIB8", infTable=infTable, realServer=realServer, gpID=gpID, ctcpAttemptFails=ctcpAttemptFails, infIpv4Address=infIpv4Address, failLastSec=failLastSec, caSyslogTrap=caSyslogTrap, responsesOnRegex=responsesOnRegex, vsHashURLHits=vsHashURLHits, vsEntry=vsEntry, realPort=realPort, cacheMissWNoncacheReq=cacheMissWNoncacheReq, vhostNum=vhostNum, requestsToUrl=requestsToUrl, logHistoryTable=logHistoryTable, revProxyCache=revProxyCache, vsICookieHits=vsICookieHits, ctcpOutRsts=ctcpOutRsts, PYSNMP_MODULE_ID=arrayNetworks, succLastSec=succLastSec, clusterVirAuthType=clusterVirAuthType, infAddress=infAddress, vhostName=vhostName, vsCount=vsCount, sslGeneral=sslGeneral, hcStatsTable=hcStatsTable, ipAddr=ipAddr, sysMemory=sysMemory, realAddr=realAddr, cacheStatus=cacheStatus, slbStats=slbStats, gpStatsTable=gpStatsTable, infOutUcastPkts=infOutUcastPkts, vrrpEntry=vrrpEntry, maxCluster=maxCluster, ipBytesOut=ipBytesOut, logHistTableMaxLength=logHistTableMaxLength, rcvdBytesPerSec=rcvdBytesPerSec, reqLastSec5=reqLastSec5, virtualPort=virtualPort, reqPeakHour=reqPeakHour, serialNumber=serialNumber, failLastHour=failLastHour, syslogTrap=syslogTrap, infIndex=infIndex, hitsReplyWNotModified=hitsReplyWNotModified, resumableSess=resumableSess, succPeakSec=succPeakSec, ctcpRetransSegs=ctcpRetransSegs, ctcpCurrEstab=ctcpCurrEstab, currentTime=currentTime, vsID=vsID, infInBcastPkts=infInBcastPkts, clusterVirAddress=clusterVirAddress, vipStatus=vipStatus)
