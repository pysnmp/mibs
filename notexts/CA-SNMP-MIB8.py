...
ModuleIdentity, Integer32, Opaque, NotificationType, Counter64, Counter32, iso, Bits, TimeTicks, Unsigned32, IpAddress, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, ObjectIdentity, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Integer32", "Opaque", "NotificationType", "Counter64", "Counter32", "iso", "Bits", "TimeTicks", "Unsigned32", "IpAddress", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "ObjectIdentity", "Gauge32")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsPerstntCookieHits=vsPerstntCookieHits, infInUnknownProtos=infInUnknownProtos, revProxyCache=revProxyCache, failLastSec5=failLastSec5, caStart=caStart, totalBytesRcvd=totalBytesRcvd, vsURLHits=vsURLHits, vsDefaultHits=vsDefaultHits, reqPeakSec5=reqPeakSec5, hcIndex=hcIndex, virtualIndex=virtualIndex, logHistoryEntry=logHistoryEntry, headRequests=headRequests, vsICookieHits=vsICookieHits, requestsOnRegex=requestsOnRegex, cacheStatus=cacheStatus, failLastHour=failLastHour, connectionsPerSec=connectionsPerSec, cacheSkip=cacheSkip, gpIndex=gpIndex, vsBackupHits=vsBackupHits, vsRCookieHits=vsRCookieHits, infOutNUcastPkts=infOutNUcastPkts, gpStatsEntry=gpStatsEntry, logHistoryTable=logHistoryTable, hcStats=hcStats, rsStatsTable=rsStatsTable, cacheMissWRespNo=cacheMissWRespNo, caSyslogTrap=caSyslogTrap, groupIndex=groupIndex, ctcpConnRemAddrType=ctcpConnRemAddrType, vsCount=vsCount, ctcpOutRsts=ctcpOutRsts, fanspeed=fanspeed, gpTotalHits=gpTotalHits, virtualStats=virtualStats, totalBytesSent=totalBytesSent, clusterVirAuthPasswd=clusterVirAuthPasswd, clusterVirState=clusterVirState, logNotificationsSent=logNotificationsSent, hcName=hcName, resumableSess=resumableSess, sslMIB=sslMIB, vsQosNetworkHits=vsQosNetworkHits, ipPktsOut=ipPktsOut, hcNumDowns=hcNumDowns, clusterVirAddressType=clusterVirAddressType, logMaxSeverity=logMaxSeverity, ctcpConnTable=ctcpConnTable, clusterVirPreempt=clusterVirPreempt, ctcpConnRemAddress=ctcpConnRemAddress, reqPeakMin=reqPeakMin, purgeRequests=purgeRequests, vsHostnameHits=vsHostnameHits, rsMaxConn=rsMaxConn, realID=realID, infInOctets=infInOctets, vsRedirectHits=vsRedirectHits, cacheBasicStats=cacheBasicStats, rsEntry=rsEntry, vsCacheHits=vsCacheHits, rsIpAddress=rsIpAddress, sentBytesPerSec=sentBytesPerSec, cacheMissWNewEntry=cacheMissWNewEntry, succLastMin=succLastMin, infIpv4Address=infIpv4Address, succPeakHour=succPeakHour, reqPeakHour=reqPeakHour, responsesOnRegex=responsesOnRegex, ipPktsIn=ipPktsIn, ctcpEstabResets=ctcpEstabResets, vsStatsEntry=vsStatsEntry, slbGeneral=slbGeneral, vrrpEntry=vrrpEntry, realAddress=realAddress, virtServerID=virtServerID, clusterVirPriority=clusterVirPriority, clickTcp=clickTcp, hcAddr=hcAddr, severity=severity, postRequests=postRequests, gpStatsTable=gpStatsTable, requestsPerSec=requestsPerSec, ipStatsTable=ipStatsTable, connsPerSec=connsPerSec, vsHashURLHits=vsHashURLHits, reqPeakDay=reqPeakDay, sysMemory=sysMemory, infEntry=infEntry, sslEntry=sslEntry, ipStatsEntry=ipStatsEntry, caTraps=caTraps, sysSwapUsed=sysSwapUsed, ctcpConnState=ctcpConnState, hcAddressType=hcAddressType, SyslogSeverity=SyslogSeverity, infInBcastPkts=infInBcastPkts, ctcpConnLocalAddress=ctcpConnLocalAddress, powerstate=powerstate, caSyslog=caSyslog, vsRegexHits=vsRegexHits, succPeakSec=succPeakSec, totalOpenSSLConns=totalOpenSSLConns, gpID=gpID, hcConnAttempt=hcConnAttempt, ctcpConnEntry=ctcpConnEntry, rsCntOfReq=rsCntOfReq, performance=performance, realServerID=realServerID, reqLastHour=reqLastHour, vsHeaderHits=vsHeaderHits, Float=Float, arrayNetworks=arrayNetworks, requestedConns=requestedConns, systemInfo=systemInfo, totalSucc=totalSucc, failLastMin=failLastMin, vsQosBodyHits=vsQosBodyHits, serialNumber=serialNumber, virtualAddressType=virtualAddressType, reqLastSec=reqLastSec, vhostNum=vhostNum, logBasic=logBasic, hcRSCount=hcRSCount, getRequests=getRequests, realServer=realServer, groupCount=groupCount, sysMemoryUtilization=sysMemoryUtilization, ctcpConnLocalPort=ctcpConnLocalPort, rsAvgRespTime=rsAvgRespTime, infTotalInOctets=infTotalInOctets, totalIPPktsIn=totalIPPktsIn, infDescr=infDescr, ipAddrType=ipAddrType, gpTable=gpTable, requestsReceived=requestsReceived, hcStatsEntry=hcStatsEntry, openSSLConns=openSSLConns, slbMIB=slbMIB, sysSwapCapacity=sysSwapCapacity, succLastDay=succLastDay, succPeakMin=succPeakMin, realStats=realStats, reqPeakSec=reqPeakSec, rsStatsEntry=rsStatsEntry, requestsToHttps=requestsToHttps, ctcpRetransSegs=ctcpRetransSegs, totalRequestedConns=totalRequestedConns, hcCause=hcCause, responsesToHttps=responsesToHttps, totalIPPktsOut=totalIPPktsOut, ctcpInErrs=ctcpInErrs, clusterVirAddress=clusterVirAddress, virtualPort=virtualPort, infIndex=infIndex, ctcpIndex=ctcpIndex, infTotalOutOctets=infTotalOutOctets, realAddr=realAddr, sslIndex=sslIndex, ipAddress=ipAddress, memory=memory, clusterId=clusterId, infInDiscards=infInDiscards, clientEstabConn=clientEstabConn, hitsReplyWPreFailed=hitsReplyWPreFailed, vrrpTable=vrrpTable, gpEntry=gpEntry, vsConnCnt=vsConnCnt, clusterVirIndex=clusterVirIndex, ipIndex=ipIndex, succLastSec5=succLastSec5, hitsReply=hitsReply, resumedSess=resumedSess, ctcpAttemptFails=ctcpAttemptFails, msgText=msgText, cacheHitRatio=cacheHitRatio, licenseRemainingDays=licenseRemainingDays, infOutMcastPkts=infOutMcastPkts, vsIndex=vsIndex, totalFail=totalFail, succLastHour=succLastHour, failLastSec=failLastSec, virtualAddress=virtualAddress, sdns=sdns, vipStats=vipStats, rsIpAddr=rsIpAddr, reqLastMin=reqLastMin, vhostName=vhostName, reqLastDay=reqLastDay, realPort=realPort, sslTable=sslTable, logHistTableMaxLength=logHistTableMaxLength, infInMcastPkts=infInMcastPkts, ctcpInSegs=ctcpInSegs, ipAddr=ipAddr, hcConnFail=hcConnFail, groupCurCfg=groupCurCfg, hcPort=hcPort, realStatus=realStatus, totalAcceptedConns=totalAcceptedConns, compression=compression, totalIPBytesOut=totalIPBytesOut, vipStatus=vipStatus, hcConnSuccess=hcConnSuccess, logNotificationsEnabled=logNotificationsEnabled, ctcpConnLocalAddr=ctcpConnLocalAddr, hcAddress=hcAddress, ctcpConnRemPort=ctcpConnRemPort, ctcpConnRemAddr=ctcpConnRemAddr, activeTransac=activeTransac, rsStatus=rsStatus, infNumber=infNumber, rsTotalHits=rsTotalHits, index=index, vsTable=vsTable, vsProtocol=vsProtocol, vsStatsTable=vsStatsTable, infOperStatus=infOperStatus, succPeakSec5=succPeakSec5, currentTime=currentTime, syslogTrap=syslogTrap, infInNUcastPkts=infInNUcastPkts, requestsToUrl=requestsToUrl, infIpv6Address=infIpv6Address, ctcpCurrEstab=ctcpCurrEstab, infInUcastPkts=infInUcastPkts, rsIndex=rsIndex, vsQosCookieHits=vsQosCookieHits, ctcpPassiveOpens=ctcpPassiveOpens, infInErrors=infInErrors, infOutErrors=infOutErrors, vsStaticHits=vsStaticHits, hcStatus=hcStatus, peakRcvdBytesPerSec=peakRcvdBytesPerSec, ctcpActiveOpens=ctcpActiveOpens, hostName=hostName, clusterVirInterval=clusterVirInterval, maxCluster=maxCluster, infIpv6AddressType=infIpv6AddressType, reqLastSec5=reqLastSec5, rcvdBytesPerSec=rcvdBytesPerSec, infInBandwidth=infInBandwidth, vsQosURLHits=vsQosURLHits, monitor=monitor, groupID=groupID, PYSNMP_MODULE_ID=arrayNetworks, infOutBandwidth=infOutBandwidth, rsProtocol=rsProtocol, hcNumUps=hcNumUps, totalReq=totalReq, cpuUtilization=cpuUtilization, logHistory=logHistory, vsQosClientPortHits=vsQosClientPortHits, vsIpAddressType=vsIpAddressType)
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsMaxConn=vsMaxConn, sslStats=sslStats, rsCount=rsCount, virtualServer=virtualServer, vrrp=vrrp, vsIpAddr=vsIpAddr, caShutdown=caShutdown, clusterVirIfname=clusterVirIfname, slbStats=slbStats, virtualAddr=virtualAddr, sslGeneral=sslGeneral, cputemp=cputemp, rsID=rsID, infIpv4AddressType=infIpv4AddressType, clusterNum=clusterNum, clusterVrrp=clusterVrrp, cacheMissWNoncacheReq=cacheMissWNoncacheReq, vsPerstntURLHits=vsPerstntURLHits, startTime=startTime, peakSentBytesPerSec=peakSentBytesPerSec, vsID=vsID, ctcpOutSegs=ctcpOutSegs, totalIPBytesIn=totalIPBytesIn, ipBytesIn=ipBytesIn, gpMetrics=gpMetrics, realIndex=realIndex, succLastSec=succLastSec, acceptedConns=acceptedConns, missSess=missSess, realAddressType=realAddressType, clusterVirAuthType=clusterVirAuthType, clusterVirAddr=clusterVirAddr, ipBytesOut=ipBytesOut, rsPort=rsPort, vsEntry=vsEntry, hitsReplyWNotModified=hitsReplyWNotModified, infOutUcastPkts=infOutUcastPkts, rsIpAddressType=rsIpAddressType, groupStats=groupStats, rsConnCnt=rsConnCnt, infOutOctets=infOutOctets, infTable=infTable, rsTable=rsTable, hcStatsTable=hcStatsTable, hitRevalidate=hitRevalidate, succPeakDay=succPeakDay, infAddress=infAddress, healthCheck=healthCheck, vsPort=vsPort, serverEstabConn=serverEstabConn, infOutBcastPkts=infOutBcastPkts, vsIpAddress=vsIpAddress, failLastDay=failLastDay, ifTraffic=ifTraffic, ctcpConnLocalAddrType=ctcpConnLocalAddrType)
