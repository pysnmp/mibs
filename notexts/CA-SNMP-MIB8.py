...
ModuleIdentity, Integer32, Counter64, Bits, ObjectIdentity, Opaque, Unsigned32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, MibIdentifier, iso, enterprises, Counter32, IpAddress, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Integer32", "Counter64", "Bits", "ObjectIdentity", "Opaque", "Unsigned32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "MibIdentifier", "iso", "enterprises", "Counter32", "IpAddress", "TimeTicks")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", clusterVirAuthPasswd=clusterVirAuthPasswd, rsCount=rsCount, rsIpAddress=rsIpAddress, peakSentBytesPerSec=peakSentBytesPerSec, caSyslog=caSyslog, reqPeakDay=reqPeakDay, vsRegexHits=vsRegexHits, ctcpConnRemAddrType=ctcpConnRemAddrType, logNotificationsSent=logNotificationsSent, infAddress=infAddress, succLastMin=succLastMin, infTotalOutOctets=infTotalOutOctets, clusterVirState=clusterVirState, sslMIB=sslMIB, succPeakMin=succPeakMin, fanspeed=fanspeed, vsIndex=vsIndex, infOutNUcastPkts=infOutNUcastPkts, vsCount=vsCount, groupID=groupID, succLastSec5=succLastSec5, cpuUtilization=cpuUtilization, vsQosClientPortHits=vsQosClientPortHits, hcRSCount=hcRSCount, infIpv4Address=infIpv4Address, reqPeakMin=reqPeakMin, infInErrors=infInErrors, rsIndex=rsIndex, revProxyCache=revProxyCache, hcPort=hcPort, vsStatsTable=vsStatsTable, connsPerSec=connsPerSec, slbStats=slbStats, failLastHour=failLastHour, ctcpConnRemAddr=ctcpConnRemAddr, ipPktsIn=ipPktsIn, infOutOctets=infOutOctets, ipAddress=ipAddress, PYSNMP_MODULE_ID=arrayNetworks, serialNumber=serialNumber, ctcpRetransSegs=ctcpRetransSegs, hcAddressType=hcAddressType, vipStats=vipStats, totalIPBytesOut=totalIPBytesOut, virtualServer=virtualServer, ctcpConnRemPort=ctcpConnRemPort, hcStatus=hcStatus, caTraps=caTraps, hcNumUps=hcNumUps, vsStatsEntry=vsStatsEntry, vsIpAddressType=vsIpAddressType, totalIPPktsIn=totalIPPktsIn, hcName=hcName, sysMemoryUtilization=sysMemoryUtilization, infTable=infTable, infOutMcastPkts=infOutMcastPkts, infInUcastPkts=infInUcastPkts, virtualAddress=virtualAddress, logHistoryEntry=logHistoryEntry, hcAddr=hcAddr, ipAddrType=ipAddrType, reqPeakSec5=reqPeakSec5, sslGeneral=sslGeneral, syslogTrap=syslogTrap, succPeakHour=succPeakHour, caStart=caStart, rsIpAddressType=rsIpAddressType, hitsReply=hitsReply, hcStatsEntry=hcStatsEntry, hcStatsTable=hcStatsTable, clusterVirPriority=clusterVirPriority, totalIPPktsOut=totalIPPktsOut, ctcpConnEntry=ctcpConnEntry, hitsReplyWPreFailed=hitsReplyWPreFailed, powerstate=powerstate, realStats=realStats, clusterVirAddr=clusterVirAddr, rsTotalHits=rsTotalHits, gpStatsEntry=gpStatsEntry, realServer=realServer, reqLastHour=reqLastHour, vsQosBodyHits=vsQosBodyHits, totalRequestedConns=totalRequestedConns, totalOpenSSLConns=totalOpenSSLConns, failLastDay=failLastDay, infNumber=infNumber, virtualPort=virtualPort, sslIndex=sslIndex, slbGeneral=slbGeneral, rsStatsTable=rsStatsTable, msgText=msgText, currentTime=currentTime, vrrpEntry=vrrpEntry, vipStatus=vipStatus, vsBackupHits=vsBackupHits, ctcpAttemptFails=ctcpAttemptFails, totalBytesSent=totalBytesSent, infInBandwidth=infInBandwidth, sslTable=sslTable, performance=performance, virtualAddr=virtualAddr, hcConnFail=hcConnFail, clusterVirAddressType=clusterVirAddressType, infIpv4AddressType=infIpv4AddressType, succLastDay=succLastDay, logHistoryTable=logHistoryTable, infOperStatus=infOperStatus, failLastSec5=failLastSec5, headRequests=headRequests, vsICookieHits=vsICookieHits, clusterVirIfname=clusterVirIfname, requestsToHttps=requestsToHttps, cacheMissWNewEntry=cacheMissWNewEntry, caShutdown=caShutdown, clickTcp=clickTcp, vsStaticHits=vsStaticHits, activeTransac=activeTransac, clusterNum=clusterNum, totalReq=totalReq, infEntry=infEntry, ipStatsTable=ipStatsTable, systemInfo=systemInfo, monitor=monitor, hcNumDowns=hcNumDowns, ctcpActiveOpens=ctcpActiveOpens, peakRcvdBytesPerSec=peakRcvdBytesPerSec, infOutBcastPkts=infOutBcastPkts, failLastSec=failLastSec, ipIndex=ipIndex, rsPort=rsPort, rsStatus=rsStatus, realPort=realPort, vsCacheHits=vsCacheHits, missSess=missSess, licenseRemainingDays=licenseRemainingDays, ctcpConnState=ctcpConnState, cacheMissWRespNo=cacheMissWRespNo, vsQosURLHits=vsQosURLHits, vsPort=vsPort, SyslogSeverity=SyslogSeverity, rsConnCnt=rsConnCnt, rsCntOfReq=rsCntOfReq, Float=Float, realID=realID, vrrp=vrrp, vsPerstntCookieHits=vsPerstntCookieHits, vhostName=vhostName, virtualStats=virtualStats, rsStatsEntry=rsStatsEntry, openSSLConns=openSSLConns, serverEstabConn=serverEstabConn, vsPerstntURLHits=vsPerstntURLHits, gpTable=gpTable, vsIpAddress=vsIpAddress, virtualIndex=virtualIndex, ctcpPassiveOpens=ctcpPassiveOpens, groupStats=groupStats, sslStats=sslStats, hcStats=hcStats, reqLastSec=reqLastSec, gpStatsTable=gpStatsTable, infOutBandwidth=infOutBandwidth, cacheBasicStats=cacheBasicStats, realAddressType=realAddressType, clusterVirInterval=clusterVirInterval, infInNUcastPkts=infInNUcastPkts, ctcpConnLocalPort=ctcpConnLocalPort, rsIpAddr=rsIpAddr, ctcpConnLocalAddrType=ctcpConnLocalAddrType, totalBytesRcvd=totalBytesRcvd, responsesToHttps=responsesToHttps, infIpv6Address=infIpv6Address, vhostNum=vhostNum, virtServerID=virtServerID, hitRevalidate=hitRevalidate, succLastHour=succLastHour, sysSwapCapacity=sysSwapCapacity, vsID=vsID, healthCheck=healthCheck, memory=memory, vsIpAddr=vsIpAddr, virtualAddressType=virtualAddressType, requestsReceived=requestsReceived, ipAddr=ipAddr, infOutUcastPkts=infOutUcastPkts, groupCurCfg=groupCurCfg, succPeakDay=succPeakDay, requestsOnRegex=requestsOnRegex, clusterVirAuthType=clusterVirAuthType, slbMIB=slbMIB, vsDefaultHits=vsDefaultHits, vsTable=vsTable, clientEstabConn=clientEstabConn, ctcpIndex=ctcpIndex, acceptedConns=acceptedConns, ctcpConnLocalAddress=ctcpConnLocalAddress, ctcpConnLocalAddr=ctcpConnLocalAddr, clusterVirAddress=clusterVirAddress, groupIndex=groupIndex, cacheMissWNoncacheReq=cacheMissWNoncacheReq, vsMaxConn=vsMaxConn, ctcpConnTable=ctcpConnTable, ctcpCurrEstab=ctcpCurrEstab, realIndex=realIndex, sysSwapUsed=sysSwapUsed, reqLastDay=reqLastDay, sslEntry=sslEntry, vsProtocol=vsProtocol, rsEntry=rsEntry, vsEntry=vsEntry, totalAcceptedConns=totalAcceptedConns, gpEntry=gpEntry, vsQosNetworkHits=vsQosNetworkHits, ipBytesOut=ipBytesOut, succLastSec=succLastSec, ctcpOutSegs=ctcpOutSegs, realServerID=realServerID, infInBcastPkts=infInBcastPkts, rsAvgRespTime=rsAvgRespTime, sdns=sdns, groupCount=groupCount, clusterVirPreempt=clusterVirPreempt, infIndex=infIndex, logBasic=logBasic, vsHostnameHits=vsHostnameHits, rsID=rsID, infIpv6AddressType=infIpv6AddressType, ctcpInSegs=ctcpInSegs, infInUnknownProtos=infInUnknownProtos, hcConnAttempt=hcConnAttempt, requestedConns=requestedConns, logNotificationsEnabled=logNotificationsEnabled, maxCluster=maxCluster, requestsToUrl=requestsToUrl, logHistTableMaxLength=logHistTableMaxLength, compression=compression, clusterId=clusterId, totalIPBytesIn=totalIPBytesIn, connectionsPerSec=connectionsPerSec, resumedSess=resumedSess, startTime=startTime, ifTraffic=ifTraffic, index=index, resumableSess=resumableSess, hitsReplyWNotModified=hitsReplyWNotModified, logHistory=logHistory, ipBytesIn=ipBytesIn, failLastMin=failLastMin, vsConnCnt=vsConnCnt, infOutErrors=infOutErrors, reqLastSec5=reqLastSec5, hcIndex=hcIndex, infDescr=infDescr, cacheStatus=cacheStatus)
mibBuilder.exportSymbols("CA-SNMP-MIB8", succPeakSec=succPeakSec, reqPeakSec=reqPeakSec, realAddress=realAddress, vsURLHits=vsURLHits, reqLastMin=reqLastMin, severity=severity, vsRCookieHits=vsRCookieHits, infInOctets=infInOctets, gpMetrics=gpMetrics, clusterVrrp=clusterVrrp, ipStatsEntry=ipStatsEntry, gpID=gpID, totalFail=totalFail, infTotalInOctets=infTotalInOctets, totalSucc=totalSucc, reqPeakHour=reqPeakHour, succPeakSec5=succPeakSec5, rsMaxConn=rsMaxConn, vsHashURLHits=vsHashURLHits, ctcpEstabResets=ctcpEstabResets, sentBytesPerSec=sentBytesPerSec, arrayNetworks=arrayNetworks, realStatus=realStatus, hcAddress=hcAddress, gpTotalHits=gpTotalHits, getRequests=getRequests, vrrpTable=vrrpTable, cacheHitRatio=cacheHitRatio, vsQosCookieHits=vsQosCookieHits, ipPktsOut=ipPktsOut, ctcpOutRsts=ctcpOutRsts, realAddr=realAddr, vsRedirectHits=vsRedirectHits, hostName=hostName, vsHeaderHits=vsHeaderHits, hcCause=hcCause, caSyslogTrap=caSyslogTrap, rcvdBytesPerSec=rcvdBytesPerSec, clusterVirIndex=clusterVirIndex, cputemp=cputemp, purgeRequests=purgeRequests, rsTable=rsTable, ctcpInErrs=ctcpInErrs, rsProtocol=rsProtocol, responsesOnRegex=responsesOnRegex, infInMcastPkts=infInMcastPkts, sysMemory=sysMemory, logMaxSeverity=logMaxSeverity, infInDiscards=infInDiscards, ctcpConnRemAddress=ctcpConnRemAddress, gpIndex=gpIndex, requestsPerSec=requestsPerSec, cacheSkip=cacheSkip, hcConnSuccess=hcConnSuccess, postRequests=postRequests)
