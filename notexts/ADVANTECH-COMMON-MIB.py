...
# ASN.1 source ADVANTECH-COMMON-MIB
# Source digest sha256:172fa12af45b0b9dc0b28d838fd1521d3659061f33cba0ea447da2bd3ca5f057
...
if mibBuilder.loadTexts: advantechCommonMIB.setLastUpdated('2015-01-06 00:00')
...
snmpTrapSrvTable = MibTable((1, 3, 6, 1, 4, 1, 10297, 100, 2, 1, 2), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: snmpTrapSrvTable.setStatus('mandatory')
snmpTrapSrvEntry = MibTableRow((1, 3, 6, 1, 4, 1, 10297, 100, 2, 1, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "ADVANTECH-COMMON-MIB", "snmpTrapSrvIndex"))
...
pciSlotTable = MibTable((1, 3, 6, 1, 4, 1, 10297, 100, 3, 1, 2), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: pciSlotTable.setStatus('current')
pciSlotEntry = MibTableRow((1, 3, 6, 1, 4, 1, 10297, 100, 3, 1, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "ADVANTECH-COMMON-MIB", "psIndex"))
...
mibBuilder.exportSymbols("ADVANTECH-COMMON-MIB", PYSNMP_MODULE_ID=advantechCommonMIB, advantech=advantech, advantechCommonMIB=advantechCommonMIB, atMgmt=atMgmt, atPciConfig=atPciConfig, atSystem=atSystem, pciConfigObj=pciConfigObj, pciSlotEntry=pciSlotEntry, pciSlotTable=pciSlotTable, psBaseAddress=psBaseAddress, psBusIndex=psBusIndex, psClassCode=psClassCode, psDescr=psDescr, psDeviceID=psDeviceID, psDeviceIndex=psDeviceIndex, psDisplayName=psDisplayName, psFunctionIndex=psFunctionIndex, psIRQ=psIRQ, psIndex=psIndex, psLength=psLength, psLocation=psLocation, psManufacturer=psManufacturer, psModulePorts=psModulePorts, psModuleType=psModuleType, psNumber=psNumber, psState=psState, psSubsysDeviceID=psSubsysDeviceID, psSubsysVendorID=psSubsysVendorID, psVendorID=psVendorID, snmpTrapSrvAuthentication=snmpTrapSrvAuthentication, snmpTrapSrvCommunity=snmpTrapSrvCommunity, snmpTrapSrvEntry=snmpTrapSrvEntry, snmpTrapSrvIP=snmpTrapSrvIP, snmpTrapSrvIndex=snmpTrapSrvIndex, snmpTrapSrvNumber=snmpTrapSrvNumber, snmpTrapSrvObj=snmpTrapSrvObj, snmpTrapSrvPort=snmpTrapSrvPort, snmpTrapSrvTable=snmpTrapSrvTable, snmpTrapVersion=snmpTrapVersion, sysBootCount=sysBootCount, sysBootTime=sysBootTime, sysDescr=sysDescr, sysDeviceName=sysDeviceName, sysFirstBootTime=sysFirstBootTime, sysImageVersion=sysImageVersion, sysModuleID=sysModuleID, sysReleaseDate=sysReleaseDate)
