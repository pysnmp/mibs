...
# Produced by pysmi-1.1.8 at Fri Jan  7 17:32:46 2022
...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spTMSPrefix=spTMSPrefix, deviceSwapSpace=deviceSwapSpace, spCollector=spCollector, spThirdPartyAddr=spThirdPartyAddr, mitigationThirdPartyStart=mitigationThirdPartyStart, licenseError=licenseError, spBGPTrapEvent=spBGPTrapEvent, fingerprintReceive=fingerprintReceive, spDNSObserved=spDNSObserved, spBlackholePrefix=spBlackholePrefix, spServiceName=spServiceName, deviceDiskUsage=deviceDiskUsage, fingerprintFeedback=fingerprintFeedback, managedObjectUsageDone=managedObjectUsageDone, spFlowspecCommunity=spFlowspecCommunity, serviceUsage=serviceUsage, spServiceElement=spServiceElement, deviceTotalFlows=deviceTotalFlows, interfaceUsageDone=interfaceUsageDone, peakflowSPMgr=peakflowSPMgr, routingFailover=routingFailover, spInetAddress=spInetAddress, spLicenseErrType=spLicenseErrType, peakflowSPCMI=peakflowSPCMI, spInterfaceSpeedHC=spInterfaceSpeedHC, cloudSignalingFaultDone=cloudSignalingFaultDone, serviceUsageDone=serviceUsageDone, spDetector=spDetector, notificationLimit=notificationLimit, collectorStart=collectorStart, spPravail=spPravail, spGreTunnelDestination=spGreTunnelDestination, spLicenseErrDescription=spLicenseErrDescription, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBGPTrapPrefix=spBGPTrapPrefix, spMitigationStart=spMitigationStart, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, dosHostDetectionAlert=dosHostDetectionAlert, hardwareFailure=hardwareFailure, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spFingerprintSender=spFingerprintSender, spRouter=spRouter, spBlackholeTimeout=spBlackholeTimeout, spFingerprintMessage=spFingerprintMessage, spHijackLocal=spHijackLocal, mitigationBlackholeStart=mitigationBlackholeStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, fingerprintUsageDone=fingerprintUsageDone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, licenseErrorDone=licenseErrorDone, spUsage=spUsage, peakflowSPMIB=peakflowSPMIB, fingerprintUsage=fingerprintUsage, spCommFailureDescription=spCommFailureDescription, spFingerprintName=spFingerprintName, devicePhysicalMemory=devicePhysicalMemory, greDownDone=greDownDone, spGreTunnelName=spGreTunnelName, deviceTotalFlowsHC=deviceTotalFlowsHC, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, trafficAutoMitigation=trafficAutoMitigation, collectorDown=collectorDown, hardwareFailureDone=hardwareFailureDone, autoclassifyStart=autoclassifyStart, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spDNSExpected=spDNSExpected, spBGPTrapName=spBGPTrapName, bgpInstabilityDone=bgpInstabilityDone, spImpactPps=spImpactPps, spInetAddressType=spInetAddressType, spInterface=spInterface, spMitigationName=spMitigationName, spImpactBps=spImpactBps, flowUp=flowUp, spDNSName=spDNSName, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, collectorUp=collectorUp, bgpDown=bgpDown, spThreshold=spThreshold, spUsageType=spUsageType, bgpTrap=bgpTrap, spSystemErrorType=spSystemErrorType, spLicenseErrLimit=spLicenseErrLimit, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spReportName=spReportName, mitigationFlowspecStart=mitigationFlowspecStart, spMitigationID=spMitigationID, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceSystemError=deviceSystemError, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, cloudSignalingFault=cloudSignalingFault, spInterfaceSpeed=spInterfaceSpeed, spDuration=spDuration, spBlackholeCommunity=spBlackholeCommunity, interfaceUsage=interfaceUsage, spInterfaceIndex=spInterfaceIndex, snmpDown=snmpDown, spBGPInstability=spBGPInstability, spThirdPartyZone=spThirdPartyZone, spcommFailure=spcommFailure, spScriptCommand=spScriptCommand, spAlertID=spAlertID, spInterfaceUsage=spInterfaceUsage, bgpUp=bgpUp, spScriptStart=spScriptStart, spReportID=spReportID, greDown=greDown, spCommFailureDestination=spCommFailureDestination, spReportURL=spReportURL, spScriptHost=spScriptHost, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, peakflowSPTraps=peakflowSPTraps, spVersion=spVersion, spManagedObjects=spManagedObjects, reportDone=reportDone, spDetectedCountries=spDetectedCountries, spBGPEvent=spBGPEvent, spHijackAttr=spHijackAttr, spHardwareFailureDescription=spHardwareFailureDescription, spManagedObject=spManagedObject, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBlackholeNexthop=spBlackholeNexthop, mitigationTMSStartV2=mitigationTMSStartV2, spInterfaceUsageHC=spInterfaceUsageHC, spUsername=spUsername, spApplicationName=spApplicationName, alertScript=alertScript, spLicenseErrCount=spLicenseErrCount, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceHealth=deviceHealth, spHijackRoute=spHijackRoute, mitigationTMSStart=mitigationTMSStart, spFlowspecTimeout=spFlowspecTimeout, spSystemErrorDescription=spSystemErrorDescription, dnsBaseline=dnsBaseline, spFingerprintFeedback=spFingerprintFeedback, mitigationDone=mitigationDone, configChange=configChange, spManagedObjectFamily=spManagedObjectFamily, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceSystemErrorDone=deviceSystemErrorDone, spTMSMultiPrefix=spTMSMultiPrefix, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, bgpInstability=bgpInstability, bgpHijack=bgpHijack, spUnit=spUnit, flowDown=flowDown, spScriptPort=spScriptPort, snmpUp=snmpUp, spDNSObservedMax=spDNSObservedMax, managedObjectUsage=managedObjectUsage, spDNSObservedMean=spDNSObservedMean, PYSNMP_MODULE_ID=peakflowSPMIB, spTMSTimeout=spTMSTimeout, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dnsBaselineDone=dnsBaselineDone, spcommFailureDone=spcommFailureDone, spAlertDetectionSignatures=spAlertDetectionSignatures, spTMSCommunity=spTMSCommunity)
