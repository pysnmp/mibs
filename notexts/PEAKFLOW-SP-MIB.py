...
# Produced by pysmi-1.1.3 at Wed Dec  8 19:07:00 2021
...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceSpeed=spInterfaceSpeed, spHijackRoute=spHijackRoute, spTMSPrefix=spTMSPrefix, spReportName=spReportName, spScriptPort=spScriptPort, spInterfaceUsageHC=spInterfaceUsageHC, mitigationTMSStart=mitigationTMSStart, PYSNMP_MODULE_ID=peakflowSPMIB, spUnit=spUnit, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spBGPTrapName=spBGPTrapName, bgpInstabilityDone=bgpInstabilityDone, spFingerprintName=spFingerprintName, spHardwareFailureDescription=spHardwareFailureDescription, spFlowspecCommunity=spFlowspecCommunity, spReportURL=spReportURL, spReportID=spReportID, managedObjectUsageDone=managedObjectUsageDone, spBlackholeTimeout=spBlackholeTimeout, hardwareFailureDone=hardwareFailureDone, spMitigationID=spMitigationID, spcommFailureDone=spcommFailureDone, configChange=configChange, spManagedObject=spManagedObject, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spManagedObjects=spManagedObjects, spInterface=spInterface, spAlertID=spAlertID, reportDone=reportDone, mitigationDone=mitigationDone, deviceDiskUsage=deviceDiskUsage, serviceUsage=serviceUsage, spInterfaceUsage=spInterfaceUsage, spServiceName=spServiceName, spTMSMultiPrefix=spTMSMultiPrefix, spVersion=spVersion, spMitigationStart=spMitigationStart, dosHostDetectionAlert=dosHostDetectionAlert, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spThreshold=spThreshold, spSystemErrorType=spSystemErrorType, spFingerprintSender=spFingerprintSender, spCommFailureDescription=spCommFailureDescription, spInetAddress=spInetAddress, greDown=greDown, mitigationTMSStartV2=mitigationTMSStartV2, flowDown=flowDown, spDetector=spDetector, dnsBaselineDone=dnsBaselineDone, spGreTunnelName=spGreTunnelName, collectorDown=collectorDown, spImpactBps=spImpactBps, spUsage=spUsage, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spScriptCommand=spScriptCommand, spCollector=spCollector, deviceHealth=deviceHealth, spDuration=spDuration, serviceUsageDone=serviceUsageDone, spThirdPartyZone=spThirdPartyZone, managedObjectUsage=managedObjectUsage, spSystemErrorDescription=spSystemErrorDescription, spManagedObjectFamily=spManagedObjectFamily, spScriptHost=spScriptHost, spBlackholeCommunity=spBlackholeCommunity, spBGPTrapPrefix=spBGPTrapPrefix, snmpDown=snmpDown, spUsername=spUsername, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSExpected=spDNSExpected, spDNSName=spDNSName, spRouter=spRouter, spBGPTrapEvent=spBGPTrapEvent, spTMSCommunity=spTMSCommunity, spCommFailureDestination=spCommFailureDestination, spLicenseErrType=spLicenseErrType, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spcommFailure=spcommFailure, bgpTrap=bgpTrap, spDetectedCountries=spDetectedCountries, spUsageType=spUsageType, bgpHijack=bgpHijack, spDNSObserved=spDNSObserved, flowUp=flowUp, peakflowSPTraps=peakflowSPTraps, peakflowSPMIB=peakflowSPMIB, spAlertDetectionSignatures=spAlertDetectionSignatures, notificationLimit=notificationLimit, mitigationFlowspecStart=mitigationFlowspecStart, greDownDone=greDownDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceTotalFlows=deviceTotalFlows, spInterfaceIndex=spInterfaceIndex, spScriptStart=spScriptStart, deviceSwapSpace=deviceSwapSpace, licenseErrorDone=licenseErrorDone, spBGPEvent=spBGPEvent, mitigationBlackholeStart=mitigationBlackholeStart, spTMSTimeout=spTMSTimeout, spInetAddressType=spInetAddressType, deviceSystemError=deviceSystemError, fingerprintReceive=fingerprintReceive, spThirdPartyAddr=spThirdPartyAddr, dnsBaseline=dnsBaseline, spGreTunnelDestination=spGreTunnelDestination, spApplicationName=spApplicationName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, fingerprintUsageDone=fingerprintUsageDone, cloudSignalingFault=cloudSignalingFault, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, interfaceUsage=interfaceUsage, spLicenseErrDescription=spLicenseErrDescription, bgpDown=bgpDown, devicePhysicalMemory=devicePhysicalMemory, spDNSObservedMean=spDNSObservedMean, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spDNSObservedMax=spDNSObservedMax, snmpUp=snmpUp, dosNetworkProfiledAlert=dosNetworkProfiledAlert, hardwareFailure=hardwareFailure, spPravail=spPravail, spImpactPps=spImpactPps, deviceSwapSpaceUsage=deviceSwapSpaceUsage, licenseError=licenseError, alertScript=alertScript, fingerprintFeedback=fingerprintFeedback, spFlowspecTimeout=spFlowspecTimeout, spLicenseErrCount=spLicenseErrCount, fingerprintUsage=fingerprintUsage, interfaceUsageDone=interfaceUsageDone, spFingerprintFeedback=spFingerprintFeedback, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spHijackAttr=spHijackAttr, spBlackholeNexthop=spBlackholeNexthop, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpUp=bgpUp, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBlackholePrefix=spBlackholePrefix, cloudSignalingFaultDone=cloudSignalingFaultDone, spBGPInstability=spBGPInstability, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spHijackLocal=spHijackLocal, bgpInstability=bgpInstability, spFingerprintMessage=spFingerprintMessage, collectorStart=collectorStart, deviceSystemErrorDone=deviceSystemErrorDone, spLicenseErrLimit=spLicenseErrLimit, collectorUp=collectorUp, peakflowSPMgr=peakflowSPMgr, spBGPTrapOldAttributes=spBGPTrapOldAttributes, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spServiceElement=spServiceElement, routingFailover=routingFailover, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, peakflowSPCMI=peakflowSPCMI, spMitigationName=spMitigationName, trafficAutoMitigation=trafficAutoMitigation, autoclassifyStart=autoclassifyStart, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, mitigationThirdPartyStart=mitigationThirdPartyStart)
