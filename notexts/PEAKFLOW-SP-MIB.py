...
# Produced by pysmi-1.1.8 at Thu Feb  9 11:56:23 2023
...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDuration=spDuration, spBGPTrapName=spBGPTrapName, peakflowSPCMI=peakflowSPCMI, spBGPTrapEvent=spBGPTrapEvent, spManagedObject=spManagedObject, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, routingFailover=routingFailover, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, greDown=greDown, spPravail=spPravail, spMitigationID=spMitigationID, spApplicationName=spApplicationName, deviceTotalFlowsHC=deviceTotalFlowsHC, deviceSystemErrorDone=deviceSystemErrorDone, spScriptStart=spScriptStart, deviceSystemError=deviceSystemError, dnsBaseline=dnsBaseline, spLicenseErrLimit=spLicenseErrLimit, spReportName=spReportName, autoclassifyStart=autoclassifyStart, peakflowSPMgr=peakflowSPMgr, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, bgpInstabilityDone=bgpInstabilityDone, spBGPInstability=spBGPInstability, bgpUp=bgpUp, spTMSCommunity=spTMSCommunity, spSystemErrorType=spSystemErrorType, serviceUsage=serviceUsage, spLicenseErrDescription=spLicenseErrDescription, spBlackholeTimeout=spBlackholeTimeout, spDetectedCountries=spDetectedCountries, deviceDiskUsage=deviceDiskUsage, spBlackholeCommunity=spBlackholeCommunity, bgpHijack=bgpHijack, fingerprintFeedback=fingerprintFeedback, fingerprintReceive=fingerprintReceive, spHijackRoute=spHijackRoute, serviceUsageDone=serviceUsageDone, mitigationBlackholeStart=mitigationBlackholeStart, spGreTunnelDestination=spGreTunnelDestination, PYSNMP_MODULE_ID=peakflowSPMIB, spBGPEvent=spBGPEvent, spBGPTrapPrefix=spBGPTrapPrefix, spInetAddress=spInetAddress, spTMSPrefix=spTMSPrefix, dnsBaselineDone=dnsBaselineDone, hardwareFailure=hardwareFailure, spSystemErrorDescription=spSystemErrorDescription, collectorUp=collectorUp, bgpTrap=bgpTrap, trafficAutoMitigation=trafficAutoMitigation, notificationLimit=notificationLimit, spScriptHost=spScriptHost, spTMSTimeout=spTMSTimeout, spFingerprintSender=spFingerprintSender, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spReportID=spReportID, hardwareFailureDone=hardwareFailureDone, spDNSObservedMax=spDNSObservedMax, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spDNSObservedMean=spDNSObservedMean, spInterfaceUsageHC=spInterfaceUsageHC, fingerprintUsage=fingerprintUsage, spHardwareFailureDescription=spHardwareFailureDescription, spFlowspecCommunity=spFlowspecCommunity, dosHostDetectionAlert=dosHostDetectionAlert, spInterfaceUsage=spInterfaceUsage, spCollector=spCollector, spBlackholePrefix=spBlackholePrefix, spImpactBps=spImpactBps, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spFingerprintMessage=spFingerprintMessage, spDNSExpected=spDNSExpected, spThreshold=spThreshold, bgpDown=bgpDown, spLicenseErrType=spLicenseErrType, mitigationFlowspecStart=mitigationFlowspecStart, bgpInstability=bgpInstability, spServiceName=spServiceName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spcommFailure=spcommFailure, managedObjectUsage=managedObjectUsage, devicePhysicalMemory=devicePhysicalMemory, spInterfaceSpeed=spInterfaceSpeed, flowUp=flowUp, spGreTunnelName=spGreTunnelName, spMitigationStart=spMitigationStart, deviceHealth=deviceHealth, spScriptPort=spScriptPort, spCommFailureDestination=spCommFailureDestination, deviceTotalFlows=deviceTotalFlows, mitigationThirdPartyStart=mitigationThirdPartyStart, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, interfaceUsage=interfaceUsage, fingerprintUsageDone=fingerprintUsageDone, mitigationDone=mitigationDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spLicenseErrCount=spLicenseErrCount, greDownDone=greDownDone, spFingerprintFeedback=spFingerprintFeedback, licenseError=licenseError, spManagedObjectFamily=spManagedObjectFamily, cloudSignalingFaultDone=cloudSignalingFaultDone, spInterfaceSpeedHC=spInterfaceSpeedHC, reportDone=reportDone, spUnit=spUnit, flowDown=flowDown, spUsage=spUsage, spThirdPartyAddr=spThirdPartyAddr, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, peakflowSPTraps=peakflowSPTraps, spFlowspecTimeout=spFlowspecTimeout, spManagedObjects=spManagedObjects, spHijackAttr=spHijackAttr, spInetAddressType=spInetAddressType, spThirdPartyZone=spThirdPartyZone, spServiceElement=spServiceElement, spRouter=spRouter, spImpactPps=spImpactPps, deviceSwapSpace=deviceSwapSpace, spReportURL=spReportURL, snmpUp=snmpUp, interfaceUsageDone=interfaceUsageDone, spScriptCommand=spScriptCommand, snmpDown=snmpDown, alertScript=alertScript, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spDetector=spDetector, peakflowSPMIB=peakflowSPMIB, mitigationTMSStartV2=mitigationTMSStartV2, spUsageType=spUsageType, licenseErrorDone=licenseErrorDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInterface=spInterface, spDNSName=spDNSName, managedObjectUsageDone=managedObjectUsageDone, collectorStart=collectorStart, spcommFailureDone=spcommFailureDone, spMitigationName=spMitigationName, spVersion=spVersion, collectorDown=collectorDown, spUsername=spUsername, spInterfaceIndex=spInterfaceIndex, cloudSignalingFault=cloudSignalingFault, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spDNSObserved=spDNSObserved, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spTMSMultiPrefix=spTMSMultiPrefix, spAlertDetectionSignatures=spAlertDetectionSignatures, configChange=configChange, spHijackLocal=spHijackLocal, spFingerprintName=spFingerprintName, mitigationTMSStart=mitigationTMSStart, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spCommFailureDescription=spCommFailureDescription, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spAlertID=spAlertID, spBlackholeNexthop=spBlackholeNexthop)
