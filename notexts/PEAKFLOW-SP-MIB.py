...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBlackholePrefix=spBlackholePrefix, spUsageType=spUsageType, spGreTunnelDestination=spGreTunnelDestination, spSystemErrorDescription=spSystemErrorDescription, spLicenseErrLimit=spLicenseErrLimit, alertScript=alertScript, mitigationThirdPartyStart=mitigationThirdPartyStart, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spTMSCommunity=spTMSCommunity, managedObjectUsage=managedObjectUsage, spCommFailureDestination=spCommFailureDestination, spManagedObjectFamily=spManagedObjectFamily, hardwareFailureDone=hardwareFailureDone, spUsername=spUsername, fingerprintFeedback=fingerprintFeedback, mitigationTMSStart=mitigationTMSStart, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spDuration=spDuration, spDNSObservedMax=spDNSObservedMax, spDNSObserved=spDNSObserved, spPravail=spPravail, spHijackRoute=spHijackRoute, spFingerprintFeedback=spFingerprintFeedback, spCloudSignalFaultDescription=spCloudSignalFaultDescription, collectorUp=collectorUp, spMitigationStart=spMitigationStart, spInetAddressType=spInetAddressType, interfaceUsage=interfaceUsage, spLicenseErrType=spLicenseErrType, spRouter=spRouter, spReportName=spReportName, spMitigationID=spMitigationID, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, routingFailover=routingFailover, deviceDiskUsage=deviceDiskUsage, mitigationDone=mitigationDone, licenseErrorDone=licenseErrorDone, spBGPInstability=spBGPInstability, spFingerprintName=spFingerprintName, deviceSystemErrorDone=deviceSystemErrorDone, spInterfaceIndex=spInterfaceIndex, spcommFailure=spcommFailure, spThirdPartyZone=spThirdPartyZone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, configChange=configChange, spManagedObjects=spManagedObjects, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spReportID=spReportID, hardwareFailure=hardwareFailure, spFlowspecTimeout=spFlowspecTimeout, spScriptCommand=spScriptCommand, spImpactPps=spImpactPps, spMitigationName=spMitigationName, deviceTotalFlowsHC=deviceTotalFlowsHC, spTMSTimeout=spTMSTimeout, dnsBaselineDone=dnsBaselineDone, spScriptPort=spScriptPort, bgpInstability=bgpInstability, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spDetector=spDetector, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceTotalFlows=deviceTotalFlows, spThreshold=spThreshold, cloudSignalingFault=cloudSignalingFault, collectorStart=collectorStart, peakflowSPTraps=peakflowSPTraps, flowDown=flowDown, trafficAutoMitigation=trafficAutoMitigation, spUsage=spUsage, bgpInstabilityDone=bgpInstabilityDone, serviceUsageDone=serviceUsageDone, greDown=greDown, PYSNMP_MODULE_ID=peakflowSPMIB, spReportURL=spReportURL, spVersion=spVersion, spHardwareFailureDescription=spHardwareFailureDescription, peakflowSPMIB=peakflowSPMIB, spInterfaceUsageHC=spInterfaceUsageHC, spBGPTrapEvent=spBGPTrapEvent, spHijackLocal=spHijackLocal, spInterfaceSpeedHC=spInterfaceSpeedHC, spTMSPrefix=spTMSPrefix, spFlowspecCommunity=spFlowspecCommunity, interfaceUsageDone=interfaceUsageDone, cloudSignalingFaultDone=cloudSignalingFaultDone, collectorDown=collectorDown, snmpDown=snmpDown, spThirdPartyAddr=spThirdPartyAddr, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spInterfaceUsage=spInterfaceUsage, spDNSExpected=spDNSExpected, reportDone=reportDone, fingerprintUsageDone=fingerprintUsageDone, spCollector=spCollector, spCommFailureDescription=spCommFailureDescription, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, mitigationFlowspecStart=mitigationFlowspecStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSMultiPrefix=spTMSMultiPrefix, bgpDown=bgpDown, spHijackAttr=spHijackAttr, spServiceElement=spServiceElement, snmpUp=snmpUp, fingerprintReceive=fingerprintReceive, deviceSwapSpaceUsage=deviceSwapSpaceUsage, bgpTrap=bgpTrap, deviceHealth=deviceHealth, mitigationBlackholeStart=mitigationBlackholeStart, spScriptHost=spScriptHost, managedObjectUsageDone=managedObjectUsageDone, spGreTunnelName=spGreTunnelName, mitigationTMSStartV2=mitigationTMSStartV2, spApplicationName=spApplicationName, spSystemErrorType=spSystemErrorType, flowUp=flowUp, spServiceName=spServiceName, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spInetAddress=spInetAddress, spBlackholeCommunity=spBlackholeCommunity, spDetectedCountries=spDetectedCountries, dnsBaseline=dnsBaseline, deviceSwapSpace=deviceSwapSpace, spScriptStart=spScriptStart, spManagedObject=spManagedObject, spBlackholeTimeout=spBlackholeTimeout, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spLicenseErrDescription=spLicenseErrDescription, peakflowSPCMI=peakflowSPCMI, greDownDone=greDownDone, spDNSName=spDNSName, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, serviceUsage=serviceUsage, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, fingerprintUsage=fingerprintUsage, spInterfaceSpeed=spInterfaceSpeed, spDNSObservedMean=spDNSObservedMean, autoclassifyStart=autoclassifyStart, peakflowSPMgr=peakflowSPMgr, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInterface=spInterface, spBGPTrapName=spBGPTrapName, spLicenseErrCount=spLicenseErrCount, dosHostDetectionAlert=dosHostDetectionAlert, bgpHijack=bgpHijack, notificationLimit=notificationLimit, spAlertID=spAlertID, spUnit=spUnit, spBGPTrapPrefix=spBGPTrapPrefix, licenseError=licenseError, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spFingerprintMessage=spFingerprintMessage, spBlackholeNexthop=spBlackholeNexthop, deviceSystemError=deviceSystemError, spFingerprintSender=spFingerprintSender, spImpactBps=spImpactBps, spcommFailureDone=spcommFailureDone, bgpUp=bgpUp, spBGPEvent=spBGPEvent, devicePhysicalMemory=devicePhysicalMemory)
