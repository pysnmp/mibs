...
Integer32, Bits, ObjectIdentity, Counter32, Counter64, Gauge32, MibIdentifier, TimeTicks, Unsigned32, NotificationType, ModuleIdentity, IpAddress, iso, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Bits", "ObjectIdentity", "Counter32", "Counter64", "Gauge32", "MibIdentifier", "TimeTicks", "Unsigned32", "NotificationType", "ModuleIdentity", "IpAddress", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("MPIOS-MIB", sysRtrEntityMgt=sysRtrEntityMgt, taskMain=taskMain, rtrFlStaInputMaxPkts=rtrFlStaInputMaxPkts, sysRtrJitterTable=sysRtrJitterTable, sysIfIndex=sysIfIndex, sysRtrEntityTable=sysRtrEntityTable, sysRtrGroupMgt=sysRtrGroupMgt, rtrJitterState=rtrJitterState, rtrJitterMaxRtt=rtrJitterMaxRtt, rtrUdpechoPktSucc=rtrUdpechoPktSucc, rtrFlStaTmOutputMaxFlow=rtrFlStaTmOutputMaxFlow, numBlocksAlloc=numBlocksAlloc, rtrFlStatisticsInterval=rtrFlStatisticsInterval, rtrUdpechoDelayMax=rtrUdpechoDelayMax, rtrFlStaOutputMinPkts=rtrFlStaOutputMinPkts, sysCpuStatus=sysCpuStatus, rtrUdpechoPktLoss=rtrUdpechoPktLoss, cpuTaskName=cpuTaskName, sysIfDesc=sysIfDesc, sysIfInOctets=sysIfInOctets, rtrIcmpEchoPktNum=rtrIcmpEchoPktNum, rtrJitterDelaySdMax=rtrJitterDelaySdMax, rtrJitterPktNum=rtrJitterPktNum, cpuTaskDeltaTicks=cpuTaskDeltaTicks, cpuTaskTotalTicks=cpuTaskTotalTicks, rtrEntityLogType=rtrEntityLogType, taskStackSize=taskStackSize, rtrJitterDelaySdMin=rtrJitterDelaySdMin, EnabledStatus=EnabledStatus, sysRtrGroupTable=sysRtrGroupTable, rtrUdpechoTos=rtrUdpechoTos, memoryTotalBytes=memoryTotalBytes, taskId=taskId, cpuUtilTotalUsedTicks=cpuUtilTotalUsedTicks, cpuTaskDeltaUtil=cpuTaskDeltaUtil, sysCpuAlertTemper=sysCpuAlertTemper, sysRtrFlowStatisticsEntry=sysRtrFlowStatisticsEntry, sysIfPktPriStatistics=sysIfPktPriStatistics, rtrFlStaTmInputMaxPkts=rtrFlStaTmInputMaxPkts, rtrEntityLogMaxSize=rtrEntityLogMaxSize, rtrUdpechoDelayMin=rtrUdpechoDelayMin, taskStackMaxUsed=taskStackMaxUsed, sysIfInNUcastPkts=sysIfInNUcastPkts, rtrJitterSchInterval=rtrJitterSchInterval, rtrJitterPktLossSd=rtrJitterPktLossSd, rtrEntityId=rtrEntityId, rtrJitterMosMax=rtrJitterMosMax, sysNFI=sysNFI, rtrFlStaTmOutputMinPkts=rtrFlStaTmOutputMinPkts, sysIfInErrors=sysIfInErrors, rtrFlStaTmInputMinPkts=rtrFlStaTmInputMinPkts, rtrEntityRowStatus=rtrEntityRowStatus, maxBlockSizeFree=maxBlockSizeFree, taskStackEnd=taskStackEnd, taskOptions=taskOptions, rtrJitterPktLossDs=rtrJitterPktLossDs, rtrEntityName=rtrEntityName, rtrIcmpEchoIsScheduling=rtrIcmpEchoIsScheduling, rtrIcmpEchoTos=rtrIcmpEchoTos, rtrFlStaTmInputMinFlow=rtrFlStaTmInputMinFlow, taskErrorStatus=taskErrorStatus, sysRtrIcmpEchoMgt=sysRtrIcmpEchoMgt, sysIfInDiscards=sysIfInDiscards, rtrJitterCodec=rtrJitterCodec, rtrFlStatisticsEntityId=rtrFlStatisticsEntityId, avgBlockSizeAlloc=avgBlockSizeAlloc, rtrJitterTos=rtrJitterTos, rtrJitterSourcePort=rtrJitterSourcePort, allocBytesPercent=allocBytesPercent, checkCpuTimeInterval=checkCpuTimeInterval, cpuTaskId=cpuTaskId, rtrFlowStatisticsRowStatus=rtrFlowStatisticsRowStatus, sysRtrJitterMgt=sysRtrJitterMgt, sysRtrResponder=sysRtrResponder, sysIfOutDiscards=sysIfOutDiscards, rtrUdpechoState=rtrUdpechoState, taskEntry=taskEntry, rtrUdpechoDelay=rtrUdpechoDelay, iosSystem=iosSystem, sysTemperature=sysTemperature, rtrIcmpEchoSourceIp=rtrIcmpEchoSourceIp, rtrScheduleType=rtrScheduleType, cpuUtilEntry=cpuUtilEntry, sysMemory=sysMemory, rtrJitterDelayDsMax=rtrJitterDelayDsMax, rtrIcmpEchoPktTotalRcvd=rtrIcmpEchoPktTotalRcvd, rtrScheduleStartTimeFlag=rtrScheduleStartTimeFlag, rtrJitterPktLen=rtrJitterPktLen, rtrFlStaTmOutputMinFlow=rtrFlStaTmOutputMinFlow, sysIfOutOctets=sysIfOutOctets, sysIfStatistic=sysIfStatistic, PYSNMP_MODULE_ID=mpios, sysIfInUnkownProtos=sysIfInUnkownProtos, rtrJitterMinRtt=rtrJitterMinRtt, cpuTaskEntry=cpuTaskEntry, cpuTaskCurrentUtil=cpuTaskCurrentUtil, rtrFlStaInputMinPkts=rtrFlStaInputMinPkts, cpuUtilTotalTicks=cpuUtilTotalTicks, cpuTaskAverageUtil=cpuTaskAverageUtil, sysMainBoardTemper=sysMainBoardTemper, sysRtrUdpechoMgt=sysRtrUdpechoMgt, rtrJitterSourceIp=rtrJitterSourceIp, sysIfOutErrors=sysIfOutErrors, rtrIcmpEchoTargetIp=rtrIcmpEchoTargetIp, numBytesAlloc=numBytesAlloc, sysRtrGbl=sysRtrGbl, taskStackBase=taskStackBase, taskName=taskName, rtrIcmpEchoRowStatus=rtrIcmpEchoRowStatus, rtrScheduleRepeat=rtrScheduleRepeat, sysRtrFlowStatisticsMgt=sysRtrFlowStatisticsMgt, rtrIcmpEchoVrfName=rtrIcmpEchoVrfName, cpuTaskTable=cpuTaskTable, taskPriority=taskPriority, mpios=mpios, rtrFlStaTmOutputMaxPkts=rtrFlStaTmOutputMaxPkts, rtrIcmpEchoPktLen=rtrIcmpEchoPktLen, taskStackPos=taskStackPos, rtrUdpechoEntityId=rtrUdpechoEntityId, rtrScheduleLifeTime=rtrScheduleLifeTime, rtrJitterSdMin=rtrJitterSdMin, rtrUdpechoSchInterval=rtrUdpechoSchInterval, iosObjects=iosObjects, rtrEntityType=rtrEntityType, rtrScheduleRowStatus=rtrScheduleRowStatus, rtrIcmpEchoVerifyData=rtrIcmpEchoVerifyData, cpuUtilDeltaTimes=cpuUtilDeltaTimes, rtrUdpechoTimeout=rtrUdpechoTimeout, cpuUtilDeltaUtil=cpuUtilDeltaUtil, rtrIcmpEchoExtendFlag=rtrIcmpEchoExtendFlag, rtrFlStaInputMinFlow=rtrFlStaInputMinFlow, rtrJitterEntityId=rtrJitterEntityId, rtrJitterPktInterval=rtrJitterPktInterval, sysRtrIcmpEchoEntry=sysRtrIcmpEchoEntry, rtrJitterDelayDsMin=rtrJitterDelayDsMin, taskStackSizeUsage=taskStackSizeUsage, rtrUdpechoSourcePort=rtrUdpechoSourcePort, sysRtrUdpechoTable=sysRtrUdpechoTable, sysRtrScheduleTable=sysRtrScheduleTable, rtrJitterDsMin=rtrJitterDsMin, cpuUtilTotalTimes=cpuUtilTotalTimes, sysRtrCtrl=sysRtrCtrl, rtrUdpechoVrfName=rtrUdpechoVrfName, rtrJitterTimeout=rtrJitterTimeout, sysIfPktPriStaTable=sysIfPktPriStaTable, rtrGroupInterval=rtrGroupInterval, rtrScheduleStartTime=rtrScheduleStartTime, rtrScheduleInterval=rtrScheduleInterval, rtrUdpechoSourceIp=rtrUdpechoSourceIp, rtrIcmpEchoTimeout=rtrIcmpEchoTimeout, cpuUtilTable=cpuUtilTable, sysRtrIcmpEchoTable=sysRtrIcmpEchoTable, rtrIcmpEchoSetDf=rtrIcmpEchoSetDf, rtrFlStaInputMaxFlow=rtrFlStaInputMaxFlow, sysCpuTaskTabView=sysCpuTaskTabView, cpuUtilCurrentUtil=cpuUtilCurrentUtil, rtrUdpechoTargetPort=rtrUdpechoTargetPort, cpuUtilDeltaUsedTicks=cpuUtilDeltaUsedTicks, rtrJitterSdMax=rtrJitterSdMax, rtrIcmpEchoSchInterval=rtrIcmpEchoSchInterval, cpuUtilCpuId=cpuUtilCpuId, rtrUdpechoTargetIp=rtrUdpechoTargetIp, rtrJitterIcpifMax=rtrJitterIcpifMax, cpuUtilDeltaTicks=cpuUtilDeltaTicks, rtrIcmpEchoPktTotalSend=rtrIcmpEchoPktTotalSend, rtrFlStatisticsIfName=rtrFlStatisticsIfName, sysCpuTemper=sysCpuTemper, rtrFlStaTmInputMaxFlow=rtrFlStaTmInputMaxFlow, rtrScheduleId=rtrScheduleId, rtrEntityLogFilter=rtrEntityLogFilter, sysRtrScheduleMgt=sysRtrScheduleMgt, rtrUdpechoRowStatus=rtrUdpechoRowStatus, rtrJitterTargetIp=rtrJitterTargetIp, rtrScheduleAfterTime=rtrScheduleAfterTime, rtrUdpechoPktLen=rtrUdpechoPktLen, rtrIcmpEchoEntityId=rtrIcmpEchoEntityId, sysRtrUdpechoEntry=sysRtrUdpechoEntry, taskStackPtr=taskStackPtr, rtrEntityThreshold=rtrEntityThreshold, taskStatus=taskStatus, rtrScheduleLifeFlag=rtrScheduleLifeFlag, rtrFlStaOutputMinFlow=rtrFlStaOutputMinFlow, sysMainBoardAlertTemper=sysMainBoardAlertTemper, rtrGroupRowStatus=rtrGroupRowStatus, rtrJitterVrfName=rtrJitterVrfName, taskStackFree=taskStackFree, rtrGroupEntityMembers=rtrGroupEntityMembers, sysRtrFlowStatisticsTable=sysRtrFlowStatisticsTable, rtrFlStaOutputMaxPkts=rtrFlStaOutputMaxPkts, numBlocksFree=numBlocksFree, sysIfInUcastPkts=sysIfInUcastPkts, sysCpu=sysCpu, rtrJitterTargetPort=rtrJitterTargetPort, cpuUtilAverageUtil=cpuUtilAverageUtil, taskDescr=taskDescr, rtrGroupName=rtrGroupName, sysIfPktPriority=sysIfPktPriority, rtrFlStaOutputMaxFlow=rtrFlStaOutputMaxFlow, sysAlertTrapInt=sysAlertTrapInt, sysIfOutUcastPkts=sysIfOutUcastPkts, sysIfOutNUcastPkts=sysIfOutNUcastPkts, taskTable=taskTable, rtrGroupId=rtrGroupId, rtrJitterMosMin=rtrJitterMosMin, rtrScheduleAgeOut=rtrScheduleAgeOut, sysIfPktPriStaEntry=sysIfPktPriStaEntry, sysRtrJitterEntry=sysRtrJitterEntry, sysRtrEntityEntry=sysRtrEntityEntry, numBytesFree=numBytesFree, avgBlockSizeFree=avgBlockSizeFree, rtrJitterRowStatus=rtrJitterRowStatus, sysRtrGroupEntry=sysRtrGroupEntry, sysRtrScheduleEntry=sysRtrScheduleEntry, rtrScheduleObjId=rtrScheduleObjId, cpuTaskPri=cpuTaskPri, rtrJitterDsMax=rtrJitterDsMax, rtrIcmpEchoSuccessRate=rtrIcmpEchoSuccessRate, rtrJitterIcpifMin=rtrJitterIcpifMin, sysTask=sysTask)
