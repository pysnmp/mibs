...
# ASN.1 source QUIDOS-MIB
# Source digest sha256:b3d2c260a2159761e7f7960f6071b6cba811610466af95d21fb3b37c8c9640f8
...
inTable = MibTable((1, 3, 6, 1, 4, 1, 18248, 16, 2, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: inTable.setStatus('current')
inEntry = MibTableRow((1, 3, 6, 1, 4, 1, 18248, 16, 2, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "QUIDOS-MIB", "index"))
...
outTable = MibTable((1, 3, 6, 1, 4, 1, 18248, 16, 3, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: outTable.setStatus('current')
outEntry = MibTableRow((1, 3, 6, 1, 4, 1, 18248, 16, 3, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "QUIDOS-MIB", "index"))
...
termTable = MibTable((1, 3, 6, 1, 4, 1, 18248, 16, 4, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: termTable.setStatus('current')
termEntry = MibTableRow((1, 3, 6, 1, 4, 1, 18248, 16, 4, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "QUIDOS-MIB", "index"))
...
mibBuilder.exportSymbols("QUIDOS-MIB", OnOff=OnOff, PositiveInteger=PositiveInteger, StatCit=StatCit, citrw=citrw, citrwS=citrwS, device_msg=device_msg, err=err, inEntry=inEntry, inTable=inTable, in_name=in_name, mezHi=mezHi, mezLo=mezLo, modeTerm=modeTerm, out=out, outEntry=outEntry, outTable=outTable, outTwr=outTwr, out_name=out_name, papouchProjekt=papouchProjekt, pysmi_in=pysmi_in, quido_var=quido_var, quidos=quidos, table_in=table_in, table_out=table_out, table_term=table_term, temp_msg=temp_msg, temperatureReading=temperatureReading, temperature_S_Reading=temperature_S_Reading, termEntry=termEntry, termTable=termTable, time=time, user_name=user_name)
