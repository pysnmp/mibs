...
mibBuilder.exportSymbols("PT-SFP-MIB", ptSFPCompliances=ptSFPCompliances, vendorName=vendorName, ptSFPEntry=ptSFPEntry, length62m10m=length62m10m, ptSFPConformance=ptSFPConformance, installedSFP=installedSFP, installedWavelength=installedWavelength, WaveLengthTC=WaveLengthTC, txPower=txPower, biasCurrent=biasCurrent, connectorType=connectorType, ptSFPFullCompliance=ptSFPFullCompliance, vendorSn=vendorSn, temperature=temperature, ptSFPTable=ptSFPTable, ptSFPCompleteGroup=ptSFPCompleteGroup, installState=installState, length9m100m=length9m100m, brNominal=brNominal, InstallStateTC=InstallStateTC, rxPower=rxPower, lengthCopper1m=lengthCopper1m, vcc=vcc, PortInterfaceTC=PortInterfaceTC, length50m10m=length50m10m, levelRx=levelRx, ConnectorTypeTC=ConnectorTypeTC, PYSNMP_MODULE_ID=ptSFP, vendorPn=vendorPn, length9m1km=length9m1km, ptSFP=ptSFP, ptSFPGroups=ptSFPGroups, vendorOui=vendorOui, vendorRev=vendorRev, saleableEntityCode=saleableEntityCode)
