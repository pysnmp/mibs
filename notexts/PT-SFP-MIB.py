...
mibBuilder.exportSymbols("PT-SFP-MIB", length9m1km=length9m1km, ptSFPFullCompliance=ptSFPFullCompliance, saleableEntityCode=saleableEntityCode, length50m10m=length50m10m, levelRx=levelRx, temperature=temperature, length62m10m=length62m10m, rxPower=rxPower, length9m100m=length9m100m, ptSFPTable=ptSFPTable, vendorRev=vendorRev, ptSFPCompleteGroup=ptSFPCompleteGroup, vcc=vcc, ptSFPEntry=ptSFPEntry, WaveLengthTC=WaveLengthTC, connectorType=connectorType, lengthCopper1m=lengthCopper1m, InstallStateTC=InstallStateTC, vendorSn=vendorSn, ptSFPConformance=ptSFPConformance, installedSFP=installedSFP, ConnectorTypeTC=ConnectorTypeTC, installState=installState, biasCurrent=biasCurrent, ptSFPCompliances=ptSFPCompliances, installedWavelength=installedWavelength, vendorName=vendorName, ptSFP=ptSFP, txPower=txPower, vendorPn=vendorPn, PYSNMP_MODULE_ID=ptSFP, brNominal=brNominal, vendorOui=vendorOui, ptSFPGroups=ptSFPGroups, PortInterfaceTC=PortInterfaceTC)
