...
mibBuilder.exportSymbols("PT-SFP-MIB", vendorName=vendorName, length9m100m=length9m100m, WaveLengthTC=WaveLengthTC, ptSFP=ptSFP, ptSFPCompliances=ptSFPCompliances, ptSFPGroups=ptSFPGroups, brNominal=brNominal, ptSFPFullCompliance=ptSFPFullCompliance, ptSFPConformance=ptSFPConformance, ptSFPTable=ptSFPTable, txPower=txPower, PortInterfaceTC=PortInterfaceTC, vendorRev=vendorRev, biasCurrent=biasCurrent, vendorPn=vendorPn, ptSFPEntry=ptSFPEntry, installState=installState, lengthCopper1m=lengthCopper1m, InstallStateTC=InstallStateTC, length9m1km=length9m1km, temperature=temperature, installedWavelength=installedWavelength, vendorSn=vendorSn, ConnectorTypeTC=ConnectorTypeTC, levelRx=levelRx, length50m10m=length50m10m, rxPower=rxPower, installedSFP=installedSFP, ptSFPCompleteGroup=ptSFPCompleteGroup, PYSNMP_MODULE_ID=ptSFP, saleableEntityCode=saleableEntityCode, vcc=vcc, length62m10m=length62m10m, connectorType=connectorType, vendorOui=vendorOui)
