...
mibBuilder.exportSymbols("PT-SFP-MIB", brNominal=brNominal, installedSFP=installedSFP, vendorName=vendorName, ptSFPCompleteGroup=ptSFPCompleteGroup, length50m10m=length50m10m, WaveLengthTC=WaveLengthTC, txPower=txPower, ptSFP=ptSFP, ptSFPFullCompliance=ptSFPFullCompliance, biasCurrent=biasCurrent, ConnectorTypeTC=ConnectorTypeTC, installedWavelength=installedWavelength, vendorRev=vendorRev, lengthCopper1m=lengthCopper1m, vendorPn=vendorPn, ptSFPEntry=ptSFPEntry, ptSFPCompliances=ptSFPCompliances, PortInterfaceTC=PortInterfaceTC, PYSNMP_MODULE_ID=ptSFP, length62m10m=length62m10m, ptSFPTable=ptSFPTable, length9m1km=length9m1km, vendorSn=vendorSn, rxPower=rxPower, vendorOui=vendorOui, length9m100m=length9m100m, ptSFPGroups=ptSFPGroups, ptSFPConformance=ptSFPConformance, temperature=temperature, connectorType=connectorType, saleableEntityCode=saleableEntityCode, levelRx=levelRx, vcc=vcc, installState=installState, InstallStateTC=InstallStateTC)
