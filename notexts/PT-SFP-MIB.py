...
mibBuilder.exportSymbols("PT-SFP-MIB", lengthCopper1m=lengthCopper1m, PYSNMP_MODULE_ID=ptSFP, length50m10m=length50m10m, length62m10m=length62m10m, vendorOui=vendorOui, length9m100m=length9m100m, saleableEntityCode=saleableEntityCode, ptSFP=ptSFP, ConnectorTypeTC=ConnectorTypeTC, WaveLengthTC=WaveLengthTC, vendorPn=vendorPn, vendorRev=vendorRev, vcc=vcc, ptSFPCompleteGroup=ptSFPCompleteGroup, ptSFPConformance=ptSFPConformance, PortInterfaceTC=PortInterfaceTC, connectorType=connectorType, ptSFPGroups=ptSFPGroups, biasCurrent=biasCurrent, rxPower=rxPower, ptSFPTable=ptSFPTable, temperature=temperature, vendorSn=vendorSn, ptSFPFullCompliance=ptSFPFullCompliance, ptSFPEntry=ptSFPEntry, InstallStateTC=InstallStateTC, installState=installState, installedSFP=installedSFP, levelRx=levelRx, vendorName=vendorName, txPower=txPower, ptSFPCompliances=ptSFPCompliances, length9m1km=length9m1km, installedWavelength=installedWavelength, brNominal=brNominal)
