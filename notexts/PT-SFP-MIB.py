...
mibBuilder.exportSymbols("PT-SFP-MIB", length50m10m=length50m10m, InstallStateTC=InstallStateTC, connectorType=connectorType, length62m10m=length62m10m, ptSFPFullCompliance=ptSFPFullCompliance, ptSFPTable=ptSFPTable, installedWavelength=installedWavelength, levelRx=levelRx, ptSFPCompliances=ptSFPCompliances, ConnectorTypeTC=ConnectorTypeTC, ptSFPGroups=ptSFPGroups, vendorSn=vendorSn, length9m100m=length9m100m, WaveLengthTC=WaveLengthTC, ptSFPCompleteGroup=ptSFPCompleteGroup, installedSFP=installedSFP, vcc=vcc, PYSNMP_MODULE_ID=ptSFP, temperature=temperature, ptSFPEntry=ptSFPEntry, ptSFP=ptSFP, vendorName=vendorName, rxPower=rxPower, length9m1km=length9m1km, brNominal=brNominal, vendorPn=vendorPn, biasCurrent=biasCurrent, vendorOui=vendorOui, PortInterfaceTC=PortInterfaceTC, lengthCopper1m=lengthCopper1m, vendorRev=vendorRev, saleableEntityCode=saleableEntityCode, ptSFPConformance=ptSFPConformance, txPower=txPower, installState=installState)
