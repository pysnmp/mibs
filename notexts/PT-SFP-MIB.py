...
mibBuilder.exportSymbols("PT-SFP-MIB", vendorSn=vendorSn, ptSFPCompliances=ptSFPCompliances, PortInterfaceTC=PortInterfaceTC, installedSFP=installedSFP, levelRx=levelRx, ConnectorTypeTC=ConnectorTypeTC, lengthCopper1m=lengthCopper1m, ptSFPGroups=ptSFPGroups, brNominal=brNominal, biasCurrent=biasCurrent, ptSFPCompleteGroup=ptSFPCompleteGroup, vendorOui=vendorOui, connectorType=connectorType, length62m10m=length62m10m, length50m10m=length50m10m, saleableEntityCode=saleableEntityCode, installedWavelength=installedWavelength, vendorRev=vendorRev, ptSFP=ptSFP, temperature=temperature, length9m100m=length9m100m, txPower=txPower, PYSNMP_MODULE_ID=ptSFP, installState=installState, ptSFPEntry=ptSFPEntry, vendorPn=vendorPn, vcc=vcc, ptSFPFullCompliance=ptSFPFullCompliance, rxPower=rxPower, ptSFPConformance=ptSFPConformance, InstallStateTC=InstallStateTC, ptSFPTable=ptSFPTable, WaveLengthTC=WaveLengthTC, length9m1km=length9m1km, vendorName=vendorName)
