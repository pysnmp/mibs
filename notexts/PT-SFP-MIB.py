...
mibBuilder.exportSymbols("PT-SFP-MIB", length9m100m=length9m100m, ptSFPConformance=ptSFPConformance, connectorType=connectorType, installedSFP=installedSFP, ptSFPTable=ptSFPTable, ptSFP=ptSFP, txPower=txPower, PYSNMP_MODULE_ID=ptSFP, temperature=temperature, length9m1km=length9m1km, ptSFPEntry=ptSFPEntry, ConnectorTypeTC=ConnectorTypeTC, installState=installState, installedWavelength=installedWavelength, length62m10m=length62m10m, biasCurrent=biasCurrent, levelRx=levelRx, length50m10m=length50m10m, ptSFPCompliances=ptSFPCompliances, ptSFPGroups=ptSFPGroups, vendorPn=vendorPn, vendorOui=vendorOui, PortInterfaceTC=PortInterfaceTC, ptSFPCompleteGroup=ptSFPCompleteGroup, lengthCopper1m=lengthCopper1m, vendorRev=vendorRev, WaveLengthTC=WaveLengthTC, vendorName=vendorName, brNominal=brNominal, vendorSn=vendorSn, saleableEntityCode=saleableEntityCode, InstallStateTC=InstallStateTC, vcc=vcc, rxPower=rxPower, ptSFPFullCompliance=ptSFPFullCompliance)
