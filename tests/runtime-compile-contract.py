...
#: RMON does not work, and this records why rather than omitting it.
#:
#: ``1.3.6.1.2.1.16`` is RMON. The frozen index answers ``RFC1271-MIB``, the
#: 1991 RMON MIB, and loading it fails:
#:
#:     SmiError: No symbol RFC1158-MIB::DisplayString
#:
#: ``RFC1271-MIB`` imports ``DisplayString`` from ``RFC1158-MIB``, whose ASN.1
#: does define it. But ``RFC1158-MIB`` is in pysmi's
#: ``PySnmpCodeGen.baseMibs``, and pysnmp's runtime compiler registers
#: ``StubSearcher(*baseMibs)`` (``pysnmp/smi/compiler.py``), so the compiler
#: will not compile it -- it defers to the copy pysnmp ships. That copy was
#: generated by pysmi 0.1.3 in April 2017 and says of itself "a stripped
#: version of MIB that contains only symbols that is unique to SMIv1 and have
#: no analogues in SMIv2". It exports two symbols. ``DisplayString`` is not one
#: of them, so the import fails.
#:
#: sc4snmp catches this in ``load_mibs()`` and logs a warning, so it degrades
#: rather than crashing: RMON varbinds keep their ``SNMPv2-SMI::mib-2.16.…``
#: names and are emitted unresolved.
#:
#: Recorded as a known failure so the fix flips a test. pysmi's current
#: generator emits an ``RFC1158-MIB`` that does export ``DisplayString``, so
#: adopting it -- which is what pysnmp/pysnmp#198 is for -- resolves this. When
#: that lands, this entry moves into LAZY_LOADED.
#:
#: Treated differently from the wrong-but-frozen index answers in
#: ``tests/index-contract.sh``, deliberately. Those are a compatibility
#: guarantee: consumers get the answer they have always had, and pinning them
#: as bugs would assert a guarantee is a defect. This one is a hard failure
#: that preserves nothing and that no consumer can depend on.
KNOWN_FAILURES = [
    ("1.3.6.1.2.1.16.1.1.1.1.1", "RFC1271-MIB", "RFC1158-MIB::DisplayString"),
]
...
def main():
    ...
    # sc4snmp calls the pre-7.x camelCase spellings, which are deprecated
    # aliases. This file calls the current names so CI is not buried in
    # DeprecationWarnings, but the aliases are what sc4snmp actually invokes,
    # so their disappearance is a downstream break and is worth one check here.
    print("== the deprecated spellings sc4snmp calls still exist")
    # Probed on an instance, since pysnmp serves the aliases through
    # __getattr__ rather than defining them on the class -- and probing them is
    # itself what raises the DeprecationWarning, hence the filter.
    ...
    def resolve(oid):
        ...
        identity.resolve_with_mib(view.MibViewController(builder))
