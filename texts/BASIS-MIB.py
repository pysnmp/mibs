...
mibBuilder.exportSymbols("BASIS-MIB", snaportCnt=snaportCnt, ausmPortCnf=ausmPortCnf, cwsonetObjects=cwsonetObjects, basisAsm=basisAsm, atmLines=atmLines, dsx3=dsx3, x21=x21, bbIfCnf=bbIfCnf, rpmInterface=rpmInterface, snaportCnfSig=snaportCnfSig, bbChanCnfGrp=bbChanCnfGrp, voice=voice, frPortCntSig=frPortCntSig, plcp=plcp, rpmChanGrp=rpmChanGrp, dsx1Plcp=dsx1Plcp, cardGeneric=cardGeneric, axisDiagnostics=axisDiagnostics, rpmChan=rpmChan, bbIfCnfResPartGrp=bbIfCnfResPartGrp, dsx1Framing=dsx1Framing, frPort=frPort, snaPortCntSig=snaPortCntSig, bbChanStateGrp=bbChanStateGrp, frChan=frChan, dsx3Line=dsx3Line, ausmPortCnt=ausmPortCnt, cesmChan=cesmChan, bbInterface=bbInterface, frPortCnfResPartGrp=frPortCnfResPartGrp, basisShelf=basisShelf, frPortCnfX21PortGrp=frPortCnfX21PortGrp, ausmChan=ausmChan, snaportCnf=snaportCnf, atm=atm, dsx0Vism=dsx0Vism, atmLineCnfGrp=atmLineCnfGrp, bbIfStateGrp=bbIfStateGrp, basisMIB=basisMIB, vismChanCnfGrp=vismChanCnfGrp, par=par, basisSystem=basisSystem, atmLineCntGrp=atmLineCntGrp, PYSNMP_MODULE_ID=basisMIB, basisLines=basisLines, virtualInterface=virtualInterface, dsx1=dsx1, dsx1Line=dsx1Line, frPortServiceQueGrp=frPortServiceQueGrp, snaport=snaport, frameRelay=frameRelay, frPortCnf=frPortCnf, vismPort=vismPort, atmLmiSignaling=atmLmiSignaling, frPortCnt=frPortCnt, cardSpecific=cardSpecific, axisRedundancy=axisRedundancy, axisSvc=axisSvc, rpmPort=rpmPort, vismChanGrp=vismChanGrp, bbChan=bbChan, dsx3Framing=dsx3Framing, frPortCnfSig=frPortCnfSig, ausmPort=ausmPort, basisServices=basisServices, circuitEmulation=circuitEmulation, atmAddressRegistration=atmAddressRegistration, basis=basis, bbIfCnt=bbIfCnt, bbChanCntGrp=bbChanCntGrp, sonet=sonet, sna=sna)
