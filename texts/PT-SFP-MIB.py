...
mibBuilder.exportSymbols("PT-SFP-MIB", InstallStateTC=InstallStateTC, brNominal=brNominal, length9m100m=length9m100m, levelRx=levelRx, biasCurrent=biasCurrent, txPower=txPower, saleableEntityCode=saleableEntityCode, PYSNMP_MODULE_ID=ptSFP, rxPower=rxPower, length50m10m=length50m10m, ptSFPTable=ptSFPTable, lengthCopper1m=lengthCopper1m, length62m10m=length62m10m, vcc=vcc, length9m1km=length9m1km, ptSFPEntry=ptSFPEntry, ptSFPCompliances=ptSFPCompliances, vendorRev=vendorRev, PortInterfaceTC=PortInterfaceTC, ptSFP=ptSFP, vendorSn=vendorSn, ptSFPConformance=ptSFPConformance, ConnectorTypeTC=ConnectorTypeTC, installedSFP=installedSFP, vendorOui=vendorOui, connectorType=connectorType, vendorPn=vendorPn, temperature=temperature, installState=installState, ptSFPGroups=ptSFPGroups, installedWavelength=installedWavelength, vendorName=vendorName, WaveLengthTC=WaveLengthTC, ptSFPFullCompliance=ptSFPFullCompliance, ptSFPCompleteGroup=ptSFPCompleteGroup)
