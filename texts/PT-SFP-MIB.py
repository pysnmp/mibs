...
mibBuilder.exportSymbols("PT-SFP-MIB", vcc=vcc, vendorPn=vendorPn, length50m10m=length50m10m, vendorRev=vendorRev, vendorName=vendorName, rxPower=rxPower, saleableEntityCode=saleableEntityCode, WaveLengthTC=WaveLengthTC, ptSFPCompliances=ptSFPCompliances, ptSFPEntry=ptSFPEntry, txPower=txPower, length62m10m=length62m10m, length9m100m=length9m100m, ptSFPTable=ptSFPTable, ptSFPGroups=ptSFPGroups, ConnectorTypeTC=ConnectorTypeTC, installedSFP=installedSFP, installedWavelength=installedWavelength, vendorOui=vendorOui, connectorType=connectorType, temperature=temperature, PortInterfaceTC=PortInterfaceTC, length9m1km=length9m1km, ptSFPFullCompliance=ptSFPFullCompliance, InstallStateTC=InstallStateTC, installState=installState, biasCurrent=biasCurrent, ptSFPConformance=ptSFPConformance, lengthCopper1m=lengthCopper1m, PYSNMP_MODULE_ID=ptSFP, brNominal=brNominal, vendorSn=vendorSn, ptSFPCompleteGroup=ptSFPCompleteGroup, levelRx=levelRx, ptSFP=ptSFP)
