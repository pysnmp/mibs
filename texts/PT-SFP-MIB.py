...
mibBuilder.exportSymbols("PT-SFP-MIB", vendorSn=vendorSn, length9m1km=length9m1km, biasCurrent=biasCurrent, ptSFPCompliances=ptSFPCompliances, ptSFP=ptSFP, vcc=vcc, PortInterfaceTC=PortInterfaceTC, length62m10m=length62m10m, txPower=txPower, saleableEntityCode=saleableEntityCode, installedSFP=installedSFP, vendorName=vendorName, levelRx=levelRx, ptSFPConformance=ptSFPConformance, connectorType=connectorType, vendorOui=vendorOui, InstallStateTC=InstallStateTC, lengthCopper1m=lengthCopper1m, ptSFPGroups=ptSFPGroups, vendorRev=vendorRev, length50m10m=length50m10m, installedWavelength=installedWavelength, WaveLengthTC=WaveLengthTC, ConnectorTypeTC=ConnectorTypeTC, length9m100m=length9m100m, ptSFPFullCompliance=ptSFPFullCompliance, ptSFPCompleteGroup=ptSFPCompleteGroup, PYSNMP_MODULE_ID=ptSFP, rxPower=rxPower, brNominal=brNominal, ptSFPEntry=ptSFPEntry, ptSFPTable=ptSFPTable, vendorPn=vendorPn, installState=installState, temperature=temperature)
