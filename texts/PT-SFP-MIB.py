...
mibBuilder.exportSymbols("PT-SFP-MIB", PortInterfaceTC=PortInterfaceTC, brNominal=brNominal, ptSFPFullCompliance=ptSFPFullCompliance, temperature=temperature, vendorOui=vendorOui, PYSNMP_MODULE_ID=ptSFP, saleableEntityCode=saleableEntityCode, installedSFP=installedSFP, ptSFPCompliances=ptSFPCompliances, installState=installState, vendorRev=vendorRev, vendorSn=vendorSn, length62m10m=length62m10m, length50m10m=length50m10m, ptSFPEntry=ptSFPEntry, WaveLengthTC=WaveLengthTC, connectorType=connectorType, vendorPn=vendorPn, ptSFP=ptSFP, vcc=vcc, biasCurrent=biasCurrent, ConnectorTypeTC=ConnectorTypeTC, ptSFPTable=ptSFPTable, ptSFPGroups=ptSFPGroups, vendorName=vendorName, lengthCopper1m=lengthCopper1m, levelRx=levelRx, installedWavelength=installedWavelength, rxPower=rxPower, length9m1km=length9m1km, length9m100m=length9m100m, ptSFPConformance=ptSFPConformance, txPower=txPower, ptSFPCompleteGroup=ptSFPCompleteGroup, InstallStateTC=InstallStateTC)
