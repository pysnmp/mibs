...
mibBuilder.exportSymbols("PT-SFP-MIB", ConnectorTypeTC=ConnectorTypeTC, length50m10m=length50m10m, vendorSn=vendorSn, installedWavelength=installedWavelength, vendorPn=vendorPn, ptSFPGroups=ptSFPGroups, installState=installState, saleableEntityCode=saleableEntityCode, ptSFPConformance=ptSFPConformance, vendorName=vendorName, lengthCopper1m=lengthCopper1m, ptSFPFullCompliance=ptSFPFullCompliance, ptSFPCompleteGroup=ptSFPCompleteGroup, InstallStateTC=InstallStateTC, PortInterfaceTC=PortInterfaceTC, vcc=vcc, levelRx=levelRx, WaveLengthTC=WaveLengthTC, length9m100m=length9m100m, installedSFP=installedSFP, length9m1km=length9m1km, biasCurrent=biasCurrent, PYSNMP_MODULE_ID=ptSFP, ptSFP=ptSFP, connectorType=connectorType, vendorOui=vendorOui, txPower=txPower, temperature=temperature, ptSFPCompliances=ptSFPCompliances, ptSFPTable=ptSFPTable, brNominal=brNominal, length62m10m=length62m10m, ptSFPEntry=ptSFPEntry, rxPower=rxPower, vendorRev=vendorRev)
