...
# Produced by pysmi-1.1.3 at Tue Dec  7 17:27:00 2021
...
Integer32, Counter32, IpAddress, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, Bits, MibIdentifier, TimeTicks, Unsigned32, NotificationType, Gauge32, ModuleIdentity, iso, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Counter32", "IpAddress", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "Bits", "MibIdentifier", "TimeTicks", "Unsigned32", "NotificationType", "Gauge32", "ModuleIdentity", "iso", "ObjectIdentity")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", sMTPServerPortNumber=sMTPServerPortNumber, telecomGroup=telecomGroup, vectorError=vectorError, tDDControlGroup=tDDControlGroup, supplementary=supplementary, transmitFreqKHz=transmitFreqKHz, sNMPEnabledTraps=sNMPEnabledTraps, installArmState=installArmState, encryptionAlgorithm=encryptionAlgorithm, bandwidthVariant=bandwidthVariant, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, signalStrengthRatio=signalStrengthRatio, dfs=dfs, userName=userName, frequencyVariant=frequencyVariant, daylightSaving=daylightSaving, sMTPServerIPAddress=sMTPServerIPAddress, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, sNTPPollInterval=sNTPPollInterval, secondarySoftwareVersion=secondarySoftwareVersion, vLANPriorityQueueMapping=vLANPriorityQueueMapping, dataPortConfigurationMismatch=dataPortConfigurationMismatch, versionsGroup=versionsGroup, siteName=siteName, dataPortAutoNegotiation=dataPortAutoNegotiation, syslogControlGroup=syslogControlGroup, licenceGroup=licenceGroup, ptpTrapPrefix=ptpTrapPrefix, receiveModulationModeDetail=receiveModulationModeDetail, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, hardwareVersion=hardwareVersion, phyStatus=phyStatus, receivePower=receivePower, groupID=groupID, sNMPTrapTableNumber=sNMPTrapTableNumber, configuration=configuration, dataPortStatus=dataPortStatus, sNTPServerPortNumber=sNTPServerPortNumber, dataPortAutoMdix=dataPortAutoMdix, sMTPSourceEmailAddress=sMTPSourceEmailAddress, versions=versions, bootVersion=bootVersion, PYSNMP_MODULE_ID=cambium, dfsTableEntry=dfsTableEntry, ethernet=ethernet, systemClock=systemClock, altitude=altitude, dfsPeaks=dfsPeaks, sNMPTrapTableEntry=sNMPTrapTableEntry, sMTPEnabledMessages=sMTPEnabledMessages, telecomsLineCode=telecomsLineCode, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, resetGroup=resetGroup, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, maximumTransmitPower=maximumTransmitPower, dataPortStatusTrap=dataPortStatusTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, remoteIPAddress=remoteIPAddress, userPassword=userPassword, searchState=searchState, sNMPTrapVersion=sNMPTrapVersion, phyControlGroup=phyControlGroup, transmitPower=transmitPower, installStatus=installStatus, ptp=ptp, configurationGroup=configurationGroup, sntpGroup=sntpGroup, iPAddress=iPAddress, telnetAccessEnabled=telnetAccessEnabled, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, tFTPServerPortNumber=tFTPServerPortNumber, longitude=longitude, cambium=cambium, dfsMeans=dfsMeans, hTTPAccessEnabled=hTTPAccessEnabled, aAAControlGroup=aAAControlGroup, aAAControl=aAAControl, systemReset=systemReset, installStatusTrap=installStatusTrap, installArmStateTrap=installArmStateTrap, ptpGroups=ptpGroups, dfsTableNumber=dfsTableNumber, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, vLANPriorityTable=vLANPriorityTable, userTableNumber=userTableNumber, sNMPCommunityString=sNMPCommunityString, linkModeOptimisation=linkModeOptimisation, sntp=sntp, constantPowerSpectralDensity=constantPowerSpectralDensity, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, syslogLocalWrappedTrap=syslogLocalWrappedTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, snmpControl=snmpControl, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sMTPEmailAlert=sMTPEmailAlert, wirelessLinkAvailability=wirelessLinkAvailability, telecomsLoopbackTrap=telecomsLoopbackTrap, masterSlaveMode=masterSlaveMode, dfsTable=dfsTable, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPv3Enable=sNMPv3Enable, sNTPLastSync=sNTPLastSync, productVariant=productVariant, incompatibleRegionCodes=incompatibleRegionCodes, reset=reset, syslogStateTrap=syslogStateTrap, receiveModulationMode=receiveModulationMode, antennaGain=antennaGain, capacityLicenseMismatch=capacityLicenseMismatch, incompatibleMasterAndSlave=incompatibleMasterAndSlave, linkLoss=linkLoss, cableLoss=cableLoss, productName=productName, regionCode=regionCode, sNMPTrapTableIndex=sNMPTrapTableIndex, tFTPServerIPAddress=tFTPServerIPAddress, hTTPSPortNumber=hTTPSPortNumber, userTable=userTable, receiveDataRate=receiveDataRate, userEnabled=userEnabled, userRole=userRole, syslogControl=syslogControl, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, transmitModulationMode=transmitModulationMode, phyStatusGroup=phyStatusGroup, targetMACAddress=targetMACAddress, userTableEntry=userTableEntry, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, vLANPriorityTableIndex=vLANPriorityTableIndex, tDDSynchronizationStatus=tDDSynchronizationStatus, useVLANForManagementInterfaces=useVLANForManagementInterfaces, ptp500=ptp500, accessMethod=accessMethod, encryption=encryption, receiveFreqKHz=receiveFreqKHz, telecomsLoopback=telecomsLoopback, vLANManagementVID=vLANManagementVID, hTTPPortNumber=hTTPPortNumber, alarms=alarms, pubStatsGroup=pubStatsGroup, tDDSynchronizationMode=tDDSynchronizationMode, syslogClient=syslogClient, ethernetGroup=ethernetGroup, linkSymmetry=linkSymmetry, userTableIndex=userTableIndex, latitude=latitude, pubStats=pubStats, managementGroup=managementGroup, sNTPServerIPAddress=sNTPServerIPAddress, regionCodeTrap=regionCodeTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, dataPortDisabledWarning=dataPortDisabledWarning, dfsGroup=dfsGroup, sNMPTrapIPAddress=sNMPTrapIPAddress, gatewayIPAddress=gatewayIPAddress, smtp=smtp, eIRP=eIRP, telecomsCableLength=telecomsCableLength, management=management, tDDControl=tDDControl, sNTPSyncTrap=sNTPSyncTrap, vLANPriorityTableNumber=vLANPriorityTableNumber, notificationsGroup=notificationsGroup, sNTPSync=sNTPSync, smtpGroup=smtpGroup, linkName=linkName, transmitChannel=transmitChannel, sNTPState=sNTPState, telecom=telecom, linkModeOptimizationMismatch=linkModeOptimizationMismatch, ethernetFiberSupport=ethernetFiberSupport, sNMPTrapPortNumber=sNMPTrapPortNumber, phyControl=phyControl, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, byteErrorRatio=byteErrorRatio, telnetPortNumber=telnetPortNumber, softwareVersion=softwareVersion, transmitDataRate=transmitDataRate, telecomsChannelStatusTrap=telecomsChannelStatusTrap, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, telecomsChannelStatus=telecomsChannelStatus, telecomsInterface=telecomsInterface, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, channelChangeTrap=channelChangeTrap, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, licence=licence, supplementaryGroup=supplementaryGroup, hTTPSAccessEnabled=hTTPSAccessEnabled, encryptionGroup=encryptionGroup, timeZone=timeZone, sNMPPortNumber=sNMPPortNumber, receiveFreqMHz=receiveFreqMHz, ptmp=ptmp, aggregateDataRate=aggregateDataRate, range=range, wirelessLinkStatus=wirelessLinkStatus, syslogState=syslogState, sNMPTrapTable=sNMPTrapTable, ptpCompliance=ptpCompliance, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, snmpControlGroup=snmpControlGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, dataPortWirelessDownAlert=dataPortWirelessDownAlert, unitOutOfCalibration=unitOutOfCalibration, tDDSynchronizationAlarm=tDDSynchronizationAlarm, subnetMask=subnetMask, licensedCapacity=licensedCapacity, vLANManagementPriority=vLANManagementPriority, syslogClientTrap=syslogClientTrap, dfsTableIndex=dfsTableIndex, receiveChannel=receiveChannel, transmitFreqMHz=transmitFreqMHz, vLANPriorityTableEntry=vLANPriorityTableEntry, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, alarmsGroup=alarmsGroup, channelBandwidth=channelBandwidth, ptpTraps=ptpTraps)
