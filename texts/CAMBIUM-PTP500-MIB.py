...
# Produced by pysmi-1.1.3 at Wed Dec  1 15:41:11 2021
...
Integer32, Counter64, Bits, ObjectIdentity, enterprises, NotificationType, Unsigned32, Counter32, ModuleIdentity, iso, TimeTicks, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Counter64", "Bits", "ObjectIdentity", "enterprises", "NotificationType", "Unsigned32", "Counter32", "ModuleIdentity", "iso", "TimeTicks", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "Gauge32")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", dfs=dfs, receiveModulationMode=receiveModulationMode, telecomsLoopbackTrap=telecomsLoopbackTrap, tDDControl=tDDControl, ptpCompliance=ptpCompliance, sNMPv3Enable=sNMPv3Enable, sNMPTrapTable=sNMPTrapTable, hTTPAccessEnabled=hTTPAccessEnabled, ptp=ptp, tDDSynchronizationStatus=tDDSynchronizationStatus, resetGroup=resetGroup, aAAControlGroup=aAAControlGroup, daylightSaving=daylightSaving, softwareVersion=softwareVersion, hTTPSAccessEnabled=hTTPSAccessEnabled, supplementaryGroup=supplementaryGroup, dataPortStatus=dataPortStatus, telecomGroup=telecomGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, snmpControl=snmpControl, telnetPortNumber=telnetPortNumber, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, remoteIPAddress=remoteIPAddress, ethernetGroup=ethernetGroup, bootVersion=bootVersion, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, latitude=latitude, useVLANForManagementInterfaces=useVLANForManagementInterfaces, eIRP=eIRP, vLANPriorityTableEntry=vLANPriorityTableEntry, channelChangeTrap=channelChangeTrap, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, longitude=longitude, syslogClientTrap=syslogClientTrap, telecomsInterface=telecomsInterface, channelBandwidth=channelBandwidth, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, vLANPriorityTableNumber=vLANPriorityTableNumber, cableLoss=cableLoss, tFTPServerPortNumber=tFTPServerPortNumber, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, telnetAccessEnabled=telnetAccessEnabled, range=range, sNMPTrapTableIndex=sNMPTrapTableIndex, incompatibleRegionCodes=incompatibleRegionCodes, dataPortWirelessDownAlert=dataPortWirelessDownAlert, aAAControl=aAAControl, regionCodeTrap=regionCodeTrap, sNTPSync=sNTPSync, syslogState=syslogState, systemClock=systemClock, regionCode=regionCode, smtpGroup=smtpGroup, encryptionGroup=encryptionGroup, vLANManagementPriority=vLANManagementPriority, tDDControlGroup=tDDControlGroup, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, management=management, sNTPServerIPAddress=sNTPServerIPAddress, versions=versions, sMTPEnabledMessages=sMTPEnabledMessages, userTable=userTable, gatewayIPAddress=gatewayIPAddress, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, userEnabled=userEnabled, sntp=sntp, vLANPriorityTableIndex=vLANPriorityTableIndex, dfsTableEntry=dfsTableEntry, receiveFreqKHz=receiveFreqKHz, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sMTPSourceEmailAddress=sMTPSourceEmailAddress, ethernet=ethernet, versionsGroup=versionsGroup, sMTPServerIPAddress=sMTPServerIPAddress, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, configuration=configuration, sNTPSyncTrap=sNTPSyncTrap, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sNMPTrapVersion=sNMPTrapVersion, ptp500=ptp500, transmitFreqMHz=transmitFreqMHz, transmitPower=transmitPower, phyControl=phyControl, unitOutOfCalibration=unitOutOfCalibration, sNMPTrapPortNumber=sNMPTrapPortNumber, subnetMask=subnetMask, userTableEntry=userTableEntry, sMTPServerPortNumber=sMTPServerPortNumber, systemReset=systemReset, accessMethod=accessMethod, groupID=groupID, tDDSynchronizationMode=tDDSynchronizationMode, searchState=searchState, phyControlGroup=phyControlGroup, dataPortStatusTrap=dataPortStatusTrap, transmitDataRate=transmitDataRate, sNMPPortNumber=sNMPPortNumber, encryption=encryption, sNMPTrapTableEntry=sNMPTrapTableEntry, licence=licence, telecomsCableLength=telecomsCableLength, remoteMaximumTransmitPower=remoteMaximumTransmitPower, capacityLicenseMismatch=capacityLicenseMismatch, siteName=siteName, syslogControl=syslogControl, maximumTransmitPower=maximumTransmitPower, sntpGroup=sntpGroup, receivePower=receivePower, telecomsChannelStatus=telecomsChannelStatus, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, sNMPTrapIPAddress=sNMPTrapIPAddress, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, vLANManagementVID=vLANManagementVID, reset=reset, managementGroup=managementGroup, dfsTableNumber=dfsTableNumber, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sNMPEnabledTraps=sNMPEnabledTraps, aggregateDataRate=aggregateDataRate, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, iPAddress=iPAddress, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, receiveDataRate=receiveDataRate, constantPowerSpectralDensity=constantPowerSpectralDensity, syslogLocalWrappedTrap=syslogLocalWrappedTrap, vLANPriorityTable=vLANPriorityTable, frequencyVariant=frequencyVariant, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, ptpGroups=ptpGroups, ptmp=ptmp, sNTPPollInterval=sNTPPollInterval, installStatus=installStatus, installStatusTrap=installStatusTrap, encryptionAlgorithm=encryptionAlgorithm, userTableNumber=userTableNumber, sNMPTrapTableNumber=sNMPTrapTableNumber, linkName=linkName, sNMPCommunityString=sNMPCommunityString, masterSlaveMode=masterSlaveMode, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, receiveChannel=receiveChannel, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, linkModeOptimisation=linkModeOptimisation, userName=userName, dataPortAutoMdix=dataPortAutoMdix, tFTPServerIPAddress=tFTPServerIPAddress, bandwidthVariant=bandwidthVariant, dfsTable=dfsTable, transmitChannel=transmitChannel, notificationsGroup=notificationsGroup, vLANPriorityQueueMapping=vLANPriorityQueueMapping, configurationGroup=configurationGroup, licenceGroup=licenceGroup, sNTPState=sNTPState, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, PYSNMP_MODULE_ID=cambium, userRole=userRole, ptpTraps=ptpTraps, dfsTableIndex=dfsTableIndex, targetMACAddress=targetMACAddress, sNTPLastSync=sNTPLastSync, hardwareVersion=hardwareVersion, linkModeOptimizationMismatch=linkModeOptimizationMismatch, antennaGain=antennaGain, dataPortDisabledWarning=dataPortDisabledWarning, hTTPSPortNumber=hTTPSPortNumber, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, pubStats=pubStats, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNTPServerPortNumber=sNTPServerPortNumber, licensedCapacity=licensedCapacity, telecomsChannelStatusTrap=telecomsChannelStatusTrap, cambium=cambium, dfsMeans=dfsMeans, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, transmitFreqKHz=transmitFreqKHz, linkLoss=linkLoss, receiveModulationModeDetail=receiveModulationModeDetail, syslogClient=syslogClient, secondarySoftwareVersion=secondarySoftwareVersion, signalStrengthRatio=signalStrengthRatio, wirelessLinkStatus=wirelessLinkStatus, ethernetFiberSupport=ethernetFiberSupport, syslogStateTrap=syslogStateTrap, hTTPPortNumber=hTTPPortNumber, alarmsGroup=alarmsGroup, phyStatus=phyStatus, dfsPeaks=dfsPeaks, wirelessLinkAvailability=wirelessLinkAvailability, alarms=alarms, userTableIndex=userTableIndex, pubStatsGroup=pubStatsGroup, dfsGroup=dfsGroup, installArmStateTrap=installArmStateTrap, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, productVariant=productVariant, productName=productName, transmitModulationMode=transmitModulationMode, sMTPEmailAlert=sMTPEmailAlert, byteErrorRatio=byteErrorRatio, installArmState=installArmState, noWirelessChannelAvailable=noWirelessChannelAvailable, ptpTrapPrefix=ptpTrapPrefix, tDDSynchronizationAlarm=tDDSynchronizationAlarm, supplementary=supplementary, receiveFreqMHz=receiveFreqMHz, snmpControlGroup=snmpControlGroup, telecomsLineCode=telecomsLineCode, telecomsLoopback=telecomsLoopback, altitude=altitude, timeZone=timeZone, telecom=telecom, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, linkSymmetry=linkSymmetry, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, phyStatusGroup=phyStatusGroup, syslogControlGroup=syslogControlGroup, userPassword=userPassword, smtp=smtp, vectorError=vectorError)
