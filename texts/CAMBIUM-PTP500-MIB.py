...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", receiveFreqMHz=receiveFreqMHz, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, ptpGroups=ptpGroups, sntpGroup=sntpGroup, phyControlGroup=phyControlGroup, sNMPTrapVersion=sNMPTrapVersion, dfsTable=dfsTable, telecomsChannelStatusTrap=telecomsChannelStatusTrap, sNTPState=sNTPState, sMTPEnabledMessages=sMTPEnabledMessages, sNMPEnabledTraps=sNMPEnabledTraps, sMTPSourceEmailAddress=sMTPSourceEmailAddress, ptp500=ptp500, telecomsLoopback=telecomsLoopback, tFTPServerIPAddress=tFTPServerIPAddress, phyStatus=phyStatus, vLANPriorityTableEntry=vLANPriorityTableEntry, incompatibleMasterAndSlave=incompatibleMasterAndSlave, userTableNumber=userTableNumber, latitude=latitude, iPAddress=iPAddress, altitude=altitude, syslogClientTrap=syslogClientTrap, vectorError=vectorError, siteName=siteName, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, telecomsLoopbackTrap=telecomsLoopbackTrap, hTTPSPortNumber=hTTPSPortNumber, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, sNTPLastSync=sNTPLastSync, linkName=linkName, masterSlaveMode=masterSlaveMode, sMTPServerIPAddress=sMTPServerIPAddress, softwareVersion=softwareVersion, noWirelessChannelAvailable=noWirelessChannelAvailable, vLANPriorityTableIndex=vLANPriorityTableIndex, receiveModulationModeDetail=receiveModulationModeDetail, sNMPPortNumber=sNMPPortNumber, productVariant=productVariant, cableLoss=cableLoss, userRole=userRole, ptp=ptp, transmitFreqKHz=transmitFreqKHz, ptpTraps=ptpTraps, tDDSynchronizationAlarm=tDDSynchronizationAlarm, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, secondarySoftwareVersion=secondarySoftwareVersion, receiveModulationMode=receiveModulationMode, hardwareVersion=hardwareVersion, sNTPServerIPAddress=sNTPServerIPAddress, installArmStateTrap=installArmStateTrap, smtp=smtp, incompatibleRegionCodes=incompatibleRegionCodes, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, configurationGroup=configurationGroup, regionCodeTrap=regionCodeTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, vLANManagementVID=vLANManagementVID, vLANPriorityQueueMapping=vLANPriorityQueueMapping, hTTPPortNumber=hTTPPortNumber, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, alarmsGroup=alarmsGroup, tDDControlGroup=tDDControlGroup, productName=productName, bootVersion=bootVersion, PYSNMP_MODULE_ID=cambium, pubStatsGroup=pubStatsGroup, channelBandwidth=channelBandwidth, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, systemClock=systemClock, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, byteErrorRatio=byteErrorRatio, syslogClient=syslogClient, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sMTPEmailAlert=sMTPEmailAlert, telecomsLineCode=telecomsLineCode, sNMPTrapIPAddress=sNMPTrapIPAddress, dfsTableEntry=dfsTableEntry, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, dataPortStatusTrap=dataPortStatusTrap, syslogStateTrap=syslogStateTrap, ptpTrapPrefix=ptpTrapPrefix, alarms=alarms, sNTPSync=sNTPSync, sNMPTrapTableEntry=sNMPTrapTableEntry, userEnabled=userEnabled, remoteIPAddress=remoteIPAddress, sntp=sntp, linkLoss=linkLoss, dfsGroup=dfsGroup, managementGroup=managementGroup, sNTPSyncTrap=sNTPSyncTrap, snmpControl=snmpControl, resetGroup=resetGroup, vLANPriorityTableNumber=vLANPriorityTableNumber, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, supplementary=supplementary, bandwidthVariant=bandwidthVariant, aAAControl=aAAControl, telnetAccessEnabled=telnetAccessEnabled, syslogControl=syslogControl, tFTPServerPortNumber=tFTPServerPortNumber, linkSymmetry=linkSymmetry, aggregateDataRate=aggregateDataRate, longitude=longitude, ptpCompliance=ptpCompliance, licenceGroup=licenceGroup, sNMPTrapTable=sNMPTrapTable, antennaGain=antennaGain, hTTPAccessEnabled=hTTPAccessEnabled, transmitModulationMode=transmitModulationMode, unitOutOfCalibration=unitOutOfCalibration, dfsPeaks=dfsPeaks, sNTPPollInterval=sNTPPollInterval, groupID=groupID, remoteMaximumTransmitPower=remoteMaximumTransmitPower, tDDSynchronizationMode=tDDSynchronizationMode, vLANPriorityTable=vLANPriorityTable, dataPortAutoNegotiation=dataPortAutoNegotiation, userName=userName, encryption=encryption, ethernet=ethernet, sNMPCommunityString=sNMPCommunityString, smtpGroup=smtpGroup, configuration=configuration, dataPortDisabledWarning=dataPortDisabledWarning, receiveFreqKHz=receiveFreqKHz, telecom=telecom, frequencyVariant=frequencyVariant, accessMethod=accessMethod, regionCode=regionCode, licensedCapacity=licensedCapacity, telecomsChannelStatus=telecomsChannelStatus, transmitFreqMHz=transmitFreqMHz, constantPowerSpectralDensity=constantPowerSpectralDensity, daylightSaving=daylightSaving, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, ethernetFiberSupport=ethernetFiberSupport, sNMPTrapTableNumber=sNMPTrapTableNumber, userTableEntry=userTableEntry, installStatusTrap=installStatusTrap, ptmp=ptmp, transmitChannel=transmitChannel, linkModeOptimisation=linkModeOptimisation, sNTPServerPortNumber=sNTPServerPortNumber, eIRP=eIRP, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, tDDSynchronizationStatus=tDDSynchronizationStatus, userPassword=userPassword, encryptionAlgorithm=encryptionAlgorithm, dataPortStatus=dataPortStatus, sNMPv3Enable=sNMPv3Enable, notificationsGroup=notificationsGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, ethernetGroup=ethernetGroup, installStatus=installStatus, capacityLicenseMismatch=capacityLicenseMismatch, encryptionGroup=encryptionGroup, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, tDDControl=tDDControl, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, dataPortAutoMdix=dataPortAutoMdix, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, systemReset=systemReset, telecomsCableLength=telecomsCableLength, maximumTransmitPower=maximumTransmitPower, telnetPortNumber=telnetPortNumber, vLANManagementPriority=vLANManagementPriority, timeZone=timeZone, searchState=searchState, versions=versions, dfsTableNumber=dfsTableNumber, signalStrengthRatio=signalStrengthRatio, licence=licence, pubStats=pubStats, phyStatusGroup=phyStatusGroup, hTTPSAccessEnabled=hTTPSAccessEnabled, userTable=userTable, telecomsInterface=telecomsInterface, transmitDataRate=transmitDataRate, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, useVLANForManagementInterfaces=useVLANForManagementInterfaces, transmitPower=transmitPower, reset=reset, dataPortConfigurationMismatch=dataPortConfigurationMismatch, dfsTableIndex=dfsTableIndex, aAAControlGroup=aAAControlGroup, receivePower=receivePower, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, gatewayIPAddress=gatewayIPAddress, telecomGroup=telecomGroup, installArmState=installArmState, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, syslogState=syslogState, range=range, dfs=dfs, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, userTableIndex=userTableIndex, receiveChannel=receiveChannel, sNMPTrapPortNumber=sNMPTrapPortNumber, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, enabledDiagnosticAlarms=enabledDiagnosticAlarms, wirelessLinkAvailability=wirelessLinkAvailability, supplementaryGroup=supplementaryGroup, dataPortWirelessDownAlert=dataPortWirelessDownAlert, dfsMeans=dfsMeans, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sMTPServerPortNumber=sMTPServerPortNumber, phyControl=phyControl, cambium=cambium, syslogControlGroup=syslogControlGroup, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, receiveDataRate=receiveDataRate, management=management, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, channelChangeTrap=channelChangeTrap, targetMACAddress=targetMACAddress, snmpControlGroup=snmpControlGroup, subnetMask=subnetMask, wirelessLinkStatus=wirelessLinkStatus, versionsGroup=versionsGroup)
