...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", telecom=telecom, remoteIPAddress=remoteIPAddress, channelChangeTrap=channelChangeTrap, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, management=management, targetMACAddress=targetMACAddress, syslogState=syslogState, sNMPTrapTableEntry=sNMPTrapTableEntry, versionsGroup=versionsGroup, installStatus=installStatus, regionCodeTrap=regionCodeTrap, resetGroup=resetGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sntpGroup=sntpGroup, PYSNMP_MODULE_ID=cambium, transmitModulationMode=transmitModulationMode, sNTPServerPortNumber=sNTPServerPortNumber, pubStatsGroup=pubStatsGroup, wirelessLinkStatus=wirelessLinkStatus, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, snmpControlGroup=snmpControlGroup, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNMPTrapIPAddress=sNMPTrapIPAddress, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, productVariant=productVariant, vLANPriorityTableNumber=vLANPriorityTableNumber, userRole=userRole, supplementary=supplementary, sNTPSync=sNTPSync, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, dfsTableNumber=dfsTableNumber, sMTPServerIPAddress=sMTPServerIPAddress, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sMTPEnabledMessages=sMTPEnabledMessages, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, userTable=userTable, transmitFreqKHz=transmitFreqKHz, subnetMask=subnetMask, noWirelessChannelAvailable=noWirelessChannelAvailable, sNTPPollInterval=sNTPPollInterval, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, userTableEntry=userTableEntry, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, managementGroup=managementGroup, ptpTraps=ptpTraps, receiveFreqKHz=receiveFreqKHz, remoteMaximumTransmitPower=remoteMaximumTransmitPower, installStatusTrap=installStatusTrap, unitOutOfCalibration=unitOutOfCalibration, capacityLicenseMismatch=capacityLicenseMismatch, tDDSynchronizationStatus=tDDSynchronizationStatus, masterSlaveMode=masterSlaveMode, smtpGroup=smtpGroup, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, telecomsInterface=telecomsInterface, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, vLANPriorityTableIndex=vLANPriorityTableIndex, sNTPLastSync=sNTPLastSync, gatewayIPAddress=gatewayIPAddress, syslogStateTrap=syslogStateTrap, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, userPassword=userPassword, constantPowerSpectralDensity=constantPowerSpectralDensity, incompatibleRegionCodes=incompatibleRegionCodes, altitude=altitude, dfsTableIndex=dfsTableIndex, installArmState=installArmState, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sMTPServerPortNumber=sMTPServerPortNumber, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sNMPTrapPortNumber=sNMPTrapPortNumber, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, syslogClient=syslogClient, snmpControl=snmpControl, phyControl=phyControl, hTTPSAccessEnabled=hTTPSAccessEnabled, receiveChannel=receiveChannel, smtp=smtp, telnetPortNumber=telnetPortNumber, linkLoss=linkLoss, telecomsCableLength=telecomsCableLength, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, vLANPriorityQueueMapping=vLANPriorityQueueMapping, productName=productName, ptpCompliance=ptpCompliance, eIRP=eIRP, searchState=searchState, signalStrengthRatio=signalStrengthRatio, vectorError=vectorError, userEnabled=userEnabled, tFTPServerPortNumber=tFTPServerPortNumber, telnetAccessEnabled=telnetAccessEnabled, transmitPower=transmitPower, bootVersion=bootVersion, sNMPTrapVersion=sNMPTrapVersion, ptp500=ptp500, licenceGroup=licenceGroup, sNTPSyncTrap=sNTPSyncTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, telecomsLoopback=telecomsLoopback, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dataPortStatus=dataPortStatus, cableLoss=cableLoss, licensedCapacity=licensedCapacity, bandwidthVariant=bandwidthVariant, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, dataPortDisabledWarning=dataPortDisabledWarning, dfs=dfs, receiveDataRate=receiveDataRate, useVLANForManagementInterfaces=useVLANForManagementInterfaces, dfsTableEntry=dfsTableEntry, transmitDataRate=transmitDataRate, sNMPCommunityString=sNMPCommunityString, sNMPTrapTable=sNMPTrapTable, tDDControl=tDDControl, maximumTransmitPower=maximumTransmitPower, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, configuration=configuration, phyStatus=phyStatus, notificationsGroup=notificationsGroup, encryptionAlgorithm=encryptionAlgorithm, wirelessLinkAvailability=wirelessLinkAvailability, regionCode=regionCode, ptmp=ptmp, receivePower=receivePower, receiveFreqMHz=receiveFreqMHz, vLANManagementVID=vLANManagementVID, pubStats=pubStats, encryptionGroup=encryptionGroup, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, softwareVersion=softwareVersion, sNTPServerIPAddress=sNTPServerIPAddress, vLANManagementPriority=vLANManagementPriority, userTableNumber=userTableNumber, linkSymmetry=linkSymmetry, sNMPPortNumber=sNMPPortNumber, receiveModulationMode=receiveModulationMode, sNMPTrapTableNumber=sNMPTrapTableNumber, versions=versions, encryption=encryption, channelBandwidth=channelBandwidth, dfsGroup=dfsGroup, cambium=cambium, configurationGroup=configurationGroup, aAAControlGroup=aAAControlGroup, dataPortStatusTrap=dataPortStatusTrap, daylightSaving=daylightSaving, hTTPPortNumber=hTTPPortNumber, vLANPriorityTable=vLANPriorityTable, sNTPState=sNTPState, vLANPriorityTableEntry=vLANPriorityTableEntry, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, sNMPTrapTableIndex=sNMPTrapTableIndex, dfsTable=dfsTable, syslogClientTrap=syslogClientTrap, sNMPv3Enable=sNMPv3Enable, telecomGroup=telecomGroup, siteName=siteName, licence=licence, latitude=latitude, ptp=ptp, hTTPAccessEnabled=hTTPAccessEnabled, secondarySoftwareVersion=secondarySoftwareVersion, groupID=groupID, sntp=sntp, dfsMeans=dfsMeans, hTTPSPortNumber=hTTPSPortNumber, systemClock=systemClock, timeZone=timeZone, transmitFreqMHz=transmitFreqMHz, longitude=longitude, syslogControlGroup=syslogControlGroup, sNMPEnabledTraps=sNMPEnabledTraps, tFTPServerIPAddress=tFTPServerIPAddress, transmitChannel=transmitChannel, tDDControlGroup=tDDControlGroup, accessMethod=accessMethod, dataPortAutoNegotiation=dataPortAutoNegotiation, phyStatusGroup=phyStatusGroup, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sMTPEmailAlert=sMTPEmailAlert, systemReset=systemReset, aggregateDataRate=aggregateDataRate, byteErrorRatio=byteErrorRatio, hardwareVersion=hardwareVersion, receiveModulationModeDetail=receiveModulationModeDetail, linkName=linkName, dataPortAutoMdix=dataPortAutoMdix, iPAddress=iPAddress, linkModeOptimizationMismatch=linkModeOptimizationMismatch, ptpGroups=ptpGroups, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, userTableIndex=userTableIndex, reset=reset, telecomsChannelStatus=telecomsChannelStatus, syslogLocalWrappedTrap=syslogLocalWrappedTrap, phyControlGroup=phyControlGroup, supplementaryGroup=supplementaryGroup, dfsPeaks=dfsPeaks, ethernet=ethernet, ethernetGroup=ethernetGroup, tDDSynchronizationMode=tDDSynchronizationMode, antennaGain=antennaGain, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, ethernetFiberSupport=ethernetFiberSupport, alarms=alarms, telecomsLoopbackTrap=telecomsLoopbackTrap, telecomsChannelStatusTrap=telecomsChannelStatusTrap, aAAControl=aAAControl, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, ptpTrapPrefix=ptpTrapPrefix, frequencyVariant=frequencyVariant, telecomsLineCode=telecomsLineCode, syslogControl=syslogControl, linkModeOptimisation=linkModeOptimisation, range=range, alarmsGroup=alarmsGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, userName=userName, installArmStateTrap=installArmStateTrap, tDDSynchronizationAlarm=tDDSynchronizationAlarm)
