...
# Produced by pysmi-1.1.3 at Mon Nov 22 19:22:26 2021
...
Gauge32, Counter32, iso, IpAddress, ModuleIdentity, Bits, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, Counter64, Unsigned32, TimeTicks, enterprises, Integer32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Counter32", "iso", "IpAddress", "ModuleIdentity", "Bits", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "Counter64", "Unsigned32", "TimeTicks", "enterprises", "Integer32", "NotificationType")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", ethernetGroup=ethernetGroup, licence=licence, tDDSynchronizationAlarm=tDDSynchronizationAlarm, systemReset=systemReset, installArmState=installArmState, secondarySoftwareVersion=secondarySoftwareVersion, reset=reset, versionsGroup=versionsGroup, dfsTableIndex=dfsTableIndex, smtpGroup=smtpGroup, masterSlaveMode=masterSlaveMode, signalStrengthRatio=signalStrengthRatio, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNMPEnabledTraps=sNMPEnabledTraps, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, ptp500=ptp500, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, receiveModulationMode=receiveModulationMode, transmitFreqMHz=transmitFreqMHz, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, dataPortDisabledWarning=dataPortDisabledWarning, capacityLicenseMismatch=capacityLicenseMismatch, phyControl=phyControl, linkSymmetry=linkSymmetry, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sNTPState=sNTPState, sNTPServerPortNumber=sNTPServerPortNumber, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, telnetAccessEnabled=telnetAccessEnabled, management=management, antennaGain=antennaGain, sNMPTrapTableIndex=sNMPTrapTableIndex, userName=userName, ptmp=ptmp, versions=versions, ptpGroups=ptpGroups, iPAddress=iPAddress, hardwareVersion=hardwareVersion, transmitPower=transmitPower, dfsPeaks=dfsPeaks, dataPortAutoNegotiation=dataPortAutoNegotiation, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, sMTPEmailAlert=sMTPEmailAlert, tFTPServerIPAddress=tFTPServerIPAddress, sNMPTrapIPAddress=sNMPTrapIPAddress, syslogClient=syslogClient, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, vLANPriorityTableNumber=vLANPriorityTableNumber, snmpControl=snmpControl, hTTPAccessEnabled=hTTPAccessEnabled, sMTPServerPortNumber=sMTPServerPortNumber, syslogLocalWrappedTrap=syslogLocalWrappedTrap, userEnabled=userEnabled, softwareVersion=softwareVersion, receiveDataRate=receiveDataRate, sNMPTrapTableNumber=sNMPTrapTableNumber, channelBandwidth=channelBandwidth, hTTPPortNumber=hTTPPortNumber, userTableIndex=userTableIndex, altitude=altitude, supplementaryGroup=supplementaryGroup, vLANPriorityTable=vLANPriorityTable, range=range, linkLoss=linkLoss, sNMPCommunityString=sNMPCommunityString, eIRP=eIRP, sNMPTrapPortNumber=sNMPTrapPortNumber, telecom=telecom, cableLoss=cableLoss, dataPortConfigurationMismatch=dataPortConfigurationMismatch, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, encryptionGroup=encryptionGroup, sNMPv3Enable=sNMPv3Enable, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, phyStatus=phyStatus, installStatus=installStatus, accessMethod=accessMethod, regionCode=regionCode, resetGroup=resetGroup, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, PYSNMP_MODULE_ID=cambium, noWirelessChannelAvailable=noWirelessChannelAvailable, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, syslogState=syslogState, dfsTableEntry=dfsTableEntry, dfsTable=dfsTable, pubStatsGroup=pubStatsGroup, dataPortAutoMdix=dataPortAutoMdix, bandwidthVariant=bandwidthVariant, alarmsGroup=alarmsGroup, aAAControl=aAAControl, subnetMask=subnetMask, userRole=userRole, linkModeOptimizationMismatch=linkModeOptimizationMismatch, siteName=siteName, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, sNMPPortNumber=sNMPPortNumber, snmpControlGroup=snmpControlGroup, aggregateDataRate=aggregateDataRate, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, dataPortStatus=dataPortStatus, linkModeOptimisation=linkModeOptimisation, userPassword=userPassword, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, unitOutOfCalibration=unitOutOfCalibration, vLANPriorityTableIndex=vLANPriorityTableIndex, hTTPSAccessEnabled=hTTPSAccessEnabled, maximumTransmitPower=maximumTransmitPower, dfsTableNumber=dfsTableNumber, telecomsChannelStatusTrap=telecomsChannelStatusTrap, telecomsLoopbackTrap=telecomsLoopbackTrap, searchState=searchState, remoteMaximumTransmitPower=remoteMaximumTransmitPower, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, phyStatusGroup=phyStatusGroup, productName=productName, notificationsGroup=notificationsGroup, alarms=alarms, remoteIPAddress=remoteIPAddress, telecomsLineCode=telecomsLineCode, incompatibleMasterAndSlave=incompatibleMasterAndSlave, receiveModulationModeDetail=receiveModulationModeDetail, wirelessLinkStatus=wirelessLinkStatus, sNTPServerIPAddress=sNTPServerIPAddress, syslogControl=syslogControl, receivePower=receivePower, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, daylightSaving=daylightSaving, linkName=linkName, userTable=userTable, systemClock=systemClock, ptpTrapPrefix=ptpTrapPrefix, telecomsLoopback=telecomsLoopback, tDDControlGroup=tDDControlGroup, transmitChannel=transmitChannel, managementGroup=managementGroup, enabledDiagnosticAlarms=enabledDiagnosticAlarms, tDDSynchronizationStatus=tDDSynchronizationStatus, receiveFreqKHz=receiveFreqKHz, licenceGroup=licenceGroup, encryption=encryption, aAAControlGroup=aAAControlGroup, sNTPSync=sNTPSync, groupID=groupID, timeZone=timeZone, bootVersion=bootVersion, smtp=smtp, syslogClientTrap=syslogClientTrap, vectorError=vectorError, wirelessLinkAvailability=wirelessLinkAvailability, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, frequencyVariant=frequencyVariant, telnetPortNumber=telnetPortNumber, dataPortWirelessDownAlert=dataPortWirelessDownAlert, constantPowerSpectralDensity=constantPowerSpectralDensity, dfsMeans=dfsMeans, sMTPServerIPAddress=sMTPServerIPAddress, transmitModulationMode=transmitModulationMode, latitude=latitude, productVariant=productVariant, regionCodeTrap=regionCodeTrap, configuration=configuration, syslogControlGroup=syslogControlGroup, transmitFreqKHz=transmitFreqKHz, longitude=longitude, dataPortStatusTrap=dataPortStatusTrap, phyControlGroup=phyControlGroup, syslogStateTrap=syslogStateTrap, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, incompatibleRegionCodes=incompatibleRegionCodes, channelChangeTrap=channelChangeTrap, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, receiveChannel=receiveChannel, dfs=dfs, hTTPSPortNumber=hTTPSPortNumber, ethernetFiberSupport=ethernetFiberSupport, vLANPriorityTableEntry=vLANPriorityTableEntry, dfsGroup=dfsGroup, sntpGroup=sntpGroup, telecomGroup=telecomGroup, tDDSynchronizationMode=tDDSynchronizationMode, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, installArmStateTrap=installArmStateTrap, sNMPTrapVersion=sNMPTrapVersion, transmitDataRate=transmitDataRate, telecomsInterface=telecomsInterface, encryptionAlgorithm=encryptionAlgorithm, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, sNMPTrapTable=sNMPTrapTable, gatewayIPAddress=gatewayIPAddress, sMTPSourceEmailAddress=sMTPSourceEmailAddress, ptp=ptp, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, ptpTraps=ptpTraps, ptpCompliance=ptpCompliance, telecomsChannelStatus=telecomsChannelStatus, userTableEntry=userTableEntry, tFTPServerPortNumber=tFTPServerPortNumber, installStatusTrap=installStatusTrap, pubStats=pubStats, sNTPSyncTrap=sNTPSyncTrap, cambium=cambium, useVLANForManagementInterfaces=useVLANForManagementInterfaces, sntp=sntp, configurationGroup=configurationGroup, userTableNumber=userTableNumber, sNTPLastSync=sNTPLastSync, licensedCapacity=licensedCapacity, sNMPTrapTableEntry=sNMPTrapTableEntry, sNTPPollInterval=sNTPPollInterval, telecomsCableLength=telecomsCableLength, receiveFreqMHz=receiveFreqMHz, supplementary=supplementary, byteErrorRatio=byteErrorRatio, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, vLANManagementVID=vLANManagementVID, tDDControl=tDDControl, targetMACAddress=targetMACAddress, ethernet=ethernet, vLANManagementPriority=vLANManagementPriority, sMTPEnabledMessages=sMTPEnabledMessages)
