...
# Produced by pysmi-1.1.10 at Mon Oct 30 02:13:23 2023
...
MibIdentifier, IpAddress, ObjectIdentity, Unsigned32, iso, TimeTicks, Bits, Counter64, enterprises, NotificationType, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, Gauge32, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "IpAddress", "ObjectIdentity", "Unsigned32", "iso", "TimeTicks", "Bits", "Counter64", "enterprises", "NotificationType", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "Gauge32", "Counter32")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", sNTPPollInterval=sNTPPollInterval, resetGroup=resetGroup, vectorError=vectorError, userTableNumber=userTableNumber, sNMPTrapPortNumber=sNMPTrapPortNumber, receiveDataRate=receiveDataRate, userTableEntry=userTableEntry, signalStrengthRatio=signalStrengthRatio, sMTPEmailAlert=sMTPEmailAlert, snmpControlGroup=snmpControlGroup, receiveChannel=receiveChannel, sNMPEnabledTraps=sNMPEnabledTraps, ptpTraps=ptpTraps, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, installStatusTrap=installStatusTrap, tDDControl=tDDControl, ptpCompliance=ptpCompliance, alarms=alarms, telecomsLineCode=telecomsLineCode, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, versions=versions, telecomGroup=telecomGroup, cableLoss=cableLoss, telecomsLoopback=telecomsLoopback, ethernetGroup=ethernetGroup, licensedCapacity=licensedCapacity, targetMACAddress=targetMACAddress, range=range, telecomsInterface=telecomsInterface, wirelessLinkAvailability=wirelessLinkAvailability, vLANPriorityTable=vLANPriorityTable, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNMPPortNumber=sNMPPortNumber, transmitFreqKHz=transmitFreqKHz, dataPortStatusTrap=dataPortStatusTrap, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sMTPServerPortNumber=sMTPServerPortNumber, cambium=cambium, dataPortAutoNegotiation=dataPortAutoNegotiation, transmitFreqMHz=transmitFreqMHz, dfsTable=dfsTable, dfsTableIndex=dfsTableIndex, wirelessLinkStatus=wirelessLinkStatus, maximumTransmitPower=maximumTransmitPower, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, incompatibleRegionCodes=incompatibleRegionCodes, sNTPSync=sNTPSync, bandwidthVariant=bandwidthVariant, unitOutOfCalibration=unitOutOfCalibration, sNMPCommunityString=sNMPCommunityString, secondarySoftwareVersion=secondarySoftwareVersion, encryptionGroup=encryptionGroup, smtp=smtp, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, userTableIndex=userTableIndex, dfs=dfs, byteErrorRatio=byteErrorRatio, telecomsChannelStatusTrap=telecomsChannelStatusTrap, daylightSaving=daylightSaving, encryption=encryption, gatewayIPAddress=gatewayIPAddress, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, syslogControl=syslogControl, hardwareVersion=hardwareVersion, pubStats=pubStats, syslogClient=syslogClient, systemReset=systemReset, ptpTrapPrefix=ptpTrapPrefix, phyStatusGroup=phyStatusGroup, reset=reset, sNTPServerPortNumber=sNTPServerPortNumber, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, phyControl=phyControl, tDDSynchronizationAlarm=tDDSynchronizationAlarm, phyStatus=phyStatus, smtpGroup=smtpGroup, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, hTTPSAccessEnabled=hTTPSAccessEnabled, hTTPSPortNumber=hTTPSPortNumber, userRole=userRole, versionsGroup=versionsGroup, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, notificationsGroup=notificationsGroup, syslogState=syslogState, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, tDDSynchronizationMode=tDDSynchronizationMode, installArmState=installArmState, latitude=latitude, receiveFreqKHz=receiveFreqKHz, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, regionCode=regionCode, accessMethod=accessMethod, dfsTableNumber=dfsTableNumber, encryptionAlgorithm=encryptionAlgorithm, longitude=longitude, syslogStateTrap=syslogStateTrap, dataPortAutoMdix=dataPortAutoMdix, incompatibleMasterAndSlave=incompatibleMasterAndSlave, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, receiveModulationModeDetail=receiveModulationModeDetail, ptp=ptp, userEnabled=userEnabled, channelChangeTrap=channelChangeTrap, licence=licence, dfsTableEntry=dfsTableEntry, installArmStateTrap=installArmStateTrap, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, receivePower=receivePower, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, dfsGroup=dfsGroup, telnetPortNumber=telnetPortNumber, syslogClientTrap=syslogClientTrap, transmitChannel=transmitChannel, dataPortWirelessDownAlert=dataPortWirelessDownAlert, linkLoss=linkLoss, phyControlGroup=phyControlGroup, timeZone=timeZone, tDDSynchronizationStatus=tDDSynchronizationStatus, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sNTPSyncTrap=sNTPSyncTrap, vLANManagementPriority=vLANManagementPriority, iPAddress=iPAddress, telecomsCableLength=telecomsCableLength, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dfsMeans=dfsMeans, tDDControlGroup=tDDControlGroup, telnetAccessEnabled=telnetAccessEnabled, ethernetFiberSupport=ethernetFiberSupport, userName=userName, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, aggregateDataRate=aggregateDataRate, pubStatsGroup=pubStatsGroup, managementGroup=managementGroup, dataPortStatus=dataPortStatus, tFTPServerPortNumber=tFTPServerPortNumber, systemClock=systemClock, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, management=management, masterSlaveMode=masterSlaveMode, siteName=siteName, groupID=groupID, useVLANForManagementInterfaces=useVLANForManagementInterfaces, vLANPriorityTableIndex=vLANPriorityTableIndex, frequencyVariant=frequencyVariant, userPassword=userPassword, transmitPower=transmitPower, sMTPServerIPAddress=sMTPServerIPAddress, telecom=telecom, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, aAAControl=aAAControl, sNMPTrapVersion=sNMPTrapVersion, linkModeOptimisation=linkModeOptimisation, receiveFreqMHz=receiveFreqMHz, sntp=sntp, configurationGroup=configurationGroup, transmitModulationMode=transmitModulationMode, ethernet=ethernet, noWirelessChannelAvailable=noWirelessChannelAvailable, altitude=altitude, channelBandwidth=channelBandwidth, snmpControl=snmpControl, productName=productName, sNMPv3Enable=sNMPv3Enable, sNTPLastSync=sNTPLastSync, aAAControlGroup=aAAControlGroup, sNTPState=sNTPState, transmitDataRate=transmitDataRate, licenceGroup=licenceGroup, dfsPeaks=dfsPeaks, PYSNMP_MODULE_ID=cambium, telecomsChannelStatus=telecomsChannelStatus, productVariant=productVariant, configuration=configuration, hTTPAccessEnabled=hTTPAccessEnabled, sMTPSourceEmailAddress=sMTPSourceEmailAddress, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, capacityLicenseMismatch=capacityLicenseMismatch, userTable=userTable, sNTPServerIPAddress=sNTPServerIPAddress, receiveModulationMode=receiveModulationMode, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPTrapTableIndex=sNMPTrapTableIndex, vLANPriorityTableEntry=vLANPriorityTableEntry, antennaGain=antennaGain, eIRP=eIRP, dataPortDisabledWarning=dataPortDisabledWarning, sNMPTrapTableNumber=sNMPTrapTableNumber, searchState=searchState, vLANManagementVID=vLANManagementVID, ptmp=ptmp, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sMTPEnabledMessages=sMTPEnabledMessages, ptp500=ptp500, constantPowerSpectralDensity=constantPowerSpectralDensity, alarmsGroup=alarmsGroup, ptpGroups=ptpGroups, hTTPPortNumber=hTTPPortNumber, subnetMask=subnetMask, supplementaryGroup=supplementaryGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, syslogControlGroup=syslogControlGroup, softwareVersion=softwareVersion, installStatus=installStatus, sntpGroup=sntpGroup, vLANPriorityTableNumber=vLANPriorityTableNumber, tFTPServerIPAddress=tFTPServerIPAddress, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sNMPTrapTable=sNMPTrapTable, remoteIPAddress=remoteIPAddress, linkSymmetry=linkSymmetry, sNMPTrapTableEntry=sNMPTrapTableEntry, telecomsLoopbackTrap=telecomsLoopbackTrap, sNMPTrapIPAddress=sNMPTrapIPAddress, supplementary=supplementary, syslogLocalWrappedTrap=syslogLocalWrappedTrap, regionCodeTrap=regionCodeTrap, bootVersion=bootVersion, linkName=linkName)
