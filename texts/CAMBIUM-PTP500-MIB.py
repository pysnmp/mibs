...
TimeTicks, iso, enterprises, Counter32, ObjectIdentity, Counter64, ModuleIdentity, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Unsigned32, MibIdentifier, Integer32, IpAddress, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "iso", "enterprises", "Counter32", "ObjectIdentity", "Counter64", "ModuleIdentity", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Unsigned32", "MibIdentifier", "Integer32", "IpAddress", "NotificationType")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", licence=licence, vLANPriorityTableNumber=vLANPriorityTableNumber, phyStatus=phyStatus, cableLoss=cableLoss, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, userName=userName, longitude=longitude, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, encryption=encryption, range=range, sntp=sntp, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, syslogControlGroup=syslogControlGroup, dataPortWirelessDownAlert=dataPortWirelessDownAlert, installStatus=installStatus, sMTPServerIPAddress=sMTPServerIPAddress, targetMACAddress=targetMACAddress, ethernetFiberSupport=ethernetFiberSupport, hTTPSAccessEnabled=hTTPSAccessEnabled, linkModeOptimizationMismatch=linkModeOptimizationMismatch, installArmStateTrap=installArmStateTrap, sNMPPortNumber=sNMPPortNumber, dfsTableIndex=dfsTableIndex, transmitDataRate=transmitDataRate, bandwidthVariant=bandwidthVariant, wirelessLinkAvailability=wirelessLinkAvailability, alarms=alarms, hTTPAccessEnabled=hTTPAccessEnabled, sNMPTrapTableEntry=sNMPTrapTableEntry, encryptionGroup=encryptionGroup, userTable=userTable, sNMPTrapIPAddress=sNMPTrapIPAddress, ptp500=ptp500, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNTPSync=sNTPSync, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, encryptionAlgorithm=encryptionAlgorithm, sNMPEnabledTraps=sNMPEnabledTraps, licenceGroup=licenceGroup, masterSlaveMode=masterSlaveMode, tDDSynchronizationAlarm=tDDSynchronizationAlarm, timeZone=timeZone, accessMethod=accessMethod, userTableNumber=userTableNumber, secondarySoftwareVersion=secondarySoftwareVersion, phyStatusGroup=phyStatusGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, latitude=latitude, hardwareVersion=hardwareVersion, telecomsLoopback=telecomsLoopback, transmitFreqMHz=transmitFreqMHz, siteName=siteName, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, userEnabled=userEnabled, syslogStateTrap=syslogStateTrap, userRole=userRole, telnetAccessEnabled=telnetAccessEnabled, dfsTableNumber=dfsTableNumber, telecomsLineCode=telecomsLineCode, ethernetGroup=ethernetGroup, tFTPServerPortNumber=tFTPServerPortNumber, constantPowerSpectralDensity=constantPowerSpectralDensity, receiveFreqMHz=receiveFreqMHz, systemClock=systemClock, ptmp=ptmp, sNMPCommunityString=sNMPCommunityString, bootVersion=bootVersion, antennaGain=antennaGain, ptpGroups=ptpGroups, dfs=dfs, searchState=searchState, configurationGroup=configurationGroup, byteErrorRatio=byteErrorRatio, vLANManagementVID=vLANManagementVID, sNTPState=sNTPState, receiveChannel=receiveChannel, dfsTableEntry=dfsTableEntry, groupID=groupID, vLANPriorityTable=vLANPriorityTable, snmpControl=snmpControl, softwareVersion=softwareVersion, ethernet=ethernet, syslogClientTrap=syslogClientTrap, supplementary=supplementary, versionsGroup=versionsGroup, syslogLocalWrappedTrap=syslogLocalWrappedTrap, vectorError=vectorError, cambium=cambium, incompatibleMasterAndSlave=incompatibleMasterAndSlave, eIRP=eIRP, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, sNMPTrapVersion=sNMPTrapVersion, subnetMask=subnetMask, linkSymmetry=linkSymmetry, pubStats=pubStats, tFTPServerIPAddress=tFTPServerIPAddress, vLANPriorityTableEntry=vLANPriorityTableEntry, vLANPriorityQueueMapping=vLANPriorityQueueMapping, receiveModulationModeDetail=receiveModulationModeDetail, aggregateDataRate=aggregateDataRate, ptp=ptp, dataPortAutoMdix=dataPortAutoMdix, syslogControl=syslogControl, transmitFreqKHz=transmitFreqKHz, regionCodeTrap=regionCodeTrap, snmpControlGroup=snmpControlGroup, sNTPSyncTrap=sNTPSyncTrap, vLANPriorityTableIndex=vLANPriorityTableIndex, productName=productName, sNTPServerIPAddress=sNTPServerIPAddress, transmitChannel=transmitChannel, management=management, receiveDataRate=receiveDataRate, tDDControl=tDDControl, linkName=linkName, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, telecomsLoopbackTrap=telecomsLoopbackTrap, capacityLicenseMismatch=capacityLicenseMismatch, systemReset=systemReset, tDDSynchronizationMode=tDDSynchronizationMode, ptpCompliance=ptpCompliance, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, userTableEntry=userTableEntry, telecomsCableLength=telecomsCableLength, signalStrengthRatio=signalStrengthRatio, transmitModulationMode=transmitModulationMode, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, reset=reset, iPAddress=iPAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, regionCode=regionCode, ptpTraps=ptpTraps, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sMTPSourceEmailAddress=sMTPSourceEmailAddress, receiveFreqKHz=receiveFreqKHz, maximumTransmitPower=maximumTransmitPower, channelBandwidth=channelBandwidth, sNMPv3Enable=sNMPv3Enable, remoteMaximumTransmitPower=remoteMaximumTransmitPower, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, aAAControlGroup=aAAControlGroup, sNTPLastSync=sNTPLastSync, telecomsChannelStatus=telecomsChannelStatus, sNMPTrapTableNumber=sNMPTrapTableNumber, vLANManagementPriority=vLANManagementPriority, installStatusTrap=installStatusTrap, gatewayIPAddress=gatewayIPAddress, hTTPPortNumber=hTTPPortNumber, sNTPServerPortNumber=sNTPServerPortNumber, tDDControlGroup=tDDControlGroup, dfsPeaks=dfsPeaks, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, userPassword=userPassword, useVLANForManagementInterfaces=useVLANForManagementInterfaces, incompatibleRegionCodes=incompatibleRegionCodes, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dfsGroup=dfsGroup, linkModeOptimisation=linkModeOptimisation, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, sNMPTrapTableIndex=sNMPTrapTableIndex, tDDSynchronizationStatus=tDDSynchronizationStatus, telecom=telecom, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, supplementaryGroup=supplementaryGroup, remoteIPAddress=remoteIPAddress, notificationsGroup=notificationsGroup, sNTPPollInterval=sNTPPollInterval, noWirelessChannelAvailable=noWirelessChannelAvailable, versions=versions, daylightSaving=daylightSaving, dataPortStatus=dataPortStatus, productVariant=productVariant, dataPortStatusTrap=dataPortStatusTrap, receiveModulationMode=receiveModulationMode, dataPortDisabledWarning=dataPortDisabledWarning, telecomGroup=telecomGroup, receivePower=receivePower, aAAControl=aAAControl, pubStatsGroup=pubStatsGroup, resetGroup=resetGroup, smtpGroup=smtpGroup, telnetPortNumber=telnetPortNumber, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, configuration=configuration, transmitPower=transmitPower, syslogClient=syslogClient, sMTPEmailAlert=sMTPEmailAlert, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, channelChangeTrap=channelChangeTrap, telecomsInterface=telecomsInterface, licensedCapacity=licensedCapacity, phyControlGroup=phyControlGroup, hTTPSPortNumber=hTTPSPortNumber, wirelessLinkStatus=wirelessLinkStatus, installArmState=installArmState, userTableIndex=userTableIndex, dfsMeans=dfsMeans, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, sNMPTrapTable=sNMPTrapTable, frequencyVariant=frequencyVariant, altitude=altitude, phyControl=phyControl, PYSNMP_MODULE_ID=cambium, sntpGroup=sntpGroup, dfsTable=dfsTable, linkLoss=linkLoss, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sMTPEnabledMessages=sMTPEnabledMessages, telecomsChannelStatusTrap=telecomsChannelStatusTrap, managementGroup=managementGroup, syslogState=syslogState, unitOutOfCalibration=unitOutOfCalibration, ptpTrapPrefix=ptpTrapPrefix, alarmsGroup=alarmsGroup, smtp=smtp, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, sMTPServerPortNumber=sMTPServerPortNumber)
