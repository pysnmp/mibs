...
# Produced by pysmi-1.1.8 at Fri Jan  7 15:13:35 2022
...
ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, ObjectIdentity, Integer32, MibIdentifier, Counter64, TimeTicks, Unsigned32, Gauge32, enterprises, Bits, IpAddress, NotificationType, iso = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "ObjectIdentity", "Integer32", "MibIdentifier", "Counter64", "TimeTicks", "Unsigned32", "Gauge32", "enterprises", "Bits", "IpAddress", "NotificationType", "iso")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", phyControl=phyControl, vectorError=vectorError, ptpGroups=ptpGroups, incompatibleRegionCodes=incompatibleRegionCodes, syslogClient=syslogClient, targetMACAddress=targetMACAddress, ptpTraps=ptpTraps, notificationsGroup=notificationsGroup, encryptionAlgorithm=encryptionAlgorithm, vLANPriorityQueueMapping=vLANPriorityQueueMapping, alarmsGroup=alarmsGroup, encryption=encryption, sNMPTrapTable=sNMPTrapTable, sNMPCommunityString=sNMPCommunityString, channelBandwidth=channelBandwidth, hTTPSAccessEnabled=hTTPSAccessEnabled, tFTPServerPortNumber=tFTPServerPortNumber, sNMPTrapIPAddress=sNMPTrapIPAddress, subnetMask=subnetMask, receiveChannel=receiveChannel, pubStatsGroup=pubStatsGroup, telnetAccessEnabled=telnetAccessEnabled, userTableNumber=userTableNumber, systemClock=systemClock, linkSymmetry=linkSymmetry, softwareVersion=softwareVersion, snmpControlGroup=snmpControlGroup, telecomGroup=telecomGroup, telecomsChannelStatus=telecomsChannelStatus, range=range, aAAControl=aAAControl, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, alarms=alarms, sNMPPortNumber=sNMPPortNumber, sNTPSync=sNTPSync, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, hTTPPortNumber=hTTPPortNumber, dfsTable=dfsTable, installStatusTrap=installStatusTrap, telecomsLoopback=telecomsLoopback, aAAControlGroup=aAAControlGroup, sNMPTrapVersion=sNMPTrapVersion, masterSlaveMode=masterSlaveMode, reset=reset, linkLoss=linkLoss, altitude=altitude, pubStats=pubStats, productVariant=productVariant, transmitFreqKHz=transmitFreqKHz, sNMPTrapTableNumber=sNMPTrapTableNumber, systemReset=systemReset, ptmp=ptmp, sNMPTrapTableEntry=sNMPTrapTableEntry, PYSNMP_MODULE_ID=cambium, dfsTableIndex=dfsTableIndex, phyStatusGroup=phyStatusGroup, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, userRole=userRole, tFTPServerIPAddress=tFTPServerIPAddress, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, supplementaryGroup=supplementaryGroup, dfs=dfs, tDDControlGroup=tDDControlGroup, resetGroup=resetGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, transmitModulationMode=transmitModulationMode, sNTPServerPortNumber=sNTPServerPortNumber, sMTPServerIPAddress=sMTPServerIPAddress, userPassword=userPassword, transmitDataRate=transmitDataRate, smtp=smtp, regionCodeTrap=regionCodeTrap, signalStrengthRatio=signalStrengthRatio, productName=productName, sNTPState=sNTPState, transmitFreqMHz=transmitFreqMHz, ptp=ptp, syslogStateTrap=syslogStateTrap, dataPortAutoNegotiation=dataPortAutoNegotiation, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, managementGroup=managementGroup, iPAddress=iPAddress, vLANPriorityTableIndex=vLANPriorityTableIndex, bandwidthVariant=bandwidthVariant, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, dfsTableEntry=dfsTableEntry, sNTPPollInterval=sNTPPollInterval, management=management, vLANPriorityTableEntry=vLANPriorityTableEntry, eIRP=eIRP, syslogControl=syslogControl, transmitPower=transmitPower, wirelessLinkAvailability=wirelessLinkAvailability, ptp500=ptp500, timeZone=timeZone, transmitChannel=transmitChannel, syslogLocalWrappedTrap=syslogLocalWrappedTrap, tDDSynchronizationStatus=tDDSynchronizationStatus, daylightSaving=daylightSaving, vLANPriorityTable=vLANPriorityTable, cableLoss=cableLoss, telecom=telecom, dfsMeans=dfsMeans, useVLANForManagementInterfaces=useVLANForManagementInterfaces, capacityLicenseMismatch=capacityLicenseMismatch, installArmStateTrap=installArmStateTrap, phyStatus=phyStatus, tDDControl=tDDControl, sMTPEmailAlert=sMTPEmailAlert, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sMTPEnabledMessages=sMTPEnabledMessages, sNMPv3Enable=sNMPv3Enable, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, licenceGroup=licenceGroup, versionsGroup=versionsGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, remoteMaximumTransmitPower=remoteMaximumTransmitPower, dataPortAutoMdix=dataPortAutoMdix, sNMPTrapTableIndex=sNMPTrapTableIndex, sNTPSyncTrap=sNTPSyncTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, noWirelessChannelAvailable=noWirelessChannelAvailable, userEnabled=userEnabled, hTTPAccessEnabled=hTTPAccessEnabled, aggregateDataRate=aggregateDataRate, latitude=latitude, snmpControl=snmpControl, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, telecomsChannelStatusTrap=telecomsChannelStatusTrap, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, telecomsLineCode=telecomsLineCode, incompatibleMasterAndSlave=incompatibleMasterAndSlave, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, installStatus=installStatus, unitOutOfCalibration=unitOutOfCalibration, dataPortConfigurationMismatch=dataPortConfigurationMismatch, telecomsLoopbackTrap=telecomsLoopbackTrap, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, ptpCompliance=ptpCompliance, receiveModulationMode=receiveModulationMode, dataPortStatusTrap=dataPortStatusTrap, groupID=groupID, searchState=searchState, linkName=linkName, wirelessLinkStatus=wirelessLinkStatus, sMTPServerPortNumber=sMTPServerPortNumber, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, receivePower=receivePower, dfsGroup=dfsGroup, gatewayIPAddress=gatewayIPAddress, constantPowerSpectralDensity=constantPowerSpectralDensity, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, tDDSynchronizationMode=tDDSynchronizationMode, userName=userName, ethernet=ethernet, secondarySoftwareVersion=secondarySoftwareVersion, cambium=cambium, dataPortDisabledWarning=dataPortDisabledWarning, maximumTransmitPower=maximumTransmitPower, regionCode=regionCode, ethernetGroup=ethernetGroup, installArmState=installArmState, sNMPTrapPortNumber=sNMPTrapPortNumber, versions=versions, encryptionGroup=encryptionGroup, sMTPSourceEmailAddress=sMTPSourceEmailAddress, syslogControlGroup=syslogControlGroup, hardwareVersion=hardwareVersion, telecomsCableLength=telecomsCableLength, sntpGroup=sntpGroup, channelChangeTrap=channelChangeTrap, phyControlGroup=phyControlGroup, receiveDataRate=receiveDataRate, licensedCapacity=licensedCapacity, receiveModulationModeDetail=receiveModulationModeDetail, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, vLANManagementPriority=vLANManagementPriority, sntp=sntp, accessMethod=accessMethod, sNTPLastSync=sNTPLastSync, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNMPEnabledTraps=sNMPEnabledTraps, syslogClientTrap=syslogClientTrap, userTable=userTable, userTableIndex=userTableIndex, userTableEntry=userTableEntry, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, dataPortStatus=dataPortStatus, antennaGain=antennaGain, longitude=longitude, dfsPeaks=dfsPeaks, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, ptpTrapPrefix=ptpTrapPrefix, hTTPSPortNumber=hTTPSPortNumber, linkModeOptimisation=linkModeOptimisation, configurationGroup=configurationGroup, supplementary=supplementary, linkModeOptimizationMismatch=linkModeOptimizationMismatch, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, siteName=siteName, remoteIPAddress=remoteIPAddress, syslogState=syslogState, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, enabledDiagnosticAlarms=enabledDiagnosticAlarms, licence=licence, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, byteErrorRatio=byteErrorRatio, ethernetFiberSupport=ethernetFiberSupport, frequencyVariant=frequencyVariant, smtpGroup=smtpGroup, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, vLANPriorityTableNumber=vLANPriorityTableNumber, receiveFreqMHz=receiveFreqMHz, bootVersion=bootVersion, configuration=configuration, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNTPServerIPAddress=sNTPServerIPAddress, vLANManagementVID=vLANManagementVID, dfsTableNumber=dfsTableNumber, receiveFreqKHz=receiveFreqKHz, telecomsInterface=telecomsInterface, telnetPortNumber=telnetPortNumber)
