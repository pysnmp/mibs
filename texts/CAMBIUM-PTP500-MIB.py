...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", sNTPPollInterval=sNTPPollInterval, sMTPServerPortNumber=sMTPServerPortNumber, versions=versions, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, phyControl=phyControl, dfsMeans=dfsMeans, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, bootVersion=bootVersion, sNMPTrapIPAddress=sNMPTrapIPAddress, productVariant=productVariant, userName=userName, aggregateDataRate=aggregateDataRate, vLANPriorityTableEntry=vLANPriorityTableEntry, sNMPTrapPortNumber=sNMPTrapPortNumber, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, userTableIndex=userTableIndex, telecomsChannelStatus=telecomsChannelStatus, vLANPriorityTableIndex=vLANPriorityTableIndex, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, phyStatusGroup=phyStatusGroup, channelBandwidth=channelBandwidth, siteName=siteName, remoteMaximumTransmitPower=remoteMaximumTransmitPower, altitude=altitude, antennaGain=antennaGain, systemClock=systemClock, hTTPSAccessEnabled=hTTPSAccessEnabled, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, notificationsGroup=notificationsGroup, tFTPServerPortNumber=tFTPServerPortNumber, telecomsLoopbackTrap=telecomsLoopbackTrap, hTTPSPortNumber=hTTPSPortNumber, constantPowerSpectralDensity=constantPowerSpectralDensity, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, regionCodeTrap=regionCodeTrap, iPAddress=iPAddress, telecom=telecom, maximumTransmitPower=maximumTransmitPower, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, receiveModulationMode=receiveModulationMode, searchState=searchState, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sNMPPortNumber=sNMPPortNumber, sNTPServerPortNumber=sNTPServerPortNumber, transmitModulationMode=transmitModulationMode, installStatus=installStatus, gatewayIPAddress=gatewayIPAddress, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, tFTPServerIPAddress=tFTPServerIPAddress, tDDControlGroup=tDDControlGroup, licensedCapacity=licensedCapacity, userPassword=userPassword, bandwidthVariant=bandwidthVariant, pubStats=pubStats, softwareVersion=softwareVersion, useVLANForManagementInterfaces=useVLANForManagementInterfaces, sMTPSourceEmailAddress=sMTPSourceEmailAddress, encryptionGroup=encryptionGroup, telnetPortNumber=telnetPortNumber, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, ptpTraps=ptpTraps, receiveFreqMHz=receiveFreqMHz, vLANPriorityTableNumber=vLANPriorityTableNumber, transmitDataRate=transmitDataRate, dfsTable=dfsTable, linkSymmetry=linkSymmetry, ethernet=ethernet, licenceGroup=licenceGroup, sntpGroup=sntpGroup, receiveDataRate=receiveDataRate, wirelessLinkAvailability=wirelessLinkAvailability, userEnabled=userEnabled, supplementaryGroup=supplementaryGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, systemReset=systemReset, syslogControl=syslogControl, userTableEntry=userTableEntry, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, userRole=userRole, dataPortAutoMdix=dataPortAutoMdix, sNTPSyncTrap=sNTPSyncTrap, vLANManagementPriority=vLANManagementPriority, supplementary=supplementary, telecomsInterface=telecomsInterface, vLANPriorityTable=vLANPriorityTable, sNMPTrapTableNumber=sNMPTrapTableNumber, dfsGroup=dfsGroup, wirelessLinkStatus=wirelessLinkStatus, byteErrorRatio=byteErrorRatio, alarmsGroup=alarmsGroup, installArmState=installArmState, smtp=smtp, configuration=configuration, frequencyVariant=frequencyVariant, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, sNMPTrapTableIndex=sNMPTrapTableIndex, configurationGroup=configurationGroup, telecomsCableLength=telecomsCableLength, sMTPEnabledMessages=sMTPEnabledMessages, incompatibleRegionCodes=incompatibleRegionCodes, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sNMPCommunityString=sNMPCommunityString, telnetAccessEnabled=telnetAccessEnabled, linkModeOptimisation=linkModeOptimisation, licence=licence, linkLoss=linkLoss, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, versionsGroup=versionsGroup, snmpControlGroup=snmpControlGroup, dataPortStatus=dataPortStatus, sNMPTrapVersion=sNMPTrapVersion, telecomsLoopback=telecomsLoopback, vectorError=vectorError, receivePower=receivePower, receiveChannel=receiveChannel, targetMACAddress=targetMACAddress, vLANManagementVID=vLANManagementVID, latitude=latitude, alarms=alarms, snmpControl=snmpControl, management=management, unitOutOfCalibration=unitOutOfCalibration, signalStrengthRatio=signalStrengthRatio, telecomsChannelStatusTrap=telecomsChannelStatusTrap, ptp500=ptp500, reset=reset, tDDControl=tDDControl, dfsTableIndex=dfsTableIndex, dataPortConfigurationMismatch=dataPortConfigurationMismatch, installStatusTrap=installStatusTrap, ethernetFiberSupport=ethernetFiberSupport, sNMPEnabledTraps=sNMPEnabledTraps, telecomGroup=telecomGroup, eIRP=eIRP, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, ptpCompliance=ptpCompliance, dfsPeaks=dfsPeaks, ptpTrapPrefix=ptpTrapPrefix, syslogClientTrap=syslogClientTrap, sNTPState=sNTPState, range=range, telecomsLineCode=telecomsLineCode, hardwareVersion=hardwareVersion, phyStatus=phyStatus, aAAControlGroup=aAAControlGroup, userTable=userTable, ptmp=ptmp, transmitChannel=transmitChannel, syslogClient=syslogClient, managementGroup=managementGroup, encryption=encryption, linkName=linkName, dfs=dfs, PYSNMP_MODULE_ID=cambium, smtpGroup=smtpGroup, tDDSynchronizationMode=tDDSynchronizationMode, dataPortWirelessDownAlert=dataPortWirelessDownAlert, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, regionCode=regionCode, dfsTableEntry=dfsTableEntry, groupID=groupID, aAAControl=aAAControl, secondarySoftwareVersion=secondarySoftwareVersion, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, syslogState=syslogState, ptp=ptp, pubStatsGroup=pubStatsGroup, channelChangeTrap=channelChangeTrap, sNTPSync=sNTPSync, dataPortAutoNegotiation=dataPortAutoNegotiation, transmitFreqKHz=transmitFreqKHz, phyControlGroup=phyControlGroup, vLANPriorityQueueMapping=vLANPriorityQueueMapping, subnetMask=subnetMask, resetGroup=resetGroup, productName=productName, sNMPTrapTable=sNMPTrapTable, receiveFreqKHz=receiveFreqKHz, dataPortDisabledWarning=dataPortDisabledWarning, sMTPServerIPAddress=sMTPServerIPAddress, transmitPower=transmitPower, ethernetGroup=ethernetGroup, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, linkModeOptimizationMismatch=linkModeOptimizationMismatch, tDDSynchronizationAlarm=tDDSynchronizationAlarm, daylightSaving=daylightSaving, tDDSynchronizationStatus=tDDSynchronizationStatus, sntp=sntp, remoteIPAddress=remoteIPAddress, longitude=longitude, ptpGroups=ptpGroups, userTableNumber=userTableNumber, noWirelessChannelAvailable=noWirelessChannelAvailable, sMTPEmailAlert=sMTPEmailAlert, enabledDiagnosticAlarms=enabledDiagnosticAlarms, installArmStateTrap=installArmStateTrap, syslogStateTrap=syslogStateTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sNMPv3Enable=sNMPv3Enable, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, transmitFreqMHz=transmitFreqMHz, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, hTTPPortNumber=hTTPPortNumber, timeZone=timeZone, capacityLicenseMismatch=capacityLicenseMismatch, encryptionAlgorithm=encryptionAlgorithm, sNMPTrapTableEntry=sNMPTrapTableEntry, cambium=cambium, hTTPAccessEnabled=hTTPAccessEnabled, dfsTableNumber=dfsTableNumber, syslogControlGroup=syslogControlGroup, sNTPServerIPAddress=sNTPServerIPAddress, dataPortStatusTrap=dataPortStatusTrap, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNTPLastSync=sNTPLastSync, receiveModulationModeDetail=receiveModulationModeDetail, cableLoss=cableLoss, masterSlaveMode=masterSlaveMode, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, accessMethod=accessMethod)
