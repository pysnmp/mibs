...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, ptpGroups=ptpGroups, licenceGroup=licenceGroup, vectorError=vectorError, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, sNTPSyncTrap=sNTPSyncTrap, sNTPState=sNTPState, vLANPriorityTableIndex=vLANPriorityTableIndex, transmitFreqKHz=transmitFreqKHz, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, telecomsCableLength=telecomsCableLength, sntp=sntp, telecomsLoopback=telecomsLoopback, capacityLicenseMismatch=capacityLicenseMismatch, hTTPSPortNumber=hTTPSPortNumber, noWirelessChannelAvailable=noWirelessChannelAvailable, notificationsGroup=notificationsGroup, linkLoss=linkLoss, hardwareVersion=hardwareVersion, dfsPeaks=dfsPeaks, smtp=smtp, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, bandwidthVariant=bandwidthVariant, alarms=alarms, transmitPower=transmitPower, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sNMPTrapPortNumber=sNMPTrapPortNumber, sNTPServerIPAddress=sNTPServerIPAddress, longitude=longitude, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sMTPEnabledMessages=sMTPEnabledMessages, targetMACAddress=targetMACAddress, configuration=configuration, aggregateDataRate=aggregateDataRate, smtpGroup=smtpGroup, syslogControlGroup=syslogControlGroup, telecomsLineCode=telecomsLineCode, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, frequencyVariant=frequencyVariant, dataPortWirelessDownAlert=dataPortWirelessDownAlert, supplementaryGroup=supplementaryGroup, antennaGain=antennaGain, dfsMeans=dfsMeans, sNTPPollInterval=sNTPPollInterval, userTableNumber=userTableNumber, userTableEntry=userTableEntry, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, encryptionGroup=encryptionGroup, receiveModulationMode=receiveModulationMode, channelBandwidth=channelBandwidth, groupID=groupID, ptpTraps=ptpTraps, ethernetFiberSupport=ethernetFiberSupport, encryptionAlgorithm=encryptionAlgorithm, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, sMTPEmailAlert=sMTPEmailAlert, phyStatus=phyStatus, sNMPTrapTableEntry=sNMPTrapTableEntry, timeZone=timeZone, altitude=altitude, alarmsGroup=alarmsGroup, receiveChannel=receiveChannel, gatewayIPAddress=gatewayIPAddress, transmitFreqMHz=transmitFreqMHz, softwareVersion=softwareVersion, resetGroup=resetGroup, maximumTransmitPower=maximumTransmitPower, telecomsInterface=telecomsInterface, constantPowerSpectralDensity=constantPowerSpectralDensity, receiveFreqMHz=receiveFreqMHz, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sNMPTrapIPAddress=sNMPTrapIPAddress, transmitDataRate=transmitDataRate, regionCodeTrap=regionCodeTrap, vLANPriorityTableEntry=vLANPriorityTableEntry, userTableIndex=userTableIndex, sNMPTrapTable=sNMPTrapTable, aAAControlGroup=aAAControlGroup, productName=productName, dfsTableEntry=dfsTableEntry, linkName=linkName, siteName=siteName, cableLoss=cableLoss, dfsTableNumber=dfsTableNumber, linkModeOptimisation=linkModeOptimisation, tDDControlGroup=tDDControlGroup, snmpControlGroup=snmpControlGroup, dfsGroup=dfsGroup, wirelessLinkAvailability=wirelessLinkAvailability, configurationGroup=configurationGroup, subnetMask=subnetMask, ethernetGroup=ethernetGroup, dfsTableIndex=dfsTableIndex, installStatus=installStatus, hTTPAccessEnabled=hTTPAccessEnabled, sNMPTrapTableNumber=sNMPTrapTableNumber, tFTPServerPortNumber=tFTPServerPortNumber, aAAControl=aAAControl, dfsTable=dfsTable, ptpTrapPrefix=ptpTrapPrefix, installArmState=installArmState, searchState=searchState, eIRP=eIRP, linkSymmetry=linkSymmetry, sNMPv3Enable=sNMPv3Enable, daylightSaving=daylightSaving, receivePower=receivePower, linkModeOptimizationMismatch=linkModeOptimizationMismatch, productVariant=productVariant, sMTPServerIPAddress=sMTPServerIPAddress, vLANManagementPriority=vLANManagementPriority, dfs=dfs, sNMPCommunityString=sNMPCommunityString, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, range=range, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, versionsGroup=versionsGroup, sMTPSourceEmailAddress=sMTPSourceEmailAddress, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, phyControlGroup=phyControlGroup, incompatibleRegionCodes=incompatibleRegionCodes, sNMPTrapTableIndex=sNMPTrapTableIndex, accessMethod=accessMethod, telecomsChannelStatusTrap=telecomsChannelStatusTrap, userName=userName, syslogClient=syslogClient, hTTPSAccessEnabled=hTTPSAccessEnabled, sNTPSync=sNTPSync, dataPortAutoNegotiation=dataPortAutoNegotiation, telecomsLoopbackTrap=telecomsLoopbackTrap, vLANPriorityTable=vLANPriorityTable, tDDSynchronizationAlarm=tDDSynchronizationAlarm, masterSlaveMode=masterSlaveMode, signalStrengthRatio=signalStrengthRatio, phyStatusGroup=phyStatusGroup, sntpGroup=sntpGroup, remoteIPAddress=remoteIPAddress, systemClock=systemClock, pubStats=pubStats, snmpControl=snmpControl, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, managementGroup=managementGroup, transmitChannel=transmitChannel, versions=versions, PYSNMP_MODULE_ID=cambium, phyControl=phyControl, syslogControl=syslogControl, userEnabled=userEnabled, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, userRole=userRole, hTTPPortNumber=hTTPPortNumber, systemReset=systemReset, dataPortStatus=dataPortStatus, tDDControl=tDDControl, syslogClientTrap=syslogClientTrap, secondarySoftwareVersion=secondarySoftwareVersion, receiveDataRate=receiveDataRate, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, unitOutOfCalibration=unitOutOfCalibration, syslogState=syslogState, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dataPortStatusTrap=dataPortStatusTrap, vLANManagementVID=vLANManagementVID, receiveModulationModeDetail=receiveModulationModeDetail, sNMPEnabledTraps=sNMPEnabledTraps, syslogStateTrap=syslogStateTrap, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, useVLANForManagementInterfaces=useVLANForManagementInterfaces, tDDSynchronizationMode=tDDSynchronizationMode, dataPortDisabledWarning=dataPortDisabledWarning, ptp500=ptp500, userTable=userTable, telecom=telecom, installStatusTrap=installStatusTrap, regionCode=regionCode, telnetAccessEnabled=telnetAccessEnabled, pubStatsGroup=pubStatsGroup, installArmStateTrap=installArmStateTrap, transmitModulationMode=transmitModulationMode, ptmp=ptmp, sNMPTrapVersion=sNMPTrapVersion, telnetPortNumber=telnetPortNumber, channelChangeTrap=channelChangeTrap, telecomGroup=telecomGroup, sNTPServerPortNumber=sNTPServerPortNumber, vLANPriorityTableNumber=vLANPriorityTableNumber, enabledDiagnosticAlarms=enabledDiagnosticAlarms, tDDSynchronizationStatus=tDDSynchronizationStatus, licensedCapacity=licensedCapacity, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, latitude=latitude, supplementary=supplementary, syslogLocalWrappedTrap=syslogLocalWrappedTrap, management=management, telecomsChannelStatus=telecomsChannelStatus, encryption=encryption, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, remoteMaximumTransmitPower=remoteMaximumTransmitPower, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, wirelessLinkStatus=wirelessLinkStatus, licence=licence, dataPortAutoMdix=dataPortAutoMdix, byteErrorRatio=byteErrorRatio, ptpCompliance=ptpCompliance, iPAddress=iPAddress, tFTPServerIPAddress=tFTPServerIPAddress, receiveFreqKHz=receiveFreqKHz, sNMPPortNumber=sNMPPortNumber, sNTPLastSync=sNTPLastSync, sMTPServerPortNumber=sMTPServerPortNumber, cambium=cambium, vLANPriorityQueueMapping=vLANPriorityQueueMapping, ptp=ptp, ethernet=ethernet, reset=reset, bootVersion=bootVersion, userPassword=userPassword)
