...
# Produced by pysmi-1.1.3 at Sun Nov 28 21:13:31 2021
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", linkModeOptimizationMismatch=linkModeOptimizationMismatch, altitude=altitude, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, receiveModulationModeDetail=receiveModulationModeDetail, syslogState=syslogState, telnetPortNumber=telnetPortNumber, linkLoss=linkLoss, ptpCompliance=ptpCompliance, cableLoss=cableLoss, tDDControlGroup=tDDControlGroup, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, sMTPServerPortNumber=sMTPServerPortNumber, receiveFreqKHz=receiveFreqKHz, productName=productName, tFTPServerIPAddress=tFTPServerIPAddress, transmitPower=transmitPower, telecomsChannelStatus=telecomsChannelStatus, smtp=smtp, telnetAccessEnabled=telnetAccessEnabled, bandwidthVariant=bandwidthVariant, gatewayIPAddress=gatewayIPAddress, masterSlaveMode=masterSlaveMode, configurationGroup=configurationGroup, dataPortConfigurationMismatch=dataPortConfigurationMismatch, ethernetFiberSupport=ethernetFiberSupport, timeZone=timeZone, userRole=userRole, sMTPServerIPAddress=sMTPServerIPAddress, telecomsInterface=telecomsInterface, dfsTableEntry=dfsTableEntry, byteErrorRatio=byteErrorRatio, productVariant=productVariant, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sNMPTrapVersion=sNMPTrapVersion, linkSymmetry=linkSymmetry, tFTPServerPortNumber=tFTPServerPortNumber, sNMPTrapTableNumber=sNMPTrapTableNumber, telecomsCableLength=telecomsCableLength, regionCode=regionCode, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, channelChangeTrap=channelChangeTrap, smtpGroup=smtpGroup, hTTPAccessEnabled=hTTPAccessEnabled, sNMPTrapTableEntry=sNMPTrapTableEntry, userTable=userTable, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, managementGroup=managementGroup, tDDSynchronizationStatus=tDDSynchronizationStatus, antennaGain=antennaGain, versionsGroup=versionsGroup, configuration=configuration, snmpControlGroup=snmpControlGroup, dfs=dfs, capacityLicenseMismatch=capacityLicenseMismatch, frequencyVariant=frequencyVariant, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, channelBandwidth=channelBandwidth, sNMPTrapTableIndex=sNMPTrapTableIndex, targetMACAddress=targetMACAddress, licenceGroup=licenceGroup, hTTPPortNumber=hTTPPortNumber, sNMPTrapIPAddress=sNMPTrapIPAddress, sNTPServerIPAddress=sNTPServerIPAddress, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, syslogClientTrap=syslogClientTrap, userTableIndex=userTableIndex, vLANPriorityTable=vLANPriorityTable, ptp=ptp, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, hardwareVersion=hardwareVersion, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, hTTPSPortNumber=hTTPSPortNumber, sMTPEmailAlert=sMTPEmailAlert, phyControlGroup=phyControlGroup, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, aAAControl=aAAControl, sNMPTrapPortNumber=sNMPTrapPortNumber, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, installArmStateTrap=installArmStateTrap, vectorError=vectorError, sNTPState=sNTPState, alarmsGroup=alarmsGroup, linkModeOptimisation=linkModeOptimisation, hTTPSAccessEnabled=hTTPSAccessEnabled, eIRP=eIRP, dataPortWirelessDownAlert=dataPortWirelessDownAlert, groupID=groupID, siteName=siteName, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, systemReset=systemReset, latitude=latitude, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, syslogStateTrap=syslogStateTrap, tDDSynchronizationAlarm=tDDSynchronizationAlarm, transmitChannel=transmitChannel, linkName=linkName, syslogControl=syslogControl, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dfsTableNumber=dfsTableNumber, supplementaryGroup=supplementaryGroup, remoteMaximumTransmitPower=remoteMaximumTransmitPower, reset=reset, versions=versions, telecom=telecom, licence=licence, syslogClient=syslogClient, dfsPeaks=dfsPeaks, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, vLANManagementPriority=vLANManagementPriority, aAAControlGroup=aAAControlGroup, noWirelessChannelAvailable=noWirelessChannelAvailable, phyControl=phyControl, vLANPriorityTableNumber=vLANPriorityTableNumber, transmitFreqMHz=transmitFreqMHz, dfsMeans=dfsMeans, tDDSynchronizationMode=tDDSynchronizationMode, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortStatus=dataPortStatus, signalStrengthRatio=signalStrengthRatio, snmpControl=snmpControl, sNMPCommunityString=sNMPCommunityString, receiveChannel=receiveChannel, dataPortStatusTrap=dataPortStatusTrap, sMTPSourceEmailAddress=sMTPSourceEmailAddress, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, tDDControl=tDDControl, sNMPv3Enable=sNMPv3Enable, sntpGroup=sntpGroup, pubStatsGroup=pubStatsGroup, maximumTransmitPower=maximumTransmitPower, installStatusTrap=installStatusTrap, installArmState=installArmState, sNTPPollInterval=sNTPPollInterval, sntp=sntp, vLANPriorityQueueMapping=vLANPriorityQueueMapping, userTableEntry=userTableEntry, telecomGroup=telecomGroup, unitOutOfCalibration=unitOutOfCalibration, supplementary=supplementary, dataPortAutoMdix=dataPortAutoMdix, secondarySoftwareVersion=secondarySoftwareVersion, ptpTrapPrefix=ptpTrapPrefix, ptpGroups=ptpGroups, phyStatusGroup=phyStatusGroup, range=range, remoteIPAddress=remoteIPAddress, userTableNumber=userTableNumber, transmitFreqKHz=transmitFreqKHz, incompatibleMasterAndSlave=incompatibleMasterAndSlave, userName=userName, regionCodeTrap=regionCodeTrap, encryptionAlgorithm=encryptionAlgorithm, pubStats=pubStats, aggregateDataRate=aggregateDataRate, receivePower=receivePower, iPAddress=iPAddress, userPassword=userPassword, telecomsLoopbackTrap=telecomsLoopbackTrap, constantPowerSpectralDensity=constantPowerSpectralDensity, receiveFreqMHz=receiveFreqMHz, daylightSaving=daylightSaving, encryption=encryption, dfsTableIndex=dfsTableIndex, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNTPLastSync=sNTPLastSync, notificationsGroup=notificationsGroup, telecomsChannelStatusTrap=telecomsChannelStatusTrap, useVLANForManagementInterfaces=useVLANForManagementInterfaces, ethernet=ethernet, receiveDataRate=receiveDataRate, dfsTable=dfsTable, longitude=longitude, sNMPTrapTable=sNMPTrapTable, sNTPSyncTrap=sNTPSyncTrap, subnetMask=subnetMask, receiveModulationMode=receiveModulationMode, wirelessLinkAvailability=wirelessLinkAvailability, accessMethod=accessMethod, vLANPriorityTableEntry=vLANPriorityTableEntry, ptp500=ptp500, bootVersion=bootVersion, dataPortDisabledWarning=dataPortDisabledWarning, systemClock=systemClock, transmitDataRate=transmitDataRate, userEnabled=userEnabled, sNTPServerPortNumber=sNTPServerPortNumber, encryptionGroup=encryptionGroup, wirelessLinkStatus=wirelessLinkStatus, syslogControlGroup=syslogControlGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sNMPEnabledTraps=sNMPEnabledTraps, cambium=cambium, telecomsLineCode=telecomsLineCode, sNMPPortNumber=sNMPPortNumber, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, management=management, ptpTraps=ptpTraps, resetGroup=resetGroup, installStatus=installStatus, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, vLANManagementVID=vLANManagementVID, transmitModulationMode=transmitModulationMode, searchState=searchState, alarms=alarms, licensedCapacity=licensedCapacity, incompatibleRegionCodes=incompatibleRegionCodes, dfsGroup=dfsGroup, sNTPSync=sNTPSync, vLANPriorityTableIndex=vLANPriorityTableIndex, phyStatus=phyStatus, ptmp=ptmp, softwareVersion=softwareVersion, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, ethernetGroup=ethernetGroup, PYSNMP_MODULE_ID=cambium, telecomsLoopback=telecomsLoopback, sMTPEnabledMessages=sMTPEnabledMessages, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex)
