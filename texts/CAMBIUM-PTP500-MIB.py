...
# Produced by pysmi-1.1.8 at Fri Jan  7 15:56:25 2022
...
TimeTicks, Bits, Unsigned32, NotificationType, enterprises, ObjectIdentity, MibIdentifier, Counter32, Integer32, IpAddress, Counter64, ModuleIdentity, Gauge32, iso, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Bits", "Unsigned32", "NotificationType", "enterprises", "ObjectIdentity", "MibIdentifier", "Counter32", "Integer32", "IpAddress", "Counter64", "ModuleIdentity", "Gauge32", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", pubStats=pubStats, dfsTableEntry=dfsTableEntry, wirelessLinkStatus=wirelessLinkStatus, pubStatsGroup=pubStatsGroup, capacityLicenseMismatch=capacityLicenseMismatch, ethernet=ethernet, dfsTable=dfsTable, configuration=configuration, alarms=alarms, aAAControl=aAAControl, noWirelessChannelAvailable=noWirelessChannelAvailable, bandwidthVariant=bandwidthVariant, smtpGroup=smtpGroup, encryptionAlgorithm=encryptionAlgorithm, userTableIndex=userTableIndex, transmitChannel=transmitChannel, softwareVersion=softwareVersion, regionCodeTrap=regionCodeTrap, searchState=searchState, syslogStateTrap=syslogStateTrap, sNTPLastSync=sNTPLastSync, notificationsGroup=notificationsGroup, vLANManagementVID=vLANManagementVID, licenceGroup=licenceGroup, eIRP=eIRP, versions=versions, channelChangeTrap=channelChangeTrap, ethernetFiberSupport=ethernetFiberSupport, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortStatusTrap=dataPortStatusTrap, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, systemClock=systemClock, tDDControl=tDDControl, telnetAccessEnabled=telnetAccessEnabled, timeZone=timeZone, targetMACAddress=targetMACAddress, systemReset=systemReset, useVLANForManagementInterfaces=useVLANForManagementInterfaces, sMTPEnabledMessages=sMTPEnabledMessages, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, PYSNMP_MODULE_ID=cambium, dataPortDisabledWarning=dataPortDisabledWarning, syslogState=syslogState, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, receiveFreqMHz=receiveFreqMHz, telecomsInterface=telecomsInterface, installStatusTrap=installStatusTrap, reset=reset, telecomsChannelStatus=telecomsChannelStatus, receivePower=receivePower, regionCode=regionCode, ptpGroups=ptpGroups, vLANManagementPriority=vLANManagementPriority, ethernetGroup=ethernetGroup, managementGroup=managementGroup, receiveDataRate=receiveDataRate, vectorError=vectorError, aAAControlGroup=aAAControlGroup, userRole=userRole, dfsTableIndex=dfsTableIndex, hTTPPortNumber=hTTPPortNumber, tDDControlGroup=tDDControlGroup, sNMPTrapVersion=sNMPTrapVersion, sNMPTrapIPAddress=sNMPTrapIPAddress, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, sNTPServerPortNumber=sNTPServerPortNumber, userName=userName, byteErrorRatio=byteErrorRatio, masterSlaveMode=masterSlaveMode, phyStatus=phyStatus, tDDSynchronizationMode=tDDSynchronizationMode, versionsGroup=versionsGroup, syslogControlGroup=syslogControlGroup, telecomsLoopback=telecomsLoopback, altitude=altitude, telecomsLoopbackTrap=telecomsLoopbackTrap, resetGroup=resetGroup, sNTPServerIPAddress=sNTPServerIPAddress, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, productName=productName, secondarySoftwareVersion=secondarySoftwareVersion, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, tFTPServerIPAddress=tFTPServerIPAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, syslogClientTrap=syslogClientTrap, dfsMeans=dfsMeans, linkSymmetry=linkSymmetry, encryption=encryption, supplementary=supplementary, sMTPSourceEmailAddress=sMTPSourceEmailAddress, phyStatusGroup=phyStatusGroup, sNTPPollInterval=sNTPPollInterval, incompatibleRegionCodes=incompatibleRegionCodes, installStatus=installStatus, dfsGroup=dfsGroup, linkName=linkName, snmpControlGroup=snmpControlGroup, linkModeOptimizationMismatch=linkModeOptimizationMismatch, snmpControl=snmpControl, syslogControl=syslogControl, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, range=range, sNTPState=sNTPState, dataPortWirelessDownAlert=dataPortWirelessDownAlert, frequencyVariant=frequencyVariant, phyControlGroup=phyControlGroup, vLANPriorityTableNumber=vLANPriorityTableNumber, sMTPServerPortNumber=sMTPServerPortNumber, syslogClient=syslogClient, sNTPSyncTrap=sNTPSyncTrap, gatewayIPAddress=gatewayIPAddress, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, telecomsChannelStatusTrap=telecomsChannelStatusTrap, telecomsCableLength=telecomsCableLength, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, bootVersion=bootVersion, configurationGroup=configurationGroup, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, hTTPSAccessEnabled=hTTPSAccessEnabled, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, transmitModulationMode=transmitModulationMode, channelBandwidth=channelBandwidth, siteName=siteName, vLANPriorityTable=vLANPriorityTable, smtp=smtp, maximumTransmitPower=maximumTransmitPower, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, ptpTraps=ptpTraps, aggregateDataRate=aggregateDataRate, telecomGroup=telecomGroup, tDDSynchronizationStatus=tDDSynchronizationStatus, userTableEntry=userTableEntry, dataPortConfigurationMismatch=dataPortConfigurationMismatch, supplementaryGroup=supplementaryGroup, subnetMask=subnetMask, installArmState=installArmState, productVariant=productVariant, transmitFreqMHz=transmitFreqMHz, iPAddress=iPAddress, tFTPServerPortNumber=tFTPServerPortNumber, dfs=dfs, phyControl=phyControl, groupID=groupID, incompatibleMasterAndSlave=incompatibleMasterAndSlave, latitude=latitude, userTable=userTable, ptmp=ptmp, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, telecom=telecom, wirelessLinkAvailability=wirelessLinkAvailability, remoteIPAddress=remoteIPAddress, encryptionGroup=encryptionGroup, ptpTrapPrefix=ptpTrapPrefix, sNMPTrapTable=sNMPTrapTable, management=management, unitOutOfCalibration=unitOutOfCalibration, sntp=sntp, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, receiveFreqKHz=receiveFreqKHz, sNTPSync=sNTPSync, vLANPriorityTableEntry=vLANPriorityTableEntry, hardwareVersion=hardwareVersion, longitude=longitude, dataPortAutoMdix=dataPortAutoMdix, ptp500=ptp500, telecomsLineCode=telecomsLineCode, userTableNumber=userTableNumber, dataPortStatus=dataPortStatus, vLANPriorityTableIndex=vLANPriorityTableIndex, sNMPCommunityString=sNMPCommunityString, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, receiveChannel=receiveChannel, licence=licence, enabledDiagnosticAlarms=enabledDiagnosticAlarms, receiveModulationMode=receiveModulationMode, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, transmitPower=transmitPower, licensedCapacity=licensedCapacity, userPassword=userPassword, sntpGroup=sntpGroup, alarmsGroup=alarmsGroup, linkLoss=linkLoss, ptpCompliance=ptpCompliance, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, signalStrengthRatio=signalStrengthRatio, cableLoss=cableLoss, antennaGain=antennaGain, transmitDataRate=transmitDataRate, telnetPortNumber=telnetPortNumber, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, cambium=cambium, remoteMaximumTransmitPower=remoteMaximumTransmitPower, transmitFreqKHz=transmitFreqKHz, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPTrapTableNumber=sNMPTrapTableNumber, receiveModulationModeDetail=receiveModulationModeDetail, ptp=ptp, hTTPSPortNumber=hTTPSPortNumber, sNMPv3Enable=sNMPv3Enable, dfsPeaks=dfsPeaks, hTTPAccessEnabled=hTTPAccessEnabled, linkModeOptimisation=linkModeOptimisation, dfsTableNumber=dfsTableNumber, sMTPServerIPAddress=sMTPServerIPAddress, sNMPTrapTableIndex=sNMPTrapTableIndex, sMTPEmailAlert=sMTPEmailAlert, installArmStateTrap=installArmStateTrap, userEnabled=userEnabled, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNMPPortNumber=sNMPPortNumber, daylightSaving=daylightSaving, accessMethod=accessMethod, constantPowerSpectralDensity=constantPowerSpectralDensity, sNMPEnabledTraps=sNMPEnabledTraps, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap)
