...
Gauge32, iso, ObjectIdentity, Integer32, Bits, Unsigned32, IpAddress, Counter32, NotificationType, enterprises, ModuleIdentity, TimeTicks, Counter64, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "iso", "ObjectIdentity", "Integer32", "Bits", "Unsigned32", "IpAddress", "Counter32", "NotificationType", "enterprises", "ModuleIdentity", "TimeTicks", "Counter64", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", incompatibleMasterAndSlave=incompatibleMasterAndSlave, sNMPTrapTableEntry=sNMPTrapTableEntry, ethernet=ethernet, telecomsChannelStatusTrap=telecomsChannelStatusTrap, cableLoss=cableLoss, vLANManagementVID=vLANManagementVID, phyControlGroup=phyControlGroup, ethernetFiberSupport=ethernetFiberSupport, userTable=userTable, syslogControlGroup=syslogControlGroup, PYSNMP_MODULE_ID=cambium, bandwidthVariant=bandwidthVariant, hTTPSPortNumber=hTTPSPortNumber, transmitFreqKHz=transmitFreqKHz, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, dataPortAutoMdix=dataPortAutoMdix, sMTPServerIPAddress=sMTPServerIPAddress, userEnabled=userEnabled, userRole=userRole, constantPowerSpectralDensity=constantPowerSpectralDensity, productName=productName, installStatusTrap=installStatusTrap, sNMPTrapTableIndex=sNMPTrapTableIndex, configurationGroup=configurationGroup, sMTPServerPortNumber=sMTPServerPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, syslogClient=syslogClient, longitude=longitude, telnetAccessEnabled=telnetAccessEnabled, tDDSynchronizationAlarm=tDDSynchronizationAlarm, softwareVersion=softwareVersion, sNMPTrapIPAddress=sNMPTrapIPAddress, tFTPServerIPAddress=tFTPServerIPAddress, telnetPortNumber=telnetPortNumber, wirelessLinkStatus=wirelessLinkStatus, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sNTPLastSync=sNTPLastSync, sNMPTrapTable=sNMPTrapTable, ptp=ptp, encryptionAlgorithm=encryptionAlgorithm, linkModeOptimisation=linkModeOptimisation, sNTPPollInterval=sNTPPollInterval, receiveChannel=receiveChannel, range=range, linkLoss=linkLoss, sntpGroup=sntpGroup, dfsTableEntry=dfsTableEntry, systemReset=systemReset, receiveDataRate=receiveDataRate, telecomsCableLength=telecomsCableLength, versionsGroup=versionsGroup, latitude=latitude, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, regionCode=regionCode, syslogLocalWrappedTrap=syslogLocalWrappedTrap, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, subnetMask=subnetMask, phyStatus=phyStatus, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, vLANManagementPriority=vLANManagementPriority, userTableEntry=userTableEntry, accessMethod=accessMethod, telecomsChannelStatus=telecomsChannelStatus, vLANPriorityTableIndex=vLANPriorityTableIndex, licence=licence, transmitDataRate=transmitDataRate, syslogControl=syslogControl, sNTPServerIPAddress=sNTPServerIPAddress, timeZone=timeZone, sNMPCommunityString=sNMPCommunityString, linkSymmetry=linkSymmetry, vLANPriorityTableNumber=vLANPriorityTableNumber, receiveFreqMHz=receiveFreqMHz, versions=versions, vLANPriorityTable=vLANPriorityTable, phyStatusGroup=phyStatusGroup, syslogClientTrap=syslogClientTrap, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, ptmp=ptmp, telecomsInterface=telecomsInterface, sNMPPortNumber=sNMPPortNumber, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, wirelessLinkAvailability=wirelessLinkAvailability, transmitPower=transmitPower, supplementary=supplementary, eIRP=eIRP, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, aAAControl=aAAControl, supplementaryGroup=supplementaryGroup, dataPortDisabledWarning=dataPortDisabledWarning, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, bootVersion=bootVersion, hTTPSAccessEnabled=hTTPSAccessEnabled, sNMPTrapVersion=sNMPTrapVersion, management=management, productVariant=productVariant, telecomsLoopbackTrap=telecomsLoopbackTrap, hardwareVersion=hardwareVersion, aggregateDataRate=aggregateDataRate, channelBandwidth=channelBandwidth, systemClock=systemClock, aAAControlGroup=aAAControlGroup, tDDControl=tDDControl, ptpGroups=ptpGroups, pubStats=pubStats, tDDControlGroup=tDDControlGroup, hTTPPortNumber=hTTPPortNumber, installStatus=installStatus, frequencyVariant=frequencyVariant, managementGroup=managementGroup, encryptionGroup=encryptionGroup, vLANPriorityTableEntry=vLANPriorityTableEntry, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, dfsTableIndex=dfsTableIndex, pubStatsGroup=pubStatsGroup, secondarySoftwareVersion=secondarySoftwareVersion, targetMACAddress=targetMACAddress, signalStrengthRatio=signalStrengthRatio, enabledDiagnosticAlarms=enabledDiagnosticAlarms, telecomsLineCode=telecomsLineCode, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, capacityLicenseMismatch=capacityLicenseMismatch, dfsTable=dfsTable, receiveModulationMode=receiveModulationMode, tDDSynchronizationMode=tDDSynchronizationMode, alarms=alarms, altitude=altitude, vLANPriorityQueueMapping=vLANPriorityQueueMapping, dfsMeans=dfsMeans, configuration=configuration, sMTPSourceEmailAddress=sMTPSourceEmailAddress, encryption=encryption, ethernetGroup=ethernetGroup, licenceGroup=licenceGroup, tFTPServerPortNumber=tFTPServerPortNumber, receivePower=receivePower, tDDSynchronizationStatus=tDDSynchronizationStatus, sNTPState=sNTPState, incompatibleRegionCodes=incompatibleRegionCodes, userTableIndex=userTableIndex, sNTPServerPortNumber=sNTPServerPortNumber, ptpTraps=ptpTraps, receiveModulationModeDetail=receiveModulationModeDetail, remoteIPAddress=remoteIPAddress, syslogState=syslogState, userName=userName, regionCodeTrap=regionCodeTrap, sMTPEmailAlert=sMTPEmailAlert, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, daylightSaving=daylightSaving, noWirelessChannelAvailable=noWirelessChannelAvailable, alarmsGroup=alarmsGroup, linkModeOptimizationMismatch=linkModeOptimizationMismatch, siteName=siteName, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, receiveFreqKHz=receiveFreqKHz, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, unitOutOfCalibration=unitOutOfCalibration, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, dataPortAutoNegotiation=dataPortAutoNegotiation, cambium=cambium, ptpCompliance=ptpCompliance, resetGroup=resetGroup, antennaGain=antennaGain, sMTPEnabledMessages=sMTPEnabledMessages, useVLANForManagementInterfaces=useVLANForManagementInterfaces, smtpGroup=smtpGroup, dataPortStatusTrap=dataPortStatusTrap, transmitModulationMode=transmitModulationMode, userTableNumber=userTableNumber, snmpControlGroup=snmpControlGroup, telecom=telecom, searchState=searchState, transmitChannel=transmitChannel, dfsTableNumber=dfsTableNumber, snmpControl=snmpControl, smtp=smtp, dataPortStatus=dataPortStatus, reset=reset, installArmState=installArmState, notificationsGroup=notificationsGroup, dfsGroup=dfsGroup, phyControl=phyControl, vectorError=vectorError, channelChangeTrap=channelChangeTrap, sNTPSync=sNTPSync, groupID=groupID, sNMPEnabledTraps=sNMPEnabledTraps, dfsPeaks=dfsPeaks, licensedCapacity=licensedCapacity, sNMPTrapTableNumber=sNMPTrapTableNumber, maximumTransmitPower=maximumTransmitPower, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, gatewayIPAddress=gatewayIPAddress, dfs=dfs, ptpTrapPrefix=ptpTrapPrefix, remoteMaximumTransmitPower=remoteMaximumTransmitPower, transmitFreqMHz=transmitFreqMHz, dataPortWirelessDownAlert=dataPortWirelessDownAlert, sNMPv3Enable=sNMPv3Enable, byteErrorRatio=byteErrorRatio, hTTPAccessEnabled=hTTPAccessEnabled, syslogStateTrap=syslogStateTrap, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, iPAddress=iPAddress, linkName=linkName, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, telecomGroup=telecomGroup, installArmStateTrap=installArmStateTrap, sNTPSyncTrap=sNTPSyncTrap, userPassword=userPassword, telecomsLoopback=telecomsLoopback, sntp=sntp, masterSlaveMode=masterSlaveMode, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, ptp500=ptp500)
