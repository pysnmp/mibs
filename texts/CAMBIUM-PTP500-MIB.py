...
Gauge32, Unsigned32, Counter32, Counter64, iso, MibIdentifier, NotificationType, enterprises, Integer32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, ModuleIdentity, TimeTicks, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Unsigned32", "Counter32", "Counter64", "iso", "MibIdentifier", "NotificationType", "enterprises", "Integer32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "ModuleIdentity", "TimeTicks", "IpAddress")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, gatewayIPAddress=gatewayIPAddress, transmitFreqKHz=transmitFreqKHz, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, syslogClientTrap=syslogClientTrap, ethernetFiberSupport=ethernetFiberSupport, dfsTableIndex=dfsTableIndex, receiveFreqMHz=receiveFreqMHz, telecomsLoopbackTrap=telecomsLoopbackTrap, syslogControlGroup=syslogControlGroup, telecomGroup=telecomGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, userPassword=userPassword, supplementary=supplementary, ptpCompliance=ptpCompliance, resetGroup=resetGroup, ptp500=ptp500, remoteMaximumTransmitPower=remoteMaximumTransmitPower, signalStrengthRatio=signalStrengthRatio, secondarySoftwareVersion=secondarySoftwareVersion, aAAControlGroup=aAAControlGroup, userTable=userTable, transmitPower=transmitPower, tFTPServerPortNumber=tFTPServerPortNumber, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, phyControl=phyControl, pubStats=pubStats, ethernetGroup=ethernetGroup, userTableNumber=userTableNumber, regionCodeTrap=regionCodeTrap, hTTPSPortNumber=hTTPSPortNumber, iPAddress=iPAddress, vLANPriorityTableNumber=vLANPriorityTableNumber, cableLoss=cableLoss, sNMPPortNumber=sNMPPortNumber, tDDSynchronizationStatus=tDDSynchronizationStatus, sNMPCommunityString=sNMPCommunityString, softwareVersion=softwareVersion, productName=productName, vLANPriorityQueueMapping=vLANPriorityQueueMapping, dfs=dfs, sNMPTrapTableIndex=sNMPTrapTableIndex, vLANManagementPriority=vLANManagementPriority, transmitModulationMode=transmitModulationMode, sNTPLastSync=sNTPLastSync, tDDSynchronizationMode=tDDSynchronizationMode, phyStatusGroup=phyStatusGroup, incompatibleMasterAndSlave=incompatibleMasterAndSlave, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, dataPortConfigurationMismatch=dataPortConfigurationMismatch, smtp=smtp, tDDSynchronizationAlarm=tDDSynchronizationAlarm, vLANPriorityTableEntry=vLANPriorityTableEntry, productVariant=productVariant, systemReset=systemReset, userTableIndex=userTableIndex, reset=reset, searchState=searchState, sMTPServerIPAddress=sMTPServerIPAddress, accessMethod=accessMethod, subnetMask=subnetMask, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, ptpGroups=ptpGroups, transmitFreqMHz=transmitFreqMHz, licence=licence, phyStatus=phyStatus, dataPortWirelessDownAlert=dataPortWirelessDownAlert, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, hTTPAccessEnabled=hTTPAccessEnabled, linkModeOptimisation=linkModeOptimisation, syslogState=syslogState, hTTPSAccessEnabled=hTTPSAccessEnabled, versions=versions, syslogLocalWrappedTrap=syslogLocalWrappedTrap, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, telecomsLoopback=telecomsLoopback, channelChangeTrap=channelChangeTrap, vLANPriorityTable=vLANPriorityTable, cambium=cambium, syslogStateTrap=syslogStateTrap, userTableEntry=userTableEntry, syslogControl=syslogControl, receiveModulationMode=receiveModulationMode, tFTPServerIPAddress=tFTPServerIPAddress, aggregateDataRate=aggregateDataRate, dataPortAutoMdix=dataPortAutoMdix, telecomsCableLength=telecomsCableLength, configuration=configuration, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, dataPortStatus=dataPortStatus, telecomsInterface=telecomsInterface, bandwidthVariant=bandwidthVariant, alarms=alarms, sNTPState=sNTPState, capacityLicenseMismatch=capacityLicenseMismatch, installStatusTrap=installStatusTrap, groupID=groupID, tDDControlGroup=tDDControlGroup, remoteIPAddress=remoteIPAddress, daylightSaving=daylightSaving, linkSymmetry=linkSymmetry, sNMPTrapVersion=sNMPTrapVersion, dfsTableNumber=dfsTableNumber, ptp=ptp, sNTPServerIPAddress=sNTPServerIPAddress, hTTPPortNumber=hTTPPortNumber, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, snmpControlGroup=snmpControlGroup, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, sNMPEnabledTraps=sNMPEnabledTraps, dfsTable=dfsTable, sMTPSourceEmailAddress=sMTPSourceEmailAddress, hardwareVersion=hardwareVersion, receiveModulationModeDetail=receiveModulationModeDetail, longitude=longitude, altitude=altitude, supplementaryGroup=supplementaryGroup, siteName=siteName, ethernet=ethernet, aAAControl=aAAControl, unitOutOfCalibration=unitOutOfCalibration, receiveChannel=receiveChannel, sntp=sntp, installStatus=installStatus, dfsGroup=dfsGroup, receivePower=receivePower, sMTPEnabledMessages=sMTPEnabledMessages, sNMPTrapTable=sNMPTrapTable, PYSNMP_MODULE_ID=cambium, userRole=userRole, range=range, ptpTraps=ptpTraps, channelBandwidth=channelBandwidth, ptpTrapPrefix=ptpTrapPrefix, eIRP=eIRP, linkLoss=linkLoss, vectorError=vectorError, useVLANForManagementInterfaces=useVLANForManagementInterfaces, transmitDataRate=transmitDataRate, encryptionGroup=encryptionGroup, antennaGain=antennaGain, receiveDataRate=receiveDataRate, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, userEnabled=userEnabled, sNTPSync=sNTPSync, incompatibleRegionCodes=incompatibleRegionCodes, configurationGroup=configurationGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, regionCode=regionCode, userName=userName, dfsTableEntry=dfsTableEntry, telecomsLineCode=telecomsLineCode, telecomsChannelStatus=telecomsChannelStatus, dataPortDisabledWarning=dataPortDisabledWarning, sNMPTrapTableNumber=sNMPTrapTableNumber, telnetPortNumber=telnetPortNumber, systemClock=systemClock, sNMPTrapIPAddress=sNMPTrapIPAddress, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, dataPortStatusTrap=dataPortStatusTrap, ptmp=ptmp, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, wirelessLinkAvailability=wirelessLinkAvailability, sNTPPollInterval=sNTPPollInterval, smtpGroup=smtpGroup, encryption=encryption, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, sNTPSyncTrap=sNTPSyncTrap, maximumTransmitPower=maximumTransmitPower, transmitChannel=transmitChannel, vLANPriorityTableIndex=vLANPriorityTableIndex, dfsPeaks=dfsPeaks, alarmsGroup=alarmsGroup, installArmStateTrap=installArmStateTrap, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, versionsGroup=versionsGroup, sMTPEmailAlert=sMTPEmailAlert, notificationsGroup=notificationsGroup, encryptionAlgorithm=encryptionAlgorithm, snmpControl=snmpControl, enabledDiagnosticAlarms=enabledDiagnosticAlarms, licensedCapacity=licensedCapacity, targetMACAddress=targetMACAddress, telecomsChannelStatusTrap=telecomsChannelStatusTrap, noWirelessChannelAvailable=noWirelessChannelAvailable, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, syslogClient=syslogClient, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, telecom=telecom, pubStatsGroup=pubStatsGroup, constantPowerSpectralDensity=constantPowerSpectralDensity, phyControlGroup=phyControlGroup, byteErrorRatio=byteErrorRatio, sNTPServerPortNumber=sNTPServerPortNumber, wirelessLinkStatus=wirelessLinkStatus, tDDControl=tDDControl, dataPortAutoNegotiation=dataPortAutoNegotiation, linkName=linkName, sMTPServerPortNumber=sMTPServerPortNumber, installArmState=installArmState, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, management=management, vLANManagementVID=vLANManagementVID, linkModeOptimizationMismatch=linkModeOptimizationMismatch, licenceGroup=licenceGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, timeZone=timeZone, dfsMeans=dfsMeans, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, latitude=latitude, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, bootVersion=bootVersion, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, frequencyVariant=frequencyVariant, receiveFreqKHz=receiveFreqKHz, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPv3Enable=sNMPv3Enable, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, telnetAccessEnabled=telnetAccessEnabled, sntpGroup=sntpGroup, managementGroup=managementGroup, masterSlaveMode=masterSlaveMode)
