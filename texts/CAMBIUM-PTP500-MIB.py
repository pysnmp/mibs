...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", receiveModulationModeDetail=receiveModulationModeDetail, tDDControlGroup=tDDControlGroup, dfsTableEntry=dfsTableEntry, telecomsInterface=telecomsInterface, sNTPState=sNTPState, iPAddress=iPAddress, masterSlaveMode=masterSlaveMode, ptpGroups=ptpGroups, dfsTableNumber=dfsTableNumber, vLANManagementPriority=vLANManagementPriority, searchState=searchState, transmitDataRate=transmitDataRate, sNTPPollInterval=sNTPPollInterval, dataPortStatus=dataPortStatus, configuration=configuration, sNMPTrapTableIndex=sNMPTrapTableIndex, hTTPAccessEnabled=hTTPAccessEnabled, ethernetGroup=ethernetGroup, softwareVersion=softwareVersion, cambium=cambium, dfsPeaks=dfsPeaks, ptpTrapPrefix=ptpTrapPrefix, sNMPTrapTableNumber=sNMPTrapTableNumber, phyControlGroup=phyControlGroup, userEnabled=userEnabled, alarms=alarms, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, syslogClientTrap=syslogClientTrap, smtpGroup=smtpGroup, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, remoteIPAddress=remoteIPAddress, sntp=sntp, regionCode=regionCode, signalStrengthRatio=signalStrengthRatio, regionCodeTrap=regionCodeTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, vLANPriorityQueueMapping=vLANPriorityQueueMapping, constantPowerSpectralDensity=constantPowerSpectralDensity, transmitChannel=transmitChannel, secondarySoftwareVersion=secondarySoftwareVersion, telnetPortNumber=telnetPortNumber, capacityLicenseMismatch=capacityLicenseMismatch, byteErrorRatio=byteErrorRatio, sNMPEnabledTraps=sNMPEnabledTraps, installStatusTrap=installStatusTrap, sNMPTrapTableEntry=sNMPTrapTableEntry, tDDSynchronizationMode=tDDSynchronizationMode, cableLoss=cableLoss, licence=licence, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sNMPPortNumber=sNMPPortNumber, telecomsLoopbackTrap=telecomsLoopbackTrap, antennaGain=antennaGain, sNMPTrapTable=sNMPTrapTable, siteName=siteName, ptpCompliance=ptpCompliance, resetGroup=resetGroup, syslogState=syslogState, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, pubStats=pubStats, aAAControlGroup=aAAControlGroup, range=range, installArmStateTrap=installArmStateTrap, reset=reset, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, useVLANForManagementInterfaces=useVLANForManagementInterfaces, managementGroup=managementGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNTPSyncTrap=sNTPSyncTrap, bandwidthVariant=bandwidthVariant, syslogClient=syslogClient, unitOutOfCalibration=unitOutOfCalibration, longitude=longitude, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, wirelessLinkStatus=wirelessLinkStatus, dataPortStatusTrap=dataPortStatusTrap, targetMACAddress=targetMACAddress, phyControl=phyControl, userTableNumber=userTableNumber, ptmp=ptmp, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, linkLoss=linkLoss, telecom=telecom, ptp500=ptp500, syslogControl=syslogControl, sNMPv3Enable=sNMPv3Enable, syslogControlGroup=syslogControlGroup, incompatibleMasterAndSlave=incompatibleMasterAndSlave, encryptionAlgorithm=encryptionAlgorithm, accessMethod=accessMethod, hTTPPortNumber=hTTPPortNumber, systemClock=systemClock, maximumTransmitPower=maximumTransmitPower, installArmState=installArmState, telecomsLoopback=telecomsLoopback, PYSNMP_MODULE_ID=cambium, alarmsGroup=alarmsGroup, versionsGroup=versionsGroup, sMTPServerIPAddress=sMTPServerIPAddress, bootVersion=bootVersion, supplementary=supplementary, sNTPSync=sNTPSync, tDDControl=tDDControl, incompatibleRegionCodes=incompatibleRegionCodes, ethernet=ethernet, tFTPServerIPAddress=tFTPServerIPAddress, sMTPEmailAlert=sMTPEmailAlert, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dfs=dfs, eIRP=eIRP, sNTPServerPortNumber=sNTPServerPortNumber, aAAControl=aAAControl, transmitFreqKHz=transmitFreqKHz, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, enabledDiagnosticAlarms=enabledDiagnosticAlarms, versions=versions, licensedCapacity=licensedCapacity, altitude=altitude, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, sNTPServerIPAddress=sNTPServerIPAddress, snmpControlGroup=snmpControlGroup, dataPortConfigurationMismatch=dataPortConfigurationMismatch, userPassword=userPassword, receiveModulationMode=receiveModulationMode, systemReset=systemReset, sNMPTrapPortNumber=sNMPTrapPortNumber, telecomsChannelStatusTrap=telecomsChannelStatusTrap, phyStatus=phyStatus, linkModeOptimizationMismatch=linkModeOptimizationMismatch, licenceGroup=licenceGroup, userRole=userRole, receiveFreqMHz=receiveFreqMHz, ptpTraps=ptpTraps, telnetAccessEnabled=telnetAccessEnabled, sNMPTrapVersion=sNMPTrapVersion, noWirelessChannelAvailable=noWirelessChannelAvailable, sMTPSourceEmailAddress=sMTPSourceEmailAddress, remoteMaximumTransmitPower=remoteMaximumTransmitPower, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, dataPortAutoMdix=dataPortAutoMdix, transmitPower=transmitPower, telecomGroup=telecomGroup, dfsTableIndex=dfsTableIndex, syslogLocalWrappedTrap=syslogLocalWrappedTrap, encryption=encryption, hardwareVersion=hardwareVersion, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, telecomsCableLength=telecomsCableLength, dfsGroup=dfsGroup, notificationsGroup=notificationsGroup, productName=productName, timeZone=timeZone, aggregateDataRate=aggregateDataRate, pubStatsGroup=pubStatsGroup, phyStatusGroup=phyStatusGroup, daylightSaving=daylightSaving, productVariant=productVariant, sntpGroup=sntpGroup, receivePower=receivePower, ptp=ptp, management=management, transmitModulationMode=transmitModulationMode, encryptionGroup=encryptionGroup, syslogStateTrap=syslogStateTrap, linkModeOptimisation=linkModeOptimisation, sNMPTrapIPAddress=sNMPTrapIPAddress, gatewayIPAddress=gatewayIPAddress, userTableIndex=userTableIndex, tDDSynchronizationAlarm=tDDSynchronizationAlarm, dataPortDisabledWarning=dataPortDisabledWarning, userTableEntry=userTableEntry, snmpControl=snmpControl, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, smtp=smtp, tFTPServerPortNumber=tFTPServerPortNumber, latitude=latitude, userTable=userTable, vLANPriorityTable=vLANPriorityTable, dataPortAutoNegotiation=dataPortAutoNegotiation, transmitFreqMHz=transmitFreqMHz, sMTPServerPortNumber=sMTPServerPortNumber, subnetMask=subnetMask, configurationGroup=configurationGroup, linkSymmetry=linkSymmetry, sNTPLastSync=sNTPLastSync, vLANPriorityTableIndex=vLANPriorityTableIndex, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, vLANManagementVID=vLANManagementVID, vLANPriorityTableEntry=vLANPriorityTableEntry, vLANPriorityTableNumber=vLANPriorityTableNumber, receiveFreqKHz=receiveFreqKHz, groupID=groupID, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, supplementaryGroup=supplementaryGroup, frequencyVariant=frequencyVariant, vectorError=vectorError, tDDSynchronizationStatus=tDDSynchronizationStatus, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, dfsTable=dfsTable, telecomsChannelStatus=telecomsChannelStatus, sMTPEnabledMessages=sMTPEnabledMessages, telecomsLineCode=telecomsLineCode, sNMPCommunityString=sNMPCommunityString, hTTPSAccessEnabled=hTTPSAccessEnabled, receiveChannel=receiveChannel, linkName=linkName, receiveDataRate=receiveDataRate, ethernetFiberSupport=ethernetFiberSupport, dfsMeans=dfsMeans, hTTPSPortNumber=hTTPSPortNumber, userName=userName, channelChangeTrap=channelChangeTrap, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, channelBandwidth=channelBandwidth, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, installStatus=installStatus, wirelessLinkAvailability=wirelessLinkAvailability, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement)
