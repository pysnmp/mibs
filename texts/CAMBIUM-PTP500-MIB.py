...
# Produced by pysmi-1.1.3 at Mon Nov 22 12:13:14 2021
...
Integer32, enterprises, iso, Counter32, IpAddress, NotificationType, ModuleIdentity, Counter64, Bits, Gauge32, ObjectIdentity, MibIdentifier, TimeTicks, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "enterprises", "iso", "Counter32", "IpAddress", "NotificationType", "ModuleIdentity", "Counter64", "Bits", "Gauge32", "ObjectIdentity", "MibIdentifier", "TimeTicks", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", timeZone=timeZone, vectorError=vectorError, userTableIndex=userTableIndex, sNMPTrapTableNumber=sNMPTrapTableNumber, siteName=siteName, installArmStateTrap=installArmStateTrap, linkLoss=linkLoss, reset=reset, encryptionAlgorithm=encryptionAlgorithm, dfsTableNumber=dfsTableNumber, sNMPTrapPortNumber=sNMPTrapPortNumber, dataPortConfigurationMismatch=dataPortConfigurationMismatch, ethernetGroup=ethernetGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, remoteIPAddress=remoteIPAddress, ptpTrapPrefix=ptpTrapPrefix, transmitFreqMHz=transmitFreqMHz, syslogClient=syslogClient, sNTPServerPortNumber=sNTPServerPortNumber, userEnabled=userEnabled, ptpGroups=ptpGroups, tDDControl=tDDControl, sNMPv3Enable=sNMPv3Enable, ptp500=ptp500, useVLANForManagementInterfaces=useVLANForManagementInterfaces, syslogControl=syslogControl, antennaGain=antennaGain, phyStatusGroup=phyStatusGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, telecom=telecom, supplementary=supplementary, licensedCapacity=licensedCapacity, cableLoss=cableLoss, installArmState=installArmState, sNTPSync=sNTPSync, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, bootVersion=bootVersion, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, userName=userName, tDDControlGroup=tDDControlGroup, bandwidthVariant=bandwidthVariant, dataPortStatus=dataPortStatus, sNMPCommunityString=sNMPCommunityString, regionCodeTrap=regionCodeTrap, telnetAccessEnabled=telnetAccessEnabled, longitude=longitude, masterSlaveMode=masterSlaveMode, targetMACAddress=targetMACAddress, receiveModulationMode=receiveModulationMode, noWirelessChannelAvailable=noWirelessChannelAvailable, hTTPPortNumber=hTTPPortNumber, installStatusTrap=installStatusTrap, iPAddress=iPAddress, tDDSynchronizationStatus=tDDSynchronizationStatus, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dfsTableIndex=dfsTableIndex, enabledDiagnosticAlarms=enabledDiagnosticAlarms, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, licenceGroup=licenceGroup, cambium=cambium, dfsMeans=dfsMeans, sNTPSyncTrap=sNTPSyncTrap, groupID=groupID, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, telecomsLoopbackTrap=telecomsLoopbackTrap, syslogStateTrap=syslogStateTrap, phyStatus=phyStatus, dfsTable=dfsTable, sNTPServerIPAddress=sNTPServerIPAddress, systemClock=systemClock, dfsTableEntry=dfsTableEntry, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, tDDSynchronizationAlarm=tDDSynchronizationAlarm, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, altitude=altitude, linkModeOptimizationMismatch=linkModeOptimizationMismatch, tDDSynchronizationMode=tDDSynchronizationMode, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, userTableNumber=userTableNumber, vLANPriorityQueueMapping=vLANPriorityQueueMapping, pubStatsGroup=pubStatsGroup, syslogControlGroup=syslogControlGroup, phyControlGroup=phyControlGroup, dfsPeaks=dfsPeaks, channelBandwidth=channelBandwidth, aAAControlGroup=aAAControlGroup, constantPowerSpectralDensity=constantPowerSpectralDensity, dataPortDisabledWarning=dataPortDisabledWarning, dataPortStatusTrap=dataPortStatusTrap, transmitModulationMode=transmitModulationMode, hTTPSAccessEnabled=hTTPSAccessEnabled, versions=versions, configuration=configuration, dfs=dfs, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, telecomsChannelStatus=telecomsChannelStatus, resetGroup=resetGroup, dfsGroup=dfsGroup, range=range, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, ethernet=ethernet, channelChangeTrap=channelChangeTrap, daylightSaving=daylightSaving, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, encryptionGroup=encryptionGroup, systemReset=systemReset, transmitDataRate=transmitDataRate, receivePower=receivePower, managementGroup=managementGroup, dataPortWirelessDownAlert=dataPortWirelessDownAlert, incompatibleRegionCodes=incompatibleRegionCodes, sMTPServerIPAddress=sMTPServerIPAddress, sNMPTrapIPAddress=sNMPTrapIPAddress, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, remoteMaximumTransmitPower=remoteMaximumTransmitPower, eIRP=eIRP, latitude=latitude, smtpGroup=smtpGroup, syslogState=syslogState, byteErrorRatio=byteErrorRatio, transmitFreqKHz=transmitFreqKHz, vLANPriorityTableIndex=vLANPriorityTableIndex, telecomsInterface=telecomsInterface, sNMPTrapTable=sNMPTrapTable, telecomsChannelStatusTrap=telecomsChannelStatusTrap, telecomsLineCode=telecomsLineCode, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, ptpCompliance=ptpCompliance, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, userTableEntry=userTableEntry, wirelessLinkStatus=wirelessLinkStatus, userRole=userRole, vLANManagementVID=vLANManagementVID, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, dataPortAutoMdix=dataPortAutoMdix, aAAControl=aAAControl, accessMethod=accessMethod, frequencyVariant=frequencyVariant, aggregateDataRate=aggregateDataRate, unitOutOfCalibration=unitOutOfCalibration, snmpControlGroup=snmpControlGroup, signalStrengthRatio=signalStrengthRatio, telecomGroup=telecomGroup, supplementaryGroup=supplementaryGroup, receiveDataRate=receiveDataRate, userPassword=userPassword, hTTPSPortNumber=hTTPSPortNumber, ptmp=ptmp, sNMPTrapVersion=sNMPTrapVersion, receiveChannel=receiveChannel, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, telnetPortNumber=telnetPortNumber, pubStats=pubStats, linkName=linkName, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, licence=licence, vLANPriorityTableNumber=vLANPriorityTableNumber, notificationsGroup=notificationsGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, linkModeOptimisation=linkModeOptimisation, maximumTransmitPower=maximumTransmitPower, versionsGroup=versionsGroup, sNTPState=sNTPState, productVariant=productVariant, vLANPriorityTable=vLANPriorityTable, alarms=alarms, receiveFreqKHz=receiveFreqKHz, tFTPServerPortNumber=tFTPServerPortNumber, sntpGroup=sntpGroup, telecomsLoopback=telecomsLoopback, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, smtp=smtp, incompatibleMasterAndSlave=incompatibleMasterAndSlave, receiveFreqMHz=receiveFreqMHz, receiveModulationModeDetail=receiveModulationModeDetail, sMTPServerPortNumber=sMTPServerPortNumber, vLANManagementPriority=vLANManagementPriority, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNMPEnabledTraps=sNMPEnabledTraps, sntp=sntp, sNTPPollInterval=sNTPPollInterval, ptp=ptp, sNMPPortNumber=sNMPPortNumber, transmitPower=transmitPower, secondarySoftwareVersion=secondarySoftwareVersion, wirelessLinkAvailability=wirelessLinkAvailability, regionCode=regionCode, productName=productName, configurationGroup=configurationGroup, transmitChannel=transmitChannel, alarmsGroup=alarmsGroup, snmpControl=snmpControl, encryption=encryption, searchState=searchState, ptpTraps=ptpTraps, capacityLicenseMismatch=capacityLicenseMismatch, tFTPServerIPAddress=tFTPServerIPAddress, hTTPAccessEnabled=hTTPAccessEnabled, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sNTPLastSync=sNTPLastSync, installStatus=installStatus, syslogClientTrap=syslogClientTrap, PYSNMP_MODULE_ID=cambium, sMTPEmailAlert=sMTPEmailAlert, subnetMask=subnetMask, phyControl=phyControl, gatewayIPAddress=gatewayIPAddress, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, userTable=userTable, linkSymmetry=linkSymmetry, softwareVersion=softwareVersion, management=management, vLANPriorityTableEntry=vLANPriorityTableEntry, hardwareVersion=hardwareVersion, sMTPEnabledMessages=sMTPEnabledMessages, ethernetFiberSupport=ethernetFiberSupport, telecomsCableLength=telecomsCableLength)
