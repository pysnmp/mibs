...
# Produced by pysmi-1.1.3 at Sun Nov 28 17:06:25 2021
...
TimeTicks, Counter32, ModuleIdentity, Bits, ObjectIdentity, Counter64, Gauge32, Integer32, IpAddress, NotificationType, enterprises, Unsigned32, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, iso = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Counter32", "ModuleIdentity", "Bits", "ObjectIdentity", "Counter64", "Gauge32", "Integer32", "IpAddress", "NotificationType", "enterprises", "Unsigned32", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", telecomsCableLength=telecomsCableLength, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPTrapIPAddress=sNMPTrapIPAddress, management=management, sMTPSourceEmailAddress=sMTPSourceEmailAddress, wirelessLinkStatus=wirelessLinkStatus, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, tFTPServerPortNumber=tFTPServerPortNumber, vLANPriorityTable=vLANPriorityTable, receiveChannel=receiveChannel, secondarySoftwareVersion=secondarySoftwareVersion, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, encryption=encryption, telnetPortNumber=telnetPortNumber, syslogLocalWrappedTrap=syslogLocalWrappedTrap, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sNTPLastSync=sNTPLastSync, pubStats=pubStats, aggregateDataRate=aggregateDataRate, userTableIndex=userTableIndex, installArmStateTrap=installArmStateTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, reset=reset, dfs=dfs, encryptionGroup=encryptionGroup, dataPortDisabledWarning=dataPortDisabledWarning, unitOutOfCalibration=unitOutOfCalibration, timeZone=timeZone, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, licenceGroup=licenceGroup, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, groupID=groupID, syslogStateTrap=syslogStateTrap, searchState=searchState, userPassword=userPassword, tDDControlGroup=tDDControlGroup, telecomsChannelStatus=telecomsChannelStatus, installStatusTrap=installStatusTrap, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, transmitModulationMode=transmitModulationMode, sNTPSync=sNTPSync, transmitDataRate=transmitDataRate, capacityLicenseMismatch=capacityLicenseMismatch, dataPortWirelessDownAlert=dataPortWirelessDownAlert, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, linkLoss=linkLoss, accessMethod=accessMethod, sMTPEmailAlert=sMTPEmailAlert, subnetMask=subnetMask, receiveModulationMode=receiveModulationMode, noWirelessChannelAvailable=noWirelessChannelAvailable, alarms=alarms, dfsMeans=dfsMeans, userTableNumber=userTableNumber, productVariant=productVariant, dataPortStatusTrap=dataPortStatusTrap, linkModeOptimizationMismatch=linkModeOptimizationMismatch, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, dataPortAutoNegotiation=dataPortAutoNegotiation, range=range, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, frequencyVariant=frequencyVariant, pubStatsGroup=pubStatsGroup, syslogControlGroup=syslogControlGroup, tDDSynchronizationAlarm=tDDSynchronizationAlarm, installStatus=installStatus, sMTPServerIPAddress=sMTPServerIPAddress, encryptionAlgorithm=encryptionAlgorithm, hTTPSPortNumber=hTTPSPortNumber, syslogState=syslogState, versions=versions, alarmsGroup=alarmsGroup, tDDSynchronizationStatus=tDDSynchronizationStatus, remoteMaximumTransmitPower=remoteMaximumTransmitPower, productName=productName, masterSlaveMode=masterSlaveMode, receiveFreqKHz=receiveFreqKHz, sMTPEnabledMessages=sMTPEnabledMessages, PYSNMP_MODULE_ID=cambium, transmitChannel=transmitChannel, smtpGroup=smtpGroup, tDDSynchronizationMode=tDDSynchronizationMode, sNMPEnabledTraps=sNMPEnabledTraps, siteName=siteName, phyStatus=phyStatus, wirelessLinkAvailability=wirelessLinkAvailability, ptpGroups=ptpGroups, systemReset=systemReset, versionsGroup=versionsGroup, softwareVersion=softwareVersion, userEnabled=userEnabled, telecomsChannelStatusTrap=telecomsChannelStatusTrap, sNTPSyncTrap=sNTPSyncTrap, receiveFreqMHz=receiveFreqMHz, tDDControl=tDDControl, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPTrapTableNumber=sNMPTrapTableNumber, linkName=linkName, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, syslogClientTrap=syslogClientTrap, vLANPriorityTableNumber=vLANPriorityTableNumber, signalStrengthRatio=signalStrengthRatio, ptpTraps=ptpTraps, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, telecomsLineCode=telecomsLineCode, userRole=userRole, ethernetGroup=ethernetGroup, receiveDataRate=receiveDataRate, vLANPriorityTableIndex=vLANPriorityTableIndex, configuration=configuration, snmpControlGroup=snmpControlGroup, sNTPPollInterval=sNTPPollInterval, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, hTTPAccessEnabled=hTTPAccessEnabled, incompatibleRegionCodes=incompatibleRegionCodes, aAAControl=aAAControl, sNMPPortNumber=sNMPPortNumber, linkModeOptimisation=linkModeOptimisation, telecomsLoopback=telecomsLoopback, constantPowerSpectralDensity=constantPowerSpectralDensity, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, phyControl=phyControl, userTable=userTable, licensedCapacity=licensedCapacity, transmitFreqMHz=transmitFreqMHz, sntp=sntp, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, dfsTable=dfsTable, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, useVLANForManagementInterfaces=useVLANForManagementInterfaces, telecomsInterface=telecomsInterface, vectorError=vectorError, channelChangeTrap=channelChangeTrap, latitude=latitude, sNTPState=sNTPState, ptpTrapPrefix=ptpTrapPrefix, configurationGroup=configurationGroup, aAAControlGroup=aAAControlGroup, snmpControl=snmpControl, sNMPTrapVersion=sNMPTrapVersion, telecomsLoopbackTrap=telecomsLoopbackTrap, licence=licence, incompatibleMasterAndSlave=incompatibleMasterAndSlave, telnetAccessEnabled=telnetAccessEnabled, receivePower=receivePower, notificationsGroup=notificationsGroup, sNTPServerPortNumber=sNTPServerPortNumber, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, cambium=cambium, dfsPeaks=dfsPeaks, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, dfsTableNumber=dfsTableNumber, phyControlGroup=phyControlGroup, sntpGroup=sntpGroup, tFTPServerIPAddress=tFTPServerIPAddress, hTTPSAccessEnabled=hTTPSAccessEnabled, ptp=ptp, sNMPTrapPortNumber=sNMPTrapPortNumber, daylightSaving=daylightSaving, userName=userName, channelBandwidth=channelBandwidth, ethernet=ethernet, dataPortStatus=dataPortStatus, gatewayIPAddress=gatewayIPAddress, ethernetFiberSupport=ethernetFiberSupport, supplementaryGroup=supplementaryGroup, syslogClient=syslogClient, regionCodeTrap=regionCodeTrap, hTTPPortNumber=hTTPPortNumber, sNMPCommunityString=sNMPCommunityString, byteErrorRatio=byteErrorRatio, telecomGroup=telecomGroup, managementGroup=managementGroup, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, receiveModulationModeDetail=receiveModulationModeDetail, sNMPTrapTable=sNMPTrapTable, supplementary=supplementary, vLANPriorityTableEntry=vLANPriorityTableEntry, cableLoss=cableLoss, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, resetGroup=resetGroup, bootVersion=bootVersion, maximumTransmitPower=maximumTransmitPower, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, vLANManagementVID=vLANManagementVID, sNMPv3Enable=sNMPv3Enable, transmitPower=transmitPower, regionCode=regionCode, dfsTableIndex=dfsTableIndex, sNTPServerIPAddress=sNTPServerIPAddress, longitude=longitude, bandwidthVariant=bandwidthVariant, sNMPTrapTableIndex=sNMPTrapTableIndex, telecom=telecom, altitude=altitude, hardwareVersion=hardwareVersion, syslogControl=syslogControl, antennaGain=antennaGain, dfsGroup=dfsGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, ptp500=ptp500, transmitFreqKHz=transmitFreqKHz, enabledDiagnosticAlarms=enabledDiagnosticAlarms, installArmState=installArmState, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, ptmp=ptmp, phyStatusGroup=phyStatusGroup, vLANManagementPriority=vLANManagementPriority, iPAddress=iPAddress, linkSymmetry=linkSymmetry, dfsTableEntry=dfsTableEntry, targetMACAddress=targetMACAddress, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sMTPServerPortNumber=sMTPServerPortNumber, systemClock=systemClock, userTableEntry=userTableEntry, smtp=smtp, eIRP=eIRP, ptpCompliance=ptpCompliance, dataPortAutoMdix=dataPortAutoMdix, remoteIPAddress=remoteIPAddress)
