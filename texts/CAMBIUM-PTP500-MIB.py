...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", cambium=cambium, vLANPriorityTable=vLANPriorityTable, installArmState=installArmState, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, versionsGroup=versionsGroup, phyControlGroup=phyControlGroup, aAAControl=aAAControl, incompatibleMasterAndSlave=incompatibleMasterAndSlave, pubStats=pubStats, transmitPower=transmitPower, ptpCompliance=ptpCompliance, channelBandwidth=channelBandwidth, userTableNumber=userTableNumber, receiveModulationMode=receiveModulationMode, dfsMeans=dfsMeans, syslogControlGroup=syslogControlGroup, sNMPEnabledTraps=sNMPEnabledTraps, licence=licence, installStatus=installStatus, hTTPSPortNumber=hTTPSPortNumber, phyControl=phyControl, productVariant=productVariant, telecomsChannelStatusTrap=telecomsChannelStatusTrap, versions=versions, licenceGroup=licenceGroup, ptpTraps=ptpTraps, dfsPeaks=dfsPeaks, altitude=altitude, encryption=encryption, receiveModulationModeDetail=receiveModulationModeDetail, telecomsInterface=telecomsInterface, userPassword=userPassword, sMTPServerPortNumber=sMTPServerPortNumber, antennaGain=antennaGain, remoteIPAddress=remoteIPAddress, telecomsCableLength=telecomsCableLength, linkName=linkName, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, tDDControlGroup=tDDControlGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, smtpGroup=smtpGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, sNMPTrapTableNumber=sNMPTrapTableNumber, tFTPServerPortNumber=tFTPServerPortNumber, telecomsLoopback=telecomsLoopback, telecomGroup=telecomGroup, groupID=groupID, alarms=alarms, systemClock=systemClock, dfsGroup=dfsGroup, linkModeOptimisation=linkModeOptimisation, alarmsGroup=alarmsGroup, telnetPortNumber=telnetPortNumber, systemReset=systemReset, sMTPEmailAlert=sMTPEmailAlert, vLANPriorityTableEntry=vLANPriorityTableEntry, constantPowerSpectralDensity=constantPowerSpectralDensity, syslogState=syslogState, transmitFreqMHz=transmitFreqMHz, dataPortStatusTrap=dataPortStatusTrap, dfsTableNumber=dfsTableNumber, accessMethod=accessMethod, telnetAccessEnabled=telnetAccessEnabled, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, phyStatus=phyStatus, vLANPriorityTableIndex=vLANPriorityTableIndex, transmitDataRate=transmitDataRate, hTTPAccessEnabled=hTTPAccessEnabled, tFTPServerIPAddress=tFTPServerIPAddress, receiveDataRate=receiveDataRate, latitude=latitude, dfsTableIndex=dfsTableIndex, signalStrengthRatio=signalStrengthRatio, sNMPTrapPortNumber=sNMPTrapPortNumber, configuration=configuration, ptpGroups=ptpGroups, dataPortWirelessDownAlert=dataPortWirelessDownAlert, regionCode=regionCode, linkModeOptimizationMismatch=linkModeOptimizationMismatch, tDDSynchronizationMode=tDDSynchronizationMode, configurationGroup=configurationGroup, sMTPServerIPAddress=sMTPServerIPAddress, sNTPServerPortNumber=sNTPServerPortNumber, regionCodeTrap=regionCodeTrap, snmpControlGroup=snmpControlGroup, sNTPServerIPAddress=sNTPServerIPAddress, iPAddress=iPAddress, linkLoss=linkLoss, sNTPState=sNTPState, cableLoss=cableLoss, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, ptp=ptp, ptmp=ptmp, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, vLANManagementPriority=vLANManagementPriority, supplementaryGroup=supplementaryGroup, vLANPriorityTableNumber=vLANPriorityTableNumber, bandwidthVariant=bandwidthVariant, transmitModulationMode=transmitModulationMode, receivePower=receivePower, userName=userName, managementGroup=managementGroup, userRole=userRole, hTTPSAccessEnabled=hTTPSAccessEnabled, tDDSynchronizationStatus=tDDSynchronizationStatus, sNTPSync=sNTPSync, sMTPEnabledMessages=sMTPEnabledMessages, vLANPriorityQueueMapping=vLANPriorityQueueMapping, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, hTTPPortNumber=hTTPPortNumber, noWirelessChannelAvailable=noWirelessChannelAvailable, syslogLocalWrappedTrap=syslogLocalWrappedTrap, PYSNMP_MODULE_ID=cambium, eIRP=eIRP, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, wirelessLinkStatus=wirelessLinkStatus, syslogControl=syslogControl, wirelessLinkAvailability=wirelessLinkAvailability, sNMPTrapIPAddress=sNMPTrapIPAddress, pubStatsGroup=pubStatsGroup, snmpControl=snmpControl, ethernet=ethernet, transmitFreqKHz=transmitFreqKHz, userTable=userTable, subnetMask=subnetMask, daylightSaving=daylightSaving, reset=reset, sNMPTrapTableEntry=sNMPTrapTableEntry, telecomsLineCode=telecomsLineCode, sNTPPollInterval=sNTPPollInterval, syslogClient=syslogClient, secondarySoftwareVersion=secondarySoftwareVersion, syslogStateTrap=syslogStateTrap, dfs=dfs, receiveFreqMHz=receiveFreqMHz, dataPortStatus=dataPortStatus, channelChangeTrap=channelChangeTrap, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, dfsTableEntry=dfsTableEntry, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, aggregateDataRate=aggregateDataRate, sntp=sntp, sNMPCommunityString=sNMPCommunityString, longitude=longitude, hardwareVersion=hardwareVersion, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, productName=productName, range=range, dataPortAutoMdix=dataPortAutoMdix, sNMPTrapTable=sNMPTrapTable, searchState=searchState, receiveFreqKHz=receiveFreqKHz, timeZone=timeZone, installArmStateTrap=installArmStateTrap, transmitChannel=transmitChannel, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, syslogClientTrap=syslogClientTrap, licensedCapacity=licensedCapacity, byteErrorRatio=byteErrorRatio, frequencyVariant=frequencyVariant, maximumTransmitPower=maximumTransmitPower, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, userTableEntry=userTableEntry, ptpTrapPrefix=ptpTrapPrefix, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sntpGroup=sntpGroup, sNMPv3Enable=sNMPv3Enable, smtp=smtp, ethernetGroup=ethernetGroup, receiveChannel=receiveChannel, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, siteName=siteName, userEnabled=userEnabled, tDDControl=tDDControl, sMTPSourceEmailAddress=sMTPSourceEmailAddress, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, vLANManagementVID=vLANManagementVID, installStatusTrap=installStatusTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, unitOutOfCalibration=unitOutOfCalibration, sNTPSyncTrap=sNTPSyncTrap, targetMACAddress=targetMACAddress, enabledDiagnosticAlarms=enabledDiagnosticAlarms, incompatibleRegionCodes=incompatibleRegionCodes, sNMPPortNumber=sNMPPortNumber, encryptionGroup=encryptionGroup, sNTPLastSync=sNTPLastSync, encryptionAlgorithm=encryptionAlgorithm, useVLANForManagementInterfaces=useVLANForManagementInterfaces, bootVersion=bootVersion, userTableIndex=userTableIndex, sNMPTrapVersion=sNMPTrapVersion, dfsTable=dfsTable, remoteMaximumTransmitPower=remoteMaximumTransmitPower, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, aAAControlGroup=aAAControlGroup, capacityLicenseMismatch=capacityLicenseMismatch, linkSymmetry=linkSymmetry, supplementary=supplementary, telecom=telecom, ethernetFiberSupport=ethernetFiberSupport, dataPortDisabledWarning=dataPortDisabledWarning, notificationsGroup=notificationsGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, tDDSynchronizationAlarm=tDDSynchronizationAlarm, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, phyStatusGroup=phyStatusGroup, ptp500=ptp500, dataPortConfigurationMismatch=dataPortConfigurationMismatch, gatewayIPAddress=gatewayIPAddress, resetGroup=resetGroup, management=management, masterSlaveMode=masterSlaveMode, telecomsChannelStatus=telecomsChannelStatus, vectorError=vectorError, softwareVersion=softwareVersion, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dataPortAutoNegotiation=dataPortAutoNegotiation, telecomsLoopbackTrap=telecomsLoopbackTrap, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText)
