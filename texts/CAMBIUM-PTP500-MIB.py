...
# Produced by pysmi-1.1.3 at Mon Nov 22 12:27:38 2021
...
enterprises, IpAddress, MibIdentifier, Integer32, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, ObjectIdentity, Gauge32, Unsigned32, TimeTicks, ModuleIdentity, Bits, NotificationType, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "IpAddress", "MibIdentifier", "Integer32", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "ObjectIdentity", "Gauge32", "Unsigned32", "TimeTicks", "ModuleIdentity", "Bits", "NotificationType", "Counter32")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", secondarySoftwareVersion=secondarySoftwareVersion, configurationGroup=configurationGroup, productVariant=productVariant, altitude=altitude, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, telnetPortNumber=telnetPortNumber, signalStrengthRatio=signalStrengthRatio, dfsPeaks=dfsPeaks, gatewayIPAddress=gatewayIPAddress, sNTPPollInterval=sNTPPollInterval, aAAControlGroup=aAAControlGroup, transmitFreqKHz=transmitFreqKHz, hTTPPortNumber=hTTPPortNumber, linkSymmetry=linkSymmetry, masterSlaveMode=masterSlaveMode, receiveDataRate=receiveDataRate, snmpControlGroup=snmpControlGroup, vLANPriorityTableEntry=vLANPriorityTableEntry, management=management, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, sNTPSync=sNTPSync, encryption=encryption, phyControlGroup=phyControlGroup, sntpGroup=sntpGroup, encryptionGroup=encryptionGroup, ptpTraps=ptpTraps, dfsMeans=dfsMeans, sntp=sntp, syslogState=syslogState, siteName=siteName, dataPortStatusTrap=dataPortStatusTrap, reset=reset, regionCode=regionCode, installStatusTrap=installStatusTrap, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, sNMPEnabledTraps=sNMPEnabledTraps, wirelessLinkStatus=wirelessLinkStatus, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNTPServerPortNumber=sNTPServerPortNumber, userEnabled=userEnabled, userTableEntry=userTableEntry, subnetMask=subnetMask, dfsTableEntry=dfsTableEntry, telecomsInterface=telecomsInterface, dfs=dfs, remoteMaximumTransmitPower=remoteMaximumTransmitPower, wirelessLinkAvailability=wirelessLinkAvailability, longitude=longitude, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, alarmsGroup=alarmsGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, latitude=latitude, sNMPTrapPortNumber=sNMPTrapPortNumber, telecomsCableLength=telecomsCableLength, tDDControlGroup=tDDControlGroup, incompatibleRegionCodes=incompatibleRegionCodes, PYSNMP_MODULE_ID=cambium, dfsTableNumber=dfsTableNumber, phyControl=phyControl, sNMPTrapVersion=sNMPTrapVersion, sNTPLastSync=sNTPLastSync, daylightSaving=daylightSaving, pubStats=pubStats, timeZone=timeZone, vLANManagementPriority=vLANManagementPriority, sNTPServerIPAddress=sNTPServerIPAddress, licence=licence, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, receiveModulationMode=receiveModulationMode, iPAddress=iPAddress, linkModeOptimisation=linkModeOptimisation, snmpControl=snmpControl, telnetAccessEnabled=telnetAccessEnabled, dataPortConfigurationMismatch=dataPortConfigurationMismatch, tDDSynchronizationMode=tDDSynchronizationMode, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, incompatibleMasterAndSlave=incompatibleMasterAndSlave, ptpTrapPrefix=ptpTrapPrefix, sMTPServerPortNumber=sMTPServerPortNumber, syslogStateTrap=syslogStateTrap, searchState=searchState, bootVersion=bootVersion, dfsTableIndex=dfsTableIndex, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, cableLoss=cableLoss, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, channelChangeTrap=channelChangeTrap, targetMACAddress=targetMACAddress, telecomsChannelStatusTrap=telecomsChannelStatusTrap, smtp=smtp, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sNMPTrapTableNumber=sNMPTrapTableNumber, receiveFreqMHz=receiveFreqMHz, range=range, remoteIPAddress=remoteIPAddress, enabledDiagnosticAlarms=enabledDiagnosticAlarms, softwareVersion=softwareVersion, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, transmitModulationMode=transmitModulationMode, tFTPServerIPAddress=tFTPServerIPAddress, managementGroup=managementGroup, ptpCompliance=ptpCompliance, ptpGroups=ptpGroups, syslogClient=syslogClient, versions=versions, notificationsGroup=notificationsGroup, bandwidthVariant=bandwidthVariant, resetGroup=resetGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, userTableIndex=userTableIndex, productName=productName, installStatus=installStatus, phyStatus=phyStatus, tFTPServerPortNumber=tFTPServerPortNumber, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPCommunityString=sNMPCommunityString, installArmStateTrap=installArmStateTrap, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, groupID=groupID, dataPortStatus=dataPortStatus, sNMPPortNumber=sNMPPortNumber, telecomsLoopbackTrap=telecomsLoopbackTrap, ethernetFiberSupport=ethernetFiberSupport, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, configuration=configuration, linkName=linkName, installArmState=installArmState, aggregateDataRate=aggregateDataRate, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, userTableNumber=userTableNumber, alarms=alarms, versionsGroup=versionsGroup, telecomGroup=telecomGroup, pubStatsGroup=pubStatsGroup, userRole=userRole, receiveModulationModeDetail=receiveModulationModeDetail, tDDControl=tDDControl, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, dataPortAutoNegotiation=dataPortAutoNegotiation, userPassword=userPassword, sNMPTrapTableEntry=sNMPTrapTableEntry, syslogClientTrap=syslogClientTrap, ptp500=ptp500, accessMethod=accessMethod, telecomsLineCode=telecomsLineCode, sMTPEnabledMessages=sMTPEnabledMessages, sNMPTrapTable=sNMPTrapTable, userTable=userTable, vLANPriorityQueueMapping=vLANPriorityQueueMapping, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, vLANPriorityTableIndex=vLANPriorityTableIndex, aAAControl=aAAControl, ethernetGroup=ethernetGroup, ptmp=ptmp, byteErrorRatio=byteErrorRatio, vLANPriorityTable=vLANPriorityTable, constantPowerSpectralDensity=constantPowerSpectralDensity, hTTPAccessEnabled=hTTPAccessEnabled, licensedCapacity=licensedCapacity, transmitChannel=transmitChannel, receiveChannel=receiveChannel, tDDSynchronizationStatus=tDDSynchronizationStatus, dfsGroup=dfsGroup, ethernet=ethernet, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, tDDSynchronizationAlarm=tDDSynchronizationAlarm, vLANManagementVID=vLANManagementVID, linkLoss=linkLoss, dataPortDisabledWarning=dataPortDisabledWarning, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, phyStatusGroup=phyStatusGroup, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, hTTPSAccessEnabled=hTTPSAccessEnabled, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, maximumTransmitPower=maximumTransmitPower, ptp=ptp, sNTPSyncTrap=sNTPSyncTrap, telecom=telecom, encryptionAlgorithm=encryptionAlgorithm, dataPortAutoMdix=dataPortAutoMdix, linkModeOptimizationMismatch=linkModeOptimizationMismatch, frequencyVariant=frequencyVariant, channelBandwidth=channelBandwidth, useVLANForManagementInterfaces=useVLANForManagementInterfaces, sNTPState=sNTPState, antennaGain=antennaGain, dfsTable=dfsTable, hardwareVersion=hardwareVersion, transmitPower=transmitPower, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, receivePower=receivePower, hTTPSPortNumber=hTTPSPortNumber, vLANPriorityTableNumber=vLANPriorityTableNumber, regionCodeTrap=regionCodeTrap, sMTPEmailAlert=sMTPEmailAlert, systemClock=systemClock, syslogControl=syslogControl, userName=userName, capacityLicenseMismatch=capacityLicenseMismatch, vectorError=vectorError, transmitFreqMHz=transmitFreqMHz, dataPortWirelessDownAlert=dataPortWirelessDownAlert, supplementaryGroup=supplementaryGroup, sNMPTrapIPAddress=sNMPTrapIPAddress, syslogControlGroup=syslogControlGroup, cambium=cambium, eIRP=eIRP, unitOutOfCalibration=unitOutOfCalibration, licenceGroup=licenceGroup, smtpGroup=smtpGroup, sNMPv3Enable=sNMPv3Enable, systemReset=systemReset, telecomsLoopback=telecomsLoopback, telecomsChannelStatus=telecomsChannelStatus, receiveFreqKHz=receiveFreqKHz, transmitDataRate=transmitDataRate, supplementary=supplementary, sMTPServerIPAddress=sMTPServerIPAddress)
