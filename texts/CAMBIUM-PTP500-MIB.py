...
# ASN.1 source CAMBIUM-PTP500-MIB
# Source digest sha256:c4bacc38e323d2ab931f92a9e0d2e1f12edcd03e5e0432aefc704f70b9249822
...
if mibBuilder.loadTexts: cambium.setLastUpdated('2014-03-14 16:04')
...
dfsTable = MibTable((1, 3, 6, 1, 4, 1, 17713, 5, 3, 2), ).setMaxAccess("notaccessible")
...
dfsTableEntry = MibTableRow((1, 3, 6, 1, 4, 1, 17713, 5, 3, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CAMBIUM-PTP500-MIB", "dfsTableIndex"))
...
vLANPriorityTable = MibTable((1, 3, 6, 1, 4, 1, 17713, 5, 6, 15), ).setMaxAccess("notaccessible")
...
vLANPriorityTableEntry = MibTableRow((1, 3, 6, 1, 4, 1, 17713, 5, 6, 15, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CAMBIUM-PTP500-MIB", "vLANPriorityTableIndex"))
...
sNMPTrapTable = MibTable((1, 3, 6, 1, 4, 1, 17713, 5, 16, 4), ).setMaxAccess("notaccessible")
...
sNMPTrapTableEntry = MibTableRow((1, 3, 6, 1, 4, 1, 17713, 5, 16, 4, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CAMBIUM-PTP500-MIB", "sNMPTrapTableIndex"))
...
userTable = MibTable((1, 3, 6, 1, 4, 1, 17713, 5, 25, 2), ).setMaxAccess("notaccessible")
...
userTableEntry = MibTableRow((1, 3, 6, 1, 4, 1, 17713, 5, 25, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CAMBIUM-PTP500-MIB", "userTableIndex"))
...
dfsTableIndex = MibTableColumn((1, 3, 6, 1, 4, 1, 17713, 5, 3, 2, 1, 1), Integer32().subtype(subtypeSpec=ValueRangeConstraint(1, 63))).setMaxAccess("notaccessible")
...
vLANPriorityTableIndex = MibTableColumn((1, 3, 6, 1, 4, 1, 17713, 5, 6, 15, 1, 2), Integer32().subtype(subtypeSpec=ConstraintsUnion(SingleValueConstraint(1, 2, 3, 4, 5, 6, 7, 8, 9))).clone(namedValues=NamedValues(("p0", 1), ("p1", 2), ("p2", 3), ("p3", 4), ("p4", 5), ("p5", 6), ("p6", 7), ("p7", 8), ("untagged", 9)))).setMaxAccess("notaccessible")
...
sNMPTrapTableIndex = MibTableColumn((1, 3, 6, 1, 4, 1, 17713, 5, 16, 4, 1, 1), Integer32().subtype(subtypeSpec=ValueRangeConstraint(1, 2))).setMaxAccess("notaccessible")
...
userTableIndex = MibTableColumn((1, 3, 6, 1, 4, 1, 17713, 5, 25, 2, 1, 1), Integer32().subtype(subtypeSpec=ValueRangeConstraint(1, 10))).setMaxAccess("notaccessible")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", PYSNMP_MODULE_ID=cambium, aAAControl=aAAControl, aAAControlGroup=aAAControlGroup, accessMethod=accessMethod, aggregateDataRate=aggregateDataRate, alarms=alarms, alarmsGroup=alarmsGroup, altitude=altitude, antennaGain=antennaGain, bandwidthVariant=bandwidthVariant, bootVersion=bootVersion, byteErrorRatio=byteErrorRatio, cableLoss=cableLoss, cambium=cambium, capacityLicenseMismatch=capacityLicenseMismatch, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, channelBandwidth=channelBandwidth, channelChangeTrap=channelChangeTrap, configuration=configuration, configurationGroup=configurationGroup, constantPowerSpectralDensity=constantPowerSpectralDensity, dataPortAutoMdix=dataPortAutoMdix, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortConfigurationMismatch=dataPortConfigurationMismatch, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, dataPortDisabledWarning=dataPortDisabledWarning, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, dataPortStatus=dataPortStatus, dataPortStatusTrap=dataPortStatusTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, daylightSaving=daylightSaving, dfs=dfs, dfsGroup=dfsGroup, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, dfsMeans=dfsMeans, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, dfsPeaks=dfsPeaks, dfsTable=dfsTable, dfsTableEntry=dfsTableEntry, dfsTableIndex=dfsTableIndex, dfsTableNumber=dfsTableNumber, eIRP=eIRP, enabledDiagnosticAlarms=enabledDiagnosticAlarms, encryption=encryption, encryptionAlgorithm=encryptionAlgorithm, encryptionGroup=encryptionGroup, ethernet=ethernet, ethernetFiberSupport=ethernetFiberSupport, ethernetGroup=ethernetGroup, frequencyVariant=frequencyVariant, gatewayIPAddress=gatewayIPAddress, groupID=groupID, hTTPAccessEnabled=hTTPAccessEnabled, hTTPPortNumber=hTTPPortNumber, hTTPSAccessEnabled=hTTPSAccessEnabled, hTTPSPortNumber=hTTPSPortNumber, hardwareVersion=hardwareVersion, iPAddress=iPAddress, incompatibleMasterAndSlave=incompatibleMasterAndSlave, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, incompatibleRegionCodes=incompatibleRegionCodes, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, installArmState=installArmState, installArmStateTrap=installArmStateTrap, installStatus=installStatus, installStatusTrap=installStatusTrap, latitude=latitude, licence=licence, licenceGroup=licenceGroup, licensedCapacity=licensedCapacity, linkLoss=linkLoss, linkModeOptimisation=linkModeOptimisation, linkModeOptimizationMismatch=linkModeOptimizationMismatch, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, linkName=linkName, linkSymmetry=linkSymmetry, longitude=longitude, management=management, managementGroup=managementGroup, masterSlaveMode=masterSlaveMode, maximumTransmitPower=maximumTransmitPower, noWirelessChannelAvailable=noWirelessChannelAvailable, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, notificationsGroup=notificationsGroup, phyControl=phyControl, phyControlGroup=phyControlGroup, phyStatus=phyStatus, phyStatusGroup=phyStatusGroup, productName=productName, productVariant=productVariant, ptmp=ptmp, ptp500=ptp500, ptp=ptp, ptpCompliance=ptpCompliance, ptpGroups=ptpGroups, ptpTrapPrefix=ptpTrapPrefix, ptpTraps=ptpTraps, pubStats=pubStats, pubStatsGroup=pubStatsGroup, range=range, receiveChannel=receiveChannel, receiveDataRate=receiveDataRate, receiveFreqKHz=receiveFreqKHz, receiveFreqMHz=receiveFreqMHz, receiveModulationMode=receiveModulationMode, receiveModulationModeDetail=receiveModulationModeDetail, receivePower=receivePower, regionCode=regionCode, regionCodeTrap=regionCodeTrap, remoteIPAddress=remoteIPAddress, remoteMaximumTransmitPower=remoteMaximumTransmitPower, reset=reset, resetGroup=resetGroup, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sMTPEmailAlert=sMTPEmailAlert, sMTPEnabledMessages=sMTPEnabledMessages, sMTPServerIPAddress=sMTPServerIPAddress, sMTPServerPortNumber=sMTPServerPortNumber, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNMPCommunityString=sNMPCommunityString, sNMPEnabledTraps=sNMPEnabledTraps, sNMPPortNumber=sNMPPortNumber, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, sNMPTrapIPAddress=sNMPTrapIPAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, sNMPTrapTable=sNMPTrapTable, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPTrapTableIndex=sNMPTrapTableIndex, sNMPTrapTableNumber=sNMPTrapTableNumber, sNMPTrapVersion=sNMPTrapVersion, sNMPv3Enable=sNMPv3Enable, sNTPLastSync=sNTPLastSync, sNTPPollInterval=sNTPPollInterval, sNTPServerIPAddress=sNTPServerIPAddress, sNTPServerPortNumber=sNTPServerPortNumber, sNTPState=sNTPState, sNTPSync=sNTPSync, sNTPSyncTrap=sNTPSyncTrap, searchState=searchState, secondarySoftwareVersion=secondarySoftwareVersion, signalStrengthRatio=signalStrengthRatio, siteName=siteName, smtp=smtp, smtpGroup=smtpGroup, snmpControl=snmpControl, snmpControlGroup=snmpControlGroup, sntp=sntp, sntpGroup=sntpGroup, softwareVersion=softwareVersion, subnetMask=subnetMask, supplementary=supplementary, supplementaryGroup=supplementaryGroup, syslogClient=syslogClient, syslogClientTrap=syslogClientTrap, syslogControl=syslogControl, syslogControlGroup=syslogControlGroup, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, syslogState=syslogState, syslogStateTrap=syslogStateTrap, systemClock=systemClock, systemReset=systemReset, tDDControl=tDDControl, tDDControlGroup=tDDControlGroup, tDDSynchronizationAlarm=tDDSynchronizationAlarm, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, tDDSynchronizationMode=tDDSynchronizationMode, tDDSynchronizationStatus=tDDSynchronizationStatus, tFTPServerIPAddress=tFTPServerIPAddress, tFTPServerPortNumber=tFTPServerPortNumber, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, targetMACAddress=targetMACAddress, telecom=telecom, telecomGroup=telecomGroup, telecomsCableLength=telecomsCableLength, telecomsChannelStatus=telecomsChannelStatus, telecomsChannelStatusTrap=telecomsChannelStatusTrap, telecomsInterface=telecomsInterface, telecomsLineCode=telecomsLineCode, telecomsLoopback=telecomsLoopback, telecomsLoopbackTrap=telecomsLoopbackTrap, telnetAccessEnabled=telnetAccessEnabled, telnetPortNumber=telnetPortNumber, timeZone=timeZone, transmitChannel=transmitChannel, transmitDataRate=transmitDataRate, transmitFreqKHz=transmitFreqKHz, transmitFreqMHz=transmitFreqMHz, transmitModulationMode=transmitModulationMode, transmitPower=transmitPower, unitOutOfCalibration=unitOutOfCalibration, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, useVLANForManagementInterfaces=useVLANForManagementInterfaces, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, userEnabled=userEnabled, userName=userName, userPassword=userPassword, userRole=userRole, userTable=userTable, userTableEntry=userTableEntry, userTableIndex=userTableIndex, userTableNumber=userTableNumber, vLANManagementPriority=vLANManagementPriority, vLANManagementVID=vLANManagementVID, vLANPriorityQueueMapping=vLANPriorityQueueMapping, vLANPriorityTable=vLANPriorityTable, vLANPriorityTableEntry=vLANPriorityTableEntry, vLANPriorityTableIndex=vLANPriorityTableIndex, vLANPriorityTableNumber=vLANPriorityTableNumber, vectorError=vectorError, versions=versions, versionsGroup=versionsGroup, wirelessLinkAvailability=wirelessLinkAvailability, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, wirelessLinkStatus=wirelessLinkStatus)
