...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", linkSymmetry=linkSymmetry, tDDSynchronizationMode=tDDSynchronizationMode, installStatus=installStatus, syslogStateTrap=syslogStateTrap, vLANManagementVID=vLANManagementVID, sNMPTrapTable=sNMPTrapTable, phyStatus=phyStatus, altitude=altitude, syslogClient=syslogClient, telecomsChannelStatusTrap=telecomsChannelStatusTrap, ethernet=ethernet, bandwidthVariant=bandwidthVariant, regionCodeTrap=regionCodeTrap, vLANPriorityQueueMapping=vLANPriorityQueueMapping, userPassword=userPassword, daylightSaving=daylightSaving, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, dfsTable=dfsTable, syslogControl=syslogControl, telecomsCableLength=telecomsCableLength, vLANPriorityTableEntry=vLANPriorityTableEntry, supplementaryGroup=supplementaryGroup, maximumTransmitPower=maximumTransmitPower, targetMACAddress=targetMACAddress, tDDControl=tDDControl, receiveDataRate=receiveDataRate, userTableEntry=userTableEntry, range=range, receiveModulationMode=receiveModulationMode, longitude=longitude, telecom=telecom, byteErrorRatio=byteErrorRatio, sNMPPortNumber=sNMPPortNumber, sNMPTrapIPAddress=sNMPTrapIPAddress, telecomsInterface=telecomsInterface, tFTPServerPortNumber=tFTPServerPortNumber, dfsMeans=dfsMeans, systemClock=systemClock, vLANPriorityTableIndex=vLANPriorityTableIndex, softwareVersion=softwareVersion, alarmsGroup=alarmsGroup, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, smtpGroup=smtpGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, channelChangeTrap=channelChangeTrap, dataPortConfigurationMismatch=dataPortConfigurationMismatch, snmpControlGroup=snmpControlGroup, constantPowerSpectralDensity=constantPowerSpectralDensity, channelBandwidth=channelBandwidth, vectorError=vectorError, installStatusTrap=installStatusTrap, configurationGroup=configurationGroup, sNTPState=sNTPState, siteName=siteName, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, transmitPower=transmitPower, dataPortAutoMdix=dataPortAutoMdix, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, licenceGroup=licenceGroup, versionsGroup=versionsGroup, encryptionAlgorithm=encryptionAlgorithm, syslogState=syslogState, cambium=cambium, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, ptp=ptp, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, tDDControlGroup=tDDControlGroup, userTableNumber=userTableNumber, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, telnetPortNumber=telnetPortNumber, receiveFreqKHz=receiveFreqKHz, receiveFreqMHz=receiveFreqMHz, sNTPLastSync=sNTPLastSync, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dfsPeaks=dfsPeaks, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPTrapPortNumber=sNMPTrapPortNumber, sMTPSourceEmailAddress=sMTPSourceEmailAddress, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, PYSNMP_MODULE_ID=cambium, sNMPTrapVersion=sNMPTrapVersion, useVLANForManagementInterfaces=useVLANForManagementInterfaces, ptpTraps=ptpTraps, telecomGroup=telecomGroup, notificationsGroup=notificationsGroup, ethernetGroup=ethernetGroup, sNMPTrapTableNumber=sNMPTrapTableNumber, userName=userName, pubStatsGroup=pubStatsGroup, sMTPEmailAlert=sMTPEmailAlert, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, sNTPPollInterval=sNTPPollInterval, enabledDiagnosticAlarms=enabledDiagnosticAlarms, dfsGroup=dfsGroup, searchState=searchState, sNTPSync=sNTPSync, linkModeOptimisation=linkModeOptimisation, hTTPPortNumber=hTTPPortNumber, transmitFreqMHz=transmitFreqMHz, latitude=latitude, sNMPTrapTableIndex=sNMPTrapTableIndex, masterSlaveMode=masterSlaveMode, ptpGroups=ptpGroups, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, aAAControl=aAAControl, tDDSynchronizationStatus=tDDSynchronizationStatus, receiveModulationModeDetail=receiveModulationModeDetail, dfsTableEntry=dfsTableEntry, sNTPServerPortNumber=sNTPServerPortNumber, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, licensedCapacity=licensedCapacity, eIRP=eIRP, versions=versions, signalStrengthRatio=signalStrengthRatio, phyControlGroup=phyControlGroup, smtp=smtp, vLANManagementPriority=vLANManagementPriority, antennaGain=antennaGain, dataPortStatus=dataPortStatus, transmitFreqKHz=transmitFreqKHz, gatewayIPAddress=gatewayIPAddress, aAAControlGroup=aAAControlGroup, cableLoss=cableLoss, productName=productName, subnetMask=subnetMask, telnetAccessEnabled=telnetAccessEnabled, capacityLicenseMismatch=capacityLicenseMismatch, ptmp=ptmp, userTableIndex=userTableIndex, frequencyVariant=frequencyVariant, ptpTrapPrefix=ptpTrapPrefix, unitOutOfCalibration=unitOutOfCalibration, hTTPSPortNumber=hTTPSPortNumber, ptp500=ptp500, resetGroup=resetGroup, managementGroup=managementGroup, userEnabled=userEnabled, encryption=encryption, noWirelessChannelAvailable=noWirelessChannelAvailable, linkLoss=linkLoss, sMTPServerPortNumber=sMTPServerPortNumber, secondarySoftwareVersion=secondarySoftwareVersion, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, pubStats=pubStats, vLANPriorityTable=vLANPriorityTable, sNMPEnabledTraps=sNMPEnabledTraps, iPAddress=iPAddress, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, phyControl=phyControl, aggregateDataRate=aggregateDataRate, sNTPSyncTrap=sNTPSyncTrap, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, vLANPriorityTableNumber=vLANPriorityTableNumber, configuration=configuration, sNMPv3Enable=sNMPv3Enable, licence=licence, timeZone=timeZone, groupID=groupID, syslogLocalWrappedTrap=syslogLocalWrappedTrap, transmitDataRate=transmitDataRate, ptpCompliance=ptpCompliance, phyStatusGroup=phyStatusGroup, dfsTableNumber=dfsTableNumber, dfsTableIndex=dfsTableIndex, telecomsChannelStatus=telecomsChannelStatus, hTTPSAccessEnabled=hTTPSAccessEnabled, installArmStateTrap=installArmStateTrap, transmitChannel=transmitChannel, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, hardwareVersion=hardwareVersion, supplementary=supplementary, receiveChannel=receiveChannel, linkName=linkName, bootVersion=bootVersion, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, telecomsLineCode=telecomsLineCode, productVariant=productVariant, ethernetFiberSupport=ethernetFiberSupport, dataPortDisabledWarning=dataPortDisabledWarning, snmpControl=snmpControl, dataPortStatusTrap=dataPortStatusTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, sNTPServerIPAddress=sNTPServerIPAddress, receivePower=receivePower, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, dfs=dfs, transmitModulationMode=transmitModulationMode, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, sNMPTrapTableEntry=sNMPTrapTableEntry, userRole=userRole, tFTPServerIPAddress=tFTPServerIPAddress, tDDSynchronizationAlarm=tDDSynchronizationAlarm, telecomsLoopback=telecomsLoopback, installArmState=installArmState, systemReset=systemReset, syslogControlGroup=syslogControlGroup, remoteIPAddress=remoteIPAddress, sNMPCommunityString=sNMPCommunityString, wirelessLinkStatus=wirelessLinkStatus, telecomsLoopbackTrap=telecomsLoopbackTrap, regionCode=regionCode, remoteMaximumTransmitPower=remoteMaximumTransmitPower, alarms=alarms, reset=reset, incompatibleRegionCodes=incompatibleRegionCodes, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, encryptionGroup=encryptionGroup, sMTPEnabledMessages=sMTPEnabledMessages, sntp=sntp, wirelessLinkAvailability=wirelessLinkAvailability, userTable=userTable, sntpGroup=sntpGroup, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sMTPServerIPAddress=sMTPServerIPAddress, hTTPAccessEnabled=hTTPAccessEnabled, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, syslogClientTrap=syslogClientTrap, management=management, accessMethod=accessMethod)
