...
# Produced by pysmi-1.1.3 at Mon Nov 22 11:45:23 2021
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", sNMPTrapTableIndex=sNMPTrapTableIndex, sMTPServerPortNumber=sMTPServerPortNumber, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, receiveFreqMHz=receiveFreqMHz, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, siteName=siteName, aAAControl=aAAControl, softwareVersion=softwareVersion, byteErrorRatio=byteErrorRatio, cableLoss=cableLoss, telecomsLoopback=telecomsLoopback, sMTPSourceEmailAddress=sMTPSourceEmailAddress, constantPowerSpectralDensity=constantPowerSpectralDensity, range=range, sNMPTrapIPAddress=sNMPTrapIPAddress, maximumTransmitPower=maximumTransmitPower, unitOutOfCalibration=unitOutOfCalibration, sNMPTrapTable=sNMPTrapTable, encryptionAlgorithm=encryptionAlgorithm, vLANManagementVID=vLANManagementVID, reset=reset, supplementaryGroup=supplementaryGroup, userPassword=userPassword, accessMethod=accessMethod, licence=licence, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, vLANPriorityTable=vLANPriorityTable, dataPortAutoMdix=dataPortAutoMdix, sNTPSyncTrap=sNTPSyncTrap, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNMPv3Enable=sNMPv3Enable, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, smtp=smtp, receiveModulationModeDetail=receiveModulationModeDetail, installStatus=installStatus, dfsPeaks=dfsPeaks, transmitDataRate=transmitDataRate, ptpGroups=ptpGroups, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, ptpTrapPrefix=ptpTrapPrefix, telecomsLoopbackTrap=telecomsLoopbackTrap, tFTPServerIPAddress=tFTPServerIPAddress, wirelessLinkAvailability=wirelessLinkAvailability, pubStats=pubStats, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, hTTPSAccessEnabled=hTTPSAccessEnabled, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, dataPortDisabledWarning=dataPortDisabledWarning, dfs=dfs, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, sNMPTrapTableNumber=sNMPTrapTableNumber, syslogClient=syslogClient, ptp=ptp, installArmState=installArmState, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, secondarySoftwareVersion=secondarySoftwareVersion, wirelessLinkStatus=wirelessLinkStatus, receiveDataRate=receiveDataRate, aAAControlGroup=aAAControlGroup, versionsGroup=versionsGroup, regionCodeTrap=regionCodeTrap, vLANManagementPriority=vLANManagementPriority, signalStrengthRatio=signalStrengthRatio, notificationsGroup=notificationsGroup, licenceGroup=licenceGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, managementGroup=managementGroup, telecom=telecom, snmpControlGroup=snmpControlGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, telecomGroup=telecomGroup, searchState=searchState, channelBandwidth=channelBandwidth, receivePower=receivePower, timeZone=timeZone, dfsTableIndex=dfsTableIndex, transmitFreqKHz=transmitFreqKHz, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sNMPPortNumber=sNMPPortNumber, linkSymmetry=linkSymmetry, telecomsChannelStatus=telecomsChannelStatus, sNTPServerIPAddress=sNTPServerIPAddress, bootVersion=bootVersion, smtpGroup=smtpGroup, encryption=encryption, vLANPriorityTableNumber=vLANPriorityTableNumber, tDDSynchronizationStatus=tDDSynchronizationStatus, userTableNumber=userTableNumber, linkLoss=linkLoss, cambium=cambium, encryptionGroup=encryptionGroup, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, daylightSaving=daylightSaving, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, installStatusTrap=installStatusTrap, hTTPAccessEnabled=hTTPAccessEnabled, hTTPPortNumber=hTTPPortNumber, userEnabled=userEnabled, eIRP=eIRP, useVLANForManagementInterfaces=useVLANForManagementInterfaces, phyStatusGroup=phyStatusGroup, longitude=longitude, productName=productName, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNTPSync=sNTPSync, tDDSynchronizationMode=tDDSynchronizationMode, telecomsCableLength=telecomsCableLength, dfsMeans=dfsMeans, channelChangeTrap=channelChangeTrap, remoteIPAddress=remoteIPAddress, phyControlGroup=phyControlGroup, vLANPriorityTableIndex=vLANPriorityTableIndex, transmitFreqMHz=transmitFreqMHz, sMTPEnabledMessages=sMTPEnabledMessages, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sNTPState=sNTPState, sNMPTrapPortNumber=sNMPTrapPortNumber, latitude=latitude, userTable=userTable, telnetAccessEnabled=telnetAccessEnabled, tFTPServerPortNumber=tFTPServerPortNumber, sNTPServerPortNumber=sNTPServerPortNumber, ptp500=ptp500, ethernetFiberSupport=ethernetFiberSupport, sNMPTrapTableEntry=sNMPTrapTableEntry, syslogStateTrap=syslogStateTrap, frequencyVariant=frequencyVariant, receiveModulationMode=receiveModulationMode, aggregateDataRate=aggregateDataRate, ethernet=ethernet, sNMPEnabledTraps=sNMPEnabledTraps, sntpGroup=sntpGroup, phyControl=phyControl, bandwidthVariant=bandwidthVariant, transmitChannel=transmitChannel, pubStatsGroup=pubStatsGroup, linkModeOptimisation=linkModeOptimisation, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, userTableIndex=userTableIndex, regionCode=regionCode, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, syslogControl=syslogControl, groupID=groupID, systemReset=systemReset, iPAddress=iPAddress, supplementary=supplementary, dataPortStatus=dataPortStatus, installArmStateTrap=installArmStateTrap, userName=userName, configuration=configuration, alarms=alarms, receiveChannel=receiveChannel, linkName=linkName, ptpCompliance=ptpCompliance, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, alarmsGroup=alarmsGroup, gatewayIPAddress=gatewayIPAddress, sNTPPollInterval=sNTPPollInterval, tDDControl=tDDControl, noWirelessChannelAvailable=noWirelessChannelAvailable, dfsGroup=dfsGroup, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, PYSNMP_MODULE_ID=cambium, dataPortConfigurationMismatch=dataPortConfigurationMismatch, tDDControlGroup=tDDControlGroup, syslogLocalWrappedTrap=syslogLocalWrappedTrap, ptmp=ptmp, configurationGroup=configurationGroup, snmpControl=snmpControl, syslogControlGroup=syslogControlGroup, hardwareVersion=hardwareVersion, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, hTTPSPortNumber=hTTPSPortNumber, incompatibleMasterAndSlave=incompatibleMasterAndSlave, dfsTableEntry=dfsTableEntry, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, ethernetGroup=ethernetGroup, dfsTable=dfsTable, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, capacityLicenseMismatch=capacityLicenseMismatch, dataPortAutoNegotiation=dataPortAutoNegotiation, systemClock=systemClock, resetGroup=resetGroup, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, telecomsChannelStatusTrap=telecomsChannelStatusTrap, vLANPriorityTableEntry=vLANPriorityTableEntry, sNTPLastSync=sNTPLastSync, userRole=userRole, subnetMask=subnetMask, dataPortStatusTrap=dataPortStatusTrap, telnetPortNumber=telnetPortNumber, sNMPTrapVersion=sNMPTrapVersion, syslogClientTrap=syslogClientTrap, dfsTableNumber=dfsTableNumber, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, telecomsLineCode=telecomsLineCode, versions=versions, userTableEntry=userTableEntry, syslogState=syslogState, phyStatus=phyStatus, sMTPEmailAlert=sMTPEmailAlert, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, sMTPServerIPAddress=sMTPServerIPAddress, management=management, incompatibleRegionCodes=incompatibleRegionCodes, sntp=sntp, licensedCapacity=licensedCapacity, transmitPower=transmitPower, dataPortWirelessDownAlert=dataPortWirelessDownAlert, sNMPCommunityString=sNMPCommunityString, targetMACAddress=targetMACAddress, telecomsInterface=telecomsInterface, productVariant=productVariant, transmitModulationMode=transmitModulationMode, ptpTraps=ptpTraps, masterSlaveMode=masterSlaveMode, antennaGain=antennaGain, altitude=altitude, vectorError=vectorError, linkModeOptimizationMismatch=linkModeOptimizationMismatch, receiveFreqKHz=receiveFreqKHz)
