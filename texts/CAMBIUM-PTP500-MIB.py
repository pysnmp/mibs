...
Counter32, ObjectIdentity, NotificationType, iso, Bits, IpAddress, Counter64, MibIdentifier, enterprises, TimeTicks, Integer32, ModuleIdentity, Unsigned32, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "ObjectIdentity", "NotificationType", "iso", "Bits", "IpAddress", "Counter64", "MibIdentifier", "enterprises", "TimeTicks", "Integer32", "ModuleIdentity", "Unsigned32", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", userTableEntry=userTableEntry, timeZone=timeZone, dfsTableEntry=dfsTableEntry, telecomsCableLength=telecomsCableLength, ethernetFiberSupport=ethernetFiberSupport, dataPortWirelessDownAlert=dataPortWirelessDownAlert, receiveModulationMode=receiveModulationMode, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, userTableIndex=userTableIndex, licenceGroup=licenceGroup, vLANPriorityTableIndex=vLANPriorityTableIndex, installStatusTrap=installStatusTrap, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNTPSync=sNTPSync, dfsGroup=dfsGroup, sNMPTrapTable=sNMPTrapTable, encryptionAlgorithm=encryptionAlgorithm, cableLoss=cableLoss, productName=productName, smtp=smtp, frequencyVariant=frequencyVariant, installArmStateTrap=installArmStateTrap, telnetAccessEnabled=telnetAccessEnabled, sMTPServerIPAddress=sMTPServerIPAddress, signalStrengthRatio=signalStrengthRatio, management=management, iPAddress=iPAddress, sNMPv3Enable=sNMPv3Enable, ptpGroups=ptpGroups, aAAControlGroup=aAAControlGroup, receiveFreqMHz=receiveFreqMHz, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, tFTPServerIPAddress=tFTPServerIPAddress, channelChangeTrap=channelChangeTrap, sNMPTrapIPAddress=sNMPTrapIPAddress, dataPortStatus=dataPortStatus, telecomsChannelStatus=telecomsChannelStatus, syslogStateTrap=syslogStateTrap, linkModeOptimizationMismatch=linkModeOptimizationMismatch, secondarySoftwareVersion=secondarySoftwareVersion, linkLoss=linkLoss, configuration=configuration, transmitChannel=transmitChannel, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, ptp=ptp, sNTPSyncTrap=sNTPSyncTrap, vLANPriorityQueueMapping=vLANPriorityQueueMapping, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, telecomsLoopback=telecomsLoopback, ethernetGroup=ethernetGroup, syslogControlGroup=syslogControlGroup, telnetPortNumber=telnetPortNumber, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, tDDControl=tDDControl, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, telecomsChannelStatusTrap=telecomsChannelStatusTrap, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, ptpTrapPrefix=ptpTrapPrefix, receiveFreqKHz=receiveFreqKHz, sNTPState=sNTPState, encryptionGroup=encryptionGroup, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, syslogClientTrap=syslogClientTrap, transmitFreqMHz=transmitFreqMHz, userTable=userTable, sNMPCommunityString=sNMPCommunityString, ptpTraps=ptpTraps, masterSlaveMode=masterSlaveMode, snmpControlGroup=snmpControlGroup, linkSymmetry=linkSymmetry, syslogState=syslogState, sMTPServerPortNumber=sMTPServerPortNumber, encryption=encryption, cambium=cambium, unitOutOfCalibration=unitOutOfCalibration, useVLANForManagementInterfaces=useVLANForManagementInterfaces, notificationsGroup=notificationsGroup, productVariant=productVariant, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, tDDSynchronizationMode=tDDSynchronizationMode, pubStatsGroup=pubStatsGroup, dfs=dfs, subnetMask=subnetMask, noWirelessChannelAvailable=noWirelessChannelAvailable, ethernet=ethernet, tFTPServerPortNumber=tFTPServerPortNumber, incompatibleRegionCodes=incompatibleRegionCodes, sNMPTrapPortNumber=sNMPTrapPortNumber, dataPortAutoMdix=dataPortAutoMdix, sNMPTrapVersion=sNMPTrapVersion, dataPortStatusTrap=dataPortStatusTrap, sNTPLastSync=sNTPLastSync, hTTPAccessEnabled=hTTPAccessEnabled, userEnabled=userEnabled, ptpCompliance=ptpCompliance, channelBandwidth=channelBandwidth, remoteIPAddress=remoteIPAddress, receiveChannel=receiveChannel, dfsTable=dfsTable, sNMPTrapTableNumber=sNMPTrapTableNumber, telecomsInterface=telecomsInterface, sNMPTrapTableIndex=sNMPTrapTableIndex, altitude=altitude, phyControlGroup=phyControlGroup, alarmsGroup=alarmsGroup, configurationGroup=configurationGroup, phyStatus=phyStatus, incompatibleMasterAndSlave=incompatibleMasterAndSlave, longitude=longitude, telecom=telecom, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, supplementary=supplementary, hTTPPortNumber=hTTPPortNumber, userRole=userRole, supplementaryGroup=supplementaryGroup, vLANManagementPriority=vLANManagementPriority, snmpControl=snmpControl, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, vLANPriorityTable=vLANPriorityTable, sMTPEnabledMessages=sMTPEnabledMessages, bootVersion=bootVersion, byteErrorRatio=byteErrorRatio, regionCodeTrap=regionCodeTrap, syslogClient=syslogClient, userPassword=userPassword, alarms=alarms, sNTPServerPortNumber=sNTPServerPortNumber, dfsTableIndex=dfsTableIndex, gatewayIPAddress=gatewayIPAddress, PYSNMP_MODULE_ID=cambium, hardwareVersion=hardwareVersion, sMTPSourceEmailAddress=sMTPSourceEmailAddress, transmitPower=transmitPower, sNMPEnabledTraps=sNMPEnabledTraps, receiveDataRate=receiveDataRate, phyControl=phyControl, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, sMTPEmailAlert=sMTPEmailAlert, ptp500=ptp500, vLANManagementVID=vLANManagementVID, groupID=groupID, transmitModulationMode=transmitModulationMode, wirelessLinkStatus=wirelessLinkStatus, managementGroup=managementGroup, hTTPSPortNumber=hTTPSPortNumber, tDDSynchronizationStatus=tDDSynchronizationStatus, latitude=latitude, vLANPriorityTableNumber=vLANPriorityTableNumber, installArmState=installArmState, tDDControlGroup=tDDControlGroup, constantPowerSpectralDensity=constantPowerSpectralDensity, dataPortConfigurationMismatch=dataPortConfigurationMismatch, licensedCapacity=licensedCapacity, systemClock=systemClock, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, siteName=siteName, telecomsLoopbackTrap=telecomsLoopbackTrap, dataPortDisabledWarning=dataPortDisabledWarning, antennaGain=antennaGain, sntp=sntp, transmitDataRate=transmitDataRate, pubStats=pubStats, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, bandwidthVariant=bandwidthVariant, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, telecomGroup=telecomGroup, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sntpGroup=sntpGroup, linkName=linkName, searchState=searchState, syslogControl=syslogControl, ptmp=ptmp, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, phyStatusGroup=phyStatusGroup, capacityLicenseMismatch=capacityLicenseMismatch, resetGroup=resetGroup, regionCode=regionCode, userTableNumber=userTableNumber, reset=reset, accessMethod=accessMethod, vectorError=vectorError, receiveModulationModeDetail=receiveModulationModeDetail, range=range, transmitFreqKHz=transmitFreqKHz, sNTPServerIPAddress=sNTPServerIPAddress, sNMPTrapTableEntry=sNMPTrapTableEntry, remoteMaximumTransmitPower=remoteMaximumTransmitPower, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, versions=versions, dfsMeans=dfsMeans, sNMPPortNumber=sNMPPortNumber, licence=licence, dfsPeaks=dfsPeaks, receivePower=receivePower, wirelessLinkAvailability=wirelessLinkAvailability, telecomsLineCode=telecomsLineCode, targetMACAddress=targetMACAddress, linkModeOptimisation=linkModeOptimisation, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, sNTPPollInterval=sNTPPollInterval, eIRP=eIRP, maximumTransmitPower=maximumTransmitPower, smtpGroup=smtpGroup, hTTPSAccessEnabled=hTTPSAccessEnabled, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, vLANPriorityTableEntry=vLANPriorityTableEntry, userName=userName, aggregateDataRate=aggregateDataRate, systemReset=systemReset, daylightSaving=daylightSaving, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, dfsTableNumber=dfsTableNumber, versionsGroup=versionsGroup, installStatus=installStatus, aAAControl=aAAControl, softwareVersion=softwareVersion, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode)
