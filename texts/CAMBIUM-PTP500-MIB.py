...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", ptpCompliance=ptpCompliance, receivePower=receivePower, receiveChannel=receiveChannel, iPAddress=iPAddress, unitOutOfCalibration=unitOutOfCalibration, sNMPTrapTableNumber=sNMPTrapTableNumber, linkName=linkName, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, linkModeOptimisation=linkModeOptimisation, masterSlaveMode=masterSlaveMode, telecomsChannelStatus=telecomsChannelStatus, telecomsLoopback=telecomsLoopback, productName=productName, sNMPTrapIPAddress=sNMPTrapIPAddress, systemClock=systemClock, linkSymmetry=linkSymmetry, targetMACAddress=targetMACAddress, alarmsGroup=alarmsGroup, dfsTableEntry=dfsTableEntry, dataPortAutoNegotiation=dataPortAutoNegotiation, hardwareVersion=hardwareVersion, encryption=encryption, tDDControlGroup=tDDControlGroup, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, sNTPLastSync=sNTPLastSync, signalStrengthRatio=signalStrengthRatio, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sMTPEmailAlert=sMTPEmailAlert, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, phyControl=phyControl, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, telecomsInterface=telecomsInterface, bootVersion=bootVersion, ptp500=ptp500, antennaGain=antennaGain, alarms=alarms, telecom=telecom, managementGroup=managementGroup, telecomsChannelStatusTrap=telecomsChannelStatusTrap, licence=licence, userTableNumber=userTableNumber, frequencyVariant=frequencyVariant, systemReset=systemReset, remoteIPAddress=remoteIPAddress, sNMPTrapTableEntry=sNMPTrapTableEntry, hTTPSAccessEnabled=hTTPSAccessEnabled, phyControlGroup=phyControlGroup, accessMethod=accessMethod, management=management, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, aAAControl=aAAControl, receiveDataRate=receiveDataRate, incompatibleMasterAndSlave=incompatibleMasterAndSlave, tDDSynchronizationAlarm=tDDSynchronizationAlarm, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sMTPServerIPAddress=sMTPServerIPAddress, userTableIndex=userTableIndex, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, installArmState=installArmState, telnetAccessEnabled=telnetAccessEnabled, sNMPTrapTableIndex=sNMPTrapTableIndex, gatewayIPAddress=gatewayIPAddress, sNMPPortNumber=sNMPPortNumber, pubStatsGroup=pubStatsGroup, maximumTransmitPower=maximumTransmitPower, telecomsLineCode=telecomsLineCode, byteErrorRatio=byteErrorRatio, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, searchState=searchState, sNTPServerIPAddress=sNTPServerIPAddress, constantPowerSpectralDensity=constantPowerSpectralDensity, licenceGroup=licenceGroup, reset=reset, regionCodeTrap=regionCodeTrap, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, dfsMeans=dfsMeans, dfs=dfs, timeZone=timeZone, userTableEntry=userTableEntry, ethernetGroup=ethernetGroup, versions=versions, syslogState=syslogState, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, bandwidthVariant=bandwidthVariant, productVariant=productVariant, wirelessLinkStatus=wirelessLinkStatus, userPassword=userPassword, ethernetFiberSupport=ethernetFiberSupport, transmitChannel=transmitChannel, cambium=cambium, cableLoss=cableLoss, configuration=configuration, vLANManagementPriority=vLANManagementPriority, hTTPPortNumber=hTTPPortNumber, sNMPCommunityString=sNMPCommunityString, dataPortWirelessDownAlert=dataPortWirelessDownAlert, vLANPriorityQueueMapping=vLANPriorityQueueMapping, encryptionAlgorithm=encryptionAlgorithm, telecomsLoopbackTrap=telecomsLoopbackTrap, sNTPSyncTrap=sNTPSyncTrap, ptpGroups=ptpGroups, channelBandwidth=channelBandwidth, vLANPriorityTableNumber=vLANPriorityTableNumber, hTTPAccessEnabled=hTTPAccessEnabled, dataPortAutoMdix=dataPortAutoMdix, vLANPriorityTableIndex=vLANPriorityTableIndex, supplementary=supplementary, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, installArmStateTrap=installArmStateTrap, daylightSaving=daylightSaving, resetGroup=resetGroup, ptmp=ptmp, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, useVLANForManagementInterfaces=useVLANForManagementInterfaces, versionsGroup=versionsGroup, telecomGroup=telecomGroup, supplementaryGroup=supplementaryGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, sMTPEnabledMessages=sMTPEnabledMessages, dfsTable=dfsTable, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, receiveFreqKHz=receiveFreqKHz, userName=userName, telecomsCableLength=telecomsCableLength, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, dfsTableNumber=dfsTableNumber, phyStatus=phyStatus, sNMPTrapTable=sNMPTrapTable, tDDControl=tDDControl, transmitModulationMode=transmitModulationMode, transmitDataRate=transmitDataRate, sNTPSync=sNTPSync, snmpControl=snmpControl, linkModeOptimizationMismatch=linkModeOptimizationMismatch, hTTPSPortNumber=hTTPSPortNumber, receiveFreqMHz=receiveFreqMHz, tDDSynchronizationStatus=tDDSynchronizationStatus, dataPortStatusTrap=dataPortStatusTrap, PYSNMP_MODULE_ID=cambium, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPEnabledTraps=sNMPEnabledTraps, licensedCapacity=licensedCapacity, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, ptp=ptp, dfsGroup=dfsGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, vLANPriorityTableEntry=vLANPriorityTableEntry, smtp=smtp, vectorError=vectorError, notificationsGroup=notificationsGroup, sMTPServerPortNumber=sMTPServerPortNumber, smtpGroup=smtpGroup, remoteMaximumTransmitPower=remoteMaximumTransmitPower, syslogControlGroup=syslogControlGroup, groupID=groupID, userTable=userTable, dfsPeaks=dfsPeaks, linkLoss=linkLoss, userEnabled=userEnabled, altitude=altitude, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, secondarySoftwareVersion=secondarySoftwareVersion, dataPortStatus=dataPortStatus, tFTPServerIPAddress=tFTPServerIPAddress, siteName=siteName, incompatibleRegionCodes=incompatibleRegionCodes, channelChangeTrap=channelChangeTrap, sntpGroup=sntpGroup, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, ptpTrapPrefix=ptpTrapPrefix, receiveModulationModeDetail=receiveModulationModeDetail, sNMPv3Enable=sNMPv3Enable, encryptionGroup=encryptionGroup, installStatus=installStatus, phyStatusGroup=phyStatusGroup, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, tDDSynchronizationMode=tDDSynchronizationMode, telnetPortNumber=telnetPortNumber, sNTPState=sNTPState, dataPortConfigurationMismatch=dataPortConfigurationMismatch, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, snmpControlGroup=snmpControlGroup, receiveModulationMode=receiveModulationMode, aAAControlGroup=aAAControlGroup, tFTPServerPortNumber=tFTPServerPortNumber, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNTPPollInterval=sNTPPollInterval, wirelessLinkAvailability=wirelessLinkAvailability, userRole=userRole, longitude=longitude, enabledDiagnosticAlarms=enabledDiagnosticAlarms, range=range, sNMPTrapVersion=sNMPTrapVersion, softwareVersion=softwareVersion, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dfsTableIndex=dfsTableIndex, dataPortDisabledWarning=dataPortDisabledWarning, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sntp=sntp, vLANManagementVID=vLANManagementVID, capacityLicenseMismatch=capacityLicenseMismatch, configurationGroup=configurationGroup, regionCode=regionCode, syslogControl=syslogControl, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, latitude=latitude, subnetMask=subnetMask, transmitFreqKHz=transmitFreqKHz, transmitPower=transmitPower, syslogClient=syslogClient, vLANPriorityTable=vLANPriorityTable, syslogStateTrap=syslogStateTrap, installStatusTrap=installStatusTrap, ethernet=ethernet, ptpTraps=ptpTraps, transmitFreqMHz=transmitFreqMHz, eIRP=eIRP, pubStats=pubStats, aggregateDataRate=aggregateDataRate, syslogClientTrap=syslogClientTrap, sNTPServerPortNumber=sNTPServerPortNumber)
