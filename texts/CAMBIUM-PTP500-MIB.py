...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, pubStatsGroup=pubStatsGroup, syslogState=syslogState, searchState=searchState, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, pubStats=pubStats, channelBandwidth=channelBandwidth, syslogClient=syslogClient, reset=reset, enabledDiagnosticAlarms=enabledDiagnosticAlarms, encryptionGroup=encryptionGroup, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, alarms=alarms, tDDControl=tDDControl, maximumTransmitPower=maximumTransmitPower, hardwareVersion=hardwareVersion, userRole=userRole, encryption=encryption, dataPortAutoNegotiation=dataPortAutoNegotiation, dfsTableNumber=dfsTableNumber, sNTPServerIPAddress=sNTPServerIPAddress, telnetPortNumber=telnetPortNumber, phyStatusGroup=phyStatusGroup, tDDControlGroup=tDDControlGroup, tFTPServerPortNumber=tFTPServerPortNumber, telecomsLineCode=telecomsLineCode, licence=licence, sNMPTrapTable=sNMPTrapTable, systemReset=systemReset, userEnabled=userEnabled, ethernetGroup=ethernetGroup, userPassword=userPassword, sNMPTrapIPAddress=sNMPTrapIPAddress, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, telecom=telecom, installStatus=installStatus, dfsTable=dfsTable, transmitPower=transmitPower, bootVersion=bootVersion, phyControlGroup=phyControlGroup, telecomsInterface=telecomsInterface, secondarySoftwareVersion=secondarySoftwareVersion, managementGroup=managementGroup, ethernetFiberSupport=ethernetFiberSupport, productVariant=productVariant, daylightSaving=daylightSaving, sNTPSyncTrap=sNTPSyncTrap, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, sntp=sntp, antennaGain=antennaGain, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPEnabledTraps=sNMPEnabledTraps, smtpGroup=smtpGroup, sMTPServerPortNumber=sMTPServerPortNumber, tDDSynchronizationAlarm=tDDSynchronizationAlarm, useVLANForManagementInterfaces=useVLANForManagementInterfaces, regionCode=regionCode, dataPortWirelessDownAlert=dataPortWirelessDownAlert, telecomsLoopbackTrap=telecomsLoopbackTrap, groupID=groupID, ptpTraps=ptpTraps, snmpControlGroup=snmpControlGroup, telecomsChannelStatus=telecomsChannelStatus, receiveDataRate=receiveDataRate, receivePower=receivePower, dfsTableIndex=dfsTableIndex, sMTPEnabledMessages=sMTPEnabledMessages, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, vLANPriorityTable=vLANPriorityTable, syslogStateTrap=syslogStateTrap, tFTPServerIPAddress=tFTPServerIPAddress, channelChangeTrap=channelChangeTrap, ptpCompliance=ptpCompliance, accessMethod=accessMethod, vLANPriorityTableNumber=vLANPriorityTableNumber, versions=versions, sNMPPortNumber=sNMPPortNumber, licensedCapacity=licensedCapacity, dfsGroup=dfsGroup, transmitFreqMHz=transmitFreqMHz, bandwidthVariant=bandwidthVariant, encryptionAlgorithm=encryptionAlgorithm, iPAddress=iPAddress, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, userTable=userTable, receiveFreqKHz=receiveFreqKHz, userTableNumber=userTableNumber, cableLoss=cableLoss, incompatibleRegionCodes=incompatibleRegionCodes, linkSymmetry=linkSymmetry, tDDSynchronizationMode=tDDSynchronizationMode, ptp500=ptp500, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, systemClock=systemClock, frequencyVariant=frequencyVariant, licenceGroup=licenceGroup, sMTPEmailAlert=sMTPEmailAlert, sNMPv3Enable=sNMPv3Enable, latitude=latitude, sntpGroup=sntpGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, vectorError=vectorError, incompatibleMasterAndSlave=incompatibleMasterAndSlave, linkLoss=linkLoss, ethernet=ethernet, notificationsGroup=notificationsGroup, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sMTPServerIPAddress=sMTPServerIPAddress, transmitDataRate=transmitDataRate, wirelessLinkStatus=wirelessLinkStatus, userName=userName, telecomGroup=telecomGroup, altitude=altitude, ptp=ptp, sNMPTrapTableIndex=sNMPTrapTableIndex, sNTPLastSync=sNTPLastSync, capacityLicenseMismatch=capacityLicenseMismatch, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, alarmsGroup=alarmsGroup, vLANPriorityTableIndex=vLANPriorityTableIndex, dfsMeans=dfsMeans, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, telecomsChannelStatusTrap=telecomsChannelStatusTrap, syslogClientTrap=syslogClientTrap, syslogControlGroup=syslogControlGroup, linkModeOptimisation=linkModeOptimisation, aAAControlGroup=aAAControlGroup, syslogControl=syslogControl, management=management, versionsGroup=versionsGroup, gatewayIPAddress=gatewayIPAddress, signalStrengthRatio=signalStrengthRatio, sNTPState=sNTPState, telnetAccessEnabled=telnetAccessEnabled, configurationGroup=configurationGroup, userTableEntry=userTableEntry, subnetMask=subnetMask, timeZone=timeZone, constantPowerSpectralDensity=constantPowerSpectralDensity, phyControl=phyControl, sNTPServerPortNumber=sNTPServerPortNumber, dfsTableEntry=dfsTableEntry, sNMPTrapPortNumber=sNMPTrapPortNumber, userTableIndex=userTableIndex, vLANManagementVID=vLANManagementVID, resetGroup=resetGroup, eIRP=eIRP, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, dfsPeaks=dfsPeaks, wirelessLinkAvailability=wirelessLinkAvailability, dataPortConfigurationMismatch=dataPortConfigurationMismatch, dataPortStatusTrap=dataPortStatusTrap, snmpControl=snmpControl, range=range, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, vLANPriorityQueueMapping=vLANPriorityQueueMapping, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, receiveFreqMHz=receiveFreqMHz, aggregateDataRate=aggregateDataRate, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, tDDSynchronizationStatus=tDDSynchronizationStatus, configuration=configuration, unitOutOfCalibration=unitOutOfCalibration, sNMPCommunityString=sNMPCommunityString, installArmStateTrap=installArmStateTrap, sNMPTrapTableEntry=sNMPTrapTableEntry, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, receiveChannel=receiveChannel, cambium=cambium, ptpTrapPrefix=ptpTrapPrefix, softwareVersion=softwareVersion, smtp=smtp, transmitFreqKHz=transmitFreqKHz, sNMPTrapTableNumber=sNMPTrapTableNumber, masterSlaveMode=masterSlaveMode, transmitModulationMode=transmitModulationMode, supplementaryGroup=supplementaryGroup, regionCodeTrap=regionCodeTrap, telecomsLoopback=telecomsLoopback, aAAControl=aAAControl, dataPortStatus=dataPortStatus, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, sNMPTrapVersion=sNMPTrapVersion, siteName=siteName, targetMACAddress=targetMACAddress, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, longitude=longitude, transmitChannel=transmitChannel, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, PYSNMP_MODULE_ID=cambium, noWirelessChannelAvailable=noWirelessChannelAvailable, receiveModulationModeDetail=receiveModulationModeDetail, vLANPriorityTableEntry=vLANPriorityTableEntry, sMTPSourceEmailAddress=sMTPSourceEmailAddress, syslogLocalWrappedTrap=syslogLocalWrappedTrap, telecomsCableLength=telecomsCableLength, productName=productName, hTTPSAccessEnabled=hTTPSAccessEnabled, sNTPPollInterval=sNTPPollInterval, hTTPPortNumber=hTTPPortNumber, byteErrorRatio=byteErrorRatio, dfs=dfs, remoteIPAddress=remoteIPAddress, receiveModulationMode=receiveModulationMode, ptmp=ptmp, dataPortDisabledWarning=dataPortDisabledWarning, phyStatus=phyStatus, ptpGroups=ptpGroups, supplementary=supplementary, sNTPSync=sNTPSync, installArmState=installArmState, hTTPAccessEnabled=hTTPAccessEnabled, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, installStatusTrap=installStatusTrap, vLANManagementPriority=vLANManagementPriority, linkName=linkName, hTTPSPortNumber=hTTPSPortNumber, dataPortAutoMdix=dataPortAutoMdix)
