...
# Produced by pysmi-1.1.10 at Fri Nov 10 10:09:26 2023
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", ptpCompliance=ptpCompliance, resetGroup=resetGroup, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, pubStats=pubStats, encryptionAlgorithm=encryptionAlgorithm, longitude=longitude, ptmp=ptmp, transmitDataRate=transmitDataRate, userTableEntry=userTableEntry, vLANPriorityTable=vLANPriorityTable, notificationsGroup=notificationsGroup, telecomsChannelStatus=telecomsChannelStatus, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, alarms=alarms, dataPortAutoNegotiation=dataPortAutoNegotiation, sMTPServerIPAddress=sMTPServerIPAddress, systemReset=systemReset, transmitChannel=transmitChannel, ethernetFiberSupport=ethernetFiberSupport, sNMPTrapPortNumber=sNMPTrapPortNumber, licenceGroup=licenceGroup, hTTPSAccessEnabled=hTTPSAccessEnabled, management=management, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sNTPServerPortNumber=sNTPServerPortNumber, vectorError=vectorError, maximumTransmitPower=maximumTransmitPower, vLANPriorityQueueMapping=vLANPriorityQueueMapping, linkModeOptimisation=linkModeOptimisation, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sntpGroup=sntpGroup, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, sNMPTrapTable=sNMPTrapTable, ethernet=ethernet, hTTPSPortNumber=hTTPSPortNumber, tDDSynchronizationStatus=tDDSynchronizationStatus, softwareVersion=softwareVersion, sNMPTrapTableNumber=sNMPTrapTableNumber, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, incompatibleMasterAndSlave=incompatibleMasterAndSlave, phyStatus=phyStatus, vLANPriorityTableNumber=vLANPriorityTableNumber, versionsGroup=versionsGroup, hTTPAccessEnabled=hTTPAccessEnabled, unitOutOfCalibration=unitOutOfCalibration, userTable=userTable, byteErrorRatio=byteErrorRatio, productVariant=productVariant, ptpGroups=ptpGroups, smtp=smtp, vLANPriorityTableIndex=vLANPriorityTableIndex, bandwidthVariant=bandwidthVariant, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, sNMPPortNumber=sNMPPortNumber, receiveFreqKHz=receiveFreqKHz, userTableIndex=userTableIndex, syslogClient=syslogClient, receiveModulationMode=receiveModulationMode, productName=productName, telnetPortNumber=telnetPortNumber, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, dfsTableIndex=dfsTableIndex, telecomsChannelStatusTrap=telecomsChannelStatusTrap, constantPowerSpectralDensity=constantPowerSpectralDensity, dataPortDisabledWarning=dataPortDisabledWarning, telecom=telecom, configurationGroup=configurationGroup, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, dfsPeaks=dfsPeaks, receiveDataRate=receiveDataRate, installArmStateTrap=installArmStateTrap, sNTPServerIPAddress=sNTPServerIPAddress, supplementary=supplementary, dfsTable=dfsTable, hTTPPortNumber=hTTPPortNumber, receiveFreqMHz=receiveFreqMHz, sNMPTrapTableIndex=sNMPTrapTableIndex, sMTPSourceEmailAddress=sMTPSourceEmailAddress, transmitFreqKHz=transmitFreqKHz, userName=userName, systemClock=systemClock, regionCodeTrap=regionCodeTrap, userRole=userRole, PYSNMP_MODULE_ID=cambium, syslogStateTrap=syslogStateTrap, sntp=sntp, dfsGroup=dfsGroup, managementGroup=managementGroup, groupID=groupID, sNTPSync=sNTPSync, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, dfs=dfs, vLANManagementPriority=vLANManagementPriority, ethernetGroup=ethernetGroup, encryptionGroup=encryptionGroup, snmpControl=snmpControl, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sMTPServerPortNumber=sMTPServerPortNumber, versions=versions, sNTPPollInterval=sNTPPollInterval, telecomsLoopbackTrap=telecomsLoopbackTrap, supplementaryGroup=supplementaryGroup, sNTPSyncTrap=sNTPSyncTrap, useVLANForManagementInterfaces=useVLANForManagementInterfaces, aggregateDataRate=aggregateDataRate, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, sNMPCommunityString=sNMPCommunityString, linkSymmetry=linkSymmetry, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, frequencyVariant=frequencyVariant, telnetAccessEnabled=telnetAccessEnabled, configuration=configuration, snmpControlGroup=snmpControlGroup, sNMPEnabledTraps=sNMPEnabledTraps, userTableNumber=userTableNumber, tDDControlGroup=tDDControlGroup, range=range, sNMPTrapIPAddress=sNMPTrapIPAddress, capacityLicenseMismatch=capacityLicenseMismatch, phyControl=phyControl, sNMPTrapVersion=sNMPTrapVersion, ptpTraps=ptpTraps, ptp500=ptp500, userPassword=userPassword, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, dataPortStatus=dataPortStatus, dfsTableNumber=dfsTableNumber, licence=licence, tFTPServerPortNumber=tFTPServerPortNumber, linkLoss=linkLoss, linkModeOptimizationMismatch=linkModeOptimizationMismatch, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, vLANPriorityTableEntry=vLANPriorityTableEntry, alarmsGroup=alarmsGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, transmitFreqMHz=transmitFreqMHz, ptp=ptp, channelChangeTrap=channelChangeTrap, aAAControl=aAAControl, smtpGroup=smtpGroup, bootVersion=bootVersion, installArmState=installArmState, signalStrengthRatio=signalStrengthRatio, reset=reset, sNTPState=sNTPState, pubStatsGroup=pubStatsGroup, dfsMeans=dfsMeans, telecomsCableLength=telecomsCableLength, telecomsLineCode=telecomsLineCode, receivePower=receivePower, noWirelessChannelAvailable=noWirelessChannelAvailable, cableLoss=cableLoss, sMTPEmailAlert=sMTPEmailAlert, dataPortAutoMdix=dataPortAutoMdix, receiveChannel=receiveChannel, sNMPTrapTableEntry=sNMPTrapTableEntry, tDDControl=tDDControl, incompatibleRegionCodes=incompatibleRegionCodes, enabledDiagnosticAlarms=enabledDiagnosticAlarms, wirelessLinkAvailability=wirelessLinkAvailability, wirelessLinkStatus=wirelessLinkStatus, ptpTrapPrefix=ptpTrapPrefix, vLANManagementVID=vLANManagementVID, remoteIPAddress=remoteIPAddress, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, syslogClientTrap=syslogClientTrap, targetMACAddress=targetMACAddress, masterSlaveMode=masterSlaveMode, syslogControlGroup=syslogControlGroup, dataPortStatusTrap=dataPortStatusTrap, tFTPServerIPAddress=tFTPServerIPAddress, siteName=siteName, telecomGroup=telecomGroup, daylightSaving=daylightSaving, iPAddress=iPAddress, hardwareVersion=hardwareVersion, syslogState=syslogState, regionCode=regionCode, antennaGain=antennaGain, telecomsLoopback=telecomsLoopback, linkName=linkName, eIRP=eIRP, telecomsInterface=telecomsInterface, dfsTableEntry=dfsTableEntry, receiveModulationModeDetail=receiveModulationModeDetail, aAAControlGroup=aAAControlGroup, licensedCapacity=licensedCapacity, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sMTPEnabledMessages=sMTPEnabledMessages, transmitModulationMode=transmitModulationMode, altitude=altitude, installStatusTrap=installStatusTrap, subnetMask=subnetMask, syslogControl=syslogControl, gatewayIPAddress=gatewayIPAddress, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, searchState=searchState, userEnabled=userEnabled, sNTPLastSync=sNTPLastSync, timeZone=timeZone, transmitPower=transmitPower, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, latitude=latitude, secondarySoftwareVersion=secondarySoftwareVersion, encryption=encryption, syslogLocalWrappedTrap=syslogLocalWrappedTrap, accessMethod=accessMethod, installStatus=installStatus, cambium=cambium, tDDSynchronizationMode=tDDSynchronizationMode, phyControlGroup=phyControlGroup, phyStatusGroup=phyStatusGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, channelBandwidth=channelBandwidth, sNMPv3Enable=sNMPv3Enable)
