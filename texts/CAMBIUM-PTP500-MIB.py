...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", encryption=encryption, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, installArmState=installArmState, remoteIPAddress=remoteIPAddress, channelChangeTrap=channelChangeTrap, longitude=longitude, configurationGroup=configurationGroup, sMTPEnabledMessages=sMTPEnabledMessages, timeZone=timeZone, ethernetGroup=ethernetGroup, userName=userName, userEnabled=userEnabled, telecomGroup=telecomGroup, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, telnetPortNumber=telnetPortNumber, userTableNumber=userTableNumber, masterSlaveMode=masterSlaveMode, siteName=siteName, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPTrapVersion=sNMPTrapVersion, configuration=configuration, cableLoss=cableLoss, daylightSaving=daylightSaving, dataPortWirelessDownAlert=dataPortWirelessDownAlert, linkModeOptimisation=linkModeOptimisation, dataPortConfigurationMismatch=dataPortConfigurationMismatch, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, latitude=latitude, pubStatsGroup=pubStatsGroup, telecomsLoopback=telecomsLoopback, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, sNMPEnabledTraps=sNMPEnabledTraps, dataPortAutoMdix=dataPortAutoMdix, transmitFreqMHz=transmitFreqMHz, syslogState=syslogState, userTableEntry=userTableEntry, incompatibleMasterAndSlave=incompatibleMasterAndSlave, regionCodeTrap=regionCodeTrap, tFTPServerPortNumber=tFTPServerPortNumber, dfsTableNumber=dfsTableNumber, vLANManagementPriority=vLANManagementPriority, linkName=linkName, telnetAccessEnabled=telnetAccessEnabled, managementGroup=managementGroup, ptmp=ptmp, productVariant=productVariant, systemReset=systemReset, dfsPeaks=dfsPeaks, dfsTableEntry=dfsTableEntry, ptp=ptp, vectorError=vectorError, installStatusTrap=installStatusTrap, remoteMaximumTransmitPower=remoteMaximumTransmitPower, tDDSynchronizationMode=tDDSynchronizationMode, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, encryptionGroup=encryptionGroup, tDDSynchronizationStatus=tDDSynchronizationStatus, tDDControlGroup=tDDControlGroup, userTable=userTable, licenceGroup=licenceGroup, ptpTrapPrefix=ptpTrapPrefix, transmitDataRate=transmitDataRate, transmitPower=transmitPower, syslogControl=syslogControl, receiveDataRate=receiveDataRate, installStatus=installStatus, receivePower=receivePower, sNMPTrapTableNumber=sNMPTrapTableNumber, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, dataPortStatusTrap=dataPortStatusTrap, searchState=searchState, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, sNMPTrapTable=sNMPTrapTable, phyStatusGroup=phyStatusGroup, antennaGain=antennaGain, telecomsInterface=telecomsInterface, transmitFreqKHz=transmitFreqKHz, targetMACAddress=targetMACAddress, wirelessLinkStatus=wirelessLinkStatus, pubStats=pubStats, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sntp=sntp, vLANPriorityTableIndex=vLANPriorityTableIndex, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, hTTPSPortNumber=hTTPSPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, ethernet=ethernet, userRole=userRole, syslogLocalWrappedTrap=syslogLocalWrappedTrap, syslogControlGroup=syslogControlGroup, dataPortStatus=dataPortStatus, linkLoss=linkLoss, vLANPriorityTable=vLANPriorityTable, wirelessLinkAvailability=wirelessLinkAvailability, bootVersion=bootVersion, subnetMask=subnetMask, regionCode=regionCode, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, alarms=alarms, syslogClientTrap=syslogClientTrap, sMTPServerIPAddress=sMTPServerIPAddress, linkSymmetry=linkSymmetry, smtp=smtp, enabledDiagnosticAlarms=enabledDiagnosticAlarms, receiveModulationModeDetail=receiveModulationModeDetail, hardwareVersion=hardwareVersion, altitude=altitude, ptpGroups=ptpGroups, dfs=dfs, useVLANForManagementInterfaces=useVLANForManagementInterfaces, resetGroup=resetGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, phyControlGroup=phyControlGroup, sMTPEmailAlert=sMTPEmailAlert, transmitChannel=transmitChannel, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, signalStrengthRatio=signalStrengthRatio, sNTPLastSync=sNTPLastSync, telecomsCableLength=telecomsCableLength, eIRP=eIRP, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, notificationsGroup=notificationsGroup, syslogClient=syslogClient, vLANPriorityTableEntry=vLANPriorityTableEntry, sNMPv3Enable=sNMPv3Enable, channelBandwidth=channelBandwidth, dfsTable=dfsTable, telecom=telecom, telecomsLineCode=telecomsLineCode, aAAControlGroup=aAAControlGroup, maximumTransmitPower=maximumTransmitPower, sNTPState=sNTPState, syslogStateTrap=syslogStateTrap, versions=versions, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, receiveFreqKHz=receiveFreqKHz, hTTPAccessEnabled=hTTPAccessEnabled, aggregateDataRate=aggregateDataRate, dfsGroup=dfsGroup, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, frequencyVariant=frequencyVariant, byteErrorRatio=byteErrorRatio, hTTPPortNumber=hTTPPortNumber, telecomsLoopbackTrap=telecomsLoopbackTrap, phyControl=phyControl, incompatibleRegionCodes=incompatibleRegionCodes, sntpGroup=sntpGroup, vLANPriorityQueueMapping=vLANPriorityQueueMapping, versionsGroup=versionsGroup, reset=reset, supplementaryGroup=supplementaryGroup, constantPowerSpectralDensity=constantPowerSpectralDensity, sMTPServerPortNumber=sMTPServerPortNumber, softwareVersion=softwareVersion, vLANManagementVID=vLANManagementVID, sNMPTrapPortNumber=sNMPTrapPortNumber, supplementary=supplementary, sNTPSyncTrap=sNTPSyncTrap, dataPortAutoNegotiation=dataPortAutoNegotiation, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, installArmStateTrap=installArmStateTrap, accessMethod=accessMethod, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, licensedCapacity=licensedCapacity, sNTPPollInterval=sNTPPollInterval, userTableIndex=userTableIndex, bandwidthVariant=bandwidthVariant, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNTPServerIPAddress=sNTPServerIPAddress, sNMPCommunityString=sNMPCommunityString, management=management, receiveChannel=receiveChannel, ethernetFiberSupport=ethernetFiberSupport, licence=licence, ptpCompliance=ptpCompliance, tDDControl=tDDControl, smtpGroup=smtpGroup, groupID=groupID, telecomsChannelStatus=telecomsChannelStatus, range=range, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, sMTPSourceEmailAddress=sMTPSourceEmailAddress, encryptionAlgorithm=encryptionAlgorithm, productName=productName, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, secondarySoftwareVersion=secondarySoftwareVersion, ptp500=ptp500, tFTPServerIPAddress=tFTPServerIPAddress, noWirelessChannelAvailable=noWirelessChannelAvailable, snmpControl=snmpControl, dataPortDisabledWarning=dataPortDisabledWarning, sNMPPortNumber=sNMPPortNumber, sNMPTrapIPAddress=sNMPTrapIPAddress, snmpControlGroup=snmpControlGroup, transmitModulationMode=transmitModulationMode, receiveFreqMHz=receiveFreqMHz, userPassword=userPassword, capacityLicenseMismatch=capacityLicenseMismatch, alarmsGroup=alarmsGroup, sNTPSync=sNTPSync, systemClock=systemClock, sNTPServerPortNumber=sNTPServerPortNumber, aAAControl=aAAControl, telecomsChannelStatusTrap=telecomsChannelStatusTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dfsMeans=dfsMeans, cambium=cambium, hTTPSAccessEnabled=hTTPSAccessEnabled, PYSNMP_MODULE_ID=cambium, ptpTraps=ptpTraps, vLANPriorityTableNumber=vLANPriorityTableNumber, dfsTableIndex=dfsTableIndex, iPAddress=iPAddress, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, receiveModulationMode=receiveModulationMode, phyStatus=phyStatus, sNMPTrapTableIndex=sNMPTrapTableIndex, gatewayIPAddress=gatewayIPAddress, unitOutOfCalibration=unitOutOfCalibration)
