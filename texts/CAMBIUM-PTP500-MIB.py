...
# Produced by pysmi-1.1.12 at Tue Jun  4 13:49:12 2024
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", groupID=groupID, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, transmitModulationMode=transmitModulationMode, ethernetGroup=ethernetGroup, bandwidthVariant=bandwidthVariant, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, snmpControl=snmpControl, dataPortConfigurationMismatch=dataPortConfigurationMismatch, searchState=searchState, ptp500=ptp500, vLANManagementVID=vLANManagementVID, ptpCompliance=ptpCompliance, ethernet=ethernet, versions=versions, tFTPServerIPAddress=tFTPServerIPAddress, syslogClientTrap=syslogClientTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, userTableNumber=userTableNumber, encryption=encryption, supplementaryGroup=supplementaryGroup, remoteMaximumTransmitPower=remoteMaximumTransmitPower, receiveDataRate=receiveDataRate, phyControl=phyControl, hTTPAccessEnabled=hTTPAccessEnabled, sNTPServerPortNumber=sNTPServerPortNumber, phyStatus=phyStatus, sntpGroup=sntpGroup, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, byteErrorRatio=byteErrorRatio, sMTPEmailAlert=sMTPEmailAlert, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNMPCommunityString=sNMPCommunityString, eIRP=eIRP, hardwareVersion=hardwareVersion, dfsMeans=dfsMeans, ptpGroups=ptpGroups, sNTPPollInterval=sNTPPollInterval, pubStats=pubStats, altitude=altitude, telecom=telecom, frequencyVariant=frequencyVariant, telecomsLoopback=telecomsLoopback, sNMPTrapTable=sNMPTrapTable, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, userTableIndex=userTableIndex, dataPortAutoMdix=dataPortAutoMdix, telecomsInterface=telecomsInterface, ptp=ptp, signalStrengthRatio=signalStrengthRatio, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sNMPTrapTableNumber=sNMPTrapTableNumber, tFTPServerPortNumber=tFTPServerPortNumber, iPAddress=iPAddress, sNMPTrapVersion=sNMPTrapVersion, receiveChannel=receiveChannel, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, userEnabled=userEnabled, userTable=userTable, remoteIPAddress=remoteIPAddress, telecomsCableLength=telecomsCableLength, sNMPTrapIPAddress=sNMPTrapIPAddress, reset=reset, encryptionGroup=encryptionGroup, dfsTableEntry=dfsTableEntry, siteName=siteName, alarms=alarms, phyStatusGroup=phyStatusGroup, installArmState=installArmState, dfs=dfs, syslogControlGroup=syslogControlGroup, pubStatsGroup=pubStatsGroup, vLANPriorityTableNumber=vLANPriorityTableNumber, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, secondarySoftwareVersion=secondarySoftwareVersion, ptpTrapPrefix=ptpTrapPrefix, hTTPSAccessEnabled=hTTPSAccessEnabled, channelChangeTrap=channelChangeTrap, linkName=linkName, productVariant=productVariant, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, vLANPriorityTableEntry=vLANPriorityTableEntry, receiveFreqKHz=receiveFreqKHz, aggregateDataRate=aggregateDataRate, vectorError=vectorError, softwareVersion=softwareVersion, telecomsChannelStatus=telecomsChannelStatus, cableLoss=cableLoss, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPTrapTableEntry=sNMPTrapTableEntry, licensedCapacity=licensedCapacity, syslogState=syslogState, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dfsGroup=dfsGroup, installArmStateTrap=installArmStateTrap, linkModeOptimisation=linkModeOptimisation, transmitChannel=transmitChannel, regionCodeTrap=regionCodeTrap, licence=licence, linkSymmetry=linkSymmetry, telnetPortNumber=telnetPortNumber, capacityLicenseMismatch=capacityLicenseMismatch, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, configuration=configuration, sNMPv3Enable=sNMPv3Enable, phyControlGroup=phyControlGroup, cambium=cambium, tDDControlGroup=tDDControlGroup, notificationsGroup=notificationsGroup, sNMPPortNumber=sNMPPortNumber, vLANPriorityTable=vLANPriorityTable, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, range=range, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, alarmsGroup=alarmsGroup, telecomGroup=telecomGroup, installStatusTrap=installStatusTrap, accessMethod=accessMethod, managementGroup=managementGroup, telnetAccessEnabled=telnetAccessEnabled, telecomsLineCode=telecomsLineCode, smtp=smtp, sNTPState=sNTPState, targetMACAddress=targetMACAddress, syslogClient=syslogClient, daylightSaving=daylightSaving, versionsGroup=versionsGroup, dataPortStatus=dataPortStatus, constantPowerSpectralDensity=constantPowerSpectralDensity, antennaGain=antennaGain, sMTPSourceEmailAddress=sMTPSourceEmailAddress, systemClock=systemClock, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, wirelessLinkAvailability=wirelessLinkAvailability, useVLANForManagementInterfaces=useVLANForManagementInterfaces, ptpTraps=ptpTraps, aAAControlGroup=aAAControlGroup, dfsPeaks=dfsPeaks, dataPortStatusTrap=dataPortStatusTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, syslogStateTrap=syslogStateTrap, enabledDiagnosticAlarms=enabledDiagnosticAlarms, userPassword=userPassword, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, dfsTable=dfsTable, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, tDDControl=tDDControl, transmitDataRate=transmitDataRate, receiveModulationModeDetail=receiveModulationModeDetail, vLANPriorityTableIndex=vLANPriorityTableIndex, masterSlaveMode=masterSlaveMode, latitude=latitude, tDDSynchronizationMode=tDDSynchronizationMode, tDDSynchronizationStatus=tDDSynchronizationStatus, receiveFreqMHz=receiveFreqMHz, transmitFreqKHz=transmitFreqKHz, encryptionAlgorithm=encryptionAlgorithm, sNMPTrapPortNumber=sNMPTrapPortNumber, ethernetFiberSupport=ethernetFiberSupport, subnetMask=subnetMask, sNTPServerIPAddress=sNTPServerIPAddress, userRole=userRole, smtpGroup=smtpGroup, dfsTableIndex=dfsTableIndex, installStatus=installStatus, transmitFreqMHz=transmitFreqMHz, supplementary=supplementary, management=management, configurationGroup=configurationGroup, vLANPriorityQueueMapping=vLANPriorityQueueMapping, unitOutOfCalibration=unitOutOfCalibration, incompatibleRegionCodes=incompatibleRegionCodes, dataPortWirelessDownAlert=dataPortWirelessDownAlert, snmpControlGroup=snmpControlGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, receivePower=receivePower, ptmp=ptmp, dataPortDisabledWarning=dataPortDisabledWarning, licenceGroup=licenceGroup, bootVersion=bootVersion, hTTPSPortNumber=hTTPSPortNumber, noWirelessChannelAvailable=noWirelessChannelAvailable, incompatibleMasterAndSlave=incompatibleMasterAndSlave, dfsTableNumber=dfsTableNumber, gatewayIPAddress=gatewayIPAddress, wirelessLinkStatus=wirelessLinkStatus, PYSNMP_MODULE_ID=cambium, sntp=sntp, vLANManagementPriority=vLANManagementPriority, linkLoss=linkLoss, sNMPTrapTableIndex=sNMPTrapTableIndex, sNTPLastSync=sNTPLastSync, telecomsChannelStatusTrap=telecomsChannelStatusTrap, userTableEntry=userTableEntry, resetGroup=resetGroup, aAAControl=aAAControl, sNMPEnabledTraps=sNMPEnabledTraps, productName=productName, maximumTransmitPower=maximumTransmitPower, receiveModulationMode=receiveModulationMode, syslogControl=syslogControl, sMTPEnabledMessages=sMTPEnabledMessages, hTTPPortNumber=hTTPPortNumber, sNTPSyncTrap=sNTPSyncTrap, systemReset=systemReset, channelBandwidth=channelBandwidth, transmitPower=transmitPower, longitude=longitude, sMTPServerIPAddress=sMTPServerIPAddress, timeZone=timeZone, telecomsLoopbackTrap=telecomsLoopbackTrap, userName=userName, regionCode=regionCode, sMTPServerPortNumber=sMTPServerPortNumber, sNTPSync=sNTPSync, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap)
