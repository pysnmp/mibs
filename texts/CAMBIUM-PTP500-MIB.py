...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", userTableEntry=userTableEntry, management=management, telnetAccessEnabled=telnetAccessEnabled, tDDSynchronizationMode=tDDSynchronizationMode, sMTPServerIPAddress=sMTPServerIPAddress, tDDControl=tDDControl, dataPortAutoNegotiation=dataPortAutoNegotiation, productVariant=productVariant, linkName=linkName, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, masterSlaveMode=masterSlaveMode, tDDSynchronizationStatus=tDDSynchronizationStatus, tFTPServerPortNumber=tFTPServerPortNumber, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, gatewayIPAddress=gatewayIPAddress, userRole=userRole, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, userPassword=userPassword, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, byteErrorRatio=byteErrorRatio, vLANPriorityTableNumber=vLANPriorityTableNumber, receiveFreqKHz=receiveFreqKHz, ptpTrapPrefix=ptpTrapPrefix, capacityLicenseMismatch=capacityLicenseMismatch, snmpControlGroup=snmpControlGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, transmitFreqMHz=transmitFreqMHz, vLANPriorityTableEntry=vLANPriorityTableEntry, transmitFreqKHz=transmitFreqKHz, bootVersion=bootVersion, dfsGroup=dfsGroup, transmitPower=transmitPower, sNTPSyncTrap=sNTPSyncTrap, licenceGroup=licenceGroup, hTTPPortNumber=hTTPPortNumber, syslogLocalWrappedTrap=syslogLocalWrappedTrap, licensedCapacity=licensedCapacity, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sMTPEnabledMessages=sMTPEnabledMessages, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, installStatus=installStatus, tDDControlGroup=tDDControlGroup, syslogClientTrap=syslogClientTrap, searchState=searchState, telecomsInterface=telecomsInterface, installStatusTrap=installStatusTrap, syslogStateTrap=syslogStateTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, ethernetGroup=ethernetGroup, resetGroup=resetGroup, ptp=ptp, ptmp=ptmp, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, sNTPSync=sNTPSync, userName=userName, productName=productName, receiveModulationMode=receiveModulationMode, linkModeOptimizationMismatch=linkModeOptimizationMismatch, latitude=latitude, vLANManagementPriority=vLANManagementPriority, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, constantPowerSpectralDensity=constantPowerSpectralDensity, longitude=longitude, ethernetFiberSupport=ethernetFiberSupport, telecomsChannelStatus=telecomsChannelStatus, sNMPEnabledTraps=sNMPEnabledTraps, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sMTPEmailAlert=sMTPEmailAlert, receiveChannel=receiveChannel, aAAControl=aAAControl, alarms=alarms, groupID=groupID, regionCodeTrap=regionCodeTrap, sNMPv3Enable=sNMPv3Enable, supplementary=supplementary, versions=versions, systemReset=systemReset, enabledDiagnosticAlarms=enabledDiagnosticAlarms, dataPortWirelessDownAlert=dataPortWirelessDownAlert, sNMPTrapIPAddress=sNMPTrapIPAddress, sNMPCommunityString=sNMPCommunityString, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, pubStats=pubStats, sNTPLastSync=sNTPLastSync, smtpGroup=smtpGroup, signalStrengthRatio=signalStrengthRatio, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, ptpTraps=ptpTraps, vLANPriorityTable=vLANPriorityTable, hTTPSPortNumber=hTTPSPortNumber, dfsTableNumber=dfsTableNumber, cambium=cambium, dataPortConfigurationMismatch=dataPortConfigurationMismatch, remoteMaximumTransmitPower=remoteMaximumTransmitPower, receiveModulationModeDetail=receiveModulationModeDetail, aggregateDataRate=aggregateDataRate, sNMPTrapPortNumber=sNMPTrapPortNumber, incompatibleRegionCodes=incompatibleRegionCodes, accessMethod=accessMethod, receivePower=receivePower, encryptionAlgorithm=encryptionAlgorithm, syslogState=syslogState, dfsMeans=dfsMeans, versionsGroup=versionsGroup, sNMPPortNumber=sNMPPortNumber, dfsTable=dfsTable, sNMPTrapTableNumber=sNMPTrapTableNumber, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, installArmState=installArmState, receiveFreqMHz=receiveFreqMHz, snmpControl=snmpControl, telecomsLoopback=telecomsLoopback, configurationGroup=configurationGroup, vLANPriorityQueueMapping=vLANPriorityQueueMapping, dataPortStatusTrap=dataPortStatusTrap, dfsTableEntry=dfsTableEntry, sntpGroup=sntpGroup, sNMPTrapTable=sNMPTrapTable, iPAddress=iPAddress, siteName=siteName, dataPortAutoMdix=dataPortAutoMdix, vLANPriorityTableIndex=vLANPriorityTableIndex, phyStatusGroup=phyStatusGroup, phyControl=phyControl, licence=licence, transmitModulationMode=transmitModulationMode, bandwidthVariant=bandwidthVariant, tFTPServerIPAddress=tFTPServerIPAddress, ptp500=ptp500, sntp=sntp, telecomsLoopbackTrap=telecomsLoopbackTrap, range=range, ptpGroups=ptpGroups, installArmStateTrap=installArmStateTrap, sNTPState=sNTPState, channelBandwidth=channelBandwidth, channelChangeTrap=channelChangeTrap, maximumTransmitPower=maximumTransmitPower, notificationsGroup=notificationsGroup, userTable=userTable, linkModeOptimisation=linkModeOptimisation, sNTPServerPortNumber=sNTPServerPortNumber, transmitDataRate=transmitDataRate, vectorError=vectorError, altitude=altitude, pubStatsGroup=pubStatsGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, syslogControl=syslogControl, regionCode=regionCode, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, encryption=encryption, reset=reset, antennaGain=antennaGain, sNTPPollInterval=sNTPPollInterval, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sMTPServerPortNumber=sMTPServerPortNumber, supplementaryGroup=supplementaryGroup, managementGroup=managementGroup, useVLANForManagementInterfaces=useVLANForManagementInterfaces, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, tDDSynchronizationAlarm=tDDSynchronizationAlarm, wirelessLinkStatus=wirelessLinkStatus, telnetPortNumber=telnetPortNumber, vLANManagementVID=vLANManagementVID, smtp=smtp, hTTPAccessEnabled=hTTPAccessEnabled, noWirelessChannelAvailable=noWirelessChannelAvailable, phyControlGroup=phyControlGroup, secondarySoftwareVersion=secondarySoftwareVersion, receiveDataRate=receiveDataRate, targetMACAddress=targetMACAddress, userEnabled=userEnabled, dataPortStatus=dataPortStatus, dfsTableIndex=dfsTableIndex, aAAControlGroup=aAAControlGroup, systemClock=systemClock, userTableNumber=userTableNumber, linkSymmetry=linkSymmetry, linkLoss=linkLoss, telecomsLineCode=telecomsLineCode, hTTPSAccessEnabled=hTTPSAccessEnabled, sNMPTrapTableIndex=sNMPTrapTableIndex, ptpCompliance=ptpCompliance, wirelessLinkAvailability=wirelessLinkAvailability, configuration=configuration, daylightSaving=daylightSaving, telecomsChannelStatusTrap=telecomsChannelStatusTrap, softwareVersion=softwareVersion, PYSNMP_MODULE_ID=cambium, subnetMask=subnetMask, sNMPTrapVersion=sNMPTrapVersion, alarmsGroup=alarmsGroup, dfsPeaks=dfsPeaks, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, sNTPServerIPAddress=sNTPServerIPAddress, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, syslogClient=syslogClient, phyStatus=phyStatus, syslogControlGroup=syslogControlGroup, ethernet=ethernet, dfs=dfs, telecomGroup=telecomGroup, remoteIPAddress=remoteIPAddress, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, userTableIndex=userTableIndex, dataPortDisabledWarning=dataPortDisabledWarning, telecomsCableLength=telecomsCableLength, cableLoss=cableLoss, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, frequencyVariant=frequencyVariant, unitOutOfCalibration=unitOutOfCalibration, telecom=telecom, timeZone=timeZone, eIRP=eIRP, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, hardwareVersion=hardwareVersion, transmitChannel=transmitChannel, encryptionGroup=encryptionGroup)
