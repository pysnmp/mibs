...
# Produced by pysmi-1.1.8 at Thu Jan  5 11:25:04 2023
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", sntpGroup=sntpGroup, dfsTableEntry=dfsTableEntry, sntp=sntp, sNTPState=sNTPState, licenceGroup=licenceGroup, sNMPTrapTableNumber=sNMPTrapTableNumber, transmitPower=transmitPower, range=range, telnetAccessEnabled=telnetAccessEnabled, receiveFreqMHz=receiveFreqMHz, userTableIndex=userTableIndex, vLANPriorityTableEntry=vLANPriorityTableEntry, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, tDDControl=tDDControl, regionCodeTrap=regionCodeTrap, managementGroup=managementGroup, unitOutOfCalibration=unitOutOfCalibration, sMTPEnabledMessages=sMTPEnabledMessages, wirelessLinkStatus=wirelessLinkStatus, dataPortAutoMdix=dataPortAutoMdix, linkName=linkName, supplementary=supplementary, gatewayIPAddress=gatewayIPAddress, receivePower=receivePower, sMTPServerPortNumber=sMTPServerPortNumber, userEnabled=userEnabled, ptpGroups=ptpGroups, iPAddress=iPAddress, masterSlaveMode=masterSlaveMode, antennaGain=antennaGain, useVLANForManagementInterfaces=useVLANForManagementInterfaces, sNTPLastSync=sNTPLastSync, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, systemClock=systemClock, longitude=longitude, groupID=groupID, userPassword=userPassword, tFTPServerPortNumber=tFTPServerPortNumber, configurationGroup=configurationGroup, tDDSynchronizationMode=tDDSynchronizationMode, pubStats=pubStats, sNMPCommunityString=sNMPCommunityString, timeZone=timeZone, ptp=ptp, sMTPSourceEmailAddress=sMTPSourceEmailAddress, hTTPPortNumber=hTTPPortNumber, licence=licence, systemReset=systemReset, aggregateDataRate=aggregateDataRate, subnetMask=subnetMask, signalStrengthRatio=signalStrengthRatio, latitude=latitude, ptp500=ptp500, userTable=userTable, versionsGroup=versionsGroup, pubStatsGroup=pubStatsGroup, maximumTransmitPower=maximumTransmitPower, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, targetMACAddress=targetMACAddress, dfsTable=dfsTable, siteName=siteName, telecomsLoopback=telecomsLoopback, cableLoss=cableLoss, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, tDDSynchronizationAlarm=tDDSynchronizationAlarm, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, phyControl=phyControl, dataPortAutoNegotiation=dataPortAutoNegotiation, versions=versions, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, eIRP=eIRP, telecomsChannelStatusTrap=telecomsChannelStatusTrap, aAAControlGroup=aAAControlGroup, vLANPriorityTableNumber=vLANPriorityTableNumber, installArmStateTrap=installArmStateTrap, userName=userName, bootVersion=bootVersion, linkSymmetry=linkSymmetry, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, encryption=encryption, receiveModulationMode=receiveModulationMode, channelBandwidth=channelBandwidth, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, productName=productName, encryptionAlgorithm=encryptionAlgorithm, snmpControl=snmpControl, dataPortConfigurationMismatch=dataPortConfigurationMismatch, receiveChannel=receiveChannel, sNMPTrapTable=sNMPTrapTable, vLANManagementPriority=vLANManagementPriority, linkModeOptimizationMismatch=linkModeOptimizationMismatch, PYSNMP_MODULE_ID=cambium, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, dfs=dfs, sNMPEnabledTraps=sNMPEnabledTraps, sNMPv3Enable=sNMPv3Enable, alarmsGroup=alarmsGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, dfsTableNumber=dfsTableNumber, productVariant=productVariant, hTTPSAccessEnabled=hTTPSAccessEnabled, daylightSaving=daylightSaving, telecomsLoopbackTrap=telecomsLoopbackTrap, cambium=cambium, sNMPTrapPortNumber=sNMPTrapPortNumber, telecomsInterface=telecomsInterface, smtp=smtp, linkLoss=linkLoss, hTTPSPortNumber=hTTPSPortNumber, hTTPAccessEnabled=hTTPAccessEnabled, transmitFreqMHz=transmitFreqMHz, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, phyStatusGroup=phyStatusGroup, sMTPEmailAlert=sMTPEmailAlert, searchState=searchState, sNTPServerPortNumber=sNTPServerPortNumber, installStatus=installStatus, dataPortDisabledWarning=dataPortDisabledWarning, receiveDataRate=receiveDataRate, sNTPPollInterval=sNTPPollInterval, wirelessLinkAvailability=wirelessLinkAvailability, regionCode=regionCode, sNTPSync=sNTPSync, softwareVersion=softwareVersion, vLANPriorityTable=vLANPriorityTable, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, telecom=telecom, vLANPriorityQueueMapping=vLANPriorityQueueMapping, encryptionGroup=encryptionGroup, transmitModulationMode=transmitModulationMode, sNMPPortNumber=sNMPPortNumber, hardwareVersion=hardwareVersion, syslogClientTrap=syslogClientTrap, tDDControlGroup=tDDControlGroup, ethernetGroup=ethernetGroup, remoteMaximumTransmitPower=remoteMaximumTransmitPower, telecomGroup=telecomGroup, dfsTableIndex=dfsTableIndex, dataPortWirelessDownAlert=dataPortWirelessDownAlert, ethernetFiberSupport=ethernetFiberSupport, sNMPTrapIPAddress=sNMPTrapIPAddress, ethernet=ethernet, telecomsLineCode=telecomsLineCode, aAAControl=aAAControl, incompatibleMasterAndSlave=incompatibleMasterAndSlave, userRole=userRole, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, receiveModulationModeDetail=receiveModulationModeDetail, phyStatus=phyStatus, notificationsGroup=notificationsGroup, channelChangeTrap=channelChangeTrap, ptpTraps=ptpTraps, reset=reset, configuration=configuration, installStatusTrap=installStatusTrap, management=management, altitude=altitude, syslogControl=syslogControl, sNTPServerIPAddress=sNTPServerIPAddress, constantPowerSpectralDensity=constantPowerSpectralDensity, syslogClient=syslogClient, byteErrorRatio=byteErrorRatio, dataPortStatusTrap=dataPortStatusTrap, capacityLicenseMismatch=capacityLicenseMismatch, ptpCompliance=ptpCompliance, vLANPriorityTableIndex=vLANPriorityTableIndex, telnetPortNumber=telnetPortNumber, tDDSynchronizationStatus=tDDSynchronizationStatus, snmpControlGroup=snmpControlGroup, transmitFreqKHz=transmitFreqKHz, dfsMeans=dfsMeans, ptmp=ptmp, resetGroup=resetGroup, linkModeOptimisation=linkModeOptimisation, supplementaryGroup=supplementaryGroup, transmitChannel=transmitChannel, frequencyVariant=frequencyVariant, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, enabledDiagnosticAlarms=enabledDiagnosticAlarms, tFTPServerIPAddress=tFTPServerIPAddress, licensedCapacity=licensedCapacity, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, receiveFreqKHz=receiveFreqKHz, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, phyControlGroup=phyControlGroup, installArmState=installArmState, smtpGroup=smtpGroup, ptpTrapPrefix=ptpTrapPrefix, dfsPeaks=dfsPeaks, dataPortStatus=dataPortStatus, secondarySoftwareVersion=secondarySoftwareVersion, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, bandwidthVariant=bandwidthVariant, accessMethod=accessMethod, vLANManagementVID=vLANManagementVID, alarms=alarms, remoteIPAddress=remoteIPAddress, dfsGroup=dfsGroup, sNMPTrapVersion=sNMPTrapVersion, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, userTableNumber=userTableNumber, userTableEntry=userTableEntry, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, transmitDataRate=transmitDataRate, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNTPSyncTrap=sNTPSyncTrap, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, telecomsChannelStatus=telecomsChannelStatus, noWirelessChannelAvailable=noWirelessChannelAvailable, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, incompatibleRegionCodes=incompatibleRegionCodes, telecomsCableLength=telecomsCableLength, syslogControlGroup=syslogControlGroup, syslogState=syslogState, syslogLocalWrappedTrap=syslogLocalWrappedTrap, vectorError=vectorError, sNMPTrapTableIndex=sNMPTrapTableIndex, sMTPServerIPAddress=sMTPServerIPAddress, syslogStateTrap=syslogStateTrap)
