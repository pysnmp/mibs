...
# Produced by pysmi-1.1.8 at Tue Jan 11 21:10:51 2022
...
ModuleIdentity, iso, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, Gauge32, IpAddress, Integer32, NotificationType, Counter64, Bits, Unsigned32, MibIdentifier, ObjectIdentity, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "iso", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "Gauge32", "IpAddress", "Integer32", "NotificationType", "Counter64", "Bits", "Unsigned32", "MibIdentifier", "ObjectIdentity", "TimeTicks")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, dfsPeaks=dfsPeaks, userPassword=userPassword, sNMPTrapPortNumber=sNMPTrapPortNumber, signalStrengthRatio=signalStrengthRatio, systemReset=systemReset, tFTPServerPortNumber=tFTPServerPortNumber, transmitChannel=transmitChannel, sNMPTrapVersion=sNMPTrapVersion, phyControlGroup=phyControlGroup, dataPortDisabledWarning=dataPortDisabledWarning, userTableNumber=userTableNumber, sNTPPollInterval=sNTPPollInterval, longitude=longitude, receiveFreqKHz=receiveFreqKHz, pubStatsGroup=pubStatsGroup, receiveDataRate=receiveDataRate, enabledDiagnosticAlarms=enabledDiagnosticAlarms, alarms=alarms, bandwidthVariant=bandwidthVariant, constantPowerSpectralDensity=constantPowerSpectralDensity, receivePower=receivePower, wirelessLinkStatus=wirelessLinkStatus, installStatus=installStatus, ethernetFiberSupport=ethernetFiberSupport, snmpControlGroup=snmpControlGroup, telnetAccessEnabled=telnetAccessEnabled, bootVersion=bootVersion, useVLANForManagementInterfaces=useVLANForManagementInterfaces, transmitFreqMHz=transmitFreqMHz, telecomsInterface=telecomsInterface, dfs=dfs, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, ptpTrapPrefix=ptpTrapPrefix, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, sNTPLastSync=sNTPLastSync, pubStats=pubStats, telecomsLoopback=telecomsLoopback, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, incompatibleRegionCodes=incompatibleRegionCodes, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, cambium=cambium, syslogClient=syslogClient, productName=productName, versions=versions, dfsTableNumber=dfsTableNumber, licenceGroup=licenceGroup, secondarySoftwareVersion=secondarySoftwareVersion, incompatibleMasterAndSlave=incompatibleMasterAndSlave, vLANPriorityTableNumber=vLANPriorityTableNumber, phyStatus=phyStatus, userName=userName, systemClock=systemClock, managementGroup=managementGroup, ptmp=ptmp, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, cableLoss=cableLoss, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, transmitFreqKHz=transmitFreqKHz, sNTPServerPortNumber=sNTPServerPortNumber, accessMethod=accessMethod, telecomsChannelStatusTrap=telecomsChannelStatusTrap, dataPortAutoMdix=dataPortAutoMdix, subnetMask=subnetMask, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, linkModeOptimizationMismatch=linkModeOptimizationMismatch, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, userTableEntry=userTableEntry, linkSymmetry=linkSymmetry, noWirelessChannelAvailable=noWirelessChannelAvailable, sNTPState=sNTPState, gatewayIPAddress=gatewayIPAddress, unitOutOfCalibration=unitOutOfCalibration, resetGroup=resetGroup, sNMPPortNumber=sNMPPortNumber, ptpCompliance=ptpCompliance, dataPortStatus=dataPortStatus, tDDSynchronizationStatus=tDDSynchronizationStatus, userTable=userTable, transmitPower=transmitPower, aggregateDataRate=aggregateDataRate, tFTPServerIPAddress=tFTPServerIPAddress, dfsMeans=dfsMeans, management=management, vLANPriorityTableIndex=vLANPriorityTableIndex, tDDSynchronizationMode=tDDSynchronizationMode, sMTPEmailAlert=sMTPEmailAlert, masterSlaveMode=masterSlaveMode, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, installStatusTrap=installStatusTrap, channelBandwidth=channelBandwidth, latitude=latitude, licensedCapacity=licensedCapacity, altitude=altitude, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, byteErrorRatio=byteErrorRatio, channelChangeTrap=channelChangeTrap, smtp=smtp, dfsTableIndex=dfsTableIndex, ptpTraps=ptpTraps, alarmsGroup=alarmsGroup, vLANPriorityTable=vLANPriorityTable, timeZone=timeZone, iPAddress=iPAddress, ethernetGroup=ethernetGroup, telecomGroup=telecomGroup, installArmState=installArmState, PYSNMP_MODULE_ID=cambium, searchState=searchState, sNTPSync=sNTPSync, sNMPTrapTableEntry=sNMPTrapTableEntry, configuration=configuration, dataPortConfigurationMismatch=dataPortConfigurationMismatch, userRole=userRole, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, ptp500=ptp500, syslogStateTrap=syslogStateTrap, telecomsChannelStatus=telecomsChannelStatus, supplementary=supplementary, capacityLicenseMismatch=capacityLicenseMismatch, syslogControlGroup=syslogControlGroup, dataPortWirelessDownAlert=dataPortWirelessDownAlert, vectorError=vectorError, dataPortAutoNegotiation=dataPortAutoNegotiation, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, versionsGroup=versionsGroup, sNMPTrapTable=sNMPTrapTable, remoteIPAddress=remoteIPAddress, snmpControl=snmpControl, phyStatusGroup=phyStatusGroup, vLANManagementPriority=vLANManagementPriority, siteName=siteName, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, supplementaryGroup=supplementaryGroup, hTTPSAccessEnabled=hTTPSAccessEnabled, sMTPServerPortNumber=sMTPServerPortNumber, vLANPriorityQueueMapping=vLANPriorityQueueMapping, hTTPAccessEnabled=hTTPAccessEnabled, sMTPServerIPAddress=sMTPServerIPAddress, configurationGroup=configurationGroup, licence=licence, aAAControlGroup=aAAControlGroup, sntpGroup=sntpGroup, telecomsLoopbackTrap=telecomsLoopbackTrap, aAAControl=aAAControl, receiveChannel=receiveChannel, dfsTable=dfsTable, dfsTableEntry=dfsTableEntry, telecomsLineCode=telecomsLineCode, maximumTransmitPower=maximumTransmitPower, syslogState=syslogState, productVariant=productVariant, sNMPEnabledTraps=sNMPEnabledTraps, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, sNTPServerIPAddress=sNTPServerIPAddress, sNTPSyncTrap=sNTPSyncTrap, syslogClientTrap=syslogClientTrap, transmitModulationMode=transmitModulationMode, frequencyVariant=frequencyVariant, sntp=sntp, eIRP=eIRP, receiveFreqMHz=receiveFreqMHz, reset=reset, telecom=telecom, regionCode=regionCode, sNMPTrapTableNumber=sNMPTrapTableNumber, remoteMaximumTransmitPower=remoteMaximumTransmitPower, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, ptp=ptp, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, encryption=encryption, sNMPTrapIPAddress=sNMPTrapIPAddress, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNMPTrapTableIndex=sNMPTrapTableIndex, installArmStateTrap=installArmStateTrap, vLANPriorityTableEntry=vLANPriorityTableEntry, vLANManagementVID=vLANManagementVID, telnetPortNumber=telnetPortNumber, tDDControlGroup=tDDControlGroup, linkModeOptimisation=linkModeOptimisation, hTTPPortNumber=hTTPPortNumber, sNMPCommunityString=sNMPCommunityString, encryptionAlgorithm=encryptionAlgorithm, linkName=linkName, ptpGroups=ptpGroups, linkLoss=linkLoss, groupID=groupID, phyControl=phyControl, sMTPSourceEmailAddress=sMTPSourceEmailAddress, targetMACAddress=targetMACAddress, wirelessLinkAvailability=wirelessLinkAvailability, syslogControl=syslogControl, antennaGain=antennaGain, telecomsCableLength=telecomsCableLength, receiveModulationModeDetail=receiveModulationModeDetail, tDDControl=tDDControl, range=range, dataPortStatusTrap=dataPortStatusTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, smtpGroup=smtpGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dfsGroup=dfsGroup, hardwareVersion=hardwareVersion, sNMPv3Enable=sNMPv3Enable, hTTPSPortNumber=hTTPSPortNumber, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, daylightSaving=daylightSaving, userEnabled=userEnabled, notificationsGroup=notificationsGroup, softwareVersion=softwareVersion, transmitDataRate=transmitDataRate, userTableIndex=userTableIndex, receiveModulationMode=receiveModulationMode, sMTPEnabledMessages=sMTPEnabledMessages, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, ethernet=ethernet, regionCodeTrap=regionCodeTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, encryptionGroup=encryptionGroup)
