...
# Produced by pysmi-1.1.3 at Sat Nov 20 21:25:59 2021
...
Bits, TimeTicks, ObjectIdentity, NotificationType, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, Integer32, IpAddress, iso, Gauge32, enterprises, Counter64, ModuleIdentity, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "TimeTicks", "ObjectIdentity", "NotificationType", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "Integer32", "IpAddress", "iso", "Gauge32", "enterprises", "Counter64", "ModuleIdentity", "MibIdentifier")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", useVLANForManagementInterfaces=useVLANForManagementInterfaces, ptpCompliance=ptpCompliance, tDDSynchronizationAlarm=tDDSynchronizationAlarm, cableLoss=cableLoss, gatewayIPAddress=gatewayIPAddress, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, userRole=userRole, snmpControlGroup=snmpControlGroup, notificationsGroup=notificationsGroup, reset=reset, timeZone=timeZone, systemReset=systemReset, daylightSaving=daylightSaving, maximumTransmitPower=maximumTransmitPower, tDDSynchronizationMode=tDDSynchronizationMode, sNMPCommunityString=sNMPCommunityString, sNTPServerIPAddress=sNTPServerIPAddress, sNMPTrapIPAddress=sNMPTrapIPAddress, smtp=smtp, phyStatusGroup=phyStatusGroup, channelChangeTrap=channelChangeTrap, eIRP=eIRP, telecomsLineCode=telecomsLineCode, supplementary=supplementary, softwareVersion=softwareVersion, capacityLicenseMismatch=capacityLicenseMismatch, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, telecomsChannelStatusTrap=telecomsChannelStatusTrap, telecomsLoopback=telecomsLoopback, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, versions=versions, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, dfsTableEntry=dfsTableEntry, ptpTraps=ptpTraps, configuration=configuration, linkLoss=linkLoss, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, vLANPriorityTableEntry=vLANPriorityTableEntry, noWirelessChannelAvailable=noWirelessChannelAvailable, sNTPServerPortNumber=sNTPServerPortNumber, installStatus=installStatus, dfsGroup=dfsGroup, range=range, ptmp=ptmp, sntp=sntp, vLANPriorityTableNumber=vLANPriorityTableNumber, remoteMaximumTransmitPower=remoteMaximumTransmitPower, receiveModulationModeDetail=receiveModulationModeDetail, userPassword=userPassword, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, ethernetGroup=ethernetGroup, enabledDiagnosticAlarms=enabledDiagnosticAlarms, secondarySoftwareVersion=secondarySoftwareVersion, antennaGain=antennaGain, receiveChannel=receiveChannel, smtpGroup=smtpGroup, userTableIndex=userTableIndex, sNMPTrapTable=sNMPTrapTable, vectorError=vectorError, telecom=telecom, aAAControl=aAAControl, PYSNMP_MODULE_ID=cambium, syslogControl=syslogControl, syslogLocalWrappedTrap=syslogLocalWrappedTrap, masterSlaveMode=masterSlaveMode, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sMTPSourceEmailAddress=sMTPSourceEmailAddress, receiveDataRate=receiveDataRate, groupID=groupID, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, phyControlGroup=phyControlGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, syslogClient=syslogClient, byteErrorRatio=byteErrorRatio, installArmState=installArmState, incompatibleRegionCodes=incompatibleRegionCodes, dataPortAutoMdix=dataPortAutoMdix, userTable=userTable, bootVersion=bootVersion, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, ethernetFiberSupport=ethernetFiberSupport, encryptionGroup=encryptionGroup, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sntpGroup=sntpGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, hTTPPortNumber=hTTPPortNumber, unitOutOfCalibration=unitOutOfCalibration, configurationGroup=configurationGroup, receiveModulationMode=receiveModulationMode, cambium=cambium, sNMPTrapTableIndex=sNMPTrapTableIndex, dfs=dfs, ptp=ptp, installStatusTrap=installStatusTrap, tFTPServerIPAddress=tFTPServerIPAddress, tFTPServerPortNumber=tFTPServerPortNumber, alarms=alarms, sMTPServerIPAddress=sMTPServerIPAddress, aggregateDataRate=aggregateDataRate, telecomsLoopbackTrap=telecomsLoopbackTrap, syslogStateTrap=syslogStateTrap, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, licenceGroup=licenceGroup, receiveFreqKHz=receiveFreqKHz, resetGroup=resetGroup, vLANPriorityTableIndex=vLANPriorityTableIndex, dataPortConfigurationMismatch=dataPortConfigurationMismatch, systemClock=systemClock, wirelessLinkStatus=wirelessLinkStatus, sNMPTrapTableNumber=sNMPTrapTableNumber, pubStatsGroup=pubStatsGroup, sNMPPortNumber=sNMPPortNumber, userName=userName, productVariant=productVariant, userEnabled=userEnabled, dfsTableNumber=dfsTableNumber, sNMPEnabledTraps=sNMPEnabledTraps, regionCodeTrap=regionCodeTrap, snmpControl=snmpControl, latitude=latitude, telnetPortNumber=telnetPortNumber, vLANManagementVID=vLANManagementVID, syslogState=syslogState, receiveFreqMHz=receiveFreqMHz, alarmsGroup=alarmsGroup, dfsTableIndex=dfsTableIndex, sNTPState=sNTPState, linkModeOptimizationMismatch=linkModeOptimizationMismatch, incompatibleMasterAndSlave=incompatibleMasterAndSlave, hTTPSPortNumber=hTTPSPortNumber, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, transmitFreqMHz=transmitFreqMHz, encryptionAlgorithm=encryptionAlgorithm, telecomsChannelStatus=telecomsChannelStatus, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, syslogClientTrap=syslogClientTrap, supplementaryGroup=supplementaryGroup, pubStats=pubStats, tDDSynchronizationStatus=tDDSynchronizationStatus, userTableNumber=userTableNumber, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, sNTPSync=sNTPSync, dataPortWirelessDownAlert=dataPortWirelessDownAlert, transmitModulationMode=transmitModulationMode, linkName=linkName, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, dataPortStatusTrap=dataPortStatusTrap, ptpGroups=ptpGroups, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, dfsMeans=dfsMeans, targetMACAddress=targetMACAddress, management=management, vLANManagementPriority=vLANManagementPriority, phyStatus=phyStatus, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, tDDControlGroup=tDDControlGroup, telecomsCableLength=telecomsCableLength, transmitFreqKHz=transmitFreqKHz, sNTPLastSync=sNTPLastSync, dataPortDisabledWarning=dataPortDisabledWarning, remoteIPAddress=remoteIPAddress, frequencyVariant=frequencyVariant, sNMPv3Enable=sNMPv3Enable, longitude=longitude, ethernet=ethernet, transmitDataRate=transmitDataRate, ptpTrapPrefix=ptpTrapPrefix, syslogControlGroup=syslogControlGroup, vLANPriorityTable=vLANPriorityTable, linkModeOptimisation=linkModeOptimisation, transmitChannel=transmitChannel, dfsTable=dfsTable, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNMPTrapTableEntry=sNMPTrapTableEntry, tDDControl=tDDControl, telecomsInterface=telecomsInterface, hTTPAccessEnabled=hTTPAccessEnabled, telnetAccessEnabled=telnetAccessEnabled, sMTPEnabledMessages=sMTPEnabledMessages, installArmStateTrap=installArmStateTrap, receivePower=receivePower, subnetMask=subnetMask, dfsPeaks=dfsPeaks, regionCode=regionCode, altitude=altitude, iPAddress=iPAddress, accessMethod=accessMethod, ptp500=ptp500, sMTPEmailAlert=sMTPEmailAlert, phyControl=phyControl, siteName=siteName, licensedCapacity=licensedCapacity, channelBandwidth=channelBandwidth, sNTPSyncTrap=sNTPSyncTrap, constantPowerSpectralDensity=constantPowerSpectralDensity, linkSymmetry=linkSymmetry, wirelessLinkAvailability=wirelessLinkAvailability, searchState=searchState, sMTPServerPortNumber=sMTPServerPortNumber, userTableEntry=userTableEntry, signalStrengthRatio=signalStrengthRatio, transmitPower=transmitPower, sNTPPollInterval=sNTPPollInterval, aAAControlGroup=aAAControlGroup, dataPortStatus=dataPortStatus, telecomGroup=telecomGroup, managementGroup=managementGroup, versionsGroup=versionsGroup, productName=productName, sNMPTrapPortNumber=sNMPTrapPortNumber, licence=licence, bandwidthVariant=bandwidthVariant, sNMPTrapVersion=sNMPTrapVersion, encryption=encryption, hardwareVersion=hardwareVersion, hTTPSAccessEnabled=hTTPSAccessEnabled)
