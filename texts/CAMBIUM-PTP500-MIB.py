...
# Produced by pysmi-1.1.3 at Sun Nov 28 19:50:21 2021
...
Integer32, ObjectIdentity, MibIdentifier, TimeTicks, Gauge32, Counter64, Bits, IpAddress, enterprises, NotificationType, Unsigned32, ModuleIdentity, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, iso = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "ObjectIdentity", "MibIdentifier", "TimeTicks", "Gauge32", "Counter64", "Bits", "IpAddress", "enterprises", "NotificationType", "Unsigned32", "ModuleIdentity", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", encryptionGroup=encryptionGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, dataPortConfigurationMismatch=dataPortConfigurationMismatch, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, vLANPriorityQueueMapping=vLANPriorityQueueMapping, tDDSynchronizationStatus=tDDSynchronizationStatus, range=range, constantPowerSpectralDensity=constantPowerSpectralDensity, phyControl=phyControl, regionCodeTrap=regionCodeTrap, productVariant=productVariant, alarmsGroup=alarmsGroup, vLANManagementVID=vLANManagementVID, wirelessLinkAvailability=wirelessLinkAvailability, syslogControl=syslogControl, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, supplementaryGroup=supplementaryGroup, bandwidthVariant=bandwidthVariant, dfsTableIndex=dfsTableIndex, ptpTraps=ptpTraps, syslogClientTrap=syslogClientTrap, subnetMask=subnetMask, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, telecomGroup=telecomGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, transmitFreqMHz=transmitFreqMHz, sNMPEnabledTraps=sNMPEnabledTraps, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, ptp500=ptp500, tDDControlGroup=tDDControlGroup, signalStrengthRatio=signalStrengthRatio, ptp=ptp, linkName=linkName, installStatus=installStatus, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPCommunityString=sNMPCommunityString, syslogClient=syslogClient, bootVersion=bootVersion, receiveModulationModeDetail=receiveModulationModeDetail, ptpGroups=ptpGroups, latitude=latitude, telecomsLoopback=telecomsLoopback, maximumTransmitPower=maximumTransmitPower, syslogLocalWrappedTrap=syslogLocalWrappedTrap, dfsPeaks=dfsPeaks, receiveFreqMHz=receiveFreqMHz, smtpGroup=smtpGroup, versionsGroup=versionsGroup, sntpGroup=sntpGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, tFTPServerIPAddress=tFTPServerIPAddress, sNTPServerPortNumber=sNTPServerPortNumber, timeZone=timeZone, dataPortWirelessDownAlert=dataPortWirelessDownAlert, userTableIndex=userTableIndex, telecomsLineCode=telecomsLineCode, antennaGain=antennaGain, sNMPTrapVersion=sNMPTrapVersion, userRole=userRole, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, wirelessLinkStatus=wirelessLinkStatus, ptpTrapPrefix=ptpTrapPrefix, snmpControl=snmpControl, telecom=telecom, sMTPServerPortNumber=sMTPServerPortNumber, encryption=encryption, accessMethod=accessMethod, phyControlGroup=phyControlGroup, ethernetFiberSupport=ethernetFiberSupport, dataPortAutoNegotiation=dataPortAutoNegotiation, ptpCompliance=ptpCompliance, hTTPPortNumber=hTTPPortNumber, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, dataPortAutoMdix=dataPortAutoMdix, reset=reset, sMTPServerIPAddress=sMTPServerIPAddress, telecomsCableLength=telecomsCableLength, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, notificationsGroup=notificationsGroup, sMTPEnabledMessages=sMTPEnabledMessages, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sntp=sntp, pubStatsGroup=pubStatsGroup, frequencyVariant=frequencyVariant, telecomsChannelStatus=telecomsChannelStatus, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, sNTPPollInterval=sNTPPollInterval, syslogStateTrap=syslogStateTrap, resetGroup=resetGroup, sMTPEmailAlert=sMTPEmailAlert, aggregateDataRate=aggregateDataRate, dfs=dfs, snmpControlGroup=snmpControlGroup, incompatibleRegionCodes=incompatibleRegionCodes, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNTPServerIPAddress=sNTPServerIPAddress, userEnabled=userEnabled, telecomsLoopbackTrap=telecomsLoopbackTrap, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, sNMPv3Enable=sNMPv3Enable, dataPortStatus=dataPortStatus, iPAddress=iPAddress, sNTPSync=sNTPSync, dataPortStatusTrap=dataPortStatusTrap, vLANPriorityTableNumber=vLANPriorityTableNumber, incompatibleMasterAndSlave=incompatibleMasterAndSlave, vLANPriorityTable=vLANPriorityTable, alarms=alarms, userTableNumber=userTableNumber, dfsTableNumber=dfsTableNumber, transmitChannel=transmitChannel, configurationGroup=configurationGroup, smtp=smtp, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dataPortDisabledWarning=dataPortDisabledWarning, capacityLicenseMismatch=capacityLicenseMismatch, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, phyStatus=phyStatus, hTTPSPortNumber=hTTPSPortNumber, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, installArmStateTrap=installArmStateTrap, regionCode=regionCode, configuration=configuration, linkSymmetry=linkSymmetry, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNMPTrapPortNumber=sNMPTrapPortNumber, telnetAccessEnabled=telnetAccessEnabled, telecomsInterface=telecomsInterface, siteName=siteName, licence=licence, vLANPriorityTableIndex=vLANPriorityTableIndex, hardwareVersion=hardwareVersion, receiveChannel=receiveChannel, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, pubStats=pubStats, syslogState=syslogState, receiveDataRate=receiveDataRate, altitude=altitude, daylightSaving=daylightSaving, licenceGroup=licenceGroup, masterSlaveMode=masterSlaveMode, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, linkModeOptimizationMismatch=linkModeOptimizationMismatch, dfsTable=dfsTable, telnetPortNumber=telnetPortNumber, targetMACAddress=targetMACAddress, ptmp=ptmp, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, transmitModulationMode=transmitModulationMode, sNMPTrapIPAddress=sNMPTrapIPAddress, supplementary=supplementary, phyStatusGroup=phyStatusGroup, softwareVersion=softwareVersion, userPassword=userPassword, receiveFreqKHz=receiveFreqKHz, versions=versions, secondarySoftwareVersion=secondarySoftwareVersion, receivePower=receivePower, systemClock=systemClock, transmitFreqKHz=transmitFreqKHz, byteErrorRatio=byteErrorRatio, transmitPower=transmitPower, dfsGroup=dfsGroup, ethernetGroup=ethernetGroup, groupID=groupID, cambium=cambium, useVLANForManagementInterfaces=useVLANForManagementInterfaces, tDDSynchronizationMode=tDDSynchronizationMode, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, encryptionAlgorithm=encryptionAlgorithm, PYSNMP_MODULE_ID=cambium, eIRP=eIRP, licensedCapacity=licensedCapacity, longitude=longitude, linkLoss=linkLoss, sNMPTrapTableIndex=sNMPTrapTableIndex, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, aAAControlGroup=aAAControlGroup, systemReset=systemReset, syslogControlGroup=syslogControlGroup, sNMPPortNumber=sNMPPortNumber, receiveModulationMode=receiveModulationMode, unitOutOfCalibration=unitOutOfCalibration, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, sNMPTrapTable=sNMPTrapTable, userTableEntry=userTableEntry, installArmState=installArmState, dfsTableEntry=dfsTableEntry, dfsMeans=dfsMeans, vectorError=vectorError, tDDControl=tDDControl, vLANManagementPriority=vLANManagementPriority, sNTPState=sNTPState, management=management, gatewayIPAddress=gatewayIPAddress, linkModeOptimisation=linkModeOptimisation, telecomsChannelStatusTrap=telecomsChannelStatusTrap, sNTPSyncTrap=sNTPSyncTrap, ethernet=ethernet, channelBandwidth=channelBandwidth, productName=productName, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPTrapTableNumber=sNMPTrapTableNumber, channelChangeTrap=channelChangeTrap, tFTPServerPortNumber=tFTPServerPortNumber, transmitDataRate=transmitDataRate, userTable=userTable, cableLoss=cableLoss, hTTPAccessEnabled=hTTPAccessEnabled, userName=userName, searchState=searchState, hTTPSAccessEnabled=hTTPSAccessEnabled, vLANPriorityTableEntry=vLANPriorityTableEntry, remoteMaximumTransmitPower=remoteMaximumTransmitPower, remoteIPAddress=remoteIPAddress, installStatusTrap=installStatusTrap, sNTPLastSync=sNTPLastSync, managementGroup=managementGroup, aAAControl=aAAControl)
