...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", receivePower=receivePower, incompatibleRegionCodes=incompatibleRegionCodes, sntpGroup=sntpGroup, userTableEntry=userTableEntry, vLANPriorityTableEntry=vLANPriorityTableEntry, antennaGain=antennaGain, bandwidthVariant=bandwidthVariant, transmitPower=transmitPower, sNMPv3Enable=sNMPv3Enable, sMTPEnabledMessages=sMTPEnabledMessages, telecomsLoopback=telecomsLoopback, dataPortAutoMdix=dataPortAutoMdix, tFTPServerPortNumber=tFTPServerPortNumber, sNMPTrapVersion=sNMPTrapVersion, sNMPTrapTable=sNMPTrapTable, supplementary=supplementary, vLANPriorityTable=vLANPriorityTable, tFTPServerIPAddress=tFTPServerIPAddress, installArmState=installArmState, syslogStateTrap=syslogStateTrap, wirelessLinkAvailability=wirelessLinkAvailability, ptp500=ptp500, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, tDDSynchronizationAlarm=tDDSynchronizationAlarm, hTTPPortNumber=hTTPPortNumber, userTable=userTable, userPassword=userPassword, ptmp=ptmp, configuration=configuration, vLANPriorityTableNumber=vLANPriorityTableNumber, cableLoss=cableLoss, receiveModulationMode=receiveModulationMode, userName=userName, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, signalStrengthRatio=signalStrengthRatio, cambium=cambium, linkLoss=linkLoss, capacityLicenseMismatch=capacityLicenseMismatch, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, sNMPTrapIPAddress=sNMPTrapIPAddress, sntp=sntp, dfsPeaks=dfsPeaks, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, ptpGroups=ptpGroups, productVariant=productVariant, sNMPPortNumber=sNMPPortNumber, transmitModulationMode=transmitModulationMode, pubStatsGroup=pubStatsGroup, ptp=ptp, snmpControlGroup=snmpControlGroup, phyControlGroup=phyControlGroup, range=range, frequencyVariant=frequencyVariant, ethernetFiberSupport=ethernetFiberSupport, linkSymmetry=linkSymmetry, unitOutOfCalibration=unitOutOfCalibration, linkModeOptimisation=linkModeOptimisation, telecomsLineCode=telecomsLineCode, tDDSynchronizationMode=tDDSynchronizationMode, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, receiveChannel=receiveChannel, hTTPSPortNumber=hTTPSPortNumber, aAAControlGroup=aAAControlGroup, dfsTable=dfsTable, licence=licence, sNTPSync=sNTPSync, userEnabled=userEnabled, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, telecomsInterface=telecomsInterface, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, incompatibleMasterAndSlave=incompatibleMasterAndSlave, syslogControl=syslogControl, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, hardwareVersion=hardwareVersion, alarmsGroup=alarmsGroup, dfsGroup=dfsGroup, wirelessLinkStatus=wirelessLinkStatus, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, aggregateDataRate=aggregateDataRate, encryptionAlgorithm=encryptionAlgorithm, telecomsChannelStatus=telecomsChannelStatus, management=management, noWirelessChannelAvailable=noWirelessChannelAvailable, remoteMaximumTransmitPower=remoteMaximumTransmitPower, reset=reset, versions=versions, sNMPTrapTableIndex=sNMPTrapTableIndex, tDDControl=tDDControl, softwareVersion=softwareVersion, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPCommunityString=sNMPCommunityString, telecom=telecom, syslogClient=syslogClient, longitude=longitude, phyStatus=phyStatus, constantPowerSpectralDensity=constantPowerSpectralDensity, searchState=searchState, telecomsLoopbackTrap=telecomsLoopbackTrap, tDDControlGroup=tDDControlGroup, aAAControl=aAAControl, receiveDataRate=receiveDataRate, userTableNumber=userTableNumber, sNMPTrapTableEntry=sNMPTrapTableEntry, receiveFreqMHz=receiveFreqMHz, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, remoteIPAddress=remoteIPAddress, sMTPSourceEmailAddress=sMTPSourceEmailAddress, phyStatusGroup=phyStatusGroup, receiveFreqKHz=receiveFreqKHz, telecomsChannelStatusTrap=telecomsChannelStatusTrap, masterSlaveMode=masterSlaveMode, transmitFreqMHz=transmitFreqMHz, sNTPPollInterval=sNTPPollInterval, dfsTableEntry=dfsTableEntry, sNMPEnabledTraps=sNMPEnabledTraps, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, dataPortAutoNegotiation=dataPortAutoNegotiation, dfsTableNumber=dfsTableNumber, iPAddress=iPAddress, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sNTPSyncTrap=sNTPSyncTrap, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, PYSNMP_MODULE_ID=cambium, sNTPServerPortNumber=sNTPServerPortNumber, alarms=alarms, sNTPLastSync=sNTPLastSync, versionsGroup=versionsGroup, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, transmitFreqKHz=transmitFreqKHz, systemClock=systemClock, maximumTransmitPower=maximumTransmitPower, syslogClientTrap=syslogClientTrap, siteName=siteName, sNTPState=sNTPState, sMTPServerPortNumber=sMTPServerPortNumber, smtpGroup=smtpGroup, useVLANForManagementInterfaces=useVLANForManagementInterfaces, regionCode=regionCode, hTTPSAccessEnabled=hTTPSAccessEnabled, secondarySoftwareVersion=secondarySoftwareVersion, syslogControlGroup=syslogControlGroup, vLANManagementVID=vLANManagementVID, transmitChannel=transmitChannel, installStatusTrap=installStatusTrap, encryption=encryption, notificationsGroup=notificationsGroup, dfs=dfs, daylightSaving=daylightSaving, receiveModulationModeDetail=receiveModulationModeDetail, hTTPAccessEnabled=hTTPAccessEnabled, bootVersion=bootVersion, ptpTraps=ptpTraps, vLANManagementPriority=vLANManagementPriority, sNMPTrapPortNumber=sNMPTrapPortNumber, supplementaryGroup=supplementaryGroup, licensedCapacity=licensedCapacity, latitude=latitude, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, sMTPServerIPAddress=sMTPServerIPAddress, altitude=altitude, enabledDiagnosticAlarms=enabledDiagnosticAlarms, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, telecomGroup=telecomGroup, licenceGroup=licenceGroup, sNMPTrapTableNumber=sNMPTrapTableNumber, telnetPortNumber=telnetPortNumber, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, subnetMask=subnetMask, byteErrorRatio=byteErrorRatio, smtp=smtp, pubStats=pubStats, userRole=userRole, timeZone=timeZone, encryptionGroup=encryptionGroup, groupID=groupID, vectorError=vectorError, regionCodeTrap=regionCodeTrap, dataPortConfigurationMismatch=dataPortConfigurationMismatch, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, tDDSynchronizationStatus=tDDSynchronizationStatus, vLANPriorityTableIndex=vLANPriorityTableIndex, productName=productName, resetGroup=resetGroup, phyControl=phyControl, ptpTrapPrefix=ptpTrapPrefix, dataPortDisabledWarning=dataPortDisabledWarning, transmitDataRate=transmitDataRate, telnetAccessEnabled=telnetAccessEnabled, dfsTableIndex=dfsTableIndex, gatewayIPAddress=gatewayIPAddress, systemReset=systemReset, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, syslogState=syslogState, accessMethod=accessMethod, sNTPServerIPAddress=sNTPServerIPAddress, installArmStateTrap=installArmStateTrap, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dfsMeans=dfsMeans, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, ethernetGroup=ethernetGroup, telecomsCableLength=telecomsCableLength, installStatus=installStatus, ptpCompliance=ptpCompliance, managementGroup=managementGroup, channelBandwidth=channelBandwidth, snmpControl=snmpControl, linkName=linkName, channelChangeTrap=channelChangeTrap, targetMACAddress=targetMACAddress, ethernet=ethernet, dataPortStatus=dataPortStatus, dataPortStatusTrap=dataPortStatusTrap, configurationGroup=configurationGroup, eIRP=eIRP, sMTPEmailAlert=sMTPEmailAlert, userTableIndex=userTableIndex, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode)
