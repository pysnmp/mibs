...
# Produced by pysmi-1.1.3 at Wed Dec  1 17:04:13 2021
...
enterprises, ModuleIdentity, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Unsigned32, Counter32, Counter64, ObjectIdentity, Bits, MibIdentifier, TimeTicks, Integer32, NotificationType, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "ModuleIdentity", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Unsigned32", "Counter32", "Counter64", "ObjectIdentity", "Bits", "MibIdentifier", "TimeTicks", "Integer32", "NotificationType", "IpAddress")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", sNTPServerIPAddress=sNTPServerIPAddress, regionCodeTrap=regionCodeTrap, transmitModulationMode=transmitModulationMode, smtpGroup=smtpGroup, dataPortStatusTrap=dataPortStatusTrap, syslogClientTrap=syslogClientTrap, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, transmitPower=transmitPower, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, telecomsChannelStatus=telecomsChannelStatus, linkName=linkName, sNMPTrapTableEntry=sNMPTrapTableEntry, tDDSynchronizationMode=tDDSynchronizationMode, hardwareVersion=hardwareVersion, alarms=alarms, syslogClient=syslogClient, cableLoss=cableLoss, ptpTrapPrefix=ptpTrapPrefix, telecomsLineCode=telecomsLineCode, pubStatsGroup=pubStatsGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, telnetAccessEnabled=telnetAccessEnabled, licence=licence, subnetMask=subnetMask, softwareVersion=softwareVersion, pubStats=pubStats, ptmp=ptmp, tDDControl=tDDControl, vLANManagementPriority=vLANManagementPriority, phyStatus=phyStatus, useVLANForManagementInterfaces=useVLANForManagementInterfaces, installStatusTrap=installStatusTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, telecomsCableLength=telecomsCableLength, sntpGroup=sntpGroup, systemReset=systemReset, userRole=userRole, managementGroup=managementGroup, sNMPTrapVersion=sNMPTrapVersion, syslogState=syslogState, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, versions=versions, ptp500=ptp500, ptp=ptp, receiveModulationModeDetail=receiveModulationModeDetail, latitude=latitude, tDDSynchronizationAlarm=tDDSynchronizationAlarm, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, smtp=smtp, ethernetGroup=ethernetGroup, installStatus=installStatus, receiveChannel=receiveChannel, syslogLocalWrappedTrap=syslogLocalWrappedTrap, tDDSynchronizationStatus=tDDSynchronizationStatus, telecomGroup=telecomGroup, sNTPPollInterval=sNTPPollInterval, vLANPriorityTableIndex=vLANPriorityTableIndex, snmpControlGroup=snmpControlGroup, unitOutOfCalibration=unitOutOfCalibration, altitude=altitude, aggregateDataRate=aggregateDataRate, maximumTransmitPower=maximumTransmitPower, syslogControl=syslogControl, groupID=groupID, ethernet=ethernet, aAAControlGroup=aAAControlGroup, versionsGroup=versionsGroup, telecomsInterface=telecomsInterface, sNMPCommunityString=sNMPCommunityString, userPassword=userPassword, sNMPTrapPortNumber=sNMPTrapPortNumber, aAAControl=aAAControl, capacityLicenseMismatch=capacityLicenseMismatch, sNMPTrapTableNumber=sNMPTrapTableNumber, linkSymmetry=linkSymmetry, transmitFreqMHz=transmitFreqMHz, alarmsGroup=alarmsGroup, secondarySoftwareVersion=secondarySoftwareVersion, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, sntp=sntp, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dataPortAutoMdix=dataPortAutoMdix, productName=productName, frequencyVariant=frequencyVariant, tDDControlGroup=tDDControlGroup, dfsTableEntry=dfsTableEntry, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, sNTPLastSync=sNTPLastSync, masterSlaveMode=masterSlaveMode, byteErrorRatio=byteErrorRatio, dfsGroup=dfsGroup, siteName=siteName, supplementary=supplementary, phyStatusGroup=phyStatusGroup, hTTPSPortNumber=hTTPSPortNumber, dataPortDisabledWarning=dataPortDisabledWarning, noWirelessChannelAvailable=noWirelessChannelAvailable, sMTPServerIPAddress=sMTPServerIPAddress, sNTPSync=sNTPSync, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, hTTPAccessEnabled=hTTPAccessEnabled, sMTPEmailAlert=sMTPEmailAlert, tFTPServerIPAddress=tFTPServerIPAddress, phyControlGroup=phyControlGroup, searchState=searchState, wirelessLinkAvailability=wirelessLinkAvailability, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, telecomsLoopbackTrap=telecomsLoopbackTrap, notificationsGroup=notificationsGroup, signalStrengthRatio=signalStrengthRatio, linkModeOptimizationMismatch=linkModeOptimizationMismatch, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, receiveDataRate=receiveDataRate, syslogStateTrap=syslogStateTrap, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, telecomsChannelStatusTrap=telecomsChannelStatusTrap, sNMPPortNumber=sNMPPortNumber, dfs=dfs, tFTPServerPortNumber=tFTPServerPortNumber, transmitDataRate=transmitDataRate, resetGroup=resetGroup, transmitChannel=transmitChannel, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, userTableIndex=userTableIndex, licensedCapacity=licensedCapacity, sNTPState=sNTPState, sNTPSyncTrap=sNTPSyncTrap, productVariant=productVariant, vLANManagementVID=vLANManagementVID, snmpControl=snmpControl, userTable=userTable, supplementaryGroup=supplementaryGroup, range=range, vLANPriorityTableEntry=vLANPriorityTableEntry, sNMPTrapTableIndex=sNMPTrapTableIndex, dataPortConfigurationMismatch=dataPortConfigurationMismatch, linkLoss=linkLoss, telnetPortNumber=telnetPortNumber, iPAddress=iPAddress, constantPowerSpectralDensity=constantPowerSpectralDensity, bandwidthVariant=bandwidthVariant, installArmStateTrap=installArmStateTrap, dfsTable=dfsTable, gatewayIPAddress=gatewayIPAddress, configurationGroup=configurationGroup, channelChangeTrap=channelChangeTrap, timeZone=timeZone, linkModeOptimisation=linkModeOptimisation, syslogControlGroup=syslogControlGroup, userTableEntry=userTableEntry, ptpTraps=ptpTraps, ptpCompliance=ptpCompliance, licenceGroup=licenceGroup, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, phyControl=phyControl, systemClock=systemClock, accessMethod=accessMethod, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, encryptionGroup=encryptionGroup, vLANPriorityTableNumber=vLANPriorityTableNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, userName=userName, receivePower=receivePower, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, sNTPServerPortNumber=sNTPServerPortNumber, bootVersion=bootVersion, regionCode=regionCode, configuration=configuration, vLANPriorityQueueMapping=vLANPriorityQueueMapping, wirelessLinkStatus=wirelessLinkStatus, PYSNMP_MODULE_ID=cambium, userEnabled=userEnabled, management=management, eIRP=eIRP, remoteMaximumTransmitPower=remoteMaximumTransmitPower, vectorError=vectorError, telecomsLoopback=telecomsLoopback, encryptionAlgorithm=encryptionAlgorithm, telecom=telecom, sMTPSourceEmailAddress=sMTPSourceEmailAddress, dataPortStatus=dataPortStatus, userTableNumber=userTableNumber, daylightSaving=daylightSaving, dfsPeaks=dfsPeaks, sNMPv3Enable=sNMPv3Enable, cambium=cambium, channelBandwidth=channelBandwidth, dfsMeans=dfsMeans, hTTPSAccessEnabled=hTTPSAccessEnabled, incompatibleMasterAndSlave=incompatibleMasterAndSlave, receiveModulationMode=receiveModulationMode, remoteIPAddress=remoteIPAddress, transmitFreqKHz=transmitFreqKHz, sMTPServerPortNumber=sMTPServerPortNumber, dataPortWirelessDownAlert=dataPortWirelessDownAlert, dfsTableIndex=dfsTableIndex, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, sNMPTrapIPAddress=sNMPTrapIPAddress, hTTPPortNumber=hTTPPortNumber, incompatibleRegionCodes=incompatibleRegionCodes, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, targetMACAddress=targetMACAddress, antennaGain=antennaGain, receiveFreqMHz=receiveFreqMHz, sMTPEnabledMessages=sMTPEnabledMessages, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNMPTrapTable=sNMPTrapTable, dfsTableNumber=dfsTableNumber, longitude=longitude, dataPortAutoNegotiation=dataPortAutoNegotiation, ethernetFiberSupport=ethernetFiberSupport, reset=reset, sNMPEnabledTraps=sNMPEnabledTraps, ptpGroups=ptpGroups, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, receiveFreqKHz=receiveFreqKHz, encryption=encryption, installArmState=installArmState, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, vLANPriorityTable=vLANPriorityTable)
