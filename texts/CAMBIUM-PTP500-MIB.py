...
# Produced by pysmi-1.1.3 at Sun Nov 28 21:14:36 2021
...
Integer32, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, enterprises, Counter64, Bits, ModuleIdentity, MibIdentifier, IpAddress, NotificationType, Unsigned32, ObjectIdentity, TimeTicks, iso = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "enterprises", "Counter64", "Bits", "ModuleIdentity", "MibIdentifier", "IpAddress", "NotificationType", "Unsigned32", "ObjectIdentity", "TimeTicks", "iso")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", dataPortConfigurationMismatch=dataPortConfigurationMismatch, pubStatsGroup=pubStatsGroup, vLANPriorityTableNumber=vLANPriorityTableNumber, telecomsCableLength=telecomsCableLength, systemReset=systemReset, telnetAccessEnabled=telnetAccessEnabled, signalStrengthRatio=signalStrengthRatio, vLANManagementPriority=vLANManagementPriority, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, transmitDataRate=transmitDataRate, eIRP=eIRP, byteErrorRatio=byteErrorRatio, masterSlaveMode=masterSlaveMode, transmitPower=transmitPower, transmitFreqMHz=transmitFreqMHz, licence=licence, sNMPTrapIPAddress=sNMPTrapIPAddress, sNTPSyncTrap=sNTPSyncTrap, syslogControl=syslogControl, tFTPServerPortNumber=tFTPServerPortNumber, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, management=management, telecomsChannelStatus=telecomsChannelStatus, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, incompatibleMasterAndSlave=incompatibleMasterAndSlave, snmpControl=snmpControl, licensedCapacity=licensedCapacity, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sNMPCommunityString=sNMPCommunityString, dfsPeaks=dfsPeaks, sNTPSync=sNTPSync, dfsTableIndex=dfsTableIndex, smtp=smtp, capacityLicenseMismatch=capacityLicenseMismatch, tDDControlGroup=tDDControlGroup, dataPortStatus=dataPortStatus, tDDSynchronizationStatus=tDDSynchronizationStatus, subnetMask=subnetMask, snmpControlGroup=snmpControlGroup, dfsTable=dfsTable, softwareVersion=softwareVersion, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, range=range, userTable=userTable, receiveFreqKHz=receiveFreqKHz, sNTPState=sNTPState, receiveModulationModeDetail=receiveModulationModeDetail, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, noWirelessChannelAvailable=noWirelessChannelAvailable, productName=productName, linkSymmetry=linkSymmetry, sNMPTrapPortNumber=sNMPTrapPortNumber, ptpGroups=ptpGroups, versions=versions, receivePower=receivePower, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sntp=sntp, transmitChannel=transmitChannel, altitude=altitude, PYSNMP_MODULE_ID=cambium, gatewayIPAddress=gatewayIPAddress, ethernetGroup=ethernetGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, dataPortAutoMdix=dataPortAutoMdix, latitude=latitude, dfsTableNumber=dfsTableNumber, ethernetFiberSupport=ethernetFiberSupport, sMTPServerIPAddress=sMTPServerIPAddress, sNMPTrapVersion=sNMPTrapVersion, ptp500=ptp500, vLANPriorityTableIndex=vLANPriorityTableIndex, iPAddress=iPAddress, ptmp=ptmp, resetGroup=resetGroup, userName=userName, installArmState=installArmState, productVariant=productVariant, syslogLocalWrappedTrap=syslogLocalWrappedTrap, targetMACAddress=targetMACAddress, ethernet=ethernet, constantPowerSpectralDensity=constantPowerSpectralDensity, dataPortStatusTrap=dataPortStatusTrap, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, incompatibleRegionCodes=incompatibleRegionCodes, sNMPPortNumber=sNMPPortNumber, encryption=encryption, maximumTransmitPower=maximumTransmitPower, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, receiveChannel=receiveChannel, regionCodeTrap=regionCodeTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, installStatusTrap=installStatusTrap, dataPortDisabledWarning=dataPortDisabledWarning, wirelessLinkAvailability=wirelessLinkAvailability, useVLANForManagementInterfaces=useVLANForManagementInterfaces, telecomsInterface=telecomsInterface, accessMethod=accessMethod, tDDSynchronizationMode=tDDSynchronizationMode, sNMPTrapTableEntry=sNMPTrapTableEntry, transmitModulationMode=transmitModulationMode, secondarySoftwareVersion=secondarySoftwareVersion, syslogState=syslogState, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, pubStats=pubStats, aAAControlGroup=aAAControlGroup, sMTPEnabledMessages=sMTPEnabledMessages, syslogStateTrap=syslogStateTrap, userTableNumber=userTableNumber, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, sNMPv3Enable=sNMPv3Enable, sntpGroup=sntpGroup, remoteIPAddress=remoteIPAddress, userRole=userRole, dataPortAutoNegotiation=dataPortAutoNegotiation, bandwidthVariant=bandwidthVariant, hTTPSAccessEnabled=hTTPSAccessEnabled, sNTPServerPortNumber=sNTPServerPortNumber, supplementaryGroup=supplementaryGroup, sMTPEmailAlert=sMTPEmailAlert, hTTPAccessEnabled=hTTPAccessEnabled, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, encryptionGroup=encryptionGroup, receiveModulationMode=receiveModulationMode, syslogClient=syslogClient, telecomsChannelStatusTrap=telecomsChannelStatusTrap, ptpTraps=ptpTraps, dfsGroup=dfsGroup, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sNTPPollInterval=sNTPPollInterval, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNTPLastSync=sNTPLastSync, vLANPriorityTable=vLANPriorityTable, phyControlGroup=phyControlGroup, dfsMeans=dfsMeans, vectorError=vectorError, alarms=alarms, hTTPSPortNumber=hTTPSPortNumber, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, dfsTableEntry=dfsTableEntry, linkLoss=linkLoss, receiveFreqMHz=receiveFreqMHz, vLANManagementVID=vLANManagementVID, sNTPServerIPAddress=sNTPServerIPAddress, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, timeZone=timeZone, versionsGroup=versionsGroup, channelChangeTrap=channelChangeTrap, telecomsLineCode=telecomsLineCode, regionCode=regionCode, syslogClientTrap=syslogClientTrap, linkName=linkName, supplementary=supplementary, telnetPortNumber=telnetPortNumber, receiveDataRate=receiveDataRate, telecomGroup=telecomGroup, licenceGroup=licenceGroup, telecom=telecom, antennaGain=antennaGain, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, vLANPriorityTableEntry=vLANPriorityTableEntry, telecomsLoopback=telecomsLoopback, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, syslogControlGroup=syslogControlGroup, tDDSynchronizationAlarm=tDDSynchronizationAlarm, ptp=ptp, aAAControl=aAAControl, systemClock=systemClock, tFTPServerIPAddress=tFTPServerIPAddress, daylightSaving=daylightSaving, userTableEntry=userTableEntry, userTableIndex=userTableIndex, phyControl=phyControl, bootVersion=bootVersion, userPassword=userPassword, linkModeOptimisation=linkModeOptimisation, groupID=groupID, hTTPPortNumber=hTTPPortNumber, wirelessLinkStatus=wirelessLinkStatus, configuration=configuration, reset=reset, userEnabled=userEnabled, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dfs=dfs, installArmStateTrap=installArmStateTrap, ptpCompliance=ptpCompliance, frequencyVariant=frequencyVariant, phyStatus=phyStatus, ptpTrapPrefix=ptpTrapPrefix, sNMPTrapTable=sNMPTrapTable, hardwareVersion=hardwareVersion, managementGroup=managementGroup, siteName=siteName, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNMPEnabledTraps=sNMPEnabledTraps, encryptionAlgorithm=encryptionAlgorithm, longitude=longitude, dataPortWirelessDownAlert=dataPortWirelessDownAlert, remoteMaximumTransmitPower=remoteMaximumTransmitPower, searchState=searchState, phyStatusGroup=phyStatusGroup, telecomsLoopbackTrap=telecomsLoopbackTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, smtpGroup=smtpGroup, unitOutOfCalibration=unitOutOfCalibration, sNMPTrapTableNumber=sNMPTrapTableNumber, configurationGroup=configurationGroup, transmitFreqKHz=transmitFreqKHz, tDDControl=tDDControl, sMTPServerPortNumber=sMTPServerPortNumber, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, alarmsGroup=alarmsGroup, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, channelBandwidth=channelBandwidth, aggregateDataRate=aggregateDataRate, installStatus=installStatus, notificationsGroup=notificationsGroup, cableLoss=cableLoss, cambium=cambium)
