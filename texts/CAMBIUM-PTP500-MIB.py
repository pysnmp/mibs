...
# Produced by pysmi-1.1.8 at Fri Jan 27 15:41:37 2023
...
Bits, Counter64, Unsigned32, enterprises, MibIdentifier, ObjectIdentity, ModuleIdentity, Integer32, iso, IpAddress, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, NotificationType, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Counter64", "Unsigned32", "enterprises", "MibIdentifier", "ObjectIdentity", "ModuleIdentity", "Integer32", "iso", "IpAddress", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "NotificationType", "Counter32")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", receivePower=receivePower, resetGroup=resetGroup, ptpCompliance=ptpCompliance, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, management=management, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPTrapVersion=sNMPTrapVersion, vLANManagementPriority=vLANManagementPriority, receiveFreqMHz=receiveFreqMHz, versions=versions, reset=reset, tDDSynchronizationAlarm=tDDSynchronizationAlarm, userName=userName, telnetPortNumber=telnetPortNumber, aggregateDataRate=aggregateDataRate, dataPortDisabledWarning=dataPortDisabledWarning, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, productVariant=productVariant, PYSNMP_MODULE_ID=cambium, installArmState=installArmState, syslogControl=syslogControl, transmitPower=transmitPower, tDDSynchronizationMode=tDDSynchronizationMode, sNTPState=sNTPState, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, channelBandwidth=channelBandwidth, ethernetGroup=ethernetGroup, maximumTransmitPower=maximumTransmitPower, sNTPServerPortNumber=sNTPServerPortNumber, sNTPLastSync=sNTPLastSync, syslogControlGroup=syslogControlGroup, dfsTableNumber=dfsTableNumber, remoteMaximumTransmitPower=remoteMaximumTransmitPower, supplementary=supplementary, dataPortConfigurationMismatch=dataPortConfigurationMismatch, useVLANForManagementInterfaces=useVLANForManagementInterfaces, licenceGroup=licenceGroup, ethernetFiberSupport=ethernetFiberSupport, telecomGroup=telecomGroup, tFTPServerIPAddress=tFTPServerIPAddress, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, systemClock=systemClock, phyControlGroup=phyControlGroup, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, configurationGroup=configurationGroup, vectorError=vectorError, dataPortWirelessDownAlert=dataPortWirelessDownAlert, sNTPPollInterval=sNTPPollInterval, systemReset=systemReset, dfs=dfs, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, secondarySoftwareVersion=secondarySoftwareVersion, syslogClient=syslogClient, dfsPeaks=dfsPeaks, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, sNMPPortNumber=sNMPPortNumber, ptp500=ptp500, telecomsInterface=telecomsInterface, syslogClientTrap=syslogClientTrap, antennaGain=antennaGain, licence=licence, telecomsLineCode=telecomsLineCode, configuration=configuration, sNMPTrapTableEntry=sNMPTrapTableEntry, gatewayIPAddress=gatewayIPAddress, supplementaryGroup=supplementaryGroup, installStatus=installStatus, receiveFreqKHz=receiveFreqKHz, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNMPTrapPortNumber=sNMPTrapPortNumber, sNMPTrapIPAddress=sNMPTrapIPAddress, receiveChannel=receiveChannel, sNMPTrapTableNumber=sNMPTrapTableNumber, dfsTable=dfsTable, smtpGroup=smtpGroup, sntpGroup=sntpGroup, timeZone=timeZone, sNTPSyncTrap=sNTPSyncTrap, capacityLicenseMismatch=capacityLicenseMismatch, smtp=smtp, vLANPriorityTable=vLANPriorityTable, encryptionGroup=encryptionGroup, syslogState=syslogState, sMTPSourceEmailAddress=sMTPSourceEmailAddress, aAAControl=aAAControl, vLANPriorityTableNumber=vLANPriorityTableNumber, range=range, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, notificationsGroup=notificationsGroup, linkSymmetry=linkSymmetry, transmitChannel=transmitChannel, snmpControl=snmpControl, dfsTableEntry=dfsTableEntry, userTableNumber=userTableNumber, licensedCapacity=licensedCapacity, longitude=longitude, transmitModulationMode=transmitModulationMode, unitOutOfCalibration=unitOutOfCalibration, bootVersion=bootVersion, tDDSynchronizationStatus=tDDSynchronizationStatus, cableLoss=cableLoss, targetMACAddress=targetMACAddress, productName=productName, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, linkName=linkName, aAAControlGroup=aAAControlGroup, userTableIndex=userTableIndex, userRole=userRole, ptpTraps=ptpTraps, phyControl=phyControl, ethernet=ethernet, searchState=searchState, transmitFreqKHz=transmitFreqKHz, userEnabled=userEnabled, pubStatsGroup=pubStatsGroup, vLANManagementVID=vLANManagementVID, eIRP=eIRP, tDDControl=tDDControl, wirelessLinkStatus=wirelessLinkStatus, incompatibleMasterAndSlave=incompatibleMasterAndSlave, bandwidthVariant=bandwidthVariant, dataPortAutoNegotiation=dataPortAutoNegotiation, receiveModulationModeDetail=receiveModulationModeDetail, sMTPEmailAlert=sMTPEmailAlert, installStatusTrap=installStatusTrap, hTTPAccessEnabled=hTTPAccessEnabled, snmpControlGroup=snmpControlGroup, ptpGroups=ptpGroups, linkModeOptimisation=linkModeOptimisation, iPAddress=iPAddress, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, dataPortStatus=dataPortStatus, installArmStateTrap=installArmStateTrap, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, accessMethod=accessMethod, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, userTable=userTable, noWirelessChannelAvailable=noWirelessChannelAvailable, regionCodeTrap=regionCodeTrap, telecomsLoopbackTrap=telecomsLoopbackTrap, hTTPPortNumber=hTTPPortNumber, sNMPEnabledTraps=sNMPEnabledTraps, latitude=latitude, dfsGroup=dfsGroup, telecomsLoopback=telecomsLoopback, pubStats=pubStats, telecomsChannelStatusTrap=telecomsChannelStatusTrap, sNTPSync=sNTPSync, telecom=telecom, enabledDiagnosticAlarms=enabledDiagnosticAlarms, remoteIPAddress=remoteIPAddress, channelChangeTrap=channelChangeTrap, sNMPCommunityString=sNMPCommunityString, regionCode=regionCode, sMTPEnabledMessages=sMTPEnabledMessages, sNMPTrapTable=sNMPTrapTable, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, daylightSaving=daylightSaving, sNMPTrapTableIndex=sNMPTrapTableIndex, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, telnetAccessEnabled=telnetAccessEnabled, dfsMeans=dfsMeans, hTTPSAccessEnabled=hTTPSAccessEnabled, groupID=groupID, dataPortStatusTrap=dataPortStatusTrap, siteName=siteName, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, ptmp=ptmp, phyStatus=phyStatus, cambium=cambium, dataPortAutoMdix=dataPortAutoMdix, constantPowerSpectralDensity=constantPowerSpectralDensity, userTableEntry=userTableEntry, transmitFreqMHz=transmitFreqMHz, sNTPServerIPAddress=sNTPServerIPAddress, subnetMask=subnetMask, encryption=encryption, linkLoss=linkLoss, sntp=sntp, tFTPServerPortNumber=tFTPServerPortNumber, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, receiveModulationMode=receiveModulationMode, softwareVersion=softwareVersion, wirelessLinkAvailability=wirelessLinkAvailability, alarmsGroup=alarmsGroup, transmitDataRate=transmitDataRate, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, telecomsCableLength=telecomsCableLength, sMTPServerPortNumber=sMTPServerPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, vLANPriorityQueueMapping=vLANPriorityQueueMapping, tDDControlGroup=tDDControlGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, signalStrengthRatio=signalStrengthRatio, dfsTableIndex=dfsTableIndex, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, phyStatusGroup=phyStatusGroup, vLANPriorityTableEntry=vLANPriorityTableEntry, sMTPServerIPAddress=sMTPServerIPAddress, byteErrorRatio=byteErrorRatio, vLANPriorityTableIndex=vLANPriorityTableIndex, encryptionAlgorithm=encryptionAlgorithm, frequencyVariant=frequencyVariant, sNMPv3Enable=sNMPv3Enable, hardwareVersion=hardwareVersion, userPassword=userPassword, altitude=altitude, incompatibleRegionCodes=incompatibleRegionCodes, versionsGroup=versionsGroup, masterSlaveMode=masterSlaveMode, receiveDataRate=receiveDataRate, alarms=alarms, ptpTrapPrefix=ptpTrapPrefix, managementGroup=managementGroup, hTTPSPortNumber=hTTPSPortNumber, telecomsChannelStatus=telecomsChannelStatus, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, ptp=ptp, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, syslogStateTrap=syslogStateTrap)
