...
# Produced by pysmi-1.1.3 at Sun Nov 28 20:17:50 2021
...
iso, Integer32, Counter64, Unsigned32, NotificationType, TimeTicks, Gauge32, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, ModuleIdentity, MibIdentifier, Bits, Counter32, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Integer32", "Counter64", "Unsigned32", "NotificationType", "TimeTicks", "Gauge32", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "ModuleIdentity", "MibIdentifier", "Bits", "Counter32", "ObjectIdentity")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", receiveFreqMHz=receiveFreqMHz, sMTPEnabledMessages=sMTPEnabledMessages, phyControl=phyControl, receivePower=receivePower, ptp500=ptp500, transmitDataRate=transmitDataRate, sNMPTrapTableNumber=sNMPTrapTableNumber, dataPortAutoMdix=dataPortAutoMdix, telecomsInterface=telecomsInterface, syslogClientTrap=syslogClientTrap, signalStrengthRatio=signalStrengthRatio, remoteMaximumTransmitPower=remoteMaximumTransmitPower, cableLoss=cableLoss, ptmp=ptmp, receiveFreqKHz=receiveFreqKHz, telecom=telecom, systemReset=systemReset, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, antennaGain=antennaGain, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, dfs=dfs, resetGroup=resetGroup, remoteIPAddress=remoteIPAddress, subnetMask=subnetMask, sNMPTrapTable=sNMPTrapTable, sMTPEmailAlert=sMTPEmailAlert, supplementaryGroup=supplementaryGroup, hTTPPortNumber=hTTPPortNumber, telnetPortNumber=telnetPortNumber, capacityLicenseMismatch=capacityLicenseMismatch, installArmStateTrap=installArmStateTrap, sntpGroup=sntpGroup, regionCode=regionCode, tDDControl=tDDControl, dataPortWirelessDownAlert=dataPortWirelessDownAlert, tFTPServerPortNumber=tFTPServerPortNumber, vLANPriorityTableIndex=vLANPriorityTableIndex, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dataPortStatusTrap=dataPortStatusTrap, frequencyVariant=frequencyVariant, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, tDDSynchronizationStatus=tDDSynchronizationStatus, constantPowerSpectralDensity=constantPowerSpectralDensity, dataPortAutoNegotiation=dataPortAutoNegotiation, hardwareVersion=hardwareVersion, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, syslogControlGroup=syslogControlGroup, receiveModulationMode=receiveModulationMode, sNTPState=sNTPState, snmpControl=snmpControl, userTable=userTable, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, latitude=latitude, transmitFreqMHz=transmitFreqMHz, dfsGroup=dfsGroup, dataPortStatus=dataPortStatus, maximumTransmitPower=maximumTransmitPower, configuration=configuration, vLANManagementPriority=vLANManagementPriority, sNTPPollInterval=sNTPPollInterval, telecomsLoopback=telecomsLoopback, installStatus=installStatus, PYSNMP_MODULE_ID=cambium, ptpGroups=ptpGroups, userPassword=userPassword, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, ptpCompliance=ptpCompliance, dfsTable=dfsTable, smtpGroup=smtpGroup, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, encryption=encryption, telecomsLoopbackTrap=telecomsLoopbackTrap, configurationGroup=configurationGroup, managementGroup=managementGroup, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, transmitFreqKHz=transmitFreqKHz, ethernet=ethernet, sNMPTrapTableEntry=sNMPTrapTableEntry, licence=licence, transmitModulationMode=transmitModulationMode, userRole=userRole, sMTPServerIPAddress=sMTPServerIPAddress, userTableNumber=userTableNumber, ethernetFiberSupport=ethernetFiberSupport, versionsGroup=versionsGroup, useVLANForManagementInterfaces=useVLANForManagementInterfaces, encryptionAlgorithm=encryptionAlgorithm, searchState=searchState, dfsPeaks=dfsPeaks, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, incompatibleMasterAndSlave=incompatibleMasterAndSlave, licenceGroup=licenceGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, vLANPriorityTableNumber=vLANPriorityTableNumber, wirelessLinkAvailability=wirelessLinkAvailability, byteErrorRatio=byteErrorRatio, installArmState=installArmState, receiveDataRate=receiveDataRate, daylightSaving=daylightSaving, sNMPTrapVersion=sNMPTrapVersion, channelChangeTrap=channelChangeTrap, dataPortDisabledWarning=dataPortDisabledWarning, snmpControlGroup=snmpControlGroup, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sMTPServerPortNumber=sMTPServerPortNumber, vLANPriorityTableEntry=vLANPriorityTableEntry, hTTPAccessEnabled=hTTPAccessEnabled, vLANManagementVID=vLANManagementVID, enabledDiagnosticAlarms=enabledDiagnosticAlarms, syslogStateTrap=syslogStateTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, sNMPPortNumber=sNMPPortNumber, smtp=smtp, receiveChannel=receiveChannel, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, receiveModulationModeDetail=receiveModulationModeDetail, sNTPSync=sNTPSync, altitude=altitude, aAAControlGroup=aAAControlGroup, cambium=cambium, productVariant=productVariant, syslogControl=syslogControl, alarms=alarms, encryptionGroup=encryptionGroup, dfsTableIndex=dfsTableIndex, eIRP=eIRP, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPTrapIPAddress=sNMPTrapIPAddress, telecomsLineCode=telecomsLineCode, transmitPower=transmitPower, incompatibleRegionCodes=incompatibleRegionCodes, ptpTrapPrefix=ptpTrapPrefix, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, productName=productName, userName=userName, installStatusTrap=installStatusTrap, bandwidthVariant=bandwidthVariant, ptpTraps=ptpTraps, telecomGroup=telecomGroup, sNMPCommunityString=sNMPCommunityString, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, syslogState=syslogState, notificationsGroup=notificationsGroup, hTTPSAccessEnabled=hTTPSAccessEnabled, bootVersion=bootVersion, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, systemClock=systemClock, vectorError=vectorError, sNMPEnabledTraps=sNMPEnabledTraps, syslogClient=syslogClient, telecomsCableLength=telecomsCableLength, sNTPLastSync=sNTPLastSync, linkModeOptimisation=linkModeOptimisation, groupID=groupID, sNMPv3Enable=sNMPv3Enable, channelBandwidth=channelBandwidth, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, userEnabled=userEnabled, versions=versions, phyStatusGroup=phyStatusGroup, softwareVersion=softwareVersion, userTableEntry=userTableEntry, sntp=sntp, ptp=ptp, telecomsChannelStatus=telecomsChannelStatus, dfsMeans=dfsMeans, dfsTableEntry=dfsTableEntry, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, linkLoss=linkLoss, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, pubStatsGroup=pubStatsGroup, targetMACAddress=targetMACAddress, tDDSynchronizationMode=tDDSynchronizationMode, timeZone=timeZone, telecomsChannelStatusTrap=telecomsChannelStatusTrap, ethernetGroup=ethernetGroup, tDDControlGroup=tDDControlGroup, transmitChannel=transmitChannel, licensedCapacity=licensedCapacity, siteName=siteName, linkSymmetry=linkSymmetry, unitOutOfCalibration=unitOutOfCalibration, dfsTableNumber=dfsTableNumber, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, aggregateDataRate=aggregateDataRate, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, linkName=linkName, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, sNTPServerIPAddress=sNTPServerIPAddress, regionCodeTrap=regionCodeTrap, sNTPServerPortNumber=sNTPServerPortNumber, iPAddress=iPAddress, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, aAAControl=aAAControl, reset=reset, management=management, vLANPriorityTable=vLANPriorityTable, secondarySoftwareVersion=secondarySoftwareVersion, phyControlGroup=phyControlGroup, vLANPriorityQueueMapping=vLANPriorityQueueMapping, telnetAccessEnabled=telnetAccessEnabled, wirelessLinkStatus=wirelessLinkStatus, phyStatus=phyStatus, tDDSynchronizationAlarm=tDDSynchronizationAlarm, supplementary=supplementary, longitude=longitude, sNTPSyncTrap=sNTPSyncTrap, sMTPSourceEmailAddress=sMTPSourceEmailAddress, linkModeOptimizationMismatch=linkModeOptimizationMismatch, accessMethod=accessMethod, range=range, pubStats=pubStats, masterSlaveMode=masterSlaveMode, gatewayIPAddress=gatewayIPAddress, alarmsGroup=alarmsGroup, userTableIndex=userTableIndex, sNMPTrapTableIndex=sNMPTrapTableIndex, hTTPSPortNumber=hTTPSPortNumber, tFTPServerIPAddress=tFTPServerIPAddress)
