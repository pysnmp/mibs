...
# Produced by pysmi-1.1.3 at Wed Dec  8 19:05:40 2021
...
NotificationType, TimeTicks, ModuleIdentity, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, Unsigned32, iso, Bits, Counter64, MibIdentifier, ObjectIdentity, IpAddress, Integer32, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "TimeTicks", "ModuleIdentity", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "Unsigned32", "iso", "Bits", "Counter64", "MibIdentifier", "ObjectIdentity", "IpAddress", "Integer32", "Counter32")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, hardwareVersion=hardwareVersion, encryptionAlgorithm=encryptionAlgorithm, hTTPSPortNumber=hTTPSPortNumber, wirelessLinkAvailability=wirelessLinkAvailability, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, dfsPeaks=dfsPeaks, noWirelessChannelAvailable=noWirelessChannelAvailable, channelBandwidth=channelBandwidth, managementGroup=managementGroup, phyStatus=phyStatus, syslogControlGroup=syslogControlGroup, iPAddress=iPAddress, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, tDDSynchronizationMode=tDDSynchronizationMode, sNMPPortNumber=sNMPPortNumber, userName=userName, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, receiveFreqKHz=receiveFreqKHz, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, sNMPCommunityString=sNMPCommunityString, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, aggregateDataRate=aggregateDataRate, vLANPriorityQueueMapping=vLANPriorityQueueMapping, telecom=telecom, subnetMask=subnetMask, management=management, receiveChannel=receiveChannel, versionsGroup=versionsGroup, longitude=longitude, PYSNMP_MODULE_ID=cambium, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, vectorError=vectorError, telecomsInterface=telecomsInterface, pubStats=pubStats, notificationsGroup=notificationsGroup, productName=productName, ethernetFiberSupport=ethernetFiberSupport, sNTPServerIPAddress=sNTPServerIPAddress, phyControlGroup=phyControlGroup, transmitModulationMode=transmitModulationMode, licensedCapacity=licensedCapacity, dfsTableIndex=dfsTableIndex, sMTPServerIPAddress=sMTPServerIPAddress, linkModeOptimisation=linkModeOptimisation, ptpCompliance=ptpCompliance, snmpControlGroup=snmpControlGroup, aAAControlGroup=aAAControlGroup, dataPortAutoMdix=dataPortAutoMdix, hTTPSAccessEnabled=hTTPSAccessEnabled, sNMPTrapVersion=sNMPTrapVersion, receiveFreqMHz=receiveFreqMHz, linkLoss=linkLoss, targetMACAddress=targetMACAddress, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, cableLoss=cableLoss, aAAControl=aAAControl, phyControl=phyControl, telecomGroup=telecomGroup, encryptionGroup=encryptionGroup, linkSymmetry=linkSymmetry, ethernetGroup=ethernetGroup, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, ptmp=ptmp, userRole=userRole, sNMPTrapTableIndex=sNMPTrapTableIndex, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, sntp=sntp, sNMPTrapTableNumber=sNMPTrapTableNumber, receiveModulationMode=receiveModulationMode, sNMPTrapTable=sNMPTrapTable, ptp500=ptp500, sNMPEnabledTraps=sNMPEnabledTraps, receivePower=receivePower, wirelessLinkStatus=wirelessLinkStatus, siteName=siteName, transmitDataRate=transmitDataRate, dataPortAutoNegotiation=dataPortAutoNegotiation, ptpTrapPrefix=ptpTrapPrefix, receiveDataRate=receiveDataRate, searchState=searchState, sNTPPollInterval=sNTPPollInterval, userEnabled=userEnabled, dataPortStatusTrap=dataPortStatusTrap, installArmStateTrap=installArmStateTrap, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sntpGroup=sntpGroup, tDDControl=tDDControl, sNMPv3Enable=sNMPv3Enable, phyStatusGroup=phyStatusGroup, accessMethod=accessMethod, dfs=dfs, dfsGroup=dfsGroup, eIRP=eIRP, smtp=smtp, groupID=groupID, dataPortWirelessDownAlert=dataPortWirelessDownAlert, syslogStateTrap=syslogStateTrap, vLANManagementPriority=vLANManagementPriority, dfsTableNumber=dfsTableNumber, range=range, ptpGroups=ptpGroups, dfsTableEntry=dfsTableEntry, sNTPSync=sNTPSync, installStatusTrap=installStatusTrap, telecomsLoopbackTrap=telecomsLoopbackTrap, telecomsLineCode=telecomsLineCode, resetGroup=resetGroup, ptp=ptp, telnetAccessEnabled=telnetAccessEnabled, unitOutOfCalibration=unitOutOfCalibration, capacityLicenseMismatch=capacityLicenseMismatch, reset=reset, enabledDiagnosticAlarms=enabledDiagnosticAlarms, installArmState=installArmState, telecomsChannelStatus=telecomsChannelStatus, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, tDDControlGroup=tDDControlGroup, sNTPSyncTrap=sNTPSyncTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, sNTPLastSync=sNTPLastSync, tFTPServerPortNumber=tFTPServerPortNumber, transmitFreqKHz=transmitFreqKHz, remoteIPAddress=remoteIPAddress, tDDSynchronizationStatus=tDDSynchronizationStatus, maximumTransmitPower=maximumTransmitPower, sMTPSourceEmailAddress=sMTPSourceEmailAddress, latitude=latitude, sMTPEmailAlert=sMTPEmailAlert, syslogState=syslogState, transmitFreqMHz=transmitFreqMHz, syslogControl=syslogControl, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, incompatibleRegionCodes=incompatibleRegionCodes, dataPortDisabledWarning=dataPortDisabledWarning, telecomsCableLength=telecomsCableLength, sNMPTrapIPAddress=sNMPTrapIPAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, cambium=cambium, userTableEntry=userTableEntry, sMTPEnabledMessages=sMTPEnabledMessages, configuration=configuration, sNMPTrapTableEntry=sNMPTrapTableEntry, signalStrengthRatio=signalStrengthRatio, regionCodeTrap=regionCodeTrap, userTable=userTable, telecomsLoopback=telecomsLoopback, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, vLANPriorityTableEntry=vLANPriorityTableEntry, incompatibleMasterAndSlave=incompatibleMasterAndSlave, timeZone=timeZone, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, ptpTraps=ptpTraps, softwareVersion=softwareVersion, userPassword=userPassword, vLANPriorityTable=vLANPriorityTable, productVariant=productVariant, sMTPServerPortNumber=sMTPServerPortNumber, systemClock=systemClock, altitude=altitude, receiveModulationModeDetail=receiveModulationModeDetail, channelChangeTrap=channelChangeTrap, telecomsChannelStatusTrap=telecomsChannelStatusTrap, useVLANForManagementInterfaces=useVLANForManagementInterfaces, configurationGroup=configurationGroup, frequencyVariant=frequencyVariant, userTableNumber=userTableNumber, linkModeOptimizationMismatch=linkModeOptimizationMismatch, transmitPower=transmitPower, secondarySoftwareVersion=secondarySoftwareVersion, syslogLocalWrappedTrap=syslogLocalWrappedTrap, smtpGroup=smtpGroup, tFTPServerIPAddress=tFTPServerIPAddress, gatewayIPAddress=gatewayIPAddress, daylightSaving=daylightSaving, installStatus=installStatus, constantPowerSpectralDensity=constantPowerSpectralDensity, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, dataPortConfigurationMismatch=dataPortConfigurationMismatch, ethernet=ethernet, regionCode=regionCode, linkName=linkName, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, versions=versions, snmpControl=snmpControl, vLANManagementVID=vLANManagementVID, hTTPPortNumber=hTTPPortNumber, tDDSynchronizationAlarm=tDDSynchronizationAlarm, bootVersion=bootVersion, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, encryption=encryption, supplementary=supplementary, bandwidthVariant=bandwidthVariant, masterSlaveMode=masterSlaveMode, systemReset=systemReset, dataPortStatus=dataPortStatus, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, byteErrorRatio=byteErrorRatio, alarmsGroup=alarmsGroup, dfsMeans=dfsMeans, supplementaryGroup=supplementaryGroup, transmitChannel=transmitChannel, licenceGroup=licenceGroup, telnetPortNumber=telnetPortNumber, vLANPriorityTableNumber=vLANPriorityTableNumber, userTableIndex=userTableIndex, vLANPriorityTableIndex=vLANPriorityTableIndex, sNTPServerPortNumber=sNTPServerPortNumber, dfsTable=dfsTable, syslogClient=syslogClient, pubStatsGroup=pubStatsGroup, syslogClientTrap=syslogClientTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, alarms=alarms, sNTPState=sNTPState, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, hTTPAccessEnabled=hTTPAccessEnabled, antennaGain=antennaGain, licence=licence)
