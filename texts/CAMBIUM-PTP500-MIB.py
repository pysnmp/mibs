...
# Produced by pysmi-1.1.8 at Fri Jan  7 00:57:41 2022
...
iso, enterprises, Counter32, ModuleIdentity, Integer32, Counter64, ObjectIdentity, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, Gauge32, MibIdentifier, Unsigned32, IpAddress, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "enterprises", "Counter32", "ModuleIdentity", "Integer32", "Counter64", "ObjectIdentity", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "Gauge32", "MibIdentifier", "Unsigned32", "IpAddress", "NotificationType")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", encryptionGroup=encryptionGroup, linkSymmetry=linkSymmetry, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, tDDControl=tDDControl, sNMPTrapTable=sNMPTrapTable, sNMPTrapPortNumber=sNMPTrapPortNumber, licenceGroup=licenceGroup, range=range, receiveChannel=receiveChannel, syslogClient=syslogClient, cableLoss=cableLoss, regionCodeTrap=regionCodeTrap, syslogState=syslogState, dfsTable=dfsTable, vectorError=vectorError, vLANPriorityQueueMapping=vLANPriorityQueueMapping, ethernetGroup=ethernetGroup, transmitModulationMode=transmitModulationMode, groupID=groupID, ethernet=ethernet, versionsGroup=versionsGroup, sNTPLastSync=sNTPLastSync, systemClock=systemClock, pubStatsGroup=pubStatsGroup, sNMPEnabledTraps=sNMPEnabledTraps, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sNMPTrapVersion=sNMPTrapVersion, tFTPServerPortNumber=tFTPServerPortNumber, systemReset=systemReset, smtpGroup=smtpGroup, aAAControl=aAAControl, receiveFreqMHz=receiveFreqMHz, transmitDataRate=transmitDataRate, sNMPPortNumber=sNMPPortNumber, receiveModulationModeDetail=receiveModulationModeDetail, syslogControlGroup=syslogControlGroup, syslogStateTrap=syslogStateTrap, longitude=longitude, eIRP=eIRP, dfs=dfs, telecom=telecom, antennaGain=antennaGain, wirelessLinkStatus=wirelessLinkStatus, sntpGroup=sntpGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, tDDControlGroup=tDDControlGroup, gatewayIPAddress=gatewayIPAddress, telecomsLoopback=telecomsLoopback, notificationsGroup=notificationsGroup, supplementaryGroup=supplementaryGroup, telnetPortNumber=telnetPortNumber, transmitPower=transmitPower, reset=reset, sNTPSync=sNTPSync, telecomsLineCode=telecomsLineCode, dataPortStatusTrap=dataPortStatusTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, tDDSynchronizationStatus=tDDSynchronizationStatus, aggregateDataRate=aggregateDataRate, management=management, accessMethod=accessMethod, aAAControlGroup=aAAControlGroup, telecomsInterface=telecomsInterface, dataPortDisabledWarning=dataPortDisabledWarning, noWirelessChannelAvailable=noWirelessChannelAvailable, licence=licence, constantPowerSpectralDensity=constantPowerSpectralDensity, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNTPServerIPAddress=sNTPServerIPAddress, smtp=smtp, daylightSaving=daylightSaving, productName=productName, receivePower=receivePower, productVariant=productVariant, dfsTableIndex=dfsTableIndex, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, capacityLicenseMismatch=capacityLicenseMismatch, telecomsLoopbackTrap=telecomsLoopbackTrap, channelBandwidth=channelBandwidth, dataPortConfigurationMismatch=dataPortConfigurationMismatch, cambium=cambium, iPAddress=iPAddress, vLANManagementVID=vLANManagementVID, sNTPPollInterval=sNTPPollInterval, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, telecomsChannelStatusTrap=telecomsChannelStatusTrap, hTTPSAccessEnabled=hTTPSAccessEnabled, tDDSynchronizationMode=tDDSynchronizationMode, sNMPv3Enable=sNMPv3Enable, vLANManagementPriority=vLANManagementPriority, telnetAccessEnabled=telnetAccessEnabled, versions=versions, sNMPCommunityString=sNMPCommunityString, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, maximumTransmitPower=maximumTransmitPower, userTableNumber=userTableNumber, bootVersion=bootVersion, sNTPServerPortNumber=sNTPServerPortNumber, managementGroup=managementGroup, remoteIPAddress=remoteIPAddress, dfsTableNumber=dfsTableNumber, wirelessLinkAvailability=wirelessLinkAvailability, phyStatusGroup=phyStatusGroup, secondarySoftwareVersion=secondarySoftwareVersion, ptpCompliance=ptpCompliance, ethernetFiberSupport=ethernetFiberSupport, ptpTrapPrefix=ptpTrapPrefix, encryption=encryption, sMTPEnabledMessages=sMTPEnabledMessages, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, linkModeOptimisation=linkModeOptimisation, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, configuration=configuration, linkName=linkName, sNTPSyncTrap=sNTPSyncTrap, softwareVersion=softwareVersion, receiveDataRate=receiveDataRate, timeZone=timeZone, transmitFreqMHz=transmitFreqMHz, installArmState=installArmState, licensedCapacity=licensedCapacity, frequencyVariant=frequencyVariant, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sNTPState=sNTPState, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, enabledDiagnosticAlarms=enabledDiagnosticAlarms, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, pubStats=pubStats, installArmStateTrap=installArmStateTrap, resetGroup=resetGroup, vLANPriorityTable=vLANPriorityTable, receiveFreqKHz=receiveFreqKHz, sntp=sntp, phyControlGroup=phyControlGroup, masterSlaveMode=masterSlaveMode, vLANPriorityTableEntry=vLANPriorityTableEntry, sMTPEmailAlert=sMTPEmailAlert, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, PYSNMP_MODULE_ID=cambium, sMTPServerPortNumber=sMTPServerPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sMTPServerIPAddress=sMTPServerIPAddress, altitude=altitude, dfsTableEntry=dfsTableEntry, subnetMask=subnetMask, sNMPTrapIPAddress=sNMPTrapIPAddress, userRole=userRole, ptp=ptp, dataPortStatus=dataPortStatus, hTTPPortNumber=hTTPPortNumber, hardwareVersion=hardwareVersion, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, useVLANForManagementInterfaces=useVLANForManagementInterfaces, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, hTTPSPortNumber=hTTPSPortNumber, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, ptp500=ptp500, phyControl=phyControl, configurationGroup=configurationGroup, userEnabled=userEnabled, targetMACAddress=targetMACAddress, searchState=searchState, incompatibleMasterAndSlave=incompatibleMasterAndSlave, phyStatus=phyStatus, byteErrorRatio=byteErrorRatio, syslogClientTrap=syslogClientTrap, snmpControlGroup=snmpControlGroup, siteName=siteName, ptpTraps=ptpTraps, sNMPTrapTableIndex=sNMPTrapTableIndex, signalStrengthRatio=signalStrengthRatio, ptmp=ptmp, syslogControl=syslogControl, dataPortAutoMdix=dataPortAutoMdix, transmitFreqKHz=transmitFreqKHz, alarmsGroup=alarmsGroup, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, vLANPriorityTableIndex=vLANPriorityTableIndex, userTable=userTable, linkModeOptimizationMismatch=linkModeOptimizationMismatch, supplementary=supplementary, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, userTableEntry=userTableEntry, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, userPassword=userPassword, hTTPAccessEnabled=hTTPAccessEnabled, syslogLocalWrappedTrap=syslogLocalWrappedTrap, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dfsMeans=dfsMeans, ptpGroups=ptpGroups, regionCode=regionCode, installStatus=installStatus, encryptionAlgorithm=encryptionAlgorithm, snmpControl=snmpControl, alarms=alarms, sNMPTrapTableNumber=sNMPTrapTableNumber, telecomsCableLength=telecomsCableLength, bandwidthVariant=bandwidthVariant, receiveModulationMode=receiveModulationMode, sMTPSourceEmailAddress=sMTPSourceEmailAddress, telecomsChannelStatus=telecomsChannelStatus, dfsGroup=dfsGroup, dataPortWirelessDownAlert=dataPortWirelessDownAlert, userName=userName, tFTPServerIPAddress=tFTPServerIPAddress, linkLoss=linkLoss, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, dfsPeaks=dfsPeaks, channelChangeTrap=channelChangeTrap, telecomGroup=telecomGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, userTableIndex=userTableIndex, transmitChannel=transmitChannel, unitOutOfCalibration=unitOutOfCalibration, latitude=latitude, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, incompatibleRegionCodes=incompatibleRegionCodes, vLANPriorityTableNumber=vLANPriorityTableNumber, installStatusTrap=installStatusTrap)
