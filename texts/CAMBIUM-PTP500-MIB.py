...
# Produced by pysmi-1.1.8 at Thu Jan 13 23:36:46 2022
...
Counter32, ModuleIdentity, TimeTicks, ObjectIdentity, IpAddress, Unsigned32, MibIdentifier, NotificationType, Gauge32, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, Integer32, iso, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "ModuleIdentity", "TimeTicks", "ObjectIdentity", "IpAddress", "Unsigned32", "MibIdentifier", "NotificationType", "Gauge32", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "Integer32", "iso", "enterprises")
...
mibBuilder.exportSymbols("CAMBIUM-PTP500-MIB", dfsTableIndex=dfsTableIndex, signalStrengthRatio=signalStrengthRatio, telecomsLoopbackTrap=telecomsLoopbackTrap, regionCodeTrap=regionCodeTrap, sNMPTrapTable=sNMPTrapTable, systemReset=systemReset, telecomsChannelStatus=telecomsChannelStatus, tDDSynchronizationStatus=tDDSynchronizationStatus, snmpControlGroup=snmpControlGroup, userEnabled=userEnabled, regionCode=regionCode, licence=licence, configuration=configuration, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, ethernetGroup=ethernetGroup, telecomsCableLength=telecomsCableLength, tDDControl=tDDControl, capacityLicenseMismatchTrap=capacityLicenseMismatchTrap, pubStats=pubStats, configurationGroup=configurationGroup, userTableEntry=userTableEntry, useVLANForManagementInterfaces=useVLANForManagementInterfaces, phyControl=phyControl, siteName=siteName, phyStatusGroup=phyStatusGroup, receiveDataRate=receiveDataRate, sNMPPortNumber=sNMPPortNumber, receiveChannel=receiveChannel, vectorError=vectorError, ethernetFiberSupport=ethernetFiberSupport, versionsGroup=versionsGroup, linkSymmetry=linkSymmetry, aAAControlGroup=aAAControlGroup, channelChangeTrap=channelChangeTrap, sNMPTrapTableNumber=sNMPTrapTableNumber, receiveFreqKHz=receiveFreqKHz, sNTPServerIPAddress=sNTPServerIPAddress, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, incompatibleRegionCodes=incompatibleRegionCodes, telnetAccessEnabled=telnetAccessEnabled, transmitModulationMode=transmitModulationMode, masterSlaveMode=masterSlaveMode, eIRP=eIRP, linkModeOptimizationMismatch=linkModeOptimizationMismatch, dataPortAutoMdix=dataPortAutoMdix, ptp500=ptp500, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, receivePower=receivePower, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, range=range, hardwareVersion=hardwareVersion, syslogControlGroup=syslogControlGroup, enabledDiagnosticAlarms=enabledDiagnosticAlarms, hTTPSPortNumber=hTTPSPortNumber, telecomGroup=telecomGroup, vLANPriorityTableEntry=vLANPriorityTableEntry, supplementary=supplementary, telecomsInterface=telecomsInterface, ethernet=ethernet, aggregateDataRate=aggregateDataRate, systemClock=systemClock, telecomsLoopback=telecomsLoopback, targetMACAddress=targetMACAddress, userName=userName, ptp=ptp, maximumTransmitPower=maximumTransmitPower, management=management, bandwidthVariant=bandwidthVariant, userTableNumber=userTableNumber, vLANPriorityQueueMapping=vLANPriorityQueueMapping, sMTPEmailAlert=sMTPEmailAlert, telecomsLineCode=telecomsLineCode, installArmStateTrap=installArmStateTrap, byteErrorRatio=byteErrorRatio, PYSNMP_MODULE_ID=cambium, timeZone=timeZone, phyControlGroup=phyControlGroup, sMTPEnabledMessages=sMTPEnabledMessages, unitOutOfCalibration=unitOutOfCalibration, transmitPower=transmitPower, ptmp=ptmp, snmpControl=snmpControl, userPassword=userPassword, syslogStateTrap=syslogStateTrap, capacityLicenseMismatch=capacityLicenseMismatch, cambium=cambium, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, tFTPServerIPAddress=tFTPServerIPAddress, remoteMaximumTransmitPower=remoteMaximumTransmitPower, dfsTable=dfsTable, linkModeOptimisation=linkModeOptimisation, transmitFreqMHz=transmitFreqMHz, searchState=searchState, sNTPSync=sNTPSync, userTableIndex=userTableIndex, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, sNMPTrapTableEntry=sNMPTrapTableEntry, syslogControl=syslogControl, subnetMask=subnetMask, wirelessLinkStatus=wirelessLinkStatus, sMTPServerPortNumber=sMTPServerPortNumber, syslogState=syslogState, vLANPriorityTable=vLANPriorityTable, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, tDDControlGroup=tDDControlGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, dataPortWirelessDownAlert=dataPortWirelessDownAlert, antennaGain=antennaGain, sNTPServerPortNumber=sNTPServerPortNumber, licensedCapacity=licensedCapacity, sNMPTrapVersion=sNMPTrapVersion, dfsPeaks=dfsPeaks, ptpTraps=ptpTraps, receiveModulationModeDetail=receiveModulationModeDetail, sNTPSyncTrap=sNTPSyncTrap, dataPortDisabledWarning=dataPortDisabledWarning, telecom=telecom, constantPowerSpectralDensity=constantPowerSpectralDensity, userTable=userTable, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, wirelessLinkAvailability=wirelessLinkAvailability, supplementaryGroup=supplementaryGroup, sNTPLastSync=sNTPLastSync, tDDSynchronizationMode=tDDSynchronizationMode, alarmsGroup=alarmsGroup, linkLoss=linkLoss, dfs=dfs, remoteIPAddress=remoteIPAddress, tDDSynchronizationAlarm=tDDSynchronizationAlarm, resetGroup=resetGroup, softwareVersion=softwareVersion, sNTPState=sNTPState, tFTPServerPortNumber=tFTPServerPortNumber, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, sNMPv3Enable=sNMPv3Enable, sNMPTrapTableIndex=sNMPTrapTableIndex, productVariant=productVariant, altitude=altitude, installStatus=installStatus, dataPortStatusTrap=dataPortStatusTrap, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, userRole=userRole, pubStatsGroup=pubStatsGroup, dfsTableEntry=dfsTableEntry, encryptionGroup=encryptionGroup, ptpTrapPrefix=ptpTrapPrefix, vLANManagementPriority=vLANManagementPriority, cableLoss=cableLoss, vLANPriorityTableNumber=vLANPriorityTableNumber, sMTPServerIPAddress=sMTPServerIPAddress, channelBandwidth=channelBandwidth, telnetPortNumber=telnetPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, hTTPSAccessEnabled=hTTPSAccessEnabled, dataPortStatus=dataPortStatus, telecomsChannelStatusTrap=telecomsChannelStatusTrap, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, syslogClientTrap=syslogClientTrap, sNMPCommunityString=sNMPCommunityString, receiveModulationMode=receiveModulationMode, encryptionAlgorithm=encryptionAlgorithm, ptpGroups=ptpGroups, smtp=smtp, notificationsGroup=notificationsGroup, reset=reset, dfsGroup=dfsGroup, transmitDataRate=transmitDataRate, alarms=alarms, dataPortAutoNegotiation=dataPortAutoNegotiation, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, installArmState=installArmState, incompatibleMasterAndSlave=incompatibleMasterAndSlave, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, longitude=longitude, installStatusTrap=installStatusTrap, sNMPTrapIPAddress=sNMPTrapIPAddress, noWirelessChannelAvailable=noWirelessChannelAvailable, versions=versions, accessMethod=accessMethod, hTTPAccessEnabled=hTTPAccessEnabled, sMTPSourceEmailAddress=sMTPSourceEmailAddress, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, ptpCompliance=ptpCompliance, frequencyVariant=frequencyVariant, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, daylightSaving=daylightSaving, sntp=sntp, aAAControl=aAAControl, groupID=groupID, sNTPPollInterval=sNTPPollInterval, encryption=encryption, hTTPPortNumber=hTTPPortNumber, linkName=linkName, receiveFreqMHz=receiveFreqMHz, transmitFreqKHz=transmitFreqKHz, vLANManagementVID=vLANManagementVID, syslogClient=syslogClient, productName=productName, managementGroup=managementGroup, latitude=latitude, sntpGroup=sntpGroup, licenceGroup=licenceGroup, dataPortConfigurationMismatch=dataPortConfigurationMismatch, gatewayIPAddress=gatewayIPAddress, vLANPriorityTableIndex=vLANPriorityTableIndex, secondarySoftwareVersion=secondarySoftwareVersion, dfsTableNumber=dfsTableNumber, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, iPAddress=iPAddress, bootVersion=bootVersion, phyStatus=phyStatus, smtpGroup=smtpGroup, dfsMeans=dfsMeans, sNMPEnabledTraps=sNMPEnabledTraps, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, transmitChannel=transmitChannel, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex)
