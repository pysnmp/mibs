...
# ASN.1 source HOST-RESOURCES-MIB
# Source digest sha256:8ba80ad8cb76d1962d8c2937ed02c2260f389c5de96c2201fdd0633e22966161
...
if getattr(mibBuilder, 'version', (0, 0, 0)) > (4, 4, 0):
    if mibBuilder.loadTexts: hostResourcesMibModule.setRevisionsDescriptions(('Clarifications and bug fixes based on implementation\n          experience.  This revision was also reformatted in the SMIv2\n          format. The revisions made were:\n\n          New RFC document standards:\n             Added Copyright notice, updated introduction to SNMP\n             Framework, updated references section, added reference to\n             RFC 2119, and added a meaningful Security Considerations\n             section.\n\n          New IANA considerations section for registration of new types\n\n          Conversion to new SMIv2 syntax for the following types and\n          macros:\n              Counter32, Integer32, Gauge32, MODULE-IDENTITY,\n              OBJECT-TYPE, TEXTUAL-CONVENTION, OBJECT-IDENTITY,\n              MODULE-COMPLIANCE, OBJECT-GROUP\n\n          Used new Textual Conventions:\n              TruthValue, DateAndTime, AutonomousType,\n              InterfaceIndexOrZero\n\n          Fixed typo in hrPrinterStatus.\n\n          Added missing error bits to hrPrinterDetectedErrorState and\n          clarified confusion resulting from suggested mappings to\n          hrPrinterStatus.\n          Clarified that size of objects of type\n          InternationalDisplayString is number of octets, not number\n          of encoded symbols.\n\n          Clarified the use of the following objects based on\n          implementation experience:\n              hrSystemInitialLoadDevice, hrSystemInitialLoadParameters,\n              hrMemorySize, hrStorageSize, hrStorageAllocationFailures,\n              hrDeviceErrors, hrProcessorLoad, hrNetworkIfIndex,\n              hrDiskStorageCapacity, hrSWRunStatus, hrSWRunPerfCPU,\n              and hrSWInstalledDate.\n\n          Clarified implementation technique for hrSWInstalledTable.\n\n          Used new AUGMENTS clause for hrSWRunPerfTable.\n\n          Added Internationalization Considerations section.\n\n   This revision published as RFC2790.', 'The original version of this MIB, published as\n          RFC1514.',))
if mibBuilder.loadTexts: hostResourcesMibModule.setLastUpdated('2000-03-06 00:00')
...
hrStorageTable = MibTable((1, 3, 6, 1, 2, 1, 25, 2, 3), ).setMaxAccess("notaccessible")
...
hrStorageEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 2, 3, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrStorageIndex"))
...
hrDeviceTable = MibTable((1, 3, 6, 1, 2, 1, 25, 3, 2), ).setMaxAccess("notaccessible")
...
hrDeviceEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 3, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrDeviceIndex"))
...
hrProcessorTable = MibTable((1, 3, 6, 1, 2, 1, 25, 3, 3), ).setMaxAccess("notaccessible")
...
hrProcessorEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 3, 3, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrDeviceIndex"))
...
hrNetworkTable = MibTable((1, 3, 6, 1, 2, 1, 25, 3, 4), ).setMaxAccess("notaccessible")
...
hrNetworkEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 3, 4, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrDeviceIndex"))
...
hrPrinterTable = MibTable((1, 3, 6, 1, 2, 1, 25, 3, 5), ).setMaxAccess("notaccessible")
...
hrPrinterEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 3, 5, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrDeviceIndex"))
...
hrDiskStorageTable = MibTable((1, 3, 6, 1, 2, 1, 25, 3, 6), ).setMaxAccess("notaccessible")
...
hrDiskStorageEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 3, 6, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrDeviceIndex"))
...
hrPartitionTable = MibTable((1, 3, 6, 1, 2, 1, 25, 3, 7), ).setMaxAccess("notaccessible")
...
hrPartitionEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 3, 7, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrDeviceIndex"), (0, "HOST-RESOURCES-MIB", "hrPartitionIndex"))
...
hrFSTable = MibTable((1, 3, 6, 1, 2, 1, 25, 3, 8), ).setMaxAccess("notaccessible")
...
hrFSEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 3, 8, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrFSIndex"))
...
hrSWRunTable = MibTable((1, 3, 6, 1, 2, 1, 25, 4, 2), ).setMaxAccess("notaccessible")
...
hrSWRunEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 4, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrSWRunIndex"))
...
hrSWRunPerfTable = MibTable((1, 3, 6, 1, 2, 1, 25, 5, 1), ).setMaxAccess("notaccessible")
...
hrSWRunPerfEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 5, 1, 1), ).setMaxAccess("notaccessible")
...
hrSWInstalledTable = MibTable((1, 3, 6, 1, 2, 1, 25, 6, 3), ).setMaxAccess("notaccessible")
...
hrSWInstalledEntry = MibTableRow((1, 3, 6, 1, 2, 1, 25, 6, 3, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "HOST-RESOURCES-MIB", "hrSWInstalledIndex"))
...
hrMIBCompliance = ModuleCompliance((1, 3, 6, 1, 2, 1, 25, 7, 2, 1)).setObjects(("HOST-RESOURCES-MIB", "hrSystemGroup"), ("HOST-RESOURCES-MIB", "hrStorageGroup"), ("HOST-RESOURCES-MIB", "hrDeviceGroup"), ("HOST-RESOURCES-MIB", "hrSWRunGroup"), ("HOST-RESOURCES-MIB", "hrSWRunPerfGroup"), ("HOST-RESOURCES-MIB", "hrSWInstalledGroup"))
...
mibBuilder.exportSymbols("HOST-RESOURCES-MIB", InternationalDisplayString=InternationalDisplayString, KBytes=KBytes, PYSNMP_MODULE_ID=hostResourcesMibModule, ProductID=ProductID, host=host, hostResourcesMibModule=hostResourcesMibModule, hrDevice=hrDevice, hrDeviceDescr=hrDeviceDescr, hrDeviceEntry=hrDeviceEntry, hrDeviceErrors=hrDeviceErrors, hrDeviceGroup=hrDeviceGroup, hrDeviceID=hrDeviceID, hrDeviceIndex=hrDeviceIndex, hrDeviceStatus=hrDeviceStatus, hrDeviceTable=hrDeviceTable, hrDeviceType=hrDeviceType, hrDeviceTypes=hrDeviceTypes, hrDiskStorageAccess=hrDiskStorageAccess, hrDiskStorageCapacity=hrDiskStorageCapacity, hrDiskStorageEntry=hrDiskStorageEntry, hrDiskStorageMedia=hrDiskStorageMedia, hrDiskStorageRemoveble=hrDiskStorageRemoveble, hrDiskStorageTable=hrDiskStorageTable, hrFSAccess=hrFSAccess, hrFSBootable=hrFSBootable, hrFSEntry=hrFSEntry, hrFSIndex=hrFSIndex, hrFSLastFullBackupDate=hrFSLastFullBackupDate, hrFSLastPartialBackupDate=hrFSLastPartialBackupDate, hrFSMountPoint=hrFSMountPoint, hrFSRemoteMountPoint=hrFSRemoteMountPoint, hrFSStorageIndex=hrFSStorageIndex, hrFSTable=hrFSTable, hrFSType=hrFSType, hrFSTypes=hrFSTypes, hrMIBAdminInfo=hrMIBAdminInfo, hrMIBCompliance=hrMIBCompliance, hrMIBCompliances=hrMIBCompliances, hrMIBGroups=hrMIBGroups, hrMemorySize=hrMemorySize, hrNetworkEntry=hrNetworkEntry, hrNetworkIfIndex=hrNetworkIfIndex, hrNetworkTable=hrNetworkTable, hrPartitionEntry=hrPartitionEntry, hrPartitionFSIndex=hrPartitionFSIndex, hrPartitionID=hrPartitionID, hrPartitionIndex=hrPartitionIndex, hrPartitionLabel=hrPartitionLabel, hrPartitionSize=hrPartitionSize, hrPartitionTable=hrPartitionTable, hrPrinterDetectedErrorState=hrPrinterDetectedErrorState, hrPrinterEntry=hrPrinterEntry, hrPrinterStatus=hrPrinterStatus, hrPrinterTable=hrPrinterTable, hrProcessorEntry=hrProcessorEntry, hrProcessorFrwID=hrProcessorFrwID, hrProcessorLoad=hrProcessorLoad, hrProcessorTable=hrProcessorTable, hrSWInstalled=hrSWInstalled, hrSWInstalledDate=hrSWInstalledDate, hrSWInstalledEntry=hrSWInstalledEntry, hrSWInstalledGroup=hrSWInstalledGroup, hrSWInstalledID=hrSWInstalledID, hrSWInstalledIndex=hrSWInstalledIndex, hrSWInstalledLastChange=hrSWInstalledLastChange, hrSWInstalledLastUpdateTime=hrSWInstalledLastUpdateTime, hrSWInstalledName=hrSWInstalledName, hrSWInstalledTable=hrSWInstalledTable, hrSWInstalledType=hrSWInstalledType, hrSWOSIndex=hrSWOSIndex, hrSWRun=hrSWRun, hrSWRunEntry=hrSWRunEntry, hrSWRunGroup=hrSWRunGroup, hrSWRunID=hrSWRunID, hrSWRunIndex=hrSWRunIndex, hrSWRunName=hrSWRunName, hrSWRunParameters=hrSWRunParameters, hrSWRunPath=hrSWRunPath, hrSWRunPerf=hrSWRunPerf, hrSWRunPerfCPU=hrSWRunPerfCPU, hrSWRunPerfEntry=hrSWRunPerfEntry, hrSWRunPerfGroup=hrSWRunPerfGroup, hrSWRunPerfMem=hrSWRunPerfMem, hrSWRunPerfTable=hrSWRunPerfTable, hrSWRunStatus=hrSWRunStatus, hrSWRunTable=hrSWRunTable, hrSWRunType=hrSWRunType, hrStorage=hrStorage, hrStorageAllocationFailures=hrStorageAllocationFailures, hrStorageAllocationUnits=hrStorageAllocationUnits, hrStorageDescr=hrStorageDescr, hrStorageEntry=hrStorageEntry, hrStorageGroup=hrStorageGroup, hrStorageIndex=hrStorageIndex, hrStorageSize=hrStorageSize, hrStorageTable=hrStorageTable, hrStorageType=hrStorageType, hrStorageTypes=hrStorageTypes, hrStorageUsed=hrStorageUsed, hrSystem=hrSystem, hrSystemDate=hrSystemDate, hrSystemGroup=hrSystemGroup, hrSystemInitialLoadDevice=hrSystemInitialLoadDevice, hrSystemInitialLoadParameters=hrSystemInitialLoadParameters, hrSystemMaxProcesses=hrSystemMaxProcesses, hrSystemNumUsers=hrSystemNumUsers, hrSystemProcesses=hrSystemProcesses, hrSystemUptime=hrSystemUptime)
