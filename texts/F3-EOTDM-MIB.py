...
DisplayString, VariablePointer, TextualConvention, TruthValue, StorageType, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "VariablePointer", "TextualConvention", "TruthValue", "StorageType", "RowStatus")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgMemberIfIndex=vcgMemberIfIndex, vcgIndex=vcgIndex, WtrTime=WtrTime, LcasSoState=LcasSoState, f3EotdmCompliance=f3EotdmCompliance, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgAdminState=vcgAdminState, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, HoldOffTime=HoldOffTime, LcasSkState=LcasSkState, vcgType=vcgType, vcgMemberEntry=vcgMemberEntry, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgMemberRowStatus=vcgMemberRowStatus, vcgClearWtrTimer=vcgClearWtrTimer, vcgSecondaryState=vcgSecondaryState, vcgEntry=vcgEntry, vcgTable=vcgTable, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, f3EotdmGroups=f3EotdmGroups, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, f3EOTDMMIB=f3EOTDMMIB, vcgOperationalState=vcgOperationalState, vcgHoldOffTimer=vcgHoldOffTimer, CtrlState=CtrlState, vcgIfIndex=vcgIfIndex, MstState=MstState, vcgMemberIndex=vcgMemberIndex, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgWtrTimer=vcgWtrTimer, f3EotdmObjects=f3EotdmObjects, PYSNMP_MODULE_ID=f3EOTDMMIB, f3EotdmConformance=f3EotdmConformance, f3EotdmCompliances=f3EotdmCompliances, VcgType=VcgType, vcgLcasEnabled=vcgLcasEnabled, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgMemberTable=vcgMemberTable, vcgRowStatus=vcgRowStatus)
