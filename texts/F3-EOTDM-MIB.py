...
# ASN.1 source F3-EOTDM-MIB
# Source digest sha256:0cd5c9679e1804abbe80ca5bb628771ea1af7aad3c994e96e94f8d216c15b09c
...
if mibBuilder.loadTexts: f3EOTDMMIB.setLastUpdated('2012-05-10 00:00')
...
vcgTable = MibTable((1, 3, 6, 1, 4, 1, 2544, 1, 12, 17, 1, 1), ).setMaxAccess("notaccessible")
...
vcgEntry = MibTableRow((1, 3, 6, 1, 4, 1, 2544, 1, 12, 17, 1, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CM-ENTITY-MIB", "neIndex"), (0, "CM-ENTITY-MIB", "shelfIndex"), (0, "CM-ENTITY-MIB", "slotIndex"), (0, "F3-EOTDM-MIB", "vcgIndex"))
...
vcgMemberTable = MibTable((1, 3, 6, 1, 4, 1, 2544, 1, 12, 17, 1, 2), ).setMaxAccess("notaccessible")
...
vcgMemberEntry = MibTableRow((1, 3, 6, 1, 4, 1, 2544, 1, 12, 17, 1, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CM-ENTITY-MIB", "neIndex"), (0, "CM-ENTITY-MIB", "shelfIndex"), (0, "CM-ENTITY-MIB", "slotIndex"), (0, "F3-EOTDM-MIB", "vcgIndex"), (0, "F3-EOTDM-MIB", "vcgMemberIndex"))
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", CtrlState=CtrlState, HoldOffTime=HoldOffTime, LcasSkState=LcasSkState, LcasSoState=LcasSoState, MstState=MstState, PYSNMP_MODULE_ID=f3EOTDMMIB, VcgType=VcgType, WtrTime=WtrTime, f3EOTDMMIB=f3EOTDMMIB, f3EotdmCompliance=f3EotdmCompliance, f3EotdmCompliances=f3EotdmCompliances, f3EotdmConformance=f3EotdmConformance, f3EotdmGroups=f3EotdmGroups, f3EotdmObjectGroup=f3EotdmObjectGroup, f3EotdmObjects=f3EotdmObjects, vcgAdminState=vcgAdminState, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgClearWtrTimer=vcgClearWtrTimer, vcgEntry=vcgEntry, vcgHoldOffTimer=vcgHoldOffTimer, vcgIfIndex=vcgIfIndex, vcgIndex=vcgIndex, vcgLcasEnabled=vcgLcasEnabled, vcgMemberEntry=vcgMemberEntry, vcgMemberIfIndex=vcgMemberIfIndex, vcgMemberIndex=vcgMemberIndex, vcgMemberRowStatus=vcgMemberRowStatus, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgMemberTable=vcgMemberTable, vcgOperationalState=vcgOperationalState, vcgRowStatus=vcgRowStatus, vcgSecondaryState=vcgSecondaryState, vcgTable=vcgTable, vcgType=vcgType, vcgWtrTimer=vcgWtrTimer)
