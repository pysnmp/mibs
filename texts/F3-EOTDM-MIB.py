...
StorageType, DisplayString, RowStatus, TextualConvention, VariablePointer, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "StorageType", "DisplayString", "RowStatus", "TextualConvention", "VariablePointer", "TruthValue")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", VcgType=VcgType, f3EOTDMMIB=f3EOTDMMIB, f3EotdmCompliances=f3EotdmCompliances, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgMemberIfIndex=vcgMemberIfIndex, vcgIfIndex=vcgIfIndex, vcgMemberSoLcasState=vcgMemberSoLcasState, vcgClearWtrTimer=vcgClearWtrTimer, vcgIndex=vcgIndex, vcgMemberSoSendSeq=vcgMemberSoSendSeq, vcgWtrTimer=vcgWtrTimer, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq, LcasSkState=LcasSkState, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgOperationalState=vcgOperationalState, HoldOffTime=HoldOffTime, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, CtrlState=CtrlState, LcasSoState=LcasSoState, vcgMemberTable=vcgMemberTable, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, f3EotdmCompliance=f3EotdmCompliance, MstState=MstState, vcgRowStatus=vcgRowStatus, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgHoldOffTimer=vcgHoldOffTimer, vcgAdminState=vcgAdminState, vcgType=vcgType, vcgMemberRowStatus=vcgMemberRowStatus, f3EotdmConformance=f3EotdmConformance, vcgMemberEntry=vcgMemberEntry, PYSNMP_MODULE_ID=f3EOTDMMIB, f3EotdmGroups=f3EotdmGroups, vcgTable=vcgTable, WtrTime=WtrTime, vcgEntry=vcgEntry, vcgMemberIndex=vcgMemberIndex, f3EotdmObjects=f3EotdmObjects, vcgLcasEnabled=vcgLcasEnabled, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgSecondaryState=vcgSecondaryState, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst)
