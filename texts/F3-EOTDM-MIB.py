...
VariablePointer, StorageType, TextualConvention, RowStatus, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "VariablePointer", "StorageType", "TextualConvention", "RowStatus", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("F3-EOTDM-MIB", vcgClearWtrTimer=vcgClearWtrTimer, vcgMemberSkRecvExpectSeq=vcgMemberSkRecvExpectSeq, vcgWtrTimer=vcgWtrTimer, vcgMemberIndex=vcgMemberIndex, MstState=MstState, vcgMemberSkLcasState=vcgMemberSkLcasState, vcgMemberSoSendSeq=vcgMemberSoSendSeq, CtrlState=CtrlState, vcgHoldOffTimer=vcgHoldOffTimer, vcgTable=vcgTable, vcgLcasEnabled=vcgLcasEnabled, vcgAdminState=vcgAdminState, vcgAssociatedEthernetPort=vcgAssociatedEthernetPort, vcgRowStatus=vcgRowStatus, vcgEntry=vcgEntry, vcgMemberIfIndex=vcgMemberIfIndex, vcgIndex=vcgIndex, PYSNMP_MODULE_ID=f3EOTDMMIB, vcgMemberEntry=vcgMemberEntry, LcasSkState=LcasSkState, f3EotdmCompliances=f3EotdmCompliances, vcgMemberSoLcasRecvMst=vcgMemberSoLcasRecvMst, vcgMemberSkLcasSendMst=vcgMemberSkLcasSendMst, f3EotdmObjectGroup=f3EotdmObjectGroup, vcgMemberSoLcasSendCtrl=vcgMemberSoLcasSendCtrl, vcgType=vcgType, f3EotdmGroups=f3EotdmGroups, WtrTime=WtrTime, vcgMemberSoLcasState=vcgMemberSoLcasState, f3EotdmObjects=f3EotdmObjects, HoldOffTime=HoldOffTime, VcgType=VcgType, vcgSecondaryState=vcgSecondaryState, f3EotdmConformance=f3EotdmConformance, vcgMemberTable=vcgMemberTable, vcgOperationalState=vcgOperationalState, LcasSoState=LcasSoState, f3EotdmCompliance=f3EotdmCompliance, vcgIfIndex=vcgIfIndex, vcgMemberSkLcasRecvCtrl=vcgMemberSkLcasRecvCtrl, f3EOTDMMIB=f3EOTDMMIB, vcgMemberRowStatus=vcgMemberRowStatus, vcgMemberSkRecvSeq=vcgMemberSkRecvSeq)
