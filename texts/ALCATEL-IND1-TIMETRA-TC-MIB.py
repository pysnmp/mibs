...
mibBuilder.exportSymbols("ALCATEL-IND1-TIMETRA-TC-MIB", TmnxEnabledDisabled=TmnxEnabledDisabled, TmnxBGPFamilyType=TmnxBGPFamilyType, TPIRRateOrZero=TPIRRateOrZero, TSecondaryShaper10GPIRRate=TSecondaryShaper10GPIRRate, TmnxIgmpGroupFilterMode=TmnxIgmpGroupFilterMode, TmnxAppProfileString=TmnxAppProfileString, TmnxMldVersion=TmnxMldVersion, TItemDescription=TItemDescription, TSapEgressPolicyID=TSapEgressPolicyID, TmnxTunnelID=TmnxTunnelID, SdpBindId=SdpBindId, TIpProtocol=TIpProtocol, TmnxAncpStringOrZero=TmnxAncpStringOrZero, TmnxAppProfileStringOrEmpty=TmnxAppProfileStringOrEmpty, TmnxSubIdentStringOrEmpty=TmnxSubIdentStringOrEmpty, ServiceOperStatus=ServiceOperStatus, TPIRRateOverride=TPIRRateOverride, TDSCPName=TDSCPName, TPortSchedulerCIR=TPortSchedulerCIR, TPortSchedulerPIR=TPortSchedulerPIR, TmnxStatus=TmnxStatus, TmnxAncpString=TmnxAncpString, TmnxSubIdentString=TmnxSubIdentString, TmnxStrSapId=TmnxStrSapId, TmnxManagedRouteStatus=TmnxManagedRouteStatus, TPolicyID=TPolicyID, TmnxDhcpOptionType=TmnxDhcpOptionType, TCpmProtPolicyID=TCpmProtPolicyID, TFCType=TFCType, TmnxVRtrIDOrZero=TmnxVRtrIDOrZero, TTcpUdpPortOperator=TTcpUdpPortOperator, TmnxVRtrID=TmnxVRtrID, TIpOption=TIpOption, TmnxIgmpGroupType=TmnxIgmpGroupType, TEgressQueueId=TEgressQueueId, TmnxPppoeUserNameOrEmpty=TmnxPppoeUserNameOrEmpty, TmnxCustId=TmnxCustId, TmnxVcIdOrNone=TmnxVcIdOrNone, TFrameType=TFrameType, TFCNameOrEmpty=TFCNameOrEmpty, TmnxIgmpVersion=TmnxIgmpVersion, TDSCPNameOrEmpty=TDSCPNameOrEmpty, TmnxDHCP6MsgType=TmnxDHCP6MsgType, TTcpUdpPort=TTcpUdpPort, TmnxVcId=TmnxVcId, TmnxServId=TmnxServId, Dot1PPriority=Dot1PPriority, QTagOrZero=QTagOrZero, TLspExpValue=TLspExpValue, TmnxBgpLocalPreference=TmnxBgpLocalPreference, TDSCPFilterActionValue=TDSCPFilterActionValue, TSapIngressPolicyID=TSapIngressPolicyID, QTag=QTag, TmnxSlaProfileString=TmnxSlaProfileString, PYSNMP_MODULE_ID=timetraTCMIBModule, TmnxMldGroupType=TmnxMldGroupType, TmnxVcType=TmnxVcType, TNamedItemOrEmpty=TNamedItemOrEmpty, TmnxSlaProfileStringOrEmpty=TmnxSlaProfileStringOrEmpty, TMlpppQoSProfileId=TMlpppQoSProfileId, TIngressQueueId=TIngressQueueId, TFCSet=TFCSet, TmnxOspfInstance=TmnxOspfInstance, TmnxTunnelType=TmnxTunnelType, TmnxVRtrMplsLspID=TmnxVRtrMplsLspID, TPIRRate=TPIRRate, TmnxMldGroupFilterMode=TmnxMldGroupFilterMode, TQueueId=TQueueId, TmnxActionType=TmnxActionType, TDSCPValueOrNone=TDSCPValueOrNone, TmnxEncapVal=TmnxEncapVal, TPolicyStatementNameOrEmpty=TPolicyStatementNameOrEmpty, TCIRRate=TCIRRate, TmnxSubProfileStringOrEmpty=TmnxSubProfileStringOrEmpty, TmnxSubMgtIntDestIdOrEmpty=TmnxSubMgtIntDestIdOrEmpty, TmnxMulticastAddrFamily=TmnxMulticastAddrFamily, TmnxDhcpVendorOption=TmnxDhcpVendorOption, TNamedItem=TNamedItem, TmnxAdminState=TmnxAdminState, TItemLongDescription=TItemLongDescription, TmnxSubProfileString=TmnxSubProfileString, TDSCPValue=TDSCPValue, TmnxOperState=TmnxOperState, ServiceAdminStatus=ServiceAdminStatus, TmnxPortID=TmnxPortID, InterfaceIndex=InterfaceIndex, TmnxSubMgtIntDestId=TmnxSubMgtIntDestId, TmnxBgpAutonomousSystem=TmnxBgpAutonomousSystem, TFCName=TFCName, TmnxVPNRouteDistinguisher=TmnxVPNRouteDistinguisher, IpAddressPrefixLength=IpAddressPrefixLength, TWeight=TWeight, ServiceAccessPoint=ServiceAccessPoint, timetraTCMIBModule=timetraTCMIBModule, TmnxBgpPreference=TmnxBgpPreference, TmnxBgpRouteTarget=TmnxBgpRouteTarget)
