...
mibBuilder.exportSymbols("ALCATEL-IND1-TIMETRA-TC-MIB", TSapEgressPolicyID=TSapEgressPolicyID, TmnxStatus=TmnxStatus, TmnxVcId=TmnxVcId, TmnxVRtrIDOrZero=TmnxVRtrIDOrZero, TDSCPNameOrEmpty=TDSCPNameOrEmpty, TPortSchedulerCIR=TPortSchedulerCIR, TFCSet=TFCSet, TCIRRate=TCIRRate, TmnxSubProfileStringOrEmpty=TmnxSubProfileStringOrEmpty, TFCNameOrEmpty=TFCNameOrEmpty, TTcpUdpPortOperator=TTcpUdpPortOperator, TItemLongDescription=TItemLongDescription, TWeight=TWeight, TFCName=TFCName, TEgressQueueId=TEgressQueueId, TmnxStrSapId=TmnxStrSapId, TmnxCustId=TmnxCustId, TmnxMldGroupType=TmnxMldGroupType, TmnxOperState=TmnxOperState, QTag=QTag, TmnxActionType=TmnxActionType, SdpBindId=SdpBindId, TmnxBGPFamilyType=TmnxBGPFamilyType, TPIRRateOrZero=TPIRRateOrZero, QTagOrZero=QTagOrZero, TmnxEnabledDisabled=TmnxEnabledDisabled, TmnxSlaProfileString=TmnxSlaProfileString, TmnxVRtrMplsLspID=TmnxVRtrMplsLspID, TDSCPValue=TDSCPValue, TTcpUdpPort=TTcpUdpPort, timetraTCMIBModule=timetraTCMIBModule, TmnxBgpAutonomousSystem=TmnxBgpAutonomousSystem, TFCType=TFCType, TmnxOspfInstance=TmnxOspfInstance, TPIRRateOverride=TPIRRateOverride, TmnxServId=TmnxServId, TmnxPppoeUserNameOrEmpty=TmnxPppoeUserNameOrEmpty, TmnxBgpLocalPreference=TmnxBgpLocalPreference, PYSNMP_MODULE_ID=timetraTCMIBModule, TmnxDhcpVendorOption=TmnxDhcpVendorOption, TmnxAppProfileStringOrEmpty=TmnxAppProfileStringOrEmpty, TmnxTunnelID=TmnxTunnelID, TmnxSubIdentString=TmnxSubIdentString, ServiceOperStatus=ServiceOperStatus, TmnxBgpPreference=TmnxBgpPreference, TPolicyID=TPolicyID, TItemDescription=TItemDescription, TQueueId=TQueueId, TMlpppQoSProfileId=TMlpppQoSProfileId, TmnxVcType=TmnxVcType, TmnxDhcpOptionType=TmnxDhcpOptionType, TmnxEncapVal=TmnxEncapVal, IpAddressPrefixLength=IpAddressPrefixLength, TmnxSubIdentStringOrEmpty=TmnxSubIdentStringOrEmpty, TNamedItemOrEmpty=TNamedItemOrEmpty, TmnxMldVersion=TmnxMldVersion, TmnxSubMgtIntDestIdOrEmpty=TmnxSubMgtIntDestIdOrEmpty, TmnxIgmpGroupFilterMode=TmnxIgmpGroupFilterMode, TmnxDHCP6MsgType=TmnxDHCP6MsgType, TmnxIgmpVersion=TmnxIgmpVersion, TPIRRate=TPIRRate, TmnxIgmpGroupType=TmnxIgmpGroupType, TDSCPName=TDSCPName, TNamedItem=TNamedItem, TSapIngressPolicyID=TSapIngressPolicyID, TmnxMldGroupFilterMode=TmnxMldGroupFilterMode, TmnxVRtrID=TmnxVRtrID, TCpmProtPolicyID=TCpmProtPolicyID, TLspExpValue=TLspExpValue, ServiceAdminStatus=ServiceAdminStatus, TmnxAncpStringOrZero=TmnxAncpStringOrZero, TFrameType=TFrameType, TIpProtocol=TIpProtocol, TmnxBgpRouteTarget=TmnxBgpRouteTarget, TPortSchedulerPIR=TPortSchedulerPIR, ServiceAccessPoint=ServiceAccessPoint, TmnxVcIdOrNone=TmnxVcIdOrNone, TmnxMulticastAddrFamily=TmnxMulticastAddrFamily, TPolicyStatementNameOrEmpty=TPolicyStatementNameOrEmpty, TIpOption=TIpOption, TmnxSubMgtIntDestId=TmnxSubMgtIntDestId, TmnxPortID=TmnxPortID, TDSCPFilterActionValue=TDSCPFilterActionValue, TSecondaryShaper10GPIRRate=TSecondaryShaper10GPIRRate, InterfaceIndex=InterfaceIndex, TmnxVPNRouteDistinguisher=TmnxVPNRouteDistinguisher, TDSCPValueOrNone=TDSCPValueOrNone, Dot1PPriority=Dot1PPriority, TmnxSubProfileString=TmnxSubProfileString, TIngressQueueId=TIngressQueueId, TmnxAdminState=TmnxAdminState, TmnxTunnelType=TmnxTunnelType, TmnxAncpString=TmnxAncpString, TmnxManagedRouteStatus=TmnxManagedRouteStatus, TmnxAppProfileString=TmnxAppProfileString, TmnxSlaProfileStringOrEmpty=TmnxSlaProfileStringOrEmpty)
