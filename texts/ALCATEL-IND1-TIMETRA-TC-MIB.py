...
mibBuilder.exportSymbols("ALCATEL-IND1-TIMETRA-TC-MIB", TmnxBgpLocalPreference=TmnxBgpLocalPreference, TmnxTunnelType=TmnxTunnelType, TDSCPNameOrEmpty=TDSCPNameOrEmpty, TmnxAncpString=TmnxAncpString, TmnxSlaProfileStringOrEmpty=TmnxSlaProfileStringOrEmpty, TNamedItemOrEmpty=TNamedItemOrEmpty, TmnxEncapVal=TmnxEncapVal, TLspExpValue=TLspExpValue, TQueueId=TQueueId, TDSCPValueOrNone=TDSCPValueOrNone, QTagOrZero=QTagOrZero, TmnxOperState=TmnxOperState, TmnxServId=TmnxServId, TItemDescription=TItemDescription, TmnxMldVersion=TmnxMldVersion, TmnxSubProfileString=TmnxSubProfileString, TmnxAppProfileStringOrEmpty=TmnxAppProfileStringOrEmpty, TmnxIgmpVersion=TmnxIgmpVersion, timetraTCMIBModule=timetraTCMIBModule, SdpBindId=SdpBindId, TEgressQueueId=TEgressQueueId, TmnxIgmpGroupType=TmnxIgmpGroupType, TmnxSubIdentStringOrEmpty=TmnxSubIdentStringOrEmpty, TFCType=TFCType, TCIRRate=TCIRRate, TmnxTunnelID=TmnxTunnelID, TIpProtocol=TIpProtocol, TCpmProtPolicyID=TCpmProtPolicyID, TmnxVRtrID=TmnxVRtrID, TmnxSubProfileStringOrEmpty=TmnxSubProfileStringOrEmpty, TmnxMldGroupType=TmnxMldGroupType, TmnxManagedRouteStatus=TmnxManagedRouteStatus, ServiceOperStatus=ServiceOperStatus, TmnxBgpPreference=TmnxBgpPreference, TmnxAdminState=TmnxAdminState, TPIRRateOverride=TPIRRateOverride, TNamedItem=TNamedItem, TmnxDhcpOptionType=TmnxDhcpOptionType, TIngressQueueId=TIngressQueueId, TmnxBgpAutonomousSystem=TmnxBgpAutonomousSystem, TPortSchedulerPIR=TPortSchedulerPIR, TmnxVcId=TmnxVcId, TItemLongDescription=TItemLongDescription, TPolicyStatementNameOrEmpty=TPolicyStatementNameOrEmpty, TmnxDhcpVendorOption=TmnxDhcpVendorOption, TDSCPName=TDSCPName, TmnxVPNRouteDistinguisher=TmnxVPNRouteDistinguisher, TFCNameOrEmpty=TFCNameOrEmpty, TmnxSubMgtIntDestIdOrEmpty=TmnxSubMgtIntDestIdOrEmpty, TmnxVRtrIDOrZero=TmnxVRtrIDOrZero, ServiceAdminStatus=ServiceAdminStatus, TmnxPppoeUserNameOrEmpty=TmnxPppoeUserNameOrEmpty, TmnxVRtrMplsLspID=TmnxVRtrMplsLspID, TmnxSubMgtIntDestId=TmnxSubMgtIntDestId, InterfaceIndex=InterfaceIndex, ServiceAccessPoint=ServiceAccessPoint, TDSCPFilterActionValue=TDSCPFilterActionValue, TmnxVcType=TmnxVcType, TMlpppQoSProfileId=TMlpppQoSProfileId, QTag=QTag, TDSCPValue=TDSCPValue, TmnxStatus=TmnxStatus, TPolicyID=TPolicyID, TPortSchedulerCIR=TPortSchedulerCIR, IpAddressPrefixLength=IpAddressPrefixLength, TmnxMldGroupFilterMode=TmnxMldGroupFilterMode, TmnxEnabledDisabled=TmnxEnabledDisabled, TWeight=TWeight, PYSNMP_MODULE_ID=timetraTCMIBModule, TmnxBGPFamilyType=TmnxBGPFamilyType, TIpOption=TIpOption, TmnxSubIdentString=TmnxSubIdentString, TmnxMulticastAddrFamily=TmnxMulticastAddrFamily, TmnxPortID=TmnxPortID, TmnxStrSapId=TmnxStrSapId, TmnxSlaProfileString=TmnxSlaProfileString, TmnxVcIdOrNone=TmnxVcIdOrNone, TSecondaryShaper10GPIRRate=TSecondaryShaper10GPIRRate, TFCSet=TFCSet, TmnxCustId=TmnxCustId, TmnxActionType=TmnxActionType, TSapEgressPolicyID=TSapEgressPolicyID, TSapIngressPolicyID=TSapIngressPolicyID, TmnxDHCP6MsgType=TmnxDHCP6MsgType, TmnxBgpRouteTarget=TmnxBgpRouteTarget, TmnxAncpStringOrZero=TmnxAncpStringOrZero, TFCName=TFCName, TTcpUdpPort=TTcpUdpPort, TTcpUdpPortOperator=TTcpUdpPortOperator, TPIRRate=TPIRRate, TPIRRateOrZero=TPIRRateOrZero, TmnxOspfInstance=TmnxOspfInstance, Dot1PPriority=Dot1PPriority, TmnxIgmpGroupFilterMode=TmnxIgmpGroupFilterMode, TFrameType=TFrameType, TmnxAppProfileString=TmnxAppProfileString)
