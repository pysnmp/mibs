...
mibBuilder.exportSymbols("Printer-MIB", prtInterpreterIndex=prtInterpreterIndex, prtMarkerSouthMargin=prtMarkerSouthMargin, prtConsoleOffTime=prtConsoleOffTime, prtInputVendorName=prtInputVendorName, prtExtendedOutputGroup=prtExtendedOutputGroup, PrtAlertSeverityLevelTC=PrtAlertSeverityLevelTC, prtInputCapacityUnit=prtInputCapacityUnit, prtAlertAllEvents=prtAlertAllEvents, prtInputName=prtInputName, prtExtendedInputGroup=prtExtendedInputGroup, prtInputMediaFormParts=prtInputMediaFormParts, prtGeneralServicePerson=prtGeneralServicePerson, prtOutputOffsetStacking=prtOutputOffsetStacking, PrtOutputStackingOrderTC=PrtOutputStackingOrderTC, prtAuxiliarySheetBannerPage=prtAuxiliarySheetBannerPage, prtInterpreterDefaultOrientation=prtInterpreterDefaultOrientation, prtChannelIndex=prtChannelIndex, prtMediaPathTable=prtMediaPathTable, prtInputDefaultIndex=prtInputDefaultIndex, prtMarkerColorantTable=prtMarkerColorantTable, prtMediaPathMaxSpeedPrintUnit=prtMediaPathMaxSpeedPrintUnit, prtMediaPathEntry=prtMediaPathEntry, prtDeviceRefSeqNumber=prtDeviceRefSeqNumber, prtMarkerCounterUnit=prtMarkerCounterUnit, printmib=printmib, prtInputStatus=prtInputStatus, prtChannelCurrentJobCntlLangIndex=prtChannelCurrentJobCntlLangIndex, prtOutputIndex=prtOutputIndex, prtOutputDimUnit=prtOutputDimUnit, prtMarkerColorantRole=prtMarkerColorantRole, prtDeviceRefIndex=prtDeviceRefIndex, prtOutputType=prtOutputType, prtMarkerEastMargin=prtMarkerEastMargin, prtInterpreterDescription=prtInterpreterDescription, prtConsoleLightTable=prtConsoleLightTable, prtInputMaxCapacity=prtInputMaxCapacity, prtInputNextIndex=prtInputNextIndex, prtInputType=prtInputType, prtAlertEntry=prtAlertEntry, prtMarkerNorthMargin=prtMarkerNorthMargin, prtMarkerSuppliesIndex=prtMarkerSuppliesIndex, prtAlertTableGroup=prtAlertTableGroup, prtMarkerLifeCount=prtMarkerLifeCount, prtMediaPath=prtMediaPath, prtChannel=prtChannel, prtOutputMinDimXFeedDir=prtOutputMinDimXFeedDir, prtInterpreterDefaultCharSetIn=prtInterpreterDefaultCharSetIn, prtConsoleGroup=prtConsoleGroup, prtMarkerColorantTonality=prtMarkerColorantTonality, prtOutputVersion=prtOutputVersion, prtMediaPathType=prtMediaPathType, prtCoverDescription=prtCoverDescription, PYSNMP_MODULE_ID=printmib, prtConsoleDisable=prtConsoleDisable, prtInputDimUnit=prtInputDimUnit, PrtConsoleDescriptionStringTC=PrtConsoleDescriptionStringTC, prtInputMediaDimXFeedDirDeclared=prtInputMediaDimXFeedDirDeclared, prtGeneralPrinterName=prtGeneralPrinterName, PrtMediaPathMaxSpeedPrintUnitTC=PrtMediaPathMaxSpeedPrintUnitTC, prtAlertTable=prtAlertTable, prtGeneralGroup=prtGeneralGroup, prtMIB2Groups=prtMIB2Groups, prtInput=prtInput, prtCoverIndex=prtCoverIndex, prtOutputMaxDimFeedDir=prtOutputMaxDimFeedDir, prtOutputFeaturesGroup=prtOutputFeaturesGroup, prtAuxiliarySheetStartupPage=prtAuxiliarySheetStartupPage, prtAlertSeverityLevel=prtAlertSeverityLevel, prtOutputPageCollated=prtOutputPageCollated, prtMarkerStatus=prtMarkerStatus, prtInterpreterTwoWay=prtInterpreterTwoWay, prtInputTable=prtInputTable, prtChannelProtocolVersion=prtChannelProtocolVersion, prtMarkerAddressabilityUnit=prtMarkerAddressabilityUnit, prtConsoleDisplayBufferEntry=prtConsoleDisplayBufferEntry, prtOutput=prtOutput, prtAlertLocation=prtAlertLocation, prtMarkerColorantValue=prtMarkerColorantValue, prtOutputPageDeliveryOrientation=prtOutputPageDeliveryOrientation, prtMarkerSuppliesType=prtMarkerSuppliesType, prtGeneralCurrentLocalization=prtGeneralCurrentLocalization, prtChannelDefaultPageDescLangIndex=prtChannelDefaultPageDescLangIndex, prtMediaPathDefaultIndex=prtMediaPathDefaultIndex, prtMarkerSupplies=prtMarkerSupplies, prtCover=prtCover, prtInputEntry=prtInputEntry, prtMediaPathGroup=prtMediaPathGroup, prtOutputMinDimFeedDir=prtOutputMinDimFeedDir, prtAuxiliarySheetGroup=prtAuxiliarySheetGroup, prtAlertTableV2Group=prtAlertTableV2Group, prtInputIndex=prtInputIndex, prtMarkerSpotColorants=prtMarkerSpotColorants, prtConsoleDescription=prtConsoleDescription, prtResponsiblePartyGroup=prtResponsiblePartyGroup, prtInputCurrentLevel=prtInputCurrentLevel, prtMarkerColorantGroup=prtMarkerColorantGroup, prtMarkerSuppliesColorantIndex=prtMarkerSuppliesColorantIndex, SubUnitStatus=SubUnitStatus, prtOutputMaxDimXFeedDir=prtOutputMaxDimXFeedDir, prtOutputSerialNumber=prtOutputSerialNumber, prtInterpreterDefaultCharSetOut=prtInterpreterDefaultCharSetOut, printerV1Alert=printerV1Alert, prtMarkerSuppliesGroup=prtMarkerSuppliesGroup, prtConsoleDisplayBufferTable=prtConsoleDisplayBufferTable, prtInterpreterVersion=prtInterpreterVersion, prtAlertTime=prtAlertTime, prtMarkerSuppliesEntry=prtMarkerSuppliesEntry, PrtOutputPageDeliveryOrientationTC=PrtOutputPageDeliveryOrientationTC, PrtMarkerColorantRoleTC=PrtMarkerColorantRoleTC, prtInputModel=prtInputModel, prtMarkerAddressabilityFeedDir=prtMarkerAddressabilityFeedDir, prtMarkerAddressabilityXFeedDir=prtMarkerAddressabilityXFeedDir, prtMarkerColorant=prtMarkerColorant, PrtInterpreterTwoWayTC=PrtInterpreterTwoWayTC, prtInputSerialNumber=prtInputSerialNumber, prtMediaPathMaxSpeed=prtMediaPathMaxSpeed, prtConsoleNumberOfDisplayLines=prtConsoleNumberOfDisplayLines, PrtCapacityUnitTC=PrtCapacityUnitTC, prtGeneralSerialNumber=prtGeneralSerialNumber, prtMediaPathMinMediaFeedDir=prtMediaPathMinMediaFeedDir, prtGeneralTable=prtGeneralTable, prtMIBConformance=prtMIBConformance, prtChannelV2Group=prtChannelV2Group, prtChannelStatus=prtChannelStatus, printerV2Alert=printerV2Alert, prtMarkerColorantIndex=prtMarkerColorantIndex, prtOutputGroup=prtOutputGroup, prtConsoleNumberOfDisplayChars=prtConsoleNumberOfDisplayChars, prtAlertTimeGroup=prtAlertTimeGroup, prtAlertDescription=prtAlertDescription, prtOutputStatus=prtOutputStatus, PrtMarkerCounterUnitTC=PrtMarkerCounterUnitTC, prtMarkerPowerOnCount=prtMarkerPowerOnCount, prtInputVersion=prtInputVersion, prtMarkerColorantMarkerIndex=prtMarkerColorantMarkerIndex, prtAlert=prtAlert, prtMarkerSuppliesMaxCapacity=prtMarkerSuppliesMaxCapacity, prtMIB2Compliance=prtMIB2Compliance, prtOutputVendorName=prtOutputVendorName, prtMediaPathStatus=prtMediaPathStatus, prtMarkerSuppliesLevel=prtMarkerSuppliesLevel, prtChannelEntry=prtChannelEntry, prtMarker=prtMarker, prtInputMediaLoadTimeout=prtInputMediaLoadTimeout, prtAlertTrapGroup=prtAlertTrapGroup, prtOutputStackingOrder=prtOutputStackingOrder, prtInputMediaType=prtInputMediaType, prtOutputBursting=prtOutputBursting, prtMIBCompliance=prtMIBCompliance, prtMarkerProcessColorants=prtMarkerProcessColorants, prtMIBGroups=prtMIBGroups, prtInterpreterLangFamily=prtInterpreterLangFamily, prtMediaPathMediaSizeUnit=prtMediaPathMediaSizeUnit, PrtMarkerSuppliesSupplyUnitTC=PrtMarkerSuppliesSupplyUnitTC, prtStorageRefTable=prtStorageRefTable, prtStorageRefEntry=prtStorageRefEntry, prtGeneralEntry=prtGeneralEntry, prtMarkerDefaultIndex=prtMarkerDefaultIndex, prtMarkerWestMargin=prtMarkerWestMargin, printerV2AlertPrefix=printerV2AlertPrefix, prtMarkerGroup=prtMarkerGroup, prtLocalizationIndex=prtLocalizationIndex, prtMediaPathMinMediaXFeedDir=prtMediaPathMinMediaXFeedDir, prtChannelInformation=prtChannelInformation, prtMarkerSuppliesTable=prtMarkerSuppliesTable, prtOutputMaxCapacity=prtOutputMaxCapacity, prtMarkerIndex=prtMarkerIndex, PrtMarkerSuppliesClassTC=PrtMarkerSuppliesClassTC, prtConsoleDisplayBufferIndex=prtConsoleDisplayBufferIndex, prtOutputTable=prtOutputTable, prtMarkerColorantEntry=prtMarkerColorantEntry, prtInputGroup=prtInputGroup, prtMarkerTable=prtMarkerTable, prtCoverStatus=prtCoverStatus, prtStorageRefIndex=prtStorageRefIndex, prtInputMediaDimFeedDirDeclared=prtInputMediaDimFeedDirDeclared, prtLocalizationEntry=prtLocalizationEntry, prtMediaPathMaxMediaFeedDir=prtMediaPathMaxMediaFeedDir, prtInterpreterEntry=prtInterpreterEntry, prtLocalizationLanguage=prtLocalizationLanguage, prtInterpreterTable=prtInterpreterTable, prtMediaPathMaxMediaXFeedDir=prtMediaPathMaxMediaXFeedDir, prtLocalization=prtLocalization, prtOutputEntry=prtOutputEntry, PrtLocalizedDescriptionStringTC=PrtLocalizedDescriptionStringTC, prtDeviceRefTable=prtDeviceRefTable, prtInputMediaWeight=prtInputMediaWeight, prtOutputModel=prtOutputModel, prtGeneralReset=prtGeneralReset, prtMarkerEntry=prtMarkerEntry, prtMarkerMarkTech=prtMarkerMarkTech, prtMarkerSuppliesMarkerIndex=prtMarkerSuppliesMarkerIndex, PrtSubUnitStatusTC=PrtSubUnitStatusTC, prtConsoleLights=prtConsoleLights, prtConsoleLightIndex=prtConsoleLightIndex, prtGeneralV2Group=prtGeneralV2Group, prtMediaPathIndex=prtMediaPathIndex, prtInputSecurity=prtInputSecurity, prtDeviceRefEntry=prtDeviceRefEntry, prtOutputCapacityUnit=prtOutputCapacityUnit, prtConsoleDisplayBuffer=prtConsoleDisplayBuffer, prtAlertGroup=prtAlertGroup, prtAlertCode=prtAlertCode, prtOutputSecurity=prtOutputSecurity, prtAlertCriticalEvents=prtAlertCriticalEvents, prtInterpreterGroup=prtInterpreterGroup, MediaUnit=MediaUnit, prtInputMediaName=prtInputMediaName, prtOutputDefaultIndex=prtOutputDefaultIndex, prtInterpreter=prtInterpreter, prtOutputDimensionsGroup=prtOutputDimensionsGroup, PrtPrintOrientationTC=PrtPrintOrientationTC, prtMarkerSuppliesSupplyUnit=prtMarkerSuppliesSupplyUnit, prtOutputRemainingCapacity=prtOutputRemainingCapacity, prtInputDescription=prtInputDescription, prtInterpreterXFeedAddressability=prtInterpreterXFeedAddressability, PrtMediaUnitTC=PrtMediaUnitTC, prtConsoleLocalization=prtConsoleLocalization, prtLocalizationTable=prtLocalizationTable, prtCoverEntry=prtCoverEntry, prtOutputDescription=prtOutputDescription, prtChannelState=prtChannelState, prtConsoleColor=prtConsoleColor, prtMediaPathDescription=prtMediaPathDescription, prtConsoleLightEntry=prtConsoleLightEntry, prtMarkerSuppliesDescription=prtMarkerSuppliesDescription, prtMarkerSuppliesClass=prtMarkerSuppliesClass, CapacityUnit=CapacityUnit, prtConsoleOnTime=prtConsoleOnTime, prtInputMediaDimFeedDirChosen=prtInputMediaDimFeedDirChosen, prtInputSwitchingGroup=prtInputSwitchingGroup, prtAlertIndex=prtAlertIndex, PrtChannelStateTC=PrtChannelStateTC, prtChannelGroup=prtChannelGroup, prtOutputName=prtOutputName, prtChannelTable=prtChannelTable, prtInterpreterLangVersion=prtInterpreterLangVersion, prtStorageRefSeqNumber=prtStorageRefSeqNumber, PrtMarkerAddressabilityUnitTC=PrtMarkerAddressabilityUnitTC, prtInputMediaColor=prtInputMediaColor, CodedCharSet=CodedCharSet, prtAlertGroupIndex=prtAlertGroupIndex, prtCoverTable=prtCoverTable, prtChannelType=prtChannelType, prtChannelIfIndex=prtChannelIfIndex, prtInputMediaDimXFeedDirChosen=prtInputMediaDimXFeedDirChosen, prtInterpreterLangLevel=prtInterpreterLangLevel, prtInputMediaGroup=prtInputMediaGroup, prtGeneral=prtGeneral, prtLocalizationCountry=prtLocalizationCountry, prtAlertTrainingLevel=prtAlertTrainingLevel, prtGeneralCurrentOperator=prtGeneralCurrentOperator, prtOutputDecollating=prtOutputDecollating, prtLocalizationCharacterSet=prtLocalizationCharacterSet)
mibBuilder.exportSymbols("Printer-MIB", prtInterpreterFeedAddressability=prtInterpreterFeedAddressability, prtGeneralConfigChanges=prtGeneralConfigChanges, PresentOnOff=PresentOnOff, prtConsoleDisplayBufferText=prtConsoleDisplayBufferText)
