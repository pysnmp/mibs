...
PrtInterpreterLangFamilyTC, PrtInputTypeTC, PrtMarkerSuppliesTypeTC, PrtMediaPathTypeTC, PrtChannelTypeTC, PrtConsoleDisableTC, PrtConsoleColorTC, PrtGeneralResetTC, PrtAlertCodeTC, PrtAlertTrainingLevelTC, PrtMarkerMarkTechTC, PrtOutputTypeTC, PrtAlertGroupTC, PrtCoverStatusTC = mibBuilder.importSymbols("IANA-PRINTER-MIB", "PrtInterpreterLangFamilyTC", "PrtInputTypeTC", "PrtMarkerSuppliesTypeTC", "PrtMediaPathTypeTC", "PrtChannelTypeTC", "PrtConsoleDisableTC", "PrtConsoleColorTC", "PrtGeneralResetTC", "PrtAlertCodeTC", "PrtAlertTrainingLevelTC", "PrtMarkerMarkTechTC", "PrtOutputTypeTC", "PrtAlertGroupTC", "PrtCoverStatusTC")
...
mibBuilder.exportSymbols("Printer-MIB", prtOutputDecollating=prtOutputDecollating, prtInputVendorName=prtInputVendorName, prtInterpreterVersion=prtInterpreterVersion, prtMediaPathDefaultIndex=prtMediaPathDefaultIndex, prtInputType=prtInputType, prtInterpreterIndex=prtInterpreterIndex, prtInputDescription=prtInputDescription, prtMediaPathMediaSizeUnit=prtMediaPathMediaSizeUnit, prtInputEntry=prtInputEntry, prtChannelInformation=prtChannelInformation, prtOutputSerialNumber=prtOutputSerialNumber, prtCoverEntry=prtCoverEntry, prtInterpreterLangVersion=prtInterpreterLangVersion, prtMarkerStatus=prtMarkerStatus, PrtSubUnitStatusTC=PrtSubUnitStatusTC, prtInputMediaType=prtInputMediaType, prtInputMediaDimFeedDirDeclared=prtInputMediaDimFeedDirDeclared, prtOutputMaxDimFeedDir=prtOutputMaxDimFeedDir, prtOutputCapacityUnit=prtOutputCapacityUnit, prtOutputPageDeliveryOrientation=prtOutputPageDeliveryOrientation, prtOutputPageCollated=prtOutputPageCollated, prtMarkerLifeCount=prtMarkerLifeCount, prtMarkerNorthMargin=prtMarkerNorthMargin, prtConsoleLightEntry=prtConsoleLightEntry, prtInputSecurity=prtInputSecurity, prtMarkerColorantEntry=prtMarkerColorantEntry, prtLocalizationTable=prtLocalizationTable, prtMarkerPowerOnCount=prtMarkerPowerOnCount, prtMarkerColorant=prtMarkerColorant, PrtPrintOrientationTC=PrtPrintOrientationTC, prtMarkerDefaultIndex=prtMarkerDefaultIndex, prtMediaPathMaxMediaXFeedDir=prtMediaPathMaxMediaXFeedDir, prtMediaPathDescription=prtMediaPathDescription, prtGeneralReset=prtGeneralReset, prtInputMediaWeight=prtInputMediaWeight, PrtInterpreterTwoWayTC=PrtInterpreterTwoWayTC, prtMarkerAddressabilityUnit=prtMarkerAddressabilityUnit, prtInputMediaName=prtInputMediaName, prtInterpreterDefaultCharSetOut=prtInterpreterDefaultCharSetOut, prtDeviceRefTable=prtDeviceRefTable, prtResponsiblePartyGroup=prtResponsiblePartyGroup, prtStorageRefSeqNumber=prtStorageRefSeqNumber, prtMarkerSouthMargin=prtMarkerSouthMargin, prtAlertDescription=prtAlertDescription, prtDeviceRefIndex=prtDeviceRefIndex, prtMediaPathIndex=prtMediaPathIndex, prtGeneral=prtGeneral, prtGeneralPrinterName=prtGeneralPrinterName, prtMarkerProcessColorants=prtMarkerProcessColorants, prtInterpreterDescription=prtInterpreterDescription, prtOutputEntry=prtOutputEntry, prtInputMediaDimFeedDirChosen=prtInputMediaDimFeedDirChosen, prtChannelProtocolVersion=prtChannelProtocolVersion, prtCover=prtCover, prtOutputDescription=prtOutputDescription, PYSNMP_MODULE_ID=printmib, prtInputCurrentLevel=prtInputCurrentLevel, prtChannelDefaultPageDescLangIndex=prtChannelDefaultPageDescLangIndex, prtCoverTable=prtCoverTable, prtMarkerSuppliesGroup=prtMarkerSuppliesGroup, prtMarkerIndex=prtMarkerIndex, PrtMarkerColorantRoleTC=PrtMarkerColorantRoleTC, prtMarkerAddressabilityXFeedDir=prtMarkerAddressabilityXFeedDir, prtInterpreterXFeedAddressability=prtInterpreterXFeedAddressability, printmib=printmib, prtOutputType=prtOutputType, prtInterpreterDefaultCharSetIn=prtInterpreterDefaultCharSetIn, PrtLocalizedDescriptionStringTC=PrtLocalizedDescriptionStringTC, printerV2Alert=printerV2Alert, prtMIBCompliance=prtMIBCompliance, prtChannelV2Group=prtChannelV2Group, prtInputMediaGroup=prtInputMediaGroup, prtOutputTable=prtOutputTable, prtChannelGroup=prtChannelGroup, prtOutputName=prtOutputName, prtConsoleColor=prtConsoleColor, prtAlertIndex=prtAlertIndex, prtAlertCode=prtAlertCode, printerV1Alert=printerV1Alert, prtInterpreterLangFamily=prtInterpreterLangFamily, prtGeneralV2Group=prtGeneralV2Group, prtAlertTrapGroup=prtAlertTrapGroup, prtMIB2Compliance=prtMIB2Compliance, prtInterpreterEntry=prtInterpreterEntry, prtInterpreterTwoWay=prtInterpreterTwoWay, prtInputSerialNumber=prtInputSerialNumber, prtChannelIndex=prtChannelIndex, prtOutputStackingOrder=prtOutputStackingOrder, prtGeneralCurrentOperator=prtGeneralCurrentOperator, prtInterpreterTable=prtInterpreterTable, prtAlertTrainingLevel=prtAlertTrainingLevel, prtMarkerEntry=prtMarkerEntry, prtInputMaxCapacity=prtInputMaxCapacity, prtInputStatus=prtInputStatus, prtOutputStatus=prtOutputStatus, prtMarkerEastMargin=prtMarkerEastMargin, prtMarkerSpotColorants=prtMarkerSpotColorants, prtCoverIndex=prtCoverIndex, prtConsoleDisplayBuffer=prtConsoleDisplayBuffer, prtOutputFeaturesGroup=prtOutputFeaturesGroup, prtMarkerColorantRole=prtMarkerColorantRole, prtMediaPathTable=prtMediaPathTable, prtChannelState=prtChannelState, prtOutput=prtOutput, PrtMediaUnitTC=PrtMediaUnitTC, SubUnitStatus=SubUnitStatus, prtAlertAllEvents=prtAlertAllEvents, prtMediaPathMaxMediaFeedDir=prtMediaPathMaxMediaFeedDir, prtGeneralSerialNumber=prtGeneralSerialNumber, prtLocalizationEntry=prtLocalizationEntry, prtOutputMinDimFeedDir=prtOutputMinDimFeedDir, prtMIBGroups=prtMIBGroups, prtInputDimUnit=prtInputDimUnit, prtOutputMaxCapacity=prtOutputMaxCapacity, prtAuxiliarySheetBannerPage=prtAuxiliarySheetBannerPage, prtOutputDimensionsGroup=prtOutputDimensionsGroup, prtConsoleNumberOfDisplayChars=prtConsoleNumberOfDisplayChars, prtStorageRefIndex=prtStorageRefIndex, prtMarkerSuppliesColorantIndex=prtMarkerSuppliesColorantIndex, prtInputVersion=prtInputVersion, prtChannelTable=prtChannelTable, prtChannelStatus=prtChannelStatus, prtOutputOffsetStacking=prtOutputOffsetStacking, prtMediaPathStatus=prtMediaPathStatus, prtConsoleOffTime=prtConsoleOffTime, prtExtendedInputGroup=prtExtendedInputGroup, prtMarkerCounterUnit=prtMarkerCounterUnit, prtInputModel=prtInputModel, prtInterpreter=prtInterpreter, prtInterpreterFeedAddressability=prtInterpreterFeedAddressability, prtLocalizationIndex=prtLocalizationIndex, prtLocalizationLanguage=prtLocalizationLanguage, prtInputCapacityUnit=prtInputCapacityUnit, prtMediaPathMinMediaFeedDir=prtMediaPathMinMediaFeedDir, prtMIBConformance=prtMIBConformance, prtOutputMaxDimXFeedDir=prtOutputMaxDimXFeedDir, prtLocalizationCharacterSet=prtLocalizationCharacterSet, prtInputMediaDimXFeedDirDeclared=prtInputMediaDimXFeedDirDeclared, prtConsoleDisable=prtConsoleDisable, prtMarkerWestMargin=prtMarkerWestMargin, prtDeviceRefEntry=prtDeviceRefEntry, prtConsoleLightTable=prtConsoleLightTable, prtMarkerMarkTech=prtMarkerMarkTech, prtMediaPathMaxSpeed=prtMediaPathMaxSpeed, prtLocalization=prtLocalization, prtInputName=prtInputName, prtMarkerSuppliesMaxCapacity=prtMarkerSuppliesMaxCapacity, prtCoverStatus=prtCoverStatus, prtConsoleDisplayBufferTable=prtConsoleDisplayBufferTable, prtMarkerSuppliesMarkerIndex=prtMarkerSuppliesMarkerIndex, prtInputDefaultIndex=prtInputDefaultIndex, prtAuxiliarySheetStartupPage=prtAuxiliarySheetStartupPage, prtOutputVersion=prtOutputVersion, prtOutputMinDimXFeedDir=prtOutputMinDimXFeedDir, prtOutputGroup=prtOutputGroup, prtMarkerSuppliesEntry=prtMarkerSuppliesEntry, prtMediaPathEntry=prtMediaPathEntry, prtGeneralTable=prtGeneralTable, prtChannelIfIndex=prtChannelIfIndex, PrtMarkerSuppliesClassTC=PrtMarkerSuppliesClassTC, CapacityUnit=CapacityUnit, prtInput=prtInput, prtInputSwitchingGroup=prtInputSwitchingGroup, PrtMarkerAddressabilityUnitTC=PrtMarkerAddressabilityUnitTC, prtMarkerColorantGroup=prtMarkerColorantGroup, prtConsoleGroup=prtConsoleGroup, prtMarkerSuppliesIndex=prtMarkerSuppliesIndex, prtConsoleOnTime=prtConsoleOnTime, prtMarkerSuppliesLevel=prtMarkerSuppliesLevel, PrtCapacityUnitTC=PrtCapacityUnitTC, prtMarkerColorantTable=prtMarkerColorantTable, PrtChannelStateTC=PrtChannelStateTC, prtConsoleDescription=prtConsoleDescription, prtOutputVendorName=prtOutputVendorName, prtInputTable=prtInputTable, prtMarkerSuppliesTable=prtMarkerSuppliesTable, prtExtendedOutputGroup=prtExtendedOutputGroup, prtMarkerGroup=prtMarkerGroup, PrtMarkerSuppliesSupplyUnitTC=PrtMarkerSuppliesSupplyUnitTC, prtConsoleDisplayBufferText=prtConsoleDisplayBufferText, prtInterpreterLangLevel=prtInterpreterLangLevel, prtCoverDescription=prtCoverDescription, prtOutputBursting=prtOutputBursting, prtMarkerAddressabilityFeedDir=prtMarkerAddressabilityFeedDir, prtMediaPathType=prtMediaPathType, prtMIB2Groups=prtMIB2Groups, prtAlertTableV2Group=prtAlertTableV2Group, prtAlertGroup=prtAlertGroup, prtMarkerSuppliesDescription=prtMarkerSuppliesDescription, printerV2AlertPrefix=printerV2AlertPrefix, prtMarkerColorantTonality=prtMarkerColorantTonality, prtMediaPathMinMediaXFeedDir=prtMediaPathMinMediaXFeedDir, prtMarker=prtMarker, prtMarkerColorantValue=prtMarkerColorantValue, prtAlertLocation=prtAlertLocation, PrtConsoleDescriptionStringTC=PrtConsoleDescriptionStringTC, prtOutputRemainingCapacity=prtOutputRemainingCapacity, prtInputMediaColor=prtInputMediaColor, prtInputNextIndex=prtInputNextIndex, prtConsoleLights=prtConsoleLights, prtMarkerSuppliesSupplyUnit=prtMarkerSuppliesSupplyUnit, prtOutputModel=prtOutputModel, MediaUnit=MediaUnit, prtLocalizationCountry=prtLocalizationCountry, prtInterpreterGroup=prtInterpreterGroup, prtConsoleLightIndex=prtConsoleLightIndex, PrtAlertSeverityLevelTC=PrtAlertSeverityLevelTC, prtInputMediaFormParts=prtInputMediaFormParts, PresentOnOff=PresentOnOff, prtInterpreterDefaultOrientation=prtInterpreterDefaultOrientation, PrtOutputPageDeliveryOrientationTC=PrtOutputPageDeliveryOrientationTC, prtConsoleLocalization=prtConsoleLocalization, prtConsoleDisplayBufferEntry=prtConsoleDisplayBufferEntry, prtGeneralEntry=prtGeneralEntry, prtConsoleNumberOfDisplayLines=prtConsoleNumberOfDisplayLines, prtOutputDimUnit=prtOutputDimUnit, prtAlertEntry=prtAlertEntry, prtInputMediaDimXFeedDirChosen=prtInputMediaDimXFeedDirChosen, prtAlertSeverityLevel=prtAlertSeverityLevel, prtInputIndex=prtInputIndex, prtChannelType=prtChannelType, prtOutputIndex=prtOutputIndex, prtGeneralServicePerson=prtGeneralServicePerson, PrtOutputStackingOrderTC=PrtOutputStackingOrderTC, prtMediaPathGroup=prtMediaPathGroup, prtGeneralGroup=prtGeneralGroup, prtGeneralConfigChanges=prtGeneralConfigChanges, prtMarkerColorantMarkerIndex=prtMarkerColorantMarkerIndex, prtOutputDefaultIndex=prtOutputDefaultIndex, prtMarkerSupplies=prtMarkerSupplies, prtAlertTable=prtAlertTable, prtMarkerTable=prtMarkerTable, prtDeviceRefSeqNumber=prtDeviceRefSeqNumber, PrtMarkerCounterUnitTC=PrtMarkerCounterUnitTC, prtStorageRefTable=prtStorageRefTable, prtMarkerColorantIndex=prtMarkerColorantIndex, prtGeneralCurrentLocalization=prtGeneralCurrentLocalization, prtAlert=prtAlert, prtInputMediaLoadTimeout=prtInputMediaLoadTimeout, prtAlertTableGroup=prtAlertTableGroup, PrtMediaPathMaxSpeedPrintUnitTC=PrtMediaPathMaxSpeedPrintUnitTC, prtChannelEntry=prtChannelEntry, prtAlertCriticalEvents=prtAlertCriticalEvents, prtStorageRefEntry=prtStorageRefEntry, prtChannelCurrentJobCntlLangIndex=prtChannelCurrentJobCntlLangIndex, CodedCharSet=CodedCharSet, prtAuxiliarySheetGroup=prtAuxiliarySheetGroup, prtChannel=prtChannel, prtMediaPathMaxSpeedPrintUnit=prtMediaPathMaxSpeedPrintUnit, prtAlertTimeGroup=prtAlertTimeGroup, prtOutputSecurity=prtOutputSecurity, prtConsoleDisplayBufferIndex=prtConsoleDisplayBufferIndex, prtMarkerSuppliesClass=prtMarkerSuppliesClass, prtMarkerSuppliesType=prtMarkerSuppliesType)
mibBuilder.exportSymbols("Printer-MIB", prtInputGroup=prtInputGroup, prtMediaPath=prtMediaPath, prtAlertGroupIndex=prtAlertGroupIndex, prtAlertTime=prtAlertTime)
