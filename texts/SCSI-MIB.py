...
Gauge32, NotificationType, Unsigned32, Counter64, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, Bits, ModuleIdentity, MibIdentifier, mib_2, TimeTicks, IpAddress, iso, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "NotificationType", "Unsigned32", "Counter64", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "Bits", "ModuleIdentity", "MibIdentifier", "mib-2", "TimeTicks", "IpAddress", "iso", "Counter32")
...
mibBuilder.exportSymbols("SCSI-MIB", scsiLuRevisionId=scsiLuRevisionId, scsiPortEntry=scsiPortEntry, scsiAuthIntrLastCreation=scsiAuthIntrLastCreation, scsiDiscoveryHSStatsGroup=scsiDiscoveryHSStatsGroup, scsiDevicePortNumber=scsiDevicePortNumber, scsiDeviceAlias=scsiDeviceAlias, scsiTgtDevEntry=scsiTgtDevEntry, scsiInstIndex=scsiInstIndex, scsiLuInResets=scsiLuInResets, scsiDscTgtInCommands=scsiDscTgtInCommands, scsiTransportFCP=scsiTransportFCP, scsiTargetHSStatsGroup=scsiTargetHSStatsGroup, scsiIntrDevEntry=scsiIntrDevEntry, scsiAuthIntrDevOrPort=scsiAuthIntrDevOrPort, scsiGroups=scsiGroups, scsiDscTgtHSInCommands=scsiDscTgtHSInCommands, scsiAttTgtPortName=scsiAttTgtPortName, scsiDscTgtName=scsiDscTgtName, scsiAuthIntrLunMapIndex=scsiAuthIntrLunMapIndex, scsiLunMapGroup=scsiLunMapGroup, scsiLuOutQueueFullStatus=scsiLuOutQueueFullStatus, scsiIntrPrtOutCommands=scsiIntrPrtOutCommands, scsiAuthorizedIntrTable=scsiAuthorizedIntrTable, scsiLogicalUnit=scsiLogicalUnit, scsiObjects=scsiObjects, ScsiIndexValue=ScsiIndexValue, ScsiDeviceOrPort=ScsiDeviceOrPort, scsiDeviceTable=scsiDeviceTable, scsiAuthIntrWrittenMegaBytes=scsiAuthIntrWrittenMegaBytes, scsiAuthIntrRowStatus=scsiAuthIntrRowStatus, scsiRemoteInitiators=scsiRemoteInitiators, scsiTgtPortWrittenMegaBytes=scsiTgtPortWrittenMegaBytes, scsiInitiatorHSStatsGroup=scsiInitiatorHSStatsGroup, scsiDiscoveryStatsGroup=scsiDiscoveryStatsGroup, ScsiIdCodeSet=ScsiIdCodeSet, scsiAttIntrPrtId=scsiAttIntrPrtId, scsiLuIndex=scsiLuIndex, scsiLuIdType=scsiLuIdType, scsiLuWrittenMegaBytes=scsiLuWrittenMegaBytes, scsiAuthIntrName=scsiAuthIntrName, ScsiIdentifier=ScsiIdentifier, scsiTgtPortName=scsiTgtPortName, PYSNMP_MODULE_ID=scsiModule, scsiDscTgtIndex=scsiDscTgtIndex, scsiLunMapIndex=scsiLunMapIndex, scsiAuthorizedIntrEntry=scsiAuthorizedIntrEntry, ScsiIdType=ScsiIdType, scsiTgtPortReadMegaBytes=scsiTgtPortReadMegaBytes, scsiTransportTypes=scsiTransportTypes, scsiTgtPortInCommands=scsiTgtPortInCommands, scsiTranportSPI=scsiTranportSPI, scsiLuIdCodeSet=scsiLuIdCodeSet, scsiLunMapRowStatus=scsiLunMapRowStatus, scsiDscTgtDevOrPort=scsiDscTgtDevOrPort, scsiTgtPortEntry=scsiTgtPortEntry, ScsiNameIdOrZero=ScsiNameIdOrZero, scsiLuState=scsiLuState, scsiAuthIntrIndex=scsiAuthIntrIndex, scsiTargetStatsGroup=scsiTargetStatsGroup, scsiDscLunIndex=scsiDscLunIndex, scsiDscLunTable=scsiDscLunTable, scsiDscLunLun=scsiDscLunLun, scsiPortTrnsptPtr=scsiPortTrnsptPtr, scsiDscLunIdAssociation=scsiDscLunIdAssociation, scsiLunMapLun=scsiLunMapLun, scsiDscTgtReadMegaBytes=scsiDscTgtReadMegaBytes, scsiLuHSInCommands=scsiLuHSInCommands, scsiModule=scsiModule, scsiDscTgtLastCreation=scsiDscTgtLastCreation, scsiTrnsptIndex=scsiTrnsptIndex, scsiDeviceStatGroup=scsiDeviceStatGroup, scsiTgtDeviceStatus=scsiTgtDeviceStatus, scsiTargetGroup=scsiTargetGroup, scsiPortBusyStatuses=scsiPortBusyStatuses, scsiLunMapLuIndex=scsiLunMapLuIndex, scsiDeviceEntry=scsiDeviceEntry, ScsiIdValue=ScsiIdValue, scsiRemoteTarget=scsiRemoteTarget, scsiTgtDevNonAccessibleLUs=scsiTgtDevNonAccessibleLUs, scsiTranportOther=scsiTranportOther, scsiDiscoveryGroup=scsiDiscoveryGroup, scsiDscLunIdValue=scsiDscLunIdValue, scsiDscTgtTable=scsiDscTgtTable, scsiAuthIntrHSOutCommands=scsiAuthIntrHSOutCommands, scsiAuthIntrAttachedTimes=scsiAuthIntrAttachedTimes, scsiDscLunIdIndex=scsiDscLunIdIndex, scsiLuWwnName=scsiLuWwnName, scsiTgtDevTable=scsiTgtDevTable, ScsiLUNOrZero=ScsiLUNOrZero, scsiAttIntrPrtTable=scsiAttIntrPrtTable, scsiAttTgtPortDscTgtIdx=scsiAttTgtPortDscTgtIdx, scsiAttIntrPrtIdx=scsiAttIntrPrtIdx, scsiPortRole=scsiPortRole, scsiCompliance=scsiCompliance, HrSWInstalledIndexOrZero=HrSWInstalledIndexOrZero, scsiTrnsptTable=scsiTrnsptTable, scsiTrnsptDevName=scsiTrnsptDevName, scsiPortTable=scsiPortTable, ScsiPortIndexValueOrZero=ScsiPortIndexValueOrZero, scsiLuIdEntry=scsiLuIdEntry, scsiLuStatusChanged=scsiLuStatusChanged, scsiDeviceRole=scsiDeviceRole, scsiTgtPortIdentifier=scsiTgtPortIdentifier, scsiAttIntrPrtAuthIntrIdx=scsiAttIntrPrtAuthIntrIdx, scsiPortIndex=scsiPortIndex, scsiDscTgtConfigured=scsiDscTgtConfigured, scsiLuDefaultLun=scsiLuDefaultLun, scsiLuProductId=scsiLuProductId, scsiLuIdTable=scsiLuIdTable, scsiLuVendorId=scsiLuVendorId, scsiLuTable=scsiLuTable, scsiAuthIntrReadMegaBytes=scsiAuthIntrReadMegaBytes, scsiLuStatus=scsiLuStatus, scsiGeneral=scsiGeneral, scsiTrnsptPointer=scsiTrnsptPointer, scsiIntrPrtTable=scsiIntrPrtTable, scsiInstAlias=scsiInstAlias, scsiIntrDevTgtAccessMode=scsiIntrDevTgtAccessMode, scsiInitiatorGroup=scsiInitiatorGroup, scsiDscLunEntry=scsiDscLunEntry, ScsiIndexValueOrZero=ScsiIndexValueOrZero, scsiTarget=scsiTarget, scsiIntrPrtWrittenMegaBytes=scsiIntrPrtWrittenMegaBytes, scsiAttIntrPrtName=scsiAttIntrPrtName, scsiInstScsiNotificationsEnable=scsiInstScsiNotificationsEnable, scsiAttTgtPortIndex=scsiAttTgtPortIndex, scsiAttTgtPortTable=scsiAttTgtPortTable, scsiTransportSRP=scsiTransportSRP, scsiLuEntry=scsiLuEntry, scsiIntrPrtEntry=scsiIntrPrtEntry, scsiIntrPrtReadMegaBytes=scsiIntrPrtReadMegaBytes, scsiTrnsptType=scsiTrnsptType, scsiInitiatorStatsGroup=scsiInitiatorStatsGroup, scsiTgtPortTable=scsiTgtPortTable, scsiLuIdIndex=scsiLuIdIndex, ScsiName=ScsiName, scsiIntrPrtIdentifier=scsiIntrPrtIdentifier, scsiTgtDevNumberOfLUs=scsiTgtDevNumberOfLUs, scsiTransportSBP=scsiTransportSBP, scsiLuPeripheralType=scsiLuPeripheralType, scsiTgtDeviceStatusChanged=scsiTgtDeviceStatusChanged, scsiIntrDevOutResets=scsiIntrDevOutResets, scsiTransportISCSI=scsiTransportISCSI, scsiIntrDevTable=scsiIntrDevTable, scsiDscTgtIntrPortIndex=scsiDscTgtIntrPortIndex, scsiDeviceGroup=scsiDeviceGroup, scsiLuReadMegaBytes=scsiLuReadMegaBytes, scsiDscTgtEntry=scsiDscTgtEntry, scsiLunMapStatsGroup=scsiLunMapStatsGroup, scsiLuIdValue=scsiLuIdValue, scsiDeviceIndex=scsiDeviceIndex, scsiTrnsptEntry=scsiTrnsptEntry, scsiLuIdAssociation=scsiLuIdAssociation, scsiDeviceResets=scsiDeviceResets, scsiAttTgtPortEntry=scsiAttTgtPortEntry, scsiInstVendorVersion=scsiInstVendorVersion, scsiNotifications=scsiNotifications, scsiDscLunIdCodeSet=scsiDscLunIdCodeSet, scsiDscLunIdType=scsiDscLunIdType, scsiDscTgtRowStatus=scsiDscTgtRowStatus, scsiLunMapTable=scsiLunMapTable, scsiDscLunIdEntry=scsiDscLunIdEntry, scsiInstanceTable=scsiInstanceTable, scsiDscTgtDiscovered=scsiDscTgtDiscovered, scsiLunMapEntry=scsiLunMapEntry, scsiNotificationsPrefix=scsiNotificationsPrefix, scsiConformance=scsiConformance, scsiInstSoftwareIndex=scsiInstSoftwareIndex, ScsiIdAssociation=ScsiIdAssociation, scsiInitiator=scsiInitiator, scsiLunMapHSStatsGroup=scsiLunMapHSStatsGroup, scsiAuthIntrTgtPortIndex=scsiAuthIntrTgtPortIndex, scsiCompliances=scsiCompliances, scsiDscLunIdTable=scsiDscLunIdTable, scsiAttTgtPortIdentifier=scsiAttTgtPortIdentifier, scsiAttIntrPrtEntry=scsiAttIntrPrtEntry, scsiLuInCommands=scsiLuInCommands, scsiTgtPortHSInCommands=scsiTgtPortHSInCommands, scsiIntrPrtName=scsiIntrPrtName, scsiInstanceEntry=scsiInstanceEntry, scsiIntrPrtHSOutCommands=scsiIntrPrtHSOutCommands, scsiDscTgtWrittenMegaBytes=scsiDscTgtWrittenMegaBytes, scsiAuthIntrOutCommands=scsiAuthIntrOutCommands)
