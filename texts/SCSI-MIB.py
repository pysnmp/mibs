...
MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, TimeTicks, Counter32, ModuleIdentity, Unsigned32, MibIdentifier, NotificationType, Counter64, Gauge32, IpAddress, Bits, mib_2, iso, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "TimeTicks", "Counter32", "ModuleIdentity", "Unsigned32", "MibIdentifier", "NotificationType", "Counter64", "Gauge32", "IpAddress", "Bits", "mib-2", "iso", "Integer32")
AutonomousType, TruthValue, RowStatus, DisplayString, TimeStamp, TextualConvention, RowPointer = mibBuilder.importSymbols("SNMPv2-TC", "AutonomousType", "TruthValue", "RowStatus", "DisplayString", "TimeStamp", "TextualConvention", "RowPointer")
...
mibBuilder.exportSymbols("SCSI-MIB", scsiIntrPrtHSOutCommands=scsiIntrPrtHSOutCommands, scsiDscLunIdCodeSet=scsiDscLunIdCodeSet, scsiIntrPrtIdentifier=scsiIntrPrtIdentifier, scsiAuthIntrIndex=scsiAuthIntrIndex, ScsiPortIndexValueOrZero=ScsiPortIndexValueOrZero, scsiRemoteInitiators=scsiRemoteInitiators, scsiTargetStatsGroup=scsiTargetStatsGroup, ScsiIdValue=ScsiIdValue, scsiAuthorizedIntrTable=scsiAuthorizedIntrTable, scsiLogicalUnit=scsiLogicalUnit, scsiAttIntrPrtName=scsiAttIntrPrtName, scsiDscLunIdAssociation=scsiDscLunIdAssociation, scsiLuIdType=scsiLuIdType, scsiDeviceIndex=scsiDeviceIndex, scsiDevicePortNumber=scsiDevicePortNumber, scsiLuDefaultLun=scsiLuDefaultLun, scsiDscTgtEntry=scsiDscTgtEntry, scsiLunMapIndex=scsiLunMapIndex, scsiIntrPrtTable=scsiIntrPrtTable, scsiDscTgtLastCreation=scsiDscTgtLastCreation, scsiInitiatorGroup=scsiInitiatorGroup, scsiInstAlias=scsiInstAlias, scsiIntrPrtReadMegaBytes=scsiIntrPrtReadMegaBytes, scsiDiscoveryGroup=scsiDiscoveryGroup, scsiLunMapGroup=scsiLunMapGroup, scsiCompliances=scsiCompliances, scsiTgtPortHSInCommands=scsiTgtPortHSInCommands, scsiLunMapStatsGroup=scsiLunMapStatsGroup, scsiTrnsptTable=scsiTrnsptTable, scsiGeneral=scsiGeneral, scsiTransportISCSI=scsiTransportISCSI, scsiDscTgtDevOrPort=scsiDscTgtDevOrPort, scsiDscTgtConfigured=scsiDscTgtConfigured, ScsiIdAssociation=ScsiIdAssociation, scsiDeviceTable=scsiDeviceTable, scsiDeviceAlias=scsiDeviceAlias, scsiTarget=scsiTarget, scsiTgtDevEntry=scsiTgtDevEntry, scsiTransportFCP=scsiTransportFCP, scsiAuthIntrName=scsiAuthIntrName, scsiLuIdCodeSet=scsiLuIdCodeSet, scsiTgtPortName=scsiTgtPortName, scsiTrnsptType=scsiTrnsptType, ScsiIndexValueOrZero=ScsiIndexValueOrZero, scsiAuthIntrAttachedTimes=scsiAuthIntrAttachedTimes, scsiAuthIntrTgtPortIndex=scsiAuthIntrTgtPortIndex, scsiLuWrittenMegaBytes=scsiLuWrittenMegaBytes, ScsiIdType=ScsiIdType, scsiLuState=scsiLuState, scsiLunMapEntry=scsiLunMapEntry, scsiIntrDevTable=scsiIntrDevTable, scsiLuIdIndex=scsiLuIdIndex, scsiAttIntrPrtEntry=scsiAttIntrPrtEntry, scsiPortBusyStatuses=scsiPortBusyStatuses, scsiLuReadMegaBytes=scsiLuReadMegaBytes, ScsiDeviceOrPort=ScsiDeviceOrPort, ScsiIndexValue=ScsiIndexValue, scsiInstSoftwareIndex=scsiInstSoftwareIndex, scsiLuIdEntry=scsiLuIdEntry, scsiConformance=scsiConformance, scsiAuthIntrLastCreation=scsiAuthIntrLastCreation, scsiPortEntry=scsiPortEntry, scsiLuStatus=scsiLuStatus, scsiDscTgtIndex=scsiDscTgtIndex, scsiDscLunIndex=scsiDscLunIndex, scsiTgtDeviceStatusChanged=scsiTgtDeviceStatusChanged, scsiIntrDevEntry=scsiIntrDevEntry, scsiAttIntrPrtId=scsiAttIntrPrtId, scsiDscLunIdTable=scsiDscLunIdTable, scsiAttTgtPortIdentifier=scsiAttTgtPortIdentifier, scsiDscTgtInCommands=scsiDscTgtInCommands, scsiAuthIntrWrittenMegaBytes=scsiAuthIntrWrittenMegaBytes, scsiTgtDevTable=scsiTgtDevTable, scsiModule=scsiModule, scsiDeviceEntry=scsiDeviceEntry, scsiDscLunIdIndex=scsiDscLunIdIndex, scsiTransportSRP=scsiTransportSRP, scsiInstIndex=scsiInstIndex, scsiLunMapLun=scsiLunMapLun, scsiLuWwnName=scsiLuWwnName, scsiDiscoveryStatsGroup=scsiDiscoveryStatsGroup, scsiNotifications=scsiNotifications, scsiInstanceEntry=scsiInstanceEntry, scsiTgtDevNonAccessibleLUs=scsiTgtDevNonAccessibleLUs, scsiDscTgtDiscovered=scsiDscTgtDiscovered, scsiAuthIntrRowStatus=scsiAuthIntrRowStatus, scsiLuHSInCommands=scsiLuHSInCommands, scsiInstScsiNotificationsEnable=scsiInstScsiNotificationsEnable, scsiDeviceRole=scsiDeviceRole, scsiDscLunIdType=scsiDscLunIdType, scsiTrnsptDevName=scsiTrnsptDevName, scsiLunMapHSStatsGroup=scsiLunMapHSStatsGroup, scsiLuTable=scsiLuTable, scsiInstVendorVersion=scsiInstVendorVersion, scsiDeviceStatGroup=scsiDeviceStatGroup, ScsiNameIdOrZero=ScsiNameIdOrZero, scsiAttTgtPortDscTgtIdx=scsiAttTgtPortDscTgtIdx, scsiObjects=scsiObjects, scsiLuIdAssociation=scsiLuIdAssociation, scsiRemoteTarget=scsiRemoteTarget, scsiTrnsptIndex=scsiTrnsptIndex, scsiDscLunEntry=scsiDscLunEntry, scsiAttIntrPrtTable=scsiAttIntrPrtTable, scsiDscTgtReadMegaBytes=scsiDscTgtReadMegaBytes, scsiTgtDeviceStatus=scsiTgtDeviceStatus, scsiDscTgtIntrPortIndex=scsiDscTgtIntrPortIndex, scsiTargetHSStatsGroup=scsiTargetHSStatsGroup, scsiDiscoveryHSStatsGroup=scsiDiscoveryHSStatsGroup, scsiTgtPortInCommands=scsiTgtPortInCommands, scsiAttTgtPortEntry=scsiAttTgtPortEntry, scsiPortRole=scsiPortRole, scsiTgtPortTable=scsiTgtPortTable, scsiPortTable=scsiPortTable, scsiDscLunTable=scsiDscLunTable, scsiTgtDevNumberOfLUs=scsiTgtDevNumberOfLUs, scsiLunMapLuIndex=scsiLunMapLuIndex, scsiLunMapRowStatus=scsiLunMapRowStatus, scsiGroups=scsiGroups, scsiDscLunIdEntry=scsiDscLunIdEntry, scsiInitiatorHSStatsGroup=scsiInitiatorHSStatsGroup, scsiAttIntrPrtAuthIntrIdx=scsiAttIntrPrtAuthIntrIdx, scsiTrnsptPointer=scsiTrnsptPointer, scsiTargetGroup=scsiTargetGroup, scsiLuPeripheralType=scsiLuPeripheralType, scsiDscTgtTable=scsiDscTgtTable, scsiLuIdTable=scsiLuIdTable, scsiAttTgtPortName=scsiAttTgtPortName, scsiAuthIntrOutCommands=scsiAuthIntrOutCommands, scsiTgtPortReadMegaBytes=scsiTgtPortReadMegaBytes, scsiLuProductId=scsiLuProductId, scsiLuRevisionId=scsiLuRevisionId, scsiDeviceGroup=scsiDeviceGroup, scsiPortIndex=scsiPortIndex, scsiDscLunLun=scsiDscLunLun, scsiAuthIntrDevOrPort=scsiAuthIntrDevOrPort, scsiAuthorizedIntrEntry=scsiAuthorizedIntrEntry, scsiCompliance=scsiCompliance, scsiDscLunIdValue=scsiDscLunIdValue, scsiAttTgtPortTable=scsiAttTgtPortTable, scsiLuIndex=scsiLuIndex, scsiLuVendorId=scsiLuVendorId, PYSNMP_MODULE_ID=scsiModule, scsiAuthIntrHSOutCommands=scsiAuthIntrHSOutCommands, scsiTgtPortWrittenMegaBytes=scsiTgtPortWrittenMegaBytes, scsiLuStatusChanged=scsiLuStatusChanged, scsiTransportSBP=scsiTransportSBP, scsiAuthIntrReadMegaBytes=scsiAuthIntrReadMegaBytes, scsiLunMapTable=scsiLunMapTable, scsiTransportTypes=scsiTransportTypes, scsiTranportOther=scsiTranportOther, ScsiIdentifier=ScsiIdentifier, scsiIntrPrtEntry=scsiIntrPrtEntry, scsiLuInCommands=scsiLuInCommands, scsiIntrDevOutResets=scsiIntrDevOutResets, scsiLuOutQueueFullStatus=scsiLuOutQueueFullStatus, scsiLuInResets=scsiLuInResets, scsiIntrPrtWrittenMegaBytes=scsiIntrPrtWrittenMegaBytes, scsiDscTgtRowStatus=scsiDscTgtRowStatus, scsiAuthIntrLunMapIndex=scsiAuthIntrLunMapIndex, scsiTranportSPI=scsiTranportSPI, scsiInitiatorStatsGroup=scsiInitiatorStatsGroup, ScsiName=ScsiName, scsiDeviceResets=scsiDeviceResets, scsiInstanceTable=scsiInstanceTable, scsiLuIdValue=scsiLuIdValue, ScsiIdCodeSet=ScsiIdCodeSet, scsiPortTrnsptPtr=scsiPortTrnsptPtr, scsiDscTgtName=scsiDscTgtName, ScsiLUNOrZero=ScsiLUNOrZero, scsiDscTgtWrittenMegaBytes=scsiDscTgtWrittenMegaBytes, scsiLuEntry=scsiLuEntry, HrSWInstalledIndexOrZero=HrSWInstalledIndexOrZero, scsiTgtPortIdentifier=scsiTgtPortIdentifier, scsiIntrDevTgtAccessMode=scsiIntrDevTgtAccessMode, scsiNotificationsPrefix=scsiNotificationsPrefix, scsiAttIntrPrtIdx=scsiAttIntrPrtIdx, scsiAttTgtPortIndex=scsiAttTgtPortIndex, scsiInitiator=scsiInitiator, scsiDscTgtHSInCommands=scsiDscTgtHSInCommands, scsiTgtPortEntry=scsiTgtPortEntry, scsiIntrPrtOutCommands=scsiIntrPrtOutCommands, scsiIntrPrtName=scsiIntrPrtName, scsiTrnsptEntry=scsiTrnsptEntry)
