...
TimeStamp, TruthValue, RowPointer, TextualConvention, DisplayString, AutonomousType, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "RowPointer", "TextualConvention", "DisplayString", "AutonomousType", "RowStatus")
...
mibBuilder.exportSymbols("SCSI-MIB", scsiInitiatorStatsGroup=scsiInitiatorStatsGroup, scsiLuStatus=scsiLuStatus, ScsiNameIdOrZero=ScsiNameIdOrZero, scsiDscLunIdType=scsiDscLunIdType, scsiAuthIntrLastCreation=scsiAuthIntrLastCreation, scsiAttTgtPortTable=scsiAttTgtPortTable, scsiAttIntrPrtName=scsiAttIntrPrtName, scsiDscLunLun=scsiDscLunLun, scsiIntrPrtWrittenMegaBytes=scsiIntrPrtWrittenMegaBytes, scsiDscTgtInCommands=scsiDscTgtInCommands, scsiNotificationsPrefix=scsiNotificationsPrefix, scsiLunMapEntry=scsiLunMapEntry, scsiAuthIntrName=scsiAuthIntrName, scsiDscLunIdAssociation=scsiDscLunIdAssociation, ScsiIdValue=ScsiIdValue, scsiAuthIntrRowStatus=scsiAuthIntrRowStatus, scsiDscTgtDiscovered=scsiDscTgtDiscovered, ScsiLUNOrZero=ScsiLUNOrZero, scsiLuInResets=scsiLuInResets, scsiRemoteInitiators=scsiRemoteInitiators, scsiTransportFCP=scsiTransportFCP, scsiLunMapRowStatus=scsiLunMapRowStatus, scsiInstScsiNotificationsEnable=scsiInstScsiNotificationsEnable, scsiInstIndex=scsiInstIndex, HrSWInstalledIndexOrZero=HrSWInstalledIndexOrZero, scsiAuthIntrOutCommands=scsiAuthIntrOutCommands, scsiCompliance=scsiCompliance, scsiTransportISCSI=scsiTransportISCSI, scsiLunMapTable=scsiLunMapTable, scsiLunMapLuIndex=scsiLunMapLuIndex, scsiLuInCommands=scsiLuInCommands, PYSNMP_MODULE_ID=scsiModule, scsiDeviceTable=scsiDeviceTable, scsiAuthIntrHSOutCommands=scsiAuthIntrHSOutCommands, scsiLuDefaultLun=scsiLuDefaultLun, scsiPortIndex=scsiPortIndex, scsiInitiatorHSStatsGroup=scsiInitiatorHSStatsGroup, scsiIntrDevTable=scsiIntrDevTable, scsiPortTable=scsiPortTable, scsiTgtDeviceStatus=scsiTgtDeviceStatus, scsiLuPeripheralType=scsiLuPeripheralType, scsiLuEntry=scsiLuEntry, scsiDeviceResets=scsiDeviceResets, scsiLuIdValue=scsiLuIdValue, scsiPortRole=scsiPortRole, scsiLuProductId=scsiLuProductId, scsiPortEntry=scsiPortEntry, scsiDeviceStatGroup=scsiDeviceStatGroup, scsiAttIntrPrtId=scsiAttIntrPrtId, scsiDscLunIdValue=scsiDscLunIdValue, ScsiIdentifier=ScsiIdentifier, scsiLogicalUnit=scsiLogicalUnit, scsiLuState=scsiLuState, scsiPortTrnsptPtr=scsiPortTrnsptPtr, scsiDscLunIdCodeSet=scsiDscLunIdCodeSet, scsiTgtPortTable=scsiTgtPortTable, ScsiDeviceOrPort=ScsiDeviceOrPort, scsiLuIdTable=scsiLuIdTable, scsiTransportTypes=scsiTransportTypes, scsiLuRevisionId=scsiLuRevisionId, scsiAuthorizedIntrTable=scsiAuthorizedIntrTable, scsiInstVendorVersion=scsiInstVendorVersion, scsiTgtDevNumberOfLUs=scsiTgtDevNumberOfLUs, scsiLunMapStatsGroup=scsiLunMapStatsGroup, scsiIntrPrtTable=scsiIntrPrtTable, scsiTgtPortWrittenMegaBytes=scsiTgtPortWrittenMegaBytes, scsiAttIntrPrtEntry=scsiAttIntrPrtEntry, scsiAttTgtPortDscTgtIdx=scsiAttTgtPortDscTgtIdx, scsiDscTgtRowStatus=scsiDscTgtRowStatus, scsiRemoteTarget=scsiRemoteTarget, scsiDscTgtIntrPortIndex=scsiDscTgtIntrPortIndex, scsiIntrPrtIdentifier=scsiIntrPrtIdentifier, scsiLuOutQueueFullStatus=scsiLuOutQueueFullStatus, scsiAuthIntrDevOrPort=scsiAuthIntrDevOrPort, scsiLuWrittenMegaBytes=scsiLuWrittenMegaBytes, scsiTargetGroup=scsiTargetGroup, scsiLuIdType=scsiLuIdType, ScsiIndexValueOrZero=ScsiIndexValueOrZero, scsiDscTgtReadMegaBytes=scsiDscTgtReadMegaBytes, scsiAuthIntrLunMapIndex=scsiAuthIntrLunMapIndex, scsiTgtDevNonAccessibleLUs=scsiTgtDevNonAccessibleLUs, scsiInstanceTable=scsiInstanceTable, scsiTgtPortIdentifier=scsiTgtPortIdentifier, scsiLuTable=scsiLuTable, scsiAttTgtPortEntry=scsiAttTgtPortEntry, scsiLuReadMegaBytes=scsiLuReadMegaBytes, scsiTgtPortEntry=scsiTgtPortEntry, scsiLuIdIndex=scsiLuIdIndex, scsiDeviceRole=scsiDeviceRole, scsiDscLunTable=scsiDscLunTable, scsiIntrPrtHSOutCommands=scsiIntrPrtHSOutCommands, scsiDscLunIndex=scsiDscLunIndex, scsiCompliances=scsiCompliances, ScsiIdType=ScsiIdType, scsiDscTgtLastCreation=scsiDscTgtLastCreation, scsiAuthorizedIntrEntry=scsiAuthorizedIntrEntry, scsiLuVendorId=scsiLuVendorId, scsiInitiatorGroup=scsiInitiatorGroup, scsiDscLunIdIndex=scsiDscLunIdIndex, scsiDscLunIdEntry=scsiDscLunIdEntry, scsiLunMapIndex=scsiLunMapIndex, scsiPortBusyStatuses=scsiPortBusyStatuses, scsiDiscoveryStatsGroup=scsiDiscoveryStatsGroup, scsiTrnsptPointer=scsiTrnsptPointer, scsiIntrDevEntry=scsiIntrDevEntry, scsiDeviceGroup=scsiDeviceGroup, scsiIntrPrtEntry=scsiIntrPrtEntry, scsiTgtDeviceStatusChanged=scsiTgtDeviceStatusChanged, scsiDscTgtTable=scsiDscTgtTable, scsiLunMapHSStatsGroup=scsiLunMapHSStatsGroup, scsiDiscoveryGroup=scsiDiscoveryGroup, scsiAttIntrPrtIdx=scsiAttIntrPrtIdx, scsiLuIdEntry=scsiLuIdEntry, scsiDscTgtWrittenMegaBytes=scsiDscTgtWrittenMegaBytes, scsiTranportOther=scsiTranportOther, scsiDeviceEntry=scsiDeviceEntry, scsiAttTgtPortIdentifier=scsiAttTgtPortIdentifier, scsiInstSoftwareIndex=scsiInstSoftwareIndex, scsiNotifications=scsiNotifications, ScsiName=ScsiName, scsiGeneral=scsiGeneral, scsiDeviceIndex=scsiDeviceIndex, scsiTargetHSStatsGroup=scsiTargetHSStatsGroup, scsiDscTgtEntry=scsiDscTgtEntry, scsiGroups=scsiGroups, scsiTargetStatsGroup=scsiTargetStatsGroup, scsiLuIndex=scsiLuIndex, scsiTgtDevTable=scsiTgtDevTable, scsiTgtPortReadMegaBytes=scsiTgtPortReadMegaBytes, scsiAuthIntrTgtPortIndex=scsiAuthIntrTgtPortIndex, ScsiIdAssociation=ScsiIdAssociation, scsiLuWwnName=scsiLuWwnName, scsiInstanceEntry=scsiInstanceEntry, scsiTrnsptIndex=scsiTrnsptIndex, scsiAuthIntrIndex=scsiAuthIntrIndex, scsiAttTgtPortIndex=scsiAttTgtPortIndex, scsiIntrDevTgtAccessMode=scsiIntrDevTgtAccessMode, scsiAttIntrPrtAuthIntrIdx=scsiAttIntrPrtAuthIntrIdx, scsiModule=scsiModule, ScsiIdCodeSet=ScsiIdCodeSet, scsiTranportSPI=scsiTranportSPI, scsiObjects=scsiObjects, scsiLunMapLun=scsiLunMapLun, scsiDscTgtName=scsiDscTgtName, scsiAttTgtPortName=scsiAttTgtPortName, scsiConformance=scsiConformance, scsiDscTgtHSInCommands=scsiDscTgtHSInCommands, scsiTrnsptEntry=scsiTrnsptEntry, scsiLuStatusChanged=scsiLuStatusChanged, scsiTransportSBP=scsiTransportSBP, ScsiIndexValue=ScsiIndexValue, scsiTrnsptType=scsiTrnsptType, scsiTgtDevEntry=scsiTgtDevEntry, scsiDscTgtIndex=scsiDscTgtIndex, scsiAttIntrPrtTable=scsiAttIntrPrtTable, scsiTgtPortInCommands=scsiTgtPortInCommands, scsiTrnsptDevName=scsiTrnsptDevName, scsiAuthIntrReadMegaBytes=scsiAuthIntrReadMegaBytes, scsiLuIdCodeSet=scsiLuIdCodeSet, scsiLuIdAssociation=scsiLuIdAssociation, scsiTgtPortHSInCommands=scsiTgtPortHSInCommands, scsiIntrPrtReadMegaBytes=scsiIntrPrtReadMegaBytes, scsiIntrPrtName=scsiIntrPrtName, scsiTgtPortName=scsiTgtPortName, scsiDscLunIdTable=scsiDscLunIdTable, scsiAuthIntrAttachedTimes=scsiAuthIntrAttachedTimes, scsiAuthIntrWrittenMegaBytes=scsiAuthIntrWrittenMegaBytes, scsiDscTgtConfigured=scsiDscTgtConfigured, scsiTransportSRP=scsiTransportSRP, scsiInitiator=scsiInitiator, scsiIntrPrtOutCommands=scsiIntrPrtOutCommands, scsiInstAlias=scsiInstAlias, scsiTarget=scsiTarget, ScsiPortIndexValueOrZero=ScsiPortIndexValueOrZero, scsiDscLunEntry=scsiDscLunEntry, scsiDeviceAlias=scsiDeviceAlias, scsiTrnsptTable=scsiTrnsptTable, scsiLunMapGroup=scsiLunMapGroup, scsiDiscoveryHSStatsGroup=scsiDiscoveryHSStatsGroup, scsiDevicePortNumber=scsiDevicePortNumber, scsiDscTgtDevOrPort=scsiDscTgtDevOrPort, scsiIntrDevOutResets=scsiIntrDevOutResets, scsiLuHSInCommands=scsiLuHSInCommands)
