...
NotificationType, MibIdentifier, mib_2, ModuleIdentity, iso, Unsigned32, ObjectIdentity, Integer32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Counter64, TimeTicks, IpAddress, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "MibIdentifier", "mib-2", "ModuleIdentity", "iso", "Unsigned32", "ObjectIdentity", "Integer32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Counter64", "TimeTicks", "IpAddress", "Counter32")
...
mibBuilder.exportSymbols("SCSI-MIB", scsiDeviceGroup=scsiDeviceGroup, scsiInstSoftwareIndex=scsiInstSoftwareIndex, scsiLuInCommands=scsiLuInCommands, scsiDeviceRole=scsiDeviceRole, scsiAuthorizedIntrEntry=scsiAuthorizedIntrEntry, scsiPortTrnsptPtr=scsiPortTrnsptPtr, scsiDscTgtTable=scsiDscTgtTable, scsiTgtDevNumberOfLUs=scsiTgtDevNumberOfLUs, scsiAttIntrPrtAuthIntrIdx=scsiAttIntrPrtAuthIntrIdx, scsiAttIntrPrtId=scsiAttIntrPrtId, scsiLunMapGroup=scsiLunMapGroup, scsiDscTgtIntrPortIndex=scsiDscTgtIntrPortIndex, scsiDscLunIdType=scsiDscLunIdType, scsiConformance=scsiConformance, scsiInitiator=scsiInitiator, scsiPortEntry=scsiPortEntry, scsiLunMapIndex=scsiLunMapIndex, scsiPortRole=scsiPortRole, scsiAuthIntrIndex=scsiAuthIntrIndex, scsiLunMapLun=scsiLunMapLun, scsiDscTgtReadMegaBytes=scsiDscTgtReadMegaBytes, scsiIntrPrtTable=scsiIntrPrtTable, ScsiIdValue=ScsiIdValue, scsiDscTgtLastCreation=scsiDscTgtLastCreation, scsiDiscoveryGroup=scsiDiscoveryGroup, scsiDscTgtDiscovered=scsiDscTgtDiscovered, scsiDscLunIdEntry=scsiDscLunIdEntry, ScsiIndexValueOrZero=ScsiIndexValueOrZero, scsiAttTgtPortName=scsiAttTgtPortName, scsiCompliance=scsiCompliance, scsiTransportSBP=scsiTransportSBP, scsiDscLunIdTable=scsiDscLunIdTable, scsiCompliances=scsiCompliances, scsiPortIndex=scsiPortIndex, scsiTrnsptDevName=scsiTrnsptDevName, scsiLuIdAssociation=scsiLuIdAssociation, ScsiDeviceOrPort=ScsiDeviceOrPort, scsiIntrPrtReadMegaBytes=scsiIntrPrtReadMegaBytes, scsiIntrDevEntry=scsiIntrDevEntry, scsiInstScsiNotificationsEnable=scsiInstScsiNotificationsEnable, scsiLuReadMegaBytes=scsiLuReadMegaBytes, scsiDscLunIdIndex=scsiDscLunIdIndex, scsiLogicalUnit=scsiLogicalUnit, scsiIntrDevTable=scsiIntrDevTable, scsiLuEntry=scsiLuEntry, scsiInstanceEntry=scsiInstanceEntry, scsiDeviceAlias=scsiDeviceAlias, HrSWInstalledIndexOrZero=HrSWInstalledIndexOrZero, scsiTgtPortEntry=scsiTgtPortEntry, scsiLuIdCodeSet=scsiLuIdCodeSet, scsiInitiatorHSStatsGroup=scsiInitiatorHSStatsGroup, scsiDeviceResets=scsiDeviceResets, scsiTgtDeviceStatus=scsiTgtDeviceStatus, scsiRemoteTarget=scsiRemoteTarget, scsiLuStatus=scsiLuStatus, PYSNMP_MODULE_ID=scsiModule, scsiAuthIntrLastCreation=scsiAuthIntrLastCreation, scsiDscLunEntry=scsiDscLunEntry, scsiLuTable=scsiLuTable, scsiDscLunIdCodeSet=scsiDscLunIdCodeSet, scsiTgtPortInCommands=scsiTgtPortInCommands, ScsiIndexValue=ScsiIndexValue, ScsiIdAssociation=ScsiIdAssociation, scsiTargetGroup=scsiTargetGroup, scsiDscLunIdValue=scsiDscLunIdValue, scsiDeviceTable=scsiDeviceTable, ScsiPortIndexValueOrZero=ScsiPortIndexValueOrZero, scsiDeviceStatGroup=scsiDeviceStatGroup, scsiAuthorizedIntrTable=scsiAuthorizedIntrTable, scsiGeneral=scsiGeneral, scsiTrnsptIndex=scsiTrnsptIndex, scsiLuDefaultLun=scsiLuDefaultLun, scsiAttTgtPortEntry=scsiAttTgtPortEntry, scsiModule=scsiModule, scsiLuVendorId=scsiLuVendorId, scsiAttTgtPortTable=scsiAttTgtPortTable, scsiDscTgtDevOrPort=scsiDscTgtDevOrPort, scsiLuHSInCommands=scsiLuHSInCommands, scsiTransportFCP=scsiTransportFCP, scsiDscTgtIndex=scsiDscTgtIndex, scsiLuPeripheralType=scsiLuPeripheralType, scsiLuInResets=scsiLuInResets, scsiDeviceEntry=scsiDeviceEntry, scsiTrnsptTable=scsiTrnsptTable, scsiInstIndex=scsiInstIndex, scsiTgtPortTable=scsiTgtPortTable, scsiDscLunIdAssociation=scsiDscLunIdAssociation, scsiNotifications=scsiNotifications, scsiLuStatusChanged=scsiLuStatusChanged, scsiAttTgtPortIndex=scsiAttTgtPortIndex, ScsiName=ScsiName, scsiDscTgtName=scsiDscTgtName, scsiInstAlias=scsiInstAlias, scsiAttTgtPortDscTgtIdx=scsiAttTgtPortDscTgtIdx, scsiAuthIntrWrittenMegaBytes=scsiAuthIntrWrittenMegaBytes, scsiAuthIntrTgtPortIndex=scsiAuthIntrTgtPortIndex, scsiTgtPortName=scsiTgtPortName, scsiLuState=scsiLuState, scsiRemoteInitiators=scsiRemoteInitiators, scsiDscLunIndex=scsiDscLunIndex, scsiLuIdIndex=scsiLuIdIndex, ScsiLUNOrZero=ScsiLUNOrZero, scsiTgtPortHSInCommands=scsiTgtPortHSInCommands, scsiDscTgtEntry=scsiDscTgtEntry, scsiObjects=scsiObjects, scsiTranportSPI=scsiTranportSPI, ScsiIdentifier=ScsiIdentifier, scsiDscLunLun=scsiDscLunLun, scsiIntrDevTgtAccessMode=scsiIntrDevTgtAccessMode, scsiAuthIntrName=scsiAuthIntrName, scsiAttIntrPrtEntry=scsiAttIntrPrtEntry, scsiDiscoveryStatsGroup=scsiDiscoveryStatsGroup, scsiInstanceTable=scsiInstanceTable, ScsiIdType=ScsiIdType, scsiDeviceIndex=scsiDeviceIndex, scsiLuIndex=scsiLuIndex, scsiGroups=scsiGroups, scsiDscLunTable=scsiDscLunTable, scsiLuIdValue=scsiLuIdValue, scsiDscTgtWrittenMegaBytes=scsiDscTgtWrittenMegaBytes, scsiDscTgtInCommands=scsiDscTgtInCommands, scsiTgtDevEntry=scsiTgtDevEntry, scsiTgtDevNonAccessibleLUs=scsiTgtDevNonAccessibleLUs, scsiTransportISCSI=scsiTransportISCSI, scsiLuWrittenMegaBytes=scsiLuWrittenMegaBytes, scsiAttIntrPrtName=scsiAttIntrPrtName, scsiAuthIntrAttachedTimes=scsiAuthIntrAttachedTimes, scsiPortTable=scsiPortTable, scsiTgtPortWrittenMegaBytes=scsiTgtPortWrittenMegaBytes, scsiInitiatorGroup=scsiInitiatorGroup, scsiTargetStatsGroup=scsiTargetStatsGroup, scsiLuWwnName=scsiLuWwnName, scsiDevicePortNumber=scsiDevicePortNumber, scsiAuthIntrDevOrPort=scsiAuthIntrDevOrPort, scsiLunMapTable=scsiLunMapTable, scsiLunMapEntry=scsiLunMapEntry, scsiAuthIntrOutCommands=scsiAuthIntrOutCommands, scsiDscTgtHSInCommands=scsiDscTgtHSInCommands, scsiIntrPrtEntry=scsiIntrPrtEntry, scsiDscTgtConfigured=scsiDscTgtConfigured, scsiIntrPrtIdentifier=scsiIntrPrtIdentifier, scsiTgtPortReadMegaBytes=scsiTgtPortReadMegaBytes, scsiPortBusyStatuses=scsiPortBusyStatuses, scsiLuProductId=scsiLuProductId, scsiIntrPrtWrittenMegaBytes=scsiIntrPrtWrittenMegaBytes, scsiTgtDevTable=scsiTgtDevTable, scsiAttIntrPrtIdx=scsiAttIntrPrtIdx, scsiNotificationsPrefix=scsiNotificationsPrefix, ScsiNameIdOrZero=ScsiNameIdOrZero, ScsiIdCodeSet=ScsiIdCodeSet, scsiLuIdTable=scsiLuIdTable, scsiTransportSRP=scsiTransportSRP, scsiLunMapRowStatus=scsiLunMapRowStatus, scsiDiscoveryHSStatsGroup=scsiDiscoveryHSStatsGroup, scsiTgtDeviceStatusChanged=scsiTgtDeviceStatusChanged, scsiLuRevisionId=scsiLuRevisionId, scsiLunMapStatsGroup=scsiLunMapStatsGroup, scsiAuthIntrRowStatus=scsiAuthIntrRowStatus, scsiIntrPrtName=scsiIntrPrtName, scsiAuthIntrReadMegaBytes=scsiAuthIntrReadMegaBytes, scsiTgtPortIdentifier=scsiTgtPortIdentifier, scsiIntrDevOutResets=scsiIntrDevOutResets, scsiTrnsptType=scsiTrnsptType, scsiLuIdEntry=scsiLuIdEntry, scsiTarget=scsiTarget, scsiIntrPrtHSOutCommands=scsiIntrPrtHSOutCommands, scsiAuthIntrLunMapIndex=scsiAuthIntrLunMapIndex, scsiTranportOther=scsiTranportOther, scsiTrnsptEntry=scsiTrnsptEntry, scsiTrnsptPointer=scsiTrnsptPointer, scsiAttIntrPrtTable=scsiAttIntrPrtTable, scsiDscTgtRowStatus=scsiDscTgtRowStatus, scsiLuIdType=scsiLuIdType, scsiLunMapLuIndex=scsiLunMapLuIndex, scsiLunMapHSStatsGroup=scsiLunMapHSStatsGroup, scsiLuOutQueueFullStatus=scsiLuOutQueueFullStatus, scsiAuthIntrHSOutCommands=scsiAuthIntrHSOutCommands, scsiInitiatorStatsGroup=scsiInitiatorStatsGroup, scsiIntrPrtOutCommands=scsiIntrPrtOutCommands, scsiTransportTypes=scsiTransportTypes, scsiAttTgtPortIdentifier=scsiAttTgtPortIdentifier, scsiInstVendorVersion=scsiInstVendorVersion, scsiTargetHSStatsGroup=scsiTargetHSStatsGroup)
