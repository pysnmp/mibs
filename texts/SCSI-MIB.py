...
Counter32, IpAddress, MibIdentifier, Bits, Integer32, mib_2, Unsigned32, NotificationType, Gauge32, iso, ModuleIdentity, ObjectIdentity, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "IpAddress", "MibIdentifier", "Bits", "Integer32", "mib-2", "Unsigned32", "NotificationType", "Gauge32", "iso", "ModuleIdentity", "ObjectIdentity", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks")
...
mibBuilder.exportSymbols("SCSI-MIB", scsiTgtDevNumberOfLUs=scsiTgtDevNumberOfLUs, scsiDevicePortNumber=scsiDevicePortNumber, scsiIntrPrtWrittenMegaBytes=scsiIntrPrtWrittenMegaBytes, scsiDscLunIdType=scsiDscLunIdType, scsiLunMapLun=scsiLunMapLun, ScsiLUNOrZero=ScsiLUNOrZero, scsiLunMapIndex=scsiLunMapIndex, scsiGeneral=scsiGeneral, scsiPortEntry=scsiPortEntry, scsiLuWrittenMegaBytes=scsiLuWrittenMegaBytes, ScsiIdentifier=ScsiIdentifier, scsiTgtPortName=scsiTgtPortName, scsiDeviceGroup=scsiDeviceGroup, scsiConformance=scsiConformance, scsiLuIndex=scsiLuIndex, scsiLuIdIndex=scsiLuIdIndex, scsiAttTgtPortDscTgtIdx=scsiAttTgtPortDscTgtIdx, ScsiIdType=ScsiIdType, scsiLuProductId=scsiLuProductId, scsiInstSoftwareIndex=scsiInstSoftwareIndex, scsiTrnsptIndex=scsiTrnsptIndex, scsiInstIndex=scsiInstIndex, scsiTransportSBP=scsiTransportSBP, scsiAuthIntrHSOutCommands=scsiAuthIntrHSOutCommands, scsiAuthIntrAttachedTimes=scsiAuthIntrAttachedTimes, scsiTrnsptTable=scsiTrnsptTable, scsiLuState=scsiLuState, scsiLuIdTable=scsiLuIdTable, scsiDeviceTable=scsiDeviceTable, scsiIntrDevTgtAccessMode=scsiIntrDevTgtAccessMode, scsiLuHSInCommands=scsiLuHSInCommands, scsiGroups=scsiGroups, scsiLunMapRowStatus=scsiLunMapRowStatus, scsiAttIntrPrtIdx=scsiAttIntrPrtIdx, scsiAuthIntrTgtPortIndex=scsiAuthIntrTgtPortIndex, scsiDscTgtWrittenMegaBytes=scsiDscTgtWrittenMegaBytes, ScsiNameIdOrZero=ScsiNameIdOrZero, scsiLuIdAssociation=scsiLuIdAssociation, scsiDeviceIndex=scsiDeviceIndex, scsiPortTable=scsiPortTable, scsiAttTgtPortIndex=scsiAttTgtPortIndex, scsiLunMapEntry=scsiLunMapEntry, scsiDeviceEntry=scsiDeviceEntry, scsiDiscoveryHSStatsGroup=scsiDiscoveryHSStatsGroup, scsiPortTrnsptPtr=scsiPortTrnsptPtr, scsiLuOutQueueFullStatus=scsiLuOutQueueFullStatus, scsiIntrPrtTable=scsiIntrPrtTable, scsiTgtDevEntry=scsiTgtDevEntry, scsiInitiatorHSStatsGroup=scsiInitiatorHSStatsGroup, scsiDscTgtConfigured=scsiDscTgtConfigured, scsiLunMapStatsGroup=scsiLunMapStatsGroup, scsiLuVendorId=scsiLuVendorId, scsiAuthorizedIntrEntry=scsiAuthorizedIntrEntry, scsiDiscoveryStatsGroup=scsiDiscoveryStatsGroup, scsiDeviceResets=scsiDeviceResets, scsiLuIdType=scsiLuIdType, ScsiName=ScsiName, scsiDscLunIdEntry=scsiDscLunIdEntry, scsiTrnsptDevName=scsiTrnsptDevName, scsiTgtDevNonAccessibleLUs=scsiTgtDevNonAccessibleLUs, scsiAttIntrPrtName=scsiAttIntrPrtName, scsiDscTgtRowStatus=scsiDscTgtRowStatus, scsiLuRevisionId=scsiLuRevisionId, scsiLunMapHSStatsGroup=scsiLunMapHSStatsGroup, scsiDscLunIdIndex=scsiDscLunIdIndex, scsiRemoteTarget=scsiRemoteTarget, ScsiIdCodeSet=ScsiIdCodeSet, scsiTgtPortTable=scsiTgtPortTable, scsiDscLunIdTable=scsiDscLunIdTable, scsiDeviceAlias=scsiDeviceAlias, scsiLuInResets=scsiLuInResets, scsiTrnsptType=scsiTrnsptType, ScsiIdValue=ScsiIdValue, scsiTgtPortHSInCommands=scsiTgtPortHSInCommands, scsiIntrPrtEntry=scsiIntrPrtEntry, scsiDscLunIdValue=scsiDscLunIdValue, scsiAuthIntrRowStatus=scsiAuthIntrRowStatus, PYSNMP_MODULE_ID=scsiModule, scsiDscTgtDiscovered=scsiDscTgtDiscovered, scsiInstanceTable=scsiInstanceTable, scsiTargetHSStatsGroup=scsiTargetHSStatsGroup, scsiLuStatus=scsiLuStatus, scsiAuthIntrReadMegaBytes=scsiAuthIntrReadMegaBytes, scsiDscTgtInCommands=scsiDscTgtInCommands, scsiAuthIntrLunMapIndex=scsiAuthIntrLunMapIndex, scsiNotifications=scsiNotifications, scsiLuReadMegaBytes=scsiLuReadMegaBytes, scsiDscLunTable=scsiDscLunTable, scsiAuthorizedIntrTable=scsiAuthorizedIntrTable, scsiLuIdValue=scsiLuIdValue, scsiDscTgtLastCreation=scsiDscTgtLastCreation, scsiAttTgtPortTable=scsiAttTgtPortTable, scsiAuthIntrIndex=scsiAuthIntrIndex, scsiNotificationsPrefix=scsiNotificationsPrefix, scsiLunMapLuIndex=scsiLunMapLuIndex, scsiTransportFCP=scsiTransportFCP, scsiIntrDevOutResets=scsiIntrDevOutResets, scsiInitiatorGroup=scsiInitiatorGroup, scsiAuthIntrOutCommands=scsiAuthIntrOutCommands, scsiIntrPrtHSOutCommands=scsiIntrPrtHSOutCommands, scsiAttTgtPortName=scsiAttTgtPortName, ScsiIdAssociation=ScsiIdAssociation, scsiDscTgtEntry=scsiDscTgtEntry, scsiRemoteInitiators=scsiRemoteInitiators, scsiDscTgtIndex=scsiDscTgtIndex, scsiDscLunIndex=scsiDscLunIndex, scsiInstVendorVersion=scsiInstVendorVersion, scsiInitiator=scsiInitiator, scsiAuthIntrLastCreation=scsiAuthIntrLastCreation, scsiTgtPortIdentifier=scsiTgtPortIdentifier, scsiDscTgtHSInCommands=scsiDscTgtHSInCommands, scsiAuthIntrDevOrPort=scsiAuthIntrDevOrPort, ScsiDeviceOrPort=ScsiDeviceOrPort, scsiAttIntrPrtTable=scsiAttIntrPrtTable, scsiLuStatusChanged=scsiLuStatusChanged, scsiTgtDevTable=scsiTgtDevTable, scsiTgtPortReadMegaBytes=scsiTgtPortReadMegaBytes, scsiTargetStatsGroup=scsiTargetStatsGroup, scsiLuTable=scsiLuTable, scsiAttTgtPortEntry=scsiAttTgtPortEntry, scsiDscLunLun=scsiDscLunLun, scsiLuPeripheralType=scsiLuPeripheralType, scsiDscTgtReadMegaBytes=scsiDscTgtReadMegaBytes, scsiLuIdEntry=scsiLuIdEntry, scsiDiscoveryGroup=scsiDiscoveryGroup, scsiCompliances=scsiCompliances, HrSWInstalledIndexOrZero=HrSWInstalledIndexOrZero, scsiAttIntrPrtEntry=scsiAttIntrPrtEntry, scsiLuEntry=scsiLuEntry, scsiInstScsiNotificationsEnable=scsiInstScsiNotificationsEnable, scsiTgtDeviceStatus=scsiTgtDeviceStatus, scsiTgtPortInCommands=scsiTgtPortInCommands, scsiLuDefaultLun=scsiLuDefaultLun, scsiTgtDeviceStatusChanged=scsiTgtDeviceStatusChanged, scsiLuWwnName=scsiLuWwnName, scsiAttTgtPortIdentifier=scsiAttTgtPortIdentifier, scsiTargetGroup=scsiTargetGroup, scsiModule=scsiModule, scsiPortBusyStatuses=scsiPortBusyStatuses, scsiInitiatorStatsGroup=scsiInitiatorStatsGroup, scsiObjects=scsiObjects, scsiIntrPrtReadMegaBytes=scsiIntrPrtReadMegaBytes, scsiDscLunIdCodeSet=scsiDscLunIdCodeSet, scsiIntrPrtName=scsiIntrPrtName, ScsiPortIndexValueOrZero=ScsiPortIndexValueOrZero, scsiDscTgtIntrPortIndex=scsiDscTgtIntrPortIndex, scsiLunMapGroup=scsiLunMapGroup, ScsiIndexValueOrZero=ScsiIndexValueOrZero, scsiDscTgtName=scsiDscTgtName, scsiTransportTypes=scsiTransportTypes, scsiDeviceRole=scsiDeviceRole, scsiPortRole=scsiPortRole, scsiLuIdCodeSet=scsiLuIdCodeSet, scsiInstAlias=scsiInstAlias, scsiIntrPrtOutCommands=scsiIntrPrtOutCommands, scsiTgtPortEntry=scsiTgtPortEntry, scsiDeviceStatGroup=scsiDeviceStatGroup, scsiAuthIntrName=scsiAuthIntrName, scsiDscLunEntry=scsiDscLunEntry, scsiPortIndex=scsiPortIndex, scsiLogicalUnit=scsiLogicalUnit, scsiTrnsptPointer=scsiTrnsptPointer, scsiTranportSPI=scsiTranportSPI, scsiIntrDevTable=scsiIntrDevTable, scsiAttIntrPrtId=scsiAttIntrPrtId, scsiLuInCommands=scsiLuInCommands, scsiDscLunIdAssociation=scsiDscLunIdAssociation, scsiAttIntrPrtAuthIntrIdx=scsiAttIntrPrtAuthIntrIdx, ScsiIndexValue=ScsiIndexValue, scsiAuthIntrWrittenMegaBytes=scsiAuthIntrWrittenMegaBytes, scsiTarget=scsiTarget, scsiLunMapTable=scsiLunMapTable, scsiIntrPrtIdentifier=scsiIntrPrtIdentifier, scsiDscTgtDevOrPort=scsiDscTgtDevOrPort, scsiTranportOther=scsiTranportOther, scsiDscTgtTable=scsiDscTgtTable, scsiIntrDevEntry=scsiIntrDevEntry, scsiCompliance=scsiCompliance, scsiInstanceEntry=scsiInstanceEntry, scsiTransportISCSI=scsiTransportISCSI, scsiTransportSRP=scsiTransportSRP, scsiTrnsptEntry=scsiTrnsptEntry, scsiTgtPortWrittenMegaBytes=scsiTgtPortWrittenMegaBytes)
