...
mibBuilder.exportSymbols("CAMBIUM-PTP670-MIB", maxLinkRangeExceeded=maxLinkRangeExceeded, deviceMACAddress=deviceMACAddress, auxPortPowerOverEthernetOutputStatus=auxPortPowerOverEthernetOutputStatus, configurationInstancedTableEntry=configurationInstancedTableEntry, licenseSFPPortSupport=licenseSFPPortSupport, capacityVariantMismatchTrap=capacityVariantMismatchTrap, alarmInstancedTableIndex=alarmInstancedTableIndex, sNTPServerStatus=sNTPServerStatus, cableDiagPair2Distance=cableDiagPair2Distance, sNTPServerResolvedInternetAddress=sNTPServerResolvedInternetAddress, sFPErrorTrap=sFPErrorTrap, dataBridgingStatusAlarmLinked=dataBridgingStatusAlarmLinked, unitIdentificationGroup=unitIdentificationGroup, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, tDMEnabledChannels=tDMEnabledChannels, tFTPServerInternetAddress=tFTPServerInternetAddress, mainPSUPortSyncERxStatus=mainPSUPortSyncERxStatus, dfs=dfs, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, regulatoryBand=regulatoryBand, tDDSynchronizationStatus=tDDSynchronizationStatus, mainPSUPortSyncEMasterSlaveStatus=mainPSUPortSyncEMasterSlaveStatus, sFPPortConfigurationMismatchTrap=sFPPortConfigurationMismatchTrap, licenseMaxLinkRange=licenseMaxLinkRange, hTTPPortNumber=hTTPPortNumber, dfsTableNumber=dfsTableNumber, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, licenseGroupAccess=licenseGroupAccess, tDDControl=tDDControl, mainPSUPortQLRxOverwrite=mainPSUPortQLRxOverwrite, mABCurrentTransmitCapacity=mABCurrentTransmitCapacity, userEnabled=userEnabled, secureModeAlarmTrap=secureModeAlarmTrap, phyControlInstancedTableIndex=phyControlInstancedTableIndex, syncETracking=syncETracking, sNTPSyncTrap=sNTPSyncTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, cableDiagnosticsResultTableNumber=cableDiagnosticsResultTableNumber, authorizationControlTable=authorizationControlTable, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sMTPServerInternetAddress=sMTPServerInternetAddress, nIDULanPortGigabitMasterSlaveStatus=nIDULanPortGigabitMasterSlaveStatus, vLANManagementPriority=vLANManagementPriority, licenseUnitSerialNumber=licenseUnitSerialNumber, mainPSUPortGigabitMasterSlaveStatus=mainPSUPortGigabitMasterSlaveStatus, mainPSUPortSSMTx=mainPSUPortSSMTx, remoteInternetAddressTypeLinked=remoteInternetAddressTypeLinked, aAAControl=aAAControl, tLSMinimumSecurityLevel=tLSMinimumSecurityLevel, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, ptpTraps=ptpTraps, tDMGroup=tDMGroup, dfsTableIndex=dfsTableIndex, phyStatusGroup=phyStatusGroup, capacityVariantMismatch=capacityVariantMismatch, useVLANForManagementInterfaces=useVLANForManagementInterfaces, licenseGasGroup=licenseGasGroup, eIRP=eIRP, byteErrorRatio=byteErrorRatio, sFPPortAutoMdix=sFPPortAutoMdix, licenseRegulatoryBandsTable=licenseRegulatoryBandsTable, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, iPv6GatewayAddress=iPv6GatewayAddress, tDMChannelLineCode=tDMChannelLineCode, rangeLinked=rangeLinked, sNMPTrapTableNumber=sNMPTrapTableNumber, sNMPCommunityString=sNMPCommunityString, phyControlGroup=phyControlGroup, oOBPriorityQueueMapping=oOBPriorityQueueMapping, aggregateDataRate=aggregateDataRate, range=range, sNTPServerRetries=sNTPServerRetries, licenseMinFirmwareVersionPTP650Emulation=licenseMinFirmwareVersionPTP650Emulation, ethernetGroup=ethernetGroup, pubInstancedStatsTableNumber=pubInstancedStatsTableNumber, licenseCapacity=licenseCapacity, licenseIPv6Support=licenseIPv6Support, tdmTableNumber=tdmTableNumber, extendedSpectrumScanning=extendedSpectrumScanning, dNSResolver=dNSResolver, cableDiagPair2Results=cableDiagPair2Results, portAllocationMismatchLinked=portAllocationMismatchLinked, enableTransmission=enableTransmission, auxPortQLTx=auxPortQLTx, dfsTableEntry=dfsTableEntry, telnetPortNumber=telnetPortNumber, installationMode=installationMode, alarmInstancedTable=alarmInstancedTable, lowestDataModulationMode=lowestDataModulationMode, dNS=dNS, sNMPPortNumber=sNMPPortNumber, sNTPServerKeyIdentifier=sNTPServerKeyIdentifier, userTable=userTable, groupID=groupID, syslogState=syslogState, ethernet=ethernet, phyInstancedStatusTableIndex=phyInstancedStatusTableIndex, mainPSUPortConfigurationMismatchTrap=mainPSUPortConfigurationMismatchTrap, enabledDiagnosticAlarms=enabledDiagnosticAlarms, tDMChannelStatus=tDMChannelStatus, encryptionAlgorithm=encryptionAlgorithm, hCMPLinkSymmetry=hCMPLinkSymmetry, ethernetStatisticsDataSource=ethernetStatisticsDataSource, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, snmpControlGroup=snmpControlGroup, cableDiagnosticsGroup=cableDiagnosticsGroup, linkModeOptimizationMismatch=linkModeOptimizationMismatch, ptp=ptp, licenseMinimumFirmwareVersion=licenseMinimumFirmwareVersion, transparentClockPort=transparentClockPort, sFPPortQLTx=sFPPortQLTx, auxPortSpeedAndDuplex=auxPortSpeedAndDuplex, dfsTable=dfsTable, cableDiagPair4Distance=cableDiagPair4Distance, configurationGroup=configurationGroup, receiveFreqMHz=receiveFreqMHz, packetsFromInternalStack=packetsFromInternalStack, tdmTable=tdmTable, erroredSeconds=erroredSeconds, dNSServerTableEntry=dNSServerTableEntry, notificationsGroup=notificationsGroup, sMTPServerInternetAddressType=sMTPServerInternetAddressType, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, sFPPortSpeedAndDuplex=sFPPortSpeedAndDuplex, phyControlInstancedTableEntry=phyControlInstancedTableEntry, licenseNumberOfRegulatoryBands=licenseNumberOfRegulatoryBands, productName=productName, licenseCountry=licenseCountry, lbtDetectedTrap=lbtDetectedTrap, wirelessLinkStatusAlarm=wirelessLinkStatusAlarm, unitIdentification=unitIdentification, licenseMaxNumberOfTDMChannels=licenseMaxNumberOfTDMChannels, dNSServerInternetAddress=dNSServerInternetAddress, hTTPSPortNumber=hTTPSPortNumber, sNTPState=sNTPState, versions=versions, managementGroup=managementGroup, tDMConfigurationMismatchTrap=tDMConfigurationMismatchTrap, sFPPortStatus=sFPPortStatus, licenseSyncESupport=licenseSyncESupport, nIDULanPortAutoMdix=nIDULanPortAutoMdix, dNSPrimaryServer=dNSPrimaryServer, configurationInstancedTableNumber=configurationInstancedTableNumber, licenseHCMPSupport=licenseHCMPSupport, licenseVersion=licenseVersion, severelyErroredSeconds=severelyErroredSeconds, phyInstancedStatusTableNumber=phyInstancedStatusTableNumber, unavailableSeconds=unavailableSeconds, userRole=userRole, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, nIDULanPortStatusTrap=nIDULanPortStatusTrap, sNMPTrapInternetAddress=sNMPTrapInternetAddress, auxPortStatusTrap=auxPortStatusTrap, userTableEntry=userTableEntry, receiveModulationModeDetail=receiveModulationModeDetail, tdmTableIndex=tdmTableIndex, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, licenseAuxiliaryPortSupport=licenseAuxiliaryPortSupport, receiveDataRate=receiveDataRate, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, sNTPServerTimeout=sNTPServerTimeout, tDMChannelLoopbackTrap=tDMChannelLoopbackTrap, phyControlInstancedTableNumber=phyControlInstancedTableNumber, ethernetStatisticsTxOctets=ethernetStatisticsTxOctets, wirelessTopology=wirelessTopology, channelChangeTrap=channelChangeTrap, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, pubStats=pubStats, systemClock=systemClock, maximumNumberOfSlaves=maximumNumberOfSlaves, cableDiagPair1Distance=cableDiagPair1Distance, cableDiagPair4Results=cableDiagPair4Results, dataBridgingStatus=dataBridgingStatus, l2CPPriorityTable=l2CPPriorityTable, linkLossLinked=linkLossLinked, dNSServerTableIndex=dNSServerTableIndex, tDMChannelCableLength=tDMChannelCableLength, cableDiagPair3Results=cableDiagPair3Results, hCMPMaximumLinkRange=hCMPMaximumLinkRange, mgmtInstancedTableNumber=mgmtInstancedTableNumber, txColorCode=txColorCode, licenseSecurityLevel=licenseSecurityLevel, transmitDataRate=transmitDataRate, mainPSUPortAllocation=mainPSUPortAllocation, mainPSUPortDisabledWarningTrap=mainPSUPortDisabledWarningTrap, mABHoldoffPeriod=mABHoldoffPeriod, sNMPTrapTableEntry=sNMPTrapTableEntry, installStatusLinked=installStatusLinked, sMTPSourceEmailAddress=sMTPSourceEmailAddress, ethernetStatisticsRxFramesOversize=ethernetStatisticsRxFramesOversize, authorizationControlTableEntry=authorizationControlTableEntry, blacklistRMMacAddress=blacklistRMMacAddress, linkModeOptimization=linkModeOptimization, syncETrackingState=syncETrackingState, auxPortDisabledWarning=auxPortDisabledWarning, searchStateLinked=searchStateLinked, sNTPSync=sNTPSync, maximumTransmitPower=maximumTransmitPower, localPacketFiltering=localPacketFiltering, antennaSelection=antennaSelection, configuration=configuration, targetRange=targetRange, atpcHcmpMasterTargetRxPower=atpcHcmpMasterTargetRxPower, sFPPortAutoNegotiation=sFPPortAutoNegotiation, aggregateDataRateLinked=aggregateDataRateLinked, remoteUnitNameLinked=remoteUnitNameLinked, secureLicenseSignatureAlarmTrap=secureLicenseSignatureAlarmTrap, rawReceivePowerLinked=rawReceivePowerLinked, installStatusTrap=installStatusTrap, receiveModulationMode=receiveModulationMode, auxPortSyncEMasterSlaveStatus=auxPortSyncEMasterSlaveStatus, dNSServerInternetAddressType=dNSServerInternetAddressType, pubInstancedStatsTableEntry=pubInstancedStatsTableEntry, tDDControlGroup=tDDControlGroup, receivePowerLinked=receivePowerLinked, mABMaintenanceLevel=mABMaintenanceLevel, regulatoryBandTrap=regulatoryBandTrap, subnetMask=subnetMask, mPLSTCPriorityTable=mPLSTCPriorityTable, vLANManagementVID=vLANManagementVID, blacklistRMEnabled=blacklistRMEnabled, transmitFreqKHz=transmitFreqKHz, sFPPortQLRxOverwrite=sFPPortQLRxOverwrite, ethernetPriorityTable=ethernetPriorityTable, syslogClientTrap=syslogClientTrap, tDDSynchronizationMode=tDDSynchronizationMode, incompatibleRegulatoryBandsLinked=incompatibleRegulatoryBandsLinked, wirelessLinkStatusLinked=wirelessLinkStatusLinked, phyControlInstancedTable=phyControlInstancedTable, cableDiagnosticsResultsDateTime=cableDiagnosticsResultsDateTime, pubInstancedStatsTable=pubInstancedStatsTable, smtpGroup=smtpGroup, alarms=alarms, ethernetStatisticsTableEntry=ethernetStatisticsTableEntry, PYSNMP_MODULE_ID=cambium, dfsPeaks=dfsPeaks, incompatibleMasterAndSlaveLinked=incompatibleMasterAndSlaveLinked, transparentClockVID=transparentClockVID, licenseIEEE1588Support=licenseIEEE1588Support, transparentClockSourcePortAlarm=transparentClockSourcePortAlarm, phyInstancedStatusTable=phyInstancedStatusTable, sNMPTrapTable=sNMPTrapTable, ethernetStatisticsTxBroadcasts=ethernetStatisticsTxBroadcasts, dNSServerTableNumber=dNSServerTableNumber, licenseLongMinimumFirmwareVersion=licenseLongMinimumFirmwareVersion, remoteMACAddressLinked=remoteMACAddressLinked, nIDULanPortAutoNegotiation=nIDULanPortAutoNegotiation, dataPortWirelessDownAlert=dataPortWirelessDownAlert, installStatus=installStatus, transmitDataRateLinked=transmitDataRateLinked, bootVersion=bootVersion, auxPortDisabledWarningTrap=auxPortDisabledWarningTrap, iPDSCPPriorityTable=iPDSCPPriorityTable, packetsToInternalStack=packetsToInternalStack)
mibBuilder.exportSymbols("CAMBIUM-PTP670-MIB", mABProtocolVLANPriority=mABProtocolVLANPriority, license=license, auxPortAllocation=auxPortAllocation, bridge=bridge, capacityVariantMismatchLinked=capacityVariantMismatchLinked, auxPortConfigurationMismatch=auxPortConfigurationMismatch, secondarySoftwareVersion=secondarySoftwareVersion, supplementary=supplementary, hardwareVersion=hardwareVersion, licenseRARSupport=licenseRARSupport, incompatibleRegulatoryBandsTrap=incompatibleRegulatoryBandsTrap, licenseRemainingTrialPeriodAlarmTrap=licenseRemainingTrialPeriodAlarmTrap, mainPSUPortConfigurationMismatch=mainPSUPortConfigurationMismatch, sNTPServerInternetAddress=sNTPServerInternetAddress, whitelistRMEnabled=whitelistRMEnabled, licenseBandwidthCap=licenseBandwidthCap, auxPortGigabitMasterSlaveStatus=auxPortGigabitMasterSlaveStatus, installArmState=installArmState, auxPortQLRx=auxPortQLRx, transmitChannel=transmitChannel, portAllocationMismatchTrap=portAllocationMismatchTrap, ptmp=ptmp, receiveFreqKHz=receiveFreqKHz, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, ptp670=ptp670, mainPSUPortDisabledWarning=mainPSUPortDisabledWarning, ethernetPriorityTableEntry=ethernetPriorityTableEntry, hTTPAccessEnabled=hTTPAccessEnabled, ethernetInstancedTableNumber=ethernetInstancedTableNumber, nIDULanPortAutoNegAdvertisement=nIDULanPortAutoNegAdvertisement, mgmtInstancedTable=mgmtInstancedTable, cableLoss=cableLoss, auxPortAcceptedQLRx=auxPortAcceptedQLRx, sNTPPollInterval=sNTPPollInterval, encryption=encryption, unitOutOfCalibration=unitOutOfCalibration, cambium=cambium, sFPPortDisabledWarningTrap=sFPPortDisabledWarningTrap, sFPPortAcceptedQLRx=sFPPortAcceptedQLRx, sNTPServerInternetAddressType=sNTPServerInternetAddressType, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, userName=userName, cableDiagnosticsControl=cableDiagnosticsControl, sFPPortDisabledWarning=sFPPortDisabledWarning, dataBridgingStatusAlarm=dataBridgingStatusAlarm, ethernetInstancedTableEntry=ethernetInstancedTableEntry, vectorError=vectorError, licenseRemainingTrialPeriodAlarm=licenseRemainingTrialPeriodAlarm, syncEEquipmentClock=syncEEquipmentClock, syslogClient=syslogClient, apcGroup=apcGroup, pubStatsGroup=pubStatsGroup, tDMChannelStatusTrap=tDMChannelStatusTrap, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, subbandHighestFrequency=subbandHighestFrequency, linkLoss=linkLoss, dataBridgingAvailability=dataBridgingAvailability, longitude=longitude, smtp=smtp, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, tDDSynchronizationAlarm=tDDSynchronizationAlarm, resetGroup=resetGroup, microwaveAdaptiveBandwidth=microwaveAdaptiveBandwidth, cableDiagnosticsResultTable=cableDiagnosticsResultTable, unitName=unitName, authorizationControlTableIndex=authorizationControlTableIndex, auxPortSSMTx=auxPortSSMTx, receiveModulationModeLinked=receiveModulationModeLinked, maxLinkRangeExceededLinked=maxLinkRangeExceededLinked, auxPortPowerOverEthernetOutputStatusTrap=auxPortPowerOverEthernetOutputStatusTrap, installArmStateTrap=installArmStateTrap, tDMInterfaceStatus=tDMInterfaceStatus, licenseInstancedTableIndex=licenseInstancedTableIndex, sNMPTrapPortNumber=sNMPTrapPortNumber, syncESlavePort=syncESlavePort, authorizationControl=authorizationControl, wirelessOutOctets=wirelessOutOctets, remoteMaximumTransmitPower=remoteMaximumTransmitPower, licenseRemainingTrialPeriod=licenseRemainingTrialPeriod, receivePower=receivePower, alarmInstancedTableNumber=alarmInstancedTableNumber, antennaGain=antennaGain, dfsMeans=dfsMeans, sNTPPrimaryServer=sNTPPrimaryServer, sntp=sntp, sFPPortSSMTx=sFPPortSSMTx, sFPPortQLRx=sFPPortQLRx, ethernetStatisticsRxFramesUndersize=ethernetStatisticsRxFramesUndersize, deviceMSN=deviceMSN, linkName=linkName, txMABFrames=txMABFrames, portAllocationMismatch=portAllocationMismatch, mABTransmissionInterval=mABTransmissionInterval, iPVersion=iPVersion, licenseTrialPeriod=licenseTrialPeriod, dNSGroup=dNSGroup, qOSPriorityScheme=qOSPriorityScheme, syslogStateTrap=syslogStateTrap, linkModeOptimizationMismatchLinked=linkModeOptimizationMismatchLinked, nIDULanPortConfigurationMismatch=nIDULanPortConfigurationMismatch, userTableNumber=userTableNumber, atpcHcmpMasterTxPower=atpcHcmpMasterTxPower, maxLinkRangeExceededTrap=maxLinkRangeExceededTrap, versionsGroup=versionsGroup, sNTPServerTable=sNTPServerTable, wirelessLinkAvailabilityLinked=wirelessLinkAvailabilityLinked, siteName=siteName, sFPPortGigabitMasterSlaveStatus=sFPPortGigabitMasterSlaveStatus, auxPortSyncERxStatus=auxPortSyncERxStatus, phyControl=phyControl, dataBridgingAvailabilityLinked=dataBridgingAvailabilityLinked, ethernetStatisticsRxFrames=ethernetStatisticsRxFrames, userTableIndex=userTableIndex, nIDULanPortSpeedAndDuplex=nIDULanPortSpeedAndDuplex, transmitPowerLinked=transmitPowerLinked, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, rxColorCode=rxColorCode, configurationInstancedTable=configurationInstancedTable, whitelistRMMacAddress=whitelistRMMacAddress, cableDiagnosticsResultTableIndex=cableDiagnosticsResultTableIndex, sNTPLastSync=sNTPLastSync, aAAControlGroup=aAAControlGroup, mainPSUPortSpeedAndDuplex=mainPSUPortSpeedAndDuplex, ethernetStatisticsRxFramesWithError=ethernetStatisticsRxFramesWithError, iPv6Address=iPv6Address, channelBandwidth=channelBandwidth, licenseAdvanceHCMPSupport=licenseAdvanceHCMPSupport, tFTPServerPortNumber=tFTPServerPortNumber, mgmtInstancedTableIndex=mgmtInstancedTableIndex, transmitPower=transmitPower, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, frequencyVariant=frequencyVariant, alarmInstancedTableEntry=alarmInstancedTableEntry, ethernetPriorityTableIndex=ethernetPriorityTableIndex, wirelessLinkStatus=wirelessLinkStatus, mainPSUPortAutoNegotiation=mainPSUPortAutoNegotiation, subbandLowestFrequency=subbandLowestFrequency, licenseGroup=licenseGroup, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, linkSymmetry=linkSymmetry, sFPPortAllocation=sFPPortAllocation, secureModeAlarm=secureModeAlarm, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, mainPSUPortStatusTrap=mainPSUPortStatusTrap, telnetAccessEnabled=telnetAccessEnabled, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, byteErrorRatioLinked=byteErrorRatioLinked, bridgeGroup=bridgeGroup, auxPortPowerOverEthernetOutput=auxPortPowerOverEthernetOutput, ethernetStatisticsTxFrames=ethernetStatisticsTxFrames, l2CPPriorityTableNumber=l2CPPriorityTableNumber, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, sNMPv3Enable=sNMPv3Enable, softwareVersion=softwareVersion, systemReset=systemReset, managementPortWirelessDownAlert=managementPortWirelessDownAlert, tDM=tDM, licenseFullCapabilityTrialStatus=licenseFullCapabilityTrialStatus, iPv6PrefixLength=iPv6PrefixLength, gatewayIPAddress=gatewayIPAddress, masterSlaveMode=masterSlaveMode, ethernetPriorityTableNumber=ethernetPriorityTableNumber, sNTPServerTableEntry=sNTPServerTableEntry, nIDULanPortStatus=nIDULanPortStatus, sMTPServerResolvedInternetAddress=sMTPServerResolvedInternetAddress, syslogControlGroup=syslogControlGroup, mainPSUPortQLTx=mainPSUPortQLTx, licenseRegulatoryBandsTableEntry=licenseRegulatoryBandsTableEntry, cableDiagnosticsWarning=cableDiagnosticsWarning, syslogControl=syslogControl, mABNominalModulationMode=mABNominalModulationMode, transmitFreqMHz=transmitFreqMHz, licenseInstancedTableNumber=licenseInstancedTableNumber, dataPortPauseFrames=dataPortPauseFrames, wirelessLinkAvailability=wirelessLinkAvailability, apc=apc, nIDULanPortDisabledWarningTrap=nIDULanPortDisabledWarningTrap, lowestTDMModulationMode=lowestTDMModulationMode, mainPSUPortAcceptedQLRx=mainPSUPortAcceptedQLRx, ptpTrapPrefix=ptpTrapPrefix, tFTPClient=tFTPClient, transmitModulationModeLinked=transmitModulationModeLinked, deviceESN=deviceESN, cableDiagnosticsResultTableEntry=cableDiagnosticsResultTableEntry, sNMPTrapVersion=sNMPTrapVersion, licenseRegulatoryBandsTableIndex=licenseRegulatoryBandsTableIndex, incompatibleRegulatoryBands=incompatibleRegulatoryBands, licenseInstancedTableEntry=licenseInstancedTableEntry, sMTPServerPortNumber=sMTPServerPortNumber, mainPSUPortQLRx=mainPSUPortQLRx, sNTPServerTableIndex=sNTPServerTableIndex, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, auxPortConfigurationMismatchTrap=auxPortConfigurationMismatchTrap, supplementaryGroup=supplementaryGroup, tDMChannelLoopback=tDMChannelLoopback, remoteInternetAddressType=remoteInternetAddressType, daylightSaving=daylightSaving, transmitterChannels=transmitterChannels, receiveDataRateLinked=receiveDataRateLinked, sFPError=sFPError, sFPPortSyncERxStatus=sFPPortSyncERxStatus, wirelessLinkStatusAlarmTrap=wirelessLinkStatusAlarmTrap, ptpCompliance=ptpCompliance, altitude=altitude, licenseTDDSyncSupport=licenseTDDSyncSupport, tFTPServerInternetAddressType=tFTPServerInternetAddressType, phyInstancedStatusTableEntry=phyInstancedStatusTableEntry, mgmtInstancedTableEntry=mgmtInstancedTableEntry, licenseInstancedTable=licenseInstancedTable, sNMPTrapTableIndex=sNMPTrapTableIndex, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, l2CPPriorityTableIndex=l2CPPriorityTableIndex, signalStrengthRatio=signalStrengthRatio, incompatibleMasterAndSlave=incompatibleMasterAndSlave, cableDiagnostics=cableDiagnostics, hTTPSAccessEnabled=hTTPSAccessEnabled, auxPortStatus=auxPortStatus, mABProtocolVID=mABProtocolVID, remoteMACAddress=remoteMACAddress, authorizationMethod=authorizationMethod, transparentClock=transparentClock, sFPPortAutoNegAdvertisement=sFPPortAutoNegAdvertisement, alarmsGroup=alarmsGroup, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, sNTPServerAuthenticationProtocol=sNTPServerAuthenticationProtocol, snmpControl=snmpControl, sFPPortConfigurationMismatch=sFPPortConfigurationMismatch, nIDULanPortConfigurationMismatchTrap=nIDULanPortConfigurationMismatchTrap, authorizationControlGroup=authorizationControlGroup, remoteInternetAddress=remoteInternetAddress, licenseOOBManagementSupport=licenseOOBManagementSupport, dataBridgingStatusAlarmTrap=dataBridgingStatusAlarmTrap, configurationInstancedTableIndex=configurationInstancedTableIndex, ethernetStatisticsTable=ethernetStatisticsTable, ethernetStatisticsTableNumber=ethernetStatisticsTableNumber, transparentClockVLAN=transparentClockVLAN, rangingMode=rangingMode, dSCPManagementPriority=dSCPManagementPriority, l2CPPriorityTableEntry=l2CPPriorityTableEntry, ethernetStatisticsRxBroadcasts=ethernetStatisticsRxBroadcasts, sntpGroup=sntpGroup, licenseEncryption=licenseEncryption, dataBridgingStatusLinked=dataBridgingStatusLinked, reset=reset, tempPcb=tempPcb, sNTPServerPortNumber=sNTPServerPortNumber, licenseTLSRekey=licenseTLSRekey, mainPSUPortAutoMdix=mainPSUPortAutoMdix, sFPPortSyncEMasterSlaveStatus=sFPPortSyncEMasterSlaveStatus, wirelessOutEthernetOctets=wirelessOutEthernetOctets, searchState=searchState, rawReceivePower=rawReceivePower)
mibBuilder.exportSymbols("CAMBIUM-PTP670-MIB", noWirelessChannelAvailable=noWirelessChannelAvailable, remoteMaximumTransmitPowerLinked=remoteMaximumTransmitPowerLinked, userPassword=userPassword, encryptionGroup=encryptionGroup, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, iPv4Address=iPv4Address, sNTPServerTableNumber=sNTPServerTableNumber, phyStatus=phyStatus, sMTPEmailAlert=sMTPEmailAlert, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, ethernetInstancedTableIndex=ethernetInstancedTableIndex, tDMInterfaceStatusTrap=tDMInterfaceStatusTrap, mainPSUPortStatus=mainPSUPortStatus, tDMConfigurationMismatch=tDMConfigurationMismatch, pubInstancedStatsTableIndex=pubInstancedStatsTableIndex, mainPSUPortAutoNegAdvertisement=mainPSUPortAutoNegAdvertisement, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, licenseIssueNumber=licenseIssueNumber, vectorErrorLinked=vectorErrorLinked, sNMPEnabledTraps=sNMPEnabledTraps, ethernetStatisticsRxOctets=ethernetStatisticsRxOctets, dNSServerTable=dNSServerTable, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, useVLANForMABProtocol=useVLANForMABProtocol, mABState=mABState, authorizationControlTableNumber=authorizationControlTableNumber, sNMPTrapResolvedInternetAddress=sNMPTrapResolvedInternetAddress, management=management, timeZone=timeZone, sMTPEnabledMessages=sMTPEnabledMessages, mABNominalTransmitCapacity=mABNominalTransmitCapacity, remoteInternetAddressLinked=remoteInternetAddressLinked, productVariant=productVariant, transmitModulationMode=transmitModulationMode, remoteUnitName=remoteUnitName, tDMInterfaceControl=tDMInterfaceControl, cableDiagPair1Results=cableDiagPair1Results, syslogLocalWrappedTrap=syslogLocalWrappedTrap, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, tFTPServerResolvedInternetAddress=tFTPServerResolvedInternetAddress, dfsGroup=dfsGroup, routerProtocols=routerProtocols, sFPPortStatusTrap=sFPPortStatusTrap, licenseRegulatoryBandsList=licenseRegulatoryBandsList, accessMethod=accessMethod, latitude=latitude, ethernetStatisticsTableIndex=ethernetStatisticsTableIndex, dNSServerPortNumber=dNSServerPortNumber, managementNetworkAccessEnabled=managementNetworkAccessEnabled, wirelessOutAllOctets=wirelessOutAllOctets, tdmTableEntry=tdmTableEntry, cableDiagPair3Distance=cableDiagPair3Distance, cableDiagnosticsPorts=cableDiagnosticsPorts, ethernetInstancedTable=ethernetInstancedTable, ptpGroups=ptpGroups, signalStrengthRatioLinked=signalStrengthRatioLinked, nIDULanPortDisabledWarning=nIDULanPortDisabledWarning, routerProtocolsGroup=routerProtocolsGroup, secureLicenseSignatureAlarm=secureLicenseSignatureAlarm, targetMACAddress=targetMACAddress, receiveChannel=receiveChannel, receiveModulationModeDetailLinked=receiveModulationModeDetailLinked)
