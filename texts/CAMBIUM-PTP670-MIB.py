...
mibBuilder.exportSymbols("CAMBIUM-PTP670-MIB", incompatibleMasterAndSlave=incompatibleMasterAndSlave, cableDiagPair2Results=cableDiagPair2Results, dNSPrimaryServer=dNSPrimaryServer, tDMChannelStatusTrap=tDMChannelStatusTrap, incompatibleRegulatoryBands=incompatibleRegulatoryBands, maxLinkRangeExceeded=maxLinkRangeExceeded, transmitterChannels=transmitterChannels, ethernetStatisticsRxBroadcasts=ethernetStatisticsRxBroadcasts, cableDiagnosticsResultTableNumber=cableDiagnosticsResultTableNumber, userTable=userTable, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, ethernetStatisticsDataSource=ethernetStatisticsDataSource, transmitFreqKHz=transmitFreqKHz, alarmInstancedTable=alarmInstancedTable, pubInstancedStatsTable=pubInstancedStatsTable, productName=productName, tDMChannelLoopback=tDMChannelLoopback, atpcHcmpMasterTargetRxPower=atpcHcmpMasterTargetRxPower, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, tDM=tDM, ethernetInstancedTableNumber=ethernetInstancedTableNumber, secureLicenseSignatureAlarmTrap=secureLicenseSignatureAlarmTrap, ethernetPriorityTable=ethernetPriorityTable, softwareVersion=softwareVersion, rangeLinked=rangeLinked, license=license, sNTPLastSync=sNTPLastSync, sNMPTrapTable=sNMPTrapTable, encryptionAlgorithm=encryptionAlgorithm, dNSServerInternetAddressType=dNSServerInternetAddressType, licenseMinFirmwareVersionPTP650Emulation=licenseMinFirmwareVersionPTP650Emulation, iPv6PrefixLength=iPv6PrefixLength, range=range, extendedSpectrumScanning=extendedSpectrumScanning, linkLoss=linkLoss, tDMConfigurationMismatchTrap=tDMConfigurationMismatchTrap, receiveDataRate=receiveDataRate, txMABFrames=txMABFrames, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, incompatibleRegulatoryBandsLinked=incompatibleRegulatoryBandsLinked, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, mgmtInstancedTableEntry=mgmtInstancedTableEntry, l2CPPriorityTableEntry=l2CPPriorityTableEntry, syncESlavePort=syncESlavePort, userTableEntry=userTableEntry, lowestDataModulationMode=lowestDataModulationMode, sFPError=sFPError, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, versions=versions, maximumNumberOfSlaves=maximumNumberOfSlaves, targetRange=targetRange, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, ptmp=ptmp, remoteMACAddress=remoteMACAddress, nIDULanPortConfigurationMismatchTrap=nIDULanPortConfigurationMismatchTrap, receiveModulationModeDetailLinked=receiveModulationModeDetailLinked, sNMPPortNumber=sNMPPortNumber, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, transparentClockSourcePortAlarm=transparentClockSourcePortAlarm, supplementaryGroup=supplementaryGroup, nIDULanPortDisabledWarningTrap=nIDULanPortDisabledWarningTrap, bootVersion=bootVersion, mainPSUPortDisabledWarningTrap=mainPSUPortDisabledWarningTrap, cableDiagPair3Distance=cableDiagPair3Distance, productVariant=productVariant, licenseInstancedTableEntry=licenseInstancedTableEntry, aggregateDataRate=aggregateDataRate, wirelessOutEthernetOctets=wirelessOutEthernetOctets, auxPortStatus=auxPortStatus, mainPSUPortQLTx=mainPSUPortQLTx, transmitPowerLinked=transmitPowerLinked, cableDiagnosticsResultsDateTime=cableDiagnosticsResultsDateTime, tDDControl=tDDControl, whitelistRMMacAddress=whitelistRMMacAddress, ptpGroups=ptpGroups, licenseRegulatoryBandsTable=licenseRegulatoryBandsTable, installStatus=installStatus, alarmInstancedTableNumber=alarmInstancedTableNumber, encryptionGroup=encryptionGroup, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, tDMInterfaceControl=tDMInterfaceControl, tempPcb=tempPcb, mainPSUPortDisabledWarning=mainPSUPortDisabledWarning, deviceESN=deviceESN, altitude=altitude, licenseIEEE1588Support=licenseIEEE1588Support, transparentClockPort=transparentClockPort, dataBridgingStatusAlarmLinked=dataBridgingStatusAlarmLinked, licenseRemainingTrialPeriodAlarmTrap=licenseRemainingTrialPeriodAlarmTrap, pubInstancedStatsTableNumber=pubInstancedStatsTableNumber, pubInstancedStatsTableIndex=pubInstancedStatsTableIndex, wirelessTopology=wirelessTopology, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNTPState=sNTPState, tDDSynchronizationStatus=tDDSynchronizationStatus, cableDiagnosticsResultTableEntry=cableDiagnosticsResultTableEntry, iPVersion=iPVersion, secureLicenseSignatureAlarm=secureLicenseSignatureAlarm, userName=userName, auxPortPowerOverEthernetOutput=auxPortPowerOverEthernetOutput, nIDULanPortStatus=nIDULanPortStatus, dataBridgingStatus=dataBridgingStatus, mainPSUPortAcceptedQLRx=mainPSUPortAcceptedQLRx, configurationInstancedTable=configurationInstancedTable, alarmsGroup=alarmsGroup, bridge=bridge, unitName=unitName, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, deviceMACAddress=deviceMACAddress, dSCPManagementPriority=dSCPManagementPriority, sFPPortStatus=sFPPortStatus, rangingMode=rangingMode, useVLANForManagementInterfaces=useVLANForManagementInterfaces, ethernetStatisticsRxFramesOversize=ethernetStatisticsRxFramesOversize, licenseAdvanceHCMPSupport=licenseAdvanceHCMPSupport, mABCurrentTransmitCapacity=mABCurrentTransmitCapacity, remoteMACAddressLinked=remoteMACAddressLinked, ethernetStatisticsRxFramesUndersize=ethernetStatisticsRxFramesUndersize, wirelessLinkStatusAlarmTrap=wirelessLinkStatusAlarmTrap, sNMPTrapResolvedInternetAddress=sNMPTrapResolvedInternetAddress, dNSServerTable=dNSServerTable, ethernetStatisticsRxFramesWithError=ethernetStatisticsRxFramesWithError, sNTPSync=sNTPSync, licenseMaxLinkRange=licenseMaxLinkRange, maximumTransmitPower=maximumTransmitPower, auxPortConfigurationMismatch=auxPortConfigurationMismatch, dfs=dfs, sNTPServerTable=sNTPServerTable, transmitFreqMHz=transmitFreqMHz, tdmTableEntry=tdmTableEntry, linkName=linkName, tdmTableIndex=tdmTableIndex, installArmStateTrap=installArmStateTrap, auxPortGigabitMasterSlaveStatus=auxPortGigabitMasterSlaveStatus, iPv6GatewayAddress=iPv6GatewayAddress, sntp=sntp, aggregateDataRateLinked=aggregateDataRateLinked, dNSServerPortNumber=dNSServerPortNumber, erroredSeconds=erroredSeconds, sFPPortConfigurationMismatchTrap=sFPPortConfigurationMismatchTrap, sFPPortDisabledWarningTrap=sFPPortDisabledWarningTrap, mainPSUPortGigabitMasterSlaveStatus=mainPSUPortGigabitMasterSlaveStatus, sFPPortConfigurationMismatch=sFPPortConfigurationMismatch, phyInstancedStatusTableEntry=phyInstancedStatusTableEntry, phyControlInstancedTableNumber=phyControlInstancedTableNumber, linkLossLinked=linkLossLinked, lbtDetectedTrap=lbtDetectedTrap, installStatusTrap=installStatusTrap, sNMPTrapTableIndex=sNMPTrapTableIndex, wirelessOutAllOctets=wirelessOutAllOctets, unavailableSeconds=unavailableSeconds, sNTPServerTableIndex=sNTPServerTableIndex, dataBridgingAvailability=dataBridgingAvailability, phyControlInstancedTableEntry=phyControlInstancedTableEntry, nIDULanPortConfigurationMismatch=nIDULanPortConfigurationMismatch, mainPSUPortQLRx=mainPSUPortQLRx, pubInstancedStatsTableEntry=pubInstancedStatsTableEntry, managementPortWirelessDownAlert=managementPortWirelessDownAlert, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, ptp=ptp, phyControlInstancedTable=phyControlInstancedTable, sNTPPollInterval=sNTPPollInterval, cableDiagnosticsResultTable=cableDiagnosticsResultTable, dNSServerTableEntry=dNSServerTableEntry, systemReset=systemReset, packetsFromInternalStack=packetsFromInternalStack, tDMConfigurationMismatch=tDMConfigurationMismatch, wirelessLinkAvailabilityLinked=wirelessLinkAvailabilityLinked, sFPErrorTrap=sFPErrorTrap, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, syslogLocalWrappedTrap=syslogLocalWrappedTrap, authorizationControlTableNumber=authorizationControlTableNumber, subbandLowestFrequency=subbandLowestFrequency, mABProtocolVLANPriority=mABProtocolVLANPriority, vLANManagementPriority=vLANManagementPriority, mainPSUPortAutoMdix=mainPSUPortAutoMdix, dataBridgingAvailabilityLinked=dataBridgingAvailabilityLinked, ethernetStatisticsTable=ethernetStatisticsTable, remoteUnitName=remoteUnitName, mainPSUPortQLRxOverwrite=mainPSUPortQLRxOverwrite, rawReceivePowerLinked=rawReceivePowerLinked, ethernetInstancedTableEntry=ethernetInstancedTableEntry, mainPSUPortSyncERxStatus=mainPSUPortSyncERxStatus, licenseOOBManagementSupport=licenseOOBManagementSupport, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, supplementary=supplementary, ethernetInstancedTable=ethernetInstancedTable, microwaveAdaptiveBandwidth=microwaveAdaptiveBandwidth, managementNetworkAccessEnabled=managementNetworkAccessEnabled, auxPortPowerOverEthernetOutputStatus=auxPortPowerOverEthernetOutputStatus, enableTransmission=enableTransmission, licenseNumberOfRegulatoryBands=licenseNumberOfRegulatoryBands, ethernetStatisticsTxFrames=ethernetStatisticsTxFrames, secureModeAlarmTrap=secureModeAlarmTrap, systemClock=systemClock, dataPortPauseFrames=dataPortPauseFrames, dNSServerTableIndex=dNSServerTableIndex, pubStats=pubStats, vectorErrorLinked=vectorErrorLinked, mABTransmissionInterval=mABTransmissionInterval, transparentClockVID=transparentClockVID, mainPSUPortSpeedAndDuplex=mainPSUPortSpeedAndDuplex, auxPortQLRx=auxPortQLRx, dfsTableIndex=dfsTableIndex, hCMPMaximumLinkRange=hCMPMaximumLinkRange, byteErrorRatioLinked=byteErrorRatioLinked, apc=apc, licenseVersion=licenseVersion, signalStrengthRatio=signalStrengthRatio, enabledDiagnosticAlarms=enabledDiagnosticAlarms, hCMPLinkSymmetry=hCMPLinkSymmetry, tDDSynchronizationMode=tDDSynchronizationMode, dfsTable=dfsTable, receiveModulationModeLinked=receiveModulationModeLinked, alarms=alarms, nIDULanPortAutoNegotiation=nIDULanPortAutoNegotiation, l2CPPriorityTableIndex=l2CPPriorityTableIndex, configuration=configuration, wirelessLinkStatusAlarm=wirelessLinkStatusAlarm, encryption=encryption, ethernetGroup=ethernetGroup, sMTPServerResolvedInternetAddress=sMTPServerResolvedInternetAddress, syslogState=syslogState, mABHoldoffPeriod=mABHoldoffPeriod, blacklistRMMacAddress=blacklistRMMacAddress, sMTPEmailAlert=sMTPEmailAlert, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, remoteInternetAddress=remoteInternetAddress, daylightSaving=daylightSaving, auxPortPowerOverEthernetOutputStatusTrap=auxPortPowerOverEthernetOutputStatusTrap, ethernetPriorityTableIndex=ethernetPriorityTableIndex, authorizationControlTableIndex=authorizationControlTableIndex, linkModeOptimizationMismatch=linkModeOptimizationMismatch, mainPSUPortStatusTrap=mainPSUPortStatusTrap, configurationInstancedTableEntry=configurationInstancedTableEntry, unitOutOfCalibration=unitOutOfCalibration, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNTPServerTableNumber=sNTPServerTableNumber, deviceMSN=deviceMSN, receiveDataRateLinked=receiveDataRateLinked, ptpCompliance=ptpCompliance, smtpGroup=smtpGroup, remoteUnitNameLinked=remoteUnitNameLinked, licenseFullCapabilityTrialStatus=licenseFullCapabilityTrialStatus, transmitModulationMode=transmitModulationMode, auxPortDisabledWarning=auxPortDisabledWarning, searchState=searchState, licenseLongMinimumFirmwareVersion=licenseLongMinimumFirmwareVersion, sNTPServerKeyIdentifier=sNTPServerKeyIdentifier, apcGroup=apcGroup, snmpControl=snmpControl, dfsTableNumber=dfsTableNumber, sFPPortQLTx=sFPPortQLTx, tDMGroup=tDMGroup)
mibBuilder.exportSymbols("CAMBIUM-PTP670-MIB", phyControl=phyControl, mgmtInstancedTableIndex=mgmtInstancedTableIndex, groupID=groupID, licenseTLSRekey=licenseTLSRekey, licenseIPv6Support=licenseIPv6Support, wirelessOutOctets=wirelessOutOctets, dataBridgingStatusAlarm=dataBridgingStatusAlarm, signalStrengthRatioLinked=signalStrengthRatioLinked, remoteMaximumTransmitPower=remoteMaximumTransmitPower, dataPortWirelessDownAlert=dataPortWirelessDownAlert, byteErrorRatio=byteErrorRatio, sFPPortSpeedAndDuplex=sFPPortSpeedAndDuplex, hTTPAccessEnabled=hTTPAccessEnabled, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, mABState=mABState, cableDiagPair2Distance=cableDiagPair2Distance, rawReceivePower=rawReceivePower, sNTPSyncTrap=sNTPSyncTrap, userPassword=userPassword, iPv4Address=iPv4Address, dNSResolver=dNSResolver, licenseEncryption=licenseEncryption, licenseRemainingTrialPeriod=licenseRemainingTrialPeriod, cableLoss=cableLoss, wirelessLinkStatusLinked=wirelessLinkStatusLinked, iPv6Address=iPv6Address, auxPortSSMTx=auxPortSSMTx, oOBPriorityQueueMapping=oOBPriorityQueueMapping, syslogControlGroup=syslogControlGroup, maxLinkRangeExceededLinked=maxLinkRangeExceededLinked, hTTPSPortNumber=hTTPSPortNumber, nIDULanPortSpeedAndDuplex=nIDULanPortSpeedAndDuplex, mABMaintenanceLevel=mABMaintenanceLevel, whitelistRMEnabled=whitelistRMEnabled, userEnabled=userEnabled, antennaSelection=antennaSelection, regulatoryBandTrap=regulatoryBandTrap, tLSMinimumSecurityLevel=tLSMinimumSecurityLevel, capacityVariantMismatchLinked=capacityVariantMismatchLinked, phyInstancedStatusTableIndex=phyInstancedStatusTableIndex, incompatibleMasterAndSlaveLinked=incompatibleMasterAndSlaveLinked, useVLANForMABProtocol=useVLANForMABProtocol, sNMPTrapPortNumber=sNMPTrapPortNumber, licenseRARSupport=licenseRARSupport, telnetAccessEnabled=telnetAccessEnabled, sFPPortSyncEMasterSlaveStatus=sFPPortSyncEMasterSlaveStatus, ethernetStatisticsRxFrames=ethernetStatisticsRxFrames, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, tDMInterfaceStatus=tDMInterfaceStatus, licenseRegulatoryBandsTableEntry=licenseRegulatoryBandsTableEntry, telnetPortNumber=telnetPortNumber, licenseAuxiliaryPortSupport=licenseAuxiliaryPortSupport, wirelessLinkStatus=wirelessLinkStatus, cableDiagnosticsWarning=cableDiagnosticsWarning, subbandHighestFrequency=subbandHighestFrequency, ethernetStatisticsTxOctets=ethernetStatisticsTxOctets, licenseRegulatoryBandsList=licenseRegulatoryBandsList, cableDiagnosticsPorts=cableDiagnosticsPorts, cableDiagPair4Distance=cableDiagPair4Distance, receiveChannel=receiveChannel, sNTPServerRetries=sNTPServerRetries, authorizationControlGroup=authorizationControlGroup, dfsTableEntry=dfsTableEntry, licenseInstancedTableIndex=licenseInstancedTableIndex, latitude=latitude, versionsGroup=versionsGroup, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, subnetMask=subnetMask, mainPSUPortStatus=mainPSUPortStatus, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, capacityVariantMismatchTrap=capacityVariantMismatchTrap, rxColorCode=rxColorCode, sFPPortAcceptedQLRx=sFPPortAcceptedQLRx, accessMethod=accessMethod, bridgeGroup=bridgeGroup, transmitDataRate=transmitDataRate, mainPSUPortSyncEMasterSlaveStatus=mainPSUPortSyncEMasterSlaveStatus, linkSymmetry=linkSymmetry, receivePowerLinked=receivePowerLinked, sNTPServerInternetAddressType=sNTPServerInternetAddressType, licenseRemainingTrialPeriodAlarm=licenseRemainingTrialPeriodAlarm, transmitPower=transmitPower, sFPPortStatusTrap=sFPPortStatusTrap, aAAControl=aAAControl, ethernetStatisticsTableNumber=ethernetStatisticsTableNumber, licenseGasGroup=licenseGasGroup, hTTPSAccessEnabled=hTTPSAccessEnabled, transparentClock=transparentClock, licenseTrialPeriod=licenseTrialPeriod, sNMPTrapVersion=sNMPTrapVersion, mainPSUPortConfigurationMismatchTrap=mainPSUPortConfigurationMismatchTrap, sNTPServerAuthenticationProtocol=sNTPServerAuthenticationProtocol, cableDiagPair4Results=cableDiagPair4Results, hardwareVersion=hardwareVersion, channelBandwidth=channelBandwidth, nIDULanPortAutoNegAdvertisement=nIDULanPortAutoNegAdvertisement, aAAControlGroup=aAAControlGroup, configurationInstancedTableNumber=configurationInstancedTableNumber, hTTPPortNumber=hTTPPortNumber, frequencyVariant=frequencyVariant, managementGroup=managementGroup, portAllocationMismatch=portAllocationMismatch, pubStatsGroup=pubStatsGroup, sNTPPrimaryServer=sNTPPrimaryServer, sNTPServerTimeout=sNTPServerTimeout, linkModeOptimizationMismatchLinked=linkModeOptimizationMismatchLinked, sNTPServerResolvedInternetAddress=sNTPServerResolvedInternetAddress, auxPortDisabledWarningTrap=auxPortDisabledWarningTrap, blacklistRMEnabled=blacklistRMEnabled, dataBridgingStatusAlarmTrap=dataBridgingStatusAlarmTrap, auxPortSyncERxStatus=auxPortSyncERxStatus, mgmtInstancedTableNumber=mgmtInstancedTableNumber, sFPPortAutoMdix=sFPPortAutoMdix, lowestTDMModulationMode=lowestTDMModulationMode, tFTPServerPortNumber=tFTPServerPortNumber, receivePower=receivePower, licenseMinimumFirmwareVersion=licenseMinimumFirmwareVersion, nIDULanPortAutoMdix=nIDULanPortAutoMdix, cableDiagnosticsGroup=cableDiagnosticsGroup, tFTPClient=tFTPClient, ptpTraps=ptpTraps, sNTPServerPortNumber=sNTPServerPortNumber, receiveModulationModeDetail=receiveModulationModeDetail, cableDiagPair3Results=cableDiagPair3Results, sFPPortSyncERxStatus=sFPPortSyncERxStatus, authorizationControlTableEntry=authorizationControlTableEntry, tDMChannelCableLength=tDMChannelCableLength, sFPPortQLRx=sFPPortQLRx, alarmInstancedTableEntry=alarmInstancedTableEntry, portAllocationMismatchLinked=portAllocationMismatchLinked, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, phyInstancedStatusTable=phyInstancedStatusTable, remoteMaximumTransmitPowerLinked=remoteMaximumTransmitPowerLinked, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, licenseInstancedTable=licenseInstancedTable, authorizationMethod=authorizationMethod, sFPPortAllocation=sFPPortAllocation, ethernetStatisticsTxBroadcasts=ethernetStatisticsTxBroadcasts, resetGroup=resetGroup, sMTPEnabledMessages=sMTPEnabledMessages, transparentClockVLAN=transparentClockVLAN, ptp670=ptp670, configurationInstancedTableIndex=configurationInstancedTableIndex, licenseCapacity=licenseCapacity, snmpControlGroup=snmpControlGroup, qOSPriorityScheme=qOSPriorityScheme, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, tDMChannelStatus=tDMChannelStatus, reset=reset, notificationsGroup=notificationsGroup, licenseSFPPortSupport=licenseSFPPortSupport, vectorError=vectorError, licenseUnitSerialNumber=licenseUnitSerialNumber, cableDiagPair1Results=cableDiagPair1Results, auxPortStatusTrap=auxPortStatusTrap, ethernetStatisticsTableEntry=ethernetStatisticsTableEntry, dfsPeaks=dfsPeaks, sFPPortAutoNegAdvertisement=sFPPortAutoNegAdvertisement, sNMPv3Enable=sNMPv3Enable, licenseIssueNumber=licenseIssueNumber, syncEEquipmentClock=syncEEquipmentClock, syslogStateTrap=syslogStateTrap, l2CPPriorityTableNumber=l2CPPriorityTableNumber, remoteInternetAddressLinked=remoteInternetAddressLinked, severelyErroredSeconds=severelyErroredSeconds, tdmTableNumber=tdmTableNumber, masterSlaveMode=masterSlaveMode, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, tDMChannelLoopbackTrap=tDMChannelLoopbackTrap, mABNominalModulationMode=mABNominalModulationMode, auxPortAllocation=auxPortAllocation, sNMPTrapTableEntry=sNMPTrapTableEntry, ethernet=ethernet, installationMode=installationMode, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, sNTPServerTableEntry=sNTPServerTableEntry, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, vLANManagementVID=vLANManagementVID, syncETracking=syncETracking, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, phyControlGroup=phyControlGroup, phyInstancedStatusTableNumber=phyInstancedStatusTableNumber, PYSNMP_MODULE_ID=cambium, secureModeAlarm=secureModeAlarm, mainPSUPortConfigurationMismatch=mainPSUPortConfigurationMismatch, userRole=userRole, routerProtocolsGroup=routerProtocolsGroup, configurationGroup=configurationGroup, sMTPServerPortNumber=sMTPServerPortNumber, linkModeOptimization=linkModeOptimization, syslogClient=syslogClient, longitude=longitude, ethernetStatisticsTableIndex=ethernetStatisticsTableIndex, tDDSynchronizationAlarm=tDDSynchronizationAlarm, auxPortConfigurationMismatchTrap=auxPortConfigurationMismatchTrap, txColorCode=txColorCode, licenseTDDSyncSupport=licenseTDDSyncSupport, auxPortSpeedAndDuplex=auxPortSpeedAndDuplex, incompatibleRegulatoryBandsTrap=incompatibleRegulatoryBandsTrap, dNSGroup=dNSGroup, sntpGroup=sntpGroup, sNTPServerInternetAddress=sNTPServerInternetAddress, dNS=dNS, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, remoteInternetAddressTypeLinked=remoteInternetAddressTypeLinked, phyStatus=phyStatus, nIDULanPortDisabledWarning=nIDULanPortDisabledWarning, authorizationControl=authorizationControl, licenseRegulatoryBandsTableIndex=licenseRegulatoryBandsTableIndex, syslogControl=syslogControl, timeZone=timeZone, sNMPCommunityString=sNMPCommunityString, receiveFreqMHz=receiveFreqMHz, phyStatusGroup=phyStatusGroup, regulatoryBand=regulatoryBand, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, mainPSUPortSSMTx=mainPSUPortSSMTx, licenseGroupAccess=licenseGroupAccess, cableDiagnosticsResultTableIndex=cableDiagnosticsResultTableIndex, wirelessLinkAvailability=wirelessLinkAvailability, auxPortAcceptedQLRx=auxPortAcceptedQLRx, eIRP=eIRP, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, mABNominalTransmitCapacity=mABNominalTransmitCapacity, sFPPortAutoNegotiation=sFPPortAutoNegotiation, auxPortQLTx=auxPortQLTx, auxPortSyncEMasterSlaveStatus=auxPortSyncEMasterSlaveStatus, management=management, ethernetPriorityTableNumber=ethernetPriorityTableNumber, localPacketFiltering=localPacketFiltering, tDMEnabledChannels=tDMEnabledChannels, sFPPortSSMTx=sFPPortSSMTx, tFTPServerInternetAddressType=tFTPServerInternetAddressType, dataBridgingStatusLinked=dataBridgingStatusLinked, tDDControlGroup=tDDControlGroup, ethernetInstancedTableIndex=ethernetInstancedTableIndex, tFTPServerInternetAddress=tFTPServerInternetAddress, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, sNTPServerStatus=sNTPServerStatus, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, mainPSUPortAutoNegotiation=mainPSUPortAutoNegotiation, dNSServerTableNumber=dNSServerTableNumber, secondarySoftwareVersion=secondarySoftwareVersion, phyControlInstancedTableIndex=phyControlInstancedTableIndex, remoteInternetAddressType=remoteInternetAddressType, targetMACAddress=targetMACAddress, nIDULanPortStatusTrap=nIDULanPortStatusTrap, tFTPServerResolvedInternetAddress=tFTPServerResolvedInternetAddress, mainPSUPortAllocation=mainPSUPortAllocation, sFPPortQLRxOverwrite=sFPPortQLRxOverwrite, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, receiveModulationMode=receiveModulationMode, mABProtocolVID=mABProtocolVID)
mibBuilder.exportSymbols("CAMBIUM-PTP670-MIB", capacityVariantMismatch=capacityVariantMismatch, dfsMeans=dfsMeans, portAllocationMismatchTrap=portAllocationMismatchTrap, routerProtocols=routerProtocols, tDMChannelLineCode=tDMChannelLineCode, licenseCountry=licenseCountry, userTableIndex=userTableIndex, sNMPTrapInternetAddress=sNMPTrapInternetAddress, mainPSUPortAutoNegAdvertisement=mainPSUPortAutoNegAdvertisement, packetsToInternalStack=packetsToInternalStack, sMTPServerInternetAddressType=sMTPServerInternetAddressType, sFPPortDisabledWarning=sFPPortDisabledWarning, nIDULanPortGigabitMasterSlaveStatus=nIDULanPortGigabitMasterSlaveStatus, licenseMaxNumberOfTDMChannels=licenseMaxNumberOfTDMChannels, searchStateLinked=searchStateLinked, noWirelessChannelAvailable=noWirelessChannelAvailable, transmitDataRateLinked=transmitDataRateLinked, mPLSTCPriorityTable=mPLSTCPriorityTable, alarmInstancedTableIndex=alarmInstancedTableIndex, siteName=siteName, installStatusLinked=installStatusLinked, syncETrackingState=syncETrackingState, tdmTable=tdmTable, ethernetPriorityTableEntry=ethernetPriorityTableEntry, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, receiveFreqKHz=receiveFreqKHz, unitIdentificationGroup=unitIdentificationGroup, cableDiagnostics=cableDiagnostics, licenseSecurityLevel=licenseSecurityLevel, cambium=cambium, antennaGain=antennaGain, unitIdentification=unitIdentification, cableDiagPair1Distance=cableDiagPair1Distance, licenseSyncESupport=licenseSyncESupport, userTableNumber=userTableNumber, authorizationControlTable=authorizationControlTable, atpcHcmpMasterTxPower=atpcHcmpMasterTxPower, transmitChannel=transmitChannel, ethernetStatisticsRxOctets=ethernetStatisticsRxOctets, sMTPServerInternetAddress=sMTPServerInternetAddress, smtp=smtp, installArmState=installArmState, iPDSCPPriorityTable=iPDSCPPriorityTable, dfsGroup=dfsGroup, sFPPortGigabitMasterSlaveStatus=sFPPortGigabitMasterSlaveStatus, transmitModulationModeLinked=transmitModulationModeLinked, channelChangeTrap=channelChangeTrap, l2CPPriorityTable=l2CPPriorityTable, maxLinkRangeExceededTrap=maxLinkRangeExceededTrap, mgmtInstancedTable=mgmtInstancedTable, sNMPEnabledTraps=sNMPEnabledTraps, sNMPTrapTableNumber=sNMPTrapTableNumber, gatewayIPAddress=gatewayIPAddress, ptpTrapPrefix=ptpTrapPrefix, dNSServerInternetAddress=dNSServerInternetAddress, licenseInstancedTableNumber=licenseInstancedTableNumber, licenseBandwidthCap=licenseBandwidthCap, licenseHCMPSupport=licenseHCMPSupport, tDMInterfaceStatusTrap=tDMInterfaceStatusTrap, licenseGroup=licenseGroup, syslogClientTrap=syslogClientTrap, cableDiagnosticsControl=cableDiagnosticsControl)
