...
# Produced by pysmi-1.1.10 at Fri Oct 27 07:43:11 2023
...
mib_2, IpAddress, ModuleIdentity, Counter64, enterprises, MibIdentifier, TimeTicks, Counter32, snmpModules, Gauge32, ObjectIdentity, Bits, Unsigned32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, iso = mibBuilder.importSymbols("SNMPv2-SMI", "mib-2", "IpAddress", "ModuleIdentity", "Counter64", "enterprises", "MibIdentifier", "TimeTicks", "Counter32", "snmpModules", "Gauge32", "ObjectIdentity", "Bits", "Unsigned32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "iso")
...
mibBuilder.exportSymbols("XAVI-XG6846-MIB", pauseReceived=pauseReceived, port1=port1, staticIpSubMask=staticIpSubMask, reboot=reboot, lan2=lan2, rxPower=rxPower, portIndex=portIndex, jumb=jumb, qosTable=qosTable, unicastsReceived=unicastsReceived, temperature=temperature, jumbwan=jumbwan, vlan=vlan, bias=bias, deviceinfo=deviceinfo, vlangroupTable=vlangroupTable, staticIpDomainName=staticIpDomainName, groupid=groupid, vendorPn=vendorPn, action=action, unicastsTransmitted=unicastsTransmitted, vendorName=vendorName, qos=qos, portStatEntry=portStatEntry, statisticTable=statisticTable, primaryDns=primaryDns, groupEntry=groupEntry, vlanid=vlanid, qmode=qmode, tftp=tftp, staticIpSecondaryDns=staticIpSecondaryDns, jumblan4=jumblan4, lan1=lan1, portName=portName, portTable=portTable, vendorRev=vendorRev, qosEntry=qosEntry, priority=priority, staticIpAddr=staticIpAddr, broadcastsTransmitted=broadcastsTransmitted, staticIpHostName=staticIpHostName, fcsErrorTransmitted=fcsErrorTransmitted, serialnum=serialnum, lanportspeed=lanportspeed, tftpSevIp=tftpSevIp, groupIndex=groupIndex, txPower=txPower, catvEnable=catvEnable, adminStatus=adminStatus, staticIpGateway=staticIpGateway, lan4=lan4, portPower=portPower, internetPort=internetPort, fileType=fileType, catv=catv, wan=wan, port4=port4, jumblan2=jumblan2, multicastsTransmitted=multicastsTransmitted, qmapping=qmapping, qosIndex=qosIndex, port3=port3, dscpEnable=dscpEnable, lan3=lan3, secondaryDns=secondaryDns, portmode=portmode, fileName=fileName, vendorOui=vendorOui, lanportpause=lanportpause, broadcastsReceived=broadcastsReceived, jumblan1=jumblan1, staticDns=staticDns, hostname=hostname, lanportIndex=lanportIndex, voltage=voltage, multicastsReceived=multicastsReceived, wanType=wanType, speed=speed, portEntry=portEntry, lanportTable=lanportTable, statIndex=statIndex, domainame=domainame, rebootEnable=rebootEnable, staticIpPrimaryDns=staticIpPrimaryDns, operStatus=operStatus, modeEntry=modeEntry, lanportName=lanportName, fcsErrorReceived=fcsErrorReceived, port2=port2, ddminfo=ddminfo, xg6846=xg6846, jumblan3=jumblan3, portStatistic=portStatistic, pauseTransmitted=pauseTransmitted)
