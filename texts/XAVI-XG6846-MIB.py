...
# Produced by pysmi-1.1.8 at Mon Jan 16 15:37:21 2023
...
ModuleIdentity, Counter64, NotificationType, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, ObjectIdentity, MibIdentifier, Integer32, iso, Gauge32, enterprises, Bits, TimeTicks, Counter32, mib_2, snmpModules = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Counter64", "NotificationType", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "ObjectIdentity", "MibIdentifier", "Integer32", "iso", "Gauge32", "enterprises", "Bits", "TimeTicks", "Counter32", "mib-2", "snmpModules")
...
mibBuilder.exportSymbols("XAVI-XG6846-MIB", tftpSevIp=tftpSevIp, qmapping=qmapping, pauseTransmitted=pauseTransmitted, staticIpDomainName=staticIpDomainName, staticDns=staticDns, groupIndex=groupIndex, lanportIndex=lanportIndex, jumblan3=jumblan3, wan=wan, xg6846=xg6846, fileName=fileName, lan1=lan1, vlan=vlan, catvEnable=catvEnable, tftp=tftp, portPower=portPower, statisticTable=statisticTable, portName=portName, port4=port4, port1=port1, staticIpSecondaryDns=staticIpSecondaryDns, multicastsReceived=multicastsReceived, hostname=hostname, multicastsTransmitted=multicastsTransmitted, rxPower=rxPower, portEntry=portEntry, vendorRev=vendorRev, priority=priority, speed=speed, fileType=fileType, lanportpause=lanportpause, lan4=lan4, modeEntry=modeEntry, bias=bias, broadcastsTransmitted=broadcastsTransmitted, vlangroupTable=vlangroupTable, unicastsTransmitted=unicastsTransmitted, unicastsReceived=unicastsReceived, lan3=lan3, rebootEnable=rebootEnable, internetPort=internetPort, vlanid=vlanid, jumblan2=jumblan2, lanportTable=lanportTable, qosEntry=qosEntry, operStatus=operStatus, pauseReceived=pauseReceived, qmode=qmode, fcsErrorReceived=fcsErrorReceived, staticIpHostName=staticIpHostName, fcsErrorTransmitted=fcsErrorTransmitted, jumbwan=jumbwan, jumb=jumb, qosIndex=qosIndex, groupEntry=groupEntry, staticIpPrimaryDns=staticIpPrimaryDns, adminStatus=adminStatus, voltage=voltage, action=action, broadcastsReceived=broadcastsReceived, lanportName=lanportName, portStatEntry=portStatEntry, vendorName=vendorName, staticIpAddr=staticIpAddr, wanType=wanType, secondaryDns=secondaryDns, domainame=domainame, staticIpGateway=staticIpGateway, lan2=lan2, portmode=portmode, portIndex=portIndex, vendorPn=vendorPn, portStatistic=portStatistic, dscpEnable=dscpEnable, port3=port3, primaryDns=primaryDns, jumblan1=jumblan1, groupid=groupid, reboot=reboot, staticIpSubMask=staticIpSubMask, lanportspeed=lanportspeed, temperature=temperature, qosTable=qosTable, jumblan4=jumblan4, txPower=txPower, ddminfo=ddminfo, statIndex=statIndex, deviceinfo=deviceinfo, vendorOui=vendorOui, qos=qos, portTable=portTable, port2=port2, serialnum=serialnum, catv=catv)
