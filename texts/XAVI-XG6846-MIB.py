...
# Produced by pysmi-1.1.8 at Fri Dec  2 16:47:31 2022
...
ModuleIdentity, Counter64, MibIdentifier, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, snmpModules, ObjectIdentity, iso, Counter32, Unsigned32, Integer32, enterprises, Bits, Gauge32, mib_2, NotificationType, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Counter64", "MibIdentifier", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "snmpModules", "ObjectIdentity", "iso", "Counter32", "Unsigned32", "Integer32", "enterprises", "Bits", "Gauge32", "mib-2", "NotificationType", "IpAddress")
...
mibBuilder.exportSymbols("XAVI-XG6846-MIB", reboot=reboot, primaryDns=primaryDns, txPower=txPower, port4=port4, portName=portName, action=action, fileName=fileName, staticIpSubMask=staticIpSubMask, lanportName=lanportName, lanportIndex=lanportIndex, catvEnable=catvEnable, vendorName=vendorName, dscpEnable=dscpEnable, vlan=vlan, staticIpPrimaryDns=staticIpPrimaryDns, tftp=tftp, xg6846=xg6846, jumblan3=jumblan3, vendorRev=vendorRev, operStatus=operStatus, qmapping=qmapping, adminStatus=adminStatus, qosIndex=qosIndex, staticIpSecondaryDns=staticIpSecondaryDns, portIndex=portIndex, portEntry=portEntry, tftpSevIp=tftpSevIp, port2=port2, internetPort=internetPort, broadcastsTransmitted=broadcastsTransmitted, secondaryDns=secondaryDns, speed=speed, catv=catv, hostname=hostname, staticIpGateway=staticIpGateway, serialnum=serialnum, portTable=portTable, unicastsReceived=unicastsReceived, pauseReceived=pauseReceived, staticDns=staticDns, jumblan1=jumblan1, statisticTable=statisticTable, deviceinfo=deviceinfo, staticIpDomainName=staticIpDomainName, jumbwan=jumbwan, temperature=temperature, rebootEnable=rebootEnable, statIndex=statIndex, groupid=groupid, lan1=lan1, qmode=qmode, qos=qos, lan3=lan3, portPower=portPower, broadcastsReceived=broadcastsReceived, qosTable=qosTable, vendorOui=vendorOui, priority=priority, wan=wan, staticIpHostName=staticIpHostName, fcsErrorTransmitted=fcsErrorTransmitted, multicastsTransmitted=multicastsTransmitted, pauseTransmitted=pauseTransmitted, jumblan4=jumblan4, modeEntry=modeEntry, bias=bias, rxPower=rxPower, qosEntry=qosEntry, port1=port1, staticIpAddr=staticIpAddr, jumblan2=jumblan2, portStatEntry=portStatEntry, lanportTable=lanportTable, portmode=portmode, voltage=voltage, fcsErrorReceived=fcsErrorReceived, fileType=fileType, lan4=lan4, lanportpause=lanportpause, groupIndex=groupIndex, port3=port3, ddminfo=ddminfo, lan2=lan2, domainame=domainame, vendorPn=vendorPn, vlangroupTable=vlangroupTable, lanportspeed=lanportspeed, unicastsTransmitted=unicastsTransmitted, wanType=wanType, portStatistic=portStatistic, multicastsReceived=multicastsReceived, jumb=jumb, vlanid=vlanid, groupEntry=groupEntry)
