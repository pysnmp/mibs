...
# Produced by pysmi-1.1.8 at Thu Sep  7 12:06:11 2023
...
Bits, ModuleIdentity, snmpModules, MibIdentifier, TimeTicks, Counter32, iso, Counter64, Gauge32, Unsigned32, IpAddress, mib_2, Integer32, enterprises, NotificationType, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "ModuleIdentity", "snmpModules", "MibIdentifier", "TimeTicks", "Counter32", "iso", "Counter64", "Gauge32", "Unsigned32", "IpAddress", "mib-2", "Integer32", "enterprises", "NotificationType", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("XAVI-XG6846-MIB", qmapping=qmapping, catv=catv, primaryDns=primaryDns, qosTable=qosTable, qmode=qmode, vlan=vlan, qosIndex=qosIndex, jumblan4=jumblan4, portmode=portmode, vendorRev=vendorRev, fcsErrorTransmitted=fcsErrorTransmitted, lan4=lan4, txPower=txPower, staticIpSubMask=staticIpSubMask, catvEnable=catvEnable, rxPower=rxPower, lan2=lan2, bias=bias, portPower=portPower, multicastsReceived=multicastsReceived, multicastsTransmitted=multicastsTransmitted, port3=port3, portTable=portTable, deviceinfo=deviceinfo, fileType=fileType, fileName=fileName, staticIpDomainName=staticIpDomainName, jumblan3=jumblan3, jumb=jumb, unicastsReceived=unicastsReceived, lanportIndex=lanportIndex, statIndex=statIndex, rebootEnable=rebootEnable, adminStatus=adminStatus, priority=priority, groupEntry=groupEntry, statisticTable=statisticTable, vendorPn=vendorPn, portName=portName, secondaryDns=secondaryDns, lanportspeed=lanportspeed, lanportpause=lanportpause, port2=port2, port1=port1, speed=speed, tftpSevIp=tftpSevIp, staticIpHostName=staticIpHostName, broadcastsTransmitted=broadcastsTransmitted, dscpEnable=dscpEnable, staticIpPrimaryDns=staticIpPrimaryDns, staticIpSecondaryDns=staticIpSecondaryDns, vlangroupTable=vlangroupTable, lan3=lan3, wan=wan, modeEntry=modeEntry, broadcastsReceived=broadcastsReceived, staticIpAddr=staticIpAddr, lanportName=lanportName, wanType=wanType, portStatEntry=portStatEntry, port4=port4, lanportTable=lanportTable, operStatus=operStatus, voltage=voltage, pauseReceived=pauseReceived, pauseTransmitted=pauseTransmitted, staticIpGateway=staticIpGateway, lan1=lan1, portStatistic=portStatistic, tftp=tftp, qosEntry=qosEntry, qos=qos, serialnum=serialnum, domainame=domainame, action=action, fcsErrorReceived=fcsErrorReceived, staticDns=staticDns, internetPort=internetPort, vlanid=vlanid, unicastsTransmitted=unicastsTransmitted, vendorName=vendorName, jumblan1=jumblan1, reboot=reboot, hostname=hostname, groupIndex=groupIndex, xg6846=xg6846, temperature=temperature, portIndex=portIndex, jumblan2=jumblan2, vendorOui=vendorOui, portEntry=portEntry, jumbwan=jumbwan, ddminfo=ddminfo, groupid=groupid)
