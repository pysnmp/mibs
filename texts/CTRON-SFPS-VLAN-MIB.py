...
vlanTestAPI, vlanAMRRules, vlanAPI, vlanName, vlanCountAPI, vlanPort, vlanAMRSubnets, vlanSystem, vlanAMRStats = mibBuilder.importSymbols("CTRON-SFPS-INCLUDE-MIB", "vlanTestAPI", "vlanAMRRules", "vlanAPI", "vlanName", "vlanCountAPI", "vlanPort", "vlanAMRSubnets", "vlanSystem", "vlanAMRStats")
...
mibBuilder.exportSymbols("CTRON-SFPS-VLAN-MIB", sfpsVAPIVerb=sfpsVAPIVerb, vlanTestAPIOutputPort=vlanTestAPIOutputPort, vlanPortRouterPort=vlanPortRouterPort, vlanAMRSubnetsMask=vlanAMRSubnetsMask, vlanPortPortNum=vlanPortPortNum, vlanTestAPIVlanName=vlanTestAPIVlanName, vlanPortVlanName=vlanPortVlanName, vlanPortPortPolicy=vlanPortPortPolicy, vlanSystemDeltaPollTime=vlanSystemDeltaPollTime, sfpsVAPILayer3Learning=sfpsVAPILayer3Learning, vlanTestAPIOutputEntry=vlanTestAPIOutputEntry, vlanNameFloodPorts=vlanNameFloodPorts, vlanPortPortStatus=vlanPortPortStatus, VlanSwitchInstance=VlanSwitchInstance, sfpsVAPIUserAliasTag=sfpsVAPIUserAliasTag, vlanSystemTable=vlanSystemTable, vlanTestAPIOutputTable=vlanTestAPIOutputTable, sfpsVAPIOutPort=sfpsVAPIOutPort, vlanSystemEntry=vlanSystemEntry, HexInteger=HexInteger, vlanSystemLastPollTime=vlanSystemLastPollTime, sfpsVAPIRouterPort=sfpsVAPIRouterPort, vlanNameEnabledPorts=vlanNameEnabledPorts, vlanTestAPIOutputMap=vlanTestAPIOutputMap, SfpsAddress=SfpsAddress, vlanPortUniPolicy=vlanPortUniPolicy, sfpsVAPIUserMAC=sfpsVAPIUserMAC, vlanCountAPIAutoreg=vlanCountAPIAutoreg, vlanNameIndex=vlanNameIndex, sfpsVAPIVlanName=sfpsVAPIVlanName, vlanNameMappedPorts=vlanNameMappedPorts, vlanNameFloodPolicy=vlanNameFloodPolicy, vlanTestAPIVerb=vlanTestAPIVerb, vlanTestAPIOutputAble=vlanTestAPIOutputAble, vlanSystemAgentIP=vlanSystemAgentIP, vlanTestAPIOutputTap=vlanTestAPIOutputTap, vlanNameVlanId=vlanNameVlanId, vlanCountAPITotal=vlanCountAPITotal, vlanNameNHash=vlanNameNHash, vlanPortRelayAgent=vlanPortRelayAgent, vlanTestAPIVlanId=vlanTestAPIVlanId, vlanNameTable=vlanNameTable, vlanAMRRulesTable=vlanAMRRulesTable, sfpsVAPIAdminStatus=sfpsVAPIAdminStatus, vlanSystemSwitchInstance=vlanSystemSwitchInstance, SfpsSwitchPort=SfpsSwitchPort, vlanSystemOperStatus=vlanSystemOperStatus, vlanTestAPIOutputVlanName=vlanTestAPIOutputVlanName, sfpsVAPIPortPolicy=sfpsVAPIPortPolicy, vlanPortEntry=vlanPortEntry, vlanSystemAdminReset=vlanSystemAdminReset, vlanPortAdminStatus=vlanPortAdminStatus, vlanPortFloodPolicy=vlanPortFloodPolicy, vlanTestAPIOutputUserCnt=vlanTestAPIOutputUserCnt, vlanSystemFirstPollTime=vlanSystemFirstPollTime, vlanTestAPIOutputPolicy=vlanTestAPIOutputPolicy, sfpsVAPIAutoRegValue=sfpsVAPIAutoRegValue, vlanSystemPriorPollTime=vlanSystemPriorPollTime, sfpsVAPIInPort=sfpsVAPIInPort, vlanNameOperStatus=vlanNameOperStatus, vlanAMRStatsNumRulesEnabled=vlanAMRStatsNumRulesEnabled, vlanAMRRulesRule=vlanAMRRulesRule, sfpsVAPIUnicastPolicy=sfpsVAPIUnicastPolicy, sfpsVAPIVlanId=sfpsVAPIVlanId, vlanAMRStatsSingleMask=vlanAMRStatsSingleMask, vlanSystemVersion=vlanSystemVersion, sfpsVAPIUserAlias=sfpsVAPIUserAlias, vlanNameAdminStatus=vlanNameAdminStatus, sfpsVAPIRelayAgent=sfpsVAPIRelayAgent, vlanAMRSubnetsPrefix=vlanAMRSubnetsPrefix, vlanNameVlanRule=vlanNameVlanRule, vlanNameVlanName=vlanNameVlanName, vlanTestAPIOutputStatus=vlanTestAPIOutputStatus, vlanPortTable=vlanPortTable, vlanTestAPIOutputIndex=vlanTestAPIOutputIndex, sfpsVAPIAutoRegMask=sfpsVAPIAutoRegMask, vlanNameStatusTime=vlanNameStatusTime, vlanSystemAdminStatus=vlanSystemAdminStatus, vlanSystemLastChange=vlanSystemLastChange, vlanAMRRulesEntry=vlanAMRRulesEntry, vlanSystemDomainName=vlanSystemDomainName, sfpsVAPIAutoRegisterRule=sfpsVAPIAutoRegisterRule, vlanNameUniPolicy=vlanNameUniPolicy, vlanSystemOperTime=vlanSystemOperTime, vlanPortVlanId=vlanPortVlanId, vlanNameEntry=vlanNameEntry, vlanSystemMibRev=vlanSystemMibRev, vlanTestAPIOutputVlanId=vlanTestAPIOutputVlanId, vlanCountAPIAdmin=vlanCountAPIAdmin, sfpsVAPIFloodPolicy=sfpsVAPIFloodPolicy, vlanSystemPollCount=vlanSystemPollCount, vlanNameRelayAgent=vlanNameRelayAgent, vlanAMRRulesStatus=vlanAMRRulesStatus, vlanNameNumUsers=vlanNameNumUsers, vlanTestAPIPort=vlanTestAPIPort, vlanPortLayer3Learning=vlanPortLayer3Learning, sfpsVAPINvramId=sfpsVAPINvramId)
