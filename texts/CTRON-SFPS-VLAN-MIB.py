...
vlanTestAPI, vlanAPI, vlanAMRStats, vlanCountAPI, vlanPort, vlanAMRRules, vlanAMRSubnets, vlanName, vlanSystem = mibBuilder.importSymbols("CTRON-SFPS-INCLUDE-MIB", "vlanTestAPI", "vlanAPI", "vlanAMRStats", "vlanCountAPI", "vlanPort", "vlanAMRRules", "vlanAMRSubnets", "vlanName", "vlanSystem")
...
mibBuilder.exportSymbols("CTRON-SFPS-VLAN-MIB", vlanAMRStatsNumRulesEnabled=vlanAMRStatsNumRulesEnabled, vlanPortPortStatus=vlanPortPortStatus, vlanNameIndex=vlanNameIndex, vlanCountAPITotal=vlanCountAPITotal, vlanNameFloodPorts=vlanNameFloodPorts, vlanPortFloodPolicy=vlanPortFloodPolicy, vlanTestAPIVerb=vlanTestAPIVerb, vlanTestAPIOutputTap=vlanTestAPIOutputTap, vlanTestAPIOutputAble=vlanTestAPIOutputAble, vlanCountAPIAdmin=vlanCountAPIAdmin, vlanTestAPIVlanId=vlanTestAPIVlanId, sfpsVAPIAutoRegisterRule=sfpsVAPIAutoRegisterRule, vlanTestAPIVlanName=vlanTestAPIVlanName, vlanAMRStatsSingleMask=vlanAMRStatsSingleMask, vlanTestAPIPort=vlanTestAPIPort, vlanSystemVersion=vlanSystemVersion, vlanSystemDeltaPollTime=vlanSystemDeltaPollTime, vlanTestAPIOutputEntry=vlanTestAPIOutputEntry, vlanAMRRulesTable=vlanAMRRulesTable, vlanTestAPIOutputStatus=vlanTestAPIOutputStatus, vlanAMRRulesStatus=vlanAMRRulesStatus, sfpsVAPIVlanName=sfpsVAPIVlanName, vlanNameVlanId=vlanNameVlanId, sfpsVAPIAutoRegMask=sfpsVAPIAutoRegMask, sfpsVAPIUserAliasTag=sfpsVAPIUserAliasTag, vlanNameUniPolicy=vlanNameUniPolicy, vlanSystemOperTime=vlanSystemOperTime, vlanSystemAgentIP=vlanSystemAgentIP, sfpsVAPIUnicastPolicy=sfpsVAPIUnicastPolicy, vlanNameNHash=vlanNameNHash, vlanPortUniPolicy=vlanPortUniPolicy, vlanSystemPriorPollTime=vlanSystemPriorPollTime, vlanSystemLastChange=vlanSystemLastChange, vlanNameVlanRule=vlanNameVlanRule, vlanTestAPIOutputPolicy=vlanTestAPIOutputPolicy, vlanAMRRulesEntry=vlanAMRRulesEntry, vlanAMRSubnetsMask=vlanAMRSubnetsMask, sfpsVAPIAutoRegValue=sfpsVAPIAutoRegValue, vlanSystemTable=vlanSystemTable, vlanAMRRulesRule=vlanAMRRulesRule, SfpsAddress=SfpsAddress, vlanNameTable=vlanNameTable, sfpsVAPIFloodPolicy=sfpsVAPIFloodPolicy, vlanNameRelayAgent=vlanNameRelayAgent, vlanTestAPIOutputVlanName=vlanTestAPIOutputVlanName, vlanAMRSubnetsPrefix=vlanAMRSubnetsPrefix, vlanSystemDomainName=vlanSystemDomainName, vlanSystemSwitchInstance=vlanSystemSwitchInstance, vlanPortPortNum=vlanPortPortNum, vlanPortPortPolicy=vlanPortPortPolicy, vlanPortVlanId=vlanPortVlanId, vlanNameEnabledPorts=vlanNameEnabledPorts, vlanSystemPollCount=vlanSystemPollCount, vlanTestAPIOutputVlanId=vlanTestAPIOutputVlanId, sfpsVAPIVlanId=sfpsVAPIVlanId, sfpsVAPIRouterPort=sfpsVAPIRouterPort, vlanSystemAdminStatus=vlanSystemAdminStatus, vlanPortTable=vlanPortTable, vlanSystemLastPollTime=vlanSystemLastPollTime, vlanTestAPIOutputTable=vlanTestAPIOutputTable, vlanSystemAdminReset=vlanSystemAdminReset, vlanPortRouterPort=vlanPortRouterPort, vlanSystemOperStatus=vlanSystemOperStatus, sfpsVAPIUserAlias=sfpsVAPIUserAlias, vlanNameVlanName=vlanNameVlanName, vlanPortLayer3Learning=vlanPortLayer3Learning, vlanPortRelayAgent=vlanPortRelayAgent, vlanTestAPIOutputMap=vlanTestAPIOutputMap, vlanPortAdminStatus=vlanPortAdminStatus, vlanTestAPIOutputIndex=vlanTestAPIOutputIndex, SfpsSwitchPort=SfpsSwitchPort, VlanSwitchInstance=VlanSwitchInstance, sfpsVAPIVerb=sfpsVAPIVerb, sfpsVAPIAdminStatus=sfpsVAPIAdminStatus, vlanNameEntry=vlanNameEntry, vlanPortVlanName=vlanPortVlanName, vlanSystemEntry=vlanSystemEntry, sfpsVAPIPortPolicy=sfpsVAPIPortPolicy, sfpsVAPIOutPort=sfpsVAPIOutPort, vlanNameAdminStatus=vlanNameAdminStatus, sfpsVAPILayer3Learning=sfpsVAPILayer3Learning, HexInteger=HexInteger, sfpsVAPINvramId=sfpsVAPINvramId, vlanNameMappedPorts=vlanNameMappedPorts, sfpsVAPIUserMAC=sfpsVAPIUserMAC, vlanNameOperStatus=vlanNameOperStatus, vlanNameFloodPolicy=vlanNameFloodPolicy, vlanSystemFirstPollTime=vlanSystemFirstPollTime, sfpsVAPIRelayAgent=sfpsVAPIRelayAgent, vlanCountAPIAutoreg=vlanCountAPIAutoreg, vlanSystemMibRev=vlanSystemMibRev, sfpsVAPIInPort=sfpsVAPIInPort, vlanTestAPIOutputUserCnt=vlanTestAPIOutputUserCnt, vlanNameStatusTime=vlanNameStatusTime, vlanPortEntry=vlanPortEntry, vlanNameNumUsers=vlanNameNumUsers, vlanTestAPIOutputPort=vlanTestAPIOutputPort)
