...
vlanTestAPI, vlanCountAPI, vlanAMRSubnets, vlanAPI, vlanName, vlanSystem, vlanAMRRules, vlanAMRStats, vlanPort = mibBuilder.importSymbols("CTRON-SFPS-INCLUDE-MIB", "vlanTestAPI", "vlanCountAPI", "vlanAMRSubnets", "vlanAPI", "vlanName", "vlanSystem", "vlanAMRRules", "vlanAMRStats", "vlanPort")
...
mibBuilder.exportSymbols("CTRON-SFPS-VLAN-MIB", vlanPortPortStatus=vlanPortPortStatus, vlanNameTable=vlanNameTable, vlanSystemLastPollTime=vlanSystemLastPollTime, vlanTestAPIOutputMap=vlanTestAPIOutputMap, vlanCountAPIAutoreg=vlanCountAPIAutoreg, vlanPortAdminStatus=vlanPortAdminStatus, sfpsVAPIUserAliasTag=sfpsVAPIUserAliasTag, sfpsVAPILayer3Learning=sfpsVAPILayer3Learning, vlanNameEnabledPorts=vlanNameEnabledPorts, vlanSystemEntry=vlanSystemEntry, vlanTestAPIOutputVlanName=vlanTestAPIOutputVlanName, vlanTestAPIOutputUserCnt=vlanTestAPIOutputUserCnt, vlanAMRStatsSingleMask=vlanAMRStatsSingleMask, vlanAMRSubnetsPrefix=vlanAMRSubnetsPrefix, vlanNameNHash=vlanNameNHash, SfpsSwitchPort=SfpsSwitchPort, vlanPortTable=vlanPortTable, vlanPortPortNum=vlanPortPortNum, vlanSystemPriorPollTime=vlanSystemPriorPollTime, vlanNameFloodPolicy=vlanNameFloodPolicy, sfpsVAPIVlanId=sfpsVAPIVlanId, HexInteger=HexInteger, vlanSystemMibRev=vlanSystemMibRev, vlanTestAPIVlanId=vlanTestAPIVlanId, vlanNameRelayAgent=vlanNameRelayAgent, vlanAMRRulesRule=vlanAMRRulesRule, vlanTestAPIOutputPort=vlanTestAPIOutputPort, vlanAMRSubnetsMask=vlanAMRSubnetsMask, vlanSystemTable=vlanSystemTable, vlanSystemOperStatus=vlanSystemOperStatus, vlanTestAPIVlanName=vlanTestAPIVlanName, sfpsVAPIRelayAgent=sfpsVAPIRelayAgent, vlanPortRelayAgent=vlanPortRelayAgent, vlanNameIndex=vlanNameIndex, vlanPortEntry=vlanPortEntry, sfpsVAPIVerb=sfpsVAPIVerb, vlanPortUniPolicy=vlanPortUniPolicy, vlanTestAPIOutputStatus=vlanTestAPIOutputStatus, vlanPortVlanName=vlanPortVlanName, vlanTestAPIOutputTap=vlanTestAPIOutputTap, vlanCountAPITotal=vlanCountAPITotal, vlanNameAdminStatus=vlanNameAdminStatus, sfpsVAPIFloodPolicy=sfpsVAPIFloodPolicy, vlanTestAPIOutputPolicy=vlanTestAPIOutputPolicy, sfpsVAPIUserAlias=sfpsVAPIUserAlias, vlanTestAPIOutputAble=vlanTestAPIOutputAble, vlanSystemAgentIP=vlanSystemAgentIP, sfpsVAPIUnicastPolicy=sfpsVAPIUnicastPolicy, vlanNameVlanName=vlanNameVlanName, vlanSystemDomainName=vlanSystemDomainName, vlanSystemOperTime=vlanSystemOperTime, vlanAMRRulesEntry=vlanAMRRulesEntry, vlanNameMappedPorts=vlanNameMappedPorts, sfpsVAPIAutoRegisterRule=sfpsVAPIAutoRegisterRule, vlanNameVlanId=vlanNameVlanId, vlanNameUniPolicy=vlanNameUniPolicy, sfpsVAPIRouterPort=sfpsVAPIRouterPort, vlanSystemVersion=vlanSystemVersion, vlanPortRouterPort=vlanPortRouterPort, vlanTestAPIPort=vlanTestAPIPort, vlanSystemPollCount=vlanSystemPollCount, vlanAMRRulesTable=vlanAMRRulesTable, vlanPortPortPolicy=vlanPortPortPolicy, sfpsVAPIAdminStatus=sfpsVAPIAdminStatus, sfpsVAPIAutoRegMask=sfpsVAPIAutoRegMask, vlanCountAPIAdmin=vlanCountAPIAdmin, vlanNameEntry=vlanNameEntry, VlanSwitchInstance=VlanSwitchInstance, vlanSystemDeltaPollTime=vlanSystemDeltaPollTime, vlanNameStatusTime=vlanNameStatusTime, sfpsVAPIVlanName=sfpsVAPIVlanName, vlanTestAPIOutputIndex=vlanTestAPIOutputIndex, vlanSystemLastChange=vlanSystemLastChange, vlanTestAPIOutputVlanId=vlanTestAPIOutputVlanId, vlanSystemSwitchInstance=vlanSystemSwitchInstance, vlanSystemAdminReset=vlanSystemAdminReset, vlanAMRStatsNumRulesEnabled=vlanAMRStatsNumRulesEnabled, SfpsAddress=SfpsAddress, sfpsVAPINvramId=sfpsVAPINvramId, sfpsVAPIOutPort=sfpsVAPIOutPort, vlanSystemFirstPollTime=vlanSystemFirstPollTime, sfpsVAPIAutoRegValue=sfpsVAPIAutoRegValue, vlanTestAPIVerb=vlanTestAPIVerb, sfpsVAPIUserMAC=sfpsVAPIUserMAC, vlanNameOperStatus=vlanNameOperStatus, vlanTestAPIOutputEntry=vlanTestAPIOutputEntry, vlanSystemAdminStatus=vlanSystemAdminStatus, sfpsVAPIPortPolicy=sfpsVAPIPortPolicy, sfpsVAPIInPort=sfpsVAPIInPort, vlanNameFloodPorts=vlanNameFloodPorts, vlanAMRRulesStatus=vlanAMRRulesStatus, vlanPortVlanId=vlanPortVlanId, vlanNameVlanRule=vlanNameVlanRule, vlanPortLayer3Learning=vlanPortLayer3Learning, vlanNameNumUsers=vlanNameNumUsers, vlanTestAPIOutputTable=vlanTestAPIOutputTable, vlanPortFloodPolicy=vlanPortFloodPolicy)
