...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", aggregateDataRate=aggregateDataRate, smtpGroup=smtpGroup, ethernet=ethernet, ptmp=ptmp, installArmState=installArmState, systemClock=systemClock, ntpGroup=ntpGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, ptp=ptp, configuration=configuration, transmitModulationMode=transmitModulationMode, noWirelessChannelAvailable=noWirelessChannelAvailable, licenceGroup=licenceGroup, radarDetectChannel=radarDetectChannel, vlanPriority=vlanPriority, extendedChannel=extendedChannel, maximumTransmitPower=maximumTransmitPower, licence=licence, siteName=siteName, sNMPEnabledTraps=sNMPEnabledTraps, transmitPower=transmitPower, currentChannel=currentChannel, productVariant=productVariant, phyStatus=phyStatus, timeZone=timeZone, snmpControlGroup=snmpControlGroup, sNMPCommunityTableNumber=sNMPCommunityTableNumber, extendedFreqMHz=extendedFreqMHz, sNMPCommunityAccess=sNMPCommunityAccess, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, channelSelection=channelSelection, cableLoss=cableLoss, remoteIPAddress=remoteIPAddress, currentFreqMHz=currentFreqMHz, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, bootVersion=bootVersion, sNMPTrapVersion=sNMPTrapVersion, linkName=linkName, dataPortStatusTrap=dataPortStatusTrap, channelBandwidth=channelBandwidth, sNMPCommunityString=sNMPCommunityString, vlanId=vlanId, ethernetGroup=ethernetGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, nTPSync=nTPSync, alarmsGroup=alarmsGroup, fixedModMode=fixedModMode, configurationGroup=configurationGroup, sNMPCommunityOid=sNMPCommunityOid, sNMPTrapTableIndex=sNMPTrapTableIndex, dataPortAutoNegotiation=dataPortAutoNegotiation, nTPState=nTPState, dataPortStatus=dataPortStatus, productName=productName, pubStats=pubStats, sNMPCommunityTableEntry=sNMPCommunityTableEntry, sMTPEnabledMessages=sMTPEnabledMessages, sMTPServerIPAddress=sMTPServerIPAddress, alarms=alarms, versions=versions, sNMPTrapIPAddress=sNMPTrapIPAddress, configuredRange=configuredRange, ptpTraps=ptpTraps, ptpCompliance=ptpCompliance, managementGroup=managementGroup, management=management, pubStatsGroup=pubStatsGroup, sMTPSourceEmailAddress=sMTPSourceEmailAddress, searchState=searchState, remoteMACAddress=remoteMACAddress, signalStrengthRatio=signalStrengthRatio, sNMPTrapTable=sNMPTrapTable, configuredModulationMode=configuredModulationMode, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, versionsGroup=versionsGroup, sMTPServerPortNumber=sMTPServerPortNumber, sNMPTrapCommunity=sNMPTrapCommunity, sNMPTrapTableNumber=sNMPTrapTableNumber, dualPayload=dualPayload, phyStatusGroup=phyStatusGroup, ptp250=ptp250, measuredRange=measuredRange, sNMPCommunityTableIndex=sNMPCommunityTableIndex, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, linkStatusTrap=linkStatusTrap, iPAddress=iPAddress, smtp=smtp, noiseFloor=noiseFloor, tFTPServerPortNumber=tFTPServerPortNumber, sNMPTrapTableEntry=sNMPTrapTableEntry, masterSlaveMode=masterSlaveMode, softwareVersion=softwareVersion, subnetMask=subnetMask, PYSNMP_MODULE_ID=cambium, nTPServerIp=nTPServerIp, cambium=cambium, regionCode=regionCode, installArmStateTrap=installArmStateTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, nTPServerPortNumber=nTPServerPortNumber, band=band, antennaGain=antennaGain, wirelessLinkStatus=wirelessLinkStatus, vlanTagging=vlanTagging, snmpControl=snmpControl, transmitDataRate=transmitDataRate, ntp=ntp, sNMPCommunityTable=sNMPCommunityTable, gatewayIPAddress=gatewayIPAddress, receiveModulationMode=receiveModulationMode, vectorError=vectorError, receiveDataRate=receiveDataRate, ptpTrapPrefix=ptpTrapPrefix, nTPPollInterval=nTPPollInterval, tFTPServerIPAddress=tFTPServerIPAddress, receivePower=receivePower, notificationsGroup=notificationsGroup, hardwareVersion=hardwareVersion, linkLoss=linkLoss, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, nTPLastSync=nTPLastSync, radarDetectTrap=radarDetectTrap, ptpGroups=ptpGroups)
