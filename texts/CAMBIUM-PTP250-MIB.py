...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", nTPServerIp=nTPServerIp, sNMPTrapCommunity=sNMPTrapCommunity, managementGroup=managementGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, cableLoss=cableLoss, sNMPTrapTableNumber=sNMPTrapTableNumber, transmitDataRate=transmitDataRate, ntpGroup=ntpGroup, maximumTransmitPower=maximumTransmitPower, pubStatsGroup=pubStatsGroup, nTPPollInterval=nTPPollInterval, softwareVersion=softwareVersion, masterSlaveMode=masterSlaveMode, productName=productName, channelSelection=channelSelection, linkLoss=linkLoss, tFTPServerIPAddress=tFTPServerIPAddress, sNMPCommunityAccess=sNMPCommunityAccess, remoteMACAddress=remoteMACAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, sMTPSourceEmailAddress=sMTPSourceEmailAddress, noiseFloor=noiseFloor, gatewayIPAddress=gatewayIPAddress, dataPortStatus=dataPortStatus, extendedFreqMHz=extendedFreqMHz, sNMPTrapIPAddress=sNMPTrapIPAddress, dualPayload=dualPayload, configuredModulationMode=configuredModulationMode, licence=licence, timeZone=timeZone, ethernetGroup=ethernetGroup, alarmsGroup=alarmsGroup, cambium=cambium, smtp=smtp, subnetMask=subnetMask, aggregateDataRate=aggregateDataRate, measuredRange=measuredRange, fixedModMode=fixedModMode, radarDetectChannel=radarDetectChannel, sNMPTrapTableEntry=sNMPTrapTableEntry, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, alarms=alarms, vlanPriority=vlanPriority, sNMPCommunityTable=sNMPCommunityTable, ptp=ptp, sNMPCommunityString=sNMPCommunityString, ethernet=ethernet, receiveModulationMode=receiveModulationMode, pubStats=pubStats, nTPServerPortNumber=nTPServerPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNMPTrapTable=sNMPTrapTable, configuration=configuration, notificationsGroup=notificationsGroup, systemClock=systemClock, siteName=siteName, signalStrengthRatio=signalStrengthRatio, vlanId=vlanId, phyStatus=phyStatus, sNMPCommunityTableNumber=sNMPCommunityTableNumber, remoteIPAddress=remoteIPAddress, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, ptpTrapPrefix=ptpTrapPrefix, configuredRange=configuredRange, searchState=searchState, nTPState=nTPState, sNMPEnabledTraps=sNMPEnabledTraps, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, management=management, smtpGroup=smtpGroup, installArmStateTrap=installArmStateTrap, currentFreqMHz=currentFreqMHz, transmitModulationMode=transmitModulationMode, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNMPCommunityTableIndex=sNMPCommunityTableIndex, nTPLastSync=nTPLastSync, channelBandwidth=channelBandwidth, phyStatusGroup=phyStatusGroup, vectorError=vectorError, receivePower=receivePower, noWirelessChannelAvailable=noWirelessChannelAvailable, ptpGroups=ptpGroups, ptmp=ptmp, sNMPTrapVersion=sNMPTrapVersion, transmitPower=transmitPower, linkStatusTrap=linkStatusTrap, configurationGroup=configurationGroup, bootVersion=bootVersion, ptpCompliance=ptpCompliance, radarDetectTrap=radarDetectTrap, band=band, extendedChannel=extendedChannel, ntp=ntp, licenceGroup=licenceGroup, linkName=linkName, dataPortStatusTrap=dataPortStatusTrap, sNMPCommunityOid=sNMPCommunityOid, ptp250=ptp250, regionCode=regionCode, ptpTraps=ptpTraps, tFTPServerPortNumber=tFTPServerPortNumber, hardwareVersion=hardwareVersion, versions=versions, sMTPServerPortNumber=sMTPServerPortNumber, currentChannel=currentChannel, versionsGroup=versionsGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, receiveDataRate=receiveDataRate, productVariant=productVariant, PYSNMP_MODULE_ID=cambium, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, snmpControl=snmpControl, iPAddress=iPAddress, vlanTagging=vlanTagging, snmpControlGroup=snmpControlGroup, sNMPCommunityTableEntry=sNMPCommunityTableEntry, antennaGain=antennaGain, wirelessLinkStatus=wirelessLinkStatus, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, nTPSync=nTPSync, installArmState=installArmState, sMTPServerIPAddress=sMTPServerIPAddress, sMTPEnabledMessages=sMTPEnabledMessages)
