...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", sNMPCommunityTableEntry=sNMPCommunityTableEntry, sNMPEnabledTraps=sNMPEnabledTraps, sNMPTrapTable=sNMPTrapTable, extendedChannel=extendedChannel, sNMPTrapTableEntry=sNMPTrapTableEntry, extendedFreqMHz=extendedFreqMHz, ptp=ptp, configuredModulationMode=configuredModulationMode, pubStats=pubStats, pubStatsGroup=pubStatsGroup, linkLoss=linkLoss, productVariant=productVariant, managementGroup=managementGroup, versionsGroup=versionsGroup, cambium=cambium, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, configuredRange=configuredRange, siteName=siteName, sNMPTrapTableIndex=sNMPTrapTableIndex, notificationsGroup=notificationsGroup, softwareVersion=softwareVersion, productName=productName, sMTPSourceEmailAddress=sMTPSourceEmailAddress, hardwareVersion=hardwareVersion, sNMPTrapPortNumber=sNMPTrapPortNumber, nTPSync=nTPSync, aggregateDataRate=aggregateDataRate, transmitPower=transmitPower, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, linkName=linkName, sMTPEnabledMessages=sMTPEnabledMessages, ptmp=ptmp, vectorError=vectorError, bootVersion=bootVersion, vlanPriority=vlanPriority, tFTPServerPortNumber=tFTPServerPortNumber, antennaGain=antennaGain, nTPServerIp=nTPServerIp, configuration=configuration, alarmsGroup=alarmsGroup, PYSNMP_MODULE_ID=cambium, ethernet=ethernet, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPTrapIPAddress=sNMPTrapIPAddress, iPAddress=iPAddress, measuredRange=measuredRange, smtpGroup=smtpGroup, nTPServerPortNumber=nTPServerPortNumber, subnetMask=subnetMask, sNMPCommunityTableNumber=sNMPCommunityTableNumber, snmpControl=snmpControl, ntp=ntp, noWirelessChannelAvailable=noWirelessChannelAvailable, snmpControlGroup=snmpControlGroup, vlanId=vlanId, sNMPTrapTableNumber=sNMPTrapTableNumber, sNMPCommunityTable=sNMPCommunityTable, phyStatusGroup=phyStatusGroup, remoteIPAddress=remoteIPAddress, channelSelection=channelSelection, sNMPTrapCommunity=sNMPTrapCommunity, nTPLastSync=nTPLastSync, noiseFloor=noiseFloor, ntpGroup=ntpGroup, licence=licence, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, installArmStateTrap=installArmStateTrap, sNMPCommunityAccess=sNMPCommunityAccess, installArmState=installArmState, sNMPTrapVersion=sNMPTrapVersion, configurationGroup=configurationGroup, ptpTrapPrefix=ptpTrapPrefix, transmitDataRate=transmitDataRate, signalStrengthRatio=signalStrengthRatio, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, dualPayload=dualPayload, smtp=smtp, masterSlaveMode=masterSlaveMode, fixedModMode=fixedModMode, currentFreqMHz=currentFreqMHz, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, wirelessLinkStatus=wirelessLinkStatus, linkStatusTrap=linkStatusTrap, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortStatus=dataPortStatus, remoteMACAddress=remoteMACAddress, sNMPCommunityTableIndex=sNMPCommunityTableIndex, ptpGroups=ptpGroups, cableLoss=cableLoss, alarms=alarms, receivePower=receivePower, maximumTransmitPower=maximumTransmitPower, searchState=searchState, management=management, sNMPCommunityString=sNMPCommunityString, receiveDataRate=receiveDataRate, channelBandwidth=channelBandwidth, nTPState=nTPState, band=band, ptpTraps=ptpTraps, regionCode=regionCode, ethernetGroup=ethernetGroup, versions=versions, dataPortStatusTrap=dataPortStatusTrap, radarDetectTrap=radarDetectTrap, tFTPServerIPAddress=tFTPServerIPAddress, ptpCompliance=ptpCompliance, transmitModulationMode=transmitModulationMode, radarDetectChannel=radarDetectChannel, gatewayIPAddress=gatewayIPAddress, sNMPCommunityOid=sNMPCommunityOid, systemClock=systemClock, vlanTagging=vlanTagging, sMTPServerIPAddress=sMTPServerIPAddress, currentChannel=currentChannel, licenceGroup=licenceGroup, phyStatus=phyStatus, sMTPServerPortNumber=sMTPServerPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, nTPPollInterval=nTPPollInterval, timeZone=timeZone, receiveModulationMode=receiveModulationMode, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, ptp250=ptp250)
