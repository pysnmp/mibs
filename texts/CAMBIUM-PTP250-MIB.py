...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", receivePower=receivePower, antennaGain=antennaGain, sNMPCommunityTableIndex=sNMPCommunityTableIndex, transmitPower=transmitPower, sMTPEnabledMessages=sMTPEnabledMessages, radarDetectChannel=radarDetectChannel, tFTPServerPortNumber=tFTPServerPortNumber, sNMPTrapVersion=sNMPTrapVersion, remoteMACAddress=remoteMACAddress, sNMPTrapTable=sNMPTrapTable, tFTPServerIPAddress=tFTPServerIPAddress, radarDetectTrap=radarDetectTrap, installArmState=installArmState, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, ptmp=ptmp, nTPServerIp=nTPServerIp, configuration=configuration, cableLoss=cableLoss, receiveModulationMode=receiveModulationMode, signalStrengthRatio=signalStrengthRatio, cambium=cambium, linkLoss=linkLoss, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPTrapIPAddress=sNMPTrapIPAddress, channelSelection=channelSelection, ptpGroups=ptpGroups, productVariant=productVariant, linkStatusTrap=linkStatusTrap, transmitModulationMode=transmitModulationMode, pubStatsGroup=pubStatsGroup, ptp=ptp, snmpControlGroup=snmpControlGroup, measuredRange=measuredRange, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sNMPTrapCommunity=sNMPTrapCommunity, licence=licence, hardwareVersion=hardwareVersion, alarmsGroup=alarmsGroup, sNMPCommunityTable=sNMPCommunityTable, wirelessLinkStatus=wirelessLinkStatus, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, aggregateDataRate=aggregateDataRate, nTPServerPortNumber=nTPServerPortNumber, nTPPollInterval=nTPPollInterval, nTPSync=nTPSync, nTPLastSync=nTPLastSync, management=management, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPTrapTableIndex=sNMPTrapTableIndex, versions=versions, softwareVersion=softwareVersion, sNMPCommunityString=sNMPCommunityString, phyStatus=phyStatus, fixedModMode=fixedModMode, searchState=searchState, configuredRange=configuredRange, receiveDataRate=receiveDataRate, sNMPCommunityOid=sNMPCommunityOid, configuredModulationMode=configuredModulationMode, sNMPTrapTableEntry=sNMPTrapTableEntry, remoteIPAddress=remoteIPAddress, sMTPSourceEmailAddress=sMTPSourceEmailAddress, phyStatusGroup=phyStatusGroup, masterSlaveMode=masterSlaveMode, sNMPEnabledTraps=sNMPEnabledTraps, dualPayload=dualPayload, dataPortAutoNegotiation=dataPortAutoNegotiation, iPAddress=iPAddress, PYSNMP_MODULE_ID=cambium, vlanTagging=vlanTagging, alarms=alarms, versionsGroup=versionsGroup, systemClock=systemClock, ntpGroup=ntpGroup, maximumTransmitPower=maximumTransmitPower, siteName=siteName, sMTPServerPortNumber=sMTPServerPortNumber, smtpGroup=smtpGroup, regionCode=regionCode, currentChannel=currentChannel, notificationsGroup=notificationsGroup, bootVersion=bootVersion, ptpTraps=ptpTraps, extendedChannel=extendedChannel, sNMPTrapPortNumber=sNMPTrapPortNumber, sNMPCommunityTableNumber=sNMPCommunityTableNumber, vlanPriority=vlanPriority, sMTPServerIPAddress=sMTPServerIPAddress, licenceGroup=licenceGroup, sNMPTrapTableNumber=sNMPTrapTableNumber, currentFreqMHz=currentFreqMHz, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, subnetMask=subnetMask, smtp=smtp, ptp250=ptp250, pubStats=pubStats, timeZone=timeZone, vectorError=vectorError, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, productName=productName, sNMPCommunityAccess=sNMPCommunityAccess, ptpTrapPrefix=ptpTrapPrefix, band=band, transmitDataRate=transmitDataRate, extendedFreqMHz=extendedFreqMHz, vlanId=vlanId, gatewayIPAddress=gatewayIPAddress, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, ntp=ntp, installArmStateTrap=installArmStateTrap, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNMPCommunityTableEntry=sNMPCommunityTableEntry, ethernetGroup=ethernetGroup, ptpCompliance=ptpCompliance, managementGroup=managementGroup, channelBandwidth=channelBandwidth, snmpControl=snmpControl, linkName=linkName, ethernet=ethernet, dataPortStatus=dataPortStatus, dataPortStatusTrap=dataPortStatusTrap, configurationGroup=configurationGroup, nTPState=nTPState, noiseFloor=noiseFloor)
