...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", sNMPTrapPortNumber=sNMPTrapPortNumber, configuredModulationMode=configuredModulationMode, hardwareVersion=hardwareVersion, remoteIPAddress=remoteIPAddress, productName=productName, ptmp=ptmp, PYSNMP_MODULE_ID=cambium, sNMPTrapTableIndex=sNMPTrapTableIndex, aggregateDataRate=aggregateDataRate, receiveDataRate=receiveDataRate, linkLoss=linkLoss, smtp=smtp, extendedChannel=extendedChannel, nTPPollInterval=nTPPollInterval, sNMPCommunityString=sNMPCommunityString, vectorError=vectorError, extendedFreqMHz=extendedFreqMHz, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, nTPState=nTPState, dataPortAutoNegotiation=dataPortAutoNegotiation, ptp=ptp, nTPServerIp=nTPServerIp, wirelessLinkStatus=wirelessLinkStatus, dataPortStatusTrap=dataPortStatusTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, sNMPCommunityTableIndex=sNMPCommunityTableIndex, dualPayload=dualPayload, nTPLastSync=nTPLastSync, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, nTPSync=nTPSync, nTPServerPortNumber=nTPServerPortNumber, gatewayIPAddress=gatewayIPAddress, ptpCompliance=ptpCompliance, signalStrengthRatio=signalStrengthRatio, channelSelection=channelSelection, currentFreqMHz=currentFreqMHz, searchState=searchState, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNMPTrapCommunity=sNMPTrapCommunity, pubStatsGroup=pubStatsGroup, timeZone=timeZone, notificationsGroup=notificationsGroup, transmitPower=transmitPower, noWirelessChannelAvailable=noWirelessChannelAvailable, receivePower=receivePower, softwareVersion=softwareVersion, radarDetectTrap=radarDetectTrap, ntp=ntp, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, antennaGain=antennaGain, vlanId=vlanId, configurationGroup=configurationGroup, band=band, ethernet=ethernet, sNMPCommunityTable=sNMPCommunityTable, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, ptpGroups=ptpGroups, transmitDataRate=transmitDataRate, snmpControlGroup=snmpControlGroup, noiseFloor=noiseFloor, versionsGroup=versionsGroup, sNMPCommunityAccess=sNMPCommunityAccess, sNMPEnabledTraps=sNMPEnabledTraps, bootVersion=bootVersion, subnetMask=subnetMask, configuredRange=configuredRange, transmitModulationMode=transmitModulationMode, systemClock=systemClock, linkName=linkName, configuration=configuration, tFTPServerPortNumber=tFTPServerPortNumber, receiveModulationMode=receiveModulationMode, alarmsGroup=alarmsGroup, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, alarms=alarms, versions=versions, sNMPTrapTableEntry=sNMPTrapTableEntry, snmpControl=snmpControl, sNMPTrapIPAddress=sNMPTrapIPAddress, ptpTraps=ptpTraps, pubStats=pubStats, sNMPTrapTable=sNMPTrapTable, installArmState=installArmState, regionCode=regionCode, tFTPServerIPAddress=tFTPServerIPAddress, cableLoss=cableLoss, sNMPCommunityTableNumber=sNMPCommunityTableNumber, phyStatusGroup=phyStatusGroup, sNMPCommunityTableEntry=sNMPCommunityTableEntry, masterSlaveMode=masterSlaveMode, ptpTrapPrefix=ptpTrapPrefix, vlanPriority=vlanPriority, ptp250=ptp250, remoteMACAddress=remoteMACAddress, phyStatus=phyStatus, sNMPTrapTableNumber=sNMPTrapTableNumber, currentChannel=currentChannel, sNMPTrapVersion=sNMPTrapVersion, managementGroup=managementGroup, installArmStateTrap=installArmStateTrap, siteName=siteName, measuredRange=measuredRange, ethernetGroup=ethernetGroup, ntpGroup=ntpGroup, smtpGroup=smtpGroup, vlanTagging=vlanTagging, sMTPServerPortNumber=sMTPServerPortNumber, sNMPCommunityOid=sNMPCommunityOid, management=management, productVariant=productVariant, iPAddress=iPAddress, maximumTransmitPower=maximumTransmitPower, dataPortStatus=dataPortStatus, sMTPEnabledMessages=sMTPEnabledMessages, cambium=cambium, licenceGroup=licenceGroup, fixedModMode=fixedModMode, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, channelBandwidth=channelBandwidth, linkStatusTrap=linkStatusTrap, licence=licence, radarDetectChannel=radarDetectChannel, sMTPServerIPAddress=sMTPServerIPAddress)
