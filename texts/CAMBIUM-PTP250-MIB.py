...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", versionsGroup=versionsGroup, managementGroup=managementGroup, transmitPower=transmitPower, configuration=configuration, nTPPollInterval=nTPPollInterval, masterSlaveMode=masterSlaveMode, sMTPServerPortNumber=sMTPServerPortNumber, management=management, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, radarDetectChannel=radarDetectChannel, vectorError=vectorError, ptp250=ptp250, pubStats=pubStats, ptpGroups=ptpGroups, sNMPTrapTableEntry=sNMPTrapTableEntry, noiseFloor=noiseFloor, sMTPEnabledMessages=sMTPEnabledMessages, remoteMACAddress=remoteMACAddress, nTPSync=nTPSync, systemClock=systemClock, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, remoteIPAddress=remoteIPAddress, linkName=linkName, tFTPServerPortNumber=tFTPServerPortNumber, installArmState=installArmState, sMTPSourceEmailAddress=sMTPSourceEmailAddress, licence=licence, receiveModulationMode=receiveModulationMode, signalStrengthRatio=signalStrengthRatio, sNMPCommunityTableNumber=sNMPCommunityTableNumber, configuredRange=configuredRange, smtpGroup=smtpGroup, subnetMask=subnetMask, ethernetGroup=ethernetGroup, sNMPCommunityString=sNMPCommunityString, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNMPEnabledTraps=sNMPEnabledTraps, transmitDataRate=transmitDataRate, iPAddress=iPAddress, searchState=searchState, noWirelessChannelAvailable=noWirelessChannelAvailable, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, versions=versions, configurationGroup=configurationGroup, sNMPCommunityTableEntry=sNMPCommunityTableEntry, vlanTagging=vlanTagging, sNMPTrapTable=sNMPTrapTable, fixedModMode=fixedModMode, regionCode=regionCode, measuredRange=measuredRange, linkStatusTrap=linkStatusTrap, softwareVersion=softwareVersion, currentFreqMHz=currentFreqMHz, dataPortStatus=dataPortStatus, receiveDataRate=receiveDataRate, smtp=smtp, ntpGroup=ntpGroup, cambium=cambium, bootVersion=bootVersion, ethernet=ethernet, siteName=siteName, installArmStateTrap=installArmStateTrap, licenceGroup=licenceGroup, maximumTransmitPower=maximumTransmitPower, alarms=alarms, antennaGain=antennaGain, sNMPTrapPortNumber=sNMPTrapPortNumber, sNMPCommunityOid=sNMPCommunityOid, extendedFreqMHz=extendedFreqMHz, nTPLastSync=nTPLastSync, dataPortAutoNegotiation=dataPortAutoNegotiation, vlanPriority=vlanPriority, sNMPTrapIPAddress=sNMPTrapIPAddress, hardwareVersion=hardwareVersion, timeZone=timeZone, sNMPTrapCommunity=sNMPTrapCommunity, dataPortStatusTrap=dataPortStatusTrap, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, phyStatus=phyStatus, productVariant=productVariant, sNMPCommunityTable=sNMPCommunityTable, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, radarDetectTrap=radarDetectTrap, phyStatusGroup=phyStatusGroup, snmpControlGroup=snmpControlGroup, nTPServerPortNumber=nTPServerPortNumber, sNMPTrapVersion=sNMPTrapVersion, gatewayIPAddress=gatewayIPAddress, configuredModulationMode=configuredModulationMode, sNMPCommunityTableIndex=sNMPCommunityTableIndex, ptpTraps=ptpTraps, ptpCompliance=ptpCompliance, ntp=ntp, extendedChannel=extendedChannel, sNMPTrapTableNumber=sNMPTrapTableNumber, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, wirelessLinkStatus=wirelessLinkStatus, ptp=ptp, notificationsGroup=notificationsGroup, vlanId=vlanId, sMTPServerIPAddress=sMTPServerIPAddress, pubStatsGroup=pubStatsGroup, band=band, alarmsGroup=alarmsGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, channelSelection=channelSelection, currentChannel=currentChannel, dualPayload=dualPayload, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, channelBandwidth=channelBandwidth, productName=productName, cableLoss=cableLoss, snmpControl=snmpControl, transmitModulationMode=transmitModulationMode, tFTPServerIPAddress=tFTPServerIPAddress, aggregateDataRate=aggregateDataRate, nTPState=nTPState, PYSNMP_MODULE_ID=cambium, ptmp=ptmp, sNMPCommunityAccess=sNMPCommunityAccess, receivePower=receivePower, ptpTrapPrefix=ptpTrapPrefix, linkLoss=linkLoss, nTPServerIp=nTPServerIp)
