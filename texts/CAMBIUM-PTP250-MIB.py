...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", sNMPTrapTableIndex=sNMPTrapTableIndex, ntp=ntp, dataPortAutoNegotiation=dataPortAutoNegotiation, productVariant=productVariant, remoteIPAddress=remoteIPAddress, sNMPTrapIPAddress=sNMPTrapIPAddress, versions=versions, snmpControl=snmpControl, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, transmitPower=transmitPower, sNMPCommunityTableIndex=sNMPCommunityTableIndex, configuration=configuration, masterSlaveMode=masterSlaveMode, ptp250=ptp250, subnetMask=subnetMask, channelSelection=channelSelection, transmitModulationMode=transmitModulationMode, ptpTraps=ptpTraps, configuredModulationMode=configuredModulationMode, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, configuredRange=configuredRange, sNMPTrapTableNumber=sNMPTrapTableNumber, sNMPTrapTableEntry=sNMPTrapTableEntry, radarDetectTrap=radarDetectTrap, remoteMACAddress=remoteMACAddress, iPAddress=iPAddress, management=management, pubStatsGroup=pubStatsGroup, timeZone=timeZone, linkStatusTrap=linkStatusTrap, sMTPSourceEmailAddress=sMTPSourceEmailAddress, gatewayIPAddress=gatewayIPAddress, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNMPEnabledTraps=sNMPEnabledTraps, currentFreqMHz=currentFreqMHz, sNMPCommunityOid=sNMPCommunityOid, ethernet=ethernet, sNMPCommunityString=sNMPCommunityString, softwareVersion=softwareVersion, ptp=ptp, sMTPEnabledMessages=sMTPEnabledMessages, transmitDataRate=transmitDataRate, ptpCompliance=ptpCompliance, smtpGroup=smtpGroup, ptpGroups=ptpGroups, sNMPTrapCommunity=sNMPTrapCommunity, sNMPCommunityTableEntry=sNMPCommunityTableEntry, ptpTrapPrefix=ptpTrapPrefix, noiseFloor=noiseFloor, linkName=linkName, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, vectorError=vectorError, fixedModMode=fixedModMode, currentChannel=currentChannel, managementGroup=managementGroup, measuredRange=measuredRange, linkLoss=linkLoss, notificationsGroup=notificationsGroup, phyStatusGroup=phyStatusGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, radarDetectChannel=radarDetectChannel, nTPPollInterval=nTPPollInterval, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, nTPServerIp=nTPServerIp, snmpControlGroup=snmpControlGroup, phyStatus=phyStatus, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNMPTrapTable=sNMPTrapTable, receiveModulationMode=receiveModulationMode, vlanPriority=vlanPriority, cableLoss=cableLoss, alarms=alarms, pubStats=pubStats, sNMPCommunityTableNumber=sNMPCommunityTableNumber, dataPortStatus=dataPortStatus, cambium=cambium, regionCode=regionCode, systemClock=systemClock, extendedChannel=extendedChannel, PYSNMP_MODULE_ID=cambium, smtp=smtp, noWirelessChannelAvailable=noWirelessChannelAvailable, sMTPServerIPAddress=sMTPServerIPAddress, dualPayload=dualPayload, nTPSync=nTPSync, receivePower=receivePower, searchState=searchState, configurationGroup=configurationGroup, sMTPServerPortNumber=sMTPServerPortNumber, nTPServerPortNumber=nTPServerPortNumber, dataPortStatusTrap=dataPortStatusTrap, versionsGroup=versionsGroup, ntpGroup=ntpGroup, sNMPCommunityTable=sNMPCommunityTable, maximumTransmitPower=maximumTransmitPower, nTPLastSync=nTPLastSync, nTPState=nTPState, receiveDataRate=receiveDataRate, extendedFreqMHz=extendedFreqMHz, ethernetGroup=ethernetGroup, installArmState=installArmState, sNMPTrapVersion=sNMPTrapVersion, antennaGain=antennaGain, aggregateDataRate=aggregateDataRate, bootVersion=bootVersion, band=band, installArmStateTrap=installArmStateTrap, ptmp=ptmp, tFTPServerPortNumber=tFTPServerPortNumber, alarmsGroup=alarmsGroup, vlanTagging=vlanTagging, hardwareVersion=hardwareVersion, licence=licence, productName=productName, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, channelBandwidth=channelBandwidth, siteName=siteName, sNMPCommunityAccess=sNMPCommunityAccess, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, vlanId=vlanId, signalStrengthRatio=signalStrengthRatio, tFTPServerIPAddress=tFTPServerIPAddress, wirelessLinkStatus=wirelessLinkStatus, licenceGroup=licenceGroup)
