...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", sNMPCommunityOid=sNMPCommunityOid, pubStats=pubStats, ptmp=ptmp, configuredRange=configuredRange, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, regionCode=regionCode, sNMPTrapTableNumber=sNMPTrapTableNumber, transmitDataRate=transmitDataRate, nTPServerPortNumber=nTPServerPortNumber, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, currentChannel=currentChannel, ntp=ntp, ptpCompliance=ptpCompliance, sNMPTrapTable=sNMPTrapTable, dualPayload=dualPayload, remoteIPAddress=remoteIPAddress, phyStatusGroup=phyStatusGroup, sMTPServerIPAddress=sMTPServerIPAddress, nTPServerIp=nTPServerIp, productVariant=productVariant, licence=licence, sMTPEnabledMessages=sMTPEnabledMessages, sNMPCommunityAccess=sNMPCommunityAccess, ethernetGroup=ethernetGroup, antennaGain=antennaGain, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, managementGroup=managementGroup, phyStatus=phyStatus, ptp250=ptp250, PYSNMP_MODULE_ID=cambium, nTPSync=nTPSync, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sMTPSourceEmailAddress=sMTPSourceEmailAddress, snmpControlGroup=snmpControlGroup, installArmState=installArmState, noWirelessChannelAvailable=noWirelessChannelAvailable, hardwareVersion=hardwareVersion, dataPortStatusTrap=dataPortStatusTrap, snmpControl=snmpControl, configuration=configuration, configuredModulationMode=configuredModulationMode, cableLoss=cableLoss, receiveModulationMode=receiveModulationMode, installArmStateTrap=installArmStateTrap, linkName=linkName, channelSelection=channelSelection, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, radarDetectChannel=radarDetectChannel, iPAddress=iPAddress, fixedModMode=fixedModMode, dataPortStatus=dataPortStatus, sNMPCommunityTableIndex=sNMPCommunityTableIndex, signalStrengthRatio=signalStrengthRatio, transmitPower=transmitPower, linkLoss=linkLoss, sMTPServerPortNumber=sMTPServerPortNumber, sNMPTrapIPAddress=sNMPTrapIPAddress, softwareVersion=softwareVersion, searchState=searchState, tFTPServerIPAddress=tFTPServerIPAddress, versions=versions, timeZone=timeZone, ntpGroup=ntpGroup, band=band, wirelessLinkStatus=wirelessLinkStatus, sNMPCommunityTable=sNMPCommunityTable, vlanId=vlanId, smtp=smtp, vlanTagging=vlanTagging, sNMPCommunityString=sNMPCommunityString, channelBandwidth=channelBandwidth, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, currentFreqMHz=currentFreqMHz, productName=productName, receiveDataRate=receiveDataRate, nTPPollInterval=nTPPollInterval, extendedFreqMHz=extendedFreqMHz, nTPLastSync=nTPLastSync, ethernet=ethernet, systemClock=systemClock, transmitModulationMode=transmitModulationMode, siteName=siteName, configurationGroup=configurationGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, management=management, nTPState=nTPState, vlanPriority=vlanPriority, noiseFloor=noiseFloor, vectorError=vectorError, sNMPEnabledTraps=sNMPEnabledTraps, aggregateDataRate=aggregateDataRate, sNMPTrapVersion=sNMPTrapVersion, licenceGroup=licenceGroup, pubStatsGroup=pubStatsGroup, ptpGroups=ptpGroups, radarDetectTrap=radarDetectTrap, sNMPCommunityTableEntry=sNMPCommunityTableEntry, versionsGroup=versionsGroup, tFTPServerPortNumber=tFTPServerPortNumber, linkStatusTrap=linkStatusTrap, alarmsGroup=alarmsGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, masterSlaveMode=masterSlaveMode, measuredRange=measuredRange, notificationsGroup=notificationsGroup, gatewayIPAddress=gatewayIPAddress, sNMPCommunityTableNumber=sNMPCommunityTableNumber, ptpTraps=ptpTraps, remoteMACAddress=remoteMACAddress, sNMPTrapCommunity=sNMPTrapCommunity, ptpTrapPrefix=ptpTrapPrefix, cambium=cambium, smtpGroup=smtpGroup, extendedChannel=extendedChannel, alarms=alarms, ptp=ptp, receivePower=receivePower, bootVersion=bootVersion, subnetMask=subnetMask, maximumTransmitPower=maximumTransmitPower, sNMPTrapTableEntry=sNMPTrapTableEntry, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName)
