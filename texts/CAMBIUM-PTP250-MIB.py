...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", licence=licence, receivePower=receivePower, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sMTPSourceEmailAddress=sMTPSourceEmailAddress, band=band, configuredRange=configuredRange, phyStatusGroup=phyStatusGroup, sNMPTrapCommunity=sNMPTrapCommunity, ptpTrapPrefix=ptpTrapPrefix, nTPServerIp=nTPServerIp, productName=productName, nTPPollInterval=nTPPollInterval, configuredModulationMode=configuredModulationMode, licenceGroup=licenceGroup, channelBandwidth=channelBandwidth, sNMPCommunityAccess=sNMPCommunityAccess, dataPortAutoNegotiation=dataPortAutoNegotiation, fixedModMode=fixedModMode, sNMPTrapTableNumber=sNMPTrapTableNumber, sNMPTrapTableIndex=sNMPTrapTableIndex, productVariant=productVariant, configurationGroup=configurationGroup, ptpCompliance=ptpCompliance, ethernet=ethernet, transmitModulationMode=transmitModulationMode, extendedFreqMHz=extendedFreqMHz, dataPortStatusTrap=dataPortStatusTrap, sNMPTrapTable=sNMPTrapTable, sNMPCommunityOid=sNMPCommunityOid, noWirelessChannelAvailable=noWirelessChannelAvailable, radarDetectTrap=radarDetectTrap, versionsGroup=versionsGroup, sNMPEnabledTraps=sNMPEnabledTraps, sNMPCommunityTable=sNMPCommunityTable, alarms=alarms, regionCode=regionCode, ntp=ntp, tFTPServerPortNumber=tFTPServerPortNumber, phyStatus=phyStatus, timeZone=timeZone, cableLoss=cableLoss, channelSelection=channelSelection, cambium=cambium, management=management, remoteMACAddress=remoteMACAddress, siteName=siteName, vlanPriority=vlanPriority, radarDetectChannel=radarDetectChannel, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, subnetMask=subnetMask, maximumTransmitPower=maximumTransmitPower, PYSNMP_MODULE_ID=cambium, linkLoss=linkLoss, wirelessLinkStatus=wirelessLinkStatus, smtp=smtp, sNMPCommunityTableEntry=sNMPCommunityTableEntry, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, snmpControl=snmpControl, sMTPEnabledMessages=sMTPEnabledMessages, receiveModulationMode=receiveModulationMode, sNMPTrapVersion=sNMPTrapVersion, ptp=ptp, ptpTraps=ptpTraps, measuredRange=measuredRange, versions=versions, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, alarmsGroup=alarmsGroup, noiseFloor=noiseFloor, vlanId=vlanId, receiveDataRate=receiveDataRate, dataPortStatus=dataPortStatus, pubStats=pubStats, sMTPServerPortNumber=sMTPServerPortNumber, notificationsGroup=notificationsGroup, nTPLastSync=nTPLastSync, ptmp=ptmp, sNMPCommunityTableNumber=sNMPCommunityTableNumber, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, nTPServerPortNumber=nTPServerPortNumber, sNMPTrapPortNumber=sNMPTrapPortNumber, configuration=configuration, antennaGain=antennaGain, transmitDataRate=transmitDataRate, vectorError=vectorError, linkName=linkName, managementGroup=managementGroup, searchState=searchState, masterSlaveMode=masterSlaveMode, remoteIPAddress=remoteIPAddress, installArmStateTrap=installArmStateTrap, ntpGroup=ntpGroup, softwareVersion=softwareVersion, linkStatusTrap=linkStatusTrap, bootVersion=bootVersion, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPCommunityTableIndex=sNMPCommunityTableIndex, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, ptpGroups=ptpGroups, sNMPCommunityString=sNMPCommunityString, installArmState=installArmState, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, pubStatsGroup=pubStatsGroup, currentChannel=currentChannel, currentFreqMHz=currentFreqMHz, aggregateDataRate=aggregateDataRate, extendedChannel=extendedChannel, ethernetGroup=ethernetGroup, snmpControlGroup=snmpControlGroup, vlanTagging=vlanTagging, ptp250=ptp250, tFTPServerIPAddress=tFTPServerIPAddress, signalStrengthRatio=signalStrengthRatio, nTPState=nTPState, sNMPTrapIPAddress=sNMPTrapIPAddress, smtpGroup=smtpGroup, transmitPower=transmitPower, nTPSync=nTPSync, hardwareVersion=hardwareVersion, gatewayIPAddress=gatewayIPAddress, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, systemClock=systemClock, iPAddress=iPAddress, sMTPServerIPAddress=sMTPServerIPAddress, dualPayload=dualPayload)
