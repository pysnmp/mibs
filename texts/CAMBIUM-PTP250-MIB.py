...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, sNMPCommunityTable=sNMPCommunityTable, sNMPTrapIPAddress=sNMPTrapIPAddress, cableLoss=cableLoss, nTPState=nTPState, PYSNMP_MODULE_ID=cambium, signalStrengthRatio=signalStrengthRatio, fixedModMode=fixedModMode, dataPortAutoNegotiation=dataPortAutoNegotiation, gatewayIPAddress=gatewayIPAddress, licenceGroup=licenceGroup, currentChannel=currentChannel, smtpGroup=smtpGroup, extendedFreqMHz=extendedFreqMHz, versionsGroup=versionsGroup, aggregateDataRate=aggregateDataRate, installArmStateTrap=installArmStateTrap, snmpControl=snmpControl, ptpGroups=ptpGroups, alarmsGroup=alarmsGroup, ethernetGroup=ethernetGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, wirelessLinkStatus=wirelessLinkStatus, noiseFloor=noiseFloor, cambium=cambium, versions=versions, bootVersion=bootVersion, phyStatus=phyStatus, sNMPCommunityTableNumber=sNMPCommunityTableNumber, configuredModulationMode=configuredModulationMode, ethernet=ethernet, configuration=configuration, sNMPCommunityOid=sNMPCommunityOid, nTPSync=nTPSync, receiveModulationMode=receiveModulationMode, transmitDataRate=transmitDataRate, licence=licence, remoteMACAddress=remoteMACAddress, configuredRange=configuredRange, nTPServerPortNumber=nTPServerPortNumber, subnetMask=subnetMask, systemClock=systemClock, snmpControlGroup=snmpControlGroup, smtp=smtp, nTPPollInterval=nTPPollInterval, dualPayload=dualPayload, ptpTrapPrefix=ptpTrapPrefix, ntp=ntp, remoteIPAddress=remoteIPAddress, transmitPower=transmitPower, ptpTraps=ptpTraps, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, measuredRange=measuredRange, channelSelection=channelSelection, sMTPServerIPAddress=sMTPServerIPAddress, sNMPTrapTableEntry=sNMPTrapTableEntry, configurationGroup=configurationGroup, transmitModulationMode=transmitModulationMode, timeZone=timeZone, sNMPCommunityString=sNMPCommunityString, band=band, sNMPCommunityAccess=sNMPCommunityAccess, maximumTransmitPower=maximumTransmitPower, sNMPTrapVersion=sNMPTrapVersion, nTPLastSync=nTPLastSync, ptmp=ptmp, currentFreqMHz=currentFreqMHz, ptpCompliance=ptpCompliance, receivePower=receivePower, extendedChannel=extendedChannel, iPAddress=iPAddress, nTPServerIp=nTPServerIp, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPEnabledTraps=sNMPEnabledTraps, pubStatsGroup=pubStatsGroup, vectorError=vectorError, tFTPServerIPAddress=tFTPServerIPAddress, alarms=alarms, notificationsGroup=notificationsGroup, siteName=siteName, sNMPTrapPortNumber=sNMPTrapPortNumber, sMTPEnabledMessages=sMTPEnabledMessages, productVariant=productVariant, regionCode=regionCode, sNMPTrapTableNumber=sNMPTrapTableNumber, vlanId=vlanId, installArmState=installArmState, sNMPCommunityTableEntry=sNMPCommunityTableEntry, masterSlaveMode=masterSlaveMode, sNMPTrapTable=sNMPTrapTable, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, dataPortStatus=dataPortStatus, linkName=linkName, channelBandwidth=channelBandwidth, searchState=searchState, sNMPCommunityTableIndex=sNMPCommunityTableIndex, managementGroup=managementGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, pubStats=pubStats, linkStatusTrap=linkStatusTrap, radarDetectChannel=radarDetectChannel, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sMTPServerPortNumber=sMTPServerPortNumber, receiveDataRate=receiveDataRate, radarDetectTrap=radarDetectTrap, dataPortStatusTrap=dataPortStatusTrap, ptp250=ptp250, hardwareVersion=hardwareVersion, linkLoss=linkLoss, vlanTagging=vlanTagging, ptp=ptp, management=management, antennaGain=antennaGain, phyStatusGroup=phyStatusGroup, softwareVersion=softwareVersion, sNMPTrapTableIndex=sNMPTrapTableIndex, sMTPSourceEmailAddress=sMTPSourceEmailAddress, tFTPServerPortNumber=tFTPServerPortNumber, vlanPriority=vlanPriority, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPTrapCommunity=sNMPTrapCommunity, ntpGroup=ntpGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, productName=productName)
