...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", receivePower=receivePower, extendedFreqMHz=extendedFreqMHz, ptpCompliance=ptpCompliance, management=management, sNMPTrapVersion=sNMPTrapVersion, versions=versions, sNMPTrapCommunity=sNMPTrapCommunity, nTPServerPortNumber=nTPServerPortNumber, sNMPCommunityOid=sNMPCommunityOid, aggregateDataRate=aggregateDataRate, dualPayload=dualPayload, productVariant=productVariant, PYSNMP_MODULE_ID=cambium, installArmState=installArmState, transmitPower=transmitPower, sNMPCommunityTableIndex=sNMPCommunityTableIndex, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, channelBandwidth=channelBandwidth, ethernetGroup=ethernetGroup, maximumTransmitPower=maximumTransmitPower, linkStatusTrap=linkStatusTrap, licenceGroup=licenceGroup, ntpGroup=ntpGroup, tFTPServerIPAddress=tFTPServerIPAddress, systemClock=systemClock, channelSelection=channelSelection, configurationGroup=configurationGroup, vectorError=vectorError, sNMPCommunityTableEntry=sNMPCommunityTableEntry, nTPServerIp=nTPServerIp, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, antennaGain=antennaGain, licence=licence, currentFreqMHz=currentFreqMHz, configuration=configuration, sNMPTrapTableEntry=sNMPTrapTableEntry, gatewayIPAddress=gatewayIPAddress, nTPPollInterval=nTPPollInterval, sNMPCommunityTableNumber=sNMPCommunityTableNumber, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNMPTrapPortNumber=sNMPTrapPortNumber, sNMPTrapIPAddress=sNMPTrapIPAddress, sNMPTrapTableNumber=sNMPTrapTableNumber, noiseFloor=noiseFloor, smtpGroup=smtpGroup, timeZone=timeZone, smtp=smtp, sMTPSourceEmailAddress=sMTPSourceEmailAddress, notificationsGroup=notificationsGroup, band=band, snmpControl=snmpControl, transmitModulationMode=transmitModulationMode, extendedChannel=extendedChannel, bootVersion=bootVersion, cableLoss=cableLoss, productName=productName, nTPLastSync=nTPLastSync, linkName=linkName, sNMPCommunityTable=sNMPCommunityTable, ptpTraps=ptpTraps, configuredModulationMode=configuredModulationMode, nTPSync=nTPSync, ethernet=ethernet, searchState=searchState, ptp250=ptp250, pubStatsGroup=pubStatsGroup, radarDetectChannel=radarDetectChannel, remoteMACAddress=remoteMACAddress, wirelessLinkStatus=wirelessLinkStatus, fixedModMode=fixedModMode, sNMPCommunityAccess=sNMPCommunityAccess, dataPortAutoNegotiation=dataPortAutoNegotiation, snmpControlGroup=snmpControlGroup, currentChannel=currentChannel, ptpGroups=ptpGroups, iPAddress=iPAddress, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, radarDetectTrap=radarDetectTrap, dataPortStatus=dataPortStatus, installArmStateTrap=installArmStateTrap, noWirelessChannelAvailable=noWirelessChannelAvailable, configuredRange=configuredRange, vlanPriority=vlanPriority, sNMPEnabledTraps=sNMPEnabledTraps, pubStats=pubStats, ntp=ntp, nTPState=nTPState, remoteIPAddress=remoteIPAddress, vlanTagging=vlanTagging, sNMPCommunityString=sNMPCommunityString, regionCode=regionCode, sMTPEnabledMessages=sMTPEnabledMessages, sNMPTrapTable=sNMPTrapTable, sNMPTrapTableIndex=sNMPTrapTableIndex, vlanId=vlanId, dataPortStatusTrap=dataPortStatusTrap, siteName=siteName, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, ptmp=ptmp, phyStatus=phyStatus, cambium=cambium, subnetMask=subnetMask, linkLoss=linkLoss, tFTPServerPortNumber=tFTPServerPortNumber, receiveModulationMode=receiveModulationMode, softwareVersion=softwareVersion, alarmsGroup=alarmsGroup, transmitDataRate=transmitDataRate, sMTPServerPortNumber=sMTPServerPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, signalStrengthRatio=signalStrengthRatio, phyStatusGroup=phyStatusGroup, sMTPServerIPAddress=sMTPServerIPAddress, hardwareVersion=hardwareVersion, versionsGroup=versionsGroup, masterSlaveMode=masterSlaveMode, receiveDataRate=receiveDataRate, alarms=alarms, ptpTrapPrefix=ptpTrapPrefix, managementGroup=managementGroup, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, ptp=ptp, measuredRange=measuredRange)
