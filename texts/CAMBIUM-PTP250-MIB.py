...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", antennaGain=antennaGain, alarms=alarms, ethernetGroup=ethernetGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, sNMPTrapCommunity=sNMPTrapCommunity, dataPortStatusTrap=dataPortStatusTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, receiveDataRate=receiveDataRate, configuredModulationMode=configuredModulationMode, nTPServerPortNumber=nTPServerPortNumber, sNMPCommunityTableIndex=sNMPCommunityTableIndex, ntpGroup=ntpGroup, transmitModulationMode=transmitModulationMode, ptpTrapPrefix=ptpTrapPrefix, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, wirelessLinkStatus=wirelessLinkStatus, sNMPCommunityTableNumber=sNMPCommunityTableNumber, dataPortStatus=dataPortStatus, maximumTransmitPower=maximumTransmitPower, ethernet=ethernet, vlanPriority=vlanPriority, ptp=ptp, nTPState=nTPState, vectorError=vectorError, softwareVersion=softwareVersion, channelBandwidth=channelBandwidth, searchState=searchState, vlanId=vlanId, notificationsGroup=notificationsGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, ptmp=ptmp, snmpControl=snmpControl, channelSelection=channelSelection, licenceGroup=licenceGroup, cambium=cambium, aggregateDataRate=aggregateDataRate, extendedFreqMHz=extendedFreqMHz, transmitDataRate=transmitDataRate, smtpGroup=smtpGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sMTPSourceEmailAddress=sMTPSourceEmailAddress, managementGroup=managementGroup, subnetMask=subnetMask, nTPLastSync=nTPLastSync, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, nTPSync=nTPSync, sMTPEnabledMessages=sMTPEnabledMessages, sNMPCommunityTable=sNMPCommunityTable, sNMPTrapTableNumber=sNMPTrapTableNumber, smtp=smtp, regionCode=regionCode, dualPayload=dualPayload, gatewayIPAddress=gatewayIPAddress, band=band, productName=productName, extendedChannel=extendedChannel, ntp=ntp, management=management, versions=versions, sNMPTrapPortNumber=sNMPTrapPortNumber, radarDetectTrap=radarDetectTrap, signalStrengthRatio=signalStrengthRatio, PYSNMP_MODULE_ID=cambium, cableLoss=cableLoss, timeZone=timeZone, sNMPCommunityString=sNMPCommunityString, masterSlaveMode=masterSlaveMode, tFTPServerPortNumber=tFTPServerPortNumber, sNMPTrapTable=sNMPTrapTable, currentFreqMHz=currentFreqMHz, vlanTagging=vlanTagging, tFTPServerIPAddress=tFTPServerIPAddress, ptpCompliance=ptpCompliance, remoteMACAddress=remoteMACAddress, hardwareVersion=hardwareVersion, receivePower=receivePower, ptp250=ptp250, linkStatusTrap=linkStatusTrap, bootVersion=bootVersion, installArmStateTrap=installArmStateTrap, sNMPCommunityOid=sNMPCommunityOid, configuration=configuration, noiseFloor=noiseFloor, systemClock=systemClock, measuredRange=measuredRange, linkName=linkName, sNMPTrapIPAddress=sNMPTrapIPAddress, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, configurationGroup=configurationGroup, pubStats=pubStats, alarmsGroup=alarmsGroup, iPAddress=iPAddress, fixedModMode=fixedModMode, versionsGroup=versionsGroup, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, nTPPollInterval=nTPPollInterval, sNMPCommunityTableEntry=sNMPCommunityTableEntry, snmpControlGroup=snmpControlGroup, linkLoss=linkLoss, sMTPServerIPAddress=sMTPServerIPAddress, siteName=siteName, licence=licence, nTPServerIp=nTPServerIp, phyStatusGroup=phyStatusGroup, installArmState=installArmState, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, pubStatsGroup=pubStatsGroup, sNMPEnabledTraps=sNMPEnabledTraps, phyStatus=phyStatus, sNMPCommunityAccess=sNMPCommunityAccess, sNMPTrapVersion=sNMPTrapVersion, productVariant=productVariant, receiveModulationMode=receiveModulationMode, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, currentChannel=currentChannel, sMTPServerPortNumber=sMTPServerPortNumber, ptpGroups=ptpGroups, ptpTraps=ptpTraps, configuredRange=configuredRange, transmitPower=transmitPower, remoteIPAddress=remoteIPAddress, noWirelessChannelAvailable=noWirelessChannelAvailable, radarDetectChannel=radarDetectChannel)
