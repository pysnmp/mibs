...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", PYSNMP_MODULE_ID=cambium, timeZone=timeZone, ethernet=ethernet, currentChannel=currentChannel, noWirelessChannelAvailable=noWirelessChannelAvailable, ptpTrapPrefix=ptpTrapPrefix, management=management, sMTPEnabledMessages=sMTPEnabledMessages, transmitDataRate=transmitDataRate, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, maximumTransmitPower=maximumTransmitPower, channelSelection=channelSelection, aggregateDataRate=aggregateDataRate, nTPLastSync=nTPLastSync, productName=productName, vlanTagging=vlanTagging, sMTPServerIPAddress=sMTPServerIPAddress, sNMPTrapTableIndex=sNMPTrapTableIndex, vectorError=vectorError, vlanPriority=vlanPriority, phyStatusGroup=phyStatusGroup, nTPServerIp=nTPServerIp, alarmsGroup=alarmsGroup, phyStatus=phyStatus, receiveDataRate=receiveDataRate, managementGroup=managementGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, smtp=smtp, noiseFloor=noiseFloor, notificationsGroup=notificationsGroup, nTPState=nTPState, wirelessLinkStatus=wirelessLinkStatus, cambium=cambium, ptpCompliance=ptpCompliance, systemClock=systemClock, ntp=ntp, extendedFreqMHz=extendedFreqMHz, tFTPServerPortNumber=tFTPServerPortNumber, sNMPCommunityTableEntry=sNMPCommunityTableEntry, dataPortStatus=dataPortStatus, ethernetGroup=ethernetGroup, nTPServerPortNumber=nTPServerPortNumber, radarDetectChannel=radarDetectChannel, ptpTraps=ptpTraps, currentFreqMHz=currentFreqMHz, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, productVariant=productVariant, iPAddress=iPAddress, ptpGroups=ptpGroups, transmitModulationMode=transmitModulationMode, sNMPTrapTable=sNMPTrapTable, gatewayIPAddress=gatewayIPAddress, bootVersion=bootVersion, antennaGain=antennaGain, ptp=ptp, versions=versions, dataPortAutoNegotiation=dataPortAutoNegotiation, licence=licence, configuredModulationMode=configuredModulationMode, fixedModMode=fixedModMode, sNMPTrapCommunity=sNMPTrapCommunity, transmitPower=transmitPower, receiveModulationMode=receiveModulationMode, sNMPCommunityString=sNMPCommunityString, signalStrengthRatio=signalStrengthRatio, sNMPTrapTableNumber=sNMPTrapTableNumber, smtpGroup=smtpGroup, sNMPCommunityOid=sNMPCommunityOid, nTPPollInterval=nTPPollInterval, hardwareVersion=hardwareVersion, sNMPTrapIPAddress=sNMPTrapIPAddress, sMTPServerPortNumber=sMTPServerPortNumber, cableLoss=cableLoss, sNMPCommunityTable=sNMPCommunityTable, pubStats=pubStats, remoteIPAddress=remoteIPAddress, configuredRange=configuredRange, installArmState=installArmState, sNMPCommunityTableIndex=sNMPCommunityTableIndex, pubStatsGroup=pubStatsGroup, tFTPServerIPAddress=tFTPServerIPAddress, installArmStateTrap=installArmStateTrap, regionCode=regionCode, dualPayload=dualPayload, nTPSync=nTPSync, configurationGroup=configurationGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, receivePower=receivePower, dataPortStatusTrap=dataPortStatusTrap, ptmp=ptmp, extendedChannel=extendedChannel, licenceGroup=licenceGroup, siteName=siteName, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, remoteMACAddress=remoteMACAddress, snmpControlGroup=snmpControlGroup, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, radarDetectTrap=radarDetectTrap, sNMPEnabledTraps=sNMPEnabledTraps, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, channelBandwidth=channelBandwidth, linkName=linkName, sNMPTrapVersion=sNMPTrapVersion, sNMPCommunityTableNumber=sNMPCommunityTableNumber, versionsGroup=versionsGroup, ntpGroup=ntpGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, snmpControl=snmpControl, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, subnetMask=subnetMask, alarms=alarms, linkStatusTrap=linkStatusTrap, softwareVersion=softwareVersion, sMTPSourceEmailAddress=sMTPSourceEmailAddress, searchState=searchState, configuration=configuration, ptp250=ptp250, sNMPTrapPortNumber=sNMPTrapPortNumber, band=band, measuredRange=measuredRange, masterSlaveMode=masterSlaveMode, sNMPCommunityAccess=sNMPCommunityAccess, linkLoss=linkLoss, vlanId=vlanId)
