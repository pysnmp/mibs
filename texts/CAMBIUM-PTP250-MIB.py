...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPCommunityOid=sNMPCommunityOid, subnetMask=subnetMask, configurationGroup=configurationGroup, licence=licence, receiveDataRate=receiveDataRate, vlanTagging=vlanTagging, extendedChannel=extendedChannel, receiveModulationMode=receiveModulationMode, fixedModMode=fixedModMode, measuredRange=measuredRange, nTPLastSync=nTPLastSync, timeZone=timeZone, channelSelection=channelSelection, configuration=configuration, sMTPSourceEmailAddress=sMTPSourceEmailAddress, installArmStateTrap=installArmStateTrap, currentChannel=currentChannel, alarmsGroup=alarmsGroup, pubStats=pubStats, transmitModulationMode=transmitModulationMode, noiseFloor=noiseFloor, sNMPCommunityTableNumber=sNMPCommunityTableNumber, receivePower=receivePower, antennaGain=antennaGain, transmitDataRate=transmitDataRate, smtpGroup=smtpGroup, vlanId=vlanId, softwareVersion=softwareVersion, systemClock=systemClock, sNMPTrapCommunity=sNMPTrapCommunity, configuredRange=configuredRange, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, ptmp=ptmp, bootVersion=bootVersion, radarDetectTrap=radarDetectTrap, currentFreqMHz=currentFreqMHz, linkLoss=linkLoss, smtp=smtp, phyStatus=phyStatus, ptp250=ptp250, sNMPTrapVersion=sNMPTrapVersion, gatewayIPAddress=gatewayIPAddress, sNMPTrapTableEntry=sNMPTrapTableEntry, versions=versions, channelBandwidth=channelBandwidth, configuredModulationMode=configuredModulationMode, maximumTransmitPower=maximumTransmitPower, cambium=cambium, vlanPriority=vlanPriority, sNMPTrapPortNumber=sNMPTrapPortNumber, tFTPServerIPAddress=tFTPServerIPAddress, ptp=ptp, aggregateDataRate=aggregateDataRate, iPAddress=iPAddress, remoteMACAddress=remoteMACAddress, ntp=ntp, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, dataPortStatusTrap=dataPortStatusTrap, ethernet=ethernet, cableLoss=cableLoss, regionCode=regionCode, vectorError=vectorError, productVariant=productVariant, phyStatusGroup=phyStatusGroup, siteName=siteName, sNMPTrapTableIndex=sNMPTrapTableIndex, nTPServerIp=nTPServerIp, nTPSync=nTPSync, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, nTPServerPortNumber=nTPServerPortNumber, snmpControl=snmpControl, sMTPServerPortNumber=sMTPServerPortNumber, sNMPCommunityTable=sNMPCommunityTable, signalStrengthRatio=signalStrengthRatio, hardwareVersion=hardwareVersion, masterSlaveMode=masterSlaveMode, searchState=searchState, wirelessLinkStatus=wirelessLinkStatus, ptpCompliance=ptpCompliance, transmitPower=transmitPower, pubStatsGroup=pubStatsGroup, tFTPServerPortNumber=tFTPServerPortNumber, sNMPTrapTableNumber=sNMPTrapTableNumber, sNMPCommunityAccess=sNMPCommunityAccess, sMTPServerIPAddress=sMTPServerIPAddress, productName=productName, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, dataPortAutoNegotiation=dataPortAutoNegotiation, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNMPCommunityTableEntry=sNMPCommunityTableEntry, sMTPEnabledMessages=sMTPEnabledMessages, sNMPCommunityTableIndex=sNMPCommunityTableIndex, sNMPTrapTable=sNMPTrapTable, ptpGroups=ptpGroups, nTPPollInterval=nTPPollInterval, managementGroup=managementGroup, PYSNMP_MODULE_ID=cambium, ethernetGroup=ethernetGroup, linkStatusTrap=linkStatusTrap, band=band, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, sNMPTrapIPAddress=sNMPTrapIPAddress, ptpTraps=ptpTraps, linkName=linkName, dualPayload=dualPayload, sNMPEnabledTraps=sNMPEnabledTraps, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNMPCommunityString=sNMPCommunityString, ptpTrapPrefix=ptpTrapPrefix, versionsGroup=versionsGroup, snmpControlGroup=snmpControlGroup, installArmState=installArmState, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, dataPortStatus=dataPortStatus, alarms=alarms, ntpGroup=ntpGroup, management=management, licenceGroup=licenceGroup, nTPState=nTPState, remoteIPAddress=remoteIPAddress, notificationsGroup=notificationsGroup, extendedFreqMHz=extendedFreqMHz, radarDetectChannel=radarDetectChannel)
