...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", wirelessLinkStatus=wirelessLinkStatus, sNMPCommunityTableIndex=sNMPCommunityTableIndex, installArmState=installArmState, ptmp=ptmp, vectorError=vectorError, PYSNMP_MODULE_ID=cambium, sMTPServerIPAddress=sMTPServerIPAddress, ptpTrapPrefix=ptpTrapPrefix, sNMPTrapVersion=sNMPTrapVersion, systemClock=systemClock, band=band, tFTPServerIPAddress=tFTPServerIPAddress, siteName=siteName, smtp=smtp, sMTPSourceEmailAddress=sMTPSourceEmailAddress, nTPLastSync=nTPLastSync, nTPServerIp=nTPServerIp, fixedModMode=fixedModMode, linkName=linkName, versions=versions, sNMPCommunityTableNumber=sNMPCommunityTableNumber, configurationGroup=configurationGroup, cableLoss=cableLoss, dataPortStatus=dataPortStatus, sNMPTrapTableNumber=sNMPTrapTableNumber, transmitModulationMode=transmitModulationMode, configuredModulationMode=configuredModulationMode, receiveDataRate=receiveDataRate, nTPPollInterval=nTPPollInterval, searchState=searchState, pubStats=pubStats, maximumTransmitPower=maximumTransmitPower, measuredRange=measuredRange, sNMPTrapIPAddress=sNMPTrapIPAddress, ptpGroups=ptpGroups, sMTPServerPortNumber=sMTPServerPortNumber, receivePower=receivePower, sNMPCommunityOid=sNMPCommunityOid, licenceGroup=licenceGroup, remoteIPAddress=remoteIPAddress, linkStatusTrap=linkStatusTrap, alarmsGroup=alarmsGroup, dualPayload=dualPayload, sNMPTrapCommunity=sNMPTrapCommunity, aggregateDataRate=aggregateDataRate, dataPortStatusTrap=dataPortStatusTrap, subnetMask=subnetMask, sNMPEnabledTraps=sNMPEnabledTraps, dataPortAutoNegotiation=dataPortAutoNegotiation, ptpCompliance=ptpCompliance, remoteMACAddress=remoteMACAddress, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, bootVersion=bootVersion, versionsGroup=versionsGroup, transmitPower=transmitPower, snmpControl=snmpControl, currentFreqMHz=currentFreqMHz, sNMPCommunityTableEntry=sNMPCommunityTableEntry, nTPSync=nTPSync, channelBandwidth=channelBandwidth, receiveModulationMode=receiveModulationMode, sNMPTrapTableIndex=sNMPTrapTableIndex, antennaGain=antennaGain, sNMPTrapTableEntry=sNMPTrapTableEntry, vlanId=vlanId, nTPServerPortNumber=nTPServerPortNumber, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, phyStatusGroup=phyStatusGroup, channelSelection=channelSelection, tFTPServerPortNumber=tFTPServerPortNumber, sNMPTrapPortNumber=sNMPTrapPortNumber, gatewayIPAddress=gatewayIPAddress, currentChannel=currentChannel, ptp250=ptp250, ethernetGroup=ethernetGroup, sNMPCommunityTable=sNMPCommunityTable, vlanTagging=vlanTagging, extendedFreqMHz=extendedFreqMHz, ptp=ptp, cambium=cambium, sNMPTrapTable=sNMPTrapTable, sNMPCommunityString=sNMPCommunityString, vlanPriority=vlanPriority, masterSlaveMode=masterSlaveMode, management=management, softwareVersion=softwareVersion, configuredRange=configuredRange, regionCode=regionCode, extendedChannel=extendedChannel, smtpGroup=smtpGroup, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, configuration=configuration, installArmStateTrap=installArmStateTrap, sMTPEnabledMessages=sMTPEnabledMessages, noiseFloor=noiseFloor, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, pubStatsGroup=pubStatsGroup, timeZone=timeZone, radarDetectTrap=radarDetectTrap, ntp=ntp, iPAddress=iPAddress, notificationsGroup=notificationsGroup, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, ethernet=ethernet, ptpTraps=ptpTraps, noWirelessChannelAvailable=noWirelessChannelAvailable, transmitDataRate=transmitDataRate, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, licence=licence, managementGroup=managementGroup, hardwareVersion=hardwareVersion, phyStatus=phyStatus, productName=productName, radarDetectChannel=radarDetectChannel, ntpGroup=ntpGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, signalStrengthRatio=signalStrengthRatio, nTPState=nTPState, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, sNMPCommunityAccess=sNMPCommunityAccess, productVariant=productVariant, alarms=alarms, snmpControlGroup=snmpControlGroup, linkLoss=linkLoss)
