...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", vlanId=vlanId, phyStatusGroup=phyStatusGroup, cambium=cambium, sNMPCommunityTableIndex=sNMPCommunityTableIndex, sNMPTrapTableNumber=sNMPTrapTableNumber, dataPortAutoNegotiation=dataPortAutoNegotiation, ethernet=ethernet, hardwareVersion=hardwareVersion, nTPPollInterval=nTPPollInterval, sNMPTrapTableIndex=sNMPTrapTableIndex, configurationGroup=configurationGroup, channelSelection=channelSelection, pubStatsGroup=pubStatsGroup, smtp=smtp, alarmsGroup=alarmsGroup, configuration=configuration, tFTPServerPortNumber=tFTPServerPortNumber, receivePower=receivePower, searchState=searchState, snmpControl=snmpControl, radarDetectTrap=radarDetectTrap, receiveModulationMode=receiveModulationMode, softwareVersion=softwareVersion, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, nTPServerIp=nTPServerIp, ptpTrapPrefix=ptpTrapPrefix, iPAddress=iPAddress, ptp=ptp, ptpCompliance=ptpCompliance, wirelessLinkStatus=wirelessLinkStatus, installArmStateTrap=installArmStateTrap, ptp250=ptp250, productName=productName, productVariant=productVariant, pubStats=pubStats, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, phyStatus=phyStatus, subnetMask=subnetMask, noWirelessChannelAvailable=noWirelessChannelAvailable, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, nTPSync=nTPSync, notificationsGroup=notificationsGroup, sNMPCommunityTableEntry=sNMPCommunityTableEntry, vlanTagging=vlanTagging, sMTPServerPortNumber=sMTPServerPortNumber, noiseFloor=noiseFloor, gatewayIPAddress=gatewayIPAddress, configuredModulationMode=configuredModulationMode, smtpGroup=smtpGroup, currentFreqMHz=currentFreqMHz, sNMPCommunityAccess=sNMPCommunityAccess, linkName=linkName, transmitPower=transmitPower, sNMPTrapVersion=sNMPTrapVersion, alarms=alarms, dualPayload=dualPayload, fixedModMode=fixedModMode, managementGroup=managementGroup, remoteIPAddress=remoteIPAddress, aggregateDataRate=aggregateDataRate, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, channelBandwidth=channelBandwidth, configuredRange=configuredRange, siteName=siteName, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, vectorError=vectorError, extendedFreqMHz=extendedFreqMHz, versionsGroup=versionsGroup, sMTPEnabledMessages=sMTPEnabledMessages, licenceGroup=licenceGroup, cableLoss=cableLoss, sNMPCommunityTableNumber=sNMPCommunityTableNumber, sNMPTrapTableEntry=sNMPTrapTableEntry, vlanPriority=vlanPriority, extendedChannel=extendedChannel, ntpGroup=ntpGroup, remoteMACAddress=remoteMACAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, band=band, versions=versions, linkLoss=linkLoss, sNMPTrapTable=sNMPTrapTable, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, installArmState=installArmState, measuredRange=measuredRange, dataPortStatus=dataPortStatus, nTPState=nTPState, systemClock=systemClock, sNMPCommunityTable=sNMPCommunityTable, sMTPServerIPAddress=sMTPServerIPAddress, sNMPCommunityString=sNMPCommunityString, dataPortStatusTrap=dataPortStatusTrap, maximumTransmitPower=maximumTransmitPower, ntp=ntp, ethernetGroup=ethernetGroup, antennaGain=antennaGain, currentChannel=currentChannel, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, nTPLastSync=nTPLastSync, transmitModulationMode=transmitModulationMode, radarDetectChannel=radarDetectChannel, ptmp=ptmp, snmpControlGroup=snmpControlGroup, receiveDataRate=receiveDataRate, sNMPTrapIPAddress=sNMPTrapIPAddress, tFTPServerIPAddress=tFTPServerIPAddress, bootVersion=bootVersion, timeZone=timeZone, ptpTraps=ptpTraps, sNMPTrapCommunity=sNMPTrapCommunity, sNMPEnabledTraps=sNMPEnabledTraps, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNMPCommunityOid=sNMPCommunityOid, licence=licence, ptpGroups=ptpGroups, masterSlaveMode=masterSlaveMode, transmitDataRate=transmitDataRate, signalStrengthRatio=signalStrengthRatio, PYSNMP_MODULE_ID=cambium, management=management, linkStatusTrap=linkStatusTrap, regionCode=regionCode, nTPServerPortNumber=nTPServerPortNumber)
