...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", installArmStateTrap=installArmStateTrap, transmitPower=transmitPower, pubStatsGroup=pubStatsGroup, ptpGroups=ptpGroups, nTPLastSync=nTPLastSync, snmpControl=snmpControl, sNMPTrapTableIndex=sNMPTrapTableIndex, sMTPEnabledMessages=sMTPEnabledMessages, bootVersion=bootVersion, wirelessLinkStatus=wirelessLinkStatus, ptpTraps=ptpTraps, ethernet=ethernet, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, currentFreqMHz=currentFreqMHz, cableLoss=cableLoss, sMTPServerPortNumber=sMTPServerPortNumber, antennaGain=antennaGain, nTPServerIp=nTPServerIp, dataPortAutoNegotiation=dataPortAutoNegotiation, ntp=ntp, receivePower=receivePower, extendedFreqMHz=extendedFreqMHz, iPAddress=iPAddress, sMTPSourceEmailAddress=sMTPSourceEmailAddress, receiveDataRate=receiveDataRate, productVariant=productVariant, subnetMask=subnetMask, alarms=alarms, linkLoss=linkLoss, vectorError=vectorError, licenceGroup=licenceGroup, nTPServerPortNumber=nTPServerPortNumber, softwareVersion=softwareVersion, nTPState=nTPState, productName=productName, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, phyStatus=phyStatus, vlanPriority=vlanPriority, remoteMACAddress=remoteMACAddress, versionsGroup=versionsGroup, phyStatusGroup=phyStatusGroup, fixedModMode=fixedModMode, siteName=siteName, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, sMTPServerIPAddress=sMTPServerIPAddress, sNMPTrapTableEntry=sNMPTrapTableEntry, smtp=smtp, sNMPCommunityTableNumber=sNMPCommunityTableNumber, channelBandwidth=channelBandwidth, radarDetectTrap=radarDetectTrap, aggregateDataRate=aggregateDataRate, dataPortStatusTrap=dataPortStatusTrap, sNMPCommunityString=sNMPCommunityString, PYSNMP_MODULE_ID=cambium, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, maximumTransmitPower=maximumTransmitPower, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, extendedChannel=extendedChannel, transmitModulationMode=transmitModulationMode, sNMPCommunityTableIndex=sNMPCommunityTableIndex, ptp=ptp, notificationsGroup=notificationsGroup, ethernetGroup=ethernetGroup, regionCode=regionCode, ntpGroup=ntpGroup, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, gatewayIPAddress=gatewayIPAddress, ptmp=ptmp, configuredRange=configuredRange, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, remoteIPAddress=remoteIPAddress, snmpControlGroup=snmpControlGroup, configurationGroup=configurationGroup, vlanTagging=vlanTagging, currentChannel=currentChannel, alarmsGroup=alarmsGroup, timeZone=timeZone, signalStrengthRatio=signalStrengthRatio, sNMPTrapTableNumber=sNMPTrapTableNumber, configuration=configuration, receiveModulationMode=receiveModulationMode, ptpCompliance=ptpCompliance, masterSlaveMode=masterSlaveMode, ptpTrapPrefix=ptpTrapPrefix, linkName=linkName, sNMPCommunityAccess=sNMPCommunityAccess, dataPortStatus=dataPortStatus, managementGroup=managementGroup, configuredModulationMode=configuredModulationMode, sNMPCommunityTableEntry=sNMPCommunityTableEntry, versions=versions, sNMPTrapTable=sNMPTrapTable, cambium=cambium, management=management, band=band, radarDetectChannel=radarDetectChannel, noWirelessChannelAvailable=noWirelessChannelAvailable, pubStats=pubStats, sNMPCommunityTable=sNMPCommunityTable, systemClock=systemClock, channelSelection=channelSelection, tFTPServerIPAddress=tFTPServerIPAddress, sNMPTrapIPAddress=sNMPTrapIPAddress, sNMPCommunityOid=sNMPCommunityOid, vlanId=vlanId, dualPayload=dualPayload, linkStatusTrap=linkStatusTrap, ptp250=ptp250, hardwareVersion=hardwareVersion, tFTPServerPortNumber=tFTPServerPortNumber, nTPSync=nTPSync, sNMPEnabledTraps=sNMPEnabledTraps, noiseFloor=noiseFloor, smtpGroup=smtpGroup, measuredRange=measuredRange, sNMPTrapPortNumber=sNMPTrapPortNumber, sNMPTrapCommunity=sNMPTrapCommunity, licence=licence, sNMPTrapVersion=sNMPTrapVersion, nTPPollInterval=nTPPollInterval, transmitDataRate=transmitDataRate, searchState=searchState, installArmState=installArmState)
