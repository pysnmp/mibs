...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", fixedModMode=fixedModMode, channelBandwidth=channelBandwidth, ptpTraps=ptpTraps, tFTPServerPortNumber=tFTPServerPortNumber, ethernet=ethernet, sNMPCommunityTableEntry=sNMPCommunityTableEntry, linkName=linkName, hardwareVersion=hardwareVersion, remoteIPAddress=remoteIPAddress, subnetMask=subnetMask, sNMPTrapTableEntry=sNMPTrapTableEntry, measuredRange=measuredRange, searchState=searchState, ntp=ntp, noWirelessChannelAvailable=noWirelessChannelAvailable, sMTPServerIPAddress=sMTPServerIPAddress, maximumTransmitPower=maximumTransmitPower, timeZone=timeZone, sNMPCommunityOid=sNMPCommunityOid, sNMPTrapCommunity=sNMPTrapCommunity, sMTPServerPortNumber=sMTPServerPortNumber, wirelessLinkStatus=wirelessLinkStatus, pubStats=pubStats, alarms=alarms, receivePower=receivePower, nTPPollInterval=nTPPollInterval, ptp250=ptp250, sNMPTrapIPAddress=sNMPTrapIPAddress, snmpControlGroup=snmpControlGroup, ethernetGroup=ethernetGroup, vlanTagging=vlanTagging, remoteMACAddress=remoteMACAddress, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, tFTPServerIPAddress=tFTPServerIPAddress, dataPortStatusTrap=dataPortStatusTrap, sNMPCommunityTableNumber=sNMPCommunityTableNumber, management=management, iPAddress=iPAddress, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, installArmStateTrap=installArmStateTrap, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, notificationsGroup=notificationsGroup, versions=versions, productName=productName, transmitDataRate=transmitDataRate, siteName=siteName, smtpGroup=smtpGroup, nTPSync=nTPSync, sNMPTrapTableNumber=sNMPTrapTableNumber, transmitPower=transmitPower, configuredModulationMode=configuredModulationMode, sNMPCommunityTableIndex=sNMPCommunityTableIndex, alarmsGroup=alarmsGroup, cambium=cambium, transmitModulationMode=transmitModulationMode, sNMPEnabledTraps=sNMPEnabledTraps, ptpGroups=ptpGroups, signalStrengthRatio=signalStrengthRatio, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, cableLoss=cableLoss, noiseFloor=noiseFloor, bootVersion=bootVersion, managementGroup=managementGroup, sMTPEnabledMessages=sMTPEnabledMessages, ptpTrapPrefix=ptpTrapPrefix, extendedFreqMHz=extendedFreqMHz, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, aggregateDataRate=aggregateDataRate, masterSlaveMode=masterSlaveMode, receiveModulationMode=receiveModulationMode, sNMPCommunityAccess=sNMPCommunityAccess, vectorError=vectorError, sNMPTrapTable=sNMPTrapTable, phyStatusGroup=phyStatusGroup, linkStatusTrap=linkStatusTrap, sNMPCommunityTable=sNMPCommunityTable, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, smtp=smtp, nTPLastSync=nTPLastSync, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, linkLoss=linkLoss, antennaGain=antennaGain, nTPServerPortNumber=nTPServerPortNumber, phyStatus=phyStatus, currentChannel=currentChannel, sNMPCommunityString=sNMPCommunityString, sMTPSourceEmailAddress=sMTPSourceEmailAddress, systemClock=systemClock, sNMPTrapTableIndex=sNMPTrapTableIndex, vlanId=vlanId, pubStatsGroup=pubStatsGroup, radarDetectTrap=radarDetectTrap, ptmp=ptmp, configuration=configuration, installArmState=installArmState, receiveDataRate=receiveDataRate, ptp=ptp, channelSelection=channelSelection, vlanPriority=vlanPriority, currentFreqMHz=currentFreqMHz, dualPayload=dualPayload, ptpCompliance=ptpCompliance, PYSNMP_MODULE_ID=cambium, regionCode=regionCode, softwareVersion=softwareVersion, ntpGroup=ntpGroup, dataPortStatus=dataPortStatus, sNMPTrapVersion=sNMPTrapVersion, gatewayIPAddress=gatewayIPAddress, configurationGroup=configurationGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, versionsGroup=versionsGroup, nTPState=nTPState, nTPServerIp=nTPServerIp, band=band, productVariant=productVariant, radarDetectChannel=radarDetectChannel, licence=licence, configuredRange=configuredRange, dataPortAutoNegotiation=dataPortAutoNegotiation, licenceGroup=licenceGroup, snmpControl=snmpControl, extendedChannel=extendedChannel, sNMPTrapPortNumber=sNMPTrapPortNumber)
