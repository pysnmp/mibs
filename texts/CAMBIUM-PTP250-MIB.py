...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", vlanId=vlanId, management=management, dataPortStatus=dataPortStatus, extendedChannel=extendedChannel, nTPPollInterval=nTPPollInterval, ptmp=ptmp, smtpGroup=smtpGroup, sNMPCommunityTableEntry=sNMPCommunityTableEntry, versionsGroup=versionsGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, cambium=cambium, sMTPServerPortNumber=sMTPServerPortNumber, pubStats=pubStats, receiveModulationMode=receiveModulationMode, pubStatsGroup=pubStatsGroup, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPTrapVersion=sNMPTrapVersion, ptpTraps=ptpTraps, productVariant=productVariant, nTPServerIp=nTPServerIp, bootVersion=bootVersion, vectorError=vectorError, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, configurationGroup=configurationGroup, ptp250=ptp250, phyStatus=phyStatus, ntpGroup=ntpGroup, versions=versions, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, radarDetectChannel=radarDetectChannel, maximumTransmitPower=maximumTransmitPower, receivePower=receivePower, extendedFreqMHz=extendedFreqMHz, snmpControlGroup=snmpControlGroup, sMTPEnabledMessages=sMTPEnabledMessages, vlanTagging=vlanTagging, configuredRange=configuredRange, fixedModMode=fixedModMode, iPAddress=iPAddress, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, nTPServerPortNumber=nTPServerPortNumber, smtp=smtp, regionCode=regionCode, notificationsGroup=notificationsGroup, softwareVersion=softwareVersion, receiveDataRate=receiveDataRate, noWirelessChannelAvailable=noWirelessChannelAvailable, transmitModulationMode=transmitModulationMode, searchState=searchState, ptpCompliance=ptpCompliance, installArmStateTrap=installArmStateTrap, sNMPCommunityTableNumber=sNMPCommunityTableNumber, tFTPServerPortNumber=tFTPServerPortNumber, sNMPCommunityTableIndex=sNMPCommunityTableIndex, sMTPSourceEmailAddress=sMTPSourceEmailAddress, systemClock=systemClock, cableLoss=cableLoss, channelBandwidth=channelBandwidth, sNMPEnabledTraps=sNMPEnabledTraps, nTPState=nTPState, dualPayload=dualPayload, remoteIPAddress=remoteIPAddress, sNMPTrapIPAddress=sNMPTrapIPAddress, ptpTrapPrefix=ptpTrapPrefix, phyStatusGroup=phyStatusGroup, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, managementGroup=managementGroup, tFTPServerIPAddress=tFTPServerIPAddress, radarDetectTrap=radarDetectTrap, ntp=ntp, masterSlaveMode=masterSlaveMode, alarms=alarms, ethernetGroup=ethernetGroup, alarmsGroup=alarmsGroup, configuredModulationMode=configuredModulationMode, sNMPCommunityOid=sNMPCommunityOid, transmitDataRate=transmitDataRate, sMTPServerIPAddress=sMTPServerIPAddress, productName=productName, channelSelection=channelSelection, sNMPCommunityAccess=sNMPCommunityAccess, antennaGain=antennaGain, aggregateDataRate=aggregateDataRate, noiseFloor=noiseFloor, vlanPriority=vlanPriority, signalStrengthRatio=signalStrengthRatio, PYSNMP_MODULE_ID=cambium, ptpGroups=ptpGroups, sNMPCommunityString=sNMPCommunityString, currentFreqMHz=currentFreqMHz, measuredRange=measuredRange, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, sNMPTrapCommunity=sNMPTrapCommunity, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, timeZone=timeZone, ptp=ptp, currentChannel=currentChannel, dataPortStatusTrap=dataPortStatusTrap, band=band, linkName=linkName, wirelessLinkStatus=wirelessLinkStatus, transmitPower=transmitPower, configuration=configuration, sNMPTrapTableIndex=sNMPTrapTableIndex, linkLoss=linkLoss, sNMPTrapTable=sNMPTrapTable, sNMPTrapTableNumber=sNMPTrapTableNumber, nTPLastSync=nTPLastSync, linkStatusTrap=linkStatusTrap, hardwareVersion=hardwareVersion, snmpControl=snmpControl, siteName=siteName, nTPSync=nTPSync, installArmState=installArmState, remoteMACAddress=remoteMACAddress, licence=licence, licenceGroup=licenceGroup, sNMPCommunityTable=sNMPCommunityTable, gatewayIPAddress=gatewayIPAddress, ethernet=ethernet, subnetMask=subnetMask, dataPortAutoNegotiation=dataPortAutoNegotiation, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap)
