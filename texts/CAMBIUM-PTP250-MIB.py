...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", noWirelessChannelAvailable=noWirelessChannelAvailable, phyStatusGroup=phyStatusGroup, installArmState=installArmState, remoteMACAddress=remoteMACAddress, licence=licence, managementGroup=managementGroup, versionsGroup=versionsGroup, signalStrengthRatio=signalStrengthRatio, vlanTagging=vlanTagging, ptp=ptp, tFTPServerPortNumber=tFTPServerPortNumber, iPAddress=iPAddress, hardwareVersion=hardwareVersion, ethernetGroup=ethernetGroup, sNMPCommunityTableIndex=sNMPCommunityTableIndex, siteName=siteName, nTPLastSync=nTPLastSync, PYSNMP_MODULE_ID=cambium, currentFreqMHz=currentFreqMHz, nTPState=nTPState, linkStatusTrap=linkStatusTrap, ptpCompliance=ptpCompliance, ptpGroups=ptpGroups, noiseFloor=noiseFloor, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNMPCommunityOid=sNMPCommunityOid, linkLoss=linkLoss, ptmp=ptmp, aggregateDataRate=aggregateDataRate, snmpControlGroup=snmpControlGroup, fixedModMode=fixedModMode, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, cambium=cambium, sNMPTrapTable=sNMPTrapTable, nTPSync=nTPSync, ptpTrapPrefix=ptpTrapPrefix, dataPortAutoNegotiation=dataPortAutoNegotiation, nTPPollInterval=nTPPollInterval, gatewayIPAddress=gatewayIPAddress, productVariant=productVariant, sNMPTrapTableIndex=sNMPTrapTableIndex, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, sMTPEnabledMessages=sMTPEnabledMessages, remoteIPAddress=remoteIPAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, ntpGroup=ntpGroup, pubStats=pubStats, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNMPTrapIPAddress=sNMPTrapIPAddress, phyStatus=phyStatus, measuredRange=measuredRange, configuredRange=configuredRange, sNMPCommunityTable=sNMPCommunityTable, vlanPriority=vlanPriority, band=band, cableLoss=cableLoss, radarDetectChannel=radarDetectChannel, nTPServerPortNumber=nTPServerPortNumber, vectorError=vectorError, channelSelection=channelSelection, timeZone=timeZone, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNMPTrapTableNumber=sNMPTrapTableNumber, receiveDataRate=receiveDataRate, sMTPServerPortNumber=sMTPServerPortNumber, nTPServerIp=nTPServerIp, extendedChannel=extendedChannel, sNMPCommunityString=sNMPCommunityString, tFTPServerIPAddress=tFTPServerIPAddress, ptp250=ptp250, sNMPTrapTableEntry=sNMPTrapTableEntry, transmitModulationMode=transmitModulationMode, dualPayload=dualPayload, bootVersion=bootVersion, sNMPTrapCommunity=sNMPTrapCommunity, sNMPCommunityTableEntry=sNMPCommunityTableEntry, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, transmitPower=transmitPower, dataPortStatusTrap=dataPortStatusTrap, channelBandwidth=channelBandwidth, management=management, linkName=linkName, configuredModulationMode=configuredModulationMode, sNMPTrapVersion=sNMPTrapVersion, installArmStateTrap=installArmStateTrap, maximumTransmitPower=maximumTransmitPower, sMTPServerIPAddress=sMTPServerIPAddress, regionCode=regionCode, searchState=searchState, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, smtp=smtp, vlanId=vlanId, snmpControl=snmpControl, sNMPCommunityTableNumber=sNMPCommunityTableNumber, smtpGroup=smtpGroup, productName=productName, pubStatsGroup=pubStatsGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, receiveModulationMode=receiveModulationMode, ptpTraps=ptpTraps, configurationGroup=configurationGroup, alarms=alarms, masterSlaveMode=masterSlaveMode, notificationsGroup=notificationsGroup, systemClock=systemClock, transmitDataRate=transmitDataRate, versions=versions, currentChannel=currentChannel, receivePower=receivePower, sNMPCommunityAccess=sNMPCommunityAccess, ethernet=ethernet, configuration=configuration, wirelessLinkStatus=wirelessLinkStatus, sNMPEnabledTraps=sNMPEnabledTraps, antennaGain=antennaGain, radarDetectTrap=radarDetectTrap, subnetMask=subnetMask, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, alarmsGroup=alarmsGroup, softwareVersion=softwareVersion, dataPortStatus=dataPortStatus, licenceGroup=licenceGroup, ntp=ntp, extendedFreqMHz=extendedFreqMHz)
