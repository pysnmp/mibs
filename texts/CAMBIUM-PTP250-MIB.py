...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", pubStats=pubStats, sNMPCommunityTableNumber=sNMPCommunityTableNumber, sNMPTrapTable=sNMPTrapTable, sNMPTrapTableNumber=sNMPTrapTableNumber, notificationsGroup=notificationsGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, receiveModulationMode=receiveModulationMode, configuredModulationMode=configuredModulationMode, licenceGroup=licenceGroup, vectorError=vectorError, transmitModulationMode=transmitModulationMode, maximumTransmitPower=maximumTransmitPower, gatewayIPAddress=gatewayIPAddress, installArmStateTrap=installArmStateTrap, nTPPollInterval=nTPPollInterval, channelBandwidth=channelBandwidth, ptpGroups=ptpGroups, managementGroup=managementGroup, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, ntp=ntp, sNMPCommunityAccess=sNMPCommunityAccess, pubStatsGroup=pubStatsGroup, ptpTraps=ptpTraps, sNMPTrapPortNumber=sNMPTrapPortNumber, licence=licence, nTPLastSync=nTPLastSync, signalStrengthRatio=signalStrengthRatio, masterSlaveMode=masterSlaveMode, snmpControlGroup=snmpControlGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, cambium=cambium, aggregateDataRate=aggregateDataRate, receivePower=receivePower, nTPServerIp=nTPServerIp, currentFreqMHz=currentFreqMHz, fixedModMode=fixedModMode, noiseFloor=noiseFloor, versions=versions, linkLoss=linkLoss, alarmsGroup=alarmsGroup, sNMPTrapVersion=sNMPTrapVersion, ptmp=ptmp, dualPayload=dualPayload, productName=productName, nTPServerPortNumber=nTPServerPortNumber, phyStatusGroup=phyStatusGroup, installArmState=installArmState, sMTPServerIPAddress=sMTPServerIPAddress, ptp250=ptp250, vlanId=vlanId, sNMPCommunityTable=sNMPCommunityTable, ptp=ptp, sNMPEnabledTraps=sNMPEnabledTraps, softwareVersion=softwareVersion, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, sNMPCommunityOid=sNMPCommunityOid, nTPSync=nTPSync, wirelessLinkStatus=wirelessLinkStatus, alarms=alarms, tFTPServerIPAddress=tFTPServerIPAddress, sMTPServerPortNumber=sMTPServerPortNumber, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, hardwareVersion=hardwareVersion, transmitDataRate=transmitDataRate, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, vlanPriority=vlanPriority, band=band, sNMPTrapTableEntry=sNMPTrapTableEntry, phyStatus=phyStatus, sNMPCommunityTableIndex=sNMPCommunityTableIndex, radarDetectChannel=radarDetectChannel, noWirelessChannelAvailable=noWirelessChannelAvailable, sMTPSourceEmailAddress=sMTPSourceEmailAddress, dataPortAutoNegotiation=dataPortAutoNegotiation, productVariant=productVariant, radarDetectTrap=radarDetectTrap, regionCode=regionCode, ethernet=ethernet, linkName=linkName, snmpControl=snmpControl, sNMPTrapTableIndex=sNMPTrapTableIndex, ptpCompliance=ptpCompliance, extendedFreqMHz=extendedFreqMHz, timeZone=timeZone, versionsGroup=versionsGroup, PYSNMP_MODULE_ID=cambium, dataPortStatus=dataPortStatus, bootVersion=bootVersion, siteName=siteName, vlanTagging=vlanTagging, smtp=smtp, ptpTrapPrefix=ptpTrapPrefix, sMTPEnabledMessages=sMTPEnabledMessages, remoteIPAddress=remoteIPAddress, sNMPTrapIPAddress=sNMPTrapIPAddress, sNMPTrapCommunity=sNMPTrapCommunity, iPAddress=iPAddress, sNMPCommunityString=sNMPCommunityString, cableLoss=cableLoss, configuration=configuration, currentChannel=currentChannel, configurationGroup=configurationGroup, measuredRange=measuredRange, tFTPServerPortNumber=tFTPServerPortNumber, remoteMACAddress=remoteMACAddress, antennaGain=antennaGain, configuredRange=configuredRange, receiveDataRate=receiveDataRate, dataPortStatusTrap=dataPortStatusTrap, transmitPower=transmitPower, ntpGroup=ntpGroup, searchState=searchState, nTPState=nTPState, smtpGroup=smtpGroup, linkStatusTrap=linkStatusTrap, management=management, systemClock=systemClock, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, extendedChannel=extendedChannel, subnetMask=subnetMask, channelSelection=channelSelection, ethernetGroup=ethernetGroup, sNMPCommunityTableEntry=sNMPCommunityTableEntry)
