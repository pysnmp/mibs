...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", ptp250=ptp250, sNMPTrapTableNumber=sNMPTrapTableNumber, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, phyStatusGroup=phyStatusGroup, nTPLastSync=nTPLastSync, signalStrengthRatio=signalStrengthRatio, configuredModulationMode=configuredModulationMode, sNMPTrapVersion=sNMPTrapVersion, configuration=configuration, ntpGroup=ntpGroup, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, linkStatusTrap=linkStatusTrap, transmitModulationMode=transmitModulationMode, receiveDataRate=receiveDataRate, radarDetectTrap=radarDetectTrap, ptpTrapPrefix=ptpTrapPrefix, regionCode=regionCode, channelSelection=channelSelection, dualPayload=dualPayload, ptpTraps=ptpTraps, softwareVersion=softwareVersion, aggregateDataRate=aggregateDataRate, vlanTagging=vlanTagging, ptpCompliance=ptpCompliance, transmitDataRate=transmitDataRate, configurationGroup=configurationGroup, pubStatsGroup=pubStatsGroup, alarmsGroup=alarmsGroup, timeZone=timeZone, sMTPServerIPAddress=sMTPServerIPAddress, nTPServerPortNumber=nTPServerPortNumber, tFTPServerIPAddress=tFTPServerIPAddress, wirelessLinkStatus=wirelessLinkStatus, productVariant=productVariant, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, systemClock=systemClock, sNMPCommunityString=sNMPCommunityString, alarms=alarms, ptp=ptp, sNMPEnabledTraps=sNMPEnabledTraps, sNMPCommunityTable=sNMPCommunityTable, pubStats=pubStats, sNMPCommunityTableEntry=sNMPCommunityTableEntry, sNMPTrapTable=sNMPTrapTable, sNMPCommunityTableNumber=sNMPCommunityTableNumber, ptpGroups=ptpGroups, configuredRange=configuredRange, channelBandwidth=channelBandwidth, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sMTPServerPortNumber=sMTPServerPortNumber, vlanId=vlanId, sNMPCommunityTableIndex=sNMPCommunityTableIndex, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sMTPEnabledMessages=sMTPEnabledMessages, nTPState=nTPState, antennaGain=antennaGain, ntp=ntp, fixedModMode=fixedModMode, vectorError=vectorError, nTPServerIp=nTPServerIp, dataPortStatusTrap=dataPortStatusTrap, snmpControlGroup=snmpControlGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, managementGroup=managementGroup, management=management, ptmp=ptmp, receivePower=receivePower, maximumTransmitPower=maximumTransmitPower, licence=licence, nTPPollInterval=nTPPollInterval, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, subnetMask=subnetMask, receiveModulationMode=receiveModulationMode, smtpGroup=smtpGroup, noiseFloor=noiseFloor, currentChannel=currentChannel, snmpControl=snmpControl, cableLoss=cableLoss, transmitPower=transmitPower, siteName=siteName, remoteMACAddress=remoteMACAddress, band=band, sNMPTrapIPAddress=sNMPTrapIPAddress, hardwareVersion=hardwareVersion, linkName=linkName, versions=versions, iPAddress=iPAddress, notificationsGroup=notificationsGroup, remoteIPAddress=remoteIPAddress, installArmStateTrap=installArmStateTrap, radarDetectChannel=radarDetectChannel, ethernetGroup=ethernetGroup, linkLoss=linkLoss, sNMPCommunityAccess=sNMPCommunityAccess, smtp=smtp, PYSNMP_MODULE_ID=cambium, phyStatus=phyStatus, noWirelessChannelAvailable=noWirelessChannelAvailable, currentFreqMHz=currentFreqMHz, bootVersion=bootVersion, sNMPTrapCommunity=sNMPTrapCommunity, ethernet=ethernet, nTPSync=nTPSync, licenceGroup=licenceGroup, tFTPServerPortNumber=tFTPServerPortNumber, gatewayIPAddress=gatewayIPAddress, measuredRange=measuredRange, cambium=cambium, sNMPTrapTableEntry=sNMPTrapTableEntry, extendedFreqMHz=extendedFreqMHz, searchState=searchState, dataPortAutoNegotiation=dataPortAutoNegotiation, masterSlaveMode=masterSlaveMode, installArmState=installArmState, versionsGroup=versionsGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNMPCommunityOid=sNMPCommunityOid, productName=productName, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, extendedChannel=extendedChannel, dataPortStatus=dataPortStatus, sNMPTrapTableIndex=sNMPTrapTableIndex, vlanPriority=vlanPriority)
