...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", licenceGroup=licenceGroup, dualPayload=dualPayload, snmpControlGroup=snmpControlGroup, ptpTraps=ptpTraps, notificationsGroup=notificationsGroup, ptp=ptp, cambium=cambium, cableLoss=cableLoss, productName=productName, sMTPEnabledMessages=sMTPEnabledMessages, nTPPollInterval=nTPPollInterval, bootVersion=bootVersion, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, tFTPServerPortNumber=tFTPServerPortNumber, smtp=smtp, receiveDataRate=receiveDataRate, vlanTagging=vlanTagging, sNMPCommunityTableNumber=sNMPCommunityTableNumber, receivePower=receivePower, noWirelessChannelAvailable=noWirelessChannelAvailable, pubStatsGroup=pubStatsGroup, transmitPower=transmitPower, band=band, configuredRange=configuredRange, licence=licence, timeZone=timeZone, iPAddress=iPAddress, phyStatus=phyStatus, radarDetectChannel=radarDetectChannel, smtpGroup=smtpGroup, ethernetGroup=ethernetGroup, nTPServerPortNumber=nTPServerPortNumber, sNMPCommunityTableIndex=sNMPCommunityTableIndex, sNMPCommunityTableEntry=sNMPCommunityTableEntry, masterSlaveMode=masterSlaveMode, sNMPTrapVersion=sNMPTrapVersion, currentChannel=currentChannel, maximumTransmitPower=maximumTransmitPower, sNMPTrapTableNumber=sNMPTrapTableNumber, fixedModMode=fixedModMode, linkName=linkName, sMTPServerIPAddress=sMTPServerIPAddress, sNMPCommunityString=sNMPCommunityString, sNMPTrapTableEntry=sNMPTrapTableEntry, remoteMACAddress=remoteMACAddress, siteName=siteName, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, vectorError=vectorError, extendedChannel=extendedChannel, alarms=alarms, configuration=configuration, sNMPEnabledTraps=sNMPEnabledTraps, ethernet=ethernet, regionCode=regionCode, channelSelection=channelSelection, hardwareVersion=hardwareVersion, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, antennaGain=antennaGain, versions=versions, nTPSync=nTPSync, configurationGroup=configurationGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, extendedFreqMHz=extendedFreqMHz, sNMPTrapTableIndex=sNMPTrapTableIndex, sNMPTrapIPAddress=sNMPTrapIPAddress, dataPortAutoNegotiation=dataPortAutoNegotiation, dataPortStatus=dataPortStatus, phyStatusGroup=phyStatusGroup, gatewayIPAddress=gatewayIPAddress, vlanId=vlanId, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, installArmState=installArmState, linkLoss=linkLoss, searchState=searchState, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, measuredRange=measuredRange, remoteIPAddress=remoteIPAddress, nTPState=nTPState, ptmp=ptmp, softwareVersion=softwareVersion, dataPortStatusTrap=dataPortStatusTrap, management=management, configuredModulationMode=configuredModulationMode, snmpControl=snmpControl, ntpGroup=ntpGroup, sNMPCommunityOid=sNMPCommunityOid, sNMPTrapPortNumber=sNMPTrapPortNumber, ptpTrapPrefix=ptpTrapPrefix, versionsGroup=versionsGroup, alarmsGroup=alarmsGroup, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, PYSNMP_MODULE_ID=cambium, ptpGroups=ptpGroups, wirelessLinkStatus=wirelessLinkStatus, ntp=ntp, vlanPriority=vlanPriority, ptp250=ptp250, aggregateDataRate=aggregateDataRate, managementGroup=managementGroup, sMTPServerPortNumber=sMTPServerPortNumber, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNMPCommunityTable=sNMPCommunityTable, installArmStateTrap=installArmStateTrap, transmitDataRate=transmitDataRate, currentFreqMHz=currentFreqMHz, systemClock=systemClock, ptpCompliance=ptpCompliance, subnetMask=subnetMask, nTPServerIp=nTPServerIp, transmitModulationMode=transmitModulationMode, channelBandwidth=channelBandwidth, sNMPCommunityAccess=sNMPCommunityAccess, tFTPServerIPAddress=tFTPServerIPAddress, sNMPTrapCommunity=sNMPTrapCommunity, productVariant=productVariant, receiveModulationMode=receiveModulationMode, noiseFloor=noiseFloor, radarDetectTrap=radarDetectTrap, signalStrengthRatio=signalStrengthRatio, linkStatusTrap=linkStatusTrap, sNMPTrapTable=sNMPTrapTable, nTPLastSync=nTPLastSync, pubStats=pubStats)
