...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", sNMPCommunityTableIndex=sNMPCommunityTableIndex, receivePower=receivePower, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, timeZone=timeZone, licenceGroup=licenceGroup, management=management, nTPPollInterval=nTPPollInterval, smtp=smtp, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sNMPEnabledTraps=sNMPEnabledTraps, sNMPCommunityTable=sNMPCommunityTable, managementGroup=managementGroup, vlanPriority=vlanPriority, sNMPCommunityTableEntry=sNMPCommunityTableEntry, sNMPTrapVersion=sNMPTrapVersion, aggregateDataRate=aggregateDataRate, transmitModulationMode=transmitModulationMode, sMTPServerIPAddress=sMTPServerIPAddress, ntp=ntp, channelSelection=channelSelection, searchState=searchState, ptpTraps=ptpTraps, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, alarms=alarms, linkLoss=linkLoss, snmpControlGroup=snmpControlGroup, antennaGain=antennaGain, tFTPServerIPAddress=tFTPServerIPAddress, installArmState=installArmState, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, dualPayload=dualPayload, tFTPServerPortNumber=tFTPServerPortNumber, cableLoss=cableLoss, currentChannel=currentChannel, dataPortAutoNegotiation=dataPortAutoNegotiation, ptpCompliance=ptpCompliance, configuration=configuration, PYSNMP_MODULE_ID=cambium, ptp250=ptp250, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, nTPLastSync=nTPLastSync, ptmp=ptmp, productVariant=productVariant, subnetMask=subnetMask, remoteMACAddress=remoteMACAddress, channelBandwidth=channelBandwidth, nTPServerPortNumber=nTPServerPortNumber, masterSlaveMode=masterSlaveMode, linkStatusTrap=linkStatusTrap, sNMPTrapPortNumber=sNMPTrapPortNumber, nTPSync=nTPSync, cambium=cambium, sMTPEnabledMessages=sMTPEnabledMessages, receiveDataRate=receiveDataRate, band=band, extendedFreqMHz=extendedFreqMHz, configuredModulationMode=configuredModulationMode, siteName=siteName, pubStats=pubStats, sNMPCommunityAccess=sNMPCommunityAccess, sNMPTrapTableEntry=sNMPTrapTableEntry, dataPortStatus=dataPortStatus, receiveModulationMode=receiveModulationMode, fixedModMode=fixedModMode, hardwareVersion=hardwareVersion, ptp=ptp, nTPState=nTPState, softwareVersion=softwareVersion, bootVersion=bootVersion, sNMPTrapTableNumber=sNMPTrapTableNumber, systemClock=systemClock, sNMPTrapCommunity=sNMPTrapCommunity, vlanTagging=vlanTagging, smtpGroup=smtpGroup, measuredRange=measuredRange, sNMPTrapTable=sNMPTrapTable, radarDetectChannel=radarDetectChannel, maximumTransmitPower=maximumTransmitPower, alarmsGroup=alarmsGroup, ethernet=ethernet, wirelessLinkStatus=wirelessLinkStatus, sNMPTrapTableIndex=sNMPTrapTableIndex, phyStatus=phyStatus, sMTPSourceEmailAddress=sMTPSourceEmailAddress, radarDetectTrap=radarDetectTrap, sMTPServerPortNumber=sMTPServerPortNumber, dataPortStatusTrap=dataPortStatusTrap, sNMPCommunityOid=sNMPCommunityOid, ethernetGroup=ethernetGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, versionsGroup=versionsGroup, linkName=linkName, iPAddress=iPAddress, transmitPower=transmitPower, vlanId=vlanId, sNMPTrapIPAddress=sNMPTrapIPAddress, licence=licence, ptpTrapPrefix=ptpTrapPrefix, sNMPCommunityString=sNMPCommunityString, productName=productName, currentFreqMHz=currentFreqMHz, vectorError=vectorError, extendedChannel=extendedChannel, versions=versions, snmpControl=snmpControl, configurationGroup=configurationGroup, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, installArmStateTrap=installArmStateTrap, nTPServerIp=nTPServerIp, signalStrengthRatio=signalStrengthRatio, ntpGroup=ntpGroup, gatewayIPAddress=gatewayIPAddress, pubStatsGroup=pubStatsGroup, regionCode=regionCode, notificationsGroup=notificationsGroup, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, noiseFloor=noiseFloor, phyStatusGroup=phyStatusGroup, ptpGroups=ptpGroups, remoteIPAddress=remoteIPAddress, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPCommunityTableNumber=sNMPCommunityTableNumber, transmitDataRate=transmitDataRate, configuredRange=configuredRange)
