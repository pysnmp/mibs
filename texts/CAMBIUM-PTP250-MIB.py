...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", sNMPCommunityString=sNMPCommunityString, ptmp=ptmp, notificationsGroup=notificationsGroup, hardwareVersion=hardwareVersion, searchState=searchState, currentChannel=currentChannel, configuration=configuration, fixedModMode=fixedModMode, timeZone=timeZone, channelBandwidth=channelBandwidth, sNMPTrapTableEntry=sNMPTrapTableEntry, regionCode=regionCode, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, softwareVersion=softwareVersion, receiveModulationMode=receiveModulationMode, ptp=ptp, tFTPServerIPAddress=tFTPServerIPAddress, systemClock=systemClock, snmpControlGroup=snmpControlGroup, ethernet=ethernet, antennaGain=antennaGain, configuredRange=configuredRange, sNMPCommunityTableNumber=sNMPCommunityTableNumber, sNMPCommunityAccess=sNMPCommunityAccess, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, snmpControl=snmpControl, productVariant=productVariant, ptp250=ptp250, sNMPTrapTableNumber=sNMPTrapTableNumber, phyStatusGroup=phyStatusGroup, vectorError=vectorError, vlanId=vlanId, nTPServerIp=nTPServerIp, sNMPCommunityOid=sNMPCommunityOid, remoteMACAddress=remoteMACAddress, dataPortStatusTrap=dataPortStatusTrap, aggregateDataRate=aggregateDataRate, sNMPTrapVersion=sNMPTrapVersion, siteName=siteName, band=band, vlanPriority=vlanPriority, licence=licence, dataPortAutoNegotiation=dataPortAutoNegotiation, channelSelection=channelSelection, ptpCompliance=ptpCompliance, transmitDataRate=transmitDataRate, sNMPTrapIPAddress=sNMPTrapIPAddress, receiveDataRate=receiveDataRate, radarDetectTrap=radarDetectTrap, management=management, tFTPServerPortNumber=tFTPServerPortNumber, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, vlanTagging=vlanTagging, linkName=linkName, PYSNMP_MODULE_ID=cambium, ethernetGroup=ethernetGroup, nTPState=nTPState, dualPayload=dualPayload, sMTPServerPortNumber=sMTPServerPortNumber, configuredModulationMode=configuredModulationMode, noWirelessChannelAvailable=noWirelessChannelAvailable, wirelessLinkStatus=wirelessLinkStatus, pubStats=pubStats, sNMPCommunityTableIndex=sNMPCommunityTableIndex, linkLoss=linkLoss, ptpGroups=ptpGroups, installArmState=installArmState, receivePower=receivePower, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, remoteIPAddress=remoteIPAddress, bootVersion=bootVersion, productName=productName, transmitModulationMode=transmitModulationMode, ntp=ntp, masterSlaveMode=masterSlaveMode, cableLoss=cableLoss, gatewayIPAddress=gatewayIPAddress, versions=versions, alarms=alarms, currentFreqMHz=currentFreqMHz, versionsGroup=versionsGroup, ptpTraps=ptpTraps, extendedFreqMHz=extendedFreqMHz, linkStatusTrap=linkStatusTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNMPTrapPortNumber=sNMPTrapPortNumber, sNMPCommunityTable=sNMPCommunityTable, cambium=cambium, sNMPTrapTableIndex=sNMPTrapTableIndex, sMTPServerIPAddress=sMTPServerIPAddress, smtp=smtp, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPCommunityTableEntry=sNMPCommunityTableEntry, licenceGroup=licenceGroup, subnetMask=subnetMask, phyStatus=phyStatus, nTPServerPortNumber=nTPServerPortNumber, extendedChannel=extendedChannel, pubStatsGroup=pubStatsGroup, transmitPower=transmitPower, signalStrengthRatio=signalStrengthRatio, ntpGroup=ntpGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, configurationGroup=configurationGroup, sMTPEnabledMessages=sMTPEnabledMessages, installArmStateTrap=installArmStateTrap, dataPortStatus=dataPortStatus, ptpTrapPrefix=ptpTrapPrefix, radarDetectChannel=radarDetectChannel, managementGroup=managementGroup, smtpGroup=smtpGroup, nTPLastSync=nTPLastSync, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, alarmsGroup=alarmsGroup, measuredRange=measuredRange, sMTPSourceEmailAddress=sMTPSourceEmailAddress, maximumTransmitPower=maximumTransmitPower, nTPPollInterval=nTPPollInterval, noiseFloor=noiseFloor, iPAddress=iPAddress, sNMPTrapCommunity=sNMPTrapCommunity, sNMPTrapTable=sNMPTrapTable, nTPSync=nTPSync, sNMPEnabledTraps=sNMPEnabledTraps)
