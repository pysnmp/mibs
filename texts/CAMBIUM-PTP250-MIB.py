...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", regionCode=regionCode, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, masterSlaveMode=masterSlaveMode, PYSNMP_MODULE_ID=cambium, nTPPollInterval=nTPPollInterval, ethernetGroup=ethernetGroup, managementGroup=managementGroup, snmpControl=snmpControl, channelBandwidth=channelBandwidth, vlanPriority=vlanPriority, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPCommunityTableIndex=sNMPCommunityTableIndex, ntp=ntp, receiveDataRate=receiveDataRate, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, ptp=ptp, productVariant=productVariant, sNMPTrapIPAddress=sNMPTrapIPAddress, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, linkName=linkName, aggregateDataRate=aggregateDataRate, sMTPEnabledMessages=sMTPEnabledMessages, subnetMask=subnetMask, productName=productName, installArmState=installArmState, dataPortStatusTrap=dataPortStatusTrap, dataPortAutoNegotiation=dataPortAutoNegotiation, sNMPCommunityAccess=sNMPCommunityAccess, pubStatsGroup=pubStatsGroup, noWirelessChannelAvailable=noWirelessChannelAvailable, cambium=cambium, configuration=configuration, versionsGroup=versionsGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, dataPortStatus=dataPortStatus, alarmsGroup=alarmsGroup, linkLoss=linkLoss, currentChannel=currentChannel, currentFreqMHz=currentFreqMHz, searchState=searchState, sNMPTrapVersion=sNMPTrapVersion, snmpControlGroup=snmpControlGroup, transmitModulationMode=transmitModulationMode, gatewayIPAddress=gatewayIPAddress, sNMPCommunityTableNumber=sNMPCommunityTableNumber, notificationsGroup=notificationsGroup, vlanId=vlanId, sMTPSourceEmailAddress=sMTPSourceEmailAddress, transmitDataRate=transmitDataRate, dualPayload=dualPayload, nTPSync=nTPSync, linkStatusTrap=linkStatusTrap, nTPLastSync=nTPLastSync, bootVersion=bootVersion, sNMPEnabledTraps=sNMPEnabledTraps, pubStats=pubStats, maximumTransmitPower=maximumTransmitPower, ptpTrapPrefix=ptpTrapPrefix, sNMPTrapTable=sNMPTrapTable, receivePower=receivePower, vlanTagging=vlanTagging, radarDetectChannel=radarDetectChannel, measuredRange=measuredRange, configuredModulationMode=configuredModulationMode, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, extendedFreqMHz=extendedFreqMHz, cableLoss=cableLoss, ptpGroups=ptpGroups, antennaGain=antennaGain, smtpGroup=smtpGroup, ntpGroup=ntpGroup, hardwareVersion=hardwareVersion, sNMPCommunityTableEntry=sNMPCommunityTableEntry, alarms=alarms, nTPState=nTPState, signalStrengthRatio=signalStrengthRatio, vectorError=vectorError, ptp250=ptp250, phyStatusGroup=phyStatusGroup, channelSelection=channelSelection, installArmStateTrap=installArmStateTrap, extendedChannel=extendedChannel, wirelessLinkStatus=wirelessLinkStatus, licence=licence, ethernet=ethernet, sNMPTrapCommunity=sNMPTrapCommunity, ptpTraps=ptpTraps, band=band, radarDetectTrap=radarDetectTrap, softwareVersion=softwareVersion, configurationGroup=configurationGroup, sNMPCommunityOid=sNMPCommunityOid, sNMPTrapTableEntry=sNMPTrapTableEntry, licenceGroup=licenceGroup, nTPServerIp=nTPServerIp, ptpCompliance=ptpCompliance, ptmp=ptmp, phyStatus=phyStatus, tFTPServerIPAddress=tFTPServerIPAddress, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, nTPServerPortNumber=nTPServerPortNumber, iPAddress=iPAddress, sMTPServerIPAddress=sMTPServerIPAddress, sNMPCommunityString=sNMPCommunityString, sNMPTrapTableNumber=sNMPTrapTableNumber, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, systemClock=systemClock, management=management, sNMPCommunityTable=sNMPCommunityTable, remoteMACAddress=remoteMACAddress, timeZone=timeZone, tFTPServerPortNumber=tFTPServerPortNumber, receiveModulationMode=receiveModulationMode, siteName=siteName, sNMPTrapPortNumber=sNMPTrapPortNumber, configuredRange=configuredRange, versions=versions, sMTPServerPortNumber=sMTPServerPortNumber, sNMPTrapTableIndex=sNMPTrapTableIndex, smtp=smtp, fixedModMode=fixedModMode, remoteIPAddress=remoteIPAddress, noiseFloor=noiseFloor, transmitPower=transmitPower)
