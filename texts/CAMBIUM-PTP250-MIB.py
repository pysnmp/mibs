...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", sNMPCommunityString=sNMPCommunityString, ntpGroup=ntpGroup, PYSNMP_MODULE_ID=cambium, linkName=linkName, smtp=smtp, maximumTransmitPower=maximumTransmitPower, sNMPTrapTableEntry=sNMPTrapTableEntry, radarDetectTrap=radarDetectTrap, measuredRange=measuredRange, hardwareVersion=hardwareVersion, sNMPTrapIPAddress=sNMPTrapIPAddress, wirelessLinkStatus=wirelessLinkStatus, notificationsGroup=notificationsGroup, transmitModulationMode=transmitModulationMode, nTPSync=nTPSync, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, vlanTagging=vlanTagging, snmpControlGroup=snmpControlGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, configuredRange=configuredRange, systemClock=systemClock, fixedModMode=fixedModMode, vlanId=vlanId, dataPortStatus=dataPortStatus, noWirelessChannelAvailable=noWirelessChannelAvailable, antennaGain=antennaGain, sNMPCommunityOid=sNMPCommunityOid, sNMPTrapCommunity=sNMPTrapCommunity, aggregateDataRate=aggregateDataRate, dataPortStatusTrap=dataPortStatusTrap, licence=licence, remoteIPAddress=remoteIPAddress, remoteMACAddress=remoteMACAddress, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, cambium=cambium, receivePower=receivePower, transmitPower=transmitPower, bootVersion=bootVersion, versions=versions, ntp=ntp, sMTPSourceEmailAddress=sMTPSourceEmailAddress, dataPortAutoNegotiation=dataPortAutoNegotiation, radarDetectChannel=radarDetectChannel, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sNMPTrapTableIndex=sNMPTrapTableIndex, phyStatusGroup=phyStatusGroup, ptp250=ptp250, tFTPServerIPAddress=tFTPServerIPAddress, managementGroup=managementGroup, signalStrengthRatio=signalStrengthRatio, linkLoss=linkLoss, sNMPCommunityTableNumber=sNMPCommunityTableNumber, nTPServerPortNumber=nTPServerPortNumber, productVariant=productVariant, channelBandwidth=channelBandwidth, ptmp=ptmp, installArmStateTrap=installArmStateTrap, sNMPCommunityTableEntry=sNMPCommunityTableEntry, nTPLastSync=nTPLastSync, alarmsGroup=alarmsGroup, noiseFloor=noiseFloor, phyStatus=phyStatus, pubStats=pubStats, ptpGroups=ptpGroups, sNMPTrapTable=sNMPTrapTable, ethernet=ethernet, channelSelection=channelSelection, sNMPCommunityTableIndex=sNMPCommunityTableIndex, masterSlaveMode=masterSlaveMode, regionCode=regionCode, sNMPTrapVersion=sNMPTrapVersion, extendedFreqMHz=extendedFreqMHz, ethernetGroup=ethernetGroup, installArmState=installArmState, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, nTPState=nTPState, tFTPServerPortNumber=tFTPServerPortNumber, sNMPCommunityAccess=sNMPCommunityAccess, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, configuration=configuration, linkStatusTrap=linkStatusTrap, nTPPollInterval=nTPPollInterval, iPAddress=iPAddress, receiveDataRate=receiveDataRate, subnetMask=subnetMask, softwareVersion=softwareVersion, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, ptpTrapPrefix=ptpTrapPrefix, management=management, currentChannel=currentChannel, sNMPTrapTableNumber=sNMPTrapTableNumber, cableLoss=cableLoss, siteName=siteName, sNMPCommunityTable=sNMPCommunityTable, vlanPriority=vlanPriority, vectorError=vectorError, configuredModulationMode=configuredModulationMode, timeZone=timeZone, searchState=searchState, band=band, dualPayload=dualPayload, sMTPEnabledMessages=sMTPEnabledMessages, ptpCompliance=ptpCompliance, licenceGroup=licenceGroup, sNMPEnabledTraps=sNMPEnabledTraps, ptpTraps=ptpTraps, sMTPServerIPAddress=sMTPServerIPAddress, nTPServerIp=nTPServerIp, configurationGroup=configurationGroup, productName=productName, pubStatsGroup=pubStatsGroup, transmitDataRate=transmitDataRate, sNMPTrapPortNumber=sNMPTrapPortNumber, receiveModulationMode=receiveModulationMode, sMTPServerPortNumber=sMTPServerPortNumber, currentFreqMHz=currentFreqMHz, versionsGroup=versionsGroup, ptp=ptp, extendedChannel=extendedChannel, alarms=alarms, snmpControl=snmpControl, gatewayIPAddress=gatewayIPAddress, smtpGroup=smtpGroup)
