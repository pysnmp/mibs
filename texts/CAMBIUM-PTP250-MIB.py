...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", cambium=cambium, installArmState=installArmState, versionsGroup=versionsGroup, vlanTagging=vlanTagging, pubStats=pubStats, transmitPower=transmitPower, ptpCompliance=ptpCompliance, channelBandwidth=channelBandwidth, receiveModulationMode=receiveModulationMode, nTPState=nTPState, sNMPEnabledTraps=sNMPEnabledTraps, licence=licence, productVariant=productVariant, versions=versions, licenceGroup=licenceGroup, noiseFloor=noiseFloor, ptpTraps=ptpTraps, extendedChannel=extendedChannel, sMTPServerPortNumber=sMTPServerPortNumber, antennaGain=antennaGain, remoteIPAddress=remoteIPAddress, linkName=linkName, nTPLastSync=nTPLastSync, sNMPCommunityTableEntry=sNMPCommunityTableEntry, sNMPTrapTableIndex=sNMPTrapTableIndex, smtpGroup=smtpGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNMPTrapTableNumber=sNMPTrapTableNumber, tFTPServerPortNumber=tFTPServerPortNumber, alarms=alarms, systemClock=systemClock, linkStatusTrap=linkStatusTrap, alarmsGroup=alarmsGroup, ptp250=ptp250, dataPortStatusTrap=dataPortStatusTrap, nTPServerPortNumber=nTPServerPortNumber, phyStatus=phyStatus, transmitDataRate=transmitDataRate, tFTPServerIPAddress=tFTPServerIPAddress, receiveDataRate=receiveDataRate, sNMPCommunityTableIndex=sNMPCommunityTableIndex, vlanId=vlanId, signalStrengthRatio=signalStrengthRatio, sNMPTrapPortNumber=sNMPTrapPortNumber, configuration=configuration, ptpGroups=ptpGroups, remoteMACAddress=remoteMACAddress, regionCode=regionCode, configurationGroup=configurationGroup, sMTPServerIPAddress=sMTPServerIPAddress, currentFreqMHz=currentFreqMHz, measuredRange=measuredRange, sNMPTrapCommunity=sNMPTrapCommunity, channelSelection=channelSelection, vlanPriority=vlanPriority, snmpControlGroup=snmpControlGroup, iPAddress=iPAddress, linkLoss=linkLoss, cableLoss=cableLoss, currentChannel=currentChannel, ptp=ptp, ptmp=ptmp, band=band, transmitModulationMode=transmitModulationMode, receivePower=receivePower, managementGroup=managementGroup, configuredModulationMode=configuredModulationMode, dualPayload=dualPayload, nTPPollInterval=nTPPollInterval, sMTPEnabledMessages=sMTPEnabledMessages, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, noWirelessChannelAvailable=noWirelessChannelAvailable, PYSNMP_MODULE_ID=cambium, fixedModMode=fixedModMode, wirelessLinkStatus=wirelessLinkStatus, sNMPTrapIPAddress=sNMPTrapIPAddress, pubStatsGroup=pubStatsGroup, snmpControl=snmpControl, ethernet=ethernet, subnetMask=subnetMask, sNMPTrapTableEntry=sNMPTrapTableEntry, dataPortStatus=dataPortStatus, aggregateDataRate=aggregateDataRate, sNMPCommunityString=sNMPCommunityString, hardwareVersion=hardwareVersion, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, productName=productName, sNMPCommunityTableNumber=sNMPCommunityTableNumber, sNMPTrapTable=sNMPTrapTable, searchState=searchState, timeZone=timeZone, installArmStateTrap=installArmStateTrap, nTPServerIp=nTPServerIp, maximumTransmitPower=maximumTransmitPower, ptpTrapPrefix=ptpTrapPrefix, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, smtp=smtp, ethernetGroup=ethernetGroup, siteName=siteName, radarDetectChannel=radarDetectChannel, sMTPSourceEmailAddress=sMTPSourceEmailAddress, nTPSync=nTPSync, sNMPCommunityOid=sNMPCommunityOid, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, ntp=ntp, sNMPCommunityTable=sNMPCommunityTable, bootVersion=bootVersion, sNMPTrapVersion=sNMPTrapVersion, ntpGroup=ntpGroup, configuredRange=configuredRange, sNMPCommunityAccess=sNMPCommunityAccess, notificationsGroup=notificationsGroup, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, radarDetectTrap=radarDetectTrap, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, phyStatusGroup=phyStatusGroup, gatewayIPAddress=gatewayIPAddress, management=management, masterSlaveMode=masterSlaveMode, vectorError=vectorError, softwareVersion=softwareVersion, extendedFreqMHz=extendedFreqMHz, dataPortAutoNegotiation=dataPortAutoNegotiation, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText)
