...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", receiveModulationMode=receiveModulationMode, sNMPEnabledTraps=sNMPEnabledTraps, nTPServerPortNumber=nTPServerPortNumber, cambium=cambium, configuredModulationMode=configuredModulationMode, sMTPServerIPAddress=sMTPServerIPAddress, tFTPServerIPAddress=tFTPServerIPAddress, ptpCompliance=ptpCompliance, tFTPServerPortNumber=tFTPServerPortNumber, antennaGain=antennaGain, radarDetectChannel=radarDetectChannel, remoteMACAddress=remoteMACAddress, sNMPCommunityTableNumber=sNMPCommunityTableNumber, sNMPTrapTable=sNMPTrapTable, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, phyStatus=phyStatus, ptpGroups=ptpGroups, linkName=linkName, ptmp=ptmp, pubStatsGroup=pubStatsGroup, ntp=ntp, gatewayIPAddress=gatewayIPAddress, searchState=searchState, configurationGroup=configurationGroup, hardwareVersion=hardwareVersion, remoteIPAddress=remoteIPAddress, softwareVersion=softwareVersion, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, iPAddress=iPAddress, licence=licence, extendedFreqMHz=extendedFreqMHz, sNMPTrapIPAddress=sNMPTrapIPAddress, ptp=ptp, signalStrengthRatio=signalStrengthRatio, managementGroup=managementGroup, receiveDataRate=receiveDataRate, linkStatusTrap=linkStatusTrap, nTPSync=nTPSync, nTPState=nTPState, notificationsGroup=notificationsGroup, phyStatusGroup=phyStatusGroup, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, configuration=configuration, sNMPCommunityTable=sNMPCommunityTable, alarmsGroup=alarmsGroup, siteName=siteName, fixedModMode=fixedModMode, receivePower=receivePower, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, smtpGroup=smtpGroup, sNMPCommunityTableEntry=sNMPCommunityTableEntry, vlanPriority=vlanPriority, snmpControlGroup=snmpControlGroup, sMTPEnabledMessages=sMTPEnabledMessages, dataPortStatus=dataPortStatus, sNMPTrapTableIndex=sNMPTrapTableIndex, productName=productName, nTPLastSync=nTPLastSync, extendedChannel=extendedChannel, alarms=alarms, currentFreqMHz=currentFreqMHz, ethernet=ethernet, ethernetGroup=ethernetGroup, bootVersion=bootVersion, sMTPSourceEmailAddress=sMTPSourceEmailAddress, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, ptp250=ptp250, transmitPower=transmitPower, snmpControl=snmpControl, installArmState=installArmState, productVariant=productVariant, ptpTraps=ptpTraps, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, PYSNMP_MODULE_ID=cambium, aggregateDataRate=aggregateDataRate, cableLoss=cableLoss, timeZone=timeZone, noWirelessChannelAvailable=noWirelessChannelAvailable, transmitModulationMode=transmitModulationMode, versionsGroup=versionsGroup, radarDetectTrap=radarDetectTrap, licenceGroup=licenceGroup, wirelessLinkStatus=wirelessLinkStatus, configuredRange=configuredRange, smtp=smtp, sMTPServerPortNumber=sMTPServerPortNumber, regionCode=regionCode, sNMPCommunityString=sNMPCommunityString, transmitDataRate=transmitDataRate, sNMPCommunityAccess=sNMPCommunityAccess, sNMPCommunityTableIndex=sNMPCommunityTableIndex, sNMPTrapTableNumber=sNMPTrapTableNumber, sNMPCommunityOid=sNMPCommunityOid, currentChannel=currentChannel, nTPServerIp=nTPServerIp, noiseFloor=noiseFloor, ptpTrapPrefix=ptpTrapPrefix, dataPortStatusTrap=dataPortStatusTrap, vlanId=vlanId, channelBandwidth=channelBandwidth, channelSelection=channelSelection, measuredRange=measuredRange, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, pubStats=pubStats, sNMPTrapTableEntry=sNMPTrapTableEntry, ntpGroup=ntpGroup, versions=versions, band=band, maximumTransmitPower=maximumTransmitPower, systemClock=systemClock, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, masterSlaveMode=masterSlaveMode, sNMPTrapCommunity=sNMPTrapCommunity, vlanTagging=vlanTagging, sNMPTrapPortNumber=sNMPTrapPortNumber, nTPPollInterval=nTPPollInterval, dataPortAutoNegotiation=dataPortAutoNegotiation, sNMPTrapVersion=sNMPTrapVersion, vectorError=vectorError, management=management, linkLoss=linkLoss, subnetMask=subnetMask, dualPayload=dualPayload, installArmStateTrap=installArmStateTrap)
