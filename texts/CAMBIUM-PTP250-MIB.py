...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", sNMPTrapIPAddress=sNMPTrapIPAddress, ptpGroups=ptpGroups, iPAddress=iPAddress, radarDetectTrap=radarDetectTrap, versions=versions, configuration=configuration, snmpControl=snmpControl, vlanId=vlanId, timeZone=timeZone, sMTPServerPortNumber=sMTPServerPortNumber, receiveDataRate=receiveDataRate, ntp=ntp, PYSNMP_MODULE_ID=cambium, ptpTraps=ptpTraps, remoteIPAddress=remoteIPAddress, pubStatsGroup=pubStatsGroup, sNMPTrapCommunity=sNMPTrapCommunity, nTPServerPortNumber=nTPServerPortNumber, extendedFreqMHz=extendedFreqMHz, measuredRange=measuredRange, channelBandwidth=channelBandwidth, configurationGroup=configurationGroup, nTPPollInterval=nTPPollInterval, dataPortAutoNegotiation=dataPortAutoNegotiation, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sMTPEnabledMessages=sMTPEnabledMessages, noiseFloor=noiseFloor, vlanPriority=vlanPriority, subnetMask=subnetMask, ntpGroup=ntpGroup, receivePower=receivePower, linkLoss=linkLoss, signalStrengthRatio=signalStrengthRatio, alarms=alarms, installArmStateTrap=installArmStateTrap, cableLoss=cableLoss, smtp=smtp, sMTPServerIPAddress=sMTPServerIPAddress, gatewayIPAddress=gatewayIPAddress, regionCode=regionCode, phyStatusGroup=phyStatusGroup, nTPSync=nTPSync, sNMPCommunityTable=sNMPCommunityTable, linkStatusTrap=linkStatusTrap, sNMPCommunityTableEntry=sNMPCommunityTableEntry, ptpTrapPrefix=ptpTrapPrefix, sNMPEnabledTraps=sNMPEnabledTraps, systemClock=systemClock, ptp=ptp, transmitModulationMode=transmitModulationMode, vlanTagging=vlanTagging, currentChannel=currentChannel, band=band, sNMPTrapTableNumber=sNMPTrapTableNumber, licence=licence, sNMPTrapTableEntry=sNMPTrapTableEntry, extendedChannel=extendedChannel, vectorError=vectorError, productName=productName, nTPState=nTPState, radarDetectChannel=radarDetectChannel, sNMPCommunityTableNumber=sNMPCommunityTableNumber, dataPortStatusTrap=dataPortStatusTrap, nTPLastSync=nTPLastSync, nTPServerIp=nTPServerIp, productVariant=productVariant, ptmp=ptmp, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, managementGroup=managementGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, softwareVersion=softwareVersion, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, searchState=searchState, snmpControlGroup=snmpControlGroup, ptp250=ptp250, notificationsGroup=notificationsGroup, installArmState=installArmState, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, sNMPCommunityTableIndex=sNMPCommunityTableIndex, sNMPCommunityString=sNMPCommunityString, ptpCompliance=ptpCompliance, currentFreqMHz=currentFreqMHz, channelSelection=channelSelection, sNMPTrapPortNumber=sNMPTrapPortNumber, management=management, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, wirelessLinkStatus=wirelessLinkStatus, tFTPServerIPAddress=tFTPServerIPAddress, antennaGain=antennaGain, dualPayload=dualPayload, pubStats=pubStats, transmitDataRate=transmitDataRate, smtpGroup=smtpGroup, configuredRange=configuredRange, dataPortStatus=dataPortStatus, licenceGroup=licenceGroup, sNMPTrapVersion=sNMPTrapVersion, ethernetGroup=ethernetGroup, hardwareVersion=hardwareVersion, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, masterSlaveMode=masterSlaveMode, fixedModMode=fixedModMode, aggregateDataRate=aggregateDataRate, alarmsGroup=alarmsGroup, receiveModulationMode=receiveModulationMode, tFTPServerPortNumber=tFTPServerPortNumber, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, ethernet=ethernet, maximumTransmitPower=maximumTransmitPower, versionsGroup=versionsGroup, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, linkName=linkName, siteName=siteName, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, phyStatus=phyStatus, transmitPower=transmitPower, configuredModulationMode=configuredModulationMode, remoteMACAddress=remoteMACAddress, sNMPCommunityAccess=sNMPCommunityAccess, cambium=cambium, bootVersion=bootVersion, noWirelessChannelAvailable=noWirelessChannelAvailable, sNMPTrapTable=sNMPTrapTable, sNMPCommunityOid=sNMPCommunityOid)
