...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", timeZone=timeZone, dualPayload=dualPayload, nTPState=nTPState, receiveModulationMode=receiveModulationMode, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, licenceGroup=licenceGroup, configuredRange=configuredRange, vlanPriority=vlanPriority, sNMPCommunityAccess=sNMPCommunityAccess, nTPPollInterval=nTPPollInterval, extendedFreqMHz=extendedFreqMHz, sNMPTrapTable=sNMPTrapTable, cableLoss=cableLoss, productName=productName, smtp=smtp, installArmStateTrap=installArmStateTrap, sMTPServerIPAddress=sMTPServerIPAddress, signalStrengthRatio=signalStrengthRatio, management=management, ntp=ntp, iPAddress=iPAddress, remoteMACAddress=remoteMACAddress, measuredRange=measuredRange, ptpGroups=ptpGroups, tFTPServerIPAddress=tFTPServerIPAddress, sNMPCommunityTableNumber=sNMPCommunityTableNumber, sNMPTrapIPAddress=sNMPTrapIPAddress, dataPortStatus=dataPortStatus, linkLoss=linkLoss, configuration=configuration, fixedModMode=fixedModMode, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, ptp=ptp, noiseFloor=noiseFloor, nTPLastSync=nTPLastSync, linkStatusTrap=linkStatusTrap, ethernetGroup=ethernetGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, ptpTrapPrefix=ptpTrapPrefix, sNMPCommunityString=sNMPCommunityString, ptpTraps=ptpTraps, masterSlaveMode=masterSlaveMode, snmpControlGroup=snmpControlGroup, vlanId=vlanId, sMTPServerPortNumber=sMTPServerPortNumber, cambium=cambium, notificationsGroup=notificationsGroup, nTPSync=nTPSync, productVariant=productVariant, pubStatsGroup=pubStatsGroup, subnetMask=subnetMask, noWirelessChannelAvailable=noWirelessChannelAvailable, ethernet=ethernet, tFTPServerPortNumber=tFTPServerPortNumber, extendedChannel=extendedChannel, radarDetectChannel=radarDetectChannel, currentFreqMHz=currentFreqMHz, sNMPTrapPortNumber=sNMPTrapPortNumber, sNMPCommunityTableEntry=sNMPCommunityTableEntry, sNMPTrapVersion=sNMPTrapVersion, channelSelection=channelSelection, dataPortStatusTrap=dataPortStatusTrap, ptpCompliance=ptpCompliance, channelBandwidth=channelBandwidth, remoteIPAddress=remoteIPAddress, nTPServerIp=nTPServerIp, sNMPTrapTableNumber=sNMPTrapTableNumber, sNMPTrapTableIndex=sNMPTrapTableIndex, nTPServerPortNumber=nTPServerPortNumber, alarmsGroup=alarmsGroup, configurationGroup=configurationGroup, phyStatus=phyStatus, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, snmpControl=snmpControl, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, bootVersion=bootVersion, sMTPEnabledMessages=sMTPEnabledMessages, sNMPTrapCommunity=sNMPTrapCommunity, alarms=alarms, radarDetectTrap=radarDetectTrap, PYSNMP_MODULE_ID=cambium, gatewayIPAddress=gatewayIPAddress, hardwareVersion=hardwareVersion, sMTPSourceEmailAddress=sMTPSourceEmailAddress, transmitPower=transmitPower, sNMPEnabledTraps=sNMPEnabledTraps, receiveDataRate=receiveDataRate, sNMPCommunityTable=sNMPCommunityTable, wirelessLinkStatus=wirelessLinkStatus, transmitModulationMode=transmitModulationMode, sNMPCommunityTableIndex=sNMPCommunityTableIndex, managementGroup=managementGroup, installArmState=installArmState, systemClock=systemClock, ntpGroup=ntpGroup, siteName=siteName, antennaGain=antennaGain, ptp250=ptp250, transmitDataRate=transmitDataRate, band=band, pubStats=pubStats, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, linkName=linkName, searchState=searchState, ptmp=ptmp, phyStatusGroup=phyStatusGroup, regionCode=regionCode, vectorError=vectorError, sNMPTrapTableEntry=sNMPTrapTableEntry, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, dataPortAutoNegotiation=dataPortAutoNegotiation, versions=versions, licence=licence, receivePower=receivePower, maximumTransmitPower=maximumTransmitPower, smtpGroup=smtpGroup, aggregateDataRate=aggregateDataRate, currentChannel=currentChannel, vlanTagging=vlanTagging, sNMPCommunityOid=sNMPCommunityOid, versionsGroup=versionsGroup, softwareVersion=softwareVersion, configuredModulationMode=configuredModulationMode)
