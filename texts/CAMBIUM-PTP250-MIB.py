...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, iPAddress=iPAddress, pubStatsGroup=pubStatsGroup, ptpGroups=ptpGroups, notificationsGroup=notificationsGroup, sNMPTrapCommunity=sNMPTrapCommunity, noWirelessChannelAvailable=noWirelessChannelAvailable, tFTPServerIPAddress=tFTPServerIPAddress, configuredRange=configuredRange, linkLoss=linkLoss, configuredModulationMode=configuredModulationMode, radarDetectChannel=radarDetectChannel, cambium=cambium, currentChannel=currentChannel, sMTPServerPortNumber=sMTPServerPortNumber, sNMPCommunityOid=sNMPCommunityOid, nTPPollInterval=nTPPollInterval, sMTPSourceEmailAddress=sMTPSourceEmailAddress, alarms=alarms, currentFreqMHz=currentFreqMHz, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, versions=versions, installArmStateTrap=installArmStateTrap, receiveModulationMode=receiveModulationMode, nTPServerPortNumber=nTPServerPortNumber, masterSlaveMode=masterSlaveMode, gatewayIPAddress=gatewayIPAddress, ethernet=ethernet, siteName=siteName, measuredRange=measuredRange, radarDetectTrap=radarDetectTrap, alarmsGroup=alarmsGroup, sNMPTrapVersion=sNMPTrapVersion, productName=productName, sMTPServerIPAddress=sMTPServerIPAddress, vlanId=vlanId, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, smtpGroup=smtpGroup, sNMPTrapTableNumber=sNMPTrapTableNumber, nTPLastSync=nTPLastSync, systemClock=systemClock, hardwareVersion=hardwareVersion, versionsGroup=versionsGroup, regionCode=regionCode, ntp=ntp, transmitPower=transmitPower, sNMPEnabledTraps=sNMPEnabledTraps, remoteIPAddress=remoteIPAddress, subnetMask=subnetMask, sNMPTrapIPAddress=sNMPTrapIPAddress, vectorError=vectorError, sNMPCommunityAccess=sNMPCommunityAccess, cableLoss=cableLoss, sNMPCommunityTableNumber=sNMPCommunityTableNumber, ntpGroup=ntpGroup, band=band, dualPayload=dualPayload, extendedChannel=extendedChannel, sNMPTrapTableIndex=sNMPTrapTableIndex, pubStats=pubStats, vlanPriority=vlanPriority, softwareVersion=softwareVersion, receiveDataRate=receiveDataRate, sNMPTrapTable=sNMPTrapTable, snmpControl=snmpControl, transmitDataRate=transmitDataRate, aggregateDataRate=aggregateDataRate, noiseFloor=noiseFloor, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPCommunityTableIndex=sNMPCommunityTableIndex, channelSelection=channelSelection, sNMPCommunityTableEntry=sNMPCommunityTableEntry, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, smtp=smtp, antennaGain=antennaGain, channelBandwidth=channelBandwidth, dataPortAutoNegotiation=dataPortAutoNegotiation, linkName=linkName, licenceGroup=licenceGroup, nTPServerIp=nTPServerIp, configurationGroup=configurationGroup, PYSNMP_MODULE_ID=cambium, sNMPTrapPortNumber=sNMPTrapPortNumber, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, receivePower=receivePower, maximumTransmitPower=maximumTransmitPower, sNMPCommunityString=sNMPCommunityString, installArmState=installArmState, vlanTagging=vlanTagging, transmitModulationMode=transmitModulationMode, management=management, snmpControlGroup=snmpControlGroup, tFTPServerPortNumber=tFTPServerPortNumber, ptmp=ptmp, ptpCompliance=ptpCompliance, nTPState=nTPState, linkStatusTrap=linkStatusTrap, productVariant=productVariant, ptpTrapPrefix=ptpTrapPrefix, ptp250=ptp250, remoteMACAddress=remoteMACAddress, wirelessLinkStatus=wirelessLinkStatus, dataPortStatus=dataPortStatus, ptp=ptp, dataPortStatusTrap=dataPortStatusTrap, fixedModMode=fixedModMode, licence=licence, phyStatusGroup=phyStatusGroup, nTPSync=nTPSync, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, phyStatus=phyStatus, extendedFreqMHz=extendedFreqMHz, ethernetGroup=ethernetGroup, sMTPEnabledMessages=sMTPEnabledMessages, searchState=searchState, configuration=configuration, ptpTraps=ptpTraps, bootVersion=bootVersion, sNMPCommunityTable=sNMPCommunityTable, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, signalStrengthRatio=signalStrengthRatio, managementGroup=managementGroup, timeZone=timeZone)
