...
mibBuilder.exportSymbols("CAMBIUM-PTP250-MIB", tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, linkLoss=linkLoss, sMTPEnabledMessages=sMTPEnabledMessages, band=band, sNMPTrapTableEntry=sNMPTrapTableEntry, sNMPTrapTable=sNMPTrapTable, ntp=ntp, remoteMACAddress=remoteMACAddress, receivePower=receivePower, ptmp=ptmp, regionCode=regionCode, cambium=cambium, sNMPCommunityTableIndex=sNMPCommunityTableIndex, ntpGroup=ntpGroup, vlanId=vlanId, sNMPCommunityTableEntry=sNMPCommunityTableEntry, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, siteName=siteName, vectorError=vectorError, linkStatusTrap=linkStatusTrap, receiveDataRate=receiveDataRate, ethernet=ethernet, radarDetectTrap=radarDetectTrap, nTPServerIp=nTPServerIp, tFTPServerIPAddress=tFTPServerIPAddress, tFTPServerPortNumber=tFTPServerPortNumber, pubStatsGroup=pubStatsGroup, dualPayload=dualPayload, phyStatusGroup=phyStatusGroup, nTPPollInterval=nTPPollInterval, softwareVersion=softwareVersion, sNMPTrapCommunity=sNMPTrapCommunity, PYSNMP_MODULE_ID=cambium, nTPServerPortNumber=nTPServerPortNumber, licenceGroup=licenceGroup, productVariant=productVariant, smtp=smtp, currentChannel=currentChannel, dataPortStatusTrap=dataPortStatusTrap, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, licence=licence, management=management, configuration=configuration, wirelessLinkStatus=wirelessLinkStatus, phyStatus=phyStatus, vlanPriority=vlanPriority, extendedChannel=extendedChannel, channelSelection=channelSelection, ptp=ptp, transmitDataRate=transmitDataRate, installArmStateTrap=installArmStateTrap, noiseFloor=noiseFloor, vlanTagging=vlanTagging, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, pubStats=pubStats, sNMPTrapIPAddress=sNMPTrapIPAddress, antennaGain=antennaGain, sNMPCommunityOid=sNMPCommunityOid, extendedFreqMHz=extendedFreqMHz, sNMPCommunityString=sNMPCommunityString, bootVersion=bootVersion, iPAddress=iPAddress, ethernetGroup=ethernetGroup, sNMPTrapPortNumber=sNMPTrapPortNumber, hardwareVersion=hardwareVersion, remoteIPAddress=remoteIPAddress, signalStrengthRatio=signalStrengthRatio, productName=productName, smtpGroup=smtpGroup, noWirelessChannelAvailable=noWirelessChannelAvailable, sMTPServerIPAddress=sMTPServerIPAddress, snmpControlGroup=snmpControlGroup, sNMPCommunityTableNumber=sNMPCommunityTableNumber, sNMPTrapVersion=sNMPTrapVersion, versionsGroup=versionsGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, masterSlaveMode=masterSlaveMode, sNMPCommunityTable=sNMPCommunityTable, currentFreqMHz=currentFreqMHz, sNMPTrapTableNumber=sNMPTrapTableNumber, ptpCompliance=ptpCompliance, sMTPSourceEmailAddress=sMTPSourceEmailAddress, systemClock=systemClock, dataPortAutoNegotiation=dataPortAutoNegotiation, receiveModulationMode=receiveModulationMode, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, subnetMask=subnetMask, linkName=linkName, ptp250=ptp250, sNMPEnabledTraps=sNMPEnabledTraps, cableLoss=cableLoss, notificationsGroup=notificationsGroup, nTPState=nTPState, gatewayIPAddress=gatewayIPAddress, sNMPTrapTableIndex=sNMPTrapTableIndex, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, nTPLastSync=nTPLastSync, channelBandwidth=channelBandwidth, configurationGroup=configurationGroup, installArmState=installArmState, alarms=alarms, snmpControl=snmpControl, searchState=searchState, aggregateDataRate=aggregateDataRate, configuredModulationMode=configuredModulationMode, fixedModMode=fixedModMode, versions=versions, maximumTransmitPower=maximumTransmitPower, transmitPower=transmitPower, measuredRange=measuredRange, sMTPServerPortNumber=sMTPServerPortNumber, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, alarmsGroup=alarmsGroup, ptpTraps=ptpTraps, radarDetectChannel=radarDetectChannel, ptpGroups=ptpGroups, dataPortStatus=dataPortStatus, timeZone=timeZone, sNMPCommunityAccess=sNMPCommunityAccess, transmitModulationMode=transmitModulationMode, configuredRange=configuredRange, ptpTrapPrefix=ptpTrapPrefix, managementGroup=managementGroup, nTPSync=nTPSync)
