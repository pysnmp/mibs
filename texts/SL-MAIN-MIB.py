...
TimeTicks, NotificationType, MibIdentifier, Opaque, Gauge32, iso, Bits, IpAddress, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, ObjectIdentity, ModuleIdentity, Counter64, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "NotificationType", "MibIdentifier", "Opaque", "Gauge32", "iso", "Bits", "IpAddress", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "ObjectIdentity", "ModuleIdentity", "Counter64", "Counter32")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmSnmpv3Auth=slmSnmpv3Auth, slmTrapLogParam3=slmTrapLogParam3, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmSysTemperature=slmSysTemperature, slmSnmpv3Priv=slmSnmpv3Priv, slmSyslogLevel=slmSyslogLevel, slm40GConf=slm40GConf, slmSysGatewayAddr=slmSysGatewayAddr, slmSysName=slmSysName, slmSysIpAddr=slmSysIpAddr, slmChPassOldPass=slmChPassOldPass, slmSwRevDate=slmSwRevDate, slmTrapSysLocationChange=slmTrapSysLocationChange, slmTrapUserLogin=slmTrapUserLogin, slmConfigSysUptime=slmConfigSysUptime, slMain=slMain, UserLogin=UserLogin, slmSysAlmAct=slmSysAlmAct, slmSysTrapFormat=slmSysTrapFormat, slmTrapCommunity=slmTrapCommunity, slmLicenseId=slmLicenseId, slmTrapDestTable=slmTrapDestTable, slmLogin=slmLogin, slmTrapDestRowStatus=slmTrapDestRowStatus, slmSys=slmSys, slmTrapLogParam1=slmTrapLogParam1, slmTrapDestProtVersion=slmTrapDestProtVersion, slmTid=slmTid, slmAuthEntry=slmAuthEntry, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmSysResetPm=slmSysResetPm, slmTrapDestIpAddress=slmTrapDestIpAddress, slmLicenseIndex=slmLicenseIndex, slmTrapDestAddress=slmTrapDestAddress, slmAdminEntry=slmAdminEntry, slmChPass=slmChPass, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmReadCommunity=slmReadCommunity, slmSysPmStartOfDay=slmSysPmStartOfDay, slmChPassEntry=slmChPassEntry, AdminAccess=AdminAccess, slmSysEncryptionCapability=slmSysEncryptionCapability, slmAdminAccess=slmAdminAccess, SoftwareRevision=SoftwareRevision, slmTrapLogParam6=slmTrapLogParam6, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmNetworkMode=slmNetworkMode, slmTrapSysNameChange=slmTrapSysNameChange, slmSyslogPort=slmSyslogPort, slmTrapLogParam2=slmTrapLogParam2, slmChPassTable=slmChPassTable, slmRstpEnabled=slmRstpEnabled, slmLicenseEntry=slmLicenseEntry, slmAdminCommunity=slmAdminCommunity, slmSyslogDestEntry=slmSyslogDestEntry, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmTrapLogParam4=slmTrapLogParam4, slmSysSubnetMask=slmSysSubnetMask, slmSysOperStatus=slmSysOperStatus, slmTrapLogId=slmTrapLogId, slmSnmpv3Password=slmSnmpv3Password, slmAuth=slmAuth, slmAdminPassword=slmAdminPassword, slmChPassNewPass=slmChPassNewPass, slmPsuNumber=slmPsuNumber, slmSwRevEntry=slmSwRevEntry, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmTrapEnable=slmTrapEnable, slmAdminRowStatus=slmAdminRowStatus, slmTrapLogTable=slmTrapLogTable, slmSysLocation=slmSysLocation, slmPmAvailable=slmPmAvailable, slmSysUplinkRate=slmSysUplinkRate, slmPortsNumber=slmPortsNumber, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmTrapLogName=slmTrapLogName, slmSysLanIpAddr=slmSysLanIpAddr, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, PYSNMP_MODULE_ID=slMain, slmTrap=slmTrap, slmSysAlmDeact=slmSysAlmDeact, slmAuthTable=slmAuthTable, slmTrapLogParam5=slmTrapLogParam5, slmTopologyEnabled=slmTopologyEnabled, slmSwRevDirectory=slmSwRevDirectory, slmSysChassisId=slmSysChassisId, slmSysActiveSwVersion=slmSysActiveSwVersion, slmOpticalSwitchExist=slmOpticalSwitchExist, slmWriteCommunity=slmWriteCommunity, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmLicenseExpiration=slmLicenseExpiration, slmConfigFile=slmConfigFile, slmAdminTable=slmAdminTable, UserCommunity=UserCommunity, slmSysCalendarTime=slmSysCalendarTime, slmAuthPassword=slmAuthPassword, slmAdminLogin=slmAdminLogin, slmSyslogDestAddress=slmSyslogDestAddress, slmAuthCommunity=slmAuthCommunity, slmSwRevTable=slmSwRevTable, slmTrapPort=slmTrapPort, slmBoxSize=slmBoxSize, slmAdmin=slmAdmin, slmEdfaNumber=slmEdfaNumber, slmSysAdminStatus=slmSysAdminStatus, slmSysMode=slmSysMode, slmSysNetworkPrefix=slmSysNetworkPrefix, slmSyslogDestTable=slmSyslogDestTable, slmRebootDelay=slmRebootDelay, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmMuxNumber=slmMuxNumber, slmAuthLogin=slmAuthLogin, slmChPassLogin=slmChPassLogin, slmConfigSysSignalType=slmConfigSysSignalType, slmTrapUserAccess=slmTrapUserAccess, slmOemType=slmOemType, slmTrapDestCommunity=slmTrapDestCommunity, UserPassword=UserPassword, slmLicenseTable=slmLicenseTable, slmSwRevStatus=slmSwRevStatus, slmSwRevName=slmSwRevName, slmLogFile=slmLogFile, slmTrapDestEntry=slmTrapDestEntry, slmTrapLogEntry=slmTrapLogEntry, slmVcatDelay=slmVcatDelay, slmSysSnmpVersion=slmSysSnmpVersion, slmLicense=slmLicense)
