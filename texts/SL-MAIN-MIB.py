...
iso, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Bits, NotificationType, Unsigned32, Integer32, Counter32, MibIdentifier, Gauge32, ModuleIdentity, Counter64, Opaque, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Bits", "NotificationType", "Unsigned32", "Integer32", "Counter32", "MibIdentifier", "Gauge32", "ModuleIdentity", "Counter64", "Opaque", "ObjectIdentity")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmAdmin=slmAdmin, slmSwRevDate=slmSwRevDate, slmTid=slmTid, slmSysAlmDeact=slmSysAlmDeact, slmRstpEnabled=slmRstpEnabled, slmLogin=slmLogin, slmTrapLogParam5=slmTrapLogParam5, slmAdminEntry=slmAdminEntry, slmTrapLogParam6=slmTrapLogParam6, slmSysTemperature=slmSysTemperature, PYSNMP_MODULE_ID=slMain, slmChPassNewPass=slmChPassNewPass, slmTrapCommunity=slmTrapCommunity, slmSnmpv3Priv=slmSnmpv3Priv, slmSysActiveSwVersion=slmSysActiveSwVersion, slmSyslogPort=slmSyslogPort, slmAdminAccess=slmAdminAccess, slmRebootDelay=slmRebootDelay, slmSysGatewayAddr=slmSysGatewayAddr, slmSysOperStatus=slmSysOperStatus, slmTrapDestAddress=slmTrapDestAddress, slmLicenseIndex=slmLicenseIndex, slmTrapPort=slmTrapPort, slmSysLocation=slmSysLocation, slmSysCalendarTime=slmSysCalendarTime, slmTrapDestTable=slmTrapDestTable, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmPsuNumber=slmPsuNumber, slmLicense=slmLicense, slmSyslogLevel=slmSyslogLevel, slmTrapEnable=slmTrapEnable, slmPmAvailable=slmPmAvailable, slmAuth=slmAuth, slmTrapDestIpAddress=slmTrapDestIpAddress, slmTrapLogParam4=slmTrapLogParam4, slmChPassLogin=slmChPassLogin, UserCommunity=UserCommunity, slmTrapLogName=slmTrapLogName, slmVcatDelay=slmVcatDelay, slmLogFile=slmLogFile, slmAuthCommunity=slmAuthCommunity, slmTrapDestRowStatus=slmTrapDestRowStatus, slmTopologyEnabled=slmTopologyEnabled, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmSysUplinkRate=slmSysUplinkRate, slmTrapSysLocationChange=slmTrapSysLocationChange, slmConfigFile=slmConfigFile, slmMuxNumber=slmMuxNumber, slmSwRevName=slmSwRevName, slmSnmpv3Auth=slmSnmpv3Auth, slmSysEncryptionCapability=slmSysEncryptionCapability, slmTrapLogTable=slmTrapLogTable, slmLicenseExpiration=slmLicenseExpiration, slmSyslogDestTable=slmSyslogDestTable, slmAuthEntry=slmAuthEntry, slmBoxSize=slmBoxSize, slmOemType=slmOemType, slmPortsNumber=slmPortsNumber, slmTrapLogParam3=slmTrapLogParam3, slmTrapLogId=slmTrapLogId, slmSysSnmpVersion=slmSysSnmpVersion, slmTrapUserAccess=slmTrapUserAccess, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmSysAlmAct=slmSysAlmAct, slmSysSubnetMask=slmSysSubnetMask, slmAdminCommunity=slmAdminCommunity, slmSysTrapFormat=slmSysTrapFormat, slmSyslogDestIpAddress=slmSyslogDestIpAddress, AdminAccess=AdminAccess, slmSysResetPm=slmSysResetPm, slmNetworkMode=slmNetworkMode, slmAdminLogin=slmAdminLogin, slmConfigSysSignalType=slmConfigSysSignalType, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmSwRevDirectory=slmSwRevDirectory, slmChPassOldPass=slmChPassOldPass, slmConfigSysUptime=slmConfigSysUptime, slmSysName=slmSysName, slmSysAdminStatus=slmSysAdminStatus, slmChPassEntry=slmChPassEntry, slmTrapUserLogin=slmTrapUserLogin, UserLogin=UserLogin, UserPassword=UserPassword, slmSwRevEntry=slmSwRevEntry, slmTrapSysNameChange=slmTrapSysNameChange, slmSysMode=slmSysMode, slmSnmpv3Password=slmSnmpv3Password, slmAuthLogin=slmAuthLogin, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmSysIpAddr=slmSysIpAddr, slMain=slMain, slmAdminRowStatus=slmAdminRowStatus, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmTrapLogParam1=slmTrapLogParam1, slmAuthTable=slmAuthTable, slmChPassTable=slmChPassTable, slmSysNetworkPrefix=slmSysNetworkPrefix, slmTrapDestEntry=slmTrapDestEntry, slmAdminPassword=slmAdminPassword, slmSwRevTable=slmSwRevTable, slmSys=slmSys, slmTrapLogEntry=slmTrapLogEntry, slmSwRevStatus=slmSwRevStatus, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmAuthPassword=slmAuthPassword, slmLicenseEntry=slmLicenseEntry, slmWriteCommunity=slmWriteCommunity, slmLicenseTable=slmLicenseTable, slmEdfaNumber=slmEdfaNumber, slmTrap=slmTrap, slmSysLanIpAddr=slmSysLanIpAddr, slmSyslogDestEntry=slmSyslogDestEntry, slmOpticalSwitchExist=slmOpticalSwitchExist, slmTrapLogParam2=slmTrapLogParam2, slmReadCommunity=slmReadCommunity, slm40GConf=slm40GConf, slmChPass=slmChPass, slmSyslogDestAddress=slmSyslogDestAddress, slmTrapDestProtVersion=slmTrapDestProtVersion, slmSysChassisId=slmSysChassisId, slmSysPmStartOfDay=slmSysPmStartOfDay, slmLicenseId=slmLicenseId, slmAdminTable=slmAdminTable, slmTrapDestCommunity=slmTrapDestCommunity, SoftwareRevision=SoftwareRevision)
