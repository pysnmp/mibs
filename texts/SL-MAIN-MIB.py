...
MibIdentifier, Bits, Opaque, Integer32, ObjectIdentity, Counter32, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, iso, Counter64, Gauge32, NotificationType, ModuleIdentity, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Bits", "Opaque", "Integer32", "ObjectIdentity", "Counter32", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "iso", "Counter64", "Gauge32", "NotificationType", "ModuleIdentity", "TimeTicks")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmAdminPassword=slmAdminPassword, slmAdminCommunity=slmAdminCommunity, slmTrapCommunity=slmTrapCommunity, slmRebootDelay=slmRebootDelay, slmTopologyEnabled=slmTopologyEnabled, slmOpticalSwitchExist=slmOpticalSwitchExist, slmTrapLogName=slmTrapLogName, PYSNMP_MODULE_ID=slMain, slmTrapLogEntry=slmTrapLogEntry, slmAdmin=slmAdmin, slmSysAlmAct=slmSysAlmAct, slmAdminRowStatus=slmAdminRowStatus, slmWriteCommunity=slmWriteCommunity, slmSysEncryptionCapability=slmSysEncryptionCapability, slmLicenseIndex=slmLicenseIndex, slmSyslogDestIpAddress=slmSyslogDestIpAddress, UserPassword=UserPassword, slMain=slMain, slmSysName=slmSysName, slmSysOperStatus=slmSysOperStatus, slmBoxSize=slmBoxSize, slmSysChassisId=slmSysChassisId, slmTrapDestIpAddress=slmTrapDestIpAddress, slmVcatDelay=slmVcatDelay, slmChPassTable=slmChPassTable, slmSysSubnetMask=slmSysSubnetMask, slmRstpEnabled=slmRstpEnabled, slmSysLanIpAddr=slmSysLanIpAddr, slmTrapLogTable=slmTrapLogTable, slmTrapLogParam5=slmTrapLogParam5, slmChPassEntry=slmChPassEntry, slmConfigSysUptime=slmConfigSysUptime, slmLicenseTable=slmLicenseTable, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmSysTemperature=slmSysTemperature, SoftwareRevision=SoftwareRevision, UserCommunity=UserCommunity, slmTid=slmTid, slmConfigSysSignalType=slmConfigSysSignalType, slmTrapLogId=slmTrapLogId, slmTrapEnable=slmTrapEnable, slmTrapLogParam3=slmTrapLogParam3, slmConfigFile=slmConfigFile, slmSwRevEntry=slmSwRevEntry, slmAdminAccess=slmAdminAccess, slmSysTrapFormat=slmSysTrapFormat, slmAdminTable=slmAdminTable, slmLogFile=slmLogFile, slmSnmpv3Auth=slmSnmpv3Auth, slmTrapDestAddress=slmTrapDestAddress, slmOemType=slmOemType, slmChPassNewPass=slmChPassNewPass, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmPsuNumber=slmPsuNumber, slmTrapDestProtVersion=slmTrapDestProtVersion, slmAuthTable=slmAuthTable, slmAuthCommunity=slmAuthCommunity, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmNetworkMode=slmNetworkMode, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmSyslogDestAddress=slmSyslogDestAddress, slmTrapDestCommunity=slmTrapDestCommunity, slmSysIpAddr=slmSysIpAddr, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmSysAlmDeact=slmSysAlmDeact, slmSwRevStatus=slmSwRevStatus, slmTrapUserAccess=slmTrapUserAccess, slmAuthEntry=slmAuthEntry, slmSnmpv3Priv=slmSnmpv3Priv, slmTrapUserLogin=slmTrapUserLogin, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmChPassOldPass=slmChPassOldPass, slmTrapDestRowStatus=slmTrapDestRowStatus, UserLogin=UserLogin, slmAuthPassword=slmAuthPassword, slmChPass=slmChPass, slmTrapLogParam1=slmTrapLogParam1, slmTrapLogParam2=slmTrapLogParam2, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmSysMode=slmSysMode, slmSysSnmpVersion=slmSysSnmpVersion, slmSwRevDate=slmSwRevDate, slmChPassLogin=slmChPassLogin, slmTrapSysNameChange=slmTrapSysNameChange, slmSyslogDestEntry=slmSyslogDestEntry, slmSysCalendarTime=slmSysCalendarTime, slmLicenseId=slmLicenseId, slmSwRevName=slmSwRevName, slmSysAdminStatus=slmSysAdminStatus, slmPortsNumber=slmPortsNumber, slmLicense=slmLicense, slmTrapPort=slmTrapPort, slmSysNetworkPrefix=slmSysNetworkPrefix, slmSnmpv3Password=slmSnmpv3Password, slmSys=slmSys, slmMuxNumber=slmMuxNumber, slmReadCommunity=slmReadCommunity, slmSyslogDestTable=slmSyslogDestTable, slmTrapDestEntry=slmTrapDestEntry, slmLogin=slmLogin, slmSysGatewayAddr=slmSysGatewayAddr, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmTrapDestTable=slmTrapDestTable, slmEdfaNumber=slmEdfaNumber, slmSwRevTable=slmSwRevTable, slmTrapLogParam6=slmTrapLogParam6, slmSysUplinkRate=slmSysUplinkRate, slmSysLocation=slmSysLocation, slmLicenseEntry=slmLicenseEntry, slmSwRevDirectory=slmSwRevDirectory, slmSyslogPort=slmSyslogPort, slmAuth=slmAuth, slmAdminLogin=slmAdminLogin, slmTrap=slmTrap, AdminAccess=AdminAccess, slmAdminEntry=slmAdminEntry, slmSyslogLevel=slmSyslogLevel, slmTrapLogParam4=slmTrapLogParam4, slmSysActiveSwVersion=slmSysActiveSwVersion, slm40GConf=slm40GConf, slmPmAvailable=slmPmAvailable, slmAuthLogin=slmAuthLogin, slmLicenseExpiration=slmLicenseExpiration, slmTrapSysLocationChange=slmTrapSysLocationChange, slmSysPmStartOfDay=slmSysPmStartOfDay, slmSysResetPm=slmSysResetPm)
