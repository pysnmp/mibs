...
Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Opaque, Gauge32, Counter64, ObjectIdentity, iso, ModuleIdentity, MibIdentifier, IpAddress, NotificationType, Unsigned32, Bits, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Opaque", "Gauge32", "Counter64", "ObjectIdentity", "iso", "ModuleIdentity", "MibIdentifier", "IpAddress", "NotificationType", "Unsigned32", "Bits", "Integer32")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmTrapLogParam1=slmTrapLogParam1, slmConfigFile=slmConfigFile, slmTrapLogTable=slmTrapLogTable, UserPassword=UserPassword, slMain=slMain, slmTid=slmTid, slmSwRevDirectory=slmSwRevDirectory, slmSwRevDate=slmSwRevDate, slmSysNetworkPrefix=slmSysNetworkPrefix, slmTrapDestAddress=slmTrapDestAddress, slmTrapUserLogin=slmTrapUserLogin, slmSysEncryptionCapability=slmSysEncryptionCapability, slmSysName=slmSysName, slmSysAdminStatus=slmSysAdminStatus, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmTrapLogName=slmTrapLogName, slmSwRevName=slmSwRevName, slmChPassOldPass=slmChPassOldPass, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmSyslogPort=slmSyslogPort, slmVcatDelay=slmVcatDelay, slmSysActiveSwVersion=slmSysActiveSwVersion, slmBoxSize=slmBoxSize, slmTrapPort=slmTrapPort, slmReadCommunity=slmReadCommunity, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmLicenseId=slmLicenseId, slmSysTrapFormat=slmSysTrapFormat, slmSysAlmAct=slmSysAlmAct, slmRebootDelay=slmRebootDelay, slmOpticalSwitchExist=slmOpticalSwitchExist, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmSysAlmDeact=slmSysAlmDeact, slmPortsNumber=slmPortsNumber, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmConfigSysUptime=slmConfigSysUptime, slmSyslogDestEntry=slmSyslogDestEntry, slmSnmpv3Priv=slmSnmpv3Priv, slmSnmpv3Password=slmSnmpv3Password, slmTrap=slmTrap, slmLicenseEntry=slmLicenseEntry, slmAuthCommunity=slmAuthCommunity, slmChPass=slmChPass, slmTrapLogParam3=slmTrapLogParam3, slmTrapDestEntry=slmTrapDestEntry, slmEdfaNumber=slmEdfaNumber, slmSysMode=slmSysMode, UserCommunity=UserCommunity, slmChPassEntry=slmChPassEntry, slmTrapDestCommunity=slmTrapDestCommunity, slmLogin=slmLogin, slm40GConf=slm40GConf, slmSysLanIpAddr=slmSysLanIpAddr, slmSysCalendarTime=slmSysCalendarTime, slmSysIpAddr=slmSysIpAddr, PYSNMP_MODULE_ID=slMain, slmLogFile=slmLogFile, slmSysGatewayAddr=slmSysGatewayAddr, slmPsuNumber=slmPsuNumber, slmLicenseExpiration=slmLicenseExpiration, slmTopologyEnabled=slmTopologyEnabled, slmTrapLogParam4=slmTrapLogParam4, slmTrapEnable=slmTrapEnable, slmAdminEntry=slmAdminEntry, slmAdminLogin=slmAdminLogin, slmPmAvailable=slmPmAvailable, slmChPassTable=slmChPassTable, slmTrapLogParam6=slmTrapLogParam6, slmRstpEnabled=slmRstpEnabled, slmTrapDestIpAddress=slmTrapDestIpAddress, slmSysSnmpVersion=slmSysSnmpVersion, slmSyslogDestTable=slmSyslogDestTable, slmTrapUserAccess=slmTrapUserAccess, slmNetworkMode=slmNetworkMode, slmSysPmStartOfDay=slmSysPmStartOfDay, slmSysUplinkRate=slmSysUplinkRate, slmAuthEntry=slmAuthEntry, slmChPassNewPass=slmChPassNewPass, slmSysOperStatus=slmSysOperStatus, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmTrapCommunity=slmTrapCommunity, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmTrapLogParam2=slmTrapLogParam2, slmTrapDestTable=slmTrapDestTable, slmAdminTable=slmAdminTable, slmLicenseTable=slmLicenseTable, AdminAccess=AdminAccess, slmTrapLogParam5=slmTrapLogParam5, slmSwRevTable=slmSwRevTable, slmAdminAccess=slmAdminAccess, slmSysLocation=slmSysLocation, slmSysSubnetMask=slmSysSubnetMask, slmMuxNumber=slmMuxNumber, slmSwRevEntry=slmSwRevEntry, slmSyslogDestAddress=slmSyslogDestAddress, slmAuth=slmAuth, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmChPassLogin=slmChPassLogin, slmOemType=slmOemType, slmAdminRowStatus=slmAdminRowStatus, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmAuthPassword=slmAuthPassword, slmAdminPassword=slmAdminPassword, slmSysTemperature=slmSysTemperature, slmTrapDestProtVersion=slmTrapDestProtVersion, slmLicenseIndex=slmLicenseIndex, slmSnmpv3Auth=slmSnmpv3Auth, slmSwRevStatus=slmSwRevStatus, slmAdminCommunity=slmAdminCommunity, slmTrapSysNameChange=slmTrapSysNameChange, slmTrapSysLocationChange=slmTrapSysLocationChange, slmLicense=slmLicense, slmSyslogLevel=slmSyslogLevel, UserLogin=UserLogin, slmSys=slmSys, SoftwareRevision=SoftwareRevision, slmTrapLogId=slmTrapLogId, slmTrapLogEntry=slmTrapLogEntry, slmConfigSysSignalType=slmConfigSysSignalType, slmAdmin=slmAdmin, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmAuthTable=slmAuthTable, slmSysResetPm=slmSysResetPm, slmWriteCommunity=slmWriteCommunity, slmAuthLogin=slmAuthLogin, slmTrapDestRowStatus=slmTrapDestRowStatus, slmSysChassisId=slmSysChassisId)
