...
ModuleIdentity, Gauge32, ObjectIdentity, TimeTicks, Counter32, Integer32, NotificationType, iso, Unsigned32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, Opaque, IpAddress, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Gauge32", "ObjectIdentity", "TimeTicks", "Counter32", "Integer32", "NotificationType", "iso", "Unsigned32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "Opaque", "IpAddress", "Counter64")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmTid=slmTid, slmLicenseId=slmLicenseId, UserCommunity=UserCommunity, slmNetworkMode=slmNetworkMode, slmEdfaNumber=slmEdfaNumber, slmConfigSysUptime=slmConfigSysUptime, slmTrapLogTable=slmTrapLogTable, slmConfigSysSignalType=slmConfigSysSignalType, slmAdminRowStatus=slmAdminRowStatus, slmVcatDelay=slmVcatDelay, slmMuxNumber=slmMuxNumber, slmSysTrapFormat=slmSysTrapFormat, slmTrapDestProtVersion=slmTrapDestProtVersion, slmSysAdminStatus=slmSysAdminStatus, slmSysLocation=slmSysLocation, slmTrapPort=slmTrapPort, slmSyslogLevel=slmSyslogLevel, slmAdminPassword=slmAdminPassword, slmSysAlmDeact=slmSysAlmDeact, slmSysOperStatus=slmSysOperStatus, slmAdminEntry=slmAdminEntry, slmTrapLogParam6=slmTrapLogParam6, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmLicenseEntry=slmLicenseEntry, slmSwRevDate=slmSwRevDate, slmBoxSize=slmBoxSize, slmSysTemperature=slmSysTemperature, slmLogin=slmLogin, slmAuthTable=slmAuthTable, UserLogin=UserLogin, slmSwRevStatus=slmSwRevStatus, slmSysChassisId=slmSysChassisId, slmTrap=slmTrap, slmTrapDestCommunity=slmTrapDestCommunity, slmTrapEnable=slmTrapEnable, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmSysNetworkPrefix=slmSysNetworkPrefix, slmSyslogDestAddress=slmSyslogDestAddress, slmSys=slmSys, slmSnmpv3Password=slmSnmpv3Password, slmSyslogDestEntry=slmSyslogDestEntry, slmChPassEntry=slmChPassEntry, slmTrapUserAccess=slmTrapUserAccess, slmLicenseIndex=slmLicenseIndex, slmTrapDestTable=slmTrapDestTable, slmSysName=slmSysName, slmChPassOldPass=slmChPassOldPass, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmSysUplinkRate=slmSysUplinkRate, slmTrapSysLocationChange=slmTrapSysLocationChange, slmTrapDestIpAddress=slmTrapDestIpAddress, slmAuth=slmAuth, slmTrapDestEntry=slmTrapDestEntry, SoftwareRevision=SoftwareRevision, slmSysEncryptionCapability=slmSysEncryptionCapability, slmAdmin=slmAdmin, slmRstpEnabled=slmRstpEnabled, slmSwRevDirectory=slmSwRevDirectory, slmSysIpAddr=slmSysIpAddr, slmSyslogDestTable=slmSyslogDestTable, slmAuthLogin=slmAuthLogin, slmSysSubnetMask=slmSysSubnetMask, slmTrapLogParam4=slmTrapLogParam4, slmSwRevName=slmSwRevName, slmLicenseExpiration=slmLicenseExpiration, slmLogFile=slmLogFile, AdminAccess=AdminAccess, slmTrapDestRowStatus=slmTrapDestRowStatus, slmTrapUserLogin=slmTrapUserLogin, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmChPassLogin=slmChPassLogin, slmSysLanIpAddr=slmSysLanIpAddr, slmTrapLogParam1=slmTrapLogParam1, slmLicense=slmLicense, slmTrapLogId=slmTrapLogId, slmTrapLogParam5=slmTrapLogParam5, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slm40GConf=slm40GConf, slmRebootDelay=slmRebootDelay, slmSysSnmpVersion=slmSysSnmpVersion, slmSnmpv3Auth=slmSnmpv3Auth, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmSysActiveSwVersion=slmSysActiveSwVersion, slmTrapSysNameChange=slmTrapSysNameChange, slmTrapDestAddress=slmTrapDestAddress, slmAdminTable=slmAdminTable, slmSysMode=slmSysMode, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmTrapLogName=slmTrapLogName, slmTrapCommunity=slmTrapCommunity, slmTrapLogParam2=slmTrapLogParam2, slmSyslogPort=slmSyslogPort, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmSysGatewayAddr=slmSysGatewayAddr, slmChPass=slmChPass, slmOpticalSwitchExist=slmOpticalSwitchExist, slmPsuNumber=slmPsuNumber, slmSysCalendarTime=slmSysCalendarTime, slmSnmpv3Priv=slmSnmpv3Priv, slMain=slMain, slmSysAlmAct=slmSysAlmAct, slmSwRevTable=slmSwRevTable, slmSyslogDestIpAddress=slmSyslogDestIpAddress, PYSNMP_MODULE_ID=slMain, slmSysPmStartOfDay=slmSysPmStartOfDay, slmConfigFile=slmConfigFile, slmAuthCommunity=slmAuthCommunity, slmTrapLogEntry=slmTrapLogEntry, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmWriteCommunity=slmWriteCommunity, slmOemType=slmOemType, slmTopologyEnabled=slmTopologyEnabled, slmTrapLogParam3=slmTrapLogParam3, slmChPassNewPass=slmChPassNewPass, slmAdminCommunity=slmAdminCommunity, slmAuthPassword=slmAuthPassword, slmReadCommunity=slmReadCommunity, slmAdminAccess=slmAdminAccess, UserPassword=UserPassword, slmLicenseTable=slmLicenseTable, slmSwRevEntry=slmSwRevEntry, slmChPassTable=slmChPassTable, slmPortsNumber=slmPortsNumber, slmSysResetPm=slmSysResetPm, slmAuthEntry=slmAuthEntry, slmPmAvailable=slmPmAvailable, slmAdminLogin=slmAdminLogin)
