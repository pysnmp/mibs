...
Integer32, Counter64, IpAddress, Bits, NotificationType, iso, TimeTicks, Counter32, Opaque, ObjectIdentity, Unsigned32, MibIdentifier, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Counter64", "IpAddress", "Bits", "NotificationType", "iso", "TimeTicks", "Counter32", "Opaque", "ObjectIdentity", "Unsigned32", "MibIdentifier", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmSysChassisId=slmSysChassisId, slmSysLanIpAddr=slmSysLanIpAddr, slmLicenseTable=slmLicenseTable, slmSysResetPm=slmSysResetPm, slmLogFile=slmLogFile, slmSysNetworkPrefix=slmSysNetworkPrefix, slmConfigSysUptime=slmConfigSysUptime, slmTrap=slmTrap, slmAdminAccess=slmAdminAccess, PYSNMP_MODULE_ID=slMain, slmSysName=slmSysName, slmLogin=slmLogin, slmAuthLogin=slmAuthLogin, slmSysAdminStatus=slmSysAdminStatus, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmPortsNumber=slmPortsNumber, slmTrapLogParam6=slmTrapLogParam6, UserLogin=UserLogin, slmTrapDestAddress=slmTrapDestAddress, slmTrapLogParam5=slmTrapLogParam5, slmSysCalendarTime=slmSysCalendarTime, slmSnmpv3Password=slmSnmpv3Password, slmChPassEntry=slmChPassEntry, slmPsuNumber=slmPsuNumber, slmTrapDestTable=slmTrapDestTable, slmConfigSysSignalType=slmConfigSysSignalType, slmOemType=slmOemType, slmSwRevTable=slmSwRevTable, slmAuthCommunity=slmAuthCommunity, slmTrapLogParam3=slmTrapLogParam3, UserCommunity=UserCommunity, slmMuxNumber=slmMuxNumber, slmSysSubnetMask=slmSysSubnetMask, slmSysTemperature=slmSysTemperature, slmSysSnmpVersion=slmSysSnmpVersion, slmAdminTable=slmAdminTable, slmSysEncryptionCapability=slmSysEncryptionCapability, slmTrapLogName=slmTrapLogName, slmSys=slmSys, slmTrapEnable=slmTrapEnable, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmSwRevDate=slmSwRevDate, slmAuth=slmAuth, slmSwRevDirectory=slmSwRevDirectory, slmSysTrapFormat=slmSysTrapFormat, slmSyslogLevel=slmSyslogLevel, slmAdmin=slmAdmin, slmTrapDestCommunity=slmTrapDestCommunity, slmTrapLogEntry=slmTrapLogEntry, slmSyslogDestEntry=slmSyslogDestEntry, slmAdminRowStatus=slmAdminRowStatus, slmLicenseId=slmLicenseId, slmSysLocation=slmSysLocation, slmTrapLogParam1=slmTrapLogParam1, slmAdminEntry=slmAdminEntry, slmSysActiveSwVersion=slmSysActiveSwVersion, slmPmAvailable=slmPmAvailable, slmSwRevStatus=slmSwRevStatus, slmChPass=slmChPass, slmReadCommunity=slmReadCommunity, slmTrapLogParam2=slmTrapLogParam2, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmTrapUserAccess=slmTrapUserAccess, slmSysGatewayAddr=slmSysGatewayAddr, UserPassword=UserPassword, slmSwRevName=slmSwRevName, slmRebootDelay=slmRebootDelay, slmSysIpAddr=slmSysIpAddr, slmTrapLogTable=slmTrapLogTable, slmSysOperStatus=slmSysOperStatus, slmTid=slmTid, slmSnmpv3Priv=slmSnmpv3Priv, slmTrapLogId=slmTrapLogId, slmAuthPassword=slmAuthPassword, slmNetworkMode=slmNetworkMode, slmTrapLogParam4=slmTrapLogParam4, slmTrapPort=slmTrapPort, slmTrapSysNameChange=slmTrapSysNameChange, slmLicenseExpiration=slmLicenseExpiration, slmTrapUserLogin=slmTrapUserLogin, slmWriteCommunity=slmWriteCommunity, slmSysMode=slmSysMode, slmChPassNewPass=slmChPassNewPass, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmLicense=slmLicense, slmRstpEnabled=slmRstpEnabled, slmEdfaNumber=slmEdfaNumber, SoftwareRevision=SoftwareRevision, slmVcatDelay=slmVcatDelay, slmSysPmStartOfDay=slmSysPmStartOfDay, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmAdminCommunity=slmAdminCommunity, slmSyslogDestTable=slmSyslogDestTable, slmTrapDestProtVersion=slmTrapDestProtVersion, slmTrapDestRowStatus=slmTrapDestRowStatus, slmSysUplinkRate=slmSysUplinkRate, slmSysAlmDeact=slmSysAlmDeact, slmChPassOldPass=slmChPassOldPass, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slm40GConf=slm40GConf, slmAdminLogin=slmAdminLogin, slmTrapDestEntry=slmTrapDestEntry, slmAuthTable=slmAuthTable, slmTrapDestIpAddress=slmTrapDestIpAddress, slmTrapSysLocationChange=slmTrapSysLocationChange, slmSyslogPort=slmSyslogPort, slmChPassTable=slmChPassTable, slmTopologyEnabled=slmTopologyEnabled, slMain=slMain, slmLicenseIndex=slmLicenseIndex, slmChPassLogin=slmChPassLogin, slmTrapCommunity=slmTrapCommunity, slmSnmpv3Auth=slmSnmpv3Auth, slmBoxSize=slmBoxSize, slmLicenseEntry=slmLicenseEntry, slmSyslogDestAddress=slmSyslogDestAddress, AdminAccess=AdminAccess, slmAuthEntry=slmAuthEntry, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmSysAlmAct=slmSysAlmAct, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmOpticalSwitchExist=slmOpticalSwitchExist, slmConfigFile=slmConfigFile, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmSwRevEntry=slmSwRevEntry, slmAdminPassword=slmAdminPassword)
