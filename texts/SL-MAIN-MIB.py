...
Opaque, ObjectIdentity, iso, Counter32, TimeTicks, MibIdentifier, IpAddress, Gauge32, Integer32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Bits, Counter64, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "Opaque", "ObjectIdentity", "iso", "Counter32", "TimeTicks", "MibIdentifier", "IpAddress", "Gauge32", "Integer32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Bits", "Counter64", "Unsigned32")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmSnmpv3Auth=slmSnmpv3Auth, slmConfigSysSignalType=slmConfigSysSignalType, slmLicense=slmLicense, slmSwRevTable=slmSwRevTable, UserPassword=UserPassword, slmSysAlmAct=slmSysAlmAct, slmSwRevName=slmSwRevName, slmAuthLogin=slmAuthLogin, slmEdfaNumber=slmEdfaNumber, slmSwRevStatus=slmSwRevStatus, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmTrapLogParam3=slmTrapLogParam3, slmTrap=slmTrap, slmSnmpv3Password=slmSnmpv3Password, slmTrapLogTable=slmTrapLogTable, UserCommunity=UserCommunity, slmLicenseExpiration=slmLicenseExpiration, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmSwRevDirectory=slmSwRevDirectory, slmTrapLogId=slmTrapLogId, slmTrapCommunity=slmTrapCommunity, slmTid=slmTid, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmTrapLogParam4=slmTrapLogParam4, slmSysAdminStatus=slmSysAdminStatus, slmChPassTable=slmChPassTable, slmAuthCommunity=slmAuthCommunity, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmTrapPort=slmTrapPort, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmTrapSysLocationChange=slmTrapSysLocationChange, slmChPassNewPass=slmChPassNewPass, slmSyslogLevel=slmSyslogLevel, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmRstpEnabled=slmRstpEnabled, slmChPassEntry=slmChPassEntry, slmSysTrapFormat=slmSysTrapFormat, slMain=slMain, slmTrapDestProtVersion=slmTrapDestProtVersion, slmVcatDelay=slmVcatDelay, slmSyslogDestEntry=slmSyslogDestEntry, slmChPass=slmChPass, slmSyslogDestTable=slmSyslogDestTable, slmAdminLogin=slmAdminLogin, slmPsuNumber=slmPsuNumber, slmTrapUserLogin=slmTrapUserLogin, slmMuxNumber=slmMuxNumber, slmAuthPassword=slmAuthPassword, slmConfigSysUptime=slmConfigSysUptime, slmSysNetworkPrefix=slmSysNetworkPrefix, slmChPassOldPass=slmChPassOldPass, slmSysName=slmSysName, slmBoxSize=slmBoxSize, slmSysSnmpVersion=slmSysSnmpVersion, slm40GConf=slm40GConf, slmAdminCommunity=slmAdminCommunity, slmTrapSysNameChange=slmTrapSysNameChange, slmSysOperStatus=slmSysOperStatus, slmTopologyEnabled=slmTopologyEnabled, slmTrapDestCommunity=slmTrapDestCommunity, UserLogin=UserLogin, slmSysMode=slmSysMode, slmAdminAccess=slmAdminAccess, slmSysLocation=slmSysLocation, slmLogin=slmLogin, slmWriteCommunity=slmWriteCommunity, slmLicenseIndex=slmLicenseIndex, slmTrapDestTable=slmTrapDestTable, slmConfigFile=slmConfigFile, slmSysAlmDeact=slmSysAlmDeact, slmReadCommunity=slmReadCommunity, slmOpticalSwitchExist=slmOpticalSwitchExist, slmTrapDestIpAddress=slmTrapDestIpAddress, slmTrapLogParam5=slmTrapLogParam5, slmAdmin=slmAdmin, slmAuthTable=slmAuthTable, slmTrapLogParam1=slmTrapLogParam1, slmSysSubnetMask=slmSysSubnetMask, slmAdminEntry=slmAdminEntry, slmSwRevEntry=slmSwRevEntry, slmTrapLogParam2=slmTrapLogParam2, slmSys=slmSys, slmLicenseEntry=slmLicenseEntry, slmTrapLogEntry=slmTrapLogEntry, slmTrapEnable=slmTrapEnable, slmAdminPassword=slmAdminPassword, slmTrapLogName=slmTrapLogName, slmAdminRowStatus=slmAdminRowStatus, slmSysUplinkRate=slmSysUplinkRate, slmSysChassisId=slmSysChassisId, slmSyslogDestAddress=slmSyslogDestAddress, slmSysCalendarTime=slmSysCalendarTime, slmSnmpv3Priv=slmSnmpv3Priv, slmAuthEntry=slmAuthEntry, slmSysIpAddr=slmSysIpAddr, slmSysActiveSwVersion=slmSysActiveSwVersion, SoftwareRevision=SoftwareRevision, slmNetworkMode=slmNetworkMode, slmRebootDelay=slmRebootDelay, slmSysEncryptionCapability=slmSysEncryptionCapability, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmSysGatewayAddr=slmSysGatewayAddr, slmTrapDestRowStatus=slmTrapDestRowStatus, slmSyslogPort=slmSyslogPort, slmTrapUserAccess=slmTrapUserAccess, AdminAccess=AdminAccess, slmAdminTable=slmAdminTable, slmSysTemperature=slmSysTemperature, slmLicenseTable=slmLicenseTable, slmSwRevDate=slmSwRevDate, slmSysLanIpAddr=slmSysLanIpAddr, slmPmAvailable=slmPmAvailable, slmOemType=slmOemType, PYSNMP_MODULE_ID=slMain, slmTrapDestAddress=slmTrapDestAddress, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmPortsNumber=slmPortsNumber, slmSysPmStartOfDay=slmSysPmStartOfDay, slmLogFile=slmLogFile, slmChPassLogin=slmChPassLogin, slmLicenseId=slmLicenseId, slmTrapDestEntry=slmTrapDestEntry, slmTrapLogParam6=slmTrapLogParam6, slmSysResetPm=slmSysResetPm, slmAuth=slmAuth)
