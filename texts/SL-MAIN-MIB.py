...
TimeTicks, NotificationType, MibIdentifier, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, ObjectIdentity, Bits, IpAddress, Counter32, Opaque, ModuleIdentity, Gauge32, Integer32, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "NotificationType", "MibIdentifier", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "ObjectIdentity", "Bits", "IpAddress", "Counter32", "Opaque", "ModuleIdentity", "Gauge32", "Integer32", "Unsigned32")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmAuth=slmAuth, slmLicenseTable=slmLicenseTable, slmTrapDestRowStatus=slmTrapDestRowStatus, slmTrapDestTable=slmTrapDestTable, slmAdminPassword=slmAdminPassword, slmSysNetworkPrefix=slmSysNetworkPrefix, slmAdminCommunity=slmAdminCommunity, slmSwRevStatus=slmSwRevStatus, slmTrapUserAccess=slmTrapUserAccess, slmPmAvailable=slmPmAvailable, slmPortsNumber=slmPortsNumber, slmTrapLogParam3=slmTrapLogParam3, slmNetworkMode=slmNetworkMode, slmTopologyEnabled=slmTopologyEnabled, slmSwRevName=slmSwRevName, slmChPassTable=slmChPassTable, slmAdmin=slmAdmin, slmSnmpv3Auth=slmSnmpv3Auth, slmReadCommunity=slmReadCommunity, slmAdminEntry=slmAdminEntry, slmSysTemperature=slmSysTemperature, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmSyslogDestAddress=slmSyslogDestAddress, slmConfigFile=slmConfigFile, slmChPassOldPass=slmChPassOldPass, slmTid=slmTid, slmRebootDelay=slmRebootDelay, slmSysAdminStatus=slmSysAdminStatus, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, SoftwareRevision=SoftwareRevision, slmSysLanIpAddr=slmSysLanIpAddr, slmSysOperStatus=slmSysOperStatus, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmTrapDestAddress=slmTrapDestAddress, slmChPass=slmChPass, slmSwRevDate=slmSwRevDate, slmTrapDestCommunity=slmTrapDestCommunity, slmSys=slmSys, slmSysResetPm=slmSysResetPm, slmAuthPassword=slmAuthPassword, slmTrapSysNameChange=slmTrapSysNameChange, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmSyslogDestTable=slmSyslogDestTable, UserCommunity=UserCommunity, slmSysAlmAct=slmSysAlmAct, slmLogin=slmLogin, slmTrapPort=slmTrapPort, AdminAccess=AdminAccess, slmTrapDestIpAddress=slmTrapDestIpAddress, slmSysActiveSwVersion=slmSysActiveSwVersion, slmTrapLogParam6=slmTrapLogParam6, slmTrap=slmTrap, slmSysMode=slmSysMode, slmAuthCommunity=slmAuthCommunity, slmSysIpAddr=slmSysIpAddr, slmSysCalendarTime=slmSysCalendarTime, slmTrapSysLocationChange=slmTrapSysLocationChange, slmPsuNumber=slmPsuNumber, slmSysAlmDeact=slmSysAlmDeact, slmLicenseEntry=slmLicenseEntry, slmSysPmStartOfDay=slmSysPmStartOfDay, UserPassword=UserPassword, slmAuthLogin=slmAuthLogin, slmChPassNewPass=slmChPassNewPass, slm40GConf=slm40GConf, slmSysChassisId=slmSysChassisId, slmMuxNumber=slmMuxNumber, slmAdminRowStatus=slmAdminRowStatus, slmSysTrapFormat=slmSysTrapFormat, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmAuthEntry=slmAuthEntry, slmLogFile=slmLogFile, slmChPassLogin=slmChPassLogin, PYSNMP_MODULE_ID=slMain, slmSysSnmpVersion=slmSysSnmpVersion, slMain=slMain, UserLogin=UserLogin, slmLicenseIndex=slmLicenseIndex, slmSwRevDirectory=slmSwRevDirectory, slmTrapEnable=slmTrapEnable, slmBoxSize=slmBoxSize, slmOpticalSwitchExist=slmOpticalSwitchExist, slmSnmpv3Password=slmSnmpv3Password, slmWriteCommunity=slmWriteCommunity, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmChPassEntry=slmChPassEntry, slmAdminLogin=slmAdminLogin, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmSnmpv3Priv=slmSnmpv3Priv, slmAuthTable=slmAuthTable, slmTrapDestEntry=slmTrapDestEntry, slmOemType=slmOemType, slmTrapDestProtVersion=slmTrapDestProtVersion, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmVcatDelay=slmVcatDelay, slmSysUplinkRate=slmSysUplinkRate, slmRstpEnabled=slmRstpEnabled, slmConfigSysUptime=slmConfigSysUptime, slmTrapLogParam4=slmTrapLogParam4, slmSyslogPort=slmSyslogPort, slmTrapCommunity=slmTrapCommunity, slmTrapUserLogin=slmTrapUserLogin, slmConfigSysSignalType=slmConfigSysSignalType, slmSysLocation=slmSysLocation, slmTrapLogEntry=slmTrapLogEntry, slmTrapLogId=slmTrapLogId, slmTrapLogParam1=slmTrapLogParam1, slmSysEncryptionCapability=slmSysEncryptionCapability, slmSysGatewayAddr=slmSysGatewayAddr, slmAdminTable=slmAdminTable, slmSysName=slmSysName, slmTrapLogTable=slmTrapLogTable, slmTrapLogName=slmTrapLogName, slmTrapLogParam5=slmTrapLogParam5, slmSyslogLevel=slmSyslogLevel, slmSwRevTable=slmSwRevTable, slmAdminAccess=slmAdminAccess, slmEdfaNumber=slmEdfaNumber, slmLicenseId=slmLicenseId, slmSysSubnetMask=slmSysSubnetMask, slmSwRevEntry=slmSwRevEntry, slmLicense=slmLicense, slmLicenseExpiration=slmLicenseExpiration, slmSyslogDestEntry=slmSyslogDestEntry, slmTrapLogParam2=slmTrapLogParam2)
