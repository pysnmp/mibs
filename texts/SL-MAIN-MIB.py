...
iso, Counter32, MibIdentifier, Bits, Unsigned32, Gauge32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Opaque, Counter64, ObjectIdentity, IpAddress, TimeTicks, Integer32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Counter32", "MibIdentifier", "Bits", "Unsigned32", "Gauge32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Opaque", "Counter64", "ObjectIdentity", "IpAddress", "TimeTicks", "Integer32", "NotificationType")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmTrapUserAccess=slmTrapUserAccess, slmTrapSysNameChange=slmTrapSysNameChange, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmSyslogLevel=slmSyslogLevel, slmEdfaNumber=slmEdfaNumber, slmSysLocation=slmSysLocation, slmSysCalendarTime=slmSysCalendarTime, UserPassword=UserPassword, slmSys=slmSys, slmSwRevStatus=slmSwRevStatus, slmTrapDestIpAddress=slmTrapDestIpAddress, UserCommunity=UserCommunity, slMain=slMain, slmSwRevDirectory=slmSwRevDirectory, slmSyslogDestAddress=slmSyslogDestAddress, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmLogFile=slmLogFile, slmSysAdminStatus=slmSysAdminStatus, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmRstpEnabled=slmRstpEnabled, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, PYSNMP_MODULE_ID=slMain, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmSwRevDate=slmSwRevDate, slmChPassEntry=slmChPassEntry, slmSysAlmDeact=slmSysAlmDeact, slmAuthEntry=slmAuthEntry, slmPortsNumber=slmPortsNumber, slmTrapUserLogin=slmTrapUserLogin, slmSysPmStartOfDay=slmSysPmStartOfDay, slmBoxSize=slmBoxSize, slmTrapLogParam6=slmTrapLogParam6, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmSysGatewayAddr=slmSysGatewayAddr, slmSysResetPm=slmSysResetPm, slmLicenseIndex=slmLicenseIndex, slmSysIpAddr=slmSysIpAddr, slmSwRevTable=slmSwRevTable, slmTrapEnable=slmTrapEnable, slmSyslogDestEntry=slmSyslogDestEntry, slmSysNetworkPrefix=slmSysNetworkPrefix, slmTrapLogParam4=slmTrapLogParam4, slmSwRevEntry=slmSwRevEntry, slmConfigFile=slmConfigFile, slmAuthPassword=slmAuthPassword, slmAdmin=slmAdmin, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmOemType=slmOemType, slmNetworkMode=slmNetworkMode, slmLicenseTable=slmLicenseTable, slmSysName=slmSysName, slmAdminPassword=slmAdminPassword, slmSysActiveSwVersion=slmSysActiveSwVersion, slmAuthLogin=slmAuthLogin, slmTrapSysLocationChange=slmTrapSysLocationChange, slmLicenseId=slmLicenseId, slmSyslogDestTable=slmSyslogDestTable, slmTrap=slmTrap, SoftwareRevision=SoftwareRevision, slmTrapPort=slmTrapPort, slmLicenseEntry=slmLicenseEntry, slmTid=slmTid, slmRebootDelay=slmRebootDelay, slmSysMode=slmSysMode, slmChPassTable=slmChPassTable, slmSnmpv3Auth=slmSnmpv3Auth, slmTrapDestAddress=slmTrapDestAddress, AdminAccess=AdminAccess, slm40GConf=slm40GConf, slmPsuNumber=slmPsuNumber, slmTrapDestRowStatus=slmTrapDestRowStatus, slmTrapDestEntry=slmTrapDestEntry, slmSyslogPort=slmSyslogPort, slmSysOperStatus=slmSysOperStatus, slmChPassNewPass=slmChPassNewPass, slmAdminTable=slmAdminTable, slmSysSubnetMask=slmSysSubnetMask, slmLogin=slmLogin, slmSysTemperature=slmSysTemperature, slmOpticalSwitchExist=slmOpticalSwitchExist, slmWriteCommunity=slmWriteCommunity, slmAdminCommunity=slmAdminCommunity, slmChPassLogin=slmChPassLogin, slmSysUplinkRate=slmSysUplinkRate, slmSysAlmAct=slmSysAlmAct, slmTopologyEnabled=slmTopologyEnabled, slmTrapLogParam5=slmTrapLogParam5, slmTrapCommunity=slmTrapCommunity, slmMuxNumber=slmMuxNumber, slmSysLanIpAddr=slmSysLanIpAddr, slmTrapLogParam3=slmTrapLogParam3, slmPmAvailable=slmPmAvailable, slmTrapDestCommunity=slmTrapDestCommunity, slmSnmpv3Priv=slmSnmpv3Priv, slmConfigSysUptime=slmConfigSysUptime, slmVcatDelay=slmVcatDelay, slmSysTrapFormat=slmSysTrapFormat, slmSysSnmpVersion=slmSysSnmpVersion, slmAuthTable=slmAuthTable, slmTrapLogTable=slmTrapLogTable, slmSnmpv3Password=slmSnmpv3Password, slmAdminRowStatus=slmAdminRowStatus, slmConfigSysSignalType=slmConfigSysSignalType, slmAdminEntry=slmAdminEntry, slmTrapLogId=slmTrapLogId, slmTrapDestProtVersion=slmTrapDestProtVersion, slmAdminAccess=slmAdminAccess, slmLicenseExpiration=slmLicenseExpiration, slmTrapDestTable=slmTrapDestTable, slmAuth=slmAuth, slmSysChassisId=slmSysChassisId, slmTrapLogName=slmTrapLogName, slmTrapLogEntry=slmTrapLogEntry, slmSwRevName=slmSwRevName, slmReadCommunity=slmReadCommunity, slmChPassOldPass=slmChPassOldPass, slmLicense=slmLicense, slmSyslogDestRowStatus=slmSyslogDestRowStatus, UserLogin=UserLogin, slmAdminLogin=slmAdminLogin, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmAuthCommunity=slmAuthCommunity, slmChPass=slmChPass, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmTrapLogParam2=slmTrapLogParam2, slmTrapLogParam1=slmTrapLogParam1, slmSysEncryptionCapability=slmSysEncryptionCapability)
