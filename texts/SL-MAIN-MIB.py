...
iso, TimeTicks, Integer32, Counter32, Unsigned32, Gauge32, ObjectIdentity, NotificationType, IpAddress, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Opaque, Bits, MibIdentifier, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "TimeTicks", "Integer32", "Counter32", "Unsigned32", "Gauge32", "ObjectIdentity", "NotificationType", "IpAddress", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Opaque", "Bits", "MibIdentifier", "Counter64")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmAdminPassword=slmAdminPassword, slmAuthTable=slmAuthTable, slMain=slMain, slmRebootDelay=slmRebootDelay, slmAdminAccess=slmAdminAccess, slmAdminCommunity=slmAdminCommunity, slmTrapDestAddress=slmTrapDestAddress, slmSnmpv3Password=slmSnmpv3Password, slmSyslogDestEntry=slmSyslogDestEntry, PYSNMP_MODULE_ID=slMain, slmSysLocation=slmSysLocation, slmTrapPort=slmTrapPort, slmChPass=slmChPass, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmOemType=slmOemType, slmPortsNumber=slmPortsNumber, slmSysTrapFormat=slmSysTrapFormat, slmLicenseId=slmLicenseId, slmAdminLogin=slmAdminLogin, slmBoxSize=slmBoxSize, slmSyslogDestTable=slmSyslogDestTable, slmChPassNewPass=slmChPassNewPass, slmAuthEntry=slmAuthEntry, slmTrapDestCommunity=slmTrapDestCommunity, slmSnmpv3Priv=slmSnmpv3Priv, slmLogin=slmLogin, slmSysLanIpAddr=slmSysLanIpAddr, slmConfigSysUptime=slmConfigSysUptime, slmSysGatewayAddr=slmSysGatewayAddr, UserPassword=UserPassword, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmAuthCommunity=slmAuthCommunity, slmWriteCommunity=slmWriteCommunity, slmSwRevEntry=slmSwRevEntry, slmSwRevTable=slmSwRevTable, slmTrapDestRowStatus=slmTrapDestRowStatus, slmSnmpv3Auth=slmSnmpv3Auth, slmTid=slmTid, slmTrapLogParam5=slmTrapLogParam5, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmAdminEntry=slmAdminEntry, slmChPassLogin=slmChPassLogin, slmLicenseIndex=slmLicenseIndex, slmLicenseExpiration=slmLicenseExpiration, slmTrapLogParam1=slmTrapLogParam1, slmVcatDelay=slmVcatDelay, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmSwRevDirectory=slmSwRevDirectory, slmAdminTable=slmAdminTable, slmChPassOldPass=slmChPassOldPass, slmTrapUserLogin=slmTrapUserLogin, slmLicenseEntry=slmLicenseEntry, slmSwRevDate=slmSwRevDate, slmSysIpAddr=slmSysIpAddr, slmTrapLogParam4=slmTrapLogParam4, slmSysResetPm=slmSysResetPm, slmAuthLogin=slmAuthLogin, slmChPassTable=slmChPassTable, slmTrapUserAccess=slmTrapUserAccess, slmTrapDestProtVersion=slmTrapDestProtVersion, slmSysUplinkRate=slmSysUplinkRate, slmPmAvailable=slmPmAvailable, slmTrapCommunity=slmTrapCommunity, slmSysSubnetMask=slmSysSubnetMask, slmSyslogDestAddress=slmSyslogDestAddress, slmAuthPassword=slmAuthPassword, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmSysNetworkPrefix=slmSysNetworkPrefix, slmSysName=slmSysName, slmEdfaNumber=slmEdfaNumber, slmSysPmStartOfDay=slmSysPmStartOfDay, UserCommunity=UserCommunity, slmAuth=slmAuth, slmSysOperStatus=slmSysOperStatus, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmLogFile=slmLogFile, slmSysCalendarTime=slmSysCalendarTime, slmNetworkMode=slmNetworkMode, slmTrapLogTable=slmTrapLogTable, slmAdmin=slmAdmin, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmSys=slmSys, slmTrapDestIpAddress=slmTrapDestIpAddress, slmSysAdminStatus=slmSysAdminStatus, slmTrapSysNameChange=slmTrapSysNameChange, AdminAccess=AdminAccess, slmSysActiveSwVersion=slmSysActiveSwVersion, slmTrapLogName=slmTrapLogName, slmTopologyEnabled=slmTopologyEnabled, slmSyslogPort=slmSyslogPort, slmOpticalSwitchExist=slmOpticalSwitchExist, slmSysAlmAct=slmSysAlmAct, slmLicense=slmLicense, slmChPassEntry=slmChPassEntry, slmTrapSysLocationChange=slmTrapSysLocationChange, SoftwareRevision=SoftwareRevision, slmConfigFile=slmConfigFile, slmMuxNumber=slmMuxNumber, slmSysEncryptionCapability=slmSysEncryptionCapability, slm40GConf=slm40GConf, slmSysChassisId=slmSysChassisId, slmSyslogLevel=slmSyslogLevel, UserLogin=UserLogin, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmSysAlmDeact=slmSysAlmDeact, slmConfigSysSignalType=slmConfigSysSignalType, slmTrapDestEntry=slmTrapDestEntry, slmTrapLogParam2=slmTrapLogParam2, slmSwRevStatus=slmSwRevStatus, slmSysTemperature=slmSysTemperature, slmAdminRowStatus=slmAdminRowStatus, slmSysMode=slmSysMode, slmPsuNumber=slmPsuNumber, slmTrapLogEntry=slmTrapLogEntry, slmTrapEnable=slmTrapEnable, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmTrapDestTable=slmTrapDestTable, slmSysSnmpVersion=slmSysSnmpVersion, slmTrapLogId=slmTrapLogId, slmTrapLogParam3=slmTrapLogParam3, slmSwRevName=slmSwRevName, slmLicenseTable=slmLicenseTable, slmTrapLogParam6=slmTrapLogParam6, slmReadCommunity=slmReadCommunity, slmTrap=slmTrap, slmRstpEnabled=slmRstpEnabled)
