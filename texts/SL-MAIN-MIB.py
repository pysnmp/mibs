...
Bits, MibIdentifier, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, ObjectIdentity, NotificationType, IpAddress, Integer32, Counter64, Unsigned32, Opaque, TimeTicks, ModuleIdentity, iso = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "MibIdentifier", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "ObjectIdentity", "NotificationType", "IpAddress", "Integer32", "Counter64", "Unsigned32", "Opaque", "TimeTicks", "ModuleIdentity", "iso")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmSysCalendarTime=slmSysCalendarTime, slmRstpEnabled=slmRstpEnabled, slmAuthPassword=slmAuthPassword, slmLogin=slmLogin, slmChPassTable=slmChPassTable, slmBoxSize=slmBoxSize, slmChPassLogin=slmChPassLogin, slmTrapLogParam4=slmTrapLogParam4, UserCommunity=UserCommunity, slmNetworkMode=slmNetworkMode, slmTrapDestAddress=slmTrapDestAddress, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmChPassEntry=slmChPassEntry, slmSysTemperature=slmSysTemperature, slmChPassNewPass=slmChPassNewPass, slmSysLanIpAddr=slmSysLanIpAddr, slmChPass=slmChPass, slmSwRevDate=slmSwRevDate, slmSys=slmSys, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmSysSubnetMask=slmSysSubnetMask, slmSwRevEntry=slmSwRevEntry, slmSyslogDestAddress=slmSyslogDestAddress, slmSysActiveSwVersion=slmSysActiveSwVersion, slmAdminCommunity=slmAdminCommunity, slmSysLocation=slmSysLocation, slmLogFile=slmLogFile, slmSysAlmDeact=slmSysAlmDeact, slmLicenseExpiration=slmLicenseExpiration, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmConfigSysSignalType=slmConfigSysSignalType, slmOemType=slmOemType, slmAdminPassword=slmAdminPassword, slmSwRevTable=slmSwRevTable, slmSysSnmpVersion=slmSysSnmpVersion, slmAuthTable=slmAuthTable, slmTrapDestProtVersion=slmTrapDestProtVersion, slmTrapLogParam6=slmTrapLogParam6, slmSyslogDestEntry=slmSyslogDestEntry, slmMuxNumber=slmMuxNumber, slmTrapLogParam5=slmTrapLogParam5, slmConfigFile=slmConfigFile, slmAdminTable=slmAdminTable, slmLicense=slmLicense, slmTrapLogName=slmTrapLogName, UserLogin=UserLogin, slmConfigSysUptime=slmConfigSysUptime, slmSwRevDirectory=slmSwRevDirectory, slmTid=slmTid, slmChPassOldPass=slmChPassOldPass, slmSnmpv3Priv=slmSnmpv3Priv, slmTrapCommunity=slmTrapCommunity, slmSnmpv3Auth=slmSnmpv3Auth, slmSyslogDestTable=slmSyslogDestTable, slmSnmpv3Password=slmSnmpv3Password, slmSysPmStartOfDay=slmSysPmStartOfDay, slmTrapLogParam2=slmTrapLogParam2, slmTrapDestTable=slmTrapDestTable, slmAdminRowStatus=slmAdminRowStatus, slmTrapLogParam1=slmTrapLogParam1, slmSysName=slmSysName, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmSysMode=slmSysMode, slmLicenseEntry=slmLicenseEntry, slmAuthCommunity=slmAuthCommunity, slmOpticalSwitchExist=slmOpticalSwitchExist, slmPsuNumber=slmPsuNumber, UserPassword=UserPassword, SoftwareRevision=SoftwareRevision, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmTrapDestIpAddress=slmTrapDestIpAddress, slmTrapDestEntry=slmTrapDestEntry, slmSyslogDestRowStatus=slmSyslogDestRowStatus, PYSNMP_MODULE_ID=slMain, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmLicenseTable=slmLicenseTable, slmTrapPort=slmTrapPort, slmLicenseIndex=slmLicenseIndex, slmSysUplinkRate=slmSysUplinkRate, slmLicenseId=slmLicenseId, slmVcatDelay=slmVcatDelay, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmAdminEntry=slmAdminEntry, slmRebootDelay=slmRebootDelay, slmTrapLogEntry=slmTrapLogEntry, slmReadCommunity=slmReadCommunity, slmSysResetPm=slmSysResetPm, slmPortsNumber=slmPortsNumber, slmAdmin=slmAdmin, slmSwRevName=slmSwRevName, slmTrapLogParam3=slmTrapLogParam3, slmTrapSysLocationChange=slmTrapSysLocationChange, slmAuthLogin=slmAuthLogin, slmTrapDestCommunity=slmTrapDestCommunity, slmTrapSysNameChange=slmTrapSysNameChange, slmAdminAccess=slmAdminAccess, slmSysGatewayAddr=slmSysGatewayAddr, slmPmAvailable=slmPmAvailable, slmTrapUserAccess=slmTrapUserAccess, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, AdminAccess=AdminAccess, slmAdminLogin=slmAdminLogin, slmSysOperStatus=slmSysOperStatus, slmSysAlmAct=slmSysAlmAct, slmEdfaNumber=slmEdfaNumber, slmSysEncryptionCapability=slmSysEncryptionCapability, slmSyslogLevel=slmSyslogLevel, slmTrapLogId=slmTrapLogId, slmSysChassisId=slmSysChassisId, slmSysNetworkPrefix=slmSysNetworkPrefix, slmWriteCommunity=slmWriteCommunity, slm40GConf=slm40GConf, slMain=slMain, slmTrapEnable=slmTrapEnable, slmSysTrapFormat=slmSysTrapFormat, slmSwRevStatus=slmSwRevStatus, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmTrapUserLogin=slmTrapUserLogin, slmSyslogPort=slmSyslogPort, slmTrap=slmTrap, slmTrapDestRowStatus=slmTrapDestRowStatus, slmAuth=slmAuth, slmAuthEntry=slmAuthEntry, slmTopologyEnabled=slmTopologyEnabled, slmSysAdminStatus=slmSysAdminStatus, slmSysIpAddr=slmSysIpAddr, slmTrapLogTable=slmTrapLogTable)
