...
TimeTicks, IpAddress, Unsigned32, Bits, Counter64, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Opaque, Integer32, ObjectIdentity, MibIdentifier, NotificationType, iso, Gauge32, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "IpAddress", "Unsigned32", "Bits", "Counter64", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Opaque", "Integer32", "ObjectIdentity", "MibIdentifier", "NotificationType", "iso", "Gauge32", "Counter32")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmEdfaNumber=slmEdfaNumber, slmChPassTable=slmChPassTable, slmAdminTable=slmAdminTable, slmTrapPort=slmTrapPort, slmSwRevDirectory=slmSwRevDirectory, slmSnmpv3Password=slmSnmpv3Password, slmAuthCommunity=slmAuthCommunity, slMain=slMain, slmTrapLogParam1=slmTrapLogParam1, slmTrapDestIpAddress=slmTrapDestIpAddress, slmOpticalSwitchExist=slmOpticalSwitchExist, slmTrapCommunity=slmTrapCommunity, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmTrapLogName=slmTrapLogName, slmAdminEntry=slmAdminEntry, slmSyslogPort=slmSyslogPort, slmLicenseTable=slmLicenseTable, slmLicenseIndex=slmLicenseIndex, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmLogin=slmLogin, slmPmAvailable=slmPmAvailable, slmVcatDelay=slmVcatDelay, slmSysMode=slmSysMode, slmConfigSysSignalType=slmConfigSysSignalType, UserCommunity=UserCommunity, slmConfigFile=slmConfigFile, slmSysLanIpAddr=slmSysLanIpAddr, slmAdminPassword=slmAdminPassword, slmTrapDestProtVersion=slmTrapDestProtVersion, slmAdminCommunity=slmAdminCommunity, slmReadCommunity=slmReadCommunity, slmSysAlmAct=slmSysAlmAct, slmSyslogDestAddress=slmSyslogDestAddress, UserPassword=UserPassword, slmTrapDestEntry=slmTrapDestEntry, slmLicenseExpiration=slmLicenseExpiration, slmTrapLogTable=slmTrapLogTable, slmTrapLogParam3=slmTrapLogParam3, slmTrapLogParam4=slmTrapLogParam4, slmSnmpv3Auth=slmSnmpv3Auth, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmTrapDestRowStatus=slmTrapDestRowStatus, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmTrapDestTable=slmTrapDestTable, slmSysNetworkPrefix=slmSysNetworkPrefix, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmSysOperStatus=slmSysOperStatus, slmChPassEntry=slmChPassEntry, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmConfigSysUptime=slmConfigSysUptime, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmSysTemperature=slmSysTemperature, slmLogFile=slmLogFile, slmSyslogDestTable=slmSyslogDestTable, slmSysSubnetMask=slmSysSubnetMask, slmTrapSysLocationChange=slmTrapSysLocationChange, slmLicenseId=slmLicenseId, slmSyslogLevel=slmSyslogLevel, slmChPass=slmChPass, slmRebootDelay=slmRebootDelay, slmOemType=slmOemType, slmAdminRowStatus=slmAdminRowStatus, slmAdminAccess=slmAdminAccess, slmSnmpv3Priv=slmSnmpv3Priv, slmTrapSysNameChange=slmTrapSysNameChange, slmMuxNumber=slmMuxNumber, slmSysResetPm=slmSysResetPm, slmSysTrapFormat=slmSysTrapFormat, slmSysCalendarTime=slmSysCalendarTime, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmSysEncryptionCapability=slmSysEncryptionCapability, slmSyslogDestEntry=slmSyslogDestEntry, slmSysAlmDeact=slmSysAlmDeact, slmPsuNumber=slmPsuNumber, slmTrapDestAddress=slmTrapDestAddress, slmSysGatewayAddr=slmSysGatewayAddr, slmTrapLogParam2=slmTrapLogParam2, slmLicenseEntry=slmLicenseEntry, slmPortsNumber=slmPortsNumber, slmWriteCommunity=slmWriteCommunity, slmSysLocation=slmSysLocation, slmAdmin=slmAdmin, slm40GConf=slm40GConf, slmSysName=slmSysName, slmTrapLogId=slmTrapLogId, slmSwRevTable=slmSwRevTable, slmTrap=slmTrap, slmTrapEnable=slmTrapEnable, slmSysIpAddr=slmSysIpAddr, slmTrapLogEntry=slmTrapLogEntry, slmSysAdminStatus=slmSysAdminStatus, slmTrapDestCommunity=slmTrapDestCommunity, slmRstpEnabled=slmRstpEnabled, slmSysActiveSwVersion=slmSysActiveSwVersion, slmTopologyEnabled=slmTopologyEnabled, slmSysSnmpVersion=slmSysSnmpVersion, slmAdminLogin=slmAdminLogin, AdminAccess=AdminAccess, slmAuthPassword=slmAuthPassword, slmChPassOldPass=slmChPassOldPass, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmTrapLogParam6=slmTrapLogParam6, slmSys=slmSys, slmNetworkMode=slmNetworkMode, slmTrapUserLogin=slmTrapUserLogin, slmSysChassisId=slmSysChassisId, slmAuthEntry=slmAuthEntry, slmAuthTable=slmAuthTable, slmTrapLogParam5=slmTrapLogParam5, slmSwRevName=slmSwRevName, slmSwRevStatus=slmSwRevStatus, PYSNMP_MODULE_ID=slMain, SoftwareRevision=SoftwareRevision, slmAuth=slmAuth, slmChPassNewPass=slmChPassNewPass, slmSysUplinkRate=slmSysUplinkRate, slmChPassLogin=slmChPassLogin, slmSwRevDate=slmSwRevDate, slmLicense=slmLicense, UserLogin=UserLogin, slmTid=slmTid, slmSysPmStartOfDay=slmSysPmStartOfDay, slmSwRevEntry=slmSwRevEntry, slmAuthLogin=slmAuthLogin, slmTrapUserAccess=slmTrapUserAccess, slmBoxSize=slmBoxSize)
