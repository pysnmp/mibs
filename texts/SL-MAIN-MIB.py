...
MibScalar, MibTable, MibTableRow, MibTableColumn, iso, TimeTicks, Gauge32, Counter32, Counter64, Opaque, Integer32, IpAddress, NotificationType, Unsigned32, MibIdentifier, Bits, ModuleIdentity, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "TimeTicks", "Gauge32", "Counter32", "Counter64", "Opaque", "Integer32", "IpAddress", "NotificationType", "Unsigned32", "MibIdentifier", "Bits", "ModuleIdentity", "ObjectIdentity")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmSysCalendarTime=slmSysCalendarTime, slmSwRevTable=slmSwRevTable, slmSysAlmAct=slmSysAlmAct, slmTrapDestIpAddress=slmTrapDestIpAddress, slmSysAlmDeact=slmSysAlmDeact, slmChPass=slmChPass, slmAuthPassword=slmAuthPassword, slmAdminCommunity=slmAdminCommunity, slmChPassOldPass=slmChPassOldPass, slmTrapDestProtVersion=slmTrapDestProtVersion, slmSyslogLevel=slmSyslogLevel, slmLogin=slmLogin, slmSwRevDirectory=slmSwRevDirectory, slmTrap=slmTrap, slmSysAdminStatus=slmSysAdminStatus, slmPsuNumber=slmPsuNumber, slmTrapLogParam5=slmTrapLogParam5, slmBoxSize=slmBoxSize, slmTrapDestEntry=slmTrapDestEntry, slmTopologyEnabled=slmTopologyEnabled, slmTrapUserAccess=slmTrapUserAccess, slmSwRevDate=slmSwRevDate, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmAuth=slmAuth, slmTrapLogParam4=slmTrapLogParam4, slmTrapSysLocationChange=slmTrapSysLocationChange, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmAdminEntry=slmAdminEntry, slmLicense=slmLicense, slmLicenseIndex=slmLicenseIndex, slmAdminPassword=slmAdminPassword, slmChPassLogin=slmChPassLogin, slmLicenseEntry=slmLicenseEntry, slmRebootDelay=slmRebootDelay, slmAdminRowStatus=slmAdminRowStatus, slmTrapLogParam3=slmTrapLogParam3, slmReadCommunity=slmReadCommunity, slmSysNetworkPrefix=slmSysNetworkPrefix, slmSyslogDestAddress=slmSyslogDestAddress, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slm40GConf=slm40GConf, slmAuthLogin=slmAuthLogin, slmSysSubnetMask=slmSysSubnetMask, slmChPassEntry=slmChPassEntry, slmSwRevName=slmSwRevName, slmTrapLogTable=slmTrapLogTable, slmSysTrapFormat=slmSysTrapFormat, slmTrapLogId=slmTrapLogId, slmTrapLogParam6=slmTrapLogParam6, slmSysLocation=slmSysLocation, slmTrapPort=slmTrapPort, slmSnmpv3Password=slmSnmpv3Password, slmMuxNumber=slmMuxNumber, slmSysLanIpAddr=slmSysLanIpAddr, slmAuthEntry=slmAuthEntry, slmTrapLogParam1=slmTrapLogParam1, slmLicenseTable=slmLicenseTable, slmSysName=slmSysName, PYSNMP_MODULE_ID=slMain, slmConfigSysSignalType=slmConfigSysSignalType, slmTrapDestCommunity=slmTrapDestCommunity, slmSysEncryptionCapability=slmSysEncryptionCapability, slmChPassNewPass=slmChPassNewPass, slmSysMode=slmSysMode, slmConfigFile=slmConfigFile, slmRstpEnabled=slmRstpEnabled, UserLogin=UserLogin, slmSysChassisId=slmSysChassisId, slmSnmpv3Auth=slmSnmpv3Auth, slmNetworkMode=slmNetworkMode, slmSysResetPm=slmSysResetPm, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmConfigSysUptime=slmConfigSysUptime, slmTrapLogEntry=slmTrapLogEntry, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmAuthTable=slmAuthTable, slmTrapSysNameChange=slmTrapSysNameChange, slmTrapEnable=slmTrapEnable, slmTrapCommunity=slmTrapCommunity, slmSwRevEntry=slmSwRevEntry, slmTrapDestAddress=slmTrapDestAddress, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmSyslogDestRowStatus=slmSyslogDestRowStatus, UserCommunity=UserCommunity, slmAdminLogin=slmAdminLogin, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmPmAvailable=slmPmAvailable, SoftwareRevision=SoftwareRevision, AdminAccess=AdminAccess, slmTrapUserLogin=slmTrapUserLogin, slmVcatDelay=slmVcatDelay, slmTid=slmTid, slmSysOperStatus=slmSysOperStatus, UserPassword=UserPassword, slmTrapDestTable=slmTrapDestTable, slmSysGatewayAddr=slmSysGatewayAddr, slmPortsNumber=slmPortsNumber, slmAdmin=slmAdmin, slmLicenseExpiration=slmLicenseExpiration, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmEdfaNumber=slmEdfaNumber, slmSwRevStatus=slmSwRevStatus, slmSnmpv3Priv=slmSnmpv3Priv, slmSyslogPort=slmSyslogPort, slmAdminAccess=slmAdminAccess, slmSysActiveSwVersion=slmSysActiveSwVersion, slmAuthCommunity=slmAuthCommunity, slmTrapLogName=slmTrapLogName, slmLicenseId=slmLicenseId, slmOpticalSwitchExist=slmOpticalSwitchExist, slmSysSnmpVersion=slmSysSnmpVersion, slmSysUplinkRate=slmSysUplinkRate, slmSysTemperature=slmSysTemperature, slmLogFile=slmLogFile, slmAdminTable=slmAdminTable, slmSysIpAddr=slmSysIpAddr, slmSysPmStartOfDay=slmSysPmStartOfDay, slmTrapLogParam2=slmTrapLogParam2, slmSyslogDestTable=slmSyslogDestTable, slmTrapDestRowStatus=slmTrapDestRowStatus, slmSys=slmSys, slmOemType=slmOemType, slMain=slMain, slmSyslogDestEntry=slmSyslogDestEntry, slmWriteCommunity=slmWriteCommunity, slmChPassTable=slmChPassTable)
