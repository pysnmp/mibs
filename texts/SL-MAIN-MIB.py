...
NotificationType, ModuleIdentity, Integer32, TimeTicks, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Unsigned32, Counter64, ObjectIdentity, MibIdentifier, Opaque, Bits, IpAddress, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "ModuleIdentity", "Integer32", "TimeTicks", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Unsigned32", "Counter64", "ObjectIdentity", "MibIdentifier", "Opaque", "Bits", "IpAddress", "Gauge32")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmTrapPort=slmTrapPort, slmTrapLogParam6=slmTrapLogParam6, slMain=slMain, slmMuxNumber=slmMuxNumber, slmAdminPassword=slmAdminPassword, slmTrapLogParam2=slmTrapLogParam2, slmVcatDelay=slmVcatDelay, slmTrapDestEntry=slmTrapDestEntry, slmSysIpAddr=slmSysIpAddr, slmRebootDelay=slmRebootDelay, slmSwRevDate=slmSwRevDate, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmSyslogDestEntry=slmSyslogDestEntry, slmLogFile=slmLogFile, slmPmAvailable=slmPmAvailable, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmConfigSysUptime=slmConfigSysUptime, slmAdminCommunity=slmAdminCommunity, slmTopologyEnabled=slmTopologyEnabled, slmChPassEntry=slmChPassEntry, slmConfigFile=slmConfigFile, slmTrap=slmTrap, slmOpticalSwitchExist=slmOpticalSwitchExist, slmSysActiveSwVersion=slmSysActiveSwVersion, slmAdminRowStatus=slmAdminRowStatus, slmConfigSysSignalType=slmConfigSysSignalType, slmChPassOldPass=slmChPassOldPass, slmAdminEntry=slmAdminEntry, slmPsuNumber=slmPsuNumber, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, slmTrapDestTable=slmTrapDestTable, slmSysLanIpAddr=slmSysLanIpAddr, slmAuth=slmAuth, slmPortsNumber=slmPortsNumber, slmTid=slmTid, slmEdfaNumber=slmEdfaNumber, slmAuthTable=slmAuthTable, slmSysName=slmSysName, slmAdminAccess=slmAdminAccess, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmTrapDestAddress=slmTrapDestAddress, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmSyslogDestAddress=slmSyslogDestAddress, slmLicenseTable=slmLicenseTable, slmReadCommunity=slmReadCommunity, slmLicenseId=slmLicenseId, slmLogin=slmLogin, slmSysLocation=slmSysLocation, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmTrapCommunity=slmTrapCommunity, slmLicenseIndex=slmLicenseIndex, slmChPassNewPass=slmChPassNewPass, slmTrapSysNameChange=slmTrapSysNameChange, slmSysOperStatus=slmSysOperStatus, slmSysResetPm=slmSysResetPm, slmTrapLogParam3=slmTrapLogParam3, slmLicenseExpiration=slmLicenseExpiration, slmSysGatewayAddr=slmSysGatewayAddr, slmSnmpv3Password=slmSnmpv3Password, slmSyslogLevel=slmSyslogLevel, slmBoxSize=slmBoxSize, slmSwRevStatus=slmSwRevStatus, UserPassword=UserPassword, slmSysChassisId=slmSysChassisId, slmTrapLogParam4=slmTrapLogParam4, slmAdmin=slmAdmin, slmSysUplinkRate=slmSysUplinkRate, slmOemType=slmOemType, slmTrapLogTable=slmTrapLogTable, slmTrapDestProtVersion=slmTrapDestProtVersion, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmSwRevEntry=slmSwRevEntry, slmSwRevDirectory=slmSwRevDirectory, slmSysTemperature=slmSysTemperature, slmAdminLogin=slmAdminLogin, slmSysMode=slmSysMode, PYSNMP_MODULE_ID=slMain, slmAuthEntry=slmAuthEntry, slmTrapEnable=slmTrapEnable, slmChPassLogin=slmChPassLogin, slmAuthPassword=slmAuthPassword, slmSys=slmSys, slmSysAdminStatus=slmSysAdminStatus, slmSysNetworkPrefix=slmSysNetworkPrefix, slmNetworkMode=slmNetworkMode, UserLogin=UserLogin, slmWriteCommunity=slmWriteCommunity, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmSyslogPort=slmSyslogPort, slmSyslogDestTable=slmSyslogDestTable, slmAuthLogin=slmAuthLogin, slmSysPmStartOfDay=slmSysPmStartOfDay, slmTrapLogParam1=slmTrapLogParam1, slmSysAlmDeact=slmSysAlmDeact, UserCommunity=UserCommunity, SoftwareRevision=SoftwareRevision, slmChPass=slmChPass, slmChPassTable=slmChPassTable, slmTrapLogName=slmTrapLogName, slmTrapDestIpAddress=slmTrapDestIpAddress, slmSysCalendarTime=slmSysCalendarTime, slmSysTrapFormat=slmSysTrapFormat, slmSwRevTable=slmSwRevTable, slmSysSubnetMask=slmSysSubnetMask, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmSnmpv3Priv=slmSnmpv3Priv, slmTrapUserAccess=slmTrapUserAccess, slmTrapLogEntry=slmTrapLogEntry, slmRstpEnabled=slmRstpEnabled, slmSysSnmpVersion=slmSysSnmpVersion, slmTrapDestRowStatus=slmTrapDestRowStatus, slmTrapUserLogin=slmTrapUserLogin, slm40GConf=slm40GConf, slmSysEncryptionCapability=slmSysEncryptionCapability, slmTrapSysLocationChange=slmTrapSysLocationChange, slmSysAlmAct=slmSysAlmAct, slmTrapLogId=slmTrapLogId, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmAdminTable=slmAdminTable, slmTrapDestCommunity=slmTrapDestCommunity, slmTrapLogParam5=slmTrapLogParam5, slmLicenseEntry=slmLicenseEntry, slmAuthCommunity=slmAuthCommunity, slmLicense=slmLicense, AdminAccess=AdminAccess, slmSnmpv3Auth=slmSnmpv3Auth, slmSwRevName=slmSwRevName)
