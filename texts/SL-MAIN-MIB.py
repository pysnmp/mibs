...
MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, ObjectIdentity, NotificationType, TimeTicks, Unsigned32, Counter64, Integer32, Opaque, ModuleIdentity, iso, Counter32, Bits, Gauge32, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "ObjectIdentity", "NotificationType", "TimeTicks", "Unsigned32", "Counter64", "Integer32", "Opaque", "ModuleIdentity", "iso", "Counter32", "Bits", "Gauge32", "MibIdentifier")
...
mibBuilder.exportSymbols("SL-MAIN-MIB", slmTrapDestTable=slmTrapDestTable, slmSwRevStatus=slmSwRevStatus, slmWriteCommunity=slmWriteCommunity, slmSysEffectiveIpAddr=slmSysEffectiveIpAddr, slmTrapLogName=slmTrapLogName, slmLicenseIndex=slmLicenseIndex, slmTid=slmTid, slmSyslogPort=slmSyslogPort, slmAdminRowStatus=slmAdminRowStatus, slmTrapDestProtVersion=slmTrapDestProtVersion, slmSyslogDestTable=slmSyslogDestTable, slmConfigSysUptime=slmConfigSysUptime, slmTrapLogTable=slmTrapLogTable, slmSysIpAddr=slmSysIpAddr, slmLicenseExpiration=slmLicenseExpiration, slmAuthPassword=slmAuthPassword, slmAdminAccess=slmAdminAccess, slmSysActiveSwVersion=slmSysActiveSwVersion, slmSnmpv3Auth=slmSnmpv3Auth, slmSysAdminStatus=slmSysAdminStatus, slmSysPmStartOfDay=slmSysPmStartOfDay, slmSysLocation=slmSysLocation, slmAdminTable=slmAdminTable, slmSysAlmAct=slmSysAlmAct, slmChPassTable=slmChPassTable, slmSysLanEffectiveIpAddr=slmSysLanEffectiveIpAddr, slmConfigFile=slmConfigFile, slmAuthLogin=slmAuthLogin, slmTrapEnable=slmTrapEnable, slmPsuNumber=slmPsuNumber, slmTrapDestAddress=slmTrapDestAddress, slmTrapLogParam4=slmTrapLogParam4, slmEdfaNumber=slmEdfaNumber, slmConfigSysSignalType=slmConfigSysSignalType, slmTrapSysNameChange=slmTrapSysNameChange, slmSysEncryptionCapability=slmSysEncryptionCapability, slmSysCalendarTime=slmSysCalendarTime, slmSnmpv3Password=slmSnmpv3Password, slmPmAvailable=slmPmAvailable, slmTrapLogParam3=slmTrapLogParam3, slmChPassLogin=slmChPassLogin, slmSyslogDestIpAddress=slmSyslogDestIpAddress, slmSyslogDestRowStatus=slmSyslogDestRowStatus, slmAdmin=slmAdmin, slmSysTemperature=slmSysTemperature, slmSysName=slmSysName, slmSysTrapFormat=slmSysTrapFormat, slmLicenseEntry=slmLicenseEntry, SoftwareRevision=SoftwareRevision, slmChPassOldPass=slmChPassOldPass, UserLogin=UserLogin, slmAdminEntry=slmAdminEntry, slmSysOperStatus=slmSysOperStatus, slmOemType=slmOemType, slmOpticalSwitchExist=slmOpticalSwitchExist, slmLicenseTable=slmLicenseTable, slmTrapLogEntry=slmTrapLogEntry, slmSwRevTable=slmSwRevTable, slmAdminPassword=slmAdminPassword, UserCommunity=UserCommunity, slmVcatDelay=slmVcatDelay, slmAuthTable=slmAuthTable, slmTrapCommunity=slmTrapCommunity, slmAuthEntry=slmAuthEntry, slmAuthCommunity=slmAuthCommunity, slmChPassEntry=slmChPassEntry, slmAdminLogin=slmAdminLogin, slmBoxSize=slmBoxSize, slmNetworkMode=slmNetworkMode, slmLicense=slmLicense, slmAuth=slmAuth, slmSysAlmDeact=slmSysAlmDeact, slmTrapDestRowStatus=slmTrapDestRowStatus, slmTrapLogParam6=slmTrapLogParam6, slmSysLanSubnetAddr=slmSysLanSubnetAddr, slmTrapUserLogin=slmTrapUserLogin, slmAdminCommunity=slmAdminCommunity, AdminAccess=AdminAccess, slmTrapLogId=slmTrapLogId, slmSysResetPm=slmSysResetPm, slmSysUplinkRate=slmSysUplinkRate, slmRebootDelay=slmRebootDelay, slmSysGatewayAddr=slmSysGatewayAddr, slmSysLanIpAddr=slmSysLanIpAddr, slmTrapPort=slmTrapPort, slmTrapLogParam1=slmTrapLogParam1, slmSysLanEffectiveSubnetAddr=slmSysLanEffectiveSubnetAddr, slmSyslogDestAddress=slmSyslogDestAddress, slmReadCommunity=slmReadCommunity, slmTrapDestEntry=slmTrapDestEntry, slMain=slMain, slmTrapDestIpAddress=slmTrapDestIpAddress, slmSysSnmpVersion=slmSysSnmpVersion, slmLicenseId=slmLicenseId, PYSNMP_MODULE_ID=slMain, slmTrap=slmTrap, slmRstpEnabled=slmRstpEnabled, slmTrapUserAccess=slmTrapUserAccess, slmPortsNumber=slmPortsNumber, slmTrapSysLocationChange=slmTrapSysLocationChange, slmSysChassisId=slmSysChassisId, slmSyslogDestEntry=slmSyslogDestEntry, slmChPass=slmChPass, slmSysSubnetMask=slmSysSubnetMask, slmChPassNewPass=slmChPassNewPass, slmTrapLogParam5=slmTrapLogParam5, slmLogFile=slmLogFile, slm40GConf=slm40GConf, slmSyslogLevel=slmSyslogLevel, slmSysNetworkPrefix=slmSysNetworkPrefix, slmSys=slmSys, slmSwRevDate=slmSwRevDate, slmLogin=slmLogin, slmTrapSoftwareStatusChange=slmTrapSoftwareStatusChange, UserPassword=UserPassword, slmTopologyEnabled=slmTopologyEnabled, slmMuxNumber=slmMuxNumber, slmTrapLogTimeStamp=slmTrapLogTimeStamp, slmSwRevEntry=slmSwRevEntry, slmTrapDestCommunity=slmTrapDestCommunity, slmTrapLogParam2=slmTrapLogParam2, slmSnmpv3Priv=slmSnmpv3Priv, slmSysEffectiveSubnetMask=slmSysEffectiveSubnetMask, slmSwRevName=slmSwRevName, slmSysMode=slmSysMode, slmSysGatewayEffectiveIpAddr=slmSysGatewayEffectiveIpAddr, slmSwRevDirectory=slmSwRevDirectory)
