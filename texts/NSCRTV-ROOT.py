...
# Produced by pysmi-1.1.8 at Thu Jan 13 23:43:28 2022
...
ModuleIdentity, TimeTicks, NotificationType, Gauge32, Counter32, Bits, Unsigned32, iso, IpAddress, Integer32, Counter64, ObjectIdentity, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "TimeTicks", "NotificationType", "Gauge32", "Counter32", "Bits", "Unsigned32", "iso", "IpAddress", "Integer32", "Counter64", "ObjectIdentity", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier")
DisplayString, NetworkAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "NetworkAddress", "TextualConvention")
...
if mibBuilder.loadTexts: analogPropertyEntry.setDescription("Attribute Table simulation parameters head.\n\tOID purpose as a table index,Its coding method is'lengthï¿½ï¿½OID'ï¿½ï¿½OID former two members'1.3'According to '1'and'3'respectively coding,Instead of ordinary OID encoding (0x2B).")
...
if mibBuilder.loadTexts: analogAlarmDeadband.setDescription('Dead-alarm threshold value. After the warning, parameter values should be restored to the alarm threshold and alarm threshold and the difference between the absolute value greater than the value of dead zone, the alarm can be removed.\n        This object should be stored in nonvolatile memory.')
...
if mibBuilder.loadTexts: discreteAlarmValue.setDescription('Attribute Table discrete index 2: parameter values.\n               When the equipment value of this parameter values, and so will be dealt with a warning.')
...
if mibBuilder.loadTexts: currentAlarmEntry.setDescription('The current warning Table Head.\n               OID coding methods with simulation attribute table.')
...
if mibBuilder.loadTexts: alarmLogIndex.setDescription('The only logo Alarm Index recorded a record in the table, the index value increased from 1 start of each new record, plus 1 until 32,767,\n    a record index of the re-starts at 1. Acting under the management of storage capacity can be the first choice to delete those records,\n    the specific details in this not to achieve provisions.')
...
if mibBuilder.loadTexts: hfcAlarmEvent.setDescription('When detected in the event of alarm information sent this trap, whether bundled alarmText variable parameter under warning object to determine.\n        Some of the parameters only need to bind the alarm first three variables.')
...
if mibBuilder.loadTexts: commonNELogicalID.setDescription('The logic of the designated NE identifier (LogicID), the values and attributes of other unrelated NE.\n        This target value should be stored in non-volatile memory.')
...
if mibBuilder.loadTexts: commonNEStatus.setDescription('With 7.5.4 STATRESP PDU parameters in the corresponding Status\n           Bit 0: CHNLRQST\n           Bit 1: CNTNRM\n           Bit 2: CNTCUR\n           Bit 3: MAJOR ALARMS\n           Bit 4: MINOR ALARMS\n           Bit 5: RSVD1\n           Bit 6: RSVD2\n\t\t   Bit 7: RSVD3')
...
if mibBuilder.loadTexts: commonTrapCommunityString.setDescription("Definition of Community Trap string. The default value is' public '.\nThe value of this object should be stored in non-volatile memory.")
...
if mibBuilder.loadTexts: commonCraftStatus.setDescription('NE local object that this interface (such as RS232 or RS485 interface) state. NE does not necessarily have to support the local interface.\n NE local interface without affecting the state of its MAC interface functions; If we do not support the local interface, and its value is disconnected.')
...
if mibBuilder.loadTexts: commonBackoffPeriod.setDescription('Backoff algorithm benchmark time (ms), initialize the default value of 6 ms.\nThe value of this object should be stored in non-volatile memory.')
...
if mibBuilder.loadTexts: commonACKTimeoutWindow.setDescription('NE awaiting HE overtime to send ACK response time (ms), initialize the default value is 19 ms.\n The value of this object should be stored in non-volatile memory.')
...
if mibBuilder.loadTexts: commonMaximumMACLayerRetries.setDescription('NE data packets sent the greatest number of re-examination. Initialize the default value is 16.\nThe value of this object should be stored in non-volatile memory.')
...
if mibBuilder.loadTexts: commonBackoffMinimumExponent.setDescription('The standards body of the MAC layer specification definition of the smallest backoff algorithm index value, default values for 6.\nThis value may not exceed commonBackoffMaximumValue value.\nThe value of this object should be stored in non-volatile memory.')
...
if mibBuilder.loadTexts: commonBackoffMaximumExponent.setDescription('The body of the MAC layer standard definition of backoff algorithm for standardizing the largest index value, the default value is 15.\n This value shall be not less than commonBackoffMinimum value.\n The value of this object should be stored in non-volatile memory.')
...
if mibBuilder.loadTexts: commonReturnPathFrequency.setDescription('Channel uplink frequency units Hz.\nThe value of this object should be stored in non-volatile memory.')
...
if mibBuilder.loadTexts: commonForwardPathFrequency.setDescription('Downlink frequency channel, flat Hz.\nThe value of this object should be stored in non-volatile memory.')
...
if mibBuilder.loadTexts: commonMaxReturnPower.setDescription('The largest upstream channel power levels and units for dBuV.\n The value of this object should be stored in non-volatile memory.')
...
if mibBuilder.loadTexts: commonDeviceStatus.setDescription('The state equipment\n           Bit 0: RSVD0\n           Bit 1: RSVD1\n           Bit 2: RSVD2\n           Bit 3: MAJOR ALARMS\n           Bit 4: MINOR ALARMS\n           Bit 5: RSVD5\n           Bit 6: RSVD6\n\t\t   Bit 7: RSVD7')
...
mibBuilder.exportSymbols("NSCRTV-ROOT", commonMacAddress=commonMacAddress, commonIdent=commonIdent, commonForwardPathReceiveLevel=commonForwardPathReceiveLevel, commonNELogicalID=commonNELogicalID, commonAdminUseEthernet=commonAdminUseEthernet, oaDCPowerCurrent=oaDCPowerCurrent, hfcWarmStart=hfcWarmStart, commonAgentBootWay=commonAgentBootWay, commonDevicesOID=commonDevicesOID, commonAgentMaxTraps=commonAgentMaxTraps, commonDeviceMACAddress=commonDeviceMACAddress, oaInputOpticalPower=oaInputOpticalPower, oaGainSet=oaGainSet, discretePropertyTable=discretePropertyTable, alarmLogEntry=alarmLogEntry, commonAgentTrapIP=commonAgentTrapIP, commonDeviceSerialNumber=commonDeviceSerialNumber, commonAgentTrapCommunity=commonAgentTrapCommunity, commonDeviceOID=commonDeviceOID, commonAgentTrapStatus=commonAgentTrapStatus, commonCheckCode=commonCheckCode, commonNEVendor=commonNEVendor, hfcColdStart=hfcColdStart, oaPumpEntry=oaPumpEntry, commonTrapCommunityString=commonTrapCommunityString, oaDCPowerSupplyMode=oaDCPowerSupplyMode, propertyIdent=propertyIdent, commonForwardPathCRCErrors=commonForwardPathCRCErrors, commonPhysAddress=commonPhysAddress, commonForwardPathLOSEvents=commonForwardPathLOSEvents, commonDeviceTamperStatus=commonDeviceTamperStatus, commonMacStats=commonMacStats, commonVarBindings=commonVarBindings, commonDeviceFW=commonDeviceFW, commonMaxMulticastAddresses=commonMaxMulticastAddresses, commonAdminUseRf=commonAdminUseRf, analogAlarmLOLO=analogAlarmLOLO, commonAlarmDetectionControl=commonAlarmDetectionControl, commonMulticastAddressTable=commonMulticastAddressTable, alarmLogTable=alarmLogTable, currentAlarmState=currentAlarmState, commonDeviceMFD=commonDeviceMFD, discreteAlarmEnable=discreteAlarmEnable, analogPropertyTable=analogPropertyTable, currentAlarmEntry=currentAlarmEntry, oaOutputOpticalPowerSet=oaOutputOpticalPowerSet, commonMACGroup=commonMACGroup, commonDeviceStatus=commonDeviceStatus, commonAdminGroup=commonAdminGroup, commonMaxPayloadSize=commonMaxPayloadSize, oaPumpBIAS=oaPumpBIAS, discretePropertyEntry=discretePropertyEntry, commonMaxReturnPower=commonMaxReturnPower, commonTamperStatus=commonTamperStatus, powerSupplyStatusB=powerSupplyStatusB, commonDeviceName=commonDeviceName, commonNEModelNumber=commonNEModelNumber, commonResetCause=commonResetCause, oaPumpTemp=oaPumpTemp, commonCraftStatus=commonCraftStatus, commonNEStatus=commonNEStatus, commonForwardPathFramingErrors=commonForwardPathFramingErrors, commonDeviceSlot=commonDeviceSlot, alarmLogInformation=alarmLogInformation, commonReturnPathFrequency=commonReturnPathFrequency, commonDeviceReset=commonDeviceReset, alarmsIdent=alarmsIdent, commonAgentTrapEntry=commonAgentTrapEntry, oaPumpIndex=oaPumpIndex, discreteParameterOID=discreteParameterOID, commonBackoffParams=commonBackoffParams, alarmText=alarmText, commonDeviceId=commonDeviceId, commonNESerialNumber=commonNESerialNumber, analogParameterOID=analogParameterOID, commonDeviceAlarmDetectionControl=commonDeviceAlarmDetectionControl, oaDCPowerTable=oaDCPowerTable, commonNetworkAddress=commonNetworkAddress, commonAgentTrapMinInterval=commonAgentTrapMinInterval, commonTrapAck=commonTrapAck, currentAlarmTable=currentAlarmTable, commonBackoffPeriod=commonBackoffPeriod, commonProvisionedReturnPowerLevel=commonProvisionedReturnPowerLevel, commonAgentTrapMaxInterval=commonAgentTrapMaxInterval, commonDeviceModelNumber=commonDeviceModelNumber, commonMulticastAddressEntry=commonMulticastAddressEntry, commonACKTimeoutWindow=commonACKTimeoutWindow, analogAlarmHIHI=analogAlarmHIHI, discreteAlarmValue=discreteAlarmValue, commonDeviceAcct=commonDeviceAcct, commonBackoffMaximumExponent=commonBackoffMaximumExponent, commonDeviceInfoEntry=commonDeviceInfoEntry, commonMulticastAddressIndex=commonMulticastAddressIndex, oaVendorOID=oaVendorOID, oaDCPowerIndex=oaDCPowerIndex, oaPumpTable=oaPumpTable, nscrtvRoot=nscrtvRoot, commondownload=commondownload, commonMulticastAddressNumber=commonMulticastAddressNumber, oaDCPowerEntry=oaDCPowerEntry, oaDCPowerVoltage=oaDCPowerVoltage, commonInternalTemperature=commonInternalTemperature, commonBackwardPathCollisionTimes=commonBackwardPathCollisionTimes, nscrtvHFCemsTree=nscrtvHFCemsTree, oaNumberDCPowerSupply=oaNumberDCPowerSupply, commonInvalidMacCmds=commonInvalidMacCmds, powerSupplyStatusA=powerSupplyStatusA, alarmLogLastIndex=alarmLogLastIndex, commonForwardPathFrequency=commonForwardPathFrequency, analogAlarmDeadband=analogAlarmDeadband, hfcAlarmEvent=hfcAlarmEvent, alarmEnable=alarmEnable, oaOutputOpticalPower=oaOutputOpticalPower, alarmLogNumberOfEntries=alarmLogNumberOfEntries, oaIdent=oaIdent, commonDeviceVendorInfo=commonDeviceVendorInfo, analogAlarmHI=analogAlarmHI, oaDCPowerName=oaDCPowerName, commonDeviceVendor=commonDeviceVendor, commonBackoffMinimumExponent=commonBackoffMinimumExponent, commonDevicesID=commonDevicesID, commonDeviceGroup=commonDeviceGroup, commonAgentTrapTable=commonAgentTrapTable, commonNEVendorInfo=commonNEVendorInfo, oaPumpTEC=oaPumpTEC, commonReset=commonReset, currentAlarmOID=currentAlarmOID, commonAgentTrapIndex=commonAgentTrapIndex, commonDeviceCraftStatus=commonDeviceCraftStatus, discreteAlarmState=discreteAlarmState, commonDeviceInfoTable=commonDeviceInfoTable, analogAlarmState=analogAlarmState, commonRfGroup=commonRfGroup, commonMaximumMACLayerRetries=commonMaximumMACLayerRetries, alarmLogIndex=alarmLogIndex, commonDeviceInternalTemperature=commonDeviceInternalTemperature, currentAlarmValue=currentAlarmValue, commonAgentReset=commonAgentReset, commonTime=commonTime, analogAlarmLO=analogAlarmLO, analogPropertyEntry=analogPropertyEntry, commonAgentGroup=commonAgentGroup, commonDeviceNum=commonDeviceNum, commonDeviceResetCause=commonDeviceResetCause)
