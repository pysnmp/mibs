...
Integer32, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, NotificationType, Counter64, mib_2, iso, MibIdentifier, Unsigned32, Counter32, TimeTicks, Bits, ModuleIdentity, ObjectIdentity, IpAddress, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "NotificationType", "Counter64", "mib-2", "iso", "MibIdentifier", "Unsigned32", "Counter32", "TimeTicks", "Bits", "ModuleIdentity", "ObjectIdentity", "IpAddress", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsBytesFromClient=stHttpsBytesFromClient, kMicroVersion=kMicroVersion, mwgHttps=mwgHttps, kCustomVersion=kCustomVersion, mwgStatistics=mwgStatistics, stConnectionsLegitimate=stConnectionsLegitimate, mwgMiscellaneous=mwgMiscellaneous, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpBytesFromClient=stFtpBytesFromClient, kMajorVersion=kMajorVersion, stConnectedSockets=stConnectedSockets, notifyOriginName=notifyOriginName, stCPUIOWait=stCPUIOWait, notifyAdditional=notifyAdditional, notifyOrigin=notifyOrigin, trSystem=trSystem, mwgTrapVariables=mwgTrapVariables, stBadReputation=stBadReputation, stHttpBytesToClient=stHttpBytesToClient, stMimeType=stMimeType, kProductName=kProductName, kRevision=kRevision, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgHttp=mwgHttp, stCategoriesTable=stCategoriesTable, stCategoryCount=stCategoryCount, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgInfo=mwgInfo, stHttpsBytesToServer=stHttpsBytesToServer, stHttpBytesToServer=stHttpBytesToServer, stFtpBytesToClient=stFtpBytesToClient, notifyAffectedHost=notifyAffectedHost, mwgContent=mwgContent, kProductVersion=kProductVersion, stHandleConnectToServer=stHandleConnectToServer, stMemConsumed=stMemConsumed, stFtpBytesToServer=stFtpBytesToServer, pAMProactiveVersion=pAMProactiveVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpRequests=stHttpRequests, stResolveHostViaDNS=stResolveHostViaDNS, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCategoriesEntry=stCategoriesEntry, stHttpsRequests=stHttpsRequests, stHttpsBytesToClient=stHttpsBytesToClient, mwgEngineVersions=mwgEngineVersions, stTimeForTransaction=stTimeForTransaction, notifyReasonString=notifyReasonString, stBlockedByMediaFilter=stBlockedByMediaFilter, stLastSentLastReceivedClient=stLastSentLastReceivedClient, trUser=trUser, stHttpTraffic=stHttpTraffic, stMalwareDetected=stMalwareDetected, trApplication=trApplication, stFtpBytesFromServer=stFtpBytesFromServer, pTSDBVersion=pTSDBVersion, mwg=mwg, stFtpTraffic=stFtpTraffic, pMFEDATVersion=pMFEDATVersion, stHttpsTraffic=stHttpsTraffic, stBlockedByURLFilter=stBlockedByURLFilter, mwgFTP=mwgFTP, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpBytesFromServer=stHttpBytesFromServer, stClientCount=stClientCount, stCategoryName=stCategoryName, notifySeverity=notifySeverity, mwgTraps=mwgTraps, PYSNMP_MODULE_ID=mwg, kCompanyName=kCompanyName, pAMSignatureVersion=pAMSignatureVersion, stCPULoadRaw=stCPULoadRaw, kHotfixVersion=kHotfixVersion, stHttpBytesFromClient=stHttpBytesFromClient, stCategories=stCategories, notifyReason=notifyReason, kMinorVersion=kMinorVersion, kBuildNumber=kBuildNumber, pAMEngineVersion=pAMEngineVersion, pMFEEngineVersion=pMFEEngineVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectionsBlocked=stConnectionsBlocked, stCPULoad=stCPULoad)
