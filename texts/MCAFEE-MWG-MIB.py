...
MibScalar, MibTable, MibTableRow, MibTableColumn, mib_2, iso, IpAddress, MibIdentifier, NotificationType, TimeTicks, Counter64, ModuleIdentity, ObjectIdentity, Gauge32, Bits, Unsigned32, Counter32, NotificationType, enterprises, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "mib-2", "iso", "IpAddress", "MibIdentifier", "NotificationType", "TimeTicks", "Counter64", "ModuleIdentity", "ObjectIdentity", "Gauge32", "Bits", "Unsigned32", "Counter32", "NotificationType", "enterprises", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pMFEDATVersion=pMFEDATVersion, kMajorVersion=kMajorVersion, kProductName=kProductName, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategoriesEntry=stCategoriesEntry, kProductVersion=kProductVersion, mwgMiscellaneous=mwgMiscellaneous, trSystem=trSystem, stFtpBytesToServer=stFtpBytesToServer, kBuildNumber=kBuildNumber, stHttpBytesToServer=stHttpBytesToServer, stConnectionsLegitimate=stConnectionsLegitimate, stFtpBytesFromClient=stFtpBytesFromClient, stHttpBytesToClient=stHttpBytesToClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stResolveHostViaDNS=stResolveHostViaDNS, notifyOrigin=notifyOrigin, mwgHttp=mwgHttp, stCategoryName=stCategoryName, kMicroVersion=kMicroVersion, mwgTrapVariables=mwgTrapVariables, stFtpBytesToClient=stFtpBytesToClient, kCompanyName=kCompanyName, kRevision=kRevision, stHttpsTraffic=stHttpsTraffic, stCPULoadRaw=stCPULoadRaw, stBlockedByURLFilter=stBlockedByURLFilter, stBlockedByAntiMalware=stBlockedByAntiMalware, pMFEEngineVersion=pMFEEngineVersion, stConnectionsBlocked=stConnectionsBlocked, mwg=mwg, stHttpBytesFromClient=stHttpBytesFromClient, kMinorVersion=kMinorVersion, stHttpsBytesToClient=stHttpsBytesToClient, stMimeType=stMimeType, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgHttps=mwgHttps, notifyReason=notifyReason, mwgFTP=mwgFTP, mwgContent=mwgContent, notifyAdditional=notifyAdditional, stTimeForTransaction=stTimeForTransaction, notifySeverity=notifySeverity, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgEngineVersions=mwgEngineVersions, stHttpRequests=stHttpRequests, stCategories=stCategories, trApplication=trApplication, PYSNMP_MODULE_ID=mwg, kCustomVersion=kCustomVersion, stMemConsumed=stMemConsumed, pAMEngineVersion=pAMEngineVersion, stHttpsRequests=stHttpsRequests, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stCategoriesTable=stCategoriesTable, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifyAffectedHost=notifyAffectedHost, mwgStatistics=mwgStatistics, stFtpTraffic=stFtpTraffic, stHttpsBytesFromServer=stHttpsBytesFromServer, pTSDBVersion=pTSDBVersion, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesToServer=stHttpsBytesToServer, stBadReputation=stBadReputation, stHttpBytesFromServer=stHttpBytesFromServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpTraffic=stHttpTraffic, stCPUIOWait=stCPUIOWait, stCategoryCount=stCategoryCount, pAMProactiveVersion=pAMProactiveVersion, stCPULoad=stCPULoad, stConnectedSockets=stConnectedSockets, stHandleConnectToServer=stHandleConnectToServer, notifyReasonString=notifyReasonString, mwgInfo=mwgInfo, notifyOriginName=notifyOriginName, stHttpsBytesFromClient=stHttpsBytesFromClient, stFtpBytesFromServer=stFtpBytesFromServer, stClientCount=stClientCount, stMalwareDetected=stMalwareDetected, trUser=trUser, mwgTraps=mwgTraps, kHotfixVersion=kHotfixVersion)
