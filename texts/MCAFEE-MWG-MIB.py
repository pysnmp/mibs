...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stBlockedByURLFilter=stBlockedByURLFilter, stCPULoad=stCPULoad, stFtpBytesToClient=stFtpBytesToClient, pMFEEngineVersion=pMFEEngineVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifyAdditional=notifyAdditional, mwgHttp=mwgHttp, stCPULoadRaw=stCPULoadRaw, PYSNMP_MODULE_ID=mwg, mwgTrapVariables=mwgTrapVariables, notifyOrigin=notifyOrigin, stHttpsBytesToServer=stHttpsBytesToServer, stFtpBytesFromServer=stFtpBytesFromServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsBytesToClient=stHttpsBytesToClient, stMimeType=stMimeType, stConnectionsBlocked=stConnectionsBlocked, mwg=mwg, kRevision=kRevision, stHttpBytesToClient=stHttpBytesToClient, kProductVersion=kProductVersion, stCPUIOWait=stCPUIOWait, stTimeForTransaction=stTimeForTransaction, mwgMiscellaneous=mwgMiscellaneous, mwgStatistics=mwgStatistics, mwgContent=mwgContent, trApplication=trApplication, mwgHttps=mwgHttps, stBlockedByMediaFilter=stBlockedByMediaFilter, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCategoriesTable=stCategoriesTable, stFtpTraffic=stFtpTraffic, stFtpBytesFromClient=stFtpBytesFromClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, kCustomVersion=kCustomVersion, stHttpRequests=stHttpRequests, notifyOriginName=notifyOriginName, mwgFTP=mwgFTP, stHttpsRequests=stHttpsRequests, stBlockedByAntiMalware=stBlockedByAntiMalware, pAMSignatureVersion=pAMSignatureVersion, notifyReason=notifyReason, trSystem=trSystem, pTSDBVersion=pTSDBVersion, mwgInfo=mwgInfo, pAMProactiveVersion=pAMProactiveVersion, notifyAffectedHost=notifyAffectedHost, stClientCount=stClientCount, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsBytesFromClient=stHttpsBytesFromClient, stCategories=stCategories, kHotfixVersion=kHotfixVersion, stHttpBytesFromServer=stHttpBytesFromServer, kMajorVersion=kMajorVersion, stCategoryName=stCategoryName, kCompanyName=kCompanyName, mwgEngineVersions=mwgEngineVersions, stResolveHostViaDNS=stResolveHostViaDNS, stCategoryCount=stCategoryCount, pAMEngineVersion=pAMEngineVersion, mwgTraps=mwgTraps, kMicroVersion=kMicroVersion, kBuildNumber=kBuildNumber, stHttpBytesFromClient=stHttpBytesFromClient, stConnectedSockets=stConnectedSockets, kMinorVersion=kMinorVersion, stHttpsTraffic=stHttpsTraffic, notifyReasonString=notifyReasonString, stHttpBytesToServer=stHttpBytesToServer, kProductName=kProductName, stHttpTraffic=stHttpTraffic, pMFEDATVersion=pMFEDATVersion, stCategoriesEntry=stCategoriesEntry, stMemConsumed=stMemConsumed, trUser=trUser, stHttpsBytesFromServer=stHttpsBytesFromServer, stBadReputation=stBadReputation, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stConnectionsLegitimate=stConnectionsLegitimate, stFtpBytesToServer=stFtpBytesToServer, notifySeverity=notifySeverity, stMalwareDetected=stMalwareDetected, stHandleConnectToServer=stHandleConnectToServer)
