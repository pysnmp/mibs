...
Integer32, mib_2, Counter64, ModuleIdentity, Bits, TimeTicks, Counter32, enterprises, IpAddress, Gauge32, NotificationType, Unsigned32, iso, MibIdentifier, ObjectIdentity, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "mib-2", "Counter64", "ModuleIdentity", "Bits", "TimeTicks", "Counter32", "enterprises", "IpAddress", "Gauge32", "NotificationType", "Unsigned32", "iso", "MibIdentifier", "ObjectIdentity", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategories=stCategories, mwgHttp=mwgHttp, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, notifyReason=notifyReason, stMalwareDetected=stMalwareDetected, stCategoryCount=stCategoryCount, stFtpTraffic=stFtpTraffic, stFtpBytesToClient=stFtpBytesToClient, trSystem=trSystem, stFtpBytesFromClient=stFtpBytesFromClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kRevision=kRevision, kProductName=kProductName, stHttpTraffic=stHttpTraffic, stCPUIOWait=stCPUIOWait, PYSNMP_MODULE_ID=mwg, stHttpBytesToClient=stHttpBytesToClient, mwgTrapVariables=mwgTrapVariables, pMFEDATVersion=pMFEDATVersion, stHttpRequests=stHttpRequests, stHttpBytesFromClient=stHttpBytesFromClient, notifyAffectedHost=notifyAffectedHost, mwg=mwg, mwgEngineVersions=mwgEngineVersions, stBadReputation=stBadReputation, stConnectedSockets=stConnectedSockets, pMFEEngineVersion=pMFEEngineVersion, stMemConsumed=stMemConsumed, kHotfixVersion=kHotfixVersion, trApplication=trApplication, mwgTraps=mwgTraps, trUser=trUser, stConnectionsLegitimate=stConnectionsLegitimate, notifyOrigin=notifyOrigin, kMajorVersion=kMajorVersion, kProductVersion=kProductVersion, stFtpBytesFromServer=stFtpBytesFromServer, stResolveHostViaDNS=stResolveHostViaDNS, stMimeType=stMimeType, notifyOriginName=notifyOriginName, stCategoriesEntry=stCategoriesEntry, mwgHttps=mwgHttps, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpBytesFromServer=stHttpBytesFromServer, stHttpsTraffic=stHttpsTraffic, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgMiscellaneous=mwgMiscellaneous, stHttpsBytesFromClient=stHttpsBytesFromClient, notifyReasonString=notifyReasonString, stTimeForTransaction=stTimeForTransaction, mwgInfo=mwgInfo, stCategoriesTable=stCategoriesTable, stHttpsBytesFromServer=stHttpsBytesFromServer, kMicroVersion=kMicroVersion, stFtpBytesToServer=stFtpBytesToServer, mwgFTP=mwgFTP, notifyAdditional=notifyAdditional, mwgStatistics=mwgStatistics, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stClientCount=stClientCount, pAMSignatureVersion=pAMSignatureVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, kMinorVersion=kMinorVersion, stHandleConnectToServer=stHandleConnectToServer, notifySeverity=notifySeverity, stBlockedByURLFilter=stBlockedByURLFilter, mwgContent=mwgContent, pTSDBVersion=pTSDBVersion, pAMProactiveVersion=pAMProactiveVersion, stCategoryName=stCategoryName, kCustomVersion=kCustomVersion, stCPULoad=stCPULoad, kBuildNumber=kBuildNumber, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kCompanyName=kCompanyName, stHttpsBytesToServer=stHttpsBytesToServer, stConnectionsBlocked=stConnectionsBlocked, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, pAMEngineVersion=pAMEngineVersion, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesToServer=stHttpBytesToServer, stHttpsRequests=stHttpsRequests, stCPULoadRaw=stCPULoadRaw)
