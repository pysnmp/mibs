...
Counter32, mib_2, Bits, enterprises, Gauge32, ObjectIdentity, iso, MibIdentifier, Integer32, IpAddress, ModuleIdentity, Unsigned32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, TimeTicks, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "mib-2", "Bits", "enterprises", "Gauge32", "ObjectIdentity", "iso", "MibIdentifier", "Integer32", "IpAddress", "ModuleIdentity", "Unsigned32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "TimeTicks", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgInfo=mwgInfo, stFtpTraffic=stFtpTraffic, kHotfixVersion=kHotfixVersion, trUser=trUser, kMinorVersion=kMinorVersion, stCategoriesEntry=stCategoriesEntry, stHttpBytesToClient=stHttpBytesToClient, pTSDBVersion=pTSDBVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stResolveHostViaDNS=stResolveHostViaDNS, notifyAffectedHost=notifyAffectedHost, mwgHttp=mwgHttp, stBlockedByURLFilter=stBlockedByURLFilter, pAMEngineVersion=pAMEngineVersion, mwgHttps=mwgHttps, kRevision=kRevision, mwgStatistics=mwgStatistics, stHttpBytesToServer=stHttpBytesToServer, mwgTrapVariables=mwgTrapVariables, pMFEEngineVersion=pMFEEngineVersion, pAMProactiveVersion=pAMProactiveVersion, notifyReason=notifyReason, stHttpBytesFromClient=stHttpBytesFromClient, stCategoriesTable=stCategoriesTable, stBadReputation=stBadReputation, stHandleConnectToServer=stHandleConnectToServer, trSystem=trSystem, mwg=mwg, pMFEDATVersion=pMFEDATVersion, notifyOriginName=notifyOriginName, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpsBytesFromClient=stHttpsBytesFromClient, stCPULoadRaw=stCPULoadRaw, mwgEngineVersions=mwgEngineVersions, stFtpBytesFromServer=stFtpBytesFromServer, stCategoryName=stCategoryName, mwgMiscellaneous=mwgMiscellaneous, stCPULoad=stCPULoad, notifyReasonString=notifyReasonString, PYSNMP_MODULE_ID=mwg, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpBytesFromClient=stFtpBytesFromClient, stConnectedSockets=stConnectedSockets, stCategoryCount=stCategoryCount, stClientCount=stClientCount, mwgTraps=mwgTraps, stHttpBytesFromServer=stHttpBytesFromServer, kProductName=kProductName, kMicroVersion=kMicroVersion, stMemConsumed=stMemConsumed, stTimeForTransaction=stTimeForTransaction, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kCustomVersion=kCustomVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, trApplication=trApplication, stHttpsBytesToClient=stHttpsBytesToClient, kProductVersion=kProductVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, notifyAdditional=notifyAdditional, stHttpRequests=stHttpRequests, stHttpsBytesToServer=stHttpsBytesToServer, stFtpBytesToServer=stFtpBytesToServer, stHttpsTraffic=stHttpsTraffic, stHttpsRequests=stHttpsRequests, stHttpTraffic=stHttpTraffic, notifySeverity=notifySeverity, pAMSignatureVersion=pAMSignatureVersion, kMajorVersion=kMajorVersion, stConnectionsLegitimate=stConnectionsLegitimate, stBlockedByAntiMalware=stBlockedByAntiMalware, stMalwareDetected=stMalwareDetected, stCategories=stCategories, stFtpBytesToClient=stFtpBytesToClient, mwgFTP=mwgFTP, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyOrigin=notifyOrigin, stHttpsBytesFromServer=stHttpsBytesFromServer, stCPUIOWait=stCPUIOWait, kCompanyName=kCompanyName, stMimeType=stMimeType, stConnectionsBlocked=stConnectionsBlocked, mwgContent=mwgContent, kBuildNumber=kBuildNumber)
