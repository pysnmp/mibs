...
Integer32, Counter64, TimeTicks, Gauge32, enterprises, ObjectIdentity, IpAddress, iso, NotificationType, mib_2, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, NotificationType, Bits, Counter32, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Counter64", "TimeTicks", "Gauge32", "enterprises", "ObjectIdentity", "IpAddress", "iso", "NotificationType", "mib-2", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "NotificationType", "Bits", "Counter32", "MibIdentifier")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kMicroVersion=kMicroVersion, pAMSignatureVersion=pAMSignatureVersion, stCategoryName=stCategoryName, stHttpsBytesFromServer=stHttpsBytesFromServer, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsBytesFromClient=stHttpsBytesFromClient, stBlockedByMediaFilter=stBlockedByMediaFilter, pMFEEngineVersion=pMFEEngineVersion, stCategoryCount=stCategoryCount, pAMProactiveVersion=pAMProactiveVersion, trApplication=trApplication, mwgStatistics=mwgStatistics, stFtpBytesToServer=stFtpBytesToServer, stHttpBytesFromServer=stHttpBytesFromServer, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsTraffic=stHttpsTraffic, mwgContent=mwgContent, notifyOriginName=notifyOriginName, stCategories=stCategories, stHttpRequests=stHttpRequests, stBlockedByAntiMalware=stBlockedByAntiMalware, PYSNMP_MODULE_ID=mwg, stHttpsRequests=stHttpsRequests, kRevision=kRevision, stConnectedSockets=stConnectedSockets, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpBytesToClient=stHttpBytesToClient, stCPULoadRaw=stCPULoadRaw, notifyReasonString=notifyReasonString, stMimeType=stMimeType, stTimeForTransaction=stTimeForTransaction, mwgHttps=mwgHttps, notifyAdditional=notifyAdditional, kMajorVersion=kMajorVersion, stHttpTraffic=stHttpTraffic, stMemConsumed=stMemConsumed, trSystem=trSystem, stHandleConnectToServer=stHandleConnectToServer, mwg=mwg, kHotfixVersion=kHotfixVersion, kCustomVersion=kCustomVersion, stHttpBytesFromClient=stHttpBytesFromClient, kBuildNumber=kBuildNumber, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pTSDBVersion=pTSDBVersion, stFtpBytesFromServer=stFtpBytesFromServer, stBadReputation=stBadReputation, mwgMiscellaneous=mwgMiscellaneous, stMalwareDetected=stMalwareDetected, stCPUIOWait=stCPUIOWait, kProductName=kProductName, kMinorVersion=kMinorVersion, stCategoriesTable=stCategoriesTable, stCPULoad=stCPULoad, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stResolveHostViaDNS=stResolveHostViaDNS, mwgEngineVersions=mwgEngineVersions, stHttpBytesToServer=stHttpBytesToServer, pAMEngineVersion=pAMEngineVersion, pMFEDATVersion=pMFEDATVersion, stFtpBytesToClient=stFtpBytesToClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyReason=notifyReason, mwgTrapVariables=mwgTrapVariables, stConnectionsBlocked=stConnectionsBlocked, kCompanyName=kCompanyName, stClientCount=stClientCount, notifyAffectedHost=notifyAffectedHost, stCategoriesEntry=stCategoriesEntry, notifySeverity=notifySeverity, stBlockedByURLFilter=stBlockedByURLFilter, trUser=trUser, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsBytesToClient=stHttpsBytesToClient, stFtpTraffic=stFtpTraffic, stFtpBytesFromClient=stFtpBytesFromClient, notifyOrigin=notifyOrigin, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgHttp=mwgHttp, mwgInfo=mwgInfo, kProductVersion=kProductVersion, mwgFTP=mwgFTP, mwgTraps=mwgTraps)
