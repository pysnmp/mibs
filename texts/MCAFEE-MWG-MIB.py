...
mib_2, Gauge32, iso, MibIdentifier, NotificationType, NotificationType, Counter32, Counter64, Bits, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, TimeTicks, IpAddress, Unsigned32, Integer32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "mib-2", "Gauge32", "iso", "MibIdentifier", "NotificationType", "NotificationType", "Counter32", "Counter64", "Bits", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "TimeTicks", "IpAddress", "Unsigned32", "Integer32", "ModuleIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsRequests=stHttpsRequests, stHttpBytesToServer=stHttpBytesToServer, notifyAffectedHost=notifyAffectedHost, kRevision=kRevision, stCategoriesTable=stCategoriesTable, kHotfixVersion=kHotfixVersion, kCompanyName=kCompanyName, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFtpBytesFromClient=stFtpBytesFromClient, stClientCount=stClientCount, mwgFTP=mwgFTP, kProductVersion=kProductVersion, stCPUIOWait=stCPUIOWait, stFtpBytesFromServer=stFtpBytesFromServer, mwg=mwg, stFtpTraffic=stFtpTraffic, notifyOrigin=notifyOrigin, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpsTraffic=stHttpsTraffic, kBuildNumber=kBuildNumber, stTimeForTransaction=stTimeForTransaction, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kMajorVersion=kMajorVersion, stHttpBytesFromServer=stHttpBytesFromServer, notifySeverity=notifySeverity, notifyReason=notifyReason, pMFEEngineVersion=pMFEEngineVersion, stBadReputation=stBadReputation, stHttpsBytesToServer=stHttpsBytesToServer, stConnectionsBlocked=stConnectionsBlocked, stHttpBytesToClient=stHttpBytesToClient, pMFEDATVersion=pMFEDATVersion, stFtpBytesToClient=stFtpBytesToClient, stMimeType=stMimeType, mwgTraps=mwgTraps, stMalwareDetected=stMalwareDetected, stFtpBytesToServer=stFtpBytesToServer, notifyOriginName=notifyOriginName, mwgInfo=mwgInfo, mwgMiscellaneous=mwgMiscellaneous, stCPULoad=stCPULoad, stHandleConnectToServer=stHandleConnectToServer, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgTrapVariables=mwgTrapVariables, trSystem=trSystem, stCategoryCount=stCategoryCount, PYSNMP_MODULE_ID=mwg, stHttpsBytesToClient=stHttpsBytesToClient, notifyAdditional=notifyAdditional, kMinorVersion=kMinorVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, kCustomVersion=kCustomVersion, pAMProactiveVersion=pAMProactiveVersion, mwgHttp=mwgHttp, stCPULoadRaw=stCPULoadRaw, trApplication=trApplication, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgContent=mwgContent, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategoriesEntry=stCategoriesEntry, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyReasonString=notifyReasonString, stBlockedByURLFilter=stBlockedByURLFilter, trUser=trUser, stResolveHostViaDNS=stResolveHostViaDNS, stHttpRequests=stHttpRequests, mwgEngineVersions=mwgEngineVersions, stConnectionsLegitimate=stConnectionsLegitimate, stBlockedByMediaFilter=stBlockedByMediaFilter, pTSDBVersion=pTSDBVersion, kMicroVersion=kMicroVersion, stCategoryName=stCategoryName, mwgHttps=mwgHttps, pAMEngineVersion=pAMEngineVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stMemConsumed=stMemConsumed, kProductName=kProductName, stHttpBytesFromClient=stHttpBytesFromClient, stCategories=stCategories, stConnectedSockets=stConnectedSockets, mwgStatistics=mwgStatistics, stHttpTraffic=stHttpTraffic)
