...
Integer32, Counter32, Gauge32, enterprises, TimeTicks, Counter64, Unsigned32, mib_2, ModuleIdentity, NotificationType, NotificationType, IpAddress, iso, Bits, ObjectIdentity, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Counter32", "Gauge32", "enterprises", "TimeTicks", "Counter64", "Unsigned32", "mib-2", "ModuleIdentity", "NotificationType", "NotificationType", "IpAddress", "iso", "Bits", "ObjectIdentity", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategoriesEntry=stCategoriesEntry, stConnectionsBlocked=stConnectionsBlocked, mwgFTP=mwgFTP, mwgMiscellaneous=mwgMiscellaneous, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kCustomVersion=kCustomVersion, kProductVersion=kProductVersion, stMemConsumed=stMemConsumed, stCPULoad=stCPULoad, stCategoryName=stCategoryName, kCompanyName=kCompanyName, pAMSignatureVersion=pAMSignatureVersion, stMimeType=stMimeType, stHttpsBytesFromServer=stHttpsBytesFromServer, kMinorVersion=kMinorVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsRequests=stHttpsRequests, trApplication=trApplication, kProductName=kProductName, kMicroVersion=kMicroVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, trUser=trUser, stClientCount=stClientCount, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpBytesToClient=stFtpBytesToClient, stFtpBytesToServer=stFtpBytesToServer, notifyReason=notifyReason, notifyReasonString=notifyReasonString, stHttpRequests=stHttpRequests, stHandleConnectToServer=stHandleConnectToServer, notifyAdditional=notifyAdditional, notifySeverity=notifySeverity, notifyAffectedHost=notifyAffectedHost, stFtpTraffic=stFtpTraffic, stHttpsBytesFromClient=stHttpsBytesFromClient, pMFEEngineVersion=pMFEEngineVersion, stTimeForTransaction=stTimeForTransaction, stConnectedSockets=stConnectedSockets, mwgEngineVersions=mwgEngineVersions, trSystem=trSystem, pAMProactiveVersion=pAMProactiveVersion, stResolveHostViaDNS=stResolveHostViaDNS, mwgHttp=mwgHttp, stHttpsBytesToServer=stHttpsBytesToServer, stHttpTraffic=stHttpTraffic, stHttpBytesToServer=stHttpBytesToServer, mwgTraps=mwgTraps, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategories=stCategories, kRevision=kRevision, notifyOrigin=notifyOrigin, stHttpBytesFromClient=stHttpBytesFromClient, stBlockedByURLFilter=stBlockedByURLFilter, mwgStatistics=mwgStatistics, mwgContent=mwgContent, mwg=mwg, stFtpBytesFromServer=stFtpBytesFromServer, stCPUIOWait=stCPUIOWait, stFtpBytesFromClient=stFtpBytesFromClient, mwgHttps=mwgHttps, stBlockedByAntiMalware=stBlockedByAntiMalware, kHotfixVersion=kHotfixVersion, kBuildNumber=kBuildNumber, stCPULoadRaw=stCPULoadRaw, pMFEDATVersion=pMFEDATVersion, stHttpBytesToClient=stHttpBytesToClient, stBadReputation=stBadReputation, mwgInfo=mwgInfo, stCategoriesTable=stCategoriesTable, stHttpBytesFromServer=stHttpBytesFromServer, stHttpsTraffic=stHttpsTraffic, stConnectionsLegitimate=stConnectionsLegitimate, PYSNMP_MODULE_ID=mwg, notifyOriginName=notifyOriginName, stCategoryCount=stCategoryCount, pAMEngineVersion=pAMEngineVersion, mwgTrapVariables=mwgTrapVariables, stMalwareDetected=stMalwareDetected, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpsBytesToClient=stHttpsBytesToClient, kMajorVersion=kMajorVersion, pTSDBVersion=pTSDBVersion)
