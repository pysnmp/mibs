...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pAMProactiveVersion=pAMProactiveVersion, trSystem=trSystem, kCompanyName=kCompanyName, mwg=mwg, stMimeType=stMimeType, stFtpTraffic=stFtpTraffic, mwgContent=mwgContent, stMemConsumed=stMemConsumed, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpBytesFromClient=stHttpBytesFromClient, kBuildNumber=kBuildNumber, stHttpBytesFromServer=stHttpBytesFromServer, kRevision=kRevision, trUser=trUser, stHttpsRequests=stHttpsRequests, stFtpBytesToClient=stFtpBytesToClient, stMalwareDetected=stMalwareDetected, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsBytesToServer=stHttpsBytesToServer, stBlockedByAntiMalware=stBlockedByAntiMalware, pAMEngineVersion=pAMEngineVersion, stHttpsTraffic=stHttpsTraffic, trApplication=trApplication, stCategoryName=stCategoryName, stResolveHostViaDNS=stResolveHostViaDNS, notifyAdditional=notifyAdditional, kProductName=kProductName, notifySeverity=notifySeverity, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pTSDBVersion=pTSDBVersion, kMajorVersion=kMajorVersion, stCPULoad=stCPULoad, mwgTrapVariables=mwgTrapVariables, notifyReasonString=notifyReasonString, stHttpsBytesFromClient=stHttpsBytesFromClient, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpBytesToClient=stHttpBytesToClient, stConnectedSockets=stConnectedSockets, mwgEngineVersions=mwgEngineVersions, stFtpBytesFromServer=stFtpBytesFromServer, stHttpRequests=stHttpRequests, pMFEDATVersion=pMFEDATVersion, stClientCount=stClientCount, notifyOrigin=notifyOrigin, stCPUIOWait=stCPUIOWait, stConnectionsBlocked=stConnectionsBlocked, kMinorVersion=kMinorVersion, stHandleConnectToServer=stHandleConnectToServer, mwgMiscellaneous=mwgMiscellaneous, stCategoriesTable=stCategoriesTable, kHotfixVersion=kHotfixVersion, stTimeForTransaction=stTimeForTransaction, stLastSentLastReceivedClient=stLastSentLastReceivedClient, pAMSignatureVersion=pAMSignatureVersion, mwgHttp=mwgHttp, mwgStatistics=mwgStatistics, stFtpBytesFromClient=stFtpBytesFromClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCategoriesEntry=stCategoriesEntry, kMicroVersion=kMicroVersion, PYSNMP_MODULE_ID=mwg, stFtpBytesToServer=stFtpBytesToServer, kCustomVersion=kCustomVersion, kProductVersion=kProductVersion, stBadReputation=stBadReputation, stHttpTraffic=stHttpTraffic, stBlockedByURLFilter=stBlockedByURLFilter, stCPULoadRaw=stCPULoadRaw, mwgInfo=mwgInfo, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesToServer=stHttpBytesToServer, mwgTraps=mwgTraps, notifyReason=notifyReason, pMFEEngineVersion=pMFEEngineVersion, notifyOriginName=notifyOriginName, stConnectionsLegitimate=stConnectionsLegitimate, notifyAffectedHost=notifyAffectedHost, stCategories=stCategories, mwgFTP=mwgFTP, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgHttps=mwgHttps, stCategoryCount=stCategoryCount)
