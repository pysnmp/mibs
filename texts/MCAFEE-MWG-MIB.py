...
enterprises, ModuleIdentity, Integer32, NotificationType, MibIdentifier, IpAddress, iso, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, mib_2, Counter32, Bits, Counter64, TimeTicks, ObjectIdentity, Unsigned32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "ModuleIdentity", "Integer32", "NotificationType", "MibIdentifier", "IpAddress", "iso", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "mib-2", "Counter32", "Bits", "Counter64", "TimeTicks", "ObjectIdentity", "Unsigned32", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpBytesToClient=stFtpBytesToClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stMimeType=stMimeType, pAMProactiveVersion=pAMProactiveVersion, mwgMiscellaneous=mwgMiscellaneous, stCategoryCount=stCategoryCount, stLastSentLastReceivedServer=stLastSentLastReceivedServer, PYSNMP_MODULE_ID=mwg, kCustomVersion=kCustomVersion, notifyAffectedHost=notifyAffectedHost, mwgTrapVariables=mwgTrapVariables, pMFEEngineVersion=pMFEEngineVersion, stCategoriesEntry=stCategoriesEntry, stBlockedByURLFilter=stBlockedByURLFilter, stConnectedSockets=stConnectedSockets, stFtpBytesFromClient=stFtpBytesFromClient, stCPULoadRaw=stCPULoadRaw, pAMSignatureVersion=pAMSignatureVersion, kMicroVersion=kMicroVersion, stHttpsBytesToClient=stHttpsBytesToClient, kProductName=kProductName, stCategoryName=stCategoryName, stHttpsBytesToServer=stHttpsBytesToServer, stMalwareDetected=stMalwareDetected, trUser=trUser, mwgContent=mwgContent, notifyReason=notifyReason, mwgHttp=mwgHttp, stBlockedByMediaFilter=stBlockedByMediaFilter, stClientCount=stClientCount, stCPUIOWait=stCPUIOWait, stHttpBytesToClient=stHttpBytesToClient, stTimeForTransaction=stTimeForTransaction, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsTraffic=stHttpsTraffic, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsRequests=stHttpsRequests, stHttpRequests=stHttpRequests, kHotfixVersion=kHotfixVersion, stConnectionsBlocked=stConnectionsBlocked, mwgFTP=mwgFTP, stFtpTraffic=stFtpTraffic, stCategoriesTable=stCategoriesTable, stFtpBytesToServer=stFtpBytesToServer, stBadReputation=stBadReputation, mwgHttps=mwgHttps, stCategories=stCategories, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpBytesToServer=stHttpBytesToServer, stHttpsBytesFromClient=stHttpsBytesFromClient, pMFEDATVersion=pMFEDATVersion, pTSDBVersion=pTSDBVersion, stHttpBytesFromServer=stHttpBytesFromServer, stResolveHostViaDNS=stResolveHostViaDNS, notifyOriginName=notifyOriginName, notifySeverity=notifySeverity, trApplication=trApplication, kProductVersion=kProductVersion, stHttpBytesFromClient=stHttpBytesFromClient, stCPULoad=stCPULoad, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stMemConsumed=stMemConsumed, mwgTraps=mwgTraps, kRevision=kRevision, kMajorVersion=kMajorVersion, mwg=mwg, mwgEngineVersions=mwgEngineVersions, stHttpTraffic=stHttpTraffic, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kMinorVersion=kMinorVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, kCompanyName=kCompanyName, pAMEngineVersion=pAMEngineVersion, notifyOrigin=notifyOrigin, trSystem=trSystem, stFtpBytesFromServer=stFtpBytesFromServer, notifyReasonString=notifyReasonString, mwgStatistics=mwgStatistics, notifyAdditional=notifyAdditional, mwgInfo=mwgInfo, stHandleConnectToServer=stHandleConnectToServer, stBlockedByAntiMalware=stBlockedByAntiMalware, kBuildNumber=kBuildNumber)
