...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsBytesFromClient=stHttpsBytesFromClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifySeverity=notifySeverity, stCategoryName=stCategoryName, stCPUIOWait=stCPUIOWait, stHttpRequests=stHttpRequests, PYSNMP_MODULE_ID=mwg, stConnectionsBlocked=stConnectionsBlocked, stHttpsRequests=stHttpsRequests, pAMEngineVersion=pAMEngineVersion, stFtpBytesFromClient=stFtpBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyOrigin=notifyOrigin, stTimeForTransaction=stTimeForTransaction, pAMSignatureVersion=pAMSignatureVersion, trApplication=trApplication, stFtpBytesToClient=stFtpBytesToClient, stCategories=stCategories, mwgTrapVariables=mwgTrapVariables, stHttpBytesFromServer=stHttpBytesFromServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kHotfixVersion=kHotfixVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyReasonString=notifyReasonString, kCustomVersion=kCustomVersion, stFtpBytesFromServer=stFtpBytesFromServer, stHttpTraffic=stHttpTraffic, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpBytesToClient=stHttpBytesToClient, trSystem=trSystem, kMinorVersion=kMinorVersion, stHttpsTraffic=stHttpsTraffic, kMicroVersion=kMicroVersion, notifyAffectedHost=notifyAffectedHost, stMalwareDetected=stMalwareDetected, stCPULoad=stCPULoad, mwgHttp=mwgHttp, pMFEDATVersion=pMFEDATVersion, stMimeType=stMimeType, stHttpsBytesToClient=stHttpsBytesToClient, mwgStatistics=mwgStatistics, kRevision=kRevision, stCategoriesTable=stCategoriesTable, kBuildNumber=kBuildNumber, trUser=trUser, kProductName=kProductName, stMemConsumed=stMemConsumed, mwgInfo=mwgInfo, mwgContent=mwgContent, kCompanyName=kCompanyName, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpBytesToServer=stHttpBytesToServer, stCPULoadRaw=stCPULoadRaw, stHttpBytesFromClient=stHttpBytesFromClient, stCategoryCount=stCategoryCount, stBlockedByURLFilter=stBlockedByURLFilter, mwgHttps=mwgHttps, stCategoriesEntry=stCategoriesEntry, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgEngineVersions=mwgEngineVersions, stClientCount=stClientCount, stBadReputation=stBadReputation, mwg=mwg, pAMProactiveVersion=pAMProactiveVersion, stHttpsBytesToServer=stHttpsBytesToServer, notifyAdditional=notifyAdditional, pMFEEngineVersion=pMFEEngineVersion, notifyReason=notifyReason, stFtpTraffic=stFtpTraffic, mwgTraps=mwgTraps, stResolveHostViaDNS=stResolveHostViaDNS, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kProductVersion=kProductVersion, notifyOriginName=notifyOriginName, stHttpsBytesFromServer=stHttpsBytesFromServer, stConnectedSockets=stConnectedSockets, mwgFTP=mwgFTP, pTSDBVersion=pTSDBVersion, stFtpBytesToServer=stFtpBytesToServer, mwgMiscellaneous=mwgMiscellaneous, kMajorVersion=kMajorVersion, stHandleConnectToServer=stHandleConnectToServer)
