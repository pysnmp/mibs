...
mib_2, Integer32, Unsigned32, IpAddress, iso, MibIdentifier, TimeTicks, Gauge32, Counter64, NotificationType, ModuleIdentity, Bits, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, enterprises, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "mib-2", "Integer32", "Unsigned32", "IpAddress", "iso", "MibIdentifier", "TimeTicks", "Gauge32", "Counter64", "NotificationType", "ModuleIdentity", "Bits", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "enterprises", "Counter32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kMajorVersion=kMajorVersion, stCategoriesEntry=stCategoriesEntry, stHttpsBytesFromServer=stHttpsBytesFromServer, notifyAdditional=notifyAdditional, stHttpBytesToServer=stHttpBytesToServer, pAMEngineVersion=pAMEngineVersion, stCPULoad=stCPULoad, mwgInfo=mwgInfo, mwgEngineVersions=mwgEngineVersions, kRevision=kRevision, stClientCount=stClientCount, stCategoryCount=stCategoryCount, pMFEEngineVersion=pMFEEngineVersion, mwgTrapVariables=mwgTrapVariables, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgTraps=mwgTraps, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgMiscellaneous=mwgMiscellaneous, mwgContent=mwgContent, kCompanyName=kCompanyName, pAMSignatureVersion=pAMSignatureVersion, stCategories=stCategories, PYSNMP_MODULE_ID=mwg, mwgHttp=mwgHttp, kMinorVersion=kMinorVersion, stCategoryName=stCategoryName, stTimeForTransaction=stTimeForTransaction, trSystem=trSystem, notifyAffectedHost=notifyAffectedHost, stMemConsumed=stMemConsumed, stHttpsBytesFromClient=stHttpsBytesFromClient, kBuildNumber=kBuildNumber, pTSDBVersion=pTSDBVersion, stHttpsRequests=stHttpsRequests, pMFEDATVersion=pMFEDATVersion, stHttpBytesFromServer=stHttpBytesFromServer, stFtpTraffic=stFtpTraffic, trApplication=trApplication, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsTraffic=stHttpsTraffic, stFtpBytesToClient=stFtpBytesToClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectionsLegitimate=stConnectionsLegitimate, stHttpRequests=stHttpRequests, stHttpBytesFromClient=stHttpBytesFromClient, stConnectedSockets=stConnectedSockets, stHttpsBytesToClient=stHttpsBytesToClient, mwgHttps=mwgHttps, stHandleConnectToServer=stHandleConnectToServer, mwgFTP=mwgFTP, kProductVersion=kProductVersion, kHotfixVersion=kHotfixVersion, mwg=mwg, stFtpBytesToServer=stFtpBytesToServer, notifyReason=notifyReason, trUser=trUser, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgStatistics=mwgStatistics, pAMProactiveVersion=pAMProactiveVersion, stCPUIOWait=stCPUIOWait, stBadReputation=stBadReputation, stFtpBytesFromClient=stFtpBytesFromClient, stBlockedByURLFilter=stBlockedByURLFilter, stCPULoadRaw=stCPULoadRaw, notifyReasonString=notifyReasonString, notifyOriginName=notifyOriginName, stFtpBytesFromServer=stFtpBytesFromServer, stHttpBytesToClient=stHttpBytesToClient, stConnectionsBlocked=stConnectionsBlocked, stHttpTraffic=stHttpTraffic, stMalwareDetected=stMalwareDetected, kMicroVersion=kMicroVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, notifySeverity=notifySeverity, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyOrigin=notifyOrigin, kCustomVersion=kCustomVersion, stMimeType=stMimeType, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kProductName=kProductName, stBlockedByAntiMalware=stBlockedByAntiMalware, stResolveHostViaDNS=stResolveHostViaDNS, stCategoriesTable=stCategoriesTable)
