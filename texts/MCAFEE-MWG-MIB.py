...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpBytesFromServer=stHttpBytesFromServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwg=mwg, stFtpTraffic=stFtpTraffic, notifyAffectedHost=notifyAffectedHost, stFtpBytesToClient=stFtpBytesToClient, stCPUIOWait=stCPUIOWait, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, pAMEngineVersion=pAMEngineVersion, pTSDBVersion=pTSDBVersion, notifyOriginName=notifyOriginName, kHotfixVersion=kHotfixVersion, stBlockedByURLFilter=stBlockedByURLFilter, stConnectionsLegitimate=stConnectionsLegitimate, notifySeverity=notifySeverity, stHttpsTraffic=stHttpsTraffic, stBlockedByAntiMalware=stBlockedByAntiMalware, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kBuildNumber=kBuildNumber, pAMProactiveVersion=pAMProactiveVersion, mwgMiscellaneous=mwgMiscellaneous, kMinorVersion=kMinorVersion, stFtpBytesFromClient=stFtpBytesFromClient, stHttpsBytesToClient=stHttpsBytesToClient, stMemConsumed=stMemConsumed, stHttpBytesToServer=stHttpBytesToServer, kProductVersion=kProductVersion, stHandleConnectToServer=stHandleConnectToServer, trSystem=trSystem, kMicroVersion=kMicroVersion, mwgHttps=mwgHttps, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgTrapVariables=mwgTrapVariables, stCategoryName=stCategoryName, stCategoriesTable=stCategoriesTable, stHttpBytesToClient=stHttpBytesToClient, notifyAdditional=notifyAdditional, pAMSignatureVersion=pAMSignatureVersion, stBadReputation=stBadReputation, mwgFTP=mwgFTP, trUser=trUser, mwgStatistics=mwgStatistics, stMimeType=stMimeType, notifyReason=notifyReason, stFtpBytesFromServer=stFtpBytesFromServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, pMFEDATVersion=pMFEDATVersion, kCompanyName=kCompanyName, stHttpsRequests=stHttpsRequests, stFtpBytesToServer=stFtpBytesToServer, stTimeForTransaction=stTimeForTransaction, stHttpTraffic=stHttpTraffic, kRevision=kRevision, stClientCount=stClientCount, pMFEEngineVersion=pMFEEngineVersion, stCategoriesEntry=stCategoriesEntry, PYSNMP_MODULE_ID=mwg, stHttpRequests=stHttpRequests, mwgContent=mwgContent, stConnectedSockets=stConnectedSockets, kMajorVersion=kMajorVersion, stCPULoadRaw=stCPULoadRaw, stHttpBytesFromClient=stHttpBytesFromClient, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgTraps=mwgTraps, trApplication=trApplication, stCategories=stCategories, mwgEngineVersions=mwgEngineVersions, stCPULoad=stCPULoad, notifyOrigin=notifyOrigin, kCustomVersion=kCustomVersion, mwgInfo=mwgInfo, kProductName=kProductName, stResolveHostViaDNS=stResolveHostViaDNS, stMalwareDetected=stMalwareDetected, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectionsBlocked=stConnectionsBlocked, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsBytesToServer=stHttpsBytesToServer, stCategoryCount=stCategoryCount, mwgHttp=mwgHttp, stHttpsBytesFromClient=stHttpsBytesFromClient, notifyReasonString=notifyReasonString)
