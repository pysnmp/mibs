...
enterprises, Counter32, Gauge32, Integer32, TimeTicks, NotificationType, mib_2, Unsigned32, IpAddress, NotificationType, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, iso, ModuleIdentity, ObjectIdentity, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Counter32", "Gauge32", "Integer32", "TimeTicks", "NotificationType", "mib-2", "Unsigned32", "IpAddress", "NotificationType", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "iso", "ModuleIdentity", "ObjectIdentity", "Counter64")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stTimeForTransaction=stTimeForTransaction, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCategories=stCategories, stConnectionsBlocked=stConnectionsBlocked, stCategoriesEntry=stCategoriesEntry, kMicroVersion=kMicroVersion, kMinorVersion=kMinorVersion, kProductName=kProductName, stCategoryName=stCategoryName, mwgHttps=mwgHttps, trSystem=trSystem, stHttpBytesToServer=stHttpBytesToServer, stCPUIOWait=stCPUIOWait, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpBytesFromClient=stHttpBytesFromClient, stCategoryCount=stCategoryCount, stHttpsRequests=stHttpsRequests, notifyAffectedHost=notifyAffectedHost, pAMEngineVersion=pAMEngineVersion, stHttpsBytesToServer=stHttpsBytesToServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsBytesFromClient=stHttpsBytesFromClient, pTSDBVersion=pTSDBVersion, stFtpBytesToClient=stFtpBytesToClient, pAMSignatureVersion=pAMSignatureVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, trApplication=trApplication, notifyOriginName=notifyOriginName, stCPULoadRaw=stCPULoadRaw, trUser=trUser, pMFEEngineVersion=pMFEEngineVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stBlockedByURLFilter=stBlockedByURLFilter, mwgHttp=mwgHttp, stHttpsBytesToClient=stHttpsBytesToClient, kBuildNumber=kBuildNumber, pAMProactiveVersion=pAMProactiveVersion, stFtpBytesFromClient=stFtpBytesFromClient, stMalwareDetected=stMalwareDetected, mwgEngineVersions=mwgEngineVersions, kRevision=kRevision, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyOrigin=notifyOrigin, stHttpsTraffic=stHttpsTraffic, stConnectedSockets=stConnectedSockets, stHttpBytesFromServer=stHttpBytesFromServer, mwg=mwg, stCPULoad=stCPULoad, stHttpTraffic=stHttpTraffic, kCustomVersion=kCustomVersion, stConnectionsLegitimate=stConnectionsLegitimate, stFtpBytesFromServer=stFtpBytesFromServer, mwgMiscellaneous=mwgMiscellaneous, stBadReputation=stBadReputation, stFtpTraffic=stFtpTraffic, kProductVersion=kProductVersion, stFtpBytesToServer=stFtpBytesToServer, stResolveHostViaDNS=stResolveHostViaDNS, mwgTrapVariables=mwgTrapVariables, mwgFTP=mwgFTP, notifyAdditional=notifyAdditional, stMimeType=stMimeType, stHttpRequests=stHttpRequests, stHandleConnectToServer=stHandleConnectToServer, notifySeverity=notifySeverity, stCategoriesTable=stCategoriesTable, kCompanyName=kCompanyName, mwgStatistics=mwgStatistics, kMajorVersion=kMajorVersion, mwgContent=mwgContent, stHttpBytesToClient=stHttpBytesToClient, mwgTraps=mwgTraps, stHttpsBytesFromServer=stHttpsBytesFromServer, stClientCount=stClientCount, pMFEDATVersion=pMFEDATVersion, PYSNMP_MODULE_ID=mwg, stBlockedByMediaFilter=stBlockedByMediaFilter, kHotfixVersion=kHotfixVersion, mwgInfo=mwgInfo, notifyReason=notifyReason, notifyReasonString=notifyReasonString, stMemConsumed=stMemConsumed, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer)
