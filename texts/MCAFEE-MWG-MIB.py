...
enterprises, NotificationType, Gauge32, mib_2, NotificationType, ObjectIdentity, iso, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Counter64, IpAddress, TimeTicks, MibIdentifier, Bits, ModuleIdentity, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "NotificationType", "Gauge32", "mib-2", "NotificationType", "ObjectIdentity", "iso", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Counter64", "IpAddress", "TimeTicks", "MibIdentifier", "Bits", "ModuleIdentity", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stMemConsumed=stMemConsumed, stHttpBytesToClient=stHttpBytesToClient, kCustomVersion=kCustomVersion, stCPULoad=stCPULoad, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCategoriesTable=stCategoriesTable, notifySeverity=notifySeverity, mwgInfo=mwgInfo, stConnectionsBlocked=stConnectionsBlocked, notifyOriginName=notifyOriginName, stCategoryName=stCategoryName, stBlockedByMediaFilter=stBlockedByMediaFilter, mwg=mwg, stFtpTraffic=stFtpTraffic, notifyReasonString=notifyReasonString, stCPUIOWait=stCPUIOWait, stHttpsBytesFromServer=stHttpsBytesFromServer, stFtpBytesToServer=stFtpBytesToServer, stFtpBytesFromServer=stFtpBytesFromServer, stBlockedByAntiMalware=stBlockedByAntiMalware, kCompanyName=kCompanyName, stHttpBytesToServer=stHttpBytesToServer, stFtpBytesToClient=stFtpBytesToClient, mwgEngineVersions=mwgEngineVersions, kBuildNumber=kBuildNumber, stHttpTraffic=stHttpTraffic, pMFEEngineVersion=pMFEEngineVersion, trUser=trUser, notifyOrigin=notifyOrigin, notifyAdditional=notifyAdditional, stMimeType=stMimeType, stFtpBytesFromClient=stFtpBytesFromClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stClientCount=stClientCount, mwgContent=mwgContent, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpBytesFromServer=stHttpBytesFromServer, stHttpBytesFromClient=stHttpBytesFromClient, stTimeForTransaction=stTimeForTransaction, kHotfixVersion=kHotfixVersion, stBadReputation=stBadReputation, stConnectionsLegitimate=stConnectionsLegitimate, mwgHttps=mwgHttps, mwgHttp=mwgHttp, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stResolveHostViaDNS=stResolveHostViaDNS, stCategoryCount=stCategoryCount, mwgStatistics=mwgStatistics, stConnectedSockets=stConnectedSockets, kMajorVersion=kMajorVersion, kMinorVersion=kMinorVersion, pAMEngineVersion=pAMEngineVersion, PYSNMP_MODULE_ID=mwg, notifyAffectedHost=notifyAffectedHost, stHttpsTraffic=stHttpsTraffic, mwgTrapVariables=mwgTrapVariables, stHttpsRequests=stHttpsRequests, stCategoriesEntry=stCategoriesEntry, stCategories=stCategories, stHandleConnectToServer=stHandleConnectToServer, mwgTraps=mwgTraps, trSystem=trSystem, pAMSignatureVersion=pAMSignatureVersion, stMalwareDetected=stMalwareDetected, kRevision=kRevision, stBlockedByURLFilter=stBlockedByURLFilter, kProductName=kProductName, mwgFTP=mwgFTP, kMicroVersion=kMicroVersion, pTSDBVersion=pTSDBVersion, stCPULoadRaw=stCPULoadRaw, pAMProactiveVersion=pAMProactiveVersion, notifyReason=notifyReason, mwgMiscellaneous=mwgMiscellaneous, kProductVersion=kProductVersion, stHttpRequests=stHttpRequests, pMFEDATVersion=pMFEDATVersion, stHttpsBytesToClient=stHttpsBytesToClient, trApplication=trApplication, stHttpsBytesToServer=stHttpsBytesToServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsBytesFromClient=stHttpsBytesFromClient)
