...
Counter32, Bits, mib_2, MibIdentifier, ObjectIdentity, NotificationType, IpAddress, ModuleIdentity, NotificationType, Integer32, Unsigned32, enterprises, TimeTicks, Counter64, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "Bits", "mib-2", "MibIdentifier", "ObjectIdentity", "NotificationType", "IpAddress", "ModuleIdentity", "NotificationType", "Integer32", "Unsigned32", "enterprises", "TimeTicks", "Counter64", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kMinorVersion=kMinorVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, trUser=trUser, stTimeForTransaction=stTimeForTransaction, kProductVersion=kProductVersion, stMemConsumed=stMemConsumed, stCPULoadRaw=stCPULoadRaw, stHttpsBytesFromClient=stHttpsBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, PYSNMP_MODULE_ID=mwg, mwgInfo=mwgInfo, kBuildNumber=kBuildNumber, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgContent=mwgContent, mwgTrapVariables=mwgTrapVariables, notifyAffectedHost=notifyAffectedHost, stClientCount=stClientCount, stHandleConnectToServer=stHandleConnectToServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCategoriesEntry=stCategoriesEntry, stMimeType=stMimeType, kMajorVersion=kMajorVersion, trSystem=trSystem, trApplication=trApplication, stHttpTraffic=stHttpTraffic, stHttpBytesFromServer=stHttpBytesFromServer, stFtpBytesFromClient=stFtpBytesFromClient, pTSDBVersion=pTSDBVersion, stHttpsTraffic=stHttpsTraffic, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgHttps=mwgHttps, stFtpTraffic=stFtpTraffic, mwgTraps=mwgTraps, mwgFTP=mwgFTP, notifyOriginName=notifyOriginName, kCompanyName=kCompanyName, notifyAdditional=notifyAdditional, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stBadReputation=stBadReputation, stHttpBytesToClient=stHttpBytesToClient, stFtpBytesToClient=stFtpBytesToClient, stFtpBytesToServer=stFtpBytesToServer, notifyReasonString=notifyReasonString, stCPULoad=stCPULoad, stResolveHostViaDNS=stResolveHostViaDNS, stHttpsBytesToClient=stHttpsBytesToClient, notifySeverity=notifySeverity, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesToServer=stHttpBytesToServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyOrigin=notifyOrigin, stBlockedByAntiMalware=stBlockedByAntiMalware, pMFEEngineVersion=pMFEEngineVersion, mwgStatistics=mwgStatistics, stBlockedByURLFilter=stBlockedByURLFilter, kCustomVersion=kCustomVersion, kHotfixVersion=kHotfixVersion, stCategoriesTable=stCategoriesTable, pAMEngineVersion=pAMEngineVersion, notifyReason=notifyReason, mwgMiscellaneous=mwgMiscellaneous, pMFEDATVersion=pMFEDATVersion, stMalwareDetected=stMalwareDetected, stHttpsBytesToServer=stHttpsBytesToServer, stCPUIOWait=stCPUIOWait, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCategoryName=stCategoryName, mwg=mwg, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategories=stCategories, mwgHttp=mwgHttp, kRevision=kRevision, stConnectedSockets=stConnectedSockets, stHttpsRequests=stHttpsRequests, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesFromServer=stFtpBytesFromServer, kMicroVersion=kMicroVersion, stHttpRequests=stHttpRequests, kProductName=kProductName, mwgEngineVersions=mwgEngineVersions, stCategoryCount=stCategoryCount, stHttpBytesFromClient=stHttpBytesFromClient, pAMProactiveVersion=pAMProactiveVersion)
