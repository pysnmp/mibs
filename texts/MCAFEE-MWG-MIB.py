...
Integer32, mib_2, Gauge32, Bits, TimeTicks, NotificationType, Unsigned32, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, ModuleIdentity, MibIdentifier, NotificationType, IpAddress, enterprises, Counter64, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "mib-2", "Gauge32", "Bits", "TimeTicks", "NotificationType", "Unsigned32", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "ModuleIdentity", "MibIdentifier", "NotificationType", "IpAddress", "enterprises", "Counter64", "ObjectIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", notifyReason=notifyReason, stConnectedSockets=stConnectedSockets, mwgTrapVariables=mwgTrapVariables, stCategoryName=stCategoryName, kBuildNumber=kBuildNumber, stMimeType=stMimeType, stHttpsTraffic=stHttpsTraffic, PYSNMP_MODULE_ID=mwg, notifySeverity=notifySeverity, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyReasonString=notifyReasonString, stFtpBytesFromClient=stFtpBytesFromClient, stFtpBytesToServer=stFtpBytesToServer, kMicroVersion=kMicroVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, trUser=trUser, mwgInfo=mwgInfo, mwgStatistics=mwgStatistics, stCategories=stCategories, stMalwareDetected=stMalwareDetected, stHttpsBytesToClient=stHttpsBytesToClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpBytesToServer=stHttpBytesToServer, stHttpRequests=stHttpRequests, notifyOriginName=notifyOriginName, notifyAdditional=notifyAdditional, pTSDBVersion=pTSDBVersion, stMemConsumed=stMemConsumed, stResolveHostViaDNS=stResolveHostViaDNS, notifyOrigin=notifyOrigin, stHttpsRequests=stHttpsRequests, pMFEEngineVersion=pMFEEngineVersion, notifyAffectedHost=notifyAffectedHost, kCustomVersion=kCustomVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, trApplication=trApplication, stTimeForTransaction=stTimeForTransaction, mwgHttp=mwgHttp, mwgMiscellaneous=mwgMiscellaneous, kMajorVersion=kMajorVersion, kCompanyName=kCompanyName, stCategoriesEntry=stCategoriesEntry, mwgFTP=mwgFTP, pAMProactiveVersion=pAMProactiveVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgHttps=mwgHttps, stBadReputation=stBadReputation, stHttpsBytesFromClient=stHttpsBytesFromClient, mwgTraps=mwgTraps, stClientCount=stClientCount, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsBytesToServer=stHttpsBytesToServer, stFtpBytesFromServer=stFtpBytesFromServer, stCPUIOWait=stCPUIOWait, kProductVersion=kProductVersion, stHandleConnectToServer=stHandleConnectToServer, stHttpBytesToClient=stHttpBytesToClient, stBlockedByMediaFilter=stBlockedByMediaFilter, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesFromServer=stHttpBytesFromServer, pAMEngineVersion=pAMEngineVersion, kProductName=kProductName, stFtpTraffic=stFtpTraffic, pMFEDATVersion=pMFEDATVersion, stHttpTraffic=stHttpTraffic, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgEngineVersions=mwgEngineVersions, stConnectionsBlocked=stConnectionsBlocked, mwg=mwg, stCPULoad=stCPULoad, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, trSystem=trSystem, stConnectionsLegitimate=stConnectionsLegitimate, stCPULoadRaw=stCPULoadRaw, kMinorVersion=kMinorVersion, stCategoriesTable=stCategoriesTable, stCategoryCount=stCategoryCount, kRevision=kRevision, stFtpBytesToClient=stFtpBytesToClient, stHttpBytesFromClient=stHttpBytesFromClient, stBlockedByURLFilter=stBlockedByURLFilter, mwgContent=mwgContent, kHotfixVersion=kHotfixVersion)
