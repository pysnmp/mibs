...
Counter32, Bits, Unsigned32, mib_2, iso, Counter64, NotificationType, IpAddress, NotificationType, Gauge32, MibIdentifier, ModuleIdentity, enterprises, ObjectIdentity, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "Bits", "Unsigned32", "mib-2", "iso", "Counter64", "NotificationType", "IpAddress", "NotificationType", "Gauge32", "MibIdentifier", "ModuleIdentity", "enterprises", "ObjectIdentity", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsTraffic=stHttpsTraffic, notifyReasonString=notifyReasonString, trSystem=trSystem, pMFEDATVersion=pMFEDATVersion, stMalwareDetected=stMalwareDetected, stCategoriesTable=stCategoriesTable, kCompanyName=kCompanyName, pAMSignatureVersion=pAMSignatureVersion, mwgFTP=mwgFTP, stHttpTraffic=stHttpTraffic, stCategoriesEntry=stCategoriesEntry, notifyAffectedHost=notifyAffectedHost, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kCustomVersion=kCustomVersion, kMinorVersion=kMinorVersion, mwgTrapVariables=mwgTrapVariables, notifyOrigin=notifyOrigin, stCategoryCount=stCategoryCount, stFtpBytesFromServer=stFtpBytesFromServer, kRevision=kRevision, stHttpsBytesToClient=stHttpsBytesToClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpBytesToClient=stHttpBytesToClient, mwgMiscellaneous=mwgMiscellaneous, mwgInfo=mwgInfo, pTSDBVersion=pTSDBVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgHttp=mwgHttp, stCPULoad=stCPULoad, stHttpsBytesToServer=stHttpsBytesToServer, pMFEEngineVersion=pMFEEngineVersion, stFtpBytesToClient=stFtpBytesToClient, stCategories=stCategories, trUser=trUser, notifyReason=notifyReason, PYSNMP_MODULE_ID=mwg, mwgTraps=mwgTraps, stBadReputation=stBadReputation, stConnectedSockets=stConnectedSockets, stBlockedByURLFilter=stBlockedByURLFilter, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCategoryName=stCategoryName, stCPUIOWait=stCPUIOWait, trApplication=trApplication, kHotfixVersion=kHotfixVersion, mwgStatistics=mwgStatistics, stClientCount=stClientCount, mwgContent=mwgContent, stHttpBytesToServer=stHttpBytesToServer, stConnectionsBlocked=stConnectionsBlocked, stCPULoadRaw=stCPULoadRaw, mwgEngineVersions=mwgEngineVersions, stHttpRequests=stHttpRequests, stConnectionsLegitimate=stConnectionsLegitimate, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kProductVersion=kProductVersion, notifyOriginName=notifyOriginName, kProductName=kProductName, stBlockedByMediaFilter=stBlockedByMediaFilter, notifySeverity=notifySeverity, stHandleConnectToServer=stHandleConnectToServer, notifyAdditional=notifyAdditional, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stMimeType=stMimeType, stFtpBytesFromClient=stFtpBytesFromClient, pAMEngineVersion=pAMEngineVersion, stResolveHostViaDNS=stResolveHostViaDNS, stHttpBytesFromServer=stHttpBytesFromServer, stTimeForTransaction=stTimeForTransaction, stHttpsBytesFromServer=stHttpsBytesFromServer, stFtpTraffic=stFtpTraffic, kMajorVersion=kMajorVersion, stHttpBytesFromClient=stHttpBytesFromClient, stMemConsumed=stMemConsumed, mwgHttps=mwgHttps, pAMProactiveVersion=pAMProactiveVersion, stHttpsRequests=stHttpsRequests, mwg=mwg, stHttpsBytesFromClient=stHttpsBytesFromClient, stFtpBytesToServer=stFtpBytesToServer, kBuildNumber=kBuildNumber, stBlockedByAntiMalware=stBlockedByAntiMalware, kMicroVersion=kMicroVersion)
