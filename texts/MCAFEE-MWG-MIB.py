...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pTSDBVersion=pTSDBVersion, kBuildNumber=kBuildNumber, stBlockedByAntiMalware=stBlockedByAntiMalware, trApplication=trApplication, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kProductVersion=kProductVersion, pMFEEngineVersion=pMFEEngineVersion, stCPULoadRaw=stCPULoadRaw, stClientCount=stClientCount, stMimeType=stMimeType, kCustomVersion=kCustomVersion, stCategoriesTable=stCategoriesTable, mwgTraps=mwgTraps, PYSNMP_MODULE_ID=mwg, stConnectionsLegitimate=stConnectionsLegitimate, mwgFTP=mwgFTP, kCompanyName=kCompanyName, notifyOriginName=notifyOriginName, mwgHttp=mwgHttp, kRevision=kRevision, stFtpBytesFromClient=stFtpBytesFromClient, notifyReason=notifyReason, stCPUIOWait=stCPUIOWait, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwg=mwg, pAMSignatureVersion=pAMSignatureVersion, notifySeverity=notifySeverity, kMinorVersion=kMinorVersion, mwgStatistics=mwgStatistics, stHttpBytesFromClient=stHttpBytesFromClient, stCPULoad=stCPULoad, kHotfixVersion=kHotfixVersion, stMalwareDetected=stMalwareDetected, stConnectionsBlocked=stConnectionsBlocked, stBadReputation=stBadReputation, pAMProactiveVersion=pAMProactiveVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, trSystem=trSystem, stBlockedByURLFilter=stBlockedByURLFilter, stConnectedSockets=stConnectedSockets, stCategoryCount=stCategoryCount, trUser=trUser, stHttpBytesToClient=stHttpBytesToClient, stMemConsumed=stMemConsumed, notifyReasonString=notifyReasonString, stHttpsBytesToClient=stHttpsBytesToClient, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesToServer=stHttpsBytesToServer, stCategoriesEntry=stCategoriesEntry, stFtpBytesFromServer=stFtpBytesFromServer, stHttpsTraffic=stHttpsTraffic, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgContent=mwgContent, kProductName=kProductName, pAMEngineVersion=pAMEngineVersion, stHandleConnectToServer=stHandleConnectToServer, stHttpBytesToServer=stHttpBytesToServer, mwgHttps=mwgHttps, stTimeForTransaction=stTimeForTransaction, mwgTrapVariables=mwgTrapVariables, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCategoryName=stCategoryName, kMajorVersion=kMajorVersion, mwgEngineVersions=mwgEngineVersions, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpsBytesFromServer=stHttpsBytesFromServer, stFtpBytesToClient=stFtpBytesToClient, stHttpRequests=stHttpRequests, stResolveHostViaDNS=stResolveHostViaDNS, mwgMiscellaneous=mwgMiscellaneous, kMicroVersion=kMicroVersion, stHttpTraffic=stHttpTraffic, notifyOrigin=notifyOrigin, mwgInfo=mwgInfo, stHttpsRequests=stHttpsRequests, stCategories=stCategories, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpTraffic=stFtpTraffic, notifyAffectedHost=notifyAffectedHost, stHttpBytesFromServer=stHttpBytesFromServer, notifyAdditional=notifyAdditional, pMFEDATVersion=pMFEDATVersion, stFtpBytesToServer=stFtpBytesToServer)
