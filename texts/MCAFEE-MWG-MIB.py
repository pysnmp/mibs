...
enterprises, iso, NotificationType, Gauge32, NotificationType, MibIdentifier, ModuleIdentity, Integer32, Counter32, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, TimeTicks, ObjectIdentity, Bits, Unsigned32, mib_2 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "iso", "NotificationType", "Gauge32", "NotificationType", "MibIdentifier", "ModuleIdentity", "Integer32", "Counter32", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "TimeTicks", "ObjectIdentity", "Bits", "Unsigned32", "mib-2")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kMinorVersion=kMinorVersion, notifyReason=notifyReason, kMajorVersion=kMajorVersion, stCPUIOWait=stCPUIOWait, stTimeForTransaction=stTimeForTransaction, stHttpBytesFromClient=stHttpBytesFromClient, notifyOrigin=notifyOrigin, stHandleConnectToServer=stHandleConnectToServer, stCategoryCount=stCategoryCount, kCustomVersion=kCustomVersion, stMimeType=stMimeType, trUser=trUser, mwgFTP=mwgFTP, stFtpBytesFromClient=stFtpBytesFromClient, mwgHttps=mwgHttps, stConnectionsLegitimate=stConnectionsLegitimate, mwg=mwg, kRevision=kRevision, stFtpBytesFromServer=stFtpBytesFromServer, stHttpTraffic=stHttpTraffic, stHttpBytesToServer=stHttpBytesToServer, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyOriginName=notifyOriginName, stLastSentLastReceivedClient=stLastSentLastReceivedClient, pMFEEngineVersion=pMFEEngineVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgContent=mwgContent, stConnectedSockets=stConnectedSockets, notifySeverity=notifySeverity, pAMProactiveVersion=pAMProactiveVersion, kCompanyName=kCompanyName, stHttpsBytesToClient=stHttpsBytesToClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyAffectedHost=notifyAffectedHost, kProductVersion=kProductVersion, stResolveHostViaDNS=stResolveHostViaDNS, mwgHttp=mwgHttp, stCategoriesEntry=stCategoriesEntry, stCategoriesTable=stCategoriesTable, stHttpBytesFromServer=stHttpBytesFromServer, stConnectionsBlocked=stConnectionsBlocked, stMemConsumed=stMemConsumed, stFtpTraffic=stFtpTraffic, notifyAdditional=notifyAdditional, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgEngineVersions=mwgEngineVersions, mwgMiscellaneous=mwgMiscellaneous, stFtpBytesToClient=stFtpBytesToClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, pMFEDATVersion=pMFEDATVersion, notifyReasonString=notifyReasonString, stHttpsBytesToServer=stHttpsBytesToServer, mwgInfo=mwgInfo, stHttpsBytesFromClient=stHttpsBytesFromClient, kBuildNumber=kBuildNumber, mwgStatistics=mwgStatistics, stCPULoad=stCPULoad, stBadReputation=stBadReputation, stCategories=stCategories, mwgTrapVariables=mwgTrapVariables, stFtpBytesToServer=stFtpBytesToServer, stHttpsRequests=stHttpsRequests, stHttpRequests=stHttpRequests, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, trSystem=trSystem, kHotfixVersion=kHotfixVersion, kProductName=kProductName, stBlockedByURLFilter=stBlockedByURLFilter, PYSNMP_MODULE_ID=mwg, kMicroVersion=kMicroVersion, trApplication=trApplication, stHttpBytesToClient=stHttpBytesToClient, pAMSignatureVersion=pAMSignatureVersion, stHttpsTraffic=stHttpsTraffic, mwgTraps=mwgTraps, pAMEngineVersion=pAMEngineVersion, stCategoryName=stCategoryName, pTSDBVersion=pTSDBVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stClientCount=stClientCount, stMalwareDetected=stMalwareDetected, stCPULoadRaw=stCPULoadRaw)
