...
ObjectIdentity, Unsigned32, NotificationType, Gauge32, Counter32, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, Bits, enterprises, MibIdentifier, Counter64, TimeTicks, iso, mib_2, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Unsigned32", "NotificationType", "Gauge32", "Counter32", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "Bits", "enterprises", "MibIdentifier", "Counter64", "TimeTicks", "iso", "mib-2", "ModuleIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgHttps=mwgHttps, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesToServer=stHttpBytesToServer, trApplication=trApplication, stCategoryName=stCategoryName, stHandleConnectToServer=stHandleConnectToServer, pAMEngineVersion=pAMEngineVersion, stHttpBytesFromClient=stHttpBytesFromClient, stFtpBytesToServer=stFtpBytesToServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stCategories=stCategories, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgTrapVariables=mwgTrapVariables, kMajorVersion=kMajorVersion, mwgMiscellaneous=mwgMiscellaneous, kProductVersion=kProductVersion, stHttpBytesToClient=stHttpBytesToClient, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyAdditional=notifyAdditional, mwgFTP=mwgFTP, stMemConsumed=stMemConsumed, kRevision=kRevision, kMinorVersion=kMinorVersion, kMicroVersion=kMicroVersion, stTimeForTransaction=stTimeForTransaction, kCompanyName=kCompanyName, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpRequests=stHttpRequests, kBuildNumber=kBuildNumber, notifyReason=notifyReason, stBadReputation=stBadReputation, trUser=trUser, kCustomVersion=kCustomVersion, pTSDBVersion=pTSDBVersion, mwg=mwg, stFtpBytesToClient=stFtpBytesToClient, mwgEngineVersions=mwgEngineVersions, stMalwareDetected=stMalwareDetected, notifyReasonString=notifyReasonString, stHttpBytesFromServer=stHttpBytesFromServer, stCPUIOWait=stCPUIOWait, stCPULoad=stCPULoad, notifyOriginName=notifyOriginName, stConnectedSockets=stConnectedSockets, stMimeType=stMimeType, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsBytesFromClient=stHttpsBytesFromClient, stFtpBytesFromClient=stFtpBytesFromClient, stBlockedByURLFilter=stBlockedByURLFilter, stHttpsTraffic=stHttpsTraffic, pMFEDATVersion=pMFEDATVersion, stHttpTraffic=stHttpTraffic, PYSNMP_MODULE_ID=mwg, mwgHttp=mwgHttp, mwgContent=mwgContent, mwgTraps=mwgTraps, kProductName=kProductName, stConnectionsBlocked=stConnectionsBlocked, stConnectionsLegitimate=stConnectionsLegitimate, stResolveHostViaDNS=stResolveHostViaDNS, notifyAffectedHost=notifyAffectedHost, stBlockedByMediaFilter=stBlockedByMediaFilter, stCPULoadRaw=stCPULoadRaw, stCategoriesEntry=stCategoriesEntry, stHttpsBytesFromServer=stHttpsBytesFromServer, kHotfixVersion=kHotfixVersion, stFtpBytesFromServer=stFtpBytesFromServer, pAMProactiveVersion=pAMProactiveVersion, stHttpsRequests=stHttpsRequests, stHttpsBytesToServer=stHttpsBytesToServer, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifySeverity=notifySeverity, mwgInfo=mwgInfo, stClientCount=stClientCount, trSystem=trSystem, notifyOrigin=notifyOrigin, stFtpTraffic=stFtpTraffic, pAMSignatureVersion=pAMSignatureVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, pMFEEngineVersion=pMFEEngineVersion, mwgStatistics=mwgStatistics, stCategoryCount=stCategoryCount, stCategoriesTable=stCategoriesTable)
