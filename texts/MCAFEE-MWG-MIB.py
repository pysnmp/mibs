...
ModuleIdentity, iso, Bits, Gauge32, TimeTicks, ObjectIdentity, Counter32, Counter64, Unsigned32, enterprises, mib_2, NotificationType, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "iso", "Bits", "Gauge32", "TimeTicks", "ObjectIdentity", "Counter32", "Counter64", "Unsigned32", "enterprises", "mib-2", "NotificationType", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", notifyReasonString=notifyReasonString, stMalwareDetected=stMalwareDetected, stFtpBytesToClient=stFtpBytesToClient, stHttpBytesFromServer=stHttpBytesFromServer, stHttpBytesToClient=stHttpBytesToClient, kProductName=kProductName, mwgContent=mwgContent, kRevision=kRevision, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategoryCount=stCategoryCount, stFtpBytesFromClient=stFtpBytesFromClient, stHttpBytesFromClient=stHttpBytesFromClient, pMFEDATVersion=pMFEDATVersion, kBuildNumber=kBuildNumber, notifyAdditional=notifyAdditional, stCategoriesEntry=stCategoriesEntry, stHttpsBytesFromServer=stHttpsBytesFromServer, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesToServer=stHttpBytesToServer, stClientCount=stClientCount, stCPULoadRaw=stCPULoadRaw, mwgTraps=mwgTraps, kMicroVersion=kMicroVersion, mwgEngineVersions=mwgEngineVersions, stFtpTraffic=stFtpTraffic, kMinorVersion=kMinorVersion, notifyAffectedHost=notifyAffectedHost, PYSNMP_MODULE_ID=mwg, pAMProactiveVersion=pAMProactiveVersion, stHttpRequests=stHttpRequests, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCategories=stCategories, mwgFTP=mwgFTP, stResolveHostViaDNS=stResolveHostViaDNS, notifySeverity=notifySeverity, stBadReputation=stBadReputation, stConnectedSockets=stConnectedSockets, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwg=mwg, stMimeType=stMimeType, stTimeForTransaction=stTimeForTransaction, stCPULoad=stCPULoad, mwgStatistics=mwgStatistics, pAMEngineVersion=pAMEngineVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, trSystem=trSystem, kCustomVersion=kCustomVersion, stFtpBytesFromServer=stFtpBytesFromServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kMajorVersion=kMajorVersion, kHotfixVersion=kHotfixVersion, pMFEEngineVersion=pMFEEngineVersion, stCategoryName=stCategoryName, stHttpsTraffic=stHttpsTraffic, notifyOriginName=notifyOriginName, stMemConsumed=stMemConsumed, stFtpBytesToServer=stFtpBytesToServer, stBlockedByURLFilter=stBlockedByURLFilter, stBlockedByAntiMalware=stBlockedByAntiMalware, trUser=trUser, stCPUIOWait=stCPUIOWait, notifyOrigin=notifyOrigin, mwgTrapVariables=mwgTrapVariables, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpsBytesToServer=stHttpsBytesToServer, trApplication=trApplication, stHandleConnectToServer=stHandleConnectToServer, stHttpsBytesFromClient=stHttpsBytesFromClient, kProductVersion=kProductVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgHttp=mwgHttp, stConnectionsBlocked=stConnectionsBlocked, pTSDBVersion=pTSDBVersion, stHttpsBytesToClient=stHttpsBytesToClient, mwgHttps=mwgHttps, stHttpsRequests=stHttpsRequests, stCategoriesTable=stCategoriesTable, mwgMiscellaneous=mwgMiscellaneous, notifyReason=notifyReason, mwgInfo=mwgInfo, stConnectionsLegitimate=stConnectionsLegitimate, stHttpTraffic=stHttpTraffic, kCompanyName=kCompanyName)
