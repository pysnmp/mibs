...
NotificationType, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, mib_2, Counter32, ObjectIdentity, Counter64, Bits, MibIdentifier, TimeTicks, iso, Gauge32, NotificationType, enterprises, Integer32, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "mib-2", "Counter32", "ObjectIdentity", "Counter64", "Bits", "MibIdentifier", "TimeTicks", "iso", "Gauge32", "NotificationType", "enterprises", "Integer32", "IpAddress")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pAMSignatureVersion=pAMSignatureVersion, trUser=trUser, notifyAdditional=notifyAdditional, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesFromClient=stHttpsBytesFromClient, stCPUIOWait=stCPUIOWait, kMicroVersion=kMicroVersion, pAMProactiveVersion=pAMProactiveVersion, stResolveHostViaDNS=stResolveHostViaDNS, kHotfixVersion=kHotfixVersion, stCPULoad=stCPULoad, mwgInfo=mwgInfo, stHttpBytesFromServer=stHttpBytesFromServer, stBlockedByURLFilter=stBlockedByURLFilter, kBuildNumber=kBuildNumber, pTSDBVersion=pTSDBVersion, mwgStatistics=mwgStatistics, mwgHttps=mwgHttps, stHttpBytesToServer=stHttpBytesToServer, stConnectedSockets=stConnectedSockets, stFtpTraffic=stFtpTraffic, kCustomVersion=kCustomVersion, stHttpsTraffic=stHttpsTraffic, notifyReason=notifyReason, trApplication=trApplication, pMFEEngineVersion=pMFEEngineVersion, kProductName=kProductName, stMimeType=stMimeType, stHttpsBytesFromServer=stHttpsBytesFromServer, PYSNMP_MODULE_ID=mwg, stBadReputation=stBadReputation, stHttpsRequests=stHttpsRequests, stCPULoadRaw=stCPULoadRaw, kCompanyName=kCompanyName, stHandleConnectToServer=stHandleConnectToServer, notifySeverity=notifySeverity, stHttpRequests=stHttpRequests, stMemConsumed=stMemConsumed, notifyAffectedHost=notifyAffectedHost, mwgTrapVariables=mwgTrapVariables, mwgTraps=mwgTraps, kMajorVersion=kMajorVersion, pAMEngineVersion=pAMEngineVersion, stCategoriesTable=stCategoriesTable, stCategoryCount=stCategoryCount, stClientCount=stClientCount, stCategoryName=stCategoryName, stConnectionsLegitimate=stConnectionsLegitimate, stFtpBytesToServer=stFtpBytesToServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwg=mwg, mwgContent=mwgContent, stCategories=stCategories, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgHttp=mwgHttp, stHttpTraffic=stHttpTraffic, kProductVersion=kProductVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgFTP=mwgFTP, stTimeForTransaction=stTimeForTransaction, stBlockedByAntiMalware=stBlockedByAntiMalware, kRevision=kRevision, trSystem=trSystem, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpBytesFromClient=stHttpBytesFromClient, notifyOriginName=notifyOriginName, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsBytesToServer=stHttpsBytesToServer, stFtpBytesFromServer=stFtpBytesFromServer, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesToClient=stFtpBytesToClient, stMalwareDetected=stMalwareDetected, stHttpsBytesToClient=stHttpsBytesToClient, pMFEDATVersion=pMFEDATVersion, stFtpBytesFromClient=stFtpBytesFromClient, notifyReasonString=notifyReasonString, mwgEngineVersions=mwgEngineVersions, kMinorVersion=kMinorVersion, mwgMiscellaneous=mwgMiscellaneous, notifyOrigin=notifyOrigin, stHttpBytesToClient=stHttpBytesToClient, stCategoriesEntry=stCategoriesEntry)
