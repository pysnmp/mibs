...
TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, Unsigned32, Counter32, Bits, Gauge32, IpAddress, NotificationType, MibIdentifier, mib_2, ObjectIdentity, NotificationType, ModuleIdentity, Counter64, iso, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "Unsigned32", "Counter32", "Bits", "Gauge32", "IpAddress", "NotificationType", "MibIdentifier", "mib-2", "ObjectIdentity", "NotificationType", "ModuleIdentity", "Counter64", "iso", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategories=stCategories, stHttpRequests=stHttpRequests, mwgEngineVersions=mwgEngineVersions, stClientCount=stClientCount, mwgFTP=mwgFTP, stFtpBytesFromClient=stFtpBytesFromClient, stHttpBytesToServer=stHttpBytesToServer, stFtpBytesFromServer=stFtpBytesFromServer, stHttpsBytesFromClient=stHttpsBytesFromClient, stBlockedByURLFilter=stBlockedByURLFilter, stBadReputation=stBadReputation, stBlockedByMediaFilter=stBlockedByMediaFilter, kHotfixVersion=kHotfixVersion, notifyReasonString=notifyReasonString, mwg=mwg, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kRevision=kRevision, stHttpBytesFromServer=stHttpBytesFromServer, kProductName=kProductName, stMalwareDetected=stMalwareDetected, kCompanyName=kCompanyName, kMajorVersion=kMajorVersion, stHttpsBytesToClient=stHttpsBytesToClient, stFtpBytesToClient=stFtpBytesToClient, PYSNMP_MODULE_ID=mwg, stCategoryName=stCategoryName, kCustomVersion=kCustomVersion, trApplication=trApplication, kBuildNumber=kBuildNumber, pMFEDATVersion=pMFEDATVersion, mwgStatistics=mwgStatistics, stHttpsBytesToServer=stHttpsBytesToServer, pAMProactiveVersion=pAMProactiveVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoryCount=stCategoryCount, trUser=trUser, notifyOriginName=notifyOriginName, kMinorVersion=kMinorVersion, notifyReason=notifyReason, stHttpBytesFromClient=stHttpBytesFromClient, stConnectedSockets=stConnectedSockets, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCPULoad=stCPULoad, stCPUIOWait=stCPUIOWait, stCategoriesTable=stCategoriesTable, pMFEEngineVersion=pMFEEngineVersion, stFtpBytesToServer=stFtpBytesToServer, mwgInfo=mwgInfo, stHandleConnectToServer=stHandleConnectToServer, mwgTrapVariables=mwgTrapVariables, kMicroVersion=kMicroVersion, pTSDBVersion=pTSDBVersion, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsRequests=stHttpsRequests, stFtpTraffic=stFtpTraffic, stHttpBytesToClient=stHttpBytesToClient, stMimeType=stMimeType, notifyAdditional=notifyAdditional, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgTraps=mwgTraps, stConnectionsBlocked=stConnectionsBlocked, stCPULoadRaw=stCPULoadRaw, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgHttps=mwgHttps, stHttpTraffic=stHttpTraffic, mwgContent=mwgContent, kProductVersion=kProductVersion, notifySeverity=notifySeverity, mwgMiscellaneous=mwgMiscellaneous, notifyAffectedHost=notifyAffectedHost, stResolveHostViaDNS=stResolveHostViaDNS, stCategoriesEntry=stCategoriesEntry, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stTimeForTransaction=stTimeForTransaction, notifyOrigin=notifyOrigin, stMemConsumed=stMemConsumed, pAMSignatureVersion=pAMSignatureVersion, stHttpsTraffic=stHttpsTraffic, mwgHttp=mwgHttp, trSystem=trSystem, pAMEngineVersion=pAMEngineVersion)
