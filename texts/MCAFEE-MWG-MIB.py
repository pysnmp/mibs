...
MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Counter64, IpAddress, NotificationType, mib_2, ObjectIdentity, enterprises, NotificationType, MibIdentifier, Bits, Integer32, TimeTicks, iso, Unsigned32, ModuleIdentity, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Counter64", "IpAddress", "NotificationType", "mib-2", "ObjectIdentity", "enterprises", "NotificationType", "MibIdentifier", "Bits", "Integer32", "TimeTicks", "iso", "Unsigned32", "ModuleIdentity", "Gauge32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pAMSignatureVersion=pAMSignatureVersion, mwg=mwg, stHttpsBytesToClient=stHttpsBytesToClient, PYSNMP_MODULE_ID=mwg, stCategoryCount=stCategoryCount, mwgFTP=mwgFTP, stConnectionsLegitimate=stConnectionsLegitimate, kHotfixVersion=kHotfixVersion, stCPULoadRaw=stCPULoadRaw, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsBytesFromServer=stHttpsBytesFromServer, pMFEEngineVersion=pMFEEngineVersion, stHttpBytesToServer=stHttpBytesToServer, stCPULoad=stCPULoad, notifyReasonString=notifyReasonString, stCategoryName=stCategoryName, kProductName=kProductName, kMinorVersion=kMinorVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCategoriesEntry=stCategoriesEntry, kCompanyName=kCompanyName, mwgEngineVersions=mwgEngineVersions, mwgTrapVariables=mwgTrapVariables, mwgMiscellaneous=mwgMiscellaneous, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpTraffic=stHttpTraffic, stFtpBytesToServer=stFtpBytesToServer, trApplication=trApplication, pTSDBVersion=pTSDBVersion, pAMEngineVersion=pAMEngineVersion, stConnectionsBlocked=stConnectionsBlocked, stHttpsRequests=stHttpsRequests, notifyReason=notifyReason, stMalwareDetected=stMalwareDetected, kMicroVersion=kMicroVersion, pMFEDATVersion=pMFEDATVersion, stHandleConnectToServer=stHandleConnectToServer, stMemConsumed=stMemConsumed, stBadReputation=stBadReputation, stCategories=stCategories, kCustomVersion=kCustomVersion, stFtpTraffic=stFtpTraffic, notifySeverity=notifySeverity, mwgHttp=mwgHttp, mwgContent=mwgContent, stHttpBytesFromServer=stHttpBytesFromServer, notifyOriginName=notifyOriginName, kProductVersion=kProductVersion, notifyAdditional=notifyAdditional, trUser=trUser, stHttpRequests=stHttpRequests, kRevision=kRevision, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, pAMProactiveVersion=pAMProactiveVersion, stHttpBytesToClient=stHttpBytesToClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpBytesFromClient=stHttpBytesFromClient, stCPUIOWait=stCPUIOWait, stCategoriesTable=stCategoriesTable, stFtpBytesToClient=stFtpBytesToClient, stHttpsTraffic=stHttpsTraffic, notifyOrigin=notifyOrigin, mwgHttps=mwgHttps, kBuildNumber=kBuildNumber, mwgTraps=mwgTraps, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyAffectedHost=notifyAffectedHost, stBlockedByAntiMalware=stBlockedByAntiMalware, stClientCount=stClientCount, mwgInfo=mwgInfo, trSystem=trSystem, stMimeType=stMimeType, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesToServer=stHttpsBytesToServer, stResolveHostViaDNS=stResolveHostViaDNS, stConnectedSockets=stConnectedSockets, kMajorVersion=kMajorVersion, stBlockedByURLFilter=stBlockedByURLFilter, mwgStatistics=mwgStatistics, stTimeForTransaction=stTimeForTransaction, stFtpBytesFromClient=stFtpBytesFromClient, stFtpBytesFromServer=stFtpBytesFromServer)
