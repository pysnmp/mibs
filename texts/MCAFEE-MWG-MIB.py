...
ObjectIdentity, Unsigned32, ModuleIdentity, mib_2, TimeTicks, Counter64, NotificationType, Counter32, iso, NotificationType, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, enterprises, Integer32, Bits, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Unsigned32", "ModuleIdentity", "mib-2", "TimeTicks", "Counter64", "NotificationType", "Counter32", "iso", "NotificationType", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "enterprises", "Integer32", "Bits", "Gauge32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpBytesFromClient=stHttpBytesFromClient, stHttpsTraffic=stHttpsTraffic, kMajorVersion=kMajorVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, kMicroVersion=kMicroVersion, stTimeForTransaction=stTimeForTransaction, pMFEEngineVersion=pMFEEngineVersion, stCPULoad=stCPULoad, stBlockedByAntiMalware=stBlockedByAntiMalware, kProductName=kProductName, mwgStatistics=mwgStatistics, stHttpBytesToServer=stHttpBytesToServer, mwgTrapVariables=mwgTrapVariables, stConnectionsLegitimate=stConnectionsLegitimate, stClientCount=stClientCount, pAMEngineVersion=pAMEngineVersion, stCategoriesTable=stCategoriesTable, stCPUIOWait=stCPUIOWait, mwgMiscellaneous=mwgMiscellaneous, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stCategoryName=stCategoryName, stFtpTraffic=stFtpTraffic, stCategoriesEntry=stCategoriesEntry, trSystem=trSystem, notifyOrigin=notifyOrigin, pMFEDATVersion=pMFEDATVersion, mwg=mwg, mwgTraps=mwgTraps, stHttpBytesToClient=stHttpBytesToClient, stConnectionsBlocked=stConnectionsBlocked, stHttpBytesFromServer=stHttpBytesFromServer, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgInfo=mwgInfo, mwgFTP=mwgFTP, stBlockedByURLFilter=stBlockedByURLFilter, mwgHttp=mwgHttp, stLastSentLastReceivedClient=stLastSentLastReceivedClient, pAMProactiveVersion=pAMProactiveVersion, mwgContent=mwgContent, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stMalwareDetected=stMalwareDetected, notifyReasonString=notifyReasonString, kBuildNumber=kBuildNumber, mwgEngineVersions=mwgEngineVersions, stFtpBytesToServer=stFtpBytesToServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stBadReputation=stBadReputation, notifyReason=notifyReason, stFtpBytesFromServer=stFtpBytesFromServer, notifyOriginName=notifyOriginName, stMemConsumed=stMemConsumed, stResolveHostViaDNS=stResolveHostViaDNS, mwgHttps=mwgHttps, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpBytesFromClient=stFtpBytesFromClient, trApplication=trApplication, kMinorVersion=kMinorVersion, notifySeverity=notifySeverity, kCustomVersion=kCustomVersion, trUser=trUser, kCompanyName=kCompanyName, stMimeType=stMimeType, kHotfixVersion=kHotfixVersion, stConnectedSockets=stConnectedSockets, stFtpBytesToClient=stFtpBytesToClient, stHandleConnectToServer=stHandleConnectToServer, PYSNMP_MODULE_ID=mwg, stCPULoadRaw=stCPULoadRaw, stCategoryCount=stCategoryCount, stHttpsBytesFromServer=stHttpsBytesFromServer, kProductVersion=kProductVersion, stHttpRequests=stHttpRequests, kRevision=kRevision, stHttpsBytesToServer=stHttpsBytesToServer, notifyAffectedHost=notifyAffectedHost, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsRequests=stHttpsRequests, stHttpsBytesToClient=stHttpsBytesToClient, notifyAdditional=notifyAdditional, stHttpTraffic=stHttpTraffic, stCategories=stCategories, pAMSignatureVersion=pAMSignatureVersion, pTSDBVersion=pTSDBVersion)
