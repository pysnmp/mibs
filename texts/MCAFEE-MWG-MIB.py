...
Gauge32, Counter32, Bits, NotificationType, NotificationType, MibIdentifier, iso, TimeTicks, ObjectIdentity, ModuleIdentity, Integer32, mib_2, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, enterprises, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Counter32", "Bits", "NotificationType", "NotificationType", "MibIdentifier", "iso", "TimeTicks", "ObjectIdentity", "ModuleIdentity", "Integer32", "mib-2", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "enterprises", "IpAddress")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsBytesFromClient=stHttpsBytesFromClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kRevision=kRevision, trUser=trUser, stMimeType=stMimeType, mwgTrapVariables=mwgTrapVariables, stCPULoad=stCPULoad, stResolveHostViaDNS=stResolveHostViaDNS, kBuildNumber=kBuildNumber, kMajorVersion=kMajorVersion, stFtpBytesFromServer=stFtpBytesFromServer, stConnectionsBlocked=stConnectionsBlocked, notifyOrigin=notifyOrigin, kHotfixVersion=kHotfixVersion, stClientCount=stClientCount, stFtpBytesToClient=stFtpBytesToClient, pMFEDATVersion=pMFEDATVersion, stCategoryName=stCategoryName, notifySeverity=notifySeverity, pAMProactiveVersion=pAMProactiveVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoryCount=stCategoryCount, stHttpBytesToServer=stHttpBytesToServer, stHttpBytesFromClient=stHttpBytesFromClient, stHttpsTraffic=stHttpsTraffic, stCategories=stCategories, stHttpsRequests=stHttpsRequests, stHttpsBytesToServer=stHttpsBytesToServer, pMFEEngineVersion=pMFEEngineVersion, stTimeForTransaction=stTimeForTransaction, stMemConsumed=stMemConsumed, trSystem=trSystem, stMalwareDetected=stMalwareDetected, kMicroVersion=kMicroVersion, pAMEngineVersion=pAMEngineVersion, stConnectionsLegitimate=stConnectionsLegitimate, mwgHttps=mwgHttps, pAMSignatureVersion=pAMSignatureVersion, PYSNMP_MODULE_ID=mwg, stHttpBytesToClient=stHttpBytesToClient, pTSDBVersion=pTSDBVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stConnectedSockets=stConnectedSockets, stFtpBytesToServer=stFtpBytesToServer, mwgContent=mwgContent, kCompanyName=kCompanyName, stFtpTraffic=stFtpTraffic, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kProductVersion=kProductVersion, stBadReputation=stBadReputation, mwgHttp=mwgHttp, stCategoriesEntry=stCategoriesEntry, mwg=mwg, mwgStatistics=mwgStatistics, stBlockedByMediaFilter=stBlockedByMediaFilter, kMinorVersion=kMinorVersion, stHttpsBytesToClient=stHttpsBytesToClient, stHttpsBytesFromServer=stHttpsBytesFromServer, notifyAffectedHost=notifyAffectedHost, mwgFTP=mwgFTP, notifyOriginName=notifyOriginName, notifyAdditional=notifyAdditional, mwgMiscellaneous=mwgMiscellaneous, stCPUIOWait=stCPUIOWait, trApplication=trApplication, stBlockedByURLFilter=stBlockedByURLFilter, notifyReasonString=notifyReasonString, kProductName=kProductName, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgInfo=mwgInfo, notifyReason=notifyReason, stCategoriesTable=stCategoriesTable, mwgTraps=mwgTraps, stHttpRequests=stHttpRequests, kCustomVersion=kCustomVersion, stHandleConnectToServer=stHandleConnectToServer, stHttpBytesFromServer=stHttpBytesFromServer, stHttpTraffic=stHttpTraffic, stFtpBytesFromClient=stFtpBytesFromClient, mwgEngineVersions=mwgEngineVersions, stCPULoadRaw=stCPULoadRaw)
