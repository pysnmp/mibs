...
ModuleIdentity, Bits, NotificationType, Counter64, ObjectIdentity, mib_2, TimeTicks, iso, Counter32, NotificationType, enterprises, IpAddress, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Gauge32, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Bits", "NotificationType", "Counter64", "ObjectIdentity", "mib-2", "TimeTicks", "iso", "Counter32", "NotificationType", "enterprises", "IpAddress", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Gauge32", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", notifySeverity=notifySeverity, stCPULoadRaw=stCPULoadRaw, notifyAffectedHost=notifyAffectedHost, mwgFTP=mwgFTP, stCPULoad=stCPULoad, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stMemConsumed=stMemConsumed, stConnectionsLegitimate=stConnectionsLegitimate, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stBlockedByURLFilter=stBlockedByURLFilter, kProductVersion=kProductVersion, mwgInfo=mwgInfo, stTimeForTransaction=stTimeForTransaction, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kCustomVersion=kCustomVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwg=mwg, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stCategoryName=stCategoryName, stHttpRequests=stHttpRequests, mwgEngineVersions=mwgEngineVersions, pAMEngineVersion=pAMEngineVersion, pMFEEngineVersion=pMFEEngineVersion, stHttpBytesFromServer=stHttpBytesFromServer, mwgTrapVariables=mwgTrapVariables, kProductName=kProductName, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgTraps=mwgTraps, pTSDBVersion=pTSDBVersion, stHttpBytesToServer=stHttpBytesToServer, stCPUIOWait=stCPUIOWait, trSystem=trSystem, notifyOrigin=notifyOrigin, trApplication=trApplication, stCategoriesEntry=stCategoriesEntry, stFtpTraffic=stFtpTraffic, pAMProactiveVersion=pAMProactiveVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgHttps=mwgHttps, stHttpBytesToClient=stHttpBytesToClient, pMFEDATVersion=pMFEDATVersion, mwgStatistics=mwgStatistics, kCompanyName=kCompanyName, stConnectionsBlocked=stConnectionsBlocked, kMajorVersion=kMajorVersion, stFtpBytesFromClient=stFtpBytesFromClient, stFtpBytesToClient=stFtpBytesToClient, stHandleConnectToServer=stHandleConnectToServer, stCategories=stCategories, notifyAdditional=notifyAdditional, PYSNMP_MODULE_ID=mwg, mwgMiscellaneous=mwgMiscellaneous, stMimeType=stMimeType, stHttpTraffic=stHttpTraffic, notifyReason=notifyReason, notifyOriginName=notifyOriginName, stHttpsRequests=stHttpsRequests, stHttpBytesFromClient=stHttpBytesFromClient, trUser=trUser, stClientCount=stClientCount, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpsTraffic=stHttpsTraffic, mwgHttp=mwgHttp, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyReasonString=notifyReasonString, stHttpsBytesToClient=stHttpsBytesToClient, stFtpBytesFromServer=stFtpBytesFromServer, stResolveHostViaDNS=stResolveHostViaDNS, stCategoryCount=stCategoryCount, kMicroVersion=kMicroVersion, stHttpsBytesToServer=stHttpsBytesToServer, kMinorVersion=kMinorVersion, mwgContent=mwgContent, stMalwareDetected=stMalwareDetected, kBuildNumber=kBuildNumber, stBadReputation=stBadReputation, stCategoriesTable=stCategoriesTable, stConnectedSockets=stConnectedSockets, kRevision=kRevision, kHotfixVersion=kHotfixVersion, stFtpBytesToServer=stFtpBytesToServer)
