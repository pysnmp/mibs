...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwg=mwg, stConnectedSockets=stConnectedSockets, stTimeForTransaction=stTimeForTransaction, mwgTrapVariables=mwgTrapVariables, stHttpBytesToClient=stHttpBytesToClient, pMFEEngineVersion=pMFEEngineVersion, stHttpsBytesToClient=stHttpsBytesToClient, kMicroVersion=kMicroVersion, stHttpBytesFromClient=stHttpBytesFromClient, stFtpBytesToServer=stFtpBytesToServer, stHttpsBytesFromClient=stHttpsBytesFromClient, stCPULoad=stCPULoad, trUser=trUser, stMimeType=stMimeType, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgFTP=mwgFTP, mwgEngineVersions=mwgEngineVersions, kMajorVersion=kMajorVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgInfo=mwgInfo, pTSDBVersion=pTSDBVersion, kCustomVersion=kCustomVersion, kCompanyName=kCompanyName, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, kProductName=kProductName, stCategoriesTable=stCategoriesTable, notifyOrigin=notifyOrigin, mwgContent=mwgContent, kProductVersion=kProductVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, pAMEngineVersion=pAMEngineVersion, PYSNMP_MODULE_ID=mwg, pAMProactiveVersion=pAMProactiveVersion, stBlockedByURLFilter=stBlockedByURLFilter, stResolveHostViaDNS=stResolveHostViaDNS, stBadReputation=stBadReputation, stCategoriesEntry=stCategoriesEntry, stFtpBytesToClient=stFtpBytesToClient, stFtpBytesFromServer=stFtpBytesFromServer, stConnectionsBlocked=stConnectionsBlocked, stHttpsRequests=stHttpsRequests, stCategoryName=stCategoryName, notifyReasonString=notifyReasonString, stHttpsTraffic=stHttpsTraffic, stCategoryCount=stCategoryCount, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpBytesFromServer=stHttpBytesFromServer, kHotfixVersion=kHotfixVersion, stFtpBytesFromClient=stFtpBytesFromClient, mwgHttp=mwgHttp, stMalwareDetected=stMalwareDetected, stFtpTraffic=stFtpTraffic, stClientCount=stClientCount, kRevision=kRevision, stHttpRequests=stHttpRequests, mwgMiscellaneous=mwgMiscellaneous, pMFEDATVersion=pMFEDATVersion, stHttpTraffic=stHttpTraffic, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stMemConsumed=stMemConsumed, stCategories=stCategories, stHandleConnectToServer=stHandleConnectToServer, stCPUIOWait=stCPUIOWait, notifyOriginName=notifyOriginName, notifySeverity=notifySeverity, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsBytesToServer=stHttpsBytesToServer, stHttpBytesToServer=stHttpBytesToServer, trSystem=trSystem, kMinorVersion=kMinorVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgTraps=mwgTraps, notifyAffectedHost=notifyAffectedHost, notifyAdditional=notifyAdditional, mwgStatistics=mwgStatistics, mwgHttps=mwgHttps, trApplication=trApplication, stCPULoadRaw=stCPULoadRaw, notifyReason=notifyReason, kBuildNumber=kBuildNumber)
