...
NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, mib_2, Unsigned32, ObjectIdentity, iso, MibIdentifier, NotificationType, TimeTicks, IpAddress, Gauge32, enterprises, Bits, Counter64, ModuleIdentity, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "mib-2", "Unsigned32", "ObjectIdentity", "iso", "MibIdentifier", "NotificationType", "TimeTicks", "IpAddress", "Gauge32", "enterprises", "Bits", "Counter64", "ModuleIdentity", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpBytesToServer=stHttpBytesToServer, stCategoryName=stCategoryName, stCPULoadRaw=stCPULoadRaw, kMinorVersion=kMinorVersion, stConnectionsBlocked=stConnectionsBlocked, stFtpTraffic=stFtpTraffic, kMicroVersion=kMicroVersion, mwgTraps=mwgTraps, stHttpsTraffic=stHttpsTraffic, kProductVersion=kProductVersion, stFtpBytesToServer=stFtpBytesToServer, stFtpBytesFromServer=stFtpBytesFromServer, stHttpBytesToClient=stHttpBytesToClient, stCategoryCount=stCategoryCount, notifyReason=notifyReason, kCompanyName=kCompanyName, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesFromServer=stHttpBytesFromServer, mwgHttps=mwgHttps, kProductName=kProductName, notifyAdditional=notifyAdditional, trUser=trUser, stCategoriesEntry=stCategoriesEntry, stHttpsRequests=stHttpsRequests, mwgStatistics=mwgStatistics, stHttpsBytesFromClient=stHttpsBytesFromClient, stBlockedByMediaFilter=stBlockedByMediaFilter, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgMiscellaneous=mwgMiscellaneous, notifySeverity=notifySeverity, pTSDBVersion=pTSDBVersion, kBuildNumber=kBuildNumber, notifyOrigin=notifyOrigin, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stCategories=stCategories, notifyAffectedHost=notifyAffectedHost, mwgInfo=mwgInfo, mwgHttp=mwgHttp, stCategoriesTable=stCategoriesTable, trSystem=trSystem, notifyReasonString=notifyReasonString, kCustomVersion=kCustomVersion, notifyOriginName=notifyOriginName, pMFEEngineVersion=pMFEEngineVersion, stBlockedByURLFilter=stBlockedByURLFilter, PYSNMP_MODULE_ID=mwg, kRevision=kRevision, stHttpBytesFromClient=stHttpBytesFromClient, stBlockedByAntiMalware=stBlockedByAntiMalware, pAMProactiveVersion=pAMProactiveVersion, stHttpsBytesToServer=stHttpsBytesToServer, stMalwareDetected=stMalwareDetected, stConnectedSockets=stConnectedSockets, mwgFTP=mwgFTP, stHandleConnectToServer=stHandleConnectToServer, pMFEDATVersion=pMFEDATVersion, stConnectionsLegitimate=stConnectionsLegitimate, stFtpBytesFromClient=stFtpBytesFromClient, stHttpTraffic=stHttpTraffic, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpRequests=stHttpRequests, pAMSignatureVersion=pAMSignatureVersion, stFtpBytesToClient=stFtpBytesToClient, trApplication=trApplication, stResolveHostViaDNS=stResolveHostViaDNS, stTimeForTransaction=stTimeForTransaction, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwg=mwg, stClientCount=stClientCount, kMajorVersion=kMajorVersion, stCPULoad=stCPULoad, stMemConsumed=stMemConsumed, mwgContent=mwgContent, pAMEngineVersion=pAMEngineVersion, stBadReputation=stBadReputation, stMimeType=stMimeType, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCPUIOWait=stCPUIOWait, kHotfixVersion=kHotfixVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgTrapVariables=mwgTrapVariables, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgEngineVersions=mwgEngineVersions)
