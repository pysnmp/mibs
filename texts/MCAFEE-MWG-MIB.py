...
ModuleIdentity, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, mib_2, iso, Counter32, Unsigned32, Bits, MibIdentifier, Gauge32, NotificationType, Counter64, IpAddress, enterprises, ObjectIdentity, TimeTicks, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "mib-2", "iso", "Counter32", "Unsigned32", "Bits", "MibIdentifier", "Gauge32", "NotificationType", "Counter64", "IpAddress", "enterprises", "ObjectIdentity", "TimeTicks", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpBytesFromClient=stFtpBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, stHttpRequests=stHttpRequests, stHttpBytesToClient=stHttpBytesToClient, mwgTraps=mwgTraps, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, trApplication=trApplication, mwgFTP=mwgFTP, kProductVersion=kProductVersion, stResolveHostViaDNS=stResolveHostViaDNS, stFtpBytesFromServer=stFtpBytesFromServer, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpTraffic=stHttpTraffic, stConnectedSockets=stConnectedSockets, stHttpsBytesToServer=stHttpsBytesToServer, trSystem=trSystem, notifySeverity=notifySeverity, stHttpsBytesToClient=stHttpsBytesToClient, notifyReasonString=notifyReasonString, stHttpBytesFromClient=stHttpBytesFromClient, stCategoriesTable=stCategoriesTable, stTimeForTransaction=stTimeForTransaction, kProductName=kProductName, stFtpBytesToServer=stFtpBytesToServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpTraffic=stFtpTraffic, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stCategoriesEntry=stCategoriesEntry, pAMEngineVersion=pAMEngineVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kCustomVersion=kCustomVersion, kMicroVersion=kMicroVersion, kMinorVersion=kMinorVersion, stCategories=stCategories, stCPUIOWait=stCPUIOWait, pMFEDATVersion=pMFEDATVersion, notifyAdditional=notifyAdditional, kHotfixVersion=kHotfixVersion, trUser=trUser, stHttpsBytesFromClient=stHttpsBytesFromClient, stCPULoad=stCPULoad, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stMalwareDetected=stMalwareDetected, mwgTrapVariables=mwgTrapVariables, mwgMiscellaneous=mwgMiscellaneous, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHandleConnectToServer=stHandleConnectToServer, notifyReason=notifyReason, stConnectionsBlocked=stConnectionsBlocked, mwgStatistics=mwgStatistics, stFtpBytesToClient=stFtpBytesToClient, mwg=mwg, stBlockedByURLFilter=stBlockedByURLFilter, mwgHttp=mwgHttp, kCompanyName=kCompanyName, pTSDBVersion=pTSDBVersion, pAMProactiveVersion=pAMProactiveVersion, PYSNMP_MODULE_ID=mwg, stHttpBytesToServer=stHttpBytesToServer, notifyOriginName=notifyOriginName, stCPULoadRaw=stCPULoadRaw, pAMSignatureVersion=pAMSignatureVersion, stMemConsumed=stMemConsumed, notifyAffectedHost=notifyAffectedHost, stBadReputation=stBadReputation, kBuildNumber=kBuildNumber, stCategoryName=stCategoryName, kMajorVersion=kMajorVersion, stHttpsTraffic=stHttpsTraffic, stMimeType=stMimeType, mwgHttps=mwgHttps, stCategoryCount=stCategoryCount, stHttpBytesFromServer=stHttpBytesFromServer, mwgEngineVersions=mwgEngineVersions, kRevision=kRevision, pMFEEngineVersion=pMFEEngineVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stClientCount=stClientCount, stHttpsRequests=stHttpsRequests, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgContent=mwgContent, notifyOrigin=notifyOrigin, mwgInfo=mwgInfo)
