...
NotificationType, Gauge32, Counter64, Counter32, NotificationType, MibIdentifier, enterprises, iso, mib_2, ModuleIdentity, IpAddress, Integer32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, TimeTicks, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "Gauge32", "Counter64", "Counter32", "NotificationType", "MibIdentifier", "enterprises", "iso", "mib-2", "ModuleIdentity", "IpAddress", "Integer32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "TimeTicks", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pTSDBVersion=pTSDBVersion, kCustomVersion=kCustomVersion, mwgMiscellaneous=mwgMiscellaneous, kHotfixVersion=kHotfixVersion, notifyReasonString=notifyReasonString, stCategoriesEntry=stCategoriesEntry, stClientCount=stClientCount, stBlockedByURLFilter=stBlockedByURLFilter, stHttpTraffic=stHttpTraffic, stFtpBytesToServer=stFtpBytesToServer, notifyOrigin=notifyOrigin, stHttpBytesFromServer=stHttpBytesFromServer, pMFEDATVersion=pMFEDATVersion, pAMProactiveVersion=pAMProactiveVersion, stMemConsumed=stMemConsumed, PYSNMP_MODULE_ID=mwg, mwg=mwg, mwgContent=mwgContent, kCompanyName=kCompanyName, mwgInfo=mwgInfo, stHttpRequests=stHttpRequests, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgTrapVariables=mwgTrapVariables, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyAdditional=notifyAdditional, stCategories=stCategories, stHttpBytesToClient=stHttpBytesToClient, stBadReputation=stBadReputation, stResolveHostViaDNS=stResolveHostViaDNS, kMajorVersion=kMajorVersion, mwgTraps=mwgTraps, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCPULoad=stCPULoad, kMicroVersion=kMicroVersion, kProductName=kProductName, stHttpBytesFromClient=stHttpBytesFromClient, mwgHttp=mwgHttp, mwgEngineVersions=mwgEngineVersions, stHttpsBytesFromClient=stHttpsBytesFromClient, stHandleConnectToServer=stHandleConnectToServer, mwgStatistics=mwgStatistics, trApplication=trApplication, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsBytesToClient=stHttpsBytesToClient, kProductVersion=kProductVersion, notifyOriginName=notifyOriginName, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpBytesToServer=stHttpBytesToServer, notifySeverity=notifySeverity, pAMSignatureVersion=pAMSignatureVersion, stCategoriesTable=stCategoriesTable, stConnectionsLegitimate=stConnectionsLegitimate, trSystem=trSystem, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyAffectedHost=notifyAffectedHost, stCPUIOWait=stCPUIOWait, trUser=trUser, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgHttps=mwgHttps, stCPULoadRaw=stCPULoadRaw, kMinorVersion=kMinorVersion, kBuildNumber=kBuildNumber, pMFEEngineVersion=pMFEEngineVersion, mwgFTP=mwgFTP, stFtpTraffic=stFtpTraffic, stCategoryCount=stCategoryCount, pAMEngineVersion=pAMEngineVersion, stConnectionsBlocked=stConnectionsBlocked, stMalwareDetected=stMalwareDetected, stTimeForTransaction=stTimeForTransaction, stHttpsBytesFromServer=stHttpsBytesFromServer, kRevision=kRevision, notifyReason=notifyReason, stHttpsTraffic=stHttpsTraffic, stConnectedSockets=stConnectedSockets, stFtpBytesToClient=stFtpBytesToClient, stFtpBytesFromServer=stFtpBytesFromServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpBytesFromClient=stFtpBytesFromClient, stCategoryName=stCategoryName, stHttpsRequests=stHttpsRequests, stMimeType=stMimeType)
