...
iso, MibIdentifier, Unsigned32, Integer32, Counter64, NotificationType, IpAddress, mib_2, ModuleIdentity, enterprises, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Bits, Counter32, ObjectIdentity, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "MibIdentifier", "Unsigned32", "Integer32", "Counter64", "NotificationType", "IpAddress", "mib-2", "ModuleIdentity", "enterprises", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Bits", "Counter32", "ObjectIdentity", "TimeTicks")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpBytesFromClient=stFtpBytesFromClient, stHttpBytesFromServer=stHttpBytesFromServer, kCompanyName=kCompanyName, PYSNMP_MODULE_ID=mwg, kHotfixVersion=kHotfixVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpRequests=stHttpRequests, pMFEDATVersion=pMFEDATVersion, stClientCount=stClientCount, stConnectionsLegitimate=stConnectionsLegitimate, mwgHttp=mwgHttp, stHttpsBytesToClient=stHttpsBytesToClient, pAMProactiveVersion=pAMProactiveVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifyAdditional=notifyAdditional, stFtpBytesFromServer=stFtpBytesFromServer, kProductVersion=kProductVersion, stConnectedSockets=stConnectedSockets, trApplication=trApplication, mwgStatistics=mwgStatistics, mwgContent=mwgContent, stFtpBytesToServer=stFtpBytesToServer, stCategories=stCategories, stBlockedByURLFilter=stBlockedByURLFilter, stCategoriesEntry=stCategoriesEntry, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, pMFEEngineVersion=pMFEEngineVersion, kMicroVersion=kMicroVersion, trUser=trUser, stCategoryName=stCategoryName, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, notifyReasonString=notifyReasonString, stHttpsBytesFromClient=stHttpsBytesFromClient, stCategoryCount=stCategoryCount, stCPULoad=stCPULoad, stHttpTraffic=stHttpTraffic, mwgTraps=mwgTraps, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgInfo=mwgInfo, stMalwareDetected=stMalwareDetected, mwgEngineVersions=mwgEngineVersions, pTSDBVersion=pTSDBVersion, mwgMiscellaneous=mwgMiscellaneous, kBuildNumber=kBuildNumber, kCustomVersion=kCustomVersion, notifyOriginName=notifyOriginName, stBadReputation=stBadReputation, stMimeType=stMimeType, mwg=mwg, kProductName=kProductName, stCPUIOWait=stCPUIOWait, stHttpBytesFromClient=stHttpBytesFromClient, notifySeverity=notifySeverity, trSystem=trSystem, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesToClient=stHttpBytesToClient, mwgHttps=mwgHttps, stCategoriesTable=stCategoriesTable, stTimeForTransaction=stTimeForTransaction, notifyAffectedHost=notifyAffectedHost, notifyOrigin=notifyOrigin, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectionsBlocked=stConnectionsBlocked, stHttpsTraffic=stHttpsTraffic, stHttpsBytesFromServer=stHttpsBytesFromServer, stResolveHostViaDNS=stResolveHostViaDNS, kMajorVersion=kMajorVersion, stFtpTraffic=stFtpTraffic, kMinorVersion=kMinorVersion, stHttpBytesToServer=stHttpBytesToServer, pAMEngineVersion=pAMEngineVersion, stFtpBytesToClient=stFtpBytesToClient, stHttpsRequests=stHttpsRequests, notifyReason=notifyReason, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgFTP=mwgFTP, stHandleConnectToServer=stHandleConnectToServer, stMemConsumed=stMemConsumed, stHttpsBytesToServer=stHttpsBytesToServer, stCPULoadRaw=stCPULoadRaw, kRevision=kRevision, mwgTrapVariables=mwgTrapVariables, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer)
