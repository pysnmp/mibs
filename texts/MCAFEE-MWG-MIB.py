...
mib_2, Bits, Counter32, ObjectIdentity, IpAddress, Unsigned32, ModuleIdentity, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, MibIdentifier, NotificationType, Gauge32, Integer32, enterprises, TimeTicks, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "mib-2", "Bits", "Counter32", "ObjectIdentity", "IpAddress", "Unsigned32", "ModuleIdentity", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "MibIdentifier", "NotificationType", "Gauge32", "Integer32", "enterprises", "TimeTicks", "Counter64")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategoryName=stCategoryName, mwgContent=mwgContent, notifyReasonString=notifyReasonString, pAMEngineVersion=pAMEngineVersion, stCPULoad=stCPULoad, kBuildNumber=kBuildNumber, stFtpBytesFromServer=stFtpBytesFromServer, mwgMiscellaneous=mwgMiscellaneous, stFtpBytesFromClient=stFtpBytesFromClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategoryCount=stCategoryCount, kHotfixVersion=kHotfixVersion, stHttpTraffic=stHttpTraffic, stHttpsTraffic=stHttpsTraffic, mwgFTP=mwgFTP, stHttpsBytesToClient=stHttpsBytesToClient, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifySeverity=notifySeverity, trSystem=trSystem, notifyOriginName=notifyOriginName, stHttpsRequests=stHttpsRequests, stBadReputation=stBadReputation, notifyAdditional=notifyAdditional, stMalwareDetected=stMalwareDetected, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyReason=notifyReason, kMinorVersion=kMinorVersion, stHttpBytesFromServer=stHttpBytesFromServer, stFtpBytesToClient=stFtpBytesToClient, stCPUIOWait=stCPUIOWait, pTSDBVersion=pTSDBVersion, stFtpBytesToServer=stFtpBytesToServer, kProductVersion=kProductVersion, stConnectedSockets=stConnectedSockets, stCategories=stCategories, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stConnectionsBlocked=stConnectionsBlocked, stHttpBytesFromClient=stHttpBytesFromClient, stHttpRequests=stHttpRequests, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFtpTraffic=stFtpTraffic, trUser=trUser, kRevision=kRevision, pAMProactiveVersion=pAMProactiveVersion, stBlockedByURLFilter=stBlockedByURLFilter, kMicroVersion=kMicroVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, stHandleConnectToServer=stHandleConnectToServer, mwgHttp=mwgHttp, pAMSignatureVersion=pAMSignatureVersion, mwgHttps=mwgHttps, stCPULoadRaw=stCPULoadRaw, stCategoriesEntry=stCategoriesEntry, mwgStatistics=mwgStatistics, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgInfo=mwgInfo, pMFEDATVersion=pMFEDATVersion, kCustomVersion=kCustomVersion, PYSNMP_MODULE_ID=mwg, stClientCount=stClientCount, stHttpsBytesFromClient=stHttpsBytesFromClient, mwg=mwg, mwgTrapVariables=mwgTrapVariables, mwgTraps=mwgTraps, stMimeType=stMimeType, stResolveHostViaDNS=stResolveHostViaDNS, mwgEngineVersions=mwgEngineVersions, stMemConsumed=stMemConsumed, kMajorVersion=kMajorVersion, stTimeForTransaction=stTimeForTransaction, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kCompanyName=kCompanyName, kProductName=kProductName, stHttpsBytesToServer=stHttpsBytesToServer, trApplication=trApplication, stCategoriesTable=stCategoriesTable, notifyAffectedHost=notifyAffectedHost, stConnectionsLegitimate=stConnectionsLegitimate, stHttpBytesToServer=stHttpBytesToServer, stHttpBytesToClient=stHttpBytesToClient, pMFEEngineVersion=pMFEEngineVersion, notifyOrigin=notifyOrigin)
