...
NotificationType, NotificationType, IpAddress, TimeTicks, ModuleIdentity, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, enterprises, Unsigned32, iso, Bits, Counter64, Gauge32, mib_2, Counter32, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "NotificationType", "IpAddress", "TimeTicks", "ModuleIdentity", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "enterprises", "Unsigned32", "iso", "Bits", "Counter64", "Gauge32", "mib-2", "Counter32", "MibIdentifier")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpBytesFromServer=stFtpBytesFromServer, kCustomVersion=kCustomVersion, stFtpBytesToClient=stFtpBytesToClient, kProductName=kProductName, stCategories=stCategories, mwgStatistics=mwgStatistics, stHttpTraffic=stHttpTraffic, stCPULoadRaw=stCPULoadRaw, notifyOrigin=notifyOrigin, stBlockedByMediaFilter=stBlockedByMediaFilter, pMFEDATVersion=pMFEDATVersion, pAMSignatureVersion=pAMSignatureVersion, stBlockedByURLFilter=stBlockedByURLFilter, mwgFTP=mwgFTP, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyReason=notifyReason, stHttpRequests=stHttpRequests, trUser=trUser, stFtpTraffic=stFtpTraffic, stHttpBytesToServer=stHttpBytesToServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kCompanyName=kCompanyName, stHttpBytesFromServer=stHttpBytesFromServer, notifyAffectedHost=notifyAffectedHost, pMFEEngineVersion=pMFEEngineVersion, stCategoriesEntry=stCategoriesEntry, notifyReasonString=notifyReasonString, kProductVersion=kProductVersion, stHttpsBytesToServer=stHttpsBytesToServer, kRevision=kRevision, notifySeverity=notifySeverity, mwgTraps=mwgTraps, stHttpBytesFromClient=stHttpBytesFromClient, stHttpsBytesToClient=stHttpsBytesToClient, pAMProactiveVersion=pAMProactiveVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, trApplication=trApplication, kBuildNumber=kBuildNumber, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgEngineVersions=mwgEngineVersions, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stConnectionsLegitimate=stConnectionsLegitimate, notifyAdditional=notifyAdditional, stMalwareDetected=stMalwareDetected, stFtpBytesFromClient=stFtpBytesFromClient, stConnectionsBlocked=stConnectionsBlocked, kMicroVersion=kMicroVersion, stCategoriesTable=stCategoriesTable, kMajorVersion=kMajorVersion, stBadReputation=stBadReputation, stFtpBytesToServer=stFtpBytesToServer, mwgHttps=mwgHttps, stMimeType=stMimeType, stClientCount=stClientCount, mwg=mwg, stHttpBytesToClient=stHttpBytesToClient, kMinorVersion=kMinorVersion, stCategoryName=stCategoryName, kHotfixVersion=kHotfixVersion, pAMEngineVersion=pAMEngineVersion, stHttpsTraffic=stHttpsTraffic, stTimeForTransaction=stTimeForTransaction, trSystem=trSystem, stCategoryCount=stCategoryCount, mwgContent=mwgContent, mwgTrapVariables=mwgTrapVariables, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgMiscellaneous=mwgMiscellaneous, notifyOriginName=notifyOriginName, stResolveHostViaDNS=stResolveHostViaDNS, mwgInfo=mwgInfo, stCPULoad=stCPULoad, mwgHttp=mwgHttp, stHttpsRequests=stHttpsRequests, stHandleConnectToServer=stHandleConnectToServer, stConnectedSockets=stConnectedSockets, stCPUIOWait=stCPUIOWait, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stMemConsumed=stMemConsumed, pTSDBVersion=pTSDBVersion, PYSNMP_MODULE_ID=mwg, stHttpsBytesFromClient=stHttpsBytesFromClient)
