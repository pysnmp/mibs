...
ModuleIdentity, Integer32, Unsigned32, iso, enterprises, TimeTicks, ObjectIdentity, mib_2, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, NotificationType, Bits, MibIdentifier, NotificationType, IpAddress, Counter32, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Integer32", "Unsigned32", "iso", "enterprises", "TimeTicks", "ObjectIdentity", "mib-2", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "NotificationType", "Bits", "MibIdentifier", "NotificationType", "IpAddress", "Counter32", "Counter64")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stMemConsumed=stMemConsumed, pTSDBVersion=pTSDBVersion, stHttpBytesFromClient=stHttpBytesFromClient, kCustomVersion=kCustomVersion, stHandleConnectToServer=stHandleConnectToServer, stConnectionsLegitimate=stConnectionsLegitimate, pAMProactiveVersion=pAMProactiveVersion, stHttpTraffic=stHttpTraffic, mwgTraps=mwgTraps, stHttpRequests=stHttpRequests, pAMSignatureVersion=pAMSignatureVersion, notifyReasonString=notifyReasonString, notifyAdditional=notifyAdditional, stHttpBytesToClient=stHttpBytesToClient, kHotfixVersion=kHotfixVersion, stFtpBytesFromServer=stFtpBytesFromServer, stResolveHostViaDNS=stResolveHostViaDNS, trSystem=trSystem, kMicroVersion=kMicroVersion, mwgInfo=mwgInfo, kRevision=kRevision, kMinorVersion=kMinorVersion, mwgHttps=mwgHttps, kProductVersion=kProductVersion, stCPULoadRaw=stCPULoadRaw, kCompanyName=kCompanyName, stCategoryName=stCategoryName, kMajorVersion=kMajorVersion, mwgMiscellaneous=mwgMiscellaneous, stCPUIOWait=stCPUIOWait, stFtpTraffic=stFtpTraffic, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stBlockedByMediaFilter=stBlockedByMediaFilter, kBuildNumber=kBuildNumber, stCategories=stCategories, stTimeForTransaction=stTimeForTransaction, stClientCount=stClientCount, PYSNMP_MODULE_ID=mwg, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpsRequests=stHttpsRequests, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyAffectedHost=notifyAffectedHost, stHttpsBytesToServer=stHttpsBytesToServer, stFtpBytesFromClient=stFtpBytesFromClient, mwgHttp=mwgHttp, stBlockedByURLFilter=stBlockedByURLFilter, stHttpBytesFromServer=stHttpBytesFromServer, notifyOrigin=notifyOrigin, kProductName=kProductName, pAMEngineVersion=pAMEngineVersion, mwg=mwg, stCategoryCount=stCategoryCount, mwgFTP=mwgFTP, mwgTrapVariables=mwgTrapVariables, notifySeverity=notifySeverity, mwgStatistics=mwgStatistics, trApplication=trApplication, trUser=trUser, pMFEEngineVersion=pMFEEngineVersion, notifyOriginName=notifyOriginName, mwgContent=mwgContent, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stCategoriesTable=stCategoriesTable, mwgEngineVersions=mwgEngineVersions, stCategoriesEntry=stCategoriesEntry, stCPULoad=stCPULoad, stHttpsBytesFromClient=stHttpsBytesFromClient, stConnectedSockets=stConnectedSockets, stMimeType=stMimeType, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stFtpBytesToClient=stFtpBytesToClient, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesToServer=stFtpBytesToServer, pMFEDATVersion=pMFEDATVersion, stHttpsTraffic=stHttpsTraffic, stBadReputation=stBadReputation, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpBytesToServer=stHttpBytesToServer, stMalwareDetected=stMalwareDetected, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifyReason=notifyReason, stHttpsBytesToClient=stHttpsBytesToClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine)
