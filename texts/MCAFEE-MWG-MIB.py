...
Integer32, NotificationType, iso, Bits, enterprises, ModuleIdentity, Counter32, mib_2, IpAddress, Counter64, ObjectIdentity, Gauge32, Unsigned32, TimeTicks, NotificationType, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "NotificationType", "iso", "Bits", "enterprises", "ModuleIdentity", "Counter32", "mib-2", "IpAddress", "Counter64", "ObjectIdentity", "Gauge32", "Unsigned32", "TimeTicks", "NotificationType", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCPUIOWait=stCPUIOWait, kRevision=kRevision, mwgEngineVersions=mwgEngineVersions, PYSNMP_MODULE_ID=mwg, stBlockedByURLFilter=stBlockedByURLFilter, stClientCount=stClientCount, stHttpsTraffic=stHttpsTraffic, notifyOrigin=notifyOrigin, mwgTraps=mwgTraps, mwgTrapVariables=mwgTrapVariables, mwgInfo=mwgInfo, notifyOriginName=notifyOriginName, stBadReputation=stBadReputation, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kMinorVersion=kMinorVersion, pTSDBVersion=pTSDBVersion, stHttpBytesFromServer=stHttpBytesFromServer, kProductName=kProductName, pMFEDATVersion=pMFEDATVersion, stResolveHostViaDNS=stResolveHostViaDNS, kCustomVersion=kCustomVersion, kHotfixVersion=kHotfixVersion, notifyAdditional=notifyAdditional, notifyAffectedHost=notifyAffectedHost, stCPULoad=stCPULoad, stMemConsumed=stMemConsumed, stCategoriesEntry=stCategoriesEntry, kMicroVersion=kMicroVersion, stHttpTraffic=stHttpTraffic, mwgContent=mwgContent, notifyReasonString=notifyReasonString, stHttpsBytesFromClient=stHttpsBytesFromClient, stFtpBytesToServer=stFtpBytesToServer, stHttpsRequests=stHttpsRequests, stBlockedByAntiMalware=stBlockedByAntiMalware, stCPULoadRaw=stCPULoadRaw, kMajorVersion=kMajorVersion, stConnectedSockets=stConnectedSockets, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFtpBytesFromClient=stFtpBytesFromClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pAMEngineVersion=pAMEngineVersion, mwgMiscellaneous=mwgMiscellaneous, stHttpBytesFromClient=stHttpBytesFromClient, stMalwareDetected=stMalwareDetected, pAMProactiveVersion=pAMProactiveVersion, mwgHttps=mwgHttps, stConnectionsBlocked=stConnectionsBlocked, mwgStatistics=mwgStatistics, notifySeverity=notifySeverity, stHttpsBytesToClient=stHttpsBytesToClient, stHandleConnectToServer=stHandleConnectToServer, stTimeForTransaction=stTimeForTransaction, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kProductVersion=kProductVersion, stFtpBytesToClient=stFtpBytesToClient, stCategoryCount=stCategoryCount, stConnectionsLegitimate=stConnectionsLegitimate, stHttpBytesToServer=stHttpBytesToServer, trUser=trUser, stCategories=stCategories, stCategoriesTable=stCategoriesTable, trApplication=trApplication, notifyReason=notifyReason, stCategoryName=stCategoryName, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesToClient=stHttpBytesToClient, stMimeType=stMimeType, stFtpBytesFromServer=stFtpBytesFromServer, stFtpTraffic=stFtpTraffic, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpRequests=stHttpRequests, mwg=mwg, kBuildNumber=kBuildNumber, kCompanyName=kCompanyName, mwgFTP=mwgFTP, trSystem=trSystem, pMFEEngineVersion=pMFEEngineVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsBytesToServer=stHttpsBytesToServer, mwgHttp=mwgHttp)
