...
ModuleIdentity, ObjectIdentity, Integer32, TimeTicks, Counter32, enterprises, IpAddress, Counter64, iso, NotificationType, MibIdentifier, mib_2, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, NotificationType, Gauge32, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "ObjectIdentity", "Integer32", "TimeTicks", "Counter32", "enterprises", "IpAddress", "Counter64", "iso", "NotificationType", "MibIdentifier", "mib-2", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "NotificationType", "Gauge32", "Bits")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stBadReputation=stBadReputation, kMicroVersion=kMicroVersion, stCategories=stCategories, stMemConsumed=stMemConsumed, mwgFTP=mwgFTP, kProductName=kProductName, stResolveHostViaDNS=stResolveHostViaDNS, pAMSignatureVersion=pAMSignatureVersion, kRevision=kRevision, stCPUIOWait=stCPUIOWait, mwgMiscellaneous=mwgMiscellaneous, stCPULoadRaw=stCPULoadRaw, stCPULoad=stCPULoad, stHttpTraffic=stHttpTraffic, stTimeForTransaction=stTimeForTransaction, stClientCount=stClientCount, mwgTraps=mwgTraps, stHttpsTraffic=stHttpsTraffic, pAMProactiveVersion=pAMProactiveVersion, stFtpBytesToClient=stFtpBytesToClient, kMajorVersion=kMajorVersion, stHttpsBytesToServer=stHttpsBytesToServer, kCompanyName=kCompanyName, trSystem=trSystem, mwgTrapVariables=mwgTrapVariables, kCustomVersion=kCustomVersion, stMimeType=stMimeType, stCategoryCount=stCategoryCount, stCategoriesTable=stCategoriesTable, stConnectionsLegitimate=stConnectionsLegitimate, notifySeverity=notifySeverity, stCategoryName=stCategoryName, trUser=trUser, stFtpTraffic=stFtpTraffic, stMalwareDetected=stMalwareDetected, stCategoriesEntry=stCategoriesEntry, kMinorVersion=kMinorVersion, pTSDBVersion=pTSDBVersion, stBlockedByURLFilter=stBlockedByURLFilter, pAMEngineVersion=pAMEngineVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyOriginName=notifyOriginName, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesFromClient=stHttpBytesFromClient, kProductVersion=kProductVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgContent=mwgContent, stHandleConnectToServer=stHandleConnectToServer, kHotfixVersion=kHotfixVersion, stFtpBytesToServer=stFtpBytesToServer, mwgEngineVersions=mwgEngineVersions, stHttpBytesFromServer=stHttpBytesFromServer, notifyReasonString=notifyReasonString, mwgHttps=mwgHttps, mwgHttp=mwgHttp, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, pMFEDATVersion=pMFEDATVersion, mwg=mwg, notifyAdditional=notifyAdditional, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesFromServer=stFtpBytesFromServer, mwgStatistics=mwgStatistics, stHttpsRequests=stHttpsRequests, PYSNMP_MODULE_ID=mwg, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgInfo=mwgInfo, stHttpBytesToClient=stHttpBytesToClient, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpBytesToServer=stHttpBytesToServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpBytesFromClient=stFtpBytesFromClient, stHttpRequests=stHttpRequests, notifyOrigin=notifyOrigin, stHttpsBytesFromServer=stHttpsBytesFromServer, notifyReason=notifyReason, kBuildNumber=kBuildNumber, stConnectedSockets=stConnectedSockets, trApplication=trApplication, notifyAffectedHost=notifyAffectedHost, pMFEEngineVersion=pMFEEngineVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine)
