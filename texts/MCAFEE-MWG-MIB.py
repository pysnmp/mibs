...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stBlockedByURLFilter=stBlockedByURLFilter, stBlockedByMediaFilter=stBlockedByMediaFilter, kMicroVersion=kMicroVersion, kMajorVersion=kMajorVersion, kCustomVersion=kCustomVersion, stHttpRequests=stHttpRequests, stHttpBytesToClient=stHttpBytesToClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stMemConsumed=stMemConsumed, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, notifyOrigin=notifyOrigin, notifyReason=notifyReason, stFtpTraffic=stFtpTraffic, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpsBytesToClient=stHttpsBytesToClient, stBadReputation=stBadReputation, kHotfixVersion=kHotfixVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, kBuildNumber=kBuildNumber, stHttpBytesToServer=stHttpBytesToServer, trUser=trUser, stFtpBytesFromClient=stFtpBytesFromClient, kProductVersion=kProductVersion, stCategoryName=stCategoryName, pMFEEngineVersion=pMFEEngineVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgEngineVersions=mwgEngineVersions, mwgTraps=mwgTraps, stCategoriesTable=stCategoriesTable, stHttpsBytesToServer=stHttpsBytesToServer, pAMEngineVersion=pAMEngineVersion, notifyAdditional=notifyAdditional, notifyReasonString=notifyReasonString, mwgInfo=mwgInfo, stHttpsBytesFromClient=stHttpsBytesFromClient, pAMSignatureVersion=pAMSignatureVersion, stHttpsRequests=stHttpsRequests, stHandleConnectToServer=stHandleConnectToServer, stResolveHostViaDNS=stResolveHostViaDNS, stHttpTraffic=stHttpTraffic, mwgHttp=mwgHttp, stConnectionsBlocked=stConnectionsBlocked, notifyAffectedHost=notifyAffectedHost, stMalwareDetected=stMalwareDetected, trApplication=trApplication, stConnectedSockets=stConnectedSockets, stCPULoadRaw=stCPULoadRaw, stFtpBytesFromServer=stFtpBytesFromServer, kMinorVersion=kMinorVersion, stCPUIOWait=stCPUIOWait, stCPULoad=stCPULoad, stConnectionsLegitimate=stConnectionsLegitimate, stCategoryCount=stCategoryCount, pTSDBVersion=pTSDBVersion, pMFEDATVersion=pMFEDATVersion, mwgMiscellaneous=mwgMiscellaneous, stFtpBytesToServer=stFtpBytesToServer, kCompanyName=kCompanyName, notifySeverity=notifySeverity, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kRevision=kRevision, stCategoriesEntry=stCategoriesEntry, stHttpBytesFromServer=stHttpBytesFromServer, stCategories=stCategories, mwgStatistics=mwgStatistics, mwgFTP=mwgFTP, stMimeType=stMimeType, pAMProactiveVersion=pAMProactiveVersion, mwg=mwg, notifyOriginName=notifyOriginName, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFtpBytesToClient=stFtpBytesToClient, stClientCount=stClientCount, stTimeForTransaction=stTimeForTransaction, stHttpsTraffic=stHttpsTraffic, mwgHttps=mwgHttps, kProductName=kProductName, trSystem=trSystem, mwgTrapVariables=mwgTrapVariables, mwgContent=mwgContent, stHttpBytesFromClient=stHttpBytesFromClient, PYSNMP_MODULE_ID=mwg)
