...
enterprises, NotificationType, NotificationType, Counter64, ObjectIdentity, IpAddress, Unsigned32, Integer32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, MibIdentifier, mib_2, TimeTicks, Gauge32, iso, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "NotificationType", "NotificationType", "Counter64", "ObjectIdentity", "IpAddress", "Unsigned32", "Integer32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "MibIdentifier", "mib-2", "TimeTicks", "Gauge32", "iso", "Counter32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stLastSentLastReceivedClient=stLastSentLastReceivedClient, stConnectedSockets=stConnectedSockets, kProductName=kProductName, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpsRequests=stHttpsRequests, stConnectionsBlocked=stConnectionsBlocked, kMajorVersion=kMajorVersion, pAMEngineVersion=pAMEngineVersion, notifyReasonString=notifyReasonString, mwgInfo=mwgInfo, stMimeType=stMimeType, stFtpBytesToClient=stFtpBytesToClient, stHandleConnectToServer=stHandleConnectToServer, stFtpBytesToServer=stFtpBytesToServer, stHttpRequests=stHttpRequests, stCategoryCount=stCategoryCount, mwgEngineVersions=mwgEngineVersions, mwgStatistics=mwgStatistics, kMinorVersion=kMinorVersion, PYSNMP_MODULE_ID=mwg, stFtpTraffic=stFtpTraffic, notifyOriginName=notifyOriginName, kCustomVersion=kCustomVersion, trUser=trUser, stCPULoad=stCPULoad, stHttpsBytesFromClient=stHttpsBytesFromClient, pMFEDATVersion=pMFEDATVersion, notifyAffectedHost=notifyAffectedHost, stConnectionsLegitimate=stConnectionsLegitimate, kMicroVersion=kMicroVersion, mwgTrapVariables=mwgTrapVariables, pTSDBVersion=pTSDBVersion, pMFEEngineVersion=pMFEEngineVersion, kProductVersion=kProductVersion, notifyReason=notifyReason, stResolveHostViaDNS=stResolveHostViaDNS, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwg=mwg, stBlockedByURLFilter=stBlockedByURLFilter, pAMSignatureVersion=pAMSignatureVersion, stTimeForTransaction=stTimeForTransaction, mwgTraps=mwgTraps, mwgMiscellaneous=mwgMiscellaneous, stCategoriesEntry=stCategoriesEntry, kBuildNumber=kBuildNumber, kCompanyName=kCompanyName, stCategoryName=stCategoryName, kRevision=kRevision, pAMProactiveVersion=pAMProactiveVersion, stHttpsTraffic=stHttpsTraffic, mwgHttp=mwgHttp, stBlockedByMediaFilter=stBlockedByMediaFilter, stFtpBytesFromClient=stFtpBytesFromClient, stHttpsBytesToClient=stHttpsBytesToClient, stCPULoadRaw=stCPULoadRaw, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpBytesToClient=stHttpBytesToClient, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpBytesToServer=stHttpBytesToServer, stMalwareDetected=stMalwareDetected, trSystem=trSystem, stHttpTraffic=stHttpTraffic, stHttpBytesFromServer=stHttpBytesFromServer, stFtpBytesFromServer=stFtpBytesFromServer, notifyAdditional=notifyAdditional, notifySeverity=notifySeverity, mwgFTP=mwgFTP, stCPUIOWait=stCPUIOWait, stClientCount=stClientCount, stCategories=stCategories, mwgContent=mwgContent, trApplication=trApplication, stCategoriesTable=stCategoriesTable, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgHttps=mwgHttps, stMemConsumed=stMemConsumed, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyOrigin=notifyOrigin, stBadReputation=stBadReputation, stHttpBytesFromClient=stHttpBytesFromClient, kHotfixVersion=kHotfixVersion, stHttpsBytesToServer=stHttpsBytesToServer)
