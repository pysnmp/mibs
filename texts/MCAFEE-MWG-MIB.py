...
Integer32, ObjectIdentity, Unsigned32, NotificationType, Counter64, enterprises, mib_2, MibIdentifier, IpAddress, Counter32, iso, Gauge32, ModuleIdentity, TimeTicks, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "ObjectIdentity", "Unsigned32", "NotificationType", "Counter64", "enterprises", "mib-2", "MibIdentifier", "IpAddress", "Counter32", "iso", "Gauge32", "ModuleIdentity", "TimeTicks", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpTraffic=stFtpTraffic, mwg=mwg, pTSDBVersion=pTSDBVersion, stClientCount=stClientCount, stCategoryName=stCategoryName, trApplication=trApplication, stCategoriesEntry=stCategoriesEntry, stCPULoad=stCPULoad, pAMEngineVersion=pAMEngineVersion, kProductName=kProductName, stCPUIOWait=stCPUIOWait, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyOriginName=notifyOriginName, stHttpsBytesToClient=stHttpsBytesToClient, pMFEEngineVersion=pMFEEngineVersion, stCategories=stCategories, notifyOrigin=notifyOrigin, stHttpBytesToClient=stHttpBytesToClient, notifyAffectedHost=notifyAffectedHost, kMicroVersion=kMicroVersion, stTimeForTransaction=stTimeForTransaction, stHttpsBytesFromClient=stHttpsBytesFromClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsRequests=stHttpsRequests, kCustomVersion=kCustomVersion, mwgInfo=mwgInfo, pMFEDATVersion=pMFEDATVersion, notifyReasonString=notifyReasonString, kHotfixVersion=kHotfixVersion, stConnectedSockets=stConnectedSockets, stFtpBytesToClient=stFtpBytesToClient, stConnectionsBlocked=stConnectionsBlocked, mwgEngineVersions=mwgEngineVersions, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyAdditional=notifyAdditional, stHttpsBytesFromServer=stHttpsBytesFromServer, stMimeType=stMimeType, stHttpBytesFromClient=stHttpBytesFromClient, stHandleConnectToServer=stHandleConnectToServer, kCompanyName=kCompanyName, stBadReputation=stBadReputation, mwgTrapVariables=mwgTrapVariables, stHttpsTraffic=stHttpsTraffic, stCategoryCount=stCategoryCount, stCategoriesTable=stCategoriesTable, notifySeverity=notifySeverity, mwgHttp=mwgHttp, kRevision=kRevision, stResolveHostViaDNS=stResolveHostViaDNS, stCPULoadRaw=stCPULoadRaw, kMinorVersion=kMinorVersion, stMemConsumed=stMemConsumed, stMalwareDetected=stMalwareDetected, mwgFTP=mwgFTP, kBuildNumber=kBuildNumber, kProductVersion=kProductVersion, stHttpBytesToServer=stHttpBytesToServer, mwgContent=mwgContent, stHttpRequests=stHttpRequests, mwgMiscellaneous=mwgMiscellaneous, stFtpBytesFromServer=stFtpBytesFromServer, stHttpBytesFromServer=stHttpBytesFromServer, stHttpTraffic=stHttpTraffic, stBlockedByURLFilter=stBlockedByURLFilter, trSystem=trSystem, pAMSignatureVersion=pAMSignatureVersion, pAMProactiveVersion=pAMProactiveVersion, stConnectionsLegitimate=stConnectionsLegitimate, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifyReason=notifyReason, mwgHttps=mwgHttps, mwgTraps=mwgTraps, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kMajorVersion=kMajorVersion, mwgStatistics=mwgStatistics, stFtpBytesToServer=stFtpBytesToServer, PYSNMP_MODULE_ID=mwg, stFtpBytesFromClient=stFtpBytesFromClient, trUser=trUser, stHttpsBytesToServer=stHttpsBytesToServer)
