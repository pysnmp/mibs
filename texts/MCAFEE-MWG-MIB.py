...
Gauge32, mib_2, NotificationType, NotificationType, Counter32, iso, Bits, TimeTicks, ModuleIdentity, Counter64, MibIdentifier, IpAddress, Unsigned32, enterprises, Integer32, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "mib-2", "NotificationType", "NotificationType", "Counter32", "iso", "Bits", "TimeTicks", "ModuleIdentity", "Counter64", "MibIdentifier", "IpAddress", "Unsigned32", "enterprises", "Integer32", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgHttps=mwgHttps, pAMEngineVersion=pAMEngineVersion, stCategoryCount=stCategoryCount, pAMSignatureVersion=pAMSignatureVersion, stCategories=stCategories, stHttpsBytesToServer=stHttpsBytesToServer, mwgFTP=mwgFTP, notifyAdditional=notifyAdditional, stConnectionsLegitimate=stConnectionsLegitimate, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCPULoad=stCPULoad, stResolveHostViaDNS=stResolveHostViaDNS, pTSDBVersion=pTSDBVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, trUser=trUser, stFtpBytesFromClient=stFtpBytesFromClient, kHotfixVersion=kHotfixVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, mwg=mwg, kMinorVersion=kMinorVersion, stMimeType=stMimeType, kMicroVersion=kMicroVersion, stHttpsBytesToClient=stHttpsBytesToClient, notifyAffectedHost=notifyAffectedHost, notifyReason=notifyReason, stHttpBytesFromClient=stHttpBytesFromClient, stBadReputation=stBadReputation, stFtpBytesToClient=stFtpBytesToClient, kMajorVersion=kMajorVersion, kCustomVersion=kCustomVersion, stCategoriesTable=stCategoriesTable, mwgHttp=mwgHttp, pMFEDATVersion=pMFEDATVersion, stFtpBytesToServer=stFtpBytesToServer, stHttpTraffic=stHttpTraffic, stHttpsRequests=stHttpsRequests, mwgContent=mwgContent, trApplication=trApplication, stHttpBytesToServer=stHttpBytesToServer, notifyOrigin=notifyOrigin, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpsBytesFromServer=stHttpsBytesFromServer, stCPULoadRaw=stCPULoadRaw, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stConnectionsBlocked=stConnectionsBlocked, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kBuildNumber=kBuildNumber, stFtpBytesFromServer=stFtpBytesFromServer, stCategoriesEntry=stCategoriesEntry, stHttpBytesToClient=stHttpBytesToClient, notifyReasonString=notifyReasonString, stBlockedByAntiMalware=stBlockedByAntiMalware, PYSNMP_MODULE_ID=mwg, pAMProactiveVersion=pAMProactiveVersion, notifySeverity=notifySeverity, stMemConsumed=stMemConsumed, mwgEngineVersions=mwgEngineVersions, trSystem=trSystem, kProductName=kProductName, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stMalwareDetected=stMalwareDetected, mwgInfo=mwgInfo, pMFEEngineVersion=pMFEEngineVersion, mwgTraps=mwgTraps, stFtpTraffic=stFtpTraffic, stHttpsTraffic=stHttpsTraffic, stCPUIOWait=stCPUIOWait, notifyOriginName=notifyOriginName, stBlockedByURLFilter=stBlockedByURLFilter, kRevision=kRevision, stHttpRequests=stHttpRequests, stTimeForTransaction=stTimeForTransaction, kProductVersion=kProductVersion, stHandleConnectToServer=stHandleConnectToServer, mwgMiscellaneous=mwgMiscellaneous, mwgTrapVariables=mwgTrapVariables, mwgStatistics=mwgStatistics, stConnectedSockets=stConnectedSockets, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kCompanyName=kCompanyName, stCategoryName=stCategoryName, stClientCount=stClientCount, stHttpBytesFromServer=stHttpBytesFromServer)
