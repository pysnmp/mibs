...
MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, MibIdentifier, ObjectIdentity, Unsigned32, Counter32, ModuleIdentity, Counter64, NotificationType, Gauge32, mib_2, Bits, enterprises, NotificationType, iso, TimeTicks, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "MibIdentifier", "ObjectIdentity", "Unsigned32", "Counter32", "ModuleIdentity", "Counter64", "NotificationType", "Gauge32", "mib-2", "Bits", "enterprises", "NotificationType", "iso", "TimeTicks", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpTraffic=stFtpTraffic, stBadReputation=stBadReputation, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stBlockedByMediaFilter=stBlockedByMediaFilter, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, PYSNMP_MODULE_ID=mwg, kCompanyName=kCompanyName, mwgContent=mwgContent, stHttpBytesToServer=stHttpBytesToServer, mwgHttp=mwgHttp, stHandleConnectToServer=stHandleConnectToServer, stHttpsRequests=stHttpsRequests, stHttpTraffic=stHttpTraffic, kHotfixVersion=kHotfixVersion, stMalwareDetected=stMalwareDetected, stHttpsBytesToClient=stHttpsBytesToClient, notifyReasonString=notifyReasonString, stCategories=stCategories, stFtpBytesToServer=stFtpBytesToServer, stHttpsTraffic=stHttpsTraffic, stClientCount=stClientCount, stResolveHostViaDNS=stResolveHostViaDNS, kProductName=kProductName, stMemConsumed=stMemConsumed, stTimeForTransaction=stTimeForTransaction, notifyOriginName=notifyOriginName, stBlockedByURLFilter=stBlockedByURLFilter, stHttpBytesFromServer=stHttpBytesFromServer, notifyAffectedHost=notifyAffectedHost, stCategoryName=stCategoryName, stFtpBytesToClient=stFtpBytesToClient, pAMProactiveVersion=pAMProactiveVersion, mwgInfo=mwgInfo, kMajorVersion=kMajorVersion, stConnectionsBlocked=stConnectionsBlocked, stConnectionsLegitimate=stConnectionsLegitimate, trSystem=trSystem, stHttpsBytesFromServer=stHttpsBytesFromServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, pAMSignatureVersion=pAMSignatureVersion, kBuildNumber=kBuildNumber, stFtpBytesFromClient=stFtpBytesFromClient, pMFEDATVersion=pMFEDATVersion, kProductVersion=kProductVersion, notifyReason=notifyReason, pAMEngineVersion=pAMEngineVersion, mwgFTP=mwgFTP, stCPULoadRaw=stCPULoadRaw, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoryCount=stCategoryCount, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kRevision=kRevision, stCategoriesEntry=stCategoriesEntry, stHttpRequests=stHttpRequests, stFtpBytesFromServer=stFtpBytesFromServer, stCPUIOWait=stCPUIOWait, notifyOrigin=notifyOrigin, notifySeverity=notifySeverity, mwg=mwg, stHttpBytesToClient=stHttpBytesToClient, notifyAdditional=notifyAdditional, mwgStatistics=mwgStatistics, mwgTrapVariables=mwgTrapVariables, pMFEEngineVersion=pMFEEngineVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pTSDBVersion=pTSDBVersion, trUser=trUser, stConnectedSockets=stConnectedSockets, mwgTraps=mwgTraps, mwgEngineVersions=mwgEngineVersions, stHttpsBytesFromClient=stHttpsBytesFromClient, stCategoriesTable=stCategoriesTable, stMimeType=stMimeType, mwgMiscellaneous=mwgMiscellaneous, mwgHttps=mwgHttps, stHttpsBytesToServer=stHttpsBytesToServer, kMicroVersion=kMicroVersion, stCPULoad=stCPULoad, kCustomVersion=kCustomVersion, kMinorVersion=kMinorVersion, stHttpBytesFromClient=stHttpBytesFromClient, trApplication=trApplication)
