...
NotificationType, ModuleIdentity, Bits, Gauge32, Counter64, enterprises, Unsigned32, mib_2, MibIdentifier, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Integer32, iso, Counter32, ObjectIdentity, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "ModuleIdentity", "Bits", "Gauge32", "Counter64", "enterprises", "Unsigned32", "mib-2", "MibIdentifier", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Integer32", "iso", "Counter32", "ObjectIdentity", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategories=stCategories, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyReasonString=notifyReasonString, stHttpsBytesFromServer=stHttpsBytesFromServer, stCategoriesEntry=stCategoriesEntry, kRevision=kRevision, mwgFTP=mwgFTP, mwgHttp=mwgHttp, stHttpsBytesFromClient=stHttpsBytesFromClient, stFtpBytesToClient=stFtpBytesToClient, stMimeType=stMimeType, stCPULoad=stCPULoad, stConnectedSockets=stConnectedSockets, kProductVersion=kProductVersion, notifyAffectedHost=notifyAffectedHost, mwgEngineVersions=mwgEngineVersions, stCPUIOWait=stCPUIOWait, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpsBytesToServer=stHttpsBytesToServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient, trUser=trUser, pAMEngineVersion=pAMEngineVersion, kCompanyName=kCompanyName, stCategoriesTable=stCategoriesTable, stFtpTraffic=stFtpTraffic, pMFEEngineVersion=pMFEEngineVersion, mwgTrapVariables=mwgTrapVariables, notifyOriginName=notifyOriginName, stHttpBytesToClient=stHttpBytesToClient, kBuildNumber=kBuildNumber, pAMProactiveVersion=pAMProactiveVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpBytesFromServer=stHttpBytesFromServer, stConnectionsBlocked=stConnectionsBlocked, stHttpBytesToServer=stHttpBytesToServer, trSystem=trSystem, kCustomVersion=kCustomVersion, trApplication=trApplication, stHandleConnectToServer=stHandleConnectToServer, pTSDBVersion=pTSDBVersion, stBadReputation=stBadReputation, kProductName=kProductName, pMFEDATVersion=pMFEDATVersion, stMalwareDetected=stMalwareDetected, kMinorVersion=kMinorVersion, notifyAdditional=notifyAdditional, stConnectionsLegitimate=stConnectionsLegitimate, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwg=mwg, mwgMiscellaneous=mwgMiscellaneous, stFtpBytesToServer=stFtpBytesToServer, mwgStatistics=mwgStatistics, kMicroVersion=kMicroVersion, stCPULoadRaw=stCPULoadRaw, kMajorVersion=kMajorVersion, stHttpsTraffic=stHttpsTraffic, stResolveHostViaDNS=stResolveHostViaDNS, stTimeForTransaction=stTimeForTransaction, stMemConsumed=stMemConsumed, stCategoryCount=stCategoryCount, mwgContent=mwgContent, notifyReason=notifyReason, stHttpsRequests=stHttpsRequests, pAMSignatureVersion=pAMSignatureVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgHttps=mwgHttps, stBlockedByURLFilter=stBlockedByURLFilter, kHotfixVersion=kHotfixVersion, stHttpRequests=stHttpRequests, stHttpTraffic=stHttpTraffic, stFtpBytesFromClient=stFtpBytesFromClient, stClientCount=stClientCount, stHttpsBytesToClient=stHttpsBytesToClient, PYSNMP_MODULE_ID=mwg, stFtpBytesFromServer=stFtpBytesFromServer, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCategoryName=stCategoryName, notifyOrigin=notifyOrigin, stHttpBytesFromClient=stHttpBytesFromClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgInfo=mwgInfo, notifySeverity=notifySeverity, mwgTraps=mwgTraps)
