...
Bits, Counter64, ObjectIdentity, TimeTicks, NotificationType, mib_2, enterprises, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, ModuleIdentity, iso, Counter32, MibIdentifier, Integer32, IpAddress, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Counter64", "ObjectIdentity", "TimeTicks", "NotificationType", "mib-2", "enterprises", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "ModuleIdentity", "iso", "Counter32", "MibIdentifier", "Integer32", "IpAddress", "Gauge32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kCustomVersion=kCustomVersion, stHttpBytesToServer=stHttpBytesToServer, kRevision=kRevision, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kProductVersion=kProductVersion, stHttpsBytesToServer=stHttpsBytesToServer, kMicroVersion=kMicroVersion, trSystem=trSystem, stMemConsumed=stMemConsumed, notifySeverity=notifySeverity, stClientCount=stClientCount, stHttpBytesFromClient=stHttpBytesFromClient, PYSNMP_MODULE_ID=mwg, mwgTrapVariables=mwgTrapVariables, pAMProactiveVersion=pAMProactiveVersion, stHttpBytesFromServer=stHttpBytesFromServer, mwg=mwg, mwgMiscellaneous=mwgMiscellaneous, mwgHttp=mwgHttp, stHttpsTraffic=stHttpsTraffic, stBlockedByAntiMalware=stBlockedByAntiMalware, stFtpBytesToClient=stFtpBytesToClient, trUser=trUser, trApplication=trApplication, stConnectionsBlocked=stConnectionsBlocked, stHttpsBytesFromServer=stHttpsBytesFromServer, kProductName=kProductName, stCategoriesTable=stCategoriesTable, notifyOriginName=notifyOriginName, stResolveHostViaDNS=stResolveHostViaDNS, stFtpBytesFromServer=stFtpBytesFromServer, pAMEngineVersion=pAMEngineVersion, stMimeType=stMimeType, mwgHttps=mwgHttps, kCompanyName=kCompanyName, notifyReason=notifyReason, kMajorVersion=kMajorVersion, mwgTraps=mwgTraps, kBuildNumber=kBuildNumber, notifyReasonString=notifyReasonString, notifyAffectedHost=notifyAffectedHost, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectionsLegitimate=stConnectionsLegitimate, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stConnectedSockets=stConnectedSockets, stCategoryName=stCategoryName, stHttpsBytesFromClient=stHttpsBytesFromClient, stCategories=stCategories, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFtpBytesToServer=stFtpBytesToServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, pMFEEngineVersion=pMFEEngineVersion, stCPUIOWait=stCPUIOWait, mwgFTP=mwgFTP, stCategoryCount=stCategoryCount, stHttpTraffic=stHttpTraffic, notifyAdditional=notifyAdditional, stBlockedByMediaFilter=stBlockedByMediaFilter, kMinorVersion=kMinorVersion, notifyOrigin=notifyOrigin, pAMSignatureVersion=pAMSignatureVersion, pMFEDATVersion=pMFEDATVersion, stFtpBytesFromClient=stFtpBytesFromClient, stBlockedByURLFilter=stBlockedByURLFilter, kHotfixVersion=kHotfixVersion, stCategoriesEntry=stCategoriesEntry, mwgEngineVersions=mwgEngineVersions, pTSDBVersion=pTSDBVersion, stFtpTraffic=stFtpTraffic, stHttpsRequests=stHttpsRequests, stCPULoad=stCPULoad, mwgInfo=mwgInfo, stBadReputation=stBadReputation, stTimeForTransaction=stTimeForTransaction, stHandleConnectToServer=stHandleConnectToServer, stMalwareDetected=stMalwareDetected, mwgContent=mwgContent, stHttpRequests=stHttpRequests, stHttpsBytesToClient=stHttpsBytesToClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgStatistics=mwgStatistics, stHttpBytesToClient=stHttpBytesToClient, stCPULoadRaw=stCPULoadRaw)
