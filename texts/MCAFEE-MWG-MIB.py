...
NotificationType, TimeTicks, Gauge32, ObjectIdentity, Counter32, Counter64, mib_2, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, Integer32, Unsigned32, enterprises, ModuleIdentity, IpAddress, iso, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "TimeTicks", "Gauge32", "ObjectIdentity", "Counter32", "Counter64", "mib-2", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "Integer32", "Unsigned32", "enterprises", "ModuleIdentity", "IpAddress", "iso", "MibIdentifier")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsTraffic=stHttpsTraffic, stCategoryCount=stCategoryCount, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, pMFEEngineVersion=pMFEEngineVersion, stHttpBytesToClient=stHttpBytesToClient, stFtpTraffic=stFtpTraffic, kProductName=kProductName, stCategories=stCategories, stMimeType=stMimeType, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCPULoad=stCPULoad, stHttpBytesFromServer=stHttpBytesFromServer, stResolveHostViaDNS=stResolveHostViaDNS, notifyReason=notifyReason, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyOrigin=notifyOrigin, kMajorVersion=kMajorVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgTrapVariables=mwgTrapVariables, stCategoriesTable=stCategoriesTable, notifyOriginName=notifyOriginName, notifyReasonString=notifyReasonString, mwgHttp=mwgHttp, stHttpBytesToServer=stHttpBytesToServer, notifySeverity=notifySeverity, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kCustomVersion=kCustomVersion, stFtpBytesToClient=stFtpBytesToClient, mwg=mwg, kMinorVersion=kMinorVersion, notifyAdditional=notifyAdditional, mwgContent=mwgContent, stHandleConnectToServer=stHandleConnectToServer, stFtpBytesToServer=stFtpBytesToServer, kMicroVersion=kMicroVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, mwgMiscellaneous=mwgMiscellaneous, kHotfixVersion=kHotfixVersion, stCPULoadRaw=stCPULoadRaw, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, pTSDBVersion=pTSDBVersion, stCPUIOWait=stCPUIOWait, stHttpsBytesToServer=stHttpsBytesToServer, stConnectedSockets=stConnectedSockets, mwgEngineVersions=mwgEngineVersions, kBuildNumber=kBuildNumber, mwgHttps=mwgHttps, stMalwareDetected=stMalwareDetected, stFtpBytesFromClient=stFtpBytesFromClient, stClientCount=stClientCount, mwgStatistics=mwgStatistics, stConnectionsLegitimate=stConnectionsLegitimate, stFtpBytesFromServer=stFtpBytesFromServer, kProductVersion=kProductVersion, mwgTraps=mwgTraps, stTimeForTransaction=stTimeForTransaction, stHttpBytesFromClient=stHttpBytesFromClient, stMemConsumed=stMemConsumed, kRevision=kRevision, stHttpTraffic=stHttpTraffic, stBadReputation=stBadReputation, pAMProactiveVersion=pAMProactiveVersion, stCategoryName=stCategoryName, stHttpsBytesToClient=stHttpsBytesToClient, trSystem=trSystem, notifyAffectedHost=notifyAffectedHost, trUser=trUser, mwgFTP=mwgFTP, stBlockedByURLFilter=stBlockedByURLFilter, stHttpsRequests=stHttpsRequests, stCategoriesEntry=stCategoriesEntry, stConnectionsBlocked=stConnectionsBlocked, stHttpRequests=stHttpRequests, stBlockedByAntiMalware=stBlockedByAntiMalware, stBlockedByMediaFilter=stBlockedByMediaFilter, pMFEDATVersion=pMFEDATVersion, PYSNMP_MODULE_ID=mwg, pAMEngineVersion=pAMEngineVersion, mwgInfo=mwgInfo, kCompanyName=kCompanyName, pAMSignatureVersion=pAMSignatureVersion, trApplication=trApplication, stHttpsBytesFromServer=stHttpsBytesFromServer)
