...
Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, enterprises, MibIdentifier, mib_2, Gauge32, Bits, ModuleIdentity, Integer32, iso, ObjectIdentity, TimeTicks, NotificationType, NotificationType, IpAddress, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "enterprises", "MibIdentifier", "mib-2", "Gauge32", "Bits", "ModuleIdentity", "Integer32", "iso", "ObjectIdentity", "TimeTicks", "NotificationType", "NotificationType", "IpAddress", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kMicroVersion=kMicroVersion, stBadReputation=stBadReputation, stMemConsumed=stMemConsumed, stHandleConnectToServer=stHandleConnectToServer, mwgTrapVariables=mwgTrapVariables, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpRequests=stHttpRequests, mwgFTP=mwgFTP, kBuildNumber=kBuildNumber, stCPUIOWait=stCPUIOWait, stHttpBytesToClient=stHttpBytesToClient, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgEngineVersions=mwgEngineVersions, kMinorVersion=kMinorVersion, stBlockedByURLFilter=stBlockedByURLFilter, notifyOrigin=notifyOrigin, mwgInfo=mwgInfo, pAMEngineVersion=pAMEngineVersion, stHttpBytesToServer=stHttpBytesToServer, kMajorVersion=kMajorVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgHttp=mwgHttp, stCategoriesTable=stCategoriesTable, mwgHttps=mwgHttps, kProductVersion=kProductVersion, mwgContent=mwgContent, stFtpBytesFromServer=stFtpBytesFromServer, stMimeType=stMimeType, stLastSentLastReceivedClient=stLastSentLastReceivedClient, trSystem=trSystem, stHttpTraffic=stHttpTraffic, kCustomVersion=kCustomVersion, mwgMiscellaneous=mwgMiscellaneous, pAMSignatureVersion=pAMSignatureVersion, kRevision=kRevision, stFtpBytesToClient=stFtpBytesToClient, pMFEDATVersion=pMFEDATVersion, trApplication=trApplication, stFtpBytesFromClient=stFtpBytesFromClient, stFtpTraffic=stFtpTraffic, mwgTraps=mwgTraps, kHotfixVersion=kHotfixVersion, trUser=trUser, stHttpBytesFromServer=stHttpBytesFromServer, pMFEEngineVersion=pMFEEngineVersion, mwgStatistics=mwgStatistics, notifySeverity=notifySeverity, notifyAdditional=notifyAdditional, notifyReason=notifyReason, stCategories=stCategories, stMalwareDetected=stMalwareDetected, notifyOriginName=notifyOriginName, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesFromClient=stHttpBytesFromClient, stCPULoadRaw=stCPULoadRaw, stHttpsTraffic=stHttpsTraffic, stCPULoad=stCPULoad, kCompanyName=kCompanyName, PYSNMP_MODULE_ID=mwg, mwg=mwg, stBlockedByAntiMalware=stBlockedByAntiMalware, stConnectedSockets=stConnectedSockets, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCategoryCount=stCategoryCount, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsBytesFromServer=stHttpsBytesFromServer, stCategoriesEntry=stCategoriesEntry, pAMProactiveVersion=pAMProactiveVersion, stResolveHostViaDNS=stResolveHostViaDNS, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stTimeForTransaction=stTimeForTransaction, pTSDBVersion=pTSDBVersion, stCategoryName=stCategoryName, stConnectionsBlocked=stConnectionsBlocked, notifyAffectedHost=notifyAffectedHost, stFtpBytesToServer=stFtpBytesToServer, stHttpsRequests=stHttpsRequests, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsBytesFromClient=stHttpsBytesFromClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kProductName=kProductName, notifyReasonString=notifyReasonString, stClientCount=stClientCount)
