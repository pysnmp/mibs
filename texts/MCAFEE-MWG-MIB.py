...
Gauge32, Unsigned32, enterprises, NotificationType, MibIdentifier, ObjectIdentity, IpAddress, Counter32, iso, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, mib_2, Integer32, NotificationType, ModuleIdentity, TimeTicks, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Unsigned32", "enterprises", "NotificationType", "MibIdentifier", "ObjectIdentity", "IpAddress", "Counter32", "iso", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "mib-2", "Integer32", "NotificationType", "ModuleIdentity", "TimeTicks", "Counter64")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kHotfixVersion=kHotfixVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, trUser=trUser, mwgFTP=mwgFTP, mwgInfo=mwgInfo, stResolveHostViaDNS=stResolveHostViaDNS, notifyOrigin=notifyOrigin, pAMSignatureVersion=pAMSignatureVersion, stCategoriesTable=stCategoriesTable, mwg=mwg, pMFEDATVersion=pMFEDATVersion, mwgHttps=mwgHttps, stHandleConnectToServer=stHandleConnectToServer, pTSDBVersion=pTSDBVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesToServer=stFtpBytesToServer, stHttpTraffic=stHttpTraffic, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyAffectedHost=notifyAffectedHost, stHttpBytesToServer=stHttpBytesToServer, trSystem=trSystem, stMimeType=stMimeType, kProductVersion=kProductVersion, kMajorVersion=kMajorVersion, stFtpBytesFromClient=stFtpBytesFromClient, kCompanyName=kCompanyName, stCategoriesEntry=stCategoriesEntry, trApplication=trApplication, stHttpRequests=stHttpRequests, stHttpsBytesToServer=stHttpsBytesToServer, stConnectionsLegitimate=stConnectionsLegitimate, PYSNMP_MODULE_ID=mwg, stHttpsBytesToClient=stHttpsBytesToClient, stCPULoad=stCPULoad, stHttpBytesFromClient=stHttpBytesFromClient, stCPULoadRaw=stCPULoadRaw, notifyReason=notifyReason, stConnectedSockets=stConnectedSockets, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgStatistics=mwgStatistics, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kMinorVersion=kMinorVersion, kMicroVersion=kMicroVersion, stClientCount=stClientCount, stHttpsRequests=stHttpsRequests, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kCustomVersion=kCustomVersion, stMemConsumed=stMemConsumed, mwgContent=mwgContent, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgHttp=mwgHttp, stFtpBytesToClient=stFtpBytesToClient, stCategoryName=stCategoryName, stHttpBytesFromServer=stHttpBytesFromServer, pAMProactiveVersion=pAMProactiveVersion, stMalwareDetected=stMalwareDetected, kBuildNumber=kBuildNumber, pMFEEngineVersion=pMFEEngineVersion, mwgEngineVersions=mwgEngineVersions, stBlockedByURLFilter=stBlockedByURLFilter, stTimeForTransaction=stTimeForTransaction, kProductName=kProductName, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpBytesToClient=stHttpBytesToClient, mwgMiscellaneous=mwgMiscellaneous, stCPUIOWait=stCPUIOWait, notifyAdditional=notifyAdditional, pAMEngineVersion=pAMEngineVersion, stBadReputation=stBadReputation, kRevision=kRevision, mwgTrapVariables=mwgTrapVariables, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategoryCount=stCategoryCount, stHttpsTraffic=stHttpsTraffic, notifyOriginName=notifyOriginName, stCategories=stCategories, stFtpTraffic=stFtpTraffic, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpBytesFromServer=stFtpBytesFromServer, mwgTraps=mwgTraps, notifyReasonString=notifyReasonString, notifySeverity=notifySeverity)
