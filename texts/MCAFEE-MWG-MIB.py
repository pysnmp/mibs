...
enterprises, MibIdentifier, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, Gauge32, IpAddress, Counter32, mib_2, Unsigned32, NotificationType, Bits, Counter64, Integer32, TimeTicks, iso, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "MibIdentifier", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "Gauge32", "IpAddress", "Counter32", "mib-2", "Unsigned32", "NotificationType", "Bits", "Counter64", "Integer32", "TimeTicks", "iso", "ModuleIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgEngineVersions=mwgEngineVersions, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpTraffic=stHttpTraffic, stHttpsBytesToClient=stHttpsBytesToClient, PYSNMP_MODULE_ID=mwg, pMFEDATVersion=pMFEDATVersion, stConnectionsBlocked=stConnectionsBlocked, stMemConsumed=stMemConsumed, kBuildNumber=kBuildNumber, stCategoryCount=stCategoryCount, mwgHttp=mwgHttp, stFtpTraffic=stFtpTraffic, stHttpBytesToClient=stHttpBytesToClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsBytesFromServer=stHttpsBytesFromServer, stBadReputation=stBadReputation, stMimeType=stMimeType, kMinorVersion=kMinorVersion, stCategoriesEntry=stCategoriesEntry, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, trSystem=trSystem, stClientCount=stClientCount, trUser=trUser, mwgInfo=mwgInfo, stHttpRequests=stHttpRequests, mwgFTP=mwgFTP, stMalwareDetected=stMalwareDetected, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kCustomVersion=kCustomVersion, stHttpsTraffic=stHttpsTraffic, mwgHttps=mwgHttps, notifyReason=notifyReason, stHttpsRequests=stHttpsRequests, notifyReasonString=notifyReasonString, stHttpBytesFromClient=stHttpBytesFromClient, stFtpBytesFromServer=stFtpBytesFromServer, stBlockedByURLFilter=stBlockedByURLFilter, stCPULoadRaw=stCPULoadRaw, mwgMiscellaneous=mwgMiscellaneous, kMicroVersion=kMicroVersion, stFtpBytesFromClient=stFtpBytesFromClient, kProductVersion=kProductVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwg=mwg, stFtpBytesToServer=stFtpBytesToServer, notifyOrigin=notifyOrigin, mwgTrapVariables=mwgTrapVariables, kHotfixVersion=kHotfixVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpBytesFromServer=stHttpBytesFromServer, stResolveHostViaDNS=stResolveHostViaDNS, mwgTraps=mwgTraps, notifyOriginName=notifyOriginName, stCategoryName=stCategoryName, kProductName=kProductName, pMFEEngineVersion=pMFEEngineVersion, stCPULoad=stCPULoad, stHttpBytesToServer=stHttpBytesToServer, stConnectionsLegitimate=stConnectionsLegitimate, pAMEngineVersion=pAMEngineVersion, stConnectedSockets=stConnectedSockets, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pTSDBVersion=pTSDBVersion, stFtpBytesToClient=stFtpBytesToClient, stCategoriesTable=stCategoriesTable, trApplication=trApplication, kCompanyName=kCompanyName, stCategories=stCategories, stTimeForTransaction=stTimeForTransaction, notifySeverity=notifySeverity, stHandleConnectToServer=stHandleConnectToServer, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesToServer=stHttpsBytesToServer, notifyAdditional=notifyAdditional, notifyAffectedHost=notifyAffectedHost, kRevision=kRevision, mwgStatistics=mwgStatistics, pAMProactiveVersion=pAMProactiveVersion, kMajorVersion=kMajorVersion, mwgContent=mwgContent, stHttpsBytesFromClient=stHttpsBytesFromClient, stCPUIOWait=stCPUIOWait)
