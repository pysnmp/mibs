...
TimeTicks, Bits, Counter32, Integer32, Counter64, iso, Unsigned32, ObjectIdentity, NotificationType, ModuleIdentity, mib_2, MibIdentifier, IpAddress, enterprises, Gauge32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Bits", "Counter32", "Integer32", "Counter64", "iso", "Unsigned32", "ObjectIdentity", "NotificationType", "ModuleIdentity", "mib-2", "MibIdentifier", "IpAddress", "enterprises", "Gauge32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgFTP=mwgFTP, stBadReputation=stBadReputation, stFtpBytesFromClient=stFtpBytesFromClient, stBlockedByURLFilter=stBlockedByURLFilter, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsTraffic=stHttpsTraffic, mwgTrapVariables=mwgTrapVariables, stHttpsBytesFromServer=stHttpsBytesFromServer, notifyAdditional=notifyAdditional, stMalwareDetected=stMalwareDetected, mwgEngineVersions=mwgEngineVersions, notifyOrigin=notifyOrigin, pAMSignatureVersion=pAMSignatureVersion, stCategoryName=stCategoryName, kBuildNumber=kBuildNumber, kMinorVersion=kMinorVersion, stHttpBytesToClient=stHttpBytesToClient, mwgInfo=mwgInfo, pAMProactiveVersion=pAMProactiveVersion, stResolveHostViaDNS=stResolveHostViaDNS, stTimeForTransaction=stTimeForTransaction, stCategoryCount=stCategoryCount, stCategories=stCategories, notifyReasonString=notifyReasonString, mwgMiscellaneous=mwgMiscellaneous, pMFEEngineVersion=pMFEEngineVersion, kCompanyName=kCompanyName, mwg=mwg, stConnectionsBlocked=stConnectionsBlocked, stHttpsBytesToClient=stHttpsBytesToClient, pMFEDATVersion=pMFEDATVersion, kCustomVersion=kCustomVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpBytesFromServer=stFtpBytesFromServer, stHandleConnectToServer=stHandleConnectToServer, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpTraffic=stHttpTraffic, kHotfixVersion=kHotfixVersion, mwgStatistics=mwgStatistics, stFtpBytesToClient=stFtpBytesToClient, stFtpTraffic=stFtpTraffic, stCPULoad=stCPULoad, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgHttps=mwgHttps, stMemConsumed=stMemConsumed, notifyOriginName=notifyOriginName, PYSNMP_MODULE_ID=mwg, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgTraps=mwgTraps, notifyReason=notifyReason, trUser=trUser, kRevision=kRevision, stHttpRequests=stHttpRequests, trApplication=trApplication, stMimeType=stMimeType, kProductVersion=kProductVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, stBlockedByAntiMalware=stBlockedByAntiMalware, trSystem=trSystem, mwgContent=mwgContent, stCPUIOWait=stCPUIOWait, stHttpBytesFromClient=stHttpBytesFromClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCPULoadRaw=stCPULoadRaw, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifySeverity=notifySeverity, kMajorVersion=kMajorVersion, kProductName=kProductName, notifyAffectedHost=notifyAffectedHost, stFtpBytesToServer=stFtpBytesToServer, stClientCount=stClientCount, stConnectedSockets=stConnectedSockets, pTSDBVersion=pTSDBVersion, stHttpBytesToServer=stHttpBytesToServer, stHttpsRequests=stHttpsRequests, stHttpBytesFromServer=stHttpBytesFromServer, mwgHttp=mwgHttp, kMicroVersion=kMicroVersion, stConnectionsLegitimate=stConnectionsLegitimate, pAMEngineVersion=pAMEngineVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategoriesEntry=stCategoriesEntry, stCategoriesTable=stCategoriesTable)
