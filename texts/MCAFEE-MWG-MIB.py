...
iso, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, MibIdentifier, NotificationType, Counter32, ModuleIdentity, Counter64, Gauge32, mib_2, enterprises, TimeTicks, IpAddress, Unsigned32, Integer32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "MibIdentifier", "NotificationType", "Counter32", "ModuleIdentity", "Counter64", "Gauge32", "mib-2", "enterprises", "TimeTicks", "IpAddress", "Unsigned32", "Integer32", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kRevision=kRevision, notifyReason=notifyReason, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kCustomVersion=kCustomVersion, stCategoryName=stCategoryName, kBuildNumber=kBuildNumber, stHttpBytesToClient=stHttpBytesToClient, stHttpTraffic=stHttpTraffic, pAMProactiveVersion=pAMProactiveVersion, stConnectionsBlocked=stConnectionsBlocked, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpsBytesToServer=stHttpsBytesToServer, notifySeverity=notifySeverity, kMajorVersion=kMajorVersion, stFtpBytesFromClient=stFtpBytesFromClient, stHttpBytesToServer=stHttpBytesToServer, mwgFTP=mwgFTP, mwgHttps=mwgHttps, stHandleConnectToServer=stHandleConnectToServer, kCompanyName=kCompanyName, stMimeType=stMimeType, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgStatistics=mwgStatistics, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kMinorVersion=kMinorVersion, notifyOrigin=notifyOrigin, stHttpsTraffic=stHttpsTraffic, stHttpsBytesToClient=stHttpsBytesToClient, mwgTraps=mwgTraps, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsBytesFromClient=stHttpsBytesFromClient, stCategoriesTable=stCategoriesTable, stFtpBytesToClient=stFtpBytesToClient, pAMSignatureVersion=pAMSignatureVersion, notifyAffectedHost=notifyAffectedHost, stHttpRequests=stHttpRequests, mwgInfo=mwgInfo, notifyAdditional=notifyAdditional, kProductVersion=kProductVersion, kHotfixVersion=kHotfixVersion, notifyReasonString=notifyReasonString, kProductName=kProductName, stTimeForTransaction=stTimeForTransaction, stMalwareDetected=stMalwareDetected, pMFEEngineVersion=pMFEEngineVersion, mwgMiscellaneous=mwgMiscellaneous, stHttpBytesFromClient=stHttpBytesFromClient, stCPULoadRaw=stCPULoadRaw, mwgEngineVersions=mwgEngineVersions, stCategoryCount=stCategoryCount, pMFEDATVersion=pMFEDATVersion, stFtpTraffic=stFtpTraffic, stHttpsRequests=stHttpsRequests, stFtpBytesFromServer=stFtpBytesFromServer, stFtpBytesToServer=stFtpBytesToServer, stBlockedByURLFilter=stBlockedByURLFilter, PYSNMP_MODULE_ID=mwg, trSystem=trSystem, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgTrapVariables=mwgTrapVariables, pAMEngineVersion=pAMEngineVersion, stBadReputation=stBadReputation, stMemConsumed=stMemConsumed, mwgContent=mwgContent, stBlockedByMediaFilter=stBlockedByMediaFilter, mwg=mwg, stCategories=stCategories, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifyOriginName=notifyOriginName, trApplication=trApplication, stResolveHostViaDNS=stResolveHostViaDNS, stConnectedSockets=stConnectedSockets, trUser=trUser, stCPULoad=stCPULoad, stHttpBytesFromServer=stHttpBytesFromServer, stCategoriesEntry=stCategoriesEntry, kMicroVersion=kMicroVersion, pTSDBVersion=pTSDBVersion, stClientCount=stClientCount, stCPUIOWait=stCPUIOWait, mwgHttp=mwgHttp)
