...
Bits, Unsigned32, ModuleIdentity, Counter64, NotificationType, Counter32, enterprises, iso, TimeTicks, ObjectIdentity, MibIdentifier, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, mib_2, Integer32, Gauge32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Unsigned32", "ModuleIdentity", "Counter64", "NotificationType", "Counter32", "enterprises", "iso", "TimeTicks", "ObjectIdentity", "MibIdentifier", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "mib-2", "Integer32", "Gauge32", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", trUser=trUser, kMajorVersion=kMajorVersion, pMFEDATVersion=pMFEDATVersion, stHttpsRequests=stHttpsRequests, kMicroVersion=kMicroVersion, stHttpsTraffic=stHttpsTraffic, pTSDBVersion=pTSDBVersion, kCompanyName=kCompanyName, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCPUIOWait=stCPUIOWait, stFtpTraffic=stFtpTraffic, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stMalwareDetected=stMalwareDetected, stHttpBytesToClient=stHttpBytesToClient, stHttpTraffic=stHttpTraffic, stResolveHostViaDNS=stResolveHostViaDNS, mwgHttps=mwgHttps, notifyAffectedHost=notifyAffectedHost, stHttpRequests=stHttpRequests, kProductName=kProductName, stBlockedByURLFilter=stBlockedByURLFilter, mwg=mwg, mwgStatistics=mwgStatistics, pAMProactiveVersion=pAMProactiveVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stBlockedByMediaFilter=stBlockedByMediaFilter, stCPULoadRaw=stCPULoadRaw, kCustomVersion=kCustomVersion, stHttpsBytesToServer=stHttpsBytesToServer, stBadReputation=stBadReputation, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kMinorVersion=kMinorVersion, stFtpBytesToClient=stFtpBytesToClient, stHttpsBytesToClient=stHttpsBytesToClient, stFtpBytesFromClient=stFtpBytesFromClient, stTimeForTransaction=stTimeForTransaction, stClientCount=stClientCount, mwgEngineVersions=mwgEngineVersions, stHttpBytesFromClient=stHttpBytesFromClient, stMimeType=stMimeType, pAMSignatureVersion=pAMSignatureVersion, kHotfixVersion=kHotfixVersion, stCategoryName=stCategoryName, stMemConsumed=stMemConsumed, notifySeverity=notifySeverity, notifyOrigin=notifyOrigin, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgInfo=mwgInfo, kBuildNumber=kBuildNumber, stLastSentLastReceivedServer=stLastSentLastReceivedServer, PYSNMP_MODULE_ID=mwg, kRevision=kRevision, mwgHttp=mwgHttp, notifyReasonString=notifyReasonString, mwgTrapVariables=mwgTrapVariables, trSystem=trSystem, stHttpBytesToServer=stHttpBytesToServer, stConnectionsLegitimate=stConnectionsLegitimate, mwgContent=mwgContent, stHttpBytesFromServer=stHttpBytesFromServer, stCategories=stCategories, stConnectedSockets=stConnectedSockets, stFtpBytesFromServer=stFtpBytesFromServer, kProductVersion=kProductVersion, notifyAdditional=notifyAdditional, stConnectionsBlocked=stConnectionsBlocked, stCategoriesEntry=stCategoriesEntry, stCPULoad=stCPULoad, stCategoryCount=stCategoryCount, notifyOriginName=notifyOriginName, trApplication=trApplication, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgMiscellaneous=mwgMiscellaneous, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgTraps=mwgTraps, stCategoriesTable=stCategoriesTable, notifyReason=notifyReason, stFtpBytesToServer=stFtpBytesToServer, pAMEngineVersion=pAMEngineVersion, stHandleConnectToServer=stHandleConnectToServer, mwgFTP=mwgFTP, pMFEEngineVersion=pMFEEngineVersion)
