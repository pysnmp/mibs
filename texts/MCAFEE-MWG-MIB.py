...
NotificationType, Counter32, ObjectIdentity, MibIdentifier, Integer32, mib_2, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, Counter64, ModuleIdentity, enterprises, TimeTicks, iso, IpAddress, Bits, Gauge32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "Counter32", "ObjectIdentity", "MibIdentifier", "Integer32", "mib-2", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "Counter64", "ModuleIdentity", "enterprises", "TimeTicks", "iso", "IpAddress", "Bits", "Gauge32", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgFTP=mwgFTP, stClientCount=stClientCount, stConnectedSockets=stConnectedSockets, stHttpRequests=stHttpRequests, notifySeverity=notifySeverity, pMFEDATVersion=pMFEDATVersion, stCategoriesTable=stCategoriesTable, trApplication=trApplication, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpBytesFromClient=stFtpBytesFromClient, pAMProactiveVersion=pAMProactiveVersion, stMalwareDetected=stMalwareDetected, stHttpsBytesFromServer=stHttpsBytesFromServer, kMicroVersion=kMicroVersion, notifyReasonString=notifyReasonString, stConnectionsBlocked=stConnectionsBlocked, stCPULoadRaw=stCPULoadRaw, stHttpBytesToClient=stHttpBytesToClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stResolveHostViaDNS=stResolveHostViaDNS, stCategoryName=stCategoryName, stCategoryCount=stCategoryCount, stCategoriesEntry=stCategoriesEntry, kProductVersion=kProductVersion, pAMSignatureVersion=pAMSignatureVersion, stMimeType=stMimeType, kCustomVersion=kCustomVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stBlockedByURLFilter=stBlockedByURLFilter, stHttpTraffic=stHttpTraffic, stFtpBytesFromServer=stFtpBytesFromServer, mwgEngineVersions=mwgEngineVersions, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsBytesToClient=stHttpsBytesToClient, mwgInfo=mwgInfo, stBadReputation=stBadReputation, trSystem=trSystem, pAMEngineVersion=pAMEngineVersion, notifyAffectedHost=notifyAffectedHost, stFtpBytesToServer=stFtpBytesToServer, PYSNMP_MODULE_ID=mwg, mwgHttp=mwgHttp, stHttpBytesFromServer=stHttpBytesFromServer, notifyOrigin=notifyOrigin, mwgHttps=mwgHttps, stTimeForTransaction=stTimeForTransaction, mwgTraps=mwgTraps, pMFEEngineVersion=pMFEEngineVersion, notifyOriginName=notifyOriginName, notifyReason=notifyReason, stCPULoad=stCPULoad, kRevision=kRevision, stHandleConnectToServer=stHandleConnectToServer, stHttpBytesToServer=stHttpBytesToServer, stHttpsRequests=stHttpsRequests, stCategories=stCategories, notifyAdditional=notifyAdditional, stBlockedByMediaFilter=stBlockedByMediaFilter, stMemConsumed=stMemConsumed, stHttpsBytesToServer=stHttpsBytesToServer, kHotfixVersion=kHotfixVersion, mwgTrapVariables=mwgTrapVariables, stFtpTraffic=stFtpTraffic, kProductName=kProductName, stFtpBytesToClient=stFtpBytesToClient, mwgContent=mwgContent, kBuildNumber=kBuildNumber, mwg=mwg, trUser=trUser, stHttpBytesFromClient=stHttpBytesFromClient, stHttpsBytesFromClient=stHttpsBytesFromClient, mwgMiscellaneous=mwgMiscellaneous, kMinorVersion=kMinorVersion, stCPUIOWait=stCPUIOWait, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stBlockedByAntiMalware=stBlockedByAntiMalware, pTSDBVersion=pTSDBVersion, kMajorVersion=kMajorVersion, kCompanyName=kCompanyName, mwgStatistics=mwgStatistics, stHttpsTraffic=stHttpsTraffic)
