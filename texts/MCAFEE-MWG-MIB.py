...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpBytesFromClient=stHttpBytesFromClient, mwgMiscellaneous=mwgMiscellaneous, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpRequests=stHttpRequests, trApplication=trApplication, stHttpBytesToClient=stHttpBytesToClient, mwgEngineVersions=mwgEngineVersions, stBlockedByAntiMalware=stBlockedByAntiMalware, stBlockedByURLFilter=stBlockedByURLFilter, stHttpsBytesFromClient=stHttpsBytesFromClient, pAMProactiveVersion=pAMProactiveVersion, stCategories=stCategories, stCPULoad=stCPULoad, notifySeverity=notifySeverity, stHttpsTraffic=stHttpsTraffic, notifyAffectedHost=notifyAffectedHost, mwgTrapVariables=mwgTrapVariables, mwgContent=mwgContent, stFtpBytesFromServer=stFtpBytesFromServer, stFtpBytesToClient=stFtpBytesToClient, stHttpsBytesFromServer=stHttpsBytesFromServer, pTSDBVersion=pTSDBVersion, pAMEngineVersion=pAMEngineVersion, stMalwareDetected=stMalwareDetected, notifyAdditional=notifyAdditional, mwgHttp=mwgHttp, stHttpsBytesToServer=stHttpsBytesToServer, kMicroVersion=kMicroVersion, mwgFTP=mwgFTP, pMFEEngineVersion=pMFEEngineVersion, stConnectedSockets=stConnectedSockets, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stResolveHostViaDNS=stResolveHostViaDNS, stTimeForTransaction=stTimeForTransaction, pMFEDATVersion=pMFEDATVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifyOrigin=notifyOrigin, kProductVersion=kProductVersion, kRevision=kRevision, mwg=mwg, stBadReputation=stBadReputation, stLastSentLastReceivedServer=stLastSentLastReceivedServer, kMinorVersion=kMinorVersion, stHttpsBytesToClient=stHttpsBytesToClient, pAMSignatureVersion=pAMSignatureVersion, kCustomVersion=kCustomVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpBytesToServer=stHttpBytesToServer, mwgTraps=mwgTraps, mwgHttps=mwgHttps, kMajorVersion=kMajorVersion, stCategoriesEntry=stCategoriesEntry, stCategoryCount=stCategoryCount, stCPULoadRaw=stCPULoadRaw, trSystem=trSystem, kHotfixVersion=kHotfixVersion, stCategoriesTable=stCategoriesTable, stHandleConnectToServer=stHandleConnectToServer, stHttpTraffic=stHttpTraffic, stCategoryName=stCategoryName, notifyOriginName=notifyOriginName, trUser=trUser, stFtpBytesFromClient=stFtpBytesFromClient, stConnectionsBlocked=stConnectionsBlocked, mwgInfo=mwgInfo, kBuildNumber=kBuildNumber, mwgStatistics=mwgStatistics, kCompanyName=kCompanyName, stHttpsRequests=stHttpsRequests, stFtpBytesToServer=stFtpBytesToServer, stClientCount=stClientCount, PYSNMP_MODULE_ID=mwg, stMimeType=stMimeType, notifyReasonString=notifyReasonString, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kProductName=kProductName, stMemConsumed=stMemConsumed, stHttpBytesFromServer=stHttpBytesFromServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCPUIOWait=stCPUIOWait, stConnectionsLegitimate=stConnectionsLegitimate, notifyReason=notifyReason, stFtpTraffic=stFtpTraffic)
