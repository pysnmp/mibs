...
NotificationType, NotificationType, Counter64, Counter32, IpAddress, TimeTicks, ObjectIdentity, Integer32, Unsigned32, mib_2, iso, MibIdentifier, ModuleIdentity, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "NotificationType", "Counter64", "Counter32", "IpAddress", "TimeTicks", "ObjectIdentity", "Integer32", "Unsigned32", "mib-2", "iso", "MibIdentifier", "ModuleIdentity", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Bits")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", notifyOriginName=notifyOriginName, pAMProactiveVersion=pAMProactiveVersion, mwgContent=mwgContent, notifyOrigin=notifyOrigin, stFtpBytesFromServer=stFtpBytesFromServer, stHttpTraffic=stHttpTraffic, kCompanyName=kCompanyName, stHttpsBytesToClient=stHttpsBytesToClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCPULoadRaw=stCPULoadRaw, kCustomVersion=kCustomVersion, kRevision=kRevision, notifyAffectedHost=notifyAffectedHost, pMFEDATVersion=pMFEDATVersion, stHandleConnectToServer=stHandleConnectToServer, stMimeType=stMimeType, stTimeForTransaction=stTimeForTransaction, pMFEEngineVersion=pMFEEngineVersion, pAMSignatureVersion=pAMSignatureVersion, stHttpsTraffic=stHttpsTraffic, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifySeverity=notifySeverity, stHttpBytesFromClient=stHttpBytesFromClient, stHttpsBytesToServer=stHttpsBytesToServer, stFtpBytesToClient=stFtpBytesToClient, mwg=mwg, stHttpsBytesFromServer=stHttpsBytesFromServer, trUser=trUser, stCategoriesEntry=stCategoriesEntry, stHttpsBytesFromClient=stHttpsBytesFromClient, pTSDBVersion=pTSDBVersion, trApplication=trApplication, stBlockedByMediaFilter=stBlockedByMediaFilter, trSystem=trSystem, mwgHttp=mwgHttp, stCategoriesTable=stCategoriesTable, stResolveHostViaDNS=stResolveHostViaDNS, notifyReason=notifyReason, kMinorVersion=kMinorVersion, mwgTraps=mwgTraps, kProductVersion=kProductVersion, notifyReasonString=notifyReasonString, kHotfixVersion=kHotfixVersion, stFtpTraffic=stFtpTraffic, stFtpBytesToServer=stFtpBytesToServer, mwgEngineVersions=mwgEngineVersions, kMicroVersion=kMicroVersion, stHttpRequests=stHttpRequests, stCategoryCount=stCategoryCount, stBlockedByURLFilter=stBlockedByURLFilter, mwgFTP=mwgFTP, stHttpBytesToClient=stHttpBytesToClient, stHttpBytesToServer=stHttpBytesToServer, stMalwareDetected=stMalwareDetected, stMemConsumed=stMemConsumed, mwgTrapVariables=mwgTrapVariables, stCPULoad=stCPULoad, stConnectedSockets=stConnectedSockets, mwgHttps=mwgHttps, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFtpBytesFromClient=stFtpBytesFromClient, stClientCount=stClientCount, stCategoryName=stCategoryName, mwgStatistics=mwgStatistics, stHttpBytesFromServer=stHttpBytesFromServer, stConnectionsLegitimate=stConnectionsLegitimate, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategories=stCategories, kProductName=kProductName, stCPUIOWait=stCPUIOWait, notifyAdditional=notifyAdditional, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stBadReputation=stBadReputation, mwgInfo=mwgInfo, kBuildNumber=kBuildNumber, stHttpsRequests=stHttpsRequests, stConnectionsBlocked=stConnectionsBlocked, mwgMiscellaneous=mwgMiscellaneous, pAMEngineVersion=pAMEngineVersion, PYSNMP_MODULE_ID=mwg, kMajorVersion=kMajorVersion)
