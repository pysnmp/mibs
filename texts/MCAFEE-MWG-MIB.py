...
mib_2, Unsigned32, Counter32, TimeTicks, ObjectIdentity, iso, Counter64, IpAddress, ModuleIdentity, Bits, MibIdentifier, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Gauge32, NotificationType, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "mib-2", "Unsigned32", "Counter32", "TimeTicks", "ObjectIdentity", "iso", "Counter64", "IpAddress", "ModuleIdentity", "Bits", "MibIdentifier", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Gauge32", "NotificationType", "enterprises")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgStatistics=mwgStatistics, stBadReputation=stBadReputation, notifyOrigin=notifyOrigin, stHttpsBytesFromClient=stHttpsBytesFromClient, kBuildNumber=kBuildNumber, stCategoryName=stCategoryName, stFtpBytesToServer=stFtpBytesToServer, mwgInfo=mwgInfo, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyReasonString=notifyReasonString, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCPUIOWait=stCPUIOWait, mwg=mwg, stResolveHostViaDNS=stResolveHostViaDNS, pAMSignatureVersion=pAMSignatureVersion, pMFEEngineVersion=pMFEEngineVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCPULoadRaw=stCPULoadRaw, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesToClient=stHttpsBytesToClient, stFtpBytesFromServer=stFtpBytesFromServer, pTSDBVersion=pTSDBVersion, notifyOriginName=notifyOriginName, stHttpsRequests=stHttpsRequests, kRevision=kRevision, stHttpBytesToServer=stHttpBytesToServer, mwgMiscellaneous=mwgMiscellaneous, stClientCount=stClientCount, pAMProactiveVersion=pAMProactiveVersion, stTimeForTransaction=stTimeForTransaction, kMajorVersion=kMajorVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyReason=notifyReason, stMemConsumed=stMemConsumed, mwgTraps=mwgTraps, trSystem=trSystem, trApplication=trApplication, mwgEngineVersions=mwgEngineVersions, stHttpBytesFromServer=stHttpBytesFromServer, stFtpBytesFromClient=stFtpBytesFromClient, notifySeverity=notifySeverity, kProductName=kProductName, stConnectedSockets=stConnectedSockets, stCategoriesEntry=stCategoriesEntry, mwgHttps=mwgHttps, stFtpTraffic=stFtpTraffic, stHttpTraffic=stHttpTraffic, kProductVersion=kProductVersion, stHandleConnectToServer=stHandleConnectToServer, stFtpBytesToClient=stFtpBytesToClient, pAMEngineVersion=pAMEngineVersion, stCPULoad=stCPULoad, stBlockedByURLFilter=stBlockedByURLFilter, kCompanyName=kCompanyName, mwgContent=mwgContent, PYSNMP_MODULE_ID=mwg, mwgFTP=mwgFTP, trUser=trUser, stHttpsBytesToServer=stHttpsBytesToServer, kMicroVersion=kMicroVersion, kCustomVersion=kCustomVersion, stConnectionsLegitimate=stConnectionsLegitimate, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpBytesFromClient=stHttpBytesFromClient, kHotfixVersion=kHotfixVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, notifyAdditional=notifyAdditional, pMFEDATVersion=pMFEDATVersion, stMimeType=stMimeType, mwgTrapVariables=mwgTrapVariables, stCategories=stCategories, stHttpsTraffic=stHttpsTraffic, stCategoriesTable=stCategoriesTable, stMalwareDetected=stMalwareDetected, stHttpBytesToClient=stHttpBytesToClient, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgHttp=mwgHttp, stCategoryCount=stCategoryCount, kMinorVersion=kMinorVersion, notifyAffectedHost=notifyAffectedHost, stHttpRequests=stHttpRequests, stConnectionsBlocked=stConnectionsBlocked, stHttpsBytesFromServer=stHttpsBytesFromServer)
