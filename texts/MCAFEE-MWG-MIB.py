...
enterprises, TimeTicks, ObjectIdentity, Counter64, iso, IpAddress, MibIdentifier, ModuleIdentity, Integer32, NotificationType, Counter32, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Bits, mib_2, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "TimeTicks", "ObjectIdentity", "Counter64", "iso", "IpAddress", "MibIdentifier", "ModuleIdentity", "Integer32", "NotificationType", "Counter32", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Bits", "mib-2", "Gauge32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kMajorVersion=kMajorVersion, stCategories=stCategories, kMinorVersion=kMinorVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, mwg=mwg, stCategoryName=stCategoryName, PYSNMP_MODULE_ID=mwg, pMFEDATVersion=pMFEDATVersion, stConnectionsBlocked=stConnectionsBlocked, mwgFTP=mwgFTP, stCPUIOWait=stCPUIOWait, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgStatistics=mwgStatistics, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgMiscellaneous=mwgMiscellaneous, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgHttp=mwgHttp, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kBuildNumber=kBuildNumber, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsTraffic=stHttpsTraffic, stMemConsumed=stMemConsumed, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, pAMEngineVersion=pAMEngineVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsBytesFromClient=stHttpsBytesFromClient, kCompanyName=kCompanyName, kProductName=kProductName, stFtpBytesFromClient=stFtpBytesFromClient, stHttpBytesFromServer=stHttpBytesFromServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stBlockedByURLFilter=stBlockedByURLFilter, mwgEngineVersions=mwgEngineVersions, stCategoryCount=stCategoryCount, trApplication=trApplication, stFtpBytesToClient=stFtpBytesToClient, stCPULoad=stCPULoad, stHttpBytesToServer=stHttpBytesToServer, pTSDBVersion=pTSDBVersion, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsRequests=stHttpsRequests, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesToClient=stHttpBytesToClient, stHandleConnectToServer=stHandleConnectToServer, stCPULoadRaw=stCPULoadRaw, stHttpTraffic=stHttpTraffic, stFtpBytesFromServer=stFtpBytesFromServer, notifyAffectedHost=notifyAffectedHost, stHttpBytesFromClient=stHttpBytesFromClient, mwgInfo=mwgInfo, notifyReason=notifyReason, stCategoriesEntry=stCategoriesEntry, mwgTraps=mwgTraps, stMalwareDetected=stMalwareDetected, trUser=trUser, mwgTrapVariables=mwgTrapVariables, stFtpBytesToServer=stFtpBytesToServer, stMimeType=stMimeType, stConnectedSockets=stConnectedSockets, stTimeForTransaction=stTimeForTransaction, notifyReasonString=notifyReasonString, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyAdditional=notifyAdditional, kHotfixVersion=kHotfixVersion, kCustomVersion=kCustomVersion, pMFEEngineVersion=pMFEEngineVersion, stFtpTraffic=stFtpTraffic, mwgHttps=mwgHttps, trSystem=trSystem, stCategoriesTable=stCategoriesTable, kMicroVersion=kMicroVersion, kRevision=kRevision, kProductVersion=kProductVersion, stHttpRequests=stHttpRequests, stBadReputation=stBadReputation, notifyOrigin=notifyOrigin, mwgContent=mwgContent, notifySeverity=notifySeverity, stClientCount=stClientCount, pAMProactiveVersion=pAMProactiveVersion, stResolveHostViaDNS=stResolveHostViaDNS, notifyOriginName=notifyOriginName)
