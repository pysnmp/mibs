...
Gauge32, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Counter64, Unsigned32, mib_2, NotificationType, ModuleIdentity, IpAddress, NotificationType, MibIdentifier, TimeTicks, Bits, ObjectIdentity, enterprises, iso = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Counter64", "Unsigned32", "mib-2", "NotificationType", "ModuleIdentity", "IpAddress", "NotificationType", "MibIdentifier", "TimeTicks", "Bits", "ObjectIdentity", "enterprises", "iso")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pTSDBVersion=pTSDBVersion, stResolveHostViaDNS=stResolveHostViaDNS, trSystem=trSystem, pMFEDATVersion=pMFEDATVersion, stHttpBytesToServer=stHttpBytesToServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsBytesFromServer=stHttpsBytesFromServer, stMemConsumed=stMemConsumed, trApplication=trApplication, stConnectionsLegitimate=stConnectionsLegitimate, mwgContent=mwgContent, kCompanyName=kCompanyName, stFtpBytesToServer=stFtpBytesToServer, stTimeForTransaction=stTimeForTransaction, mwgHttp=mwgHttp, stCategoriesEntry=stCategoriesEntry, pMFEEngineVersion=pMFEEngineVersion, stFtpBytesFromClient=stFtpBytesFromClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kBuildNumber=kBuildNumber, stFtpTraffic=stFtpTraffic, kCustomVersion=kCustomVersion, stHttpsBytesToClient=stHttpsBytesToClient, PYSNMP_MODULE_ID=mwg, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, notifyReasonString=notifyReasonString, stBlockedByURLFilter=stBlockedByURLFilter, stBlockedByMediaFilter=stBlockedByMediaFilter, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgHttps=mwgHttps, stMalwareDetected=stMalwareDetected, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpRequests=stHttpRequests, mwgStatistics=mwgStatistics, kRevision=kRevision, notifyOrigin=notifyOrigin, stHttpsRequests=stHttpsRequests, stHttpBytesFromServer=stHttpBytesFromServer, kMajorVersion=kMajorVersion, stFtpBytesFromServer=stFtpBytesFromServer, stCPUIOWait=stCPUIOWait, stHttpTraffic=stHttpTraffic, stHttpsBytesToServer=stHttpsBytesToServer, stConnectionsBlocked=stConnectionsBlocked, mwgTraps=mwgTraps, notifyAffectedHost=notifyAffectedHost, stHandleConnectToServer=stHandleConnectToServer, mwgTrapVariables=mwgTrapVariables, notifyOriginName=notifyOriginName, stBlockedByAntiMalware=stBlockedByAntiMalware, pAMProactiveVersion=pAMProactiveVersion, stHttpBytesToClient=stHttpBytesToClient, notifySeverity=notifySeverity, stConnectedSockets=stConnectedSockets, mwgEngineVersions=mwgEngineVersions, pAMEngineVersion=pAMEngineVersion, stHttpsTraffic=stHttpsTraffic, stClientCount=stClientCount, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCategoriesTable=stCategoriesTable, kProductVersion=kProductVersion, pAMSignatureVersion=pAMSignatureVersion, stCategoryName=stCategoryName, kMinorVersion=kMinorVersion, stBadReputation=stBadReputation, trUser=trUser, mwgMiscellaneous=mwgMiscellaneous, stCategoryCount=stCategoryCount, stFtpBytesToClient=stFtpBytesToClient, stCPULoadRaw=stCPULoadRaw, notifyAdditional=notifyAdditional, mwgFTP=mwgFTP, mwgInfo=mwgInfo, stHttpBytesFromClient=stHttpBytesFromClient, notifyReason=notifyReason, stCategories=stCategories, stHttpsBytesFromClient=stHttpsBytesFromClient, stCPULoad=stCPULoad, stMimeType=stMimeType, mwg=mwg, kMicroVersion=kMicroVersion, kProductName=kProductName, kHotfixVersion=kHotfixVersion)
