...
Gauge32, Integer32, Unsigned32, Counter64, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, iso, NotificationType, ObjectIdentity, NotificationType, Bits, TimeTicks, MibIdentifier, mib_2, Counter32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Integer32", "Unsigned32", "Counter64", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "iso", "NotificationType", "ObjectIdentity", "NotificationType", "Bits", "TimeTicks", "MibIdentifier", "mib-2", "Counter32", "ModuleIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pMFEEngineVersion=pMFEEngineVersion, pMFEDATVersion=pMFEDATVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgHttp=mwgHttp, stCategoriesTable=stCategoriesTable, pTSDBVersion=pTSDBVersion, mwgTrapVariables=mwgTrapVariables, stCategories=stCategories, stFtpBytesFromClient=stFtpBytesFromClient, kProductName=kProductName, kProductVersion=kProductVersion, pAMSignatureVersion=pAMSignatureVersion, stHttpsRequests=stHttpsRequests, stClientCount=stClientCount, PYSNMP_MODULE_ID=mwg, kRevision=kRevision, kCustomVersion=kCustomVersion, trSystem=trSystem, mwgHttps=mwgHttps, mwgContent=mwgContent, kHotfixVersion=kHotfixVersion, kBuildNumber=kBuildNumber, notifySeverity=notifySeverity, mwgStatistics=mwgStatistics, mwgEngineVersions=mwgEngineVersions, stConnectedSockets=stConnectedSockets, stCPULoad=stCPULoad, stHttpTraffic=stHttpTraffic, trUser=trUser, mwgInfo=mwgInfo, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategoriesEntry=stCategoriesEntry, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stMemConsumed=stMemConsumed, notifyAffectedHost=notifyAffectedHost, stHttpsBytesToClient=stHttpsBytesToClient, pAMProactiveVersion=pAMProactiveVersion, stFtpBytesToServer=stFtpBytesToServer, stBadReputation=stBadReputation, stCPUIOWait=stCPUIOWait, stMimeType=stMimeType, kMinorVersion=kMinorVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgTraps=mwgTraps, notifyReasonString=notifyReasonString, stConnectionsLegitimate=stConnectionsLegitimate, stBlockedByURLFilter=stBlockedByURLFilter, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifyOriginName=notifyOriginName, stCategoryCount=stCategoryCount, stCPULoadRaw=stCPULoadRaw, stHttpsBytesFromClient=stHttpsBytesFromClient, mwg=mwg, stBlockedByMediaFilter=stBlockedByMediaFilter, kMicroVersion=kMicroVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpBytesFromServer=stFtpBytesFromServer, notifyReason=notifyReason, stBlockedByAntiMalware=stBlockedByAntiMalware, stTimeForTransaction=stTimeForTransaction, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, trApplication=trApplication, stConnectionsBlocked=stConnectionsBlocked, mwgFTP=mwgFTP, notifyAdditional=notifyAdditional, stHttpsBytesToServer=stHttpsBytesToServer, stResolveHostViaDNS=stResolveHostViaDNS, stHandleConnectToServer=stHandleConnectToServer, notifyOrigin=notifyOrigin, stHttpBytesToClient=stHttpBytesToClient, stHttpsTraffic=stHttpsTraffic, stFtpBytesToClient=stFtpBytesToClient, stHttpBytesFromServer=stHttpBytesFromServer, pAMEngineVersion=pAMEngineVersion, kMajorVersion=kMajorVersion, mwgMiscellaneous=mwgMiscellaneous, stFtpTraffic=stFtpTraffic, stCategoryName=stCategoryName, kCompanyName=kCompanyName, stMalwareDetected=stMalwareDetected, stHttpBytesToServer=stHttpBytesToServer, stHttpBytesFromClient=stHttpBytesFromClient, stHttpRequests=stHttpRequests)
