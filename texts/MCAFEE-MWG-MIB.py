...
Gauge32, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, mib_2, enterprises, IpAddress, iso, ObjectIdentity, Counter64, TimeTicks, ModuleIdentity, Bits, Counter32, NotificationType, MibIdentifier, Integer32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "mib-2", "enterprises", "IpAddress", "iso", "ObjectIdentity", "Counter64", "TimeTicks", "ModuleIdentity", "Bits", "Counter32", "NotificationType", "MibIdentifier", "Integer32", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHandleConnectToServer=stHandleConnectToServer, notifyAffectedHost=notifyAffectedHost, mwgTraps=mwgTraps, stHttpBytesToServer=stHttpBytesToServer, kCompanyName=kCompanyName, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgHttps=mwgHttps, stBlockedByMediaFilter=stBlockedByMediaFilter, PYSNMP_MODULE_ID=mwg, stHttpsRequests=stHttpsRequests, stCPULoad=stCPULoad, stClientCount=stClientCount, stTimeForTransaction=stTimeForTransaction, stLastSentLastReceivedServer=stLastSentLastReceivedServer, trApplication=trApplication, mwgStatistics=mwgStatistics, pAMProactiveVersion=pAMProactiveVersion, stFtpBytesFromServer=stFtpBytesFromServer, kHotfixVersion=kHotfixVersion, stCPUIOWait=stCPUIOWait, stBadReputation=stBadReputation, kProductName=kProductName, pAMSignatureVersion=pAMSignatureVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifyOriginName=notifyOriginName, stCategories=stCategories, kBuildNumber=kBuildNumber, stCategoriesEntry=stCategoriesEntry, notifyReasonString=notifyReasonString, mwgFTP=mwgFTP, pMFEEngineVersion=pMFEEngineVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgMiscellaneous=mwgMiscellaneous, stConnectionsLegitimate=stConnectionsLegitimate, stMemConsumed=stMemConsumed, stHttpsBytesToServer=stHttpsBytesToServer, kRevision=kRevision, stHttpTraffic=stHttpTraffic, stFtpBytesToClient=stFtpBytesToClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, notifyOrigin=notifyOrigin, pAMEngineVersion=pAMEngineVersion, stFtpBytesToServer=stFtpBytesToServer, kMajorVersion=kMajorVersion, stCategoryName=stCategoryName, stMalwareDetected=stMalwareDetected, pTSDBVersion=pTSDBVersion, stHttpsTraffic=stHttpsTraffic, stHttpBytesToClient=stHttpBytesToClient, stMimeType=stMimeType, stHttpBytesFromServer=stHttpBytesFromServer, notifyAdditional=notifyAdditional, mwgContent=mwgContent, mwgHttp=mwgHttp, kCustomVersion=kCustomVersion, kProductVersion=kProductVersion, stCategoryCount=stCategoryCount, mwgTrapVariables=mwgTrapVariables, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, notifyReason=notifyReason, mwgInfo=mwgInfo, stCategoriesTable=stCategoriesTable, stBlockedByURLFilter=stBlockedByURLFilter, trUser=trUser, stHttpsBytesFromClient=stHttpsBytesFromClient, stConnectedSockets=stConnectedSockets, stHttpsBytesFromServer=stHttpsBytesFromServer, stCPULoadRaw=stCPULoadRaw, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpsBytesToClient=stHttpsBytesToClient, notifySeverity=notifySeverity, kMinorVersion=kMinorVersion, mwg=mwg, pMFEDATVersion=pMFEDATVersion, stHttpBytesFromClient=stHttpBytesFromClient, stHttpRequests=stHttpRequests, stFtpBytesFromClient=stFtpBytesFromClient, stConnectionsBlocked=stConnectionsBlocked, kMicroVersion=kMicroVersion, stFtpTraffic=stFtpTraffic, mwgEngineVersions=mwgEngineVersions, trSystem=trSystem, stResolveHostViaDNS=stResolveHostViaDNS)
