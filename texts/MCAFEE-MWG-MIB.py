...
mib_2, Gauge32, enterprises, Counter64, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, TimeTicks, MibIdentifier, NotificationType, iso, Bits, ObjectIdentity, Integer32, ModuleIdentity, Counter32, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "mib-2", "Gauge32", "enterprises", "Counter64", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "TimeTicks", "MibIdentifier", "NotificationType", "iso", "Bits", "ObjectIdentity", "Integer32", "ModuleIdentity", "Counter32", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsBytesToClient=stHttpsBytesToClient, stMemConsumed=stMemConsumed, stHttpsBytesFromClient=stHttpsBytesFromClient, pMFEEngineVersion=pMFEEngineVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stConnectedSockets=stConnectedSockets, pMFEDATVersion=pMFEDATVersion, kProductName=kProductName, kProductVersion=kProductVersion, stBlockedByURLFilter=stBlockedByURLFilter, mwgStatistics=mwgStatistics, stMimeType=stMimeType, notifySeverity=notifySeverity, notifyAdditional=notifyAdditional, stCategories=stCategories, stFtpBytesFromClient=stFtpBytesFromClient, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgHttps=mwgHttps, stCategoryCount=stCategoryCount, stMalwareDetected=stMalwareDetected, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgTrapVariables=mwgTrapVariables, pAMProactiveVersion=pAMProactiveVersion, trSystem=trSystem, mwgMiscellaneous=mwgMiscellaneous, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpBytesToServer=stHttpBytesToServer, kMicroVersion=kMicroVersion, stHttpBytesFromServer=stHttpBytesFromServer, stClientCount=stClientCount, kCompanyName=kCompanyName, kCustomVersion=kCustomVersion, stCategoriesEntry=stCategoriesEntry, kMinorVersion=kMinorVersion, stHttpTraffic=stHttpTraffic, stConnectionsLegitimate=stConnectionsLegitimate, notifyAffectedHost=notifyAffectedHost, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pAMSignatureVersion=pAMSignatureVersion, mwgTraps=mwgTraps, notifyOrigin=notifyOrigin, stHttpBytesToClient=stHttpBytesToClient, stCategoriesTable=stCategoriesTable, kMajorVersion=kMajorVersion, stCategoryName=stCategoryName, stResolveHostViaDNS=stResolveHostViaDNS, stFtpBytesToClient=stFtpBytesToClient, stHttpsRequests=stHttpsRequests, trUser=trUser, kBuildNumber=kBuildNumber, stCPUIOWait=stCPUIOWait, stHttpsTraffic=stHttpsTraffic, mwgContent=mwgContent, kHotfixVersion=kHotfixVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwg=mwg, pTSDBVersion=pTSDBVersion, stHandleConnectToServer=stHandleConnectToServer, PYSNMP_MODULE_ID=mwg, trApplication=trApplication, stConnectionsBlocked=stConnectionsBlocked, mwgHttp=mwgHttp, stBadReputation=stBadReputation, stTimeForTransaction=stTimeForTransaction, kRevision=kRevision, stFtpTraffic=stFtpTraffic, notifyReason=notifyReason, stFtpBytesFromServer=stFtpBytesFromServer, pAMEngineVersion=pAMEngineVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgInfo=mwgInfo, stFtpBytesToServer=stFtpBytesToServer, notifyReasonString=notifyReasonString, stCPULoadRaw=stCPULoadRaw, stHttpBytesFromClient=stHttpBytesFromClient, stHttpRequests=stHttpRequests, mwgFTP=mwgFTP, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpsBytesToServer=stHttpsBytesToServer, notifyOriginName=notifyOriginName, stCPULoad=stCPULoad, mwgEngineVersions=mwgEngineVersions)
