...
NotificationType, ModuleIdentity, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, IpAddress, MibIdentifier, Counter32, Gauge32, TimeTicks, iso, Bits, Integer32, ObjectIdentity, mib_2, Counter64, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "ModuleIdentity", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "IpAddress", "MibIdentifier", "Counter32", "Gauge32", "TimeTicks", "iso", "Bits", "Integer32", "ObjectIdentity", "mib-2", "Counter64", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stBlockedByURLFilter=stBlockedByURLFilter, mwgStatistics=mwgStatistics, kBuildNumber=kBuildNumber, stHttpsRequests=stHttpsRequests, stCPULoadRaw=stCPULoadRaw, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stBadReputation=stBadReputation, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stClientCount=stClientCount, mwgFTP=mwgFTP, notifyReasonString=notifyReasonString, stCategoriesTable=stCategoriesTable, stMemConsumed=stMemConsumed, pMFEDATVersion=pMFEDATVersion, stConnectedSockets=stConnectedSockets, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesFromServer=stHttpsBytesFromServer, stFtpBytesToServer=stFtpBytesToServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, pAMSignatureVersion=pAMSignatureVersion, stHttpRequests=stHttpRequests, stHttpsBytesToServer=stHttpsBytesToServer, kCompanyName=kCompanyName, pAMProactiveVersion=pAMProactiveVersion, stCPULoad=stCPULoad, stCategoriesEntry=stCategoriesEntry, kMicroVersion=kMicroVersion, mwgHttp=mwgHttp, stHandleConnectToServer=stHandleConnectToServer, mwgEngineVersions=mwgEngineVersions, stCPUIOWait=stCPUIOWait, notifyAffectedHost=notifyAffectedHost, kProductVersion=kProductVersion, stResolveHostViaDNS=stResolveHostViaDNS, kMajorVersion=kMajorVersion, stHttpBytesToClient=stHttpBytesToClient, notifyOrigin=notifyOrigin, mwgTrapVariables=mwgTrapVariables, stCategories=stCategories, mwgTraps=mwgTraps, stConnectionsBlocked=stConnectionsBlocked, trSystem=trSystem, mwgHttps=mwgHttps, stHttpBytesFromClient=stHttpBytesFromClient, notifyOriginName=notifyOriginName, mwgInfo=mwgInfo, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stMimeType=stMimeType, stFtpBytesToClient=stFtpBytesToClient, stHttpTraffic=stHttpTraffic, stFtpBytesFromServer=stFtpBytesFromServer, mwg=mwg, notifyReason=notifyReason, kProductName=kProductName, stHttpBytesFromServer=stHttpBytesFromServer, pMFEEngineVersion=pMFEEngineVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, pTSDBVersion=pTSDBVersion, stTimeForTransaction=stTimeForTransaction, stConnectionsLegitimate=stConnectionsLegitimate, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, PYSNMP_MODULE_ID=mwg, stHttpsTraffic=stHttpsTraffic, stHttpsBytesFromClient=stHttpsBytesFromClient, kCustomVersion=kCustomVersion, kRevision=kRevision, stFtpTraffic=stFtpTraffic, stCategoryName=stCategoryName, stMalwareDetected=stMalwareDetected, kHotfixVersion=kHotfixVersion, mwgContent=mwgContent, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, trApplication=trApplication, stFtpBytesFromClient=stFtpBytesFromClient, notifySeverity=notifySeverity, notifyAdditional=notifyAdditional, trUser=trUser, stCategoryCount=stCategoryCount, stHttpsBytesToClient=stHttpsBytesToClient, kMinorVersion=kMinorVersion, mwgMiscellaneous=mwgMiscellaneous, stHttpBytesToServer=stHttpBytesToServer, pAMEngineVersion=pAMEngineVersion)
