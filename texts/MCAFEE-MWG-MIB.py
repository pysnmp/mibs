...
MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Bits, iso, Counter32, mib_2, TimeTicks, NotificationType, Integer32, enterprises, NotificationType, Counter64, ObjectIdentity, Unsigned32, IpAddress, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Bits", "iso", "Counter32", "mib-2", "TimeTicks", "NotificationType", "Integer32", "enterprises", "NotificationType", "Counter64", "ObjectIdentity", "Unsigned32", "IpAddress", "ModuleIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsRequests=stHttpsRequests, mwgHttps=mwgHttps, notifySeverity=notifySeverity, mwgInfo=mwgInfo, notifyAdditional=notifyAdditional, stFtpBytesFromClient=stFtpBytesFromClient, stBlockedByMediaFilter=stBlockedByMediaFilter, kMicroVersion=kMicroVersion, mwgFTP=mwgFTP, stHttpBytesFromServer=stHttpBytesFromServer, trSystem=trSystem, mwgMiscellaneous=mwgMiscellaneous, stFtpBytesFromServer=stFtpBytesFromServer, stHttpTraffic=stHttpTraffic, pMFEDATVersion=pMFEDATVersion, stCPULoad=stCPULoad, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyOriginName=notifyOriginName, stHttpBytesToClient=stHttpBytesToClient, notifyOrigin=notifyOrigin, pTSDBVersion=pTSDBVersion, mwg=mwg, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stCategoriesEntry=stCategoriesEntry, kMinorVersion=kMinorVersion, stHttpRequests=stHttpRequests, pAMSignatureVersion=pAMSignatureVersion, stFtpTraffic=stFtpTraffic, stConnectedSockets=stConnectedSockets, pAMProactiveVersion=pAMProactiveVersion, stTimeForTransaction=stTimeForTransaction, stBadReputation=stBadReputation, stHttpBytesFromClient=stHttpBytesFromClient, stCategoriesTable=stCategoriesTable, stFtpBytesToServer=stFtpBytesToServer, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCPUIOWait=stCPUIOWait, stCategoryName=stCategoryName, stResolveHostViaDNS=stResolveHostViaDNS, kHotfixVersion=kHotfixVersion, mwgTrapVariables=mwgTrapVariables, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kMajorVersion=kMajorVersion, kCompanyName=kCompanyName, stClientCount=stClientCount, kRevision=kRevision, stConnectionsBlocked=stConnectionsBlocked, kBuildNumber=kBuildNumber, kProductVersion=kProductVersion, notifyAffectedHost=notifyAffectedHost, notifyReason=notifyReason, stHttpsBytesToServer=stHttpsBytesToServer, stCategoryCount=stCategoryCount, trApplication=trApplication, pMFEEngineVersion=pMFEEngineVersion, mwgHttp=mwgHttp, stHttpsBytesToClient=stHttpsBytesToClient, stMalwareDetected=stMalwareDetected, pAMEngineVersion=pAMEngineVersion, kCustomVersion=kCustomVersion, stFtpBytesToClient=stFtpBytesToClient, stCategories=stCategories, mwgEngineVersions=mwgEngineVersions, stBlockedByURLFilter=stBlockedByURLFilter, PYSNMP_MODULE_ID=mwg, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpsTraffic=stHttpsTraffic, notifyReasonString=notifyReasonString, stHttpBytesToServer=stHttpBytesToServer, stCPULoadRaw=stCPULoadRaw, stMemConsumed=stMemConsumed, stHttpsBytesFromClient=stHttpsBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, mwgTraps=mwgTraps, mwgContent=mwgContent, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, trUser=trUser, kProductName=kProductName, stMimeType=stMimeType, stHandleConnectToServer=stHandleConnectToServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgStatistics=mwgStatistics, stHttpsBytesFromServer=stHttpsBytesFromServer)
