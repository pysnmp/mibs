...
Counter32, mib_2, iso, ObjectIdentity, MibIdentifier, Integer32, ModuleIdentity, Unsigned32, NotificationType, enterprises, TimeTicks, IpAddress, Gauge32, Bits, Counter64, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "mib-2", "iso", "ObjectIdentity", "MibIdentifier", "Integer32", "ModuleIdentity", "Unsigned32", "NotificationType", "enterprises", "TimeTicks", "IpAddress", "Gauge32", "Bits", "Counter64", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", notifyOriginName=notifyOriginName, kRevision=kRevision, notifyOrigin=notifyOrigin, mwgEngineVersions=mwgEngineVersions, stConnectionsLegitimate=stConnectionsLegitimate, mwgHttp=mwgHttp, notifyReason=notifyReason, kCompanyName=kCompanyName, kMinorVersion=kMinorVersion, mwgTraps=mwgTraps, stHttpsTraffic=stHttpsTraffic, stBlockedByURLFilter=stBlockedByURLFilter, stHttpsBytesFromServer=stHttpsBytesFromServer, stBadReputation=stBadReputation, PYSNMP_MODULE_ID=mwg, stCPULoad=stCPULoad, mwgMiscellaneous=mwgMiscellaneous, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stMalwareDetected=stMalwareDetected, kMicroVersion=kMicroVersion, stFtpBytesToServer=stFtpBytesToServer, notifyAffectedHost=notifyAffectedHost, trApplication=trApplication, stHttpsBytesToServer=stHttpsBytesToServer, stFtpTraffic=stFtpTraffic, stMemConsumed=stMemConsumed, stLastSentLastReceivedClient=stLastSentLastReceivedClient, pAMEngineVersion=pAMEngineVersion, stCategoriesEntry=stCategoriesEntry, mwgTrapVariables=mwgTrapVariables, mwgContent=mwgContent, pTSDBVersion=pTSDBVersion, trUser=trUser, stCategoriesTable=stCategoriesTable, pAMSignatureVersion=pAMSignatureVersion, stCPUIOWait=stCPUIOWait, stMimeType=stMimeType, notifyReasonString=notifyReasonString, mwgFTP=mwgFTP, stClientCount=stClientCount, notifyAdditional=notifyAdditional, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsBytesFromClient=stHttpsBytesFromClient, stTimeForTransaction=stTimeForTransaction, stConnectionsBlocked=stConnectionsBlocked, stConnectedSockets=stConnectedSockets, stHttpTraffic=stHttpTraffic, pMFEEngineVersion=pMFEEngineVersion, stHttpBytesToClient=stHttpBytesToClient, stHttpBytesToServer=stHttpBytesToServer, stFtpBytesFromClient=stFtpBytesFromClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgStatistics=mwgStatistics, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, trSystem=trSystem, kProductName=kProductName, stHttpBytesFromClient=stHttpBytesFromClient, pMFEDATVersion=pMFEDATVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategories=stCategories, stCategoryCount=stCategoryCount, mwgInfo=mwgInfo, kMajorVersion=kMajorVersion, stHttpsRequests=stHttpsRequests, kProductVersion=kProductVersion, stHandleConnectToServer=stHandleConnectToServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stResolveHostViaDNS=stResolveHostViaDNS, kCustomVersion=kCustomVersion, stFtpBytesToClient=stFtpBytesToClient, mwg=mwg, mwgHttps=mwgHttps, stHttpsBytesToClient=stHttpsBytesToClient, kBuildNumber=kBuildNumber, stFtpBytesFromServer=stFtpBytesFromServer, notifySeverity=notifySeverity, stHttpBytesFromServer=stHttpBytesFromServer, kHotfixVersion=kHotfixVersion, stHttpRequests=stHttpRequests, stCategoryName=stCategoryName, pAMProactiveVersion=pAMProactiveVersion, stCPULoadRaw=stCPULoadRaw)
