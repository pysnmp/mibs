...
Counter32, NotificationType, iso, Bits, ModuleIdentity, Unsigned32, mib_2, TimeTicks, IpAddress, NotificationType, Counter64, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, enterprises, MibIdentifier, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "NotificationType", "iso", "Bits", "ModuleIdentity", "Unsigned32", "mib-2", "TimeTicks", "IpAddress", "NotificationType", "Counter64", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "enterprises", "MibIdentifier", "ObjectIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpBytesToServer=stHttpBytesToServer, trApplication=trApplication, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwg=mwg, stFtpBytesToServer=stFtpBytesToServer, stCategoryName=stCategoryName, notifyAdditional=notifyAdditional, stFtpBytesFromServer=stFtpBytesFromServer, mwgHttps=mwgHttps, stBlockedByURLFilter=stBlockedByURLFilter, stMemConsumed=stMemConsumed, kBuildNumber=kBuildNumber, notifyOrigin=notifyOrigin, mwgHttp=mwgHttp, stHttpBytesFromClient=stHttpBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsBytesFromServer=stHttpsBytesFromServer, stCPULoad=stCPULoad, stCPUIOWait=stCPUIOWait, mwgMiscellaneous=mwgMiscellaneous, pAMEngineVersion=pAMEngineVersion, kMicroVersion=kMicroVersion, stResolveHostViaDNS=stResolveHostViaDNS, kHotfixVersion=kHotfixVersion, stMalwareDetected=stMalwareDetected, kMajorVersion=kMajorVersion, stCategories=stCategories, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgTraps=mwgTraps, stCPULoadRaw=stCPULoadRaw, stClientCount=stClientCount, stConnectedSockets=stConnectedSockets, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kProductVersion=kProductVersion, pMFEDATVersion=pMFEDATVersion, stHttpsRequests=stHttpsRequests, mwgContent=mwgContent, stFtpTraffic=stFtpTraffic, pTSDBVersion=pTSDBVersion, stHttpsTraffic=stHttpsTraffic, notifyReason=notifyReason, stCategoriesTable=stCategoriesTable, mwgEngineVersions=mwgEngineVersions, mwgFTP=mwgFTP, stTimeForTransaction=stTimeForTransaction, notifyReasonString=notifyReasonString, stHttpsBytesToClient=stHttpsBytesToClient, stHttpsBytesFromClient=stHttpsBytesFromClient, kMinorVersion=kMinorVersion, stHttpRequests=stHttpRequests, notifyOriginName=notifyOriginName, mwgTrapVariables=mwgTrapVariables, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesToServer=stHttpsBytesToServer, stFtpBytesToClient=stFtpBytesToClient, kProductName=kProductName, notifySeverity=notifySeverity, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kCompanyName=kCompanyName, pAMProactiveVersion=pAMProactiveVersion, stCategoriesEntry=stCategoriesEntry, stHttpBytesFromServer=stHttpBytesFromServer, mwgInfo=mwgInfo, stMimeType=stMimeType, pMFEEngineVersion=pMFEEngineVersion, mwgStatistics=mwgStatistics, stHandleConnectToServer=stHandleConnectToServer, stHttpTraffic=stHttpTraffic, PYSNMP_MODULE_ID=mwg, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, notifyAffectedHost=notifyAffectedHost, stBadReputation=stBadReputation, stFtpBytesFromClient=stFtpBytesFromClient, kRevision=kRevision, stHttpBytesToClient=stHttpBytesToClient, stBlockedByAntiMalware=stBlockedByAntiMalware, trSystem=trSystem, stConnectionsBlocked=stConnectionsBlocked, trUser=trUser, kCustomVersion=kCustomVersion, stCategoryCount=stCategoryCount)
