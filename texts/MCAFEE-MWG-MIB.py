...
Counter32, TimeTicks, NotificationType, mib_2, Counter64, Unsigned32, IpAddress, Bits, Integer32, ModuleIdentity, enterprises, iso, ObjectIdentity, MibIdentifier, Gauge32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "TimeTicks", "NotificationType", "mib-2", "Counter64", "Unsigned32", "IpAddress", "Bits", "Integer32", "ModuleIdentity", "enterprises", "iso", "ObjectIdentity", "MibIdentifier", "Gauge32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", notifyOriginName=notifyOriginName, stMimeType=stMimeType, stCategories=stCategories, PYSNMP_MODULE_ID=mwg, stMalwareDetected=stMalwareDetected, stConnectedSockets=stConnectedSockets, mwgMiscellaneous=mwgMiscellaneous, stHttpsBytesFromClient=stHttpsBytesFromClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsTraffic=stHttpsTraffic, pMFEDATVersion=pMFEDATVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgContent=mwgContent, kCompanyName=kCompanyName, kHotfixVersion=kHotfixVersion, stCategoriesTable=stCategoriesTable, stHttpBytesToClient=stHttpBytesToClient, trUser=trUser, stCategoryName=stCategoryName, stCPULoad=stCPULoad, kMajorVersion=kMajorVersion, stCPUIOWait=stCPUIOWait, stHttpBytesToServer=stHttpBytesToServer, stHttpRequests=stHttpRequests, stFtpTraffic=stFtpTraffic, kRevision=kRevision, stTimeForTransaction=stTimeForTransaction, mwgHttp=mwgHttp, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesToClient=stHttpsBytesToClient, notifyOrigin=notifyOrigin, mwgInfo=mwgInfo, kBuildNumber=kBuildNumber, pAMProactiveVersion=pAMProactiveVersion, mwgTraps=mwgTraps, stFtpBytesFromServer=stFtpBytesFromServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient, pAMSignatureVersion=pAMSignatureVersion, stFtpBytesToClient=stFtpBytesToClient, mwgFTP=mwgFTP, stConnectionsLegitimate=stConnectionsLegitimate, mwg=mwg, notifyReasonString=notifyReasonString, kMinorVersion=kMinorVersion, stFtpBytesToServer=stFtpBytesToServer, pMFEEngineVersion=pMFEEngineVersion, stHttpBytesFromClient=stHttpBytesFromClient, stConnectionsBlocked=stConnectionsBlocked, stHttpsRequests=stHttpsRequests, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpTraffic=stHttpTraffic, stHttpBytesFromServer=stHttpBytesFromServer, stFtpBytesFromClient=stFtpBytesFromClient, pTSDBVersion=pTSDBVersion, kProductName=kProductName, kCustomVersion=kCustomVersion, notifySeverity=notifySeverity, stCategoriesEntry=stCategoriesEntry, kMicroVersion=kMicroVersion, notifyReason=notifyReason, stCategoryCount=stCategoryCount, stClientCount=stClientCount, stHttpsBytesToServer=stHttpsBytesToServer, mwgStatistics=mwgStatistics, trSystem=trSystem, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stBlockedByURLFilter=stBlockedByURLFilter, mwgTrapVariables=mwgTrapVariables, pAMEngineVersion=pAMEngineVersion, kProductVersion=kProductVersion, stCPULoadRaw=stCPULoadRaw, stHandleConnectToServer=stHandleConnectToServer, notifyAdditional=notifyAdditional, mwgEngineVersions=mwgEngineVersions, stMemConsumed=stMemConsumed, stLastSentLastReceivedServer=stLastSentLastReceivedServer, trApplication=trApplication, stBadReputation=stBadReputation, stResolveHostViaDNS=stResolveHostViaDNS, mwgHttps=mwgHttps, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyAffectedHost=notifyAffectedHost)
