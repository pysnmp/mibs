...
TimeTicks, Integer32, IpAddress, enterprises, Gauge32, Counter32, Unsigned32, MibIdentifier, ModuleIdentity, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, mib_2, NotificationType, iso, Bits, ObjectIdentity, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Integer32", "IpAddress", "enterprises", "Gauge32", "Counter32", "Unsigned32", "MibIdentifier", "ModuleIdentity", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "mib-2", "NotificationType", "iso", "Bits", "ObjectIdentity", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgMiscellaneous=mwgMiscellaneous, stCategoriesEntry=stCategoriesEntry, stClientCount=stClientCount, pAMProactiveVersion=pAMProactiveVersion, pAMSignatureVersion=pAMSignatureVersion, stFtpTraffic=stFtpTraffic, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategoryCount=stCategoryCount, kCustomVersion=kCustomVersion, stCPULoad=stCPULoad, stHttpsBytesFromClient=stHttpsBytesFromClient, trApplication=trApplication, mwgHttps=mwgHttps, stHttpsTraffic=stHttpsTraffic, stLastSentLastReceivedServer=stLastSentLastReceivedServer, kProductName=kProductName, stHttpsBytesFromServer=stHttpsBytesFromServer, kMicroVersion=kMicroVersion, stHttpBytesFromClient=stHttpBytesFromClient, kMajorVersion=kMajorVersion, stTimeForTransaction=stTimeForTransaction, stMimeType=stMimeType, mwgStatistics=mwgStatistics, kHotfixVersion=kHotfixVersion, stCPULoadRaw=stCPULoadRaw, stConnectionsLegitimate=stConnectionsLegitimate, stBlockedByAntiMalware=stBlockedByAntiMalware, PYSNMP_MODULE_ID=mwg, stCategories=stCategories, pAMEngineVersion=pAMEngineVersion, kRevision=kRevision, pMFEEngineVersion=pMFEEngineVersion, kProductVersion=kProductVersion, stConnectionsBlocked=stConnectionsBlocked, stHttpRequests=stHttpRequests, stFtpBytesToClient=stFtpBytesToClient, mwgTraps=mwgTraps, stFtpBytesFromServer=stFtpBytesFromServer, stConnectedSockets=stConnectedSockets, mwgTrapVariables=mwgTrapVariables, notifyReasonString=notifyReasonString, stHttpsBytesToServer=stHttpsBytesToServer, notifyOrigin=notifyOrigin, mwgInfo=mwgInfo, stCPUIOWait=stCPUIOWait, kBuildNumber=kBuildNumber, notifyAffectedHost=notifyAffectedHost, trSystem=trSystem, mwgFTP=mwgFTP, stFtpBytesFromClient=stFtpBytesFromClient, stHandleConnectToServer=stHandleConnectToServer, stFtpBytesToServer=stFtpBytesToServer, pTSDBVersion=pTSDBVersion, stHttpsBytesToClient=stHttpsBytesToClient, kCompanyName=kCompanyName, stBlockedByURLFilter=stBlockedByURLFilter, stHttpBytesToServer=stHttpBytesToServer, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgContent=mwgContent, mwgHttp=mwgHttp, stHttpBytesToClient=stHttpBytesToClient, stMalwareDetected=stMalwareDetected, notifyAdditional=notifyAdditional, notifyReason=notifyReason, mwgEngineVersions=mwgEngineVersions, stHttpsRequests=stHttpsRequests, kMinorVersion=kMinorVersion, stHttpTraffic=stHttpTraffic, pMFEDATVersion=pMFEDATVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stResolveHostViaDNS=stResolveHostViaDNS, trUser=trUser, notifyOriginName=notifyOriginName, mwg=mwg, stBadReputation=stBadReputation, notifySeverity=notifySeverity, stMemConsumed=stMemConsumed, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategoryName=stCategoryName, stCategoriesTable=stCategoriesTable, stHttpBytesFromServer=stHttpBytesFromServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient)
