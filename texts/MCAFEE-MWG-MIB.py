...
NotificationType, ObjectIdentity, NotificationType, Gauge32, mib_2, TimeTicks, IpAddress, MibIdentifier, Counter64, iso, enterprises, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, ModuleIdentity, Counter32, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "ObjectIdentity", "NotificationType", "Gauge32", "mib-2", "TimeTicks", "IpAddress", "MibIdentifier", "Counter64", "iso", "enterprises", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "ModuleIdentity", "Counter32", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgContent=mwgContent, mwgMiscellaneous=mwgMiscellaneous, stHttpsBytesFromClient=stHttpsBytesFromClient, mwg=mwg, notifyOriginName=notifyOriginName, stCategoriesTable=stCategoriesTable, notifyAffectedHost=notifyAffectedHost, mwgInfo=mwgInfo, kHotfixVersion=kHotfixVersion, notifyReason=notifyReason, stConnectedSockets=stConnectedSockets, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stBlockedByURLFilter=stBlockedByURLFilter, kMinorVersion=kMinorVersion, stCategoriesEntry=stCategoriesEntry, stMimeType=stMimeType, notifyReasonString=notifyReasonString, kMajorVersion=kMajorVersion, mwgEngineVersions=mwgEngineVersions, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoryName=stCategoryName, stFtpTraffic=stFtpTraffic, stHttpsTraffic=stHttpsTraffic, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, notifyOrigin=notifyOrigin, stFtpBytesFromServer=stFtpBytesFromServer, PYSNMP_MODULE_ID=mwg, trUser=trUser, stCategoryCount=stCategoryCount, stHttpsBytesFromServer=stHttpsBytesFromServer, stConnectionsLegitimate=stConnectionsLegitimate, stFtpBytesFromClient=stFtpBytesFromClient, stBlockedByMediaFilter=stBlockedByMediaFilter, notifySeverity=notifySeverity, mwgHttps=mwgHttps, stCPULoad=stCPULoad, stHandleConnectToServer=stHandleConnectToServer, stCategories=stCategories, stResolveHostViaDNS=stResolveHostViaDNS, stConnectionsBlocked=stConnectionsBlocked, stBadReputation=stBadReputation, kBuildNumber=kBuildNumber, stHttpRequests=stHttpRequests, stHttpsRequests=stHttpsRequests, pAMEngineVersion=pAMEngineVersion, kProductVersion=kProductVersion, stHttpBytesFromServer=stHttpBytesFromServer, kCustomVersion=kCustomVersion, stMemConsumed=stMemConsumed, stTimeForTransaction=stTimeForTransaction, stFtpBytesToClient=stFtpBytesToClient, pTSDBVersion=pTSDBVersion, stHttpsBytesToServer=stHttpsBytesToServer, trApplication=trApplication, stHttpTraffic=stHttpTraffic, stClientCount=stClientCount, pAMSignatureVersion=pAMSignatureVersion, mwgHttp=mwgHttp, kMicroVersion=kMicroVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgTraps=mwgTraps, stHttpsBytesToClient=stHttpsBytesToClient, stFtpBytesToServer=stFtpBytesToServer, kCompanyName=kCompanyName, mwgFTP=mwgFTP, notifyAdditional=notifyAdditional, pAMProactiveVersion=pAMProactiveVersion, stHttpBytesToServer=stHttpBytesToServer, stMalwareDetected=stMalwareDetected, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kRevision=kRevision, kProductName=kProductName, trSystem=trSystem, mwgStatistics=mwgStatistics, stHttpBytesFromClient=stHttpBytesFromClient, stHttpBytesToClient=stHttpBytesToClient, stCPULoadRaw=stCPULoadRaw, stCPUIOWait=stCPUIOWait, pMFEDATVersion=pMFEDATVersion, pMFEEngineVersion=pMFEEngineVersion, mwgTrapVariables=mwgTrapVariables, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer)
