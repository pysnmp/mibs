...
iso, NotificationType, Bits, mib_2, Unsigned32, IpAddress, TimeTicks, Integer32, NotificationType, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Counter32, ObjectIdentity, ModuleIdentity, enterprises, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "NotificationType", "Bits", "mib-2", "Unsigned32", "IpAddress", "TimeTicks", "Integer32", "NotificationType", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Counter32", "ObjectIdentity", "ModuleIdentity", "enterprises", "Counter64")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpTraffic=stFtpTraffic, stBlockedByURLFilter=stBlockedByURLFilter, pAMEngineVersion=pAMEngineVersion, trApplication=trApplication, stCategories=stCategories, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpRequests=stHttpRequests, kRevision=kRevision, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stCategoryName=stCategoryName, stHttpsTraffic=stHttpsTraffic, stFtpBytesFromServer=stFtpBytesFromServer, trSystem=trSystem, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifyAdditional=notifyAdditional, stHttpBytesFromServer=stHttpBytesFromServer, kHotfixVersion=kHotfixVersion, stFtpBytesToServer=stFtpBytesToServer, stHttpsBytesToClient=stHttpsBytesToClient, stConnectionsLegitimate=stConnectionsLegitimate, pMFEEngineVersion=pMFEEngineVersion, kProductVersion=kProductVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpBytesToServer=stHttpBytesToServer, stConnectionsBlocked=stConnectionsBlocked, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stClientCount=stClientCount, mwgHttp=mwgHttp, stBadReputation=stBadReputation, stHttpBytesToClient=stHttpBytesToClient, mwgStatistics=mwgStatistics, stCPULoad=stCPULoad, pAMSignatureVersion=pAMSignatureVersion, mwgInfo=mwgInfo, stHttpsBytesFromServer=stHttpsBytesFromServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoriesTable=stCategoriesTable, stConnectedSockets=stConnectedSockets, kBuildNumber=kBuildNumber, notifyReason=notifyReason, stHandleConnectToServer=stHandleConnectToServer, kProductName=kProductName, pMFEDATVersion=pMFEDATVersion, stFtpBytesFromClient=stFtpBytesFromClient, stFtpBytesToClient=stFtpBytesToClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stMimeType=stMimeType, notifyAffectedHost=notifyAffectedHost, pAMProactiveVersion=pAMProactiveVersion, stMemConsumed=stMemConsumed, kMicroVersion=kMicroVersion, stCPULoadRaw=stCPULoadRaw, stHttpBytesFromClient=stHttpBytesFromClient, stTimeForTransaction=stTimeForTransaction, stCPUIOWait=stCPUIOWait, trUser=trUser, PYSNMP_MODULE_ID=mwg, notifyReasonString=notifyReasonString, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kMajorVersion=kMajorVersion, kCustomVersion=kCustomVersion, mwgEngineVersions=mwgEngineVersions, mwgHttps=mwgHttps, stResolveHostViaDNS=stResolveHostViaDNS, stCategoryCount=stCategoryCount, mwgContent=mwgContent, mwg=mwg, stMalwareDetected=stMalwareDetected, mwgTrapVariables=mwgTrapVariables, stHttpTraffic=stHttpTraffic, kCompanyName=kCompanyName, kMinorVersion=kMinorVersion, stHttpsBytesToServer=stHttpsBytesToServer, mwgTraps=mwgTraps, stCategoriesEntry=stCategoriesEntry, mwgFTP=mwgFTP, notifyOriginName=notifyOriginName, mwgMiscellaneous=mwgMiscellaneous, pTSDBVersion=pTSDBVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpsRequests=stHttpsRequests, notifySeverity=notifySeverity, notifyOrigin=notifyOrigin)
