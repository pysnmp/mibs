...
Gauge32, mib_2, Integer32, MibIdentifier, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, NotificationType, Unsigned32, Counter64, IpAddress, enterprises, ObjectIdentity, ModuleIdentity, NotificationType, Counter32, iso = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "mib-2", "Integer32", "MibIdentifier", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "NotificationType", "Unsigned32", "Counter64", "IpAddress", "enterprises", "ObjectIdentity", "ModuleIdentity", "NotificationType", "Counter32", "iso")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", PYSNMP_MODULE_ID=mwg, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyAdditional=notifyAdditional, mwgHttp=mwgHttp, stFtpBytesFromClient=stFtpBytesFromClient, stHttpsRequests=stHttpsRequests, stHttpRequests=stHttpRequests, mwgContent=mwgContent, stResolveHostViaDNS=stResolveHostViaDNS, mwgTrapVariables=mwgTrapVariables, stBadReputation=stBadReputation, stHttpBytesToClient=stHttpBytesToClient, stFtpBytesFromServer=stFtpBytesFromServer, stHttpsBytesFromServer=stHttpsBytesFromServer, stCategories=stCategories, kHotfixVersion=kHotfixVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pAMSignatureVersion=pAMSignatureVersion, stMemConsumed=stMemConsumed, mwgFTP=mwgFTP, mwgMiscellaneous=mwgMiscellaneous, mwg=mwg, stMimeType=stMimeType, stHttpsBytesToServer=stHttpsBytesToServer, kProductName=kProductName, stHttpBytesFromClient=stHttpBytesFromClient, stCPUIOWait=stCPUIOWait, notifyReasonString=notifyReasonString, mwgStatistics=mwgStatistics, mwgInfo=mwgInfo, stConnectionsLegitimate=stConnectionsLegitimate, pTSDBVersion=pTSDBVersion, stFtpBytesToClient=stFtpBytesToClient, stHttpTraffic=stHttpTraffic, pMFEEngineVersion=pMFEEngineVersion, notifySeverity=notifySeverity, kCompanyName=kCompanyName, notifyReason=notifyReason, stTimeForTransaction=stTimeForTransaction, stHttpsTraffic=stHttpsTraffic, stHttpsBytesToClient=stHttpsBytesToClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgTraps=mwgTraps, notifyAffectedHost=notifyAffectedHost, kCustomVersion=kCustomVersion, stConnectedSockets=stConnectedSockets, kMajorVersion=kMajorVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, trSystem=trSystem, kBuildNumber=kBuildNumber, trApplication=trApplication, stFtpBytesToServer=stFtpBytesToServer, stMalwareDetected=stMalwareDetected, kRevision=kRevision, stHttpsBytesFromClient=stHttpsBytesFromClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kMinorVersion=kMinorVersion, notifyOrigin=notifyOrigin, stCategoryName=stCategoryName, stCategoryCount=stCategoryCount, pMFEDATVersion=pMFEDATVersion, stHttpBytesFromServer=stHttpBytesFromServer, pAMEngineVersion=pAMEngineVersion, notifyOriginName=notifyOriginName, stBlockedByURLFilter=stBlockedByURLFilter, stBlockedByMediaFilter=stBlockedByMediaFilter, kMicroVersion=kMicroVersion, stCategoriesTable=stCategoriesTable, mwgHttps=mwgHttps, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kProductVersion=kProductVersion, stConnectionsBlocked=stConnectionsBlocked, trUser=trUser, stFtpTraffic=stFtpTraffic, stClientCount=stClientCount, stCategoriesEntry=stCategoriesEntry, stHttpBytesToServer=stHttpBytesToServer, stCPULoadRaw=stCPULoadRaw, mwgEngineVersions=mwgEngineVersions, pAMProactiveVersion=pAMProactiveVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHandleConnectToServer=stHandleConnectToServer, stCPULoad=stCPULoad)
