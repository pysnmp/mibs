...
enterprises, MibIdentifier, Gauge32, Bits, NotificationType, NotificationType, Counter32, Counter64, Unsigned32, TimeTicks, mib_2, ModuleIdentity, iso, ObjectIdentity, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "MibIdentifier", "Gauge32", "Bits", "NotificationType", "NotificationType", "Counter32", "Counter64", "Unsigned32", "TimeTicks", "mib-2", "ModuleIdentity", "iso", "ObjectIdentity", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsTraffic=stHttpsTraffic, notifyReason=notifyReason, stCPULoad=stCPULoad, pTSDBVersion=pTSDBVersion, stConnectedSockets=stConnectedSockets, pAMEngineVersion=pAMEngineVersion, stBlockedByURLFilter=stBlockedByURLFilter, mwgHttps=mwgHttps, stMimeType=stMimeType, stCategoriesTable=stCategoriesTable, stHttpTraffic=stHttpTraffic, kCompanyName=kCompanyName, mwgContent=mwgContent, kMinorVersion=kMinorVersion, stCPUIOWait=stCPUIOWait, notifySeverity=notifySeverity, notifyOriginName=notifyOriginName, notifyReasonString=notifyReasonString, kRevision=kRevision, pAMProactiveVersion=pAMProactiveVersion, pMFEEngineVersion=pMFEEngineVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpsBytesToServer=stHttpsBytesToServer, notifyAffectedHost=notifyAffectedHost, mwgEngineVersions=mwgEngineVersions, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, trUser=trUser, stHttpBytesToClient=stHttpBytesToClient, stHttpBytesFromClient=stHttpBytesFromClient, stResolveHostViaDNS=stResolveHostViaDNS, stHandleConnectToServer=stHandleConnectToServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectionsBlocked=stConnectionsBlocked, mwgInfo=mwgInfo, stTimeForTransaction=stTimeForTransaction, stHttpBytesToServer=stHttpBytesToServer, stFtpBytesToServer=stFtpBytesToServer, kMicroVersion=kMicroVersion, kHotfixVersion=kHotfixVersion, stHttpsBytesToClient=stHttpsBytesToClient, stCPULoadRaw=stCPULoadRaw, stBlockedByAntiMalware=stBlockedByAntiMalware, stMalwareDetected=stMalwareDetected, pMFEDATVersion=pMFEDATVersion, trApplication=trApplication, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stClientCount=stClientCount, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFtpBytesFromServer=stFtpBytesFromServer, stCategories=stCategories, stBlockedByMediaFilter=stBlockedByMediaFilter, kProductName=kProductName, stFtpBytesToClient=stFtpBytesToClient, stMemConsumed=stMemConsumed, notifyOrigin=notifyOrigin, PYSNMP_MODULE_ID=mwg, stHttpBytesFromServer=stHttpBytesFromServer, kBuildNumber=kBuildNumber, mwgFTP=mwgFTP, stCategoriesEntry=stCategoriesEntry, kMajorVersion=kMajorVersion, stHttpsRequests=stHttpsRequests, notifyAdditional=notifyAdditional, stBadReputation=stBadReputation, mwg=mwg, pAMSignatureVersion=pAMSignatureVersion, stFtpTraffic=stFtpTraffic, mwgMiscellaneous=mwgMiscellaneous, stCategoryName=stCategoryName, stConnectionsLegitimate=stConnectionsLegitimate, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsBytesFromClient=stHttpsBytesFromClient, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kProductVersion=kProductVersion, stHttpRequests=stHttpRequests, mwgStatistics=mwgStatistics, trSystem=trSystem, mwgTrapVariables=mwgTrapVariables, stFtpBytesFromClient=stFtpBytesFromClient, mwgHttp=mwgHttp, stCategoryCount=stCategoryCount, mwgTraps=mwgTraps, kCustomVersion=kCustomVersion)
