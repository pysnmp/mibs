...
MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, iso, Bits, Unsigned32, NotificationType, TimeTicks, ObjectIdentity, Counter32, mib_2, Integer32, Counter64, ModuleIdentity, Gauge32, MibIdentifier, enterprises, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "iso", "Bits", "Unsigned32", "NotificationType", "TimeTicks", "ObjectIdentity", "Counter32", "mib-2", "Integer32", "Counter64", "ModuleIdentity", "Gauge32", "MibIdentifier", "enterprises", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpBytesToClient=stHttpBytesToClient, mwgFTP=mwgFTP, mwg=mwg, stClientCount=stClientCount, stFtpBytesFromServer=stFtpBytesFromServer, pMFEDATVersion=pMFEDATVersion, stConnectionsBlocked=stConnectionsBlocked, kCompanyName=kCompanyName, pTSDBVersion=pTSDBVersion, stHttpsBytesToClient=stHttpsBytesToClient, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyOrigin=notifyOrigin, mwgHttps=mwgHttps, stHttpRequests=stHttpRequests, stMimeType=stMimeType, stHttpsBytesToServer=stHttpsBytesToServer, stTimeForTransaction=stTimeForTransaction, mwgContent=mwgContent, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stCPUIOWait=stCPUIOWait, kHotfixVersion=kHotfixVersion, trSystem=trSystem, stLastSentLastReceivedServer=stLastSentLastReceivedServer, kProductName=kProductName, trUser=trUser, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stBlockedByURLFilter=stBlockedByURLFilter, kRevision=kRevision, stConnectionsLegitimate=stConnectionsLegitimate, pAMSignatureVersion=pAMSignatureVersion, notifySeverity=notifySeverity, notifyAffectedHost=notifyAffectedHost, stHttpTraffic=stHttpTraffic, mwgInfo=mwgInfo, kMinorVersion=kMinorVersion, kBuildNumber=kBuildNumber, stHttpBytesFromClient=stHttpBytesFromClient, stCPULoad=stCPULoad, pAMEngineVersion=pAMEngineVersion, trApplication=trApplication, stHttpsRequests=stHttpsRequests, stMalwareDetected=stMalwareDetected, stMemConsumed=stMemConsumed, notifyOriginName=notifyOriginName, stConnectedSockets=stConnectedSockets, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCPULoadRaw=stCPULoadRaw, kCustomVersion=kCustomVersion, mwgEngineVersions=mwgEngineVersions, stHttpBytesToServer=stHttpBytesToServer, mwgStatistics=mwgStatistics, stResolveHostViaDNS=stResolveHostViaDNS, stCategoriesTable=stCategoriesTable, stCategoriesEntry=stCategoriesEntry, stBadReputation=stBadReputation, stFtpBytesToServer=stFtpBytesToServer, stCategoryName=stCategoryName, stHttpsBytesFromClient=stHttpsBytesFromClient, stFtpTraffic=stFtpTraffic, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgTrapVariables=mwgTrapVariables, PYSNMP_MODULE_ID=mwg, stHttpsTraffic=stHttpsTraffic, stFtpBytesToClient=stFtpBytesToClient, kMajorVersion=kMajorVersion, stCategories=stCategories, stHttpBytesFromServer=stHttpBytesFromServer, mwgMiscellaneous=mwgMiscellaneous, pMFEEngineVersion=pMFEEngineVersion, stHandleConnectToServer=stHandleConnectToServer, pAMProactiveVersion=pAMProactiveVersion, mwgTraps=mwgTraps, kProductVersion=kProductVersion, mwgHttp=mwgHttp, notifyReason=notifyReason, notifyReasonString=notifyReasonString, stFtpBytesFromClient=stFtpBytesFromClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyAdditional=notifyAdditional, stCategoryCount=stCategoryCount, kMicroVersion=kMicroVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient)
