...
mib_2, Counter64, TimeTicks, MibIdentifier, Integer32, iso, IpAddress, enterprises, NotificationType, ModuleIdentity, Bits, NotificationType, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, Unsigned32, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "mib-2", "Counter64", "TimeTicks", "MibIdentifier", "Integer32", "iso", "IpAddress", "enterprises", "NotificationType", "ModuleIdentity", "Bits", "NotificationType", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "Unsigned32", "Counter32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", trApplication=trApplication, stResolveHostViaDNS=stResolveHostViaDNS, trSystem=trSystem, kMicroVersion=kMicroVersion, pMFEDATVersion=pMFEDATVersion, notifyReasonString=notifyReasonString, mwgInfo=mwgInfo, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stConnectionsLegitimate=stConnectionsLegitimate, stHttpRequests=stHttpRequests, stHttpsBytesToClient=stHttpsBytesToClient, notifyReason=notifyReason, notifyAffectedHost=notifyAffectedHost, stCategoryName=stCategoryName, kProductName=kProductName, mwgEngineVersions=mwgEngineVersions, kCompanyName=kCompanyName, pAMProactiveVersion=pAMProactiveVersion, stHttpsBytesToServer=stHttpsBytesToServer, PYSNMP_MODULE_ID=mwg, notifyOrigin=notifyOrigin, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifySeverity=notifySeverity, stCPULoadRaw=stCPULoadRaw, stHttpTraffic=stHttpTraffic, kRevision=kRevision, pAMEngineVersion=pAMEngineVersion, notifyAdditional=notifyAdditional, kCustomVersion=kCustomVersion, mwgHttp=mwgHttp, stMemConsumed=stMemConsumed, kProductVersion=kProductVersion, mwgMiscellaneous=mwgMiscellaneous, stCPULoad=stCPULoad, mwgContent=mwgContent, stCategoryCount=stCategoryCount, stHttpsRequests=stHttpsRequests, trUser=trUser, notifyOriginName=notifyOriginName, stTimeForTransaction=stTimeForTransaction, stMimeType=stMimeType, stHttpsBytesFromClient=stHttpsBytesFromClient, stConnectionsBlocked=stConnectionsBlocked, stCategoriesEntry=stCategoriesEntry, kBuildNumber=kBuildNumber, stClientCount=stClientCount, stConnectedSockets=stConnectedSockets, stCategories=stCategories, mwgTraps=mwgTraps, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stFtpBytesToServer=stFtpBytesToServer, stHttpBytesFromClient=stHttpBytesFromClient, stHttpBytesToClient=stHttpBytesToClient, kMajorVersion=kMajorVersion, stCPUIOWait=stCPUIOWait, mwgFTP=mwgFTP, pAMSignatureVersion=pAMSignatureVersion, stBlockedByURLFilter=stBlockedByURLFilter, stHandleConnectToServer=stHandleConnectToServer, stHttpBytesFromServer=stHttpBytesFromServer, stFtpTraffic=stFtpTraffic, stHttpBytesToServer=stHttpBytesToServer, pMFEEngineVersion=pMFEEngineVersion, stBadReputation=stBadReputation, stFtpBytesFromClient=stFtpBytesFromClient, mwgStatistics=mwgStatistics, mwgHttps=mwgHttps, stFtpBytesToClient=stFtpBytesToClient, mwg=mwg, stHttpsTraffic=stHttpsTraffic, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoriesTable=stCategoriesTable, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgTrapVariables=mwgTrapVariables, pTSDBVersion=pTSDBVersion, stMalwareDetected=stMalwareDetected, kMinorVersion=kMinorVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, stFtpBytesFromServer=stFtpBytesFromServer, kHotfixVersion=kHotfixVersion)
