...
MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, TimeTicks, ModuleIdentity, ObjectIdentity, Gauge32, iso, NotificationType, Unsigned32, Bits, MibIdentifier, IpAddress, NotificationType, Counter64, mib_2, Counter32, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "TimeTicks", "ModuleIdentity", "ObjectIdentity", "Gauge32", "iso", "NotificationType", "Unsigned32", "Bits", "MibIdentifier", "IpAddress", "NotificationType", "Counter64", "mib-2", "Counter32", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHandleConnectToServer=stHandleConnectToServer, mwgTrapVariables=mwgTrapVariables, mwgMiscellaneous=mwgMiscellaneous, stHttpBytesToServer=stHttpBytesToServer, mwgHttp=mwgHttp, mwgStatistics=mwgStatistics, kMajorVersion=kMajorVersion, stCPULoadRaw=stCPULoadRaw, stCategoriesEntry=stCategoriesEntry, stBlockedByURLFilter=stBlockedByURLFilter, stClientCount=stClientCount, stBlockedByMediaFilter=stBlockedByMediaFilter, stTimeForTransaction=stTimeForTransaction, stMimeType=stMimeType, stHttpTraffic=stHttpTraffic, stHttpsBytesToServer=stHttpsBytesToServer, mwgFTP=mwgFTP, pMFEEngineVersion=pMFEEngineVersion, pAMEngineVersion=pAMEngineVersion, stFtpBytesFromClient=stFtpBytesFromClient, PYSNMP_MODULE_ID=mwg, kMicroVersion=kMicroVersion, mwgContent=mwgContent, stCPUIOWait=stCPUIOWait, notifyOriginName=notifyOriginName, stMalwareDetected=stMalwareDetected, mwgTraps=mwgTraps, stHttpBytesFromClient=stHttpBytesFromClient, kProductVersion=kProductVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpsTraffic=stHttpsTraffic, kCustomVersion=kCustomVersion, notifyOrigin=notifyOrigin, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwg=mwg, stFtpBytesToClient=stFtpBytesToClient, stMemConsumed=stMemConsumed, kBuildNumber=kBuildNumber, stHttpsRequests=stHttpsRequests, mwgInfo=mwgInfo, pAMProactiveVersion=pAMProactiveVersion, notifyReason=notifyReason, stHttpsBytesToClient=stHttpsBytesToClient, kCompanyName=kCompanyName, notifyAffectedHost=notifyAffectedHost, mwgHttps=mwgHttps, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyReasonString=notifyReasonString, stCategories=stCategories, pAMSignatureVersion=pAMSignatureVersion, kMinorVersion=kMinorVersion, stFtpTraffic=stFtpTraffic, stHttpBytesToClient=stHttpBytesToClient, trUser=trUser, stCategoryCount=stCategoryCount, kProductName=kProductName, stHttpBytesFromServer=stHttpBytesFromServer, pMFEDATVersion=pMFEDATVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpBytesToServer=stFtpBytesToServer, stCategoryName=stCategoryName, notifySeverity=notifySeverity, trSystem=trSystem, stBadReputation=stBadReputation, stBlockedByAntiMalware=stBlockedByAntiMalware, stConnectionsLegitimate=stConnectionsLegitimate, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stResolveHostViaDNS=stResolveHostViaDNS, kHotfixVersion=kHotfixVersion, stConnectionsBlocked=stConnectionsBlocked, stConnectedSockets=stConnectedSockets, mwgEngineVersions=mwgEngineVersions, stCategoriesTable=stCategoriesTable, stCPULoad=stCPULoad, kRevision=kRevision, notifyAdditional=notifyAdditional, stLastSentLastReceivedServer=stLastSentLastReceivedServer, trApplication=trApplication, stHttpRequests=stHttpRequests, pTSDBVersion=pTSDBVersion, stFtpBytesFromServer=stFtpBytesFromServer)
