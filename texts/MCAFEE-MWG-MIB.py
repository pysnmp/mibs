...
enterprises, Gauge32, iso, mib_2, NotificationType, MibIdentifier, Integer32, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, NotificationType, ObjectIdentity, TimeTicks, Bits, IpAddress, Counter64, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Gauge32", "iso", "mib-2", "NotificationType", "MibIdentifier", "Integer32", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "NotificationType", "ObjectIdentity", "TimeTicks", "Bits", "IpAddress", "Counter64", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpTraffic=stFtpTraffic, stHttpsTraffic=stHttpsTraffic, mwgTrapVariables=mwgTrapVariables, stHandleConnectToServer=stHandleConnectToServer, stConnectionsBlocked=stConnectionsBlocked, stCategories=stCategories, trUser=trUser, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stFtpBytesToClient=stFtpBytesToClient, stMemConsumed=stMemConsumed, notifyReason=notifyReason, mwgTraps=mwgTraps, trSystem=trSystem, PYSNMP_MODULE_ID=mwg, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kMinorVersion=kMinorVersion, mwgContent=mwgContent, mwg=mwg, notifyReasonString=notifyReasonString, kProductVersion=kProductVersion, stBlockedByURLFilter=stBlockedByURLFilter, mwgInfo=mwgInfo, kBuildNumber=kBuildNumber, mwgMiscellaneous=mwgMiscellaneous, stTimeForTransaction=stTimeForTransaction, kRevision=kRevision, pMFEEngineVersion=pMFEEngineVersion, kCompanyName=kCompanyName, stMimeType=stMimeType, stBlockedByAntiMalware=stBlockedByAntiMalware, pMFEDATVersion=pMFEDATVersion, notifyOrigin=notifyOrigin, mwgEngineVersions=mwgEngineVersions, trApplication=trApplication, notifyAdditional=notifyAdditional, stHttpsBytesFromServer=stHttpsBytesFromServer, pAMProactiveVersion=pAMProactiveVersion, pAMEngineVersion=pAMEngineVersion, mwgStatistics=mwgStatistics, stFtpBytesFromServer=stFtpBytesFromServer, stBadReputation=stBadReputation, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpBytesToClient=stHttpBytesToClient, stCPULoad=stCPULoad, kProductName=kProductName, kMicroVersion=kMicroVersion, stHttpsBytesToClient=stHttpsBytesToClient, mwgHttp=mwgHttp, stHttpsRequests=stHttpsRequests, stClientCount=stClientCount, stHttpBytesFromClient=stHttpBytesFromClient, stMalwareDetected=stMalwareDetected, pTSDBVersion=pTSDBVersion, stFtpBytesToServer=stFtpBytesToServer, stCPUIOWait=stCPUIOWait, stConnectedSockets=stConnectedSockets, mwgFTP=mwgFTP, stHttpTraffic=stHttpTraffic, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCategoriesEntry=stCategoriesEntry, stCategoryName=stCategoryName, stHttpBytesToServer=stHttpBytesToServer, notifyAffectedHost=notifyAffectedHost, stHttpBytesFromServer=stHttpBytesFromServer, kMajorVersion=kMajorVersion, stCategoriesTable=stCategoriesTable, stHttpRequests=stHttpRequests, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpsBytesToServer=stHttpsBytesToServer, notifySeverity=notifySeverity, stFtpBytesFromClient=stFtpBytesFromClient, pAMSignatureVersion=pAMSignatureVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, notifyOriginName=notifyOriginName, stCPULoadRaw=stCPULoadRaw, kHotfixVersion=kHotfixVersion, stCategoryCount=stCategoryCount, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kCustomVersion=kCustomVersion, stResolveHostViaDNS=stResolveHostViaDNS, stConnectionsLegitimate=stConnectionsLegitimate, mwgHttps=mwgHttps)
