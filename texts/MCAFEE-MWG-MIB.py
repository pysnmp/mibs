...
Gauge32, TimeTicks, iso, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, MibIdentifier, NotificationType, NotificationType, ObjectIdentity, ModuleIdentity, mib_2, Integer32, Counter32, IpAddress, Counter64, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "TimeTicks", "iso", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "MibIdentifier", "NotificationType", "NotificationType", "ObjectIdentity", "ModuleIdentity", "mib-2", "Integer32", "Counter32", "IpAddress", "Counter64", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stMemConsumed=stMemConsumed, kCustomVersion=kCustomVersion, mwgTrapVariables=mwgTrapVariables, stCategoriesTable=stCategoriesTable, mwgMiscellaneous=mwgMiscellaneous, mwg=mwg, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kMicroVersion=kMicroVersion, stCPULoadRaw=stCPULoadRaw, stBadReputation=stBadReputation, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCPULoad=stCPULoad, stConnectionsLegitimate=stConnectionsLegitimate, notifyAffectedHost=notifyAffectedHost, kCompanyName=kCompanyName, stFtpBytesToClient=stFtpBytesToClient, mwgStatistics=mwgStatistics, mwgTraps=mwgTraps, stCategoryCount=stCategoryCount, stHttpsBytesFromClient=stHttpsBytesFromClient, trSystem=trSystem, stHttpsBytesToServer=stHttpsBytesToServer, stHttpBytesFromServer=stHttpBytesFromServer, stBlockedByURLFilter=stBlockedByURLFilter, kMajorVersion=kMajorVersion, kMinorVersion=kMinorVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsRequests=stHttpsRequests, stResolveHostViaDNS=stResolveHostViaDNS, pMFEDATVersion=pMFEDATVersion, stCPUIOWait=stCPUIOWait, pAMSignatureVersion=pAMSignatureVersion, trApplication=trApplication, stConnectedSockets=stConnectedSockets, kProductVersion=kProductVersion, stHttpsTraffic=stHttpsTraffic, mwgEngineVersions=mwgEngineVersions, kBuildNumber=kBuildNumber, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kProductName=kProductName, mwgContent=mwgContent, stHttpsBytesFromServer=stHttpsBytesFromServer, stBlockedByMediaFilter=stBlockedByMediaFilter, pAMProactiveVersion=pAMProactiveVersion, stHandleConnectToServer=stHandleConnectToServer, pAMEngineVersion=pAMEngineVersion, stTimeForTransaction=stTimeForTransaction, stFtpBytesFromClient=stFtpBytesFromClient, kHotfixVersion=kHotfixVersion, stClientCount=stClientCount, mwgInfo=mwgInfo, notifyOrigin=notifyOrigin, mwgFTP=mwgFTP, pTSDBVersion=pTSDBVersion, stHttpBytesFromClient=stHttpBytesFromClient, notifyReason=notifyReason, kRevision=kRevision, notifyReasonString=notifyReasonString, stMimeType=stMimeType, stFtpBytesFromServer=stFtpBytesFromServer, mwgHttps=mwgHttps, stCategories=stCategories, stCategoryName=stCategoryName, stConnectionsBlocked=stConnectionsBlocked, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpTraffic=stHttpTraffic, notifyAdditional=notifyAdditional, stMalwareDetected=stMalwareDetected, notifySeverity=notifySeverity, stHttpBytesToClient=stHttpBytesToClient, pMFEEngineVersion=pMFEEngineVersion, PYSNMP_MODULE_ID=mwg, stHttpBytesToServer=stHttpBytesToServer, notifyOriginName=notifyOriginName, mwgHttp=mwgHttp, stHttpsBytesToClient=stHttpsBytesToClient, stCategoriesEntry=stCategoriesEntry, stFtpTraffic=stFtpTraffic, trUser=trUser, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpRequests=stHttpRequests, stFtpBytesToServer=stFtpBytesToServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer)
