...
NotificationType, MibIdentifier, Unsigned32, mib_2, IpAddress, Integer32, Bits, Counter32, ObjectIdentity, ModuleIdentity, Gauge32, NotificationType, Counter64, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, iso = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "MibIdentifier", "Unsigned32", "mib-2", "IpAddress", "Integer32", "Bits", "Counter32", "ObjectIdentity", "ModuleIdentity", "Gauge32", "NotificationType", "Counter64", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "iso")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwg=mwg, pMFEEngineVersion=pMFEEngineVersion, stConnectedSockets=stConnectedSockets, notifySeverity=notifySeverity, stResolveHostViaDNS=stResolveHostViaDNS, stBlockedByURLFilter=stBlockedByURLFilter, notifyReasonString=notifyReasonString, kCompanyName=kCompanyName, notifyReason=notifyReason, stConnectionsLegitimate=stConnectionsLegitimate, pAMSignatureVersion=pAMSignatureVersion, mwgStatistics=mwgStatistics, stCPUIOWait=stCPUIOWait, mwgTraps=mwgTraps, kHotfixVersion=kHotfixVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCategories=stCategories, stHandleConnectToServer=stHandleConnectToServer, stHttpsBytesFromServer=stHttpsBytesFromServer, kRevision=kRevision, kMicroVersion=kMicroVersion, stConnectionsBlocked=stConnectionsBlocked, stCategoryCount=stCategoryCount, stHttpBytesToClient=stHttpBytesToClient, trUser=trUser, stHttpsBytesToServer=stHttpsBytesToServer, stMemConsumed=stMemConsumed, stHttpsBytesToClient=stHttpsBytesToClient, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyAdditional=notifyAdditional, kProductName=kProductName, mwgInfo=mwgInfo, notifyOrigin=notifyOrigin, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kMajorVersion=kMajorVersion, mwgHttp=mwgHttp, stHttpRequests=stHttpRequests, stHttpsTraffic=stHttpsTraffic, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stClientCount=stClientCount, trApplication=trApplication, stHttpBytesToServer=stHttpBytesToServer, stTimeForTransaction=stTimeForTransaction, PYSNMP_MODULE_ID=mwg, mwgEngineVersions=mwgEngineVersions, notifyOriginName=notifyOriginName, pTSDBVersion=pTSDBVersion, mwgMiscellaneous=mwgMiscellaneous, stMalwareDetected=stMalwareDetected, stHttpBytesFromClient=stHttpBytesFromClient, mwgFTP=mwgFTP, stCPULoad=stCPULoad, kCustomVersion=kCustomVersion, stMimeType=stMimeType, stCategoryName=stCategoryName, stCategoriesTable=stCategoriesTable, stFtpBytesFromClient=stFtpBytesFromClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgHttps=mwgHttps, stHttpsBytesFromClient=stHttpsBytesFromClient, stBadReputation=stBadReputation, trSystem=trSystem, stCategoriesEntry=stCategoriesEntry, pMFEDATVersion=pMFEDATVersion, pAMProactiveVersion=pAMProactiveVersion, kMinorVersion=kMinorVersion, stCPULoadRaw=stCPULoadRaw, pAMEngineVersion=pAMEngineVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsRequests=stHttpsRequests, kProductVersion=kProductVersion, stFtpTraffic=stFtpTraffic, stFtpBytesToServer=stFtpBytesToServer, stFtpBytesToClient=stFtpBytesToClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgContent=mwgContent, stFtpBytesFromServer=stFtpBytesFromServer, notifyAffectedHost=notifyAffectedHost, stHttpTraffic=stHttpTraffic, kBuildNumber=kBuildNumber, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgTrapVariables=mwgTrapVariables, stHttpBytesFromServer=stHttpBytesFromServer)
