...
Gauge32, Counter64, IpAddress, Integer32, iso, TimeTicks, ObjectIdentity, mib_2, NotificationType, Unsigned32, Counter32, MibIdentifier, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, enterprises, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Counter64", "IpAddress", "Integer32", "iso", "TimeTicks", "ObjectIdentity", "mib-2", "NotificationType", "Unsigned32", "Counter32", "MibIdentifier", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "enterprises", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgEngineVersions=mwgEngineVersions, pMFEEngineVersion=pMFEEngineVersion, stCategoriesTable=stCategoriesTable, stMemConsumed=stMemConsumed, stHttpBytesFromServer=stHttpBytesFromServer, notifyOriginName=notifyOriginName, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, pTSDBVersion=pTSDBVersion, trApplication=trApplication, stResolveHostViaDNS=stResolveHostViaDNS, mwgHttp=mwgHttp, stCategoryName=stCategoryName, notifyOrigin=notifyOrigin, stFtpBytesToServer=stFtpBytesToServer, stFtpBytesFromClient=stFtpBytesFromClient, mwgMiscellaneous=mwgMiscellaneous, stCategories=stCategories, pMFEDATVersion=pMFEDATVersion, stBadReputation=stBadReputation, stHttpRequests=stHttpRequests, pAMProactiveVersion=pAMProactiveVersion, stHttpsBytesToClient=stHttpsBytesToClient, stConnectionsLegitimate=stConnectionsLegitimate, PYSNMP_MODULE_ID=mwg, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stBlockedByURLFilter=stBlockedByURLFilter, stHttpsBytesFromServer=stHttpsBytesFromServer, notifyReasonString=notifyReasonString, stCPULoad=stCPULoad, mwg=mwg, stConnectedSockets=stConnectedSockets, mwgInfo=mwgInfo, mwgContent=mwgContent, kCustomVersion=kCustomVersion, stHttpsTraffic=stHttpsTraffic, stBlockedByMediaFilter=stBlockedByMediaFilter, kBuildNumber=kBuildNumber, mwgHttps=mwgHttps, stFtpBytesToClient=stFtpBytesToClient, stMalwareDetected=stMalwareDetected, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpBytesToClient=stHttpBytesToClient, trSystem=trSystem, stHttpBytesFromClient=stHttpBytesFromClient, notifyReason=notifyReason, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCPULoadRaw=stCPULoadRaw, stTimeForTransaction=stTimeForTransaction, kRevision=kRevision, stHandleConnectToServer=stHandleConnectToServer, kMinorVersion=kMinorVersion, notifyAffectedHost=notifyAffectedHost, stConnectionsBlocked=stConnectionsBlocked, mwgStatistics=mwgStatistics, pAMSignatureVersion=pAMSignatureVersion, stFtpTraffic=stFtpTraffic, mwgTraps=mwgTraps, stHttpBytesToServer=stHttpBytesToServer, mwgTrapVariables=mwgTrapVariables, stClientCount=stClientCount, notifySeverity=notifySeverity, trUser=trUser, kProductVersion=kProductVersion, stHttpsBytesToServer=stHttpsBytesToServer, stMimeType=stMimeType, stCategoryCount=stCategoryCount, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stBlockedByAntiMalware=stBlockedByAntiMalware, kHotfixVersion=kHotfixVersion, stCategoriesEntry=stCategoriesEntry, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kMajorVersion=kMajorVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, kMicroVersion=kMicroVersion, notifyAdditional=notifyAdditional, kCompanyName=kCompanyName, stHttpTraffic=stHttpTraffic, stHttpsRequests=stHttpsRequests, pAMEngineVersion=pAMEngineVersion, stCPUIOWait=stCPUIOWait, kProductName=kProductName, stFtpBytesFromServer=stFtpBytesFromServer, mwgFTP=mwgFTP)
