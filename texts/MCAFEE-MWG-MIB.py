...
Bits, Gauge32, mib_2, ObjectIdentity, iso, enterprises, Integer32, MibIdentifier, Counter32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, IpAddress, NotificationType, NotificationType, TimeTicks, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Gauge32", "mib-2", "ObjectIdentity", "iso", "enterprises", "Integer32", "MibIdentifier", "Counter32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "IpAddress", "NotificationType", "NotificationType", "TimeTicks", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", notifyOrigin=notifyOrigin, mwgEngineVersions=mwgEngineVersions, stHttpTraffic=stHttpTraffic, kHotfixVersion=kHotfixVersion, stHttpsBytesToClient=stHttpsBytesToClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kRevision=kRevision, stBlockedByMediaFilter=stBlockedByMediaFilter, stFtpTraffic=stFtpTraffic, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stCategoryName=stCategoryName, stMemConsumed=stMemConsumed, stCategoriesTable=stCategoriesTable, PYSNMP_MODULE_ID=mwg, notifySeverity=notifySeverity, notifyAffectedHost=notifyAffectedHost, mwgMiscellaneous=mwgMiscellaneous, pAMProactiveVersion=pAMProactiveVersion, stClientCount=stClientCount, stCPULoadRaw=stCPULoadRaw, mwgHttp=mwgHttp, stBadReputation=stBadReputation, stHttpsTraffic=stHttpsTraffic, stFtpBytesToClient=stFtpBytesToClient, kBuildNumber=kBuildNumber, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgFTP=mwgFTP, stCategories=stCategories, pTSDBVersion=pTSDBVersion, stMimeType=stMimeType, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, pAMSignatureVersion=pAMSignatureVersion, mwgTraps=mwgTraps, notifyOriginName=notifyOriginName, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesFromServer=stFtpBytesFromServer, stCategoriesEntry=stCategoriesEntry, mwgStatistics=mwgStatistics, notifyReason=notifyReason, stHttpsRequests=stHttpsRequests, stHttpBytesToClient=stHttpBytesToClient, mwgTrapVariables=mwgTrapVariables, stResolveHostViaDNS=stResolveHostViaDNS, stHttpBytesFromServer=stHttpBytesFromServer, kProductName=kProductName, pAMEngineVersion=pAMEngineVersion, notifyReasonString=notifyReasonString, stHttpBytesToServer=stHttpBytesToServer, stConnectionsLegitimate=stConnectionsLegitimate, stHandleConnectToServer=stHandleConnectToServer, kMinorVersion=kMinorVersion, trSystem=trSystem, kMajorVersion=kMajorVersion, stCPULoad=stCPULoad, stMalwareDetected=stMalwareDetected, mwg=mwg, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgInfo=mwgInfo, kMicroVersion=kMicroVersion, mwgContent=mwgContent, stCategoryCount=stCategoryCount, stHttpsBytesToServer=stHttpsBytesToServer, stHttpBytesFromClient=stHttpBytesFromClient, mwgHttps=mwgHttps, trApplication=trApplication, kCompanyName=kCompanyName, stHttpRequests=stHttpRequests, stHttpsBytesFromClient=stHttpsBytesFromClient, stFtpBytesToServer=stFtpBytesToServer, stBlockedByURLFilter=stBlockedByURLFilter, stConnectedSockets=stConnectedSockets, kProductVersion=kProductVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, kCustomVersion=kCustomVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifyAdditional=notifyAdditional, stLastSentLastReceivedClient=stLastSentLastReceivedClient, pMFEEngineVersion=pMFEEngineVersion, stCPUIOWait=stCPUIOWait, trUser=trUser, stFtpBytesFromClient=stFtpBytesFromClient, pMFEDATVersion=pMFEDATVersion, stTimeForTransaction=stTimeForTransaction)
