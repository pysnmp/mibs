...
ModuleIdentity, mib_2, NotificationType, ObjectIdentity, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Counter64, Unsigned32, NotificationType, Bits, Integer32, iso, enterprises, Counter32, TimeTicks, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "mib-2", "NotificationType", "ObjectIdentity", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Counter64", "Unsigned32", "NotificationType", "Bits", "Integer32", "iso", "enterprises", "Counter32", "TimeTicks", "IpAddress")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stLastSentLastReceivedServer=stLastSentLastReceivedServer, stBlockedByURLFilter=stBlockedByURLFilter, kRevision=kRevision, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgMiscellaneous=mwgMiscellaneous, kHotfixVersion=kHotfixVersion, kMicroVersion=kMicroVersion, trApplication=trApplication, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesToClient=stHttpBytesToClient, trSystem=trSystem, stHttpsBytesToServer=stHttpsBytesToServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stConnectedSockets=stConnectedSockets, stCPUIOWait=stCPUIOWait, trUser=trUser, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgTraps=mwgTraps, kBuildNumber=kBuildNumber, stCategories=stCategories, kProductVersion=kProductVersion, stFtpTraffic=stFtpTraffic, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stFtpBytesFromServer=stFtpBytesFromServer, kCompanyName=kCompanyName, kCustomVersion=kCustomVersion, notifyOriginName=notifyOriginName, mwgStatistics=mwgStatistics, kProductName=kProductName, stHttpBytesFromClient=stHttpBytesFromClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgHttp=mwgHttp, mwg=mwg, pAMSignatureVersion=pAMSignatureVersion, mwgHttps=mwgHttps, stFtpBytesFromClient=stFtpBytesFromClient, stHttpTraffic=stHttpTraffic, notifyAffectedHost=notifyAffectedHost, stCategoryName=stCategoryName, stCategoriesTable=stCategoriesTable, pAMProactiveVersion=pAMProactiveVersion, stConnectionsLegitimate=stConnectionsLegitimate, stCPULoadRaw=stCPULoadRaw, stMimeType=stMimeType, stTimeForTransaction=stTimeForTransaction, pTSDBVersion=pTSDBVersion, stHttpsTraffic=stHttpsTraffic, pMFEEngineVersion=pMFEEngineVersion, mwgFTP=mwgFTP, kMinorVersion=kMinorVersion, stFtpBytesToClient=stFtpBytesToClient, notifyOrigin=notifyOrigin, stHttpsBytesFromClient=stHttpsBytesFromClient, pMFEDATVersion=pMFEDATVersion, mwgEngineVersions=mwgEngineVersions, stBadReputation=stBadReputation, stHttpsRequests=stHttpsRequests, stConnectionsBlocked=stConnectionsBlocked, kMajorVersion=kMajorVersion, stCategoriesEntry=stCategoriesEntry, notifySeverity=notifySeverity, PYSNMP_MODULE_ID=mwg, stFtpBytesToServer=stFtpBytesToServer, notifyReasonString=notifyReasonString, stMalwareDetected=stMalwareDetected, stBlockedByMediaFilter=stBlockedByMediaFilter, stHandleConnectToServer=stHandleConnectToServer, mwgInfo=mwgInfo, stHttpRequests=stHttpRequests, pAMEngineVersion=pAMEngineVersion, stClientCount=stClientCount, stMemConsumed=stMemConsumed, stHttpBytesToServer=stHttpBytesToServer, stCategoryCount=stCategoryCount, stResolveHostViaDNS=stResolveHostViaDNS, stHttpBytesFromServer=stHttpBytesFromServer, stCPULoad=stCPULoad, stLastSentLastReceivedClient=stLastSentLastReceivedClient, notifyAdditional=notifyAdditional, mwgTrapVariables=mwgTrapVariables, notifyReason=notifyReason, mwgContent=mwgContent)
