...
enterprises, mib_2, NotificationType, MibIdentifier, ModuleIdentity, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Counter64, iso, Unsigned32, Gauge32, Counter32, IpAddress, Bits, ObjectIdentity, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "mib-2", "NotificationType", "MibIdentifier", "ModuleIdentity", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Counter64", "iso", "Unsigned32", "Gauge32", "Counter32", "IpAddress", "Bits", "ObjectIdentity", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFtpBytesToClient=stFtpBytesToClient, stCategories=stCategories, stCPULoad=stCPULoad, mwgFTP=mwgFTP, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgHttp=mwgHttp, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stBlockedByAntiMalware=stBlockedByAntiMalware, kRevision=kRevision, stHttpsBytesToServer=stHttpsBytesToServer, stCategoryName=stCategoryName, stCPUIOWait=stCPUIOWait, stHttpTraffic=stHttpTraffic, stCategoryCount=stCategoryCount, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stMemConsumed=stMemConsumed, notifyReason=notifyReason, kMinorVersion=kMinorVersion, stHttpBytesToClient=stHttpBytesToClient, stFtpTraffic=stFtpTraffic, notifyOriginName=notifyOriginName, kCustomVersion=kCustomVersion, notifyOrigin=notifyOrigin, stFtpBytesToServer=stFtpBytesToServer, mwgContent=mwgContent, mwgInfo=mwgInfo, stResolveHostViaDNS=stResolveHostViaDNS, stHttpBytesToServer=stHttpBytesToServer, kHotfixVersion=kHotfixVersion, stConnectionsBlocked=stConnectionsBlocked, kMajorVersion=kMajorVersion, stHandleConnectToServer=stHandleConnectToServer, pMFEDATVersion=pMFEDATVersion, stMimeType=stMimeType, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgTraps=mwgTraps, pMFEEngineVersion=pMFEEngineVersion, stFtpBytesFromClient=stFtpBytesFromClient, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpBytesFromClient=stHttpBytesFromClient, pTSDBVersion=pTSDBVersion, kBuildNumber=kBuildNumber, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpsBytesToClient=stHttpsBytesToClient, mwg=mwg, stCategoriesTable=stCategoriesTable, mwgTrapVariables=mwgTrapVariables, trSystem=trSystem, stBadReputation=stBadReputation, pAMEngineVersion=pAMEngineVersion, notifyAdditional=notifyAdditional, stMalwareDetected=stMalwareDetected, mwgStatistics=mwgStatistics, kCompanyName=kCompanyName, stCPULoadRaw=stCPULoadRaw, PYSNMP_MODULE_ID=mwg, kProductVersion=kProductVersion, kProductName=kProductName, stTimeForTransaction=stTimeForTransaction, stCategoriesEntry=stCategoriesEntry, mwgMiscellaneous=mwgMiscellaneous, notifyReasonString=notifyReasonString, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgEngineVersions=mwgEngineVersions, stConnectionsLegitimate=stConnectionsLegitimate, stBlockedByURLFilter=stBlockedByURLFilter, mwgHttps=mwgHttps, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesFromServer=stHttpBytesFromServer, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifySeverity=notifySeverity, stHttpsRequests=stHttpsRequests, stHttpsTraffic=stHttpsTraffic, trUser=trUser, stConnectedSockets=stConnectedSockets, stClientCount=stClientCount, notifyAffectedHost=notifyAffectedHost, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kMicroVersion=kMicroVersion, stHttpRequests=stHttpRequests, pAMProactiveVersion=pAMProactiveVersion, trApplication=trApplication, stFtpBytesFromServer=stFtpBytesFromServer)
