...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgInfo=mwgInfo, mwgHttps=mwgHttps, kHotfixVersion=kHotfixVersion, mwgHttp=mwgHttp, stCategoryCount=stCategoryCount, pTSDBVersion=pTSDBVersion, kBuildNumber=kBuildNumber, trSystem=trSystem, kProductName=kProductName, stTimeForTransaction=stTimeForTransaction, kMinorVersion=kMinorVersion, stHttpBytesFromServer=stHttpBytesFromServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpBytesToServer=stHttpBytesToServer, kMicroVersion=kMicroVersion, stClientCount=stClientCount, stMemConsumed=stMemConsumed, stMalwareDetected=stMalwareDetected, stHttpsBytesFromServer=stHttpsBytesFromServer, pAMProactiveVersion=pAMProactiveVersion, stHttpBytesFromClient=stHttpBytesFromClient, notifyReason=notifyReason, stCategoriesTable=stCategoriesTable, mwgEngineVersions=mwgEngineVersions, stBlockedByURLFilter=stBlockedByURLFilter, stFtpBytesToServer=stFtpBytesToServer, mwgContent=mwgContent, stCPUIOWait=stCPUIOWait, notifyOrigin=notifyOrigin, pMFEEngineVersion=pMFEEngineVersion, stFtpTraffic=stFtpTraffic, stConnectionsBlocked=stConnectionsBlocked, mwgMiscellaneous=mwgMiscellaneous, mwgStatistics=mwgStatistics, PYSNMP_MODULE_ID=mwg, mwg=mwg, kCompanyName=kCompanyName, stConnectedSockets=stConnectedSockets, mwgTraps=mwgTraps, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifyReasonString=notifyReasonString, trUser=trUser, mwgTrapVariables=mwgTrapVariables, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpsRequests=stHttpsRequests, kCustomVersion=kCustomVersion, pAMEngineVersion=pAMEngineVersion, mwgFTP=mwgFTP, stBlockedByMediaFilter=stBlockedByMediaFilter, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kMajorVersion=kMajorVersion, notifyOriginName=notifyOriginName, notifySeverity=notifySeverity, stHttpsTraffic=stHttpsTraffic, stConnectionsLegitimate=stConnectionsLegitimate, stCategoryName=stCategoryName, stResolveHostViaDNS=stResolveHostViaDNS, stCategoriesEntry=stCategoriesEntry, stHttpRequests=stHttpRequests, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kProductVersion=kProductVersion, stCPULoadRaw=stCPULoadRaw, stBadReputation=stBadReputation, stHttpsBytesToClient=stHttpsBytesToClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsBytesFromClient=stHttpsBytesFromClient, stHandleConnectToServer=stHandleConnectToServer, pMFEDATVersion=pMFEDATVersion, kRevision=kRevision, notifyAdditional=notifyAdditional, stFtpBytesToClient=stFtpBytesToClient, stCategories=stCategories, stFtpBytesFromServer=stFtpBytesFromServer, stHttpsBytesToServer=stHttpsBytesToServer, stHttpTraffic=stHttpTraffic, notifyAffectedHost=notifyAffectedHost, stFtpBytesFromClient=stFtpBytesFromClient, stCPULoad=stCPULoad, stHttpBytesToClient=stHttpBytesToClient, stMimeType=stMimeType, pAMSignatureVersion=pAMSignatureVersion, trApplication=trApplication)
