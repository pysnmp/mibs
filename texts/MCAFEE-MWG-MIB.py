...
Integer32, Gauge32, ObjectIdentity, Counter64, ModuleIdentity, iso, Unsigned32, enterprises, MibIdentifier, NotificationType, TimeTicks, Bits, mib_2, Counter32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Gauge32", "ObjectIdentity", "Counter64", "ModuleIdentity", "iso", "Unsigned32", "enterprises", "MibIdentifier", "NotificationType", "TimeTicks", "Bits", "mib-2", "Counter32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgHttp=mwgHttp, stCategoryName=stCategoryName, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyOrigin=notifyOrigin, pMFEEngineVersion=pMFEEngineVersion, stHttpsBytesToClient=stHttpsBytesToClient, kCompanyName=kCompanyName, stCPULoadRaw=stCPULoadRaw, stMalwareDetected=stMalwareDetected, mwgEngineVersions=mwgEngineVersions, kProductVersion=kProductVersion, mwgStatistics=mwgStatistics, stCategories=stCategories, stClientCount=stClientCount, trUser=trUser, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stConnectionsLegitimate=stConnectionsLegitimate, kCustomVersion=kCustomVersion, stHttpsRequests=stHttpsRequests, stHttpRequests=stHttpRequests, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, pMFEDATVersion=pMFEDATVersion, stFtpBytesToServer=stFtpBytesToServer, stResolveHostViaDNS=stResolveHostViaDNS, stHttpTraffic=stHttpTraffic, stFtpBytesFromClient=stFtpBytesFromClient, stCategoriesTable=stCategoriesTable, mwgContent=mwgContent, kBuildNumber=kBuildNumber, stFtpBytesFromServer=stFtpBytesFromServer, stFtpTraffic=stFtpTraffic, notifyAffectedHost=notifyAffectedHost, kMajorVersion=kMajorVersion, mwgInfo=mwgInfo, kProductName=kProductName, pAMEngineVersion=pAMEngineVersion, stBlockedByURLFilter=stBlockedByURLFilter, mwgTraps=mwgTraps, kMinorVersion=kMinorVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgTrapVariables=mwgTrapVariables, stBadReputation=stBadReputation, stFtpBytesToClient=stFtpBytesToClient, notifyAdditional=notifyAdditional, stHttpBytesToServer=stHttpBytesToServer, stCPUIOWait=stCPUIOWait, kMicroVersion=kMicroVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stHandleConnectToServer=stHandleConnectToServer, pTSDBVersion=pTSDBVersion, pAMProactiveVersion=pAMProactiveVersion, stConnectionsBlocked=stConnectionsBlocked, notifySeverity=notifySeverity, mwgHttps=mwgHttps, stCategoriesEntry=stCategoriesEntry, mwg=mwg, stBlockedByAntiMalware=stBlockedByAntiMalware, stConnectedSockets=stConnectedSockets, stTimeForTransaction=stTimeForTransaction, stHttpsBytesToServer=stHttpsBytesToServer, notifyReason=notifyReason, pAMSignatureVersion=pAMSignatureVersion, trSystem=trSystem, notifyOriginName=notifyOriginName, stHttpBytesFromServer=stHttpBytesFromServer, mwgFTP=mwgFTP, stCPULoad=stCPULoad, stMemConsumed=stMemConsumed, PYSNMP_MODULE_ID=mwg, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgMiscellaneous=mwgMiscellaneous, stHttpsBytesFromServer=stHttpsBytesFromServer, kHotfixVersion=kHotfixVersion, stHttpsTraffic=stHttpsTraffic, stMimeType=stMimeType, trApplication=trApplication, kRevision=kRevision, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpBytesFromClient=stHttpBytesFromClient, notifyReasonString=notifyReasonString, stCategoryCount=stCategoryCount, stHttpBytesToClient=stHttpBytesToClient)
