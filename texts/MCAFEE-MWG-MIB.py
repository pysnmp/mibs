...
MibScalar, MibTable, MibTableRow, MibTableColumn, mib_2, Integer32, NotificationType, TimeTicks, ObjectIdentity, Bits, enterprises, iso, Counter64, ModuleIdentity, Counter32, Unsigned32, IpAddress, MibIdentifier, NotificationType, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "mib-2", "Integer32", "NotificationType", "TimeTicks", "ObjectIdentity", "Bits", "enterprises", "iso", "Counter64", "ModuleIdentity", "Counter32", "Unsigned32", "IpAddress", "MibIdentifier", "NotificationType", "Gauge32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stBlockedByURLFilter=stBlockedByURLFilter, stCPULoad=stCPULoad, stBlockedByMediaFilter=stBlockedByMediaFilter, stMalwareDetected=stMalwareDetected, stHttpsRequests=stHttpsRequests, stConnectedSockets=stConnectedSockets, mwgHttps=mwgHttps, mwgEngineVersions=mwgEngineVersions, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpRequests=stHttpRequests, pAMSignatureVersion=pAMSignatureVersion, stFtpBytesToServer=stFtpBytesToServer, notifyOriginName=notifyOriginName, mwgContent=mwgContent, kProductVersion=kProductVersion, stClientCount=stClientCount, stFtpBytesToClient=stFtpBytesToClient, stCategories=stCategories, stBadReputation=stBadReputation, stHttpsBytesToServer=stHttpsBytesToServer, kCompanyName=kCompanyName, stHttpBytesFromClient=stHttpBytesFromClient, pTSDBVersion=pTSDBVersion, stFtpBytesFromClient=stFtpBytesFromClient, trApplication=trApplication, notifyAdditional=notifyAdditional, mwgTraps=mwgTraps, notifyReason=notifyReason, stHttpsBytesFromServer=stHttpsBytesFromServer, pMFEDATVersion=pMFEDATVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpTraffic=stFtpTraffic, stMimeType=stMimeType, trUser=trUser, stHttpBytesFromServer=stHttpBytesFromServer, stHandleConnectToServer=stHandleConnectToServer, stCategoriesEntry=stCategoriesEntry, stCPUIOWait=stCPUIOWait, stHttpTraffic=stHttpTraffic, mwgStatistics=mwgStatistics, stCategoryCount=stCategoryCount, PYSNMP_MODULE_ID=mwg, notifyReasonString=notifyReasonString, kMicroVersion=kMicroVersion, mwgTrapVariables=mwgTrapVariables, stCategoryName=stCategoryName, stCPULoadRaw=stCPULoadRaw, stHttpsBytesToClient=stHttpsBytesToClient, stMemConsumed=stMemConsumed, kBuildNumber=kBuildNumber, pMFEEngineVersion=pMFEEngineVersion, pAMEngineVersion=pAMEngineVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyAffectedHost=notifyAffectedHost, stCategoriesTable=stCategoriesTable, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsTraffic=stHttpsTraffic, pAMProactiveVersion=pAMProactiveVersion, kProductName=kProductName, kMinorVersion=kMinorVersion, trSystem=trSystem, stResolveHostViaDNS=stResolveHostViaDNS, mwgFTP=mwgFTP, stTimeForTransaction=stTimeForTransaction, mwgInfo=mwgInfo, kRevision=kRevision, kMajorVersion=kMajorVersion, mwgHttp=mwgHttp, kCustomVersion=kCustomVersion, stConnectionsLegitimate=stConnectionsLegitimate, mwg=mwg, notifySeverity=notifySeverity, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgMiscellaneous=mwgMiscellaneous, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpBytesToServer=stHttpBytesToServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stConnectionsBlocked=stConnectionsBlocked, notifyOrigin=notifyOrigin, stHttpBytesToClient=stHttpBytesToClient, kHotfixVersion=kHotfixVersion, stFtpBytesFromServer=stFtpBytesFromServer)
