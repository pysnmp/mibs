...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategoriesEntry=stCategoriesEntry, stCPUIOWait=stCPUIOWait, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesFromServer=stHttpBytesFromServer, stHttpsRequests=stHttpsRequests, mwgTrapVariables=mwgTrapVariables, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kMinorVersion=kMinorVersion, stHttpBytesFromClient=stHttpBytesFromClient, trSystem=trSystem, stCategoriesTable=stCategoriesTable, stMemConsumed=stMemConsumed, notifyReasonString=notifyReasonString, stHttpsTraffic=stHttpsTraffic, stFtpBytesToClient=stFtpBytesToClient, stConnectionsLegitimate=stConnectionsLegitimate, stClientCount=stClientCount, stCategoryName=stCategoryName, stHttpsBytesFromClient=stHttpsBytesFromClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kMicroVersion=kMicroVersion, notifyReason=notifyReason, pMFEDATVersion=pMFEDATVersion, stCategoryCount=stCategoryCount, stCPULoad=stCPULoad, stFtpBytesFromClient=stFtpBytesFromClient, stCategories=stCategories, stMimeType=stMimeType, stFtpBytesFromServer=stFtpBytesFromServer, kCompanyName=kCompanyName, mwgInfo=mwgInfo, kCustomVersion=kCustomVersion, kMajorVersion=kMajorVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kProductVersion=kProductVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpsBytesFromServer=stHttpsBytesFromServer, stResolveHostViaDNS=stResolveHostViaDNS, trUser=trUser, notifySeverity=notifySeverity, notifyAffectedHost=notifyAffectedHost, stTimeForTransaction=stTimeForTransaction, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgEngineVersions=mwgEngineVersions, stBadReputation=stBadReputation, kProductName=kProductName, stHttpsBytesToServer=stHttpsBytesToServer, stBlockedByURLFilter=stBlockedByURLFilter, stConnectionsBlocked=stConnectionsBlocked, notifyOriginName=notifyOriginName, stBlockedByMediaFilter=stBlockedByMediaFilter, pAMEngineVersion=pAMEngineVersion, mwgTraps=mwgTraps, stFtpTraffic=stFtpTraffic, pMFEEngineVersion=pMFEEngineVersion, mwgMiscellaneous=mwgMiscellaneous, kRevision=kRevision, stHandleConnectToServer=stHandleConnectToServer, kBuildNumber=kBuildNumber, stConnectedSockets=stConnectedSockets, mwgContent=mwgContent, pTSDBVersion=pTSDBVersion, stHttpBytesToClient=stHttpBytesToClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, PYSNMP_MODULE_ID=mwg, mwgHttp=mwgHttp, mwgStatistics=mwgStatistics, stFtpBytesToServer=stFtpBytesToServer, stMalwareDetected=stMalwareDetected, pAMProactiveVersion=pAMProactiveVersion, stHttpsBytesToClient=stHttpsBytesToClient, mwgFTP=mwgFTP, notifyOrigin=notifyOrigin, mwgHttps=mwgHttps, trApplication=trApplication, stCPULoadRaw=stCPULoadRaw, stHttpRequests=stHttpRequests, stHttpBytesToServer=stHttpBytesToServer, stHttpTraffic=stHttpTraffic, notifyAdditional=notifyAdditional, mwg=mwg, kHotfixVersion=kHotfixVersion)
