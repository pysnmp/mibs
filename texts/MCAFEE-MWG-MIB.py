...
Counter32, IpAddress, Gauge32, Unsigned32, iso, Counter64, ModuleIdentity, Integer32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, mib_2, MibIdentifier, Bits, enterprises, TimeTicks, NotificationType, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "IpAddress", "Gauge32", "Unsigned32", "iso", "Counter64", "ModuleIdentity", "Integer32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "mib-2", "MibIdentifier", "Bits", "enterprises", "TimeTicks", "NotificationType", "ObjectIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsBytesFromClient=stHttpsBytesFromClient, stMemConsumed=stMemConsumed, stCategoryName=stCategoryName, notifyAdditional=notifyAdditional, kProductName=kProductName, stHttpsRequests=stHttpsRequests, kMajorVersion=kMajorVersion, stCPUIOWait=stCPUIOWait, stCategories=stCategories, stFtpBytesFromClient=stFtpBytesFromClient, stCPULoadRaw=stCPULoadRaw, mwgTrapVariables=mwgTrapVariables, stBadReputation=stBadReputation, mwgContent=mwgContent, stCategoriesEntry=stCategoriesEntry, stTimeForTransaction=stTimeForTransaction, kCustomVersion=kCustomVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, kBuildNumber=kBuildNumber, kMicroVersion=kMicroVersion, stCPULoad=stCPULoad, stFtpBytesFromServer=stFtpBytesFromServer, stConnectionsBlocked=stConnectionsBlocked, pMFEEngineVersion=pMFEEngineVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kCompanyName=kCompanyName, mwgEngineVersions=mwgEngineVersions, mwgMiscellaneous=mwgMiscellaneous, mwgFTP=mwgFTP, stHttpsTraffic=stHttpsTraffic, trApplication=trApplication, stHttpBytesFromServer=stHttpBytesFromServer, mwgHttps=mwgHttps, mwgTraps=mwgTraps, kHotfixVersion=kHotfixVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, pAMEngineVersion=pAMEngineVersion, notifySeverity=notifySeverity, stFtpBytesToServer=stFtpBytesToServer, trSystem=trSystem, stHttpTraffic=stHttpTraffic, stConnectionsLegitimate=stConnectionsLegitimate, stResolveHostViaDNS=stResolveHostViaDNS, stCategoryCount=stCategoryCount, stBlockedByURLFilter=stBlockedByURLFilter, pMFEDATVersion=pMFEDATVersion, pAMProactiveVersion=pAMProactiveVersion, stMimeType=stMimeType, pTSDBVersion=pTSDBVersion, stHttpBytesFromClient=stHttpBytesFromClient, PYSNMP_MODULE_ID=mwg, kRevision=kRevision, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpRequests=stHttpRequests, mwgInfo=mwgInfo, notifyReason=notifyReason, stFtpTraffic=stFtpTraffic, stHttpBytesToClient=stHttpBytesToClient, stFtpBytesToClient=stFtpBytesToClient, kMinorVersion=kMinorVersion, notifyOriginName=notifyOriginName, notifyOrigin=notifyOrigin, pAMSignatureVersion=pAMSignatureVersion, kProductVersion=kProductVersion, stMalwareDetected=stMalwareDetected, trUser=trUser, stHttpsBytesToClient=stHttpsBytesToClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stBlockedByAntiMalware=stBlockedByAntiMalware, mwg=mwg, stHttpBytesToServer=stHttpBytesToServer, stClientCount=stClientCount, mwgStatistics=mwgStatistics, stHttpsBytesToServer=stHttpsBytesToServer, notifyReasonString=notifyReasonString, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategoriesTable=stCategoriesTable, notifyAffectedHost=notifyAffectedHost, mwgHttp=mwgHttp, stConnectedSockets=stConnectedSockets, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHandleConnectToServer=stHandleConnectToServer)
