...
NotificationType, Gauge32, Bits, MibIdentifier, ObjectIdentity, TimeTicks, Counter64, iso, ModuleIdentity, Unsigned32, enterprises, Integer32, mib_2, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "Gauge32", "Bits", "MibIdentifier", "ObjectIdentity", "TimeTicks", "Counter64", "iso", "ModuleIdentity", "Unsigned32", "enterprises", "Integer32", "mib-2", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "Counter32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pTSDBVersion=pTSDBVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyAdditional=notifyAdditional, stCPUIOWait=stCPUIOWait, pMFEEngineVersion=pMFEEngineVersion, pAMSignatureVersion=pAMSignatureVersion, kMicroVersion=kMicroVersion, stHttpBytesFromClient=stHttpBytesFromClient, trUser=trUser, kCompanyName=kCompanyName, trSystem=trSystem, stConnectionsLegitimate=stConnectionsLegitimate, kCustomVersion=kCustomVersion, stFtpBytesToServer=stFtpBytesToServer, stHttpsBytesToClient=stHttpsBytesToClient, PYSNMP_MODULE_ID=mwg, pAMEngineVersion=pAMEngineVersion, notifyAffectedHost=notifyAffectedHost, stClientCount=stClientCount, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgMiscellaneous=mwgMiscellaneous, stMalwareDetected=stMalwareDetected, pMFEDATVersion=pMFEDATVersion, mwgHttps=mwgHttps, kHotfixVersion=kHotfixVersion, stHttpsRequests=stHttpsRequests, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpTraffic=stHttpTraffic, stConnectedSockets=stConnectedSockets, stHttpsBytesToServer=stHttpsBytesToServer, kRevision=kRevision, pAMProactiveVersion=pAMProactiveVersion, stFtpTraffic=stFtpTraffic, stHttpBytesFromServer=stHttpBytesFromServer, mwgTrapVariables=mwgTrapVariables, mwgHttp=mwgHttp, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stCategories=stCategories, mwg=mwg, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, notifyOriginName=notifyOriginName, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifySeverity=notifySeverity, stMimeType=stMimeType, stBadReputation=stBadReputation, kBuildNumber=kBuildNumber, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stBlockedByMediaFilter=stBlockedByMediaFilter, stConnectionsBlocked=stConnectionsBlocked, notifyReason=notifyReason, mwgFTP=mwgFTP, mwgEngineVersions=mwgEngineVersions, mwgTraps=mwgTraps, stFtpBytesFromServer=stFtpBytesFromServer, stFtpBytesToClient=stFtpBytesToClient, mwgInfo=mwgInfo, stFtpBytesFromClient=stFtpBytesFromClient, mwgStatistics=mwgStatistics, stCategoriesEntry=stCategoriesEntry, stTimeForTransaction=stTimeForTransaction, notifyReasonString=notifyReasonString, stMemConsumed=stMemConsumed, notifyOrigin=notifyOrigin, stHandleConnectToServer=stHandleConnectToServer, kMajorVersion=kMajorVersion, stCategoryCount=stCategoryCount, stCPULoadRaw=stCPULoadRaw, mwgContent=mwgContent, stBlockedByAntiMalware=stBlockedByAntiMalware, stCPULoad=stCPULoad, stBlockedByURLFilter=stBlockedByURLFilter, kProductName=kProductName, trApplication=trApplication, stHttpBytesToClient=stHttpBytesToClient, stHttpRequests=stHttpRequests, kProductVersion=kProductVersion, stCategoryName=stCategoryName, kMinorVersion=kMinorVersion, stCategoriesTable=stCategoriesTable, stHttpBytesToServer=stHttpBytesToServer, stHttpsTraffic=stHttpsTraffic, stResolveHostViaDNS=stResolveHostViaDNS)
