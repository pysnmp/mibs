...
Integer32, NotificationType, ObjectIdentity, Counter32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, ModuleIdentity, Unsigned32, Counter64, iso, TimeTicks, mib_2, NotificationType, enterprises, IpAddress, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "NotificationType", "ObjectIdentity", "Counter32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "ModuleIdentity", "Unsigned32", "Counter64", "iso", "TimeTicks", "mib-2", "NotificationType", "enterprises", "IpAddress", "MibIdentifier")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategoryCount=stCategoryCount, trSystem=trSystem, stHttpsBytesFromServer=stHttpsBytesFromServer, stCategories=stCategories, mwgTraps=mwgTraps, trUser=trUser, mwgEngineVersions=mwgEngineVersions, kCompanyName=kCompanyName, stMimeType=stMimeType, stCPUIOWait=stCPUIOWait, kProductVersion=kProductVersion, pAMProactiveVersion=pAMProactiveVersion, stClientCount=stClientCount, mwgContent=mwgContent, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpBytesToClient=stHttpBytesToClient, trApplication=trApplication, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCategoriesEntry=stCategoriesEntry, stHttpsBytesToServer=stHttpsBytesToServer, kMinorVersion=kMinorVersion, notifyAdditional=notifyAdditional, kBuildNumber=kBuildNumber, stHttpBytesFromServer=stHttpBytesFromServer, stBadReputation=stBadReputation, notifyOriginName=notifyOriginName, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, PYSNMP_MODULE_ID=mwg, stHttpRequests=stHttpRequests, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategoryName=stCategoryName, stFtpBytesFromServer=stFtpBytesFromServer, stConnectionsBlocked=stConnectionsBlocked, stHandleConnectToServer=stHandleConnectToServer, stHttpsBytesToClient=stHttpsBytesToClient, stCPULoad=stCPULoad, mwgHttps=mwgHttps, stCategoriesTable=stCategoriesTable, stHttpsRequests=stHttpsRequests, mwgHttp=mwgHttp, stMalwareDetected=stMalwareDetected, mwgMiscellaneous=mwgMiscellaneous, notifySeverity=notifySeverity, mwg=mwg, stHttpsTraffic=stHttpsTraffic, notifyAffectedHost=notifyAffectedHost, stFtpBytesFromClient=stFtpBytesFromClient, kHotfixVersion=kHotfixVersion, kCustomVersion=kCustomVersion, stTimeForTransaction=stTimeForTransaction, stBlockedByAntiMalware=stBlockedByAntiMalware, stCPULoadRaw=stCPULoadRaw, stMemConsumed=stMemConsumed, kRevision=kRevision, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFtpTraffic=stFtpTraffic, pAMSignatureVersion=pAMSignatureVersion, stHttpTraffic=stHttpTraffic, mwgStatistics=mwgStatistics, stConnectedSockets=stConnectedSockets, kMajorVersion=kMajorVersion, stHttpBytesToServer=stHttpBytesToServer, pMFEEngineVersion=pMFEEngineVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, pAMEngineVersion=pAMEngineVersion, mwgInfo=mwgInfo, pMFEDATVersion=pMFEDATVersion, stFtpBytesToClient=stFtpBytesToClient, stBlockedByMediaFilter=stBlockedByMediaFilter, stFtpBytesToServer=stFtpBytesToServer, kProductName=kProductName, kMicroVersion=kMicroVersion, mwgFTP=mwgFTP, notifyOrigin=notifyOrigin, stConnectionsLegitimate=stConnectionsLegitimate, notifyReason=notifyReason, pTSDBVersion=pTSDBVersion, notifyReasonString=notifyReasonString, stHttpBytesFromClient=stHttpBytesFromClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stBlockedByURLFilter=stBlockedByURLFilter, mwgTrapVariables=mwgTrapVariables, stResolveHostViaDNS=stResolveHostViaDNS)
