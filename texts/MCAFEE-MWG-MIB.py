...
enterprises, Gauge32, iso, TimeTicks, Integer32, MibIdentifier, ObjectIdentity, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, mib_2, Counter32, NotificationType, ModuleIdentity, Bits, NotificationType, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Gauge32", "iso", "TimeTicks", "Integer32", "MibIdentifier", "ObjectIdentity", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "mib-2", "Counter32", "NotificationType", "ModuleIdentity", "Bits", "NotificationType", "Counter64")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgTrapVariables=mwgTrapVariables, mwgMiscellaneous=mwgMiscellaneous, pMFEDATVersion=pMFEDATVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stResolveHostViaDNS=stResolveHostViaDNS, kProductVersion=kProductVersion, stHttpBytesToClient=stHttpBytesToClient, notifyReasonString=notifyReasonString, kMicroVersion=kMicroVersion, stHttpsBytesToServer=stHttpsBytesToServer, kBuildNumber=kBuildNumber, stConnectedSockets=stConnectedSockets, notifyAdditional=notifyAdditional, stHttpTraffic=stHttpTraffic, notifyReason=notifyReason, kCustomVersion=kCustomVersion, kCompanyName=kCompanyName, stFtpTraffic=stFtpTraffic, trApplication=trApplication, stFtpBytesToClient=stFtpBytesToClient, stTimeForTransaction=stTimeForTransaction, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpBytesFromClient=stHttpBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, kRevision=kRevision, pTSDBVersion=pTSDBVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, PYSNMP_MODULE_ID=mwg, kMinorVersion=kMinorVersion, stHttpRequests=stHttpRequests, mwgFTP=mwgFTP, pAMSignatureVersion=pAMSignatureVersion, pAMEngineVersion=pAMEngineVersion, trSystem=trSystem, stFtpBytesFromServer=stFtpBytesFromServer, stBlockedByURLFilter=stBlockedByURLFilter, trUser=trUser, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kMajorVersion=kMajorVersion, stCategoriesTable=stCategoriesTable, mwgTraps=mwgTraps, stHttpsBytesToClient=stHttpsBytesToClient, stCPULoadRaw=stCPULoadRaw, stMemConsumed=stMemConsumed, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwg=mwg, mwgStatistics=mwgStatistics, stMimeType=stMimeType, pMFEEngineVersion=pMFEEngineVersion, mwgContent=mwgContent, mwgEngineVersions=mwgEngineVersions, stCategories=stCategories, mwgHttp=mwgHttp, stConnectionsBlocked=stConnectionsBlocked, mwgHttps=mwgHttps, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgInfo=mwgInfo, stFtpBytesToServer=stFtpBytesToServer, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsRequests=stHttpsRequests, stHandleConnectToServer=stHandleConnectToServer, notifyOrigin=notifyOrigin, notifyOriginName=notifyOriginName, stFtpBytesFromClient=stFtpBytesFromClient, pAMProactiveVersion=pAMProactiveVersion, notifyAffectedHost=notifyAffectedHost, stBadReputation=stBadReputation, stCategoriesEntry=stCategoriesEntry, stHttpsTraffic=stHttpsTraffic, notifySeverity=notifySeverity, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stMalwareDetected=stMalwareDetected, stHttpBytesToServer=stHttpBytesToServer, stCategoryCount=stCategoryCount, stCategoryName=stCategoryName, kHotfixVersion=kHotfixVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stCPULoad=stCPULoad, stHttpBytesFromServer=stHttpBytesFromServer, kProductName=kProductName, stClientCount=stClientCount, stCPUIOWait=stCPUIOWait)
