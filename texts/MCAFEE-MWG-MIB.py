...
NotificationType, Counter32, IpAddress, enterprises, Bits, iso, NotificationType, ModuleIdentity, Counter64, Unsigned32, TimeTicks, mib_2, ObjectIdentity, Integer32, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "Counter32", "IpAddress", "enterprises", "Bits", "iso", "NotificationType", "ModuleIdentity", "Counter64", "Unsigned32", "TimeTicks", "mib-2", "ObjectIdentity", "Integer32", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pAMEngineVersion=pAMEngineVersion, stFtpBytesFromClient=stFtpBytesFromClient, stHttpsRequests=stHttpsRequests, kCompanyName=kCompanyName, mwgHttp=mwgHttp, stCPULoadRaw=stCPULoadRaw, stHttpRequests=stHttpRequests, notifyOriginName=notifyOriginName, stTimeForTransaction=stTimeForTransaction, pTSDBVersion=pTSDBVersion, stHttpBytesToClient=stHttpBytesToClient, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stConnectionsLegitimate=stConnectionsLegitimate, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kHotfixVersion=kHotfixVersion, pMFEDATVersion=pMFEDATVersion, stConnectionsBlocked=stConnectionsBlocked, stClientCount=stClientCount, trSystem=trSystem, pAMProactiveVersion=pAMProactiveVersion, stHttpsTraffic=stHttpsTraffic, mwgEngineVersions=mwgEngineVersions, kMicroVersion=kMicroVersion, kRevision=kRevision, notifyReason=notifyReason, stFtpTraffic=stFtpTraffic, stCPUIOWait=stCPUIOWait, mwgTraps=mwgTraps, stFtpBytesToServer=stFtpBytesToServer, notifyOrigin=notifyOrigin, kProductName=kProductName, stHttpBytesToServer=stHttpBytesToServer, notifySeverity=notifySeverity, stMimeType=stMimeType, kBuildNumber=kBuildNumber, mwg=mwg, stBadReputation=stBadReputation, mwgMiscellaneous=mwgMiscellaneous, stCategoryCount=stCategoryCount, kMajorVersion=kMajorVersion, trApplication=trApplication, stHttpsBytesFromClient=stHttpsBytesFromClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHandleConnectToServer=stHandleConnectToServer, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesToServer=stHttpsBytesToServer, kCustomVersion=kCustomVersion, stResolveHostViaDNS=stResolveHostViaDNS, PYSNMP_MODULE_ID=mwg, notifyAdditional=notifyAdditional, mwgStatistics=mwgStatistics, stFtpBytesToClient=stFtpBytesToClient, notifyReasonString=notifyReasonString, stConnectedSockets=stConnectedSockets, stMalwareDetected=stMalwareDetected, stCPULoad=stCPULoad, stMemConsumed=stMemConsumed, kMinorVersion=kMinorVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, pMFEEngineVersion=pMFEEngineVersion, stHttpBytesFromClient=stHttpBytesFromClient, stFtpBytesFromServer=stFtpBytesFromServer, mwgInfo=mwgInfo, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgContent=mwgContent, stCategoriesTable=stCategoriesTable, stCategoryName=stCategoryName, mwgTrapVariables=mwgTrapVariables, stHttpTraffic=stHttpTraffic, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgFTP=mwgFTP, notifyAffectedHost=notifyAffectedHost, stHttpBytesFromServer=stHttpBytesFromServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stBlockedByURLFilter=stBlockedByURLFilter, kProductVersion=kProductVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgHttps=mwgHttps, stHttpsBytesToClient=stHttpsBytesToClient, stCategories=stCategories, trUser=trUser, stCategoriesEntry=stCategoriesEntry, stBlockedByMediaFilter=stBlockedByMediaFilter)
