...
NotificationType, Integer32, ObjectIdentity, enterprises, Gauge32, mib_2, TimeTicks, MibIdentifier, NotificationType, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Counter32, Bits, ModuleIdentity, Unsigned32, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "Integer32", "ObjectIdentity", "enterprises", "Gauge32", "mib-2", "TimeTicks", "MibIdentifier", "NotificationType", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Counter32", "Bits", "ModuleIdentity", "Unsigned32", "Counter64")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, trUser=trUser, stCategories=stCategories, stHandleConnectToServer=stHandleConnectToServer, stResolveHostViaDNS=stResolveHostViaDNS, pTSDBVersion=pTSDBVersion, notifyAffectedHost=notifyAffectedHost, mwgHttp=mwgHttp, notifyOriginName=notifyOriginName, stCategoryCount=stCategoryCount, mwgTrapVariables=mwgTrapVariables, stHttpsRequests=stHttpsRequests, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgFTP=mwgFTP, kMicroVersion=kMicroVersion, kProductName=kProductName, trSystem=trSystem, mwgTraps=mwgTraps, stHttpRequests=stHttpRequests, kMajorVersion=kMajorVersion, stConnectionsLegitimate=stConnectionsLegitimate, stHttpBytesToClient=stHttpBytesToClient, stHttpsBytesToClient=stHttpsBytesToClient, stFtpTraffic=stFtpTraffic, stFtpBytesToClient=stFtpBytesToClient, stCPUIOWait=stCPUIOWait, stConnectionsBlocked=stConnectionsBlocked, trApplication=trApplication, stCategoriesEntry=stCategoriesEntry, mwgMiscellaneous=mwgMiscellaneous, mwgHttps=mwgHttps, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stMemConsumed=stMemConsumed, kProductVersion=kProductVersion, kMinorVersion=kMinorVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stFtpBytesFromServer=stFtpBytesFromServer, kBuildNumber=kBuildNumber, notifyAdditional=notifyAdditional, pMFEDATVersion=pMFEDATVersion, stClientCount=stClientCount, stCPULoad=stCPULoad, stLastSentLastReceivedClient=stLastSentLastReceivedClient, pAMEngineVersion=pAMEngineVersion, stFtpBytesFromClient=stFtpBytesFromClient, notifySeverity=notifySeverity, stBlockedByURLFilter=stBlockedByURLFilter, stCategoryName=stCategoryName, stFtpBytesToServer=stFtpBytesToServer, notifyReason=notifyReason, notifyReasonString=notifyReasonString, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyOrigin=notifyOrigin, stCPULoadRaw=stCPULoadRaw, mwgEngineVersions=mwgEngineVersions, mwg=mwg, stHttpsTraffic=stHttpsTraffic, stHttpBytesFromServer=stHttpBytesFromServer, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgStatistics=mwgStatistics, kRevision=kRevision, stMimeType=stMimeType, stConnectedSockets=stConnectedSockets, stBadReputation=stBadReputation, stTimeForTransaction=stTimeForTransaction, kCustomVersion=kCustomVersion, stHttpBytesFromClient=stHttpBytesFromClient, PYSNMP_MODULE_ID=mwg, pMFEEngineVersion=pMFEEngineVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, pAMProactiveVersion=pAMProactiveVersion, stMalwareDetected=stMalwareDetected, stCategoriesTable=stCategoriesTable, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpTraffic=stHttpTraffic, stHttpBytesToServer=stHttpBytesToServer, mwgInfo=mwgInfo, kHotfixVersion=kHotfixVersion, mwgContent=mwgContent, kCompanyName=kCompanyName, stHttpsBytesToServer=stHttpsBytesToServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient)
