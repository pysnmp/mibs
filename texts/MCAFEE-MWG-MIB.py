...
MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Counter32, mib_2, Counter64, Gauge32, ObjectIdentity, Bits, NotificationType, ModuleIdentity, NotificationType, enterprises, MibIdentifier, Unsigned32, iso, IpAddress, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Counter32", "mib-2", "Counter64", "Gauge32", "ObjectIdentity", "Bits", "NotificationType", "ModuleIdentity", "NotificationType", "enterprises", "MibIdentifier", "Unsigned32", "iso", "IpAddress", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pAMSignatureVersion=pAMSignatureVersion, kBuildNumber=kBuildNumber, stConnectedSockets=stConnectedSockets, notifyAffectedHost=notifyAffectedHost, stHttpsTraffic=stHttpsTraffic, stLastSentLastReceivedServer=stLastSentLastReceivedServer, pMFEDATVersion=pMFEDATVersion, mwgTraps=mwgTraps, kMinorVersion=kMinorVersion, notifySeverity=notifySeverity, stConnectionsBlocked=stConnectionsBlocked, trUser=trUser, mwgMiscellaneous=mwgMiscellaneous, kRevision=kRevision, mwgHttps=mwgHttps, stHttpsBytesFromClient=stHttpsBytesFromClient, kHotfixVersion=kHotfixVersion, notifyOriginName=notifyOriginName, mwgHttp=mwgHttp, stLastSentLastReceivedClient=stLastSentLastReceivedClient, pMFEEngineVersion=pMFEEngineVersion, stFtpBytesFromServer=stFtpBytesFromServer, stHandleConnectToServer=stHandleConnectToServer, kMicroVersion=kMicroVersion, stMemConsumed=stMemConsumed, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoryName=stCategoryName, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpBytesFromClient=stHttpBytesFromClient, notifyAdditional=notifyAdditional, stFtpBytesToServer=stFtpBytesToServer, kProductName=kProductName, notifyOrigin=notifyOrigin, pTSDBVersion=pTSDBVersion, stHttpRequests=stHttpRequests, mwgContent=mwgContent, stClientCount=stClientCount, stBlockedByMediaFilter=stBlockedByMediaFilter, PYSNMP_MODULE_ID=mwg, stFtpBytesToClient=stFtpBytesToClient, stCategoriesEntry=stCategoriesEntry, mwgInfo=mwgInfo, stCategories=stCategories, kMajorVersion=kMajorVersion, stConnectionsLegitimate=stConnectionsLegitimate, mwgTrapVariables=mwgTrapVariables, pAMEngineVersion=pAMEngineVersion, stBadReputation=stBadReputation, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgFTP=mwgFTP, kCustomVersion=kCustomVersion, stTimeForTransaction=stTimeForTransaction, stMalwareDetected=stMalwareDetected, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stResolveHostViaDNS=stResolveHostViaDNS, stHttpBytesToClient=stHttpBytesToClient, stFtpBytesFromClient=stFtpBytesFromClient, mwg=mwg, mwgStatistics=mwgStatistics, stFtpTraffic=stFtpTraffic, stHttpBytesToServer=stHttpBytesToServer, trApplication=trApplication, pAMProactiveVersion=pAMProactiveVersion, trSystem=trSystem, stHttpsRequests=stHttpsRequests, stCPULoad=stCPULoad, notifyReason=notifyReason, stBlockedByURLFilter=stBlockedByURLFilter, mwgEngineVersions=mwgEngineVersions, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsBytesToClient=stHttpsBytesToClient, kCompanyName=kCompanyName, stHttpTraffic=stHttpTraffic, stMimeType=stMimeType, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCategoryCount=stCategoryCount, stCPULoadRaw=stCPULoadRaw, stCategoriesTable=stCategoriesTable, kProductVersion=kProductVersion, stHttpBytesFromServer=stHttpBytesFromServer, stCPUIOWait=stCPUIOWait, notifyReasonString=notifyReasonString, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer)
