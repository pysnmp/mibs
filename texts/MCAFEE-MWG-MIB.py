...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pAMEngineVersion=pAMEngineVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kMinorVersion=kMinorVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, kCompanyName=kCompanyName, stCPULoad=stCPULoad, mwgMiscellaneous=mwgMiscellaneous, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyOriginName=notifyOriginName, stBlockedByURLFilter=stBlockedByURLFilter, kCustomVersion=kCustomVersion, mwgHttp=mwgHttp, trSystem=trSystem, stConnectedSockets=stConnectedSockets, notifyAffectedHost=notifyAffectedHost, trApplication=trApplication, mwgHttps=mwgHttps, stHttpBytesFromServer=stHttpBytesFromServer, stCategoryName=stCategoryName, pAMProactiveVersion=pAMProactiveVersion, stCategories=stCategories, stResolveHostViaDNS=stResolveHostViaDNS, stCategoriesTable=stCategoriesTable, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, kProductName=kProductName, kProductVersion=kProductVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategoriesEntry=stCategoriesEntry, stHttpsBytesToClient=stHttpsBytesToClient, pAMSignatureVersion=pAMSignatureVersion, stMimeType=stMimeType, mwgFTP=mwgFTP, notifyOrigin=notifyOrigin, kHotfixVersion=kHotfixVersion, stClientCount=stClientCount, stHttpRequests=stHttpRequests, mwgStatistics=mwgStatistics, stBlockedByAntiMalware=stBlockedByAntiMalware, kBuildNumber=kBuildNumber, stHandleConnectToServer=stHandleConnectToServer, stCategoryCount=stCategoryCount, stHttpBytesToClient=stHttpBytesToClient, stHttpBytesToServer=stHttpBytesToServer, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesFromServer=stFtpBytesFromServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgTraps=mwgTraps, stHttpsBytesFromClient=stHttpsBytesFromClient, stMemConsumed=stMemConsumed, mwgEngineVersions=mwgEngineVersions, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFtpBytesToServer=stFtpBytesToServer, mwg=mwg, notifyReason=notifyReason, stHttpsBytesToServer=stHttpsBytesToServer, stConnectionsLegitimate=stConnectionsLegitimate, stMalwareDetected=stMalwareDetected, stHttpsRequests=stHttpsRequests, stBadReputation=stBadReputation, notifySeverity=notifySeverity, stCPULoadRaw=stCPULoadRaw, kRevision=kRevision, stHttpsBytesFromServer=stHttpsBytesFromServer, trUser=trUser, kMicroVersion=kMicroVersion, notifyReasonString=notifyReasonString, notifyAdditional=notifyAdditional, pMFEEngineVersion=pMFEEngineVersion, stFtpTraffic=stFtpTraffic, stHttpsTraffic=stHttpsTraffic, stFtpBytesFromClient=stFtpBytesFromClient, stCPUIOWait=stCPUIOWait, stHttpTraffic=stHttpTraffic, stHttpBytesFromClient=stHttpBytesFromClient, stTimeForTransaction=stTimeForTransaction, mwgTrapVariables=mwgTrapVariables, mwgContent=mwgContent, mwgInfo=mwgInfo, pMFEDATVersion=pMFEDATVersion, pTSDBVersion=pTSDBVersion, stFtpBytesToClient=stFtpBytesToClient, kMajorVersion=kMajorVersion, PYSNMP_MODULE_ID=mwg)
