...
TimeTicks, IpAddress, Counter32, enterprises, iso, Counter64, Integer32, mib_2, NotificationType, MibIdentifier, NotificationType, Unsigned32, ObjectIdentity, ModuleIdentity, Gauge32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "IpAddress", "Counter32", "enterprises", "iso", "Counter64", "Integer32", "mib-2", "NotificationType", "MibIdentifier", "NotificationType", "Unsigned32", "ObjectIdentity", "ModuleIdentity", "Gauge32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategoryName=stCategoryName, mwgHttp=mwgHttp, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pAMEngineVersion=pAMEngineVersion, kHotfixVersion=kHotfixVersion, notifyOrigin=notifyOrigin, stConnectedSockets=stConnectedSockets, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpBytesFromClient=stHttpBytesFromClient, stHttpBytesToServer=stHttpBytesToServer, PYSNMP_MODULE_ID=mwg, stConnectionsBlocked=stConnectionsBlocked, stResolveHostViaDNS=stResolveHostViaDNS, mwgContent=mwgContent, pMFEEngineVersion=pMFEEngineVersion, trApplication=trApplication, stCategoriesEntry=stCategoriesEntry, notifyReason=notifyReason, kProductVersion=kProductVersion, pMFEDATVersion=pMFEDATVersion, stHttpsBytesToServer=stHttpsBytesToServer, kRevision=kRevision, kMinorVersion=kMinorVersion, stBlockedByURLFilter=stBlockedByURLFilter, stTimeForTransaction=stTimeForTransaction, stCategoryCount=stCategoryCount, stHttpsRequests=stHttpsRequests, stFtpBytesFromClient=stFtpBytesFromClient, mwgStatistics=mwgStatistics, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stFtpBytesFromServer=stFtpBytesFromServer, mwgTrapVariables=mwgTrapVariables, mwgInfo=mwgInfo, mwgHttps=mwgHttps, notifyAdditional=notifyAdditional, pAMSignatureVersion=pAMSignatureVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, kMicroVersion=kMicroVersion, kMajorVersion=kMajorVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpsBytesFromServer=stHttpsBytesFromServer, stCPUIOWait=stCPUIOWait, stHttpsBytesToClient=stHttpsBytesToClient, stFtpTraffic=stFtpTraffic, trUser=trUser, notifyReasonString=notifyReasonString, stHandleConnectToServer=stHandleConnectToServer, stHttpsTraffic=stHttpsTraffic, mwgFTP=mwgFTP, stCPULoadRaw=stCPULoadRaw, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgEngineVersions=mwgEngineVersions, stFtpBytesToServer=stFtpBytesToServer, stHttpRequests=stHttpRequests, stFtpBytesToClient=stFtpBytesToClient, stCategoriesTable=stCategoriesTable, notifySeverity=notifySeverity, stMemConsumed=stMemConsumed, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stBadReputation=stBadReputation, stMalwareDetected=stMalwareDetected, kBuildNumber=kBuildNumber, stHttpTraffic=stHttpTraffic, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategories=stCategories, mwgMiscellaneous=mwgMiscellaneous, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, notifyOriginName=notifyOriginName, kCompanyName=kCompanyName, kCustomVersion=kCustomVersion, pTSDBVersion=pTSDBVersion, notifyAffectedHost=notifyAffectedHost, mwgTraps=mwgTraps, pAMProactiveVersion=pAMProactiveVersion, stHttpBytesToClient=stHttpBytesToClient, stCPULoad=stCPULoad, mwg=mwg, stHttpBytesFromServer=stHttpBytesFromServer, stMimeType=stMimeType, stConnectionsLegitimate=stConnectionsLegitimate, trSystem=trSystem, kProductName=kProductName, stClientCount=stClientCount)
