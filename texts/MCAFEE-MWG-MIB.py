...
NotificationType, IpAddress, Counter64, enterprises, NotificationType, iso, TimeTicks, Gauge32, mib_2, ObjectIdentity, Bits, Integer32, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, ModuleIdentity, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "IpAddress", "Counter64", "enterprises", "NotificationType", "iso", "TimeTicks", "Gauge32", "mib-2", "ObjectIdentity", "Bits", "Integer32", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "ModuleIdentity", "Counter32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", trUser=trUser, kHotfixVersion=kHotfixVersion, stMalwareDetected=stMalwareDetected, pMFEDATVersion=pMFEDATVersion, notifyOrigin=notifyOrigin, stCategoryName=stCategoryName, kCompanyName=kCompanyName, stHttpsBytesToClient=stHttpsBytesToClient, stBlockedByURLFilter=stBlockedByURLFilter, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgFTP=mwgFTP, stHttpBytesFromClient=stHttpBytesFromClient, stBadReputation=stBadReputation, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgStatistics=mwgStatistics, stHttpsBytesFromServer=stHttpsBytesFromServer, stConnectedSockets=stConnectedSockets, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, pMFEEngineVersion=pMFEEngineVersion, stHttpsTraffic=stHttpsTraffic, stHttpBytesToClient=stHttpBytesToClient, stCPULoad=stCPULoad, mwgInfo=mwgInfo, stConnectionsLegitimate=stConnectionsLegitimate, kProductName=kProductName, mwgTrapVariables=mwgTrapVariables, kMicroVersion=kMicroVersion, stClientCount=stClientCount, kMajorVersion=kMajorVersion, stHttpsBytesToServer=stHttpsBytesToServer, trSystem=trSystem, pTSDBVersion=pTSDBVersion, pAMProactiveVersion=pAMProactiveVersion, stHttpRequests=stHttpRequests, mwgTraps=mwgTraps, stCategoryCount=stCategoryCount, notifyAffectedHost=notifyAffectedHost, stHttpBytesToServer=stHttpBytesToServer, stFtpBytesFromServer=stFtpBytesFromServer, stHttpBytesFromServer=stHttpBytesFromServer, stCategories=stCategories, stResolveHostViaDNS=stResolveHostViaDNS, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyAdditional=notifyAdditional, mwgHttps=mwgHttps, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsBytesFromClient=stHttpsBytesFromClient, stBlockedByMediaFilter=stBlockedByMediaFilter, stCPUIOWait=stCPUIOWait, kRevision=kRevision, stConnectionsBlocked=stConnectionsBlocked, PYSNMP_MODULE_ID=mwg, trApplication=trApplication, stHttpTraffic=stHttpTraffic, mwgMiscellaneous=mwgMiscellaneous, notifySeverity=notifySeverity, stMemConsumed=stMemConsumed, stFtpBytesToServer=stFtpBytesToServer, stCategoriesTable=stCategoriesTable, kMinorVersion=kMinorVersion, kBuildNumber=kBuildNumber, stCategoriesEntry=stCategoriesEntry, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwg=mwg, mwgEngineVersions=mwgEngineVersions, stTimeForTransaction=stTimeForTransaction, stFtpBytesToClient=stFtpBytesToClient, stHttpsRequests=stHttpsRequests, notifyReason=notifyReason, kCustomVersion=kCustomVersion, stFtpBytesFromClient=stFtpBytesFromClient, notifyOriginName=notifyOriginName, stHandleConnectToServer=stHandleConnectToServer, mwgHttp=mwgHttp, kProductVersion=kProductVersion, stCPULoadRaw=stCPULoadRaw, stMimeType=stMimeType, pAMEngineVersion=pAMEngineVersion, notifyReasonString=notifyReasonString, mwgContent=mwgContent, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stFtpTraffic=stFtpTraffic, pAMSignatureVersion=pAMSignatureVersion)
