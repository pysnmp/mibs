...
Integer32, NotificationType, iso, Unsigned32, IpAddress, MibIdentifier, enterprises, NotificationType, Bits, mib_2, ObjectIdentity, Counter32, Counter64, Gauge32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "NotificationType", "iso", "Unsigned32", "IpAddress", "MibIdentifier", "enterprises", "NotificationType", "Bits", "mib-2", "ObjectIdentity", "Counter32", "Counter64", "Gauge32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgTraps=mwgTraps, stCategoriesEntry=stCategoriesEntry, stMalwareDetected=stMalwareDetected, kProductVersion=kProductVersion, mwgInfo=mwgInfo, stHttpBytesToServer=stHttpBytesToServer, mwgHttp=mwgHttp, notifySeverity=notifySeverity, stClientCount=stClientCount, notifyOrigin=notifyOrigin, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stBadReputation=stBadReputation, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, pAMProactiveVersion=pAMProactiveVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, stCategoriesTable=stCategoriesTable, stHttpsBytesFromClient=stHttpsBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, trSystem=trSystem, stCPUIOWait=stCPUIOWait, stFtpTraffic=stFtpTraffic, kMicroVersion=kMicroVersion, stConnectedSockets=stConnectedSockets, stCPULoad=stCPULoad, stHttpBytesToClient=stHttpBytesToClient, mwgHttps=mwgHttps, pMFEDATVersion=pMFEDATVersion, stFtpBytesFromClient=stFtpBytesFromClient, stHttpsTraffic=stHttpsTraffic, stCategoryCount=stCategoryCount, mwg=mwg, stMemConsumed=stMemConsumed, stHttpTraffic=stHttpTraffic, stFtpBytesFromServer=stFtpBytesFromServer, stHttpsRequests=stHttpsRequests, stHttpBytesFromClient=stHttpBytesFromClient, stCategories=stCategories, stHttpBytesFromServer=stHttpBytesFromServer, stHttpsBytesToServer=stHttpsBytesToServer, notifyOriginName=notifyOriginName, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpsBytesToClient=stHttpsBytesToClient, mwgEngineVersions=mwgEngineVersions, pAMSignatureVersion=pAMSignatureVersion, kBuildNumber=kBuildNumber, notifyAffectedHost=notifyAffectedHost, stHttpsBytesFromServer=stHttpsBytesFromServer, notifyReasonString=notifyReasonString, notifyReason=notifyReason, stFtpBytesToServer=stFtpBytesToServer, PYSNMP_MODULE_ID=mwg, pMFEEngineVersion=pMFEEngineVersion, stResolveHostViaDNS=stResolveHostViaDNS, mwgMiscellaneous=mwgMiscellaneous, stMimeType=stMimeType, mwgFTP=mwgFTP, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyAdditional=notifyAdditional, stConnectionsBlocked=stConnectionsBlocked, stTimeForTransaction=stTimeForTransaction, stCategoryName=stCategoryName, stBlockedByURLFilter=stBlockedByURLFilter, stCPULoadRaw=stCPULoadRaw, stHttpRequests=stHttpRequests, kMajorVersion=kMajorVersion, pTSDBVersion=pTSDBVersion, stFtpBytesToClient=stFtpBytesToClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHandleConnectToServer=stHandleConnectToServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, trUser=trUser, kCompanyName=kCompanyName, kCustomVersion=kCustomVersion, pAMEngineVersion=pAMEngineVersion, trApplication=trApplication, mwgTrapVariables=mwgTrapVariables, mwgContent=mwgContent, kRevision=kRevision, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kProductName=kProductName, kHotfixVersion=kHotfixVersion, mwgStatistics=mwgStatistics, kMinorVersion=kMinorVersion)
