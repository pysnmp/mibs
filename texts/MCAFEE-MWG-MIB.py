...
MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, NotificationType, enterprises, NotificationType, TimeTicks, Counter32, Gauge32, IpAddress, mib_2, Integer32, iso, Unsigned32, Counter64, Bits, ObjectIdentity, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "NotificationType", "enterprises", "NotificationType", "TimeTicks", "Counter32", "Gauge32", "IpAddress", "mib-2", "Integer32", "iso", "Unsigned32", "Counter64", "Bits", "ObjectIdentity", "ModuleIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kRevision=kRevision, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, trApplication=trApplication, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoriesEntry=stCategoriesEntry, stMemConsumed=stMemConsumed, mwgHttps=mwgHttps, stHttpBytesToClient=stHttpBytesToClient, stFtpTraffic=stFtpTraffic, stClientCount=stClientCount, mwgStatistics=mwgStatistics, notifyAffectedHost=notifyAffectedHost, stCPULoadRaw=stCPULoadRaw, stHttpRequests=stHttpRequests, kMinorVersion=kMinorVersion, stCategoryName=stCategoryName, mwg=mwg, notifyReasonString=notifyReasonString, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stCPULoad=stCPULoad, PYSNMP_MODULE_ID=mwg, stFtpBytesFromServer=stFtpBytesFromServer, stHttpBytesFromServer=stHttpBytesFromServer, pTSDBVersion=pTSDBVersion, stHttpsRequests=stHttpsRequests, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgTraps=mwgTraps, kCompanyName=kCompanyName, stMalwareDetected=stMalwareDetected, mwgFTP=mwgFTP, notifyOrigin=notifyOrigin, kMajorVersion=kMajorVersion, stHttpsBytesToServer=stHttpsBytesToServer, stHandleConnectToServer=stHandleConnectToServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpBytesToServer=stFtpBytesToServer, mwgEngineVersions=mwgEngineVersions, stHttpBytesToServer=stHttpBytesToServer, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsTraffic=stHttpsTraffic, stFtpBytesFromClient=stFtpBytesFromClient, stCategoryCount=stCategoryCount, stCategoriesTable=stCategoriesTable, notifyOriginName=notifyOriginName, kProductName=kProductName, stHttpsBytesToClient=stHttpsBytesToClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kMicroVersion=kMicroVersion, pMFEEngineVersion=pMFEEngineVersion, pMFEDATVersion=pMFEDATVersion, mwgHttp=mwgHttp, stTimeForTransaction=stTimeForTransaction, notifyReason=notifyReason, notifyAdditional=notifyAdditional, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsBytesFromServer=stHttpsBytesFromServer, stResolveHostViaDNS=stResolveHostViaDNS, kBuildNumber=kBuildNumber, notifySeverity=notifySeverity, stHttpBytesFromClient=stHttpBytesFromClient, pAMProactiveVersion=pAMProactiveVersion, stFtpBytesToClient=stFtpBytesToClient, stHttpsBytesFromClient=stHttpsBytesFromClient, kCustomVersion=kCustomVersion, stConnectionsBlocked=stConnectionsBlocked, pAMEngineVersion=pAMEngineVersion, kProductVersion=kProductVersion, mwgContent=mwgContent, stMimeType=stMimeType, mwgTrapVariables=mwgTrapVariables, trUser=trUser, stConnectedSockets=stConnectedSockets, mwgMiscellaneous=mwgMiscellaneous, stBlockedByURLFilter=stBlockedByURLFilter, mwgInfo=mwgInfo, stBadReputation=stBadReputation, stHttpTraffic=stHttpTraffic, stCPUIOWait=stCPUIOWait, trSystem=trSystem, stLastSentLastReceivedClient=stLastSentLastReceivedClient, pAMSignatureVersion=pAMSignatureVersion, kHotfixVersion=kHotfixVersion, stCategories=stCategories)
