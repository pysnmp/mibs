...
TimeTicks, mib_2, NotificationType, iso, Unsigned32, enterprises, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, MibIdentifier, ModuleIdentity, Counter64, Integer32, Gauge32, Counter32, IpAddress, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "mib-2", "NotificationType", "iso", "Unsigned32", "enterprises", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "MibIdentifier", "ModuleIdentity", "Counter64", "Integer32", "Gauge32", "Counter32", "IpAddress", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgMiscellaneous=mwgMiscellaneous, notifyReason=notifyReason, mwg=mwg, stFtpBytesFromClient=stFtpBytesFromClient, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kProductName=kProductName, notifyReasonString=notifyReasonString, mwgEngineVersions=mwgEngineVersions, kRevision=kRevision, stHttpBytesFromServer=stHttpBytesFromServer, stClientCount=stClientCount, stBlockedByAntiMalware=stBlockedByAntiMalware, kHotfixVersion=kHotfixVersion, mwgHttps=mwgHttps, mwgTraps=mwgTraps, stBadReputation=stBadReputation, stCategories=stCategories, stHttpsBytesFromClient=stHttpsBytesFromClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCPULoadRaw=stCPULoadRaw, mwgTrapVariables=mwgTrapVariables, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsTraffic=stHttpsTraffic, stHttpBytesFromClient=stHttpBytesFromClient, stHttpBytesToServer=stHttpBytesToServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHandleConnectToServer=stHandleConnectToServer, stCPULoad=stCPULoad, mwgInfo=mwgInfo, stFtpBytesFromServer=stFtpBytesFromServer, kCompanyName=kCompanyName, stBlockedByURLFilter=stBlockedByURLFilter, stCategoriesEntry=stCategoriesEntry, mwgStatistics=mwgStatistics, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgHttp=mwgHttp, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsBytesToClient=stHttpsBytesToClient, stHttpRequests=stHttpRequests, pAMSignatureVersion=pAMSignatureVersion, stMemConsumed=stMemConsumed, stCategoryName=stCategoryName, stHttpsRequests=stHttpsRequests, notifyOriginName=notifyOriginName, stCategoryCount=stCategoryCount, stCPUIOWait=stCPUIOWait, notifySeverity=notifySeverity, stConnectionsLegitimate=stConnectionsLegitimate, pMFEDATVersion=pMFEDATVersion, mwgFTP=mwgFTP, stTimeForTransaction=stTimeForTransaction, stHttpsBytesFromServer=stHttpsBytesFromServer, pAMProactiveVersion=pAMProactiveVersion, pTSDBVersion=pTSDBVersion, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesToServer=stFtpBytesToServer, kMicroVersion=kMicroVersion, stHttpBytesToClient=stHttpBytesToClient, notifyAdditional=notifyAdditional, trApplication=trApplication, kBuildNumber=kBuildNumber, trSystem=trSystem, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCategoriesTable=stCategoriesTable, kMajorVersion=kMajorVersion, stMalwareDetected=stMalwareDetected, kCustomVersion=kCustomVersion, pMFEEngineVersion=pMFEEngineVersion, stMimeType=stMimeType, stConnectedSockets=stConnectedSockets, stFtpBytesToClient=stFtpBytesToClient, stFtpTraffic=stFtpTraffic, kMinorVersion=kMinorVersion, notifyOrigin=notifyOrigin, stHttpTraffic=stHttpTraffic, PYSNMP_MODULE_ID=mwg, notifyAffectedHost=notifyAffectedHost, mwgContent=mwgContent, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stResolveHostViaDNS=stResolveHostViaDNS, trUser=trUser, pAMEngineVersion=pAMEngineVersion, kProductVersion=kProductVersion)
