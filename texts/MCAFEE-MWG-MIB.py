...
Counter32, NotificationType, NotificationType, iso, MibIdentifier, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, ModuleIdentity, TimeTicks, IpAddress, Gauge32, mib_2, ObjectIdentity, Integer32, enterprises, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "NotificationType", "NotificationType", "iso", "MibIdentifier", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "ModuleIdentity", "TimeTicks", "IpAddress", "Gauge32", "mib-2", "ObjectIdentity", "Integer32", "enterprises", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stLastSentLastReceivedClient=stLastSentLastReceivedClient, kCompanyName=kCompanyName, kHotfixVersion=kHotfixVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stCPULoad=stCPULoad, stHttpBytesFromServer=stHttpBytesFromServer, stMimeType=stMimeType, stHttpsBytesToClient=stHttpsBytesToClient, mwgInfo=mwgInfo, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, notifyAdditional=notifyAdditional, stCPUIOWait=stCPUIOWait, mwgStatistics=mwgStatistics, mwg=mwg, stFtpBytesToClient=stFtpBytesToClient, notifySeverity=notifySeverity, pMFEDATVersion=pMFEDATVersion, stConnectionsBlocked=stConnectionsBlocked, stFtpBytesFromClient=stFtpBytesFromClient, stCategoriesTable=stCategoriesTable, stFtpTraffic=stFtpTraffic, kBuildNumber=kBuildNumber, trApplication=trApplication, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stBlockedByURLFilter=stBlockedByURLFilter, stFtpBytesToServer=stFtpBytesToServer, stHttpBytesFromClient=stHttpBytesFromClient, stHttpsBytesFromServer=stHttpsBytesFromServer, kMajorVersion=kMajorVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgEngineVersions=mwgEngineVersions, stCategoryName=stCategoryName, stHttpsTraffic=stHttpsTraffic, stCPULoadRaw=stCPULoadRaw, pAMSignatureVersion=pAMSignatureVersion, pTSDBVersion=pTSDBVersion, stBadReputation=stBadReputation, stResolveHostViaDNS=stResolveHostViaDNS, stConnectionsLegitimate=stConnectionsLegitimate, trSystem=trSystem, stCategoriesEntry=stCategoriesEntry, kRevision=kRevision, kCustomVersion=kCustomVersion, pMFEEngineVersion=pMFEEngineVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, pAMProactiveVersion=pAMProactiveVersion, stHttpTraffic=stHttpTraffic, notifyOrigin=notifyOrigin, notifyAffectedHost=notifyAffectedHost, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgHttps=mwgHttps, notifyReasonString=notifyReasonString, stClientCount=stClientCount, trUser=trUser, mwgTrapVariables=mwgTrapVariables, stCategoryCount=stCategoryCount, kProductVersion=kProductVersion, notifyReason=notifyReason, mwgContent=mwgContent, stFtpBytesFromServer=stFtpBytesFromServer, PYSNMP_MODULE_ID=mwg, mwgMiscellaneous=mwgMiscellaneous, stHttpsBytesFromClient=stHttpsBytesFromClient, stHandleConnectToServer=stHandleConnectToServer, stMemConsumed=stMemConsumed, stHttpBytesToServer=stHttpBytesToServer, stHttpsBytesToServer=stHttpsBytesToServer, mwgFTP=mwgFTP, stMalwareDetected=stMalwareDetected, stCategories=stCategories, pAMEngineVersion=pAMEngineVersion, notifyOriginName=notifyOriginName, mwgHttp=mwgHttp, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsRequests=stHttpsRequests, mwgTraps=mwgTraps, stHttpRequests=stHttpRequests, kMicroVersion=kMicroVersion, stConnectedSockets=stConnectedSockets, kMinorVersion=kMinorVersion, stTimeForTransaction=stTimeForTransaction, stHttpBytesToClient=stHttpBytesToClient, kProductName=kProductName)
