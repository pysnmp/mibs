...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", kCompanyName=kCompanyName, stHttpBytesFromServer=stHttpBytesFromServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgFTP=mwgFTP, kHotfixVersion=kHotfixVersion, kProductName=kProductName, kMinorVersion=kMinorVersion, stFtpTraffic=stFtpTraffic, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpBytesToServer=stHttpBytesToServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHandleConnectToServer=stHandleConnectToServer, pAMEngineVersion=pAMEngineVersion, stMalwareDetected=stMalwareDetected, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stFtpBytesToServer=stFtpBytesToServer, kRevision=kRevision, stTimeForTransaction=stTimeForTransaction, kCustomVersion=kCustomVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCPUIOWait=stCPUIOWait, mwgHttp=mwgHttp, kProductVersion=kProductVersion, stBadReputation=stBadReputation, notifyReasonString=notifyReasonString, notifyAdditional=notifyAdditional, pMFEDATVersion=pMFEDATVersion, pMFEEngineVersion=pMFEEngineVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, mwgTraps=mwgTraps, mwgContent=mwgContent, stClientCount=stClientCount, trApplication=trApplication, stCategoriesTable=stCategoriesTable, stCategoryCount=stCategoryCount, stFtpBytesFromClient=stFtpBytesFromClient, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsRequests=stHttpsRequests, mwgEngineVersions=mwgEngineVersions, mwgStatistics=mwgStatistics, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpBytesToClient=stHttpBytesToClient, stFtpBytesToClient=stFtpBytesToClient, mwgTrapVariables=mwgTrapVariables, mwg=mwg, pAMSignatureVersion=pAMSignatureVersion, kBuildNumber=kBuildNumber, notifyAffectedHost=notifyAffectedHost, mwgInfo=mwgInfo, stMemConsumed=stMemConsumed, notifyOriginName=notifyOriginName, stConnectedSockets=stConnectedSockets, stBlockedByURLFilter=stBlockedByURLFilter, stCPULoad=stCPULoad, stConnectionsBlocked=stConnectionsBlocked, kMajorVersion=kMajorVersion, kMicroVersion=kMicroVersion, stCategoriesEntry=stCategoriesEntry, notifyOrigin=notifyOrigin, stHttpsBytesFromServer=stHttpsBytesFromServer, stHttpsTraffic=stHttpsTraffic, mwgHttps=mwgHttps, notifySeverity=notifySeverity, trUser=trUser, stCPULoadRaw=stCPULoadRaw, stHttpRequests=stHttpRequests, stCategories=stCategories, stFtpBytesFromServer=stFtpBytesFromServer, stMimeType=stMimeType, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpBytesFromClient=stHttpBytesFromClient, notifyReason=notifyReason, stHttpsBytesToClient=stHttpsBytesToClient, stHttpsBytesToServer=stHttpsBytesToServer, PYSNMP_MODULE_ID=mwg, pTSDBVersion=pTSDBVersion, stResolveHostViaDNS=stResolveHostViaDNS, mwgMiscellaneous=mwgMiscellaneous, trSystem=trSystem, stHttpTraffic=stHttpTraffic, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, pAMProactiveVersion=pAMProactiveVersion, stCategoryName=stCategoryName)
