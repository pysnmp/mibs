...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", mwgEngineVersions=mwgEngineVersions, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpBytesFromServer=stHttpBytesFromServer, mwgHttp=mwgHttp, notifyOriginName=notifyOriginName, pAMSignatureVersion=pAMSignatureVersion, notifySeverity=notifySeverity, notifyAdditional=notifyAdditional, mwgTrapVariables=mwgTrapVariables, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stFtpTraffic=stFtpTraffic, stHttpsBytesToServer=stHttpsBytesToServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stFtpBytesFromServer=stFtpBytesFromServer, stMimeType=stMimeType, stHttpsBytesToClient=stHttpsBytesToClient, PYSNMP_MODULE_ID=mwg, pMFEDATVersion=pMFEDATVersion, stFtpBytesFromClient=stFtpBytesFromClient, stHttpTraffic=stHttpTraffic, stConnectedSockets=stConnectedSockets, stBadReputation=stBadReputation, stResolveHostViaDNS=stResolveHostViaDNS, kMinorVersion=kMinorVersion, stCategoryName=stCategoryName, notifyReason=notifyReason, stHttpBytesToClient=stHttpBytesToClient, pMFEEngineVersion=pMFEEngineVersion, stHttpsRequests=stHttpsRequests, kMicroVersion=kMicroVersion, stTimeForTransaction=stTimeForTransaction, kProductName=kProductName, stFtpBytesToClient=stFtpBytesToClient, kHotfixVersion=kHotfixVersion, kBuildNumber=kBuildNumber, stCPULoadRaw=stCPULoadRaw, stCPULoad=stCPULoad, trApplication=trApplication, stMalwareDetected=stMalwareDetected, mwgContent=mwgContent, pAMProactiveVersion=pAMProactiveVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyAffectedHost=notifyAffectedHost, stConnectionsBlocked=stConnectionsBlocked, notifyReasonString=notifyReasonString, kCompanyName=kCompanyName, stCategoriesEntry=stCategoriesEntry, stHttpBytesFromClient=stHttpBytesFromClient, kCustomVersion=kCustomVersion, stConnectionsLegitimate=stConnectionsLegitimate, mwgInfo=mwgInfo, mwgStatistics=mwgStatistics, stCPUIOWait=stCPUIOWait, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHandleConnectToServer=stHandleConnectToServer, pTSDBVersion=pTSDBVersion, stFtpBytesToServer=stFtpBytesToServer, kRevision=kRevision, mwgFTP=mwgFTP, stHttpsTraffic=stHttpsTraffic, mwgMiscellaneous=mwgMiscellaneous, stCategoryCount=stCategoryCount, kProductVersion=kProductVersion, trUser=trUser, stHttpsBytesFromServer=stHttpsBytesFromServer, stCategories=stCategories, mwgHttps=mwgHttps, trSystem=trSystem, pAMEngineVersion=pAMEngineVersion, stHttpRequests=stHttpRequests, mwgTraps=mwgTraps, stMemConsumed=stMemConsumed, notifyOrigin=notifyOrigin, kMajorVersion=kMajorVersion, stBlockedByURLFilter=stBlockedByURLFilter, mwg=mwg, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCategoriesTable=stCategoriesTable, stClientCount=stClientCount, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsBytesFromClient=stHttpsBytesFromClient, stHttpBytesToServer=stHttpBytesToServer)
