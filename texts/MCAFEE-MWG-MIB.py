...
iso, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, ObjectIdentity, Counter64, enterprises, Unsigned32, Counter32, Bits, TimeTicks, MibIdentifier, mib_2, Gauge32, ModuleIdentity, Integer32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "ObjectIdentity", "Counter64", "enterprises", "Unsigned32", "Counter32", "Bits", "TimeTicks", "MibIdentifier", "mib-2", "Gauge32", "ModuleIdentity", "Integer32", "NotificationType")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategoryName=stCategoryName, stHttpRequests=stHttpRequests, stCPULoad=stCPULoad, stFtpBytesFromClient=stFtpBytesFromClient, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpsBytesFromServer=stHttpsBytesFromServer, notifyAdditional=notifyAdditional, stHandleConnectToServer=stHandleConnectToServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, pMFEDATVersion=pMFEDATVersion, stFtpTraffic=stFtpTraffic, stFtpBytesToServer=stFtpBytesToServer, stCPULoadRaw=stCPULoadRaw, trSystem=trSystem, stFtpBytesFromServer=stFtpBytesFromServer, stHttpsRequests=stHttpsRequests, stCategories=stCategories, mwgHttp=mwgHttp, stFtpBytesToClient=stFtpBytesToClient, mwgStatistics=mwgStatistics, notifyReason=notifyReason, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stCategoryCount=stCategoryCount, stTimeForTransaction=stTimeForTransaction, notifySeverity=notifySeverity, mwgContent=mwgContent, notifyOriginName=notifyOriginName, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesToClient=stHttpBytesToClient, kMajorVersion=kMajorVersion, mwgTraps=mwgTraps, stCategoriesEntry=stCategoriesEntry, stMalwareDetected=stMalwareDetected, mwg=mwg, mwgHttps=mwgHttps, kCustomVersion=kCustomVersion, stBlockedByURLFilter=stBlockedByURLFilter, kHotfixVersion=kHotfixVersion, pAMEngineVersion=pAMEngineVersion, stConnectionsBlocked=stConnectionsBlocked, stCPUIOWait=stCPUIOWait, mwgTrapVariables=mwgTrapVariables, kMinorVersion=kMinorVersion, stBadReputation=stBadReputation, notifyReasonString=notifyReasonString, kRevision=kRevision, pMFEEngineVersion=pMFEEngineVersion, pAMProactiveVersion=pAMProactiveVersion, notifyOrigin=notifyOrigin, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesToServer=stHttpsBytesToServer, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, kMicroVersion=kMicroVersion, stHttpBytesFromClient=stHttpBytesFromClient, stHttpsBytesFromClient=stHttpsBytesFromClient, pTSDBVersion=pTSDBVersion, kBuildNumber=kBuildNumber, notifyAffectedHost=notifyAffectedHost, stClientCount=stClientCount, PYSNMP_MODULE_ID=mwg, kProductVersion=kProductVersion, stHttpsTraffic=stHttpsTraffic, stHttpTraffic=stHttpTraffic, stHttpBytesToServer=stHttpBytesToServer, stConnectedSockets=stConnectedSockets, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stResolveHostViaDNS=stResolveHostViaDNS, pAMSignatureVersion=pAMSignatureVersion, trUser=trUser, mwgInfo=mwgInfo, kCompanyName=kCompanyName, stCategoriesTable=stCategoriesTable, mwgEngineVersions=mwgEngineVersions, stConnectionsLegitimate=stConnectionsLegitimate, stMimeType=stMimeType, mwgFTP=mwgFTP, kProductName=kProductName, trApplication=trApplication, mwgMiscellaneous=mwgMiscellaneous, stMemConsumed=stMemConsumed, stHttpBytesFromServer=stHttpBytesFromServer)
