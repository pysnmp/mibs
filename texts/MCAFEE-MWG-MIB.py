...
mib_2, Counter64, iso, MibIdentifier, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, Gauge32, Bits, TimeTicks, NotificationType, Unsigned32, NotificationType, Counter32, ModuleIdentity, Integer32, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "mib-2", "Counter64", "iso", "MibIdentifier", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "Gauge32", "Bits", "TimeTicks", "NotificationType", "Unsigned32", "NotificationType", "Counter32", "ModuleIdentity", "Integer32", "ObjectIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pMFEDATVersion=pMFEDATVersion, stHttpsTraffic=stHttpsTraffic, stCategoryName=stCategoryName, stConnectionsBlocked=stConnectionsBlocked, stBlockedByMediaFilter=stBlockedByMediaFilter, kProductName=kProductName, kMajorVersion=kMajorVersion, mwgStatistics=mwgStatistics, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stMalwareDetected=stMalwareDetected, stMimeType=stMimeType, kProductVersion=kProductVersion, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, kMinorVersion=kMinorVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stHttpsBytesFromClient=stHttpsBytesFromClient, stClientCount=stClientCount, stCPULoad=stCPULoad, kRevision=kRevision, stHttpTraffic=stHttpTraffic, mwgTrapVariables=mwgTrapVariables, stHttpBytesFromServer=stHttpBytesFromServer, stConnectionsLegitimate=stConnectionsLegitimate, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyOriginName=notifyOriginName, stFtpTraffic=stFtpTraffic, stLastSentLastReceivedServer=stLastSentLastReceivedServer, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesToClient=stHttpBytesToClient, mwgTraps=mwgTraps, stFtpBytesToServer=stFtpBytesToServer, pTSDBVersion=pTSDBVersion, notifyReasonString=notifyReasonString, mwgInfo=mwgInfo, stHttpsBytesToClient=stHttpsBytesToClient, notifyReason=notifyReason, kMicroVersion=kMicroVersion, mwg=mwg, trUser=trUser, stBadReputation=stBadReputation, notifyOrigin=notifyOrigin, mwgHttps=mwgHttps, stHttpBytesFromClient=stHttpBytesFromClient, stTimeForTransaction=stTimeForTransaction, stBlockedByURLFilter=stBlockedByURLFilter, kCustomVersion=kCustomVersion, kHotfixVersion=kHotfixVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kBuildNumber=kBuildNumber, stConnectedSockets=stConnectedSockets, stFtpBytesToClient=stFtpBytesToClient, pAMEngineVersion=pAMEngineVersion, pAMProactiveVersion=pAMProactiveVersion, trSystem=trSystem, stFtpBytesFromServer=stFtpBytesFromServer, stCategoriesTable=stCategoriesTable, trApplication=trApplication, stHandleConnectToServer=stHandleConnectToServer, stCPUIOWait=stCPUIOWait, stCategoriesEntry=stCategoriesEntry, stCPULoadRaw=stCPULoadRaw, notifySeverity=notifySeverity, stCategoryCount=stCategoryCount, stHttpsBytesFromServer=stHttpsBytesFromServer, mwgContent=mwgContent, mwgEngineVersions=mwgEngineVersions, mwgHttp=mwgHttp, pMFEEngineVersion=pMFEEngineVersion, stCategories=stCategories, mwgMiscellaneous=mwgMiscellaneous, stMemConsumed=stMemConsumed, stHttpsRequests=stHttpsRequests, mwgFTP=mwgFTP, notifyAdditional=notifyAdditional, stHttpsBytesToServer=stHttpsBytesToServer, stResolveHostViaDNS=stResolveHostViaDNS, notifyAffectedHost=notifyAffectedHost, PYSNMP_MODULE_ID=mwg, stHttpRequests=stHttpRequests, kCompanyName=kCompanyName, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpBytesFromClient=stFtpBytesFromClient, stHttpBytesToServer=stHttpBytesToServer)
