...
Gauge32, iso, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, mib_2, Bits, Counter64, NotificationType, IpAddress, NotificationType, ObjectIdentity, Unsigned32, MibIdentifier, Counter32, Integer32, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "iso", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "mib-2", "Bits", "Counter64", "NotificationType", "IpAddress", "NotificationType", "ObjectIdentity", "Unsigned32", "MibIdentifier", "Counter32", "Integer32", "enterprises")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategoryCount=stCategoryCount, stHttpBytesToServer=stHttpBytesToServer, stCategories=stCategories, trUser=trUser, stHttpBytesToClient=stHttpBytesToClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpsBytesToServer=stHttpsBytesToServer, PYSNMP_MODULE_ID=mwg, kMinorVersion=kMinorVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stConnectedSockets=stConnectedSockets, stCategoriesEntry=stCategoriesEntry, stHttpsBytesFromServer=stHttpsBytesFromServer, stBadReputation=stBadReputation, mwg=mwg, pMFEEngineVersion=pMFEEngineVersion, stMimeType=stMimeType, mwgHttps=mwgHttps, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kBuildNumber=kBuildNumber, stBlockedByURLFilter=stBlockedByURLFilter, stCPUIOWait=stCPUIOWait, notifyReasonString=notifyReasonString, kProductVersion=kProductVersion, stFtpBytesFromServer=stFtpBytesFromServer, stMalwareDetected=stMalwareDetected, mwgHttp=mwgHttp, notifyOriginName=notifyOriginName, stCPULoadRaw=stCPULoadRaw, mwgContent=mwgContent, stFtpTraffic=stFtpTraffic, mwgMiscellaneous=mwgMiscellaneous, stConnectionsBlocked=stConnectionsBlocked, stHttpsRequests=stHttpsRequests, mwgFTP=mwgFTP, stCategoryName=stCategoryName, stTimeForTransaction=stTimeForTransaction, pAMEngineVersion=pAMEngineVersion, pMFEDATVersion=pMFEDATVersion, stHttpTraffic=stHttpTraffic, trSystem=trSystem, kCompanyName=kCompanyName, stHttpBytesFromServer=stHttpBytesFromServer, kCustomVersion=kCustomVersion, pTSDBVersion=pTSDBVersion, kMajorVersion=kMajorVersion, stFtpBytesToServer=stFtpBytesToServer, stResolveHostViaDNS=stResolveHostViaDNS, mwgTrapVariables=mwgTrapVariables, stConnectionsLegitimate=stConnectionsLegitimate, pAMSignatureVersion=pAMSignatureVersion, mwgStatistics=mwgStatistics, kProductName=kProductName, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoriesTable=stCategoriesTable, stBlockedByMediaFilter=stBlockedByMediaFilter, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpBytesFromClient=stHttpBytesFromClient, kRevision=kRevision, stCPULoad=stCPULoad, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, mwgInfo=mwgInfo, pAMProactiveVersion=pAMProactiveVersion, kHotfixVersion=kHotfixVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgEngineVersions=mwgEngineVersions, stHttpRequests=stHttpRequests, stHttpsBytesToClient=stHttpsBytesToClient, stFtpBytesToClient=stFtpBytesToClient, stFtpBytesFromClient=stFtpBytesFromClient, notifyAffectedHost=notifyAffectedHost, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stClientCount=stClientCount, stHandleConnectToServer=stHandleConnectToServer, notifyReason=notifyReason, notifyOrigin=notifyOrigin, mwgTraps=mwgTraps, trApplication=trApplication, stMemConsumed=stMemConsumed, notifySeverity=notifySeverity, kMicroVersion=kMicroVersion, stHttpsTraffic=stHttpsTraffic, notifyAdditional=notifyAdditional)
