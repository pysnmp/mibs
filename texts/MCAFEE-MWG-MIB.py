...
Integer32, NotificationType, MibIdentifier, Gauge32, Bits, Counter32, NotificationType, TimeTicks, IpAddress, iso, ObjectIdentity, enterprises, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, Counter64, mib_2 = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "NotificationType", "MibIdentifier", "Gauge32", "Bits", "Counter32", "NotificationType", "TimeTicks", "IpAddress", "iso", "ObjectIdentity", "enterprises", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "Counter64", "mib-2")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stConnectedSockets=stConnectedSockets, mwgInfo=mwgInfo, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyOriginName=notifyOriginName, mwgTraps=mwgTraps, stHttpsBytesFromClient=stHttpsBytesFromClient, stBadReputation=stBadReputation, notifyReasonString=notifyReasonString, kProductName=kProductName, pAMEngineVersion=pAMEngineVersion, trSystem=trSystem, kProductVersion=kProductVersion, stHttpRequests=stHttpRequests, stHttpsBytesFromServer=stHttpsBytesFromServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stFtpBytesToServer=stFtpBytesToServer, stFtpBytesFromClient=stFtpBytesFromClient, stMemConsumed=stMemConsumed, stHandleConnectToServer=stHandleConnectToServer, kRevision=kRevision, stHttpTraffic=stHttpTraffic, notifyAdditional=notifyAdditional, stHttpsBytesToClient=stHttpsBytesToClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgTrapVariables=mwgTrapVariables, notifyAffectedHost=notifyAffectedHost, pAMProactiveVersion=pAMProactiveVersion, kCompanyName=kCompanyName, mwgHttps=mwgHttps, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stMimeType=stMimeType, kMicroVersion=kMicroVersion, stCategories=stCategories, kCustomVersion=kCustomVersion, stResolveHostViaDNS=stResolveHostViaDNS, trUser=trUser, stConnectionsLegitimate=stConnectionsLegitimate, stCategoryName=stCategoryName, stConnectionsBlocked=stConnectionsBlocked, kHotfixVersion=kHotfixVersion, mwgContent=mwgContent, stCPULoad=stCPULoad, pTSDBVersion=pTSDBVersion, stFtpTraffic=stFtpTraffic, pMFEDATVersion=pMFEDATVersion, stMalwareDetected=stMalwareDetected, mwgEngineVersions=mwgEngineVersions, stHttpsTraffic=stHttpsTraffic, notifyReason=notifyReason, notifySeverity=notifySeverity, stCategoryCount=stCategoryCount, stHttpBytesToClient=stHttpBytesToClient, stHttpBytesFromServer=stHttpBytesFromServer, stClientCount=stClientCount, stTimeForTransaction=stTimeForTransaction, kBuildNumber=kBuildNumber, mwg=mwg, mwgMiscellaneous=mwgMiscellaneous, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpBytesFromClient=stHttpBytesFromClient, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyOrigin=notifyOrigin, mwgHttp=mwgHttp, pMFEEngineVersion=pMFEEngineVersion, mwgStatistics=mwgStatistics, stFtpBytesFromServer=stFtpBytesFromServer, stHttpBytesToServer=stHttpBytesToServer, stCPUIOWait=stCPUIOWait, mwgFTP=mwgFTP, trApplication=trApplication, stHttpsBytesToServer=stHttpsBytesToServer, stBlockedByURLFilter=stBlockedByURLFilter, stHttpsRequests=stHttpsRequests, stFtpBytesToClient=stFtpBytesToClient, kMinorVersion=kMinorVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stCategoriesEntry=stCategoriesEntry, pAMSignatureVersion=pAMSignatureVersion, stCategoriesTable=stCategoriesTable, kMajorVersion=kMajorVersion, stCPULoadRaw=stCPULoadRaw, PYSNMP_MODULE_ID=mwg)
