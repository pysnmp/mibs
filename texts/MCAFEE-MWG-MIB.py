...
mib_2, iso, MibIdentifier, ObjectIdentity, TimeTicks, NotificationType, enterprises, NotificationType, Integer32, Bits, Counter32, IpAddress, Gauge32, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "mib-2", "iso", "MibIdentifier", "ObjectIdentity", "TimeTicks", "NotificationType", "enterprises", "NotificationType", "Integer32", "Bits", "Counter32", "IpAddress", "Gauge32", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "ModuleIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pAMSignatureVersion=pAMSignatureVersion, stMalwareDetected=stMalwareDetected, pMFEDATVersion=pMFEDATVersion, kRevision=kRevision, stHttpBytesToServer=stHttpBytesToServer, stHttpsBytesFromClient=stHttpsBytesFromClient, trUser=trUser, stHttpsBytesFromServer=stHttpsBytesFromServer, kCustomVersion=kCustomVersion, kMicroVersion=kMicroVersion, mwgTraps=mwgTraps, notifyAdditional=notifyAdditional, mwgInfo=mwgInfo, stHttpsBytesToClient=stHttpsBytesToClient, stBlockedByAntiMalware=stBlockedByAntiMalware, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stCategoriesTable=stCategoriesTable, pTSDBVersion=pTSDBVersion, stFtpBytesFromServer=stFtpBytesFromServer, notifyReasonString=notifyReasonString, mwgHttps=mwgHttps, kProductName=kProductName, stHttpBytesFromClient=stHttpBytesFromClient, stResolveHostViaDNS=stResolveHostViaDNS, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stBlockedByURLFilter=stBlockedByURLFilter, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsBytesToServer=stHttpsBytesToServer, stHttpsTraffic=stHttpsTraffic, notifyOriginName=notifyOriginName, stConnectionsLegitimate=stConnectionsLegitimate, stFtpBytesFromClient=stFtpBytesFromClient, notifyReason=notifyReason, trApplication=trApplication, pAMEngineVersion=pAMEngineVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, stClientCount=stClientCount, mwgStatistics=mwgStatistics, notifyAffectedHost=notifyAffectedHost, mwgEngineVersions=mwgEngineVersions, stCategories=stCategories, kMajorVersion=kMajorVersion, mwgHttp=mwgHttp, PYSNMP_MODULE_ID=mwg, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFtpBytesToClient=stFtpBytesToClient, mwg=mwg, notifySeverity=notifySeverity, stMemConsumed=stMemConsumed, kHotfixVersion=kHotfixVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kCompanyName=kCompanyName, kBuildNumber=kBuildNumber, stCategoryName=stCategoryName, stCPUIOWait=stCPUIOWait, stCategoriesEntry=stCategoriesEntry, notifyOrigin=notifyOrigin, stCategoryCount=stCategoryCount, trSystem=trSystem, kMinorVersion=kMinorVersion, stMimeType=stMimeType, mwgContent=mwgContent, stHttpsRequests=stHttpsRequests, stFtpBytesToServer=stFtpBytesToServer, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stHttpTraffic=stHttpTraffic, stTimeForTransaction=stTimeForTransaction, stCPULoad=stCPULoad, stConnectionsBlocked=stConnectionsBlocked, pAMProactiveVersion=pAMProactiveVersion, kProductVersion=kProductVersion, pMFEEngineVersion=pMFEEngineVersion, stHttpBytesToClient=stHttpBytesToClient, stHttpRequests=stHttpRequests, mwgFTP=mwgFTP, mwgTrapVariables=mwgTrapVariables, stCPULoadRaw=stCPULoadRaw, stHttpBytesFromServer=stHttpBytesFromServer, stBadReputation=stBadReputation, stHandleConnectToServer=stHandleConnectToServer, stConnectedSockets=stConnectedSockets, stFtpTraffic=stFtpTraffic, mwgMiscellaneous=mwgMiscellaneous)
