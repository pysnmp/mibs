...
ModuleIdentity, Gauge32, MibIdentifier, TimeTicks, IpAddress, NotificationType, NotificationType, mib_2, Counter64, Counter32, iso, Integer32, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, Bits, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Gauge32", "MibIdentifier", "TimeTicks", "IpAddress", "NotificationType", "NotificationType", "mib-2", "Counter64", "Counter32", "iso", "Integer32", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "Bits", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pAMEngineVersion=pAMEngineVersion, pAMSignatureVersion=pAMSignatureVersion, kProductName=kProductName, stTimeForTransaction=stTimeForTransaction, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stFtpBytesFromServer=stFtpBytesFromServer, notifyReasonString=notifyReasonString, mwgHttp=mwgHttp, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpBytesToServer=stHttpBytesToServer, stCPULoad=stCPULoad, stFtpBytesToClient=stFtpBytesToClient, kCustomVersion=kCustomVersion, kRevision=kRevision, kProductVersion=kProductVersion, pMFEEngineVersion=pMFEEngineVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, notifyOriginName=notifyOriginName, notifySeverity=notifySeverity, stFtpBytesFromClient=stFtpBytesFromClient, notifyAdditional=notifyAdditional, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpTraffic=stHttpTraffic, stMemConsumed=stMemConsumed, notifyOrigin=notifyOrigin, stHttpsRequests=stHttpsRequests, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stMalwareDetected=stMalwareDetected, stCategoryCount=stCategoryCount, kMicroVersion=kMicroVersion, stHttpBytesToClient=stHttpBytesToClient, stCategories=stCategories, mwgTraps=mwgTraps, mwgHttps=mwgHttps, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHandleConnectToServer=stHandleConnectToServer, kMinorVersion=kMinorVersion, mwgTrapVariables=mwgTrapVariables, stHttpsBytesToClient=stHttpsBytesToClient, mwgInfo=mwgInfo, stFtpTraffic=stFtpTraffic, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwg=mwg, trUser=trUser, stBadReputation=stBadReputation, stCPUIOWait=stCPUIOWait, stResolveHostViaDNS=stResolveHostViaDNS, stHttpRequests=stHttpRequests, stClientCount=stClientCount, trSystem=trSystem, kHotfixVersion=kHotfixVersion, stHttpsTraffic=stHttpsTraffic, stHttpsBytesToServer=stHttpsBytesToServer, kBuildNumber=kBuildNumber, mwgStatistics=mwgStatistics, stConnectionsBlocked=stConnectionsBlocked, stHttpBytesFromServer=stHttpBytesFromServer, stConnectedSockets=stConnectedSockets, pMFEDATVersion=pMFEDATVersion, kCompanyName=kCompanyName, stCategoriesTable=stCategoriesTable, stConnectionsLegitimate=stConnectionsLegitimate, notifyAffectedHost=notifyAffectedHost, stHttpBytesFromClient=stHttpBytesFromClient, stCategoryName=stCategoryName, kMajorVersion=kMajorVersion, mwgFTP=mwgFTP, PYSNMP_MODULE_ID=mwg, mwgMiscellaneous=mwgMiscellaneous, stCPULoadRaw=stCPULoadRaw, trApplication=trApplication, pAMProactiveVersion=pAMProactiveVersion, mwgContent=mwgContent, stMimeType=stMimeType, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, mwgEngineVersions=mwgEngineVersions, stHttpsBytesFromServer=stHttpsBytesFromServer, pTSDBVersion=pTSDBVersion, stCategoriesEntry=stCategoriesEntry, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFtpBytesToServer=stFtpBytesToServer, stBlockedByURLFilter=stBlockedByURLFilter, notifyReason=notifyReason)
