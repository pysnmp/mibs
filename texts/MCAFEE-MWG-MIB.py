...
TimeTicks, Gauge32, Integer32, mib_2, MibIdentifier, ModuleIdentity, iso, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, Unsigned32, Counter64, enterprises, Bits, NotificationType, NotificationType, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Gauge32", "Integer32", "mib-2", "MibIdentifier", "ModuleIdentity", "iso", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "Unsigned32", "Counter64", "enterprises", "Bits", "NotificationType", "NotificationType", "IpAddress")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", pMFEEngineVersion=pMFEEngineVersion, kProductVersion=kProductVersion, stHandleConnectToServer=stHandleConnectToServer, stFtpBytesToServer=stFtpBytesToServer, pAMProactiveVersion=pAMProactiveVersion, mwgStatistics=mwgStatistics, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesToServer=stHttpsBytesToServer, mwgTrapVariables=mwgTrapVariables, mwgEngineVersions=mwgEngineVersions, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stConnectedSockets=stConnectedSockets, notifySeverity=notifySeverity, stBlockedByURLFilter=stBlockedByURLFilter, stFtpBytesFromClient=stFtpBytesFromClient, stCPULoad=stCPULoad, mwgHttps=mwgHttps, trSystem=trSystem, stMalwareDetected=stMalwareDetected, mwgFTP=mwgFTP, stHttpsBytesFromClient=stHttpsBytesFromClient, mwgHttp=mwgHttp, kMinorVersion=kMinorVersion, stMemConsumed=stMemConsumed, pAMEngineVersion=pAMEngineVersion, stHttpBytesFromClient=stHttpBytesFromClient, stTimeForTransaction=stTimeForTransaction, stFtpBytesFromServer=stFtpBytesFromServer, mwgContent=mwgContent, mwg=mwg, kCompanyName=kCompanyName, kProductName=kProductName, kCustomVersion=kCustomVersion, notifyReason=notifyReason, stCategoriesTable=stCategoriesTable, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyOriginName=notifyOriginName, stHttpBytesToServer=stHttpBytesToServer, pAMSignatureVersion=pAMSignatureVersion, stResolveHostViaDNS=stResolveHostViaDNS, stCategoriesEntry=stCategoriesEntry, notifyReasonString=notifyReasonString, stHttpsTraffic=stHttpsTraffic, kMicroVersion=kMicroVersion, stCategoryName=stCategoryName, stFtpTraffic=stFtpTraffic, stMimeType=stMimeType, stHttpBytesFromServer=stHttpBytesFromServer, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, notifyAdditional=notifyAdditional, PYSNMP_MODULE_ID=mwg, notifyAffectedHost=notifyAffectedHost, stHttpsBytesFromServer=stHttpsBytesFromServer, kMajorVersion=kMajorVersion, kRevision=kRevision, stHttpsRequests=stHttpsRequests, mwgMiscellaneous=mwgMiscellaneous, stHttpBytesToClient=stHttpBytesToClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stCPULoadRaw=stCPULoadRaw, stHttpRequests=stHttpRequests, stFtpBytesToClient=stFtpBytesToClient, stBadReputation=stBadReputation, stClientCount=stClientCount, stCategoryCount=stCategoryCount, pTSDBVersion=pTSDBVersion, trApplication=trApplication, pMFEDATVersion=pMFEDATVersion, stConnectionsLegitimate=stConnectionsLegitimate, mwgTraps=mwgTraps, stCPUIOWait=stCPUIOWait, stHttpTraffic=stHttpTraffic, trUser=trUser, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategories=stCategories, notifyOrigin=notifyOrigin, stConnectionsBlocked=stConnectionsBlocked, stHttpsBytesToClient=stHttpsBytesToClient, kHotfixVersion=kHotfixVersion, mwgInfo=mwgInfo, kBuildNumber=kBuildNumber)
