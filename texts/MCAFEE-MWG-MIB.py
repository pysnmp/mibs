...
iso, ModuleIdentity, Integer32, Unsigned32, Gauge32, IpAddress, TimeTicks, mib_2, Counter64, enterprises, NotificationType, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, MibIdentifier, NotificationType, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "ModuleIdentity", "Integer32", "Unsigned32", "Gauge32", "IpAddress", "TimeTicks", "mib-2", "Counter64", "enterprises", "NotificationType", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "MibIdentifier", "NotificationType", "ObjectIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategoriesTable=stCategoriesTable, stBlockedByMediaFilter=stBlockedByMediaFilter, kBuildNumber=kBuildNumber, stCategories=stCategories, stCPUIOWait=stCPUIOWait, kMajorVersion=kMajorVersion, stMalwareDetected=stMalwareDetected, mwg=mwg, kHotfixVersion=kHotfixVersion, stCategoriesEntry=stCategoriesEntry, stHttpsBytesFromClient=stHttpsBytesFromClient, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifySeverity=notifySeverity, stHttpBytesToClient=stHttpBytesToClient, notifyOrigin=notifyOrigin, mwgTraps=mwgTraps, stHttpsBytesFromServer=stHttpsBytesFromServer, stCPULoad=stCPULoad, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpTraffic=stHttpTraffic, stResolveHostViaDNS=stResolveHostViaDNS, stBlockedByURLFilter=stBlockedByURLFilter, stFtpBytesToServer=stFtpBytesToServer, notifyAffectedHost=notifyAffectedHost, pMFEDATVersion=pMFEDATVersion, mwgMiscellaneous=mwgMiscellaneous, stConnectionsLegitimate=stConnectionsLegitimate, notifyAdditional=notifyAdditional, notifyReason=notifyReason, trApplication=trApplication, mwgStatistics=mwgStatistics, pAMSignatureVersion=pAMSignatureVersion, stFtpBytesFromClient=stFtpBytesFromClient, mwgFTP=mwgFTP, stTimeForTransaction=stTimeForTransaction, mwgHttp=mwgHttp, pAMProactiveVersion=pAMProactiveVersion, mwgHttps=mwgHttps, stHttpRequests=stHttpRequests, stHttpBytesFromServer=stHttpBytesFromServer, mwgEngineVersions=mwgEngineVersions, stFtpBytesFromServer=stFtpBytesFromServer, trSystem=trSystem, pAMEngineVersion=pAMEngineVersion, mwgContent=mwgContent, stMimeType=stMimeType, stCategoryCount=stCategoryCount, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stConnectionsBlocked=stConnectionsBlocked, kMicroVersion=kMicroVersion, stMemConsumed=stMemConsumed, stLastSentLastReceivedClient=stLastSentLastReceivedClient, kProductName=kProductName, stHttpBytesToServer=stHttpBytesToServer, PYSNMP_MODULE_ID=mwg, kMinorVersion=kMinorVersion, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stBadReputation=stBadReputation, notifyReasonString=notifyReasonString, trUser=trUser, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stHttpsBytesToServer=stHttpsBytesToServer, mwgInfo=mwgInfo, kCustomVersion=kCustomVersion, stConnectedSockets=stConnectedSockets, kProductVersion=kProductVersion, pMFEEngineVersion=pMFEEngineVersion, stClientCount=stClientCount, stCPULoadRaw=stCPULoadRaw, stHttpsRequests=stHttpsRequests, stHandleConnectToServer=stHandleConnectToServer, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsTraffic=stHttpsTraffic, kRevision=kRevision, pTSDBVersion=pTSDBVersion, stHttpsBytesToClient=stHttpsBytesToClient, notifyOriginName=notifyOriginName, stHttpBytesFromClient=stHttpBytesFromClient, stCategoryName=stCategoryName, mwgTrapVariables=mwgTrapVariables, kCompanyName=kCompanyName, stFtpTraffic=stFtpTraffic, stFtpBytesToClient=stFtpBytesToClient)
