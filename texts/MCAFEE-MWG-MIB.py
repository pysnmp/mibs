...
ObjectIdentity, Integer32, TimeTicks, MibIdentifier, Counter64, iso, mib_2, Counter32, Gauge32, Unsigned32, NotificationType, NotificationType, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, enterprises, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Integer32", "TimeTicks", "MibIdentifier", "Counter64", "iso", "mib-2", "Counter32", "Gauge32", "Unsigned32", "NotificationType", "NotificationType", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "enterprises", "ModuleIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", trApplication=trApplication, stHttpRequests=stHttpRequests, mwgStatistics=mwgStatistics, pMFEDATVersion=pMFEDATVersion, kMajorVersion=kMajorVersion, stBlockedByMediaFilter=stBlockedByMediaFilter, notifyReasonString=notifyReasonString, kBuildNumber=kBuildNumber, stCPULoadRaw=stCPULoadRaw, stHttpBytesToServer=stHttpBytesToServer, kCustomVersion=kCustomVersion, stConnectionsLegitimate=stConnectionsLegitimate, stFtpBytesFromClient=stFtpBytesFromClient, stFtpBytesToClient=stFtpBytesToClient, stCategoriesEntry=stCategoriesEntry, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stCategoriesTable=stCategoriesTable, trSystem=trSystem, stConnectionsBlocked=stConnectionsBlocked, mwgContent=mwgContent, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, mwgTraps=mwgTraps, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectedSockets=stConnectedSockets, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgTrapVariables=mwgTrapVariables, stBadReputation=stBadReputation, stFtpBytesFromServer=stFtpBytesFromServer, mwgHttps=mwgHttps, kHotfixVersion=kHotfixVersion, mwgFTP=mwgFTP, mwgHttp=mwgHttp, notifyAffectedHost=notifyAffectedHost, kRevision=kRevision, stCPUIOWait=stCPUIOWait, stHttpTraffic=stHttpTraffic, stBlockedByAntiMalware=stBlockedByAntiMalware, stCategoryName=stCategoryName, stCategoryCount=stCategoryCount, pAMSignatureVersion=pAMSignatureVersion, kMicroVersion=kMicroVersion, mwg=mwg, trUser=trUser, stHttpsBytesToClient=stHttpsBytesToClient, stHttpBytesFromClient=stHttpBytesFromClient, pMFEEngineVersion=pMFEEngineVersion, stClientCount=stClientCount, stHandleConnectToServer=stHandleConnectToServer, pAMProactiveVersion=pAMProactiveVersion, kCompanyName=kCompanyName, stResolveHostViaDNS=stResolveHostViaDNS, pAMEngineVersion=pAMEngineVersion, stMalwareDetected=stMalwareDetected, notifyOrigin=notifyOrigin, stHttpBytesToClient=stHttpBytesToClient, kProductName=kProductName, pTSDBVersion=pTSDBVersion, mwgInfo=mwgInfo, stCPULoad=stCPULoad, kMinorVersion=kMinorVersion, stCategories=stCategories, notifyOriginName=notifyOriginName, notifyReason=notifyReason, stHttpBytesFromServer=stHttpBytesFromServer, stHttpsBytesToServer=stHttpsBytesToServer, kProductVersion=kProductVersion, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFtpBytesToServer=stFtpBytesToServer, stHttpsBytesFromServer=stHttpsBytesFromServer, stBlockedByURLFilter=stBlockedByURLFilter, notifyAdditional=notifyAdditional, PYSNMP_MODULE_ID=mwg, stHttpsBytesFromClient=stHttpsBytesFromClient, stMimeType=stMimeType, mwgMiscellaneous=mwgMiscellaneous, stFtpTraffic=stFtpTraffic, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpsTraffic=stHttpsTraffic, mwgEngineVersions=mwgEngineVersions, stTimeForTransaction=stTimeForTransaction, stHttpsRequests=stHttpsRequests, stMemConsumed=stMemConsumed, notifySeverity=notifySeverity)
