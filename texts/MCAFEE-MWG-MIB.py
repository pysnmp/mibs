...
Gauge32, Integer32, Unsigned32, mib_2, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, TimeTicks, ObjectIdentity, MibIdentifier, Bits, Counter64, NotificationType, iso, IpAddress, NotificationType, enterprises, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Integer32", "Unsigned32", "mib-2", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "TimeTicks", "ObjectIdentity", "MibIdentifier", "Bits", "Counter64", "NotificationType", "iso", "IpAddress", "NotificationType", "enterprises", "ModuleIdentity")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stHttpsBytesToClient=stHttpsBytesToClient, stMalwareDetected=stMalwareDetected, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stHttpTraffic=stHttpTraffic, stHttpsTraffic=stHttpsTraffic, mwgInfo=mwgInfo, stCategoryName=stCategoryName, mwgMiscellaneous=mwgMiscellaneous, stResolveHostViaDNS=stResolveHostViaDNS, pAMSignatureVersion=pAMSignatureVersion, pTSDBVersion=pTSDBVersion, kMicroVersion=kMicroVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, mwgEngineVersions=mwgEngineVersions, stHttpBytesFromServer=stHttpBytesFromServer, stFtpBytesToServer=stFtpBytesToServer, stBlockedByURLFilter=stBlockedByURLFilter, kCompanyName=kCompanyName, stCPULoad=stCPULoad, stHttpsBytesFromClient=stHttpsBytesFromClient, mwgContent=mwgContent, stCategoriesEntry=stCategoriesEntry, kMinorVersion=kMinorVersion, stHttpRequests=stHttpRequests, stHttpBytesFromClient=stHttpBytesFromClient, mwg=mwg, kHotfixVersion=kHotfixVersion, stTimeForTransaction=stTimeForTransaction, notifySeverity=notifySeverity, trSystem=trSystem, notifyAffectedHost=notifyAffectedHost, mwgStatistics=mwgStatistics, pMFEDATVersion=pMFEDATVersion, mwgTrapVariables=mwgTrapVariables, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stCategories=stCategories, mwgHttp=mwgHttp, stCPULoadRaw=stCPULoadRaw, stFtpTraffic=stFtpTraffic, stConnectionsBlocked=stConnectionsBlocked, trApplication=trApplication, trUser=trUser, stFtpBytesToClient=stFtpBytesToClient, stCategoryCount=stCategoryCount, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsBytesFromServer=stHttpsBytesFromServer, stClientCount=stClientCount, kRevision=kRevision, stMemConsumed=stMemConsumed, stHandleConnectToServer=stHandleConnectToServer, pMFEEngineVersion=pMFEEngineVersion, stFtpBytesFromServer=stFtpBytesFromServer, PYSNMP_MODULE_ID=mwg, mwgFTP=mwgFTP, kProductVersion=kProductVersion, stFtpBytesFromClient=stFtpBytesFromClient, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stConnectedSockets=stConnectedSockets, stHttpBytesToServer=stHttpBytesToServer, stHttpBytesToClient=stHttpBytesToClient, kCustomVersion=kCustomVersion, stMimeType=stMimeType, kProductName=kProductName, kBuildNumber=kBuildNumber, pAMEngineVersion=pAMEngineVersion, stBlockedByAntiMalware=stBlockedByAntiMalware, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsBytesToServer=stHttpsBytesToServer, mwgTraps=mwgTraps, notifyOrigin=notifyOrigin, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stCategoriesTable=stCategoriesTable, kMajorVersion=kMajorVersion, notifyReasonString=notifyReasonString, pAMProactiveVersion=pAMProactiveVersion, notifyReason=notifyReason, mwgHttps=mwgHttps, stBadReputation=stBadReputation, stHttpsRequests=stHttpsRequests, stCPUIOWait=stCPUIOWait, notifyAdditional=notifyAdditional, notifyOriginName=notifyOriginName)
