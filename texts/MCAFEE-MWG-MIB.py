...
TimeTicks, enterprises, Unsigned32, mib_2, Counter64, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, NotificationType, iso, ObjectIdentity, Counter32, MibIdentifier, Gauge32, Bits, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "enterprises", "Unsigned32", "mib-2", "Counter64", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "NotificationType", "iso", "ObjectIdentity", "Counter32", "MibIdentifier", "Gauge32", "Bits", "Integer32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stBlockedByURLFilter=stBlockedByURLFilter, stFtpBytesToServer=stFtpBytesToServer, mwgFTP=mwgFTP, stHttpTraffic=stHttpTraffic, trSystem=trSystem, stHttpsBytesFromClient=stHttpsBytesFromClient, stLastSentLastReceivedClient=stLastSentLastReceivedClient, mwgTraps=mwgTraps, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgTrapVariables=mwgTrapVariables, stHttpsBytesToClient=stHttpsBytesToClient, notifyReason=notifyReason, mwgStatistics=mwgStatistics, stFtpBytesFromClient=stFtpBytesFromClient, notifyOrigin=notifyOrigin, stHttpRequests=stHttpRequests, kCustomVersion=kCustomVersion, kHotfixVersion=kHotfixVersion, notifyOriginName=notifyOriginName, mwgInfo=mwgInfo, pMFEEngineVersion=pMFEEngineVersion, trApplication=trApplication, pAMSignatureVersion=pAMSignatureVersion, stHttpBytesFromClient=stHttpBytesFromClient, stHandleConnectToServer=stHandleConnectToServer, notifySeverity=notifySeverity, kMajorVersion=kMajorVersion, stMimeType=stMimeType, stHttpsBytesFromServer=stHttpsBytesFromServer, PYSNMP_MODULE_ID=mwg, stResolveHostViaDNS=stResolveHostViaDNS, stCategories=stCategories, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stFtpBytesFromServer=stFtpBytesFromServer, stCPULoad=stCPULoad, kMicroVersion=kMicroVersion, kProductName=kProductName, stHttpBytesToClient=stHttpBytesToClient, stBlockedByAntiMalware=stBlockedByAntiMalware, pAMEngineVersion=pAMEngineVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stFtpBytesToClient=stFtpBytesToClient, stCPULoadRaw=stCPULoadRaw, kBuildNumber=kBuildNumber, pMFEDATVersion=pMFEDATVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stConnectedSockets=stConnectedSockets, stTimeForTransaction=stTimeForTransaction, mwgMiscellaneous=mwgMiscellaneous, stClientCount=stClientCount, stFtpTraffic=stFtpTraffic, stCategoryCount=stCategoryCount, notifyReasonString=notifyReasonString, kProductVersion=kProductVersion, trUser=trUser, mwgHttps=mwgHttps, stCPUIOWait=stCPUIOWait, kCompanyName=kCompanyName, kMinorVersion=kMinorVersion, stCategoriesTable=stCategoriesTable, stHttpsTraffic=stHttpsTraffic, mwgContent=mwgContent, mwg=mwg, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stHttpsRequests=stHttpsRequests, stHttpBytesFromServer=stHttpBytesFromServer, pAMProactiveVersion=pAMProactiveVersion, stHttpBytesToServer=stHttpBytesToServer, stCategoriesEntry=stCategoriesEntry, stConnectionsLegitimate=stConnectionsLegitimate, stHttpsBytesToServer=stHttpsBytesToServer, notifyAffectedHost=notifyAffectedHost, stConnectionsBlocked=stConnectionsBlocked, mwgHttp=mwgHttp, pTSDBVersion=pTSDBVersion, mwgEngineVersions=mwgEngineVersions, kRevision=kRevision, stCategoryName=stCategoryName, stBadReputation=stBadReputation, notifyAdditional=notifyAdditional, stMalwareDetected=stMalwareDetected, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, stMemConsumed=stMemConsumed)
