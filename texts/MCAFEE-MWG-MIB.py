...
ModuleIdentity, Counter64, NotificationType, MibIdentifier, Bits, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, NotificationType, ObjectIdentity, IpAddress, enterprises, iso, Integer32, Counter32, mib_2, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Counter64", "NotificationType", "MibIdentifier", "Bits", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "NotificationType", "ObjectIdentity", "IpAddress", "enterprises", "iso", "Integer32", "Counter32", "mib-2", "Gauge32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stMemConsumed=stMemConsumed, trUser=trUser, stHttpsBytesToClient=stHttpsBytesToClient, stFtpTraffic=stFtpTraffic, pAMSignatureVersion=pAMSignatureVersion, mwgMiscellaneous=mwgMiscellaneous, stBadReputation=stBadReputation, stMalwareDetected=stMalwareDetected, stBlockedByAntiMalware=stBlockedByAntiMalware, kHotfixVersion=kHotfixVersion, kMajorVersion=kMajorVersion, mwgContent=mwgContent, stTimeForTransaction=stTimeForTransaction, trApplication=trApplication, kCompanyName=kCompanyName, stConnectedSockets=stConnectedSockets, stHttpBytesToServer=stHttpBytesToServer, mwgHttps=mwgHttps, notifyAdditional=notifyAdditional, mwgTraps=mwgTraps, stCPUIOWait=stCPUIOWait, stConnectionsLegitimate=stConnectionsLegitimate, notifyReasonString=notifyReasonString, stHttpTraffic=stHttpTraffic, mwgStatistics=mwgStatistics, mwg=mwg, stHttpBytesFromServer=stHttpBytesFromServer, stHttpsBytesToServer=stHttpsBytesToServer, stCategories=stCategories, kProductName=kProductName, kMinorVersion=kMinorVersion, mwgEngineVersions=mwgEngineVersions, stFtpBytesFromServer=stFtpBytesFromServer, stCPULoadRaw=stCPULoadRaw, stMimeType=stMimeType, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, notifyOriginName=notifyOriginName, notifyAffectedHost=notifyAffectedHost, mwgTrapVariables=mwgTrapVariables, kMicroVersion=kMicroVersion, kBuildNumber=kBuildNumber, stCategoryCount=stCategoryCount, stFtpBytesToServer=stFtpBytesToServer, stConnectionsBlocked=stConnectionsBlocked, stBlockedByMediaFilter=stBlockedByMediaFilter, stHttpsRequests=stHttpsRequests, kProductVersion=kProductVersion, mwgInfo=mwgInfo, pMFEEngineVersion=pMFEEngineVersion, stCPULoad=stCPULoad, stHttpRequests=stHttpRequests, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stClientCount=stClientCount, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, notifyOrigin=notifyOrigin, stHttpsTraffic=stHttpsTraffic, kCustomVersion=kCustomVersion, stCategoryName=stCategoryName, stHttpBytesFromClient=stHttpBytesFromClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stBlockedByURLFilter=stBlockedByURLFilter, stFtpBytesFromClient=stFtpBytesFromClient, stCategoriesTable=stCategoriesTable, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifySeverity=notifySeverity, stResolveHostViaDNS=stResolveHostViaDNS, trSystem=trSystem, stHttpBytesToClient=stHttpBytesToClient, notifyReason=notifyReason, kRevision=kRevision, pAMProactiveVersion=pAMProactiveVersion, stHttpsBytesFromClient=stHttpsBytesFromClient, stHandleConnectToServer=stHandleConnectToServer, pMFEDATVersion=pMFEDATVersion, stFtpBytesToClient=stFtpBytesToClient, pAMEngineVersion=pAMEngineVersion, mwgHttp=mwgHttp, PYSNMP_MODULE_ID=mwg, pTSDBVersion=pTSDBVersion, mwgFTP=mwgFTP, stCategoriesEntry=stCategoriesEntry, stHttpsBytesFromServer=stHttpsBytesFromServer)
