...
NotificationType, ModuleIdentity, IpAddress, NotificationType, ObjectIdentity, Counter32, enterprises, iso, TimeTicks, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, Gauge32, Integer32, mib_2, Counter64, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "ModuleIdentity", "IpAddress", "NotificationType", "ObjectIdentity", "Counter32", "enterprises", "iso", "TimeTicks", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "Gauge32", "Integer32", "mib-2", "Counter64", "Bits")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stCategoriesTable=stCategoriesTable, mwg=mwg, stFtpBytesToServer=stFtpBytesToServer, stTimeForTransaction=stTimeForTransaction, notifyReason=notifyReason, notifySeverity=notifySeverity, mwgMiscellaneous=mwgMiscellaneous, stCategoryName=stCategoryName, mwgStatistics=mwgStatistics, PYSNMP_MODULE_ID=mwg, notifyReasonString=notifyReasonString, kRevision=kRevision, stHttpBytesToClient=stHttpBytesToClient, stBlockedByURLFilter=stBlockedByURLFilter, stCPUIOWait=stCPUIOWait, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, mwgHttps=mwgHttps, stHttpsRequests=stHttpsRequests, pAMSignatureVersion=pAMSignatureVersion, stHandleConnectToServer=stHandleConnectToServer, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, notifyOriginName=notifyOriginName, pAMProactiveVersion=pAMProactiveVersion, stHttpBytesFromClient=stHttpBytesFromClient, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stFtpTraffic=stFtpTraffic, pMFEDATVersion=pMFEDATVersion, pTSDBVersion=pTSDBVersion, pAMEngineVersion=pAMEngineVersion, stHttpTraffic=stHttpTraffic, stBadReputation=stBadReputation, stHttpsBytesFromClient=stHttpsBytesFromClient, stClientCount=stClientCount, stLastSentLastReceivedServer=stLastSentLastReceivedServer, stConnectedSockets=stConnectedSockets, mwgTrapVariables=mwgTrapVariables, notifyAffectedHost=notifyAffectedHost, mwgFTP=mwgFTP, stFtpBytesFromServer=stFtpBytesFromServer, stFtpBytesToClient=stFtpBytesToClient, mwgEngineVersions=mwgEngineVersions, stCPULoad=stCPULoad, notifyAdditional=notifyAdditional, kProductVersion=kProductVersion, stMimeType=stMimeType, stHttpsBytesToClient=stHttpsBytesToClient, mwgHttp=mwgHttp, stBlockedByMediaFilter=stBlockedByMediaFilter, stFtpBytesFromClient=stFtpBytesFromClient, stMalwareDetected=stMalwareDetected, mwgContent=mwgContent, stLastSentLastReceivedClient=stLastSentLastReceivedClient, stHttpRequests=stHttpRequests, mwgTraps=mwgTraps, stResolveHostViaDNS=stResolveHostViaDNS, stHttpBytesFromServer=stHttpBytesFromServer, mwgInfo=mwgInfo, stHttpsBytesFromServer=stHttpsBytesFromServer, stBlockedByAntiMalware=stBlockedByAntiMalware, stHttpBytesToServer=stHttpBytesToServer, notifyOrigin=notifyOrigin, stHttpsBytesToServer=stHttpsBytesToServer, stConnectionsBlocked=stConnectionsBlocked, kProductName=kProductName, stHttpsTraffic=stHttpsTraffic, stMemConsumed=stMemConsumed, kCustomVersion=kCustomVersion, kCompanyName=kCompanyName, stCategoriesEntry=stCategoriesEntry, kBuildNumber=kBuildNumber, stCategories=stCategories, stConnectionsLegitimate=stConnectionsLegitimate, kMajorVersion=kMajorVersion, trUser=trUser, kHotfixVersion=kHotfixVersion, stCategoryCount=stCategoryCount, trApplication=trApplication, stCPULoadRaw=stCPULoadRaw, kMicroVersion=kMicroVersion, trSystem=trSystem, pMFEEngineVersion=pMFEEngineVersion, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, kMinorVersion=kMinorVersion)
