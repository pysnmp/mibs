...
MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, enterprises, ObjectIdentity, Integer32, ModuleIdentity, TimeTicks, MibIdentifier, iso, Gauge32, mib_2, NotificationType, Bits, Counter64, Counter32, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "enterprises", "ObjectIdentity", "Integer32", "ModuleIdentity", "TimeTicks", "MibIdentifier", "iso", "Gauge32", "mib-2", "NotificationType", "Bits", "Counter64", "Counter32", "Unsigned32")
...
mibBuilder.exportSymbols("MCAFEE-MWG-MIB", stLastSentLastReceivedClient=stLastSentLastReceivedClient, kBuildNumber=kBuildNumber, notifyReasonString=notifyReasonString, mwgTrapVariables=mwgTrapVariables, trSystem=trSystem, stConnectionsBlocked=stConnectionsBlocked, stHttpBytesFromServer=stHttpBytesFromServer, stHttpRequests=stHttpRequests, notifyAdditional=notifyAdditional, kMinorVersion=kMinorVersion, notifyOrigin=notifyOrigin, stHttpTraffic=stHttpTraffic, mwgStatistics=mwgStatistics, mwgInfo=mwgInfo, stHttpsRequests=stHttpsRequests, kMajorVersion=kMajorVersion, stFtpBytesFromServer=stFtpBytesFromServer, stCategoriesEntry=stCategoriesEntry, stCPULoad=stCPULoad, stBlockedByAntiMalware=stBlockedByAntiMalware, notifyOriginName=notifyOriginName, stFtpBytesToClient=stFtpBytesToClient, mwgMiscellaneous=mwgMiscellaneous, kCompanyName=kCompanyName, stConnectedSockets=stConnectedSockets, stClientCount=stClientCount, stCategories=stCategories, stHttpBytesToServer=stHttpBytesToServer, kProductName=kProductName, stCategoriesTable=stCategoriesTable, stHandleConnectToServer=stHandleConnectToServer, pMFEEngineVersion=pMFEEngineVersion, kProductVersion=kProductVersion, PYSNMP_MODULE_ID=mwg, stCategoryCount=stCategoryCount, notifyAffectedHost=notifyAffectedHost, kCustomVersion=kCustomVersion, mwgEngineVersions=mwgEngineVersions, mwg=mwg, kMicroVersion=kMicroVersion, stHttpsBytesFromServer=stHttpsBytesFromServer, stCPULoadRaw=stCPULoadRaw, mwgFTP=mwgFTP, kRevision=kRevision, stTimeConsumedByRuleEngine=stTimeConsumedByRuleEngine, stFtpTraffic=stFtpTraffic, stCPUIOWait=stCPUIOWait, stConnectionsLegitimate=stConnectionsLegitimate, trUser=trUser, stBadReputation=stBadReputation, stHttpsBytesToClient=stHttpsBytesToClient, stLastSentFirstReceivedServer=stLastSentFirstReceivedServer, stMalwareDetected=stMalwareDetected, stMemConsumed=stMemConsumed, stBlockedByMediaFilter=stBlockedByMediaFilter, mwgHttp=mwgHttp, pAMProactiveVersion=pAMProactiveVersion, stHttpsBytesToServer=stHttpsBytesToServer, mwgHttps=mwgHttps, stFtpBytesFromClient=stFtpBytesFromClient, trApplication=trApplication, stHttpBytesFromClient=stHttpBytesFromClient, stFtpBytesToServer=stFtpBytesToServer, pAMEngineVersion=pAMEngineVersion, stResolveHostViaDNS=stResolveHostViaDNS, stHttpBytesToClient=stHttpBytesToClient, mwgTraps=mwgTraps, stHttpsTraffic=stHttpsTraffic, stCategoryName=stCategoryName, kHotfixVersion=kHotfixVersion, stFirstSentFirstReceivedClient=stFirstSentFirstReceivedClient, stBlockedByURLFilter=stBlockedByURLFilter, stFirstSentFirstReceivedServer=stFirstSentFirstReceivedServer, pAMSignatureVersion=pAMSignatureVersion, notifySeverity=notifySeverity, pMFEDATVersion=pMFEDATVersion, stLastSentLastReceivedServer=stLastSentLastReceivedServer, notifyReason=notifyReason, stMimeType=stMimeType, pTSDBVersion=pTSDBVersion, stTimeForTransaction=stTimeForTransaction, stHttpsBytesFromClient=stHttpsBytesFromClient, mwgContent=mwgContent)
