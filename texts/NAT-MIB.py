...
mibBuilder.exportSymbols("NAT-MIB", natAddrMapName=natAddrMapName, natAddrMapIndex=natAddrMapIndex, natSessionPrivateAddrType=natSessionPrivateAddrType, natGroupStatefulObjects=natGroupStatefulObjects, natPacketDiscard=natPacketDiscard, natStatsProtocolGroup=natStatsProtocolGroup, natNotifSubscriberMappings=natNotifSubscriberMappings, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natSubscriberIntPrefix=natSubscriberIntPrefix, natAddrBindSessions=natAddrBindSessions, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, natMIB=natMIB, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natInterfaceServiceType=natInterfaceServiceType, natBasicStatelessCompliance=natBasicStatelessCompliance, natInterfaceRealm=natInterfaceRealm, natL4ProtocolNumber=natL4ProtocolNumber, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natPoolObjects=natPoolObjects, natGroupBasicNotifications=natGroupBasicNotifications, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natInstanceTable=natInstanceTable, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natInstanceEntry=natInstanceEntry, natMapIntAddrTable=natMapIntAddrTable, natInterfaceTable=natInterfaceTable, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natAddrBindInTranslates=natAddrBindInTranslates, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natAddressMappingCreations=natAddressMappingCreations, natSessionPublicAddrType=natSessionPublicAddrType, natMIBNotificationGroup=natMIBNotificationGroup, natAddrPortBindType=natAddrPortBindType, natInstanceAlias=natInstanceAlias, natMapIntAddrExt=natMapIntAddrExt, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natMapIntAddrExtRealm=natMapIntAddrExtRealm, natMappingIntRealm=natMappingIntRealm, natAddrBindEntry=natAddrBindEntry, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natCGNCompliance=natCGNCompliance, natSubscriberIndex=natSubscriberIndex, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natPoolEntry=natPoolEntry, natSubscriberQuotaDrops=natSubscriberQuotaDrops, natSessionIndex=natSessionIndex, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natPoolRangeType=natPoolRangeType, natInterfaceDiscards=natInterfaceDiscards, natQuotaDrops=natQuotaDrops, natPoolIndex=natPoolIndex, NatAssociationType=NatAssociationType, NatSessionId=NatSessionId, natMIBNotifications=natMIBNotifications, natInterfaceInTranslates=natInterfaceInTranslates, natNotifCtrl=natNotifCtrl, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natMIBGroups=natMIBGroups, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natGroupAddrMapObjects=natGroupAddrMapObjects, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natMappingTable=natMappingTable, natMappingExtRealm=natMappingExtRealm, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natL4ProtocolTable=natL4ProtocolTable, natMappingIntAddress=natMappingIntAddress, natSubscriberLimitMappings=natSubscriberLimitMappings, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natAddrMapEntry=natAddrMapEntry, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natSubscribers=natSubscribers, natProtocolOutTranslates=natProtocolOutTranslates, natGroupFragmentObjects=natGroupFragmentObjects, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natLimitsEntry=natLimitsEntry, natAddrMapRowStatus=natAddrMapRowStatus, natMapObjects=natMapObjects, natInterfaceStorageType=natInterfaceStorageType, natTranslations=natTranslations, natSessionProtocolType=natSessionProtocolType, natAddrBindType=natAddrBindType, natL4ProtocolTranslations=natL4ProtocolTranslations, natInterfaceEntry=natInterfaceEntry, natPoolRangeEntry=natPoolRangeEntry, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, SubscriberIndex=SubscriberIndex, PYSNMP_MODULE_ID=natMIB, natMappingIntPort=natMappingIntPort, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natInterfaceRowStatus=natInterfaceRowStatus, natAddrMapProtocol=natAddrMapProtocol, natProtocolEntry=natProtocolEntry, natLimitsTable=natLimitsTable, natAddrPortBindEntry=natAddrPortBindEntry, natAddrBindLocalAddr=natAddrBindLocalAddr, natMappingIntAddressType=natMappingIntAddressType, natSubscribersTable=natSubscribersTable, natGroupStatelessObjects=natGroupStatelessObjects, natMappingEntry=natMappingEntry, natBasicStatefulCompliance=natBasicStatefulCompliance, natSessionInTranslates=natSessionInTranslates, natFragmentsCompliance=natFragmentsCompliance, natNotifMappings=natNotifMappings, natInstanceIndex=natInstanceIndex, natSubscriberMappingCreations=natSubscriberMappingCreations, natLimitFragments=natLimitFragments, NatPoolingType=NatPoolingType, natLimitMappings=natLimitMappings, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natMapIntAddrEntry=natMapIntAddrEntry, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natSessionPrivateDstPort=natSessionPrivateDstPort, natAddrBindMapIndex=natAddrBindMapIndex, natMapIntAddrInt=natMapIntAddrInt, natPoolRealm=natPoolRealm, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, natBindDefIdleTimeout=natBindDefIdleTimeout, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natLimitSubscribers=natLimitSubscribers, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natProtocolTable=natProtocolTable, natPoolRangePoolIndex=natPoolRangePoolIndex, natAddrPortBindSessions=natAddrPortBindSessions, NatBindId=NatBindId, NatBindIdOrZero=NatBindIdOrZero, natMIBObjects=natMIBObjects, natLimitAddressMappings=natLimitAddressMappings, natAddrBindId=natAddrBindId, natPoolRangeEnd=natPoolRangeEnd, NatAddrMapId=NatAddrMapId, natMIBFullCompliance=natMIBFullCompliance, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natPoolWatermarkHigh=natPoolWatermarkHigh, VlanIndexOrZero=VlanIndexOrZero, SubscriberIdentifierType=SubscriberIdentifierType, natCountersEntry=natCountersEntry, natPoolRangeTable=natPoolRangeTable, natGroupSubscriberObjects=natGroupSubscriberObjects, natCounters=natCounters, natOutOfPortErrors=natOutOfPortErrors, natAddrMapEntryType=natAddrMapEntryType, natSessionTable=natSessionTable, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natAddressMappingRemovals=natAddressMappingRemovals, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natSessionUpTime=natSessionUpTime, natMappingExtAddressType=natMappingExtAddressType, natResourceErrors=natResourceErrors, natPoolRangeBegin=natPoolRangeBegin, natAddrMapCompliance=natAddrMapCompliance, NatProtocolMap=NatProtocolMap, natAddrPortBindProtocol=natAddrPortBindProtocol, natTcpDefIdleTimeout=natTcpDefIdleTimeout, natMappingExtPort=natMappingExtPort, natMappingProto=natMappingProto, natSessionPublicDstPort=natSessionPublicDstPort, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natMIBConformance=natMIBConformance, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natTcpDefNegTimeout=natTcpDefNegTimeout, natMappingExtAddress=natMappingExtAddress, natConfigGroup=natConfigGroup, natMappingCreations=natMappingCreations, natPoolRangeAllocations=natPoolRangeAllocations, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natPoolPortMin=natPoolPortMin, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natAddrMapLocalAddrType=natAddrMapLocalAddrType, natAddrBindOutTranslates=natAddrBindOutTranslates, natCountersTable=natCountersTable, natL4ProtocolEntry=natL4ProtocolEntry, natAddrMapAddrUsed=natAddrMapAddrUsed, natPoolWatermarkLow=natPoolWatermarkLow, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natSubscriberIdentifierType=natSubscriberIdentifierType, NatProtocolType=NatProtocolType, natAddrMapTable=natAddrMapTable, natAddrBindTable=natAddrBindTable, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natOtherDefIdleTimeout=natOtherDefIdleTimeout, natSessionAddrMapIndex=natSessionAddrMapIndex, natTranslationGroup=natTranslationGroup, ProtocolNumber=ProtocolNumber, natSessionDirection=natSessionDirection, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natProtocolInTranslates=natProtocolInTranslates, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natAddrMapDiscards=natAddrMapDiscards, natMappingRemovals=natMappingRemovals, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, natMapIntAddrExtType=natMapIntAddrExtType, natAddrMapInTranslates=natAddrMapInTranslates, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natPoolTable=natPoolTable, natSubscriberTranslations=natSubscriberTranslations, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natSessionPublicDstAddr=natSessionPublicDstAddr, natPoolPortMax=natPoolPortMax, natAddrMapOutTranslates=natAddrMapOutTranslates, natSubscriberResourceErrors=natSubscriberResourceErrors, natAddrMapStorageType=natAddrMapStorageType, natAddrPortBindId=natAddrPortBindId, natPoolRangeDeallocations=natPoolRangeDeallocations, NatTranslationEntity=NatTranslationEntity, natNotifThrottlingInterval=natNotifThrottlingInterval, natMappingAddressPooling=natMappingAddressPooling, natSessionMaxIdleTime=natSessionMaxIdleTime, NatPoolId=NatPoolId, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natStatsAddrMapGroup=natStatsAddrMapGroup, natSubscriberRealm=natSubscriberRealm, NatBindMode=NatBindMode, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, natSessionEntry=natSessionEntry, NatBehaviorType=NatBehaviorType, natSessionPublicSrcPort=natSessionPublicSrcPort, natProtocol=natProtocol, natInterfaceOutTranslates=natInterfaceOutTranslates, natSubsInterfaceIndex=natSubsInterfaceIndex, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natMapIntAddrIntType=natMapIntAddrIntType, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natMappingSubsIndex=natMappingSubsIndex, natSubscribersEntry=natSubscribersEntry, natSessionOutTranslates=natSessionOutTranslates, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natAddrPortBindTable=natAddrPortBindTable, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natDefTimeouts=natDefTimeouts, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natStatsInterfaceGroup=natStatsInterfaceGroup, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natMappingMapBehavior=natMappingMapBehavior, natMappingFilterBehavior=natMappingFilterBehavior, natMIBCompliances=natMIBCompliances, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natProtocolDiscards=natProtocolDiscards, natAddrMapTranslationEntity=natAddrMapTranslationEntity)
mibBuilder.exportSymbols("NAT-MIB", natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId, natMappingPool=natMappingPool, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natNotifAddrMappings=natNotifAddrMappings, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength)
