...
mibBuilder.exportSymbols("NAT-MIB", natCountersEntry=natCountersEntry, NatBindId=NatBindId, natAddressMappingCreations=natAddressMappingCreations, NatBindIdOrZero=NatBindIdOrZero, NatPoolId=NatPoolId, natAddrMapRowStatus=natAddrMapRowStatus, NatSessionId=NatSessionId, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natProtocolInTranslates=natProtocolInTranslates, natNotifSubscriberMappings=natNotifSubscriberMappings, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, natSessionUpTime=natSessionUpTime, natSessionInTranslates=natSessionInTranslates, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natGroupAddrMapObjects=natGroupAddrMapObjects, natFragmentsCompliance=natFragmentsCompliance, natSessionOutTranslates=natSessionOutTranslates, natAddrPortBindId=natAddrPortBindId, natMapObjects=natMapObjects, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natAddrPortBindType=natAddrPortBindType, natInterfaceServiceType=natInterfaceServiceType, natLimitsTable=natLimitsTable, natAddrPortBindSessions=natAddrPortBindSessions, natStatsInterfaceGroup=natStatsInterfaceGroup, natMapIntAddrExt=natMapIntAddrExt, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, NatTranslationEntity=NatTranslationEntity, NatBindMode=NatBindMode, natSessionAddrMapIndex=natSessionAddrMapIndex, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natInstanceIndex=natInstanceIndex, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natMappingExtPort=natMappingExtPort, natGroupSubscriberObjects=natGroupSubscriberObjects, natPoolTable=natPoolTable, NatProtocolMap=NatProtocolMap, natInstanceTable=natInstanceTable, natCountersTable=natCountersTable, natSessionDirection=natSessionDirection, natInterfaceOutTranslates=natInterfaceOutTranslates, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, PYSNMP_MODULE_ID=natMIB, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natMapIntAddrTable=natMapIntAddrTable, natSubscriberIntPrefix=natSubscriberIntPrefix, natSubscriberQuotaDrops=natSubscriberQuotaDrops, natSessionMaxIdleTime=natSessionMaxIdleTime, natMapIntAddrExtRealm=natMapIntAddrExtRealm, natSubscriberLimitMappings=natSubscriberLimitMappings, natPoolRangeType=natPoolRangeType, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natAddrBindEntry=natAddrBindEntry, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, ProtocolNumber=ProtocolNumber, natAddrMapStorageType=natAddrMapStorageType, natMappingIntRealm=natMappingIntRealm, natSessionIndex=natSessionIndex, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natGroupFragmentObjects=natGroupFragmentObjects, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natSessionPublicDstPort=natSessionPublicDstPort, natNotifThrottlingInterval=natNotifThrottlingInterval, natPoolEntry=natPoolEntry, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natBasicStatefulCompliance=natBasicStatefulCompliance, natPoolObjects=natPoolObjects, natSubscriberTranslations=natSubscriberTranslations, natPacketDiscard=natPacketDiscard, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natLimitFragments=natLimitFragments, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natPoolWatermarkHigh=natPoolWatermarkHigh, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, natInterfaceTable=natInterfaceTable, SubscriberIdentifierType=SubscriberIdentifierType, natLimitsEntry=natLimitsEntry, natMapIntAddrExtType=natMapIntAddrExtType, natMappingRemovals=natMappingRemovals, natGroupBasicNotifications=natGroupBasicNotifications, natQuotaDrops=natQuotaDrops, natSubscribersEntry=natSubscribersEntry, natAddrBindTable=natAddrBindTable, NatAddrMapId=NatAddrMapId, natConfigGroup=natConfigGroup, natCounters=natCounters, natMIBGroups=natMIBGroups, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natGroupStatefulObjects=natGroupStatefulObjects, natSessionPublicDstAddr=natSessionPublicDstAddr, natMappingIntAddress=natMappingIntAddress, natPoolRangeEntry=natPoolRangeEntry, natProtocolOutTranslates=natProtocolOutTranslates, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natPoolRangeBegin=natPoolRangeBegin, natNotifCtrl=natNotifCtrl, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natAddrMapDiscards=natAddrMapDiscards, natSessionPrivateDstPort=natSessionPrivateDstPort, natPoolRangePoolIndex=natPoolRangePoolIndex, natAddrMapTable=natAddrMapTable, natInstanceEntry=natInstanceEntry, natInterfaceDiscards=natInterfaceDiscards, natAddrPortBindTable=natAddrPortBindTable, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natDefTimeouts=natDefTimeouts, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, natSubscriberRealm=natSubscriberRealm, natAddrMapAddrUsed=natAddrMapAddrUsed, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId, natLimitAddressMappings=natLimitAddressMappings, natTcpDefIdleTimeout=natTcpDefIdleTimeout, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natSessionPrivateAddrType=natSessionPrivateAddrType, natAddrMapLocalAddrType=natAddrMapLocalAddrType, VlanIndexOrZero=VlanIndexOrZero, natL4ProtocolTable=natL4ProtocolTable, natPoolWatermarkLow=natPoolWatermarkLow, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natInterfaceRealm=natInterfaceRealm, natResourceErrors=natResourceErrors, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natPoolRangeTable=natPoolRangeTable, natAddrMapCompliance=natAddrMapCompliance, natMapIntAddrEntry=natMapIntAddrEntry, natAddrPortBindEntry=natAddrPortBindEntry, natMIBNotifications=natMIBNotifications, natNotifAddrMappings=natNotifAddrMappings, natAddrMapIndex=natAddrMapIndex, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natAddrMapInTranslates=natAddrMapInTranslates, natMIBCompliances=natMIBCompliances, natAddrMapProtocol=natAddrMapProtocol, natOutOfPortErrors=natOutOfPortErrors, natAddrMapEntry=natAddrMapEntry, natL4ProtocolEntry=natL4ProtocolEntry, natMappingEntry=natMappingEntry, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natOtherDefIdleTimeout=natOtherDefIdleTimeout, natSubscriberIndex=natSubscriberIndex, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natMappingCreations=natMappingCreations, natSubscriberMappingCreations=natSubscriberMappingCreations, natMappingProto=natMappingProto, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natAddrBindInTranslates=natAddrBindInTranslates, natAddressMappingRemovals=natAddressMappingRemovals, natMappingSubsIndex=natMappingSubsIndex, natSubscriberIdentifierType=natSubscriberIdentifierType, natMappingTable=natMappingTable, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natMappingMapBehavior=natMappingMapBehavior, natMappingExtRealm=natMappingExtRealm, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natPoolRangeAllocations=natPoolRangeAllocations, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natLimitSubscribers=natLimitSubscribers, natTranslationGroup=natTranslationGroup, natPoolIndex=natPoolIndex, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natProtocol=natProtocol, natSessionPrivateDstAddr=natSessionPrivateDstAddr, NatPoolingType=NatPoolingType, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natPoolRealm=natPoolRealm, natAddrBindLocalAddr=natAddrBindLocalAddr, natMIBNotificationGroup=natMIBNotificationGroup, natMappingExtAddress=natMappingExtAddress, natAddrPortBindProtocol=natAddrPortBindProtocol, natSessionPublicSrcPort=natSessionPublicSrcPort, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natMapIntAddrIntType=natMapIntAddrIntType, natInterfaceEntry=natInterfaceEntry, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, SubscriberIndex=SubscriberIndex, natCGNCompliance=natCGNCompliance, natInterfaceStorageType=natInterfaceStorageType, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natSessionPublicAddrType=natSessionPublicAddrType, natAddrBindOutTranslates=natAddrBindOutTranslates, natPoolRangeEnd=natPoolRangeEnd, natStatsAddrMapGroup=natStatsAddrMapGroup, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natAddrMapOutTranslates=natAddrMapOutTranslates, natMIB=natMIB, natBasicStatelessCompliance=natBasicStatelessCompliance, natSubsInterfaceIndex=natSubsInterfaceIndex, natAddrBindMapIndex=natAddrBindMapIndex, natMappingFilterBehavior=natMappingFilterBehavior, natTcpDefNegTimeout=natTcpDefNegTimeout, natAddrMapName=natAddrMapName, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natMappingIntPort=natMappingIntPort, natInstanceAlias=natInstanceAlias, natSubscribersTable=natSubscribersTable, natMIBConformance=natMIBConformance, natSessionTable=natSessionTable, natAddrMapEntryType=natAddrMapEntryType, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natGroupStatelessObjects=natGroupStatelessObjects, natProtocolTable=natProtocolTable, natStatsProtocolGroup=natStatsProtocolGroup, natL4ProtocolNumber=natL4ProtocolNumber, natProtocolDiscards=natProtocolDiscards, natInterfaceRowStatus=natInterfaceRowStatus, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natMappingExtAddressType=natMappingExtAddressType, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natNotifMappings=natNotifMappings, natPoolPortMax=natPoolPortMax, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natAddrBindType=natAddrBindType, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, NatBehaviorType=NatBehaviorType, natInterfaceInTranslates=natInterfaceInTranslates, NatAssociationType=NatAssociationType, natSessionEntry=natSessionEntry, natMappingAddressPooling=natMappingAddressPooling, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natMIBObjects=natMIBObjects, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natPoolRangeDeallocations=natPoolRangeDeallocations, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natL4ProtocolTranslations=natL4ProtocolTranslations, natSubscriberResourceErrors=natSubscriberResourceErrors, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natAddrBindSessions=natAddrBindSessions, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natProtocolEntry=natProtocolEntry, natLimitMappings=natLimitMappings, natMIBFullCompliance=natMIBFullCompliance, natBindDefIdleTimeout=natBindDefIdleTimeout, natTranslations=natTranslations, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natAddrBindId=natAddrBindId, natSubscribers=natSubscribers, natSessionProtocolType=natSessionProtocolType)
mibBuilder.exportSymbols("NAT-MIB", NatProtocolType=NatProtocolType, natMapIntAddrInt=natMapIntAddrInt, natMappingPool=natMappingPool, natMappingIntAddressType=natMappingIntAddressType, natPoolPortMin=natPoolPortMin)
