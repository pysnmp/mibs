...
mibBuilder.exportSymbols("NAT-MIB", natGroupBasicNotifications=natGroupBasicNotifications, natQuotaDrops=natQuotaDrops, natLimitsTable=natLimitsTable, natOutOfPortErrors=natOutOfPortErrors, natBasicStatefulCompliance=natBasicStatefulCompliance, natMappingExtAddressType=natMappingExtAddressType, natBasicStatelessCompliance=natBasicStatelessCompliance, natMappingTable=natMappingTable, natMappingIntAddressType=natMappingIntAddressType, natGroupAddrMapObjects=natGroupAddrMapObjects, natTcpDefIdleTimeout=natTcpDefIdleTimeout, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natMappingExtRealm=natMappingExtRealm, natInterfaceTable=natInterfaceTable, natStatsAddrMapGroup=natStatsAddrMapGroup, natPoolRealm=natPoolRealm, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natFragmentsCompliance=natFragmentsCompliance, NatBindMode=NatBindMode, natSessionPublicDstPort=natSessionPublicDstPort, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natMapIntAddrExt=natMapIntAddrExt, natSessionMaxIdleTime=natSessionMaxIdleTime, natSessionPublicAddrType=natSessionPublicAddrType, NatTranslationEntity=NatTranslationEntity, natInstanceIndex=natInstanceIndex, natTranslations=natTranslations, natSessionPublicDstAddr=natSessionPublicDstAddr, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natLimitSubscribers=natLimitSubscribers, natAddrMapInTranslates=natAddrMapInTranslates, natSessionPrivateDstPort=natSessionPrivateDstPort, natPoolPortMax=natPoolPortMax, natLimitMappings=natLimitMappings, natMappingPool=natMappingPool, natPoolWatermarkHigh=natPoolWatermarkHigh, natMappingSubsIndex=natMappingSubsIndex, natSessionTable=natSessionTable, natAddrMapCompliance=natAddrMapCompliance, natMappingMapBehavior=natMappingMapBehavior, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natInterfaceEntry=natInterfaceEntry, NatBindId=NatBindId, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, natAddrMapStorageType=natAddrMapStorageType, natLimitFragments=natLimitFragments, natSubscriberMappingCreations=natSubscriberMappingCreations, natSubscriberIntPrefix=natSubscriberIntPrefix, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natProtocolEntry=natProtocolEntry, natAddressMappingRemovals=natAddressMappingRemovals, natAddrBindLocalAddr=natAddrBindLocalAddr, natPoolRangeEnd=natPoolRangeEnd, natPoolRangeDeallocations=natPoolRangeDeallocations, natAddrMapTable=natAddrMapTable, natAddrMapLocalAddrType=natAddrMapLocalAddrType, natCountersEntry=natCountersEntry, natSessionPublicSrcPort=natSessionPublicSrcPort, natPoolRangeAllocations=natPoolRangeAllocations, natMappingAddressPooling=natMappingAddressPooling, natProtocol=natProtocol, natOtherDefIdleTimeout=natOtherDefIdleTimeout, natLimitAddressMappings=natLimitAddressMappings, natPoolPortMin=natPoolPortMin, natMappingIntPort=natMappingIntPort, natSubscriberRealm=natSubscriberRealm, natSubscribersEntry=natSubscribersEntry, natSessionPublicSrcAddr=natSessionPublicSrcAddr, NatProtocolType=NatProtocolType, natAddrPortBindId=natAddrPortBindId, natAddrPortBindType=natAddrPortBindType, natProtocolOutTranslates=natProtocolOutTranslates, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, natAddrMapDiscards=natAddrMapDiscards, natNotifThrottlingInterval=natNotifThrottlingInterval, natAddrMapOutTranslates=natAddrMapOutTranslates, natSessionInTranslates=natSessionInTranslates, natL4ProtocolTranslations=natL4ProtocolTranslations, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, SubscriberIdentifierType=SubscriberIdentifierType, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natProtocolTable=natProtocolTable, natSubscribers=natSubscribers, natStatsInterfaceGroup=natStatsInterfaceGroup, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natAddrMapRowStatus=natAddrMapRowStatus, natSubscribersTable=natSubscribersTable, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natMapObjects=natMapObjects, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natMappingEntry=natMappingEntry, natAddrMapIndex=natAddrMapIndex, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natSessionEntry=natSessionEntry, NatPoolingType=NatPoolingType, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natProtocolDiscards=natProtocolDiscards, NatAddrMapId=NatAddrMapId, natMIBConformance=natMIBConformance, natMIBCompliances=natMIBCompliances, natCounters=natCounters, natInterfaceDiscards=natInterfaceDiscards, natPoolRangeBegin=natPoolRangeBegin, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natCountersTable=natCountersTable, natMappingIntRealm=natMappingIntRealm, natAddressMappingCreations=natAddressMappingCreations, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, NatSessionId=NatSessionId, natMapIntAddrTable=natMapIntAddrTable, natMapIntAddrExtRealm=natMapIntAddrExtRealm, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natAddrBindTable=natAddrBindTable, natAddrMapAddrUsed=natAddrMapAddrUsed, natSessionProtocolType=natSessionProtocolType, NatAssociationType=NatAssociationType, natCGNCompliance=natCGNCompliance, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, natPoolEntry=natPoolEntry, natInstanceTable=natInstanceTable, natMapIntAddrIntType=natMapIntAddrIntType, natMappingExtPort=natMappingExtPort, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natAddrPortBindSessions=natAddrPortBindSessions, natSessionPrivateAddrType=natSessionPrivateAddrType, NatBehaviorType=NatBehaviorType, natGroupSubscriberObjects=natGroupSubscriberObjects, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natMIBNotificationGroup=natMIBNotificationGroup, natAddrBindOutTranslates=natAddrBindOutTranslates, natMIBFullCompliance=natMIBFullCompliance, natNotifMappings=natNotifMappings, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natL4ProtocolTable=natL4ProtocolTable, natPoolWatermarkLow=natPoolWatermarkLow, natSubsInterfaceIndex=natSubsInterfaceIndex, natGroupFragmentObjects=natGroupFragmentObjects, PYSNMP_MODULE_ID=natMIB, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natPoolRangeType=natPoolRangeType, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natInterfaceRealm=natInterfaceRealm, natSubscriberTranslations=natSubscriberTranslations, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natMappingRemovals=natMappingRemovals, natSubscriberResourceErrors=natSubscriberResourceErrors, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, natMappingCreations=natMappingCreations, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natPoolTable=natPoolTable, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natMapIntAddrEntry=natMapIntAddrEntry, natDefTimeouts=natDefTimeouts, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natMapIntAddrExtType=natMapIntAddrExtType, natAddrPortBindProtocol=natAddrPortBindProtocol, natInterfaceInTranslates=natInterfaceInTranslates, natMIBObjects=natMIBObjects, natAddrMapEntry=natAddrMapEntry, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natResourceErrors=natResourceErrors, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natL4ProtocolEntry=natL4ProtocolEntry, natSessionDirection=natSessionDirection, natMappingFilterBehavior=natMappingFilterBehavior, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natPoolRangeTable=natPoolRangeTable, natL4ProtocolNumber=natL4ProtocolNumber, natMIBGroups=natMIBGroups, ProtocolNumber=ProtocolNumber, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natPoolIndex=natPoolIndex, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natAddrMapEntryType=natAddrMapEntryType, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natSessionIndex=natSessionIndex, natSessionAddrMapIndex=natSessionAddrMapIndex, natPoolRangeEntry=natPoolRangeEntry, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natMappingProto=natMappingProto, natPacketDiscard=natPacketDiscard, natSessionUpTime=natSessionUpTime, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, NatBindIdOrZero=NatBindIdOrZero, natPoolRangePoolIndex=natPoolRangePoolIndex, natSessionOutTranslates=natSessionOutTranslates, natInterfaceServiceType=natInterfaceServiceType, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natSubscriberIndex=natSubscriberIndex, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natInterfaceOutTranslates=natInterfaceOutTranslates, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natBindDefIdleTimeout=natBindDefIdleTimeout, natAddrMapName=natAddrMapName, natAddrBindEntry=natAddrBindEntry, natMIBNotifications=natMIBNotifications, VlanIndexOrZero=VlanIndexOrZero, natInstanceEntry=natInstanceEntry, natAddrBindType=natAddrBindType, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natConfigGroup=natConfigGroup, natNotifAddrMappings=natNotifAddrMappings, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, NatProtocolMap=NatProtocolMap, natSubscriberQuotaDrops=natSubscriberQuotaDrops, natMIB=natMIB, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natInstanceAlias=natInstanceAlias, natSubscriberIdentifierType=natSubscriberIdentifierType, natTranslationGroup=natTranslationGroup, natGroupStatelessObjects=natGroupStatelessObjects, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natAddrBindInTranslates=natAddrBindInTranslates, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natInterfaceRowStatus=natInterfaceRowStatus, natAddrBindId=natAddrBindId, natAddrBindMapIndex=natAddrBindMapIndex, natNotifCtrl=natNotifCtrl, natMapIntAddrInt=natMapIntAddrInt, natAddrMapProtocol=natAddrMapProtocol, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natInterfaceStorageType=natInterfaceStorageType, natProtocolInTranslates=natProtocolInTranslates, natMappingIntAddress=natMappingIntAddress, natAddrBindSessions=natAddrBindSessions, natGroupStatefulObjects=natGroupStatefulObjects, natNotifSubscriberMappings=natNotifSubscriberMappings, natPoolObjects=natPoolObjects, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, NatPoolId=NatPoolId, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natMappingExtAddress=natMappingExtAddress, natLimitsEntry=natLimitsEntry, natStatsProtocolGroup=natStatsProtocolGroup, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natTcpDefNegTimeout=natTcpDefNegTimeout, natAddrPortBindTable=natAddrPortBindTable)
mibBuilder.exportSymbols("NAT-MIB", natSubscriberLimitMappings=natSubscriberLimitMappings, SubscriberIndex=SubscriberIndex, natAddrPortBindEntry=natAddrPortBindEntry, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier)
