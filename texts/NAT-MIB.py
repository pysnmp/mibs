...
mibBuilder.exportSymbols("NAT-MIB", natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natAddrBindMapIndex=natAddrBindMapIndex, natSessionPublicSrcPort=natSessionPublicSrcPort, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natMIBNotificationGroup=natMIBNotificationGroup, natPoolPortMax=natPoolPortMax, NatPoolingType=NatPoolingType, NatBindMode=NatBindMode, natAddrPortBindType=natAddrPortBindType, natInterfaceServiceType=natInterfaceServiceType, natLimitSubscribers=natLimitSubscribers, natMappingFilterBehavior=natMappingFilterBehavior, natProtocolOutTranslates=natProtocolOutTranslates, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natSubscriberMappingCreations=natSubscriberMappingCreations, natAddrBindLocalAddr=natAddrBindLocalAddr, natTranslationGroup=natTranslationGroup, natMappingSubsIndex=natMappingSubsIndex, natPoolRangeTable=natPoolRangeTable, natAddrPortBindId=natAddrPortBindId, natAddrMapEntryType=natAddrMapEntryType, natAddrBindTable=natAddrBindTable, natGroupSubscriberObjects=natGroupSubscriberObjects, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natMappingTable=natMappingTable, natPoolObjects=natPoolObjects, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, natSubsInterfaceIndex=natSubsInterfaceIndex, natAddrPortBindTable=natAddrPortBindTable, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natL4ProtocolNumber=natL4ProtocolNumber, natConfigGroup=natConfigGroup, natGroupStatefulObjects=natGroupStatefulObjects, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, NatSessionId=NatSessionId, natSessionPublicAddrType=natSessionPublicAddrType, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natPoolRealm=natPoolRealm, NatPoolId=NatPoolId, natSessionPrivateAddrType=natSessionPrivateAddrType, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natDefTimeouts=natDefTimeouts, natSessionIndex=natSessionIndex, natMIBObjects=natMIBObjects, natPoolRangeBegin=natPoolRangeBegin, natL4ProtocolTable=natL4ProtocolTable, natAddrMapOutTranslates=natAddrMapOutTranslates, natOutOfPortErrors=natOutOfPortErrors, natNotifThrottlingInterval=natNotifThrottlingInterval, natGroupBasicNotifications=natGroupBasicNotifications, natSubscriberIntPrefixType=natSubscriberIntPrefixType, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natBasicStatelessCompliance=natBasicStatelessCompliance, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natMappingPool=natMappingPool, natMapIntAddrEntry=natMapIntAddrEntry, natInterfaceTable=natInterfaceTable, natMIBConformance=natMIBConformance, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, natGroupAddrMapObjects=natGroupAddrMapObjects, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natMapIntAddrInt=natMapIntAddrInt, natInterfaceInTranslates=natInterfaceInTranslates, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natAddrBindLocalAddrType=natAddrBindLocalAddrType, NatAddrMapId=NatAddrMapId, natSessionOutTranslates=natSessionOutTranslates, natInterfaceRowStatus=natInterfaceRowStatus, natAddrMapProtocol=natAddrMapProtocol, natSubscriberIndex=natSubscriberIndex, SubscriberIndex=SubscriberIndex, natInterfaceStorageType=natInterfaceStorageType, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natCountersTable=natCountersTable, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natMappingIntAddress=natMappingIntAddress, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natAddrMapCompliance=natAddrMapCompliance, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, NatProtocolType=NatProtocolType, natProtocolDiscards=natProtocolDiscards, natSessionProtocolType=natSessionProtocolType, natLimitsTable=natLimitsTable, natMIBNotifications=natMIBNotifications, natProtocolTable=natProtocolTable, natSubscriberIntPrefix=natSubscriberIntPrefix, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natMapIntAddrIntType=natMapIntAddrIntType, natNotifMappings=natNotifMappings, natSessionEntry=natSessionEntry, natSubscriberLimitMappings=natSubscriberLimitMappings, natLimitsEntry=natLimitsEntry, natMapIntAddrExtRealm=natMapIntAddrExtRealm, natInterfaceEntry=natInterfaceEntry, natAddrBindOutTranslates=natAddrBindOutTranslates, natCGNCompliance=natCGNCompliance, natMappingIntPort=natMappingIntPort, natMappingExtAddress=natMappingExtAddress, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natSubscribersEntry=natSubscribersEntry, natAddrPortBindSessions=natAddrPortBindSessions, natMappingExtAddressType=natMappingExtAddressType, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natAddrMapDiscards=natAddrMapDiscards, natMapIntAddrTable=natMapIntAddrTable, natMappingExtRealm=natMappingExtRealm, natMIBCompliances=natMIBCompliances, natAddrMapStorageType=natAddrMapStorageType, NatBindIdOrZero=NatBindIdOrZero, natAddrMapRowStatus=natAddrMapRowStatus, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natStatsAddrMapGroup=natStatsAddrMapGroup, natSessionTable=natSessionTable, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natOtherDefIdleTimeout=natOtherDefIdleTimeout, VlanIndexOrZero=VlanIndexOrZero, natMapIntAddrExt=natMapIntAddrExt, natMIBFullCompliance=natMIBFullCompliance, natSubscriberRealm=natSubscriberRealm, natAddrPortBindProtocol=natAddrPortBindProtocol, natPoolWatermarkHigh=natPoolWatermarkHigh, natSessionPublicDstPort=natSessionPublicDstPort, natPacketDiscard=natPacketDiscard, natAddrMapLocalAddrType=natAddrMapLocalAddrType, natMappingIntAddressType=natMappingIntAddressType, NatProtocolMap=NatProtocolMap, natPoolWatermarkLow=natPoolWatermarkLow, natBasicStatefulCompliance=natBasicStatefulCompliance, natCountersEntry=natCountersEntry, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natAddrBindEntry=natAddrBindEntry, natAddrMapInTranslates=natAddrMapInTranslates, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natSessionInTranslates=natSessionInTranslates, natMIBGroups=natMIBGroups, natMapObjects=natMapObjects, natInstanceTable=natInstanceTable, natL4ProtocolEntry=natL4ProtocolEntry, natSubscribers=natSubscribers, natInterfaceRealm=natInterfaceRealm, natAddrMapName=natAddrMapName, natSessionPrivateDstPort=natSessionPrivateDstPort, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natSessionPublicDstAddr=natSessionPublicDstAddr, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natMappingProto=natMappingProto, natL4ProtocolTranslations=natL4ProtocolTranslations, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natSubscriberResourceErrors=natSubscriberResourceErrors, natStatsInterfaceGroup=natStatsInterfaceGroup, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, natFragmentsCompliance=natFragmentsCompliance, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natTcpDefIdleTimeout=natTcpDefIdleTimeout, natBindDefIdleTimeout=natBindDefIdleTimeout, natProtocolInTranslates=natProtocolInTranslates, natNotifAddrMappings=natNotifAddrMappings, natTcpDefNegTimeout=natTcpDefNegTimeout, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natMappingCreations=natMappingCreations, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natSubscriberTranslations=natSubscriberTranslations, natAddrBindInTranslates=natAddrBindInTranslates, NatAssociationType=NatAssociationType, natPoolIndex=natPoolIndex, natTranslations=natTranslations, natMapIntAddrExtType=natMapIntAddrExtType, natInterfaceOutTranslates=natInterfaceOutTranslates, natNotifSubscriberMappings=natNotifSubscriberMappings, natMappingAddressPooling=natMappingAddressPooling, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, NatBindId=NatBindId, natAddrMapTable=natAddrMapTable, natGroupStatelessObjects=natGroupStatelessObjects, natLimitAddressMappings=natLimitAddressMappings, natAddrBindId=natAddrBindId, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natSessionMaxIdleTime=natSessionMaxIdleTime, natProtocolEntry=natProtocolEntry, natAddrPortBindEntry=natAddrPortBindEntry, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId, natMappingExtPort=natMappingExtPort, natSessionDirection=natSessionDirection, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natStatsProtocolGroup=natStatsProtocolGroup, natPoolEntry=natPoolEntry, natSessionAddrMapIndex=natSessionAddrMapIndex, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natSessionUpTime=natSessionUpTime, natInstanceAlias=natInstanceAlias, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natAddressMappingCreations=natAddressMappingCreations, natPoolPortMin=natPoolPortMin, natGroupFragmentObjects=natGroupFragmentObjects, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natMappingMapBehavior=natMappingMapBehavior, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natPoolRangePoolIndex=natPoolRangePoolIndex, NatTranslationEntity=NatTranslationEntity, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natAddrMapAddrUsed=natAddrMapAddrUsed, natPoolRangeEntry=natPoolRangeEntry, natPoolRangeAllocations=natPoolRangeAllocations, natSubscriberQuotaDrops=natSubscriberQuotaDrops, ProtocolNumber=ProtocolNumber, natCounters=natCounters, natPoolRangeDeallocations=natPoolRangeDeallocations, natMappingRemovals=natMappingRemovals, natResourceErrors=natResourceErrors, natNotifCtrl=natNotifCtrl, natLimitFragments=natLimitFragments, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natSubscribersTable=natSubscribersTable, natLimitMappings=natLimitMappings, natAddrMapEntry=natAddrMapEntry, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natInterfaceDiscards=natInterfaceDiscards, natQuotaDrops=natQuotaDrops, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natPoolTable=natPoolTable, natSubscriberIdentifierType=natSubscriberIdentifierType, natUdpDefIdleTimeout=natUdpDefIdleTimeout, PYSNMP_MODULE_ID=natMIB, natAddressMappingRemovals=natAddressMappingRemovals, natAddrBindSessions=natAddrBindSessions, natPoolRangeEnd=natPoolRangeEnd, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, SubscriberIdentifierType=SubscriberIdentifierType, natMappingEntry=natMappingEntry, NatBehaviorType=NatBehaviorType, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natAddrMapIndex=natAddrMapIndex, natPoolRangeType=natPoolRangeType, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natInstanceIndex=natInstanceIndex, natProtocol=natProtocol)
mibBuilder.exportSymbols("NAT-MIB", natInstanceEntry=natInstanceEntry, natMappingIntRealm=natMappingIntRealm, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natAddrBindType=natAddrBindType, natMIB=natMIB)
