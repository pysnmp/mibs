...
mibBuilder.exportSymbols("NAT-MIB", natProtocolInTranslates=natProtocolInTranslates, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natLimitsTable=natLimitsTable, natAddrPortBindSessions=natAddrPortBindSessions, natAddressMappingRemovals=natAddressMappingRemovals, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natInstanceIndex=natInstanceIndex, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natPoolRangePoolIndex=natPoolRangePoolIndex, NatBindId=NatBindId, natMappingIntRealm=natMappingIntRealm, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natGroupSubscriberObjects=natGroupSubscriberObjects, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natMapIntAddrEntry=natMapIntAddrEntry, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natProtocolOutTranslates=natProtocolOutTranslates, natPoolWatermarkLow=natPoolWatermarkLow, natMappingRemovals=natMappingRemovals, NatProtocolType=NatProtocolType, natSessionPublicDstAddr=natSessionPublicDstAddr, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natGroupBasicNotifications=natGroupBasicNotifications, natSessionPublicDstPort=natSessionPublicDstPort, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natPoolRangeEnd=natPoolRangeEnd, natSessionPrivateAddrType=natSessionPrivateAddrType, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natGroupFragmentObjects=natGroupFragmentObjects, natAddrMapRowStatus=natAddrMapRowStatus, natLimitMappings=natLimitMappings, natMappingExtAddressType=natMappingExtAddressType, natStatsProtocolGroup=natStatsProtocolGroup, natMappingExtAddress=natMappingExtAddress, natMappingIntAddress=natMappingIntAddress, natAddrBindOutTranslates=natAddrBindOutTranslates, natAddrMapCompliance=natAddrMapCompliance, natInterfaceServiceType=natInterfaceServiceType, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, natAddrPortBindType=natAddrPortBindType, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natMappingProto=natMappingProto, natNotifThrottlingInterval=natNotifThrottlingInterval, natFragmentsCompliance=natFragmentsCompliance, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natMIB=natMIB, natAddrMapTable=natAddrMapTable, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natInterfaceDiscards=natInterfaceDiscards, natMappingIntPort=natMappingIntPort, natAddrPortBindTable=natAddrPortBindTable, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natProtocol=natProtocol, natPoolRangeEntry=natPoolRangeEntry, natBasicStatefulCompliance=natBasicStatefulCompliance, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natAddrMapProtocol=natAddrMapProtocol, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natMappingExtPort=natMappingExtPort, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natMIBCompliances=natMIBCompliances, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natAddrPortBindProtocol=natAddrPortBindProtocol, natPoolRangeType=natPoolRangeType, natMapIntAddrIntType=natMapIntAddrIntType, natPoolTable=natPoolTable, natSubscriberIdentifierType=natSubscriberIdentifierType, natPoolEntry=natPoolEntry, natStatsInterfaceGroup=natStatsInterfaceGroup, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natAddrMapDiscards=natAddrMapDiscards, natMappingTable=natMappingTable, natMapObjects=natMapObjects, natAddrMapStorageType=natAddrMapStorageType, natOutOfPortErrors=natOutOfPortErrors, natL4ProtocolNumber=natL4ProtocolNumber, NatPoolId=NatPoolId, natCountersEntry=natCountersEntry, natInterfaceRealm=natInterfaceRealm, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natMapIntAddrInt=natMapIntAddrInt, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, SubscriberIdentifierType=SubscriberIdentifierType, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natAddrBindGlobalAddr=natAddrBindGlobalAddr, NatPoolingType=NatPoolingType, natPoolRealm=natPoolRealm, natMappingMapBehavior=natMappingMapBehavior, natCGNCompliance=natCGNCompliance, natNotifMappings=natNotifMappings, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natAddrMapName=natAddrMapName, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natProtocolEntry=natProtocolEntry, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natMappingCreations=natMappingCreations, SubscriberIndex=SubscriberIndex, natQuotaDrops=natQuotaDrops, VlanIndexOrZero=VlanIndexOrZero, natSubscribers=natSubscribers, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, natAddrBindInTranslates=natAddrBindInTranslates, natInstanceEntry=natInstanceEntry, natTranslationGroup=natTranslationGroup, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natSubscriberTranslations=natSubscriberTranslations, natTcpDefNegTimeout=natTcpDefNegTimeout, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natSessionTable=natSessionTable, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natAddrMapEntry=natAddrMapEntry, natAddrBindEntry=natAddrBindEntry, natPoolIndex=natPoolIndex, natAddrBindTable=natAddrBindTable, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natInterfaceOutTranslates=natInterfaceOutTranslates, natInterfaceInTranslates=natInterfaceInTranslates, natAddrPortBindId=natAddrPortBindId, natMIBNotifications=natMIBNotifications, natSessionOutTranslates=natSessionOutTranslates, natInterfaceStorageType=natInterfaceStorageType, natInterfaceTable=natInterfaceTable, natInterfaceRowStatus=natInterfaceRowStatus, natAddrMapEntryType=natAddrMapEntryType, natSessionDirection=natSessionDirection, natGroupStatefulObjects=natGroupStatefulObjects, natMIBNotificationGroup=natMIBNotificationGroup, natAddrMapOutTranslates=natAddrMapOutTranslates, natSubscriberIntPrefix=natSubscriberIntPrefix, natMappingIntAddressType=natMappingIntAddressType, natTranslations=natTranslations, natAddrBindType=natAddrBindType, natPoolPortMax=natPoolPortMax, natMapIntAddrExt=natMapIntAddrExt, natPoolObjects=natPoolObjects, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natAddrPortBindEntry=natAddrPortBindEntry, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natMIBObjects=natMIBObjects, natBindDefIdleTimeout=natBindDefIdleTimeout, natSubscribersTable=natSubscribersTable, natSubscribersEntry=natSubscribersEntry, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natL4ProtocolTranslations=natL4ProtocolTranslations, NatBehaviorType=NatBehaviorType, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natInterfaceEntry=natInterfaceEntry, natAddressMappingCreations=natAddressMappingCreations, natSessionEntry=natSessionEntry, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natSessionIndex=natSessionIndex, natSubscriberIndex=natSubscriberIndex, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natAddrMapInTranslates=natAddrMapInTranslates, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natGroupStatelessObjects=natGroupStatelessObjects, natMappingAddressPooling=natMappingAddressPooling, natLimitsEntry=natLimitsEntry, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natProtocolDiscards=natProtocolDiscards, natSessionUpTime=natSessionUpTime, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, ProtocolNumber=ProtocolNumber, natLimitSubscribers=natLimitSubscribers, natMappingEntry=natMappingEntry, natMapIntAddrTable=natMapIntAddrTable, natDefTimeouts=natDefTimeouts, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natCounters=natCounters, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natPacketDiscard=natPacketDiscard, natCountersTable=natCountersTable, natTcpDefIdleTimeout=natTcpDefIdleTimeout, natMIBFullCompliance=natMIBFullCompliance, natMapIntAddrExtRealm=natMapIntAddrExtRealm, NatBindIdOrZero=NatBindIdOrZero, natMIBConformance=natMIBConformance, NatTranslationEntity=NatTranslationEntity, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natAddrBindMapIndex=natAddrBindMapIndex, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natLimitAddressMappings=natLimitAddressMappings, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natBasicStatelessCompliance=natBasicStatelessCompliance, natSessionAddrMapIndex=natSessionAddrMapIndex, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natConfigGroup=natConfigGroup, natLimitFragments=natLimitFragments, natAddrMapIndex=natAddrMapIndex, natMappingExtRealm=natMappingExtRealm, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natSessionPublicAddrType=natSessionPublicAddrType, natPoolPortMin=natPoolPortMin, natSubscriberLimitMappings=natSubscriberLimitMappings, natProtocolTable=natProtocolTable, natAddrBindSessions=natAddrBindSessions, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natL4ProtocolEntry=natL4ProtocolEntry, natPoolRangeAllocations=natPoolRangeAllocations, natMapIntAddrExtType=natMapIntAddrExtType, natInstanceTable=natInstanceTable, natMappingFilterBehavior=natMappingFilterBehavior, PYSNMP_MODULE_ID=natMIB, NatAssociationType=NatAssociationType, natOtherDefIdleTimeout=natOtherDefIdleTimeout, natL4ProtocolTable=natL4ProtocolTable, NatProtocolMap=NatProtocolMap, natNotifSubscriberMappings=natNotifSubscriberMappings, natSessionPublicSrcPort=natSessionPublicSrcPort, natInstanceAlias=natInstanceAlias, natPoolRangeTable=natPoolRangeTable, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natMIBGroups=natMIBGroups, natNotifAddrMappings=natNotifAddrMappings, natMappingSubsIndex=natMappingSubsIndex, natSessionMaxIdleTime=natSessionMaxIdleTime, natNotifCtrl=natNotifCtrl, natPoolWatermarkHigh=natPoolWatermarkHigh, natResourceErrors=natResourceErrors, natSessionProtocolType=natSessionProtocolType, natAddrMapLocalAddrType=natAddrMapLocalAddrType, natSubsInterfaceIndex=natSubsInterfaceIndex, NatBindMode=NatBindMode, NatAddrMapId=NatAddrMapId, natSessionPrivateDstPort=natSessionPrivateDstPort, natSessionInTranslates=natSessionInTranslates, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, natStatsAddrMapGroup=natStatsAddrMapGroup, natSubscriberQuotaDrops=natSubscriberQuotaDrops, natGroupAddrMapObjects=natGroupAddrMapObjects, natSubscriberResourceErrors=natSubscriberResourceErrors, natPoolRangeDeallocations=natPoolRangeDeallocations, natMappingPool=natMappingPool, natAddrMapAddrUsed=natAddrMapAddrUsed, natAddrBindId=natAddrBindId, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId)
mibBuilder.exportSymbols("NAT-MIB", natPoolRangeBegin=natPoolRangeBegin, natSubscriberRealm=natSubscriberRealm, natSubscriberMappingCreations=natSubscriberMappingCreations, NatSessionId=NatSessionId, natAddrBindLocalAddr=natAddrBindLocalAddr)
