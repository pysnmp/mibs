...
mibBuilder.exportSymbols("NAT-MIB", natLimitsEntry=natLimitsEntry, natInterfaceEntry=natInterfaceEntry, NatPoolId=NatPoolId, natBasicStatelessCompliance=natBasicStatelessCompliance, natMappingAddressPooling=natMappingAddressPooling, natStatsAddrMapGroup=natStatsAddrMapGroup, natSessionTable=natSessionTable, natPoolTable=natPoolTable, natAddrPortBindId=natAddrPortBindId, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, natOtherDefIdleTimeout=natOtherDefIdleTimeout, natAddrMapIndex=natAddrMapIndex, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, NatBehaviorType=NatBehaviorType, natPoolRangeEnd=natPoolRangeEnd, natMappingCreations=natMappingCreations, natSessionPrivateDstPort=natSessionPrivateDstPort, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natPoolRangeTable=natPoolRangeTable, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natSessionEntry=natSessionEntry, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, SubscriberIndex=SubscriberIndex, natMIBGroups=natMIBGroups, natMappingSubsIndex=natMappingSubsIndex, natMappingFilterBehavior=natMappingFilterBehavior, natMappingIntAddress=natMappingIntAddress, natAddrMapInTranslates=natAddrMapInTranslates, natProtocolTable=natProtocolTable, natPoolRangePoolIndex=natPoolRangePoolIndex, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natSessionIndex=natSessionIndex, natSessionPublicAddrType=natSessionPublicAddrType, natInstanceIndex=natInstanceIndex, natInterfaceStorageType=natInterfaceStorageType, natMappingIntAddressType=natMappingIntAddressType, natInterfaceServiceType=natInterfaceServiceType, SubscriberIdentifierType=SubscriberIdentifierType, natSubscriberLimitMappings=natSubscriberLimitMappings, natAddrMapEntry=natAddrMapEntry, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natPoolRealm=natPoolRealm, NatBindId=NatBindId, natProtocolOutTranslates=natProtocolOutTranslates, natSessionUpTime=natSessionUpTime, natPoolRangeEntry=natPoolRangeEntry, natLimitsTable=natLimitsTable, natGroupBasicNotifications=natGroupBasicNotifications, natL4ProtocolNumber=natL4ProtocolNumber, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natSubsInterfaceIndex=natSubsInterfaceIndex, natAddrMapTable=natAddrMapTable, natNotifMappings=natNotifMappings, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natSessionPublicDstPort=natSessionPublicDstPort, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natMappingExtAddress=natMappingExtAddress, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natAddrMapLocalAddrType=natAddrMapLocalAddrType, natMappingMapBehavior=natMappingMapBehavior, natProtocolEntry=natProtocolEntry, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, PYSNMP_MODULE_ID=natMIB, natSessionDirection=natSessionDirection, natMappingExtRealm=natMappingExtRealm, natAddrMapAddrUsed=natAddrMapAddrUsed, natInterfaceOutTranslates=natInterfaceOutTranslates, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natSubscriberIntPrefix=natSubscriberIntPrefix, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natLimitSubscribers=natLimitSubscribers, NatBindIdOrZero=NatBindIdOrZero, natSessionPrivateAddrType=natSessionPrivateAddrType, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, NatProtocolType=NatProtocolType, natAddrMapDiscards=natAddrMapDiscards, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natMappingIntRealm=natMappingIntRealm, natStatsProtocolGroup=natStatsProtocolGroup, natInstanceEntry=natInstanceEntry, natPoolRangeBegin=natPoolRangeBegin, natMappingExtAddressType=natMappingExtAddressType, natSubscriberIndex=natSubscriberIndex, natAddrPortBindSessions=natAddrPortBindSessions, natMappingEntry=natMappingEntry, natPoolPortMax=natPoolPortMax, natSubscriberMappingCreations=natSubscriberMappingCreations, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natGroupStatelessObjects=natGroupStatelessObjects, natNotifCtrl=natNotifCtrl, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natAddressMappingRemovals=natAddressMappingRemovals, natAddrMapRowStatus=natAddrMapRowStatus, natL4ProtocolTranslations=natL4ProtocolTranslations, natAddrMapCompliance=natAddrMapCompliance, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natMIB=natMIB, natNotifThrottlingInterval=natNotifThrottlingInterval, natAddrMapName=natAddrMapName, natDefTimeouts=natDefTimeouts, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natBasicStatefulCompliance=natBasicStatefulCompliance, natAddrPortBindTable=natAddrPortBindTable, natMappingRemovals=natMappingRemovals, natAddrBindTable=natAddrBindTable, natMIBNotificationGroup=natMIBNotificationGroup, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natMIBObjects=natMIBObjects, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natL4ProtocolTable=natL4ProtocolTable, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natPoolIndex=natPoolIndex, natResourceErrors=natResourceErrors, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natNotifSubscriberMappings=natNotifSubscriberMappings, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natMapIntAddrIntRealm=natMapIntAddrIntRealm, NatAddrMapId=NatAddrMapId, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natMapIntAddrTable=natMapIntAddrTable, natTranslations=natTranslations, natPacketDiscard=natPacketDiscard, natLimitFragments=natLimitFragments, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, NatSessionId=NatSessionId, natPoolWatermarkHigh=natPoolWatermarkHigh, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, natPoolEntry=natPoolEntry, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natAddrMapOutTranslates=natAddrMapOutTranslates, natMIBFullCompliance=natMIBFullCompliance, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natGroupAddrMapObjects=natGroupAddrMapObjects, natProtocol=natProtocol, natSessionInTranslates=natSessionInTranslates, natAddrBindId=natAddrBindId, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natSessionMaxIdleTime=natSessionMaxIdleTime, natMappingProto=natMappingProto, natTranslationGroup=natTranslationGroup, natAddrMapStorageType=natAddrMapStorageType, natNotifAddrMappings=natNotifAddrMappings, natSubscribersEntry=natSubscribersEntry, natStatsInterfaceGroup=natStatsInterfaceGroup, natMappingPool=natMappingPool, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natSubscriberQuotaDrops=natSubscriberQuotaDrops, natSessionOutTranslates=natSessionOutTranslates, natProtocolInTranslates=natProtocolInTranslates, natAddrBindMapIndex=natAddrBindMapIndex, natSessionAddrMapIndex=natSessionAddrMapIndex, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natSubscribersTable=natSubscribersTable, natGroupSubscriberObjects=natGroupSubscriberObjects, natSubscribers=natSubscribers, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natAddrPortBindProtocol=natAddrPortBindProtocol, natMIBNotifications=natMIBNotifications, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natMapIntAddrIntType=natMapIntAddrIntType, natMapIntAddrInt=natMapIntAddrInt, natSessionPublicDstAddr=natSessionPublicDstAddr, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natGroupFragmentObjects=natGroupFragmentObjects, natCountersEntry=natCountersEntry, natAddrBindType=natAddrBindType, natInterfaceInTranslates=natInterfaceInTranslates, natConfigGroup=natConfigGroup, natAddrBindEntry=natAddrBindEntry, natMappingExtPort=natMappingExtPort, VlanIndexOrZero=VlanIndexOrZero, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natMIBCompliances=natMIBCompliances, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natInstanceTable=natInstanceTable, natCountersTable=natCountersTable, natAddrPortBindEntry=natAddrPortBindEntry, natInterfaceRealm=natInterfaceRealm, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natCounters=natCounters, natInterfaceDiscards=natInterfaceDiscards, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natMapObjects=natMapObjects, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, natInterfaceTable=natInterfaceTable, natMapIntAddrExtType=natMapIntAddrExtType, natAddrBindSessions=natAddrBindSessions, natProtocolDiscards=natProtocolDiscards, ProtocolNumber=ProtocolNumber, natFragmentsCompliance=natFragmentsCompliance, natAddrPortBindType=natAddrPortBindType, NatPoolingType=NatPoolingType, natSessionProtocolType=natSessionProtocolType, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, natLimitAddressMappings=natLimitAddressMappings, natAddrMapProtocol=natAddrMapProtocol, natQuotaDrops=natQuotaDrops, natGroupStatefulObjects=natGroupStatefulObjects, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natBindDefIdleTimeout=natBindDefIdleTimeout, natMIBConformance=natMIBConformance, natAddressMappingCreations=natAddressMappingCreations, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natAddrBindLocalAddr=natAddrBindLocalAddr, natAddrBindInTranslates=natAddrBindInTranslates, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natMapIntAddrExt=natMapIntAddrExt, natPoolRangeType=natPoolRangeType, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natAddrBindOutTranslates=natAddrBindOutTranslates, natPoolObjects=natPoolObjects, natSubscriberRealm=natSubscriberRealm, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natMapIntAddrExtRealm=natMapIntAddrExtRealm, natCGNCompliance=natCGNCompliance, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natLimitMappings=natLimitMappings, natInstanceAlias=natInstanceAlias, natPoolRangeDeallocations=natPoolRangeDeallocations, natPoolPortMin=natPoolPortMin, NatBindMode=NatBindMode, natMappingTable=natMappingTable, natMappingIntPort=natMappingIntPort, natTcpDefNegTimeout=natTcpDefNegTimeout, natSubscriberIdentifierType=natSubscriberIdentifierType, natPoolRangeAllocations=natPoolRangeAllocations, natTcpDefIdleTimeout=natTcpDefIdleTimeout, natAddrMapEntryType=natAddrMapEntryType, natOutOfPortErrors=natOutOfPortErrors, NatProtocolMap=NatProtocolMap, natMapIntAddrEntry=natMapIntAddrEntry, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natSubscriberResourceErrors=natSubscriberResourceErrors, NatAssociationType=NatAssociationType, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natPoolWatermarkLow=natPoolWatermarkLow, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natSessionPublicSrcPort=natSessionPublicSrcPort, natInterfaceRowStatus=natInterfaceRowStatus)
mibBuilder.exportSymbols("NAT-MIB", natSubscriberTranslations=natSubscriberTranslations, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natL4ProtocolEntry=natL4ProtocolEntry, NatTranslationEntity=NatTranslationEntity, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId)
