...
mibBuilder.exportSymbols("NAT-MIB", natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natMappingExtPort=natMappingExtPort, natPoolRangeTable=natPoolRangeTable, natCGNCompliance=natCGNCompliance, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId, natMappingSubsIndex=natMappingSubsIndex, natAddrBindEntry=natAddrBindEntry, natAddrPortBindEntry=natAddrPortBindEntry, SubscriberIdentifierType=SubscriberIdentifierType, natMappingEntry=natMappingEntry, natMappingExtRealm=natMappingExtRealm, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natProtocolTable=natProtocolTable, natAddrBindTable=natAddrBindTable, NatBindMode=NatBindMode, natPoolObjects=natPoolObjects, natConfigGroup=natConfigGroup, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natGroupStatefulObjects=natGroupStatefulObjects, NatProtocolMap=NatProtocolMap, NatProtocolType=NatProtocolType, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natL4ProtocolTable=natL4ProtocolTable, natPoolRangeAllocations=natPoolRangeAllocations, ProtocolNumber=ProtocolNumber, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natAddrMapCompliance=natAddrMapCompliance, natAddrBindOutTranslates=natAddrBindOutTranslates, natAddrPortBindType=natAddrPortBindType, natSubscriberQuotaDrops=natSubscriberQuotaDrops, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natAddrBindId=natAddrBindId, natPoolRealm=natPoolRealm, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natStatsAddrMapGroup=natStatsAddrMapGroup, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natAddrPortBindSessions=natAddrPortBindSessions, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natSessionPublicDstPort=natSessionPublicDstPort, natProtocol=natProtocol, natSubscribers=natSubscribers, natInterfaceTable=natInterfaceTable, natMIBCompliances=natMIBCompliances, natAddrPortBindId=natAddrPortBindId, natQuotaDrops=natQuotaDrops, natTcpDefIdleTimeout=natTcpDefIdleTimeout, NatAssociationType=NatAssociationType, natMIBConformance=natMIBConformance, natNotifMappings=natNotifMappings, NatPoolId=NatPoolId, natMappingAddressPooling=natMappingAddressPooling, natSubscriberTranslations=natSubscriberTranslations, natBasicStatefulCompliance=natBasicStatefulCompliance, natAddrMapOutTranslates=natAddrMapOutTranslates, natNotifThrottlingInterval=natNotifThrottlingInterval, natMappingFilterBehavior=natMappingFilterBehavior, natMIBFullCompliance=natMIBFullCompliance, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natInterfaceInTranslates=natInterfaceInTranslates, natAddrMapAddrUsed=natAddrMapAddrUsed, natAddrBindSessions=natAddrBindSessions, natStatsProtocolGroup=natStatsProtocolGroup, natAddrPortBindTable=natAddrPortBindTable, natSubscriberIndex=natSubscriberIndex, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natMappingIntPort=natMappingIntPort, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natSubscriberMappingCreations=natSubscriberMappingCreations, natPoolRangeType=natPoolRangeType, natAddrMapEntry=natAddrMapEntry, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natFragmentsCompliance=natFragmentsCompliance, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natMapIntAddrEntry=natMapIntAddrEntry, natGroupStatelessObjects=natGroupStatelessObjects, natL4ProtocolNumber=natL4ProtocolNumber, natPoolWatermarkHigh=natPoolWatermarkHigh, natLimitMappings=natLimitMappings, natSubscriberIdentifierType=natSubscriberIdentifierType, natInterfaceOutTranslates=natInterfaceOutTranslates, natSubscriberIntPrefix=natSubscriberIntPrefix, natAddrMapLocalAddrType=natAddrMapLocalAddrType, natAddrBindLocalAddr=natAddrBindLocalAddr, natPoolRangePoolIndex=natPoolRangePoolIndex, natBasicStatelessCompliance=natBasicStatelessCompliance, natPacketDiscard=natPacketDiscard, natMappingProto=natMappingProto, natSessionIndex=natSessionIndex, natInstanceEntry=natInstanceEntry, natSubsInterfaceIndex=natSubsInterfaceIndex, natSubscriberResourceErrors=natSubscriberResourceErrors, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natGroupFragmentObjects=natGroupFragmentObjects, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natMappingPool=natMappingPool, natSubscribersEntry=natSubscribersEntry, natAddrMapEntryType=natAddrMapEntryType, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natMappingTable=natMappingTable, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natCountersTable=natCountersTable, natTranslationGroup=natTranslationGroup, natNotifSubscriberMappings=natNotifSubscriberMappings, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natAddrBindType=natAddrBindType, natSessionUpTime=natSessionUpTime, natGroupSubscriberObjects=natGroupSubscriberObjects, natMapIntAddrExtRealm=natMapIntAddrExtRealm, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natMappingIntAddressType=natMappingIntAddressType, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, natBindDefIdleTimeout=natBindDefIdleTimeout, natProtocolOutTranslates=natProtocolOutTranslates, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natStatsInterfaceGroup=natStatsInterfaceGroup, natMIBObjects=natMIBObjects, natOutOfPortErrors=natOutOfPortErrors, natPoolEntry=natPoolEntry, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natMapIntAddrIntType=natMapIntAddrIntType, natInterfaceRealm=natInterfaceRealm, natMappingRemovals=natMappingRemovals, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natInterfaceEntry=natInterfaceEntry, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natInstanceAlias=natInstanceAlias, natAddrMapRowStatus=natAddrMapRowStatus, natPoolRangeEntry=natPoolRangeEntry, natSubscribersTable=natSubscribersTable, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natL4ProtocolTranslations=natL4ProtocolTranslations, natInterfaceStorageType=natInterfaceStorageType, natInterfaceDiscards=natInterfaceDiscards, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, natSessionProtocolType=natSessionProtocolType, NatBehaviorType=NatBehaviorType, natInstanceIndex=natInstanceIndex, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natPoolWatermarkLow=natPoolWatermarkLow, natMapIntAddrInt=natMapIntAddrInt, natMappingExtAddress=natMappingExtAddress, natGroupAddrMapObjects=natGroupAddrMapObjects, natSessionAddrMapIndex=natSessionAddrMapIndex, natMapIntAddrExtType=natMapIntAddrExtType, natTranslations=natTranslations, natLimitFragments=natLimitFragments, natSessionDirection=natSessionDirection, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natGroupBasicNotifications=natGroupBasicNotifications, natPoolRangeDeallocations=natPoolRangeDeallocations, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natAddrMapStorageType=natAddrMapStorageType, natMIBNotifications=natMIBNotifications, natSubscriberLimitMappings=natSubscriberLimitMappings, natProtocolDiscards=natProtocolDiscards, natAddrPortBindProtocol=natAddrPortBindProtocol, natAddrMapIndex=natAddrMapIndex, natSessionPrivateDstPort=natSessionPrivateDstPort, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, natAddressMappingCreations=natAddressMappingCreations, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natAddrBindInTranslates=natAddrBindInTranslates, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natMappingIntAddress=natMappingIntAddress, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, natSessionMaxIdleTime=natSessionMaxIdleTime, natCountersEntry=natCountersEntry, natDefTimeouts=natDefTimeouts, natSessionPublicDstAddr=natSessionPublicDstAddr, NatBindIdOrZero=NatBindIdOrZero, natPoolTable=natPoolTable, NatAddrMapId=NatAddrMapId, natMapObjects=natMapObjects, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natSubscriberRealm=natSubscriberRealm, natSessionPublicSrcPort=natSessionPublicSrcPort, NatSessionId=NatSessionId, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natLimitAddressMappings=natLimitAddressMappings, natNotifCtrl=natNotifCtrl, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natAddressMappingRemovals=natAddressMappingRemovals, natSessionInTranslates=natSessionInTranslates, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natPoolRangeBegin=natPoolRangeBegin, NatTranslationEntity=NatTranslationEntity, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natInterfaceRowStatus=natInterfaceRowStatus, natMappingMapBehavior=natMappingMapBehavior, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natSessionPublicAddrType=natSessionPublicAddrType, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natPoolPortMin=natPoolPortMin, natPoolRangeEnd=natPoolRangeEnd, natMapIntAddrTable=natMapIntAddrTable, natAddrMapProtocol=natAddrMapProtocol, natAddrMapInTranslates=natAddrMapInTranslates, natAddrBindMapIndex=natAddrBindMapIndex, natInstanceTable=natInstanceTable, natSessionTable=natSessionTable, natSessionPrivateAddrType=natSessionPrivateAddrType, natOtherDefIdleTimeout=natOtherDefIdleTimeout, natL4ProtocolEntry=natL4ProtocolEntry, natPoolIndex=natPoolIndex, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natResourceErrors=natResourceErrors, natLimitsTable=natLimitsTable, natTcpDefNegTimeout=natTcpDefNegTimeout, natProtocolInTranslates=natProtocolInTranslates, natSessionOutTranslates=natSessionOutTranslates, natMIBNotificationGroup=natMIBNotificationGroup, natLimitsEntry=natLimitsEntry, natAddrMapDiscards=natAddrMapDiscards, natProtocolEntry=natProtocolEntry, natMappingIntRealm=natMappingIntRealm, VlanIndexOrZero=VlanIndexOrZero, NatBindId=NatBindId, natAddrMapTable=natAddrMapTable, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natMIBGroups=natMIBGroups, natSessionEntry=natSessionEntry, natCounters=natCounters, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natAddrMapName=natAddrMapName, natMappingExtAddressType=natMappingExtAddressType, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natMIB=natMIB, natInterfaceServiceType=natInterfaceServiceType, PYSNMP_MODULE_ID=natMIB, SubscriberIndex=SubscriberIndex, natNotifAddrMappings=natNotifAddrMappings, NatPoolingType=NatPoolingType)
mibBuilder.exportSymbols("NAT-MIB", natPoolPortMax=natPoolPortMax, natLimitSubscribers=natLimitSubscribers, natMappingCreations=natMappingCreations, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natMapIntAddrExt=natMapIntAddrExt)
