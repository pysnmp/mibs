...
mibBuilder.exportSymbols("NAT-MIB", natAddrBindOutTranslates=natAddrBindOutTranslates, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natPoolObjects=natPoolObjects, natAddressMappingCreations=natAddressMappingCreations, natNotifThrottlingInterval=natNotifThrottlingInterval, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natAddressMappingRemovals=natAddressMappingRemovals, natSubscriberIndex=natSubscriberIndex, natAddrMapName=natAddrMapName, natSubscriberIntPrefix=natSubscriberIntPrefix, NatTranslationEntity=NatTranslationEntity, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natAddrBindMapIndex=natAddrBindMapIndex, natMappingProto=natMappingProto, natMappingPool=natMappingPool, natBasicStatefulCompliance=natBasicStatefulCompliance, VlanIndexOrZero=VlanIndexOrZero, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natMappingEntry=natMappingEntry, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, NatBehaviorType=NatBehaviorType, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natInstanceAlias=natInstanceAlias, natSubscriberMappingCreations=natSubscriberMappingCreations, natSubscribers=natSubscribers, natProtocol=natProtocol, natMIBConformance=natMIBConformance, natProtocolEntry=natProtocolEntry, natInstanceEntry=natInstanceEntry, natInterfaceRealm=natInterfaceRealm, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, natPoolRangeBegin=natPoolRangeBegin, natDefTimeouts=natDefTimeouts, natTranslationGroup=natTranslationGroup, natGroupSubscriberObjects=natGroupSubscriberObjects, natSessionPublicDstAddr=natSessionPublicDstAddr, PYSNMP_MODULE_ID=natMIB, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, natMIBFullCompliance=natMIBFullCompliance, natNotifSubscriberMappings=natNotifSubscriberMappings, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natSessionMaxIdleTime=natSessionMaxIdleTime, natSessionTable=natSessionTable, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natSessionPrivateDstPort=natSessionPrivateDstPort, natAddrMapEntryType=natAddrMapEntryType, natPoolRangePoolIndex=natPoolRangePoolIndex, natMIBNotificationGroup=natMIBNotificationGroup, natResourceErrors=natResourceErrors, natSubscriberLimitMappings=natSubscriberLimitMappings, natAddrMapCompliance=natAddrMapCompliance, natSessionEntry=natSessionEntry, natSubscriberTranslations=natSubscriberTranslations, natPoolWatermarkLow=natPoolWatermarkLow, natAddrMapIndex=natAddrMapIndex, NatPoolId=NatPoolId, natGroupFragmentObjects=natGroupFragmentObjects, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, natMapIntAddrIntType=natMapIntAddrIntType, natAddrMapEntry=natAddrMapEntry, NatBindId=NatBindId, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natAddrBindLocalAddr=natAddrBindLocalAddr, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natPoolRangeTable=natPoolRangeTable, NatBindMode=NatBindMode, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natFragmentsCompliance=natFragmentsCompliance, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natTcpDefIdleTimeout=natTcpDefIdleTimeout, natLimitAddressMappings=natLimitAddressMappings, natPoolRangeType=natPoolRangeType, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natMapIntAddrInt=natMapIntAddrInt, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natPoolRangeAllocations=natPoolRangeAllocations, natAddrPortBindId=natAddrPortBindId, natSessionDirection=natSessionDirection, natConfigGroup=natConfigGroup, natAddrMapAddrUsed=natAddrMapAddrUsed, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natMappingExtAddress=natMappingExtAddress, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natProtocolDiscards=natProtocolDiscards, natInterfaceRowStatus=natInterfaceRowStatus, natLimitsTable=natLimitsTable, natTcpDefNegTimeout=natTcpDefNegTimeout, natMappingCreations=natMappingCreations, natAddrBindType=natAddrBindType, natL4ProtocolTable=natL4ProtocolTable, SubscriberIndex=SubscriberIndex, natSessionOutTranslates=natSessionOutTranslates, natMIBNotifications=natMIBNotifications, natSubscriberRealm=natSubscriberRealm, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natMapIntAddrExtRealm=natMapIntAddrExtRealm, natInterfaceTable=natInterfaceTable, natSessionInTranslates=natSessionInTranslates, natAddrPortBindProtocol=natAddrPortBindProtocol, natLimitMappings=natLimitMappings, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, NatAssociationType=NatAssociationType, natOtherDefIdleTimeout=natOtherDefIdleTimeout, natSubscribersTable=natSubscribersTable, natPoolWatermarkHigh=natPoolWatermarkHigh, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natMappingFilterBehavior=natMappingFilterBehavior, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natInterfaceEntry=natInterfaceEntry, natAddrBindSessions=natAddrBindSessions, natMIBObjects=natMIBObjects, natProtocolOutTranslates=natProtocolOutTranslates, natInstanceTable=natInstanceTable, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natAddrBindEntry=natAddrBindEntry, natAddrPortBindTable=natAddrPortBindTable, natSessionPublicSrcPort=natSessionPublicSrcPort, natQuotaDrops=natQuotaDrops, natMappingRemovals=natMappingRemovals, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natNotifMappings=natNotifMappings, natCountersTable=natCountersTable, natAddrMapStorageType=natAddrMapStorageType, natSubscriberResourceErrors=natSubscriberResourceErrors, natBindDefIdleTimeout=natBindDefIdleTimeout, natAddrMapLocalAddrType=natAddrMapLocalAddrType, natNotifCtrl=natNotifCtrl, natMapIntAddrEntry=natMapIntAddrEntry, natAddrMapOutTranslates=natAddrMapOutTranslates, NatProtocolMap=NatProtocolMap, natAddrMapTable=natAddrMapTable, NatPoolingType=NatPoolingType, natL4ProtocolNumber=natL4ProtocolNumber, natPoolIndex=natPoolIndex, natPoolRangeEntry=natPoolRangeEntry, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natMappingExtPort=natMappingExtPort, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natMapIntAddrTable=natMapIntAddrTable, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natAddrBindInTranslates=natAddrBindInTranslates, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natInterfaceStorageType=natInterfaceStorageType, NatAddrMapId=NatAddrMapId, natMappingIntAddress=natMappingIntAddress, natSubscribersEntry=natSubscribersEntry, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natLimitFragments=natLimitFragments, natSessionPublicAddrType=natSessionPublicAddrType, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natStatsAddrMapGroup=natStatsAddrMapGroup, ProtocolNumber=ProtocolNumber, NatBindIdOrZero=NatBindIdOrZero, natInstanceIndex=natInstanceIndex, natSessionPrivateAddrType=natSessionPrivateAddrType, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natStatsProtocolGroup=natStatsProtocolGroup, natAddrPortBindType=natAddrPortBindType, natMappingIntAddressType=natMappingIntAddressType, natPacketDiscard=natPacketDiscard, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natAddrPortBindEntry=natAddrPortBindEntry, natCounters=natCounters, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natInterfaceOutTranslates=natInterfaceOutTranslates, natMapIntAddrExt=natMapIntAddrExt, natSessionIndex=natSessionIndex, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId, natMapObjects=natMapObjects, natSubscriberIdentifierType=natSubscriberIdentifierType, natSubsInterfaceIndex=natSubsInterfaceIndex, natSessionProtocolType=natSessionProtocolType, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natAddrMapProtocol=natAddrMapProtocol, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natTranslations=natTranslations, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natMappingSubsIndex=natMappingSubsIndex, natAddrMapRowStatus=natAddrMapRowStatus, natMIBGroups=natMIBGroups, natL4ProtocolEntry=natL4ProtocolEntry, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natMappingExtAddressType=natMappingExtAddressType, natProtocolInTranslates=natProtocolInTranslates, NatSessionId=NatSessionId, natPoolPortMin=natPoolPortMin, natMappingMapBehavior=natMappingMapBehavior, natPoolRealm=natPoolRealm, natCGNCompliance=natCGNCompliance, natSessionAddrMapIndex=natSessionAddrMapIndex, natPoolRangeEnd=natPoolRangeEnd, natGroupBasicNotifications=natGroupBasicNotifications, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natAddrMapInTranslates=natAddrMapInTranslates, natBasicStatelessCompliance=natBasicStatelessCompliance, natMapIntAddrExtType=natMapIntAddrExtType, natProtocolTable=natProtocolTable, natLimitsEntry=natLimitsEntry, natPoolPortMax=natPoolPortMax, natPoolEntry=natPoolEntry, natMappingIntPort=natMappingIntPort, natMIBCompliances=natMIBCompliances, natAddrBindId=natAddrBindId, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natMappingIntRealm=natMappingIntRealm, natOutOfPortErrors=natOutOfPortErrors, natStatsInterfaceGroup=natStatsInterfaceGroup, natGroupStatelessObjects=natGroupStatelessObjects, natSessionPublicDstPort=natSessionPublicDstPort, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natMappingExtRealm=natMappingExtRealm, natGroupStatefulObjects=natGroupStatefulObjects, natAddrBindTable=natAddrBindTable, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natAddrMapDiscards=natAddrMapDiscards, natAddrPortBindSessions=natAddrPortBindSessions, natCountersEntry=natCountersEntry, natPoolRangeDeallocations=natPoolRangeDeallocations, natInterfaceServiceType=natInterfaceServiceType, natMappingAddressPooling=natMappingAddressPooling, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natLimitSubscribers=natLimitSubscribers, natSubscriberQuotaDrops=natSubscriberQuotaDrops, SubscriberIdentifierType=SubscriberIdentifierType, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, natInterfaceInTranslates=natInterfaceInTranslates, NatProtocolType=NatProtocolType, natPoolTable=natPoolTable, natGroupAddrMapObjects=natGroupAddrMapObjects, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natNotifAddrMappings=natNotifAddrMappings, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natMappingTable=natMappingTable, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh)
mibBuilder.exportSymbols("NAT-MIB", natInterfaceDiscards=natInterfaceDiscards, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natL4ProtocolTranslations=natL4ProtocolTranslations, natMIB=natMIB, natSessionUpTime=natSessionUpTime)
