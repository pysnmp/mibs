...
mibBuilder.exportSymbols("NAT-MIB", natNotifCtrl=natNotifCtrl, natStatsAddrMapGroup=natStatsAddrMapGroup, natInterfaceEntry=natInterfaceEntry, natSessionProtocolType=natSessionProtocolType, natSessionOutTranslates=natSessionOutTranslates, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natProtocolTable=natProtocolTable, natSessionPublicAddrType=natSessionPublicAddrType, natAddrMapEntry=natAddrMapEntry, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, ProtocolNumber=ProtocolNumber, natSessionUpTime=natSessionUpTime, natAddrMapOutTranslates=natAddrMapOutTranslates, natPoolObjects=natPoolObjects, natPoolRangeEnd=natPoolRangeEnd, natPoolWatermarkLow=natPoolWatermarkLow, natMappingIntAddressType=natMappingIntAddressType, natTranslationGroup=natTranslationGroup, natMappingExtRealm=natMappingExtRealm, natGroupStatefulObjects=natGroupStatefulObjects, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natAddrBindInTranslates=natAddrBindInTranslates, natSubscriberIndex=natSubscriberIndex, natPoolEntry=natPoolEntry, natMapIntAddrExtRealm=natMapIntAddrExtRealm, NatProtocolType=NatProtocolType, natPoolRangePoolIndex=natPoolRangePoolIndex, natMappingIntRealm=natMappingIntRealm, natPoolRangeAllocations=natPoolRangeAllocations, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natMappingMapBehavior=natMappingMapBehavior, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, natSubscriberLimitMappings=natSubscriberLimitMappings, natGroupFragmentObjects=natGroupFragmentObjects, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, natResourceErrors=natResourceErrors, natSessionPublicDstAddr=natSessionPublicDstAddr, natInstanceAlias=natInstanceAlias, natLimitMappings=natLimitMappings, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natInterfaceStorageType=natInterfaceStorageType, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natAddrBindSessions=natAddrBindSessions, natProtocolDiscards=natProtocolDiscards, NatPoolingType=NatPoolingType, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natAddrBindMapIndex=natAddrBindMapIndex, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natAddrPortBindSessions=natAddrPortBindSessions, natAddrBindType=natAddrBindType, natSessionDirection=natSessionDirection, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natMIBConformance=natMIBConformance, natLimitFragments=natLimitFragments, natMIB=natMIB, natSessionAddrMapIndex=natSessionAddrMapIndex, natAddrBindId=natAddrBindId, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natInstanceTable=natInstanceTable, natProtocolEntry=natProtocolEntry, natPoolRealm=natPoolRealm, NatBehaviorType=NatBehaviorType, natMappingFilterBehavior=natMappingFilterBehavior, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natBindDefIdleTimeout=natBindDefIdleTimeout, natNotifThrottlingInterval=natNotifThrottlingInterval, natMIBNotifications=natMIBNotifications, natMIBCompliances=natMIBCompliances, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, SubscriberIdentifierType=SubscriberIdentifierType, natMIBFullCompliance=natMIBFullCompliance, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natSubscriberQuotaDrops=natSubscriberQuotaDrops, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natMappingIntPort=natMappingIntPort, NatAssociationType=NatAssociationType, natMIBNotificationGroup=natMIBNotificationGroup, natAddrMapInTranslates=natAddrMapInTranslates, natMappingCreations=natMappingCreations, natL4ProtocolNumber=natL4ProtocolNumber, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natSubsInterfaceIndex=natSubsInterfaceIndex, natAddrMapName=natAddrMapName, natLimitsTable=natLimitsTable, natAddrBindLocalAddr=natAddrBindLocalAddr, natInterfaceOutTranslates=natInterfaceOutTranslates, natSessionIndex=natSessionIndex, natSessionTable=natSessionTable, natMapObjects=natMapObjects, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natMapIntAddrTable=natMapIntAddrTable, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, natQuotaDrops=natQuotaDrops, natMappingIntAddress=natMappingIntAddress, natSubscribers=natSubscribers, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natSubscriberIdentifierType=natSubscriberIdentifierType, natPoolTable=natPoolTable, natMappingAddressPooling=natMappingAddressPooling, natSubscriberIntPrefix=natSubscriberIntPrefix, natAddrPortBindEntry=natAddrPortBindEntry, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId, natInstanceEntry=natInstanceEntry, natSubscriberResourceErrors=natSubscriberResourceErrors, NatBindIdOrZero=NatBindIdOrZero, NatTranslationEntity=NatTranslationEntity, natPoolRangeBegin=natPoolRangeBegin, natAddrMapTable=natAddrMapTable, VlanIndexOrZero=VlanIndexOrZero, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, NatSessionId=NatSessionId, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natTcpDefNegTimeout=natTcpDefNegTimeout, natAddrBindEntry=natAddrBindEntry, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natMappingRemovals=natMappingRemovals, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natPoolWatermarkHigh=natPoolWatermarkHigh, natInterfaceRealm=natInterfaceRealm, natAddrMapLocalAddrType=natAddrMapLocalAddrType, natBasicStatelessCompliance=natBasicStatelessCompliance, natAddrMapCompliance=natAddrMapCompliance, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natAddrMapIndex=natAddrMapIndex, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natSessionPrivateDstPort=natSessionPrivateDstPort, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, NatBindId=NatBindId, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natAddrMapEntryType=natAddrMapEntryType, natGroupBasicNotifications=natGroupBasicNotifications, natMapIntAddrEntry=natMapIntAddrEntry, natDefTimeouts=natDefTimeouts, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natSubscribersEntry=natSubscribersEntry, natAddressMappingRemovals=natAddressMappingRemovals, natMappingEntry=natMappingEntry, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natProtocolOutTranslates=natProtocolOutTranslates, natLimitsEntry=natLimitsEntry, natTcpDefIdleTimeout=natTcpDefIdleTimeout, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natInterfaceTable=natInterfaceTable, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, natMIBGroups=natMIBGroups, natConfigGroup=natConfigGroup, natInterfaceInTranslates=natInterfaceInTranslates, natStatsProtocolGroup=natStatsProtocolGroup, natPoolPortMax=natPoolPortMax, natMapIntAddrInt=natMapIntAddrInt, NatProtocolMap=NatProtocolMap, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, natBasicStatefulCompliance=natBasicStatefulCompliance, natAddrPortBindTable=natAddrPortBindTable, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natNotifSubscriberMappings=natNotifSubscriberMappings, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natAddrBindTable=natAddrBindTable, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natLimitAddressMappings=natLimitAddressMappings, natMapIntAddrIntType=natMapIntAddrIntType, natNotifAddrMappings=natNotifAddrMappings, natL4ProtocolTable=natL4ProtocolTable, natLimitSubscribers=natLimitSubscribers, natStatsInterfaceGroup=natStatsInterfaceGroup, natL4ProtocolEntry=natL4ProtocolEntry, natInterfaceRowStatus=natInterfaceRowStatus, natOutOfPortErrors=natOutOfPortErrors, natMappingPool=natMappingPool, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, natL4ProtocolTranslations=natL4ProtocolTranslations, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natTranslations=natTranslations, natSubscriberRealm=natSubscriberRealm, natPoolRangeTable=natPoolRangeTable, natSessionMaxIdleTime=natSessionMaxIdleTime, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natPoolRangeDeallocations=natPoolRangeDeallocations, natSessionEntry=natSessionEntry, natMappingTable=natMappingTable, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natNotifMappings=natNotifMappings, natOtherDefIdleTimeout=natOtherDefIdleTimeout, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natAddrMapDiscards=natAddrMapDiscards, natSessionInTranslates=natSessionInTranslates, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, natMapIntAddrExt=natMapIntAddrExt, natProtocolInTranslates=natProtocolInTranslates, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natMappingSubsIndex=natMappingSubsIndex, natInterfaceDiscards=natInterfaceDiscards, natGroupStatelessObjects=natGroupStatelessObjects, natMappingExtPort=natMappingExtPort, natAddrMapLocalPortTo=natAddrMapLocalPortTo, NatPoolId=NatPoolId, natMappingExtAddressType=natMappingExtAddressType, natGroupSubscriberObjects=natGroupSubscriberObjects, natPoolIndex=natPoolIndex, natSubscriberMappingCreations=natSubscriberMappingCreations, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natAddrMapStorageType=natAddrMapStorageType, natAddrPortBindProtocol=natAddrPortBindProtocol, natSessionPublicDstPort=natSessionPublicDstPort, natMappingProto=natMappingProto, natCountersEntry=natCountersEntry, natAddrMapRowStatus=natAddrMapRowStatus, NatBindMode=NatBindMode, natCGNCompliance=natCGNCompliance, natFragmentsCompliance=natFragmentsCompliance, natGroupAddrMapObjects=natGroupAddrMapObjects, natAddrPortBindId=natAddrPortBindId, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natPoolRangeEntry=natPoolRangeEntry, NatAddrMapId=NatAddrMapId, natInterfaceServiceType=natInterfaceServiceType, natCountersTable=natCountersTable, SubscriberIndex=SubscriberIndex, natMapIntAddrExtType=natMapIntAddrExtType, natMappingExtAddress=natMappingExtAddress, natMIBObjects=natMIBObjects, natPoolPortMin=natPoolPortMin, natSubscriberTranslations=natSubscriberTranslations, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natSubscribersTable=natSubscribersTable, natSessionPublicSrcPort=natSessionPublicSrcPort, natPoolRangeType=natPoolRangeType, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natAddrBindOutTranslates=natAddrBindOutTranslates, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natAddrMapProtocol=natAddrMapProtocol, natPacketDiscard=natPacketDiscard, PYSNMP_MODULE_ID=natMIB, natAddrMapAddrUsed=natAddrMapAddrUsed, natCounters=natCounters, natAddrPortBindType=natAddrPortBindType)
mibBuilder.exportSymbols("NAT-MIB", natAddressMappingCreations=natAddressMappingCreations, natInstanceIndex=natInstanceIndex, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natProtocol=natProtocol, natSessionPrivateAddrType=natSessionPrivateAddrType)
