...
mibBuilder.exportSymbols("NAT-MIB", NatProtocolMap=NatProtocolMap, natAddrMapLocalAddrType=natAddrMapLocalAddrType, natMappingTable=natMappingTable, natPoolRangePoolIndex=natPoolRangePoolIndex, natMapObjects=natMapObjects, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, natBindDefIdleTimeout=natBindDefIdleTimeout, natAddrBindId=natAddrBindId, natSessionUpTime=natSessionUpTime, natDefTimeouts=natDefTimeouts, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natNotifMappings=natNotifMappings, natPoolRangeEnd=natPoolRangeEnd, natSessionAddrMapIndex=natSessionAddrMapIndex, natAddrMapOutTranslates=natAddrMapOutTranslates, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natTranslations=natTranslations, natSessionDirection=natSessionDirection, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natAddrBindSessions=natAddrBindSessions, natPoolWatermarkHigh=natPoolWatermarkHigh, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, NatBindMode=NatBindMode, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, NatProtocolType=NatProtocolType, natInterfaceOutTranslates=natInterfaceOutTranslates, natAddrMapRowStatus=natAddrMapRowStatus, natAddrBindMapIndex=natAddrBindMapIndex, natQuotaDrops=natQuotaDrops, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, natL4ProtocolTable=natL4ProtocolTable, natGroupBasicNotifications=natGroupBasicNotifications, NatBindId=NatBindId, natSessionPublicDstPort=natSessionPublicDstPort, natSessionPrivateDstPort=natSessionPrivateDstPort, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natMIBObjects=natMIBObjects, natLimitsTable=natLimitsTable, natPoolRangeAllocations=natPoolRangeAllocations, natInterfaceStorageType=natInterfaceStorageType, natNotifAddrMappings=natNotifAddrMappings, natSubscriberMappingCreations=natSubscriberMappingCreations, natAddrMapCompliance=natAddrMapCompliance, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natInstanceIndex=natInstanceIndex, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natMIBNotificationGroup=natMIBNotificationGroup, natMappingPool=natMappingPool, natMappingExtRealm=natMappingExtRealm, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natAddrMapEntryType=natAddrMapEntryType, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natInterfaceServiceType=natInterfaceServiceType, natMappingIntPort=natMappingIntPort, natSessionInTranslates=natSessionInTranslates, natGroupFragmentObjects=natGroupFragmentObjects, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natLimitAddressMappings=natLimitAddressMappings, natSubscribersEntry=natSubscribersEntry, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natMapIntAddrTable=natMapIntAddrTable, natPoolPortMin=natPoolPortMin, natNotifThrottlingInterval=natNotifThrottlingInterval, natMIBGroups=natMIBGroups, natMapIntAddrIntType=natMapIntAddrIntType, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, natMappingIntAddressType=natMappingIntAddressType, natMappingAddressPooling=natMappingAddressPooling, natSubscriberIntPrefix=natSubscriberIntPrefix, natPacketDiscard=natPacketDiscard, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natL4ProtocolNumber=natL4ProtocolNumber, natAddrMapStorageType=natAddrMapStorageType, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natMapIntAddrExt=natMapIntAddrExt, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId, natPoolRangeBegin=natPoolRangeBegin, natSubsInterfaceIndex=natSubsInterfaceIndex, natMapIntAddrEntry=natMapIntAddrEntry, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natSubscribers=natSubscribers, natInstanceTable=natInstanceTable, natSessionPrivateAddrType=natSessionPrivateAddrType, natAddrPortBindTable=natAddrPortBindTable, natLimitFragments=natLimitFragments, natAddrPortBindSessions=natAddrPortBindSessions, natPoolRangeEntry=natPoolRangeEntry, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natAddrPortBindId=natAddrPortBindId, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natAddrBindTable=natAddrBindTable, natMappingExtAddress=natMappingExtAddress, natMappingMapBehavior=natMappingMapBehavior, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natL4ProtocolTranslations=natL4ProtocolTranslations, natAddrBindEntry=natAddrBindEntry, natSubscriberLimitMappings=natSubscriberLimitMappings, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natLimitsEntry=natLimitsEntry, natInterfaceInTranslates=natInterfaceInTranslates, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natProtocolDiscards=natProtocolDiscards, natStatsProtocolGroup=natStatsProtocolGroup, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natGroupAddrMapObjects=natGroupAddrMapObjects, natPoolWatermarkLow=natPoolWatermarkLow, natPoolRangeDeallocations=natPoolRangeDeallocations, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natSubscriberTranslations=natSubscriberTranslations, natOtherDefIdleTimeout=natOtherDefIdleTimeout, natCGNCompliance=natCGNCompliance, natAddressMappingCreations=natAddressMappingCreations, natAddrBindOutTranslates=natAddrBindOutTranslates, natMappingIntRealm=natMappingIntRealm, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natMappingIntAddress=natMappingIntAddress, NatAddrMapId=NatAddrMapId, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natInterfaceDiscards=natInterfaceDiscards, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natAddrMapDiscards=natAddrMapDiscards, natConfigGroup=natConfigGroup, NatBehaviorType=NatBehaviorType, natInstanceAlias=natInstanceAlias, natCountersTable=natCountersTable, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natGroupStatefulObjects=natGroupStatefulObjects, natStatsInterfaceGroup=natStatsInterfaceGroup, natLimitMappings=natLimitMappings, natMIBCompliances=natMIBCompliances, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natSubscriberIdentifierType=natSubscriberIdentifierType, NatPoolingType=NatPoolingType, natSessionPrivateSrcPort=natSessionPrivateSrcPort, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natInterfaceRowStatus=natInterfaceRowStatus, natAddrMapName=natAddrMapName, natSubscriberQuotaDrops=natSubscriberQuotaDrops, natSubscribersTable=natSubscribersTable, natLimitSubscribers=natLimitSubscribers, natMapIntAddrExtType=natMapIntAddrExtType, natInstanceEntry=natInstanceEntry, natMappingEntry=natMappingEntry, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natSessionTable=natSessionTable, natMIBConformance=natMIBConformance, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, natSubscriberResourceErrors=natSubscriberResourceErrors, natNotifCtrl=natNotifCtrl, natAddrMapEntry=natAddrMapEntry, natAddrPortBindProtocol=natAddrPortBindProtocol, natSessionIndex=natSessionIndex, natPoolPortMax=natPoolPortMax, natTcpDefIdleTimeout=natTcpDefIdleTimeout, NatSessionId=NatSessionId, NatPoolId=NatPoolId, natAddrBindInTranslates=natAddrBindInTranslates, natMIB=natMIB, natAddrBindLocalAddr=natAddrBindLocalAddr, natProtocolInTranslates=natProtocolInTranslates, natMappingExtPort=natMappingExtPort, natSubscriberIndex=natSubscriberIndex, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natTcpDefNegTimeout=natTcpDefNegTimeout, natPoolRangeTable=natPoolRangeTable, NatTranslationEntity=NatTranslationEntity, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natAddrPortBindEntry=natAddrPortBindEntry, SubscriberIndex=SubscriberIndex, natStatsAddrMapGroup=natStatsAddrMapGroup, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natMappingRemovals=natMappingRemovals, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natGroupSubscriberObjects=natGroupSubscriberObjects, natSessionPublicDstAddr=natSessionPublicDstAddr, natProtocolEntry=natProtocolEntry, natPoolIndex=natPoolIndex, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, natSessionOutTranslates=natSessionOutTranslates, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natPoolTable=natPoolTable, natSubscriberRealm=natSubscriberRealm, ProtocolNumber=ProtocolNumber, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natAddrBindType=natAddrBindType, natFragmentsCompliance=natFragmentsCompliance, natCounters=natCounters, natSessionMaxIdleTime=natSessionMaxIdleTime, natCountersEntry=natCountersEntry, NatAssociationType=NatAssociationType, natSessionProtocolType=natSessionProtocolType, VlanIndexOrZero=VlanIndexOrZero, natProtocolOutTranslates=natProtocolOutTranslates, natProtocolTable=natProtocolTable, natSessionEntry=natSessionEntry, natPoolObjects=natPoolObjects, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natNotifSubscriberMappings=natNotifSubscriberMappings, natGroupStatelessObjects=natGroupStatelessObjects, natMapIntAddrInt=natMapIntAddrInt, natAddrMapIndex=natAddrMapIndex, natMappingProto=natMappingProto, natBasicStatelessCompliance=natBasicStatelessCompliance, natMappingFilterBehavior=natMappingFilterBehavior, PYSNMP_MODULE_ID=natMIB, natAddrMapAddrUsed=natAddrMapAddrUsed, natPoolRangeType=natPoolRangeType, natAddressMappingRemovals=natAddressMappingRemovals, NatBindIdOrZero=NatBindIdOrZero, natInterfaceRealm=natInterfaceRealm, natAddrMapTable=natAddrMapTable, natSessionPublicAddrType=natSessionPublicAddrType, natAddrPortBindType=natAddrPortBindType, natAddrMapProtocol=natAddrMapProtocol, natInterfaceEntry=natInterfaceEntry, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, natProtocol=natProtocol, natMIBFullCompliance=natMIBFullCompliance, natPoolRealm=natPoolRealm, natResourceErrors=natResourceErrors, natMappingSubsIndex=natMappingSubsIndex, natBasicStatefulCompliance=natBasicStatefulCompliance, SubscriberIdentifierType=SubscriberIdentifierType, natSessionPublicSrcPort=natSessionPublicSrcPort, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natAddrMapInTranslates=natAddrMapInTranslates, natOutOfPortErrors=natOutOfPortErrors, natMappingCreations=natMappingCreations, natMapIntAddrExtRealm=natMapIntAddrExtRealm, natTranslationGroup=natTranslationGroup, natInterfaceTable=natInterfaceTable, natPoolEntry=natPoolEntry, natL4ProtocolEntry=natL4ProtocolEntry, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors)
mibBuilder.exportSymbols("NAT-MIB", natSessionCurrentIdleTime=natSessionCurrentIdleTime, natMappingExtAddressType=natMappingExtAddressType, natMIBNotifications=natMIBNotifications, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType)
