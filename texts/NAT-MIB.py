...
mibBuilder.exportSymbols("NAT-MIB", natInstanceEntry=natInstanceEntry, natPoolIndex=natPoolIndex, natPoolRangeEnd=natPoolRangeEnd, natSessionUpTime=natSessionUpTime, natGroupAddrMapNotifications=natGroupAddrMapNotifications, natAddrPortBindProtocol=natAddrPortBindProtocol, natPacketDiscard=natPacketDiscard, natAddrPortBindGlobalPort=natAddrPortBindGlobalPort, natSessionPublicSrcPort=natSessionPublicSrcPort, natAddrBindLocalAddrType=natAddrBindLocalAddrType, natMapIntAddrIntRealm=natMapIntAddrIntRealm, natDefTimeouts=natDefTimeouts, natInterfaceRowStatus=natInterfaceRowStatus, NatProtocolType=NatProtocolType, natSessionInTranslates=natSessionInTranslates, natL4ProtocolTranslations=natL4ProtocolTranslations, natLimitsTable=natLimitsTable, natLimitAddressMappings=natLimitAddressMappings, natGroupAddrMapObjects=natGroupAddrMapObjects, natGroupFragmentObjects=natGroupFragmentObjects, natAddrMapDiscards=natAddrMapDiscards, natQuotaDrops=natQuotaDrops, natL4ProtocolOutOfPortErrors=natL4ProtocolOutOfPortErrors, natAddrMapInTranslates=natAddrMapInTranslates, natMappingIntAddressType=natMappingIntAddressType, natAddrMapIndex=natAddrMapIndex, natPoolWatermarkLow=natPoolWatermarkLow, NatBindIdOrZero=NatBindIdOrZero, natSubsInterfaceIdSubsIndex=natSubsInterfaceIdSubsIndex, natMapIntAddrSubsIndex=natMapIntAddrSubsIndex, natNotifMappings=natNotifMappings, natMappingFilterBehavior=natMappingFilterBehavior, natMIBObjects=natMIBObjects, natSubscribersTable=natSubscribersTable, NatProtocolMap=NatProtocolMap, natSubscriberIPEncapsIdType=natSubscriberIPEncapsIdType, natMapIntAddrEntry=natMapIntAddrEntry, natSubscriberLimitMappings=natSubscriberLimitMappings, natAddrMapProtocol=natAddrMapProtocol, natMappingIntRealm=natMappingIntRealm, NatSessionId=NatSessionId, natTcpDefNegTimeout=natTcpDefNegTimeout, natInterfaceInTranslates=natInterfaceInTranslates, natAddrMapLocalPortFrom=natAddrMapLocalPortFrom, natConfigGroup=natConfigGroup, natAddrMapLocalAddrFrom=natAddrMapLocalAddrFrom, natPoolRangeBegin=natPoolRangeBegin, natMapIntAddrInt=natMapIntAddrInt, natAddrMapGlobalPortTo=natAddrMapGlobalPortTo, natOtherDefIdleTimeout=natOtherDefIdleTimeout, NatAssociationType=NatAssociationType, natProtocolOutTranslates=natProtocolOutTranslates, natAddrMapName=natAddrMapName, natMappingProto=natMappingProto, natInterfaceRealm=natInterfaceRealm, natGroupSubscriberObjects=natGroupSubscriberObjects, natAddrPortBindLocalAddr=natAddrPortBindLocalAddr, PYSNMP_MODULE_ID=natMIB, natAddrBindId=natAddrBindId, natAddrPortBindTranslationEntity=natAddrPortBindTranslationEntity, natPoolRealm=natPoolRealm, natAddrMapAddrUsed=natAddrMapAddrUsed, natMIBNotificationGroup=natMIBNotificationGroup, NatPoolingType=NatPoolingType, natMIBFullCompliance=natMIBFullCompliance, natSubscriberVpnIdentifier=natSubscriberVpnIdentifier, natSessionPublicSrcAddr=natSessionPublicSrcAddr, natSessionCurrentIdleTime=natSessionCurrentIdleTime, natMIBConformance=natMIBConformance, natAddressMappingCreations=natAddressMappingCreations, natPoolRangeEntry=natPoolRangeEntry, natMapIntAddrExtRealm=natMapIntAddrExtRealm, natAddrMapLocalAddrType=natAddrMapLocalAddrType, NatBehaviorType=NatBehaviorType, natAddrPortBindInTranslates=natAddrPortBindInTranslates, natNotifThrottlingInterval=natNotifThrottlingInterval, natAddrBindTable=natAddrBindTable, natPoolPortMin=natPoolPortMin, natAddrPortBindGlobalAddr=natAddrPortBindGlobalAddr, natMapIntAddrExtType=natMapIntAddrExtType, natPoolPortMax=natPoolPortMax, natMappingIntPort=natMappingIntPort, natMappingSubsIndex=natMappingSubsIndex, natSessionDirection=natSessionDirection, natSubsInterfaceIdentifierTable=natSubsInterfaceIdentifierTable, natCounters=natCounters, natL4ProtocolEntry=natL4ProtocolEntry, natInstanceTable=natInstanceTable, natL4ProtocolResourceErrors=natL4ProtocolResourceErrors, natSessionPrivateSrcPort=natSessionPrivateSrcPort, VlanIndexOrZero=VlanIndexOrZero, natMIBReadOnlyCompliance=natMIBReadOnlyCompliance, natSessionPrivateDstPort=natSessionPrivateDstPort, natSubscriberOutOfPortErrors=natSubscriberOutOfPortErrors, natAddrBindGlobalAddrType=natAddrBindGlobalAddrType, natMapIntAddrIntType=natMapIntAddrIntType, natInstanceAlias=natInstanceAlias, natSubscriberIndex=natSubscriberIndex, natAddrMapLocalAddrTo=natAddrMapLocalAddrTo, natAddrMapLocalPortTo=natAddrMapLocalPortTo, natOutOfPortErrors=natOutOfPortErrors, natAddrMapNotifyThreshold=natAddrMapNotifyThreshold, natAddrPortBindEntry=natAddrPortBindEntry, natL4ProtocolMappingRemovals=natL4ProtocolMappingRemovals, natMIB=natMIB, NatBindMode=NatBindMode, natAddrMapStorageType=natAddrMapStorageType, natProtocolEntry=natProtocolEntry, NatPoolId=NatPoolId, natAddrPortBindCurrentIdleTime=natAddrPortBindCurrentIdleTime, natSessionOutTranslates=natSessionOutTranslates, natBasicStatelessCompliance=natBasicStatelessCompliance, natStatsAddrMapGroup=natStatsAddrMapGroup, NatBindId=NatBindId, natSessionPrivateAddrType=natSessionPrivateAddrType, natMIBGroups=natMIBGroups, natNotifSubscriberMappings=natNotifSubscriberMappings, natMappingExtRealm=natMappingExtRealm, natTranslations=natTranslations, natSubscriberIntPrefixLength=natSubscriberIntPrefixLength, natGroupStatefulObjects=natGroupStatefulObjects, natSubscriberIntPrefix=natSubscriberIntPrefix, natSessionIndex=natSessionIndex, natCountersTable=natCountersTable, natSessionEntry=natSessionEntry, natMIBCompliances=natMIBCompliances, natUdpDefIdleTimeout=natUdpDefIdleTimeout, natAddrBindType=natAddrBindType, natSessionPublicDstAddr=natSessionPublicDstAddr, natPoolRangeDeallocations=natPoolRangeDeallocations, natSubscriberVlanIdentifier=natSubscriberVlanIdentifier, natMIBNotifications=natMIBNotifications, natSubscriberMappingCreations=natSubscriberMappingCreations, SubscriberIndex=SubscriberIndex, natAddrBindNumberOfEntries=natAddrBindNumberOfEntries, natAddrMapTable=natAddrMapTable, natIcmpDefIdleTimeout=natIcmpDefIdleTimeout, natStatsProtocolGroup=natStatsProtocolGroup, natLimitsEntry=natLimitsEntry, natLimitFragments=natLimitFragments, natAddrPortBindMapIndex=natAddrPortBindMapIndex, natSessionPublicDstPort=natSessionPublicDstPort, natAddrMapTranslationEntity=natAddrMapTranslationEntity, natNotifAddrMappings=natNotifAddrMappings, natSessionPublicAddrType=natSessionPublicAddrType, natInterfaceTable=natInterfaceTable, natPoolRangeAllocations=natPoolRangeAllocations, natSessionPrivateDstEPBindMode=natSessionPrivateDstEPBindMode, natAddrPortBindGlobalAddrType=natAddrPortBindGlobalAddrType, natAddrMapGlobalAddrFrom=natAddrMapGlobalAddrFrom, natAddrMapCompliance=natAddrMapCompliance, natNotifCtrl=natNotifCtrl, natMapIntAddrTable=natMapIntAddrTable, natAddrPortBindTable=natAddrPortBindTable, natAddrPortBindLocalPort=natAddrPortBindLocalPort, natMapIntAddrExt=natMapIntAddrExt, natAddrPortBindId=natAddrPortBindId, natSessionPrivateDstAddr=natSessionPrivateDstAddr, natNotifPoolWatermarkHigh=natNotifPoolWatermarkHigh, natSessionPrivateSrcEPBindId=natSessionPrivateSrcEPBindId, natSubsInterfaceIndex=natSubsInterfaceIndex, natAddrBindEntry=natAddrBindEntry, natTcpDefIdleTimeout=natTcpDefIdleTimeout, natSubscriberIntPrefixType=natSubscriberIntPrefixType, natAddrMapRowStatus=natAddrMapRowStatus, natAddrBindGlobalAddr=natAddrBindGlobalAddr, natInterfaceDiscards=natInterfaceDiscards, natAddrBindTranslationEntity=natAddrBindTranslationEntity, natLimitSubscribers=natLimitSubscribers, natProtocolTable=natProtocolTable, natInstanceIndex=natInstanceIndex, SubscriberIdentifierType=SubscriberIdentifierType, natAddrBindMaxIdleTime=natAddrBindMaxIdleTime, natSubscribersEntry=natSubscribersEntry, natGroupBasicNotifications=natGroupBasicNotifications, natInterfaceOutTranslates=natInterfaceOutTranslates, natAddrPortBindOutTranslates=natAddrPortBindOutTranslates, natPoolObjects=natPoolObjects, natAddrBindOutTranslates=natAddrBindOutTranslates, natL4ProtocolQuotaDrops=natL4ProtocolQuotaDrops, natSessionProtocolType=natSessionProtocolType, natMapObjects=natMapObjects, natAddrBindLocalAddr=natAddrBindLocalAddr, natLimitMappings=natLimitMappings, natMappingAddressPooling=natMappingAddressPooling, natAddrMapEntry=natAddrMapEntry, natMappingsNotifyThreshold=natMappingsNotifyThreshold, natSubscriberQuotaDrops=natSubscriberQuotaDrops, natSubscriberMapNotifyThresh=natSubscriberMapNotifyThresh, natAddrBindCurrentIdleTime=natAddrBindCurrentIdleTime, natInterfaceServiceType=natInterfaceServiceType, natAddrMapGlobalAddrType=natAddrMapGlobalAddrType, natAddrPortBindMaxIdleTime=natAddrPortBindMaxIdleTime, natProtocol=natProtocol, natMappingIntAddress=natMappingIntAddress, natSubsInterfaceIdRowIndex=natSubsInterfaceIdRowIndex, natSubscriberRealm=natSubscriberRealm, natPoolWatermarkHigh=natPoolWatermarkHigh, natResourceErrors=natResourceErrors, natAddrMapEntryType=natAddrMapEntryType, natNotifPoolWatermarkLow=natNotifPoolWatermarkLow, natL4ProtocolMappingCreations=natL4ProtocolMappingCreations, natSubscriberResourceErrors=natSubscriberResourceErrors, ProtocolNumber=ProtocolNumber, natProtocolInTranslates=natProtocolInTranslates, NatTranslationEntity=NatTranslationEntity, natInterfaceStorageType=natInterfaceStorageType, natMappingTable=natMappingTable, natPoolEntry=natPoolEntry, natTranslationGroup=natTranslationGroup, natInterfaceEntry=natInterfaceEntry, natBasicStatefulCompliance=natBasicStatefulCompliance, natSessionMaxIdleTime=natSessionMaxIdleTime, natAddrPortBindLocalAddrType=natAddrPortBindLocalAddrType, natMappingExtAddress=natMappingExtAddress, natSubscriberTranslations=natSubscriberTranslations, natMappingCreations=natMappingCreations, natMappingPool=natMappingPool, natMappingExtAddressType=natMappingExtAddressType, natMappingRemovals=natMappingRemovals, natPoolRangeTable=natPoolRangeTable, natSubscriberIdentifierType=natSubscriberIdentifierType, natAddrBindMapIndex=natAddrBindMapIndex, natPoolRangePoolIndex=natPoolRangePoolIndex, natL4ProtocolTable=natL4ProtocolTable, natMappingMapBehavior=natMappingMapBehavior, natSubscriberMappingRemovals=natSubscriberMappingRemovals, natSubscriberIPEncapsIdAddr=natSubscriberIPEncapsIdAddr, SubsInterfaceIdRowIndex=SubsInterfaceIdRowIndex, natMappingEntry=natMappingEntry, natSessionPrivateDstEPBindId=natSessionPrivateDstEPBindId, natStatsInterfaceGroup=natStatsInterfaceGroup, natL4ProtocolNumber=natL4ProtocolNumber, NatAddrMapId=NatAddrMapId, natSessionPrivateSrcAddr=natSessionPrivateSrcAddr, natProtocolDiscards=natProtocolDiscards, natSessionTable=natSessionTable, natAddrMapGlobalPortFrom=natAddrMapGlobalPortFrom, natAddrPortBindNumberOfEntries=natAddrPortBindNumberOfEntries, natSubscribers=natSubscribers, natAddrBindSessions=natAddrBindSessions, natAddrMapGlobalAddrTo=natAddrMapGlobalAddrTo, natSessionAddrMapIndex=natSessionAddrMapIndex, natBindDefIdleTimeout=natBindDefIdleTimeout, natGroupStatelessObjects=natGroupStatelessObjects, natPoolTable=natPoolTable, natMappingExtPort=natMappingExtPort, natSessionPrivateSrcEPBindMode=natSessionPrivateSrcEPBindMode, natPoolRangeType=natPoolRangeType, natAddrBindInTranslates=natAddrBindInTranslates, natFragmentsCompliance=natFragmentsCompliance, natCGNCompliance=natCGNCompliance, natSubsInterfaceIdentifierEntry=natSubsInterfaceIdentifierEntry, natAddrMapOutTranslates=natAddrMapOutTranslates)
mibBuilder.exportSymbols("NAT-MIB", natAddrPortBindSessions=natAddrPortBindSessions, natGroupSubscriberNotifications=natGroupSubscriberNotifications, natCountersEntry=natCountersEntry, natAddressMappingRemovals=natAddressMappingRemovals, natAddrPortBindType=natAddrPortBindType)
