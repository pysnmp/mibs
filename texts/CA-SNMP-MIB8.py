...
Integer32, ObjectIdentity, Gauge32, Unsigned32, Counter64, ModuleIdentity, Bits, enterprises, IpAddress, iso, TimeTicks, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Counter32, Opaque = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "ObjectIdentity", "Gauge32", "Unsigned32", "Counter64", "ModuleIdentity", "Bits", "enterprises", "IpAddress", "iso", "TimeTicks", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Counter32", "Opaque")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", infOperStatus=infOperStatus, ctcpCurrEstab=ctcpCurrEstab, infIpv4Address=infIpv4Address, succLastSec5=succLastSec5, vsQosBodyHits=vsQosBodyHits, succLastDay=succLastDay, memory=memory, connectionsPerSec=connectionsPerSec, powerstate=powerstate, logNotificationsSent=logNotificationsSent, headRequests=headRequests, vsPort=vsPort, logBasic=logBasic, sdns=sdns, realStats=realStats, gpStatsTable=gpStatsTable, succPeakSec=succPeakSec, ctcpConnLocalAddress=ctcpConnLocalAddress, systemInfo=systemInfo, ctcpIndex=ctcpIndex, sslEntry=sslEntry, reqPeakDay=reqPeakDay, sysMemory=sysMemory, vsIpAddressType=vsIpAddressType, gpIndex=gpIndex, infOutBcastPkts=infOutBcastPkts, gpEntry=gpEntry, logMaxSeverity=logMaxSeverity, ctcpConnState=ctcpConnState, realPort=realPort, rsIndex=rsIndex, vsStatsEntry=vsStatsEntry, cacheSkip=cacheSkip, SyslogSeverity=SyslogSeverity, realServerID=realServerID, hcStatsEntry=hcStatsEntry, vsQosURLHits=vsQosURLHits, ipIndex=ipIndex, ctcpConnLocalAddrType=ctcpConnLocalAddrType, clusterVrrp=clusterVrrp, infIndex=infIndex, cacheHitRatio=cacheHitRatio, cacheStatus=cacheStatus, totalIPPktsIn=totalIPPktsIn, acceptedConns=acceptedConns, succLastMin=succLastMin, totalBytesSent=totalBytesSent, ipPktsIn=ipPktsIn, ipStatsTable=ipStatsTable, infOutBandwidth=infOutBandwidth, virtServerID=virtServerID, infEntry=infEntry, sslGeneral=sslGeneral, ctcpConnRemAddress=ctcpConnRemAddress, msgText=msgText, totalBytesRcvd=totalBytesRcvd, connsPerSec=connsPerSec, ctcpConnRemAddr=ctcpConnRemAddr, rsAvgRespTime=rsAvgRespTime, hcPort=hcPort, gpMetrics=gpMetrics, cputemp=cputemp, postRequests=postRequests, virtualServer=virtualServer, cacheMissWNewEntry=cacheMissWNewEntry, hcNumDowns=hcNumDowns, vsStaticHits=vsStaticHits, vsRCookieHits=vsRCookieHits, rsID=rsID, totalIPPktsOut=totalIPPktsOut, infTable=infTable, virtualIndex=virtualIndex, vsRegexHits=vsRegexHits, openSSLConns=openSSLConns, ctcpEstabResets=ctcpEstabResets, sysSwapUsed=sysSwapUsed, rsEntry=rsEntry, hcIndex=hcIndex, failLastSec=failLastSec, vsHashURLHits=vsHashURLHits, vsQosCookieHits=vsQosCookieHits, realIndex=realIndex, groupStats=groupStats, succLastSec=succLastSec, infInUnknownProtos=infInUnknownProtos, caSyslog=caSyslog, hostName=hostName, requestsReceived=requestsReceived, infIpv4AddressType=infIpv4AddressType, clusterId=clusterId, reqLastSec5=reqLastSec5, vrrpTable=vrrpTable, totalAcceptedConns=totalAcceptedConns, vrrp=vrrp, totalRequestedConns=totalRequestedConns, caTraps=caTraps, maxCluster=maxCluster, realServer=realServer, vsRedirectHits=vsRedirectHits, peakSentBytesPerSec=peakSentBytesPerSec, infAddress=infAddress, caSyslogTrap=caSyslogTrap, requestsPerSec=requestsPerSec, infOutNUcastPkts=infOutNUcastPkts, ipAddress=ipAddress, hcConnFail=hcConnFail, groupCount=groupCount, failLastSec5=failLastSec5, clusterVirState=clusterVirState, vrrpEntry=vrrpEntry, vsHostnameHits=vsHostnameHits, vsCacheHits=vsCacheHits, hitsReplyWPreFailed=hitsReplyWPreFailed, rsConnCnt=rsConnCnt, vsPerstntCookieHits=vsPerstntCookieHits, vsICookieHits=vsICookieHits, ctcpConnLocalPort=ctcpConnLocalPort, clusterNum=clusterNum, rsProtocol=rsProtocol, ctcpActiveOpens=ctcpActiveOpens, PYSNMP_MODULE_ID=arrayNetworks, vsIpAddress=vsIpAddress, infInErrors=infInErrors, rsTable=rsTable, ctcpInSegs=ctcpInSegs, groupID=groupID, hcAddr=hcAddr, succPeakHour=succPeakHour, responsesToHttps=responsesToHttps, vsCount=vsCount, performance=performance, infInOctets=infInOctets, totalOpenSSLConns=totalOpenSSLConns, gpTable=gpTable, reqLastMin=reqLastMin, rsStatsEntry=rsStatsEntry, clusterVirAuthType=clusterVirAuthType, groupCurCfg=groupCurCfg, realAddress=realAddress, sslTable=sslTable, infOutUcastPkts=infOutUcastPkts, slbMIB=slbMIB, hcStats=hcStats, hcStatsTable=hcStatsTable, requestsToHttps=requestsToHttps, reqPeakMin=reqPeakMin, syslogTrap=syslogTrap, sslMIB=sslMIB, logHistoryEntry=logHistoryEntry, healthCheck=healthCheck, totalReq=totalReq, missSess=missSess, ctcpConnEntry=ctcpConnEntry, rsIpAddressType=rsIpAddressType, compression=compression, clusterVirPreempt=clusterVirPreempt, succPeakSec5=succPeakSec5, ctcpConnRemPort=ctcpConnRemPort, infInBcastPkts=infInBcastPkts, ctcpRetransSegs=ctcpRetransSegs, ctcpOutRsts=ctcpOutRsts, rsIpAddress=rsIpAddress, ctcpConnRemAddrType=ctcpConnRemAddrType, failLastMin=failLastMin, ctcpAttemptFails=ctcpAttemptFails, rsCount=rsCount, vsBackupHits=vsBackupHits, reqPeakSec5=reqPeakSec5, requestsOnRegex=requestsOnRegex, ctcpConnLocalAddr=ctcpConnLocalAddr, hcStatus=hcStatus, infOutErrors=infOutErrors, licenseRemainingDays=licenseRemainingDays, slbStats=slbStats, infNumber=infNumber, rcvdBytesPerSec=rcvdBytesPerSec, vsQosClientPortHits=vsQosClientPortHits, rsStatus=rsStatus, infIpv6Address=infIpv6Address, monitor=monitor, infInMcastPkts=infInMcastPkts, virtualPort=virtualPort, hitRevalidate=hitRevalidate, infInUcastPkts=infInUcastPkts, infTotalInOctets=infTotalInOctets, clusterVirAddressType=clusterVirAddressType, vsTable=vsTable, totalFail=totalFail, ipBytesIn=ipBytesIn, sysSwapCapacity=sysSwapCapacity, logHistoryTable=logHistoryTable, failLastDay=failLastDay, requestedConns=requestedConns, gpTotalHits=gpTotalHits, failLastHour=failLastHour, clusterVirIndex=clusterVirIndex, getRequests=getRequests, infInDiscards=infInDiscards, ctcpInErrs=ctcpInErrs, vsID=vsID, startTime=startTime, realID=realID, hcNumUps=hcNumUps, ipAddrType=ipAddrType, resumedSess=resumedSess, totalIPBytesIn=totalIPBytesIn, revProxyCache=revProxyCache, hitsReply=hitsReply, infInNUcastPkts=infInNUcastPkts, vsProtocol=vsProtocol, clickTcp=clickTcp, vsURLHits=vsURLHits, totalSucc=totalSucc, ipStatsEntry=ipStatsEntry, ifTraffic=ifTraffic, logNotificationsEnabled=logNotificationsEnabled, virtualAddressType=virtualAddressType, logHistTableMaxLength=logHistTableMaxLength, infOutMcastPkts=infOutMcastPkts, virtualAddr=virtualAddr, vsIpAddr=vsIpAddr, currentTime=currentTime, cacheMissWRespNo=cacheMissWRespNo, vsDefaultHits=vsDefaultHits, infOutOctets=infOutOctets, ctcpOutSegs=ctcpOutSegs, clusterVirAddr=clusterVirAddr, hcAddress=hcAddress, vsHeaderHits=vsHeaderHits, reqLastSec=reqLastSec, clusterVirAddress=clusterVirAddress, severity=severity, rsMaxConn=rsMaxConn, activeTransac=activeTransac, hitsReplyWNotModified=hitsReplyWNotModified, ipAddr=ipAddr, caShutdown=caShutdown, gpID=gpID, hcCause=hcCause, serialNumber=serialNumber, hcName=hcName, realAddressType=realAddressType, reqLastHour=reqLastHour, vsQosNetworkHits=vsQosNetworkHits, vhostNum=vhostNum, hcConnAttempt=hcConnAttempt, vsMaxConn=vsMaxConn, vipStatus=vipStatus, fanspeed=fanspeed, clusterVirIfname=clusterVirIfname, sslStats=sslStats, Float=Float, clusterVirInterval=clusterVirInterval, virtualStats=virtualStats)
mibBuilder.exportSymbols("CA-SNMP-MIB8", rsPort=rsPort, logHistory=logHistory, gpStatsEntry=gpStatsEntry, requestsToUrl=requestsToUrl, rsCntOfReq=rsCntOfReq, vipStats=vipStats, arrayNetworks=arrayNetworks, ipPktsOut=ipPktsOut, vsIndex=vsIndex, infTotalOutOctets=infTotalOutOctets, virtualAddress=virtualAddress, totalIPBytesOut=totalIPBytesOut, peakRcvdBytesPerSec=peakRcvdBytesPerSec, succPeakMin=succPeakMin, caStart=caStart, infDescr=infDescr, succPeakDay=succPeakDay, clusterVirPriority=clusterVirPriority, reqLastDay=reqLastDay, realAddr=realAddr, hcConnSuccess=hcConnSuccess, reqPeakSec=reqPeakSec, vhostName=vhostName, ipBytesOut=ipBytesOut, vsPerstntURLHits=vsPerstntURLHits, infInBandwidth=infInBandwidth, ctcpConnTable=ctcpConnTable, resumableSess=resumableSess, rsStatsTable=rsStatsTable, clientEstabConn=clientEstabConn, infIpv6AddressType=infIpv6AddressType, succLastHour=succLastHour, groupIndex=groupIndex, cacheBasicStats=cacheBasicStats, vsEntry=vsEntry, slbGeneral=slbGeneral, purgeRequests=purgeRequests, ctcpPassiveOpens=ctcpPassiveOpens, cpuUtilization=cpuUtilization, realStatus=realStatus, hcRSCount=hcRSCount, rsIpAddr=rsIpAddr, vsStatsTable=vsStatsTable, responsesOnRegex=responsesOnRegex, serverEstabConn=serverEstabConn, cacheMissWNoncacheReq=cacheMissWNoncacheReq, vsConnCnt=vsConnCnt, hcAddressType=hcAddressType, clusterVirAuthPasswd=clusterVirAuthPasswd, rsTotalHits=rsTotalHits, reqPeakHour=reqPeakHour, sentBytesPerSec=sentBytesPerSec, index=index, sslIndex=sslIndex, sysMemoryUtilization=sysMemoryUtilization)
