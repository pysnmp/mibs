...
enterprises, Counter32, TimeTicks, Unsigned32, Gauge32, Bits, ModuleIdentity, Opaque, IpAddress, ObjectIdentity, MibIdentifier, Integer32, Counter64, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, iso = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Counter32", "TimeTicks", "Unsigned32", "Gauge32", "Bits", "ModuleIdentity", "Opaque", "IpAddress", "ObjectIdentity", "MibIdentifier", "Integer32", "Counter64", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsCount=vsCount, infOutBandwidth=infOutBandwidth, succLastMin=succLastMin, severity=severity, totalBytesRcvd=totalBytesRcvd, reqPeakSec5=reqPeakSec5, groupCount=groupCount, vsQosBodyHits=vsQosBodyHits, groupID=groupID, succPeakSec5=succPeakSec5, responsesToHttps=responsesToHttps, connectionsPerSec=connectionsPerSec, vsIpAddressType=vsIpAddressType, groupIndex=groupIndex, sslEntry=sslEntry, vrrpEntry=vrrpEntry, clusterVrrp=clusterVrrp, virtualAddr=virtualAddr, vsTable=vsTable, vsEntry=vsEntry, vsQosClientPortHits=vsQosClientPortHits, ipPktsOut=ipPktsOut, hcNumDowns=hcNumDowns, infInMcastPkts=infInMcastPkts, realStatus=realStatus, vsDefaultHits=vsDefaultHits, sslIndex=sslIndex, sysSwapCapacity=sysSwapCapacity, licenseRemainingDays=licenseRemainingDays, reqLastHour=reqLastHour, realStats=realStats, infTotalInOctets=infTotalInOctets, infOutBcastPkts=infOutBcastPkts, rsCount=rsCount, slbGeneral=slbGeneral, hcConnAttempt=hcConnAttempt, hcRSCount=hcRSCount, ctcpConnEntry=ctcpConnEntry, clusterVirState=clusterVirState, succLastSec=succLastSec, caStart=caStart, infOperStatus=infOperStatus, realServerID=realServerID, gpMetrics=gpMetrics, vsMaxConn=vsMaxConn, clusterVirAddressType=clusterVirAddressType, ctcpConnRemAddr=ctcpConnRemAddr, ctcpOutRsts=ctcpOutRsts, realPort=realPort, succPeakHour=succPeakHour, infInUnknownProtos=infInUnknownProtos, serialNumber=serialNumber, virtualAddress=virtualAddress, ctcpCurrEstab=ctcpCurrEstab, cacheMissWRespNo=cacheMissWRespNo, vsURLHits=vsURLHits, ipAddress=ipAddress, reqPeakDay=reqPeakDay, memory=memory, logHistory=logHistory, infIpv4Address=infIpv4Address, hitsReplyWPreFailed=hitsReplyWPreFailed, infDescr=infDescr, rsTable=rsTable, totalIPBytesOut=totalIPBytesOut, clusterId=clusterId, vsRCookieHits=vsRCookieHits, failLastMin=failLastMin, vsStaticHits=vsStaticHits, logBasic=logBasic, hcStatsTable=hcStatsTable, succPeakMin=succPeakMin, infIndex=infIndex, vsProtocol=vsProtocol, gpIndex=gpIndex, ctcpOutSegs=ctcpOutSegs, virtualStats=virtualStats, rsIpAddr=rsIpAddr, hitsReply=hitsReply, logHistoryEntry=logHistoryEntry, sysSwapUsed=sysSwapUsed, cacheBasicStats=cacheBasicStats, rcvdBytesPerSec=rcvdBytesPerSec, vsRegexHits=vsRegexHits, infOutMcastPkts=infOutMcastPkts, vsQosNetworkHits=vsQosNetworkHits, hcIndex=hcIndex, requestsToHttps=requestsToHttps, ctcpConnLocalPort=ctcpConnLocalPort, vsPerstntURLHits=vsPerstntURLHits, hcStatsEntry=hcStatsEntry, openSSLConns=openSSLConns, ctcpIndex=ctcpIndex, virtualPort=virtualPort, vsIpAddress=vsIpAddress, cpuUtilization=cpuUtilization, clusterVirIndex=clusterVirIndex, peakRcvdBytesPerSec=peakRcvdBytesPerSec, sentBytesPerSec=sentBytesPerSec, virtualIndex=virtualIndex, vsQosCookieHits=vsQosCookieHits, hostName=hostName, clickTcp=clickTcp, requestsToUrl=requestsToUrl, infInBandwidth=infInBandwidth, vsStatsTable=vsStatsTable, clusterVirAuthType=clusterVirAuthType, infIpv6AddressType=infIpv6AddressType, failLastSec=failLastSec, resumableSess=resumableSess, infTotalOutOctets=infTotalOutOctets, reqLastSec5=reqLastSec5, infIpv6Address=infIpv6Address, rsConnCnt=rsConnCnt, gpStatsEntry=gpStatsEntry, logHistTableMaxLength=logHistTableMaxLength, rsMaxConn=rsMaxConn, maxCluster=maxCluster, cacheMissWNoncacheReq=cacheMissWNoncacheReq, reqPeakHour=reqPeakHour, msgText=msgText, sslMIB=sslMIB, rsIpAddress=rsIpAddress, sysMemory=sysMemory, groupCurCfg=groupCurCfg, gpEntry=gpEntry, ipStatsTable=ipStatsTable, totalFail=totalFail, acceptedConns=acceptedConns, ctcpEstabResets=ctcpEstabResets, sslStats=sslStats, vhostName=vhostName, clusterVirAuthPasswd=clusterVirAuthPasswd, rsStatus=rsStatus, infOutNUcastPkts=infOutNUcastPkts, infInOctets=infInOctets, caTraps=caTraps, realAddress=realAddress, currentTime=currentTime, totalBytesSent=totalBytesSent, infInUcastPkts=infInUcastPkts, rsAvgRespTime=rsAvgRespTime, headRequests=headRequests, rsProtocol=rsProtocol, reqLastMin=reqLastMin, performance=performance, gpID=gpID, ipBytesOut=ipBytesOut, vrrpTable=vrrpTable, ipAddrType=ipAddrType, cacheStatus=cacheStatus, rsIndex=rsIndex, vhostNum=vhostNum, vsID=vsID, ipStatsEntry=ipStatsEntry, failLastDay=failLastDay, hcAddr=hcAddr, caSyslogTrap=caSyslogTrap, clusterVirPriority=clusterVirPriority, ctcpPassiveOpens=ctcpPassiveOpens, realAddr=realAddr, totalIPPktsIn=totalIPPktsIn, reqPeakMin=reqPeakMin, succPeakSec=succPeakSec, ipPktsIn=ipPktsIn, reqLastDay=reqLastDay, ipIndex=ipIndex, succLastSec5=succLastSec5, infAddress=infAddress, infInNUcastPkts=infInNUcastPkts, vsIndex=vsIndex, cputemp=cputemp, syslogTrap=syslogTrap, reqPeakSec=reqPeakSec, slbStats=slbStats, monitor=monitor, ctcpConnRemAddress=ctcpConnRemAddress, clusterVirIfname=clusterVirIfname, succLastHour=succLastHour, hcName=hcName, logNotificationsEnabled=logNotificationsEnabled, groupStats=groupStats, hcPort=hcPort, vsQosURLHits=vsQosURLHits, succLastDay=succLastDay, hcConnSuccess=hcConnSuccess, clusterNum=clusterNum, connsPerSec=connsPerSec, virtServerID=virtServerID, requestedConns=requestedConns, ctcpInErrs=ctcpInErrs, sysMemoryUtilization=sysMemoryUtilization, activeTransac=activeTransac, arrayNetworks=arrayNetworks, caShutdown=caShutdown, missSess=missSess, rsStatsTable=rsStatsTable, requestsPerSec=requestsPerSec, caSyslog=caSyslog, powerstate=powerstate, ctcpActiveOpens=ctcpActiveOpens, sdns=sdns, realIndex=realIndex, cacheHitRatio=cacheHitRatio, hitsReplyWNotModified=hitsReplyWNotModified, clientEstabConn=clientEstabConn, serverEstabConn=serverEstabConn, compression=compression, infInBcastPkts=infInBcastPkts, clusterVirPreempt=clusterVirPreempt, ctcpConnTable=ctcpConnTable, realID=realID, vsHostnameHits=vsHostnameHits, vsPerstntCookieHits=vsPerstntCookieHits, cacheMissWNewEntry=cacheMissWNewEntry, infTable=infTable, rsPort=rsPort, vsIpAddr=vsIpAddr, peakSentBytesPerSec=peakSentBytesPerSec, gpStatsTable=gpStatsTable, slbMIB=slbMIB, hcAddressType=hcAddressType, ctcpAttemptFails=ctcpAttemptFails, hcStatus=hcStatus, hcConnFail=hcConnFail, infEntry=infEntry, infInErrors=infInErrors, infOutOctets=infOutOctets, rsTotalHits=rsTotalHits, vsHeaderHits=vsHeaderHits, responsesOnRegex=responsesOnRegex, startTime=startTime, clusterVirInterval=clusterVirInterval, cacheSkip=cacheSkip, vsPort=vsPort, rsEntry=rsEntry, vsStatsEntry=vsStatsEntry, vrrp=vrrp, ctcpConnLocalAddress=ctcpConnLocalAddress, ctcpConnRemAddrType=ctcpConnRemAddrType, logNotificationsSent=logNotificationsSent, ipBytesIn=ipBytesIn, purgeRequests=purgeRequests, succPeakDay=succPeakDay, failLastSec5=failLastSec5, hitRevalidate=hitRevalidate, ctcpRetransSegs=ctcpRetransSegs, infNumber=infNumber, hcStats=hcStats, vipStats=vipStats, vsHashURLHits=vsHashURLHits, realServer=realServer, reqLastSec=reqLastSec, failLastHour=failLastHour)
mibBuilder.exportSymbols("CA-SNMP-MIB8", gpTotalHits=gpTotalHits, ifTraffic=ifTraffic, clusterVirAddr=clusterVirAddr, ctcpInSegs=ctcpInSegs, totalReq=totalReq, fanspeed=fanspeed, vsConnCnt=vsConnCnt, clusterVirAddress=clusterVirAddress, totalRequestedConns=totalRequestedConns, gpTable=gpTable, hcAddress=hcAddress, infIpv4AddressType=infIpv4AddressType, ctcpConnLocalAddr=ctcpConnLocalAddr, sslGeneral=sslGeneral, rsCntOfReq=rsCntOfReq, hcNumUps=hcNumUps, systemInfo=systemInfo, logHistoryTable=logHistoryTable, totalIPBytesIn=totalIPBytesIn, getRequests=getRequests, postRequests=postRequests, totalSucc=totalSucc, infInDiscards=infInDiscards, realAddressType=realAddressType, ipAddr=ipAddr, vsCacheHits=vsCacheHits, revProxyCache=revProxyCache, ctcpConnRemPort=ctcpConnRemPort, infOutErrors=infOutErrors, healthCheck=healthCheck, Float=Float, logMaxSeverity=logMaxSeverity, vsRedirectHits=vsRedirectHits, vsBackupHits=vsBackupHits, totalOpenSSLConns=totalOpenSSLConns, infOutUcastPkts=infOutUcastPkts, rsID=rsID, rsIpAddressType=rsIpAddressType, virtualServer=virtualServer, ctcpConnState=ctcpConnState, requestsReceived=requestsReceived, ctcpConnLocalAddrType=ctcpConnLocalAddrType, virtualAddressType=virtualAddressType, vipStatus=vipStatus, totalIPPktsOut=totalIPPktsOut, requestsOnRegex=requestsOnRegex, index=index, sslTable=sslTable, hcCause=hcCause, rsStatsEntry=rsStatsEntry, totalAcceptedConns=totalAcceptedConns, PYSNMP_MODULE_ID=arrayNetworks, resumedSess=resumedSess, vsICookieHits=vsICookieHits, SyslogSeverity=SyslogSeverity)
