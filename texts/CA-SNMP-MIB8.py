...
ModuleIdentity, Counter64, Opaque, Gauge32, IpAddress, NotificationType, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Unsigned32, enterprises, MibIdentifier, TimeTicks, ObjectIdentity, iso, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Counter64", "Opaque", "Gauge32", "IpAddress", "NotificationType", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Unsigned32", "enterprises", "MibIdentifier", "TimeTicks", "ObjectIdentity", "iso", "Integer32")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", gpTable=gpTable, ipAddrType=ipAddrType, infIpv4AddressType=infIpv4AddressType, cacheHitRatio=cacheHitRatio, getRequests=getRequests, vsPerstntCookieHits=vsPerstntCookieHits, activeTransac=activeTransac, ctcpInSegs=ctcpInSegs, acceptedConns=acceptedConns, rsCntOfReq=rsCntOfReq, ctcpOutSegs=ctcpOutSegs, vsQosClientPortHits=vsQosClientPortHits, infInUcastPkts=infInUcastPkts, succPeakSec=succPeakSec, vsPort=vsPort, infInUnknownProtos=infInUnknownProtos, failLastSec5=failLastSec5, failLastMin=failLastMin, vsID=vsID, severity=severity, hitRevalidate=hitRevalidate, sysMemoryUtilization=sysMemoryUtilization, ctcpConnLocalAddress=ctcpConnLocalAddress, virtServerID=virtServerID, rsStatus=rsStatus, ipPktsIn=ipPktsIn, hcStats=hcStats, failLastDay=failLastDay, hcStatus=hcStatus, ctcpAttemptFails=ctcpAttemptFails, vsBackupHits=vsBackupHits, vsCacheHits=vsCacheHits, rsMaxConn=rsMaxConn, clusterVirPriority=clusterVirPriority, infInNUcastPkts=infInNUcastPkts, clientEstabConn=clientEstabConn, systemInfo=systemInfo, hcAddressType=hcAddressType, groupCurCfg=groupCurCfg, realStatus=realStatus, realAddr=realAddr, resumedSess=resumedSess, caSyslog=caSyslog, ctcpConnState=ctcpConnState, cacheMissWNoncacheReq=cacheMissWNoncacheReq, clusterVirAddress=clusterVirAddress, gpID=gpID, reqPeakSec=reqPeakSec, openSSLConns=openSSLConns, SyslogSeverity=SyslogSeverity, vsIpAddressType=vsIpAddressType, totalIPBytesIn=totalIPBytesIn, virtualAddr=virtualAddr, reqLastMin=reqLastMin, vsIndex=vsIndex, virtualStats=virtualStats, realIndex=realIndex, reqLastDay=reqLastDay, sysMemory=sysMemory, syslogTrap=syslogTrap, sdns=sdns, vsQosCookieHits=vsQosCookieHits, caTraps=caTraps, failLastSec=failLastSec, infIpv4Address=infIpv4Address, logNotificationsEnabled=logNotificationsEnabled, ctcpConnRemAddr=ctcpConnRemAddr, vsQosNetworkHits=vsQosNetworkHits, connsPerSec=connsPerSec, logMaxSeverity=logMaxSeverity, peakSentBytesPerSec=peakSentBytesPerSec, rsStatsTable=rsStatsTable, vhostName=vhostName, totalOpenSSLConns=totalOpenSSLConns, currentTime=currentTime, ipStatsEntry=ipStatsEntry, succPeakDay=succPeakDay, vsRegexHits=vsRegexHits, ctcpConnRemPort=ctcpConnRemPort, ctcpInErrs=ctcpInErrs, virtualAddressType=virtualAddressType, rsTotalHits=rsTotalHits, resumableSess=resumableSess, virtualPort=virtualPort, vsICookieHits=vsICookieHits, vsStatsEntry=vsStatsEntry, reqPeakDay=reqPeakDay, gpStatsEntry=gpStatsEntry, caSyslogTrap=caSyslogTrap, succLastHour=succLastHour, totalAcceptedConns=totalAcceptedConns, gpStatsTable=gpStatsTable, realPort=realPort, ctcpIndex=ctcpIndex, infIpv6AddressType=infIpv6AddressType, groupCount=groupCount, virtualAddress=virtualAddress, vsPerstntURLHits=vsPerstntURLHits, infInBcastPkts=infInBcastPkts, cacheBasicStats=cacheBasicStats, rsID=rsID, Float=Float, reqPeakSec5=reqPeakSec5, cputemp=cputemp, gpTotalHits=gpTotalHits, index=index, rsIpAddressType=rsIpAddressType, requestsOnRegex=requestsOnRegex, infInBandwidth=infInBandwidth, sysSwapCapacity=sysSwapCapacity, virtualIndex=virtualIndex, ifTraffic=ifTraffic, vrrp=vrrp, failLastHour=failLastHour, clusterVirPreempt=clusterVirPreempt, maxCluster=maxCluster, infTotalInOctets=infTotalInOctets, infOutMcastPkts=infOutMcastPkts, vsURLHits=vsURLHits, responsesToHttps=responsesToHttps, vsEntry=vsEntry, gpEntry=gpEntry, connectionsPerSec=connectionsPerSec, vsTable=vsTable, totalSucc=totalSucc, slbGeneral=slbGeneral, vrrpEntry=vrrpEntry, vsRedirectHits=vsRedirectHits, infOutOctets=infOutOctets, infInErrors=infInErrors, vsHashURLHits=vsHashURLHits, purgeRequests=purgeRequests, rsPort=rsPort, cacheMissWNewEntry=cacheMissWNewEntry, realStats=realStats, hcPort=hcPort, hostName=hostName, hcIndex=hcIndex, infInMcastPkts=infInMcastPkts, memory=memory, ctcpConnTable=ctcpConnTable, healthCheck=healthCheck, totalReq=totalReq, reqPeakMin=reqPeakMin, responsesOnRegex=responsesOnRegex, groupStats=groupStats, reqLastHour=reqLastHour, sslIndex=sslIndex, rsIpAddr=rsIpAddr, infAddress=infAddress, succPeakMin=succPeakMin, arrayNetworks=arrayNetworks, reqLastSec5=reqLastSec5, infTable=infTable, rsTable=rsTable, hitsReply=hitsReply, totalIPBytesOut=totalIPBytesOut, ipAddress=ipAddress, rcvdBytesPerSec=rcvdBytesPerSec, sslGeneral=sslGeneral, realServerID=realServerID, ctcpActiveOpens=ctcpActiveOpens, rsProtocol=rsProtocol, hcCause=hcCause, vsIpAddr=vsIpAddr, hcRSCount=hcRSCount, clusterNum=clusterNum, serverEstabConn=serverEstabConn, clusterVirIndex=clusterVirIndex, succPeakHour=succPeakHour, totalFail=totalFail, hitsReplyWPreFailed=hitsReplyWPreFailed, hcConnSuccess=hcConnSuccess, ctcpConnLocalAddrType=ctcpConnLocalAddrType, clusterVirAddressType=clusterVirAddressType, sslTable=sslTable, infEntry=infEntry, slbMIB=slbMIB, rsIndex=rsIndex, infOperStatus=infOperStatus, succLastSec5=succLastSec5, missSess=missSess, cacheStatus=cacheStatus, reqLastSec=reqLastSec, realServer=realServer, clusterVirAddr=clusterVirAddr, ctcpConnEntry=ctcpConnEntry, powerstate=powerstate, infOutUcastPkts=infOutUcastPkts, infOutNUcastPkts=infOutNUcastPkts, infOutBcastPkts=infOutBcastPkts, infTotalOutOctets=infTotalOutOctets, revProxyCache=revProxyCache, vipStatus=vipStatus, totalIPPktsOut=totalIPPktsOut, ctcpConnLocalPort=ctcpConnLocalPort, ctcpPassiveOpens=ctcpPassiveOpens, clusterVirState=clusterVirState, clusterVirAuthType=clusterVirAuthType, vsIpAddress=vsIpAddress, clusterVirAuthPasswd=clusterVirAuthPasswd, vipStats=vipStats, cacheMissWRespNo=cacheMissWRespNo, startTime=startTime, totalIPPktsIn=totalIPPktsIn, requestsToUrl=requestsToUrl, vsQosBodyHits=vsQosBodyHits, cpuUtilization=cpuUtilization, hcNumUps=hcNumUps, infOutErrors=infOutErrors, ctcpEstabResets=ctcpEstabResets, logHistory=logHistory, logHistoryTable=logHistoryTable, rsConnCnt=rsConnCnt, ctcpCurrEstab=ctcpCurrEstab, sysSwapUsed=sysSwapUsed, slbStats=slbStats, requestsToHttps=requestsToHttps, vsStaticHits=vsStaticHits, gpIndex=gpIndex, vsDefaultHits=vsDefaultHits, vsMaxConn=vsMaxConn, monitor=monitor, vsHostnameHits=vsHostnameHits, realAddressType=realAddressType, rsEntry=rsEntry, ipStatsTable=ipStatsTable, vhostNum=vhostNum, sslMIB=sslMIB, hcConnFail=hcConnFail, infOutBandwidth=infOutBandwidth, succLastMin=succLastMin, realID=realID, PYSNMP_MODULE_ID=arrayNetworks, totalBytesRcvd=totalBytesRcvd, succLastDay=succLastDay, rsIpAddress=rsIpAddress, ctcpConnRemAddress=ctcpConnRemAddress, rsAvgRespTime=rsAvgRespTime, reqPeakHour=reqPeakHour, caShutdown=caShutdown, requestedConns=requestedConns, hcNumDowns=hcNumDowns, infDescr=infDescr, logHistoryEntry=logHistoryEntry, logNotificationsSent=logNotificationsSent, fanspeed=fanspeed, sslStats=sslStats, hcConnAttempt=hcConnAttempt, headRequests=headRequests, vsCount=vsCount, clusterId=clusterId, rsStatsEntry=rsStatsEntry, ipAddr=ipAddr)
mibBuilder.exportSymbols("CA-SNMP-MIB8", infIpv6Address=infIpv6Address, infInDiscards=infInDiscards, realAddress=realAddress, succLastSec=succLastSec, ctcpConnLocalAddr=ctcpConnLocalAddr, succPeakSec5=succPeakSec5, hcAddress=hcAddress, infNumber=infNumber, groupID=groupID, vsStatsTable=vsStatsTable, totalRequestedConns=totalRequestedConns, clusterVirIfname=clusterVirIfname, vsHeaderHits=vsHeaderHits, compression=compression, gpMetrics=gpMetrics, ipPktsOut=ipPktsOut, vrrpTable=vrrpTable, ctcpRetransSegs=ctcpRetransSegs, vsProtocol=vsProtocol, logHistTableMaxLength=logHistTableMaxLength, postRequests=postRequests, sslEntry=sslEntry, infIndex=infIndex, licenseRemainingDays=licenseRemainingDays, logBasic=logBasic, ctcpOutRsts=ctcpOutRsts, hcStatsTable=hcStatsTable, peakRcvdBytesPerSec=peakRcvdBytesPerSec, msgText=msgText, caStart=caStart, infInOctets=infInOctets, vsConnCnt=vsConnCnt, serialNumber=serialNumber, totalBytesSent=totalBytesSent, rsCount=rsCount, groupIndex=groupIndex, hcName=hcName, ipIndex=ipIndex, hcStatsEntry=hcStatsEntry, requestsReceived=requestsReceived, clickTcp=clickTcp, clusterVirInterval=clusterVirInterval, performance=performance, cacheSkip=cacheSkip, sentBytesPerSec=sentBytesPerSec, vsRCookieHits=vsRCookieHits, requestsPerSec=requestsPerSec, ipBytesOut=ipBytesOut, hcAddr=hcAddr, virtualServer=virtualServer, clusterVrrp=clusterVrrp, hitsReplyWNotModified=hitsReplyWNotModified, ipBytesIn=ipBytesIn, vsQosURLHits=vsQosURLHits, ctcpConnRemAddrType=ctcpConnRemAddrType)
