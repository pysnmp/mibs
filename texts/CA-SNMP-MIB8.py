...
ModuleIdentity, Integer32, Unsigned32, TimeTicks, Counter32, iso, ObjectIdentity, Bits, Gauge32, Counter64, enterprises, NotificationType, IpAddress, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Opaque = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Integer32", "Unsigned32", "TimeTicks", "Counter32", "iso", "ObjectIdentity", "Bits", "Gauge32", "Counter64", "enterprises", "NotificationType", "IpAddress", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Opaque")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsBackupHits=vsBackupHits, hcNumUps=hcNumUps, vsID=vsID, clusterVirInterval=clusterVirInterval, succLastMin=succLastMin, revProxyCache=revProxyCache, gpMetrics=gpMetrics, infIpv4Address=infIpv4Address, sysSwapUsed=sysSwapUsed, clusterId=clusterId, hcStats=hcStats, ctcpConnLocalPort=ctcpConnLocalPort, realServerID=realServerID, responsesOnRegex=responsesOnRegex, gpEntry=gpEntry, infIndex=infIndex, infDescr=infDescr, virtualStats=virtualStats, gpIndex=gpIndex, failLastDay=failLastDay, reqPeakHour=reqPeakHour, performance=performance, realPort=realPort, responsesToHttps=responsesToHttps, slbMIB=slbMIB, virtualAddr=virtualAddr, hcAddress=hcAddress, infOutErrors=infOutErrors, memory=memory, hcIndex=hcIndex, vsEntry=vsEntry, vsStaticHits=vsStaticHits, vsMaxConn=vsMaxConn, groupIndex=groupIndex, hcConnAttempt=hcConnAttempt, reqPeakSec5=reqPeakSec5, severity=severity, ctcpConnRemPort=ctcpConnRemPort, ipStatsTable=ipStatsTable, rsIpAddr=rsIpAddr, clusterVirAuthType=clusterVirAuthType, rsIndex=rsIndex, clusterVirAddressType=clusterVirAddressType, groupCurCfg=groupCurCfg, currentTime=currentTime, logNotificationsEnabled=logNotificationsEnabled, hcRSCount=hcRSCount, cacheMissWRespNo=cacheMissWRespNo, cacheStatus=cacheStatus, rsID=rsID, totalBytesRcvd=totalBytesRcvd, succLastDay=succLastDay, sysSwapCapacity=sysSwapCapacity, resumableSess=resumableSess, infInUnknownProtos=infInUnknownProtos, realIndex=realIndex, vsDefaultHits=vsDefaultHits, index=index, ifTraffic=ifTraffic, vsQosURLHits=vsQosURLHits, ctcpConnLocalAddress=ctcpConnLocalAddress, vsURLHits=vsURLHits, rsCntOfReq=rsCntOfReq, realStats=realStats, requestsReceived=requestsReceived, cacheMissWNewEntry=cacheMissWNewEntry, activeTransac=activeTransac, sslGeneral=sslGeneral, sdns=sdns, systemInfo=systemInfo, rsIpAddress=rsIpAddress, gpStatsTable=gpStatsTable, virtualAddress=virtualAddress, failLastSec5=failLastSec5, requestsToHttps=requestsToHttps, ipPktsOut=ipPktsOut, acceptedConns=acceptedConns, clusterVirState=clusterVirState, vsProtocol=vsProtocol, cacheBasicStats=cacheBasicStats, groupStats=groupStats, logBasic=logBasic, vhostName=vhostName, serialNumber=serialNumber, clusterVirAddr=clusterVirAddr, infInBandwidth=infInBandwidth, purgeRequests=purgeRequests, ctcpAttemptFails=ctcpAttemptFails, ctcpConnRemAddress=ctcpConnRemAddress, vsICookieHits=vsICookieHits, fanspeed=fanspeed, infInMcastPkts=infInMcastPkts, vsIndex=vsIndex, ipPktsIn=ipPktsIn, peakSentBytesPerSec=peakSentBytesPerSec, infIpv4AddressType=infIpv4AddressType, msgText=msgText, gpTable=gpTable, hcConnSuccess=hcConnSuccess, hcAddressType=hcAddressType, succLastSec5=succLastSec5, rsProtocol=rsProtocol, hcStatsEntry=hcStatsEntry, ctcpActiveOpens=ctcpActiveOpens, ctcpOutRsts=ctcpOutRsts, caSyslog=caSyslog, vrrp=vrrp, hitsReply=hitsReply, sysMemory=sysMemory, reqLastMin=reqLastMin, vsTable=vsTable, totalIPBytesOut=totalIPBytesOut, failLastMin=failLastMin, rcvdBytesPerSec=rcvdBytesPerSec, requestedConns=requestedConns, vhostNum=vhostNum, clientEstabConn=clientEstabConn, realStatus=realStatus, hitRevalidate=hitRevalidate, infOutBcastPkts=infOutBcastPkts, rsMaxConn=rsMaxConn, totalIPBytesIn=totalIPBytesIn, groupCount=groupCount, ctcpIndex=ctcpIndex, totalIPPktsIn=totalIPPktsIn, ipStatsEntry=ipStatsEntry, Float=Float, slbStats=slbStats, rsPort=rsPort, requestsToUrl=requestsToUrl, infInErrors=infInErrors, vsHashURLHits=vsHashURLHits, vsIpAddressType=vsIpAddressType, realAddr=realAddr, ctcpConnLocalAddrType=ctcpConnLocalAddrType, virtServerID=virtServerID, infTotalOutOctets=infTotalOutOctets, clusterVirAddress=clusterVirAddress, rsEntry=rsEntry, syslogTrap=syslogTrap, cacheHitRatio=cacheHitRatio, infTable=infTable, ipBytesIn=ipBytesIn, ipAddress=ipAddress, succPeakSec=succPeakSec, ctcpConnRemAddrType=ctcpConnRemAddrType, hitsReplyWPreFailed=hitsReplyWPreFailed, sslTable=sslTable, rsCount=rsCount, ctcpInSegs=ctcpInSegs, vsIpAddr=vsIpAddr, realServer=realServer, vsQosBodyHits=vsQosBodyHits, hcAddr=hcAddr, sysMemoryUtilization=sysMemoryUtilization, infInUcastPkts=infInUcastPkts, hcConnFail=hcConnFail, hitsReplyWNotModified=hitsReplyWNotModified, resumedSess=resumedSess, totalIPPktsOut=totalIPPktsOut, gpStatsEntry=gpStatsEntry, reqLastSec5=reqLastSec5, sslMIB=sslMIB, infInOctets=infInOctets, hcName=hcName, vsPerstntCookieHits=vsPerstntCookieHits, rsIpAddressType=rsIpAddressType, infEntry=infEntry, openSSLConns=openSSLConns, succPeakHour=succPeakHour, vipStatus=vipStatus, vsIpAddress=vsIpAddress, getRequests=getRequests, headRequests=headRequests, caShutdown=caShutdown, infOutBandwidth=infOutBandwidth, clusterVirIndex=clusterVirIndex, rsTotalHits=rsTotalHits, totalBytesSent=totalBytesSent, vsStatsEntry=vsStatsEntry, infIpv6Address=infIpv6Address, totalOpenSSLConns=totalOpenSSLConns, clickTcp=clickTcp, logHistTableMaxLength=logHistTableMaxLength, ctcpOutSegs=ctcpOutSegs, logMaxSeverity=logMaxSeverity, hcStatus=hcStatus, ctcpInErrs=ctcpInErrs, vsStatsTable=vsStatsTable, licenseRemainingDays=licenseRemainingDays, infOutNUcastPkts=infOutNUcastPkts, reqLastDay=reqLastDay, peakRcvdBytesPerSec=peakRcvdBytesPerSec, succLastHour=succLastHour, rsAvgRespTime=rsAvgRespTime, reqPeakDay=reqPeakDay, reqLastHour=reqLastHour, ctcpConnLocalAddr=ctcpConnLocalAddr, vsQosNetworkHits=vsQosNetworkHits, ctcpConnTable=ctcpConnTable, vsQosClientPortHits=vsQosClientPortHits, ipBytesOut=ipBytesOut, totalAcceptedConns=totalAcceptedConns, ctcpConnState=ctcpConnState, hcPort=hcPort, caSyslogTrap=caSyslogTrap, reqPeakSec=reqPeakSec, caTraps=caTraps, ipAddr=ipAddr, vsHeaderHits=vsHeaderHits, reqPeakMin=reqPeakMin, logHistoryTable=logHistoryTable, infNumber=infNumber, hcStatsTable=hcStatsTable, cputemp=cputemp, powerstate=powerstate, rsTable=rsTable, clusterVirAuthPasswd=clusterVirAuthPasswd, ctcpPassiveOpens=ctcpPassiveOpens, arrayNetworks=arrayNetworks, totalFail=totalFail, logHistoryEntry=logHistoryEntry, ipIndex=ipIndex, vsCount=vsCount, cacheMissWNoncacheReq=cacheMissWNoncacheReq, logNotificationsSent=logNotificationsSent, clusterNum=clusterNum, virtualIndex=virtualIndex, requestsPerSec=requestsPerSec, ipAddrType=ipAddrType, infIpv6AddressType=infIpv6AddressType, slbGeneral=slbGeneral, logHistory=logHistory, reqLastSec=reqLastSec, totalReq=totalReq, totalRequestedConns=totalRequestedConns, infOutUcastPkts=infOutUcastPkts, vrrpEntry=vrrpEntry, monitor=monitor, groupID=groupID, failLastHour=failLastHour, gpID=gpID, clusterVirPreempt=clusterVirPreempt, realAddressType=realAddressType, sslEntry=sslEntry, sslIndex=sslIndex, totalSucc=totalSucc, rsStatus=rsStatus, vsCacheHits=vsCacheHits, sslStats=sslStats, ctcpConnRemAddr=ctcpConnRemAddr, infOutMcastPkts=infOutMcastPkts, vsRCookieHits=vsRCookieHits, failLastSec=failLastSec)
mibBuilder.exportSymbols("CA-SNMP-MIB8", succLastSec=succLastSec, gpTotalHits=gpTotalHits, infInDiscards=infInDiscards, infTotalInOctets=infTotalInOctets, PYSNMP_MODULE_ID=arrayNetworks, vipStats=vipStats, clusterVrrp=clusterVrrp, infAddress=infAddress, healthCheck=healthCheck, succPeakMin=succPeakMin, hcNumDowns=hcNumDowns, cpuUtilization=cpuUtilization, caStart=caStart, vsHostnameHits=vsHostnameHits, startTime=startTime, postRequests=postRequests, serverEstabConn=serverEstabConn, ctcpConnEntry=ctcpConnEntry, succPeakDay=succPeakDay, realAddress=realAddress, compression=compression, rsStatsTable=rsStatsTable, vrrpTable=vrrpTable, vsQosCookieHits=vsQosCookieHits, hostName=hostName, rsConnCnt=rsConnCnt, vsRegexHits=vsRegexHits, clusterVirPriority=clusterVirPriority, infOutOctets=infOutOctets, realID=realID, succPeakSec5=succPeakSec5, ctcpRetransSegs=ctcpRetransSegs, infOperStatus=infOperStatus, vsConnCnt=vsConnCnt, infInBcastPkts=infInBcastPkts, SyslogSeverity=SyslogSeverity, virtualPort=virtualPort, virtualServer=virtualServer, maxCluster=maxCluster, virtualAddressType=virtualAddressType, connectionsPerSec=connectionsPerSec, rsStatsEntry=rsStatsEntry, vsPort=vsPort, hcCause=hcCause, requestsOnRegex=requestsOnRegex, clusterVirIfname=clusterVirIfname, ctcpCurrEstab=ctcpCurrEstab, cacheSkip=cacheSkip, vsRedirectHits=vsRedirectHits, missSess=missSess, infInNUcastPkts=infInNUcastPkts, vsPerstntURLHits=vsPerstntURLHits, sentBytesPerSec=sentBytesPerSec, connsPerSec=connsPerSec, ctcpEstabResets=ctcpEstabResets)
