...
Counter64, TimeTicks, Integer32, Unsigned32, IpAddress, Bits, Gauge32, ObjectIdentity, Opaque, Counter32, NotificationType, ModuleIdentity, MibIdentifier, iso, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Counter64", "TimeTicks", "Integer32", "Unsigned32", "IpAddress", "Bits", "Gauge32", "ObjectIdentity", "Opaque", "Counter32", "NotificationType", "ModuleIdentity", "MibIdentifier", "iso", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", infInOctets=infInOctets, vhostNum=vhostNum, hcStatsTable=hcStatsTable, vrrp=vrrp, ctcpConnTable=ctcpConnTable, hcConnAttempt=hcConnAttempt, Float=Float, ctcpConnLocalPort=ctcpConnLocalPort, responsesToHttps=responsesToHttps, groupID=groupID, vsIpAddr=vsIpAddr, reqLastSec=reqLastSec, ipPktsOut=ipPktsOut, ctcpCurrEstab=ctcpCurrEstab, virtualIndex=virtualIndex, ctcpConnLocalAddrType=ctcpConnLocalAddrType, cacheStatus=cacheStatus, failLastHour=failLastHour, realServer=realServer, infInMcastPkts=infInMcastPkts, vsIpAddressType=vsIpAddressType, hcCause=hcCause, failLastDay=failLastDay, sysSwapCapacity=sysSwapCapacity, rsIpAddressType=rsIpAddressType, infInNUcastPkts=infInNUcastPkts, sysMemoryUtilization=sysMemoryUtilization, rsIpAddr=rsIpAddr, logHistory=logHistory, infOutBandwidth=infOutBandwidth, clusterVirPriority=clusterVirPriority, vsPerstntURLHits=vsPerstntURLHits, logHistTableMaxLength=logHistTableMaxLength, groupCount=groupCount, sslEntry=sslEntry, SyslogSeverity=SyslogSeverity, realAddr=realAddr, caSyslogTrap=caSyslogTrap, msgText=msgText, vsPort=vsPort, cacheHitRatio=cacheHitRatio, rsID=rsID, infOperStatus=infOperStatus, syslogTrap=syslogTrap, sslGeneral=sslGeneral, connsPerSec=connsPerSec, vrrpEntry=vrrpEntry, succPeakMin=succPeakMin, infOutNUcastPkts=infOutNUcastPkts, ctcpEstabResets=ctcpEstabResets, ctcpPassiveOpens=ctcpPassiveOpens, PYSNMP_MODULE_ID=arrayNetworks, arrayNetworks=arrayNetworks, succPeakSec=succPeakSec, totalRequestedConns=totalRequestedConns, vipStats=vipStats, caShutdown=caShutdown, ipIndex=ipIndex, requestedConns=requestedConns, ifTraffic=ifTraffic, activeTransac=activeTransac, cputemp=cputemp, requestsToUrl=requestsToUrl, vsRedirectHits=vsRedirectHits, ctcpConnLocalAddr=ctcpConnLocalAddr, serverEstabConn=serverEstabConn, connectionsPerSec=connectionsPerSec, performance=performance, ipAddress=ipAddress, succLastSec=succLastSec, vsRegexHits=vsRegexHits, clickTcp=clickTcp, rsIndex=rsIndex, infNumber=infNumber, gpEntry=gpEntry, reqPeakMin=reqPeakMin, gpStatsEntry=gpStatsEntry, clusterVirState=clusterVirState, gpTotalHits=gpTotalHits, hcStats=hcStats, ipPktsIn=ipPktsIn, hcPort=hcPort, virtServerID=virtServerID, hcRSCount=hcRSCount, cacheMissWRespNo=cacheMissWRespNo, hitsReplyWNotModified=hitsReplyWNotModified, rsCount=rsCount, hcName=hcName, sslMIB=sslMIB, sysSwapUsed=sysSwapUsed, ctcpConnRemAddress=ctcpConnRemAddress, clusterVirAddressType=clusterVirAddressType, virtualAddress=virtualAddress, infInBcastPkts=infInBcastPkts, postRequests=postRequests, vrrpTable=vrrpTable, cacheMissWNoncacheReq=cacheMissWNoncacheReq, slbMIB=slbMIB, vsMaxConn=vsMaxConn, monitor=monitor, requestsPerSec=requestsPerSec, vsURLHits=vsURLHits, vsQosClientPortHits=vsQosClientPortHits, sysMemory=sysMemory, severity=severity, reqPeakSec5=reqPeakSec5, succPeakHour=succPeakHour, missSess=missSess, totalIPBytesOut=totalIPBytesOut, headRequests=headRequests, slbStats=slbStats, virtualServer=virtualServer, sdns=sdns, vsHashURLHits=vsHashURLHits, ipBytesOut=ipBytesOut, fanspeed=fanspeed, totalBytesRcvd=totalBytesRcvd, cacheBasicStats=cacheBasicStats, hcStatsEntry=hcStatsEntry, clusterVirIndex=clusterVirIndex, rsStatsTable=rsStatsTable, hcStatus=hcStatus, ipStatsTable=ipStatsTable, infIpv6Address=infIpv6Address, gpMetrics=gpMetrics, failLastMin=failLastMin, reqLastMin=reqLastMin, reqLastSec5=reqLastSec5, index=index, requestsOnRegex=requestsOnRegex, virtualAddr=virtualAddr, virtualPort=virtualPort, succLastMin=succLastMin, infDescr=infDescr, succPeakSec5=succPeakSec5, clusterId=clusterId, memory=memory, infOutBcastPkts=infOutBcastPkts, resumedSess=resumedSess, rsIpAddress=rsIpAddress, totalSucc=totalSucc, rcvdBytesPerSec=rcvdBytesPerSec, vsConnCnt=vsConnCnt, licenseRemainingDays=licenseRemainingDays, infInErrors=infInErrors, hitsReply=hitsReply, infOutErrors=infOutErrors, vsTable=vsTable, clusterVirIfname=clusterVirIfname, revProxyCache=revProxyCache, hcConnFail=hcConnFail, powerstate=powerstate, vsStaticHits=vsStaticHits, rsConnCnt=rsConnCnt, ctcpConnLocalAddress=ctcpConnLocalAddress, clusterVirAuthPasswd=clusterVirAuthPasswd, ipAddrType=ipAddrType, totalIPPktsIn=totalIPPktsIn, gpStatsTable=gpStatsTable, clusterVirAddr=clusterVirAddr, vsQosURLHits=vsQosURLHits, vsDefaultHits=vsDefaultHits, reqPeakHour=reqPeakHour, groupCurCfg=groupCurCfg, rsTotalHits=rsTotalHits, vsID=vsID, hcAddr=hcAddr, totalBytesSent=totalBytesSent, infIpv4AddressType=infIpv4AddressType, vsIpAddress=vsIpAddress, vsQosBodyHits=vsQosBodyHits, infTable=infTable, hitsReplyWPreFailed=hitsReplyWPreFailed, infInBandwidth=infInBandwidth, rsAvgRespTime=rsAvgRespTime, vsQosCookieHits=vsQosCookieHits, acceptedConns=acceptedConns, logNotificationsEnabled=logNotificationsEnabled, realStatus=realStatus, succLastHour=succLastHour, infIpv4Address=infIpv4Address, succLastDay=succLastDay, realServerID=realServerID, vsCacheHits=vsCacheHits, clientEstabConn=clientEstabConn, maxCluster=maxCluster, peakRcvdBytesPerSec=peakRcvdBytesPerSec, hcAddress=hcAddress, caTraps=caTraps, rsStatsEntry=rsStatsEntry, totalOpenSSLConns=totalOpenSSLConns, vsICookieHits=vsICookieHits, rsPort=rsPort, infInDiscards=infInDiscards, ctcpConnState=ctcpConnState, reqLastHour=reqLastHour, systemInfo=systemInfo, virtualStats=virtualStats, realIndex=realIndex, vsEntry=vsEntry, clusterNum=clusterNum, virtualAddressType=virtualAddressType, vsBackupHits=vsBackupHits, ctcpActiveOpens=ctcpActiveOpens, openSSLConns=openSSLConns, caStart=caStart, vsCount=vsCount, healthCheck=healthCheck, clusterVirAuthType=clusterVirAuthType, rsTable=rsTable, ctcpConnRemPort=ctcpConnRemPort, logBasic=logBasic, resumableSess=resumableSess, compression=compression, rsStatus=rsStatus, vsStatsEntry=vsStatsEntry, totalAcceptedConns=totalAcceptedConns, clusterVirInterval=clusterVirInterval, realPort=realPort, vsQosNetworkHits=vsQosNetworkHits, vsHostnameHits=vsHostnameHits, sentBytesPerSec=sentBytesPerSec, logHistoryEntry=logHistoryEntry, requestsToHttps=requestsToHttps, vhostName=vhostName, ctcpConnEntry=ctcpConnEntry, ctcpConnRemAddr=ctcpConnRemAddr, hcNumUps=hcNumUps, vsHeaderHits=vsHeaderHits, ctcpInErrs=ctcpInErrs, realStats=realStats, vsIndex=vsIndex, currentTime=currentTime, logMaxSeverity=logMaxSeverity, peakSentBytesPerSec=peakSentBytesPerSec, failLastSec=failLastSec, infOutMcastPkts=infOutMcastPkts, groupIndex=groupIndex, ctcpOutRsts=ctcpOutRsts, sslStats=sslStats, hcConnSuccess=hcConnSuccess, succPeakDay=succPeakDay, ctcpInSegs=ctcpInSegs, rsCntOfReq=rsCntOfReq, realAddress=realAddress, infTotalInOctets=infTotalInOctets, groupStats=groupStats, infIndex=infIndex, cpuUtilization=cpuUtilization, cacheSkip=cacheSkip, serialNumber=serialNumber, requestsReceived=requestsReceived, hcIndex=hcIndex, realID=realID)
mibBuilder.exportSymbols("CA-SNMP-MIB8", infIpv6AddressType=infIpv6AddressType, rsProtocol=rsProtocol, ctcpAttemptFails=ctcpAttemptFails, rsEntry=rsEntry, purgeRequests=purgeRequests, ipBytesIn=ipBytesIn, sslTable=sslTable, reqPeakSec=reqPeakSec, totalIPPktsOut=totalIPPktsOut, hitRevalidate=hitRevalidate, hcAddressType=hcAddressType, hostName=hostName, vsProtocol=vsProtocol, hcNumDowns=hcNumDowns, ipAddr=ipAddr, getRequests=getRequests, cacheMissWNewEntry=cacheMissWNewEntry, infOutOctets=infOutOctets, gpIndex=gpIndex, failLastSec5=failLastSec5, gpID=gpID, succLastSec5=succLastSec5, vsPerstntCookieHits=vsPerstntCookieHits, infInUcastPkts=infInUcastPkts, realAddressType=realAddressType, totalFail=totalFail, sslIndex=sslIndex, clusterVirAddress=clusterVirAddress, vipStatus=vipStatus, ctcpOutSegs=ctcpOutSegs, reqLastDay=reqLastDay, responsesOnRegex=responsesOnRegex, logHistoryTable=logHistoryTable, infEntry=infEntry, reqPeakDay=reqPeakDay, slbGeneral=slbGeneral, vsStatsTable=vsStatsTable, infAddress=infAddress, rsMaxConn=rsMaxConn, caSyslog=caSyslog, ctcpIndex=ctcpIndex, ctcpRetransSegs=ctcpRetransSegs, gpTable=gpTable, logNotificationsSent=logNotificationsSent, startTime=startTime, vsRCookieHits=vsRCookieHits, infInUnknownProtos=infInUnknownProtos, clusterVirPreempt=clusterVirPreempt, totalReq=totalReq, ipStatsEntry=ipStatsEntry, infTotalOutOctets=infTotalOutOctets, totalIPBytesIn=totalIPBytesIn, ctcpConnRemAddrType=ctcpConnRemAddrType, infOutUcastPkts=infOutUcastPkts, clusterVrrp=clusterVrrp)
