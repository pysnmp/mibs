...
MibIdentifier, Counter32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Integer32, Bits, Opaque, IpAddress, enterprises, Unsigned32, Gauge32, iso, ObjectIdentity, Counter64, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Counter32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Integer32", "Bits", "Opaque", "IpAddress", "enterprises", "Unsigned32", "Gauge32", "iso", "ObjectIdentity", "Counter64", "TimeTicks")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsHeaderHits=vsHeaderHits, reqLastSec=reqLastSec, rsMaxConn=rsMaxConn, slbStats=slbStats, logHistoryEntry=logHistoryEntry, hcNumUps=hcNumUps, infInUcastPkts=infInUcastPkts, infInErrors=infInErrors, ctcpActiveOpens=ctcpActiveOpens, sysMemory=sysMemory, totalBytesRcvd=totalBytesRcvd, healthCheck=healthCheck, compression=compression, ipAddrType=ipAddrType, cputemp=cputemp, rsTable=rsTable, succLastMin=succLastMin, vsID=vsID, groupIndex=groupIndex, totalIPBytesIn=totalIPBytesIn, infIpv4AddressType=infIpv4AddressType, caShutdown=caShutdown, vsQosCookieHits=vsQosCookieHits, clusterVirAuthPasswd=clusterVirAuthPasswd, realAddressType=realAddressType, sslMIB=sslMIB, totalReq=totalReq, gpStatsEntry=gpStatsEntry, totalIPPktsIn=totalIPPktsIn, infDescr=infDescr, ctcpOutSegs=ctcpOutSegs, postRequests=postRequests, cacheBasicStats=cacheBasicStats, vsIndex=vsIndex, rsIpAddressType=rsIpAddressType, sdns=sdns, infIpv6Address=infIpv6Address, realAddress=realAddress, serverEstabConn=serverEstabConn, failLastSec5=failLastSec5, ctcpConnLocalAddress=ctcpConnLocalAddress, hcAddressType=hcAddressType, arrayNetworks=arrayNetworks, failLastSec=failLastSec, vsTable=vsTable, rsTotalHits=rsTotalHits, severity=severity, hcConnSuccess=hcConnSuccess, clusterVirAddr=clusterVirAddr, hcCause=hcCause, virtualStats=virtualStats, acceptedConns=acceptedConns, ipAddress=ipAddress, ctcpInErrs=ctcpInErrs, ipPktsOut=ipPktsOut, succPeakSec=succPeakSec, infOutUcastPkts=infOutUcastPkts, rsStatsEntry=rsStatsEntry, infInBandwidth=infInBandwidth, ctcpConnLocalAddr=ctcpConnLocalAddr, ctcpConnRemPort=ctcpConnRemPort, vsURLHits=vsURLHits, succPeakSec5=succPeakSec5, vsStatsTable=vsStatsTable, cacheHitRatio=cacheHitRatio, vsICookieHits=vsICookieHits, totalRequestedConns=totalRequestedConns, currentTime=currentTime, reqPeakSec=reqPeakSec, ctcpEstabResets=ctcpEstabResets, msgText=msgText, hcConnFail=hcConnFail, rsIpAddress=rsIpAddress, infInOctets=infInOctets, sslIndex=sslIndex, vsEntry=vsEntry, ctcpConnRemAddress=ctcpConnRemAddress, cacheMissWRespNo=cacheMissWRespNo, infInMcastPkts=infInMcastPkts, infIpv6AddressType=infIpv6AddressType, virtualServer=virtualServer, sslEntry=sslEntry, cpuUtilization=cpuUtilization, succLastSec=succLastSec, index=index, infOutMcastPkts=infOutMcastPkts, ctcpRetransSegs=ctcpRetransSegs, rsCount=rsCount, rsAvgRespTime=rsAvgRespTime, syslogTrap=syslogTrap, ctcpConnEntry=ctcpConnEntry, caSyslog=caSyslog, realStats=realStats, vipStats=vipStats, ipBytesOut=ipBytesOut, cacheSkip=cacheSkip, vsPort=vsPort, rsCntOfReq=rsCntOfReq, licenseRemainingDays=licenseRemainingDays, virtualIndex=virtualIndex, reqPeakDay=reqPeakDay, clusterVirIndex=clusterVirIndex, infTotalOutOctets=infTotalOutOctets, infInUnknownProtos=infInUnknownProtos, rsProtocol=rsProtocol, vsQosNetworkHits=vsQosNetworkHits, sysSwapCapacity=sysSwapCapacity, infIpv4Address=infIpv4Address, vsCount=vsCount, connectionsPerSec=connectionsPerSec, ctcpConnRemAddr=ctcpConnRemAddr, hcAddr=hcAddr, infOutBcastPkts=infOutBcastPkts, rsEntry=rsEntry, hostName=hostName, failLastDay=failLastDay, vsHashURLHits=vsHashURLHits, PYSNMP_MODULE_ID=arrayNetworks, vsQosURLHits=vsQosURLHits, responsesToHttps=responsesToHttps, groupCount=groupCount, vsProtocol=vsProtocol, hcNumDowns=hcNumDowns, realPort=realPort, vsCacheHits=vsCacheHits, logNotificationsEnabled=logNotificationsEnabled, infAddress=infAddress, requestsOnRegex=requestsOnRegex, gpMetrics=gpMetrics, infOperStatus=infOperStatus, totalFail=totalFail, vrrpEntry=vrrpEntry, requestedConns=requestedConns, clickTcp=clickTcp, gpIndex=gpIndex, vhostName=vhostName, cacheStatus=cacheStatus, Float=Float, hcConnAttempt=hcConnAttempt, memory=memory, caStart=caStart, virtualAddr=virtualAddr, ctcpAttemptFails=ctcpAttemptFails, clusterVirIfname=clusterVirIfname, hcIndex=hcIndex, ipStatsEntry=ipStatsEntry, groupCurCfg=groupCurCfg, virtualAddressType=virtualAddressType, connsPerSec=connsPerSec, infNumber=infNumber, headRequests=headRequests, ipBytesIn=ipBytesIn, clusterVirAddressType=clusterVirAddressType, cacheMissWNoncacheReq=cacheMissWNoncacheReq, reqPeakHour=reqPeakHour, clientEstabConn=clientEstabConn, hitsReplyWNotModified=hitsReplyWNotModified, sysMemoryUtilization=sysMemoryUtilization, systemInfo=systemInfo, slbMIB=slbMIB, reqPeakMin=reqPeakMin, clusterVirPreempt=clusterVirPreempt, rsIndex=rsIndex, vrrp=vrrp, vsRCookieHits=vsRCookieHits, requestsReceived=requestsReceived, ctcpInSegs=ctcpInSegs, caSyslogTrap=caSyslogTrap, openSSLConns=openSSLConns, totalSucc=totalSucc, ctcpConnState=ctcpConnState, virtualPort=virtualPort, succPeakHour=succPeakHour, ipAddr=ipAddr, reqLastHour=reqLastHour, clusterVirAddress=clusterVirAddress, vsStatsEntry=vsStatsEntry, infTotalInOctets=infTotalInOctets, rsStatsTable=rsStatsTable, infInDiscards=infInDiscards, vsRegexHits=vsRegexHits, realAddr=realAddr, infInBcastPkts=infInBcastPkts, vrrpTable=vrrpTable, powerstate=powerstate, totalBytesSent=totalBytesSent, clusterVirInterval=clusterVirInterval, vhostNum=vhostNum, rsStatus=rsStatus, vsIpAddr=vsIpAddr, ctcpOutRsts=ctcpOutRsts, resumableSess=resumableSess, performance=performance, hitsReply=hitsReply, vipStatus=vipStatus, realStatus=realStatus, hcName=hcName, realIndex=realIndex, fanspeed=fanspeed, logHistTableMaxLength=logHistTableMaxLength, ifTraffic=ifTraffic, requestsToHttps=requestsToHttps, startTime=startTime, vsIpAddressType=vsIpAddressType, reqLastMin=reqLastMin, sentBytesPerSec=sentBytesPerSec, logBasic=logBasic, requestsToUrl=requestsToUrl, groupStats=groupStats, ctcpCurrEstab=ctcpCurrEstab, infOutErrors=infOutErrors, peakRcvdBytesPerSec=peakRcvdBytesPerSec, vsHostnameHits=vsHostnameHits, realID=realID, clusterId=clusterId, vsQosBodyHits=vsQosBodyHits, infTable=infTable, logHistoryTable=logHistoryTable, infInNUcastPkts=infInNUcastPkts, cacheMissWNewEntry=cacheMissWNewEntry, realServer=realServer, clusterVirPriority=clusterVirPriority, ctcpConnLocalAddrType=ctcpConnLocalAddrType, infOutOctets=infOutOctets, failLastMin=failLastMin, maxCluster=maxCluster, totalIPBytesOut=totalIPBytesOut, gpTotalHits=gpTotalHits, virtualAddress=virtualAddress, vsMaxConn=vsMaxConn, vsConnCnt=vsConnCnt, hcStatsEntry=hcStatsEntry, hitsReplyWPreFailed=hitsReplyWPreFailed, infEntry=infEntry, totalAcceptedConns=totalAcceptedConns, hcAddress=hcAddress, peakSentBytesPerSec=peakSentBytesPerSec, sysSwapUsed=sysSwapUsed, missSess=missSess, realServerID=realServerID, clusterVrrp=clusterVrrp, succLastDay=succLastDay, rsID=rsID, succPeakDay=succPeakDay, failLastHour=failLastHour, SyslogSeverity=SyslogSeverity, requestsPerSec=requestsPerSec, ctcpConnRemAddrType=ctcpConnRemAddrType, logHistory=logHistory, clusterVirAuthType=clusterVirAuthType, caTraps=caTraps, hcRSCount=hcRSCount, succLastHour=succLastHour, clusterNum=clusterNum)
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsDefaultHits=vsDefaultHits, hcStats=hcStats, revProxyCache=revProxyCache, rsConnCnt=rsConnCnt, ctcpIndex=ctcpIndex, rcvdBytesPerSec=rcvdBytesPerSec, reqLastDay=reqLastDay, sslGeneral=sslGeneral, vsPerstntURLHits=vsPerstntURLHits, ipPktsIn=ipPktsIn, gpStatsTable=gpStatsTable, ctcpConnTable=ctcpConnTable, reqPeakSec5=reqPeakSec5, slbGeneral=slbGeneral, vsStaticHits=vsStaticHits, hcStatus=hcStatus, succPeakMin=succPeakMin, vsBackupHits=vsBackupHits, logNotificationsSent=logNotificationsSent, getRequests=getRequests, sslStats=sslStats, sslTable=sslTable, infOutBandwidth=infOutBandwidth, vsQosClientPortHits=vsQosClientPortHits, rsPort=rsPort, hitRevalidate=hitRevalidate, responsesOnRegex=responsesOnRegex, vsRedirectHits=vsRedirectHits, gpID=gpID, logMaxSeverity=logMaxSeverity, groupID=groupID, ipStatsTable=ipStatsTable, totalOpenSSLConns=totalOpenSSLConns, hcPort=hcPort, ctcpConnLocalPort=ctcpConnLocalPort, vsIpAddress=vsIpAddress, gpTable=gpTable, totalIPPktsOut=totalIPPktsOut, ctcpPassiveOpens=ctcpPassiveOpens, gpEntry=gpEntry, reqLastSec5=reqLastSec5, vsPerstntCookieHits=vsPerstntCookieHits, succLastSec5=succLastSec5, purgeRequests=purgeRequests, infOutNUcastPkts=infOutNUcastPkts, activeTransac=activeTransac, monitor=monitor, virtServerID=virtServerID, infIndex=infIndex, hcStatsTable=hcStatsTable, resumedSess=resumedSess, clusterVirState=clusterVirState, ipIndex=ipIndex, rsIpAddr=rsIpAddr, serialNumber=serialNumber)
