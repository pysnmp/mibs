...
MibIdentifier, TimeTicks, Gauge32, Unsigned32, Bits, Counter64, Opaque, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, Integer32, ModuleIdentity, ObjectIdentity, enterprises, Counter32, iso = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "TimeTicks", "Gauge32", "Unsigned32", "Bits", "Counter64", "Opaque", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "Integer32", "ModuleIdentity", "ObjectIdentity", "enterprises", "Counter32", "iso")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", ctcpConnState=ctcpConnState, hcAddr=hcAddr, groupCount=groupCount, infAddress=infAddress, hcConnFail=hcConnFail, currentTime=currentTime, peakSentBytesPerSec=peakSentBytesPerSec, infIpv4Address=infIpv4Address, succLastSec=succLastSec, ipPktsIn=ipPktsIn, totalOpenSSLConns=totalOpenSSLConns, totalBytesRcvd=totalBytesRcvd, sentBytesPerSec=sentBytesPerSec, vsDefaultHits=vsDefaultHits, logNotificationsSent=logNotificationsSent, caTraps=caTraps, Float=Float, virtualAddr=virtualAddr, systemInfo=systemInfo, vsHashURLHits=vsHashURLHits, resumedSess=resumedSess, ctcpEstabResets=ctcpEstabResets, gpIndex=gpIndex, vsICookieHits=vsICookieHits, sslIndex=sslIndex, cacheMissWNoncacheReq=cacheMissWNoncacheReq, infInDiscards=infInDiscards, openSSLConns=openSSLConns, totalReq=totalReq, vsHostnameHits=vsHostnameHits, sslEntry=sslEntry, requestsPerSec=requestsPerSec, cacheHitRatio=cacheHitRatio, gpTable=gpTable, realPort=realPort, vsIndex=vsIndex, infDescr=infDescr, hcIndex=hcIndex, sysSwapUsed=sysSwapUsed, reqLastSec5=reqLastSec5, reqLastMin=reqLastMin, msgText=msgText, vsRCookieHits=vsRCookieHits, reqPeakDay=reqPeakDay, hitsReply=hitsReply, fanspeed=fanspeed, sdns=sdns, ctcpOutSegs=ctcpOutSegs, vsEntry=vsEntry, rsIpAddress=rsIpAddress, cacheStatus=cacheStatus, ctcpInSegs=ctcpInSegs, acceptedConns=acceptedConns, responsesToHttps=responsesToHttps, hcAddress=hcAddress, vsPerstntURLHits=vsPerstntURLHits, healthCheck=healthCheck, reqPeakSec=reqPeakSec, gpMetrics=gpMetrics, vrrp=vrrp, severity=severity, vsPort=vsPort, vsCount=vsCount, vsRegexHits=vsRegexHits, totalBytesSent=totalBytesSent, rsTable=rsTable, rsAvgRespTime=rsAvgRespTime, cacheMissWNewEntry=cacheMissWNewEntry, ipPktsOut=ipPktsOut, virtualPort=virtualPort, ipAddrType=ipAddrType, slbStats=slbStats, rsStatsEntry=rsStatsEntry, ctcpIndex=ctcpIndex, infIpv6Address=infIpv6Address, infTable=infTable, licenseRemainingDays=licenseRemainingDays, realID=realID, ctcpConnLocalPort=ctcpConnLocalPort, clusterVirIndex=clusterVirIndex, groupCurCfg=groupCurCfg, gpTotalHits=gpTotalHits, infInMcastPkts=infInMcastPkts, failLastSec=failLastSec, serialNumber=serialNumber, connectionsPerSec=connectionsPerSec, hcRSCount=hcRSCount, clusterNum=clusterNum, vsStaticHits=vsStaticHits, hostName=hostName, rsIndex=rsIndex, rsProtocol=rsProtocol, vsQosURLHits=vsQosURLHits, ctcpConnRemAddrType=ctcpConnRemAddrType, cpuUtilization=cpuUtilization, infInBandwidth=infInBandwidth, PYSNMP_MODULE_ID=arrayNetworks, clusterVirState=clusterVirState, infOutUcastPkts=infOutUcastPkts, headRequests=headRequests, reqLastHour=reqLastHour, postRequests=postRequests, virtualAddress=virtualAddress, compression=compression, vsMaxConn=vsMaxConn, realServerID=realServerID, clusterVirAddressType=clusterVirAddressType, gpStatsTable=gpStatsTable, ipStatsTable=ipStatsTable, reqPeakHour=reqPeakHour, clusterId=clusterId, startTime=startTime, gpEntry=gpEntry, reqLastSec=reqLastSec, rsTotalHits=rsTotalHits, totalIPPktsIn=totalIPPktsIn, infOutBandwidth=infOutBandwidth, totalIPBytesIn=totalIPBytesIn, ctcpConnLocalAddr=ctcpConnLocalAddr, sslTable=sslTable, responsesOnRegex=responsesOnRegex, totalFail=totalFail, hcStatus=hcStatus, rsConnCnt=rsConnCnt, vipStats=vipStats, totalIPPktsOut=totalIPPktsOut, rsEntry=rsEntry, revProxyCache=revProxyCache, ctcpCurrEstab=ctcpCurrEstab, ctcpActiveOpens=ctcpActiveOpens, virtServerID=virtServerID, hcName=hcName, logHistory=logHistory, vsQosNetworkHits=vsQosNetworkHits, infIpv6AddressType=infIpv6AddressType, clientEstabConn=clientEstabConn, clusterVirIfname=clusterVirIfname, vsCacheHits=vsCacheHits, ctcpConnEntry=ctcpConnEntry, clusterVirInterval=clusterVirInterval, hcStats=hcStats, succLastSec5=succLastSec5, infInNUcastPkts=infInNUcastPkts, vsHeaderHits=vsHeaderHits, realStats=realStats, ctcpAttemptFails=ctcpAttemptFails, infTotalOutOctets=infTotalOutOctets, vsQosCookieHits=vsQosCookieHits, hcCause=hcCause, vipStatus=vipStatus, failLastSec5=failLastSec5, virtualIndex=virtualIndex, rsIpAddressType=rsIpAddressType, failLastMin=failLastMin, realIndex=realIndex, ipBytesIn=ipBytesIn, logHistoryEntry=logHistoryEntry, maxCluster=maxCluster, totalAcceptedConns=totalAcceptedConns, succLastMin=succLastMin, infIpv4AddressType=infIpv4AddressType, clusterVirAddress=clusterVirAddress, ipStatsEntry=ipStatsEntry, hitsReplyWPreFailed=hitsReplyWPreFailed, clickTcp=clickTcp, clusterVirAuthType=clusterVirAuthType, hcNumDowns=hcNumDowns, infInOctets=infInOctets, vsPerstntCookieHits=vsPerstntCookieHits, logNotificationsEnabled=logNotificationsEnabled, cputemp=cputemp, clusterVrrp=clusterVrrp, getRequests=getRequests, logHistoryTable=logHistoryTable, slbMIB=slbMIB, sysSwapCapacity=sysSwapCapacity, hcAddressType=hcAddressType, infInUnknownProtos=infInUnknownProtos, ctcpConnRemAddress=ctcpConnRemAddress, requestsReceived=requestsReceived, vsIpAddr=vsIpAddr, rcvdBytesPerSec=rcvdBytesPerSec, hitsReplyWNotModified=hitsReplyWNotModified, infOutOctets=infOutOctets, ctcpPassiveOpens=ctcpPassiveOpens, succPeakDay=succPeakDay, hitRevalidate=hitRevalidate, groupStats=groupStats, reqPeakSec5=reqPeakSec5, requestsOnRegex=requestsOnRegex, succPeakSec=succPeakSec, succLastDay=succLastDay, infNumber=infNumber, vsStatsTable=vsStatsTable, reqLastDay=reqLastDay, virtualStats=virtualStats, realStatus=realStatus, cacheMissWRespNo=cacheMissWRespNo, rsPort=rsPort, infInUcastPkts=infInUcastPkts, ipIndex=ipIndex, monitor=monitor, hcStatsEntry=hcStatsEntry, vsID=vsID, peakRcvdBytesPerSec=peakRcvdBytesPerSec, vsConnCnt=vsConnCnt, infOutNUcastPkts=infOutNUcastPkts, virtualServer=virtualServer, succPeakMin=succPeakMin, caSyslog=caSyslog, arrayNetworks=arrayNetworks, caSyslogTrap=caSyslogTrap, totalSucc=totalSucc, memory=memory, sysMemory=sysMemory, vrrpTable=vrrpTable, ifTraffic=ifTraffic, virtualAddressType=virtualAddressType, rsCntOfReq=rsCntOfReq, hcConnSuccess=hcConnSuccess, vsQosBodyHits=vsQosBodyHits, infOutBcastPkts=infOutBcastPkts, vsProtocol=vsProtocol, vsIpAddress=vsIpAddress, ctcpConnLocalAddress=ctcpConnLocalAddress, succPeakSec5=succPeakSec5, rsMaxConn=rsMaxConn, requestedConns=requestedConns, purgeRequests=purgeRequests, logBasic=logBasic, resumableSess=resumableSess, clusterVirAddr=clusterVirAddr, totalIPBytesOut=totalIPBytesOut, requestsToUrl=requestsToUrl, cacheSkip=cacheSkip, clusterVirPreempt=clusterVirPreempt, sslStats=sslStats, logHistTableMaxLength=logHistTableMaxLength, rsStatsTable=rsStatsTable, failLastHour=failLastHour, ctcpInErrs=ctcpInErrs, realServer=realServer, ipBytesOut=ipBytesOut, ctcpOutRsts=ctcpOutRsts, infIndex=infIndex, vrrpEntry=vrrpEntry, vsRedirectHits=vsRedirectHits, infOutErrors=infOutErrors, syslogTrap=syslogTrap, serverEstabConn=serverEstabConn, missSess=missSess, vsURLHits=vsURLHits, ipAddress=ipAddress, ctcpConnTable=ctcpConnTable)
mibBuilder.exportSymbols("CA-SNMP-MIB8", hcConnAttempt=hcConnAttempt, groupIndex=groupIndex, infInErrors=infInErrors, rsCount=rsCount, ctcpConnRemAddr=ctcpConnRemAddr, succPeakHour=succPeakHour, ipAddr=ipAddr, gpStatsEntry=gpStatsEntry, ctcpConnLocalAddrType=ctcpConnLocalAddrType, cacheBasicStats=cacheBasicStats, index=index, connsPerSec=connsPerSec, gpID=gpID, infOutMcastPkts=infOutMcastPkts, succLastHour=succLastHour, hcNumUps=hcNumUps, hcStatsTable=hcStatsTable, caStart=caStart, infOperStatus=infOperStatus, vhostName=vhostName, requestsToHttps=requestsToHttps, logMaxSeverity=logMaxSeverity, ctcpConnRemPort=ctcpConnRemPort, sysMemoryUtilization=sysMemoryUtilization, activeTransac=activeTransac, infTotalInOctets=infTotalInOctets, failLastDay=failLastDay, vhostNum=vhostNum, totalRequestedConns=totalRequestedConns, realAddressType=realAddressType, performance=performance, sslGeneral=sslGeneral, vsBackupHits=vsBackupHits, rsID=rsID, powerstate=powerstate, rsStatus=rsStatus, clusterVirPriority=clusterVirPriority, slbGeneral=slbGeneral, sslMIB=sslMIB, caShutdown=caShutdown, infInBcastPkts=infInBcastPkts, vsTable=vsTable, infEntry=infEntry, groupID=groupID, hcPort=hcPort, rsIpAddr=rsIpAddr, vsStatsEntry=vsStatsEntry, clusterVirAuthPasswd=clusterVirAuthPasswd, realAddress=realAddress, vsIpAddressType=vsIpAddressType, reqPeakMin=reqPeakMin, vsQosClientPortHits=vsQosClientPortHits, realAddr=realAddr, ctcpRetransSegs=ctcpRetransSegs, SyslogSeverity=SyslogSeverity)
