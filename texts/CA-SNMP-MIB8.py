...
IpAddress, Counter32, MibIdentifier, ModuleIdentity, Gauge32, enterprises, Bits, Integer32, Unsigned32, iso, NotificationType, TimeTicks, Opaque, ObjectIdentity, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "IpAddress", "Counter32", "MibIdentifier", "ModuleIdentity", "Gauge32", "enterprises", "Bits", "Integer32", "Unsigned32", "iso", "NotificationType", "TimeTicks", "Opaque", "ObjectIdentity", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsPerstntCookieHits=vsPerstntCookieHits, syslogTrap=syslogTrap, gpMetrics=gpMetrics, sslIndex=sslIndex, infOutMcastPkts=infOutMcastPkts, hcRSCount=hcRSCount, ipStatsEntry=ipStatsEntry, hcStats=hcStats, hcCause=hcCause, severity=severity, vipStats=vipStats, caStart=caStart, totalAcceptedConns=totalAcceptedConns, memory=memory, gpIndex=gpIndex, clusterVirAddress=clusterVirAddress, vsCount=vsCount, vsRCookieHits=vsRCookieHits, serverEstabConn=serverEstabConn, responsesToHttps=responsesToHttps, cacheMissWNewEntry=cacheMissWNewEntry, clusterVirState=clusterVirState, rsMaxConn=rsMaxConn, sysMemory=sysMemory, systemInfo=systemInfo, ctcpConnRemPort=ctcpConnRemPort, groupStats=groupStats, vsProtocol=vsProtocol, logNotificationsEnabled=logNotificationsEnabled, ctcpConnLocalPort=ctcpConnLocalPort, succPeakHour=succPeakHour, cacheMissWNoncacheReq=cacheMissWNoncacheReq, clusterVirAuthType=clusterVirAuthType, infInBandwidth=infInBandwidth, vsEntry=vsEntry, sysSwapUsed=sysSwapUsed, connsPerSec=connsPerSec, PYSNMP_MODULE_ID=arrayNetworks, performance=performance, infIpv6AddressType=infIpv6AddressType, vsHostnameHits=vsHostnameHits, acceptedConns=acceptedConns, slbStats=slbStats, ctcpRetransSegs=ctcpRetransSegs, reqLastSec=reqLastSec, realAddressType=realAddressType, infOutOctets=infOutOctets, infIpv6Address=infIpv6Address, infNumber=infNumber, startTime=startTime, caSyslogTrap=caSyslogTrap, clusterVirPreempt=clusterVirPreempt, hcName=hcName, rsTable=rsTable, ctcpConnTable=ctcpConnTable, virtualStats=virtualStats, powerstate=powerstate, cacheBasicStats=cacheBasicStats, vsIpAddr=vsIpAddr, rsTotalHits=rsTotalHits, ctcpActiveOpens=ctcpActiveOpens, hcPort=hcPort, vsConnCnt=vsConnCnt, hcStatus=hcStatus, totalBytesSent=totalBytesSent, vsPerstntURLHits=vsPerstntURLHits, failLastSec5=failLastSec5, totalBytesRcvd=totalBytesRcvd, index=index, clickTcp=clickTcp, slbGeneral=slbGeneral, clusterVirIndex=clusterVirIndex, totalIPBytesIn=totalIPBytesIn, vsCacheHits=vsCacheHits, ctcpOutRsts=ctcpOutRsts, purgeRequests=purgeRequests, ctcpInErrs=ctcpInErrs, vsStatsEntry=vsStatsEntry, groupIndex=groupIndex, vsDefaultHits=vsDefaultHits, reqLastSec5=reqLastSec5, ctcpInSegs=ctcpInSegs, postRequests=postRequests, hcAddress=hcAddress, caSyslog=caSyslog, clusterId=clusterId, hitsReplyWPreFailed=hitsReplyWPreFailed, gpTable=gpTable, infEntry=infEntry, hcNumUps=hcNumUps, failLastDay=failLastDay, SyslogSeverity=SyslogSeverity, gpID=gpID, rcvdBytesPerSec=rcvdBytesPerSec, hitsReplyWNotModified=hitsReplyWNotModified, groupID=groupID, infOutBcastPkts=infOutBcastPkts, succLastSec=succLastSec, vsQosClientPortHits=vsQosClientPortHits, infIpv4AddressType=infIpv4AddressType, infOperStatus=infOperStatus, ctcpConnRemAddress=ctcpConnRemAddress, getRequests=getRequests, hitRevalidate=hitRevalidate, vsRedirectHits=vsRedirectHits, ctcpConnRemAddr=ctcpConnRemAddr, ctcpConnLocalAddrType=ctcpConnLocalAddrType, cacheHitRatio=cacheHitRatio, cputemp=cputemp, responsesOnRegex=responsesOnRegex, infOutBandwidth=infOutBandwidth, rsIpAddressType=rsIpAddressType, gpStatsEntry=gpStatsEntry, revProxyCache=revProxyCache, sslEntry=sslEntry, virtualAddr=virtualAddr, realServer=realServer, sysSwapCapacity=sysSwapCapacity, ctcpCurrEstab=ctcpCurrEstab, rsConnCnt=rsConnCnt, virtualAddressType=virtualAddressType, reqPeakSec5=reqPeakSec5, maxCluster=maxCluster, rsCntOfReq=rsCntOfReq, succLastMin=succLastMin, succLastSec5=succLastSec5, ctcpConnLocalAddress=ctcpConnLocalAddress, connectionsPerSec=connectionsPerSec, clusterVirIfname=clusterVirIfname, rsIpAddr=rsIpAddr, vsMaxConn=vsMaxConn, realAddress=realAddress, realServerID=realServerID, virtualPort=virtualPort, infInUcastPkts=infInUcastPkts, vrrpTable=vrrpTable, requestsToHttps=requestsToHttps, cpuUtilization=cpuUtilization, reqLastMin=reqLastMin, cacheStatus=cacheStatus, logMaxSeverity=logMaxSeverity, ctcpAttemptFails=ctcpAttemptFails, hcStatsEntry=hcStatsEntry, realIndex=realIndex, clusterVrrp=clusterVrrp, hcStatsTable=hcStatsTable, infInBcastPkts=infInBcastPkts, hcConnAttempt=hcConnAttempt, vsQosURLHits=vsQosURLHits, vrrpEntry=vrrpEntry, infIndex=infIndex, gpStatsTable=gpStatsTable, totalRequestedConns=totalRequestedConns, hcConnSuccess=hcConnSuccess, vipStatus=vipStatus, virtServerID=virtServerID, vsQosBodyHits=vsQosBodyHits, logHistoryEntry=logHistoryEntry, vsICookieHits=vsICookieHits, caShutdown=caShutdown, ipIndex=ipIndex, reqLastHour=reqLastHour, clusterVirPriority=clusterVirPriority, infInMcastPkts=infInMcastPkts, gpTotalHits=gpTotalHits, requestsPerSec=requestsPerSec, missSess=missSess, cacheMissWRespNo=cacheMissWRespNo, vsBackupHits=vsBackupHits, healthCheck=healthCheck, vhostName=vhostName, logNotificationsSent=logNotificationsSent, peakRcvdBytesPerSec=peakRcvdBytesPerSec, requestsToUrl=requestsToUrl, rsStatsTable=rsStatsTable, infInUnknownProtos=infInUnknownProtos, requestedConns=requestedConns, rsIndex=rsIndex, totalFail=totalFail, ifTraffic=ifTraffic, vsID=vsID, peakSentBytesPerSec=peakSentBytesPerSec, slbMIB=slbMIB, logHistTableMaxLength=logHistTableMaxLength, logHistory=logHistory, infOutUcastPkts=infOutUcastPkts, hcAddr=hcAddr, currentTime=currentTime, succPeakDay=succPeakDay, vsStaticHits=vsStaticHits, sslGeneral=sslGeneral, totalReq=totalReq, infInNUcastPkts=infInNUcastPkts, headRequests=headRequests, rsAvgRespTime=rsAvgRespTime, reqPeakDay=reqPeakDay, sentBytesPerSec=sentBytesPerSec, infInOctets=infInOctets, totalSucc=totalSucc, virtualServer=virtualServer, virtualAddress=virtualAddress, vsIpAddress=vsIpAddress, resumableSess=resumableSess, infInDiscards=infInDiscards, rsCount=rsCount, ipBytesIn=ipBytesIn, rsStatsEntry=rsStatsEntry, realPort=realPort, infOutErrors=infOutErrors, sdns=sdns, requestsReceived=requestsReceived, activeTransac=activeTransac, logHistoryTable=logHistoryTable, rsPort=rsPort, ctcpConnEntry=ctcpConnEntry, rsProtocol=rsProtocol, infTable=infTable, succPeakSec5=succPeakSec5, totalIPBytesOut=totalIPBytesOut, vsTable=vsTable, failLastHour=failLastHour, infTotalOutOctets=infTotalOutOctets, clusterVirAuthPasswd=clusterVirAuthPasswd, rsIpAddress=rsIpAddress, failLastMin=failLastMin, vsStatsTable=vsStatsTable, ipPktsOut=ipPktsOut, virtualIndex=virtualIndex, ipBytesOut=ipBytesOut, caTraps=caTraps, vsQosCookieHits=vsQosCookieHits, realStatus=realStatus, requestsOnRegex=requestsOnRegex, vsHeaderHits=vsHeaderHits, reqPeakHour=reqPeakHour, resumedSess=resumedSess, vrrp=vrrp, sslStats=sslStats, hcConnFail=hcConnFail, reqPeakSec=reqPeakSec, totalOpenSSLConns=totalOpenSSLConns, ctcpPassiveOpens=ctcpPassiveOpens, monitor=monitor, cacheSkip=cacheSkip, vsQosNetworkHits=vsQosNetworkHits, Float=Float, totalIPPktsIn=totalIPPktsIn, infInErrors=infInErrors, fanspeed=fanspeed, realAddr=realAddr, serialNumber=serialNumber, sslMIB=sslMIB, licenseRemainingDays=licenseRemainingDays, groupCurCfg=groupCurCfg)
mibBuilder.exportSymbols("CA-SNMP-MIB8", hostName=hostName, compression=compression, openSSLConns=openSSLConns, hitsReply=hitsReply, infTotalInOctets=infTotalInOctets, succPeakSec=succPeakSec, clusterVirInterval=clusterVirInterval, vsIndex=vsIndex, clientEstabConn=clientEstabConn, arrayNetworks=arrayNetworks, infDescr=infDescr, hcAddressType=hcAddressType, ipAddress=ipAddress, groupCount=groupCount, vsIpAddressType=vsIpAddressType, realStats=realStats, vhostNum=vhostNum, vsPort=vsPort, rsStatus=rsStatus, infIpv4Address=infIpv4Address, rsEntry=rsEntry, ctcpEstabResets=ctcpEstabResets, succLastDay=succLastDay, succLastHour=succLastHour, vsRegexHits=vsRegexHits, sysMemoryUtilization=sysMemoryUtilization, msgText=msgText, vsHashURLHits=vsHashURLHits, gpEntry=gpEntry, reqLastDay=reqLastDay, ipPktsIn=ipPktsIn, reqPeakMin=reqPeakMin, infAddress=infAddress, failLastSec=failLastSec, ctcpConnState=ctcpConnState, ctcpConnRemAddrType=ctcpConnRemAddrType, clusterVirAddressType=clusterVirAddressType, totalIPPktsOut=totalIPPktsOut, ctcpIndex=ctcpIndex, realID=realID, succPeakMin=succPeakMin, ipAddr=ipAddr, hcIndex=hcIndex, vsURLHits=vsURLHits, ctcpConnLocalAddr=ctcpConnLocalAddr, clusterNum=clusterNum, ctcpOutSegs=ctcpOutSegs, ipAddrType=ipAddrType, sslTable=sslTable, clusterVirAddr=clusterVirAddr, infOutNUcastPkts=infOutNUcastPkts, hcNumDowns=hcNumDowns, rsID=rsID, ipStatsTable=ipStatsTable, logBasic=logBasic)
