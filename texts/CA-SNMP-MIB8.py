...
Integer32, ModuleIdentity, Bits, NotificationType, Unsigned32, Counter32, iso, Counter64, TimeTicks, ObjectIdentity, Gauge32, IpAddress, Opaque, enterprises, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "ModuleIdentity", "Bits", "NotificationType", "Unsigned32", "Counter32", "iso", "Counter64", "TimeTicks", "ObjectIdentity", "Gauge32", "IpAddress", "Opaque", "enterprises", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", cpuUtilization=cpuUtilization, vsQosNetworkHits=vsQosNetworkHits, caStart=caStart, ipBytesIn=ipBytesIn, totalReq=totalReq, infOutOctets=infOutOctets, vsBackupHits=vsBackupHits, virtualServer=virtualServer, clusterNum=clusterNum, vsQosURLHits=vsQosURLHits, infTotalInOctets=infTotalInOctets, vsHostnameHits=vsHostnameHits, logNotificationsEnabled=logNotificationsEnabled, reqPeakSec=reqPeakSec, infDescr=infDescr, rsIpAddress=rsIpAddress, arrayNetworks=arrayNetworks, hcAddress=hcAddress, hostName=hostName, virtualAddr=virtualAddr, ctcpOutSegs=ctcpOutSegs, infInBcastPkts=infInBcastPkts, cacheHitRatio=cacheHitRatio, rsTable=rsTable, cacheMissWNoncacheReq=cacheMissWNoncacheReq, vsCount=vsCount, failLastHour=failLastHour, realID=realID, acceptedConns=acceptedConns, gpID=gpID, serverEstabConn=serverEstabConn, logBasic=logBasic, ctcpConnRemAddr=ctcpConnRemAddr, vsTable=vsTable, gpMetrics=gpMetrics, rsConnCnt=rsConnCnt, realAddressType=realAddressType, groupID=groupID, clusterVirAuthType=clusterVirAuthType, vsIpAddr=vsIpAddr, succLastDay=succLastDay, ipBytesOut=ipBytesOut, sslTable=sslTable, vsICookieHits=vsICookieHits, gpEntry=gpEntry, hcIndex=hcIndex, cacheStatus=cacheStatus, vsIndex=vsIndex, PYSNMP_MODULE_ID=arrayNetworks, syslogTrap=syslogTrap, reqPeakHour=reqPeakHour, gpTotalHits=gpTotalHits, virtualAddressType=virtualAddressType, succLastSec=succLastSec, cacheBasicStats=cacheBasicStats, gpIndex=gpIndex, cacheMissWNewEntry=cacheMissWNewEntry, sdns=sdns, ctcpCurrEstab=ctcpCurrEstab, infNumber=infNumber, hitRevalidate=hitRevalidate, healthCheck=healthCheck, realServer=realServer, realAddr=realAddr, infInOctets=infInOctets, reqLastMin=reqLastMin, revProxyCache=revProxyCache, peakSentBytesPerSec=peakSentBytesPerSec, succLastSec5=succLastSec5, powerstate=powerstate, infIpv4AddressType=infIpv4AddressType, activeTransac=activeTransac, infInErrors=infInErrors, infInBandwidth=infInBandwidth, startTime=startTime, serialNumber=serialNumber, vsURLHits=vsURLHits, caSyslog=caSyslog, hcConnFail=hcConnFail, vsMaxConn=vsMaxConn, logHistoryTable=logHistoryTable, ipStatsEntry=ipStatsEntry, infIndex=infIndex, logHistoryEntry=logHistoryEntry, ctcpConnLocalAddrType=ctcpConnLocalAddrType, getRequests=getRequests, vsPort=vsPort, rsPort=rsPort, vsHashURLHits=vsHashURLHits, sentBytesPerSec=sentBytesPerSec, clusterVirState=clusterVirState, totalIPBytesOut=totalIPBytesOut, ipAddress=ipAddress, hcName=hcName, ipPktsOut=ipPktsOut, virtualStats=virtualStats, vsID=vsID, clusterVirIndex=clusterVirIndex, slbGeneral=slbGeneral, hcRSCount=hcRSCount, realAddress=realAddress, cacheSkip=cacheSkip, succPeakDay=succPeakDay, infAddress=infAddress, infOutBandwidth=infOutBandwidth, infInUcastPkts=infInUcastPkts, vsProtocol=vsProtocol, connectionsPerSec=connectionsPerSec, totalRequestedConns=totalRequestedConns, licenseRemainingDays=licenseRemainingDays, clusterVirAuthPasswd=clusterVirAuthPasswd, vsRedirectHits=vsRedirectHits, responsesOnRegex=responsesOnRegex, vsHeaderHits=vsHeaderHits, sslIndex=sslIndex, sysSwapCapacity=sysSwapCapacity, rsStatus=rsStatus, hitsReplyWPreFailed=hitsReplyWPreFailed, ipAddrType=ipAddrType, currentTime=currentTime, vhostName=vhostName, vrrpTable=vrrpTable, vrrp=vrrp, sysSwapUsed=sysSwapUsed, caShutdown=caShutdown, hcNumUps=hcNumUps, clusterVirAddressType=clusterVirAddressType, vsEntry=vsEntry, monitor=monitor, groupIndex=groupIndex, rsStatsTable=rsStatsTable, ipPktsIn=ipPktsIn, virtualPort=virtualPort, realIndex=realIndex, groupCount=groupCount, rsMaxConn=rsMaxConn, vsPerstntCookieHits=vsPerstntCookieHits, vhostNum=vhostNum, infOutMcastPkts=infOutMcastPkts, ctcpConnRemAddress=ctcpConnRemAddress, hcConnSuccess=hcConnSuccess, ctcpIndex=ctcpIndex, virtualAddress=virtualAddress, hcCause=hcCause, gpTable=gpTable, failLastDay=failLastDay, requestsReceived=requestsReceived, rsEntry=rsEntry, hcAddressType=hcAddressType, slbMIB=slbMIB, infTable=infTable, cputemp=cputemp, infIpv4Address=infIpv4Address, hcConnAttempt=hcConnAttempt, ctcpRetransSegs=ctcpRetransSegs, systemInfo=systemInfo, sysMemoryUtilization=sysMemoryUtilization, infOutNUcastPkts=infOutNUcastPkts, rsIpAddressType=rsIpAddressType, groupStats=groupStats, purgeRequests=purgeRequests, ipStatsTable=ipStatsTable, ifTraffic=ifTraffic, ctcpConnLocalAddress=ctcpConnLocalAddress, reqLastHour=reqLastHour, ipIndex=ipIndex, caSyslogTrap=caSyslogTrap, vipStatus=vipStatus, ctcpConnRemAddrType=ctcpConnRemAddrType, hcStats=hcStats, connsPerSec=connsPerSec, msgText=msgText, succPeakSec=succPeakSec, vsIpAddressType=vsIpAddressType, ctcpEstabResets=ctcpEstabResets, sslStats=sslStats, ipAddr=ipAddr, maxCluster=maxCluster, vsStaticHits=vsStaticHits, rsIpAddr=rsIpAddr, totalOpenSSLConns=totalOpenSSLConns, infOutBcastPkts=infOutBcastPkts, infEntry=infEntry, vsQosBodyHits=vsQosBodyHits, infIpv6AddressType=infIpv6AddressType, reqPeakMin=reqPeakMin, infTotalOutOctets=infTotalOutOctets, groupCurCfg=groupCurCfg, hcStatsEntry=hcStatsEntry, SyslogSeverity=SyslogSeverity, requestedConns=requestedConns, infOutErrors=infOutErrors, clusterVrrp=clusterVrrp, vrrpEntry=vrrpEntry, vsRCookieHits=vsRCookieHits, rsCntOfReq=rsCntOfReq, compression=compression, vsRegexHits=vsRegexHits, hcPort=hcPort, Float=Float, requestsPerSec=requestsPerSec, totalIPPktsOut=totalIPPktsOut, reqLastSec=reqLastSec, ctcpConnLocalPort=ctcpConnLocalPort, hitsReplyWNotModified=hitsReplyWNotModified, logMaxSeverity=logMaxSeverity, ctcpConnState=ctcpConnState, failLastMin=failLastMin, ctcpInErrs=ctcpInErrs, index=index, sslMIB=sslMIB, vsStatsTable=vsStatsTable, totalFail=totalFail, requestsToUrl=requestsToUrl, infOperStatus=infOperStatus, hcNumDowns=hcNumDowns, ctcpOutRsts=ctcpOutRsts, requestsOnRegex=requestsOnRegex, resumableSess=resumableSess, vsConnCnt=vsConnCnt, hcStatsTable=hcStatsTable, rsTotalHits=rsTotalHits, rsAvgRespTime=rsAvgRespTime, failLastSec5=failLastSec5, logHistTableMaxLength=logHistTableMaxLength, openSSLConns=openSSLConns, cacheMissWRespNo=cacheMissWRespNo, requestsToHttps=requestsToHttps, realStatus=realStatus, totalSucc=totalSucc, clusterVirPriority=clusterVirPriority, rsIndex=rsIndex, ctcpConnRemPort=ctcpConnRemPort, succLastHour=succLastHour, rsID=rsID, performance=performance, sslGeneral=sslGeneral, succPeakMin=succPeakMin, clusterVirPreempt=clusterVirPreempt, clusterVirInterval=clusterVirInterval, peakRcvdBytesPerSec=peakRcvdBytesPerSec, rsCount=rsCount, vipStats=vipStats, sysMemory=sysMemory, hitsReply=hitsReply, reqPeakDay=reqPeakDay, vsPerstntURLHits=vsPerstntURLHits, virtServerID=virtServerID, rsProtocol=rsProtocol, memory=memory, infInNUcastPkts=infInNUcastPkts, totalIPPktsIn=totalIPPktsIn, caTraps=caTraps, ctcpActiveOpens=ctcpActiveOpens)
mibBuilder.exportSymbols("CA-SNMP-MIB8", missSess=missSess, rcvdBytesPerSec=rcvdBytesPerSec, logHistory=logHistory, infInDiscards=infInDiscards, ctcpInSegs=ctcpInSegs, infOutUcastPkts=infOutUcastPkts, sslEntry=sslEntry, totalBytesSent=totalBytesSent, failLastSec=failLastSec, ctcpConnTable=ctcpConnTable, vsQosCookieHits=vsQosCookieHits, reqPeakSec5=reqPeakSec5, vsIpAddress=vsIpAddress, totalBytesRcvd=totalBytesRcvd, virtualIndex=virtualIndex, realStats=realStats, totalIPBytesIn=totalIPBytesIn, ctcpConnEntry=ctcpConnEntry, headRequests=headRequests, clusterVirAddr=clusterVirAddr, responsesToHttps=responsesToHttps, clusterVirAddress=clusterVirAddress, gpStatsEntry=gpStatsEntry, resumedSess=resumedSess, gpStatsTable=gpStatsTable, realPort=realPort, succLastMin=succLastMin, fanspeed=fanspeed, ctcpConnLocalAddr=ctcpConnLocalAddr, ctcpAttemptFails=ctcpAttemptFails, reqLastDay=reqLastDay, vsQosClientPortHits=vsQosClientPortHits, ctcpPassiveOpens=ctcpPassiveOpens, infInMcastPkts=infInMcastPkts, realServerID=realServerID, rsStatsEntry=rsStatsEntry, infInUnknownProtos=infInUnknownProtos, vsStatsEntry=vsStatsEntry, clusterVirIfname=clusterVirIfname, hcAddr=hcAddr, hcStatus=hcStatus, clientEstabConn=clientEstabConn, slbStats=slbStats, totalAcceptedConns=totalAcceptedConns, postRequests=postRequests, clusterId=clusterId, succPeakSec5=succPeakSec5, logNotificationsSent=logNotificationsSent, severity=severity, reqLastSec5=reqLastSec5, vsDefaultHits=vsDefaultHits, clickTcp=clickTcp, vsCacheHits=vsCacheHits, succPeakHour=succPeakHour, infIpv6Address=infIpv6Address)
