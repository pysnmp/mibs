...
MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Gauge32, Counter32, TimeTicks, ModuleIdentity, Unsigned32, NotificationType, Opaque, IpAddress, Bits, ObjectIdentity, MibIdentifier, enterprises, Integer32, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Gauge32", "Counter32", "TimeTicks", "ModuleIdentity", "Unsigned32", "NotificationType", "Opaque", "IpAddress", "Bits", "ObjectIdentity", "MibIdentifier", "enterprises", "Integer32", "Counter64")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", clusterVirAddressType=clusterVirAddressType, cacheHitRatio=cacheHitRatio, hcAddress=hcAddress, realStats=realStats, ipBytesOut=ipBytesOut, hcStatus=hcStatus, failLastSec=failLastSec, msgText=msgText, ctcpOutRsts=ctcpOutRsts, ctcpOutSegs=ctcpOutSegs, virtualStats=virtualStats, ctcpConnState=ctcpConnState, totalSucc=totalSucc, vsRedirectHits=vsRedirectHits, realPort=realPort, reqLastMin=reqLastMin, infIpv4Address=infIpv4Address, clusterVirAuthType=clusterVirAuthType, ctcpConnLocalPort=ctcpConnLocalPort, ctcpRetransSegs=ctcpRetransSegs, vipStats=vipStats, vipStatus=vipStatus, vsIndex=vsIndex, rsEntry=rsEntry, infOutErrors=infOutErrors, clusterVrrp=clusterVrrp, gpID=gpID, hitsReply=hitsReply, clientEstabConn=clientEstabConn, infOutBcastPkts=infOutBcastPkts, totalOpenSSLConns=totalOpenSSLConns, responsesOnRegex=responsesOnRegex, vsQosBodyHits=vsQosBodyHits, sysMemoryUtilization=sysMemoryUtilization, hcStatsTable=hcStatsTable, vsDefaultHits=vsDefaultHits, gpStatsTable=gpStatsTable, requestsReceived=requestsReceived, succLastDay=succLastDay, hcStats=hcStats, totalReq=totalReq, vsIpAddressType=vsIpAddressType, sdns=sdns, virtualAddressType=virtualAddressType, realServer=realServer, gpTotalHits=gpTotalHits, severity=severity, vsIpAddr=vsIpAddr, sentBytesPerSec=sentBytesPerSec, infDescr=infDescr, clusterId=clusterId, serialNumber=serialNumber, requestsToHttps=requestsToHttps, infTotalOutOctets=infTotalOutOctets, vsCacheHits=vsCacheHits, logMaxSeverity=logMaxSeverity, vsStatsTable=vsStatsTable, infInUcastPkts=infInUcastPkts, rsID=rsID, totalIPBytesOut=totalIPBytesOut, clickTcp=clickTcp, sslIndex=sslIndex, totalBytesRcvd=totalBytesRcvd, infInUnknownProtos=infInUnknownProtos, vsQosClientPortHits=vsQosClientPortHits, hcConnFail=hcConnFail, clusterVirState=clusterVirState, vsProtocol=vsProtocol, ctcpCurrEstab=ctcpCurrEstab, ipPktsOut=ipPktsOut, performance=performance, ipAddress=ipAddress, infOutUcastPkts=infOutUcastPkts, reqPeakHour=reqPeakHour, cacheSkip=cacheSkip, succLastMin=succLastMin, slbGeneral=slbGeneral, cacheMissWNewEntry=cacheMissWNewEntry, logBasic=logBasic, infOutMcastPkts=infOutMcastPkts, infIpv6Address=infIpv6Address, vsRegexHits=vsRegexHits, index=index, peakSentBytesPerSec=peakSentBytesPerSec, clusterVirInterval=clusterVirInterval, ctcpConnRemAddress=ctcpConnRemAddress, requestsOnRegex=requestsOnRegex, vsURLHits=vsURLHits, reqLastSec=reqLastSec, Float=Float, groupStats=groupStats, clusterNum=clusterNum, caSyslog=caSyslog, vsCount=vsCount, ctcpActiveOpens=ctcpActiveOpens, ctcpConnEntry=ctcpConnEntry, rsIpAddr=rsIpAddr, ctcpConnRemAddrType=ctcpConnRemAddrType, hitsReplyWNotModified=hitsReplyWNotModified, reqPeakMin=reqPeakMin, reqPeakSec5=reqPeakSec5, clusterVirIfname=clusterVirIfname, rsStatus=rsStatus, vsEntry=vsEntry, infInOctets=infInOctets, reqPeakSec=reqPeakSec, rsIndex=rsIndex, totalRequestedConns=totalRequestedConns, succPeakDay=succPeakDay, infIpv4AddressType=infIpv4AddressType, hcName=hcName, realServerID=realServerID, succLastSec5=succLastSec5, vsHeaderHits=vsHeaderHits, rsProtocol=rsProtocol, vsStaticHits=vsStaticHits, ipIndex=ipIndex, vsRCookieHits=vsRCookieHits, caShutdown=caShutdown, caStart=caStart, requestsPerSec=requestsPerSec, ctcpIndex=ctcpIndex, rsStatsEntry=rsStatsEntry, sysMemory=sysMemory, realAddressType=realAddressType, vsPerstntURLHits=vsPerstntURLHits, ctcpInSegs=ctcpInSegs, vhostName=vhostName, succPeakSec5=succPeakSec5, cpuUtilization=cpuUtilization, ctcpConnRemAddr=ctcpConnRemAddr, vsHostnameHits=vsHostnameHits, syslogTrap=syslogTrap, infIpv6AddressType=infIpv6AddressType, cacheBasicStats=cacheBasicStats, caSyslogTrap=caSyslogTrap, gpTable=gpTable, ipBytesIn=ipBytesIn, hcConnAttempt=hcConnAttempt, hcPort=hcPort, ctcpPassiveOpens=ctcpPassiveOpens, SyslogSeverity=SyslogSeverity, slbMIB=slbMIB, hcStatsEntry=hcStatsEntry, clusterVirAuthPasswd=clusterVirAuthPasswd, reqLastSec5=reqLastSec5, sysSwapUsed=sysSwapUsed, postRequests=postRequests, gpStatsEntry=gpStatsEntry, acceptedConns=acceptedConns, infInNUcastPkts=infInNUcastPkts, infOperStatus=infOperStatus, infInDiscards=infInDiscards, clusterVirIndex=clusterVirIndex, gpMetrics=gpMetrics, vsPerstntCookieHits=vsPerstntCookieHits, infEntry=infEntry, fanspeed=fanspeed, groupID=groupID, infOutOctets=infOutOctets, reqPeakDay=reqPeakDay, ctcpEstabResets=ctcpEstabResets, rsCntOfReq=rsCntOfReq, vrrpEntry=vrrpEntry, openSSLConns=openSSLConns, requestedConns=requestedConns, vsQosNetworkHits=vsQosNetworkHits, startTime=startTime, responsesToHttps=responsesToHttps, serverEstabConn=serverEstabConn, hcCause=hcCause, arrayNetworks=arrayNetworks, vsICookieHits=vsICookieHits, logNotificationsSent=logNotificationsSent, rsIpAddress=rsIpAddress, ipPktsIn=ipPktsIn, rsPort=rsPort, vsID=vsID, caTraps=caTraps, clusterVirAddr=clusterVirAddr, rsIpAddressType=rsIpAddressType, vsTable=vsTable, gpIndex=gpIndex, infInMcastPkts=infInMcastPkts, realIndex=realIndex, infInErrors=infInErrors, rsAvgRespTime=rsAvgRespTime, logHistoryEntry=logHistoryEntry, groupCurCfg=groupCurCfg, vhostNum=vhostNum, infInBcastPkts=infInBcastPkts, healthCheck=healthCheck, rsConnCnt=rsConnCnt, hitsReplyWPreFailed=hitsReplyWPreFailed, succLastSec=succLastSec, ipStatsTable=ipStatsTable, rsCount=rsCount, infAddress=infAddress, ctcpAttemptFails=ctcpAttemptFails, maxCluster=maxCluster, vrrp=vrrp, getRequests=getRequests, hcAddressType=hcAddressType, infInBandwidth=infInBandwidth, sslTable=sslTable, rsStatsTable=rsStatsTable, hostName=hostName, ipAddr=ipAddr, logHistTableMaxLength=logHistTableMaxLength, hcNumDowns=hcNumDowns, succPeakHour=succPeakHour, hcIndex=hcIndex, virtualIndex=virtualIndex, sslEntry=sslEntry, infIndex=infIndex, activeTransac=activeTransac, sslGeneral=sslGeneral, purgeRequests=purgeRequests, reqLastDay=reqLastDay, slbStats=slbStats, vsStatsEntry=vsStatsEntry, failLastSec5=failLastSec5, failLastHour=failLastHour, clusterVirAddress=clusterVirAddress, headRequests=headRequests, realID=realID, virtualAddr=virtualAddr, infNumber=infNumber, vrrpTable=vrrpTable, clusterVirPriority=clusterVirPriority, hcAddr=hcAddr, memory=memory, ctcpConnRemPort=ctcpConnRemPort, virtualPort=virtualPort, rsTotalHits=rsTotalHits, rsTable=rsTable, ctcpConnTable=ctcpConnTable, vsConnCnt=vsConnCnt, connectionsPerSec=connectionsPerSec, rcvdBytesPerSec=rcvdBytesPerSec, vsMaxConn=vsMaxConn, logHistory=logHistory, revProxyCache=revProxyCache, infOutBandwidth=infOutBandwidth, vsBackupHits=vsBackupHits, virtualServer=virtualServer, sslStats=sslStats, ctcpConnLocalAddr=ctcpConnLocalAddr, ctcpConnLocalAddress=ctcpConnLocalAddress, realAddress=realAddress, failLastMin=failLastMin, resumedSess=resumedSess, vsPort=vsPort, powerstate=powerstate)
mibBuilder.exportSymbols("CA-SNMP-MIB8", failLastDay=failLastDay, succLastHour=succLastHour, infTable=infTable, gpEntry=gpEntry, logHistoryTable=logHistoryTable, infTotalInOctets=infTotalInOctets, ctcpConnLocalAddrType=ctcpConnLocalAddrType, systemInfo=systemInfo, vsIpAddress=vsIpAddress, ipAddrType=ipAddrType, virtServerID=virtServerID, totalBytesSent=totalBytesSent, cacheMissWRespNo=cacheMissWRespNo, ifTraffic=ifTraffic, licenseRemainingDays=licenseRemainingDays, groupCount=groupCount, monitor=monitor, currentTime=currentTime, hcNumUps=hcNumUps, realAddr=realAddr, cacheMissWNoncacheReq=cacheMissWNoncacheReq, vsQosCookieHits=vsQosCookieHits, totalFail=totalFail, realStatus=realStatus, sslMIB=sslMIB, ctcpInErrs=ctcpInErrs, reqLastHour=reqLastHour, hitRevalidate=hitRevalidate, succPeakSec=succPeakSec, cacheStatus=cacheStatus, resumableSess=resumableSess, infOutNUcastPkts=infOutNUcastPkts, ipStatsEntry=ipStatsEntry, groupIndex=groupIndex, vsHashURLHits=vsHashURLHits, logNotificationsEnabled=logNotificationsEnabled, vsQosURLHits=vsQosURLHits, compression=compression, peakRcvdBytesPerSec=peakRcvdBytesPerSec, rsMaxConn=rsMaxConn, totalIPPktsIn=totalIPPktsIn, totalIPPktsOut=totalIPPktsOut, sysSwapCapacity=sysSwapCapacity, virtualAddress=virtualAddress, clusterVirPreempt=clusterVirPreempt, succPeakMin=succPeakMin, connsPerSec=connsPerSec, requestsToUrl=requestsToUrl, PYSNMP_MODULE_ID=arrayNetworks, hcRSCount=hcRSCount, totalIPBytesIn=totalIPBytesIn, hcConnSuccess=hcConnSuccess, missSess=missSess, cputemp=cputemp, totalAcceptedConns=totalAcceptedConns)
