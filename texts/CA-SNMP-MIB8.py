...
Bits, TimeTicks, Counter64, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, Integer32, NotificationType, Counter32, Opaque, enterprises, iso, IpAddress, Unsigned32, ModuleIdentity, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "TimeTicks", "Counter64", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "Integer32", "NotificationType", "Counter32", "Opaque", "enterprises", "iso", "IpAddress", "Unsigned32", "ModuleIdentity", "ObjectIdentity")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", rcvdBytesPerSec=rcvdBytesPerSec, infTotalOutOctets=infTotalOutOctets, totalAcceptedConns=totalAcceptedConns, hcAddress=hcAddress, vsQosCookieHits=vsQosCookieHits, SyslogSeverity=SyslogSeverity, currentTime=currentTime, rsIndex=rsIndex, caSyslogTrap=caSyslogTrap, vhostNum=vhostNum, realID=realID, sslMIB=sslMIB, slbGeneral=slbGeneral, requestsOnRegex=requestsOnRegex, reqLastSec5=reqLastSec5, requestsReceived=requestsReceived, logHistoryEntry=logHistoryEntry, vsHashURLHits=vsHashURLHits, infOutMcastPkts=infOutMcastPkts, clusterVirAddress=clusterVirAddress, virtualPort=virtualPort, clusterId=clusterId, vsBackupHits=vsBackupHits, clusterVirIndex=clusterVirIndex, clickTcp=clickTcp, acceptedConns=acceptedConns, succLastMin=succLastMin, caTraps=caTraps, totalSucc=totalSucc, reqPeakHour=reqPeakHour, msgText=msgText, infInOctets=infInOctets, ipBytesIn=ipBytesIn, vsHostnameHits=vsHostnameHits, vsQosBodyHits=vsQosBodyHits, ctcpConnLocalAddress=ctcpConnLocalAddress, clientEstabConn=clientEstabConn, ctcpActiveOpens=ctcpActiveOpens, infTable=infTable, gpStatsEntry=gpStatsEntry, succPeakDay=succPeakDay, clusterVirIfname=clusterVirIfname, ipStatsEntry=ipStatsEntry, logNotificationsSent=logNotificationsSent, requestsPerSec=requestsPerSec, ctcpPassiveOpens=ctcpPassiveOpens, maxCluster=maxCluster, sdns=sdns, totalIPBytesIn=totalIPBytesIn, licenseRemainingDays=licenseRemainingDays, vsCacheHits=vsCacheHits, hcAddressType=hcAddressType, ifTraffic=ifTraffic, ctcpCurrEstab=ctcpCurrEstab, cacheMissWNewEntry=cacheMissWNewEntry, reqPeakSec5=reqPeakSec5, virtualStats=virtualStats, virtualAddressType=virtualAddressType, cacheMissWNoncacheReq=cacheMissWNoncacheReq, groupStats=groupStats, vipStatus=vipStatus, realPort=realPort, ipPktsOut=ipPktsOut, hitsReply=hitsReply, fanspeed=fanspeed, ctcpConnEntry=ctcpConnEntry, headRequests=headRequests, infInMcastPkts=infInMcastPkts, logBasic=logBasic, ctcpInErrs=ctcpInErrs, reqLastDay=reqLastDay, hcConnAttempt=hcConnAttempt, infInBcastPkts=infInBcastPkts, ctcpConnLocalPort=ctcpConnLocalPort, rsID=rsID, vsRedirectHits=vsRedirectHits, clusterVirAddr=clusterVirAddr, openSSLConns=openSSLConns, performance=performance, infNumber=infNumber, infInUnknownProtos=infInUnknownProtos, realStats=realStats, sslEntry=sslEntry, hcStatsEntry=hcStatsEntry, vipStats=vipStats, sysMemory=sysMemory, monitor=monitor, totalOpenSSLConns=totalOpenSSLConns, caSyslog=caSyslog, logMaxSeverity=logMaxSeverity, clusterVirAddressType=clusterVirAddressType, vsStatsTable=vsStatsTable, gpTotalHits=gpTotalHits, infOperStatus=infOperStatus, vsPort=vsPort, ctcpConnTable=ctcpConnTable, vsHeaderHits=vsHeaderHits, hcRSCount=hcRSCount, gpEntry=gpEntry, realStatus=realStatus, virtServerID=virtServerID, cacheSkip=cacheSkip, hcName=hcName, succLastSec5=succLastSec5, index=index, groupCurCfg=groupCurCfg, sslStats=sslStats, missSess=missSess, vrrp=vrrp, totalIPPktsOut=totalIPPktsOut, rsConnCnt=rsConnCnt, hcConnSuccess=hcConnSuccess, failLastSec=failLastSec, revProxyCache=revProxyCache, reqPeakDay=reqPeakDay, clusterVrrp=clusterVrrp, rsPort=rsPort, rsIpAddress=rsIpAddress, rsStatus=rsStatus, sysMemoryUtilization=sysMemoryUtilization, infOutUcastPkts=infOutUcastPkts, ipIndex=ipIndex, ipBytesOut=ipBytesOut, ctcpConnState=ctcpConnState, realAddressType=realAddressType, purgeRequests=purgeRequests, vsStatsEntry=vsStatsEntry, groupIndex=groupIndex, logNotificationsEnabled=logNotificationsEnabled, requestsToHttps=requestsToHttps, serverEstabConn=serverEstabConn, activeTransac=activeTransac, succLastHour=succLastHour, rsEntry=rsEntry, vsEntry=vsEntry, powerstate=powerstate, ctcpConnLocalAddr=ctcpConnLocalAddr, infInDiscards=infInDiscards, vsStaticHits=vsStaticHits, hcPort=hcPort, vsIpAddr=vsIpAddr, vsQosURLHits=vsQosURLHits, slbStats=slbStats, rsCount=rsCount, ctcpRetransSegs=ctcpRetransSegs, infIpv6Address=infIpv6Address, infIpv4Address=infIpv4Address, vsRCookieHits=vsRCookieHits, succPeakMin=succPeakMin, hcConnFail=hcConnFail, infOutBcastPkts=infOutBcastPkts, vsIndex=vsIndex, ctcpConnRemAddr=ctcpConnRemAddr, succPeakHour=succPeakHour, clusterVirInterval=clusterVirInterval, rsCntOfReq=rsCntOfReq, realAddress=realAddress, peakRcvdBytesPerSec=peakRcvdBytesPerSec, gpID=gpID, sysSwapCapacity=sysSwapCapacity, memory=memory, totalReq=totalReq, virtualServer=virtualServer, realIndex=realIndex, resumedSess=resumedSess, syslogTrap=syslogTrap, requestedConns=requestedConns, clusterVirPriority=clusterVirPriority, totalBytesSent=totalBytesSent, requestsToUrl=requestsToUrl, rsIpAddressType=rsIpAddressType, succLastDay=succLastDay, ipAddrType=ipAddrType, ctcpAttemptFails=ctcpAttemptFails, vsConnCnt=vsConnCnt, vsTable=vsTable, reqPeakMin=reqPeakMin, ipPktsIn=ipPktsIn, postRequests=postRequests, infOutNUcastPkts=infOutNUcastPkts, virtualAddress=virtualAddress, vsIpAddress=vsIpAddress, vsRegexHits=vsRegexHits, ctcpEstabResets=ctcpEstabResets, ctcpOutRsts=ctcpOutRsts, slbMIB=slbMIB, vrrpTable=vrrpTable, vsMaxConn=vsMaxConn, vsURLHits=vsURLHits, sslTable=sslTable, responsesToHttps=responsesToHttps, reqLastMin=reqLastMin, infDescr=infDescr, vsQosNetworkHits=vsQosNetworkHits, ipStatsTable=ipStatsTable, vsIpAddressType=vsIpAddressType, reqPeakSec=reqPeakSec, gpMetrics=gpMetrics, rsStatsTable=rsStatsTable, clusterVirAuthPasswd=clusterVirAuthPasswd, hitsReplyWNotModified=hitsReplyWNotModified, totalIPBytesOut=totalIPBytesOut, logHistory=logHistory, caShutdown=caShutdown, hcAddr=hcAddr, totalFail=totalFail, infIpv6AddressType=infIpv6AddressType, totalRequestedConns=totalRequestedConns, ctcpConnRemAddrType=ctcpConnRemAddrType, resumableSess=resumableSess, ctcpIndex=ctcpIndex, vrrpEntry=vrrpEntry, ctcpConnRemAddress=ctcpConnRemAddress, virtualIndex=virtualIndex, vsPerstntURLHits=vsPerstntURLHits, failLastSec5=failLastSec5, reqLastSec=reqLastSec, logHistTableMaxLength=logHistTableMaxLength, hitsReplyWPreFailed=hitsReplyWPreFailed, cacheHitRatio=cacheHitRatio, vsID=vsID, failLastHour=failLastHour, realAddr=realAddr, getRequests=getRequests, vhostName=vhostName, sysSwapUsed=sysSwapUsed, clusterVirState=clusterVirState, logHistoryTable=logHistoryTable, gpTable=gpTable, rsTotalHits=rsTotalHits, reqLastHour=reqLastHour, rsProtocol=rsProtocol, failLastDay=failLastDay, PYSNMP_MODULE_ID=arrayNetworks, clusterVirPreempt=clusterVirPreempt, cacheStatus=cacheStatus, ctcpConnRemPort=ctcpConnRemPort, connectionsPerSec=connectionsPerSec, virtualAddr=virtualAddr, hcStatsTable=hcStatsTable, ctcpConnLocalAddrType=ctcpConnLocalAddrType, clusterVirAuthType=clusterVirAuthType, cpuUtilization=cpuUtilization, succLastSec=succLastSec, succPeakSec=succPeakSec, cacheBasicStats=cacheBasicStats, totalBytesRcvd=totalBytesRcvd, healthCheck=healthCheck, compression=compression, infInUcastPkts=infInUcastPkts, ctcpInSegs=ctcpInSegs, ctcpOutSegs=ctcpOutSegs, groupID=groupID, vsProtocol=vsProtocol)
mibBuilder.exportSymbols("CA-SNMP-MIB8", peakSentBytesPerSec=peakSentBytesPerSec, infIndex=infIndex, sslGeneral=sslGeneral, infTotalInOctets=infTotalInOctets, rsStatsEntry=rsStatsEntry, systemInfo=systemInfo, hcIndex=hcIndex, vsPerstntCookieHits=vsPerstntCookieHits, totalIPPktsIn=totalIPPktsIn, infOutOctets=infOutOctets, vsDefaultHits=vsDefaultHits, groupCount=groupCount, severity=severity, hcCause=hcCause, sentBytesPerSec=sentBytesPerSec, realServer=realServer, cacheMissWRespNo=cacheMissWRespNo, clusterNum=clusterNum, rsIpAddr=rsIpAddr, rsAvgRespTime=rsAvgRespTime, infInErrors=infInErrors, hitRevalidate=hitRevalidate, infAddress=infAddress, vsICookieHits=vsICookieHits, Float=Float, serialNumber=serialNumber, vsCount=vsCount, succPeakSec5=succPeakSec5, hcNumDowns=hcNumDowns, realServerID=realServerID, hostName=hostName, arrayNetworks=arrayNetworks, gpStatsTable=gpStatsTable, failLastMin=failLastMin, infOutErrors=infOutErrors, caStart=caStart, hcStatus=hcStatus, sslIndex=sslIndex, vsQosClientPortHits=vsQosClientPortHits, ipAddr=ipAddr, responsesOnRegex=responsesOnRegex, infInBandwidth=infInBandwidth, cputemp=cputemp, startTime=startTime, infInNUcastPkts=infInNUcastPkts, infIpv4AddressType=infIpv4AddressType, connsPerSec=connsPerSec, infOutBandwidth=infOutBandwidth, ipAddress=ipAddress, rsMaxConn=rsMaxConn, infEntry=infEntry, gpIndex=gpIndex, hcStats=hcStats, rsTable=rsTable, hcNumUps=hcNumUps)
