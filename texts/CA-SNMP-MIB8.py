...
ModuleIdentity, TimeTicks, Counter64, NotificationType, Counter32, ObjectIdentity, Gauge32, Unsigned32, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, Bits, Opaque, Integer32, iso, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "TimeTicks", "Counter64", "NotificationType", "Counter32", "ObjectIdentity", "Gauge32", "Unsigned32", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "Bits", "Opaque", "Integer32", "iso", "IpAddress")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", clusterVrrp=clusterVrrp, sysSwapUsed=sysSwapUsed, rcvdBytesPerSec=rcvdBytesPerSec, hostName=hostName, sysMemoryUtilization=sysMemoryUtilization, infInBandwidth=infInBandwidth, hcStatus=hcStatus, infInDiscards=infInDiscards, vsEntry=vsEntry, revProxyCache=revProxyCache, ctcpRetransSegs=ctcpRetransSegs, ctcpCurrEstab=ctcpCurrEstab, sslGeneral=sslGeneral, rsConnCnt=rsConnCnt, succLastDay=succLastDay, vrrpTable=vrrpTable, sslEntry=sslEntry, caTraps=caTraps, ifTraffic=ifTraffic, reqLastHour=reqLastHour, totalBytesRcvd=totalBytesRcvd, vsStatsTable=vsStatsTable, clusterId=clusterId, infOutOctets=infOutOctets, gpTotalHits=gpTotalHits, logNotificationsSent=logNotificationsSent, rsID=rsID, resumedSess=resumedSess, vsDefaultHits=vsDefaultHits, realID=realID, infEntry=infEntry, sslIndex=sslIndex, cacheMissWNewEntry=cacheMissWNewEntry, virtualPort=virtualPort, failLastSec5=failLastSec5, caSyslogTrap=caSyslogTrap, succPeakHour=succPeakHour, ctcpActiveOpens=ctcpActiveOpens, infInErrors=infInErrors, infOutNUcastPkts=infOutNUcastPkts, succLastSec5=succLastSec5, succPeakDay=succPeakDay, vrrpEntry=vrrpEntry, systemInfo=systemInfo, PYSNMP_MODULE_ID=arrayNetworks, succLastSec=succLastSec, vsStaticHits=vsStaticHits, SyslogSeverity=SyslogSeverity, ctcpPassiveOpens=ctcpPassiveOpens, totalSucc=totalSucc, totalReq=totalReq, totalIPPktsIn=totalIPPktsIn, gpStatsTable=gpStatsTable, connsPerSec=connsPerSec, reqLastSec5=reqLastSec5, hcCause=hcCause, vsIpAddress=vsIpAddress, failLastSec=failLastSec, vsCount=vsCount, hcConnAttempt=hcConnAttempt, vsID=vsID, ctcpConnTable=ctcpConnTable, ipPktsIn=ipPktsIn, hcStatsEntry=hcStatsEntry, hitsReplyWNotModified=hitsReplyWNotModified, caSyslog=caSyslog, infTotalOutOctets=infTotalOutOctets, rsIpAddressType=rsIpAddressType, gpTable=gpTable, ipBytesOut=ipBytesOut, infInNUcastPkts=infInNUcastPkts, reqPeakSec=reqPeakSec, succPeakSec=succPeakSec, virtualAddress=virtualAddress, infIndex=infIndex, vrrp=vrrp, vsRedirectHits=vsRedirectHits, activeTransac=activeTransac, rsIndex=rsIndex, failLastMin=failLastMin, cacheMissWNoncacheReq=cacheMissWNoncacheReq, licenseRemainingDays=licenseRemainingDays, vsPort=vsPort, infTotalInOctets=infTotalInOctets, sentBytesPerSec=sentBytesPerSec, ctcpInErrs=ctcpInErrs, realServerID=realServerID, totalOpenSSLConns=totalOpenSSLConns, ctcpInSegs=ctcpInSegs, slbMIB=slbMIB, clusterVirAddr=clusterVirAddr, totalFail=totalFail, succLastHour=succLastHour, vsQosBodyHits=vsQosBodyHits, groupStats=groupStats, vipStatus=vipStatus, gpStatsEntry=gpStatsEntry, postRequests=postRequests, ctcpConnLocalAddrType=ctcpConnLocalAddrType, virtualAddr=virtualAddr, clientEstabConn=clientEstabConn, cacheMissWRespNo=cacheMissWRespNo, groupID=groupID, rsCount=rsCount, hcAddress=hcAddress, succPeakSec5=succPeakSec5, infOutBandwidth=infOutBandwidth, gpEntry=gpEntry, vsRCookieHits=vsRCookieHits, sslStats=sslStats, sysSwapCapacity=sysSwapCapacity, hitsReplyWPreFailed=hitsReplyWPreFailed, infDescr=infDescr, clusterVirAddressType=clusterVirAddressType, hitRevalidate=hitRevalidate, sslMIB=sslMIB, ipPktsOut=ipPktsOut, hcNumDowns=hcNumDowns, logHistoryEntry=logHistoryEntry, powerstate=powerstate, vsIndex=vsIndex, reqLastDay=reqLastDay, purgeRequests=purgeRequests, rsIpAddr=rsIpAddr, index=index, performance=performance, startTime=startTime, cacheBasicStats=cacheBasicStats, slbStats=slbStats, infOperStatus=infOperStatus, logBasic=logBasic, realIndex=realIndex, realAddressType=realAddressType, vsCacheHits=vsCacheHits, responsesToHttps=responsesToHttps, infInMcastPkts=infInMcastPkts, rsMaxConn=rsMaxConn, reqPeakHour=reqPeakHour, clusterVirIfname=clusterVirIfname, monitor=monitor, vsQosURLHits=vsQosURLHits, ctcpOutRsts=ctcpOutRsts, ipAddr=ipAddr, ctcpConnRemAddrType=ctcpConnRemAddrType, caStart=caStart, succPeakMin=succPeakMin, vsIpAddr=vsIpAddr, requestsToHttps=requestsToHttps, serialNumber=serialNumber, vsHostnameHits=vsHostnameHits, vsConnCnt=vsConnCnt, ipAddress=ipAddress, ctcpConnLocalAddress=ctcpConnLocalAddress, totalIPPktsOut=totalIPPktsOut, hcStatsTable=hcStatsTable, infNumber=infNumber, clusterVirPriority=clusterVirPriority, rsPort=rsPort, rsIpAddress=rsIpAddress, ctcpEstabResets=ctcpEstabResets, virtualStats=virtualStats, clusterVirAddress=clusterVirAddress, totalIPBytesIn=totalIPBytesIn, vsIpAddressType=vsIpAddressType, vsQosClientPortHits=vsQosClientPortHits, groupIndex=groupIndex, arrayNetworks=arrayNetworks, clusterVirPreempt=clusterVirPreempt, peakSentBytesPerSec=peakSentBytesPerSec, acceptedConns=acceptedConns, serverEstabConn=serverEstabConn, msgText=msgText, logHistTableMaxLength=logHistTableMaxLength, virtualIndex=virtualIndex, cputemp=cputemp, healthCheck=healthCheck, ipStatsTable=ipStatsTable, hcAddr=hcAddr, vsQosNetworkHits=vsQosNetworkHits, reqPeakMin=reqPeakMin, logMaxSeverity=logMaxSeverity, gpIndex=gpIndex, vsBackupHits=vsBackupHits, vipStats=vipStats, ctcpIndex=ctcpIndex, infTable=infTable, vsStatsEntry=vsStatsEntry, vhostName=vhostName, hcStats=hcStats, ctcpConnRemPort=ctcpConnRemPort, openSSLConns=openSSLConns, ctcpConnEntry=ctcpConnEntry, infOutErrors=infOutErrors, groupCount=groupCount, vsPerstntCookieHits=vsPerstntCookieHits, ctcpConnRemAddress=ctcpConnRemAddress, vhostNum=vhostNum, hcAddressType=hcAddressType, totalAcceptedConns=totalAcceptedConns, rsStatsEntry=rsStatsEntry, infInOctets=infInOctets, infIpv6AddressType=infIpv6AddressType, hcConnFail=hcConnFail, reqLastMin=reqLastMin, realServer=realServer, vsHashURLHits=vsHashURLHits, hcIndex=hcIndex, connectionsPerSec=connectionsPerSec, sysMemory=sysMemory, rsStatus=rsStatus, vsICookieHits=vsICookieHits, resumableSess=resumableSess, missSess=missSess, cacheSkip=cacheSkip, logNotificationsEnabled=logNotificationsEnabled, infOutBcastPkts=infOutBcastPkts, realAddress=realAddress, syslogTrap=syslogTrap, rsCntOfReq=rsCntOfReq, hcPort=hcPort, ctcpConnLocalAddr=ctcpConnLocalAddr, requestsOnRegex=requestsOnRegex, clusterNum=clusterNum, infIpv6Address=infIpv6Address, reqLastSec=reqLastSec, totalIPBytesOut=totalIPBytesOut, groupCurCfg=groupCurCfg, clusterVirAuthPasswd=clusterVirAuthPasswd, infAddress=infAddress, infInUnknownProtos=infInUnknownProtos, infOutUcastPkts=infOutUcastPkts, sdns=sdns, severity=severity, requestsToUrl=requestsToUrl, slbGeneral=slbGeneral, succLastMin=succLastMin, rsProtocol=rsProtocol, rsStatsTable=rsStatsTable, reqPeakSec5=reqPeakSec5, vsTable=vsTable, failLastHour=failLastHour, requestedConns=requestedConns, totalRequestedConns=totalRequestedConns, requestsReceived=requestsReceived, ctcpConnRemAddr=ctcpConnRemAddr, infInBcastPkts=infInBcastPkts, sslTable=sslTable, memory=memory, fanspeed=fanspeed, clickTcp=clickTcp, realAddr=realAddr, infOutMcastPkts=infOutMcastPkts, hcNumUps=hcNumUps, caShutdown=caShutdown)
mibBuilder.exportSymbols("CA-SNMP-MIB8", hcName=hcName, currentTime=currentTime, ipAddrType=ipAddrType, realStatus=realStatus, hcConnSuccess=hcConnSuccess, ctcpConnState=ctcpConnState, requestsPerSec=requestsPerSec, peakRcvdBytesPerSec=peakRcvdBytesPerSec, ipBytesIn=ipBytesIn, ipStatsEntry=ipStatsEntry, maxCluster=maxCluster, compression=compression, clusterVirIndex=clusterVirIndex, rsEntry=rsEntry, cpuUtilization=cpuUtilization, clusterVirAuthType=clusterVirAuthType, logHistory=logHistory, failLastDay=failLastDay, vsMaxConn=vsMaxConn, gpMetrics=gpMetrics, logHistoryTable=logHistoryTable, reqPeakDay=reqPeakDay, headRequests=headRequests, getRequests=getRequests, rsTotalHits=rsTotalHits, virtServerID=virtServerID, rsAvgRespTime=rsAvgRespTime, virtualAddressType=virtualAddressType, infInUcastPkts=infInUcastPkts, hitsReply=hitsReply, vsHeaderHits=vsHeaderHits, rsTable=rsTable, clusterVirInterval=clusterVirInterval, vsProtocol=vsProtocol, vsURLHits=vsURLHits, clusterVirState=clusterVirState, realStats=realStats, virtualServer=virtualServer, ctcpConnLocalPort=ctcpConnLocalPort, cacheHitRatio=cacheHitRatio, vsQosCookieHits=vsQosCookieHits, gpID=gpID, hcRSCount=hcRSCount, realPort=realPort, ctcpOutSegs=ctcpOutSegs, ipIndex=ipIndex, vsRegexHits=vsRegexHits, ctcpAttemptFails=ctcpAttemptFails, cacheStatus=cacheStatus, infIpv4AddressType=infIpv4AddressType, responsesOnRegex=responsesOnRegex, vsPerstntURLHits=vsPerstntURLHits, totalBytesSent=totalBytesSent, Float=Float, infIpv4Address=infIpv4Address)
