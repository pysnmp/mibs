...
Counter32, Integer32, TimeTicks, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, enterprises, NotificationType, iso, ObjectIdentity, MibIdentifier, Opaque, IpAddress, Bits, Counter64, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "Integer32", "TimeTicks", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "enterprises", "NotificationType", "iso", "ObjectIdentity", "MibIdentifier", "Opaque", "IpAddress", "Bits", "Counter64", "Gauge32")
...
mibBuilder.exportSymbols("CA-SNMP-MIB8", realPort=realPort, hcRSCount=hcRSCount, failLastSec5=failLastSec5, slbMIB=slbMIB, succPeakMin=succPeakMin, succPeakDay=succPeakDay, rsCntOfReq=rsCntOfReq, realServer=realServer, ctcpCurrEstab=ctcpCurrEstab, clusterVirAuthType=clusterVirAuthType, caShutdown=caShutdown, ctcpConnRemAddr=ctcpConnRemAddr, succLastSec5=succLastSec5, caTraps=caTraps, sslIndex=sslIndex, infOutOctets=infOutOctets, infIpv6Address=infIpv6Address, logNotificationsEnabled=logNotificationsEnabled, succLastMin=succLastMin, rcvdBytesPerSec=rcvdBytesPerSec, hitRevalidate=hitRevalidate, logHistoryEntry=logHistoryEntry, infOutBandwidth=infOutBandwidth, gpID=gpID, connsPerSec=connsPerSec, clusterVirPriority=clusterVirPriority, vsPerstntURLHits=vsPerstntURLHits, totalBytesSent=totalBytesSent, infIpv4Address=infIpv4Address, vsIpAddress=vsIpAddress, clusterVirPreempt=clusterVirPreempt, infOutMcastPkts=infOutMcastPkts, vrrp=vrrp, infAddress=infAddress, clusterVirAddress=clusterVirAddress, failLastHour=failLastHour, rsEntry=rsEntry, ctcpOutSegs=ctcpOutSegs, syslogTrap=syslogTrap, sslMIB=sslMIB, ipPktsIn=ipPktsIn, vsProtocol=vsProtocol, performance=performance, ctcpConnLocalPort=ctcpConnLocalPort, hitsReply=hitsReply, vsCacheHits=vsCacheHits, clusterVirIfname=clusterVirIfname, compression=compression, reqLastMin=reqLastMin, groupStats=groupStats, hcStatsEntry=hcStatsEntry, totalReq=totalReq, rsStatsEntry=rsStatsEntry, ctcpAttemptFails=ctcpAttemptFails, vsHashURLHits=vsHashURLHits, cacheStatus=cacheStatus, missSess=missSess, logNotificationsSent=logNotificationsSent, infEntry=infEntry, logBasic=logBasic, gpStatsEntry=gpStatsEntry, ctcpIndex=ctcpIndex, ipAddrType=ipAddrType, reqPeakDay=reqPeakDay, ipAddress=ipAddress, sysMemoryUtilization=sysMemoryUtilization, hcConnAttempt=hcConnAttempt, hcName=hcName, rsProtocol=rsProtocol, vsICookieHits=vsICookieHits, responsesOnRegex=responsesOnRegex, hcStatsTable=hcStatsTable, memory=memory, hcCause=hcCause, acceptedConns=acceptedConns, ctcpConnLocalAddrType=ctcpConnLocalAddrType, vsCount=vsCount, ipStatsEntry=ipStatsEntry, gpTotalHits=gpTotalHits, cacheMissWNoncacheReq=cacheMissWNoncacheReq, realAddr=realAddr, succPeakSec=succPeakSec, ipBytesOut=ipBytesOut, sysSwapUsed=sysSwapUsed, caSyslogTrap=caSyslogTrap, clientEstabConn=clientEstabConn, infIpv4AddressType=infIpv4AddressType, infInBandwidth=infInBandwidth, ipBytesIn=ipBytesIn, openSSLConns=openSSLConns, ctcpConnLocalAddress=ctcpConnLocalAddress, headRequests=headRequests, vsIpAddr=vsIpAddr, groupIndex=groupIndex, virtualAddressType=virtualAddressType, infOutBcastPkts=infOutBcastPkts, cacheBasicStats=cacheBasicStats, virtualServer=virtualServer, totalBytesRcvd=totalBytesRcvd, vsRedirectHits=vsRedirectHits, currentTime=currentTime, vsPort=vsPort, virtServerID=virtServerID, index=index, vipStatus=vipStatus, totalIPBytesOut=totalIPBytesOut, totalIPBytesIn=totalIPBytesIn, rsIpAddr=rsIpAddr, infOutNUcastPkts=infOutNUcastPkts, totalAcceptedConns=totalAcceptedConns, gpEntry=gpEntry, hcStats=hcStats, succLastSec=succLastSec, groupID=groupID, infOutUcastPkts=infOutUcastPkts, connectionsPerSec=connectionsPerSec, serialNumber=serialNumber, requestsReceived=requestsReceived, rsStatus=rsStatus, ipIndex=ipIndex, licenseRemainingDays=licenseRemainingDays, vsQosNetworkHits=vsQosNetworkHits, vrrpEntry=vrrpEntry, resumableSess=resumableSess, sslStats=sslStats, virtualPort=virtualPort, hostName=hostName, reqLastHour=reqLastHour, vsRegexHits=vsRegexHits, ipAddr=ipAddr, cacheMissWRespNo=cacheMissWRespNo, ipPktsOut=ipPktsOut, gpMetrics=gpMetrics, vsHeaderHits=vsHeaderHits, powerstate=powerstate, severity=severity, startTime=startTime, realIndex=realIndex, vsIndex=vsIndex, infTotalOutOctets=infTotalOutOctets, vsRCookieHits=vsRCookieHits, clusterVirIndex=clusterVirIndex, gpTable=gpTable, responsesToHttps=responsesToHttps, reqPeakSec=reqPeakSec, infOutErrors=infOutErrors, serverEstabConn=serverEstabConn, clusterVirAddr=clusterVirAddr, sdns=sdns, succPeakSec5=succPeakSec5, ctcpConnState=ctcpConnState, vhostName=vhostName, logHistTableMaxLength=logHistTableMaxLength, ctcpActiveOpens=ctcpActiveOpens, postRequests=postRequests, vsConnCnt=vsConnCnt, virtualAddress=virtualAddress, caSyslog=caSyslog, cacheMissWNewEntry=cacheMissWNewEntry, sysSwapCapacity=sysSwapCapacity, logHistory=logHistory, hitsReplyWNotModified=hitsReplyWNotModified, totalIPPktsOut=totalIPPktsOut, cpuUtilization=cpuUtilization, activeTransac=activeTransac, rsTotalHits=rsTotalHits, rsTable=rsTable, sentBytesPerSec=sentBytesPerSec, reqPeakHour=reqPeakHour, totalSucc=totalSucc, sslGeneral=sslGeneral, realStats=realStats, vsQosURLHits=vsQosURLHits, hcPort=hcPort, vsTable=vsTable, revProxyCache=revProxyCache, virtualAddr=virtualAddr, vsStaticHits=vsStaticHits, hcConnFail=hcConnFail, rsPort=rsPort, ctcpInErrs=ctcpInErrs, vsID=vsID, ctcpConnTable=ctcpConnTable, clusterNum=clusterNum, vsBackupHits=vsBackupHits, logMaxSeverity=logMaxSeverity, cacheHitRatio=cacheHitRatio, sslEntry=sslEntry, hcAddr=hcAddr, failLastMin=failLastMin, requestsOnRegex=requestsOnRegex, hitsReplyWPreFailed=hitsReplyWPreFailed, requestsPerSec=requestsPerSec, vsStatsEntry=vsStatsEntry, infInBcastPkts=infInBcastPkts, rsIpAddressType=rsIpAddressType, requestsToUrl=requestsToUrl, hcNumUps=hcNumUps, vsHostnameHits=vsHostnameHits, cputemp=cputemp, realID=realID, groupCurCfg=groupCurCfg, msgText=msgText, infTotalInOctets=infTotalInOctets, clusterVirInterval=clusterVirInterval, ctcpConnRemPort=ctcpConnRemPort, succLastHour=succLastHour, maxCluster=maxCluster, caStart=caStart, rsID=rsID, clusterVirAddressType=clusterVirAddressType, peakRcvdBytesPerSec=peakRcvdBytesPerSec, infInUcastPkts=infInUcastPkts, vipStats=vipStats, arrayNetworks=arrayNetworks, infInDiscards=infInDiscards, vsIpAddressType=vsIpAddressType, rsIpAddress=rsIpAddress, vsQosClientPortHits=vsQosClientPortHits, hcStatus=hcStatus, reqLastSec5=reqLastSec5, purgeRequests=purgeRequests, hcConnSuccess=hcConnSuccess, vsURLHits=vsURLHits, rsMaxConn=rsMaxConn, rsStatsTable=rsStatsTable, cacheSkip=cacheSkip, fanspeed=fanspeed, clickTcp=clickTcp, requestedConns=requestedConns, PYSNMP_MODULE_ID=arrayNetworks, realAddress=realAddress, vsPerstntCookieHits=vsPerstntCookieHits, infNumber=infNumber, rsCount=rsCount, virtualIndex=virtualIndex, infIpv6AddressType=infIpv6AddressType, succLastDay=succLastDay, ctcpOutRsts=ctcpOutRsts, succPeakHour=succPeakHour, slbGeneral=slbGeneral, hcAddress=hcAddress, totalIPPktsIn=totalIPPktsIn, infInErrors=infInErrors, rsAvgRespTime=rsAvgRespTime, gpStatsTable=gpStatsTable, vsQosBodyHits=vsQosBodyHits, healthCheck=healthCheck, realAddressType=realAddressType, sysMemory=sysMemory, peakSentBytesPerSec=peakSentBytesPerSec, failLastDay=failLastDay, reqLastDay=reqLastDay, vrrpTable=vrrpTable)
mibBuilder.exportSymbols("CA-SNMP-MIB8", vsEntry=vsEntry, realStatus=realStatus, ifTraffic=ifTraffic, rsIndex=rsIndex, realServerID=realServerID, totalRequestedConns=totalRequestedConns, infInUnknownProtos=infInUnknownProtos, reqLastSec=reqLastSec, clusterVirState=clusterVirState, requestsToHttps=requestsToHttps, monitor=monitor, reqPeakMin=reqPeakMin, infInMcastPkts=infInMcastPkts, gpIndex=gpIndex, infTable=infTable, failLastSec=failLastSec, vhostNum=vhostNum, infIndex=infIndex, logHistoryTable=logHistoryTable, totalOpenSSLConns=totalOpenSSLConns, ipStatsTable=ipStatsTable, vsStatsTable=vsStatsTable, clusterVrrp=clusterVrrp, ctcpRetransSegs=ctcpRetransSegs, hcAddressType=hcAddressType, ctcpConnRemAddress=ctcpConnRemAddress, SyslogSeverity=SyslogSeverity, reqPeakSec5=reqPeakSec5, infDescr=infDescr, hcNumDowns=hcNumDowns, rsConnCnt=rsConnCnt, vsMaxConn=vsMaxConn, virtualStats=virtualStats, sslTable=sslTable, systemInfo=systemInfo, clusterId=clusterId, vsQosCookieHits=vsQosCookieHits, hcIndex=hcIndex, clusterVirAuthPasswd=clusterVirAuthPasswd, ctcpPassiveOpens=ctcpPassiveOpens, vsDefaultHits=vsDefaultHits, ctcpConnLocalAddr=ctcpConnLocalAddr, ctcpConnEntry=ctcpConnEntry, ctcpConnRemAddrType=ctcpConnRemAddrType, infInOctets=infInOctets, getRequests=getRequests, slbStats=slbStats, resumedSess=resumedSess, infOperStatus=infOperStatus, ctcpInSegs=ctcpInSegs, Float=Float, infInNUcastPkts=infInNUcastPkts, ctcpEstabResets=ctcpEstabResets, totalFail=totalFail, groupCount=groupCount)
