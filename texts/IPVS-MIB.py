...
# ASN.1 source IPVS-MIB
# Source digest sha256:9702e303535afef0196039f97d8d4176b3c86d6903d6787813d931b6a0afcd0e
...
if mibBuilder.loadTexts: ipvs.setLastUpdated('2011-12-01 00:00')
...
ipvsVSTable = MibTable((1, 3, 6, 1, 4, 1, 12196, 12, 1), ).setMaxAccess("notaccessible")
...
ipvsRSTable = MibTable((1, 3, 6, 1, 4, 1, 12196, 12, 2), ).setMaxAccess("notaccessible")
...
ipvsRSTotalTable = MibTable((1, 3, 6, 1, 4, 1, 12196, 12, 8), ).setMaxAccess("notaccessible")
...
rsTotalEntry = MibTableRow((1, 3, 6, 1, 4, 1, 12196, 12, 8, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "IPVS-MIB", "totRSidx"))
...
totRSDesc = MibTableColumn((1, 3, 6, 1, 4, 1, 12196, 12, 8, 1, 2), OctetString().subtype(subtypeSpec=ValueSizeConstraint(0, 1024))).setMaxAccess("notaccessible")
...
vsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 12196, 12, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "IPVS-MIB", "vSidx"))
...
rsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 12196, 12, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "IPVS-MIB", "rSidx"))
if mibBuilder.loadTexts: rsEntry.setStatus('current')
if mibBuilder.loadTexts: rsEntry.setDescription('information about a RS')
rSidx = MibTableColumn((1, 3, 6, 1, 4, 1, 12196, 12, 2, 1, 1), Integer32().subtype(subtypeSpec=ValueRangeConstraint(0, 1024))).setMaxAccess("notaccessible")
...
mibBuilder.exportSymbols("IPVS-MIB", PYSNMP_MODULE_ID=ipvs, conns=conns, inBytes=inBytes, inPkts=inPkts, ipvs=ipvs, ipvsRSTable=ipvsRSTable, ipvsRSTotalTable=ipvsRSTotalTable, ipvsVSTable=ipvsVSTable, outBytes=outBytes, outPkts=outPkts, rSActiveConns=rSActiveConns, rSConns=rSConns, rSDesc=rSDesc, rSInBytes=rSInBytes, rSInPkts=rSInPkts, rSInactiveConns=rSInactiveConns, rSOutBytes=rSOutBytes, rSOutPkts=rSOutPkts, rSVSidx=rSVSidx, rSWeight=rSWeight, rSidx=rSidx, rsEntry=rsEntry, rsTotalEntry=rsTotalEntry, totRSActiveConns=totRSActiveConns, totRSConns=totRSConns, totRSDesc=totRSDesc, totRSInBytes=totRSInBytes, totRSInPkts=totRSInPkts, totRSInactiveConns=totRSInactiveConns, totRSOutBytes=totRSOutBytes, totRSOutPkts=totRSOutPkts, totRSidx=totRSidx, vSActivConns=vSActivConns, vSConns=vSConns, vSDesc=vSDesc, vSInBytes=vSInBytes, vSInPkts=vSInPkts, vSOutBytes=vSOutBytes, vSOutPkts=vSOutPkts, vSidx=vSidx, vsEntry=vsEntry)
