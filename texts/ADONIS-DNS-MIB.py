...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemon=dhcpDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceRunning=haServiceRunning, dns=dns, dnsStatsFailure=dnsStatsFailure, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseExpiry=licenseExpiry, trapTFTP=trapTFTP, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolAlert=dhcpPoolAlert, dhcpStats=dhcpStats, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonRunning=dnsDaemonRunning, adonisTraps=adonisTraps, trapDHCPDaemon=trapDHCPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, ha=ha, dnsStats=dnsStats, trapDNS=trapDNS, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, haServiceNodeType=haServiceNodeType, adonis=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetAlert=dhcpSubnetAlert, systemDaemon=systemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, tftp=tftp, trapReplicationFailure=trapReplicationFailure, PYSNMP_MODULE_ID=adonis, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServerDaemon=commandServerDaemon, system=system, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIP=dhcpFixedIP, systemState=systemState, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize, trapReplication=trapReplication, trapHA=trapHA, trapCommandServer=trapCommandServer, dhcpPoolTable=dhcpPoolTable, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEndTime=dhcpLeaseEndTime, lcdDaemon=lcdDaemon, dhcp=dhcp, commandServer=commandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFailOverState=dhcpFailOverState, tftpDaemon=tftpDaemon, dhcpPoolSize=dhcpPoolSize, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonRunning=dhcpDaemonRunning, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpIP=dhcpIP, licenseValid=licenseValid, trapSystemDaemon=trapSystemDaemon, dnsDaemon=dnsDaemon, haService=haService, trapSystem=trapSystem, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetIP=dhcpSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, dhcpLeaseBindState=dhcpLeaseBindState)
