...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dns=dns, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCP=trapDHCP, commandServer=commandServer, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, licenseValid=licenseValid, dhcpFailOverState=dhcpFailOverState, dhcpSubnetSize=dhcpSubnetSize, lcdDaemon=lcdDaemon, dhcpFixedIPTable=dhcpFixedIPTable, PYSNMP_MODULE_ID=adonis, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, trapTFTP=trapTFTP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsSuccess=dnsStatsSuccess, lcd=lcd, trapCommandServerDaemon=trapCommandServerDaemon, licenseExpiry=licenseExpiry, dhcpLeaseEntry=dhcpLeaseEntry, trapDNSDaemon=trapDNSDaemon, systemDaemon=systemDaemon, dnsDaemonRunning=dnsDaemonRunning, haReplicationBinding=haReplicationBinding, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapCommandServer=trapCommandServer, dhcpPoolEntry=dhcpPoolEntry, dhcp=dhcp, dhcpSubnetAlert=dhcpSubnetAlert, tftp=tftp, dhcpPoolSize=dhcpPoolSize, haServiceRunning=haServiceRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolEndIP=dhcpPoolEndIP, adonis=adonis, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIP=dhcpFixedIP, dhcpConfig=dhcpConfig, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsFailure=dnsStatsFailure, dnsStats=dnsStats, system=system, systemState=systemState, trapHA=trapHA, haServiceNodeType=haServiceNodeType, tftpDaemon=tftpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystemDaemon=trapSystemDaemon, dhcpIP=dhcpIP, ha=ha, dnsStatsReferral=dnsStatsReferral, dhcpPoolTable=dhcpPoolTable, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpStats=dhcpStats, trapReplication=trapReplication, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCPDaemon=trapDHCPDaemon, dnsDaemon=dnsDaemon, dhcpLeaseHostname=dhcpLeaseHostname, adonisTraps=adonisTraps, dhcpDaemon=dhcpDaemon, adonisObjects=adonisObjects, trapReplicationFailure=trapReplicationFailure, commandServerDaemon=commandServerDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetMask=dhcpSubnetMask, trapSystem=trapSystem, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones)
