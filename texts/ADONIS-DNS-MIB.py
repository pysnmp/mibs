...
IpAddress, Counter32, Integer32, ObjectIdentity, Bits, Unsigned32, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, Counter64, MibIdentifier, TimeTicks, ModuleIdentity, iso, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "IpAddress", "Counter32", "Integer32", "ObjectIdentity", "Bits", "Unsigned32", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "Counter64", "MibIdentifier", "TimeTicks", "ModuleIdentity", "iso", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcp=dhcp, systemState=systemState, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolEntry=dhcpPoolEntry, lcd=lcd, dnsDaemonDebugLevel=dnsDaemonDebugLevel, licenseValid=licenseValid, systemDaemon=systemDaemon, adonisObjects=adonisObjects, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIP=dhcpFixedIP, trapDNSDaemon=trapDNSDaemon, dhcpSubnetMask=dhcpSubnetMask, dnsStatsSuccess=dnsStatsSuccess, dhcpStats=dhcpStats, trapDHCP=trapDHCP, dhcpPoolSize=dhcpPoolSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpDaemon=dhcpDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFailOverState=dhcpFailOverState, commandServer=commandServer, dhcpPoolAlert=dhcpPoolAlert, PYSNMP_MODULE_ID=adonis, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcdDaemon=lcdDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemon=tftpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetIP=dhcpSubnetIP, dnsStats=dnsStats, haReplicationBinding=haReplicationBinding, dhcpPoolTable=dhcpPoolTable, trapDNS=trapDNS, dhcpSubnetAlert=dhcpSubnetAlert, trapSystemDaemon=trapSystemDaemon, haService=haService, dhcpSubnetTable=dhcpSubnetTable, trapReplicationFailure=trapReplicationFailure, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceRunning=haServiceRunning, trapCommandServerDaemon=trapCommandServerDaemon, trapTFTP=trapTFTP, dnsDaemon=dnsDaemon, dhcpLeaseEntry=dhcpLeaseEntry, tftpDaemonRunning=tftpDaemonRunning, trapCommandServer=trapCommandServer, dns=dns, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpConfig=dhcpConfig, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonis=adonis, trapSystem=trapSystem, dhcpLeaseBindState=dhcpLeaseBindState, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, system=system, haServiceNodeType=haServiceNodeType, licenseExpiry=licenseExpiry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpIP=dhcpIP, commandServerDaemonRunning=commandServerDaemonRunning, trapReplication=trapReplication, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsFailure=dnsStatsFailure, adonisTraps=adonisTraps, ha=ha, tftp=tftp, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemon=commandServerDaemon, dnsStatsRecursion=dnsStatsRecursion)
