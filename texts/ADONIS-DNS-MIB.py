...
ModuleIdentity, MibIdentifier, Counter32, IpAddress, Counter64, Integer32, TimeTicks, iso, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Bits, NotificationType, enterprises, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "MibIdentifier", "Counter32", "IpAddress", "Counter64", "Integer32", "TimeTicks", "iso", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Bits", "NotificationType", "enterprises", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemon=dnsDaemon, dnsStatsFailure=dnsStatsFailure, licenseExpiry=licenseExpiry, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, commandServerDaemon=commandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIP=dhcpFixedIP, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsRecursion=dnsStatsRecursion, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, dns=dns, adonisTraps=adonisTraps, tftpDaemonRunning=tftpDaemonRunning, trapHA=trapHA, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStats=dnsStats, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceRunning=haServiceRunning, tftpDaemon=tftpDaemon, commandServer=commandServer, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseTable=dhcpLeaseTable, PYSNMP_MODULE_ID=adonis, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, systemState=systemState, adonisObjects=adonisObjects, trapTFTP=trapTFTP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonis=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonRunning=dhcpDaemonRunning, haService=haService, trapDHCP=trapDHCP, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEndIP=dhcpPoolEndIP, dhcpConfig=dhcpConfig, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolSize=dhcpPoolSize, haServiceNodeType=haServiceNodeType, dhcpDaemon=dhcpDaemon, dhcpStats=dhcpStats, dhcpLeaseHostname=dhcpLeaseHostname, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolTable=dhcpPoolTable, tftp=tftp, trapDNSDaemon=trapDNSDaemon, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, ha=ha, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, haReplicationBinding=haReplicationBinding, dhcpFailOverState=dhcpFailOverState, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapReplication=trapReplication, dhcpPoolEntry=dhcpPoolEntry, trapCommandServer=trapCommandServer, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemonRunning=commandServerDaemonRunning, dhcp=dhcp, systemDaemon=systemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEndTime=dhcpLeaseEndTime, lcdDaemon=lcdDaemon, trapReplicationFailure=trapReplicationFailure, trapSystem=trapSystem, licenseValid=licenseValid, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystemDaemon=trapSystemDaemon, system=system, lcd=lcd)
