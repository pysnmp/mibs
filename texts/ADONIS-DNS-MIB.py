...
Integer32, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, iso, NotificationType, ModuleIdentity, TimeTicks, MibIdentifier, Unsigned32, IpAddress, Bits, ObjectIdentity, Counter64, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "iso", "NotificationType", "ModuleIdentity", "TimeTicks", "MibIdentifier", "Unsigned32", "IpAddress", "Bits", "ObjectIdentity", "Counter64", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceRunning=haServiceRunning, dhcpFixedIPTable=dhcpFixedIPTable, trapHAServiceFailOver=trapHAServiceFailOver, dns=dns, trapHA=trapHA, haServiceNodeType=haServiceNodeType, haService=haService, systemDaemon=systemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSize=dhcpPoolSize, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseBindState=dhcpLeaseBindState, PYSNMP_MODULE_ID=adonis, dhcpSubnetTable=dhcpSubnetTable, trapTFTPDaemon=trapTFTPDaemon, trapCommandServerDaemon=trapCommandServerDaemon, licenseValid=licenseValid, dhcpSubnetIP=dhcpSubnetIP, trapSystemDaemon=trapSystemDaemon, dnsStatsReferral=dnsStatsReferral, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, lcd=lcd, dhcpFixedIPEntry=dhcpFixedIPEntry, trapReplication=trapReplication, dhcpFailOverState=dhcpFailOverState, adonisTraps=adonisTraps, ha=ha, dhcpLeaseHostname=dhcpLeaseHostname, adonisObjects=adonisObjects, dnsStats=dnsStats, tftpDaemon=tftpDaemon, commandServerDaemon=commandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolAlert=dhcpPoolAlert, dhcp=dhcp, lcdDaemon=lcdDaemon, systemState=systemState, dhcpFixedIP=dhcpFixedIP, dhcpStats=dhcpStats, system=system, dnsStatsNXDomain=dnsStatsNXDomain, trapDNSDaemon=trapDNSDaemon, trapSystem=trapSystem, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCP=trapDHCP, dnsDaemon=dnsDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsSuccess=dnsStatsSuccess, commandServer=commandServer, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolUsed=dhcpPoolUsed, tftp=tftp, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpIP=dhcpIP, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTable=dhcpLeaseTable, tftpDaemonRunning=tftpDaemonRunning, trapDNS=trapDNS, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServer=trapCommandServer, trapTFTP=trapTFTP, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsFailure=dnsStatsFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haReplicationBinding=haReplicationBinding, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServerDaemonRunning=commandServerDaemonRunning, licenseExpiry=licenseExpiry, trapReplicationFailure=trapReplicationFailure, adonis=adonis, dhcpDaemon=dhcpDaemon, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress)
