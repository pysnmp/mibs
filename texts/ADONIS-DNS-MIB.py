...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemon=tftpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, systemDaemon=systemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDHCP=trapDHCP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServer=trapCommandServer, dhcpPoolStartIP=dhcpPoolStartIP, dhcp=dhcp, dhcpPoolSize=dhcpPoolSize, haServiceNodeType=haServiceNodeType, haService=haService, dhcpDaemon=dhcpDaemon, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHA=trapHA, ha=ha, adonis=adonis, dhcpConfig=dhcpConfig, haServiceRunning=haServiceRunning, haReplicationBinding=haReplicationBinding, tftpDaemonRunning=tftpDaemonRunning, dhcpStats=dhcpStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsReferral=dnsStatsReferral, licenseExpiry=licenseExpiry, system=system, lcdDaemon=lcdDaemon, systemState=systemState, PYSNMP_MODULE_ID=adonis, dns=dns, trapReplicationFailure=trapReplicationFailure, trapReplication=trapReplication, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetMask=dhcpSubnetMask, licenseValid=licenseValid, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTP=trapTFTP, dhcpFixedIPTable=dhcpFixedIPTable, adonisTraps=adonisTraps, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServer=commandServer, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStats=dnsStats, lcd=lcd, dhcpSubnetTable=dhcpSubnetTable, trapSystem=trapSystem, dnsDaemon=dnsDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIP=dhcpFixedIP, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetEntry=dhcpSubnetEntry, trapSystemDaemon=trapSystemDaemon, tftp=tftp, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNS=trapDNS, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEntry=dhcpPoolEntry, trapDNSDaemon=trapDNSDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsStatsFailure=dnsStatsFailure, dhcpPoolTable=dhcpPoolTable, commandServerDaemon=commandServerDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects)
