...
MibIdentifier, NotificationType, Integer32, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, TimeTicks, Gauge32, enterprises, Counter64, Unsigned32, ObjectIdentity, Bits, ModuleIdentity, iso = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "NotificationType", "Integer32", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "TimeTicks", "Gauge32", "enterprises", "Counter64", "Unsigned32", "ObjectIdentity", "Bits", "ModuleIdentity", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemon=tftpDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseHostname=dhcpLeaseHostname, haServiceRunning=haServiceRunning, licenseExpiry=licenseExpiry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, PYSNMP_MODULE_ID=adonis, tftp=tftp, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetUsed=dhcpSubnetUsed, trapDNSDaemon=trapDNSDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolAlert=dhcpPoolAlert, dns=dns, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsReferral=dnsStatsReferral, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapSystemDaemon=trapSystemDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCPDaemon=trapDHCPDaemon, adonisTraps=adonisTraps, systemDaemon=systemDaemon, dhcpFixedIP=dhcpFixedIP, lcdDaemon=lcdDaemon, dhcpPoolTable=dhcpPoolTable, tftpDaemonRunning=tftpDaemonRunning, trapCommandServer=trapCommandServer, dhcpDaemon=dhcpDaemon, trapReplicationFailure=trapReplicationFailure, dhcpPoolSize=dhcpPoolSize, trapTFTP=trapTFTP, dnsStatsNXRRSet=dnsStatsNXRRSet, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapHA=trapHA, commandServer=commandServer, ha=ha, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetIP=dhcpSubnetIP, dhcpFixedIPEntry=dhcpFixedIPEntry, haReplicationBinding=haReplicationBinding, dhcpPoolEntry=dhcpPoolEntry, systemState=systemState, dhcp=dhcp, commandServerDaemon=commandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemon=dnsDaemon, haServiceNodeType=haServiceNodeType, trapReplication=trapReplication, dhcpPoolUsed=dhcpPoolUsed, dnsStats=dnsStats, dnsDaemonRunning=dnsDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFailOverState=dhcpFailOverState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haService=haService, adonis=adonis, trapDHCP=trapDHCP, dhcpPoolStartIP=dhcpPoolStartIP, dhcpIP=dhcpIP, system=system, trapSystem=trapSystem, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonisObjects=adonisObjects, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetEntry=dhcpSubnetEntry, dhcpStats=dhcpStats, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEndIP=dhcpPoolEndIP, trapDNS=trapDNS, licenseValid=licenseValid, trapTFTPDaemon=trapTFTPDaemon, dnsStatsFailure=dnsStatsFailure, lcd=lcd, dhcpConfig=dhcpConfig)
