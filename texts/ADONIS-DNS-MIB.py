...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolTable=dhcpPoolTable, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemon=commandServerDaemon, dhcpLeaseEntry=dhcpLeaseEntry, trapCommandServer=trapCommandServer, dnsDaemonRunning=dnsDaemonRunning, licenseExpiry=licenseExpiry, dhcpPoolEntry=dhcpPoolEntry, dnsStatsFailure=dnsStatsFailure, system=system, haServiceNodeType=haServiceNodeType, dhcpSubnetSize=dhcpSubnetSize, tftpDaemon=tftpDaemon, tftp=tftp, trapTFTP=trapTFTP, lcdDaemon=lcdDaemon, haReplicationBinding=haReplicationBinding, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStats=dnsStats, dns=dns, dhcpLeaseHostname=dhcpLeaseHostname, trapReplication=trapReplication, trapReplicationFailure=trapReplicationFailure, dnsDaemon=dnsDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseBindState=dhcpLeaseBindState, systemDaemon=systemDaemon, commandServerDaemonRunning=commandServerDaemonRunning, licenseValid=licenseValid, dhcpDaemon=dhcpDaemon, dnsStatsReferral=dnsStatsReferral, adonisObjects=adonisObjects, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, ha=ha, dhcpSubnetMask=dhcpSubnetMask, trapDNS=trapDNS, dhcpPoolSize=dhcpPoolSize, dhcpFixedIPEntry=dhcpFixedIPEntry, systemState=systemState, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTable=dhcpLeaseTable, trapSystemDaemon=trapSystemDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetIP=dhcpSubnetIP, trapSystem=trapSystem, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapHAServiceFailOver=trapHAServiceFailOver, adonisTraps=adonisTraps, trapHA=trapHA, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceRunning=haServiceRunning, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFailOverState=dhcpFailOverState, trapTFTPDaemon=trapTFTPDaemon, dhcpFixedIP=dhcpFixedIP, lcd=lcd, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, PYSNMP_MODULE_ID=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonis=adonis, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNSDaemon=trapDNSDaemon, dhcpSubnetTable=dhcpSubnetTable, haService=haService, dhcp=dhcp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServer=commandServer, dhcpStats=dhcpStats, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, trapDHCP=trapDHCP, dhcpIP=dhcpIP)
