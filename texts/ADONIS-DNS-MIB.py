...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dns=dns, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolStartIP=dhcpPoolStartIP, trapHA=trapHA, dhcpIP=dhcpIP, haService=haService, dhcpPoolUsed=dhcpPoolUsed, haServiceRunning=haServiceRunning, commandServerDaemonRunning=commandServerDaemonRunning, systemState=systemState, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemon=dhcpDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemon=dnsDaemon, dhcpLeaseHostname=dhcpLeaseHostname, systemDaemon=systemDaemon, trapSystemDaemon=trapSystemDaemon, dhcpFailOverState=dhcpFailOverState, tftp=tftp, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, system=system, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEntry=dhcpPoolEntry, adonis=adonis, dhcpLeaseTable=dhcpLeaseTable, dhcp=dhcp, dhcpPoolEndIP=dhcpPoolEndIP, adonisObjects=adonisObjects, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServer=commandServer, trapReplicationFailure=trapReplicationFailure, dhcpSubnetAlert=dhcpSubnetAlert, lcd=lcd, PYSNMP_MODULE_ID=adonis, dnsStats=dnsStats, dnsStatsReferral=dnsStatsReferral, dhcpLeaseStartTime=dhcpLeaseStartTime, haServiceNodeType=haServiceNodeType, trapCommandServer=trapCommandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDHCP=trapDHCP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpStats=dhcpStats, dhcpPoolSize=dhcpPoolSize, licenseValid=licenseValid, dnsStatsSuccess=dnsStatsSuccess, lcdDaemon=lcdDaemon, dhcpConfig=dhcpConfig, dhcpFixedIP=dhcpFixedIP, trapTFTP=trapTFTP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServerDaemon=commandServerDaemon, haReplicationBinding=haReplicationBinding, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetIP=dhcpSubnetIP, adonisTraps=adonisTraps, dnsStatsFailure=dnsStatsFailure, trapDNSDaemon=trapDNSDaemon, trapReplication=trapReplication, trapSystem=trapSystem, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, trapDNS=trapDNS, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetTable=dhcpSubnetTable, trapDHCPDaemon=trapDHCPDaemon, tftpDaemon=tftpDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseExpiry=licenseExpiry, ha=ha, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure)
