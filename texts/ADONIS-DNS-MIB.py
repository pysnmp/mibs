...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServer=commandServer, trapReplicationFailure=trapReplicationFailure, licenseValid=licenseValid, dhcpSubnetTable=dhcpSubnetTable, tftp=tftp, haServiceNodeType=haServiceNodeType, trapCommandServer=trapCommandServer, dnsStatsReferral=dnsStatsReferral, dhcpLeaseTable=dhcpLeaseTable, trapDHCP=trapDHCP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemDaemon=systemDaemon, dnsStats=dnsStats, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, commandServerDaemon=commandServerDaemon, lcd=lcd, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolSize=dhcpPoolSize, dhcpSubnetSize=dhcpSubnetSize, dhcpStats=dhcpStats, trapDNS=trapDNS, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsFailure=dnsStatsFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, PYSNMP_MODULE_ID=adonis, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetIP=dhcpSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpConfig=dhcpConfig, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEntry=dhcpPoolEntry, dns=dns, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpIP=dhcpIP, lcdDaemon=lcdDaemon, trapSystem=trapSystem, dnsStatsSuccess=dnsStatsSuccess, trapTFTP=trapTFTP, dhcpSubnetMask=dhcpSubnetMask, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDNSDaemon=trapDNSDaemon, trapDHCPDaemon=trapDHCPDaemon, adonisTraps=adonisTraps, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolTable=dhcpPoolTable, trapHA=trapHA, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetAlert=dhcpSubnetAlert, ha=ha, adonisObjects=adonisObjects, haService=haService, tftpDaemonRunning=tftpDaemonRunning, trapReplication=trapReplication, dnsStatsNXDomain=dnsStatsNXDomain, trapSystemDaemon=trapSystemDaemon, system=system, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemon=dnsDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemon=dhcpDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, systemState=systemState, dhcp=dhcp, tftpDaemon=tftpDaemon, dhcpFailOverState=dhcpFailOverState, adonis=adonis, haServiceRunning=haServiceRunning, dhcpFixedIP=dhcpFixedIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, licenseExpiry=licenseExpiry, dnsStatsRecursion=dnsStatsRecursion)
