...
MibIdentifier, enterprises, Counter64, IpAddress, ModuleIdentity, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, Counter32, Unsigned32, Gauge32, TimeTicks, ObjectIdentity, iso, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "enterprises", "Counter64", "IpAddress", "ModuleIdentity", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "Counter32", "Unsigned32", "Gauge32", "TimeTicks", "ObjectIdentity", "iso", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcd=lcd, trapHAServiceFailOver=trapHAServiceFailOver, PYSNMP_MODULE_ID=adonis, trapSystemDaemon=trapSystemDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapTFTPDaemon=trapTFTPDaemon, trapDNSDaemon=trapDNSDaemon, adonisTraps=adonisTraps, dnsStatsRecursion=dnsStatsRecursion, systemState=systemState, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, adonis=adonis, ha=ha, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsReferral=dnsStatsReferral, tftpDaemon=tftpDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEndTime=dhcpLeaseEndTime, haServiceNodeType=haServiceNodeType, dhcpFailOverState=dhcpFailOverState, adonisObjects=adonisObjects, haServiceRunning=haServiceRunning, system=system, dhcpPoolAlert=dhcpPoolAlert, haReplicationBinding=haReplicationBinding, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, licenseValid=licenseValid, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServerDaemon=trapCommandServerDaemon, tftp=tftp, trapHA=trapHA, trapReplication=trapReplication, dhcpStats=dhcpStats, dnsStats=dnsStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemon=dhcpDaemon, trapCommandServer=trapCommandServer, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseTable=dhcpLeaseTable, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, systemDaemon=systemDaemon, dnsStatsFailure=dnsStatsFailure, commandServerDaemon=commandServerDaemon, dhcpIP=dhcpIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolSize=dhcpPoolSize, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServer=commandServer, trapDHCP=trapDHCP, haService=haService, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystem=trapSystem, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIP=dhcpFixedIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dns=dns, dhcpPoolTable=dhcpPoolTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpConfig=dhcpConfig, dnsStatsSuccess=dnsStatsSuccess, licenseExpiry=licenseExpiry, lcdDaemon=lcdDaemon, dhcp=dhcp, dnsDaemon=dnsDaemon, dhcpSubnetSize=dhcpSubnetSize, trapTFTP=trapTFTP, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHostname=dhcpLeaseHostname, trapDNS=trapDNS, dhcpLeaseEntry=dhcpLeaseEntry)
