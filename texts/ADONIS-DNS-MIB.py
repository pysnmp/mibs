...
enterprises, Counter32, Gauge32, IpAddress, iso, Unsigned32, NotificationType, MibIdentifier, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Bits, ObjectIdentity, ModuleIdentity, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Counter32", "Gauge32", "IpAddress", "iso", "Unsigned32", "NotificationType", "MibIdentifier", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Bits", "ObjectIdentity", "ModuleIdentity", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsReferral=dnsStatsReferral, trapHA=trapHA, dhcpDaemon=dhcpDaemon, dnsDaemon=dnsDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolEntry=dhcpPoolEntry, trapReplication=trapReplication, licenseExpiry=licenseExpiry, trapDNSDaemon=trapDNSDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFailOverState=dhcpFailOverState, haReplicationBinding=haReplicationBinding, commandServer=commandServer, dnsStatsFailure=dnsStatsFailure, systemDaemon=systemDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsNXDomain=dnsStatsNXDomain, tftpDaemon=tftpDaemon, ha=ha, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, trapHAServiceFailOver=trapHAServiceFailOver, tftpDaemonRunning=tftpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, licenseValid=licenseValid, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, system=system, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolTable=dhcpPoolTable, adonisObjects=adonisObjects, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, commandServerDaemon=commandServerDaemon, haServiceRunning=haServiceRunning, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapReplicationFailure=trapReplicationFailure, lcd=lcd, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haService=haService, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIP=dhcpFixedIP, systemState=systemState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpStats=dhcpStats, trapSystemDaemon=trapSystemDaemon, adonisTraps=adonisTraps, trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, dhcpIP=dhcpIP, trapCommandServerDaemon=trapCommandServerDaemon, lcdDaemon=lcdDaemon, dhcpPoolSize=dhcpPoolSize, dns=dns, PYSNMP_MODULE_ID=adonis, dhcpSubnetIP=dhcpSubnetIP, tftp=tftp, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolAlert=dhcpPoolAlert, trapDHCP=trapDHCP, trapDNS=trapDNS, dhcp=dhcp, dhcpConfig=dhcpConfig, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolStartIP=dhcpPoolStartIP, dnsStats=dnsStats, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetMask=dhcpSubnetMask, adonis=adonis, trapSystem=trapSystem, dnsDaemonRunning=dnsDaemonRunning, haServiceNodeType=haServiceNodeType, dhcpFixedIPEntry=dhcpFixedIPEntry, trapTFTPDaemon=trapTFTPDaemon)
