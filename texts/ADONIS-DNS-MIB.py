...
TimeTicks, Integer32, Bits, ModuleIdentity, MibIdentifier, IpAddress, Gauge32, Unsigned32, ObjectIdentity, Counter32, Counter64, enterprises, NotificationType, iso, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Integer32", "Bits", "ModuleIdentity", "MibIdentifier", "IpAddress", "Gauge32", "Unsigned32", "ObjectIdentity", "Counter32", "Counter64", "enterprises", "NotificationType", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseTable=dhcpLeaseTable, lcd=lcd, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonRunning=dnsDaemonRunning, system=system, dnsStats=dnsStats, dhcpSubnetUsed=dhcpSubnetUsed, haServiceRunning=haServiceRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcp=dhcp, dhcpDaemon=dhcpDaemon, dhcpStats=dhcpStats, dnsDaemon=dnsDaemon, trapCommandServer=trapCommandServer, trapCommandServerDaemon=trapCommandServerDaemon, systemDaemon=systemDaemon, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolAlert=dhcpPoolAlert, adonisObjects=adonisObjects, dhcpPoolUsed=dhcpPoolUsed, trapSystemDaemon=trapSystemDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpConfig=dhcpConfig, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetAlert=dhcpSubnetAlert, trapDNS=trapDNS, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTPDaemon=trapTFTPDaemon, tftpDaemon=tftpDaemon, dnsStatsReferral=dnsStatsReferral, PYSNMP_MODULE_ID=adonis, trapReplicationFailure=trapReplicationFailure, dns=dns, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolStartIP=dhcpPoolStartIP, licenseValid=licenseValid, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolEndIP=dhcpPoolEndIP, systemState=systemState, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemonRunning=tftpDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, trapSystem=trapSystem, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonRunning=dhcpDaemonRunning, trapTFTP=trapTFTP, adonisTraps=adonisTraps, dhcpPoolSize=dhcpPoolSize, trapReplication=trapReplication, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFixedIP=dhcpFixedIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServerDaemon=commandServerDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcdDaemon=lcdDaemon, haReplicationBinding=haReplicationBinding, dnsStatsSuccess=dnsStatsSuccess, ha=ha, licenseExpiry=licenseExpiry, commandServer=commandServer, trapDHCP=trapDHCP, haService=haService, tftp=tftp, dhcpSubnetIP=dhcpSubnetIP, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHostname=dhcpLeaseHostname, haServiceNodeType=haServiceNodeType, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, adonis=adonis, dhcpLeaseEntry=dhcpLeaseEntry)
