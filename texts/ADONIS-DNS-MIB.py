...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", PYSNMP_MODULE_ID=adonis, systemDaemon=systemDaemon, dhcpPoolAlert=dhcpPoolAlert, haReplicationBinding=haReplicationBinding, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, licenseValid=licenseValid, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapSystem=trapSystem, adonisTraps=adonisTraps, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, trapReplicationFailure=trapReplicationFailure, dhcpConfig=dhcpConfig, system=system, lcd=lcd, ha=ha, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dnsStats=dnsStats, lcdDaemon=lcdDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetIP=dhcpSubnetIP, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpIP=dhcpIP, dnsDaemon=dnsDaemon, trapReplication=trapReplication, trapHAServiceFailOver=trapHAServiceFailOver, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolTable=dhcpPoolTable, trapDNSDaemon=trapDNSDaemon, dhcp=dhcp, trapTFTPDaemon=trapTFTPDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haService=haService, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolEndIP=dhcpPoolEndIP, dns=dns, haServiceNodeType=haServiceNodeType, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, commandServer=commandServer, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIP=dhcpFixedIP, licenseExpiry=licenseExpiry, dhcpSubnetMask=dhcpSubnetMask, tftp=tftp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, adonisObjects=adonisObjects, adonis=adonis, trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, trapHA=trapHA, trapSystemDaemon=trapSystemDaemon, dnsStatsReferral=dnsStatsReferral, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetTable=dhcpSubnetTable, trapDHCP=trapDHCP, tftpDaemon=tftpDaemon, haServiceRunning=haServiceRunning, dhcpStats=dhcpStats, trapDNS=trapDNS, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServerDaemon=commandServerDaemon, dhcpDaemon=dhcpDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpFailOverState=dhcpFailOverState, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetAlert=dhcpSubnetAlert, systemState=systemState)
