...
ModuleIdentity, Integer32, iso, Unsigned32, Bits, NotificationType, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Counter64, enterprises, Counter32, MibIdentifier, ObjectIdentity, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Integer32", "iso", "Unsigned32", "Bits", "NotificationType", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Counter64", "enterprises", "Counter32", "MibIdentifier", "ObjectIdentity", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapHA=trapHA, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, trapDNS=trapDNS, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haServiceRunning=haServiceRunning, trapTFTP=trapTFTP, adonis=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapCommandServerDaemon=trapCommandServerDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsRecursion=dnsStatsRecursion, dnsStatsReferral=dnsStatsReferral, trapReplicationFailure=trapReplicationFailure, lcdDaemon=lcdDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemon=dhcpDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolEndIP=dhcpPoolEndIP, tftpDaemon=tftpDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonRunning=dnsDaemonRunning, PYSNMP_MODULE_ID=adonis, systemDaemon=systemDaemon, dhcp=dhcp, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolUsed=dhcpPoolUsed, ha=ha, haReplicationBinding=haReplicationBinding, trapTFTPDaemon=trapTFTPDaemon, dhcpConfig=dhcpConfig, dnsStats=dnsStats, trapSystem=trapSystem, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetIP=dhcpSubnetIP, haServiceNodeType=haServiceNodeType, dhcpFailOverState=dhcpFailOverState, licenseExpiry=licenseExpiry, commandServer=commandServer, dhcpDaemonRunning=dhcpDaemonRunning, dns=dns, dhcpFixedIP=dhcpFixedIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsFailure=dnsStatsFailure, adonisObjects=adonisObjects, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetMask=dhcpSubnetMask, dnsStatsSuccess=dnsStatsSuccess, trapDHCP=trapDHCP, dnsDaemon=dnsDaemon, systemState=systemState, trapDNSDaemon=trapDNSDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisTraps=adonisTraps, dhcpPoolTable=dhcpPoolTable, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpStats=dhcpStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolSize=dhcpPoolSize, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, trapSystemDaemon=trapSystemDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseHostname=dhcpLeaseHostname, tftp=tftp, dhcpIP=dhcpIP, commandServerDaemon=commandServerDaemon, haService=haService, system=system, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapReplication=trapReplication, licenseValid=licenseValid, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServer=trapCommandServer, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred)
