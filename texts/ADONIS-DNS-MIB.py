...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haReplicationBinding=haReplicationBinding, dhcpSubnetEntry=dhcpSubnetEntry, PYSNMP_MODULE_ID=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, dnsStatsFailure=dnsStatsFailure, trapReplicationFailure=trapReplicationFailure, dhcpLeaseBindState=dhcpLeaseBindState, trapHAServiceFailOver=trapHAServiceFailOver, adonisTraps=adonisTraps, trapCommandServer=trapCommandServer, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolTable=dhcpPoolTable, dhcpFixedIP=dhcpFixedIP, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolSize=dhcpPoolSize, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolEntry=dhcpPoolEntry, lcd=lcd, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, ha=ha, trapHA=trapHA, tftp=tftp, licenseValid=licenseValid, dns=dns, trapTFTPDaemon=trapTFTPDaemon, dnsStatsRecursion=dnsStatsRecursion, trapReplication=trapReplication, licenseExpiry=licenseExpiry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcp=dhcp, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetMask=dhcpSubnetMask, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemon=dhcpDaemon, commandServerDaemon=commandServerDaemon, dnsStatsReferral=dnsStatsReferral, trapDHCPDaemon=trapDHCPDaemon, trapSystemDaemon=trapSystemDaemon, systemState=systemState, haServiceRunning=haServiceRunning, dhcpStats=dhcpStats, lcdDaemon=lcdDaemon, trapSystem=trapSystem, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseEntry=dhcpLeaseEntry, adonis=adonis, trapDHCP=trapDHCP, system=system, dhcpIP=dhcpIP, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolStartIP=dhcpPoolStartIP, haServiceNodeType=haServiceNodeType, trapDNS=trapDNS, commandServer=commandServer, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dhcpConfig=dhcpConfig, haService=haService, adonisObjects=adonisObjects, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemon=dnsDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonRunning=dnsDaemonRunning, tftpDaemon=tftpDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFailOverState=dhcpFailOverState, dnsStats=dnsStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, systemDaemon=systemDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize)
