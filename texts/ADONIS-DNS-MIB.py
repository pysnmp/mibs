...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNSDaemon=trapDNSDaemon, trapSystem=trapSystem, dhcpPoolEntry=dhcpPoolEntry, tftpDaemon=tftpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, systemDaemon=systemDaemon, licenseValid=licenseValid, dhcpSubnetIP=dhcpSubnetIP, dnsDaemon=dnsDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemon=dhcpDaemon, dhcpIP=dhcpIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetTable=dhcpSubnetTable, tftpDaemonRunning=tftpDaemonRunning, adonisObjects=adonisObjects, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsSuccess=dnsStatsSuccess, commandServer=commandServer, trapTFTP=trapTFTP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIP=dhcpFixedIP, dhcpLeaseStartTime=dhcpLeaseStartTime, systemState=systemState, dhcpPoolTable=dhcpPoolTable, dnsStatsNXDomain=dnsStatsNXDomain, trapHAServiceFailOver=trapHAServiceFailOver, adonisTraps=adonisTraps, trapDHCP=trapDHCP, dns=dns, dhcpSubnetUsed=dhcpSubnetUsed, ha=ha, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, dhcpFailOverState=dhcpFailOverState, dhcpLeaseTable=dhcpLeaseTable, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDHCPDaemon=trapDHCPDaemon, haReplicationBinding=haReplicationBinding, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haServiceRunning=haServiceRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapSystemDaemon=trapSystemDaemon, dnsStats=dnsStats, dnsStatsFailure=dnsStatsFailure, trapCommandServer=trapCommandServer, dnsDaemonRunning=dnsDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, lcdDaemon=lcdDaemon, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplication=trapReplication, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolUsed=dhcpPoolUsed, trapDNS=trapDNS, trapReplicationFailure=trapReplicationFailure, commandServerDaemonRunning=commandServerDaemonRunning, adonis=adonis, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, tftp=tftp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, lcd=lcd, dhcpConfig=dhcpConfig, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolSize=dhcpPoolSize, system=system, dnsStatsRecursion=dnsStatsRecursion, dhcp=dhcp, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, haService=haService, commandServerDaemon=commandServerDaemon, licenseExpiry=licenseExpiry)
