...
iso, MibIdentifier, Counter64, Unsigned32, ModuleIdentity, ObjectIdentity, Gauge32, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, Integer32, NotificationType, Counter32, Bits, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "MibIdentifier", "Counter64", "Unsigned32", "ModuleIdentity", "ObjectIdentity", "Gauge32", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "Integer32", "NotificationType", "Counter32", "Bits", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNS=trapDNS, commandServer=commandServer, system=system, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolTable=dhcpPoolTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcp=dhcp, adonisObjects=adonisObjects, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpDaemon=dhcpDaemon, dns=dns, dnsStatsFailure=dnsStatsFailure, trapDHCPDaemon=trapDHCPDaemon, trapCommandServer=trapCommandServer, dhcpFixedIP=dhcpFixedIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsSuccess=dnsStatsSuccess, trapDHCP=trapDHCP, systemState=systemState, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, trapSystemDaemon=trapSystemDaemon, dhcpSubnetMask=dhcpSubnetMask, adonisTraps=adonisTraps, dnsStats=dnsStats, commandServerDaemon=commandServerDaemon, haReplicationBinding=haReplicationBinding, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemDaemon=systemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolEndIP=dhcpPoolEndIP, tftp=tftp, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, dhcpFailOverState=dhcpFailOverState, licenseValid=licenseValid, tftpDaemonRunning=tftpDaemonRunning, dhcpStats=dhcpStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonRunning=dnsDaemonRunning, trapSystem=trapSystem, trapDNSDaemon=trapDNSDaemon, haServiceRunning=haServiceRunning, dhcpPoolSize=dhcpPoolSize, haService=haService, PYSNMP_MODULE_ID=adonis, dhcpDaemonRunning=dhcpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, ha=ha, dhcpFixedIPTable=dhcpFixedIPTable, lcdDaemon=lcdDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, dnsStatsReferral=dnsStatsReferral, dnsDaemon=dnsDaemon, trapHA=trapHA, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpConfig=dhcpConfig, trapReplication=trapReplication, dhcpPoolSubnetIP=dhcpPoolSubnetIP, licenseExpiry=licenseExpiry, haServiceNodeType=haServiceNodeType, dhcpLeaseTable=dhcpLeaseTable, lcd=lcd, tftpDaemon=tftpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonis=adonis, dhcpPoolUsed=dhcpPoolUsed, trapReplicationFailure=trapReplicationFailure, dhcpIP=dhcpIP, dnsStatsRecursion=dnsStatsRecursion)
