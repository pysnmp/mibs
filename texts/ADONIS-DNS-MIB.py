...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemonRunning=tftpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemon=dhcpDaemon, system=system, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServer=commandServer, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, PYSNMP_MODULE_ID=adonis, adonisTraps=adonisTraps, adonisObjects=adonisObjects, dnsStats=dnsStats, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haReplicationBinding=haReplicationBinding, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, lcdDaemon=lcdDaemon, systemDaemon=systemDaemon, dhcpSubnetSize=dhcpSubnetSize, haServiceRunning=haServiceRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplicationFailure=trapReplicationFailure, dhcpFixedIPTable=dhcpFixedIPTable, dhcpIP=dhcpIP, dnsStatsReferral=dnsStatsReferral, dhcpPoolSize=dhcpPoolSize, trapDNSDaemon=trapDNSDaemon, tftpDaemon=tftpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, dhcpStats=dhcpStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsRecursion=dnsStatsRecursion, trapTFTPDaemon=trapTFTPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcp=dhcp, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetAlert=dhcpSubnetAlert, licenseExpiry=licenseExpiry, trapSystem=trapSystem, trapReplication=trapReplication, systemState=systemState, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, tftp=tftp, dhcpConfig=dhcpConfig, trapHA=trapHA, trapDHCP=trapDHCP, trapCommandServer=trapCommandServer, haService=haService, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseEntry=dhcpLeaseEntry, haServiceNodeType=haServiceNodeType, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystemDaemon=trapSystemDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTable=dhcpLeaseTable, dnsStatsFailure=dnsStatsFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, adonis=adonis, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolEntry=dhcpPoolEntry, ha=ha, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetMask=dhcpSubnetMask, dnsStatsSuccess=dnsStatsSuccess, trapTFTP=trapTFTP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemonRunning=commandServerDaemonRunning, trapDNS=trapDNS, dhcpFixedIP=dhcpFixedIP, lcd=lcd, dns=dns, commandServerDaemon=commandServerDaemon, licenseValid=licenseValid, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemon=dnsDaemon)
