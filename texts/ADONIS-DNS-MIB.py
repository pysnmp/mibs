...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haReplicationBinding=haReplicationBinding, dhcpFixedIP=dhcpFixedIP, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpStats=dhcpStats, dnsStatsFailure=dnsStatsFailure, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonisObjects=adonisObjects, dhcp=dhcp, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, commandServer=commandServer, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolUsed=dhcpPoolUsed, trapCommandServer=trapCommandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServerDaemon=trapCommandServerDaemon, systemDaemon=systemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseBindState=dhcpLeaseBindState, dnsStats=dnsStats, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, system=system, dhcpConfig=dhcpConfig, dnsDaemon=dnsDaemon, dhcpSubnetSize=dhcpSubnetSize, trapReplication=trapReplication, dhcpIP=dhcpIP, lcd=lcd, haServiceRunning=haServiceRunning, haService=haService, dns=dns, trapSystemDaemon=trapSystemDaemon, dhcpPoolTable=dhcpPoolTable, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTP=trapTFTP, dhcpDaemon=dhcpDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, tftpDaemon=tftpDaemon, dnsStatsRecursion=dnsStatsRecursion, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetUsed=dhcpSubnetUsed, licenseValid=licenseValid, trapDHCP=trapDHCP, trapSystem=trapSystem, commandServerDaemon=commandServerDaemon, trapHA=trapHA, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, trapReplicationFailure=trapReplicationFailure, dhcpFailOverState=dhcpFailOverState, ha=ha, lcdDaemon=lcdDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolSize=dhcpPoolSize, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDNS=trapDNS, dhcpSubnetTable=dhcpSubnetTable, haServiceNodeType=haServiceNodeType, systemState=systemState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, trapDNSDaemon=trapDNSDaemon, tftp=tftp, adonisTraps=adonisTraps, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXDomain=dnsStatsNXDomain, adonis=adonis, licenseExpiry=licenseExpiry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseTable=dhcpLeaseTable)
