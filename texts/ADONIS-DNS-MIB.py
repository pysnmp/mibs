...
Gauge32, ModuleIdentity, MibIdentifier, TimeTicks, enterprises, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Unsigned32, IpAddress, Counter32, iso, Bits, Counter64, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "ModuleIdentity", "MibIdentifier", "TimeTicks", "enterprises", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Unsigned32", "IpAddress", "Counter32", "iso", "Bits", "Counter64", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTP=trapTFTP, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsRecursion=dnsStatsRecursion, dnsStatsNXDomain=dnsStatsNXDomain, dhcpIP=dhcpIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpStats=dhcpStats, trapDNS=trapDNS, dhcpLeaseHostname=dhcpLeaseHostname, haServiceRunning=haServiceRunning, haServiceNodeType=haServiceNodeType, systemState=systemState, dnsDaemonRunning=dnsDaemonRunning, tftpDaemonRunning=tftpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseEndTime=dhcpLeaseEndTime, adonisTraps=adonisTraps, trapHA=trapHA, tftp=tftp, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetSize=dhcpSubnetSize, trapDNSDaemon=trapDNSDaemon, dhcpFailOverState=dhcpFailOverState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, lcd=lcd, licenseExpiry=licenseExpiry, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTable=dhcpLeaseTable, haReplicationBinding=haReplicationBinding, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIPTable=dhcpFixedIPTable, haService=haService, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolAlert=dhcpPoolAlert, commandServer=commandServer, dhcpSubnetAlert=dhcpSubnetAlert, trapSystem=trapSystem, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, commandServerDaemon=commandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, tftpDaemon=tftpDaemon, dhcpPoolTable=dhcpPoolTable, dnsDaemon=dnsDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseStartTime=dhcpLeaseStartTime, lcdDaemon=lcdDaemon, ha=ha, trapCommandServer=trapCommandServer, dhcp=dhcp, dhcpFixedIP=dhcpFixedIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, system=system, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplicationFailure=trapReplicationFailure, dhcpPoolSize=dhcpPoolSize, dnsStats=dnsStats, dnsStatsFailure=dnsStatsFailure, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsReferral=dnsStatsReferral, dhcpDaemon=dhcpDaemon, PYSNMP_MODULE_ID=adonis, licenseValid=licenseValid, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonis=adonis, trapDHCP=trapDHCP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpConfig=dhcpConfig, trapReplication=trapReplication, adonisObjects=adonisObjects, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTPDaemon=trapTFTPDaemon, dns=dns, trapSystemDaemon=trapSystemDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, systemDaemon=systemDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel)
