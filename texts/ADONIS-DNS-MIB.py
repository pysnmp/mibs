...
iso, Gauge32, enterprises, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, ModuleIdentity, Counter64, Bits, Counter32, Integer32, NotificationType, Unsigned32, IpAddress, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Gauge32", "enterprises", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "ModuleIdentity", "Counter64", "Bits", "Counter32", "Integer32", "NotificationType", "Unsigned32", "IpAddress", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpStats=dhcpStats, dnsStatsRecursion=dnsStatsRecursion, trapHAServiceFailOver=trapHAServiceFailOver, trapReplicationFailure=trapReplicationFailure, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, lcd=lcd, dhcpConfig=dhcpConfig, PYSNMP_MODULE_ID=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemon=dhcpDaemon, systemDaemon=systemDaemon, dhcpLeaseTable=dhcpLeaseTable, licenseExpiry=licenseExpiry, dhcpPoolTable=dhcpPoolTable, trapTFTPDaemon=trapTFTPDaemon, trapSystemDaemon=trapSystemDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIP=dhcpFixedIP, licenseValid=licenseValid, system=system, dhcpSubnetMask=dhcpSubnetMask, adonis=adonis, adonisTraps=adonisTraps, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, lcdDaemon=lcdDaemon, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCP=trapDHCP, dhcpPoolEndIP=dhcpPoolEndIP, dns=dns, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNSDaemon=trapDNSDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSize=dhcpPoolSize, dhcpSubnetEntry=dhcpSubnetEntry, trapHA=trapHA, systemState=systemState, dnsStatsNXRRSet=dnsStatsNXRRSet, ha=ha, commandServerDaemon=commandServerDaemon, dnsStatsReferral=dnsStatsReferral, tftpDaemon=tftpDaemon, trapSystem=trapSystem, tftp=tftp, trapReplication=trapReplication, trapDNS=trapDNS, dhcpPoolUsed=dhcpPoolUsed, dhcp=dhcp, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetIP=dhcpSubnetIP, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServerDaemon=trapCommandServerDaemon, commandServer=commandServer, haServiceNodeType=haServiceNodeType, commandServerDaemonRunning=commandServerDaemonRunning, haService=haService, dnsDaemonRunning=dnsDaemonRunning, trapCommandServer=trapCommandServer, dnsStatsFailure=dnsStatsFailure, dnsStats=dnsStats, dhcpIP=dhcpIP, dnsDaemon=dnsDaemon, haReplicationBinding=haReplicationBinding, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapTFTP=trapTFTP, haServiceRunning=haServiceRunning, dhcpPoolStartIP=dhcpPoolStartIP, adonisObjects=adonisObjects, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFailOverState=dhcpFailOverState, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure)
