...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpConfig=dhcpConfig, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolTable=dhcpPoolTable, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisObjects=adonisObjects, system=system, dhcpStats=dhcpStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCP=trapDHCP, dhcpLeaseTable=dhcpLeaseTable, trapTFTP=trapTFTP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTPDaemon=trapTFTPDaemon, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemon=dhcpDaemon, trapDNSDaemon=trapDNSDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseBindState=dhcpLeaseBindState, systemDaemon=systemDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dns=dns, lcdDaemon=lcdDaemon, trapCommandServer=trapCommandServer, dnsStats=dnsStats, dnsStatsReferral=dnsStatsReferral, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftp=tftp, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceRunning=haServiceRunning, trapDHCPDaemon=trapDHCPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDNS=trapDNS, ha=ha, systemState=systemState, adonisTraps=adonisTraps, dhcpFixedIPTable=dhcpFixedIPTable, trapSystemDaemon=trapSystemDaemon, commandServer=commandServer, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haService=haService, commandServerDaemon=commandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, lcd=lcd, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetTable=dhcpSubnetTable, trapSystem=trapSystem, tftpDaemon=tftpDaemon, dhcpIP=dhcpIP, haServiceNodeType=haServiceNodeType, licenseValid=licenseValid, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcp=dhcp, adonis=adonis, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonRunning=dnsDaemonRunning, dnsDaemon=dnsDaemon, licenseExpiry=licenseExpiry, trapReplicationFailure=trapReplicationFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSize=dhcpPoolSize, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolEntry=dhcpPoolEntry, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetIP=dhcpSubnetIP, trapHA=trapHA, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, trapReplication=trapReplication, dhcpPoolAlert=dhcpPoolAlert, PYSNMP_MODULE_ID=adonis)
