...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, system=system, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonisTraps=adonisTraps, dhcpConfig=dhcpConfig, ha=ha, dhcpFailOverState=dhcpFailOverState, dnsDaemon=dnsDaemon, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemon=tftpDaemon, licenseExpiry=licenseExpiry, dhcpPoolTable=dhcpPoolTable, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsFailure=dnsStatsFailure, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseStartTime=dhcpLeaseStartTime, licenseValid=licenseValid, haService=haService, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplicationFailure=trapReplicationFailure, tftp=tftp, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemon=dhcpDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNSDaemon=trapDNSDaemon, dhcpSubnetMask=dhcpSubnetMask, trapCommandServer=trapCommandServer, haReplicationBinding=haReplicationBinding, dhcpLeaseBindState=dhcpLeaseBindState, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolEndIP=dhcpPoolEndIP, haServiceRunning=haServiceRunning, dns=dns, dhcpPoolSize=dhcpPoolSize, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCP=trapDHCP, lcdDaemon=lcdDaemon, trapReplication=trapReplication, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, trapSystemDaemon=trapSystemDaemon, commandServerDaemon=commandServerDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapTFTP=trapTFTP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapHA=trapHA, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStats=dnsStats, dhcpStats=dhcpStats, systemDaemon=systemDaemon, adonisObjects=adonisObjects, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIP=dhcpFixedIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolAlert=dhcpPoolAlert, systemState=systemState, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpIP=dhcpIP, trapDHCPDaemon=trapDHCPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, commandServer=commandServer, dhcpSubnetUsed=dhcpSubnetUsed, haServiceNodeType=haServiceNodeType, adonis=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, lcd=lcd, dhcpPoolStartIP=dhcpPoolStartIP, commandServerDaemonRunning=commandServerDaemonRunning, trapSystem=trapSystem, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsReferral=dnsStatsReferral, dhcp=dhcp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTable=dhcpLeaseTable)
