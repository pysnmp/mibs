...
MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, ModuleIdentity, Bits, IpAddress, Unsigned32, Integer32, ObjectIdentity, NotificationType, Counter64, enterprises, iso, Counter32, MibIdentifier, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "ModuleIdentity", "Bits", "IpAddress", "Unsigned32", "Integer32", "ObjectIdentity", "NotificationType", "Counter64", "enterprises", "iso", "Counter32", "MibIdentifier", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNSDaemon=trapDNSDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemon=dhcpDaemon, haServiceNodeType=haServiceNodeType, lcd=lcd, dhcpPoolSize=dhcpPoolSize, systemDaemon=systemDaemon, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsFailure=dnsStatsFailure, licenseExpiry=licenseExpiry, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEndIP=dhcpPoolEndIP, trapReplication=trapReplication, trapDNS=trapDNS, adonis=adonis, dhcpFailOverState=dhcpFailOverState, adonisObjects=adonisObjects, haService=haService, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, haServiceRunning=haServiceRunning, systemState=systemState, trapReplicationFailure=trapReplicationFailure, trapSystem=trapSystem, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolStartIP=dhcpPoolStartIP, licenseValid=licenseValid, trapDHCP=trapDHCP, trapCommandServer=trapCommandServer, trapSystemDaemon=trapSystemDaemon, haReplicationBinding=haReplicationBinding, trapDHCPDaemon=trapDHCPDaemon, ha=ha, trapHA=trapHA, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcp=dhcp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, adonisTraps=adonisTraps, dhcpSubnetMask=dhcpSubnetMask, dhcpIP=dhcpIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsSuccess=dnsStatsSuccess, dnsStats=dnsStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, system=system, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, PYSNMP_MODULE_ID=adonis, tftp=tftp, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTPDaemon=trapTFTPDaemon, dhcpConfig=dhcpConfig, dhcpFixedIP=dhcpFixedIP, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTP=trapTFTP, commandServer=commandServer, dhcpPoolTable=dhcpPoolTable, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseTable=dhcpLeaseTable, dns=dns, tftpDaemon=tftpDaemon, lcdDaemon=lcdDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, dnsDaemon=dnsDaemon, dhcpLeaseEntry=dhcpLeaseEntry)
