...
ObjectIdentity, Counter64, ModuleIdentity, NotificationType, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, IpAddress, TimeTicks, Unsigned32, Integer32, enterprises, Gauge32, MibIdentifier, iso = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Counter64", "ModuleIdentity", "NotificationType", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "IpAddress", "TimeTicks", "Unsigned32", "Integer32", "enterprises", "Gauge32", "MibIdentifier", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolEntry=dhcpPoolEntry, system=system, dnsStatsRecursion=dnsStatsRecursion, trapSystem=trapSystem, dnsDaemonRunning=dnsDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, trapDNSDaemon=trapDNSDaemon, dhcpFixedIP=dhcpFixedIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemon=dhcpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpConfig=dhcpConfig, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonRunning=dhcpDaemonRunning, trapDNS=trapDNS, dhcpSubnetSize=dhcpSubnetSize, trapHAServiceFailOver=trapHAServiceFailOver, haServiceNodeType=haServiceNodeType, adonisTraps=adonisTraps, dhcpSubnetMask=dhcpSubnetMask, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemon=tftpDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsReferral=dnsStatsReferral, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, ha=ha, dhcpFixedIPEntry=dhcpFixedIPEntry, haReplicationBinding=haReplicationBinding, lcdDaemon=lcdDaemon, dhcpSubnetAlert=dhcpSubnetAlert, trapSystemDaemon=trapSystemDaemon, commandServerDaemon=commandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, lcd=lcd, trapTFTP=trapTFTP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDHCPDaemon=trapDHCPDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetUsed=dhcpSubnetUsed, PYSNMP_MODULE_ID=adonis, licenseExpiry=licenseExpiry, trapHA=trapHA, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcp=dhcp, dnsStats=dnsStats, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolUsed=dhcpPoolUsed, trapCommandServer=trapCommandServer, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseValid=licenseValid, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetIP=dhcpSubnetIP, dnsStatsFailure=dnsStatsFailure, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolAlert=dhcpPoolAlert, tftp=tftp, trapDHCP=trapDHCP, systemState=systemState, adonis=adonis, dhcpIP=dhcpIP, dns=dns, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisObjects=adonisObjects, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, trapReplication=trapReplication, dnsDaemon=dnsDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, commandServer=commandServer, dhcpFailOverState=dhcpFailOverState, dhcpStats=dhcpStats, dhcpPoolSize=dhcpPoolSize, haServiceRunning=haServiceRunning, trapReplicationFailure=trapReplicationFailure)
