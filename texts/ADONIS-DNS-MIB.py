...
Gauge32, Counter64, ModuleIdentity, Counter32, MibIdentifier, TimeTicks, Bits, NotificationType, ObjectIdentity, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, Integer32, enterprises, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Counter64", "ModuleIdentity", "Counter32", "MibIdentifier", "TimeTicks", "Bits", "NotificationType", "ObjectIdentity", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "Integer32", "enterprises", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNSDaemon=trapDNSDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapReplication=trapReplication, haService=haService, dhcpPoolEntry=dhcpPoolEntry, systemState=systemState, trapSystemDaemon=trapSystemDaemon, dhcpConfig=dhcpConfig, dhcpPoolStartIP=dhcpPoolStartIP, commandServerDaemon=commandServerDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisTraps=adonisTraps, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, lcd=lcd, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEndIP=dhcpPoolEndIP, dnsStats=dnsStats, dhcp=dhcp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTable=dhcpLeaseTable, haReplicationBinding=haReplicationBinding, lcdDaemon=lcdDaemon, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceNodeType=haServiceNodeType, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEntry=dhcpLeaseEntry, dhcpIP=dhcpIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisObjects=adonisObjects, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTP=trapTFTP, dns=dns, dnsStatsRecursion=dnsStatsRecursion, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonRunning=dhcpDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetEntry=dhcpSubnetEntry, PYSNMP_MODULE_ID=adonis, dhcpStats=dhcpStats, dhcpFixedIPEntry=dhcpFixedIPEntry, tftpDaemon=tftpDaemon, dhcpSubnetIP=dhcpSubnetIP, dnsStatsFailure=dnsStatsFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapCommandServer=trapCommandServer, commandServer=commandServer, trapReplicationFailure=trapReplicationFailure, haServiceRunning=haServiceRunning, trapHA=trapHA, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetMask=dhcpSubnetMask, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolSize=dhcpPoolSize, dhcpFailOverState=dhcpFailOverState, trapDHCP=trapDHCP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, trapSystem=trapSystem, dhcpLeaseEndTime=dhcpLeaseEndTime, adonis=adonis, tftp=tftp, systemDaemon=systemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, system=system, dnsDaemon=dnsDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemon=dhcpDaemon, dhcpFixedIP=dhcpFixedIP, licenseExpiry=licenseExpiry, ha=ha, licenseValid=licenseValid, trapCommandServerDaemon=trapCommandServerDaemon, trapDNS=trapDNS)
