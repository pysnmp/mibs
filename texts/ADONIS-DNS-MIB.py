...
ObjectIdentity, TimeTicks, MibIdentifier, Gauge32, IpAddress, enterprises, Bits, Counter32, Counter64, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, NotificationType, iso, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "TimeTicks", "MibIdentifier", "Gauge32", "IpAddress", "enterprises", "Bits", "Counter32", "Counter64", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "NotificationType", "iso", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapSystemDaemon=trapSystemDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTP=trapTFTP, dhcp=dhcp, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNSDaemon=trapDNSDaemon, dhcpConfig=dhcpConfig, dhcpFixedIP=dhcpFixedIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpStats=dhcpStats, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftpDaemonRunning=tftpDaemonRunning, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapSystem=trapSystem, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolUsed=dhcpPoolUsed, trapReplicationFailure=trapReplicationFailure, dhcpSubnetMask=dhcpSubnetMask, PYSNMP_MODULE_ID=adonis, commandServerDaemon=commandServerDaemon, trapHA=trapHA, adonisTraps=adonisTraps, tftpDaemon=tftpDaemon, haServiceNodeType=haServiceNodeType, dns=dns, dhcpPoolTable=dhcpPoolTable, trapDHCP=trapDHCP, dnsStatsReferral=dnsStatsReferral, system=system, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNS=trapDNS, trapCommandServer=trapCommandServer, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsSuccess=dnsStatsSuccess, adonisObjects=adonisObjects, trapReplication=trapReplication, adonis=adonis, dhcpPoolSize=dhcpPoolSize, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDHCPDaemon=trapDHCPDaemon, haService=haService, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, dnsStats=dnsStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemDaemon=systemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, haServiceRunning=haServiceRunning, dnsStatsFailure=dnsStatsFailure, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonRunning=dhcpDaemonRunning, lcdDaemon=lcdDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemon=dnsDaemon, dhcpIP=dhcpIP, dnsStatsNXRRSet=dnsStatsNXRRSet, lcd=lcd, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, licenseValid=licenseValid, commandServer=commandServer, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftp=tftp, dhcpFailOverState=dhcpFailOverState, dnsStatsNXDomain=dnsStatsNXDomain, ha=ha, dhcpDaemon=dhcpDaemon, dhcpSubnetTable=dhcpSubnetTable, systemState=systemState)
