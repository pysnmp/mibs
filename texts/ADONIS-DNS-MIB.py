...
enterprises, TimeTicks, Bits, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Integer32, ObjectIdentity, ModuleIdentity, MibIdentifier, Gauge32, NotificationType, Counter64, Counter32, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "TimeTicks", "Bits", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Integer32", "ObjectIdentity", "ModuleIdentity", "MibIdentifier", "Gauge32", "NotificationType", "Counter64", "Counter32", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsFailure=dnsStatsFailure, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsReferral=dnsStatsReferral, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServer=commandServer, adonisTraps=adonisTraps, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHostname=dhcpLeaseHostname, licenseExpiry=licenseExpiry, licenseValid=licenseValid, trapCommandServerDaemon=trapCommandServerDaemon, trapReplicationFailure=trapReplicationFailure, dnsDaemon=dnsDaemon, dns=dns, dhcpFailOverState=dhcpFailOverState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonis=adonis, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, systemDaemon=systemDaemon, trapTFTP=trapTFTP, dnsStatsSuccess=dnsStatsSuccess, ha=ha, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetEntry=dhcpSubnetEntry, system=system, tftpDaemon=tftpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystem=trapSystem, dhcpLeaseStartTime=dhcpLeaseStartTime, lcdDaemon=lcdDaemon, haReplicationBinding=haReplicationBinding, dhcpPoolAlert=dhcpPoolAlert, trapTFTPDaemon=trapTFTPDaemon, adonisObjects=adonisObjects, systemState=systemState, PYSNMP_MODULE_ID=adonis, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNS=trapDNS, trapDNSDaemon=trapDNSDaemon, trapCommandServer=trapCommandServer, dnsStats=dnsStats, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemon=commandServerDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapReplication=trapReplication, dhcpPoolSize=dhcpPoolSize, haServiceRunning=haServiceRunning, dhcpIP=dhcpIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapSystemDaemon=trapSystemDaemon, tftpDaemonRunning=tftpDaemonRunning, trapHA=trapHA, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, tftp=tftp, dhcpConfig=dhcpConfig, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolTable=dhcpPoolTable, dnsStatsNXRRSet=dnsStatsNXRRSet, haService=haService, trapDHCP=trapDHCP, dhcpDaemon=dhcpDaemon, dhcp=dhcp, haServiceNodeType=haServiceNodeType, dhcpFixedIP=dhcpFixedIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, lcd=lcd, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetIP=dhcpSubnetIP, dhcpFixedIPEntry=dhcpFixedIPEntry)
