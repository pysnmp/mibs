...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsNXDomain=dnsStatsNXDomain, licenseExpiry=licenseExpiry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSize=dhcpPoolSize, dhcpFixedIP=dhcpFixedIP, dhcpStats=dhcpStats, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIPEntry=dhcpFixedIPEntry, systemState=systemState, dhcpPoolEntry=dhcpPoolEntry, dhcp=dhcp, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapSystem=trapSystem, dhcpSubnetMask=dhcpSubnetMask, lcd=lcd, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServerDaemon=commandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsFailure=dnsStatsFailure, dhcpLeaseEndTime=dhcpLeaseEndTime, trapTFTP=trapTFTP, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTPDaemon=trapTFTPDaemon, dnsStats=dnsStats, haReplicationBinding=haReplicationBinding, adonisObjects=adonisObjects, dhcpLeaseHostname=dhcpLeaseHostname, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, adonis=adonis, system=system, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemDaemon=systemDaemon, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplication=trapReplication, ha=ha, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpConfig=dhcpConfig, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCP=trapDHCP, dhcpDaemon=dhcpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServer=trapCommandServer, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDNSDaemon=trapDNSDaemon, dhcpLeaseBindState=dhcpLeaseBindState, haService=haService, commandServerDaemonRunning=commandServerDaemonRunning, haServiceNodeType=haServiceNodeType, haServiceRunning=haServiceRunning, dhcpFailOverState=dhcpFailOverState, dnsDaemon=dnsDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolTable=dhcpPoolTable, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetUsed=dhcpSubnetUsed, commandServer=commandServer, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapReplicationFailure=trapReplicationFailure, dns=dns, dnsDaemonRunning=dnsDaemonRunning, trapSystemDaemon=trapSystemDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpIP=dhcpIP, tftp=tftp, licenseValid=licenseValid, dnsStatsReferral=dnsStatsReferral, lcdDaemon=lcdDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisTraps=adonisTraps, tftpDaemon=tftpDaemon)
