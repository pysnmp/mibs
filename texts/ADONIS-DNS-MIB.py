...
Counter32, Bits, enterprises, ModuleIdentity, Unsigned32, ObjectIdentity, IpAddress, MibIdentifier, NotificationType, TimeTicks, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, iso, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "Bits", "enterprises", "ModuleIdentity", "Unsigned32", "ObjectIdentity", "IpAddress", "MibIdentifier", "NotificationType", "TimeTicks", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "iso", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIP=dhcpFixedIP, dns=dns, adonisObjects=adonisObjects, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonRunning=dnsDaemonRunning, trapHA=trapHA, dhcpLeaseHostname=dhcpLeaseHostname, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemon=dnsDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, systemState=systemState, dhcpSubnetIP=dhcpSubnetIP, dnsStats=dnsStats, dhcpSubnetTable=dhcpSubnetTable, ha=ha, licenseExpiry=licenseExpiry, dnsStatsFailure=dnsStatsFailure, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, licenseValid=licenseValid, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolUsed=dhcpPoolUsed, commandServer=commandServer, dhcpPoolTable=dhcpPoolTable, commandServerDaemon=commandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetAlert=dhcpSubnetAlert, tftp=tftp, dhcpFailOverState=dhcpFailOverState, trapCommandServerDaemon=trapCommandServerDaemon, haReplicationBinding=haReplicationBinding, dnsStatsRecursion=dnsStatsRecursion, trapReplication=trapReplication, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonisTraps=adonisTraps, trapReplicationFailure=trapReplicationFailure, dhcpPoolEndIP=dhcpPoolEndIP, trapSystem=trapSystem, trapDNS=trapDNS, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcp=dhcp, system=system, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, tftpDaemonRunning=tftpDaemonRunning, trapCommandServer=trapCommandServer, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTP=trapTFTP, dhcpDaemon=dhcpDaemon, haService=haService, lcd=lcd, dhcpLeaseBindState=dhcpLeaseBindState, adonis=adonis, dhcpLeaseTable=dhcpLeaseTable, tftpDaemon=tftpDaemon, dhcpSubnetSize=dhcpSubnetSize, trapSystemDaemon=trapSystemDaemon, dnsStatsReferral=dnsStatsReferral, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolSize=dhcpPoolSize, haServiceRunning=haServiceRunning, dhcpPoolAlert=dhcpPoolAlert, lcdDaemon=lcdDaemon, systemDaemon=systemDaemon, dhcpStats=dhcpStats, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpConfig=dhcpConfig, dhcpDaemonRunning=dhcpDaemonRunning, haServiceNodeType=haServiceNodeType, trapDNSDaemon=trapDNSDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpIP=dhcpIP, trapDHCP=trapDHCP, dhcpPoolEntry=dhcpPoolEntry)
