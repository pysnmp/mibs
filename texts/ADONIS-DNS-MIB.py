...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServer=commandServer, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseHostname=dhcpLeaseHostname, systemDaemon=systemDaemon, trapReplicationFailure=trapReplicationFailure, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcp=dhcp, trapDHCP=trapDHCP, dhcpPoolTable=dhcpPoolTable, dnsStatsFailure=dnsStatsFailure, trapReplication=trapReplication, dhcpPoolEntry=dhcpPoolEntry, system=system, dhcpFixedIP=dhcpFixedIP, dhcpSubnetTable=dhcpSubnetTable, dnsStatsSuccess=dnsStatsSuccess, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolAlert=dhcpPoolAlert, adonis=adonis, dhcpPoolSize=dhcpPoolSize, dnsDaemon=dnsDaemon, dnsStatsNXDomain=dnsStatsNXDomain, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpStats=dhcpStats, haReplicationBinding=haReplicationBinding, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemon=dhcpDaemon, commandServerDaemon=commandServerDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEndIP=dhcpPoolEndIP, licenseValid=licenseValid, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStats=dnsStats, dhcpDaemonRunning=dhcpDaemonRunning, haServiceNodeType=haServiceNodeType, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, trapDNSDaemon=trapDNSDaemon, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpConfig=dhcpConfig, lcdDaemon=lcdDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServerDaemonRunning=commandServerDaemonRunning, haService=haService, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapHA=trapHA, dhcpSubnetIP=dhcpSubnetIP, PYSNMP_MODULE_ID=adonis, licenseExpiry=licenseExpiry, trapSystem=trapSystem, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolUsed=dhcpPoolUsed, tftpDaemon=tftpDaemon, trapTFTP=trapTFTP, haServiceRunning=haServiceRunning, dhcpFixedIPTable=dhcpFixedIPTable, lcd=lcd, adonisTraps=adonisTraps, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetSize=dhcpSubnetSize, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsReferral=dnsStatsReferral, dhcpSubnetMask=dhcpSubnetMask, trapCommandServer=trapCommandServer, trapCommandServerDaemon=trapCommandServerDaemon, tftp=tftp, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFailOverState=dhcpFailOverState, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDNS=trapDNS, ha=ha, systemState=systemState, dns=dns, trapTFTPDaemon=trapTFTPDaemon, trapSystemDaemon=trapSystemDaemon, adonisObjects=adonisObjects, dnsDaemonRunning=dnsDaemonRunning)
