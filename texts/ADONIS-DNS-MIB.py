...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCPDaemon=trapDHCPDaemon, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpStats=dhcpStats, adonis=adonis, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, trapHA=trapHA, trapDHCP=trapDHCP, tftpDaemonRunning=tftpDaemonRunning, system=system, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolSize=dhcpPoolSize, trapSystem=trapSystem, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetMask=dhcpSubnetMask, dhcp=dhcp, dnsDaemonRunning=dnsDaemonRunning, tftpDaemon=tftpDaemon, dns=dns, dhcpSubnetUsed=dhcpSubnetUsed, adonisObjects=adonisObjects, dhcpLeaseTable=dhcpLeaseTable, adonisTraps=adonisTraps, dhcpIP=dhcpIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseExpiry=licenseExpiry, dhcpFailOverState=dhcpFailOverState, dhcpPoolEntry=dhcpPoolEntry, trapCommandServer=trapCommandServer, dnsStatsRecursion=dnsStatsRecursion, tftp=tftp, dhcpDaemon=dhcpDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolTable=dhcpPoolTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, commandServer=commandServer, commandServerDaemon=commandServerDaemon, dnsStatsFailure=dnsStatsFailure, haServiceNodeType=haServiceNodeType, dhcpPoolAlert=dhcpPoolAlert, ha=ha, lcd=lcd, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStats=dnsStats, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEntry=dhcpLeaseEntry, trapReplication=trapReplication, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServerDaemon=trapCommandServerDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dhcpConfig=dhcpConfig, systemDaemon=systemDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapSystemDaemon=trapSystemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapReplicationFailure=trapReplicationFailure, haServiceRunning=haServiceRunning, dhcpLeaseHostname=dhcpLeaseHostname, licenseValid=licenseValid, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsSuccess=dnsStatsSuccess, systemState=systemState, dnsStatsNXRRSet=dnsStatsNXRRSet, PYSNMP_MODULE_ID=adonis, trapTFTP=trapTFTP, trapDNS=trapDNS, trapTFTPDaemon=trapTFTPDaemon, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemon=dnsDaemon, dhcpSubnetEntry=dhcpSubnetEntry, lcdDaemon=lcdDaemon)
