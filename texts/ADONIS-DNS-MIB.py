...
Counter32, Bits, Unsigned32, Counter64, IpAddress, NotificationType, TimeTicks, enterprises, ObjectIdentity, MibIdentifier, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, ModuleIdentity, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "Bits", "Unsigned32", "Counter64", "IpAddress", "NotificationType", "TimeTicks", "enterprises", "ObjectIdentity", "MibIdentifier", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "ModuleIdentity", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseStartTime=dhcpLeaseStartTime, trapCommandServerDaemon=trapCommandServerDaemon, trapSystemDaemon=trapSystemDaemon, dhcpIP=dhcpIP, haServiceRunning=haServiceRunning, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolTable=dhcpPoolTable, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolSize=dhcpPoolSize, dhcpSubnetTable=dhcpSubnetTable, trapDHCP=trapDHCP, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHostname=dhcpLeaseHostname, trapDNS=trapDNS, dhcp=dhcp, dhcpPoolEndIP=dhcpPoolEndIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsNXDomain=dnsStatsNXDomain, adonisTraps=adonisTraps, trapReplicationFailure=trapReplicationFailure, dhcpStats=dhcpStats, trapReplication=trapReplication, dhcpLeaseBindState=dhcpLeaseBindState, adonis=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPEntry=dhcpFixedIPEntry, adonisObjects=adonisObjects, PYSNMP_MODULE_ID=adonis, dhcpSubnetSize=dhcpSubnetSize, dnsDaemon=dnsDaemon, dhcpConfig=dhcpConfig, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetUsed=dhcpSubnetUsed, trapDNSDaemon=trapDNSDaemon, commandServer=commandServer, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapTFTP=trapTFTP, dhcpSubnetMask=dhcpSubnetMask, dns=dns, commandServerDaemon=commandServerDaemon, tftpDaemon=tftpDaemon, dnsStatsSuccess=dnsStatsSuccess, trapHAServiceFailOver=trapHAServiceFailOver, systemState=systemState, dhcpLeaseTable=dhcpLeaseTable, ha=ha, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetEntry=dhcpSubnetEntry, licenseExpiry=licenseExpiry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonRunning=dnsDaemonRunning, trapCommandServer=trapCommandServer, dhcpFixedIP=dhcpFixedIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcdDaemon=lcdDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haServiceNodeType=haServiceNodeType, systemDaemon=systemDaemon, system=system, dnsStatsFailure=dnsStatsFailure, dhcpLeaseEntry=dhcpLeaseEntry, trapSystem=trapSystem, tftp=tftp, dnsStats=dnsStats, trapHA=trapHA, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, lcd=lcd, haService=haService, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFailOverState=dhcpFailOverState, licenseValid=licenseValid)
