...
Counter64, enterprises, Integer32, Gauge32, Counter32, Unsigned32, TimeTicks, MibIdentifier, iso, ObjectIdentity, IpAddress, Bits, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Counter64", "enterprises", "Integer32", "Gauge32", "Counter32", "Unsigned32", "TimeTicks", "MibIdentifier", "iso", "ObjectIdentity", "IpAddress", "Bits", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolTable=dhcpPoolTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTable=dhcpLeaseTable, dnsDaemon=dnsDaemon, tftp=tftp, trapDNS=trapDNS, trapDHCP=trapDHCP, system=system, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEndIP=dhcpPoolEndIP, lcdDaemon=lcdDaemon, adonisTraps=adonisTraps, trapTFTP=trapTFTP, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, trapReplicationFailure=trapReplicationFailure, dhcpStats=dhcpStats, dnsDaemonRunning=dnsDaemonRunning, licenseValid=licenseValid, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsReferral=dnsStatsReferral, trapSystemDaemon=trapSystemDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServer=trapCommandServer, trapHAServiceFailOver=trapHAServiceFailOver, dhcp=dhcp, haServiceNodeType=haServiceNodeType, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapCommandServerDaemon=trapCommandServerDaemon, trapReplication=trapReplication, dhcpConfig=dhcpConfig, trapDNSDaemon=trapDNSDaemon, haServiceRunning=haServiceRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseEntry=dhcpLeaseEntry, dnsStats=dnsStats, commandServer=commandServer, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolAlert=dhcpPoolAlert, PYSNMP_MODULE_ID=adonis, dhcpSubnetAlert=dhcpSubnetAlert, lcd=lcd, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemon=dhcpDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFailOverState=dhcpFailOverState, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dns=dns, trapDHCPDaemon=trapDHCPDaemon, dnsStatsFailure=dnsStatsFailure, tftpDaemonRunning=tftpDaemonRunning, haService=haService, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIP=dhcpFixedIP, systemState=systemState, dhcpPoolSize=dhcpPoolSize, dhcpSubnetEntry=dhcpSubnetEntry, haReplicationBinding=haReplicationBinding, dhcpDaemonRunning=dhcpDaemonRunning, tftpDaemon=tftpDaemon, dhcpSubnetTable=dhcpSubnetTable, ha=ha, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisObjects=adonisObjects, dnsStatsRecursion=dnsStatsRecursion, licenseExpiry=licenseExpiry, commandServerDaemon=commandServerDaemon, adonis=adonis, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapSystem=trapSystem, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetIP=dhcpSubnetIP, trapHA=trapHA, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpIP=dhcpIP, dnsStatsSuccess=dnsStatsSuccess, systemDaemon=systemDaemon)
