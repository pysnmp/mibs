...
MibIdentifier, Bits, Unsigned32, Counter32, NotificationType, TimeTicks, Integer32, Counter64, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, enterprises, iso, Gauge32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Bits", "Unsigned32", "Counter32", "NotificationType", "TimeTicks", "Integer32", "Counter64", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "enterprises", "iso", "Gauge32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTP=trapTFTP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapHA=trapHA, PYSNMP_MODULE_ID=adonis, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSize=dhcpPoolSize, dnsStatsNXDomain=dnsStatsNXDomain, tftp=tftp, dhcp=dhcp, ha=ha, dhcpDaemonRunning=dhcpDaemonRunning, licenseExpiry=licenseExpiry, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCP=trapDHCP, dnsStatsSuccess=dnsStatsSuccess, dnsDaemon=dnsDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, haServiceRunning=haServiceRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseBindState=dhcpLeaseBindState, haServiceNodeType=haServiceNodeType, dhcpSubnetSize=dhcpSubnetSize, licenseValid=licenseValid, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpStats=dhcpStats, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetMask=dhcpSubnetMask, haService=haService, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, dns=dns, dnsStats=dnsStats, adonis=adonis, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, commandServer=commandServer, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolTable=dhcpPoolTable, tftpDaemon=tftpDaemon, trapSystem=trapSystem, dhcpPoolUsed=dhcpPoolUsed, trapReplication=trapReplication, dhcpSubnetTable=dhcpSubnetTable, tftpDaemonRunning=tftpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNSDaemon=trapDNSDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpConfig=dhcpConfig, dhcpDaemon=dhcpDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseTable=dhcpLeaseTable, systemState=systemState, system=system, dhcpFailOverState=dhcpFailOverState, adonisTraps=adonisTraps, dnsStatsFailure=dnsStatsFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystemDaemon=trapSystemDaemon, commandServerDaemon=commandServerDaemon, trapCommandServer=trapCommandServer, commandServerDaemonRunning=commandServerDaemonRunning, dhcpIP=dhcpIP, systemDaemon=systemDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIP=dhcpFixedIP, lcd=lcd, trapDNS=trapDNS, adonisObjects=adonisObjects, trapTFTPDaemon=trapTFTPDaemon, trapReplicationFailure=trapReplicationFailure, dnsStatsReferral=dnsStatsReferral, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolEntry=dhcpPoolEntry, lcdDaemon=lcdDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolEndIP=dhcpPoolEndIP, haReplicationBinding=haReplicationBinding, dhcpLeaseHostname=dhcpLeaseHostname)
