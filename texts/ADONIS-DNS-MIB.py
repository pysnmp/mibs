...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonisObjects=adonisObjects, ha=ha, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemon=dhcpDaemon, trapHA=trapHA, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolAlert=dhcpPoolAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsFailure=dnsStatsFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcd=lcd, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetSize=dhcpSubnetSize, dnsDaemon=dnsDaemon, dhcpPoolEndIP=dhcpPoolEndIP, trapSystem=trapSystem, trapDNSDaemon=trapDNSDaemon, systemDaemon=systemDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonRunning=dhcpDaemonRunning, adonisTraps=adonisTraps, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolTable=dhcpPoolTable, dhcpSubnetAlert=dhcpSubnetAlert, commandServerDaemon=commandServerDaemon, haServiceRunning=haServiceRunning, adonis=adonis, trapCommandServer=trapCommandServer, dhcpConfig=dhcpConfig, trapDHCPDaemon=trapDHCPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, haReplicationBinding=haReplicationBinding, trapTFTP=trapTFTP, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServer=commandServer, trapDNS=trapDNS, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapReplicationFailure=trapReplicationFailure, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, system=system, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseEndTime=dhcpLeaseEndTime, tftp=tftp, trapDHCP=trapDHCP, dhcpIP=dhcpIP, dhcpPoolUsed=dhcpPoolUsed, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetEntry=dhcpSubnetEntry, trapHAServiceFailOver=trapHAServiceFailOver, licenseValid=licenseValid, licenseExpiry=licenseExpiry, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTable=dhcpLeaseTable, lcdDaemon=lcdDaemon, dns=dns, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolSize=dhcpPoolSize, systemState=systemState, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFailOverState=dhcpFailOverState, haServiceNodeType=haServiceNodeType, trapCommandServerDaemon=trapCommandServerDaemon, PYSNMP_MODULE_ID=adonis, dnsStatsReferral=dnsStatsReferral, trapReplication=trapReplication, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcp=dhcp, trapSystemDaemon=trapSystemDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsStats=dnsStats, tftpDaemon=tftpDaemon, haService=haService, dhcpLeaseHostname=dhcpLeaseHostname, dhcpStats=dhcpStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones)
