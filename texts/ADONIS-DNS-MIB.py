...
ModuleIdentity, MibIdentifier, iso, ObjectIdentity, IpAddress, Integer32, Gauge32, Counter64, NotificationType, Bits, Counter32, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "MibIdentifier", "iso", "ObjectIdentity", "IpAddress", "Integer32", "Gauge32", "Counter64", "NotificationType", "Bits", "Counter32", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStats=dnsStats, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, systemDaemon=systemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisObjects=adonisObjects, dhcpSubnetIP=dhcpSubnetIP, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemon=dhcpDaemon, trapTFTP=trapTFTP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, ha=ha, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolUsed=dhcpPoolUsed, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolStartIP=dhcpPoolStartIP, dns=dns, dhcpLeaseEntry=dhcpLeaseEntry, lcd=lcd, trapReplication=trapReplication, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolAlert=dhcpPoolAlert, systemState=systemState, trapHA=trapHA, dhcpStats=dhcpStats, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSize=dhcpPoolSize, dhcpIP=dhcpIP, system=system, trapSystem=trapSystem, dhcpSubnetSize=dhcpSubnetSize, dhcpConfig=dhcpConfig, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolTable=dhcpPoolTable, dnsStatsReferral=dnsStatsReferral, tftp=tftp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHAServiceFailOver=trapHAServiceFailOver, haService=haService, dnsDaemon=dnsDaemon, dhcp=dhcp, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemon=commandServerDaemon, dhcpFixedIP=dhcpFixedIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftpDaemon=tftpDaemon, dnsStatsFailure=dnsStatsFailure, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, licenseExpiry=licenseExpiry, trapDNSDaemon=trapDNSDaemon, adonis=adonis, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFailOverState=dhcpFailOverState, haServiceRunning=haServiceRunning, trapReplicationFailure=trapReplicationFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapCommandServer=trapCommandServer, haReplicationBinding=haReplicationBinding, dnsStatsSuccess=dnsStatsSuccess, lcdDaemon=lcdDaemon, adonisTraps=adonisTraps, trapDHCP=trapDHCP, licenseValid=licenseValid, PYSNMP_MODULE_ID=adonis, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemonRunning=commandServerDaemonRunning, haServiceNodeType=haServiceNodeType, trapDHCPDaemon=trapDHCPDaemon, trapDNS=trapDNS, trapSystemDaemon=trapSystemDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServer=commandServer, dhcpSubnetMask=dhcpSubnetMask)
