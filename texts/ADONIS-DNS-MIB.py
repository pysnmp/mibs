...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolEntry=dhcpPoolEntry, dhcpPoolAlert=dhcpPoolAlert, dnsDaemon=dnsDaemon, tftpDaemon=tftpDaemon, dhcpFailOverState=dhcpFailOverState, trapHA=trapHA, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetSize=dhcpSubnetSize, commandServerDaemon=commandServerDaemon, adonis=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapReplicationFailure=trapReplicationFailure, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetMask=dhcpSubnetMask, haServiceNodeType=haServiceNodeType, lcd=lcd, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIP=dhcpFixedIP, licenseValid=licenseValid, ha=ha, haReplicationBinding=haReplicationBinding, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServer=commandServer, dhcpLeaseTable=dhcpLeaseTable, dhcp=dhcp, commandServerDaemonRunning=commandServerDaemonRunning, trapReplication=trapReplication, trapTFTP=trapTFTP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetIP=dhcpSubnetIP, trapCommandServer=trapCommandServer, lcdDaemon=lcdDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetAlert=dhcpSubnetAlert, trapDNSDaemon=trapDNSDaemon, dhcpConfig=dhcpConfig, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemon=dhcpDaemon, tftp=tftp, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetEntry=dhcpSubnetEntry, system=system, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, dhcpStats=dhcpStats, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseExpiry=licenseExpiry, systemState=systemState, dnsStatsNXRRSet=dnsStatsNXRRSet, haServiceRunning=haServiceRunning, dhcpIP=dhcpIP, dhcpPoolUsed=dhcpPoolUsed, PYSNMP_MODULE_ID=adonis, dnsStats=dnsStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDHCP=trapDHCP, trapTFTPDaemon=trapTFTPDaemon, adonisTraps=adonisTraps, dnsStatsReferral=dnsStatsReferral, dns=dns, trapDNS=trapDNS, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haService=haService, dhcpFixedIPTable=dhcpFixedIPTable, systemDaemon=systemDaemon, adonisObjects=adonisObjects, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsFailure=dnsStatsFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapSystemDaemon=trapSystemDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolSize=dhcpPoolSize, tftpDaemonRunning=tftpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, trapSystem=trapSystem)
