...
Counter32, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, ModuleIdentity, Gauge32, enterprises, NotificationType, Counter64, Bits, IpAddress, Integer32, ObjectIdentity, TimeTicks, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "ModuleIdentity", "Gauge32", "enterprises", "NotificationType", "Counter64", "Bits", "IpAddress", "Integer32", "ObjectIdentity", "TimeTicks", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTPDaemon=trapTFTPDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsReferral=dnsStatsReferral, commandServerDaemon=commandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, dhcp=dhcp, dhcpIP=dhcpIP, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplicationFailure=trapReplicationFailure, systemDaemon=systemDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetUsed=dhcpSubnetUsed, system=system, dnsStatsRecursion=dnsStatsRecursion, ha=ha, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemon=dnsDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haService=haService, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, trapCommandServer=trapCommandServer, licenseExpiry=licenseExpiry, commandServer=commandServer, lcdDaemon=lcdDaemon, dhcpSubnetAlert=dhcpSubnetAlert, adonisTraps=adonisTraps, trapHA=trapHA, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceNodeType=haServiceNodeType, dhcpLeaseTable=dhcpLeaseTable, trapSystemDaemon=trapSystemDaemon, trapSystem=trapSystem, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, dnsStats=dnsStats, dhcpSubnetIP=dhcpSubnetIP, dhcpConfig=dhcpConfig, haReplicationBinding=haReplicationBinding, dhcpStats=dhcpStats, PYSNMP_MODULE_ID=adonis, dhcpPoolTable=dhcpPoolTable, trapDHCPDaemon=trapDHCPDaemon, adonis=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEndIP=dhcpPoolEndIP, licenseValid=licenseValid, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftp=tftp, haServiceRunning=haServiceRunning, dhcpFailOverState=dhcpFailOverState, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetEntry=dhcpSubnetEntry, trapDNSDaemon=trapDNSDaemon, dnsStatsSuccess=dnsStatsSuccess, trapDHCP=trapDHCP, trapDNS=trapDNS, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIP=dhcpFixedIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPTable=dhcpFixedIPTable, systemState=systemState, dhcpLeaseBindState=dhcpLeaseBindState, trapReplication=trapReplication, dhcpDaemon=dhcpDaemon, adonisObjects=adonisObjects, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolAlert=dhcpPoolAlert, dns=dns, tftpDaemon=tftpDaemon, trapTFTP=trapTFTP, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress)
