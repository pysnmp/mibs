...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapSystem=trapSystem, dhcpConfig=dhcpConfig, dhcpSubnetTable=dhcpSubnetTable, trapDHCP=trapDHCP, dhcpSubnetMask=dhcpSubnetMask, system=system, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemon=commandServerDaemon, haService=haService, dhcpPoolTable=dhcpPoolTable, trapReplicationFailure=trapReplicationFailure, trapDHCPDaemon=trapDHCPDaemon, lcd=lcd, dhcpLeaseEntry=dhcpLeaseEntry, lcdDaemon=lcdDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapDNSDaemon=trapDNSDaemon, dhcpFailOverState=dhcpFailOverState, tftpDaemon=tftpDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTable=dhcpLeaseTable, trapReplication=trapReplication, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, haReplicationBinding=haReplicationBinding, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemon=dhcpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, systemDaemon=systemDaemon, haServiceRunning=haServiceRunning, trapHA=trapHA, tftp=tftp, dhcpPoolUsed=dhcpPoolUsed, trapCommandServer=trapCommandServer, trapTFTPDaemon=trapTFTPDaemon, ha=ha, dhcpDaemonRunning=dhcpDaemonRunning, adonis=adonis, dhcpPoolEntry=dhcpPoolEntry, adonisObjects=adonisObjects, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolSize=dhcpPoolSize, dhcp=dhcp, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsSuccess=dnsStatsSuccess, haServiceNodeType=haServiceNodeType, dhcpPoolEndIP=dhcpPoolEndIP, dnsStats=dnsStats, dnsDaemonRunning=dnsDaemonRunning, licenseValid=licenseValid, dhcpIP=dhcpIP, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNS=trapDNS, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolAlert=dhcpPoolAlert, adonisTraps=adonisTraps, dhcpStats=dhcpStats, trapCommandServerDaemon=trapCommandServerDaemon, trapHAServiceFailOver=trapHAServiceFailOver, commandServer=commandServer, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, systemState=systemState, dnsStatsReferral=dnsStatsReferral, trapTFTP=trapTFTP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemon=dnsDaemon, dnsStatsFailure=dnsStatsFailure, licenseExpiry=licenseExpiry, PYSNMP_MODULE_ID=adonis, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapSystemDaemon=trapSystemDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetIP=dhcpSubnetIP, dns=dns)
