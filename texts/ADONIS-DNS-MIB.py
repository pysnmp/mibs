...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, systemState=systemState, ha=ha, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemon=dnsDaemon, trapCommandServer=trapCommandServer, licenseValid=licenseValid, trapSystemDaemon=trapSystemDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDNS=trapDNS, dnsStats=dnsStats, trapReplicationFailure=trapReplicationFailure, haReplicationBinding=haReplicationBinding, tftpDaemon=tftpDaemon, dhcpSubnetMask=dhcpSubnetMask, tftp=tftp, systemDaemon=systemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseTable=dhcpLeaseTable, trapDHCPDaemon=trapDHCPDaemon, haServiceNodeType=haServiceNodeType, dnsStatsNXDomain=dnsStatsNXDomain, trapHAServiceFailOver=trapHAServiceFailOver, haService=haService, dhcpPoolTable=dhcpPoolTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCP=trapDHCP, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolEndIP=dhcpPoolEndIP, lcd=lcd, dnsStatsRecursion=dnsStatsRecursion, PYSNMP_MODULE_ID=adonis, dhcpLeaseHostname=dhcpLeaseHostname, trapReplication=trapReplication, trapSystem=trapSystem, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetAlert=dhcpSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIP=dhcpFixedIP, trapTFTPDaemon=trapTFTPDaemon, trapDNSDaemon=trapDNSDaemon, dhcpDaemon=dhcpDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapHA=trapHA, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, adonisTraps=adonisTraps, haServiceRunning=haServiceRunning, lcdDaemon=lcdDaemon, system=system, dhcpConfig=dhcpConfig, dhcpIP=dhcpIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolUsed=dhcpPoolUsed, commandServer=commandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServerDaemon=commandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, trapTFTP=trapTFTP, dhcpFailOverState=dhcpFailOverState, dhcpLeaseEntry=dhcpLeaseEntry, dns=dns, dnsStatsSuccess=dnsStatsSuccess, adonisObjects=adonisObjects, dhcpDaemonRunning=dhcpDaemonRunning, dhcp=dhcp, licenseExpiry=licenseExpiry, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpStats=dhcpStats, dnsStatsReferral=dnsStatsReferral, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSize=dhcpPoolSize, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsFailure=dnsStatsFailure, adonis=adonis)
