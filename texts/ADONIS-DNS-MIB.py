...
ModuleIdentity, Integer32, Bits, ObjectIdentity, IpAddress, TimeTicks, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Gauge32, enterprises, NotificationType, MibIdentifier, iso, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Integer32", "Bits", "ObjectIdentity", "IpAddress", "TimeTicks", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Gauge32", "enterprises", "NotificationType", "MibIdentifier", "iso", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNSDaemon=trapDNSDaemon, dhcpLeaseEntry=dhcpLeaseEntry, adonisTraps=adonisTraps, dhcpSubnetMask=dhcpSubnetMask, system=system, dhcpDaemon=dhcpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIP=dhcpFixedIP, systemState=systemState, trapSystem=trapSystem, tftp=tftp, PYSNMP_MODULE_ID=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, tftpDaemonRunning=tftpDaemonRunning, trapSystemDaemon=trapSystemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetIP=dhcpSubnetIP, dhcpFailOverState=dhcpFailOverState, dhcpConfig=dhcpConfig, dhcpDaemonRunning=dhcpDaemonRunning, haServiceNodeType=haServiceNodeType, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapReplicationFailure=trapReplicationFailure, dhcpPoolSize=dhcpPoolSize, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolTable=dhcpPoolTable, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStats=dnsStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetEntry=dhcpSubnetEntry, adonis=adonis, licenseExpiry=licenseExpiry, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsReferral=dnsStatsReferral, dns=dns, dhcpSubnetTable=dhcpSubnetTable, licenseValid=licenseValid, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsSuccess=dnsStatsSuccess, tftpDaemon=tftpDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpStats=dhcpStats, dhcpLeaseStartTime=dhcpLeaseStartTime, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, ha=ha, commandServerDaemon=commandServerDaemon, dnsDaemon=dnsDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, adonisObjects=adonisObjects, trapCommandServer=trapCommandServer, dhcpLeaseTable=dhcpLeaseTable, dnsStatsFailure=dnsStatsFailure, trapDNS=trapDNS, dhcpIP=dhcpIP, dhcp=dhcp, dhcpFixedIPTable=dhcpFixedIPTable, trapTFTP=trapTFTP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcd=lcd, commandServer=commandServer, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetUsed=dhcpSubnetUsed, systemDaemon=systemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolEntry=dhcpPoolEntry, trapHA=trapHA, haServiceRunning=haServiceRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, trapReplication=trapReplication, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonRunning=dnsDaemonRunning, lcdDaemon=lcdDaemon, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCP=trapDHCP, haService=haService, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServerDaemon=trapCommandServerDaemon, haReplicationBinding=haReplicationBinding, dhcpLeaseHostname=dhcpLeaseHostname)
