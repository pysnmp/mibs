...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", lcd=lcd, adonisTraps=adonisTraps, trapCommandServer=trapCommandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEndIP=dhcpPoolEndIP, adonisObjects=adonisObjects, tftpDaemon=tftpDaemon, dnsStatsRecursion=dnsStatsRecursion, trapReplicationFailure=trapReplicationFailure, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetIP=dhcpSubnetIP, dhcpIP=dhcpIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFailOverState=dhcpFailOverState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDNS=trapDNS, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsFailure=dnsStatsFailure, dhcpLeaseBindState=dhcpLeaseBindState, haServiceNodeType=haServiceNodeType, licenseValid=licenseValid, dhcpFixedIP=dhcpFixedIP, dhcpPoolSize=dhcpPoolSize, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetMask=dhcpSubnetMask, trapReplication=trapReplication, trapDNSDaemon=trapDNSDaemon, haReplicationBinding=haReplicationBinding, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcp=dhcp, commandServer=commandServer, adonis=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpConfig=dhcpConfig, dhcpDaemonRunning=dhcpDaemonRunning, system=system, dnsDaemon=dnsDaemon, dns=dns, systemDaemon=systemDaemon, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemonRunning=commandServerDaemonRunning, tftp=tftp, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPTable=dhcpFixedIPTable, trapDHCP=trapDHCP, dhcpPoolStartIP=dhcpPoolStartIP, trapHA=trapHA, ha=ha, trapSystem=trapSystem, dhcpLeaseHostname=dhcpLeaseHostname, licenseExpiry=licenseExpiry, dhcpFixedIPEntry=dhcpFixedIPEntry, PYSNMP_MODULE_ID=adonis, trapTFTP=trapTFTP, haServiceRunning=haServiceRunning, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, lcdDaemon=lcdDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsReferral=dnsStatsReferral, dhcpSubnetEntry=dhcpSubnetEntry, dhcpStats=dhcpStats, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemon=dhcpDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTPDaemon=trapTFTPDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, haService=haService, dhcpLeaseEndTime=dhcpLeaseEndTime, systemState=systemState, trapSystemDaemon=trapSystemDaemon, dnsStats=dnsStats, dhcpPoolTable=dhcpPoolTable, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCPDaemon=trapDHCPDaemon, commandServerDaemon=commandServerDaemon, dhcpLeaseEntry=dhcpLeaseEntry)
