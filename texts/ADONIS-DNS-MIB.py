...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonRunning=dnsDaemonRunning, systemState=systemState, commandServer=commandServer, haServiceNodeType=haServiceNodeType, trapReplication=trapReplication, dnsDaemonDebugLevel=dnsDaemonDebugLevel, systemDaemon=systemDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXDomain=dnsStatsNXDomain, trapSystemDaemon=trapSystemDaemon, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCP=trapDHCP, dhcpPoolTable=dhcpPoolTable, PYSNMP_MODULE_ID=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSize=dhcpPoolSize, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetUsed=dhcpSubnetUsed, lcd=lcd, haService=haService, dnsStatsFailure=dnsStatsFailure, dhcpConfig=dhcpConfig, tftpDaemonRunning=tftpDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseTable=dhcpLeaseTable, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonRunning=dhcpDaemonRunning, tftp=tftp, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolStartIP=dhcpPoolStartIP, licenseValid=licenseValid, trapReplicationFailure=trapReplicationFailure, commandServerDaemon=commandServerDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPTable=dhcpFixedIPTable, trapTFTP=trapTFTP, tftpDaemon=tftpDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServer=trapCommandServer, dhcpSubnetTable=dhcpSubnetTable, dnsDaemon=dnsDaemon, adonisObjects=adonisObjects, dhcpSubnetAlert=dhcpSubnetAlert, trapSystem=trapSystem, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, haReplicationBinding=haReplicationBinding, dhcpIP=dhcpIP, dhcpStats=dhcpStats, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, licenseExpiry=licenseExpiry, dnsStats=dnsStats, system=system, dhcp=dhcp, dhcpLeaseHostname=dhcpLeaseHostname, lcdDaemon=lcdDaemon, dhcpFailOverState=dhcpFailOverState, trapDNS=trapDNS, dhcpPoolUsed=dhcpPoolUsed, haServiceRunning=haServiceRunning, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolAlert=dhcpPoolAlert, adonis=adonis, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIP=dhcpFixedIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, ha=ha, adonisTraps=adonisTraps, dhcpSubnetMask=dhcpSubnetMask, trapCommandServerDaemon=trapCommandServerDaemon, dns=dns, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure)
