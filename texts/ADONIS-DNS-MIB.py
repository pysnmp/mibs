...
enterprises, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, TimeTicks, Integer32, ObjectIdentity, Unsigned32, Gauge32, NotificationType, Counter32, MibIdentifier, IpAddress, ModuleIdentity, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "TimeTicks", "Integer32", "ObjectIdentity", "Unsigned32", "Gauge32", "NotificationType", "Counter32", "MibIdentifier", "IpAddress", "ModuleIdentity", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTPDaemon=trapTFTPDaemon, dnsStatsFailure=dnsStatsFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisObjects=adonisObjects, dhcpSubnetUsed=dhcpSubnetUsed, trapReplication=trapReplication, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsNXDomain=dnsStatsNXDomain, haServiceNodeType=haServiceNodeType, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIP=dhcpFixedIP, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, haServiceRunning=haServiceRunning, commandServerDaemon=commandServerDaemon, dnsStatsReferral=dnsStatsReferral, ha=ha, licenseExpiry=licenseExpiry, trapReplicationFailure=trapReplicationFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStats=dnsStats, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServer=trapCommandServer, system=system, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetMask=dhcpSubnetMask, trapSystemDaemon=trapSystemDaemon, trapTFTP=trapTFTP, dhcpFailOverState=dhcpFailOverState, dhcpPoolTable=dhcpPoolTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolAlert=dhcpPoolAlert, tftpDaemonRunning=tftpDaemonRunning, haService=haService, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsRecursion=dnsStatsRecursion, licenseValid=licenseValid, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapCommandServerDaemon=trapCommandServerDaemon, dhcp=dhcp, trapHAServiceFailOver=trapHAServiceFailOver, commandServer=commandServer, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolStartIP=dhcpPoolStartIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, lcdDaemon=lcdDaemon, tftpDaemon=tftpDaemon, dhcpStats=dhcpStats, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetTable=dhcpSubnetTable, dnsDaemon=dnsDaemon, haReplicationBinding=haReplicationBinding, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemState=systemState, trapDHCPDaemon=trapDHCPDaemon, trapHA=trapHA, trapDHCP=trapDHCP, dhcpSubnetEntry=dhcpSubnetEntry, trapDNS=trapDNS, dhcpFixedIPEntry=dhcpFixedIPEntry, adonis=adonis, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseTable=dhcpLeaseTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseHostname=dhcpLeaseHostname, tftp=tftp, adonisTraps=adonisTraps, dhcpIP=dhcpIP, dhcpPoolEntry=dhcpPoolEntry, systemDaemon=systemDaemon, dhcpConfig=dhcpConfig, dhcpPoolSize=dhcpPoolSize, dns=dns, dhcpDaemon=dhcpDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonRunning=dhcpDaemonRunning, trapSystem=trapSystem)
