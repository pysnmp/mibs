...
Unsigned32, enterprises, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, Counter64, Gauge32, iso, TimeTicks, ObjectIdentity, MibIdentifier, Integer32, Counter32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Unsigned32", "enterprises", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "Counter64", "Gauge32", "iso", "TimeTicks", "ObjectIdentity", "MibIdentifier", "Integer32", "Counter32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNS=trapDNS, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolTable=dhcpPoolTable, dhcpSubnetSize=dhcpSubnetSize, trapSystem=trapSystem, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceNodeType=haServiceNodeType, adonisObjects=adonisObjects, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, tftp=tftp, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSize=dhcpPoolSize, trapDNSDaemon=trapDNSDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolAlert=dhcpPoolAlert, adonisTraps=adonisTraps, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, systemState=systemState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTable=dhcpLeaseTable, haService=haService, trapHA=trapHA, dnsStatsReferral=dnsStatsReferral, dhcpLeaseStartTime=dhcpLeaseStartTime, licenseValid=licenseValid, dnsStats=dnsStats, dns=dns, dhcpFailOverState=dhcpFailOverState, commandServerDaemon=commandServerDaemon, trapCommandServer=trapCommandServer, commandServer=commandServer, dhcpDaemonRunning=dhcpDaemonRunning, adonis=adonis, system=system, trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, systemDaemon=systemDaemon, dhcpSubnetAlert=dhcpSubnetAlert, haReplicationBinding=haReplicationBinding, trapReplication=trapReplication, dhcpFixedIP=dhcpFixedIP, lcd=lcd, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, trapHAServiceFailOver=trapHAServiceFailOver, PYSNMP_MODULE_ID=adonis, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcp=dhcp, lcdDaemon=lcdDaemon, tftpDaemonRunning=tftpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, ha=ha, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpStats=dhcpStats, licenseExpiry=licenseExpiry, trapDHCP=trapDHCP, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemon=dhcpDaemon, dnsStatsFailure=dnsStatsFailure, dnsStatsRecursion=dnsStatsRecursion, tftpDaemon=tftpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemon=dnsDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplicationFailure=trapReplicationFailure, trapSystemDaemon=trapSystemDaemon, dhcpSubnetMask=dhcpSubnetMask, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpIP=dhcpIP, haServiceRunning=haServiceRunning)
