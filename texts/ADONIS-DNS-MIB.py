...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapSystemDaemon=trapSystemDaemon, ha=ha, licenseExpiry=licenseExpiry, dhcpSubnetEntry=dhcpSubnetEntry, adonis=adonis, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolTable=dhcpPoolTable, dhcpSubnetMask=dhcpSubnetMask, dhcp=dhcp, dnsStatsReferral=dnsStatsReferral, dhcpFixedIPTable=dhcpFixedIPTable, lcdDaemon=lcdDaemon, tftpDaemon=tftpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetAlert=dhcpSubnetAlert, dhcpStats=dhcpStats, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, systemState=systemState, haService=haService, tftp=tftp, trapDNS=trapDNS, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonRunning=dnsDaemonRunning, trapTFTP=trapTFTP, PYSNMP_MODULE_ID=adonis, dhcpSubnetUsed=dhcpSubnetUsed, commandServerDaemon=commandServerDaemon, dhcpDaemon=dhcpDaemon, dnsStatsSuccess=dnsStatsSuccess, system=system, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapCommandServer=trapCommandServer, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseHostname=dhcpLeaseHostname, dhcpIP=dhcpIP, trapReplicationFailure=trapReplicationFailure, lcd=lcd, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisTraps=adonisTraps, trapDHCP=trapDHCP, trapDNSDaemon=trapDNSDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIP=dhcpFixedIP, haReplicationBinding=haReplicationBinding, dnsStats=dnsStats, systemDaemon=systemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsFailure=dnsStatsFailure, licenseValid=licenseValid, trapHA=trapHA, trapSystem=trapSystem, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSize=dhcpPoolSize, dhcpFailOverState=dhcpFailOverState, dhcpFixedIPEntry=dhcpFixedIPEntry, trapHAServiceFailOver=trapHAServiceFailOver, dns=dns, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplication=trapReplication, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemon=dnsDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpConfig=dhcpConfig, dhcpDaemonRunning=dhcpDaemonRunning, commandServer=commandServer, adonisObjects=adonisObjects, haServiceNodeType=haServiceNodeType, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceRunning=haServiceRunning, trapDHCPDaemon=trapDHCPDaemon)
