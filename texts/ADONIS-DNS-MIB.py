...
Gauge32, NotificationType, TimeTicks, Integer32, Counter32, Counter64, IpAddress, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Unsigned32, Bits, enterprises, ModuleIdentity, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "NotificationType", "TimeTicks", "Integer32", "Counter32", "Counter64", "IpAddress", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Unsigned32", "Bits", "enterprises", "ModuleIdentity", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNS=trapDNS, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpStats=dhcpStats, haReplicationBinding=haReplicationBinding, trapCommandServer=trapCommandServer, dhcpSubnetTable=dhcpSubnetTable, haServiceNodeType=haServiceNodeType, trapHA=trapHA, haServiceRunning=haServiceRunning, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetIP=dhcpSubnetIP, trapDHCP=trapDHCP, dhcpFixedIP=dhcpFixedIP, trapDHCPDaemon=trapDHCPDaemon, haService=haService, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, trapTFTP=trapTFTP, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetMask=dhcpSubnetMask, trapReplicationFailure=trapReplicationFailure, dhcpDaemonRunning=dhcpDaemonRunning, dhcpConfig=dhcpConfig, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolAlert=dhcpPoolAlert, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonDebugLevel=dnsDaemonDebugLevel, licenseExpiry=licenseExpiry, dhcpFixedIPTable=dhcpFixedIPTable, trapDNSDaemon=trapDNSDaemon, commandServer=commandServer, dhcpFailOverState=dhcpFailOverState, dnsDaemonRunning=dnsDaemonRunning, tftp=tftp, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStats=dnsStats, dnsDaemon=dnsDaemon, licenseValid=licenseValid, dhcpFixedIPEntry=dhcpFixedIPEntry, lcd=lcd, dhcpIP=dhcpIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystem=trapSystem, dhcpPoolTable=dhcpPoolTable, ha=ha, systemDaemon=systemDaemon, dhcpDaemon=dhcpDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSize=dhcpPoolSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServerDaemon=trapCommandServerDaemon, dns=dns, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapSystemDaemon=trapSystemDaemon, dhcpPoolEndIP=dhcpPoolEndIP, tftpDaemon=tftpDaemon, dhcpLeaseHostname=dhcpLeaseHostname, adonis=adonis, commandServerDaemon=commandServerDaemon, trapTFTPDaemon=trapTFTPDaemon, dnsStatsReferral=dnsStatsReferral, trapReplication=trapReplication, dnsStatsRecursion=dnsStatsRecursion, PYSNMP_MODULE_ID=adonis, dhcp=dhcp, systemState=systemState, adonisObjects=adonisObjects, lcdDaemon=lcdDaemon, system=system, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolUsed=dhcpPoolUsed, dnsStatsFailure=dnsStatsFailure, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonisTraps=adonisTraps)
