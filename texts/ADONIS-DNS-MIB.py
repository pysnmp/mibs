...
iso, ModuleIdentity, Counter64, Unsigned32, TimeTicks, Counter32, Bits, IpAddress, enterprises, Gauge32, Integer32, ObjectIdentity, MibIdentifier, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "ModuleIdentity", "Counter64", "Unsigned32", "TimeTicks", "Counter32", "Bits", "IpAddress", "enterprises", "Gauge32", "Integer32", "ObjectIdentity", "MibIdentifier", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", system=system, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, PYSNMP_MODULE_ID=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetAlert=dhcpSubnetAlert, licenseValid=licenseValid, haReplicationBinding=haReplicationBinding, dhcpFixedIPTable=dhcpFixedIPTable, tftp=tftp, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpIP=dhcpIP, dhcpPoolTable=dhcpPoolTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, ha=ha, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpStats=dhcpStats, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsReferral=dnsStatsReferral, dhcpLeaseTable=dhcpLeaseTable, dhcp=dhcp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetEntry=dhcpSubnetEntry, trapReplicationFailure=trapReplicationFailure, trapHAServiceFailOver=trapHAServiceFailOver, haServiceRunning=haServiceRunning, tftpDaemon=tftpDaemon, dns=dns, lcd=lcd, dhcpPoolAlert=dhcpPoolAlert, trapReplication=trapReplication, commandServerDaemonRunning=commandServerDaemonRunning, adonis=adonis, dhcpPoolUsed=dhcpPoolUsed, trapDNSDaemon=trapDNSDaemon, trapDHCPDaemon=trapDHCPDaemon, trapDNS=trapDNS, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemon=commandServerDaemon, systemState=systemState, dhcpSubnetSize=dhcpSubnetSize, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonRunning=dnsDaemonRunning, haServiceNodeType=haServiceNodeType, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcdDaemon=lcdDaemon, dhcpLeaseHostname=dhcpLeaseHostname, systemDaemon=systemDaemon, dhcpFailOverState=dhcpFailOverState, dnsStatsRecursion=dnsStatsRecursion, trapSystemDaemon=trapSystemDaemon, tftpDaemonRunning=tftpDaemonRunning, licenseExpiry=licenseExpiry, dhcpSubnetTable=dhcpSubnetTable, dnsStats=dnsStats, commandServer=commandServer, trapDHCP=trapDHCP, trapCommandServerDaemon=trapCommandServerDaemon, adonisObjects=adonisObjects, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHA=trapHA, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetMask=dhcpSubnetMask, adonisTraps=adonisTraps, haService=haService, dnsDaemon=dnsDaemon, trapSystem=trapSystem, trapCommandServer=trapCommandServer, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemon=dhcpDaemon, trapTFTP=trapTFTP, dhcpConfig=dhcpConfig, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseEndTime=dhcpLeaseEndTime)
