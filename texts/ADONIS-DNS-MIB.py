...
Bits, NotificationType, IpAddress, TimeTicks, Counter32, ObjectIdentity, ModuleIdentity, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, Gauge32, MibIdentifier, Counter64, Integer32, iso = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "NotificationType", "IpAddress", "TimeTicks", "Counter32", "ObjectIdentity", "ModuleIdentity", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "Gauge32", "MibIdentifier", "Counter64", "Integer32", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTP=trapTFTP, dhcpPoolEndIP=dhcpPoolEndIP, licenseExpiry=licenseExpiry, trapSystemDaemon=trapSystemDaemon, lcdDaemon=lcdDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetTable=dhcpSubnetTable, haServiceNodeType=haServiceNodeType, trapDNS=trapDNS, commandServerDaemon=commandServerDaemon, PYSNMP_MODULE_ID=adonis, commandServer=commandServer, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplication=trapReplication, trapSystem=trapSystem, adonisTraps=adonisTraps, dnsDaemon=dnsDaemon, system=system, trapHA=trapHA, dhcpSubnetSize=dhcpSubnetSize, dhcpFailOverState=dhcpFailOverState, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDNSDaemon=trapDNSDaemon, dhcp=dhcp, dhcpStats=dhcpStats, adonisObjects=adonisObjects, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsNXDomain=dnsStatsNXDomain, systemState=systemState, dnsStats=dnsStats, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSize=dhcpPoolSize, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolUsed=dhcpPoolUsed, dnsStatsFailure=dnsStatsFailure, tftpDaemon=tftpDaemon, tftp=tftp, dhcpPoolEntry=dhcpPoolEntry, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetUsed=dhcpSubnetUsed, trapReplicationFailure=trapReplicationFailure, trapDHCP=trapDHCP, dnsStatsReferral=dnsStatsReferral, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolAlert=dhcpPoolAlert, ha=ha, dns=dns, haService=haService, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonis=adonis, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemonRunning=commandServerDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, lcd=lcd, dhcpConfig=dhcpConfig, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemon=dhcpDaemon, dhcpFixedIP=dhcpFixedIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceRunning=haServiceRunning, dhcpIP=dhcpIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, licenseValid=licenseValid, dnsDaemonRunning=dnsDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, trapCommandServer=trapCommandServer, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsSuccess=dnsStatsSuccess, systemDaemon=systemDaemon, dhcpDaemonRunning=dhcpDaemonRunning)
