...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServerDaemon=commandServerDaemon, dhcpIP=dhcpIP, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetMask=dhcpSubnetMask, systemState=systemState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplication=trapReplication, system=system, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolUsed=dhcpPoolUsed, commandServer=commandServer, adonis=adonis, haService=haService, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, ha=ha, lcdDaemon=lcdDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIP=dhcpFixedIP, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsReferral=dnsStatsReferral, trapReplicationFailure=trapReplicationFailure, dnsDaemon=dnsDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, tftpDaemon=tftpDaemon, tftpDaemonRunning=tftpDaemonRunning, tftp=tftp, dnsStatsNXDomain=dnsStatsNXDomain, haServiceNodeType=haServiceNodeType, dhcpLeaseHostname=dhcpLeaseHostname, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFailOverState=dhcpFailOverState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpConfig=dhcpConfig, dnsStatsNXRRSet=dnsStatsNXRRSet, trapSystem=trapSystem, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTP=trapTFTP, dhcpPoolEntry=dhcpPoolEntry, PYSNMP_MODULE_ID=adonis, dhcpLeaseTable=dhcpLeaseTable, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, licenseValid=licenseValid, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseBindState=dhcpLeaseBindState, trapSystemDaemon=trapSystemDaemon, dnsStatsFailure=dnsStatsFailure, dhcp=dhcp, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolTable=dhcpPoolTable, dhcpPoolAlert=dhcpPoolAlert, dhcpStats=dhcpStats, dnsStats=dnsStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseExpiry=licenseExpiry, haServiceRunning=haServiceRunning, dns=dns, dhcpSubnetIP=dhcpSubnetIP, trapDHCPDaemon=trapDHCPDaemon, trapDNS=trapDNS, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, systemDaemon=systemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, trapDNSDaemon=trapDNSDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonisTraps=adonisTraps, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemon=dhcpDaemon, dhcpPoolSize=dhcpPoolSize, dhcpSubnetSize=dhcpSubnetSize, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServer=trapCommandServer, adonisObjects=adonisObjects, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, lcd=lcd)
