...
NotificationType, ObjectIdentity, ModuleIdentity, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, Bits, iso, TimeTicks, Counter32, Gauge32, Unsigned32, MibIdentifier, Integer32, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "ObjectIdentity", "ModuleIdentity", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "Bits", "iso", "TimeTicks", "Counter32", "Gauge32", "Unsigned32", "MibIdentifier", "Integer32", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetSize=dhcpSubnetSize, trapSystem=trapSystem, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemon=dhcpDaemon, dhcpPoolEntry=dhcpPoolEntry, haServiceNodeType=haServiceNodeType, trapDNSDaemon=trapDNSDaemon, dhcpSubnetMask=dhcpSubnetMask, trapHAServiceFailOver=trapHAServiceFailOver, licenseExpiry=licenseExpiry, haService=haService, trapReplicationFailure=trapReplicationFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, system=system, commandServer=commandServer, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haServiceRunning=haServiceRunning, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, ha=ha, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dnsStats=dnsStats, trapDHCP=trapDHCP, tftp=tftp, dnsStatsNXDomain=dnsStatsNXDomain, tftpDaemon=tftpDaemon, dhcpFixedIP=dhcpFixedIP, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, systemState=systemState, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplication=trapReplication, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolTable=dhcpPoolTable, lcd=lcd, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSize=dhcpPoolSize, dnsDaemon=dnsDaemon, lcdDaemon=lcdDaemon, licenseValid=licenseValid, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcp=dhcp, dhcpPoolAlert=dhcpPoolAlert, trapDNS=trapDNS, trapCommandServer=trapCommandServer, trapSystemDaemon=trapSystemDaemon, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIPEntry=dhcpFixedIPEntry, adonis=adonis, systemDaemon=systemDaemon, PYSNMP_MODULE_ID=adonis, adonisObjects=adonisObjects, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpConfig=dhcpConfig, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapTFTPDaemon=trapTFTPDaemon, dhcpStats=dhcpStats, dnsStatsFailure=dnsStatsFailure, haReplicationBinding=haReplicationBinding, dhcpSubnetTable=dhcpSubnetTable, dhcpIP=dhcpIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dns=dns, dhcpLeaseBindState=dhcpLeaseBindState, tftpDaemonRunning=tftpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonRunning=dnsDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolStartIP=dhcpPoolStartIP, adonisTraps=adonisTraps, dhcpFixedIPTable=dhcpFixedIPTable, trapTFTP=trapTFTP, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemonRunning=dhcpDaemonRunning)
