...
ObjectIdentity, Unsigned32, iso, IpAddress, enterprises, Gauge32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, TimeTicks, Counter32, MibIdentifier, ModuleIdentity, Counter64, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Unsigned32", "iso", "IpAddress", "enterprises", "Gauge32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "TimeTicks", "Counter32", "MibIdentifier", "ModuleIdentity", "Counter64", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", licenseValid=licenseValid, dhcpPoolSize=dhcpPoolSize, dhcpStats=dhcpStats, trapSystemDaemon=trapSystemDaemon, dnsStats=dnsStats, dhcpPoolStartIP=dhcpPoolStartIP, commandServerDaemonRunning=commandServerDaemonRunning, adonisTraps=adonisTraps, dns=dns, haServiceRunning=haServiceRunning, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemon=commandServerDaemon, dhcpFixedIP=dhcpFixedIP, PYSNMP_MODULE_ID=adonis, dhcpPoolAlert=dhcpPoolAlert, tftpDaemon=tftpDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolTable=dhcpPoolTable, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNSDaemon=trapDNSDaemon, dhcpFixedIPTable=dhcpFixedIPTable, trapSystem=trapSystem, dnsStatsFailure=dnsStatsFailure, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcp=dhcp, dhcpLeaseEntry=dhcpLeaseEntry, system=system, tftp=tftp, adonis=adonis, dnsStatsSuccess=dnsStatsSuccess, dhcpFailOverState=dhcpFailOverState, trapDHCPDaemon=trapDHCPDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapCommandServerDaemon=trapCommandServerDaemon, trapReplication=trapReplication, dhcpIP=dhcpIP, licenseExpiry=licenseExpiry, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServer=commandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplicationFailure=trapReplicationFailure, tftpDaemonRunning=tftpDaemonRunning, trapTFTP=trapTFTP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, ha=ha, dnsDaemonDebugLevel=dnsDaemonDebugLevel, systemDaemon=systemDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsReferral=dnsStatsReferral, trapDNS=trapDNS, lcdDaemon=lcdDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapHA=trapHA, haService=haService, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetUsed=dhcpSubnetUsed, adonisObjects=adonisObjects, dhcpDaemon=dhcpDaemon, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpConfig=dhcpConfig, lcd=lcd, trapCommandServer=trapCommandServer, dnsDaemon=dnsDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetMask=dhcpSubnetMask, haServiceNodeType=haServiceNodeType, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemState=systemState, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP)
