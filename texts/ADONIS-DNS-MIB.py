...
iso, ObjectIdentity, Counter64, ModuleIdentity, Unsigned32, Integer32, Gauge32, enterprises, NotificationType, TimeTicks, MibIdentifier, Counter32, Bits, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "ObjectIdentity", "Counter64", "ModuleIdentity", "Unsigned32", "Integer32", "Gauge32", "enterprises", "NotificationType", "TimeTicks", "MibIdentifier", "Counter32", "Bits", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemDaemon=systemDaemon, tftpDaemonRunning=tftpDaemonRunning, adonisTraps=adonisTraps, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, dhcpFixedIPTable=dhcpFixedIPTable, dhcp=dhcp, commandServerDaemon=commandServerDaemon, dhcpStats=dhcpStats, trapDHCP=trapDHCP, dhcpDaemon=dhcpDaemon, dhcpIP=dhcpIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapTFTP=trapTFTP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpFailOverState=dhcpFailOverState, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemon=tftpDaemon, dnsStats=dnsStats, haReplicationBinding=haReplicationBinding, dnsStatsFailure=dnsStatsFailure, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEndIP=dhcpPoolEndIP, commandServer=commandServer, lcdDaemon=lcdDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpConfig=dhcpConfig, dhcpSubnetTable=dhcpSubnetTable, trapHA=trapHA, dhcpFixedIP=dhcpFixedIP, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dns=dns, dhcpPoolAlert=dhcpPoolAlert, adonisObjects=adonisObjects, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, trapSystemDaemon=trapSystemDaemon, lcd=lcd, ha=ha, system=system, dnsStatsReferral=dnsStatsReferral, dhcpSubnetSize=dhcpSubnetSize, trapCommandServer=trapCommandServer, trapSystem=trapSystem, dhcpLeaseBindState=dhcpLeaseBindState, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSubnetIP=dhcpPoolSubnetIP, licenseExpiry=licenseExpiry, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, PYSNMP_MODULE_ID=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNS=trapDNS, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haService=haService, trapReplicationFailure=trapReplicationFailure, systemState=systemState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, licenseValid=licenseValid, trapReplication=trapReplication, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetEntry=dhcpSubnetEntry, trapDNSDaemon=trapDNSDaemon, tftp=tftp, dnsDaemon=dnsDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haServiceRunning=haServiceRunning, dhcpPoolSize=dhcpPoolSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceNodeType=haServiceNodeType, dhcpPoolStartIP=dhcpPoolStartIP, adonis=adonis)
