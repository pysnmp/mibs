...
NotificationType, Counter32, Unsigned32, Bits, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, TimeTicks, MibIdentifier, Counter64, IpAddress, ObjectIdentity, ModuleIdentity, Gauge32, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "Counter32", "Unsigned32", "Bits", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "TimeTicks", "MibIdentifier", "Counter64", "IpAddress", "ObjectIdentity", "ModuleIdentity", "Gauge32", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapSystemDaemon=trapSystemDaemon, tftpDaemon=tftpDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonRunning=dhcpDaemonRunning, system=system, PYSNMP_MODULE_ID=adonis, trapDNS=trapDNS, dnsStatsRecursion=dnsStatsRecursion, dhcpFailOverState=dhcpFailOverState, dnsStatsNXDomain=dnsStatsNXDomain, trapReplication=trapReplication, trapSystem=trapSystem, lcd=lcd, trapCommandServer=trapCommandServer, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, haServiceRunning=haServiceRunning, trapDHCP=trapDHCP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, systemDaemon=systemDaemon, dhcpPoolUsed=dhcpPoolUsed, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSize=dhcpPoolSize, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, licenseValid=licenseValid, trapReplicationFailure=trapReplicationFailure, dhcpSubnetSize=dhcpSubnetSize, dhcpIP=dhcpIP, ha=ha, trapHA=trapHA, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisTraps=adonisTraps, haReplicationBinding=haReplicationBinding, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolTable=dhcpPoolTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dns=dns, dhcpPoolAlert=dhcpPoolAlert, tftpDaemonRunning=tftpDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTP=trapTFTP, systemState=systemState, trapHAServiceFailOver=trapHAServiceFailOver, lcdDaemon=lcdDaemon, tftp=tftp, dhcp=dhcp, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetTable=dhcpSubnetTable, adonis=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStats=dnsStats, dnsStatsSuccess=dnsStatsSuccess, dnsDaemon=dnsDaemon, dnsStatsFailure=dnsStatsFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemon=dhcpDaemon, haService=haService, haServiceNodeType=haServiceNodeType, dhcpConfig=dhcpConfig, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisObjects=adonisObjects, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, licenseExpiry=licenseExpiry, trapDNSDaemon=trapDNSDaemon, dhcpStats=dhcpStats, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServerDaemon=commandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, commandServer=commandServer, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEndIP=dhcpPoolEndIP)
