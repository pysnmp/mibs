...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetMask=dhcpSubnetMask, dnsStatsSuccess=dnsStatsSuccess, trapSystemDaemon=trapSystemDaemon, trapDNS=trapDNS, systemDaemon=systemDaemon, dhcpPoolSize=dhcpPoolSize, lcdDaemon=lcdDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpConfig=dhcpConfig, tftpDaemon=tftpDaemon, haServiceNodeType=haServiceNodeType, dnsDaemon=dnsDaemon, haServiceRunning=haServiceRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpStats=dhcpStats, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dhcpIP=dhcpIP, trapHA=trapHA, dhcpSubnetAlert=dhcpSubnetAlert, adonisTraps=adonisTraps, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, dnsStats=dnsStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemon=dhcpDaemon, systemState=systemState, dhcpPoolTable=dhcpPoolTable, tftp=tftp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEntry=dhcpLeaseEntry, trapDHCPDaemon=trapDHCPDaemon, dnsStatsFailure=dnsStatsFailure, dhcpFailOverState=dhcpFailOverState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisObjects=adonisObjects, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapTFTPDaemon=trapTFTPDaemon, commandServer=commandServer, licenseValid=licenseValid, trapHAServiceFailOver=trapHAServiceFailOver, trapReplicationFailure=trapReplicationFailure, trapDHCP=trapDHCP, trapReplication=trapReplication, commandServerDaemon=commandServerDaemon, dns=dns, trapSystem=trapSystem, dhcpSubnetIP=dhcpSubnetIP, adonis=adonis, dhcpLeaseBindState=dhcpLeaseBindState, ha=ha, dhcpFixedIP=dhcpFixedIP, dnsStatsReferral=dnsStatsReferral, licenseExpiry=licenseExpiry, dhcpPoolEndIP=dhcpPoolEndIP, PYSNMP_MODULE_ID=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, trapTFTP=trapTFTP, lcd=lcd, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, system=system, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIPTable=dhcpFixedIPTable, dhcp=dhcp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolStartIP=dhcpPoolStartIP, trapDNSDaemon=trapDNSDaemon, trapCommandServer=trapCommandServer, haService=haService, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXDomain=dnsStatsNXDomain)
