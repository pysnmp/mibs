...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemon=dnsDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolAlert=dhcpPoolAlert, trapDHCP=trapDHCP, dhcpSubnetMask=dhcpSubnetMask, haReplicationBinding=haReplicationBinding, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonRunning=dhcpDaemonRunning, PYSNMP_MODULE_ID=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dns=dns, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcp=dhcp, lcdDaemon=lcdDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIP=dhcpFixedIP, haServiceRunning=haServiceRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetUsed=dhcpSubnetUsed, tftp=tftp, dnsStatsFailure=dnsStatsFailure, dnsStats=dnsStats, tftpDaemonRunning=tftpDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, systemDaemon=systemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEndIP=dhcpPoolEndIP, adonisObjects=adonisObjects, lcd=lcd, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftpDaemon=tftpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPTable=dhcpFixedIPTable, trapDNSDaemon=trapDNSDaemon, haService=haService, dhcpLeaseEntry=dhcpLeaseEntry, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpIP=dhcpIP, trapHA=trapHA, commandServerDaemon=commandServerDaemon, trapSystemDaemon=trapSystemDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplicationFailure=trapReplicationFailure, dhcpFailOverState=dhcpFailOverState, dhcpDaemon=dhcpDaemon, haServiceNodeType=haServiceNodeType, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpStats=dhcpStats, dhcpPoolTable=dhcpPoolTable, trapCommandServer=trapCommandServer, ha=ha, adonisTraps=adonisTraps, systemState=systemState, dhcpSubnetAlert=dhcpSubnetAlert, adonis=adonis, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseExpiry=licenseExpiry, dhcpConfig=dhcpConfig, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsSuccess=dnsStatsSuccess, trapReplication=trapReplication, dhcpPoolSize=dhcpPoolSize, trapSystem=trapSystem, dhcpSubnetIP=dhcpSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDNS=trapDNS, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseStartTime=dhcpLeaseStartTime, system=system, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, licenseValid=licenseValid, commandServer=commandServer, dhcpLeaseHostname=dhcpLeaseHostname, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTP=trapTFTP, dhcpLeaseTable=dhcpLeaseTable)
