...
Integer32, Gauge32, iso, IpAddress, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, NotificationType, MibIdentifier, ModuleIdentity, enterprises, ObjectIdentity, Unsigned32, Counter64, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Gauge32", "iso", "IpAddress", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "NotificationType", "MibIdentifier", "ModuleIdentity", "enterprises", "ObjectIdentity", "Unsigned32", "Counter64", "Counter32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNSDaemon=trapDNSDaemon, dhcpSubnetIP=dhcpSubnetIP, commandServer=commandServer, lcd=lcd, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dns=dns, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsSuccess=dnsStatsSuccess, trapSystemDaemon=trapSystemDaemon, trapTFTPDaemon=trapTFTPDaemon, trapSystem=trapSystem, dhcpFixedIP=dhcpFixedIP, dhcpPoolEndIP=dhcpPoolEndIP, haServiceRunning=haServiceRunning, adonisTraps=adonisTraps, trapReplication=trapReplication, dnsStatsRecursion=dnsStatsRecursion, adonisObjects=adonisObjects, trapTFTP=trapTFTP, tftpDaemon=tftpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetTable=dhcpSubnetTable, haServiceNodeType=haServiceNodeType, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolUsed=dhcpPoolUsed, haService=haService, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolTable=dhcpPoolTable, systemState=systemState, dhcpConfig=dhcpConfig, dhcpStats=dhcpStats, dhcpLeaseTable=dhcpLeaseTable, lcdDaemon=lcdDaemon, dhcpPoolAlert=dhcpPoolAlert, licenseValid=licenseValid, trapDNS=trapDNS, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemon=commandServerDaemon, dnsDaemon=dnsDaemon, trapDHCP=trapDHCP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSize=dhcpPoolSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemDaemon=systemDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplicationFailure=trapReplicationFailure, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonis=adonis, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftp=tftp, dhcpDaemon=dhcpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseHostname=dhcpLeaseHostname, trapHA=trapHA, dhcpIP=dhcpIP, dhcpSubnetSize=dhcpSubnetSize, dnsStatsReferral=dnsStatsReferral, system=system, ha=ha, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, licenseExpiry=licenseExpiry, haReplicationBinding=haReplicationBinding, trapCommandServer=trapCommandServer, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapHAServiceFailOver=trapHAServiceFailOver, dnsStats=dnsStats, dnsStatsFailure=dnsStatsFailure, dhcpPoolEntry=dhcpPoolEntry, dhcp=dhcp, PYSNMP_MODULE_ID=adonis, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonRunning=dnsDaemonRunning, dhcpFailOverState=dhcpFailOverState)
