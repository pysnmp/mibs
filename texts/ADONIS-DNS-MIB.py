...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemonRunning=tftpDaemonRunning, haService=haService, trapCommandServerDaemon=trapCommandServerDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIP=dhcpFixedIP, dnsDaemon=dnsDaemon, trapHA=trapHA, trapDNS=trapDNS, haServiceRunning=haServiceRunning, adonisTraps=adonisTraps, dhcpFixedIPEntry=dhcpFixedIPEntry, PYSNMP_MODULE_ID=adonis, dhcpDaemon=dhcpDaemon, dhcpConfig=dhcpConfig, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemon=commandServerDaemon, dhcpLeaseTable=dhcpLeaseTable, adonis=adonis, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetTable=dhcpSubnetTable, licenseExpiry=licenseExpiry, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolStartIP=dhcpPoolStartIP, haServiceNodeType=haServiceNodeType, trapTFTPDaemon=trapTFTPDaemon, dnsStatsSuccess=dnsStatsSuccess, system=system, dhcp=dhcp, tftp=tftp, dnsStatsRecursion=dnsStatsRecursion, tftpDaemon=tftpDaemon, trapSystemDaemon=trapSystemDaemon, dnsStats=dnsStats, dhcpSubnetIP=dhcpSubnetIP, haReplicationBinding=haReplicationBinding, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetSize=dhcpSubnetSize, ha=ha, dhcpIP=dhcpIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemDaemon=systemDaemon, commandServer=commandServer, lcdDaemon=lcdDaemon, trapReplication=trapReplication, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, lcd=lcd, trapHAServiceFailOver=trapHAServiceFailOver, adonisObjects=adonisObjects, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, licenseValid=licenseValid, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCP=trapDHCP, trapCommandServer=trapCommandServer, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolUsed=dhcpPoolUsed, trapDNSDaemon=trapDNSDaemon, dnsStatsReferral=dnsStatsReferral, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDHCPDaemon=trapDHCPDaemon, trapReplicationFailure=trapReplicationFailure, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonRunning=dnsDaemonRunning, dhcpFailOverState=dhcpFailOverState, dns=dns, dhcpPoolSize=dhcpPoolSize, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTP=trapTFTP, dhcpPoolTable=dhcpPoolTable, dhcpLeaseBindState=dhcpLeaseBindState, trapSystem=trapSystem, dhcpLeaseEndTime=dhcpLeaseEndTime, systemState=systemState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseEntry=dhcpLeaseEntry)
