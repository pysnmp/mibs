...
Bits, enterprises, ModuleIdentity, Integer32, Counter32, Counter64, MibIdentifier, ObjectIdentity, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Gauge32, NotificationType, IpAddress, iso = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "enterprises", "ModuleIdentity", "Integer32", "Counter32", "Counter64", "MibIdentifier", "ObjectIdentity", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Gauge32", "NotificationType", "IpAddress", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNSDaemon=trapDNSDaemon, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXDomain=dnsStatsNXDomain, tftpDaemonRunning=tftpDaemonRunning, tftp=tftp, trapReplicationFailure=trapReplicationFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseEntry=dhcpLeaseEntry, tftpDaemon=tftpDaemon, commandServerDaemonRunning=commandServerDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, trapHA=trapHA, lcd=lcd, dhcpConfig=dhcpConfig, dnsStats=dnsStats, dhcpLeaseBindState=dhcpLeaseBindState, systemState=systemState, dnsDaemon=dnsDaemon, licenseExpiry=licenseExpiry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseTable=dhcpLeaseTable, PYSNMP_MODULE_ID=adonis, trapDHCP=trapDHCP, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTP=trapTFTP, dhcpFixedIPTable=dhcpFixedIPTable, lcdDaemon=lcdDaemon, dnsStatsFailure=dnsStatsFailure, haService=haService, dhcpSubnetIP=dhcpSubnetIP, trapDHCPDaemon=trapDHCPDaemon, system=system, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetTable=dhcpSubnetTable, trapTFTPDaemon=trapTFTPDaemon, licenseValid=licenseValid, dhcpLeaseHostname=dhcpLeaseHostname, trapReplication=trapReplication, trapCommandServerDaemon=trapCommandServerDaemon, systemDaemon=systemDaemon, adonisTraps=adonisTraps, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, adonisObjects=adonisObjects, commandServer=commandServer, dhcpPoolTable=dhcpPoolTable, adonis=adonis, dnsDaemonRunning=dnsDaemonRunning, haReplicationBinding=haReplicationBinding, dhcp=dhcp, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, ha=ha, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPEntry=dhcpFixedIPEntry, trapSystemDaemon=trapSystemDaemon, dhcpPoolUsed=dhcpPoolUsed, dns=dns, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsReferral=dnsStatsReferral, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolSize=dhcpPoolSize, dhcpSubnetUsed=dhcpSubnetUsed, dhcpIP=dhcpIP, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemon=commandServerDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpFailOverState=dhcpFailOverState, trapSystem=trapSystem, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapCommandServer=trapCommandServer, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpStats=dhcpStats, haServiceRunning=haServiceRunning, trapDNS=trapDNS)
