...
Bits, ObjectIdentity, ModuleIdentity, TimeTicks, Gauge32, enterprises, NotificationType, Counter32, Counter64, MibIdentifier, Unsigned32, iso, Integer32, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "ObjectIdentity", "ModuleIdentity", "TimeTicks", "Gauge32", "enterprises", "NotificationType", "Counter32", "Counter64", "MibIdentifier", "Unsigned32", "iso", "Integer32", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplication=trapReplication, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, licenseValid=licenseValid, dnsStatsFailure=dnsStatsFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsNXDomain=dnsStatsNXDomain, haServiceNodeType=haServiceNodeType, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolAlert=dhcpPoolAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcp=dhcp, lcdDaemon=lcdDaemon, dhcpPoolSize=dhcpPoolSize, trapCommandServer=trapCommandServer, dhcpConfig=dhcpConfig, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonisTraps=adonisTraps, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemon=dhcpDaemon, system=system, trapDHCP=trapDHCP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIP=dhcpFixedIP, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, systemState=systemState, dhcpFailOverState=dhcpFailOverState, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolEndIP=dhcpPoolEndIP, licenseExpiry=licenseExpiry, haService=haService, haServiceRunning=haServiceRunning, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpStats=dhcpStats, commandServer=commandServer, dnsStats=dnsStats, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dns=dns, trapTFTPDaemon=trapTFTPDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsReferral=dnsStatsReferral, systemDaemon=systemDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisObjects=adonisObjects, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDNS=trapDNS, dnsDaemon=dnsDaemon, dnsStatsRecursion=dnsStatsRecursion, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetIP=dhcpSubnetIP, trapSystemDaemon=trapSystemDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolUsed=dhcpPoolUsed, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetSize=dhcpSubnetSize, lcd=lcd, tftpDaemon=tftpDaemon, trapReplicationFailure=trapReplicationFailure, tftp=tftp, trapSystem=trapSystem, haReplicationBinding=haReplicationBinding, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, commandServerDaemon=commandServerDaemon, ha=ha, trapTFTP=trapTFTP, dhcpLeaseTable=dhcpLeaseTable, PYSNMP_MODULE_ID=adonis, dhcpIP=dhcpIP, adonis=adonis)
