...
Bits, iso, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, ModuleIdentity, MibIdentifier, TimeTicks, Counter64, Integer32, NotificationType, ObjectIdentity, IpAddress, enterprises, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "iso", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "ModuleIdentity", "MibIdentifier", "TimeTicks", "Counter64", "Integer32", "NotificationType", "ObjectIdentity", "IpAddress", "enterprises", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetIP=dhcpSubnetIP, trapDHCP=trapDHCP, trapReplication=trapReplication, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEntry=dhcpPoolEntry, dhcpStats=dhcpStats, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIP=dhcpFixedIP, tftpDaemonRunning=tftpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, system=system, dnsStatsFailure=dnsStatsFailure, adonisObjects=adonisObjects, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, adonis=adonis, commandServer=commandServer, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftpDaemon=tftpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolAlert=dhcpPoolAlert, commandServerDaemon=commandServerDaemon, licenseValid=licenseValid, haReplicationBinding=haReplicationBinding, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpIP=dhcpIP, adonisTraps=adonisTraps, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsReferral=dnsStatsReferral, dhcpFailOverState=dhcpFailOverState, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetMask=dhcpSubnetMask, trapSystemDaemon=trapSystemDaemon, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHostname=dhcpLeaseHostname, systemDaemon=systemDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpConfig=dhcpConfig, dhcpLeaseEndTime=dhcpLeaseEndTime, lcd=lcd, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDNS=trapDNS, dns=dns, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapSystem=trapSystem, trapHAServiceFailOver=trapHAServiceFailOver, ha=ha, dhcpDaemon=dhcpDaemon, tftp=tftp, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, lcdDaemon=lcdDaemon, dnsDaemon=dnsDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcp=dhcp, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolTable=dhcpPoolTable, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, licenseExpiry=licenseExpiry, haServiceRunning=haServiceRunning, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapHA=trapHA, trapDNSDaemon=trapDNSDaemon, haService=haService, dnsStats=dnsStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceNodeType=haServiceNodeType, dnsStatsNXDomain=dnsStatsNXDomain, systemState=systemState, trapCommandServer=trapCommandServer, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapTFTP=trapTFTP, PYSNMP_MODULE_ID=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSize=dhcpPoolSize)
