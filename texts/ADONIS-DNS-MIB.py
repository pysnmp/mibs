...
Bits, TimeTicks, MibIdentifier, Gauge32, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, IpAddress, Counter64, iso, enterprises, Unsigned32, NotificationType, Counter32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "TimeTicks", "MibIdentifier", "Gauge32", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "IpAddress", "Counter64", "iso", "enterprises", "Unsigned32", "NotificationType", "Counter32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haService=haService, dhcpLeaseTable=dhcpLeaseTable, trapDNS=trapDNS, dhcpPoolEndIP=dhcpPoolEndIP, dhcpStats=dhcpStats, licenseValid=licenseValid, dhcpIP=dhcpIP, dhcpPoolSize=dhcpPoolSize, tftp=tftp, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHostname=dhcpLeaseHostname, dnsStats=dnsStats, trapSystem=trapSystem, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNSDaemon=trapDNSDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, PYSNMP_MODULE_ID=adonis, adonisObjects=adonisObjects, dhcpFailOverState=dhcpFailOverState, adonisTraps=adonisTraps, dnsStatsRecursion=dnsStatsRecursion, lcdDaemon=lcdDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolEntry=dhcpPoolEntry, systemState=systemState, trapHA=trapHA, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemon=dnsDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, ha=ha, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCP=trapDHCP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceRunning=haServiceRunning, dhcpSubnetUsed=dhcpSubnetUsed, haReplicationBinding=haReplicationBinding, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPTable=dhcpFixedIPTable, system=system, dhcpPoolUsed=dhcpPoolUsed, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolTable=dhcpPoolTable, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, tftpDaemon=tftpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetMask=dhcpSubnetMask, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapTFTP=trapTFTP, licenseExpiry=licenseExpiry, dhcpSubnetIP=dhcpSubnetIP, commandServer=commandServer, dhcpLeaseStartTime=dhcpLeaseStartTime, lcd=lcd, trapCommandServer=trapCommandServer, tftpDaemonRunning=tftpDaemonRunning, dnsStatsReferral=dnsStatsReferral, dhcp=dhcp, dhcpConfig=dhcpConfig, trapReplication=trapReplication, dnsStatsFailure=dnsStatsFailure, trapSystemDaemon=trapSystemDaemon, dns=dns, haServiceNodeType=haServiceNodeType, dhcpFixedIP=dhcpFixedIP, commandServerDaemonRunning=commandServerDaemonRunning, trapReplicationFailure=trapReplicationFailure, dhcpDaemon=dhcpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, systemDaemon=systemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, adonis=adonis, dhcpSubnetSize=dhcpSubnetSize)
