...
enterprises, iso, IpAddress, Gauge32, Integer32, ModuleIdentity, Counter32, ObjectIdentity, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, NotificationType, Bits, MibIdentifier, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "iso", "IpAddress", "Gauge32", "Integer32", "ModuleIdentity", "Counter32", "ObjectIdentity", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "NotificationType", "Bits", "MibIdentifier", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsSuccess=dnsStatsSuccess, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, dhcpPoolStartIP=dhcpPoolStartIP, adonis=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemon=dnsDaemon, trapTFTPDaemon=trapTFTPDaemon, dnsStats=dnsStats, dns=dns, tftpDaemon=tftpDaemon, trapDNS=trapDNS, dhcpLeaseTable=dhcpLeaseTable, trapReplication=trapReplication, haService=haService, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceRunning=haServiceRunning, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcp=dhcp, dhcpPoolUsed=dhcpPoolUsed, dhcpFailOverState=dhcpFailOverState, tftp=tftp, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetIP=dhcpSubnetIP, trapSystem=trapSystem, system=system, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystemDaemon=trapSystemDaemon, trapHA=trapHA, commandServer=commandServer, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, trapCommandServer=trapCommandServer, systemState=systemState, licenseValid=licenseValid, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolTable=dhcpPoolTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEntry=dhcpLeaseEntry, commandServerDaemon=commandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemDaemon=systemDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemon=dhcpDaemon, dnsStatsReferral=dnsStatsReferral, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEndTime=dhcpLeaseEndTime, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetTable=dhcpSubnetTable, lcdDaemon=lcdDaemon, dhcpStats=dhcpStats, trapDNSDaemon=trapDNSDaemon, adonisObjects=adonisObjects, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, trapDHCPDaemon=trapDHCPDaemon, dnsStatsFailure=dnsStatsFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseExpiry=licenseExpiry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIPTable=dhcpFixedIPTable, lcd=lcd, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpIP=dhcpIP, dhcpSubnetSize=dhcpSubnetSize, dhcpConfig=dhcpConfig, dhcpPoolSize=dhcpPoolSize, haServiceNodeType=haServiceNodeType, adonisTraps=adonisTraps, ha=ha, dhcpPoolAlert=dhcpPoolAlert, dhcpFixedIP=dhcpFixedIP)
