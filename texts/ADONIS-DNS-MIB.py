...
Integer32, IpAddress, enterprises, MibIdentifier, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Gauge32, NotificationType, ModuleIdentity, Bits, ObjectIdentity, Counter64, Counter32, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "IpAddress", "enterprises", "MibIdentifier", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Gauge32", "NotificationType", "ModuleIdentity", "Bits", "ObjectIdentity", "Counter64", "Counter32", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapSystemDaemon=trapSystemDaemon, dhcpSubnetSize=dhcpSubnetSize, dnsDaemon=dnsDaemon, licenseValid=licenseValid, dhcpPoolAlert=dhcpPoolAlert, system=system, adonis=adonis, trapReplication=trapReplication, dhcpFailOverState=dhcpFailOverState, dhcpPoolSize=dhcpPoolSize, trapCommandServerDaemon=trapCommandServerDaemon, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseBindState=dhcpLeaseBindState, systemDaemon=systemDaemon, tftpDaemon=tftpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapHAServiceFailOver=trapHAServiceFailOver, haServiceNodeType=haServiceNodeType, trapSystem=trapSystem, adonisTraps=adonisTraps, trapCommandServer=trapCommandServer, lcdDaemon=lcdDaemon, trapDNSDaemon=trapDNSDaemon, systemState=systemState, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHA=trapHA, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, PYSNMP_MODULE_ID=adonis, licenseExpiry=licenseExpiry, dnsStatsSuccess=dnsStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, ha=ha, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dns=dns, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsReferral=dnsStatsReferral, adonisObjects=adonisObjects, dhcp=dhcp, trapDHCP=trapDHCP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapReplicationFailure=trapReplicationFailure, tftpDaemonRunning=tftpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, tftp=tftp, dhcpPoolUsed=dhcpPoolUsed, trapTFTP=trapTFTP, haService=haService, trapDHCPDaemon=trapDHCPDaemon, dhcpStats=dhcpStats, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolTable=dhcpPoolTable, dnsStatsFailure=dnsStatsFailure, dnsStats=dnsStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetMask=dhcpSubnetMask, lcd=lcd, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServer=commandServer, dhcpSubnetTable=dhcpSubnetTable, dhcpConfig=dhcpConfig, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIP=dhcpFixedIP, dhcpDaemon=dhcpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, commandServerDaemon=commandServerDaemon, haServiceRunning=haServiceRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEndTime=dhcpLeaseEndTime, haReplicationBinding=haReplicationBinding)
