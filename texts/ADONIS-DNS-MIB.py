...
# ASN.1 source ADONIS-DNS-MIB
# Source digest sha256:e8efee820d789a1dea80c83555b77b748f9082ce3ff1135bf002fcb20254c809
...
if mibBuilder.loadTexts: adonis.setLastUpdated('2008-10-01 00:00')
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", PYSNMP_MODULE_ID=adonis, adonis=adonis, adonisObjects=adonisObjects, adonisTraps=adonisTraps, commandServer=commandServer, commandServerDaemon=commandServerDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcp=dhcp, dhcpConfig=dhcpConfig, dhcpDaemon=dhcpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFailOverState=dhcpFailOverState, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpFixedIPTable=dhcpFixedIPTable, dhcpIP=dhcpIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSize=dhcpPoolSize, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolTable=dhcpPoolTable, dhcpPoolUsed=dhcpPoolUsed, dhcpStats=dhcpStats, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetUsed=dhcpSubnetUsed, dns=dns, dnsDaemon=dnsDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStats=dnsStats, dnsStatsFailure=dnsStatsFailure, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsRecursion=dnsStatsRecursion, dnsStatsReferral=dnsStatsReferral, dnsStatsSuccess=dnsStatsSuccess, ha=ha, haReplicationBinding=haReplicationBinding, haService=haService, haServiceNodeType=haServiceNodeType, haServiceRunning=haServiceRunning, lcd=lcd, lcdDaemon=lcdDaemon, licenseExpiry=licenseExpiry, licenseValid=licenseValid, system=system, systemDaemon=systemDaemon, systemState=systemState, tftp=tftp, tftpDaemon=tftpDaemon, tftpDaemonRunning=tftpDaemonRunning, trapCommandServer=trapCommandServer, trapCommandServerDaemon=trapCommandServerDaemon, trapDHCP=trapDHCP, trapDHCPDaemon=trapDHCPDaemon, trapDNS=trapDNS, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, trapReplication=trapReplication, trapReplicationFailure=trapReplicationFailure, trapSystem=trapSystem, trapSystemDaemon=trapSystemDaemon, trapTFTP=trapTFTP, trapTFTPDaemon=trapTFTPDaemon)
