...
IpAddress, Unsigned32, Counter64, Gauge32, Counter32, TimeTicks, enterprises, NotificationType, Integer32, Bits, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, ObjectIdentity, iso = mibBuilder.importSymbols("SNMPv2-SMI", "IpAddress", "Unsigned32", "Counter64", "Gauge32", "Counter32", "TimeTicks", "enterprises", "NotificationType", "Integer32", "Bits", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "ObjectIdentity", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonis=adonis, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, haReplicationBinding=haReplicationBinding, haServiceRunning=haServiceRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServer=commandServer, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDHCP=trapDHCP, dnsStatsFailure=dnsStatsFailure, trapSystem=trapSystem, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNS=trapDNS, dhcp=dhcp, dhcpFixedIP=dhcpFixedIP, trapReplication=trapReplication, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, dnsStatsReferral=dnsStatsReferral, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, adonisTraps=adonisTraps, dhcpDaemon=dhcpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetIP=dhcpSubnetIP, dhcpStats=dhcpStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetTable=dhcpSubnetTable, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, tftp=tftp, licenseValid=licenseValid, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpIP=dhcpIP, systemDaemon=systemDaemon, dhcpSubnetAlert=dhcpSubnetAlert, system=system, haServiceNodeType=haServiceNodeType, systemState=systemState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsRecursion=dnsStatsRecursion, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsSuccess=dnsStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, ha=ha, dhcpConfig=dhcpConfig, dnsStats=dnsStats, dnsDaemonRunning=dnsDaemonRunning, dhcpFailOverState=dhcpFailOverState, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolSize=dhcpPoolSize, licenseExpiry=licenseExpiry, dhcpPoolAlert=dhcpPoolAlert, trapReplicationFailure=trapReplicationFailure, dnsDaemon=dnsDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, tftpDaemon=tftpDaemon, trapTFTP=trapTFTP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolTable=dhcpPoolTable, lcd=lcd, haService=haService, dhcpLeaseBindState=dhcpLeaseBindState, commandServerDaemonRunning=commandServerDaemonRunning, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, PYSNMP_MODULE_ID=adonis, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHAServiceFailOver=trapHAServiceFailOver, commandServerDaemon=commandServerDaemon, adonisObjects=adonisObjects)
