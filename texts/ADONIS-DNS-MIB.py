...
iso, MibIdentifier, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, ObjectIdentity, Bits, enterprises, IpAddress, ModuleIdentity, Integer32, Unsigned32, Gauge32, Counter32, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "MibIdentifier", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "ObjectIdentity", "Bits", "enterprises", "IpAddress", "ModuleIdentity", "Integer32", "Unsigned32", "Gauge32", "Counter32", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseEndTime=dhcpLeaseEndTime, system=system, trapCommandServerDaemon=trapCommandServerDaemon, trapDNSDaemon=trapDNSDaemon, trapTFTP=trapTFTP, trapDHCP=trapDHCP, trapReplicationFailure=trapReplicationFailure, dhcpConfig=dhcpConfig, dhcpFixedIPEntry=dhcpFixedIPEntry, lcdDaemon=lcdDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, licenseExpiry=licenseExpiry, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetIP=dhcpSubnetIP, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSize=dhcpPoolSize, adonisObjects=adonisObjects, systemState=systemState, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseHostname=dhcpLeaseHostname, tftpDaemon=tftpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, haService=haService, trapHA=trapHA, trapDNS=trapDNS, commandServerDaemon=commandServerDaemon, dhcp=dhcp, ha=ha, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsNXDomain=dnsStatsNXDomain, dhcpIP=dhcpIP, dnsStats=dnsStats, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, systemDaemon=systemDaemon, dhcpLeaseBindState=dhcpLeaseBindState, PYSNMP_MODULE_ID=adonis, tftp=tftp, trapDHCPDaemon=trapDHCPDaemon, haServiceRunning=haServiceRunning, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemonRunning=commandServerDaemonRunning, trapSystemDaemon=trapSystemDaemon, commandServer=commandServer, dns=dns, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, tftpDaemonRunning=tftpDaemonRunning, dnsStatsReferral=dnsStatsReferral, dhcpSubnetMask=dhcpSubnetMask, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haReplicationBinding=haReplicationBinding, dhcpFailOverState=dhcpFailOverState, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, lcd=lcd, dhcpSubnetUsed=dhcpSubnetUsed, dhcpStats=dhcpStats, dhcpFixedIP=dhcpFixedIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisTraps=adonisTraps, trapSystem=trapSystem, haServiceNodeType=haServiceNodeType, licenseValid=licenseValid, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemon=dnsDaemon, dnsStatsFailure=dnsStatsFailure, dhcpSubnetSize=dhcpSubnetSize, trapCommandServer=trapCommandServer, trapReplication=trapReplication, adonis=adonis)
