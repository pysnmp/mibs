...
Counter32, ObjectIdentity, Gauge32, MibIdentifier, ModuleIdentity, NotificationType, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, Unsigned32, iso, Integer32, Counter64, IpAddress, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "ObjectIdentity", "Gauge32", "MibIdentifier", "ModuleIdentity", "NotificationType", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "Unsigned32", "iso", "Integer32", "Counter64", "IpAddress", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseEntry=dhcpLeaseEntry, licenseExpiry=licenseExpiry, trapDNS=trapDNS, trapTFTP=trapTFTP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonis=adonis, trapDHCPDaemon=trapDHCPDaemon, trapHA=trapHA, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, lcdDaemon=lcdDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, systemDaemon=systemDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolSize=dhcpPoolSize, trapReplication=trapReplication, licenseValid=licenseValid, dnsStatsFailure=dnsStatsFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpConfig=dhcpConfig, system=system, dnsStatsNXDomain=dnsStatsNXDomain, tftpDaemon=tftpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, trapSystem=trapSystem, systemState=systemState, dhcpStats=dhcpStats, dnsStatsReferral=dnsStatsReferral, ha=ha, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpFailOverState=dhcpFailOverState, tftp=tftp, dhcpSubnetSize=dhcpSubnetSize, trapDHCP=trapDHCP, trapReplicationFailure=trapReplicationFailure, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemonRunning=commandServerDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpIP=dhcpIP, dns=dns, dnsDaemon=dnsDaemon, commandServerDaemon=commandServerDaemon, haReplicationBinding=haReplicationBinding, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolUsed=dhcpPoolUsed, lcd=lcd, dhcpPoolTable=dhcpPoolTable, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetMask=dhcpSubnetMask, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsRecursion=dnsStatsRecursion, haService=haService, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, trapCommandServer=trapCommandServer, haServiceRunning=haServiceRunning, adonisTraps=adonisTraps, adonisObjects=adonisObjects, commandServer=commandServer, dhcp=dhcp, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStats=dnsStats, dhcpPoolAlert=dhcpPoolAlert, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemon=dhcpDaemon, tftpDaemonRunning=tftpDaemonRunning, PYSNMP_MODULE_ID=adonis, trapSystemDaemon=trapSystemDaemon)
