...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsReferral=dnsStatsReferral, dnsStatsFailure=dnsStatsFailure, PYSNMP_MODULE_ID=adonis, tftpDaemon=tftpDaemon, adonisTraps=adonisTraps, trapDNS=trapDNS, dhcpPoolUsed=dhcpPoolUsed, trapSystemDaemon=trapSystemDaemon, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHA=trapHA, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTP=trapTFTP, haReplicationBinding=haReplicationBinding, trapDNSDaemon=trapDNSDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseValid=licenseValid, system=system, tftp=tftp, dnsStatsNXDomain=dnsStatsNXDomain, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCP=trapDHCP, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonRunning=dnsDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dns=dns, dnsStats=dnsStats, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, haService=haService, haServiceRunning=haServiceRunning, commandServerDaemon=commandServerDaemon, haServiceNodeType=haServiceNodeType, trapReplication=trapReplication, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemon=dhcpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonRunning=dhcpDaemonRunning, commandServer=commandServer, lcdDaemon=lcdDaemon, dhcpPoolTable=dhcpPoolTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIP=dhcpFixedIP, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEndIP=dhcpPoolEndIP, trapSystem=trapSystem, trapTFTPDaemon=trapTFTPDaemon, dhcpFailOverState=dhcpFailOverState, dhcpLeaseEntry=dhcpLeaseEntry, systemState=systemState, trapReplicationFailure=trapReplicationFailure, trapCommandServer=trapCommandServer, dhcpSubnetMask=dhcpSubnetMask, ha=ha, lcd=lcd, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDHCPDaemon=trapDHCPDaemon, licenseExpiry=licenseExpiry, dhcpPoolSize=dhcpPoolSize, dhcp=dhcp, dhcpSubnetAlert=dhcpSubnetAlert, adonisObjects=adonisObjects, adonis=adonis, dhcpConfig=dhcpConfig, dhcpStats=dhcpStats, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemon=dnsDaemon, dhcpIP=dhcpIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonDebugLevel=dnsDaemonDebugLevel)
