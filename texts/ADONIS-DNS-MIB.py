...
MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, Bits, Gauge32, NotificationType, Integer32, iso, Unsigned32, MibIdentifier, IpAddress, TimeTicks, ModuleIdentity, Counter32, ObjectIdentity, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "Bits", "Gauge32", "NotificationType", "Integer32", "iso", "Unsigned32", "MibIdentifier", "IpAddress", "TimeTicks", "ModuleIdentity", "Counter32", "ObjectIdentity", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplicationFailure=trapReplicationFailure, dhcpStats=dhcpStats, trapDHCP=trapDHCP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, commandServerDaemon=commandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDNS=trapDNS, commandServer=commandServer, tftpDaemon=tftpDaemon, haServiceNodeType=haServiceNodeType, haServiceRunning=haServiceRunning, trapDHCPDaemon=trapDHCPDaemon, lcd=lcd, dhcpLeaseEntry=dhcpLeaseEntry, trapTFTP=trapTFTP, ha=ha, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolSize=dhcpPoolSize, trapSystemDaemon=trapSystemDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolTable=dhcpPoolTable, trapSystem=trapSystem, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, licenseValid=licenseValid, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseBindState=dhcpLeaseBindState, adonisObjects=adonisObjects, dnsStats=dnsStats, trapDNSDaemon=trapDNSDaemon, dnsStatsFailure=dnsStatsFailure, dhcpSubnetIP=dhcpSubnetIP, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, adonis=adonis, dhcpFixedIP=dhcpFixedIP, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEntry=dhcpPoolEntry, licenseExpiry=licenseExpiry, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haService=haService, dhcp=dhcp, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, systemDaemon=systemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, PYSNMP_MODULE_ID=adonis, dnsStatsReferral=dnsStatsReferral, lcdDaemon=lcdDaemon, dhcpPoolAlert=dhcpPoolAlert, systemState=systemState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haReplicationBinding=haReplicationBinding, dhcpDaemon=dhcpDaemon, system=system, dnsDaemon=dnsDaemon, dns=dns, dhcpFailOverState=dhcpFailOverState, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, trapHA=trapHA, tftp=tftp, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonRunning=dhcpDaemonRunning, dhcpIP=dhcpIP, dhcpPoolUsed=dhcpPoolUsed, dhcpConfig=dhcpConfig, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTable=dhcpLeaseTable, trapReplication=trapReplication, trapCommandServerDaemon=trapCommandServerDaemon, adonisTraps=adonisTraps)
