...
enterprises, Unsigned32, ModuleIdentity, Counter32, ObjectIdentity, Counter64, TimeTicks, Bits, MibIdentifier, Gauge32, IpAddress, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Unsigned32", "ModuleIdentity", "Counter32", "ObjectIdentity", "Counter64", "TimeTicks", "Bits", "MibIdentifier", "Gauge32", "IpAddress", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftp=tftp, system=system, licenseValid=licenseValid, licenseExpiry=licenseExpiry, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseEntry=dhcpLeaseEntry, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetUsed=dhcpSubnetUsed, haServiceRunning=haServiceRunning, dnsStats=dnsStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEntry=dhcpPoolEntry, dhcpConfig=dhcpConfig, adonisObjects=adonisObjects, dhcpIP=dhcpIP, trapHA=trapHA, systemState=systemState, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseStartTime=dhcpLeaseStartTime, haReplicationBinding=haReplicationBinding, trapSystemDaemon=trapSystemDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, ha=ha, tftpDaemon=tftpDaemon, dhcpPoolAlert=dhcpPoolAlert, trapReplicationFailure=trapReplicationFailure, trapReplication=trapReplication, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPTable=dhcpFixedIPTable, dns=dns, dhcpSubnetMask=dhcpSubnetMask, PYSNMP_MODULE_ID=adonis, dhcpPoolUsed=dhcpPoolUsed, trapDNS=trapDNS, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServer=commandServer, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonisTraps=adonisTraps, dhcpStats=dhcpStats, lcd=lcd, dhcpLeaseHostname=dhcpLeaseHostname, trapSystem=trapSystem, haServiceNodeType=haServiceNodeType, dhcpSubnetAlert=dhcpSubnetAlert, commandServerDaemon=commandServerDaemon, systemDaemon=systemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcp=dhcp, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolTable=dhcpPoolTable, trapCommandServer=trapCommandServer, dhcpFailOverState=dhcpFailOverState, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemon=dnsDaemon, adonis=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsReferral=dnsStatsReferral, dnsStatsFailure=dnsStatsFailure, haService=haService, trapDNSDaemon=trapDNSDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDHCP=trapDHCP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, trapTFTP=trapTFTP, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemon=dhcpDaemon)
