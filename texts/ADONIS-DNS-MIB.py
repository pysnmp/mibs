...
MibIdentifier, Counter64, ObjectIdentity, Integer32, enterprises, TimeTicks, Bits, NotificationType, Gauge32, iso, Counter32, IpAddress, ModuleIdentity, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Counter64", "ObjectIdentity", "Integer32", "enterprises", "TimeTicks", "Bits", "NotificationType", "Gauge32", "iso", "Counter32", "IpAddress", "ModuleIdentity", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNSDaemon=trapDNSDaemon, commandServer=commandServer, trapSystem=trapSystem, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolAlert=dhcpPoolAlert, commandServerDaemonRunning=commandServerDaemonRunning, trapCommandServer=trapCommandServer, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolTable=dhcpPoolTable, adonisTraps=adonisTraps, dnsStats=dnsStats, licenseExpiry=licenseExpiry, trapDNS=trapDNS, dhcpDaemon=dhcpDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsReferral=dnsStatsReferral, lcd=lcd, dnsStatsSuccess=dnsStatsSuccess, haServiceNodeType=haServiceNodeType, trapSystemDaemon=trapSystemDaemon, systemDaemon=systemDaemon, system=system, dhcpPoolUsed=dhcpPoolUsed, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, tftpDaemon=tftpDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, trapReplication=trapReplication, dnsStatsRecursion=dnsStatsRecursion, dhcpIP=dhcpIP, commandServerDaemon=commandServerDaemon, dhcpStats=dhcpStats, trapDHCP=trapDHCP, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetAlert=dhcpSubnetAlert, haService=haService, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFailOverState=dhcpFailOverState, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetUsed=dhcpSubnetUsed, systemState=systemState, haReplicationBinding=haReplicationBinding, trapTFTP=trapTFTP, dhcpSubnetIP=dhcpSubnetIP, trapReplicationFailure=trapReplicationFailure, dhcpLeaseEntry=dhcpLeaseEntry, adonis=adonis, dhcpPoolSubnetIP=dhcpPoolSubnetIP, tftp=tftp, trapHA=trapHA, adonisObjects=adonisObjects, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolSize=dhcpPoolSize, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, PYSNMP_MODULE_ID=adonis, trapTFTPDaemon=trapTFTPDaemon, dnsStatsFailure=dnsStatsFailure, haServiceRunning=haServiceRunning, dhcpFixedIP=dhcpFixedIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcp=dhcp, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsNXDomain=dnsStatsNXDomain, dns=dns, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, ha=ha, dnsDaemon=dnsDaemon, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, licenseValid=licenseValid, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert)
