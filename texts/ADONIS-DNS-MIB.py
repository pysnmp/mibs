...
Integer32, Unsigned32, Counter32, ModuleIdentity, Bits, Counter64, NotificationType, iso, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, TimeTicks, Gauge32, IpAddress, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Unsigned32", "Counter32", "ModuleIdentity", "Bits", "Counter64", "NotificationType", "iso", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "TimeTicks", "Gauge32", "IpAddress", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsReferral=dnsStatsReferral, lcdDaemon=lcdDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolUsed=dhcpPoolUsed, dhcpConfig=dhcpConfig, trapReplication=trapReplication, adonis=adonis, systemState=systemState, commandServer=commandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetUsed=dhcpSubnetUsed, commandServerDaemonRunning=commandServerDaemonRunning, system=system, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetIP=dhcpSubnetIP, systemDaemon=systemDaemon, trapSystemDaemon=trapSystemDaemon, haService=haService, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, tftpDaemonRunning=tftpDaemonRunning, dhcp=dhcp, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSize=dhcpPoolSize, dns=dns, trapHA=trapHA, trapTFTPDaemon=trapTFTPDaemon, dnsStats=dnsStats, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapTFTP=trapTFTP, trapDHCP=trapDHCP, dhcpPoolTable=dhcpPoolTable, trapDNS=trapDNS, dnsDaemonRunning=dnsDaemonRunning, ha=ha, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsFailure=dnsStatsFailure, PYSNMP_MODULE_ID=adonis, dhcpPoolEntry=dhcpPoolEntry, dhcpFailOverState=dhcpFailOverState, dhcpLeaseBindState=dhcpLeaseBindState, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetMask=dhcpSubnetMask, haServiceNodeType=haServiceNodeType, dhcpIP=dhcpIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServer=trapCommandServer, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemon=dhcpDaemon, haServiceRunning=haServiceRunning, licenseValid=licenseValid, commandServerDaemon=commandServerDaemon, licenseExpiry=licenseExpiry, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIP=dhcpFixedIP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, adonisTraps=adonisTraps, lcd=lcd, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapSystem=trapSystem, dhcpSubnetEntry=dhcpSubnetEntry, trapDNSDaemon=trapDNSDaemon, haReplicationBinding=haReplicationBinding, dnsDaemon=dnsDaemon, adonisObjects=adonisObjects, tftp=tftp, trapReplicationFailure=trapReplicationFailure, dnsStatsNXDomain=dnsStatsNXDomain, dhcpStats=dhcpStats, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDHCPDaemon=trapDHCPDaemon)
