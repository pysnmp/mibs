...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServerDaemon=commandServerDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dnsStats=dnsStats, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSize=dhcpPoolSize, systemState=systemState, dhcpFailOverState=dhcpFailOverState, lcdDaemon=lcdDaemon, commandServer=commandServer, system=system, trapReplication=trapReplication, dhcpConfig=dhcpConfig, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolTable=dhcpPoolTable, dhcpFixedIP=dhcpFixedIP, dhcpLeaseTable=dhcpLeaseTable, trapSystemDaemon=trapSystemDaemon, trapCommandServer=trapCommandServer, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, adonis=adonis, licenseValid=licenseValid, dhcpPoolEndIP=dhcpPoolEndIP, licenseExpiry=licenseExpiry, dnsStatsSuccess=dnsStatsSuccess, trapDHCP=trapDHCP, haServiceNodeType=haServiceNodeType, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dnsDaemon=dnsDaemon, PYSNMP_MODULE_ID=adonis, dns=dns, trapHA=trapHA, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapTFTP=trapTFTP, dnsStatsReferral=dnsStatsReferral, tftp=tftp, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetUsed=dhcpSubnetUsed, trapHAServiceFailOver=trapHAServiceFailOver, trapDNSDaemon=trapDNSDaemon, dhcpSubnetTable=dhcpSubnetTable, trapSystem=trapSystem, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIPTable=dhcpFixedIPTable, adonisObjects=adonisObjects, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, haServiceRunning=haServiceRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpIP=dhcpIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemon=dhcpDaemon, adonisTraps=adonisTraps, dhcpStats=dhcpStats, tftpDaemon=tftpDaemon, lcd=lcd, dnsStatsNXDomain=dnsStatsNXDomain, systemDaemon=systemDaemon, dhcp=dhcp, dnsStatsFailure=dnsStatsFailure, haReplicationBinding=haReplicationBinding, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonRunning=dnsDaemonRunning, ha=ha, dhcpSubnetSize=dhcpSubnetSize, trapReplicationFailure=trapReplicationFailure, trapTFTPDaemon=trapTFTPDaemon, haService=haService, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsNXRRSet=dnsStatsNXRRSet)
