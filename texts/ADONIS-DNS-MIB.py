...
Bits, enterprises, Gauge32, iso, Counter64, Integer32, TimeTicks, Counter32, Unsigned32, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, NotificationType, ObjectIdentity, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "enterprises", "Gauge32", "iso", "Counter64", "Integer32", "TimeTicks", "Counter32", "Unsigned32", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "NotificationType", "ObjectIdentity", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haService=haService, trapTFTPDaemon=trapTFTPDaemon, lcdDaemon=lcdDaemon, trapReplicationFailure=trapReplicationFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonis=adonis, dhcpPoolSize=dhcpPoolSize, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCPDaemon=trapDHCPDaemon, systemDaemon=systemDaemon, dnsDaemon=dnsDaemon, lcd=lcd, trapHA=trapHA, dns=dns, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, PYSNMP_MODULE_ID=adonis, tftpDaemonRunning=tftpDaemonRunning, adonisObjects=adonisObjects, dnsDaemonRunning=dnsDaemonRunning, trapDHCP=trapDHCP, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftpDaemon=tftpDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIP=dhcpFixedIP, tftp=tftp, dhcpFailOverState=dhcpFailOverState, dhcpLeaseStartTime=dhcpLeaseStartTime, trapReplication=trapReplication, dnsStatsReferral=dnsStatsReferral, haServiceNodeType=haServiceNodeType, commandServerDaemon=commandServerDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseTable=dhcpLeaseTable, adonisTraps=adonisTraps, dhcpPoolEntry=dhcpPoolEntry, trapDNS=trapDNS, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStats=dnsStats, dhcpPoolTable=dhcpPoolTable, trapCommandServer=trapCommandServer, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpConfig=dhcpConfig, licenseExpiry=licenseExpiry, trapCommandServerDaemon=trapCommandServerDaemon, dhcpIP=dhcpIP, dhcpLeaseBindState=dhcpLeaseBindState, trapSystem=trapSystem, dhcpSubnetEntry=dhcpSubnetEntry, commandServer=commandServer, dnsStatsFailure=dnsStatsFailure, trapSystemDaemon=trapSystemDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpStats=dhcpStats, haServiceRunning=haServiceRunning, ha=ha, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, system=system, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemState=systemState, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, licenseValid=licenseValid, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetUsed=dhcpSubnetUsed, dhcp=dhcp, haReplicationBinding=haReplicationBinding, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetMask=dhcpSubnetMask, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTP=trapTFTP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolStartIP=dhcpPoolStartIP)
