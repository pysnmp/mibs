...
Counter64, Integer32, MibIdentifier, ObjectIdentity, ModuleIdentity, iso, Unsigned32, enterprises, Gauge32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, NotificationType, IpAddress, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Counter64", "Integer32", "MibIdentifier", "ObjectIdentity", "ModuleIdentity", "iso", "Unsigned32", "enterprises", "Gauge32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "NotificationType", "IpAddress", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonisObjects=adonisObjects, tftpDaemon=tftpDaemon, tftp=tftp, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServerDaemon=commandServerDaemon, dhcp=dhcp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapCommandServerDaemon=trapCommandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize, commandServer=commandServer, dhcpLeaseTable=dhcpLeaseTable, dnsDaemon=dnsDaemon, licenseValid=licenseValid, trapReplicationFailure=trapReplicationFailure, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTP=trapTFTP, dhcpSubnetAlert=dhcpSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetUsed=dhcpSubnetUsed, trapHA=trapHA, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIPTable=dhcpFixedIPTable, trapSystem=trapSystem, lcdDaemon=lcdDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsReferral=dnsStatsReferral, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpConfig=dhcpConfig, dhcpDaemon=dhcpDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, ha=ha, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseHostname=dhcpLeaseHostname, dhcpIP=dhcpIP, dhcpStats=dhcpStats, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsFailure=dnsStatsFailure, dhcpFailOverState=dhcpFailOverState, trapSystemDaemon=trapSystemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseBindState=dhcpLeaseBindState, trapDNS=trapDNS, haReplicationBinding=haReplicationBinding, dhcpFixedIP=dhcpFixedIP, haService=haService, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolTable=dhcpPoolTable, trapDNSDaemon=trapDNSDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonRunning=dnsDaemonRunning, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, haServiceRunning=haServiceRunning, systemDaemon=systemDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapCommandServer=trapCommandServer, commandServerDaemonRunning=commandServerDaemonRunning, trapDHCP=trapDHCP, dhcpLeaseEndTime=dhcpLeaseEndTime, adonisTraps=adonisTraps, dhcpPoolSize=dhcpPoolSize, trapReplication=trapReplication, adonis=adonis, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStats=dnsStats, PYSNMP_MODULE_ID=adonis, dhcpSubnetTable=dhcpSubnetTable, dnsStatsSuccess=dnsStatsSuccess, systemState=systemState, system=system, dnsStatsNXDomain=dnsStatsNXDomain, licenseExpiry=licenseExpiry, haServiceNodeType=haServiceNodeType)
