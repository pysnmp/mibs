...
MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, Counter32, ObjectIdentity, MibIdentifier, iso, Gauge32, enterprises, TimeTicks, Counter64, NotificationType, Bits, Integer32, Unsigned32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "Counter32", "ObjectIdentity", "MibIdentifier", "iso", "Gauge32", "enterprises", "TimeTicks", "Counter64", "NotificationType", "Bits", "Integer32", "Unsigned32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonisObjects=adonisObjects, dhcpLeaseBindState=dhcpLeaseBindState, licenseValid=licenseValid, dhcpIP=dhcpIP, trapDHCP=trapDHCP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetTable=dhcpSubnetTable, tftpDaemonRunning=tftpDaemonRunning, haReplicationBinding=haReplicationBinding, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, PYSNMP_MODULE_ID=adonis, haServiceNodeType=haServiceNodeType, haServiceRunning=haServiceRunning, dhcpSubnetUsed=dhcpSubnetUsed, tftp=tftp, trapSystemDaemon=trapSystemDaemon, dhcpPoolTable=dhcpPoolTable, trapReplicationFailure=trapReplicationFailure, adonis=adonis, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolStartIP=dhcpPoolStartIP, system=system, dnsStats=dnsStats, tftpDaemon=tftpDaemon, trapReplication=trapReplication, dhcpFixedIP=dhcpFixedIP, trapSystem=trapSystem, trapDNSDaemon=trapDNSDaemon, commandServerDaemon=commandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, dnsStatsSuccess=dnsStatsSuccess, dnsStatsRecursion=dnsStatsRecursion, dnsStatsFailure=dnsStatsFailure, dhcpConfig=dhcpConfig, trapDNS=trapDNS, dhcp=dhcp, commandServerDaemonRunning=commandServerDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, dhcpFailOverState=dhcpFailOverState, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseHostname=dhcpLeaseHostname, trapTFTP=trapTFTP, ha=ha, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetMask=dhcpSubnetMask, dns=dns, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDHCPDaemon=trapDHCPDaemon, systemState=systemState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, lcd=lcd, lcdDaemon=lcdDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTPDaemon=trapTFTPDaemon, systemDaemon=systemDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPTable=dhcpFixedIPTable, licenseExpiry=licenseExpiry, dnsStatsReferral=dnsStatsReferral, adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEntry=dhcpPoolEntry, dnsDaemon=dnsDaemon, haService=haService, dhcpPoolSize=dhcpPoolSize, dhcpStats=dhcpStats, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServer=trapCommandServer, dhcpDaemon=dhcpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, commandServer=commandServer, dhcpSubnetEntry=dhcpSubnetEntry, trapHA=trapHA)
