...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapCommandServer=trapCommandServer, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpConfig=dhcpConfig, dhcpSubnetSize=dhcpSubnetSize, lcd=lcd, dnsStatsFailure=dnsStatsFailure, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIP=dhcpFixedIP, dhcpPoolTable=dhcpPoolTable, dhcpPoolUsed=dhcpPoolUsed, licenseValid=licenseValid, trapDNSDaemon=trapDNSDaemon, dhcpFixedIPTable=dhcpFixedIPTable, licenseExpiry=licenseExpiry, haServiceNodeType=haServiceNodeType, dhcp=dhcp, tftpDaemonRunning=tftpDaemonRunning, trapSystem=trapSystem, haService=haService, dhcpPoolSize=dhcpPoolSize, dns=dns, dnsStatsRecursion=dnsStatsRecursion, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServerDaemonRunning=commandServerDaemonRunning, trapReplicationFailure=trapReplicationFailure, adonis=adonis, PYSNMP_MODULE_ID=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNS=trapDNS, systemState=systemState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonRunning=dhcpDaemonRunning, trapReplication=trapReplication, adonisTraps=adonisTraps, commandServer=commandServer, dhcpSubnetTable=dhcpSubnetTable, dnsStatsSuccess=dnsStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, trapDHCP=trapDHCP, adonisObjects=adonisObjects, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapSystemDaemon=trapSystemDaemon, lcdDaemon=lcdDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetEntry=dhcpSubnetEntry, tftp=tftp, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapHA=trapHA, trapTFTP=trapTFTP, dhcpStats=dhcpStats, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemon=dnsDaemon, system=system, ha=ha, haReplicationBinding=haReplicationBinding, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, dnsStats=dnsStats, dnsStatsReferral=dnsStatsReferral, dhcpDaemon=dhcpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haServiceRunning=haServiceRunning, dhcpIP=dhcpIP)
