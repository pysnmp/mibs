...
IpAddress, MibIdentifier, Integer32, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, ModuleIdentity, NotificationType, ObjectIdentity, Bits, Counter64, Gauge32, Counter32, Unsigned32, iso = mibBuilder.importSymbols("SNMPv2-SMI", "IpAddress", "MibIdentifier", "Integer32", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "ModuleIdentity", "NotificationType", "ObjectIdentity", "Bits", "Counter64", "Gauge32", "Counter32", "Unsigned32", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServer=commandServer, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, haService=haService, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTPDaemon=trapTFTPDaemon, trapTFTP=trapTFTP, dhcpLeaseBindState=dhcpLeaseBindState, adonis=adonis, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolUsed=dhcpPoolUsed, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, trapDNS=trapDNS, dnsStatsNXRRSet=dnsStatsNXRRSet, trapSystemDaemon=trapSystemDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, trapReplicationFailure=trapReplicationFailure, trapHA=trapHA, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, haServiceNodeType=haServiceNodeType, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolTable=dhcpPoolTable, adonisTraps=adonisTraps, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolSize=dhcpPoolSize, dhcpDaemonRunning=dhcpDaemonRunning, dnsStats=dnsStats, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolAlert=dhcpPoolAlert, commandServerDaemon=commandServerDaemon, dns=dns, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetSize=dhcpSubnetSize, dnsStatsRecursion=dnsStatsRecursion, trapHAServiceFailOver=trapHAServiceFailOver, tftpDaemon=tftpDaemon, licenseExpiry=licenseExpiry, dhcpIP=dhcpIP, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, systemDaemon=systemDaemon, ha=ha, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisObjects=adonisObjects, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIP=dhcpFixedIP, haServiceRunning=haServiceRunning, tftp=tftp, dhcpLeaseTable=dhcpLeaseTable, haReplicationBinding=haReplicationBinding, licenseValid=licenseValid, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, systemState=systemState, lcd=lcd, trapSystem=trapSystem, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetEntry=dhcpSubnetEntry, lcdDaemon=lcdDaemon, dhcp=dhcp, trapReplication=trapReplication, trapDNSDaemon=trapDNSDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsReferral=dnsStatsReferral, tftpDaemonRunning=tftpDaemonRunning, dhcpStats=dhcpStats, dhcpFailOverState=dhcpFailOverState, dnsStatsFailure=dnsStatsFailure, trapCommandServer=trapCommandServer, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolEntry=dhcpPoolEntry, dnsDaemon=dnsDaemon, dhcpLeaseEntry=dhcpLeaseEntry, trapDHCPDaemon=trapDHCPDaemon, PYSNMP_MODULE_ID=adonis, dnsStatsNXDomain=dnsStatsNXDomain, system=system, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemon=dhcpDaemon, trapDHCP=trapDHCP)
