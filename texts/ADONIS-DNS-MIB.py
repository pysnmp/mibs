...
Integer32, enterprises, iso, Unsigned32, ObjectIdentity, Counter64, ModuleIdentity, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Bits, IpAddress, Counter32, TimeTicks, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "enterprises", "iso", "Unsigned32", "ObjectIdentity", "Counter64", "ModuleIdentity", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Bits", "IpAddress", "Counter32", "TimeTicks", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonRunning=dnsDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, trapReplicationFailure=trapReplicationFailure, dhcpFixedIP=dhcpFixedIP, adonisTraps=adonisTraps, ha=ha, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDHCPDaemon=trapDHCPDaemon, trapHA=trapHA, dhcpLeaseTable=dhcpLeaseTable, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolEntry=dhcpPoolEntry, lcd=lcd, adonis=adonis, dhcpPoolEndIP=dhcpPoolEndIP, haReplicationBinding=haReplicationBinding, dhcpLeaseEntry=dhcpLeaseEntry, lcdDaemon=lcdDaemon, licenseValid=licenseValid, trapDNSDaemon=trapDNSDaemon, trapTFTP=trapTFTP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCP=trapDHCP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolTable=dhcpPoolTable, dhcp=dhcp, dnsDaemon=dnsDaemon, trapReplication=trapReplication, dhcpDaemon=dhcpDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapSystemDaemon=trapSystemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpStats=dhcpStats, system=system, dhcpFixedIPEntry=dhcpFixedIPEntry, systemDaemon=systemDaemon, dnsStats=dnsStats, commandServer=commandServer, adonisObjects=adonisObjects, dns=dns, licenseExpiry=licenseExpiry, dnsStatsSuccess=dnsStatsSuccess, tftp=tftp, dhcpLeaseEndTime=dhcpLeaseEndTime, tftpDaemonRunning=tftpDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, haServiceNodeType=haServiceNodeType, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIPTable=dhcpFixedIPTable, systemState=systemState, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFailOverState=dhcpFailOverState, trapTFTPDaemon=trapTFTPDaemon, PYSNMP_MODULE_ID=adonis, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, trapCommandServerDaemon=trapCommandServerDaemon, trapCommandServer=trapCommandServer, tftpDaemon=tftpDaemon, haServiceRunning=haServiceRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetIP=dhcpSubnetIP, dnsStatsReferral=dnsStatsReferral, dhcpIP=dhcpIP, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetAlert=dhcpSubnetAlert, dhcpConfig=dhcpConfig, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haService=haService, trapHAServiceFailOver=trapHAServiceFailOver, trapDNS=trapDNS, trapSystem=trapSystem)
