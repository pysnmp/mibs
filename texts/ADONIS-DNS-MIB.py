...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplicationFailure=trapReplicationFailure, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetIP=dhcpSubnetIP, trapReplication=trapReplication, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIP=dhcpFixedIP, adonisObjects=adonisObjects, trapDNSDaemon=trapDNSDaemon, licenseValid=licenseValid, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseTable=dhcpLeaseTable, trapTFTP=trapTFTP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolTable=dhcpPoolTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPTable=dhcpFixedIPTable, trapTFTPDaemon=trapTFTPDaemon, dhcpConfig=dhcpConfig, dhcpFailOverState=dhcpFailOverState, adonis=adonis, dhcpPoolStartIP=dhcpPoolStartIP, commandServer=commandServer, trapDNS=trapDNS, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolSize=dhcpPoolSize, dnsStatsNXRRSet=dnsStatsNXRRSet, haReplicationBinding=haReplicationBinding, dnsStatsFailure=dnsStatsFailure, haService=haService, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStats=dnsStats, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, licenseExpiry=licenseExpiry, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, trapCommandServer=trapCommandServer, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolUsed=dhcpPoolUsed, trapSystemDaemon=trapSystemDaemon, dhcpSubnetUsed=dhcpSubnetUsed, lcdDaemon=lcdDaemon, tftp=tftp, trapDHCP=trapDHCP, systemState=systemState, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemon=dhcpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, system=system, trapCommandServerDaemon=trapCommandServerDaemon, trapHA=trapHA, dnsStatsSuccess=dnsStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsReferral=dnsStatsReferral, dnsStatsRecursion=dnsStatsRecursion, PYSNMP_MODULE_ID=adonis, trapSystem=trapSystem, systemDaemon=systemDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpIP=dhcpIP, haServiceRunning=haServiceRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetSize=dhcpSubnetSize, tftpDaemon=tftpDaemon, dns=dns, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, lcd=lcd, commandServerDaemon=commandServerDaemon, haServiceNodeType=haServiceNodeType, dhcpLeaseHostname=dhcpLeaseHostname, adonisTraps=adonisTraps, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolEndIP=dhcpPoolEndIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolAlert=dhcpPoolAlert, ha=ha, dnsDaemon=dnsDaemon, dhcp=dhcp, dhcpStats=dhcpStats, dhcpLeaseStartTime=dhcpLeaseStartTime)
