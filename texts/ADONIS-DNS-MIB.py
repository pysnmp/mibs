...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetAlert=dhcpSubnetAlert, tftpDaemon=tftpDaemon, dhcpIP=dhcpIP, dhcpConfig=dhcpConfig, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolUsed=dhcpPoolUsed, trapSystemDaemon=trapSystemDaemon, adonisObjects=adonisObjects, dnsDaemon=dnsDaemon, ha=ha, dhcpSubnetSize=dhcpSubnetSize, trapDNSDaemon=trapDNSDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonis=adonis, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystem=trapSystem, dhcpLeaseStartTime=dhcpLeaseStartTime, haServiceNodeType=haServiceNodeType, dnsStatsFailure=dnsStatsFailure, dhcpLeaseTable=dhcpLeaseTable, trapDHCP=trapDHCP, dhcpSubnetTable=dhcpSubnetTable, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, licenseExpiry=licenseExpiry, dnsStatsNXDomain=dnsStatsNXDomain, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTPDaemon=trapTFTPDaemon, system=system, haReplicationBinding=haReplicationBinding, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, PYSNMP_MODULE_ID=adonis, dnsStatsRecursion=dnsStatsRecursion, systemState=systemState, trapDNS=trapDNS, dhcpFixedIP=dhcpFixedIP, dhcpPoolEntry=dhcpPoolEntry, dns=dns, trapHAServiceFailOver=trapHAServiceFailOver, systemDaemon=systemDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolTable=dhcpPoolTable, haService=haService, haServiceRunning=haServiceRunning, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftp=tftp, lcd=lcd, dhcp=dhcp, dnsStats=dnsStats, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFailOverState=dhcpFailOverState, commandServer=commandServer, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemon=dhcpDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolSize=dhcpPoolSize, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHostname=dhcpLeaseHostname, trapCommandServer=trapCommandServer, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapReplicationFailure=trapReplicationFailure, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcdDaemon=lcdDaemon, commandServerDaemon=commandServerDaemon, adonisTraps=adonisTraps, licenseValid=licenseValid, trapHA=trapHA, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetUsed=dhcpSubnetUsed, dhcpStats=dhcpStats, trapTFTP=trapTFTP, trapReplication=trapReplication, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseBindState=dhcpLeaseBindState)
