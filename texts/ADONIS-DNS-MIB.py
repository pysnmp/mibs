...
Bits, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, TimeTicks, Integer32, Counter64, Counter32, enterprises, IpAddress, ModuleIdentity, NotificationType, Unsigned32, iso, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "TimeTicks", "Integer32", "Counter64", "Counter32", "enterprises", "IpAddress", "ModuleIdentity", "NotificationType", "Unsigned32", "iso", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpIP=dhcpIP, trapTFTP=trapTFTP, dnsStatsReferral=dnsStatsReferral, dhcpPoolTable=dhcpPoolTable, dhcpPoolAlert=dhcpPoolAlert, systemState=systemState, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemon=dhcpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapSystemDaemon=trapSystemDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolUsed=dhcpPoolUsed, dhcpConfig=dhcpConfig, dhcpPoolSize=dhcpPoolSize, dhcpDaemonRunning=dhcpDaemonRunning, licenseExpiry=licenseExpiry, trapReplicationFailure=trapReplicationFailure, haReplicationBinding=haReplicationBinding, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseHostname=dhcpLeaseHostname, system=system, commandServer=commandServer, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNS=trapDNS, dns=dns, trapHAServiceFailOver=trapHAServiceFailOver, haService=haService, lcdDaemon=lcdDaemon, haServiceNodeType=haServiceNodeType, dhcpSubnetUsed=dhcpSubnetUsed, trapHA=trapHA, tftpDaemonRunning=tftpDaemonRunning, dhcpFailOverState=dhcpFailOverState, trapReplication=trapReplication, trapDHCP=trapDHCP, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpStats=dhcpStats, dnsStatsFailure=dnsStatsFailure, licenseValid=licenseValid, PYSNMP_MODULE_ID=adonis, tftpDaemon=tftpDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcp=dhcp, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisTraps=adonisTraps, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStats=dnsStats, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetMask=dhcpSubnetMask, trapSystem=trapSystem, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServerDaemon=trapCommandServerDaemon, adonisObjects=adonisObjects, adonis=adonis, dhcpSubnetSize=dhcpSubnetSize, tftp=tftp, dnsDaemon=dnsDaemon, dhcpLeaseTable=dhcpLeaseTable, ha=ha, commandServerDaemon=commandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetTable=dhcpSubnetTable, haServiceRunning=haServiceRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIP=dhcpFixedIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServer=trapCommandServer, systemDaemon=systemDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, lcd=lcd, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNSDaemon=trapDNSDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP)
