...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpIP=dhcpIP, haService=haService, trapReplication=trapReplication, dnsStatsSuccess=dnsStatsSuccess, haServiceNodeType=haServiceNodeType, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolSize=dhcpPoolSize, PYSNMP_MODULE_ID=adonis, haServiceRunning=haServiceRunning, dnsStatsReferral=dnsStatsReferral, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsFailure=dnsStatsFailure, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTP=trapTFTP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHA=trapHA, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpConfig=dhcpConfig, dhcpPoolAlert=dhcpPoolAlert, adonis=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNSDaemon=trapDNSDaemon, dnsDaemon=dnsDaemon, trapCommandServer=trapCommandServer, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, ha=ha, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonRunning=dnsDaemonRunning, trapReplicationFailure=trapReplicationFailure, dhcpLeaseEntry=dhcpLeaseEntry, trapDNS=trapDNS, adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXDomain=dnsStatsNXDomain, dhcpStats=dhcpStats, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCP=trapDHCP, dhcpFixedIP=dhcpFixedIP, dhcpFailOverState=dhcpFailOverState, commandServerDaemon=commandServerDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dnsStats=dnsStats, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, dns=dns, dhcpFixedIPEntry=dhcpFixedIPEntry, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, licenseValid=licenseValid, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, adonisObjects=adonisObjects, systemState=systemState, lcd=lcd, trapTFTPDaemon=trapTFTPDaemon, tftpDaemon=tftpDaemon, lcdDaemon=lcdDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemon=dhcpDaemon, dhcp=dhcp, commandServer=commandServer, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTable=dhcpLeaseTable, system=system, licenseExpiry=licenseExpiry, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolTable=dhcpPoolTable, systemDaemon=systemDaemon, trapSystem=trapSystem, dhcpSubnetUsed=dhcpSubnetUsed, tftp=tftp, haReplicationBinding=haReplicationBinding, dhcpPoolEndIP=dhcpPoolEndIP)
