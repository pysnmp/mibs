...
Integer32, ModuleIdentity, Counter32, enterprises, TimeTicks, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Bits, Unsigned32, Gauge32, IpAddress, ObjectIdentity, Counter64, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "ModuleIdentity", "Counter32", "enterprises", "TimeTicks", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Bits", "Unsigned32", "Gauge32", "IpAddress", "ObjectIdentity", "Counter64", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapSystem=trapSystem, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetAlert=dhcpSubnetAlert, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseExpiry=licenseExpiry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseBindState=dhcpLeaseBindState, system=system, dhcpPoolSize=dhcpPoolSize, dnsStats=dnsStats, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsSuccess=dnsStatsSuccess, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsFailure=dnsStatsFailure, dhcpFailOverState=dhcpFailOverState, dhcpPoolUsed=dhcpPoolUsed, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpConfig=dhcpConfig, dhcpSubnetUsed=dhcpSubnetUsed, trapHA=trapHA, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftp=tftp, dhcpDaemonRunning=dhcpDaemonRunning, lcdDaemon=lcdDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapCommandServer=trapCommandServer, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolEntry=dhcpPoolEntry, PYSNMP_MODULE_ID=adonis, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolAlert=dhcpPoolAlert, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, trapDNS=trapDNS, ha=ha, haServiceRunning=haServiceRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEntry=dhcpLeaseEntry, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolTable=dhcpPoolTable, dhcpSubnetIP=dhcpSubnetIP, haServiceNodeType=haServiceNodeType, haService=haService, haReplicationBinding=haReplicationBinding, tftpDaemonRunning=tftpDaemonRunning, dhcpIP=dhcpIP, trapSystemDaemon=trapSystemDaemon, trapDHCP=trapDHCP, dhcpSubnetTable=dhcpSubnetTable, dnsStatsReferral=dnsStatsReferral, trapTFTPDaemon=trapTFTPDaemon, lcd=lcd, trapReplication=trapReplication, systemState=systemState, trapReplicationFailure=trapReplicationFailure, dhcp=dhcp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemon=dhcpDaemon, tftpDaemon=tftpDaemon, adonis=adonis, dhcpFixedIP=dhcpFixedIP, systemDaemon=systemDaemon, licenseValid=licenseValid, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemon=commandServerDaemon, adonisTraps=adonisTraps, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemon=dnsDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolStartIP=dhcpPoolStartIP, dns=dns, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServer=commandServer, trapDNSDaemon=trapDNSDaemon, adonisObjects=adonisObjects, trapTFTP=trapTFTP)
