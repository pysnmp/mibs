...
TimeTicks, NotificationType, Bits, ModuleIdentity, Unsigned32, enterprises, ObjectIdentity, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Counter32, IpAddress, Counter64, Integer32, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "NotificationType", "Bits", "ModuleIdentity", "Unsigned32", "enterprises", "ObjectIdentity", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Counter32", "IpAddress", "Counter64", "Integer32", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemDaemon=systemDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolTable=dhcpPoolTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemon=dnsDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dns=dns, trapSystemDaemon=trapSystemDaemon, adonisObjects=adonisObjects, dhcpDaemon=dhcpDaemon, system=system, dhcpSubnetAlert=dhcpSubnetAlert, lcd=lcd, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolUsed=dhcpPoolUsed, systemState=systemState, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonRunning=dnsDaemonRunning, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonRunning=dhcpDaemonRunning, licenseExpiry=licenseExpiry, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseBindState=dhcpLeaseBindState, commandServerDaemon=commandServerDaemon, dhcpConfig=dhcpConfig, trapSystem=trapSystem, licenseValid=licenseValid, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolStartIP=dhcpPoolStartIP, adonis=adonis, haServiceRunning=haServiceRunning, trapDHCP=trapDHCP, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetTable=dhcpSubnetTable, PYSNMP_MODULE_ID=adonis, adonisTraps=adonisTraps, dhcp=dhcp, trapDNSDaemon=trapDNSDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapHA=trapHA, dnsStatsNXDomain=dnsStatsNXDomain, ha=ha, dnsStatsReferral=dnsStatsReferral, haServiceNodeType=haServiceNodeType, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapReplicationFailure=trapReplicationFailure, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsFailure=dnsStatsFailure, dhcpLeaseTable=dhcpLeaseTable, tftpDaemon=tftpDaemon, dhcpPoolSize=dhcpPoolSize, dhcpFixedIP=dhcpFixedIP, trapTFTP=trapTFTP, trapDHCPDaemon=trapDHCPDaemon, dhcpFailOverState=dhcpFailOverState, haService=haService, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpIP=dhcpIP, dnsStats=dnsStats, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServer=commandServer, dhcpSubnetSize=dhcpSubnetSize, tftpDaemonRunning=tftpDaemonRunning, trapReplication=trapReplication, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, trapDNS=trapDNS, trapCommandServer=trapCommandServer, dhcpStats=dhcpStats, dhcpSubnetIP=dhcpSubnetIP, dnsStatsSuccess=dnsStatsSuccess, lcdDaemon=lcdDaemon, tftp=tftp, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones)
