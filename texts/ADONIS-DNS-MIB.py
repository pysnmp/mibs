...
ModuleIdentity, enterprises, IpAddress, Integer32, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, NotificationType, iso, TimeTicks, Counter32, Counter64, Bits, ObjectIdentity, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "enterprises", "IpAddress", "Integer32", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "NotificationType", "iso", "TimeTicks", "Counter32", "Counter64", "Bits", "ObjectIdentity", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcp=dhcp, dhcpFixedIP=dhcpFixedIP, commandServerDaemon=commandServerDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEntry=dhcpPoolEntry, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolTable=dhcpPoolTable, tftp=tftp, dhcpSubnetTable=dhcpSubnetTable, adonisTraps=adonisTraps, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTPDaemon=trapTFTPDaemon, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapReplication=trapReplication, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpStats=dhcpStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsRecursion=dnsStatsRecursion, dnsStatsReferral=dnsStatsReferral, dhcpDaemon=dhcpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseExpiry=licenseExpiry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetSize=dhcpSubnetSize, dnsStats=dnsStats, trapCommandServerDaemon=trapCommandServerDaemon, adonis=adonis, trapSystem=trapSystem, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, lcdDaemon=lcdDaemon, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, dnsStatsFailure=dnsStatsFailure, dhcpFailOverState=dhcpFailOverState, trapHAServiceFailOver=trapHAServiceFailOver, trapCommandServer=trapCommandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, system=system, dhcpIP=dhcpIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haReplicationBinding=haReplicationBinding, commandServer=commandServer, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpConfig=dhcpConfig, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNS=trapDNS, PYSNMP_MODULE_ID=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, haServiceRunning=haServiceRunning, systemDaemon=systemDaemon, dnsDaemonRunning=dnsDaemonRunning, systemState=systemState, tftpDaemon=tftpDaemon, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dns=dns, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTP=trapTFTP, ha=ha, trapSystemDaemon=trapSystemDaemon, dhcpPoolSize=dhcpPoolSize, dnsDaemon=dnsDaemon, dhcpDaemonRunning=dhcpDaemonRunning, trapReplicationFailure=trapReplicationFailure, trapDHCPDaemon=trapDHCPDaemon, trapDHCP=trapDHCP, commandServerDaemonRunning=commandServerDaemonRunning, lcd=lcd, adonisObjects=adonisObjects, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetMask=dhcpSubnetMask, haService=haService, licenseValid=licenseValid, dhcpLeaseEntry=dhcpLeaseEntry)
