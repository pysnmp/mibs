...
ObjectIdentity, Counter64, NotificationType, enterprises, iso, MibIdentifier, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, Unsigned32, Integer32, Counter32, Bits, TimeTicks, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Counter64", "NotificationType", "enterprises", "iso", "MibIdentifier", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "Unsigned32", "Integer32", "Counter32", "Bits", "TimeTicks", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFailOverState=dhcpFailOverState, dhcpPoolSize=dhcpPoolSize, commandServerDaemon=commandServerDaemon, system=system, systemDaemon=systemDaemon, dnsStatsFailure=dnsStatsFailure, dhcpLeaseTable=dhcpLeaseTable, haServiceNodeType=haServiceNodeType, commandServer=commandServer, dhcpPoolEntry=dhcpPoolEntry, tftpDaemonRunning=tftpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, ha=ha, dhcp=dhcp, tftp=tftp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, adonis=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemon=dhcpDaemon, lcdDaemon=lcdDaemon, dnsStatsNXDomain=dnsStatsNXDomain, licenseExpiry=licenseExpiry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEndIP=dhcpPoolEndIP, trapReplicationFailure=trapReplicationFailure, dhcpIP=dhcpIP, haServiceRunning=haServiceRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNSDaemon=trapDNSDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpStats=dhcpStats, licenseValid=licenseValid, trapDHCP=trapDHCP, trapCommandServer=trapCommandServer, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemon=dnsDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonRunning=dhcpDaemonRunning, dns=dns, adonisObjects=adonisObjects, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetSize=dhcpSubnetSize, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, haReplicationBinding=haReplicationBinding, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetIP=dhcpSubnetIP, lcd=lcd, dnsDaemonRunning=dnsDaemonRunning, haService=haService, trapDNS=trapDNS, trapReplication=trapReplication, dhcpFixedIP=dhcpFixedIP, dhcpPoolTable=dhcpPoolTable, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystem=trapSystem, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolAlert=dhcpPoolAlert, adonisTraps=adonisTraps, systemState=systemState, trapHA=trapHA, dhcpConfig=dhcpConfig, dnsStats=dnsStats, dnsStatsReferral=dnsStatsReferral, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemon=tftpDaemon, trapDHCPDaemon=trapDHCPDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTPDaemon=trapTFTPDaemon, PYSNMP_MODULE_ID=adonis, trapTFTP=trapTFTP)
