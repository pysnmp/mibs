...
Bits, Integer32, ObjectIdentity, Unsigned32, MibIdentifier, TimeTicks, IpAddress, Counter32, iso, enterprises, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, NotificationType, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Integer32", "ObjectIdentity", "Unsigned32", "MibIdentifier", "TimeTicks", "IpAddress", "Counter32", "iso", "enterprises", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "NotificationType", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haReplicationBinding=haReplicationBinding, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTPDaemon=trapTFTPDaemon, trapDHCPDaemon=trapDHCPDaemon, tftp=tftp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, ha=ha, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapHA=trapHA, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsFailure=dnsStatsFailure, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpStats=dhcpStats, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDHCP=trapDHCP, haService=haService, dhcp=dhcp, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemon=commandServerDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolTable=dhcpPoolTable, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseTable=dhcpLeaseTable, dns=dns, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, PYSNMP_MODULE_ID=adonis, dhcpConfig=dhcpConfig, dhcpFixedIP=dhcpFixedIP, systemDaemon=systemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, licenseExpiry=licenseExpiry, dnsStatsReferral=dnsStatsReferral, dhcpPoolAlert=dhcpPoolAlert, lcdDaemon=lcdDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemon=dhcpDaemon, tftpDaemonRunning=tftpDaemonRunning, tftpDaemon=tftpDaemon, commandServer=commandServer, trapReplication=trapReplication, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonRunning=dnsDaemonRunning, trapDNSDaemon=trapDNSDaemon, haServiceNodeType=haServiceNodeType, dhcpPoolSize=dhcpPoolSize, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemon=dnsDaemon, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, system=system, dnsStatsNXDomain=dnsStatsNXDomain, trapTFTP=trapTFTP, haServiceRunning=haServiceRunning, trapSystem=trapSystem, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolEndIP=dhcpPoolEndIP, systemState=systemState, adonisTraps=adonisTraps, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseValid=licenseValid, trapCommandServer=trapCommandServer, dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcd=lcd, trapDNS=trapDNS, adonis=adonis, dhcpSubnetEntry=dhcpSubnetEntry, adonisObjects=adonisObjects, trapReplicationFailure=trapReplicationFailure, trapSystemDaemon=trapSystemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dnsStats=dnsStats)
