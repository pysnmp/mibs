...
Bits, Counter64, ObjectIdentity, enterprises, iso, IpAddress, Gauge32, NotificationType, Unsigned32, ModuleIdentity, TimeTicks, Integer32, Counter32, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Counter64", "ObjectIdentity", "enterprises", "iso", "IpAddress", "Gauge32", "NotificationType", "Unsigned32", "ModuleIdentity", "TimeTicks", "Integer32", "Counter32", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsNXRRSet=dnsStatsNXRRSet, lcdDaemon=lcdDaemon, trapDHCP=trapDHCP, dnsStatsReferral=dnsStatsReferral, trapDNSDaemon=trapDNSDaemon, commandServer=commandServer, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolSize=dhcpPoolSize, dhcpPoolStartIP=dhcpPoolStartIP, systemState=systemState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dns=dns, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpStats=dhcpStats, commandServerDaemon=commandServerDaemon, adonis=adonis, dnsStatsRecursion=dnsStatsRecursion, trapCommandServer=trapCommandServer, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, dhcpIP=dhcpIP, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolEntry=dhcpPoolEntry, system=system, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetIP=dhcpSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystem=trapSystem, dhcpFailOverState=dhcpFailOverState, haServiceNodeType=haServiceNodeType, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsFailure=dnsStatsFailure, haServiceRunning=haServiceRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetMask=dhcpSubnetMask, trapDNS=trapDNS, dhcpPoolTable=dhcpPoolTable, dhcpConfig=dhcpConfig, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsNXDomain=dnsStatsNXDomain, adonisTraps=adonisTraps, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftpDaemon=tftpDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, tftp=tftp, dhcpLeaseStartTime=dhcpLeaseStartTime, trapReplication=trapReplication, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcd=lcd, dhcpDaemonRunning=dhcpDaemonRunning, trapSystemDaemon=trapSystemDaemon, dhcpSubnetAlert=dhcpSubnetAlert, trapReplicationFailure=trapReplicationFailure, haService=haService, systemDaemon=systemDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemon=dnsDaemon, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, dhcp=dhcp, licenseValid=licenseValid, dnsStats=dnsStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemon=dhcpDaemon, licenseExpiry=licenseExpiry, haReplicationBinding=haReplicationBinding, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisObjects=adonisObjects, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTP=trapTFTP, dhcpSubnetEntry=dhcpSubnetEntry, ha=ha)
