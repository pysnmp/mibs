...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapSystem=trapSystem, trapDHCP=trapDHCP, trapHA=trapHA, systemDaemon=systemDaemon, dnsDaemonRunning=dnsDaemonRunning, haService=haService, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFailOverState=dhcpFailOverState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTable=dhcpLeaseTable, commandServer=commandServer, lcd=lcd, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetEntry=dhcpSubnetEntry, dns=dns, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, trapReplicationFailure=trapReplicationFailure, dhcpConfig=dhcpConfig, tftpDaemon=tftpDaemon, adonisTraps=adonisTraps, dhcpIP=dhcpIP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapCommandServer=trapCommandServer, licenseValid=licenseValid, dnsStatsFailure=dnsStatsFailure, dhcpPoolSize=dhcpPoolSize, dhcpPoolStartIP=dhcpPoolStartIP, tftp=tftp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, PYSNMP_MODULE_ID=adonis, system=system, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, lcdDaemon=lcdDaemon, systemState=systemState, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTPDaemon=trapTFTPDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsStatsReferral=dnsStatsReferral, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStats=dnsStats, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIP=dhcpFixedIP, trapDNSDaemon=trapDNSDaemon, dhcpFixedIPTable=dhcpFixedIPTable, trapReplication=trapReplication, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolTable=dhcpPoolTable, dnsStatsNXRRSet=dnsStatsNXRRSet, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseBindState=dhcpLeaseBindState, adonis=adonis, dhcp=dhcp, dnsStatsNXDomain=dnsStatsNXDomain, licenseExpiry=licenseExpiry, commandServerDaemon=commandServerDaemon, dhcpStats=dhcpStats, trapCommandServerDaemon=trapCommandServerDaemon, trapDNS=trapDNS, haReplicationBinding=haReplicationBinding, haServiceRunning=haServiceRunning, ha=ha, dhcpSubnetIP=dhcpSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystemDaemon=trapSystemDaemon, dnsDaemon=dnsDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemon=dhcpDaemon, adonisObjects=adonisObjects, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonRunning=dhcpDaemonRunning, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress)
