...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplication=trapReplication, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpConfig=dhcpConfig, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetIP=dhcpSubnetIP, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haServiceNodeType=haServiceNodeType, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseTable=dhcpLeaseTable, systemState=systemState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsFailure=dnsStatsFailure, dnsStatsNXDomain=dnsStatsNXDomain, adonisTraps=adonisTraps, lcd=lcd, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dns=dns, system=system, tftpDaemon=tftpDaemon, licenseValid=licenseValid, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolSize=dhcpPoolSize, dhcpFailOverState=dhcpFailOverState, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCP=trapDHCP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServerDaemon=commandServerDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseBindState=dhcpLeaseBindState, ha=ha, dhcp=dhcp, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsReferral=dnsStatsReferral, dhcpPoolUsed=dhcpPoolUsed, systemDaemon=systemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetSize=dhcpSubnetSize, haServiceRunning=haServiceRunning, trapTFTPDaemon=trapTFTPDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, haService=haService, dnsStats=dnsStats, trapCommandServer=trapCommandServer, dhcpLeaseEntry=dhcpLeaseEntry, trapCommandServerDaemon=trapCommandServerDaemon, trapHA=trapHA, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapTFTP=trapTFTP, dhcpStats=dhcpStats, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemon=dhcpDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEndIP=dhcpPoolEndIP, adonisObjects=adonisObjects, dnsStatsRecursion=dnsStatsRecursion, tftp=tftp, trapDNS=trapDNS, adonis=adonis, lcdDaemon=lcdDaemon, dhcpIP=dhcpIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemon=dnsDaemon, commandServer=commandServer, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseStartTime=dhcpLeaseStartTime, PYSNMP_MODULE_ID=adonis, licenseExpiry=licenseExpiry, dnsStatsSuccess=dnsStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haReplicationBinding=haReplicationBinding, trapDNSDaemon=trapDNSDaemon, trapReplicationFailure=trapReplicationFailure, trapSystem=trapSystem, dhcpFixedIP=dhcpFixedIP)
