...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetAlert=dhcpSubnetAlert, haReplicationBinding=haReplicationBinding, dhcpLeaseEndTime=dhcpLeaseEndTime, tftpDaemonRunning=tftpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dhcp=dhcp, dnsDaemon=dnsDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, dhcpFailOverState=dhcpFailOverState, systemState=systemState, dnsStatsFailure=dnsStatsFailure, adonisTraps=adonisTraps, dhcpSubnetMask=dhcpSubnetMask, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, ha=ha, PYSNMP_MODULE_ID=adonis, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, trapHA=trapHA, dnsStatsRecursion=dnsStatsRecursion, trapReplication=trapReplication, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolAlert=dhcpPoolAlert, trapTFTP=trapTFTP, system=system, dhcpIP=dhcpIP, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXRRSet=dnsStatsNXRRSet, licenseExpiry=licenseExpiry, trapDHCPDaemon=trapDHCPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolUsed=dhcpPoolUsed, lcdDaemon=lcdDaemon, tftpDaemon=tftpDaemon, trapDHCP=trapDHCP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haService=haService, trapCommandServer=trapCommandServer, dhcpFixedIP=dhcpFixedIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemon=dhcpDaemon, haServiceNodeType=haServiceNodeType, dhcpPoolStartIP=dhcpPoolStartIP, dns=dns, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, lcd=lcd, trapSystem=trapSystem, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolEndIP=dhcpPoolEndIP, licenseValid=licenseValid, trapDNSDaemon=trapDNSDaemon, trapDNS=trapDNS, dnsStats=dnsStats, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsReferral=dnsStatsReferral, haServiceRunning=haServiceRunning, adonisObjects=adonisObjects, dhcpLeaseHostname=dhcpLeaseHostname, systemDaemon=systemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServer=commandServer, trapTFTPDaemon=trapTFTPDaemon, dhcpConfig=dhcpConfig, dhcpPoolSize=dhcpPoolSize, commandServerDaemon=commandServerDaemon, dhcpPoolTable=dhcpPoolTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonis=adonis, tftp=tftp, commandServerDaemonRunning=commandServerDaemonRunning)
