...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftp=tftp, dns=dns, trapReplicationFailure=trapReplicationFailure, commandServerDaemon=commandServerDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetUsed=dhcpSubnetUsed, haReplicationBinding=haReplicationBinding, system=system, dhcpLeaseStartTime=dhcpLeaseStartTime, trapHAServiceFailOver=trapHAServiceFailOver, dhcp=dhcp, trapHA=trapHA, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolAlert=dhcpPoolAlert, trapReplication=trapReplication, trapCommandServer=trapCommandServer, tftpDaemon=tftpDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, adonis=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, adonisObjects=adonisObjects, PYSNMP_MODULE_ID=adonis, systemDaemon=systemDaemon, dhcpDaemonRunning=dhcpDaemonRunning, trapDHCP=trapDHCP, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHostname=dhcpLeaseHostname, commandServer=commandServer, licenseExpiry=licenseExpiry, dhcpStats=dhcpStats, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIP=dhcpFixedIP, dnsStatsFailure=dnsStatsFailure, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIPTable=dhcpFixedIPTable, trapDNSDaemon=trapDNSDaemon, dnsStatsRecursion=dnsStatsRecursion, trapCommandServerDaemon=trapCommandServerDaemon, ha=ha, haService=haService, dhcpIP=dhcpIP, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStats=dnsStats, trapSystem=trapSystem, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonisTraps=adonisTraps, lcdDaemon=lcdDaemon, lcd=lcd, dhcpDaemon=dhcpDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonRunning=dnsDaemonRunning, dnsDaemon=dnsDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haServiceRunning=haServiceRunning, trapDNS=trapDNS, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseBindState=dhcpLeaseBindState, trapTFTP=trapTFTP, trapSystemDaemon=trapSystemDaemon, dhcpPoolTable=dhcpPoolTable, systemState=systemState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpConfig=dhcpConfig, dnsStatsReferral=dnsStatsReferral, licenseValid=licenseValid, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSize=dhcpPoolSize, dhcpFailOverState=dhcpFailOverState, dhcpPoolStartIP=dhcpPoolStartIP, haServiceNodeType=haServiceNodeType, trapTFTPDaemon=trapTFTPDaemon, trapDHCPDaemon=trapDHCPDaemon)
