...
NotificationType, iso, TimeTicks, ObjectIdentity, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, Counter64, Unsigned32, Counter32, Bits, Integer32, Gauge32, enterprises, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "iso", "TimeTicks", "ObjectIdentity", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "Counter64", "Unsigned32", "Counter32", "Bits", "Integer32", "Gauge32", "enterprises", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemon=dnsDaemon, commandServer=commandServer, trapDHCPDaemon=trapDHCPDaemon, dhcpConfig=dhcpConfig, trapDNS=trapDNS, dnsStatsFailure=dnsStatsFailure, haReplicationBinding=haReplicationBinding, dns=dns, licenseValid=licenseValid, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStats=dnsStats, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIP=dhcpFixedIP, dnsDaemonRunning=dnsDaemonRunning, dhcpFailOverState=dhcpFailOverState, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, haService=haService, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolUsed=dhcpPoolUsed, ha=ha, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonis=adonis, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetTable=dhcpSubnetTable, trapReplication=trapReplication, tftp=tftp, systemState=systemState, dhcpSubnetIP=dhcpSubnetIP, system=system, trapCommandServer=trapCommandServer, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, dnsStatsRecursion=dnsStatsRecursion, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolTable=dhcpPoolTable, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseEntry=dhcpLeaseEntry, commandServerDaemon=commandServerDaemon, trapTFTP=trapTFTP, dnsStatsReferral=dnsStatsReferral, trapHAServiceFailOver=trapHAServiceFailOver, adonisObjects=adonisObjects, dhcpDaemonRunning=dhcpDaemonRunning, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, tftpDaemon=tftpDaemon, dhcpLeaseHostname=dhcpLeaseHostname, trapDHCP=trapDHCP, dhcpIP=dhcpIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcp=dhcp, trapSystem=trapSystem, trapDNSDaemon=trapDNSDaemon, trapReplicationFailure=trapReplicationFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, licenseExpiry=licenseExpiry, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsNXDomain=dnsStatsNXDomain, PYSNMP_MODULE_ID=adonis, trapSystemDaemon=trapSystemDaemon, dhcpDaemon=dhcpDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseBindState=dhcpLeaseBindState, adonisTraps=adonisTraps, dhcpSubnetSize=dhcpSubnetSize, haServiceNodeType=haServiceNodeType, lcdDaemon=lcdDaemon, haServiceRunning=haServiceRunning, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsSuccess=dnsStatsSuccess, lcd=lcd, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpStats=dhcpStats, dhcpPoolSize=dhcpPoolSize)
