...
Unsigned32, enterprises, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, Gauge32, MibIdentifier, Integer32, Counter64, iso, ModuleIdentity, TimeTicks, IpAddress, Counter32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "Unsigned32", "enterprises", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "Gauge32", "MibIdentifier", "Integer32", "Counter64", "iso", "ModuleIdentity", "TimeTicks", "IpAddress", "Counter32", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsFailure=dnsStatsFailure, haServiceNodeType=haServiceNodeType, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonRunning=dnsDaemonRunning, haService=haService, dhcpFixedIPTable=dhcpFixedIPTable, tftp=tftp, trapDHCPDaemon=trapDHCPDaemon, lcdDaemon=lcdDaemon, dnsStats=dnsStats, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsReferral=dnsStatsReferral, systemState=systemState, dhcpConfig=dhcpConfig, systemDaemon=systemDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetUsed=dhcpSubnetUsed, trapHA=trapHA, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIP=dhcpFixedIP, lcd=lcd, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, ha=ha, dhcpLeaseStartTime=dhcpLeaseStartTime, system=system, trapSystemDaemon=trapSystemDaemon, trapTFTPDaemon=trapTFTPDaemon, tftpDaemon=tftpDaemon, dhcpSubnetSize=dhcpSubnetSize, dnsStatsRecursion=dnsStatsRecursion, haServiceRunning=haServiceRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapReplication=trapReplication, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetMask=dhcpSubnetMask, licenseValid=licenseValid, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTable=dhcpLeaseTable, dhcp=dhcp, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, PYSNMP_MODULE_ID=adonis, haReplicationBinding=haReplicationBinding, commandServerDaemon=commandServerDaemon, adonisTraps=adonisTraps, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetTable=dhcpSubnetTable, adonis=adonis, trapHAServiceFailOver=trapHAServiceFailOver, trapReplicationFailure=trapReplicationFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, commandServer=commandServer, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemon=dnsDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapTFTP=trapTFTP, dhcpIP=dhcpIP, dhcpStats=dhcpStats, dns=dns, trapCommandServer=trapCommandServer, dhcpFailOverState=dhcpFailOverState, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystem=trapSystem, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapCommandServerDaemon=trapCommandServerDaemon, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, trapDHCP=trapDHCP, trapDNS=trapDNS, dhcpPoolSize=dhcpPoolSize, licenseExpiry=licenseExpiry, dhcpDaemon=dhcpDaemon)
