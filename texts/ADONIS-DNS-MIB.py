...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTPDaemon=trapTFTPDaemon, commandServer=commandServer, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSize=dhcpPoolSize, dnsDaemon=dnsDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, trapTFTP=trapTFTP, dhcpFixedIPTable=dhcpFixedIPTable, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetMask=dhcpSubnetMask, licenseExpiry=licenseExpiry, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseValid=licenseValid, haReplicationBinding=haReplicationBinding, adonisObjects=adonisObjects, trapDNSDaemon=trapDNSDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, tftp=tftp, systemState=systemState, adonis=adonis, haServiceRunning=haServiceRunning, trapDHCP=trapDHCP, dhcpPoolStartIP=dhcpPoolStartIP, ha=ha, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, adonisTraps=adonisTraps, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpIP=dhcpIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHA=trapHA, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCPDaemon=trapDHCPDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemon=dhcpDaemon, dnsStatsReferral=dnsStatsReferral, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXDomain=dnsStatsNXDomain, trapReplication=trapReplication, trapSystem=trapSystem, dhcpLeaseEndTime=dhcpLeaseEndTime, trapCommandServer=trapCommandServer, dhcp=dhcp, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, PYSNMP_MODULE_ID=adonis, dhcpConfig=dhcpConfig, lcdDaemon=lcdDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIP=dhcpFixedIP, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplicationFailure=trapReplicationFailure, dnsStatsFailure=dnsStatsFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpStats=dhcpStats, dhcpPoolEndIP=dhcpPoolEndIP, system=system, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFailOverState=dhcpFailOverState, dhcpPoolAlert=dhcpPoolAlert, lcd=lcd, commandServerDaemon=commandServerDaemon, dnsStats=dnsStats, trapDNS=trapDNS, tftpDaemon=tftpDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetUsed=dhcpSubnetUsed, dns=dns, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsSuccess=dnsStatsSuccess, systemDaemon=systemDaemon, haService=haService)
