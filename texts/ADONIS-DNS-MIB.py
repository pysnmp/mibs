...
Bits, Gauge32, Unsigned32, Integer32, iso, enterprises, MibIdentifier, NotificationType, TimeTicks, Counter64, Counter32, ObjectIdentity, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Gauge32", "Unsigned32", "Integer32", "iso", "enterprises", "MibIdentifier", "NotificationType", "TimeTicks", "Counter64", "Counter32", "ObjectIdentity", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDHCPDaemon=trapDHCPDaemon, haServiceRunning=haServiceRunning, trapCommandServer=trapCommandServer, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, systemDaemon=systemDaemon, trapReplicationFailure=trapReplicationFailure, dhcp=dhcp, haServiceNodeType=haServiceNodeType, trapSystem=trapSystem, dnsStatsNXDomain=dnsStatsNXDomain, adonisTraps=adonisTraps, dnsStats=dnsStats, adonisObjects=adonisObjects, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, tftp=tftp, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFailOverState=dhcpFailOverState, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetUsed=dhcpSubnetUsed, PYSNMP_MODULE_ID=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsSuccess=dnsStatsSuccess, trapDNS=trapDNS, dhcpPoolStartIP=dhcpPoolStartIP, trapDNSDaemon=trapDNSDaemon, trapDHCP=trapDHCP, trapSystemDaemon=trapSystemDaemon, ha=ha, dhcpConfig=dhcpConfig, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsRecursion=dnsStatsRecursion, haReplicationBinding=haReplicationBinding, dhcpPoolEntry=dhcpPoolEntry, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haService=haService, commandServerDaemon=commandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, lcdDaemon=lcdDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIPTable=dhcpFixedIPTable, adonis=adonis, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, dnsStatsReferral=dnsStatsReferral, system=system, trapHA=trapHA, dnsDaemon=dnsDaemon, dhcpIP=dhcpIP, dhcpStats=dhcpStats, licenseValid=licenseValid, dnsStatsFailure=dnsStatsFailure, commandServer=commandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolEndIP=dhcpPoolEndIP, tftpDaemon=tftpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolAlert=dhcpPoolAlert, licenseExpiry=licenseExpiry, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, trapTFTPDaemon=trapTFTPDaemon, dns=dns, lcd=lcd, trapReplication=trapReplication, trapTFTP=trapTFTP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTable=dhcpLeaseTable, systemState=systemState, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFixedIP=dhcpFixedIP, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemon=dhcpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert)
