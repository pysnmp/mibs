...
TimeTicks, MibIdentifier, Unsigned32, ObjectIdentity, NotificationType, Counter32, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Gauge32, ModuleIdentity, iso, Bits, Counter64, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "MibIdentifier", "Unsigned32", "ObjectIdentity", "NotificationType", "Counter32", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Gauge32", "ModuleIdentity", "iso", "Bits", "Counter64", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolUsed=dhcpPoolUsed, haServiceRunning=haServiceRunning, trapSystemDaemon=trapSystemDaemon, commandServerDaemon=commandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemon=dnsDaemon, trapDNS=trapDNS, system=system, commandServer=commandServer, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, adonis=adonis, dhcpStats=dhcpStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsNXDomain=dnsStatsNXDomain, systemState=systemState, trapReplication=trapReplication, trapSystem=trapSystem, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsReferral=dnsStatsReferral, lcdDaemon=lcdDaemon, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, adonisTraps=adonisTraps, tftp=tftp, trapCommandServer=trapCommandServer, dhcpIP=dhcpIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseTable=dhcpLeaseTable, trapReplicationFailure=trapReplicationFailure, dhcpFailOverState=dhcpFailOverState, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsFailure=dnsStatsFailure, lcd=lcd, trapDNSDaemon=trapDNSDaemon, haServiceNodeType=haServiceNodeType, dhcpLeaseEntry=dhcpLeaseEntry, haService=haService, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemon=dhcpDaemon, tftpDaemon=tftpDaemon, dns=dns, trapTFTP=trapTFTP, licenseExpiry=licenseExpiry, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEndTime=dhcpLeaseEndTime, ha=ha, haReplicationBinding=haReplicationBinding, dhcpSubnetEntry=dhcpSubnetEntry, trapDHCP=trapDHCP, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEntry=dhcpPoolEntry, dhcpConfig=dhcpConfig, dhcpPoolStartIP=dhcpPoolStartIP, systemDaemon=systemDaemon, dhcpPoolAlert=dhcpPoolAlert, adonisObjects=adonisObjects, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseBindState=dhcpLeaseBindState, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStats=dnsStats, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcp=dhcp, licenseValid=licenseValid, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIP=dhcpFixedIP, dhcpSubnetMask=dhcpSubnetMask, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolSize=dhcpPoolSize)
