...
Counter32, enterprises, ModuleIdentity, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, IpAddress, Counter64, NotificationType, iso, Bits, ObjectIdentity, MibIdentifier, Unsigned32, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "enterprises", "ModuleIdentity", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "IpAddress", "Counter64", "NotificationType", "iso", "Bits", "ObjectIdentity", "MibIdentifier", "Unsigned32", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapHA=trapHA, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseStartTime=dhcpLeaseStartTime, licenseValid=licenseValid, systemDaemon=systemDaemon, tftpDaemonRunning=tftpDaemonRunning, trapReplicationFailure=trapReplicationFailure, dnsDaemonRunning=dnsDaemonRunning, adonisObjects=adonisObjects, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseEntry=dhcpLeaseEntry, ha=ha, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemon=dhcpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIP=dhcpFixedIP, dhcpPoolTable=dhcpPoolTable, commandServerDaemonRunning=commandServerDaemonRunning, haReplicationBinding=haReplicationBinding, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonis=adonis, PYSNMP_MODULE_ID=adonis, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpFailOverState=dhcpFailOverState, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSize=dhcpPoolSize, tftp=tftp, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetMask=dhcpSubnetMask, dnsStatsFailure=dnsStatsFailure, dhcp=dhcp, dnsStatsRecursion=dnsStatsRecursion, trapSystemDaemon=trapSystemDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolAlert=dhcpPoolAlert, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpStats=dhcpStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemon=dnsDaemon, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStats=dnsStats, dhcpConfig=dhcpConfig, tftpDaemon=tftpDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dns=dns, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, system=system, lcd=lcd, trapDNSDaemon=trapDNSDaemon, trapDNS=trapDNS, trapSystem=trapSystem, haServiceRunning=haServiceRunning, adonisTraps=adonisTraps, trapReplication=trapReplication, systemState=systemState, dhcpPoolUsed=dhcpPoolUsed, lcdDaemon=lcdDaemon, dhcpIP=dhcpIP, dhcpSubnetSize=dhcpSubnetSize, trapDHCP=trapDHCP, trapTFTP=trapTFTP, dhcpSubnetEntry=dhcpSubnetEntry, licenseExpiry=licenseExpiry, commandServerDaemon=commandServerDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haService=haService, trapTFTPDaemon=trapTFTPDaemon, trapCommandServer=trapCommandServer, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsReferral=dnsStatsReferral, haServiceNodeType=haServiceNodeType, commandServer=commandServer, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFixedIPTable=dhcpFixedIPTable)
