...
MibIdentifier, Counter64, IpAddress, ModuleIdentity, Gauge32, Integer32, Bits, Unsigned32, enterprises, iso, Counter32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Counter64", "IpAddress", "ModuleIdentity", "Gauge32", "Integer32", "Bits", "Unsigned32", "enterprises", "iso", "Counter32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "ObjectIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolEndIP=dhcpPoolEndIP, trapCommandServer=trapCommandServer, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapHAServiceFailOver=trapHAServiceFailOver, dhcpConfig=dhcpConfig, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, tftp=tftp, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonRunning=dnsDaemonRunning, trapSystemDaemon=trapSystemDaemon, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemon=commandServerDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsReferral=dnsStatsReferral, adonis=adonis, dhcpLeaseHostname=dhcpLeaseHostname, dns=dns, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpStats=dhcpStats, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, commandServer=commandServer, dhcpFixedIPTable=dhcpFixedIPTable, dhcpIP=dhcpIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolTable=dhcpPoolTable, trapReplication=trapReplication, systemState=systemState, system=system, trapDNSDaemon=trapDNSDaemon, dhcpSubnetEntry=dhcpSubnetEntry, licenseExpiry=licenseExpiry, dhcpLeaseEndTime=dhcpLeaseEndTime, trapTFTP=trapTFTP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapSystem=trapSystem, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonisObjects=adonisObjects, lcd=lcd, haService=haService, haServiceRunning=haServiceRunning, dhcpFailOverState=dhcpFailOverState, licenseValid=licenseValid, PYSNMP_MODULE_ID=adonis, dnsStats=dnsStats, trapDNS=trapDNS, dnsDaemon=dnsDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIP=dhcpFixedIP, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetIP=dhcpSubnetIP, lcdDaemon=lcdDaemon, trapDHCPDaemon=trapDHCPDaemon, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, haReplicationBinding=haReplicationBinding, ha=ha, tftpDaemonRunning=tftpDaemonRunning, adonisTraps=adonisTraps, dhcpDaemon=dhcpDaemon, dhcp=dhcp, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonRunning=dhcpDaemonRunning, trapHA=trapHA, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, tftpDaemon=tftpDaemon, haServiceNodeType=haServiceNodeType, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCP=trapDHCP, systemDaemon=systemDaemon, trapReplicationFailure=trapReplicationFailure, dhcpSubnetMask=dhcpSubnetMask)
