...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTP=trapTFTP, trapCommandServerDaemon=trapCommandServerDaemon, adonis=adonis, dnsStatsSuccess=dnsStatsSuccess, dhcpIP=dhcpIP, tftp=tftp, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemon=dhcpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetEntry=dhcpSubnetEntry, lcd=lcd, trapSystemDaemon=trapSystemDaemon, adonisObjects=adonisObjects, trapSystem=trapSystem, trapDNSDaemon=trapDNSDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsFailure=dnsStatsFailure, trapDHCPDaemon=trapDHCPDaemon, dhcpStats=dhcpStats, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEndIP=dhcpPoolEndIP, trapHA=trapHA, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsReferral=dnsStatsReferral, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemon=dnsDaemon, tftpDaemon=tftpDaemon, commandServerDaemon=commandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIP=dhcpFixedIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetTable=dhcpSubnetTable, dnsStatsRecursion=dnsStatsRecursion, dns=dns, trapDHCP=trapDHCP, commandServer=commandServer, systemState=systemState, trapHAServiceFailOver=trapHAServiceFailOver, system=system, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStats=dnsStats, tftpDaemonRunning=tftpDaemonRunning, adonisTraps=adonisTraps, trapDNS=trapDNS, licenseValid=licenseValid, trapReplicationFailure=trapReplicationFailure, haService=haService, dhcpDaemonRunning=dhcpDaemonRunning, haServiceRunning=haServiceRunning, haServiceNodeType=haServiceNodeType, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFailOverState=dhcpFailOverState, dhcpLeaseBindState=dhcpLeaseBindState, trapReplication=trapReplication, licenseExpiry=licenseExpiry, dhcp=dhcp, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServer=trapCommandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, lcdDaemon=lcdDaemon, ha=ha, dhcpSubnetIP=dhcpSubnetIP, PYSNMP_MODULE_ID=adonis, dhcpPoolSize=dhcpPoolSize, dhcpConfig=dhcpConfig, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haReplicationBinding=haReplicationBinding)
