...
enterprises, Counter64, Counter32, ModuleIdentity, NotificationType, Gauge32, Integer32, iso, TimeTicks, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, MibIdentifier, Bits, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Counter64", "Counter32", "ModuleIdentity", "NotificationType", "Gauge32", "Integer32", "iso", "TimeTicks", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "MibIdentifier", "Bits", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapCommandServer=trapCommandServer, commandServerDaemon=commandServerDaemon, ha=ha, trapTFTP=trapTFTP, commandServer=commandServer, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, trapTFTPDaemon=trapTFTPDaemon, dhcpIP=dhcpIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haReplicationBinding=haReplicationBinding, trapReplication=trapReplication, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, trapSystem=trapSystem, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, adonis=adonis, systemState=systemState, dhcpPoolEndIP=dhcpPoolEndIP, trapHA=trapHA, trapDNSDaemon=trapDNSDaemon, adonisTraps=adonisTraps, dhcpPoolTable=dhcpPoolTable, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, systemDaemon=systemDaemon, dhcpFailOverState=dhcpFailOverState, dhcpPoolSize=dhcpPoolSize, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemon=dnsDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseBindState=dhcpLeaseBindState, trapDHCPDaemon=trapDHCPDaemon, tftp=tftp, adonisObjects=adonisObjects, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemon=dhcpDaemon, haService=haService, licenseValid=licenseValid, dhcpSubnetEntry=dhcpSubnetEntry, trapReplicationFailure=trapReplicationFailure, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolStartIP=dhcpPoolStartIP, dnsStats=dnsStats, dhcpConfig=dhcpConfig, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceNodeType=haServiceNodeType, dnsStatsFailure=dnsStatsFailure, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapHAServiceFailOver=trapHAServiceFailOver, PYSNMP_MODULE_ID=adonis, system=system, trapSystemDaemon=trapSystemDaemon, lcd=lcd, tftpDaemon=tftpDaemon, trapDHCP=trapDHCP, dns=dns, haServiceRunning=haServiceRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcp=dhcp, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetUsed=dhcpSubnetUsed, trapDNS=trapDNS, dhcpPoolEntry=dhcpPoolEntry, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpFixedIP=dhcpFixedIP, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseExpiry=licenseExpiry, lcdDaemon=lcdDaemon)
