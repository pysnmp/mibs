...
NotificationType, iso, enterprises, Counter64, TimeTicks, ObjectIdentity, IpAddress, Bits, Counter32, Unsigned32, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, Integer32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "iso", "enterprises", "Counter64", "TimeTicks", "ObjectIdentity", "IpAddress", "Bits", "Counter32", "Unsigned32", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "Integer32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dns=dns, dhcpPoolAlert=dhcpPoolAlert, systemState=systemState, adonis=adonis, commandServer=commandServer, lcdDaemon=lcdDaemon, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceRunning=haServiceRunning, trapDHCPDaemon=trapDHCPDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPEntry=dhcpFixedIPEntry, adonisTraps=adonisTraps, trapTFTPDaemon=trapTFTPDaemon, trapCommandServer=trapCommandServer, tftpDaemon=tftpDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, trapCommandServerDaemon=trapCommandServerDaemon, trapTFTP=trapTFTP, dhcpFixedIP=dhcpFixedIP, dhcpSubnetIP=dhcpSubnetIP, dhcpIP=dhcpIP, licenseValid=licenseValid, trapHA=trapHA, dhcpSubnetSize=dhcpSubnetSize, trapDNS=trapDNS, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceNodeType=haServiceNodeType, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpStats=dhcpStats, adonisObjects=adonisObjects, dhcpSubnetUsed=dhcpSubnetUsed, commandServerDaemonRunning=commandServerDaemonRunning, lcd=lcd, commandServerDaemon=commandServerDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolSize=dhcpPoolSize, dnsStatsRecursion=dnsStatsRecursion, dhcp=dhcp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, ha=ha, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsNXDomain=dnsStatsNXDomain, licenseExpiry=licenseExpiry, dhcpSubnetTable=dhcpSubnetTable, system=system, dnsDaemonRunning=dnsDaemonRunning, dhcpConfig=dhcpConfig, haReplicationBinding=haReplicationBinding, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsReferral=dnsStatsReferral, dnsStats=dnsStats, trapSystemDaemon=trapSystemDaemon, dnsStatsFailure=dnsStatsFailure, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemon=dnsDaemon, dhcpPoolTable=dhcpPoolTable, haService=haService, trapDHCP=trapDHCP, dhcpSubnetMask=dhcpSubnetMask, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, tftp=tftp, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTable=dhcpLeaseTable, trapSystem=trapSystem, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, PYSNMP_MODULE_ID=adonis, dhcpFailOverState=dhcpFailOverState, dhcpSubnetEntry=dhcpSubnetEntry, systemDaemon=systemDaemon, trapDNSDaemon=trapDNSDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplication=trapReplication)
