...
MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, NotificationType, TimeTicks, ModuleIdentity, enterprises, MibIdentifier, Bits, iso, ObjectIdentity, Gauge32, Unsigned32, Integer32, Counter32, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "NotificationType", "TimeTicks", "ModuleIdentity", "enterprises", "MibIdentifier", "Bits", "iso", "ObjectIdentity", "Gauge32", "Unsigned32", "Integer32", "Counter32", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolEndIP=dhcpPoolEndIP, dhcp=dhcp, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemonRunning=dhcpDaemonRunning, trapHA=trapHA, licenseExpiry=licenseExpiry, dhcpConfig=dhcpConfig, dhcpSubnetIP=dhcpSubnetIP, haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, tftp=tftp, tftpDaemonRunning=tftpDaemonRunning, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemState=systemState, trapTFTP=trapTFTP, dhcpLeaseEntry=dhcpLeaseEntry, trapReplication=trapReplication, dnsDaemon=dnsDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTPDaemon=trapTFTPDaemon, haServiceRunning=haServiceRunning, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolStartIP=dhcpPoolStartIP, haServiceNodeType=haServiceNodeType, dhcpLeaseTable=dhcpLeaseTable, trapHAServiceFailOver=trapHAServiceFailOver, trapReplicationFailure=trapReplicationFailure, commandServerDaemon=commandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, trapCommandServer=trapCommandServer, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, systemDaemon=systemDaemon, trapDHCPDaemon=trapDHCPDaemon, adonisTraps=adonisTraps, tftpDaemon=tftpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpStats=dhcpStats, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolTable=dhcpPoolTable, trapDHCP=trapDHCP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemonRunning=commandServerDaemonRunning, dnsStats=dnsStats, dhcpPoolEntry=dhcpPoolEntry, dnsStatsReferral=dnsStatsReferral, commandServer=commandServer, dhcpPoolSize=dhcpPoolSize, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseHostname=dhcpLeaseHostname, lcdDaemon=lcdDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsStatsFailure=dnsStatsFailure, dhcpIP=dhcpIP, licenseValid=licenseValid, ha=ha, trapSystemDaemon=trapSystemDaemon, trapDNSDaemon=trapDNSDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFixedIPTable=dhcpFixedIPTable, trapSystem=trapSystem, dhcpFailOverState=dhcpFailOverState, dhcpDaemon=dhcpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, system=system, adonis=adonis, trapDNS=trapDNS, dns=dns, dhcpSubnetAlert=dhcpSubnetAlert, lcd=lcd, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisObjects=adonisObjects, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, PYSNMP_MODULE_ID=adonis)
