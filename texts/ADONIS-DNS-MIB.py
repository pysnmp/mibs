...
Gauge32, Counter64, Integer32, iso, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, ObjectIdentity, Unsigned32, MibIdentifier, IpAddress, enterprises, Bits, ModuleIdentity, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Counter64", "Integer32", "iso", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "ObjectIdentity", "Unsigned32", "MibIdentifier", "IpAddress", "enterprises", "Bits", "ModuleIdentity", "Counter32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseBindState=dhcpLeaseBindState, trapReplication=trapReplication, dns=dns, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemon=dhcpDaemon, dnsStats=dnsStats, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCP=trapDHCP, adonisObjects=adonisObjects, trapCommandServer=trapCommandServer, dnsStatsSuccess=dnsStatsSuccess, haServiceRunning=haServiceRunning, dnsDaemonRunning=dnsDaemonRunning, trapTFTP=trapTFTP, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolAlert=dhcpPoolAlert, dhcpConfig=dhcpConfig, tftpDaemonRunning=tftpDaemonRunning, lcdDaemon=lcdDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPTable=dhcpFixedIPTable, systemState=systemState, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, lcd=lcd, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsReferral=dnsStatsReferral, trapSystem=trapSystem, trapReplicationFailure=trapReplicationFailure, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcp=dhcp, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetEntry=dhcpSubnetEntry, licenseValid=licenseValid, dnsDaemon=dnsDaemon, haServiceNodeType=haServiceNodeType, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsFailure=dnsStatsFailure, dhcpFixedIP=dhcpFixedIP, haReplicationBinding=haReplicationBinding, licenseExpiry=licenseExpiry, trapDNS=trapDNS, trapHA=trapHA, dhcpIP=dhcpIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolSize=dhcpPoolSize, dhcpSubnetMask=dhcpSubnetMask, ha=ha, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftp=tftp, commandServerDaemon=commandServerDaemon, dhcpFailOverState=dhcpFailOverState, adonisTraps=adonisTraps, tftpDaemon=tftpDaemon, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, commandServer=commandServer, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, system=system, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, adonis=adonis, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNSDaemon=trapDNSDaemon, trapSystemDaemon=trapSystemDaemon, haService=haService, dhcpStats=dhcpStats, dhcpLeaseEntry=dhcpLeaseEntry)
