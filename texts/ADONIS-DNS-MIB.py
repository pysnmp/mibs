...
ObjectIdentity, IpAddress, Counter64, ModuleIdentity, NotificationType, Bits, MibIdentifier, Integer32, Gauge32, Counter32, TimeTicks, enterprises, iso, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "IpAddress", "Counter64", "ModuleIdentity", "NotificationType", "Bits", "MibIdentifier", "Integer32", "Gauge32", "Counter32", "TimeTicks", "enterprises", "iso", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemon=tftpDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFailOverState=dhcpFailOverState, dhcpSubnetTable=dhcpSubnetTable, trapDNSDaemon=trapDNSDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceRunning=haServiceRunning, trapReplication=trapReplication, dhcpDaemon=dhcpDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIP=dhcpFixedIP, dnsStatsNXDomain=dnsStatsNXDomain, commandServer=commandServer, licenseValid=licenseValid, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, systemState=systemState, dhcpPoolSize=dhcpPoolSize, systemDaemon=systemDaemon, system=system, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTP=trapTFTP, adonisObjects=adonisObjects, adonis=adonis, dnsStatsFailure=dnsStatsFailure, dhcpSubnetIP=dhcpSubnetIP, licenseExpiry=licenseExpiry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseHostname=dhcpLeaseHostname, haService=haService, dhcpFixedIPEntry=dhcpFixedIPEntry, PYSNMP_MODULE_ID=adonis, dnsDaemon=dnsDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapHAServiceFailOver=trapHAServiceFailOver, dhcpStats=dhcpStats, dhcpSubnetMask=dhcpSubnetMask, commandServerDaemon=commandServerDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftpDaemonRunning=tftpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolEntry=dhcpPoolEntry, trapDHCPDaemon=trapDHCPDaemon, dnsStats=dnsStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsReferral=dnsStatsReferral, dhcpDaemonRunning=dhcpDaemonRunning, tftp=tftp, trapReplicationFailure=trapReplicationFailure, dhcpPoolUsed=dhcpPoolUsed, dhcpConfig=dhcpConfig, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, lcdDaemon=lcdDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dns=dns, trapSystem=trapSystem, trapDNS=trapDNS, ha=ha, adonisTraps=adonisTraps, trapHA=trapHA, dhcp=dhcp, dnsStatsNXRRSet=dnsStatsNXRRSet, trapSystemDaemon=trapSystemDaemon, lcd=lcd, trapDHCP=trapDHCP, dhcpIP=dhcpIP, haServiceNodeType=haServiceNodeType, dhcpPoolTable=dhcpPoolTable, haReplicationBinding=haReplicationBinding, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServer=trapCommandServer, dhcpLeaseBindState=dhcpLeaseBindState)
