...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", licenseExpiry=licenseExpiry, dhcpFixedIPTable=dhcpFixedIPTable, systemState=systemState, haServiceRunning=haServiceRunning, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseBindState=dhcpLeaseBindState, trapReplicationFailure=trapReplicationFailure, trapCommandServer=trapCommandServer, systemDaemon=systemDaemon, dhcpPoolTable=dhcpPoolTable, haService=haService, trapReplication=trapReplication, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonRunning=dhcpDaemonRunning, adonisTraps=adonisTraps, dhcpPoolEntry=dhcpPoolEntry, dhcpStats=dhcpStats, tftpDaemon=tftpDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolSize=dhcpPoolSize, dhcp=dhcp, dnsStatsReferral=dnsStatsReferral, trapTFTPDaemon=trapTFTPDaemon, dhcpIP=dhcpIP, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, licenseValid=licenseValid, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetEntry=dhcpSubnetEntry, system=system, dhcpLeaseStartTime=dhcpLeaseStartTime, adonis=adonis, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetMask=dhcpSubnetMask, dns=dns, trapTFTP=trapTFTP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpConfig=dhcpConfig, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsNXDomain=dnsStatsNXDomain, trapDHCPDaemon=trapDHCPDaemon, commandServer=commandServer, lcd=lcd, trapDNS=trapDNS, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemon=dnsDaemon, tftp=tftp, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, dhcpLeaseEndTime=dhcpLeaseEndTime, haReplicationBinding=haReplicationBinding, haServiceNodeType=haServiceNodeType, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, PYSNMP_MODULE_ID=adonis, dnsStats=dnsStats, dnsStatsSuccess=dnsStatsSuccess, trapDHCP=trapDHCP, dhcpSubnetTable=dhcpSubnetTable, ha=ha, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolStartIP=dhcpPoolStartIP, trapSystemDaemon=trapSystemDaemon, dhcpFailOverState=dhcpFailOverState, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseEntry=dhcpLeaseEntry, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, lcdDaemon=lcdDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsFailure=dnsStatsFailure, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapSystem=trapSystem, dhcpDaemon=dhcpDaemon, dhcpFixedIP=dhcpFixedIP)
