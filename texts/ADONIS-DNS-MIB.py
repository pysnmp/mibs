...
TimeTicks, Counter64, enterprises, iso, Counter32, ObjectIdentity, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Integer32, Bits, MibIdentifier, IpAddress, Gauge32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Counter64", "enterprises", "iso", "Counter32", "ObjectIdentity", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Integer32", "Bits", "MibIdentifier", "IpAddress", "Gauge32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplication=trapReplication, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemon=commandServerDaemon, systemDaemon=systemDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, ha=ha, trapHA=trapHA, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haServiceNodeType=haServiceNodeType, dnsStatsNXDomain=dnsStatsNXDomain, systemState=systemState, trapSystemDaemon=trapSystemDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemon=dnsDaemon, tftpDaemon=tftpDaemon, commandServer=commandServer, dhcpDaemon=dhcpDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dns=dns, licenseExpiry=licenseExpiry, PYSNMP_MODULE_ID=adonis, trapDHCP=trapDHCP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, licenseValid=licenseValid, trapDNSDaemon=trapDNSDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpConfig=dhcpConfig, dhcpSubnetIP=dhcpSubnetIP, lcd=lcd, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, tftpDaemonRunning=tftpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, trapTFTP=trapTFTP, haServiceRunning=haServiceRunning, dnsStats=dnsStats, adonis=adonis, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetSize=dhcpSubnetSize, dnsStatsSuccess=dnsStatsSuccess, adonisObjects=adonisObjects, dhcpLeaseBindState=dhcpLeaseBindState, lcdDaemon=lcdDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpStats=dhcpStats, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, trapReplicationFailure=trapReplicationFailure, dnsStatsFailure=dnsStatsFailure, dhcpSubnetEntry=dhcpSubnetEntry, adonisTraps=adonisTraps, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haService=haService, dhcpSubnetUsed=dhcpSubnetUsed, dhcp=dhcp, dhcpIP=dhcpIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIP=dhcpFixedIP, trapCommandServer=trapCommandServer, commandServerDaemonRunning=commandServerDaemonRunning, trapSystem=trapSystem, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haReplicationBinding=haReplicationBinding, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolTable=dhcpPoolTable, tftp=tftp, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSize=dhcpPoolSize, dhcpPoolAlert=dhcpPoolAlert, dhcpFailOverState=dhcpFailOverState, dnsStatsNXRRSet=dnsStatsNXRRSet, system=system, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonRunning=dhcpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, trapDNS=trapDNS)
