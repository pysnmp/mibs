...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsReferral=dnsStatsReferral, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServer=commandServer, licenseExpiry=licenseExpiry, trapDHCP=trapDHCP, dhcpLeaseStartTime=dhcpLeaseStartTime, ha=ha, licenseValid=licenseValid, system=system, trapReplication=trapReplication, dhcpSubnetSize=dhcpSubnetSize, systemDaemon=systemDaemon, dhcp=dhcp, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemon=dhcpDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haService=haService, dnsDaemon=dnsDaemon, trapDNS=trapDNS, dnsStats=dnsStats, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseTable=dhcpLeaseTable, adonis=adonis, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTP=trapTFTP, dhcpFailOverState=dhcpFailOverState, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemon=commandServerDaemon, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemonRunning=commandServerDaemonRunning, systemState=systemState, tftpDaemonRunning=tftpDaemonRunning, dhcpConfig=dhcpConfig, trapReplicationFailure=trapReplicationFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolTable=dhcpPoolTable, lcd=lcd, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSize=dhcpPoolSize, dhcpSubnetTable=dhcpSubnetTable, trapHA=trapHA, dnsStatsSuccess=dnsStatsSuccess, haServiceNodeType=haServiceNodeType, dns=dns, haReplicationBinding=haReplicationBinding, trapCommandServer=trapCommandServer, haServiceRunning=haServiceRunning, trapDNSDaemon=trapDNSDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, PYSNMP_MODULE_ID=adonis, dhcpFixedIP=dhcpFixedIP, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetIP=dhcpSubnetIP, lcdDaemon=lcdDaemon, tftp=tftp, dhcpPoolAlert=dhcpPoolAlert, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftpDaemon=tftpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetMask=dhcpSubnetMask, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsFailure=dnsStatsFailure, dhcpIP=dhcpIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisObjects=adonisObjects, dhcpStats=dhcpStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonisTraps=adonisTraps, trapSystemDaemon=trapSystemDaemon, trapSystem=trapSystem)
