...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsFailure=dnsStatsFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolAlert=dhcpPoolAlert, dhcpFailOverState=dhcpFailOverState, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIP=dhcpFixedIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCPDaemon=trapDHCPDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpConfig=dhcpConfig, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisTraps=adonisTraps, dns=dns, trapHA=trapHA, dhcpIP=dhcpIP, tftp=tftp, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonRunning=dnsDaemonRunning, dnsStatsReferral=dnsStatsReferral, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonis=adonis, trapDNS=trapDNS, trapReplication=trapReplication, trapTFTP=trapTFTP, trapSystem=trapSystem, licenseValid=licenseValid, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseEntry=dhcpLeaseEntry, trapHAServiceFailOver=trapHAServiceFailOver, ha=ha, haServiceNodeType=haServiceNodeType, PYSNMP_MODULE_ID=adonis, trapDHCP=trapDHCP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemon=dhcpDaemon, dhcpSubnetSize=dhcpSubnetSize, trapCommandServerDaemon=trapCommandServerDaemon, systemDaemon=systemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, system=system, dhcpSubnetIP=dhcpSubnetIP, haServiceRunning=haServiceRunning, dhcp=dhcp, commandServer=commandServer, haReplicationBinding=haReplicationBinding, tftpDaemonRunning=tftpDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetEntry=dhcpSubnetEntry, lcd=lcd, trapCommandServer=trapCommandServer, trapDNSDaemon=trapDNSDaemon, dnsStats=dnsStats, trapSystemDaemon=trapSystemDaemon, haService=haService, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, lcdDaemon=lcdDaemon, tftpDaemon=tftpDaemon, dhcpPoolTable=dhcpPoolTable, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEndIP=dhcpPoolEndIP, systemState=systemState, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIPTable=dhcpFixedIPTable, trapReplicationFailure=trapReplicationFailure, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseTable=dhcpLeaseTable, dnsDaemon=dnsDaemon, adonisObjects=adonisObjects, dhcpPoolUsed=dhcpPoolUsed, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSize=dhcpPoolSize, dhcpStats=dhcpStats, licenseExpiry=licenseExpiry, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonDebugLevel=dnsDaemonDebugLevel)
