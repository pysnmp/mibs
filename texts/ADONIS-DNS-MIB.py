...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", PYSNMP_MODULE_ID=adonis, dhcpFixedIP=dhcpFixedIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCPDaemon=trapDHCPDaemon, tftp=tftp, systemDaemon=systemDaemon, licenseValid=licenseValid, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTP=trapTFTP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpStats=dhcpStats, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpIP=dhcpIP, licenseExpiry=licenseExpiry, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseHostname=dhcpLeaseHostname, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPEntry=dhcpFixedIPEntry, trapCommandServer=trapCommandServer, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolUsed=dhcpPoolUsed, dnsStats=dnsStats, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemon=dnsDaemon, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, tftpDaemonRunning=tftpDaemonRunning, dnsStatsReferral=dnsStatsReferral, trapDNS=trapDNS, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haServiceNodeType=haServiceNodeType, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseTable=dhcpLeaseTable, dnsStatsFailure=dnsStatsFailure, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapSystem=trapSystem, dnsStatsRecursion=dnsStatsRecursion, system=system, trapReplicationFailure=trapReplicationFailure, trapReplication=trapReplication, trapDHCP=trapDHCP, dhcpFailOverState=dhcpFailOverState, tftpDaemon=tftpDaemon, adonisObjects=adonisObjects, ha=ha, lcd=lcd, trapSystemDaemon=trapSystemDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEndIP=dhcpPoolEndIP, haService=haService, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseEntry=dhcpLeaseEntry, dhcp=dhcp, dhcpSubnetSize=dhcpSubnetSize, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetIP=dhcpSubnetIP, dhcpConfig=dhcpConfig, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemon=commandServerDaemon, haReplicationBinding=haReplicationBinding, dhcpPoolSize=dhcpPoolSize, dhcpFixedIPTable=dhcpFixedIPTable, haServiceRunning=haServiceRunning, dnsStatsSuccess=dnsStatsSuccess, commandServer=commandServer, dnsStatsNXDomain=dnsStatsNXDomain, adonis=adonis, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEntry=dhcpPoolEntry, trapHA=trapHA, dhcpPoolTable=dhcpPoolTable, lcdDaemon=lcdDaemon, adonisTraps=adonisTraps, systemState=systemState, trapCommandServerDaemon=trapCommandServerDaemon)
