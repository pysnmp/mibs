...
Bits, TimeTicks, Counter32, Gauge32, Integer32, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, enterprises, ModuleIdentity, MibIdentifier, Counter64, iso, NotificationType, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "TimeTicks", "Counter32", "Gauge32", "Integer32", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "enterprises", "ModuleIdentity", "MibIdentifier", "Counter64", "iso", "NotificationType", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsFailure=dnsStatsFailure, dhcpFixedIP=dhcpFixedIP, tftpDaemon=tftpDaemon, lcd=lcd, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEndIP=dhcpPoolEndIP, trapSystem=trapSystem, commandServerDaemonRunning=commandServerDaemonRunning, trapDHCP=trapDHCP, tftp=tftp, dhcpPoolUsed=dhcpPoolUsed, ha=ha, adonisTraps=adonisTraps, dnsStatsNXRRSet=dnsStatsNXRRSet, lcdDaemon=lcdDaemon, dns=dns, trapDHCPDaemon=trapDHCPDaemon, licenseValid=licenseValid, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcp=dhcp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haServiceRunning=haServiceRunning, trapCommandServer=trapCommandServer, dnsDaemon=dnsDaemon, dhcpFailOverState=dhcpFailOverState, trapDNSDaemon=trapDNSDaemon, dhcpSubnetSize=dhcpSubnetSize, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapHA=trapHA, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStats=dnsStats, PYSNMP_MODULE_ID=adonis, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolAlert=dhcpPoolAlert, commandServerDaemon=commandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, system=system, tftpDaemonRunning=tftpDaemonRunning, dhcpIP=dhcpIP, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIPEntry=dhcpFixedIPEntry, adonis=adonis, dhcpConfig=dhcpConfig, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolTable=dhcpPoolTable, trapDNS=trapDNS, dnsStatsNXDomain=dnsStatsNXDomain, haService=haService, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseEntry=dhcpLeaseEntry, dhcpStats=dhcpStats, trapCommandServerDaemon=trapCommandServerDaemon, adonisObjects=adonisObjects, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServer=commandServer, dhcpPoolSize=dhcpPoolSize, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTable=dhcpLeaseTable, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetIP=dhcpSubnetIP, trapTFTP=trapTFTP, systemState=systemState, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsSuccess=dnsStatsSuccess, systemDaemon=systemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapReplication=trapReplication, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, dnsStatsReferral=dnsStatsReferral, licenseExpiry=licenseExpiry, haReplicationBinding=haReplicationBinding, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemon=dhcpDaemon)
