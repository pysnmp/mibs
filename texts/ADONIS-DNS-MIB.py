...
Gauge32, IpAddress, MibIdentifier, ModuleIdentity, Counter64, Counter32, Integer32, Bits, NotificationType, ObjectIdentity, iso, TimeTicks, enterprises, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "IpAddress", "MibIdentifier", "ModuleIdentity", "Counter64", "Counter32", "Integer32", "Bits", "NotificationType", "ObjectIdentity", "iso", "TimeTicks", "enterprises", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonRunning=dnsDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapHA=trapHA, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIP=dhcpFixedIP, dhcpPoolSize=dhcpPoolSize, commandServerDaemon=commandServerDaemon, adonisTraps=adonisTraps, dhcpPoolTable=dhcpPoolTable, system=system, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpConfig=dhcpConfig, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsReferral=dnsStatsReferral, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemState=systemState, dhcpSubnetAlert=dhcpSubnetAlert, trapDNS=trapDNS, trapTFTP=trapTFTP, trapSystemDaemon=trapSystemDaemon, systemDaemon=systemDaemon, dhcpDaemonRunning=dhcpDaemonRunning, trapDHCP=trapDHCP, dnsDaemon=dnsDaemon, lcdDaemon=lcdDaemon, adonis=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, trapSystem=trapSystem, trapReplication=trapReplication, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpStats=dhcpStats, licenseValid=licenseValid, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseHostname=dhcpLeaseHostname, tftpDaemon=tftpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStats=dnsStats, trapDNSDaemon=trapDNSDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetEntry=dhcpSubnetEntry, trapDHCPDaemon=trapDHCPDaemon, haServiceRunning=haServiceRunning, lcd=lcd, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dnsStatsNXRRSet=dnsStatsNXRRSet, haService=haService, trapCommandServer=trapCommandServer, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemonRunning=tftpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, adonisObjects=adonisObjects, PYSNMP_MODULE_ID=adonis, dhcpPoolUsed=dhcpPoolUsed, tftp=tftp, dhcpSubnetTable=dhcpSubnetTable, licenseExpiry=licenseExpiry, commandServer=commandServer, dhcp=dhcp, trapTFTPDaemon=trapTFTPDaemon, ha=ha, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFailOverState=dhcpFailOverState, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, dns=dns, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpIP=dhcpIP, dhcpDaemon=dhcpDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapReplicationFailure=trapReplicationFailure, haServiceNodeType=haServiceNodeType, dnsStatsFailure=dnsStatsFailure, haReplicationBinding=haReplicationBinding, commandServerDaemonRunning=commandServerDaemonRunning)
