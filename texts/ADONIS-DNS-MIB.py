...
iso, Counter32, Gauge32, ModuleIdentity, NotificationType, MibIdentifier, Counter64, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, enterprises, IpAddress, ObjectIdentity, Unsigned32, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Counter32", "Gauge32", "ModuleIdentity", "NotificationType", "MibIdentifier", "Counter64", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "enterprises", "IpAddress", "ObjectIdentity", "Unsigned32", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTP=trapTFTP, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpStats=dhcpStats, dhcpPoolTable=dhcpPoolTable, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetMask=dhcpSubnetMask, trapReplicationFailure=trapReplicationFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEndTime=dhcpLeaseEndTime, PYSNMP_MODULE_ID=adonis, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haReplicationBinding=haReplicationBinding, commandServerDaemon=commandServerDaemon, dhcpDaemon=dhcpDaemon, systemDaemon=systemDaemon, haServiceNodeType=haServiceNodeType, dnsDaemon=dnsDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServerDaemon=trapCommandServerDaemon, licenseExpiry=licenseExpiry, adonisTraps=adonisTraps, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsFailure=dnsStatsFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, adonis=adonis, dnsStatsReferral=dnsStatsReferral, lcdDaemon=lcdDaemon, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolSize=dhcpPoolSize, dns=dns, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftp=tftp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDNSDaemon=trapDNSDaemon, system=system, dnsStatsSuccess=dnsStatsSuccess, licenseValid=licenseValid, trapCommandServer=trapCommandServer, haServiceRunning=haServiceRunning, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEntry=dhcpPoolEntry, dnsStatsRecursion=dnsStatsRecursion, dhcpConfig=dhcpConfig, dhcpFailOverState=dhcpFailOverState, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCP=trapDHCP, dhcpLeaseStartTime=dhcpLeaseStartTime, trapReplication=trapReplication, dhcpSubnetUsed=dhcpSubnetUsed, dnsStats=dnsStats, systemState=systemState, trapHA=trapHA, lcd=lcd, trapDHCPDaemon=trapDHCPDaemon, tftpDaemon=tftpDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, trapHAServiceFailOver=trapHAServiceFailOver, adonisObjects=adonisObjects, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, trapSystemDaemon=trapSystemDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapSystem=trapSystem, trapDNS=trapDNS, commandServer=commandServer, dhcpIP=dhcpIP, dhcpSubnetEntry=dhcpSubnetEntry, ha=ha, dhcp=dhcp, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haService=haService)
