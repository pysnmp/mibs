...
enterprises, IpAddress, Integer32, MibIdentifier, Unsigned32, iso, NotificationType, Bits, ObjectIdentity, Counter64, Gauge32, TimeTicks, ModuleIdentity, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "IpAddress", "Integer32", "MibIdentifier", "Unsigned32", "iso", "NotificationType", "Bits", "ObjectIdentity", "Counter64", "Gauge32", "TimeTicks", "ModuleIdentity", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", licenseValid=licenseValid, trapTFTP=trapTFTP, dhcpFailOverState=dhcpFailOverState, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseBindState=dhcpLeaseBindState, trapReplicationFailure=trapReplicationFailure, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, licenseExpiry=licenseExpiry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCP=trapDHCP, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsStats=dnsStats, trapReplication=trapReplication, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsFailure=dnsStatsFailure, dns=dns, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPEntry=dhcpFixedIPEntry, PYSNMP_MODULE_ID=adonis, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonRunning=dnsDaemonRunning, trapDNSDaemon=trapDNSDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpIP=dhcpIP, systemDaemon=systemDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolAlert=dhcpPoolAlert, ha=ha, commandServer=commandServer, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, adonisTraps=adonisTraps, dhcpFixedIPTable=dhcpFixedIPTable, tftp=tftp, system=system, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServerDaemon=commandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, systemState=systemState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsReferral=dnsStatsReferral, lcd=lcd, dhcp=dhcp, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetIP=dhcpSubnetIP, dhcpStats=dhcpStats, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolEndIP=dhcpPoolEndIP, haServiceNodeType=haServiceNodeType, haServiceRunning=haServiceRunning, trapSystem=trapSystem, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemon=tftpDaemon, dhcpSubnetMask=dhcpSubnetMask, haReplicationBinding=haReplicationBinding, trapCommandServer=trapCommandServer, trapDNS=trapDNS, dhcpLeaseStartTime=dhcpLeaseStartTime, haService=haService, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemon=dnsDaemon, adonis=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolTable=dhcpPoolTable, trapTFTPDaemon=trapTFTPDaemon, adonisObjects=adonisObjects, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDHCPDaemon=trapDHCPDaemon, dhcpConfig=dhcpConfig, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemon=dhcpDaemon, lcdDaemon=lcdDaemon, dhcpPoolSize=dhcpPoolSize, dhcpFixedIP=dhcpFixedIP)
