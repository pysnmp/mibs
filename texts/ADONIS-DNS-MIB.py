...
Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, TimeTicks, Unsigned32, ObjectIdentity, Counter32, enterprises, ModuleIdentity, MibIdentifier, Integer32, Bits, IpAddress, NotificationType, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "TimeTicks", "Unsigned32", "ObjectIdentity", "Counter32", "enterprises", "ModuleIdentity", "MibIdentifier", "Integer32", "Bits", "IpAddress", "NotificationType", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpStats=dhcpStats, dhcpPoolSize=dhcpPoolSize, commandServerDaemon=commandServerDaemon, haServiceRunning=haServiceRunning, commandServer=commandServer, dns=dns, trapHA=trapHA, systemState=systemState, commandServerDaemonRunning=commandServerDaemonRunning, adonisTraps=adonisTraps, trapCommandServer=trapCommandServer, dhcpConfig=dhcpConfig, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHostname=dhcpLeaseHostname, trapReplication=trapReplication, systemDaemon=systemDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, ha=ha, dhcpPoolUsed=dhcpPoolUsed, tftp=tftp, trapDHCP=trapDHCP, dhcp=dhcp, haServiceNodeType=haServiceNodeType, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNS=trapDNS, system=system, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftpDaemonRunning=tftpDaemonRunning, haService=haService, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, licenseValid=licenseValid, lcd=lcd, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolStartIP=dhcpPoolStartIP, lcdDaemon=lcdDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIP=dhcpFixedIP, trapReplicationFailure=trapReplicationFailure, adonis=adonis, dnsStats=dnsStats, trapTFTPDaemon=trapTFTPDaemon, PYSNMP_MODULE_ID=adonis, dhcpDaemon=dhcpDaemon, dhcpSubnetIP=dhcpSubnetIP, dnsStatsFailure=dnsStatsFailure, trapSystemDaemon=trapSystemDaemon, dnsDaemonRunning=dnsDaemonRunning, tftpDaemon=tftpDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haReplicationBinding=haReplicationBinding, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsReferral=dnsStatsReferral, dhcpPoolTable=dhcpPoolTable, adonisObjects=adonisObjects, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemon=dnsDaemon, trapSystem=trapSystem, dhcpSubnetAlert=dhcpSubnetAlert, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, licenseExpiry=licenseExpiry, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFailOverState=dhcpFailOverState, trapTFTP=trapTFTP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpIP=dhcpIP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsSuccess=dnsStatsSuccess, trapDNSDaemon=trapDNSDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState)
