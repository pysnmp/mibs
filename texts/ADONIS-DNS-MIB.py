...
MibIdentifier, Integer32, iso, NotificationType, Gauge32, enterprises, IpAddress, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, ModuleIdentity, Unsigned32, ObjectIdentity, TimeTicks, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Integer32", "iso", "NotificationType", "Gauge32", "enterprises", "IpAddress", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "ModuleIdentity", "Unsigned32", "ObjectIdentity", "TimeTicks", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetSize=dhcpSubnetSize, licenseValid=licenseValid, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpFixedIP=dhcpFixedIP, haService=haService, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, commandServerDaemon=commandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonRunning=dnsDaemonRunning, dhcpConfig=dhcpConfig, trapDNSDaemon=trapDNSDaemon, trapTFTP=trapTFTP, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetMask=dhcpSubnetMask, trapTFTPDaemon=trapTFTPDaemon, system=system, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCP=trapDHCP, adonisObjects=adonisObjects, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetTable=dhcpSubnetTable, dhcpIP=dhcpIP, dnsDaemon=dnsDaemon, dnsStats=dnsStats, dhcpLeaseTable=dhcpLeaseTable, dnsStatsSuccess=dnsStatsSuccess, trapReplicationFailure=trapReplicationFailure, systemDaemon=systemDaemon, dnsStatsFailure=dnsStatsFailure, lcd=lcd, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, lcdDaemon=lcdDaemon, dhcpLeaseHostname=dhcpLeaseHostname, trapCommandServer=trapCommandServer, trapHA=trapHA, trapReplication=trapReplication, dhcpLeaseBindState=dhcpLeaseBindState, trapDNS=trapDNS, systemState=systemState, tftp=tftp, dhcpPoolUsed=dhcpPoolUsed, dns=dns, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetUsed=dhcpSubnetUsed, licenseExpiry=licenseExpiry, PYSNMP_MODULE_ID=adonis, commandServer=commandServer, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXDomain=dnsStatsNXDomain, haReplicationBinding=haReplicationBinding, adonisTraps=adonisTraps, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceNodeType=haServiceNodeType, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseEntry=dhcpLeaseEntry, trapDHCPDaemon=trapDHCPDaemon, trapSystem=trapSystem, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolEntry=dhcpPoolEntry, dhcpFailOverState=dhcpFailOverState, adonis=adonis, ha=ha, dhcpPoolSize=dhcpPoolSize, dhcpStats=dhcpStats, dhcpDaemon=dhcpDaemon, trapHAServiceFailOver=trapHAServiceFailOver, tftpDaemon=tftpDaemon, dnsStatsReferral=dnsStatsReferral, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolTable=dhcpPoolTable, haServiceRunning=haServiceRunning, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsRecursion=dnsStatsRecursion, dhcp=dhcp)
