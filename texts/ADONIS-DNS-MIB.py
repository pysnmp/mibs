...
ObjectIdentity, MibIdentifier, Gauge32, Bits, TimeTicks, Counter64, Counter32, Integer32, iso, Unsigned32, NotificationType, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "MibIdentifier", "Gauge32", "Bits", "TimeTicks", "Counter64", "Counter32", "Integer32", "iso", "Unsigned32", "NotificationType", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, tftpDaemon=tftpDaemon, trapSystem=trapSystem, adonis=adonis, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetIP=dhcpSubnetIP, dnsStatsNXDomain=dnsStatsNXDomain, dns=dns, dhcpFailOverState=dhcpFailOverState, haService=haService, PYSNMP_MODULE_ID=adonis, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseExpiry=licenseExpiry, systemState=systemState, haServiceRunning=haServiceRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsRecursion=dnsStatsRecursion, system=system, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemon=dhcpDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsStats=dnsStats, licenseValid=licenseValid, haServiceNodeType=haServiceNodeType, dhcpPoolAlert=dhcpPoolAlert, trapReplicationFailure=trapReplicationFailure, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolTable=dhcpPoolTable, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcp=dhcp, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseEntry=dhcpLeaseEntry, dhcpStats=dhcpStats, commandServerDaemon=commandServerDaemon, ha=ha, dhcpLeaseEndTime=dhcpLeaseEndTime, adonisObjects=adonisObjects, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseTable=dhcpLeaseTable, tftpDaemonRunning=tftpDaemonRunning, dhcpConfig=dhcpConfig, dhcpDaemonRunning=dhcpDaemonRunning, trapDHCP=trapDHCP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolSize=dhcpPoolSize, trapCommandServer=trapCommandServer, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTP=trapTFTP, haReplicationBinding=haReplicationBinding, trapHA=trapHA, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsReferral=dnsStatsReferral, trapReplication=trapReplication, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDNSDaemon=trapDNSDaemon, commandServer=commandServer, adonisTraps=adonisTraps, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseBindState=dhcpLeaseBindState, systemDaemon=systemDaemon, dhcpSubnetSize=dhcpSubnetSize, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCPDaemon=trapDHCPDaemon, lcd=lcd, dnsStatsFailure=dnsStatsFailure, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIP=dhcpFixedIP, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystemDaemon=trapSystemDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemon=dnsDaemon, dhcpIP=dhcpIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcdDaemon=lcdDaemon, tftp=tftp)
