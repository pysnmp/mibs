...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNS=trapDNS, haReplicationBinding=haReplicationBinding, dns=dns, adonis=adonis, dhcpSubnetMask=dhcpSubnetMask, dnsStatsFailure=dnsStatsFailure, commandServer=commandServer, trapDNSDaemon=trapDNSDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetEntry=dhcpSubnetEntry, dhcpFailOverState=dhcpFailOverState, dhcpFixedIP=dhcpFixedIP, trapCommandServer=trapCommandServer, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseStartTime=dhcpLeaseStartTime, system=system, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCP=trapDHCP, lcdDaemon=lcdDaemon, trapReplicationFailure=trapReplicationFailure, adonisObjects=adonisObjects, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPTable=dhcpFixedIPTable, systemState=systemState, dnsStatsSuccess=dnsStatsSuccess, dhcpIP=dhcpIP, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemon=commandServerDaemon, haServiceNodeType=haServiceNodeType, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseValid=licenseValid, ha=ha, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonRunning=dnsDaemonRunning, systemDaemon=systemDaemon, adonisTraps=adonisTraps, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemon=dnsDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dnsStatsNXDomain=dnsStatsNXDomain, trapSystemDaemon=trapSystemDaemon, trapReplication=trapReplication, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemon=dhcpDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, PYSNMP_MODULE_ID=adonis, trapHA=trapHA, lcd=lcd, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, dhcpPoolStartIP=dhcpPoolStartIP, dhcp=dhcp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseHostname=dhcpLeaseHostname, tftp=tftp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTable=dhcpLeaseTable, dnsStatsReferral=dnsStatsReferral, dhcpStats=dhcpStats, commandServerDaemonRunning=commandServerDaemonRunning, trapSystem=trapSystem, tftpDaemon=tftpDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolSize=dhcpPoolSize, licenseExpiry=licenseExpiry, trapHAServiceFailOver=trapHAServiceFailOver, haServiceRunning=haServiceRunning, haService=haService, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, trapTFTP=trapTFTP, dnsStats=dnsStats, dhcpConfig=dhcpConfig, dhcpLeaseEndTime=dhcpLeaseEndTime)
