...
NotificationType, Integer32, ObjectIdentity, Counter32, enterprises, Counter64, IpAddress, iso, Gauge32, Bits, Unsigned32, ModuleIdentity, TimeTicks, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "Integer32", "ObjectIdentity", "Counter32", "enterprises", "Counter64", "IpAddress", "iso", "Gauge32", "Bits", "Unsigned32", "ModuleIdentity", "TimeTicks", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDHCPDaemon=trapDHCPDaemon, dnsStatsFailure=dnsStatsFailure, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, commandServer=commandServer, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetMask=dhcpSubnetMask, haReplicationBinding=haReplicationBinding, dhcpStats=dhcpStats, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemon=commandServerDaemon, lcd=lcd, trapCommandServer=trapCommandServer, ha=ha, trapDNS=trapDNS, systemState=systemState, dhcpLeaseEndTime=dhcpLeaseEndTime, lcdDaemon=lcdDaemon, dnsStats=dnsStats, adonis=adonis, system=system, haServiceNodeType=haServiceNodeType, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolUsed=dhcpPoolUsed, haService=haService, dhcpSubnetAlert=dhcpSubnetAlert, trapReplication=trapReplication, trapTFTPDaemon=trapTFTPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapReplicationFailure=trapReplicationFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsSuccess=dnsStatsSuccess, tftpDaemonRunning=tftpDaemonRunning, dnsStatsReferral=dnsStatsReferral, tftpDaemon=tftpDaemon, trapSystem=trapSystem, licenseExpiry=licenseExpiry, dhcpSubnetEntry=dhcpSubnetEntry, dhcp=dhcp, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSize=dhcpPoolSize, haServiceRunning=haServiceRunning, trapTFTP=trapTFTP, dhcpFixedIP=dhcpFixedIP, dnsDaemon=dnsDaemon, dns=dns, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolTable=dhcpPoolTable, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsNXDomain=dnsStatsNXDomain, adonisObjects=adonisObjects, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEntry=dhcpPoolEntry, trapHA=trapHA, trapDHCP=trapDHCP, trapDNSDaemon=trapDNSDaemon, tftp=tftp, dhcpDaemon=dhcpDaemon, dhcpIP=dhcpIP, adonisTraps=adonisTraps, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFixedIPEntry=dhcpFixedIPEntry, systemDaemon=systemDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpConfig=dhcpConfig, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, licenseValid=licenseValid, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemonRunning=commandServerDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetSize=dhcpSubnetSize, trapSystemDaemon=trapSystemDaemon, dhcpFailOverState=dhcpFailOverState)
