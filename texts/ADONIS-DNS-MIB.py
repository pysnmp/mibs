...
ModuleIdentity, TimeTicks, Counter64, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, Integer32, NotificationType, Unsigned32, enterprises, MibIdentifier, Bits, IpAddress, iso, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "TimeTicks", "Counter64", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "Integer32", "NotificationType", "Unsigned32", "enterprises", "MibIdentifier", "Bits", "IpAddress", "iso", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", ha=ha, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsSuccess=dnsStatsSuccess, trapSystem=trapSystem, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceNodeType=haServiceNodeType, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetSize=dhcpSubnetSize, dnsStatsReferral=dnsStatsReferral, tftpDaemon=tftpDaemon, dhcpSubnetMask=dhcpSubnetMask, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetTable=dhcpSubnetTable, dhcpFailOverState=dhcpFailOverState, trapReplication=trapReplication, dhcpLeaseEndTime=dhcpLeaseEndTime, trapSystemDaemon=trapSystemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, licenseExpiry=licenseExpiry, dhcpConfig=dhcpConfig, dhcpLeaseBindState=dhcpLeaseBindState, haServiceRunning=haServiceRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haService=haService, adonisObjects=adonisObjects, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDHCP=trapDHCP, dnsDaemon=dnsDaemon, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, commandServerDaemon=commandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolUsed=dhcpPoolUsed, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolSize=dhcpPoolSize, dhcpPoolStartIP=dhcpPoolStartIP, systemDaemon=systemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsFailure=dnsStatsFailure, PYSNMP_MODULE_ID=adonis, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemon=dhcpDaemon, tftp=tftp, dhcpPoolAlert=dhcpPoolAlert, commandServer=commandServer, trapHA=trapHA, dhcpStats=dhcpStats, dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServer=trapCommandServer, dhcpFixedIP=dhcpFixedIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dhcpIP=dhcpIP, dhcpLeaseTable=dhcpLeaseTable, lcd=lcd, trapDNS=trapDNS, systemState=systemState, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetAlert=dhcpSubnetAlert, dnsStats=dnsStats, system=system, dhcp=dhcp, haReplicationBinding=haReplicationBinding, trapDNSDaemon=trapDNSDaemon, lcdDaemon=lcdDaemon, commandServerDaemonRunning=commandServerDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, licenseValid=licenseValid, dns=dns, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapTFTPDaemon=trapTFTPDaemon, adonisTraps=adonisTraps, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetEntry=dhcpSubnetEntry, adonis=adonis, dhcpFixedIPTable=dhcpFixedIPTable)
