...
Counter32, TimeTicks, IpAddress, Gauge32, enterprises, NotificationType, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, Unsigned32, iso, Counter64, MibIdentifier, Integer32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "TimeTicks", "IpAddress", "Gauge32", "enterprises", "NotificationType", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "Unsigned32", "iso", "Counter64", "MibIdentifier", "Integer32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpStats=dhcpStats, dnsStatsRecursion=dnsStatsRecursion, trapDHCPDaemon=trapDHCPDaemon, trapCommandServer=trapCommandServer, dhcpSubnetIP=dhcpSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServer=commandServer, dhcpPoolStartIP=dhcpPoolStartIP, adonisObjects=adonisObjects, licenseValid=licenseValid, tftpDaemonRunning=tftpDaemonRunning, tftp=tftp, haReplicationBinding=haReplicationBinding, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemon=dnsDaemon, dhcpPoolAlert=dhcpPoolAlert, trapHA=trapHA, dhcpFailOverState=dhcpFailOverState, tftpDaemon=tftpDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpIP=dhcpIP, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIP=dhcpFixedIP, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolEndIP=dhcpPoolEndIP, dnsStats=dnsStats, system=system, trapSystemDaemon=trapSystemDaemon, dnsStatsFailure=dnsStatsFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, haServiceRunning=haServiceRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseBindState=dhcpLeaseBindState, trapSystem=trapSystem, adonis=adonis, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, trapDHCP=trapDHCP, lcdDaemon=lcdDaemon, licenseExpiry=licenseExpiry, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolSize=dhcpPoolSize, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseTable=dhcpLeaseTable, dnsStatsReferral=dnsStatsReferral, dhcpLeaseEntry=dhcpLeaseEntry, dhcpConfig=dhcpConfig, trapReplicationFailure=trapReplicationFailure, adonisTraps=adonisTraps, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, lcd=lcd, PYSNMP_MODULE_ID=adonis, ha=ha, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemDaemon=systemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServerDaemon=commandServerDaemon, trapReplication=trapReplication, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolUsed=dhcpPoolUsed, dhcp=dhcp, dns=dns, systemState=systemState, haService=haService, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDNSDaemon=trapDNSDaemon, trapTFTP=trapTFTP, dhcpDaemon=dhcpDaemon, dhcpPoolTable=dhcpPoolTable, trapDNS=trapDNS, dhcpSubnetAlert=dhcpSubnetAlert)
