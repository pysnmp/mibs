...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetIP=dhcpSubnetIP, haServiceRunning=haServiceRunning, dnsStatsReferral=dnsStatsReferral, dhcpPoolEntry=dhcpPoolEntry, licenseValid=licenseValid, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetEntry=dhcpSubnetEntry, dhcpConfig=dhcpConfig, dhcpDaemon=dhcpDaemon, dhcpFailOverState=dhcpFailOverState, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolUsed=dhcpPoolUsed, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpStats=dhcpStats, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftpDaemon=tftpDaemon, systemDaemon=systemDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFixedIP=dhcpFixedIP, trapTFTP=trapTFTP, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolAlert=dhcpPoolAlert, haService=haService, commandServer=commandServer, systemState=systemState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, tftpDaemonRunning=tftpDaemonRunning, ha=ha, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolSize=dhcpPoolSize, trapDHCPDaemon=trapDHCPDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonisTraps=adonisTraps, dhcpLeaseBindState=dhcpLeaseBindState, PYSNMP_MODULE_ID=adonis, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTPDaemon=trapTFTPDaemon, haReplicationBinding=haReplicationBinding, lcd=lcd, commandServerDaemon=commandServerDaemon, trapReplicationFailure=trapReplicationFailure, adonis=adonis, dnsDaemon=dnsDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcp=dhcp, trapDNS=trapDNS, tftp=tftp, dnsDaemonRunning=dnsDaemonRunning, trapReplication=trapReplication, dhcpPoolTable=dhcpPoolTable, dhcpIP=dhcpIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceNodeType=haServiceNodeType, dhcpSubnetTable=dhcpSubnetTable, dnsStats=dnsStats, dnsStatsFailure=dnsStatsFailure, adonisObjects=adonisObjects, dhcpFixedIPTable=dhcpFixedIPTable, trapDHCP=trapDHCP, trapHA=trapHA, dhcpSubnetSize=dhcpSubnetSize, dns=dns, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapCommandServer=trapCommandServer, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, system=system, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolStartIP=dhcpPoolStartIP, lcdDaemon=lcdDaemon, licenseExpiry=licenseExpiry, dhcpSubnetMask=dhcpSubnetMask, trapSystem=trapSystem)
