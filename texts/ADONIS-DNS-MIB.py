...
ModuleIdentity, Counter32, NotificationType, Integer32, IpAddress, Bits, MibIdentifier, Counter64, enterprises, Unsigned32, TimeTicks, iso, Gauge32, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Counter32", "NotificationType", "Integer32", "IpAddress", "Bits", "MibIdentifier", "Counter64", "enterprises", "Unsigned32", "TimeTicks", "iso", "Gauge32", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStats=dnsStats, dhcpSubnetIP=dhcpSubnetIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpConfig=dhcpConfig, dhcpLeaseEntry=dhcpLeaseEntry, trapDNS=trapDNS, dnsStatsFailure=dnsStatsFailure, adonisObjects=adonisObjects, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, lcdDaemon=lcdDaemon, dhcpPoolEntry=dhcpPoolEntry, commandServerDaemon=commandServerDaemon, licenseValid=licenseValid, dns=dns, dnsStatsReferral=dnsStatsReferral, dhcpSubnetUsed=dhcpSubnetUsed, systemDaemon=systemDaemon, systemState=systemState, lcd=lcd, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcp=dhcp, dhcpPoolAlert=dhcpPoolAlert, tftpDaemonRunning=tftpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIP=dhcpFixedIP, adonisTraps=adonisTraps, licenseExpiry=licenseExpiry, dhcpLeaseBindState=dhcpLeaseBindState, system=system, dhcpPoolTable=dhcpPoolTable, dhcpSubnetSize=dhcpSubnetSize, tftp=tftp, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, PYSNMP_MODULE_ID=adonis, trapTFTP=trapTFTP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolEndIP=dhcpPoolEndIP, tftpDaemon=tftpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFailOverState=dhcpFailOverState, dhcpPoolSize=dhcpPoolSize, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, dnsDaemon=dnsDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServerDaemonRunning=commandServerDaemonRunning, trapHA=trapHA, dhcpStats=dhcpStats, dnsStatsRecursion=dnsStatsRecursion, trapReplicationFailure=trapReplicationFailure, haReplicationBinding=haReplicationBinding, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDNSDaemon=trapDNSDaemon, trapSystem=trapSystem, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseTable=dhcpLeaseTable, trapCommandServer=trapCommandServer, trapTFTPDaemon=trapTFTPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, haServiceNodeType=haServiceNodeType, dhcpDaemon=dhcpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, ha=ha, dhcpSubnetMask=dhcpSubnetMask, haServiceRunning=haServiceRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haService=haService, commandServer=commandServer, dhcpDaemonRunning=dhcpDaemonRunning, adonis=adonis, dhcpSubnetTable=dhcpSubnetTable, trapDHCP=trapDHCP, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetAlert=dhcpSubnetAlert, trapReplication=trapReplication, dhcpIP=dhcpIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress)
