...
Gauge32, Unsigned32, ModuleIdentity, TimeTicks, Counter32, iso, NotificationType, ObjectIdentity, Counter64, IpAddress, MibIdentifier, Integer32, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Unsigned32", "ModuleIdentity", "TimeTicks", "Counter32", "iso", "NotificationType", "ObjectIdentity", "Counter64", "IpAddress", "MibIdentifier", "Integer32", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsReferral=dnsStatsReferral, tftp=tftp, dhcpPoolStartIP=dhcpPoolStartIP, lcd=lcd, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapReplicationFailure=trapReplicationFailure, dhcpStats=dhcpStats, dhcpDaemonRunning=dhcpDaemonRunning, trapSystem=trapSystem, trapTFTPDaemon=trapTFTPDaemon, dnsStatsRecursion=dnsStatsRecursion, dns=dns, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpConfig=dhcpConfig, systemState=systemState, dhcpIP=dhcpIP, licenseExpiry=licenseExpiry, haService=haService, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServer=trapCommandServer, trapDHCP=trapDHCP, dhcpSubnetAlert=dhcpSubnetAlert, adonisTraps=adonisTraps, lcdDaemon=lcdDaemon, system=system, trapTFTP=trapTFTP, dhcpFailOverState=dhcpFailOverState, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemon=tftpDaemon, haServiceRunning=haServiceRunning, dhcpSubnetTable=dhcpSubnetTable, dhcp=dhcp, commandServer=commandServer, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetIP=dhcpSubnetIP, trapHA=trapHA, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNSDaemon=trapDNSDaemon, ha=ha, trapSystemDaemon=trapSystemDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEndTime=dhcpLeaseEndTime, adonis=adonis, trapReplication=trapReplication, dhcpDaemon=dhcpDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsFailure=dnsStatsFailure, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemon=dnsDaemon, commandServerDaemon=commandServerDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseHostname=dhcpLeaseHostname, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haReplicationBinding=haReplicationBinding, dhcpLeaseTable=dhcpLeaseTable, adonisObjects=adonisObjects, licenseValid=licenseValid, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseStartTime=dhcpLeaseStartTime, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSize=dhcpPoolSize, dhcpPoolTable=dhcpPoolTable, PYSNMP_MODULE_ID=adonis, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDNS=trapDNS, dnsStats=dnsStats, systemDaemon=systemDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIP=dhcpFixedIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert)
