...
enterprises, iso, Gauge32, Unsigned32, IpAddress, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, Bits, ObjectIdentity, TimeTicks, Counter64, Integer32, Counter32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "iso", "Gauge32", "Unsigned32", "IpAddress", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "Bits", "ObjectIdentity", "TimeTicks", "Counter64", "Integer32", "Counter32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDHCPDaemon=trapDHCPDaemon, trapTFTPDaemon=trapTFTPDaemon, systemDaemon=systemDaemon, dhcpPoolSize=dhcpPoolSize, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDHCP=trapDHCP, trapDNS=trapDNS, dhcpLeaseStartTime=dhcpLeaseStartTime, licenseExpiry=licenseExpiry, dhcpLeaseEntry=dhcpLeaseEntry, trapCommandServerDaemon=trapCommandServerDaemon, ha=ha, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolAlert=dhcpPoolAlert, trapHA=trapHA, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolTable=dhcpPoolTable, dhcpSubnetTable=dhcpSubnetTable, system=system, tftp=tftp, dhcpFixedIPEntry=dhcpFixedIPEntry, adonis=adonis, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseTable=dhcpLeaseTable, trapReplicationFailure=trapReplicationFailure, adonisTraps=adonisTraps, lcd=lcd, dnsStatsNXRRSet=dnsStatsNXRRSet, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemon=dnsDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemon=dhcpDaemon, haReplicationBinding=haReplicationBinding, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonRunning=dhcpDaemonRunning, dhcpStats=dhcpStats, dhcpFixedIP=dhcpFixedIP, lcdDaemon=lcdDaemon, dnsStatsRecursion=dnsStatsRecursion, commandServer=commandServer, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetIP=dhcpSubnetIP, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsReferral=dnsStatsReferral, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEntry=dhcpPoolEntry, trapSystem=trapSystem, adonisObjects=adonisObjects, dnsStats=dnsStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, systemState=systemState, trapReplication=trapReplication, dhcpFailOverState=dhcpFailOverState, trapTFTP=trapTFTP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapCommandServer=trapCommandServer, licenseValid=licenseValid, trapSystemDaemon=trapSystemDaemon, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemon=commandServerDaemon, tftpDaemon=tftpDaemon, dnsStatsFailure=dnsStatsFailure, dns=dns, dhcp=dhcp, haServiceRunning=haServiceRunning, dhcpIP=dhcpIP, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpConfig=dhcpConfig, dhcpLeaseHostname=dhcpLeaseHostname, haService=haService)
