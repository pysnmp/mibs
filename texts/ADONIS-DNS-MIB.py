...
TimeTicks, ObjectIdentity, enterprises, Gauge32, iso, Counter32, IpAddress, Integer32, ModuleIdentity, Bits, MibIdentifier, Unsigned32, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "ObjectIdentity", "enterprises", "Gauge32", "iso", "Counter32", "IpAddress", "Integer32", "ModuleIdentity", "Bits", "MibIdentifier", "Unsigned32", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolSize=dhcpPoolSize, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolEndIP=dhcpPoolEndIP, trapDNSDaemon=trapDNSDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTP=trapTFTP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemon=dhcpDaemon, trapCommandServerDaemon=trapCommandServerDaemon, haServiceRunning=haServiceRunning, dnsStatsNXDomain=dnsStatsNXDomain, trapReplication=trapReplication, dhcpSubnetTable=dhcpSubnetTable, trapHA=trapHA, ha=ha, dhcp=dhcp, dhcpDaemonRunning=dhcpDaemonRunning, dnsStats=dnsStats, haReplicationBinding=haReplicationBinding, dhcpIP=dhcpIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftpDaemonRunning=tftpDaemonRunning, commandServerDaemon=commandServerDaemon, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, trapCommandServer=trapCommandServer, dnsStatsFailure=dnsStatsFailure, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolUsed=dhcpPoolUsed, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetIP=dhcpSubnetIP, trapSystem=trapSystem, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplicationFailure=trapReplicationFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapDNS=trapDNS, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, systemState=systemState, PYSNMP_MODULE_ID=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemon=dnsDaemon, adonis=adonis, dhcpConfig=dhcpConfig, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceNodeType=haServiceNodeType, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseBindState=dhcpLeaseBindState, haService=haService, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetEntry=dhcpSubnetEntry, adonisObjects=adonisObjects, tftp=tftp, dhcpSubnetMask=dhcpSubnetMask, dns=dns, licenseValid=licenseValid, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, tftpDaemon=tftpDaemon, systemDaemon=systemDaemon, adonisTraps=adonisTraps, system=system, dnsStatsRecursion=dnsStatsRecursion, commandServer=commandServer, trapDHCP=trapDHCP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsReferral=dnsStatsReferral, lcdDaemon=lcdDaemon, dhcpPoolTable=dhcpPoolTable, dhcpFixedIP=dhcpFixedIP, trapSystemDaemon=trapSystemDaemon, lcd=lcd, dhcpStats=dhcpStats)
