...
ObjectIdentity, NotificationType, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, iso, Counter32, MibIdentifier, Counter64, Unsigned32, TimeTicks, Bits, enterprises, ModuleIdentity, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "NotificationType", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "iso", "Counter32", "MibIdentifier", "Counter64", "Unsigned32", "TimeTicks", "Bits", "enterprises", "ModuleIdentity", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTPDaemon=trapTFTPDaemon, dnsStatsSuccess=dnsStatsSuccess, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dns=dns, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetMask=dhcpSubnetMask, PYSNMP_MODULE_ID=adonis, licenseExpiry=licenseExpiry, dnsDaemon=dnsDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcp=dhcp, dhcpFailOverState=dhcpFailOverState, dnsStats=dnsStats, trapDHCPDaemon=trapDHCPDaemon, licenseValid=licenseValid, haServiceRunning=haServiceRunning, lcdDaemon=lcdDaemon, haServiceNodeType=haServiceNodeType, dhcpSubnetUsed=dhcpSubnetUsed, haService=haService, systemDaemon=systemDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolEndIP=dhcpPoolEndIP, haReplicationBinding=haReplicationBinding, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, lcd=lcd, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseBindState=dhcpLeaseBindState, commandServer=commandServer, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetSize=dhcpSubnetSize, adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServer=trapCommandServer, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServerDaemon=commandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpConfig=dhcpConfig, dhcpDaemon=dhcpDaemon, dnsStatsFailure=dnsStatsFailure, trapReplication=trapReplication, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsRecursion=dnsStatsRecursion, ha=ha, dnsStatsReferral=dnsStatsReferral, systemState=systemState, dhcpStats=dhcpStats, tftp=tftp, trapSystem=trapSystem, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, adonis=adonis, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIP=dhcpFixedIP, tftpDaemon=tftpDaemon, dhcpPoolTable=dhcpPoolTable, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, trapDHCP=trapDHCP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolSize=dhcpPoolSize, dhcpIP=dhcpIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetEntry=dhcpSubnetEntry, system=system, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTP=trapTFTP, dhcpLeaseHostname=dhcpLeaseHostname, trapDNS=trapDNS, adonisObjects=adonisObjects, trapReplicationFailure=trapReplicationFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess)
