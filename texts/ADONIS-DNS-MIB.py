...
enterprises, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Gauge32, ObjectIdentity, iso, IpAddress, NotificationType, Integer32, Bits, Counter32, Unsigned32, ModuleIdentity, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Gauge32", "ObjectIdentity", "iso", "IpAddress", "NotificationType", "Integer32", "Bits", "Counter32", "Unsigned32", "ModuleIdentity", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapSystem=trapSystem, dhcp=dhcp, dhcpPoolSize=dhcpPoolSize, dnsStatsSuccess=dnsStatsSuccess, system=system, tftp=tftp, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStats=dnsStats, dns=dns, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, ha=ha, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolStartIP=dhcpPoolStartIP, commandServer=commandServer, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, trapDHCPDaemon=trapDHCPDaemon, trapReplicationFailure=trapReplicationFailure, adonisTraps=adonisTraps, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPTable=dhcpFixedIPTable, trapDNS=trapDNS, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseHostname=dhcpLeaseHostname, trapDHCP=trapDHCP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapCommandServerDaemon=trapCommandServerDaemon, lcd=lcd, dhcpPoolEntry=dhcpPoolEntry, tftpDaemon=tftpDaemon, dhcpConfig=dhcpConfig, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEntry=dhcpLeaseEntry, haReplicationBinding=haReplicationBinding, lcdDaemon=lcdDaemon, dhcpFixedIP=dhcpFixedIP, trapReplication=trapReplication, dhcpFailOverState=dhcpFailOverState, licenseValid=licenseValid, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsReferral=dnsStatsReferral, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapHA=trapHA, commandServerDaemon=commandServerDaemon, dhcpStats=dhcpStats, haServiceRunning=haServiceRunning, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, licenseExpiry=licenseExpiry, haServiceNodeType=haServiceNodeType, dnsStatsFailure=dnsStatsFailure, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, adonisObjects=adonisObjects, dhcpIP=dhcpIP, dhcpLeaseEndTime=dhcpLeaseEndTime, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemDaemon=systemDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsDaemon=dnsDaemon, dhcpLeaseBindState=dhcpLeaseBindState, systemState=systemState, dhcpDaemon=dhcpDaemon, adonis=adonis, dhcpPoolTable=dhcpPoolTable, trapCommandServer=trapCommandServer, trapTFTP=trapTFTP, haService=haService, dhcpPoolUsed=dhcpPoolUsed, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIPEntry=dhcpFixedIPEntry, PYSNMP_MODULE_ID=adonis, dhcpSubnetUsed=dhcpSubnetUsed, trapSystemDaemon=trapSystemDaemon)
