...
iso, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, enterprises, IpAddress, Integer32, Gauge32, ModuleIdentity, NotificationType, Counter64, TimeTicks, Counter32, ObjectIdentity, Bits, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "enterprises", "IpAddress", "Integer32", "Gauge32", "ModuleIdentity", "NotificationType", "Counter64", "TimeTicks", "Counter32", "ObjectIdentity", "Bits", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, tftpDaemon=tftpDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEndIP=dhcpPoolEndIP, haReplicationBinding=haReplicationBinding, haService=haService, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcp=dhcp, systemState=systemState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEntry=dhcpPoolEntry, trapHA=trapHA, dhcpPoolSize=dhcpPoolSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, tftpDaemonRunning=tftpDaemonRunning, adonis=adonis, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisObjects=adonisObjects, trapSystemDaemon=trapSystemDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, trapSystem=trapSystem, tftp=tftp, dns=dns, dhcpConfig=dhcpConfig, dnsStatsReferral=dnsStatsReferral, trapCommandServerDaemon=trapCommandServerDaemon, licenseValid=licenseValid, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemon=dhcpDaemon, dhcpLeaseBindState=dhcpLeaseBindState, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetSize=dhcpSubnetSize, trapReplication=trapReplication, dhcpSubnetEntry=dhcpSubnetEntry, adonisTraps=adonisTraps, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsSuccess=dnsStatsSuccess, PYSNMP_MODULE_ID=adonis, commandServerDaemon=commandServerDaemon, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTable=dhcpLeaseTable, haServiceRunning=haServiceRunning, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpIP=dhcpIP, system=system, ha=ha, dhcpStats=dhcpStats, dnsDaemon=dnsDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDNS=trapDNS, lcdDaemon=lcdDaemon, commandServer=commandServer, dhcpFailOverState=dhcpFailOverState, trapDHCPDaemon=trapDHCPDaemon, licenseExpiry=licenseExpiry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, lcd=lcd, haServiceNodeType=haServiceNodeType, trapTFTP=trapTFTP, trapReplicationFailure=trapReplicationFailure, dhcpPoolUsed=dhcpPoolUsed, dnsStatsFailure=dnsStatsFailure, trapCommandServer=trapCommandServer, trapDHCP=trapDHCP, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetIP=dhcpSubnetIP, dnsStats=dnsStats, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonRunning=dhcpDaemonRunning, systemDaemon=systemDaemon, trapDNSDaemon=trapDNSDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIP=dhcpFixedIP)
