...
enterprises, MibIdentifier, NotificationType, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, iso, ObjectIdentity, Counter32, Bits, Integer32, ModuleIdentity, TimeTicks, Gauge32, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "MibIdentifier", "NotificationType", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "iso", "ObjectIdentity", "Counter32", "Bits", "Integer32", "ModuleIdentity", "TimeTicks", "Gauge32", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFixedIP=dhcpFixedIP, trapDHCP=trapDHCP, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dns=dns, commandServerDaemon=commandServerDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonRunning=dnsDaemonRunning, systemDaemon=systemDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsReferral=dnsStatsReferral, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetUsed=dhcpSubnetUsed, haService=haService, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemonRunning=dhcpDaemonRunning, tftp=tftp, commandServer=commandServer, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsRecursion=dnsStatsRecursion, ha=ha, dnsStatsFailure=dnsStatsFailure, dhcpSubnetIP=dhcpSubnetIP, PYSNMP_MODULE_ID=adonis, tftpDaemon=tftpDaemon, trapTFTPDaemon=trapTFTPDaemon, trapSystem=trapSystem, dhcpIP=dhcpIP, dhcpSubnetSize=dhcpSubnetSize, licenseExpiry=licenseExpiry, trapReplication=trapReplication, trapTFTP=trapTFTP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, licenseValid=licenseValid, lcdDaemon=lcdDaemon, dhcpFailOverState=dhcpFailOverState, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPTable=dhcpFixedIPTable, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTable=dhcpLeaseTable, dnsStats=dnsStats, dhcpPoolSize=dhcpPoolSize, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDHCPDaemon=trapDHCPDaemon, lcd=lcd, dhcpPoolEntry=dhcpPoolEntry, systemState=systemState, dhcpDaemon=dhcpDaemon, dnsDaemon=dnsDaemon, haReplicationBinding=haReplicationBinding, trapSystemDaemon=trapSystemDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolStartIP=dhcpPoolStartIP, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolAlert=dhcpPoolAlert, dhcpStats=dhcpStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDNSDaemon=trapDNSDaemon, adonisObjects=adonisObjects, trapHA=trapHA, trapDNS=trapDNS, commandServerDaemonRunning=commandServerDaemonRunning, system=system, dhcpConfig=dhcpConfig, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseHostname=dhcpLeaseHostname, dhcp=dhcp, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haServiceRunning=haServiceRunning, trapReplicationFailure=trapReplicationFailure, trapCommandServer=trapCommandServer, adonis=adonis, dhcpPoolSubnetIP=dhcpPoolSubnetIP)
