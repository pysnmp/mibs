...
ModuleIdentity, Counter64, NotificationType, Bits, ObjectIdentity, Unsigned32, Integer32, iso, IpAddress, TimeTicks, Gauge32, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Counter64", "NotificationType", "Bits", "ObjectIdentity", "Unsigned32", "Integer32", "iso", "IpAddress", "TimeTicks", "Gauge32", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonis=adonis, dhcpSubnetMask=dhcpSubnetMask, trapTFTPDaemon=trapTFTPDaemon, dnsStatsSuccess=dnsStatsSuccess, tftpDaemon=tftpDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonRunning=dhcpDaemonRunning, trapDNS=trapDNS, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetEntry=dhcpSubnetEntry, commandServer=commandServer, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsRecursion=dnsStatsRecursion, trapDHCPDaemon=trapDHCPDaemon, trapSystemDaemon=trapSystemDaemon, dhcpPoolUsed=dhcpPoolUsed, trapDHCP=trapDHCP, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, trapReplication=trapReplication, trapCommandServer=trapCommandServer, system=system, dhcpDaemon=dhcpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemon=dnsDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolTable=dhcpPoolTable, haReplicationBinding=haReplicationBinding, haService=haService, trapCommandServerDaemon=trapCommandServerDaemon, tftp=tftp, tftpDaemonRunning=tftpDaemonRunning, lcdDaemon=lcdDaemon, PYSNMP_MODULE_ID=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, systemDaemon=systemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapTFTP=trapTFTP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, systemState=systemState, dhcpIP=dhcpIP, adonisTraps=adonisTraps, haServiceNodeType=haServiceNodeType, trapSystem=trapSystem, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsReferral=dnsStatsReferral, dhcpConfig=dhcpConfig, adonisObjects=adonisObjects, dhcpLeaseHostname=dhcpLeaseHostname, dhcpStats=dhcpStats, haServiceRunning=haServiceRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcp=dhcp, dhcpSubnetSize=dhcpSubnetSize, licenseValid=licenseValid, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dns=dns, trapDNSDaemon=trapDNSDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, trapHA=trapHA, dnsStats=dnsStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsNXDomain=dnsStatsNXDomain, trapReplicationFailure=trapReplicationFailure, dhcpFixedIP=dhcpFixedIP, ha=ha, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEntry=dhcpLeaseEntry, commandServerDaemon=commandServerDaemon, dhcpPoolEndIP=dhcpPoolEndIP, lcd=lcd, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetIP=dhcpSubnetIP)
