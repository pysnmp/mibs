...
ModuleIdentity, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, MibIdentifier, ObjectIdentity, enterprises, Counter32, Integer32, NotificationType, IpAddress, Counter64, Unsigned32, TimeTicks, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "MibIdentifier", "ObjectIdentity", "enterprises", "Counter32", "Integer32", "NotificationType", "IpAddress", "Counter64", "Unsigned32", "TimeTicks", "Bits")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetEntry=dhcpSubnetEntry, trapDNS=trapDNS, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonDebugLevel=dnsDaemonDebugLevel, PYSNMP_MODULE_ID=adonis, dhcpFixedIPTable=dhcpFixedIPTable, commandServer=commandServer, dhcpPoolEndIP=dhcpPoolEndIP, trapCommandServer=trapCommandServer, dnsStatsRecursion=dnsStatsRecursion, trapSystemDaemon=trapSystemDaemon, lcd=lcd, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapHA=trapHA, adonisObjects=adonisObjects, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetUsed=dhcpSubnetUsed, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsNXDomain=dnsStatsNXDomain, trapReplication=trapReplication, haService=haService, tftp=tftp, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpDaemon=dhcpDaemon, licenseValid=licenseValid, commandServerDaemonRunning=commandServerDaemonRunning, trapReplicationFailure=trapReplicationFailure, dhcpPoolSize=dhcpPoolSize, dhcpLeaseEntry=dhcpLeaseEntry, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDHCPDaemon=trapDHCPDaemon, dhcpStats=dhcpStats, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDNSDaemon=trapDNSDaemon, tftpDaemon=tftpDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpIP=dhcpIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcp=dhcp, dhcpSubnetTable=dhcpSubnetTable, systemDaemon=systemDaemon, systemState=systemState, dhcpFixedIP=dhcpFixedIP, haServiceNodeType=haServiceNodeType, dnsDaemonRunning=dnsDaemonRunning, haReplicationBinding=haReplicationBinding, adonisTraps=adonisTraps, dnsStats=dnsStats, dhcpConfig=dhcpConfig, trapTFTPDaemon=trapTFTPDaemon, trapTFTP=trapTFTP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, ha=ha, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCP=trapDHCP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpFailOverState=dhcpFailOverState, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemon=dnsDaemon, dns=dns, dhcpFixedIPEntry=dhcpFixedIPEntry, system=system, dhcpPoolAlert=dhcpPoolAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetIP=dhcpSubnetIP, dnsStatsReferral=dnsStatsReferral, haServiceRunning=haServiceRunning, lcdDaemon=lcdDaemon, commandServerDaemon=commandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseTable=dhcpLeaseTable, adonis=adonis, trapSystem=trapSystem, dhcpPoolTable=dhcpPoolTable, dhcpSubnetMask=dhcpSubnetMask, tftpDaemonRunning=tftpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsFailure=dnsStatsFailure)
