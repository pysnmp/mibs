...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpConfig=dhcpConfig, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, systemDaemon=systemDaemon, trapReplication=trapReplication, dhcpPoolEntry=dhcpPoolEntry, tftp=tftp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haReplicationBinding=haReplicationBinding, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolSize=dhcpPoolSize, lcdDaemon=lcdDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolUsed=dhcpPoolUsed, dhcpIP=dhcpIP, dhcpPoolTable=dhcpPoolTable, trapCommandServer=trapCommandServer, adonisTraps=adonisTraps, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetAlert=dhcpSubnetAlert, adonis=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, commandServer=commandServer, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemon=dnsDaemon, trapDHCP=trapDHCP, haServiceNodeType=haServiceNodeType, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonRunning=dnsDaemonRunning, haService=haService, dhcpSubnetMask=dhcpSubnetMask, adonisObjects=adonisObjects, trapDHCPDaemon=trapDHCPDaemon, licenseExpiry=licenseExpiry, trapCommandServerDaemon=trapCommandServerDaemon, dhcpStats=dhcpStats, trapDNSDaemon=trapDNSDaemon, dhcpPoolAlert=dhcpPoolAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsSuccess=dnsStatsSuccess, dhcpFailOverState=dhcpFailOverState, dhcpSubnetEntry=dhcpSubnetEntry, dnsStats=dnsStats, dnsStatsNXDomain=dnsStatsNXDomain, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseHostname=dhcpLeaseHostname, dns=dns, commandServerDaemon=commandServerDaemon, trapHA=trapHA, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseTable=dhcpLeaseTable, trapSystem=trapSystem, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIPEntry=dhcpFixedIPEntry, systemState=systemState, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolEndIP=dhcpPoolEndIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpDaemonRunning=dhcpDaemonRunning, ha=ha, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceRunning=haServiceRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsRecursion=dnsStatsRecursion, dhcp=dhcp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemon=dhcpDaemon, dnsStatsFailure=dnsStatsFailure, trapReplicationFailure=trapReplicationFailure, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, licenseValid=licenseValid, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapTFTP=trapTFTP, dhcpFixedIP=dhcpFixedIP, tftpDaemon=tftpDaemon, lcd=lcd, commandServerDaemonRunning=commandServerDaemonRunning, system=system, trapSystemDaemon=trapSystemDaemon, PYSNMP_MODULE_ID=adonis)
