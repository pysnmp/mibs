...
iso, Bits, ModuleIdentity, Counter32, NotificationType, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, Gauge32, Counter64, MibIdentifier, Unsigned32, enterprises, Integer32, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Bits", "ModuleIdentity", "Counter32", "NotificationType", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "Gauge32", "Counter64", "MibIdentifier", "Unsigned32", "enterprises", "Integer32", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonis=adonis, trapDNS=trapDNS, trapSystem=trapSystem, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolEndIP=dhcpPoolEndIP, dhcpStats=dhcpStats, dhcpFailOverState=dhcpFailOverState, dhcpLeaseEndTime=dhcpLeaseEndTime, tftp=tftp, trapCommandServer=trapCommandServer, dhcpFixedIP=dhcpFixedIP, dhcpPoolStartIP=dhcpPoolStartIP, systemState=systemState, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, dhcpPoolEntry=dhcpPoolEntry, tftpDaemon=tftpDaemon, lcd=lcd, trapReplicationFailure=trapReplicationFailure, commandServer=commandServer, dhcpSubnetTable=dhcpSubnetTable, trapSystemDaemon=trapSystemDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, lcdDaemon=lcdDaemon, dhcpPoolUsed=dhcpPoolUsed, haReplicationBinding=haReplicationBinding, dhcpLeaseEntry=dhcpLeaseEntry, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisObjects=adonisObjects, dhcpIP=dhcpIP, ha=ha, trapDHCP=trapDHCP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, PYSNMP_MODULE_ID=adonis, dhcpConfig=dhcpConfig, dhcpSubnetMask=dhcpSubnetMask, dnsStatsRecursion=dnsStatsRecursion, system=system, dnsStatsFailure=dnsStatsFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, dnsStats=dnsStats, dhcpPoolSize=dhcpPoolSize, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemon=commandServerDaemon, licenseExpiry=licenseExpiry, dnsStatsReferral=dnsStatsReferral, tftpDaemonRunning=tftpDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNSDaemon=trapDNSDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapCommandServerDaemon=trapCommandServerDaemon, trapHA=trapHA, systemDaemon=systemDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcp=dhcp, dhcpPoolAlert=dhcpPoolAlert, dns=dns, dhcpLeaseBindState=dhcpLeaseBindState, haServiceNodeType=haServiceNodeType, dhcpLeaseHostname=dhcpLeaseHostname, adonisTraps=adonisTraps, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolTable=dhcpPoolTable, licenseValid=licenseValid, dhcpLeaseTable=dhcpLeaseTable, dnsDaemon=dnsDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceRunning=haServiceRunning, trapReplication=trapReplication, dhcpDaemon=dhcpDaemon, haService=haService)
