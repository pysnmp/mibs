...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDHCPDaemon=trapDHCPDaemon, lcdDaemon=lcdDaemon, trapSystem=trapSystem, dhcpFailOverState=dhcpFailOverState, dnsStatsReferral=dnsStatsReferral, dnsDaemon=dnsDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolAlert=dhcpPoolAlert, trapTFTPDaemon=trapTFTPDaemon, trapReplicationFailure=trapReplicationFailure, haServiceNodeType=haServiceNodeType, trapSystemDaemon=trapSystemDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemon=dhcpDaemon, trapTFTP=trapTFTP, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpIP=dhcpIP, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolTable=dhcpPoolTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpStats=dhcpStats, commandServer=commandServer, dnsStats=dnsStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolUsed=dhcpPoolUsed, licenseExpiry=licenseExpiry, system=system, commandServerDaemon=commandServerDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDNSDaemon=trapDNSDaemon, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetIP=dhcpSubnetIP, dhcpFixedIP=dhcpFixedIP, haService=haService, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceRunning=haServiceRunning, licenseValid=licenseValid, PYSNMP_MODULE_ID=adonis, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, trapDNS=trapDNS, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, lcd=lcd, trapCommandServer=trapCommandServer, dhcpDaemonRunning=dhcpDaemonRunning, systemDaemon=systemDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsRecursion=dnsStatsRecursion, systemState=systemState, ha=ha, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, dhcp=dhcp, dhcpLeaseEndTime=dhcpLeaseEndTime, adonisObjects=adonisObjects, dhcpPoolSize=dhcpPoolSize, adonisTraps=adonisTraps, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, dns=dns, adonis=adonis, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftp=tftp, trapHA=trapHA, dnsStatsFailure=dnsStatsFailure, dnsStatsNXDomain=dnsStatsNXDomain, dhcpConfig=dhcpConfig, dhcpSubnetSize=dhcpSubnetSize, tftpDaemon=tftpDaemon, trapReplication=trapReplication, dhcpLeaseHostname=dhcpLeaseHostname)
