...
Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Bits, IpAddress, NotificationType, Gauge32, ModuleIdentity, TimeTicks, enterprises, Unsigned32, Integer32, ObjectIdentity, Counter64, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Bits", "IpAddress", "NotificationType", "Gauge32", "ModuleIdentity", "TimeTicks", "enterprises", "Unsigned32", "Integer32", "ObjectIdentity", "Counter64", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemonRunning=tftpDaemonRunning, systemDaemon=systemDaemon, commandServer=commandServer, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsNXDomain=dnsStatsNXDomain, trapHA=trapHA, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolTable=dhcpPoolTable, lcdDaemon=lcdDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolSize=dhcpPoolSize, dhcpLeaseStartTime=dhcpLeaseStartTime, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haServiceRunning=haServiceRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCP=trapDHCP, dhcpPoolUsed=dhcpPoolUsed, licenseExpiry=licenseExpiry, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNS=trapDNS, commandServerDaemonRunning=commandServerDaemonRunning, dns=dns, commandServerDaemon=commandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemon=dhcpDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcp=dhcp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, licenseValid=licenseValid, adonisTraps=adonisTraps, dhcpSubnetEntry=dhcpSubnetEntry, tftpDaemon=tftpDaemon, trapSystemDaemon=trapSystemDaemon, ha=ha, dhcpFailOverState=dhcpFailOverState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseTable=dhcpLeaseTable, dnsStatsRecursion=dnsStatsRecursion, dhcpStats=dhcpStats, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, trapCommandServer=trapCommandServer, trapTFTP=trapTFTP, dnsStatsFailure=dnsStatsFailure, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseEntry=dhcpLeaseEntry, adonis=adonis, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, PYSNMP_MODULE_ID=adonis, trapTFTPDaemon=trapTFTPDaemon, haService=haService, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStats=dnsStats, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHostname=dhcpLeaseHostname, adonisObjects=adonisObjects, haServiceNodeType=haServiceNodeType, dhcpIP=dhcpIP, haReplicationBinding=haReplicationBinding, tftp=tftp, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpConfig=dhcpConfig, trapReplication=trapReplication, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, trapReplicationFailure=trapReplicationFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapSystem=trapSystem, trapDNSDaemon=trapDNSDaemon, systemState=systemState, system=system, dnsDaemon=dnsDaemon, dhcpPoolEntry=dhcpPoolEntry, trapHAServiceFailOver=trapHAServiceFailOver, lcd=lcd)
