...
MibIdentifier, Counter64, ObjectIdentity, iso, Bits, Unsigned32, Gauge32, TimeTicks, NotificationType, enterprises, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, Counter32, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Counter64", "ObjectIdentity", "iso", "Bits", "Unsigned32", "Gauge32", "TimeTicks", "NotificationType", "enterprises", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "Counter32", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServerDaemon=commandServerDaemon, trapHA=trapHA, dhcpLeaseTable=dhcpLeaseTable, dhcpConfig=dhcpConfig, trapTFTP=trapTFTP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseEntry=dhcpLeaseEntry, dhcpIP=dhcpIP, tftp=tftp, dhcpDaemon=dhcpDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, adonisObjects=adonisObjects, commandServer=commandServer, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemon=dnsDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, trapSystemDaemon=trapSystemDaemon, haReplicationBinding=haReplicationBinding, dnsStatsFailure=dnsStatsFailure, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemState=systemState, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolSize=dhcpPoolSize, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolTable=dhcpPoolTable, dhcpPoolEndIP=dhcpPoolEndIP, haServiceNodeType=haServiceNodeType, dnsStatsReferral=dnsStatsReferral, dhcpPoolStartIP=dhcpPoolStartIP, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIP=dhcpFixedIP, haServiceRunning=haServiceRunning, trapCommandServerDaemon=trapCommandServerDaemon, trapDNSDaemon=trapDNSDaemon, tftpDaemonRunning=tftpDaemonRunning, dns=dns, system=system, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapReplicationFailure=trapReplicationFailure, lcdDaemon=lcdDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, tftpDaemon=tftpDaemon, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, lcd=lcd, commandServerDaemonRunning=commandServerDaemonRunning, haService=haService, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolUsed=dhcpPoolUsed, dhcpFailOverState=dhcpFailOverState, dhcpPoolEntry=dhcpPoolEntry, adonisTraps=adonisTraps, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetEntry=dhcpSubnetEntry, trapReplication=trapReplication, dhcpSubnetSize=dhcpSubnetSize, licenseValid=licenseValid, licenseExpiry=licenseExpiry, trapDHCPDaemon=trapDHCPDaemon, dnsStatsNXDomain=dnsStatsNXDomain, adonis=adonis, dhcp=dhcp, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, ha=ha, dhcpStats=dhcpStats, trapDHCP=trapDHCP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServer=trapCommandServer, dhcpSubnetTable=dhcpSubnetTable, dnsStats=dnsStats, systemDaemon=systemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, trapSystem=trapSystem, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert)
