...
TimeTicks, ObjectIdentity, iso, MibIdentifier, Unsigned32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Bits, Counter64, Integer32, NotificationType, IpAddress, Gauge32, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "ObjectIdentity", "iso", "MibIdentifier", "Unsigned32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Bits", "Counter64", "Integer32", "NotificationType", "IpAddress", "Gauge32", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemon=dhcpDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, PYSNMP_MODULE_ID=adonis, dhcpPoolTable=dhcpPoolTable, dhcpPoolEntry=dhcpPoolEntry, dhcpConfig=dhcpConfig, lcdDaemon=lcdDaemon, ha=ha, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceNodeType=haServiceNodeType, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dhcpFixedIPTable=dhcpFixedIPTable, dhcp=dhcp, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCPDaemon=trapDHCPDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, trapReplication=trapReplication, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, systemState=systemState, dnsDaemonRunning=dnsDaemonRunning, trapDNS=trapDNS, adonisObjects=adonisObjects, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsFailure=dnsStatsFailure, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetSize=dhcpSubnetSize, lcd=lcd, dnsStats=dnsStats, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemon=dnsDaemon, dnsStatsRecursion=dnsStatsRecursion, haService=haService, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolStartIP=dhcpPoolStartIP, adonisTraps=adonisTraps, haReplicationBinding=haReplicationBinding, dhcpPoolSize=dhcpPoolSize, dnsStatsSuccess=dnsStatsSuccess, dns=dns, dhcpPoolUsed=dhcpPoolUsed, licenseExpiry=licenseExpiry, dnsStatsNXDomain=dnsStatsNXDomain, trapTFTPDaemon=trapTFTPDaemon, trapHA=trapHA, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseEntry=dhcpLeaseEntry, systemDaemon=systemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCP=trapDHCP, commandServerDaemon=commandServerDaemon, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, adonis=adonis, dhcpIP=dhcpIP, tftp=tftp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFailOverState=dhcpFailOverState, dhcpLeaseTable=dhcpLeaseTable, dhcpSubnetUsed=dhcpSubnetUsed, haServiceRunning=haServiceRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, trapReplicationFailure=trapReplicationFailure, system=system, tftpDaemon=tftpDaemon, trapSystem=trapSystem, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIP=dhcpFixedIP, licenseValid=licenseValid, trapDNSDaemon=trapDNSDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServer=trapCommandServer, dhcpDaemonRunning=dhcpDaemonRunning, commandServer=commandServer, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP)
