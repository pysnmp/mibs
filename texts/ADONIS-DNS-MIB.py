...
Integer32, iso, Unsigned32, Counter64, Counter32, IpAddress, NotificationType, ObjectIdentity, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, enterprises, Gauge32, TimeTicks, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "iso", "Unsigned32", "Counter64", "Counter32", "IpAddress", "NotificationType", "ObjectIdentity", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "enterprises", "Gauge32", "TimeTicks", "Bits")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsReferral=dnsStatsReferral, dnsStatsFailure=dnsStatsFailure, trapSystem=trapSystem, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapTFTP=trapTFTP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServerDaemon=commandServerDaemon, adonisTraps=adonisTraps, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFailOverState=dhcpFailOverState, dhcpDaemon=dhcpDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapTFTPDaemon=trapTFTPDaemon, haService=haService, lcdDaemon=lcdDaemon, tftpDaemonRunning=tftpDaemonRunning, trapCommandServer=trapCommandServer, dhcpSubnetMask=dhcpSubnetMask, dnsStats=dnsStats, dhcpPoolSize=dhcpPoolSize, adonisObjects=adonisObjects, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapHAServiceFailOver=trapHAServiceFailOver, trapSystemDaemon=trapSystemDaemon, dhcp=dhcp, trapReplication=trapReplication, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIP=dhcpFixedIP, tftpDaemon=tftpDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolStartIP=dhcpPoolStartIP, adonis=adonis, licenseExpiry=licenseExpiry, dhcpConfig=dhcpConfig, tftp=tftp, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolTable=dhcpPoolTable, dhcpSubnetUsed=dhcpSubnetUsed, trapDNSDaemon=trapDNSDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseEntry=dhcpLeaseEntry, systemDaemon=systemDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemon=dnsDaemon, commandServer=commandServer, PYSNMP_MODULE_ID=adonis, dhcpSubnetTable=dhcpSubnetTable, trapReplicationFailure=trapReplicationFailure, dhcpSubnetIP=dhcpSubnetIP, haReplicationBinding=haReplicationBinding, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceRunning=haServiceRunning, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsRecursion=dnsStatsRecursion, systemState=systemState, trapDHCPDaemon=trapDHCPDaemon, dhcpIP=dhcpIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolAlert=dhcpPoolAlert, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDNS=trapDNS, lcd=lcd, dnsDaemonDebugLevel=dnsDaemonDebugLevel, licenseValid=licenseValid, dhcpSubnetSize=dhcpSubnetSize, dns=dns, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFixedIPTable=dhcpFixedIPTable, trapDHCP=trapDHCP, dhcpStats=dhcpStats, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, ha=ha, system=system, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceNodeType=haServiceNodeType, dhcpLeaseTable=dhcpLeaseTable)
