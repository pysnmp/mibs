...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemon=dnsDaemon, dhcpPoolUsed=dhcpPoolUsed, tftp=tftp, dhcpPoolTable=dhcpPoolTable, dhcpDaemon=dhcpDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpConfig=dhcpConfig, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, trapCommandServer=trapCommandServer, system=system, adonisTraps=adonisTraps, dhcpFixedIPTable=dhcpFixedIPTable, commandServer=commandServer, tftpDaemon=tftpDaemon, dhcpLeaseEntry=dhcpLeaseEntry, haServiceRunning=haServiceRunning, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolAlert=dhcpPoolAlert, trapDNS=trapDNS, dhcpDaemonRunning=dhcpDaemonRunning, systemDaemon=systemDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonRunning=dnsDaemonRunning, trapReplicationFailure=trapReplicationFailure, haReplicationBinding=haReplicationBinding, licenseExpiry=licenseExpiry, dhcpStats=dhcpStats, dhcpFixedIP=dhcpFixedIP, licenseValid=licenseValid, dnsStatsNXRRSet=dnsStatsNXRRSet, haServiceNodeType=haServiceNodeType, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolEntry=dhcpPoolEntry, trapDHCP=trapDHCP, dnsStatsSuccess=dnsStatsSuccess, dhcpIP=dhcpIP, dhcpPoolSize=dhcpPoolSize, adonis=adonis, trapSystem=trapSystem, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetMask=dhcpSubnetMask, dnsStats=dnsStats, dhcpPoolEndIP=dhcpPoolEndIP, trapHA=trapHA, commandServerDaemon=commandServerDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFailOverState=dhcpFailOverState, adonisObjects=adonisObjects, trapReplication=trapReplication, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTP=trapTFTP, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNSDaemon=trapDNSDaemon, systemState=systemState, lcdDaemon=lcdDaemon, dhcp=dhcp, dnsStatsReferral=dnsStatsReferral, trapSystemDaemon=trapSystemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServerDaemon=trapCommandServerDaemon, PYSNMP_MODULE_ID=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, ha=ha, dnsStatsFailure=dnsStatsFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPEntry=dhcpFixedIPEntry, lcd=lcd, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetTable=dhcpSubnetTable, dns=dns)
