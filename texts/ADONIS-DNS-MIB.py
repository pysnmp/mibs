...
NotificationType, ModuleIdentity, Bits, IpAddress, TimeTicks, Integer32, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, ObjectIdentity, MibIdentifier, Counter64, Counter32, Unsigned32, enterprises, iso = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "ModuleIdentity", "Bits", "IpAddress", "TimeTicks", "Integer32", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ObjectIdentity", "MibIdentifier", "Counter64", "Counter32", "Unsigned32", "enterprises", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haServiceRunning=haServiceRunning, dnsStats=dnsStats, dhcpFixedIPTable=dhcpFixedIPTable, trapSystem=trapSystem, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolEndIP=dhcpPoolEndIP, dhcp=dhcp, tftpDaemonRunning=tftpDaemonRunning, dhcpConfig=dhcpConfig, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, trapDHCP=trapDHCP, systemState=systemState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapCommandServer=trapCommandServer, dhcpSubnetIP=dhcpSubnetIP, trapSystemDaemon=trapSystemDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseBindState=dhcpLeaseBindState, system=system, haReplicationBinding=haReplicationBinding, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolTable=dhcpPoolTable, dnsStatsNXRRSet=dnsStatsNXRRSet, trapCommandServerDaemon=trapCommandServerDaemon, commandServer=commandServer, dhcpLeaseHostname=dhcpLeaseHostname, dhcpStats=dhcpStats, dhcpFixedIP=dhcpFixedIP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftpDaemon=tftpDaemon, dhcpDaemon=dhcpDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsReferral=dnsStatsReferral, commandServerDaemon=commandServerDaemon, dnsStatsNXDomain=dnsStatsNXDomain, ha=ha, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseValid=licenseValid, dnsStatsFailure=dnsStatsFailure, tftp=tftp, systemDaemon=systemDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonisObjects=adonisObjects, adonis=adonis, lcdDaemon=lcdDaemon, trapReplicationFailure=trapReplicationFailure, trapTFTPDaemon=trapTFTPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTP=trapTFTP, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetSize=dhcpSubnetSize, PYSNMP_MODULE_ID=adonis, dnsDaemon=dnsDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, trapReplication=trapReplication, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonRunning=dnsDaemonRunning, trapDNS=trapDNS, dhcpSubnetAlert=dhcpSubnetAlert, dhcpFixedIPEntry=dhcpFixedIPEntry, dns=dns, adonisTraps=adonisTraps, licenseExpiry=licenseExpiry, haServiceNodeType=haServiceNodeType, dnsStatsSuccess=dnsStatsSuccess, trapDNSDaemon=trapDNSDaemon, lcd=lcd, trapHA=trapHA, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSize=dhcpPoolSize)
