...
Gauge32, Bits, Counter64, IpAddress, iso, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, ModuleIdentity, NotificationType, ObjectIdentity, Integer32, Unsigned32, Counter32, enterprises, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "Bits", "Counter64", "IpAddress", "iso", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "ModuleIdentity", "NotificationType", "ObjectIdentity", "Integer32", "Unsigned32", "Counter32", "enterprises", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapHA=trapHA, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseBindState=dhcpLeaseBindState, lcdDaemon=lcdDaemon, trapTFTP=trapTFTP, adonisObjects=adonisObjects, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceRunning=haServiceRunning, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonis=adonis, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetEntry=dhcpSubnetEntry, trapDNS=trapDNS, tftpDaemonRunning=tftpDaemonRunning, systemDaemon=systemDaemon, haServiceNodeType=haServiceNodeType, trapCommandServer=trapCommandServer, dhcpSubnetIP=dhcpSubnetIP, system=system, commandServer=commandServer, haService=haService, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetSize=dhcpSubnetSize, trapCommandServerDaemon=trapCommandServerDaemon, lcd=lcd, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsReferral=dnsStatsReferral, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, trapDHCP=trapDHCP, dns=dns, dnsDaemon=dnsDaemon, trapSystem=trapSystem, licenseValid=licenseValid, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolTable=dhcpPoolTable, dhcpIP=dhcpIP, dhcpDaemon=dhcpDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseStartTime=dhcpLeaseStartTime, licenseExpiry=licenseExpiry, adonisTraps=adonisTraps, dnsStatsRecursion=dnsStatsRecursion, systemState=systemState, trapReplication=trapReplication, dhcpStats=dhcpStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haReplicationBinding=haReplicationBinding, trapSystemDaemon=trapSystemDaemon, tftp=tftp, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStats=dnsStats, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpConfig=dhcpConfig, PYSNMP_MODULE_ID=adonis, dnsStatsFailure=dnsStatsFailure, dhcpFailOverState=dhcpFailOverState, tftpDaemon=tftpDaemon, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemon=commandServerDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, ha=ha, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSize=dhcpPoolSize, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplicationFailure=trapReplicationFailure, dhcp=dhcp)
