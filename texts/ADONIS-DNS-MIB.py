...
Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Unsigned32, ModuleIdentity, Gauge32, enterprises, Integer32, Bits, MibIdentifier, Counter64, iso, NotificationType, IpAddress, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Unsigned32", "ModuleIdentity", "Gauge32", "enterprises", "Integer32", "Bits", "MibIdentifier", "Counter64", "iso", "NotificationType", "IpAddress", "ObjectIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolSize=dhcpPoolSize, dhcpPoolAlert=dhcpPoolAlert, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, tftp=tftp, trapSystem=trapSystem, haServiceNodeType=haServiceNodeType, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemon=commandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, tftpDaemonRunning=tftpDaemonRunning, trapReplication=trapReplication, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, PYSNMP_MODULE_ID=adonis, system=system, trapHA=trapHA, dhcpFixedIP=dhcpFixedIP, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpConfig=dhcpConfig, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFailOverState=dhcpFailOverState, trapReplicationFailure=trapReplicationFailure, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemon=dhcpDaemon, ha=ha, dnsStats=dnsStats, dhcpPoolStartIP=dhcpPoolStartIP, lcd=lcd, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haReplicationBinding=haReplicationBinding, trapTFTP=trapTFTP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetIP=dhcpSubnetIP, adonisObjects=adonisObjects, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpStats=dhcpStats, dnsStatsFailure=dnsStatsFailure, dns=dns, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetMask=dhcpSubnetMask, dhcp=dhcp, haServiceRunning=haServiceRunning, dnsDaemonRunning=dnsDaemonRunning, dnsStatsReferral=dnsStatsReferral, trapHAServiceFailOver=trapHAServiceFailOver, trapDNSDaemon=trapDNSDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemon=dnsDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, licenseExpiry=licenseExpiry, commandServer=commandServer, trapSystemDaemon=trapSystemDaemon, dnsStatsSuccess=dnsStatsSuccess, dnsStatsRecursion=dnsStatsRecursion, trapCommandServer=trapCommandServer, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCP=trapDHCP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseTable=dhcpLeaseTable, lcdDaemon=lcdDaemon, dhcpSubnetUsed=dhcpSubnetUsed, systemDaemon=systemDaemon, systemState=systemState, dhcpLeaseHostname=dhcpLeaseHostname, adonis=adonis, licenseValid=licenseValid, trapDNS=trapDNS, adonisTraps=adonisTraps, dnsDaemonDebugLevel=dnsDaemonDebugLevel)
