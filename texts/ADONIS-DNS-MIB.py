...
iso, enterprises, IpAddress, TimeTicks, NotificationType, ObjectIdentity, MibIdentifier, Counter64, Bits, Unsigned32, Integer32, Counter32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "enterprises", "IpAddress", "TimeTicks", "NotificationType", "ObjectIdentity", "MibIdentifier", "Counter64", "Bits", "Unsigned32", "Integer32", "Counter32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplication=trapReplication, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolEntry=dhcpPoolEntry, lcd=lcd, dhcpDaemon=dhcpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDHCP=trapDHCP, dhcpFailOverState=dhcpFailOverState, dhcpPoolEndIP=dhcpPoolEndIP, tftpDaemonRunning=tftpDaemonRunning, trapCommandServer=trapCommandServer, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceRunning=haServiceRunning, trapSystem=trapSystem, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsRecursion=dnsStatsRecursion, licenseExpiry=licenseExpiry, adonisTraps=adonisTraps, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolUsed=dhcpPoolUsed, adonisObjects=adonisObjects, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, tftp=tftp, trapCommandServerDaemon=trapCommandServerDaemon, commandServer=commandServer, dhcpStats=dhcpStats, commandServerDaemon=commandServerDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, PYSNMP_MODULE_ID=adonis, lcdDaemon=lcdDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dhcp=dhcp, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsSuccess=dnsStatsSuccess, trapTFTP=trapTFTP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseValid=licenseValid, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetSize=dhcpSubnetSize, trapHA=trapHA, dnsStats=dnsStats, dhcpLeaseTable=dhcpLeaseTable, dhcpConfig=dhcpConfig, dnsDaemon=dnsDaemon, adonis=adonis, dhcpLeaseHostname=dhcpLeaseHostname, tftpDaemon=tftpDaemon, ha=ha, systemState=systemState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsFailure=dnsStatsFailure, system=system, dhcpSubnetEntry=dhcpSubnetEntry, dns=dns, dhcpIP=dhcpIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolSize=dhcpPoolSize, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapSystemDaemon=trapSystemDaemon, dhcpPoolTable=dhcpPoolTable, dhcpFixedIP=dhcpFixedIP, haService=haService, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dnsStatsReferral=dnsStatsReferral, trapDNS=trapDNS, systemDaemon=systemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetUsed=dhcpSubnetUsed, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haServiceNodeType=haServiceNodeType, dhcpLeaseEndTime=dhcpLeaseEndTime, haReplicationBinding=haReplicationBinding, trapHAServiceFailOver=trapHAServiceFailOver)
