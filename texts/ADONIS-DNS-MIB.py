...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemon=dhcpDaemon, tftp=tftp, lcd=lcd, trapDHCP=trapDHCP, dnsStatsSuccess=dnsStatsSuccess, haServiceRunning=haServiceRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpStats=dhcpStats, haReplicationBinding=haReplicationBinding, system=system, trapHA=trapHA, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseStartTime=dhcpLeaseStartTime, trapReplication=trapReplication, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpIP=dhcpIP, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolUsed=dhcpPoolUsed, dnsStats=dnsStats, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsFailure=dnsStatsFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonRunning=dhcpDaemonRunning, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEntry=dhcpLeaseEntry, systemDaemon=systemDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dns=dns, dhcpSubnetIP=dhcpSubnetIP, dhcp=dhcp, lcdDaemon=lcdDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetSize=dhcpSubnetSize, licenseValid=licenseValid, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolTable=dhcpPoolTable, dnsDaemon=dnsDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapSystem=trapSystem, dhcpFixedIP=dhcpFixedIP, systemState=systemState, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, commandServerDaemon=commandServerDaemon, commandServer=commandServer, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEndIP=dhcpPoolEndIP, PYSNMP_MODULE_ID=adonis, dhcpLeaseHostname=dhcpLeaseHostname, haServiceNodeType=haServiceNodeType, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCPDaemon=trapDHCPDaemon, trapDNS=trapDNS, dhcpPoolSize=dhcpPoolSize, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFailOverState=dhcpFailOverState, adonisTraps=adonisTraps, trapTFTP=trapTFTP, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemon=tftpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServer=trapCommandServer, dnsStatsReferral=dnsStatsReferral, adonisObjects=adonisObjects, haService=haService, ha=ha, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXDomain=dnsStatsNXDomain, dhcpConfig=dhcpConfig, dnsStatsRecursion=dnsStatsRecursion, trapReplicationFailure=trapReplicationFailure, adonis=adonis)
