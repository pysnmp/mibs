...
NotificationType, MibIdentifier, IpAddress, Unsigned32, Gauge32, Integer32, Counter64, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, enterprises, TimeTicks, Counter32, ObjectIdentity, ModuleIdentity, iso = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "MibIdentifier", "IpAddress", "Unsigned32", "Gauge32", "Integer32", "Counter64", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "enterprises", "TimeTicks", "Counter32", "ObjectIdentity", "ModuleIdentity", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpStats=dhcpStats, dhcpSubnetIP=dhcpSubnetIP, trapDNSDaemon=trapDNSDaemon, adonis=adonis, dhcpSubnetUsed=dhcpSubnetUsed, dhcpConfig=dhcpConfig, systemDaemon=systemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, commandServerDaemonRunning=commandServerDaemonRunning, lcd=lcd, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSize=dhcpPoolSize, dns=dns, dhcpSubnetTable=dhcpSubnetTable, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsReferral=dnsStatsReferral, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDHCPDaemon=trapDHCPDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcp=dhcp, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, trapSystem=trapSystem, dhcpDaemon=dhcpDaemon, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEntry=dhcpLeaseEntry, ha=ha, dhcpPoolEntry=dhcpPoolEntry, adonisObjects=adonisObjects, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIP=dhcpFixedIP, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, commandServer=commandServer, trapHAServiceFailOver=trapHAServiceFailOver, dnsStats=dnsStats, dnsDaemonRunning=dnsDaemonRunning, lcdDaemon=lcdDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolUsed=dhcpPoolUsed, haService=haService, dhcpSubnetAlert=dhcpSubnetAlert, adonisTraps=adonisTraps, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftp=tftp, PYSNMP_MODULE_ID=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplication=trapReplication, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, licenseValid=licenseValid, trapHA=trapHA, tftpDaemon=tftpDaemon, trapDHCP=trapDHCP, haServiceNodeType=haServiceNodeType, licenseExpiry=licenseExpiry, systemState=systemState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, trapSystemDaemon=trapSystemDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolAlert=dhcpPoolAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, system=system, trapCommandServerDaemon=trapCommandServerDaemon, commandServerDaemon=commandServerDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolEndIP=dhcpPoolEndIP, dhcpIP=dhcpIP, trapCommandServer=trapCommandServer, trapDNS=trapDNS, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsFailure=dnsStatsFailure, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHostname=dhcpLeaseHostname, haServiceRunning=haServiceRunning, haReplicationBinding=haReplicationBinding, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemon=dnsDaemon, trapTFTP=trapTFTP, dhcpFailOverState=dhcpFailOverState, dhcpPoolSubnetIP=dhcpPoolSubnetIP)
