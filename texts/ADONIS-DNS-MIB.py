...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, tftpDaemon=tftpDaemon, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCP=trapDHCP, trapSystemDaemon=trapSystemDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcp=dhcp, dhcpPoolAlert=dhcpPoolAlert, dns=dns, dhcpSubnetMask=dhcpSubnetMask, tftpDaemonRunning=tftpDaemonRunning, trapDNSDaemon=trapDNSDaemon, haService=haService, lcd=lcd, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTable=dhcpLeaseTable, ha=ha, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFixedIPEntry=dhcpFixedIPEntry, adonisTraps=adonisTraps, trapDNS=trapDNS, trapReplication=trapReplication, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolSize=dhcpPoolSize, dhcpConfig=dhcpConfig, dnsStats=dnsStats, dhcpSubnetSize=dhcpSubnetSize, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftp=tftp, systemDaemon=systemDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, dhcpIP=dhcpIP, adonisObjects=adonisObjects, commandServerDaemon=commandServerDaemon, systemState=systemState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServer=commandServer, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolEndIP=dhcpPoolEndIP, dhcpFailOverState=dhcpFailOverState, dhcpStats=dhcpStats, dnsDaemon=dnsDaemon, PYSNMP_MODULE_ID=adonis, dhcpFixedIP=dhcpFixedIP, trapReplicationFailure=trapReplicationFailure, dnsDaemonRunning=dnsDaemonRunning, adonis=adonis, dhcpDaemon=dhcpDaemon, haServiceRunning=haServiceRunning, dnsStatsFailure=dnsStatsFailure, trapCommandServer=trapCommandServer, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXRRSet=dnsStatsNXRRSet, haServiceNodeType=haServiceNodeType, trapHA=trapHA, licenseExpiry=licenseExpiry, trapTFTPDaemon=trapTFTPDaemon, system=system, dhcpSubnetUsed=dhcpSubnetUsed, licenseValid=licenseValid, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapSystem=trapSystem, haReplicationBinding=haReplicationBinding, trapTFTP=trapTFTP, trapDHCPDaemon=trapDHCPDaemon, lcdDaemon=lcdDaemon)
