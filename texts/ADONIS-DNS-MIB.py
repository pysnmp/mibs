...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetUsed=dhcpSubnetUsed, haReplicationBinding=haReplicationBinding, dhcpConfig=dhcpConfig, dhcpPoolEndIP=dhcpPoolEndIP, trapTFTP=trapTFTP, system=system, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseStartTime=dhcpLeaseStartTime, tftp=tftp, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dns=dns, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapReplication=trapReplication, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcd=lcd, ha=ha, dhcp=dhcp, dhcpLeaseBindState=dhcpLeaseBindState, licenseExpiry=licenseExpiry, dhcpDaemon=dhcpDaemon, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceNodeType=haServiceNodeType, dhcpStats=dhcpStats, commandServerDaemon=commandServerDaemon, commandServer=commandServer, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsFailure=dnsStatsFailure, dhcpPoolEntry=dhcpPoolEntry, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonRunning=dnsDaemonRunning, trapDNSDaemon=trapDNSDaemon, systemState=systemState, haService=haService, adonis=adonis, dhcpSubnetEntry=dhcpSubnetEntry, adonisTraps=adonisTraps, dhcpDaemonRunning=dhcpDaemonRunning, tftpDaemon=tftpDaemon, haServiceRunning=haServiceRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplicationFailure=trapReplicationFailure, dhcpPoolSize=dhcpPoolSize, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStats=dnsStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapHA=trapHA, dhcpFixedIP=dhcpFixedIP, dhcpLeaseTable=dhcpLeaseTable, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemon=dnsDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServer=trapCommandServer, dnsStatsReferral=dnsStatsReferral, systemDaemon=systemDaemon, dhcpFailOverState=dhcpFailOverState, trapDNS=trapDNS, dhcpPoolAlert=dhcpPoolAlert, trapSystemDaemon=trapSystemDaemon, commandServerDaemonRunning=commandServerDaemonRunning, lcdDaemon=lcdDaemon, trapDHCP=trapDHCP, adonisObjects=adonisObjects, dhcpPoolTable=dhcpPoolTable, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetMask=dhcpSubnetMask, trapSystem=trapSystem, dhcpIP=dhcpIP, licenseValid=licenseValid, dnsStatsSuccess=dnsStatsSuccess)
