...
MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Bits, enterprises, Counter64, MibIdentifier, Integer32, IpAddress, Counter32, NotificationType, ObjectIdentity, ModuleIdentity, iso, Gauge32, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Bits", "enterprises", "Counter64", "MibIdentifier", "Integer32", "IpAddress", "Counter32", "NotificationType", "ObjectIdentity", "ModuleIdentity", "iso", "Gauge32", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapSystem=trapSystem, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetAlert=dhcpSubnetAlert, haReplicationBinding=haReplicationBinding, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapCommandServer=trapCommandServer, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServerDaemon=trapCommandServerDaemon, trapSystemDaemon=trapSystemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolTable=dhcpPoolTable, trapDHCP=trapDHCP, dhcpIP=dhcpIP, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsReferral=dnsStatsReferral, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcp=dhcp, trapHA=trapHA, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNSDaemon=trapDNSDaemon, trapTFTP=trapTFTP, lcd=lcd, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonRunning=dhcpDaemonRunning, systemState=systemState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetUsed=dhcpSubnetUsed, haService=haService, dhcpPoolEntry=dhcpPoolEntry, dhcpFailOverState=dhcpFailOverState, dhcpSubnetSize=dhcpSubnetSize, dnsStatsNXDomain=dnsStatsNXDomain, dnsStats=dnsStats, trapReplication=trapReplication, dhcpStats=dhcpStats, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServer=commandServer, ha=ha, dhcpPoolAlert=dhcpPoolAlert, dhcpFixedIP=dhcpFixedIP, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemon=dhcpDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, licenseValid=licenseValid, haServiceRunning=haServiceRunning, dhcpConfig=dhcpConfig, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolSize=dhcpPoolSize, tftpDaemon=tftpDaemon, trapTFTPDaemon=trapTFTPDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, system=system, dhcpSubnetMask=dhcpSubnetMask, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolEndIP=dhcpPoolEndIP, trapReplicationFailure=trapReplicationFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dns=dns, PYSNMP_MODULE_ID=adonis, haServiceNodeType=haServiceNodeType, systemDaemon=systemDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetEntry=dhcpSubnetEntry, licenseExpiry=licenseExpiry, commandServerDaemon=commandServerDaemon, dnsDaemonRunning=dnsDaemonRunning, tftp=tftp, dhcpFixedIPEntry=dhcpFixedIPEntry, adonisTraps=adonisTraps, dnsStatsRecursion=dnsStatsRecursion, trapDNS=trapDNS, dnsStatsFailure=dnsStatsFailure, adonisObjects=adonisObjects, dnsDaemon=dnsDaemon, adonis=adonis)
