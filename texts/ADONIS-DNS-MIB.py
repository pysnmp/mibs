...
enterprises, Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, ModuleIdentity, Counter64, Counter32, IpAddress, ObjectIdentity, Bits, MibIdentifier, Unsigned32, TimeTicks, Integer32, NotificationType = mibBuilder.importSymbols("SNMPv2-SMI", "enterprises", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "ModuleIdentity", "Counter64", "Counter32", "IpAddress", "ObjectIdentity", "Bits", "MibIdentifier", "Unsigned32", "TimeTicks", "Integer32", "NotificationType")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpConfig=dhcpConfig, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseValid=licenseValid, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haService=haService, trapReplicationFailure=trapReplicationFailure, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetMask=dhcpSubnetMask, dnsStatsNXDomain=dnsStatsNXDomain, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dns=dns, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystem=trapSystem, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseEntry=dhcpLeaseEntry, trapTFTP=trapTFTP, tftp=tftp, trapCommandServer=trapCommandServer, dhcpSubnetIP=dhcpSubnetIP, licenseExpiry=licenseExpiry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, systemState=systemState, trapSystemDaemon=trapSystemDaemon, dnsDaemon=dnsDaemon, dnsStatsFailure=dnsStatsFailure, lcdDaemon=lcdDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNS=trapDNS, dhcpSubnetSize=dhcpSubnetSize, dnsStatsRecursion=dnsStatsRecursion, haServiceNodeType=haServiceNodeType, dhcpFailOverState=dhcpFailOverState, system=system, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, PYSNMP_MODULE_ID=adonis, dhcpStats=dhcpStats, dhcp=dhcp, dhcpDaemon=dhcpDaemon, commandServer=commandServer, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetUsed=dhcpSubnetUsed, commandServerDaemon=commandServerDaemon, dnsStats=dnsStats, trapReplication=trapReplication, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsReferral=dnsStatsReferral, dhcpPoolSize=dhcpPoolSize, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonRunning=dnsDaemonRunning, ha=ha, dhcpFixedIP=dhcpFixedIP, commandServerDaemonRunning=commandServerDaemonRunning, adonis=adonis, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, lcd=lcd, trapHAServiceFailOver=trapHAServiceFailOver, adonisObjects=adonisObjects, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haServiceRunning=haServiceRunning, systemDaemon=systemDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpIP=dhcpIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState)
