...
Integer32, ObjectIdentity, Counter64, Gauge32, iso, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, MibIdentifier, ModuleIdentity, Unsigned32, Counter32, TimeTicks, Bits, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "ObjectIdentity", "Counter64", "Gauge32", "iso", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "MibIdentifier", "ModuleIdentity", "Unsigned32", "Counter32", "TimeTicks", "Bits", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, adonis=adonis, commandServer=commandServer, dhcpStats=dhcpStats, trapTFTP=trapTFTP, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemon=tftpDaemon, dhcpFixedIP=dhcpFixedIP, trapHA=trapHA, dhcpPoolStartIP=dhcpPoolStartIP, PYSNMP_MODULE_ID=adonis, dhcpIP=dhcpIP, dhcpPoolEntry=dhcpPoolEntry, dnsStatsFailure=dnsStatsFailure, trapDHCP=trapDHCP, adonisObjects=adonisObjects, commandServerDaemon=commandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetTable=dhcpSubnetTable, lcdDaemon=lcdDaemon, dnsDaemon=dnsDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsSuccess=dnsStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolAlert=dhcpPoolAlert, haReplicationBinding=haReplicationBinding, dnsStats=dnsStats, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, licenseExpiry=licenseExpiry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapReplicationFailure=trapReplicationFailure, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpConfig=dhcpConfig, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, ha=ha, dhcpPoolTable=dhcpPoolTable, trapCommandServer=trapCommandServer, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseStartTime=dhcpLeaseStartTime, dns=dns, dhcpDaemon=dhcpDaemon, dnsStatsReferral=dnsStatsReferral, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetMask=dhcpSubnetMask, trapDNSDaemon=trapDNSDaemon, trapReplication=trapReplication, trapDNS=trapDNS, dhcpLeaseEntry=dhcpLeaseEntry, systemState=systemState, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, lcd=lcd, haServiceRunning=haServiceRunning, licenseValid=licenseValid, tftpDaemonRunning=tftpDaemonRunning, dhcp=dhcp, dhcpPoolEndIP=dhcpPoolEndIP, systemDaemon=systemDaemon, trapCommandServerDaemon=trapCommandServerDaemon, tftp=tftp, trapSystem=trapSystem, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetAlert=dhcpSubnetAlert, system=system, dhcpPoolSize=dhcpPoolSize, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseTable=dhcpLeaseTable, dhcpFailOverState=dhcpFailOverState, dhcpPoolUsed=dhcpPoolUsed, trapSystemDaemon=trapSystemDaemon, adonisTraps=adonisTraps, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haService=haService, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIPTable=dhcpFixedIPTable, haServiceNodeType=haServiceNodeType, dhcpLeaseBindState=dhcpLeaseBindState)
