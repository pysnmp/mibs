...
MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, TimeTicks, Counter32, Integer32, NotificationType, ObjectIdentity, Unsigned32, Bits, ModuleIdentity, MibIdentifier, IpAddress, Counter64, iso, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "TimeTicks", "Counter32", "Integer32", "NotificationType", "ObjectIdentity", "Unsigned32", "Bits", "ModuleIdentity", "MibIdentifier", "IpAddress", "Counter64", "iso", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dns=dns, dnsDaemonRunning=dnsDaemonRunning, haServiceRunning=haServiceRunning, dhcpDaemonRunning=dhcpDaemonRunning, lcd=lcd, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemon=commandServerDaemon, tftpDaemonRunning=tftpDaemonRunning, trapDNS=trapDNS, dnsStatsReferral=dnsStatsReferral, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolEntry=dhcpPoolEntry, system=system, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonisTraps=adonisTraps, trapDNSDaemon=trapDNSDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetMask=dhcpSubnetMask, dhcpIP=dhcpIP, dhcpSubnetSize=dhcpSubnetSize, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemon=dhcpDaemon, dhcpConfig=dhcpConfig, trapReplicationFailure=trapReplicationFailure, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseEntry=dhcpLeaseEntry, dhcpStats=dhcpStats, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsFailure=dnsStatsFailure, trapCommandServer=trapCommandServer, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcp=dhcp, adonisObjects=adonisObjects, dnsStats=dnsStats, trapHA=trapHA, trapTFTP=trapTFTP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, PYSNMP_MODULE_ID=adonis, dhcpSubnetIP=dhcpSubnetIP, haServiceNodeType=haServiceNodeType, trapReplication=trapReplication, haReplicationBinding=haReplicationBinding, tftpDaemon=tftpDaemon, ha=ha, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetEntry=dhcpSubnetEntry, lcdDaemon=lcdDaemon, trapDHCP=trapDHCP, adonis=adonis, commandServer=commandServer, dhcpFailOverState=dhcpFailOverState, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, systemState=systemState, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolSize=dhcpPoolSize, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXDomain=dnsStatsNXDomain, systemDaemon=systemDaemon, tftp=tftp, haService=haService, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemon=dnsDaemon, dhcpLeaseTable=dhcpLeaseTable, trapSystemDaemon=trapSystemDaemon, licenseExpiry=licenseExpiry, dhcpPoolEndIP=dhcpPoolEndIP, trapSystem=trapSystem, dhcpPoolSubnetIP=dhcpPoolSubnetIP, licenseValid=licenseValid, dhcpLeaseHostname=dhcpLeaseHostname, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolTable=dhcpPoolTable, dhcpFixedIP=dhcpFixedIP, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCPDaemon=trapDHCPDaemon)
