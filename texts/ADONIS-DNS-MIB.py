...
Integer32, Unsigned32, NotificationType, MibIdentifier, ModuleIdentity, TimeTicks, iso, Counter32, Gauge32, Counter64, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, Bits, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Unsigned32", "NotificationType", "MibIdentifier", "ModuleIdentity", "TimeTicks", "iso", "Counter32", "Gauge32", "Counter64", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "Bits", "ObjectIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonisObjects=adonisObjects, dhcpFixedIPEntry=dhcpFixedIPEntry, systemDaemon=systemDaemon, dhcpLeaseTable=dhcpLeaseTable, trapCommandServer=trapCommandServer, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetEntry=dhcpSubnetEntry, adonis=adonis, system=system, haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, haServiceNodeType=haServiceNodeType, haService=haService, adonisTraps=adonisTraps, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTP=trapTFTP, lcd=lcd, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCPDaemon=trapDHCPDaemon, dnsDaemon=dnsDaemon, lcdDaemon=lcdDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIP=dhcpFixedIP, trapReplicationFailure=trapReplicationFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, licenseExpiry=licenseExpiry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsNXDomain=dnsStatsNXDomain, trapSystem=trapSystem, dnsDaemonRunning=dnsDaemonRunning, tftpDaemon=tftpDaemon, dnsStatsFailure=dnsStatsFailure, dhcp=dhcp, dhcpDaemon=dhcpDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseBindState=dhcpLeaseBindState, dhcpConfig=dhcpConfig, trapDHCP=trapDHCP, dhcpStats=dhcpStats, tftpDaemonRunning=tftpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, trapTFTPDaemon=trapTFTPDaemon, trapDNSDaemon=trapDNSDaemon, systemState=systemState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, PYSNMP_MODULE_ID=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolTable=dhcpPoolTable, dhcpFailOverState=dhcpFailOverState, licenseValid=licenseValid, haServiceRunning=haServiceRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapReplication=trapReplication, trapDNS=trapDNS, dhcpPoolSize=dhcpPoolSize, commandServerDaemon=commandServerDaemon, dns=dns, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolAlert=dhcpPoolAlert, dnsStatsReferral=dnsStatsReferral, ha=ha, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, dnsStatsRecursion=dnsStatsRecursion, dhcpIP=dhcpIP, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, trapHA=trapHA, tftp=tftp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStats=dnsStats, commandServer=commandServer, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetSize=dhcpSubnetSize, trapSystemDaemon=trapSystemDaemon, commandServerDaemonRunning=commandServerDaemonRunning)
