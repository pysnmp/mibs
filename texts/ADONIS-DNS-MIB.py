...
Counter64, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, ModuleIdentity, TimeTicks, enterprises, ObjectIdentity, Unsigned32, IpAddress, MibIdentifier, Gauge32, Counter32, NotificationType, iso = mibBuilder.importSymbols("SNMPv2-SMI", "Counter64", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "ModuleIdentity", "TimeTicks", "enterprises", "ObjectIdentity", "Unsigned32", "IpAddress", "MibIdentifier", "Gauge32", "Counter32", "NotificationType", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolStartIP=dhcpPoolStartIP, dhcpConfig=dhcpConfig, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcp=dhcp, trapCommandServer=trapCommandServer, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemDaemon=systemDaemon, dhcpSubnetIP=dhcpSubnetIP, PYSNMP_MODULE_ID=adonis, adonisTraps=adonisTraps, dnsDaemon=dnsDaemon, trapDHCP=trapDHCP, dhcpDaemonRunning=dhcpDaemonRunning, haServiceNodeType=haServiceNodeType, dhcpPoolTable=dhcpPoolTable, dhcpFailOverState=dhcpFailOverState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, trapHA=trapHA, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIP=dhcpFixedIP, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplicationFailure=trapReplicationFailure, licenseValid=licenseValid, tftp=tftp, dhcpPoolUsed=dhcpPoolUsed, trapCommandServerDaemon=trapCommandServerDaemon, lcdDaemon=lcdDaemon, dhcpFixedIPTable=dhcpFixedIPTable, commandServer=commandServer, dhcpSubnetMask=dhcpSubnetMask, licenseExpiry=licenseExpiry, dhcpSubnetSize=dhcpSubnetSize, trapDNS=trapDNS, adonis=adonis, adonisObjects=adonisObjects, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolAlert=dhcpPoolAlert, dnsStatsFailure=dnsStatsFailure, commandServerDaemon=commandServerDaemon, trapTFTPDaemon=trapTFTPDaemon, haServiceRunning=haServiceRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haService=haService, dhcpSubnetEntry=dhcpSubnetEntry, dhcpIP=dhcpIP, trapDHCPDaemon=trapDHCPDaemon, tftpDaemonRunning=tftpDaemonRunning, tftpDaemon=tftpDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpStats=dhcpStats, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseTable=dhcpLeaseTable, trapSystemDaemon=trapSystemDaemon, lcd=lcd, dnsStatsReferral=dnsStatsReferral, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapReplication=trapReplication, haReplicationBinding=haReplicationBinding, dhcpSubnetTable=dhcpSubnetTable, ha=ha, dhcpLeaseBindState=dhcpLeaseBindState, dns=dns, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapSystem=trapSystem, systemState=systemState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemonRunning=commandServerDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dnsStats=dnsStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDNSDaemon=trapDNSDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, system=system, dhcpDaemon=dhcpDaemon, dnsDaemonRunning=dnsDaemonRunning, trapTFTP=trapTFTP, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSize=dhcpPoolSize)
