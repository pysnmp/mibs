...
MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Integer32, IpAddress, iso, ObjectIdentity, Counter64, enterprises, Bits, Unsigned32, TimeTicks, NotificationType, Gauge32, MibIdentifier, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Integer32", "IpAddress", "iso", "ObjectIdentity", "Counter64", "enterprises", "Bits", "Unsigned32", "TimeTicks", "NotificationType", "Gauge32", "MibIdentifier", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, lcdDaemon=lcdDaemon, licenseValid=licenseValid, adonisObjects=adonisObjects, dnsDaemonDebugLevel=dnsDaemonDebugLevel, ha=ha, haServiceRunning=haServiceRunning, dhcp=dhcp, trapTFTPDaemon=trapTFTPDaemon, tftp=tftp, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemon=dhcpDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftpDaemon=tftpDaemon, commandServer=commandServer, dhcpSubnetIP=dhcpSubnetIP, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTP=trapTFTP, dhcpIP=dhcpIP, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolTable=dhcpPoolTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolAlert=dhcpPoolAlert, dnsStats=dnsStats, adonisTraps=adonisTraps, dhcpFailOverState=dhcpFailOverState, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIP=dhcpFixedIP, licenseExpiry=licenseExpiry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsFailure=dnsStatsFailure, trapDHCPDaemon=trapDHCPDaemon, trapCommandServer=trapCommandServer, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonRunning=dnsDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpConfig=dhcpConfig, haServiceNodeType=haServiceNodeType, dhcpLeaseStartTime=dhcpLeaseStartTime, adonis=adonis, dhcpPoolStartIP=dhcpPoolStartIP, dhcpStats=dhcpStats, trapDNSDaemon=trapDNSDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dns=dns, haService=haService, tftpDaemonRunning=tftpDaemonRunning, trapHA=trapHA, trapSystem=trapSystem, systemDaemon=systemDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseBindState=dhcpLeaseBindState, trapDNS=trapDNS, trapDHCP=trapDHCP, lcd=lcd, dhcpLeaseHostname=dhcpLeaseHostname, trapReplicationFailure=trapReplicationFailure, dnsStatsReferral=dnsStatsReferral, dnsDaemon=dnsDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolEntry=dhcpPoolEntry, commandServerDaemonRunning=commandServerDaemonRunning, trapSystemDaemon=trapSystemDaemon, systemState=systemState, commandServerDaemon=commandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, system=system, dhcpLeaseTable=dhcpLeaseTable, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolSize=dhcpPoolSize, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, PYSNMP_MODULE_ID=adonis, trapReplication=trapReplication)
