...
TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Counter64, MibIdentifier, ObjectIdentity, enterprises, Gauge32, Bits, NotificationType, Unsigned32, IpAddress, ModuleIdentity, Integer32, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Counter64", "MibIdentifier", "ObjectIdentity", "enterprises", "Gauge32", "Bits", "NotificationType", "Unsigned32", "IpAddress", "ModuleIdentity", "Integer32", "Counter32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsReferral=dnsStatsReferral, adonis=adonis, commandServerDaemonRunning=commandServerDaemonRunning, lcd=lcd, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftp=tftp, dhcpPoolEntry=dhcpPoolEntry, dnsStatsSuccess=dnsStatsSuccess, trapCommandServer=trapCommandServer, trapDHCP=trapDHCP, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseStartTime=dhcpLeaseStartTime, systemDaemon=systemDaemon, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStats=dnsStats, dhcpFixedIP=dhcpFixedIP, trapHA=trapHA, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetEntry=dhcpSubnetEntry, dns=dns, dhcpDaemon=dhcpDaemon, trapSystemDaemon=trapSystemDaemon, trapReplication=trapReplication, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonisTraps=adonisTraps, dhcpPoolSize=dhcpPoolSize, tftpDaemon=tftpDaemon, licenseExpiry=licenseExpiry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonRunning=dnsDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetIP=dhcpSubnetIP, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcp=dhcp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolTable=dhcpPoolTable, trapDNS=trapDNS, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolStartIP=dhcpPoolStartIP, dhcpConfig=dhcpConfig, dnsDaemon=dnsDaemon, dhcpStats=dhcpStats, commandServerDaemon=commandServerDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolAlert=dhcpPoolAlert, licenseValid=licenseValid, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, lcdDaemon=lcdDaemon, systemState=systemState, trapDHCPDaemon=trapDHCPDaemon, commandServer=commandServer, haReplicationBinding=haReplicationBinding, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseBindState=dhcpLeaseBindState, adonisObjects=adonisObjects, trapSystem=trapSystem, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, system=system, haService=haService, dnsStatsFailure=dnsStatsFailure, dhcpFixedIPTable=dhcpFixedIPTable, haServiceRunning=haServiceRunning, trapTFTP=trapTFTP, dnsStatsNXRRSet=dnsStatsNXRRSet, tftpDaemonRunning=tftpDaemonRunning, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, PYSNMP_MODULE_ID=adonis, trapCommandServerDaemon=trapCommandServerDaemon, haServiceNodeType=haServiceNodeType, dhcpSubnetTable=dhcpSubnetTable, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetAlert=dhcpSubnetAlert, trapReplicationFailure=trapReplicationFailure, ha=ha, dhcpSubnetUsed=dhcpSubnetUsed)
