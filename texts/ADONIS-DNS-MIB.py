...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolEndIP=dhcpPoolEndIP, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemon=commandServerDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceRunning=haServiceRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpConfig=dhcpConfig, lcdDaemon=lcdDaemon, haReplicationBinding=haReplicationBinding, dhcpStats=dhcpStats, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsReferral=dnsStatsReferral, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemon=dnsDaemon, trapReplication=trapReplication, dhcpPoolAlert=dhcpPoolAlert, trapSystem=trapSystem, dhcpSubnetMask=dhcpSubnetMask, tftpDaemon=tftpDaemon, dhcpPoolSize=dhcpPoolSize, dhcpFixedIP=dhcpFixedIP, dhcpSubnetUsed=dhcpSubnetUsed, dhcp=dhcp, adonis=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseBindState=dhcpLeaseBindState, trapHA=trapHA, trapCommandServer=trapCommandServer, dhcpSubnetIP=dhcpSubnetIP, licenseExpiry=licenseExpiry, dhcpDaemonRunning=dhcpDaemonRunning, trapTFTP=trapTFTP, commandServer=commandServer, tftp=tftp, dhcpSubnetEntry=dhcpSubnetEntry, PYSNMP_MODULE_ID=adonis, systemDaemon=systemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceNodeType=haServiceNodeType, lcd=lcd, dhcpIP=dhcpIP, system=system, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFailOverState=dhcpFailOverState, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolTable=dhcpPoolTable, systemState=systemState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, ha=ha, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dnsStats=dnsStats, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCP=trapDHCP, haService=haService, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolStartIP=dhcpPoolStartIP, licenseValid=licenseValid, trapDHCPDaemon=trapDHCPDaemon, trapSystemDaemon=trapSystemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemon=dhcpDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplicationFailure=trapReplicationFailure, adonisTraps=adonisTraps, adonisObjects=adonisObjects, dhcpLeaseStartTime=dhcpLeaseStartTime, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNS=trapDNS, dns=dns, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsFailure=dnsStatsFailure, dhcpPoolUsed=dhcpPoolUsed)
