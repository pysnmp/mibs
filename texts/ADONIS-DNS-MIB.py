...
Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, MibIdentifier, Counter64, iso, Bits, ModuleIdentity, enterprises, NotificationType, TimeTicks, ObjectIdentity, IpAddress, Counter32, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "MibIdentifier", "Counter64", "iso", "Bits", "ModuleIdentity", "enterprises", "NotificationType", "TimeTicks", "ObjectIdentity", "IpAddress", "Counter32", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolEntry=dhcpPoolEntry, dnsStatsFailure=dnsStatsFailure, dnsDaemon=dnsDaemon, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServerDaemon=trapCommandServerDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpIP=dhcpIP, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsReferral=dnsStatsReferral, commandServer=commandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, adonis=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haReplicationBinding=haReplicationBinding, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolTable=dhcpPoolTable, trapDNSDaemon=trapDNSDaemon, dhcpSubnetSize=dhcpSubnetSize, trapHA=trapHA, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcp=dhcp, trapReplicationFailure=trapReplicationFailure, ha=ha, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetUsed=dhcpSubnetUsed, haServiceNodeType=haServiceNodeType, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolStartIP=dhcpPoolStartIP, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, lcd=lcd, dhcpPoolAlert=dhcpPoolAlert, dns=dns, lcdDaemon=lcdDaemon, adonisTraps=adonisTraps, dhcpFailOverState=dhcpFailOverState, dhcpLeaseStartTime=dhcpLeaseStartTime, haService=haService, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseHostname=dhcpLeaseHostname, dhcpConfig=dhcpConfig, dhcpPoolSize=dhcpPoolSize, trapHAServiceFailOver=trapHAServiceFailOver, PYSNMP_MODULE_ID=adonis, adonisObjects=adonisObjects, trapDHCP=trapDHCP, dnsDaemonRunning=dnsDaemonRunning, dhcpStats=dhcpStats, dhcpDaemon=dhcpDaemon, haServiceRunning=haServiceRunning, dnsStats=dnsStats, licenseValid=licenseValid, trapSystem=trapSystem, dnsStatsSuccess=dnsStatsSuccess, trapDNS=trapDNS, tftpDaemon=tftpDaemon, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsNXRRSet=dnsStatsNXRRSet, trapCommandServer=trapCommandServer, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, systemDaemon=systemDaemon, dhcpFixedIP=dhcpFixedIP, commandServerDaemon=commandServerDaemon, systemState=systemState, system=system, licenseExpiry=licenseExpiry, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetIP=dhcpSubnetIP, tftp=tftp, trapReplication=trapReplication)
