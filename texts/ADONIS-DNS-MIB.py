...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haService=haService, dhcpPoolEntry=dhcpPoolEntry, trapTFTP=trapTFTP, dhcpPoolAlert=dhcpPoolAlert, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServer=commandServer, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNSDaemon=trapDNSDaemon, dhcp=dhcp, dhcpSubnetSize=dhcpSubnetSize, systemDaemon=systemDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpConfig=dhcpConfig, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsFailure=dnsStatsFailure, dhcpSubnetAlert=dhcpSubnetAlert, trapDNS=trapDNS, dhcpLeaseEntry=dhcpLeaseEntry, tftp=tftp, lcdDaemon=lcdDaemon, trapSystem=trapSystem, dhcpFixedIPTable=dhcpFixedIPTable, licenseValid=licenseValid, dnsDaemon=dnsDaemon, dns=dns, trapDHCP=trapDHCP, trapSystemDaemon=trapSystemDaemon, trapReplicationFailure=trapReplicationFailure, dnsStatsReferral=dnsStatsReferral, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetUsed=dhcpSubnetUsed, systemState=systemState, dhcpSubnetIP=dhcpSubnetIP, adonis=adonis, dnsStatsNXDomain=dnsStatsNXDomain, ha=ha, commandServerDaemon=commandServerDaemon, dhcpDaemon=dhcpDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haServiceNodeType=haServiceNodeType, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIP=dhcpFixedIP, trapCommandServer=trapCommandServer, dhcpSubnetTable=dhcpSubnetTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, adonisObjects=adonisObjects, dhcpPoolTable=dhcpPoolTable, haReplicationBinding=haReplicationBinding, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpStats=dhcpStats, licenseExpiry=licenseExpiry, dnsDaemonRunning=dnsDaemonRunning, haServiceRunning=haServiceRunning, lcd=lcd, tftpDaemon=tftpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStats=dnsStats, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSize=dhcpPoolSize, adonisTraps=adonisTraps, PYSNMP_MODULE_ID=adonis, dhcpLeaseBindState=dhcpLeaseBindState, system=system, dhcpFailOverState=dhcpFailOverState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapHA=trapHA, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpIP=dhcpIP, trapReplication=trapReplication, trapTFTPDaemon=trapTFTPDaemon)
