...
Unsigned32, Counter32, MibIdentifier, IpAddress, NotificationType, Counter64, Bits, enterprises, ModuleIdentity, ObjectIdentity, Gauge32, Integer32, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "Unsigned32", "Counter32", "MibIdentifier", "IpAddress", "NotificationType", "Counter64", "Bits", "enterprises", "ModuleIdentity", "ObjectIdentity", "Gauge32", "Integer32", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonis=adonis, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseHostname=dhcpLeaseHostname, trapCommandServerDaemon=trapCommandServerDaemon, commandServer=commandServer, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, adonisObjects=adonisObjects, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsRecursion=dnsStatsRecursion, trapSystemDaemon=trapSystemDaemon, haServiceNodeType=haServiceNodeType, dhcp=dhcp, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsFailure=dnsStatsFailure, commandServerDaemon=commandServerDaemon, tftp=tftp, trapReplicationFailure=trapReplicationFailure, haServiceRunning=haServiceRunning, dhcpFixedIP=dhcpFixedIP, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolAlert=dhcpPoolAlert, trapDNSDaemon=trapDNSDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, PYSNMP_MODULE_ID=adonis, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemon=dhcpDaemon, trapReplication=trapReplication, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemon=dnsDaemon, adonisTraps=adonisTraps, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolSize=dhcpPoolSize, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftpDaemon=tftpDaemon, dhcpFailOverState=dhcpFailOverState, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, trapSystem=trapSystem, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemonRunning=commandServerDaemonRunning, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEntry=dhcpLeaseEntry, system=system, trapTFTP=trapTFTP, haReplicationBinding=haReplicationBinding, dnsStatsReferral=dnsStatsReferral, dhcpIP=dhcpIP, dhcpPoolEndIP=dhcpPoolEndIP, trapHA=trapHA, trapCommandServer=trapCommandServer, ha=ha, systemDaemon=systemDaemon, dnsStats=dnsStats, licenseValid=licenseValid, dhcpConfig=dhcpConfig, lcd=lcd, systemState=systemState, dhcpFixedIPEntry=dhcpFixedIPEntry, licenseExpiry=licenseExpiry, trapDNS=trapDNS, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolTable=dhcpPoolTable, lcdDaemon=lcdDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haService=haService, dhcpPoolUsed=dhcpPoolUsed, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCP=trapDHCP, dhcpStats=dhcpStats, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert)
