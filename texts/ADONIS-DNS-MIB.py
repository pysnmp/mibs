...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetTable=dhcpSubnetTable, commandServerDaemon=commandServerDaemon, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, haServiceRunning=haServiceRunning, dnsStats=dnsStats, system=system, licenseExpiry=licenseExpiry, dhcpPoolSize=dhcpPoolSize, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpStats=dhcpStats, trapReplicationFailure=trapReplicationFailure, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServerDaemon=trapCommandServerDaemon, dns=dns, dhcpIP=dhcpIP, licenseValid=licenseValid, dnsStatsSuccess=dnsStatsSuccess, systemState=systemState, dhcpLeaseTable=dhcpLeaseTable, tftpDaemonRunning=tftpDaemonRunning, haReplicationBinding=haReplicationBinding, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetIP=dhcpSubnetIP, tftp=tftp, trapDNS=trapDNS, dhcp=dhcp, systemDaemon=systemDaemon, dnsDaemon=dnsDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsFailure=dnsStatsFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystem=trapSystem, dhcpConfig=dhcpConfig, dnsStatsNXDomain=dnsStatsNXDomain, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, dnsStatsReferral=dnsStatsReferral, adonisTraps=adonisTraps, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpDaemonRunning=dhcpDaemonRunning, tftpDaemon=tftpDaemon, adonis=adonis, lcdDaemon=lcdDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServer=trapCommandServer, trapDHCP=trapDHCP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, commandServer=commandServer, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapReplication=trapReplication, dhcpPoolEndIP=dhcpPoolEndIP, trapDNSDaemon=trapDNSDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemon=dhcpDaemon, commandServerDaemonRunning=commandServerDaemonRunning, lcd=lcd, haService=haService, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonisObjects=adonisObjects, dhcpPoolStartIP=dhcpPoolStartIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFailOverState=dhcpFailOverState, dhcpSubnetMask=dhcpSubnetMask, trapTFTPDaemon=trapTFTPDaemon, ha=ha, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, PYSNMP_MODULE_ID=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIP=dhcpFixedIP, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haServiceNodeType=haServiceNodeType)
