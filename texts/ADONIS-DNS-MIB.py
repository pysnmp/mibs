...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNS=trapDNS, dhcpDaemonRunning=dhcpDaemonRunning, haServiceRunning=haServiceRunning, dnsStatsFailure=dnsStatsFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServer=commandServer, trapSystem=trapSystem, systemState=systemState, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetIP=dhcpSubnetIP, dhcpFixedIPTable=dhcpFixedIPTable, dhcp=dhcp, dhcpPoolUsed=dhcpPoolUsed, dnsDaemon=dnsDaemon, dhcpDaemon=dhcpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, haService=haService, dnsStats=dnsStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseEntry=dhcpLeaseEntry, trapHA=trapHA, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, trapHAServiceFailOver=trapHAServiceFailOver, trapReplication=trapReplication, lcd=lcd, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseStartTime=dhcpLeaseStartTime, haServiceNodeType=haServiceNodeType, trapCommandServer=trapCommandServer, adonisObjects=adonisObjects, trapTFTPDaemon=trapTFTPDaemon, dhcpIP=dhcpIP, trapReplicationFailure=trapReplicationFailure, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapTFTP=trapTFTP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetEntry=dhcpSubnetEntry, lcdDaemon=lcdDaemon, ha=ha, dnsDaemonRunning=dnsDaemonRunning, dns=dns, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIP=dhcpFixedIP, trapDHCP=trapDHCP, PYSNMP_MODULE_ID=adonis, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFailOverState=dhcpFailOverState, adonis=adonis, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetTable=dhcpSubnetTable, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolAlert=dhcpPoolAlert, haReplicationBinding=haReplicationBinding, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemon=commandServerDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpFixedIPEntry=dhcpFixedIPEntry, system=system, tftp=tftp, dhcpConfig=dhcpConfig, dnsStatsSuccess=dnsStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonisTraps=adonisTraps, tftpDaemon=tftpDaemon, tftpDaemonRunning=tftpDaemonRunning, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseExpiry=licenseExpiry, dhcpSubnetAlert=dhcpSubnetAlert, trapDNSDaemon=trapDNSDaemon, systemDaemon=systemDaemon, licenseValid=licenseValid)
