...
TimeTicks, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, enterprises, Bits, ObjectIdentity, Gauge32, Unsigned32, Counter64, MibIdentifier, IpAddress, iso, Integer32, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "enterprises", "Bits", "ObjectIdentity", "Gauge32", "Unsigned32", "Counter64", "MibIdentifier", "IpAddress", "iso", "Integer32", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpConfig=dhcpConfig, dhcpPoolSize=dhcpPoolSize, PYSNMP_MODULE_ID=adonis, commandServerDaemon=commandServerDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpIP=dhcpIP, dnsDaemonRunning=dnsDaemonRunning, tftp=tftp, dhcpLeaseBindState=dhcpLeaseBindState, licenseExpiry=licenseExpiry, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolEndIP=dhcpPoolEndIP, adonisTraps=adonisTraps, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseTable=dhcpLeaseTable, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseHostname=dhcpLeaseHostname, system=system, dhcpFailOverState=dhcpFailOverState, haService=haService, trapSystemDaemon=trapSystemDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, dnsStatsSuccess=dnsStatsSuccess, ha=ha, dhcpSubnetMask=dhcpSubnetMask, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolTable=dhcpPoolTable, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolAlert=dhcpPoolAlert, dnsDaemon=dnsDaemon, tftpDaemon=tftpDaemon, systemState=systemState, haServiceRunning=haServiceRunning, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpStats=dhcpStats, licenseValid=licenseValid, dnsStatsNXRRSet=dnsStatsNXRRSet, dns=dns, adonis=adonis, dnsStats=dnsStats, trapReplicationFailure=trapReplicationFailure, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcp=dhcp, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetIP=dhcpSubnetIP, trapCommandServer=trapCommandServer, trapHA=trapHA, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsFailure=dnsStatsFailure, trapReplication=trapReplication, commandServer=commandServer, lcdDaemon=lcdDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTP=trapTFTP, dhcpFixedIP=dhcpFixedIP, systemDaemon=systemDaemon, trapSystem=trapSystem, tftpDaemonRunning=tftpDaemonRunning, adonisObjects=adonisObjects, dhcpDaemon=dhcpDaemon, lcd=lcd, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXDomain=dnsStatsNXDomain, trapDNS=trapDNS, dhcpSubnetUsed=dhcpSubnetUsed)
