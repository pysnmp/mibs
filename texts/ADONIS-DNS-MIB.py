...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTP=trapTFTP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetAlert=dhcpSubnetAlert, trapSystemDaemon=trapSystemDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFixedIP=dhcpFixedIP, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseBindState=dhcpLeaseBindState, dhcpConfig=dhcpConfig, trapDNS=trapDNS, tftp=tftp, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServerDaemon=trapCommandServerDaemon, systemState=systemState, trapHA=trapHA, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetEntry=dhcpSubnetEntry, system=system, dhcpPoolTable=dhcpPoolTable, dhcp=dhcp, adonis=adonis, dnsStatsRecursion=dnsStatsRecursion, haServiceNodeType=haServiceNodeType, haService=haService, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemon=dhcpDaemon, PYSNMP_MODULE_ID=adonis, trapDHCPDaemon=trapDHCPDaemon, systemDaemon=systemDaemon, dnsStatsFailure=dnsStatsFailure, trapCommandServer=trapCommandServer, commandServer=commandServer, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisObjects=adonisObjects, dhcpFixedIPTable=dhcpFixedIPTable, dnsStats=dnsStats, lcd=lcd, dhcpSubnetUsed=dhcpSubnetUsed, lcdDaemon=lcdDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, adonisTraps=adonisTraps, tftpDaemon=tftpDaemon, dhcpLeaseTable=dhcpLeaseTable, dnsDaemon=dnsDaemon, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, trapReplicationFailure=trapReplicationFailure, trapDHCP=trapDHCP, licenseValid=licenseValid, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpIP=dhcpIP, licenseExpiry=licenseExpiry, haReplicationBinding=haReplicationBinding, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolStartIP=dhcpPoolStartIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolEndIP=dhcpPoolEndIP, trapReplication=trapReplication, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceRunning=haServiceRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapSystem=trapSystem, commandServerDaemon=commandServerDaemon, dns=dns, dhcpPoolSize=dhcpPoolSize, dhcpSubnetMask=dhcpSubnetMask, dhcpFailOverState=dhcpFailOverState, trapTFTPDaemon=trapTFTPDaemon, trapDNSDaemon=trapDNSDaemon, dhcpStats=dhcpStats, ha=ha)
