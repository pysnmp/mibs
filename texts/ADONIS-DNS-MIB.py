...
NotificationType, Gauge32, Counter64, ModuleIdentity, ObjectIdentity, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, enterprises, Unsigned32, Bits, Counter32, iso, MibIdentifier, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "Gauge32", "Counter64", "ModuleIdentity", "ObjectIdentity", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "enterprises", "Unsigned32", "Bits", "Counter32", "iso", "MibIdentifier", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseEntry=dhcpLeaseEntry, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpIP=dhcpIP, licenseValid=licenseValid, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTPDaemon=trapTFTPDaemon, trapReplication=trapReplication, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolTable=dhcpPoolTable, dnsStatsRecursion=dnsStatsRecursion, commandServerDaemon=commandServerDaemon, dhcpPoolSize=dhcpPoolSize, haServiceRunning=haServiceRunning, haServiceNodeType=haServiceNodeType, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEntry=dhcpPoolEntry, trapSystem=trapSystem, trapCommandServer=trapCommandServer, lcd=lcd, trapDHCP=trapDHCP, dhcpFixedIP=dhcpFixedIP, dhcp=dhcp, trapSystemDaemon=trapSystemDaemon, dhcpFailOverState=dhcpFailOverState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, ha=ha, trapDNSDaemon=trapDNSDaemon, systemDaemon=systemDaemon, dhcpConfig=dhcpConfig, dnsDaemon=dnsDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haService=haService, commandServer=commandServer, adonisObjects=adonisObjects, haReplicationBinding=haReplicationBinding, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonisTraps=adonisTraps, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, tftp=tftp, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDHCPDaemon=trapDHCPDaemon, dnsStatsFailure=dnsStatsFailure, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseExpiry=licenseExpiry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, tftpDaemon=tftpDaemon, PYSNMP_MODULE_ID=adonis, lcdDaemon=lcdDaemon, dnsStats=dnsStats, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsReferral=dnsStatsReferral, dhcpSubnetSize=dhcpSubnetSize, trapTFTP=trapTFTP, systemState=systemState, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemon=dhcpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpStats=dhcpStats, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolUsed=dhcpPoolUsed, system=system, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonRunning=dnsDaemonRunning, dns=dns, adonis=adonis, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetAlert=dhcpSubnetAlert, trapHA=trapHA, trapDNS=trapDNS, trapReplicationFailure=trapReplicationFailure, dhcpSubnetMask=dhcpSubnetMask)
