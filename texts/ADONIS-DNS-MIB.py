...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpStats=dhcpStats, dhcpPoolStartIP=dhcpPoolStartIP, commandServer=commandServer, haService=haService, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsFailure=dnsStatsFailure, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsSuccess=dnsStatsSuccess, ha=ha, tftp=tftp, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseHostname=dhcpLeaseHostname, lcdDaemon=lcdDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dns=dns, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNS=trapDNS, licenseExpiry=licenseExpiry, dhcpSubnetEntry=dhcpSubnetEntry, haReplicationBinding=haReplicationBinding, dhcpSubnetSize=dhcpSubnetSize, trapHA=trapHA, dhcpPoolUsed=dhcpPoolUsed, trapTFTP=trapTFTP, trapCommandServerDaemon=trapCommandServerDaemon, dhcpConfig=dhcpConfig, adonis=adonis, trapCommandServer=trapCommandServer, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonRunning=dnsDaemonRunning, trapSystemDaemon=trapSystemDaemon, dhcpFixedIP=dhcpFixedIP, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNSDaemon=trapDNSDaemon, trapDHCP=trapDHCP, dhcpSubnetTable=dhcpSubnetTable, PYSNMP_MODULE_ID=adonis, tftpDaemon=tftpDaemon, trapReplicationFailure=trapReplicationFailure, system=system, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolTable=dhcpPoolTable, dnsStats=dnsStats, licenseValid=licenseValid, trapSystem=trapSystem, lcd=lcd, dhcpDaemon=dhcpDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTPDaemon=trapTFTPDaemon, trapReplication=trapReplication, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolSize=dhcpPoolSize, adonisObjects=adonisObjects, dhcpIP=dhcpIP, adonisTraps=adonisTraps, dhcpFailOverState=dhcpFailOverState, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsNXDomain=dnsStatsNXDomain, dhcp=dhcp, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemon=commandServerDaemon, haServiceNodeType=haServiceNodeType, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolEntry=dhcpPoolEntry, systemState=systemState, dhcpFixedIPTable=dhcpFixedIPTable, haServiceRunning=haServiceRunning, dnsStatsReferral=dnsStatsReferral, dnsDaemon=dnsDaemon, systemDaemon=systemDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess)
