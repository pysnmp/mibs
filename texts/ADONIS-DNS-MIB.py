...
Bits, TimeTicks, Counter32, Gauge32, NotificationType, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, Integer32, IpAddress, ModuleIdentity, iso, ObjectIdentity, enterprises, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "TimeTicks", "Counter32", "Gauge32", "NotificationType", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "Integer32", "IpAddress", "ModuleIdentity", "iso", "ObjectIdentity", "enterprises", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsSuccess=dnsStatsSuccess, ha=ha, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, dhcp=dhcp, tftpDaemon=tftpDaemon, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapTFTPDaemon=trapTFTPDaemon, dnsStatsFailure=dnsStatsFailure, dhcpPoolSize=dhcpPoolSize, systemState=systemState, licenseExpiry=licenseExpiry, dhcpFixedIPTable=dhcpFixedIPTable, adonis=adonis, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIP=dhcpFixedIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpConfig=dhcpConfig, trapReplication=trapReplication, dnsDaemon=dnsDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolTable=dhcpPoolTable, trapDNS=trapDNS, dns=dns, lcd=lcd, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemon=dhcpDaemon, dhcpLeaseHostname=dhcpLeaseHostname, trapTFTP=trapTFTP, tftpDaemonRunning=tftpDaemonRunning, trapHA=trapHA, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStats=dnsStats, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFailOverState=dhcpFailOverState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetSize=dhcpSubnetSize, dhcpStats=dhcpStats, trapReplicationFailure=trapReplicationFailure, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonRunning=dnsDaemonRunning, trapDHCP=trapDHCP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetIP=dhcpSubnetIP, systemDaemon=systemDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonRunning=dhcpDaemonRunning, haService=haService, trapDNSDaemon=trapDNSDaemon, dhcpPoolUsed=dhcpPoolUsed, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServer=commandServer, commandServerDaemon=commandServerDaemon, licenseValid=licenseValid, dhcpSubnetEntry=dhcpSubnetEntry, system=system, dhcpIP=dhcpIP, dhcpPoolEndIP=dhcpPoolEndIP, lcdDaemon=lcdDaemon, PYSNMP_MODULE_ID=adonis, adonisTraps=adonisTraps, haServiceRunning=haServiceRunning, trapSystem=trapSystem, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseEndTime=dhcpLeaseEndTime, haReplicationBinding=haReplicationBinding, trapCommandServer=trapCommandServer, dnsStatsReferral=dnsStatsReferral, tftp=tftp, trapSystemDaemon=trapSystemDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolStartIP=dhcpPoolStartIP)
