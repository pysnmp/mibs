...
iso, Integer32, ModuleIdentity, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, Counter32, MibIdentifier, NotificationType, Bits, Unsigned32, ObjectIdentity, IpAddress, enterprises, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Integer32", "ModuleIdentity", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "Counter32", "MibIdentifier", "NotificationType", "Bits", "Unsigned32", "ObjectIdentity", "IpAddress", "enterprises", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHostname=dhcpLeaseHostname, systemState=systemState, trapSystem=trapSystem, dhcpFailOverState=dhcpFailOverState, dhcpSubnetTable=dhcpSubnetTable, haService=haService, lcdDaemon=lcdDaemon, trapDNS=trapDNS, trapCommandServer=trapCommandServer, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXDomain=dnsStatsNXDomain, trapReplicationFailure=trapReplicationFailure, dnsStatsReferral=dnsStatsReferral, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, commandServerDaemon=commandServerDaemon, dhcpPoolEntry=dhcpPoolEntry, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, haReplicationBinding=haReplicationBinding, licenseExpiry=licenseExpiry, adonisTraps=adonisTraps, dhcpPoolTable=dhcpPoolTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetEntry=dhcpSubnetEntry, tftpDaemon=tftpDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemon=dnsDaemon, dhcpPoolSize=dhcpPoolSize, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpConfig=dhcpConfig, trapHA=trapHA, trapDNSDaemon=trapDNSDaemon, dhcpLeaseTable=dhcpLeaseTable, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServerDaemonRunning=commandServerDaemonRunning, licenseValid=licenseValid, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, lcd=lcd, adonis=adonis, dnsStatsFailure=dnsStatsFailure, dhcpPoolStartIP=dhcpPoolStartIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpFixedIPTable=dhcpFixedIPTable, trapTFTP=trapTFTP, dhcpIP=dhcpIP, systemDaemon=systemDaemon, tftp=tftp, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStats=dnsStats, adonisObjects=adonisObjects, dhcpDaemon=dhcpDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetSize=dhcpSubnetSize, trapSystemDaemon=trapSystemDaemon, ha=ha, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, trapDHCP=trapDHCP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTPDaemon=trapTFTPDaemon, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServer=commandServer, dhcp=dhcp, haServiceRunning=haServiceRunning, dhcpFixedIP=dhcpFixedIP, dhcpStats=dhcpStats, tftpDaemonRunning=tftpDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, system=system, trapReplication=trapReplication, dhcpSubnetUsed=dhcpSubnetUsed, PYSNMP_MODULE_ID=adonis)
