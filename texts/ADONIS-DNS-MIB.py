...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolTable=dhcpPoolTable, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpFixedIP=dhcpFixedIP, PYSNMP_MODULE_ID=adonis, dhcp=dhcp, ha=ha, trapSystem=trapSystem, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, trapReplication=trapReplication, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSize=dhcpPoolSize, trapHAServiceFailOver=trapHAServiceFailOver, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, trapDHCP=trapDHCP, dnsStatsReferral=dnsStatsReferral, dhcpConfig=dhcpConfig, adonis=adonis, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFailOverState=dhcpFailOverState, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, haServiceRunning=haServiceRunning, tftp=tftp, trapHA=trapHA, trapCommandServer=trapCommandServer, haService=haService, adonisObjects=adonisObjects, trapTFTP=trapTFTP, commandServerDaemon=commandServerDaemon, adonisTraps=adonisTraps, dnsStatsRecursion=dnsStatsRecursion, haReplicationBinding=haReplicationBinding, dhcpFixedIPTable=dhcpFixedIPTable, trapSystemDaemon=trapSystemDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolUsed=dhcpPoolUsed, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHostname=dhcpLeaseHostname, dhcpIP=dhcpIP, haServiceNodeType=haServiceNodeType, system=system, licenseExpiry=licenseExpiry, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonRunning=dnsDaemonRunning, tftpDaemon=tftpDaemon, dhcpDaemon=dhcpDaemon, dnsStatsFailure=dnsStatsFailure, systemDaemon=systemDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dhcpStats=dhcpStats, dnsDaemon=dnsDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseBindState=dhcpLeaseBindState, trapCommandServerDaemon=trapCommandServerDaemon, licenseValid=licenseValid, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDNSDaemon=trapDNSDaemon, dhcpSubnetMask=dhcpSubnetMask, dns=dns, dhcpFixedIPEntry=dhcpFixedIPEntry, systemState=systemState, dhcpSubnetUsed=dhcpSubnetUsed, lcdDaemon=lcdDaemon, commandServer=commandServer, dhcpSubnetAlert=dhcpSubnetAlert, lcd=lcd, trapReplicationFailure=trapReplicationFailure, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStats=dnsStats, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseEntry=dhcpLeaseEntry)
