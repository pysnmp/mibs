...
MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, ObjectIdentity, Gauge32, Unsigned32, Counter64, NotificationType, IpAddress, ModuleIdentity, TimeTicks, Counter32, MibIdentifier, iso, Bits, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "ObjectIdentity", "Gauge32", "Unsigned32", "Counter64", "NotificationType", "IpAddress", "ModuleIdentity", "TimeTicks", "Counter32", "MibIdentifier", "iso", "Bits", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDHCPDaemon=trapDHCPDaemon, dhcpPoolEntry=dhcpPoolEntry, trapHAServiceFailOver=trapHAServiceFailOver, adonisTraps=adonisTraps, trapReplication=trapReplication, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haService=haService, dhcpPoolUsed=dhcpPoolUsed, dnsStatsReferral=dnsStatsReferral, tftpDaemon=tftpDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftp=tftp, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetUsed=dhcpSubnetUsed, dhcpConfig=dhcpConfig, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemon=commandServerDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolSize=dhcpPoolSize, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolTable=dhcpPoolTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetIP=dhcpSubnetIP, dnsStatsNXRRSet=dnsStatsNXRRSet, haServiceRunning=haServiceRunning, trapSystemDaemon=trapSystemDaemon, dhcpDaemonRunning=dhcpDaemonRunning, dhcpIP=dhcpIP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, systemDaemon=systemDaemon, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolEndIP=dhcpPoolEndIP, dhcpDaemon=dhcpDaemon, dnsDaemonRunning=dnsDaemonRunning, haReplicationBinding=haReplicationBinding, system=system, dhcpSubnetMask=dhcpSubnetMask, adonis=adonis, trapHA=trapHA, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFailOverState=dhcpFailOverState, PYSNMP_MODULE_ID=adonis, trapCommandServer=trapCommandServer, trapTFTPDaemon=trapTFTPDaemon, trapCommandServerDaemon=trapCommandServerDaemon, trapDNS=trapDNS, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemon=dnsDaemon, dhcpPoolAlert=dhcpPoolAlert, systemState=systemState, dhcpPoolStartIP=dhcpPoolStartIP, ha=ha, adonisObjects=adonisObjects, trapDNSDaemon=trapDNSDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServer=commandServer, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetTable=dhcpSubnetTable, dnsStatsRecursion=dnsStatsRecursion, licenseExpiry=licenseExpiry, trapDHCP=trapDHCP, licenseValid=licenseValid, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpStats=dhcpStats, lcd=lcd, dnsStats=dnsStats, trapSystem=trapSystem, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcp=dhcp, haServiceNodeType=haServiceNodeType, dhcpFixedIPTable=dhcpFixedIPTable, trapTFTP=trapTFTP, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, dns=dns, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, lcdDaemon=lcdDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIP=dhcpFixedIP, dnsStatsFailure=dnsStatsFailure, dhcpLeaseTable=dhcpLeaseTable)
