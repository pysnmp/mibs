...
MibIdentifier, NotificationType, Counter64, TimeTicks, Bits, ObjectIdentity, IpAddress, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Unsigned32, Integer32, Counter32, enterprises, iso, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "NotificationType", "Counter64", "TimeTicks", "Bits", "ObjectIdentity", "IpAddress", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Unsigned32", "Integer32", "Counter32", "enterprises", "iso", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapReplication=trapReplication, lcdDaemon=lcdDaemon, dhcpDaemon=dhcpDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapHA=trapHA, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolEndIP=dhcpPoolEndIP, systemDaemon=systemDaemon, dhcpPoolEntry=dhcpPoolEntry, dnsStatsReferral=dnsStatsReferral, commandServerDaemon=commandServerDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, ha=ha, dhcpDaemonRunning=dhcpDaemonRunning, system=system, dhcpFixedIP=dhcpFixedIP, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, adonisObjects=adonisObjects, dnsDaemon=dnsDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonRunning=dnsDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcp=dhcp, licenseExpiry=licenseExpiry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseTable=dhcpLeaseTable, dnsStats=dnsStats, commandServer=commandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDHCPDaemon=trapDHCPDaemon, dnsStatsFailure=dnsStatsFailure, dnsStatsRecursion=dnsStatsRecursion, adonis=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpIP=dhcpIP, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, adonisTraps=adonisTraps, haReplicationBinding=haReplicationBinding, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, trapDNS=trapDNS, dnsStatsSuccess=dnsStatsSuccess, licenseValid=licenseValid, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseEndTime=dhcpLeaseEndTime, haService=haService, dhcpConfig=dhcpConfig, dhcpPoolSize=dhcpPoolSize, PYSNMP_MODULE_ID=adonis, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, trapHAServiceFailOver=trapHAServiceFailOver, haServiceRunning=haServiceRunning, systemState=systemState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFailOverState=dhcpFailOverState, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, lcd=lcd, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolTable=dhcpPoolTable, dhcpSubnetIP=dhcpSubnetIP, dns=dns, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haServiceNodeType=haServiceNodeType, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetUsed=dhcpSubnetUsed, trapSystem=trapSystem, tftp=tftp, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServer=trapCommandServer, tftpDaemon=tftpDaemon, trapDHCP=trapDHCP, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTP=trapTFTP, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel)
