...
ObjectIdentity, NotificationType, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, MibIdentifier, Counter32, Gauge32, Counter64, ModuleIdentity, TimeTicks, IpAddress, Integer32, Bits, Unsigned32 = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "NotificationType", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "MibIdentifier", "Counter32", "Gauge32", "Counter64", "ModuleIdentity", "TimeTicks", "IpAddress", "Integer32", "Bits", "Unsigned32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, licenseExpiry=licenseExpiry, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemDaemon=systemDaemon, tftpDaemonRunning=tftpDaemonRunning, trapSystemDaemon=trapSystemDaemon, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIPTable=dhcpFixedIPTable, trapDNSDaemon=trapDNSDaemon, haServiceRunning=haServiceRunning, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIP=dhcpFixedIP, dhcpStats=dhcpStats, dhcpSubnetUsed=dhcpSubnetUsed, dns=dns, licenseValid=licenseValid, dhcpDaemon=dhcpDaemon, commandServerDaemon=commandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, dhcpSubnetMask=dhcpSubnetMask, haReplicationBinding=haReplicationBinding, dhcpIP=dhcpIP, trapTFTP=trapTFTP, haService=haService, dnsStats=dnsStats, dhcpPoolStartIP=dhcpPoolStartIP, trapHA=trapHA, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolTable=dhcpPoolTable, dhcp=dhcp, dhcpSubnetIP=dhcpSubnetIP, dhcpConfig=dhcpConfig, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcd=lcd, adonisTraps=adonisTraps, ha=ha, dnsDaemon=dnsDaemon, dhcpLeaseHostname=dhcpLeaseHostname, trapSystem=trapSystem, trapReplicationFailure=trapReplicationFailure, dhcpFailOverState=dhcpFailOverState, dnsDaemonRunning=dnsDaemonRunning, tftpDaemon=tftpDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsFailure=dnsStatsFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, PYSNMP_MODULE_ID=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapTFTPDaemon=trapTFTPDaemon, trapCommandServer=trapCommandServer, system=system, dhcpSubnetEntry=dhcpSubnetEntry, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapHAServiceFailOver=trapHAServiceFailOver, adonis=adonis, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCPDaemon=trapDHCPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsReferral=dnsStatsReferral, lcdDaemon=lcdDaemon, dhcpSubnetAlert=dhcpSubnetAlert, trapReplication=trapReplication, tftp=tftp, commandServer=commandServer, trapDHCP=trapDHCP, haServiceNodeType=haServiceNodeType, systemState=systemState, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXDomain=dnsStatsNXDomain, adonisObjects=adonisObjects, dhcpPoolSize=dhcpPoolSize)
