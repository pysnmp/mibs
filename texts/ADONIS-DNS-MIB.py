...
Bits, Counter64, NotificationType, Gauge32, Integer32, enterprises, ModuleIdentity, Counter32, iso, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, TimeTicks, Unsigned32, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Counter64", "NotificationType", "Gauge32", "Integer32", "enterprises", "ModuleIdentity", "Counter32", "iso", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "TimeTicks", "Unsigned32", "ObjectIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcp=dhcp, dnsDaemon=dnsDaemon, trapSystem=trapSystem, ha=ha, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, licenseValid=licenseValid, haReplicationBinding=haReplicationBinding, dhcpPoolTable=dhcpPoolTable, tftpDaemon=tftpDaemon, dhcpSubnetIP=dhcpSubnetIP, system=system, trapDHCPDaemon=trapDHCPDaemon, trapTFTP=trapTFTP, dnsStats=dnsStats, trapSystemDaemon=trapSystemDaemon, PYSNMP_MODULE_ID=adonis, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsStatsRecursion=dnsStatsRecursion, dnsStatsSuccess=dnsStatsSuccess, trapDNSDaemon=trapDNSDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, haServiceRunning=haServiceRunning, dhcpStats=dhcpStats, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStatsFailure=dnsStatsFailure, dhcpSubnetMask=dhcpSubnetMask, adonisObjects=adonisObjects, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, commandServer=commandServer, dhcpFixedIP=dhcpFixedIP, lcd=lcd, dhcpDaemon=dhcpDaemon, trapCommandServer=trapCommandServer, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPTable=dhcpFixedIPTable, haServiceNodeType=haServiceNodeType, adonisTraps=adonisTraps, trapHA=trapHA, trapReplicationFailure=trapReplicationFailure, tftp=tftp, dnsStatsNXDomain=dnsStatsNXDomain, trapDHCP=trapDHCP, trapReplication=trapReplication, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, systemState=systemState, dhcpPoolSize=dhcpPoolSize, trapDNS=trapDNS, dnsStatsReferral=dnsStatsReferral, dhcpConfig=dhcpConfig, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, commandServerDaemon=commandServerDaemon, haService=haService, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEndIP=dhcpPoolEndIP, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseEntry=dhcpLeaseEntry, systemDaemon=systemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dns=dns, licenseExpiry=licenseExpiry, trapCommandServerDaemon=trapCommandServerDaemon, lcdDaemon=lcdDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonis=adonis, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolAlert=dhcpPoolAlert)
