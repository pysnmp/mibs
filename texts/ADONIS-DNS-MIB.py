...
Bits, Counter64, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, ModuleIdentity, Counter32, IpAddress, ObjectIdentity, NotificationType, MibIdentifier, iso, Gauge32, Unsigned32, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "Counter64", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "ModuleIdentity", "Counter32", "IpAddress", "ObjectIdentity", "NotificationType", "MibIdentifier", "iso", "Gauge32", "Unsigned32", "enterprises")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", lcd=lcd, tftp=tftp, dhcpFailOverState=dhcpFailOverState, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemon=dnsDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, PYSNMP_MODULE_ID=adonis, trapSystemDaemon=trapSystemDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetSize=dhcpSubnetSize, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dns=dns, commandServerDaemon=commandServerDaemon, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServer=trapCommandServer, dhcpLeaseEndTime=dhcpLeaseEndTime, trapTFTP=trapTFTP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDNSDaemon=trapDNSDaemon, dhcpSubnetMask=dhcpSubnetMask, dnsStats=dnsStats, haServiceRunning=haServiceRunning, dnsStatsReferral=dnsStatsReferral, dhcpConfig=dhcpConfig, trapSystem=trapSystem, trapDHCP=trapDHCP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, systemState=systemState, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, dhcpStats=dhcpStats, haService=haService, dhcpPoolTable=dhcpPoolTable, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetIP=dhcpSubnetIP, trapReplicationFailure=trapReplicationFailure, licenseValid=licenseValid, trapReplication=trapReplication, dhcpPoolSize=dhcpPoolSize, dhcpFixedIP=dhcpFixedIP, commandServer=commandServer, dnsDaemonRunning=dnsDaemonRunning, dnsStatsFailure=dnsStatsFailure, dhcpFixedIPTable=dhcpFixedIPTable, dhcpIP=dhcpIP, tftpDaemon=tftpDaemon, ha=ha, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, licenseExpiry=licenseExpiry, trapHAServiceFailOver=trapHAServiceFailOver, adonisTraps=adonisTraps, dhcpSubnetUsed=dhcpSubnetUsed, lcdDaemon=lcdDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetEntry=dhcpSubnetEntry, haReplicationBinding=haReplicationBinding, dhcpPoolUsed=dhcpPoolUsed, trapDHCPDaemon=trapDHCPDaemon, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, adonis=adonis, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemon=dhcpDaemon, trapDNS=trapDNS, trapHA=trapHA, dhcpLeaseHostname=dhcpLeaseHostname, dhcp=dhcp, dhcpLeaseStartTime=dhcpLeaseStartTime, haServiceNodeType=haServiceNodeType, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsNXRRSet=dnsStatsNXRRSet, system=system)
