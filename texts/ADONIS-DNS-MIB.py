...
ModuleIdentity, Unsigned32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, Integer32, enterprises, iso, Counter64, MibIdentifier, NotificationType, TimeTicks, Counter32, ObjectIdentity, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Unsigned32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "Integer32", "enterprises", "iso", "Counter64", "MibIdentifier", "NotificationType", "TimeTicks", "Counter32", "ObjectIdentity", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", lcdDaemon=lcdDaemon, dhcpFixedIP=dhcpFixedIP, dhcpPoolUsed=dhcpPoolUsed, haService=haService, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, haReplicationBinding=haReplicationBinding, trapDHCP=trapDHCP, trapSystem=trapSystem, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsRecursion=dnsStatsRecursion, system=system, dhcpConfig=dhcpConfig, licenseValid=licenseValid, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemonRunning=commandServerDaemonRunning, adonisTraps=adonisTraps, tftpDaemon=tftpDaemon, dhcpLeaseBindState=dhcpLeaseBindState, haServiceNodeType=haServiceNodeType, adonis=adonis, dhcpDaemon=dhcpDaemon, trapReplication=trapReplication, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolTable=dhcpPoolTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFailOverState=dhcpFailOverState, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseEntry=dhcpLeaseEntry, licenseExpiry=licenseExpiry, commandServerDaemon=commandServerDaemon, trapCommandServerDaemon=trapCommandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, ha=ha, trapTFTP=trapTFTP, dhcpSubnetTable=dhcpSubnetTable, dnsStats=dnsStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsFailure=dnsStatsFailure, lcd=lcd, trapSystemDaemon=trapSystemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServer=trapCommandServer, dnsDaemon=dnsDaemon, tftp=tftp, dhcpPoolStartIP=dhcpPoolStartIP, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolAlert=dhcpPoolAlert, systemState=systemState, trapHA=trapHA, dhcpSubnetMask=dhcpSubnetMask, commandServer=commandServer, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpIP=dhcpIP, dhcp=dhcp, haServiceRunning=haServiceRunning, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetAlert=dhcpSubnetAlert, trapReplicationFailure=trapReplicationFailure, PYSNMP_MODULE_ID=adonis, dhcpPoolSize=dhcpPoolSize, dhcpSubnetSize=dhcpSubnetSize, dns=dns, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsNXRRSet=dnsStatsNXRRSet, adonisObjects=adonisObjects, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpStats=dhcpStats, systemDaemon=systemDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseStartTime=dhcpLeaseStartTime, tftpDaemonRunning=tftpDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolEndIP=dhcpPoolEndIP, trapDNS=trapDNS)
