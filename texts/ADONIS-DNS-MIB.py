...
NotificationType, enterprises, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, iso, Bits, Integer32, Unsigned32, Counter64, Gauge32, ModuleIdentity, Counter32, TimeTicks, IpAddress, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "NotificationType", "enterprises", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "Bits", "Integer32", "Unsigned32", "Counter64", "Gauge32", "ModuleIdentity", "Counter32", "TimeTicks", "IpAddress", "ObjectIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemon=tftpDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolStartIP=dhcpPoolStartIP, systemState=systemState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTable=dhcpLeaseTable, trapSystem=trapSystem, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, adonisTraps=adonisTraps, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapCommandServer=trapCommandServer, trapReplication=trapReplication, lcdDaemon=lcdDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIPEntry=dhcpFixedIPEntry, PYSNMP_MODULE_ID=adonis, tftp=tftp, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemonRunning=commandServerDaemonRunning, trapDNSDaemon=trapDNSDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemon=dnsDaemon, dhcpSubnetEntry=dhcpSubnetEntry, trapTFTP=trapTFTP, dhcpDaemon=dhcpDaemon, adonisObjects=adonisObjects, haReplicationBinding=haReplicationBinding, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStats=dnsStats, trapReplicationFailure=trapReplicationFailure, commandServerDaemon=commandServerDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcd=lcd, licenseValid=licenseValid, dhcp=dhcp, haServiceNodeType=haServiceNodeType, dnsStatsRecursion=dnsStatsRecursion, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHostname=dhcpLeaseHostname, trapDHCP=trapDHCP, commandServer=commandServer, dhcpIP=dhcpIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetIP=dhcpSubnetIP, haService=haService, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDHCPDaemon=trapDHCPDaemon, licenseExpiry=licenseExpiry, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolSize=dhcpPoolSize, ha=ha, dhcpFailOverState=dhcpFailOverState, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpStats=dhcpStats, dhcpFixedIP=dhcpFixedIP, adonis=adonis, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolTable=dhcpPoolTable, dhcpLeaseStartTime=dhcpLeaseStartTime, tftpDaemonRunning=tftpDaemonRunning, dhcpConfig=dhcpConfig, dnsStatsNXRRSet=dnsStatsNXRRSet, trapSystemDaemon=trapSystemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPTable=dhcpFixedIPTable, trapHAServiceFailOver=trapHAServiceFailOver, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, trapDNS=trapDNS, dns=dns, dnsStatsReferral=dnsStatsReferral, haServiceRunning=haServiceRunning, systemDaemon=systemDaemon, system=system)
