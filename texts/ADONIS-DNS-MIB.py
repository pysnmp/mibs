...
ModuleIdentity, iso, Counter32, Unsigned32, Integer32, Bits, enterprises, Counter64, TimeTicks, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, Gauge32, ObjectIdentity, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "iso", "Counter32", "Unsigned32", "Integer32", "Bits", "enterprises", "Counter64", "TimeTicks", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "Gauge32", "ObjectIdentity", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemDaemon=systemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsStats=dnsStats, dns=dns, trapSystemDaemon=trapSystemDaemon, dhcpFixedIP=dhcpFixedIP, trapReplicationFailure=trapReplicationFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpIP=dhcpIP, haServiceRunning=haServiceRunning, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemon=dhcpDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpConfig=dhcpConfig, PYSNMP_MODULE_ID=adonis, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXDomain=dnsStatsNXDomain, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolTable=dhcpPoolTable, systemState=systemState, dnsStatsSuccess=dnsStatsSuccess, haReplicationBinding=haReplicationBinding, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapSystem=trapSystem, dhcpSubnetUsed=dhcpSubnetUsed, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonRunning=dnsDaemonRunning, trapDNSDaemon=trapDNSDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, lcd=lcd, system=system, ha=ha, trapTFTP=trapTFTP, trapHA=trapHA, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, tftpDaemon=tftpDaemon, commandServerDaemon=commandServerDaemon, adonisObjects=adonisObjects, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFailOverState=dhcpFailOverState, haServiceNodeType=haServiceNodeType, commandServerDaemonRunning=commandServerDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetEntry=dhcpSubnetEntry, trapDHCP=trapDHCP, dhcpLeaseBindState=dhcpLeaseBindState, dhcp=dhcp, dhcpLeaseTable=dhcpLeaseTable, dnsStatsReferral=dnsStatsReferral, trapDNS=trapDNS, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetMask=dhcpSubnetMask, trapReplication=trapReplication, licenseValid=licenseValid, adonisTraps=adonisTraps, adonis=adonis, trapCommandServer=trapCommandServer, dnsDaemon=dnsDaemon, licenseExpiry=licenseExpiry, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpStats=dhcpStats, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, haService=haService, tftp=tftp, dhcpDaemonRunning=dhcpDaemonRunning, commandServer=commandServer, lcdDaemon=lcdDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSubnetIP=dhcpPoolSubnetIP)
