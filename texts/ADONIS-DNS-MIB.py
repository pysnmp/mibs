...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", tftpDaemon=tftpDaemon, adonisTraps=adonisTraps, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolStartIP=dhcpPoolStartIP, ha=ha, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetTable=dhcpSubnetTable, trapDHCPDaemon=trapDHCPDaemon, tftp=tftp, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIPTable=dhcpFixedIPTable, trapReplicationFailure=trapReplicationFailure, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonRunning=dhcpDaemonRunning, lcd=lcd, dhcpPoolAlert=dhcpPoolAlert, dnsStatsReferral=dnsStatsReferral, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNSDaemon=trapDNSDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStatsNXRRSet=dnsStatsNXRRSet, trapCommandServer=trapCommandServer, trapDHCP=trapDHCP, dnsDaemon=dnsDaemon, haService=haService, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, commandServerDaemon=commandServerDaemon, dnsStats=dnsStats, trapSystem=trapSystem, dhcpIP=dhcpIP, systemDaemon=systemDaemon, dhcpPoolTable=dhcpPoolTable, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haReplicationBinding=haReplicationBinding, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsRecursion=dnsStatsRecursion, lcdDaemon=lcdDaemon, dhcpConfig=dhcpConfig, dhcpLeaseBindState=dhcpLeaseBindState, dns=dns, licenseExpiry=licenseExpiry, dhcpSubnetUsed=dhcpSubnetUsed, dhcpStats=dhcpStats, trapHA=trapHA, dhcpPoolEndIP=dhcpPoolEndIP, system=system, dhcpSubnetIP=dhcpSubnetIP, adonisObjects=adonisObjects, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIP=dhcpFixedIP, trapDNS=trapDNS, dhcpLeaseStartTime=dhcpLeaseStartTime, systemState=systemState, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapReplication=trapReplication, haServiceRunning=haServiceRunning, commandServer=commandServer, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemonRunning=commandServerDaemonRunning, trapSystemDaemon=trapSystemDaemon, dhcpPoolSize=dhcpPoolSize, PYSNMP_MODULE_ID=adonis, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsFailure=dnsStatsFailure, dhcpFailOverState=dhcpFailOverState, licenseValid=licenseValid, haServiceNodeType=haServiceNodeType, trapTFTP=trapTFTP, adonis=adonis, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcp=dhcp, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemon=dhcpDaemon, dhcpSubnetMask=dhcpSubnetMask)
