...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapCommandServer=trapCommandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetMask=dhcpSubnetMask, systemState=systemState, lcdDaemon=lcdDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDHCP=trapDHCP, dnsDaemon=dnsDaemon, trapReplication=trapReplication, dhcpSubnetTable=dhcpSubnetTable, trapSystemDaemon=trapSystemDaemon, haReplicationBinding=haReplicationBinding, haServiceRunning=haServiceRunning, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStats=dnsStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpIP=dhcpIP, dhcpStats=dhcpStats, dhcpLeaseHostname=dhcpLeaseHostname, dhcpConfig=dhcpConfig, licenseValid=licenseValid, lcd=lcd, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, system=system, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapDNSDaemon=trapDNSDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetSize=dhcpSubnetSize, haService=haService, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolTable=dhcpPoolTable, dhcpSubnetAlert=dhcpSubnetAlert, haServiceNodeType=haServiceNodeType, trapSystem=trapSystem, dhcpFixedIP=dhcpFixedIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEndIP=dhcpPoolEndIP, ha=ha, licenseExpiry=licenseExpiry, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServerDaemon=commandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, trapDNS=trapDNS, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseEntry=dhcpLeaseEntry, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapHA=trapHA, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, trapTFTP=trapTFTP, dnsStatsReferral=dnsStatsReferral, dhcpDaemon=dhcpDaemon, dhcpFailOverState=dhcpFailOverState, tftp=tftp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolStartIP=dhcpPoolStartIP, adonis=adonis, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTable=dhcpLeaseTable, PYSNMP_MODULE_ID=adonis, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolEntry=dhcpPoolEntry, tftpDaemonRunning=tftpDaemonRunning, trapReplicationFailure=trapReplicationFailure, tftpDaemon=tftpDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dns=dns, trapTFTPDaemon=trapTFTPDaemon, commandServer=commandServer, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, systemDaemon=systemDaemon, adonisObjects=adonisObjects, dhcp=dhcp, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolAlert=dhcpPoolAlert)
