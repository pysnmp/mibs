...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpFixedIP=dhcpFixedIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServerDaemonRunning=commandServerDaemonRunning, dns=dns, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemon=dhcpDaemon, systemState=systemState, trapSystemDaemon=trapSystemDaemon, commandServer=commandServer, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonRunning=dnsDaemonRunning, haServiceNodeType=haServiceNodeType, trapDNS=trapDNS, dnsStatsRecursion=dnsStatsRecursion, dhcpFailOverState=dhcpFailOverState, system=system, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, lcd=lcd, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, PYSNMP_MODULE_ID=adonis, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonDebugLevel=dnsDaemonDebugLevel, licenseValid=licenseValid, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseTable=dhcpLeaseTable, systemDaemon=systemDaemon, licenseExpiry=licenseExpiry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haReplicationBinding=haReplicationBinding, dhcpConfig=dhcpConfig, dnsStatsNXDomain=dnsStatsNXDomain, tftpDaemonRunning=tftpDaemonRunning, tftpDaemon=tftpDaemon, dhcpPoolAlert=dhcpPoolAlert, adonisObjects=adonisObjects, dhcpIP=dhcpIP, dhcpSubnetSize=dhcpSubnetSize, dnsStatsReferral=dnsStatsReferral, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceRunning=haServiceRunning, trapTFTP=trapTFTP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpStats=dhcpStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPTable=dhcpFixedIPTable, adonisTraps=adonisTraps, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, trapCommandServer=trapCommandServer, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDNSDaemon=trapDNSDaemon, dhcpPoolSize=dhcpPoolSize, trapCommandServerDaemon=trapCommandServerDaemon, trapReplicationFailure=trapReplicationFailure, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapSystem=trapSystem, dhcpSubnetEntry=dhcpSubnetEntry, trapHA=trapHA, tftp=tftp, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolUsed=dhcpPoolUsed, dnsStatsSuccess=dnsStatsSuccess, ha=ha, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServerDaemon=commandServerDaemon, dhcpPoolTable=dhcpPoolTable, trapDHCP=trapDHCP, trapReplication=trapReplication, dhcpDaemonRunning=dhcpDaemonRunning, lcdDaemon=lcdDaemon, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolStartIP=dhcpPoolStartIP, haService=haService, dhcp=dhcp, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStats=dnsStats, adonis=adonis, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsFailure=dnsStatsFailure, dnsDaemon=dnsDaemon)
