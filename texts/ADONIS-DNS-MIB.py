...
IpAddress, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, Counter64, Bits, ObjectIdentity, enterprises, iso, MibIdentifier, NotificationType, Unsigned32, ModuleIdentity, Counter32, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "IpAddress", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "Counter64", "Bits", "ObjectIdentity", "enterprises", "iso", "MibIdentifier", "NotificationType", "Unsigned32", "ModuleIdentity", "Counter32", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", ha=ha, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonisObjects=adonisObjects, commandServerDaemon=commandServerDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolAlert=dhcpPoolAlert, licenseValid=licenseValid, dhcpPoolEntry=dhcpPoolEntry, dnsStats=dnsStats, dhcpFixedIP=dhcpFixedIP, dhcpLeaseTable=dhcpLeaseTable, trapSystem=trapSystem, system=system, dhcpDaemon=dhcpDaemon, dnsDaemon=dnsDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolStartIP=dhcpPoolStartIP, licenseExpiry=licenseExpiry, trapSystemDaemon=trapSystemDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, tftpDaemonRunning=tftpDaemonRunning, trapCommandServer=trapCommandServer, trapDNS=trapDNS, dhcpConfig=dhcpConfig, haServiceNodeType=haServiceNodeType, adonis=adonis, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolTable=dhcpPoolTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, trapHA=trapHA, systemState=systemState, dhcpSubnetSize=dhcpSubnetSize, PYSNMP_MODULE_ID=adonis, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolUsed=dhcpPoolUsed, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpFixedIPTable=dhcpFixedIPTable, dns=dns, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpStats=dhcpStats, dhcp=dhcp, dnsDaemonRunning=dnsDaemonRunning, haService=haService, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseEndTime=dhcpLeaseEndTime, trapReplicationFailure=trapReplicationFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolSize=dhcpPoolSize, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetIP=dhcpSubnetIP, trapDNSDaemon=trapDNSDaemon, dhcpSubnetMask=dhcpSubnetMask, systemDaemon=systemDaemon, trapTFTP=trapTFTP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseHostname=dhcpLeaseHostname, dnsStatsFailure=dnsStatsFailure, commandServerDaemonRunning=commandServerDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, adonisTraps=adonisTraps, dnsStatsReferral=dnsStatsReferral, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplication=trapReplication, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCP=trapDHCP, dhcpIP=dhcpIP, lcdDaemon=lcdDaemon, dhcpFailOverState=dhcpFailOverState, tftp=tftp, tftpDaemon=tftpDaemon, commandServer=commandServer, haServiceRunning=haServiceRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcd=lcd, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred)
