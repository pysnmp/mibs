...
MibIdentifier, Integer32, IpAddress, enterprises, NotificationType, ModuleIdentity, Bits, Unsigned32, Counter32, TimeTicks, Counter64, ObjectIdentity, Gauge32, iso, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Integer32", "IpAddress", "enterprises", "NotificationType", "ModuleIdentity", "Bits", "Unsigned32", "Counter32", "TimeTicks", "Counter64", "ObjectIdentity", "Gauge32", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseBindState=dhcpLeaseBindState, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemon=commandServerDaemon, trapDHCPDaemon=trapDHCPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonRunning=dnsDaemonRunning, PYSNMP_MODULE_ID=adonis, dnsStatsReferral=dnsStatsReferral, systemState=systemState, adonisTraps=adonisTraps, dhcpIP=dhcpIP, dhcpLeaseHostname=dhcpLeaseHostname, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolTable=dhcpPoolTable, dhcpSubnetUsed=dhcpSubnetUsed, trapDNS=trapDNS, dnsStats=dnsStats, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, haReplicationBinding=haReplicationBinding, dhcpConfig=dhcpConfig, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapTFTP=trapTFTP, trapSystem=trapSystem, trapReplicationFailure=trapReplicationFailure, dnsStatsFailure=dnsStatsFailure, dhcpFixedIP=dhcpFixedIP, trapReplication=trapReplication, system=system, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolSize=dhcpPoolSize, trapDNSDaemon=trapDNSDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonis=adonis, dns=dns, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, licenseExpiry=licenseExpiry, trapHA=trapHA, licenseValid=licenseValid, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haServiceNodeType=haServiceNodeType, commandServer=commandServer, lcd=lcd, lcdDaemon=lcdDaemon, dnsStatsNXDomain=dnsStatsNXDomain, systemDaemon=systemDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, ha=ha, haServiceRunning=haServiceRunning, dhcp=dhcp, dhcpFailOverState=dhcpFailOverState, trapCommandServer=trapCommandServer, dhcpFixedIPEntry=dhcpFixedIPEntry, tftp=tftp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, tftpDaemonRunning=tftpDaemonRunning, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, dhcpStats=dhcpStats, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, adonisObjects=adonisObjects, dnsStatsRecursion=dnsStatsRecursion, dnsDaemon=dnsDaemon, trapSystemDaemon=trapSystemDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonRunning=dhcpDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, trapDHCP=trapDHCP, haService=haService, tftpDaemon=tftpDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemon=dhcpDaemon)
