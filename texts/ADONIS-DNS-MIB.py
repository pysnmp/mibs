...
iso, Counter32, MibScalar, MibTable, MibTableRow, MibTableColumn, IpAddress, NotificationType, MibIdentifier, Counter64, Integer32, ModuleIdentity, ObjectIdentity, Unsigned32, enterprises, TimeTicks, Bits, Gauge32 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Counter32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "IpAddress", "NotificationType", "MibIdentifier", "Counter64", "Integer32", "ModuleIdentity", "ObjectIdentity", "Unsigned32", "enterprises", "TimeTicks", "Bits", "Gauge32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDNS=trapDNS, dnsStatsRecursion=dnsStatsRecursion, dnsStats=dnsStats, dhcpIP=dhcpIP, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, adonis=adonis, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haReplicationBinding=haReplicationBinding, dhcpStats=dhcpStats, dhcpSubnetUsed=dhcpSubnetUsed, trapReplication=trapReplication, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, lcd=lcd, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpDaemonRunning=dhcpDaemonRunning, PYSNMP_MODULE_ID=adonis, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetTable=dhcpSubnetTable, dnsDaemon=dnsDaemon, lcdDaemon=lcdDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServerDaemon=commandServerDaemon, dnsStatsReferral=dnsStatsReferral, tftpDaemon=tftpDaemon, dhcpPoolSize=dhcpPoolSize, haServiceNodeType=haServiceNodeType, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, commandServerDaemonRunning=commandServerDaemonRunning, trapReplicationFailure=trapReplicationFailure, dnsDaemonRunning=dnsDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, ha=ha, adonisTraps=adonisTraps, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIPEntry=dhcpFixedIPEntry, haService=haService, trapSystem=trapSystem, dhcpDaemon=dhcpDaemon, licenseExpiry=licenseExpiry, adonisObjects=adonisObjects, trapCommandServer=trapCommandServer, systemState=systemState, dhcpPoolAlert=dhcpPoolAlert, haServiceRunning=haServiceRunning, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCP=trapDHCP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystemDaemon=trapSystemDaemon, trapHA=trapHA, dhcpFixedIP=dhcpFixedIP, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseTable=dhcpLeaseTable, dnsStatsFailure=dnsStatsFailure, tftp=tftp, commandServer=commandServer, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, licenseValid=licenseValid, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpSubnetEntry=dhcpSubnetEntry, dhcpSubnetSize=dhcpSubnetSize, system=system, systemDaemon=systemDaemon, dhcpPoolTable=dhcpPoolTable, dhcp=dhcp, trapDNSDaemon=trapDNSDaemon, trapTFTP=trapTFTP, dnsStatsNXRRSet=dnsStatsNXRRSet, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFailOverState=dhcpFailOverState, dns=dns, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpConfig=dhcpConfig)
