...
Bits, enterprises, Gauge32, Counter32, IpAddress, Counter64, Unsigned32, iso, ObjectIdentity, MibIdentifier, NotificationType, Integer32, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "enterprises", "Gauge32", "Counter32", "IpAddress", "Counter64", "Unsigned32", "iso", "ObjectIdentity", "MibIdentifier", "NotificationType", "Integer32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "ModuleIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemon=dhcpDaemon, lcdDaemon=lcdDaemon, haServiceNodeType=haServiceNodeType, trapDNSDaemon=trapDNSDaemon, dhcpSubnetUsed=dhcpSubnetUsed, trapDHCP=trapDHCP, tftpDaemonRunning=tftpDaemonRunning, commandServerDaemon=commandServerDaemon, commandServer=commandServer, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolSize=dhcpPoolSize, commandServerDaemonRunning=commandServerDaemonRunning, licenseExpiry=licenseExpiry, dhcpFailOverState=dhcpFailOverState, dhcpSubnetAlert=dhcpSubnetAlert, dnsStatsFailure=dnsStatsFailure, dhcpPoolEndIP=dhcpPoolEndIP, systemDaemon=systemDaemon, trapSystemDaemon=trapSystemDaemon, dhcpSubnetSize=dhcpSubnetSize, trapDNS=trapDNS, tftp=tftp, dnsStats=dnsStats, adonisTraps=adonisTraps, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolAlert=dhcpPoolAlert, dhcpLeaseBindState=dhcpLeaseBindState, dhcpIP=dhcpIP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHostname=dhcpLeaseHostname, trapCommandServer=trapCommandServer, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, system=system, dhcpSubnetMask=dhcpSubnetMask, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTP=trapTFTP, tftpDaemon=tftpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseEntry=dhcpLeaseEntry, trapSystem=trapSystem, haServiceRunning=haServiceRunning, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcp=dhcp, dhcpFixedIPEntry=dhcpFixedIPEntry, systemState=systemState, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, licenseValid=licenseValid, dnsDaemonRunning=dnsDaemonRunning, dhcpFixedIP=dhcpFixedIP, trapReplication=trapReplication, dhcpPoolStartIP=dhcpPoolStartIP, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemon=dnsDaemon, PYSNMP_MODULE_ID=adonis, ha=ha, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpStats=dhcpStats, trapReplicationFailure=trapReplicationFailure, adonis=adonis, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapHA=trapHA, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetEntry=dhcpSubnetEntry, lcd=lcd, dhcpPoolUsed=dhcpPoolUsed, adonisObjects=adonisObjects, dhcpSubnetIP=dhcpSubnetIP, dhcpConfig=dhcpConfig, dhcpPoolTable=dhcpPoolTable, dns=dns, haService=haService, haReplicationBinding=haReplicationBinding, trapDHCPDaemon=trapDHCPDaemon, dnsStatsReferral=dnsStatsReferral, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapCommandServerDaemon=trapCommandServerDaemon)
