...
Integer32, Gauge32, TimeTicks, NotificationType, Counter32, IpAddress, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, iso, ModuleIdentity, Unsigned32, Counter64, Bits, ObjectIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Gauge32", "TimeTicks", "NotificationType", "Counter32", "IpAddress", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "iso", "ModuleIdentity", "Unsigned32", "Counter64", "Bits", "ObjectIdentity")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseTable=dhcpLeaseTable, dnsStatsSuccess=dnsStatsSuccess, haServiceNodeType=haServiceNodeType, licenseValid=licenseValid, trapReplicationFailure=trapReplicationFailure, commandServer=commandServer, lcd=lcd, trapHA=trapHA, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXRRSet=dnsStatsNXRRSet, system=system, dhcpLeaseEndTime=dhcpLeaseEndTime, haService=haService, dnsDaemonRunning=dnsDaemonRunning, dhcp=dhcp, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolSize=dhcpPoolSize, dhcpLeaseEntry=dhcpLeaseEntry, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseBindState=dhcpLeaseBindState, trapHAServiceFailOver=trapHAServiceFailOver, haServiceRunning=haServiceRunning, dnsStats=dnsStats, ha=ha, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsRecursion=dnsStatsRecursion, trapDNSDaemon=trapDNSDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpStats=dhcpStats, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonis=adonis, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, tftpDaemon=tftpDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolEntry=dhcpPoolEntry, licenseExpiry=licenseExpiry, commandServerDaemon=commandServerDaemon, trapTFTP=trapTFTP, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemon=dnsDaemon, haReplicationBinding=haReplicationBinding, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, PYSNMP_MODULE_ID=adonis, dhcpDaemon=dhcpDaemon, trapSystem=trapSystem, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolAlert=dhcpPoolAlert, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolStartIP=dhcpPoolStartIP, dns=dns, dhcpPoolUsed=dhcpPoolUsed, dhcpConfig=dhcpConfig, dhcpSubnetUsed=dhcpSubnetUsed, trapDNS=trapDNS, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDHCP=trapDHCP, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseStartTime=dhcpLeaseStartTime, trapReplication=trapReplication, adonisObjects=adonisObjects, trapCommandServer=trapCommandServer, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsReferral=dnsStatsReferral, dhcpSubnetEntry=dhcpSubnetEntry, adonisTraps=adonisTraps, systemState=systemState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, tftpDaemonRunning=tftpDaemonRunning, systemDaemon=systemDaemon, dhcpSubnetSize=dhcpSubnetSize, tftp=tftp, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIP=dhcpFixedIP, dnsStatsFailure=dnsStatsFailure)
