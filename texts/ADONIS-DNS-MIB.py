...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonDebugLevel=dnsDaemonDebugLevel, systemDaemon=systemDaemon, dnsDaemonRunning=dnsDaemonRunning, dnsStats=dnsStats, dns=dns, dhcpSubnetTable=dhcpSubnetTable, trapReplicationFailure=trapReplicationFailure, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, commandServer=commandServer, adonisObjects=adonisObjects, dhcpLeaseStartTime=dhcpLeaseStartTime, trapTFTPDaemon=trapTFTPDaemon, PYSNMP_MODULE_ID=adonis, dhcpDaemon=dhcpDaemon, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, tftp=tftp, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetMask=dhcpSubnetMask, dnsStatsFailure=dnsStatsFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetUsed=dhcpSubnetUsed, licenseValid=licenseValid, dhcpPoolSize=dhcpPoolSize, dhcpLeaseBindState=dhcpLeaseBindState, dhcp=dhcp, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolStartIP=dhcpPoolStartIP, trapDNSDaemon=trapDNSDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, commandServerDaemon=commandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetEntry=dhcpSubnetEntry, haServiceRunning=haServiceRunning, dnsStatsSuccess=dnsStatsSuccess, dnsStatsNXDomain=dnsStatsNXDomain, dhcpConfig=dhcpConfig, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCP=trapDHCP, trapHA=trapHA, dhcpPoolUsed=dhcpPoolUsed, system=system, dhcpFailOverState=dhcpFailOverState, trapReplication=trapReplication, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, ha=ha, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, dhcpFixedIP=dhcpFixedIP, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServer=trapCommandServer, dhcpLeaseEntry=dhcpLeaseEntry, adonis=adonis, commandServerDaemonRunning=commandServerDaemonRunning, trapSystem=trapSystem, dnsStatsReferral=dnsStatsReferral, tftpDaemonRunning=tftpDaemonRunning, dhcpDaemonRunning=dhcpDaemonRunning, haServiceNodeType=haServiceNodeType, systemState=systemState, trapTFTP=trapTFTP, lcdDaemon=lcdDaemon, tftpDaemon=tftpDaemon, dhcpSubnetIP=dhcpSubnetIP, dnsDaemon=dnsDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, lcd=lcd, dhcpIP=dhcpIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, licenseExpiry=licenseExpiry, dhcpPoolEntry=dhcpPoolEntry, adonisTraps=adonisTraps, dhcpSubnetSize=dhcpSubnetSize, haReplicationBinding=haReplicationBinding, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolAlert=dhcpPoolAlert, trapDNS=trapDNS, dhcpStats=dhcpStats, dhcpPoolEndIP=dhcpPoolEndIP, haService=haService)
