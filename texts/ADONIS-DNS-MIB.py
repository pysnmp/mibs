...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsSuccess=dnsStatsSuccess, trapReplication=trapReplication, dnsStatsReferral=dnsStatsReferral, adonisObjects=adonisObjects, trapReplicationFailure=trapReplicationFailure, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolEndIP=dhcpPoolEndIP, commandServerDaemonRunning=commandServerDaemonRunning, PYSNMP_MODULE_ID=adonis, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, haServiceRunning=haServiceRunning, trapSystem=trapSystem, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolUsed=dhcpPoolUsed, trapDHCP=trapDHCP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNS=trapDNS, trapTFTP=trapTFTP, dhcpConfig=dhcpConfig, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseEntry=dhcpLeaseEntry, dhcpStats=dhcpStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapHA=trapHA, haReplicationBinding=haReplicationBinding, tftp=tftp, licenseExpiry=licenseExpiry, commandServerDaemon=commandServerDaemon, lcd=lcd, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, haService=haService, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetIP=dhcpSubnetIP, systemState=systemState, dhcpLeaseBindState=dhcpLeaseBindState, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpSubnetMask=dhcpSubnetMask, dhcpPoolTable=dhcpPoolTable, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetEntry=dhcpSubnetEntry, trapSystemDaemon=trapSystemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dns=dns, commandServer=commandServer, adonis=adonis, dnsStats=dnsStats, trapTFTPDaemon=trapTFTPDaemon, trapDNSDaemon=trapDNSDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, tftpDaemon=tftpDaemon, dhcpDaemon=dhcpDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapHAServiceFailOver=trapHAServiceFailOver, ha=ha, dhcpIP=dhcpIP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolStartIP=dhcpPoolStartIP, haServiceNodeType=haServiceNodeType, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, systemDaemon=systemDaemon, lcdDaemon=lcdDaemon, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFailOverState=dhcpFailOverState, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIP=dhcpFixedIP, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIPTable=dhcpFixedIPTable, adonisTraps=adonisTraps, licenseValid=licenseValid, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseTable=dhcpLeaseTable, dnsDaemon=dnsDaemon, dhcp=dhcp, system=system, dnsDaemonRunning=dnsDaemonRunning, trapCommandServer=trapCommandServer, dnsStatsNXRRSet=dnsStatsNXRRSet)
