...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServerDaemon=trapCommandServerDaemon, dhcpStats=dhcpStats, dnsStatsSuccess=dnsStatsSuccess, haServiceNodeType=haServiceNodeType, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolSize=dhcpPoolSize, dhcpSubnetMask=dhcpSubnetMask, dhcpConfig=dhcpConfig, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDHCP=trapDHCP, tftpDaemon=tftpDaemon, tftp=tftp, dhcpLeaseBindState=dhcpLeaseBindState, systemDaemon=systemDaemon, licenseValid=licenseValid, dnsStatsNXRRSet=dnsStatsNXRRSet, trapHA=trapHA, trapDNS=trapDNS, dnsDaemon=dnsDaemon, dhcpFixedIP=dhcpFixedIP, trapTFTP=trapTFTP, dnsStats=dnsStats, trapTFTPDaemon=trapTFTPDaemon, dnsStatsFailure=dnsStatsFailure, dhcpLeaseHostname=dhcpLeaseHostname, commandServer=commandServer, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, systemState=systemState, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemonRunning=commandServerDaemonRunning, adonis=adonis, dhcpPoolEntry=dhcpPoolEntry, dhcpFailOverState=dhcpFailOverState, ha=ha, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haReplicationBinding=haReplicationBinding, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpIP=dhcpIP, trapReplication=trapReplication, dhcpSubnetIP=dhcpSubnetIP, dnsStatsReferral=dnsStatsReferral, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetTable=dhcpSubnetTable, haServiceRunning=haServiceRunning, dns=dns, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, licenseExpiry=licenseExpiry, trapDNSDaemon=trapDNSDaemon, tftpDaemonRunning=tftpDaemonRunning, commandServerDaemon=commandServerDaemon, dhcpSubnetSize=dhcpSubnetSize, adonisTraps=adonisTraps, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolTable=dhcpPoolTable, dhcpLeaseEntry=dhcpLeaseEntry, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseStartTime=dhcpLeaseStartTime, lcdDaemon=lcdDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, system=system, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpDaemon=dhcpDaemon, dhcpPoolStartIP=dhcpPoolStartIP, trapHAServiceFailOver=trapHAServiceFailOver, adonisObjects=adonisObjects, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDHCPDaemon=trapDHCPDaemon, dnsStatsNXDomain=dnsStatsNXDomain, PYSNMP_MODULE_ID=adonis, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapSystem=trapSystem, dhcpPoolEndIP=dhcpPoolEndIP, lcd=lcd, dhcpSubnetUsed=dhcpSubnetUsed, trapReplicationFailure=trapReplicationFailure, trapSystemDaemon=trapSystemDaemon, haService=haService, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpDaemonRunning=dhcpDaemonRunning, trapCommandServer=trapCommandServer, dhcp=dhcp, dhcpLeaseEndTime=dhcpLeaseEndTime)
