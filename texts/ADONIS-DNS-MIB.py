...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServerDaemon=commandServerDaemon, dhcpSubnetTable=dhcpSubnetTable, trapTFTPDaemon=trapTFTPDaemon, dhcpFixedIPTable=dhcpFixedIPTable, dns=dns, dhcpConfig=dhcpConfig, dhcpLeaseEntry=dhcpLeaseEntry, dhcpIP=dhcpIP, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsFailure=dnsStatsFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapTFTP=trapTFTP, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFailOverState=dhcpFailOverState, lcd=lcd, dnsStatsReferral=dnsStatsReferral, haServiceRunning=haServiceRunning, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpFixedIP=dhcpFixedIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, trapDNS=trapDNS, trapSystem=trapSystem, dhcpDaemon=dhcpDaemon, trapHA=trapHA, adonis=adonis, dnsStats=dnsStats, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsRecursion=dnsStatsRecursion, dhcp=dhcp, dhcpPoolTable=dhcpPoolTable, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseBindState=dhcpLeaseBindState, system=system, trapReplication=trapReplication, PYSNMP_MODULE_ID=adonis, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseHostname=dhcpLeaseHostname, tftp=tftp, trapHAServiceFailOver=trapHAServiceFailOver, adonisTraps=adonisTraps, dhcpSubnetSize=dhcpSubnetSize, lcdDaemon=lcdDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemon=dnsDaemon, adonisObjects=adonisObjects, trapDHCP=trapDHCP, trapDHCPDaemon=trapDHCPDaemon, trapSystemDaemon=trapSystemDaemon, ha=ha, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpPoolSize=dhcpPoolSize, trapCommandServerDaemon=trapCommandServerDaemon, licenseExpiry=licenseExpiry, systemState=systemState, haReplicationBinding=haReplicationBinding, dhcpPoolStartIP=dhcpPoolStartIP, trapDNSDaemon=trapDNSDaemon, haService=haService, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetAlert=dhcpSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolUsed=dhcpPoolUsed, tftpDaemon=tftpDaemon, trapReplicationFailure=trapReplicationFailure, haServiceNodeType=haServiceNodeType, dhcpPoolAlert=dhcpPoolAlert, dnsStatsSuccess=dnsStatsSuccess, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, licenseValid=licenseValid, trapCommandServer=trapCommandServer, systemDaemon=systemDaemon, dhcpSubnetIP=dhcpSubnetIP, commandServer=commandServer, dhcpStats=dhcpStats, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress)
