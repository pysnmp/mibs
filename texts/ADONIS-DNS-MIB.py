...
Integer32, Gauge32, Counter64, NotificationType, IpAddress, ObjectIdentity, Bits, Counter32, Unsigned32, ModuleIdentity, iso, enterprises, TimeTicks, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Gauge32", "Counter64", "NotificationType", "IpAddress", "ObjectIdentity", "Bits", "Counter32", "Unsigned32", "ModuleIdentity", "iso", "enterprises", "TimeTicks", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapDHCPDaemon=trapDHCPDaemon, tftpDaemon=tftpDaemon, dhcpStats=dhcpStats, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolEntry=dhcpPoolEntry, dhcpIP=dhcpIP, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemon=dnsDaemon, dhcpLeaseTable=dhcpLeaseTable, dnsStatsRecursion=dnsStatsRecursion, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolSize=dhcpPoolSize, trapSystemDaemon=trapSystemDaemon, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEndIP=dhcpPoolEndIP, commandServer=commandServer, PYSNMP_MODULE_ID=adonis, dhcpLeaseEntry=dhcpLeaseEntry, trapHA=trapHA, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonRunning=dhcpDaemonRunning, adonisTraps=adonisTraps, tftpDaemonRunning=tftpDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolUsed=dhcpPoolUsed, licenseValid=licenseValid, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetMask=dhcpSubnetMask, dnsStatsFailure=dnsStatsFailure, trapReplication=trapReplication, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseBindState=dhcpLeaseBindState, dhcpConfig=dhcpConfig, adonis=adonis, dhcpPoolAlert=dhcpPoolAlert, trapReplicationFailure=trapReplicationFailure, licenseExpiry=licenseExpiry, lcd=lcd, ha=ha, dnsStatsReferral=dnsStatsReferral, trapDNSDaemon=trapDNSDaemon, dhcpPoolTable=dhcpPoolTable, dhcpDaemon=dhcpDaemon, haServiceNodeType=haServiceNodeType, trapCommandServer=trapCommandServer, commandServerDaemonRunning=commandServerDaemonRunning, trapTFTP=trapTFTP, dhcpSubnetUsed=dhcpSubnetUsed, dns=dns, lcdDaemon=lcdDaemon, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, systemState=systemState, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsNXDomain=dnsStatsNXDomain, tftp=tftp, haService=haService, system=system, dhcpFixedIPTable=dhcpFixedIPTable, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIP=dhcpFixedIP, dnsStats=dnsStats, dhcpFailOverState=dhcpFailOverState, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceRunning=haServiceRunning, dhcp=dhcp, trapDHCP=trapDHCP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetAlert=dhcpSubnetAlert, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, systemDaemon=systemDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapSystem=trapSystem, trapDNS=trapDNS, haReplicationBinding=haReplicationBinding, adonisObjects=adonisObjects, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, commandServerDaemon=commandServerDaemon)
