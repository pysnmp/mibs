...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", licenseExpiry=licenseExpiry, system=system, adonisObjects=adonisObjects, haReplicationBinding=haReplicationBinding, adonisTraps=adonisTraps, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDNSDaemon=trapDNSDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, PYSNMP_MODULE_ID=adonis, dhcpLeaseStartTime=dhcpLeaseStartTime, commandServerDaemonRunning=commandServerDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, trapSystemDaemon=trapSystemDaemon, licenseValid=licenseValid, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftp=tftp, trapSystem=trapSystem, haService=haService, trapDNS=trapDNS, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, trapTFTPDaemon=trapTFTPDaemon, dnsDaemon=dnsDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsNXRRSet=dnsStatsNXRRSet, tftpDaemon=tftpDaemon, dhcpIP=dhcpIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, dhcpFailOverState=dhcpFailOverState, ha=ha, haServiceNodeType=haServiceNodeType, dhcpSubnetUsed=dhcpSubnetUsed, commandServer=commandServer, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsSuccess=dnsStatsSuccess, haServiceRunning=haServiceRunning, dhcpPoolAlert=dhcpPoolAlert, trapHA=trapHA, systemDaemon=systemDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dns=dns, dhcp=dhcp, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseEntry=dhcpLeaseEntry, trapReplicationFailure=trapReplicationFailure, adonis=adonis, trapDHCP=trapDHCP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolEntry=dhcpPoolEntry, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, lcdDaemon=lcdDaemon, dhcpDaemon=dhcpDaemon, trapReplication=trapReplication, trapCommandServer=trapCommandServer, dhcpPoolTable=dhcpPoolTable, trapDHCPDaemon=trapDHCPDaemon, systemState=systemState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolSize=dhcpPoolSize, dnsStats=dnsStats, commandServerDaemon=commandServerDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseTable=dhcpLeaseTable, dnsStatsReferral=dnsStatsReferral, dhcpStats=dhcpStats, dhcpPoolUsed=dhcpPoolUsed, dnsStatsFailure=dnsStatsFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetTable=dhcpSubnetTable, dhcpFixedIP=dhcpFixedIP, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapTFTP=trapTFTP, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpConfig=dhcpConfig, lcd=lcd, dhcpSubnetSize=dhcpSubnetSize)
