...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolStartIP=dhcpPoolStartIP, adonisTraps=adonisTraps, trapDNS=trapDNS, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetUsed=dhcpSubnetUsed, adonis=adonis, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseStartTime=dhcpLeaseStartTime, licenseExpiry=licenseExpiry, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolUsed=dhcpPoolUsed, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemon=dnsDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpStats=dhcpStats, haServiceRunning=haServiceRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpFixedIPEntry=dhcpFixedIPEntry, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsReferral=dnsStatsReferral, trapCommandServer=trapCommandServer, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystem=trapSystem, dhcpIP=dhcpIP, trapSystemDaemon=trapSystemDaemon, trapReplication=trapReplication, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, ha=ha, lcdDaemon=lcdDaemon, trapReplicationFailure=trapReplicationFailure, dhcpFixedIPTable=dhcpFixedIPTable, trapDNSDaemon=trapDNSDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, licenseValid=licenseValid, system=system, dnsDaemonRunning=dnsDaemonRunning, tftpDaemonRunning=tftpDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetIP=dhcpSubnetIP, trapTFTPDaemon=trapTFTPDaemon, trapHA=trapHA, dhcpConfig=dhcpConfig, dhcpDaemon=dhcpDaemon, systemState=systemState, dhcpLeaseEntry=dhcpLeaseEntry, dhcpDaemonRunning=dhcpDaemonRunning, commandServerDaemon=commandServerDaemon, haService=haService, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, lcd=lcd, commandServer=commandServer, dhcpLeaseBindState=dhcpLeaseBindState, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapHAServiceFailOver=trapHAServiceFailOver, dhcp=dhcp, dhcpLeaseHostname=dhcpLeaseHostname, trapTFTP=trapTFTP, dhcpFailOverState=dhcpFailOverState, haReplicationBinding=haReplicationBinding, dhcpPoolTable=dhcpPoolTable, dns=dns, adonisObjects=adonisObjects, dnsStatsRecursion=dnsStatsRecursion, dhcpPoolEntry=dhcpPoolEntry, dhcpPoolSize=dhcpPoolSize, haServiceNodeType=haServiceNodeType, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetMask=dhcpSubnetMask, systemDaemon=systemDaemon, tftpDaemon=tftpDaemon, dnsStats=dnsStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolAlert=dhcpPoolAlert, PYSNMP_MODULE_ID=adonis, trapDHCP=trapDHCP, dnsStatsFailure=dnsStatsFailure, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIP=dhcpFixedIP, dhcpSubnetEntry=dhcpSubnetEntry, tftp=tftp)
