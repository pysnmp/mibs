...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpLeaseBindState=dhcpLeaseBindState, trapReplication=trapReplication, commandServer=commandServer, systemDaemon=systemDaemon, dhcpPoolTable=dhcpPoolTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonis=adonis, dhcpStats=dhcpStats, dhcpLeaseEndTime=dhcpLeaseEndTime, trapDNSDaemon=trapDNSDaemon, dhcpFailOverState=dhcpFailOverState, trapSystem=trapSystem, haService=haService, trapTFTP=trapTFTP, trapDNS=trapDNS, trapCommandServer=trapCommandServer, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseEntry=dhcpLeaseEntry, dns=dns, dhcpLeaseTable=dhcpLeaseTable, trapTFTPDaemon=trapTFTPDaemon, lcdDaemon=lcdDaemon, haReplicationBinding=haReplicationBinding, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolEndIP=dhcpPoolEndIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServerDaemon=commandServerDaemon, dnsStatsRecursion=dnsStatsRecursion, adonisTraps=adonisTraps, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetIP=dhcpSubnetIP, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceRunning=haServiceRunning, trapHA=trapHA, dhcpPoolAlert=dhcpPoolAlert, dnsDaemon=dnsDaemon, dhcpFixedIP=dhcpFixedIP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dnsStats=dnsStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, haServiceNodeType=haServiceNodeType, adonisObjects=adonisObjects, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, licenseExpiry=licenseExpiry, commandServerDaemonRunning=commandServerDaemonRunning, system=system, licenseValid=licenseValid, dhcp=dhcp, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsNXRRSet=dnsStatsNXRRSet, trapDHCPDaemon=trapDHCPDaemon, PYSNMP_MODULE_ID=adonis, systemState=systemState, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, tftp=tftp, dhcpIP=dhcpIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolUsed=dhcpPoolUsed, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsReferral=dnsStatsReferral, trapCommandServerDaemon=trapCommandServerDaemon, dhcpPoolSize=dhcpPoolSize, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, lcd=lcd, dhcpSubnetEntry=dhcpSubnetEntry, dhcpConfig=dhcpConfig, dhcpSubnetSize=dhcpSubnetSize, trapReplicationFailure=trapReplicationFailure, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetTable=dhcpSubnetTable, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCP=trapDHCP, dnsStatsFailure=dnsStatsFailure, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetAlert=dhcpSubnetAlert, trapSystemDaemon=trapSystemDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolEntry=dhcpPoolEntry, ha=ha)
