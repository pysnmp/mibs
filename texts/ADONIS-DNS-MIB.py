...
iso, ObjectIdentity, Gauge32, Counter64, ModuleIdentity, MibIdentifier, NotificationType, IpAddress, Counter32, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, enterprises, Unsigned32, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "ObjectIdentity", "Gauge32", "Counter64", "ModuleIdentity", "MibIdentifier", "NotificationType", "IpAddress", "Counter32", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "enterprises", "Unsigned32", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", haService=haService, systemDaemon=systemDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dns=dns, lcd=lcd, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsFailure=dnsStatsFailure, trapTFTPDaemon=trapTFTPDaemon, trapReplicationFailure=trapReplicationFailure, ha=ha, adonisTraps=adonisTraps, dnsStatsReferral=dnsStatsReferral, dnsStatsNXDomain=dnsStatsNXDomain, adonisObjects=adonisObjects, tftpDaemon=tftpDaemon, dhcpFailOverState=dhcpFailOverState, dhcpPoolTable=dhcpPoolTable, trapHAServiceFailOver=trapHAServiceFailOver, systemState=systemState, tftpDaemonRunning=tftpDaemonRunning, dhcpStats=dhcpStats, dhcpDaemon=dhcpDaemon, adonis=adonis, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetTable=dhcpSubnetTable, dhcpLeaseBindState=dhcpLeaseBindState, dhcpLeaseEntry=dhcpLeaseEntry, trapSystemDaemon=trapSystemDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetIP=dhcpSubnetIP, licenseValid=licenseValid, dhcpPoolUsed=dhcpPoolUsed, dhcpFixedIP=dhcpFixedIP, trapReplication=trapReplication, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, system=system, trapHA=trapHA, licenseExpiry=licenseExpiry, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDNSDaemon=trapDNSDaemon, haServiceRunning=haServiceRunning, trapCommandServer=trapCommandServer, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapSystem=trapSystem, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetEntry=dhcpSubnetEntry, trapDHCP=trapDHCP, dnsDaemon=dnsDaemon, dnsStats=dnsStats, lcdDaemon=lcdDaemon, dnsStatsRecursion=dnsStatsRecursion, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftp=tftp, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServerDaemonRunning=commandServerDaemonRunning, haReplicationBinding=haReplicationBinding, dnsStatsSuccess=dnsStatsSuccess, commandServerDaemon=commandServerDaemon, dhcp=dhcp, dhcpConfig=dhcpConfig, dhcpIP=dhcpIP, dhcpSubnetAlert=dhcpSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonRunning=dhcpDaemonRunning, PYSNMP_MODULE_ID=adonis, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNS=trapDNS, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSize=dhcpPoolSize, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpPoolStartIP=dhcpPoolStartIP, commandServer=commandServer, trapTFTP=trapTFTP, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolEntry=dhcpPoolEntry, haServiceNodeType=haServiceNodeType)
