...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpPoolEndIP=dhcpPoolEndIP, trapReplication=trapReplication, dnsStatsSuccess=dnsStatsSuccess, licenseValid=licenseValid, dhcpPoolTable=dhcpPoolTable, tftp=tftp, systemState=systemState, haServiceRunning=haServiceRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTP=trapTFTP, trapDHCPDaemon=trapDHCPDaemon, PYSNMP_MODULE_ID=adonis, trapReplicationFailure=trapReplicationFailure, dnsStatsRecursion=dnsStatsRecursion, dnsStatsNXRRSet=dnsStatsNXRRSet, haService=haService, dhcpFixedIP=dhcpFixedIP, dhcpStats=dhcpStats, ha=ha, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, trapDNS=trapDNS, commandServer=commandServer, dnsStatsFailure=dnsStatsFailure, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, system=system, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetIP=dhcpSubnetIP, dhcpFailOverState=dhcpFailOverState, trapHA=trapHA, dhcpPoolSize=dhcpPoolSize, dhcpFixedIPTable=dhcpFixedIPTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpConfig=dhcpConfig, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, trapSystem=trapSystem, licenseExpiry=licenseExpiry, dhcpPoolStartIP=dhcpPoolStartIP, haReplicationBinding=haReplicationBinding, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseEntry=dhcpLeaseEntry, lcd=lcd, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapSystemDaemon=trapSystemDaemon, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpDaemon=dhcpDaemon, lcdDaemon=lcdDaemon, trapCommandServer=trapCommandServer, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServerDaemon=commandServerDaemon, tftpDaemon=tftpDaemon, dnsStats=dnsStats, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpSubnetSize=dhcpSubnetSize, haServiceNodeType=haServiceNodeType, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCP=trapDHCP, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXDomain=dnsStatsNXDomain, trapDNSDaemon=trapDNSDaemon, tftpDaemonRunning=tftpDaemonRunning, adonisTraps=adonisTraps, dnsDaemon=dnsDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolUsed=dhcpPoolUsed, adonisObjects=adonisObjects, dhcp=dhcp, dhcpSubnetMask=dhcpSubnetMask, dhcpIP=dhcpIP, adonis=adonis, trapTFTPDaemon=trapTFTPDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsStatsReferral=dnsStatsReferral, systemDaemon=systemDaemon, dns=dns)
