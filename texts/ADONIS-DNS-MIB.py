...
ModuleIdentity, Counter64, Integer32, NotificationType, MibIdentifier, enterprises, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, IpAddress, TimeTicks, Bits, Gauge32, Unsigned32, ObjectIdentity, iso = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Counter64", "Integer32", "NotificationType", "MibIdentifier", "enterprises", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "IpAddress", "TimeTicks", "Bits", "Gauge32", "Unsigned32", "ObjectIdentity", "iso")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFixedIPEntry=dhcpFixedIPEntry, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIP=dhcpFixedIP, dhcpFailOverState=dhcpFailOverState, trapHAServiceFailOver=trapHAServiceFailOver, trapTFTP=trapTFTP, dns=dns, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpPoolUsed=dhcpPoolUsed, lcdDaemon=lcdDaemon, dhcp=dhcp, dhcpPoolEndIP=dhcpPoolEndIP, licenseExpiry=licenseExpiry, trapTFTPDaemon=trapTFTPDaemon, tftp=tftp, trapSystem=trapSystem, trapCommandServerDaemon=trapCommandServerDaemon, haServiceNodeType=haServiceNodeType, ha=ha, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStats=dnsStats, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetAlert=dhcpSubnetAlert, haService=haService, trapCommandServer=trapCommandServer, trapDNSDaemon=trapDNSDaemon, systemDaemon=systemDaemon, dhcpPoolEntry=dhcpPoolEntry, trapSystemDaemon=trapSystemDaemon, lcd=lcd, trapDNS=trapDNS, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsRecursion=dnsStatsRecursion, dhcpFixedIPTable=dhcpFixedIPTable, adonisObjects=adonisObjects, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceRunning=haServiceRunning, PYSNMP_MODULE_ID=adonis, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSize=dhcpPoolSize, dnsStatsReferral=dnsStatsReferral, dhcpLeaseTable=dhcpLeaseTable, dhcpLeaseEntry=dhcpLeaseEntry, trapHA=trapHA, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHostname=dhcpLeaseHostname, dhcpConfig=dhcpConfig, tftpDaemon=tftpDaemon, dhcpStats=dhcpStats, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapReplication=trapReplication, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDHCP=trapDHCP, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDHCPDaemon=trapDHCPDaemon, dhcpIP=dhcpIP, haReplicationBinding=haReplicationBinding, dnsStatsNXRRSet=dnsStatsNXRRSet, commandServerDaemon=commandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, trapReplicationFailure=trapReplicationFailure, dhcpPoolTable=dhcpPoolTable, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemonRunning=tftpDaemonRunning, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemon=dhcpDaemon, dnsStatsFailure=dnsStatsFailure, commandServer=commandServer, dhcpSubnetIP=dhcpSubnetIP, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonRunning=dnsDaemonRunning, licenseValid=licenseValid, dhcpLeaseBindState=dhcpLeaseBindState, adonis=adonis, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetMask=dhcpSubnetMask, system=system, adonisTraps=adonisTraps, dhcpPoolAlert=dhcpPoolAlert, dnsDaemon=dnsDaemon, dhcpPoolStartIP=dhcpPoolStartIP, systemState=systemState)
