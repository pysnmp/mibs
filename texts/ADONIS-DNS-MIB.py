...
Counter32, MibIdentifier, IpAddress, NotificationType, iso, Gauge32, Unsigned32, enterprises, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, TimeTicks, ObjectIdentity, Integer32, Counter64, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "MibIdentifier", "IpAddress", "NotificationType", "iso", "Gauge32", "Unsigned32", "enterprises", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "TimeTicks", "ObjectIdentity", "Integer32", "Counter64", "Bits")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcp=dhcp, dhcpSubnetUsed=dhcpSubnetUsed, trapSystem=trapSystem, dhcpFailOverState=dhcpFailOverState, dhcpFixedIP=dhcpFixedIP, dhcpPoolEntry=dhcpPoolEntry, trapDNSDaemon=trapDNSDaemon, dhcpSubnetIP=dhcpSubnetIP, tftpDaemonRunning=tftpDaemonRunning, dns=dns, adonis=adonis, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, lcdDaemon=lcdDaemon, systemDaemon=systemDaemon, dnsDaemonRunning=dnsDaemonRunning, trapDHCP=trapDHCP, system=system, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisTraps=adonisTraps, dhcpStats=dhcpStats, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsStatsNXDomain=dnsStatsNXDomain, trapReplicationFailure=trapReplicationFailure, trapReplication=trapReplication, dhcpPoolTable=dhcpPoolTable, trapDNS=trapDNS, ha=ha, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, lcd=lcd, dnsStatsReferral=dnsStatsReferral, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpConfig=dhcpConfig, commandServerDaemon=commandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, PYSNMP_MODULE_ID=adonis, dnsDaemon=dnsDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpLeaseTable=dhcpLeaseTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapDHCPDaemon=trapDHCPDaemon, dnsStatsSuccess=dnsStatsSuccess, commandServer=commandServer, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetAlert=dhcpSubnetAlert, haServiceRunning=haServiceRunning, haService=haService, trapHAServiceFailOver=trapHAServiceFailOver, trapSystemDaemon=trapSystemDaemon, trapTFTP=trapTFTP, trapCommandServer=trapCommandServer, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseHostname=dhcpLeaseHostname, dhcpIP=dhcpIP, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetEntry=dhcpSubnetEntry, haReplicationBinding=haReplicationBinding, dhcpPoolSize=dhcpPoolSize, systemState=systemState, dhcpDaemon=dhcpDaemon, dhcpSubnetSize=dhcpSubnetSize, dhcpFixedIPTable=dhcpFixedIPTable, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolStartIP=dhcpPoolStartIP, adonisObjects=adonisObjects, dhcpSubnetMask=dhcpSubnetMask, dhcpDaemonRunning=dhcpDaemonRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpPoolUsed=dhcpPoolUsed, trapHA=trapHA, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsFailure=dnsStatsFailure, haServiceNodeType=haServiceNodeType, dhcpLeaseBindState=dhcpLeaseBindState, licenseValid=licenseValid, licenseExpiry=licenseExpiry, dnsStats=dnsStats, dnsStatsRecursion=dnsStatsRecursion, tftpDaemon=tftpDaemon, tftp=tftp)
