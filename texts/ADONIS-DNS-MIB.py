...
Counter32, Integer32, ModuleIdentity, Bits, Counter64, TimeTicks, IpAddress, ObjectIdentity, MibIdentifier, enterprises, Unsigned32, NotificationType, Gauge32, iso, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Counter32", "Integer32", "ModuleIdentity", "Bits", "Counter64", "TimeTicks", "IpAddress", "ObjectIdentity", "MibIdentifier", "enterprises", "Unsigned32", "NotificationType", "Gauge32", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpFailOverState=dhcpFailOverState, tftp=tftp, systemState=systemState, haService=haService, adonisObjects=adonisObjects, dhcpPoolSubnetIP=dhcpPoolSubnetIP, trapTFTP=trapTFTP, trapReplicationFailure=trapReplicationFailure, haReplicationBinding=haReplicationBinding, trapTFTPDaemon=trapTFTPDaemon, systemDaemon=systemDaemon, dhcpPoolEndIP=dhcpPoolEndIP, lcdDaemon=lcdDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStats=dnsStats, trapReplication=trapReplication, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemon=dnsDaemon, dhcpConfig=dhcpConfig, system=system, dhcpSubnetIP=dhcpSubnetIP, trapDNSDaemon=trapDNSDaemon, licenseValid=licenseValid, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsRecursion=dnsStatsRecursion, ha=ha, dhcpFixedIPTable=dhcpFixedIPTable, dnsDaemonDebugLevel=dnsDaemonDebugLevel, lcd=lcd, adonisTraps=adonisTraps, licenseExpiry=licenseExpiry, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolAlert=dhcpPoolAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapDNS=trapDNS, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolTable=dhcpPoolTable, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemon=dhcpDaemon, trapDHCP=trapDHCP, dhcpIP=dhcpIP, adonis=adonis, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftpDaemon=tftpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetTable=dhcpSubnetTable, dhcpSubnetMask=dhcpSubnetMask, haServiceNodeType=haServiceNodeType, dhcpPoolSize=dhcpPoolSize, trapSystem=trapSystem, dnsStatsReferral=dnsStatsReferral, dhcpFixedIP=dhcpFixedIP, trapDHCPDaemon=trapDHCPDaemon, trapHA=trapHA, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolUsed=dhcpPoolUsed, trapCommandServerDaemon=trapCommandServerDaemon, dhcp=dhcp, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, haServiceRunning=haServiceRunning, dhcpLeaseTable=dhcpLeaseTable, dhcpStats=dhcpStats, commandServer=commandServer, trapCommandServer=trapCommandServer, dhcpPoolEntry=dhcpPoolEntry, dnsStatsFailure=dnsStatsFailure, dns=dns, dhcpLeaseHostname=dhcpLeaseHostname, trapSystemDaemon=trapSystemDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonRunning=dnsDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, commandServerDaemon=commandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, PYSNMP_MODULE_ID=adonis, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseEndTime=dhcpLeaseEndTime, commandServerDaemonRunning=commandServerDaemonRunning)
