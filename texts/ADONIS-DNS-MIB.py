...
Bits, IpAddress, ModuleIdentity, Integer32, Unsigned32, ObjectIdentity, Gauge32, Counter64, iso, Counter32, enterprises, TimeTicks, NotificationType, MibIdentifier, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "Bits", "IpAddress", "ModuleIdentity", "Integer32", "Unsigned32", "ObjectIdentity", "Gauge32", "Counter64", "iso", "Counter32", "enterprises", "TimeTicks", "NotificationType", "MibIdentifier", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", licenseValid=licenseValid, adonis=adonis, dhcpSubnetSize=dhcpSubnetSize, licenseExpiry=licenseExpiry, dhcpLeaseEntry=dhcpLeaseEntry, trapReplicationFailure=trapReplicationFailure, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcp=dhcp, dnsDaemon=dnsDaemon, commandServer=commandServer, lcd=lcd, dnsStats=dnsStats, dhcpSubnetTable=dhcpSubnetTable, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpDaemonRunning=dhcpDaemonRunning, dhcpStats=dhcpStats, PYSNMP_MODULE_ID=adonis, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsNXRRSet=dnsStatsNXRRSet, commandServerDaemon=commandServerDaemon, system=system, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpFixedIP=dhcpFixedIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, lcdDaemon=lcdDaemon, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemon=tftpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseEndTime=dhcpLeaseEndTime, trapTFTP=trapTFTP, dhcpFailOverState=dhcpFailOverState, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, trapSystemDaemon=trapSystemDaemon, trapHA=trapHA, dhcpLeaseHostname=dhcpLeaseHostname, adonisTraps=adonisTraps, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseTable=dhcpLeaseTable, dnsStatsFailure=dnsStatsFailure, dhcpPoolTable=dhcpPoolTable, trapSystem=trapSystem, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolSize=dhcpPoolSize, adonisObjects=adonisObjects, trapDHCPDaemon=trapDHCPDaemon, trapDNSDaemon=trapDNSDaemon, haServiceRunning=haServiceRunning, haServiceNodeType=haServiceNodeType, trapDNS=trapDNS, ha=ha, dhcpSubnetAlert=dhcpSubnetAlert, trapReplication=trapReplication, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonRunning=dnsDaemonRunning, trapCommandServerDaemon=trapCommandServerDaemon, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapCommandServer=trapCommandServer, dhcpIP=dhcpIP, dhcpSubnetIP=dhcpSubnetIP, trapHAServiceFailOver=trapHAServiceFailOver, trapDHCP=trapDHCP, dhcpSubnetUsed=dhcpSubnetUsed, haReplicationBinding=haReplicationBinding, systemState=systemState, systemDaemon=systemDaemon, dnsStatsReferral=dnsStatsReferral, dhcpConfig=dhcpConfig, dns=dns, dhcpPoolUsed=dhcpPoolUsed, trapTFTPDaemon=trapTFTPDaemon, dnsStatsRecursion=dnsStatsRecursion, haService=haService, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpPoolAlert=dhcpPoolAlert, tftp=tftp, dnsStatsSuccess=dnsStatsSuccess, dhcpDaemon=dhcpDaemon)
