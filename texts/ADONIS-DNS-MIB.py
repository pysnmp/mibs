...
TimeTicks, IpAddress, Integer32, Counter32, MibIdentifier, enterprises, iso, Gauge32, ObjectIdentity, Unsigned32, MibScalar, MibTable, MibTableRow, MibTableColumn, NotificationType, ModuleIdentity, Bits, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "IpAddress", "Integer32", "Counter32", "MibIdentifier", "enterprises", "iso", "Gauge32", "ObjectIdentity", "Unsigned32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "NotificationType", "ModuleIdentity", "Bits", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsNXRRSet=dnsStatsNXRRSet, trapCommandServerDaemon=trapCommandServerDaemon, trapSystem=trapSystem, trapReplicationFailure=trapReplicationFailure, dhcp=dhcp, dhcpPoolStartIP=dhcpPoolStartIP, haReplicationBinding=haReplicationBinding, tftpDaemonRunning=tftpDaemonRunning, lcdDaemon=lcdDaemon, tftp=tftp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTable=dhcpLeaseTable, adonisObjects=adonisObjects, dnsStatsNXDomain=dnsStatsNXDomain, trapDNS=trapDNS, tftpDaemon=tftpDaemon, haServiceNodeType=haServiceNodeType, haService=haService, ha=ha, systemState=systemState, dhcpPoolEntry=dhcpPoolEntry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, PYSNMP_MODULE_ID=adonis, dhcpFailOverState=dhcpFailOverState, dhcpSubnetTable=dhcpSubnetTable, adonisTraps=adonisTraps, dhcpConfig=dhcpConfig, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsStats=dnsStats, dhcpSubnetSize=dhcpSubnetSize, dnsDaemon=dnsDaemon, dhcpLeaseHostname=dhcpLeaseHostname, trapCommandServer=trapCommandServer, trapHAServiceFailOver=trapHAServiceFailOver, dhcpStats=dhcpStats, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSize=dhcpPoolSize, commandServerDaemonRunning=commandServerDaemonRunning, trapDHCPDaemon=trapDHCPDaemon, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseEntry=dhcpLeaseEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpIP=dhcpIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonis=adonis, dhcpSubnetAlert=dhcpSubnetAlert, licenseValid=licenseValid, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsFailure=dnsStatsFailure, dhcpPoolTable=dhcpPoolTable, dhcpDaemon=dhcpDaemon, trapSystemDaemon=trapSystemDaemon, trapTFTP=trapTFTP, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolEndIP=dhcpPoolEndIP, licenseExpiry=licenseExpiry, trapTFTPDaemon=trapTFTPDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, commandServerDaemon=commandServerDaemon, lcd=lcd, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsRecursion=dnsStatsRecursion, dns=dns, dhcpFixedIPTable=dhcpFixedIPTable, dhcpFixedIP=dhcpFixedIP, dnsStatsReferral=dnsStatsReferral, dhcpFixedIPEntry=dhcpFixedIPEntry, trapDNSDaemon=trapDNSDaemon, system=system, systemDaemon=systemDaemon, dnsDaemonRunning=dnsDaemonRunning, commandServer=commandServer, haServiceRunning=haServiceRunning, trapHA=trapHA, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetUsed=dhcpSubnetUsed, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseBindState=dhcpLeaseBindState, trapDHCP=trapDHCP, trapReplication=trapReplication)
