...
MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, NotificationType, Unsigned32, enterprises, TimeTicks, Gauge32, ObjectIdentity, Counter32, ModuleIdentity, Bits, MibIdentifier, iso, IpAddress, Integer32 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "NotificationType", "Unsigned32", "enterprises", "TimeTicks", "Gauge32", "ObjectIdentity", "Counter32", "ModuleIdentity", "Bits", "MibIdentifier", "iso", "IpAddress", "Integer32")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonRunning=dnsDaemonRunning, trapReplicationFailure=trapReplicationFailure, dnsStatsReferral=dnsStatsReferral, dhcpLeaseBindState=dhcpLeaseBindState, lcd=lcd, haServiceNodeType=haServiceNodeType, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseEntry=dhcpLeaseEntry, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsSuccess=dnsStatsSuccess, dhcpPoolSubnetIP=dhcpPoolSubnetIP, licenseExpiry=licenseExpiry, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpLeaseStartTime=dhcpLeaseStartTime, haServiceRunning=haServiceRunning, commandServerDaemonRunning=commandServerDaemonRunning, dhcpIP=dhcpIP, dhcpDaemonRunning=dhcpDaemonRunning, trapDNSDaemon=trapDNSDaemon, licenseValid=licenseValid, trapSystemDaemon=trapSystemDaemon, haReplicationBinding=haReplicationBinding, dnsDaemonDebugLevel=dnsDaemonDebugLevel, ha=ha, PYSNMP_MODULE_ID=adonis, dnsStats=dnsStats, dnsStatsNXRRSet=dnsStatsNXRRSet, trapTFTP=trapTFTP, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetAlert=dhcpSubnetAlert, trapCommandServer=trapCommandServer, adonisObjects=adonisObjects, dhcpPoolUsed=dhcpPoolUsed, tftpDaemon=tftpDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dns=dns, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetEntry=dhcpSubnetEntry, dhcpPoolStartIP=dhcpPoolStartIP, dhcpPoolEntry=dhcpPoolEntry, dnsDaemon=dnsDaemon, tftp=tftp, trapHAServiceFailOver=trapHAServiceFailOver, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, dnsStatsRecursion=dnsStatsRecursion, trapSystem=trapSystem, dhcpDaemon=dhcpDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolTable=dhcpPoolTable, dhcpFixedIP=dhcpFixedIP, trapReplication=trapReplication, trapCommandServerDaemon=trapCommandServerDaemon, dhcpLeaseTable=dhcpLeaseTable, commandServerDaemon=commandServerDaemon, haService=haService, tftpDaemonRunning=tftpDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEndTime=dhcpLeaseEndTime, trapHA=trapHA, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServer=commandServer, dhcpStats=dhcpStats, lcdDaemon=lcdDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, trapDHCP=trapDHCP, dhcp=dhcp, adonis=adonis, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, adonisTraps=adonisTraps, dnsStatsFailure=dnsStatsFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpPoolSize=dhcpPoolSize, dhcpPoolAlert=dhcpPoolAlert, systemDaemon=systemDaemon, system=system, dhcpFailOverState=dhcpFailOverState, dhcpConfig=dhcpConfig, dhcpFixedIPEntry=dhcpFixedIPEntry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, systemState=systemState)
