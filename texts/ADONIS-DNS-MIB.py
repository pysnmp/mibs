...
Gauge32, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, iso, IpAddress, Counter32, Unsigned32, NotificationType, Integer32, TimeTicks, MibIdentifier, ObjectIdentity, ModuleIdentity, enterprises, Bits = mibBuilder.importSymbols("SNMPv2-SMI", "Gauge32", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "iso", "IpAddress", "Counter32", "Unsigned32", "NotificationType", "Integer32", "TimeTicks", "MibIdentifier", "ObjectIdentity", "ModuleIdentity", "enterprises", "Bits")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetTable=dhcpSubnetTable, dhcpConfig=dhcpConfig, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNS=trapDNS, dhcpSubnetEntry=dhcpSubnetEntry, adonisTraps=adonisTraps, lcd=lcd, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, licenseExpiry=licenseExpiry, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpPoolStartIP=dhcpPoolStartIP, dhcpLeaseTable=dhcpLeaseTable, dhcpFixedIPTable=dhcpFixedIPTable, trapCommandServer=trapCommandServer, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, systemDaemon=systemDaemon, trapReplication=trapReplication, trapDNSDaemon=trapDNSDaemon, tftp=tftp, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetSize=dhcpSubnetSize, dnsStatsReferral=dnsStatsReferral, trapSystem=trapSystem, dnsDaemon=dnsDaemon, haService=haService, tftpDaemon=tftpDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, ha=ha, dhcpPoolTable=dhcpPoolTable, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetIP=dhcpSubnetIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolAlert=dhcpPoolAlert, commandServer=commandServer, dhcpFailOverState=dhcpFailOverState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, systemState=systemState, dhcpFixedIPEntry=dhcpFixedIPEntry, haServiceNodeType=haServiceNodeType, trapHAServiceFailOver=trapHAServiceFailOver, trapSystemDaemon=trapSystemDaemon, dhcpStats=dhcpStats, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsRecursion=dnsStatsRecursion, dnsStats=dnsStats, trapDHCP=trapDHCP, licenseValid=licenseValid, PYSNMP_MODULE_ID=adonis, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetUsed=dhcpSubnetUsed, dnsDaemonRunning=dnsDaemonRunning, dhcpDaemonRunning=dhcpDaemonRunning, dhcp=dhcp, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapTFTPDaemon=trapTFTPDaemon, dhcpIP=dhcpIP, trapDHCPDaemon=trapDHCPDaemon, commandServerDaemon=commandServerDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolEndIP=dhcpPoolEndIP, dns=dns, dhcpDaemon=dhcpDaemon, commandServerDaemonRunning=commandServerDaemonRunning, adonisObjects=adonisObjects, dhcpSubnetAlert=dhcpSubnetAlert, adonis=adonis, lcdDaemon=lcdDaemon, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsStatsFailure=dnsStatsFailure, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpLeaseHostname=dhcpLeaseHostname, trapReplicationFailure=trapReplicationFailure, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapTFTP=trapTFTP, dhcpPoolSize=dhcpPoolSize, dhcpPoolUsed=dhcpPoolUsed, system=system, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpFixedIP=dhcpFixedIP, dhcpLeaseEntry=dhcpLeaseEntry, trapHA=trapHA, dhcpLeaseEndTime=dhcpLeaseEndTime, haReplicationBinding=haReplicationBinding, haServiceRunning=haServiceRunning)
