...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemon=dnsDaemon, dhcpFailOverState=dhcpFailOverState, ha=ha, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseBindState=dhcpLeaseBindState, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, licenseExpiry=licenseExpiry, PYSNMP_MODULE_ID=adonis, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseEntry=dhcpLeaseEntry, dhcpFixedIPEntry=dhcpFixedIPEntry, haService=haService, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetAlert=dhcpSubnetAlert, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisTraps=adonisTraps, systemState=systemState, trapDNSDaemon=trapDNSDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, licenseValid=licenseValid, dhcpSubnetIP=dhcpSubnetIP, trapTFTPDaemon=trapTFTPDaemon, adonisObjects=adonisObjects, commandServerDaemonRunning=commandServerDaemonRunning, system=system, commandServer=commandServer, dhcpPoolAlert=dhcpPoolAlert, lcd=lcd, dhcpStats=dhcpStats, trapHA=trapHA, dhcpFixedIP=dhcpFixedIP, dnsStatsSuccess=dnsStatsSuccess, dhcpLeaseStartTime=dhcpLeaseStartTime, trapDNS=trapDNS, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXDomain=dnsStatsNXDomain, dnsStatsRecursion=dnsStatsRecursion, dns=dns, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpIP=dhcpIP, trapDHCP=trapDHCP, dnsStats=dnsStats, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, tftpDaemon=tftpDaemon, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseHostname=dhcpLeaseHostname, trapSystem=trapSystem, dhcpLeaseEndTime=dhcpLeaseEndTime, trapTFTP=trapTFTP, dnsStatsFailure=dnsStatsFailure, dhcpPoolStartIP=dhcpPoolStartIP, trapCommandServer=trapCommandServer, haServiceRunning=haServiceRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonDebugLevel=dnsDaemonDebugLevel, haReplicationBinding=haReplicationBinding, dnsStatsReferral=dnsStatsReferral, dhcpConfig=dhcpConfig, trapCommandServerDaemon=trapCommandServerDaemon, tftp=tftp, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemonRunning=tftpDaemonRunning, systemDaemon=systemDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcp=dhcp, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolTable=dhcpPoolTable, trapReplicationFailure=trapReplicationFailure, trapReplication=trapReplication, dhcpPoolSize=dhcpPoolSize, dhcpSubnetSize=dhcpSubnetSize, lcdDaemon=lcdDaemon, haServiceNodeType=haServiceNodeType, trapSystemDaemon=trapSystemDaemon, trapHAServiceFailOver=trapHAServiceFailOver, commandServerDaemon=commandServerDaemon, dhcpSubnetEntry=dhcpSubnetEntry, adonis=adonis, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetUsed=dhcpSubnetUsed, dhcpDaemon=dhcpDaemon)
