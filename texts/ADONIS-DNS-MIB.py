...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpDaemon=dhcpDaemon, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, system=system, lcd=lcd, dnsDaemonDebugLevel=dnsDaemonDebugLevel, adonisTraps=adonisTraps, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolSize=dhcpPoolSize, dnsStatsRecursion=dnsStatsRecursion, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpConfig=dhcpConfig, dhcpFixedIPEntry=dhcpFixedIPEntry, systemState=systemState, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolAlert=dhcpPoolAlert, trapSystem=trapSystem, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonis=adonis, commandServerDaemon=commandServerDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpPoolEntry=dhcpPoolEntry, trapSystemDaemon=trapSystemDaemon, dhcpFailOverState=dhcpFailOverState, dhcpDaemonRunning=dhcpDaemonRunning, trapReplicationFailure=trapReplicationFailure, dns=dns, commandServer=commandServer, dnsStats=dnsStats, dhcpFixedIPTable=dhcpFixedIPTable, dhcpStats=dhcpStats, dhcp=dhcp, haServiceNodeType=haServiceNodeType, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpPoolStartIP=dhcpPoolStartIP, tftpDaemonRunning=tftpDaemonRunning, trapCommandServer=trapCommandServer, trapHA=trapHA, trapTFTP=trapTFTP, haServiceRunning=haServiceRunning, dnsStatsReferral=dnsStatsReferral, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEndIP=dhcpPoolEndIP, licenseValid=licenseValid, PYSNMP_MODULE_ID=adonis, dhcpLeaseEntry=dhcpLeaseEntry, trapDHCPDaemon=trapDHCPDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonRunning=dnsDaemonRunning, dhcpSubnetUsed=dhcpSubnetUsed, haService=haService, tftp=tftp, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapDNSDaemon=trapDNSDaemon, licenseExpiry=licenseExpiry, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolUsed=dhcpPoolUsed, haReplicationBinding=haReplicationBinding, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServerDaemon=trapCommandServerDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIP=dhcpFixedIP, lcdDaemon=lcdDaemon, dhcpLeaseBindState=dhcpLeaseBindState, trapDHCP=trapDHCP, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, systemDaemon=systemDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpIP=dhcpIP, tftpDaemon=tftpDaemon, dhcpPoolTable=dhcpPoolTable, dnsStatsFailure=dnsStatsFailure, dhcpSubnetTable=dhcpSubnetTable, ha=ha, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpSubnetSize=dhcpSubnetSize, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, dnsDaemon=dnsDaemon, adonisObjects=adonisObjects, trapReplication=trapReplication)
