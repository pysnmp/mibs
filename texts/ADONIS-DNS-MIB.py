...
iso, Unsigned32, Integer32, MibIdentifier, NotificationType, Counter32, Counter64, ObjectIdentity, Bits, Gauge32, ModuleIdentity, enterprises, TimeTicks, IpAddress, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "Unsigned32", "Integer32", "MibIdentifier", "NotificationType", "Counter32", "Counter64", "ObjectIdentity", "Bits", "Gauge32", "ModuleIdentity", "enterprises", "TimeTicks", "IpAddress", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpSubnetIP=dhcpSubnetIP, dhcpSubnetUsed=dhcpSubnetUsed, tftp=tftp, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetTable=dhcpSubnetTable, systemState=systemState, tftpDaemonRunning=tftpDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseEntry=dhcpLeaseEntry, systemDaemon=systemDaemon, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemon=dhcpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolEndIP=dhcpPoolEndIP, dhcpLeaseBindState=dhcpLeaseBindState, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpFixedIP=dhcpFixedIP, commandServerDaemon=commandServerDaemon, licenseValid=licenseValid, lcd=lcd, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolTable=dhcpPoolTable, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dhcpSubnetSize=dhcpSubnetSize, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsNXDomain=dnsStatsNXDomain, trapTFTP=trapTFTP, dnsStatsReferral=dnsStatsReferral, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcp=dhcp, trapReplicationFailure=trapReplicationFailure, trapDHCP=trapDHCP, dnsStatsSuccess=dnsStatsSuccess, system=system, trapSystem=trapSystem, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, commandServer=commandServer, dnsStatsFailure=dnsStatsFailure, dns=dns, dnsStatsRecursion=dnsStatsRecursion, dhcpFailOverState=dhcpFailOverState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServerDaemonRunning=commandServerDaemonRunning, dhcpFixedIPEntry=dhcpFixedIPEntry, adonis=adonis, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpSubnetMask=dhcpSubnetMask, haService=haService, trapDNS=trapDNS, dnsStats=dnsStats, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemon=dnsDaemon, adonisTraps=adonisTraps, dhcpPoolSize=dhcpPoolSize, dhcpLeaseTable=dhcpLeaseTable, dhcpConfig=dhcpConfig, dhcpPoolStartIP=dhcpPoolStartIP, dhcpStats=dhcpStats, adonisObjects=adonisObjects, trapCommandServer=trapCommandServer, ha=ha, lcdDaemon=lcdDaemon, dhcpLeaseHostname=dhcpLeaseHostname, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpIP=dhcpIP, dhcpPoolAlert=dhcpPoolAlert, haServiceNodeType=haServiceNodeType, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpSubnetEntry=dhcpSubnetEntry, PYSNMP_MODULE_ID=adonis, trapDHCPDaemon=trapDHCPDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, haServiceRunning=haServiceRunning, trapSystemDaemon=trapSystemDaemon, licenseExpiry=licenseExpiry, trapHA=trapHA, trapReplication=trapReplication, dnsStatsNXRRSet=dnsStatsNXRRSet, tftpDaemon=tftpDaemon, trapHAServiceFailOver=trapHAServiceFailOver, haReplicationBinding=haReplicationBinding, dhcpFixedIPTable=dhcpFixedIPTable, trapDNSDaemon=trapDNSDaemon, dhcpPoolUsed=dhcpPoolUsed)
