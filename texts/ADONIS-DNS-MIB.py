...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", adonis=adonis, dns=dns, trapSystemDaemon=trapSystemDaemon, dnsDaemon=dnsDaemon, trapDNSDaemon=trapDNSDaemon, dnsStats=dnsStats, dhcpLeaseEntry=dhcpLeaseEntry, licenseExpiry=licenseExpiry, dhcpSubnetUsed=dhcpSubnetUsed, haServiceNodeType=haServiceNodeType, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, tftp=tftp, lcdDaemon=lcdDaemon, dhcpLeaseEndTime=dhcpLeaseEndTime, adonisTraps=adonisTraps, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, haService=haService, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsDaemonRunning=dnsDaemonRunning, tftpDaemon=tftpDaemon, dhcpSubnetAlert=dhcpSubnetAlert, haServiceRunning=haServiceRunning, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsReferral=dnsStatsReferral, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpSubnetEntry=dhcpSubnetEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, lcd=lcd, commandServerDaemonRunning=commandServerDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, trapReplication=trapReplication, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsFailure=dnsStatsFailure, systemDaemon=systemDaemon, haReplicationBinding=haReplicationBinding, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolUsed=dhcpPoolUsed, trapTFTPDaemon=trapTFTPDaemon, dhcpSubnetIP=dhcpSubnetIP, dhcpDaemonRunning=dhcpDaemonRunning, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetTable=dhcpSubnetTable, trapDHCP=trapDHCP, licenseValid=licenseValid, commandServer=commandServer, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcp=dhcp, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpConfig=dhcpConfig, dhcpLeaseTable=dhcpLeaseTable, dnsStatsSuccess=dnsStatsSuccess, commandServerDaemon=commandServerDaemon, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolEntry=dhcpPoolEntry, dhcpLeaseHostname=dhcpLeaseHostname, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpPoolSize=dhcpPoolSize, dhcpDaemon=dhcpDaemon, dnsStatsNXDomain=dnsStatsNXDomain, dhcpPoolStartIP=dhcpPoolStartIP, systemState=systemState, ha=ha, dhcpIP=dhcpIP, dhcpFixedIP=dhcpFixedIP, trapCommandServer=trapCommandServer, dhcpPoolTable=dhcpPoolTable, adonisObjects=adonisObjects, trapHA=trapHA, trapDNS=trapDNS, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpStats=dhcpStats, PYSNMP_MODULE_ID=adonis, trapTFTP=trapTFTP, trapSystem=trapSystem, dhcpFailOverState=dhcpFailOverState, tftpDaemonRunning=tftpDaemonRunning, dhcpSubnetSize=dhcpSubnetSize, trapReplicationFailure=trapReplicationFailure, dhcpSubnetMask=dhcpSubnetMask, system=system, dhcpFixedIPTable=dhcpFixedIPTable)
