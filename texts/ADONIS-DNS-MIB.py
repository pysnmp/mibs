...
ModuleIdentity, ObjectIdentity, Counter64, iso, enterprises, Bits, MibIdentifier, Unsigned32, TimeTicks, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32, Gauge32, Integer32, IpAddress = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "ObjectIdentity", "Counter64", "iso", "enterprises", "Bits", "MibIdentifier", "Unsigned32", "TimeTicks", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32", "Gauge32", "Integer32", "IpAddress")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapReplicationFailure=trapReplicationFailure, dns=dns, lcd=lcd, dhcpLeaseEndTime=dhcpLeaseEndTime, haReplicationBinding=haReplicationBinding, trapCommandServerDaemon=trapCommandServerDaemon, adonisTraps=adonisTraps, dnsStats=dnsStats, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpFailOverState=dhcpFailOverState, dhcpIP=dhcpIP, dnsDaemonRunning=dnsDaemonRunning, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpSubnetMask=dhcpSubnetMask, licenseValid=licenseValid, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseEntry=dhcpLeaseEntry, commandServerDaemonRunning=commandServerDaemonRunning, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEntry=dhcpPoolEntry, trapDNS=trapDNS, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolTable=dhcpPoolTable, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolEndIP=dhcpPoolEndIP, adonisObjects=adonisObjects, tftpDaemon=tftpDaemon, dhcpConfig=dhcpConfig, dhcpLeaseStartTime=dhcpLeaseStartTime, licenseExpiry=licenseExpiry, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, haServiceNodeType=haServiceNodeType, commandServer=commandServer, dhcp=dhcp, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetSize=dhcpSubnetSize, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, trapSystem=trapSystem, trapDNSDaemon=trapDNSDaemon, adonis=adonis, haService=haService, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, system=system, dnsStatsSuccess=dnsStatsSuccess, PYSNMP_MODULE_ID=adonis, commandServerDaemon=commandServerDaemon, trapCommandServer=trapCommandServer, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapDHCPDaemon=trapDHCPDaemon, dhcpFixedIP=dhcpFixedIP, dhcpDaemon=dhcpDaemon, ha=ha, trapSystemDaemon=trapSystemDaemon, dhcpFixedIPTable=dhcpFixedIPTable, lcdDaemon=lcdDaemon, haServiceRunning=haServiceRunning, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsFailure=dnsStatsFailure, dnsDaemon=dnsDaemon, systemState=systemState, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpSubnetEntry=dhcpSubnetEntry, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, systemDaemon=systemDaemon, dhcpLeaseTable=dhcpLeaseTable, dnsStatsRecursion=dnsStatsRecursion, trapTFTPDaemon=trapTFTPDaemon, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseHostname=dhcpLeaseHostname, trapReplication=trapReplication, trapHA=trapHA, dhcpPoolSize=dhcpPoolSize, dhcpSubnetUsed=dhcpSubnetUsed, trapTFTP=trapTFTP, tftp=tftp, trapHAServiceFailOver=trapHAServiceFailOver, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpStats=dhcpStats, dnsStatsReferral=dnsStatsReferral, dhcpSubnetAlert=dhcpSubnetAlert, trapDHCP=trapDHCP, dnsStatsNXDomain=dnsStatsNXDomain)
