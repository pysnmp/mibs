...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", systemDaemon=systemDaemon, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, commandServerDaemonRunning=commandServerDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolTable=dhcpPoolTable, dnsStats=dnsStats, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dns=dns, trapReplicationFailure=trapReplicationFailure, ha=ha, dhcpSubnetUsed=dhcpSubnetUsed, dhcpSubnetSize=dhcpSubnetSize, dhcpSubnetTable=dhcpSubnetTable, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, adonisTraps=adonisTraps, haServiceNodeType=haServiceNodeType, dhcpPoolSize=dhcpPoolSize, trapCommandServer=trapCommandServer, trapTFTP=trapTFTP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsNXRRSet=dnsStatsNXRRSet, commandServerDaemon=commandServerDaemon, lcd=lcd, trapDHCPDaemon=trapDHCPDaemon, lcdDaemon=lcdDaemon, trapReplication=trapReplication, dhcpPoolUsed=dhcpPoolUsed, systemState=systemState, dhcpFixedIPTable=dhcpFixedIPTable, tftpDaemon=tftpDaemon, tftpDaemonRunning=tftpDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, trapSystemDaemon=trapSystemDaemon, dhcpConfig=dhcpConfig, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, licenseValid=licenseValid, haService=haService, dhcpSubnetIP=dhcpSubnetIP, adonis=adonis, trapHA=trapHA, dhcpStats=dhcpStats, trapDNS=trapDNS, dhcpSubnetAlert=dhcpSubnetAlert, trapSystem=trapSystem, trapCommandServerDaemon=trapCommandServerDaemon, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpSubnetEntry=dhcpSubnetEntry, dhcpLeaseEndTime=dhcpLeaseEndTime, dnsStatsNXDomain=dnsStatsNXDomain, dhcpSubnetMask=dhcpSubnetMask, dnsDaemon=dnsDaemon, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dhcpLeaseTable=dhcpLeaseTable, dhcpPoolEndIP=dhcpPoolEndIP, dnsStatsReferral=dnsStatsReferral, haServiceRunning=haServiceRunning, trapTFTPDaemon=trapTFTPDaemon, dnsStatsFailure=dnsStatsFailure, dhcpFixedIP=dhcpFixedIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, licenseExpiry=licenseExpiry, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpFixedIPEntry=dhcpFixedIPEntry, system=system, dhcpPoolEntry=dhcpPoolEntry, commandServer=commandServer, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpDaemonRunning=dhcpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpIP=dhcpIP, haReplicationBinding=haReplicationBinding, dhcpPoolAlert=dhcpPoolAlert, trapDNSDaemon=trapDNSDaemon, dhcpLeaseBindState=dhcpLeaseBindState, tftp=tftp, dhcpFailOverState=dhcpFailOverState, dhcp=dhcp, dhcpDaemon=dhcpDaemon, dnsStatsSuccess=dnsStatsSuccess, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapHAServiceFailOver=trapHAServiceFailOver, dnsStatsRecursion=dnsStatsRecursion, adonisObjects=adonisObjects, dhcpLeaseEntry=dhcpLeaseEntry, trapDHCP=trapDHCP, PYSNMP_MODULE_ID=adonis)
