...
ModuleIdentity, Integer32, enterprises, NotificationType, Counter32, ObjectIdentity, Bits, MibScalar, MibTable, MibTableRow, MibTableColumn, Gauge32, iso, Unsigned32, Counter64, TimeTicks, IpAddress, MibIdentifier = mibBuilder.importSymbols("SNMPv2-SMI", "ModuleIdentity", "Integer32", "enterprises", "NotificationType", "Counter32", "ObjectIdentity", "Bits", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Gauge32", "iso", "Unsigned32", "Counter64", "TimeTicks", "IpAddress", "MibIdentifier")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetIP=dhcpSubnetIP, dnsStats=dnsStats, haServiceNodeType=haServiceNodeType, dnsStatsReferral=dnsStatsReferral, trapDNSDaemon=trapDNSDaemon, dhcpConfig=dhcpConfig, trapDHCP=trapDHCP, systemState=systemState, dhcpStats=dhcpStats, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dnsStatsSuccess=dnsStatsSuccess, dhcpFailOverState=dhcpFailOverState, dns=dns, licenseExpiry=licenseExpiry, dhcpPoolAlert=dhcpPoolAlert, dhcpDaemon=dhcpDaemon, tftpDaemon=tftpDaemon, licenseValid=licenseValid, trapDHCPDaemon=trapDHCPDaemon, dhcpSubnetTable=dhcpSubnetTable, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpPoolUsed=dhcpPoolUsed, dhcpPoolSize=dhcpPoolSize, dhcpSubnetAlert=dhcpSubnetAlert, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpSubnetMask=dhcpSubnetMask, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystem=trapSystem, dhcpPoolStartIP=dhcpPoolStartIP, tftpDaemonRunning=tftpDaemonRunning, lcdDaemon=lcdDaemon, tftp=tftp, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, system=system, trapTFTPDaemon=trapTFTPDaemon, haReplicationBinding=haReplicationBinding, dhcp=dhcp, dhcpSubnetEntry=dhcpSubnetEntry, commandServerDaemonRunning=commandServerDaemonRunning, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDNS=trapDNS, dnsDaemon=dnsDaemon, PYSNMP_MODULE_ID=adonis, ha=ha, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolTable=dhcpPoolTable, dhcpLeaseHostname=dhcpLeaseHostname, trapCommandServer=trapCommandServer, dhcpLeaseTable=dhcpLeaseTable, trapCommandServerDaemon=trapCommandServerDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpFixedIPTable=dhcpFixedIPTable, haServiceRunning=haServiceRunning, trapReplicationFailure=trapReplicationFailure, dhcpSubnetSize=dhcpSubnetSize, dhcpDaemonRunning=dhcpDaemonRunning, trapHAServiceFailOver=trapHAServiceFailOver, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsFailure=dnsStatsFailure, haService=haService, systemDaemon=systemDaemon, dhcpPoolEntry=dhcpPoolEntry, commandServerDaemon=commandServerDaemon, trapReplication=trapReplication, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapHA=trapHA, dhcpIP=dhcpIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpLeaseBindState=dhcpLeaseBindState, dnsStatsNXRRSet=dnsStatsNXRRSet, dnsDaemonRunning=dnsDaemonRunning, dnsStatsNXDomain=dnsStatsNXDomain, trapTFTP=trapTFTP, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dhcpFixedIP=dhcpFixedIP, lcd=lcd, adonisObjects=adonisObjects, trapSystemDaemon=trapSystemDaemon, dnsStatsRecursion=dnsStatsRecursion, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, adonisTraps=adonisTraps, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcpPoolSubnetIP=dhcpPoolSubnetIP, commandServer=commandServer, adonis=adonis, dhcpPoolEndIP=dhcpPoolEndIP)
