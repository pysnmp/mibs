...
IpAddress, Counter64, ObjectIdentity, MibIdentifier, TimeTicks, iso, Integer32, Bits, Gauge32, Unsigned32, enterprises, NotificationType, Counter32, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "IpAddress", "Counter64", "ObjectIdentity", "MibIdentifier", "TimeTicks", "iso", "Integer32", "Bits", "Gauge32", "Unsigned32", "enterprises", "NotificationType", "Counter32", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", PYSNMP_MODULE_ID=adonis, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolSize=dhcpPoolSize, adonisTraps=adonisTraps, trapTFTPDaemon=trapTFTPDaemon, ha=ha, dhcpLeaseHostname=dhcpLeaseHostname, tftpDaemonRunning=tftpDaemonRunning, dhcpConfig=dhcpConfig, dnsStatsRecursion=dnsStatsRecursion, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIP=dhcpFixedIP, dns=dns, dnsStats=dnsStats, dhcpSubnetAlert=dhcpSubnetAlert, dhcpPoolUsed=dhcpPoolUsed, dhcpDaemon=dhcpDaemon, tftpDaemon=tftpDaemon, licenseExpiry=licenseExpiry, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, trapDNSDaemon=trapDNSDaemon, trapHA=trapHA, dnsDaemonRunning=dnsDaemonRunning, dhcpPoolEntry=dhcpPoolEntry, dhcpStats=dhcpStats, dhcpPoolAlert=dhcpPoolAlert, dhcpFailOverState=dhcpFailOverState, dnsDaemon=dnsDaemon, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, dnsStatsFailure=dnsStatsFailure, trapDHCPDaemon=trapDHCPDaemon, systemDaemon=systemDaemon, trapDHCP=trapDHCP, commandServerDaemonRunning=commandServerDaemonRunning, trapReplication=trapReplication, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dnsStatsSuccess=dnsStatsSuccess, trapDNS=trapDNS, dhcpSubnetIP=dhcpSubnetIP, dhcpPoolEndIP=dhcpPoolEndIP, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, dhcpLeaseStartTime=dhcpLeaseStartTime, dnsStatsNXRRSet=dnsStatsNXRRSet, commandServer=commandServer, trapTFTP=trapTFTP, dhcpSubnetSize=dhcpSubnetSize, dhcpPoolStartIP=dhcpPoolStartIP, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpIP=dhcpIP, dhcpLeaseBindState=dhcpLeaseBindState, licenseValid=licenseValid, adonis=adonis, trapCommandServer=trapCommandServer, system=system, dhcp=dhcp, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpLeaseTable=dhcpLeaseTable, dnsStatsReferral=dnsStatsReferral, haService=haService, lcd=lcd, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, haServiceRunning=haServiceRunning, dhcpPoolTable=dhcpPoolTable, trapSystem=trapSystem, dhcpSubnetEntry=dhcpSubnetEntry, haReplicationBinding=haReplicationBinding, trapSystemDaemon=trapSystemDaemon, lcdDaemon=lcdDaemon, adonisObjects=adonisObjects, trapReplicationFailure=trapReplicationFailure, dhcpSubnetUsed=dhcpSubnetUsed, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpFixedIPTable=dhcpFixedIPTable, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonRunning=dhcpDaemonRunning, dhcpSubnetMask=dhcpSubnetMask, dhcpLeaseEntry=dhcpLeaseEntry, systemState=systemState, commandServerDaemon=commandServerDaemon, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, haServiceNodeType=haServiceNodeType, tftp=tftp, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dnsStatsNXDomain=dnsStatsNXDomain, trapCommandServerDaemon=trapCommandServerDaemon)
