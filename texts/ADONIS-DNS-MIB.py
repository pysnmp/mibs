...
MibScalar, MibTable, MibTableRow, MibTableColumn, Integer32, TimeTicks, iso, ModuleIdentity, enterprises, Unsigned32, IpAddress, Bits, MibIdentifier, NotificationType, Counter32, Gauge32, ObjectIdentity, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Integer32", "TimeTicks", "iso", "ModuleIdentity", "enterprises", "Unsigned32", "IpAddress", "Bits", "MibIdentifier", "NotificationType", "Counter32", "Gauge32", "ObjectIdentity", "Counter64")
...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", trapTFTP=trapTFTP, dhcpDaemonRunning=dhcpDaemonRunning, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, trapReplication=trapReplication, systemState=systemState, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, commandServer=commandServer, trapTFTPDaemon=trapTFTPDaemon, haReplicationBinding=haReplicationBinding, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, trapDNS=trapDNS, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, haServiceRunning=haServiceRunning, dnsStatsSuccess=dnsStatsSuccess, tftpDaemon=tftpDaemon, licenseExpiry=licenseExpiry, trapDNSDaemon=trapDNSDaemon, licenseValid=licenseValid, dhcpPoolUsed=dhcpPoolUsed, dhcpLeaseEntry=dhcpLeaseEntry, dhcpPoolSize=dhcpPoolSize, dnsStatsFailure=dnsStatsFailure, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, dhcp=dhcp, trapDHCPDaemon=trapDHCPDaemon, lcd=lcd, dhcpSubnetMask=dhcpSubnetMask, dhcpSubnetEntry=dhcpSubnetEntry, haService=haService, dns=dns, dnsDaemonDebugLevel=dnsDaemonDebugLevel, trapDHCP=trapDHCP, dnsDaemonRunning=dnsDaemonRunning, tftp=tftp, dhcpSubnetAlert=dhcpSubnetAlert, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpSubnetSize=dhcpSubnetSize, dhcpIP=dhcpIP, dhcpPoolEndIP=dhcpPoolEndIP, trapCommandServer=trapCommandServer, system=system, PYSNMP_MODULE_ID=adonis, adonisObjects=adonisObjects, adonisTraps=adonisTraps, dhcpLeaseEndTime=dhcpLeaseEndTime, dhcpLeaseHostname=dhcpLeaseHostname, dhcpFixedIPEntry=dhcpFixedIPEntry, dhcpStats=dhcpStats, dhcpFailOverState=dhcpFailOverState, dnsStats=dnsStats, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dhcpLeaseTable=dhcpLeaseTable, dhcpDaemon=dhcpDaemon, dhcpPoolAlert=dhcpPoolAlert, dhcpSubnetIP=dhcpSubnetIP, haServiceNodeType=haServiceNodeType, lcdDaemon=lcdDaemon, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, dhcpConfig=dhcpConfig, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, systemDaemon=systemDaemon, dhcpSubnetUsed=dhcpSubnetUsed, dhcpPoolTable=dhcpPoolTable, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolStartIP=dhcpPoolStartIP, dnsStatsNXDomain=dnsStatsNXDomain, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapSystemDaemon=trapSystemDaemon, trapHA=trapHA, dnsDaemon=dnsDaemon, trapReplicationFailure=trapReplicationFailure, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, dhcpSubnetTable=dhcpSubnetTable, adonis=adonis, dnsStatsReferral=dnsStatsReferral, commandServerDaemon=commandServerDaemon, trapSystem=trapSystem, dhcpFixedIPTable=dhcpFixedIPTable, dnsStatsRecursion=dnsStatsRecursion, ha=ha, trapHAServiceFailOver=trapHAServiceFailOver, dhcpFixedIP=dhcpFixedIP, dhcpLeaseBindState=dhcpLeaseBindState, dhcpPoolEntry=dhcpPoolEntry, dnsStatsNXRRSet=dnsStatsNXRRSet, trapCommandServerDaemon=trapCommandServerDaemon)
