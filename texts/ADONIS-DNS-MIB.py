...
mibBuilder.exportSymbols("ADONIS-DNS-MIB", commandServer=commandServer, systemState=systemState, trapCommandServer=trapCommandServer, dnsDaemonRunning=dnsDaemonRunning, trapTFTPDaemon=trapTFTPDaemon, dnsDaemonZoneTransferFailure=dnsDaemonZoneTransferFailure, trapSystemDaemon=trapSystemDaemon, tftpDaemon=tftpDaemon, dhcpPoolEntry=dhcpPoolEntry, dhcpSubnetSize=dhcpSubnetSize, dnsStatsNXRRSet=dnsStatsNXRRSet, dhcpLeaseTimeStamp=dhcpLeaseTimeStamp, dnsDaemonSOAQueriesInProgress=dnsDaemonSOAQueriesInProgress, dnsDaemonQueryLoggingState=dnsDaemonQueryLoggingState, dhcpConfig=dhcpConfig, trapReplicationFailure=trapReplicationFailure, dnsStats=dnsStats, dnsStatsFailure=dnsStatsFailure, dhcpLeaseStartTime=dhcpLeaseStartTime, dhcpFixedIP=dhcpFixedIP, dhcpDaemonRunning=dhcpDaemonRunning, dnsStatsRecursion=dnsStatsRecursion, dhcpDaemon=dhcpDaemon, trapDNS=trapDNS, haReplicationBinding=haReplicationBinding, dhcpFixedIPEntry=dhcpFixedIPEntry, tftp=tftp, systemDaemon=systemDaemon, dhcpPoolEndIP=dhcpPoolEndIP, trapDHCP=trapDHCP, dnsDaemonZoneTransfersDeferred=dnsDaemonZoneTransfersDeferred, dhcpLeaseHardwareAddress=dhcpLeaseHardwareAddress, trapDHCPDaemon=trapDHCPDaemon, dnsDaemonDebugLevel=dnsDaemonDebugLevel, dhcpPoolAlert=dhcpPoolAlert, commandServerDaemonRunning=commandServerDaemonRunning, dhcpPoolSubnetIP=dhcpPoolSubnetIP, dnsDaemonNumberOfZones=dnsDaemonNumberOfZones, dnsDaemonZoneTransfersInProgress=dnsDaemonZoneTransfersInProgress, trapHAServiceFailOver=trapHAServiceFailOver, dhcpPoolStartIP=dhcpPoolStartIP, dhcpSubnetTable=dhcpSubnetTable, dhcpPoolTable=dhcpPoolTable, dhcpFailOverState=dhcpFailOverState, dhcpSubnetMask=dhcpSubnetMask, dhcpIP=dhcpIP, dhcpSubnetUsed=dhcpSubnetUsed, PYSNMP_MODULE_ID=adonis, tftpDaemonRunning=tftpDaemonRunning, dhcpLeaseHostname=dhcpLeaseHostname, dhcpPoolUsed=dhcpPoolUsed, trapReplication=trapReplication, trapHA=trapHA, adonisTraps=adonisTraps, dhcpLeaseTable=dhcpLeaseTable, licenseExpiry=licenseExpiry, system=system, dhcpStats=dhcpStats, dhcpLeaseEntry=dhcpLeaseEntry, licenseValid=licenseValid, haServiceNodeType=haServiceNodeType, dhcpDaemonSubnetAlert=dhcpDaemonSubnetAlert, haServiceRunning=haServiceRunning, trapSystem=trapSystem, trapDNSDaemon=trapDNSDaemon, haService=haService, trapTFTP=trapTFTP, dnsDaemon=dnsDaemon, dhcp=dhcp, dnsStatsReferral=dnsStatsReferral, dhcpSubnetEntry=dhcpSubnetEntry, trapCommandServerDaemon=trapCommandServerDaemon, dhcpDaemonLeaseStatsSuccess=dhcpDaemonLeaseStatsSuccess, ha=ha, lcdDaemon=lcdDaemon, commandServerDaemon=commandServerDaemon, dnsStatsSuccess=dnsStatsSuccess, dhcpFixedIPTable=dhcpFixedIPTable, dhcpSubnetAlert=dhcpSubnetAlert, adonis=adonis, dhcpLeaseBindState=dhcpLeaseBindState, adonisObjects=adonisObjects, dhcpSubnetIP=dhcpSubnetIP, dnsStatsNXDomain=dnsStatsNXDomain, dns=dns, dhcpPoolSize=dhcpPoolSize, dhcpLeaseEndTime=dhcpLeaseEndTime, lcd=lcd)
