...
mibBuilder.exportSymbols("CAMBIUM-PTP800-MIB", remotePrimaryInternetAddressType=remotePrimaryInternetAddressType, sNTPServerResponse=sNTPServerResponse, sNMPTrapTableNumber=sNMPTrapTableNumber, rFUProtectionCompatibility=rFUProtectionCompatibility, l2CPPriorityTableIndex=l2CPPriorityTableIndex, productName=productName, sNTPServerInternetAddress=sNTPServerInternetAddress, remoteMACAddress=remoteMACAddress, sNMPTrapInternetAddress=sNMPTrapInternetAddress, rFUCommonIFSynthLockAlarm=rFUCommonIFSynthLockAlarm, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, rFUCommonRFSynthLockAlarm=rFUCommonRFSynthLockAlarm, sNMPTrapIPAddress=sNMPTrapIPAddress, dataPortWirelessDownAlert=dataPortWirelessDownAlert, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, rFUVersionBank1=rFUVersionBank1, protectionStatus=protectionStatus, syslogLocalWrappedTrap=syslogLocalWrappedTrap, iPVersion=iPVersion, receiveModulation=receiveModulation, transmitModulation=transmitModulation, managementPortEthernetSpeedStatus=managementPortEthernetSpeedStatus, rFUHighTemperatureAlarm=rFUHighTemperatureAlarm, radioLicenseIncompatibleAlarm=radioLicenseIncompatibleAlarm, dataPortCopperAutoNegotiation=dataPortCopperAutoNegotiation, sNMPCommunityString=sNMPCommunityString, encryptionAlgorithm=encryptionAlgorithm, ethernet=ethernet, sNTPServerInternetAddressType=sNTPServerInternetAddressType, rxDiversityDataPortStatusTrap=rxDiversityDataPortStatusTrap, activeUnit=activeUnit, sNMPTrapTable=sNMPTrapTable, encryptionEnabledMismatch=encryptionEnabledMismatch, rFUStatus=rFUStatus, sMTPEmailAlert=sMTPEmailAlert, radioLicenseMinModulation=radioLicenseMinModulation, siteName=siteName, radioLicense=radioLicense, rFURfSwitchCableAlarm=rFURfSwitchCableAlarm, userPassword=userPassword, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, remoteSecondaryInternetAddress=remoteSecondaryInternetAddress, rFFeederLoss=rFFeederLoss, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, dataPortEthernetMediaTypeToUse=dataPortEthernetMediaTypeToUse, telnetAccessEnabled=telnetAccessEnabled, dataPortFiberStatusTrap=dataPortFiberStatusTrap, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, managementPortAutoNegAdvertisement=managementPortAutoNegAdvertisement, dataPortCopperAutoNegAdvertisement=dataPortCopperAutoNegAdvertisement, managementPortEthernetSpeedStatusTrap=managementPortEthernetSpeedStatusTrap, radioLicenseRxFreq=radioLicenseRxFreq, rFURxIFSynthLockAlarm=rFURxIFSynthLockAlarm, protectionConfigurationStatus=protectionConfigurationStatus, receivePower=receivePower, dataPortEthernetSpeedStatusTrap=dataPortEthernetSpeedStatusTrap, ptmp=ptmp, managementPortAutoNegotiation=managementPortAutoNegotiation, remotePrimaryMACAddress=remotePrimaryMACAddress, managementPortForcedConfiguration=managementPortForcedConfiguration, managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, remotePrimarySecondaryMode=remotePrimarySecondaryMode, encryptionAlgorithmsAvail=encryptionAlgorithmsAvail, userEnabled=userEnabled, activeAvailableTime=activeAvailableTime, systemReset=systemReset, secondarySoftwareVersion=secondarySoftwareVersion, dSCPManagementPriority=dSCPManagementPriority, maxModulation=maxModulation, ptpTraps=ptpTraps, hardwareVersion=hardwareVersion, protectionConfig=protectionConfig, syslogControl=syslogControl, sMTPServerPortNumber=sMTPServerPortNumber, radioLicenseMaxCodeRate=radioLicenseMaxCodeRate, neighborMACAddress=neighborMACAddress, ethernetPriorityTableIndex=ethernetPriorityTableIndex, productVariant=productVariant, secureModeAlarmTrap=secureModeAlarmTrap, rFUTxPowerMax=rFUTxPowerMax, userName=userName, sNTPServerTable=sNTPServerTable, snmpControl=snmpControl, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, qOSPriorityScheme=qOSPriorityScheme, reset=reset, tFTPServerInternetAddressType=tFTPServerInternetAddressType, syslogClientTrap=syslogClientTrap, protectionConfigurationStatusTrap=protectionConfigurationStatusTrap, syslogStateTrap=syslogStateTrap, diverseRfFeederLoss=diverseRfFeederLoss, rFUTxPowerDegradedAlarm=rFUTxPowerDegradedAlarm, transmitCapacity=transmitCapacity, endWirelessReceiveSignalStatus=endWirelessReceiveSignalStatus, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, rxDiversityVlanTpid=rxDiversityVlanTpid, remoteSecondaryIPAddress=remoteSecondaryIPAddress, minModulation=minModulation, userTableNumber=userTableNumber, rFUActiveFirmwareBank=rFUActiveFirmwareBank, sNTPSyncTrap=sNTPSyncTrap, softwareVersionRemoteCompatibilityTrap=softwareVersionRemoteCompatibilityTrap, transmitDataRate=transmitDataRate, rFUPowerButtonPressed=rFUPowerButtonPressed, softwareVersion=softwareVersion, securityLevel=securityLevel, sNTPServerIPAddress=sNTPServerIPAddress, neighborInternetAddress=neighborInternetAddress, sMTPSourceEmailAddress=sMTPSourceEmailAddress, maxCodeRate=maxCodeRate, gatewayIPAddress=gatewayIPAddress, managementPortWirelessDownAlert=managementPortWirelessDownAlert, protectionState=protectionState, receiveCodeRate=receiveCodeRate, sNTPServerRetries=sNTPServerRetries, sNTPServerTableEntry=sNTPServerTableEntry, transmitCodeRate=transmitCodeRate, cambium=cambium, smtp=smtp, subnetMask=subnetMask, primaryRecoveryPeriod=primaryRecoveryPeriod, managementPortConfigurationMismatch=managementPortConfigurationMismatch, radioLicenseTxFreq=radioLicenseTxFreq, wirelessReceiveSignalStatusTrap=wirelessReceiveSignalStatusTrap, ethernetPriorityTable=ethernetPriorityTable, sNTPServerPortNumber=sNTPServerPortNumber, alarms=alarms, dataPortCopperForcedConfiguration=dataPortCopperForcedConfiguration, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, managementCommittedInformationRate=managementCommittedInformationRate, neighborInternetAddressType=neighborInternetAddressType, primarySecondaryMode=primarySecondaryMode, pubStats=pubStats, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, dataPortEthernetSpeedStatus=dataPortEthernetSpeedStatus, iPv4Address=iPv4Address, rxDiversityAvailabilityStatusTrap=rxDiversityAvailabilityStatusTrap, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, alignmentMode=alignmentMode, managementMode=managementMode, unitOutOfCalibration=unitOutOfCalibration, secureModeAlarm=secureModeAlarm, management=management, iFCableLength=iFCableLength, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, phyControl=phyControl, vectorError=vectorError, protectionAvailabilityStatus=protectionAvailabilityStatus, rFURxRFSynthLockAlarm=rFURxRFSynthLockAlarm, transmitPower=transmitPower, rFUPowerButtonPressedTrap=rFUPowerButtonPressedTrap, sMTPServerInternetAddress=sMTPServerInternetAddress, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, rFUTransmitBranchingUnitLoss=rFUTransmitBranchingUnitLoss, rFUTxFreqMax=rFUTxFreqMax, rFUConnectedPlatform=rFUConnectedPlatform, linkName=linkName, remoteSecondaryMACAddress=remoteSecondaryMACAddress, telnetPortNumber=telnetPortNumber, tFTPServerPortNumber=tFTPServerPortNumber, radioLicenseBandwidth=radioLicenseBandwidth, versions=versions, numberOfAntennas=numberOfAntennas, rFUCableAlarm=rFUCableAlarm, transmitterStatus=transmitterStatus, antennaReceiveLevelMeasurementWindow=antennaReceiveLevelMeasurementWindow, requestedProtectionState=requestedProtectionState, rFURpsAlarm=rFURpsAlarm, rxDiversityVidNeighborCompatibility=rxDiversityVidNeighborCompatibility, sNMPPortNumber=sNMPPortNumber, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, minCodeRate=minCodeRate, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, tFTPServerInternetAddress=tFTPServerInternetAddress, vLANManagementVID=vLANManagementVID, altitude=altitude, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, rFUTxRFSynthLockAlarm=rFUTxRFSynthLockAlarm, rFUSerial=rFUSerial, radioLicenseModulation=radioLicenseModulation, wirelessReceiveSignalStatus=wirelessReceiveSignalStatus, managementPortDisabledWarning=managementPortDisabledWarning, sMTPServerIPAddress=sMTPServerIPAddress, protectionInterfaceStatus=protectionInterfaceStatus, useVLANForManagementInterfaces=useVLANForManagementInterfaces, sNTPServerTableNumber=sNTPServerTableNumber, dataPortConfigurationMismatch=dataPortConfigurationMismatch, remotePrimaryIPAddress=remotePrimaryIPAddress, maximumTransmitPower=maximumTransmitPower, linkLoss=linkLoss, tFTPServerIPAddress=tFTPServerIPAddress, protectionSwitchCause=protectionSwitchCause, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, radioLicenseBand=radioLicenseBand, faultProtectionSwitching=faultProtectionSwitching, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, sNTPServerTimeout=sNTPServerTimeout, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap, userRole=userRole, aAAControl=aAAControl, dataPortStatus=dataPortStatus, protectionInterfaceStatusTrap=protectionInterfaceStatusTrap, remoteMaximumTransmitPower=remoteMaximumTransmitPower, longitude=longitude, localPacketFiltering=localPacketFiltering, antennaReceiveLevelDeltaThreshold=antennaReceiveLevelDeltaThreshold, timeZone=timeZone, radioLicenseMaxEIRP=radioLicenseMaxEIRP, activeWirelessLinkAvailability=activeWirelessLinkAvailability, rFURfBand=rFURfBand, rFUFanAssemblyAlarm=rFUFanAssemblyAlarm, sNMPTrapTableEntry=sNMPTrapTableEntry, linkNameMismatch=linkNameMismatch, rFUTxPowerMin=rFUTxPowerMin, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, wirelessLinkAvailability=wirelessLinkAvailability, iPv6PrefixLength=iPv6PrefixLength, vLANManagementPriority=vLANManagementPriority, managementPortStatus=managementPortStatus, protection=protection, dataPortDisabledWarning=dataPortDisabledWarning, daylightSaving=daylightSaving, configuration=configuration, l2CPPriorityTableNumber=l2CPPriorityTableNumber, rFUBranchingConfiguration=rFUBranchingConfiguration, phyStatus=phyStatus, sNTPPrimaryServer=sNTPPrimaryServer, encryptionEnabledMismatchTrap=encryptionEnabledMismatchTrap, alignmentModeTrap=alignmentModeTrap, remotePrimaryInternetAddress=remotePrimaryInternetAddress, rFUCableAttenuationAdjustAlarm=rFUCableAttenuationAdjustAlarm, rFURxFreqMin=rFURxFreqMin, rfu=rfu, sNMPEnabledTraps=sNMPEnabledTraps, sNTPPollInterval=sNTPPollInterval, rxDiversityDataPortStatus=rxDiversityDataPortStatus, l2CPPriorityTable=l2CPPriorityTable, sntp=sntp, transmitCapacityLimit=transmitCapacityLimit, hTTPSAccessEnabled=hTTPSAccessEnabled, protectionStateTrap=protectionStateTrap, hTTPSPortNumber=hTTPSPortNumber, remoteSecondaryInternetAddressType=remoteSecondaryInternetAddressType, ethernetPriorityTableNumber=ethernetPriorityTableNumber, l2CPPriorityTableEntry=l2CPPriorityTableEntry, protectionAvailabilityStatusTrap=protectionAvailabilityStatusTrap, ethernetPriorityTableEntry=ethernetPriorityTableEntry, wirelessLinkStatusTrap=wirelessLinkStatusTrap, licensedTransmitCapacityStatusTrap=licensedTransmitCapacityStatusTrap, vLANManagementVIDValidation=vLANManagementVIDValidation, iPv6GatewayAddress=iPv6GatewayAddress)
mibBuilder.exportSymbols("CAMBIUM-PTP800-MIB", primaryRecovery=primaryRecovery, userTable=userTable, rFUTxFreqMin=rFUTxFreqMin, iPDSCPPriorityTable=iPDSCPPriorityTable, sNTPState=sNTPState, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, rFUTransceiverLocation=rFUTransceiverLocation, antennaGain=antennaGain, iPv6Address=iPv6Address, softwareVersionRemoteCompatibility=softwareVersionRemoteCompatibility, dataPortFiberStatus=dataPortFiberStatus, sNTPServerTableIndex=sNTPServerTableIndex, neighborIPAddress=neighborIPAddress, fiberY=fiberY, rFURxFreqMax=rFURxFreqMax, protectionStats=protectionStats, rFUTxIFSynthLockAlarm=rFUTxIFSynthLockAlarm, enableTransmission=enableTransmission, rFUVersionBank2=rFUVersionBank2, radioLicenseMinCodeRate=radioLicenseMinCodeRate, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, sNMPTrapPortNumber=sNMPTrapPortNumber, activeCounterMeasurementPeriod=activeCounterMeasurementPeriod, radioLicenseCodeRate=radioLicenseCodeRate, endWirelessReceiveSignalStatusTrap=endWirelessReceiveSignalStatusTrap, rxDiversityVid=rxDiversityVid, managementPortStatusTrap=managementPortStatusTrap, hTTPAccessEnabled=hTTPAccessEnabled, licence=licence, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, aggregateDataRate=aggregateDataRate, rFUPlatformCompatibility=rFUPlatformCompatibility, encryption=encryption, bridge=bridge, diverseAntennaGain=diverseAntennaGain, ptp=ptp, rFUStatusTrap=rFUStatusTrap, syslogState=syslogState, remoteInternetAddress=remoteInternetAddress, supplementary=supplementary, rFURFSwitchAlarm=rFURFSwitchAlarm, dataPortPauseFrames=dataPortPauseFrames, rFUType=rFUType, systemClock=systemClock, rxDiversityVlanTpidNeighborCompatibility=rxDiversityVlanTpidNeighborCompatibility, ethernetFiberSupport=ethernetFiberSupport, remoteIPAddress=remoteIPAddress, userTableIndex=userTableIndex, ptpTrapPrefix=ptpTrapPrefix, rFUTxBandAboveRx=rFUTxBandAboveRx, radioLicenseMaxModulation=radioLicenseMaxModulation, radioLicenseRegion=radioLicenseRegion, enabledDiagnosticProtectionAlarms=enabledDiagnosticProtectionAlarms, hTTPPortNumber=hTTPPortNumber, rFUFreqSpacing=rFUFreqSpacing, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, linkNameMismatchTrap=linkNameMismatchTrap, rFUTxPowerAlarm=rFUTxPowerAlarm, radioLicenseIdentifier=radioLicenseIdentifier, rFULockoutAlarm=rFULockoutAlarm, latitude=latitude, sNMPTrapVersion=sNMPTrapVersion, mPLSTCPriorityTable=mPLSTCPriorityTable, rFUTxMuteStatus=rFUTxMuteStatus, bootVersion=bootVersion, sMTPServerInternetAddressType=sMTPServerInternetAddressType, rFUReceiveBranchingUnitLoss=rFUReceiveBranchingUnitLoss, rxDiversityConfigurationStatusTrap=rxDiversityConfigurationStatusTrap, ptp800=ptp800, sNTPSync=sNTPSync, dataPortStatusTrap=dataPortStatusTrap, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, userTableEntry=userTableEntry, rxDiversityConfigurationStatus=rxDiversityConfigurationStatus, rFUPowerAlarm=rFUPowerAlarm, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, remoteInternetAddressType=remoteInternetAddressType, sNMPTrapTableIndex=sNMPTrapTableIndex, receiveDataRate=receiveDataRate, rxDiversityNeighborCompatibility=rxDiversityNeighborCompatibility, rxDiversityAvailabilityStatus=rxDiversityAvailabilityStatus, licensedTransmitCapacityStatus=licensedTransmitCapacityStatus, sNTPLastSync=sNTPLastSync, aTPCEnable=aTPCEnable, syslogClient=syslogClient, wirelessLinkStatus=wirelessLinkStatus, transmitCapacityLimitDetail=transmitCapacityLimitDetail, enabledDiagnosticAlarms=enabledDiagnosticAlarms, sMTPEnabledMessages=sMTPEnabledMessages, endId=endId)
