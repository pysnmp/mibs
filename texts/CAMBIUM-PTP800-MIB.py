...
mibBuilder.exportSymbols("CAMBIUM-PTP800-MIB", alignmentModeTrap=alignmentModeTrap, vLANManagementPriority=vLANManagementPriority, sMTPServerInternetAddressType=sMTPServerInternetAddressType, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sNTPServerInternetAddress=sNTPServerInternetAddress, receiveDataRate=receiveDataRate, remoteIPAddress=remoteIPAddress, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap, radioLicenseMaxEIRP=radioLicenseMaxEIRP, remoteSecondaryInternetAddressType=remoteSecondaryInternetAddressType, protectionInterfaceStatusTrap=protectionInterfaceStatusTrap, telnetAccessEnabled=telnetAccessEnabled, userTable=userTable, antennaReceiveLevelDeltaThreshold=antennaReceiveLevelDeltaThreshold, primaryRecovery=primaryRecovery, encryptionEnabledMismatchTrap=encryptionEnabledMismatchTrap, neighborInternetAddress=neighborInternetAddress, minCodeRate=minCodeRate, ptp=ptp, softwareVersionRemoteCompatibilityTrap=softwareVersionRemoteCompatibilityTrap, managementPortConfigurationMismatch=managementPortConfigurationMismatch, sNMPTrapTableEntry=sNMPTrapTableEntry, rFUFanAssemblyAlarm=rFUFanAssemblyAlarm, rFUTransmitBranchingUnitLoss=rFUTransmitBranchingUnitLoss, linkNameMismatch=linkNameMismatch, enableTransmission=enableTransmission, encryptionAlgorithm=encryptionAlgorithm, syslogLocalWrappedTrap=syslogLocalWrappedTrap, radioLicenseBand=radioLicenseBand, remoteSecondaryInternetAddress=remoteSecondaryInternetAddress, rFUTxPowerMax=rFUTxPowerMax, rFUStatusTrap=rFUStatusTrap, supplementary=supplementary, radioLicenseMaxModulation=radioLicenseMaxModulation, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, rfu=rfu, secureModeAlarmTrap=secureModeAlarmTrap, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, sNMPTrapTableIndex=sNMPTrapTableIndex, iPv6Address=iPv6Address, rxDiversityDataPortStatus=rxDiversityDataPortStatus, syslogClient=syslogClient, dataPortEthernetMediaTypeToUse=dataPortEthernetMediaTypeToUse, maxCodeRate=maxCodeRate, sNMPCommunityString=sNMPCommunityString, iPVersion=iPVersion, radioLicenseIncompatibleAlarm=radioLicenseIncompatibleAlarm, wirelessLinkStatus=wirelessLinkStatus, management=management, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, pubStats=pubStats, hTTPAccessEnabled=hTTPAccessEnabled, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sNTPLastSync=sNTPLastSync, secondarySoftwareVersion=secondarySoftwareVersion, ptp800=ptp800, remoteInternetAddress=remoteInternetAddress, rxDiversityAvailabilityStatus=rxDiversityAvailabilityStatus, sNTPServerResponse=sNTPServerResponse, userTableIndex=userTableIndex, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, wirelessLinkAvailability=wirelessLinkAvailability, wirelessReceiveSignalStatusTrap=wirelessReceiveSignalStatusTrap, aTPCEnable=aTPCEnable, transmitPower=transmitPower, secureModeAlarm=secureModeAlarm, syslogState=syslogState, sNMPTrapPortNumber=sNMPTrapPortNumber, syslogStateTrap=syslogStateTrap, rFUStatus=rFUStatus, sNMPTrapIPAddress=sNMPTrapIPAddress, userEnabled=userEnabled, transmitCapacityLimit=transmitCapacityLimit, rFUCableAttenuationAdjustAlarm=rFUCableAttenuationAdjustAlarm, fiberY=fiberY, activeCounterMeasurementPeriod=activeCounterMeasurementPeriod, configuration=configuration, snmpControl=snmpControl, ethernetPriorityTableIndex=ethernetPriorityTableIndex, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, radioLicenseRxFreq=radioLicenseRxFreq, rFUCommonIFSynthLockAlarm=rFUCommonIFSynthLockAlarm, rFUVersionBank2=rFUVersionBank2, rFUBranchingConfiguration=rFUBranchingConfiguration, radioLicenseCodeRate=radioLicenseCodeRate, ptpTraps=ptpTraps, sNTPServerTimeout=sNTPServerTimeout, remotePrimarySecondaryMode=remotePrimarySecondaryMode, dataPortPauseFrames=dataPortPauseFrames, sMTPServerInternetAddress=sMTPServerInternetAddress, radioLicenseTxFreq=radioLicenseTxFreq, protectionAvailabilityStatus=protectionAvailabilityStatus, receivePower=receivePower, rFUTxPowerDegradedAlarm=rFUTxPowerDegradedAlarm, alignmentMode=alignmentMode, alarms=alarms, protectionConfigurationStatusTrap=protectionConfigurationStatusTrap, smtp=smtp, l2CPPriorityTableEntry=l2CPPriorityTableEntry, gatewayIPAddress=gatewayIPAddress, activeAvailableTime=activeAvailableTime, latitude=latitude, ethernetPriorityTableEntry=ethernetPriorityTableEntry, activeUnit=activeUnit, ptpTrapPrefix=ptpTrapPrefix, iFCableLength=iFCableLength, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, sNTPServerTableNumber=sNTPServerTableNumber, useVLANForManagementInterfaces=useVLANForManagementInterfaces, rFURxFreqMax=rFURxFreqMax, remotePrimaryIPAddress=remotePrimaryIPAddress, vectorError=vectorError, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, sNMPPortNumber=sNMPPortNumber, productName=productName, dataPortDisabledWarning=dataPortDisabledWarning, sNTPServerTableEntry=sNTPServerTableEntry, rFUPlatformCompatibility=rFUPlatformCompatibility, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, radioLicenseMaxCodeRate=radioLicenseMaxCodeRate, rxDiversityConfigurationStatusTrap=rxDiversityConfigurationStatusTrap, productVariant=productVariant, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, ptmp=ptmp, rFUTxPowerMin=rFUTxPowerMin, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, minModulation=minModulation, longitude=longitude, softwareVersion=softwareVersion, encryption=encryption, userTableNumber=userTableNumber, protectionSwitchCause=protectionSwitchCause, neighborInternetAddressType=neighborInternetAddressType, remoteMaximumTransmitPower=remoteMaximumTransmitPower, diverseRfFeederLoss=diverseRfFeederLoss, dataPortFiberStatus=dataPortFiberStatus, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, userTableEntry=userTableEntry, rFUTxPowerAlarm=rFUTxPowerAlarm, hardwareVersion=hardwareVersion, syslogControl=syslogControl, rFURxFreqMin=rFURxFreqMin, antennaReceiveLevelMeasurementWindow=antennaReceiveLevelMeasurementWindow, managementPortAutoNegAdvertisement=managementPortAutoNegAdvertisement, dataPortCopperForcedConfiguration=dataPortCopperForcedConfiguration, managementPortEthernetSpeedStatusTrap=managementPortEthernetSpeedStatusTrap, reset=reset, altitude=altitude, tFTPServerPortNumber=tFTPServerPortNumber, localPacketFiltering=localPacketFiltering, syslogClientTrap=syslogClientTrap, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, unitOutOfCalibration=unitOutOfCalibration, vLANManagementVIDValidation=vLANManagementVIDValidation, wirelessReceiveSignalStatus=wirelessReceiveSignalStatus, rFUPowerAlarm=rFUPowerAlarm, endWirelessReceiveSignalStatusTrap=endWirelessReceiveSignalStatusTrap, ethernetPriorityTable=ethernetPriorityTable, remoteMACAddress=remoteMACAddress, rFULockoutAlarm=rFULockoutAlarm, cambium=cambium, sNMPTrapTable=sNMPTrapTable, radioLicenseRegion=radioLicenseRegion, softwareVersionRemoteCompatibility=softwareVersionRemoteCompatibility, versions=versions, endWirelessReceiveSignalStatus=endWirelessReceiveSignalStatus, transmitCapacityLimitDetail=transmitCapacityLimitDetail, timeZone=timeZone, rFUPowerButtonPressed=rFUPowerButtonPressed, primaryRecoveryPeriod=primaryRecoveryPeriod, protectionStateTrap=protectionStateTrap, userRole=userRole, remotePrimaryInternetAddress=remotePrimaryInternetAddress, protectionAvailabilityStatusTrap=protectionAvailabilityStatusTrap, transmitterStatus=transmitterStatus, hTTPPortNumber=hTTPPortNumber, rFURxRFSynthLockAlarm=rFURxRFSynthLockAlarm, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, sMTPEnabledMessages=sMTPEnabledMessages, protectionConfigurationStatus=protectionConfigurationStatus, hTTPSAccessEnabled=hTTPSAccessEnabled, rFUTxBandAboveRx=rFUTxBandAboveRx, userName=userName, radioLicenseModulation=radioLicenseModulation, rFURxIFSynthLockAlarm=rFURxIFSynthLockAlarm, linkLoss=linkLoss, systemReset=systemReset, sMTPEmailAlert=sMTPEmailAlert, sMTPServerIPAddress=sMTPServerIPAddress, qOSPriorityScheme=qOSPriorityScheme, sNTPSync=sNTPSync, rxDiversityConfigurationStatus=rxDiversityConfigurationStatus, managementPortStatus=managementPortStatus, receiveModulation=receiveModulation, rFUTxFreqMax=rFUTxFreqMax, rFUType=rFUType, enabledDiagnosticAlarms=enabledDiagnosticAlarms, iPDSCPPriorityTable=iPDSCPPriorityTable, licence=licence, dataPortCopperAutoNegotiation=dataPortCopperAutoNegotiation, faultProtectionSwitching=faultProtectionSwitching, sNMPTrapTableNumber=sNMPTrapTableNumber, l2CPPriorityTable=l2CPPriorityTable, primarySecondaryMode=primarySecondaryMode, rFURfBand=rFURfBand, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, tFTPServerInternetAddress=tFTPServerInternetAddress, sNTPServerPortNumber=sNTPServerPortNumber, phyControl=phyControl, rFUReceiveBranchingUnitLoss=rFUReceiveBranchingUnitLoss, receiveCodeRate=receiveCodeRate, protection=protection, dSCPManagementPriority=dSCPManagementPriority, transmitModulation=transmitModulation, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, encryptionEnabledMismatch=encryptionEnabledMismatch, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, managementPortAutoNegotiation=managementPortAutoNegotiation, sNTPServerInternetAddressType=sNTPServerInternetAddressType, numberOfAntennas=numberOfAntennas, diverseAntennaGain=diverseAntennaGain, l2CPPriorityTableNumber=l2CPPriorityTableNumber, daylightSaving=daylightSaving, antennaGain=antennaGain, transmitDataRate=transmitDataRate, licensedTransmitCapacityStatus=licensedTransmitCapacityStatus, dataPortStatus=dataPortStatus, sNMPTrapVersion=sNMPTrapVersion, rFUTxMuteStatus=rFUTxMuteStatus, managementPortEthernetSpeedStatus=managementPortEthernetSpeedStatus, rFUVersionBank1=rFUVersionBank1, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, dataPortEthernetSpeedStatus=dataPortEthernetSpeedStatus, wirelessLinkStatusTrap=wirelessLinkStatusTrap, ethernetPriorityTableNumber=ethernetPriorityTableNumber, iPv6GatewayAddress=iPv6GatewayAddress, securityLevel=securityLevel, transmitCodeRate=transmitCodeRate, aggregateDataRate=aggregateDataRate, mPLSTCPriorityTable=mPLSTCPriorityTable, protectionState=protectionState, rFURFSwitchAlarm=rFURFSwitchAlarm, protectionInterfaceStatus=protectionInterfaceStatus, userPassword=userPassword, rFUFreqSpacing=rFUFreqSpacing, sntp=sntp, enabledDiagnosticProtectionAlarms=enabledDiagnosticProtectionAlarms, licensedTransmitCapacityStatusTrap=licensedTransmitCapacityStatusTrap, rFURpsAlarm=rFURpsAlarm, linkName=linkName, siteName=siteName)
mibBuilder.exportSymbols("CAMBIUM-PTP800-MIB", transmitCapacity=transmitCapacity, radioLicenseIdentifier=radioLicenseIdentifier, phyStatus=phyStatus, radioLicenseMinCodeRate=radioLicenseMinCodeRate, vLANManagementVID=vLANManagementVID, dataPortFiberStatusTrap=dataPortFiberStatusTrap, activeWirelessLinkAvailability=activeWirelessLinkAvailability, bridge=bridge, l2CPPriorityTableIndex=l2CPPriorityTableIndex, managementPortStatusTrap=managementPortStatusTrap, rxDiversityVidNeighborCompatibility=rxDiversityVidNeighborCompatibility, dataPortWirelessDownAlert=dataPortWirelessDownAlert, rFUTxIFSynthLockAlarm=rFUTxIFSynthLockAlarm, rFUConnectedPlatform=rFUConnectedPlatform, sNTPServerTableIndex=sNTPServerTableIndex, protectionStats=protectionStats, hTTPSPortNumber=hTTPSPortNumber, rFUSerial=rFUSerial, sMTPServerPortNumber=sMTPServerPortNumber, remoteInternetAddressType=remoteInternetAddressType, rFUActiveFirmwareBank=rFUActiveFirmwareBank, tFTPServerInternetAddressType=tFTPServerInternetAddressType, managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, sNTPPollInterval=sNTPPollInterval, rFUCableAlarm=rFUCableAlarm, rxDiversityVlanTpidNeighborCompatibility=rxDiversityVlanTpidNeighborCompatibility, sMTPSourceEmailAddress=sMTPSourceEmailAddress, rxDiversityAvailabilityStatusTrap=rxDiversityAvailabilityStatusTrap, subnetMask=subnetMask, managementMode=managementMode, sNTPSyncTrap=sNTPSyncTrap, sNTPServerTable=sNTPServerTable, protectionStatus=protectionStatus, sNTPState=sNTPState, rxDiversityVid=rxDiversityVid, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, aAAControl=aAAControl, sNMPTrapInternetAddress=sNMPTrapInternetAddress, dataPortCopperAutoNegAdvertisement=dataPortCopperAutoNegAdvertisement, sNMPEnabledTraps=sNMPEnabledTraps, systemClock=systemClock, rFUCommonRFSynthLockAlarm=rFUCommonRFSynthLockAlarm, rFURfSwitchCableAlarm=rFURfSwitchCableAlarm, radioLicenseMinModulation=radioLicenseMinModulation, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, rFUTxRFSynthLockAlarm=rFUTxRFSynthLockAlarm, managementPortWirelessDownAlert=managementPortWirelessDownAlert, sNTPServerRetries=sNTPServerRetries, ethernetFiberSupport=ethernetFiberSupport, dataPortStatusTrap=dataPortStatusTrap, dataPortEthernetSpeedStatusTrap=dataPortEthernetSpeedStatusTrap, rFUProtectionCompatibility=rFUProtectionCompatibility, radioLicense=radioLicense, rxDiversityVlanTpid=rxDiversityVlanTpid, remoteSecondaryMACAddress=remoteSecondaryMACAddress, neighborMACAddress=neighborMACAddress, rxDiversityNeighborCompatibility=rxDiversityNeighborCompatibility, rFFeederLoss=rFFeederLoss, remotePrimaryInternetAddressType=remotePrimaryInternetAddressType, sNTPPrimaryServer=sNTPPrimaryServer, remoteSecondaryIPAddress=remoteSecondaryIPAddress, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, iPv4Address=iPv4Address, rFUPowerButtonPressedTrap=rFUPowerButtonPressedTrap, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, tFTPServerIPAddress=tFTPServerIPAddress, requestedProtectionState=requestedProtectionState, managementPortDisabledWarning=managementPortDisabledWarning, rFUTxFreqMin=rFUTxFreqMin, rxDiversityDataPortStatusTrap=rxDiversityDataPortStatusTrap, neighborIPAddress=neighborIPAddress, managementPortForcedConfiguration=managementPortForcedConfiguration, ethernet=ethernet, maximumTransmitPower=maximumTransmitPower, remotePrimaryMACAddress=remotePrimaryMACAddress, iPv6PrefixLength=iPv6PrefixLength, endId=endId, encryptionAlgorithmsAvail=encryptionAlgorithmsAvail, maxModulation=maxModulation, rFUHighTemperatureAlarm=rFUHighTemperatureAlarm, sNTPServerIPAddress=sNTPServerIPAddress, radioLicenseBandwidth=radioLicenseBandwidth, managementCommittedInformationRate=managementCommittedInformationRate, telnetPortNumber=telnetPortNumber, rFUTransceiverLocation=rFUTransceiverLocation, protectionConfig=protectionConfig, bootVersion=bootVersion, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, linkNameMismatchTrap=linkNameMismatchTrap, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress)
