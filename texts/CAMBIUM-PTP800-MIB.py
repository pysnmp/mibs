...
mibBuilder.exportSymbols("CAMBIUM-PTP800-MIB", encryptionAlgorithm=encryptionAlgorithm, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, rFURxIFSynthLockAlarm=rFURxIFSynthLockAlarm, iPv4Address=iPv4Address, rxDiversityVlanTpidNeighborCompatibility=rxDiversityVlanTpidNeighborCompatibility, syslogStateTrap=syslogStateTrap, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, dataPortConfigurationMismatch=dataPortConfigurationMismatch, radioLicenseBandwidth=radioLicenseBandwidth, managementPortStatus=managementPortStatus, l2CPPriorityTableNumber=l2CPPriorityTableNumber, remoteSecondaryInternetAddressType=remoteSecondaryInternetAddressType, primaryRecoveryPeriod=primaryRecoveryPeriod, userPassword=userPassword, iPDSCPPriorityTable=iPDSCPPriorityTable, maxCodeRate=maxCodeRate, tFTPServerPortNumber=tFTPServerPortNumber, softwareVersion=softwareVersion, dataPortCopperForcedConfiguration=dataPortCopperForcedConfiguration, ethernetPriorityTable=ethernetPriorityTable, rFUConnectedPlatform=rFUConnectedPlatform, rFUTxRFSynthLockAlarm=rFUTxRFSynthLockAlarm, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, dataPortStatusTrap=dataPortStatusTrap, licence=licence, configuration=configuration, faultProtectionSwitching=faultProtectionSwitching, rxDiversityVlanTpid=rxDiversityVlanTpid, phyStatus=phyStatus, userTable=userTable, rFUCommonIFSynthLockAlarm=rFUCommonIFSynthLockAlarm, ptp=ptp, transmitDataRate=transmitDataRate, vLANManagementVID=vLANManagementVID, neighborMACAddress=neighborMACAddress, rxDiversityDataPortStatus=rxDiversityDataPortStatus, sNMPCommunityString=sNMPCommunityString, dataPortPauseFrames=dataPortPauseFrames, managementPortConfigurationMismatch=managementPortConfigurationMismatch, sNMPTrapPortNumber=sNMPTrapPortNumber, sNTPServerInternetAddressType=sNTPServerInternetAddressType, hardwareVersion=hardwareVersion, daylightSaving=daylightSaving, sNTPSyncTrap=sNTPSyncTrap, productName=productName, activeWirelessLinkAvailability=activeWirelessLinkAvailability, managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, dataPortCopperAutoNegAdvertisement=dataPortCopperAutoNegAdvertisement, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, remoteInternetAddressType=remoteInternetAddressType, remotePrimaryMACAddress=remotePrimaryMACAddress, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, maxModulation=maxModulation, linkName=linkName, remoteInternetAddress=remoteInternetAddress, sNMPTrapInternetAddress=sNMPTrapInternetAddress, hTTPSAccessEnabled=hTTPSAccessEnabled, rxDiversityConfigurationStatus=rxDiversityConfigurationStatus, telnetAccessEnabled=telnetAccessEnabled, remoteSecondaryIPAddress=remoteSecondaryIPAddress, managementMode=managementMode, radioLicenseTxFreq=radioLicenseTxFreq, wirelessReceiveSignalStatus=wirelessReceiveSignalStatus, secondarySoftwareVersion=secondarySoftwareVersion, sMTPServerPortNumber=sMTPServerPortNumber, rFUBranchingConfiguration=rFUBranchingConfiguration, protectionConfigurationStatus=protectionConfigurationStatus, rFUTransceiverLocation=rFUTransceiverLocation, sNTPState=sNTPState, activeUnit=activeUnit, iPv6GatewayAddress=iPv6GatewayAddress, l2CPPriorityTableEntry=l2CPPriorityTableEntry, encryptionAlgorithmsAvail=encryptionAlgorithmsAvail, syslogLocalWrappedTrap=syslogLocalWrappedTrap, rFUTxMuteStatus=rFUTxMuteStatus, sMTPEnabledMessages=sMTPEnabledMessages, alignmentMode=alignmentMode, sNTPServerResponse=sNTPServerResponse, rFURxFreqMin=rFURxFreqMin, receivePower=receivePower, radioLicenseMaxCodeRate=radioLicenseMaxCodeRate, cambium=cambium, hTTPAccessEnabled=hTTPAccessEnabled, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, fiberY=fiberY, linkNameMismatchTrap=linkNameMismatchTrap, receiveDataRate=receiveDataRate, ethernet=ethernet, protectionStats=protectionStats, sNTPServerTableEntry=sNTPServerTableEntry, rFUStatus=rFUStatus, rFUPowerButtonPressed=rFUPowerButtonPressed, rFUReceiveBranchingUnitLoss=rFUReceiveBranchingUnitLoss, sNMPPortNumber=sNMPPortNumber, managementPortAutoNegotiation=managementPortAutoNegotiation, antennaReceiveLevelDeltaThreshold=antennaReceiveLevelDeltaThreshold, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, userEnabled=userEnabled, productVariant=productVariant, smtp=smtp, syslogClient=syslogClient, ethernetPriorityTableIndex=ethernetPriorityTableIndex, l2CPPriorityTable=l2CPPriorityTable, management=management, managementPortEthernetSpeedStatusTrap=managementPortEthernetSpeedStatusTrap, sNTPSync=sNTPSync, managementPortAutoNegAdvertisement=managementPortAutoNegAdvertisement, wirelessLinkStatus=wirelessLinkStatus, rFUTxPowerMax=rFUTxPowerMax, diverseRfFeederLoss=diverseRfFeederLoss, aAAControl=aAAControl, transmitModulation=transmitModulation, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, sNMPTrapIPAddress=sNMPTrapIPAddress, protectionInterfaceStatusTrap=protectionInterfaceStatusTrap, radioLicenseRegion=radioLicenseRegion, ethernetFiberSupport=ethernetFiberSupport, sMTPServerInternetAddress=sMTPServerInternetAddress, sNTPServerInternetAddress=sNTPServerInternetAddress, telnetPortNumber=telnetPortNumber, licensedTransmitCapacityStatus=licensedTransmitCapacityStatus, hTTPSPortNumber=hTTPSPortNumber, unitOutOfCalibration=unitOutOfCalibration, rFURfBand=rFURfBand, protectionAvailabilityStatus=protectionAvailabilityStatus, dataPortCopperAutoNegotiation=dataPortCopperAutoNegotiation, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, rFFeederLoss=rFFeederLoss, iPVersion=iPVersion, receiveCodeRate=receiveCodeRate, rFUTransmitBranchingUnitLoss=rFUTransmitBranchingUnitLoss, sMTPSourceEmailAddress=sMTPSourceEmailAddress, dataPortEthernetMediaTypeToUse=dataPortEthernetMediaTypeToUse, ethernetPriorityTableEntry=ethernetPriorityTableEntry, protectionStateTrap=protectionStateTrap, sNTPServerIPAddress=sNTPServerIPAddress, altitude=altitude, rFUPowerAlarm=rFUPowerAlarm, vLANManagementVIDValidation=vLANManagementVIDValidation, softwareVersionRemoteCompatibility=softwareVersionRemoteCompatibility, securityLevel=securityLevel, gatewayIPAddress=gatewayIPAddress, rFUTxPowerMin=rFUTxPowerMin, wirelessLinkAvailability=wirelessLinkAvailability, dataPortEthernetSpeedStatusTrap=dataPortEthernetSpeedStatusTrap, rFUTxFreqMin=rFUTxFreqMin, alarms=alarms, syslogState=syslogState, enableTransmission=enableTransmission, radioLicenseModulation=radioLicenseModulation, userName=userName, rFUFreqSpacing=rFUFreqSpacing, versions=versions, transmitPower=transmitPower, sMTPServerInternetAddressType=sMTPServerInternetAddressType, phyControl=phyControl, protection=protection, ptpTraps=ptpTraps, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, ptpTrapPrefix=ptpTrapPrefix, numberOfAntennas=numberOfAntennas, secureModeAlarm=secureModeAlarm, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sntp=sntp, rFURxRFSynthLockAlarm=rFURxRFSynthLockAlarm, rFUSerial=rFUSerial, sNMPTrapTableEntry=sNMPTrapTableEntry, alignmentModeTrap=alignmentModeTrap, ptmp=ptmp, radioLicenseBand=radioLicenseBand, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, protectionConfig=protectionConfig, wirelessLinkStatusTrap=wirelessLinkStatusTrap, vectorError=vectorError, encryption=encryption, radioLicenseMinModulation=radioLicenseMinModulation, tFTPServerIPAddress=tFTPServerIPAddress, primaryRecovery=primaryRecovery, softwareVersionRemoteCompatibilityTrap=softwareVersionRemoteCompatibilityTrap, managementPortStatusTrap=managementPortStatusTrap, iPv6Address=iPv6Address, rFUCableAlarm=rFUCableAlarm, rFUPlatformCompatibility=rFUPlatformCompatibility, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, dSCPManagementPriority=dSCPManagementPriority, rFUTxIFSynthLockAlarm=rFUTxIFSynthLockAlarm, sNTPLastSync=sNTPLastSync, rFURxFreqMax=rFURxFreqMax, rFUVersionBank1=rFUVersionBank1, sNTPServerTable=sNTPServerTable, bridge=bridge, radioLicenseMinCodeRate=radioLicenseMinCodeRate, tFTPServerInternetAddress=tFTPServerInternetAddress, rFUTxBandAboveRx=rFUTxBandAboveRx, userTableEntry=userTableEntry, rFUFanAssemblyAlarm=rFUFanAssemblyAlarm, sNTPServerRetries=sNTPServerRetries, minCodeRate=minCodeRate, remotePrimaryIPAddress=remotePrimaryIPAddress, rFUProtectionCompatibility=rFUProtectionCompatibility, secureModeAlarmTrap=secureModeAlarmTrap, rxDiversityNeighborCompatibility=rxDiversityNeighborCompatibility, aTPCEnable=aTPCEnable, linkNameMismatch=linkNameMismatch, antennaGain=antennaGain, bootVersion=bootVersion, diverseAntennaGain=diverseAntennaGain, iPv6PrefixLength=iPv6PrefixLength, remotePrimaryInternetAddressType=remotePrimaryInternetAddressType, userTableIndex=userTableIndex, rFURfSwitchCableAlarm=rFURfSwitchCableAlarm, reset=reset, rFUStatusTrap=rFUStatusTrap, dataPortEthernetSpeedStatus=dataPortEthernetSpeedStatus, protectionConfigurationStatusTrap=protectionConfigurationStatusTrap, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, rFUVersionBank2=rFUVersionBank2, minModulation=minModulation, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, rFUTxFreqMax=rFUTxFreqMax, requestedProtectionState=requestedProtectionState, localPacketFiltering=localPacketFiltering, radioLicenseMaxModulation=radioLicenseMaxModulation, protectionInterfaceStatus=protectionInterfaceStatus, iFCableLength=iFCableLength, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, hTTPPortNumber=hTTPPortNumber, dataPortFiberStatusTrap=dataPortFiberStatusTrap, primarySecondaryMode=primarySecondaryMode, encryptionEnabledMismatch=encryptionEnabledMismatch, snmpControl=snmpControl, rFUType=rFUType, dataPortWirelessDownAlert=dataPortWirelessDownAlert, supplementary=supplementary, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, rFUCommonRFSynthLockAlarm=rFUCommonRFSynthLockAlarm, transmitCapacity=transmitCapacity, remotePrimarySecondaryMode=remotePrimarySecondaryMode, transmitCodeRate=transmitCodeRate, timeZone=timeZone, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, enabledDiagnosticProtectionAlarms=enabledDiagnosticProtectionAlarms, userTableNumber=userTableNumber, sNTPServerPortNumber=sNTPServerPortNumber, radioLicenseMaxEIRP=radioLicenseMaxEIRP, ptp800=ptp800, endWirelessReceiveSignalStatusTrap=endWirelessReceiveSignalStatusTrap, linkLoss=linkLoss, sNMPTrapTable=sNMPTrapTable, remoteMACAddress=remoteMACAddress, l2CPPriorityTableIndex=l2CPPriorityTableIndex)
mibBuilder.exportSymbols("CAMBIUM-PTP800-MIB", sNTPServerTableNumber=sNTPServerTableNumber, wirelessReceiveSignalStatusTrap=wirelessReceiveSignalStatusTrap, endId=endId, managementPortEthernetSpeedStatus=managementPortEthernetSpeedStatus, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, siteName=siteName, transmitCapacityLimit=transmitCapacityLimit, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, rFUActiveFirmwareBank=rFUActiveFirmwareBank, remoteSecondaryMACAddress=remoteSecondaryMACAddress, transmitterStatus=transmitterStatus, remoteMaximumTransmitPower=remoteMaximumTransmitPower, rFUHighTemperatureAlarm=rFUHighTemperatureAlarm, sNTPPollInterval=sNTPPollInterval, sNTPPrimaryServer=sNTPPrimaryServer, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, rFURFSwitchAlarm=rFURFSwitchAlarm, qOSPriorityScheme=qOSPriorityScheme, radioLicenseIdentifier=radioLicenseIdentifier, userRole=userRole, transmitCapacityLimitDetail=transmitCapacityLimitDetail, protectionSwitchCause=protectionSwitchCause, radioLicense=radioLicense, dataPortFiberStatus=dataPortFiberStatus, rFURpsAlarm=rFURpsAlarm, mPLSTCPriorityTable=mPLSTCPriorityTable, sNTPServerTimeout=sNTPServerTimeout, radioLicenseCodeRate=radioLicenseCodeRate, rxDiversityVid=rxDiversityVid, endWirelessReceiveSignalStatus=endWirelessReceiveSignalStatus, rxDiversityAvailabilityStatusTrap=rxDiversityAvailabilityStatusTrap, antennaReceiveLevelMeasurementWindow=antennaReceiveLevelMeasurementWindow, receiveModulation=receiveModulation, rxDiversityAvailabilityStatus=rxDiversityAvailabilityStatus, tFTPServerInternetAddressType=tFTPServerInternetAddressType, systemReset=systemReset, enabledDiagnosticAlarms=enabledDiagnosticAlarms, protectionStatus=protectionStatus, syslogControl=syslogControl, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, radioLicenseIncompatibleAlarm=radioLicenseIncompatibleAlarm, pubStats=pubStats, managementCommittedInformationRate=managementCommittedInformationRate, remotePrimaryInternetAddress=remotePrimaryInternetAddress, longitude=longitude, rfu=rfu, dataPortDisabledWarning=dataPortDisabledWarning, remoteIPAddress=remoteIPAddress, rFUTxPowerDegradedAlarm=rFUTxPowerDegradedAlarm, activeAvailableTime=activeAvailableTime, sMTPServerIPAddress=sMTPServerIPAddress, neighborInternetAddressType=neighborInternetAddressType, remoteSecondaryInternetAddress=remoteSecondaryInternetAddress, sNMPTrapVersion=sNMPTrapVersion, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, vLANManagementPriority=vLANManagementPriority, sNMPTrapTableNumber=sNMPTrapTableNumber, systemClock=systemClock, managementPortForcedConfiguration=managementPortForcedConfiguration, rxDiversityVidNeighborCompatibility=rxDiversityVidNeighborCompatibility, rFUTxPowerAlarm=rFUTxPowerAlarm, subnetMask=subnetMask, activeCounterMeasurementPeriod=activeCounterMeasurementPeriod, licensedTransmitCapacityStatusTrap=licensedTransmitCapacityStatusTrap, sNMPEnabledTraps=sNMPEnabledTraps, syslogClientTrap=syslogClientTrap, sNTPServerTableIndex=sNTPServerTableIndex, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, managementPortDisabledWarning=managementPortDisabledWarning, sNMPTrapTableIndex=sNMPTrapTableIndex, neighborInternetAddress=neighborInternetAddress, encryptionEnabledMismatchTrap=encryptionEnabledMismatchTrap, rFUCableAttenuationAdjustAlarm=rFUCableAttenuationAdjustAlarm, rFUPowerButtonPressedTrap=rFUPowerButtonPressedTrap, rxDiversityConfigurationStatusTrap=rxDiversityConfigurationStatusTrap, ethernetPriorityTableNumber=ethernetPriorityTableNumber, rxDiversityDataPortStatusTrap=rxDiversityDataPortStatusTrap, aggregateDataRate=aggregateDataRate, managementPortWirelessDownAlert=managementPortWirelessDownAlert, sMTPEmailAlert=sMTPEmailAlert, maximumTransmitPower=maximumTransmitPower, useVLANForManagementInterfaces=useVLANForManagementInterfaces, latitude=latitude, protectionState=protectionState, protectionAvailabilityStatusTrap=protectionAvailabilityStatusTrap, radioLicenseRxFreq=radioLicenseRxFreq, neighborIPAddress=neighborIPAddress, rFULockoutAlarm=rFULockoutAlarm, dataPortStatus=dataPortStatus, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap)
