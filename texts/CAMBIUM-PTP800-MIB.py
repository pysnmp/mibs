...
mibBuilder.exportSymbols("CAMBIUM-PTP800-MIB", encryptionEnabledMismatch=encryptionEnabledMismatch, radioLicenseMaxCodeRate=radioLicenseMaxCodeRate, transmitDataRate=transmitDataRate, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, secureModeAlarmTrap=secureModeAlarmTrap, subnetMask=subnetMask, rFUCommonRFSynthLockAlarm=rFUCommonRFSynthLockAlarm, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, rFUTransceiverLocation=rFUTransceiverLocation, productVariant=productVariant, sNTPSync=sNTPSync, aAAControl=aAAControl, tFTPServerInternetAddressType=tFTPServerInternetAddressType, sNTPLastSync=sNTPLastSync, sMTPEnabledMessages=sMTPEnabledMessages, minModulation=minModulation, remotePrimaryMACAddress=remotePrimaryMACAddress, sntp=sntp, remoteSecondaryInternetAddressType=remoteSecondaryInternetAddressType, ptp=ptp, ethernetPriorityTableIndex=ethernetPriorityTableIndex, mPLSTCPriorityTable=mPLSTCPriorityTable, userTable=userTable, protection=protection, managementPortConfigurationMismatch=managementPortConfigurationMismatch, hTTPPortNumber=hTTPPortNumber, softwareVersionRemoteCompatibility=softwareVersionRemoteCompatibility, faultProtectionSwitching=faultProtectionSwitching, radioLicenseMaxModulation=radioLicenseMaxModulation, diverseRfFeederLoss=diverseRfFeederLoss, sMTPSourceEmailAddress=sMTPSourceEmailAddress, rFUStatus=rFUStatus, rFUTxPowerMin=rFUTxPowerMin, wirelessReceiveSignalStatusTrap=wirelessReceiveSignalStatusTrap, sNTPServerIPAddress=sNTPServerIPAddress, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, rFUBranchingConfiguration=rFUBranchingConfiguration, requestedProtectionState=requestedProtectionState, ptp800=ptp800, encryption=encryption, rFUCableAttenuationAdjustAlarm=rFUCableAttenuationAdjustAlarm, aTPCEnable=aTPCEnable, cambium=cambium, rFUTxBandAboveRx=rFUTxBandAboveRx, sNTPServerPortNumber=sNTPServerPortNumber, rFUSerial=rFUSerial, linkLoss=linkLoss, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, dataPortFiberStatus=dataPortFiberStatus, siteName=siteName, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, rFUProtectionCompatibility=rFUProtectionCompatibility, maximumTransmitPower=maximumTransmitPower, l2CPPriorityTableIndex=l2CPPriorityTableIndex, protectionConfig=protectionConfig, syslogControl=syslogControl, rFURxRFSynthLockAlarm=rFURxRFSynthLockAlarm, rFUCableAlarm=rFUCableAlarm, sNTPPrimaryServer=sNTPPrimaryServer, sNMPTrapVersion=sNMPTrapVersion, userRole=userRole, managementCommittedInformationRate=managementCommittedInformationRate, dataPortFiberStatusTrap=dataPortFiberStatusTrap, neighborIPAddress=neighborIPAddress, rFUVersionBank2=rFUVersionBank2, alignmentModeTrap=alignmentModeTrap, activeCounterMeasurementPeriod=activeCounterMeasurementPeriod, rFUPowerButtonPressedTrap=rFUPowerButtonPressedTrap, rFUPowerButtonPressed=rFUPowerButtonPressed, rFUHighTemperatureAlarm=rFUHighTemperatureAlarm, syslogClient=syslogClient, hardwareVersion=hardwareVersion, primaryRecoveryPeriod=primaryRecoveryPeriod, rxDiversityConfigurationStatus=rxDiversityConfigurationStatus, protectionAvailabilityStatus=protectionAvailabilityStatus, rFUTxFreqMax=rFUTxFreqMax, protectionAvailabilityStatusTrap=protectionAvailabilityStatusTrap, rFURxFreqMin=rFURxFreqMin, radioLicenseRegion=radioLicenseRegion, syslogStateTrap=syslogStateTrap, rxDiversityVidNeighborCompatibility=rxDiversityVidNeighborCompatibility, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, rFUPlatformCompatibility=rFUPlatformCompatibility, softwareVersion=softwareVersion, bridge=bridge, pubStats=pubStats, unitOutOfCalibration=unitOutOfCalibration, telnetAccessEnabled=telnetAccessEnabled, remotePrimaryIPAddress=remotePrimaryIPAddress, rxDiversityAvailabilityStatus=rxDiversityAvailabilityStatus, minCodeRate=minCodeRate, rFUReceiveBranchingUnitLoss=rFUReceiveBranchingUnitLoss, protectionStats=protectionStats, protectionSwitchCause=protectionSwitchCause, dataPortEthernetSpeedStatusTrap=dataPortEthernetSpeedStatusTrap, hTTPAccessEnabled=hTTPAccessEnabled, iPVersion=iPVersion, managementPortEthernetSpeedStatus=managementPortEthernetSpeedStatus, vLANManagementVIDValidation=vLANManagementVIDValidation, sNTPServerTableNumber=sNTPServerTableNumber, softwareVersionRemoteCompatibilityTrap=softwareVersionRemoteCompatibilityTrap, numberOfAntennas=numberOfAntennas, sNTPPollInterval=sNTPPollInterval, systemReset=systemReset, securityLevel=securityLevel, managementPortWirelessDownAlert=managementPortWirelessDownAlert, sNMPTrapTableEntry=sNMPTrapTableEntry, primarySecondaryMode=primarySecondaryMode, timeZone=timeZone, hTTPSAccessEnabled=hTTPSAccessEnabled, sNMPPortNumber=sNMPPortNumber, sMTPServerIPAddress=sMTPServerIPAddress, managementPortStatus=managementPortStatus, radioLicenseMinCodeRate=radioLicenseMinCodeRate, vLANManagementVID=vLANManagementVID, protectionInterfaceStatus=protectionInterfaceStatus, rFURxIFSynthLockAlarm=rFURxIFSynthLockAlarm, neighborInternetAddress=neighborInternetAddress, ethernetPriorityTable=ethernetPriorityTable, l2CPPriorityTableNumber=l2CPPriorityTableNumber, rFUTxPowerDegradedAlarm=rFUTxPowerDegradedAlarm, rxDiversityVlanTpid=rxDiversityVlanTpid, activeWirelessLinkAvailability=activeWirelessLinkAvailability, sNTPServerInternetAddress=sNTPServerInternetAddress, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, antennaGain=antennaGain, useVLANForManagementInterfaces=useVLANForManagementInterfaces, encryptionEnabledMismatchTrap=encryptionEnabledMismatchTrap, sNTPSyncTrap=sNTPSyncTrap, dataPortConfigurationMismatch=dataPortConfigurationMismatch, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, radioLicenseBandwidth=radioLicenseBandwidth, daylightSaving=daylightSaving, rFUPowerAlarm=rFUPowerAlarm, sNTPState=sNTPState, phyControl=phyControl, receiveModulation=receiveModulation, hTTPSPortNumber=hTTPSPortNumber, rFURxFreqMax=rFURxFreqMax, sNMPTrapTable=sNMPTrapTable, rxDiversityVid=rxDiversityVid, transmitterStatus=transmitterStatus, iPv4Address=iPv4Address, userTableNumber=userTableNumber, rxDiversityAvailabilityStatusTrap=rxDiversityAvailabilityStatusTrap, tFTPServerIPAddress=tFTPServerIPAddress, maxModulation=maxModulation, iPDSCPPriorityTable=iPDSCPPriorityTable, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, rFUFreqSpacing=rFUFreqSpacing, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, ethernetPriorityTableNumber=ethernetPriorityTableNumber, remoteSecondaryIPAddress=remoteSecondaryIPAddress, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, rxDiversityDataPortStatus=rxDiversityDataPortStatus, radioLicenseTxFreq=radioLicenseTxFreq, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, wirelessLinkAvailability=wirelessLinkAvailability, secondarySoftwareVersion=secondarySoftwareVersion, remoteInternetAddress=remoteInternetAddress, productName=productName, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, phyStatus=phyStatus, licence=licence, linkNameMismatch=linkNameMismatch, managementPortForcedConfiguration=managementPortForcedConfiguration, dataPortStatusTrap=dataPortStatusTrap, activeAvailableTime=activeAvailableTime, syslogClientTrap=syslogClientTrap, rxDiversityDataPortStatusTrap=rxDiversityDataPortStatusTrap, endId=endId, userTableIndex=userTableIndex, sMTPServerInternetAddress=sMTPServerInternetAddress, protectionInterfaceStatusTrap=protectionInterfaceStatusTrap, dataPortCopperAutoNegAdvertisement=dataPortCopperAutoNegAdvertisement, configuration=configuration, rFUTxRFSynthLockAlarm=rFUTxRFSynthLockAlarm, sNMPTrapIPAddress=sNMPTrapIPAddress, managementPortAutoNegAdvertisement=managementPortAutoNegAdvertisement, managementPortStatusTrap=managementPortStatusTrap, aggregateDataRate=aggregateDataRate, userTableEntry=userTableEntry, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, sMTPEmailAlert=sMTPEmailAlert, ptpTraps=ptpTraps, rFUVersionBank1=rFUVersionBank1, rFUTransmitBranchingUnitLoss=rFUTransmitBranchingUnitLoss, dataPortEthernetSpeedStatus=dataPortEthernetSpeedStatus, radioLicenseIncompatibleAlarm=radioLicenseIncompatibleAlarm, reset=reset, sNTPServerTableEntry=sNTPServerTableEntry, managementPortEthernetSpeedStatusTrap=managementPortEthernetSpeedStatusTrap, longitude=longitude, ethernetPriorityTableEntry=ethernetPriorityTableEntry, neighborMACAddress=neighborMACAddress, transmitCapacityLimitDetail=transmitCapacityLimitDetail, sMTPServerPortNumber=sMTPServerPortNumber, sNTPServerTable=sNTPServerTable, supplementary=supplementary, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, remotePrimaryInternetAddressType=remotePrimaryInternetAddressType, syslogLocalWrappedTrap=syslogLocalWrappedTrap, rFUCommonIFSynthLockAlarm=rFUCommonIFSynthLockAlarm, rFURfSwitchCableAlarm=rFURfSwitchCableAlarm, licensedTransmitCapacityStatus=licensedTransmitCapacityStatus, smtp=smtp, radioLicenseRxFreq=radioLicenseRxFreq, remoteMaximumTransmitPower=remoteMaximumTransmitPower, dataPortDisabledWarning=dataPortDisabledWarning, sNTPServerTableIndex=sNTPServerTableIndex, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, ethernetFiberSupport=ethernetFiberSupport, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, protectionStateTrap=protectionStateTrap, sMTPServerInternetAddressType=sMTPServerInternetAddressType, iPv6GatewayAddress=iPv6GatewayAddress, altitude=altitude, tFTPServerInternetAddress=tFTPServerInternetAddress, protectionStatus=protectionStatus, antennaReceiveLevelMeasurementWindow=antennaReceiveLevelMeasurementWindow, iPv6Address=iPv6Address, syslogState=syslogState, l2CPPriorityTable=l2CPPriorityTable, transmitCapacity=transmitCapacity, radioLicenseMaxEIRP=radioLicenseMaxEIRP, versions=versions, sNMPEnabledTraps=sNMPEnabledTraps, managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, managementMode=managementMode, radioLicenseMinModulation=radioLicenseMinModulation, diverseAntennaGain=diverseAntennaGain, activeUnit=activeUnit, secureModeAlarm=secureModeAlarm, rxDiversityConfigurationStatusTrap=rxDiversityConfigurationStatusTrap, protectionConfigurationStatusTrap=protectionConfigurationStatusTrap, rFULockoutAlarm=rFULockoutAlarm, enableTransmission=enableTransmission, ethernet=ethernet, rFUFanAssemblyAlarm=rFUFanAssemblyAlarm, remoteInternetAddressType=remoteInternetAddressType, primaryRecovery=primaryRecovery, enabledDiagnosticAlarms=enabledDiagnosticAlarms, rfu=rfu, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, ptpTrapPrefix=ptpTrapPrefix, rFUConnectedPlatform=rFUConnectedPlatform, vectorError=vectorError, iPv6PrefixLength=iPv6PrefixLength)
mibBuilder.exportSymbols("CAMBIUM-PTP800-MIB", encryptionAlgorithm=encryptionAlgorithm, sNMPTrapTableIndex=sNMPTrapTableIndex, l2CPPriorityTableEntry=l2CPPriorityTableEntry, gatewayIPAddress=gatewayIPAddress, endWirelessReceiveSignalStatusTrap=endWirelessReceiveSignalStatusTrap, receiveCodeRate=receiveCodeRate, endWirelessReceiveSignalStatus=endWirelessReceiveSignalStatus, neighborInternetAddressType=neighborInternetAddressType, remotePrimarySecondaryMode=remotePrimarySecondaryMode, dataPortPauseFrames=dataPortPauseFrames, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, rFUTxPowerAlarm=rFUTxPowerAlarm, alignmentMode=alignmentMode, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, latitude=latitude, userEnabled=userEnabled, remoteSecondaryMACAddress=remoteSecondaryMACAddress, dataPortCopperForcedConfiguration=dataPortCopperForcedConfiguration, radioLicense=radioLicense, remotePrimaryInternetAddress=remotePrimaryInternetAddress, sNTPServerRetries=sNTPServerRetries, remoteMACAddress=remoteMACAddress, transmitCodeRate=transmitCodeRate, telnetPortNumber=telnetPortNumber, rFURpsAlarm=rFURpsAlarm, systemClock=systemClock, rFURFSwitchAlarm=rFURFSwitchAlarm, rxDiversityNeighborCompatibility=rxDiversityNeighborCompatibility, bootVersion=bootVersion, dataPortCopperAutoNegotiation=dataPortCopperAutoNegotiation, rFUActiveFirmwareBank=rFUActiveFirmwareBank, sNTPServerTimeout=sNTPServerTimeout, rFUTxMuteStatus=rFUTxMuteStatus, rFURfBand=rFURfBand, ptmp=ptmp, linkNameMismatchTrap=linkNameMismatchTrap, wirelessLinkStatusTrap=wirelessLinkStatusTrap, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, dataPortEthernetMediaTypeToUse=dataPortEthernetMediaTypeToUse, wirelessReceiveSignalStatus=wirelessReceiveSignalStatus, remoteSecondaryInternetAddress=remoteSecondaryInternetAddress, fiberY=fiberY, linkName=linkName, rxDiversityVlanTpidNeighborCompatibility=rxDiversityVlanTpidNeighborCompatibility, qOSPriorityScheme=qOSPriorityScheme, alarms=alarms, protectionConfigurationStatus=protectionConfigurationStatus, radioLicenseCodeRate=radioLicenseCodeRate, snmpControl=snmpControl, transmitModulation=transmitModulation, antennaReceiveLevelDeltaThreshold=antennaReceiveLevelDeltaThreshold, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, rFUType=rFUType, managementPortAutoNegotiation=managementPortAutoNegotiation, protectionState=protectionState, sNTPServerInternetAddressType=sNTPServerInternetAddressType, management=management, rFUTxIFSynthLockAlarm=rFUTxIFSynthLockAlarm, vLANManagementPriority=vLANManagementPriority, transmitCapacityLimit=transmitCapacityLimit, sNTPServerResponse=sNTPServerResponse, encryptionAlgorithmsAvail=encryptionAlgorithmsAvail, maxCodeRate=maxCodeRate, sNMPTrapPortNumber=sNMPTrapPortNumber, rFFeederLoss=rFFeederLoss, radioLicenseModulation=radioLicenseModulation, sNMPCommunityString=sNMPCommunityString, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap, localPacketFiltering=localPacketFiltering, radioLicenseBand=radioLicenseBand, userPassword=userPassword, licensedTransmitCapacityStatusTrap=licensedTransmitCapacityStatusTrap, dSCPManagementPriority=dSCPManagementPriority, tFTPServerPortNumber=tFTPServerPortNumber, iFCableLength=iFCableLength, managementPortDisabledWarning=managementPortDisabledWarning, rFUStatusTrap=rFUStatusTrap, receivePower=receivePower, transmitPower=transmitPower, sNMPTrapInternetAddress=sNMPTrapInternetAddress, rFUTxFreqMin=rFUTxFreqMin, enabledDiagnosticProtectionAlarms=enabledDiagnosticProtectionAlarms, rFUTxPowerMax=rFUTxPowerMax, sNMPTrapTableNumber=sNMPTrapTableNumber, userName=userName, receiveDataRate=receiveDataRate, dataPortStatus=dataPortStatus, remoteIPAddress=remoteIPAddress, radioLicenseIdentifier=radioLicenseIdentifier, wirelessLinkStatus=wirelessLinkStatus)
