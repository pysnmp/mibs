...
# Produced by pysmi-1.1.8 at Sat Jan 15 17:37:45 2022
...
mibBuilder.exportSymbols("TEGILE-MIB", lunProjectName=lunProjectName, vmwareNFSDatastoreWriteIops=vmwareNFSDatastoreWriteIops, iscsiTargetReadIops=iscsiTargetReadIops, iscsiInitiatorGroup=iscsiInitiatorGroup, diskRandomIOCount=diskRandomIOCount, vmwareNFSDatastoreEntry=vmwareNFSDatastoreEntry, diskSizeLow=diskSizeLow, snapshotRollbackFailed=snapshotRollbackFailed, iscsiTargetWriteLatency=iscsiTargetWriteLatency, initiatorGroupDeleteCompleted=initiatorGroupDeleteCompleted, notificationTargetEntityName=notificationTargetEntityName, poolPostDedupDataSizeHigh=poolPostDedupDataSizeHigh, haResourceIndex=haResourceIndex, lunTable=lunTable, poolDataReadMbps=poolDataReadMbps, cifsTotalReadMbps=cifsTotalReadMbps, tegileCompliances=tegileCompliances, projectCompressionEnabled=projectCompressionEnabled, smisServiceStopFailed=smisServiceStopFailed, lunWriteLatency=lunWriteLatency, projectEntry=projectEntry, iscsiImproperTargetGroup=iscsiImproperTargetGroup, lunDataSizeLow=lunDataSizeLow, fcInitiatorCreateCompleted=fcInitiatorCreateCompleted, notificationProps=notificationProps, fcTargetModifyCompleted=fcTargetModifyCompleted, cacheTotalWriteIops=cacheTotalWriteIops, smisServiceStarted=smisServiceStarted, shareQuotaHigh=shareQuotaHigh, notificationReadingTriggerValue=notificationReadingTriggerValue, webdocsUpgradeCompleted=webdocsUpgradeCompleted, projectUnusedReservedSizeHigh=projectUnusedReservedSizeHigh, webdocsUpgradeDownloadCompleted=webdocsUpgradeDownloadCompleted, fcAvgWriteLatency=fcAvgWriteLatency, shareEntry=shareEntry, haControllerA_Name=haControllerA_Name, shareCIFSWriteLatency=shareCIFSWriteLatency, replicationAborted=replicationAborted, poolSizeHigh=poolSizeHigh, upgradeFailed=upgradeFailed, haResources=haResources, userLoginFailed=userLoginFailed, diskIndex=diskIndex, diskEntry=diskEntry, haControllerB_IPAddr=haControllerB_IPAddr, shareTable=shareTable, shareReservedSizeHigh=shareReservedSizeHigh, tdpsUpgradeCompleted=tdpsUpgradeCompleted, iscsiTargetAuth=iscsiTargetAuth, volumeExceedsThresholdWarning=volumeExceedsThresholdWarning, replicationPaused=replicationPaused, lunCompressedRatio=lunCompressedRatio, iscsiTargetGroupError=iscsiTargetGroupError, webdocsUpgradeDownloadStarted=webdocsUpgradeDownloadStarted, fcInitiatorCreateFailed=fcInitiatorCreateFailed, snmpServiceStarted=snmpServiceStarted, nvdimmFailUnknownError=nvdimmFailUnknownError, projectDeleted=projectDeleted, tegileConformance=tegileConformance, iscsiAvgWriteLatency=iscsiAvgWriteLatency, lunWriteIops=lunWriteIops, iscsiTargetDeleteCompleted=iscsiTargetDeleteCompleted, haResourceGroup=haResourceGroup, notificationDescription=notificationDescription, notificationTime=notificationTime, networkIpmpGroupDown=networkIpmpGroupDown, replicationStarted=replicationStarted, fcInitiatorModifyCompleted=fcInitiatorModifyCompleted, shareProjectName=shareProjectName, currentNotificationGroup=currentNotificationGroup, iscsiTotalReadIops=iscsiTotalReadIops, vmwareNFSDatastoreReadIops=vmwareNFSDatastoreReadIops, iscsiTargetCreateCompleted=iscsiTargetCreateCompleted, projectThresholdExceedWarning=projectThresholdExceedWarning, lunFreeSizeHigh=lunFreeSizeHigh, notificationEventCode=notificationEventCode, projectLunCount=projectLunCount, webdocsUpgradeFailed=webdocsUpgradeFailed, targetGroupCreateCompleted=targetGroupCreateCompleted, diskDataReadMbps=diskDataReadMbps, volumeDeleteCompleted=volumeDeleteCompleted, iscsiInitiatorIndex=iscsiInitiatorIndex, fcTargetWriteLatency=fcTargetWriteLatency, poolSizeLow=poolSizeLow, diskSequentialIOCount=diskSequentialIOCount, notificationSensorSuspect=notificationSensorSuspect, poolPostDedupDataSizeLow=poolPostDedupDataSizeLow, nvdimmFailSoftError=nvdimmFailSoftError, shareDataSizeLow=shareDataSizeLow, notificationComponentName=notificationComponentName, shareCIFSReadIops=shareCIFSReadIops, diskSlowIo=diskSlowIo, projectIndex=projectIndex, poolDataReadLatency=poolDataReadLatency, replicationFailed=replicationFailed, lunName=lunName, upgradeTimeout=upgradeTimeout, controllerDown=controllerDown, snapshotCreationFailed=snapshotCreationFailed, iscsiTargetAlias=iscsiTargetAlias, targetGroupMemberRemoved=targetGroupMemberRemoved, poolExported=poolExported, ipmiFan=ipmiFan, iscsiTargetWriteMbps=iscsiTargetWriteMbps, lunReservedSizeHigh=lunReservedSizeHigh, fcInitiatorGroup=fcInitiatorGroup, shareReservedSizeLow=shareReservedSizeLow, projectUnusedReservedSizeLow=projectUnusedReservedSizeLow, poolDeletionFailed=poolDeletionFailed, poolAvailableMetaToDataRatioBelowThresholdWarning=poolAvailableMetaToDataRatioBelowThresholdWarning, cifsTotalReadIops=cifsTotalReadIops, networkTotalTransmitMbps=networkTotalTransmitMbps, fcTargetStatus=fcTargetStatus, vmwareNFSDatastoreVMName=vmwareNFSDatastoreVMName, maintenanceModeEnabled=maintenanceModeEnabled, iscsiWriteIops=iscsiWriteIops, poolTotalReadIops=poolTotalReadIops, projectTotalSaving=projectTotalSaving, vmwareNFSDatastoreIndex=vmwareNFSDatastoreIndex, haResourceDescription=haResourceDescription, shareQuotaFinished=shareQuotaFinished, lunReservedSizeLow=lunReservedSizeLow, projectShares=projectShares, iscsiTargetsTable=iscsiTargetsTable, shareSnapshotSizeHigh=shareSnapshotSizeHigh, haControllerA_SoftwareVersion=haControllerA_SoftwareVersion, iscsiProperties=iscsiProperties, nicState=nicState, alertsCleanupCompleted=alertsCleanupCompleted, cacheTotalReadMbps=cacheTotalReadMbps, tdpsUpgradeFailed=tdpsUpgradeFailed, poolExpanded=poolExpanded, poolQuotaExceedThresholdWarning=poolQuotaExceedThresholdWarning, projectDedupEnabled=projectDedupEnabled, fcTargetResetHbaPortFailed=fcTargetResetHbaPortFailed, haControllerA_IPAddr=haControllerA_IPAddr, poolState=poolState, diskAvgWriteLatency=diskAvgWriteLatency, cifsAvgWriteLatency=cifsAvgWriteLatency, controllerTimeDrift=controllerTimeDrift, vmwareNFSDatastoreESXName=vmwareNFSDatastoreESXName, snmpServiceStopFailed=snmpServiceStopFailed, sensorFailureEvent=sensorFailureEvent, cpuTotalUsage=cpuTotalUsage, poolEntry=poolEntry, haResourceName=haResourceName, shareCIFSEnabled=shareCIFSEnabled, fcTargetWriteMbps=fcTargetWriteMbps, cacheTotalReadIops=cacheTotalReadIops, snapshotCreatedSuccessfully=snapshotCreatedSuccessfully, poolReadCacheWriteIops=poolReadCacheWriteIops, iscsiInitiatorCreateFailed=iscsiInitiatorCreateFailed, cacheSSDReads=cacheSSDReads, basicCompliance=basicCompliance, shareDedupEnabled=shareDedupEnabled, poolReadCacheWriteLatency=poolReadCacheWriteLatency, controllerHardwareModel=controllerHardwareModel, shareCreatedWithNonOptimalBlockSize=shareCreatedWithNonOptimalBlockSize, notifications=notifications, ipmiCurrent=ipmiCurrent, poolMetaWriteIops=poolMetaWriteIops, lunReadIops=lunReadIops, poolDeleted=poolDeleted, nfsAvgWriteLatency=nfsAvgWriteLatency, poolWriteCacheWriteMbps=poolWriteCacheWriteMbps, projectFreeSizeHigh=projectFreeSizeHigh, replicationTargetDeleted=replicationTargetDeleted, fcInitiatorName=fcInitiatorName, poolAvgReadLatency=poolAvgReadLatency, shareNFSWriteIops=shareNFSWriteIops, initiatorGroupMemberRemoved=initiatorGroupMemberRemoved, ntpServerTimeDrift=ntpServerTimeDrift, iscsiInitiatorCreateCompleted=iscsiInitiatorCreateCompleted, shareCIFSWriteIops=shareCIFSWriteIops, fcTargetReadIops=fcTargetReadIops, shareQuotaLow=shareQuotaLow, lunSnapshotSizeHigh=lunSnapshotSizeHigh, iscsiInitiatorsTable=iscsiInitiatorsTable, lunTargetGroup=lunTargetGroup, poolReadCacheReadMbps=poolReadCacheReadMbps, lunIndex=lunIndex, tegileArray=tegileArray, iscsiTotalReadMbps=iscsiTotalReadMbps, fcAvgReadLatency=fcAvgReadLatency, fcTargetEntry=fcTargetEntry, nfsAvgReadLatency=nfsAvgReadLatency, shareExceedThresholdWarning=shareExceedThresholdWarning, poolProjectCount=poolProjectCount, lunFreeSizeLow=lunFreeSizeLow, poolTotalSaving=poolTotalSaving, vmwareNFSDatastoresTotalWriteIops=vmwareNFSDatastoresTotalWriteIops, replicationComplete=replicationComplete, iscsiTargetError=iscsiTargetError, nvdimmFailInitializationError=nvdimmFailInitializationError, diskTotalReadIops=diskTotalReadIops, shareMountPoint=shareMountPoint, lunProtocol=lunProtocol, nicTable=nicTable, diskDataReadIops=diskDataReadIops, notificationSensorNumber=notificationSensorNumber, sharePoolName=sharePoolName, fcTargetReadMbps=fcTargetReadMbps, vmwareNFSDatastoreCreated=vmwareNFSDatastoreCreated, upgradeCompleted=upgradeCompleted, memoryFailure=memoryFailure, poolMetaWriteLatency=poolMetaWriteLatency, poolUpgraded=poolUpgraded, fcPortOffline=fcPortOffline, lunPoolName=lunPoolName, smisServiceStartFailed=smisServiceStartFailed, poolUnusedReservedSizeHigh=poolUnusedReservedSizeHigh, fcTotalWriteIops=fcTotalWriteIops, poolMetaWriteMbps=poolMetaWriteMbps, replicationResumed=replicationResumed, projectPoolName=projectPoolName, iscsiInitiators=iscsiInitiators, iscsiInitiatorDeleteCompleted=iscsiInitiatorDeleteCompleted, testNotification=testNotification, poolPostCompressionDataSizeHigh=poolPostCompressionDataSizeHigh, poolCreated=poolCreated, poolDataReadIops=poolDataReadIops, vmwareNFSDatastoresAvgWriteLatency=vmwareNFSDatastoresAvgWriteLatency, fcTotalWriteMbps=fcTotalWriteMbps, upgradeDownloadCompleted=upgradeDownloadCompleted, haResourceActiveNode=haResourceActiveNode, nvdimmFailDeviceError=nvdimmFailDeviceError, nicGroup=nicGroup, upgradeDownloadStarted=upgradeDownloadStarted, haControllerB_Name=haControllerB_Name, nicTransmitMbps=nicTransmitMbps, nicEntry=nicEntry, projectCreatedWithNonOptimalBlockSize=projectCreatedWithNonOptimalBlockSize, poolPostCompressionDataSizeLow=poolPostCompressionDataSizeLow, lunSizeLow=lunSizeLow, nicName=nicName, nicReceiveMbps=nicReceiveMbps, diskTable=diskTable, ipmiCriticalInterrupt=ipmiCriticalInterrupt, iscsiInitiatorChapUser=iscsiInitiatorChapUser, nfsTotalWriteIops=nfsTotalWriteIops, vmwareNFSDatastoresTable=vmwareNFSDatastoresTable, projectDeletionFailed=projectDeletionFailed, spareDiskReplaced=spareDiskReplaced, iscsiTargetWriteIops=iscsiTargetWriteIops, PYSNMP_MODULE_ID=tegile, poolUnusedReservedSizeLow=poolUnusedReservedSizeLow, iscsiTargetGroup=iscsiTargetGroup, vmwareNFSDatastoreDeleted=vmwareNFSDatastoreDeleted, diskDataWriteIops=diskDataWriteIops, networkTotalReceiveMbps=networkTotalReceiveMbps, vmwareNFSDatastoreReadMbps=vmwareNFSDatastoreReadMbps, haControllerB_Uptime=haControllerB_Uptime, diskGoneOffline=diskGoneOffline, diskState=diskState)
mibBuilder.exportSymbols("TEGILE-MIB", haResourceStatus=haResourceStatus, haControllerA_Uptime=haControllerA_Uptime, fcTotalReadMbps=fcTotalReadMbps, aclMigrationCompleted=aclMigrationCompleted, diskIsOnline=diskIsOnline, poolWriteCacheWriteLatency=poolWriteCacheWriteLatency, haResourceEntry=haResourceEntry, diskTotalWriteMbps=diskTotalWriteMbps, haControllerB_SoftwareVersion=haControllerB_SoftwareVersion, snapshotCloneCompleted=snapshotCloneCompleted, shareDeletionFailed=shareDeletionFailed, iscsiInitiatorModifyCompleted=iscsiInitiatorModifyCompleted, shareDeleted=shareDeleted, vmwareNFSDatastoresTotalWriteMbps=vmwareNFSDatastoresTotalWriteMbps, tdpsUpgradeDownloadCompleted=tdpsUpgradeDownloadCompleted, tegileArray_notifications=tegileArray_notifications, projectQuotaSizeLow=projectQuotaSizeLow, haResourcesTable=haResourcesTable, upgradeStarted=upgradeStarted, iscsiTargetReadLatency=iscsiTargetReadLatency, poolWriteCacheWriteIops=poolWriteCacheWriteIops, notificationThresholdTriggerValue=notificationThresholdTriggerValue, shareNFSReadLatency=shareNFSReadLatency, poolAvgWriteLatency=poolAvgWriteLatency, projectCreatedSuccessfully=projectCreatedSuccessfully, properties=properties, deleteFolderFailed=deleteFolderFailed, ipmiMemory=ipmiMemory, cpuSystemCalls=cpuSystemCalls, tegileGroups=tegileGroups, fcInitiators=fcInitiators, snmpServiceStartFailed=snmpServiceStartFailed, poolOwnerController=poolOwnerController, lunCompressionEnabled=lunCompressionEnabled, snapshotDeletedSuccessfully=snapshotDeletedSuccessfully, iscsiTargets=iscsiTargets, iscsiTargetName=iscsiTargetName, iscsiInitiatorEntry=iscsiInitiatorEntry, diskTotalWriteIops=diskTotalWriteIops, lunReadMbps=lunReadMbps, poolReadCacheWriteMbps=poolReadCacheWriteMbps, cpuInterrupts=cpuInterrupts, iscsiInitiatorName=iscsiInitiatorName, shareIndex=shareIndex, poolTable=poolTable, lunDedupEnabled=lunDedupEnabled, poolQuotaFinished=poolQuotaFinished, poolDegraded=poolDegraded, maintenanceModeDisabled=maintenanceModeDisabled, pools=pools, fcProperties=fcProperties, volumeDeleteFailed=volumeDeleteFailed, initiatorGroupMemberAdded=initiatorGroupMemberAdded, vmwareNFSDatastoreReadLatency=vmwareNFSDatastoreReadLatency, projectSnapshotSizeHigh=projectSnapshotSizeHigh, lunDataSizeHigh=lunDataSizeHigh, fcTargets=fcTargets, nfsTotalWriteMbps=nfsTotalWriteMbps, shareDataSizeHigh=shareDataSizeHigh, networkIpmpMemberInterfaceAdded=networkIpmpMemberInterfaceAdded, lunGUID=lunGUID, poolTotalWriteMbps=poolTotalWriteMbps, diskSizeHigh=diskSizeHigh, shareCompressedRatio=shareCompressedRatio, diskPoolName=diskPoolName, vmwareNFSDatastoresAvgReadLatency=vmwareNFSDatastoresAvgReadLatency, ipmiThreshold=ipmiThreshold, tdpsUpgradeDownloadStarted=tdpsUpgradeDownloadStarted, notificationObjects=notificationObjects, disks=disks, shareNFSWriteMbps=shareNFSWriteMbps, fcInitiatorEntry=fcInitiatorEntry, fcTotalReadIops=fcTotalReadIops, shareNFSEnabled=shareNFSEnabled, shareCIFSWriteMbps=shareCIFSWriteMbps, projectPostCompressionDataSizeHigh=projectPostCompressionDataSizeHigh, fcTargetIndex=fcTargetIndex, initiatorGroupCreateCompleted=initiatorGroupCreateCompleted, webdocsUpgradeDownloadFailed=webdocsUpgradeDownloadFailed, ipmiTemperature=ipmiTemperature, iscsiTargetEntry=iscsiTargetEntry, poolReadCacheReadIops=poolReadCacheReadIops, projectShareCount=projectShareCount, diskAvgReadLatency=diskAvgReadLatency, lunSizeHigh=lunSizeHigh, shareCIFSReadLatency=shareCIFSReadLatency, shareNFSReadMbps=shareNFSReadMbps, diagnosticDataUploadingFailed=diagnosticDataUploadingFailed, projectLUNs=projectLUNs, adServerTimeDrift=adServerTimeDrift, ipmiOther=ipmiOther, networkIpmpMemberInterfaceRemoved=networkIpmpMemberInterfaceRemoved, diagnosticDataUploaded=diagnosticDataUploaded, targetGroupDeleteCompleted=targetGroupDeleteCompleted, poolTotalReadMbps=poolTotalReadMbps, poolImported=poolImported, nicIndex=nicIndex, ipmiVoltage=ipmiVoltage, cifsTotalWriteIops=cifsTotalWriteIops, nfsTotalReadMbps=nfsTotalReadMbps, tdpsUpgradeDownloadFailed=tdpsUpgradeDownloadFailed, poolIndex=poolIndex, poolMetaReadLatency=poolMetaReadLatency, diskDataWriteMbps=diskDataWriteMbps, fcTargetGroup=fcTargetGroup, snmpServiceStopped=snmpServiceStopped, smisServiceStopped=smisServiceStopped, poolMetaReadMbps=poolMetaReadMbps, shareCIFSReadMbps=shareCIFSReadMbps, fcTargetWriteIops=fcTargetWriteIops, snapshotCloningFailed=snapshotCloningFailed, poolUsedSizeLow=poolUsedSizeLow, projectQuotaFinished=projectQuotaFinished, diskType=diskType, projectSnapshotSizeLow=projectSnapshotSizeLow, fcTargetResetHbaPortCompleted=fcTargetResetHbaPortCompleted, haResourceGroupTakeOverCompleted=haResourceGroupTakeOverCompleted, poolHealth=poolHealth, intelliFlashSoftwareDown=intelliFlashSoftwareDown, cifsTotalWriteMbps=cifsTotalWriteMbps, lunInitiatorGroup=lunInitiatorGroup, projectQuotaSizeHigh=projectQuotaSizeHigh, volumeCreatedSuccessfully=volumeCreatedSuccessfully, projectName=projectName, poolName=poolName, poolReadCacheReadLatency=poolReadCacheReadLatency, intelliFlashSoftwareUp=intelliFlashSoftwareUp, fcTargetReadLatency=fcTargetReadLatency, poolDataWriteMbps=poolDataWriteMbps, cacheRAMReads=cacheRAMReads, poolUsedSizeHigh=poolUsedSizeHigh, projectModified=projectModified, projectPostCompressionDataSizeLow=projectPostCompressionDataSizeLow, shareCompressionEnabled=shareCompressionEnabled, poolMetaDataQuotaExceedThresholdWarning=poolMetaDataQuotaExceedThresholdWarning, tegile=tegile, networkInterfaceUp=networkInterfaceUp, replicationSourceRegistered=replicationSourceRegistered, poolDataWriteIops=poolDataWriteIops, poolFreeSizeLow=poolFreeSizeLow, notificationSensorAction=notificationSensorAction, haResourceGroupTakeBackCompleted=haResourceGroupTakeBackCompleted, upgradeDownloadFailed=upgradeDownloadFailed, projectTable=projectTable, diskAlias=diskAlias, shareCreatedSuccessfully=shareCreatedSuccessfully, diskIOCount=diskIOCount, shareSnapshotSizeLow=shareSnapshotSizeLow, sanProperties=sanProperties, targetGroupMemberAdded=targetGroupMemberAdded, lunWriteMbps=lunWriteMbps, nfsTotalReadIops=nfsTotalReadIops, projectFreeSizeLow=projectFreeSizeLow, notificationSeverity=notificationSeverity, lunBlockSize=lunBlockSize, iscsiTargetIndex=iscsiTargetIndex, notificationReadingUnit=notificationReadingUnit, vmwareNFSDatastoresTotalReadMbps=vmwareNFSDatastoresTotalReadMbps, iscsiTargetReadMbps=iscsiTargetReadMbps, deleteFolderCompleted=deleteFolderCompleted, snmpAgentVersion=snmpAgentVersion, poolDataSizeHigh=poolDataSizeHigh, netbiosSocketFailure=netbiosSocketFailure, controllerUp=controllerUp, diskTotalReadMbps=diskTotalReadMbps, shareNFSWriteLatency=shareNFSWriteLatency, upgradeCantProcessFilesManually=upgradeCantProcessFilesManually, diskCount=diskCount, fcInitiatorsTable=fcInitiatorsTable, networkIpmpGroupUp=networkIpmpGroupUp, poolDataSizeLow=poolDataSizeLow, iscsiAvgReadLatency=iscsiAvgReadLatency, vmwareNFSDatastoreWriteLatency=vmwareNFSDatastoreWriteLatency, unknownSensorEvent=unknownSensorEvent, poolTotalWriteIops=poolTotalWriteIops, cifsAvgReadLatency=cifsAvgReadLatency, vmwareNFSDatastoresTotalReadIops=vmwareNFSDatastoresTotalReadIops, snapshotDeleteFailed=snapshotDeleteFailed, smbSocketFailure=smbSocketFailure, globalStatistics=globalStatistics, iscsiTotalWriteMbps=iscsiTotalWriteMbps, fcInitiatorIndex=fcInitiatorIndex, poolDataWriteLatency=poolDataWriteLatency, cacheTotalWriteMbps=cacheTotalWriteMbps, nicMTU=nicMTU, poolMetaReadIops=poolMetaReadIops, vmwareNFSDatastoreWriteMbps=vmwareNFSDatastoreWriteMbps, ipmiPowerSupply=ipmiPowerSupply, diskChassisIdx=diskChassisIdx, projectDataSizeHigh=projectDataSizeHigh, snapshotRollbackCompleted=snapshotRollbackCompleted, volumeCreatedWithNonOptimalBlockSize=volumeCreatedWithNonOptimalBlockSize, lunEntry=lunEntry, currentObjectGroup=currentObjectGroup, poolFreeSizeHigh=poolFreeSizeHigh, projectDataSizeLow=projectDataSizeLow, vmwareNFSDatastores=vmwareNFSDatastores, shareName=shareName, lunReadLatency=lunReadLatency, tdpsUpgradeTimeout=tdpsUpgradeTimeout, fcTargetsTable=fcTargetsTable, poolCount=poolCount, aclMigrationStarted=aclMigrationStarted, replicationAbandoned=replicationAbandoned, fcTargetName=fcTargetName, network=network, lunSnapshotSizeLow=lunSnapshotSizeLow, volumeModifyCompleted=volumeModifyCompleted, fcPortOnline=fcPortOnline, shareNFSReadIops=shareNFSReadIops, diskError=diskError, poolProjects=poolProjects, volumeQuotaFinished=volumeQuotaFinished, networkInterfaceDown=networkInterfaceDown, iscsiTargetModifyCompleted=iscsiTargetModifyCompleted)
