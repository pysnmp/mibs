...
# ASN.1 source CISCO-DMN-DSG-AUDIO-MIB
# Source digest sha256:58615b7da669e81a40baa8201943937a09460aaca255830e755a2fb3c5aa9863
...
if getattr(mibBuilder, 'version', (0, 0, 0)) > (4, 4, 0):
    if mibBuilder.loadTexts: ciscoDSGAudio.setRevisionsDescriptions(('V01.00.08 2013-07-10\n                   Updated audioLangList DESCRIPTION clause.', 'V01.00.07 2012-03-07\n                    Updated for D9854 R4 Release.', 'V01.00.06 2010-08-30\n                    Updated for adherence to SNMPv2 format.', 'V01.00.05 2010-05-24\n                    The names of audioAC3Comp and audioDDPMode are\n                    changed to audioDolbyDigitalComp\n                    audioDolbyDigitalPlusMode respectively.', 'V01.00.04 2010-05-21\n                    The enumerations of Audio PMT source item updated.', 'V01.00.03 2010-04-12\n                    The objects audioDigitalComp\n                    and audioDDPMode are updated.', 'V01.00.02 2010-03-22\n                    The Syntax of Unsigned32 MIB objects whose\n                    range is within the range\n                    of Integer32, is updated to Integer32.', 'V01.00.01 2010-02-12\n                    The Syntax of read-only objects is\n                    updated to DisplayString.', 'V01.00.00 2009-12-07\n                    Initial Version.',))
if mibBuilder.loadTexts: ciscoDSGAudio.setLastUpdated('2013-07-10 12:20')
if mibBuilder.loadTexts: ciscoDSGAudio.setOrganization('Cisco Systems, Inc.')
if mibBuilder.loadTexts: ciscoDSGAudio.setContactInfo('Cisco Systems, Inc.\n        Customer Service\n        Postal: 170 W Tasman Drive\n        San Jose, CA 95134\n        USA\n        Tel: +1 800 553 NETS\n\n        E-mail: cs-ipsla@cisco.com')
if mibBuilder.loadTexts: ciscoDSGAudio.setDescription('Cisco Audio Control MIB.')
audioCtrlTable = MibTable((1, 3, 6, 1, 4, 1, 1429, 2, 2, 5, 15, 1), ).setMaxAccess("notaccessible")
...
audioCtrlEntry = MibTableRow((1, 3, 6, 1, 4, 1, 1429, 2, 2, 5, 15, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CISCO-DMN-DSG-AUDIO-MIB", "audioSelKey"))
...
if mibBuilder.loadTexts: audioLangMenu.setDescription('Select whether to chose the language based on the selection\n        from the language list, from the PMT order, or from manual entry.')
...
if mibBuilder.loadTexts: audioLangList.setDescription('Audio Language List.\n          Language option slv(34) is only supported on D9865.')
...
audioStatusTable = MibTable((1, 3, 6, 1, 4, 1, 1429, 2, 2, 5, 15, 2), ).setMaxAccess("notaccessible")
...
audioStatusEntry = MibTableRow((1, 3, 6, 1, 4, 1, 1429, 2, 2, 5, 15, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "CISCO-DMN-DSG-AUDIO-MIB", "audioStatusSelKey"))
...
audioStatusSelKey = MibTableColumn((1, 3, 6, 1, 4, 1, 1429, 2, 2, 5, 15, 2, 1, 1), Integer32().subtype(subtypeSpec=ValueRangeConstraint(1, 1))).setMaxAccess("notaccessible")
...
if mibBuilder.loadTexts: audioStatusBitRate.setDescription('Audio Bitrate in Kilo bits(Kbits).The range is from 0 to\n        4294967295 Kbits in steps of 1 Kbits.')
...
if mibBuilder.loadTexts: audioStatusBufferLevel.setDescription('Level of audio input buffer in bytes.The range is from 0\n        to 4294967295 bytes in steps of 1 byte.')
...
if mibBuilder.loadTexts: audioStatusSFR.setDescription('Audio sampling frequency in Hz.The range is from 0\n        to 4294967295 bytes in steps of 1 Hz.')
...
mibBuilder.exportSymbols("CISCO-DMN-DSG-AUDIO-MIB", PYSNMP_MODULE_ID=ciscoDSGAudio, audioConsumerVolLeft=audioConsumerVolLeft, audioConsumerVolRight=audioConsumerVolRight, audioCtrlEntry=audioCtrlEntry, audioCtrlTable=audioCtrlTable, audioDigitalComp=audioDigitalComp, audioDolbyDigitalComp=audioDolbyDigitalComp, audioDolbyDigitalPlusMode=audioDolbyDigitalPlusMode, audioLangList=audioLangList, audioLangMenu=audioLangMenu, audioManualEntry=audioManualEntry, audioMode=audioMode, audioMuted=audioMuted, audioPmtSource=audioPmtSource, audioProfAttenuationLeft=audioProfAttenuationLeft, audioProfAttenuationRight=audioProfAttenuationRight, audioSelKey=audioSelKey, audioStatusBitRate=audioStatusBitRate, audioStatusBufferLevel=audioStatusBufferLevel, audioStatusDDPMode=audioStatusDDPMode, audioStatusDualMonoMode=audioStatusDualMonoMode, audioStatusEntry=audioStatusEntry, audioStatusFormat=audioStatusFormat, audioStatusLanguage=audioStatusLanguage, audioStatusMuted=audioStatusMuted, audioStatusSFR=audioStatusSFR, audioStatusSelKey=audioStatusSelKey, audioStatusTable=audioStatusTable, ciscoDSGAudio=ciscoDSGAudio)
