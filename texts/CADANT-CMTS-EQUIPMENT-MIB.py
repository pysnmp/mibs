...
DuplexStatus, UnknownState, CardType, CardSubType, SecondaryState, PicType, EqActionType, PortId, ShelfId, PortType, FirmwareSource, AdminState, DiskVolumeUsageLevel, PortDetectedMode, CardId, FlowControlMode, PortMode, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "UnknownState", "CardType", "CardSubType", "SecondaryState", "PicType", "EqActionType", "PortId", "ShelfId", "PortType", "FirmwareSource", "AdminState", "DiskVolumeUsageLevel", "PortDetectedMode", "CardId", "FlowControlMode", "PortMode", "PrimaryState")
...
DateAndTime, MacAddress, TruthValue, DisplayString, TimeStamp, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "MacAddress", "TruthValue", "DisplayString", "TimeStamp", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeName=diskVolumeName, cardSubType=cardSubType, auditId=auditId, portSecState=portSecState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestCommand=cardTestCommand, portSecStateChange=portSecStateChange, cardSecStateChange=cardSecStateChange, cardShelfTestIndex=cardShelfTestIndex, cardPicDetected=cardPicDetected, portRxFlowControlDetected=portRxFlowControlDetected, cardLastChangeTime=cardLastChangeTime, portNumChans=portNumChans, errEvLoggingEnabled=errEvLoggingEnabled, cmResetReason=cmResetReason, systemClock=systemClock, TestCommand=TestCommand, cardType=cardType, cardTrapInh=cardTrapInh, cardId=cardId, cardAction=cardAction, portRxFlowControlMode=portRxFlowControlMode, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestResult=TestResult, cmMacAddress=cmMacAddress, portBgpId=portBgpId, diskVolumeEntry=diskVolumeEntry, equipmentDiagGroup=equipmentDiagGroup, portMinDsPower=portMinDsPower, portLastChangeTime=portLastChangeTime, diskVolumeUsagePercentage=diskVolumeUsagePercentage, trapSeverity=trapSeverity, cardSubDetected=cardSubDetected, cmIpAddress=cmIpAddress, fpgaErrorEventTable=fpgaErrorEventTable, auditTotalFailedCount=auditTotalFailedCount, portDescription=portDescription, cardSpareGroupId=cardSpareGroupId, diskVolumeTrapInh=diskVolumeTrapInh, auditCardId=auditCardId, auditStatus=auditStatus, diskVolumeSize=diskVolumeSize, portNumber=portNumber, cardPrStateChange=cardPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetNotification=cmResetNotification, equipmentShelfGroup=equipmentShelfGroup, cardCPLDVersion=cardCPLDVersion, cmRegistrationNotification=cmRegistrationNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cmVendor=cmVendor, cardFwUpdateStatus=cardFwUpdateStatus, cardSecState=cardSecState, portMaxDsPower=portMaxDsPower, cardTestScheduleCommand=cardTestScheduleCommand, cadEquipmentMibConformance=cadEquipmentMibConformance, portPrStateChange=portPrStateChange, cardCarrierFwVersion=cardCarrierFwVersion, cardTestResultNotification=cardTestResultNotification, TestSchedule=TestSchedule, errEvRowStatus=errEvRowStatus, cardCarrierSerialNum=cardCarrierSerialNum, cardFlashSize=cardFlashSize, portTable=portTable, cadEquipmentMibGroup=cadEquipmentMibGroup, TestType=TestType, cardTestTime=cardTestTime, cardTestEntry=cardTestEntry, portEntry=portEntry, portPrState=portPrState, cardTestTransId=cardTestTransId, cardTestIndex=cardTestIndex, equipmentPortGroup=equipmentPortGroup, cardTestResult=cardTestResult, cardTable=cardTable, auditTime=auditTime, cardSwVersion=cardSwVersion, cardEntry=cardEntry, auditLogThrottle=auditLogThrottle, cmDevice=cmDevice, portTxFlowControlMode=portTxFlowControlMode, portTrapInh=portTrapInh, auditName=auditName, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditShelfId=auditShelfId, portMacIfIndex=portMacIfIndex, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfNumber=shelfNumber, cardBootVersion=cardBootVersion, diskVolumeNumber=diskVolumeNumber, equipmentState=equipmentState, cardSerialNum=cardSerialNum, cardTestSchedule=cardTestSchedule, cmResetStatus=cmResetStatus, equipmentAudit=equipmentAudit, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResultDesc=cardTestResultDesc, diskDriveNumber=diskDriveNumber, shelfName=shelfName, diskVolumeUsageLevel=diskVolumeUsageLevel, auditEntry=auditEntry, portDplxStatus=portDplxStatus, diskVolumeDriveId=diskVolumeDriveId, cmUChannelID=cmUChannelID, cardTestName=cardTestName, auditDescription=auditDescription, TestId=TestId, cardTestId=cardTestId, errEvLogLevel=errEvLogLevel, cardPicHwVersion=cardPicHwVersion, cardTemperature=cardTemperature, cardPrState=cardPrState, diskVolumeFileName=diskVolumeFileName, cardDetectedChange=cardDetectedChange, cardShelfId=cardShelfId, cardDplxStatus=cardDplxStatus, cardLastBootSource=cardLastBootSource, portMode=portMode, auditAutoScheduling=auditAutoScheduling, errEvRecoveryEnabled=errEvRecoveryEnabled, portAction=portAction, consoleOutput=consoleOutput, verboseLevel=verboseLevel, portTxFlowControlDetected=portTxFlowControlDetected, portDocsIfIndex=portDocsIfIndex, equipmentAuditGroup=equipmentAuditGroup, portType=portType, systemGeneralGroup=systemGeneralGroup, diskVolumeTable=diskVolumeTable, cardTestDescription=cardTestDescription, TestTransactionId=TestTransactionId, diskVolumeShelfId=diskVolumeShelfId, cmResetInfo=cmResetInfo, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentTbl=equipmentTbl, cardBusSpeed=cardBusSpeed, cardTemperatureHighError=cardTemperatureHighError, cardCarrierHwVersion=cardCarrierHwVersion, portMacAddress=portMacAddress, cardDetected=cardDetected, cardAdminState=cardAdminState, portCardId=portCardId, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeGroup=diskVolumeGroup, cardTestType=cardTestType, shelfObjects=shelfObjects, eqDiagConfig=eqDiagConfig, diskVolumeCardId=diskVolumeCardId, cmResetClearNotification=cmResetClearNotification, downstreamPowerLoss=downstreamPowerLoss, equipmentError=equipmentError, portId=portId, equipmentCardGroup=equipmentCardGroup, auditResult=auditResult, cadEquipmentMib=cadEquipmentMib, cardUpTime=cardUpTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTemperatureHighWarn=cardTemperatureHighWarn, portCardSubType=portCardSubType, trapCounter=trapCounter, cardName=cardName, equipmentStateGroup=equipmentStateGroup, cardNumPorts=cardNumPorts, remainInDiagMode=remainInDiagMode, portConnectorId=portConnectorId, cardRamSize=cardRamSize, portDplxStatusChange=portDplxStatusChange, auditCycleCount=auditCycleCount, equipmentTraps=equipmentTraps, auditTable=auditTable, cardDplxStatusChange=cardDplxStatusChange, auditCommand=auditCommand, cardHwVersion=cardHwVersion, portDetectedMode=portDetectedMode, cardSpareGroupMode=cardSpareGroupMode, equipmentCardTestGroup=equipmentCardTestGroup, cardFwVersion=cardFwVersion, diskVolumeDiskSize=diskVolumeDiskSize, shelfSwVersion=shelfSwVersion, auditTotalPassedCount=auditTotalPassedCount, cardLastBootVersion=cardLastBootVersion, portCurrDsPower=portCurrDsPower, auditPassedCount=auditPassedCount, equipmentDiag=equipmentDiag, cardCpuType=cardCpuType, errEventId=errEventId, cardNumber=cardNumber, cardLastResetReason=cardLastResetReason, diskVolumeUsageNotification=diskVolumeUsageNotification, systemGeneral=systemGeneral, TestScheduleCommand=TestScheduleCommand, portShelfId=portShelfId, cardCpuSpeed=cardCpuSpeed, cmResetUpTime=cmResetUpTime, cmPrimarySID=cmPrimarySID, cardFpgaSource=cardFpgaSource, cardTestTable=cardTestTable, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditLogOutput=auditLogOutput, auditFailedCount=auditFailedCount, diagTestId=diagTestId, portAdminState=portAdminState, diskVolumeId=diskVolumeId, cardPicSerialNum=cardPicSerialNum, systemKey=systemKey)
