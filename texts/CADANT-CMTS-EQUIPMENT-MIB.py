...
PortType, ShelfId, PicType, CardId, DuplexStatus, FirmwareSource, AdminState, FlowControlMode, CardType, CardSubType, PortId, PrimaryState, PortMode, DiskVolumeUsageLevel, UnknownState, EqActionType, PortDetectedMode, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "PortType", "ShelfId", "PicType", "CardId", "DuplexStatus", "FirmwareSource", "AdminState", "FlowControlMode", "CardType", "CardSubType", "PortId", "PrimaryState", "PortMode", "DiskVolumeUsageLevel", "UnknownState", "EqActionType", "PortDetectedMode", "SecondaryState")
...
DisplayString, MacAddress, RowStatus, TimeStamp, TextualConvention, DateAndTime, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "RowStatus", "TimeStamp", "TextualConvention", "DateAndTime", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTrapInh=cardTrapInh, auditFailedCount=auditFailedCount, cardLastChangeTime=cardLastChangeTime, portDocsIfIndex=portDocsIfIndex, equipmentCardGroup=equipmentCardGroup, portAction=portAction, cardTestDescription=cardTestDescription, auditName=auditName, auditLogThrottle=auditLogThrottle, cardTestType=cardTestType, cardTestResult=cardTestResult, diskVolumeDriveId=diskVolumeDriveId, cardTemperatureHighError=cardTemperatureHighError, diskVolumeTable=diskVolumeTable, auditLogOutput=auditLogOutput, cardSecState=cardSecState, PYSNMP_MODULE_ID=cadEquipmentMib, TestScheduleCommand=TestScheduleCommand, cardLastResetReason=cardLastResetReason, cardTestEntry=cardTestEntry, fpgaErrorEventGroup=fpgaErrorEventGroup, cmDevice=cmDevice, auditCommand=auditCommand, cardDplxStatusChange=cardDplxStatusChange, cardDetected=cardDetected, diskVolumeEntry=diskVolumeEntry, portLastChangeTime=portLastChangeTime, auditAutoScheduling=auditAutoScheduling, cardPicDetected=cardPicDetected, portEntry=portEntry, cmRegistrationNotification=cmRegistrationNotification, TestResult=TestResult, cardSpareGroupId=cardSpareGroupId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCarrierFwVersion=cardCarrierFwVersion, portDescription=portDescription, cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMibConformance=cadEquipmentMibConformance, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskDriveNumber=diskDriveNumber, cardAction=cardAction, shelfName=shelfName, cardSerialNum=cardSerialNum, cardTestTable=cardTestTable, cardPicHwVersion=cardPicHwVersion, auditCardId=auditCardId, equipmentAuditGroup=equipmentAuditGroup, portDplxStatus=portDplxStatus, cardShelfId=cardShelfId, errEvLoggingEnabled=errEvLoggingEnabled, cardTestScheduleCommand=cardTestScheduleCommand, auditTotalFailedCount=auditTotalFailedCount, equipmentDiag=equipmentDiag, cardCPLDVersion=cardCPLDVersion, portMacAddress=portMacAddress, cardSubType=cardSubType, cmResetInfo=cmResetInfo, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditStatus=auditStatus, systemClock=systemClock, portTable=portTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentError=equipmentError, cardNumber=cardNumber, auditCycleCount=auditCycleCount, portCurrDsPower=portCurrDsPower, TestCommand=TestCommand, diskVolumeCardId=diskVolumeCardId, diagTestId=diagTestId, cardLastBootSource=cardLastBootSource, cardCarrierHwVersion=cardCarrierHwVersion, cardTestSchedule=cardTestSchedule, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeName=diskVolumeName, fpgaErrorEventTable=fpgaErrorEventTable, errEventId=errEventId, downstreamPowerLoss=downstreamPowerLoss, equipmentAudit=equipmentAudit, shelfNumber=shelfNumber, cardRamSize=cardRamSize, portMacIfIndex=portMacIfIndex, diskVolumeDiskSize=diskVolumeDiskSize, cmMacAddress=cmMacAddress, equipmentStateGroup=equipmentStateGroup, auditShelfId=auditShelfId, equipmentState=equipmentState, portMaxDsPower=portMaxDsPower, diskVolumeSize=diskVolumeSize, cardSubDetected=cardSubDetected, cardCarrierSerialNum=cardCarrierSerialNum, cardTestTransId=cardTestTransId, equipmentShelfGroup=equipmentShelfGroup, diskVolumeNumber=diskVolumeNumber, cardTestResultNotification=cardTestResultNotification, cardFwUpdateStatus=cardFwUpdateStatus, cardTemperature=cardTemperature, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, verboseLevel=verboseLevel, consoleOutput=consoleOutput, cardFpgaSource=cardFpgaSource, portDetectedMode=portDetectedMode, cardPicSerialNum=cardPicSerialNum, portTrapInh=portTrapInh, portAdminState=portAdminState, cmIpAddress=cmIpAddress, portMinDsPower=portMinDsPower, cmResetClearNotification=cmResetClearNotification, cardSecStateChange=cardSecStateChange, TestSchedule=TestSchedule, TestTransactionId=TestTransactionId, cardTestIndex=cardTestIndex, trapSeverity=trapSeverity, cardHwVersion=cardHwVersion, shelfObjects=shelfObjects, cardId=cardId, auditDescription=auditDescription, errEvLogLevel=errEvLogLevel, cmResetStatus=cmResetStatus, equipmentDiagGroup=equipmentDiagGroup, portRxFlowControlDetected=portRxFlowControlDetected, systemKey=systemKey, cardSwVersion=cardSwVersion, TestType=TestType, diskVolumeId=diskVolumeId, portShelfId=portShelfId, cardTestTime=cardTestTime, cardFwVersion=cardFwVersion, cardCpuSpeed=cardCpuSpeed, cardType=cardType, cardLastBootVersion=cardLastBootVersion, cardTestId=cardTestId, cardName=cardName, cardTestCommand=cardTestCommand, equipmentPortGroup=equipmentPortGroup, diskVolumeShelfId=diskVolumeShelfId, auditId=auditId, auditPassedCount=auditPassedCount, diskVolumeUsageNotification=diskVolumeUsageNotification, cmResetReason=cmResetReason, cmPrimarySID=cmPrimarySID, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetNotification=cmResetNotification, cardCpuType=cardCpuType, shelfSwVersion=shelfSwVersion, cardTestName=cardTestName, portPrStateChange=portPrStateChange, portMode=portMode, cmVendor=cmVendor, portSecStateChange=portSecStateChange, auditEntry=auditEntry, portId=portId, auditTotalPassedCount=auditTotalPassedCount, cardFlashSize=cardFlashSize, equipmentTbl=equipmentTbl, portConnectorId=portConnectorId, portCardId=portCardId, cardNumPorts=cardNumPorts, cardTempNoReportNotification=cardTempNoReportNotification, portBgpId=portBgpId, cardTable=cardTable, cardBootVersion=cardBootVersion, cmUChannelID=cmUChannelID, portSecState=portSecState, cardBusSpeed=cardBusSpeed, equipmentTraps=equipmentTraps, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardDplxStatus=cardDplxStatus, auditTime=auditTime, portCardSubType=portCardSubType, portTxFlowControlMode=portTxFlowControlMode, cardUpTime=cardUpTime, portPrState=portPrState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPrStateChange=cardPrStateChange, cardEntry=cardEntry, equipmentCardTestGroup=equipmentCardTestGroup, portNumber=portNumber, cardSpareGroupMode=cardSpareGroupMode, portType=portType, fpgaErrorEventEntry=fpgaErrorEventEntry, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeFileName=diskVolumeFileName, cardDetectedChange=cardDetectedChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, eqDiagConfig=eqDiagConfig, diskVolumeGroup=diskVolumeGroup, systemGeneral=systemGeneral, trapCounter=trapCounter, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardAdminState=cardAdminState, cardPrState=cardPrState, portNumChans=portNumChans, remainInDiagMode=remainInDiagMode, auditTable=auditTable, errEvRowStatus=errEvRowStatus, cadEquipmentMib=cadEquipmentMib, auditResult=auditResult, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetUpTime=cmResetUpTime, systemGeneralGroup=systemGeneralGroup, TestId=TestId, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResultDesc=cardTestResultDesc)
