...
PortDetectedMode, PrimaryState, DuplexStatus, CardId, AdminState, EqActionType, DiskVolumeUsageLevel, PortId, FlowControlMode, SecondaryState, UnknownState, FirmwareSource, ShelfId, PortMode, CardSubType, CardType, PicType, PortType = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "PrimaryState", "DuplexStatus", "CardId", "AdminState", "EqActionType", "DiskVolumeUsageLevel", "PortId", "FlowControlMode", "SecondaryState", "UnknownState", "FirmwareSource", "ShelfId", "PortMode", "CardSubType", "CardType", "PicType", "PortType")
...
DisplayString, TruthValue, RowStatus, TextualConvention, TimeStamp, MacAddress, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TruthValue", "RowStatus", "TextualConvention", "TimeStamp", "MacAddress", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmVendor=cmVendor, cardPrStateChange=cardPrStateChange, TestTransactionId=TestTransactionId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardUpTime=cardUpTime, consoleOutput=consoleOutput, cardCarrierFwVersion=cardCarrierFwVersion, remainInDiagMode=remainInDiagMode, cardSubDetected=cardSubDetected, cardTemperatureHighWarn=cardTemperatureHighWarn, systemGeneral=systemGeneral, portConnectorId=portConnectorId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskDriveNumber=diskDriveNumber, cardDplxStatus=cardDplxStatus, auditDescription=auditDescription, cardTestResult=cardTestResult, cardDetected=cardDetected, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeCardId=diskVolumeCardId, cardDetectedChange=cardDetectedChange, cardTestSchedule=cardTestSchedule, equipmentAudit=equipmentAudit, portDplxStatus=portDplxStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, errEvLogLevel=errEvLogLevel, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfSwVersion=shelfSwVersion, auditTotalPassedCount=auditTotalPassedCount, cardPicDetected=cardPicDetected, diskVolumeName=diskVolumeName, auditPassedCount=auditPassedCount, diskVolumeNumber=diskVolumeNumber, portTxFlowControlMode=portTxFlowControlMode, auditLogOutput=auditLogOutput, cardNumber=cardNumber, cardCpuType=cardCpuType, cmResetNotification=cmResetNotification, diskVolumeSize=diskVolumeSize, cardTestDescription=cardTestDescription, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portMinDsPower=portMinDsPower, cardPrState=cardPrState, portNumChans=portNumChans, shelfNumber=shelfNumber, verboseLevel=verboseLevel, shelfName=shelfName, portMacIfIndex=portMacIfIndex, errEvRecoveryEnabled=errEvRecoveryEnabled, portRxFlowControlMode=portRxFlowControlMode, cardBootVersion=cardBootVersion, diskVolumeDiskSize=diskVolumeDiskSize, cmPrimarySID=cmPrimarySID, cardPicHwVersion=cardPicHwVersion, cardSpareGroupId=cardSpareGroupId, cardFlashSize=cardFlashSize, portPrStateChange=portPrStateChange, cardAdminState=cardAdminState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeId=diskVolumeId, portSecState=portSecState, cmResetClearNotification=cmResetClearNotification, auditEntry=auditEntry, portMode=portMode, cardTestIndex=cardTestIndex, portDocsIfIndex=portDocsIfIndex, systemGeneralGroup=systemGeneralGroup, portAdminState=portAdminState, cardSecStateChange=cardSecStateChange, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeTrapInh=diskVolumeTrapInh, diagTestId=diagTestId, cardFwVersion=cardFwVersion, cmResetStatus=cmResetStatus, fpgaErrorEventTable=fpgaErrorEventTable, cardFwUpdateStatus=cardFwUpdateStatus, cmIpAddress=cmIpAddress, systemKey=systemKey, cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, auditFailedCount=auditFailedCount, cardDplxStatusChange=cardDplxStatusChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, fpgaErrorEventGroup=fpgaErrorEventGroup, PYSNMP_MODULE_ID=cadEquipmentMib, cardPicSerialNum=cardPicSerialNum, cardSecState=cardSecState, cardTestResultNotification=cardTestResultNotification, cardTrapInh=cardTrapInh, cardSpareGroupMode=cardSpareGroupMode, auditTime=auditTime, eqDiagConfig=eqDiagConfig, systemClock=systemClock, auditResult=auditResult, cardTestName=cardTestName, cmMacAddress=cmMacAddress, cmUChannelID=cmUChannelID, errEvRowStatus=errEvRowStatus, portTxFlowControlDetected=portTxFlowControlDetected, equipmentDiag=equipmentDiag, cmResetReason=cmResetReason, diskVolumeGroup=diskVolumeGroup, cardLastResetReason=cardLastResetReason, portAction=portAction, cardTestTransId=cardTestTransId, auditTotalFailedCount=auditTotalFailedCount, cardSwVersion=cardSwVersion, cardSerialNum=cardSerialNum, portEntry=portEntry, portShelfId=portShelfId, portDetectedMode=portDetectedMode, portSecStateChange=portSecStateChange, downstreamPowerLoss=downstreamPowerLoss, cardCPLDVersion=cardCPLDVersion, TestSchedule=TestSchedule, portCardId=portCardId, cardTempNoReportNotification=cardTempNoReportNotification, cardTestTime=cardTestTime, cardSubType=cardSubType, cardTestResultDesc=cardTestResultDesc, equipmentTbl=equipmentTbl, equipmentDiagGroup=equipmentDiagGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cardHwVersion=cardHwVersion, portMaxDsPower=portMaxDsPower, auditTable=auditTable, diskVolumeEntry=diskVolumeEntry, portType=portType, portDescription=portDescription, portCardSubType=portCardSubType, cardTestScheduleCommand=cardTestScheduleCommand, cardCarrierSerialNum=cardCarrierSerialNum, portPrState=portPrState, auditShelfId=auditShelfId, cmDevice=cmDevice, equipmentError=equipmentError, cardCpuSpeed=cardCpuSpeed, portRxFlowControlDetected=portRxFlowControlDetected, auditStatus=auditStatus, equipmentCardGroup=equipmentCardGroup, diskVolumeTable=diskVolumeTable, cardType=cardType, portNumber=portNumber, equipmentStateGroup=equipmentStateGroup, portTrapInh=portTrapInh, auditName=auditName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardAction=cardAction, cadEquipmentMib=cadEquipmentMib, equipmentTraps=equipmentTraps, cardTestId=cardTestId, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestResult=TestResult, cardShelfId=cardShelfId, portLastChangeTime=portLastChangeTime, errEventId=errEventId, cmRegistrationNotification=cmRegistrationNotification, auditId=auditId, equipmentShelfGroup=equipmentShelfGroup, portTable=portTable, portMacAddress=portMacAddress, auditCardId=auditCardId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestCommand=cardTestCommand, diskVolumeFileName=diskVolumeFileName, cmResetUpTime=cmResetUpTime, cmResetInfo=cmResetInfo, shelfObjects=shelfObjects, portCurrDsPower=portCurrDsPower, TestScheduleCommand=TestScheduleCommand, equipmentCardTestGroup=equipmentCardTestGroup, equipmentAuditGroup=equipmentAuditGroup, cardTestEntry=cardTestEntry, auditCycleCount=auditCycleCount, cardTable=cardTable, cardTemperatureHighError=cardTemperatureHighError, diskVolumeShelfId=diskVolumeShelfId, cardBusSpeed=cardBusSpeed, TestType=TestType, cardTestType=cardTestType, equipmentState=equipmentState, portId=portId, cardEntry=cardEntry, cardName=cardName, cardTestTable=cardTestTable, trapSeverity=trapSeverity, cardLastBootVersion=cardLastBootVersion, TestCommand=TestCommand, TestId=TestId, diskVolumeDriveId=diskVolumeDriveId, cardId=cardId, auditCommand=auditCommand, cardFpgaSource=cardFpgaSource, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portBgpId=portBgpId, cardLastBootSource=cardLastBootSource, cardRamSize=cardRamSize, auditLogThrottle=auditLogThrottle, cardTemperature=cardTemperature, cardLastChangeTime=cardLastChangeTime, equipmentPortGroup=equipmentPortGroup, auditAutoScheduling=auditAutoScheduling, trapCounter=trapCounter, portDplxStatusChange=portDplxStatusChange)
