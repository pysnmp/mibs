...
FirmwareSource, FlowControlMode, CardType, SecondaryState, AdminState, EqActionType, PortDetectedMode, PrimaryState, ShelfId, PortId, UnknownState, PortType, CardId, PortMode, DiskVolumeUsageLevel, CardSubType, DuplexStatus, PicType = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "FlowControlMode", "CardType", "SecondaryState", "AdminState", "EqActionType", "PortDetectedMode", "PrimaryState", "ShelfId", "PortId", "UnknownState", "PortType", "CardId", "PortMode", "DiskVolumeUsageLevel", "CardSubType", "DuplexStatus", "PicType")
...
MacAddress, DisplayString, TimeStamp, TextualConvention, RowStatus, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "DisplayString", "TimeStamp", "TextualConvention", "RowStatus", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTxFlowControlMode=portTxFlowControlMode, cardDetectedChange=cardDetectedChange, equipmentStateGroup=equipmentStateGroup, diskVolumeFileName=diskVolumeFileName, diskVolumeEntry=diskVolumeEntry, diskVolumeDiskSize=diskVolumeDiskSize, cardDplxStatusChange=cardDplxStatusChange, diskVolumeNumber=diskVolumeNumber, cardType=cardType, auditDescription=auditDescription, auditTime=auditTime, equipmentCardGroup=equipmentCardGroup, auditTable=auditTable, portMaxDsPower=portMaxDsPower, fpgaErrorEventEntry=fpgaErrorEventEntry, portPrState=portPrState, cardSecStateChange=cardSecStateChange, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetReason=cmResetReason, portCardSubType=portCardSubType, portDetectedMode=portDetectedMode, systemGeneral=systemGeneral, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSecState=cardSecState, auditId=auditId, cardTestTime=cardTestTime, cardSubType=cardSubType, portBgpId=portBgpId, downstreamPowerLoss=downstreamPowerLoss, cardSpareGroupId=cardSpareGroupId, cardLastBootVersion=cardLastBootVersion, shelfNumber=shelfNumber, cardTestId=cardTestId, diagTestId=diagTestId, cardPrStateChange=cardPrStateChange, auditName=auditName, portConnectorId=portConnectorId, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditPassedCount=auditPassedCount, cardCPLDVersion=cardCPLDVersion, portAction=portAction, cmPrimarySID=cmPrimarySID, portAdminState=portAdminState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentCardTestGroup=equipmentCardTestGroup, TestType=TestType, cardFwUpdateStatus=cardFwUpdateStatus, equipmentDiag=equipmentDiag, cardHwVersion=cardHwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTempOverHeatNotification=cardTempOverHeatNotification, portTrapInh=portTrapInh, cardBootVersion=cardBootVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, cardAdminState=cardAdminState, diskVolumeUsageNotification=diskVolumeUsageNotification, errEventId=errEventId, cardTestIndex=cardTestIndex, auditCycleCount=auditCycleCount, cardTestCommand=cardTestCommand, cardTrapInh=cardTrapInh, portMacAddress=portMacAddress, cardFlashSize=cardFlashSize, cardAction=cardAction, cardPrState=cardPrState, equipmentError=equipmentError, cardSubDetected=cardSubDetected, cardTable=cardTable, cmVendor=cmVendor, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestDescription=cardTestDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentDiagGroup=equipmentDiagGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResult=cardTestResult, portNumChans=portNumChans, auditTotalFailedCount=auditTotalFailedCount, cardCarrierSerialNum=cardCarrierSerialNum, cmResetStatus=cmResetStatus, portPrStateChange=portPrStateChange, cardSpareGroupMode=cardSpareGroupMode, diskVolumeSize=diskVolumeSize, portRxFlowControlMode=portRxFlowControlMode, auditLogOutput=auditLogOutput, cardDplxStatus=cardDplxStatus, auditStatus=auditStatus, TestScheduleCommand=TestScheduleCommand, portMode=portMode, cardTestSchedule=cardTestSchedule, diskVolumeCardId=diskVolumeCardId, cardTestEntry=cardTestEntry, cmResetUpTime=cmResetUpTime, diskVolumeTrapInh=diskVolumeTrapInh, cardDetected=cardDetected, cardTestResultNotification=cardTestResultNotification, portLastChangeTime=portLastChangeTime, auditEntry=auditEntry, cardTestScheduleCommand=cardTestScheduleCommand, auditFailedCount=auditFailedCount, errEvLogLevel=errEvLogLevel, cardId=cardId, auditCommand=auditCommand, portSecStateChange=portSecStateChange, auditCardId=auditCardId, equipmentState=equipmentState, cmRegistrationNotification=cmRegistrationNotification, diskDriveNumber=diskDriveNumber, cardCarrierFwVersion=cardCarrierFwVersion, cardTestType=cardTestType, cardSwVersion=cardSwVersion, cardSerialNum=cardSerialNum, TestResult=TestResult, portTable=portTable, cmDevice=cmDevice, trapSeverity=trapSeverity, diskVolumeGroup=diskVolumeGroup, portType=portType, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTable=cardTestTable, errEvRowStatus=errEvRowStatus, cmMacAddress=cmMacAddress, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, portCardId=portCardId, cardTemperatureHighError=cardTemperatureHighError, cmResetInfo=cmResetInfo, auditResult=auditResult, PYSNMP_MODULE_ID=cadEquipmentMib, cardFwVersion=cardFwVersion, cardBusSpeed=cardBusSpeed, cardLastBootSource=cardLastBootSource, portDplxStatusChange=portDplxStatusChange, portMacIfIndex=portMacIfIndex, TestCommand=TestCommand, eqDiagConfig=eqDiagConfig, cardTempNoReportNotification=cardTempNoReportNotification, cardRamSize=cardRamSize, auditTotalPassedCount=auditTotalPassedCount, portDocsIfIndex=portDocsIfIndex, portMinDsPower=portMinDsPower, portDescription=portDescription, shelfObjects=shelfObjects, cardName=cardName, cardPicDetected=cardPicDetected, cardLastResetReason=cardLastResetReason, portShelfId=portShelfId, diskVolumeDriveId=diskVolumeDriveId, consoleOutput=consoleOutput, auditShelfId=auditShelfId, equipmentShelfGroup=equipmentShelfGroup, systemClock=systemClock, cardTestTransId=cardTestTransId, errEvLoggingEnabled=errEvLoggingEnabled, portDplxStatus=portDplxStatus, portSecState=portSecState, cardPicSerialNum=cardPicSerialNum, shelfSwVersion=shelfSwVersion, cardNumPorts=cardNumPorts, equipmentPortGroup=equipmentPortGroup, TestId=TestId, TestSchedule=TestSchedule, portId=portId, diskVolumeName=diskVolumeName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmIpAddress=cmIpAddress, cardTemperature=cardTemperature, fpgaErrorEventTable=fpgaErrorEventTable, equipmentTraps=equipmentTraps, trapCounter=trapCounter, diskVolumeShelfId=diskVolumeShelfId, cmResetClearNotification=cmResetClearNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetNotification=cmResetNotification, cardTestName=cardTestName, cardPicHwVersion=cardPicHwVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmUChannelID=cmUChannelID, cardCarrierHwVersion=cardCarrierHwVersion, cardShelfTestIndex=cardShelfTestIndex, portNumber=portNumber, systemKey=systemKey, portEntry=portEntry, cardShelfId=cardShelfId, diskVolumeTable=diskVolumeTable, diskVolumeId=diskVolumeId, equipmentTbl=equipmentTbl, auditLogThrottle=auditLogThrottle, portCurrDsPower=portCurrDsPower, verboseLevel=verboseLevel, systemGeneralGroup=systemGeneralGroup, remainInDiagMode=remainInDiagMode, cardNumber=cardNumber, cardEntry=cardEntry, cardUpTime=cardUpTime, cardLastChangeTime=cardLastChangeTime, cardCpuSpeed=cardCpuSpeed, shelfName=shelfName, cardCpuType=cardCpuType, TestTransactionId=TestTransactionId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResultDesc=cardTestResultDesc, cardFpgaSource=cardFpgaSource, equipmentAudit=equipmentAudit, cadEquipmentMib=cadEquipmentMib, auditAutoScheduling=auditAutoScheduling, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile)
