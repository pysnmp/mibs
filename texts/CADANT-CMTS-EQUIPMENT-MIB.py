...
CardType, CardSubType, PortDetectedMode, AdminState, PrimaryState, ShelfId, PortType, DuplexStatus, PortMode, UnknownState, EqActionType, PortId, DiskVolumeUsageLevel, FirmwareSource, CardId, FlowControlMode, SecondaryState, PicType = mibBuilder.importSymbols("CADANT-TC", "CardType", "CardSubType", "PortDetectedMode", "AdminState", "PrimaryState", "ShelfId", "PortType", "DuplexStatus", "PortMode", "UnknownState", "EqActionType", "PortId", "DiskVolumeUsageLevel", "FirmwareSource", "CardId", "FlowControlMode", "SecondaryState", "PicType")
...
RowStatus, DateAndTime, TimeStamp, TextualConvention, MacAddress, DisplayString, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "TimeStamp", "TextualConvention", "MacAddress", "DisplayString", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeGroup=diskVolumeGroup, cardCarrierFwVersion=cardCarrierFwVersion, cmResetUpTime=cmResetUpTime, cardBootVersion=cardBootVersion, cardSwVersion=cardSwVersion, cardDplxStatus=cardDplxStatus, remainInDiagMode=remainInDiagMode, equipmentState=equipmentState, cardTestEntry=cardTestEntry, diskVolumeShelfId=diskVolumeShelfId, equipmentCardGroup=equipmentCardGroup, TestResult=TestResult, cmResetClearNotification=cmResetClearNotification, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeCardId=diskVolumeCardId, cardSecState=cardSecState, shelfNumber=shelfNumber, auditId=auditId, shelfSwVersion=shelfSwVersion, auditLogOutput=auditLogOutput, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetNotification=cmResetNotification, auditCycleCount=auditCycleCount, systemGeneralGroup=systemGeneralGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTempNoReportNotification=cardTempNoReportNotification, cardType=cardType, diskVolumeFileName=diskVolumeFileName, portTable=portTable, cardName=cardName, diskVolumeDiskSize=diskVolumeDiskSize, cardHwVersion=cardHwVersion, TestSchedule=TestSchedule, diskVolumeDriveId=diskVolumeDriveId, auditResult=auditResult, cardTestType=cardTestType, portNumber=portNumber, portCardSubType=portCardSubType, consoleOutput=consoleOutput, cardCPLDVersion=cardCPLDVersion, diskVolumeName=diskVolumeName, cardSubDetected=cardSubDetected, cmDevice=cmDevice, diskVolumeEntry=diskVolumeEntry, cmVendor=cmVendor, auditEntry=auditEntry, TestId=TestId, cardSecStateChange=cardSecStateChange, equipmentAuditGroup=equipmentAuditGroup, cardBusSpeed=cardBusSpeed, cardLastBootVersion=cardLastBootVersion, cmResetInfo=cmResetInfo, TestScheduleCommand=TestScheduleCommand, equipmentDiagGroup=equipmentDiagGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, portMinDsPower=portMinDsPower, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, portLastChangeTime=portLastChangeTime, cardLastChangeTime=cardLastChangeTime, auditTotalFailedCount=auditTotalFailedCount, cmUChannelID=cmUChannelID, cmIpAddress=cmIpAddress, cadEquipmentMibGroup=cadEquipmentMibGroup, downstreamPowerLoss=downstreamPowerLoss, equipmentDiag=equipmentDiag, cardShelfTestIndex=cardShelfTestIndex, shelfName=shelfName, portMacAddress=portMacAddress, auditTable=auditTable, equipmentShelfGroup=equipmentShelfGroup, auditShelfId=auditShelfId, cardLastBootSource=cardLastBootSource, cardTestName=cardTestName, cardTestCommand=cardTestCommand, auditCommand=auditCommand, cardTestIndex=cardTestIndex, TestTransactionId=TestTransactionId, cardTemperatureHighError=cardTemperatureHighError, cardId=cardId, cardEntry=cardEntry, auditCardId=auditCardId, cardCarrierSerialNum=cardCarrierSerialNum, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastResetReason=cardLastResetReason, cardSpareGroupMode=cardSpareGroupMode, portMacIfIndex=portMacIfIndex, cardSpareGroupId=cardSpareGroupId, portTrapInh=portTrapInh, eqDiagConfig=eqDiagConfig, auditTime=auditTime, cardPicSerialNum=cardPicSerialNum, auditPassedCount=auditPassedCount, cardUpTime=cardUpTime, portPrState=portPrState, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestCommand=TestCommand, cardTestId=cardTestId, cardTestResultNotification=cardTestResultNotification, portDescription=portDescription, TestType=TestType, systemGeneral=systemGeneral, cardFwVersion=cardFwVersion, diskVolumeId=diskVolumeId, trapCounter=trapCounter, auditDescription=auditDescription, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portCardId=portCardId, cardPrStateChange=cardPrStateChange, equipmentTbl=equipmentTbl, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardFwUpdateStatus=cardFwUpdateStatus, portType=portType, portNumChans=portNumChans, diskVolumeUsageLevel=diskVolumeUsageLevel, systemClock=systemClock, auditTotalPassedCount=auditTotalPassedCount, auditAutoScheduling=auditAutoScheduling, portDocsIfIndex=portDocsIfIndex, cardPicHwVersion=cardPicHwVersion, shelfObjects=shelfObjects, cardNumPorts=cardNumPorts, errEventId=errEventId, auditFailedCount=auditFailedCount, auditName=auditName, portEntry=portEntry, systemKey=systemKey, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentError=equipmentError, errEvLogLevel=errEvLogLevel, cardCpuType=cardCpuType, fpgaErrorEventGroup=fpgaErrorEventGroup, diagTestId=diagTestId, diskVolumeNumber=diskVolumeNumber, portDetectedMode=portDetectedMode, cardCpuSpeed=cardCpuSpeed, portId=portId, cardTestResult=cardTestResult, verboseLevel=verboseLevel, diskVolumeUsageNotification=diskVolumeUsageNotification, cardFlashSize=cardFlashSize, portSecState=portSecState, portAction=portAction, auditStatus=auditStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portSecStateChange=portSecStateChange, portDplxStatusChange=portDplxStatusChange, cardRamSize=cardRamSize, cmResetStatus=cmResetStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestTime=cardTestTime, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeSize=diskVolumeSize, cardTrapInh=cardTrapInh, cardPrState=cardPrState, portMaxDsPower=portMaxDsPower, cardShelfId=cardShelfId, portCurrDsPower=portCurrDsPower, cmMacAddress=cmMacAddress, portMode=portMode, cardCarrierHwVersion=cardCarrierHwVersion, cardTestTable=cardTestTable, cardAdminState=cardAdminState, cardDetected=cardDetected, cadEquipmentMibCompliance=cadEquipmentMibCompliance, trapSeverity=trapSeverity, equipmentPortGroup=equipmentPortGroup, portShelfId=portShelfId, portPrStateChange=portPrStateChange, diskDriveNumber=diskDriveNumber, fpgaErrorEventTable=fpgaErrorEventTable, equipmentAudit=equipmentAudit, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeTable=diskVolumeTable, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardNumber=cardNumber, cardTestResultDesc=cardTestResultDesc, portBgpId=portBgpId, cardTestTransId=cardTestTransId, cardSerialNum=cardSerialNum, portConnectorId=portConnectorId, cardTestScheduleCommand=cardTestScheduleCommand, cardTable=cardTable, portRxFlowControlDetected=portRxFlowControlDetected, cadEquipmentMib=cadEquipmentMib, cardTestSchedule=cardTestSchedule, cardDetectedChange=cardDetectedChange, equipmentTraps=equipmentTraps, cardTemperature=cardTemperature, portDplxStatus=portDplxStatus, cardDplxStatusChange=cardDplxStatusChange, cardPicDetected=cardPicDetected, portTxFlowControlMode=portTxFlowControlMode, cardTestDescription=cardTestDescription, portRxFlowControlMode=portRxFlowControlMode, cmPrimarySID=cmPrimarySID, cmRegistrationNotification=cmRegistrationNotification, cardAction=cardAction, cmResetReason=cmResetReason, errEvRowStatus=errEvRowStatus, cardFpgaSource=cardFpgaSource, auditLogThrottle=auditLogThrottle, cardSubType=cardSubType, portAdminState=portAdminState)
