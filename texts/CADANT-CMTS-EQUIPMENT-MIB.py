...
CardSubType, DuplexStatus, SecondaryState, PortType, CardId, PortDetectedMode, UnknownState, DiskVolumeUsageLevel, PicType, PortId, PortMode, PrimaryState, ShelfId, FlowControlMode, AdminState, FirmwareSource, EqActionType, CardType = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "DuplexStatus", "SecondaryState", "PortType", "CardId", "PortDetectedMode", "UnknownState", "DiskVolumeUsageLevel", "PicType", "PortId", "PortMode", "PrimaryState", "ShelfId", "FlowControlMode", "AdminState", "FirmwareSource", "EqActionType", "CardType")
...
TextualConvention, DisplayString, MacAddress, TruthValue, RowStatus, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "MacAddress", "TruthValue", "RowStatus", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestId=TestId, auditResult=auditResult, cmIpAddress=cmIpAddress, shelfName=shelfName, shelfNumber=shelfNumber, portLastChangeTime=portLastChangeTime, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, equipmentAuditGroup=equipmentAuditGroup, auditLogThrottle=auditLogThrottle, auditStatus=auditStatus, cmPrimarySID=cmPrimarySID, portBgpId=portBgpId, cardDplxStatusChange=cardDplxStatusChange, cardTemperatureHighWarn=cardTemperatureHighWarn, cardFwVersion=cardFwVersion, diskVolumeSize=diskVolumeSize, diskVolumeNumber=diskVolumeNumber, portPrStateChange=portPrStateChange, portAdminState=portAdminState, portDescription=portDescription, trapCounter=trapCounter, equipmentCardTestGroup=equipmentCardTestGroup, cardPicSerialNum=cardPicSerialNum, cardAction=cardAction, portCardSubType=portCardSubType, equipmentCardGroup=equipmentCardGroup, cardAdminState=cardAdminState, diskVolumeCardId=diskVolumeCardId, cardTemperatureHighError=cardTemperatureHighError, TestScheduleCommand=TestScheduleCommand, cardNumPorts=cardNumPorts, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeId=diskVolumeId, equipmentDiagGroup=equipmentDiagGroup, cardDplxStatus=cardDplxStatus, auditName=auditName, cardId=cardId, cardEntry=cardEntry, cardBusSpeed=cardBusSpeed, portRxFlowControlMode=portRxFlowControlMode, auditTable=auditTable, equipmentPortGroup=equipmentPortGroup, TestSchedule=TestSchedule, cardNumber=cardNumber, equipmentError=equipmentError, cmDevice=cmDevice, cardShelfTestIndex=cardShelfTestIndex, portId=portId, cardTestCommand=cardTestCommand, cardPrStateChange=cardPrStateChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCpuSpeed=cardCpuSpeed, cardTrapInh=cardTrapInh, cardPicDetected=cardPicDetected, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCPLDVersion=cardCPLDVersion, auditId=auditId, portShelfId=portShelfId, consoleOutput=consoleOutput, cardTestEntry=cardTestEntry, systemGeneralGroup=systemGeneralGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardRamSize=cardRamSize, diskDriveNumber=diskDriveNumber, equipmentTraps=equipmentTraps, trapSeverity=trapSeverity, errEventId=errEventId, downstreamPowerLoss=downstreamPowerLoss, systemClock=systemClock, cardCarrierSerialNum=cardCarrierSerialNum, cardPrState=cardPrState, systemGeneral=systemGeneral, auditTotalPassedCount=auditTotalPassedCount, portCurrDsPower=portCurrDsPower, cardSerialNum=cardSerialNum, cardLastResetReason=cardLastResetReason, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portTrapInh=portTrapInh, diskVolumeDriveId=diskVolumeDriveId, portCardId=portCardId, cardType=cardType, portSecStateChange=portSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfObjects=shelfObjects, PYSNMP_MODULE_ID=cadEquipmentMib, cardSubDetected=cardSubDetected, cardName=cardName, errEvLogLevel=errEvLogLevel, cardTempNoReportNotification=cardTempNoReportNotification, auditTime=auditTime, cardFlashSize=cardFlashSize, portPrState=portPrState, diskVolumeShelfId=diskVolumeShelfId, systemKey=systemKey, portDplxStatusChange=portDplxStatusChange, cardSubType=cardSubType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditFailedCount=auditFailedCount, TestResult=TestResult, cardTestTransId=cardTestTransId, auditEntry=auditEntry, portMode=portMode, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestTable=cardTestTable, cardTestIndex=cardTestIndex, auditPassedCount=auditPassedCount, cardDetected=cardDetected, portNumChans=portNumChans, cardTestResultDesc=cardTestResultDesc, diagTestId=diagTestId, auditShelfId=auditShelfId, cardLastChangeTime=cardLastChangeTime, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSecState=cardSecState, portDetectedMode=portDetectedMode, TestType=TestType, remainInDiagMode=remainInDiagMode, cmResetClearNotification=cmResetClearNotification, verboseLevel=verboseLevel, errEvRowStatus=errEvRowStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentShelfGroup=equipmentShelfGroup, cardPicHwVersion=cardPicHwVersion, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventEntry=fpgaErrorEventEntry, portDplxStatus=portDplxStatus, auditAutoScheduling=auditAutoScheduling, portNumber=portNumber, portEntry=portEntry, portMacAddress=portMacAddress, portConnectorId=portConnectorId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestDescription=cardTestDescription, cardTable=cardTable, diskVolumeDiskSize=diskVolumeDiskSize, cardUpTime=cardUpTime, fpgaErrorEventTable=fpgaErrorEventTable, TestTransactionId=TestTransactionId, cardCpuType=cardCpuType, TestCommand=TestCommand, equipmentDiag=equipmentDiag, cardSpareGroupId=cardSpareGroupId, cardTestId=cardTestId, cardBootVersion=cardBootVersion, auditCardId=auditCardId, cmResetReason=cmResetReason, cmVendor=cmVendor, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogOutput=auditLogOutput, portDocsIfIndex=portDocsIfIndex, portMacIfIndex=portMacIfIndex, errEvLoggingEnabled=errEvLoggingEnabled, cmResetNotification=cmResetNotification, cmRegistrationNotification=cmRegistrationNotification, auditCommand=auditCommand, diskVolumeTrapInh=diskVolumeTrapInh, cardTestResult=cardTestResult, cardShelfId=cardShelfId, cmMacAddress=cmMacAddress, cardLastBootVersion=cardLastBootVersion, eqDiagConfig=eqDiagConfig, portTxFlowControlDetected=portTxFlowControlDetected, portType=portType, portMinDsPower=portMinDsPower, portTable=portTable, cardSwVersion=cardSwVersion, cardCarrierFwVersion=cardCarrierFwVersion, auditDescription=auditDescription, cardTestResultNotification=cardTestResultNotification, portSecState=portSecState, cmResetStatus=cmResetStatus, diskVolumeTable=diskVolumeTable, cardSecStateChange=cardSecStateChange, cardTestName=cardTestName, equipmentTbl=equipmentTbl, cmResetUpTime=cmResetUpTime, cardTestScheduleCommand=cardTestScheduleCommand, cardTemperature=cardTemperature, auditCycleCount=auditCycleCount, diskVolumeGroup=diskVolumeGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardSpareGroupMode=cardSpareGroupMode, portAction=portAction, cardCarrierHwVersion=cardCarrierHwVersion, equipmentStateGroup=equipmentStateGroup, cardFpgaSource=cardFpgaSource, cardHwVersion=cardHwVersion, diskVolumeFileName=diskVolumeFileName, shelfSwVersion=shelfSwVersion, cmResetInfo=cmResetInfo, cardDetectedChange=cardDetectedChange, portTxFlowControlMode=portTxFlowControlMode, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentState=equipmentState, cardTestSchedule=cardTestSchedule, cardTestTime=cardTestTime, cardLastBootSource=cardLastBootSource, cardTestType=cardTestType, diskVolumeName=diskVolumeName, equipmentAudit=equipmentAudit, cmUChannelID=cmUChannelID)
