...
PrimaryState, DiskVolumeUsageLevel, FlowControlMode, FirmwareSource, CardType, AdminState, PortId, EqActionType, DuplexStatus, PicType, SecondaryState, CardSubType, UnknownState, ShelfId, PortMode, CardId, PortType, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "DiskVolumeUsageLevel", "FlowControlMode", "FirmwareSource", "CardType", "AdminState", "PortId", "EqActionType", "DuplexStatus", "PicType", "SecondaryState", "CardSubType", "UnknownState", "ShelfId", "PortMode", "CardId", "PortType", "PortDetectedMode")
...
TextualConvention, RowStatus, TimeStamp, DateAndTime, DisplayString, MacAddress, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "TimeStamp", "DateAndTime", "DisplayString", "MacAddress", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portSecStateChange=portSecStateChange, cmResetClearNotification=cmResetClearNotification, portTable=portTable, cmMacAddress=cmMacAddress, diskVolumeId=diskVolumeId, cardSubDetected=cardSubDetected, cmRegistrationNotification=cmRegistrationNotification, shelfName=shelfName, cardType=cardType, cadEquipmentMibConformance=cadEquipmentMibConformance, portTrapInh=portTrapInh, cardTrapInh=cardTrapInh, cardNumber=cardNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSerialNum=cardSerialNum, cardFpgaSource=cardFpgaSource, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentAuditGroup=equipmentAuditGroup, TestSchedule=TestSchedule, portEntry=portEntry, auditCycleCount=auditCycleCount, auditTotalPassedCount=auditTotalPassedCount, systemClock=systemClock, cmResetReason=cmResetReason, auditResult=auditResult, cardEntry=cardEntry, equipmentAudit=equipmentAudit, cmResetNotification=cmResetNotification, shelfObjects=shelfObjects, portConnectorId=portConnectorId, portLastChangeTime=portLastChangeTime, cardPicDetected=cardPicDetected, errEvRowStatus=errEvRowStatus, cardSecStateChange=cardSecStateChange, diagTestId=diagTestId, equipmentDiagGroup=equipmentDiagGroup, TestScheduleCommand=TestScheduleCommand, cardId=cardId, verboseLevel=verboseLevel, cardUpTime=cardUpTime, cardCarrierHwVersion=cardCarrierHwVersion, trapSeverity=trapSeverity, portDescription=portDescription, cardTestIndex=cardTestIndex, shelfSwVersion=shelfSwVersion, diskVolumeFileName=diskVolumeFileName, auditDescription=auditDescription, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeEntry=diskVolumeEntry, cardTestCommand=cardTestCommand, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portRxFlowControlDetected=portRxFlowControlDetected, cardTemperature=cardTemperature, eqDiagConfig=eqDiagConfig, systemGeneral=systemGeneral, cardAdminState=cardAdminState, errEventId=errEventId, cardSwVersion=cardSwVersion, diskVolumeCardId=diskVolumeCardId, cardHwVersion=cardHwVersion, portDplxStatus=portDplxStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestType=cardTestType, TestTransactionId=TestTransactionId, auditTable=auditTable, equipmentCardGroup=equipmentCardGroup, cardCPLDVersion=cardCPLDVersion, cardFwVersion=cardFwVersion, cmResetUpTime=cmResetUpTime, portMacIfIndex=portMacIfIndex, remainInDiagMode=remainInDiagMode, auditEntry=auditEntry, cardTestTime=cardTestTime, diskVolumeDiskSize=diskVolumeDiskSize, systemKey=systemKey, cardDplxStatusChange=cardDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, cardNumPorts=cardNumPorts, shelfNumber=shelfNumber, cardFlashSize=cardFlashSize, cardShelfTestIndex=cardShelfTestIndex, equipmentDiag=equipmentDiag, cardTemperatureHighWarn=cardTemperatureHighWarn, auditStatus=auditStatus, cardTemperatureHighError=cardTemperatureHighError, equipmentStateGroup=equipmentStateGroup, portPrState=portPrState, portMinDsPower=portMinDsPower, diskVolumeShelfId=diskVolumeShelfId, auditFailedCount=auditFailedCount, TestType=TestType, diskVolumeTable=diskVolumeTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardBootVersion=cardBootVersion, cardTestId=cardTestId, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardLastBootVersion=cardLastBootVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentTbl=equipmentTbl, portBgpId=portBgpId, consoleOutput=consoleOutput, cardTestResultDesc=cardTestResultDesc, auditPassedCount=auditPassedCount, diskDriveNumber=diskDriveNumber, portRxFlowControlMode=portRxFlowControlMode, cardCpuSpeed=cardCpuSpeed, cardTestTransId=cardTestTransId, cmIpAddress=cmIpAddress, cardAction=cardAction, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestSchedule=cardTestSchedule, cardLastBootSource=cardLastBootSource, auditCommand=auditCommand, cardDetected=cardDetected, auditName=auditName, cadEquipmentMibGroup=cadEquipmentMibGroup, cardRamSize=cardRamSize, equipmentPortGroup=equipmentPortGroup, cardShelfId=cardShelfId, TestResult=TestResult, portTxFlowControlMode=portTxFlowControlMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeNumber=diskVolumeNumber, errEvLoggingEnabled=errEvLoggingEnabled, cardName=cardName, portDetectedMode=portDetectedMode, cmDevice=cmDevice, cardTable=cardTable, auditShelfId=auditShelfId, cardTestResultNotification=cardTestResultNotification, fpgaErrorEventEntry=fpgaErrorEventEntry, fpgaErrorEventTable=fpgaErrorEventTable, equipmentTraps=equipmentTraps, auditTotalFailedCount=auditTotalFailedCount, cardBusSpeed=cardBusSpeed, cardTestName=cardTestName, equipmentState=equipmentState, auditAutoScheduling=auditAutoScheduling, cardTestEntry=cardTestEntry, cardTestScheduleCommand=cardTestScheduleCommand, cardDetectedChange=cardDetectedChange, cmResetInfo=cmResetInfo, cardTestDescription=cardTestDescription, auditCardId=auditCardId, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeTrapInh=diskVolumeTrapInh, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestId=TestId, portPrStateChange=portPrStateChange, portType=portType, auditTime=auditTime, portAdminState=portAdminState, portId=portId, diskVolumeUsageNotification=diskVolumeUsageNotification, trapCounter=trapCounter, portMaxDsPower=portMaxDsPower, auditId=auditId, diskVolumeSize=diskVolumeSize, auditLogOutput=auditLogOutput, TestCommand=TestCommand, portMacAddress=portMacAddress, auditLogThrottle=auditLogThrottle, cardTempNoReportNotification=cardTempNoReportNotification, systemGeneralGroup=systemGeneralGroup, portShelfId=portShelfId, cardSubType=cardSubType, cardPrStateChange=cardPrStateChange, cardSecState=cardSecState, portNumber=portNumber, portCardSubType=portCardSubType, equipmentShelfGroup=equipmentShelfGroup, cardTestTable=cardTestTable, cardFwUpdateStatus=cardFwUpdateStatus, cardPicSerialNum=cardPicSerialNum, cmPrimarySID=cmPrimarySID, cardPrState=cardPrState, portNumChans=portNumChans, cadEquipmentMib=cadEquipmentMib, diskVolumeName=diskVolumeName, portMode=portMode, cardPicHwVersion=cardPicHwVersion, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResult=cardTestResult, cardLastChangeTime=cardLastChangeTime, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvLogLevel=errEvLogLevel, diskVolumeGroup=diskVolumeGroup, portCardId=portCardId, portCurrDsPower=portCurrDsPower, cmUChannelID=cmUChannelID, cardDplxStatus=cardDplxStatus, cardSpareGroupMode=cardSpareGroupMode, cmResetStatus=cmResetStatus, cardLastResetReason=cardLastResetReason, diskVolumeDriveId=diskVolumeDriveId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portAction=portAction, cardCpuType=cardCpuType, equipmentError=equipmentError, portDocsIfIndex=portDocsIfIndex, portTxFlowControlDetected=portTxFlowControlDetected, cmVendor=cmVendor, portSecState=portSecState, cardSpareGroupId=cardSpareGroupId)
