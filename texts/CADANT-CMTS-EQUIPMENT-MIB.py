...
SecondaryState, CardId, DiskVolumeUsageLevel, PortDetectedMode, CardType, PortMode, FlowControlMode, CardSubType, PortType, PicType, DuplexStatus, FirmwareSource, PortId, PrimaryState, EqActionType, ShelfId, UnknownState, AdminState = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "CardId", "DiskVolumeUsageLevel", "PortDetectedMode", "CardType", "PortMode", "FlowControlMode", "CardSubType", "PortType", "PicType", "DuplexStatus", "FirmwareSource", "PortId", "PrimaryState", "EqActionType", "ShelfId", "UnknownState", "AdminState")
...
MacAddress, TruthValue, TextualConvention, DisplayString, DateAndTime, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "TextualConvention", "DisplayString", "DateAndTime", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeName=diskVolumeName, portCurrDsPower=portCurrDsPower, auditId=auditId, cardTestTime=cardTestTime, cmDevice=cmDevice, cardCPLDVersion=cardCPLDVersion, portDplxStatus=portDplxStatus, cardCpuType=cardCpuType, cardTestName=cardTestName, trapCounter=trapCounter, cardSubDetected=cardSubDetected, cadEquipmentMibGroup=cadEquipmentMibGroup, cardDetectedChange=cardDetectedChange, cardRamSize=cardRamSize, cardDetected=cardDetected, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCycleCount=auditCycleCount, diskVolumeFileName=diskVolumeFileName, portBgpId=portBgpId, equipmentError=equipmentError, cardPrState=cardPrState, cardDplxStatusChange=cardDplxStatusChange, fpgaErrorEventGroup=fpgaErrorEventGroup, cardFlashSize=cardFlashSize, portDetectedMode=portDetectedMode, cardHwVersion=cardHwVersion, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditAutoScheduling=auditAutoScheduling, equipmentShelfGroup=equipmentShelfGroup, shelfSwVersion=shelfSwVersion, auditTable=auditTable, cardNumPorts=cardNumPorts, cmResetNotification=cmResetNotification, auditEntry=auditEntry, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentTraps=equipmentTraps, cardTemperature=cardTemperature, portCardId=portCardId, cardPicHwVersion=cardPicHwVersion, portTxFlowControlDetected=portTxFlowControlDetected, portMode=portMode, cardCpuSpeed=cardCpuSpeed, cardTestResult=cardTestResult, fpgaErrorEventTable=fpgaErrorEventTable, cardTestIndex=cardTestIndex, systemGeneral=systemGeneral, errEvRowStatus=errEvRowStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, cardName=cardName, cardPicDetected=cardPicDetected, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDocsIfIndex=portDocsIfIndex, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfNumber=shelfNumber, auditShelfId=auditShelfId, portRxFlowControlDetected=portRxFlowControlDetected, TestId=TestId, equipmentAudit=equipmentAudit, portRxFlowControlMode=portRxFlowControlMode, auditName=auditName, portAdminState=portAdminState, cardTestEntry=cardTestEntry, auditCommand=auditCommand, systemGeneralGroup=systemGeneralGroup, cardBootVersion=cardBootVersion, cardLastBootVersion=cardLastBootVersion, shelfObjects=shelfObjects, portType=portType, portMacAddress=portMacAddress, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestScheduleCommand=TestScheduleCommand, TestSchedule=TestSchedule, TestResult=TestResult, diskVolumeSize=diskVolumeSize, diskVolumeNumber=diskVolumeNumber, equipmentPortGroup=equipmentPortGroup, cardSubType=cardSubType, cardLastChangeTime=cardLastChangeTime, portAction=portAction, auditPassedCount=auditPassedCount, auditTotalPassedCount=auditTotalPassedCount, errEventId=errEventId, cardFpgaSource=cardFpgaSource, portPrStateChange=portPrStateChange, cmResetInfo=cmResetInfo, trapSeverity=trapSeverity, cardTempNoReportNotification=cardTempNoReportNotification, eqDiagConfig=eqDiagConfig, portSecStateChange=portSecStateChange, portDescription=portDescription, diskVolumeTable=diskVolumeTable, portNumChans=portNumChans, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardEntry=cardEntry, cardLastBootSource=cardLastBootSource, cardTrapInh=cardTrapInh, cardTemperatureHighError=cardTemperatureHighError, auditTotalFailedCount=auditTotalFailedCount, errEvLogLevel=errEvLogLevel, cardSpareGroupMode=cardSpareGroupMode, cardDplxStatus=cardDplxStatus, cardAdminState=cardAdminState, cmResetClearNotification=cmResetClearNotification, cardAction=cardAction, cardTestId=cardTestId, equipmentCardTestGroup=equipmentCardTestGroup, cardCarrierSerialNum=cardCarrierSerialNum, cardCarrierFwVersion=cardCarrierFwVersion, cardSpareGroupId=cardSpareGroupId, cardType=cardType, portShelfId=portShelfId, auditResult=auditResult, cardTestDescription=cardTestDescription, equipmentDiagGroup=equipmentDiagGroup, diskVolumeGroup=diskVolumeGroup, auditLogOutput=auditLogOutput, downstreamPowerLoss=downstreamPowerLoss, portId=portId, cardTestCommand=cardTestCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, portMinDsPower=portMinDsPower, diskVolumeDriveId=diskVolumeDriveId, cmUChannelID=cmUChannelID, cardTestSchedule=cardTestSchedule, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMacIfIndex=portMacIfIndex, consoleOutput=consoleOutput, diskVolumeCardId=diskVolumeCardId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, portNumber=portNumber, cmMacAddress=cmMacAddress, cmIpAddress=cmIpAddress, remainInDiagMode=remainInDiagMode, portMaxDsPower=portMaxDsPower, equipmentState=equipmentState, errEvLoggingEnabled=errEvLoggingEnabled, cmPrimarySID=cmPrimarySID, equipmentAuditGroup=equipmentAuditGroup, systemKey=systemKey, portCardSubType=portCardSubType, diskVolumeDiskSize=diskVolumeDiskSize, cardSecState=cardSecState, portLastChangeTime=portLastChangeTime, systemClock=systemClock, portConnectorId=portConnectorId, cmResetReason=cmResetReason, auditCardId=auditCardId, verboseLevel=verboseLevel, cardTestType=cardTestType, diskDriveNumber=diskDriveNumber, portSecState=portSecState, diskVolumeEntry=diskVolumeEntry, portPrState=portPrState, cardFwUpdateStatus=cardFwUpdateStatus, cardLastResetReason=cardLastResetReason, cardSerialNum=cardSerialNum, cardSwVersion=cardSwVersion, portTable=portTable, diskVolumeId=diskVolumeId, diskVolumeTrapInh=diskVolumeTrapInh, diagTestId=diagTestId, auditLogThrottle=auditLogThrottle, cardFwVersion=cardFwVersion, cardTestTransId=cardTestTransId, TestType=TestType, cardCarrierHwVersion=cardCarrierHwVersion, cardSecStateChange=cardSecStateChange, TestTransactionId=TestTransactionId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardPicSerialNum=cardPicSerialNum, cmResetUpTime=cmResetUpTime, diskVolumeShelfId=diskVolumeShelfId, shelfName=shelfName, cardId=cardId, cardUpTime=cardUpTime, cardTestResultDesc=cardTestResultDesc, cardPrStateChange=cardPrStateChange, portTrapInh=portTrapInh, equipmentCardGroup=equipmentCardGroup, cmVendor=cmVendor, cardShelfTestIndex=cardShelfTestIndex, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMib=cadEquipmentMib, cardTestTable=cardTestTable, cmResetStatus=cmResetStatus, portEntry=portEntry, cardTable=cardTable, cardBusSpeed=cardBusSpeed, auditFailedCount=auditFailedCount, equipmentStateGroup=equipmentStateGroup, TestCommand=TestCommand, portTxFlowControlMode=portTxFlowControlMode, cardTestResultNotification=cardTestResultNotification, auditTime=auditTime, cardNumber=cardNumber, equipmentDiag=equipmentDiag, PYSNMP_MODULE_ID=cadEquipmentMib, cardShelfId=cardShelfId, auditStatus=auditStatus, auditDescription=auditDescription, equipmentTbl=equipmentTbl)
