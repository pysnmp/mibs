...
CardType, PortType, DuplexStatus, PortId, PortDetectedMode, ShelfId, EqActionType, PicType, AdminState, PrimaryState, DiskVolumeUsageLevel, CardId, PortMode, CardSubType, SecondaryState, UnknownState, FirmwareSource, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortType", "DuplexStatus", "PortId", "PortDetectedMode", "ShelfId", "EqActionType", "PicType", "AdminState", "PrimaryState", "DiskVolumeUsageLevel", "CardId", "PortMode", "CardSubType", "SecondaryState", "UnknownState", "FirmwareSource", "FlowControlMode")
...
MacAddress, RowStatus, TextualConvention, TimeStamp, DisplayString, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "TextualConvention", "TimeStamp", "DisplayString", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeId=diskVolumeId, remainInDiagMode=remainInDiagMode, cardTestResultDesc=cardTestResultDesc, portLastChangeTime=portLastChangeTime, cardFpgaSource=cardFpgaSource, shelfName=shelfName, cmIpAddress=cmIpAddress, portPrState=portPrState, equipmentCardTestGroup=equipmentCardTestGroup, systemGeneral=systemGeneral, cmResetClearNotification=cmResetClearNotification, downstreamPowerLoss=downstreamPowerLoss, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardShelfId=cardShelfId, portRxFlowControlMode=portRxFlowControlMode, auditTotalFailedCount=auditTotalFailedCount, equipmentError=equipmentError, cardFwUpdateStatus=cardFwUpdateStatus, diskDriveNumber=diskDriveNumber, cmVendor=cmVendor, TestType=TestType, cardTestResultNotification=cardTestResultNotification, equipmentDiag=equipmentDiag, cardTestResult=cardTestResult, cardTestIndex=cardTestIndex, fpgaErrorEventGroup=fpgaErrorEventGroup, auditTable=auditTable, cardTestId=cardTestId, auditTime=auditTime, cardType=cardType, cardCpuType=cardCpuType, errEvRecoveryEnabled=errEvRecoveryEnabled, portConnectorId=portConnectorId, cardTempNoReportNotification=cardTempNoReportNotification, cardFwVersion=cardFwVersion, cardTestType=cardTestType, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentDiagGroup=equipmentDiagGroup, portShelfId=portShelfId, portDplxStatus=portDplxStatus, cardEntry=cardEntry, cardNumPorts=cardNumPorts, cardTestScheduleCommand=cardTestScheduleCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, trapCounter=trapCounter, cardSpareGroupId=cardSpareGroupId, diskVolumeGroup=diskVolumeGroup, errEvRowStatus=errEvRowStatus, portId=portId, diskVolumeTrapInh=diskVolumeTrapInh, equipmentState=equipmentState, cardDetectedChange=cardDetectedChange, cardPicHwVersion=cardPicHwVersion, diskVolumeEntry=diskVolumeEntry, diskVolumeFileName=diskVolumeFileName, portSecState=portSecState, auditShelfId=auditShelfId, auditTotalPassedCount=auditTotalPassedCount, cardTable=cardTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portMacIfIndex=portMacIfIndex, portTrapInh=portTrapInh, cardTestEntry=cardTestEntry, systemGeneralGroup=systemGeneralGroup, portAction=portAction, cardPicDetected=cardPicDetected, systemKey=systemKey, shelfObjects=shelfObjects, TestScheduleCommand=TestScheduleCommand, cardTemperatureHighError=cardTemperatureHighError, diskVolumeCardId=diskVolumeCardId, diagTestId=diagTestId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetReason=cmResetReason, auditResult=auditResult, portBgpId=portBgpId, consoleOutput=consoleOutput, equipmentAuditGroup=equipmentAuditGroup, systemClock=systemClock, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardAction=cardAction, diskVolumeDiskSize=diskVolumeDiskSize, TestCommand=TestCommand, cardPrStateChange=cardPrStateChange, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastBootVersion=cardLastBootVersion, cardBootVersion=cardBootVersion, diskVolumeTable=diskVolumeTable, auditLogOutput=auditLogOutput, auditCardId=auditCardId, cmRegistrationNotification=cmRegistrationNotification, cmResetInfo=cmResetInfo, cardLastChangeTime=cardLastChangeTime, cmUChannelID=cmUChannelID, portAdminState=portAdminState, verboseLevel=verboseLevel, cmPrimarySID=cmPrimarySID, auditPassedCount=auditPassedCount, cardId=cardId, cardTemperature=cardTemperature, eqDiagConfig=eqDiagConfig, cardTestTable=cardTestTable, cadEquipmentMibConformance=cadEquipmentMibConformance, cmResetNotification=cmResetNotification, diskVolumeName=diskVolumeName, portSecStateChange=portSecStateChange, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditCycleCount=auditCycleCount, diskVolumeUsageLevel=diskVolumeUsageLevel, auditLogThrottle=auditLogThrottle, cardTestTransId=cardTestTransId, diskVolumeDriveId=diskVolumeDriveId, TestSchedule=TestSchedule, trapSeverity=trapSeverity, equipmentStateGroup=equipmentStateGroup, cardSecState=cardSecState, auditName=auditName, cardDplxStatus=cardDplxStatus, cardFlashSize=cardFlashSize, TestId=TestId, cardSubType=cardSubType, portPrStateChange=portPrStateChange, fpgaErrorEventTable=fpgaErrorEventTable, equipmentAudit=equipmentAudit, cardCarrierFwVersion=cardCarrierFwVersion, auditAutoScheduling=auditAutoScheduling, diskVolumeSize=diskVolumeSize, shelfSwVersion=shelfSwVersion, equipmentTraps=equipmentTraps, portDescription=portDescription, cardAdminState=cardAdminState, portTxFlowControlMode=portTxFlowControlMode, TestTransactionId=TestTransactionId, cardSwVersion=cardSwVersion, portCurrDsPower=portCurrDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmMacAddress=cmMacAddress, equipmentTbl=equipmentTbl, portMode=portMode, errEvLogLevel=errEvLogLevel, cardShelfTestIndex=cardShelfTestIndex, cardName=cardName, cardCarrierHwVersion=cardCarrierHwVersion, equipmentCardGroup=equipmentCardGroup, diskVolumeNumber=diskVolumeNumber, cardTestTime=cardTestTime, cardSerialNum=cardSerialNum, PYSNMP_MODULE_ID=cadEquipmentMib, portDocsIfIndex=portDocsIfIndex, cardRamSize=cardRamSize, cmResetUpTime=cmResetUpTime, cardSecStateChange=cardSecStateChange, equipmentPortGroup=equipmentPortGroup, portNumChans=portNumChans, cardTestDescription=cardTestDescription, portTxFlowControlDetected=portTxFlowControlDetected, cmDevice=cmDevice, cmResetStatus=cmResetStatus, errEvLoggingEnabled=errEvLoggingEnabled, cardNumber=cardNumber, equipmentShelfGroup=equipmentShelfGroup, portNumber=portNumber, cardCarrierSerialNum=cardCarrierSerialNum, portMaxDsPower=portMaxDsPower, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditFailedCount=auditFailedCount, shelfNumber=shelfNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, portEntry=portEntry, auditCommand=auditCommand, portMinDsPower=portMinDsPower, portTable=portTable, cardHwVersion=cardHwVersion, cadEquipmentMib=cadEquipmentMib, cardBusSpeed=cardBusSpeed, cardPicSerialNum=cardPicSerialNum, auditDescription=auditDescription, cardSpareGroupMode=cardSpareGroupMode, cardDetected=cardDetected, cardUpTime=cardUpTime, portCardSubType=portCardSubType, cardTestSchedule=cardTestSchedule, auditId=auditId, cardLastResetReason=cardLastResetReason, portCardId=portCardId, cardTestName=cardTestName, cardCpuSpeed=cardCpuSpeed, cardPrState=cardPrState, cardDplxStatusChange=cardDplxStatusChange, TestResult=TestResult, auditEntry=auditEntry, auditStatus=auditStatus, errEventId=errEventId, cardCPLDVersion=cardCPLDVersion, portDetectedMode=portDetectedMode, diskVolumeShelfId=diskVolumeShelfId, portMacAddress=portMacAddress, cardLastBootSource=cardLastBootSource, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestCommand=cardTestCommand, cardSubDetected=cardSubDetected, cardTrapInh=cardTrapInh, portType=portType)
