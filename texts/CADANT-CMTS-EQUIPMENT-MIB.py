...
CardId, PortId, UnknownState, EqActionType, ShelfId, CardType, PortDetectedMode, PortType, DuplexStatus, FirmwareSource, CardSubType, AdminState, PicType, SecondaryState, PrimaryState, DiskVolumeUsageLevel, PortMode, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortId", "UnknownState", "EqActionType", "ShelfId", "CardType", "PortDetectedMode", "PortType", "DuplexStatus", "FirmwareSource", "CardSubType", "AdminState", "PicType", "SecondaryState", "PrimaryState", "DiskVolumeUsageLevel", "PortMode", "FlowControlMode")
...
TruthValue, MacAddress, RowStatus, DateAndTime, DisplayString, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "MacAddress", "RowStatus", "DateAndTime", "DisplayString", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardHwVersion=cardHwVersion, equipmentState=equipmentState, auditTotalPassedCount=auditTotalPassedCount, cardTestName=cardTestName, portDetectedMode=portDetectedMode, auditLogOutput=auditLogOutput, portBgpId=portBgpId, portDescription=portDescription, auditTable=auditTable, cardLastChangeTime=cardLastChangeTime, cardRamSize=cardRamSize, auditEntry=auditEntry, cardSerialNum=cardSerialNum, diskVolumeTrapInh=diskVolumeTrapInh, cmResetNotification=cmResetNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, errEvLoggingEnabled=errEvLoggingEnabled, cardTable=cardTable, portMacIfIndex=portMacIfIndex, consoleOutput=consoleOutput, diskDriveNumber=diskDriveNumber, cardTemperature=cardTemperature, cardSpareGroupId=cardSpareGroupId, equipmentError=equipmentError, TestId=TestId, cardTestTime=cardTestTime, equipmentCardTestGroup=equipmentCardTestGroup, portType=portType, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestId=cardTestId, cardSecStateChange=cardSecStateChange, cmVendor=cmVendor, diskVolumeTable=diskVolumeTable, portEntry=portEntry, cardTestScheduleCommand=cardTestScheduleCommand, equipmentDiagGroup=equipmentDiagGroup, cardShelfId=cardShelfId, cardName=cardName, cardFwUpdateStatus=cardFwUpdateStatus, diagTestId=diagTestId, cardTestTable=cardTestTable, cardPicDetected=cardPicDetected, equipmentAuditGroup=equipmentAuditGroup, shelfNumber=shelfNumber, cmIpAddress=cmIpAddress, trapCounter=trapCounter, cardSpareGroupMode=cardSpareGroupMode, cardCarrierFwVersion=cardCarrierFwVersion, cmResetReason=cmResetReason, cardSecState=cardSecState, auditName=auditName, auditLogThrottle=auditLogThrottle, cardCpuType=cardCpuType, portPrStateChange=portPrStateChange, equipmentCardGroup=equipmentCardGroup, portLastChangeTime=portLastChangeTime, cardId=cardId, auditCommand=auditCommand, cardTemperatureHighError=cardTemperatureHighError, equipmentAudit=equipmentAudit, downstreamPowerLoss=downstreamPowerLoss, cardSubType=cardSubType, portMacAddress=portMacAddress, cardDplxStatus=cardDplxStatus, cardBootVersion=cardBootVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, trapSeverity=trapSeverity, cardTestType=cardTestType, cardUpTime=cardUpTime, portId=portId, portNumber=portNumber, cardType=cardType, portCurrDsPower=portCurrDsPower, portConnectorId=portConnectorId, cardTestSchedule=cardTestSchedule, cardDplxStatusChange=cardDplxStatusChange, cmResetUpTime=cmResetUpTime, TestType=TestType, diskVolumeCardId=diskVolumeCardId, diskVolumeEntry=diskVolumeEntry, cardPrState=cardPrState, verboseLevel=verboseLevel, errEvRowStatus=errEvRowStatus, cmResetInfo=cmResetInfo, portTxFlowControlMode=portTxFlowControlMode, cardAction=cardAction, auditResult=auditResult, errEventId=errEventId, TestCommand=TestCommand, portTxFlowControlDetected=portTxFlowControlDetected, shelfObjects=shelfObjects, cardSubDetected=cardSubDetected, cardLastBootVersion=cardLastBootVersion, cardSwVersion=cardSwVersion, cardTestResultDesc=cardTestResultDesc, auditTime=auditTime, cardCarrierSerialNum=cardCarrierSerialNum, cardDetectedChange=cardDetectedChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditCycleCount=auditCycleCount, cardTestEntry=cardTestEntry, cmResetStatus=cmResetStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvLogLevel=errEvLogLevel, auditCardId=auditCardId, cardFpgaSource=cardFpgaSource, systemGeneralGroup=systemGeneralGroup, cardTempNoReportNotification=cardTempNoReportNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, remainInDiagMode=remainInDiagMode, auditId=auditId, systemClock=systemClock, cardCPLDVersion=cardCPLDVersion, portMaxDsPower=portMaxDsPower, portTrapInh=portTrapInh, fpgaErrorEventTable=fpgaErrorEventTable, cardTestResult=cardTestResult, cardNumPorts=cardNumPorts, cardTrapInh=cardTrapInh, cmPrimarySID=cmPrimarySID, equipmentPortGroup=equipmentPortGroup, auditTotalFailedCount=auditTotalFailedCount, diskVolumeSize=diskVolumeSize, cardPicHwVersion=cardPicHwVersion, diskVolumeDriveId=diskVolumeDriveId, cardTestIndex=cardTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, PYSNMP_MODULE_ID=cadEquipmentMib, portDplxStatus=portDplxStatus, portSecStateChange=portSecStateChange, equipmentShelfGroup=equipmentShelfGroup, cardEntry=cardEntry, diskVolumeFileName=diskVolumeFileName, cardAdminState=cardAdminState, cardTestDescription=cardTestDescription, cardCarrierHwVersion=cardCarrierHwVersion, portPrState=portPrState, equipmentTraps=equipmentTraps, portRxFlowControlMode=portRxFlowControlMode, cadEquipmentMib=cadEquipmentMib, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestTransId=cardTestTransId, cmMacAddress=cmMacAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, portDplxStatusChange=portDplxStatusChange, cmRegistrationNotification=cmRegistrationNotification, portSecState=portSecState, auditAutoScheduling=auditAutoScheduling, cardFwVersion=cardFwVersion, cardCpuSpeed=cardCpuSpeed, portAdminState=portAdminState, auditFailedCount=auditFailedCount, diskVolumeDiskSize=diskVolumeDiskSize, cardTestCommand=cardTestCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardPrStateChange=cardPrStateChange, systemGeneral=systemGeneral, cmUChannelID=cmUChannelID, cardLastResetReason=cardLastResetReason, portCardId=portCardId, TestTransactionId=TestTransactionId, diskVolumeShelfId=diskVolumeShelfId, cardShelfTestIndex=cardShelfTestIndex, diskVolumeGroup=diskVolumeGroup, equipmentDiag=equipmentDiag, cardTestResultNotification=cardTestResultNotification, auditDescription=auditDescription, portMinDsPower=portMinDsPower, cardDetected=cardDetected, cardBusSpeed=cardBusSpeed, cardLastBootSource=cardLastBootSource, systemKey=systemKey, cardNumber=cardNumber, diskVolumeNumber=diskVolumeNumber, equipmentTbl=equipmentTbl, portNumChans=portNumChans, diskVolumeUsageLevel=diskVolumeUsageLevel, portTable=portTable, TestSchedule=TestSchedule, portRxFlowControlDetected=portRxFlowControlDetected, TestScheduleCommand=TestScheduleCommand, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibConformance=cadEquipmentMibConformance, portAction=portAction, shelfName=shelfName, shelfSwVersion=shelfSwVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditStatus=auditStatus, cmResetClearNotification=cmResetClearNotification, cmDevice=cmDevice, diskVolumeId=diskVolumeId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditShelfId=auditShelfId, diskVolumeName=diskVolumeName, diskVolumeUsageNotification=diskVolumeUsageNotification, TestResult=TestResult, portCardSubType=portCardSubType, portMode=portMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, eqDiagConfig=eqDiagConfig, portDocsIfIndex=portDocsIfIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentStateGroup=equipmentStateGroup, auditPassedCount=auditPassedCount, portShelfId=portShelfId, cardFlashSize=cardFlashSize)
