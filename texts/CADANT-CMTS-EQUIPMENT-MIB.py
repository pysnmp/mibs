...
SecondaryState, FlowControlMode, CardType, UnknownState, CardSubType, PortDetectedMode, AdminState, CardId, PicType, DiskVolumeUsageLevel, PortType, ShelfId, PortMode, PortId, PrimaryState, FirmwareSource, DuplexStatus, EqActionType = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "FlowControlMode", "CardType", "UnknownState", "CardSubType", "PortDetectedMode", "AdminState", "CardId", "PicType", "DiskVolumeUsageLevel", "PortType", "ShelfId", "PortMode", "PortId", "PrimaryState", "FirmwareSource", "DuplexStatus", "EqActionType")
...
MacAddress, RowStatus, TextualConvention, DateAndTime, TruthValue, DisplayString, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "TextualConvention", "DateAndTime", "TruthValue", "DisplayString", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPrState=cardPrState, diskVolumeName=diskVolumeName, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCpuSpeed=cardCpuSpeed, portNumber=portNumber, auditFailedCount=auditFailedCount, cardCarrierFwVersion=cardCarrierFwVersion, portPrStateChange=portPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditShelfId=auditShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, cmDevice=cmDevice, portType=portType, portMacIfIndex=portMacIfIndex, shelfObjects=shelfObjects, cardTestResultDesc=cardTestResultDesc, auditId=auditId, portDetectedMode=portDetectedMode, portTrapInh=portTrapInh, shelfName=shelfName, cardDetectedChange=cardDetectedChange, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResultNotification=cardTestResultNotification, cardTable=cardTable, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastChangeTime=cardLastChangeTime, cardCPLDVersion=cardCPLDVersion, cardFwUpdateStatus=cardFwUpdateStatus, cardPrStateChange=cardPrStateChange, portDocsIfIndex=portDocsIfIndex, cardNumber=cardNumber, TestCommand=TestCommand, portNumChans=portNumChans, cmResetNotification=cmResetNotification, cardDetected=cardDetected, portTable=portTable, diskVolumeTable=diskVolumeTable, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibGroup=cadEquipmentMibGroup, portRxFlowControlMode=portRxFlowControlMode, errEventId=errEventId, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDiskSize=diskVolumeDiskSize, TestTransactionId=TestTransactionId, cardLastBootSource=cardLastBootSource, cardType=cardType, equipmentAuditGroup=equipmentAuditGroup, cardEntry=cardEntry, equipmentShelfGroup=equipmentShelfGroup, cardCarrierSerialNum=cardCarrierSerialNum, trapSeverity=trapSeverity, diskVolumeId=diskVolumeId, cardTrapInh=cardTrapInh, diskVolumeFileName=diskVolumeFileName, diskVolumeUsagePercentage=diskVolumeUsagePercentage, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMib=cadEquipmentMib, portSecStateChange=portSecStateChange, auditStatus=auditStatus, trapCounter=trapCounter, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeDriveId=diskVolumeDriveId, cardTestIndex=cardTestIndex, cardName=cardName, diskVolumeCardId=diskVolumeCardId, errEvRowStatus=errEvRowStatus, cmRegistrationNotification=cmRegistrationNotification, shelfSwVersion=shelfSwVersion, portDplxStatus=portDplxStatus, cardTestSchedule=cardTestSchedule, systemGeneralGroup=systemGeneralGroup, cmMacAddress=cmMacAddress, equipmentStateGroup=equipmentStateGroup, cmPrimarySID=cmPrimarySID, portLastChangeTime=portLastChangeTime, TestType=TestType, TestScheduleCommand=TestScheduleCommand, equipmentState=equipmentState, cardCpuType=cardCpuType, diskVolumeTrapInh=diskVolumeTrapInh, cardTestTransId=cardTestTransId, TestSchedule=TestSchedule, portConnectorId=portConnectorId, cardTestCommand=cardTestCommand, cmResetReason=cmResetReason, cardLastResetReason=cardLastResetReason, cardTestName=cardTestName, auditLogThrottle=auditLogThrottle, errEvLoggingEnabled=errEvLoggingEnabled, auditResult=auditResult, cardTemperatureHighWarn=cardTemperatureHighWarn, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardBootVersion=cardBootVersion, downstreamPowerLoss=downstreamPowerLoss, diskVolumeShelfId=diskVolumeShelfId, PYSNMP_MODULE_ID=cadEquipmentMib, portMaxDsPower=portMaxDsPower, cmResetStatus=cmResetStatus, equipmentPortGroup=equipmentPortGroup, auditAutoScheduling=auditAutoScheduling, auditTime=auditTime, cardTestEntry=cardTestEntry, cardId=cardId, portTxFlowControlMode=portTxFlowControlMode, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestId=TestId, errEvLogLevel=errEvLogLevel, cardBusSpeed=cardBusSpeed, shelfNumber=shelfNumber, portCurrDsPower=portCurrDsPower, cardSubType=cardSubType, cardShelfTestIndex=cardShelfTestIndex, auditCardId=auditCardId, auditTable=auditTable, cmVendor=cmVendor, portMacAddress=portMacAddress, cardDplxStatus=cardDplxStatus, cardTestScheduleCommand=cardTestScheduleCommand, portCardSubType=portCardSubType, portDescription=portDescription, portMode=portMode, cardTemperature=cardTemperature, cardAction=cardAction, portEntry=portEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, cardUpTime=cardUpTime, cardPicDetected=cardPicDetected, cmUChannelID=cmUChannelID, auditCycleCount=auditCycleCount, diskVolumeSize=diskVolumeSize, cmResetInfo=cmResetInfo, auditPassedCount=auditPassedCount, systemGeneral=systemGeneral, cardSerialNum=cardSerialNum, cardSpareGroupMode=cardSpareGroupMode, portMinDsPower=portMinDsPower, consoleOutput=consoleOutput, cardFwVersion=cardFwVersion, cardSwVersion=cardSwVersion, auditTotalFailedCount=auditTotalFailedCount, portCardId=portCardId, cardTestResult=cardTestResult, cardSubDetected=cardSubDetected, portDplxStatusChange=portDplxStatusChange, cardFpgaSource=cardFpgaSource, fpgaErrorEventTable=fpgaErrorEventTable, cardHwVersion=cardHwVersion, verboseLevel=verboseLevel, cardTestDescription=cardTestDescription, diskVolumeEntry=diskVolumeEntry, cardSecStateChange=cardSecStateChange, diskVolumeGroup=diskVolumeGroup, systemKey=systemKey, equipmentDiag=equipmentDiag, cardPicHwVersion=cardPicHwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentError=equipmentError, cardTestTable=cardTestTable, cardNumPorts=cardNumPorts, portTxFlowControlDetected=portTxFlowControlDetected, auditDescription=auditDescription, equipmentDiagGroup=equipmentDiagGroup, auditCommand=auditCommand, cardCarrierHwVersion=cardCarrierHwVersion, cardTestType=cardTestType, portAction=portAction, equipmentTraps=equipmentTraps, auditLogOutput=auditLogOutput, portShelfId=portShelfId, systemClock=systemClock, diskVolumeNumber=diskVolumeNumber, portSecState=portSecState, equipmentTbl=equipmentTbl, cmIpAddress=cmIpAddress, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diagTestId=diagTestId, cardTestId=cardTestId, cardSpareGroupId=cardSpareGroupId, auditName=auditName, portId=portId, cmResetUpTime=cmResetUpTime, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskDriveNumber=diskDriveNumber, equipmentAudit=equipmentAudit, cardRamSize=cardRamSize, cardSecState=cardSecState, portPrState=portPrState, cardTestTime=cardTestTime, TestResult=TestResult, cardFlashSize=cardFlashSize, cardDplxStatusChange=cardDplxStatusChange, cmResetClearNotification=cmResetClearNotification, portAdminState=portAdminState, cardLastBootVersion=cardLastBootVersion, auditTotalPassedCount=auditTotalPassedCount, cardAdminState=cardAdminState, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTemperatureHighError=cardTemperatureHighError, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardShelfId=cardShelfId, cardTempNoReportNotification=cardTempNoReportNotification, equipmentCardGroup=equipmentCardGroup, auditEntry=auditEntry, portBgpId=portBgpId, eqDiagConfig=eqDiagConfig, remainInDiagMode=remainInDiagMode)
