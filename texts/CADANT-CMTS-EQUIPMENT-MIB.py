...
PicType, EqActionType, ShelfId, PrimaryState, FlowControlMode, CardId, PortMode, AdminState, CardType, PortId, DuplexStatus, FirmwareSource, PortType, SecondaryState, PortDetectedMode, UnknownState, DiskVolumeUsageLevel, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PicType", "EqActionType", "ShelfId", "PrimaryState", "FlowControlMode", "CardId", "PortMode", "AdminState", "CardType", "PortId", "DuplexStatus", "FirmwareSource", "PortType", "SecondaryState", "PortDetectedMode", "UnknownState", "DiskVolumeUsageLevel", "CardSubType")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", fpgaErrorEventTable=fpgaErrorEventTable, cardRamSize=cardRamSize, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvRowStatus=errEvRowStatus, cardHwVersion=cardHwVersion, shelfObjects=shelfObjects, equipmentAuditGroup=equipmentAuditGroup, cardDetected=cardDetected, portId=portId, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeDriveId=diskVolumeDriveId, cardTestId=cardTestId, cardLastResetReason=cardLastResetReason, portNumChans=portNumChans, diagTestId=diagTestId, cardTestSchedule=cardTestSchedule, systemGeneralGroup=systemGeneralGroup, diskVolumeGroup=diskVolumeGroup, portTrapInh=portTrapInh, portMacIfIndex=portMacIfIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, remainInDiagMode=remainInDiagMode, errEvLogLevel=errEvLogLevel, cardSecState=cardSecState, cardType=cardType, cardFwUpdateStatus=cardFwUpdateStatus, shelfName=shelfName, shelfSwVersion=shelfSwVersion, auditFailedCount=auditFailedCount, cmPrimarySID=cmPrimarySID, cardSubDetected=cardSubDetected, portDescription=portDescription, diskDriveNumber=diskDriveNumber, cardCarrierHwVersion=cardCarrierHwVersion, portPrStateChange=portPrStateChange, portConnectorId=portConnectorId, equipmentDiagGroup=equipmentDiagGroup, portMinDsPower=portMinDsPower, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmMacAddress=cmMacAddress, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portTxFlowControlMode=portTxFlowControlMode, TestType=TestType, cardTemperatureHighError=cardTemperatureHighError, systemClock=systemClock, diskVolumeCardId=diskVolumeCardId, equipmentStateGroup=equipmentStateGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardTestScheduleCommand=cardTestScheduleCommand, equipmentTraps=equipmentTraps, portDocsIfIndex=portDocsIfIndex, portBgpId=portBgpId, auditTime=auditTime, equipmentShelfGroup=equipmentShelfGroup, equipmentDiag=equipmentDiag, cardPicHwVersion=cardPicHwVersion, equipmentCardGroup=equipmentCardGroup, shelfNumber=shelfNumber, portSecState=portSecState, cmRegistrationNotification=cmRegistrationNotification, cmResetReason=cmResetReason, cardUpTime=cardUpTime, PYSNMP_MODULE_ID=cadEquipmentMib, auditTable=auditTable, cardAction=cardAction, cardSpareGroupId=cardSpareGroupId, portEntry=portEntry, portDplxStatus=portDplxStatus, portType=portType, cadEquipmentMibConformance=cadEquipmentMibConformance, TestSchedule=TestSchedule, equipmentState=equipmentState, portCardId=portCardId, portCardSubType=portCardSubType, cardTempNoReportNotification=cardTempNoReportNotification, portTable=portTable, portNumber=portNumber, cardCarrierSerialNum=cardCarrierSerialNum, consoleOutput=consoleOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierFwVersion=cardCarrierFwVersion, portMacAddress=portMacAddress, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmDevice=cmDevice, cardTable=cardTable, fpgaErrorEventEntry=fpgaErrorEventEntry, portSecStateChange=portSecStateChange, auditName=auditName, cardLastChangeTime=cardLastChangeTime, cmResetNotification=cmResetNotification, auditEntry=auditEntry, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestName=cardTestName, cardId=cardId, auditLogOutput=auditLogOutput, cardFwVersion=cardFwVersion, diskVolumeNumber=diskVolumeNumber, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentTbl=equipmentTbl, cardCpuType=cardCpuType, auditPassedCount=auditPassedCount, errEvLoggingEnabled=errEvLoggingEnabled, cmResetClearNotification=cmResetClearNotification, auditId=auditId, cardLastBootSource=cardLastBootSource, cardTestTime=cardTestTime, errEventId=errEventId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, systemGeneral=systemGeneral, portAdminState=portAdminState, cardPrStateChange=cardPrStateChange, diskVolumeTable=diskVolumeTable, TestCommand=TestCommand, auditCycleCount=auditCycleCount, trapSeverity=trapSeverity, auditStatus=auditStatus, cmVendor=cmVendor, cardFpgaSource=cardFpgaSource, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditTotalPassedCount=auditTotalPassedCount, cardTestCommand=cardTestCommand, cardNumber=cardNumber, cardCPLDVersion=cardCPLDVersion, cardShelfTestIndex=cardShelfTestIndex, cardNumPorts=cardNumPorts, equipmentError=equipmentError, equipmentPortGroup=equipmentPortGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeShelfId=diskVolumeShelfId, systemKey=systemKey, cardBootVersion=cardBootVersion, cardCpuSpeed=cardCpuSpeed, TestId=TestId, cardShelfId=cardShelfId, cardTemperature=cardTemperature, diskVolumeEntry=diskVolumeEntry, TestTransactionId=TestTransactionId, equipmentAudit=equipmentAudit, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTrapInh=cardTrapInh, auditResult=auditResult, cardSpareGroupMode=cardSpareGroupMode, TestScheduleCommand=TestScheduleCommand, cardFlashSize=cardFlashSize, portMode=portMode, cmIpAddress=cmIpAddress, auditDescription=auditDescription, auditTotalFailedCount=auditTotalFailedCount, cmUChannelID=cmUChannelID, diskVolumeFileName=diskVolumeFileName, cardSerialNum=cardSerialNum, trapCounter=trapCounter, cardAdminState=cardAdminState, cardTemperatureHighWarn=cardTemperatureHighWarn, auditLogThrottle=auditLogThrottle, portDplxStatusChange=portDplxStatusChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardPicDetected=cardPicDetected, diskVolumeSize=diskVolumeSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardName=cardName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeTrapInh=diskVolumeTrapInh, cardTestDescription=cardTestDescription, cardTestTable=cardTestTable, cardTestResultDesc=cardTestResultDesc, portPrState=portPrState, cardBusSpeed=cardBusSpeed, cardPrState=cardPrState, cardTestTransId=cardTestTransId, portAction=portAction, cardLastBootVersion=cardLastBootVersion, diskVolumeName=diskVolumeName, cardSecStateChange=cardSecStateChange, verboseLevel=verboseLevel, portShelfId=portShelfId, portCurrDsPower=portCurrDsPower, diskVolumeDiskSize=diskVolumeDiskSize, auditCommand=auditCommand, cardPicSerialNum=cardPicSerialNum, cardDplxStatus=cardDplxStatus, auditCardId=auditCardId, cmResetUpTime=cmResetUpTime, cmResetInfo=cmResetInfo, diskVolumeId=diskVolumeId, cmResetStatus=cmResetStatus, portRxFlowControlMode=portRxFlowControlMode, auditShelfId=auditShelfId, portMaxDsPower=portMaxDsPower, cardEntry=cardEntry, cardDplxStatusChange=cardDplxStatusChange, downstreamPowerLoss=downstreamPowerLoss, auditAutoScheduling=auditAutoScheduling, cardTestIndex=cardTestIndex, portDetectedMode=portDetectedMode, portLastChangeTime=portLastChangeTime, cardSwVersion=cardSwVersion, cardTestResult=cardTestResult, cardTestEntry=cardTestEntry, equipmentCardTestGroup=equipmentCardTestGroup, cardTestResultNotification=cardTestResultNotification, cardDetectedChange=cardDetectedChange, cardSubType=cardSubType, cardTestType=cardTestType, TestResult=TestResult, eqDiagConfig=eqDiagConfig)
