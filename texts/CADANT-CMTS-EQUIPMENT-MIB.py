...
CardId, CardSubType, SecondaryState, EqActionType, PortId, PortMode, FirmwareSource, PortType, ShelfId, UnknownState, CardType, DuplexStatus, AdminState, PicType, DiskVolumeUsageLevel, PrimaryState, FlowControlMode, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "CardId", "CardSubType", "SecondaryState", "EqActionType", "PortId", "PortMode", "FirmwareSource", "PortType", "ShelfId", "UnknownState", "CardType", "DuplexStatus", "AdminState", "PicType", "DiskVolumeUsageLevel", "PrimaryState", "FlowControlMode", "PortDetectedMode")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardDplxStatus=cardDplxStatus, cardName=cardName, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCpuSpeed=cardCpuSpeed, auditTable=auditTable, cardTestTime=cardTestTime, cardBusSpeed=cardBusSpeed, portSecStateChange=portSecStateChange, cmResetNotification=cmResetNotification, fpgaErrorEventTable=fpgaErrorEventTable, equipmentState=equipmentState, portCardId=portCardId, cardTestScheduleCommand=cardTestScheduleCommand, cmMacAddress=cmMacAddress, TestCommand=TestCommand, portMacIfIndex=portMacIfIndex, auditTotalPassedCount=auditTotalPassedCount, cardCarrierHwVersion=cardCarrierHwVersion, systemKey=systemKey, shelfObjects=shelfObjects, cardSpareGroupMode=cardSpareGroupMode, portDplxStatusChange=portDplxStatusChange, auditTotalFailedCount=auditTotalFailedCount, cardTestName=cardTestName, portPrState=portPrState, equipmentError=equipmentError, diskVolumeNumber=diskVolumeNumber, downstreamPowerLoss=downstreamPowerLoss, cmResetUpTime=cmResetUpTime, systemGeneral=systemGeneral, TestResult=TestResult, cardTestTransId=cardTestTransId, errEvLoggingEnabled=errEvLoggingEnabled, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentPortGroup=equipmentPortGroup, systemGeneralGroup=systemGeneralGroup, cardBootVersion=cardBootVersion, auditLogThrottle=auditLogThrottle, cardPicDetected=cardPicDetected, portDocsIfIndex=portDocsIfIndex, cardTestType=cardTestType, cardTestTable=cardTestTable, diskVolumeShelfId=diskVolumeShelfId, cardTempNoReportNotification=cardTempNoReportNotification, portBgpId=portBgpId, cardSerialNum=cardSerialNum, consoleOutput=consoleOutput, equipmentCardTestGroup=equipmentCardTestGroup, cardLastBootSource=cardLastBootSource, shelfSwVersion=shelfSwVersion, eqDiagConfig=eqDiagConfig, portCardSubType=portCardSubType, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSubDetected=cardSubDetected, cardSecState=cardSecState, portTrapInh=portTrapInh, cmResetReason=cmResetReason, cardPicHwVersion=cardPicHwVersion, portId=portId, cardEntry=cardEntry, shelfName=shelfName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeEntry=diskVolumeEntry, cardRamSize=cardRamSize, cardFpgaSource=cardFpgaSource, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cadEquipmentMib=cadEquipmentMib, cmResetInfo=cmResetInfo, cardTestId=cardTestId, cmDevice=cmDevice, systemClock=systemClock, cardTemperatureHighWarn=cardTemperatureHighWarn, cmIpAddress=cmIpAddress, errEvRowStatus=errEvRowStatus, portEntry=portEntry, cardNumPorts=cardNumPorts, cardFlashSize=cardFlashSize, diskVolumeFileName=diskVolumeFileName, cmResetClearNotification=cmResetClearNotification, cardTrapInh=cardTrapInh, diskVolumeName=diskVolumeName, cardTestEntry=cardTestEntry, portMode=portMode, cardTestResultNotification=cardTestResultNotification, cardLastChangeTime=cardLastChangeTime, cardHwVersion=cardHwVersion, cardAction=cardAction, diskVolumeDiskSize=diskVolumeDiskSize, auditStatus=auditStatus, equipmentDiag=equipmentDiag, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDplxStatus=portDplxStatus, trapSeverity=trapSeverity, cardType=cardType, diskVolumeId=diskVolumeId, cardPrState=cardPrState, portCurrDsPower=portCurrDsPower, diskDriveNumber=diskDriveNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, auditId=auditId, equipmentAuditGroup=equipmentAuditGroup, portType=portType, cardTestResult=cardTestResult, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditAutoScheduling=auditAutoScheduling, cmPrimarySID=cmPrimarySID, equipmentCardGroup=equipmentCardGroup, trapCounter=trapCounter, cardTemperatureHighError=cardTemperatureHighError, cardLastResetReason=cardLastResetReason, portTxFlowControlMode=portTxFlowControlMode, cardShelfTestIndex=cardShelfTestIndex, diskVolumeTrapInh=diskVolumeTrapInh, TestId=TestId, auditDescription=auditDescription, auditFailedCount=auditFailedCount, cmUChannelID=cmUChannelID, errEvLogLevel=errEvLogLevel, TestType=TestType, cardTable=cardTable, cardCpuType=cardCpuType, cardCarrierSerialNum=cardCarrierSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeTable=diskVolumeTable, auditLogOutput=auditLogOutput, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentShelfGroup=equipmentShelfGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTemperature=cardTemperature, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestSchedule=cardTestSchedule, cadEquipmentMibConformance=cadEquipmentMibConformance, auditTime=auditTime, diskVolumeCardId=diskVolumeCardId, cardTestDescription=cardTestDescription, portTable=portTable, diagTestId=diagTestId, cardDetectedChange=cardDetectedChange, TestSchedule=TestSchedule, cardFwUpdateStatus=cardFwUpdateStatus, cardUpTime=cardUpTime, equipmentStateGroup=equipmentStateGroup, portMacAddress=portMacAddress, auditPassedCount=auditPassedCount, cmRegistrationNotification=cmRegistrationNotification, portNumber=portNumber, cardSecStateChange=cardSecStateChange, auditResult=auditResult, cardLastBootVersion=cardLastBootVersion, errEventId=errEventId, auditCommand=auditCommand, cardDplxStatusChange=cardDplxStatusChange, cardCarrierFwVersion=cardCarrierFwVersion, portMinDsPower=portMinDsPower, auditName=auditName, cmVendor=cmVendor, diskVolumeGroup=diskVolumeGroup, equipmentAudit=equipmentAudit, cardNumber=cardNumber, portLastChangeTime=portLastChangeTime, equipmentTbl=equipmentTbl, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCPLDVersion=cardCPLDVersion, cardTestResultDesc=cardTestResultDesc, cardTestCommand=cardTestCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSubType=cardSubType, cardAdminState=cardAdminState, fpgaErrorEventEntry=fpgaErrorEventEntry, portPrStateChange=portPrStateChange, TestScheduleCommand=TestScheduleCommand, portNumChans=portNumChans, auditShelfId=auditShelfId, diskVolumeDriveId=diskVolumeDriveId, auditCardId=auditCardId, equipmentTraps=equipmentTraps, portSecState=portSecState, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDetectedMode=portDetectedMode, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentDiagGroup=equipmentDiagGroup, cardSwVersion=cardSwVersion, portTxFlowControlDetected=portTxFlowControlDetected, auditCycleCount=auditCycleCount, cardDetected=cardDetected, verboseLevel=verboseLevel, diskVolumeSize=diskVolumeSize, portAdminState=portAdminState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portAction=portAction, cardPicSerialNum=cardPicSerialNum, remainInDiagMode=remainInDiagMode, shelfNumber=shelfNumber, cardSpareGroupId=cardSpareGroupId, cardTestIndex=cardTestIndex, cardFwVersion=cardFwVersion, portShelfId=portShelfId, cmResetStatus=cmResetStatus, portConnectorId=portConnectorId, cardPrStateChange=cardPrStateChange, portDescription=portDescription, portMaxDsPower=portMaxDsPower, auditEntry=auditEntry, TestTransactionId=TestTransactionId, cardId=cardId, cardShelfId=cardShelfId)
