...
DuplexStatus, PicType, PortDetectedMode, ShelfId, PortMode, FirmwareSource, PortId, PrimaryState, EqActionType, AdminState, UnknownState, CardId, FlowControlMode, CardSubType, SecondaryState, DiskVolumeUsageLevel, CardType, PortType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PicType", "PortDetectedMode", "ShelfId", "PortMode", "FirmwareSource", "PortId", "PrimaryState", "EqActionType", "AdminState", "UnknownState", "CardId", "FlowControlMode", "CardSubType", "SecondaryState", "DiskVolumeUsageLevel", "CardType", "PortType")
...
DateAndTime, TimeStamp, TextualConvention, TruthValue, RowStatus, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TimeStamp", "TextualConvention", "TruthValue", "RowStatus", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCpuType=cardCpuType, TestTransactionId=TestTransactionId, trapSeverity=trapSeverity, cardSpareGroupId=cardSpareGroupId, cardPicHwVersion=cardPicHwVersion, cardCPLDVersion=cardCPLDVersion, cardUpTime=cardUpTime, trapCounter=trapCounter, shelfNumber=shelfNumber, cardTemperatureHighError=cardTemperatureHighError, equipmentCardGroup=equipmentCardGroup, cardPicDetected=cardPicDetected, cardPrStateChange=cardPrStateChange, equipmentState=equipmentState, cardPicSerialNum=cardPicSerialNum, cardTemperature=cardTemperature, systemKey=systemKey, diskVolumeNumber=diskVolumeNumber, cardTestId=cardTestId, equipmentPortGroup=equipmentPortGroup, portAction=portAction, cardAdminState=cardAdminState, systemGeneralGroup=systemGeneralGroup, portDescription=portDescription, portTrapInh=portTrapInh, cardTestName=cardTestName, portSecStateChange=portSecStateChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardFpgaSource=cardFpgaSource, cardBootVersion=cardBootVersion, auditPassedCount=auditPassedCount, cardDplxStatusChange=cardDplxStatusChange, portAdminState=portAdminState, auditShelfId=auditShelfId, diskVolumeGroup=diskVolumeGroup, equipmentTraps=equipmentTraps, cardTestResultNotification=cardTestResultNotification, portMinDsPower=portMinDsPower, equipmentStateGroup=equipmentStateGroup, cardSecState=cardSecState, cardTempOverHeatNotification=cardTempOverHeatNotification, systemClock=systemClock, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSecStateChange=cardSecStateChange, cmResetUpTime=cmResetUpTime, errEvRecoveryEnabled=errEvRecoveryEnabled, cmDevice=cmDevice, cardTestTable=cardTestTable, cardSubDetected=cardSubDetected, cardCarrierHwVersion=cardCarrierHwVersion, cardTestIndex=cardTestIndex, cmResetNotification=cmResetNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestScheduleCommand=cardTestScheduleCommand, cardNumPorts=cardNumPorts, portMaxDsPower=portMaxDsPower, cardDplxStatus=cardDplxStatus, portRxFlowControlMode=portRxFlowControlMode, eqDiagConfig=eqDiagConfig, auditCardId=auditCardId, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentTbl=equipmentTbl, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCarrierSerialNum=cardCarrierSerialNum, cardName=cardName, cardSubType=cardSubType, cmResetInfo=cmResetInfo, equipmentDiag=equipmentDiag, cardLastResetReason=cardLastResetReason, remainInDiagMode=remainInDiagMode, portLastChangeTime=portLastChangeTime, auditEntry=auditEntry, errEvLoggingEnabled=errEvLoggingEnabled, cardTestType=cardTestType, cardRamSize=cardRamSize, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPrState=cardPrState, portDocsIfIndex=portDocsIfIndex, auditName=auditName, portTxFlowControlMode=portTxFlowControlMode, cardNumber=cardNumber, cardTestEntry=cardTestEntry, diskDriveNumber=diskDriveNumber, diskVolumeTrapInh=diskVolumeTrapInh, equipmentCardTestGroup=equipmentCardTestGroup, equipmentShelfGroup=equipmentShelfGroup, auditLogThrottle=auditLogThrottle, auditAutoScheduling=auditAutoScheduling, errEvRowStatus=errEvRowStatus, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeDiskSize=diskVolumeDiskSize, auditResult=auditResult, cardShelfId=cardShelfId, cardSwVersion=cardSwVersion, errEvLogLevel=errEvLogLevel, portMacIfIndex=portMacIfIndex, portDplxStatusChange=portDplxStatusChange, auditTable=auditTable, cmIpAddress=cmIpAddress, auditId=auditId, portTxFlowControlDetected=portTxFlowControlDetected, portBgpId=portBgpId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestResultDesc=cardTestResultDesc, shelfName=shelfName, shelfSwVersion=shelfSwVersion, portConnectorId=portConnectorId, cmMacAddress=cmMacAddress, diskVolumeId=diskVolumeId, cardFwVersion=cardFwVersion, cardTestTransId=cardTestTransId, portShelfId=portShelfId, TestId=TestId, cardTable=cardTable, auditLogOutput=auditLogOutput, cardCpuSpeed=cardCpuSpeed, auditCommand=auditCommand, TestSchedule=TestSchedule, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeFileName=diskVolumeFileName, portCardSubType=portCardSubType, cardFwUpdateStatus=cardFwUpdateStatus, portCardId=portCardId, diskVolumeShelfId=diskVolumeShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTempNoReportNotification=cardTempNoReportNotification, cardTrapInh=cardTrapInh, cmUChannelID=cmUChannelID, cmResetReason=cmResetReason, TestResult=TestResult, equipmentDiagGroup=equipmentDiagGroup, downstreamPowerLoss=downstreamPowerLoss, verboseLevel=verboseLevel, portEntry=portEntry, auditTotalFailedCount=auditTotalFailedCount, portId=portId, cardSerialNum=cardSerialNum, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditFailedCount=auditFailedCount, portPrState=portPrState, cardBusSpeed=cardBusSpeed, diagTestId=diagTestId, auditTime=auditTime, diskVolumeEntry=diskVolumeEntry, cmRegistrationNotification=cmRegistrationNotification, cardHwVersion=cardHwVersion, portNumber=portNumber, cmResetClearNotification=cmResetClearNotification, TestCommand=TestCommand, PYSNMP_MODULE_ID=cadEquipmentMib, portCurrDsPower=portCurrDsPower, cardLastBootVersion=cardLastBootVersion, cardDetected=cardDetected, portMacAddress=portMacAddress, portMode=portMode, diskVolumeTable=diskVolumeTable, cardLastBootSource=cardLastBootSource, auditCycleCount=auditCycleCount, portDetectedMode=portDetectedMode, cardDetectedChange=cardDetectedChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestSchedule=cardTestSchedule, equipmentAuditGroup=equipmentAuditGroup, cmResetStatus=cmResetStatus, cardType=cardType, cmVendor=cmVendor, cardAction=cardAction, TestType=TestType, diskVolumeDriveId=diskVolumeDriveId, equipmentError=equipmentError, cardSpareGroupMode=cardSpareGroupMode, diskVolumeName=diskVolumeName, portRxFlowControlDetected=portRxFlowControlDetected, equipmentAudit=equipmentAudit, cadEquipmentMibConformance=cadEquipmentMibConformance, portNumChans=portNumChans, cardId=cardId, cardShelfTestIndex=cardShelfTestIndex, auditStatus=auditStatus, auditTotalPassedCount=auditTotalPassedCount, portType=portType, cardEntry=cardEntry, auditDescription=auditDescription, portDplxStatus=portDplxStatus, cardTestDescription=cardTestDescription, cardCarrierFwVersion=cardCarrierFwVersion, cardTestCommand=cardTestCommand, cmPrimarySID=cmPrimarySID, errEventId=errEventId, consoleOutput=consoleOutput, diskVolumeCardId=diskVolumeCardId, portSecState=portSecState, systemGeneral=systemGeneral, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTime=cardTestTime, diskVolumeSize=diskVolumeSize, cardTestResult=cardTestResult, fpgaErrorEventTable=fpgaErrorEventTable, portTable=portTable, cadEquipmentMib=cadEquipmentMib, shelfObjects=shelfObjects, portPrStateChange=portPrStateChange, cardFlashSize=cardFlashSize, TestScheduleCommand=TestScheduleCommand)
