...
CardId, DiskVolumeUsageLevel, FirmwareSource, PortId, PortType, SecondaryState, AdminState, ShelfId, PrimaryState, CardType, PortDetectedMode, DuplexStatus, EqActionType, FlowControlMode, UnknownState, PicType, CardSubType, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardId", "DiskVolumeUsageLevel", "FirmwareSource", "PortId", "PortType", "SecondaryState", "AdminState", "ShelfId", "PrimaryState", "CardType", "PortDetectedMode", "DuplexStatus", "EqActionType", "FlowControlMode", "UnknownState", "PicType", "CardSubType", "PortMode")
...
RowStatus, TextualConvention, TruthValue, TimeStamp, DisplayString, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "TruthValue", "TimeStamp", "DisplayString", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmResetUpTime=cmResetUpTime, systemGeneral=systemGeneral, auditLogOutput=auditLogOutput, cmResetInfo=cmResetInfo, cardCPLDVersion=cardCPLDVersion, auditFailedCount=auditFailedCount, auditTable=auditTable, diskDriveNumber=diskDriveNumber, diskVolumeEntry=diskVolumeEntry, TestScheduleCommand=TestScheduleCommand, cmResetReason=cmResetReason, errEvLogLevel=errEvLogLevel, TestResult=TestResult, cardPicSerialNum=cardPicSerialNum, cardPrStateChange=cardPrStateChange, cardType=cardType, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfNumber=shelfNumber, cmMacAddress=cmMacAddress, cardShelfId=cardShelfId, portConnectorId=portConnectorId, diskVolumeDriveId=diskVolumeDriveId, fpgaErrorEventEntry=fpgaErrorEventEntry, shelfName=shelfName, diskVolumeDiskSize=diskVolumeDiskSize, cardPicDetected=cardPicDetected, cmVendor=cmVendor, errEvRowStatus=errEvRowStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, trapSeverity=trapSeverity, systemGeneralGroup=systemGeneralGroup, cardCpuSpeed=cardCpuSpeed, cardLastResetReason=cardLastResetReason, portDescription=portDescription, cardSpareGroupMode=cardSpareGroupMode, portCardSubType=portCardSubType, portNumChans=portNumChans, cardTemperatureHighError=cardTemperatureHighError, cmRegistrationNotification=cmRegistrationNotification, diskVolumeName=diskVolumeName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardNumber=cardNumber, portDetectedMode=portDetectedMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditName=auditName, cardBusSpeed=cardBusSpeed, cardTemperatureHighWarn=cardTemperatureHighWarn, cardFlashSize=cardFlashSize, diskVolumeId=diskVolumeId, cardTrapInh=cardTrapInh, cardLastBootVersion=cardLastBootVersion, PYSNMP_MODULE_ID=cadEquipmentMib, auditStatus=auditStatus, equipmentStateGroup=equipmentStateGroup, portEntry=portEntry, diskVolumeNumber=diskVolumeNumber, diskVolumeTable=diskVolumeTable, auditTotalFailedCount=auditTotalFailedCount, portMacIfIndex=portMacIfIndex, errEventId=errEventId, auditEntry=auditEntry, portTrapInh=portTrapInh, systemKey=systemKey, portCurrDsPower=portCurrDsPower, auditDescription=auditDescription, cardTemperature=cardTemperature, TestTransactionId=TestTransactionId, cardSpareGroupId=cardSpareGroupId, verboseLevel=verboseLevel, cardTestIndex=cardTestIndex, cardTable=cardTable, portRxFlowControlMode=portRxFlowControlMode, portLastChangeTime=portLastChangeTime, downstreamPowerLoss=downstreamPowerLoss, cardSubDetected=cardSubDetected, cardTestId=cardTestId, portDplxStatusChange=portDplxStatusChange, equipmentTraps=equipmentTraps, cardTestResultDesc=cardTestResultDesc, TestSchedule=TestSchedule, eqDiagConfig=eqDiagConfig, TestId=TestId, auditAutoScheduling=auditAutoScheduling, shelfObjects=shelfObjects, equipmentCardTestGroup=equipmentCardTestGroup, cardFpgaSource=cardFpgaSource, consoleOutput=consoleOutput, cmResetNotification=cmResetNotification, portAction=portAction, auditShelfId=auditShelfId, equipmentCardGroup=equipmentCardGroup, cardSwVersion=cardSwVersion, portNumber=portNumber, cardName=cardName, portTxFlowControlDetected=portTxFlowControlDetected, cardFwVersion=cardFwVersion, portMinDsPower=portMinDsPower, equipmentAuditGroup=equipmentAuditGroup, cardNumPorts=cardNumPorts, cardCpuType=cardCpuType, cardCarrierSerialNum=cardCarrierSerialNum, portTxFlowControlMode=portTxFlowControlMode, cmUChannelID=cmUChannelID, cadEquipmentMib=cadEquipmentMib, cardRamSize=cardRamSize, diskVolumeCardId=diskVolumeCardId, cardTestResult=cardTestResult, auditCardId=auditCardId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentAudit=equipmentAudit, diskVolumeSize=diskVolumeSize, portShelfId=portShelfId, cardTempNoReportNotification=cardTempNoReportNotification, portMode=portMode, auditId=auditId, portType=portType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeGroup=diskVolumeGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardId=cardId, cardSecState=cardSecState, trapCounter=trapCounter, cardDetected=cardDetected, cardTestDescription=cardTestDescription, shelfSwVersion=shelfSwVersion, cardAction=cardAction, cardTestName=cardTestName, portAdminState=portAdminState, cardCarrierHwVersion=cardCarrierHwVersion, cardShelfTestIndex=cardShelfTestIndex, cardHwVersion=cardHwVersion, cardTestEntry=cardTestEntry, auditCommand=auditCommand, cmPrimarySID=cmPrimarySID, auditResult=auditResult, equipmentDiag=equipmentDiag, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeShelfId=diskVolumeShelfId, cardDplxStatus=cardDplxStatus, equipmentTbl=equipmentTbl, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentDiagGroup=equipmentDiagGroup, systemClock=systemClock, equipmentState=equipmentState, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSecStateChange=cardSecStateChange, portSecState=portSecState, portPrStateChange=portPrStateChange, cmResetClearNotification=cmResetClearNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portSecStateChange=portSecStateChange, portDocsIfIndex=portDocsIfIndex, cardTestResultNotification=cardTestResultNotification, diagTestId=diagTestId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditTotalPassedCount=auditTotalPassedCount, auditCycleCount=auditCycleCount, cardTestTable=cardTestTable, remainInDiagMode=remainInDiagMode, cmIpAddress=cmIpAddress, cardDplxStatusChange=cardDplxStatusChange, cardAdminState=cardAdminState, diskVolumeUsageLevel=diskVolumeUsageLevel, portBgpId=portBgpId, cardLastChangeTime=cardLastChangeTime, portCardId=portCardId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestType=TestType, auditPassedCount=auditPassedCount, auditLogThrottle=auditLogThrottle, portId=portId, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeFileName=diskVolumeFileName, cardTestType=cardTestType, cardTestSchedule=cardTestSchedule, equipmentError=equipmentError, portRxFlowControlDetected=portRxFlowControlDetected, cardPrState=cardPrState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardUpTime=cardUpTime, cardLastBootSource=cardLastBootSource, portTable=portTable, portMacAddress=portMacAddress, cardEntry=cardEntry, cardTestScheduleCommand=cardTestScheduleCommand, cmDevice=cmDevice, errEvLoggingEnabled=errEvLoggingEnabled, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentPortGroup=equipmentPortGroup, portPrState=portPrState, cardTestTime=cardTestTime, auditTime=auditTime, cardTestTransId=cardTestTransId, cardPicHwVersion=cardPicHwVersion, TestCommand=TestCommand, cardTestCommand=cardTestCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardBootVersion=cardBootVersion, cmResetStatus=cmResetStatus, cardDetectedChange=cardDetectedChange, equipmentShelfGroup=equipmentShelfGroup, cardFwUpdateStatus=cardFwUpdateStatus, portDplxStatus=portDplxStatus, cardCarrierFwVersion=cardCarrierFwVersion, portMaxDsPower=portMaxDsPower, cardSerialNum=cardSerialNum, cardSubType=cardSubType)
