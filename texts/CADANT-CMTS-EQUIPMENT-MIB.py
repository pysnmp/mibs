...
UnknownState, CardSubType, CardId, CardType, DuplexStatus, PrimaryState, DiskVolumeUsageLevel, PortMode, SecondaryState, PortType, PicType, PortDetectedMode, AdminState, PortId, FirmwareSource, ShelfId, EqActionType, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "CardSubType", "CardId", "CardType", "DuplexStatus", "PrimaryState", "DiskVolumeUsageLevel", "PortMode", "SecondaryState", "PortType", "PicType", "PortDetectedMode", "AdminState", "PortId", "FirmwareSource", "ShelfId", "EqActionType", "FlowControlMode")
...
DisplayString, MacAddress, RowStatus, DateAndTime, TextualConvention, TimeStamp, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "RowStatus", "DateAndTime", "TextualConvention", "TimeStamp", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditName=auditName, TestId=TestId, cardUpTime=cardUpTime, fpgaErrorEventTable=fpgaErrorEventTable, cmVendor=cmVendor, portSecStateChange=portSecStateChange, auditLogThrottle=auditLogThrottle, diskVolumeGroup=diskVolumeGroup, shelfObjects=shelfObjects, diskVolumeDriveId=diskVolumeDriveId, cardTestResult=cardTestResult, auditTotalPassedCount=auditTotalPassedCount, auditTotalFailedCount=auditTotalFailedCount, portDocsIfIndex=portDocsIfIndex, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentTraps=equipmentTraps, systemKey=systemKey, shelfNumber=shelfNumber, portMacAddress=portMacAddress, equipmentAuditGroup=equipmentAuditGroup, portAction=portAction, portCurrDsPower=portCurrDsPower, TestScheduleCommand=TestScheduleCommand, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditShelfId=auditShelfId, cardType=cardType, portCardId=portCardId, portPrStateChange=portPrStateChange, cadEquipmentMib=cadEquipmentMib, cardTestTime=cardTestTime, shelfSwVersion=shelfSwVersion, equipmentShelfGroup=equipmentShelfGroup, portNumChans=portNumChans, cardTestName=cardTestName, cardDplxStatusChange=cardDplxStatusChange, cardFpgaSource=cardFpgaSource, TestTransactionId=TestTransactionId, auditCycleCount=auditCycleCount, cardSecStateChange=cardSecStateChange, cardCarrierFwVersion=cardCarrierFwVersion, cardAdminState=cardAdminState, cardLastBootVersion=cardLastBootVersion, cardTestIndex=cardTestIndex, errEvRowStatus=errEvRowStatus, cardTestResultDesc=cardTestResultDesc, errEvLogLevel=errEvLogLevel, cardName=cardName, cmUChannelID=cmUChannelID, diskVolumeFileName=diskVolumeFileName, auditTime=auditTime, cardSubType=cardSubType, portBgpId=portBgpId, portDplxStatus=portDplxStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastChangeTime=cardLastChangeTime, trapCounter=trapCounter, cardShelfId=cardShelfId, portEntry=portEntry, equipmentStateGroup=equipmentStateGroup, cmMacAddress=cmMacAddress, verboseLevel=verboseLevel, diskVolumeTable=diskVolumeTable, cardTestDescription=cardTestDescription, portMinDsPower=portMinDsPower, cardSpareGroupMode=cardSpareGroupMode, auditAutoScheduling=auditAutoScheduling, diskVolumeNumber=diskVolumeNumber, equipmentState=equipmentState, cmPrimarySID=cmPrimarySID, consoleOutput=consoleOutput, cardBusSpeed=cardBusSpeed, portMacIfIndex=portMacIfIndex, portTrapInh=portTrapInh, shelfName=shelfName, diskDriveNumber=diskDriveNumber, portCardSubType=portCardSubType, cardTable=cardTable, cardEntry=cardEntry, cardTemperature=cardTemperature, cmResetReason=cmResetReason, diskVolumeEntry=diskVolumeEntry, cardPrState=cardPrState, auditCardId=auditCardId, auditLogOutput=auditLogOutput, eqDiagConfig=eqDiagConfig, cardHwVersion=cardHwVersion, auditEntry=auditEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentDiagGroup=equipmentDiagGroup, cardLastResetReason=cardLastResetReason, cardCpuType=cardCpuType, cardSecState=cardSecState, portType=portType, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestScheduleCommand=cardTestScheduleCommand, cadEquipmentMibConformance=cadEquipmentMibConformance, systemClock=systemClock, cardCpuSpeed=cardCpuSpeed, errEventId=errEventId, cardNumPorts=cardNumPorts, portRxFlowControlMode=portRxFlowControlMode, cardSpareGroupId=cardSpareGroupId, cardId=cardId, cardCarrierSerialNum=cardCarrierSerialNum, trapSeverity=trapSeverity, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentCardGroup=equipmentCardGroup, cardFlashSize=cardFlashSize, cardPicHwVersion=cardPicHwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestCommand=cardTestCommand, auditDescription=auditDescription, equipmentDiag=equipmentDiag, portDescription=portDescription, portTxFlowControlDetected=portTxFlowControlDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, cmIpAddress=cmIpAddress, equipmentTbl=equipmentTbl, portPrState=portPrState, diskVolumeCardId=diskVolumeCardId, cardCarrierHwVersion=cardCarrierHwVersion, auditResult=auditResult, portAdminState=portAdminState, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestId=cardTestId, cmResetInfo=cmResetInfo, cmDevice=cmDevice, cardDplxStatus=cardDplxStatus, cardTestResultNotification=cardTestResultNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeShelfId=diskVolumeShelfId, cardRamSize=cardRamSize, TestResult=TestResult, cardDetected=cardDetected, cardAction=cardAction, cardTempOverHeatNotification=cardTempOverHeatNotification, downstreamPowerLoss=downstreamPowerLoss, cardCPLDVersion=cardCPLDVersion, cardSubDetected=cardSubDetected, systemGeneral=systemGeneral, cardSerialNum=cardSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, cardTestSchedule=cardTestSchedule, cardDetectedChange=cardDetectedChange, cardTestType=cardTestType, cmResetUpTime=cmResetUpTime, auditTable=auditTable, portNumber=portNumber, portDetectedMode=portDetectedMode, cardFwUpdateStatus=cardFwUpdateStatus, cardBootVersion=cardBootVersion, portMaxDsPower=portMaxDsPower, auditFailedCount=auditFailedCount, diskVolumeTrapInh=diskVolumeTrapInh, diagTestId=diagTestId, cardNumber=cardNumber, auditCommand=auditCommand, portTable=portTable, remainInDiagMode=remainInDiagMode, auditId=auditId, diskVolumeName=diskVolumeName, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmRegistrationNotification=cmRegistrationNotification, cardFwVersion=cardFwVersion, cardShelfTestIndex=cardShelfTestIndex, portShelfId=portShelfId, cardTrapInh=cardTrapInh, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, portId=portId, TestCommand=TestCommand, equipmentError=equipmentError, cmResetNotification=cmResetNotification, cardSwVersion=cardSwVersion, cardPicSerialNum=cardPicSerialNum, TestType=TestType, portDplxStatusChange=portDplxStatusChange, TestSchedule=TestSchedule, cardPicDetected=cardPicDetected, portMode=portMode, auditPassedCount=auditPassedCount, equipmentPortGroup=equipmentPortGroup, cardTestTransId=cardTestTransId, cardTemperatureHighError=cardTemperatureHighError, cmResetStatus=cmResetStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, fpgaErrorEventGroup=fpgaErrorEventGroup, cardLastBootSource=cardLastBootSource, cardTempNoReportNotification=cardTempNoReportNotification, cmResetClearNotification=cmResetClearNotification, auditStatus=auditStatus, diskVolumeId=diskVolumeId, cardPrStateChange=cardPrStateChange, diskVolumeSize=diskVolumeSize, equipmentAudit=equipmentAudit, portSecState=portSecState, portConnectorId=portConnectorId, portLastChangeTime=portLastChangeTime, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestTable=cardTestTable, systemGeneralGroup=systemGeneralGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestEntry=cardTestEntry)
