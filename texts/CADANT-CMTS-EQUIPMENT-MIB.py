...
DuplexStatus, CardId, FlowControlMode, PortDetectedMode, SecondaryState, ShelfId, EqActionType, PicType, DiskVolumeUsageLevel, FirmwareSource, PrimaryState, PortType, CardType, CardSubType, AdminState, UnknownState, PortMode, PortId = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "CardId", "FlowControlMode", "PortDetectedMode", "SecondaryState", "ShelfId", "EqActionType", "PicType", "DiskVolumeUsageLevel", "FirmwareSource", "PrimaryState", "PortType", "CardType", "CardSubType", "AdminState", "UnknownState", "PortMode", "PortId")
...
DateAndTime, RowStatus, DisplayString, TruthValue, TimeStamp, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "RowStatus", "DisplayString", "TruthValue", "TimeStamp", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, shelfSwVersion=shelfSwVersion, diskVolumeName=diskVolumeName, cardTestIndex=cardTestIndex, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmRegistrationNotification=cmRegistrationNotification, cardTestName=cardTestName, equipmentAudit=equipmentAudit, equipmentError=equipmentError, cardTestEntry=cardTestEntry, TestCommand=TestCommand, cardTestCommand=cardTestCommand, systemClock=systemClock, cmResetInfo=cmResetInfo, cmPrimarySID=cmPrimarySID, portLastChangeTime=portLastChangeTime, cardCpuType=cardCpuType, auditShelfId=auditShelfId, cardDetectedChange=cardDetectedChange, portDetectedMode=portDetectedMode, errEvRecoveryEnabled=errEvRecoveryEnabled, auditResult=auditResult, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeTrapInh=diskVolumeTrapInh, equipmentCardGroup=equipmentCardGroup, cardNumber=cardNumber, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCPLDVersion=cardCPLDVersion, auditCommand=auditCommand, portSecStateChange=portSecStateChange, cardEntry=cardEntry, cardSubType=cardSubType, verboseLevel=verboseLevel, TestScheduleCommand=TestScheduleCommand, cmResetUpTime=cmResetUpTime, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portEntry=portEntry, auditStatus=auditStatus, errEvLoggingEnabled=errEvLoggingEnabled, cardSecStateChange=cardSecStateChange, auditTable=auditTable, diskVolumeFileName=diskVolumeFileName, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditDescription=auditDescription, cmResetStatus=cmResetStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, portCardId=portCardId, cardTestType=cardTestType, cardSerialNum=cardSerialNum, cardTrapInh=cardTrapInh, portAdminState=portAdminState, cardPicSerialNum=cardPicSerialNum, shelfName=shelfName, cardTestResultDesc=cardTestResultDesc, cmResetClearNotification=cmResetClearNotification, cardTestResultNotification=cardTestResultNotification, cardTestTable=cardTestTable, portMacIfIndex=portMacIfIndex, cardFlashSize=cardFlashSize, equipmentCardTestGroup=equipmentCardTestGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardLastBootVersion=cardLastBootVersion, cmDevice=cmDevice, equipmentTraps=equipmentTraps, cardFwVersion=cardFwVersion, shelfObjects=shelfObjects, cardTemperature=cardTemperature, diagTestId=diagTestId, diskVolumeNumber=diskVolumeNumber, cardPicDetected=cardPicDetected, cardAdminState=cardAdminState, TestResult=TestResult, auditLogThrottle=auditLogThrottle, auditEntry=auditEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, auditCycleCount=auditCycleCount, cardPrStateChange=cardPrStateChange, portId=portId, TestType=TestType, cardCarrierHwVersion=cardCarrierHwVersion, portTrapInh=portTrapInh, equipmentState=equipmentState, cardCarrierFwVersion=cardCarrierFwVersion, cardTestResult=cardTestResult, cmVendor=cmVendor, cardTable=cardTable, cardTestTransId=cardTestTransId, cardUpTime=cardUpTime, cadEquipmentMib=cadEquipmentMib, cardSubDetected=cardSubDetected, fpgaErrorEventTable=fpgaErrorEventTable, errEventId=errEventId, equipmentPortGroup=equipmentPortGroup, portDplxStatusChange=portDplxStatusChange, cardPrState=cardPrState, errEvRowStatus=errEvRowStatus, cardTemperatureHighError=cardTemperatureHighError, trapCounter=trapCounter, systemGeneral=systemGeneral, shelfNumber=shelfNumber, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portMaxDsPower=portMaxDsPower, portDescription=portDescription, cardTestId=cardTestId, portPrState=portPrState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditFailedCount=auditFailedCount, cardId=cardId, TestId=TestId, equipmentTbl=equipmentTbl, cardRamSize=cardRamSize, auditName=auditName, portConnectorId=portConnectorId, diskVolumeSize=diskVolumeSize, portType=portType, cardCpuSpeed=cardCpuSpeed, cardDetected=cardDetected, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestScheduleCommand=cardTestScheduleCommand, auditId=auditId, cardTestTime=cardTestTime, auditTotalPassedCount=auditTotalPassedCount, cmResetReason=cmResetReason, diskVolumeDriveId=diskVolumeDriveId, portDplxStatus=portDplxStatus, auditCardId=auditCardId, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentStateGroup=equipmentStateGroup, cardBusSpeed=cardBusSpeed, auditTime=auditTime, cardLastBootSource=cardLastBootSource, portMacAddress=portMacAddress, cardNumPorts=cardNumPorts, portAction=portAction, portCurrDsPower=portCurrDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, systemGeneralGroup=systemGeneralGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeDiskSize=diskVolumeDiskSize, portDocsIfIndex=portDocsIfIndex, cmUChannelID=cmUChannelID, portNumber=portNumber, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestSchedule=cardTestSchedule, diskDriveNumber=diskDriveNumber, cardAction=cardAction, portNumChans=portNumChans, portRxFlowControlDetected=portRxFlowControlDetected, auditPassedCount=auditPassedCount, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portCardSubType=portCardSubType, consoleOutput=consoleOutput, cardBootVersion=cardBootVersion, equipmentAuditGroup=equipmentAuditGroup, portShelfId=portShelfId, cardLastResetReason=cardLastResetReason, cardCarrierSerialNum=cardCarrierSerialNum, portBgpId=portBgpId, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTable=diskVolumeTable, systemKey=systemKey, portMode=portMode, remainInDiagMode=remainInDiagMode, cardHwVersion=cardHwVersion, cmResetNotification=cmResetNotification, cardTempNoReportNotification=cardTempNoReportNotification, cardSwVersion=cardSwVersion, cardFpgaSource=cardFpgaSource, cardDplxStatusChange=cardDplxStatusChange, cardLastChangeTime=cardLastChangeTime, auditTotalFailedCount=auditTotalFailedCount, cardName=cardName, diskVolumeShelfId=diskVolumeShelfId, diskVolumeEntry=diskVolumeEntry, cardType=cardType, portMinDsPower=portMinDsPower, eqDiagConfig=eqDiagConfig, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardDplxStatus=cardDplxStatus, TestSchedule=TestSchedule, cmIpAddress=cmIpAddress, cardShelfId=cardShelfId, cardSpareGroupId=cardSpareGroupId, cardSecState=cardSecState, auditAutoScheduling=auditAutoScheduling, auditLogOutput=auditLogOutput, errEvLogLevel=errEvLogLevel, portPrStateChange=portPrStateChange, cardPicHwVersion=cardPicHwVersion, downstreamPowerLoss=downstreamPowerLoss, portRxFlowControlMode=portRxFlowControlMode, fpgaErrorEventGroup=fpgaErrorEventGroup, cmMacAddress=cmMacAddress, TestTransactionId=TestTransactionId, trapSeverity=trapSeverity, diskVolumeCardId=diskVolumeCardId, cardSpareGroupMode=cardSpareGroupMode, portSecState=portSecState, cardShelfTestIndex=cardShelfTestIndex, cardTestDescription=cardTestDescription, equipmentShelfGroup=equipmentShelfGroup, equipmentDiagGroup=equipmentDiagGroup, diskVolumeId=diskVolumeId, portTable=portTable, equipmentDiag=equipmentDiag)
