...
PicType, CardId, PortId, PortDetectedMode, DuplexStatus, PortType, CardType, FirmwareSource, PrimaryState, ShelfId, FlowControlMode, SecondaryState, CardSubType, DiskVolumeUsageLevel, PortMode, EqActionType, AdminState, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PicType", "CardId", "PortId", "PortDetectedMode", "DuplexStatus", "PortType", "CardType", "FirmwareSource", "PrimaryState", "ShelfId", "FlowControlMode", "SecondaryState", "CardSubType", "DiskVolumeUsageLevel", "PortMode", "EqActionType", "AdminState", "UnknownState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastBootSource=cardLastBootSource, diskVolumeNumber=diskVolumeNumber, cmResetClearNotification=cmResetClearNotification, TestCommand=TestCommand, cmMacAddress=cmMacAddress, diskVolumeGroup=diskVolumeGroup, trapCounter=trapCounter, cardTestId=cardTestId, cardHwVersion=cardHwVersion, cardCarrierSerialNum=cardCarrierSerialNum, cmResetReason=cmResetReason, auditDescription=auditDescription, portConnectorId=portConnectorId, cardTestTransId=cardTestTransId, cmIpAddress=cmIpAddress, fpgaErrorEventGroup=fpgaErrorEventGroup, auditCommand=auditCommand, auditTotalPassedCount=auditTotalPassedCount, cardPicSerialNum=cardPicSerialNum, cardShelfId=cardShelfId, cmResetUpTime=cmResetUpTime, cmResetInfo=cmResetInfo, cardName=cardName, cardTestResult=cardTestResult, diskVolumeFileName=diskVolumeFileName, equipmentCardGroup=equipmentCardGroup, diskVolumeName=diskVolumeName, cadEquipmentMibCompliance=cadEquipmentMibCompliance, consoleOutput=consoleOutput, cardFwVersion=cardFwVersion, portEntry=portEntry, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestDescription=cardTestDescription, cardAction=cardAction, cardSubType=cardSubType, cardTestType=cardTestType, cardTestTable=cardTestTable, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardBusSpeed=cardBusSpeed, cardPicHwVersion=cardPicHwVersion, cardTestCommand=cardTestCommand, errEvLogLevel=errEvLogLevel, cardFlashSize=cardFlashSize, cardTestResultDesc=cardTestResultDesc, cardTempOverHeatNotification=cardTempOverHeatNotification, portMinDsPower=portMinDsPower, equipmentTraps=equipmentTraps, equipmentDiagGroup=equipmentDiagGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, remainInDiagMode=remainInDiagMode, TestTransactionId=TestTransactionId, auditName=auditName, portTrapInh=portTrapInh, equipmentShelfGroup=equipmentShelfGroup, TestSchedule=TestSchedule, cardTemperatureHighWarn=cardTemperatureHighWarn, portId=portId, auditId=auditId, cardTrapInh=cardTrapInh, cardCpuType=cardCpuType, diagTestId=diagTestId, diskVolumeDriveId=diskVolumeDriveId, fpgaErrorEventTable=fpgaErrorEventTable, shelfSwVersion=shelfSwVersion, cardSubDetected=cardSubDetected, cardPrState=cardPrState, cardTemperatureHighError=cardTemperatureHighError, equipmentError=equipmentError, PYSNMP_MODULE_ID=cadEquipmentMib, diskDriveNumber=diskDriveNumber, cardSecState=cardSecState, portTable=portTable, auditFailedCount=auditFailedCount, diskVolumeSize=diskVolumeSize, auditLogOutput=auditLogOutput, cardTestIndex=cardTestIndex, cardTestTime=cardTestTime, systemGeneral=systemGeneral, portCurrDsPower=portCurrDsPower, auditAutoScheduling=auditAutoScheduling, systemClock=systemClock, diskVolumeTable=diskVolumeTable, portDetectedMode=portDetectedMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditResult=auditResult, diskVolumeCardId=diskVolumeCardId, cardTestSchedule=cardTestSchedule, portSecStateChange=portSecStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditShelfId=auditShelfId, TestScheduleCommand=TestScheduleCommand, errEventId=errEventId, portType=portType, shelfName=shelfName, diskVolumeTrapInh=diskVolumeTrapInh, cardLastChangeTime=cardLastChangeTime, cardFpgaSource=cardFpgaSource, portNumChans=portNumChans, portLastChangeTime=portLastChangeTime, cardDetected=cardDetected, cardAdminState=cardAdminState, equipmentAuditGroup=equipmentAuditGroup, diskVolumeDiskSize=diskVolumeDiskSize, cardRamSize=cardRamSize, portMacIfIndex=portMacIfIndex, portTxFlowControlMode=portTxFlowControlMode, shelfNumber=shelfNumber, cardSwVersion=cardSwVersion, portPrState=portPrState, portRxFlowControlMode=portRxFlowControlMode, cardBootVersion=cardBootVersion, systemKey=systemKey, cardPicDetected=cardPicDetected, cardShelfTestIndex=cardShelfTestIndex, downstreamPowerLoss=downstreamPowerLoss, portDescription=portDescription, cardDplxStatusChange=cardDplxStatusChange, cardTestName=cardTestName, equipmentDiag=equipmentDiag, equipmentCardTestGroup=equipmentCardTestGroup, auditLogThrottle=auditLogThrottle, systemGeneralGroup=systemGeneralGroup, portPrStateChange=portPrStateChange, portShelfId=portShelfId, cardCpuSpeed=cardCpuSpeed, portAction=portAction, cardDetectedChange=cardDetectedChange, portDocsIfIndex=portDocsIfIndex, cardEntry=cardEntry, portMode=portMode, equipmentState=equipmentState, cardUpTime=cardUpTime, portMaxDsPower=portMaxDsPower, eqDiagConfig=eqDiagConfig, auditCycleCount=auditCycleCount, cardLastBootVersion=cardLastBootVersion, TestResult=TestResult, fpgaErrorEventEntry=fpgaErrorEventEntry, errEvRowStatus=errEvRowStatus, cardTempNoReportNotification=cardTempNoReportNotification, cardNumber=cardNumber, portCardId=portCardId, cadEquipmentMib=cadEquipmentMib, portCardSubType=portCardSubType, portSecState=portSecState, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, portNumber=portNumber, TestId=TestId, auditEntry=auditEntry, shelfObjects=shelfObjects, auditTotalFailedCount=auditTotalFailedCount, diskVolumeId=diskVolumeId, cmUChannelID=cmUChannelID, cardTestResultNotification=cardTestResultNotification, auditStatus=auditStatus, cmResetNotification=cmResetNotification, cmRegistrationNotification=cmRegistrationNotification, portDplxStatus=portDplxStatus, cardDplxStatus=cardDplxStatus, cardTable=cardTable, cardFwUpdateStatus=cardFwUpdateStatus, portMacAddress=portMacAddress, cardType=cardType, cardTestEntry=cardTestEntry, portAdminState=portAdminState, auditPassedCount=auditPassedCount, cmDevice=cmDevice, cardSerialNum=cardSerialNum, auditTime=auditTime, TestType=TestType, cmPrimarySID=cmPrimarySID, auditTable=auditTable, equipmentAudit=equipmentAudit, verboseLevel=verboseLevel, errEvLoggingEnabled=errEvLoggingEnabled, portDplxStatusChange=portDplxStatusChange, cardPrStateChange=cardPrStateChange, cardTestScheduleCommand=cardTestScheduleCommand, cardSpareGroupId=cardSpareGroupId, equipmentPortGroup=equipmentPortGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, auditCardId=auditCardId, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeShelfId=diskVolumeShelfId, equipmentStateGroup=equipmentStateGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCarrierFwVersion=cardCarrierFwVersion, cardId=cardId, portTxFlowControlDetected=portTxFlowControlDetected, cmResetStatus=cmResetStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, trapSeverity=trapSeverity, cardNumPorts=cardNumPorts, cardTemperature=cardTemperature, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardCPLDVersion=cardCPLDVersion, equipmentTbl=equipmentTbl, portRxFlowControlDetected=portRxFlowControlDetected, cmVendor=cmVendor, cardSpareGroupMode=cardSpareGroupMode, portBgpId=portBgpId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSecStateChange=cardSecStateChange, cardLastResetReason=cardLastResetReason)
