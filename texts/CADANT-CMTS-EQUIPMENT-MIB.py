...
CardSubType, PortMode, SecondaryState, DiskVolumeUsageLevel, FlowControlMode, PrimaryState, CardType, PortType, ShelfId, CardId, EqActionType, PortDetectedMode, UnknownState, DuplexStatus, PicType, AdminState, FirmwareSource, PortId = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "PortMode", "SecondaryState", "DiskVolumeUsageLevel", "FlowControlMode", "PrimaryState", "CardType", "PortType", "ShelfId", "CardId", "EqActionType", "PortDetectedMode", "UnknownState", "DuplexStatus", "PicType", "AdminState", "FirmwareSource", "PortId")
...
DisplayString, MacAddress, TextualConvention, DateAndTime, RowStatus, TimeStamp, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "TextualConvention", "DateAndTime", "RowStatus", "TimeStamp", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditId=auditId, diskVolumeEntry=diskVolumeEntry, cardDplxStatusChange=cardDplxStatusChange, cadEquipmentMibConformance=cadEquipmentMibConformance, portDetectedMode=portDetectedMode, auditEntry=auditEntry, cardSubDetected=cardSubDetected, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmRegistrationNotification=cmRegistrationNotification, cardTable=cardTable, diskVolumeUsageNotification=diskVolumeUsageNotification, portShelfId=portShelfId, auditCycleCount=auditCycleCount, cardTestDescription=cardTestDescription, diskVolumeName=diskVolumeName, portDescription=portDescription, cardCarrierSerialNum=cardCarrierSerialNum, cardTestEntry=cardTestEntry, equipmentStateGroup=equipmentStateGroup, cmResetInfo=cmResetInfo, cardCpuSpeed=cardCpuSpeed, portTable=portTable, cardBusSpeed=cardBusSpeed, auditTable=auditTable, equipmentShelfGroup=equipmentShelfGroup, diskVolumeDiskSize=diskVolumeDiskSize, cardFwVersion=cardFwVersion, portMacAddress=portMacAddress, equipmentAuditGroup=equipmentAuditGroup, consoleOutput=consoleOutput, TestType=TestType, portCardId=portCardId, portDplxStatus=portDplxStatus, cardTestIndex=cardTestIndex, portDocsIfIndex=portDocsIfIndex, cardSecStateChange=cardSecStateChange, cardPrState=cardPrState, cardDplxStatus=cardDplxStatus, diskVolumeFileName=diskVolumeFileName, auditLogOutput=auditLogOutput, auditShelfId=auditShelfId, cmResetNotification=cmResetNotification, cardFpgaSource=cardFpgaSource, portSecState=portSecState, cardNumber=cardNumber, PYSNMP_MODULE_ID=cadEquipmentMib, auditStatus=auditStatus, TestCommand=TestCommand, portNumChans=portNumChans, portTxFlowControlDetected=portTxFlowControlDetected, portDplxStatusChange=portDplxStatusChange, shelfNumber=shelfNumber, portRxFlowControlMode=portRxFlowControlMode, cardTestName=cardTestName, cmMacAddress=cmMacAddress, portAction=portAction, cmDevice=cmDevice, cardUpTime=cardUpTime, cmIpAddress=cmIpAddress, cardType=cardType, cadEquipmentMibGroup=cadEquipmentMibGroup, portRxFlowControlDetected=portRxFlowControlDetected, equipmentPortGroup=equipmentPortGroup, portMode=portMode, portConnectorId=portConnectorId, portMinDsPower=portMinDsPower, portCardSubType=portCardSubType, cmVendor=cmVendor, portAdminState=portAdminState, equipmentCardGroup=equipmentCardGroup, cardId=cardId, cardTestResultDesc=cardTestResultDesc, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfSwVersion=shelfSwVersion, cmUChannelID=cmUChannelID, equipmentCardTestGroup=equipmentCardTestGroup, cardAdminState=cardAdminState, cardDetectedChange=cardDetectedChange, auditFailedCount=auditFailedCount, cmResetStatus=cmResetStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, TestResult=TestResult, cardCarrierFwVersion=cardCarrierFwVersion, portPrStateChange=portPrStateChange, portBgpId=portBgpId, diagTestId=diagTestId, cardTestId=cardTestId, cardLastResetReason=cardLastResetReason, TestScheduleCommand=TestScheduleCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCpuType=cardCpuType, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeTrapInh=diskVolumeTrapInh, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSwVersion=cardSwVersion, portType=portType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetClearNotification=cmResetClearNotification, cardTrapInh=cardTrapInh, cardTestSchedule=cardTestSchedule, errEvLoggingEnabled=errEvLoggingEnabled, cardCPLDVersion=cardCPLDVersion, cardTestTime=cardTestTime, diskVolumeCardId=diskVolumeCardId, trapCounter=trapCounter, portEntry=portEntry, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentDiag=equipmentDiag, diskVolumeSize=diskVolumeSize, portId=portId, shelfObjects=shelfObjects, portCurrDsPower=portCurrDsPower, auditName=auditName, cardTestCommand=cardTestCommand, cardPicHwVersion=cardPicHwVersion, auditResult=auditResult, cardTestResultNotification=cardTestResultNotification, cardTestType=cardTestType, auditCommand=auditCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portNumber=portNumber, cardLastBootSource=cardLastBootSource, errEvRowStatus=errEvRowStatus, systemGeneral=systemGeneral, cardTestResult=cardTestResult, TestSchedule=TestSchedule, cardTemperature=cardTemperature, fpgaErrorEventGroup=fpgaErrorEventGroup, cardLastChangeTime=cardLastChangeTime, diskVolumeDriveId=diskVolumeDriveId, cardSpareGroupId=cardSpareGroupId, diskVolumeShelfId=diskVolumeShelfId, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardEntry=cardEntry, systemKey=systemKey, portMaxDsPower=portMaxDsPower, errEventId=errEventId, auditTime=auditTime, cardHwVersion=cardHwVersion, diskVolumeTable=diskVolumeTable, cardPrStateChange=cardPrStateChange, equipmentState=equipmentState, cardNumPorts=cardNumPorts, cardSpareGroupMode=cardSpareGroupMode, cardAction=cardAction, TestTransactionId=TestTransactionId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardPicSerialNum=cardPicSerialNum, cardShelfTestIndex=cardShelfTestIndex, cardTestScheduleCommand=cardTestScheduleCommand, systemGeneralGroup=systemGeneralGroup, equipmentTraps=equipmentTraps, diskVolumeGroup=diskVolumeGroup, cardSecState=cardSecState, verboseLevel=verboseLevel, cardTemperatureHighError=cardTemperatureHighError, portTrapInh=portTrapInh, portSecStateChange=portSecStateChange, cardTestTable=cardTestTable, trapSeverity=trapSeverity, diskVolumeId=diskVolumeId, portPrState=portPrState, portTxFlowControlMode=portTxFlowControlMode, auditLogThrottle=auditLogThrottle, equipmentAudit=equipmentAudit, diskDriveNumber=diskDriveNumber, shelfName=shelfName, cardRamSize=cardRamSize, auditTotalPassedCount=auditTotalPassedCount, cardDetected=cardDetected, cardPicDetected=cardPicDetected, cadEquipmentMib=cadEquipmentMib, portLastChangeTime=portLastChangeTime, cardLastBootVersion=cardLastBootVersion, equipmentDiagGroup=equipmentDiagGroup, portMacIfIndex=portMacIfIndex, cardBootVersion=cardBootVersion, cardName=cardName, auditCardId=auditCardId, remainInDiagMode=remainInDiagMode, cardSerialNum=cardSerialNum, downstreamPowerLoss=downstreamPowerLoss, fpgaErrorEventTable=fpgaErrorEventTable, errEvLogLevel=errEvLogLevel, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardShelfId=cardShelfId, systemClock=systemClock, cmResetReason=cmResetReason, TestId=TestId, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentTbl=equipmentTbl, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTransId=cardTestTransId, equipmentError=equipmentError, cardSubType=cardSubType, auditDescription=auditDescription, eqDiagConfig=eqDiagConfig, cmPrimarySID=cmPrimarySID, auditPassedCount=auditPassedCount, cmResetUpTime=cmResetUpTime, cardFlashSize=cardFlashSize, diskVolumeNumber=diskVolumeNumber)
