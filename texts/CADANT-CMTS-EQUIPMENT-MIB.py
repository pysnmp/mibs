...
PortId, CardType, FirmwareSource, DiskVolumeUsageLevel, EqActionType, CardId, FlowControlMode, PrimaryState, UnknownState, PicType, PortMode, PortDetectedMode, CardSubType, PortType, SecondaryState, AdminState, ShelfId, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PortId", "CardType", "FirmwareSource", "DiskVolumeUsageLevel", "EqActionType", "CardId", "FlowControlMode", "PrimaryState", "UnknownState", "PicType", "PortMode", "PortDetectedMode", "CardSubType", "PortType", "SecondaryState", "AdminState", "ShelfId", "DuplexStatus")
...
TruthValue, RowStatus, MacAddress, DateAndTime, TextualConvention, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "MacAddress", "DateAndTime", "TextualConvention", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmMacAddress=cmMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, portMacAddress=portMacAddress, equipmentAuditGroup=equipmentAuditGroup, cardSecState=cardSecState, cardDetected=cardDetected, shelfName=shelfName, cardDplxStatus=cardDplxStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, auditCommand=auditCommand, diskVolumeTable=diskVolumeTable, auditFailedCount=auditFailedCount, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeSize=diskVolumeSize, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastBootVersion=cardLastBootVersion, equipmentError=equipmentError, portDplxStatus=portDplxStatus, TestSchedule=TestSchedule, TestScheduleCommand=TestScheduleCommand, fpgaErrorEventTable=fpgaErrorEventTable, remainInDiagMode=remainInDiagMode, auditCardId=auditCardId, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentCardTestGroup=equipmentCardTestGroup, auditShelfId=auditShelfId, cmIpAddress=cmIpAddress, portConnectorId=portConnectorId, diskVolumeShelfId=diskVolumeShelfId, portMacIfIndex=portMacIfIndex, cmPrimarySID=cmPrimarySID, cmVendor=cmVendor, cardCarrierHwVersion=cardCarrierHwVersion, portSecState=portSecState, portAction=portAction, portDescription=portDescription, portTable=portTable, TestType=TestType, cardTestType=cardTestType, shelfObjects=shelfObjects, portDocsIfIndex=portDocsIfIndex, cardTestId=cardTestId, cardSerialNum=cardSerialNum, portMinDsPower=portMinDsPower, portTrapInh=portTrapInh, cardTrapInh=cardTrapInh, portSecStateChange=portSecStateChange, eqDiagConfig=eqDiagConfig, cardNumPorts=cardNumPorts, TestId=TestId, portType=portType, cardTemperatureHighError=cardTemperatureHighError, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCarrierFwVersion=cardCarrierFwVersion, cardLastBootSource=cardLastBootSource, cardLastResetReason=cardLastResetReason, cardId=cardId, errEvRowStatus=errEvRowStatus, portBgpId=portBgpId, consoleOutput=consoleOutput, systemGeneral=systemGeneral, fpgaErrorEventGroup=fpgaErrorEventGroup, auditStatus=auditStatus, auditTime=auditTime, auditDescription=auditDescription, cardTable=cardTable, cadEquipmentMibConformance=cadEquipmentMibConformance, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibGroup=cadEquipmentMibGroup, portPrState=portPrState, portNumChans=portNumChans, shelfNumber=shelfNumber, auditTotalFailedCount=auditTotalFailedCount, cardTestIndex=cardTestIndex, diskVolumeNumber=diskVolumeNumber, portTxFlowControlMode=portTxFlowControlMode, portShelfId=portShelfId, cardHwVersion=cardHwVersion, systemKey=systemKey, cardCpuSpeed=cardCpuSpeed, cmResetClearNotification=cmResetClearNotification, PYSNMP_MODULE_ID=cadEquipmentMib, cardSwVersion=cardSwVersion, cardTemperature=cardTemperature, systemClock=systemClock, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, fpgaErrorEventEntry=fpgaErrorEventEntry, portAdminState=portAdminState, diskVolumeCardId=diskVolumeCardId, diskVolumeName=diskVolumeName, cardSubDetected=cardSubDetected, diskVolumeEntry=diskVolumeEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, portMode=portMode, cardPicDetected=cardPicDetected, auditEntry=auditEntry, diskDriveNumber=diskDriveNumber, cardTestTime=cardTestTime, cardTestEntry=cardTestEntry, TestCommand=TestCommand, cmRegistrationNotification=cmRegistrationNotification, cardFwUpdateStatus=cardFwUpdateStatus, cardAdminState=cardAdminState, cardTestSchedule=cardTestSchedule, cardCpuType=cardCpuType, cadEquipmentMib=cadEquipmentMib, cardTestDescription=cardTestDescription, cardCarrierSerialNum=cardCarrierSerialNum, portDplxStatusChange=portDplxStatusChange, equipmentAudit=equipmentAudit, cardRamSize=cardRamSize, cardUpTime=cardUpTime, equipmentStateGroup=equipmentStateGroup, cardTestResultNotification=cardTestResultNotification, cardAction=cardAction, diskVolumeFileName=diskVolumeFileName, portDetectedMode=portDetectedMode, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardEntry=cardEntry, cardSpareGroupMode=cardSpareGroupMode, cardDetectedChange=cardDetectedChange, auditId=auditId, equipmentPortGroup=equipmentPortGroup, auditResult=auditResult, cardTestTransId=cardTestTransId, cardTestScheduleCommand=cardTestScheduleCommand, cardName=cardName, diagTestId=diagTestId, equipmentCardGroup=equipmentCardGroup, cardCPLDVersion=cardCPLDVersion, cardPicSerialNum=cardPicSerialNum, cardPrState=cardPrState, auditLogThrottle=auditLogThrottle, TestResult=TestResult, downstreamPowerLoss=downstreamPowerLoss, cmUChannelID=cmUChannelID, portMaxDsPower=portMaxDsPower, cmResetStatus=cmResetStatus, portTxFlowControlDetected=portTxFlowControlDetected, TestTransactionId=TestTransactionId, cmResetNotification=cmResetNotification, cardTempNoReportNotification=cardTempNoReportNotification, equipmentTraps=equipmentTraps, portCardSubType=portCardSubType, diskVolumeId=diskVolumeId, auditPassedCount=auditPassedCount, portNumber=portNumber, auditTable=auditTable, cardType=cardType, cardTestTable=cardTestTable, cardDplxStatusChange=cardDplxStatusChange, cmResetReason=cmResetReason, equipmentShelfGroup=equipmentShelfGroup, cardTestResultDesc=cardTestResultDesc, cmResetInfo=cmResetInfo, diskVolumeUsageLevel=diskVolumeUsageLevel, portLastChangeTime=portLastChangeTime, auditCycleCount=auditCycleCount, cmDevice=cmDevice, cmResetUpTime=cmResetUpTime, cardFwVersion=cardFwVersion, cardSubType=cardSubType, portPrStateChange=portPrStateChange, cardFpgaSource=cardFpgaSource, portCardId=portCardId, diskVolumeGroup=diskVolumeGroup, cardBusSpeed=cardBusSpeed, diskVolumeTrapInh=diskVolumeTrapInh, errEvRecoveryEnabled=errEvRecoveryEnabled, portEntry=portEntry, cardPrStateChange=cardPrStateChange, cardPicHwVersion=cardPicHwVersion, cardSpareGroupId=cardSpareGroupId, systemGeneralGroup=systemGeneralGroup, errEventId=errEventId, cardFlashSize=cardFlashSize, portId=portId, cardShelfId=cardShelfId, portCurrDsPower=portCurrDsPower, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, verboseLevel=verboseLevel, auditName=auditName, cardSecStateChange=cardSecStateChange, portRxFlowControlMode=portRxFlowControlMode, cardTestResult=cardTestResult, shelfSwVersion=shelfSwVersion, equipmentState=equipmentState, auditLogOutput=auditLogOutput, cardBootVersion=cardBootVersion, cardTestName=cardTestName, cardShelfTestIndex=cardShelfTestIndex, cardLastChangeTime=cardLastChangeTime, equipmentDiag=equipmentDiag, cardTestCommand=cardTestCommand, errEvLogLevel=errEvLogLevel, trapCounter=trapCounter, equipmentDiagGroup=equipmentDiagGroup, cardNumber=cardNumber, equipmentTbl=equipmentTbl, trapSeverity=trapSeverity, diskVolumeDriveId=diskVolumeDriveId)
