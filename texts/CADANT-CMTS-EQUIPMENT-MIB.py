...
PicType, CardSubType, AdminState, DuplexStatus, PortMode, FirmwareSource, PortType, PortId, ShelfId, UnknownState, CardId, FlowControlMode, CardType, DiskVolumeUsageLevel, PortDetectedMode, PrimaryState, EqActionType, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "PicType", "CardSubType", "AdminState", "DuplexStatus", "PortMode", "FirmwareSource", "PortType", "PortId", "ShelfId", "UnknownState", "CardId", "FlowControlMode", "CardType", "DiskVolumeUsageLevel", "PortDetectedMode", "PrimaryState", "EqActionType", "SecondaryState")
...
DateAndTime, DisplayString, TextualConvention, TimeStamp, RowStatus, MacAddress, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "DisplayString", "TextualConvention", "TimeStamp", "RowStatus", "MacAddress", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBusSpeed=cardBusSpeed, auditTotalFailedCount=auditTotalFailedCount, auditCommand=auditCommand, cardDetectedChange=cardDetectedChange, auditLogThrottle=auditLogThrottle, portTrapInh=portTrapInh, downstreamPowerLoss=downstreamPowerLoss, cmResetReason=cmResetReason, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diagTestId=diagTestId, shelfNumber=shelfNumber, portRxFlowControlDetected=portRxFlowControlDetected, cardCpuType=cardCpuType, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeGroup=diskVolumeGroup, portDplxStatus=portDplxStatus, auditId=auditId, cmResetNotification=cmResetNotification, portEntry=portEntry, systemGeneral=systemGeneral, cardTemperature=cardTemperature, diskVolumeDriveId=diskVolumeDriveId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardFlashSize=cardFlashSize, equipmentDiag=equipmentDiag, diskVolumeId=diskVolumeId, cardTestScheduleCommand=cardTestScheduleCommand, cardTestTime=cardTestTime, equipmentState=equipmentState, cmPrimarySID=cmPrimarySID, cmIpAddress=cmIpAddress, PYSNMP_MODULE_ID=cadEquipmentMib, cardShelfId=cardShelfId, cardTestCommand=cardTestCommand, portShelfId=portShelfId, equipmentStateGroup=equipmentStateGroup, diskVolumeEntry=diskVolumeEntry, shelfName=shelfName, equipmentCardTestGroup=equipmentCardTestGroup, cardTestDescription=cardTestDescription, cardSecState=cardSecState, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, portType=portType, diskVolumeSize=diskVolumeSize, portRxFlowControlMode=portRxFlowControlMode, cardTestIndex=cardTestIndex, cardPicHwVersion=cardPicHwVersion, cmDevice=cmDevice, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardRamSize=cardRamSize, systemGeneralGroup=systemGeneralGroup, portNumChans=portNumChans, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestTable=cardTestTable, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeName=diskVolumeName, cadEquipmentMib=cadEquipmentMib, trapSeverity=trapSeverity, diskDriveNumber=diskDriveNumber, portMaxDsPower=portMaxDsPower, TestCommand=TestCommand, cardPicSerialNum=cardPicSerialNum, portAdminState=portAdminState, cardCpuSpeed=cardCpuSpeed, errEventId=errEventId, cardTable=cardTable, cardLastChangeTime=cardLastChangeTime, cardPicDetected=cardPicDetected, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeTable=diskVolumeTable, fpgaErrorEventEntry=fpgaErrorEventEntry, cardLastBootVersion=cardLastBootVersion, eqDiagConfig=eqDiagConfig, TestType=TestType, equipmentPortGroup=equipmentPortGroup, cardLastResetReason=cardLastResetReason, cardTestResultNotification=cardTestResultNotification, cardSerialNum=cardSerialNum, cardAction=cardAction, cardEntry=cardEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, systemKey=systemKey, cardTrapInh=cardTrapInh, cardTestResult=cardTestResult, verboseLevel=verboseLevel, cardDplxStatusChange=cardDplxStatusChange, consoleOutput=consoleOutput, diskVolumeCardId=diskVolumeCardId, auditPassedCount=auditPassedCount, cardNumber=cardNumber, portId=portId, shelfObjects=shelfObjects, auditName=auditName, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTempNoReportNotification=cardTempNoReportNotification, cardSpareGroupMode=cardSpareGroupMode, cardTestTransId=cardTestTransId, auditAutoScheduling=auditAutoScheduling, cardCPLDVersion=cardCPLDVersion, cardUpTime=cardUpTime, portMode=portMode, cardCarrierFwVersion=cardCarrierFwVersion, cardName=cardName, TestScheduleCommand=TestScheduleCommand, cardId=cardId, diskVolumeShelfId=diskVolumeShelfId, equipmentAuditGroup=equipmentAuditGroup, systemClock=systemClock, TestTransactionId=TestTransactionId, cardHwVersion=cardHwVersion, auditLogOutput=auditLogOutput, cardFpgaSource=cardFpgaSource, cardBootVersion=cardBootVersion, equipmentError=equipmentError, portPrStateChange=portPrStateChange, cardSubDetected=cardSubDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastBootSource=cardLastBootSource, portCurrDsPower=portCurrDsPower, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestId=cardTestId, cmResetUpTime=cmResetUpTime, auditEntry=auditEntry, portSecState=portSecState, shelfSwVersion=shelfSwVersion, auditTime=auditTime, portDescription=portDescription, diskVolumeFileName=diskVolumeFileName, cmResetStatus=cmResetStatus, TestResult=TestResult, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDplxStatusChange=portDplxStatusChange, cmResetClearNotification=cmResetClearNotification, portPrState=portPrState, equipmentTbl=equipmentTbl, auditDescription=auditDescription, equipmentCardGroup=equipmentCardGroup, cardPrState=cardPrState, portTxFlowControlMode=portTxFlowControlMode, cardFwUpdateStatus=cardFwUpdateStatus, remainInDiagMode=remainInDiagMode, auditStatus=auditStatus, cardPrStateChange=cardPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, errEvLogLevel=errEvLogLevel, cardSubType=cardSubType, TestId=TestId, diskVolumeUsageNotification=diskVolumeUsageNotification, portCardSubType=portCardSubType, cardDetected=cardDetected, portNumber=portNumber, portSecStateChange=portSecStateChange, equipmentShelfGroup=equipmentShelfGroup, cardTemperatureHighError=cardTemperatureHighError, portConnectorId=portConnectorId, portLastChangeTime=portLastChangeTime, equipmentAudit=equipmentAudit, cardShelfTestIndex=cardShelfTestIndex, cardTestEntry=cardTestEntry, portMinDsPower=portMinDsPower, cardSwVersion=cardSwVersion, cmMacAddress=cmMacAddress, portDocsIfIndex=portDocsIfIndex, cmVendor=cmVendor, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditShelfId=auditShelfId, diskVolumeTrapInh=diskVolumeTrapInh, cardSecStateChange=cardSecStateChange, auditTotalPassedCount=auditTotalPassedCount, cardTestName=cardTestName, auditCardId=auditCardId, errEvRecoveryEnabled=errEvRecoveryEnabled, auditTable=auditTable, cardTestType=cardTestType, TestSchedule=TestSchedule, trapCounter=trapCounter, portCardId=portCardId, cmUChannelID=cmUChannelID, portTable=portTable, cardFwVersion=cardFwVersion, cardNumPorts=cardNumPorts, portDetectedMode=portDetectedMode, fpgaErrorEventTable=fpgaErrorEventTable, equipmentDiagGroup=equipmentDiagGroup, cmResetInfo=cmResetInfo, cardDplxStatus=cardDplxStatus, cardTestResultDesc=cardTestResultDesc, auditFailedCount=auditFailedCount, portMacAddress=portMacAddress, cardSpareGroupId=cardSpareGroupId, auditCycleCount=auditCycleCount, portBgpId=portBgpId, portMacIfIndex=portMacIfIndex, cardTestSchedule=cardTestSchedule, cardAdminState=cardAdminState, errEvRowStatus=errEvRowStatus, diskVolumeNumber=diskVolumeNumber, cardCarrierHwVersion=cardCarrierHwVersion, equipmentTraps=equipmentTraps, portAction=portAction, diskVolumeUsageLevel=diskVolumeUsageLevel, auditResult=auditResult, cardType=cardType, cadEquipmentMibGroup=cadEquipmentMibGroup)
