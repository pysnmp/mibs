...
AdminState, FirmwareSource, CardId, PortDetectedMode, PrimaryState, PicType, UnknownState, SecondaryState, PortId, CardType, PortType, ShelfId, EqActionType, FlowControlMode, CardSubType, DuplexStatus, DiskVolumeUsageLevel, PortMode = mibBuilder.importSymbols("CADANT-TC", "AdminState", "FirmwareSource", "CardId", "PortDetectedMode", "PrimaryState", "PicType", "UnknownState", "SecondaryState", "PortId", "CardType", "PortType", "ShelfId", "EqActionType", "FlowControlMode", "CardSubType", "DuplexStatus", "DiskVolumeUsageLevel", "PortMode")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTime=auditTime, diskDriveNumber=diskDriveNumber, cardFpgaSource=cardFpgaSource, TestResult=TestResult, cardDetectedChange=cardDetectedChange, cmResetNotification=cmResetNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentTbl=equipmentTbl, fpgaErrorEventTable=fpgaErrorEventTable, cardTestName=cardTestName, cardTestResultDesc=cardTestResultDesc, diskVolumeTable=diskVolumeTable, diagTestId=diagTestId, cardTestSchedule=cardTestSchedule, diskVolumeId=diskVolumeId, cardBusSpeed=cardBusSpeed, portRxFlowControlDetected=portRxFlowControlDetected, cardTestId=cardTestId, diskVolumeDriveId=diskVolumeDriveId, diskVolumeNumber=diskVolumeNumber, cardAdminState=cardAdminState, TestId=TestId, cardCpuType=cardCpuType, equipmentShelfGroup=equipmentShelfGroup, cardSubType=cardSubType, portNumChans=portNumChans, cmResetStatus=cmResetStatus, portTrapInh=portTrapInh, cardTestTime=cardTestTime, TestType=TestType, shelfSwVersion=shelfSwVersion, diskVolumeShelfId=diskVolumeShelfId, cardTestIndex=cardTestIndex, portType=portType, auditLogOutput=auditLogOutput, cardCarrierFwVersion=cardCarrierFwVersion, cmMacAddress=cmMacAddress, errEvLogLevel=errEvLogLevel, portPrState=portPrState, portRxFlowControlMode=portRxFlowControlMode, cardTemperature=cardTemperature, auditShelfId=auditShelfId, cardDplxStatus=cardDplxStatus, portMacIfIndex=portMacIfIndex, cardTestResult=cardTestResult, portBgpId=portBgpId, cardHwVersion=cardHwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicHwVersion=cardPicHwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentCardTestGroup=equipmentCardTestGroup, portAdminState=portAdminState, cardFwUpdateStatus=cardFwUpdateStatus, equipmentError=equipmentError, cardBootVersion=cardBootVersion, cmPrimarySID=cmPrimarySID, portId=portId, shelfNumber=shelfNumber, portSecState=portSecState, auditId=auditId, auditCycleCount=auditCycleCount, cardTable=cardTable, cardSecState=cardSecState, cardTempOverHeatNotification=cardTempOverHeatNotification, cardType=cardType, cardLastBootSource=cardLastBootSource, portMaxDsPower=portMaxDsPower, eqDiagConfig=eqDiagConfig, equipmentStateGroup=equipmentStateGroup, cardCarrierHwVersion=cardCarrierHwVersion, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMib=cadEquipmentMib, cardDetected=cardDetected, equipmentTraps=equipmentTraps, systemGeneralGroup=systemGeneralGroup, diskVolumeTrapInh=diskVolumeTrapInh, auditName=auditName, cardDplxStatusChange=cardDplxStatusChange, consoleOutput=consoleOutput, cardEntry=cardEntry, portMinDsPower=portMinDsPower, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeCardId=diskVolumeCardId, cardSpareGroupMode=cardSpareGroupMode, portTable=portTable, cmVendor=cmVendor, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCarrierSerialNum=cardCarrierSerialNum, portNumber=portNumber, auditCommand=auditCommand, auditStatus=auditStatus, equipmentPortGroup=equipmentPortGroup, cardCPLDVersion=cardCPLDVersion, auditResult=auditResult, auditCardId=auditCardId, cardPicDetected=cardPicDetected, equipmentAudit=equipmentAudit, diskVolumeUsageNotification=diskVolumeUsageNotification, errEvRowStatus=errEvRowStatus, cardPrStateChange=cardPrStateChange, equipmentAuditGroup=equipmentAuditGroup, trapCounter=trapCounter, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestCommand=TestCommand, portMacAddress=portMacAddress, cardName=cardName, equipmentDiag=equipmentDiag, cardSubDetected=cardSubDetected, portTxFlowControlMode=portTxFlowControlMode, portDplxStatus=portDplxStatus, auditTable=auditTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, trapSeverity=trapSeverity, portCardSubType=portCardSubType, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSwVersion=cardSwVersion, diskVolumeDiskSize=diskVolumeDiskSize, systemClock=systemClock, cardTestScheduleCommand=cardTestScheduleCommand, auditAutoScheduling=auditAutoScheduling, verboseLevel=verboseLevel, downstreamPowerLoss=downstreamPowerLoss, diskVolumeFileName=diskVolumeFileName, cardSpareGroupId=cardSpareGroupId, cardTestTransId=cardTestTransId, portCardId=portCardId, TestSchedule=TestSchedule, systemGeneral=systemGeneral, cardTrapInh=cardTrapInh, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portDetectedMode=portDetectedMode, equipmentState=equipmentState, auditPassedCount=auditPassedCount, remainInDiagMode=remainInDiagMode, cardCpuSpeed=cardCpuSpeed, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardPrState=cardPrState, cardLastBootVersion=cardLastBootVersion, cardFwVersion=cardFwVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastResetReason=cardLastResetReason, errEventId=errEventId, portDescription=portDescription, auditEntry=auditEntry, shelfName=shelfName, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestType=cardTestType, cardSerialNum=cardSerialNum, diskVolumeName=diskVolumeName, auditTotalFailedCount=auditTotalFailedCount, cmResetReason=cmResetReason, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portCurrDsPower=portCurrDsPower, equipmentCardGroup=equipmentCardGroup, cardShelfId=cardShelfId, cadEquipmentMibConformance=cadEquipmentMibConformance, cmResetUpTime=cmResetUpTime, portEntry=portEntry, auditDescription=auditDescription, equipmentDiagGroup=equipmentDiagGroup, auditFailedCount=auditFailedCount, cmDevice=cmDevice, portDplxStatusChange=portDplxStatusChange, cardShelfTestIndex=cardShelfTestIndex, cmUChannelID=cmUChannelID, cardTestResultNotification=cardTestResultNotification, PYSNMP_MODULE_ID=cadEquipmentMib, portShelfId=portShelfId, cmResetInfo=cmResetInfo, portMode=portMode, cardAction=cardAction, cardPicSerialNum=cardPicSerialNum, auditLogThrottle=auditLogThrottle, cardTempNoReportNotification=cardTempNoReportNotification, auditTotalPassedCount=auditTotalPassedCount, shelfObjects=shelfObjects, diskVolumeEntry=diskVolumeEntry, cardRamSize=cardRamSize, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portConnectorId=portConnectorId, cardTestCommand=cardTestCommand, portLastChangeTime=portLastChangeTime, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeSize=diskVolumeSize, portAction=portAction, cmIpAddress=cmIpAddress, portSecStateChange=portSecStateChange, cardFlashSize=cardFlashSize, cardTestEntry=cardTestEntry, TestTransactionId=TestTransactionId, cardLastChangeTime=cardLastChangeTime, TestScheduleCommand=TestScheduleCommand, cardTestTable=cardTestTable, portDocsIfIndex=portDocsIfIndex, cardTestDescription=cardTestDescription, cardNumber=cardNumber, systemKey=systemKey, cmResetClearNotification=cmResetClearNotification, diskVolumeGroup=diskVolumeGroup, cardSecStateChange=cardSecStateChange, cmRegistrationNotification=cmRegistrationNotification, cardNumPorts=cardNumPorts, cardId=cardId, portPrStateChange=portPrStateChange, cardUpTime=cardUpTime)
