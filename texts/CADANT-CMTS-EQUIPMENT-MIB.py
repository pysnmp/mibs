...
PortMode, EqActionType, FlowControlMode, PicType, FirmwareSource, ShelfId, PrimaryState, PortId, SecondaryState, CardType, AdminState, UnknownState, PortType, CardSubType, PortDetectedMode, DuplexStatus, DiskVolumeUsageLevel, CardId = mibBuilder.importSymbols("CADANT-TC", "PortMode", "EqActionType", "FlowControlMode", "PicType", "FirmwareSource", "ShelfId", "PrimaryState", "PortId", "SecondaryState", "CardType", "AdminState", "UnknownState", "PortType", "CardSubType", "PortDetectedMode", "DuplexStatus", "DiskVolumeUsageLevel", "CardId")
...
DateAndTime, TextualConvention, MacAddress, TimeStamp, RowStatus, DisplayString, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TextualConvention", "MacAddress", "TimeStamp", "RowStatus", "DisplayString", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTemperatureHighError=cardTemperatureHighError, equipmentCardTestGroup=equipmentCardTestGroup, cmVendor=cmVendor, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMibGroup=cadEquipmentMibGroup, portShelfId=portShelfId, portDplxStatus=portDplxStatus, diskVolumeId=diskVolumeId, cardTestSchedule=cardTestSchedule, portAdminState=portAdminState, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestName=cardTestName, cardFlashSize=cardFlashSize, portMode=portMode, diskVolumeCardId=diskVolumeCardId, equipmentPortGroup=equipmentPortGroup, diskVolumeNumber=diskVolumeNumber, portTable=portTable, cardEntry=cardEntry, auditResult=auditResult, auditCycleCount=auditCycleCount, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestScheduleCommand=cardTestScheduleCommand, equipmentShelfGroup=equipmentShelfGroup, cardTestResult=cardTestResult, systemGeneral=systemGeneral, auditLogThrottle=auditLogThrottle, cmPrimarySID=cmPrimarySID, cardCarrierFwVersion=cardCarrierFwVersion, cardTestDescription=cardTestDescription, auditTotalPassedCount=auditTotalPassedCount, cardShelfId=cardShelfId, cardSecState=cardSecState, auditLogOutput=auditLogOutput, cardType=cardType, portTrapInh=portTrapInh, portDocsIfIndex=portDocsIfIndex, portRxFlowControlDetected=portRxFlowControlDetected, cardBootVersion=cardBootVersion, errEvLoggingEnabled=errEvLoggingEnabled, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentError=equipmentError, cardTestType=cardTestType, verboseLevel=verboseLevel, portCardId=portCardId, portPrStateChange=portPrStateChange, cardCpuType=cardCpuType, cardCpuSpeed=cardCpuSpeed, diskVolumeSize=diskVolumeSize, cardTestId=cardTestId, auditTable=auditTable, cmResetClearNotification=cmResetClearNotification, equipmentDiag=equipmentDiag, errEvLogLevel=errEvLogLevel, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetStatus=cmResetStatus, cardSubDetected=cardSubDetected, cardPicSerialNum=cardPicSerialNum, cardHwVersion=cardHwVersion, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventGroup=fpgaErrorEventGroup, cmDevice=cmDevice, TestId=TestId, portConnectorId=portConnectorId, cardPicHwVersion=cardPicHwVersion, shelfNumber=shelfNumber, systemClock=systemClock, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeTable=diskVolumeTable, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeFileName=diskVolumeFileName, diskVolumeTrapInh=diskVolumeTrapInh, portSecStateChange=portSecStateChange, auditFailedCount=auditFailedCount, auditTotalFailedCount=auditTotalFailedCount, equipmentAuditGroup=equipmentAuditGroup, cardTestResultNotification=cardTestResultNotification, auditStatus=auditStatus, cmResetReason=cmResetReason, cardId=cardId, portAction=portAction, errEvRowStatus=errEvRowStatus, TestResult=TestResult, cardFpgaSource=cardFpgaSource, cardTestTime=cardTestTime, trapSeverity=trapSeverity, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestCommand=cardTestCommand, cardDetectedChange=cardDetectedChange, portType=portType, portPrState=portPrState, diskVolumeName=diskVolumeName, diskVolumeEntry=diskVolumeEntry, cardLastResetReason=cardLastResetReason, cardLastChangeTime=cardLastChangeTime, downstreamPowerLoss=downstreamPowerLoss, cardSubType=cardSubType, equipmentTbl=equipmentTbl, cardTrapInh=cardTrapInh, cardPicDetected=cardPicDetected, cardSpareGroupMode=cardSpareGroupMode, equipmentState=equipmentState, shelfName=shelfName, portMaxDsPower=portMaxDsPower, cardSwVersion=cardSwVersion, cardUpTime=cardUpTime, diskDriveNumber=diskDriveNumber, portMinDsPower=portMinDsPower, portBgpId=portBgpId, cmResetInfo=cmResetInfo, auditName=auditName, cardDplxStatus=cardDplxStatus, cardDetected=cardDetected, portNumber=portNumber, portDplxStatusChange=portDplxStatusChange, cardFwVersion=cardFwVersion, cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMibConformance=cadEquipmentMibConformance, auditTime=auditTime, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, consoleOutput=consoleOutput, cardSpareGroupId=cardSpareGroupId, portDetectedMode=portDetectedMode, cardSerialNum=cardSerialNum, systemKey=systemKey, portRxFlowControlMode=portRxFlowControlMode, TestTransactionId=TestTransactionId, diagTestId=diagTestId, shelfObjects=shelfObjects, cardBusSpeed=cardBusSpeed, portId=portId, portCurrDsPower=portCurrDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, fpgaErrorEventEntry=fpgaErrorEventEntry, auditCommand=auditCommand, cardTable=cardTable, cardLastBootVersion=cardLastBootVersion, portNumChans=portNumChans, cmRegistrationNotification=cmRegistrationNotification, cardLastBootSource=cardLastBootSource, auditEntry=auditEntry, diskVolumeGroup=diskVolumeGroup, cmUChannelID=cmUChannelID, cardTestResultDesc=cardTestResultDesc, diskVolumeDriveId=diskVolumeDriveId, cardTestIndex=cardTestIndex, portSecState=portSecState, TestType=TestType, cardAdminState=cardAdminState, cardTestTransId=cardTestTransId, auditAutoScheduling=auditAutoScheduling, cardSecStateChange=cardSecStateChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portEntry=portEntry, equipmentTraps=equipmentTraps, auditShelfId=auditShelfId, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageNotification=diskVolumeUsageNotification, errEventId=errEventId, cmResetNotification=cmResetNotification, auditDescription=auditDescription, equipmentCardGroup=equipmentCardGroup, cardCarrierHwVersion=cardCarrierHwVersion, eqDiagConfig=eqDiagConfig, portCardSubType=portCardSubType, cmMacAddress=cmMacAddress, cmIpAddress=cmIpAddress, cardPrStateChange=cardPrStateChange, equipmentStateGroup=equipmentStateGroup, TestScheduleCommand=TestScheduleCommand, cardRamSize=cardRamSize, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeShelfId=diskVolumeShelfId, portLastChangeTime=portLastChangeTime, trapCounter=trapCounter, shelfSwVersion=shelfSwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, fpgaErrorEventTable=fpgaErrorEventTable, portMacIfIndex=portMacIfIndex, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portMacAddress=portMacAddress, cardDplxStatusChange=cardDplxStatusChange, remainInDiagMode=remainInDiagMode, cardCPLDVersion=cardCPLDVersion, cardNumber=cardNumber, cardTestEntry=cardTestEntry, auditId=auditId, auditPassedCount=auditPassedCount, auditCardId=auditCardId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTemperature=cardTemperature, cardTestTable=cardTestTable, systemGeneralGroup=systemGeneralGroup, TestCommand=TestCommand, portDescription=portDescription, TestSchedule=TestSchedule, cardPrState=cardPrState, cardNumPorts=cardNumPorts, equipmentDiagGroup=equipmentDiagGroup, cmResetUpTime=cmResetUpTime, cardName=cardName, cardFwUpdateStatus=cardFwUpdateStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentAudit=equipmentAudit, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardAction=cardAction)
