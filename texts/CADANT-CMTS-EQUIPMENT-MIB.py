...
PortType, PrimaryState, PicType, EqActionType, PortMode, SecondaryState, CardId, AdminState, FirmwareSource, ShelfId, PortDetectedMode, PortId, UnknownState, FlowControlMode, DuplexStatus, CardSubType, DiskVolumeUsageLevel, CardType = mibBuilder.importSymbols("CADANT-TC", "PortType", "PrimaryState", "PicType", "EqActionType", "PortMode", "SecondaryState", "CardId", "AdminState", "FirmwareSource", "ShelfId", "PortDetectedMode", "PortId", "UnknownState", "FlowControlMode", "DuplexStatus", "CardSubType", "DiskVolumeUsageLevel", "CardType")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastResetReason=cardLastResetReason, diskVolumeTrapInh=diskVolumeTrapInh, TestType=TestType, auditEntry=auditEntry, portBgpId=portBgpId, cmResetUpTime=cmResetUpTime, cardCarrierSerialNum=cardCarrierSerialNum, auditAutoScheduling=auditAutoScheduling, diskDriveNumber=diskDriveNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeDriveId=diskVolumeDriveId, eqDiagConfig=eqDiagConfig, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCPLDVersion=cardCPLDVersion, diskVolumeNumber=diskVolumeNumber, shelfNumber=shelfNumber, cardBusSpeed=cardBusSpeed, portLastChangeTime=portLastChangeTime, cardSubType=cardSubType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, errEventId=errEventId, equipmentPortGroup=equipmentPortGroup, cardLastBootVersion=cardLastBootVersion, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDiskSize=diskVolumeDiskSize, cardSwVersion=cardSwVersion, portTxFlowControlDetected=portTxFlowControlDetected, cardHwVersion=cardHwVersion, cardLastChangeTime=cardLastChangeTime, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentTbl=equipmentTbl, downstreamPowerLoss=downstreamPowerLoss, cardTestTable=cardTestTable, portId=portId, cardSpareGroupMode=cardSpareGroupMode, cardCarrierHwVersion=cardCarrierHwVersion, portTrapInh=portTrapInh, systemKey=systemKey, equipmentDiagGroup=equipmentDiagGroup, cmUChannelID=cmUChannelID, portSecStateChange=portSecStateChange, cardDplxStatusChange=cardDplxStatusChange, auditTime=auditTime, cardRamSize=cardRamSize, consoleOutput=consoleOutput, cardType=cardType, equipmentAudit=equipmentAudit, portEntry=portEntry, cardSerialNum=cardSerialNum, auditStatus=auditStatus, cardPrState=cardPrState, shelfSwVersion=shelfSwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, portSecState=portSecState, portAction=portAction, portTxFlowControlMode=portTxFlowControlMode, auditName=auditName, cardAdminState=cardAdminState, equipmentError=equipmentError, systemGeneral=systemGeneral, TestTransactionId=TestTransactionId, cardTrapInh=cardTrapInh, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portDetectedMode=portDetectedMode, cmPrimarySID=cmPrimarySID, verboseLevel=verboseLevel, auditCommand=auditCommand, cardTemperatureHighError=cardTemperatureHighError, equipmentDiag=equipmentDiag, fpgaErrorEventEntry=fpgaErrorEventEntry, trapCounter=trapCounter, TestCommand=TestCommand, cardNumPorts=cardNumPorts, auditCycleCount=auditCycleCount, equipmentStateGroup=equipmentStateGroup, cmDevice=cmDevice, cadEquipmentMibGroup=cadEquipmentMibGroup, cardPicDetected=cardPicDetected, portMaxDsPower=portMaxDsPower, auditLogThrottle=auditLogThrottle, cardBootVersion=cardBootVersion, portNumber=portNumber, diskVolumeId=diskVolumeId, cardShelfTestIndex=cardShelfTestIndex, portCardId=portCardId, cardFlashSize=cardFlashSize, cardTestTime=cardTestTime, cmMacAddress=cmMacAddress, errEvRowStatus=errEvRowStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, systemClock=systemClock, remainInDiagMode=remainInDiagMode, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetStatus=cmResetStatus, cardTestIndex=cardTestIndex, portMacIfIndex=portMacIfIndex, portType=portType, TestResult=TestResult, diskVolumeTable=diskVolumeTable, auditId=auditId, cardTemperature=cardTemperature, portPrState=portPrState, cardTestResultDesc=cardTestResultDesc, cardEntry=cardEntry, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeGroup=diskVolumeGroup, cmResetReason=cmResetReason, cardTestResult=cardTestResult, diskVolumeSize=diskVolumeSize, cmResetInfo=cmResetInfo, cardFpgaSource=cardFpgaSource, auditLogOutput=auditLogOutput, auditPassedCount=auditPassedCount, cardSubDetected=cardSubDetected, portPrStateChange=portPrStateChange, diskVolumeEntry=diskVolumeEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvRecoveryEnabled=errEvRecoveryEnabled, cardDetected=cardDetected, cardSecState=cardSecState, TestId=TestId, cardTestSchedule=cardTestSchedule, portShelfId=portShelfId, diskVolumeName=diskVolumeName, cardSecStateChange=cardSecStateChange, portDplxStatus=portDplxStatus, cardFwVersion=cardFwVersion, portMacAddress=portMacAddress, diskVolumeFileName=diskVolumeFileName, diagTestId=diagTestId, cardFwUpdateStatus=cardFwUpdateStatus, trapSeverity=trapSeverity, portCurrDsPower=portCurrDsPower, portDplxStatusChange=portDplxStatusChange, cmResetClearNotification=cmResetClearNotification, cardCpuSpeed=cardCpuSpeed, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestCommand=cardTestCommand, portCardSubType=portCardSubType, cardTestType=cardTestType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portAdminState=portAdminState, auditFailedCount=auditFailedCount, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentCardGroup=equipmentCardGroup, cardPicSerialNum=cardPicSerialNum, cardTestName=cardTestName, fpgaErrorEventTable=fpgaErrorEventTable, cardNumber=cardNumber, cardDetectedChange=cardDetectedChange, cmRegistrationNotification=cmRegistrationNotification, portDescription=portDescription, cadEquipmentMib=cadEquipmentMib, diskVolumeCardId=diskVolumeCardId, cardPrStateChange=cardPrStateChange, cardTempNoReportNotification=cardTempNoReportNotification, equipmentShelfGroup=equipmentShelfGroup, cardTable=cardTable, auditResult=auditResult, portMode=portMode, portRxFlowControlMode=portRxFlowControlMode, TestSchedule=TestSchedule, cardTestScheduleCommand=cardTestScheduleCommand, auditTotalPassedCount=auditTotalPassedCount, cardUpTime=cardUpTime, portDocsIfIndex=portDocsIfIndex, cardCarrierFwVersion=cardCarrierFwVersion, portConnectorId=portConnectorId, shelfName=shelfName, cmVendor=cmVendor, cardTestTransId=cardTestTransId, cardTestDescription=cardTestDescription, cardLastBootSource=cardLastBootSource, cardShelfId=cardShelfId, auditCardId=auditCardId, cardSpareGroupId=cardSpareGroupId, shelfObjects=shelfObjects, systemGeneralGroup=systemGeneralGroup, errEvLogLevel=errEvLogLevel, diskVolumeShelfId=diskVolumeShelfId, equipmentTraps=equipmentTraps, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditTotalFailedCount=auditTotalFailedCount, cardName=cardName, cardPicHwVersion=cardPicHwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, TestScheduleCommand=TestScheduleCommand, equipmentState=equipmentState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTable=portTable, portNumChans=portNumChans, cardTestEntry=cardTestEntry, cardTestResultNotification=cardTestResultNotification, cardId=cardId, auditShelfId=auditShelfId, auditTable=auditTable, cardCpuType=cardCpuType, cmIpAddress=cmIpAddress, cadEquipmentMibConformance=cadEquipmentMibConformance, cmResetNotification=cmResetNotification, cardDplxStatus=cardDplxStatus, auditDescription=auditDescription, cardTestId=cardTestId, cardAction=cardAction, portMinDsPower=portMinDsPower)
