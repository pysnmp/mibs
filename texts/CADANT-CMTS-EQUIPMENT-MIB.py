...
DiskVolumeUsageLevel, PortType, CardType, PortDetectedMode, CardId, EqActionType, UnknownState, PicType, PortMode, FlowControlMode, AdminState, SecondaryState, DuplexStatus, PrimaryState, CardSubType, ShelfId, PortId, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "PortType", "CardType", "PortDetectedMode", "CardId", "EqActionType", "UnknownState", "PicType", "PortMode", "FlowControlMode", "AdminState", "SecondaryState", "DuplexStatus", "PrimaryState", "CardSubType", "ShelfId", "PortId", "FirmwareSource")
...
MacAddress, TextualConvention, RowStatus, DateAndTime, TimeStamp, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TextualConvention", "RowStatus", "DateAndTime", "TimeStamp", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", trapSeverity=trapSeverity, cardPicHwVersion=cardPicHwVersion, equipmentDiag=equipmentDiag, cardAdminState=cardAdminState, diskVolumeId=diskVolumeId, cmVendor=cmVendor, systemGeneral=systemGeneral, portNumChans=portNumChans, diskVolumeNumber=diskVolumeNumber, portMacIfIndex=portMacIfIndex, auditTotalPassedCount=auditTotalPassedCount, cmIpAddress=cmIpAddress, equipmentCardGroup=equipmentCardGroup, verboseLevel=verboseLevel, cardTestType=cardTestType, auditResult=auditResult, equipmentError=equipmentError, cmMacAddress=cmMacAddress, portConnectorId=portConnectorId, auditCommand=auditCommand, cardDetected=cardDetected, diskVolumeTrapInh=diskVolumeTrapInh, cmResetClearNotification=cmResetClearNotification, auditPassedCount=auditPassedCount, cmResetReason=cmResetReason, cardCarrierHwVersion=cardCarrierHwVersion, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventGroup=fpgaErrorEventGroup, portMode=portMode, cardLastChangeTime=cardLastChangeTime, cardNumber=cardNumber, PYSNMP_MODULE_ID=cadEquipmentMib, cmPrimarySID=cmPrimarySID, diskVolumeName=diskVolumeName, cardCarrierFwVersion=cardCarrierFwVersion, errEventId=errEventId, equipmentAuditGroup=equipmentAuditGroup, diagTestId=diagTestId, cardTestName=cardTestName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestSchedule=cardTestSchedule, cardShelfId=cardShelfId, portPrState=portPrState, cardTestResult=cardTestResult, cardPrState=cardPrState, auditLogOutput=auditLogOutput, cardSpareGroupMode=cardSpareGroupMode, portType=portType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, systemGeneralGroup=systemGeneralGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentState=equipmentState, cardTemperatureHighWarn=cardTemperatureHighWarn, auditDescription=auditDescription, diskVolumeUsageLevel=diskVolumeUsageLevel, TestTransactionId=TestTransactionId, eqDiagConfig=eqDiagConfig, cardSerialNum=cardSerialNum, auditEntry=auditEntry, portMinDsPower=portMinDsPower, cardPicSerialNum=cardPicSerialNum, portAdminState=portAdminState, equipmentPortGroup=equipmentPortGroup, cardFwUpdateStatus=cardFwUpdateStatus, errEvLoggingEnabled=errEvLoggingEnabled, portTxFlowControlMode=portTxFlowControlMode, cardFlashSize=cardFlashSize, cardPrStateChange=cardPrStateChange, cardFwVersion=cardFwVersion, cardTestTable=cardTestTable, portId=portId, shelfNumber=shelfNumber, cardPicDetected=cardPicDetected, cardName=cardName, systemKey=systemKey, cardCpuSpeed=cardCpuSpeed, cmResetNotification=cmResetNotification, cardTestCommand=cardTestCommand, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardUpTime=cardUpTime, auditFailedCount=auditFailedCount, auditName=auditName, auditCycleCount=auditCycleCount, cmResetStatus=cmResetStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeSize=diskVolumeSize, equipmentCardTestGroup=equipmentCardTestGroup, cardSecState=cardSecState, remainInDiagMode=remainInDiagMode, TestType=TestType, portBgpId=portBgpId, cardTable=cardTable, cardLastBootSource=cardLastBootSource, cardEntry=cardEntry, shelfSwVersion=shelfSwVersion, equipmentShelfGroup=equipmentShelfGroup, portLastChangeTime=portLastChangeTime, portDplxStatus=portDplxStatus, portTable=portTable, cardSpareGroupId=cardSpareGroupId, portNumber=portNumber, cardTemperatureHighError=cardTemperatureHighError, cardTestTransId=cardTestTransId, cardSwVersion=cardSwVersion, auditId=auditId, portMaxDsPower=portMaxDsPower, cardDetectedChange=cardDetectedChange, equipmentDiagGroup=equipmentDiagGroup, portDplxStatusChange=portDplxStatusChange, cardCpuType=cardCpuType, auditTable=auditTable, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfName=shelfName, fpgaErrorEventEntry=fpgaErrorEventEntry, cardHwVersion=cardHwVersion, cmDevice=cmDevice, cardTestIndex=cardTestIndex, errEvLogLevel=errEvLogLevel, cardTestEntry=cardTestEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, portDescription=portDescription, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeDiskSize=diskVolumeDiskSize, portTrapInh=portTrapInh, portEntry=portEntry, TestCommand=TestCommand, diskVolumeGroup=diskVolumeGroup, equipmentTbl=equipmentTbl, diskVolumeDriveId=diskVolumeDriveId, cardTestScheduleCommand=cardTestScheduleCommand, auditAutoScheduling=auditAutoScheduling, equipmentTraps=equipmentTraps, shelfObjects=shelfObjects, cardCarrierSerialNum=cardCarrierSerialNum, cardBootVersion=cardBootVersion, auditStatus=auditStatus, diskVolumeFileName=diskVolumeFileName, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTempNoReportNotification=cardTempNoReportNotification, diskDriveNumber=diskDriveNumber, cmUChannelID=cmUChannelID, portTxFlowControlDetected=portTxFlowControlDetected, TestSchedule=TestSchedule, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portPrStateChange=portPrStateChange, cardFpgaSource=cardFpgaSource, portAction=portAction, cardShelfTestIndex=cardShelfTestIndex, portCardId=portCardId, portSecState=portSecState, TestResult=TestResult, cardLastResetReason=cardLastResetReason, systemClock=systemClock, cardTestResultNotification=cardTestResultNotification, TestId=TestId, equipmentStateGroup=equipmentStateGroup, auditCardId=auditCardId, diskVolumeCardId=diskVolumeCardId, consoleOutput=consoleOutput, diskVolumeTable=diskVolumeTable, cardBusSpeed=cardBusSpeed, cardAction=cardAction, portDocsIfIndex=portDocsIfIndex, portMacAddress=portMacAddress, cardLastBootVersion=cardLastBootVersion, equipmentAudit=equipmentAudit, cmResetUpTime=cmResetUpTime, cardCPLDVersion=cardCPLDVersion, cardDplxStatus=cardDplxStatus, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSubType=cardSubType, portShelfId=portShelfId, diskVolumeShelfId=diskVolumeShelfId, auditLogThrottle=auditLogThrottle, portCardSubType=portCardSubType, auditTotalFailedCount=auditTotalFailedCount, cardTestDescription=cardTestDescription, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditShelfId=auditShelfId, errEvRowStatus=errEvRowStatus, diskVolumeEntry=diskVolumeEntry, auditTime=auditTime, cardDplxStatusChange=cardDplxStatusChange, cmRegistrationNotification=cmRegistrationNotification, TestScheduleCommand=TestScheduleCommand, portDetectedMode=portDetectedMode, cardId=cardId, cardRamSize=cardRamSize, cardTestId=cardTestId, cardSubDetected=cardSubDetected, portRxFlowControlMode=portRxFlowControlMode, portSecStateChange=portSecStateChange, cardNumPorts=cardNumPorts, cardTrapInh=cardTrapInh, portCurrDsPower=portCurrDsPower, cardTestTime=cardTestTime, cardType=cardType, downstreamPowerLoss=downstreamPowerLoss, cardSecStateChange=cardSecStateChange, trapCounter=trapCounter, cmResetInfo=cmResetInfo, cardTemperature=cardTemperature)
