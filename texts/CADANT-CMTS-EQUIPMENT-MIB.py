...
DiskVolumeUsageLevel, DuplexStatus, PortType, FlowControlMode, PicType, CardId, CardType, AdminState, EqActionType, PortMode, UnknownState, FirmwareSource, PrimaryState, SecondaryState, ShelfId, PortId, PortDetectedMode, CardSubType = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "DuplexStatus", "PortType", "FlowControlMode", "PicType", "CardId", "CardType", "AdminState", "EqActionType", "PortMode", "UnknownState", "FirmwareSource", "PrimaryState", "SecondaryState", "ShelfId", "PortId", "PortDetectedMode", "CardSubType")
...
RowStatus, TextualConvention, DateAndTime, TimeStamp, MacAddress, DisplayString, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "DateAndTime", "TimeStamp", "MacAddress", "DisplayString", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portDocsIfIndex=portDocsIfIndex, auditTime=auditTime, cardCpuSpeed=cardCpuSpeed, auditShelfId=auditShelfId, auditFailedCount=auditFailedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestEntry=cardTestEntry, TestType=TestType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTemperatureHighError=cardTemperatureHighError, portDescription=portDescription, cardRamSize=cardRamSize, errEvLogLevel=errEvLogLevel, portSecStateChange=portSecStateChange, cardHwVersion=cardHwVersion, diskVolumeTrapInh=diskVolumeTrapInh, portMaxDsPower=portMaxDsPower, portCardId=portCardId, TestSchedule=TestSchedule, portConnectorId=portConnectorId, cardSubType=cardSubType, errEventId=errEventId, cardLastBootVersion=cardLastBootVersion, systemGeneralGroup=systemGeneralGroup, cardSwVersion=cardSwVersion, cardFlashSize=cardFlashSize, cardPicDetected=cardPicDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentState=equipmentState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogThrottle=auditLogThrottle, shelfObjects=shelfObjects, auditLogOutput=auditLogOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, TestId=TestId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditTotalFailedCount=auditTotalFailedCount, cmIpAddress=cmIpAddress, auditResult=auditResult, equipmentPortGroup=equipmentPortGroup, systemGeneral=systemGeneral, portMode=portMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTable=cardTable, cmPrimarySID=cmPrimarySID, trapCounter=trapCounter, cardSpareGroupMode=cardSpareGroupMode, cardShelfId=cardShelfId, cardSecState=cardSecState, cardTestId=cardTestId, portPrStateChange=portPrStateChange, TestCommand=TestCommand, TestScheduleCommand=TestScheduleCommand, auditAutoScheduling=auditAutoScheduling, equipmentDiag=equipmentDiag, portNumber=portNumber, cardTestTable=cardTestTable, auditCycleCount=auditCycleCount, portDplxStatusChange=portDplxStatusChange, cardTestScheduleCommand=cardTestScheduleCommand, PYSNMP_MODULE_ID=cadEquipmentMib, errEvRecoveryEnabled=errEvRecoveryEnabled, cardAction=cardAction, cardCpuType=cardCpuType, auditCardId=auditCardId, cardPrStateChange=cardPrStateChange, cardPicHwVersion=cardPicHwVersion, diskVolumeCardId=diskVolumeCardId, portDplxStatus=portDplxStatus, cardTestName=cardTestName, cardNumPorts=cardNumPorts, fpgaErrorEventEntry=fpgaErrorEventEntry, cardLastChangeTime=cardLastChangeTime, cardTestDescription=cardTestDescription, portCardSubType=portCardSubType, portRxFlowControlDetected=portRxFlowControlDetected, cmResetUpTime=cmResetUpTime, cardCarrierSerialNum=cardCarrierSerialNum, cardTestIndex=cardTestIndex, cardFwUpdateStatus=cardFwUpdateStatus, cardDetected=cardDetected, cardTestCommand=cardTestCommand, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeNumber=diskVolumeNumber, cardTemperature=cardTemperature, cadEquipmentMib=cadEquipmentMib, cardLastBootSource=cardLastBootSource, diskVolumeTable=diskVolumeTable, cardId=cardId, cardFwVersion=cardFwVersion, cardName=cardName, portDetectedMode=portDetectedMode, cardTemperatureHighWarn=cardTemperatureHighWarn, portTxFlowControlDetected=portTxFlowControlDetected, portId=portId, portAdminState=portAdminState, cmMacAddress=cmMacAddress, cardNumber=cardNumber, diskVolumeDriveId=diskVolumeDriveId, cmResetClearNotification=cmResetClearNotification, cardTestTime=cardTestTime, cmUChannelID=cmUChannelID, cmDevice=cmDevice, systemKey=systemKey, cardFpgaSource=cardFpgaSource, auditId=auditId, cardLastResetReason=cardLastResetReason, auditStatus=auditStatus, cmResetReason=cmResetReason, cardTestResultNotification=cardTestResultNotification, cmResetNotification=cmResetNotification, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetStatus=cmResetStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, portMacIfIndex=portMacIfIndex, cardTrapInh=cardTrapInh, cardTestTransId=cardTestTransId, verboseLevel=verboseLevel, systemClock=systemClock, auditDescription=auditDescription, auditCommand=auditCommand, consoleOutput=consoleOutput, cardDplxStatus=cardDplxStatus, cardTestSchedule=cardTestSchedule, cardDplxStatusChange=cardDplxStatusChange, portNumChans=portNumChans, cardPicSerialNum=cardPicSerialNum, equipmentAudit=equipmentAudit, portCurrDsPower=portCurrDsPower, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestTransactionId=TestTransactionId, portRxFlowControlMode=portRxFlowControlMode, auditName=auditName, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTempNoReportNotification=cardTempNoReportNotification, portEntry=portEntry, diagTestId=diagTestId, shelfSwVersion=shelfSwVersion, cardAdminState=cardAdminState, cardTestResultDesc=cardTestResultDesc, portType=portType, cmVendor=cmVendor, portSecState=portSecState, shelfName=shelfName, cardUpTime=cardUpTime, cardTestResult=cardTestResult, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentStateGroup=equipmentStateGroup, cardSubDetected=cardSubDetected, equipmentError=equipmentError, cardDetectedChange=cardDetectedChange, cardShelfTestIndex=cardShelfTestIndex, auditPassedCount=auditPassedCount, cardType=cardType, auditEntry=auditEntry, cardSecStateChange=cardSecStateChange, downstreamPowerLoss=downstreamPowerLoss, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestType=cardTestType, portTxFlowControlMode=portTxFlowControlMode, portMacAddress=portMacAddress, errEvRowStatus=errEvRowStatus, equipmentDiagGroup=equipmentDiagGroup, cardBusSpeed=cardBusSpeed, cardSerialNum=cardSerialNum, equipmentShelfGroup=equipmentShelfGroup, cardEntry=cardEntry, diskVolumeDiskSize=diskVolumeDiskSize, diskDriveNumber=diskDriveNumber, equipmentTraps=equipmentTraps, cardBootVersion=cardBootVersion, portAction=portAction, portTrapInh=portTrapInh, diskVolumeEntry=diskVolumeEntry, portMinDsPower=portMinDsPower, portPrState=portPrState, diskVolumeId=diskVolumeId, trapSeverity=trapSeverity, portTable=portTable, auditTotalPassedCount=auditTotalPassedCount, cardCPLDVersion=cardCPLDVersion, remainInDiagMode=remainInDiagMode, diskVolumeFileName=diskVolumeFileName, cmResetInfo=cmResetInfo, shelfNumber=shelfNumber, TestResult=TestResult, diskVolumeShelfId=diskVolumeShelfId, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeGroup=diskVolumeGroup, auditTable=auditTable, diskVolumeName=diskVolumeName, portShelfId=portShelfId, diskVolumeSize=diskVolumeSize, cadEquipmentMibGroup=cadEquipmentMibGroup, portLastChangeTime=portLastChangeTime, eqDiagConfig=eqDiagConfig, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCarrierHwVersion=cardCarrierHwVersion, equipmentTbl=equipmentTbl, cardSpareGroupId=cardSpareGroupId, cmRegistrationNotification=cmRegistrationNotification, portBgpId=portBgpId, equipmentCardGroup=equipmentCardGroup, cardPrState=cardPrState, cardCarrierFwVersion=cardCarrierFwVersion)
