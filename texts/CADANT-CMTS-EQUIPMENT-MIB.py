...
PortId, CardSubType, EqActionType, PrimaryState, AdminState, CardType, PicType, PortType, CardId, DiskVolumeUsageLevel, FlowControlMode, FirmwareSource, PortDetectedMode, ShelfId, UnknownState, SecondaryState, DuplexStatus, PortMode = mibBuilder.importSymbols("CADANT-TC", "PortId", "CardSubType", "EqActionType", "PrimaryState", "AdminState", "CardType", "PicType", "PortType", "CardId", "DiskVolumeUsageLevel", "FlowControlMode", "FirmwareSource", "PortDetectedMode", "ShelfId", "UnknownState", "SecondaryState", "DuplexStatus", "PortMode")
...
DisplayString, RowStatus, TruthValue, MacAddress, DateAndTime, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "RowStatus", "TruthValue", "MacAddress", "DateAndTime", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardNumPorts=cardNumPorts, cardHwVersion=cardHwVersion, shelfSwVersion=shelfSwVersion, auditPassedCount=auditPassedCount, cardRamSize=cardRamSize, cardCpuSpeed=cardCpuSpeed, equipmentAudit=equipmentAudit, auditTotalFailedCount=auditTotalFailedCount, cardPicHwVersion=cardPicHwVersion, portAdminState=portAdminState, equipmentAuditGroup=equipmentAuditGroup, TestResult=TestResult, auditLogThrottle=auditLogThrottle, equipmentTraps=equipmentTraps, cardDplxStatusChange=cardDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, portTrapInh=portTrapInh, cardTestDescription=cardTestDescription, cardLastChangeTime=cardLastChangeTime, portTxFlowControlDetected=portTxFlowControlDetected, cmResetUpTime=cmResetUpTime, cardCpuType=cardCpuType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portLastChangeTime=portLastChangeTime, diskVolumeShelfId=diskVolumeShelfId, equipmentShelfGroup=equipmentShelfGroup, portDescription=portDescription, cardTemperatureHighError=cardTemperatureHighError, eqDiagConfig=eqDiagConfig, portMaxDsPower=portMaxDsPower, cardSubType=cardSubType, diskVolumeUsageLevel=diskVolumeUsageLevel, portCardSubType=portCardSubType, portMacAddress=portMacAddress, equipmentCardTestGroup=equipmentCardTestGroup, equipmentStateGroup=equipmentStateGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetStatus=cmResetStatus, cardTestTransId=cardTestTransId, systemKey=systemKey, cardSerialNum=cardSerialNum, trapSeverity=trapSeverity, cardPrStateChange=cardPrStateChange, auditId=auditId, cardTestResultDesc=cardTestResultDesc, portMode=portMode, cardShelfId=cardShelfId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSpareGroupId=cardSpareGroupId, cardSpareGroupMode=cardSpareGroupMode, equipmentPortGroup=equipmentPortGroup, diskVolumeGroup=diskVolumeGroup, cardLastResetReason=cardLastResetReason, cadEquipmentMibConformance=cadEquipmentMibConformance, cardAdminState=cardAdminState, cardTestCommand=cardTestCommand, portDocsIfIndex=portDocsIfIndex, downstreamPowerLoss=downstreamPowerLoss, cardCarrierFwVersion=cardCarrierFwVersion, equipmentDiagGroup=equipmentDiagGroup, cardCarrierSerialNum=cardCarrierSerialNum, auditDescription=auditDescription, cardTrapInh=cardTrapInh, systemGeneral=systemGeneral, cardPicDetected=cardPicDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, TestId=TestId, cmUChannelID=cmUChannelID, equipmentState=equipmentState, portDplxStatus=portDplxStatus, cardPicSerialNum=cardPicSerialNum, portBgpId=portBgpId, trapCounter=trapCounter, equipmentError=equipmentError, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDplxStatusChange=portDplxStatusChange, portShelfId=portShelfId, diskVolumeNumber=diskVolumeNumber, cardBootVersion=cardBootVersion, cardSecStateChange=cardSecStateChange, cadEquipmentMib=cadEquipmentMib, shelfName=shelfName, cardPrState=cardPrState, cmDevice=cmDevice, cardCarrierHwVersion=cardCarrierHwVersion, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditLogOutput=auditLogOutput, portCurrDsPower=portCurrDsPower, portTxFlowControlMode=portTxFlowControlMode, TestCommand=TestCommand, auditAutoScheduling=auditAutoScheduling, portMinDsPower=portMinDsPower, cardCPLDVersion=cardCPLDVersion, portNumber=portNumber, portId=portId, errEvLoggingEnabled=errEvLoggingEnabled, portSecStateChange=portSecStateChange, portCardId=portCardId, portPrState=portPrState, portRxFlowControlDetected=portRxFlowControlDetected, auditTime=auditTime, diskVolumeSize=diskVolumeSize, cardFwVersion=cardFwVersion, cardSubDetected=cardSubDetected, cardTestTable=cardTestTable, cmResetClearNotification=cmResetClearNotification, auditName=auditName, cardUpTime=cardUpTime, diskVolumeDriveId=diskVolumeDriveId, cardTestIndex=cardTestIndex, TestTransactionId=TestTransactionId, cardId=cardId, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeId=diskVolumeId, cmRegistrationNotification=cmRegistrationNotification, remainInDiagMode=remainInDiagMode, portPrStateChange=portPrStateChange, cardTestType=cardTestType, auditTable=auditTable, errEventId=errEventId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentDiag=equipmentDiag, fpgaErrorEventTable=fpgaErrorEventTable, shelfNumber=shelfNumber, diskVolumeCardId=diskVolumeCardId, cmResetInfo=cmResetInfo, fpgaErrorEventEntry=fpgaErrorEventEntry, fpgaErrorEventGroup=fpgaErrorEventGroup, portConnectorId=portConnectorId, portDetectedMode=portDetectedMode, cmResetNotification=cmResetNotification, portMacIfIndex=portMacIfIndex, equipmentCardGroup=equipmentCardGroup, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmMacAddress=cmMacAddress, cardTestSchedule=cardTestSchedule, diskVolumeDiskSize=diskVolumeDiskSize, cardShelfTestIndex=cardShelfTestIndex, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSwVersion=cardSwVersion, cardDetected=cardDetected, portSecState=portSecState, diskVolumeName=diskVolumeName, consoleOutput=consoleOutput, cmVendor=cmVendor, cardTestResult=cardTestResult, diskVolumeFileName=diskVolumeFileName, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmIpAddress=cmIpAddress, cmPrimarySID=cmPrimarySID, portRxFlowControlMode=portRxFlowControlMode, cardTable=cardTable, diskVolumeTable=diskVolumeTable, equipmentTbl=equipmentTbl, diskDriveNumber=diskDriveNumber, cardName=cardName, shelfObjects=shelfObjects, cardDetectedChange=cardDetectedChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portEntry=portEntry, cardTestId=cardTestId, cardType=cardType, cardSecState=cardSecState, cardTestScheduleCommand=cardTestScheduleCommand, cardFpgaSource=cardFpgaSource, systemClock=systemClock, cardLastBootSource=cardLastBootSource, verboseLevel=verboseLevel, diskVolumeEntry=diskVolumeEntry, auditStatus=auditStatus, cardTemperature=cardTemperature, cardTestEntry=cardTestEntry, auditCycleCount=auditCycleCount, errEvLogLevel=errEvLogLevel, cardBusSpeed=cardBusSpeed, errEvRowStatus=errEvRowStatus, cmResetReason=cmResetReason, portAction=portAction, portNumChans=portNumChans, TestSchedule=TestSchedule, auditResult=auditResult, cardEntry=cardEntry, cardAction=cardAction, cardDplxStatus=cardDplxStatus, portTable=portTable, TestType=TestType, cardTestTime=cardTestTime, portType=portType, systemGeneralGroup=systemGeneralGroup, auditCommand=auditCommand, auditCardId=auditCardId, TestScheduleCommand=TestScheduleCommand, cardTestName=cardTestName, cardFlashSize=cardFlashSize, diagTestId=diagTestId, cardNumber=cardNumber, cardTempNoReportNotification=cardTempNoReportNotification, auditFailedCount=auditFailedCount, diskVolumeTrapInh=diskVolumeTrapInh, cardLastBootVersion=cardLastBootVersion, cardTestResultNotification=cardTestResultNotification, auditShelfId=auditShelfId, auditEntry=auditEntry)
