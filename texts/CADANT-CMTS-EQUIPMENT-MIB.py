...
CardType, DuplexStatus, CardId, PicType, PortType, PrimaryState, PortId, DiskVolumeUsageLevel, CardSubType, EqActionType, SecondaryState, UnknownState, FlowControlMode, AdminState, ShelfId, PortDetectedMode, FirmwareSource, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardType", "DuplexStatus", "CardId", "PicType", "PortType", "PrimaryState", "PortId", "DiskVolumeUsageLevel", "CardSubType", "EqActionType", "SecondaryState", "UnknownState", "FlowControlMode", "AdminState", "ShelfId", "PortDetectedMode", "FirmwareSource", "PortMode")
...
RowStatus, DisplayString, TimeStamp, MacAddress, TextualConvention, DateAndTime, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "TimeStamp", "MacAddress", "TextualConvention", "DateAndTime", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardLastResetReason=cardLastResetReason, TestResult=TestResult, cardDplxStatusChange=cardDplxStatusChange, cardTestId=cardTestId, cardDetectedChange=cardDetectedChange, cardShelfTestIndex=cardShelfTestIndex, cardName=cardName, portEntry=portEntry, portRxFlowControlMode=portRxFlowControlMode, cardNumber=cardNumber, diskVolumeSize=diskVolumeSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeGroup=diskVolumeGroup, auditId=auditId, fpgaErrorEventTable=fpgaErrorEventTable, cardPrState=cardPrState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portTable=portTable, cmPrimarySID=cmPrimarySID, cardAction=cardAction, cardPicSerialNum=cardPicSerialNum, equipmentCardGroup=equipmentCardGroup, cardRamSize=cardRamSize, cardDetected=cardDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, cardUpTime=cardUpTime, TestId=TestId, cardLastChangeTime=cardLastChangeTime, cardCpuSpeed=cardCpuSpeed, portLastChangeTime=portLastChangeTime, errEvLogLevel=errEvLogLevel, diskDriveNumber=diskDriveNumber, cardTempNoReportNotification=cardTempNoReportNotification, cardFwVersion=cardFwVersion, portCardSubType=portCardSubType, diskVolumeFileName=diskVolumeFileName, portPrState=portPrState, remainInDiagMode=remainInDiagMode, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmUChannelID=cmUChannelID, cardTestScheduleCommand=cardTestScheduleCommand, equipmentCardTestGroup=equipmentCardTestGroup, cardSpareGroupId=cardSpareGroupId, systemGeneral=systemGeneral, TestTransactionId=TestTransactionId, shelfObjects=shelfObjects, portConnectorId=portConnectorId, diskVolumeCardId=diskVolumeCardId, equipmentShelfGroup=equipmentShelfGroup, cardTestName=cardTestName, auditPassedCount=auditPassedCount, diskVolumeNumber=diskVolumeNumber, verboseLevel=verboseLevel, auditDescription=auditDescription, consoleOutput=consoleOutput, cardTestTable=cardTestTable, auditCycleCount=auditCycleCount, errEventId=errEventId, cardTestDescription=cardTestDescription, trapCounter=trapCounter, cardNumPorts=cardNumPorts, portTrapInh=portTrapInh, equipmentAudit=equipmentAudit, auditShelfId=auditShelfId, cmMacAddress=cmMacAddress, systemGeneralGroup=systemGeneralGroup, cardSubDetected=cardSubDetected, cardTrapInh=cardTrapInh, cardCarrierHwVersion=cardCarrierHwVersion, portRxFlowControlDetected=portRxFlowControlDetected, cmResetUpTime=cmResetUpTime, portDplxStatusChange=portDplxStatusChange, diskVolumeDiskSize=diskVolumeDiskSize, cardTable=cardTable, cardCPLDVersion=cardCPLDVersion, TestType=TestType, auditResult=auditResult, cadEquipmentMibConformance=cadEquipmentMibConformance, portMacAddress=portMacAddress, auditTable=auditTable, cmIpAddress=cmIpAddress, portMinDsPower=portMinDsPower, cardTemperatureHighWarn=cardTemperatureHighWarn, portCardId=portCardId, diskVolumeTrapInh=diskVolumeTrapInh, cardTestIndex=cardTestIndex, cardTestType=cardTestType, auditLogThrottle=auditLogThrottle, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestSchedule=cardTestSchedule, equipmentDiagGroup=equipmentDiagGroup, portDetectedMode=portDetectedMode, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFpgaSource=cardFpgaSource, errEvRowStatus=errEvRowStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, cardEntry=cardEntry, portMacIfIndex=portMacIfIndex, cardAdminState=cardAdminState, auditStatus=auditStatus, cmDevice=cmDevice, portId=portId, cardTestEntry=cardTestEntry, portPrStateChange=portPrStateChange, auditTotalPassedCount=auditTotalPassedCount, auditLogOutput=auditLogOutput, shelfName=shelfName, cardPicHwVersion=cardPicHwVersion, cardTestCommand=cardTestCommand, cardSecState=cardSecState, cmVendor=cmVendor, cardFlashSize=cardFlashSize, equipmentAuditGroup=equipmentAuditGroup, portDocsIfIndex=portDocsIfIndex, diskVolumeDriveId=diskVolumeDriveId, cardSwVersion=cardSwVersion, trapSeverity=trapSeverity, cardType=cardType, auditAutoScheduling=auditAutoScheduling, cardShelfId=cardShelfId, cmResetReason=cmResetReason, equipmentState=equipmentState, portDplxStatus=portDplxStatus, cardTestResultDesc=cardTestResultDesc, PYSNMP_MODULE_ID=cadEquipmentMib, portSecStateChange=portSecStateChange, portNumChans=portNumChans, portMaxDsPower=portMaxDsPower, portTxFlowControlMode=portTxFlowControlMode, cardTestResult=cardTestResult, cmResetClearNotification=cmResetClearNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, diagTestId=diagTestId, cardTemperature=cardTemperature, cmResetNotification=cmResetNotification, cardId=cardId, portAction=portAction, portAdminState=portAdminState, portShelfId=portShelfId, portType=portType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardSecStateChange=cardSecStateChange, portSecState=portSecState, auditCardId=auditCardId, portMode=portMode, cardCarrierSerialNum=cardCarrierSerialNum, cardDplxStatus=cardDplxStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, auditEntry=auditEntry, TestScheduleCommand=TestScheduleCommand, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, shelfNumber=shelfNumber, cmResetStatus=cmResetStatus, cardCarrierFwVersion=cardCarrierFwVersion, cardSerialNum=cardSerialNum, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditName=auditName, cmRegistrationNotification=cmRegistrationNotification, portTxFlowControlDetected=portTxFlowControlDetected, systemKey=systemKey, cmResetInfo=cmResetInfo, portDescription=portDescription, cardPrStateChange=cardPrStateChange, eqDiagConfig=eqDiagConfig, diskVolumeId=diskVolumeId, diskVolumeTable=diskVolumeTable, diskVolumeName=diskVolumeName, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardBusSpeed=cardBusSpeed, systemClock=systemClock, equipmentTbl=equipmentTbl, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMib=cadEquipmentMib, TestCommand=TestCommand, auditCommand=auditCommand, cardPicDetected=cardPicDetected, equipmentError=equipmentError, equipmentTraps=equipmentTraps, cardBootVersion=cardBootVersion, cardLastBootSource=cardLastBootSource, portCurrDsPower=portCurrDsPower, cardCpuType=cardCpuType, errEvLoggingEnabled=errEvLoggingEnabled, cardTestTransId=cardTestTransId, cardFwUpdateStatus=cardFwUpdateStatus, cardHwVersion=cardHwVersion, equipmentPortGroup=equipmentPortGroup, equipmentDiag=equipmentDiag, cardTemperatureHighError=cardTemperatureHighError, cardTestTime=cardTestTime, shelfSwVersion=shelfSwVersion, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResultNotification=cardTestResultNotification, diskVolumeEntry=diskVolumeEntry, cardSpareGroupMode=cardSpareGroupMode, portBgpId=portBgpId, auditFailedCount=auditFailedCount, auditTime=auditTime, equipmentStateGroup=equipmentStateGroup, TestSchedule=TestSchedule, portNumber=portNumber, cardSubType=cardSubType)
