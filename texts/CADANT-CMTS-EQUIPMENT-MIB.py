...
FlowControlMode, SecondaryState, DiskVolumeUsageLevel, CardType, CardSubType, PicType, PortType, PrimaryState, ShelfId, PortId, PortMode, PortDetectedMode, FirmwareSource, AdminState, UnknownState, DuplexStatus, CardId, EqActionType = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "SecondaryState", "DiskVolumeUsageLevel", "CardType", "CardSubType", "PicType", "PortType", "PrimaryState", "ShelfId", "PortId", "PortMode", "PortDetectedMode", "FirmwareSource", "AdminState", "UnknownState", "DuplexStatus", "CardId", "EqActionType")
...
TextualConvention, DateAndTime, DisplayString, MacAddress, TimeStamp, RowStatus, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "DisplayString", "MacAddress", "TimeStamp", "RowStatus", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditShelfId=auditShelfId, eqDiagConfig=eqDiagConfig, diskVolumeId=diskVolumeId, diskVolumeGroup=diskVolumeGroup, cardCarrierSerialNum=cardCarrierSerialNum, fpgaErrorEventEntry=fpgaErrorEventEntry, errEvLogLevel=errEvLogLevel, fpgaErrorEventTable=fpgaErrorEventTable, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTemperatureHighError=cardTemperatureHighError, equipmentStateGroup=equipmentStateGroup, cardTemperature=cardTemperature, errEvRowStatus=errEvRowStatus, cardTestDescription=cardTestDescription, cardTestTable=cardTestTable, portPrStateChange=portPrStateChange, portDetectedMode=portDetectedMode, trapCounter=trapCounter, cardPicSerialNum=cardPicSerialNum, cardAdminState=cardAdminState, cmRegistrationNotification=cmRegistrationNotification, cardTestIndex=cardTestIndex, equipmentDiag=equipmentDiag, equipmentDiagGroup=equipmentDiagGroup, cardTestId=cardTestId, cardFlashSize=cardFlashSize, cardPrState=cardPrState, cardDetected=cardDetected, cardRamSize=cardRamSize, cardTestResult=cardTestResult, cardUpTime=cardUpTime, portDplxStatus=portDplxStatus, diskVolumeTrapInh=diskVolumeTrapInh, portDplxStatusChange=portDplxStatusChange, portDocsIfIndex=portDocsIfIndex, cardLastChangeTime=cardLastChangeTime, auditAutoScheduling=auditAutoScheduling, cardTestType=cardTestType, trapSeverity=trapSeverity, auditResult=auditResult, cmPrimarySID=cmPrimarySID, diskVolumeCardId=diskVolumeCardId, cardSpareGroupMode=cardSpareGroupMode, portMinDsPower=portMinDsPower, diskVolumeTable=diskVolumeTable, diskVolumeFileName=diskVolumeFileName, cardTable=cardTable, cmResetNotification=cmResetNotification, cardSubDetected=cardSubDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, cmResetReason=cmResetReason, cardType=cardType, cardSerialNum=cardSerialNum, errEventId=errEventId, portMode=portMode, cardBusSpeed=cardBusSpeed, equipmentState=equipmentState, auditTotalFailedCount=auditTotalFailedCount, cardTestEntry=cardTestEntry, portSecState=portSecState, equipmentError=equipmentError, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentTbl=equipmentTbl, cardFwVersion=cardFwVersion, TestTransactionId=TestTransactionId, cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestResult=TestResult, portEntry=portEntry, systemClock=systemClock, portNumChans=portNumChans, auditEntry=auditEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestId=TestId, equipmentCardTestGroup=equipmentCardTestGroup, portConnectorId=portConnectorId, shelfObjects=shelfObjects, cardFpgaSource=cardFpgaSource, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portRxFlowControlDetected=portRxFlowControlDetected, portLastChangeTime=portLastChangeTime, portTxFlowControlDetected=portTxFlowControlDetected, portMaxDsPower=portMaxDsPower, auditCommand=auditCommand, cardSwVersion=cardSwVersion, auditLogOutput=auditLogOutput, equipmentAuditGroup=equipmentAuditGroup, cmVendor=cmVendor, cardDplxStatus=cardDplxStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetUpTime=cmResetUpTime, auditId=auditId, cardTestScheduleCommand=cardTestScheduleCommand, portPrState=portPrState, cardTestResultDesc=cardTestResultDesc, equipmentAudit=equipmentAudit, portCardId=portCardId, auditTable=auditTable, portType=portType, equipmentShelfGroup=equipmentShelfGroup, systemGeneral=systemGeneral, cardCpuType=cardCpuType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardEntry=cardEntry, cardName=cardName, cardLastBootVersion=cardLastBootVersion, systemGeneralGroup=systemGeneralGroup, portRxFlowControlMode=portRxFlowControlMode, cardTestCommand=cardTestCommand, diskVolumeSize=diskVolumeSize, portCurrDsPower=portCurrDsPower, diskVolumeEntry=diskVolumeEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, cmUChannelID=cmUChannelID, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeDriveId=diskVolumeDriveId, diskVolumeName=diskVolumeName, cardDplxStatusChange=cardDplxStatusChange, TestScheduleCommand=TestScheduleCommand, auditCycleCount=auditCycleCount, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetStatus=cmResetStatus, cardSpareGroupId=cardSpareGroupId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestSchedule=TestSchedule, cadEquipmentMib=cadEquipmentMib, cardTestSchedule=cardTestSchedule, cardSubType=cardSubType, cardHwVersion=cardHwVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, portAction=portAction, cardCPLDVersion=cardCPLDVersion, cmMacAddress=cmMacAddress, cardSecStateChange=cardSecStateChange, cardCpuSpeed=cardCpuSpeed, equipmentCardGroup=equipmentCardGroup, cardShelfId=cardShelfId, auditStatus=auditStatus, auditTotalPassedCount=auditTotalPassedCount, cardNumber=cardNumber, diskVolumeShelfId=diskVolumeShelfId, cmIpAddress=cmIpAddress, cardTestName=cardTestName, TestType=TestType, cardCarrierFwVersion=cardCarrierFwVersion, shelfNumber=shelfNumber, cardTrapInh=cardTrapInh, cmResetInfo=cmResetInfo, cardLastBootSource=cardLastBootSource, auditTime=auditTime, cardTestTime=cardTestTime, portDescription=portDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, portMacAddress=portMacAddress, cardDetectedChange=cardDetectedChange, portSecStateChange=portSecStateChange, cardAction=cardAction, portBgpId=portBgpId, portId=portId, systemKey=systemKey, shelfName=shelfName, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeNumber=diskVolumeNumber, cardPrStateChange=cardPrStateChange, cardLastResetReason=cardLastResetReason, verboseLevel=verboseLevel, cardTempNoReportNotification=cardTempNoReportNotification, diagTestId=diagTestId, auditPassedCount=auditPassedCount, portTxFlowControlMode=portTxFlowControlMode, cardTestResultNotification=cardTestResultNotification, auditLogThrottle=auditLogThrottle, auditCardId=auditCardId, errEvRecoveryEnabled=errEvRecoveryEnabled, downstreamPowerLoss=downstreamPowerLoss, equipmentPortGroup=equipmentPortGroup, portTable=portTable, cardNumPorts=cardNumPorts, consoleOutput=consoleOutput, auditName=auditName, portMacIfIndex=portMacIfIndex, cmDevice=cmDevice, portTrapInh=portTrapInh, cardPicHwVersion=cardPicHwVersion, portCardSubType=portCardSubType, remainInDiagMode=remainInDiagMode, cardTestTransId=cardTestTransId, cardCarrierHwVersion=cardCarrierHwVersion, shelfSwVersion=shelfSwVersion, cardPicDetected=cardPicDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, portShelfId=portShelfId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, TestCommand=TestCommand, diskDriveNumber=diskDriveNumber, auditDescription=auditDescription, cmResetClearNotification=cmResetClearNotification, cardBootVersion=cardBootVersion, cardSecState=cardSecState, equipmentTraps=equipmentTraps, portNumber=portNumber, cardId=cardId, portAdminState=portAdminState, auditFailedCount=auditFailedCount)
