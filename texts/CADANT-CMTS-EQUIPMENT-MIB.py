...
PrimaryState, PortType, PortId, PortMode, FirmwareSource, AdminState, CardSubType, PicType, CardId, DiskVolumeUsageLevel, FlowControlMode, CardType, PortDetectedMode, ShelfId, UnknownState, SecondaryState, DuplexStatus, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "PortType", "PortId", "PortMode", "FirmwareSource", "AdminState", "CardSubType", "PicType", "CardId", "DiskVolumeUsageLevel", "FlowControlMode", "CardType", "PortDetectedMode", "ShelfId", "UnknownState", "SecondaryState", "DuplexStatus", "EqActionType")
...
RowStatus, TimeStamp, MacAddress, TextualConvention, DisplayString, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "MacAddress", "TextualConvention", "DisplayString", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", systemGeneralGroup=systemGeneralGroup, cardType=cardType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditLogThrottle=auditLogThrottle, cardTable=cardTable, diskVolumeTrapInh=diskVolumeTrapInh, TestResult=TestResult, portPrStateChange=portPrStateChange, diskVolumeCardId=diskVolumeCardId, cardSwVersion=cardSwVersion, auditTotalPassedCount=auditTotalPassedCount, cardTestTime=cardTestTime, cardTestType=cardTestType, cardAction=cardAction, diskVolumeDiskSize=diskVolumeDiskSize, cardDplxStatus=cardDplxStatus, remainInDiagMode=remainInDiagMode, cardDetected=cardDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardAdminState=cardAdminState, diskVolumeFileName=diskVolumeFileName, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmUChannelID=cmUChannelID, auditTime=auditTime, shelfSwVersion=shelfSwVersion, trapCounter=trapCounter, auditCycleCount=auditCycleCount, equipmentStateGroup=equipmentStateGroup, cmPrimarySID=cmPrimarySID, cardSecStateChange=cardSecStateChange, cardTestTable=cardTestTable, equipmentError=equipmentError, cmResetInfo=cmResetInfo, portPrState=portPrState, equipmentCardGroup=equipmentCardGroup, portMacAddress=portMacAddress, cadEquipmentMib=cadEquipmentMib, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfNumber=shelfNumber, cardFwUpdateStatus=cardFwUpdateStatus, portDocsIfIndex=portDocsIfIndex, auditId=auditId, cardSecState=cardSecState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardLastBootSource=cardLastBootSource, cmVendor=cmVendor, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeId=diskVolumeId, auditTotalFailedCount=auditTotalFailedCount, TestId=TestId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetStatus=cmResetStatus, portCurrDsPower=portCurrDsPower, equipmentDiag=equipmentDiag, cmRegistrationNotification=cmRegistrationNotification, cardLastBootVersion=cardLastBootVersion, errEvLoggingEnabled=errEvLoggingEnabled, portSecState=portSecState, cardSpareGroupId=cardSpareGroupId, cardTestTransId=cardTestTransId, TestCommand=TestCommand, portEntry=portEntry, cmMacAddress=cmMacAddress, auditResult=auditResult, portTxFlowControlMode=portTxFlowControlMode, portRxFlowControlDetected=portRxFlowControlDetected, TestScheduleCommand=TestScheduleCommand, portConnectorId=portConnectorId, systemGeneral=systemGeneral, cardSerialNum=cardSerialNum, cardTestSchedule=cardTestSchedule, auditName=auditName, consoleOutput=consoleOutput, portDplxStatus=portDplxStatus, cardDetectedChange=cardDetectedChange, cardPrState=cardPrState, cardCPLDVersion=cardCPLDVersion, diskVolumeEntry=diskVolumeEntry, portDplxStatusChange=portDplxStatusChange, cardTempOverHeatNotification=cardTempOverHeatNotification, portMinDsPower=portMinDsPower, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfName=shelfName, cardCarrierHwVersion=cardCarrierHwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetClearNotification=cmResetClearNotification, diskVolumeTable=diskVolumeTable, portCardSubType=portCardSubType, equipmentAudit=equipmentAudit, cardTestEntry=cardTestEntry, cardFlashSize=cardFlashSize, cmIpAddress=cmIpAddress, cardNumber=cardNumber, cardTemperature=cardTemperature, cardPicDetected=cardPicDetected, cardTestIndex=cardTestIndex, cardLastResetReason=cardLastResetReason, equipmentAuditGroup=equipmentAuditGroup, trapSeverity=trapSeverity, cardCarrierSerialNum=cardCarrierSerialNum, errEvRowStatus=errEvRowStatus, portDetectedMode=portDetectedMode, diskVolumeName=diskVolumeName, cardFwVersion=cardFwVersion, auditAutoScheduling=auditAutoScheduling, cardTrapInh=cardTrapInh, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSubType=cardSubType, portMode=portMode, portTable=portTable, auditLogOutput=auditLogOutput, verboseLevel=verboseLevel, auditCommand=auditCommand, cmResetUpTime=cmResetUpTime, TestTransactionId=TestTransactionId, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeNumber=diskVolumeNumber, systemClock=systemClock, cardNumPorts=cardNumPorts, equipmentState=equipmentState, diskVolumeSize=diskVolumeSize, cardBusSpeed=cardBusSpeed, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentTraps=equipmentTraps, portSecStateChange=portSecStateChange, cardUpTime=cardUpTime, auditShelfId=auditShelfId, cardBootVersion=cardBootVersion, portTxFlowControlDetected=portTxFlowControlDetected, portRxFlowControlMode=portRxFlowControlMode, cardShelfTestIndex=cardShelfTestIndex, cardFpgaSource=cardFpgaSource, portType=portType, eqDiagConfig=eqDiagConfig, equipmentShelfGroup=equipmentShelfGroup, portDescription=portDescription, cardEntry=cardEntry, cardRamSize=cardRamSize, auditCardId=auditCardId, cardTestDescription=cardTestDescription, cardTestCommand=cardTestCommand, portId=portId, cardShelfId=cardShelfId, cardHwVersion=cardHwVersion, shelfObjects=shelfObjects, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestName=cardTestName, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCarrierFwVersion=cardCarrierFwVersion, cardPicSerialNum=cardPicSerialNum, portAction=portAction, auditFailedCount=auditFailedCount, auditTable=auditTable, auditDescription=auditDescription, TestType=TestType, errEventId=errEventId, errEvLogLevel=errEvLogLevel, diskVolumeDriveId=diskVolumeDriveId, portCardId=portCardId, auditEntry=auditEntry, cmDevice=cmDevice, equipmentPortGroup=equipmentPortGroup, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighError=cardTemperatureHighError, cardName=cardName, cardCpuType=cardCpuType, portBgpId=portBgpId, auditPassedCount=auditPassedCount, cardId=cardId, diskVolumeGroup=diskVolumeGroup, cardLastChangeTime=cardLastChangeTime, portMaxDsPower=portMaxDsPower, equipmentCardTestGroup=equipmentCardTestGroup, cmResetReason=cmResetReason, portLastChangeTime=portLastChangeTime, errEvRecoveryEnabled=errEvRecoveryEnabled, diskDriveNumber=diskDriveNumber, portNumChans=portNumChans, TestSchedule=TestSchedule, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicHwVersion=cardPicHwVersion, cmResetNotification=cmResetNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentTbl=equipmentTbl, portMacIfIndex=portMacIfIndex, cardCpuSpeed=cardCpuSpeed, auditStatus=auditStatus, portTrapInh=portTrapInh, cardSubDetected=cardSubDetected, systemKey=systemKey, cardTestId=cardTestId, cardDplxStatusChange=cardDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, cardTestScheduleCommand=cardTestScheduleCommand, portNumber=portNumber, portAdminState=portAdminState, diagTestId=diagTestId, cardPrStateChange=cardPrStateChange, downstreamPowerLoss=downstreamPowerLoss, cardTestResultDesc=cardTestResultDesc, cardTempNoReportNotification=cardTempNoReportNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portShelfId=portShelfId, cardTestResult=cardTestResult)
