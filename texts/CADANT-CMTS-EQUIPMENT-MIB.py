...
DuplexStatus, ShelfId, PortType, EqActionType, CardType, AdminState, PicType, PortId, PrimaryState, SecondaryState, UnknownState, CardId, DiskVolumeUsageLevel, PortDetectedMode, CardSubType, PortMode, FirmwareSource, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "ShelfId", "PortType", "EqActionType", "CardType", "AdminState", "PicType", "PortId", "PrimaryState", "SecondaryState", "UnknownState", "CardId", "DiskVolumeUsageLevel", "PortDetectedMode", "CardSubType", "PortMode", "FirmwareSource", "FlowControlMode")
...
TruthValue, RowStatus, TimeStamp, DateAndTime, MacAddress, TextualConvention, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "TimeStamp", "DateAndTime", "MacAddress", "TextualConvention", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPicDetected=cardPicDetected, cardEntry=cardEntry, cardCPLDVersion=cardCPLDVersion, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portId=portId, auditName=auditName, cardShelfId=cardShelfId, diskVolumeNumber=diskVolumeNumber, TestType=TestType, cmUChannelID=cmUChannelID, equipmentState=equipmentState, cardTemperature=cardTemperature, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierHwVersion=cardCarrierHwVersion, cardLastBootSource=cardLastBootSource, portRxFlowControlDetected=portRxFlowControlDetected, cardSubType=cardSubType, cardTemperatureHighError=cardTemperatureHighError, shelfName=shelfName, eqDiagConfig=eqDiagConfig, auditTotalFailedCount=auditTotalFailedCount, equipmentDiag=equipmentDiag, auditAutoScheduling=auditAutoScheduling, shelfNumber=shelfNumber, cardTestSchedule=cardTestSchedule, cmResetReason=cmResetReason, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, cardHwVersion=cardHwVersion, portPrStateChange=portPrStateChange, cardTestType=cardTestType, cardPicSerialNum=cardPicSerialNum, portMacIfIndex=portMacIfIndex, equipmentShelfGroup=equipmentShelfGroup, TestScheduleCommand=TestScheduleCommand, portCardId=portCardId, cardUpTime=cardUpTime, cardTestEntry=cardTestEntry, equipmentPortGroup=equipmentPortGroup, verboseLevel=verboseLevel, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestTime=cardTestTime, auditResult=auditResult, consoleOutput=consoleOutput, cardFlashSize=cardFlashSize, trapSeverity=trapSeverity, shelfObjects=shelfObjects, portLastChangeTime=portLastChangeTime, auditCardId=auditCardId, cmResetClearNotification=cmResetClearNotification, cardAction=cardAction, cardTable=cardTable, cardTempOverHeatNotification=cardTempOverHeatNotification, cardBusSpeed=cardBusSpeed, equipmentDiagGroup=equipmentDiagGroup, auditTotalPassedCount=auditTotalPassedCount, portDescription=portDescription, cardCpuType=cardCpuType, systemGeneral=systemGeneral, systemKey=systemKey, cardDetected=cardDetected, portRxFlowControlMode=portRxFlowControlMode, auditShelfId=auditShelfId, equipmentAudit=equipmentAudit, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentTbl=equipmentTbl, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeUsageNotification=diskVolumeUsageNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, TestCommand=TestCommand, portDplxStatusChange=portDplxStatusChange, cmPrimarySID=cmPrimarySID, cardSecStateChange=cardSecStateChange, diskVolumeDiskSize=diskVolumeDiskSize, auditPassedCount=auditPassedCount, portAdminState=portAdminState, cardName=cardName, errEvRowStatus=errEvRowStatus, trapCounter=trapCounter, portSecStateChange=portSecStateChange, portConnectorId=portConnectorId, diskVolumeDriveId=diskVolumeDriveId, cardFwUpdateStatus=cardFwUpdateStatus, auditEntry=auditEntry, diskVolumeId=diskVolumeId, cardCarrierSerialNum=cardCarrierSerialNum, cardTestCommand=cardTestCommand, cardLastResetReason=cardLastResetReason, portBgpId=portBgpId, auditCycleCount=auditCycleCount, errEvLogLevel=errEvLogLevel, diskVolumeShelfId=diskVolumeShelfId, portShelfId=portShelfId, auditCommand=auditCommand, cardSerialNum=cardSerialNum, cardSpareGroupId=cardSpareGroupId, cmVendor=cmVendor, diskVolumeGroup=diskVolumeGroup, portDplxStatus=portDplxStatus, cardShelfTestIndex=cardShelfTestIndex, auditDescription=auditDescription, cardCarrierFwVersion=cardCarrierFwVersion, diagTestId=diagTestId, diskVolumeEntry=diskVolumeEntry, cardAdminState=cardAdminState, cmDevice=cmDevice, cardSecState=cardSecState, systemGeneralGroup=systemGeneralGroup, TestSchedule=TestSchedule, portAction=portAction, fpgaErrorEventEntry=fpgaErrorEventEntry, cardType=cardType, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfSwVersion=shelfSwVersion, errEventId=errEventId, diskDriveNumber=diskDriveNumber, cardFwVersion=cardFwVersion, portNumber=portNumber, diskVolumeTable=diskVolumeTable, equipmentTraps=equipmentTraps, equipmentError=equipmentError, cardTestResultNotification=cardTestResultNotification, auditId=auditId, cardTestId=cardTestId, cmResetStatus=cmResetStatus, PYSNMP_MODULE_ID=cadEquipmentMib, TestId=TestId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPicHwVersion=cardPicHwVersion, remainInDiagMode=remainInDiagMode, cardTestName=cardTestName, cardRamSize=cardRamSize, portPrState=portPrState, auditStatus=auditStatus, equipmentStateGroup=equipmentStateGroup, portDetectedMode=portDetectedMode, equipmentCardGroup=equipmentCardGroup, cardLastChangeTime=cardLastChangeTime, cardSpareGroupMode=cardSpareGroupMode, cmResetNotification=cmResetNotification, cardTestScheduleCommand=cardTestScheduleCommand, portEntry=portEntry, cadEquipmentMib=cadEquipmentMib, portSecState=portSecState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetInfo=cmResetInfo, portMaxDsPower=portMaxDsPower, diskVolumeSize=diskVolumeSize, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardId=cardId, cardTestTransId=cardTestTransId, equipmentCardTestGroup=equipmentCardTestGroup, downstreamPowerLoss=downstreamPowerLoss, cardPrStateChange=cardPrStateChange, cardCpuSpeed=cardCpuSpeed, portMode=portMode, portCurrDsPower=portCurrDsPower, cardNumber=cardNumber, auditTime=auditTime, cmMacAddress=cmMacAddress, portTrapInh=portTrapInh, cardDetectedChange=cardDetectedChange, auditLogOutput=auditLogOutput, portType=portType, cardSwVersion=cardSwVersion, diskVolumeCardId=diskVolumeCardId, cardLastBootVersion=cardLastBootVersion, equipmentAuditGroup=equipmentAuditGroup, cardNumPorts=cardNumPorts, cardTestDescription=cardTestDescription, portTable=portTable, auditLogThrottle=auditLogThrottle, cardTestIndex=cardTestIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeFileName=diskVolumeFileName, cardBootVersion=cardBootVersion, TestTransactionId=TestTransactionId, cardFpgaSource=cardFpgaSource, cardTestResult=cardTestResult, auditTable=auditTable, cmRegistrationNotification=cmRegistrationNotification, portTxFlowControlDetected=portTxFlowControlDetected, portMinDsPower=portMinDsPower, errEvLoggingEnabled=errEvLoggingEnabled, portNumChans=portNumChans, diskVolumeName=diskVolumeName, cardTrapInh=cardTrapInh, portDocsIfIndex=portDocsIfIndex, portTxFlowControlMode=portTxFlowControlMode, cardTestResultDesc=cardTestResultDesc, cardPrState=cardPrState, portCardSubType=portCardSubType, cmIpAddress=cmIpAddress, systemClock=systemClock, cmResetUpTime=cmResetUpTime, cardDplxStatus=cardDplxStatus, cardTestTable=cardTestTable, diskVolumeTrapInh=diskVolumeTrapInh, auditFailedCount=auditFailedCount, portMacAddress=portMacAddress, TestResult=TestResult, cardDplxStatusChange=cardDplxStatusChange, cardSubDetected=cardSubDetected, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMibConformance=cadEquipmentMibConformance)
