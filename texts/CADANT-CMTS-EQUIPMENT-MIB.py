...
CardSubType, DuplexStatus, EqActionType, PicType, PrimaryState, AdminState, ShelfId, PortDetectedMode, PortId, DiskVolumeUsageLevel, CardId, UnknownState, FlowControlMode, PortType, CardType, PortMode, SecondaryState, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "DuplexStatus", "EqActionType", "PicType", "PrimaryState", "AdminState", "ShelfId", "PortDetectedMode", "PortId", "DiskVolumeUsageLevel", "CardId", "UnknownState", "FlowControlMode", "PortType", "CardType", "PortMode", "SecondaryState", "FirmwareSource")
...
TimeStamp, DateAndTime, MacAddress, DisplayString, TruthValue, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DateAndTime", "MacAddress", "DisplayString", "TruthValue", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTable=portTable, portNumber=portNumber, shelfObjects=shelfObjects, cardTestId=cardTestId, TestId=TestId, auditCardId=auditCardId, systemGeneral=systemGeneral, cardDplxStatusChange=cardDplxStatusChange, cmResetReason=cmResetReason, portMode=portMode, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentTraps=equipmentTraps, cardNumPorts=cardNumPorts, portType=portType, portCardSubType=portCardSubType, cardTestEntry=cardTestEntry, portPrStateChange=portPrStateChange, auditTotalFailedCount=auditTotalFailedCount, cardTemperature=cardTemperature, portLastChangeTime=portLastChangeTime, cardCarrierFwVersion=cardCarrierFwVersion, shelfSwVersion=shelfSwVersion, cardPicSerialNum=cardPicSerialNum, diskVolumeSize=diskVolumeSize, diskVolumeId=diskVolumeId, diskVolumeFileName=diskVolumeFileName, cmMacAddress=cmMacAddress, errEvLoggingEnabled=errEvLoggingEnabled, cardSpareGroupId=cardSpareGroupId, cardTemperatureHighError=cardTemperatureHighError, cardTemperatureHighWarn=cardTemperatureHighWarn, auditCycleCount=auditCycleCount, cardTestTransId=cardTestTransId, portDocsIfIndex=portDocsIfIndex, shelfName=shelfName, diskVolumeUsageLevel=diskVolumeUsageLevel, errEventId=errEventId, TestTransactionId=TestTransactionId, auditFailedCount=auditFailedCount, cardAction=cardAction, portId=portId, diskVolumeShelfId=diskVolumeShelfId, cmRegistrationNotification=cmRegistrationNotification, cardTestSchedule=cardTestSchedule, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCarrierSerialNum=cardCarrierSerialNum, verboseLevel=verboseLevel, portBgpId=portBgpId, consoleOutput=consoleOutput, portDplxStatus=portDplxStatus, cardType=cardType, equipmentAudit=equipmentAudit, portDplxStatusChange=portDplxStatusChange, cardUpTime=cardUpTime, cardTestIndex=cardTestIndex, cmResetInfo=cmResetInfo, cardDetected=cardDetected, portRxFlowControlMode=portRxFlowControlMode, cardTestTable=cardTestTable, cardTable=cardTable, equipmentCardTestGroup=equipmentCardTestGroup, auditTotalPassedCount=auditTotalPassedCount, cardSubType=cardSubType, cardPicHwVersion=cardPicHwVersion, portSecState=portSecState, cardTestCommand=cardTestCommand, cardDetectedChange=cardDetectedChange, cmIpAddress=cmIpAddress, portMinDsPower=portMinDsPower, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetNotification=cmResetNotification, cmVendor=cmVendor, auditEntry=auditEntry, cmResetUpTime=cmResetUpTime, portMaxDsPower=portMaxDsPower, cardFwUpdateStatus=cardFwUpdateStatus, cardCpuType=cardCpuType, cardLastBootVersion=cardLastBootVersion, TestType=TestType, cardBootVersion=cardBootVersion, cardId=cardId, auditShelfId=auditShelfId, equipmentError=equipmentError, TestScheduleCommand=TestScheduleCommand, cardCarrierHwVersion=cardCarrierHwVersion, cardCpuSpeed=cardCpuSpeed, cardAdminState=cardAdminState, portEntry=portEntry, auditPassedCount=auditPassedCount, diagTestId=diagTestId, errEvLogLevel=errEvLogLevel, cardPrState=cardPrState, cmPrimarySID=cmPrimarySID, cardSwVersion=cardSwVersion, auditTable=auditTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestTime=cardTestTime, portShelfId=portShelfId, cardRamSize=cardRamSize, PYSNMP_MODULE_ID=cadEquipmentMib, cardShelfTestIndex=cardShelfTestIndex, portAction=portAction, diskVolumeEntry=diskVolumeEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentCardGroup=equipmentCardGroup, equipmentShelfGroup=equipmentShelfGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, fpgaErrorEventTable=fpgaErrorEventTable, errEvRowStatus=errEvRowStatus, cardSerialNum=cardSerialNum, cardShelfId=cardShelfId, portDescription=portDescription, diskVolumeDiskSize=diskVolumeDiskSize, cardBusSpeed=cardBusSpeed, auditTime=auditTime, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeName=diskVolumeName, auditName=auditName, portCurrDsPower=portCurrDsPower, equipmentAuditGroup=equipmentAuditGroup, diskVolumeGroup=diskVolumeGroup, cardLastBootSource=cardLastBootSource, systemClock=systemClock, diskDriveNumber=diskDriveNumber, eqDiagConfig=eqDiagConfig, auditLogOutput=auditLogOutput, cardLastChangeTime=cardLastChangeTime, cardEntry=cardEntry, portConnectorId=portConnectorId, cmResetClearNotification=cmResetClearNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardFlashSize=cardFlashSize, trapCounter=trapCounter, equipmentPortGroup=equipmentPortGroup, cardTestResultNotification=cardTestResultNotification, trapSeverity=trapSeverity, cardPicDetected=cardPicDetected, portTxFlowControlDetected=portTxFlowControlDetected, portMacIfIndex=portMacIfIndex, cardSecStateChange=cardSecStateChange, TestResult=TestResult, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentState=equipmentState, portTrapInh=portTrapInh, cardSecState=cardSecState, TestCommand=TestCommand, equipmentStateGroup=equipmentStateGroup, cardTestName=cardTestName, cardTestResultDesc=cardTestResultDesc, cmResetStatus=cmResetStatus, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentTbl=equipmentTbl, equipmentDiag=equipmentDiag, portTxFlowControlMode=portTxFlowControlMode, equipmentDiagGroup=equipmentDiagGroup, auditAutoScheduling=auditAutoScheduling, cardPrStateChange=cardPrStateChange, cardDplxStatus=cardDplxStatus, portDetectedMode=portDetectedMode, cardFwVersion=cardFwVersion, shelfNumber=shelfNumber, cardNumber=cardNumber, auditLogThrottle=auditLogThrottle, cardTempNoReportNotification=cardTempNoReportNotification, downstreamPowerLoss=downstreamPowerLoss, auditStatus=auditStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditDescription=auditDescription, TestSchedule=TestSchedule, portSecStateChange=portSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditCommand=auditCommand, cardTestDescription=cardTestDescription, portCardId=portCardId, diskVolumeTable=diskVolumeTable, cardTempOverHeatNotification=cardTempOverHeatNotification, portPrState=portPrState, cardCPLDVersion=cardCPLDVersion, cmDevice=cmDevice, diskVolumeDriveId=diskVolumeDriveId, remainInDiagMode=remainInDiagMode, cardSpareGroupMode=cardSpareGroupMode, portMacAddress=portMacAddress, cardTrapInh=cardTrapInh, auditId=auditId, portAdminState=portAdminState, systemKey=systemKey, cmUChannelID=cmUChannelID, cadEquipmentMib=cadEquipmentMib, cardFpgaSource=cardFpgaSource, portNumChans=portNumChans, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestScheduleCommand=cardTestScheduleCommand, systemGeneralGroup=systemGeneralGroup, cardHwVersion=cardHwVersion, diskVolumeNumber=diskVolumeNumber, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResult=cardTestResult, cardTestType=cardTestType, cardSubDetected=cardSubDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardName=cardName, auditResult=auditResult, diskVolumeTrapInh=diskVolumeTrapInh, cardLastResetReason=cardLastResetReason, diskVolumeCardId=diskVolumeCardId)
