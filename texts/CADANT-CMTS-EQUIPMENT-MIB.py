...
AdminState, CardId, CardSubType, FlowControlMode, FirmwareSource, PrimaryState, PortType, DuplexStatus, UnknownState, PortId, PortMode, PortDetectedMode, DiskVolumeUsageLevel, EqActionType, CardType, SecondaryState, ShelfId, PicType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "CardId", "CardSubType", "FlowControlMode", "FirmwareSource", "PrimaryState", "PortType", "DuplexStatus", "UnknownState", "PortId", "PortMode", "PortDetectedMode", "DiskVolumeUsageLevel", "EqActionType", "CardType", "SecondaryState", "ShelfId", "PicType")
...
MacAddress, RowStatus, DisplayString, TruthValue, DateAndTime, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "DisplayString", "TruthValue", "DateAndTime", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardNumPorts=cardNumPorts, cardFlashSize=cardFlashSize, equipmentError=equipmentError, portNumber=portNumber, cardCarrierHwVersion=cardCarrierHwVersion, cardNumber=cardNumber, cardTestIndex=cardTestIndex, cardSerialNum=cardSerialNum, diskVolumeNumber=diskVolumeNumber, portType=portType, cardPicDetected=cardPicDetected, cardTestId=cardTestId, equipmentStateGroup=equipmentStateGroup, cardDetectedChange=cardDetectedChange, cardFwVersion=cardFwVersion, cardTestCommand=cardTestCommand, portConnectorId=portConnectorId, auditFailedCount=auditFailedCount, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTrapInh=cardTrapInh, diskVolumeShelfId=diskVolumeShelfId, portId=portId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetStatus=cmResetStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, errEventId=errEventId, diagTestId=diagTestId, cardFpgaSource=cardFpgaSource, diskVolumeUsageNotification=diskVolumeUsageNotification, portCurrDsPower=portCurrDsPower, cadEquipmentMibConformance=cadEquipmentMibConformance, fpgaErrorEventTable=fpgaErrorEventTable, cmMacAddress=cmMacAddress, portMode=portMode, portTxFlowControlDetected=portTxFlowControlDetected, cardTempNoReportNotification=cardTempNoReportNotification, equipmentPortGroup=equipmentPortGroup, TestCommand=TestCommand, TestScheduleCommand=TestScheduleCommand, portMacIfIndex=portMacIfIndex, cardTestName=cardTestName, cardShelfTestIndex=cardShelfTestIndex, cmResetReason=cmResetReason, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeId=diskVolumeId, systemKey=systemKey, portDplxStatus=portDplxStatus, downstreamPowerLoss=downstreamPowerLoss, equipmentDiagGroup=equipmentDiagGroup, cardSecStateChange=cardSecStateChange, auditCardId=auditCardId, cardCarrierFwVersion=cardCarrierFwVersion, TestId=TestId, portTable=portTable, systemGeneral=systemGeneral, cmPrimarySID=cmPrimarySID, portBgpId=portBgpId, cardTestTable=cardTestTable, TestResult=TestResult, cardTestResultDesc=cardTestResultDesc, cardUpTime=cardUpTime, cardSecState=cardSecState, portAction=portAction, portDocsIfIndex=portDocsIfIndex, auditLogOutput=auditLogOutput, cardDplxStatusChange=cardDplxStatusChange, errEvLogLevel=errEvLogLevel, cmResetClearNotification=cmResetClearNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestType=cardTestType, diskVolumeDriveId=diskVolumeDriveId, trapCounter=trapCounter, cardTestEntry=cardTestEntry, shelfSwVersion=shelfSwVersion, auditPassedCount=auditPassedCount, equipmentCardTestGroup=equipmentCardTestGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, portDescription=portDescription, equipmentAuditGroup=equipmentAuditGroup, equipmentAudit=equipmentAudit, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeGroup=diskVolumeGroup, portMaxDsPower=portMaxDsPower, errEvLoggingEnabled=errEvLoggingEnabled, errEvRowStatus=errEvRowStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portCardSubType=portCardSubType, consoleOutput=consoleOutput, portAdminState=portAdminState, cardTestScheduleCommand=cardTestScheduleCommand, cardLastBootVersion=cardLastBootVersion, cardTestResultNotification=cardTestResultNotification, systemGeneralGroup=systemGeneralGroup, cardBusSpeed=cardBusSpeed, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfObjects=shelfObjects, cmVendor=cmVendor, TestType=TestType, auditEntry=auditEntry, auditTotalPassedCount=auditTotalPassedCount, cardType=cardType, portLastChangeTime=portLastChangeTime, auditName=auditName, diskVolumeDiskSize=diskVolumeDiskSize, equipmentShelfGroup=equipmentShelfGroup, auditShelfId=auditShelfId, cardEntry=cardEntry, equipmentDiag=equipmentDiag, cmIpAddress=cmIpAddress, portShelfId=portShelfId, cardCarrierSerialNum=cardCarrierSerialNum, shelfNumber=shelfNumber, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSwVersion=cardSwVersion, cardTemperature=cardTemperature, cardTestSchedule=cardTestSchedule, portNumChans=portNumChans, cardCpuType=cardCpuType, cmResetNotification=cmResetNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardAction=cardAction, cardSpareGroupId=cardSpareGroupId, cardLastChangeTime=cardLastChangeTime, remainInDiagMode=remainInDiagMode, auditCommand=auditCommand, equipmentCardGroup=equipmentCardGroup, cardSpareGroupMode=cardSpareGroupMode, cardPrStateChange=cardPrStateChange, diskVolumeFileName=diskVolumeFileName, cardRamSize=cardRamSize, cardBootVersion=cardBootVersion, diskVolumeCardId=diskVolumeCardId, cardShelfId=cardShelfId, auditTotalFailedCount=auditTotalFailedCount, cardDplxStatus=cardDplxStatus, portSecState=portSecState, equipmentState=equipmentState, cardHwVersion=cardHwVersion, cardCPLDVersion=cardCPLDVersion, portMinDsPower=portMinDsPower, cardTestDescription=cardTestDescription, diskVolumeTable=diskVolumeTable, auditCycleCount=auditCycleCount, cardAdminState=cardAdminState, cmUChannelID=cmUChannelID, verboseLevel=verboseLevel, auditTime=auditTime, cmRegistrationNotification=cmRegistrationNotification, cardPicSerialNum=cardPicSerialNum, diskVolumeSize=diskVolumeSize, portEntry=portEntry, auditAutoScheduling=auditAutoScheduling, diskDriveNumber=diskDriveNumber, cmDevice=cmDevice, cadEquipmentMib=cadEquipmentMib, auditTable=auditTable, auditDescription=auditDescription, cardPicHwVersion=cardPicHwVersion, cardPrState=cardPrState, portMacAddress=portMacAddress, shelfName=shelfName, auditId=auditId, auditResult=auditResult, cardTestTime=cardTestTime, cardCpuSpeed=cardCpuSpeed, cardTemperatureHighError=cardTemperatureHighError, trapSeverity=trapSeverity, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, eqDiagConfig=eqDiagConfig, equipmentTbl=equipmentTbl, portCardId=portCardId, cardTable=cardTable, portPrState=portPrState, portDetectedMode=portDetectedMode, cardTestResult=cardTestResult, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentTraps=equipmentTraps, portTrapInh=portTrapInh, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetInfo=cmResetInfo, cardDetected=cardDetected, TestTransactionId=TestTransactionId, diskVolumeEntry=diskVolumeEntry, cardTestTransId=cardTestTransId, diskVolumeTrapInh=diskVolumeTrapInh, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardLastResetReason=cardLastResetReason, cardSubType=cardSubType, cardFwUpdateStatus=cardFwUpdateStatus, cardId=cardId, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageLevel=diskVolumeUsageLevel, portPrStateChange=portPrStateChange, systemClock=systemClock, TestSchedule=TestSchedule, diskVolumeName=diskVolumeName, portDplxStatusChange=portDplxStatusChange, auditStatus=auditStatus, cardSubDetected=cardSubDetected, fpgaErrorEventGroup=fpgaErrorEventGroup, auditLogThrottle=auditLogThrottle, cardLastBootSource=cardLastBootSource, cardName=cardName, portTxFlowControlMode=portTxFlowControlMode, cmResetUpTime=cmResetUpTime, portSecStateChange=portSecStateChange)
