...
FirmwareSource, PortDetectedMode, DuplexStatus, PicType, PortId, CardType, ShelfId, DiskVolumeUsageLevel, CardSubType, PrimaryState, EqActionType, UnknownState, FlowControlMode, CardId, SecondaryState, PortType, PortMode, AdminState = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "PortDetectedMode", "DuplexStatus", "PicType", "PortId", "CardType", "ShelfId", "DiskVolumeUsageLevel", "CardSubType", "PrimaryState", "EqActionType", "UnknownState", "FlowControlMode", "CardId", "SecondaryState", "PortType", "PortMode", "AdminState")
...
TimeStamp, TruthValue, DisplayString, DateAndTime, RowStatus, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "DisplayString", "DateAndTime", "RowStatus", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTemperature=cardTemperature, cardTrapInh=cardTrapInh, cmResetClearNotification=cmResetClearNotification, TestTransactionId=TestTransactionId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetNotification=cmResetNotification, cardShelfId=cardShelfId, portEntry=portEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeCardId=diskVolumeCardId, cmMacAddress=cmMacAddress, auditEntry=auditEntry, portAction=portAction, portDescription=portDescription, cardLastBootVersion=cardLastBootVersion, auditPassedCount=auditPassedCount, cardBusSpeed=cardBusSpeed, shelfObjects=shelfObjects, auditShelfId=auditShelfId, cardSpareGroupId=cardSpareGroupId, cardTestResultDesc=cardTestResultDesc, portDplxStatusChange=portDplxStatusChange, diskVolumeNumber=diskVolumeNumber, portCardId=portCardId, equipmentShelfGroup=equipmentShelfGroup, cardRamSize=cardRamSize, eqDiagConfig=eqDiagConfig, portMaxDsPower=portMaxDsPower, cardFlashSize=cardFlashSize, auditCycleCount=auditCycleCount, diskVolumeUsageNotification=diskVolumeUsageNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestType=TestType, cardFwVersion=cardFwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardUpTime=cardUpTime, fpgaErrorEventGroup=fpgaErrorEventGroup, auditTotalFailedCount=auditTotalFailedCount, cardLastResetReason=cardLastResetReason, auditLogOutput=auditLogOutput, cardPicHwVersion=cardPicHwVersion, cardTestType=cardTestType, fpgaErrorEventTable=fpgaErrorEventTable, verboseLevel=verboseLevel, portMacIfIndex=portMacIfIndex, cardTestId=cardTestId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardNumber=cardNumber, cardCarrierSerialNum=cardCarrierSerialNum, cardShelfTestIndex=cardShelfTestIndex, portDetectedMode=portDetectedMode, diskVolumeFileName=diskVolumeFileName, cardTestIndex=cardTestIndex, auditLogThrottle=auditLogThrottle, cardCpuSpeed=cardCpuSpeed, diskVolumeId=diskVolumeId, systemClock=systemClock, portNumChans=portNumChans, cardDplxStatusChange=cardDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, equipmentAuditGroup=equipmentAuditGroup, diskVolumeDriveId=diskVolumeDriveId, diskVolumeGroup=diskVolumeGroup, cardDetectedChange=cardDetectedChange, cardPicSerialNum=cardPicSerialNum, portCurrDsPower=portCurrDsPower, cardHwVersion=cardHwVersion, shelfSwVersion=shelfSwVersion, cardCPLDVersion=cardCPLDVersion, cardDetected=cardDetected, portConnectorId=portConnectorId, cardTestName=cardTestName, cardPrState=cardPrState, diskVolumeName=diskVolumeName, diskVolumeTrapInh=diskVolumeTrapInh, shelfNumber=shelfNumber, errEvRowStatus=errEvRowStatus, cardSecStateChange=cardSecStateChange, portCardSubType=portCardSubType, cardTestResultNotification=cardTestResultNotification, cardTestDescription=cardTestDescription, equipmentPortGroup=equipmentPortGroup, cmResetUpTime=cmResetUpTime, portTable=portTable, cardPicDetected=cardPicDetected, portRxFlowControlDetected=portRxFlowControlDetected, PYSNMP_MODULE_ID=cadEquipmentMib, TestId=TestId, cmVendor=cmVendor, portTrapInh=portTrapInh, portBgpId=portBgpId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeSize=diskVolumeSize, cardTemperatureHighError=cardTemperatureHighError, cardNumPorts=cardNumPorts, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSerialNum=cardSerialNum, cardFwUpdateStatus=cardFwUpdateStatus, cmUChannelID=cmUChannelID, cardCarrierFwVersion=cardCarrierFwVersion, portPrStateChange=portPrStateChange, portAdminState=portAdminState, cmResetReason=cmResetReason, cardDplxStatus=cardDplxStatus, downstreamPowerLoss=downstreamPowerLoss, portRxFlowControlMode=portRxFlowControlMode, portMode=portMode, cardId=cardId, TestScheduleCommand=TestScheduleCommand, auditCommand=auditCommand, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTable=cardTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditResult=auditResult, remainInDiagMode=remainInDiagMode, trapSeverity=trapSeverity, auditTable=auditTable, portPrState=portPrState, diskVolumeDiskSize=diskVolumeDiskSize, cmRegistrationNotification=cmRegistrationNotification, cardAdminState=cardAdminState, auditFailedCount=auditFailedCount, fpgaErrorEventEntry=fpgaErrorEventEntry, portMinDsPower=portMinDsPower, portSecStateChange=portSecStateChange, diskVolumeEntry=diskVolumeEntry, cmPrimarySID=cmPrimarySID, diskVolumeTable=diskVolumeTable, auditCardId=auditCardId, portType=portType, cardCarrierHwVersion=cardCarrierHwVersion, systemGeneral=systemGeneral, cadEquipmentMibGroup=cadEquipmentMibGroup, auditStatus=auditStatus, errEventId=errEventId, systemGeneralGroup=systemGeneralGroup, cardLastBootSource=cardLastBootSource, portDocsIfIndex=portDocsIfIndex, cardTestSchedule=cardTestSchedule, errEvLogLevel=errEvLogLevel, equipmentStateGroup=equipmentStateGroup, portTxFlowControlMode=portTxFlowControlMode, cardSubType=cardSubType, cardSecState=cardSecState, portNumber=portNumber, cardTestScheduleCommand=cardTestScheduleCommand, portShelfId=portShelfId, cardTestEntry=cardTestEntry, auditTime=auditTime, diskVolumeShelfId=diskVolumeShelfId, cardTempNoReportNotification=cardTempNoReportNotification, cardCpuType=cardCpuType, auditDescription=auditDescription, auditTotalPassedCount=auditTotalPassedCount, cmIpAddress=cmIpAddress, cardBootVersion=cardBootVersion, cardLastChangeTime=cardLastChangeTime, equipmentTraps=equipmentTraps, cardTestTable=cardTestTable, cardAction=cardAction, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cadEquipmentMib=cadEquipmentMib, diagTestId=diagTestId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portId=portId, TestCommand=TestCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestResult=cardTestResult, portMacAddress=portMacAddress, cmResetStatus=cmResetStatus, consoleOutput=consoleOutput, cardSpareGroupMode=cardSpareGroupMode, shelfName=shelfName, trapCounter=trapCounter, cardEntry=cardEntry, systemKey=systemKey, cardSwVersion=cardSwVersion, equipmentCardTestGroup=equipmentCardTestGroup, equipmentAudit=equipmentAudit, TestResult=TestResult, auditName=auditName, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, errEvLoggingEnabled=errEvLoggingEnabled, auditAutoScheduling=auditAutoScheduling, equipmentTbl=equipmentTbl, equipmentCardGroup=equipmentCardGroup, cardSubDetected=cardSubDetected, cardTestTransId=cardTestTransId, portLastChangeTime=portLastChangeTime, equipmentState=equipmentState, TestSchedule=TestSchedule, diskDriveNumber=diskDriveNumber, portSecState=portSecState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestTime=cardTestTime, cardPrStateChange=cardPrStateChange, equipmentDiag=equipmentDiag, cardName=cardName, portDplxStatus=portDplxStatus, cardTestCommand=cardTestCommand, cardType=cardType, cmResetInfo=cmResetInfo, auditId=auditId, cmDevice=cmDevice, equipmentError=equipmentError, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFpgaSource=cardFpgaSource)
