...
AdminState, SecondaryState, DuplexStatus, PortType, PicType, PortMode, CardId, PortDetectedMode, EqActionType, UnknownState, CardType, FirmwareSource, PortId, ShelfId, DiskVolumeUsageLevel, CardSubType, PrimaryState, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "AdminState", "SecondaryState", "DuplexStatus", "PortType", "PicType", "PortMode", "CardId", "PortDetectedMode", "EqActionType", "UnknownState", "CardType", "FirmwareSource", "PortId", "ShelfId", "DiskVolumeUsageLevel", "CardSubType", "PrimaryState", "FlowControlMode")
...
DateAndTime, TruthValue, TextualConvention, RowStatus, DisplayString, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TruthValue", "TextualConvention", "RowStatus", "DisplayString", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portAdminState=portAdminState, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, equipmentError=equipmentError, cardBootVersion=cardBootVersion, downstreamPowerLoss=downstreamPowerLoss, portConnectorId=portConnectorId, diskVolumeNumber=diskVolumeNumber, cmResetUpTime=cmResetUpTime, cardTestId=cardTestId, equipmentTraps=equipmentTraps, cardFwVersion=cardFwVersion, diskVolumeGroup=diskVolumeGroup, auditCardId=auditCardId, cardFwUpdateStatus=cardFwUpdateStatus, cardPicHwVersion=cardPicHwVersion, cardCpuType=cardCpuType, errEvRowStatus=errEvRowStatus, cardHwVersion=cardHwVersion, auditLogOutput=auditLogOutput, cardName=cardName, cardCarrierFwVersion=cardCarrierFwVersion, portNumber=portNumber, fpgaErrorEventTable=fpgaErrorEventTable, cardTestResultNotification=cardTestResultNotification, cardSubType=cardSubType, consoleOutput=consoleOutput, systemGeneral=systemGeneral, cardAdminState=cardAdminState, equipmentShelfGroup=equipmentShelfGroup, cardTestTransId=cardTestTransId, portDetectedMode=portDetectedMode, diskVolumeFileName=diskVolumeFileName, portMaxDsPower=portMaxDsPower, TestCommand=TestCommand, portMode=portMode, portTxFlowControlDetected=portTxFlowControlDetected, portSecState=portSecState, diskVolumeCardId=diskVolumeCardId, remainInDiagMode=remainInDiagMode, portEntry=portEntry, TestTransactionId=TestTransactionId, cardTestScheduleCommand=cardTestScheduleCommand, PYSNMP_MODULE_ID=cadEquipmentMib, TestSchedule=TestSchedule, cardSpareGroupId=cardSpareGroupId, cardPicDetected=cardPicDetected, cardDplxStatus=cardDplxStatus, cardPrStateChange=cardPrStateChange, cardCarrierHwVersion=cardCarrierHwVersion, TestScheduleCommand=TestScheduleCommand, cardSecStateChange=cardSecStateChange, portTxFlowControlMode=portTxFlowControlMode, verboseLevel=verboseLevel, auditTotalPassedCount=auditTotalPassedCount, cardFlashSize=cardFlashSize, cardSerialNum=cardSerialNum, auditId=auditId, auditName=auditName, auditTime=auditTime, equipmentDiag=equipmentDiag, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestResult=cardTestResult, cmIpAddress=cmIpAddress, fpgaErrorEventGroup=fpgaErrorEventGroup, systemKey=systemKey, diskVolumeDiskSize=diskVolumeDiskSize, cardBusSpeed=cardBusSpeed, cmMacAddress=cmMacAddress, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeTable=diskVolumeTable, portLastChangeTime=portLastChangeTime, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardNumber=cardNumber, cardAction=cardAction, cardSpareGroupMode=cardSpareGroupMode, auditResult=auditResult, cardTestResultDesc=cardTestResultDesc, cardSubDetected=cardSubDetected, cardUpTime=cardUpTime, auditCommand=auditCommand, cardTempNoReportNotification=cardTempNoReportNotification, cardEntry=cardEntry, cardPicSerialNum=cardPicSerialNum, portDplxStatusChange=portDplxStatusChange, TestResult=TestResult, portBgpId=portBgpId, trapSeverity=trapSeverity, cmResetInfo=cmResetInfo, cmResetStatus=cmResetStatus, auditTable=auditTable, fpgaErrorEventEntry=fpgaErrorEventEntry, cmDevice=cmDevice, errEvLogLevel=errEvLogLevel, cardDetectedChange=cardDetectedChange, cardTestCommand=cardTestCommand, portNumChans=portNumChans, cardId=cardId, auditCycleCount=auditCycleCount, cardFpgaSource=cardFpgaSource, TestId=TestId, cardTestTime=cardTestTime, errEvLoggingEnabled=errEvLoggingEnabled, cmResetNotification=cmResetNotification, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTable=cardTable, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portMacAddress=portMacAddress, cardDetected=cardDetected, diskDriveNumber=diskDriveNumber, equipmentAudit=equipmentAudit, eqDiagConfig=eqDiagConfig, diagTestId=diagTestId, cardTestSchedule=cardTestSchedule, portDplxStatus=portDplxStatus, portTrapInh=portTrapInh, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfNumber=shelfNumber, equipmentPortGroup=equipmentPortGroup, cardPrState=cardPrState, auditEntry=auditEntry, cardRamSize=cardRamSize, portPrStateChange=portPrStateChange, equipmentTbl=equipmentTbl, auditAutoScheduling=auditAutoScheduling, cardTemperatureHighError=cardTemperatureHighError, diskVolumeDriveId=diskVolumeDriveId, cardTestTable=cardTestTable, cardTestIndex=cardTestIndex, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardType=cardType, cmResetReason=cmResetReason, portCardSubType=portCardSubType, shelfSwVersion=shelfSwVersion, auditStatus=auditStatus, systemGeneralGroup=systemGeneralGroup, cmUChannelID=cmUChannelID, cardLastResetReason=cardLastResetReason, portSecStateChange=portSecStateChange, cardLastBootSource=cardLastBootSource, cardShelfId=cardShelfId, portCardId=portCardId, portMacIfIndex=portMacIfIndex, TestType=TestType, cardSecState=cardSecState, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestName=cardTestName, shelfObjects=shelfObjects, cmVendor=cmVendor, cardShelfTestIndex=cardShelfTestIndex, equipmentState=equipmentState, trapCounter=trapCounter, diskVolumeTrapInh=diskVolumeTrapInh, portMinDsPower=portMinDsPower, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, errEventId=errEventId, cardDplxStatusChange=cardDplxStatusChange, cardTestDescription=cardTestDescription, cadEquipmentMibConformance=cadEquipmentMibConformance, portShelfId=portShelfId, portTable=portTable, equipmentStateGroup=equipmentStateGroup, diskVolumeShelfId=diskVolumeShelfId, portCurrDsPower=portCurrDsPower, portId=portId, portDocsIfIndex=portDocsIfIndex, diskVolumeEntry=diskVolumeEntry, cardNumPorts=cardNumPorts, cmResetClearNotification=cmResetClearNotification, diskVolumeName=diskVolumeName, diskVolumeSize=diskVolumeSize, portDescription=portDescription, systemClock=systemClock, cardCPLDVersion=cardCPLDVersion, cardLastChangeTime=cardLastChangeTime, cardCpuSpeed=cardCpuSpeed, diskVolumeId=diskVolumeId, cardCarrierSerialNum=cardCarrierSerialNum, equipmentDiagGroup=equipmentDiagGroup, portRxFlowControlMode=portRxFlowControlMode, cardTemperatureHighWarn=cardTemperatureHighWarn, portAction=portAction, cardTrapInh=cardTrapInh, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTemperature=cardTemperature, auditPassedCount=auditPassedCount, auditLogThrottle=auditLogThrottle, cardSwVersion=cardSwVersion, auditFailedCount=auditFailedCount, portPrState=portPrState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestType=cardTestType, portType=portType, shelfName=shelfName, equipmentCardGroup=equipmentCardGroup, cmPrimarySID=cmPrimarySID, auditDescription=auditDescription, cardTestEntry=cardTestEntry, auditShelfId=auditShelfId, cadEquipmentMib=cadEquipmentMib, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageNotification=diskVolumeUsageNotification)
