...
PrimaryState, AdminState, CardId, UnknownState, DuplexStatus, EqActionType, PortDetectedMode, DiskVolumeUsageLevel, CardSubType, FlowControlMode, PortType, PortId, FirmwareSource, SecondaryState, ShelfId, PicType, CardType, PortMode = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "AdminState", "CardId", "UnknownState", "DuplexStatus", "EqActionType", "PortDetectedMode", "DiskVolumeUsageLevel", "CardSubType", "FlowControlMode", "PortType", "PortId", "FirmwareSource", "SecondaryState", "ShelfId", "PicType", "CardType", "PortMode")
...
MacAddress, TruthValue, TextualConvention, TimeStamp, RowStatus, DateAndTime, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "TextualConvention", "TimeStamp", "RowStatus", "DateAndTime", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentDiag=equipmentDiag, cardSubType=cardSubType, shelfNumber=shelfNumber, diskVolumeId=diskVolumeId, auditTime=auditTime, TestScheduleCommand=TestScheduleCommand, auditLogOutput=auditLogOutput, cardId=cardId, cmPrimarySID=cmPrimarySID, cardSwVersion=cardSwVersion, cardFwUpdateStatus=cardFwUpdateStatus, fpgaErrorEventTable=fpgaErrorEventTable, auditLogThrottle=auditLogThrottle, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibCompliance=cadEquipmentMibCompliance, systemGeneral=systemGeneral, cardLastResetReason=cardLastResetReason, cardTestId=cardTestId, portCurrDsPower=portCurrDsPower, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetClearNotification=cmResetClearNotification, shelfObjects=shelfObjects, portRxFlowControlMode=portRxFlowControlMode, cardShelfId=cardShelfId, cardDplxStatus=cardDplxStatus, cmResetInfo=cmResetInfo, TestCommand=TestCommand, cardTemperatureHighError=cardTemperatureHighError, portTable=portTable, portMacAddress=portMacAddress, auditCommand=auditCommand, auditTotalFailedCount=auditTotalFailedCount, errEvRowStatus=errEvRowStatus, cardLastBootSource=cardLastBootSource, cardTempNoReportNotification=cardTempNoReportNotification, cardTestCommand=cardTestCommand, shelfName=shelfName, cmResetStatus=cmResetStatus, auditDescription=auditDescription, portMinDsPower=portMinDsPower, portTxFlowControlDetected=portTxFlowControlDetected, verboseLevel=verboseLevel, systemKey=systemKey, portShelfId=portShelfId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDescription=portDescription, equipmentAuditGroup=equipmentAuditGroup, cardFpgaSource=cardFpgaSource, diskVolumeDriveId=diskVolumeDriveId, remainInDiagMode=remainInDiagMode, diskVolumeCardId=diskVolumeCardId, cardDetected=cardDetected, portAction=portAction, portRxFlowControlDetected=portRxFlowControlDetected, cardSerialNum=cardSerialNum, cardRamSize=cardRamSize, cardTestTransId=cardTestTransId, cardTestEntry=cardTestEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditCycleCount=auditCycleCount, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestTime=cardTestTime, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentDiagGroup=equipmentDiagGroup, portTrapInh=portTrapInh, downstreamPowerLoss=downstreamPowerLoss, auditShelfId=auditShelfId, cardSpareGroupMode=cardSpareGroupMode, cardTestTable=cardTestTable, TestTransactionId=TestTransactionId, cardSpareGroupId=cardSpareGroupId, portMode=portMode, portLastChangeTime=portLastChangeTime, cardTestResultDesc=cardTestResultDesc, portBgpId=portBgpId, cmVendor=cmVendor, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardDplxStatusChange=cardDplxStatusChange, diskVolumeShelfId=diskVolumeShelfId, equipmentPortGroup=equipmentPortGroup, TestResult=TestResult, cardPicDetected=cardPicDetected, cardTestDescription=cardTestDescription, equipmentCardGroup=equipmentCardGroup, cmResetUpTime=cmResetUpTime, portAdminState=portAdminState, portMacIfIndex=portMacIfIndex, auditEntry=auditEntry, auditTotalPassedCount=auditTotalPassedCount, cardTestScheduleCommand=cardTestScheduleCommand, portSecState=portSecState, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeName=diskVolumeName, cardCarrierFwVersion=cardCarrierFwVersion, cardNumPorts=cardNumPorts, cardType=cardType, diskVolumeNumber=diskVolumeNumber, systemClock=systemClock, cardEntry=cardEntry, cardCPLDVersion=cardCPLDVersion, equipmentTraps=equipmentTraps, eqDiagConfig=eqDiagConfig, PYSNMP_MODULE_ID=cadEquipmentMib, TestType=TestType, cmResetReason=cmResetReason, portDplxStatusChange=portDplxStatusChange, equipmentCardTestGroup=equipmentCardTestGroup, cardPicSerialNum=cardPicSerialNum, auditStatus=auditStatus, cmDevice=cmDevice, portConnectorId=portConnectorId, auditTable=auditTable, errEvLogLevel=errEvLogLevel, systemGeneralGroup=systemGeneralGroup, portId=portId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestSchedule=cardTestSchedule, auditFailedCount=auditFailedCount, diskVolumeSize=diskVolumeSize, cmRegistrationNotification=cmRegistrationNotification, cmMacAddress=cmMacAddress, diskVolumeFileName=diskVolumeFileName, cardTable=cardTable, cardAction=cardAction, portPrStateChange=portPrStateChange, cmUChannelID=cmUChannelID, equipmentAudit=equipmentAudit, cardHwVersion=cardHwVersion, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageLevel=diskVolumeUsageLevel, cardPicHwVersion=cardPicHwVersion, cardTestResult=cardTestResult, auditName=auditName, cardNumber=cardNumber, cardTemperature=cardTemperature, portMaxDsPower=portMaxDsPower, cardFwVersion=cardFwVersion, cardUpTime=cardUpTime, cardTestResultNotification=cardTestResultNotification, cmIpAddress=cmIpAddress, cardSecStateChange=cardSecStateChange, cardTrapInh=cardTrapInh, TestId=TestId, cardPrState=cardPrState, trapSeverity=trapSeverity, portType=portType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCarrierSerialNum=cardCarrierSerialNum, auditPassedCount=auditPassedCount, cardFlashSize=cardFlashSize, cardTestName=cardTestName, portDplxStatus=portDplxStatus, portNumChans=portNumChans, cardShelfTestIndex=cardShelfTestIndex, diskVolumeEntry=diskVolumeEntry, consoleOutput=consoleOutput, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditAutoScheduling=auditAutoScheduling, portEntry=portEntry, cardLastBootVersion=cardLastBootVersion, cardLastChangeTime=cardLastChangeTime, cardDetectedChange=cardDetectedChange, auditResult=auditResult, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeTable=diskVolumeTable, cardCpuSpeed=cardCpuSpeed, equipmentError=equipmentError, equipmentStateGroup=equipmentStateGroup, diskVolumeDiskSize=diskVolumeDiskSize, cardPrStateChange=cardPrStateChange, TestSchedule=TestSchedule, shelfSwVersion=shelfSwVersion, cardSubDetected=cardSubDetected, cardBootVersion=cardBootVersion, portDetectedMode=portDetectedMode, cardBusSpeed=cardBusSpeed, auditCardId=auditCardId, portCardId=portCardId, cmResetNotification=cmResetNotification, equipmentTbl=equipmentTbl, errEventId=errEventId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestType=cardTestType, diskDriveNumber=diskDriveNumber, equipmentState=equipmentState, auditId=auditId, portNumber=portNumber, portDocsIfIndex=portDocsIfIndex, cardTempOverHeatNotification=cardTempOverHeatNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeGroup=diskVolumeGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardSecState=cardSecState, portPrState=portPrState, cardAdminState=cardAdminState, diagTestId=diagTestId, portTxFlowControlMode=portTxFlowControlMode, equipmentShelfGroup=equipmentShelfGroup, portSecStateChange=portSecStateChange, cardTestIndex=cardTestIndex, cardCpuType=cardCpuType, trapCounter=trapCounter, cardName=cardName, portCardSubType=portCardSubType)
