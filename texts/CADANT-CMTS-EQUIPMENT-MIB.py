...
PortId, DuplexStatus, DiskVolumeUsageLevel, AdminState, PicType, PortMode, CardId, PrimaryState, UnknownState, FlowControlMode, SecondaryState, ShelfId, PortDetectedMode, FirmwareSource, EqActionType, CardSubType, CardType, PortType = mibBuilder.importSymbols("CADANT-TC", "PortId", "DuplexStatus", "DiskVolumeUsageLevel", "AdminState", "PicType", "PortMode", "CardId", "PrimaryState", "UnknownState", "FlowControlMode", "SecondaryState", "ShelfId", "PortDetectedMode", "FirmwareSource", "EqActionType", "CardSubType", "CardType", "PortType")
...
RowStatus, DateAndTime, TimeStamp, DisplayString, TruthValue, TextualConvention, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "TimeStamp", "DisplayString", "TruthValue", "TextualConvention", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portId=portId, cmResetClearNotification=cmResetClearNotification, cardTestTable=cardTestTable, cardTestIndex=cardTestIndex, auditId=auditId, fpgaErrorEventGroup=fpgaErrorEventGroup, auditPassedCount=auditPassedCount, auditResult=auditResult, portSecStateChange=portSecStateChange, cardPrState=cardPrState, cardTestDescription=cardTestDescription, cmUChannelID=cmUChannelID, portCardId=portCardId, cardTempNoReportNotification=cardTempNoReportNotification, cardTestType=cardTestType, cardPicSerialNum=cardPicSerialNum, auditCommand=auditCommand, shelfNumber=shelfNumber, cardName=cardName, cardFpgaSource=cardFpgaSource, cardAction=cardAction, auditCardId=auditCardId, errEventId=errEventId, cardLastChangeTime=cardLastChangeTime, cardEntry=cardEntry, cardSerialNum=cardSerialNum, TestResult=TestResult, portMinDsPower=portMinDsPower, errEvRowStatus=errEvRowStatus, shelfName=shelfName, cardFwUpdateStatus=cardFwUpdateStatus, cmVendor=cmVendor, errEvLogLevel=errEvLogLevel, auditDescription=auditDescription, diskVolumeName=diskVolumeName, cardCpuSpeed=cardCpuSpeed, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portPrStateChange=portPrStateChange, cardType=cardType, cmDevice=cmDevice, auditName=auditName, equipmentState=equipmentState, portAdminState=portAdminState, cardTestTime=cardTestTime, diskVolumeUsageNotification=diskVolumeUsageNotification, cardLastResetReason=cardLastResetReason, cardPicHwVersion=cardPicHwVersion, cardTable=cardTable, equipmentDiag=equipmentDiag, downstreamPowerLoss=downstreamPowerLoss, errEvLoggingEnabled=errEvLoggingEnabled, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSpareGroupId=cardSpareGroupId, portAction=portAction, portTxFlowControlMode=portTxFlowControlMode, eqDiagConfig=eqDiagConfig, auditLogThrottle=auditLogThrottle, cardSecState=cardSecState, cardDetected=cardDetected, diskVolumeSize=diskVolumeSize, cardBusSpeed=cardBusSpeed, cardDetectedChange=cardDetectedChange, portRxFlowControlMode=portRxFlowControlMode, systemKey=systemKey, portDescription=portDescription, diskVolumeDriveId=diskVolumeDriveId, shelfObjects=shelfObjects, cmRegistrationNotification=cmRegistrationNotification, cardCarrierFwVersion=cardCarrierFwVersion, shelfSwVersion=shelfSwVersion, auditStatus=auditStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeTrapInh=diskVolumeTrapInh, portDocsIfIndex=portDocsIfIndex, cardTestId=cardTestId, cardSubDetected=cardSubDetected, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestTransId=cardTestTransId, equipmentPortGroup=equipmentPortGroup, portLastChangeTime=portLastChangeTime, equipmentAuditGroup=equipmentAuditGroup, portRxFlowControlDetected=portRxFlowControlDetected, auditAutoScheduling=auditAutoScheduling, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSubType=cardSubType, cardTestResultNotification=cardTestResultNotification, cardTestEntry=cardTestEntry, cmResetUpTime=cmResetUpTime, cmResetNotification=cmResetNotification, remainInDiagMode=remainInDiagMode, systemGeneral=systemGeneral, TestCommand=TestCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, cmMacAddress=cmMacAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, trapSeverity=trapSeverity, cardFlashSize=cardFlashSize, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeFileName=diskVolumeFileName, auditTotalFailedCount=auditTotalFailedCount, equipmentStateGroup=equipmentStateGroup, cmResetReason=cmResetReason, cardLastBootVersion=cardLastBootVersion, diskVolumeShelfId=diskVolumeShelfId, cardSwVersion=cardSwVersion, portMode=portMode, cardFwVersion=cardFwVersion, cardTrapInh=cardTrapInh, equipmentCardGroup=equipmentCardGroup, portMaxDsPower=portMaxDsPower, portSecState=portSecState, cardTestResult=cardTestResult, portTrapInh=portTrapInh, cmIpAddress=cmIpAddress, diskDriveNumber=diskDriveNumber, portConnectorId=portConnectorId, diskVolumeGroup=diskVolumeGroup, portCardSubType=portCardSubType, TestSchedule=TestSchedule, portDetectedMode=portDetectedMode, cardTestSchedule=cardTestSchedule, portMacIfIndex=portMacIfIndex, equipmentShelfGroup=equipmentShelfGroup, cardNumPorts=cardNumPorts, auditEntry=auditEntry, auditTotalPassedCount=auditTotalPassedCount, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portBgpId=portBgpId, diskVolumeId=diskVolumeId, cardCarrierSerialNum=cardCarrierSerialNum, auditLogOutput=auditLogOutput, portCurrDsPower=portCurrDsPower, diskVolumeCardId=diskVolumeCardId, auditTime=auditTime, cardTestCommand=cardTestCommand, cmPrimarySID=cmPrimarySID, portMacAddress=portMacAddress, portDplxStatusChange=portDplxStatusChange, equipmentTraps=equipmentTraps, diskVolumeTable=diskVolumeTable, equipmentDiagGroup=equipmentDiagGroup, cardId=cardId, portTable=portTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentCardTestGroup=equipmentCardTestGroup, portDplxStatus=portDplxStatus, auditTable=auditTable, TestId=TestId, cardUpTime=cardUpTime, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentAudit=equipmentAudit, portType=portType, cardNumber=cardNumber, cardRamSize=cardRamSize, verboseLevel=verboseLevel, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastBootSource=cardLastBootSource, cardTestName=cardTestName, cardTemperature=cardTemperature, diskVolumeNumber=diskVolumeNumber, diskVolumeEntry=diskVolumeEntry, cardAdminState=cardAdminState, cardShelfId=cardShelfId, auditCycleCount=auditCycleCount, systemClock=systemClock, systemGeneralGroup=systemGeneralGroup, TestTransactionId=TestTransactionId, cardBootVersion=cardBootVersion, cardSecStateChange=cardSecStateChange, cardSpareGroupMode=cardSpareGroupMode, equipmentError=equipmentError, cardCarrierHwVersion=cardCarrierHwVersion, cmResetInfo=cmResetInfo, trapCounter=trapCounter, cardHwVersion=cardHwVersion, cardShelfTestIndex=cardShelfTestIndex, diskVolumeDiskSize=diskVolumeDiskSize, cmResetStatus=cmResetStatus, portTxFlowControlDetected=portTxFlowControlDetected, TestType=TestType, cardTemperatureHighError=cardTemperatureHighError, equipmentTbl=equipmentTbl, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardDplxStatusChange=cardDplxStatusChange, portNumber=portNumber, auditShelfId=auditShelfId, cardPicDetected=cardPicDetected, diagTestId=diagTestId, cardCPLDVersion=cardCPLDVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCpuType=cardCpuType, cardPrStateChange=cardPrStateChange, portPrState=portPrState, auditFailedCount=auditFailedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, portNumChans=portNumChans, portShelfId=portShelfId, cardTestScheduleCommand=cardTestScheduleCommand, TestScheduleCommand=TestScheduleCommand, cardDplxStatus=cardDplxStatus, portEntry=portEntry, consoleOutput=consoleOutput, cardTestResultDesc=cardTestResultDesc)
