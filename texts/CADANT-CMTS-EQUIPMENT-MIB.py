...
PortId, FirmwareSource, PrimaryState, CardType, ShelfId, PortType, EqActionType, CardSubType, DiskVolumeUsageLevel, PortDetectedMode, SecondaryState, UnknownState, DuplexStatus, PortMode, CardId, AdminState, PicType, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "PortId", "FirmwareSource", "PrimaryState", "CardType", "ShelfId", "PortType", "EqActionType", "CardSubType", "DiskVolumeUsageLevel", "PortDetectedMode", "SecondaryState", "UnknownState", "DuplexStatus", "PortMode", "CardId", "AdminState", "PicType", "FlowControlMode")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSubType=cardSubType, portMacAddress=portMacAddress, trapCounter=trapCounter, portEntry=portEntry, portMode=portMode, equipmentDiagGroup=equipmentDiagGroup, cardTestId=cardTestId, portRxFlowControlMode=portRxFlowControlMode, cardPicHwVersion=cardPicHwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTrapInh=cardTrapInh, auditStatus=auditStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardType=cardType, diskVolumeNumber=diskVolumeNumber, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, cardUpTime=cardUpTime, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetStatus=cmResetStatus, diagTestId=diagTestId, systemClock=systemClock, cardSecState=cardSecState, cardName=cardName, portId=portId, cardTestName=cardTestName, portDocsIfIndex=portDocsIfIndex, cardAction=cardAction, cardCPLDVersion=cardCPLDVersion, portTxFlowControlMode=portTxFlowControlMode, cardTestIndex=cardTestIndex, portDplxStatus=portDplxStatus, cmIpAddress=cmIpAddress, downstreamPowerLoss=downstreamPowerLoss, diskVolumeName=diskVolumeName, cmUChannelID=cmUChannelID, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeTable=diskVolumeTable, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetReason=cmResetReason, cardSerialNum=cardSerialNum, systemGeneral=systemGeneral, equipmentAudit=equipmentAudit, cardEntry=cardEntry, equipmentTraps=equipmentTraps, remainInDiagMode=remainInDiagMode, auditName=auditName, portCurrDsPower=portCurrDsPower, equipmentPortGroup=equipmentPortGroup, portTable=portTable, cardPrStateChange=cardPrStateChange, auditTable=auditTable, consoleOutput=consoleOutput, portMaxDsPower=portMaxDsPower, cadEquipmentMib=cadEquipmentMib, errEventId=errEventId, cardTemperatureHighError=cardTemperatureHighError, cardTestEntry=cardTestEntry, cardPicDetected=cardPicDetected, systemGeneralGroup=systemGeneralGroup, portBgpId=portBgpId, auditShelfId=auditShelfId, errEvRowStatus=errEvRowStatus, portPrStateChange=portPrStateChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardPicSerialNum=cardPicSerialNum, diskVolumeShelfId=diskVolumeShelfId, cardFwVersion=cardFwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestResultDesc=cardTestResultDesc, auditTotalFailedCount=auditTotalFailedCount, cmResetUpTime=cmResetUpTime, cmResetNotification=cmResetNotification, auditEntry=auditEntry, cardSecStateChange=cardSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portCardSubType=portCardSubType, cardSpareGroupMode=cardSpareGroupMode, auditTotalPassedCount=auditTotalPassedCount, portMacIfIndex=portMacIfIndex, TestScheduleCommand=TestScheduleCommand, equipmentDiag=equipmentDiag, shelfSwVersion=shelfSwVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeFileName=diskVolumeFileName, equipmentCardTestGroup=equipmentCardTestGroup, cmVendor=cmVendor, cardShelfId=cardShelfId, cardSwVersion=cardSwVersion, cardAdminState=cardAdminState, diskVolumeEntry=diskVolumeEntry, cardHwVersion=cardHwVersion, cardTestDescription=cardTestDescription, portSecState=portSecState, cardDetectedChange=cardDetectedChange, TestCommand=TestCommand, fpgaErrorEventGroup=fpgaErrorEventGroup, cardBusSpeed=cardBusSpeed, cardNumber=cardNumber, cardLastBootSource=cardLastBootSource, cardTempNoReportNotification=cardTempNoReportNotification, systemKey=systemKey, cardTestType=cardTestType, TestId=TestId, cardCpuType=cardCpuType, cardCpuSpeed=cardCpuSpeed, cardTestTable=cardTestTable, cardShelfTestIndex=cardShelfTestIndex, cardFlashSize=cardFlashSize, TestResult=TestResult, TestType=TestType, cardFpgaSource=cardFpgaSource, cmPrimarySID=cmPrimarySID, cardDplxStatus=cardDplxStatus, cardRamSize=cardRamSize, portDetectedMode=portDetectedMode, cardPrState=cardPrState, diskVolumeSize=diskVolumeSize, cmDevice=cmDevice, cmResetInfo=cmResetInfo, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDplxStatusChange=portDplxStatusChange, cardLastChangeTime=cardLastChangeTime, cmResetClearNotification=cmResetClearNotification, auditCommand=auditCommand, cardDetected=cardDetected, equipmentTbl=equipmentTbl, portSecStateChange=portSecStateChange, auditLogThrottle=auditLogThrottle, cardTestResult=cardTestResult, errEvLoggingEnabled=errEvLoggingEnabled, portTrapInh=portTrapInh, eqDiagConfig=eqDiagConfig, cardTestTransId=cardTestTransId, auditTime=auditTime, equipmentCardGroup=equipmentCardGroup, diskVolumeGroup=diskVolumeGroup, equipmentState=equipmentState, cardTestTime=cardTestTime, equipmentError=equipmentError, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestTransactionId=TestTransactionId, portCardId=portCardId, shelfName=shelfName, errEvLogLevel=errEvLogLevel, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastBootVersion=cardLastBootVersion, portShelfId=portShelfId, auditPassedCount=auditPassedCount, PYSNMP_MODULE_ID=cadEquipmentMib, cardFwUpdateStatus=cardFwUpdateStatus, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeId=diskVolumeId, auditCycleCount=auditCycleCount, portDescription=portDescription, cardTestSchedule=cardTestSchedule, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestResultNotification=cardTestResultNotification, auditAutoScheduling=auditAutoScheduling, cardCarrierFwVersion=cardCarrierFwVersion, equipmentStateGroup=equipmentStateGroup, portMinDsPower=portMinDsPower, diskVolumeTrapInh=diskVolumeTrapInh, cardTemperature=cardTemperature, portNumber=portNumber, cardLastResetReason=cardLastResetReason, cardSpareGroupId=cardSpareGroupId, shelfObjects=shelfObjects, equipmentAuditGroup=equipmentAuditGroup, cardNumPorts=cardNumPorts, diskVolumeCardId=diskVolumeCardId, auditLogOutput=auditLogOutput, cardCarrierSerialNum=cardCarrierSerialNum, fpgaErrorEventTable=fpgaErrorEventTable, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSubDetected=cardSubDetected, auditFailedCount=auditFailedCount, portType=portType, verboseLevel=verboseLevel, portAdminState=portAdminState, auditDescription=auditDescription, cmMacAddress=cmMacAddress, cardDplxStatusChange=cardDplxStatusChange, shelfNumber=shelfNumber, auditResult=auditResult, auditId=auditId, equipmentShelfGroup=equipmentShelfGroup, portLastChangeTime=portLastChangeTime, portNumChans=portNumChans, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestScheduleCommand=cardTestScheduleCommand, cardTable=cardTable, trapSeverity=trapSeverity, TestSchedule=TestSchedule, auditCardId=auditCardId, cardBootVersion=cardBootVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, portPrState=portPrState, portTxFlowControlDetected=portTxFlowControlDetected, cardTestCommand=cardTestCommand, diskDriveNumber=diskDriveNumber, portAction=portAction, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardId=cardId, diskVolumeDriveId=diskVolumeDriveId, portConnectorId=portConnectorId)
