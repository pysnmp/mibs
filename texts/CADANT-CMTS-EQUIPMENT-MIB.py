...
PrimaryState, SecondaryState, FlowControlMode, CardId, EqActionType, AdminState, DiskVolumeUsageLevel, CardType, DuplexStatus, PortId, FirmwareSource, PortType, PicType, UnknownState, CardSubType, ShelfId, PortDetectedMode, PortMode = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "SecondaryState", "FlowControlMode", "CardId", "EqActionType", "AdminState", "DiskVolumeUsageLevel", "CardType", "DuplexStatus", "PortId", "FirmwareSource", "PortType", "PicType", "UnknownState", "CardSubType", "ShelfId", "PortDetectedMode", "PortMode")
...
DisplayString, TruthValue, TimeStamp, TextualConvention, RowStatus, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TruthValue", "TimeStamp", "TextualConvention", "RowStatus", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFwVersion=cardFwVersion, portMacIfIndex=portMacIfIndex, portEntry=portEntry, portPrState=portPrState, diagTestId=diagTestId, cmResetUpTime=cmResetUpTime, cardPrState=cardPrState, diskVolumeDiskSize=diskVolumeDiskSize, cardBusSpeed=cardBusSpeed, fpgaErrorEventEntry=fpgaErrorEventEntry, cardEntry=cardEntry, equipmentAudit=equipmentAudit, cardDplxStatusChange=cardDplxStatusChange, auditName=auditName, downstreamPowerLoss=downstreamPowerLoss, auditFailedCount=auditFailedCount, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portAdminState=portAdminState, portMaxDsPower=portMaxDsPower, cadEquipmentMibGroup=cadEquipmentMibGroup, portDescription=portDescription, portId=portId, remainInDiagMode=remainInDiagMode, cmResetStatus=cmResetStatus, shelfObjects=shelfObjects, equipmentPortGroup=equipmentPortGroup, shelfSwVersion=shelfSwVersion, portTxFlowControlDetected=portTxFlowControlDetected, cardCarrierSerialNum=cardCarrierSerialNum, cardName=cardName, auditTable=auditTable, fpgaErrorEventTable=fpgaErrorEventTable, cardPrStateChange=cardPrStateChange, cardAction=cardAction, portTrapInh=portTrapInh, cardShelfTestIndex=cardShelfTestIndex, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portDplxStatusChange=portDplxStatusChange, diskVolumeNumber=diskVolumeNumber, portMinDsPower=portMinDsPower, cmDevice=cmDevice, portLastChangeTime=portLastChangeTime, cardUpTime=cardUpTime, cardFwUpdateStatus=cardFwUpdateStatus, cardBootVersion=cardBootVersion, auditStatus=auditStatus, auditTotalFailedCount=auditTotalFailedCount, auditAutoScheduling=auditAutoScheduling, cardTestCommand=cardTestCommand, cardTestIndex=cardTestIndex, equipmentError=equipmentError, cadEquipmentMibConformance=cadEquipmentMibConformance, portCurrDsPower=portCurrDsPower, cardTempNoReportNotification=cardTempNoReportNotification, errEvLoggingEnabled=errEvLoggingEnabled, auditEntry=auditEntry, errEvLogLevel=errEvLogLevel, equipmentCardTestGroup=equipmentCardTestGroup, cardTestType=cardTestType, auditCommand=auditCommand, diskVolumeId=diskVolumeId, cardCpuSpeed=cardCpuSpeed, equipmentCardGroup=equipmentCardGroup, auditCardId=auditCardId, shelfName=shelfName, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentDiag=equipmentDiag, consoleOutput=consoleOutput, auditLogThrottle=auditLogThrottle, diskVolumeSize=diskVolumeSize, cardSecState=cardSecState, auditShelfId=auditShelfId, cardDetected=cardDetected, cardTestId=cardTestId, diskVolumeUsageLevel=diskVolumeUsageLevel, cmUChannelID=cmUChannelID, cmResetInfo=cmResetInfo, cardTemperatureHighError=cardTemperatureHighError, TestType=TestType, cardShelfId=cardShelfId, cardCPLDVersion=cardCPLDVersion, cardPicSerialNum=cardPicSerialNum, portType=portType, cardNumber=cardNumber, cardSubType=cardSubType, cmMacAddress=cmMacAddress, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeFileName=diskVolumeFileName, portConnectorId=portConnectorId, cardLastBootSource=cardLastBootSource, portTable=portTable, portShelfId=portShelfId, cardSpareGroupId=cardSpareGroupId, cardTempOverHeatNotification=cardTempOverHeatNotification, TestResult=TestResult, cardLastBootVersion=cardLastBootVersion, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeShelfId=diskVolumeShelfId, trapSeverity=trapSeverity, diskVolumeUsageNotification=diskVolumeUsageNotification, cardHwVersion=cardHwVersion, diskVolumeEntry=diskVolumeEntry, errEvRowStatus=errEvRowStatus, diskDriveNumber=diskDriveNumber, systemGeneralGroup=systemGeneralGroup, systemClock=systemClock, cardLastResetReason=cardLastResetReason, cmVendor=cmVendor, portNumChans=portNumChans, portDetectedMode=portDetectedMode, verboseLevel=verboseLevel, cardTestSchedule=cardTestSchedule, diskVolumeCardId=diskVolumeCardId, portRxFlowControlMode=portRxFlowControlMode, equipmentAuditGroup=equipmentAuditGroup, cmIpAddress=cmIpAddress, portSecState=portSecState, cardId=cardId, trapCounter=trapCounter, TestId=TestId, cardLastChangeTime=cardLastChangeTime, cardTestTransId=cardTestTransId, diskVolumeName=diskVolumeName, TestCommand=TestCommand, auditDescription=auditDescription, TestSchedule=TestSchedule, diskVolumeTable=diskVolumeTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestDescription=cardTestDescription, cardTable=cardTable, cardTestEntry=cardTestEntry, portMacAddress=portMacAddress, portMode=portMode, auditId=auditId, portBgpId=portBgpId, cmResetReason=cmResetReason, shelfNumber=shelfNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, TestScheduleCommand=TestScheduleCommand, auditTotalPassedCount=auditTotalPassedCount, diskVolumeDriveId=diskVolumeDriveId, cardTestResultNotification=cardTestResultNotification, cmResetNotification=cmResetNotification, equipmentStateGroup=equipmentStateGroup, cardTestTable=cardTestTable, TestTransactionId=TestTransactionId, cardCpuType=cardCpuType, auditTime=auditTime, auditCycleCount=auditCycleCount, errEvRecoveryEnabled=errEvRecoveryEnabled, cardDplxStatus=cardDplxStatus, cardTestTime=cardTestTime, cardFpgaSource=cardFpgaSource, systemKey=systemKey, equipmentState=equipmentState, diskVolumeTrapInh=diskVolumeTrapInh, portSecStateChange=portSecStateChange, cardTestResult=cardTestResult, cardTestScheduleCommand=cardTestScheduleCommand, cardNumPorts=cardNumPorts, portAction=portAction, cardAdminState=cardAdminState, eqDiagConfig=eqDiagConfig, portCardId=portCardId, cardSerialNum=cardSerialNum, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPicHwVersion=cardPicHwVersion, equipmentDiagGroup=equipmentDiagGroup, portDplxStatus=portDplxStatus, equipmentTraps=equipmentTraps, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestName=cardTestName, cardSecStateChange=cardSecStateChange, cadEquipmentMib=cadEquipmentMib, cardSwVersion=cardSwVersion, diskVolumeGroup=diskVolumeGroup, cardTemperature=cardTemperature, cardRamSize=cardRamSize, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, errEventId=errEventId, cardSubDetected=cardSubDetected, portNumber=portNumber, cardPicDetected=cardPicDetected, cardSpareGroupMode=cardSpareGroupMode, auditResult=auditResult, cardTemperatureHighWarn=cardTemperatureHighWarn, cardType=cardType, cardDetectedChange=cardDetectedChange, cardCarrierHwVersion=cardCarrierHwVersion, portDocsIfIndex=portDocsIfIndex, cardTrapInh=cardTrapInh, cardFlashSize=cardFlashSize, cardTestResultDesc=cardTestResultDesc, portCardSubType=portCardSubType, portPrStateChange=portPrStateChange, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, systemGeneral=systemGeneral, cmPrimarySID=cmPrimarySID, cmResetClearNotification=cmResetClearNotification, auditPassedCount=auditPassedCount, auditLogOutput=auditLogOutput, equipmentTbl=equipmentTbl)
