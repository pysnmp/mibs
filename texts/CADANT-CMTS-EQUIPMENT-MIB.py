...
CardId, SecondaryState, EqActionType, PortDetectedMode, PortMode, AdminState, DiskVolumeUsageLevel, CardSubType, PortType, FlowControlMode, ShelfId, CardType, FirmwareSource, PicType, PortId, PrimaryState, UnknownState, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "CardId", "SecondaryState", "EqActionType", "PortDetectedMode", "PortMode", "AdminState", "DiskVolumeUsageLevel", "CardSubType", "PortType", "FlowControlMode", "ShelfId", "CardType", "FirmwareSource", "PicType", "PortId", "PrimaryState", "UnknownState", "DuplexStatus")
...
RowStatus, DisplayString, MacAddress, DateAndTime, TextualConvention, TimeStamp, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "MacAddress", "DateAndTime", "TextualConvention", "TimeStamp", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPicSerialNum=cardPicSerialNum, cardNumPorts=cardNumPorts, portMode=portMode, portCardSubType=portCardSubType, fpgaErrorEventGroup=fpgaErrorEventGroup, fpgaErrorEventTable=fpgaErrorEventTable, auditAutoScheduling=auditAutoScheduling, portTxFlowControlMode=portTxFlowControlMode, cardEntry=cardEntry, auditResult=auditResult, cardFwUpdateStatus=cardFwUpdateStatus, cardSpareGroupMode=cardSpareGroupMode, cardSpareGroupId=cardSpareGroupId, cardSubType=cardSubType, cardSecState=cardSecState, portId=portId, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResultDesc=cardTestResultDesc, diskVolumeId=diskVolumeId, diskVolumeEntry=diskVolumeEntry, portNumChans=portNumChans, portCurrDsPower=portCurrDsPower, auditLogOutput=auditLogOutput, cardCarrierSerialNum=cardCarrierSerialNum, auditTime=auditTime, portDplxStatusChange=portDplxStatusChange, cardDplxStatusChange=cardDplxStatusChange, cardTestName=cardTestName, portLastChangeTime=portLastChangeTime, auditDescription=auditDescription, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPicHwVersion=cardPicHwVersion, equipmentStateGroup=equipmentStateGroup, errEvRowStatus=errEvRowStatus, portDplxStatus=portDplxStatus, cardId=cardId, downstreamPowerLoss=downstreamPowerLoss, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTable=cardTable, portPrState=portPrState, diskVolumeSize=diskVolumeSize, TestResult=TestResult, portRxFlowControlMode=portRxFlowControlMode, cardName=cardName, cardTestDescription=cardTestDescription, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmUChannelID=cmUChannelID, cardLastResetReason=cardLastResetReason, portSecStateChange=portSecStateChange, cardTestResult=cardTestResult, cmMacAddress=cmMacAddress, shelfObjects=shelfObjects, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestScheduleCommand=cardTestScheduleCommand, equipmentDiag=equipmentDiag, systemGeneral=systemGeneral, TestCommand=TestCommand, portAdminState=portAdminState, cardLastBootSource=cardLastBootSource, cardTestCommand=cardTestCommand, auditName=auditName, cardDplxStatus=cardDplxStatus, shelfName=shelfName, cardCpuType=cardCpuType, cardCarrierFwVersion=cardCarrierFwVersion, portAction=portAction, diskVolumeShelfId=diskVolumeShelfId, equipmentPortGroup=equipmentPortGroup, cardSerialNum=cardSerialNum, cardPrState=cardPrState, portType=portType, cardBusSpeed=cardBusSpeed, portDescription=portDescription, diskVolumeDiskSize=diskVolumeDiskSize, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeGroup=diskVolumeGroup, equipmentCardGroup=equipmentCardGroup, cadEquipmentMib=cadEquipmentMib, auditTable=auditTable, auditFailedCount=auditFailedCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestType=cardTestType, equipmentError=equipmentError, cardRamSize=cardRamSize, portPrStateChange=portPrStateChange, portMacIfIndex=portMacIfIndex, auditStatus=auditStatus, cardShelfId=cardShelfId, cardAction=cardAction, portDocsIfIndex=portDocsIfIndex, consoleOutput=consoleOutput, auditCycleCount=auditCycleCount, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardLastBootVersion=cardLastBootVersion, auditTotalFailedCount=auditTotalFailedCount, cardCPLDVersion=cardCPLDVersion, cmRegistrationNotification=cmRegistrationNotification, cardBootVersion=cardBootVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeFileName=diskVolumeFileName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCarrierHwVersion=cardCarrierHwVersion, portConnectorId=portConnectorId, cardTestTable=cardTestTable, cardFlashSize=cardFlashSize, cmIpAddress=cmIpAddress, portTxFlowControlDetected=portTxFlowControlDetected, equipmentTbl=equipmentTbl, cardSubDetected=cardSubDetected, diskVolumeTable=diskVolumeTable, shelfSwVersion=shelfSwVersion, cmVendor=cmVendor, TestTransactionId=TestTransactionId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardAdminState=cardAdminState, cardTestTransId=cardTestTransId, portEntry=portEntry, cardSwVersion=cardSwVersion, systemClock=systemClock, cmResetInfo=cmResetInfo, cardTestResultNotification=cardTestResultNotification, cardShelfTestIndex=cardShelfTestIndex, verboseLevel=verboseLevel, portCardId=portCardId, systemGeneralGroup=systemGeneralGroup, cardNumber=cardNumber, cardPicDetected=cardPicDetected, cardUpTime=cardUpTime, cardTestEntry=cardTestEntry, cmResetNotification=cmResetNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, errEvLogLevel=errEvLogLevel, eqDiagConfig=eqDiagConfig, PYSNMP_MODULE_ID=cadEquipmentMib, cardSecStateChange=cardSecStateChange, portNumber=portNumber, portMacAddress=portMacAddress, diskDriveNumber=diskDriveNumber, cmResetClearNotification=cmResetClearNotification, cardTestSchedule=cardTestSchedule, cmPrimarySID=cmPrimarySID, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portShelfId=portShelfId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTrapInh=cardTrapInh, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeName=diskVolumeName, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentTraps=equipmentTraps, diskVolumeCardId=diskVolumeCardId, cmDevice=cmDevice, errEventId=errEventId, shelfNumber=shelfNumber, portDetectedMode=portDetectedMode, equipmentDiagGroup=equipmentDiagGroup, cardFpgaSource=cardFpgaSource, auditLogThrottle=auditLogThrottle, cadEquipmentMibConformance=cadEquipmentMibConformance, auditId=auditId, systemKey=systemKey, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeDriveId=diskVolumeDriveId, cardLastChangeTime=cardLastChangeTime, cmResetStatus=cmResetStatus, auditCardId=auditCardId, trapSeverity=trapSeverity, cmResetReason=cmResetReason, remainInDiagMode=remainInDiagMode, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestId=cardTestId, cardTemperature=cardTemperature, portTable=portTable, equipmentAuditGroup=equipmentAuditGroup, cardType=cardType, auditCommand=auditCommand, auditTotalPassedCount=auditTotalPassedCount, equipmentShelfGroup=equipmentShelfGroup, equipmentAudit=equipmentAudit, TestType=TestType, cardHwVersion=cardHwVersion, trapCounter=trapCounter, diskVolumeUsageLevel=diskVolumeUsageLevel, diagTestId=diagTestId, cardPrStateChange=cardPrStateChange, portBgpId=portBgpId, portSecState=portSecState, equipmentState=equipmentState, cardTestTime=cardTestTime, portMaxDsPower=portMaxDsPower, cardTestIndex=cardTestIndex, TestSchedule=TestSchedule, portTrapInh=portTrapInh, equipmentCardTestGroup=equipmentCardTestGroup, auditShelfId=auditShelfId, auditEntry=auditEntry, auditPassedCount=auditPassedCount, cmResetUpTime=cmResetUpTime, cardFwVersion=cardFwVersion, diskVolumeNumber=diskVolumeNumber, cardTemperatureHighError=cardTemperatureHighError, cardDetectedChange=cardDetectedChange, cardDetected=cardDetected, diskVolumeTrapInh=diskVolumeTrapInh, cardCpuSpeed=cardCpuSpeed, TestScheduleCommand=TestScheduleCommand, portMinDsPower=portMinDsPower, TestId=TestId)
