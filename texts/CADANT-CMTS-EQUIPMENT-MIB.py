...
FlowControlMode, CardSubType, DuplexStatus, FirmwareSource, PrimaryState, SecondaryState, PicType, CardType, PortType, PortId, UnknownState, EqActionType, AdminState, CardId, PortMode, PortDetectedMode, ShelfId, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "CardSubType", "DuplexStatus", "FirmwareSource", "PrimaryState", "SecondaryState", "PicType", "CardType", "PortType", "PortId", "UnknownState", "EqActionType", "AdminState", "CardId", "PortMode", "PortDetectedMode", "ShelfId", "DiskVolumeUsageLevel")
...
TextualConvention, DateAndTime, RowStatus, TruthValue, MacAddress, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "RowStatus", "TruthValue", "MacAddress", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portPrStateChange=portPrStateChange, portEntry=portEntry, errEvRowStatus=errEvRowStatus, cardPicHwVersion=cardPicHwVersion, cardTestTransId=cardTestTransId, cardSerialNum=cardSerialNum, cardCarrierHwVersion=cardCarrierHwVersion, auditAutoScheduling=auditAutoScheduling, cardPrState=cardPrState, auditLogOutput=auditLogOutput, cardName=cardName, auditShelfId=auditShelfId, cmResetClearNotification=cmResetClearNotification, fpgaErrorEventTable=fpgaErrorEventTable, portAction=portAction, systemKey=systemKey, cardNumber=cardNumber, cardTempNoReportNotification=cardTempNoReportNotification, auditTable=auditTable, cardCpuSpeed=cardCpuSpeed, cardTempOverHeatNotification=cardTempOverHeatNotification, cmResetStatus=cmResetStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageNotification=diskVolumeUsageNotification, systemClock=systemClock, auditFailedCount=auditFailedCount, diskVolumeShelfId=diskVolumeShelfId, cardTestIndex=cardTestIndex, auditTotalPassedCount=auditTotalPassedCount, cardCpuType=cardCpuType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestType=TestType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portBgpId=portBgpId, portAdminState=portAdminState, cardTestTable=cardTestTable, cardDplxStatusChange=cardDplxStatusChange, portNumChans=portNumChans, cardTestId=cardTestId, cardTestEntry=cardTestEntry, cardLastResetReason=cardLastResetReason, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSwVersion=cardSwVersion, cardTestResultDesc=cardTestResultDesc, auditName=auditName, diskVolumeFileName=diskVolumeFileName, portTable=portTable, auditCycleCount=auditCycleCount, diskVolumeNumber=diskVolumeNumber, cardTestName=cardTestName, cardBusSpeed=cardBusSpeed, TestSchedule=TestSchedule, equipmentCardGroup=equipmentCardGroup, diskVolumeEntry=diskVolumeEntry, auditId=auditId, cmIpAddress=cmIpAddress, portLastChangeTime=portLastChangeTime, eqDiagConfig=eqDiagConfig, cardTestDescription=cardTestDescription, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardAdminState=cardAdminState, portMode=portMode, cardFwVersion=cardFwVersion, auditTime=auditTime, cmUChannelID=cmUChannelID, equipmentTbl=equipmentTbl, portMaxDsPower=portMaxDsPower, cardPicDetected=cardPicDetected, equipmentAudit=equipmentAudit, portShelfId=portShelfId, equipmentState=equipmentState, diskVolumeSize=diskVolumeSize, cardDetected=cardDetected, auditStatus=auditStatus, equipmentError=equipmentError, cardNumPorts=cardNumPorts, cardLastChangeTime=cardLastChangeTime, cardEntry=cardEntry, cardShelfId=cardShelfId, cardCarrierSerialNum=cardCarrierSerialNum, cmDevice=cmDevice, portCardId=portCardId, TestId=TestId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeGroup=diskVolumeGroup, errEvLogLevel=errEvLogLevel, cardDetectedChange=cardDetectedChange, cardTemperature=cardTemperature, portMinDsPower=portMinDsPower, portConnectorId=portConnectorId, cardCarrierFwVersion=cardCarrierFwVersion, portTxFlowControlDetected=portTxFlowControlDetected, cmRegistrationNotification=cmRegistrationNotification, shelfName=shelfName, cardTemperatureHighWarn=cardTemperatureHighWarn, cardType=cardType, portDplxStatus=portDplxStatus, portSecStateChange=portSecStateChange, portPrState=portPrState, cmResetNotification=cmResetNotification, cardShelfTestIndex=cardShelfTestIndex, auditTotalFailedCount=auditTotalFailedCount, cardSpareGroupId=cardSpareGroupId, cadEquipmentMibConformance=cadEquipmentMibConformance, cmMacAddress=cmMacAddress, cardLastBootVersion=cardLastBootVersion, PYSNMP_MODULE_ID=cadEquipmentMib, TestCommand=TestCommand, portCurrDsPower=portCurrDsPower, portDplxStatusChange=portDplxStatusChange, TestTransactionId=TestTransactionId, cardHwVersion=cardHwVersion, portRxFlowControlDetected=portRxFlowControlDetected, cardTestSchedule=cardTestSchedule, diskVolumeDriveId=diskVolumeDriveId, cardFlashSize=cardFlashSize, auditCommand=auditCommand, cardLastBootSource=cardLastBootSource, portRxFlowControlMode=portRxFlowControlMode, cardTestScheduleCommand=cardTestScheduleCommand, portDescription=portDescription, cardId=cardId, fpgaErrorEventGroup=fpgaErrorEventGroup, remainInDiagMode=remainInDiagMode, cardTemperatureHighError=cardTemperatureHighError, cardSubDetected=cardSubDetected, cardSecStateChange=cardSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portTrapInh=portTrapInh, shelfObjects=shelfObjects, portNumber=portNumber, cardFpgaSource=cardFpgaSource, cmResetInfo=cmResetInfo, cmResetReason=cmResetReason, equipmentCardTestGroup=equipmentCardTestGroup, portId=portId, diskVolumeDiskSize=diskVolumeDiskSize, portSecState=portSecState, verboseLevel=verboseLevel, auditResult=auditResult, equipmentAuditGroup=equipmentAuditGroup, TestResult=TestResult, TestScheduleCommand=TestScheduleCommand, cardFwUpdateStatus=cardFwUpdateStatus, errEventId=errEventId, errEvLoggingEnabled=errEvLoggingEnabled, cmPrimarySID=cmPrimarySID, diskVolumeTable=diskVolumeTable, cardTestTime=cardTestTime, trapSeverity=trapSeverity, portMacIfIndex=portMacIfIndex, cardTestResultNotification=cardTestResultNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, trapCounter=trapCounter, portCardSubType=portCardSubType, cardTable=cardTable, portMacAddress=portMacAddress, equipmentShelfGroup=equipmentShelfGroup, auditEntry=auditEntry, equipmentTraps=equipmentTraps, shelfSwVersion=shelfSwVersion, cardDplxStatus=cardDplxStatus, downstreamPowerLoss=downstreamPowerLoss, auditPassedCount=auditPassedCount, cmVendor=cmVendor, auditCardId=auditCardId, shelfNumber=shelfNumber, cardRamSize=cardRamSize, portDetectedMode=portDetectedMode, diskVolumeName=diskVolumeName, portType=portType, auditDescription=auditDescription, cardTestType=cardTestType, auditLogThrottle=auditLogThrottle, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeTrapInh=diskVolumeTrapInh, consoleOutput=consoleOutput, diagTestId=diagTestId, systemGeneralGroup=systemGeneralGroup, cardSpareGroupMode=cardSpareGroupMode, cmResetUpTime=cmResetUpTime, cardTestCommand=cardTestCommand, diskDriveNumber=diskDriveNumber, cardPrStateChange=cardPrStateChange, cardTestResult=cardTestResult, cardTrapInh=cardTrapInh, cardUpTime=cardUpTime, portDocsIfIndex=portDocsIfIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentDiagGroup=equipmentDiagGroup, portTxFlowControlMode=portTxFlowControlMode, cardPicSerialNum=cardPicSerialNum, cadEquipmentMib=cadEquipmentMib, equipmentPortGroup=equipmentPortGroup, cardSubType=cardSubType, cardCPLDVersion=cardCPLDVersion, equipmentDiag=equipmentDiag, systemGeneral=systemGeneral, cardSecState=cardSecState, cardAction=cardAction, cardBootVersion=cardBootVersion, diskVolumeId=diskVolumeId, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeCardId=diskVolumeCardId)
