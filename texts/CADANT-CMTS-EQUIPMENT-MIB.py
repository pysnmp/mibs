...
PortDetectedMode, AdminState, SecondaryState, CardId, ShelfId, DiskVolumeUsageLevel, FirmwareSource, CardSubType, CardType, FlowControlMode, PortType, DuplexStatus, PrimaryState, UnknownState, PortMode, PortId, PicType, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "AdminState", "SecondaryState", "CardId", "ShelfId", "DiskVolumeUsageLevel", "FirmwareSource", "CardSubType", "CardType", "FlowControlMode", "PortType", "DuplexStatus", "PrimaryState", "UnknownState", "PortMode", "PortId", "PicType", "EqActionType")
...
TextualConvention, DateAndTime, DisplayString, TimeStamp, MacAddress, TruthValue, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "DisplayString", "TimeStamp", "MacAddress", "TruthValue", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTrapInh=portTrapInh, diskDriveNumber=diskDriveNumber, cardTestType=cardTestType, equipmentState=equipmentState, cmPrimarySID=cmPrimarySID, cardName=cardName, auditTime=auditTime, cmDevice=cmDevice, systemGeneral=systemGeneral, cardSubDetected=cardSubDetected, portAdminState=portAdminState, cardTestTransId=cardTestTransId, cardShelfTestIndex=cardShelfTestIndex, auditTotalPassedCount=auditTotalPassedCount, auditTotalFailedCount=auditTotalFailedCount, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestTable=cardTestTable, remainInDiagMode=remainInDiagMode, equipmentPortGroup=equipmentPortGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResult=cardTestResult, cardDetected=cardDetected, cardTestId=cardTestId, cmResetClearNotification=cmResetClearNotification, cardTempNoReportNotification=cardTempNoReportNotification, cardSpareGroupMode=cardSpareGroupMode, auditCycleCount=auditCycleCount, errEventId=errEventId, TestType=TestType, cardRamSize=cardRamSize, auditFailedCount=auditFailedCount, cardAdminState=cardAdminState, cardUpTime=cardUpTime, cardSpareGroupId=cardSpareGroupId, diskVolumeFileName=diskVolumeFileName, equipmentError=equipmentError, portTxFlowControlMode=portTxFlowControlMode, portPrStateChange=portPrStateChange, cmResetNotification=cmResetNotification, auditTable=auditTable, auditName=auditName, cardPicSerialNum=cardPicSerialNum, cardTestDescription=cardTestDescription, equipmentCardGroup=equipmentCardGroup, cardTable=cardTable, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResultNotification=cardTestResultNotification, cardFwUpdateStatus=cardFwUpdateStatus, portAction=portAction, cardType=cardType, auditDescription=auditDescription, TestId=TestId, diagTestId=diagTestId, equipmentTraps=equipmentTraps, portPrState=portPrState, portDplxStatusChange=portDplxStatusChange, portConnectorId=portConnectorId, cmVendor=cmVendor, cardTemperature=cardTemperature, portSecState=portSecState, cardCarrierFwVersion=cardCarrierFwVersion, cmResetStatus=cmResetStatus, cardDplxStatus=cardDplxStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, cardCarrierHwVersion=cardCarrierHwVersion, fpgaErrorEventTable=fpgaErrorEventTable, cardPrStateChange=cardPrStateChange, equipmentDiagGroup=equipmentDiagGroup, cardSecStateChange=cardSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestSchedule=TestSchedule, cardFlashSize=cardFlashSize, auditLogOutput=auditLogOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, consoleOutput=consoleOutput, shelfName=shelfName, diskVolumeTrapInh=diskVolumeTrapInh, cardSwVersion=cardSwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, auditLogThrottle=auditLogThrottle, cmIpAddress=cmIpAddress, diskVolumeShelfId=diskVolumeShelfId, auditShelfId=auditShelfId, cardLastChangeTime=cardLastChangeTime, cardFpgaSource=cardFpgaSource, diskVolumeName=diskVolumeName, portCardSubType=portCardSubType, eqDiagConfig=eqDiagConfig, cardSubType=cardSubType, diskVolumeSize=diskVolumeSize, TestTransactionId=TestTransactionId, cmMacAddress=cmMacAddress, portId=portId, portTable=portTable, portEntry=portEntry, portBgpId=portBgpId, TestScheduleCommand=TestScheduleCommand, cmUChannelID=cmUChannelID, cardDplxStatusChange=cardDplxStatusChange, portRxFlowControlMode=portRxFlowControlMode, portSecStateChange=portSecStateChange, TestCommand=TestCommand, portNumber=portNumber, portMacAddress=portMacAddress, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeNumber=diskVolumeNumber, portDetectedMode=portDetectedMode, cardId=cardId, errEvLogLevel=errEvLogLevel, cardTestEntry=cardTestEntry, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetReason=cmResetReason, trapSeverity=trapSeverity, diskVolumeGroup=diskVolumeGroup, auditAutoScheduling=auditAutoScheduling, diskVolumeCardId=diskVolumeCardId, equipmentCardTestGroup=equipmentCardTestGroup, cardTestName=cardTestName, cardPrState=cardPrState, cardNumber=cardNumber, cardTemperatureHighError=cardTemperatureHighError, cardTrapInh=cardTrapInh, portShelfId=portShelfId, portDocsIfIndex=portDocsIfIndex, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResultDesc=cardTestResultDesc, errEvLoggingEnabled=errEvLoggingEnabled, systemGeneralGroup=systemGeneralGroup, cardEntry=cardEntry, portNumChans=portNumChans, portCurrDsPower=portCurrDsPower, cardCarrierSerialNum=cardCarrierSerialNum, cardTestScheduleCommand=cardTestScheduleCommand, auditId=auditId, auditEntry=auditEntry, auditCommand=auditCommand, cardNumPorts=cardNumPorts, cardSerialNum=cardSerialNum, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicDetected=cardPicDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditResult=auditResult, shelfObjects=shelfObjects, cardSecState=cardSecState, cardAction=cardAction, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardLastBootSource=cardLastBootSource, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portDescription=portDescription, fpgaErrorEventGroup=fpgaErrorEventGroup, portType=portType, cmRegistrationNotification=cmRegistrationNotification, cardShelfId=cardShelfId, trapCounter=trapCounter, cardCPLDVersion=cardCPLDVersion, portMacIfIndex=portMacIfIndex, verboseLevel=verboseLevel, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetInfo=cmResetInfo, cadEquipmentMib=cadEquipmentMib, diskVolumeDiskSize=diskVolumeDiskSize, shelfSwVersion=shelfSwVersion, cardPicHwVersion=cardPicHwVersion, cardTestIndex=cardTestIndex, cardCpuSpeed=cardCpuSpeed, cardFwVersion=cardFwVersion, cardCpuType=cardCpuType, equipmentDiag=equipmentDiag, cmResetUpTime=cmResetUpTime, cardTestTime=cardTestTime, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditCardId=auditCardId, portLastChangeTime=portLastChangeTime, TestResult=TestResult, cardTestCommand=cardTestCommand, auditPassedCount=auditPassedCount, shelfNumber=shelfNumber, cardTestSchedule=cardTestSchedule, cardBusSpeed=cardBusSpeed, cardBootVersion=cardBootVersion, cardLastResetReason=cardLastResetReason, diskVolumeTable=diskVolumeTable, equipmentAuditGroup=equipmentAuditGroup, portMinDsPower=portMinDsPower, systemKey=systemKey, equipmentShelfGroup=equipmentShelfGroup, portMode=portMode, cardHwVersion=cardHwVersion, portDplxStatus=portDplxStatus, auditStatus=auditStatus, systemClock=systemClock, cardDetectedChange=cardDetectedChange, equipmentAudit=equipmentAudit, portRxFlowControlDetected=portRxFlowControlDetected, downstreamPowerLoss=downstreamPowerLoss, portCardId=portCardId, equipmentTbl=equipmentTbl, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeId=diskVolumeId, errEvRowStatus=errEvRowStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portMaxDsPower=portMaxDsPower, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentStateGroup=equipmentStateGroup, diskVolumeDriveId=diskVolumeDriveId)
