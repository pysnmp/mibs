...
SecondaryState, CardId, ShelfId, DuplexStatus, PicType, DiskVolumeUsageLevel, AdminState, EqActionType, FirmwareSource, PortDetectedMode, PortId, PortType, FlowControlMode, PortMode, UnknownState, CardType, CardSubType, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "CardId", "ShelfId", "DuplexStatus", "PicType", "DiskVolumeUsageLevel", "AdminState", "EqActionType", "FirmwareSource", "PortDetectedMode", "PortId", "PortType", "FlowControlMode", "PortMode", "UnknownState", "CardType", "CardSubType", "PrimaryState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfSwVersion=shelfSwVersion, cardDetected=cardDetected, portCardSubType=portCardSubType, portMacIfIndex=portMacIfIndex, diskVolumeEntry=diskVolumeEntry, cardSecStateChange=cardSecStateChange, TestId=TestId, diskVolumeFileName=diskVolumeFileName, cardLastBootSource=cardLastBootSource, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastChangeTime=cardLastChangeTime, diskVolumeName=diskVolumeName, portRxFlowControlMode=portRxFlowControlMode, diskVolumeCardId=diskVolumeCardId, cardTemperatureHighWarn=cardTemperatureHighWarn, portCurrDsPower=portCurrDsPower, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResultNotification=cardTestResultNotification, portTable=portTable, equipmentStateGroup=equipmentStateGroup, portCardId=portCardId, portTxFlowControlDetected=portTxFlowControlDetected, cardTempNoReportNotification=cardTempNoReportNotification, equipmentDiagGroup=equipmentDiagGroup, portEntry=portEntry, portDescription=portDescription, equipmentCardTestGroup=equipmentCardTestGroup, TestResult=TestResult, cardFpgaSource=cardFpgaSource, downstreamPowerLoss=downstreamPowerLoss, errEvRecoveryEnabled=errEvRecoveryEnabled, cmIpAddress=cmIpAddress, diskVolumeDriveId=diskVolumeDriveId, cardType=cardType, portDplxStatusChange=portDplxStatusChange, portShelfId=portShelfId, cardSubType=cardSubType, cardName=cardName, cmResetStatus=cmResetStatus, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighError=cardTemperatureHighError, equipmentAuditGroup=equipmentAuditGroup, equipmentError=equipmentError, equipmentTraps=equipmentTraps, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeId=diskVolumeId, cardTestDescription=cardTestDescription, portDplxStatus=portDplxStatus, cmResetClearNotification=cmResetClearNotification, diskVolumeTable=diskVolumeTable, auditName=auditName, cardAction=cardAction, cardPicDetected=cardPicDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPicHwVersion=cardPicHwVersion, diskDriveNumber=diskDriveNumber, auditDescription=auditDescription, cardSerialNum=cardSerialNum, cmResetNotification=cmResetNotification, systemGeneralGroup=systemGeneralGroup, cardFwVersion=cardFwVersion, portAdminState=portAdminState, cardSecState=cardSecState, cardBootVersion=cardBootVersion, equipmentDiag=equipmentDiag, cmMacAddress=cmMacAddress, portTrapInh=portTrapInh, cardCPLDVersion=cardCPLDVersion, auditCardId=auditCardId, cardSubDetected=cardSubDetected, auditId=auditId, auditStatus=auditStatus, portMaxDsPower=portMaxDsPower, cardTestTransId=cardTestTransId, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSwVersion=cardSwVersion, cardNumber=cardNumber, diskVolumeSize=diskVolumeSize, cardCarrierSerialNum=cardCarrierSerialNum, cardTemperature=cardTemperature, diskVolumeUsageNotification=diskVolumeUsageNotification, verboseLevel=verboseLevel, TestCommand=TestCommand, portMacAddress=portMacAddress, auditShelfId=auditShelfId, shelfName=shelfName, equipmentAudit=equipmentAudit, errEvLogLevel=errEvLogLevel, diskVolumeDiskSize=diskVolumeDiskSize, cardDplxStatus=cardDplxStatus, consoleOutput=consoleOutput, portId=portId, portSecStateChange=portSecStateChange, PYSNMP_MODULE_ID=cadEquipmentMib, portAction=portAction, trapSeverity=trapSeverity, cardCarrierHwVersion=cardCarrierHwVersion, auditCommand=auditCommand, cardHwVersion=cardHwVersion, auditLogThrottle=auditLogThrottle, cardTable=cardTable, cardTestId=cardTestId, cardId=cardId, errEventId=errEventId, diskVolumeTrapInh=diskVolumeTrapInh, cardTestName=cardTestName, cmVendor=cmVendor, diskVolumeUsageLevel=diskVolumeUsageLevel, portSecState=portSecState, cmPrimarySID=cmPrimarySID, diagTestId=diagTestId, cardTestType=cardTestType, cardTestResultDesc=cardTestResultDesc, equipmentShelfGroup=equipmentShelfGroup, cardTestIndex=cardTestIndex, portPrState=portPrState, auditFailedCount=auditFailedCount, portNumChans=portNumChans, equipmentState=equipmentState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditEntry=auditEntry, cardEntry=cardEntry, cardDplxStatusChange=cardDplxStatusChange, diskVolumeGroup=diskVolumeGroup, auditAutoScheduling=auditAutoScheduling, remainInDiagMode=remainInDiagMode, shelfNumber=shelfNumber, cardTestCommand=cardTestCommand, cmResetReason=cmResetReason, cardTestTime=cardTestTime, shelfObjects=shelfObjects, cardLastResetReason=cardLastResetReason, cardPrStateChange=cardPrStateChange, cardSpareGroupId=cardSpareGroupId, diskVolumeNumber=diskVolumeNumber, auditTime=auditTime, auditPassedCount=auditPassedCount, cardLastBootVersion=cardLastBootVersion, equipmentCardGroup=equipmentCardGroup, TestScheduleCommand=TestScheduleCommand, cardPicSerialNum=cardPicSerialNum, TestSchedule=TestSchedule, cardCpuType=cardCpuType, portDetectedMode=portDetectedMode, cardShelfTestIndex=cardShelfTestIndex, portMinDsPower=portMinDsPower, portLastChangeTime=portLastChangeTime, errEvRowStatus=errEvRowStatus, TestType=TestType, cadEquipmentMibConformance=cadEquipmentMibConformance, portBgpId=portBgpId, fpgaErrorEventTable=fpgaErrorEventTable, cardCpuSpeed=cardCpuSpeed, cardUpTime=cardUpTime, cardTestEntry=cardTestEntry, cmResetInfo=cmResetInfo, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portConnectorId=portConnectorId, auditTable=auditTable, cardTestSchedule=cardTestSchedule, systemGeneral=systemGeneral, cardAdminState=cardAdminState, cardCarrierFwVersion=cardCarrierFwVersion, TestTransactionId=TestTransactionId, cardTestTable=cardTestTable, cmResetUpTime=cmResetUpTime, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEvLoggingEnabled=errEvLoggingEnabled, portNumber=portNumber, equipmentTbl=equipmentTbl, eqDiagConfig=eqDiagConfig, systemClock=systemClock, portMode=portMode, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMib=cadEquipmentMib, portType=portType, trapCounter=trapCounter, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTotalPassedCount=auditTotalPassedCount, cmDevice=cmDevice, fpgaErrorEventEntry=fpgaErrorEventEntry, cardRamSize=cardRamSize, cmUChannelID=cmUChannelID, cardTestScheduleCommand=cardTestScheduleCommand, auditResult=auditResult, cardTestResult=cardTestResult, systemKey=systemKey, auditLogOutput=auditLogOutput, cardNumPorts=cardNumPorts, portDocsIfIndex=portDocsIfIndex, cardBusSpeed=cardBusSpeed, cardFwUpdateStatus=cardFwUpdateStatus, portPrStateChange=portPrStateChange, cardFlashSize=cardFlashSize, diskVolumeShelfId=diskVolumeShelfId, cardShelfId=cardShelfId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDetectedChange=cardDetectedChange, auditCycleCount=auditCycleCount, auditTotalFailedCount=auditTotalFailedCount, cardPrState=cardPrState, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmRegistrationNotification=cmRegistrationNotification, cardTrapInh=cardTrapInh)
