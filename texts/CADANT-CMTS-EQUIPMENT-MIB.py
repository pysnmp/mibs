...
FirmwareSource, SecondaryState, CardSubType, AdminState, DiskVolumeUsageLevel, PortMode, CardType, ShelfId, PortType, DuplexStatus, PrimaryState, PortId, PortDetectedMode, FlowControlMode, PicType, EqActionType, CardId, UnknownState = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "SecondaryState", "CardSubType", "AdminState", "DiskVolumeUsageLevel", "PortMode", "CardType", "ShelfId", "PortType", "DuplexStatus", "PrimaryState", "PortId", "PortDetectedMode", "FlowControlMode", "PicType", "EqActionType", "CardId", "UnknownState")
...
MacAddress, TextualConvention, TimeStamp, RowStatus, TruthValue, DateAndTime, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TextualConvention", "TimeStamp", "RowStatus", "TruthValue", "DateAndTime", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portMacIfIndex=portMacIfIndex, auditDescription=auditDescription, portDetectedMode=portDetectedMode, cardTestEntry=cardTestEntry, cmResetReason=cmResetReason, portMode=portMode, cardNumber=cardNumber, cmVendor=cmVendor, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestTransId=cardTestTransId, TestId=TestId, systemGeneralGroup=systemGeneralGroup, cardTestType=cardTestType, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSwVersion=cardSwVersion, portMaxDsPower=portMaxDsPower, cardType=cardType, PYSNMP_MODULE_ID=cadEquipmentMib, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, errEvLoggingEnabled=errEvLoggingEnabled, cmResetClearNotification=cmResetClearNotification, portId=portId, fpgaErrorEventEntry=fpgaErrorEventEntry, trapCounter=trapCounter, cardPicSerialNum=cardPicSerialNum, auditEntry=auditEntry, TestType=TestType, cardBootVersion=cardBootVersion, cardRamSize=cardRamSize, systemKey=systemKey, cardLastBootVersion=cardLastBootVersion, equipmentShelfGroup=equipmentShelfGroup, diskVolumeName=diskVolumeName, portRxFlowControlDetected=portRxFlowControlDetected, auditLogOutput=auditLogOutput, remainInDiagMode=remainInDiagMode, cardDplxStatus=cardDplxStatus, auditTime=auditTime, cadEquipmentMib=cadEquipmentMib, cardTestResultDesc=cardTestResultDesc, cardTestName=cardTestName, portCurrDsPower=portCurrDsPower, auditCardId=auditCardId, systemClock=systemClock, TestCommand=TestCommand, cardTestResultNotification=cardTestResultNotification, shelfName=shelfName, TestTransactionId=TestTransactionId, portDplxStatusChange=portDplxStatusChange, cardPicDetected=cardPicDetected, portEntry=portEntry, cardSpareGroupId=cardSpareGroupId, auditCycleCount=auditCycleCount, trapSeverity=trapSeverity, cardBusSpeed=cardBusSpeed, shelfSwVersion=shelfSwVersion, cardSpareGroupMode=cardSpareGroupMode, cmDevice=cmDevice, cmResetNotification=cmResetNotification, diskVolumeNumber=diskVolumeNumber, diskDriveNumber=diskDriveNumber, cmMacAddress=cmMacAddress, errEventId=errEventId, cardName=cardName, cardPicHwVersion=cardPicHwVersion, cardTestId=cardTestId, portBgpId=portBgpId, cardSerialNum=cardSerialNum, portMacAddress=portMacAddress, cardTestResult=cardTestResult, downstreamPowerLoss=downstreamPowerLoss, portNumChans=portNumChans, eqDiagConfig=eqDiagConfig, diskVolumeDriveId=diskVolumeDriveId, systemGeneral=systemGeneral, auditId=auditId, portCardSubType=portCardSubType, cardEntry=cardEntry, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portNumber=portNumber, equipmentError=equipmentError, cardTrapInh=cardTrapInh, portConnectorId=portConnectorId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardPrStateChange=cardPrStateChange, cardFwUpdateStatus=cardFwUpdateStatus, auditPassedCount=auditPassedCount, cardSubType=cardSubType, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeUsageNotification=diskVolumeUsageNotification, cardAdminState=cardAdminState, portCardId=portCardId, cardTestTable=cardTestTable, cardTempOverHeatNotification=cardTempOverHeatNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetUpTime=cmResetUpTime, auditCommand=auditCommand, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeShelfId=diskVolumeShelfId, cardFpgaSource=cardFpgaSource, cmRegistrationNotification=cmRegistrationNotification, cardTestScheduleCommand=cardTestScheduleCommand, TestResult=TestResult, cardAction=cardAction, cardTestIndex=cardTestIndex, cardDetectedChange=cardDetectedChange, TestSchedule=TestSchedule, auditShelfId=auditShelfId, portPrStateChange=portPrStateChange, portTrapInh=portTrapInh, auditName=auditName, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeEntry=diskVolumeEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, portPrState=portPrState, portDplxStatus=portDplxStatus, cmIpAddress=cmIpAddress, equipmentDiagGroup=equipmentDiagGroup, portLastChangeTime=portLastChangeTime, cmUChannelID=cmUChannelID, cardPrState=cardPrState, TestScheduleCommand=TestScheduleCommand, auditResult=auditResult, cardCarrierFwVersion=cardCarrierFwVersion, cardShelfId=cardShelfId, cardLastChangeTime=cardLastChangeTime, auditTotalPassedCount=auditTotalPassedCount, portMinDsPower=portMinDsPower, verboseLevel=verboseLevel, portTxFlowControlMode=portTxFlowControlMode, cardTestCommand=cardTestCommand, cmPrimarySID=cmPrimarySID, portSecState=portSecState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diagTestId=diagTestId, errEvRowStatus=errEvRowStatus, cardFlashSize=cardFlashSize, cardDetected=cardDetected, portTable=portTable, cardTempNoReportNotification=cardTempNoReportNotification, equipmentState=equipmentState, cardCarrierHwVersion=cardCarrierHwVersion, cardId=cardId, cmResetInfo=cmResetInfo, cardHwVersion=cardHwVersion, cardTestDescription=cardTestDescription, errEvLogLevel=errEvLogLevel, shelfNumber=shelfNumber, cardTemperature=cardTemperature, cardNumPorts=cardNumPorts, equipmentDiag=equipmentDiag, fpgaErrorEventTable=fpgaErrorEventTable, cardTemperatureHighError=cardTemperatureHighError, cmResetStatus=cmResetStatus, equipmentPortGroup=equipmentPortGroup, auditStatus=auditStatus, equipmentAudit=equipmentAudit, cardLastResetReason=cardLastResetReason, portRxFlowControlMode=portRxFlowControlMode, cardCpuSpeed=cardCpuSpeed, portAction=portAction, cardCpuType=cardCpuType, consoleOutput=consoleOutput, cardTestTime=cardTestTime, portSecStateChange=portSecStateChange, equipmentCardGroup=equipmentCardGroup, auditTotalFailedCount=auditTotalFailedCount, auditAutoScheduling=auditAutoScheduling, equipmentTbl=equipmentTbl, cardTable=cardTable, cardShelfTestIndex=cardShelfTestIndex, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeGroup=diskVolumeGroup, shelfObjects=shelfObjects, cardDplxStatusChange=cardDplxStatusChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardUpTime=cardUpTime, cardCPLDVersion=cardCPLDVersion, portAdminState=portAdminState, cardTestSchedule=cardTestSchedule, diskVolumeSize=diskVolumeSize, cardSecState=cardSecState, equipmentTraps=equipmentTraps, cardSubDetected=cardSubDetected, portShelfId=portShelfId, diskVolumeCardId=diskVolumeCardId, equipmentCardTestGroup=equipmentCardTestGroup, portDescription=portDescription, auditLogThrottle=auditLogThrottle, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwVersion=cardFwVersion, equipmentStateGroup=equipmentStateGroup, cardLastBootSource=cardLastBootSource, auditFailedCount=auditFailedCount, equipmentAuditGroup=equipmentAuditGroup, diskVolumeFileName=diskVolumeFileName, diskVolumeId=diskVolumeId, auditTable=auditTable, portDocsIfIndex=portDocsIfIndex, diskVolumeTable=diskVolumeTable, cardSecStateChange=cardSecStateChange, portType=portType, cardTemperatureHighWarn=cardTemperatureHighWarn)
