...
DuplexStatus, SecondaryState, ShelfId, CardSubType, DiskVolumeUsageLevel, FlowControlMode, PrimaryState, FirmwareSource, UnknownState, PortId, EqActionType, PortMode, PortDetectedMode, CardId, AdminState, PicType, PortType, CardType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "SecondaryState", "ShelfId", "CardSubType", "DiskVolumeUsageLevel", "FlowControlMode", "PrimaryState", "FirmwareSource", "UnknownState", "PortId", "EqActionType", "PortMode", "PortDetectedMode", "CardId", "AdminState", "PicType", "PortType", "CardType")
...
TruthValue, MacAddress, RowStatus, TextualConvention, DisplayString, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "MacAddress", "RowStatus", "TextualConvention", "DisplayString", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", fpgaErrorEventEntry=fpgaErrorEventEntry, TestSchedule=TestSchedule, cardFwUpdateStatus=cardFwUpdateStatus, trapSeverity=trapSeverity, shelfObjects=shelfObjects, auditEntry=auditEntry, portDescription=portDescription, diskVolumeDiskSize=diskVolumeDiskSize, cmResetClearNotification=cmResetClearNotification, portCardSubType=portCardSubType, cardTemperature=cardTemperature, diskVolumeShelfId=diskVolumeShelfId, cardTestTime=cardTestTime, cardLastBootVersion=cardLastBootVersion, portNumber=portNumber, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, cardShelfTestIndex=cardShelfTestIndex, auditId=auditId, errEvLoggingEnabled=errEvLoggingEnabled, verboseLevel=verboseLevel, cardTestResultNotification=cardTestResultNotification, cardCpuSpeed=cardCpuSpeed, cardCarrierFwVersion=cardCarrierFwVersion, cardFwVersion=cardFwVersion, portTable=portTable, fpgaErrorEventTable=fpgaErrorEventTable, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardCarrierSerialNum=cardCarrierSerialNum, auditTable=auditTable, auditTotalFailedCount=auditTotalFailedCount, cmResetInfo=cmResetInfo, cardRamSize=cardRamSize, cardCarrierHwVersion=cardCarrierHwVersion, cardTestScheduleCommand=cardTestScheduleCommand, auditFailedCount=auditFailedCount, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestId=TestId, equipmentAudit=equipmentAudit, cardSecStateChange=cardSecStateChange, cardPrStateChange=cardPrStateChange, cardLastChangeTime=cardLastChangeTime, portAction=portAction, auditShelfId=auditShelfId, TestTransactionId=TestTransactionId, cardSerialNum=cardSerialNum, auditCommand=auditCommand, cardName=cardName, portTrapInh=portTrapInh, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, errEvLogLevel=errEvLogLevel, portDplxStatus=portDplxStatus, cardType=cardType, equipmentDiag=equipmentDiag, cardDetected=cardDetected, portConnectorId=portConnectorId, cmIpAddress=cmIpAddress, diskVolumeName=diskVolumeName, equipmentCardGroup=equipmentCardGroup, portSecState=portSecState, cardTable=cardTable, shelfSwVersion=shelfSwVersion, diskVolumeDriveId=diskVolumeDriveId, diagTestId=diagTestId, TestResult=TestResult, portType=portType, portCurrDsPower=portCurrDsPower, diskVolumeFileName=diskVolumeFileName, downstreamPowerLoss=downstreamPowerLoss, portSecStateChange=portSecStateChange, equipmentPortGroup=equipmentPortGroup, cardDplxStatus=cardDplxStatus, shelfNumber=shelfNumber, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibGroup=cadEquipmentMibGroup, cadEquipmentMib=cadEquipmentMib, portPrState=portPrState, equipmentError=equipmentError, portNumChans=portNumChans, cardTemperatureHighError=cardTemperatureHighError, cardTestCommand=cardTestCommand, cardBootVersion=cardBootVersion, TestScheduleCommand=TestScheduleCommand, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeSize=diskVolumeSize, portShelfId=portShelfId, portId=portId, cardSecState=cardSecState, PYSNMP_MODULE_ID=cadEquipmentMib, cardSubDetected=cardSubDetected, portBgpId=portBgpId, cardSpareGroupMode=cardSpareGroupMode, diskVolumeCardId=diskVolumeCardId, auditName=auditName, cardAction=cardAction, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentStateGroup=equipmentStateGroup, diskVolumeNumber=diskVolumeNumber, cmResetNotification=cmResetNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, auditLogOutput=auditLogOutput, diskVolumeTrapInh=diskVolumeTrapInh, equipmentTbl=equipmentTbl, portDetectedMode=portDetectedMode, diskVolumeTable=diskVolumeTable, consoleOutput=consoleOutput, equipmentTraps=equipmentTraps, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCpuType=cardCpuType, cardTestSchedule=cardTestSchedule, systemGeneral=systemGeneral, portMinDsPower=portMinDsPower, auditTotalPassedCount=auditTotalPassedCount, cardTrapInh=cardTrapInh, cardDetectedChange=cardDetectedChange, eqDiagConfig=eqDiagConfig, cardPicSerialNum=cardPicSerialNum, cardTestType=cardTestType, cardShelfId=cardShelfId, errEvRowStatus=errEvRowStatus, auditCardId=auditCardId, diskDriveNumber=diskDriveNumber, cardSwVersion=cardSwVersion, cardSubType=cardSubType, equipmentShelfGroup=equipmentShelfGroup, remainInDiagMode=remainInDiagMode, cardEntry=cardEntry, cardCPLDVersion=cardCPLDVersion, cardSpareGroupId=cardSpareGroupId, cardTestEntry=cardTestEntry, errEventId=errEventId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardPicHwVersion=cardPicHwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardLastBootSource=cardLastBootSource, cardTestDescription=cardTestDescription, portMaxDsPower=portMaxDsPower, auditLogThrottle=auditLogThrottle, cardPrState=cardPrState, cmUChannelID=cmUChannelID, cmDevice=cmDevice, portPrStateChange=portPrStateChange, diskVolumeGroup=diskVolumeGroup, cardFpgaSource=cardFpgaSource, cardTestResult=cardTestResult, cmResetStatus=cmResetStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, shelfName=shelfName, cardFlashSize=cardFlashSize, cardTestTable=cardTestTable, cardHwVersion=cardHwVersion, cardNumPorts=cardNumPorts, diskVolumeId=diskVolumeId, cardTestResultDesc=cardTestResultDesc, fpgaErrorEventGroup=fpgaErrorEventGroup, portMacAddress=portMacAddress, portMacIfIndex=portMacIfIndex, portMode=portMode, TestType=TestType, portCardId=portCardId, cardUpTime=cardUpTime, auditTime=auditTime, cmRegistrationNotification=cmRegistrationNotification, portAdminState=portAdminState, trapCounter=trapCounter, diskVolumeEntry=diskVolumeEntry, cmResetReason=cmResetReason, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestName=cardTestName, auditDescription=auditDescription, auditStatus=auditStatus, systemKey=systemKey, portRxFlowControlDetected=portRxFlowControlDetected, auditCycleCount=auditCycleCount, auditAutoScheduling=auditAutoScheduling, cardTestTransId=cardTestTransId, cardTestIndex=cardTestIndex, cardPicDetected=cardPicDetected, cardLastResetReason=cardLastResetReason, cmPrimarySID=cmPrimarySID, auditResult=auditResult, cmResetUpTime=cmResetUpTime, cardDplxStatusChange=cardDplxStatusChange, TestCommand=TestCommand, auditPassedCount=auditPassedCount, cardNumber=cardNumber, cardBusSpeed=cardBusSpeed, equipmentState=equipmentState, cmVendor=cmVendor, portDocsIfIndex=portDocsIfIndex, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestId=cardTestId, cardAdminState=cardAdminState, portEntry=portEntry, portTxFlowControlMode=portTxFlowControlMode, cmMacAddress=cmMacAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTempNoReportNotification=cardTempNoReportNotification, systemClock=systemClock, portLastChangeTime=portLastChangeTime, equipmentCardTestGroup=equipmentCardTestGroup, portDplxStatusChange=portDplxStatusChange, cardId=cardId)
