...
PrimaryState, AdminState, PortType, DuplexStatus, SecondaryState, CardSubType, CardType, EqActionType, CardId, PortId, FirmwareSource, PicType, PortMode, PortDetectedMode, FlowControlMode, ShelfId, DiskVolumeUsageLevel, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "AdminState", "PortType", "DuplexStatus", "SecondaryState", "CardSubType", "CardType", "EqActionType", "CardId", "PortId", "FirmwareSource", "PicType", "PortMode", "PortDetectedMode", "FlowControlMode", "ShelfId", "DiskVolumeUsageLevel", "UnknownState")
...
MacAddress, TimeStamp, TruthValue, DateAndTime, TextualConvention, RowStatus, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TimeStamp", "TruthValue", "DateAndTime", "TextualConvention", "RowStatus", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", trapSeverity=trapSeverity, cmResetStatus=cmResetStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSecStateChange=cardSecStateChange, cardFpgaSource=cardFpgaSource, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestResultDesc=cardTestResultDesc, cardCPLDVersion=cardCPLDVersion, consoleOutput=consoleOutput, portDetectedMode=portDetectedMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestTransactionId=TestTransactionId, cardTestResult=cardTestResult, cardSubDetected=cardSubDetected, cardType=cardType, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestResultNotification=cardTestResultNotification, shelfNumber=shelfNumber, diagTestId=diagTestId, portMinDsPower=portMinDsPower, cardHwVersion=cardHwVersion, cardBootVersion=cardBootVersion, portShelfId=portShelfId, cardTrapInh=cardTrapInh, cmIpAddress=cmIpAddress, auditLogOutput=auditLogOutput, equipmentPortGroup=equipmentPortGroup, cardBusSpeed=cardBusSpeed, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardNumPorts=cardNumPorts, portBgpId=portBgpId, auditCardId=auditCardId, cmUChannelID=cmUChannelID, cardTestSchedule=cardTestSchedule, diskVolumeName=diskVolumeName, cardFwUpdateStatus=cardFwUpdateStatus, cardTestName=cardTestName, trapCounter=trapCounter, auditDescription=auditDescription, cardDetected=cardDetected, cardUpTime=cardUpTime, cardTestDescription=cardTestDescription, auditId=auditId, errEventId=errEventId, cardSecState=cardSecState, cardPicDetected=cardPicDetected, diskVolumeNumber=diskVolumeNumber, cardEntry=cardEntry, cardId=cardId, shelfName=shelfName, cardTestIndex=cardTestIndex, cardPicSerialNum=cardPicSerialNum, eqDiagConfig=eqDiagConfig, cardTempNoReportNotification=cardTempNoReportNotification, cardShelfId=cardShelfId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portMacAddress=portMacAddress, systemClock=systemClock, diskVolumeTable=diskVolumeTable, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierSerialNum=cardCarrierSerialNum, portDplxStatus=portDplxStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestEntry=cardTestEntry, portNumber=portNumber, shelfObjects=shelfObjects, systemGeneralGroup=systemGeneralGroup, cmPrimarySID=cmPrimarySID, equipmentDiag=equipmentDiag, equipmentError=equipmentError, portLastChangeTime=portLastChangeTime, diskVolumeId=diskVolumeId, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetReason=cmResetReason, cardFwVersion=cardFwVersion, portNumChans=portNumChans, portPrStateChange=portPrStateChange, portEntry=portEntry, TestCommand=TestCommand, portDocsIfIndex=portDocsIfIndex, cardLastChangeTime=cardLastChangeTime, diskVolumeFileName=diskVolumeFileName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, systemGeneral=systemGeneral, diskVolumeUsageLevel=diskVolumeUsageLevel, auditShelfId=auditShelfId, auditPassedCount=auditPassedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentTraps=equipmentTraps, TestType=TestType, auditCommand=auditCommand, portCardId=portCardId, cardTestTable=cardTestTable, auditCycleCount=auditCycleCount, auditFailedCount=auditFailedCount, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portCardSubType=portCardSubType, auditTotalFailedCount=auditTotalFailedCount, systemKey=systemKey, auditName=auditName, diskVolumeEntry=diskVolumeEntry, portSecState=portSecState, cardSubType=cardSubType, cardAdminState=cardAdminState, cardCpuSpeed=cardCpuSpeed, auditResult=auditResult, cardSwVersion=cardSwVersion, portPrState=portPrState, diskVolumeShelfId=diskVolumeShelfId, cardName=cardName, TestId=TestId, cardPicHwVersion=cardPicHwVersion, cardTemperature=cardTemperature, remainInDiagMode=remainInDiagMode, verboseLevel=verboseLevel, cardTestCommand=cardTestCommand, cardSerialNum=cardSerialNum, portTxFlowControlDetected=portTxFlowControlDetected, cmDevice=cmDevice, cmResetNotification=cmResetNotification, auditTime=auditTime, cardSpareGroupId=cardSpareGroupId, portTrapInh=portTrapInh, portMode=portMode, shelfSwVersion=shelfSwVersion, auditTable=auditTable, portAction=portAction, portDescription=portDescription, diskVolumeTrapInh=diskVolumeTrapInh, cardPrStateChange=cardPrStateChange, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, portRxFlowControlDetected=portRxFlowControlDetected, portMacIfIndex=portMacIfIndex, cardDetectedChange=cardDetectedChange, downstreamPowerLoss=downstreamPowerLoss, diskVolumeCardId=diskVolumeCardId, cardCpuType=cardCpuType, cardTestTime=cardTestTime, cardFlashSize=cardFlashSize, cadEquipmentMib=cadEquipmentMib, equipmentState=equipmentState, cardTable=cardTable, cmResetInfo=cmResetInfo, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeSize=diskVolumeSize, portTable=portTable, portSecStateChange=portSecStateChange, PYSNMP_MODULE_ID=cadEquipmentMib, cardCarrierFwVersion=cardCarrierFwVersion, TestScheduleCommand=TestScheduleCommand, cardShelfTestIndex=cardShelfTestIndex, cardCarrierHwVersion=cardCarrierHwVersion, cardSpareGroupMode=cardSpareGroupMode, auditAutoScheduling=auditAutoScheduling, auditEntry=auditEntry, errEvLoggingEnabled=errEvLoggingEnabled, equipmentCardGroup=equipmentCardGroup, cardDplxStatus=cardDplxStatus, cardPrState=cardPrState, cardLastBootSource=cardLastBootSource, equipmentShelfGroup=equipmentShelfGroup, portAdminState=portAdminState, cardTestId=cardTestId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeDriveId=diskVolumeDriveId, cardTestTransId=cardTestTransId, portConnectorId=portConnectorId, cardDplxStatusChange=cardDplxStatusChange, TestSchedule=TestSchedule, cardAction=cardAction, portType=portType, portCurrDsPower=portCurrDsPower, cmResetClearNotification=cmResetClearNotification, portDplxStatusChange=portDplxStatusChange, equipmentTbl=equipmentTbl, errEvLogLevel=errEvLogLevel, equipmentAudit=equipmentAudit, TestResult=TestResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMibGroup=cadEquipmentMibGroup, portTxFlowControlMode=portTxFlowControlMode, auditTotalPassedCount=auditTotalPassedCount, equipmentDiagGroup=equipmentDiagGroup, equipmentAuditGroup=equipmentAuditGroup, cardTemperatureHighError=cardTemperatureHighError, cardLastBootVersion=cardLastBootVersion, cardTestScheduleCommand=cardTestScheduleCommand, cmMacAddress=cmMacAddress, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portId=portId, auditStatus=auditStatus, equipmentStateGroup=equipmentStateGroup, cmVendor=cmVendor, equipmentCardTestGroup=equipmentCardTestGroup, portRxFlowControlMode=portRxFlowControlMode, cardNumber=cardNumber, cmRegistrationNotification=cmRegistrationNotification, auditLogThrottle=auditLogThrottle, diskDriveNumber=diskDriveNumber, cardRamSize=cardRamSize, errEvRowStatus=errEvRowStatus, cardLastResetReason=cardLastResetReason, cardTestType=cardTestType, portMaxDsPower=portMaxDsPower, cmResetUpTime=cmResetUpTime)
