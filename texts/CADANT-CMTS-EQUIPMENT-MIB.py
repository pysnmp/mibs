...
PicType, DiskVolumeUsageLevel, PrimaryState, FirmwareSource, PortMode, EqActionType, PortId, CardId, FlowControlMode, PortType, CardSubType, AdminState, CardType, DuplexStatus, PortDetectedMode, ShelfId, SecondaryState, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PicType", "DiskVolumeUsageLevel", "PrimaryState", "FirmwareSource", "PortMode", "EqActionType", "PortId", "CardId", "FlowControlMode", "PortType", "CardSubType", "AdminState", "CardType", "DuplexStatus", "PortDetectedMode", "ShelfId", "SecondaryState", "UnknownState")
...
DateAndTime, TruthValue, RowStatus, TextualConvention, DisplayString, TimeStamp, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TruthValue", "RowStatus", "TextualConvention", "DisplayString", "TimeStamp", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTime=auditTime, portPrStateChange=portPrStateChange, cardTestEntry=cardTestEntry, diagTestId=diagTestId, TestTransactionId=TestTransactionId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestTransId=cardTestTransId, cmResetReason=cmResetReason, errEventId=errEventId, cmRegistrationNotification=cmRegistrationNotification, equipmentAudit=equipmentAudit, cardTestIndex=cardTestIndex, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portLastChangeTime=portLastChangeTime, cardSpareGroupId=cardSpareGroupId, portTxFlowControlMode=portTxFlowControlMode, equipmentPortGroup=equipmentPortGroup, TestSchedule=TestSchedule, cardShelfId=cardShelfId, cardPicDetected=cardPicDetected, cardTestDescription=cardTestDescription, errEvRowStatus=errEvRowStatus, equipmentCardTestGroup=equipmentCardTestGroup, cardLastBootSource=cardLastBootSource, cardHwVersion=cardHwVersion, auditAutoScheduling=auditAutoScheduling, auditTotalFailedCount=auditTotalFailedCount, cardSecStateChange=cardSecStateChange, cardDplxStatusChange=cardDplxStatusChange, portNumber=portNumber, cardTempNoReportNotification=cardTempNoReportNotification, cardSubType=cardSubType, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCpuType=cardCpuType, systemGeneralGroup=systemGeneralGroup, cadEquipmentMib=cadEquipmentMib, systemClock=systemClock, cardUpTime=cardUpTime, portPrState=portPrState, fpgaErrorEventTable=fpgaErrorEventTable, equipmentDiag=equipmentDiag, cardShelfTestIndex=cardShelfTestIndex, diskVolumeDriveId=diskVolumeDriveId, eqDiagConfig=eqDiagConfig, cardCarrierHwVersion=cardCarrierHwVersion, auditTotalPassedCount=auditTotalPassedCount, auditLogOutput=auditLogOutput, shelfObjects=shelfObjects, cardLastChangeTime=cardLastChangeTime, cardNumber=cardNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeFileName=diskVolumeFileName, portCardId=portCardId, diskVolumeGroup=diskVolumeGroup, cardDplxStatus=cardDplxStatus, TestType=TestType, cmVendor=cmVendor, errEvRecoveryEnabled=errEvRecoveryEnabled, auditTable=auditTable, cardBusSpeed=cardBusSpeed, cardBootVersion=cardBootVersion, cardId=cardId, portMaxDsPower=portMaxDsPower, portMode=portMode, auditCycleCount=auditCycleCount, cardTrapInh=cardTrapInh, portMacAddress=portMacAddress, cardSpareGroupMode=cardSpareGroupMode, cardCarrierSerialNum=cardCarrierSerialNum, portTrapInh=portTrapInh, diskVolumeShelfId=diskVolumeShelfId, auditResult=auditResult, auditStatus=auditStatus, equipmentDiagGroup=equipmentDiagGroup, cardSerialNum=cardSerialNum, diskVolumeSize=diskVolumeSize, cardTestCommand=cardTestCommand, cmResetInfo=cmResetInfo, trapCounter=trapCounter, remainInDiagMode=remainInDiagMode, equipmentTraps=equipmentTraps, diskDriveNumber=diskDriveNumber, diskVolumeUsageNotification=diskVolumeUsageNotification, cmDevice=cmDevice, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardCarrierFwVersion=cardCarrierFwVersion, cardTestScheduleCommand=cardTestScheduleCommand, portType=portType, equipmentCardGroup=equipmentCardGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestType=cardTestType, cardTestId=cardTestId, cmUChannelID=cmUChannelID, cmIpAddress=cmIpAddress, portSecStateChange=portSecStateChange, diskVolumeName=diskVolumeName, trapSeverity=trapSeverity, cardPrStateChange=cardPrStateChange, cardLastResetReason=cardLastResetReason, equipmentStateGroup=equipmentStateGroup, cardTestTime=cardTestTime, cardAction=cardAction, portDescription=portDescription, diskVolumeTable=diskVolumeTable, auditCommand=auditCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portShelfId=portShelfId, cmPrimarySID=cmPrimarySID, errEvLogLevel=errEvLogLevel, errEvLoggingEnabled=errEvLoggingEnabled, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetStatus=cmResetStatus, cardLastBootVersion=cardLastBootVersion, cardSwVersion=cardSwVersion, portAdminState=portAdminState, verboseLevel=verboseLevel, cardCPLDVersion=cardCPLDVersion, cardType=cardType, systemGeneral=systemGeneral, cardPicSerialNum=cardPicSerialNum, portCurrDsPower=portCurrDsPower, cardAdminState=cardAdminState, auditId=auditId, diskVolumeTrapInh=diskVolumeTrapInh, cmResetUpTime=cmResetUpTime, cardTestResultNotification=cardTestResultNotification, cardDetected=cardDetected, cardFpgaSource=cardFpgaSource, cardEntry=cardEntry, cardPicHwVersion=cardPicHwVersion, cardTestSchedule=cardTestSchedule, cardTestName=cardTestName, diskVolumeId=diskVolumeId, portId=portId, shelfSwVersion=shelfSwVersion, TestScheduleCommand=TestScheduleCommand, cardNumPorts=cardNumPorts, cardFlashSize=cardFlashSize, cardTable=cardTable, auditEntry=auditEntry, equipmentError=equipmentError, cardPrState=cardPrState, cmResetClearNotification=cmResetClearNotification, shelfName=shelfName, cardName=cardName, portTable=portTable, shelfNumber=shelfNumber, systemKey=systemKey, cardTestResultDesc=cardTestResultDesc, auditPassedCount=auditPassedCount, cardSubDetected=cardSubDetected, portCardSubType=portCardSubType, portBgpId=portBgpId, auditCardId=auditCardId, auditFailedCount=auditFailedCount, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentTbl=equipmentTbl, cardTemperatureHighError=cardTemperatureHighError, cmMacAddress=cmMacAddress, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditShelfId=auditShelfId, portDplxStatusChange=portDplxStatusChange, portEntry=portEntry, downstreamPowerLoss=downstreamPowerLoss, cardRamSize=cardRamSize, portNumChans=portNumChans, portDetectedMode=portDetectedMode, cardTemperature=cardTemperature, portRxFlowControlMode=portRxFlowControlMode, TestCommand=TestCommand, cardSecState=cardSecState, cadEquipmentMibConformance=cadEquipmentMibConformance, cmResetNotification=cmResetNotification, auditName=auditName, cardTestResult=cardTestResult, diskVolumeNumber=diskVolumeNumber, diskVolumeEntry=diskVolumeEntry, portMacIfIndex=portMacIfIndex, consoleOutput=consoleOutput, PYSNMP_MODULE_ID=cadEquipmentMib, portAction=portAction, cadEquipmentMibCompliance=cadEquipmentMibCompliance, fpgaErrorEventGroup=fpgaErrorEventGroup, auditDescription=auditDescription, portRxFlowControlDetected=portRxFlowControlDetected, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeCardId=diskVolumeCardId, TestResult=TestResult, portTxFlowControlDetected=portTxFlowControlDetected, auditLogThrottle=auditLogThrottle, portConnectorId=portConnectorId, equipmentState=equipmentState, cardFwUpdateStatus=cardFwUpdateStatus, cardCpuSpeed=cardCpuSpeed, portDplxStatus=portDplxStatus, portMinDsPower=portMinDsPower, equipmentShelfGroup=equipmentShelfGroup, portDocsIfIndex=portDocsIfIndex, equipmentAuditGroup=equipmentAuditGroup, cardDetectedChange=cardDetectedChange, TestId=TestId, cardFwVersion=cardFwVersion, portSecState=portSecState, diskVolumeDiskSize=diskVolumeDiskSize, cardTestTable=cardTestTable)
