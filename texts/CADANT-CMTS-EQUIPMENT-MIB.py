...
AdminState, EqActionType, PortDetectedMode, CardType, FlowControlMode, CardSubType, SecondaryState, PortMode, ShelfId, UnknownState, FirmwareSource, PortType, DuplexStatus, DiskVolumeUsageLevel, PortId, PicType, CardId, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "AdminState", "EqActionType", "PortDetectedMode", "CardType", "FlowControlMode", "CardSubType", "SecondaryState", "PortMode", "ShelfId", "UnknownState", "FirmwareSource", "PortType", "DuplexStatus", "DiskVolumeUsageLevel", "PortId", "PicType", "CardId", "PrimaryState")
...
DisplayString, DateAndTime, TimeStamp, MacAddress, RowStatus, TruthValue, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "DateAndTime", "TimeStamp", "MacAddress", "RowStatus", "TruthValue", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfNumber=shelfNumber, cardCpuType=cardCpuType, cardCarrierFwVersion=cardCarrierFwVersion, equipmentTbl=equipmentTbl, equipmentAudit=equipmentAudit, PYSNMP_MODULE_ID=cadEquipmentMib, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTime=cardTestTime, cardSecStateChange=cardSecStateChange, systemKey=systemKey, auditEntry=auditEntry, TestTransactionId=TestTransactionId, diskVolumeShelfId=diskVolumeShelfId, cmResetReason=cmResetReason, portCardId=portCardId, errEvRowStatus=errEvRowStatus, cardPicDetected=cardPicDetected, cmIpAddress=cmIpAddress, auditResult=auditResult, downstreamPowerLoss=downstreamPowerLoss, portNumber=portNumber, cardTempNoReportNotification=cardTempNoReportNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestResultDesc=cardTestResultDesc, portCardSubType=portCardSubType, diskVolumeTable=diskVolumeTable, auditStatus=auditStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMacIfIndex=portMacIfIndex, cmResetNotification=cmResetNotification, equipmentCardTestGroup=equipmentCardTestGroup, portMacAddress=portMacAddress, cardPrState=cardPrState, portMaxDsPower=portMaxDsPower, portNumChans=portNumChans, cardUpTime=cardUpTime, cardEntry=cardEntry, consoleOutput=consoleOutput, portCurrDsPower=portCurrDsPower, portSecState=portSecState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTemperatureHighError=cardTemperatureHighError, cardFlashSize=cardFlashSize, cmMacAddress=cmMacAddress, diskDriveNumber=diskDriveNumber, equipmentError=equipmentError, portTxFlowControlMode=portTxFlowControlMode, TestType=TestType, errEvLoggingEnabled=errEvLoggingEnabled, portSecStateChange=portSecStateChange, cmResetClearNotification=cmResetClearNotification, TestId=TestId, systemGeneralGroup=systemGeneralGroup, systemClock=systemClock, cardDetected=cardDetected, errEvLogLevel=errEvLogLevel, portDplxStatus=portDplxStatus, auditFailedCount=auditFailedCount, portAction=portAction, TestScheduleCommand=TestScheduleCommand, cardCpuSpeed=cardCpuSpeed, cardTestId=cardTestId, TestCommand=TestCommand, cardTestEntry=cardTestEntry, diskVolumeName=diskVolumeName, auditCommand=auditCommand, auditPassedCount=auditPassedCount, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentCardGroup=equipmentCardGroup, cardLastBootVersion=cardLastBootVersion, portConnectorId=portConnectorId, cmResetStatus=cmResetStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, portMode=portMode, fpgaErrorEventTable=fpgaErrorEventTable, portDocsIfIndex=portDocsIfIndex, diskVolumeGroup=diskVolumeGroup, portAdminState=portAdminState, cardHwVersion=cardHwVersion, cardAction=cardAction, diskVolumeDiskSize=diskVolumeDiskSize, auditTotalPassedCount=auditTotalPassedCount, cmRegistrationNotification=cmRegistrationNotification, trapCounter=trapCounter, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, auditTable=auditTable, cardLastResetReason=cardLastResetReason, diagTestId=diagTestId, diskVolumeNumber=diskVolumeNumber, cardLastChangeTime=cardLastChangeTime, cardFpgaSource=cardFpgaSource, cardType=cardType, portDplxStatusChange=portDplxStatusChange, shelfName=shelfName, diskVolumeDriveId=diskVolumeDriveId, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentState=equipmentState, equipmentStateGroup=equipmentStateGroup, cardPicHwVersion=cardPicHwVersion, cardName=cardName, portPrState=portPrState, cardTestResult=cardTestResult, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPrStateChange=cardPrStateChange, cardShelfTestIndex=cardShelfTestIndex, cardNumPorts=cardNumPorts, cardNumber=cardNumber, portBgpId=portBgpId, cardAdminState=cardAdminState, cardFwUpdateStatus=cardFwUpdateStatus, auditCardId=auditCardId, equipmentPortGroup=equipmentPortGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portPrStateChange=portPrStateChange, cardTestScheduleCommand=cardTestScheduleCommand, portLastChangeTime=portLastChangeTime, auditId=auditId, cardTestName=cardTestName, cardTestCommand=cardTestCommand, TestResult=TestResult, cardSecState=cardSecState, cardTemperature=cardTemperature, equipmentDiagGroup=equipmentDiagGroup, cardTestTransId=cardTestTransId, portId=portId, equipmentAuditGroup=equipmentAuditGroup, shelfSwVersion=shelfSwVersion, portEntry=portEntry, cardLastBootSource=cardLastBootSource, cmUChannelID=cmUChannelID, cardBusSpeed=cardBusSpeed, trapSeverity=trapSeverity, cardTestDescription=cardTestDescription, cardPicSerialNum=cardPicSerialNum, equipmentDiag=equipmentDiag, cardSpareGroupMode=cardSpareGroupMode, cardTable=cardTable, diskVolumeEntry=diskVolumeEntry, portTable=portTable, cardFwVersion=cardFwVersion, TestSchedule=TestSchedule, auditShelfId=auditShelfId, verboseLevel=verboseLevel, portShelfId=portShelfId, cardRamSize=cardRamSize, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTotalFailedCount=auditTotalFailedCount, diskVolumeId=diskVolumeId, auditLogOutput=auditLogOutput, portDetectedMode=portDetectedMode, cmDevice=cmDevice, auditName=auditName, portType=portType, cardDplxStatus=cardDplxStatus, remainInDiagMode=remainInDiagMode, auditAutoScheduling=auditAutoScheduling, cardSwVersion=cardSwVersion, cmResetUpTime=cmResetUpTime, cardCPLDVersion=cardCPLDVersion, cardCarrierSerialNum=cardCarrierSerialNum, cardTestIndex=cardTestIndex, cardId=cardId, errEventId=errEventId, cardDplxStatusChange=cardDplxStatusChange, cmResetInfo=cmResetInfo, cadEquipmentMib=cadEquipmentMib, auditDescription=auditDescription, cadEquipmentMibConformance=cadEquipmentMibConformance, portTrapInh=portTrapInh, diskVolumeTrapInh=diskVolumeTrapInh, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeFileName=diskVolumeFileName, portMinDsPower=portMinDsPower, auditLogThrottle=auditLogThrottle, auditCycleCount=auditCycleCount, equipmentShelfGroup=equipmentShelfGroup, cardSubDetected=cardSubDetected, shelfObjects=shelfObjects, cardSpareGroupId=cardSpareGroupId, cardTestType=cardTestType, equipmentTraps=equipmentTraps, cardTrapInh=cardTrapInh, diskVolumeCardId=diskVolumeCardId, eqDiagConfig=eqDiagConfig, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestTable=cardTestTable, cardDetectedChange=cardDetectedChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditTime=auditTime, portDescription=portDescription, portRxFlowControlMode=portRxFlowControlMode, cardSerialNum=cardSerialNum, cardTestSchedule=cardTestSchedule, cardCarrierHwVersion=cardCarrierHwVersion, cmPrimarySID=cmPrimarySID, cardShelfId=cardShelfId, cardSubType=cardSubType, systemGeneral=systemGeneral, cardTestResultNotification=cardTestResultNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cmVendor=cmVendor, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeSize=diskVolumeSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardBootVersion=cardBootVersion)
