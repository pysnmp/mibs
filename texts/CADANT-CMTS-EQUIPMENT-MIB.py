...
FlowControlMode, SecondaryState, ShelfId, PrimaryState, UnknownState, DuplexStatus, AdminState, DiskVolumeUsageLevel, PortMode, PicType, FirmwareSource, CardType, CardId, PortDetectedMode, PortId, PortType, CardSubType, EqActionType = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "SecondaryState", "ShelfId", "PrimaryState", "UnknownState", "DuplexStatus", "AdminState", "DiskVolumeUsageLevel", "PortMode", "PicType", "FirmwareSource", "CardType", "CardId", "PortDetectedMode", "PortId", "PortType", "CardSubType", "EqActionType")
...
DateAndTime, TextualConvention, TimeStamp, DisplayString, MacAddress, TruthValue, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TextualConvention", "TimeStamp", "DisplayString", "MacAddress", "TruthValue", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestId=TestId, fpgaErrorEventEntry=fpgaErrorEventEntry, systemClock=systemClock, auditEntry=auditEntry, cardFlashSize=cardFlashSize, cardTestEntry=cardTestEntry, cardPicHwVersion=cardPicHwVersion, cardTrapInh=cardTrapInh, auditId=auditId, errEvLoggingEnabled=errEvLoggingEnabled, equipmentDiagGroup=equipmentDiagGroup, portId=portId, cardDetectedChange=cardDetectedChange, diskVolumeId=diskVolumeId, cardTable=cardTable, systemGeneralGroup=systemGeneralGroup, cardTestIndex=cardTestIndex, portRxFlowControlMode=portRxFlowControlMode, cardDetected=cardDetected, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSecState=cardSecState, cardId=cardId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCarrierSerialNum=cardCarrierSerialNum, portCurrDsPower=portCurrDsPower, portNumChans=portNumChans, portDetectedMode=portDetectedMode, cmIpAddress=cmIpAddress, errEvLogLevel=errEvLogLevel, auditName=auditName, cardSerialNum=cardSerialNum, cardSecStateChange=cardSecStateChange, portTable=portTable, shelfObjects=shelfObjects, fpgaErrorEventGroup=fpgaErrorEventGroup, portMacIfIndex=portMacIfIndex, diskDriveNumber=diskDriveNumber, equipmentCardGroup=equipmentCardGroup, diskVolumeFileName=diskVolumeFileName, cmResetClearNotification=cmResetClearNotification, equipmentAuditGroup=equipmentAuditGroup, cardCpuType=cardCpuType, diagTestId=diagTestId, cmMacAddress=cmMacAddress, cardTestResult=cardTestResult, errEventId=errEventId, cardCpuSpeed=cardCpuSpeed, cardSubType=cardSubType, portBgpId=portBgpId, portMaxDsPower=portMaxDsPower, portNumber=portNumber, portDocsIfIndex=portDocsIfIndex, cardTestTransId=cardTestTransId, cardLastResetReason=cardLastResetReason, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSpareGroupMode=cardSpareGroupMode, cardTestDescription=cardTestDescription, cardLastBootSource=cardLastBootSource, equipmentAudit=equipmentAudit, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeTrapInh=diskVolumeTrapInh, portDplxStatusChange=portDplxStatusChange, portEntry=portEntry, cardTemperatureHighError=cardTemperatureHighError, cardSubDetected=cardSubDetected, cardTempNoReportNotification=cardTempNoReportNotification, auditAutoScheduling=auditAutoScheduling, portSecStateChange=portSecStateChange, portTxFlowControlDetected=portTxFlowControlDetected, equipmentTraps=equipmentTraps, portConnectorId=portConnectorId, portPrState=portPrState, equipmentShelfGroup=equipmentShelfGroup, TestTransactionId=TestTransactionId, auditCommand=auditCommand, shelfSwVersion=shelfSwVersion, equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, cmResetInfo=cmResetInfo, portDescription=portDescription, shelfName=shelfName, cmRegistrationNotification=cmRegistrationNotification, cardName=cardName, portMacAddress=portMacAddress, cardTestTable=cardTestTable, cardPrState=cardPrState, diskVolumeEntry=diskVolumeEntry, cardTestCommand=cardTestCommand, cardNumPorts=cardNumPorts, equipmentError=equipmentError, cadEquipmentMibGroup=cadEquipmentMibGroup, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTable=diskVolumeTable, cardSpareGroupId=cardSpareGroupId, remainInDiagMode=remainInDiagMode, cmResetUpTime=cmResetUpTime, TestScheduleCommand=TestScheduleCommand, TestType=TestType, cardTestResultNotification=cardTestResultNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardTestType=cardTestType, eqDiagConfig=eqDiagConfig, auditStatus=auditStatus, portSecState=portSecState, cardLastChangeTime=cardLastChangeTime, cardNumber=cardNumber, cardDplxStatus=cardDplxStatus, equipmentTbl=equipmentTbl, cardBootVersion=cardBootVersion, equipmentCardTestGroup=equipmentCardTestGroup, systemGeneral=systemGeneral, cardPrStateChange=cardPrStateChange, portCardId=portCardId, auditTime=auditTime, cmVendor=cmVendor, TestSchedule=TestSchedule, portAction=portAction, cardCarrierFwVersion=cardCarrierFwVersion, equipmentDiag=equipmentDiag, equipmentStateGroup=equipmentStateGroup, cmUChannelID=cmUChannelID, cardPicDetected=cardPicDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditShelfId=auditShelfId, cardShelfId=cardShelfId, cardTestScheduleCommand=cardTestScheduleCommand, portTrapInh=portTrapInh, cardTestResultDesc=cardTestResultDesc, cardHwVersion=cardHwVersion, diskVolumeDiskSize=diskVolumeDiskSize, auditResult=auditResult, auditFailedCount=auditFailedCount, auditLogThrottle=auditLogThrottle, cardBusSpeed=cardBusSpeed, cmResetStatus=cmResetStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmDevice=cmDevice, cardTestId=cardTestId, PYSNMP_MODULE_ID=cadEquipmentMib, cardCarrierHwVersion=cardCarrierHwVersion, consoleOutput=consoleOutput, cardFwVersion=cardFwVersion, cardCPLDVersion=cardCPLDVersion, errEvRowStatus=errEvRowStatus, auditCardId=auditCardId, verboseLevel=verboseLevel, trapCounter=trapCounter, diskVolumeNumber=diskVolumeNumber, auditDescription=auditDescription, auditTotalFailedCount=auditTotalFailedCount, equipmentPortGroup=equipmentPortGroup, cardTestName=cardTestName, cardAction=cardAction, cadEquipmentMibConformance=cadEquipmentMibConformance, cardShelfTestIndex=cardShelfTestIndex, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeName=diskVolumeName, portPrStateChange=portPrStateChange, portMode=portMode, cmResetReason=cmResetReason, TestResult=TestResult, portShelfId=portShelfId, shelfNumber=shelfNumber, cardTestTime=cardTestTime, portLastChangeTime=portLastChangeTime, errEvRecoveryEnabled=errEvRecoveryEnabled, cardRamSize=cardRamSize, cardFpgaSource=cardFpgaSource, cardAdminState=cardAdminState, auditLogOutput=auditLogOutput, cardFwUpdateStatus=cardFwUpdateStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, trapSeverity=trapSeverity, cardTemperature=cardTemperature, portDplxStatus=portDplxStatus, portCardSubType=portCardSubType, diskVolumeDriveId=diskVolumeDriveId, diskVolumeGroup=diskVolumeGroup, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMib=cadEquipmentMib, TestCommand=TestCommand, auditPassedCount=auditPassedCount, cardType=cardType, cardUpTime=cardUpTime, diskVolumeSize=diskVolumeSize, cardEntry=cardEntry, cardDplxStatusChange=cardDplxStatusChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetNotification=cmResetNotification, auditTable=auditTable, fpgaErrorEventTable=fpgaErrorEventTable, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastBootVersion=cardLastBootVersion, auditCycleCount=auditCycleCount, systemKey=systemKey, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestSchedule=cardTestSchedule, cardPicSerialNum=cardPicSerialNum, portType=portType, diskVolumeShelfId=diskVolumeShelfId, portMinDsPower=portMinDsPower, cardTemperatureHighWarn=cardTemperatureHighWarn, portAdminState=portAdminState, auditTotalPassedCount=auditTotalPassedCount, cardSwVersion=cardSwVersion)
