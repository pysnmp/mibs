...
AdminState, CardSubType, FlowControlMode, UnknownState, PortDetectedMode, EqActionType, PortId, DuplexStatus, SecondaryState, PortMode, PortType, CardId, CardType, DiskVolumeUsageLevel, ShelfId, FirmwareSource, PrimaryState, PicType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "CardSubType", "FlowControlMode", "UnknownState", "PortDetectedMode", "EqActionType", "PortId", "DuplexStatus", "SecondaryState", "PortMode", "PortType", "CardId", "CardType", "DiskVolumeUsageLevel", "ShelfId", "FirmwareSource", "PrimaryState", "PicType")
...
TextualConvention, DateAndTime, DisplayString, RowStatus, TruthValue, TimeStamp, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "DisplayString", "RowStatus", "TruthValue", "TimeStamp", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeName=diskVolumeName, cardName=cardName, diskVolumeUsageLevel=diskVolumeUsageLevel, auditResult=auditResult, cardSerialNum=cardSerialNum, cmDevice=cmDevice, cardBootVersion=cardBootVersion, portNumChans=portNumChans, portDplxStatusChange=portDplxStatusChange, cardTestResultNotification=cardTestResultNotification, errEvLoggingEnabled=errEvLoggingEnabled, portMacIfIndex=portMacIfIndex, portTable=portTable, portTxFlowControlMode=portTxFlowControlMode, cardDetectedChange=cardDetectedChange, portRxFlowControlDetected=portRxFlowControlDetected, portCardId=portCardId, shelfName=shelfName, cardSpareGroupId=cardSpareGroupId, consoleOutput=consoleOutput, cardTempNoReportNotification=cardTempNoReportNotification, cardEntry=cardEntry, portTrapInh=portTrapInh, cardPrState=cardPrState, errEvRowStatus=errEvRowStatus, cardCpuType=cardCpuType, portEntry=portEntry, cardTestId=cardTestId, cardTestScheduleCommand=cardTestScheduleCommand, cadEquipmentMib=cadEquipmentMib, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardNumPorts=cardNumPorts, auditAutoScheduling=auditAutoScheduling, cmMacAddress=cmMacAddress, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestName=cardTestName, auditTotalPassedCount=auditTotalPassedCount, auditEntry=auditEntry, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditTotalFailedCount=auditTotalFailedCount, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditCommand=auditCommand, equipmentTraps=equipmentTraps, cardTrapInh=cardTrapInh, cardCpuSpeed=cardCpuSpeed, cardSecStateChange=cardSecStateChange, systemClock=systemClock, cardTemperatureHighWarn=cardTemperatureHighWarn, diskDriveNumber=diskDriveNumber, auditShelfId=auditShelfId, cardTestResult=cardTestResult, remainInDiagMode=remainInDiagMode, cmResetStatus=cmResetStatus, portTxFlowControlDetected=portTxFlowControlDetected, cardFwVersion=cardFwVersion, portMinDsPower=portMinDsPower, cardHwVersion=cardHwVersion, cardPrStateChange=cardPrStateChange, cardCarrierFwVersion=cardCarrierFwVersion, cardId=cardId, diskVolumeNumber=diskVolumeNumber, diskVolumeEntry=diskVolumeEntry, shelfObjects=shelfObjects, equipmentDiag=equipmentDiag, auditStatus=auditStatus, cardTestEntry=cardTestEntry, cmResetUpTime=cmResetUpTime, cardTestDescription=cardTestDescription, portId=portId, auditId=auditId, equipmentShelfGroup=equipmentShelfGroup, diskVolumeFileName=diskVolumeFileName, cardPicHwVersion=cardPicHwVersion, cardShelfTestIndex=cardShelfTestIndex, PYSNMP_MODULE_ID=cadEquipmentMib, portNumber=portNumber, auditLogOutput=auditLogOutput, cardNumber=cardNumber, systemKey=systemKey, cardCarrierSerialNum=cardCarrierSerialNum, auditFailedCount=auditFailedCount, equipmentCardTestGroup=equipmentCardTestGroup, portSecState=portSecState, cardTestIndex=cardTestIndex, cardLastChangeTime=cardLastChangeTime, fpgaErrorEventGroup=fpgaErrorEventGroup, portDetectedMode=portDetectedMode, cardCPLDVersion=cardCPLDVersion, systemGeneral=systemGeneral, cardBusSpeed=cardBusSpeed, portType=portType, cardTestTransId=cardTestTransId, cardRamSize=cardRamSize, cardShelfId=cardShelfId, portPrState=portPrState, diskVolumeShelfId=diskVolumeShelfId, portShelfId=portShelfId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeCardId=diskVolumeCardId, equipmentAudit=equipmentAudit, equipmentError=equipmentError, cmIpAddress=cmIpAddress, equipmentDiagGroup=equipmentDiagGroup, cmResetInfo=cmResetInfo, cardUpTime=cardUpTime, errEvRecoveryEnabled=errEvRecoveryEnabled, portBgpId=portBgpId, portLastChangeTime=portLastChangeTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditCardId=auditCardId, cardLastResetReason=cardLastResetReason, cardPicDetected=cardPicDetected, cardTestSchedule=cardTestSchedule, portMaxDsPower=portMaxDsPower, cardSubDetected=cardSubDetected, portDocsIfIndex=portDocsIfIndex, portConnectorId=portConnectorId, trapCounter=trapCounter, portCurrDsPower=portCurrDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentAuditGroup=equipmentAuditGroup, auditPassedCount=auditPassedCount, cardTestTable=cardTestTable, downstreamPowerLoss=downstreamPowerLoss, eqDiagConfig=eqDiagConfig, verboseLevel=verboseLevel, cardFpgaSource=cardFpgaSource, cmPrimarySID=cmPrimarySID, fpgaErrorEventEntry=fpgaErrorEventEntry, cmResetReason=cmResetReason, systemGeneralGroup=systemGeneralGroup, cardTemperature=cardTemperature, cardTestCommand=cardTestCommand, cardTestTime=cardTestTime, portRxFlowControlMode=portRxFlowControlMode, diskVolumeSize=diskVolumeSize, cardTestResultDesc=cardTestResultDesc, cmRegistrationNotification=cmRegistrationNotification, portAdminState=portAdminState, cardSwVersion=cardSwVersion, shelfNumber=shelfNumber, auditDescription=auditDescription, cardSpareGroupMode=cardSpareGroupMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditTable=auditTable, auditTime=auditTime, TestResult=TestResult, auditName=auditName, diskVolumeGroup=diskVolumeGroup, TestSchedule=TestSchedule, portAction=portAction, errEvLogLevel=errEvLogLevel, equipmentTbl=equipmentTbl, cardLastBootSource=cardLastBootSource, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmVendor=cmVendor, TestTransactionId=TestTransactionId, cardFlashSize=cardFlashSize, cardDplxStatus=cardDplxStatus, cardAdminState=cardAdminState, cardTestType=cardTestType, auditCycleCount=auditCycleCount, equipmentStateGroup=equipmentStateGroup, equipmentCardGroup=equipmentCardGroup, shelfSwVersion=shelfSwVersion, portDplxStatus=portDplxStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTable=cardTable, cardSubType=cardSubType, portDescription=portDescription, cardDetected=cardDetected, diskVolumeDiskSize=diskVolumeDiskSize, cmResetClearNotification=cmResetClearNotification, portMacAddress=portMacAddress, diagTestId=diagTestId, equipmentState=equipmentState, cardAction=cardAction, diskVolumeTable=diskVolumeTable, diskVolumeTrapInh=diskVolumeTrapInh, portMode=portMode, cardLastBootVersion=cardLastBootVersion, portCardSubType=portCardSubType, cardTemperatureHighError=cardTemperatureHighError, cardSecState=cardSecState, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, cmUChannelID=cmUChannelID, trapSeverity=trapSeverity, errEventId=errEventId, portSecStateChange=portSecStateChange, cardType=cardType, diskVolumeId=diskVolumeId, portPrStateChange=portPrStateChange, TestId=TestId, cardPicSerialNum=cardPicSerialNum, TestCommand=TestCommand, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeDriveId=diskVolumeDriveId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetNotification=cmResetNotification, fpgaErrorEventTable=fpgaErrorEventTable, auditLogThrottle=auditLogThrottle, TestType=TestType, cardDplxStatusChange=cardDplxStatusChange, TestScheduleCommand=TestScheduleCommand)
