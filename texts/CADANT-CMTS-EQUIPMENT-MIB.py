...
FlowControlMode, FirmwareSource, SecondaryState, EqActionType, PortId, CardId, CardSubType, PicType, DiskVolumeUsageLevel, AdminState, CardType, DuplexStatus, PortDetectedMode, ShelfId, UnknownState, PortMode, PortType, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "FirmwareSource", "SecondaryState", "EqActionType", "PortId", "CardId", "CardSubType", "PicType", "DiskVolumeUsageLevel", "AdminState", "CardType", "DuplexStatus", "PortDetectedMode", "ShelfId", "UnknownState", "PortMode", "PortType", "PrimaryState")
...
TextualConvention, DateAndTime, DisplayString, RowStatus, TruthValue, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "DisplayString", "RowStatus", "TruthValue", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portMode=portMode, systemKey=systemKey, cardCpuSpeed=cardCpuSpeed, auditFailedCount=auditFailedCount, portAction=portAction, portPrState=portPrState, portCardSubType=portCardSubType, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestId=cardTestId, errEvRowStatus=errEvRowStatus, equipmentState=equipmentState, cardDplxStatus=cardDplxStatus, verboseLevel=verboseLevel, errEvRecoveryEnabled=errEvRecoveryEnabled, auditId=auditId, cmDevice=cmDevice, TestScheduleCommand=TestScheduleCommand, equipmentShelfGroup=equipmentShelfGroup, TestTransactionId=TestTransactionId, diskVolumeSize=diskVolumeSize, portMaxDsPower=portMaxDsPower, cardPrStateChange=cardPrStateChange, cardDetectedChange=cardDetectedChange, eqDiagConfig=eqDiagConfig, diskVolumeShelfId=diskVolumeShelfId, portNumChans=portNumChans, cmIpAddress=cmIpAddress, diskVolumeEntry=diskVolumeEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditStatus=auditStatus, cardCpuType=cardCpuType, trapCounter=trapCounter, portSecStateChange=portSecStateChange, auditName=auditName, auditTotalPassedCount=auditTotalPassedCount, errEvLogLevel=errEvLogLevel, cardTestSchedule=cardTestSchedule, cardTemperatureHighError=cardTemperatureHighError, cardUpTime=cardUpTime, portShelfId=portShelfId, auditAutoScheduling=auditAutoScheduling, equipmentTraps=equipmentTraps, auditCommand=auditCommand, auditTotalFailedCount=auditTotalFailedCount, portRxFlowControlDetected=portRxFlowControlDetected, errEvLoggingEnabled=errEvLoggingEnabled, portType=portType, auditTable=auditTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetNotification=cmResetNotification, cardId=cardId, cardTempNoReportNotification=cardTempNoReportNotification, cardNumPorts=cardNumPorts, cardTestDescription=cardTestDescription, cardName=cardName, cardPicDetected=cardPicDetected, portMinDsPower=portMinDsPower, cardSubType=cardSubType, TestSchedule=TestSchedule, cardPicHwVersion=cardPicHwVersion, cardDplxStatusChange=cardDplxStatusChange, cardTemperatureHighWarn=cardTemperatureHighWarn, trapSeverity=trapSeverity, portDescription=portDescription, auditLogOutput=auditLogOutput, diskVolumeGroup=diskVolumeGroup, errEventId=errEventId, cardTestTransId=cardTestTransId, equipmentTbl=equipmentTbl, remainInDiagMode=remainInDiagMode, cardEntry=cardEntry, auditResult=auditResult, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardRamSize=cardRamSize, equipmentAudit=equipmentAudit, systemClock=systemClock, diagTestId=diagTestId, cardTestTable=cardTestTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestType=cardTestType, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMib=cadEquipmentMib, portEntry=portEntry, auditEntry=auditEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetInfo=cmResetInfo, portDplxStatus=portDplxStatus, auditDescription=auditDescription, cardPrState=cardPrState, equipmentPortGroup=equipmentPortGroup, cmResetClearNotification=cmResetClearNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portBgpId=portBgpId, diskVolumeFileName=diskVolumeFileName, cardHwVersion=cardHwVersion, TestResult=TestResult, diskVolumeNumber=diskVolumeNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, cmUChannelID=cmUChannelID, cmPrimarySID=cmPrimarySID, shelfObjects=shelfObjects, portLastChangeTime=portLastChangeTime, diskVolumeDiskSize=diskVolumeDiskSize, cardSerialNum=cardSerialNum, portPrStateChange=portPrStateChange, cardSubDetected=cardSubDetected, diskDriveNumber=diskDriveNumber, diskVolumeTable=diskVolumeTable, portCardId=portCardId, cardLastBootVersion=cardLastBootVersion, cardTestIndex=cardTestIndex, TestId=TestId, cardAction=cardAction, cardTestTime=cardTestTime, cmMacAddress=cmMacAddress, cardCarrierFwVersion=cardCarrierFwVersion, auditTime=auditTime, cardFwVersion=cardFwVersion, portTxFlowControlDetected=portTxFlowControlDetected, cardBootVersion=cardBootVersion, cardSwVersion=cardSwVersion, portRxFlowControlMode=portRxFlowControlMode, consoleOutput=consoleOutput, cardShelfTestIndex=cardShelfTestIndex, portDocsIfIndex=portDocsIfIndex, portTrapInh=portTrapInh, cardCarrierHwVersion=cardCarrierHwVersion, auditShelfId=auditShelfId, cardSpareGroupMode=cardSpareGroupMode, portConnectorId=portConnectorId, TestCommand=TestCommand, cardTestName=cardTestName, cmResetReason=cmResetReason, portSecState=portSecState, cardType=cardType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portId=portId, cardTempOverHeatNotification=cardTempOverHeatNotification, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMibConformance=cadEquipmentMibConformance, shelfName=shelfName, cardTable=cardTable, equipmentCardGroup=equipmentCardGroup, cardTestCommand=cardTestCommand, portNumber=portNumber, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditPassedCount=auditPassedCount, fpgaErrorEventTable=fpgaErrorEventTable, auditCardId=auditCardId, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeId=diskVolumeId, auditLogThrottle=auditLogThrottle, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDriveId=diskVolumeDriveId, cardLastResetReason=cardLastResetReason, shelfSwVersion=shelfSwVersion, cardNumber=cardNumber, portTable=portTable, cardFpgaSource=cardFpgaSource, cardLastChangeTime=cardLastChangeTime, cardAdminState=cardAdminState, portAdminState=portAdminState, systemGeneral=systemGeneral, cardTestResult=cardTestResult, shelfNumber=shelfNumber, cardShelfId=cardShelfId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSecState=cardSecState, cmResetStatus=cmResetStatus, cardFlashSize=cardFlashSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSecStateChange=cardSecStateChange, equipmentAuditGroup=equipmentAuditGroup, equipmentDiagGroup=equipmentDiagGroup, equipmentStateGroup=equipmentStateGroup, diskVolumeCardId=diskVolumeCardId, cardDetected=cardDetected, portMacAddress=portMacAddress, cardTestEntry=cardTestEntry, cmVendor=cmVendor, portDplxStatusChange=portDplxStatusChange, diskVolumeName=diskVolumeName, cardBusSpeed=cardBusSpeed, portCurrDsPower=portCurrDsPower, portMacIfIndex=portMacIfIndex, cardTestResultNotification=cardTestResultNotification, equipmentError=equipmentError, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentDiag=equipmentDiag, cardLastBootSource=cardLastBootSource, diskVolumeTrapInh=diskVolumeTrapInh, cardSpareGroupId=cardSpareGroupId, cardTrapInh=cardTrapInh, cardCPLDVersion=cardCPLDVersion, portDetectedMode=portDetectedMode, cardCarrierSerialNum=cardCarrierSerialNum, auditCycleCount=auditCycleCount, cmResetUpTime=cmResetUpTime, cardTestScheduleCommand=cardTestScheduleCommand, cardTemperature=cardTemperature, TestType=TestType, diskVolumeUsageNotification=diskVolumeUsageNotification, systemGeneralGroup=systemGeneralGroup, downstreamPowerLoss=downstreamPowerLoss, cardTestResultDesc=cardTestResultDesc)
