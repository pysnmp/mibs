...
CardType, PortType, CardSubType, CardId, EqActionType, DuplexStatus, FlowControlMode, PortId, PicType, SecondaryState, PortMode, PortDetectedMode, UnknownState, FirmwareSource, AdminState, PrimaryState, DiskVolumeUsageLevel, ShelfId = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortType", "CardSubType", "CardId", "EqActionType", "DuplexStatus", "FlowControlMode", "PortId", "PicType", "SecondaryState", "PortMode", "PortDetectedMode", "UnknownState", "FirmwareSource", "AdminState", "PrimaryState", "DiskVolumeUsageLevel", "ShelfId")
...
DisplayString, MacAddress, TimeStamp, RowStatus, DateAndTime, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "TimeStamp", "RowStatus", "DateAndTime", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentError=equipmentError, auditLogThrottle=auditLogThrottle, cmVendor=cmVendor, portMacAddress=portMacAddress, TestSchedule=TestSchedule, portDetectedMode=portDetectedMode, cardTestCommand=cardTestCommand, cardDetectedChange=cardDetectedChange, cardSpareGroupMode=cardSpareGroupMode, TestScheduleCommand=TestScheduleCommand, diskVolumeCardId=diskVolumeCardId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentCardTestGroup=equipmentCardTestGroup, equipmentState=equipmentState, portId=portId, portDescription=portDescription, portTrapInh=portTrapInh, cmResetReason=cmResetReason, cmResetInfo=cmResetInfo, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTable=cardTable, portNumber=portNumber, diskVolumeFileName=diskVolumeFileName, portMode=portMode, auditStatus=auditStatus, portDplxStatus=portDplxStatus, auditEntry=auditEntry, portCardSubType=portCardSubType, TestCommand=TestCommand, cardCarrierFwVersion=cardCarrierFwVersion, cardBusSpeed=cardBusSpeed, errEvLoggingEnabled=errEvLoggingEnabled, TestId=TestId, equipmentDiagGroup=equipmentDiagGroup, cardTestResultNotification=cardTestResultNotification, portLastChangeTime=portLastChangeTime, cardTestId=cardTestId, auditTime=auditTime, portRxFlowControlDetected=portRxFlowControlDetected, cardPicDetected=cardPicDetected, cadEquipmentMib=cadEquipmentMib, cardNumber=cardNumber, auditLogOutput=auditLogOutput, cardTestEntry=cardTestEntry, PYSNMP_MODULE_ID=cadEquipmentMib, auditCardId=auditCardId, equipmentTraps=equipmentTraps, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardAction=cardAction, remainInDiagMode=remainInDiagMode, cardTestName=cardTestName, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSecStateChange=cardSecStateChange, diskVolumeDriveId=diskVolumeDriveId, errEvLogLevel=errEvLogLevel, cardTrapInh=cardTrapInh, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTemperature=cardTemperature, cardId=cardId, equipmentTbl=equipmentTbl, cardSerialNum=cardSerialNum, portMacIfIndex=portMacIfIndex, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentStateGroup=equipmentStateGroup, diskVolumeId=diskVolumeId, portPrState=portPrState, portCardId=portCardId, equipmentCardGroup=equipmentCardGroup, systemKey=systemKey, portType=portType, cardLastBootVersion=cardLastBootVersion, cardFwUpdateStatus=cardFwUpdateStatus, cardHwVersion=cardHwVersion, cardTestDescription=cardTestDescription, portEntry=portEntry, cardType=cardType, cardFlashSize=cardFlashSize, cardSecState=cardSecState, equipmentDiag=equipmentDiag, diskVolumeGroup=diskVolumeGroup, cardDplxStatus=cardDplxStatus, cmPrimarySID=cmPrimarySID, equipmentPortGroup=equipmentPortGroup, shelfName=shelfName, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestScheduleCommand=cardTestScheduleCommand, cmResetStatus=cmResetStatus, portConnectorId=portConnectorId, portDplxStatusChange=portDplxStatusChange, cardUpTime=cardUpTime, diskVolumeUsageNotification=diskVolumeUsageNotification, trapSeverity=trapSeverity, errEventId=errEventId, portMaxDsPower=portMaxDsPower, cardName=cardName, cmResetNotification=cmResetNotification, systemClock=systemClock, trapCounter=trapCounter, portAdminState=portAdminState, cardDplxStatusChange=cardDplxStatusChange, auditCommand=auditCommand, auditFailedCount=auditFailedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, downstreamPowerLoss=downstreamPowerLoss, cardSpareGroupId=cardSpareGroupId, auditAutoScheduling=auditAutoScheduling, cardTempNoReportNotification=cardTempNoReportNotification, portDocsIfIndex=portDocsIfIndex, cmUChannelID=cmUChannelID, cardCpuSpeed=cardCpuSpeed, cardSubDetected=cardSubDetected, cardSubType=cardSubType, auditTable=auditTable, auditTotalPassedCount=auditTotalPassedCount, systemGeneral=systemGeneral, cardCPLDVersion=cardCPLDVersion, portTable=portTable, cardShelfId=cardShelfId, equipmentShelfGroup=equipmentShelfGroup, verboseLevel=verboseLevel, cardTestTime=cardTestTime, cardPicHwVersion=cardPicHwVersion, cardRamSize=cardRamSize, portBgpId=portBgpId, cardLastBootSource=cardLastBootSource, portRxFlowControlMode=portRxFlowControlMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTable=diskVolumeTable, cardCarrierSerialNum=cardCarrierSerialNum, diagTestId=diagTestId, cardAdminState=cardAdminState, portTxFlowControlDetected=portTxFlowControlDetected, TestResult=TestResult, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditResult=auditResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeNumber=diskVolumeNumber, cmMacAddress=cmMacAddress, diskVolumeEntry=diskVolumeEntry, cmResetClearNotification=cmResetClearNotification, cardDetected=cardDetected, cardNumPorts=cardNumPorts, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, consoleOutput=consoleOutput, auditName=auditName, cmResetUpTime=cmResetUpTime, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, auditCycleCount=auditCycleCount, cardSwVersion=cardSwVersion, systemGeneralGroup=systemGeneralGroup, cardFpgaSource=cardFpgaSource, diskVolumeUsageLevel=diskVolumeUsageLevel, portMinDsPower=portMinDsPower, auditShelfId=auditShelfId, equipmentAuditGroup=equipmentAuditGroup, cardCarrierHwVersion=cardCarrierHwVersion, portSecState=portSecState, portTxFlowControlMode=portTxFlowControlMode, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portPrStateChange=portPrStateChange, cardPrState=cardPrState, cardEntry=cardEntry, cardCpuType=cardCpuType, cardLastResetReason=cardLastResetReason, cardShelfTestIndex=cardShelfTestIndex, cmDevice=cmDevice, cardTestSchedule=cardTestSchedule, auditId=auditId, cardPrStateChange=cardPrStateChange, equipmentAudit=equipmentAudit, diskVolumeName=diskVolumeName, auditPassedCount=auditPassedCount, cardTestResultDesc=cardTestResultDesc, cmIpAddress=cmIpAddress, cardTemperatureHighError=cardTemperatureHighError, shelfSwVersion=shelfSwVersion, errEvRowStatus=errEvRowStatus, portCurrDsPower=portCurrDsPower, shelfObjects=shelfObjects, diskVolumeSize=diskVolumeSize, shelfNumber=shelfNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, diskDriveNumber=diskDriveNumber, portNumChans=portNumChans, portAction=portAction, cardTestResult=cardTestResult, TestType=TestType, cardTestIndex=cardTestIndex, auditTotalFailedCount=auditTotalFailedCount, cardLastChangeTime=cardLastChangeTime, cardTestTable=cardTestTable, portSecStateChange=portSecStateChange, eqDiagConfig=eqDiagConfig, diskVolumeShelfId=diskVolumeShelfId, cardTestType=cardTestType, TestTransactionId=TestTransactionId, portShelfId=portShelfId, cardPicSerialNum=cardPicSerialNum, cardBootVersion=cardBootVersion, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeTrapInh=diskVolumeTrapInh, cardFwVersion=cardFwVersion, auditDescription=auditDescription, cardTestTransId=cardTestTransId)
