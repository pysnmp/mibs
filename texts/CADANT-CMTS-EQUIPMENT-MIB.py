...
UnknownState, FlowControlMode, CardId, CardType, CardSubType, AdminState, DuplexStatus, EqActionType, PortId, PortType, PrimaryState, PortDetectedMode, DiskVolumeUsageLevel, PicType, PortMode, ShelfId, SecondaryState, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "FlowControlMode", "CardId", "CardType", "CardSubType", "AdminState", "DuplexStatus", "EqActionType", "PortId", "PortType", "PrimaryState", "PortDetectedMode", "DiskVolumeUsageLevel", "PicType", "PortMode", "ShelfId", "SecondaryState", "FirmwareSource")
...
MacAddress, TruthValue, TimeStamp, DisplayString, DateAndTime, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "TimeStamp", "DisplayString", "DateAndTime", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskDriveNumber=diskDriveNumber, portNumChans=portNumChans, cardSecState=cardSecState, portMode=portMode, portMinDsPower=portMinDsPower, cardTempNoReportNotification=cardTempNoReportNotification, cardShelfTestIndex=cardShelfTestIndex, portCardSubType=portCardSubType, portCurrDsPower=portCurrDsPower, eqDiagConfig=eqDiagConfig, errEvLoggingEnabled=errEvLoggingEnabled, cardShelfId=cardShelfId, equipmentTbl=equipmentTbl, auditName=auditName, trapCounter=trapCounter, cardUpTime=cardUpTime, cardName=cardName, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvRowStatus=errEvRowStatus, portDplxStatus=portDplxStatus, auditStatus=auditStatus, diskVolumeNumber=diskVolumeNumber, cardSerialNum=cardSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, cardLastBootVersion=cardLastBootVersion, cardTestTransId=cardTestTransId, cardSecStateChange=cardSecStateChange, cardDplxStatus=cardDplxStatus, cardFpgaSource=cardFpgaSource, cardDetectedChange=cardDetectedChange, cardRamSize=cardRamSize, TestCommand=TestCommand, cardAction=cardAction, cardTestCommand=cardTestCommand, cmRegistrationNotification=cmRegistrationNotification, cardLastBootSource=cardLastBootSource, cardAdminState=cardAdminState, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestType=cardTestType, portTrapInh=portTrapInh, cardPrStateChange=cardPrStateChange, cardTestTime=cardTestTime, portMacAddress=portMacAddress, PYSNMP_MODULE_ID=cadEquipmentMib, portBgpId=portBgpId, portSecStateChange=portSecStateChange, equipmentAuditGroup=equipmentAuditGroup, cardCpuType=cardCpuType, TestResult=TestResult, cardSubDetected=cardSubDetected, portDetectedMode=portDetectedMode, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardPicDetected=cardPicDetected, auditAutoScheduling=auditAutoScheduling, portDplxStatusChange=portDplxStatusChange, cardTrapInh=cardTrapInh, equipmentDiag=equipmentDiag, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastChangeTime=cardLastChangeTime, portDescription=portDescription, portTxFlowControlMode=portTxFlowControlMode, portPrState=portPrState, equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, auditTotalPassedCount=auditTotalPassedCount, equipmentAudit=equipmentAudit, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeShelfId=diskVolumeShelfId, cardDetected=cardDetected, cardTestName=cardTestName, cmPrimarySID=cmPrimarySID, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMacIfIndex=portMacIfIndex, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeId=diskVolumeId, remainInDiagMode=remainInDiagMode, downstreamPowerLoss=downstreamPowerLoss, TestScheduleCommand=TestScheduleCommand, cmDevice=cmDevice, auditCommand=auditCommand, cmResetStatus=cmResetStatus, cardCpuSpeed=cardCpuSpeed, equipmentCardTestGroup=equipmentCardTestGroup, cardSpareGroupId=cardSpareGroupId, cardSwVersion=cardSwVersion, cardLastResetReason=cardLastResetReason, cmResetReason=cmResetReason, errEventId=errEventId, cmUChannelID=cmUChannelID, auditCycleCount=auditCycleCount, fpgaErrorEventTable=fpgaErrorEventTable, shelfName=shelfName, cmResetNotification=cmResetNotification, cardHwVersion=cardHwVersion, cardFwUpdateStatus=cardFwUpdateStatus, TestId=TestId, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeName=diskVolumeName, cardTestScheduleCommand=cardTestScheduleCommand, auditTable=auditTable, cmResetClearNotification=cmResetClearNotification, verboseLevel=verboseLevel, portSecState=portSecState, auditTime=auditTime, equipmentPortGroup=equipmentPortGroup, cardTestId=cardTestId, cardFwVersion=cardFwVersion, cardTestResult=cardTestResult, cardTemperature=cardTemperature, cardCarrierHwVersion=cardCarrierHwVersion, portShelfId=portShelfId, cardTemperatureHighError=cardTemperatureHighError, diskVolumeTrapInh=diskVolumeTrapInh, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentTraps=equipmentTraps, cardType=cardType, portConnectorId=portConnectorId, systemGeneral=systemGeneral, auditTotalFailedCount=auditTotalFailedCount, cardTestResultNotification=cardTestResultNotification, portEntry=portEntry, cardBusSpeed=cardBusSpeed, diskVolumeDiskSize=diskVolumeDiskSize, cmResetInfo=cmResetInfo, cardTestTable=cardTestTable, portId=portId, cmIpAddress=cmIpAddress, cardTestIndex=cardTestIndex, cardBootVersion=cardBootVersion, cardEntry=cardEntry, cardTestResultDesc=cardTestResultDesc, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSubType=cardSubType, portAction=portAction, cardFlashSize=cardFlashSize, shelfNumber=shelfNumber, portType=portType, diskVolumeFileName=diskVolumeFileName, cardTestDescription=cardTestDescription, errEvLogLevel=errEvLogLevel, auditCardId=auditCardId, diskVolumeTable=diskVolumeTable, equipmentDiagGroup=equipmentDiagGroup, diskVolumeSize=diskVolumeSize, auditDescription=auditDescription, portNumber=portNumber, diagTestId=diagTestId, cardPicSerialNum=cardPicSerialNum, consoleOutput=consoleOutput, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditEntry=auditEntry, equipmentStateGroup=equipmentStateGroup, cardTestSchedule=cardTestSchedule, auditPassedCount=auditPassedCount, portCardId=portCardId, cardCPLDVersion=cardCPLDVersion, cmMacAddress=cmMacAddress, cadEquipmentMib=cadEquipmentMib, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageNotification=diskVolumeUsageNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmVendor=cmVendor, systemGeneralGroup=systemGeneralGroup, shelfObjects=shelfObjects, equipmentError=equipmentError, fpgaErrorEventGroup=fpgaErrorEventGroup, TestType=TestType, diskVolumeEntry=diskVolumeEntry, cardNumPorts=cardNumPorts, cardCarrierFwVersion=cardCarrierFwVersion, portLastChangeTime=portLastChangeTime, cardDplxStatusChange=cardDplxStatusChange, cardPicHwVersion=cardPicHwVersion, portTable=portTable, cadEquipmentMibGroup=cadEquipmentMibGroup, portDocsIfIndex=portDocsIfIndex, systemClock=systemClock, cardTestEntry=cardTestEntry, portPrStateChange=portPrStateChange, TestSchedule=TestSchedule, portMaxDsPower=portMaxDsPower, auditShelfId=auditShelfId, cardTable=cardTable, TestTransactionId=TestTransactionId, auditLogOutput=auditLogOutput, auditResult=auditResult, cardPrState=cardPrState, equipmentCardGroup=equipmentCardGroup, diskVolumeGroup=diskVolumeGroup, equipmentShelfGroup=equipmentShelfGroup, cardId=cardId, systemKey=systemKey, cmResetUpTime=cmResetUpTime, diskVolumeDriveId=diskVolumeDriveId, cardNumber=cardNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSpareGroupMode=cardSpareGroupMode, portAdminState=portAdminState, trapSeverity=trapSeverity, auditLogThrottle=auditLogThrottle, auditId=auditId, auditFailedCount=auditFailedCount, shelfSwVersion=shelfSwVersion)
