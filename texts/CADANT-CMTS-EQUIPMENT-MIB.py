...
CardId, PortType, DiskVolumeUsageLevel, CardSubType, PortDetectedMode, FirmwareSource, PicType, CardType, DuplexStatus, PortId, EqActionType, AdminState, FlowControlMode, PortMode, SecondaryState, ShelfId, PrimaryState, UnknownState = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortType", "DiskVolumeUsageLevel", "CardSubType", "PortDetectedMode", "FirmwareSource", "PicType", "CardType", "DuplexStatus", "PortId", "EqActionType", "AdminState", "FlowControlMode", "PortMode", "SecondaryState", "ShelfId", "PrimaryState", "UnknownState")
...
TimeStamp, RowStatus, TruthValue, MacAddress, DisplayString, TextualConvention, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "RowStatus", "TruthValue", "MacAddress", "DisplayString", "TextualConvention", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", fpgaErrorEventTable=fpgaErrorEventTable, errEvLogLevel=errEvLogLevel, systemGeneral=systemGeneral, cardSubDetected=cardSubDetected, cardDplxStatus=cardDplxStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfName=shelfName, cardType=cardType, portDplxStatus=portDplxStatus, portMacIfIndex=portMacIfIndex, auditFailedCount=auditFailedCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardHwVersion=cardHwVersion, portId=portId, auditEntry=auditEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeShelfId=diskVolumeShelfId, portRxFlowControlMode=portRxFlowControlMode, shelfSwVersion=shelfSwVersion, consoleOutput=consoleOutput, portSecStateChange=portSecStateChange, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentState=equipmentState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeGroup=diskVolumeGroup, cardTrapInh=cardTrapInh, errEvRowStatus=errEvRowStatus, portShelfId=portShelfId, cardFwUpdateStatus=cardFwUpdateStatus, cardDetected=cardDetected, cardDplxStatusChange=cardDplxStatusChange, diskVolumeDiskSize=diskVolumeDiskSize, cmPrimarySID=cmPrimarySID, cadEquipmentMib=cadEquipmentMib, TestSchedule=TestSchedule, auditPassedCount=auditPassedCount, cmVendor=cmVendor, equipmentCardTestGroup=equipmentCardTestGroup, cardTestResultNotification=cardTestResultNotification, cardTemperature=cardTemperature, equipmentAuditGroup=equipmentAuditGroup, equipmentPortGroup=equipmentPortGroup, portMaxDsPower=portMaxDsPower, auditTime=auditTime, TestCommand=TestCommand, cardNumber=cardNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardLastBootSource=cardLastBootSource, diskVolumeTrapInh=diskVolumeTrapInh, trapSeverity=trapSeverity, cardFpgaSource=cardFpgaSource, auditCommand=auditCommand, cardTestCommand=cardTestCommand, auditTotalFailedCount=auditTotalFailedCount, cmResetReason=cmResetReason, auditCardId=auditCardId, eqDiagConfig=eqDiagConfig, portLastChangeTime=portLastChangeTime, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeCardId=diskVolumeCardId, cardBusSpeed=cardBusSpeed, cardSecStateChange=cardSecStateChange, portType=portType, cardTempNoReportNotification=cardTempNoReportNotification, cardId=cardId, cardSpareGroupMode=cardSpareGroupMode, equipmentError=equipmentError, cmUChannelID=cmUChannelID, TestResult=TestResult, diskVolumeName=diskVolumeName, diskVolumeFileName=diskVolumeFileName, auditTotalPassedCount=auditTotalPassedCount, cmRegistrationNotification=cmRegistrationNotification, portMode=portMode, shelfNumber=shelfNumber, TestScheduleCommand=TestScheduleCommand, cardNumPorts=cardNumPorts, portAction=portAction, fpgaErrorEventGroup=fpgaErrorEventGroup, auditResult=auditResult, cardSecState=cardSecState, cardSubType=cardSubType, cardCpuType=cardCpuType, portTxFlowControlMode=portTxFlowControlMode, equipmentAudit=equipmentAudit, cmMacAddress=cmMacAddress, portBgpId=portBgpId, cardTestType=cardTestType, equipmentStateGroup=equipmentStateGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, cardFlashSize=cardFlashSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentDiagGroup=equipmentDiagGroup, cardCpuSpeed=cardCpuSpeed, diskDriveNumber=diskDriveNumber, cardPicDetected=cardPicDetected, portSecState=portSecState, cardEntry=cardEntry, cardTestTransId=cardTestTransId, equipmentDiag=equipmentDiag, auditCycleCount=auditCycleCount, cardDetectedChange=cardDetectedChange, portTxFlowControlDetected=portTxFlowControlDetected, cardTestEntry=cardTestEntry, cardShelfTestIndex=cardShelfTestIndex, cmIpAddress=cmIpAddress, cardTestScheduleCommand=cardTestScheduleCommand, auditLogThrottle=auditLogThrottle, trapCounter=trapCounter, auditAutoScheduling=auditAutoScheduling, auditTable=auditTable, portMinDsPower=portMinDsPower, cardTestId=cardTestId, cardTable=cardTable, diagTestId=diagTestId, portCurrDsPower=portCurrDsPower, diskVolumeSize=diskVolumeSize, portTable=portTable, portDescription=portDescription, portEntry=portEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cardRamSize=cardRamSize, cardTestResult=cardTestResult, cmResetInfo=cmResetInfo, cardPicHwVersion=cardPicHwVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, portPrStateChange=portPrStateChange, cardName=cardName, portRxFlowControlDetected=portRxFlowControlDetected, portNumChans=portNumChans, TestTransactionId=TestTransactionId, errEventId=errEventId, portNumber=portNumber, diskVolumeUsageNotification=diskVolumeUsageNotification, verboseLevel=verboseLevel, diskVolumeId=diskVolumeId, equipmentShelfGroup=equipmentShelfGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardPrState=cardPrState, cardTestResultDesc=cardTestResultDesc, portDplxStatusChange=portDplxStatusChange, cardTestTable=cardTestTable, cardLastBootVersion=cardLastBootVersion, cardTestIndex=cardTestIndex, cardCarrierSerialNum=cardCarrierSerialNum, TestId=TestId, cardPrStateChange=cardPrStateChange, diskVolumeEntry=diskVolumeEntry, portAdminState=portAdminState, auditId=auditId, cardUpTime=cardUpTime, auditStatus=auditStatus, portDetectedMode=portDetectedMode, cardTestTime=cardTestTime, auditDescription=auditDescription, systemGeneralGroup=systemGeneralGroup, cardSwVersion=cardSwVersion, cardLastChangeTime=cardLastChangeTime, cardBootVersion=cardBootVersion, diskVolumeNumber=diskVolumeNumber, cardPicSerialNum=cardPicSerialNum, portDocsIfIndex=portDocsIfIndex, cardAdminState=cardAdminState, cardTestName=cardTestName, cardTestSchedule=cardTestSchedule, cadEquipmentMibConformance=cadEquipmentMibConformance, portCardSubType=portCardSubType, cardSerialNum=cardSerialNum, cmResetUpTime=cmResetUpTime, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardShelfId=cardShelfId, shelfObjects=shelfObjects, equipmentTbl=equipmentTbl, cmResetNotification=cmResetNotification, auditShelfId=auditShelfId, TestType=TestType, auditName=auditName, cardTemperatureHighError=cardTemperatureHighError, diskVolumeDriveId=diskVolumeDriveId, portCardId=portCardId, systemClock=systemClock, cmDevice=cmDevice, equipmentTraps=equipmentTraps, portPrState=portPrState, cardTestDescription=cardTestDescription, portTrapInh=portTrapInh, cmResetClearNotification=cmResetClearNotification, cardLastResetReason=cardLastResetReason, cardFwVersion=cardFwVersion, cardCPLDVersion=cardCPLDVersion, cardCarrierFwVersion=cardCarrierFwVersion, errEvLoggingEnabled=errEvLoggingEnabled, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portConnectorId=portConnectorId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeTable=diskVolumeTable, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogOutput=auditLogOutput, cmResetStatus=cmResetStatus, portMacAddress=portMacAddress, cardAction=cardAction, remainInDiagMode=remainInDiagMode, equipmentCardGroup=equipmentCardGroup, systemKey=systemKey, cardSpareGroupId=cardSpareGroupId)
