...
AdminState, UnknownState, CardSubType, CardId, DiskVolumeUsageLevel, FlowControlMode, SecondaryState, PortDetectedMode, PicType, PortId, ShelfId, PortType, FirmwareSource, PortMode, EqActionType, DuplexStatus, CardType, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "AdminState", "UnknownState", "CardSubType", "CardId", "DiskVolumeUsageLevel", "FlowControlMode", "SecondaryState", "PortDetectedMode", "PicType", "PortId", "ShelfId", "PortType", "FirmwareSource", "PortMode", "EqActionType", "DuplexStatus", "CardType", "PrimaryState")
...
TimeStamp, DateAndTime, MacAddress, TextualConvention, DisplayString, TruthValue, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DateAndTime", "MacAddress", "TextualConvention", "DisplayString", "TruthValue", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastResetReason=cardLastResetReason, TestType=TestType, cmPrimarySID=cmPrimarySID, auditName=auditName, portCardId=portCardId, cardTestTransId=cardTestTransId, TestResult=TestResult, shelfObjects=shelfObjects, cardCpuSpeed=cardCpuSpeed, auditTotalPassedCount=auditTotalPassedCount, diskVolumeTrapInh=diskVolumeTrapInh, portSecStateChange=portSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSerialNum=cardSerialNum, cardPicHwVersion=cardPicHwVersion, cardSecStateChange=cardSecStateChange, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTemperatureHighError=cardTemperatureHighError, portShelfId=portShelfId, cardCPLDVersion=cardCPLDVersion, cardSubType=cardSubType, diskVolumeGroup=diskVolumeGroup, cardSpareGroupMode=cardSpareGroupMode, cardEntry=cardEntry, cardTestTable=cardTestTable, cmResetStatus=cmResetStatus, portType=portType, portDescription=portDescription, cardNumPorts=cardNumPorts, diskVolumeId=diskVolumeId, cardBootVersion=cardBootVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, shelfSwVersion=shelfSwVersion, cardFwUpdateStatus=cardFwUpdateStatus, cardTestIndex=cardTestIndex, cardLastBootVersion=cardLastBootVersion, cardTestType=cardTestType, equipmentAuditGroup=equipmentAuditGroup, cardAction=cardAction, shelfName=shelfName, cardCpuType=cardCpuType, cardFlashSize=cardFlashSize, portMinDsPower=portMinDsPower, equipmentShelfGroup=equipmentShelfGroup, portDocsIfIndex=portDocsIfIndex, errEvRecoveryEnabled=errEvRecoveryEnabled, auditLogOutput=auditLogOutput, cmDevice=cmDevice, cmUChannelID=cmUChannelID, cardTrapInh=cardTrapInh, cardTestCommand=cardTestCommand, cmResetUpTime=cmResetUpTime, auditResult=auditResult, errEvLogLevel=errEvLogLevel, systemGeneralGroup=systemGeneralGroup, equipmentError=equipmentError, TestScheduleCommand=TestScheduleCommand, auditTable=auditTable, portLastChangeTime=portLastChangeTime, portBgpId=portBgpId, consoleOutput=consoleOutput, auditPassedCount=auditPassedCount, portDplxStatus=portDplxStatus, cardHwVersion=cardHwVersion, cardSpareGroupId=cardSpareGroupId, equipmentCardGroup=equipmentCardGroup, diskDriveNumber=diskDriveNumber, TestTransactionId=TestTransactionId, diskVolumeDiskSize=diskVolumeDiskSize, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditFailedCount=auditFailedCount, auditCycleCount=auditCycleCount, diskVolumeFileName=diskVolumeFileName, auditEntry=auditEntry, diskVolumeTable=diskVolumeTable, portTrapInh=portTrapInh, TestSchedule=TestSchedule, cardSecState=cardSecState, cardPrStateChange=cardPrStateChange, equipmentAudit=equipmentAudit, cadEquipmentMib=cadEquipmentMib, cardBusSpeed=cardBusSpeed, diskVolumeSize=diskVolumeSize, cardShelfId=cardShelfId, cardRamSize=cardRamSize, portPrStateChange=portPrStateChange, portAction=portAction, portDplxStatusChange=portDplxStatusChange, cardTestId=cardTestId, portRxFlowControlMode=portRxFlowControlMode, downstreamPowerLoss=downstreamPowerLoss, cardSubDetected=cardSubDetected, portConnectorId=portConnectorId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardId=cardId, cardTable=cardTable, cmResetReason=cmResetReason, fpgaErrorEventEntry=fpgaErrorEventEntry, portMacIfIndex=portMacIfIndex, cardTempNoReportNotification=cardTempNoReportNotification, equipmentState=equipmentState, portCardSubType=portCardSubType, systemClock=systemClock, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardShelfTestIndex=cardShelfTestIndex, diskVolumeDriveId=diskVolumeDriveId, diagTestId=diagTestId, cardUpTime=cardUpTime, cardTestResultDesc=cardTestResultDesc, auditLogThrottle=auditLogThrottle, cardTestResult=cardTestResult, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeName=diskVolumeName, eqDiagConfig=eqDiagConfig, portTxFlowControlMode=portTxFlowControlMode, portMacAddress=portMacAddress, cmResetClearNotification=cmResetClearNotification, cardDetected=cardDetected, cardTestDescription=cardTestDescription, fpgaErrorEventTable=fpgaErrorEventTable, auditDescription=auditDescription, portCurrDsPower=portCurrDsPower, equipmentDiagGroup=equipmentDiagGroup, verboseLevel=verboseLevel, portNumber=portNumber, portEntry=portEntry, cmResetNotification=cmResetNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardName=cardName, cardFpgaSource=cardFpgaSource, cardCarrierSerialNum=cardCarrierSerialNum, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDplxStatus=cardDplxStatus, cardCarrierFwVersion=cardCarrierFwVersion, systemGeneral=systemGeneral, portMaxDsPower=portMaxDsPower, errEvLoggingEnabled=errEvLoggingEnabled, cardDetectedChange=cardDetectedChange, cardTemperature=cardTemperature, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibConformance=cadEquipmentMibConformance, portSecState=portSecState, portPrState=portPrState, portRxFlowControlDetected=portRxFlowControlDetected, equipmentDiag=equipmentDiag, cardTestEntry=cardTestEntry, auditCommand=auditCommand, auditTotalFailedCount=auditTotalFailedCount, errEvRowStatus=errEvRowStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, auditStatus=auditStatus, shelfNumber=shelfNumber, cardAdminState=cardAdminState, equipmentTraps=equipmentTraps, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentPortGroup=equipmentPortGroup, cardTestResultNotification=cardTestResultNotification, cardTestSchedule=cardTestSchedule, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeCardId=diskVolumeCardId, cardTestName=cardTestName, cardLastChangeTime=cardLastChangeTime, portNumChans=portNumChans, cardSwVersion=cardSwVersion, systemKey=systemKey, equipmentStateGroup=equipmentStateGroup, auditCardId=auditCardId, portId=portId, fpgaErrorEventGroup=fpgaErrorEventGroup, cmRegistrationNotification=cmRegistrationNotification, diskVolumeNumber=diskVolumeNumber, diskVolumeShelfId=diskVolumeShelfId, auditShelfId=auditShelfId, TestId=TestId, auditTime=auditTime, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentTbl=equipmentTbl, cardNumber=cardNumber, cmVendor=cmVendor, cardTestScheduleCommand=cardTestScheduleCommand, cmMacAddress=cmMacAddress, cardPicDetected=cardPicDetected, remainInDiagMode=remainInDiagMode, trapSeverity=trapSeverity, cardType=cardType, cardTestTime=cardTestTime, portMode=portMode, portAdminState=portAdminState, TestCommand=TestCommand, errEventId=errEventId, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetInfo=cmResetInfo, cmIpAddress=cmIpAddress, cardLastBootSource=cardLastBootSource, cardFwVersion=cardFwVersion, portTxFlowControlDetected=portTxFlowControlDetected, cardCarrierHwVersion=cardCarrierHwVersion, cardPrState=cardPrState, portDetectedMode=portDetectedMode, portTable=portTable, auditId=auditId, trapCounter=trapCounter, cardDplxStatusChange=cardDplxStatusChange, diskVolumeEntry=diskVolumeEntry)
