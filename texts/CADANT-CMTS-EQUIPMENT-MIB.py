...
CardId, SecondaryState, FirmwareSource, PicType, ShelfId, CardSubType, EqActionType, PortMode, UnknownState, PortType, AdminState, PortDetectedMode, CardType, FlowControlMode, PrimaryState, DuplexStatus, PortId, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "CardId", "SecondaryState", "FirmwareSource", "PicType", "ShelfId", "CardSubType", "EqActionType", "PortMode", "UnknownState", "PortType", "AdminState", "PortDetectedMode", "CardType", "FlowControlMode", "PrimaryState", "DuplexStatus", "PortId", "DiskVolumeUsageLevel")
...
RowStatus, TimeStamp, TextualConvention, DateAndTime, DisplayString, TruthValue, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "TextualConvention", "DateAndTime", "DisplayString", "TruthValue", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmMacAddress=cmMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeNumber=diskVolumeNumber, equipmentError=equipmentError, cardLastBootSource=cardLastBootSource, cmResetStatus=cmResetStatus, cmUChannelID=cmUChannelID, cardSpareGroupId=cardSpareGroupId, equipmentShelfGroup=equipmentShelfGroup, auditName=auditName, cmResetNotification=cmResetNotification, diskVolumeId=diskVolumeId, TestScheduleCommand=TestScheduleCommand, TestId=TestId, cardType=cardType, fpgaErrorEventTable=fpgaErrorEventTable, portMaxDsPower=portMaxDsPower, cardRamSize=cardRamSize, cardDplxStatus=cardDplxStatus, cardTestTransId=cardTestTransId, portDplxStatusChange=portDplxStatusChange, cardEntry=cardEntry, portCardId=portCardId, cardSerialNum=cardSerialNum, cmResetClearNotification=cmResetClearNotification, equipmentStateGroup=equipmentStateGroup, cmResetInfo=cmResetInfo, equipmentTbl=equipmentTbl, portDetectedMode=portDetectedMode, cmPrimarySID=cmPrimarySID, cardTestDescription=cardTestDescription, TestResult=TestResult, cardLastResetReason=cardLastResetReason, systemClock=systemClock, portAdminState=portAdminState, cardPrStateChange=cardPrStateChange, cardUpTime=cardUpTime, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTempNoReportNotification=cardTempNoReportNotification, cardSubDetected=cardSubDetected, eqDiagConfig=eqDiagConfig, cardCarrierFwVersion=cardCarrierFwVersion, portDocsIfIndex=portDocsIfIndex, diskVolumeTable=diskVolumeTable, auditTotalFailedCount=auditTotalFailedCount, portCurrDsPower=portCurrDsPower, auditStatus=auditStatus, portPrState=portPrState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, systemGeneral=systemGeneral, portMinDsPower=portMinDsPower, portDescription=portDescription, equipmentPortGroup=equipmentPortGroup, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibGroup=cadEquipmentMibGroup, systemGeneralGroup=systemGeneralGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portTxFlowControlDetected=portTxFlowControlDetected, cardSwVersion=cardSwVersion, systemKey=systemKey, errEvLogLevel=errEvLogLevel, TestTransactionId=TestTransactionId, cardFlashSize=cardFlashSize, portTrapInh=portTrapInh, diskVolumeUsageNotification=diskVolumeUsageNotification, cardFpgaSource=cardFpgaSource, shelfName=shelfName, cardCpuSpeed=cardCpuSpeed, portNumber=portNumber, auditLogThrottle=auditLogThrottle, cardShelfId=cardShelfId, cardPicSerialNum=cardPicSerialNum, cardNumber=cardNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, trapSeverity=trapSeverity, portSecStateChange=portSecStateChange, portEntry=portEntry, portType=portType, cardHwVersion=cardHwVersion, diskVolumeTrapInh=diskVolumeTrapInh, portDplxStatus=portDplxStatus, diskVolumeGroup=diskVolumeGroup, shelfSwVersion=shelfSwVersion, portTxFlowControlMode=portTxFlowControlMode, cardTestType=cardTestType, auditTime=auditTime, verboseLevel=verboseLevel, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeDiskSize=diskVolumeDiskSize, cardTestName=cardTestName, cardDetectedChange=cardDetectedChange, cardTestResultDesc=cardTestResultDesc, cardPrState=cardPrState, TestCommand=TestCommand, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentDiag=equipmentDiag, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portNumChans=portNumChans, cardSpareGroupMode=cardSpareGroupMode, cardTemperature=cardTemperature, remainInDiagMode=remainInDiagMode, cmResetReason=cmResetReason, cardTestTime=cardTestTime, auditResult=auditResult, diskVolumeDriveId=diskVolumeDriveId, equipmentCardGroup=equipmentCardGroup, shelfObjects=shelfObjects, auditTotalPassedCount=auditTotalPassedCount, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeShelfId=diskVolumeShelfId, diskDriveNumber=diskDriveNumber, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlMode=portRxFlowControlMode, auditShelfId=auditShelfId, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, cmIpAddress=cmIpAddress, diskVolumeName=diskVolumeName, cardBusSpeed=cardBusSpeed, portId=portId, cardCarrierSerialNum=cardCarrierSerialNum, cardId=cardId, portConnectorId=portConnectorId, cardSubType=cardSubType, auditPassedCount=auditPassedCount, cardFwVersion=cardFwVersion, auditTable=auditTable, cadEquipmentMibConformance=cadEquipmentMibConformance, portPrStateChange=portPrStateChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portTable=portTable, cardCPLDVersion=cardCPLDVersion, portMacAddress=portMacAddress, diskVolumeCardId=diskVolumeCardId, cardDetected=cardDetected, cardFwUpdateStatus=cardFwUpdateStatus, portMode=portMode, diskVolumeSize=diskVolumeSize, cardSecState=cardSecState, cardNumPorts=cardNumPorts, cardCpuType=cardCpuType, cardShelfTestIndex=cardShelfTestIndex, cardTestSchedule=cardTestSchedule, auditCommand=auditCommand, cmResetUpTime=cmResetUpTime, cardLastBootVersion=cardLastBootVersion, errEvRowStatus=errEvRowStatus, cardName=cardName, consoleOutput=consoleOutput, cardTestCommand=cardTestCommand, cardTestScheduleCommand=cardTestScheduleCommand, cardLastChangeTime=cardLastChangeTime, auditLogOutput=auditLogOutput, equipmentCardTestGroup=equipmentCardTestGroup, equipmentAudit=equipmentAudit, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditId=auditId, portCardSubType=portCardSubType, TestType=TestType, cardTable=cardTable, equipmentDiagGroup=equipmentDiagGroup, portBgpId=portBgpId, equipmentState=equipmentState, errEventId=errEventId, cardTemperatureHighWarn=cardTemperatureHighWarn, shelfNumber=shelfNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentAuditGroup=equipmentAuditGroup, cardTestIndex=cardTestIndex, cardTestEntry=cardTestEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardPicHwVersion=cardPicHwVersion, cardAction=cardAction, portShelfId=portShelfId, TestSchedule=TestSchedule, trapCounter=trapCounter, cardTemperatureHighError=cardTemperatureHighError, cardDplxStatusChange=cardDplxStatusChange, equipmentTraps=equipmentTraps, downstreamPowerLoss=downstreamPowerLoss, cmDevice=cmDevice, diskVolumeFileName=diskVolumeFileName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMib=cadEquipmentMib, cardTestResultNotification=cardTestResultNotification, auditEntry=auditEntry, cmVendor=cmVendor, cardTestResult=cardTestResult, cardAdminState=cardAdminState, portAction=portAction, cardSecStateChange=cardSecStateChange, auditCycleCount=auditCycleCount, cardTrapInh=cardTrapInh, auditDescription=auditDescription, portSecState=portSecState, errEvLoggingEnabled=errEvLoggingEnabled, auditFailedCount=auditFailedCount, portMacIfIndex=portMacIfIndex, diagTestId=diagTestId, cardCarrierHwVersion=cardCarrierHwVersion, auditCardId=auditCardId, cardPicDetected=cardPicDetected, cardTestTable=cardTestTable, cardTestId=cardTestId, portLastChangeTime=portLastChangeTime, auditAutoScheduling=auditAutoScheduling, cardBootVersion=cardBootVersion)
