...
SecondaryState, CardId, DiskVolumeUsageLevel, PortType, CardSubType, FlowControlMode, PortDetectedMode, DuplexStatus, ShelfId, UnknownState, PicType, FirmwareSource, PortMode, AdminState, CardType, PortId, PrimaryState, EqActionType = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "CardId", "DiskVolumeUsageLevel", "PortType", "CardSubType", "FlowControlMode", "PortDetectedMode", "DuplexStatus", "ShelfId", "UnknownState", "PicType", "FirmwareSource", "PortMode", "AdminState", "CardType", "PortId", "PrimaryState", "EqActionType")
...
TruthValue, DateAndTime, MacAddress, TimeStamp, RowStatus, TextualConvention, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "MacAddress", "TimeStamp", "RowStatus", "TextualConvention", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFwVersion=cardFwVersion, portMaxDsPower=portMaxDsPower, cardName=cardName, cardSecStateChange=cardSecStateChange, equipmentAuditGroup=equipmentAuditGroup, portMacAddress=portMacAddress, cmDevice=cmDevice, auditStatus=auditStatus, PYSNMP_MODULE_ID=cadEquipmentMib, portSecState=portSecState, diskVolumeTrapInh=diskVolumeTrapInh, cardTestName=cardTestName, systemKey=systemKey, cardPicHwVersion=cardPicHwVersion, auditTotalPassedCount=auditTotalPassedCount, portMode=portMode, cardTable=cardTable, equipmentTraps=equipmentTraps, errEvRowStatus=errEvRowStatus, diskVolumeEntry=diskVolumeEntry, portId=portId, diskVolumeDiskSize=diskVolumeDiskSize, auditAutoScheduling=auditAutoScheduling, portCurrDsPower=portCurrDsPower, cardTestType=cardTestType, cmIpAddress=cmIpAddress, diskDriveNumber=diskDriveNumber, auditName=auditName, cadEquipmentMib=cadEquipmentMib, diskVolumeGroup=diskVolumeGroup, TestId=TestId, cardBootVersion=cardBootVersion, auditCardId=auditCardId, cardCarrierSerialNum=cardCarrierSerialNum, portSecStateChange=portSecStateChange, cardSubType=cardSubType, diskVolumeName=diskVolumeName, diskVolumeShelfId=diskVolumeShelfId, portDocsIfIndex=portDocsIfIndex, cardSpareGroupMode=cardSpareGroupMode, diskVolumeCardId=diskVolumeCardId, cardFwUpdateStatus=cardFwUpdateStatus, portNumber=portNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentState=equipmentState, cmUChannelID=cmUChannelID, portType=portType, cmResetUpTime=cmResetUpTime, consoleOutput=consoleOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, cmRegistrationNotification=cmRegistrationNotification, cardPrState=cardPrState, cardTempNoReportNotification=cardTempNoReportNotification, portRxFlowControlDetected=portRxFlowControlDetected, fpgaErrorEventEntry=fpgaErrorEventEntry, portLastChangeTime=portLastChangeTime, cardTestResult=cardTestResult, cardUpTime=cardUpTime, systemClock=systemClock, equipmentAudit=equipmentAudit, TestType=TestType, portNumChans=portNumChans, cardCPLDVersion=cardCPLDVersion, cardTestDescription=cardTestDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditTime=auditTime, auditTotalFailedCount=auditTotalFailedCount, cardSubDetected=cardSubDetected, cardTestCommand=cardTestCommand, cardTestEntry=cardTestEntry, equipmentPortGroup=equipmentPortGroup, auditFailedCount=auditFailedCount, equipmentCardGroup=equipmentCardGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditShelfId=auditShelfId, cardType=cardType, cmPrimarySID=cmPrimarySID, portConnectorId=portConnectorId, cardRamSize=cardRamSize, cardPicDetected=cardPicDetected, diskVolumeSize=diskVolumeSize, cardId=cardId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResultDesc=cardTestResultDesc, cardSwVersion=cardSwVersion, errEventId=errEventId, auditResult=auditResult, portTxFlowControlMode=portTxFlowControlMode, portDplxStatusChange=portDplxStatusChange, cardDetectedChange=cardDetectedChange, cardTestScheduleCommand=cardTestScheduleCommand, shelfNumber=shelfNumber, cmResetNotification=cmResetNotification, TestResult=TestResult, portDplxStatus=portDplxStatus, portAction=portAction, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, shelfName=shelfName, equipmentError=equipmentError, cardCarrierFwVersion=cardCarrierFwVersion, cardTestId=cardTestId, TestCommand=TestCommand, cardCpuType=cardCpuType, equipmentTbl=equipmentTbl, TestScheduleCommand=TestScheduleCommand, portCardSubType=portCardSubType, cmResetClearNotification=cmResetClearNotification, portTable=portTable, portPrState=portPrState, auditLogThrottle=auditLogThrottle, auditTable=auditTable, diskVolumeId=diskVolumeId, cardPrStateChange=cardPrStateChange, equipmentDiag=equipmentDiag, cardAdminState=cardAdminState, cardLastChangeTime=cardLastChangeTime, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSerialNum=cardSerialNum, systemGeneral=systemGeneral, cardLastBootSource=cardLastBootSource, equipmentStateGroup=equipmentStateGroup, shelfObjects=shelfObjects, errEvLoggingEnabled=errEvLoggingEnabled, cardEntry=cardEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTempOverHeatNotification=cardTempOverHeatNotification, cardAction=cardAction, equipmentShelfGroup=equipmentShelfGroup, portBgpId=portBgpId, cardTestSchedule=cardTestSchedule, cardCarrierHwVersion=cardCarrierHwVersion, portRxFlowControlMode=portRxFlowControlMode, auditEntry=auditEntry, cardNumber=cardNumber, diskVolumeTable=diskVolumeTable, portPrStateChange=portPrStateChange, trapCounter=trapCounter, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTrapInh=cardTrapInh, systemGeneralGroup=systemGeneralGroup, cmResetStatus=cmResetStatus, auditDescription=auditDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, auditCommand=auditCommand, TestSchedule=TestSchedule, portDescription=portDescription, diskVolumeFileName=diskVolumeFileName, cardDetected=cardDetected, downstreamPowerLoss=downstreamPowerLoss, portDetectedMode=portDetectedMode, cardHwVersion=cardHwVersion, cardFpgaSource=cardFpgaSource, auditId=auditId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEvLogLevel=errEvLogLevel, cardDplxStatus=cardDplxStatus, portAdminState=portAdminState, fpgaErrorEventTable=fpgaErrorEventTable, cardCpuSpeed=cardCpuSpeed, portTrapInh=portTrapInh, auditLogOutput=auditLogOutput, cardSpareGroupId=cardSpareGroupId, cardBusSpeed=cardBusSpeed, diskVolumeNumber=diskVolumeNumber, portCardId=portCardId, portTxFlowControlDetected=portTxFlowControlDetected, cardTestTable=cardTestTable, auditCycleCount=auditCycleCount, portShelfId=portShelfId, cardShelfId=cardShelfId, verboseLevel=verboseLevel, cardDplxStatusChange=cardDplxStatusChange, shelfSwVersion=shelfSwVersion, cmResetReason=cmResetReason, trapSeverity=trapSeverity, cmMacAddress=cmMacAddress, portEntry=portEntry, equipmentCardTestGroup=equipmentCardTestGroup, cardLastResetReason=cardLastResetReason, cardTestTransId=cardTestTransId, cardTemperatureHighError=cardTemperatureHighError, cardFlashSize=cardFlashSize, remainInDiagMode=remainInDiagMode, cmVendor=cmVendor, cmResetInfo=cmResetInfo, cardTestIndex=cardTestIndex, equipmentDiagGroup=equipmentDiagGroup, cardTestResultNotification=cardTestResultNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeDriveId=diskVolumeDriveId, errEvRecoveryEnabled=errEvRecoveryEnabled, TestTransactionId=TestTransactionId, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestTime=cardTestTime, portMinDsPower=portMinDsPower, cardSecState=cardSecState, cardShelfTestIndex=cardShelfTestIndex, cardNumPorts=cardNumPorts, auditPassedCount=auditPassedCount, eqDiagConfig=eqDiagConfig, diagTestId=diagTestId, portMacIfIndex=portMacIfIndex, cardTemperature=cardTemperature, diskVolumeUsageLevel=diskVolumeUsageLevel, cardPicSerialNum=cardPicSerialNum)
