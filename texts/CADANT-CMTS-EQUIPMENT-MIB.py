...
ShelfId, FlowControlMode, CardType, PortMode, UnknownState, PicType, PortType, EqActionType, SecondaryState, DiskVolumeUsageLevel, FirmwareSource, PortDetectedMode, CardId, PrimaryState, CardSubType, DuplexStatus, PortId, AdminState = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "FlowControlMode", "CardType", "PortMode", "UnknownState", "PicType", "PortType", "EqActionType", "SecondaryState", "DiskVolumeUsageLevel", "FirmwareSource", "PortDetectedMode", "CardId", "PrimaryState", "CardSubType", "DuplexStatus", "PortId", "AdminState")
...
RowStatus, TruthValue, MacAddress, DateAndTime, DisplayString, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TruthValue", "MacAddress", "DateAndTime", "DisplayString", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portNumber=portNumber, cardShelfId=cardShelfId, cardTestTime=cardTestTime, cardTestDescription=cardTestDescription, cardAdminState=cardAdminState, cadEquipmentMib=cadEquipmentMib, portTxFlowControlMode=portTxFlowControlMode, cardDplxStatusChange=cardDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwUpdateStatus=cardFwUpdateStatus, equipmentTraps=equipmentTraps, cardTestResultDesc=cardTestResultDesc, cardShelfTestIndex=cardShelfTestIndex, portAction=portAction, PYSNMP_MODULE_ID=cadEquipmentMib, TestResult=TestResult, diskVolumeTable=diskVolumeTable, diskVolumeGroup=diskVolumeGroup, auditName=auditName, portCardSubType=portCardSubType, cmResetReason=cmResetReason, auditStatus=auditStatus, cardSecStateChange=cardSecStateChange, cardHwVersion=cardHwVersion, eqDiagConfig=eqDiagConfig, downstreamPowerLoss=downstreamPowerLoss, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskDriveNumber=diskDriveNumber, portRxFlowControlMode=portRxFlowControlMode, errEvLogLevel=errEvLogLevel, cardPicHwVersion=cardPicHwVersion, portDplxStatusChange=portDplxStatusChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeCardId=diskVolumeCardId, TestId=TestId, portAdminState=portAdminState, remainInDiagMode=remainInDiagMode, cardSecState=cardSecState, systemKey=systemKey, diskVolumeFileName=diskVolumeFileName, auditTotalPassedCount=auditTotalPassedCount, cardCarrierHwVersion=cardCarrierHwVersion, equipmentError=equipmentError, cardTestIndex=cardTestIndex, portLastChangeTime=portLastChangeTime, cardLastChangeTime=cardLastChangeTime, equipmentShelfGroup=equipmentShelfGroup, cardCpuType=cardCpuType, portCardId=portCardId, cardTestName=cardTestName, auditResult=auditResult, cmVendor=cmVendor, diskVolumeNumber=diskVolumeNumber, cardCpuSpeed=cardCpuSpeed, portShelfId=portShelfId, portBgpId=portBgpId, trapCounter=trapCounter, cardTestSchedule=cardTestSchedule, auditId=auditId, cardTempOverHeatNotification=cardTempOverHeatNotification, verboseLevel=verboseLevel, fpgaErrorEventTable=fpgaErrorEventTable, cardFwVersion=cardFwVersion, equipmentTbl=equipmentTbl, cardTrapInh=cardTrapInh, cardPicSerialNum=cardPicSerialNum, portCurrDsPower=portCurrDsPower, diskVolumeSize=diskVolumeSize, cardTestTable=cardTestTable, portDetectedMode=portDetectedMode, cardTestResultNotification=cardTestResultNotification, portTrapInh=portTrapInh, portMaxDsPower=portMaxDsPower, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSwVersion=cardSwVersion, diagTestId=diagTestId, portSecStateChange=portSecStateChange, portConnectorId=portConnectorId, cardNumPorts=cardNumPorts, cardFpgaSource=cardFpgaSource, auditFailedCount=auditFailedCount, cardFlashSize=cardFlashSize, TestCommand=TestCommand, errEvRowStatus=errEvRowStatus, equipmentState=equipmentState, TestScheduleCommand=TestScheduleCommand, cardTemperature=cardTemperature, portSecState=portSecState, equipmentAudit=equipmentAudit, equipmentDiag=equipmentDiag, equipmentStateGroup=equipmentStateGroup, auditTotalFailedCount=auditTotalFailedCount, shelfObjects=shelfObjects, cardAction=cardAction, portPrStateChange=portPrStateChange, cardBootVersion=cardBootVersion, diskVolumeEntry=diskVolumeEntry, equipmentPortGroup=equipmentPortGroup, diskVolumeDriveId=diskVolumeDriveId, auditCommand=auditCommand, equipmentAuditGroup=equipmentAuditGroup, shelfNumber=shelfNumber, portDocsIfIndex=portDocsIfIndex, cardType=cardType, auditDescription=auditDescription, cardTemperatureHighError=cardTemperatureHighError, cardSubDetected=cardSubDetected, portMacAddress=portMacAddress, auditCardId=auditCardId, portMode=portMode, cardTestCommand=cardTestCommand, cardLastResetReason=cardLastResetReason, cardEntry=cardEntry, portTxFlowControlDetected=portTxFlowControlDetected, auditTable=auditTable, portDescription=portDescription, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardLastBootSource=cardLastBootSource, portType=portType, cardPrStateChange=cardPrStateChange, TestTransactionId=TestTransactionId, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeTrapInh=diskVolumeTrapInh, cardSubType=cardSubType, systemGeneral=systemGeneral, cardBusSpeed=cardBusSpeed, diskVolumeUsageLevel=diskVolumeUsageLevel, auditAutoScheduling=auditAutoScheduling, auditEntry=auditEntry, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSpareGroupMode=cardSpareGroupMode, cardTestId=cardTestId, cmResetInfo=cmResetInfo, fpgaErrorEventGroup=fpgaErrorEventGroup, cardNumber=cardNumber, cardTempNoReportNotification=cardTempNoReportNotification, cmPrimarySID=cmPrimarySID, cmResetNotification=cmResetNotification, portPrState=portPrState, TestSchedule=TestSchedule, cmIpAddress=cmIpAddress, diskVolumeShelfId=diskVolumeShelfId, cardTestType=cardTestType, trapSeverity=trapSeverity, portTable=portTable, cardTable=cardTable, cadEquipmentMibGroup=cadEquipmentMibGroup, cardRamSize=cardRamSize, cardPicDetected=cardPicDetected, cardCarrierSerialNum=cardCarrierSerialNum, cardId=cardId, cardTestScheduleCommand=cardTestScheduleCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmDevice=cmDevice, portDplxStatus=portDplxStatus, errEvLoggingEnabled=errEvLoggingEnabled, equipmentCardGroup=equipmentCardGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, TestType=TestType, systemGeneralGroup=systemGeneralGroup, portMacIfIndex=portMacIfIndex, errEventId=errEventId, consoleOutput=consoleOutput, cmRegistrationNotification=cmRegistrationNotification, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeName=diskVolumeName, systemClock=systemClock, diskVolumeId=diskVolumeId, auditShelfId=auditShelfId, portNumChans=portNumChans, auditTime=auditTime, cardDetectedChange=cardDetectedChange, auditLogThrottle=auditLogThrottle, cardPrState=cardPrState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmUChannelID=cmUChannelID, portMinDsPower=portMinDsPower, cardTestTransId=cardTestTransId, portId=portId, cardTestResult=cardTestResult, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetStatus=cmResetStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, cardUpTime=cardUpTime, portEntry=portEntry, shelfName=shelfName, shelfSwVersion=shelfSwVersion, cardSerialNum=cardSerialNum, cardTestEntry=cardTestEntry, auditLogOutput=auditLogOutput, cardCPLDVersion=cardCPLDVersion, cardCarrierFwVersion=cardCarrierFwVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, cardDetected=cardDetected, cardName=cardName, cardDplxStatus=cardDplxStatus, diskVolumeDiskSize=diskVolumeDiskSize, cmResetUpTime=cmResetUpTime, auditPassedCount=auditPassedCount, cmMacAddress=cmMacAddress, cardLastBootVersion=cardLastBootVersion, auditCycleCount=auditCycleCount, cmResetClearNotification=cmResetClearNotification, cardSpareGroupId=cardSpareGroupId)
