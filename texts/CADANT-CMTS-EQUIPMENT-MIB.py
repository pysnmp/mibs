...
DuplexStatus, CardSubType, PicType, UnknownState, FirmwareSource, PortType, PortId, DiskVolumeUsageLevel, PrimaryState, PortMode, CardType, CardId, PortDetectedMode, FlowControlMode, SecondaryState, ShelfId, AdminState, EqActionType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "CardSubType", "PicType", "UnknownState", "FirmwareSource", "PortType", "PortId", "DiskVolumeUsageLevel", "PrimaryState", "PortMode", "CardType", "CardId", "PortDetectedMode", "FlowControlMode", "SecondaryState", "ShelfId", "AdminState", "EqActionType")
...
RowStatus, TextualConvention, DateAndTime, MacAddress, TimeStamp, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "DateAndTime", "MacAddress", "TimeStamp", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeDriveId=diskVolumeDriveId, fpgaErrorEventEntry=fpgaErrorEventEntry, cardBusSpeed=cardBusSpeed, cardCpuSpeed=cardCpuSpeed, equipmentPortGroup=equipmentPortGroup, diskVolumeId=diskVolumeId, cardDetectedChange=cardDetectedChange, portId=portId, equipmentAudit=equipmentAudit, portAdminState=portAdminState, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetUpTime=cmResetUpTime, remainInDiagMode=remainInDiagMode, diskVolumeFileName=diskVolumeFileName, portDescription=portDescription, equipmentTraps=equipmentTraps, TestTransactionId=TestTransactionId, cardType=cardType, shelfObjects=shelfObjects, eqDiagConfig=eqDiagConfig, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCPLDVersion=cardCPLDVersion, cardUpTime=cardUpTime, cardPicDetected=cardPicDetected, cardHwVersion=cardHwVersion, cardSwVersion=cardSwVersion, TestCommand=TestCommand, cardCpuType=cardCpuType, cadEquipmentMib=cadEquipmentMib, cardCarrierFwVersion=cardCarrierFwVersion, auditDescription=auditDescription, auditCommand=auditCommand, cmResetReason=cmResetReason, portCardId=portCardId, errEvLoggingEnabled=errEvLoggingEnabled, TestScheduleCommand=TestScheduleCommand, portMacAddress=portMacAddress, auditTime=auditTime, equipmentTbl=equipmentTbl, cardTestType=cardTestType, cmDevice=cmDevice, diskVolumeTrapInh=diskVolumeTrapInh, trapSeverity=trapSeverity, diskVolumeUsageLevel=diskVolumeUsageLevel, errEventId=errEventId, portCurrDsPower=portCurrDsPower, cardFwVersion=cardFwVersion, cardPicHwVersion=cardPicHwVersion, portMode=portMode, cardEntry=cardEntry, cardSecStateChange=cardSecStateChange, equipmentShelfGroup=equipmentShelfGroup, cardTestTime=cardTestTime, portRxFlowControlDetected=portRxFlowControlDetected, auditAutoScheduling=auditAutoScheduling, cardLastBootVersion=cardLastBootVersion, systemGeneral=systemGeneral, diskVolumeGroup=diskVolumeGroup, cmUChannelID=cmUChannelID, cardTestIndex=cardTestIndex, cardTemperature=cardTemperature, cmMacAddress=cmMacAddress, portDetectedMode=portDetectedMode, cardTempNoReportNotification=cardTempNoReportNotification, cardTestEntry=cardTestEntry, auditLogThrottle=auditLogThrottle, portType=portType, cardDetected=cardDetected, diagTestId=diagTestId, TestResult=TestResult, cardTestDescription=cardTestDescription, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardName=cardName, cmVendor=cmVendor, cardLastBootSource=cardLastBootSource, shelfSwVersion=shelfSwVersion, auditFailedCount=auditFailedCount, downstreamPowerLoss=downstreamPowerLoss, auditCardId=auditCardId, auditCycleCount=auditCycleCount, portSecStateChange=portSecStateChange, diskVolumeDiskSize=diskVolumeDiskSize, cardNumPorts=cardNumPorts, portShelfId=portShelfId, cardTestName=cardTestName, cardSpareGroupId=cardSpareGroupId, auditLogOutput=auditLogOutput, portEntry=portEntry, auditName=auditName, auditShelfId=auditShelfId, cardSubDetected=cardSubDetected, cmIpAddress=cmIpAddress, cardRamSize=cardRamSize, TestType=TestType, cardBootVersion=cardBootVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardId=cardId, cardAdminState=cardAdminState, diskVolumeName=diskVolumeName, portPrState=portPrState, cardSerialNum=cardSerialNum, portDocsIfIndex=portDocsIfIndex, portCardSubType=portCardSubType, portDplxStatusChange=portDplxStatusChange, systemGeneralGroup=systemGeneralGroup, portLastChangeTime=portLastChangeTime, cardNumber=cardNumber, cardShelfId=cardShelfId, cardTemperatureHighError=cardTemperatureHighError, portSecState=portSecState, portRxFlowControlMode=portRxFlowControlMode, cardAction=cardAction, diskVolumeEntry=diskVolumeEntry, equipmentDiagGroup=equipmentDiagGroup, cardTestTable=cardTestTable, auditResult=auditResult, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentError=equipmentError, portMaxDsPower=portMaxDsPower, equipmentCardGroup=equipmentCardGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSecState=cardSecState, auditTable=auditTable, systemKey=systemKey, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResult=cardTestResult, cardTrapInh=cardTrapInh, cardSubType=cardSubType, cardTestScheduleCommand=cardTestScheduleCommand, auditPassedCount=auditPassedCount, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFwUpdateStatus=cardFwUpdateStatus, cmResetClearNotification=cmResetClearNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTempOverHeatNotification=cardTempOverHeatNotification, cardDplxStatusChange=cardDplxStatusChange, TestId=TestId, trapCounter=trapCounter, equipmentStateGroup=equipmentStateGroup, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portTrapInh=portTrapInh, auditStatus=auditStatus, diskDriveNumber=diskDriveNumber, cardLastChangeTime=cardLastChangeTime, errEvLogLevel=errEvLogLevel, diskVolumeTable=diskVolumeTable, shelfName=shelfName, diskVolumeNumber=diskVolumeNumber, diskVolumeShelfId=diskVolumeShelfId, cmResetInfo=cmResetInfo, equipmentState=equipmentState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDplxStatus=portDplxStatus, verboseLevel=verboseLevel, auditTotalFailedCount=auditTotalFailedCount, cardPicSerialNum=cardPicSerialNum, errEvRowStatus=errEvRowStatus, portNumChans=portNumChans, portBgpId=portBgpId, auditId=auditId, portAction=portAction, cardCarrierHwVersion=cardCarrierHwVersion, cardLastResetReason=cardLastResetReason, diskVolumeSize=diskVolumeSize, cardTestResultDesc=cardTestResultDesc, auditTotalPassedCount=auditTotalPassedCount, cardTestSchedule=cardTestSchedule, cmPrimarySID=cmPrimarySID, cmRegistrationNotification=cmRegistrationNotification, cardFlashSize=cardFlashSize, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portMinDsPower=portMinDsPower, cardTestTransId=cardTestTransId, cardPrState=cardPrState, portNumber=portNumber, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestCommand=cardTestCommand, TestSchedule=TestSchedule, equipmentDiag=equipmentDiag, systemClock=systemClock, consoleOutput=consoleOutput, cmResetStatus=cmResetStatus, portMacIfIndex=portMacIfIndex, cardDplxStatus=cardDplxStatus, portConnectorId=portConnectorId, equipmentCardTestGroup=equipmentCardTestGroup, cardTable=cardTable, equipmentAuditGroup=equipmentAuditGroup, auditEntry=auditEntry, cmResetNotification=cmResetNotification, cardSpareGroupMode=cardSpareGroupMode, cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeCardId=diskVolumeCardId, cardTestId=cardTestId, cardPrStateChange=cardPrStateChange, shelfNumber=shelfNumber, cardFpgaSource=cardFpgaSource, portTxFlowControlDetected=portTxFlowControlDetected, portTable=portTable, portPrStateChange=portPrStateChange)
