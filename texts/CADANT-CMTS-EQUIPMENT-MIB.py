...
ShelfId, EqActionType, PortType, PortDetectedMode, SecondaryState, CardId, PicType, FirmwareSource, PrimaryState, UnknownState, CardType, CardSubType, AdminState, FlowControlMode, PortMode, DuplexStatus, DiskVolumeUsageLevel, PortId = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "EqActionType", "PortType", "PortDetectedMode", "SecondaryState", "CardId", "PicType", "FirmwareSource", "PrimaryState", "UnknownState", "CardType", "CardSubType", "AdminState", "FlowControlMode", "PortMode", "DuplexStatus", "DiskVolumeUsageLevel", "PortId")
...
MacAddress, TruthValue, RowStatus, TimeStamp, DisplayString, TextualConvention, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "RowStatus", "TimeStamp", "DisplayString", "TextualConvention", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditResult=auditResult, cardAdminState=cardAdminState, portDplxStatus=portDplxStatus, cardTemperature=cardTemperature, cardCPLDVersion=cardCPLDVersion, cmResetStatus=cmResetStatus, cmIpAddress=cmIpAddress, cardName=cardName, systemKey=systemKey, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestCommand=cardTestCommand, cardBusSpeed=cardBusSpeed, cadEquipmentMib=cadEquipmentMib, auditAutoScheduling=auditAutoScheduling, equipmentState=equipmentState, cardTestDescription=cardTestDescription, auditShelfId=auditShelfId, diskVolumeUsageNotification=diskVolumeUsageNotification, auditStatus=auditStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmDevice=cmDevice, cardTestName=cardTestName, diskVolumeSize=diskVolumeSize, TestResult=TestResult, cardEntry=cardEntry, cardSwVersion=cardSwVersion, consoleOutput=consoleOutput, cardFwVersion=cardFwVersion, cardUpTime=cardUpTime, cardBootVersion=cardBootVersion, cardPicSerialNum=cardPicSerialNum, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditPassedCount=auditPassedCount, portMacIfIndex=portMacIfIndex, auditCommand=auditCommand, cardTestSchedule=cardTestSchedule, cmResetUpTime=cmResetUpTime, auditLogOutput=auditLogOutput, TestCommand=TestCommand, portTrapInh=portTrapInh, shelfName=shelfName, diskVolumeDiskSize=diskVolumeDiskSize, errEvRecoveryEnabled=errEvRecoveryEnabled, diskDriveNumber=diskDriveNumber, cardPrState=cardPrState, equipmentTraps=equipmentTraps, cadEquipmentMibGroup=cadEquipmentMibGroup, fpgaErrorEventTable=fpgaErrorEventTable, cardFpgaSource=cardFpgaSource, cardTestEntry=cardTestEntry, portLastChangeTime=portLastChangeTime, errEvRowStatus=errEvRowStatus, cardPicHwVersion=cardPicHwVersion, diskVolumeName=diskVolumeName, cardLastBootVersion=cardLastBootVersion, cmVendor=cmVendor, auditTotalPassedCount=auditTotalPassedCount, portTxFlowControlMode=portTxFlowControlMode, cardDetectedChange=cardDetectedChange, cardCarrierHwVersion=cardCarrierHwVersion, cmUChannelID=cmUChannelID, TestId=TestId, portEntry=portEntry, equipmentAudit=equipmentAudit, cmRegistrationNotification=cmRegistrationNotification, portMode=portMode, cardTable=cardTable, cardTestTime=cardTestTime, systemGeneralGroup=systemGeneralGroup, equipmentShelfGroup=equipmentShelfGroup, portPrState=portPrState, trapSeverity=trapSeverity, cardShelfTestIndex=cardShelfTestIndex, cardPrStateChange=cardPrStateChange, cardId=cardId, auditFailedCount=auditFailedCount, remainInDiagMode=remainInDiagMode, cardAction=cardAction, diskVolumeGroup=diskVolumeGroup, equipmentPortGroup=equipmentPortGroup, diagTestId=diagTestId, cardTestId=cardTestId, errEventId=errEventId, errEvLoggingEnabled=errEvLoggingEnabled, equipmentDiagGroup=equipmentDiagGroup, cardSubType=cardSubType, cardTemperatureHighError=cardTemperatureHighError, cardSpareGroupMode=cardSpareGroupMode, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditTime=auditTime, auditTotalFailedCount=auditTotalFailedCount, diskVolumeCardId=diskVolumeCardId, diskVolumeId=diskVolumeId, portCardId=portCardId, cardDetected=cardDetected, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDocsIfIndex=portDocsIfIndex, cardNumber=cardNumber, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardLastBootSource=cardLastBootSource, auditEntry=auditEntry, cmResetInfo=cmResetInfo, cardTempNoReportNotification=cardTempNoReportNotification, equipmentTbl=equipmentTbl, cardDplxStatusChange=cardDplxStatusChange, portRxFlowControlMode=portRxFlowControlMode, cardDplxStatus=cardDplxStatus, TestSchedule=TestSchedule, diskVolumeDriveId=diskVolumeDriveId, cardTestTable=cardTestTable, auditCardId=auditCardId, cardCarrierSerialNum=cardCarrierSerialNum, TestScheduleCommand=TestScheduleCommand, cmMacAddress=cmMacAddress, systemGeneral=systemGeneral, cardType=cardType, cmResetReason=cmResetReason, portAdminState=portAdminState, portDplxStatusChange=portDplxStatusChange, equipmentStateGroup=equipmentStateGroup, cardTestResultDesc=cardTestResultDesc, auditLogThrottle=auditLogThrottle, cardTrapInh=cardTrapInh, cardLastResetReason=cardLastResetReason, portNumChans=portNumChans, diskVolumeEntry=diskVolumeEntry, portSecState=portSecState, portAction=portAction, diskVolumeFileName=diskVolumeFileName, cardCpuSpeed=cardCpuSpeed, cardSecState=cardSecState, cardTestTransId=cardTestTransId, cardCpuType=cardCpuType, equipmentDiag=equipmentDiag, auditCycleCount=auditCycleCount, cardSubDetected=cardSubDetected, portMinDsPower=portMinDsPower, cardFlashSize=cardFlashSize, portDetectedMode=portDetectedMode, diskVolumeShelfId=diskVolumeShelfId, downstreamPowerLoss=downstreamPowerLoss, cardSerialNum=cardSerialNum, cardTestScheduleCommand=cardTestScheduleCommand, portRxFlowControlDetected=portRxFlowControlDetected, auditDescription=auditDescription, cmResetClearNotification=cmResetClearNotification, cardTestType=cardTestType, auditName=auditName, diskVolumeTable=diskVolumeTable, portConnectorId=portConnectorId, portCardSubType=portCardSubType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeTrapInh=diskVolumeTrapInh, equipmentCardTestGroup=equipmentCardTestGroup, cardHwVersion=cardHwVersion, diskVolumeNumber=diskVolumeNumber, TestType=TestType, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResult=cardTestResult, portPrStateChange=portPrStateChange, cardFwUpdateStatus=cardFwUpdateStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, PYSNMP_MODULE_ID=cadEquipmentMib, shelfObjects=shelfObjects, cardSecStateChange=cardSecStateChange, TestTransactionId=TestTransactionId, portNumber=portNumber, shelfNumber=shelfNumber, shelfSwVersion=shelfSwVersion, cardShelfId=cardShelfId, equipmentCardGroup=equipmentCardGroup, cardTestIndex=cardTestIndex, portId=portId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentError=equipmentError, cadEquipmentMibConformance=cadEquipmentMibConformance, portMacAddress=portMacAddress, auditTable=auditTable, auditId=auditId, errEvLogLevel=errEvLogLevel, portBgpId=portBgpId, cmResetNotification=cmResetNotification, portCurrDsPower=portCurrDsPower, systemClock=systemClock, portType=portType, cardNumPorts=cardNumPorts, portDescription=portDescription, portShelfId=portShelfId, trapCounter=trapCounter, verboseLevel=verboseLevel, eqDiagConfig=eqDiagConfig, portTable=portTable, cardPicDetected=cardPicDetected, portSecStateChange=portSecStateChange, cardSpareGroupId=cardSpareGroupId, portMaxDsPower=portMaxDsPower, cardRamSize=cardRamSize, cardLastChangeTime=cardLastChangeTime, equipmentAuditGroup=equipmentAuditGroup, fpgaErrorEventGroup=fpgaErrorEventGroup)
