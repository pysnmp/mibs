...
EqActionType, FirmwareSource, PortType, CardId, PicType, PortMode, CardType, UnknownState, PrimaryState, DiskVolumeUsageLevel, SecondaryState, CardSubType, PortId, PortDetectedMode, FlowControlMode, AdminState, ShelfId, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "FirmwareSource", "PortType", "CardId", "PicType", "PortMode", "CardType", "UnknownState", "PrimaryState", "DiskVolumeUsageLevel", "SecondaryState", "CardSubType", "PortId", "PortDetectedMode", "FlowControlMode", "AdminState", "ShelfId", "DuplexStatus")
...
MacAddress, RowStatus, TextualConvention, DateAndTime, DisplayString, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "TextualConvention", "DateAndTime", "DisplayString", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFlashSize=cardFlashSize, cardFwVersion=cardFwVersion, cardHwVersion=cardHwVersion, systemClock=systemClock, portDplxStatus=portDplxStatus, cardLastBootVersion=cardLastBootVersion, portMacAddress=portMacAddress, portCurrDsPower=portCurrDsPower, equipmentCardGroup=equipmentCardGroup, equipmentError=equipmentError, cardPicDetected=cardPicDetected, fpgaErrorEventEntry=fpgaErrorEventEntry, auditTotalPassedCount=auditTotalPassedCount, cardTestTime=cardTestTime, cardTestTransId=cardTestTransId, cardAction=cardAction, verboseLevel=verboseLevel, cmIpAddress=cmIpAddress, cardDetected=cardDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDescription=portDescription, cardFwUpdateStatus=cardFwUpdateStatus, cardTestCommand=cardTestCommand, equipmentTbl=equipmentTbl, cardTestResult=cardTestResult, cardTestResultNotification=cardTestResultNotification, cardLastBootSource=cardLastBootSource, TestId=TestId, systemKey=systemKey, diskVolumeUsageLevel=diskVolumeUsageLevel, auditTime=auditTime, cardSecStateChange=cardSecStateChange, portDplxStatusChange=portDplxStatusChange, cmMacAddress=cmMacAddress, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPrStateChange=cardPrStateChange, portConnectorId=portConnectorId, cardSubDetected=cardSubDetected, auditCommand=auditCommand, cardPicHwVersion=cardPicHwVersion, portSecStateChange=portSecStateChange, cardPicSerialNum=cardPicSerialNum, TestResult=TestResult, cardTestTable=cardTestTable, cardBootVersion=cardBootVersion, cmResetClearNotification=cmResetClearNotification, cardTestEntry=cardTestEntry, portLastChangeTime=portLastChangeTime, consoleOutput=consoleOutput, cardTestName=cardTestName, cardDetectedChange=cardDetectedChange, cardBusSpeed=cardBusSpeed, auditLogThrottle=auditLogThrottle, auditCycleCount=auditCycleCount, systemGeneral=systemGeneral, trapCounter=trapCounter, TestTransactionId=TestTransactionId, cardTable=cardTable, shelfNumber=shelfNumber, portShelfId=portShelfId, cmResetReason=cmResetReason, fpgaErrorEventTable=fpgaErrorEventTable, equipmentCardTestGroup=equipmentCardTestGroup, cardDplxStatus=cardDplxStatus, portSecState=portSecState, cmVendor=cmVendor, diskVolumeName=diskVolumeName, equipmentAudit=equipmentAudit, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCpuType=cardCpuType, portCardId=portCardId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardDplxStatusChange=cardDplxStatusChange, portMaxDsPower=portMaxDsPower, shelfObjects=shelfObjects, equipmentTraps=equipmentTraps, diagTestId=diagTestId, PYSNMP_MODULE_ID=cadEquipmentMib, cardShelfTestIndex=cardShelfTestIndex, auditAutoScheduling=auditAutoScheduling, TestScheduleCommand=TestScheduleCommand, diskVolumeFileName=diskVolumeFileName, cardCarrierFwVersion=cardCarrierFwVersion, cardCpuSpeed=cardCpuSpeed, cmResetInfo=cmResetInfo, portDetectedMode=portDetectedMode, portType=portType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestResultDesc=cardTestResultDesc, cadEquipmentMibConformance=cadEquipmentMibConformance, cardName=cardName, diskVolumeShelfId=diskVolumeShelfId, cardNumber=cardNumber, portAction=portAction, errEvRowStatus=errEvRowStatus, portTrapInh=portTrapInh, errEventId=errEventId, diskVolumeDriveId=diskVolumeDriveId, cardCPLDVersion=cardCPLDVersion, TestCommand=TestCommand, auditFailedCount=auditFailedCount, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestType=cardTestType, cardSecState=cardSecState, cardAdminState=cardAdminState, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentDiag=equipmentDiag, diskVolumeTrapInh=diskVolumeTrapInh, auditTable=auditTable, diskVolumeDiskSize=diskVolumeDiskSize, portRxFlowControlDetected=portRxFlowControlDetected, cardTestId=cardTestId, equipmentPortGroup=equipmentPortGroup, equipmentStateGroup=equipmentStateGroup, cardTestSchedule=cardTestSchedule, diskVolumeSize=diskVolumeSize, cardId=cardId, diskVolumeGroup=diskVolumeGroup, cardLastChangeTime=cardLastChangeTime, TestSchedule=TestSchedule, cardTestIndex=cardTestIndex, downstreamPowerLoss=downstreamPowerLoss, fpgaErrorEventGroup=fpgaErrorEventGroup, cmDevice=cmDevice, portPrState=portPrState, cardSerialNum=cardSerialNum, diskVolumeNumber=diskVolumeNumber, cadEquipmentMib=cadEquipmentMib, portNumChans=portNumChans, cmUChannelID=cmUChannelID, cmRegistrationNotification=cmRegistrationNotification, cardFpgaSource=cardFpgaSource, shelfName=shelfName, equipmentState=equipmentState, portAdminState=portAdminState, cardUpTime=cardUpTime, errEvRecoveryEnabled=errEvRecoveryEnabled, portPrStateChange=portPrStateChange, cardNumPorts=cardNumPorts, portNumber=portNumber, portBgpId=portBgpId, cmResetNotification=cmResetNotification, portEntry=portEntry, equipmentAuditGroup=equipmentAuditGroup, auditTotalFailedCount=auditTotalFailedCount, auditStatus=auditStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditId=auditId, auditName=auditName, portCardSubType=portCardSubType, portTxFlowControlMode=portTxFlowControlMode, equipmentShelfGroup=equipmentShelfGroup, auditPassedCount=auditPassedCount, cardPrState=cardPrState, diskVolumeEntry=diskVolumeEntry, auditLogOutput=auditLogOutput, cardCarrierSerialNum=cardCarrierSerialNum, portId=portId, cardRamSize=cardRamSize, cardSpareGroupMode=cardSpareGroupMode, auditEntry=auditEntry, diskDriveNumber=diskDriveNumber, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeCardId=diskVolumeCardId, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeTable=diskVolumeTable, cardSwVersion=cardSwVersion, cardLastResetReason=cardLastResetReason, errEvLoggingEnabled=errEvLoggingEnabled, cardTemperatureHighError=cardTemperatureHighError, portMode=portMode, trapSeverity=trapSeverity, shelfSwVersion=shelfSwVersion, cardCarrierHwVersion=cardCarrierHwVersion, cardEntry=cardEntry, portTxFlowControlDetected=portTxFlowControlDetected, cardShelfId=cardShelfId, diskVolumeId=diskVolumeId, equipmentDiagGroup=equipmentDiagGroup, cmResetUpTime=cmResetUpTime, cardTestDescription=cardTestDescription, cardTemperature=cardTemperature, portMacIfIndex=portMacIfIndex, eqDiagConfig=eqDiagConfig, auditCardId=auditCardId, cardSubType=cardSubType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmPrimarySID=cmPrimarySID, portMinDsPower=portMinDsPower, cardType=cardType, systemGeneralGroup=systemGeneralGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, TestType=TestType, portTable=portTable, cmResetStatus=cmResetStatus, auditShelfId=auditShelfId, errEvLogLevel=errEvLogLevel, cardTempOverHeatNotification=cardTempOverHeatNotification, portRxFlowControlMode=portRxFlowControlMode, cardTrapInh=cardTrapInh, cardSpareGroupId=cardSpareGroupId, portDocsIfIndex=portDocsIfIndex, auditDescription=auditDescription, remainInDiagMode=remainInDiagMode, auditResult=auditResult)
