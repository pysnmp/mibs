...
PortDetectedMode, CardId, PicType, EqActionType, CardSubType, PortType, DiskVolumeUsageLevel, AdminState, ShelfId, SecondaryState, PrimaryState, FirmwareSource, FlowControlMode, PortMode, UnknownState, CardType, PortId, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "CardId", "PicType", "EqActionType", "CardSubType", "PortType", "DiskVolumeUsageLevel", "AdminState", "ShelfId", "SecondaryState", "PrimaryState", "FirmwareSource", "FlowControlMode", "PortMode", "UnknownState", "CardType", "PortId", "DuplexStatus")
...
RowStatus, DateAndTime, TimeStamp, TruthValue, TextualConvention, MacAddress, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "TimeStamp", "TruthValue", "TextualConvention", "MacAddress", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTrapInh=portTrapInh, cardBusSpeed=cardBusSpeed, TestScheduleCommand=TestScheduleCommand, cardTemperature=cardTemperature, auditDescription=auditDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecStateChange=portSecStateChange, downstreamPowerLoss=downstreamPowerLoss, portLastChangeTime=portLastChangeTime, cmMacAddress=cmMacAddress, diagTestId=diagTestId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardCarrierSerialNum=cardCarrierSerialNum, cmResetInfo=cmResetInfo, cardRamSize=cardRamSize, portType=portType, portPrState=portPrState, portRxFlowControlDetected=portRxFlowControlDetected, cardDetectedChange=cardDetectedChange, portId=portId, cardShelfId=cardShelfId, errEvLogLevel=errEvLogLevel, cardCarrierFwVersion=cardCarrierFwVersion, cmResetStatus=cmResetStatus, cardSubDetected=cardSubDetected, cmDevice=cmDevice, auditFailedCount=auditFailedCount, cmRegistrationNotification=cmRegistrationNotification, cmResetUpTime=cmResetUpTime, cardCPLDVersion=cardCPLDVersion, portTxFlowControlMode=portTxFlowControlMode, cmResetReason=cmResetReason, cardTestId=cardTestId, cardTable=cardTable, equipmentStateGroup=equipmentStateGroup, remainInDiagMode=remainInDiagMode, portRxFlowControlMode=portRxFlowControlMode, cardAdminState=cardAdminState, equipmentDiag=equipmentDiag, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentAuditGroup=equipmentAuditGroup, cardPrState=cardPrState, auditLogThrottle=auditLogThrottle, portCardSubType=portCardSubType, diskVolumeTable=diskVolumeTable, auditEntry=auditEntry, cardHwVersion=cardHwVersion, equipmentDiagGroup=equipmentDiagGroup, cardSerialNum=cardSerialNum, portTable=portTable, cmIpAddress=cmIpAddress, equipmentShelfGroup=equipmentShelfGroup, auditAutoScheduling=auditAutoScheduling, cardTestIndex=cardTestIndex, equipmentCardGroup=equipmentCardGroup, auditTime=auditTime, diskVolumeId=diskVolumeId, cardShelfTestIndex=cardShelfTestIndex, TestSchedule=TestSchedule, cardSwVersion=cardSwVersion, TestCommand=TestCommand, cardSubType=cardSubType, cardLastBootSource=cardLastBootSource, cardName=cardName, cardTestTable=cardTestTable, cardTestDescription=cardTestDescription, cmResetClearNotification=cmResetClearNotification, auditShelfId=auditShelfId, cardTestEntry=cardTestEntry, cardSecStateChange=cardSecStateChange, equipmentAudit=equipmentAudit, errEvRecoveryEnabled=errEvRecoveryEnabled, portDocsIfIndex=portDocsIfIndex, portSecState=portSecState, TestType=TestType, portCardId=portCardId, cardTempNoReportNotification=cardTempNoReportNotification, trapCounter=trapCounter, cmPrimarySID=cmPrimarySID, systemGeneralGroup=systemGeneralGroup, cardTrapInh=cardTrapInh, equipmentTraps=equipmentTraps, equipmentError=equipmentError, portDetectedMode=portDetectedMode, cardId=cardId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestCommand=cardTestCommand, cardCpuSpeed=cardCpuSpeed, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, fpgaErrorEventGroup=fpgaErrorEventGroup, diskDriveNumber=diskDriveNumber, cardSecState=cardSecState, diskVolumeShelfId=diskVolumeShelfId, portAction=portAction, portConnectorId=portConnectorId, cardFlashSize=cardFlashSize, cardTestTime=cardTestTime, cadEquipmentMib=cadEquipmentMib, portMode=portMode, verboseLevel=verboseLevel, portAdminState=portAdminState, equipmentPortGroup=equipmentPortGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardFwVersion=cardFwVersion, portShelfId=portShelfId, portBgpId=portBgpId, systemKey=systemKey, fpgaErrorEventEntry=fpgaErrorEventEntry, cardType=cardType, TestResult=TestResult, cardBootVersion=cardBootVersion, cardAction=cardAction, cardTestTransId=cardTestTransId, auditCardId=auditCardId, equipmentCardTestGroup=equipmentCardTestGroup, shelfNumber=shelfNumber, diskVolumeGroup=diskVolumeGroup, auditCommand=auditCommand, cardPicHwVersion=cardPicHwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCpuType=cardCpuType, diskVolumeEntry=diskVolumeEntry, errEvRowStatus=errEvRowStatus, cardTestResult=cardTestResult, trapSeverity=trapSeverity, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, shelfObjects=shelfObjects, diskVolumeUsageLevel=diskVolumeUsageLevel, auditName=auditName, cardEntry=cardEntry, TestTransactionId=TestTransactionId, cmResetNotification=cmResetNotification, diskVolumeName=diskVolumeName, cardPicDetected=cardPicDetected, portEntry=portEntry, cardTemperatureHighError=cardTemperatureHighError, portTxFlowControlDetected=portTxFlowControlDetected, auditLogOutput=auditLogOutput, TestId=TestId, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, errEvLoggingEnabled=errEvLoggingEnabled, cardLastChangeTime=cardLastChangeTime, cmVendor=cmVendor, diskVolumeCardId=diskVolumeCardId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardPrStateChange=cardPrStateChange, consoleOutput=consoleOutput, portNumChans=portNumChans, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMaxDsPower=portMaxDsPower, cardFwUpdateStatus=cardFwUpdateStatus, shelfName=shelfName, cardTempOverHeatNotification=cardTempOverHeatNotification, portCurrDsPower=portCurrDsPower, cardTestResultDesc=cardTestResultDesc, cardLastBootVersion=cardLastBootVersion, equipmentState=equipmentState, diskVolumeNumber=diskVolumeNumber, auditTotalFailedCount=auditTotalFailedCount, cardTestName=cardTestName, auditResult=auditResult, cardTestSchedule=cardTestSchedule, cardPicSerialNum=cardPicSerialNum, eqDiagConfig=eqDiagConfig, portDescription=portDescription, equipmentTbl=equipmentTbl, cardNumber=cardNumber, portMinDsPower=portMinDsPower, auditCycleCount=auditCycleCount, cardDplxStatus=cardDplxStatus, auditTotalPassedCount=auditTotalPassedCount, portMacAddress=portMacAddress, cardNumPorts=cardNumPorts, cardSpareGroupId=cardSpareGroupId, cardSpareGroupMode=cardSpareGroupMode, cmUChannelID=cmUChannelID, diskVolumeDiskSize=diskVolumeDiskSize, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portDplxStatus=portDplxStatus, cardUpTime=cardUpTime, diskVolumeDriveId=diskVolumeDriveId, systemGeneral=systemGeneral, auditPassedCount=auditPassedCount, auditId=auditId, shelfSwVersion=shelfSwVersion, diskVolumeSize=diskVolumeSize, cardTestScheduleCommand=cardTestScheduleCommand, cardLastResetReason=cardLastResetReason, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDetected=cardDetected, auditTable=auditTable, auditStatus=auditStatus, portNumber=portNumber, cardTestType=cardTestType, portMacIfIndex=portMacIfIndex, errEventId=errEventId, systemClock=systemClock, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCarrierHwVersion=cardCarrierHwVersion, portPrStateChange=portPrStateChange, cardFpgaSource=cardFpgaSource, diskVolumeFileName=diskVolumeFileName, PYSNMP_MODULE_ID=cadEquipmentMib, cardDplxStatusChange=cardDplxStatusChange)
