...
PortType, UnknownState, PrimaryState, ShelfId, PortMode, PortId, DuplexStatus, DiskVolumeUsageLevel, CardSubType, EqActionType, FlowControlMode, PicType, SecondaryState, CardId, AdminState, CardType, PortDetectedMode, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "PortType", "UnknownState", "PrimaryState", "ShelfId", "PortMode", "PortId", "DuplexStatus", "DiskVolumeUsageLevel", "CardSubType", "EqActionType", "FlowControlMode", "PicType", "SecondaryState", "CardId", "AdminState", "CardType", "PortDetectedMode", "FirmwareSource")
...
MacAddress, TruthValue, DisplayString, RowStatus, TimeStamp, TextualConvention, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "DisplayString", "RowStatus", "TimeStamp", "TextualConvention", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", trapCounter=trapCounter, auditResult=auditResult, diskVolumeUsageLevel=diskVolumeUsageLevel, portPrStateChange=portPrStateChange, cardLastResetReason=cardLastResetReason, portBgpId=portBgpId, remainInDiagMode=remainInDiagMode, auditTotalPassedCount=auditTotalPassedCount, equipmentDiagGroup=equipmentDiagGroup, cmMacAddress=cmMacAddress, portShelfId=portShelfId, cardAdminState=cardAdminState, cardSpareGroupId=cardSpareGroupId, portSecState=portSecState, auditLogOutput=auditLogOutput, cmIpAddress=cmIpAddress, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentDiag=equipmentDiag, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestResult=cardTestResult, portConnectorId=portConnectorId, trapSeverity=trapSeverity, downstreamPowerLoss=downstreamPowerLoss, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardName=cardName, cardTestResultDesc=cardTestResultDesc, TestType=TestType, cardRamSize=cardRamSize, cmVendor=cmVendor, diskDriveNumber=diskDriveNumber, cardSubType=cardSubType, auditCycleCount=auditCycleCount, cardType=cardType, diskVolumeTable=diskVolumeTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeNumber=diskVolumeNumber, portCardSubType=portCardSubType, cardTable=cardTable, shelfObjects=shelfObjects, auditTable=auditTable, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentTraps=equipmentTraps, cardAction=cardAction, systemKey=systemKey, cmDevice=cmDevice, portTxFlowControlMode=portTxFlowControlMode, TestSchedule=TestSchedule, cardTrapInh=cardTrapInh, cardSerialNum=cardSerialNum, cardTestTable=cardTestTable, cardTestTime=cardTestTime, auditCardId=auditCardId, auditPassedCount=auditPassedCount, cardSecStateChange=cardSecStateChange, cmResetNotification=cmResetNotification, cardTestCommand=cardTestCommand, portRxFlowControlMode=portRxFlowControlMode, auditCommand=auditCommand, cmResetReason=cmResetReason, errEventId=errEventId, auditFailedCount=auditFailedCount, equipmentCardGroup=equipmentCardGroup, cardSecState=cardSecState, cardSubDetected=cardSubDetected, cardCarrierSerialNum=cardCarrierSerialNum, equipmentAuditGroup=equipmentAuditGroup, cardTestIndex=cardTestIndex, portTable=portTable, portNumber=portNumber, cardDplxStatus=cardDplxStatus, cmUChannelID=cmUChannelID, cardTestName=cardTestName, portRxFlowControlDetected=portRxFlowControlDetected, cardPicDetected=cardPicDetected, equipmentPortGroup=equipmentPortGroup, cmResetUpTime=cmResetUpTime, TestCommand=TestCommand, cardId=cardId, consoleOutput=consoleOutput, diskVolumeGroup=diskVolumeGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, cardBusSpeed=cardBusSpeed, cardFpgaSource=cardFpgaSource, errEvLogLevel=errEvLogLevel, cardShelfId=cardShelfId, portDocsIfIndex=portDocsIfIndex, TestTransactionId=TestTransactionId, cardEntry=cardEntry, cardTestScheduleCommand=cardTestScheduleCommand, fpgaErrorEventTable=fpgaErrorEventTable, portDplxStatus=portDplxStatus, diskVolumeId=diskVolumeId, cardFlashSize=cardFlashSize, portTxFlowControlDetected=portTxFlowControlDetected, verboseLevel=verboseLevel, auditTime=auditTime, equipmentCardTestGroup=equipmentCardTestGroup, cardBootVersion=cardBootVersion, cardNumPorts=cardNumPorts, cadEquipmentMib=cadEquipmentMib, portAdminState=portAdminState, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMibConformance=cadEquipmentMibConformance, auditStatus=auditStatus, systemGeneralGroup=systemGeneralGroup, cardPicSerialNum=cardPicSerialNum, portDetectedMode=portDetectedMode, cardCarrierFwVersion=cardCarrierFwVersion, portAction=portAction, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupMode=cardSpareGroupMode, portDescription=portDescription, cardDetectedChange=cardDetectedChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardFwUpdateStatus=cardFwUpdateStatus, cardTestDescription=cardTestDescription, cardPrStateChange=cardPrStateChange, cardCpuType=cardCpuType, eqDiagConfig=eqDiagConfig, portLastChangeTime=portLastChangeTime, cardTestTransId=cardTestTransId, equipmentError=equipmentError, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPrState=cardPrState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardNumber=cardNumber, cardTestEntry=cardTestEntry, diagTestId=diagTestId, portTrapInh=portTrapInh, cardCpuSpeed=cardCpuSpeed, equipmentShelfGroup=equipmentShelfGroup, cardTemperature=cardTemperature, cardHwVersion=cardHwVersion, portDplxStatusChange=portDplxStatusChange, cardCPLDVersion=cardCPLDVersion, portMaxDsPower=portMaxDsPower, diskVolumeSize=diskVolumeSize, portSecStateChange=portSecStateChange, portMode=portMode, systemGeneral=systemGeneral, cmResetInfo=cmResetInfo, cardTestSchedule=cardTestSchedule, portCurrDsPower=portCurrDsPower, portId=portId, errEvLoggingEnabled=errEvLoggingEnabled, cardTempNoReportNotification=cardTempNoReportNotification, cardLastChangeTime=cardLastChangeTime, cmRegistrationNotification=cmRegistrationNotification, auditAutoScheduling=auditAutoScheduling, diskVolumeDiskSize=diskVolumeDiskSize, cardLastBootSource=cardLastBootSource, cardDetected=cardDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetStatus=cmResetStatus, equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, auditDescription=auditDescription, cmResetClearNotification=cmResetClearNotification, portEntry=portEntry, TestScheduleCommand=TestScheduleCommand, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogThrottle=auditLogThrottle, diskVolumeName=diskVolumeName, errEvRowStatus=errEvRowStatus, auditShelfId=auditShelfId, cardSwVersion=cardSwVersion, cmPrimarySID=cmPrimarySID, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfNumber=shelfNumber, equipmentTbl=equipmentTbl, cardCarrierHwVersion=cardCarrierHwVersion, TestResult=TestResult, cardUpTime=cardUpTime, portType=portType, TestId=TestId, portPrState=portPrState, equipmentStateGroup=equipmentStateGroup, equipmentAudit=equipmentAudit, cardTestType=cardTestType, portMinDsPower=portMinDsPower, cardLastBootVersion=cardLastBootVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, portCardId=portCardId, diskVolumeFileName=diskVolumeFileName, cardTestId=cardTestId, portNumChans=portNumChans, diskVolumeDriveId=diskVolumeDriveId, portMacIfIndex=portMacIfIndex, diskVolumeShelfId=diskVolumeShelfId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardDplxStatusChange=cardDplxStatusChange, cardTestResultNotification=cardTestResultNotification, shelfName=shelfName, systemClock=systemClock, PYSNMP_MODULE_ID=cadEquipmentMib, auditEntry=auditEntry, shelfSwVersion=shelfSwVersion, cardShelfTestIndex=cardShelfTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditId=auditId, cardPicHwVersion=cardPicHwVersion, auditName=auditName, portMacAddress=portMacAddress, cardFwVersion=cardFwVersion, cardTemperatureHighError=cardTemperatureHighError, auditTotalFailedCount=auditTotalFailedCount)
