...
ShelfId, UnknownState, PicType, DiskVolumeUsageLevel, PortMode, PortDetectedMode, FlowControlMode, PortType, SecondaryState, FirmwareSource, DuplexStatus, CardType, AdminState, CardSubType, EqActionType, PortId, PrimaryState, CardId = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "UnknownState", "PicType", "DiskVolumeUsageLevel", "PortMode", "PortDetectedMode", "FlowControlMode", "PortType", "SecondaryState", "FirmwareSource", "DuplexStatus", "CardType", "AdminState", "CardSubType", "EqActionType", "PortId", "PrimaryState", "CardId")
...
DisplayString, MacAddress, TextualConvention, TruthValue, DateAndTime, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "TextualConvention", "TruthValue", "DateAndTime", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastBootVersion=cardLastBootVersion, diskVolumeTrapInh=diskVolumeTrapInh, systemKey=systemKey, cmMacAddress=cmMacAddress, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardCpuSpeed=cardCpuSpeed, auditCommand=auditCommand, equipmentDiagGroup=equipmentDiagGroup, cardBusSpeed=cardBusSpeed, diskVolumeId=diskVolumeId, diskVolumeTable=diskVolumeTable, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMibCompliances=cadEquipmentMibCompliances, shelfName=shelfName, cardDetected=cardDetected, TestScheduleCommand=TestScheduleCommand, cmResetClearNotification=cmResetClearNotification, auditTotalPassedCount=auditTotalPassedCount, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditLogOutput=auditLogOutput, auditTable=auditTable, cardCarrierHwVersion=cardCarrierHwVersion, cardName=cardName, portType=portType, auditStatus=auditStatus, portMaxDsPower=portMaxDsPower, cardTrapInh=cardTrapInh, errEvLoggingEnabled=errEvLoggingEnabled, portTxFlowControlDetected=portTxFlowControlDetected, cmDevice=cmDevice, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, cmIpAddress=cmIpAddress, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSecState=cardSecState, trapCounter=trapCounter, diskVolumeCardId=diskVolumeCardId, shelfNumber=shelfNumber, equipmentDiag=equipmentDiag, TestCommand=TestCommand, cardTestTable=cardTestTable, equipmentAuditGroup=equipmentAuditGroup, portDescription=portDescription, cardNumber=cardNumber, eqDiagConfig=eqDiagConfig, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentStateGroup=equipmentStateGroup, cardPrStateChange=cardPrStateChange, fpgaErrorEventEntry=fpgaErrorEventEntry, errEvRowStatus=errEvRowStatus, TestType=TestType, equipmentError=equipmentError, auditPassedCount=auditPassedCount, cmResetNotification=cmResetNotification, cardLastChangeTime=cardLastChangeTime, auditResult=auditResult, portCardId=portCardId, cmPrimarySID=cmPrimarySID, cardTestResultNotification=cardTestResultNotification, fpgaErrorEventTable=fpgaErrorEventTable, cardBootVersion=cardBootVersion, portTxFlowControlMode=portTxFlowControlMode, cardSpareGroupMode=cardSpareGroupMode, diskVolumeName=diskVolumeName, auditShelfId=auditShelfId, portPrStateChange=portPrStateChange, portAdminState=portAdminState, portEntry=portEntry, cardPicSerialNum=cardPicSerialNum, auditName=auditName, cardId=cardId, diagTestId=diagTestId, portRxFlowControlDetected=portRxFlowControlDetected, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardFwUpdateStatus=cardFwUpdateStatus, cardTestDescription=cardTestDescription, cardTestScheduleCommand=cardTestScheduleCommand, cardFwVersion=cardFwVersion, cardUpTime=cardUpTime, cardLastResetReason=cardLastResetReason, cardNumPorts=cardNumPorts, cadEquipmentMib=cadEquipmentMib, shelfObjects=shelfObjects, cmResetStatus=cmResetStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentState=equipmentState, portSecState=portSecState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditDescription=auditDescription, portMacIfIndex=portMacIfIndex, auditFailedCount=auditFailedCount, systemClock=systemClock, cmResetUpTime=cmResetUpTime, cardPicDetected=cardPicDetected, cardSubType=cardSubType, cardCpuType=cardCpuType, cmUChannelID=cmUChannelID, cardTestIndex=cardTestIndex, cardFpgaSource=cardFpgaSource, downstreamPowerLoss=downstreamPowerLoss, diskVolumeNumber=diskVolumeNumber, consoleOutput=consoleOutput, cardDplxStatusChange=cardDplxStatusChange, cardShelfId=cardShelfId, cardRamSize=cardRamSize, cardTestResultDesc=cardTestResultDesc, portId=portId, portDplxStatus=portDplxStatus, diskVolumeShelfId=diskVolumeShelfId, portRxFlowControlMode=portRxFlowControlMode, cadEquipmentMibGroup=cadEquipmentMibGroup, auditEntry=auditEntry, portNumber=portNumber, cardSubDetected=cardSubDetected, cardCarrierSerialNum=cardCarrierSerialNum, cardPrState=cardPrState, diskVolumeFileName=diskVolumeFileName, cardTestCommand=cardTestCommand, cardTestEntry=cardTestEntry, cardLastBootSource=cardLastBootSource, cardPicHwVersion=cardPicHwVersion, shelfSwVersion=shelfSwVersion, cardSpareGroupId=cardSpareGroupId, cardSecStateChange=cardSecStateChange, equipmentAudit=equipmentAudit, auditLogThrottle=auditLogThrottle, cardEntry=cardEntry, cardCarrierFwVersion=cardCarrierFwVersion, portDetectedMode=portDetectedMode, TestSchedule=TestSchedule, portMinDsPower=portMinDsPower, auditCardId=auditCardId, systemGeneral=systemGeneral, cardDetectedChange=cardDetectedChange, cardTemperature=cardTemperature, cardTestId=cardTestId, diskVolumeSize=diskVolumeSize, diskVolumeGroup=diskVolumeGroup, cardAction=cardAction, equipmentTraps=equipmentTraps, portTable=portTable, verboseLevel=verboseLevel, auditTotalFailedCount=auditTotalFailedCount, cardFlashSize=cardFlashSize, portMacAddress=portMacAddress, cardType=cardType, equipmentShelfGroup=equipmentShelfGroup, cardTemperatureHighError=cardTemperatureHighError, diskDriveNumber=diskDriveNumber, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portTrapInh=portTrapInh, cardSwVersion=cardSwVersion, cardTestResult=cardTestResult, cmResetReason=cmResetReason, errEventId=errEventId, cmRegistrationNotification=cmRegistrationNotification, cardHwVersion=cardHwVersion, cardDplxStatus=cardDplxStatus, TestTransactionId=TestTransactionId, portCurrDsPower=portCurrDsPower, cmVendor=cmVendor, remainInDiagMode=remainInDiagMode, cardShelfTestIndex=cardShelfTestIndex, trapSeverity=trapSeverity, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSerialNum=cardSerialNum, portPrState=portPrState, cardTestTransId=cardTestTransId, cardCPLDVersion=cardCPLDVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardAdminState=cardAdminState, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvLogLevel=errEvLogLevel, portBgpId=portBgpId, cardTestName=cardTestName, cardTestSchedule=cardTestSchedule, portNumChans=portNumChans, portMode=portMode, equipmentCardTestGroup=equipmentCardTestGroup, TestId=TestId, equipmentPortGroup=equipmentPortGroup, portConnectorId=portConnectorId, portDocsIfIndex=portDocsIfIndex, cardTestTime=cardTestTime, equipmentCardGroup=equipmentCardGroup, portLastChangeTime=portLastChangeTime, portShelfId=portShelfId, cardTestType=cardTestType, portDplxStatusChange=portDplxStatusChange, auditId=auditId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portAction=portAction, auditTime=auditTime, equipmentTbl=equipmentTbl, diskVolumeDriveId=diskVolumeDriveId, cardTable=cardTable, cmResetInfo=cmResetInfo, auditAutoScheduling=auditAutoScheduling, portSecStateChange=portSecStateChange, auditCycleCount=auditCycleCount, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portCardSubType=portCardSubType, TestResult=TestResult, diskVolumeEntry=diskVolumeEntry)
