...
PrimaryState, FirmwareSource, PortType, DiskVolumeUsageLevel, CardId, AdminState, PicType, PortMode, SecondaryState, PortDetectedMode, DuplexStatus, ShelfId, UnknownState, FlowControlMode, CardSubType, CardType, EqActionType, PortId = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "FirmwareSource", "PortType", "DiskVolumeUsageLevel", "CardId", "AdminState", "PicType", "PortMode", "SecondaryState", "PortDetectedMode", "DuplexStatus", "ShelfId", "UnknownState", "FlowControlMode", "CardSubType", "CardType", "EqActionType", "PortId")
...
RowStatus, MacAddress, DisplayString, TextualConvention, TimeStamp, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "MacAddress", "DisplayString", "TextualConvention", "TimeStamp", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portDplxStatusChange=portDplxStatusChange, portPrStateChange=portPrStateChange, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMib=cadEquipmentMib, diskVolumeCardId=diskVolumeCardId, cardId=cardId, cardFlashSize=cardFlashSize, portShelfId=portShelfId, cmIpAddress=cmIpAddress, equipmentDiagGroup=equipmentDiagGroup, cardType=cardType, diskVolumeGroup=diskVolumeGroup, cardTemperature=cardTemperature, cmResetNotification=cmResetNotification, portRxFlowControlDetected=portRxFlowControlDetected, portDocsIfIndex=portDocsIfIndex, cmResetStatus=cmResetStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCarrierHwVersion=cardCarrierHwVersion, cardNumber=cardNumber, equipmentStateGroup=equipmentStateGroup, auditLogThrottle=auditLogThrottle, cardTestResultDesc=cardTestResultDesc, equipmentShelfGroup=equipmentShelfGroup, portNumber=portNumber, portMinDsPower=portMinDsPower, cmResetReason=cmResetReason, cardTestResult=cardTestResult, equipmentError=equipmentError, portSecStateChange=portSecStateChange, cardCpuType=cardCpuType, shelfObjects=shelfObjects, fpgaErrorEventGroup=fpgaErrorEventGroup, portDplxStatus=portDplxStatus, TestTransactionId=TestTransactionId, portCardId=portCardId, cardTestTransId=cardTestTransId, cardLastChangeTime=cardLastChangeTime, auditId=auditId, cardCpuSpeed=cardCpuSpeed, TestSchedule=TestSchedule, cmDevice=cmDevice, cardTestDescription=cardTestDescription, cardTestSchedule=cardTestSchedule, diskVolumeShelfId=diskVolumeShelfId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, trapSeverity=trapSeverity, cardLastResetReason=cardLastResetReason, auditAutoScheduling=auditAutoScheduling, cmVendor=cmVendor, cmMacAddress=cmMacAddress, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, PYSNMP_MODULE_ID=cadEquipmentMib, portTxFlowControlMode=portTxFlowControlMode, cardHwVersion=cardHwVersion, cardPicDetected=cardPicDetected, eqDiagConfig=eqDiagConfig, portCurrDsPower=portCurrDsPower, diskVolumeDriveId=diskVolumeDriveId, auditTotalFailedCount=auditTotalFailedCount, cardSecStateChange=cardSecStateChange, diskVolumeSize=diskVolumeSize, cardSubType=cardSubType, cardTestId=cardTestId, auditName=auditName, portLastChangeTime=portLastChangeTime, cardDplxStatus=cardDplxStatus, cardSpareGroupMode=cardSpareGroupMode, cardSpareGroupId=cardSpareGroupId, equipmentState=equipmentState, portPrState=portPrState, errEvLogLevel=errEvLogLevel, cardPrStateChange=cardPrStateChange, cardNumPorts=cardNumPorts, auditTime=auditTime, diskVolumeUsageLevel=diskVolumeUsageLevel, cardAction=cardAction, cardDplxStatusChange=cardDplxStatusChange, remainInDiagMode=remainInDiagMode, cardName=cardName, diskVolumeEntry=diskVolumeEntry, portType=portType, portMacIfIndex=portMacIfIndex, shelfSwVersion=shelfSwVersion, diskVolumeTable=diskVolumeTable, equipmentAudit=equipmentAudit, verboseLevel=verboseLevel, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestEntry=cardTestEntry, errEventId=errEventId, errEvLoggingEnabled=errEvLoggingEnabled, equipmentDiag=equipmentDiag, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardShelfTestIndex=cardShelfTestIndex, cardTestTable=cardTestTable, cardTable=cardTable, diagTestId=diagTestId, cardTestType=cardTestType, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portTrapInh=portTrapInh, cardTestIndex=cardTestIndex, cardTestTime=cardTestTime, auditCardId=auditCardId, auditStatus=auditStatus, errEvRowStatus=errEvRowStatus, auditTotalPassedCount=auditTotalPassedCount, cmPrimarySID=cmPrimarySID, TestCommand=TestCommand, equipmentTraps=equipmentTraps, systemGeneral=systemGeneral, cmUChannelID=cmUChannelID, cardShelfId=cardShelfId, portTable=portTable, cardPrState=cardPrState, fpgaErrorEventTable=fpgaErrorEventTable, equipmentCardGroup=equipmentCardGroup, consoleOutput=consoleOutput, cardPicSerialNum=cardPicSerialNum, cardSubDetected=cardSubDetected, diskVolumeId=diskVolumeId, diskVolumeTrapInh=diskVolumeTrapInh, auditPassedCount=auditPassedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, cardBusSpeed=cardBusSpeed, portConnectorId=portConnectorId, diskVolumeFileName=diskVolumeFileName, cardSwVersion=cardSwVersion, portDescription=portDescription, cardPicHwVersion=cardPicHwVersion, auditCommand=auditCommand, auditFailedCount=auditFailedCount, cmResetClearNotification=cmResetClearNotification, cardSecState=cardSecState, cardSerialNum=cardSerialNum, cardDetected=cardDetected, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditTable=auditTable, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestType=TestType, cardTestScheduleCommand=cardTestScheduleCommand, cardLastBootSource=cardLastBootSource, portMode=portMode, cardTestName=cardTestName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestResult=TestResult, auditShelfId=auditShelfId, cardBootVersion=cardBootVersion, systemClock=systemClock, auditEntry=auditEntry, portRxFlowControlMode=portRxFlowControlMode, downstreamPowerLoss=downstreamPowerLoss, auditLogOutput=auditLogOutput, cardAdminState=cardAdminState, auditResult=auditResult, portCardSubType=portCardSubType, diskDriveNumber=diskDriveNumber, portMacAddress=portMacAddress, shelfNumber=shelfNumber, TestScheduleCommand=TestScheduleCommand, auditCycleCount=auditCycleCount, cardFpgaSource=cardFpgaSource, cardUpTime=cardUpTime, cmResetInfo=cmResetInfo, systemKey=systemKey, cardTrapInh=cardTrapInh, cardTemperatureHighWarn=cardTemperatureHighWarn, portNumChans=portNumChans, diskVolumeName=diskVolumeName, equipmentAuditGroup=equipmentAuditGroup, cardTestResultNotification=cardTestResultNotification, auditDescription=auditDescription, cardCPLDVersion=cardCPLDVersion, portBgpId=portBgpId, portMaxDsPower=portMaxDsPower, cardCarrierFwVersion=cardCarrierFwVersion, equipmentTbl=equipmentTbl, trapCounter=trapCounter, cardEntry=cardEntry, portDetectedMode=portDetectedMode, diskVolumeDiskSize=diskVolumeDiskSize, cardTestCommand=cardTestCommand, equipmentPortGroup=equipmentPortGroup, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierSerialNum=cardCarrierSerialNum, portAdminState=portAdminState, shelfName=shelfName, systemGeneralGroup=systemGeneralGroup, cardTemperatureHighError=cardTemperatureHighError, diskVolumeNumber=diskVolumeNumber, portEntry=portEntry, cardFwUpdateStatus=cardFwUpdateStatus, TestId=TestId, cardDetectedChange=cardDetectedChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portAction=portAction, cardRamSize=cardRamSize, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFwVersion=cardFwVersion, cardLastBootVersion=cardLastBootVersion, portId=portId, portSecState=portSecState, cmResetUpTime=cmResetUpTime, cmRegistrationNotification=cmRegistrationNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification)
