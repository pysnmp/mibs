...
PortDetectedMode, PortId, DiskVolumeUsageLevel, DuplexStatus, FirmwareSource, EqActionType, UnknownState, PortType, CardId, AdminState, PrimaryState, SecondaryState, FlowControlMode, PicType, ShelfId, CardType, CardSubType, PortMode = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "PortId", "DiskVolumeUsageLevel", "DuplexStatus", "FirmwareSource", "EqActionType", "UnknownState", "PortType", "CardId", "AdminState", "PrimaryState", "SecondaryState", "FlowControlMode", "PicType", "ShelfId", "CardType", "CardSubType", "PortMode")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBusSpeed=cardBusSpeed, cardSwVersion=cardSwVersion, cardTestScheduleCommand=cardTestScheduleCommand, auditTime=auditTime, trapCounter=trapCounter, cardUpTime=cardUpTime, cardCPLDVersion=cardCPLDVersion, auditEntry=auditEntry, trapSeverity=trapSeverity, cardTempOverHeatNotification=cardTempOverHeatNotification, cardFpgaSource=cardFpgaSource, portEntry=portEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSubDetected=cardSubDetected, diskVolumeCardId=diskVolumeCardId, cardTestEntry=cardTestEntry, auditCycleCount=auditCycleCount, portConnectorId=portConnectorId, cardDetectedChange=cardDetectedChange, diskVolumeFileName=diskVolumeFileName, auditStatus=auditStatus, downstreamPowerLoss=downstreamPowerLoss, diskVolumeName=diskVolumeName, cardRamSize=cardRamSize, portBgpId=portBgpId, cardId=cardId, diskVolumeDiskSize=diskVolumeDiskSize, errEvRowStatus=errEvRowStatus, cmUChannelID=cmUChannelID, cardHwVersion=cardHwVersion, portId=portId, diskVolumeUsageLevel=diskVolumeUsageLevel, TestScheduleCommand=TestScheduleCommand, equipmentPortGroup=equipmentPortGroup, errEventId=errEventId, auditDescription=auditDescription, equipmentShelfGroup=equipmentShelfGroup, cmResetClearNotification=cmResetClearNotification, auditTable=auditTable, diskVolumeDriveId=diskVolumeDriveId, equipmentState=equipmentState, portPrStateChange=portPrStateChange, cardTestTime=cardTestTime, auditCardId=auditCardId, cardPicDetected=cardPicDetected, cardAction=cardAction, equipmentTraps=equipmentTraps, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditId=auditId, portCurrDsPower=portCurrDsPower, cardTable=cardTable, cmResetReason=cmResetReason, TestId=TestId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestSchedule=cardTestSchedule, cardTestType=cardTestType, portTrapInh=portTrapInh, diskVolumeTable=diskVolumeTable, systemKey=systemKey, portNumChans=portNumChans, equipmentStateGroup=equipmentStateGroup, equipmentCardGroup=equipmentCardGroup, cardTestTransId=cardTestTransId, cardTemperatureHighError=cardTemperatureHighError, shelfName=shelfName, portRxFlowControlDetected=portRxFlowControlDetected, auditCommand=auditCommand, cardCarrierHwVersion=cardCarrierHwVersion, verboseLevel=verboseLevel, cardSecStateChange=cardSecStateChange, cmResetInfo=cmResetInfo, portTxFlowControlMode=portTxFlowControlMode, PYSNMP_MODULE_ID=cadEquipmentMib, cardSpareGroupMode=cardSpareGroupMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeEntry=diskVolumeEntry, cardType=cardType, shelfSwVersion=shelfSwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, portNumber=portNumber, cardCarrierFwVersion=cardCarrierFwVersion, portMaxDsPower=portMaxDsPower, cardTemperature=cardTemperature, eqDiagConfig=eqDiagConfig, auditTotalPassedCount=auditTotalPassedCount, cardDplxStatus=cardDplxStatus, cardAdminState=cardAdminState, portMacAddress=portMacAddress, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardEntry=cardEntry, auditLogThrottle=auditLogThrottle, cardShelfTestIndex=cardShelfTestIndex, cardShelfId=cardShelfId, cardTestId=cardTestId, cardNumPorts=cardNumPorts, portDetectedMode=portDetectedMode, cmMacAddress=cmMacAddress, cardTestCommand=cardTestCommand, TestCommand=TestCommand, TestResult=TestResult, auditName=auditName, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeSize=diskVolumeSize, cmRegistrationNotification=cmRegistrationNotification, consoleOutput=consoleOutput, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTempNoReportNotification=cardTempNoReportNotification, cardSpareGroupId=cardSpareGroupId, cardCarrierSerialNum=cardCarrierSerialNum, auditAutoScheduling=auditAutoScheduling, portAdminState=portAdminState, systemGeneral=systemGeneral, cardLastBootSource=cardLastBootSource, cardLastChangeTime=cardLastChangeTime, equipmentCardTestGroup=equipmentCardTestGroup, portRxFlowControlMode=portRxFlowControlMode, cardTrapInh=cardTrapInh, cardTestTable=cardTestTable, cmVendor=cmVendor, diskDriveNumber=diskDriveNumber, cmResetUpTime=cmResetUpTime, portMacIfIndex=portMacIfIndex, cardTestResultNotification=cardTestResultNotification, cardFlashSize=cardFlashSize, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, cardBootVersion=cardBootVersion, portCardSubType=portCardSubType, portCardId=portCardId, cardSerialNum=cardSerialNum, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, systemClock=systemClock, auditResult=auditResult, cardDetected=cardDetected, auditPassedCount=auditPassedCount, cardSecState=cardSecState, cmResetNotification=cmResetNotification, portType=portType, TestType=TestType, cardTestResult=cardTestResult, equipmentError=equipmentError, cardFwUpdateStatus=cardFwUpdateStatus, portSecState=portSecState, TestTransactionId=TestTransactionId, portDocsIfIndex=portDocsIfIndex, cmResetStatus=cmResetStatus, equipmentDiag=equipmentDiag, cardNumber=cardNumber, shelfObjects=shelfObjects, cardPicHwVersion=cardPicHwVersion, portShelfId=portShelfId, portPrState=portPrState, cmIpAddress=cmIpAddress, diskVolumeId=diskVolumeId, portDescription=portDescription, portLastChangeTime=portLastChangeTime, portSecStateChange=portSecStateChange, cardCpuType=cardCpuType, cardFwVersion=cardFwVersion, auditShelfId=auditShelfId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSubType=cardSubType, cardDplxStatusChange=cardDplxStatusChange, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventTable=fpgaErrorEventTable, TestSchedule=TestSchedule, cardLastResetReason=cardLastResetReason, shelfNumber=shelfNumber, portAction=portAction, auditLogOutput=auditLogOutput, cardName=cardName, diskVolumeTrapInh=diskVolumeTrapInh, portDplxStatusChange=portDplxStatusChange, portDplxStatus=portDplxStatus, cardLastBootVersion=cardLastBootVersion, remainInDiagMode=remainInDiagMode, equipmentTbl=equipmentTbl, cardTestIndex=cardTestIndex, cardPrStateChange=cardPrStateChange, errEvLogLevel=errEvLogLevel, cmPrimarySID=cmPrimarySID, cardPicSerialNum=cardPicSerialNum, auditTotalFailedCount=auditTotalFailedCount, cmDevice=cmDevice, systemGeneralGroup=systemGeneralGroup, cardCpuSpeed=cardCpuSpeed, cardTestDescription=cardTestDescription, cardTestResultDesc=cardTestResultDesc, portTxFlowControlDetected=portTxFlowControlDetected, equipmentAudit=equipmentAudit, diskVolumeNumber=diskVolumeNumber, cardTemperatureHighWarn=cardTemperatureHighWarn, portMinDsPower=portMinDsPower, cardTestName=cardTestName, fpgaErrorEventEntry=fpgaErrorEventEntry, portTable=portTable, diagTestId=diagTestId, cardPrState=cardPrState, equipmentDiagGroup=equipmentDiagGroup, auditFailedCount=auditFailedCount, portMode=portMode)
