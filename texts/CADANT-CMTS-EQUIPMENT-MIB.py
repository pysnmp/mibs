...
AdminState, EqActionType, DuplexStatus, UnknownState, CardType, ShelfId, PicType, PortDetectedMode, CardSubType, FirmwareSource, FlowControlMode, PortMode, PortId, PortType, PrimaryState, SecondaryState, DiskVolumeUsageLevel, CardId = mibBuilder.importSymbols("CADANT-TC", "AdminState", "EqActionType", "DuplexStatus", "UnknownState", "CardType", "ShelfId", "PicType", "PortDetectedMode", "CardSubType", "FirmwareSource", "FlowControlMode", "PortMode", "PortId", "PortType", "PrimaryState", "SecondaryState", "DiskVolumeUsageLevel", "CardId")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardHwVersion=cardHwVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfObjects=shelfObjects, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentCardGroup=equipmentCardGroup, cardSpareGroupId=cardSpareGroupId, cardFwVersion=cardFwVersion, cardSubType=cardSubType, auditId=auditId, cardDplxStatus=cardDplxStatus, cardRamSize=cardRamSize, TestType=TestType, auditLogThrottle=auditLogThrottle, auditEntry=auditEntry, cardSecStateChange=cardSecStateChange, portLastChangeTime=portLastChangeTime, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCPLDVersion=cardCPLDVersion, cmRegistrationNotification=cmRegistrationNotification, equipmentError=equipmentError, cmResetStatus=cmResetStatus, cardSpareGroupMode=cardSpareGroupMode, cardTestId=cardTestId, cardNumPorts=cardNumPorts, diskVolumeCardId=diskVolumeCardId, fpgaErrorEventTable=fpgaErrorEventTable, cmResetInfo=cmResetInfo, systemClock=systemClock, fpgaErrorEventEntry=fpgaErrorEventEntry, portId=portId, cardSwVersion=cardSwVersion, cardCpuSpeed=cardCpuSpeed, cardFpgaSource=cardFpgaSource, equipmentState=equipmentState, cmUChannelID=cmUChannelID, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestEntry=cardTestEntry, auditTime=auditTime, cardTestTime=cardTestTime, errEvRecoveryEnabled=errEvRecoveryEnabled, cmMacAddress=cmMacAddress, cardName=cardName, cardFlashSize=cardFlashSize, auditDescription=auditDescription, cmResetNotification=cmResetNotification, cardSecState=cardSecState, cardBootVersion=cardBootVersion, errEvLoggingEnabled=errEvLoggingEnabled, equipmentTraps=equipmentTraps, shelfSwVersion=shelfSwVersion, portDescription=portDescription, equipmentDiag=equipmentDiag, cmPrimarySID=cmPrimarySID, cardShelfTestIndex=cardShelfTestIndex, errEvLogLevel=errEvLogLevel, diskVolumeShelfId=diskVolumeShelfId, diskVolumeDriveId=diskVolumeDriveId, cardPicDetected=cardPicDetected, cardTestTransId=cardTestTransId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTable=cardTestTable, portDocsIfIndex=portDocsIfIndex, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestResult=cardTestResult, portMinDsPower=portMinDsPower, errEventId=errEventId, portMacIfIndex=portMacIfIndex, portDplxStatusChange=portDplxStatusChange, cardTemperature=cardTemperature, portCurrDsPower=portCurrDsPower, cadEquipmentMib=cadEquipmentMib, diskVolumeGroup=diskVolumeGroup, cardTestResultNotification=cardTestResultNotification, portMacAddress=portMacAddress, diskVolumeEntry=diskVolumeEntry, trapCounter=trapCounter, cardLastBootVersion=cardLastBootVersion, cardTestType=cardTestType, cardTestName=cardTestName, cardEntry=cardEntry, equipmentShelfGroup=equipmentShelfGroup, portTxFlowControlDetected=portTxFlowControlDetected, equipmentStateGroup=equipmentStateGroup, portSecStateChange=portSecStateChange, portRxFlowControlDetected=portRxFlowControlDetected, cardCarrierFwVersion=cardCarrierFwVersion, cardTestScheduleCommand=cardTestScheduleCommand, cardFwUpdateStatus=cardFwUpdateStatus, cardDplxStatusChange=cardDplxStatusChange, cmDevice=cmDevice, portDplxStatus=portDplxStatus, auditCycleCount=auditCycleCount, portEntry=portEntry, diagTestId=diagTestId, diskVolumeTable=diskVolumeTable, cmIpAddress=cmIpAddress, cardCarrierHwVersion=cardCarrierHwVersion, cardSubDetected=cardSubDetected, portPrStateChange=portPrStateChange, auditStatus=auditStatus, cardTestDescription=cardTestDescription, portPrState=portPrState, portShelfId=portShelfId, auditShelfId=auditShelfId, cardPrState=cardPrState, diskVolumeName=diskVolumeName, cardPicSerialNum=cardPicSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, portCardId=portCardId, portBgpId=portBgpId, portAction=portAction, diskVolumeNumber=diskVolumeNumber, cardTemperatureHighError=cardTemperatureHighError, cmResetClearNotification=cmResetClearNotification, TestCommand=TestCommand, cardAdminState=cardAdminState, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestIndex=cardTestIndex, cardAction=cardAction, cardLastChangeTime=cardLastChangeTime, cardCpuType=cardCpuType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditCardId=auditCardId, remainInDiagMode=remainInDiagMode, cardShelfId=cardShelfId, cardLastBootSource=cardLastBootSource, cardTestSchedule=cardTestSchedule, equipmentAuditGroup=equipmentAuditGroup, diskVolumeSize=diskVolumeSize, cardTrapInh=cardTrapInh, portMaxDsPower=portMaxDsPower, trapSeverity=trapSeverity, auditTotalPassedCount=auditTotalPassedCount, equipmentDiagGroup=equipmentDiagGroup, diskVolumeTrapInh=diskVolumeTrapInh, portNumChans=portNumChans, eqDiagConfig=eqDiagConfig, cardCarrierSerialNum=cardCarrierSerialNum, portTrapInh=portTrapInh, auditFailedCount=auditFailedCount, downstreamPowerLoss=downstreamPowerLoss, TestTransactionId=TestTransactionId, cmResetUpTime=cmResetUpTime, fpgaErrorEventGroup=fpgaErrorEventGroup, systemGeneralGroup=systemGeneralGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardDetectedChange=cardDetectedChange, portNumber=portNumber, portMode=portMode, equipmentAudit=equipmentAudit, cmVendor=cmVendor, cardTempNoReportNotification=cardTempNoReportNotification, TestResult=TestResult, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardDetected=cardDetected, portTable=portTable, portType=portType, PYSNMP_MODULE_ID=cadEquipmentMib, shelfName=shelfName, systemKey=systemKey, equipmentCardTestGroup=equipmentCardTestGroup, auditName=auditName, consoleOutput=consoleOutput, cardType=cardType, cardUpTime=cardUpTime, cardLastResetReason=cardLastResetReason, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageLevel=diskVolumeUsageLevel, TestId=TestId, portAdminState=portAdminState, cmResetReason=cmResetReason, cardTable=cardTable, diskDriveNumber=diskDriveNumber, shelfNumber=shelfNumber, cardPrStateChange=cardPrStateChange, diskVolumeId=diskVolumeId, portCardSubType=portCardSubType, auditTable=auditTable, auditCommand=auditCommand, cardTestCommand=cardTestCommand, verboseLevel=verboseLevel, cardPicHwVersion=cardPicHwVersion, TestSchedule=TestSchedule, cardTestResultDesc=cardTestResultDesc, portConnectorId=portConnectorId, auditResult=auditResult, cardId=cardId, TestScheduleCommand=TestScheduleCommand, portDetectedMode=portDetectedMode, cardNumber=cardNumber, equipmentPortGroup=equipmentPortGroup, portTxFlowControlMode=portTxFlowControlMode, errEvRowStatus=errEvRowStatus, equipmentTbl=equipmentTbl, portSecState=portSecState, diskVolumeFileName=diskVolumeFileName, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSerialNum=cardSerialNum, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditPassedCount=auditPassedCount, auditLogOutput=auditLogOutput, portRxFlowControlMode=portRxFlowControlMode, cardBusSpeed=cardBusSpeed, auditAutoScheduling=auditAutoScheduling, systemGeneral=systemGeneral)
