...
PortId, PicType, DiskVolumeUsageLevel, CardType, DuplexStatus, FlowControlMode, CardSubType, PrimaryState, SecondaryState, CardId, AdminState, UnknownState, ShelfId, PortType, PortMode, PortDetectedMode, EqActionType, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "PortId", "PicType", "DiskVolumeUsageLevel", "CardType", "DuplexStatus", "FlowControlMode", "CardSubType", "PrimaryState", "SecondaryState", "CardId", "AdminState", "UnknownState", "ShelfId", "PortType", "PortMode", "PortDetectedMode", "EqActionType", "FirmwareSource")
...
TimeStamp, TruthValue, MacAddress, DisplayString, TextualConvention, RowStatus, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "MacAddress", "DisplayString", "TextualConvention", "RowStatus", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentAudit=equipmentAudit, auditPassedCount=auditPassedCount, cmVendor=cmVendor, cardRamSize=cardRamSize, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestEntry=cardTestEntry, cmRegistrationNotification=cmRegistrationNotification, portPrState=portPrState, cardTrapInh=cardTrapInh, portMaxDsPower=portMaxDsPower, auditTable=auditTable, errEvRowStatus=errEvRowStatus, portDplxStatus=portDplxStatus, cardEntry=cardEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentStateGroup=equipmentStateGroup, cardCpuSpeed=cardCpuSpeed, shelfObjects=shelfObjects, cardSubDetected=cardSubDetected, verboseLevel=verboseLevel, cardShelfId=cardShelfId, cardTestName=cardTestName, portNumChans=portNumChans, cardPicDetected=cardPicDetected, shelfName=shelfName, equipmentCardGroup=equipmentCardGroup, auditLogThrottle=auditLogThrottle, diskVolumeSize=diskVolumeSize, cadEquipmentMibConformance=cadEquipmentMibConformance, auditCycleCount=auditCycleCount, diskVolumeTrapInh=diskVolumeTrapInh, errEventId=errEventId, cardLastChangeTime=cardLastChangeTime, equipmentState=equipmentState, portCardId=portCardId, diskVolumeDiskSize=diskVolumeDiskSize, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardDetectedChange=cardDetectedChange, cmResetUpTime=cmResetUpTime, cardSwVersion=cardSwVersion, cardLastBootVersion=cardLastBootVersion, auditLogOutput=auditLogOutput, cardCpuType=cardCpuType, TestScheduleCommand=TestScheduleCommand, cardTempNoReportNotification=cardTempNoReportNotification, cardFwUpdateStatus=cardFwUpdateStatus, auditResult=auditResult, equipmentShelfGroup=equipmentShelfGroup, TestSchedule=TestSchedule, cardType=cardType, diskVolumeTable=diskVolumeTable, cardBusSpeed=cardBusSpeed, auditTotalPassedCount=auditTotalPassedCount, auditId=auditId, cardCarrierFwVersion=cardCarrierFwVersion, portDplxStatusChange=portDplxStatusChange, cmDevice=cmDevice, auditCardId=auditCardId, errEvRecoveryEnabled=errEvRecoveryEnabled, remainInDiagMode=remainInDiagMode, cmResetReason=cmResetReason, diagTestId=diagTestId, portTrapInh=portTrapInh, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentError=equipmentError, cardTestType=cardTestType, systemClock=systemClock, cardTemperature=cardTemperature, cardTestTime=cardTestTime, portConnectorId=portConnectorId, cardDplxStatus=cardDplxStatus, diskVolumeEntry=diskVolumeEntry, auditDescription=auditDescription, cardNumPorts=cardNumPorts, cardLastResetReason=cardLastResetReason, cmPrimarySID=cmPrimarySID, cmResetClearNotification=cmResetClearNotification, cardPrStateChange=cardPrStateChange, systemKey=systemKey, cardPicHwVersion=cardPicHwVersion, shelfSwVersion=shelfSwVersion, diskVolumeFileName=diskVolumeFileName, portNumber=portNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, TestCommand=TestCommand, portMode=portMode, cardTestScheduleCommand=cardTestScheduleCommand, cardTestTable=cardTestTable, portTable=portTable, auditName=auditName, cardTable=cardTable, auditShelfId=auditShelfId, equipmentCardTestGroup=equipmentCardTestGroup, cardDplxStatusChange=cardDplxStatusChange, portShelfId=portShelfId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentPortGroup=equipmentPortGroup, portDescription=portDescription, cardLastBootSource=cardLastBootSource, cardId=cardId, trapCounter=trapCounter, portSecState=portSecState, cardShelfTestIndex=cardShelfTestIndex, cardSpareGroupId=cardSpareGroupId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardHwVersion=cardHwVersion, portCurrDsPower=portCurrDsPower, portTxFlowControlDetected=portTxFlowControlDetected, cardPicSerialNum=cardPicSerialNum, cardBootVersion=cardBootVersion, cardSecStateChange=cardSecStateChange, cardSpareGroupMode=cardSpareGroupMode, portEntry=portEntry, TestType=TestType, cardAction=cardAction, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetNotification=cmResetNotification, diskVolumeName=diskVolumeName, cmResetStatus=cmResetStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestResult=cardTestResult, trapSeverity=trapSeverity, cardTemperatureHighError=cardTemperatureHighError, diskVolumeShelfId=diskVolumeShelfId, cardSerialNum=cardSerialNum, diskVolumeDriveId=diskVolumeDriveId, portMacIfIndex=portMacIfIndex, portTxFlowControlMode=portTxFlowControlMode, diskVolumeNumber=diskVolumeNumber, fpgaErrorEventTable=fpgaErrorEventTable, cardName=cardName, cardTemperatureHighWarn=cardTemperatureHighWarn, consoleOutput=consoleOutput, cardFpgaSource=cardFpgaSource, eqDiagConfig=eqDiagConfig, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEvLoggingEnabled=errEvLoggingEnabled, systemGeneral=systemGeneral, diskDriveNumber=diskDriveNumber, portRxFlowControlMode=portRxFlowControlMode, cardNumber=cardNumber, cardDetected=cardDetected, portCardSubType=portCardSubType, portMinDsPower=portMinDsPower, TestId=TestId, auditTime=auditTime, equipmentTbl=equipmentTbl, portRxFlowControlDetected=portRxFlowControlDetected, cardTestCommand=cardTestCommand, cardSubType=cardSubType, PYSNMP_MODULE_ID=cadEquipmentMib, auditEntry=auditEntry, shelfNumber=shelfNumber, cardTestId=cardTestId, cardAdminState=cardAdminState, cardTestTransId=cardTestTransId, systemGeneralGroup=systemGeneralGroup, auditStatus=auditStatus, portBgpId=portBgpId, cardFlashSize=cardFlashSize, cardTestIndex=cardTestIndex, equipmentTraps=equipmentTraps, portDetectedMode=portDetectedMode, portSecStateChange=portSecStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, TestResult=TestResult, portLastChangeTime=portLastChangeTime, portMacAddress=portMacAddress, auditAutoScheduling=auditAutoScheduling, cardTestSchedule=cardTestSchedule, cmUChannelID=cmUChannelID, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardPrState=cardPrState, errEvLogLevel=errEvLogLevel, cardSecState=cardSecState, equipmentAuditGroup=equipmentAuditGroup, cardCarrierHwVersion=cardCarrierHwVersion, portPrStateChange=portPrStateChange, auditCommand=auditCommand, cardTestResultDesc=cardTestResultDesc, cardTestDescription=cardTestDescription, auditFailedCount=auditFailedCount, portAction=portAction, cardUpTime=cardUpTime, portAdminState=portAdminState, portId=portId, cmIpAddress=cmIpAddress, equipmentDiagGroup=equipmentDiagGroup, cardFwVersion=cardFwVersion, TestTransactionId=TestTransactionId, cardCPLDVersion=cardCPLDVersion, cmResetInfo=cmResetInfo, cadEquipmentMib=cadEquipmentMib, downstreamPowerLoss=downstreamPowerLoss, cmMacAddress=cmMacAddress, cardTestResultNotification=cardTestResultNotification, portDocsIfIndex=portDocsIfIndex, equipmentDiag=equipmentDiag, portType=portType, diskVolumeId=diskVolumeId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditTotalFailedCount=auditTotalFailedCount, diskVolumeCardId=diskVolumeCardId)
