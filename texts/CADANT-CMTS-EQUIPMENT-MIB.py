...
FirmwareSource, DuplexStatus, PortMode, PortDetectedMode, UnknownState, DiskVolumeUsageLevel, CardSubType, PortType, PortId, ShelfId, EqActionType, PrimaryState, AdminState, CardType, CardId, SecondaryState, PicType, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "DuplexStatus", "PortMode", "PortDetectedMode", "UnknownState", "DiskVolumeUsageLevel", "CardSubType", "PortType", "PortId", "ShelfId", "EqActionType", "PrimaryState", "AdminState", "CardType", "CardId", "SecondaryState", "PicType", "FlowControlMode")
...
TimeStamp, DateAndTime, TextualConvention, TruthValue, MacAddress, RowStatus, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DateAndTime", "TextualConvention", "TruthValue", "MacAddress", "RowStatus", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResultDesc=cardTestResultDesc, portNumber=portNumber, shelfNumber=shelfNumber, auditName=auditName, cardCarrierHwVersion=cardCarrierHwVersion, cardTestScheduleCommand=cardTestScheduleCommand, auditCommand=auditCommand, cardTestTime=cardTestTime, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeCardId=diskVolumeCardId, cardTestName=cardTestName, diskVolumeTable=diskVolumeTable, diskVolumeGroup=diskVolumeGroup, cardSpareGroupMode=cardSpareGroupMode, cardEntry=cardEntry, cardTable=cardTable, portNumChans=portNumChans, cardLastResetReason=cardLastResetReason, auditDescription=auditDescription, cadEquipmentMibConformance=cadEquipmentMibConformance, portDescription=portDescription, portMinDsPower=portMinDsPower, cmRegistrationNotification=cmRegistrationNotification, cmResetNotification=cmResetNotification, portCardId=portCardId, portDocsIfIndex=portDocsIfIndex, cardTestCommand=cardTestCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, cardNumber=cardNumber, cardTestSchedule=cardTestSchedule, auditFailedCount=auditFailedCount, portEntry=portEntry, cmDevice=cmDevice, fpgaErrorEventGroup=fpgaErrorEventGroup, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestCommand=TestCommand, auditCardId=auditCardId, shelfSwVersion=shelfSwVersion, cadEquipmentMib=cadEquipmentMib, cardAdminState=cardAdminState, equipmentCardGroup=equipmentCardGroup, equipmentShelfGroup=equipmentShelfGroup, portTxFlowControlMode=portTxFlowControlMode, portCardSubType=portCardSubType, portMacIfIndex=portMacIfIndex, cmPrimarySID=cmPrimarySID, auditTable=auditTable, equipmentError=equipmentError, errEvRowStatus=errEvRowStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portLastChangeTime=portLastChangeTime, cardFlashSize=cardFlashSize, cardSwVersion=cardSwVersion, portId=portId, cardTestEntry=cardTestEntry, cardFwVersion=cardFwVersion, auditResult=auditResult, portAdminState=portAdminState, remainInDiagMode=remainInDiagMode, diskVolumeEntry=diskVolumeEntry, equipmentDiagGroup=equipmentDiagGroup, equipmentAuditGroup=equipmentAuditGroup, equipmentState=equipmentState, portSecState=portSecState, errEvRecoveryEnabled=errEvRecoveryEnabled, downstreamPowerLoss=downstreamPowerLoss, errEvLogLevel=errEvLogLevel, equipmentTraps=equipmentTraps, cardSubType=cardSubType, cardTestTable=cardTestTable, cardTempOverHeatNotification=cardTempOverHeatNotification, portMaxDsPower=portMaxDsPower, auditCycleCount=auditCycleCount, cardSecStateChange=cardSecStateChange, systemKey=systemKey, cardRamSize=cardRamSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, verboseLevel=verboseLevel, portDetectedMode=portDetectedMode, diskVolumeDiskSize=diskVolumeDiskSize, cmMacAddress=cmMacAddress, portAction=portAction, cmResetInfo=cmResetInfo, diskVolumeTrapInh=diskVolumeTrapInh, cardTemperatureHighWarn=cardTemperatureHighWarn, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentCardTestGroup=equipmentCardTestGroup, cardShelfTestIndex=cardShelfTestIndex, errEventId=errEventId, portDplxStatus=portDplxStatus, cardBusSpeed=cardBusSpeed, TestId=TestId, shelfObjects=shelfObjects, auditTime=auditTime, auditEntry=auditEntry, cardTestResult=cardTestResult, cmResetUpTime=cmResetUpTime, cardAction=cardAction, diskVolumeFileName=diskVolumeFileName, portSecStateChange=portSecStateChange, cmUChannelID=cmUChannelID, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTemperatureHighError=cardTemperatureHighError, diskVolumeSize=diskVolumeSize, equipmentTbl=equipmentTbl, cardTestType=cardTestType, diagTestId=diagTestId, cmResetStatus=cmResetStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, portTxFlowControlDetected=portTxFlowControlDetected, cardSubDetected=cardSubDetected, cardPicDetected=cardPicDetected, diskVolumeShelfId=diskVolumeShelfId, portTrapInh=portTrapInh, systemGeneral=systemGeneral, systemClock=systemClock, equipmentStateGroup=equipmentStateGroup, cmResetReason=cmResetReason, cardPrStateChange=cardPrStateChange, cardShelfId=cardShelfId, cardCpuSpeed=cardCpuSpeed, cardTestTransId=cardTestTransId, TestTransactionId=TestTransactionId, portMacAddress=portMacAddress, cardDplxStatus=cardDplxStatus, cardNumPorts=cardNumPorts, trapCounter=trapCounter, cardLastBootVersion=cardLastBootVersion, auditStatus=auditStatus, cardCPLDVersion=cardCPLDVersion, auditLogThrottle=auditLogThrottle, cardUpTime=cardUpTime, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditLogOutput=auditLogOutput, auditTotalFailedCount=auditTotalFailedCount, TestScheduleCommand=TestScheduleCommand, cardId=cardId, cardTemperature=cardTemperature, cardDetected=cardDetected, equipmentDiag=equipmentDiag, cardPrState=cardPrState, portPrStateChange=portPrStateChange, diskVolumeNumber=diskVolumeNumber, auditTotalPassedCount=auditTotalPassedCount, cardDetectedChange=cardDetectedChange, fpgaErrorEventTable=fpgaErrorEventTable, cardTestResultNotification=cardTestResultNotification, portShelfId=portShelfId, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfName=shelfName, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portCurrDsPower=portCurrDsPower, cardSerialNum=cardSerialNum, cardCpuType=cardCpuType, portBgpId=portBgpId, portPrState=portPrState, cardTestDescription=cardTestDescription, cardLastChangeTime=cardLastChangeTime, cmResetClearNotification=cmResetClearNotification, cardCarrierFwVersion=cardCarrierFwVersion, cardFpgaSource=cardFpgaSource, auditShelfId=auditShelfId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portRxFlowControlMode=portRxFlowControlMode, equipmentPortGroup=equipmentPortGroup, eqDiagConfig=eqDiagConfig, auditId=auditId, cardDplxStatusChange=cardDplxStatusChange, diskDriveNumber=diskDriveNumber, cardPicSerialNum=cardPicSerialNum, portDplxStatusChange=portDplxStatusChange, cardType=cardType, cardHwVersion=cardHwVersion, cardSecState=cardSecState, cardTrapInh=cardTrapInh, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeName=diskVolumeName, portTable=portTable, cardBootVersion=cardBootVersion, portConnectorId=portConnectorId, consoleOutput=consoleOutput, TestResult=TestResult, cmVendor=cmVendor, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portMode=portMode, cardPicHwVersion=cardPicHwVersion, cardName=cardName, diskVolumeDriveId=diskVolumeDriveId, cardTestId=cardTestId, auditPassedCount=auditPassedCount, trapSeverity=trapSeverity, PYSNMP_MODULE_ID=cadEquipmentMib, cardLastBootSource=cardLastBootSource, equipmentAudit=equipmentAudit, cmIpAddress=cmIpAddress, TestType=TestType, TestSchedule=TestSchedule, diskVolumeId=diskVolumeId, auditAutoScheduling=auditAutoScheduling, cardFwUpdateStatus=cardFwUpdateStatus, portType=portType, cardTempNoReportNotification=cardTempNoReportNotification, cardTestIndex=cardTestIndex)
