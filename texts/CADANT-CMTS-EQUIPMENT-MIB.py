...
PicType, CardSubType, PortDetectedMode, FirmwareSource, DiskVolumeUsageLevel, PrimaryState, DuplexStatus, ShelfId, UnknownState, AdminState, EqActionType, PortMode, SecondaryState, CardId, FlowControlMode, PortId, PortType, CardType = mibBuilder.importSymbols("CADANT-TC", "PicType", "CardSubType", "PortDetectedMode", "FirmwareSource", "DiskVolumeUsageLevel", "PrimaryState", "DuplexStatus", "ShelfId", "UnknownState", "AdminState", "EqActionType", "PortMode", "SecondaryState", "CardId", "FlowControlMode", "PortId", "PortType", "CardType")
...
MacAddress, TimeStamp, DateAndTime, RowStatus, DisplayString, TruthValue, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TimeStamp", "DateAndTime", "RowStatus", "DisplayString", "TruthValue", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardAction=cardAction, cardCpuSpeed=cardCpuSpeed, auditStatus=auditStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestIndex=cardTestIndex, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFwUpdateStatus=cardFwUpdateStatus, equipmentAudit=equipmentAudit, auditId=auditId, cardTable=cardTable, cardDplxStatusChange=cardDplxStatusChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, trapCounter=trapCounter, cardDetectedChange=cardDetectedChange, portSecStateChange=portSecStateChange, cmResetNotification=cmResetNotification, portAdminState=portAdminState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditShelfId=auditShelfId, cardTestResultDesc=cardTestResultDesc, diskVolumeNumber=diskVolumeNumber, cardSpareGroupId=cardSpareGroupId, cardAdminState=cardAdminState, cardLastBootVersion=cardLastBootVersion, portBgpId=portBgpId, cardTestCommand=cardTestCommand, cardHwVersion=cardHwVersion, equipmentDiag=equipmentDiag, cardTempNoReportNotification=cardTempNoReportNotification, portRxFlowControlDetected=portRxFlowControlDetected, portDocsIfIndex=portDocsIfIndex, cardFpgaSource=cardFpgaSource, TestTransactionId=TestTransactionId, cardFlashSize=cardFlashSize, cardSerialNum=cardSerialNum, diskVolumeSize=diskVolumeSize, errEvLogLevel=errEvLogLevel, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestResultNotification=cardTestResultNotification, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardEntry=cardEntry, cardTestTransId=cardTestTransId, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestType=cardTestType, auditEntry=auditEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeCardId=diskVolumeCardId, cardSubDetected=cardSubDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardPrState=cardPrState, auditTotalPassedCount=auditTotalPassedCount, equipmentPortGroup=equipmentPortGroup, auditLogOutput=auditLogOutput, auditDescription=auditDescription, TestSchedule=TestSchedule, shelfName=shelfName, diskVolumeFileName=diskVolumeFileName, portMode=portMode, cardTestName=cardTestName, auditFailedCount=auditFailedCount, cardNumber=cardNumber, diskVolumeEntry=diskVolumeEntry, cardPicSerialNum=cardPicSerialNum, cardPrStateChange=cardPrStateChange, cardTestId=cardTestId, errEvLoggingEnabled=errEvLoggingEnabled, cmResetUpTime=cmResetUpTime, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestScheduleCommand=cardTestScheduleCommand, errEventId=errEventId, cardSwVersion=cardSwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestDescription=cardTestDescription, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCarrierHwVersion=cardCarrierHwVersion, cmRegistrationNotification=cmRegistrationNotification, auditCommand=auditCommand, cmResetStatus=cmResetStatus, portEntry=portEntry, diskVolumeGroup=diskVolumeGroup, portDescription=portDescription, equipmentDiagGroup=equipmentDiagGroup, cardType=cardType, auditAutoScheduling=auditAutoScheduling, cardBusSpeed=cardBusSpeed, diskVolumeShelfId=diskVolumeShelfId, auditName=auditName, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestTable=cardTestTable, cardLastBootSource=cardLastBootSource, remainInDiagMode=remainInDiagMode, TestId=TestId, cardUpTime=cardUpTime, cardTestResult=cardTestResult, portSecState=portSecState, cardDetected=cardDetected, portPrStateChange=portPrStateChange, equipmentCardTestGroup=equipmentCardTestGroup, cardTemperature=cardTemperature, portCardId=portCardId, systemGeneral=systemGeneral, portPrState=portPrState, cmMacAddress=cmMacAddress, portNumChans=portNumChans, TestType=TestType, equipmentState=equipmentState, cmResetReason=cmResetReason, fpgaErrorEventTable=fpgaErrorEventTable, portNumber=portNumber, cardSecState=cardSecState, cardTestEntry=cardTestEntry, equipmentShelfGroup=equipmentShelfGroup, diskVolumeDiskSize=diskVolumeDiskSize, portConnectorId=portConnectorId, TestResult=TestResult, downstreamPowerLoss=downstreamPowerLoss, cmResetInfo=cmResetInfo, cardTestSchedule=cardTestSchedule, auditTotalFailedCount=auditTotalFailedCount, systemClock=systemClock, shelfSwVersion=shelfSwVersion, systemGeneralGroup=systemGeneralGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, systemKey=systemKey, cardDplxStatus=cardDplxStatus, cardCpuType=cardCpuType, cardSpareGroupMode=cardSpareGroupMode, portTxFlowControlMode=portTxFlowControlMode, cardPicDetected=cardPicDetected, cardLastResetReason=cardLastResetReason, auditCardId=auditCardId, cardLastChangeTime=cardLastChangeTime, equipmentCardGroup=equipmentCardGroup, portMinDsPower=portMinDsPower, equipmentTraps=equipmentTraps, portTable=portTable, portDplxStatus=portDplxStatus, auditPassedCount=auditPassedCount, auditCycleCount=auditCycleCount, portMaxDsPower=portMaxDsPower, cardNumPorts=cardNumPorts, cmIpAddress=cmIpAddress, equipmentTbl=equipmentTbl, portLastChangeTime=portLastChangeTime, cardCPLDVersion=cardCPLDVersion, auditTable=auditTable, portAction=portAction, cardTemperatureHighError=cardTemperatureHighError, cardTrapInh=cardTrapInh, diskVolumeTrapInh=diskVolumeTrapInh, auditLogThrottle=auditLogThrottle, eqDiagConfig=eqDiagConfig, cardTestTime=cardTestTime, cardRamSize=cardRamSize, consoleOutput=consoleOutput, auditResult=auditResult, cmUChannelID=cmUChannelID, equipmentError=equipmentError, cmResetClearNotification=cmResetClearNotification, cardSecStateChange=cardSecStateChange, cadEquipmentMib=cadEquipmentMib, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeTable=diskVolumeTable, cardName=cardName, diskDriveNumber=diskDriveNumber, shelfNumber=shelfNumber, portDplxStatusChange=portDplxStatusChange, cardTemperatureHighWarn=cardTemperatureHighWarn, cardFwVersion=cardFwVersion, portType=portType, cadEquipmentMibGroup=cadEquipmentMibGroup, TestScheduleCommand=TestScheduleCommand, auditTime=auditTime, fpgaErrorEventEntry=fpgaErrorEventEntry, portTrapInh=portTrapInh, diskVolumeDriveId=diskVolumeDriveId, diskVolumeId=diskVolumeId, diskVolumeName=diskVolumeName, equipmentStateGroup=equipmentStateGroup, diagTestId=diagTestId, portDetectedMode=portDetectedMode, portMacIfIndex=portMacIfIndex, shelfObjects=shelfObjects, portMacAddress=portMacAddress, cardPicHwVersion=cardPicHwVersion, portCurrDsPower=portCurrDsPower, equipmentAuditGroup=equipmentAuditGroup, cardShelfId=cardShelfId, cmDevice=cmDevice, cardBootVersion=cardBootVersion, cardCarrierFwVersion=cardCarrierFwVersion, portCardSubType=portCardSubType, cmVendor=cmVendor, cardShelfTestIndex=cardShelfTestIndex, errEvRowStatus=errEvRowStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, verboseLevel=verboseLevel, portId=portId, cmPrimarySID=cmPrimarySID, cardSubType=cardSubType, portShelfId=portShelfId, trapSeverity=trapSeverity, TestCommand=TestCommand, cardId=cardId)
