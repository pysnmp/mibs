...
PicType, DiskVolumeUsageLevel, CardId, CardSubType, EqActionType, PortMode, DuplexStatus, SecondaryState, AdminState, UnknownState, PortDetectedMode, CardType, PortType, ShelfId, PrimaryState, FirmwareSource, PortId, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "PicType", "DiskVolumeUsageLevel", "CardId", "CardSubType", "EqActionType", "PortMode", "DuplexStatus", "SecondaryState", "AdminState", "UnknownState", "PortDetectedMode", "CardType", "PortType", "ShelfId", "PrimaryState", "FirmwareSource", "PortId", "FlowControlMode")
...
DateAndTime, DisplayString, TimeStamp, MacAddress, RowStatus, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "DisplayString", "TimeStamp", "MacAddress", "RowStatus", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portEntry=portEntry, cardFpgaSource=cardFpgaSource, cardTable=cardTable, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeNumber=diskVolumeNumber, cardAction=cardAction, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardHwVersion=cardHwVersion, portPrState=portPrState, cardBusSpeed=cardBusSpeed, cardPicHwVersion=cardPicHwVersion, auditName=auditName, cardTestTime=cardTestTime, diskVolumeGroup=diskVolumeGroup, systemGeneral=systemGeneral, equipmentDiag=equipmentDiag, equipmentDiagGroup=equipmentDiagGroup, portShelfId=portShelfId, cadEquipmentMibConformance=cadEquipmentMibConformance, portCurrDsPower=portCurrDsPower, cardPrState=cardPrState, portRxFlowControlMode=portRxFlowControlMode, diskVolumeShelfId=diskVolumeShelfId, auditId=auditId, fpgaErrorEventEntry=fpgaErrorEventEntry, downstreamPowerLoss=downstreamPowerLoss, TestTransactionId=TestTransactionId, cardCarrierFwVersion=cardCarrierFwVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, cmDevice=cmDevice, diskVolumeTable=diskVolumeTable, cardType=cardType, portMacAddress=portMacAddress, diskVolumeDriveId=diskVolumeDriveId, portSecState=portSecState, portDplxStatus=portDplxStatus, diskVolumeName=diskVolumeName, cardTestId=cardTestId, shelfNumber=shelfNumber, cardFlashSize=cardFlashSize, shelfSwVersion=shelfSwVersion, auditTable=auditTable, cardCPLDVersion=cardCPLDVersion, cmResetUpTime=cmResetUpTime, errEventId=errEventId, cmResetClearNotification=cmResetClearNotification, equipmentPortGroup=equipmentPortGroup, portNumber=portNumber, cardCpuSpeed=cardCpuSpeed, portMacIfIndex=portMacIfIndex, cardTestType=cardTestType, cmResetNotification=cmResetNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, systemKey=systemKey, equipmentCardTestGroup=equipmentCardTestGroup, cardTestSchedule=cardTestSchedule, portCardId=portCardId, cmUChannelID=cmUChannelID, portBgpId=portBgpId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, errEvRowStatus=errEvRowStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsageNotification=diskVolumeUsageNotification, auditResult=auditResult, equipmentAudit=equipmentAudit, auditTotalPassedCount=auditTotalPassedCount, cmVendor=cmVendor, remainInDiagMode=remainInDiagMode, cardTemperature=cardTemperature, cardTemperatureHighWarn=cardTemperatureHighWarn, portMinDsPower=portMinDsPower, portType=portType, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResultDesc=cardTestResultDesc, cardSerialNum=cardSerialNum, cardSecStateChange=cardSecStateChange, cardTrapInh=cardTrapInh, auditCycleCount=auditCycleCount, TestCommand=TestCommand, cardShelfTestIndex=cardShelfTestIndex, systemGeneralGroup=systemGeneralGroup, cardSubDetected=cardSubDetected, fpgaErrorEventTable=fpgaErrorEventTable, portId=portId, cardName=cardName, systemClock=systemClock, cardTestName=cardTestName, portLastChangeTime=portLastChangeTime, cardBootVersion=cardBootVersion, cardDetected=cardDetected, PYSNMP_MODULE_ID=cadEquipmentMib, cardUpTime=cardUpTime, cmIpAddress=cmIpAddress, cardTestIndex=cardTestIndex, cardLastResetReason=cardLastResetReason, diskVolumeId=diskVolumeId, portTrapInh=portTrapInh, equipmentState=equipmentState, errEvLoggingEnabled=errEvLoggingEnabled, cardFwVersion=cardFwVersion, cardPicSerialNum=cardPicSerialNum, auditCardId=auditCardId, auditAutoScheduling=auditAutoScheduling, trapSeverity=trapSeverity, cardTestResultNotification=cardTestResultNotification, equipmentStateGroup=equipmentStateGroup, cmMacAddress=cmMacAddress, cardSpareGroupMode=cardSpareGroupMode, auditEntry=auditEntry, cardNumPorts=cardNumPorts, diagTestId=diagTestId, portDocsIfIndex=portDocsIfIndex, cardTestEntry=cardTestEntry, cardTempNoReportNotification=cardTempNoReportNotification, auditLogOutput=auditLogOutput, cardTestScheduleCommand=cardTestScheduleCommand, cardNumber=cardNumber, errEvLogLevel=errEvLogLevel, cardSpareGroupId=cardSpareGroupId, auditTotalFailedCount=auditTotalFailedCount, cardEntry=cardEntry, eqDiagConfig=eqDiagConfig, cmResetInfo=cmResetInfo, portAdminState=portAdminState, portConnectorId=portConnectorId, consoleOutput=consoleOutput, cadEquipmentMibGroup=cadEquipmentMibGroup, cardFwUpdateStatus=cardFwUpdateStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, cardDetectedChange=cardDetectedChange, equipmentTbl=equipmentTbl, cardDplxStatus=cardDplxStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmPrimarySID=cmPrimarySID, TestScheduleCommand=TestScheduleCommand, cardSwVersion=cardSwVersion, portMaxDsPower=portMaxDsPower, auditCommand=auditCommand, auditPassedCount=auditPassedCount, cardId=cardId, equipmentCardGroup=equipmentCardGroup, portCardSubType=portCardSubType, cardCpuType=cardCpuType, cmResetReason=cmResetReason, portMode=portMode, cardTemperatureHighError=cardTemperatureHighError, auditFailedCount=auditFailedCount, shelfName=shelfName, cardTestDescription=cardTestDescription, portNumChans=portNumChans, cardCarrierHwVersion=cardCarrierHwVersion, cardDplxStatusChange=cardDplxStatusChange, cardTempOverHeatNotification=cardTempOverHeatNotification, cadEquipmentMib=cadEquipmentMib, cmRegistrationNotification=cmRegistrationNotification, diskVolumeSize=diskVolumeSize, TestId=TestId, cardLastChangeTime=cardLastChangeTime, TestResult=TestResult, auditShelfId=auditShelfId, diskVolumeEntry=diskVolumeEntry, cardLastBootSource=cardLastBootSource, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestTransId=cardTestTransId, portDescription=portDescription, cardTestResult=cardTestResult, shelfObjects=shelfObjects, cmResetStatus=cmResetStatus, auditLogThrottle=auditLogThrottle, trapCounter=trapCounter, cardSecState=cardSecState, diskVolumeDiskSize=diskVolumeDiskSize, TestSchedule=TestSchedule, equipmentTraps=equipmentTraps, cardLastBootVersion=cardLastBootVersion, cardTestCommand=cardTestCommand, cardCarrierSerialNum=cardCarrierSerialNum, portAction=portAction, auditDescription=auditDescription, diskVolumeCardId=diskVolumeCardId, auditStatus=auditStatus, equipmentShelfGroup=equipmentShelfGroup, cardTestTable=cardTestTable, cardAdminState=cardAdminState, portDetectedMode=portDetectedMode, equipmentAuditGroup=equipmentAuditGroup, auditTime=auditTime, portPrStateChange=portPrStateChange, diskDriveNumber=diskDriveNumber, portTxFlowControlMode=portTxFlowControlMode, cardPrStateChange=cardPrStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portTable=portTable, diskVolumeTrapInh=diskVolumeTrapInh, verboseLevel=verboseLevel, portDplxStatusChange=portDplxStatusChange, portSecStateChange=portSecStateChange, cardRamSize=cardRamSize, equipmentError=equipmentError, cardShelfId=cardShelfId, cardSubType=cardSubType, TestType=TestType, cardPicDetected=cardPicDetected, diskVolumeFileName=diskVolumeFileName, portTxFlowControlDetected=portTxFlowControlDetected)
