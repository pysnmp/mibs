...
CardType, PortDetectedMode, FlowControlMode, SecondaryState, ShelfId, CardId, DiskVolumeUsageLevel, CardSubType, PortId, PortMode, EqActionType, AdminState, PortType, PrimaryState, UnknownState, DuplexStatus, FirmwareSource, PicType = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortDetectedMode", "FlowControlMode", "SecondaryState", "ShelfId", "CardId", "DiskVolumeUsageLevel", "CardSubType", "PortId", "PortMode", "EqActionType", "AdminState", "PortType", "PrimaryState", "UnknownState", "DuplexStatus", "FirmwareSource", "PicType")
...
TruthValue, RowStatus, DateAndTime, MacAddress, TimeStamp, TextualConvention, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "DateAndTime", "MacAddress", "TimeStamp", "TextualConvention", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCpuType=cardCpuType, cmVendor=cmVendor, cardAdminState=cardAdminState, diskVolumeDriveId=diskVolumeDriveId, cardDetectedChange=cardDetectedChange, cardDplxStatusChange=cardDplxStatusChange, eqDiagConfig=eqDiagConfig, cmResetClearNotification=cmResetClearNotification, portMacAddress=portMacAddress, TestType=TestType, errEvLogLevel=errEvLogLevel, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeDiskSize=diskVolumeDiskSize, cardRamSize=cardRamSize, cardTestIndex=cardTestIndex, cmMacAddress=cmMacAddress, errEventId=errEventId, portBgpId=portBgpId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSwVersion=cardSwVersion, cmResetStatus=cmResetStatus, portEntry=portEntry, cmResetReason=cmResetReason, portPrStateChange=portPrStateChange, errEvRowStatus=errEvRowStatus, portLastChangeTime=portLastChangeTime, auditPassedCount=auditPassedCount, fpgaErrorEventEntry=fpgaErrorEventEntry, portType=portType, auditTable=auditTable, diskVolumeCardId=diskVolumeCardId, cardTestCommand=cardTestCommand, equipmentError=equipmentError, equipmentCardTestGroup=equipmentCardTestGroup, cardType=cardType, TestResult=TestResult, cardPrStateChange=cardPrStateChange, cardCPLDVersion=cardCPLDVersion, cardBusSpeed=cardBusSpeed, remainInDiagMode=remainInDiagMode, portMaxDsPower=portMaxDsPower, auditCommand=auditCommand, portDescription=portDescription, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardHwVersion=cardHwVersion, portMode=portMode, systemGeneral=systemGeneral, cardTable=cardTable, diskVolumeGroup=diskVolumeGroup, shelfNumber=shelfNumber, cardLastBootSource=cardLastBootSource, cardDetected=cardDetected, cardSecStateChange=cardSecStateChange, TestScheduleCommand=TestScheduleCommand, systemClock=systemClock, cardTestType=cardTestType, portAction=portAction, auditCardId=auditCardId, equipmentTbl=equipmentTbl, portMacIfIndex=portMacIfIndex, auditTotalPassedCount=auditTotalPassedCount, cardTestResultDesc=cardTestResultDesc, cardTestResultNotification=cardTestResultNotification, cmResetNotification=cmResetNotification, shelfObjects=shelfObjects, trapSeverity=trapSeverity, auditLogThrottle=auditLogThrottle, cardTestTable=cardTestTable, auditDescription=auditDescription, cardTestEntry=cardTestEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardUpTime=cardUpTime, diagTestId=diagTestId, systemKey=systemKey, portDocsIfIndex=portDocsIfIndex, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeShelfId=diskVolumeShelfId, auditShelfId=auditShelfId, auditFailedCount=auditFailedCount, cmDevice=cmDevice, cardPicSerialNum=cardPicSerialNum, portNumber=portNumber, cardLastChangeTime=cardLastChangeTime, cardLastResetReason=cardLastResetReason, cmUChannelID=cmUChannelID, equipmentCardGroup=equipmentCardGroup, equipmentAuditGroup=equipmentAuditGroup, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditLogOutput=auditLogOutput, cmRegistrationNotification=cmRegistrationNotification, cardTestResult=cardTestResult, portCardSubType=portCardSubType, cardSecState=cardSecState, cardAction=cardAction, portId=portId, TestSchedule=TestSchedule, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentAudit=equipmentAudit, cmResetInfo=cmResetInfo, cardShelfTestIndex=cardShelfTestIndex, portSecState=portSecState, portMinDsPower=portMinDsPower, cardId=cardId, TestId=TestId, auditAutoScheduling=auditAutoScheduling, portDplxStatusChange=portDplxStatusChange, cardTrapInh=cardTrapInh, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestSchedule=cardTestSchedule, PYSNMP_MODULE_ID=cadEquipmentMib, portShelfId=portShelfId, equipmentDiag=equipmentDiag, cardPrState=cardPrState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portAdminState=portAdminState, cardTestTime=cardTestTime, diskVolumeEntry=diskVolumeEntry, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeId=diskVolumeId, portTxFlowControlDetected=portTxFlowControlDetected, cardCarrierHwVersion=cardCarrierHwVersion, TestCommand=TestCommand, portDetectedMode=portDetectedMode, systemGeneralGroup=systemGeneralGroup, portTable=portTable, equipmentShelfGroup=equipmentShelfGroup, cardTestTransId=cardTestTransId, portTrapInh=portTrapInh, auditTotalFailedCount=auditTotalFailedCount, cardBootVersion=cardBootVersion, consoleOutput=consoleOutput, cmPrimarySID=cmPrimarySID, cardTestId=cardTestId, auditEntry=auditEntry, cardDplxStatus=cardDplxStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditCycleCount=auditCycleCount, diskVolumeFileName=diskVolumeFileName, cardTestName=cardTestName, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeName=diskVolumeName, diskDriveNumber=diskDriveNumber, cadEquipmentMibGroup=cadEquipmentMibGroup, fpgaErrorEventTable=fpgaErrorEventTable, equipmentStateGroup=equipmentStateGroup, downstreamPowerLoss=downstreamPowerLoss, portCurrDsPower=portCurrDsPower, verboseLevel=verboseLevel, cardLastBootVersion=cardLastBootVersion, equipmentTraps=equipmentTraps, cardName=cardName, portTxFlowControlMode=portTxFlowControlMode, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeSize=diskVolumeSize, cardCpuSpeed=cardCpuSpeed, cardSubType=cardSubType, cardTestDescription=cardTestDescription, cardTemperatureHighError=cardTemperatureHighError, portNumChans=portNumChans, cmIpAddress=cmIpAddress, portSecStateChange=portSecStateChange, cardTemperature=cardTemperature, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCarrierSerialNum=cardCarrierSerialNum, shelfName=shelfName, auditId=auditId, auditResult=auditResult, diskVolumeTrapInh=diskVolumeTrapInh, cardShelfId=cardShelfId, shelfSwVersion=shelfSwVersion, cardFlashSize=cardFlashSize, portCardId=portCardId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupId=cardSpareGroupId, portPrState=portPrState, portDplxStatus=portDplxStatus, cardSpareGroupMode=cardSpareGroupMode, cardSubDetected=cardSubDetected, diskVolumeTable=diskVolumeTable, equipmentDiagGroup=equipmentDiagGroup, cardFpgaSource=cardFpgaSource, cardNumber=cardNumber, cmResetUpTime=cmResetUpTime, cardEntry=cardEntry, cardFwVersion=cardFwVersion, cardTempNoReportNotification=cardTempNoReportNotification, cardPicDetected=cardPicDetected, diskVolumeNumber=diskVolumeNumber, cardFwUpdateStatus=cardFwUpdateStatus, auditName=auditName, cadEquipmentMib=cadEquipmentMib, auditStatus=auditStatus, errEvLoggingEnabled=errEvLoggingEnabled, cardSerialNum=cardSerialNum, trapCounter=trapCounter, portRxFlowControlMode=portRxFlowControlMode, portConnectorId=portConnectorId, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentPortGroup=equipmentPortGroup, TestTransactionId=TestTransactionId, cardPicHwVersion=cardPicHwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentState=equipmentState, cadEquipmentMibConformance=cadEquipmentMibConformance, auditTime=auditTime)
