...
ShelfId, PrimaryState, DuplexStatus, PortType, PortMode, PicType, PortId, UnknownState, AdminState, PortDetectedMode, DiskVolumeUsageLevel, FirmwareSource, SecondaryState, CardId, CardType, FlowControlMode, EqActionType, CardSubType = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "PrimaryState", "DuplexStatus", "PortType", "PortMode", "PicType", "PortId", "UnknownState", "AdminState", "PortDetectedMode", "DiskVolumeUsageLevel", "FirmwareSource", "SecondaryState", "CardId", "CardType", "FlowControlMode", "EqActionType", "CardSubType")
...
MacAddress, TruthValue, RowStatus, DisplayString, DateAndTime, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "RowStatus", "DisplayString", "DateAndTime", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSubType=cardSubType, portDplxStatus=portDplxStatus, portShelfId=portShelfId, cardTestResultDesc=cardTestResultDesc, equipmentShelfGroup=equipmentShelfGroup, diskVolumeGroup=diskVolumeGroup, portMacAddress=portMacAddress, portAdminState=portAdminState, cardLastChangeTime=cardLastChangeTime, portMinDsPower=portMinDsPower, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardDetectedChange=cardDetectedChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, eqDiagConfig=eqDiagConfig, cardName=cardName, cardSecState=cardSecState, equipmentTbl=equipmentTbl, cadEquipmentMib=cadEquipmentMib, equipmentDiag=equipmentDiag, errEvRowStatus=errEvRowStatus, cmResetNotification=cmResetNotification, cardShelfTestIndex=cardShelfTestIndex, portPrState=portPrState, auditStatus=auditStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, systemKey=systemKey, cardAdminState=cardAdminState, cmMacAddress=cmMacAddress, cmResetClearNotification=cmResetClearNotification, auditId=auditId, cardTestTable=cardTestTable, cardFpgaSource=cardFpgaSource, portTxFlowControlMode=portTxFlowControlMode, cardCpuSpeed=cardCpuSpeed, cardTestName=cardTestName, equipmentPortGroup=equipmentPortGroup, cardTestSchedule=cardTestSchedule, cardPrStateChange=cardPrStateChange, fpgaErrorEventTable=fpgaErrorEventTable, portPrStateChange=portPrStateChange, cardTestEntry=cardTestEntry, cardSwVersion=cardSwVersion, cardFwUpdateStatus=cardFwUpdateStatus, portMacIfIndex=portMacIfIndex, cardTempNoReportNotification=cardTempNoReportNotification, cardDplxStatus=cardDplxStatus, portId=portId, cardLastResetReason=cardLastResetReason, cardPrState=cardPrState, diskVolumeName=diskVolumeName, remainInDiagMode=remainInDiagMode, cardRamSize=cardRamSize, cardType=cardType, cardPicSerialNum=cardPicSerialNum, portLastChangeTime=portLastChangeTime, portEntry=portEntry, auditShelfId=auditShelfId, cmRegistrationNotification=cmRegistrationNotification, portType=portType, TestResult=TestResult, diskDriveNumber=diskDriveNumber, shelfObjects=shelfObjects, cardTestType=cardTestType, shelfName=shelfName, cardCarrierFwVersion=cardCarrierFwVersion, cmVendor=cmVendor, portRxFlowControlDetected=portRxFlowControlDetected, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentAuditGroup=equipmentAuditGroup, equipmentTraps=equipmentTraps, cardTable=cardTable, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTime=cardTestTime, auditDescription=auditDescription, portMode=portMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestDescription=cardTestDescription, portBgpId=portBgpId, cardPicDetected=cardPicDetected, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestCommand=cardTestCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSubDetected=cardSubDetected, cardLastBootSource=cardLastBootSource, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSpareGroupMode=cardSpareGroupMode, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, downstreamPowerLoss=downstreamPowerLoss, cmResetReason=cmResetReason, cardTestId=cardTestId, auditTable=auditTable, cardCpuType=cardCpuType, cardSpareGroupId=cardSpareGroupId, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibCompliance=cadEquipmentMibCompliance, verboseLevel=verboseLevel, TestId=TestId, cardFlashSize=cardFlashSize, errEvLogLevel=errEvLogLevel, cardTrapInh=cardTrapInh, trapSeverity=trapSeverity, portDetectedMode=portDetectedMode, cardFwVersion=cardFwVersion, auditAutoScheduling=auditAutoScheduling, diskVolumeNumber=diskVolumeNumber, cardSecStateChange=cardSecStateChange, equipmentCardTestGroup=equipmentCardTestGroup, auditLogThrottle=auditLogThrottle, cardTestResult=cardTestResult, cardNumPorts=cardNumPorts, auditPassedCount=auditPassedCount, cardUpTime=cardUpTime, equipmentState=equipmentState, TestCommand=TestCommand, shelfSwVersion=shelfSwVersion, TestTransactionId=TestTransactionId, portTxFlowControlDetected=portTxFlowControlDetected, auditTime=auditTime, cardTestIndex=cardTestIndex, portCardSubType=portCardSubType, auditFailedCount=auditFailedCount, cardAction=cardAction, auditName=auditName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, consoleOutput=consoleOutput, cardTempOverHeatNotification=cardTempOverHeatNotification, portAction=portAction, cardLastBootVersion=cardLastBootVersion, errEventId=errEventId, cmResetUpTime=cmResetUpTime, cardNumber=cardNumber, cardPicHwVersion=cardPicHwVersion, diskVolumeFileName=diskVolumeFileName, auditTotalPassedCount=auditTotalPassedCount, TestSchedule=TestSchedule, cmResetStatus=cmResetStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestScheduleCommand=TestScheduleCommand, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeTable=diskVolumeTable, portMaxDsPower=portMaxDsPower, portSecState=portSecState, portDocsIfIndex=portDocsIfIndex, equipmentAudit=equipmentAudit, equipmentError=equipmentError, cardTestScheduleCommand=cardTestScheduleCommand, portNumChans=portNumChans, TestType=TestType, auditCycleCount=auditCycleCount, cmIpAddress=cmIpAddress, cardEntry=cardEntry, cardTestTransId=cardTestTransId, diskVolumeId=diskVolumeId, auditLogOutput=auditLogOutput, cardBusSpeed=cardBusSpeed, portTable=portTable, errEvLoggingEnabled=errEvLoggingEnabled, portNumber=portNumber, diskVolumeDriveId=diskVolumeDriveId, cardDplxStatusChange=cardDplxStatusChange, cardShelfId=cardShelfId, diskVolumeSize=diskVolumeSize, portTrapInh=portTrapInh, systemGeneral=systemGeneral, shelfNumber=shelfNumber, cardBootVersion=cardBootVersion, cardCPLDVersion=cardCPLDVersion, portDplxStatusChange=portDplxStatusChange, cardId=cardId, portDescription=portDescription, equipmentStateGroup=equipmentStateGroup, diagTestId=diagTestId, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResultNotification=cardTestResultNotification, portRxFlowControlMode=portRxFlowControlMode, portCardId=portCardId, diskVolumeTrapInh=diskVolumeTrapInh, auditCardId=auditCardId, cardTemperature=cardTemperature, cmDevice=cmDevice, equipmentDiagGroup=equipmentDiagGroup, cardHwVersion=cardHwVersion, cmPrimarySID=cmPrimarySID, diskVolumeCardId=diskVolumeCardId, diskVolumeEntry=diskVolumeEntry, auditCommand=auditCommand, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, systemClock=systemClock, portConnectorId=portConnectorId, equipmentCardGroup=equipmentCardGroup, auditEntry=auditEntry, cardTemperatureHighError=cardTemperatureHighError, cmUChannelID=cmUChannelID, cmResetInfo=cmResetInfo, diskVolumeShelfId=diskVolumeShelfId, cardSerialNum=cardSerialNum, portCurrDsPower=portCurrDsPower, auditResult=auditResult, systemGeneralGroup=systemGeneralGroup, portSecStateChange=portSecStateChange, diskVolumeDiskSize=diskVolumeDiskSize, cardDetected=cardDetected, trapCounter=trapCounter)
