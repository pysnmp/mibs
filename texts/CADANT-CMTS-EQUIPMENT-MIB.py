...
PrimaryState, FlowControlMode, UnknownState, ShelfId, FirmwareSource, PicType, PortDetectedMode, SecondaryState, PortMode, AdminState, CardSubType, EqActionType, CardId, PortId, DuplexStatus, PortType, CardType, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "FlowControlMode", "UnknownState", "ShelfId", "FirmwareSource", "PicType", "PortDetectedMode", "SecondaryState", "PortMode", "AdminState", "CardSubType", "EqActionType", "CardId", "PortId", "DuplexStatus", "PortType", "CardType", "DiskVolumeUsageLevel")
...
DisplayString, TimeStamp, TruthValue, RowStatus, MacAddress, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "TruthValue", "RowStatus", "MacAddress", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditStatus=auditStatus, cardTestResultDesc=cardTestResultDesc, equipmentDiag=equipmentDiag, equipmentStateGroup=equipmentStateGroup, cardDetected=cardDetected, cardTestSchedule=cardTestSchedule, diskVolumeShelfId=diskVolumeShelfId, portNumber=portNumber, cardTestIndex=cardTestIndex, cardCpuSpeed=cardCpuSpeed, cardFpgaSource=cardFpgaSource, cmPrimarySID=cmPrimarySID, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentPortGroup=equipmentPortGroup, auditId=auditId, cardTestResultNotification=cardTestResultNotification, auditTime=auditTime, portTable=portTable, portMaxDsPower=portMaxDsPower, shelfNumber=shelfNumber, consoleOutput=consoleOutput, cmResetNotification=cmResetNotification, cardPicHwVersion=cardPicHwVersion, portSecState=portSecState, auditEntry=auditEntry, auditShelfId=auditShelfId, trapCounter=trapCounter, TestCommand=TestCommand, portShelfId=portShelfId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSpareGroupMode=cardSpareGroupMode, shelfSwVersion=shelfSwVersion, cardSerialNum=cardSerialNum, cardSpareGroupId=cardSpareGroupId, diskVolumeDiskSize=diskVolumeDiskSize, portDplxStatusChange=portDplxStatusChange, equipmentError=equipmentError, equipmentShelfGroup=equipmentShelfGroup, cardCPLDVersion=cardCPLDVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portPrState=portPrState, shelfObjects=shelfObjects, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardHwVersion=cardHwVersion, auditResult=auditResult, portPrStateChange=portPrStateChange, cardTestName=cardTestName, cardCpuType=cardCpuType, cardTempNoReportNotification=cardTempNoReportNotification, portMacAddress=portMacAddress, diskVolumeTable=diskVolumeTable, cardLastBootVersion=cardLastBootVersion, portCardSubType=portCardSubType, cardName=cardName, auditLogThrottle=auditLogThrottle, fpgaErrorEventTable=fpgaErrorEventTable, diagTestId=diagTestId, cardNumPorts=cardNumPorts, cardNumber=cardNumber, cardId=cardId, auditFailedCount=auditFailedCount, auditPassedCount=auditPassedCount, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTemperature=cardTemperature, cardTestType=cardTestType, cardLastChangeTime=cardLastChangeTime, auditTotalFailedCount=auditTotalFailedCount, cardType=cardType, portTxFlowControlMode=portTxFlowControlMode, diskVolumeCardId=diskVolumeCardId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portRxFlowControlDetected=portRxFlowControlDetected, cmResetStatus=cmResetStatus, auditCardId=auditCardId, systemGeneral=systemGeneral, diskVolumeEntry=diskVolumeEntry, portId=portId, diskVolumeSize=diskVolumeSize, cardDplxStatusChange=cardDplxStatusChange, verboseLevel=verboseLevel, portMode=portMode, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeName=diskVolumeName, errEventId=errEventId, errEvRowStatus=errEvRowStatus, diskVolumeNumber=diskVolumeNumber, diskVolumeDriveId=diskVolumeDriveId, cardPrStateChange=cardPrStateChange, cardFwVersion=cardFwVersion, equipmentAudit=equipmentAudit, portMinDsPower=portMinDsPower, cardBootVersion=cardBootVersion, TestSchedule=TestSchedule, cardTemperatureHighWarn=cardTemperatureHighWarn, portBgpId=portBgpId, diskVolumeFileName=diskVolumeFileName, cardTestEntry=cardTestEntry, auditDescription=auditDescription, cardLastResetReason=cardLastResetReason, auditTable=auditTable, portRxFlowControlMode=portRxFlowControlMode, TestResult=TestResult, cardTestCommand=cardTestCommand, equipmentTraps=equipmentTraps, auditCommand=auditCommand, systemKey=systemKey, cardTemperatureHighError=cardTemperatureHighError, equipmentDiagGroup=equipmentDiagGroup, portCardId=portCardId, cardDplxStatus=cardDplxStatus, diskDriveNumber=diskDriveNumber, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeId=diskVolumeId, portTxFlowControlDetected=portTxFlowControlDetected, cardTestDescription=cardTestDescription, cardSubType=cardSubType, auditAutoScheduling=auditAutoScheduling, cardSwVersion=cardSwVersion, portTrapInh=portTrapInh, cardTestTime=cardTestTime, cmDevice=cmDevice, equipmentCardGroup=equipmentCardGroup, cardEntry=cardEntry, portConnectorId=portConnectorId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestResult=cardTestResult, cardFlashSize=cardFlashSize, TestScheduleCommand=TestScheduleCommand, cardSecState=cardSecState, TestType=TestType, cardPicDetected=cardPicDetected, cardRamSize=cardRamSize, portAdminState=portAdminState, portCurrDsPower=portCurrDsPower, cardUpTime=cardUpTime, cardBusSpeed=cardBusSpeed, cardTestTransId=cardTestTransId, cardAction=cardAction, portLastChangeTime=portLastChangeTime, cardTempOverHeatNotification=cardTempOverHeatNotification, portNumChans=portNumChans, cardTestId=cardTestId, cardSubDetected=cardSubDetected, cardPicSerialNum=cardPicSerialNum, cmMacAddress=cmMacAddress, fpgaErrorEventEntry=fpgaErrorEventEntry, portAction=portAction, errEvLogLevel=errEvLogLevel, cmIpAddress=cmIpAddress, auditCycleCount=auditCycleCount, cardTable=cardTable, equipmentAuditGroup=equipmentAuditGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentTbl=equipmentTbl, remainInDiagMode=remainInDiagMode, equipmentState=equipmentState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditLogOutput=auditLogOutput, eqDiagConfig=eqDiagConfig, portMacIfIndex=portMacIfIndex, trapSeverity=trapSeverity, cmResetUpTime=cmResetUpTime, auditName=auditName, cardTestScheduleCommand=cardTestScheduleCommand, TestTransactionId=TestTransactionId, cmUChannelID=cmUChannelID, TestId=TestId, cmResetClearNotification=cmResetClearNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portEntry=portEntry, cardFwUpdateStatus=cardFwUpdateStatus, cmRegistrationNotification=cmRegistrationNotification, cardShelfTestIndex=cardShelfTestIndex, cardDetectedChange=cardDetectedChange, portDocsIfIndex=portDocsIfIndex, portDplxStatus=portDplxStatus, cmResetReason=cmResetReason, portSecStateChange=portSecStateChange, errEvLoggingEnabled=errEvLoggingEnabled, cardLastBootSource=cardLastBootSource, shelfName=shelfName, cardTestTable=cardTestTable, cardTrapInh=cardTrapInh, portDescription=portDescription, systemGeneralGroup=systemGeneralGroup, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, downstreamPowerLoss=downstreamPowerLoss, portDetectedMode=portDetectedMode, diskVolumeUsageLevel=diskVolumeUsageLevel, cmVendor=cmVendor, cardSecStateChange=cardSecStateChange, portType=portType, cardShelfId=cardShelfId, systemClock=systemClock, auditTotalPassedCount=auditTotalPassedCount, cardAdminState=cardAdminState, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeTrapInh=diskVolumeTrapInh, cardPrState=cardPrState, cmResetInfo=cmResetInfo, cardCarrierSerialNum=cardCarrierSerialNum, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentCardTestGroup=equipmentCardTestGroup)
