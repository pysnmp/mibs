...
PortType, PortId, DuplexStatus, UnknownState, CardSubType, FlowControlMode, SecondaryState, PrimaryState, CardId, EqActionType, AdminState, PicType, ShelfId, CardType, DiskVolumeUsageLevel, FirmwareSource, PortMode, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "PortType", "PortId", "DuplexStatus", "UnknownState", "CardSubType", "FlowControlMode", "SecondaryState", "PrimaryState", "CardId", "EqActionType", "AdminState", "PicType", "ShelfId", "CardType", "DiskVolumeUsageLevel", "FirmwareSource", "PortMode", "PortDetectedMode")
...
TextualConvention, TimeStamp, DisplayString, MacAddress, DateAndTime, TruthValue, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TimeStamp", "DisplayString", "MacAddress", "DateAndTime", "TruthValue", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBootVersion=cardBootVersion, cardCpuType=cardCpuType, auditCycleCount=auditCycleCount, diskVolumeSize=diskVolumeSize, cardFwVersion=cardFwVersion, portAction=portAction, portNumber=portNumber, auditCardId=auditCardId, cardDetected=cardDetected, cmIpAddress=cmIpAddress, auditTime=auditTime, cardNumber=cardNumber, cardTestEntry=cardTestEntry, cmMacAddress=cmMacAddress, downstreamPowerLoss=downstreamPowerLoss, TestSchedule=TestSchedule, portMaxDsPower=portMaxDsPower, cardHwVersion=cardHwVersion, equipmentState=equipmentState, portDocsIfIndex=portDocsIfIndex, cardTestIndex=cardTestIndex, portTxFlowControlMode=portTxFlowControlMode, equipmentShelfGroup=equipmentShelfGroup, diskVolumeEntry=diskVolumeEntry, cardShelfId=cardShelfId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditAutoScheduling=auditAutoScheduling, portShelfId=portShelfId, PYSNMP_MODULE_ID=cadEquipmentMib, cardTrapInh=cardTrapInh, cardDplxStatusChange=cardDplxStatusChange, portMode=portMode, diskVolumeId=diskVolumeId, cardLastChangeTime=cardLastChangeTime, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCarrierFwVersion=cardCarrierFwVersion, auditEntry=auditEntry, equipmentError=equipmentError, diskVolumeUsageNotification=diskVolumeUsageNotification, systemKey=systemKey, portAdminState=portAdminState, portCardSubType=portCardSubType, portBgpId=portBgpId, auditFailedCount=auditFailedCount, diskVolumeDiskSize=diskVolumeDiskSize, cardTestTable=cardTestTable, cardTemperatureHighError=cardTemperatureHighError, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestType=cardTestType, cmRegistrationNotification=cmRegistrationNotification, cardUpTime=cardUpTime, errEventId=errEventId, portId=portId, cardCarrierHwVersion=cardCarrierHwVersion, auditTable=auditTable, portType=portType, cardPrState=cardPrState, cmResetStatus=cmResetStatus, cardSpareGroupMode=cardSpareGroupMode, portNumChans=portNumChans, cardTestTransId=cardTestTransId, equipmentAuditGroup=equipmentAuditGroup, cmResetNotification=cmResetNotification, diskVolumeTrapInh=diskVolumeTrapInh, TestId=TestId, diskVolumeTable=diskVolumeTable, auditShelfId=auditShelfId, trapCounter=trapCounter, portRxFlowControlDetected=portRxFlowControlDetected, cardPicSerialNum=cardPicSerialNum, diskVolumeName=diskVolumeName, portPrStateChange=portPrStateChange, cardShelfTestIndex=cardShelfTestIndex, cardTestResultDesc=cardTestResultDesc, cardCPLDVersion=cardCPLDVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTempNoReportNotification=cardTempNoReportNotification, systemGeneral=systemGeneral, diskVolumeUsageLevel=diskVolumeUsageLevel, cmUChannelID=cmUChannelID, cardNumPorts=cardNumPorts, cardTestName=cardTestName, cmDevice=cmDevice, consoleOutput=consoleOutput, remainInDiagMode=remainInDiagMode, cardTestCommand=cardTestCommand, cardPicDetected=cardPicDetected, auditName=auditName, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestTime=cardTestTime, shelfName=shelfName, auditLogThrottle=auditLogThrottle, diskVolumeGroup=diskVolumeGroup, cadEquipmentMib=cadEquipmentMib, auditId=auditId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSubType=cardSubType, cardFlashSize=cardFlashSize, portMacAddress=portMacAddress, portLastChangeTime=portLastChangeTime, cardEntry=cardEntry, cmResetClearNotification=cmResetClearNotification, diskDriveNumber=diskDriveNumber, TestScheduleCommand=TestScheduleCommand, diskVolumeShelfId=diskVolumeShelfId, equipmentAudit=equipmentAudit, TestResult=TestResult, portSecStateChange=portSecStateChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetUpTime=cmResetUpTime, cardTable=cardTable, cadEquipmentMibConformance=cadEquipmentMibConformance, auditLogOutput=auditLogOutput, cardBusSpeed=cardBusSpeed, eqDiagConfig=eqDiagConfig, equipmentDiagGroup=equipmentDiagGroup, cmResetInfo=cmResetInfo, cardLastResetReason=cardLastResetReason, portCurrDsPower=portCurrDsPower, portDescription=portDescription, cardPrStateChange=cardPrStateChange, systemGeneralGroup=systemGeneralGroup, shelfNumber=shelfNumber, verboseLevel=verboseLevel, errEvLoggingEnabled=errEvLoggingEnabled, cmResetReason=cmResetReason, equipmentTbl=equipmentTbl, fpgaErrorEventEntry=fpgaErrorEventEntry, portTable=portTable, trapSeverity=trapSeverity, equipmentPortGroup=equipmentPortGroup, portMacIfIndex=portMacIfIndex, cardSecState=cardSecState, diskVolumeFileName=diskVolumeFileName, cardAdminState=cardAdminState, shelfObjects=shelfObjects, cardSwVersion=cardSwVersion, cardAction=cardAction, portMinDsPower=portMinDsPower, portRxFlowControlMode=portRxFlowControlMode, shelfSwVersion=shelfSwVersion, cardFpgaSource=cardFpgaSource, cardTestScheduleCommand=cardTestScheduleCommand, portConnectorId=portConnectorId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentCardTestGroup=equipmentCardTestGroup, auditDescription=auditDescription, portEntry=portEntry, cardTestDescription=cardTestDescription, cardTestId=cardTestId, errEvLogLevel=errEvLogLevel, diskVolumeDriveId=diskVolumeDriveId, TestCommand=TestCommand, auditResult=auditResult, cmPrimarySID=cmPrimarySID, cardTestResult=cardTestResult, cardRamSize=cardRamSize, diskVolumeCardId=diskVolumeCardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSpareGroupId=cardSpareGroupId, diagTestId=diagTestId, cardDplxStatus=cardDplxStatus, cardType=cardType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditCommand=auditCommand, cardCarrierSerialNum=cardCarrierSerialNum, auditStatus=auditStatus, auditPassedCount=auditPassedCount, equipmentStateGroup=equipmentStateGroup, equipmentCardGroup=equipmentCardGroup, portDplxStatusChange=portDplxStatusChange, cardPicHwVersion=cardPicHwVersion, cardCpuSpeed=cardCpuSpeed, cardSubDetected=cardSubDetected, portDplxStatus=portDplxStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSerialNum=cardSerialNum, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeNumber=diskVolumeNumber, TestType=TestType, errEvRowStatus=errEvRowStatus, cardLastBootSource=cardLastBootSource, cardName=cardName, equipmentTraps=equipmentTraps, portTxFlowControlDetected=portTxFlowControlDetected, portDetectedMode=portDetectedMode, TestTransactionId=TestTransactionId, portSecState=portSecState, systemClock=systemClock, portCardId=portCardId, fpgaErrorEventTable=fpgaErrorEventTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardLastBootVersion=cardLastBootVersion, auditTotalFailedCount=auditTotalFailedCount, portTrapInh=portTrapInh, cardTestResultNotification=cardTestResultNotification, cardId=cardId, cardFwUpdateStatus=cardFwUpdateStatus, cmVendor=cmVendor, cardSecStateChange=cardSecStateChange, equipmentDiag=equipmentDiag, auditTotalPassedCount=auditTotalPassedCount, cardDetectedChange=cardDetectedChange, portPrState=portPrState, cardTemperature=cardTemperature, cardTestSchedule=cardTestSchedule)
