...
DuplexStatus, PortId, PortDetectedMode, PortType, UnknownState, SecondaryState, CardType, FlowControlMode, AdminState, FirmwareSource, CardId, PicType, EqActionType, CardSubType, PortMode, ShelfId, DiskVolumeUsageLevel, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortId", "PortDetectedMode", "PortType", "UnknownState", "SecondaryState", "CardType", "FlowControlMode", "AdminState", "FirmwareSource", "CardId", "PicType", "EqActionType", "CardSubType", "PortMode", "ShelfId", "DiskVolumeUsageLevel", "PrimaryState")
...
DateAndTime, TimeStamp, TruthValue, DisplayString, TextualConvention, RowStatus, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TimeStamp", "TruthValue", "DisplayString", "TextualConvention", "RowStatus", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTotalFailedCount=auditTotalFailedCount, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portCardSubType=portCardSubType, systemGeneral=systemGeneral, portShelfId=portShelfId, cardTrapInh=cardTrapInh, diskVolumeDriveId=diskVolumeDriveId, cardNumPorts=cardNumPorts, cardTestDescription=cardTestDescription, cardShelfId=cardShelfId, diskVolumeShelfId=diskVolumeShelfId, cardTestCommand=cardTestCommand, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTable=auditTable, portLastChangeTime=portLastChangeTime, diagTestId=diagTestId, cardSecState=cardSecState, cardAction=cardAction, portMacAddress=portMacAddress, portNumber=portNumber, TestType=TestType, cmResetClearNotification=cmResetClearNotification, cardTestSchedule=cardTestSchedule, portPrState=portPrState, cardCpuSpeed=cardCpuSpeed, equipmentAudit=equipmentAudit, diskVolumeNumber=diskVolumeNumber, cardTempNoReportNotification=cardTempNoReportNotification, portMaxDsPower=portMaxDsPower, portMacIfIndex=portMacIfIndex, auditId=auditId, equipmentDiagGroup=equipmentDiagGroup, cardLastResetReason=cardLastResetReason, trapSeverity=trapSeverity, cardDetected=cardDetected, diskDriveNumber=diskDriveNumber, portNumChans=portNumChans, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSubDetected=cardSubDetected, cardDplxStatus=cardDplxStatus, cardTestEntry=cardTestEntry, cardFlashSize=cardFlashSize, diskVolumeSize=diskVolumeSize, diskVolumeCardId=diskVolumeCardId, diskVolumeFileName=diskVolumeFileName, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicHwVersion=cardPicHwVersion, portDocsIfIndex=portDocsIfIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentAuditGroup=equipmentAuditGroup, TestId=TestId, cmMacAddress=cmMacAddress, cardSpareGroupMode=cardSpareGroupMode, portBgpId=portBgpId, consoleOutput=consoleOutput, shelfNumber=shelfNumber, systemClock=systemClock, cardTestTable=cardTestTable, shelfObjects=shelfObjects, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSpareGroupId=cardSpareGroupId, equipmentTbl=equipmentTbl, portMinDsPower=portMinDsPower, portMode=portMode, cmRegistrationNotification=cmRegistrationNotification, cardCarrierFwVersion=cardCarrierFwVersion, cmUChannelID=cmUChannelID, portDetectedMode=portDetectedMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, downstreamPowerLoss=downstreamPowerLoss, errEventId=errEventId, cardTable=cardTable, diskVolumeTable=diskVolumeTable, equipmentDiag=equipmentDiag, portAction=portAction, cardTempOverHeatNotification=cardTempOverHeatNotification, errEvLogLevel=errEvLogLevel, diskVolumeGroup=diskVolumeGroup, cardSerialNum=cardSerialNum, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditFailedCount=auditFailedCount, errEvRowStatus=errEvRowStatus, auditLogThrottle=auditLogThrottle, portRxFlowControlMode=portRxFlowControlMode, cardSecStateChange=cardSecStateChange, portCardId=portCardId, equipmentStateGroup=equipmentStateGroup, cadEquipmentMib=cadEquipmentMib, cardCpuType=cardCpuType, cardTemperature=cardTemperature, equipmentCardTestGroup=equipmentCardTestGroup, cardPicSerialNum=cardPicSerialNum, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, TestScheduleCommand=TestScheduleCommand, cmIpAddress=cmIpAddress, portId=portId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestSchedule=TestSchedule, auditShelfId=auditShelfId, auditName=auditName, portSecStateChange=portSecStateChange, portEntry=portEntry, TestResult=TestResult, diskVolumeEntry=diskVolumeEntry, cardTestResult=cardTestResult, verboseLevel=verboseLevel, diskVolumeDiskSize=diskVolumeDiskSize, portType=portType, diskVolumeId=diskVolumeId, equipmentShelfGroup=equipmentShelfGroup, auditStatus=auditStatus, cardTestName=cardTestName, portTrapInh=portTrapInh, auditTotalPassedCount=auditTotalPassedCount, fpgaErrorEventGroup=fpgaErrorEventGroup, portDplxStatusChange=portDplxStatusChange, cardTestScheduleCommand=cardTestScheduleCommand, cardType=cardType, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTrapInh=diskVolumeTrapInh, cardTestTransId=cardTestTransId, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, portCurrDsPower=portCurrDsPower, eqDiagConfig=eqDiagConfig, cardTestTime=cardTestTime, portSecState=portSecState, cardRamSize=cardRamSize, auditAutoScheduling=auditAutoScheduling, errEvLoggingEnabled=errEvLoggingEnabled, shelfName=shelfName, cardBusSpeed=cardBusSpeed, cardDetectedChange=cardDetectedChange, auditLogOutput=auditLogOutput, portPrStateChange=portPrStateChange, portAdminState=portAdminState, cardName=cardName, auditPassedCount=auditPassedCount, cmResetUpTime=cmResetUpTime, cardTestType=cardTestType, TestTransactionId=TestTransactionId, cardCarrierSerialNum=cardCarrierSerialNum, cardTemperatureHighError=cardTemperatureHighError, cmPrimarySID=cmPrimarySID, cardUpTime=cardUpTime, cardPrStateChange=cardPrStateChange, shelfSwVersion=shelfSwVersion, portConnectorId=portConnectorId, cardBootVersion=cardBootVersion, equipmentCardGroup=equipmentCardGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastBootVersion=cardLastBootVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cmVendor=cmVendor, auditCommand=auditCommand, cmResetReason=cmResetReason, cardShelfTestIndex=cardShelfTestIndex, cardAdminState=cardAdminState, cardPrState=cardPrState, cardDplxStatusChange=cardDplxStatusChange, cardFpgaSource=cardFpgaSource, cardTestResultDesc=cardTestResultDesc, auditEntry=auditEntry, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastBootSource=cardLastBootSource, portDescription=portDescription, cmDevice=cmDevice, cardSwVersion=cardSwVersion, portTable=portTable, portDplxStatus=portDplxStatus, cardFwVersion=cardFwVersion, remainInDiagMode=remainInDiagMode, cardFwUpdateStatus=cardFwUpdateStatus, cardId=cardId, cmResetNotification=cmResetNotification, auditResult=auditResult, equipmentError=equipmentError, diskVolumeName=diskVolumeName, systemGeneralGroup=systemGeneralGroup, cardLastChangeTime=cardLastChangeTime, cardSubType=cardSubType, cardCPLDVersion=cardCPLDVersion, portRxFlowControlDetected=portRxFlowControlDetected, auditDescription=auditDescription, TestCommand=TestCommand, cardEntry=cardEntry, cardTestId=cardTestId, cardCarrierHwVersion=cardCarrierHwVersion, cardHwVersion=cardHwVersion, equipmentTraps=equipmentTraps, equipmentState=equipmentState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardNumber=cardNumber, systemKey=systemKey, cmResetStatus=cmResetStatus, auditTime=auditTime, trapCounter=trapCounter, PYSNMP_MODULE_ID=cadEquipmentMib, cardPicDetected=cardPicDetected, auditCycleCount=auditCycleCount, cmResetInfo=cmResetInfo, auditCardId=auditCardId, cardTestIndex=cardTestIndex)
