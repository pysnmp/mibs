...
ShelfId, UnknownState, DuplexStatus, PortDetectedMode, CardId, PicType, EqActionType, PortId, FlowControlMode, SecondaryState, FirmwareSource, PortType, PortMode, PrimaryState, AdminState, CardType, DiskVolumeUsageLevel, CardSubType = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "UnknownState", "DuplexStatus", "PortDetectedMode", "CardId", "PicType", "EqActionType", "PortId", "FlowControlMode", "SecondaryState", "FirmwareSource", "PortType", "PortMode", "PrimaryState", "AdminState", "CardType", "DiskVolumeUsageLevel", "CardSubType")
...
TextualConvention, TruthValue, DateAndTime, DisplayString, MacAddress, TimeStamp, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "DateAndTime", "DisplayString", "MacAddress", "TimeStamp", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfSwVersion=shelfSwVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestType=cardTestType, diskVolumeId=diskVolumeId, auditName=auditName, cmResetInfo=cmResetInfo, TestScheduleCommand=TestScheduleCommand, cardCarrierHwVersion=cardCarrierHwVersion, portId=portId, auditTotalPassedCount=auditTotalPassedCount, TestType=TestType, cardPicHwVersion=cardPicHwVersion, cardDetectedChange=cardDetectedChange, cardSpareGroupMode=cardSpareGroupMode, cardTempOverHeatNotification=cardTempOverHeatNotification, cmDevice=cmDevice, diskVolumeDiskSize=diskVolumeDiskSize, errEvRowStatus=errEvRowStatus, cardHwVersion=cardHwVersion, portMacAddress=portMacAddress, cardShelfId=cardShelfId, portNumber=portNumber, cardDplxStatus=cardDplxStatus, diskDriveNumber=diskDriveNumber, cardCarrierFwVersion=cardCarrierFwVersion, cardTestTime=cardTestTime, diskVolumeNumber=diskVolumeNumber, cmResetReason=cmResetReason, equipmentShelfGroup=equipmentShelfGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCpuSpeed=cardCpuSpeed, remainInDiagMode=remainInDiagMode, errEvLogLevel=errEvLogLevel, equipmentError=equipmentError, cardPicDetected=cardPicDetected, portTrapInh=portTrapInh, cardSecStateChange=cardSecStateChange, PYSNMP_MODULE_ID=cadEquipmentMib, fpgaErrorEventTable=fpgaErrorEventTable, cardType=cardType, auditCycleCount=auditCycleCount, cardTestScheduleCommand=cardTestScheduleCommand, auditLogOutput=auditLogOutput, cardName=cardName, fpgaErrorEventGroup=fpgaErrorEventGroup, auditDescription=auditDescription, auditTotalFailedCount=auditTotalFailedCount, TestSchedule=TestSchedule, cardTestSchedule=cardTestSchedule, portDplxStatusChange=portDplxStatusChange, auditFailedCount=auditFailedCount, diskVolumeGroup=diskVolumeGroup, portType=portType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardBootVersion=cardBootVersion, cardRamSize=cardRamSize, TestCommand=TestCommand, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTrapInh=cardTrapInh, portCardId=portCardId, auditPassedCount=auditPassedCount, portEntry=portEntry, equipmentTbl=equipmentTbl, shelfNumber=shelfNumber, auditCardId=auditCardId, portDescription=portDescription, portDocsIfIndex=portDocsIfIndex, cardLastChangeTime=cardLastChangeTime, cardCPLDVersion=cardCPLDVersion, cardPrStateChange=cardPrStateChange, cardTestEntry=cardTestEntry, cardFpgaSource=cardFpgaSource, diskVolumeDriveId=diskVolumeDriveId, cardAdminState=cardAdminState, auditAutoScheduling=auditAutoScheduling, portShelfId=portShelfId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, fpgaErrorEventEntry=fpgaErrorEventEntry, portSecStateChange=portSecStateChange, eqDiagConfig=eqDiagConfig, portCurrDsPower=portCurrDsPower, cmRegistrationNotification=cmRegistrationNotification, cardNumPorts=cardNumPorts, equipmentTraps=equipmentTraps, diskVolumeSize=diskVolumeSize, diskVolumeShelfId=diskVolumeShelfId, cmIpAddress=cmIpAddress, portDplxStatus=portDplxStatus, portMinDsPower=portMinDsPower, cardCpuType=cardCpuType, cardAction=cardAction, trapSeverity=trapSeverity, portSecState=portSecState, auditCommand=auditCommand, errEventId=errEventId, cardEntry=cardEntry, cardSwVersion=cardSwVersion, systemKey=systemKey, diskVolumeTable=diskVolumeTable, systemClock=systemClock, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentAudit=equipmentAudit, auditShelfId=auditShelfId, cmResetUpTime=cmResetUpTime, cmResetNotification=cmResetNotification, diskVolumeFileName=diskVolumeFileName, diskVolumeName=diskVolumeName, verboseLevel=verboseLevel, downstreamPowerLoss=downstreamPowerLoss, cmResetClearNotification=cmResetClearNotification, cardTestTable=cardTestTable, equipmentStateGroup=equipmentStateGroup, TestId=TestId, equipmentDiagGroup=equipmentDiagGroup, cardNumber=cardNumber, systemGeneral=systemGeneral, auditStatus=auditStatus, cardTable=cardTable, cardTemperature=cardTemperature, portNumChans=portNumChans, portBgpId=portBgpId, diskVolumeTrapInh=diskVolumeTrapInh, shelfObjects=shelfObjects, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portRxFlowControlMode=portRxFlowControlMode, cardShelfTestIndex=cardShelfTestIndex, auditId=auditId, cardTestName=cardTestName, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicSerialNum=cardPicSerialNum, portAdminState=portAdminState, cardTestResultDesc=cardTestResultDesc, errEvLoggingEnabled=errEvLoggingEnabled, cardLastResetReason=cardLastResetReason, cardCarrierSerialNum=cardCarrierSerialNum, auditTime=auditTime, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeCardId=diskVolumeCardId, cardTestId=cardTestId, cardTestIndex=cardTestIndex, cardSerialNum=cardSerialNum, cardTemperatureHighWarn=cardTemperatureHighWarn, auditTable=auditTable, cardDetected=cardDetected, portMacIfIndex=portMacIfIndex, diskVolumeUsageLevel=diskVolumeUsageLevel, portMode=portMode, diskVolumeEntry=diskVolumeEntry, cardTestCommand=cardTestCommand, cardSubDetected=cardSubDetected, cardDplxStatusChange=cardDplxStatusChange, cardBusSpeed=cardBusSpeed, cardTestResultNotification=cardTestResultNotification, cardLastBootSource=cardLastBootSource, systemGeneralGroup=systemGeneralGroup, cmUChannelID=cmUChannelID, cardTempNoReportNotification=cardTempNoReportNotification, equipmentPortGroup=equipmentPortGroup, portPrState=portPrState, diskVolumeUsageNotification=diskVolumeUsageNotification, portTxFlowControlMode=portTxFlowControlMode, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResult=cardTestResult, cadEquipmentMib=cadEquipmentMib, equipmentAuditGroup=equipmentAuditGroup, equipmentState=equipmentState, auditResult=auditResult, cmResetStatus=cmResetStatus, TestTransactionId=TestTransactionId, cardTestTransId=cardTestTransId, cmPrimarySID=cmPrimarySID, cardTemperatureHighError=cardTemperatureHighError, cardFlashSize=cardFlashSize, cardSubType=cardSubType, cardId=cardId, portMaxDsPower=portMaxDsPower, portPrStateChange=portPrStateChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestDescription=cardTestDescription, cardFwVersion=cardFwVersion, cardUpTime=cardUpTime, shelfName=shelfName, trapCounter=trapCounter, portAction=portAction, auditLogThrottle=auditLogThrottle, cmVendor=cmVendor, portDetectedMode=portDetectedMode, consoleOutput=consoleOutput, equipmentCardGroup=equipmentCardGroup, cardFwUpdateStatus=cardFwUpdateStatus, portTable=portTable, portCardSubType=portCardSubType, equipmentDiag=equipmentDiag, TestResult=TestResult, portLastChangeTime=portLastChangeTime, diagTestId=diagTestId, cardLastBootVersion=cardLastBootVersion, cardSecState=cardSecState, portConnectorId=portConnectorId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditEntry=auditEntry, cardSpareGroupId=cardSpareGroupId, cardPrState=cardPrState, portTxFlowControlDetected=portTxFlowControlDetected, cmMacAddress=cmMacAddress)
