...
CardSubType, SecondaryState, PrimaryState, UnknownState, FlowControlMode, CardType, PicType, DiskVolumeUsageLevel, CardId, EqActionType, PortMode, PortType, PortDetectedMode, AdminState, PortId, DuplexStatus, FirmwareSource, ShelfId = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "SecondaryState", "PrimaryState", "UnknownState", "FlowControlMode", "CardType", "PicType", "DiskVolumeUsageLevel", "CardId", "EqActionType", "PortMode", "PortType", "PortDetectedMode", "AdminState", "PortId", "DuplexStatus", "FirmwareSource", "ShelfId")
...
TextualConvention, MacAddress, DateAndTime, TruthValue, TimeStamp, DisplayString, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "DateAndTime", "TruthValue", "TimeStamp", "DisplayString", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeDriveId=diskVolumeDriveId, cardSwVersion=cardSwVersion, equipmentCardGroup=equipmentCardGroup, cardAction=cardAction, cardTestDescription=cardTestDescription, TestId=TestId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardFpgaSource=cardFpgaSource, portCardId=portCardId, portDescription=portDescription, auditShelfId=auditShelfId, portNumChans=portNumChans, trapCounter=trapCounter, fpgaErrorEventEntry=fpgaErrorEventEntry, cmResetClearNotification=cmResetClearNotification, trapSeverity=trapSeverity, cardDplxStatusChange=cardDplxStatusChange, shelfNumber=shelfNumber, cardBusSpeed=cardBusSpeed, diskVolumeName=diskVolumeName, cardTestEntry=cardTestEntry, portDocsIfIndex=portDocsIfIndex, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestType=TestType, portTxFlowControlDetected=portTxFlowControlDetected, portDplxStatusChange=portDplxStatusChange, cardHwVersion=cardHwVersion, cardNumber=cardNumber, cardTestScheduleCommand=cardTestScheduleCommand, systemKey=systemKey, portTxFlowControlMode=portTxFlowControlMode, diskVolumeCardId=diskVolumeCardId, portAdminState=portAdminState, auditTotalPassedCount=auditTotalPassedCount, cmDevice=cmDevice, cardTestResult=cardTestResult, cardId=cardId, errEvLoggingEnabled=errEvLoggingEnabled, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeTable=diskVolumeTable, diskVolumeFileName=diskVolumeFileName, cardTestIndex=cardTestIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSubDetected=cardSubDetected, auditTotalFailedCount=auditTotalFailedCount, cardLastBootVersion=cardLastBootVersion, equipmentDiagGroup=equipmentDiagGroup, diskVolumeNumber=diskVolumeNumber, cardAdminState=cardAdminState, cardTemperatureHighError=cardTemperatureHighError, TestSchedule=TestSchedule, auditPassedCount=auditPassedCount, TestScheduleCommand=TestScheduleCommand, cardTestTable=cardTestTable, portRxFlowControlDetected=portRxFlowControlDetected, equipmentAudit=equipmentAudit, cardPrState=cardPrState, cardLastResetReason=cardLastResetReason, portConnectorId=portConnectorId, portRxFlowControlMode=portRxFlowControlMode, portShelfId=portShelfId, portMacAddress=portMacAddress, auditLogOutput=auditLogOutput, TestResult=TestResult, cardLastBootSource=cardLastBootSource, cmResetReason=cmResetReason, portMode=portMode, shelfObjects=shelfObjects, fpgaErrorEventTable=fpgaErrorEventTable, portDetectedMode=portDetectedMode, auditFailedCount=auditFailedCount, portCardSubType=portCardSubType, equipmentDiag=equipmentDiag, cardTestTransId=cardTestTransId, cardBootVersion=cardBootVersion, errEventId=errEventId, cardCarrierSerialNum=cardCarrierSerialNum, cardCpuType=cardCpuType, equipmentTraps=equipmentTraps, errEvRowStatus=errEvRowStatus, cardFwVersion=cardFwVersion, diskVolumeId=diskVolumeId, cmResetStatus=cmResetStatus, cardCPLDVersion=cardCPLDVersion, cardDplxStatus=cardDplxStatus, portPrStateChange=portPrStateChange, cardCpuSpeed=cardCpuSpeed, auditStatus=auditStatus, auditDescription=auditDescription, portMinDsPower=portMinDsPower, portAction=portAction, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeTrapInh=diskVolumeTrapInh, equipmentShelfGroup=equipmentShelfGroup, portLastChangeTime=portLastChangeTime, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditEntry=auditEntry, shelfSwVersion=shelfSwVersion, systemGeneralGroup=systemGeneralGroup, eqDiagConfig=eqDiagConfig, TestCommand=TestCommand, cmResetInfo=cmResetInfo, shelfName=shelfName, portEntry=portEntry, cardPrStateChange=cardPrStateChange, cmRegistrationNotification=cmRegistrationNotification, portDplxStatus=portDplxStatus, equipmentAuditGroup=equipmentAuditGroup, cardTestTime=cardTestTime, cardShelfTestIndex=cardShelfTestIndex, cardCarrierFwVersion=cardCarrierFwVersion, equipmentCardTestGroup=equipmentCardTestGroup, cardUpTime=cardUpTime, diagTestId=diagTestId, cardTestSchedule=cardTestSchedule, cardName=cardName, cardShelfId=cardShelfId, auditCardId=auditCardId, remainInDiagMode=remainInDiagMode, portCurrDsPower=portCurrDsPower, cardTable=cardTable, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeEntry=diskVolumeEntry, cmMacAddress=cmMacAddress, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemGeneral=systemGeneral, portNumber=portNumber, equipmentState=equipmentState, cardFlashSize=cardFlashSize, cardPicSerialNum=cardPicSerialNum, cmResetNotification=cmResetNotification, systemClock=systemClock, cmIpAddress=cmIpAddress, cardTrapInh=cardTrapInh, consoleOutput=consoleOutput, cardTestId=cardTestId, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentStateGroup=equipmentStateGroup, cardTestResultDesc=cardTestResultDesc, auditResult=auditResult, cardSpareGroupId=cardSpareGroupId, cardPicDetected=cardPicDetected, verboseLevel=verboseLevel, cardSecStateChange=cardSecStateChange, portMacIfIndex=portMacIfIndex, downstreamPowerLoss=downstreamPowerLoss, cmUChannelID=cmUChannelID, diskVolumeUsageNotification=diskVolumeUsageNotification, portSecStateChange=portSecStateChange, equipmentTbl=equipmentTbl, cardTempNoReportNotification=cardTempNoReportNotification, portId=portId, fpgaErrorEventGroup=fpgaErrorEventGroup, cmVendor=cmVendor, cardNumPorts=cardNumPorts, auditId=auditId, cardDetected=cardDetected, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditName=auditName, cardTestName=cardTestName, errEvLogLevel=errEvLogLevel, portSecState=portSecState, auditAutoScheduling=auditAutoScheduling, cardTemperatureHighWarn=cardTemperatureHighWarn, cardRamSize=cardRamSize, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeSize=diskVolumeSize, diskVolumeShelfId=diskVolumeShelfId, diskVolumeDiskSize=diskVolumeDiskSize, cardSerialNum=cardSerialNum, auditTime=auditTime, cardTestCommand=cardTestCommand, cardDetectedChange=cardDetectedChange, portType=portType, cardSubType=cardSubType, auditCycleCount=auditCycleCount, cardSecState=cardSecState, cardTestType=cardTestType, portPrState=portPrState, portTrapInh=portTrapInh, cardSpareGroupMode=cardSpareGroupMode, cardLastChangeTime=cardLastChangeTime, cadEquipmentMibConformance=cadEquipmentMibConformance, portTable=portTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, auditTable=auditTable, auditLogThrottle=auditLogThrottle, auditCommand=auditCommand, cardEntry=cardEntry, cardType=cardType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetUpTime=cmResetUpTime, portBgpId=portBgpId, cardTemperature=cardTemperature, cadEquipmentMib=cadEquipmentMib, cardTestResultNotification=cardTestResultNotification, equipmentPortGroup=equipmentPortGroup, diskDriveNumber=diskDriveNumber, cardPicHwVersion=cardPicHwVersion, equipmentError=equipmentError, diskVolumeGroup=diskVolumeGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, TestTransactionId=TestTransactionId)
