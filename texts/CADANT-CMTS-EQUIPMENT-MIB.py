...
UnknownState, PicType, PortDetectedMode, CardType, EqActionType, ShelfId, CardSubType, DuplexStatus, PortMode, AdminState, FlowControlMode, PortType, DiskVolumeUsageLevel, PortId, CardId, PrimaryState, FirmwareSource, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "PicType", "PortDetectedMode", "CardType", "EqActionType", "ShelfId", "CardSubType", "DuplexStatus", "PortMode", "AdminState", "FlowControlMode", "PortType", "DiskVolumeUsageLevel", "PortId", "CardId", "PrimaryState", "FirmwareSource", "SecondaryState")
...
DateAndTime, MacAddress, TimeStamp, RowStatus, TextualConvention, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "MacAddress", "TimeStamp", "RowStatus", "TextualConvention", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResult=cardTestResult, cardSerialNum=cardSerialNum, diskVolumeTrapInh=diskVolumeTrapInh, cardTestTable=cardTestTable, TestId=TestId, cardTestType=cardTestType, portDetectedMode=portDetectedMode, portCurrDsPower=portCurrDsPower, equipmentCardGroup=equipmentCardGroup, portLastChangeTime=portLastChangeTime, diskVolumeFileName=diskVolumeFileName, cmUChannelID=cmUChannelID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardDetectedChange=cardDetectedChange, cardLastChangeTime=cardLastChangeTime, cardTemperature=cardTemperature, auditAutoScheduling=auditAutoScheduling, shelfName=shelfName, equipmentDiagGroup=equipmentDiagGroup, cardTestEntry=cardTestEntry, auditCycleCount=auditCycleCount, auditStatus=auditStatus, portDescription=portDescription, cmResetStatus=cmResetStatus, cardSwVersion=cardSwVersion, diskVolumeShelfId=diskVolumeShelfId, cardBootVersion=cardBootVersion, cardTestResultNotification=cardTestResultNotification, auditLogThrottle=auditLogThrottle, shelfSwVersion=shelfSwVersion, auditCommand=auditCommand, portMinDsPower=portMinDsPower, systemGeneral=systemGeneral, cardPrState=cardPrState, portPrState=portPrState, diskVolumeName=diskVolumeName, cardShelfId=cardShelfId, auditTotalPassedCount=auditTotalPassedCount, cardSubDetected=cardSubDetected, trapCounter=trapCounter, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portSecState=portSecState, cmResetInfo=cmResetInfo, portCardId=portCardId, auditShelfId=auditShelfId, auditDescription=auditDescription, diskVolumeTable=diskVolumeTable, cmPrimarySID=cmPrimarySID, cardLastResetReason=cardLastResetReason, remainInDiagMode=remainInDiagMode, portPrStateChange=portPrStateChange, diskVolumeCardId=diskVolumeCardId, cmResetClearNotification=cmResetClearNotification, equipmentAudit=equipmentAudit, cmDevice=cmDevice, TestSchedule=TestSchedule, cardTable=cardTable, equipmentCardTestGroup=equipmentCardTestGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardDplxStatus=cardDplxStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestType=TestType, portTrapInh=portTrapInh, cardDetected=cardDetected, cardTrapInh=cardTrapInh, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSecState=cardSecState, auditEntry=auditEntry, auditResult=auditResult, portRxFlowControlMode=portRxFlowControlMode, cardDplxStatusChange=cardDplxStatusChange, trapSeverity=trapSeverity, cardRamSize=cardRamSize, portShelfId=portShelfId, cardName=cardName, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestResultDesc=cardTestResultDesc, cardFpgaSource=cardFpgaSource, portId=portId, errEventId=errEventId, systemGeneralGroup=systemGeneralGroup, auditId=auditId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardAdminState=cardAdminState, diskVolumeGroup=diskVolumeGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSpareGroupMode=cardSpareGroupMode, diskVolumeSize=diskVolumeSize, auditCardId=auditCardId, auditName=auditName, cardPicSerialNum=cardPicSerialNum, equipmentTraps=equipmentTraps, cardEntry=cardEntry, portEntry=portEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSubType=cardSubType, cardFlashSize=cardFlashSize, portMacIfIndex=portMacIfIndex, eqDiagConfig=eqDiagConfig, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetNotification=cmResetNotification, portMode=portMode, portNumChans=portNumChans, cardLastBootSource=cardLastBootSource, portNumber=portNumber, diskVolumeNumber=diskVolumeNumber, errEvRowStatus=errEvRowStatus, cardCarrierHwVersion=cardCarrierHwVersion, cardUpTime=cardUpTime, portBgpId=portBgpId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentDiag=equipmentDiag, cardTestId=cardTestId, cmMacAddress=cmMacAddress, portTxFlowControlMode=portTxFlowControlMode, cardShelfTestIndex=cardShelfTestIndex, cardTestDescription=cardTestDescription, equipmentPortGroup=equipmentPortGroup, cardBusSpeed=cardBusSpeed, cardPicHwVersion=cardPicHwVersion, portTxFlowControlDetected=portTxFlowControlDetected, diagTestId=diagTestId, cardTemperatureHighError=cardTemperatureHighError, cardLastBootVersion=cardLastBootVersion, cardTestScheduleCommand=cardTestScheduleCommand, portCardSubType=portCardSubType, portDplxStatus=portDplxStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestCommand=cardTestCommand, portTable=portTable, equipmentStateGroup=equipmentStateGroup, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSecStateChange=cardSecStateChange, fpgaErrorEventGroup=fpgaErrorEventGroup, cmVendor=cmVendor, equipmentError=equipmentError, errEvLoggingEnabled=errEvLoggingEnabled, diskDriveNumber=diskDriveNumber, cardCpuSpeed=cardCpuSpeed, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardId=cardId, cardType=cardType, cardNumPorts=cardNumPorts, portSecStateChange=portSecStateChange, consoleOutput=consoleOutput, downstreamPowerLoss=downstreamPowerLoss, portType=portType, equipmentTbl=equipmentTbl, cardTestTime=cardTestTime, shelfObjects=shelfObjects, systemKey=systemKey, cardPicDetected=cardPicDetected, cardCpuType=cardCpuType, portAdminState=portAdminState, cmResetUpTime=cmResetUpTime, portMacAddress=portMacAddress, TestResult=TestResult, auditLogOutput=auditLogOutput, cardTempNoReportNotification=cardTempNoReportNotification, TestCommand=TestCommand, auditFailedCount=auditFailedCount, TestScheduleCommand=TestScheduleCommand, cardNumber=cardNumber, equipmentState=equipmentState, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetReason=cmResetReason, diskVolumeId=diskVolumeId, TestTransactionId=TestTransactionId, cardTestName=cardTestName, auditTime=auditTime, fpgaErrorEventTable=fpgaErrorEventTable, cmRegistrationNotification=cmRegistrationNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierFwVersion=cardCarrierFwVersion, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMib=cadEquipmentMib, portConnectorId=portConnectorId, portDocsIfIndex=portDocsIfIndex, errEvLogLevel=errEvLogLevel, systemClock=systemClock, cardFwVersion=cardFwVersion, cardCPLDVersion=cardCPLDVersion, portAction=portAction, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeDriveId=diskVolumeDriveId, cardPrStateChange=cardPrStateChange, cmIpAddress=cmIpAddress, auditPassedCount=auditPassedCount, cardTestTransId=cardTestTransId, portRxFlowControlDetected=portRxFlowControlDetected, verboseLevel=verboseLevel, cardTestIndex=cardTestIndex, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestSchedule=cardTestSchedule, shelfNumber=shelfNumber, auditTable=auditTable, cardHwVersion=cardHwVersion, cardSpareGroupId=cardSpareGroupId, equipmentAuditGroup=equipmentAuditGroup, equipmentShelfGroup=equipmentShelfGroup, cardCarrierSerialNum=cardCarrierSerialNum, cardAction=cardAction, portMaxDsPower=portMaxDsPower)
