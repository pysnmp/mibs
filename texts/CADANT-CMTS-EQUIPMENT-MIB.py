...
AdminState, EqActionType, PortMode, CardType, PrimaryState, FirmwareSource, DuplexStatus, SecondaryState, ShelfId, UnknownState, PortDetectedMode, FlowControlMode, PortId, CardId, CardSubType, DiskVolumeUsageLevel, PicType, PortType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "EqActionType", "PortMode", "CardType", "PrimaryState", "FirmwareSource", "DuplexStatus", "SecondaryState", "ShelfId", "UnknownState", "PortDetectedMode", "FlowControlMode", "PortId", "CardId", "CardSubType", "DiskVolumeUsageLevel", "PicType", "PortType")
...
TimeStamp, RowStatus, DisplayString, MacAddress, TruthValue, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "RowStatus", "DisplayString", "MacAddress", "TruthValue", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentDiag=equipmentDiag, cardRamSize=cardRamSize, equipmentError=equipmentError, cmUChannelID=cmUChannelID, shelfSwVersion=shelfSwVersion, cardSwVersion=cardSwVersion, cardPicSerialNum=cardPicSerialNum, cardNumber=cardNumber, cardLastBootVersion=cardLastBootVersion, cardTestTable=cardTestTable, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestName=cardTestName, cardAdminState=cardAdminState, trapSeverity=trapSeverity, auditTime=auditTime, equipmentStateGroup=equipmentStateGroup, portTxFlowControlMode=portTxFlowControlMode, cardTestTime=cardTestTime, auditAutoScheduling=auditAutoScheduling, cardSecState=cardSecState, portCardId=portCardId, cardPrState=cardPrState, cardTempNoReportNotification=cardTempNoReportNotification, equipmentShelfGroup=equipmentShelfGroup, TestTransactionId=TestTransactionId, cardTestType=cardTestType, portNumber=portNumber, cardDplxStatus=cardDplxStatus, diskVolumeShelfId=diskVolumeShelfId, diskVolumeNumber=diskVolumeNumber, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmVendor=cmVendor, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestCommand=TestCommand, auditDescription=auditDescription, cardLastBootSource=cardLastBootSource, auditStatus=auditStatus, diskVolumeTrapInh=diskVolumeTrapInh, cmDevice=cmDevice, diskVolumeGroup=diskVolumeGroup, auditShelfId=auditShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDescription=portDescription, portTable=portTable, cardBootVersion=cardBootVersion, errEvLogLevel=errEvLogLevel, cardShelfId=cardShelfId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetInfo=cmResetInfo, TestSchedule=TestSchedule, cardHwVersion=cardHwVersion, portMacAddress=portMacAddress, auditLogThrottle=auditLogThrottle, diskVolumeId=diskVolumeId, cardId=cardId, cardTable=cardTable, portConnectorId=portConnectorId, cardTestResult=cardTestResult, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentState=equipmentState, portSecStateChange=portSecStateChange, cardFlashSize=cardFlashSize, systemClock=systemClock, trapCounter=trapCounter, cardLastResetReason=cardLastResetReason, cardCarrierFwVersion=cardCarrierFwVersion, cmResetNotification=cmResetNotification, diskVolumeName=diskVolumeName, cadEquipmentMib=cadEquipmentMib, downstreamPowerLoss=downstreamPowerLoss, cardAction=cardAction, auditName=auditName, cmResetUpTime=cmResetUpTime, TestType=TestType, cardTemperature=cardTemperature, diskVolumeCardId=diskVolumeCardId, diskVolumeTable=diskVolumeTable, portAction=portAction, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSubType=cardSubType, errEventId=errEventId, equipmentPortGroup=equipmentPortGroup, cardDetected=cardDetected, portMode=portMode, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvRowStatus=errEvRowStatus, equipmentDiagGroup=equipmentDiagGroup, cardNumPorts=cardNumPorts, shelfName=shelfName, cardLastChangeTime=cardLastChangeTime, cardTestId=cardTestId, cardPicDetected=cardPicDetected, cardCarrierSerialNum=cardCarrierSerialNum, cardTrapInh=cardTrapInh, portMaxDsPower=portMaxDsPower, cardSpareGroupMode=cardSpareGroupMode, diskDriveNumber=diskDriveNumber, portLastChangeTime=portLastChangeTime, portDplxStatus=portDplxStatus, portBgpId=portBgpId, shelfObjects=shelfObjects, diskVolumeSize=diskVolumeSize, diskVolumeFileName=diskVolumeFileName, portRxFlowControlMode=portRxFlowControlMode, cmResetReason=cmResetReason, equipmentAudit=equipmentAudit, cardDplxStatusChange=cardDplxStatusChange, cardType=cardType, cardTestCommand=cardTestCommand, cmPrimarySID=cmPrimarySID, cardBusSpeed=cardBusSpeed, portMinDsPower=portMinDsPower, fpgaErrorEventTable=fpgaErrorEventTable, cardTempOverHeatNotification=cardTempOverHeatNotification, portAdminState=portAdminState, cardTestScheduleCommand=cardTestScheduleCommand, equipmentAuditGroup=equipmentAuditGroup, cardDetectedChange=cardDetectedChange, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestEntry=cardTestEntry, portDocsIfIndex=portDocsIfIndex, cardTestSchedule=cardTestSchedule, auditResult=auditResult, cardTestResultNotification=cardTestResultNotification, portType=portType, TestId=TestId, cardPicHwVersion=cardPicHwVersion, portCardSubType=portCardSubType, portShelfId=portShelfId, auditFailedCount=auditFailedCount, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentTraps=equipmentTraps, cardPrStateChange=cardPrStateChange, shelfNumber=shelfNumber, verboseLevel=verboseLevel, cardFwUpdateStatus=cardFwUpdateStatus, portCurrDsPower=portCurrDsPower, cardShelfTestIndex=cardShelfTestIndex, TestScheduleCommand=TestScheduleCommand, portTxFlowControlDetected=portTxFlowControlDetected, cmIpAddress=cmIpAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDetectedMode=portDetectedMode, portPrState=portPrState, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeEntry=diskVolumeEntry, auditEntry=auditEntry, auditTotalPassedCount=auditTotalPassedCount, cardFpgaSource=cardFpgaSource, systemGeneral=systemGeneral, cardTestTransId=cardTestTransId, auditId=auditId, cmRegistrationNotification=cmRegistrationNotification, cardCpuType=cardCpuType, cardCPLDVersion=cardCPLDVersion, portRxFlowControlDetected=portRxFlowControlDetected, systemGeneralGroup=systemGeneralGroup, cardTemperatureHighError=cardTemperatureHighError, cardSpareGroupId=cardSpareGroupId, eqDiagConfig=eqDiagConfig, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetClearNotification=cmResetClearNotification, auditTable=auditTable, cmMacAddress=cmMacAddress, portSecState=portSecState, auditCommand=auditCommand, systemKey=systemKey, cardUpTime=cardUpTime, cardCpuSpeed=cardCpuSpeed, auditLogOutput=auditLogOutput, cardSubDetected=cardSubDetected, auditCycleCount=auditCycleCount, auditTotalFailedCount=auditTotalFailedCount, portDplxStatusChange=portDplxStatusChange, remainInDiagMode=remainInDiagMode, cardTestIndex=cardTestIndex, cmResetStatus=cmResetStatus, portMacIfIndex=portMacIfIndex, cardTestResultDesc=cardTestResultDesc, equipmentTbl=equipmentTbl, cardFwVersion=cardFwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, portNumChans=portNumChans, portId=portId, cardTestDescription=cardTestDescription, auditPassedCount=auditPassedCount, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portPrStateChange=portPrStateChange, cardSecStateChange=cardSecStateChange, cadEquipmentMibGroup=cadEquipmentMibGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, portTrapInh=portTrapInh, cardSerialNum=cardSerialNum, auditCardId=auditCardId, equipmentCardGroup=equipmentCardGroup, TestResult=TestResult, consoleOutput=consoleOutput, cardName=cardName, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardEntry=cardEntry, portEntry=portEntry, diagTestId=diagTestId)
