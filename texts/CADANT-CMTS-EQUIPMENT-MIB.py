...
DuplexStatus, PortMode, CardSubType, ShelfId, AdminState, EqActionType, PortType, SecondaryState, PrimaryState, FlowControlMode, CardType, PicType, UnknownState, FirmwareSource, PortDetectedMode, CardId, PortId, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortMode", "CardSubType", "ShelfId", "AdminState", "EqActionType", "PortType", "SecondaryState", "PrimaryState", "FlowControlMode", "CardType", "PicType", "UnknownState", "FirmwareSource", "PortDetectedMode", "CardId", "PortId", "DiskVolumeUsageLevel")
...
MacAddress, DateAndTime, DisplayString, TruthValue, TimeStamp, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "DateAndTime", "DisplayString", "TruthValue", "TimeStamp", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentPortGroup=equipmentPortGroup, cardCpuType=cardCpuType, shelfObjects=shelfObjects, cardTemperature=cardTemperature, cardTestSchedule=cardTestSchedule, consoleOutput=consoleOutput, downstreamPowerLoss=downstreamPowerLoss, equipmentTraps=equipmentTraps, auditLogOutput=auditLogOutput, auditStatus=auditStatus, cardTestEntry=cardTestEntry, auditShelfId=auditShelfId, diskVolumeShelfId=diskVolumeShelfId, cardPrState=cardPrState, TestSchedule=TestSchedule, cmResetInfo=cmResetInfo, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSecState=cardSecState, cardPicDetected=cardPicDetected, cardNumPorts=cardNumPorts, portShelfId=portShelfId, portLastChangeTime=portLastChangeTime, systemGeneralGroup=systemGeneralGroup, portCardSubType=portCardSubType, portDplxStatus=portDplxStatus, auditResult=auditResult, equipmentAuditGroup=equipmentAuditGroup, equipmentAudit=equipmentAudit, cadEquipmentMib=cadEquipmentMib, shelfNumber=shelfNumber, cardName=cardName, errEvLoggingEnabled=errEvLoggingEnabled, cmIpAddress=cmIpAddress, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portAdminState=portAdminState, diskVolumeGroup=diskVolumeGroup, cardTrapInh=cardTrapInh, portId=portId, portType=portType, portRxFlowControlMode=portRxFlowControlMode, cardSubType=cardSubType, errEvRowStatus=errEvRowStatus, portDplxStatusChange=portDplxStatusChange, cardCarrierHwVersion=cardCarrierHwVersion, trapSeverity=trapSeverity, shelfName=shelfName, shelfSwVersion=shelfSwVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardShelfTestIndex=cardShelfTestIndex, portDocsIfIndex=portDocsIfIndex, portMacAddress=portMacAddress, cardTestCommand=cardTestCommand, cardTestTransId=cardTestTransId, cmPrimarySID=cmPrimarySID, auditPassedCount=auditPassedCount, cmResetUpTime=cmResetUpTime, portPrStateChange=portPrStateChange, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestName=cardTestName, TestCommand=TestCommand, portTable=portTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portCurrDsPower=portCurrDsPower, verboseLevel=verboseLevel, portMode=portMode, portBgpId=portBgpId, cadEquipmentMibConformance=cadEquipmentMibConformance, portNumber=portNumber, auditName=auditName, auditAutoScheduling=auditAutoScheduling, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portDescription=portDescription, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestResult=cardTestResult, cardSecStateChange=cardSecStateChange, cmResetStatus=cmResetStatus, portCardId=portCardId, diskVolumeEntry=diskVolumeEntry, cardTestId=cardTestId, auditTotalPassedCount=auditTotalPassedCount, systemClock=systemClock, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentError=equipmentError, cardPrStateChange=cardPrStateChange, cardShelfId=cardShelfId, cardFpgaSource=cardFpgaSource, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSpareGroupId=cardSpareGroupId, fpgaErrorEventEntry=fpgaErrorEventEntry, portDetectedMode=portDetectedMode, diskVolumeId=diskVolumeId, portTxFlowControlDetected=portTxFlowControlDetected, cardSpareGroupMode=cardSpareGroupMode, cmResetNotification=cmResetNotification, cardTestDescription=cardTestDescription, cmRegistrationNotification=cmRegistrationNotification, cardAction=cardAction, cardLastResetReason=cardLastResetReason, cardCarrierFwVersion=cardCarrierFwVersion, cmResetClearNotification=cmResetClearNotification, cmVendor=cmVendor, cardDetectedChange=cardDetectedChange, cardDplxStatusChange=cardDplxStatusChange, trapCounter=trapCounter, cardTestIndex=cardTestIndex, portAction=portAction, auditId=auditId, cardTestResultNotification=cardTestResultNotification, equipmentShelfGroup=equipmentShelfGroup, auditTable=auditTable, TestResult=TestResult, cardFwVersion=cardFwVersion, diskVolumeCardId=diskVolumeCardId, auditEntry=auditEntry, auditCardId=auditCardId, diskVolumeName=diskVolumeName, cardBusSpeed=cardBusSpeed, cardFwUpdateStatus=cardFwUpdateStatus, portEntry=portEntry, diagTestId=diagTestId, auditTotalFailedCount=auditTotalFailedCount, errEventId=errEventId, cardLastChangeTime=cardLastChangeTime, equipmentDiagGroup=equipmentDiagGroup, fpgaErrorEventTable=fpgaErrorEventTable, portConnectorId=portConnectorId, cardLastBootVersion=cardLastBootVersion, diskVolumeTrapInh=diskVolumeTrapInh, cmResetReason=cmResetReason, cardTempNoReportNotification=cardTempNoReportNotification, cardHwVersion=cardHwVersion, equipmentTbl=equipmentTbl, cardDetected=cardDetected, portPrState=portPrState, TestId=TestId, cardSubDetected=cardSubDetected, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResultDesc=cardTestResultDesc, cardSwVersion=cardSwVersion, auditLogThrottle=auditLogThrottle, cardPicSerialNum=cardPicSerialNum, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFlashSize=cardFlashSize, errEvLogLevel=errEvLogLevel, portSecState=portSecState, auditDescription=auditDescription, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageNotification=diskVolumeUsageNotification, diskDriveNumber=diskDriveNumber, cardEntry=cardEntry, portSecStateChange=portSecStateChange, portMaxDsPower=portMaxDsPower, auditCommand=auditCommand, auditFailedCount=auditFailedCount, cardDplxStatus=cardDplxStatus, cardCpuSpeed=cardCpuSpeed, portNumChans=portNumChans, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTemperatureHighError=cardTemperatureHighError, equipmentCardTestGroup=equipmentCardTestGroup, cardSerialNum=cardSerialNum, systemKey=systemKey, equipmentDiag=equipmentDiag, cardRamSize=cardRamSize, portMinDsPower=portMinDsPower, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPicHwVersion=cardPicHwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, eqDiagConfig=eqDiagConfig, cardNumber=cardNumber, cardCPLDVersion=cardCPLDVersion, TestTransactionId=TestTransactionId, cardTestType=cardTestType, equipmentCardGroup=equipmentCardGroup, diskVolumeNumber=diskVolumeNumber, TestType=TestType, cmDevice=cmDevice, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastBootSource=cardLastBootSource, diskVolumeDriveId=diskVolumeDriveId, cardTestTime=cardTestTime, cmMacAddress=cmMacAddress, diskVolumeSize=diskVolumeSize, cardId=cardId, cardTempOverHeatNotification=cardTempOverHeatNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTable=cardTestTable, equipmentState=equipmentState, diskVolumeFileName=diskVolumeFileName, equipmentStateGroup=equipmentStateGroup, TestScheduleCommand=TestScheduleCommand, cardTestScheduleCommand=cardTestScheduleCommand, auditTime=auditTime, systemGeneral=systemGeneral, remainInDiagMode=remainInDiagMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTable=cardTable, cardBootVersion=cardBootVersion, portTrapInh=portTrapInh, auditCycleCount=auditCycleCount, cmUChannelID=cmUChannelID, cardAdminState=cardAdminState, cardType=cardType, cardUpTime=cardUpTime, portMacIfIndex=portMacIfIndex, diskVolumeTable=diskVolumeTable)
