...
PicType, DuplexStatus, EqActionType, PortDetectedMode, PrimaryState, FlowControlMode, CardId, SecondaryState, DiskVolumeUsageLevel, FirmwareSource, UnknownState, PortType, CardSubType, ShelfId, CardType, PortMode, PortId, AdminState = mibBuilder.importSymbols("CADANT-TC", "PicType", "DuplexStatus", "EqActionType", "PortDetectedMode", "PrimaryState", "FlowControlMode", "CardId", "SecondaryState", "DiskVolumeUsageLevel", "FirmwareSource", "UnknownState", "PortType", "CardSubType", "ShelfId", "CardType", "PortMode", "PortId", "AdminState")
...
DisplayString, TimeStamp, DateAndTime, MacAddress, RowStatus, TextualConvention, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "DateAndTime", "MacAddress", "RowStatus", "TextualConvention", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskDriveNumber=diskDriveNumber, portNumber=portNumber, systemGeneral=systemGeneral, portMode=portMode, cardCarrierHwVersion=cardCarrierHwVersion, TestSchedule=TestSchedule, portEntry=portEntry, cmDevice=cmDevice, PYSNMP_MODULE_ID=cadEquipmentMib, portPrStateChange=portPrStateChange, cmVendor=cmVendor, cardFwVersion=cardFwVersion, cardTempNoReportNotification=cardTempNoReportNotification, cardLastBootVersion=cardLastBootVersion, systemClock=systemClock, diskVolumeDriveId=diskVolumeDriveId, equipmentAuditGroup=equipmentAuditGroup, cardSecState=cardSecState, diskVolumeTrapInh=diskVolumeTrapInh, cardTestCommand=cardTestCommand, trapCounter=trapCounter, cardUpTime=cardUpTime, cardName=cardName, diskVolumeFileName=diskVolumeFileName, cardTemperature=cardTemperature, cardAction=cardAction, fpgaErrorEventEntry=fpgaErrorEventEntry, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSpareGroupMode=cardSpareGroupMode, equipmentState=equipmentState, portMinDsPower=portMinDsPower, errEvLoggingEnabled=errEvLoggingEnabled, cardDetected=cardDetected, auditPassedCount=auditPassedCount, TestTransactionId=TestTransactionId, cardShelfId=cardShelfId, auditStatus=auditStatus, cardPicHwVersion=cardPicHwVersion, portSecStateChange=portSecStateChange, cardTestSchedule=cardTestSchedule, auditTime=auditTime, cardTestId=cardTestId, equipmentStateGroup=equipmentStateGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardPrState=cardPrState, cmResetNotification=cmResetNotification, cardLastBootSource=cardLastBootSource, auditLogThrottle=auditLogThrottle, cardCpuSpeed=cardCpuSpeed, cardDetectedChange=cardDetectedChange, portId=portId, cardPicSerialNum=cardPicSerialNum, cardBusSpeed=cardBusSpeed, auditAutoScheduling=auditAutoScheduling, cardPrStateChange=cardPrStateChange, equipmentCardGroup=equipmentCardGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestTime=cardTestTime, shelfNumber=shelfNumber, portTrapInh=portTrapInh, cardRamSize=cardRamSize, diskVolumeCardId=diskVolumeCardId, cardTestTransId=cardTestTransId, equipmentAudit=equipmentAudit, cardTestName=cardTestName, cardFwUpdateStatus=cardFwUpdateStatus, cardTestEntry=cardTestEntry, cardTestIndex=cardTestIndex, cardTestDescription=cardTestDescription, auditEntry=auditEntry, errEventId=errEventId, cardSwVersion=cardSwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, consoleOutput=consoleOutput, cardSpareGroupId=cardSpareGroupId, auditTotalFailedCount=auditTotalFailedCount, portMacIfIndex=portMacIfIndex, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portAction=portAction, diskVolumeEntry=diskVolumeEntry, equipmentShelfGroup=equipmentShelfGroup, auditFailedCount=auditFailedCount, TestResult=TestResult, cardTemperatureHighError=cardTemperatureHighError, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, remainInDiagMode=remainInDiagMode, cardTemperatureHighWarn=cardTemperatureHighWarn, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetReason=cmResetReason, portCardId=portCardId, portType=portType, shelfObjects=shelfObjects, cmRegistrationNotification=cmRegistrationNotification, portSecState=portSecState, diskVolumeSize=diskVolumeSize, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentError=equipmentError, portMaxDsPower=portMaxDsPower, portPrState=portPrState, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfName=shelfName, equipmentTraps=equipmentTraps, cardSubType=cardSubType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSerialNum=cardSerialNum, equipmentTbl=equipmentTbl, cardFlashSize=cardFlashSize, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDescription=portDescription, portMacAddress=portMacAddress, cmPrimarySID=cmPrimarySID, diskVolumeShelfId=diskVolumeShelfId, TestId=TestId, cardTable=cardTable, auditCommand=auditCommand, portCurrDsPower=portCurrDsPower, cardSecStateChange=cardSecStateChange, portDplxStatusChange=portDplxStatusChange, diskVolumeGroup=diskVolumeGroup, cardHwVersion=cardHwVersion, diskVolumeTable=diskVolumeTable, TestType=TestType, diskVolumeName=diskVolumeName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResultNotification=cardTestResultNotification, downstreamPowerLoss=downstreamPowerLoss, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeId=diskVolumeId, cardFpgaSource=cardFpgaSource, portTxFlowControlMode=portTxFlowControlMode, auditTotalPassedCount=auditTotalPassedCount, auditResult=auditResult, auditId=auditId, portConnectorId=portConnectorId, cardTestResultDesc=cardTestResultDesc, cmMacAddress=cmMacAddress, cardTrapInh=cardTrapInh, cmResetInfo=cmResetInfo, equipmentDiagGroup=equipmentDiagGroup, cardNumPorts=cardNumPorts, cardType=cardType, auditCardId=auditCardId, cmResetUpTime=cmResetUpTime, auditLogOutput=auditLogOutput, portDplxStatus=portDplxStatus, portRxFlowControlDetected=portRxFlowControlDetected, auditShelfId=auditShelfId, portNumChans=portNumChans, cardTestType=cardTestType, auditTable=auditTable, cardBootVersion=cardBootVersion, auditCycleCount=auditCycleCount, cardLastResetReason=cardLastResetReason, cardShelfTestIndex=cardShelfTestIndex, cardDplxStatusChange=cardDplxStatusChange, trapSeverity=trapSeverity, TestCommand=TestCommand, cadEquipmentMibGroup=cadEquipmentMibGroup, portShelfId=portShelfId, portCardSubType=portCardSubType, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMib=cadEquipmentMib, cardId=cardId, cmResetClearNotification=cmResetClearNotification, portAdminState=portAdminState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardPicDetected=cardPicDetected, equipmentDiag=equipmentDiag, cmIpAddress=cmIpAddress, diskVolumeDiskSize=diskVolumeDiskSize, auditName=auditName, cardCpuType=cardCpuType, fpgaErrorEventTable=fpgaErrorEventTable, cardNumber=cardNumber, portDocsIfIndex=portDocsIfIndex, eqDiagConfig=eqDiagConfig, auditDescription=auditDescription, cardSubDetected=cardSubDetected, errEvRowStatus=errEvRowStatus, cardCarrierFwVersion=cardCarrierFwVersion, cardTestTable=cardTestTable, diagTestId=diagTestId, cardAdminState=cardAdminState, TestScheduleCommand=TestScheduleCommand, portBgpId=portBgpId, shelfSwVersion=shelfSwVersion, diskVolumeNumber=diskVolumeNumber, equipmentPortGroup=equipmentPortGroup, portLastChangeTime=portLastChangeTime, verboseLevel=verboseLevel, portDetectedMode=portDetectedMode, cardDplxStatus=cardDplxStatus, cardEntry=cardEntry, portTable=portTable, cardTestResult=cardTestResult, cardTestScheduleCommand=cardTestScheduleCommand, cmResetStatus=cmResetStatus, cardCPLDVersion=cardCPLDVersion, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portRxFlowControlMode=portRxFlowControlMode, errEvLogLevel=errEvLogLevel, systemKey=systemKey, cardLastChangeTime=cardLastChangeTime, cmUChannelID=cmUChannelID)
