...
PrimaryState, UnknownState, EqActionType, SecondaryState, DiskVolumeUsageLevel, PortDetectedMode, AdminState, ShelfId, FlowControlMode, PortId, DuplexStatus, PortType, CardSubType, PicType, FirmwareSource, CardType, PortMode, CardId = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "UnknownState", "EqActionType", "SecondaryState", "DiskVolumeUsageLevel", "PortDetectedMode", "AdminState", "ShelfId", "FlowControlMode", "PortId", "DuplexStatus", "PortType", "CardSubType", "PicType", "FirmwareSource", "CardType", "PortMode", "CardId")
...
TruthValue, DateAndTime, TimeStamp, RowStatus, TextualConvention, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "TimeStamp", "RowStatus", "TextualConvention", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskDriveNumber=diskDriveNumber, verboseLevel=verboseLevel, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditName=auditName, portType=portType, cmPrimarySID=cmPrimarySID, cardAdminState=cardAdminState, systemGeneralGroup=systemGeneralGroup, diskVolumeDriveId=diskVolumeDriveId, portCardId=portCardId, cardSpareGroupMode=cardSpareGroupMode, auditLogOutput=auditLogOutput, equipmentError=equipmentError, auditCardId=auditCardId, shelfName=shelfName, cardFwVersion=cardFwVersion, cardSwVersion=cardSwVersion, auditShelfId=auditShelfId, cmResetStatus=cmResetStatus, cmIpAddress=cmIpAddress, cardTestCommand=cardTestCommand, portDplxStatus=portDplxStatus, portDetectedMode=portDetectedMode, cmResetClearNotification=cmResetClearNotification, equipmentTraps=equipmentTraps, cardTemperatureHighError=cardTemperatureHighError, diskVolumeUsageLevel=diskVolumeUsageLevel, fpgaErrorEventTable=fpgaErrorEventTable, equipmentDiag=equipmentDiag, cardNumber=cardNumber, auditEntry=auditEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, remainInDiagMode=remainInDiagMode, cardBusSpeed=cardBusSpeed, cardFpgaSource=cardFpgaSource, cardCarrierFwVersion=cardCarrierFwVersion, portShelfId=portShelfId, diskVolumeCardId=diskVolumeCardId, diskVolumeId=diskVolumeId, cardLastResetReason=cardLastResetReason, cardSecState=cardSecState, cardShelfId=cardShelfId, cardLastChangeTime=cardLastChangeTime, portEntry=portEntry, shelfObjects=shelfObjects, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResult=cardTestResult, cardTestId=cardTestId, portTrapInh=portTrapInh, portAdminState=portAdminState, cardShelfTestIndex=cardShelfTestIndex, cardPicDetected=cardPicDetected, diskVolumeEntry=diskVolumeEntry, diskVolumeDiskSize=diskVolumeDiskSize, portSecStateChange=portSecStateChange, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentAudit=equipmentAudit, portDocsIfIndex=portDocsIfIndex, cardCpuType=cardCpuType, portMaxDsPower=portMaxDsPower, diskVolumeName=diskVolumeName, systemClock=systemClock, diskVolumeSize=diskVolumeSize, cardAction=cardAction, cardRamSize=cardRamSize, auditPassedCount=auditPassedCount, equipmentCardGroup=equipmentCardGroup, portDplxStatusChange=portDplxStatusChange, cardTestType=cardTestType, cmDevice=cmDevice, cardDetected=cardDetected, portLastChangeTime=portLastChangeTime, cmUChannelID=cmUChannelID, portNumChans=portNumChans, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardType=cardType, portRxFlowControlMode=portRxFlowControlMode, cardTestTransId=cardTestTransId, errEvRecoveryEnabled=errEvRecoveryEnabled, TestScheduleCommand=TestScheduleCommand, equipmentState=equipmentState, diskVolumeUsageNotification=diskVolumeUsageNotification, cardDplxStatusChange=cardDplxStatusChange, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portConnectorId=portConnectorId, cardFwUpdateStatus=cardFwUpdateStatus, portPrStateChange=portPrStateChange, portMode=portMode, cardSubType=cardSubType, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetReason=cmResetReason, cardCPLDVersion=cardCPLDVersion, cardTestDescription=cardTestDescription, auditTotalFailedCount=auditTotalFailedCount, auditLogThrottle=auditLogThrottle, auditResult=auditResult, portCurrDsPower=portCurrDsPower, cardFlashSize=cardFlashSize, portSecState=portSecState, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTrapInh=cardTrapInh, cardTestIndex=cardTestIndex, auditAutoScheduling=auditAutoScheduling, cardSpareGroupId=cardSpareGroupId, cardTable=cardTable, cardSerialNum=cardSerialNum, cardLastBootSource=cardLastBootSource, diskVolumeNumber=diskVolumeNumber, diskVolumeGroup=diskVolumeGroup, diskVolumeShelfId=diskVolumeShelfId, portMacIfIndex=portMacIfIndex, TestId=TestId, cardTestResultNotification=cardTestResultNotification, cardTemperature=cardTemperature, auditTable=auditTable, equipmentCardTestGroup=equipmentCardTestGroup, cardCpuSpeed=cardCpuSpeed, cardName=cardName, cardCarrierHwVersion=cardCarrierHwVersion, trapSeverity=trapSeverity, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, shelfNumber=shelfNumber, portCardSubType=portCardSubType, cardId=cardId, equipmentAuditGroup=equipmentAuditGroup, portMinDsPower=portMinDsPower, portNumber=portNumber, diskVolumeFileName=diskVolumeFileName, consoleOutput=consoleOutput, cardPicHwVersion=cardPicHwVersion, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDescription=portDescription, cmRegistrationNotification=cmRegistrationNotification, cmResetUpTime=cmResetUpTime, errEventId=errEventId, equipmentPortGroup=equipmentPortGroup, auditId=auditId, auditTime=auditTime, systemKey=systemKey, portId=portId, cardUpTime=cardUpTime, portTxFlowControlDetected=portTxFlowControlDetected, auditStatus=auditStatus, TestTransactionId=TestTransactionId, errEvRowStatus=errEvRowStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, auditCommand=auditCommand, errEvLoggingEnabled=errEvLoggingEnabled, diagTestId=diagTestId, cardPicSerialNum=cardPicSerialNum, portMacAddress=portMacAddress, cardTestScheduleCommand=cardTestScheduleCommand, cardSecStateChange=cardSecStateChange, cmVendor=cmVendor, cardTestName=cardTestName, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventGroup=fpgaErrorEventGroup, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, TestResult=TestResult, cardNumPorts=cardNumPorts, cardTestTime=cardTestTime, errEvLogLevel=errEvLogLevel, cardHwVersion=cardHwVersion, cardDetectedChange=cardDetectedChange, cardDplxStatus=cardDplxStatus, portAction=portAction, equipmentDiagGroup=equipmentDiagGroup, TestSchedule=TestSchedule, TestCommand=TestCommand, portBgpId=portBgpId, cardTestResultDesc=cardTestResultDesc, shelfSwVersion=shelfSwVersion, TestType=TestType, portTxFlowControlMode=portTxFlowControlMode, equipmentShelfGroup=equipmentShelfGroup, trapCounter=trapCounter, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTempNoReportNotification=cardTempNoReportNotification, eqDiagConfig=eqDiagConfig, auditDescription=auditDescription, equipmentStateGroup=equipmentStateGroup, portTable=portTable, cmResetInfo=cmResetInfo, cardPrStateChange=cardPrStateChange, cardEntry=cardEntry, systemGeneral=systemGeneral, cardPrState=cardPrState, cardLastBootVersion=cardLastBootVersion, cmMacAddress=cmMacAddress, portPrState=portPrState, cardBootVersion=cardBootVersion, equipmentTbl=equipmentTbl, diskVolumeTable=diskVolumeTable, downstreamPowerLoss=downstreamPowerLoss, cardTestTable=cardTestTable, cardTestSchedule=cardTestSchedule, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestEntry=cardTestEntry, cmResetNotification=cmResetNotification, auditCycleCount=auditCycleCount, cardSubDetected=cardSubDetected, auditFailedCount=auditFailedCount)
