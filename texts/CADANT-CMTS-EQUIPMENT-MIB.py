...
PicType, DuplexStatus, PortId, EqActionType, CardId, AdminState, CardSubType, DiskVolumeUsageLevel, PortMode, PrimaryState, SecondaryState, UnknownState, FirmwareSource, ShelfId, FlowControlMode, PortDetectedMode, PortType, CardType = mibBuilder.importSymbols("CADANT-TC", "PicType", "DuplexStatus", "PortId", "EqActionType", "CardId", "AdminState", "CardSubType", "DiskVolumeUsageLevel", "PortMode", "PrimaryState", "SecondaryState", "UnknownState", "FirmwareSource", "ShelfId", "FlowControlMode", "PortDetectedMode", "PortType", "CardType")
...
RowStatus, MacAddress, TruthValue, DateAndTime, TimeStamp, DisplayString, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "MacAddress", "TruthValue", "DateAndTime", "TimeStamp", "DisplayString", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCpuType=cardCpuType, downstreamPowerLoss=downstreamPowerLoss, cardTemperature=cardTemperature, eqDiagConfig=eqDiagConfig, cardHwVersion=cardHwVersion, cardSecState=cardSecState, portPrState=portPrState, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeId=diskVolumeId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCpuSpeed=cardCpuSpeed, auditName=auditName, cardTestTransId=cardTestTransId, diskVolumeTrapInh=diskVolumeTrapInh, cardRamSize=cardRamSize, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentState=equipmentState, cardTestResultNotification=cardTestResultNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentDiagGroup=equipmentDiagGroup, portTable=portTable, equipmentPortGroup=equipmentPortGroup, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portType=portType, portMinDsPower=portMinDsPower, auditTotalPassedCount=auditTotalPassedCount, errEvRowStatus=errEvRowStatus, systemKey=systemKey, cardSerialNum=cardSerialNum, auditDescription=auditDescription, auditAutoScheduling=auditAutoScheduling, cmPrimarySID=cmPrimarySID, diskVolumeGroup=diskVolumeGroup, cadEquipmentMib=cadEquipmentMib, cardEntry=cardEntry, portBgpId=portBgpId, cardTestName=cardTestName, cardBootVersion=cardBootVersion, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSwVersion=cardSwVersion, diskVolumeTable=diskVolumeTable, cmDevice=cmDevice, equipmentError=equipmentError, cardCPLDVersion=cardCPLDVersion, portAdminState=portAdminState, auditCommand=auditCommand, trapCounter=trapCounter, TestResult=TestResult, portMacIfIndex=portMacIfIndex, cardShelfTestIndex=cardShelfTestIndex, trapSeverity=trapSeverity, TestCommand=TestCommand, cardCarrierFwVersion=cardCarrierFwVersion, portCardSubType=portCardSubType, auditPassedCount=auditPassedCount, cardTestSchedule=cardTestSchedule, portSecState=portSecState, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestCommand=cardTestCommand, cmResetUpTime=cmResetUpTime, cardNumber=cardNumber, cardTestIndex=cardTestIndex, cardTestId=cardTestId, portCurrDsPower=portCurrDsPower, equipmentDiag=equipmentDiag, auditEntry=auditEntry, systemClock=systemClock, errEvLogLevel=errEvLogLevel, diskVolumeSize=diskVolumeSize, diskVolumeUsageLevel=diskVolumeUsageLevel, diagTestId=diagTestId, auditCycleCount=auditCycleCount, portDetectedMode=portDetectedMode, cardPrStateChange=cardPrStateChange, cardTestResult=cardTestResult, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, shelfSwVersion=shelfSwVersion, equipmentTraps=equipmentTraps, cardFpgaSource=cardFpgaSource, diskDriveNumber=diskDriveNumber, cardTestEntry=cardTestEntry, cmMacAddress=cmMacAddress, cmResetStatus=cmResetStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSubType=cardSubType, cardDplxStatusChange=cardDplxStatusChange, cmResetClearNotification=cmResetClearNotification, cmUChannelID=cmUChannelID, portLastChangeTime=portLastChangeTime, auditId=auditId, cmVendor=cmVendor, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestType=TestType, diskVolumeNumber=diskVolumeNumber, portMacAddress=portMacAddress, diskVolumeShelfId=diskVolumeShelfId, auditResult=auditResult, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeDiskSize=diskVolumeDiskSize, consoleOutput=consoleOutput, cardName=cardName, cardSpareGroupMode=cardSpareGroupMode, auditFailedCount=auditFailedCount, cardLastResetReason=cardLastResetReason, cardType=cardType, cardPicDetected=cardPicDetected, TestTransactionId=TestTransactionId, portDplxStatusChange=portDplxStatusChange, equipmentCardGroup=equipmentCardGroup, diskVolumeName=diskVolumeName, cardUpTime=cardUpTime, cardPrState=cardPrState, cardTestType=cardTestType, portDplxStatus=portDplxStatus, cmResetReason=cmResetReason, diskVolumeCardId=diskVolumeCardId, cardDplxStatus=cardDplxStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portSecStateChange=portSecStateChange, portNumChans=portNumChans, portEntry=portEntry, portId=portId, portDescription=portDescription, cardPicSerialNum=cardPicSerialNum, systemGeneral=systemGeneral, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfName=shelfName, portConnectorId=portConnectorId, remainInDiagMode=remainInDiagMode, cardFlashSize=cardFlashSize, shelfNumber=shelfNumber, cardNumPorts=cardNumPorts, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditCardId=auditCardId, cmIpAddress=cmIpAddress, equipmentStateGroup=equipmentStateGroup, cardTemperatureHighError=cardTemperatureHighError, portNumber=portNumber, cardDetected=cardDetected, fpgaErrorEventTable=fpgaErrorEventTable, portPrStateChange=portPrStateChange, cmResetInfo=cmResetInfo, portMode=portMode, cardTestResultDesc=cardTestResultDesc, portShelfId=portShelfId, cadEquipmentMibConformance=cadEquipmentMibConformance, auditStatus=auditStatus, auditLogThrottle=auditLogThrottle, equipmentAudit=equipmentAudit, portTxFlowControlMode=portTxFlowControlMode, cardAction=cardAction, cardAdminState=cardAdminState, cardTestDescription=cardTestDescription, cardTrapInh=cardTrapInh, portAction=portAction, portMaxDsPower=portMaxDsPower, cardSubDetected=cardSubDetected, TestScheduleCommand=TestScheduleCommand, shelfObjects=shelfObjects, diskVolumeDriveId=diskVolumeDriveId, cardTestTable=cardTestTable, systemGeneralGroup=systemGeneralGroup, portTrapInh=portTrapInh, portRxFlowControlMode=portRxFlowControlMode, cardShelfId=cardShelfId, cardFwVersion=cardFwVersion, auditLogOutput=auditLogOutput, auditTable=auditTable, auditShelfId=auditShelfId, cardTempNoReportNotification=cardTempNoReportNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestScheduleCommand=cardTestScheduleCommand, portCardId=portCardId, cardPicHwVersion=cardPicHwVersion, cardTable=cardTable, TestSchedule=TestSchedule, portDocsIfIndex=portDocsIfIndex, diskVolumeFileName=diskVolumeFileName, cardFwUpdateStatus=cardFwUpdateStatus, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestId=TestId, cmRegistrationNotification=cmRegistrationNotification, cardLastBootSource=cardLastBootSource, cardCarrierSerialNum=cardCarrierSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, cardSpareGroupId=cardSpareGroupId, diskVolumeEntry=diskVolumeEntry, auditTime=auditTime, equipmentShelfGroup=equipmentShelfGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, errEventId=errEventId, cardBusSpeed=cardBusSpeed, cardId=cardId, cardLastChangeTime=cardLastChangeTime, equipmentAuditGroup=equipmentAuditGroup, cardSecStateChange=cardSecStateChange, cardTestTime=cardTestTime, cardDetectedChange=cardDetectedChange, cmResetNotification=cmResetNotification, cardCarrierHwVersion=cardCarrierHwVersion, verboseLevel=verboseLevel, equipmentTbl=equipmentTbl, errEvRecoveryEnabled=errEvRecoveryEnabled)
