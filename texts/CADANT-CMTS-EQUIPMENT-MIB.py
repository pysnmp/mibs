...
PortDetectedMode, DuplexStatus, ShelfId, PortId, UnknownState, PrimaryState, SecondaryState, CardType, PortMode, CardId, DiskVolumeUsageLevel, FlowControlMode, EqActionType, CardSubType, AdminState, PortType, PicType, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "DuplexStatus", "ShelfId", "PortId", "UnknownState", "PrimaryState", "SecondaryState", "CardType", "PortMode", "CardId", "DiskVolumeUsageLevel", "FlowControlMode", "EqActionType", "CardSubType", "AdminState", "PortType", "PicType", "FirmwareSource")
...
DisplayString, RowStatus, TextualConvention, DateAndTime, MacAddress, TimeStamp, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "RowStatus", "TextualConvention", "DateAndTime", "MacAddress", "TimeStamp", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", PYSNMP_MODULE_ID=cadEquipmentMib, auditTotalPassedCount=auditTotalPassedCount, auditName=auditName, cardTestDescription=cardTestDescription, cardDplxStatus=cardDplxStatus, errEvLogLevel=errEvLogLevel, cardRamSize=cardRamSize, cardNumPorts=cardNumPorts, errEvLoggingEnabled=errEvLoggingEnabled, cmResetClearNotification=cmResetClearNotification, portCardSubType=portCardSubType, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetNotification=cmResetNotification, cardAction=cardAction, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portCurrDsPower=portCurrDsPower, cardTestType=cardTestType, equipmentTbl=equipmentTbl, errEventId=errEventId, systemKey=systemKey, cardTempNoReportNotification=cardTempNoReportNotification, portNumChans=portNumChans, portRxFlowControlDetected=portRxFlowControlDetected, cardLastBootSource=cardLastBootSource, portTxFlowControlDetected=portTxFlowControlDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestSchedule=cardTestSchedule, auditCommand=auditCommand, portCardId=portCardId, cardEntry=cardEntry, cardTemperatureHighError=cardTemperatureHighError, cardFwVersion=cardFwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, portDplxStatusChange=portDplxStatusChange, fpgaErrorEventTable=fpgaErrorEventTable, cardName=cardName, portTrapInh=portTrapInh, diskVolumeFileName=diskVolumeFileName, cadEquipmentMibConformance=cadEquipmentMibConformance, shelfSwVersion=shelfSwVersion, cardBusSpeed=cardBusSpeed, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetStatus=cmResetStatus, cardShelfTestIndex=cardShelfTestIndex, diskVolumeDiskSize=diskVolumeDiskSize, TestType=TestType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardDplxStatusChange=cardDplxStatusChange, cardLastBootVersion=cardLastBootVersion, cardAdminState=cardAdminState, consoleOutput=consoleOutput, cmRegistrationNotification=cmRegistrationNotification, TestId=TestId, portMode=portMode, cmDevice=cmDevice, diskVolumeShelfId=diskVolumeShelfId, cardTemperature=cardTemperature, portId=portId, diskVolumeName=diskVolumeName, cardHwVersion=cardHwVersion, cardTestResultDesc=cardTestResultDesc, auditResult=auditResult, cardTestTime=cardTestTime, shelfObjects=shelfObjects, cardPrState=cardPrState, cardDetected=cardDetected, portDescription=portDescription, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestResultNotification=cardTestResultNotification, portMinDsPower=portMinDsPower, portAction=portAction, diskVolumeUsageLevel=diskVolumeUsageLevel, auditLogThrottle=auditLogThrottle, portPrState=portPrState, cardType=cardType, portMacAddress=portMacAddress, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPicHwVersion=cardPicHwVersion, cardCpuSpeed=cardCpuSpeed, TestScheduleCommand=TestScheduleCommand, auditId=auditId, cmMacAddress=cmMacAddress, cmIpAddress=cmIpAddress, auditTable=auditTable, auditTime=auditTime, auditDescription=auditDescription, cardSpareGroupId=cardSpareGroupId, cardFlashSize=cardFlashSize, cardTemperatureHighWarn=cardTemperatureHighWarn, eqDiagConfig=eqDiagConfig, cardSubType=cardSubType, errEvRowStatus=errEvRowStatus, cardFwUpdateStatus=cardFwUpdateStatus, systemClock=systemClock, cardSecStateChange=cardSecStateChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portTxFlowControlMode=portTxFlowControlMode, cardLastResetReason=cardLastResetReason, diagTestId=diagTestId, equipmentTraps=equipmentTraps, cmPrimarySID=cmPrimarySID, equipmentDiag=equipmentDiag, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageNotification=diskVolumeUsageNotification, TestTransactionId=TestTransactionId, cardTestResult=cardTestResult, equipmentAuditGroup=equipmentAuditGroup, cardId=cardId, cardPicDetected=cardPicDetected, remainInDiagMode=remainInDiagMode, cardBootVersion=cardBootVersion, diskVolumeTable=diskVolumeTable, systemGeneral=systemGeneral, cardLastChangeTime=cardLastChangeTime, portEntry=portEntry, cardDetectedChange=cardDetectedChange, diskVolumeGroup=diskVolumeGroup, cardSwVersion=cardSwVersion, cardTestTable=cardTestTable, cardTrapInh=cardTrapInh, diskVolumeDriveId=diskVolumeDriveId, cardTable=cardTable, portMacIfIndex=portMacIfIndex, portRxFlowControlMode=portRxFlowControlMode, cardTestTransId=cardTestTransId, cardSerialNum=cardSerialNum, cardFpgaSource=cardFpgaSource, portType=portType, equipmentShelfGroup=equipmentShelfGroup, shelfName=shelfName, cardTestId=cardTestId, TestSchedule=TestSchedule, portMaxDsPower=portMaxDsPower, auditPassedCount=auditPassedCount, portConnectorId=portConnectorId, cardPrStateChange=cardPrStateChange, downstreamPowerLoss=downstreamPowerLoss, cardShelfId=cardShelfId, equipmentState=equipmentState, portDetectedMode=portDetectedMode, portDplxStatus=portDplxStatus, trapSeverity=trapSeverity, verboseLevel=verboseLevel, diskVolumeEntry=diskVolumeEntry, cardUpTime=cardUpTime, cardTestIndex=cardTestIndex, portShelfId=portShelfId, trapCounter=trapCounter, cmVendor=cmVendor, cmResetUpTime=cmResetUpTime, cardSecState=cardSecState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portTable=portTable, cardCarrierSerialNum=cardCarrierSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portAdminState=portAdminState, cardTestEntry=cardTestEntry, equipmentDiagGroup=equipmentDiagGroup, auditCardId=auditCardId, portPrStateChange=portPrStateChange, equipmentCardTestGroup=equipmentCardTestGroup, portSecState=portSecState, cmUChannelID=cmUChannelID, cadEquipmentMib=cadEquipmentMib, cardNumber=cardNumber, cardSpareGroupMode=cardSpareGroupMode, equipmentCardGroup=equipmentCardGroup, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeNumber=diskVolumeNumber, portDocsIfIndex=portDocsIfIndex, equipmentAudit=equipmentAudit, shelfNumber=shelfNumber, cardTestName=cardTestName, auditLogOutput=auditLogOutput, cardCPLDVersion=cardCPLDVersion, auditAutoScheduling=auditAutoScheduling, cmResetInfo=cmResetInfo, portBgpId=portBgpId, TestResult=TestResult, cardCpuType=cardCpuType, equipmentPortGroup=equipmentPortGroup, portLastChangeTime=portLastChangeTime, cardTempOverHeatNotification=cardTempOverHeatNotification, auditShelfId=auditShelfId, systemGeneralGroup=systemGeneralGroup, auditFailedCount=auditFailedCount, portSecStateChange=portSecStateChange, cardTestCommand=cardTestCommand, cmResetReason=cmResetReason, auditCycleCount=auditCycleCount, diskVolumeTrapInh=diskVolumeTrapInh, diskDriveNumber=diskDriveNumber, TestCommand=TestCommand, equipmentStateGroup=equipmentStateGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeSize=diskVolumeSize, cardPicSerialNum=cardPicSerialNum, diskVolumeId=diskVolumeId, auditEntry=auditEntry, equipmentError=equipmentError, diskVolumeCardId=diskVolumeCardId, auditStatus=auditStatus, portNumber=portNumber, cardSubDetected=cardSubDetected)
