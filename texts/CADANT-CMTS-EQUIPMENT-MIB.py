...
UnknownState, CardType, PicType, SecondaryState, FirmwareSource, PortId, PrimaryState, EqActionType, AdminState, DuplexStatus, FlowControlMode, PortMode, PortDetectedMode, ShelfId, DiskVolumeUsageLevel, CardId, CardSubType, PortType = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "CardType", "PicType", "SecondaryState", "FirmwareSource", "PortId", "PrimaryState", "EqActionType", "AdminState", "DuplexStatus", "FlowControlMode", "PortMode", "PortDetectedMode", "ShelfId", "DiskVolumeUsageLevel", "CardId", "CardSubType", "PortType")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCarrierHwVersion=cardCarrierHwVersion, auditTime=auditTime, cardFpgaSource=cardFpgaSource, cardEntry=cardEntry, portMode=portMode, portNumber=portNumber, auditShelfId=auditShelfId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardUpTime=cardUpTime, TestScheduleCommand=TestScheduleCommand, cardTestType=cardTestType, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestTransId=cardTestTransId, cardTemperature=cardTemperature, portCurrDsPower=portCurrDsPower, cardPrState=cardPrState, cardSerialNum=cardSerialNum, diskVolumeName=diskVolumeName, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierFwVersion=cardCarrierFwVersion, equipmentShelfGroup=equipmentShelfGroup, cardFwVersion=cardFwVersion, cardType=cardType, cardSecStateChange=cardSecStateChange, errEventId=errEventId, cardTable=cardTable, cmIpAddress=cmIpAddress, diskVolumeDriveId=diskVolumeDriveId, diagTestId=diagTestId, portTxFlowControlMode=portTxFlowControlMode, cmDevice=cmDevice, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeFileName=diskVolumeFileName, diskVolumeShelfId=diskVolumeShelfId, diskVolumeCardId=diskVolumeCardId, cardCarrierSerialNum=cardCarrierSerialNum, portPrState=portPrState, cmPrimarySID=cmPrimarySID, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSwVersion=cardSwVersion, cardSpareGroupId=cardSpareGroupId, portAction=portAction, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestIndex=cardTestIndex, portTable=portTable, cmResetStatus=cmResetStatus, portDocsIfIndex=portDocsIfIndex, TestId=TestId, auditTotalFailedCount=auditTotalFailedCount, portPrStateChange=portPrStateChange, auditFailedCount=auditFailedCount, cmUChannelID=cmUChannelID, cardTempOverHeatNotification=cardTempOverHeatNotification, auditPassedCount=auditPassedCount, cadEquipmentMib=cadEquipmentMib, systemKey=systemKey, portCardSubType=portCardSubType, portDetectedMode=portDetectedMode, auditTotalPassedCount=auditTotalPassedCount, cardSubType=cardSubType, portTxFlowControlDetected=portTxFlowControlDetected, auditLogThrottle=auditLogThrottle, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardId=cardId, cardSpareGroupMode=cardSpareGroupMode, cardTestDescription=cardTestDescription, cardSecState=cardSecState, fpgaErrorEventTable=fpgaErrorEventTable, portSecState=portSecState, cardTestId=cardTestId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmRegistrationNotification=cmRegistrationNotification, portMacIfIndex=portMacIfIndex, diskVolumeUsagePercentage=diskVolumeUsagePercentage, shelfName=shelfName, equipmentTraps=equipmentTraps, systemClock=systemClock, equipmentStateGroup=equipmentStateGroup, cardPicDetected=cardPicDetected, cardShelfTestIndex=cardShelfTestIndex, cardTestSchedule=cardTestSchedule, trapSeverity=trapSeverity, cardLastBootSource=cardLastBootSource, cardNumber=cardNumber, portRxFlowControlMode=portRxFlowControlMode, equipmentAudit=equipmentAudit, TestResult=TestResult, diskVolumeSize=diskVolumeSize, equipmentState=equipmentState, portNumChans=portNumChans, cardTestResultNotification=cardTestResultNotification, portMacAddress=portMacAddress, cmResetReason=cmResetReason, auditId=auditId, auditEntry=auditEntry, equipmentError=equipmentError, fpgaErrorEventEntry=fpgaErrorEventEntry, cardAdminState=cardAdminState, diskVolumeGroup=diskVolumeGroup, cardTestEntry=cardTestEntry, auditDescription=auditDescription, errEvLoggingEnabled=errEvLoggingEnabled, cardFwUpdateStatus=cardFwUpdateStatus, equipmentDiagGroup=equipmentDiagGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardAction=cardAction, auditResult=auditResult, downstreamPowerLoss=downstreamPowerLoss, cardTestTime=cardTestTime, portShelfId=portShelfId, portMinDsPower=portMinDsPower, equipmentTbl=equipmentTbl, cardDplxStatus=cardDplxStatus, cardLastResetReason=cardLastResetReason, cardTestTable=cardTestTable, cardTemperatureHighError=cardTemperatureHighError, remainInDiagMode=remainInDiagMode, TestSchedule=TestSchedule, shelfSwVersion=shelfSwVersion, auditCardId=auditCardId, cardBusSpeed=cardBusSpeed, cadEquipmentMibGroup=cadEquipmentMibGroup, TestCommand=TestCommand, auditName=auditName, cardTestScheduleCommand=cardTestScheduleCommand, portSecStateChange=portSecStateChange, diskDriveNumber=diskDriveNumber, cmResetUpTime=cmResetUpTime, cardFlashSize=cardFlashSize, TestTransactionId=TestTransactionId, shelfNumber=shelfNumber, cardBootVersion=cardBootVersion, eqDiagConfig=eqDiagConfig, cardDplxStatusChange=cardDplxStatusChange, diskVolumeId=diskVolumeId, cardShelfId=cardShelfId, portType=portType, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentAuditGroup=equipmentAuditGroup, cardDetectedChange=cardDetectedChange, trapCounter=trapCounter, cardName=cardName, verboseLevel=verboseLevel, auditCycleCount=auditCycleCount, equipmentCardGroup=equipmentCardGroup, auditCommand=auditCommand, cardDetected=cardDetected, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardLastBootVersion=cardLastBootVersion, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditLogOutput=auditLogOutput, auditStatus=auditStatus, portDplxStatus=portDplxStatus, cardPicHwVersion=cardPicHwVersion, errEvLogLevel=errEvLogLevel, fpgaErrorEventGroup=fpgaErrorEventGroup, cardNumPorts=cardNumPorts, auditAutoScheduling=auditAutoScheduling, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentPortGroup=equipmentPortGroup, consoleOutput=consoleOutput, cmResetInfo=cmResetInfo, portEntry=portEntry, cardCpuType=cardCpuType, errEvRowStatus=errEvRowStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardPrStateChange=cardPrStateChange, cardTestName=cardTestName, cmMacAddress=cmMacAddress, portId=portId, shelfObjects=shelfObjects, cardTrapInh=cardTrapInh, portTrapInh=portTrapInh, cardPicSerialNum=cardPicSerialNum, diskVolumeEntry=diskVolumeEntry, cardTestResult=cardTestResult, portLastChangeTime=portLastChangeTime, cmResetClearNotification=cmResetClearNotification, diskVolumeTable=diskVolumeTable, portMaxDsPower=portMaxDsPower, cmResetNotification=cmResetNotification, cardSubDetected=cardSubDetected, portBgpId=portBgpId, cardCPLDVersion=cardCPLDVersion, auditTable=auditTable, cardHwVersion=cardHwVersion, diskVolumeNumber=diskVolumeNumber, cmVendor=cmVendor, portDescription=portDescription, portAdminState=portAdminState, TestType=TestType, systemGeneralGroup=systemGeneralGroup, portConnectorId=portConnectorId, cardTestCommand=cardTestCommand, portCardId=portCardId, cardCpuSpeed=cardCpuSpeed, systemGeneral=systemGeneral, cardTestResultDesc=cardTestResultDesc, cardRamSize=cardRamSize, equipmentDiag=equipmentDiag, portDplxStatusChange=portDplxStatusChange, diskVolumeDiskSize=diskVolumeDiskSize)
