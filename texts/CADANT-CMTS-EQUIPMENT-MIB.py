...
CardType, DuplexStatus, PrimaryState, CardId, PortType, ShelfId, SecondaryState, UnknownState, PortMode, PicType, PortDetectedMode, AdminState, FirmwareSource, FlowControlMode, DiskVolumeUsageLevel, CardSubType, EqActionType, PortId = mibBuilder.importSymbols("CADANT-TC", "CardType", "DuplexStatus", "PrimaryState", "CardId", "PortType", "ShelfId", "SecondaryState", "UnknownState", "PortMode", "PicType", "PortDetectedMode", "AdminState", "FirmwareSource", "FlowControlMode", "DiskVolumeUsageLevel", "CardSubType", "EqActionType", "PortId")
...
DisplayString, MacAddress, RowStatus, TextualConvention, DateAndTime, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "RowStatus", "TextualConvention", "DateAndTime", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardAdminState=cardAdminState, systemKey=systemKey, portCardId=portCardId, auditCommand=auditCommand, diskVolumeDiskSize=diskVolumeDiskSize, portDplxStatusChange=portDplxStatusChange, shelfNumber=shelfNumber, cardHwVersion=cardHwVersion, auditTotalPassedCount=auditTotalPassedCount, portSecState=portSecState, cardTestEntry=cardTestEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardCarrierFwVersion=cardCarrierFwVersion, portId=portId, cardTemperatureHighError=cardTemperatureHighError, cardFwUpdateStatus=cardFwUpdateStatus, cardSpareGroupMode=cardSpareGroupMode, diskVolumeGroup=diskVolumeGroup, auditFailedCount=auditFailedCount, cardTestTransId=cardTestTransId, errEvRowStatus=errEvRowStatus, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, trapSeverity=trapSeverity, diskVolumeName=diskVolumeName, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestTable=cardTestTable, cardTestCommand=cardTestCommand, auditResult=auditResult, cmResetStatus=cmResetStatus, auditCycleCount=auditCycleCount, TestType=TestType, cardTestResultDesc=cardTestResultDesc, cardShelfId=cardShelfId, TestSchedule=TestSchedule, cardFwVersion=cardFwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentTraps=equipmentTraps, equipmentCardGroup=equipmentCardGroup, cardTestResult=cardTestResult, portType=portType, cardSpareGroupId=cardSpareGroupId, auditLogOutput=auditLogOutput, cardTemperature=cardTemperature, auditEntry=auditEntry, shelfName=shelfName, cmMacAddress=cmMacAddress, portRxFlowControlMode=portRxFlowControlMode, cmResetUpTime=cmResetUpTime, trapCounter=trapCounter, cardCarrierHwVersion=cardCarrierHwVersion, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardShelfTestIndex=cardShelfTestIndex, cardBootVersion=cardBootVersion, portMacAddress=portMacAddress, errEventId=errEventId, cardFpgaSource=cardFpgaSource, portMinDsPower=portMinDsPower, cardNumPorts=cardNumPorts, auditTime=auditTime, auditCardId=auditCardId, diagTestId=diagTestId, TestCommand=TestCommand, cardType=cardType, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentStateGroup=equipmentStateGroup, cmIpAddress=cmIpAddress, cardPicHwVersion=cardPicHwVersion, portTxFlowControlMode=portTxFlowControlMode, cmResetInfo=cmResetInfo, diskDriveNumber=diskDriveNumber, portMacIfIndex=portMacIfIndex, cadEquipmentMib=cadEquipmentMib, portMode=portMode, portRxFlowControlDetected=portRxFlowControlDetected, TestId=TestId, equipmentError=equipmentError, cardRamSize=cardRamSize, cmDevice=cmDevice, cardSwVersion=cardSwVersion, portNumber=portNumber, cardTestDescription=cardTestDescription, systemGeneral=systemGeneral, portAdminState=portAdminState, cardTestTime=cardTestTime, cardUpTime=cardUpTime, diskVolumeShelfId=diskVolumeShelfId, cardPrStateChange=cardPrStateChange, diskVolumeFileName=diskVolumeFileName, remainInDiagMode=remainInDiagMode, cardAction=cardAction, TestTransactionId=TestTransactionId, cardEntry=cardEntry, diskVolumeId=diskVolumeId, cardSubType=cardSubType, portCurrDsPower=portCurrDsPower, auditStatus=auditStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, cmPrimarySID=cmPrimarySID, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDplxStatus=cardDplxStatus, cardCPLDVersion=cardCPLDVersion, cmResetClearNotification=cmResetClearNotification, cardLastChangeTime=cardLastChangeTime, cardFlashSize=cardFlashSize, portEntry=portEntry, portLastChangeTime=portLastChangeTime, diskVolumeSize=diskVolumeSize, cardSecStateChange=cardSecStateChange, cardName=cardName, portDocsIfIndex=portDocsIfIndex, auditPassedCount=auditPassedCount, diskVolumeNumber=diskVolumeNumber, cardTemperatureHighWarn=cardTemperatureHighWarn, portBgpId=portBgpId, auditAutoScheduling=auditAutoScheduling, auditShelfId=auditShelfId, equipmentTbl=equipmentTbl, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, portDescription=portDescription, equipmentAuditGroup=equipmentAuditGroup, verboseLevel=verboseLevel, shelfSwVersion=shelfSwVersion, cardTestResultNotification=cardTestResultNotification, equipmentDiagGroup=equipmentDiagGroup, cardPicDetected=cardPicDetected, cardTable=cardTable, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentCardTestGroup=equipmentCardTestGroup, portPrStateChange=portPrStateChange, portShelfId=portShelfId, cardTempNoReportNotification=cardTempNoReportNotification, cmUChannelID=cmUChannelID, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditId=auditId, portMaxDsPower=portMaxDsPower, diskVolumeEntry=diskVolumeEntry, fpgaErrorEventEntry=fpgaErrorEventEntry, portCardSubType=portCardSubType, shelfObjects=shelfObjects, portConnectorId=portConnectorId, cardLastResetReason=cardLastResetReason, cardLastBootVersion=cardLastBootVersion, portAction=portAction, errEvLogLevel=errEvLogLevel, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentState=equipmentState, portDetectedMode=portDetectedMode, cmResetNotification=cmResetNotification, consoleOutput=consoleOutput, cardLastBootSource=cardLastBootSource, TestScheduleCommand=TestScheduleCommand, cmVendor=cmVendor, diskVolumeTable=diskVolumeTable, cardDetectedChange=cardDetectedChange, equipmentAudit=equipmentAudit, equipmentShelfGroup=equipmentShelfGroup, cardSerialNum=cardSerialNum, systemGeneralGroup=systemGeneralGroup, portPrState=portPrState, eqDiagConfig=eqDiagConfig, cardTestType=cardTestType, cardId=cardId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardNumber=cardNumber, TestResult=TestResult, portSecStateChange=portSecStateChange, cardDplxStatusChange=cardDplxStatusChange, cardTrapInh=cardTrapInh, cardSubDetected=cardSubDetected, cardSecState=cardSecState, downstreamPowerLoss=downstreamPowerLoss, cardCarrierSerialNum=cardCarrierSerialNum, auditTotalFailedCount=auditTotalFailedCount, errEvLoggingEnabled=errEvLoggingEnabled, cardTestSchedule=cardTestSchedule, auditDescription=auditDescription, portTrapInh=portTrapInh, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardBusSpeed=cardBusSpeed, auditTable=auditTable, fpgaErrorEventTable=fpgaErrorEventTable, cardDetected=cardDetected, cardTestId=cardTestId, diskVolumeCardId=diskVolumeCardId, auditLogThrottle=auditLogThrottle, cardTestName=cardTestName, cardPrState=cardPrState, cardCpuSpeed=cardCpuSpeed, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentPortGroup=equipmentPortGroup, portDplxStatus=portDplxStatus, cmResetReason=cmResetReason, cardTestIndex=cardTestIndex, portTable=portTable, cmRegistrationNotification=cmRegistrationNotification, systemClock=systemClock, portNumChans=portNumChans, diskVolumeDriveId=diskVolumeDriveId, cardCpuType=cardCpuType, cardTestScheduleCommand=cardTestScheduleCommand, auditName=auditName, equipmentDiag=equipmentDiag, diskVolumeTrapInh=diskVolumeTrapInh)
