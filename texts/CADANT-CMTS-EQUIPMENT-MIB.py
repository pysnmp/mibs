...
PortDetectedMode, PortMode, PicType, ShelfId, UnknownState, FirmwareSource, PrimaryState, AdminState, PortId, EqActionType, DuplexStatus, PortType, SecondaryState, CardId, CardType, FlowControlMode, DiskVolumeUsageLevel, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "PortMode", "PicType", "ShelfId", "UnknownState", "FirmwareSource", "PrimaryState", "AdminState", "PortId", "EqActionType", "DuplexStatus", "PortType", "SecondaryState", "CardId", "CardType", "FlowControlMode", "DiskVolumeUsageLevel", "CardSubType")
...
TruthValue, RowStatus, DisplayString, DateAndTime, MacAddress, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "DisplayString", "DateAndTime", "MacAddress", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfName=shelfName, cardType=cardType, portCurrDsPower=portCurrDsPower, consoleOutput=consoleOutput, verboseLevel=verboseLevel, cardFwVersion=cardFwVersion, cadEquipmentMib=cadEquipmentMib, auditTotalPassedCount=auditTotalPassedCount, portType=portType, portDplxStatus=portDplxStatus, portTrapInh=portTrapInh, cardTestTable=cardTestTable, cardCPLDVersion=cardCPLDVersion, portNumber=portNumber, auditPassedCount=auditPassedCount, cardTestSchedule=cardTestSchedule, portEntry=portEntry, auditTotalFailedCount=auditTotalFailedCount, cardDplxStatusChange=cardDplxStatusChange, cardPicHwVersion=cardPicHwVersion, cmUChannelID=cmUChannelID, diskVolumeDiskSize=diskVolumeDiskSize, errEvRecoveryEnabled=errEvRecoveryEnabled, cardName=cardName, diskVolumeSize=diskVolumeSize, cmResetInfo=cmResetInfo, equipmentState=equipmentState, portPrStateChange=portPrStateChange, portAction=portAction, cardTemperatureHighWarn=cardTemperatureHighWarn, fpgaErrorEventGroup=fpgaErrorEventGroup, errEvLogLevel=errEvLogLevel, cardTestIndex=cardTestIndex, equipmentTbl=equipmentTbl, cardTempOverHeatNotification=cardTempOverHeatNotification, diagTestId=diagTestId, TestCommand=TestCommand, cadEquipmentMibConformance=cadEquipmentMibConformance, auditCardId=auditCardId, TestType=TestType, auditResult=auditResult, errEvLoggingEnabled=errEvLoggingEnabled, equipmentAudit=equipmentAudit, cmResetStatus=cmResetStatus, portDescription=portDescription, cardRamSize=cardRamSize, cardUpTime=cardUpTime, portCardSubType=portCardSubType, cardTestResult=cardTestResult, cardNumber=cardNumber, TestScheduleCommand=TestScheduleCommand, diskVolumeName=diskVolumeName, cardTestEntry=cardTestEntry, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditDescription=auditDescription, cardTestResultNotification=cardTestResultNotification, cardSecStateChange=cardSecStateChange, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeShelfId=diskVolumeShelfId, TestResult=TestResult, cardTestType=cardTestType, auditFailedCount=auditFailedCount, portId=portId, cardDetected=cardDetected, portMaxDsPower=portMaxDsPower, downstreamPowerLoss=downstreamPowerLoss, equipmentAuditGroup=equipmentAuditGroup, cardAdminState=cardAdminState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFlashSize=cardFlashSize, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, portShelfId=portShelfId, cardFpgaSource=cardFpgaSource, portDocsIfIndex=portDocsIfIndex, cmResetReason=cmResetReason, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTrapInh=cardTrapInh, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditStatus=auditStatus, remainInDiagMode=remainInDiagMode, errEventId=errEventId, cardDetectedChange=cardDetectedChange, cmResetUpTime=cmResetUpTime, equipmentDiagGroup=equipmentDiagGroup, equipmentDiag=equipmentDiag, shelfNumber=shelfNumber, cardHwVersion=cardHwVersion, portMinDsPower=portMinDsPower, equipmentError=equipmentError, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditId=auditId, portMacIfIndex=portMacIfIndex, portLastChangeTime=portLastChangeTime, auditName=auditName, portTxFlowControlDetected=portTxFlowControlDetected, cmResetClearNotification=cmResetClearNotification, portDplxStatusChange=portDplxStatusChange, cardTestId=cardTestId, cardCpuType=cardCpuType, portAdminState=portAdminState, cardLastBootVersion=cardLastBootVersion, cardShelfId=cardShelfId, cardId=cardId, cardSubType=cardSubType, diskVolumeFileName=diskVolumeFileName, equipmentCardGroup=equipmentCardGroup, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeTrapInh=diskVolumeTrapInh, shelfSwVersion=shelfSwVersion, cardAction=cardAction, TestId=TestId, cardSubDetected=cardSubDetected, cardCarrierFwVersion=cardCarrierFwVersion, cardTestDescription=cardTestDescription, cardCpuSpeed=cardCpuSpeed, diskVolumeId=diskVolumeId, cardSpareGroupId=cardSpareGroupId, diskVolumeEntry=diskVolumeEntry, portConnectorId=portConnectorId, auditEntry=auditEntry, auditAutoScheduling=auditAutoScheduling, auditTime=auditTime, cadEquipmentMibGroup=cadEquipmentMibGroup, portDetectedMode=portDetectedMode, auditCycleCount=auditCycleCount, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardEntry=cardEntry, portCardId=portCardId, TestTransactionId=TestTransactionId, trapSeverity=trapSeverity, cardBootVersion=cardBootVersion, cardTestName=cardTestName, auditLogThrottle=auditLogThrottle, diskVolumeTable=diskVolumeTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeNumber=diskVolumeNumber, portMacAddress=portMacAddress, cmVendor=cmVendor, portMode=portMode, cardBusSpeed=cardBusSpeed, equipmentStateGroup=equipmentStateGroup, shelfObjects=shelfObjects, errEvRowStatus=errEvRowStatus, portRxFlowControlDetected=portRxFlowControlDetected, portSecState=portSecState, cardTestTime=cardTestTime, cmDevice=cmDevice, cardLastResetReason=cardLastResetReason, cardCarrierSerialNum=cardCarrierSerialNum, equipmentPortGroup=equipmentPortGroup, cardTemperatureHighError=cardTemperatureHighError, cmResetNotification=cmResetNotification, cardNumPorts=cardNumPorts, portSecStateChange=portSecStateChange, cardTemperature=cardTemperature, cardPicDetected=cardPicDetected, eqDiagConfig=eqDiagConfig, PYSNMP_MODULE_ID=cadEquipmentMib, auditShelfId=auditShelfId, cardLastBootSource=cardLastBootSource, cardTestCommand=cardTestCommand, TestSchedule=TestSchedule, trapCounter=trapCounter, diskVolumeDriveId=diskVolumeDriveId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCommand=auditCommand, portNumChans=portNumChans, cardSecState=cardSecState, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTable=cardTable, systemKey=systemKey, cmPrimarySID=cmPrimarySID, portPrState=portPrState, diskVolumeCardId=diskVolumeCardId, equipmentTraps=equipmentTraps, portTable=portTable, diskDriveNumber=diskDriveNumber, cardSerialNum=cardSerialNum, cardSpareGroupMode=cardSpareGroupMode, cmIpAddress=cmIpAddress, cardDplxStatus=cardDplxStatus, cardPrStateChange=cardPrStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, auditTable=auditTable, cardPrState=cardPrState, equipmentShelfGroup=equipmentShelfGroup, systemGeneral=systemGeneral, portTxFlowControlMode=portTxFlowControlMode, portRxFlowControlMode=portRxFlowControlMode, portBgpId=portBgpId, cardTestScheduleCommand=cardTestScheduleCommand, cardLastChangeTime=cardLastChangeTime, cardTestResultDesc=cardTestResultDesc, cmRegistrationNotification=cmRegistrationNotification, systemClock=systemClock, cardPicSerialNum=cardPicSerialNum, auditLogOutput=auditLogOutput, cardSwVersion=cardSwVersion, cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeGroup=diskVolumeGroup, cardTestTransId=cardTestTransId, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierHwVersion=cardCarrierHwVersion, cmMacAddress=cmMacAddress)
