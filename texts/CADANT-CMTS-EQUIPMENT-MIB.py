...
PortType, PrimaryState, PortDetectedMode, CardType, DiskVolumeUsageLevel, ShelfId, PortId, CardSubType, FirmwareSource, AdminState, FlowControlMode, PicType, SecondaryState, EqActionType, CardId, DuplexStatus, PortMode, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PortType", "PrimaryState", "PortDetectedMode", "CardType", "DiskVolumeUsageLevel", "ShelfId", "PortId", "CardSubType", "FirmwareSource", "AdminState", "FlowControlMode", "PicType", "SecondaryState", "EqActionType", "CardId", "DuplexStatus", "PortMode", "UnknownState")
...
TextualConvention, MacAddress, DateAndTime, DisplayString, RowStatus, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "DateAndTime", "DisplayString", "RowStatus", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentDiag=equipmentDiag, equipmentCardGroup=equipmentCardGroup, cardTestIndex=cardTestIndex, portCardSubType=portCardSubType, cardName=cardName, cmPrimarySID=cmPrimarySID, TestScheduleCommand=TestScheduleCommand, auditTime=auditTime, cardDplxStatus=cardDplxStatus, auditResult=auditResult, cardSpareGroupId=cardSpareGroupId, cardTestTime=cardTestTime, trapSeverity=trapSeverity, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSpareGroupMode=cardSpareGroupMode, equipmentCardTestGroup=equipmentCardTestGroup, shelfNumber=shelfNumber, diskVolumeCardId=diskVolumeCardId, errEvRowStatus=errEvRowStatus, fpgaErrorEventTable=fpgaErrorEventTable, cardFlashSize=cardFlashSize, cardDetectedChange=cardDetectedChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, shelfSwVersion=shelfSwVersion, auditCycleCount=auditCycleCount, fpgaErrorEventGroup=fpgaErrorEventGroup, auditTable=auditTable, cardCpuSpeed=cardCpuSpeed, cardTestResultDesc=cardTestResultDesc, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeNumber=diskVolumeNumber, auditDescription=auditDescription, cardLastBootSource=cardLastBootSource, auditCardId=auditCardId, cardType=cardType, cardPicSerialNum=cardPicSerialNum, portDescription=portDescription, cardTestName=cardTestName, diskVolumeSize=diskVolumeSize, cmIpAddress=cmIpAddress, cardDetected=cardDetected, diskVolumeGroup=diskVolumeGroup, eqDiagConfig=eqDiagConfig, cardFpgaSource=cardFpgaSource, cardCpuType=cardCpuType, cardTemperatureHighError=cardTemperatureHighError, cardAction=cardAction, portNumber=portNumber, cardPrStateChange=cardPrStateChange, portEntry=portEntry, cardDplxStatusChange=cardDplxStatusChange, cardPrState=cardPrState, systemGeneralGroup=systemGeneralGroup, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, portPrStateChange=portPrStateChange, cardTestScheduleCommand=cardTestScheduleCommand, equipmentStateGroup=equipmentStateGroup, cardBusSpeed=cardBusSpeed, portShelfId=portShelfId, cardSecStateChange=cardSecStateChange, equipmentTbl=equipmentTbl, portType=portType, cardAdminState=cardAdminState, cardEntry=cardEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditLogThrottle=auditLogThrottle, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditTotalFailedCount=auditTotalFailedCount, auditAutoScheduling=auditAutoScheduling, auditId=auditId, portTrapInh=portTrapInh, equipmentError=equipmentError, TestType=TestType, TestSchedule=TestSchedule, cardTestEntry=cardTestEntry, cardRamSize=cardRamSize, cardShelfTestIndex=cardShelfTestIndex, equipmentShelfGroup=equipmentShelfGroup, cardFwVersion=cardFwVersion, cardSerialNum=cardSerialNum, auditPassedCount=auditPassedCount, cardTestId=cardTestId, cardCarrierHwVersion=cardCarrierHwVersion, TestId=TestId, systemGeneral=systemGeneral, portCardId=portCardId, cardFwUpdateStatus=cardFwUpdateStatus, shelfObjects=shelfObjects, portSecState=portSecState, portConnectorId=portConnectorId, auditStatus=auditStatus, portNumChans=portNumChans, diskVolumeDriveId=diskVolumeDriveId, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditFailedCount=auditFailedCount, auditTotalPassedCount=auditTotalPassedCount, portDplxStatusChange=portDplxStatusChange, systemKey=systemKey, diskVolumeName=diskVolumeName, cardPicHwVersion=cardPicHwVersion, cadEquipmentMib=cadEquipmentMib, auditShelfId=auditShelfId, downstreamPowerLoss=downstreamPowerLoss, cardSubDetected=cardSubDetected, portMaxDsPower=portMaxDsPower, cardLastChangeTime=cardLastChangeTime, cardShelfId=cardShelfId, portMode=portMode, cadEquipmentMibConformance=cadEquipmentMibConformance, cardNumPorts=cardNumPorts, cardTempNoReportNotification=cardTempNoReportNotification, trapCounter=trapCounter, cmResetStatus=cmResetStatus, remainInDiagMode=remainInDiagMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCPLDVersion=cardCPLDVersion, auditLogOutput=auditLogOutput, shelfName=shelfName, portMinDsPower=portMinDsPower, equipmentPortGroup=equipmentPortGroup, portMacAddress=portMacAddress, portDocsIfIndex=portDocsIfIndex, cardSwVersion=cardSwVersion, auditEntry=auditEntry, diskVolumeDiskSize=diskVolumeDiskSize, equipmentAudit=equipmentAudit, portId=portId, TestCommand=TestCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCarrierFwVersion=cardCarrierFwVersion, portSecStateChange=portSecStateChange, cmUChannelID=cmUChannelID, cmResetClearNotification=cmResetClearNotification, diskVolumeEntry=diskVolumeEntry, portPrState=portPrState, portTable=portTable, errEventId=errEventId, cardId=cardId, cmResetReason=cmResetReason, portRxFlowControlMode=portRxFlowControlMode, portDetectedMode=portDetectedMode, cardNumber=cardNumber, equipmentTraps=equipmentTraps, cmResetUpTime=cmResetUpTime, consoleOutput=consoleOutput, auditName=auditName, errEvLoggingEnabled=errEvLoggingEnabled, cardTempOverHeatNotification=cardTempOverHeatNotification, diagTestId=diagTestId, cmResetNotification=cmResetNotification, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, errEvLogLevel=errEvLogLevel, diskVolumeShelfId=diskVolumeShelfId, cardTestType=cardTestType, portMacIfIndex=portMacIfIndex, cmDevice=cmDevice, cmResetInfo=cmResetInfo, portCurrDsPower=portCurrDsPower, cardHwVersion=cardHwVersion, portAction=portAction, diskVolumeTrapInh=diskVolumeTrapInh, cardTestDescription=cardTestDescription, cardPicDetected=cardPicDetected, portDplxStatus=portDplxStatus, diskVolumeId=diskVolumeId, equipmentAuditGroup=equipmentAuditGroup, cardTestResult=cardTestResult, cardTestTable=cardTestTable, cardTestSchedule=cardTestSchedule, diskVolumeFileName=diskVolumeFileName, cardBootVersion=cardBootVersion, TestTransactionId=TestTransactionId, equipmentState=equipmentState, cardTestTransId=cardTestTransId, cardLastResetReason=cardLastResetReason, cardUpTime=cardUpTime, PYSNMP_MODULE_ID=cadEquipmentMib, diskDriveNumber=diskDriveNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, cmMacAddress=cmMacAddress, cardSecState=cardSecState, cardTrapInh=cardTrapInh, portAdminState=portAdminState, cardTable=cardTable, TestResult=TestResult, cmVendor=cmVendor, cardSubType=cardSubType, cadEquipmentMibGroup=cadEquipmentMibGroup, auditCommand=auditCommand, portBgpId=portBgpId, diskVolumeTable=diskVolumeTable, cardTestResultNotification=cardTestResultNotification, equipmentDiagGroup=equipmentDiagGroup, portLastChangeTime=portLastChangeTime, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestCommand=cardTestCommand, verboseLevel=verboseLevel, cardLastBootVersion=cardLastBootVersion, systemClock=systemClock, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTemperature=cardTemperature)
