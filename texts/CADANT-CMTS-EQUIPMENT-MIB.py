...
CardType, CardId, AdminState, EqActionType, DuplexStatus, FirmwareSource, PortMode, UnknownState, PortDetectedMode, PrimaryState, DiskVolumeUsageLevel, CardSubType, PicType, ShelfId, PortType, PortId, FlowControlMode, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "CardType", "CardId", "AdminState", "EqActionType", "DuplexStatus", "FirmwareSource", "PortMode", "UnknownState", "PortDetectedMode", "PrimaryState", "DiskVolumeUsageLevel", "CardSubType", "PicType", "ShelfId", "PortType", "PortId", "FlowControlMode", "SecondaryState")
...
TruthValue, RowStatus, DisplayString, DateAndTime, TimeStamp, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "DisplayString", "DateAndTime", "TimeStamp", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastResetReason=cardLastResetReason, cardName=cardName, errEvLogLevel=errEvLogLevel, cardBootVersion=cardBootVersion, auditCycleCount=auditCycleCount, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeEntry=diskVolumeEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portPrState=portPrState, diskVolumeUsageNotification=diskVolumeUsageNotification, auditCardId=auditCardId, auditAutoScheduling=auditAutoScheduling, cardHwVersion=cardHwVersion, cardNumPorts=cardNumPorts, portShelfId=portShelfId, cmMacAddress=cmMacAddress, portMacIfIndex=portMacIfIndex, cardDetected=cardDetected, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastBootSource=cardLastBootSource, cardTestTransId=cardTestTransId, portRxFlowControlMode=portRxFlowControlMode, equipmentDiag=equipmentDiag, PYSNMP_MODULE_ID=cadEquipmentMib, cardSubType=cardSubType, portNumber=portNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, auditPassedCount=auditPassedCount, diskDriveNumber=diskDriveNumber, equipmentAuditGroup=equipmentAuditGroup, auditTable=auditTable, downstreamPowerLoss=downstreamPowerLoss, cardBusSpeed=cardBusSpeed, systemKey=systemKey, fpgaErrorEventTable=fpgaErrorEventTable, remainInDiagMode=remainInDiagMode, diskVolumeDriveId=diskVolumeDriveId, portMinDsPower=portMinDsPower, diskVolumeGroup=diskVolumeGroup, cardShelfTestIndex=cardShelfTestIndex, cardId=cardId, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMib=cadEquipmentMib, cardSubDetected=cardSubDetected, cardCPLDVersion=cardCPLDVersion, auditLogThrottle=auditLogThrottle, cardDetectedChange=cardDetectedChange, cardSwVersion=cardSwVersion, portDplxStatusChange=portDplxStatusChange, cardTestDescription=cardTestDescription, diskVolumeSize=diskVolumeSize, portTxFlowControlDetected=portTxFlowControlDetected, auditDescription=auditDescription, portSecStateChange=portSecStateChange, diskVolumeTable=diskVolumeTable, fpgaErrorEventGroup=fpgaErrorEventGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardCpuSpeed=cardCpuSpeed, cardEntry=cardEntry, portDetectedMode=portDetectedMode, portAdminState=portAdminState, equipmentPortGroup=equipmentPortGroup, systemGeneralGroup=systemGeneralGroup, cmResetNotification=cmResetNotification, equipmentCardGroup=equipmentCardGroup, cardAdminState=cardAdminState, portNumChans=portNumChans, cmDevice=cmDevice, auditFailedCount=auditFailedCount, trapSeverity=trapSeverity, portSecState=portSecState, diskVolumeNumber=diskVolumeNumber, cmResetInfo=cmResetInfo, TestType=TestType, cardTrapInh=cardTrapInh, cardCarrierHwVersion=cardCarrierHwVersion, cmIpAddress=cmIpAddress, portDplxStatus=portDplxStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardRamSize=cardRamSize, auditCommand=auditCommand, cmUChannelID=cmUChannelID, cardSpareGroupId=cardSpareGroupId, cardShelfId=cardShelfId, cardTable=cardTable, cardLastBootVersion=cardLastBootVersion, errEventId=errEventId, cardCarrierFwVersion=cardCarrierFwVersion, TestResult=TestResult, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portCardId=portCardId, cardTestResultNotification=cardTestResultNotification, cardFpgaSource=cardFpgaSource, cmResetStatus=cmResetStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, consoleOutput=consoleOutput, cardSerialNum=cardSerialNum, cardTestType=cardTestType, cardTestName=cardTestName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardType=cardType, cardUpTime=cardUpTime, cardNumber=cardNumber, equipmentAudit=equipmentAudit, auditTotalFailedCount=auditTotalFailedCount, portMacAddress=portMacAddress, TestSchedule=TestSchedule, cardSecStateChange=cardSecStateChange, portTrapInh=portTrapInh, cardTestResultDesc=cardTestResultDesc, equipmentShelfGroup=equipmentShelfGroup, cardTemperature=cardTemperature, auditName=auditName, portPrStateChange=portPrStateChange, cardPrState=cardPrState, portEntry=portEntry, auditStatus=auditStatus, shelfNumber=shelfNumber, cardPicSerialNum=cardPicSerialNum, equipmentCardTestGroup=equipmentCardTestGroup, cmPrimarySID=cmPrimarySID, diskVolumeCardId=diskVolumeCardId, trapCounter=trapCounter, cardTemperatureHighError=cardTemperatureHighError, errEvRowStatus=errEvRowStatus, auditId=auditId, portAction=portAction, shelfSwVersion=shelfSwVersion, shelfObjects=shelfObjects, portMode=portMode, portMaxDsPower=portMaxDsPower, diskVolumeFileName=diskVolumeFileName, cardPicDetected=cardPicDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentTraps=equipmentTraps, TestTransactionId=TestTransactionId, auditShelfId=auditShelfId, cmResetUpTime=cmResetUpTime, portDocsIfIndex=portDocsIfIndex, TestId=TestId, auditResult=auditResult, cardTestCommand=cardTestCommand, equipmentError=equipmentError, cardCpuType=cardCpuType, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestIndex=cardTestIndex, cardTestId=cardTestId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmRegistrationNotification=cmRegistrationNotification, portCurrDsPower=portCurrDsPower, systemGeneral=systemGeneral, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentState=equipmentState, diskVolumeShelfId=diskVolumeShelfId, cardCarrierSerialNum=cardCarrierSerialNum, portTxFlowControlMode=portTxFlowControlMode, portConnectorId=portConnectorId, portType=portType, diagTestId=diagTestId, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, cardDplxStatus=cardDplxStatus, cardTestEntry=cardTestEntry, cmVendor=cmVendor, cardTestScheduleCommand=cardTestScheduleCommand, auditTime=auditTime, errEvLoggingEnabled=errEvLoggingEnabled, systemClock=systemClock, equipmentStateGroup=equipmentStateGroup, diskVolumeDiskSize=diskVolumeDiskSize, cardTestTime=cardTestTime, cardTestResult=cardTestResult, diskVolumeName=diskVolumeName, equipmentTbl=equipmentTbl, cardTestTable=cardTestTable, portLastChangeTime=portLastChangeTime, cardSecState=cardSecState, portTable=portTable, cardSpareGroupMode=cardSpareGroupMode, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetReason=cmResetReason, portCardSubType=portCardSubType, cardPicHwVersion=cardPicHwVersion, auditLogOutput=auditLogOutput, verboseLevel=verboseLevel, auditEntry=auditEntry, cardAction=cardAction, auditTotalPassedCount=auditTotalPassedCount, cardFlashSize=cardFlashSize, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetClearNotification=cmResetClearNotification, cardFwVersion=cardFwVersion, shelfName=shelfName, diskVolumeId=diskVolumeId, TestScheduleCommand=TestScheduleCommand, cardTempOverHeatNotification=cardTempOverHeatNotification, portBgpId=portBgpId, cardTemperatureHighWarn=cardTemperatureHighWarn, TestCommand=TestCommand, portDescription=portDescription, eqDiagConfig=eqDiagConfig, portId=portId, cardDplxStatusChange=cardDplxStatusChange, cardPrStateChange=cardPrStateChange, cardTestSchedule=cardTestSchedule, cardFwUpdateStatus=cardFwUpdateStatus)
