...
SecondaryState, EqActionType, PrimaryState, CardId, ShelfId, CardSubType, DuplexStatus, DiskVolumeUsageLevel, FirmwareSource, PortMode, PortDetectedMode, AdminState, PortId, FlowControlMode, PortType, CardType, PicType, UnknownState = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "EqActionType", "PrimaryState", "CardId", "ShelfId", "CardSubType", "DuplexStatus", "DiskVolumeUsageLevel", "FirmwareSource", "PortMode", "PortDetectedMode", "AdminState", "PortId", "FlowControlMode", "PortType", "CardType", "PicType", "UnknownState")
...
TextualConvention, DateAndTime, TimeStamp, TruthValue, DisplayString, MacAddress, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DateAndTime", "TimeStamp", "TruthValue", "DisplayString", "MacAddress", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPrStateChange=cardPrStateChange, cardFpgaSource=cardFpgaSource, portNumChans=portNumChans, cardLastResetReason=cardLastResetReason, downstreamPowerLoss=downstreamPowerLoss, cardAction=cardAction, portBgpId=portBgpId, auditTable=auditTable, auditFailedCount=auditFailedCount, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDescription=portDescription, systemGeneralGroup=systemGeneralGroup, equipmentStateGroup=equipmentStateGroup, auditEntry=auditEntry, cardTestSchedule=cardTestSchedule, cardBootVersion=cardBootVersion, cardTestIndex=cardTestIndex, systemClock=systemClock, cardShelfId=cardShelfId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardPrState=cardPrState, portAction=portAction, eqDiagConfig=eqDiagConfig, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCpuType=cardCpuType, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentAudit=equipmentAudit, diskVolumeFileName=diskVolumeFileName, cardPicSerialNum=cardPicSerialNum, auditCardId=auditCardId, diskVolumeShelfId=diskVolumeShelfId, portCurrDsPower=portCurrDsPower, remainInDiagMode=remainInDiagMode, portTrapInh=portTrapInh, cmResetClearNotification=cmResetClearNotification, cardSwVersion=cardSwVersion, cardTestResultDesc=cardTestResultDesc, shelfObjects=shelfObjects, diskVolumeName=diskVolumeName, trapCounter=trapCounter, TestResult=TestResult, cardLastBootVersion=cardLastBootVersion, cmResetReason=cmResetReason, cmResetNotification=cmResetNotification, cadEquipmentMib=cadEquipmentMib, TestCommand=TestCommand, shelfNumber=shelfNumber, trapSeverity=trapSeverity, portMinDsPower=portMinDsPower, equipmentCardGroup=equipmentCardGroup, diskVolumeGroup=diskVolumeGroup, cmResetUpTime=cmResetUpTime, diskVolumeNumber=diskVolumeNumber, cmMacAddress=cmMacAddress, fpgaErrorEventGroup=fpgaErrorEventGroup, errEventId=errEventId, diskVolumeTable=diskVolumeTable, diskVolumeSize=diskVolumeSize, TestType=TestType, cardFwUpdateStatus=cardFwUpdateStatus, portPrStateChange=portPrStateChange, portRxFlowControlDetected=portRxFlowControlDetected, auditLogOutput=auditLogOutput, diskVolumeCardId=diskVolumeCardId, cardAdminState=cardAdminState, auditDescription=auditDescription, TestTransactionId=TestTransactionId, portDetectedMode=portDetectedMode, auditResult=auditResult, systemKey=systemKey, cadEquipmentMibGroup=cadEquipmentMibGroup, systemGeneral=systemGeneral, diskVolumeDriveId=diskVolumeDriveId, cmVendor=cmVendor, cardSpareGroupMode=cardSpareGroupMode, cardSubType=cardSubType, cardDplxStatus=cardDplxStatus, portTxFlowControlDetected=portTxFlowControlDetected, auditTime=auditTime, portAdminState=portAdminState, portRxFlowControlMode=portRxFlowControlMode, portTable=portTable, diskDriveNumber=diskDriveNumber, equipmentTbl=equipmentTbl, auditAutoScheduling=auditAutoScheduling, portSecStateChange=portSecStateChange, portCardSubType=portCardSubType, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestName=cardTestName, cardTempNoReportNotification=cardTempNoReportNotification, equipmentAuditGroup=equipmentAuditGroup, auditStatus=auditStatus, cardSubDetected=cardSubDetected, equipmentCardTestGroup=equipmentCardTestGroup, portMaxDsPower=portMaxDsPower, fpgaErrorEventTable=fpgaErrorEventTable, cardSecStateChange=cardSecStateChange, cardSerialNum=cardSerialNum, errEvLoggingEnabled=errEvLoggingEnabled, cardTrapInh=cardTrapInh, cardTestEntry=cardTestEntry, cardTemperatureHighError=cardTemperatureHighError, auditTotalPassedCount=auditTotalPassedCount, cardCPLDVersion=cardCPLDVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTemperature=cardTemperature, cardHwVersion=cardHwVersion, cardNumber=cardNumber, portShelfId=portShelfId, portDplxStatus=portDplxStatus, cardCpuSpeed=cardCpuSpeed, cmPrimarySID=cmPrimarySID, errEvRowStatus=errEvRowStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, portTxFlowControlMode=portTxFlowControlMode, cardType=cardType, portId=portId, portPrState=portPrState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDplxStatusChange=portDplxStatusChange, shelfSwVersion=shelfSwVersion, cardDetectedChange=cardDetectedChange, cardFwVersion=cardFwVersion, consoleOutput=consoleOutput, cardTestTransId=cardTestTransId, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, TestSchedule=TestSchedule, cardUpTime=cardUpTime, cardTemperatureHighWarn=cardTemperatureHighWarn, cardShelfTestIndex=cardShelfTestIndex, cardTestId=cardTestId, cardDplxStatusChange=cardDplxStatusChange, auditShelfId=auditShelfId, portMacAddress=portMacAddress, portMacIfIndex=portMacIfIndex, auditPassedCount=auditPassedCount, diskVolumeDiskSize=diskVolumeDiskSize, verboseLevel=verboseLevel, equipmentDiag=equipmentDiag, cardDetected=cardDetected, diskVolumeEntry=diskVolumeEntry, cardTable=cardTable, auditName=auditName, errEvRecoveryEnabled=errEvRecoveryEnabled, fpgaErrorEventEntry=fpgaErrorEventEntry, portSecState=portSecState, cardName=cardName, portLastChangeTime=portLastChangeTime, equipmentTraps=equipmentTraps, cardTestResult=cardTestResult, portConnectorId=portConnectorId, equipmentPortGroup=equipmentPortGroup, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPicDetected=cardPicDetected, cardTestTime=cardTestTime, cmUChannelID=cmUChannelID, cmRegistrationNotification=cmRegistrationNotification, auditCycleCount=auditCycleCount, portCardId=portCardId, cardEntry=cardEntry, diskVolumeId=diskVolumeId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmDevice=cmDevice, cardTestTable=cardTestTable, auditLogThrottle=auditLogThrottle, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmIpAddress=cmIpAddress, TestId=TestId, errEvLogLevel=errEvLogLevel, cardRamSize=cardRamSize, shelfName=shelfName, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCarrierHwVersion=cardCarrierHwVersion, portEntry=portEntry, portType=portType, cardBusSpeed=cardBusSpeed, cardTestDescription=cardTestDescription, cmResetStatus=cmResetStatus, cardNumPorts=cardNumPorts, diskVolumeTrapInh=diskVolumeTrapInh, auditCommand=auditCommand, portDocsIfIndex=portDocsIfIndex, auditId=auditId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSecState=cardSecState, portMode=portMode, cardCarrierSerialNum=cardCarrierSerialNum, cardSpareGroupId=cardSpareGroupId, cardCarrierFwVersion=cardCarrierFwVersion, cardTestType=cardTestType, equipmentState=equipmentState, equipmentShelfGroup=equipmentShelfGroup, cardTestScheduleCommand=cardTestScheduleCommand, TestScheduleCommand=TestScheduleCommand, cardFlashSize=cardFlashSize, cardTestCommand=cardTestCommand, cardLastBootSource=cardLastBootSource, diagTestId=diagTestId, cmResetInfo=cmResetInfo, portNumber=portNumber, cardTestResultNotification=cardTestResultNotification, cardLastChangeTime=cardLastChangeTime, equipmentError=equipmentError, cardPicHwVersion=cardPicHwVersion, cardId=cardId)
