...
UnknownState, PortDetectedMode, FlowControlMode, CardType, CardSubType, SecondaryState, DiskVolumeUsageLevel, ShelfId, AdminState, PortId, DuplexStatus, FirmwareSource, PicType, EqActionType, PortType, CardId, PortMode, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "PortDetectedMode", "FlowControlMode", "CardType", "CardSubType", "SecondaryState", "DiskVolumeUsageLevel", "ShelfId", "AdminState", "PortId", "DuplexStatus", "FirmwareSource", "PicType", "EqActionType", "PortType", "CardId", "PortMode", "PrimaryState")
...
TruthValue, DateAndTime, TextualConvention, TimeStamp, DisplayString, RowStatus, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "TextualConvention", "TimeStamp", "DisplayString", "RowStatus", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardNumPorts=cardNumPorts, cardUpTime=cardUpTime, portEntry=portEntry, cardTable=cardTable, fpgaErrorEventGroup=fpgaErrorEventGroup, trapSeverity=trapSeverity, cardShelfId=cardShelfId, cardSpareGroupId=cardSpareGroupId, cardRamSize=cardRamSize, cardTestTime=cardTestTime, shelfName=shelfName, cardCarrierHwVersion=cardCarrierHwVersion, portNumber=portNumber, equipmentTbl=equipmentTbl, cardTrapInh=cardTrapInh, portTxFlowControlDetected=portTxFlowControlDetected, portMacIfIndex=portMacIfIndex, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestScheduleCommand=cardTestScheduleCommand, portRxFlowControlMode=portRxFlowControlMode, auditShelfId=auditShelfId, portSecStateChange=portSecStateChange, cardFwVersion=cardFwVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardFlashSize=cardFlashSize, diskVolumeShelfId=diskVolumeShelfId, cardTestIndex=cardTestIndex, portTrapInh=portTrapInh, equipmentAuditGroup=equipmentAuditGroup, auditPassedCount=auditPassedCount, cmUChannelID=cmUChannelID, TestResult=TestResult, cardNumber=cardNumber, cardTestResult=cardTestResult, verboseLevel=verboseLevel, auditCardId=auditCardId, cardSecState=cardSecState, portAction=portAction, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeCardId=diskVolumeCardId, cardLastChangeTime=cardLastChangeTime, diagTestId=diagTestId, equipmentError=equipmentError, cardSpareGroupMode=cardSpareGroupMode, cardTestName=cardTestName, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardBootVersion=cardBootVersion, cardDplxStatus=cardDplxStatus, cardPicHwVersion=cardPicHwVersion, equipmentCardGroup=equipmentCardGroup, cardPrState=cardPrState, portMaxDsPower=portMaxDsPower, auditLogOutput=auditLogOutput, fpgaErrorEventTable=fpgaErrorEventTable, auditTotalPassedCount=auditTotalPassedCount, portPrStateChange=portPrStateChange, cardTestCommand=cardTestCommand, cardSerialNum=cardSerialNum, TestSchedule=TestSchedule, consoleOutput=consoleOutput, cmResetInfo=cmResetInfo, portDocsIfIndex=portDocsIfIndex, cardCarrierFwVersion=cardCarrierFwVersion, cardLastBootVersion=cardLastBootVersion, portMode=portMode, cmResetStatus=cmResetStatus, errEvRowStatus=errEvRowStatus, cardFwUpdateStatus=cardFwUpdateStatus, cardTempNoReportNotification=cardTempNoReportNotification, cardTemperature=cardTemperature, portTable=portTable, portDescription=portDescription, trapCounter=trapCounter, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTxFlowControlMode=portTxFlowControlMode, cmResetReason=cmResetReason, cardCPLDVersion=cardCPLDVersion, errEvLogLevel=errEvLogLevel, cmVendor=cmVendor, cmResetUpTime=cmResetUpTime, auditTable=auditTable, cmDevice=cmDevice, shelfObjects=shelfObjects, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentState=equipmentState, downstreamPowerLoss=downstreamPowerLoss, portType=portType, cardTemperatureHighWarn=cardTemperatureHighWarn, systemClock=systemClock, PYSNMP_MODULE_ID=cadEquipmentMib, cardId=cardId, cardShelfTestIndex=cardShelfTestIndex, diskVolumeUsageNotification=diskVolumeUsageNotification, auditCommand=auditCommand, cardBusSpeed=cardBusSpeed, portShelfId=portShelfId, cardCarrierSerialNum=cardCarrierSerialNum, cardPicSerialNum=cardPicSerialNum, portId=portId, portAdminState=portAdminState, cardEntry=cardEntry, cmRegistrationNotification=cmRegistrationNotification, cmMacAddress=cmMacAddress, remainInDiagMode=remainInDiagMode, diskVolumeEntry=diskVolumeEntry, cardTestResultDesc=cardTestResultDesc, cardSwVersion=cardSwVersion, portDetectedMode=portDetectedMode, portDplxStatusChange=portDplxStatusChange, portMinDsPower=portMinDsPower, cardTestEntry=cardTestEntry, equipmentStateGroup=equipmentStateGroup, shelfNumber=shelfNumber, cardTestSchedule=cardTestSchedule, equipmentShelfGroup=equipmentShelfGroup, cardHwVersion=cardHwVersion, TestCommand=TestCommand, cardTestDescription=cardTestDescription, auditTotalFailedCount=auditTotalFailedCount, portConnectorId=portConnectorId, auditDescription=auditDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSecStateChange=cardSecStateChange, cardSubDetected=cardSubDetected, auditCycleCount=auditCycleCount, cardCpuSpeed=cardCpuSpeed, eqDiagConfig=eqDiagConfig, auditName=auditName, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portNumChans=portNumChans, equipmentAudit=equipmentAudit, cardAction=cardAction, errEventId=errEventId, TestScheduleCommand=TestScheduleCommand, diskVolumeName=diskVolumeName, portCurrDsPower=portCurrDsPower, cmResetNotification=cmResetNotification, diskDriveNumber=diskDriveNumber, diskVolumeTrapInh=diskVolumeTrapInh, TestType=TestType, portMacAddress=portMacAddress, systemKey=systemKey, systemGeneral=systemGeneral, cardTemperatureHighError=cardTemperatureHighError, cardName=cardName, portCardId=portCardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetClearNotification=cmResetClearNotification, cardType=cardType, cardLastResetReason=cardLastResetReason, auditId=auditId, auditLogThrottle=auditLogThrottle, auditStatus=auditStatus, cmIpAddress=cmIpAddress, cmPrimarySID=cmPrimarySID, cardTestId=cardTestId, diskVolumeSize=diskVolumeSize, diskVolumeGroup=diskVolumeGroup, cardPicDetected=cardPicDetected, portBgpId=portBgpId, cardDplxStatusChange=cardDplxStatusChange, cardTestTable=cardTestTable, diskVolumeFileName=diskVolumeFileName, equipmentDiag=equipmentDiag, auditFailedCount=auditFailedCount, cardLastBootSource=cardLastBootSource, portCardSubType=portCardSubType, diskVolumeNumber=diskVolumeNumber, cardCpuType=cardCpuType, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentTraps=equipmentTraps, cardSubType=cardSubType, shelfSwVersion=shelfSwVersion, systemGeneralGroup=systemGeneralGroup, cardDetected=cardDetected, portSecState=portSecState, fpgaErrorEventEntry=fpgaErrorEventEntry, portRxFlowControlDetected=portRxFlowControlDetected, cardTestType=cardTestType, auditEntry=auditEntry, TestId=TestId, auditAutoScheduling=auditAutoScheduling, cardTestResultNotification=cardTestResultNotification, diskVolumeTable=diskVolumeTable, cardFpgaSource=cardFpgaSource, equipmentDiagGroup=equipmentDiagGroup, auditResult=auditResult, cardTempOverHeatNotification=cardTempOverHeatNotification, cadEquipmentMib=cadEquipmentMib, errEvLoggingEnabled=errEvLoggingEnabled, cardPrStateChange=cardPrStateChange, TestTransactionId=TestTransactionId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestTransId=cardTestTransId, auditTime=auditTime, portDplxStatus=portDplxStatus, diskVolumeId=diskVolumeId, portLastChangeTime=portLastChangeTime, diskVolumeDiskSize=diskVolumeDiskSize, equipmentPortGroup=equipmentPortGroup, portPrState=portPrState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, errEvRecoveryEnabled=errEvRecoveryEnabled, cardDetectedChange=cardDetectedChange, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDriveId=diskVolumeDriveId, cardAdminState=cardAdminState)
