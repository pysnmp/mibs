...
ShelfId, FlowControlMode, CardId, PortMode, DiskVolumeUsageLevel, DuplexStatus, PortId, PortDetectedMode, CardSubType, SecondaryState, AdminState, FirmwareSource, PicType, UnknownState, PortType, EqActionType, PrimaryState, CardType = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "FlowControlMode", "CardId", "PortMode", "DiskVolumeUsageLevel", "DuplexStatus", "PortId", "PortDetectedMode", "CardSubType", "SecondaryState", "AdminState", "FirmwareSource", "PicType", "UnknownState", "PortType", "EqActionType", "PrimaryState", "CardType")
...
DateAndTime, TruthValue, DisplayString, RowStatus, MacAddress, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TruthValue", "DisplayString", "RowStatus", "MacAddress", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardNumber=cardNumber, systemGeneralGroup=systemGeneralGroup, consoleOutput=consoleOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, verboseLevel=verboseLevel, cmDevice=cmDevice, equipmentError=equipmentError, cardEntry=cardEntry, cardCpuSpeed=cardCpuSpeed, auditLogThrottle=auditLogThrottle, auditTotalFailedCount=auditTotalFailedCount, portRxFlowControlMode=portRxFlowControlMode, errEventId=errEventId, TestId=TestId, cardPicDetected=cardPicDetected, portPrState=portPrState, cardTestResultDesc=cardTestResultDesc, equipmentPortGroup=equipmentPortGroup, cmResetInfo=cmResetInfo, TestType=TestType, cardSecStateChange=cardSecStateChange, cardSpareGroupMode=cardSpareGroupMode, portTxFlowControlDetected=portTxFlowControlDetected, cardSecState=cardSecState, cardCarrierSerialNum=cardCarrierSerialNum, auditCommand=auditCommand, portCurrDsPower=portCurrDsPower, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portEntry=portEntry, cardSubDetected=cardSubDetected, portDetectedMode=portDetectedMode, cardTestScheduleCommand=cardTestScheduleCommand, portDplxStatus=portDplxStatus, cardCPLDVersion=cardCPLDVersion, cardTempNoReportNotification=cardTempNoReportNotification, auditTime=auditTime, equipmentDiag=equipmentDiag, cardSpareGroupId=cardSpareGroupId, cmResetStatus=cmResetStatus, cardRamSize=cardRamSize, cardBusSpeed=cardBusSpeed, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portTable=portTable, portAction=portAction, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibConformance=cadEquipmentMibConformance, remainInDiagMode=remainInDiagMode, equipmentAuditGroup=equipmentAuditGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCarrierFwVersion=cardCarrierFwVersion, cardTemperature=cardTemperature, cmMacAddress=cmMacAddress, portLastChangeTime=portLastChangeTime, portPrStateChange=portPrStateChange, cardType=cardType, auditDescription=auditDescription, TestResult=TestResult, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestEntry=cardTestEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfSwVersion=shelfSwVersion, cardTestId=cardTestId, cardDetectedChange=cardDetectedChange, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMibCompliances=cadEquipmentMibCompliances, eqDiagConfig=eqDiagConfig, cardLastBootVersion=cardLastBootVersion, diagTestId=diagTestId, cardAction=cardAction, auditAutoScheduling=auditAutoScheduling, auditPassedCount=auditPassedCount, cmRegistrationNotification=cmRegistrationNotification, cardSerialNum=cardSerialNum, auditFailedCount=auditFailedCount, cardTestDescription=cardTestDescription, portDescription=portDescription, portBgpId=portBgpId, portMinDsPower=portMinDsPower, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestCommand=cardTestCommand, downstreamPowerLoss=downstreamPowerLoss, cardTestResult=cardTestResult, equipmentShelfGroup=equipmentShelfGroup, portShelfId=portShelfId, auditId=auditId, cardFwUpdateStatus=cardFwUpdateStatus, cardFlashSize=cardFlashSize, cardDplxStatus=cardDplxStatus, auditStatus=auditStatus, cardBootVersion=cardBootVersion, portMode=portMode, fpgaErrorEventEntry=fpgaErrorEventEntry, cardHwVersion=cardHwVersion, auditLogOutput=auditLogOutput, shelfObjects=shelfObjects, portCardId=portCardId, TestSchedule=TestSchedule, auditCycleCount=auditCycleCount, errEvRecoveryEnabled=errEvRecoveryEnabled, portRxFlowControlDetected=portRxFlowControlDetected, systemClock=systemClock, cardPicSerialNum=cardPicSerialNum, cmUChannelID=cmUChannelID, cardTemperatureHighError=cardTemperatureHighError, cardCpuType=cardCpuType, cardAdminState=cardAdminState, portAdminState=portAdminState, portMaxDsPower=portMaxDsPower, cardNumPorts=cardNumPorts, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeName=diskVolumeName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardPicHwVersion=cardPicHwVersion, cardLastBootSource=cardLastBootSource, equipmentState=equipmentState, cardTestSchedule=cardTestSchedule, cardTestName=cardTestName, cardLastChangeTime=cardLastChangeTime, auditTable=auditTable, equipmentCardGroup=equipmentCardGroup, equipmentTbl=equipmentTbl, portNumChans=portNumChans, systemKey=systemKey, cmResetClearNotification=cmResetClearNotification, portNumber=portNumber, cardFpgaSource=cardFpgaSource, cardDetected=cardDetected, cardSwVersion=cardSwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, TestTransactionId=TestTransactionId, portDocsIfIndex=portDocsIfIndex, auditCardId=auditCardId, portId=portId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentCardTestGroup=equipmentCardTestGroup, cardDplxStatusChange=cardDplxStatusChange, diskVolumeShelfId=diskVolumeShelfId, auditTotalPassedCount=auditTotalPassedCount, cardName=cardName, diskVolumeCardId=diskVolumeCardId, cmResetUpTime=cmResetUpTime, auditEntry=auditEntry, cmPrimarySID=cmPrimarySID, cmIpAddress=cmIpAddress, cardFwVersion=cardFwVersion, cardTestResultNotification=cardTestResultNotification, errEvRowStatus=errEvRowStatus, portMacIfIndex=portMacIfIndex, equipmentTraps=equipmentTraps, shelfNumber=shelfNumber, portConnectorId=portConnectorId, auditName=auditName, diskVolumeFileName=diskVolumeFileName, portSecState=portSecState, equipmentStateGroup=equipmentStateGroup, trapCounter=trapCounter, cardCarrierHwVersion=cardCarrierHwVersion, portSecStateChange=portSecStateChange, cardSubType=cardSubType, shelfName=shelfName, portMacAddress=portMacAddress, diskVolumeTrapInh=diskVolumeTrapInh, diskDriveNumber=diskDriveNumber, diskVolumeNumber=diskVolumeNumber, cardTestTime=cardTestTime, auditShelfId=auditShelfId, cardTable=cardTable, errEvLoggingEnabled=errEvLoggingEnabled, auditResult=auditResult, cardPrState=cardPrState, cardTestTable=cardTestTable, diskVolumeSize=diskVolumeSize, cardTempOverHeatNotification=cardTempOverHeatNotification, systemGeneral=systemGeneral, cardId=cardId, portTxFlowControlMode=portTxFlowControlMode, equipmentDiagGroup=equipmentDiagGroup, cmVendor=cmVendor, trapSeverity=trapSeverity, diskVolumeTable=diskVolumeTable, diskVolumeGroup=diskVolumeGroup, TestCommand=TestCommand, cmResetReason=cmResetReason, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, fpgaErrorEventTable=fpgaErrorEventTable, errEvLogLevel=errEvLogLevel, portDplxStatusChange=portDplxStatusChange, TestScheduleCommand=TestScheduleCommand, portCardSubType=portCardSubType, diskVolumeEntry=diskVolumeEntry, diskVolumeDriveId=diskVolumeDriveId, portType=portType, cardTestTransId=cardTestTransId, cardLastResetReason=cardLastResetReason, equipmentAudit=equipmentAudit, cardTrapInh=cardTrapInh, diskVolumeId=diskVolumeId, cardUpTime=cardUpTime, cardShelfId=cardShelfId, cardTestType=cardTestType, cmResetNotification=cmResetNotification, cardPrStateChange=cardPrStateChange, cardTestIndex=cardTestIndex, portTrapInh=portTrapInh)
