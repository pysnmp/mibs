...
PortType, ShelfId, PortDetectedMode, EqActionType, FlowControlMode, PicType, DuplexStatus, CardType, DiskVolumeUsageLevel, FirmwareSource, SecondaryState, UnknownState, PrimaryState, CardId, PortId, CardSubType, AdminState, PortMode = mibBuilder.importSymbols("CADANT-TC", "PortType", "ShelfId", "PortDetectedMode", "EqActionType", "FlowControlMode", "PicType", "DuplexStatus", "CardType", "DiskVolumeUsageLevel", "FirmwareSource", "SecondaryState", "UnknownState", "PrimaryState", "CardId", "PortId", "CardSubType", "AdminState", "PortMode")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSecStateChange=cardSecStateChange, cmResetUpTime=cmResetUpTime, auditTime=auditTime, cardPicSerialNum=cardPicSerialNum, portCurrDsPower=portCurrDsPower, cadEquipmentMib=cadEquipmentMib, cardSubDetected=cardSubDetected, portDetectedMode=portDetectedMode, consoleOutput=consoleOutput, diskVolumeId=diskVolumeId, cmResetNotification=cmResetNotification, trapSeverity=trapSeverity, systemGeneralGroup=systemGeneralGroup, cardFwVersion=cardFwVersion, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardAdminState=cardAdminState, equipmentDiagGroup=equipmentDiagGroup, cardFpgaSource=cardFpgaSource, portLastChangeTime=portLastChangeTime, auditName=auditName, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSpareGroupId=cardSpareGroupId, cardPrState=cardPrState, diskVolumeTable=diskVolumeTable, TestType=TestType, auditTotalFailedCount=auditTotalFailedCount, cardCarrierHwVersion=cardCarrierHwVersion, cardShelfTestIndex=cardShelfTestIndex, equipmentStateGroup=equipmentStateGroup, cmResetClearNotification=cmResetClearNotification, diskVolumeFileName=diskVolumeFileName, equipmentCardTestGroup=equipmentCardTestGroup, cardPicHwVersion=cardPicHwVersion, auditShelfId=auditShelfId, cardTrapInh=cardTrapInh, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestSchedule=cardTestSchedule, cardCpuSpeed=cardCpuSpeed, portEntry=portEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portRxFlowControlDetected=portRxFlowControlDetected, verboseLevel=verboseLevel, portTxFlowControlDetected=portTxFlowControlDetected, TestCommand=TestCommand, auditStatus=auditStatus, cardSubType=cardSubType, diskVolumeTrapInh=diskVolumeTrapInh, cardPicDetected=cardPicDetected, cardLastBootVersion=cardLastBootVersion, auditCycleCount=auditCycleCount, cardTestIndex=cardTestIndex, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageLevel=diskVolumeUsageLevel, cmUChannelID=cmUChannelID, portDplxStatusChange=portDplxStatusChange, cardTestCommand=cardTestCommand, fpgaErrorEventTable=fpgaErrorEventTable, portMinDsPower=portMinDsPower, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardEntry=cardEntry, cardTestScheduleCommand=cardTestScheduleCommand, portCardSubType=portCardSubType, cmDevice=cmDevice, portAction=portAction, diskVolumeEntry=diskVolumeEntry, diskVolumeSize=diskVolumeSize, cmRegistrationNotification=cmRegistrationNotification, diskVolumeDiskSize=diskVolumeDiskSize, systemClock=systemClock, shelfName=shelfName, cardFlashSize=cardFlashSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portMaxDsPower=portMaxDsPower, portSecState=portSecState, diskVolumeGroup=diskVolumeGroup, auditResult=auditResult, errEvLogLevel=errEvLogLevel, cardTestResultDesc=cardTestResultDesc, cardTestName=cardTestName, cardCarrierSerialNum=cardCarrierSerialNum, cardTemperatureHighWarn=cardTemperatureHighWarn, portTable=portTable, diskVolumeNumber=diskVolumeNumber, portAdminState=portAdminState, cardLastBootSource=cardLastBootSource, cardTestTable=cardTestTable, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuType=cardCpuType, cardBootVersion=cardBootVersion, portConnectorId=portConnectorId, cardNumPorts=cardNumPorts, diskVolumeCardId=diskVolumeCardId, systemKey=systemKey, portDocsIfIndex=portDocsIfIndex, auditCardId=auditCardId, auditDescription=auditDescription, cardLastResetReason=cardLastResetReason, cardShelfId=cardShelfId, TestTransactionId=TestTransactionId, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditId=auditId, cmResetStatus=cmResetStatus, cmResetInfo=cmResetInfo, downstreamPowerLoss=downstreamPowerLoss, fpgaErrorEventGroup=fpgaErrorEventGroup, cardType=cardType, cadEquipmentMibGroup=cadEquipmentMibGroup, cmMacAddress=cmMacAddress, equipmentTbl=equipmentTbl, portPrState=portPrState, shelfSwVersion=shelfSwVersion, equipmentAuditGroup=equipmentAuditGroup, cardSwVersion=cardSwVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, trapCounter=trapCounter, cardTemperature=cardTemperature, cardDplxStatusChange=cardDplxStatusChange, eqDiagConfig=eqDiagConfig, cardSerialNum=cardSerialNum, portShelfId=portShelfId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardUpTime=cardUpTime, shelfNumber=shelfNumber, diskVolumeName=diskVolumeName, cardTestTransId=cardTestTransId, cardSpareGroupMode=cardSpareGroupMode, errEvLoggingEnabled=errEvLoggingEnabled, cardFwUpdateStatus=cardFwUpdateStatus, portCardId=portCardId, portTrapInh=portTrapInh, equipmentCardGroup=equipmentCardGroup, cmIpAddress=cmIpAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, auditLogOutput=auditLogOutput, portMacIfIndex=portMacIfIndex, diskVolumeDriveId=diskVolumeDriveId, cardRamSize=cardRamSize, cardTestTime=cardTestTime, auditPassedCount=auditPassedCount, cardTempNoReportNotification=cardTempNoReportNotification, cardBusSpeed=cardBusSpeed, cardNumber=cardNumber, equipmentState=equipmentState, diskVolumeUsageNotification=diskVolumeUsageNotification, errEvRowStatus=errEvRowStatus, auditLogThrottle=auditLogThrottle, cardDplxStatus=cardDplxStatus, systemGeneral=systemGeneral, TestResult=TestResult, auditCommand=auditCommand, portPrStateChange=portPrStateChange, equipmentAudit=equipmentAudit, auditEntry=auditEntry, cmVendor=cmVendor, cardName=cardName, shelfObjects=shelfObjects, cardTemperatureHighError=cardTemperatureHighError, fpgaErrorEventEntry=fpgaErrorEventEntry, cardDetectedChange=cardDetectedChange, diskDriveNumber=diskDriveNumber, cardTable=cardTable, auditTable=auditTable, equipmentTraps=equipmentTraps, cardTempOverHeatNotification=cardTempOverHeatNotification, portId=portId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentError=equipmentError, cardPrStateChange=cardPrStateChange, errEventId=errEventId, cardTestType=cardTestType, equipmentShelfGroup=equipmentShelfGroup, cardDetected=cardDetected, portDescription=portDescription, equipmentDiag=equipmentDiag, cardTestId=cardTestId, equipmentPortGroup=equipmentPortGroup, cardAction=cardAction, cardTestResult=cardTestResult, cmResetReason=cmResetReason, TestScheduleCommand=TestScheduleCommand, cardId=cardId, cardCarrierFwVersion=cardCarrierFwVersion, cardTestEntry=cardTestEntry, portRxFlowControlMode=portRxFlowControlMode, cardTestResultNotification=cardTestResultNotification, auditTotalPassedCount=auditTotalPassedCount, TestSchedule=TestSchedule, portDplxStatus=portDplxStatus, auditFailedCount=auditFailedCount, portSecStateChange=portSecStateChange, cardCPLDVersion=cardCPLDVersion, diskVolumeShelfId=diskVolumeShelfId, portMacAddress=portMacAddress, portNumber=portNumber, remainInDiagMode=remainInDiagMode, TestId=TestId, portType=portType, portBgpId=portBgpId, portMode=portMode, cardTestDescription=cardTestDescription, cardSecState=cardSecState, cardHwVersion=cardHwVersion, portNumChans=portNumChans, auditAutoScheduling=auditAutoScheduling, diagTestId=diagTestId)
