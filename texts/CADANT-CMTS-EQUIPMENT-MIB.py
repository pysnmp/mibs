...
DiskVolumeUsageLevel, SecondaryState, FirmwareSource, ShelfId, CardType, CardId, DuplexStatus, EqActionType, PortType, PrimaryState, PicType, PortMode, UnknownState, PortDetectedMode, PortId, FlowControlMode, CardSubType, AdminState = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "SecondaryState", "FirmwareSource", "ShelfId", "CardType", "CardId", "DuplexStatus", "EqActionType", "PortType", "PrimaryState", "PicType", "PortMode", "UnknownState", "PortDetectedMode", "PortId", "FlowControlMode", "CardSubType", "AdminState")
...
DisplayString, RowStatus, TimeStamp, TextualConvention, MacAddress, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "RowStatus", "TimeStamp", "TextualConvention", "MacAddress", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardId=cardId, cardSecState=cardSecState, auditCycleCount=auditCycleCount, cardTemperature=cardTemperature, cardSpareGroupId=cardSpareGroupId, portConnectorId=portConnectorId, cardTestDescription=cardTestDescription, cardFpgaSource=cardFpgaSource, cadEquipmentMib=cadEquipmentMib, equipmentError=equipmentError, cmUChannelID=cmUChannelID, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardUpTime=cardUpTime, portType=portType, diskVolumeDriveId=diskVolumeDriveId, cardTempOverHeatNotification=cardTempOverHeatNotification, portMacAddress=portMacAddress, cmRegistrationNotification=cmRegistrationNotification, equipmentTbl=equipmentTbl, errEvLogLevel=errEvLogLevel, portTable=portTable, cmResetUpTime=cmResetUpTime, systemGeneral=systemGeneral, portDplxStatusChange=portDplxStatusChange, cardLastBootVersion=cardLastBootVersion, auditAutoScheduling=auditAutoScheduling, shelfSwVersion=shelfSwVersion, cardHwVersion=cardHwVersion, fpgaErrorEventTable=fpgaErrorEventTable, cardFwVersion=cardFwVersion, diskVolumeShelfId=diskVolumeShelfId, cardTrapInh=cardTrapInh, TestResult=TestResult, auditFailedCount=auditFailedCount, diskVolumeId=diskVolumeId, cardNumber=cardNumber, trapSeverity=trapSeverity, equipmentAuditGroup=equipmentAuditGroup, equipmentShelfGroup=equipmentShelfGroup, cardTestResultNotification=cardTestResultNotification, auditResult=auditResult, auditCardId=auditCardId, portCardId=portCardId, auditEntry=auditEntry, portAdminState=portAdminState, cardDetectedChange=cardDetectedChange, diskVolumeCardId=diskVolumeCardId, auditStatus=auditStatus, cardTestResultDesc=cardTestResultDesc, equipmentAudit=equipmentAudit, cadEquipmentMibConformance=cadEquipmentMibConformance, systemGeneralGroup=systemGeneralGroup, auditLogOutput=auditLogOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPicDetected=cardPicDetected, cardTestSchedule=cardTestSchedule, cmResetStatus=cmResetStatus, trapCounter=trapCounter, cardAction=cardAction, cardTestIndex=cardTestIndex, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetReason=cmResetReason, equipmentState=equipmentState, diskVolumeDiskSize=diskVolumeDiskSize, auditDescription=auditDescription, auditCommand=auditCommand, portCurrDsPower=portCurrDsPower, auditTotalPassedCount=auditTotalPassedCount, cardSubDetected=cardSubDetected, auditTotalFailedCount=auditTotalFailedCount, auditId=auditId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, downstreamPowerLoss=downstreamPowerLoss, shelfObjects=shelfObjects, auditPassedCount=auditPassedCount, portLastChangeTime=portLastChangeTime, cardCPLDVersion=cardCPLDVersion, diskDriveNumber=diskDriveNumber, cardEntry=cardEntry, cmMacAddress=cmMacAddress, portDplxStatus=portDplxStatus, cardTestName=cardTestName, portCardSubType=portCardSubType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cmDevice=cmDevice, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeSize=diskVolumeSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogThrottle=auditLogThrottle, TestCommand=TestCommand, portNumber=portNumber, cardShelfId=cardShelfId, cardNumPorts=cardNumPorts, portEntry=portEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMibGroup=cadEquipmentMibGroup, cardPrStateChange=cardPrStateChange, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestTransId=cardTestTransId, diskVolumeName=diskVolumeName, auditShelfId=auditShelfId, portRxFlowControlMode=portRxFlowControlMode, portDocsIfIndex=portDocsIfIndex, cardCpuSpeed=cardCpuSpeed, equipmentDiag=equipmentDiag, cmIpAddress=cmIpAddress, portDescription=portDescription, fpgaErrorEventEntry=fpgaErrorEventEntry, cardDetected=cardDetected, systemKey=systemKey, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardSwVersion=cardSwVersion, TestTransactionId=TestTransactionId, diskVolumeTrapInh=diskVolumeTrapInh, cardTestType=cardTestType, cardRamSize=cardRamSize, auditTime=auditTime, cmPrimarySID=cmPrimarySID, portSecState=portSecState, auditName=auditName, equipmentTraps=equipmentTraps, cardSpareGroupMode=cardSpareGroupMode, cardTestCommand=cardTestCommand, portMode=portMode, cardTestEntry=cardTestEntry, errEventId=errEventId, portId=portId, cardBootVersion=cardBootVersion, cardType=cardType, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentCardGroup=equipmentCardGroup, equipmentStateGroup=equipmentStateGroup, cardTestId=cardTestId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portPrState=portPrState, diagTestId=diagTestId, auditTable=auditTable, portSecStateChange=portSecStateChange, diskVolumeGroup=diskVolumeGroup, cardLastBootSource=cardLastBootSource, errEvRecoveryEnabled=errEvRecoveryEnabled, portTrapInh=portTrapInh, equipmentDiagGroup=equipmentDiagGroup, cardBusSpeed=cardBusSpeed, cmResetNotification=cmResetNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTable=cardTable, shelfNumber=shelfNumber, cmResetInfo=cmResetInfo, cardName=cardName, portRxFlowControlDetected=portRxFlowControlDetected, remainInDiagMode=remainInDiagMode, cardSubType=cardSubType, cmResetClearNotification=cmResetClearNotification, cardTestTime=cardTestTime, diskVolumeFileName=diskVolumeFileName, cardTestResult=cardTestResult, portTxFlowControlMode=portTxFlowControlMode, portMacIfIndex=portMacIfIndex, cardAdminState=cardAdminState, cardCarrierFwVersion=cardCarrierFwVersion, equipmentCardTestGroup=equipmentCardTestGroup, TestScheduleCommand=TestScheduleCommand, systemClock=systemClock, cardLastChangeTime=cardLastChangeTime, cmVendor=cmVendor, cardPicHwVersion=cardPicHwVersion, portDetectedMode=portDetectedMode, eqDiagConfig=eqDiagConfig, portMaxDsPower=portMaxDsPower, portAction=portAction, cardPrState=cardPrState, consoleOutput=consoleOutput, diskVolumeEntry=diskVolumeEntry, diskVolumeTable=diskVolumeTable, TestId=TestId, shelfName=shelfName, equipmentPortGroup=equipmentPortGroup, cardDplxStatusChange=cardDplxStatusChange, verboseLevel=verboseLevel, cardTestTable=cardTestTable, cardShelfTestIndex=cardShelfTestIndex, cardTemperatureHighError=cardTemperatureHighError, portMinDsPower=portMinDsPower, cardLastResetReason=cardLastResetReason, portBgpId=portBgpId, TestSchedule=TestSchedule, cardCarrierSerialNum=cardCarrierSerialNum, cardSecStateChange=cardSecStateChange, TestType=TestType, portTxFlowControlDetected=portTxFlowControlDetected, portPrStateChange=portPrStateChange, portShelfId=portShelfId, cardDplxStatus=cardDplxStatus, portNumChans=portNumChans, cardSerialNum=cardSerialNum, cardPicSerialNum=cardPicSerialNum, errEvRowStatus=errEvRowStatus, cardTestScheduleCommand=cardTestScheduleCommand, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeNumber=diskVolumeNumber, cardFlashSize=cardFlashSize, diskVolumeUsageLevel=diskVolumeUsageLevel, cardCarrierHwVersion=cardCarrierHwVersion, cardCpuType=cardCpuType)
