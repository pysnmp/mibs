...
PrimaryState, CardId, CardSubType, FlowControlMode, ShelfId, DiskVolumeUsageLevel, DuplexStatus, UnknownState, AdminState, FirmwareSource, PicType, PortId, PortMode, PortType, SecondaryState, EqActionType, PortDetectedMode, CardType = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "CardId", "CardSubType", "FlowControlMode", "ShelfId", "DiskVolumeUsageLevel", "DuplexStatus", "UnknownState", "AdminState", "FirmwareSource", "PicType", "PortId", "PortMode", "PortType", "SecondaryState", "EqActionType", "PortDetectedMode", "CardType")
...
TruthValue, TextualConvention, RowStatus, DisplayString, MacAddress, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TextualConvention", "RowStatus", "DisplayString", "MacAddress", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentTraps=equipmentTraps, cardTestTime=cardTestTime, cmResetClearNotification=cmResetClearNotification, portPrStateChange=portPrStateChange, equipmentCardTestGroup=equipmentCardTestGroup, TestTransactionId=TestTransactionId, portDetectedMode=portDetectedMode, cmVendor=cmVendor, cardCPLDVersion=cardCPLDVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, systemKey=systemKey, auditCardId=auditCardId, portShelfId=portShelfId, portDplxStatus=portDplxStatus, cardDplxStatus=cardDplxStatus, portTxFlowControlDetected=portTxFlowControlDetected, cardTestTable=cardTestTable, auditCycleCount=auditCycleCount, cardTempOverHeatNotification=cardTempOverHeatNotification, auditPassedCount=auditPassedCount, cardEntry=cardEntry, shelfNumber=shelfNumber, diskVolumeId=diskVolumeId, shelfName=shelfName, diskVolumeCardId=diskVolumeCardId, cardTestResultDesc=cardTestResultDesc, errEvRowStatus=errEvRowStatus, cardDetected=cardDetected, equipmentShelfGroup=equipmentShelfGroup, cardNumPorts=cardNumPorts, cardPicDetected=cardPicDetected, cardShelfTestIndex=cardShelfTestIndex, auditLogThrottle=auditLogThrottle, portRxFlowControlDetected=portRxFlowControlDetected, cmIpAddress=cmIpAddress, cardPrStateChange=cardPrStateChange, cadEquipmentMib=cadEquipmentMib, portPrState=portPrState, auditTable=auditTable, cardSwVersion=cardSwVersion, portMacAddress=portMacAddress, portTxFlowControlMode=portTxFlowControlMode, auditDescription=auditDescription, portTrapInh=portTrapInh, equipmentCardGroup=equipmentCardGroup, systemGeneralGroup=systemGeneralGroup, cardSecState=cardSecState, cardSubDetected=cardSubDetected, equipmentAuditGroup=equipmentAuditGroup, portDplxStatusChange=portDplxStatusChange, equipmentError=equipmentError, systemGeneral=systemGeneral, portEntry=portEntry, portNumChans=portNumChans, cardTestEntry=cardTestEntry, auditFailedCount=auditFailedCount, cardTestResult=cardTestResult, portBgpId=portBgpId, portSecState=portSecState, diagTestId=diagTestId, cmMacAddress=cmMacAddress, cardPrState=cardPrState, cardName=cardName, trapCounter=trapCounter, cardTestScheduleCommand=cardTestScheduleCommand, portMinDsPower=portMinDsPower, auditEntry=auditEntry, diskVolumeSize=diskVolumeSize, cardTemperatureHighError=cardTemperatureHighError, diskVolumeNumber=diskVolumeNumber, portMacIfIndex=portMacIfIndex, equipmentStateGroup=equipmentStateGroup, cardNumber=cardNumber, cardUpTime=cardUpTime, cardShelfId=cardShelfId, portDescription=portDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, portRxFlowControlMode=portRxFlowControlMode, cardPicSerialNum=cardPicSerialNum, cardDetectedChange=cardDetectedChange, portCurrDsPower=portCurrDsPower, auditLogOutput=auditLogOutput, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portNumber=portNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetNotification=cmResetNotification, cmRegistrationNotification=cmRegistrationNotification, diskVolumeDriveId=diskVolumeDriveId, cardSpareGroupId=cardSpareGroupId, portSecStateChange=portSecStateChange, cardCarrierSerialNum=cardCarrierSerialNum, cardAdminState=cardAdminState, TestScheduleCommand=TestScheduleCommand, cardSerialNum=cardSerialNum, portLastChangeTime=portLastChangeTime, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardAction=cardAction, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestTransId=cardTestTransId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestType=cardTestType, cardLastChangeTime=cardLastChangeTime, auditStatus=auditStatus, portAction=portAction, cardLastBootSource=cardLastBootSource, cardTestCommand=cardTestCommand, auditShelfId=auditShelfId, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfSwVersion=shelfSwVersion, cardCarrierFwVersion=cardCarrierFwVersion, portCardSubType=portCardSubType, fpgaErrorEventTable=fpgaErrorEventTable, errEvRecoveryEnabled=errEvRecoveryEnabled, cardHwVersion=cardHwVersion, cardLastBootVersion=cardLastBootVersion, equipmentDiagGroup=equipmentDiagGroup, auditTotalPassedCount=auditTotalPassedCount, errEvLogLevel=errEvLogLevel, cardTestId=cardTestId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestDescription=cardTestDescription, cardCarrierHwVersion=cardCarrierHwVersion, TestType=TestType, diskVolumeFileName=diskVolumeFileName, fpgaErrorEventEntry=fpgaErrorEventEntry, errEventId=errEventId, equipmentTbl=equipmentTbl, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, downstreamPowerLoss=downstreamPowerLoss, diskVolumeDiskSize=diskVolumeDiskSize, portCardId=portCardId, cmDevice=cmDevice, portType=portType, equipmentPortGroup=equipmentPortGroup, portMaxDsPower=portMaxDsPower, cardTestSchedule=cardTestSchedule, TestCommand=TestCommand, cmPrimarySID=cmPrimarySID, diskDriveNumber=diskDriveNumber, cardCpuType=cardCpuType, cardTempNoReportNotification=cardTempNoReportNotification, cardFlashSize=cardFlashSize, eqDiagConfig=eqDiagConfig, cardCpuSpeed=cardCpuSpeed, cardTestIndex=cardTestIndex, auditId=auditId, diskVolumeShelfId=diskVolumeShelfId, cmResetInfo=cmResetInfo, systemClock=systemClock, equipmentState=equipmentState, cardTable=cardTable, TestId=TestId, portConnectorId=portConnectorId, cardFpgaSource=cardFpgaSource, portDocsIfIndex=portDocsIfIndex, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardFwVersion=cardFwVersion, diskVolumeGroup=diskVolumeGroup, auditTotalFailedCount=auditTotalFailedCount, auditTime=auditTime, TestResult=TestResult, TestSchedule=TestSchedule, cardFwUpdateStatus=cardFwUpdateStatus, cmUChannelID=cmUChannelID, cardTestResultNotification=cardTestResultNotification, cardRamSize=cardRamSize, diskVolumeTrapInh=diskVolumeTrapInh, auditName=auditName, diskVolumeTable=diskVolumeTable, cmResetUpTime=cmResetUpTime, auditResult=auditResult, auditCommand=auditCommand, equipmentDiag=equipmentDiag, cardBootVersion=cardBootVersion, cardSubType=cardSubType, diskVolumeName=diskVolumeName, cardSecStateChange=cardSecStateChange, cardType=cardType, errEvLoggingEnabled=errEvLoggingEnabled, shelfObjects=shelfObjects, cardSpareGroupMode=cardSpareGroupMode, portAdminState=portAdminState, cardId=cardId, cardTemperature=cardTemperature, remainInDiagMode=remainInDiagMode, auditAutoScheduling=auditAutoScheduling, PYSNMP_MODULE_ID=cadEquipmentMib, consoleOutput=consoleOutput, cadEquipmentMibCompliance=cadEquipmentMibCompliance, trapSeverity=trapSeverity, cardBusSpeed=cardBusSpeed, cardPicHwVersion=cardPicHwVersion, cardDplxStatusChange=cardDplxStatusChange, cardLastResetReason=cardLastResetReason, verboseLevel=verboseLevel, cardTrapInh=cardTrapInh, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentAudit=equipmentAudit, portId=portId, diskVolumeEntry=diskVolumeEntry, cmResetStatus=cmResetStatus, cmResetReason=cmResetReason, cardTestName=cardTestName, portMode=portMode, portTable=portTable)
