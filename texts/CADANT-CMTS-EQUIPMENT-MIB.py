...
EqActionType, FirmwareSource, PortType, PicType, UnknownState, CardType, PortId, PortDetectedMode, DuplexStatus, PrimaryState, CardId, FlowControlMode, DiskVolumeUsageLevel, PortMode, SecondaryState, ShelfId, AdminState, CardSubType = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "FirmwareSource", "PortType", "PicType", "UnknownState", "CardType", "PortId", "PortDetectedMode", "DuplexStatus", "PrimaryState", "CardId", "FlowControlMode", "DiskVolumeUsageLevel", "PortMode", "SecondaryState", "ShelfId", "AdminState", "CardSubType")
...
MacAddress, DisplayString, DateAndTime, TruthValue, TimeStamp, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "DisplayString", "DateAndTime", "TruthValue", "TimeStamp", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResultDesc=cardTestResultDesc, cadEquipmentMibGroup=cadEquipmentMibGroup, TestScheduleCommand=TestScheduleCommand, cardAdminState=cardAdminState, errEvRowStatus=errEvRowStatus, cardSecStateChange=cardSecStateChange, remainInDiagMode=remainInDiagMode, diskVolumeGroup=diskVolumeGroup, cardNumPorts=cardNumPorts, cardTable=cardTable, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeNumber=diskVolumeNumber, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardNumber=cardNumber, diskVolumeId=diskVolumeId, auditDescription=auditDescription, cmIpAddress=cmIpAddress, portShelfId=portShelfId, portTable=portTable, cardShelfId=cardShelfId, cardCarrierSerialNum=cardCarrierSerialNum, cardLastBootVersion=cardLastBootVersion, cardTestResult=cardTestResult, cardDetectedChange=cardDetectedChange, portMode=portMode, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSecState=cardSecState, cardBusSpeed=cardBusSpeed, diskVolumeSize=diskVolumeSize, portRxFlowControlMode=portRxFlowControlMode, auditTime=auditTime, errEvLogLevel=errEvLogLevel, portMinDsPower=portMinDsPower, diskVolumeDriveId=diskVolumeDriveId, portPrState=portPrState, cardHwVersion=cardHwVersion, diskVolumeName=diskVolumeName, portSecStateChange=portSecStateChange, trapCounter=trapCounter, equipmentDiag=equipmentDiag, portRxFlowControlDetected=portRxFlowControlDetected, cardTestEntry=cardTestEntry, auditId=auditId, cmResetStatus=cmResetStatus, cardDplxStatusChange=cardDplxStatusChange, fpgaErrorEventTable=fpgaErrorEventTable, shelfName=shelfName, cmVendor=cmVendor, equipmentDiagGroup=equipmentDiagGroup, shelfNumber=shelfNumber, cardCarrierHwVersion=cardCarrierHwVersion, cardTestName=cardTestName, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSubDetected=cardSubDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditTotalFailedCount=auditTotalFailedCount, portCardSubType=portCardSubType, cardTemperatureHighError=cardTemperatureHighError, equipmentShelfGroup=equipmentShelfGroup, cardTestTransId=cardTestTransId, cardBootVersion=cardBootVersion, auditCardId=auditCardId, cardTestType=cardTestType, equipmentAuditGroup=equipmentAuditGroup, downstreamPowerLoss=downstreamPowerLoss, cardPrStateChange=cardPrStateChange, portType=portType, cardTestResultNotification=cardTestResultNotification, diskVolumeShelfId=diskVolumeShelfId, cardSubType=cardSubType, cardLastBootSource=cardLastBootSource, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentTraps=equipmentTraps, cmResetNotification=cmResetNotification, portDplxStatus=portDplxStatus, cardTestId=cardTestId, auditTotalPassedCount=auditTotalPassedCount, portMaxDsPower=portMaxDsPower, diskVolumeEntry=diskVolumeEntry, auditTable=auditTable, systemClock=systemClock, cardFpgaSource=cardFpgaSource, TestCommand=TestCommand, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMibCompliances=cadEquipmentMibCompliances, shelfObjects=shelfObjects, portDescription=portDescription, TestTransactionId=TestTransactionId, portLastChangeTime=portLastChangeTime, cardTestSchedule=cardTestSchedule, cmResetInfo=cmResetInfo, portBgpId=portBgpId, equipmentPortGroup=equipmentPortGroup, systemGeneral=systemGeneral, cardSpareGroupMode=cardSpareGroupMode, cardPicHwVersion=cardPicHwVersion, TestResult=TestResult, cardSerialNum=cardSerialNum, cardTestScheduleCommand=cardTestScheduleCommand, cardTemperature=cardTemperature, auditPassedCount=auditPassedCount, cadEquipmentMib=cadEquipmentMib, cardShelfTestIndex=cardShelfTestIndex, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeTable=diskVolumeTable, equipmentStateGroup=equipmentStateGroup, auditResult=auditResult, diskVolumeTrapInh=diskVolumeTrapInh, auditLogThrottle=auditLogThrottle, cardPicDetected=cardPicDetected, portEntry=portEntry, diskVolumeCardId=diskVolumeCardId, cardTestDescription=cardTestDescription, portTxFlowControlMode=portTxFlowControlMode, eqDiagConfig=eqDiagConfig, cardPrState=cardPrState, cardPicSerialNum=cardPicSerialNum, auditCommand=auditCommand, cmResetReason=cmResetReason, equipmentTbl=equipmentTbl, portDocsIfIndex=portDocsIfIndex, auditEntry=auditEntry, cardDetected=cardDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestSchedule=TestSchedule, systemGeneralGroup=systemGeneralGroup, auditLogOutput=auditLogOutput, auditFailedCount=auditFailedCount, equipmentAudit=equipmentAudit, cardFwVersion=cardFwVersion, auditShelfId=auditShelfId, cardId=cardId, diskVolumeFileName=diskVolumeFileName, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetUpTime=cmResetUpTime, cmPrimarySID=cmPrimarySID, portAction=portAction, auditName=auditName, cardUpTime=cardUpTime, cardCPLDVersion=cardCPLDVersion, portDetectedMode=portDetectedMode, cmDevice=cmDevice, cardTemperatureHighWarn=cardTemperatureHighWarn, trapSeverity=trapSeverity, cardCarrierFwVersion=cardCarrierFwVersion, auditCycleCount=auditCycleCount, portPrStateChange=portPrStateChange, diagTestId=diagTestId, cardTrapInh=cardTrapInh, consoleOutput=consoleOutput, cardTestIndex=cardTestIndex, portNumber=portNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, cardAction=cardAction, equipmentState=equipmentState, portMacAddress=portMacAddress, cardTestTable=cardTestTable, auditStatus=auditStatus, TestType=TestType, cardFlashSize=cardFlashSize, cardLastChangeTime=cardLastChangeTime, portCurrDsPower=portCurrDsPower, portTrapInh=portTrapInh, portId=portId, diskDriveNumber=diskDriveNumber, cardFwUpdateStatus=cardFwUpdateStatus, cmRegistrationNotification=cmRegistrationNotification, cardCpuSpeed=cardCpuSpeed, verboseLevel=verboseLevel, cardEntry=cardEntry, portConnectorId=portConnectorId, systemKey=systemKey, cmResetClearNotification=cmResetClearNotification, cardTestTime=cardTestTime, portCardId=portCardId, cmUChannelID=cmUChannelID, cardDplxStatus=cardDplxStatus, equipmentCardTestGroup=equipmentCardTestGroup, cardType=cardType, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSpareGroupId=cardSpareGroupId, cardLastResetReason=cardLastResetReason, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardName=cardName, cardCpuType=cardCpuType, TestId=TestId, cardRamSize=cardRamSize, portDplxStatusChange=portDplxStatusChange, portNumChans=portNumChans, cardSwVersion=cardSwVersion, portAdminState=portAdminState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmMacAddress=cmMacAddress, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestCommand=cardTestCommand, shelfSwVersion=shelfSwVersion, portTxFlowControlDetected=portTxFlowControlDetected, equipmentCardGroup=equipmentCardGroup, equipmentError=equipmentError, errEvLoggingEnabled=errEvLoggingEnabled, portMacIfIndex=portMacIfIndex, auditAutoScheduling=auditAutoScheduling, errEventId=errEventId, portSecState=portSecState, cardTempNoReportNotification=cardTempNoReportNotification)
