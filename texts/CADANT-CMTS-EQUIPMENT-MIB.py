...
PortType, PortId, PortDetectedMode, DiskVolumeUsageLevel, FlowControlMode, UnknownState, PortMode, PrimaryState, SecondaryState, CardId, CardType, ShelfId, DuplexStatus, CardSubType, PicType, EqActionType, FirmwareSource, AdminState = mibBuilder.importSymbols("CADANT-TC", "PortType", "PortId", "PortDetectedMode", "DiskVolumeUsageLevel", "FlowControlMode", "UnknownState", "PortMode", "PrimaryState", "SecondaryState", "CardId", "CardType", "ShelfId", "DuplexStatus", "CardSubType", "PicType", "EqActionType", "FirmwareSource", "AdminState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBusSpeed=cardBusSpeed, cmResetInfo=cmResetInfo, auditLogOutput=auditLogOutput, cardSwVersion=cardSwVersion, auditAutoScheduling=auditAutoScheduling, auditEntry=auditEntry, cmResetReason=cmResetReason, cardTestEntry=cardTestEntry, cardTestIndex=cardTestIndex, cardFwUpdateStatus=cardFwUpdateStatus, cardShelfTestIndex=cardShelfTestIndex, cardEntry=cardEntry, cardSpareGroupId=cardSpareGroupId, cardPicHwVersion=cardPicHwVersion, auditLogThrottle=auditLogThrottle, diskVolumeCardId=diskVolumeCardId, fpgaErrorEventEntry=fpgaErrorEventEntry, portLastChangeTime=portLastChangeTime, TestScheduleCommand=TestScheduleCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeName=diskVolumeName, diskVolumeId=diskVolumeId, cardTempNoReportNotification=cardTempNoReportNotification, cardTestId=cardTestId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardNumPorts=cardNumPorts, portAdminState=portAdminState, cardTestTime=cardTestTime, errEvRowStatus=errEvRowStatus, fpgaErrorEventTable=fpgaErrorEventTable, cmPrimarySID=cmPrimarySID, cardFwVersion=cardFwVersion, auditCardId=auditCardId, auditFailedCount=auditFailedCount, cardTestDescription=cardTestDescription, trapCounter=trapCounter, equipmentError=equipmentError, cardTrapInh=cardTrapInh, cmDevice=cmDevice, auditResult=auditResult, cardName=cardName, portDocsIfIndex=portDocsIfIndex, auditDescription=auditDescription, cardSubDetected=cardSubDetected, portTxFlowControlDetected=portTxFlowControlDetected, equipmentAuditGroup=equipmentAuditGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, diagTestId=diagTestId, errEvLoggingEnabled=errEvLoggingEnabled, portSecStateChange=portSecStateChange, systemKey=systemKey, cmResetStatus=cmResetStatus, portNumber=portNumber, shelfSwVersion=shelfSwVersion, cardRamSize=cardRamSize, portEntry=portEntry, shelfName=shelfName, cardUpTime=cardUpTime, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portId=portId, downstreamPowerLoss=downstreamPowerLoss, cardTemperature=cardTemperature, equipmentShelfGroup=equipmentShelfGroup, TestSchedule=TestSchedule, cardTestScheduleCommand=cardTestScheduleCommand, cardId=cardId, TestId=TestId, diskVolumeGroup=diskVolumeGroup, auditShelfId=auditShelfId, cardCpuType=cardCpuType, cardSerialNum=cardSerialNum, errEvLogLevel=errEvLogLevel, auditName=auditName, equipmentPortGroup=equipmentPortGroup, portTrapInh=portTrapInh, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cmRegistrationNotification=cmRegistrationNotification, portDescription=portDescription, cardDetected=cardDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portDetectedMode=portDetectedMode, cmIpAddress=cmIpAddress, TestCommand=TestCommand, PYSNMP_MODULE_ID=cadEquipmentMib, cardLastBootSource=cardLastBootSource, portPrStateChange=portPrStateChange, shelfNumber=shelfNumber, cardTestName=cardTestName, cardPicDetected=cardPicDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, portMode=portMode, portTxFlowControlMode=portTxFlowControlMode, cardTestResultNotification=cardTestResultNotification, cardAdminState=cardAdminState, shelfObjects=shelfObjects, portCurrDsPower=portCurrDsPower, auditCycleCount=auditCycleCount, cardTestTransId=cardTestTransId, cardDplxStatus=cardDplxStatus, diskVolumeShelfId=diskVolumeShelfId, cmUChannelID=cmUChannelID, remainInDiagMode=remainInDiagMode, cardCPLDVersion=cardCPLDVersion, diskVolumeTable=diskVolumeTable, trapSeverity=trapSeverity, cardNumber=cardNumber, TestType=TestType, cardLastBootVersion=cardLastBootVersion, portTable=portTable, cardCarrierSerialNum=cardCarrierSerialNum, cardTemperatureHighError=cardTemperatureHighError, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmMacAddress=cmMacAddress, cardDplxStatusChange=cardDplxStatusChange, diskVolumeTrapInh=diskVolumeTrapInh, cardTable=cardTable, cardAction=cardAction, equipmentDiag=equipmentDiag, portType=portType, auditTotalPassedCount=auditTotalPassedCount, portAction=portAction, auditPassedCount=auditPassedCount, equipmentTbl=equipmentTbl, cardTestResultDesc=cardTestResultDesc, cardLastChangeTime=cardLastChangeTime, TestResult=TestResult, cardFlashSize=cardFlashSize, portDplxStatus=portDplxStatus, portDplxStatusChange=portDplxStatusChange, auditStatus=auditStatus, portRxFlowControlDetected=portRxFlowControlDetected, portRxFlowControlMode=portRxFlowControlMode, portPrState=portPrState, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeNumber=diskVolumeNumber, systemClock=systemClock, equipmentStateGroup=equipmentStateGroup, cardPrState=cardPrState, cardTestSchedule=cardTestSchedule, diskVolumeEntry=diskVolumeEntry, cardBootVersion=cardBootVersion, auditId=auditId, auditTime=auditTime, portCardId=portCardId, systemGeneralGroup=systemGeneralGroup, errEventId=errEventId, cardLastResetReason=cardLastResetReason, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibCompliances=cadEquipmentMibCompliances, consoleOutput=consoleOutput, auditTable=auditTable, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeFileName=diskVolumeFileName, verboseLevel=verboseLevel, cardTestCommand=cardTestCommand, portMinDsPower=portMinDsPower, equipmentCardGroup=equipmentCardGroup, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageLevel=diskVolumeUsageLevel, systemGeneral=systemGeneral, cmVendor=cmVendor, cardTestResult=cardTestResult, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestTable=cardTestTable, cardShelfId=cardShelfId, portMacIfIndex=portMacIfIndex, diskDriveNumber=diskDriveNumber, cardTestType=cardTestType, equipmentTraps=equipmentTraps, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetClearNotification=cmResetClearNotification, cardPrStateChange=cardPrStateChange, portSecState=portSecState, equipmentAudit=equipmentAudit, cmResetNotification=cmResetNotification, portBgpId=portBgpId, eqDiagConfig=eqDiagConfig, cadEquipmentMib=cadEquipmentMib, portConnectorId=portConnectorId, auditTotalFailedCount=auditTotalFailedCount, cardCpuSpeed=cardCpuSpeed, portNumChans=portNumChans, errEvRecoveryEnabled=errEvRecoveryEnabled, portShelfId=portShelfId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSecStateChange=cardSecStateChange, portMacAddress=portMacAddress, cardHwVersion=cardHwVersion, diskVolumeDriveId=diskVolumeDriveId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCommand=auditCommand, cardCarrierHwVersion=cardCarrierHwVersion, cardSecState=cardSecState, cmResetUpTime=cmResetUpTime, cardCarrierFwVersion=cardCarrierFwVersion, portMaxDsPower=portMaxDsPower, cardDetectedChange=cardDetectedChange, diskVolumeSize=diskVolumeSize, TestTransactionId=TestTransactionId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentState=equipmentState, cardFpgaSource=cardFpgaSource, cardType=cardType, cardSubType=cardSubType, portCardSubType=portCardSubType)
