...
PicType, PortId, SecondaryState, AdminState, PortDetectedMode, PrimaryState, UnknownState, FirmwareSource, DiskVolumeUsageLevel, CardId, CardSubType, DuplexStatus, PortType, PortMode, FlowControlMode, ShelfId, CardType, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PicType", "PortId", "SecondaryState", "AdminState", "PortDetectedMode", "PrimaryState", "UnknownState", "FirmwareSource", "DiskVolumeUsageLevel", "CardId", "CardSubType", "DuplexStatus", "PortType", "PortMode", "FlowControlMode", "ShelfId", "CardType", "EqActionType")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskDriveNumber=diskDriveNumber, cardDetected=cardDetected, diagTestId=diagTestId, portConnectorId=portConnectorId, diskVolumeCardId=diskVolumeCardId, cardLastBootVersion=cardLastBootVersion, TestType=TestType, cardTestName=cardTestName, cmResetReason=cmResetReason, cmResetStatus=cmResetStatus, portPrStateChange=portPrStateChange, equipmentShelfGroup=equipmentShelfGroup, portMacIfIndex=portMacIfIndex, portAction=portAction, cmVendor=cmVendor, equipmentStateGroup=equipmentStateGroup, portSecState=portSecState, cardRamSize=cardRamSize, portMaxDsPower=portMaxDsPower, shelfName=shelfName, cardPicHwVersion=cardPicHwVersion, cmIpAddress=cmIpAddress, diskVolumeDriveId=diskVolumeDriveId, diskVolumeId=diskVolumeId, auditAutoScheduling=auditAutoScheduling, cmResetUpTime=cmResetUpTime, equipmentAuditGroup=equipmentAuditGroup, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestId=cardTestId, errEvRowStatus=errEvRowStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, cmPrimarySID=cmPrimarySID, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, shelfNumber=shelfNumber, shelfObjects=shelfObjects, diskVolumeShelfId=diskVolumeShelfId, cardTemperature=cardTemperature, trapSeverity=trapSeverity, cardTestEntry=cardTestEntry, cardNumber=cardNumber, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMib=cadEquipmentMib, cardCpuSpeed=cardCpuSpeed, cardSubDetected=cardSubDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portNumChans=portNumChans, TestSchedule=TestSchedule, remainInDiagMode=remainInDiagMode, cardTestTime=cardTestTime, auditTotalFailedCount=auditTotalFailedCount, auditFailedCount=auditFailedCount, equipmentTraps=equipmentTraps, fpgaErrorEventGroup=fpgaErrorEventGroup, TestTransactionId=TestTransactionId, equipmentError=equipmentError, cardCarrierHwVersion=cardCarrierHwVersion, cardDplxStatusChange=cardDplxStatusChange, equipmentCardGroup=equipmentCardGroup, diskVolumeEntry=diskVolumeEntry, cardTestDescription=cardTestDescription, cardSpareGroupId=cardSpareGroupId, portTable=portTable, consoleOutput=consoleOutput, cmResetNotification=cmResetNotification, cardFlashSize=cardFlashSize, cardType=cardType, portDescription=portDescription, portCardSubType=portCardSubType, portAdminState=portAdminState, cardHwVersion=cardHwVersion, systemKey=systemKey, cardId=cardId, cardSwVersion=cardSwVersion, auditEntry=auditEntry, equipmentAudit=equipmentAudit, cmMacAddress=cmMacAddress, systemClock=systemClock, equipmentState=equipmentState, cardSubType=cardSubType, cardTempNoReportNotification=cardTempNoReportNotification, portShelfId=portShelfId, portTrapInh=portTrapInh, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastResetReason=cardLastResetReason, portDetectedMode=portDetectedMode, cardFwUpdateStatus=cardFwUpdateStatus, cardLastBootSource=cardLastBootSource, portId=portId, cardTestType=cardTestType, cardAction=cardAction, portNumber=portNumber, auditCommand=auditCommand, equipmentPortGroup=equipmentPortGroup, cardTestSchedule=cardTestSchedule, cardFpgaSource=cardFpgaSource, diskVolumeGroup=diskVolumeGroup, cardPrStateChange=cardPrStateChange, equipmentCardTestGroup=equipmentCardTestGroup, errEvLoggingEnabled=errEvLoggingEnabled, cardDetectedChange=cardDetectedChange, auditTotalPassedCount=auditTotalPassedCount, equipmentTbl=equipmentTbl, portCardId=portCardId, cardTestTransId=cardTestTransId, cmUChannelID=cmUChannelID, systemGeneralGroup=systemGeneralGroup, cardEntry=cardEntry, cardPrState=cardPrState, diskVolumeTable=diskVolumeTable, cmRegistrationNotification=cmRegistrationNotification, diskVolumeTrapInh=diskVolumeTrapInh, portDocsIfIndex=portDocsIfIndex, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSerialNum=cardSerialNum, auditCardId=auditCardId, shelfSwVersion=shelfSwVersion, cardNumPorts=cardNumPorts, cardTemperatureHighError=cardTemperatureHighError, downstreamPowerLoss=downstreamPowerLoss, cardSecState=cardSecState, auditShelfId=auditShelfId, equipmentDiagGroup=equipmentDiagGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPicDetected=cardPicDetected, TestScheduleCommand=TestScheduleCommand, cardLastChangeTime=cardLastChangeTime, cardFwVersion=cardFwVersion, diskVolumeDiskSize=diskVolumeDiskSize, cardSecStateChange=cardSecStateChange, cardName=cardName, cardTrapInh=cardTrapInh, portRxFlowControlMode=portRxFlowControlMode, cardTestCommand=cardTestCommand, cardTestResult=cardTestResult, cardSpareGroupMode=cardSpareGroupMode, TestId=TestId, portBgpId=portBgpId, TestResult=TestResult, portEntry=portEntry, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, auditPassedCount=auditPassedCount, cardTable=cardTable, diskVolumeName=diskVolumeName, portTxFlowControlMode=portTxFlowControlMode, portType=portType, auditCycleCount=auditCycleCount, cardCPLDVersion=cardCPLDVersion, portSecStateChange=portSecStateChange, diskVolumeFileName=diskVolumeFileName, verboseLevel=verboseLevel, auditId=auditId, portLastChangeTime=portLastChangeTime, cmResetClearNotification=cmResetClearNotification, equipmentDiag=equipmentDiag, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestScheduleCommand=cardTestScheduleCommand, auditResult=auditResult, portMacAddress=portMacAddress, diskVolumeUsageLevel=diskVolumeUsageLevel, cardShelfTestIndex=cardShelfTestIndex, eqDiagConfig=eqDiagConfig, PYSNMP_MODULE_ID=cadEquipmentMib, portTxFlowControlDetected=portTxFlowControlDetected, portDplxStatusChange=portDplxStatusChange, auditTime=auditTime, trapCounter=trapCounter, TestCommand=TestCommand, errEvLogLevel=errEvLogLevel, auditLogOutput=auditLogOutput, systemGeneral=systemGeneral, auditStatus=auditStatus, errEventId=errEventId, cardShelfId=cardShelfId, cardAdminState=cardAdminState, cmDevice=cmDevice, portMode=portMode, diskVolumeSize=diskVolumeSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestIndex=cardTestIndex, portCurrDsPower=portCurrDsPower, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestResultNotification=cardTestResultNotification, portDplxStatus=portDplxStatus, cmResetInfo=cmResetInfo, cardUpTime=cardUpTime, cardTestResultDesc=cardTestResultDesc, auditTable=auditTable, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibGroup=cadEquipmentMibGroup, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portRxFlowControlDetected=portRxFlowControlDetected, auditDescription=auditDescription, cardBootVersion=cardBootVersion, cardCpuType=cardCpuType, portPrState=portPrState, cardCarrierSerialNum=cardCarrierSerialNum, portMinDsPower=portMinDsPower, diskVolumeNumber=diskVolumeNumber, auditName=auditName, cardDplxStatus=cardDplxStatus, cardTestTable=cardTestTable, cardBusSpeed=cardBusSpeed, auditLogThrottle=auditLogThrottle)
