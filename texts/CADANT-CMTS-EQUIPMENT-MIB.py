...
DiskVolumeUsageLevel, CardSubType, EqActionType, FlowControlMode, DuplexStatus, PortId, PortDetectedMode, PortType, ShelfId, PrimaryState, AdminState, PortMode, PicType, UnknownState, SecondaryState, CardType, CardId, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "CardSubType", "EqActionType", "FlowControlMode", "DuplexStatus", "PortId", "PortDetectedMode", "PortType", "ShelfId", "PrimaryState", "AdminState", "PortMode", "PicType", "UnknownState", "SecondaryState", "CardType", "CardId", "FirmwareSource")
...
RowStatus, TextualConvention, DateAndTime, TimeStamp, TruthValue, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "DateAndTime", "TimeStamp", "TruthValue", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", fpgaErrorEventTable=fpgaErrorEventTable, cardTestType=cardTestType, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageLevel=diskVolumeUsageLevel, trapCounter=trapCounter, diskVolumeTrapInh=diskVolumeTrapInh, remainInDiagMode=remainInDiagMode, auditTime=auditTime, auditTotalPassedCount=auditTotalPassedCount, portBgpId=portBgpId, equipmentDiag=equipmentDiag, equipmentCardGroup=equipmentCardGroup, portAction=portAction, cardTestTime=cardTestTime, cmMacAddress=cmMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, auditId=auditId, equipmentAudit=equipmentAudit, cadEquipmentMib=cadEquipmentMib, cardTestSchedule=cardTestSchedule, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestId=cardTestId, cardTrapInh=cardTrapInh, cardTestTransId=cardTestTransId, auditLogThrottle=auditLogThrottle, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestDescription=cardTestDescription, equipmentShelfGroup=equipmentShelfGroup, cardSecState=cardSecState, cardTestTable=cardTestTable, auditCommand=auditCommand, errEventId=errEventId, portTxFlowControlDetected=portTxFlowControlDetected, auditTotalFailedCount=auditTotalFailedCount, cmResetInfo=cmResetInfo, cardRamSize=cardRamSize, diskVolumeShelfId=diskVolumeShelfId, diskDriveNumber=diskDriveNumber, portPrStateChange=portPrStateChange, cadEquipmentMibGroup=cadEquipmentMibGroup, cardFlashSize=cardFlashSize, trapSeverity=trapSeverity, cardTestIndex=cardTestIndex, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestEntry=cardTestEntry, diskVolumeTable=diskVolumeTable, diskVolumeNumber=diskVolumeNumber, cmUChannelID=cmUChannelID, cardNumber=cardNumber, portCardSubType=portCardSubType, diskVolumeSize=diskVolumeSize, cardFwVersion=cardFwVersion, cmDevice=cmDevice, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardType=cardType, cardPicDetected=cardPicDetected, portNumber=portNumber, cardTempOverHeatNotification=cardTempOverHeatNotification, PYSNMP_MODULE_ID=cadEquipmentMib, cardCpuType=cardCpuType, portPrState=portPrState, cardLastBootVersion=cardLastBootVersion, cmResetReason=cmResetReason, equipmentTbl=equipmentTbl, portMaxDsPower=portMaxDsPower, diskVolumeDriveId=diskVolumeDriveId, cardTestCommand=cardTestCommand, errEvLoggingEnabled=errEvLoggingEnabled, cardPrStateChange=cardPrStateChange, portNumChans=portNumChans, shelfSwVersion=shelfSwVersion, cmPrimarySID=cmPrimarySID, fpgaErrorEventEntry=fpgaErrorEventEntry, portType=portType, auditShelfId=auditShelfId, portMacAddress=portMacAddress, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTable=auditTable, portRxFlowControlMode=portRxFlowControlMode, cardLastResetReason=cardLastResetReason, portShelfId=portShelfId, cardFwUpdateStatus=cardFwUpdateStatus, eqDiagConfig=eqDiagConfig, diskVolumeGroup=diskVolumeGroup, TestSchedule=TestSchedule, consoleOutput=consoleOutput, cardTestResult=cardTestResult, portMode=portMode, cardPicSerialNum=cardPicSerialNum, auditResult=auditResult, cardFpgaSource=cardFpgaSource, portEntry=portEntry, cardTemperatureHighError=cardTemperatureHighError, shelfName=shelfName, cardDetected=cardDetected, cardTestScheduleCommand=cardTestScheduleCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditPassedCount=auditPassedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portSecStateChange=portSecStateChange, cardDetectedChange=cardDetectedChange, cardCpuSpeed=cardCpuSpeed, cardSubDetected=cardSubDetected, portDocsIfIndex=portDocsIfIndex, cardShelfTestIndex=cardShelfTestIndex, cardSwVersion=cardSwVersion, cardLastChangeTime=cardLastChangeTime, auditCardId=auditCardId, portMinDsPower=portMinDsPower, cadEquipmentMibConformance=cadEquipmentMibConformance, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentPortGroup=equipmentPortGroup, cardCarrierHwVersion=cardCarrierHwVersion, shelfNumber=shelfNumber, cardSpareGroupId=cardSpareGroupId, cmResetClearNotification=cmResetClearNotification, equipmentAuditGroup=equipmentAuditGroup, cardAdminState=cardAdminState, cardCarrierSerialNum=cardCarrierSerialNum, TestCommand=TestCommand, diskVolumeId=diskVolumeId, cardSpareGroupMode=cardSpareGroupMode, equipmentDiagGroup=equipmentDiagGroup, cardLastBootSource=cardLastBootSource, TestId=TestId, cardPrState=cardPrState, diskVolumeDiskSize=diskVolumeDiskSize, cmResetStatus=cmResetStatus, shelfObjects=shelfObjects, auditStatus=auditStatus, cardId=cardId, auditFailedCount=auditFailedCount, systemClock=systemClock, cardTempNoReportNotification=cardTempNoReportNotification, portAdminState=portAdminState, cmResetNotification=cmResetNotification, cmIpAddress=cmIpAddress, portTable=portTable, cmRegistrationNotification=cmRegistrationNotification, TestTransactionId=TestTransactionId, auditDescription=auditDescription, cmVendor=cmVendor, portId=portId, errEvLogLevel=errEvLogLevel, cardUpTime=cardUpTime, cardDplxStatus=cardDplxStatus, verboseLevel=verboseLevel, cardAction=cardAction, diskVolumeName=diskVolumeName, diskVolumeFileName=diskVolumeFileName, cardTestResultDesc=cardTestResultDesc, diskVolumeCardId=diskVolumeCardId, cardTestResultNotification=cardTestResultNotification, equipmentCardTestGroup=equipmentCardTestGroup, TestType=TestType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCarrierFwVersion=cardCarrierFwVersion, cardHwVersion=cardHwVersion, portCurrDsPower=portCurrDsPower, portLastChangeTime=portLastChangeTime, diagTestId=diagTestId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentTraps=equipmentTraps, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSerialNum=cardSerialNum, errEvRowStatus=errEvRowStatus, equipmentError=equipmentError, portDetectedMode=portDetectedMode, systemGeneralGroup=systemGeneralGroup, portTxFlowControlMode=portTxFlowControlMode, downstreamPowerLoss=downstreamPowerLoss, cmResetUpTime=cmResetUpTime, cardBusSpeed=cardBusSpeed, portSecState=portSecState, cardTemperature=cardTemperature, cardDplxStatusChange=cardDplxStatusChange, cardSubType=cardSubType, cardShelfId=cardShelfId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, systemGeneral=systemGeneral, cardEntry=cardEntry, cardSecStateChange=cardSecStateChange, cardPicHwVersion=cardPicHwVersion, cardTestName=cardTestName, systemKey=systemKey, cardName=cardName, portTrapInh=portTrapInh, fpgaErrorEventGroup=fpgaErrorEventGroup, portMacIfIndex=portMacIfIndex, portDplxStatus=portDplxStatus, diskVolumeEntry=diskVolumeEntry, portCardId=portCardId, auditLogOutput=auditLogOutput, auditCycleCount=auditCycleCount, cardTable=cardTable, cardBootVersion=cardBootVersion, equipmentStateGroup=equipmentStateGroup, portDescription=portDescription, TestResult=TestResult, portConnectorId=portConnectorId, TestScheduleCommand=TestScheduleCommand, auditName=auditName, cardNumPorts=cardNumPorts, cardCPLDVersion=cardCPLDVersion, auditEntry=auditEntry, equipmentState=equipmentState)
