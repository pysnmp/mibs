...
PortId, PrimaryState, ShelfId, PicType, DuplexStatus, PortMode, FirmwareSource, SecondaryState, PortType, DiskVolumeUsageLevel, FlowControlMode, AdminState, CardId, CardSubType, CardType, EqActionType, PortDetectedMode, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PortId", "PrimaryState", "ShelfId", "PicType", "DuplexStatus", "PortMode", "FirmwareSource", "SecondaryState", "PortType", "DiskVolumeUsageLevel", "FlowControlMode", "AdminState", "CardId", "CardSubType", "CardType", "EqActionType", "PortDetectedMode", "UnknownState")
...
TruthValue, DateAndTime, TextualConvention, RowStatus, DisplayString, TimeStamp, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "TextualConvention", "RowStatus", "DisplayString", "TimeStamp", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmResetStatus=cmResetStatus, cardTestScheduleCommand=cardTestScheduleCommand, auditTotalFailedCount=auditTotalFailedCount, portCardSubType=portCardSubType, PYSNMP_MODULE_ID=cadEquipmentMib, portMacIfIndex=portMacIfIndex, auditId=auditId, cardFlashSize=cardFlashSize, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTotalPassedCount=auditTotalPassedCount, equipmentTbl=equipmentTbl, cardPicDetected=cardPicDetected, cardDplxStatus=cardDplxStatus, cardAction=cardAction, cardTemperatureHighError=cardTemperatureHighError, portDescription=portDescription, portTxFlowControlMode=portTxFlowControlMode, cardTestTable=cardTestTable, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestResultDesc=cardTestResultDesc, cardLastResetReason=cardLastResetReason, diskVolumeNumber=diskVolumeNumber, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTrapInh=cardTrapInh, cardTestId=cardTestId, cardDplxStatusChange=cardDplxStatusChange, errEvRowStatus=errEvRowStatus, diskVolumeEntry=diskVolumeEntry, TestSchedule=TestSchedule, eqDiagConfig=eqDiagConfig, trapSeverity=trapSeverity, cardPrState=cardPrState, diskVolumeName=diskVolumeName, cardFwUpdateStatus=cardFwUpdateStatus, cardShelfId=cardShelfId, portPrState=portPrState, portAction=portAction, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portAdminState=portAdminState, cardTestDescription=cardTestDescription, cmMacAddress=cmMacAddress, cmResetNotification=cmResetNotification, diskVolumeGroup=diskVolumeGroup, portEntry=portEntry, cmUChannelID=cmUChannelID, cardSerialNum=cardSerialNum, portTable=portTable, equipmentPortGroup=equipmentPortGroup, portPrStateChange=portPrStateChange, cmResetUpTime=cmResetUpTime, cardSwVersion=cardSwVersion, portId=portId, diskVolumeId=diskVolumeId, portSecState=portSecState, verboseLevel=verboseLevel, portMaxDsPower=portMaxDsPower, diskVolumeFileName=diskVolumeFileName, cardTestTransId=cardTestTransId, diagTestId=diagTestId, cardSubDetected=cardSubDetected, portMinDsPower=portMinDsPower, diskVolumeSize=diskVolumeSize, portDplxStatus=portDplxStatus, portCardId=portCardId, remainInDiagMode=remainInDiagMode, cardNumber=cardNumber, portNumChans=portNumChans, cardDetectedChange=cardDetectedChange, fpgaErrorEventGroup=fpgaErrorEventGroup, cardBootVersion=cardBootVersion, trapCounter=trapCounter, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetReason=cmResetReason, portMode=portMode, errEvLoggingEnabled=errEvLoggingEnabled, cmRegistrationNotification=cmRegistrationNotification, equipmentCardTestGroup=equipmentCardTestGroup, cardCpuSpeed=cardCpuSpeed, cmPrimarySID=cmPrimarySID, portDetectedMode=portDetectedMode, portTrapInh=portTrapInh, diskVolumeTable=diskVolumeTable, cardPicHwVersion=cardPicHwVersion, cardTestEntry=cardTestEntry, auditFailedCount=auditFailedCount, diskVolumeDriveId=diskVolumeDriveId, errEvLogLevel=errEvLogLevel, cardTestResultNotification=cardTestResultNotification, shelfName=shelfName, portBgpId=portBgpId, cardUpTime=cardUpTime, portCurrDsPower=portCurrDsPower, portMacAddress=portMacAddress, auditPassedCount=auditPassedCount, cardTestCommand=cardTestCommand, shelfSwVersion=shelfSwVersion, cardTemperature=cardTemperature, cardBusSpeed=cardBusSpeed, equipmentDiag=equipmentDiag, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeShelfId=diskVolumeShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, TestTransactionId=TestTransactionId, portDplxStatusChange=portDplxStatusChange, errEvRecoveryEnabled=errEvRecoveryEnabled, portLastChangeTime=portLastChangeTime, cardType=cardType, errEventId=errEventId, portDocsIfIndex=portDocsIfIndex, auditLogThrottle=auditLogThrottle, equipmentShelfGroup=equipmentShelfGroup, cardLastBootVersion=cardLastBootVersion, cardPrStateChange=cardPrStateChange, auditShelfId=auditShelfId, auditResult=auditResult, cardCPLDVersion=cardCPLDVersion, cardRamSize=cardRamSize, cmResetClearNotification=cmResetClearNotification, cardDetected=cardDetected, systemGeneralGroup=systemGeneralGroup, auditAutoScheduling=auditAutoScheduling, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTime=auditTime, portSecStateChange=portSecStateChange, cardEntry=cardEntry, TestScheduleCommand=TestScheduleCommand, auditStatus=auditStatus, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeCardId=diskVolumeCardId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, downstreamPowerLoss=downstreamPowerLoss, equipmentAudit=equipmentAudit, cardCarrierHwVersion=cardCarrierHwVersion, cardSpareGroupMode=cardSpareGroupMode, cmResetInfo=cmResetInfo, cardTable=cardTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardId=cardId, diskVolumeTrapInh=diskVolumeTrapInh, cardSubType=cardSubType, cardSecState=cardSecState, systemClock=systemClock, auditCardId=auditCardId, systemGeneral=systemGeneral, equipmentAuditGroup=equipmentAuditGroup, portRxFlowControlDetected=portRxFlowControlDetected, auditLogOutput=auditLogOutput, equipmentTraps=equipmentTraps, portType=portType, equipmentDiagGroup=equipmentDiagGroup, cardFpgaSource=cardFpgaSource, cardName=cardName, equipmentError=equipmentError, shelfNumber=shelfNumber, TestResult=TestResult, portConnectorId=portConnectorId, cardTestType=cardTestType, auditTable=auditTable, portNumber=portNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, cardShelfTestIndex=cardShelfTestIndex, auditDescription=auditDescription, cardTestIndex=cardTestIndex, auditEntry=auditEntry, auditName=auditName, cardNumPorts=cardNumPorts, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSecStateChange=cardSecStateChange, diskDriveNumber=diskDriveNumber, cardHwVersion=cardHwVersion, equipmentState=equipmentState, cardCarrierSerialNum=cardCarrierSerialNum, cardTestTime=cardTestTime, TestId=TestId, consoleOutput=consoleOutput, cardAdminState=cardAdminState, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestName=cardTestName, cardTestResult=cardTestResult, auditCommand=auditCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestSchedule=cardTestSchedule, TestType=TestType, TestCommand=TestCommand, auditCycleCount=auditCycleCount, cmIpAddress=cmIpAddress, equipmentStateGroup=equipmentStateGroup, fpgaErrorEventTable=fpgaErrorEventTable, cardSpareGroupId=cardSpareGroupId, cardCarrierFwVersion=cardCarrierFwVersion, portRxFlowControlMode=portRxFlowControlMode, equipmentCardGroup=equipmentCardGroup, shelfObjects=shelfObjects, cmVendor=cmVendor, cardLastChangeTime=cardLastChangeTime, cardPicSerialNum=cardPicSerialNum, cardCpuType=cardCpuType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmDevice=cmDevice, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portShelfId=portShelfId, cardFwVersion=cardFwVersion, cardLastBootSource=cardLastBootSource, cadEquipmentMib=cadEquipmentMib, systemKey=systemKey)
