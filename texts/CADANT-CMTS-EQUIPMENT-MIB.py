...
CardId, EqActionType, CardType, AdminState, FlowControlMode, DiskVolumeUsageLevel, DuplexStatus, PicType, UnknownState, FirmwareSource, PortId, PortDetectedMode, PrimaryState, PortMode, SecondaryState, CardSubType, ShelfId, PortType = mibBuilder.importSymbols("CADANT-TC", "CardId", "EqActionType", "CardType", "AdminState", "FlowControlMode", "DiskVolumeUsageLevel", "DuplexStatus", "PicType", "UnknownState", "FirmwareSource", "PortId", "PortDetectedMode", "PrimaryState", "PortMode", "SecondaryState", "CardSubType", "ShelfId", "PortType")
...
TruthValue, DisplayString, TextualConvention, RowStatus, TimeStamp, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DisplayString", "TextualConvention", "RowStatus", "TimeStamp", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeNumber=diskVolumeNumber, equipmentTraps=equipmentTraps, cmUChannelID=cmUChannelID, cardTestIndex=cardTestIndex, portLastChangeTime=portLastChangeTime, cardShelfId=cardShelfId, cardTestResult=cardTestResult, cardTestResultNotification=cardTestResultNotification, cardName=cardName, diskVolumeSize=diskVolumeSize, fpgaErrorEventGroup=fpgaErrorEventGroup, cardBootVersion=cardBootVersion, cardTestResultDesc=cardTestResultDesc, systemKey=systemKey, equipmentCardTestGroup=equipmentCardTestGroup, shelfNumber=shelfNumber, auditCycleCount=auditCycleCount, cardFwUpdateStatus=cardFwUpdateStatus, cardCpuSpeed=cardCpuSpeed, diskVolumeEntry=diskVolumeEntry, cardTestSchedule=cardTestSchedule, auditTotalPassedCount=auditTotalPassedCount, auditId=auditId, equipmentPortGroup=equipmentPortGroup, cardTestDescription=cardTestDescription, cardTestTransId=cardTestTransId, portSecStateChange=portSecStateChange, TestScheduleCommand=TestScheduleCommand, errEventId=errEventId, auditEntry=auditEntry, cardDplxStatus=cardDplxStatus, cardPicDetected=cardPicDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestCommand=TestCommand, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardRamSize=cardRamSize, cmDevice=cmDevice, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portMacIfIndex=portMacIfIndex, equipmentState=equipmentState, fpgaErrorEventEntry=fpgaErrorEventEntry, portNumChans=portNumChans, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCarrierSerialNum=cardCarrierSerialNum, cmVendor=cmVendor, cardLastBootVersion=cardLastBootVersion, portCardId=portCardId, cardTestType=cardTestType, cardHwVersion=cardHwVersion, portMaxDsPower=portMaxDsPower, PYSNMP_MODULE_ID=cadEquipmentMib, TestId=TestId, auditLogThrottle=auditLogThrottle, auditShelfId=auditShelfId, auditDescription=auditDescription, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardBusSpeed=cardBusSpeed, diskVolumeDiskSize=diskVolumeDiskSize, cardShelfTestIndex=cardShelfTestIndex, cardTestScheduleCommand=cardTestScheduleCommand, portSecState=portSecState, diskVolumeTable=diskVolumeTable, cardPrStateChange=cardPrStateChange, cmResetClearNotification=cmResetClearNotification, auditStatus=auditStatus, diskVolumeShelfId=diskVolumeShelfId, portBgpId=portBgpId, cardTestTime=cardTestTime, portPrStateChange=portPrStateChange, cardCarrierFwVersion=cardCarrierFwVersion, portRxFlowControlMode=portRxFlowControlMode, portRxFlowControlDetected=portRxFlowControlDetected, portDetectedMode=portDetectedMode, cardLastResetReason=cardLastResetReason, auditTime=auditTime, systemGeneral=systemGeneral, cardCPLDVersion=cardCPLDVersion, cardDetectedChange=cardDetectedChange, auditName=auditName, diagTestId=diagTestId, portConnectorId=portConnectorId, cardTempOverHeatNotification=cardTempOverHeatNotification, trapSeverity=trapSeverity, diskVolumeName=diskVolumeName, cmPrimarySID=cmPrimarySID, cardTestCommand=cardTestCommand, cardPicSerialNum=cardPicSerialNum, portAdminState=portAdminState, auditTable=auditTable, consoleOutput=consoleOutput, diskVolumeId=diskVolumeId, auditCardId=auditCardId, cmIpAddress=cmIpAddress, diskVolumeDriveId=diskVolumeDriveId, portMacAddress=portMacAddress, cardTemperatureHighError=cardTemperatureHighError, errEvLoggingEnabled=errEvLoggingEnabled, cardTestName=cardTestName, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSecState=cardSecState, portTxFlowControlDetected=portTxFlowControlDetected, cardTestId=cardTestId, errEvRecoveryEnabled=errEvRecoveryEnabled, portDocsIfIndex=portDocsIfIndex, cardId=cardId, auditAutoScheduling=auditAutoScheduling, systemGeneralGroup=systemGeneralGroup, cardSubType=cardSubType, cardPrState=cardPrState, cardTable=cardTable, cardTemperature=cardTemperature, downstreamPowerLoss=downstreamPowerLoss, cardDplxStatusChange=cardDplxStatusChange, equipmentAudit=equipmentAudit, errEvRowStatus=errEvRowStatus, portDplxStatus=portDplxStatus, cardTestTable=cardTestTable, cadEquipmentMib=cadEquipmentMib, cardTemperatureHighWarn=cardTemperatureHighWarn, remainInDiagMode=remainInDiagMode, cardAdminState=cardAdminState, portType=portType, cmResetReason=cmResetReason, cardType=cardType, cardUpTime=cardUpTime, portMode=portMode, diskVolumeCardId=diskVolumeCardId, cardLastBootSource=cardLastBootSource, diskVolumeTrapInh=diskVolumeTrapInh, portNumber=portNumber, portPrState=portPrState, diskDriveNumber=diskDriveNumber, auditCommand=auditCommand, TestResult=TestResult, TestSchedule=TestSchedule, cardLastChangeTime=cardLastChangeTime, cardNumPorts=cardNumPorts, auditLogOutput=auditLogOutput, cmResetUpTime=cmResetUpTime, auditTotalFailedCount=auditTotalFailedCount, cardTempNoReportNotification=cardTempNoReportNotification, auditResult=auditResult, equipmentDiag=equipmentDiag, cardSpareGroupId=cardSpareGroupId, cardPicHwVersion=cardPicHwVersion, cmMacAddress=cmMacAddress, portId=portId, portCurrDsPower=portCurrDsPower, cardSpareGroupMode=cardSpareGroupMode, cardFlashSize=cardFlashSize, cmResetInfo=cmResetInfo, equipmentDiagGroup=equipmentDiagGroup, shelfSwVersion=shelfSwVersion, trapCounter=trapCounter, auditFailedCount=auditFailedCount, portDplxStatusChange=portDplxStatusChange, cardNumber=cardNumber, cardCpuType=cardCpuType, portEntry=portEntry, errEvLogLevel=errEvLogLevel, equipmentError=equipmentError, cardSecStateChange=cardSecStateChange, cardFpgaSource=cardFpgaSource, cmResetNotification=cmResetNotification, cmResetStatus=cmResetStatus, cardDetected=cardDetected, cardSubDetected=cardSubDetected, portTable=portTable, portCardSubType=portCardSubType, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentAuditGroup=equipmentAuditGroup, diskVolumeFileName=diskVolumeFileName, portAction=portAction, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTrapInh=cardTrapInh, cardTestEntry=cardTestEntry, shelfName=shelfName, cardSwVersion=cardSwVersion, portTxFlowControlMode=portTxFlowControlMode, portMinDsPower=portMinDsPower, TestType=TestType, eqDiagConfig=eqDiagConfig, diskVolumeGroup=diskVolumeGroup, portTrapInh=portTrapInh, equipmentShelfGroup=equipmentShelfGroup, cardCarrierHwVersion=cardCarrierHwVersion, shelfObjects=shelfObjects, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmRegistrationNotification=cmRegistrationNotification, systemClock=systemClock, cardSerialNum=cardSerialNum, auditPassedCount=auditPassedCount, portShelfId=portShelfId, cardFwVersion=cardFwVersion, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentCardGroup=equipmentCardGroup, TestTransactionId=TestTransactionId, equipmentTbl=equipmentTbl, cardAction=cardAction, verboseLevel=verboseLevel, portDescription=portDescription, cardEntry=cardEntry)
