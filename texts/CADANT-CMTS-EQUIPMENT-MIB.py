...
PortMode, AdminState, CardId, EqActionType, PortType, PortId, CardType, PicType, PrimaryState, FlowControlMode, DiskVolumeUsageLevel, PortDetectedMode, DuplexStatus, ShelfId, SecondaryState, UnknownState, CardSubType, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "PortMode", "AdminState", "CardId", "EqActionType", "PortType", "PortId", "CardType", "PicType", "PrimaryState", "FlowControlMode", "DiskVolumeUsageLevel", "PortDetectedMode", "DuplexStatus", "ShelfId", "SecondaryState", "UnknownState", "CardSubType", "FirmwareSource")
...
DisplayString, TimeStamp, TextualConvention, TruthValue, RowStatus, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "TextualConvention", "TruthValue", "RowStatus", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSecState=cardSecState, cardTestEntry=cardTestEntry, portConnectorId=portConnectorId, fpgaErrorEventEntry=fpgaErrorEventEntry, trapCounter=trapCounter, equipmentError=equipmentError, cardName=cardName, cardFlashSize=cardFlashSize, cardDplxStatus=cardDplxStatus, cardShelfId=cardShelfId, auditResult=auditResult, portDocsIfIndex=portDocsIfIndex, cardFwVersion=cardFwVersion, cardHwVersion=cardHwVersion, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, portAction=portAction, portNumber=portNumber, cardTempOverHeatNotification=cardTempOverHeatNotification, auditEntry=auditEntry, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCpuType=cardCpuType, cardCarrierFwVersion=cardCarrierFwVersion, auditTable=auditTable, auditCycleCount=auditCycleCount, systemClock=systemClock, systemGeneralGroup=systemGeneralGroup, cardLastResetReason=cardLastResetReason, cmResetClearNotification=cmResetClearNotification, downstreamPowerLoss=downstreamPowerLoss, cardDplxStatusChange=cardDplxStatusChange, equipmentDiag=equipmentDiag, cardBusSpeed=cardBusSpeed, auditDescription=auditDescription, TestResult=TestResult, portPrState=portPrState, portMacAddress=portMacAddress, diskVolumeShelfId=diskVolumeShelfId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeDriveId=diskVolumeDriveId, cardTestName=cardTestName, shelfObjects=shelfObjects, cardTable=cardTable, remainInDiagMode=remainInDiagMode, portSecStateChange=portSecStateChange, cardCpuSpeed=cardCpuSpeed, cmPrimarySID=cmPrimarySID, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentState=equipmentState, TestSchedule=TestSchedule, cardTestSchedule=cardTestSchedule, cardSecStateChange=cardSecStateChange, portDplxStatus=portDplxStatus, diskVolumeId=diskVolumeId, cardTestCommand=cardTestCommand, cardTestType=cardTestType, diskVolumeFileName=diskVolumeFileName, equipmentAuditGroup=equipmentAuditGroup, cardTestResultNotification=cardTestResultNotification, cardSubType=cardSubType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestTransactionId=TestTransactionId, cardCPLDVersion=cardCPLDVersion, PYSNMP_MODULE_ID=cadEquipmentMib, shelfName=shelfName, cadEquipmentMib=cadEquipmentMib, cardTemperatureHighError=cardTemperatureHighError, portMaxDsPower=portMaxDsPower, portDescription=portDescription, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditTotalFailedCount=auditTotalFailedCount, cmResetInfo=cmResetInfo, portDplxStatusChange=portDplxStatusChange, diskVolumeNumber=diskVolumeNumber, portBgpId=portBgpId, cmVendor=cmVendor, systemGeneral=systemGeneral, cardTestTransId=cardTestTransId, auditName=auditName, portCardSubType=portCardSubType, fpgaErrorEventGroup=fpgaErrorEventGroup, portMinDsPower=portMinDsPower, cardPrStateChange=cardPrStateChange, verboseLevel=verboseLevel, cardTestIndex=cardTestIndex, cmRegistrationNotification=cmRegistrationNotification, cardType=cardType, fpgaErrorEventTable=fpgaErrorEventTable, shelfSwVersion=shelfSwVersion, cmResetNotification=cmResetNotification, equipmentShelfGroup=equipmentShelfGroup, diagTestId=diagTestId, cmMacAddress=cmMacAddress, cardSpareGroupId=cardSpareGroupId, consoleOutput=consoleOutput, diskVolumeUsagePercentage=diskVolumeUsagePercentage, eqDiagConfig=eqDiagConfig, diskVolumeName=diskVolumeName, cardDetectedChange=cardDetectedChange, cardId=cardId, portRxFlowControlMode=portRxFlowControlMode, auditId=auditId, equipmentAudit=equipmentAudit, auditLogThrottle=auditLogThrottle, cardCarrierHwVersion=cardCarrierHwVersion, cardSerialNum=cardSerialNum, portId=portId, diskVolumeTable=diskVolumeTable, portShelfId=portShelfId, shelfNumber=shelfNumber, errEvLoggingEnabled=errEvLoggingEnabled, cardAction=cardAction, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentStateGroup=equipmentStateGroup, cardTrapInh=cardTrapInh, portLastChangeTime=portLastChangeTime, auditCardId=auditCardId, diskVolumeGroup=diskVolumeGroup, cardTempNoReportNotification=cardTempNoReportNotification, portAdminState=portAdminState, equipmentDiagGroup=equipmentDiagGroup, cardLastBootSource=cardLastBootSource, trapSeverity=trapSeverity, cardTestResultDesc=cardTestResultDesc, cardSwVersion=cardSwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, portMode=portMode, cmIpAddress=cmIpAddress, cardNumber=cardNumber, cardFwUpdateStatus=cardFwUpdateStatus, auditPassedCount=auditPassedCount, cmResetUpTime=cmResetUpTime, auditShelfId=auditShelfId, auditCommand=auditCommand, cardShelfTestIndex=cardShelfTestIndex, cardPicHwVersion=cardPicHwVersion, auditAutoScheduling=auditAutoScheduling, auditFailedCount=auditFailedCount, equipmentCardGroup=equipmentCardGroup, cardTestResult=cardTestResult, portTxFlowControlDetected=portTxFlowControlDetected, TestScheduleCommand=TestScheduleCommand, cardDetected=cardDetected, cardAdminState=cardAdminState, equipmentTbl=equipmentTbl, cardLastBootVersion=cardLastBootVersion, cardTestDescription=cardTestDescription, cardRamSize=cardRamSize, cmResetReason=cmResetReason, auditTime=auditTime, systemKey=systemKey, diskVolumeSize=diskVolumeSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestScheduleCommand=cardTestScheduleCommand, auditStatus=auditStatus, portRxFlowControlDetected=portRxFlowControlDetected, cardEntry=cardEntry, cardTestTime=cardTestTime, cardTemperatureHighWarn=cardTemperatureHighWarn, errEvLogLevel=errEvLogLevel, TestCommand=TestCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastChangeTime=cardLastChangeTime, cardBootVersion=cardBootVersion, cardTemperature=cardTemperature, cardUpTime=cardUpTime, cardPicDetected=cardPicDetected, diskVolumeTrapInh=diskVolumeTrapInh, portPrStateChange=portPrStateChange, portTable=portTable, portCurrDsPower=portCurrDsPower, cardTestTable=cardTestTable, auditTotalPassedCount=auditTotalPassedCount, portDetectedMode=portDetectedMode, auditLogOutput=auditLogOutput, portEntry=portEntry, cardTestId=cardTestId, cmDevice=cmDevice, diskVolumeCardId=diskVolumeCardId, TestType=TestType, cardPrState=cardPrState, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, portType=portType, portCardId=portCardId, cardFpgaSource=cardFpgaSource, equipmentPortGroup=equipmentPortGroup, diskDriveNumber=diskDriveNumber, portMacIfIndex=portMacIfIndex, TestId=TestId, cmResetStatus=cmResetStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardNumPorts=cardNumPorts, portTrapInh=portTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSpareGroupMode=cardSpareGroupMode, cmUChannelID=cmUChannelID, portNumChans=portNumChans, diskVolumeEntry=diskVolumeEntry, portTxFlowControlMode=portTxFlowControlMode, errEvRowStatus=errEvRowStatus, cardSubDetected=cardSubDetected, portSecState=portSecState, equipmentTraps=equipmentTraps, cardPicSerialNum=cardPicSerialNum, errEventId=errEventId)
