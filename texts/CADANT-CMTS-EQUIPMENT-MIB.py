...
PortType, CardSubType, DiskVolumeUsageLevel, CardType, DuplexStatus, FirmwareSource, FlowControlMode, PicType, PortDetectedMode, PrimaryState, SecondaryState, PortMode, UnknownState, CardId, EqActionType, PortId, ShelfId, AdminState = mibBuilder.importSymbols("CADANT-TC", "PortType", "CardSubType", "DiskVolumeUsageLevel", "CardType", "DuplexStatus", "FirmwareSource", "FlowControlMode", "PicType", "PortDetectedMode", "PrimaryState", "SecondaryState", "PortMode", "UnknownState", "CardId", "EqActionType", "PortId", "ShelfId", "AdminState")
...
RowStatus, DateAndTime, TimeStamp, MacAddress, DisplayString, TruthValue, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "TimeStamp", "MacAddress", "DisplayString", "TruthValue", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeName=diskVolumeName, equipmentDiag=equipmentDiag, cardLastBootVersion=cardLastBootVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardRamSize=cardRamSize, portSecState=portSecState, diskVolumeId=diskVolumeId, auditAutoScheduling=auditAutoScheduling, TestId=TestId, cardLastBootSource=cardLastBootSource, equipmentState=equipmentState, cmResetUpTime=cmResetUpTime, cardTestResult=cardTestResult, cmResetInfo=cmResetInfo, cardSecStateChange=cardSecStateChange, cardTestTable=cardTestTable, cardPrState=cardPrState, portTxFlowControlDetected=portTxFlowControlDetected, TestSchedule=TestSchedule, cardType=cardType, portDocsIfIndex=portDocsIfIndex, portTable=portTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditTime=auditTime, cardSubType=cardSubType, cardAction=cardAction, diskVolumeCardId=diskVolumeCardId, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierFwVersion=cardCarrierFwVersion, errEvLogLevel=errEvLogLevel, diskVolumeNumber=diskVolumeNumber, cardTempNoReportNotification=cardTempNoReportNotification, cardAdminState=cardAdminState, cardTestResultNotification=cardTestResultNotification, cardTestTime=cardTestTime, auditStatus=auditStatus, diagTestId=diagTestId, portNumChans=portNumChans, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSecState=cardSecState, portTxFlowControlMode=portTxFlowControlMode, shelfName=shelfName, cardFpgaSource=cardFpgaSource, equipmentPortGroup=equipmentPortGroup, equipmentError=equipmentError, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, verboseLevel=verboseLevel, cardTestDescription=cardTestDescription, auditTable=auditTable, cmUChannelID=cmUChannelID, remainInDiagMode=remainInDiagMode, cadEquipmentMib=cadEquipmentMib, cardPicSerialNum=cardPicSerialNum, portNumber=portNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetNotification=cmResetNotification, cardShelfId=cardShelfId, cardSubDetected=cardSubDetected, portMode=portMode, systemKey=systemKey, diskVolumeTable=diskVolumeTable, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecStateChange=portSecStateChange, equipmentAuditGroup=equipmentAuditGroup, auditCardId=auditCardId, cardTempOverHeatNotification=cardTempOverHeatNotification, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestCommand=TestCommand, consoleOutput=consoleOutput, cardPrStateChange=cardPrStateChange, cardTable=cardTable, cmIpAddress=cmIpAddress, portRxFlowControlDetected=portRxFlowControlDetected, portShelfId=portShelfId, diskVolumeSize=diskVolumeSize, TestResult=TestResult, portCardId=portCardId, cardTestId=cardTestId, cardHwVersion=cardHwVersion, systemGeneralGroup=systemGeneralGroup, portDescription=portDescription, cardLastResetReason=cardLastResetReason, cardTestEntry=cardTestEntry, auditName=auditName, cmRegistrationNotification=cmRegistrationNotification, cardDplxStatusChange=cardDplxStatusChange, portDplxStatusChange=portDplxStatusChange, trapSeverity=trapSeverity, cardSpareGroupId=cardSpareGroupId, portCardSubType=portCardSubType, cardTestTransId=cardTestTransId, cardTestResultDesc=cardTestResultDesc, eqDiagConfig=eqDiagConfig, portAction=portAction, errEvRowStatus=errEvRowStatus, errEventId=errEventId, diskVolumeShelfId=diskVolumeShelfId, equipmentStateGroup=equipmentStateGroup, portBgpId=portBgpId, cmVendor=cmVendor, systemGeneral=systemGeneral, systemClock=systemClock, cardFwVersion=cardFwVersion, cardId=cardId, cmMacAddress=cmMacAddress, diskVolumeGroup=diskVolumeGroup, TestTransactionId=TestTransactionId, auditCommand=auditCommand, auditLogOutput=auditLogOutput, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmResetReason=cmResetReason, diskDriveNumber=diskDriveNumber, shelfNumber=shelfNumber, cardTemperatureHighError=cardTemperatureHighError, cardTestCommand=cardTestCommand, equipmentCardGroup=equipmentCardGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, portTrapInh=portTrapInh, cardSwVersion=cardSwVersion, cardCPLDVersion=cardCPLDVersion, cardCpuType=cardCpuType, auditResult=auditResult, diskVolumeFileName=diskVolumeFileName, cardDetected=cardDetected, cmResetClearNotification=cmResetClearNotification, cardNumPorts=cardNumPorts, cmPrimarySID=cmPrimarySID, portId=portId, cardName=cardName, auditCycleCount=auditCycleCount, cardPicDetected=cardPicDetected, portPrStateChange=portPrStateChange, cardUpTime=cardUpTime, auditTotalFailedCount=auditTotalFailedCount, equipmentTbl=equipmentTbl, cardNumber=cardNumber, cardTestIndex=cardTestIndex, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditId=auditId, portDetectedMode=portDetectedMode, cardFwUpdateStatus=cardFwUpdateStatus, PYSNMP_MODULE_ID=cadEquipmentMib, auditShelfId=auditShelfId, cardDplxStatus=cardDplxStatus, cmDevice=cmDevice, trapCounter=trapCounter, shelfObjects=shelfObjects, diskVolumeDriveId=diskVolumeDriveId, cardTestScheduleCommand=cardTestScheduleCommand, cardTestName=cardTestName, portMacAddress=portMacAddress, cardPicHwVersion=cardPicHwVersion, TestScheduleCommand=TestScheduleCommand, portMaxDsPower=portMaxDsPower, cardCpuSpeed=cardCpuSpeed, portEntry=portEntry, portRxFlowControlMode=portRxFlowControlMode, portMacIfIndex=portMacIfIndex, cardFlashSize=cardFlashSize, equipmentDiagGroup=equipmentDiagGroup, downstreamPowerLoss=downstreamPowerLoss, diskVolumeEntry=diskVolumeEntry, auditDescription=auditDescription, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestType=cardTestType, cardCarrierHwVersion=cardCarrierHwVersion, portAdminState=portAdminState, cardBusSpeed=cardBusSpeed, equipmentAudit=equipmentAudit, cardTrapInh=cardTrapInh, auditTotalPassedCount=auditTotalPassedCount, portLastChangeTime=portLastChangeTime, portType=portType, diskVolumeDiskSize=diskVolumeDiskSize, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentTraps=equipmentTraps, cardSpareGroupMode=cardSpareGroupMode, shelfSwVersion=shelfSwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, portDplxStatus=portDplxStatus, portConnectorId=portConnectorId, auditLogThrottle=auditLogThrottle, errEvLoggingEnabled=errEvLoggingEnabled, TestType=TestType, auditPassedCount=auditPassedCount, portMinDsPower=portMinDsPower, portPrState=portPrState, cardEntry=cardEntry, auditEntry=auditEntry, diskVolumeTrapInh=diskVolumeTrapInh, cardShelfTestIndex=cardShelfTestIndex, diskVolumeUsageLevel=diskVolumeUsageLevel, auditFailedCount=auditFailedCount, cardDetectedChange=cardDetectedChange, cardTestSchedule=cardTestSchedule, portCurrDsPower=portCurrDsPower, cardTemperature=cardTemperature, cardLastChangeTime=cardLastChangeTime, equipmentShelfGroup=equipmentShelfGroup, cardCarrierSerialNum=cardCarrierSerialNum, cardSerialNum=cardSerialNum, cardBootVersion=cardBootVersion, cmResetStatus=cmResetStatus)
