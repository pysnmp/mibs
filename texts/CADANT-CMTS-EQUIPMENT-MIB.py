...
PortId, SecondaryState, UnknownState, DiskVolumeUsageLevel, AdminState, DuplexStatus, PortType, EqActionType, PicType, ShelfId, CardId, PrimaryState, FlowControlMode, PortMode, PortDetectedMode, FirmwareSource, CardType, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PortId", "SecondaryState", "UnknownState", "DiskVolumeUsageLevel", "AdminState", "DuplexStatus", "PortType", "EqActionType", "PicType", "ShelfId", "CardId", "PrimaryState", "FlowControlMode", "PortMode", "PortDetectedMode", "FirmwareSource", "CardType", "CardSubType")
...
MacAddress, TimeStamp, DateAndTime, TextualConvention, TruthValue, RowStatus, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TimeStamp", "DateAndTime", "TextualConvention", "TruthValue", "RowStatus", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmUChannelID=cmUChannelID, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestType=cardTestType, shelfSwVersion=shelfSwVersion, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestTable=cardTestTable, auditTotalPassedCount=auditTotalPassedCount, cardLastBootSource=cardLastBootSource, portCardId=portCardId, verboseLevel=verboseLevel, errEventId=errEventId, cardLastChangeTime=cardLastChangeTime, cardFwUpdateStatus=cardFwUpdateStatus, equipmentShelfGroup=equipmentShelfGroup, shelfName=shelfName, cardTestScheduleCommand=cardTestScheduleCommand, portAction=portAction, equipmentTbl=equipmentTbl, cardTestResult=cardTestResult, cardTestIndex=cardTestIndex, diskVolumeDriveId=diskVolumeDriveId, cardDetectedChange=cardDetectedChange, portTable=portTable, portNumChans=portNumChans, equipmentTraps=equipmentTraps, equipmentPortGroup=equipmentPortGroup, portSecState=portSecState, cardPrState=cardPrState, consoleOutput=consoleOutput, TestCommand=TestCommand, portDetectedMode=portDetectedMode, portDocsIfIndex=portDocsIfIndex, cardSwVersion=cardSwVersion, TestType=TestType, diskVolumeFileName=diskVolumeFileName, cardId=cardId, cmResetNotification=cmResetNotification, portMacAddress=portMacAddress, diskVolumeCardId=diskVolumeCardId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTime=auditTime, cardShelfId=cardShelfId, cardSpareGroupId=cardSpareGroupId, shelfObjects=shelfObjects, cardShelfTestIndex=cardShelfTestIndex, cardTemperatureHighError=cardTemperatureHighError, equipmentDiagGroup=equipmentDiagGroup, errEvRecoveryEnabled=errEvRecoveryEnabled, portDplxStatus=portDplxStatus, cmResetInfo=cmResetInfo, cardSpareGroupMode=cardSpareGroupMode, systemGeneralGroup=systemGeneralGroup, portEntry=portEntry, cardCpuType=cardCpuType, cmResetClearNotification=cmResetClearNotification, cardSecStateChange=cardSecStateChange, auditStatus=auditStatus, cmResetStatus=cmResetStatus, cardPicDetected=cardPicDetected, portMode=portMode, portCurrDsPower=portCurrDsPower, cadEquipmentMib=cadEquipmentMib, cardNumber=cardNumber, cardPrStateChange=cardPrStateChange, cardAction=cardAction, PYSNMP_MODULE_ID=cadEquipmentMib, portAdminState=portAdminState, cardCarrierHwVersion=cardCarrierHwVersion, auditCardId=auditCardId, cmPrimarySID=cmPrimarySID, cardName=cardName, cardDetected=cardDetected, auditPassedCount=auditPassedCount, portNumber=portNumber, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestSchedule=cardTestSchedule, auditTotalFailedCount=auditTotalFailedCount, cmRegistrationNotification=cmRegistrationNotification, cardTable=cardTable, cardTestEntry=cardTestEntry, portShelfId=portShelfId, cardSerialNum=cardSerialNum, TestScheduleCommand=TestScheduleCommand, cardAdminState=cardAdminState, cardTestTransId=cardTestTransId, auditCycleCount=auditCycleCount, auditDescription=auditDescription, diskDriveNumber=diskDriveNumber, diskVolumeShelfId=diskVolumeShelfId, cmVendor=cmVendor, portMacIfIndex=portMacIfIndex, cardFpgaSource=cardFpgaSource, equipmentCardGroup=equipmentCardGroup, auditFailedCount=auditFailedCount, portType=portType, portDplxStatusChange=portDplxStatusChange, cardDplxStatus=cardDplxStatus, cardTestResultDesc=cardTestResultDesc, cardFwVersion=cardFwVersion, auditCommand=auditCommand, cardCpuSpeed=cardCpuSpeed, cardTestName=cardTestName, portConnectorId=portConnectorId, cardBootVersion=cardBootVersion, cardCarrierSerialNum=cardCarrierSerialNum, portId=portId, cardUpTime=cardUpTime, equipmentAuditGroup=equipmentAuditGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, errEvLogLevel=errEvLogLevel, trapCounter=trapCounter, cardPicHwVersion=cardPicHwVersion, portMinDsPower=portMinDsPower, fpgaErrorEventEntry=fpgaErrorEventEntry, auditTable=auditTable, shelfNumber=shelfNumber, TestTransactionId=TestTransactionId, auditEntry=auditEntry, cardNumPorts=cardNumPorts, cardTestCommand=cardTestCommand, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, systemClock=systemClock, diskVolumeTable=diskVolumeTable, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeNumber=diskVolumeNumber, portCardSubType=portCardSubType, cardHwVersion=cardHwVersion, auditLogThrottle=auditLogThrottle, remainInDiagMode=remainInDiagMode, cardSubType=cardSubType, errEvRowStatus=errEvRowStatus, cardTestResultNotification=cardTestResultNotification, cardSecState=cardSecState, auditId=auditId, TestSchedule=TestSchedule, downstreamPowerLoss=downstreamPowerLoss, equipmentDiag=equipmentDiag, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardTestId=cardTestId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, cardEntry=cardEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestDescription=cardTestDescription, portMaxDsPower=portMaxDsPower, diagTestId=diagTestId, cmResetReason=cmResetReason, eqDiagConfig=eqDiagConfig, cmMacAddress=cmMacAddress, portPrState=portPrState, auditAutoScheduling=auditAutoScheduling, cardRamSize=cardRamSize, cardTrapInh=cardTrapInh, cardTemperature=cardTemperature, TestResult=TestResult, cardType=cardType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTime=cardTestTime, portSecStateChange=portSecStateChange, portTrapInh=portTrapInh, cmIpAddress=cmIpAddress, diskVolumeGroup=diskVolumeGroup, diskVolumeId=diskVolumeId, cardLastResetReason=cardLastResetReason, trapSeverity=trapSeverity, portLastChangeTime=portLastChangeTime, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeSize=diskVolumeSize, systemGeneral=systemGeneral, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetUpTime=cmResetUpTime, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemKey=systemKey, cardDplxStatusChange=cardDplxStatusChange, cardLastBootVersion=cardLastBootVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portRxFlowControlDetected=portRxFlowControlDetected, auditLogOutput=auditLogOutput, portBgpId=portBgpId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCPLDVersion=cardCPLDVersion, cmDevice=cmDevice, diskVolumeEntry=diskVolumeEntry, portRxFlowControlMode=portRxFlowControlMode, cardBusSpeed=cardBusSpeed, portDescription=portDescription, equipmentAudit=equipmentAudit, equipmentStateGroup=equipmentStateGroup, auditShelfId=auditShelfId, cardFlashSize=cardFlashSize, cardPicSerialNum=cardPicSerialNum, cardTempNoReportNotification=cardTempNoReportNotification, equipmentState=equipmentState, cardSubDetected=cardSubDetected, auditName=auditName, auditResult=auditResult, cardCarrierFwVersion=cardCarrierFwVersion, TestId=TestId, diskVolumeName=diskVolumeName, diskVolumeDiskSize=diskVolumeDiskSize, fpgaErrorEventTable=fpgaErrorEventTable, portPrStateChange=portPrStateChange, equipmentError=equipmentError)
