...
CardId, CardType, AdminState, FirmwareSource, PortMode, PortType, SecondaryState, DuplexStatus, PortId, PortDetectedMode, EqActionType, FlowControlMode, PicType, CardSubType, PrimaryState, DiskVolumeUsageLevel, ShelfId, UnknownState = mibBuilder.importSymbols("CADANT-TC", "CardId", "CardType", "AdminState", "FirmwareSource", "PortMode", "PortType", "SecondaryState", "DuplexStatus", "PortId", "PortDetectedMode", "EqActionType", "FlowControlMode", "PicType", "CardSubType", "PrimaryState", "DiskVolumeUsageLevel", "ShelfId", "UnknownState")
...
TruthValue, DateAndTime, MacAddress, DisplayString, TextualConvention, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "MacAddress", "DisplayString", "TextualConvention", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageLevel=diskVolumeUsageLevel, cardTemperature=cardTemperature, systemGeneralGroup=systemGeneralGroup, portAction=portAction, PYSNMP_MODULE_ID=cadEquipmentMib, cardPicHwVersion=cardPicHwVersion, portMinDsPower=portMinDsPower, eqDiagConfig=eqDiagConfig, cmVendor=cmVendor, cmResetNotification=cmResetNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditAutoScheduling=auditAutoScheduling, equipmentTraps=equipmentTraps, cardShelfId=cardShelfId, portCardSubType=portCardSubType, diskVolumeDriveId=diskVolumeDriveId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestType=cardTestType, cardDplxStatus=cardDplxStatus, cardLastBootSource=cardLastBootSource, cardTestResult=cardTestResult, systemKey=systemKey, portMacIfIndex=portMacIfIndex, auditTime=auditTime, cardName=cardName, equipmentTbl=equipmentTbl, portTxFlowControlMode=portTxFlowControlMode, cardDplxStatusChange=cardDplxStatusChange, equipmentState=equipmentState, auditResult=auditResult, auditEntry=auditEntry, portLastChangeTime=portLastChangeTime, cardSecState=cardSecState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeFileName=diskVolumeFileName, equipmentCardTestGroup=equipmentCardTestGroup, cardHwVersion=cardHwVersion, cardTestId=cardTestId, diskDriveNumber=diskDriveNumber, shelfObjects=shelfObjects, cardPrStateChange=cardPrStateChange, portDplxStatusChange=portDplxStatusChange, downstreamPowerLoss=downstreamPowerLoss, cardSpareGroupMode=cardSpareGroupMode, cardLastResetReason=cardLastResetReason, cardShelfTestIndex=cardShelfTestIndex, TestScheduleCommand=TestScheduleCommand, cardTemperatureHighError=cardTemperatureHighError, auditCardId=auditCardId, cardNumPorts=cardNumPorts, errEvRowStatus=errEvRowStatus, auditName=auditName, equipmentPortGroup=equipmentPortGroup, shelfNumber=shelfNumber, portDetectedMode=portDetectedMode, portMacAddress=portMacAddress, diskVolumeId=diskVolumeId, TestTransactionId=TestTransactionId, cardSwVersion=cardSwVersion, auditCommand=auditCommand, cardLastBootVersion=cardLastBootVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardAction=cardAction, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeTable=diskVolumeTable, cardTestResultNotification=cardTestResultNotification, diskVolumeSize=diskVolumeSize, equipmentShelfGroup=equipmentShelfGroup, portSecState=portSecState, cmResetReason=cmResetReason, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierHwVersion=cardCarrierHwVersion, cardTestCommand=cardTestCommand, portSecStateChange=portSecStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, auditLogThrottle=auditLogThrottle, cardTable=cardTable, fpgaErrorEventEntry=fpgaErrorEventEntry, cardNumber=cardNumber, cardFlashSize=cardFlashSize, auditStatus=auditStatus, cardCpuType=cardCpuType, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCpuSpeed=cardCpuSpeed, systemGeneral=systemGeneral, TestCommand=TestCommand, cardPicSerialNum=cardPicSerialNum, cardSubDetected=cardSubDetected, portMode=portMode, cardTestTransId=cardTestTransId, cardSecStateChange=cardSecStateChange, cardId=cardId, auditId=auditId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResultDesc=cardTestResultDesc, portNumber=portNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, trapCounter=trapCounter, cardFpgaSource=cardFpgaSource, auditTotalFailedCount=auditTotalFailedCount, trapSeverity=trapSeverity, equipmentStateGroup=equipmentStateGroup, portShelfId=portShelfId, errEvLoggingEnabled=errEvLoggingEnabled, equipmentDiag=equipmentDiag, auditTotalPassedCount=auditTotalPassedCount, portTrapInh=portTrapInh, cadEquipmentMibConformance=cadEquipmentMibConformance, cardBusSpeed=cardBusSpeed, consoleOutput=consoleOutput, cardAdminState=cardAdminState, TestId=TestId, portCurrDsPower=portCurrDsPower, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDescription=portDescription, diskVolumeNumber=diskVolumeNumber, verboseLevel=verboseLevel, cardRamSize=cardRamSize, cardSubType=cardSubType, auditPassedCount=auditPassedCount, cardLastChangeTime=cardLastChangeTime, cardUpTime=cardUpTime, equipmentDiagGroup=equipmentDiagGroup, cardSpareGroupId=cardSpareGroupId, TestType=TestType, portId=portId, portPrStateChange=portPrStateChange, TestResult=TestResult, cardPrState=cardPrState, cardBootVersion=cardBootVersion, cardCPLDVersion=cardCPLDVersion, diskVolumeTrapInh=diskVolumeTrapInh, auditLogOutput=auditLogOutput, cmResetStatus=cmResetStatus, cardTestName=cardTestName, equipmentAudit=equipmentAudit, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeGroup=diskVolumeGroup, cardTestTime=cardTestTime, portPrState=portPrState, cmIpAddress=cmIpAddress, cmDevice=cmDevice, TestSchedule=TestSchedule, portCardId=portCardId, cadEquipmentMibGroup=cadEquipmentMibGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardTestScheduleCommand=cardTestScheduleCommand, equipmentCardGroup=equipmentCardGroup, diskVolumeShelfId=diskVolumeShelfId, systemClock=systemClock, cardEntry=cardEntry, portTable=portTable, cardTestIndex=cardTestIndex, diagTestId=diagTestId, portDplxStatus=portDplxStatus, portNumChans=portNumChans, cardTestEntry=cardTestEntry, cmPrimarySID=cmPrimarySID, auditTable=auditTable, diskVolumeCardId=diskVolumeCardId, auditFailedCount=auditFailedCount, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvRecoveryEnabled=errEvRecoveryEnabled, portConnectorId=portConnectorId, cmResetClearNotification=cmResetClearNotification, auditCycleCount=auditCycleCount, cmRegistrationNotification=cmRegistrationNotification, portDocsIfIndex=portDocsIfIndex, equipmentAuditGroup=equipmentAuditGroup, portMaxDsPower=portMaxDsPower, cmResetUpTime=cmResetUpTime, portEntry=portEntry, cardSerialNum=cardSerialNum, cmMacAddress=cmMacAddress, cmResetInfo=cmResetInfo, auditDescription=auditDescription, diskVolumeDiskSize=diskVolumeDiskSize, cardTestSchedule=cardTestSchedule, equipmentError=equipmentError, shelfSwVersion=shelfSwVersion, remainInDiagMode=remainInDiagMode, auditShelfId=auditShelfId, portType=portType, shelfName=shelfName, cardFwVersion=cardFwVersion, cardFwUpdateStatus=cardFwUpdateStatus, cardDetected=cardDetected, portBgpId=portBgpId, cardPicDetected=cardPicDetected, errEventId=errEventId, fpgaErrorEventTable=fpgaErrorEventTable, cardTrapInh=cardTrapInh, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeEntry=diskVolumeEntry, cadEquipmentMib=cadEquipmentMib, cardCarrierSerialNum=cardCarrierSerialNum, cardType=cardType, diskVolumeName=diskVolumeName, cardTestDescription=cardTestDescription, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTable=cardTestTable, cardDetectedChange=cardDetectedChange, errEvLogLevel=errEvLogLevel, cmUChannelID=cmUChannelID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portAdminState=portAdminState)
