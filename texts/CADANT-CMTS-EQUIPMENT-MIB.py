...
CardType, PortDetectedMode, CardSubType, PicType, PortId, AdminState, CardId, FirmwareSource, PortMode, EqActionType, PortType, ShelfId, DiskVolumeUsageLevel, PrimaryState, SecondaryState, UnknownState, FlowControlMode, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortDetectedMode", "CardSubType", "PicType", "PortId", "AdminState", "CardId", "FirmwareSource", "PortMode", "EqActionType", "PortType", "ShelfId", "DiskVolumeUsageLevel", "PrimaryState", "SecondaryState", "UnknownState", "FlowControlMode", "DuplexStatus")
...
TruthValue, TextualConvention, DisplayString, RowStatus, TimeStamp, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TextualConvention", "DisplayString", "RowStatus", "TimeStamp", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPrStateChange=cardPrStateChange, diagTestId=diagTestId, cardTestEntry=cardTestEntry, auditLogOutput=auditLogOutput, cardPicDetected=cardPicDetected, auditFailedCount=auditFailedCount, trapCounter=trapCounter, TestId=TestId, cardAction=cardAction, diskVolumeDriveId=diskVolumeDriveId, shelfName=shelfName, cardRamSize=cardRamSize, cmIpAddress=cmIpAddress, errEvLoggingEnabled=errEvLoggingEnabled, portBgpId=portBgpId, cardTestName=cardTestName, diskVolumeSize=diskVolumeSize, cardTestResultDesc=cardTestResultDesc, TestResult=TestResult, cardFlashSize=cardFlashSize, portMinDsPower=portMinDsPower, cardAdminState=cardAdminState, cardEntry=cardEntry, auditTotalPassedCount=auditTotalPassedCount, portMaxDsPower=portMaxDsPower, portDplxStatusChange=portDplxStatusChange, diskVolumeName=diskVolumeName, portCurrDsPower=portCurrDsPower, systemGeneral=systemGeneral, portMacAddress=portMacAddress, portCardSubType=portCardSubType, cardPicSerialNum=cardPicSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portMacIfIndex=portMacIfIndex, systemClock=systemClock, portAdminState=portAdminState, cmMacAddress=cmMacAddress, cardDetectedChange=cardDetectedChange, cardLastBootVersion=cardLastBootVersion, auditPassedCount=auditPassedCount, trapSeverity=trapSeverity, auditTable=auditTable, diskVolumeNumber=diskVolumeNumber, downstreamPowerLoss=downstreamPowerLoss, cardCpuSpeed=cardCpuSpeed, cardFwVersion=cardFwVersion, portTable=portTable, equipmentDiag=equipmentDiag, cardTestSchedule=cardTestSchedule, portTxFlowControlMode=portTxFlowControlMode, consoleOutput=consoleOutput, TestType=TestType, portRxFlowControlDetected=portRxFlowControlDetected, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageLevel=diskVolumeUsageLevel, auditTotalFailedCount=auditTotalFailedCount, cardSubDetected=cardSubDetected, diskVolumeTrapInh=diskVolumeTrapInh, errEvLogLevel=errEvLogLevel, auditShelfId=auditShelfId, auditId=auditId, cardTempNoReportNotification=cardTempNoReportNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, cardUpTime=cardUpTime, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetUpTime=cmResetUpTime, diskVolumeTable=diskVolumeTable, PYSNMP_MODULE_ID=cadEquipmentMib, portPrState=portPrState, equipmentAuditGroup=equipmentAuditGroup, cardBootVersion=cardBootVersion, cardTemperatureHighError=cardTemperatureHighError, cardSecState=cardSecState, auditDescription=auditDescription, cardSerialNum=cardSerialNum, TestCommand=TestCommand, errEventId=errEventId, cardTestDescription=cardTestDescription, equipmentStateGroup=equipmentStateGroup, cardTestScheduleCommand=cardTestScheduleCommand, portSecState=portSecState, auditStatus=auditStatus, auditResult=auditResult, portMode=portMode, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditCardId=auditCardId, cardFpgaSource=cardFpgaSource, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSecStateChange=cardSecStateChange, cmResetInfo=cmResetInfo, equipmentAudit=equipmentAudit, shelfNumber=shelfNumber, cardHwVersion=cardHwVersion, cardLastChangeTime=cardLastChangeTime, cardLastBootSource=cardLastBootSource, portDescription=portDescription, diskVolumeId=diskVolumeId, systemGeneralGroup=systemGeneralGroup, cardSubType=cardSubType, cardSpareGroupMode=cardSpareGroupMode, diskVolumeCardId=diskVolumeCardId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeFileName=diskVolumeFileName, cardType=cardType, portTxFlowControlDetected=portTxFlowControlDetected, shelfSwVersion=shelfSwVersion, cmResetNotification=cmResetNotification, auditName=auditName, TestTransactionId=TestTransactionId, cmUChannelID=cmUChannelID, portEntry=portEntry, cardId=cardId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestCommand=cardTestCommand, errEvRowStatus=errEvRowStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, portNumber=portNumber, diskVolumeEntry=diskVolumeEntry, equipmentTbl=equipmentTbl, cardFwUpdateStatus=cardFwUpdateStatus, cmResetStatus=cmResetStatus, cardNumber=cardNumber, cardCPLDVersion=cardCPLDVersion, cmRegistrationNotification=cmRegistrationNotification, verboseLevel=verboseLevel, fpgaErrorEventEntry=fpgaErrorEventEntry, remainInDiagMode=remainInDiagMode, cardShelfId=cardShelfId, diskDriveNumber=diskDriveNumber, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, systemKey=systemKey, cardTestResult=cardTestResult, cardTestTime=cardTestTime, cardNumPorts=cardNumPorts, portDocsIfIndex=portDocsIfIndex, cardLastResetReason=cardLastResetReason, TestSchedule=TestSchedule, cardTestIndex=cardTestIndex, cardCarrierSerialNum=cardCarrierSerialNum, equipmentError=equipmentError, diskVolumeUsageNotification=diskVolumeUsageNotification, auditLogThrottle=auditLogThrottle, portSecStateChange=portSecStateChange, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardGroup=equipmentCardGroup, diskVolumeGroup=diskVolumeGroup, cardTable=cardTable, equipmentCardTestGroup=equipmentCardTestGroup, portRxFlowControlMode=portRxFlowControlMode, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDplxStatus=cardDplxStatus, equipmentPortGroup=equipmentPortGroup, cardCarrierHwVersion=cardCarrierHwVersion, cardTrapInh=cardTrapInh, cmVendor=cmVendor, fpgaErrorEventTable=fpgaErrorEventTable, cardTestTable=cardTestTable, cardBusSpeed=cardBusSpeed, portId=portId, cmResetClearNotification=cmResetClearNotification, portAction=portAction, auditTime=auditTime, cardName=cardName, diskVolumeDiskSize=diskVolumeDiskSize, cardPrState=cardPrState, equipmentTraps=equipmentTraps, cardDetected=cardDetected, equipmentShelfGroup=equipmentShelfGroup, cardSwVersion=cardSwVersion, cmResetReason=cmResetReason, portConnectorId=portConnectorId, auditAutoScheduling=auditAutoScheduling, cardTestResultNotification=cardTestResultNotification, portType=portType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardShelfTestIndex=cardShelfTestIndex, portNumChans=portNumChans, cardTestTransId=cardTestTransId, auditEntry=auditEntry, portLastChangeTime=portLastChangeTime, cadEquipmentMib=cadEquipmentMib, cardTemperature=cardTemperature, equipmentState=equipmentState, cmPrimarySID=cmPrimarySID, cadEquipmentMibGroup=cadEquipmentMibGroup, auditCommand=auditCommand, cardDplxStatusChange=cardDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, portDetectedMode=portDetectedMode, cardCpuType=cardCpuType, shelfObjects=shelfObjects, auditCycleCount=auditCycleCount, portCardId=portCardId, TestScheduleCommand=TestScheduleCommand, eqDiagConfig=eqDiagConfig, cmDevice=cmDevice, portDplxStatus=portDplxStatus, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestType=cardTestType, cardPicHwVersion=cardPicHwVersion, portTrapInh=portTrapInh, cardTestId=cardTestId, portPrStateChange=portPrStateChange, diskVolumeShelfId=diskVolumeShelfId, portShelfId=portShelfId)
