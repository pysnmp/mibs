...
EqActionType, DiskVolumeUsageLevel, PicType, PrimaryState, ShelfId, CardSubType, DuplexStatus, PortId, PortDetectedMode, AdminState, SecondaryState, CardType, UnknownState, PortType, FirmwareSource, CardId, FlowControlMode, PortMode = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "DiskVolumeUsageLevel", "PicType", "PrimaryState", "ShelfId", "CardSubType", "DuplexStatus", "PortId", "PortDetectedMode", "AdminState", "SecondaryState", "CardType", "UnknownState", "PortType", "FirmwareSource", "CardId", "FlowControlMode", "PortMode")
...
DateAndTime, TimeStamp, TruthValue, TextualConvention, MacAddress, DisplayString, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TimeStamp", "TruthValue", "TextualConvention", "MacAddress", "DisplayString", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portMinDsPower=portMinDsPower, auditAutoScheduling=auditAutoScheduling, cardSecState=cardSecState, portBgpId=portBgpId, cardTestId=cardTestId, portTable=portTable, cardBusSpeed=cardBusSpeed, fpgaErrorEventGroup=fpgaErrorEventGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardBootVersion=cardBootVersion, portMode=portMode, cmResetStatus=cmResetStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestCommand=cardTestCommand, cardCPLDVersion=cardCPLDVersion, cardSecStateChange=cardSecStateChange, auditLogOutput=auditLogOutput, portLastChangeTime=portLastChangeTime, eqDiagConfig=eqDiagConfig, TestTransactionId=TestTransactionId, auditEntry=auditEntry, systemClock=systemClock, cardName=cardName, portSecState=portSecState, cardTestResultDesc=cardTestResultDesc, cardFlashSize=cardFlashSize, diskVolumeShelfId=diskVolumeShelfId, diskVolumeGroup=diskVolumeGroup, cardSerialNum=cardSerialNum, TestType=TestType, trapSeverity=trapSeverity, portTrapInh=portTrapInh, cardCarrierHwVersion=cardCarrierHwVersion, cardLastResetReason=cardLastResetReason, portDescription=portDescription, cardTestResultNotification=cardTestResultNotification, cardPrState=cardPrState, systemKey=systemKey, cardTestTable=cardTestTable, TestId=TestId, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeTrapInh=diskVolumeTrapInh, auditLogThrottle=auditLogThrottle, cmIpAddress=cmIpAddress, cardEntry=cardEntry, cardTestResult=cardTestResult, portPrStateChange=portPrStateChange, auditTotalPassedCount=auditTotalPassedCount, systemGeneralGroup=systemGeneralGroup, equipmentShelfGroup=equipmentShelfGroup, equipmentAudit=equipmentAudit, cardTemperature=cardTemperature, portCardSubType=portCardSubType, portMaxDsPower=portMaxDsPower, cardShelfTestIndex=cardShelfTestIndex, cmVendor=cmVendor, cardPicDetected=cardPicDetected, cardSpareGroupId=cardSpareGroupId, cmPrimarySID=cmPrimarySID, portId=portId, cardNumber=cardNumber, errEventId=errEventId, cardFwVersion=cardFwVersion, errEvLogLevel=errEvLogLevel, TestScheduleCommand=TestScheduleCommand, portDocsIfIndex=portDocsIfIndex, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMib=cadEquipmentMib, auditName=auditName, equipmentDiag=equipmentDiag, cardSubType=cardSubType, cardTestTime=cardTestTime, PYSNMP_MODULE_ID=cadEquipmentMib, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDplxStatus=cardDplxStatus, equipmentCardGroup=equipmentCardGroup, cardPicSerialNum=cardPicSerialNum, auditTable=auditTable, cardDplxStatusChange=cardDplxStatusChange, equipmentError=equipmentError, diskVolumeCardId=diskVolumeCardId, downstreamPowerLoss=downstreamPowerLoss, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDetectedMode=portDetectedMode, auditPassedCount=auditPassedCount, diskVolumeDiskSize=diskVolumeDiskSize, portEntry=portEntry, portMacAddress=portMacAddress, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeSize=diskVolumeSize, auditCardId=auditCardId, shelfName=shelfName, fpgaErrorEventTable=fpgaErrorEventTable, cardTestDescription=cardTestDescription, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditTotalFailedCount=auditTotalFailedCount, equipmentCardTestGroup=equipmentCardTestGroup, auditStatus=auditStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfObjects=shelfObjects, equipmentState=equipmentState, cardHwVersion=cardHwVersion, cmUChannelID=cmUChannelID, portNumChans=portNumChans, cardNumPorts=cardNumPorts, cardLastBootVersion=cardLastBootVersion, cardUpTime=cardUpTime, cardId=cardId, portAction=portAction, diskVolumeEntry=diskVolumeEntry, cardCpuType=cardCpuType, verboseLevel=verboseLevel, shelfSwVersion=shelfSwVersion, auditShelfId=auditShelfId, cmResetReason=cmResetReason, cardCpuSpeed=cardCpuSpeed, diagTestId=diagTestId, cardFpgaSource=cardFpgaSource, portCurrDsPower=portCurrDsPower, portCardId=portCardId, cardTestTransId=cardTestTransId, cmRegistrationNotification=cmRegistrationNotification, trapCounter=trapCounter, cmResetUpTime=cmResetUpTime, errEvRowStatus=errEvRowStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTxFlowControlDetected=portTxFlowControlDetected, equipmentTraps=equipmentTraps, diskVolumeId=diskVolumeId, cardDetectedChange=cardDetectedChange, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentStateGroup=equipmentStateGroup, portRxFlowControlDetected=portRxFlowControlDetected, auditFailedCount=auditFailedCount, cardDetected=cardDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, TestCommand=TestCommand, portDplxStatusChange=portDplxStatusChange, auditId=auditId, portPrState=portPrState, auditCycleCount=auditCycleCount, diskVolumeUsageLevel=diskVolumeUsageLevel, cardShelfId=cardShelfId, equipmentPortGroup=equipmentPortGroup, cardPicHwVersion=cardPicHwVersion, cmResetNotification=cmResetNotification, errEvLoggingEnabled=errEvLoggingEnabled, auditResult=auditResult, cardTestScheduleCommand=cardTestScheduleCommand, cmResetInfo=cmResetInfo, diskVolumeTable=diskVolumeTable, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMibConformance=cadEquipmentMibConformance, portMacIfIndex=portMacIfIndex, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditDescription=auditDescription, portNumber=portNumber, portType=portType, cardSpareGroupMode=cardSpareGroupMode, portDplxStatus=portDplxStatus, cmResetClearNotification=cmResetClearNotification, diskVolumeName=diskVolumeName, cardTestEntry=cardTestEntry, portTxFlowControlMode=portTxFlowControlMode, cmMacAddress=cmMacAddress, TestSchedule=TestSchedule, systemGeneral=systemGeneral, cardTrapInh=cardTrapInh, cardRamSize=cardRamSize, TestResult=TestResult, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardCarrierSerialNum=cardCarrierSerialNum, cardLastBootSource=cardLastBootSource, cardTable=cardTable, consoleOutput=consoleOutput, cmDevice=cmDevice, cardAction=cardAction, diskDriveNumber=diskDriveNumber, portAdminState=portAdminState, cardTestIndex=cardTestIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentAuditGroup=equipmentAuditGroup, cardTestName=cardTestName, portConnectorId=portConnectorId, cardPrStateChange=cardPrStateChange, cardAdminState=cardAdminState, portShelfId=portShelfId, remainInDiagMode=remainInDiagMode, cardSubDetected=cardSubDetected, cardSwVersion=cardSwVersion, cardTestType=cardTestType, cardTempNoReportNotification=cardTempNoReportNotification, equipmentTbl=equipmentTbl, shelfNumber=shelfNumber, cardType=cardType, diskVolumeFileName=diskVolumeFileName, auditCommand=auditCommand, portSecStateChange=portSecStateChange, diskVolumeNumber=diskVolumeNumber, cardTestSchedule=cardTestSchedule, auditTime=auditTime, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeDriveId=diskVolumeDriveId)
