...
DuplexStatus, CardType, PortType, PortId, CardSubType, PrimaryState, DiskVolumeUsageLevel, AdminState, CardId, UnknownState, FirmwareSource, FlowControlMode, PicType, PortMode, SecondaryState, PortDetectedMode, EqActionType, ShelfId = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "CardType", "PortType", "PortId", "CardSubType", "PrimaryState", "DiskVolumeUsageLevel", "AdminState", "CardId", "UnknownState", "FirmwareSource", "FlowControlMode", "PicType", "PortMode", "SecondaryState", "PortDetectedMode", "EqActionType", "ShelfId")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cmResetReason=cmResetReason, diskVolumeUsagePercentage=diskVolumeUsagePercentage, errEvRowStatus=errEvRowStatus, auditTotalPassedCount=auditTotalPassedCount, shelfSwVersion=shelfSwVersion, cardBusSpeed=cardBusSpeed, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeTable=diskVolumeTable, diskVolumeUsageLevel=diskVolumeUsageLevel, portPrState=portPrState, cardTemperature=cardTemperature, cardFlashSize=cardFlashSize, cmVendor=cmVendor, cardLastChangeTime=cardLastChangeTime, portDplxStatus=portDplxStatus, cardTrapInh=cardTrapInh, portShelfId=portShelfId, cardUpTime=cardUpTime, cmResetInfo=cmResetInfo, cardType=cardType, portPrStateChange=portPrStateChange, errEventId=errEventId, portNumChans=portNumChans, cmIpAddress=cmIpAddress, trapSeverity=trapSeverity, cmDevice=cmDevice, equipmentTraps=equipmentTraps, cardDplxStatus=cardDplxStatus, shelfNumber=shelfNumber, eqDiagConfig=eqDiagConfig, cardTempOverHeatNotification=cardTempOverHeatNotification, verboseLevel=verboseLevel, errEvLoggingEnabled=errEvLoggingEnabled, cardShelfId=cardShelfId, TestId=TestId, auditCardId=auditCardId, TestCommand=TestCommand, portType=portType, cardPrState=cardPrState, portTxFlowControlDetected=portTxFlowControlDetected, cardLastResetReason=cardLastResetReason, portId=portId, downstreamPowerLoss=downstreamPowerLoss, cmRegistrationNotification=cmRegistrationNotification, equipmentShelfGroup=equipmentShelfGroup, auditCommand=auditCommand, shelfName=shelfName, cardLastBootVersion=cardLastBootVersion, diskVolumeDriveId=diskVolumeDriveId, diskVolumeName=diskVolumeName, cmResetUpTime=cmResetUpTime, portRxFlowControlMode=portRxFlowControlMode, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeEntry=diskVolumeEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskDriveNumber=diskDriveNumber, cardDetected=cardDetected, cardBootVersion=cardBootVersion, portDescription=portDescription, portLastChangeTime=portLastChangeTime, systemKey=systemKey, cardSerialNum=cardSerialNum, cardTestResultDesc=cardTestResultDesc, systemClock=systemClock, cardTestEntry=cardTestEntry, systemGeneral=systemGeneral, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditLogOutput=auditLogOutput, cmResetStatus=cmResetStatus, cardCarrierHwVersion=cardCarrierHwVersion, cardLastBootSource=cardLastBootSource, cardPicSerialNum=cardPicSerialNum, cmPrimarySID=cmPrimarySID, portMacAddress=portMacAddress, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diagTestId=diagTestId, TestSchedule=TestSchedule, cmResetClearNotification=cmResetClearNotification, cardRamSize=cardRamSize, cardFpgaSource=cardFpgaSource, portDocsIfIndex=portDocsIfIndex, portDplxStatusChange=portDplxStatusChange, cardSwVersion=cardSwVersion, errEvLogLevel=errEvLogLevel, cardTestName=cardTestName, equipmentState=equipmentState, portDetectedMode=portDetectedMode, diskVolumeCardId=diskVolumeCardId, cardTestResultNotification=cardTestResultNotification, portConnectorId=portConnectorId, cardFwVersion=cardFwVersion, portTxFlowControlMode=portTxFlowControlMode, portCurrDsPower=portCurrDsPower, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestCommand=cardTestCommand, cardCarrierSerialNum=cardCarrierSerialNum, equipmentPortGroup=equipmentPortGroup, TestTransactionId=TestTransactionId, equipmentDiagGroup=equipmentDiagGroup, cardTestSchedule=cardTestSchedule, equipmentStateGroup=equipmentStateGroup, cardCPLDVersion=cardCPLDVersion, portBgpId=portBgpId, diskVolumeId=diskVolumeId, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portMaxDsPower=portMaxDsPower, diskVolumeGroup=diskVolumeGroup, shelfObjects=shelfObjects, cardTestScheduleCommand=cardTestScheduleCommand, equipmentAuditGroup=equipmentAuditGroup, cardDplxStatusChange=cardDplxStatusChange, cmMacAddress=cmMacAddress, portAdminState=portAdminState, cardHwVersion=cardHwVersion, cardCpuSpeed=cardCpuSpeed, diskVolumeTrapInh=diskVolumeTrapInh, auditTable=auditTable, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierFwVersion=cardCarrierFwVersion, cardAdminState=cardAdminState, cardSecStateChange=cardSecStateChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, consoleOutput=consoleOutput, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeSize=diskVolumeSize, auditResult=auditResult, cardSubType=cardSubType, cardTestDescription=cardTestDescription, cardSubDetected=cardSubDetected, auditStatus=auditStatus, auditPassedCount=auditPassedCount, cardTempNoReportNotification=cardTempNoReportNotification, cardPicDetected=cardPicDetected, portSecStateChange=portSecStateChange, cardPrStateChange=cardPrStateChange, cardDetectedChange=cardDetectedChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portEntry=portEntry, cardSecState=cardSecState, trapCounter=trapCounter, equipmentTbl=equipmentTbl, equipmentError=equipmentError, cardTestId=cardTestId, portMinDsPower=portMinDsPower, cadEquipmentMibConformance=cadEquipmentMibConformance, cardShelfTestIndex=cardShelfTestIndex, portTable=portTable, cmResetNotification=cmResetNotification, cardName=cardName, auditCycleCount=auditCycleCount, equipmentAudit=equipmentAudit, cardAction=cardAction, diskVolumeNumber=diskVolumeNumber, cardNumber=cardNumber, diskVolumeShelfId=diskVolumeShelfId, cardPicHwVersion=cardPicHwVersion, portSecState=portSecState, auditEntry=auditEntry, portAction=portAction, cardSpareGroupMode=cardSpareGroupMode, auditTotalFailedCount=auditTotalFailedCount, cmUChannelID=cmUChannelID, auditShelfId=auditShelfId, TestScheduleCommand=TestScheduleCommand, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTable=cardTestTable, cardCpuType=cardCpuType, auditName=auditName, fpgaErrorEventEntry=fpgaErrorEventEntry, TestResult=TestResult, cadEquipmentMib=cadEquipmentMib, portTrapInh=portTrapInh, auditAutoScheduling=auditAutoScheduling, cardTable=cardTable, cardTemperatureHighError=cardTemperatureHighError, cardId=cardId, auditLogThrottle=auditLogThrottle, diskVolumeUsageNotification=diskVolumeUsageNotification, portNumber=portNumber, auditTime=auditTime, portCardId=portCardId, portMode=portMode, equipmentDiag=equipmentDiag, auditDescription=auditDescription, cardTestType=cardTestType, TestType=TestType, cardTestIndex=cardTestIndex, auditId=auditId, portMacIfIndex=portMacIfIndex, portCardSubType=portCardSubType, cardTestResult=cardTestResult, cardNumPorts=cardNumPorts, cardSpareGroupId=cardSpareGroupId, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeFileName=diskVolumeFileName, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwUpdateStatus=cardFwUpdateStatus, systemGeneralGroup=systemGeneralGroup, cardEntry=cardEntry, auditFailedCount=auditFailedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, remainInDiagMode=remainInDiagMode, cardTestTransId=cardTestTransId, cardTestTime=cardTestTime)
