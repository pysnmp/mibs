...
CardId, DiskVolumeUsageLevel, CardType, PortId, PicType, EqActionType, UnknownState, SecondaryState, PortMode, PrimaryState, ShelfId, PortDetectedMode, PortType, DuplexStatus, CardSubType, FirmwareSource, AdminState, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "CardId", "DiskVolumeUsageLevel", "CardType", "PortId", "PicType", "EqActionType", "UnknownState", "SecondaryState", "PortMode", "PrimaryState", "ShelfId", "PortDetectedMode", "PortType", "DuplexStatus", "CardSubType", "FirmwareSource", "AdminState", "FlowControlMode")
...
TimeStamp, MacAddress, DateAndTime, TextualConvention, TruthValue, RowStatus, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "MacAddress", "DateAndTime", "TextualConvention", "TruthValue", "RowStatus", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditLogThrottle=auditLogThrottle, auditTime=auditTime, cardAction=cardAction, TestId=TestId, auditShelfId=auditShelfId, cardPrState=cardPrState, cmResetStatus=cmResetStatus, equipmentTbl=equipmentTbl, cardFpgaSource=cardFpgaSource, cardCarrierFwVersion=cardCarrierFwVersion, cardFlashSize=cardFlashSize, TestScheduleCommand=TestScheduleCommand, auditTotalFailedCount=auditTotalFailedCount, cardFwUpdateStatus=cardFwUpdateStatus, auditStatus=auditStatus, cardAdminState=cardAdminState, cardSpareGroupMode=cardSpareGroupMode, portPrStateChange=portPrStateChange, systemKey=systemKey, portNumChans=portNumChans, cardDplxStatusChange=cardDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, portNumber=portNumber, cardTestSchedule=cardTestSchedule, cardTempOverHeatNotification=cardTempOverHeatNotification, cardBusSpeed=cardBusSpeed, cardSubDetected=cardSubDetected, diskVolumeName=diskVolumeName, portTrapInh=portTrapInh, cardLastResetReason=cardLastResetReason, diskVolumeGroup=diskVolumeGroup, shelfSwVersion=shelfSwVersion, portEntry=portEntry, portSecState=portSecState, cardLastChangeTime=cardLastChangeTime, cardTable=cardTable, cardTestResultDesc=cardTestResultDesc, portMode=portMode, cardName=cardName, equipmentCardTestGroup=equipmentCardTestGroup, errEvLogLevel=errEvLogLevel, diskVolumeFileName=diskVolumeFileName, portBgpId=portBgpId, auditFailedCount=auditFailedCount, cardId=cardId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSubType=cardSubType, diagTestId=diagTestId, consoleOutput=consoleOutput, cardTestDescription=cardTestDescription, fpgaErrorEventTable=fpgaErrorEventTable, cmIpAddress=cmIpAddress, eqDiagConfig=eqDiagConfig, portMacAddress=portMacAddress, cardHwVersion=cardHwVersion, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTable=diskVolumeTable, cardTestScheduleCommand=cardTestScheduleCommand, auditCardId=auditCardId, cmUChannelID=cmUChannelID, diskVolumeEntry=diskVolumeEntry, auditName=auditName, auditEntry=auditEntry, cardSerialNum=cardSerialNum, cadEquipmentMibGroup=cadEquipmentMibGroup, shelfObjects=shelfObjects, portPrState=portPrState, cardTestResult=cardTestResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestType=cardTestType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierHwVersion=cardCarrierHwVersion, downstreamPowerLoss=downstreamPowerLoss, errEvRowStatus=errEvRowStatus, portConnectorId=portConnectorId, equipmentStateGroup=equipmentStateGroup, diskVolumeShelfId=diskVolumeShelfId, systemGeneralGroup=systemGeneralGroup, cardLastBootVersion=cardLastBootVersion, portMaxDsPower=portMaxDsPower, portDplxStatus=portDplxStatus, diskVolumeDriveId=diskVolumeDriveId, portCardSubType=portCardSubType, cardTestCommand=cardTestCommand, cmPrimarySID=cmPrimarySID, diskVolumeNumber=diskVolumeNumber, cardDetected=cardDetected, portRxFlowControlDetected=portRxFlowControlDetected, auditPassedCount=auditPassedCount, cardCpuType=cardCpuType, portLastChangeTime=portLastChangeTime, portId=portId, cardTestTime=cardTestTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSpareGroupId=cardSpareGroupId, shelfName=shelfName, cmRegistrationNotification=cmRegistrationNotification, cardShelfTestIndex=cardShelfTestIndex, cardDplxStatus=cardDplxStatus, equipmentCardGroup=equipmentCardGroup, cardLastBootSource=cardLastBootSource, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentShelfGroup=equipmentShelfGroup, auditDescription=auditDescription, diskVolumeUsageNotification=diskVolumeUsageNotification, diskDriveNumber=diskDriveNumber, cardCarrierSerialNum=cardCarrierSerialNum, TestResult=TestResult, cardSecState=cardSecState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditTable=auditTable, portMinDsPower=portMinDsPower, equipmentTraps=equipmentTraps, cardTestId=cardTestId, cadEquipmentMib=cadEquipmentMib, diskVolumeDiskSize=diskVolumeDiskSize, TestTransactionId=TestTransactionId, cadEquipmentMibConformance=cadEquipmentMibConformance, auditCycleCount=auditCycleCount, cardCPLDVersion=cardCPLDVersion, auditAutoScheduling=auditAutoScheduling, systemGeneral=systemGeneral, cardTestIndex=cardTestIndex, cardDetectedChange=cardDetectedChange, equipmentAuditGroup=equipmentAuditGroup, cmResetReason=cmResetReason, auditLogOutput=auditLogOutput, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestTable=cardTestTable, cardTestTransId=cardTestTransId, verboseLevel=verboseLevel, cardPicDetected=cardPicDetected, portSecStateChange=portSecStateChange, cardTestEntry=cardTestEntry, TestType=TestType, portType=portType, portDplxStatusChange=portDplxStatusChange, systemClock=systemClock, cmResetClearNotification=cmResetClearNotification, cardUpTime=cardUpTime, cardTemperatureHighWarn=cardTemperatureHighWarn, cardType=cardType, cardEntry=cardEntry, equipmentDiag=equipmentDiag, cardNumPorts=cardNumPorts, cardShelfId=cardShelfId, diskVolumeCardId=diskVolumeCardId, diskVolumeTrapInh=diskVolumeTrapInh, auditId=auditId, portDetectedMode=portDetectedMode, portTable=portTable, cmResetInfo=cmResetInfo, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlMode=portRxFlowControlMode, portAction=portAction, cmMacAddress=cmMacAddress, cardRamSize=cardRamSize, equipmentPortGroup=equipmentPortGroup, cardBootVersion=cardBootVersion, cardTestName=cardTestName, portShelfId=portShelfId, portMacIfIndex=portMacIfIndex, cardSecStateChange=cardSecStateChange, portCardId=portCardId, cardFwVersion=cardFwVersion, cardPicSerialNum=cardPicSerialNum, auditCommand=auditCommand, cmResetNotification=cmResetNotification, equipmentState=equipmentState, cmResetUpTime=cmResetUpTime, cardTemperatureHighError=cardTemperatureHighError, equipmentAudit=equipmentAudit, equipmentError=equipmentError, cardPrStateChange=cardPrStateChange, cmDevice=cmDevice, diskVolumeId=diskVolumeId, remainInDiagMode=remainInDiagMode, cardSwVersion=cardSwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portDescription=portDescription, diskVolumeUsageLevel=diskVolumeUsageLevel, portDocsIfIndex=portDocsIfIndex, diskVolumeSize=diskVolumeSize, TestCommand=TestCommand, cardTrapInh=cardTrapInh, cardTemperature=cardTemperature, cardCpuSpeed=cardCpuSpeed, cardPicHwVersion=cardPicHwVersion, auditTotalPassedCount=auditTotalPassedCount, shelfNumber=shelfNumber, auditResult=auditResult, fpgaErrorEventEntry=fpgaErrorEventEntry, cmVendor=cmVendor, portTxFlowControlDetected=portTxFlowControlDetected, portAdminState=portAdminState, errEventId=errEventId, cardNumber=cardNumber, cadEquipmentMibCompliance=cadEquipmentMibCompliance, trapCounter=trapCounter, TestSchedule=TestSchedule, PYSNMP_MODULE_ID=cadEquipmentMib, portCurrDsPower=portCurrDsPower, trapSeverity=trapSeverity, cardTestResultNotification=cardTestResultNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile)
