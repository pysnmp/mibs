...
CardSubType, PortId, FirmwareSource, DuplexStatus, UnknownState, ShelfId, DiskVolumeUsageLevel, AdminState, CardType, FlowControlMode, PortType, SecondaryState, EqActionType, PrimaryState, PortDetectedMode, CardId, PicType, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "PortId", "FirmwareSource", "DuplexStatus", "UnknownState", "ShelfId", "DiskVolumeUsageLevel", "AdminState", "CardType", "FlowControlMode", "PortType", "SecondaryState", "EqActionType", "PrimaryState", "PortDetectedMode", "CardId", "PicType", "PortMode")
...
MacAddress, TimeStamp, TextualConvention, RowStatus, DateAndTime, DisplayString, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TimeStamp", "TextualConvention", "RowStatus", "DateAndTime", "DisplayString", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTable=cardTable, cmDevice=cmDevice, diskVolumeGroup=diskVolumeGroup, cardLastBootSource=cardLastBootSource, diskVolumeNumber=diskVolumeNumber, portCurrDsPower=portCurrDsPower, cardShelfId=cardShelfId, cardTempOverHeatNotification=cardTempOverHeatNotification, TestResult=TestResult, cardBusSpeed=cardBusSpeed, cardTestScheduleCommand=cardTestScheduleCommand, equipmentAuditGroup=equipmentAuditGroup, remainInDiagMode=remainInDiagMode, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSpareGroupMode=cardSpareGroupMode, fpgaErrorEventTable=fpgaErrorEventTable, portDocsIfIndex=portDocsIfIndex, errEvRowStatus=errEvRowStatus, TestScheduleCommand=TestScheduleCommand, auditTime=auditTime, systemGeneral=systemGeneral, auditFailedCount=auditFailedCount, portPrStateChange=portPrStateChange, portDplxStatusChange=portDplxStatusChange, trapCounter=trapCounter, cardTestId=cardTestId, portLastChangeTime=portLastChangeTime, auditShelfId=auditShelfId, cmMacAddress=cmMacAddress, cmVendor=cmVendor, cardTestTime=cardTestTime, TestTransactionId=TestTransactionId, cardSerialNum=cardSerialNum, cardRamSize=cardRamSize, auditTable=auditTable, TestCommand=TestCommand, cardTestName=cardTestName, auditResult=auditResult, diskVolumeSize=diskVolumeSize, eqDiagConfig=eqDiagConfig, TestId=TestId, cardTestResult=cardTestResult, cardLastChangeTime=cardLastChangeTime, cadEquipmentMib=cadEquipmentMib, diskDriveNumber=diskDriveNumber, portMacAddress=portMacAddress, cmResetReason=cmResetReason, equipmentCardGroup=equipmentCardGroup, trapSeverity=trapSeverity, cardFwUpdateStatus=cardFwUpdateStatus, cardBootVersion=cardBootVersion, auditPassedCount=auditPassedCount, portSecStateChange=portSecStateChange, auditId=auditId, diskVolumeDriveId=diskVolumeDriveId, equipmentStateGroup=equipmentStateGroup, portEntry=portEntry, cmResetClearNotification=cmResetClearNotification, systemKey=systemKey, portConnectorId=portConnectorId, cardPicDetected=cardPicDetected, portDplxStatus=portDplxStatus, cardTestIndex=cardTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardTestTransId=cardTestTransId, cardPrState=cardPrState, diskVolumeCardId=diskVolumeCardId, cardSubType=cardSubType, cardTemperatureHighError=cardTemperatureHighError, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetUpTime=cmResetUpTime, cardDetectedChange=cardDetectedChange, systemGeneralGroup=systemGeneralGroup, errEvRecoveryEnabled=errEvRecoveryEnabled, errEvLoggingEnabled=errEvLoggingEnabled, cardFpgaSource=cardFpgaSource, portCardId=portCardId, equipmentDiag=equipmentDiag, portTable=portTable, cardCPLDVersion=cardCPLDVersion, auditDescription=auditDescription, auditLogThrottle=auditLogThrottle, cmUChannelID=cmUChannelID, auditName=auditName, shelfNumber=shelfNumber, cardCpuType=cardCpuType, shelfName=shelfName, diskVolumeEntry=diskVolumeEntry, portNumChans=portNumChans, diskVolumeTable=diskVolumeTable, cmResetNotification=cmResetNotification, diskVolumeName=diskVolumeName, cardTemperature=cardTemperature, portMode=portMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPicHwVersion=cardPicHwVersion, equipmentShelfGroup=equipmentShelfGroup, cardDetected=cardDetected, cardLastResetReason=cardLastResetReason, auditTotalFailedCount=auditTotalFailedCount, cardPrStateChange=cardPrStateChange, cardSpareGroupId=cardSpareGroupId, cardSubDetected=cardSubDetected, portBgpId=portBgpId, portTrapInh=portTrapInh, PYSNMP_MODULE_ID=cadEquipmentMib, cardAdminState=cardAdminState, diagTestId=diagTestId, cardCarrierHwVersion=cardCarrierHwVersion, cardNumPorts=cardNumPorts, auditStatus=auditStatus, equipmentAudit=equipmentAudit, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemClock=systemClock, equipmentError=equipmentError, cardName=cardName, cadEquipmentMibConformance=cadEquipmentMibConformance, cardFlashSize=cardFlashSize, cardLastBootVersion=cardLastBootVersion, portDetectedMode=portDetectedMode, portDescription=portDescription, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTempNoReportNotification=cardTempNoReportNotification, equipmentDiagGroup=equipmentDiagGroup, TestSchedule=TestSchedule, errEventId=errEventId, cardShelfTestIndex=cardShelfTestIndex, shelfSwVersion=shelfSwVersion, cardNumber=cardNumber, portShelfId=portShelfId, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageNotification=diskVolumeUsageNotification, cardEntry=cardEntry, equipmentState=equipmentState, downstreamPowerLoss=downstreamPowerLoss, cardUpTime=cardUpTime, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSecState=cardSecState, diskVolumeDiskSize=diskVolumeDiskSize, portId=portId, auditLogOutput=auditLogOutput, portMinDsPower=portMinDsPower, portNumber=portNumber, cardCarrierSerialNum=cardCarrierSerialNum, portCardSubType=portCardSubType, diskVolumeTrapInh=diskVolumeTrapInh, cardSecStateChange=cardSecStateChange, cardDplxStatusChange=cardDplxStatusChange, fpgaErrorEventEntry=fpgaErrorEventEntry, portAdminState=portAdminState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentCardTestGroup=equipmentCardTestGroup, cardTestTable=cardTestTable, cardSwVersion=cardSwVersion, portAction=portAction, cardId=cardId, errEvLogLevel=errEvLogLevel, portSecState=portSecState, portMaxDsPower=portMaxDsPower, auditCommand=auditCommand, cardType=cardType, cardTestDescription=cardTestDescription, cardDplxStatus=cardDplxStatus, auditCycleCount=auditCycleCount, portPrState=portPrState, cardTestSchedule=cardTestSchedule, cmPrimarySID=cmPrimarySID, cardTestCommand=cardTestCommand, cardFwVersion=cardFwVersion, cardCpuSpeed=cardCpuSpeed, portMacIfIndex=portMacIfIndex, portType=portType, verboseLevel=verboseLevel, auditCardId=auditCardId, diskVolumeFileName=diskVolumeFileName, cmIpAddress=cmIpAddress, shelfObjects=shelfObjects, cardTrapInh=cardTrapInh, cardTestResultNotification=cardTestResultNotification, TestType=TestType, diskVolumeId=diskVolumeId, cardCarrierFwVersion=cardCarrierFwVersion, cardTestEntry=cardTestEntry, cardTestType=cardTestType, equipmentTbl=equipmentTbl, diskVolumeUsageLevel=diskVolumeUsageLevel, auditTotalPassedCount=auditTotalPassedCount, cardAction=cardAction, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicSerialNum=cardPicSerialNum, equipmentTraps=equipmentTraps, auditEntry=auditEntry, cmResetStatus=cmResetStatus, consoleOutput=consoleOutput, cardHwVersion=cardHwVersion, portTxFlowControlMode=portTxFlowControlMode, diskVolumeShelfId=diskVolumeShelfId, cmResetInfo=cmResetInfo, auditAutoScheduling=auditAutoScheduling, cardTestResultDesc=cardTestResultDesc)
