...
CardId, CardType, UnknownState, PrimaryState, SecondaryState, DiskVolumeUsageLevel, PortMode, AdminState, DuplexStatus, FirmwareSource, CardSubType, ShelfId, PortType, EqActionType, FlowControlMode, PortDetectedMode, PortId, PicType = mibBuilder.importSymbols("CADANT-TC", "CardId", "CardType", "UnknownState", "PrimaryState", "SecondaryState", "DiskVolumeUsageLevel", "PortMode", "AdminState", "DuplexStatus", "FirmwareSource", "CardSubType", "ShelfId", "PortType", "EqActionType", "FlowControlMode", "PortDetectedMode", "PortId", "PicType")
...
TruthValue, TimeStamp, TextualConvention, MacAddress, DisplayString, RowStatus, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TimeStamp", "TextualConvention", "MacAddress", "DisplayString", "RowStatus", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portPrStateChange=portPrStateChange, cardTestTransId=cardTestTransId, TestId=TestId, cardSecState=cardSecState, cardRamSize=cardRamSize, cardCarrierHwVersion=cardCarrierHwVersion, cardLastBootVersion=cardLastBootVersion, portRxFlowControlDetected=portRxFlowControlDetected, cardId=cardId, equipmentPortGroup=equipmentPortGroup, portNumChans=portNumChans, equipmentStateGroup=equipmentStateGroup, equipmentState=equipmentState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardCpuType=cardCpuType, portLastChangeTime=portLastChangeTime, shelfSwVersion=shelfSwVersion, cardTestEntry=cardTestEntry, cardSubDetected=cardSubDetected, portTable=portTable, cardPicDetected=cardPicDetected, trapCounter=trapCounter, errEvLoggingEnabled=errEvLoggingEnabled, cardTable=cardTable, eqDiagConfig=eqDiagConfig, portMinDsPower=portMinDsPower, cardLastBootSource=cardLastBootSource, diskDriveNumber=diskDriveNumber, equipmentCardGroup=equipmentCardGroup, cadEquipmentMib=cadEquipmentMib, cardSpareGroupId=cardSpareGroupId, cardTestTime=cardTestTime, cardTestResult=cardTestResult, diskVolumeId=diskVolumeId, cmResetUpTime=cmResetUpTime, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeDiskSize=diskVolumeDiskSize, cardBootVersion=cardBootVersion, cardHwVersion=cardHwVersion, portPrState=portPrState, diskVolumeShelfId=diskVolumeShelfId, cardTestType=cardTestType, diskVolumeCardId=diskVolumeCardId, cardEntry=cardEntry, cmMacAddress=cmMacAddress, portDocsIfIndex=portDocsIfIndex, portCardSubType=portCardSubType, diskVolumeNumber=diskVolumeNumber, PYSNMP_MODULE_ID=cadEquipmentMib, cardBusSpeed=cardBusSpeed, cardSubType=cardSubType, verboseLevel=verboseLevel, equipmentDiagGroup=equipmentDiagGroup, TestResult=TestResult, shelfName=shelfName, auditCycleCount=auditCycleCount, cardSwVersion=cardSwVersion, auditTime=auditTime, cmResetReason=cmResetReason, trapSeverity=trapSeverity, cardFlashSize=cardFlashSize, TestCommand=TestCommand, cardPicHwVersion=cardPicHwVersion, portBgpId=portBgpId, cardDetectedChange=cardDetectedChange, cardTestResultNotification=cardTestResultNotification, diskVolumeName=diskVolumeName, cardTestSchedule=cardTestSchedule, cardDplxStatus=cardDplxStatus, systemGeneralGroup=systemGeneralGroup, cardTestId=cardTestId, errEvLogLevel=errEvLogLevel, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageNotification=diskVolumeUsageNotification, cardType=cardType, portSecStateChange=portSecStateChange, cardTestName=cardTestName, cardTestDescription=cardTestDescription, auditName=auditName, errEventId=errEventId, cardTemperature=cardTemperature, diskVolumeGroup=diskVolumeGroup, cardTemperatureHighError=cardTemperatureHighError, portCurrDsPower=portCurrDsPower, cmUChannelID=cmUChannelID, equipmentTraps=equipmentTraps, cmResetInfo=cmResetInfo, cmResetNotification=cmResetNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestScheduleCommand=cardTestScheduleCommand, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwVersion=cardFwVersion, cmVendor=cmVendor, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeFileName=diskVolumeFileName, cadEquipmentMibConformance=cadEquipmentMibConformance, auditResult=auditResult, cardFwUpdateStatus=cardFwUpdateStatus, auditStatus=auditStatus, portDplxStatusChange=portDplxStatusChange, auditFailedCount=auditFailedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, portConnectorId=portConnectorId, auditLogThrottle=auditLogThrottle, cmResetClearNotification=cmResetClearNotification, cmResetStatus=cmResetStatus, errEvRowStatus=errEvRowStatus, diskVolumeSize=diskVolumeSize, cmPrimarySID=cmPrimarySID, portMode=portMode, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTrapInh=cardTrapInh, portTrapInh=portTrapInh, equipmentDiag=equipmentDiag, portNumber=portNumber, cardUpTime=cardUpTime, equipmentAudit=equipmentAudit, cardTempNoReportNotification=cardTempNoReportNotification, cardShelfTestIndex=cardShelfTestIndex, TestType=TestType, auditDescription=auditDescription, portDetectedMode=portDetectedMode, diskVolumeEntry=diskVolumeEntry, shelfObjects=shelfObjects, portMacAddress=portMacAddress, auditCardId=auditCardId, cardFpgaSource=cardFpgaSource, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfNumber=shelfNumber, systemClock=systemClock, equipmentAuditGroup=equipmentAuditGroup, portId=portId, portDescription=portDescription, downstreamPowerLoss=downstreamPowerLoss, auditId=auditId, auditTotalFailedCount=auditTotalFailedCount, portAdminState=portAdminState, cardSecStateChange=cardSecStateChange, cardTestTable=cardTestTable, diskVolumeTable=diskVolumeTable, portShelfId=portShelfId, cmRegistrationNotification=cmRegistrationNotification, cardPrStateChange=cardPrStateChange, auditEntry=auditEntry, equipmentTbl=equipmentTbl, cardTestIndex=cardTestIndex, auditCommand=auditCommand, diskVolumeTrapInh=diskVolumeTrapInh, equipmentError=equipmentError, portTxFlowControlMode=portTxFlowControlMode, cmIpAddress=cmIpAddress, TestTransactionId=TestTransactionId, cardLastResetReason=cardLastResetReason, cardDetected=cardDetected, consoleOutput=consoleOutput, portRxFlowControlMode=portRxFlowControlMode, auditShelfId=auditShelfId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portSecState=portSecState, auditAutoScheduling=auditAutoScheduling, cardTemperatureHighWarn=cardTemperatureHighWarn, systemKey=systemKey, cmDevice=cmDevice, fpgaErrorEventGroup=fpgaErrorEventGroup, portType=portType, cardTestResultDesc=cardTestResultDesc, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardAdminState=cardAdminState, cardPicSerialNum=cardPicSerialNum, cardAction=cardAction, cardSpareGroupMode=cardSpareGroupMode, TestSchedule=TestSchedule, portAction=portAction, cardPrState=cardPrState, portMaxDsPower=portMaxDsPower, remainInDiagMode=remainInDiagMode, diagTestId=diagTestId, cardDplxStatusChange=cardDplxStatusChange, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageLevel=diskVolumeUsageLevel, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSerialNum=cardSerialNum, cardCpuSpeed=cardCpuSpeed, auditLogOutput=auditLogOutput, cardLastChangeTime=cardLastChangeTime, cardShelfId=cardShelfId, TestScheduleCommand=TestScheduleCommand, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardName=cardName, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCPLDVersion=cardCPLDVersion, equipmentShelfGroup=equipmentShelfGroup, fpgaErrorEventTable=fpgaErrorEventTable, cardNumPorts=cardNumPorts, portTxFlowControlDetected=portTxFlowControlDetected, cardTestCommand=cardTestCommand, systemGeneral=systemGeneral, portDplxStatus=portDplxStatus, portMacIfIndex=portMacIfIndex, auditTable=auditTable, auditPassedCount=auditPassedCount, portCardId=portCardId, portEntry=portEntry, cardNumber=cardNumber)
