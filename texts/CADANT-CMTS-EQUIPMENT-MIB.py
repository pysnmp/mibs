...
UnknownState, DuplexStatus, PortType, PortDetectedMode, ShelfId, PortId, PicType, SecondaryState, FlowControlMode, CardType, PortMode, DiskVolumeUsageLevel, FirmwareSource, AdminState, PrimaryState, CardId, EqActionType, CardSubType = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "DuplexStatus", "PortType", "PortDetectedMode", "ShelfId", "PortId", "PicType", "SecondaryState", "FlowControlMode", "CardType", "PortMode", "DiskVolumeUsageLevel", "FirmwareSource", "AdminState", "PrimaryState", "CardId", "EqActionType", "CardSubType")
...
RowStatus, TimeStamp, MacAddress, TextualConvention, TruthValue, DateAndTime, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "MacAddress", "TextualConvention", "TruthValue", "DateAndTime", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditStatus=auditStatus, equipmentAuditGroup=equipmentAuditGroup, cardTestResultNotification=cardTestResultNotification, cmResetStatus=cmResetStatus, TestScheduleCommand=TestScheduleCommand, cardTemperature=cardTemperature, portDocsIfIndex=portDocsIfIndex, cardTestId=cardTestId, equipmentAudit=equipmentAudit, shelfName=shelfName, cardPrStateChange=cardPrStateChange, cardTemperatureHighWarn=cardTemperatureHighWarn, portMacIfIndex=portMacIfIndex, diskDriveNumber=diskDriveNumber, cardPrState=cardPrState, cardPicHwVersion=cardPicHwVersion, cmPrimarySID=cmPrimarySID, diskVolumeDiskSize=diskVolumeDiskSize, equipmentShelfGroup=equipmentShelfGroup, cardPicDetected=cardPicDetected, cardShelfTestIndex=cardShelfTestIndex, cardTestTransId=cardTestTransId, portNumber=portNumber, errEventId=errEventId, systemGeneral=systemGeneral, cardAction=cardAction, TestType=TestType, portEntry=portEntry, cmResetInfo=cmResetInfo, cardTestTime=cardTestTime, TestTransactionId=TestTransactionId, TestCommand=TestCommand, portRxFlowControlDetected=portRxFlowControlDetected, cardSecStateChange=cardSecStateChange, cardLastChangeTime=cardLastChangeTime, cmDevice=cmDevice, cmMacAddress=cmMacAddress, portDplxStatusChange=portDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, cardTestResult=cardTestResult, portTxFlowControlMode=portTxFlowControlMode, systemGeneralGroup=systemGeneralGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, systemClock=systemClock, portType=portType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardLastBootSource=cardLastBootSource, cardTrapInh=cardTrapInh, TestSchedule=TestSchedule, auditTable=auditTable, cardBusSpeed=cardBusSpeed, remainInDiagMode=remainInDiagMode, portCurrDsPower=portCurrDsPower, auditEntry=auditEntry, cardCpuType=cardCpuType, cadEquipmentMib=cadEquipmentMib, cardType=cardType, cardBootVersion=cardBootVersion, trapSeverity=trapSeverity, portCardSubType=portCardSubType, cardTestCommand=cardTestCommand, auditLogOutput=auditLogOutput, fpgaErrorEventGroup=fpgaErrorEventGroup, portDescription=portDescription, cardTestEntry=cardTestEntry, cardSpareGroupId=cardSpareGroupId, diskVolumeId=diskVolumeId, cardFwUpdateStatus=cardFwUpdateStatus, cardName=cardName, cardDetected=cardDetected, portMacAddress=portMacAddress, portLastChangeTime=portLastChangeTime, portBgpId=portBgpId, equipmentPortGroup=equipmentPortGroup, portDetectedMode=portDetectedMode, auditCardId=auditCardId, fpgaErrorEventEntry=fpgaErrorEventEntry, portSecStateChange=portSecStateChange, cardTestDescription=cardTestDescription, cardLastBootVersion=cardLastBootVersion, diagTestId=diagTestId, equipmentTraps=equipmentTraps, auditShelfId=auditShelfId, cardTable=cardTable, portNumChans=portNumChans, diskVolumeShelfId=diskVolumeShelfId, diskVolumeTrapInh=diskVolumeTrapInh, cardFpgaSource=cardFpgaSource, diskVolumeTable=diskVolumeTable, equipmentCardTestGroup=equipmentCardTestGroup, cardSecState=cardSecState, trapCounter=trapCounter, cmResetUpTime=cmResetUpTime, cardFlashSize=cardFlashSize, diskVolumeNumber=diskVolumeNumber, cardSubType=cardSubType, errEvLogLevel=errEvLogLevel, cardCarrierSerialNum=cardCarrierSerialNum, portMaxDsPower=portMaxDsPower, cardCpuSpeed=cardCpuSpeed, diskVolumeGroup=diskVolumeGroup, cardShelfId=cardShelfId, auditTotalFailedCount=auditTotalFailedCount, downstreamPowerLoss=downstreamPowerLoss, systemKey=systemKey, cardHwVersion=cardHwVersion, equipmentTbl=equipmentTbl, cardUpTime=cardUpTime, portAction=portAction, errEvLoggingEnabled=errEvLoggingEnabled, shelfSwVersion=shelfSwVersion, cardTestName=cardTestName, cmRegistrationNotification=cmRegistrationNotification, cardSwVersion=cardSwVersion, portMinDsPower=portMinDsPower, portMode=portMode, cardTestScheduleCommand=cardTestScheduleCommand, auditLogThrottle=auditLogThrottle, cardFwVersion=cardFwVersion, diskVolumeName=diskVolumeName, cardTempNoReportNotification=cardTempNoReportNotification, cmIpAddress=cmIpAddress, cardRamSize=cardRamSize, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierFwVersion=cardCarrierFwVersion, auditPassedCount=auditPassedCount, equipmentStateGroup=equipmentStateGroup, portConnectorId=portConnectorId, eqDiagConfig=eqDiagConfig, cardTestSchedule=cardTestSchedule, portCardId=portCardId, cardId=cardId, cardSerialNum=cardSerialNum, cardCPLDVersion=cardCPLDVersion, cardSubDetected=cardSubDetected, cardDplxStatusChange=cardDplxStatusChange, cmResetNotification=cmResetNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditName=auditName, cardTemperatureHighError=cardTemperatureHighError, cardTestTable=cardTestTable, portTable=portTable, auditDescription=auditDescription, auditTime=auditTime, portAdminState=portAdminState, portTxFlowControlDetected=portTxFlowControlDetected, cardTestIndex=cardTestIndex, auditFailedCount=auditFailedCount, cmResetClearNotification=cmResetClearNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResultDesc=cardTestResultDesc, cardTempOverHeatNotification=cardTempOverHeatNotification, cardEntry=cardEntry, portSecState=portSecState, diskVolumeSize=diskVolumeSize, auditResult=auditResult, portId=portId, cadEquipmentMibGroup=cadEquipmentMibGroup, portShelfId=portShelfId, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardGroup=equipmentCardGroup, shelfNumber=shelfNumber, shelfObjects=shelfObjects, consoleOutput=consoleOutput, auditAutoScheduling=auditAutoScheduling, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeEntry=diskVolumeEntry, cardLastResetReason=cardLastResetReason, cardNumber=cardNumber, cardPicSerialNum=cardPicSerialNum, errEvRecoveryEnabled=errEvRecoveryEnabled, portPrState=portPrState, cmResetReason=cmResetReason, portDplxStatus=portDplxStatus, cardDplxStatus=cardDplxStatus, cardNumPorts=cardNumPorts, cardSpareGroupMode=cardSpareGroupMode, errEvRowStatus=errEvRowStatus, equipmentState=equipmentState, equipmentError=equipmentError, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCarrierHwVersion=cardCarrierHwVersion, verboseLevel=verboseLevel, cmVendor=cmVendor, cardDetectedChange=cardDetectedChange, portPrStateChange=portPrStateChange, diskVolumeFileName=diskVolumeFileName, diskVolumeCardId=diskVolumeCardId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageLevel=diskVolumeUsageLevel, portTrapInh=portTrapInh, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, PYSNMP_MODULE_ID=cadEquipmentMib, auditId=auditId, cardAdminState=cardAdminState, equipmentDiag=equipmentDiag, portRxFlowControlMode=portRxFlowControlMode, auditCommand=auditCommand, cmUChannelID=cmUChannelID, TestResult=TestResult, cardTestType=cardTestType, auditCycleCount=auditCycleCount, TestId=TestId, auditTotalPassedCount=auditTotalPassedCount, diskVolumeDriveId=diskVolumeDriveId, cadEquipmentMibCompliance=cadEquipmentMibCompliance)
