...
ShelfId, AdminState, UnknownState, FlowControlMode, PortType, PrimaryState, CardSubType, CardId, FirmwareSource, DiskVolumeUsageLevel, PortDetectedMode, CardType, EqActionType, PortMode, PortId, DuplexStatus, PicType, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "AdminState", "UnknownState", "FlowControlMode", "PortType", "PrimaryState", "CardSubType", "CardId", "FirmwareSource", "DiskVolumeUsageLevel", "PortDetectedMode", "CardType", "EqActionType", "PortMode", "PortId", "DuplexStatus", "PicType", "SecondaryState")
...
TimeStamp, MacAddress, DateAndTime, TextualConvention, TruthValue, DisplayString, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "MacAddress", "DateAndTime", "TextualConvention", "TruthValue", "DisplayString", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portPrState=portPrState, cmDevice=cmDevice, errEventId=errEventId, portNumber=portNumber, cardSpareGroupId=cardSpareGroupId, cardNumPorts=cardNumPorts, cardTempNoReportNotification=cardTempNoReportNotification, portEntry=portEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentAudit=equipmentAudit, systemGeneral=systemGeneral, auditDescription=auditDescription, portMaxDsPower=portMaxDsPower, cardCpuSpeed=cardCpuSpeed, portMinDsPower=portMinDsPower, auditLogThrottle=auditLogThrottle, diskVolumeNumber=diskVolumeNumber, cardSecStateChange=cardSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardPicSerialNum=cardPicSerialNum, cardLastBootVersion=cardLastBootVersion, cardSerialNum=cardSerialNum, auditCommand=auditCommand, cardFwVersion=cardFwVersion, equipmentDiagGroup=equipmentDiagGroup, cardId=cardId, auditTable=auditTable, cardAdminState=cardAdminState, cardName=cardName, auditStatus=auditStatus, trapCounter=trapCounter, portDescription=portDescription, fpgaErrorEventTable=fpgaErrorEventTable, cardLastResetReason=cardLastResetReason, equipmentStateGroup=equipmentStateGroup, portTrapInh=portTrapInh, cardTemperatureHighError=cardTemperatureHighError, cardTestTime=cardTestTime, cardCarrierFwVersion=cardCarrierFwVersion, auditPassedCount=auditPassedCount, TestType=TestType, equipmentState=equipmentState, trapSeverity=trapSeverity, portAdminState=portAdminState, TestSchedule=TestSchedule, cardCarrierSerialNum=cardCarrierSerialNum, auditCycleCount=auditCycleCount, TestId=TestId, diskVolumeDriveId=diskVolumeDriveId, cardTable=cardTable, systemGeneralGroup=systemGeneralGroup, auditId=auditId, cardFwUpdateStatus=cardFwUpdateStatus, cardSpareGroupMode=cardSpareGroupMode, portLastChangeTime=portLastChangeTime, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portShelfId=portShelfId, cardDetectedChange=cardDetectedChange, TestScheduleCommand=TestScheduleCommand, cmRegistrationNotification=cmRegistrationNotification, auditName=auditName, diagTestId=diagTestId, equipmentTraps=equipmentTraps, cardAction=cardAction, cardBusSpeed=cardBusSpeed, diskVolumeEntry=diskVolumeEntry, cardShelfTestIndex=cardShelfTestIndex, cmIpAddress=cmIpAddress, cardSubType=cardSubType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeName=diskVolumeName, portBgpId=portBgpId, cmPrimarySID=cmPrimarySID, cardTestCommand=cardTestCommand, cmMacAddress=cmMacAddress, downstreamPowerLoss=downstreamPowerLoss, equipmentAuditGroup=equipmentAuditGroup, systemKey=systemKey, diskVolumeTable=diskVolumeTable, diskVolumeTrapInh=diskVolumeTrapInh, errEvLogLevel=errEvLogLevel, equipmentPortGroup=equipmentPortGroup, cardDetected=cardDetected, auditEntry=auditEntry, equipmentCardGroup=equipmentCardGroup, cardType=cardType, cardPicHwVersion=cardPicHwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, cardPrStateChange=cardPrStateChange, portType=portType, equipmentTbl=equipmentTbl, cmResetReason=cmResetReason, cardEntry=cardEntry, cardTestTransId=cardTestTransId, fpgaErrorEventEntry=fpgaErrorEventEntry, portDplxStatusChange=portDplxStatusChange, cardRamSize=cardRamSize, cmResetUpTime=cmResetUpTime, cardBootVersion=cardBootVersion, equipmentShelfGroup=equipmentShelfGroup, cmResetInfo=cmResetInfo, auditLogOutput=auditLogOutput, TestTransactionId=TestTransactionId, cardFpgaSource=cardFpgaSource, errEvRowStatus=errEvRowStatus, cardSwVersion=cardSwVersion, portMode=portMode, cardPicDetected=cardPicDetected, auditTime=auditTime, portCardId=portCardId, cardFlashSize=cardFlashSize, portNumChans=portNumChans, cardTestId=cardTestId, cardTempOverHeatNotification=cardTempOverHeatNotification, auditTotalFailedCount=auditTotalFailedCount, cardTestName=cardTestName, verboseLevel=verboseLevel, cardTestTable=cardTestTable, cmResetStatus=cmResetStatus, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, consoleOutput=consoleOutput, auditFailedCount=auditFailedCount, equipmentError=equipmentError, cardCpuType=cardCpuType, PYSNMP_MODULE_ID=cadEquipmentMib, remainInDiagMode=remainInDiagMode, diskVolumeSize=diskVolumeSize, systemClock=systemClock, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestSchedule=cardTestSchedule, fpgaErrorEventGroup=fpgaErrorEventGroup, portCardSubType=portCardSubType, cardTestResult=cardTestResult, cmVendor=cmVendor, auditResult=auditResult, diskVolumeShelfId=diskVolumeShelfId, cardDplxStatus=cardDplxStatus, portPrStateChange=portPrStateChange, portTxFlowControlMode=portTxFlowControlMode, cardCarrierHwVersion=cardCarrierHwVersion, portAction=portAction, cardLastChangeTime=cardLastChangeTime, portDetectedMode=portDetectedMode, portDocsIfIndex=portDocsIfIndex, cardTestDescription=cardTestDescription, cardTrapInh=cardTrapInh, portCurrDsPower=portCurrDsPower, cardSecState=cardSecState, cardShelfId=cardShelfId, portSecState=portSecState, equipmentCardTestGroup=equipmentCardTestGroup, diskDriveNumber=diskDriveNumber, diskVolumeGroup=diskVolumeGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardUpTime=cardUpTime, eqDiagConfig=eqDiagConfig, cadEquipmentMib=cadEquipmentMib, cardLastBootSource=cardLastBootSource, cmResetNotification=cmResetNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeId=diskVolumeId, portConnectorId=portConnectorId, portSecStateChange=portSecStateChange, diskVolumeFileName=diskVolumeFileName, TestResult=TestResult, portMacIfIndex=portMacIfIndex, cardTestResultNotification=cardTestResultNotification, cardPrState=cardPrState, portRxFlowControlDetected=portRxFlowControlDetected, portTxFlowControlDetected=portTxFlowControlDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestResultDesc=cardTestResultDesc, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentDiag=equipmentDiag, diskVolumeDiskSize=diskVolumeDiskSize, cardNumber=cardNumber, shelfNumber=shelfNumber, shelfName=shelfName, portTable=portTable, cmResetClearNotification=cmResetClearNotification, cardHwVersion=cardHwVersion, cardSubDetected=cardSubDetected, shelfObjects=shelfObjects, cardCPLDVersion=cardCPLDVersion, cardTestIndex=cardTestIndex, auditCardId=auditCardId, portId=portId, TestCommand=TestCommand, cmUChannelID=cmUChannelID, portMacAddress=portMacAddress, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestScheduleCommand=cardTestScheduleCommand, cardTestEntry=cardTestEntry, cardDplxStatusChange=cardDplxStatusChange, diskVolumeCardId=diskVolumeCardId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditShelfId=auditShelfId, portDplxStatus=portDplxStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTemperature=cardTemperature, cardTestType=cardTestType, shelfSwVersion=shelfSwVersion)
