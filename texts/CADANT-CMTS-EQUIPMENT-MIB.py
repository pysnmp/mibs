...
PortDetectedMode, PortType, PortMode, FlowControlMode, AdminState, CardType, CardId, FirmwareSource, SecondaryState, EqActionType, ShelfId, PrimaryState, DiskVolumeUsageLevel, PicType, PortId, UnknownState, DuplexStatus, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "PortType", "PortMode", "FlowControlMode", "AdminState", "CardType", "CardId", "FirmwareSource", "SecondaryState", "EqActionType", "ShelfId", "PrimaryState", "DiskVolumeUsageLevel", "PicType", "PortId", "UnknownState", "DuplexStatus", "CardSubType")
...
TimeStamp, MacAddress, DisplayString, TruthValue, TextualConvention, RowStatus, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "MacAddress", "DisplayString", "TruthValue", "TextualConvention", "RowStatus", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", verboseLevel=verboseLevel, equipmentState=equipmentState, equipmentTraps=equipmentTraps, cmVendor=cmVendor, cardEntry=cardEntry, auditLogOutput=auditLogOutput, cmResetStatus=cmResetStatus, portCurrDsPower=portCurrDsPower, diskVolumeId=diskVolumeId, cardTestType=cardTestType, portDetectedMode=portDetectedMode, portMinDsPower=portMinDsPower, cardTestTable=cardTestTable, cardTestDescription=cardTestDescription, auditEntry=auditEntry, cmMacAddress=cmMacAddress, auditCommand=auditCommand, auditLogThrottle=auditLogThrottle, auditStatus=auditStatus, cardDplxStatusChange=cardDplxStatusChange, cmResetUpTime=cmResetUpTime, cadEquipmentMib=cadEquipmentMib, auditPassedCount=auditPassedCount, cardTestIndex=cardTestIndex, diskVolumeFileName=diskVolumeFileName, portNumChans=portNumChans, diskVolumeDiskSize=diskVolumeDiskSize, portSecState=portSecState, portLastChangeTime=portLastChangeTime, cardSpareGroupId=cardSpareGroupId, TestCommand=TestCommand, cmIpAddress=cmIpAddress, equipmentDiagGroup=equipmentDiagGroup, diskVolumeEntry=diskVolumeEntry, equipmentAuditGroup=equipmentAuditGroup, diskVolumeGroup=diskVolumeGroup, cardAdminState=cardAdminState, cmResetInfo=cmResetInfo, shelfSwVersion=shelfSwVersion, cardCpuType=cardCpuType, cardNumber=cardNumber, cardShelfId=cardShelfId, cardTempOverHeatNotification=cardTempOverHeatNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, portTable=portTable, auditTime=auditTime, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSwVersion=cardSwVersion, cardUpTime=cardUpTime, cardTestCommand=cardTestCommand, portAction=portAction, diskVolumeName=diskVolumeName, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestTransId=cardTestTransId, auditTable=auditTable, portCardId=portCardId, auditFailedCount=auditFailedCount, eqDiagConfig=eqDiagConfig, auditDescription=auditDescription, portTrapInh=portTrapInh, errEvLogLevel=errEvLogLevel, cardLastChangeTime=cardLastChangeTime, cardPicHwVersion=cardPicHwVersion, auditName=auditName, auditShelfId=auditShelfId, portDocsIfIndex=portDocsIfIndex, auditCardId=auditCardId, equipmentPortGroup=equipmentPortGroup, portCardSubType=portCardSubType, diskVolumeShelfId=diskVolumeShelfId, cmResetNotification=cmResetNotification, downstreamPowerLoss=downstreamPowerLoss, systemGeneralGroup=systemGeneralGroup, errEvRowStatus=errEvRowStatus, cardId=cardId, equipmentShelfGroup=equipmentShelfGroup, cardNumPorts=cardNumPorts, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResult=cardTestResult, cmRegistrationNotification=cmRegistrationNotification, cardFlashSize=cardFlashSize, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTable=cardTable, portSecStateChange=portSecStateChange, diskVolumeTrapInh=diskVolumeTrapInh, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestResultDesc=cardTestResultDesc, TestType=TestType, cardLastBootSource=cardLastBootSource, cardPrStateChange=cardPrStateChange, shelfNumber=shelfNumber, portMacIfIndex=portMacIfIndex, TestScheduleCommand=TestScheduleCommand, cardFpgaSource=cardFpgaSource, cardTestEntry=cardTestEntry, cardName=cardName, TestId=TestId, portBgpId=portBgpId, auditTotalFailedCount=auditTotalFailedCount, cardTempNoReportNotification=cardTempNoReportNotification, consoleOutput=consoleOutput, portTxFlowControlMode=portTxFlowControlMode, cardShelfTestIndex=cardShelfTestIndex, equipmentTbl=equipmentTbl, portPrStateChange=portPrStateChange, shelfObjects=shelfObjects, cardCarrierSerialNum=cardCarrierSerialNum, cardCarrierHwVersion=cardCarrierHwVersion, portEntry=portEntry, cardAction=cardAction, cmResetReason=cmResetReason, cardCarrierFwVersion=cardCarrierFwVersion, systemGeneral=systemGeneral, equipmentStateGroup=equipmentStateGroup, diskVolumeSize=diskVolumeSize, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeDriveId=diskVolumeDriveId, cardPicDetected=cardPicDetected, auditAutoScheduling=auditAutoScheduling, cardFwUpdateStatus=cardFwUpdateStatus, cardCpuSpeed=cardCpuSpeed, portConnectorId=portConnectorId, cardTestName=cardTestName, cadEquipmentMibGroup=cadEquipmentMibGroup, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSubDetected=cardSubDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardType=cardType, portRxFlowControlDetected=portRxFlowControlDetected, cardTestSchedule=cardTestSchedule, cmPrimarySID=cmPrimarySID, diskVolumeCardId=diskVolumeCardId, errEventId=errEventId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, remainInDiagMode=remainInDiagMode, cardTestTime=cardTestTime, portMaxDsPower=portMaxDsPower, errEvLoggingEnabled=errEvLoggingEnabled, cardTestId=cardTestId, systemKey=systemKey, cardFwVersion=cardFwVersion, cmUChannelID=cmUChannelID, diskVolumeTable=diskVolumeTable, equipmentError=equipmentError, cardLastBootVersion=cardLastBootVersion, auditId=auditId, cardTemperature=cardTemperature, portAdminState=portAdminState, cardTrapInh=cardTrapInh, cardRamSize=cardRamSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portId=portId, portDplxStatus=portDplxStatus, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventTable=fpgaErrorEventTable, portDplxStatusChange=portDplxStatusChange, diskDriveNumber=diskDriveNumber, cardHwVersion=cardHwVersion, cardSpareGroupMode=cardSpareGroupMode, diskVolumeNumber=diskVolumeNumber, portMacAddress=portMacAddress, cardDetectedChange=cardDetectedChange, fpgaErrorEventEntry=fpgaErrorEventEntry, auditResult=auditResult, cmResetClearNotification=cmResetClearNotification, cardPrState=cardPrState, cardDplxStatus=cardDplxStatus, cardBootVersion=cardBootVersion, cardDetected=cardDetected, cardTestResultNotification=cardTestResultNotification, shelfName=shelfName, cardTestScheduleCommand=cardTestScheduleCommand, auditCycleCount=auditCycleCount, cardCPLDVersion=cardCPLDVersion, PYSNMP_MODULE_ID=cadEquipmentMib, TestSchedule=TestSchedule, portMode=portMode, cmDevice=cmDevice, portPrState=portPrState, trapCounter=trapCounter, diagTestId=diagTestId, systemClock=systemClock, cardSubType=cardSubType, cardTemperatureHighError=cardTemperatureHighError, cardLastResetReason=cardLastResetReason, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSecState=cardSecState, portNumber=portNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portTxFlowControlDetected=portTxFlowControlDetected, portShelfId=portShelfId, cardBusSpeed=cardBusSpeed, cardSecStateChange=cardSecStateChange, TestResult=TestResult, portType=portType, equipmentAudit=equipmentAudit, portDescription=portDescription, equipmentDiag=equipmentDiag, trapSeverity=trapSeverity, equipmentCardGroup=equipmentCardGroup, TestTransactionId=TestTransactionId, auditTotalPassedCount=auditTotalPassedCount, cardSerialNum=cardSerialNum)
