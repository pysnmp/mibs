...
PortId, CardId, FirmwareSource, DuplexStatus, PortType, SecondaryState, AdminState, FlowControlMode, ShelfId, EqActionType, UnknownState, PrimaryState, PortDetectedMode, CardSubType, DiskVolumeUsageLevel, CardType, PicType, PortMode = mibBuilder.importSymbols("CADANT-TC", "PortId", "CardId", "FirmwareSource", "DuplexStatus", "PortType", "SecondaryState", "AdminState", "FlowControlMode", "ShelfId", "EqActionType", "UnknownState", "PrimaryState", "PortDetectedMode", "CardSubType", "DiskVolumeUsageLevel", "CardType", "PicType", "PortMode")
...
TimeStamp, MacAddress, DisplayString, RowStatus, TruthValue, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "MacAddress", "DisplayString", "RowStatus", "TruthValue", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentAudit=equipmentAudit, auditName=auditName, cmDevice=cmDevice, diskVolumeCardId=diskVolumeCardId, cardTrapInh=cardTrapInh, cmUChannelID=cmUChannelID, portCardId=portCardId, fpgaErrorEventTable=fpgaErrorEventTable, portNumChans=portNumChans, diskVolumeGroup=diskVolumeGroup, cardLastBootVersion=cardLastBootVersion, cardDplxStatus=cardDplxStatus, equipmentCardTestGroup=equipmentCardTestGroup, cardSpareGroupMode=cardSpareGroupMode, errEvRecoveryEnabled=errEvRecoveryEnabled, portMacIfIndex=portMacIfIndex, cardNumber=cardNumber, cmPrimarySID=cmPrimarySID, cardLastBootSource=cardLastBootSource, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmMacAddress=cmMacAddress, cardBusSpeed=cardBusSpeed, equipmentPortGroup=equipmentPortGroup, cardDetected=cardDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, trapCounter=trapCounter, cardPicDetected=cardPicDetected, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardName=cardName, shelfObjects=shelfObjects, cardTestEntry=cardTestEntry, auditFailedCount=auditFailedCount, diskVolumeUsageLevel=diskVolumeUsageLevel, portType=portType, portId=portId, equipmentAuditGroup=equipmentAuditGroup, auditCycleCount=auditCycleCount, systemGeneral=systemGeneral, PYSNMP_MODULE_ID=cadEquipmentMib, cardSwVersion=cardSwVersion, cardRamSize=cardRamSize, cardCarrierSerialNum=cardCarrierSerialNum, equipmentError=equipmentError, diskVolumeUsageNotification=diskVolumeUsageNotification, cardLastChangeTime=cardLastChangeTime, cardCPLDVersion=cardCPLDVersion, cardTestTable=cardTestTable, equipmentShelfGroup=equipmentShelfGroup, auditResult=auditResult, auditLogOutput=auditLogOutput, auditCommand=auditCommand, diskDriveNumber=diskDriveNumber, cardTestTransId=cardTestTransId, cardSecState=cardSecState, cardCpuType=cardCpuType, cmRegistrationNotification=cmRegistrationNotification, auditPassedCount=auditPassedCount, portRxFlowControlMode=portRxFlowControlMode, cardTestScheduleCommand=cardTestScheduleCommand, trapSeverity=trapSeverity, portConnectorId=portConnectorId, cardTempNoReportNotification=cardTempNoReportNotification, shelfName=shelfName, equipmentCardGroup=equipmentCardGroup, TestSchedule=TestSchedule, diskVolumeTable=diskVolumeTable, portAction=portAction, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetInfo=cmResetInfo, portMacAddress=portMacAddress, cardPicHwVersion=cardPicHwVersion, cardFlashSize=cardFlashSize, cardTestResultNotification=cardTestResultNotification, portEntry=portEntry, diskVolumeTrapInh=diskVolumeTrapInh, eqDiagConfig=eqDiagConfig, cardTestResultDesc=cardTestResultDesc, TestResult=TestResult, portMinDsPower=portMinDsPower, portDplxStatusChange=portDplxStatusChange, cmResetUpTime=cmResetUpTime, diskVolumeEntry=diskVolumeEntry, cardAdminState=cardAdminState, verboseLevel=verboseLevel, auditCardId=auditCardId, systemKey=systemKey, cardTestType=cardTestType, auditTotalFailedCount=auditTotalFailedCount, cardPrStateChange=cardPrStateChange, cardShelfTestIndex=cardShelfTestIndex, auditTime=auditTime, auditAutoScheduling=auditAutoScheduling, portAdminState=portAdminState, shelfNumber=shelfNumber, portDocsIfIndex=portDocsIfIndex, auditId=auditId, cardTestCommand=cardTestCommand, cardUpTime=cardUpTime, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCarrierHwVersion=cardCarrierHwVersion, equipmentState=equipmentState, diskVolumeDiskSize=diskVolumeDiskSize, equipmentDiag=equipmentDiag, portNumber=portNumber, cardFpgaSource=cardFpgaSource, cardTestId=cardTestId, equipmentStateGroup=equipmentStateGroup, cmResetReason=cmResetReason, errEvLoggingEnabled=errEvLoggingEnabled, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeShelfId=diskVolumeShelfId, cardSecStateChange=cardSecStateChange, auditTable=auditTable, portRxFlowControlDetected=portRxFlowControlDetected, cardHwVersion=cardHwVersion, cardId=cardId, cardPrState=cardPrState, portTxFlowControlDetected=portTxFlowControlDetected, auditShelfId=auditShelfId, cardPicSerialNum=cardPicSerialNum, cardShelfId=cardShelfId, portCurrDsPower=portCurrDsPower, cardFwVersion=cardFwVersion, diskVolumeId=diskVolumeId, cardTemperatureHighWarn=cardTemperatureHighWarn, TestTransactionId=TestTransactionId, cardTestResult=cardTestResult, consoleOutput=consoleOutput, cmIpAddress=cmIpAddress, auditTotalPassedCount=auditTotalPassedCount, portDplxStatus=portDplxStatus, cardSubType=cardSubType, TestCommand=TestCommand, portTable=portTable, cmResetStatus=cmResetStatus, errEventId=errEventId, diskVolumeDriveId=diskVolumeDriveId, portLastChangeTime=portLastChangeTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestId=TestId, diskVolumeSize=diskVolumeSize, cardDetectedChange=cardDetectedChange, auditEntry=auditEntry, errEvLogLevel=errEvLogLevel, cardNumPorts=cardNumPorts, shelfSwVersion=shelfSwVersion, equipmentTraps=equipmentTraps, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSpareGroupId=cardSpareGroupId, cardTestName=cardTestName, portMode=portMode, cardBootVersion=cardBootVersion, portBgpId=portBgpId, diskVolumeName=diskVolumeName, remainInDiagMode=remainInDiagMode, cardCpuSpeed=cardCpuSpeed, cardTestIndex=cardTestIndex, equipmentTbl=equipmentTbl, auditDescription=auditDescription, portCardSubType=portCardSubType, cardSubDetected=cardSubDetected, cardLastResetReason=cardLastResetReason, cardTestSchedule=cardTestSchedule, diagTestId=diagTestId, cardTable=cardTable, portTrapInh=portTrapInh, portPrStateChange=portPrStateChange, cardFwUpdateStatus=cardFwUpdateStatus, portSecState=portSecState, cardTemperatureHighError=cardTemperatureHighError, equipmentDiagGroup=equipmentDiagGroup, portShelfId=portShelfId, fpgaErrorEventGroup=fpgaErrorEventGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardEntry=cardEntry, cardAction=cardAction, portMaxDsPower=portMaxDsPower, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portSecStateChange=portSecStateChange, errEvRowStatus=errEvRowStatus, diskVolumeNumber=diskVolumeNumber, auditLogThrottle=auditLogThrottle, systemClock=systemClock, auditStatus=auditStatus, portTxFlowControlMode=portTxFlowControlMode, cmVendor=cmVendor, cmResetNotification=cmResetNotification, cardType=cardType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTemperature=cardTemperature, downstreamPowerLoss=downstreamPowerLoss, portDetectedMode=portDetectedMode, diskVolumeFileName=diskVolumeFileName, portPrState=portPrState, cardTestTime=cardTestTime, cardTestDescription=cardTestDescription, cmResetClearNotification=cmResetClearNotification, systemGeneralGroup=systemGeneralGroup, cardSerialNum=cardSerialNum, TestScheduleCommand=TestScheduleCommand, cadEquipmentMib=cadEquipmentMib, portDescription=portDescription, TestType=TestType, cardDplxStatusChange=cardDplxStatusChange)
