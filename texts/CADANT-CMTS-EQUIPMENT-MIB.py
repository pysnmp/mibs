...
UnknownState, PortType, FirmwareSource, CardSubType, SecondaryState, PortId, ShelfId, DuplexStatus, PicType, PortMode, CardType, EqActionType, CardId, PrimaryState, PortDetectedMode, FlowControlMode, DiskVolumeUsageLevel, AdminState = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "PortType", "FirmwareSource", "CardSubType", "SecondaryState", "PortId", "ShelfId", "DuplexStatus", "PicType", "PortMode", "CardType", "EqActionType", "CardId", "PrimaryState", "PortDetectedMode", "FlowControlMode", "DiskVolumeUsageLevel", "AdminState")
...
DisplayString, DateAndTime, TextualConvention, TruthValue, MacAddress, TimeStamp, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "DateAndTime", "TextualConvention", "TruthValue", "MacAddress", "TimeStamp", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portConnectorId=portConnectorId, cardTestTable=cardTestTable, portEntry=portEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardShelfId=cardShelfId, portType=portType, portAdminState=portAdminState, equipmentAudit=equipmentAudit, equipmentError=equipmentError, fpgaErrorEventTable=fpgaErrorEventTable, cardBootVersion=cardBootVersion, cardTempNoReportNotification=cardTempNoReportNotification, trapCounter=trapCounter, cardLastChangeTime=cardLastChangeTime, portPrStateChange=portPrStateChange, portLastChangeTime=portLastChangeTime, diskVolumeUsageNotification=diskVolumeUsageNotification, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestIndex=cardTestIndex, equipmentAuditGroup=equipmentAuditGroup, auditEntry=auditEntry, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portBgpId=portBgpId, diskVolumeFileName=diskVolumeFileName, auditCardId=auditCardId, auditStatus=auditStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, TestTransactionId=TestTransactionId, cardTrapInh=cardTrapInh, cardTestDescription=cardTestDescription, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSpareGroupMode=cardSpareGroupMode, TestCommand=TestCommand, cardTable=cardTable, portTable=portTable, errEventId=errEventId, diskVolumeDiskSize=diskVolumeDiskSize, cadEquipmentMib=cadEquipmentMib, cardNumber=cardNumber, cardRamSize=cardRamSize, cardTestEntry=cardTestEntry, cardSecStateChange=cardSecStateChange, shelfObjects=shelfObjects, cmVendor=cmVendor, cardBusSpeed=cardBusSpeed, portCurrDsPower=portCurrDsPower, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardUpTime=cardUpTime, cardTestName=cardTestName, shelfName=shelfName, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeTrapInh=diskVolumeTrapInh, TestScheduleCommand=TestScheduleCommand, TestSchedule=TestSchedule, diskVolumeGroup=diskVolumeGroup, cardTestTime=cardTestTime, diskVolumeId=diskVolumeId, errEvLogLevel=errEvLogLevel, cardCpuType=cardCpuType, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTemperatureHighError=cardTemperatureHighError, auditCommand=auditCommand, remainInDiagMode=remainInDiagMode, systemGeneralGroup=systemGeneralGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardLastResetReason=cardLastResetReason, cardCpuSpeed=cardCpuSpeed, portSecState=portSecState, auditResult=auditResult, portAction=portAction, portShelfId=portShelfId, diskVolumeEntry=diskVolumeEntry, cmResetNotification=cmResetNotification, equipmentStateGroup=equipmentStateGroup, portMinDsPower=portMinDsPower, cardNumPorts=cardNumPorts, cardType=cardType, systemClock=systemClock, auditId=auditId, equipmentCardTestGroup=equipmentCardTestGroup, cardPrStateChange=cardPrStateChange, portCardSubType=portCardSubType, auditShelfId=auditShelfId, portMacAddress=portMacAddress, portDescription=portDescription, cardSecState=cardSecState, cardTestResultDesc=cardTestResultDesc, cardFpgaSource=cardFpgaSource, shelfNumber=shelfNumber, cmResetStatus=cmResetStatus, cardTestSchedule=cardTestSchedule, portMode=portMode, diskVolumeSize=diskVolumeSize, shelfSwVersion=shelfSwVersion, diskVolumeDriveId=diskVolumeDriveId, cardPicHwVersion=cardPicHwVersion, auditLogThrottle=auditLogThrottle, auditName=auditName, portDocsIfIndex=portDocsIfIndex, cardCPLDVersion=cardCPLDVersion, auditTotalPassedCount=auditTotalPassedCount, portSecStateChange=portSecStateChange, cardSubDetected=cardSubDetected, portDplxStatus=portDplxStatus, auditLogOutput=auditLogOutput, cardTestResult=cardTestResult, auditPassedCount=auditPassedCount, TestId=TestId, diskVolumeTable=diskVolumeTable, diskVolumeNumber=diskVolumeNumber, cardFlashSize=cardFlashSize, cardShelfTestIndex=cardShelfTestIndex, auditCycleCount=auditCycleCount, verboseLevel=verboseLevel, equipmentPortGroup=equipmentPortGroup, cardName=cardName, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSerialNum=cardSerialNum, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentTraps=equipmentTraps, downstreamPowerLoss=downstreamPowerLoss, cardTemperature=cardTemperature, cadEquipmentMibConformance=cadEquipmentMibConformance, cardId=cardId, diskDriveNumber=diskDriveNumber, diskVolumeShelfId=diskVolumeShelfId, portTrapInh=portTrapInh, trapSeverity=trapSeverity, portId=portId, systemKey=systemKey, errEvLoggingEnabled=errEvLoggingEnabled, portNumChans=portNumChans, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicDetected=cardPicDetected, cmDevice=cmDevice, cmResetUpTime=cmResetUpTime, cardTestType=cardTestType, diagTestId=diagTestId, cardTestTransId=cardTestTransId, cadEquipmentMibGroup=cadEquipmentMibGroup, portDplxStatusChange=portDplxStatusChange, equipmentTbl=equipmentTbl, cmRegistrationNotification=cmRegistrationNotification, cardSubType=cardSubType, cardDplxStatus=cardDplxStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardLastBootSource=cardLastBootSource, portNumber=portNumber, portPrState=portPrState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardPrState=cardPrState, systemGeneral=systemGeneral, equipmentDiag=equipmentDiag, cardTemperatureHighWarn=cardTemperatureHighWarn, cmPrimarySID=cmPrimarySID, TestResult=TestResult, cardCarrierSerialNum=cardCarrierSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, cardHwVersion=cardHwVersion, equipmentCardGroup=equipmentCardGroup, auditAutoScheduling=auditAutoScheduling, cardSwVersion=cardSwVersion, portTxFlowControlMode=portTxFlowControlMode, cmResetInfo=cmResetInfo, cardTestId=cardTestId, cmUChannelID=cmUChannelID, cardAdminState=cardAdminState, portDetectedMode=portDetectedMode, equipmentShelfGroup=equipmentShelfGroup, portMacIfIndex=portMacIfIndex, cardEntry=cardEntry, cmResetReason=cmResetReason, cardFwVersion=cardFwVersion, cardTestCommand=cardTestCommand, cmMacAddress=cmMacAddress, cardAction=cardAction, auditDescription=auditDescription, auditTotalFailedCount=auditTotalFailedCount, TestType=TestType, cardCarrierFwVersion=cardCarrierFwVersion, equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, consoleOutput=consoleOutput, auditTable=auditTable, cardTestScheduleCommand=cardTestScheduleCommand, auditTime=auditTime, auditFailedCount=auditFailedCount, cardDetected=cardDetected, diskVolumeName=diskVolumeName, cmIpAddress=cmIpAddress, cardSpareGroupId=cardSpareGroupId, cardLastBootVersion=cardLastBootVersion, portCardId=portCardId, eqDiagConfig=eqDiagConfig, cardDetectedChange=cardDetectedChange, fpgaErrorEventGroup=fpgaErrorEventGroup, portRxFlowControlMode=portRxFlowControlMode, errEvRowStatus=errEvRowStatus, cardDplxStatusChange=cardDplxStatusChange, cmResetClearNotification=cmResetClearNotification, portMaxDsPower=portMaxDsPower, equipmentDiagGroup=equipmentDiagGroup, cardTestResultNotification=cardTestResultNotification)
