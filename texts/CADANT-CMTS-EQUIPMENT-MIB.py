...
PortType, PicType, EqActionType, SecondaryState, FlowControlMode, CardType, CardSubType, ShelfId, PrimaryState, DuplexStatus, PortId, DiskVolumeUsageLevel, PortMode, PortDetectedMode, UnknownState, AdminState, CardId, FirmwareSource = mibBuilder.importSymbols("CADANT-TC", "PortType", "PicType", "EqActionType", "SecondaryState", "FlowControlMode", "CardType", "CardSubType", "ShelfId", "PrimaryState", "DuplexStatus", "PortId", "DiskVolumeUsageLevel", "PortMode", "PortDetectedMode", "UnknownState", "AdminState", "CardId", "FirmwareSource")
...
TextualConvention, MacAddress, TimeStamp, DisplayString, TruthValue, DateAndTime, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "TimeStamp", "DisplayString", "TruthValue", "DateAndTime", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestScheduleCommand=cardTestScheduleCommand, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestSchedule=cardTestSchedule, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditLogOutput=auditLogOutput, portMaxDsPower=portMaxDsPower, portTrapInh=portTrapInh, cardTempOverHeatNotification=cardTempOverHeatNotification, consoleOutput=consoleOutput, TestId=TestId, cardPrStateChange=cardPrStateChange, diskVolumeFileName=diskVolumeFileName, diskVolumeDriveId=diskVolumeDriveId, cardLastResetReason=cardLastResetReason, diskVolumeName=diskVolumeName, portAdminState=portAdminState, cardDetected=cardDetected, equipmentAuditGroup=equipmentAuditGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentDiagGroup=equipmentDiagGroup, cardTestTable=cardTestTable, diskVolumeEntry=diskVolumeEntry, diskVolumeCardId=diskVolumeCardId, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResultDesc=cardTestResultDesc, cmResetNotification=cmResetNotification, trapCounter=trapCounter, auditPassedCount=auditPassedCount, cardCpuSpeed=cardCpuSpeed, cmResetReason=cmResetReason, cardSubDetected=cardSubDetected, portDplxStatus=portDplxStatus, auditTotalFailedCount=auditTotalFailedCount, errEvRowStatus=errEvRowStatus, diskDriveNumber=diskDriveNumber, portRxFlowControlMode=portRxFlowControlMode, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestIndex=cardTestIndex, portCardId=portCardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMacIfIndex=portMacIfIndex, cardTable=cardTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portCardSubType=portCardSubType, auditTotalPassedCount=auditTotalPassedCount, cardShelfId=cardShelfId, cardTestDescription=cardTestDescription, cmUChannelID=cmUChannelID, cardDplxStatusChange=cardDplxStatusChange, cardType=cardType, auditDescription=auditDescription, equipmentError=equipmentError, cmResetStatus=cmResetStatus, TestSchedule=TestSchedule, cardDplxStatus=cardDplxStatus, portLastChangeTime=portLastChangeTime, diskVolumeTable=diskVolumeTable, diskVolumeUsageLevel=diskVolumeUsageLevel, cardFpgaSource=cardFpgaSource, shelfObjects=shelfObjects, cardId=cardId, portSecStateChange=portSecStateChange, diagTestId=diagTestId, cardTrapInh=cardTrapInh, shelfNumber=shelfNumber, portAction=portAction, cardSecState=cardSecState, cardSecStateChange=cardSecStateChange, portSecState=portSecState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentCardGroup=equipmentCardGroup, cardCPLDVersion=cardCPLDVersion, cardShelfTestIndex=cardShelfTestIndex, diskVolumeUsageNotification=diskVolumeUsageNotification, portDocsIfIndex=portDocsIfIndex, portMacAddress=portMacAddress, equipmentDiag=equipmentDiag, auditFailedCount=auditFailedCount, cardTestResultNotification=cardTestResultNotification, cmPrimarySID=cmPrimarySID, equipmentShelfGroup=equipmentShelfGroup, cmIpAddress=cmIpAddress, portMinDsPower=portMinDsPower, cardTestTransId=cardTestTransId, cardCpuType=cardCpuType, auditLogThrottle=auditLogThrottle, TestType=TestType, auditShelfId=auditShelfId, portType=portType, auditCardId=auditCardId, remainInDiagMode=remainInDiagMode, cardTemperatureHighError=cardTemperatureHighError, cardLastBootVersion=cardLastBootVersion, auditStatus=auditStatus, auditResult=auditResult, trapSeverity=trapSeverity, equipmentState=equipmentState, portShelfId=portShelfId, systemGeneral=systemGeneral, diskVolumeShelfId=diskVolumeShelfId, cmResetUpTime=cmResetUpTime, cardLastBootSource=cardLastBootSource, portEntry=portEntry, cardUpTime=cardUpTime, cardBusSpeed=cardBusSpeed, diskVolumeId=diskVolumeId, cmMacAddress=cmMacAddress, portNumChans=portNumChans, cmResetInfo=cmResetInfo, cardSwVersion=cardSwVersion, cadEquipmentMib=cadEquipmentMib, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardRamSize=cardRamSize, cardTestTime=cardTestTime, cardPicDetected=cardPicDetected, auditCycleCount=auditCycleCount, portDescription=portDescription, cardCarrierFwVersion=cardCarrierFwVersion, portCurrDsPower=portCurrDsPower, cardSpareGroupId=cardSpareGroupId, cardTestCommand=cardTestCommand, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cadEquipmentMibGroup=cadEquipmentMibGroup, verboseLevel=verboseLevel, equipmentTbl=equipmentTbl, cardPrState=cardPrState, eqDiagConfig=eqDiagConfig, cardFlashSize=cardFlashSize, TestTransactionId=TestTransactionId, cardPicHwVersion=cardPicHwVersion, systemClock=systemClock, cardFwUpdateStatus=cardFwUpdateStatus, equipmentPortGroup=equipmentPortGroup, diskVolumeNumber=diskVolumeNumber, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeTrapInh=diskVolumeTrapInh, cardTestResult=cardTestResult, cardSerialNum=cardSerialNum, portDetectedMode=portDetectedMode, auditEntry=auditEntry, portConnectorId=portConnectorId, cardHwVersion=cardHwVersion, fpgaErrorEventTable=fpgaErrorEventTable, errEvRecoveryEnabled=errEvRecoveryEnabled, errEvLoggingEnabled=errEvLoggingEnabled, auditTable=auditTable, PYSNMP_MODULE_ID=cadEquipmentMib, portTxFlowControlMode=portTxFlowControlMode, cardCarrierSerialNum=cardCarrierSerialNum, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditTime=auditTime, diskVolumeGroup=diskVolumeGroup, equipmentStateGroup=equipmentStateGroup, portPrState=portPrState, portId=portId, portRxFlowControlDetected=portRxFlowControlDetected, cardDetectedChange=cardDetectedChange, cardTemperature=cardTemperature, auditName=auditName, cardTestEntry=cardTestEntry, portTable=portTable, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibConformance=cadEquipmentMibConformance, cardCarrierHwVersion=cardCarrierHwVersion, portPrStateChange=portPrStateChange, cardTestId=cardTestId, cmDevice=cmDevice, cardTestName=cardTestName, auditId=auditId, equipmentTraps=equipmentTraps, cmVendor=cmVendor, cardNumber=cardNumber, cardEntry=cardEntry, cardAdminState=cardAdminState, portNumber=portNumber, portMode=portMode, systemGeneralGroup=systemGeneralGroup, systemKey=systemKey, cardFwVersion=cardFwVersion, diskVolumeSize=diskVolumeSize, TestCommand=TestCommand, auditCommand=auditCommand, cardName=cardName, equipmentAudit=equipmentAudit, cardSubType=cardSubType, errEventId=errEventId, shelfName=shelfName, cardSpareGroupMode=cardSpareGroupMode, cardAction=cardAction, cmResetClearNotification=cmResetClearNotification, cmRegistrationNotification=cmRegistrationNotification, portBgpId=portBgpId, TestScheduleCommand=TestScheduleCommand, downstreamPowerLoss=downstreamPowerLoss, cardTempNoReportNotification=cardTempNoReportNotification, cardTestType=cardTestType, TestResult=TestResult, cardLastChangeTime=cardLastChangeTime, cardBootVersion=cardBootVersion, shelfSwVersion=shelfSwVersion, errEvLogLevel=errEvLogLevel, equipmentCardTestGroup=equipmentCardTestGroup)
