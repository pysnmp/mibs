...
CardType, PortType, PrimaryState, ShelfId, PicType, FirmwareSource, UnknownState, CardId, CardSubType, FlowControlMode, SecondaryState, PortDetectedMode, AdminState, DiskVolumeUsageLevel, DuplexStatus, PortId, EqActionType, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortType", "PrimaryState", "ShelfId", "PicType", "FirmwareSource", "UnknownState", "CardId", "CardSubType", "FlowControlMode", "SecondaryState", "PortDetectedMode", "AdminState", "DiskVolumeUsageLevel", "DuplexStatus", "PortId", "EqActionType", "PortMode")
...
DisplayString, TruthValue, DateAndTime, MacAddress, RowStatus, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TruthValue", "DateAndTime", "MacAddress", "RowStatus", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", trapCounter=trapCounter, portEntry=portEntry, cardSerialNum=cardSerialNum, TestResult=TestResult, portId=portId, diskVolumeNumber=diskVolumeNumber, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portPrStateChange=portPrStateChange, shelfName=shelfName, cardAction=cardAction, cardDplxStatus=cardDplxStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCarrierFwVersion=cardCarrierFwVersion, portCardId=portCardId, cardFwUpdateStatus=cardFwUpdateStatus, cardTestResultNotification=cardTestResultNotification, downstreamPowerLoss=downstreamPowerLoss, equipmentAuditGroup=equipmentAuditGroup, portTxFlowControlMode=portTxFlowControlMode, cmResetInfo=cmResetInfo, auditPassedCount=auditPassedCount, TestScheduleCommand=TestScheduleCommand, cardName=cardName, consoleOutput=consoleOutput, cardBootVersion=cardBootVersion, portSecState=portSecState, portMode=portMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeFileName=diskVolumeFileName, cardFwVersion=cardFwVersion, cardSubDetected=cardSubDetected, cardTemperature=cardTemperature, auditFailedCount=auditFailedCount, cmResetReason=cmResetReason, portLastChangeTime=portLastChangeTime, portTxFlowControlDetected=portTxFlowControlDetected, portMacAddress=portMacAddress, remainInDiagMode=remainInDiagMode, auditAutoScheduling=auditAutoScheduling, equipmentTbl=equipmentTbl, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTrapInh=cardTrapInh, equipmentTraps=equipmentTraps, cardTestScheduleCommand=cardTestScheduleCommand, cardPicDetected=cardPicDetected, cardSpareGroupId=cardSpareGroupId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeTable=diskVolumeTable, fpgaErrorEventEntry=fpgaErrorEventEntry, portPrState=portPrState, cardNumPorts=cardNumPorts, cardUpTime=cardUpTime, portDescription=portDescription, cardNumber=cardNumber, portDplxStatusChange=portDplxStatusChange, cardTestResultDesc=cardTestResultDesc, cardTestName=cardTestName, auditCommand=auditCommand, cardTestType=cardTestType, cardSwVersion=cardSwVersion, cardPrState=cardPrState, portRxFlowControlDetected=portRxFlowControlDetected, auditTable=auditTable, systemClock=systemClock, cardTestResult=cardTestResult, portCurrDsPower=portCurrDsPower, cmVendor=cmVendor, diskVolumeShelfId=diskVolumeShelfId, cardSecStateChange=cardSecStateChange, cardPicHwVersion=cardPicHwVersion, diskVolumeCardId=diskVolumeCardId, portConnectorId=portConnectorId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentDiagGroup=equipmentDiagGroup, portNumChans=portNumChans, cardDetectedChange=cardDetectedChange, TestCommand=TestCommand, cardTestTime=cardTestTime, cardTestEntry=cardTestEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, portBgpId=portBgpId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardLastResetReason=cardLastResetReason, cardHwVersion=cardHwVersion, cardCPLDVersion=cardCPLDVersion, cardTestTransId=cardTestTransId, cardAdminState=cardAdminState, cardDplxStatusChange=cardDplxStatusChange, cardSecState=cardSecState, systemGeneralGroup=systemGeneralGroup, cmUChannelID=cmUChannelID, auditCycleCount=auditCycleCount, cardCpuType=cardCpuType, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portSecStateChange=portSecStateChange, cmMacAddress=cmMacAddress, portTable=portTable, auditTime=auditTime, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeName=diskVolumeName, errEvRowStatus=errEvRowStatus, cardCarrierSerialNum=cardCarrierSerialNum, cadEquipmentMib=cadEquipmentMib, cardFpgaSource=cardFpgaSource, shelfNumber=shelfNumber, cardFlashSize=cardFlashSize, cardCarrierHwVersion=cardCarrierHwVersion, cmResetUpTime=cmResetUpTime, auditCardId=auditCardId, errEvLoggingEnabled=errEvLoggingEnabled, cardTestDescription=cardTestDescription, auditEntry=auditEntry, cardRamSize=cardRamSize, cardBusSpeed=cardBusSpeed, PYSNMP_MODULE_ID=cadEquipmentMib, TestTransactionId=TestTransactionId, portNumber=portNumber, cardLastBootSource=cardLastBootSource, shelfObjects=shelfObjects, diskDriveNumber=diskDriveNumber, cardTemperatureHighError=cardTemperatureHighError, cardTestTable=cardTestTable, equipmentAudit=equipmentAudit, auditResult=auditResult, cardShelfId=cardShelfId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestSchedule=TestSchedule, equipmentPortGroup=equipmentPortGroup, cardCpuSpeed=cardCpuSpeed, auditDescription=auditDescription, cmPrimarySID=cmPrimarySID, cardTable=cardTable, trapSeverity=trapSeverity, auditId=auditId, equipmentDiag=equipmentDiag, cardTestIndex=cardTestIndex, cardType=cardType, portAction=portAction, cardPicSerialNum=cardPicSerialNum, auditLogOutput=auditLogOutput, portTrapInh=portTrapInh, cardTestSchedule=cardTestSchedule, auditLogThrottle=auditLogThrottle, cardTemperatureHighWarn=cardTemperatureHighWarn, portType=portType, auditName=auditName, cmIpAddress=cmIpAddress, portAdminState=portAdminState, equipmentStateGroup=equipmentStateGroup, cmRegistrationNotification=cmRegistrationNotification, cardLastBootVersion=cardLastBootVersion, cmResetNotification=cmResetNotification, TestId=TestId, portRxFlowControlMode=portRxFlowControlMode, cardTestCommand=cardTestCommand, auditTotalFailedCount=auditTotalFailedCount, equipmentError=equipmentError, portDplxStatus=portDplxStatus, systemKey=systemKey, cmDevice=cmDevice, cardTestId=cardTestId, diskVolumeDiskSize=diskVolumeDiskSize, cardDetected=cardDetected, portDocsIfIndex=portDocsIfIndex, diskVolumeTrapInh=diskVolumeTrapInh, cardTempNoReportNotification=cardTempNoReportNotification, equipmentShelfGroup=equipmentShelfGroup, portMinDsPower=portMinDsPower, diskVolumeUsageLevel=diskVolumeUsageLevel, auditShelfId=auditShelfId, cardSpareGroupMode=cardSpareGroupMode, portMaxDsPower=portMaxDsPower, diskVolumeSize=diskVolumeSize, cmResetStatus=cmResetStatus, auditTotalPassedCount=auditTotalPassedCount, errEventId=errEventId, cardPrStateChange=cardPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, portCardSubType=portCardSubType, cardLastChangeTime=cardLastChangeTime, TestType=TestType, cardSubType=cardSubType, portDetectedMode=portDetectedMode, auditStatus=auditStatus, diskVolumeDriveId=diskVolumeDriveId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardShelfTestIndex=cardShelfTestIndex, diskVolumeId=diskVolumeId, cmResetClearNotification=cmResetClearNotification, errEvLogLevel=errEvLogLevel, eqDiagConfig=eqDiagConfig, fpgaErrorEventTable=fpgaErrorEventTable, cardId=cardId, portShelfId=portShelfId, diagTestId=diagTestId, systemGeneral=systemGeneral, shelfSwVersion=shelfSwVersion, verboseLevel=verboseLevel, equipmentState=equipmentState, diskVolumeGroup=diskVolumeGroup, cardEntry=cardEntry, portMacIfIndex=portMacIfIndex, errEvRecoveryEnabled=errEvRecoveryEnabled)
