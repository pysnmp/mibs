...
FirmwareSource, CardSubType, PortMode, EqActionType, AdminState, PortId, CardId, PortDetectedMode, PortType, SecondaryState, PicType, PrimaryState, ShelfId, DiskVolumeUsageLevel, DuplexStatus, FlowControlMode, UnknownState, CardType = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "CardSubType", "PortMode", "EqActionType", "AdminState", "PortId", "CardId", "PortDetectedMode", "PortType", "SecondaryState", "PicType", "PrimaryState", "ShelfId", "DiskVolumeUsageLevel", "DuplexStatus", "FlowControlMode", "UnknownState", "CardType")
...
MacAddress, RowStatus, TimeStamp, DateAndTime, TruthValue, TextualConvention, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "TimeStamp", "DateAndTime", "TruthValue", "TextualConvention", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", trapSeverity=trapSeverity, shelfObjects=shelfObjects, cardName=cardName, portConnectorId=portConnectorId, portTxFlowControlDetected=portTxFlowControlDetected, auditPassedCount=auditPassedCount, cmMacAddress=cmMacAddress, cardFlashSize=cardFlashSize, cardLastBootSource=cardLastBootSource, auditTable=auditTable, cmResetUpTime=cmResetUpTime, systemKey=systemKey, TestTransactionId=TestTransactionId, cmResetReason=cmResetReason, cmIpAddress=cmIpAddress, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDplxStatusChange=portDplxStatusChange, equipmentError=equipmentError, equipmentAuditGroup=equipmentAuditGroup, cardSerialNum=cardSerialNum, diskVolumeSize=diskVolumeSize, diskVolumeTrapInh=diskVolumeTrapInh, portLastChangeTime=portLastChangeTime, cardShelfId=cardShelfId, portRxFlowControlMode=portRxFlowControlMode, portAdminState=portAdminState, diskVolumeUsageNotification=diskVolumeUsageNotification, fpgaErrorEventTable=fpgaErrorEventTable, cardTestDescription=cardTestDescription, cmDevice=cmDevice, cmRegistrationNotification=cmRegistrationNotification, portMinDsPower=portMinDsPower, cardUpTime=cardUpTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentCardTestGroup=equipmentCardTestGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfNumber=shelfNumber, diskVolumeNumber=diskVolumeNumber, cardRamSize=cardRamSize, cardFpgaSource=cardFpgaSource, cmUChannelID=cmUChannelID, TestCommand=TestCommand, TestId=TestId, cardTestCommand=cardTestCommand, TestScheduleCommand=TestScheduleCommand, diskVolumeDriveId=diskVolumeDriveId, cardLastResetReason=cardLastResetReason, cardDetectedChange=cardDetectedChange, cmResetClearNotification=cmResetClearNotification, auditCommand=auditCommand, portCurrDsPower=portCurrDsPower, cardNumber=cardNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, auditTotalPassedCount=auditTotalPassedCount, cardAdminState=cardAdminState, portTrapInh=portTrapInh, equipmentAudit=equipmentAudit, cardTempNoReportNotification=cardTempNoReportNotification, cardPicHwVersion=cardPicHwVersion, cardSubType=cardSubType, errEvLogLevel=errEvLogLevel, portTable=portTable, equipmentStateGroup=equipmentStateGroup, diskVolumeName=diskVolumeName, equipmentPortGroup=equipmentPortGroup, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventGroup=fpgaErrorEventGroup, portAction=portAction, diskVolumeDiskSize=diskVolumeDiskSize, cardTestEntry=cardTestEntry, cardCarrierSerialNum=cardCarrierSerialNum, cardSubDetected=cardSubDetected, shelfName=shelfName, eqDiagConfig=eqDiagConfig, portMaxDsPower=portMaxDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentShelfGroup=equipmentShelfGroup, downstreamPowerLoss=downstreamPowerLoss, cardTemperature=cardTemperature, diagTestId=diagTestId, cardId=cardId, diskDriveNumber=diskDriveNumber, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardType=cardType, portDetectedMode=portDetectedMode, cardTestTransId=cardTestTransId, portMacIfIndex=portMacIfIndex, shelfSwVersion=shelfSwVersion, diskVolumeShelfId=diskVolumeShelfId, errEvRowStatus=errEvRowStatus, cardFwVersion=cardFwVersion, auditCycleCount=auditCycleCount, cardDetected=cardDetected, cardTestResultDesc=cardTestResultDesc, cmResetNotification=cmResetNotification, cardTable=cardTable, portPrStateChange=portPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentTraps=equipmentTraps, cardLastChangeTime=cardLastChangeTime, auditId=auditId, cardPicSerialNum=cardPicSerialNum, TestSchedule=TestSchedule, portMacAddress=portMacAddress, equipmentTbl=equipmentTbl, diskVolumeTable=diskVolumeTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditAutoScheduling=auditAutoScheduling, portNumber=portNumber, systemGeneral=systemGeneral, remainInDiagMode=remainInDiagMode, cardAction=cardAction, portTxFlowControlMode=portTxFlowControlMode, portSecState=portSecState, cardPicDetected=cardPicDetected, auditName=auditName, cardNumPorts=cardNumPorts, portCardId=portCardId, cardSpareGroupMode=cardSpareGroupMode, cmPrimarySID=cmPrimarySID, cardSpareGroupId=cardSpareGroupId, portPrState=portPrState, trapCounter=trapCounter, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestIndex=cardTestIndex, cardCpuSpeed=cardCpuSpeed, auditFailedCount=auditFailedCount, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portDocsIfIndex=portDocsIfIndex, cardTestTable=cardTestTable, cardFwUpdateStatus=cardFwUpdateStatus, cardSwVersion=cardSwVersion, portSecStateChange=portSecStateChange, auditLogThrottle=auditLogThrottle, cardCarrierHwVersion=cardCarrierHwVersion, cardTestScheduleCommand=cardTestScheduleCommand, auditDescription=auditDescription, systemGeneralGroup=systemGeneralGroup, portShelfId=portShelfId, cardTestType=cardTestType, cardSecState=cardSecState, cardPrStateChange=cardPrStateChange, cardBusSpeed=cardBusSpeed, equipmentDiagGroup=equipmentDiagGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, verboseLevel=verboseLevel, errEventId=errEventId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portNumChans=portNumChans, cardTrapInh=cardTrapInh, cardTestResult=cardTestResult, portCardSubType=portCardSubType, cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, cardBootVersion=cardBootVersion, cardDplxStatus=cardDplxStatus, PYSNMP_MODULE_ID=cadEquipmentMib, portEntry=portEntry, auditTime=auditTime, equipmentState=equipmentState, cardTemperatureHighError=cardTemperatureHighError, cardPrState=cardPrState, cardEntry=cardEntry, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeEntry=diskVolumeEntry, diskVolumeFileName=diskVolumeFileName, errEvLoggingEnabled=errEvLoggingEnabled, cmVendor=cmVendor, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeCardId=diskVolumeCardId, cmResetInfo=cmResetInfo, cardTestResultNotification=cardTestResultNotification, auditCardId=auditCardId, cardDplxStatusChange=cardDplxStatusChange, auditLogOutput=auditLogOutput, auditEntry=auditEntry, cardCPLDVersion=cardCPLDVersion, portMode=portMode, TestType=TestType, auditResult=auditResult, portDplxStatus=portDplxStatus, portDescription=portDescription, diskVolumeId=diskVolumeId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestTime=cardTestTime, equipmentCardGroup=equipmentCardGroup, TestResult=TestResult, cardTestId=cardTestId, cardTestSchedule=cardTestSchedule, errEvRecoveryEnabled=errEvRecoveryEnabled, consoleOutput=consoleOutput, auditShelfId=auditShelfId, systemClock=systemClock, cardLastBootVersion=cardLastBootVersion, portId=portId, cmResetStatus=cmResetStatus, cardSecStateChange=cardSecStateChange, portType=portType, cardHwVersion=cardHwVersion, auditTotalFailedCount=auditTotalFailedCount, cardTestName=cardTestName, equipmentDiag=equipmentDiag, diskVolumeGroup=diskVolumeGroup, portBgpId=portBgpId, auditStatus=auditStatus, cardCpuType=cardCpuType)
