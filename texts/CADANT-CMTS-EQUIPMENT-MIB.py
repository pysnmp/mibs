...
EqActionType, CardType, SecondaryState, FirmwareSource, PicType, DiskVolumeUsageLevel, CardId, DuplexStatus, AdminState, FlowControlMode, PortMode, PortDetectedMode, PrimaryState, UnknownState, ShelfId, PortType, CardSubType, PortId = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "CardType", "SecondaryState", "FirmwareSource", "PicType", "DiskVolumeUsageLevel", "CardId", "DuplexStatus", "AdminState", "FlowControlMode", "PortMode", "PortDetectedMode", "PrimaryState", "UnknownState", "ShelfId", "PortType", "CardSubType", "PortId")
...
TimeStamp, DisplayString, DateAndTime, TruthValue, MacAddress, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DisplayString", "DateAndTime", "TruthValue", "MacAddress", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTotalFailedCount=auditTotalFailedCount, cardShelfId=cardShelfId, portShelfId=portShelfId, cmResetStatus=cmResetStatus, cardTestDescription=cardTestDescription, portPrStateChange=portPrStateChange, cardTestEntry=cardTestEntry, cardType=cardType, auditCommand=auditCommand, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeTrapInh=diskVolumeTrapInh, equipmentShelfGroup=equipmentShelfGroup, auditTime=auditTime, errEvLoggingEnabled=errEvLoggingEnabled, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestTransId=cardTestTransId, trapSeverity=trapSeverity, equipmentCardTestGroup=equipmentCardTestGroup, portSecState=portSecState, cadEquipmentMibGroup=cadEquipmentMibGroup, portMaxDsPower=portMaxDsPower, cmResetClearNotification=cmResetClearNotification, cardTemperature=cardTemperature, cardCpuSpeed=cardCpuSpeed, cardDetected=cardDetected, equipmentDiag=equipmentDiag, TestResult=TestResult, cardTestType=cardTestType, systemClock=systemClock, cardFpgaSource=cardFpgaSource, cardPrStateChange=cardPrStateChange, shelfObjects=shelfObjects, equipmentDiagGroup=equipmentDiagGroup, cardHwVersion=cardHwVersion, auditResult=auditResult, TestScheduleCommand=TestScheduleCommand, cardSwVersion=cardSwVersion, verboseLevel=verboseLevel, cardId=cardId, cmPrimarySID=cmPrimarySID, cmResetNotification=cmResetNotification, cardFwUpdateStatus=cardFwUpdateStatus, cardSecState=cardSecState, portRxFlowControlDetected=portRxFlowControlDetected, cardLastResetReason=cardLastResetReason, portLastChangeTime=portLastChangeTime, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, eqDiagConfig=eqDiagConfig, auditId=auditId, diskVolumeDriveId=diskVolumeDriveId, portMinDsPower=portMinDsPower, cardTestSchedule=cardTestSchedule, cardTestResultDesc=cardTestResultDesc, cardPicHwVersion=cardPicHwVersion, portTxFlowControlMode=portTxFlowControlMode, diskVolumeFileName=diskVolumeFileName, portNumber=portNumber, cardAdminState=cardAdminState, portEntry=portEntry, cardPrState=cardPrState, portTrapInh=portTrapInh, cmResetInfo=cmResetInfo, cardTestResultNotification=cardTestResultNotification, portCardSubType=portCardSubType, equipmentAudit=equipmentAudit, cardDetectedChange=cardDetectedChange, diagTestId=diagTestId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResult=cardTestResult, portNumChans=portNumChans, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeCardId=diskVolumeCardId, cardBusSpeed=cardBusSpeed, systemKey=systemKey, diskDriveNumber=diskDriveNumber, portAdminState=portAdminState, auditName=auditName, trapCounter=trapCounter, equipmentAuditGroup=equipmentAuditGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardLastChangeTime=cardLastChangeTime, remainInDiagMode=remainInDiagMode, diskVolumeNumber=diskVolumeNumber, auditLogOutput=auditLogOutput, portId=portId, cardCarrierSerialNum=cardCarrierSerialNum, portDetectedMode=portDetectedMode, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentPortGroup=equipmentPortGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierHwVersion=cardCarrierHwVersion, portDplxStatusChange=portDplxStatusChange, cardSubDetected=cardSubDetected, cmRegistrationNotification=cmRegistrationNotification, cardSerialNum=cardSerialNum, cmIpAddress=cmIpAddress, shelfSwVersion=shelfSwVersion, cmResetUpTime=cmResetUpTime, portTxFlowControlDetected=portTxFlowControlDetected, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeDiskSize=diskVolumeDiskSize, equipmentTbl=equipmentTbl, downstreamPowerLoss=downstreamPowerLoss, cardNumber=cardNumber, cardFwVersion=cardFwVersion, auditLogThrottle=auditLogThrottle, cmDevice=cmDevice, portDocsIfIndex=portDocsIfIndex, cardTable=cardTable, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardAction=cardAction, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentStateGroup=equipmentStateGroup, shelfName=shelfName, cadEquipmentMib=cadEquipmentMib, TestCommand=TestCommand, errEvRowStatus=errEvRowStatus, cardDplxStatusChange=cardDplxStatusChange, TestId=TestId, cardLastBootSource=cardLastBootSource, cardCpuType=cardCpuType, portTable=portTable, cardTestScheduleCommand=cardTestScheduleCommand, systemGeneralGroup=systemGeneralGroup, TestType=TestType, diskVolumeEntry=diskVolumeEntry, portCurrDsPower=portCurrDsPower, auditTotalPassedCount=auditTotalPassedCount, portSecStateChange=portSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestIndex=cardTestIndex, TestSchedule=TestSchedule, cardPicDetected=cardPicDetected, shelfNumber=shelfNumber, cmMacAddress=cmMacAddress, cardTestTable=cardTestTable, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeShelfId=diskVolumeShelfId, cardSecStateChange=cardSecStateChange, auditDescription=auditDescription, cardSpareGroupId=cardSpareGroupId, portMode=portMode, equipmentError=equipmentError, cardCPLDVersion=cardCPLDVersion, diskVolumeSize=diskVolumeSize, cmUChannelID=cmUChannelID, errEvLogLevel=errEvLogLevel, diskVolumeId=diskVolumeId, auditCardId=auditCardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSpareGroupMode=cardSpareGroupMode, equipmentState=equipmentState, auditShelfId=auditShelfId, errEventId=errEventId, cardLastBootVersion=cardLastBootVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentTraps=equipmentTraps, portMacAddress=portMacAddress, diskVolumeName=diskVolumeName, cardTestId=cardTestId, portType=portType, portAction=portAction, portDescription=portDescription, cardSubType=cardSubType, auditCycleCount=auditCycleCount, portConnectorId=portConnectorId, portPrState=portPrState, portRxFlowControlMode=portRxFlowControlMode, diskVolumeTable=diskVolumeTable, auditEntry=auditEntry, cardTestName=cardTestName, TestTransactionId=TestTransactionId, cardTestCommand=cardTestCommand, cardTestTime=cardTestTime, portCardId=portCardId, portBgpId=portBgpId, consoleOutput=consoleOutput, cmResetReason=cmResetReason, auditAutoScheduling=auditAutoScheduling, diskVolumeGroup=diskVolumeGroup, auditPassedCount=auditPassedCount, cardTrapInh=cardTrapInh, cmVendor=cmVendor, systemGeneral=systemGeneral, cardName=cardName, cardEntry=cardEntry, auditStatus=auditStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardBootVersion=cardBootVersion, cardCarrierFwVersion=cardCarrierFwVersion, portDplxStatus=portDplxStatus, portMacIfIndex=portMacIfIndex, auditTable=auditTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardUpTime=cardUpTime, equipmentCardGroup=equipmentCardGroup, cardFlashSize=cardFlashSize, cardPicSerialNum=cardPicSerialNum, cardTemperatureHighError=cardTemperatureHighError, cardTempNoReportNotification=cardTempNoReportNotification, cardRamSize=cardRamSize, cardNumPorts=cardNumPorts, cardShelfTestIndex=cardShelfTestIndex, auditFailedCount=auditFailedCount, cardDplxStatus=cardDplxStatus)
