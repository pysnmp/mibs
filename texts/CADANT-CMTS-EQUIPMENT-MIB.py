...
PortDetectedMode, CardType, FirmwareSource, PortType, DuplexStatus, SecondaryState, CardId, PicType, FlowControlMode, ShelfId, CardSubType, DiskVolumeUsageLevel, PrimaryState, AdminState, PortMode, EqActionType, UnknownState, PortId = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "CardType", "FirmwareSource", "PortType", "DuplexStatus", "SecondaryState", "CardId", "PicType", "FlowControlMode", "ShelfId", "CardSubType", "DiskVolumeUsageLevel", "PrimaryState", "AdminState", "PortMode", "EqActionType", "UnknownState", "PortId")
...
MacAddress, RowStatus, TimeStamp, DateAndTime, TextualConvention, TruthValue, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "RowStatus", "TimeStamp", "DateAndTime", "TextualConvention", "TruthValue", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTable=portTable, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeTable=diskVolumeTable, cardTrapInh=cardTrapInh, equipmentTbl=equipmentTbl, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeTrapInh=diskVolumeTrapInh, cardLastBootSource=cardLastBootSource, auditTime=auditTime, cardTestIndex=cardTestIndex, diskVolumeName=diskVolumeName, TestScheduleCommand=TestScheduleCommand, portTrapInh=portTrapInh, auditAutoScheduling=auditAutoScheduling, cmUChannelID=cmUChannelID, fpgaErrorEventEntry=fpgaErrorEventEntry, cardAction=cardAction, auditName=auditName, errEvRecoveryEnabled=errEvRecoveryEnabled, portDplxStatus=portDplxStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentCardTestGroup=equipmentCardTestGroup, cardCarrierFwVersion=cardCarrierFwVersion, auditPassedCount=auditPassedCount, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardFwUpdateStatus=cardFwUpdateStatus, auditShelfId=auditShelfId, cardNumPorts=cardNumPorts, cardCarrierSerialNum=cardCarrierSerialNum, cardDplxStatusChange=cardDplxStatusChange, equipmentDiag=equipmentDiag, consoleOutput=consoleOutput, diskVolumeShelfId=diskVolumeShelfId, cmResetClearNotification=cmResetClearNotification, diskVolumeDiskSize=diskVolumeDiskSize, cardCarrierHwVersion=cardCarrierHwVersion, auditEntry=auditEntry, portEntry=portEntry, cardId=cardId, auditCardId=auditCardId, portSecState=portSecState, portDocsIfIndex=portDocsIfIndex, portNumChans=portNumChans, auditLogOutput=auditLogOutput, cardTempNoReportNotification=cardTempNoReportNotification, cardTestScheduleCommand=cardTestScheduleCommand, portMode=portMode, cardTestSchedule=cardTestSchedule, portNumber=portNumber, auditId=auditId, cardSubType=cardSubType, cmResetReason=cmResetReason, cardPrState=cardPrState, TestType=TestType, shelfNumber=shelfNumber, cardFpgaSource=cardFpgaSource, cardTestTime=cardTestTime, TestId=TestId, portShelfId=portShelfId, remainInDiagMode=remainInDiagMode, cardEntry=cardEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, cardLastChangeTime=cardLastChangeTime, cmVendor=cmVendor, cardRamSize=cardRamSize, portRxFlowControlMode=portRxFlowControlMode, cmResetStatus=cmResetStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, systemGeneral=systemGeneral, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCpuType=cardCpuType, cardName=cardName, equipmentStateGroup=equipmentStateGroup, diskVolumeDriveId=diskVolumeDriveId, cardPrStateChange=cardPrStateChange, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMib=cadEquipmentMib, diskVolumeUsageLevel=diskVolumeUsageLevel, auditTotalFailedCount=auditTotalFailedCount, equipmentTraps=equipmentTraps, cardHwVersion=cardHwVersion, equipmentDiagGroup=equipmentDiagGroup, cardTestTable=cardTestTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeFileName=diskVolumeFileName, auditTotalPassedCount=auditTotalPassedCount, cardSubDetected=cardSubDetected, cardAdminState=cardAdminState, cmResetUpTime=cmResetUpTime, errEvRowStatus=errEvRowStatus, diskVolumeCardId=diskVolumeCardId, shelfSwVersion=shelfSwVersion, auditTable=auditTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemGeneralGroup=systemGeneralGroup, portAdminState=portAdminState, cardTestName=cardTestName, cardTemperatureHighError=cardTemperatureHighError, cardTestId=cardTestId, portId=portId, cmDevice=cmDevice, cardCpuSpeed=cardCpuSpeed, diskVolumeNumber=diskVolumeNumber, equipmentPortGroup=equipmentPortGroup, equipmentState=equipmentState, TestCommand=TestCommand, portSecStateChange=portSecStateChange, diskVolumeId=diskVolumeId, portType=portType, portDplxStatusChange=portDplxStatusChange, diagTestId=diagTestId, cmRegistrationNotification=cmRegistrationNotification, auditLogThrottle=auditLogThrottle, fpgaErrorEventTable=fpgaErrorEventTable, portDetectedMode=portDetectedMode, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMibCompliance=cadEquipmentMibCompliance, shelfObjects=shelfObjects, fpgaErrorEventGroup=fpgaErrorEventGroup, portMaxDsPower=portMaxDsPower, cardPicSerialNum=cardPicSerialNum, portMacIfIndex=portMacIfIndex, portPrState=portPrState, PYSNMP_MODULE_ID=cadEquipmentMib, TestResult=TestResult, cardBootVersion=cardBootVersion, cardDplxStatus=cardDplxStatus, cardTestEntry=cardTestEntry, cardTestTransId=cardTestTransId, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeSize=diskVolumeSize, cardTestResult=cardTestResult, auditCommand=auditCommand, cmResetNotification=cmResetNotification, cardFwVersion=cardFwVersion, trapCounter=trapCounter, TestSchedule=TestSchedule, equipmentCardGroup=equipmentCardGroup, cardTestResultDesc=cardTestResultDesc, cardLastResetReason=cardLastResetReason, portCardSubType=portCardSubType, cardShelfId=cardShelfId, portMacAddress=portMacAddress, portPrStateChange=portPrStateChange, auditCycleCount=auditCycleCount, cardTestResultNotification=cardTestResultNotification, errEvLogLevel=errEvLogLevel, cardTestCommand=cardTestCommand, cardCPLDVersion=cardCPLDVersion, auditStatus=auditStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, auditFailedCount=auditFailedCount, cardSwVersion=cardSwVersion, trapSeverity=trapSeverity, TestTransactionId=TestTransactionId, portConnectorId=portConnectorId, cmPrimarySID=cmPrimarySID, cardDetected=cardDetected, cmIpAddress=cmIpAddress, auditDescription=auditDescription, cardTestDescription=cardTestDescription, shelfName=shelfName, verboseLevel=verboseLevel, cmMacAddress=cmMacAddress, cmResetInfo=cmResetInfo, cardTable=cardTable, cardDetectedChange=cardDetectedChange, cardSerialNum=cardSerialNum, cardSecState=cardSecState, cardUpTime=cardUpTime, systemClock=systemClock, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSpareGroupMode=cardSpareGroupMode, portCurrDsPower=portCurrDsPower, cardPicDetected=cardPicDetected, cardBusSpeed=cardBusSpeed, portDescription=portDescription, auditResult=auditResult, cardType=cardType, equipmentAudit=equipmentAudit, cardLastBootVersion=cardLastBootVersion, cardShelfTestIndex=cardShelfTestIndex, diskVolumeEntry=diskVolumeEntry, diskDriveNumber=diskDriveNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portAction=portAction, portBgpId=portBgpId, portLastChangeTime=portLastChangeTime, diskVolumeGroup=diskVolumeGroup, eqDiagConfig=eqDiagConfig, cardNumber=cardNumber, cardTestType=cardTestType, cardFlashSize=cardFlashSize, cadEquipmentMibConformance=cadEquipmentMibConformance, systemKey=systemKey, equipmentError=equipmentError, portMinDsPower=portMinDsPower, cardSecStateChange=cardSecStateChange, errEventId=errEventId, equipmentAuditGroup=equipmentAuditGroup, equipmentShelfGroup=equipmentShelfGroup, cardSpareGroupId=cardSpareGroupId, cardTemperature=cardTemperature, cardPicHwVersion=cardPicHwVersion, portCardId=portCardId)
