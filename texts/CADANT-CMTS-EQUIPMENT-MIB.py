...
DuplexStatus, PortMode, CardSubType, DiskVolumeUsageLevel, PortType, EqActionType, FirmwareSource, ShelfId, AdminState, PortId, FlowControlMode, PrimaryState, UnknownState, PicType, CardType, PortDetectedMode, CardId, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortMode", "CardSubType", "DiskVolumeUsageLevel", "PortType", "EqActionType", "FirmwareSource", "ShelfId", "AdminState", "PortId", "FlowControlMode", "PrimaryState", "UnknownState", "PicType", "CardType", "PortDetectedMode", "CardId", "SecondaryState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskDriveNumber=diskDriveNumber, auditStatus=auditStatus, cardTemperature=cardTemperature, cardSecState=cardSecState, equipmentTraps=equipmentTraps, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecState=portSecState, diskVolumeFileName=diskVolumeFileName, diagTestId=diagTestId, portTxFlowControlMode=portTxFlowControlMode, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentError=equipmentError, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageNotification=diskVolumeUsageNotification, auditEntry=auditEntry, cardPrStateChange=cardPrStateChange, auditCycleCount=auditCycleCount, portNumChans=portNumChans, systemGeneralGroup=systemGeneralGroup, cardTestCommand=cardTestCommand, cardLastResetReason=cardLastResetReason, auditResult=auditResult, cardTestTransId=cardTestTransId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfNumber=shelfNumber, equipmentTbl=equipmentTbl, auditTime=auditTime, cardDetected=cardDetected, shelfObjects=shelfObjects, portSecStateChange=portSecStateChange, cardSubType=cardSubType, cmResetNotification=cmResetNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeCardId=diskVolumeCardId, cardTestResultNotification=cardTestResultNotification, cardFwUpdateStatus=cardFwUpdateStatus, cardDetectedChange=cardDetectedChange, equipmentPortGroup=equipmentPortGroup, cardRamSize=cardRamSize, fpgaErrorEventTable=fpgaErrorEventTable, cardHwVersion=cardHwVersion, cardTestResult=cardTestResult, cmRegistrationNotification=cmRegistrationNotification, systemKey=systemKey, diskVolumeDriveId=diskVolumeDriveId, cardTestSchedule=cardTestSchedule, auditTable=auditTable, cmVendor=cmVendor, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditTotalFailedCount=auditTotalFailedCount, cmUChannelID=cmUChannelID, cmResetClearNotification=cmResetClearNotification, cardCarrierHwVersion=cardCarrierHwVersion, portType=portType, auditLogOutput=auditLogOutput, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeEntry=diskVolumeEntry, cmMacAddress=cmMacAddress, portMinDsPower=portMinDsPower, diskVolumeGroup=diskVolumeGroup, cardFwVersion=cardFwVersion, cardCPLDVersion=cardCPLDVersion, diskVolumeTrapInh=diskVolumeTrapInh, cardDplxStatus=cardDplxStatus, cardSerialNum=cardSerialNum, cardSpareGroupId=cardSpareGroupId, diskVolumeShelfId=diskVolumeShelfId, remainInDiagMode=remainInDiagMode, eqDiagConfig=eqDiagConfig, consoleOutput=consoleOutput, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicSerialNum=cardPicSerialNum, portCardSubType=portCardSubType, diskVolumeId=diskVolumeId, diskVolumeDiskSize=diskVolumeDiskSize, cmResetStatus=cmResetStatus, equipmentCardGroup=equipmentCardGroup, cardPicDetected=cardPicDetected, portTrapInh=portTrapInh, cardTestName=cardTestName, cardUpTime=cardUpTime, errEvLoggingEnabled=errEvLoggingEnabled, portMode=portMode, fpgaErrorEventGroup=fpgaErrorEventGroup, auditFailedCount=auditFailedCount, portCardId=portCardId, portTable=portTable, portLastChangeTime=portLastChangeTime, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cadEquipmentMibConformance=cadEquipmentMibConformance, portDetectedMode=portDetectedMode, cardEntry=cardEntry, equipmentDiagGroup=equipmentDiagGroup, errEvRowStatus=errEvRowStatus, TestType=TestType, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentState=equipmentState, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portMaxDsPower=portMaxDsPower, portBgpId=portBgpId, cardBootVersion=cardBootVersion, errEvLogLevel=errEvLogLevel, portDocsIfIndex=portDocsIfIndex, cmDevice=cmDevice, portMacIfIndex=portMacIfIndex, cardTestDescription=cardTestDescription, PYSNMP_MODULE_ID=cadEquipmentMib, errEvRecoveryEnabled=errEvRecoveryEnabled, portShelfId=portShelfId, cardTrapInh=cardTrapInh, diskVolumeSize=diskVolumeSize, portAction=portAction, cardTestId=cardTestId, portTxFlowControlDetected=portTxFlowControlDetected, auditShelfId=auditShelfId, cardAction=cardAction, downstreamPowerLoss=downstreamPowerLoss, cardId=cardId, equipmentAuditGroup=equipmentAuditGroup, equipmentAudit=equipmentAudit, auditAutoScheduling=auditAutoScheduling, diskVolumeNumber=diskVolumeNumber, shelfSwVersion=shelfSwVersion, portDescription=portDescription, auditTotalPassedCount=auditTotalPassedCount, cmIpAddress=cmIpAddress, portPrState=portPrState, diskVolumeName=diskVolumeName, cardTestScheduleCommand=cardTestScheduleCommand, cmPrimarySID=cmPrimarySID, TestId=TestId, systemClock=systemClock, cardTestEntry=cardTestEntry, equipmentShelfGroup=equipmentShelfGroup, cardName=cardName, cardBusSpeed=cardBusSpeed, cardLastBootSource=cardLastBootSource, cardTestTime=cardTestTime, cardTable=cardTable, cardTestIndex=cardTestIndex, auditCommand=auditCommand, portId=portId, cardCpuSpeed=cardCpuSpeed, cardAdminState=cardAdminState, diskVolumeUsageLevel=diskVolumeUsageLevel, portConnectorId=portConnectorId, portPrStateChange=portPrStateChange, portMacAddress=portMacAddress, TestResult=TestResult, cardSecStateChange=cardSecStateChange, portDplxStatusChange=portDplxStatusChange, cadEquipmentMib=cadEquipmentMib, cardFlashSize=cardFlashSize, portEntry=portEntry, cardSwVersion=cardSwVersion, cardTemperatureHighError=cardTemperatureHighError, cardPrState=cardPrState, auditPassedCount=auditPassedCount, equipmentDiag=equipmentDiag, portAdminState=portAdminState, auditCardId=auditCardId, TestTransactionId=TestTransactionId, cardLastBootVersion=cardLastBootVersion, cardTempNoReportNotification=cardTempNoReportNotification, systemGeneral=systemGeneral, TestCommand=TestCommand, portRxFlowControlMode=portRxFlowControlMode, cardTestTable=cardTestTable, cmResetUpTime=cmResetUpTime, auditId=auditId, trapSeverity=trapSeverity, cardType=cardType, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCarrierSerialNum=cardCarrierSerialNum, TestSchedule=TestSchedule, portNumber=portNumber, cmResetInfo=cmResetInfo, cardTempOverHeatNotification=cardTempOverHeatNotification, cardDplxStatusChange=cardDplxStatusChange, cardCpuType=cardCpuType, portDplxStatus=portDplxStatus, portCurrDsPower=portCurrDsPower, cardNumPorts=cardNumPorts, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, errEventId=errEventId, cardCarrierFwVersion=cardCarrierFwVersion, auditName=auditName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestType=cardTestType, shelfName=shelfName, cardSubDetected=cardSubDetected, TestScheduleCommand=TestScheduleCommand, cardShelfId=cardShelfId, cmResetReason=cmResetReason, cardSpareGroupMode=cardSpareGroupMode, cardPicHwVersion=cardPicHwVersion, equipmentCardTestGroup=equipmentCardTestGroup, cardShelfTestIndex=cardShelfTestIndex, diskVolumeTable=diskVolumeTable, trapCounter=trapCounter, cardFpgaSource=cardFpgaSource, auditDescription=auditDescription, verboseLevel=verboseLevel, auditLogThrottle=auditLogThrottle, equipmentStateGroup=equipmentStateGroup, cardNumber=cardNumber)
