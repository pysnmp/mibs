...
DuplexStatus, CardSubType, CardType, UnknownState, FirmwareSource, DiskVolumeUsageLevel, SecondaryState, AdminState, EqActionType, PicType, FlowControlMode, PortMode, PortType, CardId, PortDetectedMode, PortId, PrimaryState, ShelfId = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "CardSubType", "CardType", "UnknownState", "FirmwareSource", "DiskVolumeUsageLevel", "SecondaryState", "AdminState", "EqActionType", "PicType", "FlowControlMode", "PortMode", "PortType", "CardId", "PortDetectedMode", "PortId", "PrimaryState", "ShelfId")
...
RowStatus, TextualConvention, MacAddress, DisplayString, TruthValue, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "MacAddress", "DisplayString", "TruthValue", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfSwVersion=shelfSwVersion, cardCpuType=cardCpuType, portPrState=portPrState, diskVolumeEntry=diskVolumeEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPicSerialNum=cardPicSerialNum, TestCommand=TestCommand, PYSNMP_MODULE_ID=cadEquipmentMib, cardShelfId=cardShelfId, equipmentCardGroup=equipmentCardGroup, errEvLoggingEnabled=errEvLoggingEnabled, equipmentError=equipmentError, cardCarrierSerialNum=cardCarrierSerialNum, portSecStateChange=portSecStateChange, cardLastChangeTime=cardLastChangeTime, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeGroup=diskVolumeGroup, diskVolumeFileName=diskVolumeFileName, cardDplxStatusChange=cardDplxStatusChange, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmUChannelID=cmUChannelID, equipmentDiag=equipmentDiag, auditAutoScheduling=auditAutoScheduling, cardTemperatureHighError=cardTemperatureHighError, portTable=portTable, cardType=cardType, cardPicHwVersion=cardPicHwVersion, errEvRowStatus=errEvRowStatus, auditTime=auditTime, auditStatus=auditStatus, cardDetected=cardDetected, cardNumber=cardNumber, portNumChans=portNumChans, cardTestEntry=cardTestEntry, portNumber=portNumber, cardEntry=cardEntry, trapSeverity=trapSeverity, cardFpgaSource=cardFpgaSource, cardDplxStatus=cardDplxStatus, cmDevice=cmDevice, cardSerialNum=cardSerialNum, cardId=cardId, cardTestDescription=cardTestDescription, cardSubDetected=cardSubDetected, diagTestId=diagTestId, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeSize=diskVolumeSize, cardName=cardName, portTxFlowControlMode=portTxFlowControlMode, cardBootVersion=cardBootVersion, cardCPLDVersion=cardCPLDVersion, auditShelfId=auditShelfId, auditCardId=auditCardId, cardTestIndex=cardTestIndex, portDetectedMode=portDetectedMode, auditResult=auditResult, diskVolumeNumber=diskVolumeNumber, portCardSubType=portCardSubType, portDescription=portDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestName=cardTestName, shelfNumber=shelfNumber, diskVolumeShelfId=diskVolumeShelfId, cardShelfTestIndex=cardShelfTestIndex, cardTemperature=cardTemperature, diskVolumeUsageLevel=diskVolumeUsageLevel, portMaxDsPower=portMaxDsPower, cardBusSpeed=cardBusSpeed, cmVendor=cmVendor, cardTestResultDesc=cardTestResultDesc, remainInDiagMode=remainInDiagMode, equipmentAuditGroup=equipmentAuditGroup, cardTestId=cardTestId, errEvRecoveryEnabled=errEvRecoveryEnabled, fpgaErrorEventTable=fpgaErrorEventTable, cardTestType=cardTestType, shelfName=shelfName, auditPassedCount=auditPassedCount, systemGeneral=systemGeneral, equipmentShelfGroup=equipmentShelfGroup, shelfObjects=shelfObjects, cardTestTable=cardTestTable, portConnectorId=portConnectorId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, downstreamPowerLoss=downstreamPowerLoss, diskVolumeTable=diskVolumeTable, auditLogOutput=auditLogOutput, TestTransactionId=TestTransactionId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastBootSource=cardLastBootSource, eqDiagConfig=eqDiagConfig, cardTemperatureHighWarn=cardTemperatureHighWarn, portMinDsPower=portMinDsPower, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardLastBootVersion=cardLastBootVersion, verboseLevel=verboseLevel, errEvLogLevel=errEvLogLevel, portAction=portAction, auditDescription=auditDescription, TestResult=TestResult, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, consoleOutput=consoleOutput, auditCommand=auditCommand, cardRamSize=cardRamSize, portAdminState=portAdminState, cadEquipmentMib=cadEquipmentMib, systemClock=systemClock, portBgpId=portBgpId, auditCycleCount=auditCycleCount, TestSchedule=TestSchedule, TestType=TestType, diskVolumeId=diskVolumeId, cardPrState=cardPrState, diskVolumeDriveId=diskVolumeDriveId, cardSubType=cardSubType, portDocsIfIndex=portDocsIfIndex, cmResetClearNotification=cmResetClearNotification, cardTestSchedule=cardTestSchedule, cardDetectedChange=cardDetectedChange, portEntry=portEntry, equipmentState=equipmentState, cmResetReason=cmResetReason, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSwVersion=cardSwVersion, cardPrStateChange=cardPrStateChange, cardSecStateChange=cardSecStateChange, cardNumPorts=cardNumPorts, portDplxStatusChange=portDplxStatusChange, cardTempNoReportNotification=cardTempNoReportNotification, cardTestScheduleCommand=cardTestScheduleCommand, cardFwUpdateStatus=cardFwUpdateStatus, portCurrDsPower=portCurrDsPower, equipmentDiagGroup=equipmentDiagGroup, portSecState=portSecState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, errEventId=errEventId, cmResetStatus=cmResetStatus, cardTrapInh=cardTrapInh, auditName=auditName, cmResetNotification=cmResetNotification, cardCpuSpeed=cardCpuSpeed, cadEquipmentMibConformance=cadEquipmentMibConformance, cmRegistrationNotification=cmRegistrationNotification, cardSpareGroupId=cardSpareGroupId, cmIpAddress=cmIpAddress, diskVolumeUsageNotification=diskVolumeUsageNotification, portTrapInh=portTrapInh, cardSpareGroupMode=cardSpareGroupMode, systemKey=systemKey, cardAction=cardAction, cmResetInfo=cmResetInfo, cmPrimarySID=cmPrimarySID, equipmentAudit=equipmentAudit, equipmentCardTestGroup=equipmentCardTestGroup, cardFlashSize=cardFlashSize, cmResetUpTime=cmResetUpTime, cardFwVersion=cardFwVersion, auditId=auditId, cardAdminState=cardAdminState, cardTestResult=cardTestResult, equipmentStateGroup=equipmentStateGroup, portRxFlowControlMode=portRxFlowControlMode, systemGeneralGroup=systemGeneralGroup, trapCounter=trapCounter, portType=portType, diskVolumeCardId=diskVolumeCardId, cardCarrierHwVersion=cardCarrierHwVersion, cardTestTransId=cardTestTransId, auditTotalFailedCount=auditTotalFailedCount, diskDriveNumber=diskDriveNumber, cardSecState=cardSecState, portMacAddress=portMacAddress, cardPicDetected=cardPicDetected, cardUpTime=cardUpTime, diskVolumeName=diskVolumeName, auditLogThrottle=auditLogThrottle, portDplxStatus=portDplxStatus, portPrStateChange=portPrStateChange, cardTestResultNotification=cardTestResultNotification, portShelfId=portShelfId, auditTable=auditTable, cardCarrierFwVersion=cardCarrierFwVersion, equipmentTraps=equipmentTraps, cardHwVersion=cardHwVersion, cardTable=cardTable, diskVolumeDiskSize=diskVolumeDiskSize, equipmentPortGroup=equipmentPortGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardTempOverHeatNotification=cardTempOverHeatNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portMode=portMode, auditEntry=auditEntry, TestScheduleCommand=TestScheduleCommand, cardLastResetReason=cardLastResetReason, portLastChangeTime=portLastChangeTime, TestId=TestId, portId=portId, cardTestTime=cardTestTime, auditFailedCount=auditFailedCount, cmMacAddress=cmMacAddress, equipmentTbl=equipmentTbl, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestCommand=cardTestCommand, portMacIfIndex=portMacIfIndex, portCardId=portCardId)
