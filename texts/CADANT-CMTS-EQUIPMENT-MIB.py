...
DuplexStatus, AdminState, SecondaryState, PortId, ShelfId, PortMode, CardType, PortType, CardId, DiskVolumeUsageLevel, CardSubType, PicType, UnknownState, EqActionType, FirmwareSource, PrimaryState, FlowControlMode, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "AdminState", "SecondaryState", "PortId", "ShelfId", "PortMode", "CardType", "PortType", "CardId", "DiskVolumeUsageLevel", "CardSubType", "PicType", "UnknownState", "EqActionType", "FirmwareSource", "PrimaryState", "FlowControlMode", "PortDetectedMode")
...
DisplayString, TimeStamp, TextualConvention, DateAndTime, RowStatus, TruthValue, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "TextualConvention", "DateAndTime", "RowStatus", "TruthValue", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTrapInh=portTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, auditCommand=auditCommand, consoleOutput=consoleOutput, cmResetUpTime=cmResetUpTime, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMacAddress=portMacAddress, cardCpuType=cardCpuType, diskVolumeTrapInh=diskVolumeTrapInh, shelfNumber=shelfNumber, equipmentAuditGroup=equipmentAuditGroup, auditShelfId=auditShelfId, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, errEvLoggingEnabled=errEvLoggingEnabled, portRxFlowControlDetected=portRxFlowControlDetected, portConnectorId=portConnectorId, cardPrStateChange=cardPrStateChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditCycleCount=auditCycleCount, cardTestResult=cardTestResult, portMaxDsPower=portMaxDsPower, shelfSwVersion=shelfSwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, errEvLogLevel=errEvLogLevel, auditTotalFailedCount=auditTotalFailedCount, portMacIfIndex=portMacIfIndex, cardCarrierSerialNum=cardCarrierSerialNum, trapSeverity=trapSeverity, auditLogThrottle=auditLogThrottle, diskVolumeTable=diskVolumeTable, cardSpareGroupMode=cardSpareGroupMode, portCardSubType=portCardSubType, portNumChans=portNumChans, equipmentTbl=equipmentTbl, equipmentState=equipmentState, systemKey=systemKey, portType=portType, auditFailedCount=auditFailedCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestEntry=cardTestEntry, cardPrState=cardPrState, cardTestTable=cardTestTable, diskVolumeShelfId=diskVolumeShelfId, portSecState=portSecState, equipmentPortGroup=equipmentPortGroup, cmResetClearNotification=cmResetClearNotification, cardSwVersion=cardSwVersion, eqDiagConfig=eqDiagConfig, cardBusSpeed=cardBusSpeed, diskVolumeNumber=diskVolumeNumber, TestResult=TestResult, cardSecState=cardSecState, cardDplxStatus=cardDplxStatus, cardTestScheduleCommand=cardTestScheduleCommand, downstreamPowerLoss=downstreamPowerLoss, TestCommand=TestCommand, cardDetected=cardDetected, cardPicSerialNum=cardPicSerialNum, cardTestDescription=cardTestDescription, TestSchedule=TestSchedule, portNumber=portNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, cardUpTime=cardUpTime, portCardId=portCardId, cardTestId=cardTestId, cmResetInfo=cmResetInfo, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardDplxStatusChange=cardDplxStatusChange, cmResetNotification=cmResetNotification, cardAdminState=cardAdminState, cardNumber=cardNumber, portDescription=portDescription, cardTestIndex=cardTestIndex, portPrState=portPrState, shelfName=shelfName, auditName=auditName, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestScheduleCommand=TestScheduleCommand, cardTestResultDesc=cardTestResultDesc, portDetectedMode=portDetectedMode, cardTestSchedule=cardTestSchedule, PYSNMP_MODULE_ID=cadEquipmentMib, cardTable=cardTable, cardTemperature=cardTemperature, portPrStateChange=portPrStateChange, equipmentTraps=equipmentTraps, cardSecStateChange=cardSecStateChange, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeSize=diskVolumeSize, equipmentCardTestGroup=equipmentCardTestGroup, cardType=cardType, cardBootVersion=cardBootVersion, cardId=cardId, portAction=portAction, cardName=cardName, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmResetReason=cmResetReason, auditCardId=auditCardId, cardTempNoReportNotification=cardTempNoReportNotification, portBgpId=portBgpId, TestType=TestType, cardEntry=cardEntry, cardSerialNum=cardSerialNum, cardFpgaSource=cardFpgaSource, cardLastBootVersion=cardLastBootVersion, auditLogOutput=auditLogOutput, equipmentAudit=equipmentAudit, equipmentDiag=equipmentDiag, cardNumPorts=cardNumPorts, errEvRowStatus=errEvRowStatus, portDplxStatus=portDplxStatus, diskVolumeGroup=diskVolumeGroup, diskVolumeFileName=diskVolumeFileName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmVendor=cmVendor, equipmentShelfGroup=equipmentShelfGroup, TestTransactionId=TestTransactionId, cmUChannelID=cmUChannelID, auditResult=auditResult, portSecStateChange=portSecStateChange, cmPrimarySID=cmPrimarySID, systemClock=systemClock, verboseLevel=verboseLevel, cmMacAddress=cmMacAddress, cardLastChangeTime=cardLastChangeTime, errEventId=errEventId, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResultNotification=cardTestResultNotification, cardAction=cardAction, portMinDsPower=portMinDsPower, cardTestTime=cardTestTime, cmIpAddress=cmIpAddress, cardFwUpdateStatus=cardFwUpdateStatus, cardLastBootSource=cardLastBootSource, trapCounter=trapCounter, portLastChangeTime=portLastChangeTime, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageNotification=diskVolumeUsageNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardFwVersion=cardFwVersion, diskDriveNumber=diskDriveNumber, diskVolumeId=diskVolumeId, diskVolumeCardId=diskVolumeCardId, portTable=portTable, cardHwVersion=cardHwVersion, cardTestTransId=cardTestTransId, cardCPLDVersion=cardCPLDVersion, cardSubDetected=cardSubDetected, cardRamSize=cardRamSize, equipmentCardGroup=equipmentCardGroup, cardShelfTestIndex=cardShelfTestIndex, cardSubType=cardSubType, portShelfId=portShelfId, diskVolumeUsageLevel=diskVolumeUsageLevel, auditAutoScheduling=auditAutoScheduling, auditTime=auditTime, errEvRecoveryEnabled=errEvRecoveryEnabled, systemGeneral=systemGeneral, cardTrapInh=cardTrapInh, cardTemperatureHighError=cardTemperatureHighError, auditTable=auditTable, cardCarrierHwVersion=cardCarrierHwVersion, auditEntry=auditEntry, cardLastResetReason=cardLastResetReason, portMode=portMode, diskVolumeDriveId=diskVolumeDriveId, cardCarrierFwVersion=cardCarrierFwVersion, cardSpareGroupId=cardSpareGroupId, auditPassedCount=auditPassedCount, cardTestType=cardTestType, portEntry=portEntry, cmResetStatus=cmResetStatus, cmDevice=cmDevice, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentDiagGroup=equipmentDiagGroup, auditStatus=auditStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diagTestId=diagTestId, cardPicHwVersion=cardPicHwVersion, cardShelfId=cardShelfId, portId=portId, cardPicDetected=cardPicDetected, cadEquipmentMib=cadEquipmentMib, portDplxStatusChange=portDplxStatusChange, diskVolumeEntry=diskVolumeEntry, remainInDiagMode=remainInDiagMode, portCurrDsPower=portCurrDsPower, auditDescription=auditDescription, cardTestCommand=cardTestCommand, cmRegistrationNotification=cmRegistrationNotification, TestId=TestId, cardFlashSize=cardFlashSize, equipmentStateGroup=equipmentStateGroup, portTxFlowControlMode=portTxFlowControlMode, cardCpuSpeed=cardCpuSpeed, auditId=auditId, equipmentError=equipmentError, portAdminState=portAdminState, portDocsIfIndex=portDocsIfIndex, portRxFlowControlMode=portRxFlowControlMode, shelfObjects=shelfObjects, cardDetectedChange=cardDetectedChange, systemGeneralGroup=systemGeneralGroup, cardTestName=cardTestName, diskVolumeName=diskVolumeName)
