...
PicType, PortMode, FirmwareSource, EqActionType, DiskVolumeUsageLevel, CardId, PortId, ShelfId, FlowControlMode, PrimaryState, SecondaryState, CardSubType, AdminState, DuplexStatus, PortType, UnknownState, PortDetectedMode, CardType = mibBuilder.importSymbols("CADANT-TC", "PicType", "PortMode", "FirmwareSource", "EqActionType", "DiskVolumeUsageLevel", "CardId", "PortId", "ShelfId", "FlowControlMode", "PrimaryState", "SecondaryState", "CardSubType", "AdminState", "DuplexStatus", "PortType", "UnknownState", "PortDetectedMode", "CardType")
...
MacAddress, TruthValue, DateAndTime, TimeStamp, DisplayString, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "DateAndTime", "TimeStamp", "DisplayString", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastChangeTime=cardLastChangeTime, portId=portId, cmResetReason=cmResetReason, cmResetStatus=cmResetStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, auditFailedCount=auditFailedCount, errEvRowStatus=errEvRowStatus, diagTestId=diagTestId, cardShelfId=cardShelfId, TestResult=TestResult, portMacIfIndex=portMacIfIndex, auditTable=auditTable, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentCardGroup=equipmentCardGroup, portMode=portMode, portNumChans=portNumChans, auditLogThrottle=auditLogThrottle, cardSpareGroupId=cardSpareGroupId, equipmentDiag=equipmentDiag, cardCpuSpeed=cardCpuSpeed, cardTestScheduleCommand=cardTestScheduleCommand, cardTestDescription=cardTestDescription, equipmentCardTestGroup=equipmentCardTestGroup, equipmentDiagGroup=equipmentDiagGroup, cardCarrierHwVersion=cardCarrierHwVersion, cardPicDetected=cardPicDetected, cardDetectedChange=cardDetectedChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, shelfObjects=shelfObjects, cmResetInfo=cmResetInfo, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentTraps=equipmentTraps, diskVolumeFileName=diskVolumeFileName, diskVolumeEntry=diskVolumeEntry, diskVolumeNumber=diskVolumeNumber, shelfName=shelfName, portCurrDsPower=portCurrDsPower, errEventId=errEventId, cardDplxStatusChange=cardDplxStatusChange, auditCycleCount=auditCycleCount, portTxFlowControlDetected=portTxFlowControlDetected, cardAction=cardAction, portMinDsPower=portMinDsPower, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeId=diskVolumeId, cardTestTransId=cardTestTransId, auditPassedCount=auditPassedCount, cardTestType=cardTestType, portDocsIfIndex=portDocsIfIndex, cardCpuType=cardCpuType, cardTable=cardTable, auditName=auditName, cardTestCommand=cardTestCommand, cardShelfTestIndex=cardShelfTestIndex, diskVolumeName=diskVolumeName, portMacAddress=portMacAddress, diskVolumeDriveId=diskVolumeDriveId, cmResetUpTime=cmResetUpTime, portEntry=portEntry, shelfSwVersion=shelfSwVersion, cardTemperature=cardTemperature, portType=portType, cardBootVersion=cardBootVersion, cardCarrierSerialNum=cardCarrierSerialNum, cardBusSpeed=cardBusSpeed, equipmentAuditGroup=equipmentAuditGroup, errEvLogLevel=errEvLogLevel, cardSecState=cardSecState, portAction=portAction, portSecState=portSecState, cmResetClearNotification=cmResetClearNotification, portSecStateChange=portSecStateChange, cardTestResultDesc=cardTestResultDesc, portShelfId=portShelfId, cardName=cardName, cmPrimarySID=cmPrimarySID, auditShelfId=auditShelfId, portDplxStatusChange=portDplxStatusChange, cardSerialNum=cardSerialNum, cardTemperatureHighError=cardTemperatureHighError, portDescription=portDescription, cardSecStateChange=cardSecStateChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestCommand=TestCommand, TestId=TestId, equipmentStateGroup=equipmentStateGroup, cardDetected=cardDetected, cardPicHwVersion=cardPicHwVersion, portTable=portTable, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCommand=auditCommand, portTrapInh=portTrapInh, fpgaErrorEventEntry=fpgaErrorEventEntry, portConnectorId=portConnectorId, cardTestTime=cardTestTime, cardTestResult=cardTestResult, cardHwVersion=cardHwVersion, cardTestName=cardTestName, portBgpId=portBgpId, diskVolumeCardId=diskVolumeCardId, cardDplxStatus=cardDplxStatus, cmResetNotification=cmResetNotification, diskVolumeShelfId=diskVolumeShelfId, remainInDiagMode=remainInDiagMode, systemGeneral=systemGeneral, cardLastBootVersion=cardLastBootVersion, cardSubType=cardSubType, auditDescription=auditDescription, cardSwVersion=cardSwVersion, portPrStateChange=portPrStateChange, portCardId=portCardId, equipmentState=equipmentState, diskVolumeGroup=diskVolumeGroup, cardNumPorts=cardNumPorts, TestTransactionId=TestTransactionId, cardUpTime=cardUpTime, consoleOutput=consoleOutput, portAdminState=portAdminState, cardCarrierFwVersion=cardCarrierFwVersion, cardId=cardId, cardRamSize=cardRamSize, auditAutoScheduling=auditAutoScheduling, cardTempNoReportNotification=cardTempNoReportNotification, cmDevice=cmDevice, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditStatus=auditStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, fpgaErrorEventTable=fpgaErrorEventTable, errEvLoggingEnabled=errEvLoggingEnabled, cardNumber=cardNumber, cardLastResetReason=cardLastResetReason, auditCardId=auditCardId, portLastChangeTime=portLastChangeTime, cardEntry=cardEntry, TestScheduleCommand=TestScheduleCommand, portPrState=portPrState, equipmentError=equipmentError, systemGeneralGroup=systemGeneralGroup, cmRegistrationNotification=cmRegistrationNotification, eqDiagConfig=eqDiagConfig, trapCounter=trapCounter, cmUChannelID=cmUChannelID, auditLogOutput=auditLogOutput, systemClock=systemClock, diskVolumeUsageNotification=diskVolumeUsageNotification, portNumber=portNumber, errEvRecoveryEnabled=errEvRecoveryEnabled, systemKey=systemKey, equipmentShelfGroup=equipmentShelfGroup, portRxFlowControlMode=portRxFlowControlMode, cardPrStateChange=cardPrStateChange, cardTrapInh=cardTrapInh, cardType=cardType, cardTestEntry=cardTestEntry, cardTestId=cardTestId, portDplxStatus=portDplxStatus, cmMacAddress=cmMacAddress, cardSubDetected=cardSubDetected, auditEntry=auditEntry, cmIpAddress=cmIpAddress, portTxFlowControlMode=portTxFlowControlMode, cardFlashSize=cardFlashSize, portDetectedMode=portDetectedMode, cardTestTable=cardTestTable, cmVendor=cmVendor, trapSeverity=trapSeverity, downstreamPowerLoss=downstreamPowerLoss, TestSchedule=TestSchedule, equipmentTbl=equipmentTbl, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSpareGroupMode=cardSpareGroupMode, cardTestResultNotification=cardTestResultNotification, auditTotalPassedCount=auditTotalPassedCount, shelfNumber=shelfNumber, cardCPLDVersion=cardCPLDVersion, cardTestSchedule=cardTestSchedule, diskDriveNumber=diskDriveNumber, diskVolumeSize=diskVolumeSize, cadEquipmentMib=cadEquipmentMib, TestType=TestType, cardPrState=cardPrState, PYSNMP_MODULE_ID=cadEquipmentMib, portMaxDsPower=portMaxDsPower, cardAdminState=cardAdminState, cardTemperatureHighWarn=cardTemperatureHighWarn, auditTotalFailedCount=auditTotalFailedCount, verboseLevel=verboseLevel, auditResult=auditResult, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibConformance=cadEquipmentMibConformance, auditTime=auditTime, equipmentAudit=equipmentAudit, cardLastBootSource=cardLastBootSource, diskVolumeTable=diskVolumeTable, equipmentPortGroup=equipmentPortGroup, portRxFlowControlDetected=portRxFlowControlDetected, auditId=auditId, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeDiskSize=diskVolumeDiskSize, cardFwVersion=cardFwVersion, portCardSubType=portCardSubType, cardTestIndex=cardTestIndex, cardFpgaSource=cardFpgaSource, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardFwUpdateStatus=cardFwUpdateStatus)
