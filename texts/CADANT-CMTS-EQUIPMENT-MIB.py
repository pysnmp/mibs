...
SecondaryState, CardType, FlowControlMode, ShelfId, PortMode, AdminState, CardSubType, CardId, UnknownState, PortDetectedMode, DuplexStatus, PortType, EqActionType, PrimaryState, DiskVolumeUsageLevel, FirmwareSource, PortId, PicType = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "CardType", "FlowControlMode", "ShelfId", "PortMode", "AdminState", "CardSubType", "CardId", "UnknownState", "PortDetectedMode", "DuplexStatus", "PortType", "EqActionType", "PrimaryState", "DiskVolumeUsageLevel", "FirmwareSource", "PortId", "PicType")
...
MacAddress, TruthValue, RowStatus, TimeStamp, DisplayString, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TruthValue", "RowStatus", "TimeStamp", "DisplayString", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTime=auditTime, auditDescription=auditDescription, cardPrStateChange=cardPrStateChange, cardEntry=cardEntry, TestTransactionId=TestTransactionId, verboseLevel=verboseLevel, cmVendor=cmVendor, cardNumber=cardNumber, diskVolumeNumber=diskVolumeNumber, portTable=portTable, TestSchedule=TestSchedule, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSecStateChange=cardSecStateChange, equipmentDiagGroup=equipmentDiagGroup, equipmentAuditGroup=equipmentAuditGroup, diskVolumeEntry=diskVolumeEntry, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portTrapInh=portTrapInh, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmRegistrationNotification=cmRegistrationNotification, portDplxStatus=portDplxStatus, shelfObjects=shelfObjects, eqDiagConfig=eqDiagConfig, cardSpareGroupId=cardSpareGroupId, cardFlashSize=cardFlashSize, cmResetClearNotification=cmResetClearNotification, portRxFlowControlMode=portRxFlowControlMode, auditShelfId=auditShelfId, equipmentDiag=equipmentDiag, cmResetUpTime=cmResetUpTime, auditId=auditId, cardTestIndex=cardTestIndex, cmResetReason=cmResetReason, auditResult=auditResult, cardCarrierSerialNum=cardCarrierSerialNum, cardCpuSpeed=cardCpuSpeed, auditTable=auditTable, errEvLogLevel=errEvLogLevel, diskVolumeShelfId=diskVolumeShelfId, portConnectorId=portConnectorId, cmMacAddress=cmMacAddress, shelfNumber=shelfNumber, equipmentError=equipmentError, portAdminState=portAdminState, portDescription=portDescription, portCurrDsPower=portCurrDsPower, cardShelfTestIndex=cardShelfTestIndex, cmIpAddress=cmIpAddress, cardTestTable=cardTestTable, cardSerialNum=cardSerialNum, auditName=auditName, equipmentShelfGroup=equipmentShelfGroup, diskVolumeGroup=diskVolumeGroup, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeId=diskVolumeId, cardId=cardId, systemGeneralGroup=systemGeneralGroup, cmPrimarySID=cmPrimarySID, cardLastChangeTime=cardLastChangeTime, TestType=TestType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeFileName=diskVolumeFileName, cardTestTime=cardTestTime, portId=portId, portPrState=portPrState, cardAction=cardAction, diskVolumeName=diskVolumeName, cardTestId=cardTestId, portTxFlowControlDetected=portTxFlowControlDetected, cardLastBootSource=cardLastBootSource, portMode=portMode, diskVolumeSize=diskVolumeSize, remainInDiagMode=remainInDiagMode, cardTempNoReportNotification=cardTempNoReportNotification, portPrStateChange=portPrStateChange, PYSNMP_MODULE_ID=cadEquipmentMib, cardBootVersion=cardBootVersion, cardLastBootVersion=cardLastBootVersion, shelfName=shelfName, auditLogOutput=auditLogOutput, trapCounter=trapCounter, cardSpareGroupMode=cardSpareGroupMode, cardCarrierFwVersion=cardCarrierFwVersion, TestId=TestId, cardDplxStatusChange=cardDplxStatusChange, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTable=cardTable, diskVolumeTable=diskVolumeTable, portLastChangeTime=portLastChangeTime, downstreamPowerLoss=downstreamPowerLoss, systemClock=systemClock, errEventId=errEventId, cardTemperature=cardTemperature, cmResetNotification=cmResetNotification, diskVolumeDriveId=diskVolumeDriveId, cardSubType=cardSubType, cardFwUpdateStatus=cardFwUpdateStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, portBgpId=portBgpId, cardTempOverHeatNotification=cardTempOverHeatNotification, portSecStateChange=portSecStateChange, diskVolumeDiskSize=diskVolumeDiskSize, fpgaErrorEventTable=fpgaErrorEventTable, cardDetected=cardDetected, portSecState=portSecState, cardPicSerialNum=cardPicSerialNum, portCardSubType=portCardSubType, auditEntry=auditEntry, portCardId=portCardId, cardTestScheduleCommand=cardTestScheduleCommand, cardTestResult=cardTestResult, auditFailedCount=auditFailedCount, portDocsIfIndex=portDocsIfIndex, equipmentState=equipmentState, equipmentTbl=equipmentTbl, cardCpuType=cardCpuType, cardTestResultNotification=cardTestResultNotification, cardTestName=cardTestName, equipmentAudit=equipmentAudit, diskVolumeCardId=diskVolumeCardId, cardDetectedChange=cardDetectedChange, cardTemperatureHighWarn=cardTemperatureHighWarn, portMinDsPower=portMinDsPower, portDplxStatusChange=portDplxStatusChange, cardHwVersion=cardHwVersion, cardBusSpeed=cardBusSpeed, auditPassedCount=auditPassedCount, equipmentCardGroup=equipmentCardGroup, systemGeneral=systemGeneral, auditAutoScheduling=auditAutoScheduling, portNumChans=portNumChans, cardPicHwVersion=cardPicHwVersion, cardDplxStatus=cardDplxStatus, consoleOutput=consoleOutput, cmUChannelID=cmUChannelID, TestScheduleCommand=TestScheduleCommand, equipmentStateGroup=equipmentStateGroup, cardPicDetected=cardPicDetected, cardTestType=cardTestType, cardCarrierHwVersion=cardCarrierHwVersion, equipmentPortGroup=equipmentPortGroup, portDetectedMode=portDetectedMode, TestCommand=TestCommand, cardTestSchedule=cardTestSchedule, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageLevel=diskVolumeUsageLevel, auditTotalPassedCount=auditTotalPassedCount, auditStatus=auditStatus, cardTestCommand=cardTestCommand, cardLastResetReason=cardLastResetReason, cardTestEntry=cardTestEntry, auditTotalFailedCount=auditTotalFailedCount, shelfSwVersion=shelfSwVersion, cardFwVersion=cardFwVersion, portMacAddress=portMacAddress, cardSecState=cardSecState, trapSeverity=trapSeverity, cadEquipmentMib=cadEquipmentMib, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portNumber=portNumber, portTxFlowControlMode=portTxFlowControlMode, cardName=cardName, portMacIfIndex=portMacIfIndex, diagTestId=diagTestId, equipmentCardTestGroup=equipmentCardTestGroup, cardFpgaSource=cardFpgaSource, portAction=portAction, portShelfId=portShelfId, cardAdminState=cardAdminState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditCycleCount=auditCycleCount, cardTrapInh=cardTrapInh, cardPrState=cardPrState, auditCardId=auditCardId, errEvRowStatus=errEvRowStatus, portType=portType, auditCommand=auditCommand, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardRamSize=cardRamSize, cardTemperatureHighError=cardTemperatureHighError, diskDriveNumber=diskDriveNumber, cardType=cardType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardShelfId=cardShelfId, cardTestDescription=cardTestDescription, cardSubDetected=cardSubDetected, auditLogThrottle=auditLogThrottle, cadEquipmentMibGroup=cadEquipmentMibGroup, portEntry=portEntry, equipmentTraps=equipmentTraps, cmDevice=cmDevice, cmResetStatus=cmResetStatus, cardUpTime=cardUpTime, errEvRecoveryEnabled=errEvRecoveryEnabled, portMaxDsPower=portMaxDsPower, systemKey=systemKey, cardCPLDVersion=cardCPLDVersion, cardSwVersion=cardSwVersion, diskVolumeTrapInh=diskVolumeTrapInh, cmResetInfo=cmResetInfo, cardTestTransId=cardTestTransId, errEvLoggingEnabled=errEvLoggingEnabled, TestResult=TestResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances)
