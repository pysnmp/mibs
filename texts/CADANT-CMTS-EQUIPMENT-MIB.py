...
CardId, PortDetectedMode, UnknownState, PortMode, DiskVolumeUsageLevel, FirmwareSource, PicType, ShelfId, SecondaryState, DuplexStatus, CardType, FlowControlMode, PortId, PrimaryState, EqActionType, PortType, CardSubType, AdminState = mibBuilder.importSymbols("CADANT-TC", "CardId", "PortDetectedMode", "UnknownState", "PortMode", "DiskVolumeUsageLevel", "FirmwareSource", "PicType", "ShelfId", "SecondaryState", "DuplexStatus", "CardType", "FlowControlMode", "PortId", "PrimaryState", "EqActionType", "PortType", "CardSubType", "AdminState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastChangeTime=cardLastChangeTime, diskVolumeEntry=diskVolumeEntry, cardPicDetected=cardPicDetected, cardCPLDVersion=cardCPLDVersion, auditCardId=auditCardId, cmVendor=cmVendor, portMode=portMode, portPrState=portPrState, portShelfId=portShelfId, portSecState=portSecState, equipmentShelfGroup=equipmentShelfGroup, equipmentError=equipmentError, downstreamPowerLoss=downstreamPowerLoss, cardSwVersion=cardSwVersion, diskVolumeGroup=diskVolumeGroup, cardTrapInh=cardTrapInh, cardLastBootSource=cardLastBootSource, cadEquipmentMibGroup=cadEquipmentMibGroup, cardDplxStatusChange=cardDplxStatusChange, cardTestResult=cardTestResult, cardTempOverHeatNotification=cardTempOverHeatNotification, auditFailedCount=auditFailedCount, equipmentCardGroup=equipmentCardGroup, cardUpTime=cardUpTime, cardTestTable=cardTestTable, portId=portId, errEvLoggingEnabled=errEvLoggingEnabled, portEntry=portEntry, systemKey=systemKey, cardCpuSpeed=cardCpuSpeed, cardTestIndex=cardTestIndex, cmRegistrationNotification=cmRegistrationNotification, errEvLogLevel=errEvLogLevel, cardSpareGroupMode=cardSpareGroupMode, eqDiagConfig=eqDiagConfig, consoleOutput=consoleOutput, cardTable=cardTable, diskVolumeName=diskVolumeName, auditTotalFailedCount=auditTotalFailedCount, remainInDiagMode=remainInDiagMode, TestCommand=TestCommand, auditShelfId=auditShelfId, portDescription=portDescription, auditCycleCount=auditCycleCount, auditLogOutput=auditLogOutput, shelfObjects=shelfObjects, fpgaErrorEventTable=fpgaErrorEventTable, errEvRowStatus=errEvRowStatus, portMinDsPower=portMinDsPower, portLastChangeTime=portLastChangeTime, equipmentStateGroup=equipmentStateGroup, portTable=portTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardNumPorts=cardNumPorts, equipmentAuditGroup=equipmentAuditGroup, cardSecState=cardSecState, equipmentAudit=equipmentAudit, equipmentPortGroup=equipmentPortGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardShelfTestIndex=cardShelfTestIndex, cardShelfId=cardShelfId, cardCarrierFwVersion=cardCarrierFwVersion, PYSNMP_MODULE_ID=cadEquipmentMib, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeTable=diskVolumeTable, portDplxStatus=portDplxStatus, portNumber=portNumber, diskVolumeShelfId=diskVolumeShelfId, portRxFlowControlMode=portRxFlowControlMode, cardLastResetReason=cardLastResetReason, auditTable=auditTable, auditDescription=auditDescription, systemGeneralGroup=systemGeneralGroup, cmIpAddress=cmIpAddress, diskVolumeSize=diskVolumeSize, trapSeverity=trapSeverity, portMaxDsPower=portMaxDsPower, cmResetNotification=cmResetNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestScheduleCommand=TestScheduleCommand, portCurrDsPower=portCurrDsPower, cardTemperatureHighError=cardTemperatureHighError, portSecStateChange=portSecStateChange, trapCounter=trapCounter, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portBgpId=portBgpId, cardPicSerialNum=cardPicSerialNum, cardTestId=cardTestId, errEventId=errEventId, equipmentDiagGroup=equipmentDiagGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portDetectedMode=portDetectedMode, auditCommand=auditCommand, cardTestSchedule=cardTestSchedule, cmResetUpTime=cmResetUpTime, cardId=cardId, cardDplxStatus=cardDplxStatus, cardTestTime=cardTestTime, portTxFlowControlDetected=portTxFlowControlDetected, cardRamSize=cardRamSize, cardPicHwVersion=cardPicHwVersion, cardSecStateChange=cardSecStateChange, cardPrState=cardPrState, portRxFlowControlDetected=portRxFlowControlDetected, cardTestScheduleCommand=cardTestScheduleCommand, equipmentTbl=equipmentTbl, portCardId=portCardId, equipmentTraps=equipmentTraps, shelfSwVersion=shelfSwVersion, cardSubType=cardSubType, diskVolumeDriveId=diskVolumeDriveId, cardAdminState=cardAdminState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, verboseLevel=verboseLevel, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfNumber=shelfNumber, auditName=auditName, diskVolumeNumber=diskVolumeNumber, shelfName=shelfName, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, auditLogThrottle=auditLogThrottle, diskDriveNumber=diskDriveNumber, cardBootVersion=cardBootVersion, cardTestType=cardTestType, cardFwUpdateStatus=cardFwUpdateStatus, auditResult=auditResult, portConnectorId=portConnectorId, equipmentDiag=equipmentDiag, portTrapInh=portTrapInh, cardDetectedChange=cardDetectedChange, portAdminState=portAdminState, cmMacAddress=cmMacAddress, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardNumber=cardNumber, portAction=portAction, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTempNoReportNotification=cardTempNoReportNotification, portDplxStatusChange=portDplxStatusChange, diskVolumeCardId=diskVolumeCardId, diskVolumeUsageLevel=diskVolumeUsageLevel, TestResult=TestResult, systemClock=systemClock, cardTestResultDesc=cardTestResultDesc, TestTransactionId=TestTransactionId, auditEntry=auditEntry, cardDetected=cardDetected, cardCpuType=cardCpuType, cmDevice=cmDevice, portType=portType, portMacIfIndex=portMacIfIndex, cmResetInfo=cmResetInfo, cardCarrierSerialNum=cardCarrierSerialNum, cardFlashSize=cardFlashSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portNumChans=portNumChans, portMacAddress=portMacAddress, cardBusSpeed=cardBusSpeed, portTxFlowControlMode=portTxFlowControlMode, portDocsIfIndex=portDocsIfIndex, diskVolumeTrapInh=diskVolumeTrapInh, cardHwVersion=cardHwVersion, cardTestCommand=cardTestCommand, cmResetReason=cmResetReason, diskVolumeUsageNotification=diskVolumeUsageNotification, fpgaErrorEventEntry=fpgaErrorEventEntry, auditAutoScheduling=auditAutoScheduling, TestType=TestType, cardName=cardName, auditPassedCount=auditPassedCount, cardLastBootVersion=cardLastBootVersion, cadEquipmentMib=cadEquipmentMib, portCardSubType=portCardSubType, cardCarrierHwVersion=cardCarrierHwVersion, TestSchedule=TestSchedule, diskVolumeId=diskVolumeId, auditStatus=auditStatus, TestId=TestId, diagTestId=diagTestId, cardTestResultNotification=cardTestResultNotification, cardPrStateChange=cardPrStateChange, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeFileName=diskVolumeFileName, cmResetStatus=cmResetStatus, cardFpgaSource=cardFpgaSource, cardTestName=cardTestName, auditId=auditId, cardTemperature=cardTemperature, diskVolumeDiskSize=diskVolumeDiskSize, cmPrimarySID=cmPrimarySID, systemGeneral=systemGeneral, portPrStateChange=portPrStateChange, cardSubDetected=cardSubDetected, cmResetClearNotification=cmResetClearNotification, auditTime=auditTime, cardSerialNum=cardSerialNum, cardTestDescription=cardTestDescription, cmUChannelID=cmUChannelID, cardTestEntry=cardTestEntry, cardFwVersion=cardFwVersion, cardType=cardType, equipmentState=equipmentState, cardAction=cardAction, cardTestTransId=cardTestTransId, cardEntry=cardEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupId=cardSpareGroupId)
