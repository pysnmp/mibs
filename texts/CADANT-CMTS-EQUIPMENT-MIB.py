...
PortMode, PortDetectedMode, DiskVolumeUsageLevel, CardType, DuplexStatus, PrimaryState, PortType, AdminState, ShelfId, CardSubType, SecondaryState, PicType, FlowControlMode, FirmwareSource, EqActionType, PortId, CardId, UnknownState = mibBuilder.importSymbols("CADANT-TC", "PortMode", "PortDetectedMode", "DiskVolumeUsageLevel", "CardType", "DuplexStatus", "PrimaryState", "PortType", "AdminState", "ShelfId", "CardSubType", "SecondaryState", "PicType", "FlowControlMode", "FirmwareSource", "EqActionType", "PortId", "CardId", "UnknownState")
...
RowStatus, DisplayString, TruthValue, TextualConvention, MacAddress, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "TruthValue", "TextualConvention", "MacAddress", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBusSpeed=cardBusSpeed, diskVolumeNumber=diskVolumeNumber, cardCpuSpeed=cardCpuSpeed, portMinDsPower=portMinDsPower, portDetectedMode=portDetectedMode, errEvRowStatus=errEvRowStatus, equipmentDiagGroup=equipmentDiagGroup, cardTestCommand=cardTestCommand, cardPicHwVersion=cardPicHwVersion, cardSpareGroupId=cardSpareGroupId, cardDetectedChange=cardDetectedChange, equipmentDiag=equipmentDiag, auditTotalPassedCount=auditTotalPassedCount, portPrState=portPrState, cardTable=cardTable, auditCardId=auditCardId, auditName=auditName, systemClock=systemClock, portRxFlowControlMode=portRxFlowControlMode, cmVendor=cmVendor, cardCpuType=cardCpuType, cardFlashSize=cardFlashSize, cardTestTable=cardTestTable, cardTemperature=cardTemperature, portPrStateChange=portPrStateChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmRegistrationNotification=cmRegistrationNotification, auditDescription=auditDescription, consoleOutput=consoleOutput, portNumber=portNumber, cmUChannelID=cmUChannelID, cardShelfId=cardShelfId, cardTestResultDesc=cardTestResultDesc, diagTestId=diagTestId, errEvRecoveryEnabled=errEvRecoveryEnabled, auditCycleCount=auditCycleCount, equipmentTbl=equipmentTbl, equipmentShelfGroup=equipmentShelfGroup, cmIpAddress=cmIpAddress, cardSerialNum=cardSerialNum, portMode=portMode, auditFailedCount=auditFailedCount, auditLogOutput=auditLogOutput, cardName=cardName, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeEntry=diskVolumeEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentStateGroup=equipmentStateGroup, portLastChangeTime=portLastChangeTime, cardLastBootVersion=cardLastBootVersion, TestCommand=TestCommand, cardSubType=cardSubType, cardHwVersion=cardHwVersion, diskVolumeFileName=diskVolumeFileName, TestResult=TestResult, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCarrierSerialNum=cardCarrierSerialNum, cardNumPorts=cardNumPorts, cardLastResetReason=cardLastResetReason, cardUpTime=cardUpTime, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditStatus=auditStatus, cardTestDescription=cardTestDescription, portDplxStatusChange=portDplxStatusChange, equipmentAuditGroup=equipmentAuditGroup, cmResetClearNotification=cmResetClearNotification, equipmentState=equipmentState, cardDetected=cardDetected, portConnectorId=portConnectorId, shelfName=shelfName, cmPrimarySID=cmPrimarySID, cardSecStateChange=cardSecStateChange, cardTemperatureHighError=cardTemperatureHighError, errEventId=errEventId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditPassedCount=auditPassedCount, cardCarrierHwVersion=cardCarrierHwVersion, cardAdminState=cardAdminState, cardFwUpdateStatus=cardFwUpdateStatus, cardTestName=cardTestName, cardLastChangeTime=cardLastChangeTime, downstreamPowerLoss=downstreamPowerLoss, cardTestResultNotification=cardTestResultNotification, portTrapInh=portTrapInh, diskVolumeName=diskVolumeName, systemGeneralGroup=systemGeneralGroup, diskVolumeTable=diskVolumeTable, cadEquipmentMib=cadEquipmentMib, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, cardType=cardType, portTxFlowControlDetected=portTxFlowControlDetected, auditTotalFailedCount=auditTotalFailedCount, eqDiagConfig=eqDiagConfig, portId=portId, diskVolumeDriveId=diskVolumeDriveId, portMaxDsPower=portMaxDsPower, cardSwVersion=cardSwVersion, diskVolumeShelfId=diskVolumeShelfId, cardTestResult=cardTestResult, cardTestTime=cardTestTime, cardTempNoReportNotification=cardTempNoReportNotification, cardFpgaSource=cardFpgaSource, equipmentPortGroup=equipmentPortGroup, cardPicDetected=cardPicDetected, equipmentCardTestGroup=equipmentCardTestGroup, TestSchedule=TestSchedule, diskVolumeSize=diskVolumeSize, portCurrDsPower=portCurrDsPower, auditLogThrottle=auditLogThrottle, cmResetUpTime=cmResetUpTime, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardCarrierFwVersion=cardCarrierFwVersion, portShelfId=portShelfId, PYSNMP_MODULE_ID=cadEquipmentMib, cardTemperatureHighWarn=cardTemperatureHighWarn, verboseLevel=verboseLevel, cardShelfTestIndex=cardShelfTestIndex, cardTestEntry=cardTestEntry, auditTime=auditTime, portRxFlowControlDetected=portRxFlowControlDetected, auditCommand=auditCommand, cmResetInfo=cmResetInfo, trapCounter=trapCounter, TestType=TestType, diskVolumeDiskSize=diskVolumeDiskSize, portCardSubType=portCardSubType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmDevice=cmDevice, diskVolumeGroup=diskVolumeGroup, errEvLogLevel=errEvLogLevel, systemGeneral=systemGeneral, auditEntry=auditEntry, cardNumber=cardNumber, portTable=portTable, cardId=cardId, portSecStateChange=portSecStateChange, cardBootVersion=cardBootVersion, portAction=portAction, shelfNumber=shelfNumber, portMacIfIndex=portMacIfIndex, remainInDiagMode=remainInDiagMode, portDocsIfIndex=portDocsIfIndex, auditResult=auditResult, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestId=cardTestId, fpgaErrorEventGroup=fpgaErrorEventGroup, diskDriveNumber=diskDriveNumber, cardPicSerialNum=cardPicSerialNum, cardRamSize=cardRamSize, cardTestScheduleCommand=cardTestScheduleCommand, cardPrState=cardPrState, cardFwVersion=cardFwVersion, portDplxStatus=portDplxStatus, portNumChans=portNumChans, cardTestSchedule=cardTestSchedule, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageNotification=diskVolumeUsageNotification, TestId=TestId, TestTransactionId=TestTransactionId, auditTable=auditTable, cardSecState=cardSecState, cmMacAddress=cmMacAddress, cardTestIndex=cardTestIndex, cmResetStatus=cmResetStatus, shelfObjects=shelfObjects, TestScheduleCommand=TestScheduleCommand, cardCPLDVersion=cardCPLDVersion, portType=portType, cardDplxStatus=cardDplxStatus, equipmentCardGroup=equipmentCardGroup, equipmentAudit=equipmentAudit, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentError=equipmentError, diskVolumeCardId=diskVolumeCardId, portTxFlowControlMode=portTxFlowControlMode, cardDplxStatusChange=cardDplxStatusChange, auditId=auditId, portEntry=portEntry, cardSpareGroupMode=cardSpareGroupMode, portAdminState=portAdminState, diskVolumeTrapInh=diskVolumeTrapInh, portMacAddress=portMacAddress, auditShelfId=auditShelfId, portBgpId=portBgpId, cmResetNotification=cmResetNotification, equipmentTraps=equipmentTraps, shelfSwVersion=shelfSwVersion, portDescription=portDescription, cardAction=cardAction, diskVolumeId=diskVolumeId, cardTestTransId=cardTestTransId, cardEntry=cardEntry, portCardId=portCardId, portSecState=portSecState, cmResetReason=cmResetReason, cardTestType=cardTestType, cardTrapInh=cardTrapInh, cardTempOverHeatNotification=cardTempOverHeatNotification, trapSeverity=trapSeverity, cardSubDetected=cardSubDetected, cardLastBootSource=cardLastBootSource, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, systemKey=systemKey, errEvLoggingEnabled=errEvLoggingEnabled, cardPrStateChange=cardPrStateChange)
