...
FlowControlMode, AdminState, CardSubType, PicType, PortMode, PrimaryState, EqActionType, ShelfId, PortType, UnknownState, PortId, CardType, PortDetectedMode, CardId, DuplexStatus, FirmwareSource, SecondaryState, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "AdminState", "CardSubType", "PicType", "PortMode", "PrimaryState", "EqActionType", "ShelfId", "PortType", "UnknownState", "PortId", "CardType", "PortDetectedMode", "CardId", "DuplexStatus", "FirmwareSource", "SecondaryState", "DiskVolumeUsageLevel")
...
TimeStamp, TextualConvention, DateAndTime, DisplayString, TruthValue, RowStatus, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TextualConvention", "DateAndTime", "DisplayString", "TruthValue", "RowStatus", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentShelfGroup=equipmentShelfGroup, systemClock=systemClock, diskVolumeShelfId=diskVolumeShelfId, diskVolumeEntry=diskVolumeEntry, cardLastChangeTime=cardLastChangeTime, cardNumPorts=cardNumPorts, cardTestIndex=cardTestIndex, auditCommand=auditCommand, cardSecStateChange=cardSecStateChange, cardTestScheduleCommand=cardTestScheduleCommand, auditAutoScheduling=auditAutoScheduling, auditLogThrottle=auditLogThrottle, auditCycleCount=auditCycleCount, cmDevice=cmDevice, shelfNumber=shelfNumber, cardCarrierFwVersion=cardCarrierFwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, portTxFlowControlDetected=portTxFlowControlDetected, errEvLogLevel=errEvLogLevel, equipmentDiag=equipmentDiag, cardCPLDVersion=cardCPLDVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardAction=cardAction, portTrapInh=portTrapInh, portMaxDsPower=portMaxDsPower, cardRamSize=cardRamSize, portDplxStatus=portDplxStatus, diskDriveNumber=diskDriveNumber, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portMacAddress=portMacAddress, cardTestResult=cardTestResult, cardTemperature=cardTemperature, auditStatus=auditStatus, diskVolumeTrapInh=diskVolumeTrapInh, cardUpTime=cardUpTime, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetStatus=cmResetStatus, equipmentPortGroup=equipmentPortGroup, auditTime=auditTime, fpgaErrorEventGroup=fpgaErrorEventGroup, cardName=cardName, cardPicHwVersion=cardPicHwVersion, equipmentAuditGroup=equipmentAuditGroup, cardTestTable=cardTestTable, TestResult=TestResult, cmMacAddress=cmMacAddress, cmVendor=cmVendor, systemGeneral=systemGeneral, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSpareGroupMode=cardSpareGroupMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeSize=diskVolumeSize, cmResetUpTime=cmResetUpTime, trapCounter=trapCounter, portShelfId=portShelfId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeCardId=diskVolumeCardId, trapSeverity=trapSeverity, equipmentStateGroup=equipmentStateGroup, diskVolumeId=diskVolumeId, portNumber=portNumber, auditLogOutput=auditLogOutput, auditTotalPassedCount=auditTotalPassedCount, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portBgpId=portBgpId, TestId=TestId, errEventId=errEventId, equipmentDiagGroup=equipmentDiagGroup, TestCommand=TestCommand, portMinDsPower=portMinDsPower, cardTestId=cardTestId, portMode=portMode, equipmentAudit=equipmentAudit, auditName=auditName, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDetectedMode=portDetectedMode, diskVolumeNumber=diskVolumeNumber, diskVolumeDriveId=diskVolumeDriveId, portTable=portTable, errEvRowStatus=errEvRowStatus, downstreamPowerLoss=downstreamPowerLoss, eqDiagConfig=eqDiagConfig, TestSchedule=TestSchedule, cardFpgaSource=cardFpgaSource, cardShelfTestIndex=cardShelfTestIndex, cardTestDescription=cardTestDescription, diskVolumeFileName=diskVolumeFileName, cardDetected=cardDetected, cardPicDetected=cardPicDetected, cardTemperatureHighError=cardTemperatureHighError, cardCarrierSerialNum=cardCarrierSerialNum, cardTestCommand=cardTestCommand, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeUsageNotification=diskVolumeUsageNotification, TestScheduleCommand=TestScheduleCommand, remainInDiagMode=remainInDiagMode, auditCardId=auditCardId, auditDescription=auditDescription, diskVolumeTable=diskVolumeTable, cmResetReason=cmResetReason, cardFwVersion=cardFwVersion, portEntry=portEntry, verboseLevel=verboseLevel, portCurrDsPower=portCurrDsPower, cardTempOverHeatNotification=cardTempOverHeatNotification, auditTable=auditTable, cardTestEntry=cardTestEntry, cardSecState=cardSecState, systemGeneralGroup=systemGeneralGroup, portDocsIfIndex=portDocsIfIndex, cardEntry=cardEntry, cardFwUpdateStatus=cardFwUpdateStatus, auditPassedCount=auditPassedCount, cardLastBootSource=cardLastBootSource, cadEquipmentMib=cadEquipmentMib, portNumChans=portNumChans, auditResult=auditResult, cardCarrierHwVersion=cardCarrierHwVersion, consoleOutput=consoleOutput, equipmentState=equipmentState, portRxFlowControlMode=portRxFlowControlMode, diskVolumeName=diskVolumeName, portAdminState=portAdminState, cardType=cardType, errEvLoggingEnabled=errEvLoggingEnabled, cardHwVersion=cardHwVersion, portPrState=portPrState, portPrStateChange=portPrStateChange, cardShelfId=cardShelfId, equipmentError=equipmentError, shelfSwVersion=shelfSwVersion, cmResetClearNotification=cmResetClearNotification, auditEntry=auditEntry, diskVolumeGroup=diskVolumeGroup, portMacIfIndex=portMacIfIndex, TestTransactionId=TestTransactionId, diagTestId=diagTestId, cmResetInfo=cmResetInfo, portLastChangeTime=portLastChangeTime, cardTestType=cardTestType, cardTrapInh=cardTrapInh, cardSpareGroupId=cardSpareGroupId, equipmentTraps=equipmentTraps, cardNumber=cardNumber, cardTestName=cardTestName, portDescription=portDescription, cardLastResetReason=cardLastResetReason, cmRegistrationNotification=cmRegistrationNotification, portTxFlowControlMode=portTxFlowControlMode, cardSubType=cardSubType, cardPicSerialNum=cardPicSerialNum, auditId=auditId, cardCpuType=cardCpuType, cardTestTransId=cardTestTransId, cardBootVersion=cardBootVersion, cardDplxStatusChange=cardDplxStatusChange, equipmentCardGroup=equipmentCardGroup, portSecState=portSecState, portCardSubType=portCardSubType, diskVolumeUsageLevel=diskVolumeUsageLevel, cardLastBootVersion=cardLastBootVersion, shelfObjects=shelfObjects, portRxFlowControlDetected=portRxFlowControlDetected, equipmentTbl=equipmentTbl, auditFailedCount=auditFailedCount, equipmentCardTestGroup=equipmentCardTestGroup, cardCpuSpeed=cardCpuSpeed, portId=portId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetNotification=cmResetNotification, cmUChannelID=cmUChannelID, portConnectorId=portConnectorId, TestType=TestType, fpgaErrorEventTable=fpgaErrorEventTable, cardDplxStatus=cardDplxStatus, cardPrState=cardPrState, cmPrimarySID=cmPrimarySID, cardTestResultNotification=cardTestResultNotification, diskVolumeDiskSize=diskVolumeDiskSize, portSecStateChange=portSecStateChange, cardSerialNum=cardSerialNum, cardSubDetected=cardSubDetected, cardTemperatureHighWarn=cardTemperatureHighWarn, auditTotalFailedCount=auditTotalFailedCount, cardAdminState=cardAdminState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portAction=portAction, cardTable=cardTable, cardBusSpeed=cardBusSpeed, cardTestTime=cardTestTime, PYSNMP_MODULE_ID=cadEquipmentMib, cardSwVersion=cardSwVersion, shelfName=shelfName, portDplxStatusChange=portDplxStatusChange, portType=portType, portCardId=portCardId, systemKey=systemKey, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResultDesc=cardTestResultDesc, cmIpAddress=cmIpAddress, cardId=cardId, cardTempNoReportNotification=cardTempNoReportNotification, cardDetectedChange=cardDetectedChange, cardTestSchedule=cardTestSchedule, cardPrStateChange=cardPrStateChange, auditShelfId=auditShelfId, cardFlashSize=cardFlashSize)
