...
FlowControlMode, PicType, PortMode, CardId, EqActionType, PortDetectedMode, UnknownState, CardType, ShelfId, AdminState, PortId, FirmwareSource, DiskVolumeUsageLevel, PrimaryState, SecondaryState, PortType, DuplexStatus, CardSubType = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PicType", "PortMode", "CardId", "EqActionType", "PortDetectedMode", "UnknownState", "CardType", "ShelfId", "AdminState", "PortId", "FirmwareSource", "DiskVolumeUsageLevel", "PrimaryState", "SecondaryState", "PortType", "DuplexStatus", "CardSubType")
...
TimeStamp, TextualConvention, RowStatus, MacAddress, DateAndTime, DisplayString, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TextualConvention", "RowStatus", "MacAddress", "DateAndTime", "DisplayString", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portDescription=portDescription, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentShelfGroup=equipmentShelfGroup, cardHwVersion=cardHwVersion, cardEntry=cardEntry, portMinDsPower=portMinDsPower, cardFpgaSource=cardFpgaSource, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmMacAddress=cmMacAddress, cmVendor=cmVendor, auditShelfId=auditShelfId, cardTestIndex=cardTestIndex, auditResult=auditResult, cardFwUpdateStatus=cardFwUpdateStatus, portNumChans=portNumChans, TestSchedule=TestSchedule, PYSNMP_MODULE_ID=cadEquipmentMib, errEvRecoveryEnabled=errEvRecoveryEnabled, cardAdminState=cardAdminState, auditDescription=auditDescription, portRxFlowControlMode=portRxFlowControlMode, cardDetectedChange=cardDetectedChange, cardCarrierFwVersion=cardCarrierFwVersion, systemGeneralGroup=systemGeneralGroup, auditCycleCount=auditCycleCount, cardTestEntry=cardTestEntry, TestScheduleCommand=TestScheduleCommand, cardLastBootVersion=cardLastBootVersion, cardPrStateChange=cardPrStateChange, cardSubType=cardSubType, fpgaErrorEventGroup=fpgaErrorEventGroup, auditTime=auditTime, auditTable=auditTable, cardNumber=cardNumber, cardType=cardType, cmIpAddress=cmIpAddress, cmRegistrationNotification=cmRegistrationNotification, diskVolumeId=diskVolumeId, cardAction=cardAction, portRxFlowControlDetected=portRxFlowControlDetected, equipmentCardGroup=equipmentCardGroup, diskVolumeEntry=diskVolumeEntry, portPrState=portPrState, equipmentPortGroup=equipmentPortGroup, diskVolumeName=diskVolumeName, cardTemperatureHighError=cardTemperatureHighError, cardTrapInh=cardTrapInh, cardLastBootSource=cardLastBootSource, TestCommand=TestCommand, portShelfId=portShelfId, cardTestSchedule=cardTestSchedule, diskVolumeCardId=diskVolumeCardId, equipmentTbl=equipmentTbl, cardTestScheduleCommand=cardTestScheduleCommand, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResult=cardTestResult, cardFwVersion=cardFwVersion, cardTestTime=cardTestTime, shelfObjects=shelfObjects, cardTestType=cardTestType, cardSpareGroupMode=cardSpareGroupMode, equipmentTraps=equipmentTraps, cardDplxStatus=cardDplxStatus, auditId=auditId, cardTempNoReportNotification=cardTempNoReportNotification, portTxFlowControlMode=portTxFlowControlMode, equipmentAuditGroup=equipmentAuditGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, diskDriveNumber=diskDriveNumber, errEvLogLevel=errEvLogLevel, cardShelfTestIndex=cardShelfTestIndex, equipmentStateGroup=equipmentStateGroup, portMacIfIndex=portMacIfIndex, cmResetInfo=cmResetInfo, cardDplxStatusChange=cardDplxStatusChange, cardSerialNum=cardSerialNum, cardTestDescription=cardTestDescription, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTable=cardTable, cmPrimarySID=cmPrimarySID, cardPicSerialNum=cardPicSerialNum, cardTemperature=cardTemperature, cardTestResultDesc=cardTestResultDesc, portDplxStatusChange=portDplxStatusChange, equipmentDiag=equipmentDiag, errEvLoggingEnabled=errEvLoggingEnabled, auditFailedCount=auditFailedCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, systemGeneral=systemGeneral, cmResetReason=cmResetReason, portMacAddress=portMacAddress, cardPicDetected=cardPicDetected, portPrStateChange=portPrStateChange, cardTestName=cardTestName, auditTotalFailedCount=auditTotalFailedCount, diskVolumeTable=diskVolumeTable, portTxFlowControlDetected=portTxFlowControlDetected, cmUChannelID=cmUChannelID, systemClock=systemClock, auditTotalPassedCount=auditTotalPassedCount, verboseLevel=verboseLevel, diagTestId=diagTestId, cardCPLDVersion=cardCPLDVersion, cardSwVersion=cardSwVersion, portMaxDsPower=portMaxDsPower, cmResetClearNotification=cmResetClearNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditLogOutput=auditLogOutput, errEventId=errEventId, equipmentAudit=equipmentAudit, TestId=TestId, portTrapInh=portTrapInh, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestCommand=cardTestCommand, diskVolumeDriveId=diskVolumeDriveId, cardUpTime=cardUpTime, portDplxStatus=portDplxStatus, portNumber=portNumber, portEntry=portEntry, portSecState=portSecState, portAdminState=portAdminState, portMode=portMode, cardSpareGroupId=cardSpareGroupId, cmResetUpTime=cmResetUpTime, equipmentError=equipmentError, TestTransactionId=TestTransactionId, cardLastResetReason=cardLastResetReason, cardFlashSize=cardFlashSize, equipmentDiagGroup=equipmentDiagGroup, portDetectedMode=portDetectedMode, cardRamSize=cardRamSize, cardName=cardName, portBgpId=portBgpId, cardPrState=cardPrState, errEvRowStatus=errEvRowStatus, cardSecState=cardSecState, cardNumPorts=cardNumPorts, portAction=portAction, diskVolumeSize=diskVolumeSize, auditStatus=auditStatus, trapSeverity=trapSeverity, fpgaErrorEventEntry=fpgaErrorEventEntry, shelfName=shelfName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastChangeTime=cardLastChangeTime, eqDiagConfig=eqDiagConfig, auditLogThrottle=auditLogThrottle, auditEntry=auditEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardBusSpeed=cardBusSpeed, cardPicHwVersion=cardPicHwVersion, cardTestId=cardTestId, cardShelfId=cardShelfId, cardCarrierHwVersion=cardCarrierHwVersion, cmDevice=cmDevice, portType=portType, remainInDiagMode=remainInDiagMode, cadEquipmentMibConformance=cadEquipmentMibConformance, cardBootVersion=cardBootVersion, cardSecStateChange=cardSecStateChange, auditName=auditName, cmResetNotification=cmResetNotification, cardTestTable=cardTestTable, auditAutoScheduling=auditAutoScheduling, equipmentState=equipmentState, cardCarrierSerialNum=cardCarrierSerialNum, cardCpuSpeed=cardCpuSpeed, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeTrapInh=diskVolumeTrapInh, portTable=portTable, trapCounter=trapCounter, portCardId=portCardId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portLastChangeTime=portLastChangeTime, cardTestTransId=cardTestTransId, systemKey=systemKey, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetStatus=cmResetStatus, cardTestResultNotification=cardTestResultNotification, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMib=cadEquipmentMib, cardCpuType=cardCpuType, diskVolumeFileName=diskVolumeFileName, shelfSwVersion=shelfSwVersion, portDocsIfIndex=portDocsIfIndex, auditCardId=auditCardId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardDetected=cardDetected, shelfNumber=shelfNumber, portCardSubType=portCardSubType, consoleOutput=consoleOutput, portCurrDsPower=portCurrDsPower, diskVolumeNumber=diskVolumeNumber, diskVolumeGroup=diskVolumeGroup, auditPassedCount=auditPassedCount, TestType=TestType, diskVolumeShelfId=diskVolumeShelfId, diskVolumeDiskSize=diskVolumeDiskSize, portSecStateChange=portSecStateChange, TestResult=TestResult, fpgaErrorEventTable=fpgaErrorEventTable, cardSubDetected=cardSubDetected, cardId=cardId, portConnectorId=portConnectorId, portId=portId, auditCommand=auditCommand)
