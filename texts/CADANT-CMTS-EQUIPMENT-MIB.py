...
CardType, DiskVolumeUsageLevel, AdminState, EqActionType, FlowControlMode, PortType, UnknownState, PicType, CardSubType, DuplexStatus, ShelfId, PortId, FirmwareSource, CardId, PrimaryState, SecondaryState, PortDetectedMode, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardType", "DiskVolumeUsageLevel", "AdminState", "EqActionType", "FlowControlMode", "PortType", "UnknownState", "PicType", "CardSubType", "DuplexStatus", "ShelfId", "PortId", "FirmwareSource", "CardId", "PrimaryState", "SecondaryState", "PortDetectedMode", "PortMode")
...
TimeStamp, DisplayString, TextualConvention, DateAndTime, TruthValue, MacAddress, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DisplayString", "TextualConvention", "DateAndTime", "TruthValue", "MacAddress", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardPicHwVersion=cardPicHwVersion, portMacAddress=portMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, errEvRowStatus=errEvRowStatus, equipmentTraps=equipmentTraps, portTxFlowControlMode=portTxFlowControlMode, cardUpTime=cardUpTime, cardDplxStatus=cardDplxStatus, PYSNMP_MODULE_ID=cadEquipmentMib, portAdminState=portAdminState, cadEquipmentMibConformance=cadEquipmentMibConformance, trapCounter=trapCounter, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, shelfName=shelfName, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCpuType=cardCpuType, cardSecStateChange=cardSecStateChange, auditPassedCount=auditPassedCount, cardShelfTestIndex=cardShelfTestIndex, portMinDsPower=portMinDsPower, cardRamSize=cardRamSize, diskVolumeId=diskVolumeId, equipmentDiag=equipmentDiag, cardCarrierSerialNum=cardCarrierSerialNum, cardLastResetReason=cardLastResetReason, auditTable=auditTable, systemKey=systemKey, cardBusSpeed=cardBusSpeed, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeEntry=diskVolumeEntry, diskVolumeDiskSize=diskVolumeDiskSize, cardTestId=cardTestId, cardEntry=cardEntry, cardHwVersion=cardHwVersion, cardSwVersion=cardSwVersion, cardPicDetected=cardPicDetected, cardName=cardName, equipmentShelfGroup=equipmentShelfGroup, cardTestSchedule=cardTestSchedule, equipmentState=equipmentState, cardDetected=cardDetected, portEntry=portEntry, diskVolumeShelfId=diskVolumeShelfId, cardTestScheduleCommand=cardTestScheduleCommand, errEventId=errEventId, cardType=cardType, portAction=portAction, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTemperatureHighWarn=cardTemperatureHighWarn, auditEntry=auditEntry, portSecStateChange=portSecStateChange, portMaxDsPower=portMaxDsPower, consoleOutput=consoleOutput, cardFpgaSource=cardFpgaSource, cardFlashSize=cardFlashSize, diskVolumeGroup=diskVolumeGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, systemClock=systemClock, cmIpAddress=cmIpAddress, cardTestTransId=cardTestTransId, auditLogThrottle=auditLogThrottle, portDplxStatusChange=portDplxStatusChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, cardShelfId=cardShelfId, equipmentAuditGroup=equipmentAuditGroup, cardAdminState=cardAdminState, auditCycleCount=auditCycleCount, cardLastChangeTime=cardLastChangeTime, cmResetUpTime=cmResetUpTime, TestTransactionId=TestTransactionId, diskVolumeName=diskVolumeName, TestId=TestId, cardId=cardId, cardTemperatureHighError=cardTemperatureHighError, cardNumber=cardNumber, cardTable=cardTable, cardTestResultDesc=cardTestResultDesc, fpgaErrorEventTable=fpgaErrorEventTable, cardPrState=cardPrState, cardDplxStatusChange=cardDplxStatusChange, shelfObjects=shelfObjects, cardTemperature=cardTemperature, diskVolumeDriveId=diskVolumeDriveId, eqDiagConfig=eqDiagConfig, cardTestTime=cardTestTime, cmResetNotification=cmResetNotification, portTable=portTable, cardTestIndex=cardTestIndex, cmUChannelID=cmUChannelID, auditLogOutput=auditLogOutput, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSubType=cardSubType, portNumChans=portNumChans, cardTestType=cardTestType, verboseLevel=verboseLevel, cadEquipmentMib=cadEquipmentMib, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditStatus=auditStatus, portDocsIfIndex=portDocsIfIndex, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestCommand=cardTestCommand, diskVolumeSize=diskVolumeSize, auditShelfId=auditShelfId, cardNumPorts=cardNumPorts, diskDriveNumber=diskDriveNumber, cardTestDescription=cardTestDescription, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResult=cardTestResult, equipmentStateGroup=equipmentStateGroup, equipmentCardGroup=equipmentCardGroup, auditTime=auditTime, portCurrDsPower=portCurrDsPower, portRxFlowControlMode=portRxFlowControlMode, cardTestName=cardTestName, cardAction=cardAction, auditResult=auditResult, portDescription=portDescription, diagTestId=diagTestId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestTable=cardTestTable, TestSchedule=TestSchedule, equipmentPortGroup=equipmentPortGroup, portType=portType, diskVolumeCardId=diskVolumeCardId, cardLastBootVersion=cardLastBootVersion, auditTotalFailedCount=auditTotalFailedCount, equipmentError=equipmentError, cardLastBootSource=cardLastBootSource, cardSpareGroupMode=cardSpareGroupMode, cardCarrierHwVersion=cardCarrierHwVersion, cmDevice=cmDevice, equipmentCardTestGroup=equipmentCardTestGroup, errEvLoggingEnabled=errEvLoggingEnabled, cardTrapInh=cardTrapInh, portMacIfIndex=portMacIfIndex, portDetectedMode=portDetectedMode, TestType=TestType, systemGeneralGroup=systemGeneralGroup, systemGeneral=systemGeneral, cardBootVersion=cardBootVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, portCardSubType=portCardSubType, auditCommand=auditCommand, portConnectorId=portConnectorId, cmResetClearNotification=cmResetClearNotification, portBgpId=portBgpId, cmResetReason=cmResetReason, portLastChangeTime=portLastChangeTime, cardTempNoReportNotification=cardTempNoReportNotification, auditFailedCount=auditFailedCount, auditCardId=auditCardId, equipmentTbl=equipmentTbl, auditId=auditId, portCardId=portCardId, portNumber=portNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, remainInDiagMode=remainInDiagMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentDiagGroup=equipmentDiagGroup, trapSeverity=trapSeverity, cardCPLDVersion=cardCPLDVersion, cardPrStateChange=cardPrStateChange, TestScheduleCommand=TestScheduleCommand, equipmentAudit=equipmentAudit, auditName=auditName, cardDetectedChange=cardDetectedChange, cmPrimarySID=cmPrimarySID, cmVendor=cmVendor, auditAutoScheduling=auditAutoScheduling, portId=portId, downstreamPowerLoss=downstreamPowerLoss, diskVolumeFileName=diskVolumeFileName, auditDescription=auditDescription, cardTestEntry=cardTestEntry, portDplxStatus=portDplxStatus, cmRegistrationNotification=cmRegistrationNotification, TestResult=TestResult, cardSecState=cardSecState, portMode=portMode, errEvLogLevel=errEvLogLevel, portSecState=portSecState, TestCommand=TestCommand, cardCpuSpeed=cardCpuSpeed, cardFwUpdateStatus=cardFwUpdateStatus, cmResetInfo=cmResetInfo, cardSpareGroupId=cardSpareGroupId, cardSerialNum=cardSerialNum, cardSubDetected=cardSubDetected, auditTotalPassedCount=auditTotalPassedCount, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, shelfSwVersion=shelfSwVersion, diskVolumeTable=diskVolumeTable, cmResetStatus=cmResetStatus, cardFwVersion=cardFwVersion, shelfNumber=shelfNumber, diskVolumeNumber=diskVolumeNumber, diskVolumeTrapInh=diskVolumeTrapInh, portPrStateChange=portPrStateChange, portTxFlowControlDetected=portTxFlowControlDetected, portShelfId=portShelfId, cmMacAddress=cmMacAddress, portTrapInh=portTrapInh, portPrState=portPrState, cardTestResultNotification=cardTestResultNotification)
