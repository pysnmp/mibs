...
PortDetectedMode, PortType, FirmwareSource, PortMode, PrimaryState, DiskVolumeUsageLevel, ShelfId, UnknownState, CardType, DuplexStatus, CardId, PicType, PortId, SecondaryState, FlowControlMode, CardSubType, AdminState, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "PortType", "FirmwareSource", "PortMode", "PrimaryState", "DiskVolumeUsageLevel", "ShelfId", "UnknownState", "CardType", "DuplexStatus", "CardId", "PicType", "PortId", "SecondaryState", "FlowControlMode", "CardSubType", "AdminState", "EqActionType")
...
DateAndTime, DisplayString, MacAddress, TruthValue, RowStatus, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "DisplayString", "MacAddress", "TruthValue", "RowStatus", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentDiagGroup=equipmentDiagGroup, errEvRowStatus=errEvRowStatus, shelfName=shelfName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvLogLevel=errEvLogLevel, portPrState=portPrState, cadEquipmentMibConformance=cadEquipmentMibConformance, auditFailedCount=auditFailedCount, cardSecState=cardSecState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardType=cardType, eqDiagConfig=eqDiagConfig, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestResult=TestResult, cadEquipmentMib=cadEquipmentMib, cmDevice=cmDevice, systemGeneralGroup=systemGeneralGroup, cardBootVersion=cardBootVersion, portTxFlowControlMode=portTxFlowControlMode, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmMacAddress=cmMacAddress, cardTestTime=cardTestTime, cardNumber=cardNumber, cardDplxStatus=cardDplxStatus, portAdminState=portAdminState, portCardId=portCardId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardName=cardName, cardFlashSize=cardFlashSize, cardPicSerialNum=cardPicSerialNum, portMacAddress=portMacAddress, fpgaErrorEventTable=fpgaErrorEventTable, cardSerialNum=cardSerialNum, portPrStateChange=portPrStateChange, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentCardTestGroup=equipmentCardTestGroup, cardDplxStatusChange=cardDplxStatusChange, portShelfId=portShelfId, auditCommand=auditCommand, cardHwVersion=cardHwVersion, portDplxStatus=portDplxStatus, cmResetStatus=cmResetStatus, diskVolumeCardId=diskVolumeCardId, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestCommand=cardTestCommand, cardLastBootSource=cardLastBootSource, auditEntry=auditEntry, cardPrState=cardPrState, cardCarrierHwVersion=cardCarrierHwVersion, equipmentError=equipmentError, cardLastResetReason=cardLastResetReason, portSecStateChange=portSecStateChange, cardUpTime=cardUpTime, cardCpuType=cardCpuType, verboseLevel=verboseLevel, cardTestTable=cardTestTable, auditId=auditId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portCardSubType=portCardSubType, equipmentShelfGroup=equipmentShelfGroup, cardCpuSpeed=cardCpuSpeed, systemClock=systemClock, portLastChangeTime=portLastChangeTime, diagTestId=diagTestId, portBgpId=portBgpId, cardAction=cardAction, portTable=portTable, trapCounter=trapCounter, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeTable=diskVolumeTable, equipmentDiag=equipmentDiag, cardRamSize=cardRamSize, auditCardId=auditCardId, cardPicHwVersion=cardPicHwVersion, cardTestType=cardTestType, TestTransactionId=TestTransactionId, equipmentStateGroup=equipmentStateGroup, portDescription=portDescription, shelfNumber=shelfNumber, cmResetUpTime=cmResetUpTime, errEventId=errEventId, cmIpAddress=cmIpAddress, cardTrapInh=cardTrapInh, cardTable=cardTable, auditName=auditName, remainInDiagMode=remainInDiagMode, cardTestResultNotification=cardTestResultNotification, cardSecStateChange=cardSecStateChange, portType=portType, cardTestIndex=cardTestIndex, shelfSwVersion=shelfSwVersion, cardAdminState=cardAdminState, cmPrimarySID=cmPrimarySID, cardTempNoReportNotification=cardTempNoReportNotification, cardDetectedChange=cardDetectedChange, auditStatus=auditStatus, cardCarrierFwVersion=cardCarrierFwVersion, cardSubDetected=cardSubDetected, equipmentPortGroup=equipmentPortGroup, diskVolumeSize=diskVolumeSize, fpgaErrorEventGroup=fpgaErrorEventGroup, auditCycleCount=auditCycleCount, portDocsIfIndex=portDocsIfIndex, auditLogOutput=auditLogOutput, shelfObjects=shelfObjects, auditLogThrottle=auditLogThrottle, TestScheduleCommand=TestScheduleCommand, cardNumPorts=cardNumPorts, equipmentTbl=equipmentTbl, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestSchedule=cardTestSchedule, portConnectorId=portConnectorId, equipmentTraps=equipmentTraps, cmVendor=cmVendor, trapSeverity=trapSeverity, cmResetReason=cmResetReason, auditShelfId=auditShelfId, PYSNMP_MODULE_ID=cadEquipmentMib, cardDetected=cardDetected, cmResetClearNotification=cmResetClearNotification, portDplxStatusChange=portDplxStatusChange, cardTemperature=cardTemperature, portMinDsPower=portMinDsPower, portMaxDsPower=portMaxDsPower, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeFileName=diskVolumeFileName, TestType=TestType, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardShelfId=cardShelfId, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibGroup=cadEquipmentMibGroup, portTrapInh=portTrapInh, portRxFlowControlMode=portRxFlowControlMode, portMode=portMode, diskVolumeShelfId=diskVolumeShelfId, systemGeneral=systemGeneral, TestId=TestId, TestCommand=TestCommand, cardLastBootVersion=cardLastBootVersion, cmResetNotification=cmResetNotification, cardFpgaSource=cardFpgaSource, consoleOutput=consoleOutput, portMacIfIndex=portMacIfIndex, cmResetInfo=cmResetInfo, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeName=diskVolumeName, cardBusSpeed=cardBusSpeed, cardSubType=cardSubType, cardCPLDVersion=cardCPLDVersion, cardTestTransId=cardTestTransId, auditPassedCount=auditPassedCount, portAction=portAction, portCurrDsPower=portCurrDsPower, diskVolumeNumber=diskVolumeNumber, portTxFlowControlDetected=portTxFlowControlDetected, cmUChannelID=cmUChannelID, portEntry=portEntry, portDetectedMode=portDetectedMode, downstreamPowerLoss=downstreamPowerLoss, errEvRecoveryEnabled=errEvRecoveryEnabled, portNumChans=portNumChans, cmRegistrationNotification=cmRegistrationNotification, diskVolumeDriveId=diskVolumeDriveId, cardPicDetected=cardPicDetected, cardSpareGroupId=cardSpareGroupId, cardFwUpdateStatus=cardFwUpdateStatus, auditDescription=auditDescription, diskVolumeGroup=diskVolumeGroup, auditTime=auditTime, diskDriveNumber=diskDriveNumber, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSwVersion=cardSwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestResultDesc=cardTestResultDesc, auditTotalFailedCount=auditTotalFailedCount, portNumber=portNumber, diskVolumeTrapInh=diskVolumeTrapInh, equipmentState=equipmentState, diskVolumeId=diskVolumeId, cardTestId=cardTestId, cardTestEntry=cardTestEntry, systemKey=systemKey, cardShelfTestIndex=cardShelfTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentAuditGroup=equipmentAuditGroup, cardFwVersion=cardFwVersion, cardPrStateChange=cardPrStateChange, cardTestDescription=cardTestDescription, cardLastChangeTime=cardLastChangeTime, auditTable=auditTable, auditResult=auditResult, equipmentAudit=equipmentAudit, cardTestName=cardTestName, cardId=cardId, cardEntry=cardEntry, TestSchedule=TestSchedule, auditTotalPassedCount=auditTotalPassedCount, cardTestScheduleCommand=cardTestScheduleCommand, cardTestResult=cardTestResult, portId=portId, auditAutoScheduling=auditAutoScheduling, cardTemperatureHighError=cardTemperatureHighError, diskVolumeEntry=diskVolumeEntry, portSecState=portSecState)
