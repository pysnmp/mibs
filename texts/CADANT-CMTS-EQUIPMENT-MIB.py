...
PortId, PortDetectedMode, FirmwareSource, SecondaryState, CardType, FlowControlMode, CardId, DiskVolumeUsageLevel, CardSubType, DuplexStatus, PortType, PrimaryState, PortMode, ShelfId, PicType, UnknownState, EqActionType, AdminState = mibBuilder.importSymbols("CADANT-TC", "PortId", "PortDetectedMode", "FirmwareSource", "SecondaryState", "CardType", "FlowControlMode", "CardId", "DiskVolumeUsageLevel", "CardSubType", "DuplexStatus", "PortType", "PrimaryState", "PortMode", "ShelfId", "PicType", "UnknownState", "EqActionType", "AdminState")
...
DisplayString, TextualConvention, RowStatus, TruthValue, MacAddress, TimeStamp, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TextualConvention", "RowStatus", "TruthValue", "MacAddress", "TimeStamp", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditName=auditName, systemKey=systemKey, diskVolumeShelfId=diskVolumeShelfId, portId=portId, cardTestType=cardTestType, cmResetStatus=cmResetStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmDevice=cmDevice, cardSpareGroupId=cardSpareGroupId, cmResetReason=cmResetReason, cardSubDetected=cardSubDetected, cardTable=cardTable, portLastChangeTime=portLastChangeTime, systemGeneralGroup=systemGeneralGroup, portDplxStatus=portDplxStatus, equipmentPortGroup=equipmentPortGroup, errEvLogLevel=errEvLogLevel, portMinDsPower=portMinDsPower, cardTestTime=cardTestTime, portAction=portAction, cmUChannelID=cmUChannelID, cardBusSpeed=cardBusSpeed, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditTime=auditTime, cardEntry=cardEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardGroup=equipmentCardGroup, portDetectedMode=portDetectedMode, shelfSwVersion=shelfSwVersion, portTable=portTable, cardTrapInh=cardTrapInh, cardDetected=cardDetected, cardRamSize=cardRamSize, auditShelfId=auditShelfId, auditDescription=auditDescription, cardDplxStatusChange=cardDplxStatusChange, cardTempNoReportNotification=cardTempNoReportNotification, cardTestResultDesc=cardTestResultDesc, diskDriveNumber=diskDriveNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, eqDiagConfig=eqDiagConfig, shelfName=shelfName, diskVolumeTable=diskVolumeTable, cmPrimarySID=cmPrimarySID, shelfObjects=shelfObjects, diskVolumeName=diskVolumeName, cardSecStateChange=cardSecStateChange, portCardSubType=portCardSubType, trapSeverity=trapSeverity, cardShelfId=cardShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentShelfGroup=equipmentShelfGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmVendor=cmVendor, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditEntry=auditEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, systemGeneral=systemGeneral, portSecState=portSecState, diskVolumeSize=diskVolumeSize, equipmentState=equipmentState, TestSchedule=TestSchedule, cardPicHwVersion=cardPicHwVersion, cmResetInfo=cmResetInfo, portDescription=portDescription, portPrStateChange=portPrStateChange, auditStatus=auditStatus, portMacAddress=portMacAddress, cmResetClearNotification=cmResetClearNotification, cardCarrierHwVersion=cardCarrierHwVersion, consoleOutput=consoleOutput, TestTransactionId=TestTransactionId, cardCpuSpeed=cardCpuSpeed, diskVolumeEntry=diskVolumeEntry, diskVolumeCardId=diskVolumeCardId, equipmentCardTestGroup=equipmentCardTestGroup, portDplxStatusChange=portDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, cardDetectedChange=cardDetectedChange, cardCarrierFwVersion=cardCarrierFwVersion, cardPicSerialNum=cardPicSerialNum, cardType=cardType, TestScheduleCommand=TestScheduleCommand, cardAction=cardAction, portCurrDsPower=portCurrDsPower, trapCounter=trapCounter, portMaxDsPower=portMaxDsPower, remainInDiagMode=remainInDiagMode, portRxFlowControlDetected=portRxFlowControlDetected, shelfNumber=shelfNumber, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, verboseLevel=verboseLevel, cardTestTransId=cardTestTransId, auditLogOutput=auditLogOutput, portPrState=portPrState, auditResult=auditResult, cardDplxStatus=cardDplxStatus, cardTestIndex=cardTestIndex, errEventId=errEventId, auditTable=auditTable, cardSubType=cardSubType, equipmentTraps=equipmentTraps, cardFlashSize=cardFlashSize, cmResetUpTime=cmResetUpTime, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastBootVersion=cardLastBootVersion, cardId=cardId, diskVolumeDiskSize=diskVolumeDiskSize, cardSerialNum=cardSerialNum, cardAdminState=cardAdminState, cmRegistrationNotification=cmRegistrationNotification, portTrapInh=portTrapInh, diskVolumeNumber=diskVolumeNumber, TestResult=TestResult, cardPrState=cardPrState, portType=portType, portRxFlowControlMode=portRxFlowControlMode, cardTestName=cardTestName, cardNumPorts=cardNumPorts, diskVolumeUsageNotification=diskVolumeUsageNotification, portNumber=portNumber, TestCommand=TestCommand, portTxFlowControlDetected=portTxFlowControlDetected, diagTestId=diagTestId, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, fpgaErrorEventGroup=fpgaErrorEventGroup, cardBootVersion=cardBootVersion, cardCpuType=cardCpuType, portEntry=portEntry, cardShelfTestIndex=cardShelfTestIndex, auditPassedCount=auditPassedCount, equipmentAuditGroup=equipmentAuditGroup, cardHwVersion=cardHwVersion, auditTotalPassedCount=auditTotalPassedCount, cardTestResultNotification=cardTestResultNotification, portDocsIfIndex=portDocsIfIndex, downstreamPowerLoss=downstreamPowerLoss, cardTemperatureHighError=cardTemperatureHighError, cardCPLDVersion=cardCPLDVersion, cardPrStateChange=cardPrStateChange, diskVolumeDriveId=diskVolumeDriveId, cardTestEntry=cardTestEntry, auditAutoScheduling=auditAutoScheduling, auditFailedCount=auditFailedCount, systemClock=systemClock, cmIpAddress=cmIpAddress, portBgpId=portBgpId, cardUpTime=cardUpTime, cardTestScheduleCommand=cardTestScheduleCommand, TestId=TestId, fpgaErrorEventTable=fpgaErrorEventTable, cardSwVersion=cardSwVersion, portAdminState=portAdminState, cardCarrierSerialNum=cardCarrierSerialNum, cardFwUpdateStatus=cardFwUpdateStatus, errEvLoggingEnabled=errEvLoggingEnabled, equipmentAudit=equipmentAudit, cardNumber=cardNumber, errEvRowStatus=errEvRowStatus, cardFpgaSource=cardFpgaSource, cardTestDescription=cardTestDescription, cardSecState=cardSecState, fpgaErrorEventEntry=fpgaErrorEventEntry, portShelfId=portShelfId, diskVolumeTrapInh=diskVolumeTrapInh, cardLastResetReason=cardLastResetReason, auditCycleCount=auditCycleCount, portCardId=portCardId, cmResetNotification=cmResetNotification, cardTemperature=cardTemperature, portSecStateChange=portSecStateChange, equipmentStateGroup=equipmentStateGroup, diskVolumeId=diskVolumeId, portNumChans=portNumChans, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestCommand=cardTestCommand, cadEquipmentMib=cadEquipmentMib, auditCommand=auditCommand, cardLastBootSource=cardLastBootSource, cmMacAddress=cmMacAddress, cardFwVersion=cardFwVersion, cardTestTable=cardTestTable, auditTotalFailedCount=auditTotalFailedCount, cardTestId=cardTestId, auditCardId=auditCardId, TestType=TestType, portTxFlowControlMode=portTxFlowControlMode, cardSpareGroupMode=cardSpareGroupMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentError=equipmentError, auditId=auditId, cardName=cardName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeGroup=diskVolumeGroup, portConnectorId=portConnectorId, cardPicDetected=cardPicDetected, cardTestSchedule=cardTestSchedule, diskVolumeFileName=diskVolumeFileName, cardLastChangeTime=cardLastChangeTime, portMode=portMode, portMacIfIndex=portMacIfIndex, equipmentDiag=equipmentDiag, cardTestResult=cardTestResult, auditLogThrottle=auditLogThrottle, equipmentTbl=equipmentTbl)
