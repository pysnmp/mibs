...
PrimaryState, UnknownState, DiskVolumeUsageLevel, PortDetectedMode, SecondaryState, FirmwareSource, CardType, CardSubType, CardId, DuplexStatus, PortMode, PortType, PicType, PortId, ShelfId, AdminState, EqActionType, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "UnknownState", "DiskVolumeUsageLevel", "PortDetectedMode", "SecondaryState", "FirmwareSource", "CardType", "CardSubType", "CardId", "DuplexStatus", "PortMode", "PortType", "PicType", "PortId", "ShelfId", "AdminState", "EqActionType", "FlowControlMode")
...
TextualConvention, TruthValue, DisplayString, TimeStamp, RowStatus, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "DisplayString", "TimeStamp", "RowStatus", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTable=auditTable, auditLogOutput=auditLogOutput, cardTestResultDesc=cardTestResultDesc, cardPicDetected=cardPicDetected, diskVolumeTrapInh=diskVolumeTrapInh, auditLogThrottle=auditLogThrottle, auditDescription=auditDescription, cardCarrierHwVersion=cardCarrierHwVersion, portEntry=portEntry, cardTestCommand=cardTestCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, shelfNumber=shelfNumber, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTemperatureHighError=cardTemperatureHighError, cardType=cardType, cardDplxStatus=cardDplxStatus, auditShelfId=auditShelfId, diskVolumeTable=diskVolumeTable, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetInfo=cmResetInfo, cardLastResetReason=cardLastResetReason, equipmentStateGroup=equipmentStateGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardTestType=cardTestType, errEvLoggingEnabled=errEvLoggingEnabled, cardBootVersion=cardBootVersion, cardShelfTestIndex=cardShelfTestIndex, cardBusSpeed=cardBusSpeed, cardTestScheduleCommand=cardTestScheduleCommand, portType=portType, auditId=auditId, portMode=portMode, cardDetectedChange=cardDetectedChange, cardTempNoReportNotification=cardTempNoReportNotification, consoleOutput=consoleOutput, equipmentShelfGroup=equipmentShelfGroup, portPrState=portPrState, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetReason=cmResetReason, cardSubDetected=cardSubDetected, cardPrStateChange=cardPrStateChange, cardFpgaSource=cardFpgaSource, auditEntry=auditEntry, cmUChannelID=cmUChannelID, portNumChans=portNumChans, portPrStateChange=portPrStateChange, cmResetStatus=cmResetStatus, TestId=TestId, cardTestResultNotification=cardTestResultNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, portRxFlowControlMode=portRxFlowControlMode, TestSchedule=TestSchedule, cardTestTable=cardTestTable, diskDriveNumber=diskDriveNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDplxStatusChange=portDplxStatusChange, cadEquipmentMibGroup=cadEquipmentMibGroup, portMacAddress=portMacAddress, cardLastChangeTime=cardLastChangeTime, auditTime=auditTime, diskVolumeUsageNotification=diskVolumeUsageNotification, cardAdminState=cardAdminState, portMaxDsPower=portMaxDsPower, portTxFlowControlDetected=portTxFlowControlDetected, auditPassedCount=auditPassedCount, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestResult=TestResult, portSecState=portSecState, portDetectedMode=portDetectedMode, portDplxStatus=portDplxStatus, cardLastBootVersion=cardLastBootVersion, portNumber=portNumber, portCurrDsPower=portCurrDsPower, TestCommand=TestCommand, equipmentTraps=equipmentTraps, auditTotalFailedCount=auditTotalFailedCount, equipmentError=equipmentError, cardSubType=cardSubType, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diagTestId=diagTestId, diskVolumeCardId=diskVolumeCardId, cardCarrierFwVersion=cardCarrierFwVersion, auditResult=auditResult, systemKey=systemKey, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTable=cardTable, cardTestIndex=cardTestIndex, diskVolumeId=diskVolumeId, verboseLevel=verboseLevel, cardTestName=cardTestName, cardTestDescription=cardTestDescription, cmDevice=cmDevice, cardTestEntry=cardTestEntry, diskVolumeFileName=diskVolumeFileName, fpgaErrorEventTable=fpgaErrorEventTable, auditName=auditName, downstreamPowerLoss=downstreamPowerLoss, diskVolumeDiskSize=diskVolumeDiskSize, cardCpuType=cardCpuType, portMacIfIndex=portMacIfIndex, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmMacAddress=cmMacAddress, auditTotalPassedCount=auditTotalPassedCount, equipmentPortGroup=equipmentPortGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, cardCPLDVersion=cardCPLDVersion, auditStatus=auditStatus, cardSpareGroupMode=cardSpareGroupMode, cardLastBootSource=cardLastBootSource, equipmentAuditGroup=equipmentAuditGroup, TestType=TestType, equipmentTbl=equipmentTbl, diskVolumeSize=diskVolumeSize, eqDiagConfig=eqDiagConfig, portCardSubType=portCardSubType, diskVolumeNumber=diskVolumeNumber, cardFwVersion=cardFwVersion, cmResetUpTime=cmResetUpTime, portShelfId=portShelfId, cardTestId=cardTestId, cadEquipmentMib=cadEquipmentMib, portBgpId=portBgpId, cmResetClearNotification=cmResetClearNotification, cardEntry=cardEntry, errEvLogLevel=errEvLogLevel, cardSecState=cardSecState, cmVendor=cmVendor, portConnectorId=portConnectorId, portId=portId, auditCardId=auditCardId, cardTestResult=cardTestResult, errEvRowStatus=errEvRowStatus, cardNumber=cardNumber, cardTestTime=cardTestTime, TestTransactionId=TestTransactionId, cardId=cardId, fpgaErrorEventEntry=fpgaErrorEventEntry, TestScheduleCommand=TestScheduleCommand, shelfObjects=shelfObjects, equipmentState=equipmentState, trapSeverity=trapSeverity, cardTemperature=cardTemperature, cardDetected=cardDetected, equipmentCardGroup=equipmentCardGroup, portMinDsPower=portMinDsPower, cmIpAddress=cmIpAddress, cardSecStateChange=cardSecStateChange, equipmentAudit=equipmentAudit, cardHwVersion=cardHwVersion, diskVolumeDriveId=diskVolumeDriveId, auditAutoScheduling=auditAutoScheduling, cardPicSerialNum=cardPicSerialNum, cardCarrierSerialNum=cardCarrierSerialNum, portTable=portTable, cardCpuSpeed=cardCpuSpeed, cardNumPorts=cardNumPorts, portCardId=portCardId, portSecStateChange=portSecStateChange, cardUpTime=cardUpTime, diskVolumeShelfId=diskVolumeShelfId, auditCommand=auditCommand, equipmentDiag=equipmentDiag, portDescription=portDescription, cardFlashSize=cardFlashSize, diskVolumeEntry=diskVolumeEntry, cardName=cardName, cardDplxStatusChange=cardDplxStatusChange, cardPicHwVersion=cardPicHwVersion, auditCycleCount=auditCycleCount, cardTrapInh=cardTrapInh, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPrState=cardPrState, errEventId=errEventId, shelfSwVersion=shelfSwVersion, cardShelfId=cardShelfId, cardSpareGroupId=cardSpareGroupId, portTxFlowControlMode=portTxFlowControlMode, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentCardTestGroup=equipmentCardTestGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, systemGeneralGroup=systemGeneralGroup, cardRamSize=cardRamSize, cardSwVersion=cardSwVersion, systemGeneral=systemGeneral, diskVolumeGroup=diskVolumeGroup, cmResetNotification=cmResetNotification, trapCounter=trapCounter, portDocsIfIndex=portDocsIfIndex, portRxFlowControlDetected=portRxFlowControlDetected, portLastChangeTime=portLastChangeTime, cardAction=cardAction, cardSerialNum=cardSerialNum, portAdminState=portAdminState, diskVolumeName=diskVolumeName, portAction=portAction, cardTestTransId=cardTestTransId, auditFailedCount=auditFailedCount, systemClock=systemClock, remainInDiagMode=remainInDiagMode, cardTestSchedule=cardTestSchedule, cadEquipmentMibConformance=cadEquipmentMibConformance, shelfName=shelfName, portTrapInh=portTrapInh, equipmentDiagGroup=equipmentDiagGroup, cmPrimarySID=cmPrimarySID, cmRegistrationNotification=cmRegistrationNotification)
