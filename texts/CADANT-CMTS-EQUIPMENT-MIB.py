...
PicType, ShelfId, PortType, DuplexStatus, CardId, PrimaryState, EqActionType, DiskVolumeUsageLevel, FlowControlMode, PortId, PortDetectedMode, UnknownState, SecondaryState, CardSubType, AdminState, FirmwareSource, PortMode, CardType = mibBuilder.importSymbols("CADANT-TC", "PicType", "ShelfId", "PortType", "DuplexStatus", "CardId", "PrimaryState", "EqActionType", "DiskVolumeUsageLevel", "FlowControlMode", "PortId", "PortDetectedMode", "UnknownState", "SecondaryState", "CardSubType", "AdminState", "FirmwareSource", "PortMode", "CardType")
...
MacAddress, DateAndTime, TimeStamp, TruthValue, DisplayString, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "DateAndTime", "TimeStamp", "TruthValue", "DisplayString", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastResetReason=cardLastResetReason, auditTable=auditTable, portCardSubType=portCardSubType, cardType=cardType, cardSerialNum=cardSerialNum, cardDetectedChange=cardDetectedChange, cardCpuSpeed=cardCpuSpeed, diagTestId=diagTestId, portDetectedMode=portDetectedMode, auditPassedCount=auditPassedCount, cardEntry=cardEntry, cardSpareGroupMode=cardSpareGroupMode, cardShelfId=cardShelfId, portTxFlowControlMode=portTxFlowControlMode, verboseLevel=verboseLevel, cardUpTime=cardUpTime, downstreamPowerLoss=downstreamPowerLoss, cardDplxStatus=cardDplxStatus, diskDriveNumber=diskDriveNumber, portBgpId=portBgpId, equipmentDiag=equipmentDiag, trapSeverity=trapSeverity, remainInDiagMode=remainInDiagMode, auditLogOutput=auditLogOutput, TestSchedule=TestSchedule, cardBusSpeed=cardBusSpeed, diskVolumeNumber=diskVolumeNumber, portDescription=portDescription, shelfName=shelfName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetClearNotification=cmResetClearNotification, portMacAddress=portMacAddress, portDplxStatus=portDplxStatus, cardTestTable=cardTestTable, cardTestTime=cardTestTime, portNumber=portNumber, shelfNumber=shelfNumber, cardFwUpdateStatus=cardFwUpdateStatus, cardTestType=cardTestType, auditName=auditName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetReason=cmResetReason, equipmentState=equipmentState, equipmentTraps=equipmentTraps, cardLastBootVersion=cardLastBootVersion, consoleOutput=consoleOutput, cmVendor=cmVendor, cardDplxStatusChange=cardDplxStatusChange, portRxFlowControlMode=portRxFlowControlMode, diskVolumeCardId=diskVolumeCardId, trapCounter=trapCounter, systemGeneralGroup=systemGeneralGroup, cardLastChangeTime=cardLastChangeTime, cardSubDetected=cardSubDetected, diskVolumeFileName=diskVolumeFileName, TestId=TestId, cardTempNoReportNotification=cardTempNoReportNotification, cardTemperature=cardTemperature, errEventId=errEventId, errEvRowStatus=errEvRowStatus, portSecStateChange=portSecStateChange, cardSwVersion=cardSwVersion, diskVolumeSize=diskVolumeSize, cardHwVersion=cardHwVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, errEvLoggingEnabled=errEvLoggingEnabled, eqDiagConfig=eqDiagConfig, diskVolumeGroup=diskVolumeGroup, cardPicSerialNum=cardPicSerialNum, cmMacAddress=cmMacAddress, cardTestIndex=cardTestIndex, TestType=TestType, equipmentError=equipmentError, portDplxStatusChange=portDplxStatusChange, cardTestName=cardTestName, cardPrStateChange=cardPrStateChange, cmPrimarySID=cmPrimarySID, fpgaErrorEventGroup=fpgaErrorEventGroup, systemKey=systemKey, cardId=cardId, auditTotalFailedCount=auditTotalFailedCount, portTxFlowControlDetected=portTxFlowControlDetected, cmDevice=cmDevice, cmRegistrationNotification=cmRegistrationNotification, cardCPLDVersion=cardCPLDVersion, cardPicDetected=cardPicDetected, cardTestResultDesc=cardTestResultDesc, cardLastBootSource=cardLastBootSource, portMinDsPower=portMinDsPower, errEvRecoveryEnabled=errEvRecoveryEnabled, cardAdminState=cardAdminState, cardSecStateChange=cardSecStateChange, equipmentStateGroup=equipmentStateGroup, diskVolumeTable=diskVolumeTable, portPrState=portPrState, shelfObjects=shelfObjects, cardBootVersion=cardBootVersion, auditEntry=auditEntry, diskVolumeEntry=diskVolumeEntry, auditFailedCount=auditFailedCount, cmResetUpTime=cmResetUpTime, portMacIfIndex=portMacIfIndex, auditCardId=auditCardId, cardTestScheduleCommand=cardTestScheduleCommand, cardTestId=cardTestId, cadEquipmentMib=cadEquipmentMib, diskVolumeShelfId=diskVolumeShelfId, cmIpAddress=cmIpAddress, portPrStateChange=portPrStateChange, cardName=cardName, portLastChangeTime=portLastChangeTime, cardRamSize=cardRamSize, cardTemperatureHighError=cardTemperatureHighError, diskVolumeId=diskVolumeId, errEvLogLevel=errEvLogLevel, cardTemperatureHighWarn=cardTemperatureHighWarn, auditStatus=auditStatus, shelfSwVersion=shelfSwVersion, auditShelfId=auditShelfId, portTable=portTable, portId=portId, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestEntry=cardTestEntry, equipmentCardTestGroup=equipmentCardTestGroup, cardDetected=cardDetected, cardFpgaSource=cardFpgaSource, cardNumPorts=cardNumPorts, systemClock=systemClock, cardTestSchedule=cardTestSchedule, portCurrDsPower=portCurrDsPower, equipmentShelfGroup=equipmentShelfGroup, cardTestTransId=cardTestTransId, auditResult=auditResult, cmResetInfo=cmResetInfo, auditAutoScheduling=auditAutoScheduling, cmResetNotification=cmResetNotification, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCpuType=cardCpuType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, systemGeneral=systemGeneral, auditId=auditId, TestCommand=TestCommand, cardTestResult=cardTestResult, TestResult=TestResult, auditDescription=auditDescription, cardTestDescription=cardTestDescription, PYSNMP_MODULE_ID=cadEquipmentMib, cardCarrierHwVersion=cardCarrierHwVersion, cardAction=cardAction, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPrState=cardPrState, equipmentTbl=equipmentTbl, portRxFlowControlDetected=portRxFlowControlDetected, TestScheduleCommand=TestScheduleCommand, portDocsIfIndex=portDocsIfIndex, cardTestCommand=cardTestCommand, equipmentPortGroup=equipmentPortGroup, cmResetStatus=cmResetStatus, cardCarrierSerialNum=cardCarrierSerialNum, equipmentDiagGroup=equipmentDiagGroup, portAction=portAction, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portTrapInh=portTrapInh, equipmentAudit=equipmentAudit, portAdminState=portAdminState, auditTime=auditTime, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTable=cardTable, portEntry=portEntry, portCardId=portCardId, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentCardGroup=equipmentCardGroup, cardSecState=cardSecState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTrapInh=cardTrapInh, portMode=portMode, portType=portType, cardCarrierFwVersion=cardCarrierFwVersion, auditCommand=auditCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, equipmentAuditGroup=equipmentAuditGroup, diskVolumeTrapInh=diskVolumeTrapInh, cardPicHwVersion=cardPicHwVersion, cardFlashSize=cardFlashSize, diskVolumeName=diskVolumeName, portConnectorId=portConnectorId, cardSubType=cardSubType, auditLogThrottle=auditLogThrottle, fpgaErrorEventTable=fpgaErrorEventTable, auditCycleCount=auditCycleCount, portNumChans=portNumChans, cmUChannelID=cmUChannelID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSpareGroupId=cardSpareGroupId, TestTransactionId=TestTransactionId, cardShelfTestIndex=cardShelfTestIndex, cardTestResultNotification=cardTestResultNotification, diskVolumeDriveId=diskVolumeDriveId, cardFwVersion=cardFwVersion, cardNumber=cardNumber, portSecState=portSecState, portShelfId=portShelfId)
