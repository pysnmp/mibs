...
CardSubType, FirmwareSource, SecondaryState, ShelfId, CardId, UnknownState, PrimaryState, DuplexStatus, FlowControlMode, PortMode, PicType, PortType, EqActionType, PortId, DiskVolumeUsageLevel, CardType, PortDetectedMode, AdminState = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "FirmwareSource", "SecondaryState", "ShelfId", "CardId", "UnknownState", "PrimaryState", "DuplexStatus", "FlowControlMode", "PortMode", "PicType", "PortType", "EqActionType", "PortId", "DiskVolumeUsageLevel", "CardType", "PortDetectedMode", "AdminState")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardCpuSpeed=cardCpuSpeed, portMode=portMode, cardId=cardId, TestCommand=TestCommand, portCardId=portCardId, diskVolumeShelfId=diskVolumeShelfId, diskVolumeUsageLevel=diskVolumeUsageLevel, cmUChannelID=cmUChannelID, cardTable=cardTable, equipmentState=equipmentState, portLastChangeTime=portLastChangeTime, cardAction=cardAction, cardTemperatureHighError=cardTemperatureHighError, cardFlashSize=cardFlashSize, cardCarrierFwVersion=cardCarrierFwVersion, cardShelfTestIndex=cardShelfTestIndex, auditLogOutput=auditLogOutput, diskVolumeUsagePercentage=diskVolumeUsagePercentage, fpgaErrorEventTable=fpgaErrorEventTable, cmDevice=cmDevice, systemGeneralGroup=systemGeneralGroup, cardUpTime=cardUpTime, portSecState=portSecState, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeDiskSize=diskVolumeDiskSize, cardSecState=cardSecState, cardShelfId=cardShelfId, cardCarrierHwVersion=cardCarrierHwVersion, auditTotalPassedCount=auditTotalPassedCount, cardBusSpeed=cardBusSpeed, equipmentAudit=equipmentAudit, shelfObjects=shelfObjects, cardDetectedChange=cardDetectedChange, auditTotalFailedCount=auditTotalFailedCount, auditCycleCount=auditCycleCount, cardTestIndex=cardTestIndex, portPrState=portPrState, portBgpId=portBgpId, cardTestTransId=cardTestTransId, cardFpgaSource=cardFpgaSource, remainInDiagMode=remainInDiagMode, cardAdminState=cardAdminState, cardRamSize=cardRamSize, cmResetClearNotification=cmResetClearNotification, auditPassedCount=auditPassedCount, portConnectorId=portConnectorId, cardPicHwVersion=cardPicHwVersion, cardBootVersion=cardBootVersion, cardTestId=cardTestId, portDetectedMode=portDetectedMode, fpgaErrorEventEntry=fpgaErrorEventEntry, consoleOutput=consoleOutput, portDescription=portDescription, cardTempOverHeatNotification=cardTempOverHeatNotification, diskDriveNumber=diskDriveNumber, shelfSwVersion=shelfSwVersion, cardLastBootSource=cardLastBootSource, cmResetInfo=cmResetInfo, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditAutoScheduling=auditAutoScheduling, cardSecStateChange=cardSecStateChange, equipmentShelfGroup=equipmentShelfGroup, diagTestId=diagTestId, diskVolumeDriveId=diskVolumeDriveId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, errEvRowStatus=errEvRowStatus, portCardSubType=portCardSubType, cardTestResultDesc=cardTestResultDesc, TestScheduleCommand=TestScheduleCommand, diskVolumeEntry=diskVolumeEntry, portTrapInh=portTrapInh, auditShelfId=auditShelfId, equipmentAuditGroup=equipmentAuditGroup, diskVolumeId=diskVolumeId, portId=portId, systemGeneral=systemGeneral, cmResetStatus=cmResetStatus, cardEntry=cardEntry, equipmentError=equipmentError, cardType=cardType, portAction=portAction, cardTestType=cardTestType, portRxFlowControlMode=portRxFlowControlMode, auditStatus=auditStatus, cardTestDescription=cardTestDescription, diskVolumeTable=diskVolumeTable, cardTestCommand=cardTestCommand, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardFwVersion=cardFwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSpareGroupId=cardSpareGroupId, diskVolumeName=diskVolumeName, portDocsIfIndex=portDocsIfIndex, portShelfId=portShelfId, TestId=TestId, diskVolumeCardId=diskVolumeCardId, portTable=portTable, cardFwUpdateStatus=cardFwUpdateStatus, cardSpareGroupMode=cardSpareGroupMode, equipmentStateGroup=equipmentStateGroup, equipmentCardTestGroup=equipmentCardTestGroup, eqDiagConfig=eqDiagConfig, equipmentDiag=equipmentDiag, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMib=cadEquipmentMib, cardTestSchedule=cardTestSchedule, equipmentPortGroup=equipmentPortGroup, shelfName=shelfName, shelfNumber=shelfNumber, cardTemperature=cardTemperature, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastBootVersion=cardLastBootVersion, cardName=cardName, portAdminState=portAdminState, portPrStateChange=portPrStateChange, portMacIfIndex=portMacIfIndex, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, verboseLevel=verboseLevel, cardTestResultNotification=cardTestResultNotification, portSecStateChange=portSecStateChange, cardTestResult=cardTestResult, auditCommand=auditCommand, cardTestScheduleCommand=cardTestScheduleCommand, equipmentTraps=equipmentTraps, cardTestTable=cardTestTable, PYSNMP_MODULE_ID=cadEquipmentMib, portType=portType, auditFailedCount=auditFailedCount, systemClock=systemClock, cmResetUpTime=cmResetUpTime, portTxFlowControlDetected=portTxFlowControlDetected, auditName=auditName, cardDplxStatus=cardDplxStatus, cardNumber=cardNumber, portCurrDsPower=portCurrDsPower, cardCarrierSerialNum=cardCarrierSerialNum, TestSchedule=TestSchedule, cardHwVersion=cardHwVersion, auditTable=auditTable, errEventId=errEventId, cardTempNoReportNotification=cardTempNoReportNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestTime=cardTestTime, TestResult=TestResult, trapSeverity=trapSeverity, portEntry=portEntry, portRxFlowControlDetected=portRxFlowControlDetected, cmVendor=cmVendor, diskVolumeNumber=diskVolumeNumber, cardNumPorts=cardNumPorts, errEvLogLevel=errEvLogLevel, cardLastResetReason=cardLastResetReason, cmPrimarySID=cmPrimarySID, portMinDsPower=portMinDsPower, auditResult=auditResult, cardSerialNum=cardSerialNum, cardDetected=cardDetected, portDplxStatusChange=portDplxStatusChange, equipmentTbl=equipmentTbl, cmIpAddress=cmIpAddress, portDplxStatus=portDplxStatus, diskVolumeFileName=diskVolumeFileName, cardSubType=cardSubType, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestEntry=cardTestEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, cardLastChangeTime=cardLastChangeTime, cardTestName=cardTestName, cmRegistrationNotification=cmRegistrationNotification, auditId=auditId, cardPicDetected=cardPicDetected, auditCardId=auditCardId, cmMacAddress=cmMacAddress, cardPrState=cardPrState, cardPicSerialNum=cardPicSerialNum, auditTime=auditTime, cardCPLDVersion=cardCPLDVersion, errEvLoggingEnabled=errEvLoggingEnabled, equipmentCardGroup=equipmentCardGroup, TestTransactionId=TestTransactionId, cardSubDetected=cardSubDetected, portNumChans=portNumChans, TestType=TestType, cardTrapInh=cardTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, systemKey=systemKey, cardCpuType=cardCpuType, cmResetNotification=cmResetNotification, portTxFlowControlMode=portTxFlowControlMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeGroup=diskVolumeGroup, portMacAddress=portMacAddress, portNumber=portNumber, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetReason=cmResetReason, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardDplxStatusChange=cardDplxStatusChange, cardPrStateChange=cardPrStateChange, trapCounter=trapCounter, cardSwVersion=cardSwVersion, auditEntry=auditEntry, auditDescription=auditDescription, auditLogThrottle=auditLogThrottle, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeSize=diskVolumeSize)
