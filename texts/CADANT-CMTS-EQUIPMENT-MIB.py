...
DiskVolumeUsageLevel, PicType, CardSubType, CardType, PortType, ShelfId, EqActionType, SecondaryState, AdminState, UnknownState, FlowControlMode, CardId, DuplexStatus, PortDetectedMode, PortMode, PortId, FirmwareSource, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "PicType", "CardSubType", "CardType", "PortType", "ShelfId", "EqActionType", "SecondaryState", "AdminState", "UnknownState", "FlowControlMode", "CardId", "DuplexStatus", "PortDetectedMode", "PortMode", "PortId", "FirmwareSource", "PrimaryState")
...
DisplayString, TimeStamp, TextualConvention, DateAndTime, TruthValue, RowStatus, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "TextualConvention", "DateAndTime", "TruthValue", "RowStatus", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardAction=cardAction, portDocsIfIndex=portDocsIfIndex, cardTestCommand=cardTestCommand, cardTestTable=cardTestTable, diskVolumeDriveId=diskVolumeDriveId, auditPassedCount=auditPassedCount, portAdminState=portAdminState, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, TestSchedule=TestSchedule, portId=portId, cmResetStatus=cmResetStatus, cardTestScheduleCommand=cardTestScheduleCommand, cardFpgaSource=cardFpgaSource, portDetectedMode=portDetectedMode, errEventId=errEventId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardCpuType=cardCpuType, cardUpTime=cardUpTime, remainInDiagMode=remainInDiagMode, cmPrimarySID=cmPrimarySID, cardSubType=cardSubType, equipmentShelfGroup=equipmentShelfGroup, PYSNMP_MODULE_ID=cadEquipmentMib, diskDriveNumber=diskDriveNumber, auditTable=auditTable, auditEntry=auditEntry, auditAutoScheduling=auditAutoScheduling, equipmentCardGroup=equipmentCardGroup, shelfSwVersion=shelfSwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardPrStateChange=cardPrStateChange, cardFlashSize=cardFlashSize, cardTestResultDesc=cardTestResultDesc, cmResetUpTime=cmResetUpTime, cardCarrierHwVersion=cardCarrierHwVersion, auditCycleCount=auditCycleCount, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestDescription=cardTestDescription, systemGeneralGroup=systemGeneralGroup, portTrapInh=portTrapInh, portCardId=portCardId, portPrStateChange=portPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTotalFailedCount=auditTotalFailedCount, cardSpareGroupId=cardSpareGroupId, shelfObjects=shelfObjects, cardDetected=cardDetected, portRxFlowControlMode=portRxFlowControlMode, portDplxStatus=portDplxStatus, cardLastChangeTime=cardLastChangeTime, portTxFlowControlDetected=portTxFlowControlDetected, cardFwVersion=cardFwVersion, cardTrapInh=cardTrapInh, shelfName=shelfName, auditShelfId=auditShelfId, auditResult=auditResult, systemGeneral=systemGeneral, cardPrState=cardPrState, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeEntry=diskVolumeEntry, cardPicSerialNum=cardPicSerialNum, auditTotalPassedCount=auditTotalPassedCount, cardType=cardType, portDplxStatusChange=portDplxStatusChange, cardPicDetected=cardPicDetected, cardTestId=cardTestId, portMacAddress=portMacAddress, auditLogOutput=auditLogOutput, downstreamPowerLoss=downstreamPowerLoss, auditStatus=auditStatus, cmIpAddress=cmIpAddress, portMacIfIndex=portMacIfIndex, errEvLogLevel=errEvLogLevel, cardPicHwVersion=cardPicHwVersion, cardEntry=cardEntry, TestTransactionId=TestTransactionId, cmRegistrationNotification=cmRegistrationNotification, cardTestTime=cardTestTime, cardCarrierSerialNum=cardCarrierSerialNum, portBgpId=portBgpId, auditTime=auditTime, cardCarrierFwVersion=cardCarrierFwVersion, auditCardId=auditCardId, equipmentState=equipmentState, portDescription=portDescription, cardTempOverHeatNotification=cardTempOverHeatNotification, portPrState=portPrState, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTable=diskVolumeTable, cardNumber=cardNumber, cmResetNotification=cmResetNotification, cardSerialNum=cardSerialNum, equipmentPortGroup=equipmentPortGroup, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeCardId=diskVolumeCardId, portEntry=portEntry, equipmentAudit=equipmentAudit, cardTestResultNotification=cardTestResultNotification, portAction=portAction, cardId=cardId, cardTestType=cardTestType, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsageNotification=diskVolumeUsageNotification, auditName=auditName, cardSubDetected=cardSubDetected, cardNumPorts=cardNumPorts, cardCPLDVersion=cardCPLDVersion, cardBootVersion=cardBootVersion, consoleOutput=consoleOutput, diskVolumeNumber=diskVolumeNumber, TestType=TestType, equipmentDiagGroup=equipmentDiagGroup, cardTemperatureHighError=cardTemperatureHighError, cardTestName=cardTestName, cmResetClearNotification=cmResetClearNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardDplxStatus=cardDplxStatus, cardSpareGroupMode=cardSpareGroupMode, portMinDsPower=portMinDsPower, cardAdminState=cardAdminState, diskVolumeUsageLevel=diskVolumeUsageLevel, cardHwVersion=cardHwVersion, cardBusSpeed=cardBusSpeed, cardTemperatureHighWarn=cardTemperatureHighWarn, portTxFlowControlMode=portTxFlowControlMode, portNumChans=portNumChans, cmMacAddress=cmMacAddress, portMode=portMode, portShelfId=portShelfId, cardSwVersion=cardSwVersion, cardTestSchedule=cardTestSchedule, diskVolumeSize=diskVolumeSize, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeName=diskVolumeName, equipmentStateGroup=equipmentStateGroup, portType=portType, portTable=portTable, cardTestEntry=cardTestEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTemperature=cardTemperature, diskVolumeGroup=diskVolumeGroup, portSecStateChange=portSecStateChange, cardShelfId=cardShelfId, errEvRowStatus=errEvRowStatus, portCardSubType=portCardSubType, diskVolumeId=diskVolumeId, cardRamSize=cardRamSize, portSecState=portSecState, portCurrDsPower=portCurrDsPower, diskVolumeTrapInh=diskVolumeTrapInh, cardTestIndex=cardTestIndex, equipmentCardTestGroup=equipmentCardTestGroup, equipmentAuditGroup=equipmentAuditGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmDevice=cmDevice, cardFwUpdateStatus=cardFwUpdateStatus, cardDetectedChange=cardDetectedChange, shelfNumber=shelfNumber, cmResetReason=cmResetReason, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditFailedCount=auditFailedCount, systemClock=systemClock, cardLastBootVersion=cardLastBootVersion, TestScheduleCommand=TestScheduleCommand, equipmentTbl=equipmentTbl, errEvLoggingEnabled=errEvLoggingEnabled, TestId=TestId, equipmentError=equipmentError, equipmentDiag=equipmentDiag, portMaxDsPower=portMaxDsPower, cardCpuSpeed=cardCpuSpeed, auditDescription=auditDescription, cardSecState=cardSecState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentTraps=equipmentTraps, systemKey=systemKey, diagTestId=diagTestId, cmResetInfo=cmResetInfo, TestCommand=TestCommand, diskVolumeShelfId=diskVolumeShelfId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardName=cardName, auditId=auditId, cardDplxStatusChange=cardDplxStatusChange, cadEquipmentMib=cadEquipmentMib, TestResult=TestResult, auditLogThrottle=auditLogThrottle, cmVendor=cmVendor, cmUChannelID=cmUChannelID, eqDiagConfig=eqDiagConfig, trapSeverity=trapSeverity, diskVolumeFileName=diskVolumeFileName, verboseLevel=verboseLevel, cardTestResult=cardTestResult, cardSecStateChange=cardSecStateChange, cardTable=cardTable, trapCounter=trapCounter, cardShelfTestIndex=cardShelfTestIndex, cardTestTransId=cardTestTransId, portConnectorId=portConnectorId, cardLastResetReason=cardLastResetReason, portNumber=portNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, portLastChangeTime=portLastChangeTime, auditCommand=auditCommand, cardLastBootSource=cardLastBootSource)
