...
AdminState, PortMode, EqActionType, DiskVolumeUsageLevel, FlowControlMode, PortType, SecondaryState, ShelfId, PortId, DuplexStatus, CardType, PortDetectedMode, UnknownState, PrimaryState, FirmwareSource, CardId, CardSubType, PicType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PortMode", "EqActionType", "DiskVolumeUsageLevel", "FlowControlMode", "PortType", "SecondaryState", "ShelfId", "PortId", "DuplexStatus", "CardType", "PortDetectedMode", "UnknownState", "PrimaryState", "FirmwareSource", "CardId", "CardSubType", "PicType")
...
DateAndTime, MacAddress, TruthValue, RowStatus, DisplayString, TextualConvention, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "MacAddress", "TruthValue", "RowStatus", "DisplayString", "TextualConvention", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentDiag=equipmentDiag, cardSerialNum=cardSerialNum, cardBusSpeed=cardBusSpeed, cardTestTime=cardTestTime, fpgaErrorEventGroup=fpgaErrorEventGroup, cmIpAddress=cmIpAddress, trapCounter=trapCounter, cardLastChangeTime=cardLastChangeTime, verboseLevel=verboseLevel, diskVolumeShelfId=diskVolumeShelfId, eqDiagConfig=eqDiagConfig, errEvRowStatus=errEvRowStatus, consoleOutput=consoleOutput, systemGeneralGroup=systemGeneralGroup, portType=portType, diskVolumeName=diskVolumeName, cardTestTransId=cardTestTransId, cmDevice=cmDevice, portDplxStatusChange=portDplxStatusChange, portAction=portAction, auditCommand=auditCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditCycleCount=auditCycleCount, cardName=cardName, cardPicSerialNum=cardPicSerialNum, portMode=portMode, shelfObjects=shelfObjects, errEvLogLevel=errEvLogLevel, auditLogOutput=auditLogOutput, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentError=equipmentError, portCardSubType=portCardSubType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditName=auditName, cardTestResultNotification=cardTestResultNotification, cardLastBootSource=cardLastBootSource, portCardId=portCardId, cardLastResetReason=cardLastResetReason, portNumChans=portNumChans, cardDetectedChange=cardDetectedChange, cardDetected=cardDetected, portConnectorId=portConnectorId, auditTable=auditTable, auditTotalPassedCount=auditTotalPassedCount, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierHwVersion=cardCarrierHwVersion, equipmentStateGroup=equipmentStateGroup, systemClock=systemClock, cardTestCommand=cardTestCommand, cmResetReason=cmResetReason, cardTemperatureHighWarn=cardTemperatureHighWarn, auditPassedCount=auditPassedCount, diskVolumeNumber=diskVolumeNumber, equipmentState=equipmentState, cmResetNotification=cmResetNotification, cmUChannelID=cmUChannelID, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestType=cardTestType, cardCpuType=cardCpuType, cardTemperature=cardTemperature, auditResult=auditResult, cmResetStatus=cmResetStatus, cardTestId=cardTestId, cardTestResultDesc=cardTestResultDesc, auditDescription=auditDescription, diskVolumeCardId=diskVolumeCardId, equipmentTbl=equipmentTbl, cardNumPorts=cardNumPorts, cardPicDetected=cardPicDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardSubDetected=cardSubDetected, cardFpgaSource=cardFpgaSource, diskDriveNumber=diskDriveNumber, diskVolumeGroup=diskVolumeGroup, diskVolumeId=diskVolumeId, equipmentTraps=equipmentTraps, systemKey=systemKey, TestCommand=TestCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, systemGeneral=systemGeneral, diskVolumeTrapInh=diskVolumeTrapInh, portMacIfIndex=portMacIfIndex, cardTemperatureHighError=cardTemperatureHighError, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardFlashSize=cardFlashSize, auditTime=auditTime, auditLogThrottle=auditLogThrottle, portMinDsPower=portMinDsPower, cardSecStateChange=cardSecStateChange, TestScheduleCommand=TestScheduleCommand, portBgpId=portBgpId, portNumber=portNumber, cardCarrierFwVersion=cardCarrierFwVersion, cardDplxStatusChange=cardDplxStatusChange, cardRamSize=cardRamSize, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeEntry=diskVolumeEntry, cardCarrierSerialNum=cardCarrierSerialNum, portDocsIfIndex=portDocsIfIndex, diskVolumeTable=diskVolumeTable, diskVolumeDiskSize=diskVolumeDiskSize, cardTestEntry=cardTestEntry, cardTestTable=cardTestTable, portShelfId=portShelfId, portDplxStatus=portDplxStatus, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeFileName=diskVolumeFileName, cardHwVersion=cardHwVersion, portTable=portTable, cardCpuSpeed=cardCpuSpeed, auditStatus=auditStatus, portDescription=portDescription, portPrState=portPrState, cardSpareGroupId=cardSpareGroupId, cardShelfId=cardShelfId, TestResult=TestResult, portRxFlowControlMode=portRxFlowControlMode, portLastChangeTime=portLastChangeTime, cardTestName=cardTestName, auditTotalFailedCount=auditTotalFailedCount, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeDriveId=diskVolumeDriveId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSubType=cardSubType, cmMacAddress=cmMacAddress, cadEquipmentMibCompliance=cadEquipmentMibCompliance, TestType=TestType, portCurrDsPower=portCurrDsPower, cardDplxStatus=cardDplxStatus, portId=portId, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetClearNotification=cmResetClearNotification, auditFailedCount=auditFailedCount, cmPrimarySID=cmPrimarySID, cardLastBootVersion=cardLastBootVersion, cardTable=cardTable, cardBootVersion=cardBootVersion, cardTestIndex=cardTestIndex, cadEquipmentMibConformance=cadEquipmentMibConformance, portDetectedMode=portDetectedMode, portTrapInh=portTrapInh, cmResetUpTime=cmResetUpTime, equipmentPortGroup=equipmentPortGroup, cardEntry=cardEntry, auditShelfId=auditShelfId, portMaxDsPower=portMaxDsPower, portSecStateChange=portSecStateChange, TestSchedule=TestSchedule, cmVendor=cmVendor, TestId=TestId, auditId=auditId, diskVolumeSize=diskVolumeSize, PYSNMP_MODULE_ID=cadEquipmentMib, cardAction=cardAction, cardId=cardId, cardFwUpdateStatus=cardFwUpdateStatus, cardSpareGroupMode=cardSpareGroupMode, cardPrState=cardPrState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardShelfTestIndex=cardShelfTestIndex, cardNumber=cardNumber, shelfNumber=shelfNumber, portEntry=portEntry, shelfSwVersion=shelfSwVersion, portPrStateChange=portPrStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, fpgaErrorEventTable=fpgaErrorEventTable, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicHwVersion=cardPicHwVersion, auditAutoScheduling=auditAutoScheduling, portAdminState=portAdminState, cardTestResult=cardTestResult, auditEntry=auditEntry, equipmentAuditGroup=equipmentAuditGroup, cmResetInfo=cmResetInfo, equipmentCardTestGroup=equipmentCardTestGroup, cardType=cardType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMib=cadEquipmentMib, cardSwVersion=cardSwVersion, errEventId=errEventId, portRxFlowControlDetected=portRxFlowControlDetected, cardUpTime=cardUpTime, errEvLoggingEnabled=errEvLoggingEnabled, cmRegistrationNotification=cmRegistrationNotification, shelfName=shelfName, equipmentShelfGroup=equipmentShelfGroup, cardSecState=cardSecState, cardPrStateChange=cardPrStateChange, diagTestId=diagTestId, auditCardId=auditCardId, TestTransactionId=TestTransactionId, cardTestSchedule=cardTestSchedule, portSecState=portSecState, cardTestDescription=cardTestDescription, cardFwVersion=cardFwVersion, equipmentCardGroup=equipmentCardGroup, trapSeverity=trapSeverity, remainInDiagMode=remainInDiagMode, cardTrapInh=cardTrapInh, portMacAddress=portMacAddress, equipmentAudit=equipmentAudit, cardAdminState=cardAdminState, equipmentDiagGroup=equipmentDiagGroup, portTxFlowControlMode=portTxFlowControlMode, cardCPLDVersion=cardCPLDVersion)
