...
CardType, PortType, ShelfId, AdminState, CardId, CardSubType, PortDetectedMode, UnknownState, PicType, DiskVolumeUsageLevel, PortId, PortMode, FirmwareSource, EqActionType, FlowControlMode, DuplexStatus, SecondaryState, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "CardType", "PortType", "ShelfId", "AdminState", "CardId", "CardSubType", "PortDetectedMode", "UnknownState", "PicType", "DiskVolumeUsageLevel", "PortId", "PortMode", "FirmwareSource", "EqActionType", "FlowControlMode", "DuplexStatus", "SecondaryState", "PrimaryState")
...
RowStatus, DateAndTime, MacAddress, TruthValue, TextualConvention, TimeStamp, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "MacAddress", "TruthValue", "TextualConvention", "TimeStamp", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardHwVersion=cardHwVersion, auditEntry=auditEntry, cardAdminState=cardAdminState, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeDriveId=diskVolumeDriveId, cardTestSchedule=cardTestSchedule, auditCommand=auditCommand, equipmentAuditGroup=equipmentAuditGroup, cardDplxStatus=cardDplxStatus, auditLogThrottle=auditLogThrottle, portPrStateChange=portPrStateChange, cardTestId=cardTestId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTrapInh=cardTrapInh, cardTestCommand=cardTestCommand, cmResetNotification=cmResetNotification, auditTime=auditTime, cardCpuSpeed=cardCpuSpeed, cmMacAddress=cmMacAddress, cardFpgaSource=cardFpgaSource, TestCommand=TestCommand, portId=portId, PYSNMP_MODULE_ID=cadEquipmentMib, cardSubType=cardSubType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, systemGeneralGroup=systemGeneralGroup, portConnectorId=portConnectorId, portTrapInh=portTrapInh, auditLogOutput=auditLogOutput, auditCardId=auditCardId, cardTestName=cardTestName, portSecStateChange=portSecStateChange, cardAction=cardAction, portMode=portMode, cardLastBootVersion=cardLastBootVersion, diskVolumeSize=diskVolumeSize, equipmentDiagGroup=equipmentDiagGroup, equipmentStateGroup=equipmentStateGroup, TestSchedule=TestSchedule, portLastChangeTime=portLastChangeTime, cardDetectedChange=cardDetectedChange, portMacAddress=portMacAddress, portTxFlowControlMode=portTxFlowControlMode, cardUpTime=cardUpTime, diskVolumeId=diskVolumeId, equipmentDiag=equipmentDiag, errEvRowStatus=errEvRowStatus, auditTotalPassedCount=auditTotalPassedCount, shelfObjects=shelfObjects, cardBootVersion=cardBootVersion, equipmentState=equipmentState, cardShelfTestIndex=cardShelfTestIndex, diskVolumeCardId=diskVolumeCardId, diskVolumeTrapInh=diskVolumeTrapInh, cardTestEntry=cardTestEntry, cardCarrierSerialNum=cardCarrierSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardName=cardName, cardSpareGroupMode=cardSpareGroupMode, cardType=cardType, fpgaErrorEventGroup=fpgaErrorEventGroup, portAdminState=portAdminState, portCurrDsPower=portCurrDsPower, cardCarrierHwVersion=cardCarrierHwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardDetected=cardDetected, portShelfId=portShelfId, cardId=cardId, cardTestResultNotification=cardTestResultNotification, cardNumPorts=cardNumPorts, diskVolumeDiskSize=diskVolumeDiskSize, portSecState=portSecState, cardBusSpeed=cardBusSpeed, remainInDiagMode=remainInDiagMode, verboseLevel=verboseLevel, cardFlashSize=cardFlashSize, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDetectedMode=portDetectedMode, portType=portType, cmIpAddress=cmIpAddress, cardShelfId=cardShelfId, cardTempNoReportNotification=cardTempNoReportNotification, cardTestResultDesc=cardTestResultDesc, portRxFlowControlMode=portRxFlowControlMode, auditFailedCount=auditFailedCount, auditCycleCount=auditCycleCount, diskVolumeFileName=diskVolumeFileName, cardSecState=cardSecState, fpgaErrorEventTable=fpgaErrorEventTable, equipmentTbl=equipmentTbl, errEventId=errEventId, portAction=portAction, fpgaErrorEventEntry=fpgaErrorEventEntry, portMacIfIndex=portMacIfIndex, cardTable=cardTable, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, systemGeneral=systemGeneral, cardEntry=cardEntry, TestScheduleCommand=TestScheduleCommand, portCardSubType=portCardSubType, portDescription=portDescription, cardTestScheduleCommand=cardTestScheduleCommand, consoleOutput=consoleOutput, auditStatus=auditStatus, cmUChannelID=cmUChannelID, cmResetReason=cmResetReason, diskVolumeName=diskVolumeName, cardTestDescription=cardTestDescription, cardSpareGroupId=cardSpareGroupId, cardTestType=cardTestType, cardTestIndex=cardTestIndex, cardSwVersion=cardSwVersion, portPrState=portPrState, equipmentCardTestGroup=equipmentCardTestGroup, equipmentTraps=equipmentTraps, cardSecStateChange=cardSecStateChange, errEvRecoveryEnabled=errEvRecoveryEnabled, cardLastBootSource=cardLastBootSource, cardPicDetected=cardPicDetected, cmVendor=cmVendor, cmResetUpTime=cmResetUpTime, diskVolumeGroup=diskVolumeGroup, trapSeverity=trapSeverity, diskVolumeUsageLevel=diskVolumeUsageLevel, portMinDsPower=portMinDsPower, cardTestResult=cardTestResult, portEntry=portEntry, equipmentCardGroup=equipmentCardGroup, cardTemperature=cardTemperature, cardTemperatureHighError=cardTemperatureHighError, downstreamPowerLoss=downstreamPowerLoss, cardCpuType=cardCpuType, portDocsIfIndex=portDocsIfIndex, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portNumber=portNumber, auditTable=auditTable, cmDevice=cmDevice, cmPrimarySID=cmPrimarySID, TestId=TestId, shelfNumber=shelfNumber, TestType=TestType, diskVolumeTable=diskVolumeTable, cmResetInfo=cmResetInfo, cardLastChangeTime=cardLastChangeTime, cardSubDetected=cardSubDetected, diskVolumeNumber=diskVolumeNumber, TestTransactionId=TestTransactionId, shelfSwVersion=shelfSwVersion, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibGroup=cadEquipmentMibGroup, cardPrStateChange=cardPrStateChange, equipmentPortGroup=equipmentPortGroup, auditTotalFailedCount=auditTotalFailedCount, equipmentAudit=equipmentAudit, TestResult=TestResult, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTransId=cardTestTransId, cmRegistrationNotification=cmRegistrationNotification, auditAutoScheduling=auditAutoScheduling, diskVolumeEntry=diskVolumeEntry, eqDiagConfig=eqDiagConfig, auditName=auditName, cardDplxStatusChange=cardDplxStatusChange, cardFwVersion=cardFwVersion, portRxFlowControlDetected=portRxFlowControlDetected, cmResetClearNotification=cmResetClearNotification, cardTestTime=cardTestTime, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDplxStatusChange=portDplxStatusChange, portNumChans=portNumChans, diskVolumeShelfId=diskVolumeShelfId, systemKey=systemKey, portDplxStatus=portDplxStatus, portMaxDsPower=portMaxDsPower, cardPicHwVersion=cardPicHwVersion, cardPicSerialNum=cardPicSerialNum, cardTemperatureHighWarn=cardTemperatureHighWarn, diskDriveNumber=diskDriveNumber, cardTestTable=cardTestTable, portBgpId=portBgpId, trapCounter=trapCounter, cardCPLDVersion=cardCPLDVersion, errEvLogLevel=errEvLogLevel, equipmentShelfGroup=equipmentShelfGroup, cardRamSize=cardRamSize, diagTestId=diagTestId, portTable=portTable, auditDescription=auditDescription, systemClock=systemClock, portCardId=portCardId, auditId=auditId, cardLastResetReason=cardLastResetReason, auditPassedCount=auditPassedCount, cardFwUpdateStatus=cardFwUpdateStatus, auditShelfId=auditShelfId, equipmentError=equipmentError, cmResetStatus=cmResetStatus, diskVolumeUsageNotification=diskVolumeUsageNotification, cardPrState=cardPrState, cadEquipmentMib=cadEquipmentMib, cardCarrierFwVersion=cardCarrierFwVersion, shelfName=shelfName, cardSerialNum=cardSerialNum, portTxFlowControlDetected=portTxFlowControlDetected, cardNumber=cardNumber, auditResult=auditResult, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold)
