...
DuplexStatus, EqActionType, PrimaryState, FirmwareSource, DiskVolumeUsageLevel, FlowControlMode, PortType, CardId, PortDetectedMode, PortMode, CardType, AdminState, ShelfId, CardSubType, PicType, UnknownState, SecondaryState, PortId = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "EqActionType", "PrimaryState", "FirmwareSource", "DiskVolumeUsageLevel", "FlowControlMode", "PortType", "CardId", "PortDetectedMode", "PortMode", "CardType", "AdminState", "ShelfId", "CardSubType", "PicType", "UnknownState", "SecondaryState", "PortId")
...
RowStatus, TruthValue, TimeStamp, DateAndTime, TextualConvention, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TruthValue", "TimeStamp", "DateAndTime", "TextualConvention", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portDescription=portDescription, cmResetStatus=cmResetStatus, cardLastResetReason=cardLastResetReason, cardRamSize=cardRamSize, shelfNumber=shelfNumber, cmResetClearNotification=cmResetClearNotification, auditEntry=auditEntry, systemGeneral=systemGeneral, equipmentState=equipmentState, cardDetected=cardDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFwVersion=cardFwVersion, diskVolumeUsageLevel=diskVolumeUsageLevel, cmIpAddress=cmIpAddress, cardTestResultDesc=cardTestResultDesc, auditId=auditId, equipmentDiagGroup=equipmentDiagGroup, diskVolumeShelfId=diskVolumeShelfId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portAction=portAction, cardTempNoReportNotification=cardTempNoReportNotification, cardTestResult=cardTestResult, cardSerialNum=cardSerialNum, cardFwUpdateStatus=cardFwUpdateStatus, cardPrStateChange=cardPrStateChange, PYSNMP_MODULE_ID=cadEquipmentMib, portMode=portMode, cardCPLDVersion=cardCPLDVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardShelfId=cardShelfId, cardSecState=cardSecState, cardCarrierFwVersion=cardCarrierFwVersion, cardSubDetected=cardSubDetected, cardTestName=cardTestName, cardTestResultNotification=cardTestResultNotification, cardSubType=cardSubType, remainInDiagMode=remainInDiagMode, cardLastChangeTime=cardLastChangeTime, diskVolumeId=diskVolumeId, equipmentPortGroup=equipmentPortGroup, TestSchedule=TestSchedule, cardTestDescription=cardTestDescription, cardDplxStatus=cardDplxStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestCommand=cardTestCommand, cardDetectedChange=cardDetectedChange, cardFlashSize=cardFlashSize, cardTestTime=cardTestTime, cardTestSchedule=cardTestSchedule, cardSpareGroupId=cardSpareGroupId, diskVolumeCardId=diskVolumeCardId, diskVolumeFileName=diskVolumeFileName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentStateGroup=equipmentStateGroup, diskDriveNumber=diskDriveNumber, portEntry=portEntry, verboseLevel=verboseLevel, cmResetInfo=cmResetInfo, diskVolumeUsageNotification=diskVolumeUsageNotification, auditShelfId=auditShelfId, TestResult=TestResult, portType=portType, cardShelfTestIndex=cardShelfTestIndex, portLastChangeTime=portLastChangeTime, auditFailedCount=auditFailedCount, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeTable=diskVolumeTable, cardTrapInh=cardTrapInh, cardTempOverHeatNotification=cardTempOverHeatNotification, cardEntry=cardEntry, portPrState=portPrState, equipmentShelfGroup=equipmentShelfGroup, equipmentCardTestGroup=equipmentCardTestGroup, auditAutoScheduling=auditAutoScheduling, equipmentDiag=equipmentDiag, TestCommand=TestCommand, portDplxStatus=portDplxStatus, TestTransactionId=TestTransactionId, cardCarrierSerialNum=cardCarrierSerialNum, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEvRecoveryEnabled=errEvRecoveryEnabled, TestScheduleCommand=TestScheduleCommand, cardUpTime=cardUpTime, shelfObjects=shelfObjects, cardFpgaSource=cardFpgaSource, cardCpuSpeed=cardCpuSpeed, portCurrDsPower=portCurrDsPower, portRxFlowControlDetected=portRxFlowControlDetected, auditLogOutput=auditLogOutput, diskVolumeSize=diskVolumeSize, auditResult=auditResult, portTable=portTable, diskVolumeTrapInh=diskVolumeTrapInh, portDplxStatusChange=portDplxStatusChange, portRxFlowControlMode=portRxFlowControlMode, portNumber=portNumber, cardBusSpeed=cardBusSpeed, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicSerialNum=cardPicSerialNum, auditCardId=auditCardId, diskVolumeDiskSize=diskVolumeDiskSize, cardLastBootSource=cardLastBootSource, cardTestIndex=cardTestIndex, systemKey=systemKey, equipmentAudit=equipmentAudit, equipmentCardGroup=equipmentCardGroup, TestType=TestType, cardLastBootVersion=cardLastBootVersion, portMaxDsPower=portMaxDsPower, systemGeneralGroup=systemGeneralGroup, portTrapInh=portTrapInh, cardHwVersion=cardHwVersion, portDocsIfIndex=portDocsIfIndex, portConnectorId=portConnectorId, cmMacAddress=cmMacAddress, cardSwVersion=cardSwVersion, cardTestId=cardTestId, cmResetReason=cmResetReason, cardCpuType=cardCpuType, cmUChannelID=cmUChannelID, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSecStateChange=cardSecStateChange, cardNumber=cardNumber, cadEquipmentMibGroup=cadEquipmentMibGroup, cmDevice=cmDevice, auditName=auditName, errEvLogLevel=errEvLogLevel, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, consoleOutput=consoleOutput, portDetectedMode=portDetectedMode, errEvLoggingEnabled=errEvLoggingEnabled, shelfName=shelfName, diskVolumeEntry=diskVolumeEntry, auditTotalPassedCount=auditTotalPassedCount, cardTestType=cardTestType, cardAdminState=cardAdminState, portMacIfIndex=portMacIfIndex, portCardId=portCardId, cardDplxStatusChange=cardDplxStatusChange, diskVolumeGroup=diskVolumeGroup, trapCounter=trapCounter, portSecStateChange=portSecStateChange, cardPrState=cardPrState, cardPicHwVersion=cardPicHwVersion, diskVolumeName=diskVolumeName, trapSeverity=trapSeverity, diagTestId=diagTestId, auditCycleCount=auditCycleCount, portNumChans=portNumChans, cardTestTable=cardTestTable, equipmentTbl=equipmentTbl, cardTestScheduleCommand=cardTestScheduleCommand, auditDescription=auditDescription, cardTestTransId=cardTestTransId, portAdminState=portAdminState, equipmentAuditGroup=equipmentAuditGroup, auditTable=auditTable, errEventId=errEventId, cardAction=cardAction, shelfSwVersion=shelfSwVersion, portMacAddress=portMacAddress, downstreamPowerLoss=downstreamPowerLoss, portCardSubType=portCardSubType, portShelfId=portShelfId, eqDiagConfig=eqDiagConfig, auditPassedCount=auditPassedCount, TestId=TestId, cardNumPorts=cardNumPorts, cardId=cardId, cardPicDetected=cardPicDetected, systemClock=systemClock, cardBootVersion=cardBootVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTable=cardTable, cmResetUpTime=cmResetUpTime, cardName=cardName, portBgpId=portBgpId, cmVendor=cmVendor, diskVolumeDriveId=diskVolumeDriveId, cmResetNotification=cmResetNotification, equipmentError=equipmentError, portTxFlowControlDetected=portTxFlowControlDetected, auditTime=auditTime, cmPrimarySID=cmPrimarySID, cardTestEntry=cardTestEntry, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMib=cadEquipmentMib, auditStatus=auditStatus, portMinDsPower=portMinDsPower, cadEquipmentMibConformance=cadEquipmentMibConformance, auditCommand=auditCommand, cardTemperature=cardTemperature, cardType=cardType, equipmentTraps=equipmentTraps, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditLogThrottle=auditLogThrottle, portPrStateChange=portPrStateChange, portId=portId, cardSpareGroupMode=cardSpareGroupMode, auditTotalFailedCount=auditTotalFailedCount, portSecState=portSecState, errEvRowStatus=errEvRowStatus, diskVolumeNumber=diskVolumeNumber)
