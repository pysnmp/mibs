...
DuplexStatus, PortDetectedMode, PortMode, AdminState, PortId, PrimaryState, UnknownState, EqActionType, FlowControlMode, DiskVolumeUsageLevel, PortType, PicType, ShelfId, SecondaryState, FirmwareSource, CardId, CardSubType, CardType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortDetectedMode", "PortMode", "AdminState", "PortId", "PrimaryState", "UnknownState", "EqActionType", "FlowControlMode", "DiskVolumeUsageLevel", "PortType", "PicType", "ShelfId", "SecondaryState", "FirmwareSource", "CardId", "CardSubType", "CardType")
...
TimeStamp, RowStatus, MacAddress, TruthValue, DateAndTime, TextualConvention, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "RowStatus", "MacAddress", "TruthValue", "DateAndTime", "TextualConvention", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestResultDesc=cardTestResultDesc, portConnectorId=portConnectorId, diskVolumeDiskSize=diskVolumeDiskSize, portPrState=portPrState, systemClock=systemClock, fpgaErrorEventTable=fpgaErrorEventTable, cardSecState=cardSecState, auditStatus=auditStatus, remainInDiagMode=remainInDiagMode, cardTestResult=cardTestResult, cardShelfId=cardShelfId, cardTestTable=cardTestTable, cardTestId=cardTestId, auditEntry=auditEntry, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeTable=diskVolumeTable, cardTestTransId=cardTestTransId, portAction=portAction, cardTestResultNotification=cardTestResultNotification, cmResetInfo=cmResetInfo, cardTestTime=cardTestTime, auditAutoScheduling=auditAutoScheduling, cardCarrierHwVersion=cardCarrierHwVersion, portCardSubType=portCardSubType, portTxFlowControlDetected=portTxFlowControlDetected, cardUpTime=cardUpTime, cardFpgaSource=cardFpgaSource, auditId=auditId, cardAdminState=cardAdminState, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeFileName=diskVolumeFileName, equipmentTbl=equipmentTbl, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecStateChange=portSecStateChange, systemKey=systemKey, cardId=cardId, equipmentCardTestGroup=equipmentCardTestGroup, cardTemperature=cardTemperature, downstreamPowerLoss=downstreamPowerLoss, portEntry=portEntry, cmResetStatus=cmResetStatus, TestId=TestId, cardPicDetected=cardPicDetected, cardFwVersion=cardFwVersion, cardTemperatureHighError=cardTemperatureHighError, auditTotalFailedCount=auditTotalFailedCount, cardType=cardType, portDplxStatus=portDplxStatus, verboseLevel=verboseLevel, equipmentError=equipmentError, cardDplxStatus=cardDplxStatus, portNumber=portNumber, TestScheduleCommand=TestScheduleCommand, cardNumPorts=cardNumPorts, portDescription=portDescription, diskVolumeTrapInh=diskVolumeTrapInh, auditLogOutput=auditLogOutput, portRxFlowControlDetected=portRxFlowControlDetected, shelfNumber=shelfNumber, cardPicHwVersion=cardPicHwVersion, systemGeneral=systemGeneral, equipmentCardGroup=equipmentCardGroup, diskVolumeEntry=diskVolumeEntry, portBgpId=portBgpId, cardTestCommand=cardTestCommand, diskVolumeGroup=diskVolumeGroup, cardDetected=cardDetected, cmPrimarySID=cmPrimarySID, shelfName=shelfName, cardTemperatureHighWarn=cardTemperatureHighWarn, cmVendor=cmVendor, cardSecStateChange=cardSecStateChange, cmRegistrationNotification=cmRegistrationNotification, auditCommand=auditCommand, portCardId=portCardId, cardBootVersion=cardBootVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, auditShelfId=auditShelfId, cmUChannelID=cmUChannelID, equipmentAuditGroup=equipmentAuditGroup, cardLastChangeTime=cardLastChangeTime, shelfSwVersion=shelfSwVersion, auditTime=auditTime, consoleOutput=consoleOutput, diskVolumeId=diskVolumeId, portDetectedMode=portDetectedMode, diskVolumeName=diskVolumeName, errEventId=errEventId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDplxStatusChange=portDplxStatusChange, cardDplxStatusChange=cardDplxStatusChange, equipmentDiag=equipmentDiag, cardSerialNum=cardSerialNum, cmMacAddress=cmMacAddress, fpgaErrorEventEntry=fpgaErrorEventEntry, cmDevice=cmDevice, cmIpAddress=cmIpAddress, portMaxDsPower=portMaxDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, PYSNMP_MODULE_ID=cadEquipmentMib, TestCommand=TestCommand, cardTestSchedule=cardTestSchedule, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentPortGroup=equipmentPortGroup, cardBusSpeed=cardBusSpeed, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, systemGeneralGroup=systemGeneralGroup, cardNumber=cardNumber, portRxFlowControlMode=portRxFlowControlMode, TestTransactionId=TestTransactionId, cardTestIndex=cardTestIndex, auditFailedCount=auditFailedCount, equipmentStateGroup=equipmentStateGroup, portShelfId=portShelfId, trapCounter=trapCounter, cardName=cardName, diskVolumeSize=diskVolumeSize, portMacAddress=portMacAddress, cardSubType=cardSubType, cardLastResetReason=cardLastResetReason, auditName=auditName, portType=portType, cardCpuType=cardCpuType, portSecState=portSecState, portDocsIfIndex=portDocsIfIndex, diskVolumeShelfId=diskVolumeShelfId, auditTotalPassedCount=auditTotalPassedCount, cardTestEntry=cardTestEntry, equipmentTraps=equipmentTraps, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageNotification=diskVolumeUsageNotification, portMode=portMode, auditCycleCount=auditCycleCount, trapSeverity=trapSeverity, cardTrapInh=cardTrapInh, cardFwUpdateStatus=cardFwUpdateStatus, cardAction=cardAction, portTable=portTable, cardSpareGroupMode=cardSpareGroupMode, portTxFlowControlMode=portTxFlowControlMode, eqDiagConfig=eqDiagConfig, portAdminState=portAdminState, cardPicSerialNum=cardPicSerialNum, portMacIfIndex=portMacIfIndex, diskVolumeCardId=diskVolumeCardId, diskDriveNumber=diskDriveNumber, cardTestDescription=cardTestDescription, diagTestId=diagTestId, cardDetectedChange=cardDetectedChange, equipmentState=equipmentState, TestResult=TestResult, portMinDsPower=portMinDsPower, portPrStateChange=portPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, cadEquipmentMib=cadEquipmentMib, cardCarrierFwVersion=cardCarrierFwVersion, auditTable=auditTable, cardTable=cardTable, portCurrDsPower=portCurrDsPower, diskVolumeNumber=diskVolumeNumber, cmResetReason=cmResetReason, cardPrStateChange=cardPrStateChange, cmResetNotification=cmResetNotification, portId=portId, cadEquipmentMibConformance=cadEquipmentMibConformance, errEvLoggingEnabled=errEvLoggingEnabled, cardSpareGroupId=cardSpareGroupId, cardCarrierSerialNum=cardCarrierSerialNum, cardSubDetected=cardSubDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCPLDVersion=cardCPLDVersion, errEvRowStatus=errEvRowStatus, TestType=TestType, cardPrState=cardPrState, cardRamSize=cardRamSize, cmResetUpTime=cmResetUpTime, errEvLogLevel=errEvLogLevel, auditLogThrottle=auditLogThrottle, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, shelfObjects=shelfObjects, cardTestName=cardTestName, cardFlashSize=cardFlashSize, cmResetClearNotification=cmResetClearNotification, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardHwVersion=cardHwVersion, cardEntry=cardEntry, equipmentAudit=equipmentAudit, auditResult=auditResult, auditPassedCount=auditPassedCount, cardTestScheduleCommand=cardTestScheduleCommand, cardShelfTestIndex=cardShelfTestIndex, portNumChans=portNumChans, equipmentShelfGroup=equipmentShelfGroup, portLastChangeTime=portLastChangeTime, cardTestType=cardTestType, cardSwVersion=cardSwVersion, TestSchedule=TestSchedule, equipmentDiagGroup=equipmentDiagGroup, portTrapInh=portTrapInh, cardCpuSpeed=cardCpuSpeed, diskVolumeDriveId=diskVolumeDriveId, cardLastBootSource=cardLastBootSource, auditCardId=auditCardId, auditDescription=auditDescription)
