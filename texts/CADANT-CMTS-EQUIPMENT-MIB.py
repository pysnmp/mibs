...
PicType, UnknownState, AdminState, PrimaryState, SecondaryState, ShelfId, FlowControlMode, DuplexStatus, CardId, DiskVolumeUsageLevel, PortId, FirmwareSource, PortMode, PortType, PortDetectedMode, CardSubType, CardType, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PicType", "UnknownState", "AdminState", "PrimaryState", "SecondaryState", "ShelfId", "FlowControlMode", "DuplexStatus", "CardId", "DiskVolumeUsageLevel", "PortId", "FirmwareSource", "PortMode", "PortType", "PortDetectedMode", "CardSubType", "CardType", "EqActionType")
...
TruthValue, RowStatus, DateAndTime, MacAddress, TimeStamp, DisplayString, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "DateAndTime", "MacAddress", "TimeStamp", "DisplayString", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFwUpdateStatus=cardFwUpdateStatus, PYSNMP_MODULE_ID=cadEquipmentMib, auditName=auditName, cardLastChangeTime=cardLastChangeTime, equipmentCardGroup=equipmentCardGroup, portNumber=portNumber, cardBootVersion=cardBootVersion, cardCarrierFwVersion=cardCarrierFwVersion, cardFwVersion=cardFwVersion, cardPrState=cardPrState, errEventId=errEventId, portPrState=portPrState, portSecState=portSecState, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfSwVersion=shelfSwVersion, cardName=cardName, cardTestSchedule=cardTestSchedule, cardPicDetected=cardPicDetected, trapSeverity=trapSeverity, cardTestDescription=cardTestDescription, cardTestTable=cardTestTable, portRxFlowControlMode=portRxFlowControlMode, errEvRecoveryEnabled=errEvRecoveryEnabled, auditTotalFailedCount=auditTotalFailedCount, cardTestName=cardTestName, equipmentPortGroup=equipmentPortGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardTable=cardTable, auditDescription=auditDescription, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmResetReason=cmResetReason, cmResetClearNotification=cmResetClearNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portType=portType, auditAutoScheduling=auditAutoScheduling, auditStatus=auditStatus, auditId=auditId, diskVolumeFileName=diskVolumeFileName, cardAction=cardAction, cardDplxStatus=cardDplxStatus, cardDetectedChange=cardDetectedChange, trapCounter=trapCounter, remainInDiagMode=remainInDiagMode, auditTime=auditTime, errEvRowStatus=errEvRowStatus, cardPicHwVersion=cardPicHwVersion, cardId=cardId, TestResult=TestResult, cardNumPorts=cardNumPorts, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeNumber=diskVolumeNumber, shelfNumber=shelfNumber, cardTestResultDesc=cardTestResultDesc, cardTemperatureHighError=cardTemperatureHighError, portMacIfIndex=portMacIfIndex, diskVolumeTable=diskVolumeTable, diskVolumeSize=diskVolumeSize, fpgaErrorEventTable=fpgaErrorEventTable, auditTotalPassedCount=auditTotalPassedCount, cardShelfTestIndex=cardShelfTestIndex, cardTestIndex=cardTestIndex, systemGeneral=systemGeneral, fpgaErrorEventGroup=fpgaErrorEventGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portDplxStatus=portDplxStatus, cardTestEntry=cardTestEntry, portDplxStatusChange=portDplxStatusChange, cardSubType=cardSubType, cardDetected=cardDetected, cardLastBootSource=cardLastBootSource, TestSchedule=TestSchedule, portMaxDsPower=portMaxDsPower, cardTestTransId=cardTestTransId, errEvLoggingEnabled=errEvLoggingEnabled, portConnectorId=portConnectorId, portDetectedMode=portDetectedMode, cardLastBootVersion=cardLastBootVersion, cmResetInfo=cmResetInfo, cadEquipmentMib=cadEquipmentMib, cardFpgaSource=cardFpgaSource, portMacAddress=portMacAddress, cardLastResetReason=cardLastResetReason, cardTempNoReportNotification=cardTempNoReportNotification, downstreamPowerLoss=downstreamPowerLoss, auditResult=auditResult, cardSecStateChange=cardSecStateChange, cmMacAddress=cmMacAddress, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardSecState=cardSecState, portTable=portTable, TestTransactionId=TestTransactionId, portShelfId=portShelfId, equipmentDiagGroup=equipmentDiagGroup, portDocsIfIndex=portDocsIfIndex, cardType=cardType, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeGroup=diskVolumeGroup, portNumChans=portNumChans, equipmentError=equipmentError, auditCycleCount=auditCycleCount, auditFailedCount=auditFailedCount, cardSubDetected=cardSubDetected, cardHwVersion=cardHwVersion, cmVendor=cmVendor, portId=portId, consoleOutput=consoleOutput, diskDriveNumber=diskDriveNumber, cmPrimarySID=cmPrimarySID, cmResetNotification=cmResetNotification, errEvLogLevel=errEvLogLevel, systemGeneralGroup=systemGeneralGroup, TestId=TestId, TestType=TestType, portAdminState=portAdminState, portCurrDsPower=portCurrDsPower, auditTable=auditTable, diskVolumeEntry=diskVolumeEntry, cardDplxStatusChange=cardDplxStatusChange, cardAdminState=cardAdminState, cardSpareGroupId=cardSpareGroupId, cardUpTime=cardUpTime, auditCommand=auditCommand, cardTestResultNotification=cardTestResultNotification, diskVolumeTrapInh=diskVolumeTrapInh, auditPassedCount=auditPassedCount, cmDevice=cmDevice, diskVolumeShelfId=diskVolumeShelfId, cardBusSpeed=cardBusSpeed, cardTestId=cardTestId, portPrStateChange=portPrStateChange, cardTestTime=cardTestTime, eqDiagConfig=eqDiagConfig, auditLogThrottle=auditLogThrottle, equipmentAudit=equipmentAudit, cardTemperatureHighWarn=cardTemperatureHighWarn, cmUChannelID=cmUChannelID, cardFlashSize=cardFlashSize, portCardId=portCardId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSerialNum=cardSerialNum, cardTrapInh=cardTrapInh, cardCPLDVersion=cardCPLDVersion, cardCpuType=cardCpuType, portMinDsPower=portMinDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardShelfId=cardShelfId, shelfName=shelfName, cardTestCommand=cardTestCommand, portTxFlowControlDetected=portTxFlowControlDetected, cardTestType=cardTestType, auditEntry=auditEntry, cardSwVersion=cardSwVersion, cmResetStatus=cmResetStatus, diskVolumeName=diskVolumeName, equipmentTraps=equipmentTraps, auditShelfId=auditShelfId, equipmentShelfGroup=equipmentShelfGroup, systemKey=systemKey, TestScheduleCommand=TestScheduleCommand, portLastChangeTime=portLastChangeTime, equipmentTbl=equipmentTbl, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCpuSpeed=cardCpuSpeed, portCardSubType=portCardSubType, cardTemperature=cardTemperature, cardPicSerialNum=cardPicSerialNum, shelfObjects=shelfObjects, portTxFlowControlMode=portTxFlowControlMode, verboseLevel=verboseLevel, portEntry=portEntry, cardPrStateChange=cardPrStateChange, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResult=cardTestResult, systemClock=systemClock, portSecStateChange=portSecStateChange, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, portTrapInh=portTrapInh, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentStateGroup=equipmentStateGroup, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeDriveId=diskVolumeDriveId, cardTestScheduleCommand=cardTestScheduleCommand, equipmentAuditGroup=equipmentAuditGroup, TestCommand=TestCommand, cmResetUpTime=cmResetUpTime, diskVolumeCardId=diskVolumeCardId, equipmentDiag=equipmentDiag, portDescription=portDescription, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portBgpId=portBgpId, cardSpareGroupMode=cardSpareGroupMode, cardEntry=cardEntry, equipmentState=equipmentState, cmIpAddress=cmIpAddress, portAction=portAction, cardNumber=cardNumber, cardRamSize=cardRamSize, auditLogOutput=auditLogOutput, diagTestId=diagTestId, auditCardId=auditCardId, diskVolumeId=diskVolumeId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portMode=portMode)
