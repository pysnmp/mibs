...
PortMode, ShelfId, PrimaryState, CardType, FirmwareSource, PortType, DiskVolumeUsageLevel, PicType, FlowControlMode, PortDetectedMode, AdminState, EqActionType, CardId, DuplexStatus, UnknownState, PortId, SecondaryState, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PortMode", "ShelfId", "PrimaryState", "CardType", "FirmwareSource", "PortType", "DiskVolumeUsageLevel", "PicType", "FlowControlMode", "PortDetectedMode", "AdminState", "EqActionType", "CardId", "DuplexStatus", "UnknownState", "PortId", "SecondaryState", "CardSubType")
...
TimeStamp, RowStatus, DisplayString, TextualConvention, TruthValue, DateAndTime, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "RowStatus", "DisplayString", "TextualConvention", "TruthValue", "DateAndTime", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portEntry=portEntry, auditLogOutput=auditLogOutput, auditLogThrottle=auditLogThrottle, trapSeverity=trapSeverity, portNumber=portNumber, cardTestResultDesc=cardTestResultDesc, cmUChannelID=cmUChannelID, errEvLoggingEnabled=errEvLoggingEnabled, portMinDsPower=portMinDsPower, cardCarrierHwVersion=cardCarrierHwVersion, cardCarrierSerialNum=cardCarrierSerialNum, errEventId=errEventId, eqDiagConfig=eqDiagConfig, shelfNumber=shelfNumber, portDocsIfIndex=portDocsIfIndex, equipmentCardTestGroup=equipmentCardTestGroup, portConnectorId=portConnectorId, cardTestSchedule=cardTestSchedule, equipmentAudit=equipmentAudit, cardCPLDVersion=cardCPLDVersion, auditCommand=auditCommand, cardTemperature=cardTemperature, cardTestEntry=cardTestEntry, TestResult=TestResult, equipmentDiag=equipmentDiag, cardSpareGroupId=cardSpareGroupId, cardTempOverHeatNotification=cardTempOverHeatNotification, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentDiagGroup=equipmentDiagGroup, cmRegistrationNotification=cmRegistrationNotification, diskVolumeName=diskVolumeName, cmResetReason=cmResetReason, portDplxStatus=portDplxStatus, cmResetStatus=cmResetStatus, cardDplxStatusChange=cardDplxStatusChange, equipmentShelfGroup=equipmentShelfGroup, cardSecStateChange=cardSecStateChange, fpgaErrorEventEntry=fpgaErrorEventEntry, portCurrDsPower=portCurrDsPower, cardSwVersion=cardSwVersion, portAction=portAction, diskVolumeUsageLevel=diskVolumeUsageLevel, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, trapCounter=trapCounter, equipmentAuditGroup=equipmentAuditGroup, portTable=portTable, portBgpId=portBgpId, diagTestId=diagTestId, auditDescription=auditDescription, cardNumPorts=cardNumPorts, portShelfId=portShelfId, TestType=TestType, cardLastResetReason=cardLastResetReason, systemClock=systemClock, equipmentPortGroup=equipmentPortGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cmDevice=cmDevice, cardEntry=cardEntry, cardCpuType=cardCpuType, cardPrStateChange=cardPrStateChange, auditName=auditName, shelfName=shelfName, errEvLogLevel=errEvLogLevel, cardAdminState=cardAdminState, auditCycleCount=auditCycleCount, cardAction=cardAction, portTrapInh=portTrapInh, portPrStateChange=portPrStateChange, systemKey=systemKey, cardPicHwVersion=cardPicHwVersion, cardPrState=cardPrState, TestTransactionId=TestTransactionId, cardSubType=cardSubType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEvRowStatus=errEvRowStatus, diskVolumeTable=diskVolumeTable, equipmentTraps=equipmentTraps, cardNumber=cardNumber, cardBusSpeed=cardBusSpeed, portId=portId, auditTime=auditTime, diskVolumeId=diskVolumeId, TestScheduleCommand=TestScheduleCommand, portMacIfIndex=portMacIfIndex, auditStatus=auditStatus, portRxFlowControlMode=portRxFlowControlMode, portSecStateChange=portSecStateChange, cardSubDetected=cardSubDetected, cardCpuSpeed=cardCpuSpeed, diskVolumeFileName=diskVolumeFileName, portDescription=portDescription, cadEquipmentMib=cadEquipmentMib, equipmentTbl=equipmentTbl, TestCommand=TestCommand, cardLastChangeTime=cardLastChangeTime, cardSpareGroupMode=cardSpareGroupMode, diskVolumeTrapInh=diskVolumeTrapInh, consoleOutput=consoleOutput, cmResetNotification=cmResetNotification, cardTestTable=cardTestTable, portDetectedMode=portDetectedMode, portRxFlowControlDetected=portRxFlowControlDetected, verboseLevel=verboseLevel, portPrState=portPrState, fpgaErrorEventTable=fpgaErrorEventTable, cardFpgaSource=cardFpgaSource, equipmentError=equipmentError, cardPicSerialNum=cardPicSerialNum, portLastChangeTime=portLastChangeTime, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portTxFlowControlDetected=portTxFlowControlDetected, cmResetClearNotification=cmResetClearNotification, portType=portType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardFlashSize=cardFlashSize, portMaxDsPower=portMaxDsPower, auditFailedCount=auditFailedCount, equipmentCardGroup=equipmentCardGroup, systemGeneral=systemGeneral, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestResult=cardTestResult, auditPassedCount=auditPassedCount, cardType=cardType, cardDetected=cardDetected, equipmentState=equipmentState, cardTemperatureHighWarn=cardTemperatureHighWarn, portMode=portMode, cardTestId=cardTestId, cardBootVersion=cardBootVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, auditAutoScheduling=auditAutoScheduling, cardSecState=cardSecState, cardLastBootVersion=cardLastBootVersion, cardPicDetected=cardPicDetected, cmResetUpTime=cmResetUpTime, portAdminState=portAdminState, cadEquipmentMibConformance=cadEquipmentMibConformance, diskDriveNumber=diskDriveNumber, cardHwVersion=cardHwVersion, cardTestIndex=cardTestIndex, cardTestCommand=cardTestCommand, cardTestDescription=cardTestDescription, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditTable=auditTable, cardFwVersion=cardFwVersion, cardId=cardId, auditCardId=auditCardId, cardTrapInh=cardTrapInh, diskVolumeCardId=diskVolumeCardId, cmIpAddress=cmIpAddress, cardSerialNum=cardSerialNum, diskVolumeShelfId=diskVolumeShelfId, auditTotalFailedCount=auditTotalFailedCount, cardShelfTestIndex=cardShelfTestIndex, cardCarrierFwVersion=cardCarrierFwVersion, cardTemperatureHighError=cardTemperatureHighError, diskVolumeEntry=diskVolumeEntry, portSecState=portSecState, cardTable=cardTable, cardTestResultNotification=cardTestResultNotification, auditTotalPassedCount=auditTotalPassedCount, portNumChans=portNumChans, cardTestTransId=cardTestTransId, diskVolumeSize=diskVolumeSize, errEvRecoveryEnabled=errEvRecoveryEnabled, portDplxStatusChange=portDplxStatusChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestName=cardTestName, shelfObjects=shelfObjects, cardTestType=cardTestType, cardTestTime=cardTestTime, diskVolumeGroup=diskVolumeGroup, equipmentStateGroup=equipmentStateGroup, shelfSwVersion=shelfSwVersion, portCardId=portCardId, cardTestScheduleCommand=cardTestScheduleCommand, auditEntry=auditEntry, auditResult=auditResult, diskVolumeNumber=diskVolumeNumber, diskVolumeDriveId=diskVolumeDriveId, cardDplxStatus=cardDplxStatus, diskVolumeDiskSize=diskVolumeDiskSize, cardUpTime=cardUpTime, auditId=auditId, cmMacAddress=cmMacAddress, cmResetInfo=cmResetInfo, cadEquipmentMibGroup=cadEquipmentMibGroup, cardName=cardName, systemGeneralGroup=systemGeneralGroup, remainInDiagMode=remainInDiagMode, TestSchedule=TestSchedule, cardTempNoReportNotification=cardTempNoReportNotification, cardDetectedChange=cardDetectedChange, portCardSubType=portCardSubType, cmPrimarySID=cmPrimarySID, TestId=TestId, portMacAddress=portMacAddress, auditShelfId=auditShelfId, cardLastBootSource=cardLastBootSource, cmVendor=cmVendor, downstreamPowerLoss=downstreamPowerLoss, cardRamSize=cardRamSize, cardShelfId=cardShelfId, PYSNMP_MODULE_ID=cadEquipmentMib)
