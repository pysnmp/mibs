...
FirmwareSource, PortType, CardSubType, AdminState, EqActionType, PicType, DuplexStatus, PortMode, ShelfId, UnknownState, PortId, PrimaryState, FlowControlMode, CardId, CardType, SecondaryState, PortDetectedMode, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "PortType", "CardSubType", "AdminState", "EqActionType", "PicType", "DuplexStatus", "PortMode", "ShelfId", "UnknownState", "PortId", "PrimaryState", "FlowControlMode", "CardId", "CardType", "SecondaryState", "PortDetectedMode", "DiskVolumeUsageLevel")
...
TruthValue, TimeStamp, DateAndTime, MacAddress, TextualConvention, RowStatus, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TimeStamp", "DateAndTime", "MacAddress", "TextualConvention", "RowStatus", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestType=TestType, cardNumber=cardNumber, portDocsIfIndex=portDocsIfIndex, cardSubDetected=cardSubDetected, portMacAddress=portMacAddress, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibConformance=cadEquipmentMibConformance, cardRamSize=cardRamSize, portSecStateChange=portSecStateChange, cmResetNotification=cmResetNotification, cmMacAddress=cmMacAddress, cardDplxStatus=cardDplxStatus, portEntry=portEntry, cardTestTime=cardTestTime, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResultNotification=cardTestResultNotification, cardAction=cardAction, portTrapInh=portTrapInh, diskVolumeGroup=diskVolumeGroup, cardCarrierSerialNum=cardCarrierSerialNum, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeCardId=diskVolumeCardId, equipmentDiag=equipmentDiag, auditDescription=auditDescription, portNumber=portNumber, cardDetected=cardDetected, portLastChangeTime=portLastChangeTime, portShelfId=portShelfId, portId=portId, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageNotification=diskVolumeUsageNotification, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardSecState=cardSecState, cardTestName=cardTestName, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestTable=cardTestTable, cardTestCommand=cardTestCommand, cardTempOverHeatNotification=cardTempOverHeatNotification, auditTotalFailedCount=auditTotalFailedCount, auditTime=auditTime, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, portSecState=portSecState, cmDevice=cmDevice, cardPicHwVersion=cardPicHwVersion, equipmentCardTestGroup=equipmentCardTestGroup, cardSubType=cardSubType, cardTestDescription=cardTestDescription, cardShelfTestIndex=cardShelfTestIndex, auditCycleCount=auditCycleCount, cardId=cardId, cmResetReason=cmResetReason, shelfObjects=shelfObjects, cardNumPorts=cardNumPorts, cardCpuSpeed=cardCpuSpeed, cardTestResult=cardTestResult, portType=portType, auditStatus=auditStatus, portDescription=portDescription, diskVolumeId=diskVolumeId, cardTestEntry=cardTestEntry, portMaxDsPower=portMaxDsPower, portNumChans=portNumChans, cardTestTransId=cardTestTransId, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCarrierFwVersion=cardCarrierFwVersion, diagTestId=diagTestId, diskVolumeUsageLevel=diskVolumeUsageLevel, auditName=auditName, shelfSwVersion=shelfSwVersion, equipmentAudit=equipmentAudit, TestScheduleCommand=TestScheduleCommand, equipmentCardGroup=equipmentCardGroup, portAction=portAction, portCurrDsPower=portCurrDsPower, cardShelfId=cardShelfId, cardTestType=cardTestType, portTable=portTable, cardBusSpeed=cardBusSpeed, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeNumber=diskVolumeNumber, TestResult=TestResult, TestCommand=TestCommand, trapSeverity=trapSeverity, cardLastChangeTime=cardLastChangeTime, remainInDiagMode=remainInDiagMode, cardTempNoReportNotification=cardTempNoReportNotification, equipmentState=equipmentState, portConnectorId=portConnectorId, eqDiagConfig=eqDiagConfig, auditTable=auditTable, auditCardId=auditCardId, cmResetStatus=cmResetStatus, diskDriveNumber=diskDriveNumber, portDplxStatus=portDplxStatus, portPrStateChange=portPrStateChange, errEvRowStatus=errEvRowStatus, downstreamPowerLoss=downstreamPowerLoss, cardTestIndex=cardTestIndex, auditShelfId=auditShelfId, cardTestResultDesc=cardTestResultDesc, cmResetClearNotification=cmResetClearNotification, cardLastBootSource=cardLastBootSource, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardHwVersion=cardHwVersion, equipmentTraps=equipmentTraps, portAdminState=portAdminState, cardFwUpdateStatus=cardFwUpdateStatus, cardFpgaSource=cardFpgaSource, TestSchedule=TestSchedule, portCardSubType=portCardSubType, diskVolumeTrapInh=diskVolumeTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, portMode=portMode, shelfNumber=shelfNumber, cardSpareGroupId=cardSpareGroupId, cardTestId=cardTestId, portCardId=portCardId, auditLogThrottle=auditLogThrottle, portRxFlowControlMode=portRxFlowControlMode, portMacIfIndex=portMacIfIndex, systemGeneralGroup=systemGeneralGroup, equipmentShelfGroup=equipmentShelfGroup, equipmentPortGroup=equipmentPortGroup, equipmentAuditGroup=equipmentAuditGroup, systemClock=systemClock, systemKey=systemKey, cardTemperature=cardTemperature, shelfName=shelfName, cardTable=cardTable, diskVolumeEntry=diskVolumeEntry, cardName=cardName, cardCPLDVersion=cardCPLDVersion, cardSpareGroupMode=cardSpareGroupMode, PYSNMP_MODULE_ID=cadEquipmentMib, cardFlashSize=cardFlashSize, auditCommand=auditCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentDiagGroup=equipmentDiagGroup, cardLastResetReason=cardLastResetReason, cardCpuType=cardCpuType, equipmentError=equipmentError, verboseLevel=verboseLevel, cardTestScheduleCommand=cardTestScheduleCommand, cmVendor=cmVendor, cardPicDetected=cardPicDetected, auditLogOutput=auditLogOutput, cmResetUpTime=cmResetUpTime, portTxFlowControlMode=portTxFlowControlMode, diskVolumeShelfId=diskVolumeShelfId, cmUChannelID=cmUChannelID, trapCounter=trapCounter, cmIpAddress=cmIpAddress, cardBootVersion=cardBootVersion, cardFwVersion=cardFwVersion, equipmentStateGroup=equipmentStateGroup, portBgpId=portBgpId, auditPassedCount=auditPassedCount, auditEntry=auditEntry, cardUpTime=cardUpTime, auditTotalPassedCount=auditTotalPassedCount, errEventId=errEventId, cardDplxStatusChange=cardDplxStatusChange, auditAutoScheduling=auditAutoScheduling, portPrState=portPrState, diskVolumeSize=diskVolumeSize, portTxFlowControlDetected=portTxFlowControlDetected, cardTestSchedule=cardTestSchedule, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeFileName=diskVolumeFileName, auditFailedCount=auditFailedCount, portDetectedMode=portDetectedMode, cardTrapInh=cardTrapInh, cmResetInfo=cmResetInfo, cardPrState=cardPrState, equipmentTbl=equipmentTbl, auditId=auditId, systemGeneral=systemGeneral, portMinDsPower=portMinDsPower, cardType=cardType, TestId=TestId, cardLastBootVersion=cardLastBootVersion, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmPrimarySID=cmPrimarySID, diskVolumeTable=diskVolumeTable, diskVolumeDriveId=diskVolumeDriveId, auditResult=auditResult, cadEquipmentMib=cadEquipmentMib, cardSwVersion=cardSwVersion, cardPrStateChange=cardPrStateChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestTransactionId=TestTransactionId, consoleOutput=consoleOutput, errEvLogLevel=errEvLogLevel, cardDetectedChange=cardDetectedChange, cardAdminState=cardAdminState, cardTemperatureHighError=cardTemperatureHighError, cardSecStateChange=cardSecStateChange, cardEntry=cardEntry, cardSerialNum=cardSerialNum, diskVolumeName=diskVolumeName)
