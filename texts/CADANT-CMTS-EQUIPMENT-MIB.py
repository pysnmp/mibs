...
PortMode, CardType, PortDetectedMode, PortType, UnknownState, SecondaryState, DiskVolumeUsageLevel, FirmwareSource, CardSubType, EqActionType, DuplexStatus, FlowControlMode, ShelfId, PicType, PortId, PrimaryState, AdminState, CardId = mibBuilder.importSymbols("CADANT-TC", "PortMode", "CardType", "PortDetectedMode", "PortType", "UnknownState", "SecondaryState", "DiskVolumeUsageLevel", "FirmwareSource", "CardSubType", "EqActionType", "DuplexStatus", "FlowControlMode", "ShelfId", "PicType", "PortId", "PrimaryState", "AdminState", "CardId")
...
TimeStamp, TextualConvention, DateAndTime, RowStatus, DisplayString, MacAddress, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TextualConvention", "DateAndTime", "RowStatus", "DisplayString", "MacAddress", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portLastChangeTime=portLastChangeTime, cardCarrierSerialNum=cardCarrierSerialNum, portMacIfIndex=portMacIfIndex, errEvRowStatus=errEvRowStatus, cmResetStatus=cmResetStatus, cardLastBootSource=cardLastBootSource, cardCPLDVersion=cardCPLDVersion, cardCarrierHwVersion=cardCarrierHwVersion, auditStatus=auditStatus, cmRegistrationNotification=cmRegistrationNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portAction=portAction, cardTestResult=cardTestResult, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditCardId=auditCardId, errEventId=errEventId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, verboseLevel=verboseLevel, auditPassedCount=auditPassedCount, errEvLogLevel=errEvLogLevel, cardPrState=cardPrState, portMaxDsPower=portMaxDsPower, cmIpAddress=cmIpAddress, cardTestResultNotification=cardTestResultNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeDriveId=diskVolumeDriveId, portCardId=portCardId, cardTestSchedule=cardTestSchedule, cardPicHwVersion=cardPicHwVersion, cardTestIndex=cardTestIndex, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeShelfId=diskVolumeShelfId, equipmentState=equipmentState, equipmentCardTestGroup=equipmentCardTestGroup, cmVendor=cmVendor, auditCommand=auditCommand, auditCycleCount=auditCycleCount, cmResetNotification=cmResetNotification, TestSchedule=TestSchedule, cardLastBootVersion=cardLastBootVersion, diskVolumeFileName=diskVolumeFileName, portMacAddress=portMacAddress, portType=portType, cardTestName=cardTestName, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestId=cardTestId, portTrapInh=portTrapInh, auditShelfId=auditShelfId, systemGeneral=systemGeneral, cardDplxStatus=cardDplxStatus, cardFpgaSource=cardFpgaSource, TestCommand=TestCommand, cardTable=cardTable, diskVolumeDiskSize=diskVolumeDiskSize, cmMacAddress=cmMacAddress, errEvRecoveryEnabled=errEvRecoveryEnabled, systemClock=systemClock, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeId=diskVolumeId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portDescription=portDescription, cardShelfId=cardShelfId, shelfName=shelfName, auditResult=auditResult, cardTestType=cardTestType, cardBusSpeed=cardBusSpeed, PYSNMP_MODULE_ID=cadEquipmentMib, portConnectorId=portConnectorId, TestType=TestType, equipmentTraps=equipmentTraps, cardSerialNum=cardSerialNum, equipmentStateGroup=equipmentStateGroup, cardFlashSize=cardFlashSize, trapSeverity=trapSeverity, diskVolumeGroup=diskVolumeGroup, cardSwVersion=cardSwVersion, portSecState=portSecState, cardSecStateChange=cardSecStateChange, cmResetUpTime=cmResetUpTime, cardTempOverHeatNotification=cardTempOverHeatNotification, portEntry=portEntry, portShelfId=portShelfId, cmUChannelID=cmUChannelID, auditDescription=auditDescription, cardSubDetected=cardSubDetected, diskVolumeTable=diskVolumeTable, diagTestId=diagTestId, portSecStateChange=portSecStateChange, cardTestResultDesc=cardTestResultDesc, cardPrStateChange=cardPrStateChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestEntry=cardTestEntry, diskVolumeNumber=diskVolumeNumber, equipmentAudit=equipmentAudit, portCardSubType=portCardSubType, portDplxStatus=portDplxStatus, portId=portId, cardTemperature=cardTemperature, auditLogOutput=auditLogOutput, equipmentError=equipmentError, equipmentPortGroup=equipmentPortGroup, cardTemperatureHighError=cardTemperatureHighError, auditName=auditName, cardTestDescription=cardTestDescription, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeSize=diskVolumeSize, portDplxStatusChange=portDplxStatusChange, systemGeneralGroup=systemGeneralGroup, shelfObjects=shelfObjects, equipmentTbl=equipmentTbl, portPrState=portPrState, cardName=cardName, cardBootVersion=cardBootVersion, cmResetInfo=cmResetInfo, equipmentDiag=equipmentDiag, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestTable=cardTestTable, cardLastChangeTime=cardLastChangeTime, portDocsIfIndex=portDocsIfIndex, cmResetReason=cmResetReason, portPrStateChange=portPrStateChange, cardTrapInh=cardTrapInh, portMinDsPower=portMinDsPower, equipmentShelfGroup=equipmentShelfGroup, cardDetected=cardDetected, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageLevel=diskVolumeUsageLevel, cardPicDetected=cardPicDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portRxFlowControlMode=portRxFlowControlMode, TestTransactionId=TestTransactionId, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliance=cadEquipmentMibCompliance, eqDiagConfig=eqDiagConfig, cardLastResetReason=cardLastResetReason, auditId=auditId, cardPicSerialNum=cardPicSerialNum, auditAutoScheduling=auditAutoScheduling, portNumber=portNumber, auditLogThrottle=auditLogThrottle, cmResetClearNotification=cmResetClearNotification, downstreamPowerLoss=downstreamPowerLoss, TestResult=TestResult, fpgaErrorEventTable=fpgaErrorEventTable, cardSubType=cardSubType, TestId=TestId, diskVolumeName=diskVolumeName, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestTime=cardTestTime, auditTime=auditTime, cardAdminState=cardAdminState, diskDriveNumber=diskDriveNumber, cardType=cardType, equipmentAuditGroup=equipmentAuditGroup, auditFailedCount=auditFailedCount, cardRamSize=cardRamSize, cardSpareGroupMode=cardSpareGroupMode, portBgpId=portBgpId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMib=cadEquipmentMib, cardDplxStatusChange=cardDplxStatusChange, cardCpuType=cardCpuType, cardCpuSpeed=cardCpuSpeed, trapCounter=trapCounter, portMode=portMode, remainInDiagMode=remainInDiagMode, auditTable=auditTable, cmDevice=cmDevice, cmPrimarySID=cmPrimarySID, auditTotalPassedCount=auditTotalPassedCount, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, shelfSwVersion=shelfSwVersion, portTable=portTable, consoleOutput=consoleOutput, cardId=cardId, diskVolumeCardId=diskVolumeCardId, portRxFlowControlDetected=portRxFlowControlDetected, cardSecState=cardSecState, portCurrDsPower=portCurrDsPower, cardTestScheduleCommand=cardTestScheduleCommand, systemKey=systemKey, fpgaErrorEventEntry=fpgaErrorEventEntry, cardUpTime=cardUpTime, cardCarrierFwVersion=cardCarrierFwVersion, cardDetectedChange=cardDetectedChange, cardTempNoReportNotification=cardTempNoReportNotification, portAdminState=portAdminState, cadEquipmentMibGroup=cadEquipmentMibGroup, cardAction=cardAction, cardSpareGroupId=cardSpareGroupId, equipmentDiagGroup=equipmentDiagGroup, cardNumPorts=cardNumPorts, cardNumber=cardNumber, auditEntry=auditEntry, portDetectedMode=portDetectedMode, TestScheduleCommand=TestScheduleCommand, shelfNumber=shelfNumber, portNumChans=portNumChans, cardFwVersion=cardFwVersion, cardTestCommand=cardTestCommand, cardTestTransId=cardTestTransId, cardEntry=cardEntry, diskVolumeEntry=diskVolumeEntry, portTxFlowControlMode=portTxFlowControlMode, equipmentCardGroup=equipmentCardGroup, cardHwVersion=cardHwVersion)
