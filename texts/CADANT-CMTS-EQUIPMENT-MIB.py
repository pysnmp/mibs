...
DiskVolumeUsageLevel, PortId, FirmwareSource, ShelfId, AdminState, CardType, FlowControlMode, CardSubType, CardId, PicType, PortDetectedMode, PrimaryState, SecondaryState, DuplexStatus, EqActionType, PortMode, PortType, UnknownState = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "PortId", "FirmwareSource", "ShelfId", "AdminState", "CardType", "FlowControlMode", "CardSubType", "CardId", "PicType", "PortDetectedMode", "PrimaryState", "SecondaryState", "DuplexStatus", "EqActionType", "PortMode", "PortType", "UnknownState")
...
TextualConvention, MacAddress, RowStatus, DisplayString, TimeStamp, DateAndTime, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "MacAddress", "RowStatus", "DisplayString", "TimeStamp", "DateAndTime", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeCardId=diskVolumeCardId, portAdminState=portAdminState, auditLogOutput=auditLogOutput, cardNumPorts=cardNumPorts, portNumChans=portNumChans, cardFlashSize=cardFlashSize, cardId=cardId, cmResetUpTime=cmResetUpTime, portLastChangeTime=portLastChangeTime, cardLastBootVersion=cardLastBootVersion, auditTotalFailedCount=auditTotalFailedCount, cardTestScheduleCommand=cardTestScheduleCommand, shelfSwVersion=shelfSwVersion, diskVolumeDiskSize=diskVolumeDiskSize, errEvLoggingEnabled=errEvLoggingEnabled, cardHwVersion=cardHwVersion, cmResetClearNotification=cmResetClearNotification, cardSubType=cardSubType, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, downstreamPowerLoss=downstreamPowerLoss, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardEntry=cardEntry, diskDriveNumber=diskDriveNumber, cardSpareGroupMode=cardSpareGroupMode, cardPicSerialNum=cardPicSerialNum, systemGeneral=systemGeneral, auditShelfId=auditShelfId, portId=portId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeTrapInh=diskVolumeTrapInh, auditCommand=auditCommand, auditTotalPassedCount=auditTotalPassedCount, systemKey=systemKey, cardTestCommand=cardTestCommand, cmIpAddress=cmIpAddress, cmDevice=cmDevice, cadEquipmentMib=cadEquipmentMib, portDescription=portDescription, diskVolumeUsageLevel=diskVolumeUsageLevel, portTrapInh=portTrapInh, portMacIfIndex=portMacIfIndex, cardFpgaSource=cardFpgaSource, portShelfId=portShelfId, portTable=portTable, portSecStateChange=portSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, auditCardId=auditCardId, portTxFlowControlDetected=portTxFlowControlDetected, auditCycleCount=auditCycleCount, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentTbl=equipmentTbl, cardTestTime=cardTestTime, portMaxDsPower=portMaxDsPower, cmUChannelID=cmUChannelID, cardCPLDVersion=cardCPLDVersion, cardFwVersion=cardFwVersion, TestResult=TestResult, PYSNMP_MODULE_ID=cadEquipmentMib, eqDiagConfig=eqDiagConfig, portCardId=portCardId, diagTestId=diagTestId, cardTestTable=cardTestTable, equipmentAuditGroup=equipmentAuditGroup, shelfObjects=shelfObjects, portAction=portAction, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSecStateChange=cardSecStateChange, portNumber=portNumber, portDplxStatus=portDplxStatus, portTxFlowControlMode=portTxFlowControlMode, portRxFlowControlMode=portRxFlowControlMode, cmResetNotification=cmResetNotification, diskVolumeId=diskVolumeId, equipmentAudit=equipmentAudit, equipmentShelfGroup=equipmentShelfGroup, cardDetected=cardDetected, portEntry=portEntry, auditLogThrottle=auditLogThrottle, trapCounter=trapCounter, auditFailedCount=auditFailedCount, cardPrStateChange=cardPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentCardGroup=equipmentCardGroup, portDetectedMode=portDetectedMode, equipmentState=equipmentState, cardTempNoReportNotification=cardTempNoReportNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardTemperatureHighWarn=cardTemperatureHighWarn, auditTable=auditTable, auditTime=auditTime, cmResetStatus=cmResetStatus, equipmentStateGroup=equipmentStateGroup, cardType=cardType, cardTemperatureHighError=cardTemperatureHighError, errEventId=errEventId, cardLastResetReason=cardLastResetReason, TestCommand=TestCommand, cmVendor=cmVendor, portPrStateChange=portPrStateChange, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCarrierSerialNum=cardCarrierSerialNum, shelfNumber=shelfNumber, cardTable=cardTable, equipmentDiagGroup=equipmentDiagGroup, cmResetReason=cmResetReason, cardTestName=cardTestName, TestScheduleCommand=TestScheduleCommand, diskVolumeFileName=diskVolumeFileName, cardTestId=cardTestId, cmRegistrationNotification=cmRegistrationNotification, consoleOutput=consoleOutput, cardPicDetected=cardPicDetected, cardAction=cardAction, cardName=cardName, cardTestEntry=cardTestEntry, portType=portType, cardCpuSpeed=cardCpuSpeed, cardBootVersion=cardBootVersion, diskVolumeGroup=diskVolumeGroup, equipmentCardTestGroup=equipmentCardTestGroup, cardSerialNum=cardSerialNum, auditAutoScheduling=auditAutoScheduling, cardShelfId=cardShelfId, diskVolumeDriveId=diskVolumeDriveId, equipmentDiag=equipmentDiag, cardFwUpdateStatus=cardFwUpdateStatus, TestId=TestId, shelfName=shelfName, TestSchedule=TestSchedule, cardNumber=cardNumber, auditEntry=auditEntry, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeSize=diskVolumeSize, cardCpuType=cardCpuType, portDplxStatusChange=portDplxStatusChange, diskVolumeTable=diskVolumeTable, cardDetectedChange=cardDetectedChange, portMinDsPower=portMinDsPower, auditPassedCount=auditPassedCount, auditId=auditId, cardLastBootSource=cardLastBootSource, systemClock=systemClock, trapSeverity=trapSeverity, equipmentTraps=equipmentTraps, auditStatus=auditStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, verboseLevel=verboseLevel, cardSpareGroupId=cardSpareGroupId, cardTestResult=cardTestResult, cmPrimarySID=cmPrimarySID, TestTransactionId=TestTransactionId, equipmentPortGroup=equipmentPortGroup, errEvRowStatus=errEvRowStatus, portMacAddress=portMacAddress, portSecState=portSecState, cardPrState=cardPrState, portCurrDsPower=portCurrDsPower, cardTestSchedule=cardTestSchedule, cardDplxStatusChange=cardDplxStatusChange, auditName=auditName, cardSecState=cardSecState, cardTemperature=cardTemperature, cardTestIndex=cardTestIndex, diskVolumeName=diskVolumeName, cardSubDetected=cardSubDetected, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portConnectorId=portConnectorId, cardPicHwVersion=cardPicHwVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, cardBusSpeed=cardBusSpeed, auditDescription=auditDescription, cardAdminState=cardAdminState, cardTestTransId=cardTestTransId, cardUpTime=cardUpTime, cardDplxStatus=cardDplxStatus, portCardSubType=portCardSubType, cmMacAddress=cmMacAddress, cardShelfTestIndex=cardShelfTestIndex, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestDescription=cardTestDescription, TestType=TestType, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestType=cardTestType, cardSwVersion=cardSwVersion, errEvLogLevel=errEvLogLevel, portDocsIfIndex=portDocsIfIndex, cardLastChangeTime=cardLastChangeTime, diskVolumeShelfId=diskVolumeShelfId, auditResult=auditResult, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResultNotification=cardTestResultNotification, systemGeneralGroup=systemGeneralGroup, equipmentError=equipmentError, diskVolumeNumber=diskVolumeNumber, cardRamSize=cardRamSize, cardCarrierFwVersion=cardCarrierFwVersion, portPrState=portPrState, portBgpId=portBgpId, cmResetInfo=cmResetInfo, fpgaErrorEventTable=fpgaErrorEventTable, cardTrapInh=cardTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, remainInDiagMode=remainInDiagMode, diskVolumeEntry=diskVolumeEntry, portMode=portMode)
