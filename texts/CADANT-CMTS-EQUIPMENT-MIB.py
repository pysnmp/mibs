...
PortId, ShelfId, PortMode, PicType, EqActionType, PortType, FlowControlMode, UnknownState, DiskVolumeUsageLevel, CardSubType, FirmwareSource, SecondaryState, DuplexStatus, PrimaryState, AdminState, CardType, CardId, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "PortId", "ShelfId", "PortMode", "PicType", "EqActionType", "PortType", "FlowControlMode", "UnknownState", "DiskVolumeUsageLevel", "CardSubType", "FirmwareSource", "SecondaryState", "DuplexStatus", "PrimaryState", "AdminState", "CardType", "CardId", "PortDetectedMode")
...
TextualConvention, TimeStamp, TruthValue, DisplayString, RowStatus, MacAddress, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TimeStamp", "TruthValue", "DisplayString", "RowStatus", "MacAddress", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portPrState=portPrState, auditTime=auditTime, diskVolumeShelfId=diskVolumeShelfId, portNumber=portNumber, portMode=portMode, cmDevice=cmDevice, cardTrapInh=cardTrapInh, cardCarrierHwVersion=cardCarrierHwVersion, cardLastResetReason=cardLastResetReason, auditCommand=auditCommand, portSecStateChange=portSecStateChange, systemGeneral=systemGeneral, errEvLogLevel=errEvLogLevel, portDplxStatus=portDplxStatus, shelfName=shelfName, cardFwVersion=cardFwVersion, errEventId=errEventId, systemKey=systemKey, remainInDiagMode=remainInDiagMode, cardTestTime=cardTestTime, portNumChans=portNumChans, cmResetInfo=cmResetInfo, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetUpTime=cmResetUpTime, cardDplxStatusChange=cardDplxStatusChange, fpgaErrorEventTable=fpgaErrorEventTable, cmVendor=cmVendor, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeEntry=diskVolumeEntry, portMacAddress=portMacAddress, systemGeneralGroup=systemGeneralGroup, equipmentDiagGroup=equipmentDiagGroup, equipmentDiag=equipmentDiag, cmResetReason=cmResetReason, shelfNumber=shelfNumber, portCardId=portCardId, equipmentPortGroup=equipmentPortGroup, TestId=TestId, cardSwVersion=cardSwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditFailedCount=auditFailedCount, cardTestSchedule=cardTestSchedule, cardTemperature=cardTemperature, cardSubType=cardSubType, cardTestName=cardTestName, cardRamSize=cardRamSize, cardLastBootSource=cardLastBootSource, portMaxDsPower=portMaxDsPower, diagTestId=diagTestId, cardCarrierSerialNum=cardCarrierSerialNum, auditLogOutput=auditLogOutput, cmMacAddress=cmMacAddress, cadEquipmentMibConformance=cadEquipmentMibConformance, auditPassedCount=auditPassedCount, diskVolumeSize=diskVolumeSize, cardTable=cardTable, cardTestTransId=cardTestTransId, portPrStateChange=portPrStateChange, portBgpId=portBgpId, cmPrimarySID=cmPrimarySID, cardNumPorts=cardNumPorts, cardTestTable=cardTestTable, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardBusSpeed=cardBusSpeed, portCardSubType=portCardSubType, cadEquipmentMibGroup=cadEquipmentMibGroup, cardCarrierFwVersion=cardCarrierFwVersion, portAdminState=portAdminState, verboseLevel=verboseLevel, auditDescription=auditDescription, portMinDsPower=portMinDsPower, cardShelfTestIndex=cardShelfTestIndex, cardShelfId=cardShelfId, cardBootVersion=cardBootVersion, portRxFlowControlMode=portRxFlowControlMode, cardTestCommand=cardTestCommand, portDescription=portDescription, portTxFlowControlDetected=portTxFlowControlDetected, cardFlashSize=cardFlashSize, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeTable=diskVolumeTable, cardUpTime=cardUpTime, diskVolumeCardId=diskVolumeCardId, auditResult=auditResult, cardTestResult=cardTestResult, cardTestIndex=cardTestIndex, portShelfId=portShelfId, auditShelfId=auditShelfId, portTable=portTable, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeNumber=diskVolumeNumber, portSecState=portSecState, TestTransactionId=TestTransactionId, downstreamPowerLoss=downstreamPowerLoss, cardDplxStatus=cardDplxStatus, portType=portType, cardTestResultDesc=cardTestResultDesc, auditName=auditName, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentError=equipmentError, fpgaErrorEventGroup=fpgaErrorEventGroup, TestResult=TestResult, cardPicDetected=cardPicDetected, portTrapInh=portTrapInh, auditEntry=auditEntry, portDocsIfIndex=portDocsIfIndex, diskDriveNumber=diskDriveNumber, equipmentStateGroup=equipmentStateGroup, cardDetectedChange=cardDetectedChange, auditTotalPassedCount=auditTotalPassedCount, shelfObjects=shelfObjects, equipmentAudit=equipmentAudit, cardTestType=cardTestType, cardTemperatureHighError=cardTemperatureHighError, consoleOutput=consoleOutput, auditTable=auditTable, cardSpareGroupId=cardSpareGroupId, cardSubDetected=cardSubDetected, cardNumber=cardNumber, cardHwVersion=cardHwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestId=cardTestId, auditCycleCount=auditCycleCount, diskVolumeDiskSize=diskVolumeDiskSize, cardEntry=cardEntry, TestType=TestType, auditId=auditId, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeGroup=diskVolumeGroup, cardCPLDVersion=cardCPLDVersion, equipmentState=equipmentState, cadEquipmentMib=cadEquipmentMib, cardTempNoReportNotification=cardTempNoReportNotification, trapCounter=trapCounter, cardType=cardType, fpgaErrorEventEntry=fpgaErrorEventEntry, TestSchedule=TestSchedule, equipmentAuditGroup=equipmentAuditGroup, cardFpgaSource=cardFpgaSource, cardSpareGroupMode=cardSpareGroupMode, cardTestEntry=cardTestEntry, portDetectedMode=portDetectedMode, portCurrDsPower=portCurrDsPower, portTxFlowControlMode=portTxFlowControlMode, portMacIfIndex=portMacIfIndex, systemClock=systemClock, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmUChannelID=cmUChannelID, cardDetected=cardDetected, cardSerialNum=cardSerialNum, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFwUpdateStatus=cardFwUpdateStatus, cardPrState=cardPrState, cardName=cardName, cardId=cardId, diskVolumeName=diskVolumeName, portId=portId, diskVolumeTrapInh=diskVolumeTrapInh, equipmentShelfGroup=equipmentShelfGroup, cmResetNotification=cmResetNotification, TestScheduleCommand=TestScheduleCommand, cardSecState=cardSecState, cardTestResultNotification=cardTestResultNotification, errEvRowStatus=errEvRowStatus, cardTestDescription=cardTestDescription, diskVolumeDriveId=diskVolumeDriveId, cardCpuSpeed=cardCpuSpeed, cmRegistrationNotification=cmRegistrationNotification, TestCommand=TestCommand, cardPicHwVersion=cardPicHwVersion, cardSecStateChange=cardSecStateChange, cardTestScheduleCommand=cardTestScheduleCommand, equipmentTraps=equipmentTraps, cardTempOverHeatNotification=cardTempOverHeatNotification, cardLastChangeTime=cardLastChangeTime, equipmentCardGroup=equipmentCardGroup, auditTotalFailedCount=auditTotalFailedCount, cmResetClearNotification=cmResetClearNotification, portLastChangeTime=portLastChangeTime, shelfSwVersion=shelfSwVersion, cardAction=cardAction, cmResetStatus=cmResetStatus, portRxFlowControlDetected=portRxFlowControlDetected, auditAutoScheduling=auditAutoScheduling, auditLogThrottle=auditLogThrottle, diskVolumeId=diskVolumeId, cardAdminState=cardAdminState, cmIpAddress=cmIpAddress, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeFileName=diskVolumeFileName, eqDiagConfig=eqDiagConfig, cardPrStateChange=cardPrStateChange, portConnectorId=portConnectorId, auditCardId=auditCardId, cardLastBootVersion=cardLastBootVersion, portAction=portAction, cardCpuType=cardCpuType, auditStatus=auditStatus, errEvLoggingEnabled=errEvLoggingEnabled, portEntry=portEntry, trapSeverity=trapSeverity, cardPicSerialNum=cardPicSerialNum, equipmentTbl=equipmentTbl)
