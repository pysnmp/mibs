...
PortType, SecondaryState, PicType, DuplexStatus, DiskVolumeUsageLevel, PrimaryState, ShelfId, FirmwareSource, AdminState, FlowControlMode, PortMode, CardType, CardId, PortId, PortDetectedMode, UnknownState, EqActionType, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PortType", "SecondaryState", "PicType", "DuplexStatus", "DiskVolumeUsageLevel", "PrimaryState", "ShelfId", "FirmwareSource", "AdminState", "FlowControlMode", "PortMode", "CardType", "CardId", "PortId", "PortDetectedMode", "UnknownState", "EqActionType", "CardSubType")
...
TruthValue, DateAndTime, MacAddress, TextualConvention, TimeStamp, RowStatus, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "MacAddress", "TextualConvention", "TimeStamp", "RowStatus", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardShelfTestIndex=cardShelfTestIndex, portNumChans=portNumChans, remainInDiagMode=remainInDiagMode, portCurrDsPower=portCurrDsPower, portLastChangeTime=portLastChangeTime, portPrStateChange=portPrStateChange, auditPassedCount=auditPassedCount, cardBusSpeed=cardBusSpeed, cardPrStateChange=cardPrStateChange, diskVolumeUsageLevel=diskVolumeUsageLevel, cardShelfId=cardShelfId, cardTestTable=cardTestTable, cmResetReason=cmResetReason, cardTestSchedule=cardTestSchedule, auditCycleCount=auditCycleCount, cmResetClearNotification=cmResetClearNotification, portBgpId=portBgpId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResultDesc=cardTestResultDesc, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeTable=diskVolumeTable, cardDetectedChange=cardDetectedChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeShelfId=diskVolumeShelfId, equipmentAuditGroup=equipmentAuditGroup, cardUpTime=cardUpTime, auditCardId=auditCardId, cardDplxStatus=cardDplxStatus, cardTestTime=cardTestTime, equipmentAudit=equipmentAudit, auditLogThrottle=auditLogThrottle, cardCpuSpeed=cardCpuSpeed, diagTestId=diagTestId, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestScheduleCommand=TestScheduleCommand, cmDevice=cmDevice, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeNumber=diskVolumeNumber, cardEntry=cardEntry, systemGeneralGroup=systemGeneralGroup, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSubDetected=cardSubDetected, shelfSwVersion=shelfSwVersion, cardSpareGroupMode=cardSpareGroupMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cmUChannelID=cmUChannelID, portType=portType, portDplxStatus=portDplxStatus, cardSecStateChange=cardSecStateChange, portMode=portMode, shelfObjects=shelfObjects, portDescription=portDescription, portDetectedMode=portDetectedMode, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTempOverHeatNotification=cardTempOverHeatNotification, cmResetNotification=cmResetNotification, cardTestDescription=cardTestDescription, portId=portId, equipmentTbl=equipmentTbl, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestResult=TestResult, cardTestResultNotification=cardTestResultNotification, portAdminState=portAdminState, cardLastChangeTime=cardLastChangeTime, errEventId=errEventId, cmMacAddress=cmMacAddress, cardSubType=cardSubType, auditResult=auditResult, diskVolumeUsageNotification=diskVolumeUsageNotification, errEvRowStatus=errEvRowStatus, cardTrapInh=cardTrapInh, diskVolumeEntry=diskVolumeEntry, cardTable=cardTable, cardTestName=cardTestName, cardTestTransId=cardTestTransId, portShelfId=portShelfId, cardType=cardType, cardFwVersion=cardFwVersion, diskVolumeSize=diskVolumeSize, cardDetected=cardDetected, portTxFlowControlDetected=portTxFlowControlDetected, shelfNumber=shelfNumber, cadEquipmentMib=cadEquipmentMib, PYSNMP_MODULE_ID=cadEquipmentMib, portAction=portAction, portMacIfIndex=portMacIfIndex, auditName=auditName, portDplxStatusChange=portDplxStatusChange, equipmentPortGroup=equipmentPortGroup, diskVolumeFileName=diskVolumeFileName, verboseLevel=verboseLevel, cardHwVersion=cardHwVersion, portCardSubType=portCardSubType, portRxFlowControlDetected=portRxFlowControlDetected, cardTempNoReportNotification=cardTempNoReportNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardBootVersion=cardBootVersion, systemClock=systemClock, equipmentShelfGroup=equipmentShelfGroup, cardLastBootSource=cardLastBootSource, portMaxDsPower=portMaxDsPower, cardFpgaSource=cardFpgaSource, auditAutoScheduling=auditAutoScheduling, cardCPLDVersion=cardCPLDVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardAdminState=cardAdminState, diskVolumeTrapInh=diskVolumeTrapInh, cardSerialNum=cardSerialNum, diskVolumeId=diskVolumeId, cardSwVersion=cardSwVersion, portMinDsPower=portMinDsPower, cardFlashSize=cardFlashSize, TestCommand=TestCommand, systemKey=systemKey, shelfName=shelfName, cardCpuType=cardCpuType, portPrState=portPrState, eqDiagConfig=eqDiagConfig, equipmentState=equipmentState, portSecState=portSecState, systemGeneral=systemGeneral, equipmentCardGroup=equipmentCardGroup, auditTable=auditTable, auditEntry=auditEntry, TestType=TestType, portCardId=portCardId, portMacAddress=portMacAddress, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentTraps=equipmentTraps, cardTemperature=cardTemperature, auditShelfId=auditShelfId, cmResetStatus=cmResetStatus, cardCarrierHwVersion=cardCarrierHwVersion, cardSecState=cardSecState, trapCounter=trapCounter, portEntry=portEntry, cardId=cardId, diskVolumeCardId=diskVolumeCardId, auditFailedCount=auditFailedCount, auditStatus=auditStatus, equipmentError=equipmentError, portTable=portTable, cardLastResetReason=cardLastResetReason, trapSeverity=trapSeverity, cmResetInfo=cmResetInfo, downstreamPowerLoss=downstreamPowerLoss, cmVendor=cmVendor, cmResetUpTime=cmResetUpTime, portRxFlowControlMode=portRxFlowControlMode, cmIpAddress=cmIpAddress, cardCarrierSerialNum=cardCarrierSerialNum, cardTestIndex=cardTestIndex, cardTestEntry=cardTestEntry, cardSpareGroupId=cardSpareGroupId, auditDescription=auditDescription, portNumber=portNumber, cardPicHwVersion=cardPicHwVersion, portTrapInh=portTrapInh, auditLogOutput=auditLogOutput, equipmentDiag=equipmentDiag, portDocsIfIndex=portDocsIfIndex, diskDriveNumber=diskDriveNumber, errEvRecoveryEnabled=errEvRecoveryEnabled, cardRamSize=cardRamSize, cardNumPorts=cardNumPorts, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCommand=auditCommand, cardPicSerialNum=cardPicSerialNum, cardPicDetected=cardPicDetected, cardName=cardName, cardTestType=cardTestType, cardDplxStatusChange=cardDplxStatusChange, equipmentDiagGroup=equipmentDiagGroup, cardLastBootVersion=cardLastBootVersion, cmPrimarySID=cmPrimarySID, diskVolumeName=diskVolumeName, portSecStateChange=portSecStateChange, cardPrState=cardPrState, TestSchedule=TestSchedule, diskVolumeDriveId=diskVolumeDriveId, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, auditTotalPassedCount=auditTotalPassedCount, portConnectorId=portConnectorId, consoleOutput=consoleOutput, TestTransactionId=TestTransactionId, cardNumber=cardNumber, errEvLogLevel=errEvLogLevel, cardCarrierFwVersion=cardCarrierFwVersion, cardAction=cardAction, cardFwUpdateStatus=cardFwUpdateStatus, portTxFlowControlMode=portTxFlowControlMode, cmRegistrationNotification=cmRegistrationNotification, diskVolumeDiskSize=diskVolumeDiskSize, cardTestCommand=cardTestCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTemperatureHighError=cardTemperatureHighError, auditTime=auditTime, cardTestResult=cardTestResult, cardTestId=cardTestId, cardTestScheduleCommand=cardTestScheduleCommand, auditId=auditId, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeGroup=diskVolumeGroup, TestId=TestId)
