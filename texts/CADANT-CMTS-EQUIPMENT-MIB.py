...
CardSubType, CardId, AdminState, CardType, DuplexStatus, ShelfId, DiskVolumeUsageLevel, EqActionType, FlowControlMode, PortMode, PortType, FirmwareSource, PrimaryState, PortDetectedMode, UnknownState, PortId, SecondaryState, PicType = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "CardId", "AdminState", "CardType", "DuplexStatus", "ShelfId", "DiskVolumeUsageLevel", "EqActionType", "FlowControlMode", "PortMode", "PortType", "FirmwareSource", "PrimaryState", "PortDetectedMode", "UnknownState", "PortId", "SecondaryState", "PicType")
...
DateAndTime, RowStatus, TextualConvention, TruthValue, TimeStamp, MacAddress, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "RowStatus", "TextualConvention", "TruthValue", "TimeStamp", "MacAddress", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", eqDiagConfig=eqDiagConfig, portNumChans=portNumChans, cardId=cardId, portNumber=portNumber, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierHwVersion=cardCarrierHwVersion, cardAction=cardAction, cardTrapInh=cardTrapInh, cardSpareGroupId=cardSpareGroupId, diskVolumeShelfId=diskVolumeShelfId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, TestSchedule=TestSchedule, equipmentTbl=equipmentTbl, auditTime=auditTime, auditStatus=auditStatus, cardTempOverHeatNotification=cardTempOverHeatNotification, auditTotalFailedCount=auditTotalFailedCount, cardShelfId=cardShelfId, auditAutoScheduling=auditAutoScheduling, portCurrDsPower=portCurrDsPower, portSecState=portSecState, equipmentPortGroup=equipmentPortGroup, cardPicHwVersion=cardPicHwVersion, cmMacAddress=cmMacAddress, portRxFlowControlDetected=portRxFlowControlDetected, cardPicDetected=cardPicDetected, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastChangeTime=cardLastChangeTime, cardSubDetected=cardSubDetected, cardBootVersion=cardBootVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestId=cardTestId, TestResult=TestResult, cardCarrierFwVersion=cardCarrierFwVersion, cmResetClearNotification=cmResetClearNotification, diskVolumeDiskSize=diskVolumeDiskSize, cardCPLDVersion=cardCPLDVersion, diskVolumeTable=diskVolumeTable, errEventId=errEventId, cardTestName=cardTestName, cardTestResultNotification=cardTestResultNotification, cardNumber=cardNumber, portMode=portMode, portCardId=portCardId, portDetectedMode=portDetectedMode, portMacIfIndex=portMacIfIndex, cardSecState=cardSecState, cmDevice=cmDevice, TestScheduleCommand=TestScheduleCommand, cardRamSize=cardRamSize, errEvLogLevel=errEvLogLevel, cardBusSpeed=cardBusSpeed, cardHwVersion=cardHwVersion, cardCpuType=cardCpuType, auditShelfId=auditShelfId, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDetectedChange=cardDetectedChange, TestCommand=TestCommand, TestId=TestId, cardPicSerialNum=cardPicSerialNum, systemKey=systemKey, auditResult=auditResult, auditTotalPassedCount=auditTotalPassedCount, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskDriveNumber=diskDriveNumber, portDplxStatus=portDplxStatus, errEvRowStatus=errEvRowStatus, trapSeverity=trapSeverity, cardAdminState=cardAdminState, portSecStateChange=portSecStateChange, cardPrStateChange=cardPrStateChange, equipmentCardTestGroup=equipmentCardTestGroup, cardSwVersion=cardSwVersion, auditCycleCount=auditCycleCount, cardTestResultDesc=cardTestResultDesc, equipmentState=equipmentState, diskVolumeName=diskVolumeName, cardNumPorts=cardNumPorts, errEvLoggingEnabled=errEvLoggingEnabled, cadEquipmentMibGroup=cadEquipmentMibGroup, portPrState=portPrState, cmResetUpTime=cmResetUpTime, cmResetStatus=cmResetStatus, diskVolumeId=diskVolumeId, cadEquipmentMib=cadEquipmentMib, cardTable=cardTable, cardTestType=cardTestType, diskVolumeTrapInh=diskVolumeTrapInh, cardTestTime=cardTestTime, errEvRecoveryEnabled=errEvRecoveryEnabled, portTrapInh=portTrapInh, cardTestResult=cardTestResult, trapCounter=trapCounter, cardFlashSize=cardFlashSize, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmPrimarySID=cmPrimarySID, cardTestDescription=cardTestDescription, cardSubType=cardSubType, diskVolumeFileName=diskVolumeFileName, portTxFlowControlDetected=portTxFlowControlDetected, portMinDsPower=portMinDsPower, portAdminState=portAdminState, cardUpTime=cardUpTime, portPrStateChange=portPrStateChange, cardEntry=cardEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestTransactionId=TestTransactionId, cardSerialNum=cardSerialNum, cardCpuSpeed=cardCpuSpeed, equipmentShelfGroup=equipmentShelfGroup, cardTempNoReportNotification=cardTempNoReportNotification, cardFwUpdateStatus=cardFwUpdateStatus, portRxFlowControlMode=portRxFlowControlMode, portTable=portTable, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeNumber=diskVolumeNumber, diskVolumeDriveId=diskVolumeDriveId, auditName=auditName, cardLastBootVersion=cardLastBootVersion, auditFailedCount=auditFailedCount, equipmentAudit=equipmentAudit, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeGroup=diskVolumeGroup, cmRegistrationNotification=cmRegistrationNotification, systemGeneralGroup=systemGeneralGroup, portBgpId=portBgpId, portId=portId, cardTestCommand=cardTestCommand, cardTestSchedule=cardTestSchedule, cmResetInfo=cmResetInfo, auditCommand=auditCommand, cardTestTransId=cardTestTransId, verboseLevel=verboseLevel, equipmentError=equipmentError, portMacAddress=portMacAddress, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardType=cardType, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSecStateChange=cardSecStateChange, portAction=portAction, auditEntry=auditEntry, portLastChangeTime=portLastChangeTime, equipmentTraps=equipmentTraps, remainInDiagMode=remainInDiagMode, auditTable=auditTable, cmResetNotification=cmResetNotification, downstreamPowerLoss=downstreamPowerLoss, diagTestId=diagTestId, shelfNumber=shelfNumber, cardSpareGroupMode=cardSpareGroupMode, consoleOutput=consoleOutput, cardTestIndex=cardTestIndex, cmResetReason=cmResetReason, equipmentAuditGroup=equipmentAuditGroup, portMaxDsPower=portMaxDsPower, portShelfId=portShelfId, auditCardId=auditCardId, cardFwVersion=cardFwVersion, auditId=auditId, portTxFlowControlMode=portTxFlowControlMode, portDplxStatusChange=portDplxStatusChange, systemClock=systemClock, cardTestTable=cardTestTable, cardPrState=cardPrState, cardShelfTestIndex=cardShelfTestIndex, cardTestEntry=cardTestEntry, auditPassedCount=auditPassedCount, cmIpAddress=cmIpAddress, shelfName=shelfName, auditDescription=auditDescription, cardTestScheduleCommand=cardTestScheduleCommand, cardName=cardName, systemGeneral=systemGeneral, cardDplxStatus=cardDplxStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTemperature=cardTemperature, cmVendor=cmVendor, portCardSubType=portCardSubType, portDescription=portDescription, fpgaErrorEventGroup=fpgaErrorEventGroup, cardFpgaSource=cardFpgaSource, cardLastBootSource=cardLastBootSource, shelfObjects=shelfObjects, diskVolumeCardId=diskVolumeCardId, shelfSwVersion=shelfSwVersion, cardDplxStatusChange=cardDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentStateGroup=equipmentStateGroup, equipmentDiagGroup=equipmentDiagGroup, TestType=TestType, equipmentCardGroup=equipmentCardGroup, cardCarrierSerialNum=cardCarrierSerialNum, portType=portType, diskVolumeSize=diskVolumeSize, equipmentDiag=equipmentDiag, cmUChannelID=cmUChannelID, auditLogOutput=auditLogOutput, portConnectorId=portConnectorId, cardDetected=cardDetected, cardLastResetReason=cardLastResetReason, auditLogThrottle=auditLogThrottle, portEntry=portEntry, portDocsIfIndex=portDocsIfIndex)
