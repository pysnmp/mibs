...
PortDetectedMode, ShelfId, CardType, PortType, PicType, CardId, CardSubType, EqActionType, FlowControlMode, PortMode, FirmwareSource, SecondaryState, UnknownState, PortId, PrimaryState, DuplexStatus, AdminState, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "ShelfId", "CardType", "PortType", "PicType", "CardId", "CardSubType", "EqActionType", "FlowControlMode", "PortMode", "FirmwareSource", "SecondaryState", "UnknownState", "PortId", "PrimaryState", "DuplexStatus", "AdminState", "DiskVolumeUsageLevel")
...
RowStatus, TruthValue, MacAddress, TextualConvention, DisplayString, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TruthValue", "MacAddress", "TextualConvention", "DisplayString", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTestTime=cardTestTime, portDescription=portDescription, cardTrapInh=cardTrapInh, cardTemperature=cardTemperature, portNumber=portNumber, TestType=TestType, cardType=cardType, cardTestResult=cardTestResult, diskVolumeDiskSize=diskVolumeDiskSize, equipmentShelfGroup=equipmentShelfGroup, cardTestIndex=cardTestIndex, diskVolumeNumber=diskVolumeNumber, portTable=portTable, cmPrimarySID=cmPrimarySID, portTxFlowControlDetected=portTxFlowControlDetected, auditEntry=auditEntry, equipmentTbl=equipmentTbl, cardCPLDVersion=cardCPLDVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, consoleOutput=consoleOutput, diagTestId=diagTestId, cardShelfTestIndex=cardShelfTestIndex, portRxFlowControlMode=portRxFlowControlMode, systemClock=systemClock, auditStatus=auditStatus, cardDetectedChange=cardDetectedChange, cardSwVersion=cardSwVersion, cardSecState=cardSecState, portTxFlowControlMode=portTxFlowControlMode, equipmentDiagGroup=equipmentDiagGroup, TestId=TestId, cmUChannelID=cmUChannelID, cmIpAddress=cmIpAddress, PYSNMP_MODULE_ID=cadEquipmentMib, errEvLoggingEnabled=errEvLoggingEnabled, cardFlashSize=cardFlashSize, cardLastBootSource=cardLastBootSource, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSubDetected=cardSubDetected, cardSecStateChange=cardSecStateChange, auditCardId=auditCardId, cardCarrierFwVersion=cardCarrierFwVersion, equipmentStateGroup=equipmentStateGroup, cardNumPorts=cardNumPorts, portSecState=portSecState, cardTestCommand=cardTestCommand, cardDplxStatusChange=cardDplxStatusChange, cardPicDetected=cardPicDetected, equipmentTraps=equipmentTraps, trapSeverity=trapSeverity, auditTable=auditTable, cardNumber=cardNumber, auditCycleCount=auditCycleCount, cardHwVersion=cardHwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, remainInDiagMode=remainInDiagMode, cardTempNoReportNotification=cardTempNoReportNotification, portPrState=portPrState, cmResetStatus=cmResetStatus, portDplxStatus=portDplxStatus, shelfObjects=shelfObjects, auditId=auditId, shelfNumber=shelfNumber, cardPicHwVersion=cardPicHwVersion, diskVolumeName=diskVolumeName, portCardId=portCardId, portBgpId=portBgpId, cardTestName=cardTestName, cardRamSize=cardRamSize, cardId=cardId, portMinDsPower=portMinDsPower, portDocsIfIndex=portDocsIfIndex, auditShelfId=auditShelfId, auditDescription=auditDescription, cardLastResetReason=cardLastResetReason, cmResetClearNotification=cmResetClearNotification, TestScheduleCommand=TestScheduleCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, shelfSwVersion=shelfSwVersion, cmMacAddress=cmMacAddress, cardTestResultNotification=cardTestResultNotification, cardSpareGroupMode=cardSpareGroupMode, cardPicSerialNum=cardPicSerialNum, portNumChans=portNumChans, cardPrStateChange=cardPrStateChange, auditTime=auditTime, TestCommand=TestCommand, portDplxStatusChange=portDplxStatusChange, cardTestType=cardTestType, cardCpuType=cardCpuType, cardTestResultDesc=cardTestResultDesc, equipmentCardGroup=equipmentCardGroup, systemGeneral=systemGeneral, portMacAddress=portMacAddress, equipmentDiag=equipmentDiag, auditFailedCount=auditFailedCount, TestResult=TestResult, cmDevice=cmDevice, diskVolumeEntry=diskVolumeEntry, equipmentPortGroup=equipmentPortGroup, diskDriveNumber=diskDriveNumber, cardSerialNum=cardSerialNum, cardTestTransId=cardTestTransId, diskVolumeTable=diskVolumeTable, portEntry=portEntry, diskVolumeId=diskVolumeId, cmResetUpTime=cmResetUpTime, errEvLogLevel=errEvLogLevel, cardCpuSpeed=cardCpuSpeed, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditTotalFailedCount=auditTotalFailedCount, portMaxDsPower=portMaxDsPower, auditLogOutput=auditLogOutput, cardName=cardName, portRxFlowControlDetected=portRxFlowControlDetected, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeCardId=diskVolumeCardId, auditTotalPassedCount=auditTotalPassedCount, systemGeneralGroup=systemGeneralGroup, portTrapInh=portTrapInh, cardAction=cardAction, cardBootVersion=cardBootVersion, cardShelfId=cardShelfId, errEventId=errEventId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierHwVersion=cardCarrierHwVersion, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetNotification=cmResetNotification, portId=portId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, TestSchedule=TestSchedule, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditCommand=auditCommand, cardAdminState=cardAdminState, auditPassedCount=auditPassedCount, auditResult=auditResult, shelfName=shelfName, diskVolumeDriveId=diskVolumeDriveId, portPrStateChange=portPrStateChange, diskVolumeGroup=diskVolumeGroup, portAction=portAction, cardDplxStatus=cardDplxStatus, cardSpareGroupId=cardSpareGroupId, cmRegistrationNotification=cmRegistrationNotification, portCardSubType=portCardSubType, portCurrDsPower=portCurrDsPower, cardSubType=cardSubType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestSchedule=cardTestSchedule, equipmentCardTestGroup=equipmentCardTestGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardTestDescription=cardTestDescription, auditName=auditName, portAdminState=portAdminState, TestTransactionId=TestTransactionId, portMode=portMode, cmResetInfo=cmResetInfo, cardTemperatureHighError=cardTemperatureHighError, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentAuditGroup=equipmentAuditGroup, systemKey=systemKey, cardTestEntry=cardTestEntry, cardBusSpeed=cardBusSpeed, cadEquipmentMibGroup=cadEquipmentMibGroup, portMacIfIndex=portMacIfIndex, portDetectedMode=portDetectedMode, downstreamPowerLoss=downstreamPowerLoss, cardFpgaSource=cardFpgaSource, auditLogThrottle=auditLogThrottle, equipmentError=equipmentError, equipmentAudit=equipmentAudit, portShelfId=portShelfId, diskVolumeShelfId=diskVolumeShelfId, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeTrapInh=diskVolumeTrapInh, cardPrState=cardPrState, errEvRowStatus=errEvRowStatus, equipmentState=equipmentState, diskVolumeFileName=diskVolumeFileName, cardEntry=cardEntry, eqDiagConfig=eqDiagConfig, portType=portType, cardFwVersion=cardFwVersion, cardLastChangeTime=cardLastChangeTime, cardDetected=cardDetected, diskVolumeSize=diskVolumeSize, auditAutoScheduling=auditAutoScheduling, cmVendor=cmVendor, fpgaErrorEventTable=fpgaErrorEventTable, errEvRecoveryEnabled=errEvRecoveryEnabled, trapCounter=trapCounter, cardCarrierSerialNum=cardCarrierSerialNum, portSecStateChange=portSecStateChange, portConnectorId=portConnectorId, verboseLevel=verboseLevel, cardUpTime=cardUpTime, cardLastBootVersion=cardLastBootVersion, cadEquipmentMib=cadEquipmentMib, portLastChangeTime=portLastChangeTime, cardTestId=cardTestId, cmResetReason=cmResetReason, cardTable=cardTable, cardTestTable=cardTestTable)
