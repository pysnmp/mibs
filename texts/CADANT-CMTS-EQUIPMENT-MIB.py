...
PortId, DiskVolumeUsageLevel, FirmwareSource, SecondaryState, UnknownState, DuplexStatus, PicType, PortMode, EqActionType, PortDetectedMode, AdminState, PortType, ShelfId, CardId, CardSubType, CardType, PrimaryState, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "PortId", "DiskVolumeUsageLevel", "FirmwareSource", "SecondaryState", "UnknownState", "DuplexStatus", "PicType", "PortMode", "EqActionType", "PortDetectedMode", "AdminState", "PortType", "ShelfId", "CardId", "CardSubType", "CardType", "PrimaryState", "FlowControlMode")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTxFlowControlDetected=portTxFlowControlDetected, portMacAddress=portMacAddress, diskVolumeName=diskVolumeName, cmIpAddress=cmIpAddress, portDetectedMode=portDetectedMode, auditId=auditId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTime=cardTestTime, auditTime=auditTime, cmResetNotification=cmResetNotification, cardSecStateChange=cardSecStateChange, eqDiagConfig=eqDiagConfig, auditAutoScheduling=auditAutoScheduling, cardTemperatureHighError=cardTemperatureHighError, errEvLoggingEnabled=errEvLoggingEnabled, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentShelfGroup=equipmentShelfGroup, cardCpuSpeed=cardCpuSpeed, equipmentTbl=equipmentTbl, auditShelfId=auditShelfId, cardTestSchedule=cardTestSchedule, portMaxDsPower=portMaxDsPower, cmUChannelID=cmUChannelID, equipmentDiagGroup=equipmentDiagGroup, cardCarrierSerialNum=cardCarrierSerialNum, shelfName=shelfName, equipmentCardTestGroup=equipmentCardTestGroup, verboseLevel=verboseLevel, portDplxStatusChange=portDplxStatusChange, cardFwVersion=cardFwVersion, portBgpId=portBgpId, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeTrapInh=diskVolumeTrapInh, TestResult=TestResult, cardTemperatureHighWarn=cardTemperatureHighWarn, portId=portId, portDescription=portDescription, portCurrDsPower=portCurrDsPower, cardType=cardType, cardNumPorts=cardNumPorts, diskVolumeTable=diskVolumeTable, cmDevice=cmDevice, cardSpareGroupMode=cardSpareGroupMode, shelfSwVersion=shelfSwVersion, portTable=portTable, cmResetStatus=cmResetStatus, cardHwVersion=cardHwVersion, auditStatus=auditStatus, cmMacAddress=cmMacAddress, portCardId=portCardId, portRxFlowControlMode=portRxFlowControlMode, equipmentState=equipmentState, trapSeverity=trapSeverity, portMinDsPower=portMinDsPower, auditCycleCount=auditCycleCount, portSecStateChange=portSecStateChange, remainInDiagMode=remainInDiagMode, diagTestId=diagTestId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardLastResetReason=cardLastResetReason, cardTestEntry=cardTestEntry, trapCounter=trapCounter, portConnectorId=portConnectorId, auditLogOutput=auditLogOutput, fpgaErrorEventGroup=fpgaErrorEventGroup, systemGeneralGroup=systemGeneralGroup, cardTestId=cardTestId, cardFpgaSource=cardFpgaSource, portShelfId=portShelfId, portLastChangeTime=portLastChangeTime, auditCommand=auditCommand, cardPicHwVersion=cardPicHwVersion, portEntry=portEntry, portMacIfIndex=portMacIfIndex, cmResetReason=cmResetReason, diskVolumeDriveId=diskVolumeDriveId, cmResetClearNotification=cmResetClearNotification, auditLogThrottle=auditLogThrottle, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardLastBootVersion=cardLastBootVersion, downstreamPowerLoss=downstreamPowerLoss, TestType=TestType, cardNumber=cardNumber, auditTable=auditTable, TestScheduleCommand=TestScheduleCommand, auditTotalPassedCount=auditTotalPassedCount, diskVolumeDiskSize=diskVolumeDiskSize, portPrStateChange=portPrStateChange, portNumChans=portNumChans, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTempOverHeatNotification=cardTempOverHeatNotification, systemKey=systemKey, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventTable=fpgaErrorEventTable, auditResult=auditResult, TestTransactionId=TestTransactionId, cardRamSize=cardRamSize, cardCPLDVersion=cardCPLDVersion, cardTestIndex=cardTestIndex, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestName=cardTestName, cmRegistrationNotification=cmRegistrationNotification, TestCommand=TestCommand, auditDescription=auditDescription, cardBootVersion=cardBootVersion, cardDplxStatusChange=cardDplxStatusChange, cardShelfId=cardShelfId, cmResetInfo=cmResetInfo, cardCpuType=cardCpuType, TestId=TestId, diskDriveNumber=diskDriveNumber, auditCardId=auditCardId, cardTrapInh=cardTrapInh, cardPicSerialNum=cardPicSerialNum, cardTestTransId=cardTestTransId, portRxFlowControlDetected=portRxFlowControlDetected, cardFlashSize=cardFlashSize, cardTestResult=cardTestResult, diskVolumeId=diskVolumeId, cardLastBootSource=cardLastBootSource, cardTempNoReportNotification=cardTempNoReportNotification, systemGeneral=systemGeneral, cardSubType=cardSubType, shelfObjects=shelfObjects, portDplxStatus=portDplxStatus, cardSecState=cardSecState, systemClock=systemClock, cardId=cardId, auditEntry=auditEntry, cardDetectedChange=cardDetectedChange, cardFwUpdateStatus=cardFwUpdateStatus, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentAudit=equipmentAudit, diskVolumeGroup=diskVolumeGroup, cardDetected=cardDetected, portNumber=portNumber, cardUpTime=cardUpTime, shelfNumber=shelfNumber, portType=portType, cardTestScheduleCommand=cardTestScheduleCommand, cardCarrierFwVersion=cardCarrierFwVersion, cadEquipmentMib=cadEquipmentMib, errEvRowStatus=errEvRowStatus, cardPrStateChange=cardPrStateChange, auditFailedCount=auditFailedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentDiag=equipmentDiag, diskVolumeEntry=diskVolumeEntry, cmPrimarySID=cmPrimarySID, cardTestTable=cardTestTable, cardTestResultNotification=cardTestResultNotification, cardSpareGroupId=cardSpareGroupId, diskVolumeShelfId=diskVolumeShelfId, cardSwVersion=cardSwVersion, equipmentError=equipmentError, errEventId=errEventId, cardLastChangeTime=cardLastChangeTime, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditPassedCount=auditPassedCount, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardDplxStatus=cardDplxStatus, equipmentStateGroup=equipmentStateGroup, portAdminState=portAdminState, cardAdminState=cardAdminState, portSecState=portSecState, portPrState=portPrState, cardBusSpeed=cardBusSpeed, cardPrState=cardPrState, auditName=auditName, diskVolumeCardId=diskVolumeCardId, cardName=cardName, cardEntry=cardEntry, cardTestType=cardTestType, auditTotalFailedCount=auditTotalFailedCount, consoleOutput=consoleOutput, portTrapInh=portTrapInh, portDocsIfIndex=portDocsIfIndex, diskVolumeNumber=diskVolumeNumber, cardTable=cardTable, diskVolumeSize=diskVolumeSize, equipmentPortGroup=equipmentPortGroup, TestSchedule=TestSchedule, cardTemperature=cardTemperature, cardTestCommand=cardTestCommand, cadEquipmentMibGroup=cadEquipmentMibGroup, cardShelfTestIndex=cardShelfTestIndex, portAction=portAction, cmResetUpTime=cmResetUpTime, errEvLogLevel=errEvLogLevel, cardTestDescription=cardTestDescription, cardPicDetected=cardPicDetected, cardCarrierHwVersion=cardCarrierHwVersion, portMode=portMode, cmVendor=cmVendor, equipmentAuditGroup=equipmentAuditGroup, diskVolumeFileName=diskVolumeFileName, cardSubDetected=cardSubDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardSerialNum=cardSerialNum, portCardSubType=portCardSubType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentTraps=equipmentTraps, cardAction=cardAction)
