...
PortMode, FlowControlMode, SecondaryState, CardType, PrimaryState, PortId, PortDetectedMode, PortType, ShelfId, CardId, FirmwareSource, UnknownState, AdminState, DuplexStatus, DiskVolumeUsageLevel, CardSubType, EqActionType, PicType = mibBuilder.importSymbols("CADANT-TC", "PortMode", "FlowControlMode", "SecondaryState", "CardType", "PrimaryState", "PortId", "PortDetectedMode", "PortType", "ShelfId", "CardId", "FirmwareSource", "UnknownState", "AdminState", "DuplexStatus", "DiskVolumeUsageLevel", "CardSubType", "EqActionType", "PicType")
...
TimeStamp, TruthValue, DateAndTime, DisplayString, RowStatus, TextualConvention, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "DateAndTime", "DisplayString", "RowStatus", "TextualConvention", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTotalPassedCount=auditTotalPassedCount, cardSpareGroupMode=cardSpareGroupMode, systemClock=systemClock, cardLastResetReason=cardLastResetReason, cardTestTime=cardTestTime, cardType=cardType, portMinDsPower=portMinDsPower, cardTestResultDesc=cardTestResultDesc, cardSerialNum=cardSerialNum, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentCardGroup=equipmentCardGroup, cardLastChangeTime=cardLastChangeTime, cmResetReason=cmResetReason, cardEntry=cardEntry, cardSubType=cardSubType, portDetectedMode=portDetectedMode, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardNumber=cardNumber, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cmRegistrationNotification=cmRegistrationNotification, cardTrapInh=cardTrapInh, cardTestSchedule=cardTestSchedule, cardSubDetected=cardSubDetected, cardCPLDVersion=cardCPLDVersion, cadEquipmentMib=cadEquipmentMib, fpgaErrorEventTable=fpgaErrorEventTable, cardTempNoReportNotification=cardTempNoReportNotification, cmResetNotification=cmResetNotification, cardId=cardId, equipmentDiag=equipmentDiag, cadEquipmentMibConformance=cadEquipmentMibConformance, cmIpAddress=cmIpAddress, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditId=auditId, portMacAddress=portMacAddress, portMacIfIndex=portMacIfIndex, portPrStateChange=portPrStateChange, shelfSwVersion=shelfSwVersion, portSecState=portSecState, diskVolumeNumber=diskVolumeNumber, systemGeneralGroup=systemGeneralGroup, cmPrimarySID=cmPrimarySID, cardFpgaSource=cardFpgaSource, diskVolumeId=diskVolumeId, equipmentError=equipmentError, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDriveId=diskVolumeDriveId, cardTestResult=cardTestResult, equipmentShelfGroup=equipmentShelfGroup, cardTemperature=cardTemperature, auditLogOutput=auditLogOutput, auditCycleCount=auditCycleCount, systemGeneral=systemGeneral, diskVolumeTable=diskVolumeTable, PYSNMP_MODULE_ID=cadEquipmentMib, auditCommand=auditCommand, cardHwVersion=cardHwVersion, TestId=TestId, equipmentPortGroup=equipmentPortGroup, trapCounter=trapCounter, auditName=auditName, portId=portId, portTxFlowControlDetected=portTxFlowControlDetected, downstreamPowerLoss=downstreamPowerLoss, equipmentStateGroup=equipmentStateGroup, cardSecStateChange=cardSecStateChange, auditResult=auditResult, TestScheduleCommand=TestScheduleCommand, cmDevice=cmDevice, cardCarrierFwVersion=cardCarrierFwVersion, portDescription=portDescription, cardUpTime=cardUpTime, portConnectorId=portConnectorId, diskVolumeUsageLevel=diskVolumeUsageLevel, auditAutoScheduling=auditAutoScheduling, errEventId=errEventId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeCardId=diskVolumeCardId, consoleOutput=consoleOutput, portLastChangeTime=portLastChangeTime, TestResult=TestResult, shelfNumber=shelfNumber, cardTestIndex=cardTestIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentDiagGroup=equipmentDiagGroup, portCardSubType=portCardSubType, diskVolumeSize=diskVolumeSize, cardTestId=cardTestId, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentTraps=equipmentTraps, portAdminState=portAdminState, eqDiagConfig=eqDiagConfig, cmResetClearNotification=cmResetClearNotification, diskVolumeTrapInh=diskVolumeTrapInh, diagTestId=diagTestId, cardTestType=cardTestType, cardTestCommand=cardTestCommand, diskDriveNumber=diskDriveNumber, cardDplxStatus=cardDplxStatus, auditTable=auditTable, cardBusSpeed=cardBusSpeed, portDplxStatusChange=portDplxStatusChange, cardAction=cardAction, cardFwUpdateStatus=cardFwUpdateStatus, cardTestTable=cardTestTable, cardCpuType=cardCpuType, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentAudit=equipmentAudit, cardTestScheduleCommand=cardTestScheduleCommand, portRxFlowControlDetected=portRxFlowControlDetected, cardTestEntry=cardTestEntry, cardSpareGroupId=cardSpareGroupId, portAction=portAction, cardBootVersion=cardBootVersion, cardNumPorts=cardNumPorts, errEvLogLevel=errEvLogLevel, cardShelfId=cardShelfId, cardPicDetected=cardPicDetected, cmResetInfo=cmResetInfo, cardPrState=cardPrState, TestSchedule=TestSchedule, cardTestName=cardTestName, cardTable=cardTable, cardTestResultNotification=cardTestResultNotification, cardFlashSize=cardFlashSize, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeDiskSize=diskVolumeDiskSize, auditCardId=auditCardId, portMaxDsPower=portMaxDsPower, auditTime=auditTime, portSecStateChange=portSecStateChange, cardLastBootVersion=cardLastBootVersion, cardAdminState=cardAdminState, portEntry=portEntry, portCurrDsPower=portCurrDsPower, portPrState=portPrState, trapSeverity=trapSeverity, cardSecState=cardSecState, errEvRowStatus=errEvRowStatus, portTable=portTable, cardPicHwVersion=cardPicHwVersion, auditLogThrottle=auditLogThrottle, shelfObjects=shelfObjects, cardSwVersion=cardSwVersion, cardRamSize=cardRamSize, cmResetStatus=cmResetStatus, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDplxStatus=portDplxStatus, portRxFlowControlMode=portRxFlowControlMode, portTrapInh=portTrapInh, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardLastBootSource=cardLastBootSource, cardTemperatureHighError=cardTemperatureHighError, cardTestTransId=cardTestTransId, TestCommand=TestCommand, cardPrStateChange=cardPrStateChange, portMode=portMode, verboseLevel=verboseLevel, cardTempOverHeatNotification=cardTempOverHeatNotification, cardName=cardName, remainInDiagMode=remainInDiagMode, portTxFlowControlMode=portTxFlowControlMode, TestType=TestType, diskVolumeUsageNotification=diskVolumeUsageNotification, cardShelfTestIndex=cardShelfTestIndex, cadEquipmentMibCompliance=cadEquipmentMibCompliance, shelfName=shelfName, cardDetectedChange=cardDetectedChange, cardFwVersion=cardFwVersion, equipmentTbl=equipmentTbl, diskVolumeName=diskVolumeName, auditEntry=auditEntry, auditStatus=auditStatus, cardDplxStatusChange=cardDplxStatusChange, cmMacAddress=cmMacAddress, equipmentState=equipmentState, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeGroup=diskVolumeGroup, diskVolumeShelfId=diskVolumeShelfId, cmVendor=cmVendor, cmUChannelID=cmUChannelID, cardDetected=cardDetected, auditDescription=auditDescription, auditPassedCount=auditPassedCount, portNumChans=portNumChans, portBgpId=portBgpId, systemKey=systemKey, auditTotalFailedCount=auditTotalFailedCount, portShelfId=portShelfId, portNumber=portNumber, portCardId=portCardId, auditFailedCount=auditFailedCount, cardTestDescription=cardTestDescription, cardCpuSpeed=cardCpuSpeed, errEvLoggingEnabled=errEvLoggingEnabled, portType=portType, diskVolumeFileName=diskVolumeFileName, cmResetUpTime=cmResetUpTime, TestTransactionId=TestTransactionId, equipmentAuditGroup=equipmentAuditGroup, cardPicSerialNum=cardPicSerialNum, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditShelfId=auditShelfId, cardCarrierHwVersion=cardCarrierHwVersion, portDocsIfIndex=portDocsIfIndex)
