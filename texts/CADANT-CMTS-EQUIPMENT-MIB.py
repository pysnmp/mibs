...
PortDetectedMode, CardType, FlowControlMode, DiskVolumeUsageLevel, CardSubType, SecondaryState, PortMode, AdminState, PortType, ShelfId, UnknownState, CardId, FirmwareSource, EqActionType, PrimaryState, PicType, DuplexStatus, PortId = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "CardType", "FlowControlMode", "DiskVolumeUsageLevel", "CardSubType", "SecondaryState", "PortMode", "AdminState", "PortType", "ShelfId", "UnknownState", "CardId", "FirmwareSource", "EqActionType", "PrimaryState", "PicType", "DuplexStatus", "PortId")
...
DisplayString, RowStatus, TruthValue, TextualConvention, DateAndTime, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "RowStatus", "TruthValue", "TextualConvention", "DateAndTime", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentState=equipmentState, cardTemperatureHighWarn=cardTemperatureHighWarn, cmMacAddress=cmMacAddress, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeId=diskVolumeId, trapCounter=trapCounter, cardFlashSize=cardFlashSize, auditTotalPassedCount=auditTotalPassedCount, portDplxStatus=portDplxStatus, remainInDiagMode=remainInDiagMode, equipmentDiag=equipmentDiag, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditResult=auditResult, cmResetReason=cmResetReason, portMacIfIndex=portMacIfIndex, diskVolumeEntry=diskVolumeEntry, portConnectorId=portConnectorId, systemGeneral=systemGeneral, auditName=auditName, cardPrState=cardPrState, diagTestId=diagTestId, cardAdminState=cardAdminState, portShelfId=portShelfId, portPrState=portPrState, cardTestName=cardTestName, cardCarrierSerialNum=cardCarrierSerialNum, cardSecStateChange=cardSecStateChange, shelfSwVersion=shelfSwVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTemperature=cardTemperature, cardHwVersion=cardHwVersion, cardTestEntry=cardTestEntry, equipmentTraps=equipmentTraps, cardCarrierFwVersion=cardCarrierFwVersion, TestTransactionId=TestTransactionId, cardType=cardType, cardBootVersion=cardBootVersion, portDocsIfIndex=portDocsIfIndex, portType=portType, diskVolumeDiskSize=diskVolumeDiskSize, downstreamPowerLoss=downstreamPowerLoss, portDetectedMode=portDetectedMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTable=diskVolumeTable, errEventId=errEventId, auditTable=auditTable, cardId=cardId, cmVendor=cmVendor, equipmentPortGroup=equipmentPortGroup, diskVolumeShelfId=diskVolumeShelfId, cardTestTime=cardTestTime, equipmentCardGroup=equipmentCardGroup, portAction=portAction, portSecStateChange=portSecStateChange, diskVolumeSize=diskVolumeSize, TestScheduleCommand=TestScheduleCommand, cadEquipmentMib=cadEquipmentMib, cardTrapInh=cardTrapInh, portBgpId=portBgpId, cardTestType=cardTestType, cardTestTransId=cardTestTransId, equipmentShelfGroup=equipmentShelfGroup, equipmentDiagGroup=equipmentDiagGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeCardId=diskVolumeCardId, equipmentCardTestGroup=equipmentCardTestGroup, cardFwUpdateStatus=cardFwUpdateStatus, portRxFlowControlMode=portRxFlowControlMode, portDescription=portDescription, cardSubType=cardSubType, systemKey=systemKey, auditStatus=auditStatus, cadEquipmentMibConformance=cadEquipmentMibConformance, shelfObjects=shelfObjects, cardPicSerialNum=cardPicSerialNum, auditDescription=auditDescription, cmResetUpTime=cmResetUpTime, cardPicDetected=cardPicDetected, auditFailedCount=auditFailedCount, portCurrDsPower=portCurrDsPower, TestType=TestType, diskVolumeNumber=diskVolumeNumber, cardUpTime=cardUpTime, auditAutoScheduling=auditAutoScheduling, cardName=cardName, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSecState=cardSecState, cardSubDetected=cardSubDetected, cardTestSchedule=cardTestSchedule, auditId=auditId, fpgaErrorEventEntry=fpgaErrorEventEntry, cardRamSize=cardRamSize, equipmentAuditGroup=equipmentAuditGroup, cardPicHwVersion=cardPicHwVersion, cardLastBootSource=cardLastBootSource, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditEntry=auditEntry, portCardId=portCardId, portId=portId, cardPrStateChange=cardPrStateChange, portLastChangeTime=portLastChangeTime, cmDevice=cmDevice, cmPrimarySID=cmPrimarySID, cmResetInfo=cmResetInfo, cardDplxStatusChange=cardDplxStatusChange, fpgaErrorEventTable=fpgaErrorEventTable, cardSerialNum=cardSerialNum, verboseLevel=verboseLevel, cardTestResultNotification=cardTestResultNotification, portTable=portTable, cmIpAddress=cmIpAddress, cardTestDescription=cardTestDescription, portTxFlowControlMode=portTxFlowControlMode, cardTestTable=cardTestTable, cardDetected=cardDetected, TestCommand=TestCommand, cardEntry=cardEntry, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardSwVersion=cardSwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTemperatureHighError=cardTemperatureHighError, cardTestResult=cardTestResult, auditCycleCount=auditCycleCount, errEvLoggingEnabled=errEvLoggingEnabled, auditPassedCount=auditPassedCount, cardLastBootVersion=cardLastBootVersion, shelfNumber=shelfNumber, portAdminState=portAdminState, cardCPLDVersion=cardCPLDVersion, errEvLogLevel=errEvLogLevel, cardTestId=cardTestId, TestId=TestId, cardNumPorts=cardNumPorts, cardSpareGroupId=cardSpareGroupId, cardCpuType=cardCpuType, portMinDsPower=portMinDsPower, cardTestCommand=cardTestCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetStatus=cmResetStatus, eqDiagConfig=eqDiagConfig, portPrStateChange=portPrStateChange, cardLastChangeTime=cardLastChangeTime, cmResetClearNotification=cmResetClearNotification, cardShelfId=cardShelfId, cardCpuSpeed=cardCpuSpeed, systemClock=systemClock, cardDplxStatus=cardDplxStatus, shelfName=shelfName, portEntry=portEntry, diskVolumeDriveId=diskVolumeDriveId, auditTime=auditTime, portMacAddress=portMacAddress, cardCarrierHwVersion=cardCarrierHwVersion, cardBusSpeed=cardBusSpeed, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portCardSubType=portCardSubType, cardTempOverHeatNotification=cardTempOverHeatNotification, systemGeneralGroup=systemGeneralGroup, cardFpgaSource=cardFpgaSource, cardShelfTestIndex=cardShelfTestIndex, portTxFlowControlDetected=portTxFlowControlDetected, trapSeverity=trapSeverity, portNumChans=portNumChans, cardTestIndex=cardTestIndex, cardTempNoReportNotification=cardTempNoReportNotification, equipmentTbl=equipmentTbl, TestSchedule=TestSchedule, diskVolumeName=diskVolumeName, portTrapInh=portTrapInh, portMode=portMode, cardFwVersion=cardFwVersion, cmRegistrationNotification=cmRegistrationNotification, cardTable=cardTable, cardAction=cardAction, cardSpareGroupMode=cardSpareGroupMode, equipmentAudit=equipmentAudit, auditCommand=auditCommand, TestResult=TestResult, portNumber=portNumber, diskVolumeFileName=diskVolumeFileName, cmUChannelID=cmUChannelID, equipmentError=equipmentError, equipmentStateGroup=equipmentStateGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditShelfId=auditShelfId, diskVolumeGroup=diskVolumeGroup, auditCardId=auditCardId, diskVolumeTrapInh=diskVolumeTrapInh, cardLastResetReason=cardLastResetReason, auditLogOutput=auditLogOutput, portRxFlowControlDetected=portRxFlowControlDetected, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardNumber=cardNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestResultDesc=cardTestResultDesc, cmResetNotification=cmResetNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, errEvRowStatus=errEvRowStatus, cardDetectedChange=cardDetectedChange, portMaxDsPower=portMaxDsPower, diskDriveNumber=diskDriveNumber, auditTotalFailedCount=auditTotalFailedCount, auditLogThrottle=auditLogThrottle, consoleOutput=consoleOutput, portSecState=portSecState)
