...
FlowControlMode, PortMode, CardId, DuplexStatus, PortDetectedMode, SecondaryState, CardSubType, AdminState, PortType, ShelfId, UnknownState, PicType, CardType, EqActionType, FirmwareSource, PrimaryState, DiskVolumeUsageLevel, PortId = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PortMode", "CardId", "DuplexStatus", "PortDetectedMode", "SecondaryState", "CardSubType", "AdminState", "PortType", "ShelfId", "UnknownState", "PicType", "CardType", "EqActionType", "FirmwareSource", "PrimaryState", "DiskVolumeUsageLevel", "PortId")
...
TimeStamp, TruthValue, RowStatus, DisplayString, TextualConvention, MacAddress, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "TruthValue", "RowStatus", "DisplayString", "TextualConvention", "MacAddress", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portCardSubType=portCardSubType, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeDiskSize=diskVolumeDiskSize, auditTime=auditTime, cardNumber=cardNumber, cardLastBootVersion=cardLastBootVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, cardAdminState=cardAdminState, cardFlashSize=cardFlashSize, equipmentDiag=equipmentDiag, auditLogThrottle=auditLogThrottle, cardTestType=cardTestType, cardSubType=cardSubType, cardCpuType=cardCpuType, trapCounter=trapCounter, cmResetClearNotification=cmResetClearNotification, diskVolumeId=diskVolumeId, cmResetStatus=cmResetStatus, equipmentDiagGroup=equipmentDiagGroup, cardCarrierHwVersion=cardCarrierHwVersion, cardSpareGroupMode=cardSpareGroupMode, systemKey=systemKey, portTxFlowControlDetected=portTxFlowControlDetected, cardUpTime=cardUpTime, shelfSwVersion=shelfSwVersion, auditFailedCount=auditFailedCount, cardTestResultNotification=cardTestResultNotification, TestScheduleCommand=TestScheduleCommand, portBgpId=portBgpId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCarrierFwVersion=cardCarrierFwVersion, cardLastResetReason=cardLastResetReason, cardSwVersion=cardSwVersion, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCpuSpeed=cardCpuSpeed, TestCommand=TestCommand, cardNumPorts=cardNumPorts, auditName=auditName, cardDplxStatus=cardDplxStatus, cardTestResultDesc=cardTestResultDesc, cmMacAddress=cmMacAddress, equipmentTraps=equipmentTraps, cardTestResult=cardTestResult, cmResetReason=cmResetReason, diskVolumeSize=diskVolumeSize, cardFwVersion=cardFwVersion, auditCycleCount=auditCycleCount, equipmentCardTestGroup=equipmentCardTestGroup, cmResetInfo=cmResetInfo, equipmentTbl=equipmentTbl, portTxFlowControlMode=portTxFlowControlMode, cardDetected=cardDetected, errEvLoggingEnabled=errEvLoggingEnabled, portAdminState=portAdminState, cardName=cardName, portTable=portTable, cardTable=cardTable, cardPrState=cardPrState, cardRamSize=cardRamSize, cardTemperature=cardTemperature, portAction=portAction, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeShelfId=diskVolumeShelfId, cmUChannelID=cmUChannelID, cardTrapInh=cardTrapInh, cardFpgaSource=cardFpgaSource, portRxFlowControlDetected=portRxFlowControlDetected, cardBootVersion=cardBootVersion, TestType=TestType, cadEquipmentMib=cadEquipmentMib, cmDevice=cmDevice, systemGeneral=systemGeneral, portLastChangeTime=portLastChangeTime, diskVolumeTrapInh=diskVolumeTrapInh, remainInDiagMode=remainInDiagMode, auditDescription=auditDescription, equipmentStateGroup=equipmentStateGroup, diskVolumeFileName=diskVolumeFileName, cardSpareGroupId=cardSpareGroupId, cardFwUpdateStatus=cardFwUpdateStatus, portTrapInh=portTrapInh, cardShelfTestIndex=cardShelfTestIndex, auditTotalPassedCount=auditTotalPassedCount, diagTestId=diagTestId, cardTestSchedule=cardTestSchedule, cardTempNoReportNotification=cardTempNoReportNotification, cardSecStateChange=cardSecStateChange, cardDetectedChange=cardDetectedChange, cmVendor=cmVendor, portDescription=portDescription, errEvRowStatus=errEvRowStatus, auditShelfId=auditShelfId, portMacIfIndex=portMacIfIndex, cardTestTime=cardTestTime, cmRegistrationNotification=cmRegistrationNotification, cmIpAddress=cmIpAddress, shelfObjects=shelfObjects, auditCardId=auditCardId, cardSubDetected=cardSubDetected, cardSecState=cardSecState, portEntry=portEntry, portMinDsPower=portMinDsPower, portRxFlowControlMode=portRxFlowControlMode, cardTempOverHeatNotification=cardTempOverHeatNotification, portSecState=portSecState, portMacAddress=portMacAddress, portDplxStatusChange=portDplxStatusChange, diskVolumeDriveId=diskVolumeDriveId, portConnectorId=portConnectorId, auditResult=auditResult, portId=portId, diskVolumeName=diskVolumeName, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestTransId=cardTestTransId, equipmentAudit=equipmentAudit, auditId=auditId, TestResult=TestResult, auditAutoScheduling=auditAutoScheduling, portPrStateChange=portPrStateChange, portPrState=portPrState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portCardId=portCardId, shelfNumber=shelfNumber, auditTable=auditTable, cardPrStateChange=cardPrStateChange, systemClock=systemClock, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSerialNum=cardSerialNum, cardLastBootSource=cardLastBootSource, cmResetUpTime=cmResetUpTime, TestTransactionId=TestTransactionId, equipmentError=equipmentError, equipmentAuditGroup=equipmentAuditGroup, TestId=TestId, shelfName=shelfName, cardLastChangeTime=cardLastChangeTime, equipmentShelfGroup=equipmentShelfGroup, auditLogOutput=auditLogOutput, auditStatus=auditStatus, cardAction=cardAction, cardPicSerialNum=cardPicSerialNum, portDplxStatus=portDplxStatus, auditCommand=auditCommand, cmResetNotification=cmResetNotification, cardTestCommand=cardTestCommand, portDetectedMode=portDetectedMode, errEvLogLevel=errEvLogLevel, portShelfId=portShelfId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeGroup=diskVolumeGroup, cardTestIndex=cardTestIndex, portSecStateChange=portSecStateChange, diskVolumeTable=diskVolumeTable, cardType=cardType, equipmentPortGroup=equipmentPortGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestTable=cardTestTable, diskVolumeCardId=diskVolumeCardId, eqDiagConfig=eqDiagConfig, consoleOutput=consoleOutput, auditPassedCount=auditPassedCount, equipmentState=equipmentState, verboseLevel=verboseLevel, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestDescription=cardTestDescription, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDocsIfIndex=portDocsIfIndex, TestSchedule=TestSchedule, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditEntry=auditEntry, errEventId=errEventId, equipmentCardGroup=equipmentCardGroup, cmPrimarySID=cmPrimarySID, fpgaErrorEventGroup=fpgaErrorEventGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTemperatureHighWarn=cardTemperatureHighWarn, cardPicDetected=cardPicDetected, cardPicHwVersion=cardPicHwVersion, cardHwVersion=cardHwVersion, portNumber=portNumber, cardCPLDVersion=cardCPLDVersion, cardTemperatureHighError=cardTemperatureHighError, cardTestScheduleCommand=cardTestScheduleCommand, cardId=cardId, portType=portType, auditTotalFailedCount=auditTotalFailedCount, cardDplxStatusChange=cardDplxStatusChange, diskDriveNumber=diskDriveNumber, cardTestId=cardTestId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardShelfId=cardShelfId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portCurrDsPower=portCurrDsPower, diskVolumeNumber=diskVolumeNumber, cardEntry=cardEntry, portMode=portMode, portNumChans=portNumChans, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageLevel=diskVolumeUsageLevel, portMaxDsPower=portMaxDsPower, downstreamPowerLoss=downstreamPowerLoss, diskVolumeEntry=diskVolumeEntry, cardTestEntry=cardTestEntry, cardBusSpeed=cardBusSpeed, systemGeneralGroup=systemGeneralGroup, cardTestName=cardTestName, trapSeverity=trapSeverity)
