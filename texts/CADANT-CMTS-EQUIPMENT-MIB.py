...
PortMode, CardId, PicType, SecondaryState, UnknownState, CardSubType, FirmwareSource, PortDetectedMode, EqActionType, AdminState, PortId, PrimaryState, CardType, ShelfId, PortType, DiskVolumeUsageLevel, FlowControlMode, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PortMode", "CardId", "PicType", "SecondaryState", "UnknownState", "CardSubType", "FirmwareSource", "PortDetectedMode", "EqActionType", "AdminState", "PortId", "PrimaryState", "CardType", "ShelfId", "PortType", "DiskVolumeUsageLevel", "FlowControlMode", "DuplexStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeId=diskVolumeId, cmResetNotification=cmResetNotification, cardDplxStatus=cardDplxStatus, shelfNumber=shelfNumber, errEvLoggingEnabled=errEvLoggingEnabled, equipmentShelfGroup=equipmentShelfGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSecState=cardSecState, portAction=portAction, fpgaErrorEventGroup=fpgaErrorEventGroup, cadEquipmentMib=cadEquipmentMib, portDplxStatusChange=portDplxStatusChange, cmResetUpTime=cmResetUpTime, cmIpAddress=cmIpAddress, shelfSwVersion=shelfSwVersion, cardTemperature=cardTemperature, cardTrapInh=cardTrapInh, cmMacAddress=cmMacAddress, TestScheduleCommand=TestScheduleCommand, diskVolumeNumber=diskVolumeNumber, cmResetInfo=cmResetInfo, TestTransactionId=TestTransactionId, portMode=portMode, portLastChangeTime=portLastChangeTime, TestType=TestType, cardTable=cardTable, fpgaErrorEventEntry=fpgaErrorEventEntry, shelfName=shelfName, cmResetClearNotification=cmResetClearNotification, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditAutoScheduling=auditAutoScheduling, auditTime=auditTime, trapCounter=trapCounter, diskVolumeSize=diskVolumeSize, TestCommand=TestCommand, auditTotalFailedCount=auditTotalFailedCount, cardNumber=cardNumber, cardCpuSpeed=cardCpuSpeed, portSecStateChange=portSecStateChange, cardTestEntry=cardTestEntry, cardTestTransId=cardTestTransId, cardEntry=cardEntry, portAdminState=portAdminState, cardLastBootVersion=cardLastBootVersion, portSecState=portSecState, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmVendor=cmVendor, equipmentStateGroup=equipmentStateGroup, portDplxStatus=portDplxStatus, auditCycleCount=auditCycleCount, auditTable=auditTable, auditResult=auditResult, cardFwVersion=cardFwVersion, cardId=cardId, fpgaErrorEventTable=fpgaErrorEventTable, cardTestCommand=cardTestCommand, auditPassedCount=auditPassedCount, PYSNMP_MODULE_ID=cadEquipmentMib, cmPrimarySID=cmPrimarySID, portBgpId=portBgpId, cardTestTime=cardTestTime, cardSubDetected=cardSubDetected, cardDetected=cardDetected, cardTestSchedule=cardTestSchedule, portEntry=portEntry, auditLogOutput=auditLogOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, cardSpareGroupId=cardSpareGroupId, portDocsIfIndex=portDocsIfIndex, cmDevice=cmDevice, cardBusSpeed=cardBusSpeed, portPrState=portPrState, systemKey=systemKey, cardTestId=cardTestId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEvLogLevel=errEvLogLevel, equipmentCardTestGroup=equipmentCardTestGroup, auditStatus=auditStatus, cardPicDetected=cardPicDetected, cardTestResultDesc=cardTestResultDesc, portCurrDsPower=portCurrDsPower, portMacAddress=portMacAddress, diskVolumeName=diskVolumeName, diskVolumeDriveId=diskVolumeDriveId, cardTestResult=cardTestResult, diskVolumeDiskSize=diskVolumeDiskSize, cardCarrierSerialNum=cardCarrierSerialNum, equipmentAuditGroup=equipmentAuditGroup, cardName=cardName, equipmentDiag=equipmentDiag, diskVolumeUsageLevel=diskVolumeUsageLevel, cardRamSize=cardRamSize, TestResult=TestResult, auditFailedCount=auditFailedCount, cardTestTable=cardTestTable, cardPicSerialNum=cardPicSerialNum, cardTestName=cardTestName, diskVolumeTrapInh=diskVolumeTrapInh, cardAction=cardAction, cardPrStateChange=cardPrStateChange, cardSpareGroupMode=cardSpareGroupMode, equipmentDiagGroup=equipmentDiagGroup, auditId=auditId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTemperatureHighWarn=cardTemperatureHighWarn, portCardSubType=portCardSubType, auditLogThrottle=auditLogThrottle, diagTestId=diagTestId, cardShelfTestIndex=cardShelfTestIndex, equipmentAudit=equipmentAudit, auditCardId=auditCardId, consoleOutput=consoleOutput, cardShelfId=cardShelfId, portRxFlowControlMode=portRxFlowControlMode, systemClock=systemClock, equipmentPortGroup=equipmentPortGroup, cardLastBootSource=cardLastBootSource, equipmentTbl=equipmentTbl, portId=portId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardAdminState=cardAdminState, cardSecStateChange=cardSecStateChange, portDetectedMode=portDetectedMode, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestDescription=cardTestDescription, diskVolumeTable=diskVolumeTable, portNumChans=portNumChans, portMacIfIndex=portMacIfIndex, cardBootVersion=cardBootVersion, cardLastResetReason=cardLastResetReason, portConnectorId=portConnectorId, auditTotalPassedCount=auditTotalPassedCount, cardCarrierHwVersion=cardCarrierHwVersion, systemGeneral=systemGeneral, cardCpuType=cardCpuType, portNumber=portNumber, cardCPLDVersion=cardCPLDVersion, portTxFlowControlDetected=portTxFlowControlDetected, TestSchedule=TestSchedule, cmRegistrationNotification=cmRegistrationNotification, eqDiagConfig=eqDiagConfig, systemGeneralGroup=systemGeneralGroup, equipmentError=equipmentError, cardHwVersion=cardHwVersion, cmResetReason=cmResetReason, cardTemperatureHighError=cardTemperatureHighError, equipmentCardGroup=equipmentCardGroup, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeGroup=diskVolumeGroup, cardSwVersion=cardSwVersion, cardLastChangeTime=cardLastChangeTime, remainInDiagMode=remainInDiagMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResultNotification=cardTestResultNotification, auditShelfId=auditShelfId, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeEntry=diskVolumeEntry, auditName=auditName, auditEntry=auditEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portTxFlowControlMode=portTxFlowControlMode, downstreamPowerLoss=downstreamPowerLoss, portMinDsPower=portMinDsPower, cardPrState=cardPrState, cardPicHwVersion=cardPicHwVersion, cardFlashSize=cardFlashSize, cardDplxStatusChange=cardDplxStatusChange, portDescription=portDescription, cardSubType=cardSubType, diskVolumeShelfId=diskVolumeShelfId, portShelfId=portShelfId, TestId=TestId, diskVolumeFileName=diskVolumeFileName, portTrapInh=portTrapInh, cardSerialNum=cardSerialNum, auditDescription=auditDescription, diskDriveNumber=diskDriveNumber, equipmentState=equipmentState, portPrStateChange=portPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, portTable=portTable, cmUChannelID=cmUChannelID, cmResetStatus=cmResetStatus, equipmentTraps=equipmentTraps, diskVolumeCardId=diskVolumeCardId, errEvRowStatus=errEvRowStatus, shelfObjects=shelfObjects, portCardId=portCardId, portType=portType, cardTestType=cardTestType, cardNumPorts=cardNumPorts, cardUpTime=cardUpTime, auditCommand=auditCommand, errEventId=errEventId, verboseLevel=verboseLevel, portMaxDsPower=portMaxDsPower, cardFpgaSource=cardFpgaSource, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, trapSeverity=trapSeverity, cardType=cardType, cardFwUpdateStatus=cardFwUpdateStatus, cardDetectedChange=cardDetectedChange, cardTestIndex=cardTestIndex, portRxFlowControlDetected=portRxFlowControlDetected)
