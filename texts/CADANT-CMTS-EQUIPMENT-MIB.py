...
PicType, DiskVolumeUsageLevel, DuplexStatus, PortMode, PortType, UnknownState, ShelfId, EqActionType, CardType, PortId, SecondaryState, CardSubType, AdminState, PortDetectedMode, PrimaryState, FirmwareSource, FlowControlMode, CardId = mibBuilder.importSymbols("CADANT-TC", "PicType", "DiskVolumeUsageLevel", "DuplexStatus", "PortMode", "PortType", "UnknownState", "ShelfId", "EqActionType", "CardType", "PortId", "SecondaryState", "CardSubType", "AdminState", "PortDetectedMode", "PrimaryState", "FirmwareSource", "FlowControlMode", "CardId")
...
TruthValue, MacAddress, RowStatus, DisplayString, TimeStamp, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "MacAddress", "RowStatus", "DisplayString", "TimeStamp", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentAuditGroup=equipmentAuditGroup, cardTestTable=cardTestTable, portShelfId=portShelfId, auditTable=auditTable, cardEntry=cardEntry, cmUChannelID=cmUChannelID, portAdminState=portAdminState, auditResult=auditResult, equipmentDiagGroup=equipmentDiagGroup, diskVolumeEntry=diskVolumeEntry, portConnectorId=portConnectorId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, trapCounter=trapCounter, diskVolumeDiskSize=diskVolumeDiskSize, verboseLevel=verboseLevel, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeFileName=diskVolumeFileName, cardRamSize=cardRamSize, cardFwUpdateStatus=cardFwUpdateStatus, cardTestCommand=cardTestCommand, cadEquipmentMibGroup=cadEquipmentMibGroup, systemKey=systemKey, portMode=portMode, cardPrStateChange=cardPrStateChange, cardSecStateChange=cardSecStateChange, cardSwVersion=cardSwVersion, cardTestSchedule=cardTestSchedule, TestCommand=TestCommand, cmResetStatus=cmResetStatus, cardTestResultNotification=cardTestResultNotification, portDplxStatus=portDplxStatus, cardAdminState=cardAdminState, auditCardId=auditCardId, cardDetected=cardDetected, cardBusSpeed=cardBusSpeed, diskVolumeTable=diskVolumeTable, cardNumPorts=cardNumPorts, portBgpId=portBgpId, auditEntry=auditEntry, cmIpAddress=cmIpAddress, cardPicHwVersion=cardPicHwVersion, cardShelfId=cardShelfId, diskDriveNumber=diskDriveNumber, cardTestScheduleCommand=cardTestScheduleCommand, equipmentCardGroup=equipmentCardGroup, portTable=portTable, cmResetUpTime=cmResetUpTime, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMacIfIndex=portMacIfIndex, cadEquipmentMib=cadEquipmentMib, cardSpareGroupMode=cardSpareGroupMode, cardSubType=cardSubType, cardTrapInh=cardTrapInh, portDescription=portDescription, equipmentAudit=equipmentAudit, cardCarrierSerialNum=cardCarrierSerialNum, equipmentDiag=equipmentDiag, cardCarrierHwVersion=cardCarrierHwVersion, cardTestIndex=cardTestIndex, cadEquipmentMibConformance=cadEquipmentMibConformance, auditCommand=auditCommand, auditId=auditId, cardTestTime=cardTestTime, cardTestResultDesc=cardTestResultDesc, cardTestName=cardTestName, errEvRowStatus=errEvRowStatus, diskVolumeId=diskVolumeId, auditStatus=auditStatus, cardAction=cardAction, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditPassedCount=auditPassedCount, cardName=cardName, portMaxDsPower=portMaxDsPower, cardSpareGroupId=cardSpareGroupId, portNumChans=portNumChans, cardLastBootSource=cardLastBootSource, cardLastResetReason=cardLastResetReason, equipmentError=equipmentError, diagTestId=diagTestId, cardBootVersion=cardBootVersion, diskVolumeGroup=diskVolumeGroup, cardCpuSpeed=cardCpuSpeed, auditDescription=auditDescription, cmResetInfo=cmResetInfo, portCardSubType=portCardSubType, auditFailedCount=auditFailedCount, diskVolumeSize=diskVolumeSize, portTrapInh=portTrapInh, portId=portId, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCPLDVersion=cardCPLDVersion, cardTestDescription=cardTestDescription, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSerialNum=cardSerialNum, portCurrDsPower=portCurrDsPower, consoleOutput=consoleOutput, equipmentTbl=equipmentTbl, portType=portType, portMacAddress=portMacAddress, diskVolumeShelfId=diskVolumeShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, cardLastChangeTime=cardLastChangeTime, cmPrimarySID=cmPrimarySID, systemGeneralGroup=systemGeneralGroup, TestScheduleCommand=TestScheduleCommand, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeName=diskVolumeName, cmDevice=cmDevice, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portNumber=portNumber, portSecStateChange=portSecStateChange, auditAutoScheduling=auditAutoScheduling, eqDiagConfig=eqDiagConfig, cardFpgaSource=cardFpgaSource, cardTestTransId=cardTestTransId, equipmentStateGroup=equipmentStateGroup, cardShelfTestIndex=cardShelfTestIndex, cmRegistrationNotification=cmRegistrationNotification, portDetectedMode=portDetectedMode, portLastChangeTime=portLastChangeTime, cardPrState=cardPrState, equipmentShelfGroup=equipmentShelfGroup, cardTempNoReportNotification=cardTempNoReportNotification, equipmentTraps=equipmentTraps, TestTransactionId=TestTransactionId, equipmentState=equipmentState, portRxFlowControlMode=portRxFlowControlMode, cardFwVersion=cardFwVersion, cmResetNotification=cmResetNotification, cardSecState=cardSecState, cardHwVersion=cardHwVersion, cardTestResult=cardTestResult, PYSNMP_MODULE_ID=cadEquipmentMib, cardTable=cardTable, cardNumber=cardNumber, trapSeverity=trapSeverity, cardTestId=cardTestId, systemGeneral=systemGeneral, portEntry=portEntry, TestResult=TestResult, portAction=portAction, cardDetectedChange=cardDetectedChange, diskVolumeNumber=diskVolumeNumber, portPrStateChange=portPrStateChange, fpgaErrorEventGroup=fpgaErrorEventGroup, cmMacAddress=cmMacAddress, auditLogThrottle=auditLogThrottle, errEventId=errEventId, portPrState=portPrState, equipmentPortGroup=equipmentPortGroup, TestType=TestType, cardTestEntry=cardTestEntry, remainInDiagMode=remainInDiagMode, shelfObjects=shelfObjects, portDocsIfIndex=portDocsIfIndex, shelfNumber=shelfNumber, cardPicDetected=cardPicDetected, auditShelfId=auditShelfId, auditTotalFailedCount=auditTotalFailedCount, equipmentCardTestGroup=equipmentCardTestGroup, cmResetReason=cmResetReason, cardTemperatureHighError=cardTemperatureHighError, cardPicSerialNum=cardPicSerialNum, cardTemperature=cardTemperature, cardId=cardId, cardTestType=cardTestType, cardFlashSize=cardFlashSize, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCpuType=cardCpuType, cardSubDetected=cardSubDetected, portTxFlowControlDetected=portTxFlowControlDetected, TestId=TestId, fpgaErrorEventTable=fpgaErrorEventTable, TestSchedule=TestSchedule, downstreamPowerLoss=downstreamPowerLoss, cardDplxStatus=cardDplxStatus, cardUpTime=cardUpTime, cardTempOverHeatNotification=cardTempOverHeatNotification, portMinDsPower=portMinDsPower, cadEquipmentMibCompliance=cadEquipmentMibCompliance, systemClock=systemClock, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardDplxStatusChange=cardDplxStatusChange, auditTotalPassedCount=auditTotalPassedCount, errEvLoggingEnabled=errEvLoggingEnabled, shelfSwVersion=shelfSwVersion, auditName=auditName, errEvLogLevel=errEvLogLevel, cardCarrierFwVersion=cardCarrierFwVersion, cmVendor=cmVendor, diskVolumeCardId=diskVolumeCardId, portDplxStatusChange=portDplxStatusChange, portSecState=portSecState, auditCycleCount=auditCycleCount, portTxFlowControlMode=portTxFlowControlMode, portCardId=portCardId, auditTime=auditTime, shelfName=shelfName, cmResetClearNotification=cmResetClearNotification, cardLastBootVersion=cardLastBootVersion, diskVolumeTrapInh=diskVolumeTrapInh, auditLogOutput=auditLogOutput, cardType=cardType, diskVolumeUsageLevel=diskVolumeUsageLevel)
