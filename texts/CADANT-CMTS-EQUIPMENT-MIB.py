...
PortMode, PrimaryState, PortType, EqActionType, FirmwareSource, DiskVolumeUsageLevel, CardType, UnknownState, ShelfId, PortId, DuplexStatus, PicType, CardId, AdminState, SecondaryState, PortDetectedMode, CardSubType, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "PortMode", "PrimaryState", "PortType", "EqActionType", "FirmwareSource", "DiskVolumeUsageLevel", "CardType", "UnknownState", "ShelfId", "PortId", "DuplexStatus", "PicType", "CardId", "AdminState", "SecondaryState", "PortDetectedMode", "CardSubType", "FlowControlMode")
...
TimeStamp, MacAddress, TruthValue, RowStatus, DisplayString, DateAndTime, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "MacAddress", "TruthValue", "RowStatus", "DisplayString", "DateAndTime", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", shelfSwVersion=shelfSwVersion, cardTestTransId=cardTestTransId, portLastChangeTime=portLastChangeTime, cardDplxStatusChange=cardDplxStatusChange, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestName=cardTestName, auditAutoScheduling=auditAutoScheduling, auditStatus=auditStatus, cardCPLDVersion=cardCPLDVersion, auditEntry=auditEntry, portCardId=portCardId, cardLastResetReason=cardLastResetReason, cardLastBootSource=cardLastBootSource, cardSwVersion=cardSwVersion, portEntry=portEntry, TestSchedule=TestSchedule, cadEquipmentMibGroup=cadEquipmentMibGroup, portTrapInh=portTrapInh, cardTestType=cardTestType, cardHwVersion=cardHwVersion, portMacIfIndex=portMacIfIndex, TestTransactionId=TestTransactionId, cardTestResult=cardTestResult, cadEquipmentMib=cadEquipmentMib, auditResult=auditResult, cmPrimarySID=cmPrimarySID, cardSerialNum=cardSerialNum, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFwUpdateStatus=cardFwUpdateStatus, portNumChans=portNumChans, auditId=auditId, cmVendor=cmVendor, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentDiagGroup=equipmentDiagGroup, cardEntry=cardEntry, cardSubType=cardSubType, cardDetected=cardDetected, cardCpuSpeed=cardCpuSpeed, cardPicDetected=cardPicDetected, systemGeneral=systemGeneral, portType=portType, eqDiagConfig=eqDiagConfig, cardAdminState=cardAdminState, diskVolumeId=diskVolumeId, auditCycleCount=auditCycleCount, portDplxStatus=portDplxStatus, cardTrapInh=cardTrapInh, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResultNotification=cardTestResultNotification, equipmentTbl=equipmentTbl, shelfObjects=shelfObjects, cardAction=cardAction, cardLastBootVersion=cardLastBootVersion, cardName=cardName, auditTable=auditTable, diagTestId=diagTestId, cardFlashSize=cardFlashSize, cardCarrierSerialNum=cardCarrierSerialNum, cardId=cardId, auditShelfId=auditShelfId, portBgpId=portBgpId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portRxFlowControlMode=portRxFlowControlMode, diskVolumeFileName=diskVolumeFileName, equipmentError=equipmentError, cardCpuType=cardCpuType, cardSpareGroupId=cardSpareGroupId, cardFpgaSource=cardFpgaSource, cardTestIndex=cardTestIndex, cardNumber=cardNumber, cmUChannelID=cmUChannelID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeEntry=diskVolumeEntry, cardSpareGroupMode=cardSpareGroupMode, equipmentDiag=equipmentDiag, auditTime=auditTime, cardTempNoReportNotification=cardTempNoReportNotification, cardDplxStatus=cardDplxStatus, TestScheduleCommand=TestScheduleCommand, cardTestTime=cardTestTime, portSecState=portSecState, diskVolumeShelfId=diskVolumeShelfId, cardSecState=cardSecState, cardNumPorts=cardNumPorts, cardTestTable=cardTestTable, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicSerialNum=cardPicSerialNum, equipmentStateGroup=equipmentStateGroup, errEvLoggingEnabled=errEvLoggingEnabled, cardTestId=cardTestId, cardTemperature=cardTemperature, diskVolumeCardId=diskVolumeCardId, portSecStateChange=portSecStateChange, diskVolumeGroup=diskVolumeGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTable=cardTable, cardType=cardType, portTxFlowControlMode=portTxFlowControlMode, diskVolumeTable=diskVolumeTable, systemKey=systemKey, fpgaErrorEventTable=fpgaErrorEventTable, cmRegistrationNotification=cmRegistrationNotification, systemGeneralGroup=systemGeneralGroup, cardPrState=cardPrState, downstreamPowerLoss=downstreamPowerLoss, equipmentAuditGroup=equipmentAuditGroup, cardSubDetected=cardSubDetected, portId=portId, shelfName=shelfName, cardFwVersion=cardFwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTemperatureHighError=cardTemperatureHighError, portAdminState=portAdminState, portNumber=portNumber, cardShelfTestIndex=cardShelfTestIndex, diskVolumeTrapInh=diskVolumeTrapInh, equipmentCardGroup=equipmentCardGroup, cmResetInfo=cmResetInfo, errEvRowStatus=errEvRowStatus, shelfNumber=shelfNumber, auditCardId=auditCardId, cmIpAddress=cmIpAddress, cardTestCommand=cardTestCommand, equipmentAudit=equipmentAudit, cmResetStatus=cmResetStatus, errEvLogLevel=errEvLogLevel, cardTestDescription=cardTestDescription, portCardSubType=portCardSubType, trapSeverity=trapSeverity, cmResetClearNotification=cmResetClearNotification, remainInDiagMode=remainInDiagMode, cmResetReason=cmResetReason, portMaxDsPower=portMaxDsPower, cardTestScheduleCommand=cardTestScheduleCommand, diskDriveNumber=diskDriveNumber, auditDescription=auditDescription, portTxFlowControlDetected=portTxFlowControlDetected, fpgaErrorEventGroup=fpgaErrorEventGroup, cardPrStateChange=cardPrStateChange, cardCarrierFwVersion=cardCarrierFwVersion, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeDiskSize=diskVolumeDiskSize, equipmentState=equipmentState, cardBootVersion=cardBootVersion, cardBusSpeed=cardBusSpeed, TestType=TestType, diskVolumeSize=diskVolumeSize, systemClock=systemClock, equipmentTraps=equipmentTraps, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeName=diskVolumeName, portAction=portAction, cardTemperatureHighWarn=cardTemperatureHighWarn, TestCommand=TestCommand, cardUpTime=cardUpTime, cardRamSize=cardRamSize, auditTotalPassedCount=auditTotalPassedCount, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cmResetUpTime=cmResetUpTime, cmMacAddress=cmMacAddress, cardTestResultDesc=cardTestResultDesc, portPrState=portPrState, portDocsIfIndex=portDocsIfIndex, auditName=auditName, portPrStateChange=portPrStateChange, portMinDsPower=portMinDsPower, auditFailedCount=auditFailedCount, portMode=portMode, diskVolumeUsageLevel=diskVolumeUsageLevel, portCurrDsPower=portCurrDsPower, cardPicHwVersion=cardPicHwVersion, auditLogOutput=auditLogOutput, portDescription=portDescription, cardSecStateChange=cardSecStateChange, portShelfId=portShelfId, portRxFlowControlDetected=portRxFlowControlDetected, portMacAddress=portMacAddress, auditCommand=auditCommand, TestId=TestId, cardCarrierHwVersion=cardCarrierHwVersion, cmDevice=cmDevice, errEventId=errEventId, cardTestSchedule=cardTestSchedule, equipmentShelfGroup=equipmentShelfGroup, verboseLevel=verboseLevel, diskVolumeNumber=diskVolumeNumber, cardShelfId=cardShelfId, consoleOutput=consoleOutput, cardTestEntry=cardTestEntry, portConnectorId=portConnectorId, cardLastChangeTime=cardLastChangeTime, cardTempOverHeatNotification=cardTempOverHeatNotification, cardDetectedChange=cardDetectedChange, portDplxStatusChange=portDplxStatusChange, diskVolumeDriveId=diskVolumeDriveId, auditLogThrottle=auditLogThrottle, portTable=portTable, portDetectedMode=portDetectedMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cmResetNotification=cmResetNotification, equipmentPortGroup=equipmentPortGroup, trapCounter=trapCounter, auditPassedCount=auditPassedCount, TestResult=TestResult, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile)
