...
FlowControlMode, DuplexStatus, CardType, CardId, EqActionType, AdminState, PrimaryState, DiskVolumeUsageLevel, PortMode, ShelfId, PicType, UnknownState, PortType, SecondaryState, FirmwareSource, CardSubType, PortId, PortDetectedMode = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "DuplexStatus", "CardType", "CardId", "EqActionType", "AdminState", "PrimaryState", "DiskVolumeUsageLevel", "PortMode", "ShelfId", "PicType", "UnknownState", "PortType", "SecondaryState", "FirmwareSource", "CardSubType", "PortId", "PortDetectedMode")
...
TruthValue, TimeStamp, TextualConvention, RowStatus, DateAndTime, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TimeStamp", "TextualConvention", "RowStatus", "DateAndTime", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTime=auditTime, portDplxStatusChange=portDplxStatusChange, portRxFlowControlDetected=portRxFlowControlDetected, cmResetReason=cmResetReason, cardBusSpeed=cardBusSpeed, systemGeneralGroup=systemGeneralGroup, cardDetected=cardDetected, portNumber=portNumber, cardShelfId=cardShelfId, portCardSubType=portCardSubType, cardTestEntry=cardTestEntry, portMaxDsPower=portMaxDsPower, portConnectorId=portConnectorId, portTxFlowControlDetected=portTxFlowControlDetected, portDetectedMode=portDetectedMode, cmResetInfo=cmResetInfo, auditLogThrottle=auditLogThrottle, cardTestResultDesc=cardTestResultDesc, auditTotalFailedCount=auditTotalFailedCount, cardPrState=cardPrState, cardTrapInh=cardTrapInh, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeDriveId=diskVolumeDriveId, eqDiagConfig=eqDiagConfig, cardDplxStatus=cardDplxStatus, TestTransactionId=TestTransactionId, cardCPLDVersion=cardCPLDVersion, diskVolumeSize=diskVolumeSize, auditResult=auditResult, diskVolumeFileName=diskVolumeFileName, portTrapInh=portTrapInh, auditTotalPassedCount=auditTotalPassedCount, cadEquipmentMibGroup=cadEquipmentMibGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCarrierHwVersion=cardCarrierHwVersion, TestSchedule=TestSchedule, portMacIfIndex=portMacIfIndex, auditTable=auditTable, portAction=portAction, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, TestScheduleCommand=TestScheduleCommand, cardPicDetected=cardPicDetected, portPrStateChange=portPrStateChange, cardCpuType=cardCpuType, cardShelfTestIndex=cardShelfTestIndex, auditLogOutput=auditLogOutput, cmVendor=cmVendor, auditPassedCount=auditPassedCount, portPrState=portPrState, cardTestCommand=cardTestCommand, auditCommand=auditCommand, cmResetNotification=cmResetNotification, cardSubDetected=cardSubDetected, equipmentCardGroup=equipmentCardGroup, cardType=cardType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portNumChans=portNumChans, equipmentAuditGroup=equipmentAuditGroup, systemGeneral=systemGeneral, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentAudit=equipmentAudit, cardSecStateChange=cardSecStateChange, shelfNumber=shelfNumber, TestCommand=TestCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardRamSize=cardRamSize, portId=portId, portRxFlowControlMode=portRxFlowControlMode, diskVolumeEntry=diskVolumeEntry, fpgaErrorEventTable=fpgaErrorEventTable, cardFwVersion=cardFwVersion, equipmentPortGroup=equipmentPortGroup, equipmentStateGroup=equipmentStateGroup, cardLastChangeTime=cardLastChangeTime, auditShelfId=auditShelfId, portMode=portMode, cardSecState=cardSecState, portDescription=portDescription, cardPrStateChange=cardPrStateChange, cardTestTable=cardTestTable, portLastChangeTime=portLastChangeTime, equipmentError=equipmentError, diskVolumeCardId=diskVolumeCardId, cardDetectedChange=cardDetectedChange, cardName=cardName, systemKey=systemKey, cardFpgaSource=cardFpgaSource, cardTestTime=cardTestTime, diskVolumeTable=diskVolumeTable, cardDplxStatusChange=cardDplxStatusChange, cmResetStatus=cmResetStatus, diskVolumeName=diskVolumeName, auditId=auditId, cardTestTransId=cardTestTransId, portDplxStatus=portDplxStatus, portCardId=portCardId, cardCpuSpeed=cardCpuSpeed, cardTestDescription=cardTestDescription, cardTestName=cardTestName, errEventId=errEventId, downstreamPowerLoss=downstreamPowerLoss, cardTestResultNotification=cardTestResultNotification, equipmentDiag=equipmentDiag, cardTestResult=cardTestResult, cadEquipmentMib=cadEquipmentMib, shelfSwVersion=shelfSwVersion, cardTestIndex=cardTestIndex, diskVolumeDiskSize=diskVolumeDiskSize, portMinDsPower=portMinDsPower, cardTestScheduleCommand=cardTestScheduleCommand, equipmentShelfGroup=equipmentShelfGroup, systemClock=systemClock, cardSerialNum=cardSerialNum, auditEntry=auditEntry, portType=portType, diskVolumeTrapInh=diskVolumeTrapInh, cmResetUpTime=cmResetUpTime, portTxFlowControlMode=portTxFlowControlMode, portDocsIfIndex=portDocsIfIndex, auditCardId=auditCardId, auditStatus=auditStatus, remainInDiagMode=remainInDiagMode, cardTemperatureHighError=cardTemperatureHighError, equipmentTraps=equipmentTraps, errEvLoggingEnabled=errEvLoggingEnabled, portCurrDsPower=portCurrDsPower, cmPrimarySID=cmPrimarySID, equipmentCardTestGroup=equipmentCardTestGroup, diskVolumeId=diskVolumeId, auditCycleCount=auditCycleCount, trapSeverity=trapSeverity, cardSubType=cardSubType, trapCounter=trapCounter, diskDriveNumber=diskDriveNumber, errEvRowStatus=errEvRowStatus, TestType=TestType, portAdminState=portAdminState, cmRegistrationNotification=cmRegistrationNotification, diskVolumeGroup=diskVolumeGroup, cardCarrierFwVersion=cardCarrierFwVersion, TestId=TestId, cardNumber=cardNumber, cardTestType=cardTestType, cardLastBootVersion=cardLastBootVersion, verboseLevel=verboseLevel, errEvRecoveryEnabled=errEvRecoveryEnabled, cardEntry=cardEntry, cardTable=cardTable, auditName=auditName, cardNumPorts=cardNumPorts, cardCarrierSerialNum=cardCarrierSerialNum, cardPicHwVersion=cardPicHwVersion, cardTempNoReportNotification=cardTempNoReportNotification, equipmentState=equipmentState, cardFlashSize=cardFlashSize, auditAutoScheduling=auditAutoScheduling, fpgaErrorEventGroup=fpgaErrorEventGroup, cardAdminState=cardAdminState, cardTestSchedule=cardTestSchedule, diagTestId=diagTestId, cardAction=cardAction, cardHwVersion=cardHwVersion, auditDescription=auditDescription, auditFailedCount=auditFailedCount, cmResetClearNotification=cmResetClearNotification, cmDevice=cmDevice, cardTemperature=cardTemperature, portTable=portTable, diskVolumeNumber=diskVolumeNumber, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicSerialNum=cardPicSerialNum, cadEquipmentMibCompliance=cadEquipmentMibCompliance, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentTbl=equipmentTbl, cadEquipmentMibConformance=cadEquipmentMibConformance, cardSpareGroupId=cardSpareGroupId, consoleOutput=consoleOutput, cardFwUpdateStatus=cardFwUpdateStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardId=cardId, portEntry=portEntry, cardLastResetReason=cardLastResetReason, portSecStateChange=portSecStateChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvLogLevel=errEvLogLevel, cardSwVersion=cardSwVersion, portMacAddress=portMacAddress, cmIpAddress=cmIpAddress, cardLastBootSource=cardLastBootSource, equipmentDiagGroup=equipmentDiagGroup, shelfObjects=shelfObjects, TestResult=TestResult, cardSpareGroupMode=cardSpareGroupMode, cmMacAddress=cmMacAddress, cmUChannelID=cmUChannelID, cardTestId=cardTestId, shelfName=shelfName, cardBootVersion=cardBootVersion, portShelfId=portShelfId, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, portSecState=portSecState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portBgpId=portBgpId, cardUpTime=cardUpTime)
