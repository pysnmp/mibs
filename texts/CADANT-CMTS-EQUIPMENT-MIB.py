...
FlowControlMode, CardSubType, DuplexStatus, EqActionType, DiskVolumeUsageLevel, PortId, FirmwareSource, PortDetectedMode, AdminState, PortMode, UnknownState, ShelfId, PortType, SecondaryState, PrimaryState, PicType, CardId, CardType = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "CardSubType", "DuplexStatus", "EqActionType", "DiskVolumeUsageLevel", "PortId", "FirmwareSource", "PortDetectedMode", "AdminState", "PortMode", "UnknownState", "ShelfId", "PortType", "SecondaryState", "PrimaryState", "PicType", "CardId", "CardType")
...
MacAddress, TimeStamp, DisplayString, DateAndTime, TruthValue, RowStatus, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TimeStamp", "DisplayString", "DateAndTime", "TruthValue", "RowStatus", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardTrapInh=cardTrapInh, portDescription=portDescription, cardTestCommand=cardTestCommand, portTxFlowControlMode=portTxFlowControlMode, portRxFlowControlMode=portRxFlowControlMode, cardTestIndex=cardTestIndex, verboseLevel=verboseLevel, cardTestDescription=cardTestDescription, portEntry=portEntry, equipmentDiagGroup=equipmentDiagGroup, cardEntry=cardEntry, cardSecStateChange=cardSecStateChange, auditLogOutput=auditLogOutput, cmResetStatus=cmResetStatus, cardCpuSpeed=cardCpuSpeed, cardTestName=cardTestName, cardTestSchedule=cardTestSchedule, cardShelfId=cardShelfId, portDplxStatus=portDplxStatus, auditLogThrottle=auditLogThrottle, cardTestResultDesc=cardTestResultDesc, portAction=portAction, cardShelfTestIndex=cardShelfTestIndex, cardSwVersion=cardSwVersion, cardLastBootSource=cardLastBootSource, diskVolumeUsageLevel=diskVolumeUsageLevel, portDetectedMode=portDetectedMode, portPrState=portPrState, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeName=diskVolumeName, cmResetReason=cmResetReason, cardFpgaSource=cardFpgaSource, cardPicDetected=cardPicDetected, shelfNumber=shelfNumber, shelfName=shelfName, portNumChans=portNumChans, cardNumber=cardNumber, systemClock=systemClock, cardSerialNum=cardSerialNum, cardPrState=cardPrState, TestScheduleCommand=TestScheduleCommand, cardAction=cardAction, cardHwVersion=cardHwVersion, auditDescription=auditDescription, cmResetUpTime=cmResetUpTime, cardUpTime=cardUpTime, equipmentCardTestGroup=equipmentCardTestGroup, cardPrStateChange=cardPrStateChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeGroup=diskVolumeGroup, cardCarrierFwVersion=cardCarrierFwVersion, auditCycleCount=auditCycleCount, systemGeneralGroup=systemGeneralGroup, TestTransactionId=TestTransactionId, portMacIfIndex=portMacIfIndex, diskVolumeCardId=diskVolumeCardId, cmDevice=cmDevice, cardSpareGroupId=cardSpareGroupId, errEvLoggingEnabled=errEvLoggingEnabled, diskDriveNumber=diskDriveNumber, errEventId=errEventId, cardNumPorts=cardNumPorts, diskVolumeSize=diskVolumeSize, cadEquipmentMibConformance=cadEquipmentMibConformance, portConnectorId=portConnectorId, cardTestTime=cardTestTime, cardSecState=cardSecState, auditEntry=auditEntry, diskVolumeNumber=diskVolumeNumber, cardTestResultNotification=cardTestResultNotification, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentAuditGroup=equipmentAuditGroup, cmPrimarySID=cmPrimarySID, cardDplxStatusChange=cardDplxStatusChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardDplxStatus=cardDplxStatus, errEvRowStatus=errEvRowStatus, remainInDiagMode=remainInDiagMode, portShelfId=portShelfId, cardLastResetReason=cardLastResetReason, portMacAddress=portMacAddress, diskVolumeEntry=diskVolumeEntry, cardTestEntry=cardTestEntry, diskVolumeTrapInh=diskVolumeTrapInh, portSecStateChange=portSecStateChange, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardFwUpdateStatus=cardFwUpdateStatus, cardRamSize=cardRamSize, cadEquipmentMib=cadEquipmentMib, TestId=TestId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portNumber=portNumber, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCPLDVersion=cardCPLDVersion, TestResult=TestResult, cardSpareGroupMode=cardSpareGroupMode, diskVolumeId=diskVolumeId, portPrStateChange=portPrStateChange, cardId=cardId, portAdminState=portAdminState, portTrapInh=portTrapInh, cardFlashSize=cardFlashSize, auditTotalFailedCount=auditTotalFailedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicSerialNum=cardPicSerialNum, diskVolumeTable=diskVolumeTable, cardSubType=cardSubType, auditId=auditId, errEvLogLevel=errEvLogLevel, auditShelfId=auditShelfId, equipmentTbl=equipmentTbl, equipmentPortGroup=equipmentPortGroup, portType=portType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmResetNotification=cmResetNotification, errEvRecoveryEnabled=errEvRecoveryEnabled, systemKey=systemKey, systemGeneral=systemGeneral, cmResetInfo=cmResetInfo, cardName=cardName, auditStatus=auditStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, auditTable=auditTable, cmResetClearNotification=cmResetClearNotification, downstreamPowerLoss=downstreamPowerLoss, TestCommand=TestCommand, portMode=portMode, cardTestTransId=cardTestTransId, cardFwVersion=cardFwVersion, cardCpuType=cardCpuType, cardBootVersion=cardBootVersion, equipmentStateGroup=equipmentStateGroup, auditResult=auditResult, cardTestScheduleCommand=cardTestScheduleCommand, equipmentError=equipmentError, cardSubDetected=cardSubDetected, shelfSwVersion=shelfSwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeDriveId=diskVolumeDriveId, cmUChannelID=cmUChannelID, cardAdminState=cardAdminState, portBgpId=portBgpId, consoleOutput=consoleOutput, portCardId=portCardId, portId=portId, portSecState=portSecState, auditCommand=auditCommand, auditTotalPassedCount=auditTotalPassedCount, cardTemperatureHighError=cardTemperatureHighError, portTxFlowControlDetected=portTxFlowControlDetected, cardTable=cardTable, cardDetectedChange=cardDetectedChange, portMinDsPower=portMinDsPower, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeUsageNotification=diskVolumeUsageNotification, portCurrDsPower=portCurrDsPower, cardLastChangeTime=cardLastChangeTime, equipmentDiag=equipmentDiag, diskVolumeFileName=diskVolumeFileName, eqDiagConfig=eqDiagConfig, cardTempNoReportNotification=cardTempNoReportNotification, equipmentState=equipmentState, shelfObjects=shelfObjects, portDocsIfIndex=portDocsIfIndex, cmIpAddress=cmIpAddress, TestSchedule=TestSchedule, cardDetected=cardDetected, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestType=cardTestType, cardTemperature=cardTemperature, diskVolumeDiskSize=diskVolumeDiskSize, auditFailedCount=auditFailedCount, TestType=TestType, cardTemperatureHighWarn=cardTemperatureHighWarn, cmMacAddress=cmMacAddress, cardPicHwVersion=cardPicHwVersion, portDplxStatusChange=portDplxStatusChange, diagTestId=diagTestId, trapSeverity=trapSeverity, auditAutoScheduling=auditAutoScheduling, portRxFlowControlDetected=portRxFlowControlDetected, cardType=cardType, cardBusSpeed=cardBusSpeed, cardTestId=cardTestId, cardTestResult=cardTestResult, auditTime=auditTime, cmVendor=cmVendor, portLastChangeTime=portLastChangeTime, cardTestTable=cardTestTable, auditCardId=auditCardId, auditName=auditName, cmRegistrationNotification=cmRegistrationNotification, portCardSubType=portCardSubType, equipmentAudit=equipmentAudit, portTable=portTable, portMaxDsPower=portMaxDsPower, equipmentCardGroup=equipmentCardGroup, auditPassedCount=auditPassedCount, trapCounter=trapCounter, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentTraps=equipmentTraps, cardLastBootVersion=cardLastBootVersion, diskVolumeShelfId=diskVolumeShelfId, equipmentShelfGroup=equipmentShelfGroup)
