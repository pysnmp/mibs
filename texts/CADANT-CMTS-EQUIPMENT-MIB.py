...
ShelfId, PortId, PortType, CardId, DiskVolumeUsageLevel, FirmwareSource, FlowControlMode, EqActionType, DuplexStatus, CardType, AdminState, CardSubType, PortDetectedMode, UnknownState, PortMode, SecondaryState, PrimaryState, PicType = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "PortId", "PortType", "CardId", "DiskVolumeUsageLevel", "FirmwareSource", "FlowControlMode", "EqActionType", "DuplexStatus", "CardType", "AdminState", "CardSubType", "PortDetectedMode", "UnknownState", "PortMode", "SecondaryState", "PrimaryState", "PicType")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeNumber=diskVolumeNumber, portMacAddress=portMacAddress, portTxFlowControlDetected=portTxFlowControlDetected, portTrapInh=portTrapInh, cmResetStatus=cmResetStatus, cardType=cardType, cardSpareGroupMode=cardSpareGroupMode, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, errEventId=errEventId, cardTemperature=cardTemperature, portEntry=portEntry, portRxFlowControlDetected=portRxFlowControlDetected, cardTestSchedule=cardTestSchedule, portAction=portAction, cardSerialNum=cardSerialNum, auditFailedCount=auditFailedCount, cmRegistrationNotification=cmRegistrationNotification, cmUChannelID=cmUChannelID, cardRamSize=cardRamSize, auditName=auditName, cardTemperatureHighError=cardTemperatureHighError, cardAdminState=cardAdminState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmDevice=cmDevice, verboseLevel=verboseLevel, equipmentTbl=equipmentTbl, portDplxStatus=portDplxStatus, cmMacAddress=cmMacAddress, cmResetNotification=cmResetNotification, TestCommand=TestCommand, cardLastChangeTime=cardLastChangeTime, TestTransactionId=TestTransactionId, auditResult=auditResult, auditTotalFailedCount=auditTotalFailedCount, portLastChangeTime=portLastChangeTime, cardBootVersion=cardBootVersion, cardBusSpeed=cardBusSpeed, diagTestId=diagTestId, cardAction=cardAction, auditCardId=auditCardId, portSecStateChange=portSecStateChange, cardPicSerialNum=cardPicSerialNum, portMinDsPower=portMinDsPower, cardCPLDVersion=cardCPLDVersion, diskVolumeId=diskVolumeId, downstreamPowerLoss=downstreamPowerLoss, cardLastBootVersion=cardLastBootVersion, equipmentState=equipmentState, cardTable=cardTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestResultDesc=cardTestResultDesc, cardPrState=cardPrState, auditLogOutput=auditLogOutput, TestResult=TestResult, cardTestResult=cardTestResult, portId=portId, equipmentCardTestGroup=equipmentCardTestGroup, cmIpAddress=cmIpAddress, diskVolumeDriveId=diskVolumeDriveId, diskVolumeFileName=diskVolumeFileName, TestSchedule=TestSchedule, equipmentTraps=equipmentTraps, auditAutoScheduling=auditAutoScheduling, auditShelfId=auditShelfId, portPrStateChange=portPrStateChange, consoleOutput=consoleOutput, cardTestId=cardTestId, equipmentStateGroup=equipmentStateGroup, portMode=portMode, diskVolumeGroup=diskVolumeGroup, portDescription=portDescription, diskVolumeTable=diskVolumeTable, cardLastBootSource=cardLastBootSource, cardDplxStatusChange=cardDplxStatusChange, cardTestTransId=cardTestTransId, portCardSubType=portCardSubType, equipmentError=equipmentError, TestType=TestType, portSecState=portSecState, cardSecStateChange=cardSecStateChange, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDocsIfIndex=portDocsIfIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeName=diskVolumeName, auditCommand=auditCommand, portTxFlowControlMode=portTxFlowControlMode, systemGeneralGroup=systemGeneralGroup, cardCpuType=cardCpuType, cardFwVersion=cardFwVersion, cardTestEntry=cardTestEntry, fpgaErrorEventGroup=fpgaErrorEventGroup, auditCycleCount=auditCycleCount, fpgaErrorEventTable=fpgaErrorEventTable, portConnectorId=portConnectorId, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentAuditGroup=equipmentAuditGroup, cmResetReason=cmResetReason, diskVolumeTrapInh=diskVolumeTrapInh, trapSeverity=trapSeverity, diskVolumeSize=diskVolumeSize, auditTime=auditTime, portAdminState=portAdminState, portRxFlowControlMode=portRxFlowControlMode, cardCarrierSerialNum=cardCarrierSerialNum, cardFlashSize=cardFlashSize, eqDiagConfig=eqDiagConfig, cardTestResultNotification=cardTestResultNotification, cmVendor=cmVendor, cardPicHwVersion=cardPicHwVersion, portMacIfIndex=portMacIfIndex, diskVolumeShelfId=diskVolumeShelfId, cardTempNoReportNotification=cardTempNoReportNotification, cardEntry=cardEntry, cardSubType=cardSubType, cardSpareGroupId=cardSpareGroupId, auditLogThrottle=auditLogThrottle, auditTable=auditTable, shelfSwVersion=shelfSwVersion, cardHwVersion=cardHwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardShelfTestIndex=cardShelfTestIndex, auditEntry=auditEntry, cmPrimarySID=cmPrimarySID, systemKey=systemKey, cardTestTime=cardTestTime, portNumber=portNumber, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentPortGroup=equipmentPortGroup, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestName=cardTestName, cardTestTable=cardTestTable, remainInDiagMode=remainInDiagMode, cardTestType=cardTestType, errEvRecoveryEnabled=errEvRecoveryEnabled, cardShelfId=cardShelfId, portBgpId=portBgpId, portDplxStatusChange=portDplxStatusChange, cardId=cardId, TestId=TestId, diskVolumeCardId=diskVolumeCardId, portNumChans=portNumChans, cadEquipmentMibCompliance=cadEquipmentMibCompliance, fpgaErrorEventEntry=fpgaErrorEventEntry, systemGeneral=systemGeneral, cardCarrierFwVersion=cardCarrierFwVersion, auditStatus=auditStatus, TestScheduleCommand=TestScheduleCommand, cmResetClearNotification=cmResetClearNotification, auditId=auditId, diskDriveNumber=diskDriveNumber, portPrState=portPrState, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTrapInh=cardTrapInh, portDetectedMode=portDetectedMode, auditDescription=auditDescription, equipmentDiag=equipmentDiag, cardName=cardName, systemClock=systemClock, cardCarrierHwVersion=cardCarrierHwVersion, cardPicDetected=cardPicDetected, auditPassedCount=auditPassedCount, cardFpgaSource=cardFpgaSource, cardFwUpdateStatus=cardFwUpdateStatus, portCardId=portCardId, equipmentDiagGroup=equipmentDiagGroup, cardDetected=cardDetected, equipmentCardGroup=equipmentCardGroup, cmResetUpTime=cmResetUpTime, cardPrStateChange=cardPrStateChange, cadEquipmentMib=cadEquipmentMib, shelfNumber=shelfNumber, cardSecState=cardSecState, diskVolumeEntry=diskVolumeEntry, auditTotalPassedCount=auditTotalPassedCount, cardNumber=cardNumber, portShelfId=portShelfId, cardTestCommand=cardTestCommand, cmResetInfo=cmResetInfo, errEvLogLevel=errEvLogLevel, cardTemperatureHighWarn=cardTemperatureHighWarn, shelfName=shelfName, cardSubDetected=cardSubDetected, errEvRowStatus=errEvRowStatus, cardUpTime=cardUpTime, equipmentAudit=equipmentAudit, cardDetectedChange=cardDetectedChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestIndex=cardTestIndex, errEvLoggingEnabled=errEvLoggingEnabled, cardNumPorts=cardNumPorts, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portTable=portTable, cardLastResetReason=cardLastResetReason, portType=portType, equipmentShelfGroup=equipmentShelfGroup, cardSwVersion=cardSwVersion, portMaxDsPower=portMaxDsPower, portCurrDsPower=portCurrDsPower, cardTempOverHeatNotification=cardTempOverHeatNotification, cardDplxStatus=cardDplxStatus, trapCounter=trapCounter, shelfObjects=shelfObjects, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestDescription=cardTestDescription, diskVolumeDiskSize=diskVolumeDiskSize, cardCpuSpeed=cardCpuSpeed)
