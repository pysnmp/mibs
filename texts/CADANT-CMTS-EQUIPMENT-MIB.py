...
FirmwareSource, PortMode, ShelfId, DuplexStatus, PortType, CardSubType, EqActionType, CardId, DiskVolumeUsageLevel, PicType, PortId, PortDetectedMode, CardType, PrimaryState, UnknownState, FlowControlMode, SecondaryState, AdminState = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "PortMode", "ShelfId", "DuplexStatus", "PortType", "CardSubType", "EqActionType", "CardId", "DiskVolumeUsageLevel", "PicType", "PortId", "PortDetectedMode", "CardType", "PrimaryState", "UnknownState", "FlowControlMode", "SecondaryState", "AdminState")
...
DisplayString, MacAddress, TextualConvention, DateAndTime, RowStatus, TruthValue, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "TextualConvention", "DateAndTime", "RowStatus", "TruthValue", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeDriveId=diskVolumeDriveId, TestTransactionId=TestTransactionId, portCardId=portCardId, cardFwUpdateStatus=cardFwUpdateStatus, auditStatus=auditStatus, cardSpareGroupMode=cardSpareGroupMode, portPrStateChange=portPrStateChange, equipmentError=equipmentError, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portPrState=portPrState, auditResult=auditResult, portAdminState=portAdminState, cardLastChangeTime=cardLastChangeTime, cardPrState=cardPrState, TestScheduleCommand=TestScheduleCommand, cardDplxStatus=cardDplxStatus, equipmentAudit=equipmentAudit, portDplxStatus=portDplxStatus, systemGeneralGroup=systemGeneralGroup, cardPicSerialNum=cardPicSerialNum, cmResetStatus=cmResetStatus, cardTestResultDesc=cardTestResultDesc, portEntry=portEntry, auditLogOutput=auditLogOutput, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmPrimarySID=cmPrimarySID, portSecStateChange=portSecStateChange, cardTemperature=cardTemperature, errEventId=errEventId, errEvRowStatus=errEvRowStatus, cardDplxStatusChange=cardDplxStatusChange, TestCommand=TestCommand, portNumber=portNumber, diskDriveNumber=diskDriveNumber, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditId=auditId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestIndex=cardTestIndex, cardSecState=cardSecState, equipmentStateGroup=equipmentStateGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardCarrierHwVersion=cardCarrierHwVersion, shelfName=shelfName, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portTxFlowControlMode=portTxFlowControlMode, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeDiskSize=diskVolumeDiskSize, cardName=cardName, cmUChannelID=cmUChannelID, shelfObjects=shelfObjects, diskVolumeName=diskVolumeName, cardBootVersion=cardBootVersion, TestSchedule=TestSchedule, portShelfId=portShelfId, diskVolumeTrapInh=diskVolumeTrapInh, equipmentShelfGroup=equipmentShelfGroup, cardShelfId=cardShelfId, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentTraps=equipmentTraps, cardCpuSpeed=cardCpuSpeed, cardTestTable=cardTestTable, diskVolumeCardId=diskVolumeCardId, portLastChangeTime=portLastChangeTime, cardTrapInh=cardTrapInh, cardTable=cardTable, cmResetReason=cmResetReason, cardTestName=cardTestName, diskVolumeShelfId=diskVolumeShelfId, cmDevice=cmDevice, systemClock=systemClock, auditTime=auditTime, portTable=portTable, cardTestResult=cardTestResult, portCurrDsPower=portCurrDsPower, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentAuditGroup=equipmentAuditGroup, diskVolumeSize=diskVolumeSize, cardTestDescription=cardTestDescription, cardLastBootVersion=cardLastBootVersion, portMacAddress=portMacAddress, cardTemperatureHighError=cardTemperatureHighError, diagTestId=diagTestId, remainInDiagMode=remainInDiagMode, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditTable=auditTable, downstreamPowerLoss=downstreamPowerLoss, portNumChans=portNumChans, auditEntry=auditEntry, cardPrStateChange=cardPrStateChange, portBgpId=portBgpId, trapCounter=trapCounter, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCPLDVersion=cardCPLDVersion, systemKey=systemKey, cardBusSpeed=cardBusSpeed, portCardSubType=portCardSubType, portRxFlowControlMode=portRxFlowControlMode, fpgaErrorEventGroup=fpgaErrorEventGroup, portMode=portMode, fpgaErrorEventTable=fpgaErrorEventTable, cardPicDetected=cardPicDetected, cardSubDetected=cardSubDetected, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, equipmentCardTestGroup=equipmentCardTestGroup, cardType=cardType, eqDiagConfig=eqDiagConfig, systemGeneral=systemGeneral, portSecState=portSecState, auditDescription=auditDescription, cardTestSchedule=cardTestSchedule, cmResetNotification=cmResetNotification, cardId=cardId, cardNumber=cardNumber, cmIpAddress=cmIpAddress, portDplxStatusChange=portDplxStatusChange, portDescription=portDescription, cardUpTime=cardUpTime, auditTotalFailedCount=auditTotalFailedCount, diskVolumeFileName=diskVolumeFileName, cardTestEntry=cardTestEntry, equipmentCardGroup=equipmentCardGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentState=equipmentState, cmVendor=cmVendor, auditCycleCount=auditCycleCount, cmResetInfo=cmResetInfo, equipmentDiagGroup=equipmentDiagGroup, auditFailedCount=auditFailedCount, cardLastResetReason=cardLastResetReason, cardSerialNum=cardSerialNum, cardCarrierSerialNum=cardCarrierSerialNum, cardAdminState=cardAdminState, portRxFlowControlDetected=portRxFlowControlDetected, consoleOutput=consoleOutput, portConnectorId=portConnectorId, verboseLevel=verboseLevel, cardSpareGroupId=cardSpareGroupId, cardLastBootSource=cardLastBootSource, shelfSwVersion=shelfSwVersion, auditLogThrottle=auditLogThrottle, cardTestId=cardTestId, auditPassedCount=auditPassedCount, errEvRecoveryEnabled=errEvRecoveryEnabled, cmMacAddress=cmMacAddress, cardTestCommand=cardTestCommand, TestResult=TestResult, cardEntry=cardEntry, equipmentTbl=equipmentTbl, cardAction=cardAction, cardTestTime=cardTestTime, diskVolumeNumber=diskVolumeNumber, portMaxDsPower=portMaxDsPower, cadEquipmentMib=cadEquipmentMib, cmResetClearNotification=cmResetClearNotification, diskVolumeTable=diskVolumeTable, portMacIfIndex=portMacIfIndex, diskVolumeId=diskVolumeId, cardTestType=cardTestType, cardSwVersion=cardSwVersion, errEvLoggingEnabled=errEvLoggingEnabled, cardFwVersion=cardFwVersion, cardSecStateChange=cardSecStateChange, auditCardId=auditCardId, cardPicHwVersion=cardPicHwVersion, portDocsIfIndex=portDocsIfIndex, auditShelfId=auditShelfId, cardTempNoReportNotification=cardTempNoReportNotification, portId=portId, auditTotalPassedCount=auditTotalPassedCount, errEvLogLevel=errEvLogLevel, cardTestResultNotification=cardTestResultNotification, cardFpgaSource=cardFpgaSource, cardTestScheduleCommand=cardTestScheduleCommand, cardNumPorts=cardNumPorts, cardShelfTestIndex=cardShelfTestIndex, cmRegistrationNotification=cmRegistrationNotification, cardHwVersion=cardHwVersion, auditName=auditName, TestId=TestId, cardTestTransId=cardTestTransId, equipmentPortGroup=equipmentPortGroup, portMinDsPower=portMinDsPower, cardCpuType=cardCpuType, portAction=portAction, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, PYSNMP_MODULE_ID=cadEquipmentMib, cadEquipmentMibConformance=cadEquipmentMibConformance, auditAutoScheduling=auditAutoScheduling, TestType=TestType, portTrapInh=portTrapInh, cadEquipmentMibGroup=cadEquipmentMibGroup, auditCommand=auditCommand, diskVolumeEntry=diskVolumeEntry, cardDetected=cardDetected, cardRamSize=cardRamSize, diskVolumeGroup=diskVolumeGroup, portType=portType, equipmentDiag=equipmentDiag, shelfNumber=shelfNumber, trapSeverity=trapSeverity, cardSubType=cardSubType, cardCarrierFwVersion=cardCarrierFwVersion, portDetectedMode=portDetectedMode, cardDetectedChange=cardDetectedChange, cmResetUpTime=cmResetUpTime, cardFlashSize=cardFlashSize)
