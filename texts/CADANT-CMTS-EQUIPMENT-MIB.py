...
CardType, FlowControlMode, PortMode, SecondaryState, CardSubType, PrimaryState, ShelfId, CardId, UnknownState, DuplexStatus, FirmwareSource, PicType, DiskVolumeUsageLevel, PortId, PortDetectedMode, AdminState, PortType, EqActionType = mibBuilder.importSymbols("CADANT-TC", "CardType", "FlowControlMode", "PortMode", "SecondaryState", "CardSubType", "PrimaryState", "ShelfId", "CardId", "UnknownState", "DuplexStatus", "FirmwareSource", "PicType", "DiskVolumeUsageLevel", "PortId", "PortDetectedMode", "AdminState", "PortType", "EqActionType")
...
DisplayString, TruthValue, MacAddress, DateAndTime, TextualConvention, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TruthValue", "MacAddress", "DateAndTime", "TextualConvention", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portEntry=portEntry, auditFailedCount=auditFailedCount, verboseLevel=verboseLevel, cardAction=cardAction, cardLastBootSource=cardLastBootSource, equipmentTbl=equipmentTbl, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardPrStateChange=cardPrStateChange, shelfSwVersion=shelfSwVersion, diskDriveNumber=diskDriveNumber, cardShelfId=cardShelfId, diskVolumeGroup=diskVolumeGroup, cardSwVersion=cardSwVersion, portNumber=portNumber, cardTestName=cardTestName, TestId=TestId, auditShelfId=auditShelfId, systemGeneral=systemGeneral, diskVolumeFileName=diskVolumeFileName, portTxFlowControlDetected=portTxFlowControlDetected, TestResult=TestResult, eqDiagConfig=eqDiagConfig, errEvRecoveryEnabled=errEvRecoveryEnabled, systemClock=systemClock, fpgaErrorEventEntry=fpgaErrorEventEntry, cmResetUpTime=cmResetUpTime, auditLogThrottle=auditLogThrottle, auditName=auditName, equipmentDiag=equipmentDiag, equipmentState=equipmentState, portRxFlowControlMode=portRxFlowControlMode, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCpuSpeed=cardCpuSpeed, portTxFlowControlMode=portTxFlowControlMode, diskVolumeEntry=diskVolumeEntry, equipmentCardTestGroup=equipmentCardTestGroup, cardAdminState=cardAdminState, TestCommand=TestCommand, cardDetected=cardDetected, auditPassedCount=auditPassedCount, cardPicSerialNum=cardPicSerialNum, cardBusSpeed=cardBusSpeed, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestResult=cardTestResult, equipmentAudit=equipmentAudit, portPrState=portPrState, auditTotalPassedCount=auditTotalPassedCount, portLastChangeTime=portLastChangeTime, cardName=cardName, diskVolumeDriveId=diskVolumeDriveId, shelfNumber=shelfNumber, cardTestIndex=cardTestIndex, portMode=portMode, portDplxStatus=portDplxStatus, cardCPLDVersion=cardCPLDVersion, cardNumber=cardNumber, cardSecState=cardSecState, diskVolumeId=diskVolumeId, cardTestEntry=cardTestEntry, shelfName=shelfName, cmVendor=cmVendor, cadEquipmentMibCompliances=cadEquipmentMibCompliances, errEventId=errEventId, auditTotalFailedCount=auditTotalFailedCount, cardCpuType=cardCpuType, TestTransactionId=TestTransactionId, shelfObjects=shelfObjects, cardFwVersion=cardFwVersion, cardTestDescription=cardTestDescription, auditId=auditId, portTrapInh=portTrapInh, cardPrState=cardPrState, cardCarrierHwVersion=cardCarrierHwVersion, portCardSubType=portCardSubType, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestTable=cardTestTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, errEvRowStatus=errEvRowStatus, auditCardId=auditCardId, downstreamPowerLoss=downstreamPowerLoss, diskVolumeTable=diskVolumeTable, diskVolumeSize=diskVolumeSize, diskVolumeCardId=diskVolumeCardId, equipmentError=equipmentError, auditCommand=auditCommand, fpgaErrorEventGroup=fpgaErrorEventGroup, cardId=cardId, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, portBgpId=portBgpId, portMinDsPower=portMinDsPower, auditStatus=auditStatus, errEvLogLevel=errEvLogLevel, errEvLoggingEnabled=errEvLoggingEnabled, portPrStateChange=portPrStateChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentTraps=equipmentTraps, trapSeverity=trapSeverity, portType=portType, systemKey=systemKey, diskVolumeUsageNotification=diskVolumeUsageNotification, trapCounter=trapCounter, cmPrimarySID=cmPrimarySID, cardLastResetReason=cardLastResetReason, portDplxStatusChange=portDplxStatusChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardLastChangeTime=cardLastChangeTime, cardTemperature=cardTemperature, portAction=portAction, cardSecStateChange=cardSecStateChange, cardTrapInh=cardTrapInh, cardFlashSize=cardFlashSize, diskVolumeNumber=diskVolumeNumber, cmResetClearNotification=cmResetClearNotification, auditAutoScheduling=auditAutoScheduling, cardTestId=cardTestId, cardTestTransId=cardTestTransId, PYSNMP_MODULE_ID=cadEquipmentMib, cardHwVersion=cardHwVersion, auditResult=auditResult, cmDevice=cmDevice, cardFpgaSource=cardFpgaSource, cardTempNoReportNotification=cardTempNoReportNotification, TestScheduleCommand=TestScheduleCommand, cardTestScheduleCommand=cardTestScheduleCommand, cardTable=cardTable, cardFwUpdateStatus=cardFwUpdateStatus, cardLastBootVersion=cardLastBootVersion, cmResetNotification=cmResetNotification, diskVolumeDiskSize=diskVolumeDiskSize, auditDescription=auditDescription, cardSpareGroupMode=cardSpareGroupMode, cmResetReason=cmResetReason, diskVolumeTrapInh=diskVolumeTrapInh, cardDplxStatus=cardDplxStatus, diskVolumeName=diskVolumeName, cardSerialNum=cardSerialNum, portMaxDsPower=portMaxDsPower, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentCardGroup=equipmentCardGroup, cardPicDetected=cardPicDetected, portDocsIfIndex=portDocsIfIndex, cardPicHwVersion=cardPicHwVersion, cardNumPorts=cardNumPorts, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentShelfGroup=equipmentShelfGroup, cardDetectedChange=cardDetectedChange, portConnectorId=portConnectorId, diskVolumeUsageLevel=diskVolumeUsageLevel, cmIpAddress=cmIpAddress, cardRamSize=cardRamSize, fpgaErrorEventTable=fpgaErrorEventTable, portDetectedMode=portDetectedMode, cardSubType=cardSubType, portCardId=portCardId, portAdminState=portAdminState, equipmentStateGroup=equipmentStateGroup, cardCarrierSerialNum=cardCarrierSerialNum, cardEntry=cardEntry, cadEquipmentMibGroup=cadEquipmentMibGroup, auditEntry=auditEntry, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDescription=portDescription, remainInDiagMode=remainInDiagMode, portTable=portTable, cardBootVersion=cardBootVersion, portId=portId, portShelfId=portShelfId, systemGeneralGroup=systemGeneralGroup, diskVolumeShelfId=diskVolumeShelfId, cmUChannelID=cmUChannelID, cardTestTime=cardTestTime, cardCarrierFwVersion=cardCarrierFwVersion, cardSpareGroupId=cardSpareGroupId, equipmentPortGroup=equipmentPortGroup, portSecState=portSecState, cardTestType=cardTestType, auditTable=auditTable, portMacIfIndex=portMacIfIndex, auditTime=auditTime, cardSubDetected=cardSubDetected, cardTestResultDesc=cardTestResultDesc, portNumChans=portNumChans, cmResetInfo=cmResetInfo, diagTestId=diagTestId, cardShelfTestIndex=cardShelfTestIndex, cardTestSchedule=cardTestSchedule, cardTestResultNotification=cardTestResultNotification, portMacAddress=portMacAddress, TestSchedule=TestSchedule, equipmentDiagGroup=equipmentDiagGroup, cardType=cardType, cardTemperatureHighError=cardTemperatureHighError, TestType=TestType, portSecStateChange=portSecStateChange, cardUpTime=cardUpTime, consoleOutput=consoleOutput, auditCycleCount=auditCycleCount, cardTestCommand=cardTestCommand, cmMacAddress=cmMacAddress, portCurrDsPower=portCurrDsPower, equipmentAuditGroup=equipmentAuditGroup, cadEquipmentMib=cadEquipmentMib, cardDplxStatusChange=cardDplxStatusChange, auditLogOutput=auditLogOutput, cmResetStatus=cmResetStatus)
