...
PicType, DuplexStatus, SecondaryState, UnknownState, FlowControlMode, CardSubType, PortType, ShelfId, PortId, AdminState, CardType, PortDetectedMode, CardId, FirmwareSource, PortMode, PrimaryState, DiskVolumeUsageLevel, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PicType", "DuplexStatus", "SecondaryState", "UnknownState", "FlowControlMode", "CardSubType", "PortType", "ShelfId", "PortId", "AdminState", "CardType", "PortDetectedMode", "CardId", "FirmwareSource", "PortMode", "PrimaryState", "DiskVolumeUsageLevel", "EqActionType")
...
RowStatus, DateAndTime, MacAddress, TruthValue, DisplayString, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "MacAddress", "TruthValue", "DisplayString", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardDetectedChange=cardDetectedChange, equipmentState=equipmentState, equipmentTraps=equipmentTraps, diskVolumeTrapInh=diskVolumeTrapInh, auditLogOutput=auditLogOutput, portDocsIfIndex=portDocsIfIndex, cardHwVersion=cardHwVersion, auditEntry=auditEntry, portDescription=portDescription, cmResetReason=cmResetReason, portType=portType, errEvLogLevel=errEvLogLevel, cardLastResetReason=cardLastResetReason, diskVolumeFileName=diskVolumeFileName, portMaxDsPower=portMaxDsPower, portRxFlowControlMode=portRxFlowControlMode, cmResetInfo=cmResetInfo, shelfNumber=shelfNumber, cardSubDetected=cardSubDetected, auditCardId=auditCardId, cardShelfId=cardShelfId, equipmentCardGroup=equipmentCardGroup, consoleOutput=consoleOutput, cmUChannelID=cmUChannelID, diskVolumeEntry=diskVolumeEntry, diskVolumeGroup=diskVolumeGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, portSecState=portSecState, cardSecStateChange=cardSecStateChange, auditName=auditName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditStatus=auditStatus, cardType=cardType, portPrState=portPrState, cardTestDescription=cardTestDescription, equipmentTbl=equipmentTbl, equipmentAuditGroup=equipmentAuditGroup, portNumChans=portNumChans, portLastChangeTime=portLastChangeTime, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicSerialNum=cardPicSerialNum, cmPrimarySID=cmPrimarySID, errEvRowStatus=errEvRowStatus, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSpareGroupId=cardSpareGroupId, equipmentPortGroup=equipmentPortGroup, cardFlashSize=cardFlashSize, equipmentAudit=equipmentAudit, cardDetected=cardDetected, cardTemperatureHighError=cardTemperatureHighError, portAction=portAction, cardShelfTestIndex=cardShelfTestIndex, trapSeverity=trapSeverity, TestResult=TestResult, auditTotalFailedCount=auditTotalFailedCount, cmResetStatus=cmResetStatus, portMinDsPower=portMinDsPower, shelfObjects=shelfObjects, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diagTestId=diagTestId, cardTestResultDesc=cardTestResultDesc, systemKey=systemKey, cardCarrierFwVersion=cardCarrierFwVersion, portTxFlowControlDetected=portTxFlowControlDetected, portNumber=portNumber, portShelfId=portShelfId, portTrapInh=portTrapInh, cardFwVersion=cardFwVersion, cardSerialNum=cardSerialNum, cardNumPorts=cardNumPorts, cmResetNotification=cmResetNotification, cardCpuType=cardCpuType, diskVolumeId=diskVolumeId, cardFpgaSource=cardFpgaSource, cardPicDetected=cardPicDetected, diskVolumeShelfId=diskVolumeShelfId, cardTestName=cardTestName, systemGeneralGroup=systemGeneralGroup, diskVolumeDriveId=diskVolumeDriveId, cardUpTime=cardUpTime, cardSwVersion=cardSwVersion, portConnectorId=portConnectorId, diskVolumeSize=diskVolumeSize, cardPicHwVersion=cardPicHwVersion, cmIpAddress=cmIpAddress, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeName=diskVolumeName, equipmentCardTestGroup=equipmentCardTestGroup, portCardSubType=portCardSubType, fpgaErrorEventTable=fpgaErrorEventTable, cardNumber=cardNumber, cardTable=cardTable, auditFailedCount=auditFailedCount, portId=portId, cmMacAddress=cmMacAddress, cardLastChangeTime=cardLastChangeTime, equipmentError=equipmentError, diskVolumeDiskSize=diskVolumeDiskSize, portDetectedMode=portDetectedMode, cmDevice=cmDevice, auditResult=auditResult, equipmentDiag=equipmentDiag, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentStateGroup=equipmentStateGroup, TestCommand=TestCommand, errEventId=errEventId, cardTestTime=cardTestTime, cmResetUpTime=cmResetUpTime, portCurrDsPower=portCurrDsPower, portSecStateChange=portSecStateChange, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCpuSpeed=cardCpuSpeed, TestScheduleCommand=TestScheduleCommand, cardBusSpeed=cardBusSpeed, portMacAddress=portMacAddress, cadEquipmentMibGroup=cadEquipmentMibGroup, cmRegistrationNotification=cmRegistrationNotification, portPrStateChange=portPrStateChange, cardTestEntry=cardTestEntry, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portMacIfIndex=portMacIfIndex, cardAction=cardAction, cardRamSize=cardRamSize, equipmentShelfGroup=equipmentShelfGroup, cardTestIndex=cardTestIndex, TestId=TestId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTemperature=cardTemperature, portTable=portTable, auditCycleCount=auditCycleCount, cardTrapInh=cardTrapInh, cardTempNoReportNotification=cardTempNoReportNotification, TestTransactionId=TestTransactionId, portTxFlowControlMode=portTxFlowControlMode, auditPassedCount=auditPassedCount, cardDplxStatus=cardDplxStatus, systemGeneral=systemGeneral, cardId=cardId, cmVendor=cmVendor, portDplxStatusChange=portDplxStatusChange, trapCounter=trapCounter, remainInDiagMode=remainInDiagMode, cardCPLDVersion=cardCPLDVersion, eqDiagConfig=eqDiagConfig, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsageLevel=diskVolumeUsageLevel, PYSNMP_MODULE_ID=cadEquipmentMib, cardSecState=cardSecState, auditDescription=auditDescription, diskVolumeNumber=diskVolumeNumber, cardTemperatureHighWarn=cardTemperatureHighWarn, portAdminState=portAdminState, shelfSwVersion=shelfSwVersion, cadEquipmentMib=cadEquipmentMib, diskDriveNumber=diskDriveNumber, cardTestTransId=cardTestTransId, cardTestResult=cardTestResult, diskVolumeCardId=diskVolumeCardId, downstreamPowerLoss=downstreamPowerLoss, TestType=TestType, errEvLoggingEnabled=errEvLoggingEnabled, auditCommand=auditCommand, portBgpId=portBgpId, cardTestCommand=cardTestCommand, systemClock=systemClock, auditTime=auditTime, portDplxStatus=portDplxStatus, auditTable=auditTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portRxFlowControlDetected=portRxFlowControlDetected, cardPrState=cardPrState, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestScheduleCommand=cardTestScheduleCommand, cmResetClearNotification=cmResetClearNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cardAdminState=cardAdminState, cardLastBootSource=cardLastBootSource, cardSubType=cardSubType, cardTestType=cardTestType, portEntry=portEntry, cardTestId=cardTestId, cardTestSchedule=cardTestSchedule, auditId=auditId, cardPrStateChange=cardPrStateChange, TestSchedule=TestSchedule, portCardId=portCardId, cardBootVersion=cardBootVersion, auditLogThrottle=auditLogThrottle, cardTempOverHeatNotification=cardTempOverHeatNotification, shelfName=shelfName, cardDplxStatusChange=cardDplxStatusChange, verboseLevel=verboseLevel, cardSpareGroupMode=cardSpareGroupMode, cardTestTable=cardTestTable, auditAutoScheduling=auditAutoScheduling, cardLastBootVersion=cardLastBootVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, auditShelfId=auditShelfId, diskVolumeTable=diskVolumeTable, cardName=cardName, cardTestResultNotification=cardTestResultNotification, equipmentDiagGroup=equipmentDiagGroup, cardEntry=cardEntry, portMode=portMode, cardFwUpdateStatus=cardFwUpdateStatus)
