...
EqActionType, DuplexStatus, CardSubType, UnknownState, DiskVolumeUsageLevel, PortType, PortMode, AdminState, PrimaryState, PortDetectedMode, FlowControlMode, SecondaryState, CardType, PortId, FirmwareSource, CardId, PicType, ShelfId = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "DuplexStatus", "CardSubType", "UnknownState", "DiskVolumeUsageLevel", "PortType", "PortMode", "AdminState", "PrimaryState", "PortDetectedMode", "FlowControlMode", "SecondaryState", "CardType", "PortId", "FirmwareSource", "CardId", "PicType", "ShelfId")
...
MacAddress, DisplayString, DateAndTime, TruthValue, TextualConvention, TimeStamp, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "DisplayString", "DateAndTime", "TruthValue", "TextualConvention", "TimeStamp", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBusSpeed=cardBusSpeed, equipmentState=equipmentState, equipmentTraps=equipmentTraps, auditTable=auditTable, cardTestType=cardTestType, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, auditCardId=auditCardId, portDplxStatusChange=portDplxStatusChange, TestSchedule=TestSchedule, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeNumber=diskVolumeNumber, cardTrapInh=cardTrapInh, diskVolumeName=diskVolumeName, cardSerialNum=cardSerialNum, auditShelfId=auditShelfId, diskVolumeTable=diskVolumeTable, equipmentShelfGroup=equipmentShelfGroup, cardEntry=cardEntry, cardFpgaSource=cardFpgaSource, equipmentStateGroup=equipmentStateGroup, portDetectedMode=portDetectedMode, cardType=cardType, portConnectorId=portConnectorId, cardName=cardName, portTrapInh=portTrapInh, errEventId=errEventId, cardPrState=cardPrState, cardCPLDVersion=cardCPLDVersion, cardTestId=cardTestId, auditPassedCount=auditPassedCount, cmResetReason=cmResetReason, auditDescription=auditDescription, portDplxStatus=portDplxStatus, cardRamSize=cardRamSize, auditId=auditId, cardDetectedChange=cardDetectedChange, cardDplxStatusChange=cardDplxStatusChange, cardTestName=cardTestName, auditCommand=auditCommand, verboseLevel=verboseLevel, cardTemperature=cardTemperature, portMacIfIndex=portMacIfIndex, downstreamPowerLoss=downstreamPowerLoss, cardId=cardId, diskVolumeTrapInh=diskVolumeTrapInh, cardFwUpdateStatus=cardFwUpdateStatus, cardNumber=cardNumber, shelfName=shelfName, cmMacAddress=cmMacAddress, trapCounter=trapCounter, equipmentCardGroup=equipmentCardGroup, portPrState=portPrState, cardShelfTestIndex=cardShelfTestIndex, cardTestCommand=cardTestCommand, diskVolumeGroup=diskVolumeGroup, cardAction=cardAction, portRxFlowControlMode=portRxFlowControlMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmResetStatus=cmResetStatus, cardPicSerialNum=cardPicSerialNum, equipmentAuditGroup=equipmentAuditGroup, equipmentError=equipmentError, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetNotification=cmResetNotification, remainInDiagMode=remainInDiagMode, portTxFlowControlDetected=portTxFlowControlDetected, auditCycleCount=auditCycleCount, auditLogThrottle=auditLogThrottle, cardNumPorts=cardNumPorts, diskVolumeEntry=diskVolumeEntry, diskVolumeShelfId=diskVolumeShelfId, systemGeneralGroup=systemGeneralGroup, trapSeverity=trapSeverity, TestScheduleCommand=TestScheduleCommand, cardBootVersion=cardBootVersion, cardTestTable=cardTestTable, cardSpareGroupMode=cardSpareGroupMode, portMode=portMode, diskVolumeCardId=diskVolumeCardId, portDocsIfIndex=portDocsIfIndex, cardLastChangeTime=cardLastChangeTime, errEvLogLevel=errEvLogLevel, cardHwVersion=cardHwVersion, cardShelfId=cardShelfId, portId=portId, portMacAddress=portMacAddress, equipmentDiagGroup=equipmentDiagGroup, auditName=auditName, equipmentPortGroup=equipmentPortGroup, portCardSubType=portCardSubType, cmResetUpTime=cmResetUpTime, cardDplxStatus=cardDplxStatus, cmResetInfo=cmResetInfo, TestTransactionId=TestTransactionId, TestCommand=TestCommand, auditResult=auditResult, PYSNMP_MODULE_ID=cadEquipmentMib, shelfNumber=shelfNumber, diagTestId=diagTestId, auditStatus=auditStatus, equipmentAudit=equipmentAudit, cardPicDetected=cardPicDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, portSecState=portSecState, diskVolumeDriveId=diskVolumeDriveId, diskVolumeId=diskVolumeId, auditTotalFailedCount=auditTotalFailedCount, auditTotalPassedCount=auditTotalPassedCount, cmVendor=cmVendor, systemClock=systemClock, portType=portType, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResultNotification=cardTestResultNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCarrierFwVersion=cardCarrierFwVersion, auditAutoScheduling=auditAutoScheduling, cmRegistrationNotification=cmRegistrationNotification, cmUChannelID=cmUChannelID, cardTemperatureHighWarn=cardTemperatureHighWarn, equipmentTbl=equipmentTbl, portMinDsPower=portMinDsPower, fpgaErrorEventTable=fpgaErrorEventTable, cmResetClearNotification=cmResetClearNotification, cardSwVersion=cardSwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, eqDiagConfig=eqDiagConfig, TestResult=TestResult, cardTestDescription=cardTestDescription, cardSubDetected=cardSubDetected, portTable=portTable, cardSubType=cardSubType, portAdminState=portAdminState, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMib=cadEquipmentMib, cardPicHwVersion=cardPicHwVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, cardLastBootSource=cardLastBootSource, fpgaErrorEventGroup=fpgaErrorEventGroup, cardCpuType=cardCpuType, shelfObjects=shelfObjects, cardFlashSize=cardFlashSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeFileName=diskVolumeFileName, auditEntry=auditEntry, cardPrStateChange=cardPrStateChange, cmIpAddress=cmIpAddress, cardTestTime=cardTestTime, errEvLoggingEnabled=errEvLoggingEnabled, cardTable=cardTable, equipmentDiag=equipmentDiag, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResultDesc=cardTestResultDesc, cardTestResult=cardTestResult, equipmentCardTestGroup=equipmentCardTestGroup, systemKey=systemKey, cardAdminState=cardAdminState, cardCpuSpeed=cardCpuSpeed, portCardId=portCardId, portCurrDsPower=portCurrDsPower, diskVolumeDiskSize=diskVolumeDiskSize, cardUpTime=cardUpTime, cardFwVersion=cardFwVersion, cardTestEntry=cardTestEntry, portNumber=portNumber, portNumChans=portNumChans, portSecStateChange=portSecStateChange, auditLogOutput=auditLogOutput, portTxFlowControlMode=portTxFlowControlMode, cardSecState=cardSecState, consoleOutput=consoleOutput, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeSize=diskVolumeSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestTransId=cardTestTransId, diskDriveNumber=diskDriveNumber, auditFailedCount=auditFailedCount, TestType=TestType, portMaxDsPower=portMaxDsPower, cardTestScheduleCommand=cardTestScheduleCommand, TestId=TestId, auditTime=auditTime, cardLastBootVersion=cardLastBootVersion, cmPrimarySID=cmPrimarySID, portAction=portAction, portPrStateChange=portPrStateChange, cardCarrierHwVersion=cardCarrierHwVersion, cardTestSchedule=cardTestSchedule, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCarrierSerialNum=cardCarrierSerialNum, cardSecStateChange=cardSecStateChange, portEntry=portEntry, systemGeneral=systemGeneral, portLastChangeTime=portLastChangeTime, cardTestIndex=cardTestIndex, diskVolumeUsageLevel=diskVolumeUsageLevel, shelfSwVersion=shelfSwVersion, portShelfId=portShelfId, portDescription=portDescription, cmDevice=cmDevice, cardLastResetReason=cardLastResetReason, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, errEvRowStatus=errEvRowStatus, cardDetected=cardDetected, portBgpId=portBgpId)
