...
PrimaryState, ShelfId, AdminState, SecondaryState, EqActionType, PortType, UnknownState, PortDetectedMode, FlowControlMode, PortId, DiskVolumeUsageLevel, PicType, CardId, FirmwareSource, PortMode, CardType, CardSubType, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "ShelfId", "AdminState", "SecondaryState", "EqActionType", "PortType", "UnknownState", "PortDetectedMode", "FlowControlMode", "PortId", "DiskVolumeUsageLevel", "PicType", "CardId", "FirmwareSource", "PortMode", "CardType", "CardSubType", "DuplexStatus")
...
DateAndTime, MacAddress, RowStatus, TextualConvention, TimeStamp, DisplayString, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "MacAddress", "RowStatus", "TextualConvention", "TimeStamp", "DisplayString", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardType=cardType, cardFwUpdateStatus=cardFwUpdateStatus, equipmentTraps=equipmentTraps, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, shelfName=shelfName, portRxFlowControlDetected=portRxFlowControlDetected, cardSecStateChange=cardSecStateChange, auditTable=auditTable, diskVolumeTable=diskVolumeTable, downstreamPowerLoss=downstreamPowerLoss, auditCycleCount=auditCycleCount, shelfObjects=shelfObjects, cmResetStatus=cmResetStatus, TestType=TestType, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, systemGeneral=systemGeneral, portDplxStatusChange=portDplxStatusChange, diskVolumeDriveId=diskVolumeDriveId, errEvLoggingEnabled=errEvLoggingEnabled, remainInDiagMode=remainInDiagMode, equipmentCardTestGroup=equipmentCardTestGroup, portLastChangeTime=portLastChangeTime, portDescription=portDescription, cardPrState=cardPrState, cardCpuType=cardCpuType, cmResetNotification=cmResetNotification, cardTestId=cardTestId, auditTotalPassedCount=auditTotalPassedCount, TestId=TestId, cardNumPorts=cardNumPorts, cardTestTable=cardTestTable, trapSeverity=trapSeverity, cmPrimarySID=cmPrimarySID, diskVolumeTrapInh=diskVolumeTrapInh, cardSubType=cardSubType, equipmentPortGroup=equipmentPortGroup, cardShelfTestIndex=cardShelfTestIndex, equipmentAudit=equipmentAudit, auditName=auditName, cardTestCommand=cardTestCommand, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeUsageNotification=diskVolumeUsageNotification, cardFwVersion=cardFwVersion, portPrStateChange=portPrStateChange, cmResetClearNotification=cmResetClearNotification, portCardSubType=portCardSubType, TestScheduleCommand=TestScheduleCommand, auditId=auditId, diskVolumeCardId=diskVolumeCardId, cardHwVersion=cardHwVersion, portBgpId=portBgpId, cardNumber=cardNumber, shelfSwVersion=shelfSwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestDescription=cardTestDescription, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeFileName=diskVolumeFileName, auditStatus=auditStatus, auditDescription=auditDescription, cardDplxStatusChange=cardDplxStatusChange, cardLastBootVersion=cardLastBootVersion, verboseLevel=verboseLevel, cardLastBootSource=cardLastBootSource, portAction=portAction, auditEntry=auditEntry, cardDplxStatus=cardDplxStatus, cardFpgaSource=cardFpgaSource, diskVolumeName=diskVolumeName, auditTotalFailedCount=auditTotalFailedCount, auditFailedCount=auditFailedCount, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogThrottle=auditLogThrottle, cardDetected=cardDetected, equipmentShelfGroup=equipmentShelfGroup, cardAdminState=cardAdminState, cardShelfId=cardShelfId, trapCounter=trapCounter, cardName=cardName, diskVolumeId=diskVolumeId, portMacAddress=portMacAddress, cardFlashSize=cardFlashSize, cardRamSize=cardRamSize, cardTestType=cardTestType, TestResult=TestResult, cardDetectedChange=cardDetectedChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTable=portTable, cardSerialNum=cardSerialNum, cardSubDetected=cardSubDetected, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibGroup=cadEquipmentMibGroup, portDetectedMode=portDetectedMode, portNumChans=portNumChans, diskVolumeShelfId=diskVolumeShelfId, errEvRecoveryEnabled=errEvRecoveryEnabled, portSecState=portSecState, auditTime=auditTime, cmResetReason=cmResetReason, cardLastResetReason=cardLastResetReason, cardTestName=cardTestName, cardAction=cardAction, cardTestIndex=cardTestIndex, cmResetUpTime=cmResetUpTime, systemClock=systemClock, cardTestEntry=cardTestEntry, cardTestTransId=cardTestTransId, portDplxStatus=portDplxStatus, cmResetInfo=cmResetInfo, cardBusSpeed=cardBusSpeed, portMaxDsPower=portMaxDsPower, auditCommand=auditCommand, cardEntry=cardEntry, portDocsIfIndex=portDocsIfIndex, systemGeneralGroup=systemGeneralGroup, eqDiagConfig=eqDiagConfig, TestCommand=TestCommand, errEventId=errEventId, portEntry=portEntry, cmUChannelID=cmUChannelID, cardCPLDVersion=cardCPLDVersion, portPrState=portPrState, cmMacAddress=cmMacAddress, cardUpTime=cardUpTime, portNumber=portNumber, portTxFlowControlMode=portTxFlowControlMode, cardTable=cardTable, cardTestResultDesc=cardTestResultDesc, auditResult=auditResult, systemKey=systemKey, portConnectorId=portConnectorId, cardPicDetected=cardPicDetected, cmIpAddress=cmIpAddress, cardTempNoReportNotification=cardTempNoReportNotification, cardTestScheduleCommand=cardTestScheduleCommand, cardPrStateChange=cardPrStateChange, shelfNumber=shelfNumber, cardLastChangeTime=cardLastChangeTime, cardPicHwVersion=cardPicHwVersion, cardCpuSpeed=cardCpuSpeed, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeEntry=diskVolumeEntry, equipmentState=equipmentState, consoleOutput=consoleOutput, diagTestId=diagTestId, cmVendor=cmVendor, portShelfId=portShelfId, cmDevice=cmDevice, diskDriveNumber=diskDriveNumber, diskVolumeDiskSize=diskVolumeDiskSize, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTrapInh=cardTrapInh, cardCarrierHwVersion=cardCarrierHwVersion, equipmentAuditGroup=equipmentAuditGroup, auditLogOutput=auditLogOutput, portMinDsPower=portMinDsPower, diskVolumeNumber=diskVolumeNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, portMacIfIndex=portMacIfIndex, portAdminState=portAdminState, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCarrierSerialNum=cardCarrierSerialNum, cmRegistrationNotification=cmRegistrationNotification, cardTemperatureHighError=cardTemperatureHighError, diskVolumeSize=diskVolumeSize, cardSecState=cardSecState, cardPicSerialNum=cardPicSerialNum, auditCardId=auditCardId, portType=portType, cardTemperatureHighWarn=cardTemperatureHighWarn, portRxFlowControlMode=portRxFlowControlMode, auditShelfId=auditShelfId, errEvLogLevel=errEvLogLevel, equipmentTbl=equipmentTbl, equipmentStateGroup=equipmentStateGroup, equipmentCardGroup=equipmentCardGroup, equipmentDiagGroup=equipmentDiagGroup, cardTemperature=cardTemperature, cadEquipmentMib=cadEquipmentMib, diskVolumeGroup=diskVolumeGroup, errEvRowStatus=errEvRowStatus, portTrapInh=portTrapInh, portId=portId, equipmentDiag=equipmentDiag, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, TestTransactionId=TestTransactionId, cardId=cardId, cardTestSchedule=cardTestSchedule, cardBootVersion=cardBootVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSpareGroupMode=cardSpareGroupMode, auditPassedCount=auditPassedCount, portCurrDsPower=portCurrDsPower, cardCarrierFwVersion=cardCarrierFwVersion, cardTestTime=cardTestTime, portSecStateChange=portSecStateChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, auditAutoScheduling=auditAutoScheduling, TestSchedule=TestSchedule, cardTestResultNotification=cardTestResultNotification, cardSwVersion=cardSwVersion, portMode=portMode, cardTestResult=cardTestResult, portCardId=portCardId, equipmentError=equipmentError, fpgaErrorEventTable=fpgaErrorEventTable, cardSpareGroupId=cardSpareGroupId)
