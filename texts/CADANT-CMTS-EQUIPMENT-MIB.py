...
PortMode, DiskVolumeUsageLevel, CardSubType, PrimaryState, FlowControlMode, PortDetectedMode, SecondaryState, UnknownState, EqActionType, CardType, PortType, FirmwareSource, ShelfId, AdminState, CardId, PicType, DuplexStatus, PortId = mibBuilder.importSymbols("CADANT-TC", "PortMode", "DiskVolumeUsageLevel", "CardSubType", "PrimaryState", "FlowControlMode", "PortDetectedMode", "SecondaryState", "UnknownState", "EqActionType", "CardType", "PortType", "FirmwareSource", "ShelfId", "AdminState", "CardId", "PicType", "DuplexStatus", "PortId")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", trapSeverity=trapSeverity, equipmentTbl=equipmentTbl, cardTestResult=cardTestResult, cardTemperatureHighWarn=cardTemperatureHighWarn, diskDriveNumber=diskDriveNumber, errEvRowStatus=errEvRowStatus, cardSubDetected=cardSubDetected, portPrStateChange=portPrStateChange, portMacIfIndex=portMacIfIndex, eqDiagConfig=eqDiagConfig, portRxFlowControlMode=portRxFlowControlMode, cardSubType=cardSubType, cmMacAddress=cmMacAddress, equipmentCardGroup=equipmentCardGroup, equipmentState=equipmentState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, shelfNumber=shelfNumber, cardEntry=cardEntry, cardDplxStatus=cardDplxStatus, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, systemClock=systemClock, systemGeneralGroup=systemGeneralGroup, cardType=cardType, portMode=portMode, errEvLogLevel=errEvLogLevel, cardDetected=cardDetected, cardShelfTestIndex=cardShelfTestIndex, equipmentDiag=equipmentDiag, auditName=auditName, portLastChangeTime=portLastChangeTime, cmResetClearNotification=cmResetClearNotification, cardAdminState=cardAdminState, trapCounter=trapCounter, portAction=portAction, cardSwVersion=cardSwVersion, shelfSwVersion=shelfSwVersion, auditPassedCount=auditPassedCount, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageNotification=diskVolumeUsageNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentDiagGroup=equipmentDiagGroup, portDocsIfIndex=portDocsIfIndex, cardName=cardName, cardTestTime=cardTestTime, cardNumPorts=cardNumPorts, cardBusSpeed=cardBusSpeed, cardCarrierHwVersion=cardCarrierHwVersion, auditLogOutput=auditLogOutput, portDplxStatusChange=portDplxStatusChange, cmRegistrationNotification=cmRegistrationNotification, diskVolumeSize=diskVolumeSize, fpgaErrorEventGroup=fpgaErrorEventGroup, portDescription=portDescription, cardRamSize=cardRamSize, errEvRecoveryEnabled=errEvRecoveryEnabled, cardShelfId=cardShelfId, cardTestResultNotification=cardTestResultNotification, auditCardId=auditCardId, portPrState=portPrState, portMinDsPower=portMinDsPower, cardCpuSpeed=cardCpuSpeed, cmResetInfo=cmResetInfo, cardPicDetected=cardPicDetected, equipmentPortGroup=equipmentPortGroup, cardTrapInh=cardTrapInh, diskVolumeTable=diskVolumeTable, cardFwUpdateStatus=cardFwUpdateStatus, cardTestTransId=cardTestTransId, cardAction=cardAction, auditId=auditId, cmPrimarySID=cmPrimarySID, portTable=portTable, cardCarrierFwVersion=cardCarrierFwVersion, auditStatus=auditStatus, cardTemperature=cardTemperature, cardTestEntry=cardTestEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestDescription=cardTestDescription, diskVolumeGroup=diskVolumeGroup, TestId=TestId, downstreamPowerLoss=downstreamPowerLoss, cmDevice=cmDevice, cardTestIndex=cardTestIndex, cmResetReason=cmResetReason, diskVolumeCardId=diskVolumeCardId, cmVendor=cmVendor, TestType=TestType, auditTotalFailedCount=auditTotalFailedCount, auditCycleCount=auditCycleCount, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeName=diskVolumeName, cardFpgaSource=cardFpgaSource, cardTestScheduleCommand=cardTestScheduleCommand, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeShelfId=diskVolumeShelfId, auditResult=auditResult, equipmentStateGroup=equipmentStateGroup, cardTestTable=cardTestTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardSecState=cardSecState, portId=portId, equipmentAudit=equipmentAudit, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portType=portType, fpgaErrorEventTable=fpgaErrorEventTable, shelfName=shelfName, cmUChannelID=cmUChannelID, portMaxDsPower=portMaxDsPower, cardTestType=cardTestType, cardLastBootSource=cardLastBootSource, portConnectorId=portConnectorId, cardSecStateChange=cardSecStateChange, cardTable=cardTable, diskVolumeTrapInh=diskVolumeTrapInh, auditEntry=auditEntry, cmIpAddress=cmIpAddress, cardId=cardId, cmResetNotification=cmResetNotification, consoleOutput=consoleOutput, portDplxStatus=portDplxStatus, diskVolumeDiskSize=diskVolumeDiskSize, cardSerialNum=cardSerialNum, portEntry=portEntry, auditCommand=auditCommand, portSecStateChange=portSecStateChange, equipmentShelfGroup=equipmentShelfGroup, portShelfId=portShelfId, TestResult=TestResult, auditTotalPassedCount=auditTotalPassedCount, cardTestSchedule=cardTestSchedule, cardLastBootVersion=cardLastBootVersion, cardLastResetReason=cardLastResetReason, errEventId=errEventId, TestTransactionId=TestTransactionId, cardPicSerialNum=cardPicSerialNum, equipmentTraps=equipmentTraps, fpgaErrorEventEntry=fpgaErrorEventEntry, PYSNMP_MODULE_ID=cadEquipmentMib, auditDescription=auditDescription, portTrapInh=portTrapInh, diskVolumeNumber=diskVolumeNumber, TestSchedule=TestSchedule, cardSpareGroupMode=cardSpareGroupMode, portNumber=portNumber, cardFwVersion=cardFwVersion, portCardSubType=portCardSubType, equipmentAuditGroup=equipmentAuditGroup, auditLogThrottle=auditLogThrottle, auditTable=auditTable, remainInDiagMode=remainInDiagMode, cardTestId=cardTestId, cardTestName=cardTestName, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResultDesc=cardTestResultDesc, portTxFlowControlDetected=portTxFlowControlDetected, diagTestId=diagTestId, cardCPLDVersion=cardCPLDVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMibConformance=cadEquipmentMibConformance, portCurrDsPower=portCurrDsPower, cardBootVersion=cardBootVersion, equipmentCardTestGroup=equipmentCardTestGroup, systemKey=systemKey, portSecState=portSecState, cardPrState=cardPrState, auditTime=auditTime, auditFailedCount=auditFailedCount, systemGeneral=systemGeneral, cardPicHwVersion=cardPicHwVersion, cardHwVersion=cardHwVersion, portNumChans=portNumChans, TestCommand=TestCommand, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardCarrierSerialNum=cardCarrierSerialNum, cardLastChangeTime=cardLastChangeTime, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTemperatureHighError=cardTemperatureHighError, cardFlashSize=cardFlashSize, portAdminState=portAdminState, diskVolumeFileName=diskVolumeFileName, cardDetectedChange=cardDetectedChange, auditAutoScheduling=auditAutoScheduling, cardTempOverHeatNotification=cardTempOverHeatNotification, portCardId=portCardId, cadEquipmentMib=cadEquipmentMib, cardDplxStatusChange=cardDplxStatusChange, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, equipmentError=equipmentError, cardSpareGroupId=cardSpareGroupId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, shelfObjects=shelfObjects, auditShelfId=auditShelfId, diskVolumeId=diskVolumeId, cmResetUpTime=cmResetUpTime, cardPrStateChange=cardPrStateChange, cardCpuType=cardCpuType, portTxFlowControlMode=portTxFlowControlMode, portDetectedMode=portDetectedMode, diskVolumeEntry=diskVolumeEntry, cmResetStatus=cmResetStatus, portMacAddress=portMacAddress, cardTestCommand=cardTestCommand, cardNumber=cardNumber, TestScheduleCommand=TestScheduleCommand, portBgpId=portBgpId, verboseLevel=verboseLevel, cardUpTime=cardUpTime)
