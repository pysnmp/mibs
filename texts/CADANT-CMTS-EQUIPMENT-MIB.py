...
PicType, CardId, PortId, PortDetectedMode, PortType, DiskVolumeUsageLevel, PrimaryState, SecondaryState, AdminState, CardType, ShelfId, PortMode, FlowControlMode, FirmwareSource, EqActionType, UnknownState, CardSubType, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PicType", "CardId", "PortId", "PortDetectedMode", "PortType", "DiskVolumeUsageLevel", "PrimaryState", "SecondaryState", "AdminState", "CardType", "ShelfId", "PortMode", "FlowControlMode", "FirmwareSource", "EqActionType", "UnknownState", "CardSubType", "DuplexStatus")
...
DisplayString, TimeStamp, TextualConvention, MacAddress, RowStatus, TruthValue, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "TextualConvention", "MacAddress", "RowStatus", "TruthValue", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFpgaSource=cardFpgaSource, equipmentDiagGroup=equipmentDiagGroup, cardSerialNum=cardSerialNum, eqDiagConfig=eqDiagConfig, verboseLevel=verboseLevel, downstreamPowerLoss=downstreamPowerLoss, portMacIfIndex=portMacIfIndex, portTable=portTable, cardLastChangeTime=cardLastChangeTime, portNumChans=portNumChans, cardSubType=cardSubType, cardSpareGroupMode=cardSpareGroupMode, cardDetectedChange=cardDetectedChange, cardId=cardId, cardBusSpeed=cardBusSpeed, portMaxDsPower=portMaxDsPower, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTemperature=cardTemperature, portPrState=portPrState, cardTestCommand=cardTestCommand, cmRegistrationNotification=cmRegistrationNotification, cardTable=cardTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portMacAddress=portMacAddress, equipmentAuditGroup=equipmentAuditGroup, cardLastBootVersion=cardLastBootVersion, auditCommand=auditCommand, diskVolumeGroup=diskVolumeGroup, cardTemperatureHighWarn=cardTemperatureHighWarn, TestScheduleCommand=TestScheduleCommand, cardPrStateChange=cardPrStateChange, cardTestIndex=cardTestIndex, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResultNotification=cardTestResultNotification, diskDriveNumber=diskDriveNumber, TestCommand=TestCommand, cardShelfId=cardShelfId, equipmentAudit=equipmentAudit, fpgaErrorEventTable=fpgaErrorEventTable, systemGeneral=systemGeneral, portDocsIfIndex=portDocsIfIndex, auditTime=auditTime, cardTemperatureHighError=cardTemperatureHighError, cardPicSerialNum=cardPicSerialNum, cardSwVersion=cardSwVersion, auditResult=auditResult, portEntry=portEntry, cmResetClearNotification=cmResetClearNotification, auditTotalFailedCount=auditTotalFailedCount, cadEquipmentMibCompliance=cadEquipmentMibCompliance, errEvRowStatus=errEvRowStatus, cardTestDescription=cardTestDescription, cardSecStateChange=cardSecStateChange, cardRamSize=cardRamSize, portTrapInh=portTrapInh, systemGeneralGroup=systemGeneralGroup, auditFailedCount=auditFailedCount, cardCpuType=cardCpuType, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFwVersion=cardFwVersion, auditDescription=auditDescription, remainInDiagMode=remainInDiagMode, consoleOutput=consoleOutput, cardAction=cardAction, cardCarrierHwVersion=cardCarrierHwVersion, auditId=auditId, equipmentCardGroup=equipmentCardGroup, auditCardId=auditCardId, diagTestId=diagTestId, auditName=auditName, errEventId=errEventId, portShelfId=portShelfId, diskVolumeShelfId=diskVolumeShelfId, cmResetStatus=cmResetStatus, cardTestTransId=cardTestTransId, cadEquipmentMib=cadEquipmentMib, cardTestTable=cardTestTable, auditAutoScheduling=auditAutoScheduling, auditTable=auditTable, cardSpareGroupId=cardSpareGroupId, portTxFlowControlMode=portTxFlowControlMode, diskVolumeDiskSize=diskVolumeDiskSize, cardPrState=cardPrState, auditPassedCount=auditPassedCount, portCardSubType=portCardSubType, cardNumPorts=cardNumPorts, portMinDsPower=portMinDsPower, cardTrapInh=cardTrapInh, cardShelfTestIndex=cardShelfTestIndex, auditCycleCount=auditCycleCount, cardCpuSpeed=cardCpuSpeed, portDescription=portDescription, cardTempNoReportNotification=cardTempNoReportNotification, cardAdminState=cardAdminState, cardFwUpdateStatus=cardFwUpdateStatus, cmDevice=cmDevice, errEvRecoveryEnabled=errEvRecoveryEnabled, cmResetUpTime=cmResetUpTime, cmIpAddress=cmIpAddress, cardTestResult=cardTestResult, cmVendor=cmVendor, cardLastBootSource=cardLastBootSource, cmMacAddress=cmMacAddress, diskVolumeTable=diskVolumeTable, equipmentState=equipmentState, diskVolumeDriveId=diskVolumeDriveId, cmPrimarySID=cmPrimarySID, TestId=TestId, auditStatus=auditStatus, cmResetNotification=cmResetNotification, cardUpTime=cardUpTime, cmResetReason=cmResetReason, portAction=portAction, systemClock=systemClock, TestResult=TestResult, shelfNumber=shelfNumber, cardDetected=cardDetected, cardBootVersion=cardBootVersion, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentTraps=equipmentTraps, auditLogOutput=auditLogOutput, portType=portType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardHwVersion=cardHwVersion, shelfName=shelfName, portId=portId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestId=cardTestId, cardTestSchedule=cardTestSchedule, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardName=cardName, cardSecState=cardSecState, errEvLogLevel=errEvLogLevel, cardTestType=cardTestType, portLastChangeTime=portLastChangeTime, PYSNMP_MODULE_ID=cadEquipmentMib, portSecState=portSecState, cardDplxStatusChange=cardDplxStatusChange, cardTestScheduleCommand=cardTestScheduleCommand, cardFlashSize=cardFlashSize, equipmentPortGroup=equipmentPortGroup, trapSeverity=trapSeverity, cardTestTime=cardTestTime, errEvLoggingEnabled=errEvLoggingEnabled, cardNumber=cardNumber, portDetectedMode=portDetectedMode, diskVolumeEntry=diskVolumeEntry, portMode=portMode, equipmentTbl=equipmentTbl, equipmentCardTestGroup=equipmentCardTestGroup, auditLogThrottle=auditLogThrottle, cardTestName=cardTestName, equipmentShelfGroup=equipmentShelfGroup, cmUChannelID=cmUChannelID, diskVolumeId=diskVolumeId, TestTransactionId=TestTransactionId, auditEntry=auditEntry, cmResetInfo=cmResetInfo, portRxFlowControlMode=portRxFlowControlMode, cadEquipmentMibConformance=cadEquipmentMibConformance, portSecStateChange=portSecStateChange, cardTestResultDesc=cardTestResultDesc, cardCarrierFwVersion=cardCarrierFwVersion, cardSubDetected=cardSubDetected, cadEquipmentMibGroup=cadEquipmentMibGroup, systemKey=systemKey, diskVolumeName=diskVolumeName, diskVolumeSize=diskVolumeSize, cardDplxStatus=cardDplxStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portRxFlowControlDetected=portRxFlowControlDetected, portDplxStatusChange=portDplxStatusChange, portPrStateChange=portPrStateChange, shelfObjects=shelfObjects, portCardId=portCardId, auditTotalPassedCount=auditTotalPassedCount, auditShelfId=auditShelfId, portBgpId=portBgpId, equipmentError=equipmentError, equipmentDiag=equipmentDiag, portAdminState=portAdminState, cardLastResetReason=cardLastResetReason, diskVolumeCardId=diskVolumeCardId, cardTestEntry=cardTestEntry, portConnectorId=portConnectorId, diskVolumeNumber=diskVolumeNumber, cardType=cardType, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeUsageLevel=diskVolumeUsageLevel, cardEntry=cardEntry, cardCPLDVersion=cardCPLDVersion, fpgaErrorEventGroup=fpgaErrorEventGroup, shelfSwVersion=shelfSwVersion, portDplxStatus=portDplxStatus, portCurrDsPower=portCurrDsPower, portNumber=portNumber, TestType=TestType, TestSchedule=TestSchedule, diskVolumeTrapInh=diskVolumeTrapInh, cardCarrierSerialNum=cardCarrierSerialNum, trapCounter=trapCounter, cardPicHwVersion=cardPicHwVersion, diskVolumeFileName=diskVolumeFileName, cardPicDetected=cardPicDetected)
