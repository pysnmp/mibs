...
CardId, CardType, PortDetectedMode, ShelfId, FirmwareSource, PortType, DiskVolumeUsageLevel, PrimaryState, SecondaryState, PicType, PortId, FlowControlMode, CardSubType, PortMode, UnknownState, DuplexStatus, AdminState, EqActionType = mibBuilder.importSymbols("CADANT-TC", "CardId", "CardType", "PortDetectedMode", "ShelfId", "FirmwareSource", "PortType", "DiskVolumeUsageLevel", "PrimaryState", "SecondaryState", "PicType", "PortId", "FlowControlMode", "CardSubType", "PortMode", "UnknownState", "DuplexStatus", "AdminState", "EqActionType")
...
MacAddress, DateAndTime, DisplayString, TextualConvention, TimeStamp, TruthValue, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "DateAndTime", "DisplayString", "TextualConvention", "TimeStamp", "TruthValue", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditId=auditId, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageNotification=diskVolumeUsageNotification, portLastChangeTime=portLastChangeTime, portNumChans=portNumChans, portRxFlowControlMode=portRxFlowControlMode, cardPicSerialNum=cardPicSerialNum, systemClock=systemClock, cardNumber=cardNumber, portType=portType, portShelfId=portShelfId, cardAction=cardAction, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardLastChangeTime=cardLastChangeTime, cardTestName=cardTestName, cardTestCommand=cardTestCommand, auditAutoScheduling=auditAutoScheduling, equipmentAudit=equipmentAudit, diskVolumeFileName=diskVolumeFileName, diskVolumeNumber=diskVolumeNumber, auditCommand=auditCommand, diskVolumeId=diskVolumeId, portMode=portMode, cardTestTable=cardTestTable, portCurrDsPower=portCurrDsPower, cardCarrierFwVersion=cardCarrierFwVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestResult=cardTestResult, auditTable=auditTable, cmResetClearNotification=cmResetClearNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecState=portSecState, equipmentDiagGroup=equipmentDiagGroup, portTable=portTable, cardTemperature=cardTemperature, cardId=cardId, auditResult=auditResult, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, TestSchedule=TestSchedule, cmVendor=cmVendor, trapSeverity=trapSeverity, portCardSubType=portCardSubType, equipmentTraps=equipmentTraps, cardSubType=cardSubType, cmIpAddress=cmIpAddress, diskVolumeUsageLevel=diskVolumeUsageLevel, cardCpuSpeed=cardCpuSpeed, cardPicHwVersion=cardPicHwVersion, portDplxStatus=portDplxStatus, cardTable=cardTable, errEventId=errEventId, portConnectorId=portConnectorId, diskVolumeCardId=diskVolumeCardId, auditCardId=auditCardId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardFwVersion=cardFwVersion, portEntry=portEntry, portDescription=portDescription, equipmentError=equipmentError, cardPrState=cardPrState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestTime=cardTestTime, cardNumPorts=cardNumPorts, auditLogThrottle=auditLogThrottle, cardLastResetReason=cardLastResetReason, auditFailedCount=auditFailedCount, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCPLDVersion=cardCPLDVersion, cardBusSpeed=cardBusSpeed, diskVolumeName=diskVolumeName, systemGeneral=systemGeneral, cardTestResultNotification=cardTestResultNotification, diskVolumeSize=diskVolumeSize, auditStatus=auditStatus, auditDescription=auditDescription, cardSpareGroupId=cardSpareGroupId, cardSpareGroupMode=cardSpareGroupMode, cardName=cardName, TestScheduleCommand=TestScheduleCommand, cardLastBootVersion=cardLastBootVersion, shelfName=shelfName, cmResetStatus=cmResetStatus, shelfNumber=shelfNumber, cardTestScheduleCommand=cardTestScheduleCommand, cardCarrierSerialNum=cardCarrierSerialNum, auditLogOutput=auditLogOutput, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardShelfTestIndex=cardShelfTestIndex, cardPrStateChange=cardPrStateChange, cardSecStateChange=cardSecStateChange, cardBootVersion=cardBootVersion, portDetectedMode=portDetectedMode, cardTestId=cardTestId, cardSwVersion=cardSwVersion, portAction=portAction, diskVolumeDiskSize=diskVolumeDiskSize, TestCommand=TestCommand, cardShelfId=cardShelfId, cardTestEntry=cardTestEntry, cardTestDescription=cardTestDescription, errEvLogLevel=errEvLogLevel, diskVolumeGroup=diskVolumeGroup, auditCycleCount=auditCycleCount, portMaxDsPower=portMaxDsPower, portTrapInh=portTrapInh, cardTrapInh=cardTrapInh, auditName=auditName, auditTime=auditTime, TestId=TestId, cardSubDetected=cardSubDetected, cardDplxStatusChange=cardDplxStatusChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestSchedule=cardTestSchedule, verboseLevel=verboseLevel, consoleOutput=consoleOutput, cardType=cardType, auditTotalPassedCount=auditTotalPassedCount, portBgpId=portBgpId, cardTestResultDesc=cardTestResultDesc, cmResetReason=cmResetReason, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestType=cardTestType, cmPrimarySID=cmPrimarySID, portTxFlowControlMode=portTxFlowControlMode, cardTestIndex=cardTestIndex, cardHwVersion=cardHwVersion, portNumber=portNumber, equipmentDiag=equipmentDiag, cmDevice=cmDevice, TestResult=TestResult, cmUChannelID=cmUChannelID, portMinDsPower=portMinDsPower, auditPassedCount=auditPassedCount, equipmentState=equipmentState, diskVolumeShelfId=diskVolumeShelfId, equipmentCardTestGroup=equipmentCardTestGroup, portDplxStatusChange=portDplxStatusChange, cardDetected=cardDetected, equipmentCardGroup=equipmentCardGroup, shelfObjects=shelfObjects, cardSecState=cardSecState, portPrStateChange=portPrStateChange, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portDocsIfIndex=portDocsIfIndex, diskDriveNumber=diskDriveNumber, cadEquipmentMib=cadEquipmentMib, eqDiagConfig=eqDiagConfig, cardUpTime=cardUpTime, portRxFlowControlDetected=portRxFlowControlDetected, auditShelfId=auditShelfId, cmRegistrationNotification=cmRegistrationNotification, cardFwUpdateStatus=cardFwUpdateStatus, errEvRowStatus=errEvRowStatus, cardPicDetected=cardPicDetected, cardFlashSize=cardFlashSize, diskVolumeTable=diskVolumeTable, equipmentTbl=equipmentTbl, cardTempNoReportNotification=cardTempNoReportNotification, systemGeneralGroup=systemGeneralGroup, cardAdminState=cardAdminState, portCardId=portCardId, portTxFlowControlDetected=portTxFlowControlDetected, cadEquipmentMibConformance=cadEquipmentMibConformance, trapCounter=trapCounter, portAdminState=portAdminState, cardCpuType=cardCpuType, cmResetNotification=cmResetNotification, cardCarrierHwVersion=cardCarrierHwVersion, remainInDiagMode=remainInDiagMode, diskVolumeDriveId=diskVolumeDriveId, cardDetectedChange=cardDetectedChange, auditEntry=auditEntry, equipmentShelfGroup=equipmentShelfGroup, portSecStateChange=portSecStateChange, cmResetInfo=cmResetInfo, auditTotalFailedCount=auditTotalFailedCount, equipmentAuditGroup=equipmentAuditGroup, diagTestId=diagTestId, equipmentStateGroup=equipmentStateGroup, fpgaErrorEventTable=fpgaErrorEventTable, cardFpgaSource=cardFpgaSource, PYSNMP_MODULE_ID=cadEquipmentMib, cardTemperatureHighError=cardTemperatureHighError, diskVolumeEntry=diskVolumeEntry, diskVolumeTrapInh=diskVolumeTrapInh, TestTransactionId=TestTransactionId, equipmentPortGroup=equipmentPortGroup, downstreamPowerLoss=downstreamPowerLoss, portMacIfIndex=portMacIfIndex, cardDplxStatus=cardDplxStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardRamSize=cardRamSize, TestType=TestType, cardSerialNum=cardSerialNum, cardLastBootSource=cardLastBootSource, portId=portId, portMacAddress=portMacAddress, cmResetUpTime=cmResetUpTime, fpgaErrorEventEntry=fpgaErrorEventEntry, fpgaErrorEventGroup=fpgaErrorEventGroup, shelfSwVersion=shelfSwVersion, cardTestTransId=cardTestTransId, cmMacAddress=cmMacAddress, systemKey=systemKey, cardEntry=cardEntry, portPrState=portPrState)
