...
FlowControlMode, PrimaryState, ShelfId, CardType, EqActionType, CardId, PortType, DuplexStatus, SecondaryState, PortMode, PicType, CardSubType, AdminState, DiskVolumeUsageLevel, FirmwareSource, PortDetectedMode, UnknownState, PortId = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "PrimaryState", "ShelfId", "CardType", "EqActionType", "CardId", "PortType", "DuplexStatus", "SecondaryState", "PortMode", "PicType", "CardSubType", "AdminState", "DiskVolumeUsageLevel", "FirmwareSource", "PortDetectedMode", "UnknownState", "PortId")
...
TextualConvention, RowStatus, TruthValue, DateAndTime, DisplayString, MacAddress, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "RowStatus", "TruthValue", "DateAndTime", "DisplayString", "MacAddress", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardFpgaSource=cardFpgaSource, cardTemperature=cardTemperature, downstreamPowerLoss=downstreamPowerLoss, equipmentShelfGroup=equipmentShelfGroup, diskVolumeTrapInh=diskVolumeTrapInh, equipmentError=equipmentError, auditId=auditId, auditPassedCount=auditPassedCount, TestTransactionId=TestTransactionId, auditFailedCount=auditFailedCount, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentCardGroup=equipmentCardGroup, portAction=portAction, cardLastChangeTime=cardLastChangeTime, portMinDsPower=portMinDsPower, cardSpareGroupMode=cardSpareGroupMode, cmUChannelID=cmUChannelID, portRxFlowControlMode=portRxFlowControlMode, cmResetUpTime=cmResetUpTime, cardEntry=cardEntry, cardShelfId=cardShelfId, cardNumber=cardNumber, cardName=cardName, diskVolumeSize=diskVolumeSize, diskVolumeDiskSize=diskVolumeDiskSize, portTrapInh=portTrapInh, portDocsIfIndex=portDocsIfIndex, cmResetReason=cmResetReason, cmResetInfo=cmResetInfo, cardTestType=cardTestType, cardCarrierSerialNum=cardCarrierSerialNum, auditLogOutput=auditLogOutput, diskVolumeCardId=diskVolumeCardId, cardBusSpeed=cardBusSpeed, portPrStateChange=portPrStateChange, diskDriveNumber=diskDriveNumber, auditTime=auditTime, cmRegistrationNotification=cmRegistrationNotification, diskVolumeGroup=diskVolumeGroup, portTxFlowControlDetected=portTxFlowControlDetected, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardBootVersion=cardBootVersion, portSecStateChange=portSecStateChange, cmResetNotification=cmResetNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestResult=cardTestResult, portEntry=portEntry, equipmentAuditGroup=equipmentAuditGroup, portConnectorId=portConnectorId, auditTotalFailedCount=auditTotalFailedCount, verboseLevel=verboseLevel, cardDplxStatus=cardDplxStatus, cardFlashSize=cardFlashSize, diskVolumeId=diskVolumeId, cardCpuSpeed=cardCpuSpeed, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestTransId=cardTestTransId, cardTestTable=cardTestTable, equipmentStateGroup=equipmentStateGroup, equipmentDiag=equipmentDiag, diskVolumeNumber=diskVolumeNumber, auditTotalPassedCount=auditTotalPassedCount, errEvRecoveryEnabled=errEvRecoveryEnabled, errEventId=errEventId, portDescription=portDescription, cardCPLDVersion=cardCPLDVersion, portMaxDsPower=portMaxDsPower, cardDetectedChange=cardDetectedChange, shelfName=shelfName, cardTestResultDesc=cardTestResultDesc, errEvRowStatus=errEvRowStatus, cardSecState=cardSecState, diskVolumeName=diskVolumeName, cardId=cardId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardPrState=cardPrState, cardSecStateChange=cardSecStateChange, portBgpId=portBgpId, cardTestSchedule=cardTestSchedule, cardDetected=cardDetected, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portCardSubType=portCardSubType, cardTestId=cardTestId, portCardId=portCardId, cardSubDetected=cardSubDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeUsageNotification=diskVolumeUsageNotification, portDplxStatus=portDplxStatus, cardSerialNum=cardSerialNum, cardTestIndex=cardTestIndex, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portNumChans=portNumChans, portLastChangeTime=portLastChangeTime, cardHwVersion=cardHwVersion, auditCommand=auditCommand, cardFwUpdateStatus=cardFwUpdateStatus, equipmentAudit=equipmentAudit, auditCycleCount=auditCycleCount, cmResetStatus=cmResetStatus, equipmentState=equipmentState, auditLogThrottle=auditLogThrottle, diskVolumeTable=diskVolumeTable, diagTestId=diagTestId, cardPicDetected=cardPicDetected, portId=portId, cmIpAddress=cmIpAddress, cardFwVersion=cardFwVersion, portDetectedMode=portDetectedMode, cmDevice=cmDevice, TestType=TestType, portCurrDsPower=portCurrDsPower, cardAdminState=cardAdminState, eqDiagConfig=eqDiagConfig, cmPrimarySID=cmPrimarySID, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTestTime=cardTestTime, auditDescription=auditDescription, portShelfId=portShelfId, TestCommand=TestCommand, cardTemperatureHighError=cardTemperatureHighError, cardTestName=cardTestName, cmMacAddress=cmMacAddress, auditAutoScheduling=auditAutoScheduling, cmVendor=cmVendor, TestId=TestId, diskVolumeFileName=diskVolumeFileName, systemGeneral=systemGeneral, auditTable=auditTable, equipmentDiagGroup=equipmentDiagGroup, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeEntry=diskVolumeEntry, errEvLoggingEnabled=errEvLoggingEnabled, auditStatus=auditStatus, shelfNumber=shelfNumber, cardSubType=cardSubType, cardAction=cardAction, trapSeverity=trapSeverity, cardTemperatureHighWarn=cardTemperatureHighWarn, shelfSwVersion=shelfSwVersion, equipmentTraps=equipmentTraps, auditShelfId=auditShelfId, equipmentPortGroup=equipmentPortGroup, cadEquipmentMibCompliance=cadEquipmentMibCompliance, systemClock=systemClock, auditResult=auditResult, cardTestDescription=cardTestDescription, cardTestCommand=cardTestCommand, cardDplxStatusChange=cardDplxStatusChange, cardLastBootVersion=cardLastBootVersion, portMacIfIndex=portMacIfIndex, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, fpgaErrorEventTable=fpgaErrorEventTable, cardTestEntry=cardTestEntry, portTxFlowControlMode=portTxFlowControlMode, diskVolumeDriveId=diskVolumeDriveId, diskVolumeShelfId=diskVolumeShelfId, cmResetClearNotification=cmResetClearNotification, cardUpTime=cardUpTime, cadEquipmentMib=cadEquipmentMib, cardShelfTestIndex=cardShelfTestIndex, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSpareGroupId=cardSpareGroupId, systemGeneralGroup=systemGeneralGroup, equipmentTbl=equipmentTbl, cardCpuType=cardCpuType, equipmentCardTestGroup=equipmentCardTestGroup, portNumber=portNumber, systemKey=systemKey, cardCarrierHwVersion=cardCarrierHwVersion, TestSchedule=TestSchedule, cardNumPorts=cardNumPorts, portMacAddress=portMacAddress, cardTestScheduleCommand=cardTestScheduleCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, auditName=auditName, cardTable=cardTable, cardPicHwVersion=cardPicHwVersion, portAdminState=portAdminState, TestScheduleCommand=TestScheduleCommand, cardType=cardType, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditCardId=auditCardId, cardPrStateChange=cardPrStateChange, TestResult=TestResult, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portPrState=portPrState, cardPicSerialNum=cardPicSerialNum, cardTrapInh=cardTrapInh, portDplxStatusChange=portDplxStatusChange, cardRamSize=cardRamSize, portTable=portTable, portMode=portMode, portSecState=portSecState, cardLastResetReason=cardLastResetReason, shelfObjects=shelfObjects, cardSwVersion=cardSwVersion, portType=portType, cardLastBootSource=cardLastBootSource, auditEntry=auditEntry, trapCounter=trapCounter, consoleOutput=consoleOutput, PYSNMP_MODULE_ID=cadEquipmentMib, errEvLogLevel=errEvLogLevel, cardCarrierFwVersion=cardCarrierFwVersion, remainInDiagMode=remainInDiagMode, cardTestResultNotification=cardTestResultNotification)
