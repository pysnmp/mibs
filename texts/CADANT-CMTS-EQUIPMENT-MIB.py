...
PortMode, UnknownState, PortDetectedMode, PrimaryState, SecondaryState, CardSubType, DuplexStatus, DiskVolumeUsageLevel, ShelfId, CardType, PortId, AdminState, CardId, FirmwareSource, FlowControlMode, PicType, PortType, EqActionType = mibBuilder.importSymbols("CADANT-TC", "PortMode", "UnknownState", "PortDetectedMode", "PrimaryState", "SecondaryState", "CardSubType", "DuplexStatus", "DiskVolumeUsageLevel", "ShelfId", "CardType", "PortId", "AdminState", "CardId", "FirmwareSource", "FlowControlMode", "PicType", "PortType", "EqActionType")
...
TextualConvention, TruthValue, DisplayString, TimeStamp, MacAddress, RowStatus, DateAndTime = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TruthValue", "DisplayString", "TimeStamp", "MacAddress", "RowStatus", "DateAndTime")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTrapInh=portTrapInh, cardCarrierFwVersion=cardCarrierFwVersion, cardSpareGroupMode=cardSpareGroupMode, cardBusSpeed=cardBusSpeed, cardDetectedChange=cardDetectedChange, cardPrState=cardPrState, auditLogThrottle=auditLogThrottle, cardTempNoReportNotification=cardTempNoReportNotification, shelfObjects=shelfObjects, portDplxStatusChange=portDplxStatusChange, cardTestResultNotification=cardTestResultNotification, equipmentCardGroup=equipmentCardGroup, systemClock=systemClock, cardCpuSpeed=cardCpuSpeed, cardTestEntry=cardTestEntry, cardTestTransId=cardTestTransId, cardName=cardName, auditTotalFailedCount=auditTotalFailedCount, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTable=cardTable, cardFwVersion=cardFwVersion, shelfNumber=shelfNumber, TestCommand=TestCommand, cardAction=cardAction, cardFwUpdateStatus=cardFwUpdateStatus, auditAutoScheduling=auditAutoScheduling, cmUChannelID=cmUChannelID, portCurrDsPower=portCurrDsPower, cardTestSchedule=cardTestSchedule, trapCounter=trapCounter, TestResult=TestResult, cardTestIndex=cardTestIndex, cmResetStatus=cmResetStatus, cardUpTime=cardUpTime, diskVolumeFileName=diskVolumeFileName, portMode=portMode, trapSeverity=trapSeverity, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardFlashSize=cardFlashSize, eqDiagConfig=eqDiagConfig, cmRegistrationNotification=cmRegistrationNotification, equipmentError=equipmentError, diskVolumeCardId=diskVolumeCardId, diskVolumeTrapInh=diskVolumeTrapInh, auditId=auditId, diskVolumeNumber=diskVolumeNumber, PYSNMP_MODULE_ID=cadEquipmentMib, cardSecState=cardSecState, portShelfId=portShelfId, cardPicHwVersion=cardPicHwVersion, cardTrapInh=cardTrapInh, auditCardId=auditCardId, portDocsIfIndex=portDocsIfIndex, cardCpuType=cardCpuType, verboseLevel=verboseLevel, cardLastBootVersion=cardLastBootVersion, cmVendor=cmVendor, cmResetReason=cmResetReason, cardPrStateChange=cardPrStateChange, cardTestType=cardTestType, cardDplxStatusChange=cardDplxStatusChange, portConnectorId=portConnectorId, cardLastBootSource=cardLastBootSource, errEvLogLevel=errEvLogLevel, errEvRowStatus=errEvRowStatus, cardTestResultDesc=cardTestResultDesc, shelfSwVersion=shelfSwVersion, systemGeneralGroup=systemGeneralGroup, cardTestTime=cardTestTime, cardTestDescription=cardTestDescription, cardTestId=cardTestId, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditTable=auditTable, cardType=cardType, cardLastResetReason=cardLastResetReason, cardPicDetected=cardPicDetected, auditPassedCount=auditPassedCount, equipmentTbl=equipmentTbl, cardTemperatureHighError=cardTemperatureHighError, equipmentDiag=equipmentDiag, auditTime=auditTime, portMacIfIndex=portMacIfIndex, diskVolumeName=diskVolumeName, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentTraps=equipmentTraps, systemKey=systemKey, auditShelfId=auditShelfId, equipmentState=equipmentState, portTxFlowControlDetected=portTxFlowControlDetected, fpgaErrorEventTable=fpgaErrorEventTable, cmMacAddress=cmMacAddress, systemGeneral=systemGeneral, cmResetClearNotification=cmResetClearNotification, cmIpAddress=cmIpAddress, auditStatus=auditStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeSize=diskVolumeSize, cardSubDetected=cardSubDetected, portSecStateChange=portSecStateChange, auditEntry=auditEntry, auditDescription=auditDescription, equipmentPortGroup=equipmentPortGroup, downstreamPowerLoss=downstreamPowerLoss, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeId=diskVolumeId, consoleOutput=consoleOutput, errEventId=errEventId, portBgpId=portBgpId, auditCommand=auditCommand, cmResetInfo=cmResetInfo, portDescription=portDescription, cardDetected=cardDetected, cardCarrierSerialNum=cardCarrierSerialNum, cardFpgaSource=cardFpgaSource, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestCommand=cardTestCommand, cardSwVersion=cardSwVersion, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeTable=diskVolumeTable, portDplxStatus=portDplxStatus, portCardSubType=portCardSubType, diskVolumeUsageNotification=diskVolumeUsageNotification, portSecState=portSecState, portTable=portTable, cmDevice=cmDevice, cadEquipmentMibConformance=cadEquipmentMibConformance, portTxFlowControlMode=portTxFlowControlMode, cardSerialNum=cardSerialNum, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portEntry=portEntry, cardPicSerialNum=cardPicSerialNum, portAdminState=portAdminState, cardTestName=cardTestName, cardTemperature=cardTemperature, shelfName=shelfName, cardNumber=cardNumber, remainInDiagMode=remainInDiagMode, auditLogOutput=auditLogOutput, errEvLoggingEnabled=errEvLoggingEnabled, auditFailedCount=auditFailedCount, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portMaxDsPower=portMaxDsPower, cardNumPorts=cardNumPorts, TestId=TestId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, diskVolumeEntry=diskVolumeEntry, TestType=TestType, cadEquipmentMib=cadEquipmentMib, cardCPLDVersion=cardCPLDVersion, TestSchedule=TestSchedule, portRxFlowControlMode=portRxFlowControlMode, auditName=auditName, cardShelfTestIndex=cardShelfTestIndex, cardEntry=cardEntry, cmResetUpTime=cmResetUpTime, equipmentCardTestGroup=equipmentCardTestGroup, auditTotalPassedCount=auditTotalPassedCount, cardTestTable=cardTestTable, cardLastChangeTime=cardLastChangeTime, portPrStateChange=portPrStateChange, portLastChangeTime=portLastChangeTime, portMinDsPower=portMinDsPower, fpgaErrorEventEntry=fpgaErrorEventEntry, portId=portId, equipmentAudit=equipmentAudit, auditResult=auditResult, equipmentDiagGroup=equipmentDiagGroup, portCardId=portCardId, TestScheduleCommand=TestScheduleCommand, cardSecStateChange=cardSecStateChange, cardAdminState=cardAdminState, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeGroup=diskVolumeGroup, cardHwVersion=cardHwVersion, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeShelfId=diskVolumeShelfId, TestTransactionId=TestTransactionId, cardShelfId=cardShelfId, equipmentAuditGroup=equipmentAuditGroup, cmPrimarySID=cmPrimarySID, cardBootVersion=cardBootVersion, diskDriveNumber=diskDriveNumber, cardRamSize=cardRamSize, diagTestId=diagTestId, equipmentShelfGroup=equipmentShelfGroup, cardDplxStatus=cardDplxStatus, diskVolumeDriveId=diskVolumeDriveId, portNumber=portNumber, portMacAddress=portMacAddress, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portAction=portAction, portNumChans=portNumChans, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portPrState=portPrState, portDetectedMode=portDetectedMode, diskVolumeUsageLevel=diskVolumeUsageLevel, cmResetNotification=cmResetNotification, portType=portType, auditCycleCount=auditCycleCount, cardSpareGroupId=cardSpareGroupId, cardTestResult=cardTestResult, equipmentStateGroup=equipmentStateGroup, cardSubType=cardSubType, cardCarrierHwVersion=cardCarrierHwVersion, cardId=cardId)
