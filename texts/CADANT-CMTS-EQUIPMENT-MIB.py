...
PortDetectedMode, CardId, PicType, EqActionType, PortMode, PortId, CardSubType, FlowControlMode, PrimaryState, SecondaryState, DiskVolumeUsageLevel, DuplexStatus, PortType, CardType, UnknownState, ShelfId, FirmwareSource, AdminState = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "CardId", "PicType", "EqActionType", "PortMode", "PortId", "CardSubType", "FlowControlMode", "PrimaryState", "SecondaryState", "DiskVolumeUsageLevel", "DuplexStatus", "PortType", "CardType", "UnknownState", "ShelfId", "FirmwareSource", "AdminState")
...
TruthValue, DateAndTime, DisplayString, MacAddress, TextualConvention, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DateAndTime", "DisplayString", "MacAddress", "TextualConvention", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentState=equipmentState, diskVolumeCardId=diskVolumeCardId, cardNumPorts=cardNumPorts, auditDescription=auditDescription, cardBootVersion=cardBootVersion, errEventId=errEventId, cardFpgaSource=cardFpgaSource, equipmentCardGroup=equipmentCardGroup, cardTestTable=cardTestTable, auditPassedCount=auditPassedCount, equipmentDiag=equipmentDiag, shelfNumber=shelfNumber, cardTestDescription=cardTestDescription, cardTestResultDesc=cardTestResultDesc, cadEquipmentMib=cadEquipmentMib, cardTestCommand=cardTestCommand, auditResult=auditResult, cmRegistrationNotification=cmRegistrationNotification, errEvRowStatus=errEvRowStatus, auditTotalFailedCount=auditTotalFailedCount, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentTbl=equipmentTbl, cardSerialNum=cardSerialNum, portPrStateChange=portPrStateChange, shelfObjects=shelfObjects, portPrState=portPrState, portDplxStatus=portDplxStatus, cardTestEntry=cardTestEntry, portMaxDsPower=portMaxDsPower, auditCardId=auditCardId, cardPrStateChange=cardPrStateChange, equipmentError=equipmentError, TestResult=TestResult, cardLastBootVersion=cardLastBootVersion, portEntry=portEntry, cardCarrierSerialNum=cardCarrierSerialNum, cardLastChangeTime=cardLastChangeTime, equipmentShelfGroup=equipmentShelfGroup, cardUpTime=cardUpTime, errEvLoggingEnabled=errEvLoggingEnabled, cardId=cardId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestScheduleCommand=cardTestScheduleCommand, cardSecStateChange=cardSecStateChange, auditLogThrottle=auditLogThrottle, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDplxStatus=cardDplxStatus, cmResetStatus=cmResetStatus, cardFwUpdateStatus=cardFwUpdateStatus, auditLogOutput=auditLogOutput, auditCycleCount=auditCycleCount, auditShelfId=auditShelfId, portAdminState=portAdminState, diagTestId=diagTestId, portNumber=portNumber, cardHwVersion=cardHwVersion, downstreamPowerLoss=downstreamPowerLoss, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageLevel=diskVolumeUsageLevel, cardSubDetected=cardSubDetected, portCurrDsPower=portCurrDsPower, cardLastResetReason=cardLastResetReason, cardTempNoReportNotification=cardTempNoReportNotification, auditStatus=auditStatus, cardFwVersion=cardFwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResultNotification=cardTestResultNotification, cardCPLDVersion=cardCPLDVersion, cardNumber=cardNumber, cardPicHwVersion=cardPicHwVersion, portMode=portMode, diskVolumeDriveId=diskVolumeDriveId, portSecState=portSecState, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSecState=cardSecState, systemClock=systemClock, equipmentTraps=equipmentTraps, cardPrState=cardPrState, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicSerialNum=cardPicSerialNum, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditAutoScheduling=auditAutoScheduling, cmResetClearNotification=cmResetClearNotification, cardSubType=cardSubType, cmMacAddress=cmMacAddress, TestScheduleCommand=TestScheduleCommand, portDocsIfIndex=portDocsIfIndex, portBgpId=portBgpId, cardEntry=cardEntry, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeEntry=diskVolumeEntry, errEvLogLevel=errEvLogLevel, trapCounter=trapCounter, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeDiskSize=diskVolumeDiskSize, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, shelfSwVersion=shelfSwVersion, cardName=cardName, equipmentStateGroup=equipmentStateGroup, portLastChangeTime=portLastChangeTime, cardShelfId=cardShelfId, portAction=portAction, cardTestTransId=cardTestTransId, cadEquipmentMibConformance=cadEquipmentMibConformance, portDescription=portDescription, cardBusSpeed=cardBusSpeed, trapSeverity=trapSeverity, cardTestIndex=cardTestIndex, cardAdminState=cardAdminState, cmResetInfo=cmResetInfo, cardSpareGroupMode=cardSpareGroupMode, cmResetNotification=cmResetNotification, consoleOutput=consoleOutput, cmIpAddress=cmIpAddress, cardTestResult=cardTestResult, cmDevice=cmDevice, cardCpuType=cardCpuType, eqDiagConfig=eqDiagConfig, equipmentAudit=equipmentAudit, TestId=TestId, diskVolumeShelfId=diskVolumeShelfId, cmUChannelID=cmUChannelID, portMacIfIndex=portMacIfIndex, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentCardTestGroup=equipmentCardTestGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, auditId=auditId, diskVolumeNumber=diskVolumeNumber, portType=portType, cmResetUpTime=cmResetUpTime, equipmentPortGroup=equipmentPortGroup, cardTable=cardTable, cardLastBootSource=cardLastBootSource, portNumChans=portNumChans, cardDetectedChange=cardDetectedChange, cardTemperatureHighError=cardTemperatureHighError, TestCommand=TestCommand, portSecStateChange=portSecStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, portDetectedMode=portDetectedMode, equipmentAuditGroup=equipmentAuditGroup, TestSchedule=TestSchedule, diskVolumeGroup=diskVolumeGroup, cardDplxStatusChange=cardDplxStatusChange, cardShelfTestIndex=cardShelfTestIndex, systemGeneralGroup=systemGeneralGroup, diskVolumeTable=diskVolumeTable, verboseLevel=verboseLevel, TestType=TestType, cardRamSize=cardRamSize, cardCpuSpeed=cardCpuSpeed, portMacAddress=portMacAddress, diskVolumeId=diskVolumeId, diskVolumeFileName=diskVolumeFileName, fpgaErrorEventTable=fpgaErrorEventTable, TestTransactionId=TestTransactionId, cardDetected=cardDetected, fpgaErrorEventGroup=fpgaErrorEventGroup, cmPrimarySID=cmPrimarySID, cardFlashSize=cardFlashSize, diskVolumeSize=diskVolumeSize, auditName=auditName, systemKey=systemKey, auditTime=auditTime, cardType=cardType, cardPicDetected=cardPicDetected, auditFailedCount=auditFailedCount, portShelfId=portShelfId, systemGeneral=systemGeneral, shelfName=shelfName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeTrapInh=diskVolumeTrapInh, auditEntry=auditEntry, portDplxStatusChange=portDplxStatusChange, cardSwVersion=cardSwVersion, cardTestId=cardTestId, diskDriveNumber=diskDriveNumber, portCardSubType=portCardSubType, cardSpareGroupId=cardSpareGroupId, cardTrapInh=cardTrapInh, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditTotalPassedCount=auditTotalPassedCount, diskVolumeName=diskVolumeName, cardTestName=cardTestName, equipmentDiagGroup=equipmentDiagGroup, portCardId=portCardId, auditCommand=auditCommand, cmVendor=cmVendor, portId=portId, portMinDsPower=portMinDsPower, portConnectorId=portConnectorId, portRxFlowControlMode=portRxFlowControlMode, cardCarrierFwVersion=cardCarrierFwVersion, cardTestSchedule=cardTestSchedule, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portTable=portTable, cardTemperature=cardTemperature, cardTestType=cardTestType, auditTable=auditTable, cmResetReason=cmResetReason, portRxFlowControlDetected=portRxFlowControlDetected, remainInDiagMode=remainInDiagMode, cardTestTime=cardTestTime, portTrapInh=portTrapInh, cardCarrierHwVersion=cardCarrierHwVersion, cardAction=cardAction)
