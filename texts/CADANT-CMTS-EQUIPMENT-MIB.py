...
PrimaryState, PicType, EqActionType, FirmwareSource, AdminState, PortMode, PortType, SecondaryState, UnknownState, DiskVolumeUsageLevel, DuplexStatus, CardId, FlowControlMode, PortDetectedMode, CardType, CardSubType, ShelfId, PortId = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "PicType", "EqActionType", "FirmwareSource", "AdminState", "PortMode", "PortType", "SecondaryState", "UnknownState", "DiskVolumeUsageLevel", "DuplexStatus", "CardId", "FlowControlMode", "PortDetectedMode", "CardType", "CardSubType", "ShelfId", "PortId")
...
DisplayString, MacAddress, DateAndTime, RowStatus, TimeStamp, TruthValue, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "DateAndTime", "RowStatus", "TimeStamp", "TruthValue", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentTraps=equipmentTraps, portRxFlowControlDetected=portRxFlowControlDetected, cardTemperatureHighError=cardTemperatureHighError, cardCpuType=cardCpuType, portPrState=portPrState, auditDescription=auditDescription, auditTotalPassedCount=auditTotalPassedCount, shelfNumber=shelfNumber, cardBootVersion=cardBootVersion, cardPicSerialNum=cardPicSerialNum, portDplxStatusChange=portDplxStatusChange, cmIpAddress=cmIpAddress, PYSNMP_MODULE_ID=cadEquipmentMib, cardType=cardType, cardPrState=cardPrState, cardTestResultDesc=cardTestResultDesc, shelfSwVersion=shelfSwVersion, cardTemperature=cardTemperature, verboseLevel=verboseLevel, auditCommand=auditCommand, fpgaErrorEventEntry=fpgaErrorEventEntry, errEvRowStatus=errEvRowStatus, auditName=auditName, cardTestSchedule=cardTestSchedule, cardLastResetReason=cardLastResetReason, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, auditAutoScheduling=auditAutoScheduling, diskVolumeUsageNotification=diskVolumeUsageNotification, cardLastBootSource=cardLastBootSource, trapSeverity=trapSeverity, cardSubDetected=cardSubDetected, portCardId=portCardId, auditTable=auditTable, cadEquipmentMibConformance=cadEquipmentMibConformance, cardLastChangeTime=cardLastChangeTime, fpgaErrorEventTable=fpgaErrorEventTable, downstreamPowerLoss=downstreamPowerLoss, cardDetected=cardDetected, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTemperatureHighWarn=cardTemperatureHighWarn, TestId=TestId, portAction=portAction, portCurrDsPower=portCurrDsPower, cardNumber=cardNumber, cardCpuSpeed=cardCpuSpeed, portTrapInh=portTrapInh, portPrStateChange=portPrStateChange, equipmentStateGroup=equipmentStateGroup, diskVolumeTrapInh=diskVolumeTrapInh, cmResetInfo=cmResetInfo, equipmentShelfGroup=equipmentShelfGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardSecState=cardSecState, cardAction=cardAction, diskVolumeEntry=diskVolumeEntry, cmMacAddress=cmMacAddress, cmVendor=cmVendor, TestCommand=TestCommand, cardHwVersion=cardHwVersion, auditTotalFailedCount=auditTotalFailedCount, cardCarrierHwVersion=cardCarrierHwVersion, portType=portType, portSecState=portSecState, cardBusSpeed=cardBusSpeed, diskVolumeTable=diskVolumeTable, auditCycleCount=auditCycleCount, diskVolumeShelfId=diskVolumeShelfId, portDetectedMode=portDetectedMode, cardDplxStatus=cardDplxStatus, diskDriveNumber=diskDriveNumber, systemGeneral=systemGeneral, cardSecStateChange=cardSecStateChange, diagTestId=diagTestId, diskVolumeName=diskVolumeName, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardRamSize=cardRamSize, cardTestIndex=cardTestIndex, diskVolumeSize=diskVolumeSize, TestTransactionId=TestTransactionId, remainInDiagMode=remainInDiagMode, cardTrapInh=cardTrapInh, equipmentAudit=equipmentAudit, TestSchedule=TestSchedule, diskVolumeDiskSize=diskVolumeDiskSize, cmUChannelID=cmUChannelID, equipmentError=equipmentError, TestScheduleCommand=TestScheduleCommand, cardUpTime=cardUpTime, shelfObjects=shelfObjects, errEventId=errEventId, portDocsIfIndex=portDocsIfIndex, cadEquipmentMib=cadEquipmentMib, cardLastBootVersion=cardLastBootVersion, consoleOutput=consoleOutput, trapCounter=trapCounter, cardTestDescription=cardTestDescription, auditTime=auditTime, auditEntry=auditEntry, portRxFlowControlMode=portRxFlowControlMode, portTxFlowControlDetected=portTxFlowControlDetected, cardTestTable=cardTestTable, portSecStateChange=portSecStateChange, portBgpId=portBgpId, cardTestName=cardTestName, portConnectorId=portConnectorId, cardTestResult=cardTestResult, systemGeneralGroup=systemGeneralGroup, cmResetClearNotification=cmResetClearNotification, cardSwVersion=cardSwVersion, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageLevel=diskVolumeUsageLevel, auditPassedCount=auditPassedCount, equipmentDiagGroup=equipmentDiagGroup, cardPicDetected=cardPicDetected, TestType=TestType, cmPrimarySID=cmPrimarySID, errEvLogLevel=errEvLogLevel, auditLogOutput=auditLogOutput, cardCarrierFwVersion=cardCarrierFwVersion, cardDplxStatusChange=cardDplxStatusChange, diskVolumeDriveId=diskVolumeDriveId, portTable=portTable, portEntry=portEntry, cmDevice=cmDevice, systemClock=systemClock, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestId=cardTestId, portCardSubType=portCardSubType, diskVolumeId=diskVolumeId, cardTestScheduleCommand=cardTestScheduleCommand, cardTestType=cardTestType, cardTestEntry=cardTestEntry, equipmentState=equipmentState, cardFpgaSource=cardFpgaSource, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTestCommand=cardTestCommand, equipmentDiag=equipmentDiag, equipmentCardTestGroup=equipmentCardTestGroup, cardTestTime=cardTestTime, cmRegistrationNotification=cmRegistrationNotification, portMacIfIndex=portMacIfIndex, cardFwUpdateStatus=cardFwUpdateStatus, portMaxDsPower=portMaxDsPower, portTxFlowControlMode=portTxFlowControlMode, portMacAddress=portMacAddress, diskVolumeCardId=diskVolumeCardId, auditStatus=auditStatus, cardTable=cardTable, portNumber=portNumber, portShelfId=portShelfId, portNumChans=portNumChans, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardId=cardId, cardPrStateChange=cardPrStateChange, equipmentPortGroup=equipmentPortGroup, auditId=auditId, errEvLoggingEnabled=errEvLoggingEnabled, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResultNotification=cardTestResultNotification, equipmentCardGroup=equipmentCardGroup, auditCardId=auditCardId, auditShelfId=auditShelfId, portDplxStatus=portDplxStatus, cardDetectedChange=cardDetectedChange, cmResetNotification=cmResetNotification, auditFailedCount=auditFailedCount, cardSubType=cardSubType, portMode=portMode, shelfName=shelfName, TestResult=TestResult, equipmentTbl=equipmentTbl, cardName=cardName, cardTempNoReportNotification=cardTempNoReportNotification, eqDiagConfig=eqDiagConfig, cardPicHwVersion=cardPicHwVersion, portAdminState=portAdminState, cardShelfId=cardShelfId, auditResult=auditResult, cmResetUpTime=cmResetUpTime, portLastChangeTime=portLastChangeTime, cardSpareGroupId=cardSpareGroupId, portId=portId, cardEntry=cardEntry, diskVolumeNumber=diskVolumeNumber, portMinDsPower=portMinDsPower, systemKey=systemKey, cardCPLDVersion=cardCPLDVersion, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeFileName=diskVolumeFileName, auditLogThrottle=auditLogThrottle, equipmentAuditGroup=equipmentAuditGroup, cardFlashSize=cardFlashSize, cardTestTransId=cardTestTransId, cardShelfTestIndex=cardShelfTestIndex, cardSerialNum=cardSerialNum, cmResetReason=cmResetReason, diskVolumeGroup=diskVolumeGroup, cardAdminState=cardAdminState, cardFwVersion=cardFwVersion, cardSpareGroupMode=cardSpareGroupMode, cmResetStatus=cmResetStatus, cardNumPorts=cardNumPorts, portDescription=portDescription)
