...
DuplexStatus, PortMode, FlowControlMode, UnknownState, CardSubType, PortDetectedMode, AdminState, PrimaryState, SecondaryState, DiskVolumeUsageLevel, ShelfId, FirmwareSource, CardId, PortType, EqActionType, PicType, PortId, CardType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortMode", "FlowControlMode", "UnknownState", "CardSubType", "PortDetectedMode", "AdminState", "PrimaryState", "SecondaryState", "DiskVolumeUsageLevel", "ShelfId", "FirmwareSource", "CardId", "PortType", "EqActionType", "PicType", "PortId", "CardType")
...
RowStatus, TextualConvention, DateAndTime, MacAddress, DisplayString, TimeStamp, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "DateAndTime", "MacAddress", "DisplayString", "TimeStamp", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeFileName=diskVolumeFileName, cmResetNotification=cmResetNotification, shelfName=shelfName, systemGeneralGroup=systemGeneralGroup, shelfObjects=shelfObjects, cardPrState=cardPrState, portRxFlowControlMode=portRxFlowControlMode, auditPassedCount=auditPassedCount, cardTestType=cardTestType, cmPrimarySID=cmPrimarySID, cardTestName=cardTestName, cardTemperature=cardTemperature, portDplxStatus=portDplxStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, cardCarrierFwVersion=cardCarrierFwVersion, portCardSubType=portCardSubType, cardSecStateChange=cardSecStateChange, cardTestCommand=cardTestCommand, cardTestResultNotification=cardTestResultNotification, equipmentCardTestGroup=equipmentCardTestGroup, cardTestSchedule=cardTestSchedule, cardDplxStatusChange=cardDplxStatusChange, portDetectedMode=portDetectedMode, systemKey=systemKey, cardUpTime=cardUpTime, shelfNumber=shelfNumber, auditTotalFailedCount=auditTotalFailedCount, portPrStateChange=portPrStateChange, portEntry=portEntry, portBgpId=portBgpId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardFwUpdateStatus=cardFwUpdateStatus, portTable=portTable, cardPicHwVersion=cardPicHwVersion, cardCPLDVersion=cardCPLDVersion, diskVolumeEntry=diskVolumeEntry, cmResetStatus=cmResetStatus, portCardId=portCardId, diskVolumeNumber=diskVolumeNumber, portRxFlowControlDetected=portRxFlowControlDetected, portMacAddress=portMacAddress, cardCpuType=cardCpuType, cmResetUpTime=cmResetUpTime, TestCommand=TestCommand, cardLastBootSource=cardLastBootSource, portMinDsPower=portMinDsPower, diskVolumeDiskSize=diskVolumeDiskSize, cardTestDescription=cardTestDescription, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDetected=cardDetected, trapCounter=trapCounter, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmMacAddress=cmMacAddress, cardFpgaSource=cardFpgaSource, systemGeneral=systemGeneral, diskVolumeDriveId=diskVolumeDriveId, cardPrStateChange=cardPrStateChange, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupId=cardSpareGroupId, diskVolumeId=diskVolumeId, cmResetInfo=cmResetInfo, cardAction=cardAction, equipmentTbl=equipmentTbl, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeTable=diskVolumeTable, auditLogThrottle=auditLogThrottle, portDescription=portDescription, cmResetReason=cmResetReason, equipmentShelfGroup=equipmentShelfGroup, cardNumber=cardNumber, portTxFlowControlDetected=portTxFlowControlDetected, TestType=TestType, cmDevice=cmDevice, cadEquipmentMibConformance=cadEquipmentMibConformance, auditFailedCount=auditFailedCount, shelfSwVersion=shelfSwVersion, cardTestResult=cardTestResult, portSecStateChange=portSecStateChange, auditAutoScheduling=auditAutoScheduling, portTrapInh=portTrapInh, diskVolumeCardId=diskVolumeCardId, diskVolumeSize=diskVolumeSize, diagTestId=diagTestId, equipmentAuditGroup=equipmentAuditGroup, diskVolumeGroup=diskVolumeGroup, auditName=auditName, cardShelfTestIndex=cardShelfTestIndex, equipmentState=equipmentState, auditShelfId=auditShelfId, downstreamPowerLoss=downstreamPowerLoss, auditTime=auditTime, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeUsageLevel=diskVolumeUsageLevel, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestTransId=cardTestTransId, TestSchedule=TestSchedule, cardTestId=cardTestId, auditTotalPassedCount=auditTotalPassedCount, diskDriveNumber=diskDriveNumber, trapSeverity=trapSeverity, cardEntry=cardEntry, cardSubType=cardSubType, cardTestTable=cardTestTable, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSubDetected=cardSubDetected, cardBusSpeed=cardBusSpeed, portNumChans=portNumChans, cardTempNoReportNotification=cardTempNoReportNotification, consoleOutput=consoleOutput, cardPicDetected=cardPicDetected, cardSpareGroupMode=cardSpareGroupMode, auditCycleCount=auditCycleCount, cardCarrierSerialNum=cardCarrierSerialNum, systemClock=systemClock, portLastChangeTime=portLastChangeTime, equipmentAudit=equipmentAudit, equipmentPortGroup=equipmentPortGroup, equipmentTraps=equipmentTraps, cadEquipmentMibGroup=cadEquipmentMibGroup, cardNumPorts=cardNumPorts, cardTestIndex=cardTestIndex, equipmentStateGroup=equipmentStateGroup, cardSwVersion=cardSwVersion, auditTable=auditTable, cardName=cardName, cardShelfId=cardShelfId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditResult=auditResult, portConnectorId=portConnectorId, portMacIfIndex=portMacIfIndex, auditDescription=auditDescription, auditStatus=auditStatus, portId=portId, portNumber=portNumber, cardSecState=cardSecState, auditEntry=auditEntry, errEvRowStatus=errEvRowStatus, equipmentError=equipmentError, cmUChannelID=cmUChannelID, equipmentDiagGroup=equipmentDiagGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portAdminState=portAdminState, cardBootVersion=cardBootVersion, portType=portType, cardTempOverHeatNotification=cardTempOverHeatNotification, portCurrDsPower=portCurrDsPower, portShelfId=portShelfId, portAction=portAction, portDocsIfIndex=portDocsIfIndex, portTxFlowControlMode=portTxFlowControlMode, cmRegistrationNotification=cmRegistrationNotification, diskVolumeName=diskVolumeName, cardTestEntry=cardTestEntry, cardTable=cardTable, errEvLogLevel=errEvLogLevel, portPrState=portPrState, remainInDiagMode=remainInDiagMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMib=cadEquipmentMib, TestResult=TestResult, fpgaErrorEventGroup=fpgaErrorEventGroup, cardLastChangeTime=cardLastChangeTime, cardHwVersion=cardHwVersion, TestId=TestId, cardId=cardId, diskVolumeTrapInh=diskVolumeTrapInh, equipmentCardGroup=equipmentCardGroup, cardRamSize=cardRamSize, cardDetectedChange=cardDetectedChange, auditCardId=auditCardId, cardFwVersion=cardFwVersion, cmResetClearNotification=cmResetClearNotification, cardDplxStatus=cardDplxStatus, auditLogOutput=auditLogOutput, errEvLoggingEnabled=errEvLoggingEnabled, auditId=auditId, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, eqDiagConfig=eqDiagConfig, cardCpuSpeed=cardCpuSpeed, portMaxDsPower=portMaxDsPower, cardCarrierHwVersion=cardCarrierHwVersion, cardTemperatureHighError=cardTemperatureHighError, cmIpAddress=cmIpAddress, auditCommand=auditCommand, cardTestTime=cardTestTime, cardFlashSize=cardFlashSize, TestScheduleCommand=TestScheduleCommand, cardAdminState=cardAdminState, cardLastResetReason=cardLastResetReason, fpgaErrorEventTable=fpgaErrorEventTable, cardTrapInh=cardTrapInh, diskVolumeShelfId=diskVolumeShelfId, cardLastBootVersion=cardLastBootVersion, cardPicSerialNum=cardPicSerialNum, equipmentDiag=equipmentDiag, cmVendor=cmVendor, portMode=portMode, portSecState=portSecState, errEventId=errEventId, cardType=cardType, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestResultDesc=cardTestResultDesc, verboseLevel=verboseLevel, portDplxStatusChange=portDplxStatusChange, TestTransactionId=TestTransactionId, cardSerialNum=cardSerialNum)
