...
ShelfId, FlowControlMode, DuplexStatus, EqActionType, PicType, CardId, AdminState, FirmwareSource, SecondaryState, PortType, PortMode, CardSubType, PortDetectedMode, UnknownState, CardType, PortId, PrimaryState, DiskVolumeUsageLevel = mibBuilder.importSymbols("CADANT-TC", "ShelfId", "FlowControlMode", "DuplexStatus", "EqActionType", "PicType", "CardId", "AdminState", "FirmwareSource", "SecondaryState", "PortType", "PortMode", "CardSubType", "PortDetectedMode", "UnknownState", "CardType", "PortId", "PrimaryState", "DiskVolumeUsageLevel")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portNumChans=portNumChans, cardCpuSpeed=cardCpuSpeed, portTable=portTable, cardPicDetected=cardPicDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardDplxStatusChange=cardDplxStatusChange, cardLastResetReason=cardLastResetReason, cardTestScheduleCommand=cardTestScheduleCommand, auditFailedCount=auditFailedCount, portPrStateChange=portPrStateChange, cardRamSize=cardRamSize, portRxFlowControlDetected=portRxFlowControlDetected, cardLastChangeTime=cardLastChangeTime, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmIpAddress=cmIpAddress, portMaxDsPower=portMaxDsPower, diskVolumeGroup=diskVolumeGroup, diskVolumeFileName=diskVolumeFileName, cmVendor=cmVendor, cardId=cardId, auditName=auditName, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeDiskSize=diskVolumeDiskSize, cardEntry=cardEntry, TestCommand=TestCommand, auditPassedCount=auditPassedCount, cardSubDetected=cardSubDetected, cardTestName=cardTestName, cadEquipmentMibGroup=cadEquipmentMibGroup, TestType=TestType, cardSubType=cardSubType, cardType=cardType, portEntry=portEntry, portBgpId=portBgpId, portConnectorId=portConnectorId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTempNoReportNotification=cardTempNoReportNotification, cardDetected=cardDetected, cardAdminState=cardAdminState, TestSchedule=TestSchedule, cardFpgaSource=cardFpgaSource, diskVolumeDriveId=diskVolumeDriveId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTemperature=cardTemperature, cardTestResultDesc=cardTestResultDesc, cmResetNotification=cmResetNotification, TestId=TestId, equipmentStateGroup=equipmentStateGroup, trapSeverity=trapSeverity, portType=portType, errEvLogLevel=errEvLogLevel, portId=portId, systemKey=systemKey, portDescription=portDescription, portSecState=portSecState, portCardId=portCardId, cardTempOverHeatNotification=cardTempOverHeatNotification, auditCardId=auditCardId, cardSecState=cardSecState, diskVolumeId=diskVolumeId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portTxFlowControlMode=portTxFlowControlMode, fpgaErrorEventEntry=fpgaErrorEventEntry, portSecStateChange=portSecStateChange, diskVolumeNumber=diskVolumeNumber, portDocsIfIndex=portDocsIfIndex, auditEntry=auditEntry, diskVolumeSize=diskVolumeSize, portMacAddress=portMacAddress, cardSerialNum=cardSerialNum, diskVolumeTrapInh=diskVolumeTrapInh, cardSecStateChange=cardSecStateChange, diskVolumeEntry=diskVolumeEntry, cardUpTime=cardUpTime, cardTemperatureHighError=cardTemperatureHighError, portCurrDsPower=portCurrDsPower, cardTestTable=cardTestTable, equipmentDiagGroup=equipmentDiagGroup, shelfObjects=shelfObjects, cardLastBootVersion=cardLastBootVersion, verboseLevel=verboseLevel, TestResult=TestResult, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portDetectedMode=portDetectedMode, cmResetUpTime=cmResetUpTime, cardAction=cardAction, cardCPLDVersion=cardCPLDVersion, cardTrapInh=cardTrapInh, portAction=portAction, errEvLoggingEnabled=errEvLoggingEnabled, cardPicHwVersion=cardPicHwVersion, systemClock=systemClock, portRxFlowControlMode=portRxFlowControlMode, auditShelfId=auditShelfId, equipmentDiag=equipmentDiag, portLastChangeTime=portLastChangeTime, equipmentTraps=equipmentTraps, portDplxStatus=portDplxStatus, equipmentCardGroup=equipmentCardGroup, cmDevice=cmDevice, diskVolumeShelfId=diskVolumeShelfId, portShelfId=portShelfId, cardTestResultNotification=cardTestResultNotification, cardDplxStatus=cardDplxStatus, shelfName=shelfName, cardFwUpdateStatus=cardFwUpdateStatus, portDplxStatusChange=portDplxStatusChange, cardShelfTestIndex=cardShelfTestIndex, cardName=cardName, cardSpareGroupId=cardSpareGroupId, equipmentState=equipmentState, auditTotalFailedCount=auditTotalFailedCount, portNumber=portNumber, cmResetInfo=cmResetInfo, fpgaErrorEventTable=fpgaErrorEventTable, cardDetectedChange=cardDetectedChange, cardTestDescription=cardTestDescription, portTrapInh=portTrapInh, cardTestSchedule=cardTestSchedule, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeCardId=diskVolumeCardId, TestTransactionId=TestTransactionId, auditResult=auditResult, systemGeneralGroup=systemGeneralGroup, equipmentTbl=equipmentTbl, auditAutoScheduling=auditAutoScheduling, errEvRowStatus=errEvRowStatus, portCardSubType=portCardSubType, cardTestType=cardTestType, auditStatus=auditStatus, cardBusSpeed=cardBusSpeed, cardTestTransId=cardTestTransId, shelfSwVersion=shelfSwVersion, auditTotalPassedCount=auditTotalPassedCount, TestScheduleCommand=TestScheduleCommand, cardTable=cardTable, eqDiagConfig=eqDiagConfig, cardTestCommand=cardTestCommand, diskVolumeName=diskVolumeName, cmMacAddress=cmMacAddress, errEventId=errEventId, cardTemperatureHighWarn=cardTemperatureHighWarn, cmResetStatus=cmResetStatus, cmResetClearNotification=cmResetClearNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardNumber=cardNumber, diskDriveNumber=diskDriveNumber, auditLogThrottle=auditLogThrottle, portMode=portMode, equipmentCardTestGroup=equipmentCardTestGroup, cardTestId=cardTestId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicSerialNum=cardPicSerialNum, diagTestId=diagTestId, portMacIfIndex=portMacIfIndex, diskVolumeUsageLevel=diskVolumeUsageLevel, shelfNumber=shelfNumber, cardHwVersion=cardHwVersion, cardBootVersion=cardBootVersion, cardLastBootSource=cardLastBootSource, auditCycleCount=auditCycleCount, portPrState=portPrState, trapCounter=trapCounter, cardShelfId=cardShelfId, equipmentAudit=equipmentAudit, auditCommand=auditCommand, cardPrStateChange=cardPrStateChange, cardFwVersion=cardFwVersion, auditTable=auditTable, equipmentError=equipmentError, cmRegistrationNotification=cmRegistrationNotification, equipmentPortGroup=equipmentPortGroup, auditDescription=auditDescription, diskVolumeTable=diskVolumeTable, cardTestTime=cardTestTime, cmResetReason=cmResetReason, cardFlashSize=cardFlashSize, cardPrState=cardPrState, systemGeneral=systemGeneral, equipmentShelfGroup=equipmentShelfGroup, auditLogOutput=auditLogOutput, cmPrimarySID=cmPrimarySID, cmUChannelID=cmUChannelID, remainInDiagMode=remainInDiagMode, cardSpareGroupMode=cardSpareGroupMode, PYSNMP_MODULE_ID=cadEquipmentMib, cadEquipmentMib=cadEquipmentMib, auditId=auditId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardSwVersion=cardSwVersion, cardTestIndex=cardTestIndex, cardCarrierHwVersion=cardCarrierHwVersion, equipmentAuditGroup=equipmentAuditGroup, auditTime=auditTime, cardTestEntry=cardTestEntry, cardCpuType=cardCpuType, cardCarrierFwVersion=cardCarrierFwVersion, portMinDsPower=portMinDsPower, consoleOutput=consoleOutput, portAdminState=portAdminState, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierSerialNum=cardCarrierSerialNum, cardNumPorts=cardNumPorts, downstreamPowerLoss=downstreamPowerLoss, fpgaErrorEventGroup=fpgaErrorEventGroup, portTxFlowControlDetected=portTxFlowControlDetected, cardTestResult=cardTestResult)
