...
PortDetectedMode, PicType, PrimaryState, UnknownState, PortType, CardType, DuplexStatus, AdminState, CardSubType, PortMode, SecondaryState, ShelfId, CardId, EqActionType, DiskVolumeUsageLevel, PortId, FirmwareSource, FlowControlMode = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "PicType", "PrimaryState", "UnknownState", "PortType", "CardType", "DuplexStatus", "AdminState", "CardSubType", "PortMode", "SecondaryState", "ShelfId", "CardId", "EqActionType", "DiskVolumeUsageLevel", "PortId", "FirmwareSource", "FlowControlMode")
...
RowStatus, MacAddress, DateAndTime, TruthValue, DisplayString, TimeStamp, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "MacAddress", "DateAndTime", "TruthValue", "DisplayString", "TimeStamp", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditLogOutput=auditLogOutput, errEvLoggingEnabled=errEvLoggingEnabled, systemClock=systemClock, portNumber=portNumber, cardNumber=cardNumber, diskDriveNumber=diskDriveNumber, cardTestName=cardTestName, cardDplxStatusChange=cardDplxStatusChange, TestResult=TestResult, portCardId=portCardId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeId=diskVolumeId, cardTestType=cardTestType, TestSchedule=TestSchedule, portMacIfIndex=portMacIfIndex, equipmentDiagGroup=equipmentDiagGroup, portId=portId, diskVolumeFileName=diskVolumeFileName, portDocsIfIndex=portDocsIfIndex, cmResetNotification=cmResetNotification, cardTestCommand=cardTestCommand, equipmentState=equipmentState, trapSeverity=trapSeverity, portMode=portMode, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeUsageNotification=diskVolumeUsageNotification, diskVolumeGroup=diskVolumeGroup, cardSpareGroupMode=cardSpareGroupMode, cardTestId=cardTestId, diskVolumeCardId=diskVolumeCardId, cardPicSerialNum=cardPicSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, auditFailedCount=auditFailedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeShelfId=diskVolumeShelfId, equipmentCardTestGroup=equipmentCardTestGroup, portPrStateChange=portPrStateChange, shelfName=shelfName, portDplxStatusChange=portDplxStatusChange, equipmentShelfGroup=equipmentShelfGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, portMacAddress=portMacAddress, cardSecState=cardSecState, cardPicDetected=cardPicDetected, cardLastBootSource=cardLastBootSource, cmResetStatus=cmResetStatus, cardDplxStatus=cardDplxStatus, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestSchedule=cardTestSchedule, cardPrState=cardPrState, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTrapInh=cardTrapInh, cardLastBootVersion=cardLastBootVersion, cardSubType=cardSubType, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portMinDsPower=portMinDsPower, portDetectedMode=portDetectedMode, TestCommand=TestCommand, cardTestResultDesc=cardTestResultDesc, TestId=TestId, cardTestResultNotification=cardTestResultNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditTime=auditTime, shelfSwVersion=shelfSwVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portType=portType, equipmentTraps=equipmentTraps, cardTemperature=cardTemperature, portCardSubType=portCardSubType, cmDevice=cmDevice, auditShelfId=auditShelfId, auditResult=auditResult, cardTempNoReportNotification=cardTempNoReportNotification, systemKey=systemKey, diskVolumeName=diskVolumeName, cardTestResult=cardTestResult, downstreamPowerLoss=downstreamPowerLoss, equipmentDiag=equipmentDiag, verboseLevel=verboseLevel, equipmentCardGroup=equipmentCardGroup, cardRamSize=cardRamSize, portEntry=portEntry, equipmentAuditGroup=equipmentAuditGroup, cardLastChangeTime=cardLastChangeTime, cardFwUpdateStatus=cardFwUpdateStatus, cardHwVersion=cardHwVersion, equipmentAudit=equipmentAudit, portLastChangeTime=portLastChangeTime, diskVolumeEntry=diskVolumeEntry, cardAdminState=cardAdminState, diskVolumeTrapInh=diskVolumeTrapInh, auditTotalPassedCount=auditTotalPassedCount, portDescription=portDescription, cardTestIndex=cardTestIndex, auditCardId=auditCardId, cadEquipmentMibGroup=cadEquipmentMibGroup, cardShelfId=cardShelfId, cmResetUpTime=cmResetUpTime, systemGeneralGroup=systemGeneralGroup, equipmentStateGroup=equipmentStateGroup, cardType=cardType, cardSpareGroupId=cardSpareGroupId, equipmentPortGroup=equipmentPortGroup, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogThrottle=auditLogThrottle, cardTempOverHeatNotification=cardTempOverHeatNotification, portSecStateChange=portSecStateChange, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardFwVersion=cardFwVersion, portSecState=portSecState, auditCommand=auditCommand, errEventId=errEventId, shelfObjects=shelfObjects, portDplxStatus=portDplxStatus, cardAction=cardAction, cmPrimarySID=cmPrimarySID, cardDetectedChange=cardDetectedChange, cardTemperatureHighWarn=cardTemperatureHighWarn, cardName=cardName, cardTestEntry=cardTestEntry, cardTemperatureHighError=cardTemperatureHighError, cardDetected=cardDetected, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMib=cadEquipmentMib, cardTable=cardTable, cardBootVersion=cardBootVersion, eqDiagConfig=eqDiagConfig, diskVolumeSize=diskVolumeSize, cardTestTable=cardTestTable, auditEntry=auditEntry, cardTestScheduleCommand=cardTestScheduleCommand, errEvLogLevel=errEvLogLevel, cardFpgaSource=cardFpgaSource, shelfNumber=shelfNumber, cardEntry=cardEntry, portMaxDsPower=portMaxDsPower, cardId=cardId, cardShelfTestIndex=cardShelfTestIndex, cardTestDescription=cardTestDescription, cmRegistrationNotification=cmRegistrationNotification, cardPrStateChange=cardPrStateChange, cardTestTime=cardTestTime, cardCpuType=cardCpuType, portAdminState=portAdminState, fpgaErrorEventTable=fpgaErrorEventTable, remainInDiagMode=remainInDiagMode, auditId=auditId, cmUChannelID=cmUChannelID, diskVolumeUsageLevel=diskVolumeUsageLevel, auditPassedCount=auditPassedCount, cardFlashSize=cardFlashSize, cardLastResetReason=cardLastResetReason, auditCycleCount=auditCycleCount, portNumChans=portNumChans, equipmentTbl=equipmentTbl, cmIpAddress=cmIpAddress, cardCpuSpeed=cardCpuSpeed, cmVendor=cmVendor, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diagTestId=diagTestId, portCurrDsPower=portCurrDsPower, portAction=portAction, cardUpTime=cardUpTime, TestTransactionId=TestTransactionId, diskVolumeDiskSize=diskVolumeDiskSize, cardSecStateChange=cardSecStateChange, auditStatus=auditStatus, errEvRowStatus=errEvRowStatus, portBgpId=portBgpId, cmMacAddress=cmMacAddress, cmResetInfo=cmResetInfo, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeNumber=diskVolumeNumber, portConnectorId=portConnectorId, cardSwVersion=cardSwVersion, auditDescription=auditDescription, cmResetReason=cmResetReason, systemGeneral=systemGeneral, cardBusSpeed=cardBusSpeed, consoleOutput=consoleOutput, portTxFlowControlMode=portTxFlowControlMode, auditTotalFailedCount=auditTotalFailedCount, diskVolumeDriveId=diskVolumeDriveId, portTxFlowControlDetected=portTxFlowControlDetected, TestType=TestType, portRxFlowControlMode=portRxFlowControlMode, cardTestTransId=cardTestTransId, cardSerialNum=cardSerialNum, portTable=portTable, portShelfId=portShelfId, auditAutoScheduling=auditAutoScheduling, cardCPLDVersion=cardCPLDVersion, cardNumPorts=cardNumPorts, equipmentError=equipmentError, portTrapInh=portTrapInh, auditTable=auditTable, TestScheduleCommand=TestScheduleCommand, portPrState=portPrState, cmResetClearNotification=cmResetClearNotification, auditName=auditName, cardPicHwVersion=cardPicHwVersion, diskVolumeTable=diskVolumeTable, trapCounter=trapCounter, cardSubDetected=cardSubDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold)
