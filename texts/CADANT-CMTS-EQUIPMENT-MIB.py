...
PrimaryState, FirmwareSource, SecondaryState, ShelfId, PortDetectedMode, DiskVolumeUsageLevel, PortType, EqActionType, AdminState, FlowControlMode, PortMode, DuplexStatus, UnknownState, CardType, PortId, PicType, CardId, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PrimaryState", "FirmwareSource", "SecondaryState", "ShelfId", "PortDetectedMode", "DiskVolumeUsageLevel", "PortType", "EqActionType", "AdminState", "FlowControlMode", "PortMode", "DuplexStatus", "UnknownState", "CardType", "PortId", "PicType", "CardId", "CardSubType")
...
TruthValue, DisplayString, DateAndTime, TextualConvention, TimeStamp, MacAddress, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "DisplayString", "DateAndTime", "TextualConvention", "TimeStamp", "MacAddress", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portType=portType, portTxFlowControlMode=portTxFlowControlMode, cardSpareGroupMode=cardSpareGroupMode, cardShelfId=cardShelfId, TestResult=TestResult, cardFlashSize=cardFlashSize, portBgpId=portBgpId, cardTestDescription=cardTestDescription, auditEntry=auditEntry, cardLastChangeTime=cardLastChangeTime, portNumChans=portNumChans, auditFailedCount=auditFailedCount, cardDetectedChange=cardDetectedChange, portAction=portAction, portMacIfIndex=portMacIfIndex, cardSecStateChange=cardSecStateChange, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentTbl=equipmentTbl, cardSpareGroupId=cardSpareGroupId, cmIpAddress=cmIpAddress, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portRxFlowControlMode=portRxFlowControlMode, cardTestTransId=cardTestTransId, portTrapInh=portTrapInh, cardLastResetReason=cardLastResetReason, portMaxDsPower=portMaxDsPower, portMode=portMode, cardTrapInh=cardTrapInh, PYSNMP_MODULE_ID=cadEquipmentMib, cardTemperatureHighWarn=cardTemperatureHighWarn, portMinDsPower=portMinDsPower, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestTime=cardTestTime, diskVolumeId=diskVolumeId, errEvLogLevel=errEvLogLevel, equipmentAuditGroup=equipmentAuditGroup, cardShelfTestIndex=cardShelfTestIndex, auditDescription=auditDescription, TestCommand=TestCommand, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentState=equipmentState, cardCarrierHwVersion=cardCarrierHwVersion, TestSchedule=TestSchedule, cardTestResultNotification=cardTestResultNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCpuSpeed=cardCpuSpeed, auditShelfId=auditShelfId, portCardSubType=portCardSubType, portRxFlowControlDetected=portRxFlowControlDetected, portDplxStatusChange=portDplxStatusChange, trapSeverity=trapSeverity, portTable=portTable, cardUpTime=cardUpTime, cmResetClearNotification=cmResetClearNotification, cardFpgaSource=cardFpgaSource, cardTable=cardTable, portNumber=portNumber, diskVolumeTrapInh=diskVolumeTrapInh, cardTestCommand=cardTestCommand, cardTestScheduleCommand=cardTestScheduleCommand, cmResetNotification=cmResetNotification, diskVolumeDriveId=diskVolumeDriveId, fpgaErrorEventGroup=fpgaErrorEventGroup, auditLogThrottle=auditLogThrottle, diskVolumeShelfId=diskVolumeShelfId, diskVolumeCardId=diskVolumeCardId, cardTestId=cardTestId, TestId=TestId, cardAdminState=cardAdminState, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPrState=cardPrState, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeName=diskVolumeName, cardTempNoReportNotification=cardTempNoReportNotification, auditPassedCount=auditPassedCount, portSecStateChange=portSecStateChange, auditStatus=auditStatus, portCurrDsPower=portCurrDsPower, equipmentDiagGroup=equipmentDiagGroup, cardDplxStatus=cardDplxStatus, portPrState=portPrState, portConnectorId=portConnectorId, cardTestTable=cardTestTable, portLastChangeTime=portLastChangeTime, portDetectedMode=portDetectedMode, equipmentShelfGroup=equipmentShelfGroup, equipmentStateGroup=equipmentStateGroup, cardFwUpdateStatus=cardFwUpdateStatus, shelfObjects=shelfObjects, auditCardId=auditCardId, cmDevice=cmDevice, portDplxStatus=portDplxStatus, remainInDiagMode=remainInDiagMode, cmRegistrationNotification=cmRegistrationNotification, auditTable=auditTable, cardCarrierSerialNum=cardCarrierSerialNum, portDocsIfIndex=portDocsIfIndex, diskVolumeEntry=diskVolumeEntry, cardType=cardType, diskVolumeTable=diskVolumeTable, auditLogOutput=auditLogOutput, cardLastBootVersion=cardLastBootVersion, auditCommand=auditCommand, equipmentCardGroup=equipmentCardGroup, auditAutoScheduling=auditAutoScheduling, cadEquipmentMib=cadEquipmentMib, shelfName=shelfName, TestTransactionId=TestTransactionId, cardTemperature=cardTemperature, cardHwVersion=cardHwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, cardBusSpeed=cardBusSpeed, cardTemperatureHighError=cardTemperatureHighError, portShelfId=portShelfId, portId=portId, downstreamPowerLoss=downstreamPowerLoss, equipmentTraps=equipmentTraps, cardCPLDVersion=cardCPLDVersion, systemGeneralGroup=systemGeneralGroup, errEvRowStatus=errEvRowStatus, equipmentCardTestGroup=equipmentCardTestGroup, diagTestId=diagTestId, cardBootVersion=cardBootVersion, cardTestResultDesc=cardTestResultDesc, errEvLoggingEnabled=errEvLoggingEnabled, auditCycleCount=auditCycleCount, TestScheduleCommand=TestScheduleCommand, auditTime=auditTime, cardDetected=cardDetected, auditResult=auditResult, cardTestIndex=cardTestIndex, trapCounter=trapCounter, cmPrimarySID=cmPrimarySID, equipmentError=equipmentError, portCardId=portCardId, cardEntry=cardEntry, cardSubDetected=cardSubDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardAction=cardAction, diskVolumeNumber=diskVolumeNumber, cardSwVersion=cardSwVersion, cmResetStatus=cmResetStatus, portSecState=portSecState, cardFwVersion=cardFwVersion, consoleOutput=consoleOutput, cardCpuType=cardCpuType, equipmentDiag=equipmentDiag, cardId=cardId, auditId=auditId, cmResetUpTime=cmResetUpTime, cardLastBootSource=cardLastBootSource, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, equipmentAudit=equipmentAudit, fpgaErrorEventTable=fpgaErrorEventTable, portPrStateChange=portPrStateChange, cardTestType=cardTestType, portDescription=portDescription, systemKey=systemKey, shelfSwVersion=shelfSwVersion, cmResetReason=cmResetReason, eqDiagConfig=eqDiagConfig, cardTestSchedule=cardTestSchedule, auditName=auditName, portEntry=portEntry, TestType=TestType, cardTestEntry=cardTestEntry, diskVolumeSize=diskVolumeSize, cardNumber=cardNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, cardPicHwVersion=cardPicHwVersion, cardSecState=cardSecState, cardCarrierFwVersion=cardCarrierFwVersion, cmResetInfo=cmResetInfo, cardTestResult=cardTestResult, cardPrStateChange=cardPrStateChange, portAdminState=portAdminState, cardSubType=cardSubType, cardTempOverHeatNotification=cardTempOverHeatNotification, cardPicSerialNum=cardPicSerialNum, diskDriveNumber=diskDriveNumber, portMacAddress=portMacAddress, cardRamSize=cardRamSize, cardSerialNum=cardSerialNum, cardDplxStatusChange=cardDplxStatusChange, shelfNumber=shelfNumber, auditTotalPassedCount=auditTotalPassedCount, cmUChannelID=cmUChannelID, cardTestName=cardTestName, cadEquipmentMibCompliances=cadEquipmentMibCompliances, systemClock=systemClock, diskVolumeGroup=diskVolumeGroup, diskVolumeDiskSize=diskVolumeDiskSize, diskVolumeUsageNotification=diskVolumeUsageNotification, errEventId=errEventId, cadEquipmentMibGroup=cadEquipmentMibGroup, verboseLevel=verboseLevel, cmMacAddress=cmMacAddress, cardPicDetected=cardPicDetected, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmVendor=cmVendor, equipmentPortGroup=equipmentPortGroup, systemGeneral=systemGeneral, auditTotalFailedCount=auditTotalFailedCount, cardNumPorts=cardNumPorts, diskVolumeFileName=diskVolumeFileName, cardName=cardName, diskVolumeUsageLevel=diskVolumeUsageLevel)
