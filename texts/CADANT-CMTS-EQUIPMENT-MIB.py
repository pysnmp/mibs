...
CardSubType, FirmwareSource, PortId, PicType, PortType, UnknownState, PortDetectedMode, DiskVolumeUsageLevel, PrimaryState, CardId, ShelfId, SecondaryState, FlowControlMode, AdminState, CardType, DuplexStatus, EqActionType, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "FirmwareSource", "PortId", "PicType", "PortType", "UnknownState", "PortDetectedMode", "DiskVolumeUsageLevel", "PrimaryState", "CardId", "ShelfId", "SecondaryState", "FlowControlMode", "AdminState", "CardType", "DuplexStatus", "EqActionType", "PortMode")
...
DisplayString, TextualConvention, MacAddress, DateAndTime, TimeStamp, TruthValue, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TextualConvention", "MacAddress", "DateAndTime", "TimeStamp", "TruthValue", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portDescription=portDescription, cardDetectedChange=cardDetectedChange, auditTime=auditTime, TestSchedule=TestSchedule, errEventId=errEventId, portDplxStatusChange=portDplxStatusChange, cardTestEntry=cardTestEntry, cmResetInfo=cmResetInfo, cardSecStateChange=cardSecStateChange, portType=portType, cardDetected=cardDetected, cardLastResetReason=cardLastResetReason, portCardId=portCardId, cardTestDescription=cardTestDescription, cardTestTime=cardTestTime, portNumber=portNumber, fpgaErrorEventGroup=fpgaErrorEventGroup, equipmentAuditGroup=equipmentAuditGroup, cardTable=cardTable, portCurrDsPower=portCurrDsPower, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, diskVolumeId=diskVolumeId, cardTestName=cardTestName, cmVendor=cmVendor, cardTrapInh=cardTrapInh, portPrState=portPrState, portSecState=portSecState, diskVolumeTable=diskVolumeTable, cardTemperature=cardTemperature, portAction=portAction, cardFwUpdateStatus=cardFwUpdateStatus, shelfNumber=shelfNumber, trapSeverity=trapSeverity, equipmentStateGroup=equipmentStateGroup, cardSubType=cardSubType, cardPrState=cardPrState, portPrStateChange=portPrStateChange, cardTestCommand=cardTestCommand, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestSchedule=cardTestSchedule, cardCpuSpeed=cardCpuSpeed, cardRamSize=cardRamSize, cardName=cardName, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardId=cardId, diagTestId=diagTestId, auditFailedCount=auditFailedCount, cardPicDetected=cardPicDetected, auditStatus=auditStatus, portMaxDsPower=portMaxDsPower, systemClock=systemClock, TestScheduleCommand=TestScheduleCommand, diskVolumeCardId=diskVolumeCardId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeSize=diskVolumeSize, portCardSubType=portCardSubType, errEvLogLevel=errEvLogLevel, auditShelfId=auditShelfId, fpgaErrorEventEntry=fpgaErrorEventEntry, cardEntry=cardEntry, cardTestIndex=cardTestIndex, auditTotalPassedCount=auditTotalPassedCount, fpgaErrorEventTable=fpgaErrorEventTable, portRxFlowControlMode=portRxFlowControlMode, cardSubDetected=cardSubDetected, auditEntry=auditEntry, portTxFlowControlDetected=portTxFlowControlDetected, auditCommand=auditCommand, cardTestResultDesc=cardTestResultDesc, cmIpAddress=cmIpAddress, auditCardId=auditCardId, portMacIfIndex=portMacIfIndex, TestId=TestId, auditName=auditName, cmResetClearNotification=cmResetClearNotification, cardTestResult=cardTestResult, equipmentCardTestGroup=equipmentCardTestGroup, cardNumber=cardNumber, auditCycleCount=auditCycleCount, equipmentTbl=equipmentTbl, portEntry=portEntry, errEvRowStatus=errEvRowStatus, cardTestType=cardTestType, cmResetNotification=cmResetNotification, verboseLevel=verboseLevel, downstreamPowerLoss=downstreamPowerLoss, portConnectorId=portConnectorId, equipmentCardGroup=equipmentCardGroup, equipmentError=equipmentError, TestTransactionId=TestTransactionId, portDocsIfIndex=portDocsIfIndex, cardTestTable=cardTestTable, portDplxStatus=portDplxStatus, portShelfId=portShelfId, auditLogThrottle=auditLogThrottle, shelfName=shelfName, cardTestTransId=cardTestTransId, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecStateChange=portSecStateChange, TestCommand=TestCommand, systemKey=systemKey, equipmentState=equipmentState, cardBusSpeed=cardBusSpeed, cardCarrierSerialNum=cardCarrierSerialNum, cardSwVersion=cardSwVersion, portMinDsPower=portMinDsPower, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmUChannelID=cmUChannelID, equipmentDiagGroup=equipmentDiagGroup, portLastChangeTime=portLastChangeTime, cardDplxStatus=cardDplxStatus, cardAdminState=cardAdminState, TestType=TestType, cardTempNoReportNotification=cardTempNoReportNotification, systemGeneral=systemGeneral, equipmentPortGroup=equipmentPortGroup, cardDplxStatusChange=cardDplxStatusChange, auditPassedCount=auditPassedCount, cardSerialNum=cardSerialNum, portBgpId=portBgpId, diskVolumeEntry=diskVolumeEntry, cardTestId=cardTestId, shelfObjects=shelfObjects, cmResetReason=cmResetReason, cardLastBootVersion=cardLastBootVersion, cardSecState=cardSecState, diskVolumeShelfId=diskVolumeShelfId, portRxFlowControlDetected=portRxFlowControlDetected, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCpuType=cardCpuType, cardUpTime=cardUpTime, cardCarrierHwVersion=cardCarrierHwVersion, cardSpareGroupId=cardSpareGroupId, cardShelfTestIndex=cardShelfTestIndex, diskVolumeDiskSize=diskVolumeDiskSize, PYSNMP_MODULE_ID=cadEquipmentMib, portTrapInh=portTrapInh, TestResult=TestResult, diskVolumeGroup=diskVolumeGroup, portMode=portMode, diskVolumeFileName=diskVolumeFileName, cardFwVersion=cardFwVersion, equipmentTraps=equipmentTraps, cardType=cardType, portId=portId, portMacAddress=portMacAddress, portTxFlowControlMode=portTxFlowControlMode, shelfSwVersion=shelfSwVersion, cmMacAddress=cmMacAddress, cardFlashSize=cardFlashSize, equipmentShelfGroup=equipmentShelfGroup, cardTestResultNotification=cardTestResultNotification, trapCounter=trapCounter, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmRegistrationNotification=cmRegistrationNotification, auditDescription=auditDescription, cadEquipmentMibConformance=cadEquipmentMibConformance, cardHwVersion=cardHwVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardPrStateChange=cardPrStateChange, cardPicSerialNum=cardPicSerialNum, portAdminState=portAdminState, diskVolumeUsageNotification=diskVolumeUsageNotification, cmResetUpTime=cmResetUpTime, auditId=auditId, portDetectedMode=portDetectedMode, auditTotalFailedCount=auditTotalFailedCount, systemGeneralGroup=systemGeneralGroup, diskVolumeNumber=diskVolumeNumber, diskVolumeTrapInh=diskVolumeTrapInh, cardFpgaSource=cardFpgaSource, auditTable=auditTable, equipmentDiag=equipmentDiag, diskVolumeName=diskVolumeName, cardShelfId=cardShelfId, cardTemperatureHighError=cardTemperatureHighError, diskDriveNumber=diskDriveNumber, errEvLoggingEnabled=errEvLoggingEnabled, equipmentAudit=equipmentAudit, cardNumPorts=cardNumPorts, auditResult=auditResult, cardLastChangeTime=cardLastChangeTime, cardTestScheduleCommand=cardTestScheduleCommand, auditLogOutput=auditLogOutput, diskVolumeUsageLevel=diskVolumeUsageLevel, eqDiagConfig=eqDiagConfig, cardLastBootSource=cardLastBootSource, cadEquipmentMib=cadEquipmentMib, diskVolumeDriveId=diskVolumeDriveId, cardCPLDVersion=cardCPLDVersion, cardBootVersion=cardBootVersion, cardTempOverHeatNotification=cardTempOverHeatNotification, cmResetStatus=cmResetStatus, cardTemperatureHighWarn=cardTemperatureHighWarn, remainInDiagMode=remainInDiagMode, cardPicHwVersion=cardPicHwVersion, portNumChans=portNumChans, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardAction=cardAction, cmPrimarySID=cmPrimarySID, auditAutoScheduling=auditAutoScheduling, portTable=portTable, cmDevice=cmDevice, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSpareGroupMode=cardSpareGroupMode, consoleOutput=consoleOutput, cardCarrierFwVersion=cardCarrierFwVersion)
