...
PortId, SecondaryState, PortMode, FirmwareSource, PortDetectedMode, AdminState, CardType, EqActionType, CardSubType, UnknownState, DuplexStatus, FlowControlMode, PrimaryState, ShelfId, CardId, PicType, DiskVolumeUsageLevel, PortType = mibBuilder.importSymbols("CADANT-TC", "PortId", "SecondaryState", "PortMode", "FirmwareSource", "PortDetectedMode", "AdminState", "CardType", "EqActionType", "CardSubType", "UnknownState", "DuplexStatus", "FlowControlMode", "PrimaryState", "ShelfId", "CardId", "PicType", "DiskVolumeUsageLevel", "PortType")
...
MacAddress, TextualConvention, RowStatus, DisplayString, DateAndTime, TimeStamp, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "MacAddress", "TextualConvention", "RowStatus", "DisplayString", "DateAndTime", "TimeStamp", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardBusSpeed=cardBusSpeed, PYSNMP_MODULE_ID=cadEquipmentMib, cmIpAddress=cmIpAddress, cmResetClearNotification=cmResetClearNotification, cardId=cardId, cardFwVersion=cardFwVersion, auditTime=auditTime, cardLastBootSource=cardLastBootSource, TestType=TestType, cardSerialNum=cardSerialNum, auditTotalPassedCount=auditTotalPassedCount, cardTemperature=cardTemperature, cardTestTransId=cardTestTransId, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditLogOutput=auditLogOutput, portPrState=portPrState, portLastChangeTime=portLastChangeTime, cardBootVersion=cardBootVersion, cardTestResult=cardTestResult, cmResetStatus=cmResetStatus, cardNumber=cardNumber, cardTemperatureHighError=cardTemperatureHighError, portMacAddress=portMacAddress, cardLastBootVersion=cardLastBootVersion, shelfName=shelfName, portDocsIfIndex=portDocsIfIndex, cardRamSize=cardRamSize, portTable=portTable, diskVolumeCardId=diskVolumeCardId, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardSecState=cardSecState, portTxFlowControlDetected=portTxFlowControlDetected, cardUpTime=cardUpTime, portRxFlowControlDetected=portRxFlowControlDetected, auditDescription=auditDescription, portEntry=portEntry, cardTestId=cardTestId, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, fpgaErrorEventEntry=fpgaErrorEventEntry, cardEntry=cardEntry, equipmentCardTestGroup=equipmentCardTestGroup, cmResetUpTime=cmResetUpTime, portAction=portAction, TestResult=TestResult, cardDetected=cardDetected, diskVolumeEntry=diskVolumeEntry, cmVendor=cmVendor, cardFwUpdateStatus=cardFwUpdateStatus, cardTestResultDesc=cardTestResultDesc, auditCommand=auditCommand, portDetectedMode=portDetectedMode, diskVolumeShelfId=diskVolumeShelfId, cardFlashSize=cardFlashSize, cardDplxStatusChange=cardDplxStatusChange, shelfNumber=shelfNumber, cardShelfId=cardShelfId, equipmentError=equipmentError, equipmentDiagGroup=equipmentDiagGroup, auditShelfId=auditShelfId, cmResetReason=cmResetReason, portTrapInh=portTrapInh, errEvRowStatus=errEvRowStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, cardSpareGroupMode=cardSpareGroupMode, portAdminState=portAdminState, cardTestEntry=cardTestEntry, cardSpareGroupId=cardSpareGroupId, equipmentState=equipmentState, TestCommand=TestCommand, portType=portType, diskVolumeDriveId=diskVolumeDriveId, cardTestDescription=cardTestDescription, auditTable=auditTable, portBgpId=portBgpId, portCurrDsPower=portCurrDsPower, cardTestName=cardTestName, TestSchedule=TestSchedule, cardPicHwVersion=cardPicHwVersion, cardPrStateChange=cardPrStateChange, cardAdminState=cardAdminState, portPrStateChange=portPrStateChange, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentDiag=equipmentDiag, shelfObjects=shelfObjects, cardCarrierSerialNum=cardCarrierSerialNum, consoleOutput=consoleOutput, auditCardId=auditCardId, auditStatus=auditStatus, portSecStateChange=portSecStateChange, cardTestTable=cardTestTable, equipmentTbl=equipmentTbl, portShelfId=portShelfId, TestId=TestId, cmPrimarySID=cmPrimarySID, diskVolumeFileName=diskVolumeFileName, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageLevel=diskVolumeUsageLevel, cadEquipmentMibCompliances=cadEquipmentMibCompliances, eqDiagConfig=eqDiagConfig, diskVolumeNumber=diskVolumeNumber, shelfSwVersion=shelfSwVersion, cmMacAddress=cmMacAddress, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portNumber=portNumber, cardSwVersion=cardSwVersion, diskVolumeTrapInh=diskVolumeTrapInh, auditLogThrottle=auditLogThrottle, systemGeneral=systemGeneral, cardCpuType=cardCpuType, portMacIfIndex=portMacIfIndex, cardDetectedChange=cardDetectedChange, portNumChans=portNumChans, cardLastResetReason=cardLastResetReason, cardTable=cardTable, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvLogLevel=errEvLogLevel, cardType=cardType, diskVolumeName=diskVolumeName, cardCarrierFwVersion=cardCarrierFwVersion, cardCPLDVersion=cardCPLDVersion, remainInDiagMode=remainInDiagMode, portMaxDsPower=portMaxDsPower, downstreamPowerLoss=downstreamPowerLoss, cardShelfTestIndex=cardShelfTestIndex, portRxFlowControlMode=portRxFlowControlMode, auditId=auditId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetInfo=cmResetInfo, cardTrapInh=cardTrapInh, systemGeneralGroup=systemGeneralGroup, diskVolumeDiskSize=diskVolumeDiskSize, cardNumPorts=cardNumPorts, cardTestIndex=cardTestIndex, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentCardGroup=equipmentCardGroup, auditName=auditName, cardTempOverHeatNotification=cardTempOverHeatNotification, cardHwVersion=cardHwVersion, cardPicDetected=cardPicDetected, diskDriveNumber=diskDriveNumber, TestScheduleCommand=TestScheduleCommand, portId=portId, diskVolumeTable=diskVolumeTable, cardPicSerialNum=cardPicSerialNum, diskVolumeSize=diskVolumeSize, auditCycleCount=auditCycleCount, portCardId=portCardId, cardFpgaSource=cardFpgaSource, portSecState=portSecState, portDescription=portDescription, cmRegistrationNotification=cmRegistrationNotification, cardName=cardName, cardLastChangeTime=cardLastChangeTime, equipmentAudit=equipmentAudit, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeId=diskVolumeId, diagTestId=diagTestId, cardTempNoReportNotification=cardTempNoReportNotification, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestTime=cardTestTime, auditFailedCount=auditFailedCount, cmUChannelID=cmUChannelID, cardSubDetected=cardSubDetected, cardAction=cardAction, diskVolumeGroup=diskVolumeGroup, portMode=portMode, systemClock=systemClock, portConnectorId=portConnectorId, cardTestScheduleCommand=cardTestScheduleCommand, portMinDsPower=portMinDsPower, auditPassedCount=auditPassedCount, portCardSubType=portCardSubType, cardSecStateChange=cardSecStateChange, auditResult=auditResult, auditEntry=auditEntry, portDplxStatus=portDplxStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, errEventId=errEventId, cadEquipmentMib=cadEquipmentMib, cardTestSchedule=cardTestSchedule, verboseLevel=verboseLevel, cardTestResultNotification=cardTestResultNotification, equipmentAuditGroup=equipmentAuditGroup, cardTestType=cardTestType, equipmentPortGroup=equipmentPortGroup, equipmentShelfGroup=equipmentShelfGroup, equipmentTraps=equipmentTraps, fpgaErrorEventTable=fpgaErrorEventTable, cardCarrierHwVersion=cardCarrierHwVersion, errEvLoggingEnabled=errEvLoggingEnabled, auditTotalFailedCount=auditTotalFailedCount, auditAutoScheduling=auditAutoScheduling, cmResetNotification=cmResetNotification, cardDplxStatus=cardDplxStatus, TestTransactionId=TestTransactionId, trapSeverity=trapSeverity, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestCommand=cardTestCommand, cardPrState=cardPrState, systemKey=systemKey, cmDevice=cmDevice, cardSubType=cardSubType, portTxFlowControlMode=portTxFlowControlMode, trapCounter=trapCounter, cardCpuSpeed=cardCpuSpeed)
