...
SecondaryState, CardSubType, PicType, UnknownState, PrimaryState, FirmwareSource, CardType, DuplexStatus, PortDetectedMode, AdminState, PortId, ShelfId, CardId, PortMode, EqActionType, FlowControlMode, DiskVolumeUsageLevel, PortType = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "CardSubType", "PicType", "UnknownState", "PrimaryState", "FirmwareSource", "CardType", "DuplexStatus", "PortDetectedMode", "AdminState", "PortId", "ShelfId", "CardId", "PortMode", "EqActionType", "FlowControlMode", "DiskVolumeUsageLevel", "PortType")
...
TimeStamp, DisplayString, DateAndTime, MacAddress, TruthValue, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TimeStamp", "DisplayString", "DateAndTime", "MacAddress", "TruthValue", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTable=portTable, shelfNumber=shelfNumber, cardTempNoReportNotification=cardTempNoReportNotification, cardNumber=cardNumber, equipmentAudit=equipmentAudit, cardTestType=cardTestType, auditLogOutput=auditLogOutput, cardNumPorts=cardNumPorts, equipmentTbl=equipmentTbl, cardPicHwVersion=cardPicHwVersion, equipmentShelfGroup=equipmentShelfGroup, equipmentCardTestGroup=equipmentCardTestGroup, cardShelfTestIndex=cardShelfTestIndex, shelfSwVersion=shelfSwVersion, cardTestIndex=cardTestIndex, errEventId=errEventId, cardPicDetected=cardPicDetected, cardCpuSpeed=cardCpuSpeed, diskVolumeDiskSize=diskVolumeDiskSize, auditStatus=auditStatus, cmUChannelID=cmUChannelID, diskVolumeFileName=diskVolumeFileName, cardTestTime=cardTestTime, auditTotalFailedCount=auditTotalFailedCount, cardDetectedChange=cardDetectedChange, cardTemperatureHighWarn=cardTemperatureHighWarn, auditName=auditName, auditTotalPassedCount=auditTotalPassedCount, portMode=portMode, cardTestCommand=cardTestCommand, cmResetStatus=cmResetStatus, systemClock=systemClock, cardTestDescription=cardTestDescription, cardSerialNum=cardSerialNum, cardBootVersion=cardBootVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDplxStatusChange=portDplxStatusChange, verboseLevel=verboseLevel, auditFailedCount=auditFailedCount, cardType=cardType, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeTrapInh=diskVolumeTrapInh, cardTestTransId=cardTestTransId, equipmentTraps=equipmentTraps, trapCounter=trapCounter, diskVolumeNumber=diskVolumeNumber, cardTestTable=cardTestTable, portAdminState=portAdminState, cardPrStateChange=cardPrStateChange, cardTable=cardTable, trapSeverity=trapSeverity, fpgaErrorEventTable=fpgaErrorEventTable, cmResetClearNotification=cmResetClearNotification, portMinDsPower=portMinDsPower, diskVolumeDriveId=diskVolumeDriveId, shelfName=shelfName, cardFpgaSource=cardFpgaSource, portType=portType, errEvRecoveryEnabled=errEvRecoveryEnabled, portMacIfIndex=portMacIfIndex, diskVolumeUsageNotification=diskVolumeUsageNotification, portSecState=portSecState, TestScheduleCommand=TestScheduleCommand, cardEntry=cardEntry, cardLastBootVersion=cardLastBootVersion, cmResetReason=cmResetReason, cardDplxStatus=cardDplxStatus, cardSubType=cardSubType, cardLastBootSource=cardLastBootSource, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portTxFlowControlMode=portTxFlowControlMode, cmResetNotification=cmResetNotification, cardSpareGroupMode=cardSpareGroupMode, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentState=equipmentState, diskVolumeSize=diskVolumeSize, auditAutoScheduling=auditAutoScheduling, auditId=auditId, errEvLogLevel=errEvLogLevel, shelfObjects=shelfObjects, cardPrState=cardPrState, portMacAddress=portMacAddress, systemKey=systemKey, TestId=TestId, cardFlashSize=cardFlashSize, cardTestResultNotification=cardTestResultNotification, portCardSubType=portCardSubType, equipmentDiag=equipmentDiag, diskVolumeGroup=diskVolumeGroup, cardDplxStatusChange=cardDplxStatusChange, cmRegistrationNotification=cmRegistrationNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentStateGroup=equipmentStateGroup, cardTestScheduleCommand=cardTestScheduleCommand, downstreamPowerLoss=downstreamPowerLoss, portConnectorId=portConnectorId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardUpTime=cardUpTime, auditCommand=auditCommand, cardTrapInh=cardTrapInh, cardTestEntry=cardTestEntry, cardLastChangeTime=cardLastChangeTime, portMaxDsPower=portMaxDsPower, portAction=portAction, cardFwVersion=cardFwVersion, auditPassedCount=auditPassedCount, cadEquipmentMibConformance=cadEquipmentMibConformance, portBgpId=portBgpId, cmResetUpTime=cmResetUpTime, cardTemperatureHighError=cardTemperatureHighError, portNumber=portNumber, cardSpareGroupId=cardSpareGroupId, portEntry=portEntry, PYSNMP_MODULE_ID=cadEquipmentMib, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cmPrimarySID=cmPrimarySID, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardCarrierHwVersion=cardCarrierHwVersion, TestResult=TestResult, cardSwVersion=cardSwVersion, auditCycleCount=auditCycleCount, eqDiagConfig=eqDiagConfig, cardDetected=cardDetected, portTrapInh=portTrapInh, auditEntry=auditEntry, portDetectedMode=portDetectedMode, portRxFlowControlMode=portRxFlowControlMode, diskVolumeEntry=diskVolumeEntry, TestTransactionId=TestTransactionId, cardTestName=cardTestName, auditTable=auditTable, cmMacAddress=cmMacAddress, portPrState=portPrState, cardTestResult=cardTestResult, portCurrDsPower=portCurrDsPower, cardTemperature=cardTemperature, systemGeneral=systemGeneral, cardHwVersion=cardHwVersion, cardCarrierSerialNum=cardCarrierSerialNum, auditCardId=auditCardId, cadEquipmentMib=cadEquipmentMib, auditShelfId=auditShelfId, equipmentAuditGroup=equipmentAuditGroup, portDplxStatus=portDplxStatus, diskVolumeUsageLevel=diskVolumeUsageLevel, diskDriveNumber=diskDriveNumber, cardId=cardId, diskVolumeCardId=diskVolumeCardId, portNumChans=portNumChans, cardTestSchedule=cardTestSchedule, portShelfId=portShelfId, portDocsIfIndex=portDocsIfIndex, diskVolumeName=diskVolumeName, TestCommand=TestCommand, fpgaErrorEventGroup=fpgaErrorEventGroup, auditDescription=auditDescription, cardSecStateChange=cardSecStateChange, cmResetInfo=cmResetInfo, portPrStateChange=portPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeShelfId=diskVolumeShelfId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardCPLDVersion=cardCPLDVersion, cardLastResetReason=cardLastResetReason, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardBusSpeed=cardBusSpeed, cardSecState=cardSecState, portDescription=portDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, cardTestId=cardTestId, equipmentPortGroup=equipmentPortGroup, auditResult=auditResult, cardPicSerialNum=cardPicSerialNum, portCardId=portCardId, auditTime=auditTime, portLastChangeTime=portLastChangeTime, cardName=cardName, errEvLoggingEnabled=errEvLoggingEnabled, TestSchedule=TestSchedule, portSecStateChange=portSecStateChange, diskVolumeId=diskVolumeId, diagTestId=diagTestId, equipmentDiagGroup=equipmentDiagGroup, portId=portId, cardAdminState=cardAdminState, consoleOutput=consoleOutput, equipmentError=equipmentError, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardTestResultDesc=cardTestResultDesc, diskVolumeTable=diskVolumeTable, cmDevice=cmDevice, TestType=TestType, cardFwUpdateStatus=cardFwUpdateStatus, cardCarrierFwVersion=cardCarrierFwVersion, systemGeneralGroup=systemGeneralGroup, cardShelfId=cardShelfId, cardSubDetected=cardSubDetected, portRxFlowControlDetected=portRxFlowControlDetected, remainInDiagMode=remainInDiagMode, cmIpAddress=cmIpAddress, cmVendor=cmVendor, auditLogThrottle=auditLogThrottle, cardAction=cardAction, cardCpuType=cardCpuType, cardRamSize=cardRamSize, errEvRowStatus=errEvRowStatus)
