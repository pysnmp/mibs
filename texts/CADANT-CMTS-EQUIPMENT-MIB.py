...
DuplexStatus, PortDetectedMode, PortId, CardSubType, EqActionType, PicType, SecondaryState, AdminState, FirmwareSource, PortType, UnknownState, FlowControlMode, DiskVolumeUsageLevel, PrimaryState, CardId, ShelfId, PortMode, CardType = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortDetectedMode", "PortId", "CardSubType", "EqActionType", "PicType", "SecondaryState", "AdminState", "FirmwareSource", "PortType", "UnknownState", "FlowControlMode", "DiskVolumeUsageLevel", "PrimaryState", "CardId", "ShelfId", "PortMode", "CardType")
...
DisplayString, MacAddress, TruthValue, DateAndTime, TextualConvention, RowStatus, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "MacAddress", "TruthValue", "DateAndTime", "TextualConvention", "RowStatus", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentTraps=equipmentTraps, diskVolumeDiskSize=diskVolumeDiskSize, cardFwUpdateStatus=cardFwUpdateStatus, cardSerialNum=cardSerialNum, cardTestResultDesc=cardTestResultDesc, cardPrState=cardPrState, cardSpareGroupId=cardSpareGroupId, consoleOutput=consoleOutput, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDplxStatus=portDplxStatus, portDescription=portDescription, verboseLevel=verboseLevel, portRxFlowControlMode=portRxFlowControlMode, auditStatus=auditStatus, equipmentCardTestGroup=equipmentCardTestGroup, auditDescription=auditDescription, cmResetInfo=cmResetInfo, equipmentError=equipmentError, equipmentTbl=equipmentTbl, cardFwVersion=cardFwVersion, cardLastResetReason=cardLastResetReason, cardTemperature=cardTemperature, portCardId=portCardId, portAction=portAction, TestCommand=TestCommand, diskVolumeEntry=diskVolumeEntry, portTxFlowControlDetected=portTxFlowControlDetected, cardSpareGroupMode=cardSpareGroupMode, fpgaErrorEventTable=fpgaErrorEventTable, portAdminState=portAdminState, cardBootVersion=cardBootVersion, auditCommand=auditCommand, auditTime=auditTime, auditId=auditId, diskVolumeTable=diskVolumeTable, trapSeverity=trapSeverity, cardTestCommand=cardTestCommand, shelfNumber=shelfNumber, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, auditTable=auditTable, diagTestId=diagTestId, cardLastBootVersion=cardLastBootVersion, portCardSubType=portCardSubType, diskVolumeSize=diskVolumeSize, fpgaErrorEventEntry=fpgaErrorEventEntry, portShelfId=portShelfId, diskVolumeNumber=diskVolumeNumber, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentDiagGroup=equipmentDiagGroup, cmIpAddress=cmIpAddress, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portPrStateChange=portPrStateChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestDescription=cardTestDescription, diskVolumeUsageLevel=diskVolumeUsageLevel, auditAutoScheduling=auditAutoScheduling, cardFlashSize=cardFlashSize, cardSubDetected=cardSubDetected, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cadEquipmentMib=cadEquipmentMib, diskVolumeUsageNotification=diskVolumeUsageNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, cardDetectedChange=cardDetectedChange, errEventId=errEventId, cardNumber=cardNumber, portSecState=portSecState, cmResetStatus=cmResetStatus, equipmentStateGroup=equipmentStateGroup, errEvLogLevel=errEvLogLevel, portDocsIfIndex=portDocsIfIndex, cardAction=cardAction, auditCardId=auditCardId, equipmentCardGroup=equipmentCardGroup, shelfName=shelfName, cardAdminState=cardAdminState, cardPicHwVersion=cardPicHwVersion, cardId=cardId, cardCpuSpeed=cardCpuSpeed, TestResult=TestResult, cardTemperatureHighWarn=cardTemperatureHighWarn, portMaxDsPower=portMaxDsPower, cardTestEntry=cardTestEntry, cmMacAddress=cmMacAddress, diskVolumeGroup=diskVolumeGroup, cardCarrierFwVersion=cardCarrierFwVersion, cmDevice=cmDevice, cardPicDetected=cardPicDetected, cardTrapInh=cardTrapInh, cmResetUpTime=cmResetUpTime, equipmentAudit=equipmentAudit, auditName=auditName, errEvRowStatus=errEvRowStatus, diskVolumeShelfId=diskVolumeShelfId, errEvLoggingEnabled=errEvLoggingEnabled, cardTempOverHeatNotification=cardTempOverHeatNotification, portPrState=portPrState, cmResetReason=cmResetReason, cardCpuType=cardCpuType, cardTestIndex=cardTestIndex, cardTemperatureHighError=cardTemperatureHighError, auditPassedCount=auditPassedCount, portTrapInh=portTrapInh, cardName=cardName, shelfSwVersion=shelfSwVersion, portNumber=portNumber, portEntry=portEntry, shelfObjects=shelfObjects, cardTestName=cardTestName, portType=portType, auditLogThrottle=auditLogThrottle, auditEntry=auditEntry, auditCycleCount=auditCycleCount, portMacIfIndex=portMacIfIndex, systemKey=systemKey, diskVolumeCardId=diskVolumeCardId, cmResetClearNotification=cmResetClearNotification, cardCPLDVersion=cardCPLDVersion, cardTestTime=cardTestTime, TestScheduleCommand=TestScheduleCommand, cardTestSchedule=cardTestSchedule, portBgpId=portBgpId, diskVolumeName=diskVolumeName, portTable=portTable, trapCounter=trapCounter, cmRegistrationNotification=cmRegistrationNotification, systemClock=systemClock, cardSwVersion=cardSwVersion, cardPicSerialNum=cardPicSerialNum, cardDplxStatus=cardDplxStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, cadEquipmentMibConformance=cadEquipmentMibConformance, auditResult=auditResult, cardUpTime=cardUpTime, auditShelfId=auditShelfId, equipmentAuditGroup=equipmentAuditGroup, systemGeneral=systemGeneral, cardHwVersion=cardHwVersion, remainInDiagMode=remainInDiagMode, cardTestId=cardTestId, cmResetNotification=cmResetNotification, auditTotalFailedCount=auditTotalFailedCount, portConnectorId=portConnectorId, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeDriveId=diskVolumeDriveId, cardSecStateChange=cardSecStateChange, portSecStateChange=portSecStateChange, portId=portId, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portDplxStatusChange=portDplxStatusChange, cardShelfId=cardShelfId, portMode=portMode, TestTransactionId=TestTransactionId, equipmentState=equipmentState, cardTestResultNotification=cardTestResultNotification, portNumChans=portNumChans, cardDplxStatusChange=cardDplxStatusChange, cardSecState=cardSecState, diskDriveNumber=diskDriveNumber, cardTestType=cardTestType, cardFpgaSource=cardFpgaSource, diskVolumeId=diskVolumeId, cardLastChangeTime=cardLastChangeTime, auditLogOutput=auditLogOutput, equipmentPortGroup=equipmentPortGroup, portTxFlowControlMode=portTxFlowControlMode, downstreamPowerLoss=downstreamPowerLoss, cardShelfTestIndex=cardShelfTestIndex, systemGeneralGroup=systemGeneralGroup, cardRamSize=cardRamSize, cardEntry=cardEntry, portLastChangeTime=portLastChangeTime, cardNumPorts=cardNumPorts, cardPrStateChange=cardPrStateChange, equipmentShelfGroup=equipmentShelfGroup, eqDiagConfig=eqDiagConfig, cmVendor=cmVendor, cardType=cardType, auditFailedCount=auditFailedCount, cardDetected=cardDetected, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentDiag=equipmentDiag, TestType=TestType, portDetectedMode=portDetectedMode, diskVolumeTrapInh=diskVolumeTrapInh, cardCarrierSerialNum=cardCarrierSerialNum, TestSchedule=TestSchedule, portMacAddress=portMacAddress, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, diskVolumeFileName=diskVolumeFileName, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestTable=cardTestTable, cardTestTransId=cardTestTransId, cardTestResult=cardTestResult, cmUChannelID=cmUChannelID, portCurrDsPower=portCurrDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTable=cardTable, cardBusSpeed=cardBusSpeed, cardCarrierHwVersion=cardCarrierHwVersion, TestId=TestId, auditTotalPassedCount=auditTotalPassedCount, portMinDsPower=portMinDsPower, cmPrimarySID=cmPrimarySID, cardSubType=cardSubType, cardLastBootSource=cardLastBootSource)
