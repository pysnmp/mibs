...
FlowControlMode, DuplexStatus, PortDetectedMode, FirmwareSource, ShelfId, PortType, PortId, AdminState, CardType, UnknownState, DiskVolumeUsageLevel, CardSubType, EqActionType, PortMode, PrimaryState, CardId, SecondaryState, PicType = mibBuilder.importSymbols("CADANT-TC", "FlowControlMode", "DuplexStatus", "PortDetectedMode", "FirmwareSource", "ShelfId", "PortType", "PortId", "AdminState", "CardType", "UnknownState", "DiskVolumeUsageLevel", "CardSubType", "EqActionType", "PortMode", "PrimaryState", "CardId", "SecondaryState", "PicType")
...
RowStatus, TextualConvention, MacAddress, DateAndTime, TruthValue, DisplayString, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "MacAddress", "DateAndTime", "TruthValue", "DisplayString", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portMaxDsPower=portMaxDsPower, cardLastBootVersion=cardLastBootVersion, cardLastChangeTime=cardLastChangeTime, cardTestCommand=cardTestCommand, cardLastResetReason=cardLastResetReason, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portAdminState=portAdminState, portMinDsPower=portMinDsPower, diskVolumeId=diskVolumeId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCarrierFwVersion=cardCarrierFwVersion, auditAutoScheduling=auditAutoScheduling, portPrState=portPrState, cardTestResult=cardTestResult, shelfSwVersion=shelfSwVersion, cardDplxStatus=cardDplxStatus, cardTrapInh=cardTrapInh, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portTable=portTable, diskVolumeEntry=diskVolumeEntry, diskVolumeName=diskVolumeName, cardTestResultDesc=cardTestResultDesc, cardBusSpeed=cardBusSpeed, auditId=auditId, cardNumPorts=cardNumPorts, consoleOutput=consoleOutput, portLastChangeTime=portLastChangeTime, cardHwVersion=cardHwVersion, equipmentTbl=equipmentTbl, cardPicHwVersion=cardPicHwVersion, TestScheduleCommand=TestScheduleCommand, cardId=cardId, cardBootVersion=cardBootVersion, cmResetStatus=cmResetStatus, cardPrStateChange=cardPrStateChange, trapSeverity=trapSeverity, portBgpId=portBgpId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardFwVersion=cardFwVersion, portConnectorId=portConnectorId, auditLogOutput=auditLogOutput, TestTransactionId=TestTransactionId, auditTotalPassedCount=auditTotalPassedCount, auditTime=auditTime, diskDriveNumber=diskDriveNumber, cardTestTransId=cardTestTransId, fpgaErrorEventTable=fpgaErrorEventTable, equipmentStateGroup=equipmentStateGroup, diskVolumeTable=diskVolumeTable, equipmentError=equipmentError, cardAction=cardAction, cardCarrierHwVersion=cardCarrierHwVersion, shelfNumber=shelfNumber, cardSecState=cardSecState, portId=portId, cardCpuType=cardCpuType, diskVolumeTrapInh=diskVolumeTrapInh, cardDplxStatusChange=cardDplxStatusChange, cardSpareGroupId=cardSpareGroupId, systemGeneral=systemGeneral, equipmentTraps=equipmentTraps, cardTestScheduleCommand=cardTestScheduleCommand, diskVolumeUsageLevel=diskVolumeUsageLevel, auditName=auditName, cmResetNotification=cmResetNotification, auditTotalFailedCount=auditTotalFailedCount, diskVolumeShelfId=diskVolumeShelfId, cardFpgaSource=cardFpgaSource, auditEntry=auditEntry, cardTempOverHeatNotification=cardTempOverHeatNotification, portDetectedMode=portDetectedMode, auditCardId=auditCardId, portDescription=portDescription, equipmentAudit=equipmentAudit, cmResetReason=cmResetReason, equipmentState=equipmentState, equipmentAuditGroup=equipmentAuditGroup, verboseLevel=verboseLevel, cardCpuSpeed=cardCpuSpeed, cardSerialNum=cardSerialNum, portTrapInh=portTrapInh, diskVolumeDriveId=diskVolumeDriveId, errEvLogLevel=errEvLogLevel, TestCommand=TestCommand, portNumChans=portNumChans, cardTestId=cardTestId, cardTestEntry=cardTestEntry, portTxFlowControlDetected=portTxFlowControlDetected, diagTestId=diagTestId, cmResetInfo=cmResetInfo, portMacIfIndex=portMacIfIndex, diskVolumeNumber=diskVolumeNumber, auditPassedCount=auditPassedCount, equipmentCardGroup=equipmentCardGroup, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeFileName=diskVolumeFileName, portSecState=portSecState, cardLastBootSource=cardLastBootSource, cardName=cardName, auditLogThrottle=auditLogThrottle, TestResult=TestResult, cardShelfTestIndex=cardShelfTestIndex, cardTestDescription=cardTestDescription, auditShelfId=auditShelfId, cardSecStateChange=cardSecStateChange, cardTestTable=cardTestTable, cardTestResultNotification=cardTestResultNotification, cardPicSerialNum=cardPicSerialNum, cmPrimarySID=cmPrimarySID, portDplxStatus=portDplxStatus, cardFlashSize=cardFlashSize, cardCarrierSerialNum=cardCarrierSerialNum, portEntry=portEntry, auditTable=auditTable, eqDiagConfig=eqDiagConfig, equipmentCardTestGroup=equipmentCardTestGroup, portCardSubType=portCardSubType, errEventId=errEventId, fpgaErrorEventGroup=fpgaErrorEventGroup, portCardId=portCardId, cardDetectedChange=cardDetectedChange, trapCounter=trapCounter, diskVolumeCardId=diskVolumeCardId, cardShelfId=cardShelfId, remainInDiagMode=remainInDiagMode, systemClock=systemClock, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDplxStatusChange=portDplxStatusChange, auditResult=auditResult, cadEquipmentMibCompliance=cadEquipmentMibCompliance, portShelfId=portShelfId, auditDescription=auditDescription, cardTestTime=cardTestTime, cmVendor=cmVendor, cardPicDetected=cardPicDetected, cadEquipmentMibGroup=cadEquipmentMibGroup, portSecStateChange=portSecStateChange, systemGeneralGroup=systemGeneralGroup, cardCPLDVersion=cardCPLDVersion, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardTemperature=cardTemperature, auditStatus=auditStatus, equipmentDiag=equipmentDiag, PYSNMP_MODULE_ID=cadEquipmentMib, cardTestSchedule=cardTestSchedule, cadEquipmentMibConformance=cadEquipmentMibConformance, cardUpTime=cardUpTime, equipmentShelfGroup=equipmentShelfGroup, portTxFlowControlMode=portTxFlowControlMode, cardRamSize=cardRamSize, errEvRecoveryEnabled=errEvRecoveryEnabled, portRxFlowControlMode=portRxFlowControlMode, cardType=cardType, cmMacAddress=cmMacAddress, cmDevice=cmDevice, cardSubDetected=cardSubDetected, portPrStateChange=portPrStateChange, equipmentPortGroup=equipmentPortGroup, auditCycleCount=auditCycleCount, cmUChannelID=cmUChannelID, cardTestIndex=cardTestIndex, downstreamPowerLoss=downstreamPowerLoss, cmResetUpTime=cmResetUpTime, shelfName=shelfName, cardTestType=cardTestType, cardNumber=cardNumber, cmRegistrationNotification=cmRegistrationNotification, TestId=TestId, auditCommand=auditCommand, systemKey=systemKey, TestType=TestType, portAction=portAction, cardFwUpdateStatus=cardFwUpdateStatus, shelfObjects=shelfObjects, errEvRowStatus=errEvRowStatus, diskVolumeGroup=diskVolumeGroup, portCurrDsPower=portCurrDsPower, cmIpAddress=cmIpAddress, errEvLoggingEnabled=errEvLoggingEnabled, cmResetClearNotification=cmResetClearNotification, cardTemperatureHighError=cardTemperatureHighError, cadEquipmentMib=cadEquipmentMib, cardPrState=cardPrState, diskVolumeDiskSize=diskVolumeDiskSize, cardDetected=cardDetected, cardSpareGroupMode=cardSpareGroupMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentDiagGroup=equipmentDiagGroup, cardSwVersion=cardSwVersion, portMacAddress=portMacAddress, diskVolumeSize=diskVolumeSize, diskVolumeUsageNotification=diskVolumeUsageNotification, TestSchedule=TestSchedule, portType=portType, cardTable=cardTable, cardSubType=cardSubType, portMode=portMode, cardEntry=cardEntry, portNumber=portNumber, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestName=cardTestName, portDocsIfIndex=portDocsIfIndex, auditFailedCount=auditFailedCount, cardAdminState=cardAdminState)
