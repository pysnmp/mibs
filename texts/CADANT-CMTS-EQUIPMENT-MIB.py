...
CardSubType, SecondaryState, DuplexStatus, PortDetectedMode, DiskVolumeUsageLevel, CardId, CardType, PortId, AdminState, EqActionType, FlowControlMode, PortType, ShelfId, UnknownState, FirmwareSource, PrimaryState, PicType, PortMode = mibBuilder.importSymbols("CADANT-TC", "CardSubType", "SecondaryState", "DuplexStatus", "PortDetectedMode", "DiskVolumeUsageLevel", "CardId", "CardType", "PortId", "AdminState", "EqActionType", "FlowControlMode", "PortType", "ShelfId", "UnknownState", "FirmwareSource", "PrimaryState", "PicType", "PortMode")
...
DisplayString, TimeStamp, TruthValue, MacAddress, DateAndTime, TextualConvention, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "TruthValue", "MacAddress", "DateAndTime", "TextualConvention", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardLastResetReason=cardLastResetReason, TestCommand=TestCommand, cardTestSchedule=cardTestSchedule, cardShelfTestIndex=cardShelfTestIndex, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestTime=cardTestTime, portCurrDsPower=portCurrDsPower, auditTotalPassedCount=auditTotalPassedCount, cardPicSerialNum=cardPicSerialNum, cardName=cardName, consoleOutput=consoleOutput, equipmentState=equipmentState, errEvRecoveryEnabled=errEvRecoveryEnabled, shelfName=shelfName, systemGeneral=systemGeneral, shelfNumber=shelfNumber, cadEquipmentMibConformance=cadEquipmentMibConformance, cardFlashSize=cardFlashSize, cardTestType=cardTestType, auditShelfId=auditShelfId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, fpgaErrorEventTable=fpgaErrorEventTable, cardPicHwVersion=cardPicHwVersion, portSecState=portSecState, equipmentShelfGroup=equipmentShelfGroup, portDocsIfIndex=portDocsIfIndex, cardId=cardId, cardSecStateChange=cardSecStateChange, portId=portId, errEvLoggingEnabled=errEvLoggingEnabled, cardTemperatureHighError=cardTemperatureHighError, auditEntry=auditEntry, cardBusSpeed=cardBusSpeed, auditFailedCount=auditFailedCount, portDplxStatusChange=portDplxStatusChange, cardTestIndex=cardTestIndex, errEvRowStatus=errEvRowStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestName=cardTestName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardFpgaSource=cardFpgaSource, cmResetStatus=cmResetStatus, fpgaErrorEventEntry=fpgaErrorEventEntry, cardNumber=cardNumber, portDescription=portDescription, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSerialNum=cardSerialNum, portNumChans=portNumChans, auditId=auditId, cmResetUpTime=cmResetUpTime, cardTestResultDesc=cardTestResultDesc, auditPassedCount=auditPassedCount, diskVolumeCardId=diskVolumeCardId, auditCardId=auditCardId, portLastChangeTime=portLastChangeTime, trapCounter=trapCounter, cardCPLDVersion=cardCPLDVersion, cardLastBootSource=cardLastBootSource, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageNotification=diskVolumeUsageNotification, auditAutoScheduling=auditAutoScheduling, cardLastBootVersion=cardLastBootVersion, portDetectedMode=portDetectedMode, cadEquipmentMib=cadEquipmentMib, cardCarrierFwVersion=cardCarrierFwVersion, cardHwVersion=cardHwVersion, downstreamPowerLoss=downstreamPowerLoss, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentCardGroup=equipmentCardGroup, cardTestCommand=cardTestCommand, diskVolumeShelfId=diskVolumeShelfId, cardBootVersion=cardBootVersion, portEntry=portEntry, diskVolumeNumber=diskVolumeNumber, remainInDiagMode=remainInDiagMode, portMacIfIndex=portMacIfIndex, TestType=TestType, eqDiagConfig=eqDiagConfig, equipmentDiagGroup=equipmentDiagGroup, portNumber=portNumber, portTable=portTable, equipmentCardTestGroup=equipmentCardTestGroup, TestSchedule=TestSchedule, diagTestId=diagTestId, cardSwVersion=cardSwVersion, cardTrapInh=cardTrapInh, verboseLevel=verboseLevel, cardTempNoReportNotification=cardTempNoReportNotification, cardType=cardType, cardNumPorts=cardNumPorts, diskVolumeId=diskVolumeId, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeName=diskVolumeName, cmPrimarySID=cmPrimarySID, auditTable=auditTable, TestResult=TestResult, cardSecState=cardSecState, diskVolumeFileName=diskVolumeFileName, cardTestScheduleCommand=cardTestScheduleCommand, trapSeverity=trapSeverity, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardFwUpdateStatus=cardFwUpdateStatus, portRxFlowControlMode=portRxFlowControlMode, cardAdminState=cardAdminState, diskVolumeDiskSize=diskVolumeDiskSize, portMacAddress=portMacAddress, cmMacAddress=cmMacAddress, cmIpAddress=cmIpAddress, auditLogThrottle=auditLogThrottle, portCardId=portCardId, portBgpId=portBgpId, portDplxStatus=portDplxStatus, auditName=auditName, cardSubType=cardSubType, cardAction=cardAction, TestTransactionId=TestTransactionId, cardTable=cardTable, equipmentAudit=equipmentAudit, cardTestResultNotification=cardTestResultNotification, equipmentStateGroup=equipmentStateGroup, portCardSubType=portCardSubType, auditLogOutput=auditLogOutput, errEventId=errEventId, portConnectorId=portConnectorId, cardTestEntry=cardTestEntry, portTxFlowControlMode=portTxFlowControlMode, auditResult=auditResult, cardDetected=cardDetected, cardCpuSpeed=cardCpuSpeed, portShelfId=portShelfId, portTxFlowControlDetected=portTxFlowControlDetected, cardTestDescription=cardTestDescription, cmResetClearNotification=cmResetClearNotification, cardTemperature=cardTemperature, cardDetectedChange=cardDetectedChange, TestScheduleCommand=TestScheduleCommand, cadEquipmentMibCompliance=cadEquipmentMibCompliance, diskVolumeEntry=diskVolumeEntry, cmRegistrationNotification=cmRegistrationNotification, portRxFlowControlDetected=portRxFlowControlDetected, auditTime=auditTime, diskVolumeSize=diskVolumeSize, cmResetNotification=cmResetNotification, portType=portType, cardCarrierHwVersion=cardCarrierHwVersion, portMinDsPower=portMinDsPower, diskVolumeTable=diskVolumeTable, portMode=portMode, cmUChannelID=cmUChannelID, equipmentAuditGroup=equipmentAuditGroup, cardTestResult=cardTestResult, shelfObjects=shelfObjects, cardSpareGroupMode=cardSpareGroupMode, cardSpareGroupId=cardSpareGroupId, cardSubDetected=cardSubDetected, equipmentError=equipmentError, cardEntry=cardEntry, systemKey=systemKey, portPrState=portPrState, auditStatus=auditStatus, equipmentTraps=equipmentTraps, cardFwVersion=cardFwVersion, equipmentDiag=equipmentDiag, cardTestTransId=cardTestTransId, cardPrState=cardPrState, portTrapInh=portTrapInh, cardDplxStatus=cardDplxStatus, cmVendor=cmVendor, fpgaErrorEventGroup=fpgaErrorEventGroup, portAction=portAction, auditCommand=auditCommand, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardCpuType=cardCpuType, cardUpTime=cardUpTime, TestId=TestId, systemClock=systemClock, equipmentTbl=equipmentTbl, cardTestTable=cardTestTable, cmResetInfo=cmResetInfo, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMaxDsPower=portMaxDsPower, cardPrStateChange=cardPrStateChange, cmResetReason=cmResetReason, PYSNMP_MODULE_ID=cadEquipmentMib, systemGeneralGroup=systemGeneralGroup, diskVolumeGroup=diskVolumeGroup, diskVolumeDriveId=diskVolumeDriveId, cardPicDetected=cardPicDetected, diskDriveNumber=diskDriveNumber, cardLastChangeTime=cardLastChangeTime, auditCycleCount=auditCycleCount, cardCarrierSerialNum=cardCarrierSerialNum, portPrStateChange=portPrStateChange, shelfSwVersion=shelfSwVersion, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentPortGroup=equipmentPortGroup, cardRamSize=cardRamSize, auditDescription=auditDescription, portAdminState=portAdminState, cardDplxStatusChange=cardDplxStatusChange, cardShelfId=cardShelfId, cmDevice=cmDevice, portSecStateChange=portSecStateChange, cardTestId=cardTestId, errEvLogLevel=errEvLogLevel, diskVolumeTrapInh=diskVolumeTrapInh)
