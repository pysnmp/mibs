...
UnknownState, PortType, PicType, ShelfId, PortId, PortMode, CardType, PrimaryState, FlowControlMode, AdminState, FirmwareSource, PortDetectedMode, DiskVolumeUsageLevel, CardId, CardSubType, EqActionType, SecondaryState, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "UnknownState", "PortType", "PicType", "ShelfId", "PortId", "PortMode", "CardType", "PrimaryState", "FlowControlMode", "AdminState", "FirmwareSource", "PortDetectedMode", "DiskVolumeUsageLevel", "CardId", "CardSubType", "EqActionType", "SecondaryState", "DuplexStatus")
...
RowStatus, DateAndTime, TruthValue, TimeStamp, DisplayString, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DateAndTime", "TruthValue", "TimeStamp", "DisplayString", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", errEventId=errEventId, cardHwVersion=cardHwVersion, systemGeneralGroup=systemGeneralGroup, equipmentAuditGroup=equipmentAuditGroup, cmResetNotification=cmResetNotification, auditTime=auditTime, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardShelfId=cardShelfId, auditAutoScheduling=auditAutoScheduling, auditTable=auditTable, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTable=cardTable, errEvRowStatus=errEvRowStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, cardShelfTestIndex=cardShelfTestIndex, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, trapCounter=trapCounter, portMinDsPower=portMinDsPower, equipmentAudit=equipmentAudit, portSecStateChange=portSecStateChange, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, trapSeverity=trapSeverity, cardNumber=cardNumber, auditLogThrottle=auditLogThrottle, equipmentError=equipmentError, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTempOverHeatNotification=cardTempOverHeatNotification, cardCpuType=cardCpuType, portPrStateChange=portPrStateChange, cardTemperature=cardTemperature, equipmentCardGroup=equipmentCardGroup, TestId=TestId, systemClock=systemClock, portType=portType, TestType=TestType, cardCarrierHwVersion=cardCarrierHwVersion, TestCommand=TestCommand, diskVolumeTable=diskVolumeTable, auditFailedCount=auditFailedCount, cardSubDetected=cardSubDetected, diskVolumeId=diskVolumeId, equipmentPortGroup=equipmentPortGroup, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeFileName=diskVolumeFileName, cmPrimarySID=cmPrimarySID, portBgpId=portBgpId, cardTestDescription=cardTestDescription, cadEquipmentMib=cadEquipmentMib, shelfObjects=shelfObjects, TestTransactionId=TestTransactionId, equipmentStateGroup=equipmentStateGroup, cardEntry=cardEntry, cardLastResetReason=cardLastResetReason, cardSecState=cardSecState, cmResetUpTime=cmResetUpTime, fpgaErrorEventGroup=fpgaErrorEventGroup, remainInDiagMode=remainInDiagMode, cardTemperatureHighWarn=cardTemperatureHighWarn, cardId=cardId, cardLastBootVersion=cardLastBootVersion, diskVolumeName=diskVolumeName, portShelfId=portShelfId, TestSchedule=TestSchedule, equipmentTbl=equipmentTbl, cardRamSize=cardRamSize, cardType=cardType, portTrapInh=portTrapInh, cardTestTime=cardTestTime, auditDescription=auditDescription, cardTestSchedule=cardTestSchedule, portDplxStatus=portDplxStatus, TestResult=TestResult, auditTotalPassedCount=auditTotalPassedCount, cardTemperatureHighError=cardTemperatureHighError, auditLogOutput=auditLogOutput, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMibCompliances=cadEquipmentMibCompliances, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentShelfGroup=equipmentShelfGroup, cardDplxStatusChange=cardDplxStatusChange, diskVolumeUsageLevel=diskVolumeUsageLevel, cmUChannelID=cmUChannelID, cardTestIndex=cardTestIndex, cadEquipmentMibConformance=cadEquipmentMibConformance, portRxFlowControlMode=portRxFlowControlMode, cardDetected=cardDetected, verboseLevel=verboseLevel, portNumChans=portNumChans, equipmentDiagGroup=equipmentDiagGroup, portId=portId, cardFwUpdateStatus=cardFwUpdateStatus, diskDriveNumber=diskDriveNumber, cardTestScheduleCommand=cardTestScheduleCommand, portAction=portAction, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResult=cardTestResult, portMode=portMode, cardFwVersion=cardFwVersion, cmVendor=cmVendor, cardCpuSpeed=cardCpuSpeed, systemGeneral=systemGeneral, cardSecStateChange=cardSecStateChange, portNumber=portNumber, cmResetClearNotification=cmResetClearNotification, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portPrState=portPrState, auditResult=auditResult, eqDiagConfig=eqDiagConfig, cmIpAddress=cmIpAddress, portLastChangeTime=portLastChangeTime, cardTestCommand=cardTestCommand, diskVolumeNumber=diskVolumeNumber, cardDplxStatus=cardDplxStatus, cardTestName=cardTestName, cardTestResultDesc=cardTestResultDesc, cmRegistrationNotification=cmRegistrationNotification, TestScheduleCommand=TestScheduleCommand, diskVolumeCardId=diskVolumeCardId, equipmentDiag=equipmentDiag, cardTestTransId=cardTestTransId, portCurrDsPower=portCurrDsPower, auditCommand=auditCommand, errEvLogLevel=errEvLogLevel, auditTotalFailedCount=auditTotalFailedCount, portTable=portTable, consoleOutput=consoleOutput, cardTestId=cardTestId, portMacIfIndex=portMacIfIndex, systemKey=systemKey, cardName=cardName, errEvRecoveryEnabled=errEvRecoveryEnabled, cardPicSerialNum=cardPicSerialNum, portDescription=portDescription, cardCPLDVersion=cardCPLDVersion, cardCarrierFwVersion=cardCarrierFwVersion, diskVolumeSize=diskVolumeSize, fpgaErrorEventTable=fpgaErrorEventTable, cardDetectedChange=cardDetectedChange, auditShelfId=auditShelfId, diskVolumeEntry=diskVolumeEntry, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestResultNotification=cardTestResultNotification, cardPrStateChange=cardPrStateChange, cardSpareGroupId=cardSpareGroupId, portConnectorId=portConnectorId, portMacAddress=portMacAddress, cardTrapInh=cardTrapInh, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardAction=cardAction, equipmentTraps=equipmentTraps, cardSpareGroupMode=cardSpareGroupMode, auditPassedCount=auditPassedCount, downstreamPowerLoss=downstreamPowerLoss, cardPrState=cardPrState, cmResetInfo=cmResetInfo, portDplxStatusChange=portDplxStatusChange, cardAdminState=cardAdminState, cardFpgaSource=cardFpgaSource, auditCycleCount=auditCycleCount, shelfNumber=shelfNumber, cardNumPorts=cardNumPorts, cardLastChangeTime=cardLastChangeTime, equipmentCardTestGroup=equipmentCardTestGroup, auditCardId=auditCardId, cardTestType=cardTestType, diskVolumeShelfId=diskVolumeShelfId, equipmentState=equipmentState, cardLastBootSource=cardLastBootSource, cardSwVersion=cardSwVersion, cardUpTime=cardUpTime, cardCarrierSerialNum=cardCarrierSerialNum, auditEntry=auditEntry, cmDevice=cmDevice, cmMacAddress=cmMacAddress, cardPicHwVersion=cardPicHwVersion, diskVolumeGroup=diskVolumeGroup, cardSubType=cardSubType, cardTestEntry=cardTestEntry, cardBootVersion=cardBootVersion, portTxFlowControlMode=portTxFlowControlMode, portSecState=portSecState, diagTestId=diagTestId, portEntry=portEntry, PYSNMP_MODULE_ID=cadEquipmentMib, cmResetStatus=cmResetStatus, portDocsIfIndex=portDocsIfIndex, portCardSubType=portCardSubType, cardBusSpeed=cardBusSpeed, portCardId=portCardId, portTxFlowControlDetected=portTxFlowControlDetected, portDetectedMode=portDetectedMode, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeTrapInh=diskVolumeTrapInh, diskVolumeDriveId=diskVolumeDriveId, cardTestTable=cardTestTable, cmResetReason=cmResetReason, auditId=auditId, portAdminState=portAdminState, shelfName=shelfName, auditName=auditName, auditStatus=auditStatus, cardPicDetected=cardPicDetected, cardSerialNum=cardSerialNum, portRxFlowControlDetected=portRxFlowControlDetected, cardFlashSize=cardFlashSize, shelfSwVersion=shelfSwVersion)
