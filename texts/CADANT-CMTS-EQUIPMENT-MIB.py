...
PortId, PortDetectedMode, CardType, CardSubType, CardId, PicType, DiskVolumeUsageLevel, FlowControlMode, FirmwareSource, PortType, PrimaryState, EqActionType, UnknownState, AdminState, ShelfId, SecondaryState, PortMode, DuplexStatus = mibBuilder.importSymbols("CADANT-TC", "PortId", "PortDetectedMode", "CardType", "CardSubType", "CardId", "PicType", "DiskVolumeUsageLevel", "FlowControlMode", "FirmwareSource", "PortType", "PrimaryState", "EqActionType", "UnknownState", "AdminState", "ShelfId", "SecondaryState", "PortMode", "DuplexStatus")
...
TextualConvention, DisplayString, RowStatus, TruthValue, DateAndTime, TimeStamp, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "RowStatus", "TruthValue", "DateAndTime", "TimeStamp", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portDetectedMode=portDetectedMode, equipmentError=equipmentError, cardTempOverHeatNotification=cardTempOverHeatNotification, diskDriveNumber=diskDriveNumber, portCardSubType=portCardSubType, TestSchedule=TestSchedule, remainInDiagMode=remainInDiagMode, portCurrDsPower=portCurrDsPower, diskVolumeCardId=diskVolumeCardId, portId=portId, diskVolumeNumber=diskVolumeNumber, portMacIfIndex=portMacIfIndex, portType=portType, portDocsIfIndex=portDocsIfIndex, portEntry=portEntry, cardSubDetected=cardSubDetected, cardEntry=cardEntry, portMode=portMode, portSecState=portSecState, diskVolumeTrapInh=diskVolumeTrapInh, trapCounter=trapCounter, cardTemperatureHighError=cardTemperatureHighError, cardBusSpeed=cardBusSpeed, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cmIpAddress=cmIpAddress, cmResetReason=cmResetReason, equipmentPortGroup=equipmentPortGroup, diskVolumeShelfId=diskVolumeShelfId, portAction=portAction, cardFpgaSource=cardFpgaSource, cardShelfTestIndex=cardShelfTestIndex, cmResetClearNotification=cmResetClearNotification, TestId=TestId, errEvLogLevel=errEvLogLevel, cardBootVersion=cardBootVersion, auditTime=auditTime, TestCommand=TestCommand, TestTransactionId=TestTransactionId, cardFlashSize=cardFlashSize, diskVolumeFileName=diskVolumeFileName, portSecStateChange=portSecStateChange, downstreamPowerLoss=downstreamPowerLoss, cardFwUpdateStatus=cardFwUpdateStatus, cmResetNotification=cmResetNotification, portNumChans=portNumChans, eqDiagConfig=eqDiagConfig, cardTestIndex=cardTestIndex, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardName=cardName, portMacAddress=portMacAddress, portMaxDsPower=portMaxDsPower, errEvLoggingEnabled=errEvLoggingEnabled, cardLastBootSource=cardLastBootSource, cardNumber=cardNumber, cardPicDetected=cardPicDetected, cardSwVersion=cardSwVersion, auditTable=auditTable, consoleOutput=consoleOutput, cardTemperature=cardTemperature, auditPassedCount=auditPassedCount, portTrapInh=portTrapInh, cardTestTransId=cardTestTransId, cmMacAddress=cmMacAddress, auditDescription=auditDescription, cardFwVersion=cardFwVersion, portShelfId=portShelfId, auditEntry=auditEntry, portRxFlowControlDetected=portRxFlowControlDetected, cardTestScheduleCommand=cardTestScheduleCommand, cardSecState=cardSecState, cardTestCommand=cardTestCommand, cardTrapInh=cardTrapInh, cardCPLDVersion=cardCPLDVersion, equipmentTraps=equipmentTraps, cardTestResultDesc=cardTestResultDesc, cmUChannelID=cmUChannelID, portMinDsPower=portMinDsPower, cardSerialNum=cardSerialNum, auditId=auditId, systemGeneralGroup=systemGeneralGroup, cardSecStateChange=cardSecStateChange, cardDplxStatusChange=cardDplxStatusChange, systemKey=systemKey, cardNumPorts=cardNumPorts, cardDetected=cardDetected, diskVolumeTable=diskVolumeTable, portTable=portTable, cardAdminState=cardAdminState, cardRamSize=cardRamSize, diskVolumeUsageNotification=diskVolumeUsageNotification, systemGeneral=systemGeneral, PYSNMP_MODULE_ID=cadEquipmentMib, cardAction=cardAction, portTxFlowControlDetected=portTxFlowControlDetected, equipmentCardGroup=equipmentCardGroup, cardTestId=cardTestId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardPrState=cardPrState, cardLastResetReason=cardLastResetReason, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentAuditGroup=equipmentAuditGroup, portBgpId=portBgpId, cmResetInfo=cmResetInfo, TestScheduleCommand=TestScheduleCommand, cardSpareGroupId=cardSpareGroupId, cardDplxStatus=cardDplxStatus, shelfNumber=shelfNumber, cardCarrierSerialNum=cardCarrierSerialNum, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cmVendor=cmVendor, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portConnectorId=portConnectorId, auditCycleCount=auditCycleCount, diskVolumeId=diskVolumeId, diskVolumeGroup=diskVolumeGroup, diskVolumeSize=diskVolumeSize, cardTestName=cardTestName, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portLastChangeTime=portLastChangeTime, fpgaErrorEventTable=fpgaErrorEventTable, cardTestSchedule=cardTestSchedule, diskVolumeDriveId=diskVolumeDriveId, cardShelfId=cardShelfId, TestType=TestType, errEvRowStatus=errEvRowStatus, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeDiskSize=diskVolumeDiskSize, trapSeverity=trapSeverity, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, TestResult=TestResult, cardLastChangeTime=cardLastChangeTime, cadEquipmentMib=cadEquipmentMib, portPrState=portPrState, auditTotalFailedCount=auditTotalFailedCount, cardCpuType=cardCpuType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardDetectedChange=cardDetectedChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeEntry=diskVolumeEntry, portNumber=portNumber, portPrStateChange=portPrStateChange, diagTestId=diagTestId, cardTable=cardTable, cardCpuSpeed=cardCpuSpeed, equipmentDiag=equipmentDiag, errEventId=errEventId, cmDevice=cmDevice, errEvRecoveryEnabled=errEvRecoveryEnabled, auditStatus=auditStatus, portDescription=portDescription, auditShelfId=auditShelfId, auditCommand=auditCommand, portRxFlowControlMode=portRxFlowControlMode, equipmentAudit=equipmentAudit, cardTestType=cardTestType, auditTotalPassedCount=auditTotalPassedCount, cardPrStateChange=cardPrStateChange, equipmentStateGroup=equipmentStateGroup, diskVolumeName=diskVolumeName, auditResult=auditResult, portAdminState=portAdminState, auditCardId=auditCardId, cardTestDescription=cardTestDescription, shelfObjects=shelfObjects, cardLastBootVersion=cardLastBootVersion, equipmentShelfGroup=equipmentShelfGroup, auditName=auditName, cardHwVersion=cardHwVersion, cardUpTime=cardUpTime, cmPrimarySID=cmPrimarySID, systemClock=systemClock, fpgaErrorEventGroup=fpgaErrorEventGroup, shelfSwVersion=shelfSwVersion, cardCarrierFwVersion=cardCarrierFwVersion, verboseLevel=verboseLevel, auditLogThrottle=auditLogThrottle, equipmentState=equipmentState, cardTempNoReportNotification=cardTempNoReportNotification, cardTestResultNotification=cardTestResultNotification, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardPicSerialNum=cardPicSerialNum, cardId=cardId, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentDiagGroup=equipmentDiagGroup, cadEquipmentMibGroup=cadEquipmentMibGroup, cmResetUpTime=cmResetUpTime, cardPicHwVersion=cardPicHwVersion, auditFailedCount=auditFailedCount, cmResetStatus=cmResetStatus, cardTestTime=cardTestTime, portTxFlowControlMode=portTxFlowControlMode, portCardId=portCardId, shelfName=shelfName, equipmentTbl=equipmentTbl, cardTestTable=cardTestTable, cardType=cardType, cardTestEntry=cardTestEntry, portDplxStatusChange=portDplxStatusChange, cmRegistrationNotification=cmRegistrationNotification, auditAutoScheduling=auditAutoScheduling, auditLogOutput=auditLogOutput, fpgaErrorEventEntry=fpgaErrorEventEntry, cardSubType=cardSubType, cardSpareGroupMode=cardSpareGroupMode, cardTestResult=cardTestResult, portDplxStatus=portDplxStatus, equipmentCardTestGroup=equipmentCardTestGroup)
