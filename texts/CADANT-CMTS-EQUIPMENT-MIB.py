...
DuplexStatus, PortMode, CardId, EqActionType, AdminState, SecondaryState, DiskVolumeUsageLevel, PortId, PicType, ShelfId, PortType, CardSubType, CardType, UnknownState, FirmwareSource, PortDetectedMode, FlowControlMode, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PortMode", "CardId", "EqActionType", "AdminState", "SecondaryState", "DiskVolumeUsageLevel", "PortId", "PicType", "ShelfId", "PortType", "CardSubType", "CardType", "UnknownState", "FirmwareSource", "PortDetectedMode", "FlowControlMode", "PrimaryState")
...
DateAndTime, TextualConvention, MacAddress, RowStatus, TruthValue, DisplayString, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TextualConvention", "MacAddress", "RowStatus", "TruthValue", "DisplayString", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", errEvLogLevel=errEvLogLevel, cardDplxStatusChange=cardDplxStatusChange, cardLastResetReason=cardLastResetReason, cardRamSize=cardRamSize, cardTestSchedule=cardTestSchedule, portDplxStatus=portDplxStatus, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, PYSNMP_MODULE_ID=cadEquipmentMib, cmDevice=cmDevice, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentCardGroup=equipmentCardGroup, fpgaErrorEventEntry=fpgaErrorEventEntry, cardNumber=cardNumber, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmResetClearNotification=cmResetClearNotification, auditDescription=auditDescription, cardSubType=cardSubType, cardCpuType=cardCpuType, cardTemperatureHighError=cardTemperatureHighError, cardDetected=cardDetected, cardAction=cardAction, portNumChans=portNumChans, TestId=TestId, cardTestEntry=cardTestEntry, portLastChangeTime=portLastChangeTime, cardTestIndex=cardTestIndex, cmResetNotification=cmResetNotification, consoleOutput=consoleOutput, remainInDiagMode=remainInDiagMode, cardTestResultDesc=cardTestResultDesc, portTrapInh=portTrapInh, portCardSubType=portCardSubType, cardUpTime=cardUpTime, cardSpareGroupMode=cardSpareGroupMode, TestResult=TestResult, cardPrState=cardPrState, TestCommand=TestCommand, cardTestTable=cardTestTable, cardPrStateChange=cardPrStateChange, cardSecState=cardSecState, portRxFlowControlMode=portRxFlowControlMode, auditFailedCount=auditFailedCount, systemGeneralGroup=systemGeneralGroup, cardSpareGroupId=cardSpareGroupId, portBgpId=portBgpId, TestType=TestType, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicHwVersion=cardPicHwVersion, cardTestDescription=cardTestDescription, auditName=auditName, portDetectedMode=portDetectedMode, auditTable=auditTable, diskDriveNumber=diskDriveNumber, TestTransactionId=TestTransactionId, cardSerialNum=cardSerialNum, portDescription=portDescription, equipmentDiag=equipmentDiag, diagTestId=diagTestId, auditTime=auditTime, cmMacAddress=cmMacAddress, equipmentDiagGroup=equipmentDiagGroup, portMaxDsPower=portMaxDsPower, cardDplxStatus=cardDplxStatus, auditLogThrottle=auditLogThrottle, auditAutoScheduling=auditAutoScheduling, cardBusSpeed=cardBusSpeed, diskVolumeNumber=diskVolumeNumber, equipmentState=equipmentState, cardTestName=cardTestName, cardTestResult=cardTestResult, portType=portType, cadEquipmentMib=cadEquipmentMib, cmVendor=cmVendor, auditEntry=auditEntry, cardFwVersion=cardFwVersion, cardType=cardType, cardAdminState=cardAdminState, cardSubDetected=cardSubDetected, auditResult=auditResult, trapCounter=trapCounter, cardLastChangeTime=cardLastChangeTime, auditCycleCount=auditCycleCount, diskVolumeDriveId=diskVolumeDriveId, diskVolumeFileName=diskVolumeFileName, systemGeneral=systemGeneral, cardSecStateChange=cardSecStateChange, shelfSwVersion=shelfSwVersion, diskVolumeName=diskVolumeName, diskVolumeTrapInh=diskVolumeTrapInh, portPrState=portPrState, equipmentPortGroup=equipmentPortGroup, cardDetectedChange=cardDetectedChange, cardShelfId=cardShelfId, auditLogOutput=auditLogOutput, portMacIfIndex=portMacIfIndex, cardTestResultNotification=cardTestResultNotification, portConnectorId=portConnectorId, systemClock=systemClock, cardCarrierHwVersion=cardCarrierHwVersion, cardId=cardId, TestScheduleCommand=TestScheduleCommand, TestSchedule=TestSchedule, portTxFlowControlMode=portTxFlowControlMode, auditTotalPassedCount=auditTotalPassedCount, cmPrimarySID=cmPrimarySID, portSecStateChange=portSecStateChange, cardLastBootVersion=cardLastBootVersion, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeDiskSize=diskVolumeDiskSize, portMinDsPower=portMinDsPower, cardTemperature=cardTemperature, cardPicSerialNum=cardPicSerialNum, cmResetStatus=cmResetStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardTestId=cardTestId, cadEquipmentMibGroup=cadEquipmentMibGroup, errEventId=errEventId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portId=portId, diskVolumeId=diskVolumeId, diskVolumeSize=diskVolumeSize, auditStatus=auditStatus, equipmentStateGroup=equipmentStateGroup, cardBootVersion=cardBootVersion, diskVolumeGroup=diskVolumeGroup, cardCarrierFwVersion=cardCarrierFwVersion, shelfNumber=shelfNumber, errEvLoggingEnabled=errEvLoggingEnabled, auditPassedCount=auditPassedCount, portTxFlowControlDetected=portTxFlowControlDetected, errEvRowStatus=errEvRowStatus, cmIpAddress=cmIpAddress, cardTestTime=cardTestTime, equipmentCardTestGroup=equipmentCardTestGroup, auditShelfId=auditShelfId, portMode=portMode, portRxFlowControlDetected=portRxFlowControlDetected, cardCpuSpeed=cardCpuSpeed, portEntry=portEntry, diskVolumeShelfId=diskVolumeShelfId, equipmentError=equipmentError, fpgaErrorEventGroup=fpgaErrorEventGroup, trapSeverity=trapSeverity, cardShelfTestIndex=cardShelfTestIndex, cardTempOverHeatNotification=cardTempOverHeatNotification, cardTestType=cardTestType, cardTrapInh=cardTrapInh, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portCurrDsPower=portCurrDsPower, equipmentTraps=equipmentTraps, portNumber=portNumber, cardNumPorts=cardNumPorts, diskVolumeUsageNotification=diskVolumeUsageNotification, equipmentAuditGroup=equipmentAuditGroup, cardFpgaSource=cardFpgaSource, cmResetInfo=cmResetInfo, portTable=portTable, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardName=cardName, fpgaErrorEventTable=fpgaErrorEventTable, shelfName=shelfName, portDocsIfIndex=portDocsIfIndex, cardEntry=cardEntry, cardTestCommand=cardTestCommand, systemKey=systemKey, auditId=auditId, cardSwVersion=cardSwVersion, auditTotalFailedCount=auditTotalFailedCount, downstreamPowerLoss=downstreamPowerLoss, shelfObjects=shelfObjects, cmRegistrationNotification=cmRegistrationNotification, portMacAddress=portMacAddress, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditCommand=auditCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, cmUChannelID=cmUChannelID, cmResetUpTime=cmResetUpTime, equipmentAudit=equipmentAudit, auditCardId=auditCardId, diskVolumeCardId=diskVolumeCardId, portCardId=portCardId, cardPicDetected=cardPicDetected, portShelfId=portShelfId, cardCPLDVersion=cardCPLDVersion, portAdminState=portAdminState, equipmentShelfGroup=equipmentShelfGroup, eqDiagConfig=eqDiagConfig, portAction=portAction, cardLastBootSource=cardLastBootSource, diskVolumeTable=diskVolumeTable, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTransId=cardTestTransId, portPrStateChange=portPrStateChange, cardTable=cardTable, cardHwVersion=cardHwVersion, portSecState=portSecState, portDplxStatusChange=portDplxStatusChange, cmResetReason=cmResetReason, cardFlashSize=cardFlashSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeEntry=diskVolumeEntry, cardTestScheduleCommand=cardTestScheduleCommand, equipmentTbl=equipmentTbl, verboseLevel=verboseLevel, cardCarrierSerialNum=cardCarrierSerialNum)
