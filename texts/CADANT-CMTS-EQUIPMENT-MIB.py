...
AdminState, PortId, ShelfId, UnknownState, PortType, PicType, SecondaryState, EqActionType, PrimaryState, DiskVolumeUsageLevel, FlowControlMode, PortMode, CardSubType, DuplexStatus, FirmwareSource, PortDetectedMode, CardId, CardType = mibBuilder.importSymbols("CADANT-TC", "AdminState", "PortId", "ShelfId", "UnknownState", "PortType", "PicType", "SecondaryState", "EqActionType", "PrimaryState", "DiskVolumeUsageLevel", "FlowControlMode", "PortMode", "CardSubType", "DuplexStatus", "FirmwareSource", "PortDetectedMode", "CardId", "CardType")
...
RowStatus, TimeStamp, TruthValue, DateAndTime, DisplayString, MacAddress, TextualConvention = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TimeStamp", "TruthValue", "DateAndTime", "DisplayString", "MacAddress", "TextualConvention")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portSecStateChange=portSecStateChange, cardTestCommand=cardTestCommand, equipmentPortGroup=equipmentPortGroup, shelfNumber=shelfNumber, cardTempOverHeatNotification=cardTempOverHeatNotification, portPrState=portPrState, auditDescription=auditDescription, cardPrState=cardPrState, portConnectorId=portConnectorId, cardCarrierFwVersion=cardCarrierFwVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, cardFwUpdateStatus=cardFwUpdateStatus, portEntry=portEntry, portMinDsPower=portMinDsPower, diskVolumeSize=diskVolumeSize, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portAdminState=portAdminState, auditTotalFailedCount=auditTotalFailedCount, cmUChannelID=cmUChannelID, TestType=TestType, equipmentDiag=equipmentDiag, TestCommand=TestCommand, cardPicDetected=cardPicDetected, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardRamSize=cardRamSize, cardTestEntry=cardTestEntry, cmPrimarySID=cmPrimarySID, auditShelfId=auditShelfId, cardShelfTestIndex=cardShelfTestIndex, portBgpId=portBgpId, diskVolumeCardId=diskVolumeCardId, portMacAddress=portMacAddress, cardBusSpeed=cardBusSpeed, portPrStateChange=portPrStateChange, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSubType=cardSubType, cardSecStateChange=cardSecStateChange, auditName=auditName, cardSerialNum=cardSerialNum, auditCycleCount=auditCycleCount, cardTemperatureHighWarn=cardTemperatureHighWarn, auditFailedCount=auditFailedCount, portAction=portAction, auditStatus=auditStatus, cardTestResultDesc=cardTestResultDesc, auditCommand=auditCommand, cardTestTable=cardTestTable, cardSubDetected=cardSubDetected, diskVolumeFileName=diskVolumeFileName, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, verboseLevel=verboseLevel, cardDetected=cardDetected, cardDetectedChange=cardDetectedChange, cardTestId=cardTestId, equipmentAuditGroup=equipmentAuditGroup, diskVolumeId=diskVolumeId, diskVolumeUsageNotification=diskVolumeUsageNotification, cardEntry=cardEntry, portMaxDsPower=portMaxDsPower, cardNumPorts=cardNumPorts, cardId=cardId, cardTemperature=cardTemperature, cardTestScheduleCommand=cardTestScheduleCommand, equipmentState=equipmentState, cardBootVersion=cardBootVersion, portDetectedMode=portDetectedMode, auditLogThrottle=auditLogThrottle, equipmentCardGroup=equipmentCardGroup, portShelfId=portShelfId, portRxFlowControlMode=portRxFlowControlMode, cmRegistrationNotification=cmRegistrationNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, auditPassedCount=auditPassedCount, cmMacAddress=cmMacAddress, portCardSubType=portCardSubType, equipmentError=equipmentError, portLastChangeTime=portLastChangeTime, cmDevice=cmDevice, auditTime=auditTime, portNumber=portNumber, cardLastResetReason=cardLastResetReason, portMacIfIndex=portMacIfIndex, cardFlashSize=cardFlashSize, cardPicSerialNum=cardPicSerialNum, shelfName=shelfName, diskVolumeEntry=diskVolumeEntry, cardTestTransId=cardTestTransId, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeName=diskVolumeName, diskVolumeUsageLevel=diskVolumeUsageLevel, equipmentStateGroup=equipmentStateGroup, systemClock=systemClock, cmResetReason=cmResetReason, auditCardId=auditCardId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cmResetStatus=cmResetStatus, cardTestSchedule=cardTestSchedule, cardTestResultNotification=cardTestResultNotification, auditEntry=auditEntry, portTable=portTable, TestResult=TestResult, portType=portType, cardFpgaSource=cardFpgaSource, cardTestTime=cardTestTime, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeShelfId=diskVolumeShelfId, cardLastBootSource=cardLastBootSource, portMode=portMode, cmResetUpTime=cmResetUpTime, fpgaErrorEventEntry=fpgaErrorEventEntry, cardCpuType=cardCpuType, cmResetInfo=cmResetInfo, cardTemperatureHighError=cardTemperatureHighError, diagTestId=diagTestId, diskVolumeGroup=diskVolumeGroup, cardSwVersion=cardSwVersion, portDescription=portDescription, equipmentCardTestGroup=equipmentCardTestGroup, TestScheduleCommand=TestScheduleCommand, TestId=TestId, cardSpareGroupMode=cardSpareGroupMode, portSecState=portSecState, shelfObjects=shelfObjects, portId=portId, cardDplxStatus=cardDplxStatus, cardCpuSpeed=cardCpuSpeed, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeTrapInh=diskVolumeTrapInh, cardTestDescription=cardTestDescription, cardTestType=cardTestType, cardType=cardType, cmVendor=cmVendor, cadEquipmentMibConformance=cadEquipmentMibConformance, cardLastBootVersion=cardLastBootVersion, cardPicHwVersion=cardPicHwVersion, cadEquipmentMibCompliances=cadEquipmentMibCompliances, TestTransactionId=TestTransactionId, cadEquipmentMib=cadEquipmentMib, cardName=cardName, portDocsIfIndex=portDocsIfIndex, cardTestIndex=cardTestIndex, cardTestResult=cardTestResult, cardFwVersion=cardFwVersion, cmResetClearNotification=cmResetClearNotification, auditResult=auditResult, portTxFlowControlMode=portTxFlowControlMode, cardTrapInh=cardTrapInh, portDplxStatusChange=portDplxStatusChange, downstreamPowerLoss=downstreamPowerLoss, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentTbl=equipmentTbl, cardSecState=cardSecState, diskDriveNumber=diskDriveNumber, cmIpAddress=cmIpAddress, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentAudit=equipmentAudit, remainInDiagMode=remainInDiagMode, equipmentDiagGroup=equipmentDiagGroup, auditTable=auditTable, cardUpTime=cardUpTime, TestSchedule=TestSchedule, cardTempNoReportNotification=cardTempNoReportNotification, systemGeneral=systemGeneral, systemKey=systemKey, portRxFlowControlDetected=portRxFlowControlDetected, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardLastChangeTime=cardLastChangeTime, portDplxStatus=portDplxStatus, errEvLogLevel=errEvLogLevel, consoleOutput=consoleOutput, cardNumber=cardNumber, diskVolumeDriveId=diskVolumeDriveId, cardPrStateChange=cardPrStateChange, shelfSwVersion=shelfSwVersion, equipmentTraps=equipmentTraps, errEventId=errEventId, equipmentShelfGroup=equipmentShelfGroup, cardHwVersion=cardHwVersion, errEvRowStatus=errEvRowStatus, errEvLoggingEnabled=errEvLoggingEnabled, cardCarrierSerialNum=cardCarrierSerialNum, cardTestName=cardTestName, auditId=auditId, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeNumber=diskVolumeNumber, cardAdminState=cardAdminState, cardTable=cardTable, eqDiagConfig=eqDiagConfig, diskVolumeDiskSize=diskVolumeDiskSize, portTrapInh=portTrapInh, systemGeneralGroup=systemGeneralGroup, cardCarrierHwVersion=cardCarrierHwVersion, auditLogOutput=auditLogOutput, diskVolumeTable=diskVolumeTable, cardAction=cardAction, auditAutoScheduling=auditAutoScheduling, cmResetNotification=cmResetNotification, cardSpareGroupId=cardSpareGroupId, portNumChans=portNumChans, cardCPLDVersion=cardCPLDVersion, portCurrDsPower=portCurrDsPower, cardShelfId=cardShelfId, trapSeverity=trapSeverity, trapCounter=trapCounter, portCardId=portCardId, cardDplxStatusChange=cardDplxStatusChange, auditTotalPassedCount=auditTotalPassedCount)
