...
SecondaryState, DiskVolumeUsageLevel, UnknownState, EqActionType, FirmwareSource, CardType, PicType, DuplexStatus, PortMode, AdminState, PrimaryState, PortDetectedMode, PortType, CardId, FlowControlMode, CardSubType, PortId, ShelfId = mibBuilder.importSymbols("CADANT-TC", "SecondaryState", "DiskVolumeUsageLevel", "UnknownState", "EqActionType", "FirmwareSource", "CardType", "PicType", "DuplexStatus", "PortMode", "AdminState", "PrimaryState", "PortDetectedMode", "PortType", "CardId", "FlowControlMode", "CardSubType", "PortId", "ShelfId")
...
TextualConvention, TimeStamp, DateAndTime, TruthValue, RowStatus, MacAddress, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TimeStamp", "DateAndTime", "TruthValue", "RowStatus", "MacAddress", "DisplayString")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portDescription=portDescription, cmRegistrationNotification=cmRegistrationNotification, portTxFlowControlDetected=portTxFlowControlDetected, systemGeneral=systemGeneral, portType=portType, portPrState=portPrState, portTable=portTable, cardShelfTestIndex=cardShelfTestIndex, fpgaErrorEventGroup=fpgaErrorEventGroup, auditId=auditId, TestResult=TestResult, portAction=portAction, cardAdminState=cardAdminState, cardNumber=cardNumber, portDplxStatusChange=portDplxStatusChange, cardTemperatureHighError=cardTemperatureHighError, portMacAddress=portMacAddress, cardTestResult=cardTestResult, cardLastChangeTime=cardLastChangeTime, cardSubType=cardSubType, cardNumPorts=cardNumPorts, diskVolumeGroup=diskVolumeGroup, auditCycleCount=auditCycleCount, cardFpgaSource=cardFpgaSource, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentPortGroup=equipmentPortGroup, cardCpuSpeed=cardCpuSpeed, cardTestName=cardTestName, cardDetected=cardDetected, auditFailedCount=auditFailedCount, equipmentTbl=equipmentTbl, cardEntry=cardEntry, equipmentDiagGroup=equipmentDiagGroup, portCardId=portCardId, cardCarrierHwVersion=cardCarrierHwVersion, diskVolumeFileName=diskVolumeFileName, auditLogThrottle=auditLogThrottle, cadEquipmentMibGroup=cadEquipmentMibGroup, TestScheduleCommand=TestScheduleCommand, cardTestSchedule=cardTestSchedule, cardTestScheduleCommand=cardTestScheduleCommand, systemKey=systemKey, cardSerialNum=cardSerialNum, cardLastBootSource=cardLastBootSource, trapCounter=trapCounter, cardTestId=cardTestId, cardUpTime=cardUpTime, cmResetReason=cmResetReason, portShelfId=portShelfId, cardType=cardType, cardPicDetected=cardPicDetected, cardFwUpdateStatus=cardFwUpdateStatus, cardSecState=cardSecState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardTestTable=cardTestTable, cardTestResultDesc=cardTestResultDesc, auditTotalPassedCount=auditTotalPassedCount, diskVolumeDiskSize=diskVolumeDiskSize, portTxFlowControlMode=portTxFlowControlMode, cardFlashSize=cardFlashSize, cardFwVersion=cardFwVersion, diskVolumeCardId=diskVolumeCardId, equipmentTraps=equipmentTraps, cardBootVersion=cardBootVersion, cardBusSpeed=cardBusSpeed, cardDplxStatus=cardDplxStatus, shelfObjects=shelfObjects, portMode=portMode, auditLogOutput=auditLogOutput, cardCarrierFwVersion=cardCarrierFwVersion, portRxFlowControlMode=portRxFlowControlMode, auditDescription=auditDescription, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditCommand=auditCommand, auditName=auditName, cardHwVersion=cardHwVersion, cmMacAddress=cmMacAddress, eqDiagConfig=eqDiagConfig, cardPrStateChange=cardPrStateChange, auditCardId=auditCardId, cardDetectedChange=cardDetectedChange, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardPicSerialNum=cardPicSerialNum, portMacIfIndex=portMacIfIndex, portMaxDsPower=portMaxDsPower, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portSecStateChange=portSecStateChange, cardCarrierSerialNum=cardCarrierSerialNum, cardTable=cardTable, cardTestDescription=cardTestDescription, cardTestTime=cardTestTime, fpgaErrorEventEntry=fpgaErrorEventEntry, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardSecStateChange=cardSecStateChange, auditPassedCount=auditPassedCount, auditAutoScheduling=auditAutoScheduling, portPrStateChange=portPrStateChange, cardLastResetReason=cardLastResetReason, cardSubDetected=cardSubDetected, diskVolumeNumber=diskVolumeNumber, PYSNMP_MODULE_ID=cadEquipmentMib, TestSchedule=TestSchedule, portNumChans=portNumChans, shelfNumber=shelfNumber, portMinDsPower=portMinDsPower, auditShelfId=auditShelfId, cmUChannelID=cmUChannelID, cmIpAddress=cmIpAddress, diskVolumeUsageNotification=diskVolumeUsageNotification, portTrapInh=portTrapInh, diskVolumeUsageLevel=diskVolumeUsageLevel, cardTestTransId=cardTestTransId, equipmentCardTestGroup=equipmentCardTestGroup, auditTime=auditTime, cardCPLDVersion=cardCPLDVersion, cardSpareGroupMode=cardSpareGroupMode, fpgaErrorEventTable=fpgaErrorEventTable, portCurrDsPower=portCurrDsPower, cadEquipmentMibCompliances=cadEquipmentMibCompliances, remainInDiagMode=remainInDiagMode, diskVolumeTrapInh=diskVolumeTrapInh, cardTemperature=cardTemperature, auditStatus=auditStatus, portEntry=portEntry, cmPrimarySID=cmPrimarySID, errEvRowStatus=errEvRowStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diagTestId=diagTestId, portNumber=portNumber, cardId=cardId, cadEquipmentMibConformance=cadEquipmentMibConformance, portDocsIfIndex=portDocsIfIndex, diskVolumeSize=diskVolumeSize, portDplxStatus=portDplxStatus, equipmentAudit=equipmentAudit, cmResetUpTime=cmResetUpTime, errEvLogLevel=errEvLogLevel, TestTransactionId=TestTransactionId, shelfSwVersion=shelfSwVersion, cardTempNoReportNotification=cardTempNoReportNotification, errEventId=errEventId, auditResult=auditResult, portBgpId=portBgpId, TestType=TestType, shelfName=shelfName, equipmentState=equipmentState, cardSpareGroupId=cardSpareGroupId, cardAction=cardAction, cardName=cardName, trapSeverity=trapSeverity, cardLastBootVersion=cardLastBootVersion, diskVolumeEntry=diskVolumeEntry, auditTable=auditTable, cmVendor=cmVendor, cmResetClearNotification=cmResetClearNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, TestId=TestId, diskDriveNumber=diskDriveNumber, cardTestType=cardTestType, cardSwVersion=cardSwVersion, equipmentAuditGroup=equipmentAuditGroup, cardTrapInh=cardTrapInh, cardShelfId=cardShelfId, portSecState=portSecState, equipmentCardGroup=equipmentCardGroup, diskVolumeShelfId=diskVolumeShelfId, equipmentError=equipmentError, systemClock=systemClock, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeTable=diskVolumeTable, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestCommand=cardTestCommand, equipmentShelfGroup=equipmentShelfGroup, TestCommand=TestCommand, auditEntry=auditEntry, diskVolumeName=diskVolumeName, cardRamSize=cardRamSize, diskVolumeId=diskVolumeId, cardPrState=cardPrState, equipmentDiag=equipmentDiag, portLastChangeTime=portLastChangeTime, cmResetStatus=cmResetStatus, cmResetNotification=cmResetNotification, systemGeneralGroup=systemGeneralGroup, portRxFlowControlDetected=portRxFlowControlDetected, cardCpuType=cardCpuType, cadEquipmentMib=cadEquipmentMib, auditTotalFailedCount=auditTotalFailedCount, cardTestResultNotification=cardTestResultNotification, portId=portId, portConnectorId=portConnectorId, portCardSubType=portCardSubType, cardTestEntry=cardTestEntry, cmResetInfo=cmResetInfo, errEvRecoveryEnabled=errEvRecoveryEnabled, equipmentStateGroup=equipmentStateGroup, cardTestIndex=cardTestIndex, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, consoleOutput=consoleOutput, downstreamPowerLoss=downstreamPowerLoss, portDetectedMode=portDetectedMode, verboseLevel=verboseLevel, portAdminState=portAdminState, diskVolumeDriveId=diskVolumeDriveId, cmDevice=cmDevice, cardDplxStatusChange=cardDplxStatusChange, cardPicHwVersion=cardPicHwVersion)
