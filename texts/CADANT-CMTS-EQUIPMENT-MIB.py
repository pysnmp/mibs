...
PortDetectedMode, SecondaryState, PortId, PrimaryState, PortType, EqActionType, FirmwareSource, FlowControlMode, PicType, PortMode, DuplexStatus, UnknownState, CardType, AdminState, ShelfId, CardId, DiskVolumeUsageLevel, CardSubType = mibBuilder.importSymbols("CADANT-TC", "PortDetectedMode", "SecondaryState", "PortId", "PrimaryState", "PortType", "EqActionType", "FirmwareSource", "FlowControlMode", "PicType", "PortMode", "DuplexStatus", "UnknownState", "CardType", "AdminState", "ShelfId", "CardId", "DiskVolumeUsageLevel", "CardSubType")
...
TruthValue, RowStatus, TimeStamp, DateAndTime, TextualConvention, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "RowStatus", "TimeStamp", "DateAndTime", "TextualConvention", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", TestSchedule=TestSchedule, portDplxStatusChange=portDplxStatusChange, cardFpgaSource=cardFpgaSource, portDocsIfIndex=portDocsIfIndex, systemKey=systemKey, auditFailedCount=auditFailedCount, auditTotalFailedCount=auditTotalFailedCount, cmMacAddress=cmMacAddress, equipmentDiag=equipmentDiag, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTempOverHeatNotification=cardTempOverHeatNotification, errEvRowStatus=errEvRowStatus, cardTemperature=cardTemperature, TestType=TestType, equipmentStateGroup=equipmentStateGroup, cardNumber=cardNumber, errEvLoggingEnabled=errEvLoggingEnabled, portCurrDsPower=portCurrDsPower, portNumber=portNumber, cardTestName=cardTestName, portMinDsPower=portMinDsPower, systemClock=systemClock, diskVolumeShelfId=diskVolumeShelfId, cardCpuSpeed=cardCpuSpeed, downstreamPowerLoss=downstreamPowerLoss, cardDplxStatus=cardDplxStatus, portMaxDsPower=portMaxDsPower, trapCounter=trapCounter, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTestTable=cardTestTable, cardTestIndex=cardTestIndex, portTrapInh=portTrapInh, cardLastResetReason=cardLastResetReason, verboseLevel=verboseLevel, auditLogThrottle=auditLogThrottle, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, errEventId=errEventId, cardSwVersion=cardSwVersion, cardSpareGroupId=cardSpareGroupId, trapSeverity=trapSeverity, portShelfId=portShelfId, shelfName=shelfName, cardLastBootSource=cardLastBootSource, cardTestResult=cardTestResult, diskVolumeNumber=diskVolumeNumber, cardLastBootVersion=cardLastBootVersion, portMode=portMode, portTxFlowControlDetected=portTxFlowControlDetected, cardHwVersion=cardHwVersion, cardTestEntry=cardTestEntry, TestScheduleCommand=TestScheduleCommand, equipmentAuditGroup=equipmentAuditGroup, cmResetStatus=cmResetStatus, diskVolumeTable=diskVolumeTable, cardSubType=cardSubType, consoleOutput=consoleOutput, portDplxStatus=portDplxStatus, portTxFlowControlMode=portTxFlowControlMode, cardBusSpeed=cardBusSpeed, errEvRecoveryEnabled=errEvRecoveryEnabled, systemGeneralGroup=systemGeneralGroup, cardRamSize=cardRamSize, portType=portType, auditEntry=auditEntry, portAction=portAction, cardPrState=cardPrState, cardTestTime=cardTestTime, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardName=cardName, cardAdminState=cardAdminState, equipmentShelfGroup=equipmentShelfGroup, portAdminState=portAdminState, portMacAddress=portMacAddress, remainInDiagMode=remainInDiagMode, equipmentAudit=equipmentAudit, cardUpTime=cardUpTime, fpgaErrorEventTable=fpgaErrorEventTable, TestCommand=TestCommand, cardTable=cardTable, cardTestSchedule=cardTestSchedule, cardAction=cardAction, portPrStateChange=portPrStateChange, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portDetectedMode=portDetectedMode, auditResult=auditResult, diskVolumeGroup=diskVolumeGroup, cardPicDetected=cardPicDetected, cardTestResultDesc=cardTestResultDesc, diskVolumeCardId=diskVolumeCardId, cardDetectedChange=cardDetectedChange, auditTotalPassedCount=auditTotalPassedCount, cmDevice=cmDevice, cardPrStateChange=cardPrStateChange, shelfSwVersion=shelfSwVersion, auditName=auditName, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeEntry=diskVolumeEntry, diagTestId=diagTestId, cmVendor=cmVendor, cardTestType=cardTestType, TestTransactionId=TestTransactionId, cardFwUpdateStatus=cardFwUpdateStatus, equipmentState=equipmentState, cmRegistrationNotification=cmRegistrationNotification, cardTestTransId=cardTestTransId, portCardId=portCardId, auditShelfId=auditShelfId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskDriveNumber=diskDriveNumber, cardTrapInh=cardTrapInh, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portBgpId=portBgpId, diskVolumeDiskSize=diskVolumeDiskSize, cmResetNotification=cmResetNotification, portSecState=portSecState, portCardSubType=portCardSubType, equipmentError=equipmentError, cardSubDetected=cardSubDetected, TestId=TestId, cardEntry=cardEntry, cardLastChangeTime=cardLastChangeTime, diskVolumeId=diskVolumeId, equipmentTraps=equipmentTraps, cadEquipmentMibGroup=cadEquipmentMibGroup, eqDiagConfig=eqDiagConfig, cardTestCommand=cardTestCommand, portSecStateChange=portSecStateChange, diskVolumeFileName=diskVolumeFileName, portConnectorId=portConnectorId, cardSpareGroupMode=cardSpareGroupMode, cardCPLDVersion=cardCPLDVersion, PYSNMP_MODULE_ID=cadEquipmentMib, systemGeneral=systemGeneral, shelfObjects=shelfObjects, equipmentTbl=equipmentTbl, cardFlashSize=cardFlashSize, auditDescription=auditDescription, cardFwVersion=cardFwVersion, cardTestResultNotification=cardTestResultNotification, cmResetClearNotification=cmResetClearNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, cardCarrierFwVersion=cardCarrierFwVersion, cmResetUpTime=cmResetUpTime, cmResetReason=cmResetReason, cardBootVersion=cardBootVersion, equipmentCardGroup=equipmentCardGroup, diskVolumeTrapInh=diskVolumeTrapInh, portTable=portTable, cardTempNoReportNotification=cardTempNoReportNotification, errEvLogLevel=errEvLogLevel, cardPicHwVersion=cardPicHwVersion, equipmentPortGroup=equipmentPortGroup, diskVolumeName=diskVolumeName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardCarrierHwVersion=cardCarrierHwVersion, portNumChans=portNumChans, portRxFlowControlMode=portRxFlowControlMode, cardSecStateChange=cardSecStateChange, cardCpuType=cardCpuType, auditCardId=auditCardId, portMacIfIndex=portMacIfIndex, cardTestDescription=cardTestDescription, cardCarrierSerialNum=cardCarrierSerialNum, auditLogOutput=auditLogOutput, equipmentCardTestGroup=equipmentCardTestGroup, cardType=cardType, diskVolumeSize=diskVolumeSize, auditAutoScheduling=auditAutoScheduling, cardId=cardId, cardDplxStatusChange=cardDplxStatusChange, cardTemperatureHighError=cardTemperatureHighError, portId=portId, shelfNumber=shelfNumber, auditId=auditId, cadEquipmentMibConformance=cadEquipmentMibConformance, cardPicSerialNum=cardPicSerialNum, cardSecState=cardSecState, auditTable=auditTable, equipmentDiagGroup=equipmentDiagGroup, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditTime=auditTime, portEntry=portEntry, portPrState=portPrState, auditStatus=auditStatus, portDescription=portDescription, diskVolumeUsageLevel=diskVolumeUsageLevel, portRxFlowControlDetected=portRxFlowControlDetected, cardDetected=cardDetected, auditCycleCount=auditCycleCount, cmPrimarySID=cmPrimarySID, cardNumPorts=cardNumPorts, cardTestScheduleCommand=cardTestScheduleCommand, auditPassedCount=auditPassedCount, cardShelfId=cardShelfId, auditCommand=auditCommand, diskVolumeDriveId=diskVolumeDriveId, portLastChangeTime=portLastChangeTime, cardSerialNum=cardSerialNum, cmResetInfo=cmResetInfo, TestResult=TestResult, cardShelfTestIndex=cardShelfTestIndex, cmIpAddress=cmIpAddress, cmUChannelID=cmUChannelID, cardTestId=cardTestId, cadEquipmentMib=cadEquipmentMib)
