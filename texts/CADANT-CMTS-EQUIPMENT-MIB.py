...
PortType, EqActionType, CardType, DuplexStatus, PortId, PortDetectedMode, CardId, PicType, PrimaryState, UnknownState, FirmwareSource, ShelfId, PortMode, AdminState, FlowControlMode, DiskVolumeUsageLevel, CardSubType, SecondaryState = mibBuilder.importSymbols("CADANT-TC", "PortType", "EqActionType", "CardType", "DuplexStatus", "PortId", "PortDetectedMode", "CardId", "PicType", "PrimaryState", "UnknownState", "FirmwareSource", "ShelfId", "PortMode", "AdminState", "FlowControlMode", "DiskVolumeUsageLevel", "CardSubType", "SecondaryState")
...
TextualConvention, TimeStamp, RowStatus, DateAndTime, MacAddress, DisplayString, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TimeStamp", "RowStatus", "DateAndTime", "MacAddress", "DisplayString", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", portTrapInh=portTrapInh, diskVolumeUsageLevel=diskVolumeUsageLevel, portDescription=portDescription, cmResetInfo=cmResetInfo, errEventId=errEventId, equipmentStateGroup=equipmentStateGroup, TestId=TestId, cardShelfTestIndex=cardShelfTestIndex, diskVolumeTrapInh=diskVolumeTrapInh, errEvLogLevel=errEvLogLevel, portBgpId=portBgpId, TestTransactionId=TestTransactionId, diskVolumeNumber=diskVolumeNumber, auditTotalPassedCount=auditTotalPassedCount, cardTrapInh=cardTrapInh, portNumChans=portNumChans, diskVolumeName=diskVolumeName, remainInDiagMode=remainInDiagMode, auditAutoScheduling=auditAutoScheduling, diskVolumeShelfId=diskVolumeShelfId, cardTestResultDesc=cardTestResultDesc, cardSpareGroupId=cardSpareGroupId, portDetectedMode=portDetectedMode, cmVendor=cmVendor, auditPassedCount=auditPassedCount, portRxFlowControlDetected=portRxFlowControlDetected, TestScheduleCommand=TestScheduleCommand, portAdminState=portAdminState, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cmIpAddress=cmIpAddress, cardId=cardId, cardTestTransId=cardTestTransId, cardFlashSize=cardFlashSize, cardFwVersion=cardFwVersion, cardLastResetReason=cardLastResetReason, cardTemperature=cardTemperature, cardFpgaSource=cardFpgaSource, cardTestTable=cardTestTable, errEvLoggingEnabled=errEvLoggingEnabled, trapSeverity=trapSeverity, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTempOverHeatNotification=cardTempOverHeatNotification, equipmentDiagGroup=equipmentDiagGroup, cardTemperatureHighError=cardTemperatureHighError, TestCommand=TestCommand, portSecState=portSecState, cardSwVersion=cardSwVersion, errEvRecoveryEnabled=errEvRecoveryEnabled, portSecStateChange=portSecStateChange, cardNumPorts=cardNumPorts, equipmentAudit=equipmentAudit, portMinDsPower=portMinDsPower, diskVolumeUsageNotification=diskVolumeUsageNotification, auditTotalFailedCount=auditTotalFailedCount, cardAdminState=cardAdminState, cardEntry=cardEntry, portCardId=portCardId, portDocsIfIndex=portDocsIfIndex, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeFileName=diskVolumeFileName, fpgaErrorEventGroup=fpgaErrorEventGroup, auditLogThrottle=auditLogThrottle, cardType=cardType, cmDevice=cmDevice, auditDescription=auditDescription, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTestName=cardTestName, systemGeneral=systemGeneral, portTable=portTable, equipmentCardTestGroup=equipmentCardTestGroup, portNumber=portNumber, TestResult=TestResult, cardBootVersion=cardBootVersion, cardUpTime=cardUpTime, portDplxStatus=portDplxStatus, equipmentState=equipmentState, portRxFlowControlMode=portRxFlowControlMode, diskVolumeSize=diskVolumeSize, diskVolumeDiskSize=diskVolumeDiskSize, PYSNMP_MODULE_ID=cadEquipmentMib, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, diagTestId=diagTestId, auditTime=auditTime, cmUChannelID=cmUChannelID, cardDetected=cardDetected, equipmentAuditGroup=equipmentAuditGroup, diskVolumeCardId=diskVolumeCardId, cardSerialNum=cardSerialNum, cardTestDescription=cardTestDescription, errEvRowStatus=errEvRowStatus, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portEntry=portEntry, diskVolumeId=diskVolumeId, portConnectorId=portConnectorId, diskVolumeTable=diskVolumeTable, cardSpareGroupMode=cardSpareGroupMode, cmMacAddress=cmMacAddress, cmRegistrationNotification=cmRegistrationNotification, TestSchedule=TestSchedule, cardBusSpeed=cardBusSpeed, systemKey=systemKey, cmPrimarySID=cmPrimarySID, cadEquipmentMibCompliance=cadEquipmentMibCompliance, equipmentCardGroup=equipmentCardGroup, portCardSubType=portCardSubType, cardPicHwVersion=cardPicHwVersion, cardName=cardName, diskVolumeGroup=diskVolumeGroup, auditEntry=auditEntry, cardCPLDVersion=cardCPLDVersion, portLastChangeTime=portLastChangeTime, cmResetStatus=cmResetStatus, shelfName=shelfName, auditCycleCount=auditCycleCount, cardSecStateChange=cardSecStateChange, diskVolumeDriveId=diskVolumeDriveId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestId=cardTestId, cardTestType=cardTestType, cardTestSchedule=cardTestSchedule, cardPicDetected=cardPicDetected, portMacIfIndex=portMacIfIndex, cardCarrierSerialNum=cardCarrierSerialNum, cardSubType=cardSubType, equipmentDiag=equipmentDiag, diskVolumeEntry=diskVolumeEntry, auditStatus=auditStatus, equipmentTraps=equipmentTraps, cardAction=cardAction, systemClock=systemClock, cardCpuType=cardCpuType, auditCommand=auditCommand, portCurrDsPower=portCurrDsPower, fpgaErrorEventTable=fpgaErrorEventTable, cardLastBootSource=cardLastBootSource, downstreamPowerLoss=downstreamPowerLoss, auditLogOutput=auditLogOutput, cardCarrierFwVersion=cardCarrierFwVersion, auditTable=auditTable, cardDplxStatusChange=cardDplxStatusChange, cardTestResult=cardTestResult, portPrStateChange=portPrStateChange, cardLastChangeTime=cardLastChangeTime, auditId=auditId, portMacAddress=portMacAddress, shelfSwVersion=shelfSwVersion, fpgaErrorEventEntry=fpgaErrorEventEntry, auditShelfId=auditShelfId, equipmentTbl=equipmentTbl, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardTestResultNotification=cardTestResultNotification, verboseLevel=verboseLevel, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentShelfGroup=equipmentShelfGroup, cardPicSerialNum=cardPicSerialNum, auditFailedCount=auditFailedCount, cmResetNotification=cmResetNotification, cardTempNoReportNotification=cardTempNoReportNotification, cardPrStateChange=cardPrStateChange, cardTable=cardTable, cardPrState=cardPrState, portMode=portMode, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTxFlowControlMode=portTxFlowControlMode, cardTestIndex=cardTestIndex, cadEquipmentMib=cadEquipmentMib, cardNumber=cardNumber, cardCpuSpeed=cardCpuSpeed, cardSubDetected=cardSubDetected, cardDetectedChange=cardDetectedChange, portShelfId=portShelfId, systemGeneralGroup=systemGeneralGroup, cardSecState=cardSecState, portMaxDsPower=portMaxDsPower, portPrState=portPrState, portAction=portAction, cardShelfId=cardShelfId, diskDriveNumber=diskDriveNumber, consoleOutput=consoleOutput, cardRamSize=cardRamSize, cardHwVersion=cardHwVersion, TestType=TestType, portDplxStatusChange=portDplxStatusChange, cardLastBootVersion=cardLastBootVersion, shelfObjects=shelfObjects, auditResult=auditResult, portType=portType, portId=portId, cmResetClearNotification=cmResetClearNotification, trapCounter=trapCounter, cardCarrierHwVersion=cardCarrierHwVersion, cmResetReason=cmResetReason, equipmentError=equipmentError, cardFwUpdateStatus=cardFwUpdateStatus, eqDiagConfig=eqDiagConfig, cardTestCommand=cardTestCommand, cardTemperatureHighWarn=cardTemperatureHighWarn, auditName=auditName, cardDplxStatus=cardDplxStatus, cardTestTime=cardTestTime, cardTestScheduleCommand=cardTestScheduleCommand, cmResetUpTime=cmResetUpTime, cardTestEntry=cardTestEntry, shelfNumber=shelfNumber, auditCardId=auditCardId, equipmentPortGroup=equipmentPortGroup)
