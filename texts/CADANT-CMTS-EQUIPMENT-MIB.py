...
DuplexStatus, CardSubType, PortMode, PicType, FlowControlMode, ShelfId, SecondaryState, EqActionType, CardType, FirmwareSource, PrimaryState, UnknownState, PortType, CardId, DiskVolumeUsageLevel, PortId, PortDetectedMode, AdminState = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "CardSubType", "PortMode", "PicType", "FlowControlMode", "ShelfId", "SecondaryState", "EqActionType", "CardType", "FirmwareSource", "PrimaryState", "UnknownState", "PortType", "CardId", "DiskVolumeUsageLevel", "PortId", "PortDetectedMode", "AdminState")
...
DisplayString, TimeStamp, DateAndTime, TextualConvention, TruthValue, MacAddress, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "DateAndTime", "TextualConvention", "TruthValue", "MacAddress", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, portTable=portTable, cardTestEntry=cardTestEntry, portNumber=portNumber, diskVolumeCardId=diskVolumeCardId, portTrapInh=portTrapInh, diskVolumeFileName=diskVolumeFileName, cardSpareGroupId=cardSpareGroupId, cardCpuSpeed=cardCpuSpeed, portMacIfIndex=portMacIfIndex, auditAutoScheduling=auditAutoScheduling, equipmentDiagGroup=equipmentDiagGroup, cardTestDescription=cardTestDescription, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, equipmentCardTestGroup=equipmentCardTestGroup, cardBusSpeed=cardBusSpeed, cardCarrierHwVersion=cardCarrierHwVersion, downstreamPowerLoss=downstreamPowerLoss, auditTime=auditTime, TestType=TestType, systemClock=systemClock, portCardId=portCardId, cardLastChangeTime=cardLastChangeTime, cardTestId=cardTestId, auditEntry=auditEntry, cardLastBootVersion=cardLastBootVersion, cmResetReason=cmResetReason, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardCpuType=cardCpuType, consoleOutput=consoleOutput, auditPassedCount=auditPassedCount, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestIndex=cardTestIndex, cardFwUpdateStatus=cardFwUpdateStatus, cardLastBootSource=cardLastBootSource, errEvRecoveryEnabled=errEvRecoveryEnabled, errEventId=errEventId, cardAction=cardAction, auditLogOutput=auditLogOutput, cardNumber=cardNumber, cmResetInfo=cmResetInfo, portCardSubType=portCardSubType, cardPicHwVersion=cardPicHwVersion, verboseLevel=verboseLevel, portLastChangeTime=portLastChangeTime, trapCounter=trapCounter, cardTestSchedule=cardTestSchedule, diskVolumeNumber=diskVolumeNumber, equipmentPortGroup=equipmentPortGroup, cardType=cardType, equipmentAuditGroup=equipmentAuditGroup, remainInDiagMode=remainInDiagMode, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardSubDetected=cardSubDetected, portNumChans=portNumChans, portAdminState=portAdminState, cadEquipmentMibConformance=cadEquipmentMibConformance, shelfObjects=shelfObjects, cardFlashSize=cardFlashSize, equipmentAudit=equipmentAudit, systemKey=systemKey, cardSubType=cardSubType, cardDetected=cardDetected, diskVolumeId=diskVolumeId, cardSecStateChange=cardSecStateChange, equipmentTraps=equipmentTraps, cardTempNoReportNotification=cardTempNoReportNotification, diskVolumeUsageNotification=diskVolumeUsageNotification, portDocsIfIndex=portDocsIfIndex, shelfSwVersion=shelfSwVersion, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageLevel=diskVolumeUsageLevel, cardRamSize=cardRamSize, cardTestResult=cardTestResult, TestSchedule=TestSchedule, auditCardId=auditCardId, cmUChannelID=cmUChannelID, auditTotalFailedCount=auditTotalFailedCount, portSecState=portSecState, fpgaErrorEventTable=fpgaErrorEventTable, diskVolumeTable=diskVolumeTable, diskVolumeDiskSize=diskVolumeDiskSize, equipmentStateGroup=equipmentStateGroup, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardTestCommand=cardTestCommand, cmMacAddress=cmMacAddress, cmPrimarySID=cmPrimarySID, auditResult=auditResult, portAction=portAction, equipmentDiag=equipmentDiag, diskVolumeSize=diskVolumeSize, cadEquipmentMib=cadEquipmentMib, auditLogThrottle=auditLogThrottle, cardPicDetected=cardPicDetected, cardCarrierSerialNum=cardCarrierSerialNum, cardLastResetReason=cardLastResetReason, auditId=auditId, cardShelfTestIndex=cardShelfTestIndex, cardTestTransId=cardTestTransId, cmResetNotification=cmResetNotification, portMaxDsPower=portMaxDsPower, cardName=cardName, auditTable=auditTable, errEvLoggingEnabled=errEvLoggingEnabled, cardTestName=cardTestName, cardTemperature=cardTemperature, TestCommand=TestCommand, cardBootVersion=cardBootVersion, portMode=portMode, portDetectedMode=portDetectedMode, cmResetStatus=cmResetStatus, cadEquipmentMibGroup=cadEquipmentMibGroup, cardShelfId=cardShelfId, cardAdminState=cardAdminState, auditFailedCount=auditFailedCount, cardCPLDVersion=cardCPLDVersion, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeName=diskVolumeName, equipmentCardGroup=equipmentCardGroup, cardPicSerialNum=cardPicSerialNum, cardTemperatureHighError=cardTemperatureHighError, cardFpgaSource=cardFpgaSource, cardSecState=cardSecState, cardDplxStatusChange=cardDplxStatusChange, cardFwVersion=cardFwVersion, equipmentTbl=equipmentTbl, portType=portType, cmIpAddress=cmIpAddress, portId=portId, diskVolumeDriveId=diskVolumeDriveId, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSwVersion=cardSwVersion, cardSerialNum=cardSerialNum, auditCycleCount=auditCycleCount, equipmentState=equipmentState, shelfName=shelfName, cmResetUpTime=cmResetUpTime, fpgaErrorEventEntry=fpgaErrorEventEntry, cardTable=cardTable, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, trapSeverity=trapSeverity, diskVolumeUsagePercentage=diskVolumeUsagePercentage, PYSNMP_MODULE_ID=cadEquipmentMib, portBgpId=portBgpId, cardTestResultNotification=cardTestResultNotification, systemGeneralGroup=systemGeneralGroup, systemGeneral=systemGeneral, diskVolumeGroup=diskVolumeGroup, auditDescription=auditDescription, portMinDsPower=portMinDsPower, auditStatus=auditStatus, cardTestScheduleCommand=cardTestScheduleCommand, auditShelfId=auditShelfId, auditName=auditName, cardPrStateChange=cardPrStateChange, portPrStateChange=portPrStateChange, cardTrapInh=cardTrapInh, portShelfId=portShelfId, equipmentError=equipmentError, cardId=cardId, auditTotalPassedCount=auditTotalPassedCount, cardDetectedChange=cardDetectedChange, eqDiagConfig=eqDiagConfig, cardPrState=cardPrState, cardHwVersion=cardHwVersion, portDplxStatusChange=portDplxStatusChange, portConnectorId=portConnectorId, cmRegistrationNotification=cmRegistrationNotification, errEvRowStatus=errEvRowStatus, cardTestType=cardTestType, cmVendor=cmVendor, portRxFlowControlDetected=portRxFlowControlDetected, TestResult=TestResult, cardEntry=cardEntry, cardTestTime=cardTestTime, portPrState=portPrState, cardTestResultDesc=cardTestResultDesc, portSecStateChange=portSecStateChange, cardSpareGroupMode=cardSpareGroupMode, cardUpTime=cardUpTime, diskVolumeTrapInh=diskVolumeTrapInh, portTxFlowControlMode=portTxFlowControlMode, portDplxStatus=portDplxStatus, equipmentShelfGroup=equipmentShelfGroup, shelfNumber=shelfNumber, cardTestTable=cardTestTable, diskVolumeEntry=diskVolumeEntry, errEvLogLevel=errEvLogLevel, cardCarrierFwVersion=cardCarrierFwVersion, portDescription=portDescription, auditCommand=auditCommand, portEntry=portEntry, fpgaErrorEventGroup=fpgaErrorEventGroup, diskVolumeShelfId=diskVolumeShelfId, cmDevice=cmDevice, portCurrDsPower=portCurrDsPower, TestTransactionId=TestTransactionId, cmResetClearNotification=cmResetClearNotification, cardDplxStatus=cardDplxStatus, diskDriveNumber=diskDriveNumber, TestId=TestId, cardNumPorts=cardNumPorts, cardTemperatureHighWarn=cardTemperatureHighWarn, portMacAddress=portMacAddress, TestScheduleCommand=TestScheduleCommand, diagTestId=diagTestId)
