...
EqActionType, AdminState, DuplexStatus, PortDetectedMode, PortType, SecondaryState, CardId, FirmwareSource, PicType, PortId, PrimaryState, CardSubType, PortMode, FlowControlMode, ShelfId, UnknownState, DiskVolumeUsageLevel, CardType = mibBuilder.importSymbols("CADANT-TC", "EqActionType", "AdminState", "DuplexStatus", "PortDetectedMode", "PortType", "SecondaryState", "CardId", "FirmwareSource", "PicType", "PortId", "PrimaryState", "CardSubType", "PortMode", "FlowControlMode", "ShelfId", "UnknownState", "DiskVolumeUsageLevel", "CardType")
...
RowStatus, DisplayString, TruthValue, TextualConvention, DateAndTime, TimeStamp, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "DisplayString", "TruthValue", "TextualConvention", "DateAndTime", "TimeStamp", "MacAddress")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditTotalFailedCount=auditTotalFailedCount, equipmentError=equipmentError, equipmentDiagGroup=equipmentDiagGroup, cardTempNoReportNotification=cardTempNoReportNotification, errEvLoggingEnabled=errEvLoggingEnabled, fpgaErrorEventGroup=fpgaErrorEventGroup, cardSpareGroupMode=cardSpareGroupMode, cardNumber=cardNumber, auditLogThrottle=auditLogThrottle, portType=portType, cardNumPorts=cardNumPorts, portMaxDsPower=portMaxDsPower, TestTransactionId=TestTransactionId, diskVolumeShelfId=diskVolumeShelfId, cardTestId=cardTestId, errEvLogLevel=errEvLogLevel, cardTemperatureHighError=cardTemperatureHighError, cardRamSize=cardRamSize, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, auditEntry=auditEntry, cardDetectedChange=cardDetectedChange, cmDevice=cmDevice, cardType=cardType, portId=portId, cardTestDescription=cardTestDescription, equipmentAuditGroup=equipmentAuditGroup, cmResetUpTime=cmResetUpTime, cardPrStateChange=cardPrStateChange, cardTempOverHeatNotification=cardTempOverHeatNotification, cardFwVersion=cardFwVersion, portMacIfIndex=portMacIfIndex, cardCpuSpeed=cardCpuSpeed, cardSwVersion=cardSwVersion, cmResetStatus=cmResetStatus, cardSubDetected=cardSubDetected, shelfSwVersion=shelfSwVersion, cardTemperature=cardTemperature, diskVolumeId=diskVolumeId, portCardId=portCardId, cardUpTime=cardUpTime, cardAdminState=cardAdminState, portEntry=portEntry, cardBusSpeed=cardBusSpeed, cmResetReason=cmResetReason, TestCommand=TestCommand, cmIpAddress=cmIpAddress, cardCarrierFwVersion=cardCarrierFwVersion, portRxFlowControlMode=portRxFlowControlMode, diskVolumeName=diskVolumeName, TestType=TestType, cardDplxStatus=cardDplxStatus, portMode=portMode, equipmentDiag=equipmentDiag, trapSeverity=trapSeverity, cmMacAddress=cmMacAddress, portTxFlowControlMode=portTxFlowControlMode, auditName=auditName, cardLastResetReason=cardLastResetReason, auditLogOutput=auditLogOutput, diskVolumeDriveId=diskVolumeDriveId, TestId=TestId, cmResetInfo=cmResetInfo, auditResult=auditResult, cardBootVersion=cardBootVersion, downstreamPowerLoss=downstreamPowerLoss, auditTable=auditTable, remainInDiagMode=remainInDiagMode, cardEntry=cardEntry, cardTestName=cardTestName, systemGeneral=systemGeneral, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmUChannelID=cmUChannelID, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardAction=cardAction, portDplxStatusChange=portDplxStatusChange, errEvRowStatus=errEvRowStatus, diskVolumeCardId=diskVolumeCardId, cardDetected=cardDetected, portMinDsPower=portMinDsPower, diskDriveNumber=diskDriveNumber, consoleOutput=consoleOutput, diskVolumeUsageNotification=diskVolumeUsageNotification, cardTestType=cardTestType, portRxFlowControlDetected=portRxFlowControlDetected, cardTestResult=cardTestResult, portBgpId=portBgpId, cardTestCommand=cardTestCommand, auditCardId=auditCardId, portCardSubType=portCardSubType, cmResetClearNotification=cmResetClearNotification, portAdminState=portAdminState, portNumber=portNumber, diskVolumeTable=diskVolumeTable, equipmentTbl=equipmentTbl, cardCPLDVersion=cardCPLDVersion, portSecStateChange=portSecStateChange, cardTable=cardTable, auditTime=auditTime, errEvRecoveryEnabled=errEvRecoveryEnabled, eqDiagConfig=eqDiagConfig, equipmentShelfGroup=equipmentShelfGroup, cardPicSerialNum=cardPicSerialNum, diskVolumeTrapInh=diskVolumeTrapInh, PYSNMP_MODULE_ID=cadEquipmentMib, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeSize=diskVolumeSize, portPrStateChange=portPrStateChange, auditFailedCount=auditFailedCount, diskVolumeUsageLevel=diskVolumeUsageLevel, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portDocsIfIndex=portDocsIfIndex, cardTestScheduleCommand=cardTestScheduleCommand, shelfNumber=shelfNumber, cardPicHwVersion=cardPicHwVersion, auditDescription=auditDescription, cmResetNotification=cmResetNotification, cardSecState=cardSecState, portConnectorId=portConnectorId, cardTestTransId=cardTestTransId, auditCommand=auditCommand, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, diskVolumeGroup=diskVolumeGroup, diskVolumeNumber=diskVolumeNumber, fpgaErrorEventEntry=fpgaErrorEventEntry, systemKey=systemKey, portCurrDsPower=portCurrDsPower, TestResult=TestResult, shelfObjects=shelfObjects, cardPrState=cardPrState, shelfName=shelfName, portDescription=portDescription, auditStatus=auditStatus, portShelfId=portShelfId, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardLastBootSource=cardLastBootSource, portAction=portAction, auditPassedCount=auditPassedCount, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portMacAddress=portMacAddress, systemClock=systemClock, cardShelfTestIndex=cardShelfTestIndex, portDplxStatus=portDplxStatus, cardFpgaSource=cardFpgaSource, cardDplxStatusChange=cardDplxStatusChange, cardShelfId=cardShelfId, auditId=auditId, equipmentPortGroup=equipmentPortGroup, cardTestEntry=cardTestEntry, portTable=portTable, cardName=cardName, diagTestId=diagTestId, cardSubType=cardSubType, equipmentState=equipmentState, cadEquipmentMib=cadEquipmentMib, cadEquipmentMibGroup=cadEquipmentMibGroup, cardSecStateChange=cardSecStateChange, cardTrapInh=cardTrapInh, cardTestIndex=cardTestIndex, trapCounter=trapCounter, auditAutoScheduling=auditAutoScheduling, portTrapInh=portTrapInh, cardTemperatureHighWarn=cardTemperatureHighWarn, fpgaErrorEventTable=fpgaErrorEventTable, cardTestTime=cardTestTime, cardFlashSize=cardFlashSize, portDetectedMode=portDetectedMode, cardLastChangeTime=cardLastChangeTime, diskVolumeFileName=diskVolumeFileName, equipmentCardGroup=equipmentCardGroup, cardLastBootVersion=cardLastBootVersion, cardSerialNum=cardSerialNum, cardCpuType=cardCpuType, auditShelfId=auditShelfId, equipmentCardTestGroup=equipmentCardTestGroup, cardTestSchedule=cardTestSchedule, cardCarrierSerialNum=cardCarrierSerialNum, cardTestTable=cardTestTable, diskVolumeEntry=diskVolumeEntry, cardId=cardId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portTxFlowControlDetected=portTxFlowControlDetected, auditTotalPassedCount=auditTotalPassedCount, equipmentTraps=equipmentTraps, systemGeneralGroup=systemGeneralGroup, cardFwUpdateStatus=cardFwUpdateStatus, cardHwVersion=cardHwVersion, portSecState=portSecState, cmRegistrationNotification=cmRegistrationNotification, cardPicDetected=cardPicDetected, cardTestResultNotification=cardTestResultNotification, auditCycleCount=auditCycleCount, cardCarrierHwVersion=cardCarrierHwVersion, cardTestResultDesc=cardTestResultDesc, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardSpareGroupId=cardSpareGroupId, cmPrimarySID=cmPrimarySID, TestScheduleCommand=TestScheduleCommand, portPrState=portPrState, diskVolumeDiskSize=diskVolumeDiskSize, portNumChans=portNumChans, verboseLevel=verboseLevel, cmVendor=cmVendor, equipmentStateGroup=equipmentStateGroup, equipmentAudit=equipmentAudit, errEventId=errEventId, portLastChangeTime=portLastChangeTime, TestSchedule=TestSchedule)
