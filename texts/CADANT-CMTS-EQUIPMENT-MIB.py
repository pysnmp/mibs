...
PortType, PicType, PortId, EqActionType, PortDetectedMode, CardSubType, FirmwareSource, PrimaryState, FlowControlMode, UnknownState, SecondaryState, AdminState, ShelfId, DuplexStatus, CardId, PortMode, DiskVolumeUsageLevel, CardType = mibBuilder.importSymbols("CADANT-TC", "PortType", "PicType", "PortId", "EqActionType", "PortDetectedMode", "CardSubType", "FirmwareSource", "PrimaryState", "FlowControlMode", "UnknownState", "SecondaryState", "AdminState", "ShelfId", "DuplexStatus", "CardId", "PortMode", "DiskVolumeUsageLevel", "CardType")
...
TextualConvention, TimeStamp, DateAndTime, DisplayString, MacAddress, TruthValue, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "TimeStamp", "DateAndTime", "DisplayString", "MacAddress", "TruthValue", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", consoleOutput=consoleOutput, portMode=portMode, cardTestResultDesc=cardTestResultDesc, equipmentDiagGroup=equipmentDiagGroup, auditTotalFailedCount=auditTotalFailedCount, shelfSwVersion=shelfSwVersion, cmResetClearNotification=cmResetClearNotification, cardCarrierSerialNum=cardCarrierSerialNum, cmResetUpTime=cmResetUpTime, cmResetNotification=cmResetNotification, portCardSubType=portCardSubType, diskVolumeNumber=diskVolumeNumber, diskVolumeId=diskVolumeId, diskVolumeTable=diskVolumeTable, cardSecStateChange=cardSecStateChange, cardTrapInh=cardTrapInh, portAction=portAction, cardHwVersion=cardHwVersion, cardAdminState=cardAdminState, cardTestScheduleCommand=cardTestScheduleCommand, cmVendor=cmVendor, portTxFlowControlMode=portTxFlowControlMode, diskVolumeUsageNotification=diskVolumeUsageNotification, portDetectedMode=portDetectedMode, cadEquipmentMibConformance=cadEquipmentMibConformance, diskVolumeCardId=diskVolumeCardId, auditLogOutput=auditLogOutput, cardShelfId=cardShelfId, diskVolumeUsageLevel=diskVolumeUsageLevel, cardDplxStatus=cardDplxStatus, errEvRecoveryEnabled=errEvRecoveryEnabled, portPrStateChange=portPrStateChange, cardDplxStatusChange=cardDplxStatusChange, cardTestId=cardTestId, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTable=cardTable, equipmentCardTestGroup=equipmentCardTestGroup, cardTemperatureHighError=cardTemperatureHighError, diskVolumeDriveId=diskVolumeDriveId, cardPicHwVersion=cardPicHwVersion, diskVolumeSize=diskVolumeSize, auditLogThrottle=auditLogThrottle, shelfNumber=shelfNumber, equipmentTbl=equipmentTbl, cardTestSchedule=cardTestSchedule, cardFlashSize=cardFlashSize, cmRegistrationNotification=cmRegistrationNotification, cardLastBootVersion=cardLastBootVersion, equipmentDiag=equipmentDiag, TestResult=TestResult, downstreamPowerLoss=downstreamPowerLoss, equipmentAuditGroup=equipmentAuditGroup, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, diskVolumeDiskSize=diskVolumeDiskSize, trapSeverity=trapSeverity, fpgaErrorEventEntry=fpgaErrorEventEntry, diskVolumeName=diskVolumeName, cardSpareGroupMode=cardSpareGroupMode, TestType=TestType, TestId=TestId, systemKey=systemKey, errEvLoggingEnabled=errEvLoggingEnabled, cardCpuSpeed=cardCpuSpeed, cardCpuType=cardCpuType, cardSubType=cardSubType, cardSecState=cardSecState, cardTestType=cardTestType, cardFwUpdateStatus=cardFwUpdateStatus, diskVolumeFileName=diskVolumeFileName, equipmentStateGroup=equipmentStateGroup, cardSpareGroupId=cardSpareGroupId, cardTestIndex=cardTestIndex, cardTestResult=cardTestResult, portType=portType, eqDiagConfig=eqDiagConfig, cardTestDescription=cardTestDescription, cardCPLDVersion=cardCPLDVersion, auditStatus=auditStatus, cadEquipmentMibCompliances=cadEquipmentMibCompliances, portId=portId, cardSubDetected=cardSubDetected, equipmentAudit=equipmentAudit, systemGeneral=systemGeneral, diskVolumeUsagePercentage=diskVolumeUsagePercentage, equipmentCardGroup=equipmentCardGroup, cardLastBootSource=cardLastBootSource, cardPicSerialNum=cardPicSerialNum, auditCycleCount=auditCycleCount, cardBusSpeed=cardBusSpeed, portMacAddress=portMacAddress, auditAutoScheduling=auditAutoScheduling, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cadEquipmentMib=cadEquipmentMib, cardName=cardName, auditDescription=auditDescription, auditShelfId=auditShelfId, portTable=portTable, auditCommand=auditCommand, cardType=cardType, cardUpTime=cardUpTime, remainInDiagMode=remainInDiagMode, shelfName=shelfName, portPrState=portPrState, portDplxStatus=portDplxStatus, systemClock=systemClock, portMinDsPower=portMinDsPower, cardPicDetected=cardPicDetected, cardTestName=cardTestName, diskVolumeTrapInh=diskVolumeTrapInh, cardDetectedChange=cardDetectedChange, diagTestId=diagTestId, TestScheduleCommand=TestScheduleCommand, cardTestTime=cardTestTime, auditCardId=auditCardId, cardLastChangeTime=cardLastChangeTime, portNumChans=portNumChans, cardRamSize=cardRamSize, portSecStateChange=portSecStateChange, diskVolumeShelfId=diskVolumeShelfId, cardTestTable=cardTestTable, cardFpgaSource=cardFpgaSource, shelfObjects=shelfObjects, diskDriveNumber=diskDriveNumber, cmResetInfo=cmResetInfo, cardTemperature=cardTemperature, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portCurrDsPower=portCurrDsPower, portCardId=portCardId, portDocsIfIndex=portDocsIfIndex, cmMacAddress=cmMacAddress, cmPrimarySID=cmPrimarySID, cardTemperatureHighWarn=cardTemperatureHighWarn, cardTestEntry=cardTestEntry, cardLastResetReason=cardLastResetReason, portRxFlowControlMode=portRxFlowControlMode, portRxFlowControlDetected=portRxFlowControlDetected, cardPrState=cardPrState, cardBootVersion=cardBootVersion, portSecState=portSecState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmDevice=cmDevice, portDplxStatusChange=portDplxStatusChange, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentError=equipmentError, cmUChannelID=cmUChannelID, cardShelfTestIndex=cardShelfTestIndex, diskVolumeGroup=diskVolumeGroup, portBgpId=portBgpId, errEvLogLevel=errEvLogLevel, PYSNMP_MODULE_ID=cadEquipmentMib, cmIpAddress=cmIpAddress, trapCounter=trapCounter, cardTempOverHeatNotification=cardTempOverHeatNotification, portMacIfIndex=portMacIfIndex, portMaxDsPower=portMaxDsPower, TestCommand=TestCommand, portDescription=portDescription, portTrapInh=portTrapInh, portEntry=portEntry, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardFwVersion=cardFwVersion, auditPassedCount=auditPassedCount, cardCarrierHwVersion=cardCarrierHwVersion, cmResetReason=cmResetReason, portAdminState=portAdminState, TestTransactionId=TestTransactionId, portConnectorId=portConnectorId, portTxFlowControlDetected=portTxFlowControlDetected, cardNumber=cardNumber, auditTime=auditTime, portNumber=portNumber, equipmentState=equipmentState, portShelfId=portShelfId, cmResetStatus=cmResetStatus, fpgaErrorEventGroup=fpgaErrorEventGroup, cardEntry=cardEntry, cardTestResultNotification=cardTestResultNotification, equipmentTraps=equipmentTraps, verboseLevel=verboseLevel, cardSwVersion=cardSwVersion, fpgaErrorEventTable=fpgaErrorEventTable, equipmentShelfGroup=equipmentShelfGroup, cardPrStateChange=cardPrStateChange, auditTable=auditTable, auditId=auditId, equipmentPortGroup=equipmentPortGroup, portLastChangeTime=portLastChangeTime, cardDetected=cardDetected, diskVolumeEntry=diskVolumeEntry, cardSerialNum=cardSerialNum, TestSchedule=TestSchedule, cardNumPorts=cardNumPorts, cardTestCommand=cardTestCommand, cadEquipmentMibGroup=cadEquipmentMibGroup, auditEntry=auditEntry, auditTotalPassedCount=auditTotalPassedCount, auditName=auditName, errEvRowStatus=errEvRowStatus, cardTempNoReportNotification=cardTempNoReportNotification, auditFailedCount=auditFailedCount, cardTestTransId=cardTestTransId, cardId=cardId, errEventId=errEventId, auditResult=auditResult, systemGeneralGroup=systemGeneralGroup, cardAction=cardAction, cardCarrierFwVersion=cardCarrierFwVersion)
