...
DuplexStatus, PrimaryState, PortMode, UnknownState, CardSubType, PortDetectedMode, EqActionType, SecondaryState, AdminState, FirmwareSource, CardType, FlowControlMode, PortType, ShelfId, DiskVolumeUsageLevel, CardId, PicType, PortId = mibBuilder.importSymbols("CADANT-TC", "DuplexStatus", "PrimaryState", "PortMode", "UnknownState", "CardSubType", "PortDetectedMode", "EqActionType", "SecondaryState", "AdminState", "FirmwareSource", "CardType", "FlowControlMode", "PortType", "ShelfId", "DiskVolumeUsageLevel", "CardId", "PicType", "PortId")
...
DateAndTime, TruthValue, TimeStamp, MacAddress, TextualConvention, DisplayString, RowStatus = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TruthValue", "TimeStamp", "MacAddress", "TextualConvention", "DisplayString", "RowStatus")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", diskVolumeShelfId=diskVolumeShelfId, auditStatus=auditStatus, cardPicSerialNum=cardPicSerialNum, portShelfId=portShelfId, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, cardBusSpeed=cardBusSpeed, cardEntry=cardEntry, cardTestName=cardTestName, errEvRecoveryEnabled=errEvRecoveryEnabled, auditTotalPassedCount=auditTotalPassedCount, cardPicDetected=cardPicDetected, auditTable=auditTable, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portPrStateChange=portPrStateChange, auditCommand=auditCommand, cardId=cardId, cmResetReason=cmResetReason, cardUpTime=cardUpTime, trapSeverity=trapSeverity, cardCPLDVersion=cardCPLDVersion, cmResetInfo=cmResetInfo, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, cardDetectedChange=cardDetectedChange, cardFwVersion=cardFwVersion, portMode=portMode, cardTestCommand=cardTestCommand, portTxFlowControlMode=portTxFlowControlMode, cardLastResetReason=cardLastResetReason, auditTime=auditTime, equipmentAuditGroup=equipmentAuditGroup, equipmentStateGroup=equipmentStateGroup, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, diskVolumeFileName=diskVolumeFileName, cardTestScheduleCommand=cardTestScheduleCommand, cardFlashSize=cardFlashSize, cardTemperatureHighWarn=cardTemperatureHighWarn, cardNumPorts=cardNumPorts, cmResetStatus=cmResetStatus, diskVolumeDriveId=diskVolumeDriveId, cmPrimarySID=cmPrimarySID, TestScheduleCommand=TestScheduleCommand, cardTempNoReportNotification=cardTempNoReportNotification, cmMacAddress=cmMacAddress, portCardId=portCardId, portPrState=portPrState, cardTestResultNotification=cardTestResultNotification, portDetectedMode=portDetectedMode, portTable=portTable, portLastChangeTime=portLastChangeTime, cardTestTransId=cardTestTransId, cmResetUpTime=cmResetUpTime, shelfName=shelfName, TestCommand=TestCommand, errEvRowStatus=errEvRowStatus, cardCpuType=cardCpuType, errEvLoggingEnabled=errEvLoggingEnabled, diskVolumeUsageLevel=diskVolumeUsageLevel, cardShelfId=cardShelfId, diskVolumeTable=diskVolumeTable, portMinDsPower=portMinDsPower, cardTestId=cardTestId, cardFwUpdateStatus=cardFwUpdateStatus, auditAutoScheduling=auditAutoScheduling, diskVolumeSize=diskVolumeSize, cardLastChangeTime=cardLastChangeTime, cmResetNotification=cmResetNotification, equipmentState=equipmentState, cardCarrierFwVersion=cardCarrierFwVersion, systemKey=systemKey, fpgaErrorEventEntry=fpgaErrorEventEntry, equipmentCardGroup=equipmentCardGroup, systemClock=systemClock, cardTestTime=cardTestTime, cardDplxStatusChange=cardDplxStatusChange, cardPrStateChange=cardPrStateChange, cmVendor=cmVendor, cardTestType=cardTestType, diskVolumeCardId=diskVolumeCardId, downstreamPowerLoss=downstreamPowerLoss, diskVolumeNumber=diskVolumeNumber, cardSubDetected=cardSubDetected, auditName=auditName, portNumber=portNumber, portSecStateChange=portSecStateChange, portType=portType, portConnectorId=portConnectorId, cadEquipmentMib=cadEquipmentMib, portDplxStatusChange=portDplxStatusChange, equipmentDiag=equipmentDiag, equipmentShelfGroup=equipmentShelfGroup, portCardSubType=portCardSubType, portAction=portAction, auditTotalFailedCount=auditTotalFailedCount, TestSchedule=TestSchedule, cardHwVersion=cardHwVersion, TestResult=TestResult, portTrapInh=portTrapInh, cardSwVersion=cardSwVersion, auditDescription=auditDescription, fpgaErrorEventTable=fpgaErrorEventTable, diagTestId=diagTestId, cardTemperature=cardTemperature, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardShelfTestIndex=cardShelfTestIndex, equipmentDiagGroup=equipmentDiagGroup, portMacAddress=portMacAddress, cardPrState=cardPrState, equipmentPortGroup=equipmentPortGroup, auditCycleCount=auditCycleCount, equipmentTraps=equipmentTraps, TestType=TestType, diskVolumeEntry=diskVolumeEntry, diskVolumeId=diskVolumeId, systemGeneralGroup=systemGeneralGroup, portMacIfIndex=portMacIfIndex, PYSNMP_MODULE_ID=cadEquipmentMib, portBgpId=portBgpId, cardFpgaSource=cardFpgaSource, cardCarrierSerialNum=cardCarrierSerialNum, cardRamSize=cardRamSize, equipmentAudit=equipmentAudit, shelfSwVersion=shelfSwVersion, auditShelfId=auditShelfId, diskVolumeDiskSize=diskVolumeDiskSize, cardSpareGroupMode=cardSpareGroupMode, verboseLevel=verboseLevel, cardTestSchedule=cardTestSchedule, cardPicHwVersion=cardPicHwVersion, shelfObjects=shelfObjects, auditId=auditId, cardLastBootVersion=cardLastBootVersion, cardDplxStatus=cardDplxStatus, cardCarrierHwVersion=cardCarrierHwVersion, consoleOutput=consoleOutput, cmDevice=cmDevice, cardBootVersion=cardBootVersion, trapCounter=trapCounter, cardSubType=cardSubType, cardSecStateChange=cardSecStateChange, cardTemperatureHighError=cardTemperatureHighError, diskVolumeUsageNotification=diskVolumeUsageNotification, shelfNumber=shelfNumber, cardTestTable=cardTestTable, cmIpAddress=cmIpAddress, cardName=cardName, portRxFlowControlMode=portRxFlowControlMode, diskVolumeGroup=diskVolumeGroup, cardCpuSpeed=cardCpuSpeed, portDplxStatus=portDplxStatus, errEvLogLevel=errEvLogLevel, cmUChannelID=cmUChannelID, auditLogOutput=auditLogOutput, portMaxDsPower=portMaxDsPower, cardType=cardType, cardTrapInh=cardTrapInh, cardTable=cardTable, auditFailedCount=auditFailedCount, cmRegistrationNotification=cmRegistrationNotification, equipmentCardTestGroup=equipmentCardTestGroup, portId=portId, portTxFlowControlDetected=portTxFlowControlDetected, cardSpareGroupId=cardSpareGroupId, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardTestResultDesc=cardTestResultDesc, cardAdminState=cardAdminState, cardTempOverHeatNotification=cardTempOverHeatNotification, cardSerialNum=cardSerialNum, portNumChans=portNumChans, remainInDiagMode=remainInDiagMode, cardTestEntry=cardTestEntry, cardTestResult=cardTestResult, auditResult=auditResult, diskVolumeName=diskVolumeName, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, errEventId=errEventId, portCurrDsPower=portCurrDsPower, portRxFlowControlDetected=portRxFlowControlDetected, auditPassedCount=auditPassedCount, cardAction=cardAction, diskVolumeTrapInh=diskVolumeTrapInh, portDescription=portDescription, diskDriveNumber=diskDriveNumber, auditCardId=auditCardId, cardDetected=cardDetected, cardLastBootSource=cardLastBootSource, cardTestIndex=cardTestIndex, eqDiagConfig=eqDiagConfig, cadEquipmentMibGroup=cadEquipmentMibGroup, fpgaErrorEventGroup=fpgaErrorEventGroup, cmResetClearNotification=cmResetClearNotification, portSecState=portSecState, equipmentTbl=equipmentTbl, systemGeneral=systemGeneral, TestId=TestId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, auditEntry=auditEntry, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentError=equipmentError, cardSecState=cardSecState, portDocsIfIndex=portDocsIfIndex, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardNumber=cardNumber, portEntry=portEntry, auditLogThrottle=auditLogThrottle, TestTransactionId=TestTransactionId, cardTestDescription=cardTestDescription, portAdminState=portAdminState)
