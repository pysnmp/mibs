...
FirmwareSource, CardType, DuplexStatus, DiskVolumeUsageLevel, PicType, PortMode, PortDetectedMode, SecondaryState, PrimaryState, UnknownState, FlowControlMode, PortType, EqActionType, ShelfId, PortId, CardSubType, AdminState, CardId = mibBuilder.importSymbols("CADANT-TC", "FirmwareSource", "CardType", "DuplexStatus", "DiskVolumeUsageLevel", "PicType", "PortMode", "PortDetectedMode", "SecondaryState", "PrimaryState", "UnknownState", "FlowControlMode", "PortType", "EqActionType", "ShelfId", "PortId", "CardSubType", "AdminState", "CardId")
...
TextualConvention, DisplayString, RowStatus, TruthValue, MacAddress, DateAndTime, TimeStamp = mibBuilder.importSymbols("SNMPv2-TC", "TextualConvention", "DisplayString", "RowStatus", "TruthValue", "MacAddress", "DateAndTime", "TimeStamp")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", auditFailedCount=auditFailedCount, shelfObjects=shelfObjects, cardLastResetReason=cardLastResetReason, portAdminState=portAdminState, diskVolumeSize=diskVolumeSize, equipmentCardTestGroup=equipmentCardTestGroup, shelfName=shelfName, portTxFlowControlMode=portTxFlowControlMode, cadEquipmentMibGroup=cadEquipmentMibGroup, systemClock=systemClock, cardBusSpeed=cardBusSpeed, auditName=auditName, shelfNumber=shelfNumber, cardNumPorts=cardNumPorts, portTable=portTable, cardSecStateChange=cardSecStateChange, eqDiagConfig=eqDiagConfig, trapCounter=trapCounter, errEvLoggingEnabled=errEvLoggingEnabled, TestTransactionId=TestTransactionId, cardHwVersion=cardHwVersion, cmMacAddress=cmMacAddress, cmResetStatus=cmResetStatus, cardRamSize=cardRamSize, diskVolumeDriveId=diskVolumeDriveId, diskDriveNumber=diskDriveNumber, diskVolumeName=diskVolumeName, diskVolumeUsageNotification=diskVolumeUsageNotification, portMode=portMode, auditResult=auditResult, cardSwVersion=cardSwVersion, portMacAddress=portMacAddress, equipmentError=equipmentError, diskVolumeGroup=diskVolumeGroup, consoleOutput=consoleOutput, portNumChans=portNumChans, cardTestName=cardTestName, cardTestResult=cardTestResult, cardTempOverHeatNotification=cardTempOverHeatNotification, errEvRowStatus=errEvRowStatus, cardSubDetected=cardSubDetected, portEntry=portEntry, cardCpuType=cardCpuType, cardTestScheduleCommand=cardTestScheduleCommand, cardTestIndex=cardTestIndex, TestScheduleCommand=TestScheduleCommand, cmResetUpTime=cmResetUpTime, portId=portId, cardTemperatureHighWarn=cardTemperatureHighWarn, cardSpareGroupId=cardSpareGroupId, portTrapInh=portTrapInh, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cardDetectedChange=cardDetectedChange, portBgpId=portBgpId, errEvLogLevel=errEvLogLevel, cardCPLDVersion=cardCPLDVersion, cmResetReason=cmResetReason, portDplxStatus=portDplxStatus, diagTestId=diagTestId, cardLastBootSource=cardLastBootSource, cardSerialNum=cardSerialNum, cardCpuSpeed=cardCpuSpeed, equipmentShelfGroup=equipmentShelfGroup, trapSeverity=trapSeverity, cmResetNotification=cmResetNotification, auditAutoScheduling=auditAutoScheduling, cardDetected=cardDetected, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, portDocsIfIndex=portDocsIfIndex, cmResetClearNotification=cmResetClearNotification, portCardSubType=portCardSubType, cardDplxStatus=cardDplxStatus, cardCarrierFwVersion=cardCarrierFwVersion, cardTestEntry=cardTestEntry, cardTestCommand=cardTestCommand, auditShelfId=auditShelfId, portCurrDsPower=portCurrDsPower, cardTestDescription=cardTestDescription, cardAction=cardAction, fpgaErrorEventEntry=fpgaErrorEventEntry, shelfSwVersion=shelfSwVersion, cardTestSchedule=cardTestSchedule, systemGeneral=systemGeneral, cardFwUpdateStatus=cardFwUpdateStatus, cadEquipmentMib=cadEquipmentMib, TestType=TestType, cardTemperatureHighError=cardTemperatureHighError, auditId=auditId, cardDplxStatusChange=cardDplxStatusChange, auditTable=auditTable, cardBootVersion=cardBootVersion, portConnectorId=portConnectorId, portDescription=portDescription, systemGeneralGroup=systemGeneralGroup, cardPrStateChange=cardPrStateChange, cardTempNoReportNotification=cardTempNoReportNotification, auditLogOutput=auditLogOutput, auditCycleCount=auditCycleCount, cardSpareGroupMode=cardSpareGroupMode, cardTestTable=cardTestTable, TestSchedule=TestSchedule, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, portMaxDsPower=portMaxDsPower, diskVolumeUsagePercentage=diskVolumeUsagePercentage, portShelfId=portShelfId, equipmentTbl=equipmentTbl, cardPrState=cardPrState, systemKey=systemKey, diskVolumeCardId=diskVolumeCardId, auditTime=auditTime, portAction=portAction, cardTestId=cardTestId, cmDevice=cmDevice, cardPicDetected=cardPicDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, cardTable=cardTable, auditCardId=auditCardId, cardFlashSize=cardFlashSize, errEventId=errEventId, cmIpAddress=cmIpAddress, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, equipmentStateGroup=equipmentStateGroup, remainInDiagMode=remainInDiagMode, cardTemperature=cardTemperature, portPrStateChange=portPrStateChange, auditEntry=auditEntry, diskVolumeFileName=diskVolumeFileName, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicHwVersion=cardPicHwVersion, cardTestResultDesc=cardTestResultDesc, cardLastChangeTime=cardLastChangeTime, portRxFlowControlDetected=portRxFlowControlDetected, auditCommand=auditCommand, auditStatus=auditStatus, portLastChangeTime=portLastChangeTime, cardUpTime=cardUpTime, cadEquipmentMibConformance=cadEquipmentMibConformance, cardEntry=cardEntry, cardAdminState=cardAdminState, auditTotalFailedCount=auditTotalFailedCount, cardSubType=cardSubType, portTxFlowControlDetected=portTxFlowControlDetected, diskVolumeId=diskVolumeId, equipmentDiag=equipmentDiag, cmRegistrationNotification=cmRegistrationNotification, fpgaErrorEventGroup=fpgaErrorEventGroup, portSecState=portSecState, equipmentState=equipmentState, cardShelfId=cardShelfId, cmVendor=cmVendor, portMinDsPower=portMinDsPower, cmResetInfo=cmResetInfo, equipmentPortGroup=equipmentPortGroup, equipmentAudit=equipmentAudit, portCardId=portCardId, cardTestResultNotification=cardTestResultNotification, diskVolumeTrapInh=diskVolumeTrapInh, portDplxStatusChange=portDplxStatusChange, auditLogThrottle=auditLogThrottle, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, portPrState=portPrState, diskVolumeNumber=diskVolumeNumber, equipmentTraps=equipmentTraps, equipmentAuditGroup=equipmentAuditGroup, diskVolumeUsageLevel=diskVolumeUsageLevel, cardName=cardName, cardLastBootVersion=cardLastBootVersion, cardCarrierHwVersion=cardCarrierHwVersion, portRxFlowControlMode=portRxFlowControlMode, cardId=cardId, cardTrapInh=cardTrapInh, downstreamPowerLoss=downstreamPowerLoss, cardSecState=cardSecState, PYSNMP_MODULE_ID=cadEquipmentMib, cardCarrierSerialNum=cardCarrierSerialNum, auditTotalPassedCount=auditTotalPassedCount, cmPrimarySID=cmPrimarySID, TestResult=TestResult, TestId=TestId, cardTestTime=cardTestTime, cardNumber=cardNumber, verboseLevel=verboseLevel, diskVolumeEntry=diskVolumeEntry, auditPassedCount=auditPassedCount, cardTestType=cardTestType, cardType=cardType, cardPicSerialNum=cardPicSerialNum, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, fpgaErrorEventTable=fpgaErrorEventTable, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardFwVersion=cardFwVersion, cardShelfTestIndex=cardShelfTestIndex, equipmentCardGroup=equipmentCardGroup, portDetectedMode=portDetectedMode, cardFpgaSource=cardFpgaSource, portMacIfIndex=portMacIfIndex, auditDescription=auditDescription, equipmentDiagGroup=equipmentDiagGroup, TestCommand=TestCommand, portNumber=portNumber, diskVolumeTable=diskVolumeTable, diskVolumeDiskSize=diskVolumeDiskSize, cardTestTransId=cardTestTransId, portSecStateChange=portSecStateChange, portType=portType, cmUChannelID=cmUChannelID, diskVolumeShelfId=diskVolumeShelfId)
