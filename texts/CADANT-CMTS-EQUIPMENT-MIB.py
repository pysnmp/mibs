...
PicType, PortMode, CardId, SecondaryState, PortId, AdminState, UnknownState, DiskVolumeUsageLevel, EqActionType, PortDetectedMode, FlowControlMode, ShelfId, CardSubType, FirmwareSource, DuplexStatus, CardType, PortType, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "PicType", "PortMode", "CardId", "SecondaryState", "PortId", "AdminState", "UnknownState", "DiskVolumeUsageLevel", "EqActionType", "PortDetectedMode", "FlowControlMode", "ShelfId", "CardSubType", "FirmwareSource", "DuplexStatus", "CardType", "PortType", "PrimaryState")
...
DateAndTime, TextualConvention, TimeStamp, DisplayString, RowStatus, MacAddress, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DateAndTime", "TextualConvention", "TimeStamp", "DisplayString", "RowStatus", "MacAddress", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", cardSubType=cardSubType, equipmentPortGroup=equipmentPortGroup, systemKey=systemKey, equipmentDiag=equipmentDiag, cardDetectedChange=cardDetectedChange, cardLastChangeTime=cardLastChangeTime, consoleOutput=consoleOutput, cardDplxStatusChange=cardDplxStatusChange, cardSerialNum=cardSerialNum, cardAction=cardAction, diskVolumeTable=diskVolumeTable, cardTestId=cardTestId, cardPrState=cardPrState, cmResetReason=cmResetReason, diskVolumeDiskSize=diskVolumeDiskSize, cardTestResult=cardTestResult, cmIpAddress=cmIpAddress, cardTable=cardTable, cardTestTable=cardTestTable, cardTestResultNotification=cardTestResultNotification, remainInDiagMode=remainInDiagMode, verboseLevel=verboseLevel, cardNumber=cardNumber, cmRegistrationNotification=cmRegistrationNotification, portDocsIfIndex=portDocsIfIndex, cardTestName=cardTestName, cardId=cardId, portMacIfIndex=portMacIfIndex, diskVolumeUsagePercentage=diskVolumeUsagePercentage, cardCpuSpeed=cardCpuSpeed, portTrapInh=portTrapInh, diagTestId=diagTestId, portTable=portTable, portDplxStatus=portDplxStatus, cardTestEntry=cardTestEntry, cardTestResultDesc=cardTestResultDesc, cmResetStatus=cmResetStatus, errEvRowStatus=errEvRowStatus, TestType=TestType, portMacAddress=portMacAddress, diskVolumeFileName=diskVolumeFileName, errEvRecoveryEnabled=errEvRecoveryEnabled, diskVolumeTrapInh=diskVolumeTrapInh, auditDescription=auditDescription, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardPicSerialNum=cardPicSerialNum, portCardSubType=portCardSubType, portShelfId=portShelfId, equipmentCardGroup=equipmentCardGroup, auditFailedCount=auditFailedCount, cmResetClearNotification=cmResetClearNotification, portTxFlowControlMode=portTxFlowControlMode, eqDiagConfig=eqDiagConfig, auditId=auditId, cardTestType=cardTestType, auditCardId=auditCardId, cardFpgaSource=cardFpgaSource, cardPicDetected=cardPicDetected, diskVolumeCardId=diskVolumeCardId, TestResult=TestResult, shelfNumber=shelfNumber, equipmentCardTestGroup=equipmentCardTestGroup, cardPicHwVersion=cardPicHwVersion, equipmentShelfGroup=equipmentShelfGroup, auditTable=auditTable, portRxFlowControlMode=portRxFlowControlMode, diskVolumeUsageLevel=diskVolumeUsageLevel, cardLastResetReason=cardLastResetReason, portAction=portAction, fpgaErrorEventEntry=fpgaErrorEventEntry, cardShelfTestIndex=cardShelfTestIndex, portNumChans=portNumChans, auditStatus=auditStatus, cardLastBootSource=cardLastBootSource, PYSNMP_MODULE_ID=cadEquipmentMib, cardType=cardType, portRxFlowControlDetected=portRxFlowControlDetected, cardTestDescription=cardTestDescription, diskVolumeNumber=diskVolumeNumber, cmResetInfo=cmResetInfo, cardTrapInh=cardTrapInh, diskVolumeDriveId=diskVolumeDriveId, cardName=cardName, diskVolumeSize=diskVolumeSize, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, auditLogOutput=auditLogOutput, auditTotalFailedCount=auditTotalFailedCount, diskVolumeUsageNotification=diskVolumeUsageNotification, portDplxStatusChange=portDplxStatusChange, cardTestSchedule=cardTestSchedule, cardTestCommand=cardTestCommand, cardDetected=cardDetected, auditName=auditName, TestId=TestId, cardCPLDVersion=cardCPLDVersion, portDetectedMode=portDetectedMode, portMinDsPower=portMinDsPower, portPrState=portPrState, portConnectorId=portConnectorId, equipmentState=equipmentState, cadEquipmentMibCompliances=cadEquipmentMibCompliances, cardCarrierSerialNum=cardCarrierSerialNum, portType=portType, cardHwVersion=cardHwVersion, cardAdminState=cardAdminState, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cmVendor=cmVendor, portCurrDsPower=portCurrDsPower, auditEntry=auditEntry, cardTemperature=cardTemperature, errEvLogLevel=errEvLogLevel, cardCarrierFwVersion=cardCarrierFwVersion, equipmentTraps=equipmentTraps, cardDplxStatus=cardDplxStatus, portDescription=portDescription, cadEquipmentMibConformance=cadEquipmentMibConformance, equipmentAudit=equipmentAudit, portSecState=portSecState, cardTemperatureHighError=cardTemperatureHighError, errEvLoggingEnabled=errEvLoggingEnabled, cmResetNotification=cmResetNotification, cardTestIndex=cardTestIndex, auditShelfId=auditShelfId, portBgpId=portBgpId, equipmentTbl=equipmentTbl, cmUChannelID=cmUChannelID, downstreamPowerLoss=downstreamPowerLoss, systemClock=systemClock, cardUpTime=cardUpTime, cardSwVersion=cardSwVersion, cardFlashSize=cardFlashSize, cardBusSpeed=cardBusSpeed, portSecStateChange=portSecStateChange, portEntry=portEntry, diskDriveNumber=diskDriveNumber, cmResetUpTime=cmResetUpTime, TestScheduleCommand=TestScheduleCommand, cardBootVersion=cardBootVersion, cardRamSize=cardRamSize, cmPrimarySID=cmPrimarySID, cadEquipmentMib=cadEquipmentMib, diskVolumeGroup=diskVolumeGroup, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTestScheduleCommand=cardTestScheduleCommand, shelfObjects=shelfObjects, cadEquipmentMibGroup=cadEquipmentMibGroup, equipmentAuditGroup=equipmentAuditGroup, cardSubDetected=cardSubDetected, portLastChangeTime=portLastChangeTime, equipmentError=equipmentError, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, cmDevice=cmDevice, trapSeverity=trapSeverity, auditResult=auditResult, fpgaErrorEventGroup=fpgaErrorEventGroup, cardTestTransId=cardTestTransId, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, cardLastBootVersion=cardLastBootVersion, cardTestTime=cardTestTime, TestSchedule=TestSchedule, cardShelfId=cardShelfId, cardSecStateChange=cardSecStateChange, equipmentDiagGroup=equipmentDiagGroup, portNumber=portNumber, diskVolumeId=diskVolumeId, cardSecState=cardSecState, cardSpareGroupId=cardSpareGroupId, cardNumPorts=cardNumPorts, diskVolumeEntry=diskVolumeEntry, auditTime=auditTime, auditCommand=auditCommand, auditCycleCount=auditCycleCount, cmMacAddress=cmMacAddress, cardPrStateChange=cardPrStateChange, cardTempNoReportNotification=cardTempNoReportNotification, cardCarrierHwVersion=cardCarrierHwVersion, auditTotalPassedCount=auditTotalPassedCount, portCardId=portCardId, equipmentStateGroup=equipmentStateGroup, portMode=portMode, auditLogThrottle=auditLogThrottle, trapCounter=trapCounter, shelfName=shelfName, diskVolumeName=diskVolumeName, cardCpuType=cardCpuType, cardTemperatureHighWarn=cardTemperatureHighWarn, portId=portId, portMaxDsPower=portMaxDsPower, TestTransactionId=TestTransactionId, cardTempOverHeatNotification=cardTempOverHeatNotification, portTxFlowControlDetected=portTxFlowControlDetected, cardFwUpdateStatus=cardFwUpdateStatus, shelfSwVersion=shelfSwVersion, errEventId=errEventId, systemGeneral=systemGeneral, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portAdminState=portAdminState, cardFwVersion=cardFwVersion, TestCommand=TestCommand, auditAutoScheduling=auditAutoScheduling, portPrStateChange=portPrStateChange, cardEntry=cardEntry, diskVolumeShelfId=diskVolumeShelfId, auditPassedCount=auditPassedCount, cardSpareGroupMode=cardSpareGroupMode, fpgaErrorEventTable=fpgaErrorEventTable, systemGeneralGroup=systemGeneralGroup)
