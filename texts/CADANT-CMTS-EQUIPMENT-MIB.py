...
DiskVolumeUsageLevel, CardId, SecondaryState, CardSubType, DuplexStatus, ShelfId, PortMode, CardType, FlowControlMode, FirmwareSource, PortDetectedMode, PortType, UnknownState, PortId, PicType, EqActionType, AdminState, PrimaryState = mibBuilder.importSymbols("CADANT-TC", "DiskVolumeUsageLevel", "CardId", "SecondaryState", "CardSubType", "DuplexStatus", "ShelfId", "PortMode", "CardType", "FlowControlMode", "FirmwareSource", "PortDetectedMode", "PortType", "UnknownState", "PortId", "PicType", "EqActionType", "AdminState", "PrimaryState")
...
DisplayString, TimeStamp, RowStatus, TextualConvention, DateAndTime, MacAddress, TruthValue = mibBuilder.importSymbols("SNMPv2-TC", "DisplayString", "TimeStamp", "RowStatus", "TextualConvention", "DateAndTime", "MacAddress", "TruthValue")
...
mibBuilder.exportSymbols("CADANT-CMTS-EQUIPMENT-MIB", equipmentError=equipmentError, diskVolumeUsagePercentage=diskVolumeUsagePercentage, auditName=auditName, cardTestName=cardTestName, portNumber=portNumber, TestScheduleCommand=TestScheduleCommand, cmDevice=cmDevice, diskVolumeSize=diskVolumeSize, portDetectedMode=portDetectedMode, cardSubType=cardSubType, portDescription=portDescription, cardTemperatureHighError=cardTemperatureHighError, diskVolumeAutoDeleteUnusedFile=diskVolumeAutoDeleteUnusedFile, portPrStateChange=portPrStateChange, downstreamPowerLoss=downstreamPowerLoss, diskVolumeUsageMajorThreshold=diskVolumeUsageMajorThreshold, equipmentShelfGroup=equipmentShelfGroup, cardSwVersion=cardSwVersion, portTxFlowControlDetected=portTxFlowControlDetected, cmVendor=cmVendor, portLastChangeTime=portLastChangeTime, eqDiagConfig=eqDiagConfig, cadEquipmentMibCompliances=cadEquipmentMibCompliances, equipmentPortGroup=equipmentPortGroup, auditCommand=auditCommand, cardTestResultDesc=cardTestResultDesc, cardFwUpdateStatus=cardFwUpdateStatus, errEvLoggingEnabled=errEvLoggingEnabled, consoleOutput=consoleOutput, equipmentTbl=equipmentTbl, cardLastResetReason=cardLastResetReason, diskVolumeTrapInh=diskVolumeTrapInh, equipmentAudit=equipmentAudit, portRxFlowControlMode=portRxFlowControlMode, equipmentStateGroup=equipmentStateGroup, cardTempOverHeatNotification=cardTempOverHeatNotification, diskVolumeAutoDeleteFileNotification=diskVolumeAutoDeleteFileNotification, cardTempOutOfRangeNotification=cardTempOutOfRangeNotification, portDocsIfIndex=portDocsIfIndex, cmPrimarySID=cmPrimarySID, cmResetNotification=cmResetNotification, shelfName=shelfName, portDplxStatus=portDplxStatus, portMode=portMode, diskVolumeCardId=diskVolumeCardId, diskVolumeId=diskVolumeId, shelfNumber=shelfNumber, TestId=TestId, cardTestSchedule=cardTestSchedule, cardSubDetected=cardSubDetected, portNumChans=portNumChans, TestSchedule=TestSchedule, diskVolumeEntry=diskVolumeEntry, cardTestIndex=cardTestIndex, portCardSubType=portCardSubType, cardSecState=cardSecState, diskVolumeShelfId=diskVolumeShelfId, portSecState=portSecState, cardSpareGroupMode=cardSpareGroupMode, cardDetectedChange=cardDetectedChange, cardLastChangeTime=cardLastChangeTime, cardTemperatureHighWarn=cardTemperatureHighWarn, diskVolumeDiskSize=diskVolumeDiskSize, equipmentCardGroup=equipmentCardGroup, diagTestId=diagTestId, cardPicSerialNum=cardPicSerialNum, portShelfId=portShelfId, cardTestEntry=cardTestEntry, cardUpTime=cardUpTime, portConnectorId=portConnectorId, auditId=auditId, portAction=portAction, portType=portType, trapSeverity=trapSeverity, portSecStateChange=portSecStateChange, equipmentAuditGroup=equipmentAuditGroup, cardName=cardName, portRxFlowControlDetected=portRxFlowControlDetected, auditFailedCount=auditFailedCount, cardDplxStatusChange=cardDplxStatusChange, portAdminState=portAdminState, verboseLevel=verboseLevel, cardTestResultNotification=cardTestResultNotification, cardTestType=cardTestType, portMinDsPower=portMinDsPower, TestResult=TestResult, cardSpareGroupId=cardSpareGroupId, auditDescription=auditDescription, cardSerialNum=cardSerialNum, portId=portId, fpgaErrorEventGroup=fpgaErrorEventGroup, TestType=TestType, portPrState=portPrState, cardDplxStatus=cardDplxStatus, cardId=cardId, portBgpId=portBgpId, cardDetected=cardDetected, cmResetStatus=cmResetStatus, cardBootVersion=cardBootVersion, cardBusSpeed=cardBusSpeed, cardTestScheduleCommand=cardTestScheduleCommand, errEventId=errEventId, cardTable=cardTable, diskVolumeUsageCriticalThreshold=diskVolumeUsageCriticalThreshold, cardShelfId=cardShelfId, cardCpuType=cardCpuType, auditPassedCount=auditPassedCount, portCurrDsPower=portCurrDsPower, diskVolumeDriveId=diskVolumeDriveId, cardTestDescription=cardTestDescription, cmResetInfo=cmResetInfo, equipmentTraps=equipmentTraps, cardFpgaSource=cardFpgaSource, portEntry=portEntry, auditTable=auditTable, auditCardId=auditCardId, systemClock=systemClock, PYSNMP_MODULE_ID=cadEquipmentMib, cardAdminState=cardAdminState, auditCycleCount=auditCycleCount, auditShelfId=auditShelfId, cmResetReason=cmResetReason, cardHwVersion=cardHwVersion, cadEquipmentMibConformance=cadEquipmentMibConformance, cardTemperature=cardTemperature, equipmentDiag=equipmentDiag, auditTime=auditTime, cardCarrierHwVersion=cardCarrierHwVersion, auditAutoScheduling=auditAutoScheduling, portMacAddress=portMacAddress, cardNumPorts=cardNumPorts, systemGeneralGroup=systemGeneralGroup, cardCPLDVersion=cardCPLDVersion, cmResetClearNotification=cmResetClearNotification, portTxFlowControlMode=portTxFlowControlMode, cmUChannelID=cmUChannelID, cardFlashSize=cardFlashSize, cardLastBootSource=cardLastBootSource, diskVolumeNumber=diskVolumeNumber, cmRegistrationNotification=cmRegistrationNotification, cadEquipmentMibGroup=cadEquipmentMibGroup, cardPicHwVersion=cardPicHwVersion, diskVolumeFileName=diskVolumeFileName, errEvRowStatus=errEvRowStatus, diskVolumeGroup=diskVolumeGroup, cmIpAddress=cmIpAddress, cardEntry=cardEntry, diskVolumeTable=diskVolumeTable, cardPicDetected=cardPicDetected, errEvRecoveryEnabled=errEvRecoveryEnabled, auditResult=auditResult, shelfObjects=shelfObjects, cardTempNoReportNotification=cardTempNoReportNotification, cadEquipmentMib=cadEquipmentMib, cardCpuSpeed=cardCpuSpeed, auditStatus=auditStatus, trapCounter=trapCounter, cardLastBootVersion=cardLastBootVersion, auditLogThrottle=auditLogThrottle, systemGeneral=systemGeneral, cardTestCommand=cardTestCommand, cardCarrierFwVersion=cardCarrierFwVersion, portTrapInh=portTrapInh, equipmentCardTestGroup=equipmentCardTestGroup, cmResetUpTime=cmResetUpTime, diskVolumeName=diskVolumeName, remainInDiagMode=remainInDiagMode, cardPrState=cardPrState, auditTotalPassedCount=auditTotalPassedCount, diskVolumeUsageMinorThreshold=diskVolumeUsageMinorThreshold, systemKey=systemKey, cardTestTransId=cardTestTransId, cardAction=cardAction, equipmentState=equipmentState, diskVolumeUsageNotification=diskVolumeUsageNotification, cardFwVersion=cardFwVersion, cadEquipmentMibCompliance=cadEquipmentMibCompliance, cardTestResult=cardTestResult, diskDriveNumber=diskDriveNumber, cardTrapInh=cardTrapInh, cardShelfTestIndex=cardShelfTestIndex, cardTestTable=cardTestTable, cardTestTime=cardTestTime, errEvLogLevel=errEvLogLevel, cardSecStateChange=cardSecStateChange, equipmentDiagGroup=equipmentDiagGroup, cardCarrierSerialNum=cardCarrierSerialNum, portTable=portTable, auditEntry=auditEntry, diskVolumeUsageLevel=diskVolumeUsageLevel, fpgaErrorEventEntry=fpgaErrorEventEntry, auditTotalFailedCount=auditTotalFailedCount, cardPrStateChange=cardPrStateChange, portCardId=portCardId, portDplxStatusChange=portDplxStatusChange, fpgaErrorEventTable=fpgaErrorEventTable, portMacIfIndex=portMacIfIndex, cardTestId=cardTestId, portMaxDsPower=portMaxDsPower, shelfSwVersion=shelfSwVersion, cardType=cardType, auditLogOutput=auditLogOutput, cardNumber=cardNumber, cmMacAddress=cmMacAddress, TestCommand=TestCommand, cardRamSize=cardRamSize, TestTransactionId=TestTransactionId)
