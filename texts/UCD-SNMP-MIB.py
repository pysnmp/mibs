...
MibScalar, MibTable, MibTableRow, MibTableColumn, iso, IpAddress, Gauge32, Integer32, Opaque, NotificationType, Counter32, Unsigned32, TimeTicks, Bits, ObjectIdentity, MibIdentifier, ModuleIdentity, Counter64, enterprises = mibBuilder.importSymbols("SNMPv2-SMI", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "iso", "IpAddress", "Gauge32", "Integer32", "Opaque", "NotificationType", "Counter32", "Unsigned32", "TimeTicks", "Bits", "ObjectIdentity", "MibIdentifier", "ModuleIdentity", "Counter64", "enterprises")
...
mibBuilder.exportSymbols("UCD-SNMP-MIB", memTotalSwap=memTotalSwap, versionDoDebugging=versionDoDebugging, fileName=fileName, memTotalSwapX=memTotalSwapX, ssCpuNumCpus=ssCpuNumCpus, ssIOReceive=ssIOReceive, ssCpuSystem=ssCpuSystem, ssCpuRawSystem=ssCpuRawSystem, memAvailSwapTXT=memAvailSwapTXT, versionTag=versionTag, laLoadFloat=laLoadFloat, laErrorFlag=laErrorFlag, mrEntry=mrEntry, logMatchCounter=logMatchCounter, memUsedRealTXT=memUsedRealTXT, ssCpuRawSteal=ssCpuRawSteal, memTotalRealX=memTotalRealX, laNames=laNames, memBufferX=memBufferX, dskMinPercent=dskMinPercent, memIndex=memIndex, extTable=extTable, fileIndex=fileIndex, memSwapError=memSwapError, aix=aix, dskTotal=dskTotal, memCachedX=memCachedX, prCount=prCount, dskUsedLow=dskUsedLow, snmperrIndex=snmperrIndex, extErrFix=extErrFix, dskErrorMsg=dskErrorMsg, fileEntry=fileEntry, memUsedSwapTXT=memUsedSwapTXT, prMin=prMin, prMax=prMax, prTable=prTable, version=version, memTotalFree=memTotalFree, logMatchCurrentCounter=logMatchCurrentCounter, ssCpuRawSoftIRQ=ssCpuRawSoftIRQ, dskPercentNode=dskPercentNode, extIndex=extIndex, dskAvailLow=dskAvailLow, laLoadInt=laLoadInt, memTotalReal=memTotalReal, ucdTraps=ucdTraps, dskAvail=dskAvail, ssCpuRawWait=ssCpuRawWait, versionIdent=versionIdent, prErrMessage=prErrMessage, prErrFixCmd=prErrFixCmd, logMatchName=logMatchName, UCDErrorFix=UCDErrorFix, dskPercent=dskPercent, ssCpuRawUser=ssCpuRawUser, extOutput=extOutput, snmperrs=snmperrs, logMatchFilename=logMatchFilename, versionUpdateConfig=versionUpdateConfig, snmperrNames=snmperrNames, logMatchRegExCompilation=logMatchRegExCompilation, memShared=memShared, versionSavePersistentData=versionSavePersistentData, snmperrErrorFlag=snmperrErrorFlag, prNames=prNames, hpux11=hpux11, ssCpuRawNice=ssCpuRawNice, ssCpuRawInterrupt=ssCpuRawInterrupt, versionDate=versionDate, versionIndex=versionIndex, fileErrorFlag=fileErrorFlag, dragonfly=dragonfly, prErrorFlag=prErrorFlag, logMatchRegEx=logMatchRegEx, ssSysContext=ssSysContext, laIndex=laIndex, dskUsedHigh=dskUsedHigh, unknown=unknown, laTable=laTable, snmperrErrMessage=snmperrErrMessage, memory=memory, ucdInternal=ucdInternal, ssIORawReceived=ssIORawReceived, freebsd=freebsd, logMatchTable=logMatchTable, UCDErrorFlag=UCDErrorFlag, prEntry=prEntry, versionConfigureOptions=versionConfigureOptions, memAvailRealX=memAvailRealX, extResult=extResult, ssRawContexts=ssRawContexts, Float=Float, mrTable=mrTable, fileTable=fileTable, memTotalRealTXT=memTotalRealTXT, ssSwapIn=ssSwapIn, logMatchMaxEntries=logMatchMaxEntries, memAvailSwapX=memAvailSwapX, dskTable=dskTable, memTotalSwapTXT=memTotalSwapTXT, logMatchErrorFlag=logMatchErrorFlag, logMatchCount=logMatchCount, ssIndex=ssIndex, memErrorName=memErrorName, ssCpuUser=ssCpuUser, irix=irix, netbsd1=netbsd1, win32=win32, memTotalFreeX=memTotalFreeX, ssCpuIdle=ssCpuIdle, hpux9=hpux9, ssIORawSent=ssIORawSent, memAvailRealTXT=memAvailRealTXT, mrModuleName=mrModuleName, fileErrorMsg=fileErrorMsg, extNames=extNames, ssRawSwapIn=ssRawSwapIn, extEntry=extEntry, prIndex=prIndex, logMatchGlobalCount=logMatchGlobalCount, osf=osf, openbsd=openbsd, dskAvailHigh=dskAvailHigh, fileSize=fileSize, prErrFix=prErrFix, ultrix=ultrix, memAvailReal=memAvailReal, dskUsed=dskUsed, logMatchCycle=logMatchCycle, macosx=macosx, dskIndex=dskIndex, laErrMessage=laErrMessage, solaris=solaris, linux=linux, logMatchEntry=logMatchEntry, dskTotalLow=dskTotalLow, logMatchIndex=logMatchIndex, dskDevice=dskDevice, PYSNMP_MODULE_ID=ucdavis, ssRawSwapOut=ssRawSwapOut, ssCpuRawGuestNice=ssCpuRawGuestNice, ssSwapOut=ssSwapOut, laEntry=laEntry, dskPath=dskPath, logMatch=logMatch, logMatchCurrentCount=logMatchCurrentCount, ssRawInterrupts=ssRawInterrupts, mrIndex=mrIndex, ucdExperimental=ucdExperimental, memAvailSwap=memAvailSwap, systemStats=systemStats, hpux10=hpux10, fileMax=fileMax, laConfig=laConfig, sunos4=sunos4, ssErrorName=ssErrorName, ssIOSent=ssIOSent, logMatchGlobalCounter=logMatchGlobalCounter, bsdi=bsdi, extErrFixCmd=extErrFixCmd, versionCDate=versionCDate, ssSysInterrupts=ssSysInterrupts, ssCpuRawGuest=ssCpuRawGuest, memMinimumSwap=memMinimumSwap, ucdavis=ucdavis, memBuffer=memBuffer, memCached=memCached, memSwapErrorMsg=memSwapErrorMsg, ucdShutdown=ucdShutdown, dskMinimum=dskMinimum, memSharedX=memSharedX, ucdStart=ucdStart, ucdSnmpAgent=ucdSnmpAgent, versionRestartAgent=versionRestartAgent, laLoad=laLoad, memMinimumSwapX=memMinimumSwapX, extCommand=extCommand, dskErrorFlag=dskErrorFlag, dskTotalHigh=dskTotalHigh, versionClearCache=versionClearCache, ssCpuRawIdle=ssCpuRawIdle, ssCpuRawKernel=ssCpuRawKernel, dskEntry=dskEntry)
