...
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, hAZLOCConfiguration=hAZLOCConfiguration, smtp=smtp, encryption=encryption, channelALoopback=channelALoopback, iPv6Address=iPv6Address, alarmsGroup=alarmsGroup, gatewayIPAddress=gatewayIPAddress, pubStatsGroup=pubStatsGroup, snmpControl=snmpControl, tFTPServerIPAddress=tFTPServerIPAddress, range=range, syslogLocalWrappedTrap=syslogLocalWrappedTrap, dfsTableNumber=dfsTableNumber, secureModeAlarm=secureModeAlarm, dfsGroup=dfsGroup, management=management, groupID=groupID, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, sNMPPortNumber=sNMPPortNumber, ptp600=ptp600, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, dataPortWirelessDownAlert=dataPortWirelessDownAlert, sNMPTrapIPAddress=sNMPTrapIPAddress, managementPortDisabledWarning=managementPortDisabledWarning, dataPortStatusTrap=dataPortStatusTrap, userTable=userTable, receivePower=receivePower, siteName=siteName, iPv6PrefixLength=iPv6PrefixLength, rangingMode=rangingMode, receiveFreqKHz=receiveFreqKHz, incompatibleRegionCodes=incompatibleRegionCodes, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, sNTPState=sNTPState, linkSymmetry=linkSymmetry, encryptionAlgorithm=encryptionAlgorithm, productName=productName, sNMPEnabledTraps=sNMPEnabledTraps, managementPortStatus=managementPortStatus, managementPortConfigurationMismatch=managementPortConfigurationMismatch, tFTPServerPortNumber=tFTPServerPortNumber, sNMPTrapInternetAddress=sNMPTrapInternetAddress, channelALoopbackTrap=channelALoopbackTrap, dataPortFiberStatusTrap=dataPortFiberStatusTrap, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, bootVersion=bootVersion, l2CPPriorityTable=l2CPPriorityTable, licence=licence, linkName=linkName, phyControl=phyControl, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, hardwareVersion=hardwareVersion, tFTPServerInternetAddressType=tFTPServerInternetAddressType, wirelessLinkStatus=wirelessLinkStatus, channelBLoopbackTrap=channelBLoopbackTrap, bridgeGroup=bridgeGroup, ptp=ptp, syslogControlGroup=syslogControlGroup, managementGroup=managementGroup, sNTPPrimaryServer=sNTPPrimaryServer, dataPortStatus=dataPortStatus, managementMode=managementMode, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, securityLevel=securityLevel, supplementary=supplementary, remoteInternetAddress=remoteInternetAddress, unitOutOfCalibration=unitOutOfCalibration, subnetMask=subnetMask, channelBCableLength=channelBCableLength, dataPortAutoMdix=dataPortAutoMdix, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, userTableNumber=userTableNumber, sNTPLastSync=sNTPLastSync, PYSNMP_MODULE_ID=cambium, aAAControlGroup=aAAControlGroup, ethernet=ethernet, timeZone=timeZone, phyStatus=phyStatus, ethernetPriorityTableNumber=ethernetPriorityTableNumber, sMTPEnabledMessages=sMTPEnabledMessages, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, dataPortDisabledWarning=dataPortDisabledWarning, phyStatusGroup=phyStatusGroup, channelBandwidth=channelBandwidth, bandwidthVariant=bandwidthVariant, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, useVLANForManagementInterfaces=useVLANForManagementInterfaces, productVariant=productVariant, ptpGroups=ptpGroups, ptpTrapPrefix=ptpTrapPrefix, regionCode=regionCode, transmitFreqMHz=transmitFreqMHz, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, dfsMeans=dfsMeans, dfsPeaks=dfsPeaks, managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, licenceGroup=licenceGroup, installArmState=installArmState, sNTPServerTableNumber=sNTPServerTableNumber, cableLoss=cableLoss, sNTPServerPortNumber=sNTPServerPortNumber, snmpControlGroup=snmpControlGroup, txColorCode=txColorCode, ptpTraps=ptpTraps, tFTPServerInternetAddress=tFTPServerInternetAddress, tDDSynchronizationStatus=tDDSynchronizationStatus, sNMPTrapVersion=sNMPTrapVersion, cambium=cambium, configuration=configuration, ethernetFiberSupport=ethernetFiberSupport, installArmStateTrap=installArmStateTrap, targetMACAddress=targetMACAddress, syslogControl=syslogControl, channelACableLength=channelACableLength, antennaGain=antennaGain, transmitPower=transmitPower, sNTPServerIPAddress=sNTPServerIPAddress, dfs=dfs, sMTPServerPortNumber=sMTPServerPortNumber, sNTPPollInterval=sNTPPollInterval, aggregateDataRate=aggregateDataRate, masterSlaveMode=masterSlaveMode, telecomsChannelAStatusTrap=telecomsChannelAStatusTrap, sNMPCommunityString=sNMPCommunityString, versionsGroup=versionsGroup, userTableEntry=userTableEntry, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sMTPServerInternetAddress=sMTPServerInternetAddress, softwareVersion=softwareVersion, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, receiveFreqMHz=receiveFreqMHz, mPLSTCPriorityTable=mPLSTCPriorityTable, dSCPManagementPriority=dSCPManagementPriority, userEnabled=userEnabled, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, receiveChannel=receiveChannel, iPDSCPPriorityTable=iPDSCPPriorityTable, smtpGroup=smtpGroup, sntp=sntp, tDDControlGroup=tDDControlGroup, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, ethernetPriorityTable=ethernetPriorityTable, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, dataPortFiberStatus=dataPortFiberStatus, sNTPServerInternetAddress=sNTPServerInternetAddress, remoteIPAddress=remoteIPAddress, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, sMTPServerIPAddress=sMTPServerIPAddress, sNTPSyncTrap=sNTPSyncTrap, transmitDataRate=transmitDataRate, eIRP=eIRP, sNTPServerRetries=sNTPServerRetries, sNTPServerStatus=sNTPServerStatus, qOSPriorityScheme=qOSPriorityScheme, sNMPTrapTableEntry=sNMPTrapTableEntry, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, notificationsGroup=notificationsGroup, sNTPServerTable=sNTPServerTable, phyControlGroup=phyControlGroup, telnetPortNumber=telnetPortNumber, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, sNMPTrapPortNumber=sNMPTrapPortNumber, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, maximumTransmitPower=maximumTransmitPower, channelBLineCode=channelBLineCode, reset=reset, enabledDiagnosticAlarms=enabledDiagnosticAlarms, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, regionCodeTrap=regionCodeTrap, telecomsChannelBStatus=telecomsChannelBStatus, l2CPPriorityTableNumber=l2CPPriorityTableNumber, ethernetGroup=ethernetGroup, l2CPPriorityTableEntry=l2CPPriorityTableEntry, ethernetBridgingStatusAlarm=ethernetBridgingStatusAlarm, transmitModulationMode=transmitModulationMode, telecomsInterface=telecomsInterface, sNMPTrapTableIndex=sNMPTrapTableIndex, remoteInternetAddressType=remoteInternetAddressType, resetGroup=resetGroup, telnetAccessEnabled=telnetAccessEnabled, remoteMaximumTransmitPower=remoteMaximumTransmitPower, ethernetBridgingStatusAlarmTrap=ethernetBridgingStatusAlarmTrap, signalStrengthRatio=signalStrengthRatio, ethernetPriorityTableIndex=ethernetPriorityTableIndex, ptmp=ptmp, configurationGroup=configurationGroup, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, frequencyVariant=frequencyVariant, constantPowerSpectralDensity=constantPowerSpectralDensity, tDDSynchronizationAlarm=tDDSynchronizationAlarm, telecomsChannelAStatus=telecomsChannelAStatus, dfsTableEntry=dfsTableEntry, channelALineCode=channelALineCode, telecom=telecom, vLANManagementPriority=vLANManagementPriority, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, telecomsChannelSelection=telecomsChannelSelection, automaticTxPowerControl=automaticTxPowerControl, incompatibleMasterAndSlave=incompatibleMasterAndSlave, longitude=longitude, sNMPTrapTable=sNMPTrapTable, transmitChannel=transmitChannel, secureModeAlarmTrap=secureModeAlarmTrap, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, vectorError=vectorError, receiveModulationModeDetail=receiveModulationModeDetail, versions=versions, dataPortAutoNegotiation=dataPortAutoNegotiation, latitude=latitude, targetRange=targetRange, managementPortStatusTrap=managementPortStatusTrap, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, sNTPServerTimeout=sNTPServerTimeout, userName=userName, wirelessLinkAvailability=wirelessLinkAvailability, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, hTTPSPortNumber=hTTPSPortNumber, hTTPAccessEnabled=hTTPAccessEnabled, altitude=altitude, syslogClientTrap=syslogClientTrap, ethernetPriorityTableEntry=ethernetPriorityTableEntry, secondarySoftwareVersion=secondarySoftwareVersion, syslogClient=syslogClient, pubStats=pubStats, syslogStateTrap=syslogStateTrap, installStatus=installStatus, sNMPv3Enable=sNMPv3Enable, iPv6GatewayAddress=iPv6GatewayAddress, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, linkModeOptimizationMismatch=linkModeOptimizationMismatch, channelBLoopback=channelBLoopback, sNTPServerTableIndex=sNTPServerTableIndex, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, ethernetCappedMaxWirelessSpeed=ethernetCappedMaxWirelessSpeed, daylightSaving=daylightSaving, alarms=alarms, hTTPPortNumber=hTTPPortNumber, transmitFreqKHz=transmitFreqKHz, linkModeOptimisation=linkModeOptimisation, userPassword=userPassword, installStatusTrap=installStatusTrap, encryptionGroup=encryptionGroup, l2CPPriorityTableIndex=l2CPPriorityTableIndex, searchState=searchState, linkLoss=linkLoss, remoteRxTargetPower=remoteRxTargetPower, sntpGroup=sntpGroup, vLANManagementVID=vLANManagementVID, syslogState=syslogState, rxColorCode=rxColorCode)
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", sNMPTrapTableNumber=sNMPTrapTableNumber, localPacketFiltering=localPacketFiltering, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap, managementPortWirelessDownAlert=managementPortWirelessDownAlert, dfsTable=dfsTable, hTTPSAccessEnabled=hTTPSAccessEnabled, channelChangeTrap=channelChangeTrap, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, userRole=userRole, userTableIndex=userTableIndex, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, tDDControl=tDDControl, byteErrorRatio=byteErrorRatio, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, ethernetBridgingStatus=ethernetBridgingStatus, aAAControl=aAAControl, supplementaryGroup=supplementaryGroup, receiveModulationMode=receiveModulationMode, sNTPSync=sNTPSync, iPv4Address=iPv4Address, bridge=bridge, sNTPServerTableEntry=sNTPServerTableEntry, telecomGroup=telecomGroup, dfsTableIndex=dfsTableIndex, noWirelessChannelAvailable=noWirelessChannelAvailable, receiveDataRate=receiveDataRate, sNTPServerInternetAddressType=sNTPServerInternetAddressType, ptpCompliance=ptpCompliance, tDDSynchronizationMode=tDDSynchronizationMode, rawReceivePower=rawReceivePower, accessMethod=accessMethod, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, iPVersion=iPVersion, sMTPEmailAlert=sMTPEmailAlert, sMTPSourceEmailAddress=sMTPSourceEmailAddress, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, systemClock=systemClock, telecomsChannelBStatusTrap=telecomsChannelBStatusTrap, sMTPServerInternetAddressType=sMTPServerInternetAddressType, dataPortConfigurationMismatch=dataPortConfigurationMismatch, systemReset=systemReset, ethernetBridgingAvailability=ethernetBridgingAvailability)
