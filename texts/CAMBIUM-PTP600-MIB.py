...
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", securityLevel=securityLevel, receiveModulationModeDetail=receiveModulationModeDetail, tDDControlGroup=tDDControlGroup, dfsTableEntry=dfsTableEntry, telecomsInterface=telecomsInterface, sNTPState=sNTPState, telecomsChannelSelection=telecomsChannelSelection, masterSlaveMode=masterSlaveMode, ptpGroups=ptpGroups, remoteInternetAddress=remoteInternetAddress, dfsTableNumber=dfsTableNumber, vLANManagementPriority=vLANManagementPriority, l2CPPriorityTableEntry=l2CPPriorityTableEntry, searchState=searchState, transmitDataRate=transmitDataRate, ethernetBridgingAvailability=ethernetBridgingAvailability, sNTPPollInterval=sNTPPollInterval, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, dataPortStatus=dataPortStatus, configuration=configuration, dataPortFiberStatus=dataPortFiberStatus, sNMPTrapTableIndex=sNMPTrapTableIndex, sMTPServerInternetAddressType=sMTPServerInternetAddressType, hTTPAccessEnabled=hTTPAccessEnabled, ethernetGroup=ethernetGroup, softwareVersion=softwareVersion, cambium=cambium, dfsPeaks=dfsPeaks, ptpTrapPrefix=ptpTrapPrefix, channelALineCode=channelALineCode, sNMPTrapTableNumber=sNMPTrapTableNumber, channelBCableLength=channelBCableLength, phyControlGroup=phyControlGroup, userEnabled=userEnabled, alarms=alarms, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, ethernetBridgingStatus=ethernetBridgingStatus, telecomsChannelAStatusTrap=telecomsChannelAStatusTrap, localPacketFiltering=localPacketFiltering, syslogClientTrap=syslogClientTrap, smtpGroup=smtpGroup, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, iPv4Address=iPv4Address, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, remoteIPAddress=remoteIPAddress, sntp=sntp, ethernetBridgingStatusAlarm=ethernetBridgingStatusAlarm, managementPortDisabledWarning=managementPortDisabledWarning, regionCode=regionCode, signalStrengthRatio=signalStrengthRatio, regionCodeTrap=regionCodeTrap, ethernetPriorityTableNumber=ethernetPriorityTableNumber, dataPortWirelessDownAlert=dataPortWirelessDownAlert, constantPowerSpectralDensity=constantPowerSpectralDensity, channelALoopback=channelALoopback, transmitChannel=transmitChannel, sMTPServerInternetAddress=sMTPServerInternetAddress, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, secondarySoftwareVersion=secondarySoftwareVersion, l2CPPriorityTable=l2CPPriorityTable, telnetPortNumber=telnetPortNumber, byteErrorRatio=byteErrorRatio, ethernetPriorityTableIndex=ethernetPriorityTableIndex, sNMPEnabledTraps=sNMPEnabledTraps, installStatusTrap=installStatusTrap, telecomsChannelBStatus=telecomsChannelBStatus, sNMPTrapTableEntry=sNMPTrapTableEntry, tDDSynchronizationMode=tDDSynchronizationMode, iPv6Address=iPv6Address, licence=licence, cableLoss=cableLoss, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, sNMPPortNumber=sNMPPortNumber, managementPortStatusTrap=managementPortStatusTrap, antennaGain=antennaGain, ethernetCappedMaxWirelessSpeed=ethernetCappedMaxWirelessSpeed, iPv6GatewayAddress=iPv6GatewayAddress, sNMPTrapTable=sNMPTrapTable, siteName=siteName, ptpCompliance=ptpCompliance, resetGroup=resetGroup, syslogState=syslogState, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, pubStats=pubStats, sNTPServerTimeout=sNTPServerTimeout, managementPortStatus=managementPortStatus, aAAControlGroup=aAAControlGroup, range=range, installArmStateTrap=installArmStateTrap, reset=reset, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, useVLANForManagementInterfaces=useVLANForManagementInterfaces, managementGroup=managementGroup, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, telecomsChannelBStatusTrap=telecomsChannelBStatusTrap, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, sNTPSyncTrap=sNTPSyncTrap, bandwidthVariant=bandwidthVariant, secureModeAlarm=secureModeAlarm, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap, syslogClient=syslogClient, unitOutOfCalibration=unitOutOfCalibration, longitude=longitude, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, rxColorCode=rxColorCode, wirelessLinkStatus=wirelessLinkStatus, secureModeAlarmTrap=secureModeAlarmTrap, dataPortStatusTrap=dataPortStatusTrap, targetMACAddress=targetMACAddress, phyControl=phyControl, userTableNumber=userTableNumber, mPLSTCPriorityTable=mPLSTCPriorityTable, sNTPServerTableNumber=sNTPServerTableNumber, ptmp=ptmp, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, linkLoss=linkLoss, telecom=telecom, syslogControl=syslogControl, sNMPv3Enable=sNMPv3Enable, channelBLineCode=channelBLineCode, tFTPServerInternetAddressType=tFTPServerInternetAddressType, syslogControlGroup=syslogControlGroup, incompatibleMasterAndSlave=incompatibleMasterAndSlave, l2CPPriorityTableIndex=l2CPPriorityTableIndex, encryptionAlgorithm=encryptionAlgorithm, channelACableLength=channelACableLength, accessMethod=accessMethod, hTTPPortNumber=hTTPPortNumber, systemClock=systemClock, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, maximumTransmitPower=maximumTransmitPower, installArmState=installArmState, PYSNMP_MODULE_ID=cambium, alarmsGroup=alarmsGroup, versionsGroup=versionsGroup, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, sMTPServerIPAddress=sMTPServerIPAddress, bootVersion=bootVersion, supplementary=supplementary, sNTPSync=sNTPSync, tDDControl=tDDControl, rangingMode=rangingMode, ethernetPriorityTableEntry=ethernetPriorityTableEntry, incompatibleRegionCodes=incompatibleRegionCodes, ethernet=ethernet, sNTPPrimaryServer=sNTPPrimaryServer, tFTPServerIPAddress=tFTPServerIPAddress, hAZLOCConfiguration=hAZLOCConfiguration, sMTPEmailAlert=sMTPEmailAlert, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dfs=dfs, eIRP=eIRP, sNTPServerPortNumber=sNTPServerPortNumber, aAAControl=aAAControl, transmitFreqKHz=transmitFreqKHz, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, enabledDiagnosticAlarms=enabledDiagnosticAlarms, versions=versions, altitude=altitude, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, sNTPServerIPAddress=sNTPServerIPAddress, managementPortConfigurationMismatch=managementPortConfigurationMismatch, snmpControlGroup=snmpControlGroup, managementMode=managementMode, targetRange=targetRange, dataPortConfigurationMismatch=dataPortConfigurationMismatch, userPassword=userPassword, receiveModulationMode=receiveModulationMode, systemReset=systemReset, sNMPTrapPortNumber=sNMPTrapPortNumber, phyStatus=phyStatus, linkModeOptimizationMismatch=linkModeOptimizationMismatch, licenceGroup=licenceGroup, userRole=userRole, iPv6PrefixLength=iPv6PrefixLength, l2CPPriorityTableNumber=l2CPPriorityTableNumber, receiveFreqMHz=receiveFreqMHz, ptp600=ptp600, qOSPriorityScheme=qOSPriorityScheme, ptpTraps=ptpTraps, telnetAccessEnabled=telnetAccessEnabled, iPVersion=iPVersion, noWirelessChannelAvailable=noWirelessChannelAvailable, sMTPSourceEmailAddress=sMTPSourceEmailAddress, sNMPTrapVersion=sNMPTrapVersion, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, remoteMaximumTransmitPower=remoteMaximumTransmitPower, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, dataPortAutoMdix=dataPortAutoMdix, transmitPower=transmitPower, telecomGroup=telecomGroup, dfsTableIndex=dfsTableIndex, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, encryption=encryption, hardwareVersion=hardwareVersion, syslogLocalWrappedTrap=syslogLocalWrappedTrap, rawReceivePower=rawReceivePower, sNTPServerStatus=sNTPServerStatus, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, dfsGroup=dfsGroup, notificationsGroup=notificationsGroup, productName=productName, timeZone=timeZone, aggregateDataRate=aggregateDataRate, pubStatsGroup=pubStatsGroup, phyStatusGroup=phyStatusGroup, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, productVariant=productVariant, sNTPServerTableEntry=sNTPServerTableEntry, daylightSaving=daylightSaving, sntpGroup=sntpGroup, txColorCode=txColorCode, receivePower=receivePower, sNMPTrapInternetAddress=sNMPTrapInternetAddress, ptp=ptp, management=management, transmitModulationMode=transmitModulationMode, sNTPServerInternetAddress=sNTPServerInternetAddress, ethernetPriorityTable=ethernetPriorityTable, encryptionGroup=encryptionGroup, remoteInternetAddressType=remoteInternetAddressType, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, linkModeOptimisation=linkModeOptimisation, sNMPTrapIPAddress=sNMPTrapIPAddress, sNTPServerInternetAddressType=sNTPServerInternetAddressType, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, gatewayIPAddress=gatewayIPAddress, dSCPManagementPriority=dSCPManagementPriority, syslogStateTrap=syslogStateTrap, automaticTxPowerControl=automaticTxPowerControl, userTableIndex=userTableIndex, tDDSynchronizationAlarm=tDDSynchronizationAlarm, dataPortDisabledWarning=dataPortDisabledWarning, userTableEntry=userTableEntry, snmpControl=snmpControl, tFTPServerInternetAddress=tFTPServerInternetAddress, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, smtp=smtp, tFTPServerPortNumber=tFTPServerPortNumber, channelBLoopbackTrap=channelBLoopbackTrap, latitude=latitude, managementPortWirelessDownAlert=managementPortWirelessDownAlert, channelALoopbackTrap=channelALoopbackTrap, bridgeGroup=bridgeGroup, userTable=userTable, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, dataPortAutoNegotiation=dataPortAutoNegotiation, transmitFreqMHz=transmitFreqMHz, sMTPServerPortNumber=sMTPServerPortNumber, subnetMask=subnetMask, configurationGroup=configurationGroup)
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", linkSymmetry=linkSymmetry, sNTPLastSync=sNTPLastSync, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, vLANManagementVID=vLANManagementVID, bridge=bridge, managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, receiveFreqKHz=receiveFreqKHz, groupID=groupID, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, supplementaryGroup=supplementaryGroup, frequencyVariant=frequencyVariant, sNTPServerTable=sNTPServerTable, vectorError=vectorError, tDDSynchronizationStatus=tDDSynchronizationStatus, dfsTable=dfsTable, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, remoteRxTargetPower=remoteRxTargetPower, channelBLoopback=channelBLoopback, sMTPEnabledMessages=sMTPEnabledMessages, sNMPCommunityString=sNMPCommunityString, hTTPSAccessEnabled=hTTPSAccessEnabled, receiveChannel=receiveChannel, linkName=linkName, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, receiveDataRate=receiveDataRate, ethernetBridgingStatusAlarmTrap=ethernetBridgingStatusAlarmTrap, telecomsChannelAStatus=telecomsChannelAStatus, ethernetFiberSupport=ethernetFiberSupport, dfsMeans=dfsMeans, hTTPSPortNumber=hTTPSPortNumber, sNTPServerTableIndex=sNTPServerTableIndex, userName=userName, channelChangeTrap=channelChangeTrap, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, dataPortFiberStatusTrap=dataPortFiberStatusTrap, sNTPServerRetries=sNTPServerRetries, channelBandwidth=channelBandwidth, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, installStatus=installStatus, iPDSCPPriorityTable=iPDSCPPriorityTable, wirelessLinkAvailability=wirelessLinkAvailability, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement)
