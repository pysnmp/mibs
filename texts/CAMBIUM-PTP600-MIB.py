...
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", managementGroup=managementGroup, dfsTableEntry=dfsTableEntry, secureModeAlarm=secureModeAlarm, tDDSynchronizationAlarm=tDDSynchronizationAlarm, hTTPPortNumber=hTTPPortNumber, sNTPPollInterval=sNTPPollInterval, licenceGroup=licenceGroup, accessMethod=accessMethod, ethernetFiberSupport=ethernetFiberSupport, remoteMaximumTransmitPower=remoteMaximumTransmitPower, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, iPv4Address=iPv4Address, dfsMeans=dfsMeans, receiveFreqKHz=receiveFreqKHz, sNTPServerTableNumber=sNTPServerTableNumber, wirelessLinkStatus=wirelessLinkStatus, tDDSynchronizationStatus=tDDSynchronizationStatus, ethernetBridgingStatus=ethernetBridgingStatus, sNMPTrapIPAddress=sNMPTrapIPAddress, regionCode=regionCode, productVariant=productVariant, byteErrorRatio=byteErrorRatio, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, hTTPAccessEnabled=hTTPAccessEnabled, sNTPServerPortNumber=sNTPServerPortNumber, pubStatsGroup=pubStatsGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, altitude=altitude, hTTPSPortNumber=hTTPSPortNumber, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, ptmp=ptmp, channelALineCode=channelALineCode, targetRange=targetRange, linkLoss=linkLoss, constantPowerSpectralDensity=constantPowerSpectralDensity, PYSNMP_MODULE_ID=cambium, rangingMode=rangingMode, sNTPServerStatus=sNTPServerStatus, userRole=userRole, encryptionGroup=encryptionGroup, systemReset=systemReset, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, secureModeAlarmTrap=secureModeAlarmTrap, channelBLoopbackTrap=channelBLoopbackTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, ethernetBridgingStatusAlarmTrap=ethernetBridgingStatusAlarmTrap, syslogClientTrap=syslogClientTrap, encryption=encryption, bridge=bridge, sNMPTrapVersion=sNMPTrapVersion, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, tDDControl=tDDControl, gatewayIPAddress=gatewayIPAddress, remoteIPAddress=remoteIPAddress, sMTPServerInternetAddress=sMTPServerInternetAddress, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, sNMPTrapPortNumber=sNMPTrapPortNumber, longitude=longitude, ethernet=ethernet, ethernetPriorityTableEntry=ethernetPriorityTableEntry, sNMPv3Enable=sNMPv3Enable, hTTPSAccessEnabled=hTTPSAccessEnabled, siteName=siteName, antennaGain=antennaGain, cambium=cambium, searchState=searchState, iPDSCPPriorityTable=iPDSCPPriorityTable, daylightSaving=daylightSaving, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, l2CPPriorityTableIndex=l2CPPriorityTableIndex, phyStatus=phyStatus, sNTPServerTableIndex=sNTPServerTableIndex, sNMPTrapTableNumber=sNMPTrapTableNumber, installStatusTrap=installStatusTrap, receiveFreqMHz=receiveFreqMHz, sNTPState=sNTPState, supplementary=supplementary, phyStatusGroup=phyStatusGroup, managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, bandwidthVariant=bandwidthVariant, transmitModulationMode=transmitModulationMode, bridgeGroup=bridgeGroup, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, phyControl=phyControl, ethernetCappedMaxWirelessSpeed=ethernetCappedMaxWirelessSpeed, useVLANForManagementInterfaces=useVLANForManagementInterfaces, remoteRxTargetPower=remoteRxTargetPower, sMTPServerPortNumber=sMTPServerPortNumber, sMTPServerInternetAddressType=sMTPServerInternetAddressType, sNMPEnabledTraps=sNMPEnabledTraps, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, transmitPower=transmitPower, syslogClient=syslogClient, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, rxColorCode=rxColorCode, telecomsChannelAStatusTrap=telecomsChannelAStatusTrap, receiveChannel=receiveChannel, sNTPServerTable=sNTPServerTable, dataPortFiberStatus=dataPortFiberStatus, securityLevel=securityLevel, sMTPServerIPAddress=sMTPServerIPAddress, ethernetPriorityTable=ethernetPriorityTable, iPv6Address=iPv6Address, channelBLineCode=channelBLineCode, telecomsChannelSelection=telecomsChannelSelection, aAAControl=aAAControl, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, mPLSTCPriorityTable=mPLSTCPriorityTable, sNMPCommunityString=sNMPCommunityString, ptp=ptp, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, sntp=sntp, versions=versions, channelACableLength=channelACableLength, txColorCode=txColorCode, l2CPPriorityTable=l2CPPriorityTable, supplementaryGroup=supplementaryGroup, timeZone=timeZone, tFTPServerIPAddress=tFTPServerIPAddress, sNTPServerInternetAddress=sNTPServerInternetAddress, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, sNTPServerRetries=sNTPServerRetries, dataPortStatus=dataPortStatus, ethernetPriorityTableNumber=ethernetPriorityTableNumber, regionCodeTrap=regionCodeTrap, vLANManagementPriority=vLANManagementPriority, pubStats=pubStats, transmitDataRate=transmitDataRate, tFTPServerPortNumber=tFTPServerPortNumber, dfsTableNumber=dfsTableNumber, userPassword=userPassword, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, configuration=configuration, reset=reset, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, remoteInternetAddress=remoteInternetAddress, sNTPServerIPAddress=sNTPServerIPAddress, signalStrengthRatio=signalStrengthRatio, qOSPriorityScheme=qOSPriorityScheme, ptpTrapPrefix=ptpTrapPrefix, sNMPPortNumber=sNMPPortNumber, dfsPeaks=dfsPeaks, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, targetMACAddress=targetMACAddress, eIRP=eIRP, dSCPManagementPriority=dSCPManagementPriority, ethernetBridgingAvailability=ethernetBridgingAvailability, managementPortStatus=managementPortStatus, range=range, licence=licence, telnetPortNumber=telnetPortNumber, sNMPTrapInternetAddress=sNMPTrapInternetAddress, phyControlGroup=phyControlGroup, syslogStateTrap=syslogStateTrap, ethernetPriorityTableIndex=ethernetPriorityTableIndex, dataPortStatusTrap=dataPortStatusTrap, noWirelessChannelAvailable=noWirelessChannelAvailable, iPv6GatewayAddress=iPv6GatewayAddress, installArmState=installArmState, alarmsGroup=alarmsGroup, dataPortAutoMdix=dataPortAutoMdix, hAZLOCConfiguration=hAZLOCConfiguration, channelChangeTrap=channelChangeTrap, receiveModulationModeDetail=receiveModulationModeDetail, sNTPServerInternetAddressType=sNTPServerInternetAddressType, managementMode=managementMode, userTableEntry=userTableEntry, managementPortDisabledWarning=managementPortDisabledWarning, channelBLoopback=channelBLoopback, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, vLANManagementVID=vLANManagementVID, dataPortConfigurationMismatch=dataPortConfigurationMismatch, transmitFreqMHz=transmitFreqMHz, automaticTxPowerControl=automaticTxPowerControl, installStatus=installStatus, frequencyVariant=frequencyVariant, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, channelBCableLength=channelBCableLength, sNTPServerTimeout=sNTPServerTimeout, localPacketFiltering=localPacketFiltering, receiveModulationMode=receiveModulationMode, resetGroup=resetGroup, dataPortWirelessDownAlert=dataPortWirelessDownAlert, l2CPPriorityTableEntry=l2CPPriorityTableEntry, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, sMTPEmailAlert=sMTPEmailAlert, maximumTransmitPower=maximumTransmitPower, sMTPEnabledMessages=sMTPEnabledMessages, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, channelBandwidth=channelBandwidth, telecomGroup=telecomGroup, syslogControl=syslogControl, userName=userName, incompatibleMasterAndSlave=incompatibleMasterAndSlave, productName=productName, tFTPServerInternetAddress=tFTPServerInternetAddress, receiveDataRate=receiveDataRate, enabledDiagnosticAlarms=enabledDiagnosticAlarms, dfsGroup=dfsGroup, ptpCompliance=ptpCompliance, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, smtpGroup=smtpGroup, aggregateDataRate=aggregateDataRate, installArmStateTrap=installArmStateTrap, snmpControl=snmpControl, userEnabled=userEnabled, l2CPPriorityTableNumber=l2CPPriorityTableNumber, latitude=latitude, dfsTableIndex=dfsTableIndex, telecomsChannelAStatus=telecomsChannelAStatus, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, iPv6PrefixLength=iPv6PrefixLength, secondarySoftwareVersion=secondarySoftwareVersion, encryptionAlgorithm=encryptionAlgorithm, channelALoopback=channelALoopback, dfs=dfs, ptp600=ptp600, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, linkModeOptimisation=linkModeOptimisation, sNTPSync=sNTPSync, syslogControlGroup=syslogControlGroup, ptpGroups=ptpGroups, sMTPSourceEmailAddress=sMTPSourceEmailAddress, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, telecom=telecom, transmitChannel=transmitChannel, cableLoss=cableLoss, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, receivePower=receivePower, bootVersion=bootVersion, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, wirelessLinkAvailability=wirelessLinkAvailability, telecomsChannelBStatus=telecomsChannelBStatus, rawReceivePower=rawReceivePower, sNTPServerTableEntry=sNTPServerTableEntry, incompatibleRegionCodes=incompatibleRegionCodes, hardwareVersion=hardwareVersion, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, alarms=alarms, groupID=groupID, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, iPVersion=iPVersion)
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", ethernetGroup=ethernetGroup, sNMPTrapTableIndex=sNMPTrapTableIndex, smtp=smtp, ptpTraps=ptpTraps, tFTPServerInternetAddressType=tFTPServerInternetAddressType, snmpControlGroup=snmpControlGroup, tDDSynchronizationMode=tDDSynchronizationMode, unitOutOfCalibration=unitOutOfCalibration, dataPortDisabledWarning=dataPortDisabledWarning, vectorError=vectorError, channelALoopbackTrap=channelALoopbackTrap, linkName=linkName, sNTPLastSync=sNTPLastSync, dataPortFiberStatusTrap=dataPortFiberStatusTrap, versionsGroup=versionsGroup, sNMPTrapTable=sNMPTrapTable, masterSlaveMode=masterSlaveMode, userTableIndex=userTableIndex, configurationGroup=configurationGroup, telnetAccessEnabled=telnetAccessEnabled, aAAControlGroup=aAAControlGroup, sNTPSyncTrap=sNTPSyncTrap, systemClock=systemClock, managementPortStatusTrap=managementPortStatusTrap, notificationsGroup=notificationsGroup, sNTPPrimaryServer=sNTPPrimaryServer, transmitFreqKHz=transmitFreqKHz, telecomsInterface=telecomsInterface, softwareVersion=softwareVersion, sNMPTrapTableEntry=sNMPTrapTableEntry, linkSymmetry=linkSymmetry, userTable=userTable, remoteInternetAddressType=remoteInternetAddressType, managementPortWirelessDownAlert=managementPortWirelessDownAlert, userTableNumber=userTableNumber, syslogState=syslogState, management=management, subnetMask=subnetMask, managementPortConfigurationMismatch=managementPortConfigurationMismatch, sntpGroup=sntpGroup, linkModeOptimizationMismatch=linkModeOptimizationMismatch, telecomsChannelBStatusTrap=telecomsChannelBStatusTrap, tDDControlGroup=tDDControlGroup, ethernetBridgingStatusAlarm=ethernetBridgingStatusAlarm, dfsTable=dfsTable)
