...
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", syslogControl=syslogControl, iPv6GatewayAddress=iPv6GatewayAddress, transmitModulationMode=transmitModulationMode, incompatibleMasterAndSlave=incompatibleMasterAndSlave, vectorError=vectorError, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, sNTPServerRetries=sNTPServerRetries, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, localPacketFiltering=localPacketFiltering, remoteInternetAddressType=remoteInternetAddressType, sNMPTrapVersion=sNMPTrapVersion, management=management, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, qOSPriorityScheme=qOSPriorityScheme, systemClock=systemClock, vLANManagementPriority=vLANManagementPriority, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, noWirelessChannelAvailable=noWirelessChannelAvailable, secureModeAlarmTrap=secureModeAlarmTrap, userTableIndex=userTableIndex, cableLoss=cableLoss, aAAControl=aAAControl, iPv6PrefixLength=iPv6PrefixLength, sNTPServerPortNumber=sNTPServerPortNumber, securityLevel=securityLevel, tDDSynchronizationStatus=tDDSynchronizationStatus, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, channelBLineCode=channelBLineCode, dataPortConfigurationMismatch=dataPortConfigurationMismatch, versionsGroup=versionsGroup, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, encryption=encryption, incompatibleRegionCodes=incompatibleRegionCodes, channelACableLength=channelACableLength, managementPortStatus=managementPortStatus, iPv4Address=iPv4Address, cambium=cambium, ptp600=ptp600, bootVersion=bootVersion, transmitFreqMHz=transmitFreqMHz, syslogControlGroup=syslogControlGroup, channelBLoopback=channelBLoopback, syslogClientTrap=syslogClientTrap, notificationsGroup=notificationsGroup, byteErrorRatio=byteErrorRatio, dfsTable=dfsTable, eIRP=eIRP, dataPortFiberStatusTrap=dataPortFiberStatusTrap, targetRange=targetRange, receiveChannel=receiveChannel, sNTPLastSync=sNTPLastSync, telnetPortNumber=telnetPortNumber, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, encryptionGroup=encryptionGroup, softwareVersion=softwareVersion, iPDSCPPriorityTable=iPDSCPPriorityTable, hTTPSPortNumber=hTTPSPortNumber, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, dataPortWirelessDownAlert=dataPortWirelessDownAlert, ptmp=ptmp, bridge=bridge, telecomsChannelSelection=telecomsChannelSelection, channelBLoopbackTrap=channelBLoopbackTrap, regionCode=regionCode, dfsTableEntry=dfsTableEntry, alarmsGroup=alarmsGroup, ethernetGroup=ethernetGroup, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, masterSlaveMode=masterSlaveMode, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, rangingMode=rangingMode, sMTPServerIPAddress=sMTPServerIPAddress, hAZLOCConfiguration=hAZLOCConfiguration, managementPortWirelessDownAlert=managementPortWirelessDownAlert, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, ethernetPriorityTableEntry=ethernetPriorityTableEntry, userTable=userTable, linkLoss=linkLoss, sMTPEnabledMessages=sMTPEnabledMessages, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, PYSNMP_MODULE_ID=cambium, signalStrengthRatio=signalStrengthRatio, tDDSynchronizationMode=tDDSynchronizationMode, sNMPCommunityString=sNMPCommunityString, tFTPServerPortNumber=tFTPServerPortNumber, remoteRxTargetPower=remoteRxTargetPower, wirelessLinkStatus=wirelessLinkStatus, syslogClient=syslogClient, enabledDiagnosticAlarms=enabledDiagnosticAlarms, remoteIPAddress=remoteIPAddress, daylightSaving=daylightSaving, reset=reset, managementMode=managementMode, ethernetBridgingStatus=ethernetBridgingStatus, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, userTableNumber=userTableNumber, sMTPServerInternetAddress=sMTPServerInternetAddress, userPassword=userPassword, telecom=telecom, dataPortAutoMdix=dataPortAutoMdix, ethernetPriorityTableNumber=ethernetPriorityTableNumber, hTTPAccessEnabled=hTTPAccessEnabled, unitOutOfCalibration=unitOutOfCalibration, tDDControlGroup=tDDControlGroup, managementGroup=managementGroup, iPv6Address=iPv6Address, telecomGroup=telecomGroup, tDDSynchronizationAlarm=tDDSynchronizationAlarm, subnetMask=subnetMask, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, sMTPServerPortNumber=sMTPServerPortNumber, dSCPManagementPriority=dSCPManagementPriority, telecomsChannelAStatus=telecomsChannelAStatus, wirelessLinkAvailability=wirelessLinkAvailability, txColorCode=txColorCode, rawReceivePower=rawReceivePower, userName=userName, smtpGroup=smtpGroup, dfsGroup=dfsGroup, alarms=alarms, telecomsInterface=telecomsInterface, sNTPPrimaryServer=sNTPPrimaryServer, transmitFreqKHz=transmitFreqKHz, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, tFTPServerInternetAddress=tFTPServerInternetAddress, syslogStateTrap=syslogStateTrap, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, ethernetFiberSupport=ethernetFiberSupport, syslogLocalWrappedTrap=syslogLocalWrappedTrap, linkSymmetry=linkSymmetry, installStatus=installStatus, timeZone=timeZone, sNMPPortNumber=sNMPPortNumber, targetMACAddress=targetMACAddress, aAAControlGroup=aAAControlGroup, sNMPTrapTable=sNMPTrapTable, userRole=userRole, userEnabled=userEnabled, range=range, sNMPTrapPortNumber=sNMPTrapPortNumber, sNMPTrapTableNumber=sNMPTrapTableNumber, iPVersion=iPVersion, sNMPTrapTableIndex=sNMPTrapTableIndex, sNTPServerInternetAddressType=sNTPServerInternetAddressType, phyStatus=phyStatus, tFTPServerInternetAddressType=tFTPServerInternetAddressType, antennaGain=antennaGain, automaticTxPowerControl=automaticTxPowerControl, hardwareVersion=hardwareVersion, sNTPSync=sNTPSync, syslogState=syslogState, installArmStateTrap=installArmStateTrap, dataPortAutoNegotiation=dataPortAutoNegotiation, ethernetCappedMaxWirelessSpeed=ethernetCappedMaxWirelessSpeed, rxColorCode=rxColorCode, bridgeGroup=bridgeGroup, bandwidthVariant=bandwidthVariant, pubStats=pubStats, ptpCompliance=ptpCompliance, l2CPPriorityTableEntry=l2CPPriorityTableEntry, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, userTableEntry=userTableEntry, receiveDataRate=receiveDataRate, ptpTrapPrefix=ptpTrapPrefix, configurationGroup=configurationGroup, dfsPeaks=dfsPeaks, dataPortStatus=dataPortStatus, l2CPPriorityTableIndex=l2CPPriorityTableIndex, productName=productName, maximumTransmitPower=maximumTransmitPower, installArmState=installArmState, licence=licence, telnetAccessEnabled=telnetAccessEnabled, sNTPServerTableIndex=sNTPServerTableIndex, sNTPServerTable=sNTPServerTable, dfsTableNumber=dfsTableNumber, accessMethod=accessMethod, aggregateDataRate=aggregateDataRate, searchState=searchState, receiveModulationModeDetail=receiveModulationModeDetail, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, sNTPSyncTrap=sNTPSyncTrap, pubStatsGroup=pubStatsGroup, latitude=latitude, smtp=smtp, vLANManagementVID=vLANManagementVID, supplementary=supplementary, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, systemReset=systemReset, hTTPSAccessEnabled=hTTPSAccessEnabled, resetGroup=resetGroup, mPLSTCPriorityTable=mPLSTCPriorityTable, channelChangeTrap=channelChangeTrap, sNTPServerTableEntry=sNTPServerTableEntry, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, sNTPServerTableNumber=sNTPServerTableNumber, versions=versions, licenceGroup=licenceGroup, channelBCableLength=channelBCableLength, dfsMeans=dfsMeans, transmitChannel=transmitChannel, linkName=linkName, groupID=groupID, ethernetBridgingAvailability=ethernetBridgingAvailability, altitude=altitude, frequencyVariant=frequencyVariant, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap, tDDControl=tDDControl, channelALineCode=channelALineCode, regionCodeTrap=regionCodeTrap, phyControl=phyControl, ethernet=ethernet, receiveModulationMode=receiveModulationMode, dataPortFiberStatus=dataPortFiberStatus, ethernetBridgingStatusAlarm=ethernetBridgingStatusAlarm, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, sNTPPollInterval=sNTPPollInterval, useVLANForManagementInterfaces=useVLANForManagementInterfaces, hTTPPortNumber=hTTPPortNumber, sNMPTrapInternetAddress=sNMPTrapInternetAddress, dfsTableIndex=dfsTableIndex, supplementaryGroup=supplementaryGroup, snmpControlGroup=snmpControlGroup, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, dfs=dfs, siteName=siteName, installStatusTrap=installStatusTrap, phyStatusGroup=phyStatusGroup, sNTPServerInternetAddress=sNTPServerInternetAddress, sMTPServerInternetAddressType=sMTPServerInternetAddressType, sntpGroup=sntpGroup, channelBandwidth=channelBandwidth, channelALoopback=channelALoopback, remoteMaximumTransmitPower=remoteMaximumTransmitPower, ethernetPriorityTableIndex=ethernetPriorityTableIndex, ptpTraps=ptpTraps, tFTPServerIPAddress=tFTPServerIPAddress, remoteInternetAddress=remoteInternetAddress, sNTPServerTimeout=sNTPServerTimeout, ethernetBridgingStatusAlarmTrap=ethernetBridgingStatusAlarmTrap, secondarySoftwareVersion=secondarySoftwareVersion, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, ptp=ptp, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, managementPortConfigurationMismatch=managementPortConfigurationMismatch, sNTPState=sNTPState, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, sNTPServerStatus=sNTPServerStatus, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, managementPortStatusTrap=managementPortStatusTrap)
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, telecomsChannelBStatusTrap=telecomsChannelBStatusTrap, sNMPTrapTableEntry=sNMPTrapTableEntry, telecomsChannelBStatus=telecomsChannelBStatus, l2CPPriorityTableNumber=l2CPPriorityTableNumber, productVariant=productVariant, sMTPSourceEmailAddress=sMTPSourceEmailAddress, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, managementPortDisabledWarning=managementPortDisabledWarning, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPEnabledTraps=sNMPEnabledTraps, encryptionAlgorithm=encryptionAlgorithm, ptpGroups=ptpGroups, transmitDataRate=transmitDataRate, receiveFreqMHz=receiveFreqMHz, gatewayIPAddress=gatewayIPAddress, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, receivePower=receivePower, transmitPower=transmitPower, phyControlGroup=phyControlGroup, sNMPv3Enable=sNMPv3Enable, channelALoopbackTrap=channelALoopbackTrap, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, dataPortDisabledWarning=dataPortDisabledWarning, secureModeAlarm=secureModeAlarm, longitude=longitude, dataPortStatusTrap=dataPortStatusTrap, receiveFreqKHz=receiveFreqKHz, sNTPServerIPAddress=sNTPServerIPAddress, sntp=sntp, snmpControl=snmpControl, linkModeOptimisation=linkModeOptimisation, configuration=configuration, constantPowerSpectralDensity=constantPowerSpectralDensity, sNMPTrapIPAddress=sNMPTrapIPAddress, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, ethernetPriorityTable=ethernetPriorityTable, sMTPEmailAlert=sMTPEmailAlert, telecomsChannelAStatusTrap=telecomsChannelAStatusTrap, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, l2CPPriorityTable=l2CPPriorityTable, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap)
