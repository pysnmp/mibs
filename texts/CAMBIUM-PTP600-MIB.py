...
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", ethernetFiberSupport=ethernetFiberSupport, targetMACAddress=targetMACAddress, txColorCode=txColorCode, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, sMTPSourceEmailAddress=sMTPSourceEmailAddress, licenceGroup=licenceGroup, smtpGroup=smtpGroup, snmpControl=snmpControl, productVariant=productVariant, sNMPTrapIPAddress=sNMPTrapIPAddress, groupID=groupID, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, iPv6Address=iPv6Address, userTableEntry=userTableEntry, ethernetPriorityTable=ethernetPriorityTable, phyControlGroup=phyControlGroup, sNTPLastSync=sNTPLastSync, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, channelALoopback=channelALoopback, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, systemReset=systemReset, sNMPEnabledTraps=sNMPEnabledTraps, daylightSaving=daylightSaving, sNTPServerTimeout=sNTPServerTimeout, phyStatus=phyStatus, accessMethod=accessMethod, channelBLoopback=channelBLoopback, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, dataPortFiberStatusTrap=dataPortFiberStatusTrap, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap, installStatus=installStatus, smtp=smtp, automaticTxPowerControl=automaticTxPowerControl, receiveFreqMHz=receiveFreqMHz, sMTPServerInternetAddressType=sMTPServerInternetAddressType, aggregateDataRate=aggregateDataRate, supplementary=supplementary, eIRP=eIRP, remoteInternetAddress=remoteInternetAddress, vectorError=vectorError, managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, userPassword=userPassword, managementPortStatusTrap=managementPortStatusTrap, ethernetPriorityTableIndex=ethernetPriorityTableIndex, gatewayIPAddress=gatewayIPAddress, l2CPPriorityTableIndex=l2CPPriorityTableIndex, transmitModulationMode=transmitModulationMode, rangingMode=rangingMode, ptpTrapPrefix=ptpTrapPrefix, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, sNMPTrapTableNumber=sNMPTrapTableNumber, remoteIPAddress=remoteIPAddress, receiveModulationMode=receiveModulationMode, channelACableLength=channelACableLength, telecomsChannelBStatus=telecomsChannelBStatus, notificationsGroup=notificationsGroup, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPTrapInternetAddress=sNMPTrapInternetAddress, byteErrorRatio=byteErrorRatio, dfsTableEntry=dfsTableEntry, userRole=userRole, userEnabled=userEnabled, cambium=cambium, sNTPServerRetries=sNTPServerRetries, syslogClientTrap=syslogClientTrap, productName=productName, channelBCableLength=channelBCableLength, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, configuration=configuration, sMTPEmailAlert=sMTPEmailAlert, transmitPower=transmitPower, ptp=ptp, hardwareVersion=hardwareVersion, ethernetBridgingStatusAlarmTrap=ethernetBridgingStatusAlarmTrap, dfsTableNumber=dfsTableNumber, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, hTTPSPortNumber=hTTPSPortNumber, iPVersion=iPVersion, sNTPServerInternetAddress=sNTPServerInternetAddress, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, installArmState=installArmState, tFTPServerInternetAddress=tFTPServerInternetAddress, sNTPPrimaryServer=sNTPPrimaryServer, channelBLoopbackTrap=channelBLoopbackTrap, installStatusTrap=installStatusTrap, transmitChannel=transmitChannel, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, dfs=dfs, longitude=longitude, sNMPTrapTableEntry=sNMPTrapTableEntry, sNTPServerTable=sNTPServerTable, remoteMaximumTransmitPower=remoteMaximumTransmitPower, regionCodeTrap=regionCodeTrap, syslogLocalWrappedTrap=syslogLocalWrappedTrap, snmpControlGroup=snmpControlGroup, ptpGroups=ptpGroups, vLANManagementVID=vLANManagementVID, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, dataPortDisabledWarning=dataPortDisabledWarning, transmitFreqMHz=transmitFreqMHz, sNTPServerPortNumber=sNTPServerPortNumber, ptp600=ptp600, managementPortWirelessDownAlert=managementPortWirelessDownAlert, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, transmitFreqKHz=transmitFreqKHz, bridgeGroup=bridgeGroup, dfsMeans=dfsMeans, frequencyVariant=frequencyVariant, sNMPTrapTableIndex=sNMPTrapTableIndex, sNTPPollInterval=sNTPPollInterval, siteName=siteName, systemClock=systemClock, sMTPServerInternetAddress=sMTPServerInternetAddress, ethernetPriorityTableEntry=ethernetPriorityTableEntry, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, tDDSynchronizationMode=tDDSynchronizationMode, mPLSTCPriorityTable=mPLSTCPriorityTable, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, hTTPAccessEnabled=hTTPAccessEnabled, linkSymmetry=linkSymmetry, telecomsChannelAStatusTrap=telecomsChannelAStatusTrap, l2CPPriorityTableEntry=l2CPPriorityTableEntry, dataPortConfigurationMismatch=dataPortConfigurationMismatch, sNTPSyncTrap=sNTPSyncTrap, sNMPTrapTable=sNMPTrapTable, hTTPSAccessEnabled=hTTPSAccessEnabled, unitOutOfCalibration=unitOutOfCalibration, latitude=latitude, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, receiveFreqKHz=receiveFreqKHz, dSCPManagementPriority=dSCPManagementPriority, tDDSynchronizationAlarm=tDDSynchronizationAlarm, sNMPPortNumber=sNMPPortNumber, sMTPServerIPAddress=sMTPServerIPAddress, versions=versions, encryptionGroup=encryptionGroup, dataPortFiberStatus=dataPortFiberStatus, range=range, sMTPEnabledMessages=sMTPEnabledMessages, aAAControl=aAAControl, alarmsGroup=alarmsGroup, securityLevel=securityLevel, transmitDataRate=transmitDataRate, channelBLineCode=channelBLineCode, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, telecomsInterface=telecomsInterface, userTable=userTable, linkName=linkName, bandwidthVariant=bandwidthVariant, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, ethernetPriorityTableNumber=ethernetPriorityTableNumber, ethernetBridgingStatus=ethernetBridgingStatus, pubStats=pubStats, incompatibleMasterAndSlave=incompatibleMasterAndSlave, tDDSynchronizationStatus=tDDSynchronizationStatus, tFTPServerIPAddress=tFTPServerIPAddress, installArmStateTrap=installArmStateTrap, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, masterSlaveMode=masterSlaveMode, ptpTraps=ptpTraps, iPv4Address=iPv4Address, secureModeAlarmTrap=secureModeAlarmTrap, ethernetCappedMaxWirelessSpeed=ethernetCappedMaxWirelessSpeed, sntpGroup=sntpGroup, targetRange=targetRange, versionsGroup=versionsGroup, dataPortAutoMdix=dataPortAutoMdix, qOSPriorityScheme=qOSPriorityScheme, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, syslogControlGroup=syslogControlGroup, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, PYSNMP_MODULE_ID=cambium, aAAControlGroup=aAAControlGroup, antennaGain=antennaGain, sNTPServerTableIndex=sNTPServerTableIndex, sNTPServerStatus=sNTPServerStatus, telecomGroup=telecomGroup, sNTPSync=sNTPSync, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, dfsTable=dfsTable, managementPortDisabledWarning=managementPortDisabledWarning, localPacketFiltering=localPacketFiltering, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, wirelessLinkAvailability=wirelessLinkAvailability, receiveModulationModeDetail=receiveModulationModeDetail, dfsTableIndex=dfsTableIndex, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, noWirelessChannelAvailable=noWirelessChannelAvailable, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, userName=userName, cableLoss=cableLoss, receivePower=receivePower, channelALineCode=channelALineCode, rawReceivePower=rawReceivePower, timeZone=timeZone, dataPortWirelessDownAlert=dataPortWirelessDownAlert, management=management, altitude=altitude, sNMPv3Enable=sNMPv3Enable, remoteRxTargetPower=remoteRxTargetPower, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, searchState=searchState, pubStatsGroup=pubStatsGroup, maximumTransmitPower=maximumTransmitPower, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, iPDSCPPriorityTable=iPDSCPPriorityTable, iPv6GatewayAddress=iPv6GatewayAddress, managementMode=managementMode, userTableIndex=userTableIndex, sNTPServerTableNumber=sNTPServerTableNumber, ethernet=ethernet, tDDControl=tDDControl, tDDControlGroup=tDDControlGroup, remoteInternetAddressType=remoteInternetAddressType, l2CPPriorityTableNumber=l2CPPriorityTableNumber, ptmp=ptmp, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, telecomsChannelAStatus=telecomsChannelAStatus, regionCode=regionCode, telecom=telecom, incompatibleRegionCodes=incompatibleRegionCodes, bootVersion=bootVersion, alarms=alarms, iPv6PrefixLength=iPv6PrefixLength, telnetPortNumber=telnetPortNumber, syslogClient=syslogClient, dataPortStatusTrap=dataPortStatusTrap, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, secureModeAlarm=secureModeAlarm, reset=reset, ethernetBridgingStatusAlarm=ethernetBridgingStatusAlarm, channelBandwidth=channelBandwidth, tFTPServerInternetAddressType=tFTPServerInternetAddressType, ptpCompliance=ptpCompliance, managementGroup=managementGroup, encryptionAlgorithm=encryptionAlgorithm, linkModeOptimisation=linkModeOptimisation, sNMPTrapVersion=sNMPTrapVersion, syslogState=syslogState, vLANManagementPriority=vLANManagementPriority, signalStrengthRatio=signalStrengthRatio, sNTPServerIPAddress=sNTPServerIPAddress, receiveDataRate=receiveDataRate, channelALoopbackTrap=channelALoopbackTrap, receiveChannel=receiveChannel, sNMPTrapPortNumber=sNMPTrapPortNumber, syslogControl=syslogControl)
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", telnetAccessEnabled=telnetAccessEnabled, resetGroup=resetGroup, enabledDiagnosticAlarms=enabledDiagnosticAlarms, supplementaryGroup=supplementaryGroup, linkLoss=linkLoss, dataPortStatus=dataPortStatus, channelChangeTrap=channelChangeTrap, hTTPPortNumber=hTTPPortNumber, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, ethernetGroup=ethernetGroup, ethernetBridgingAvailability=ethernetBridgingAvailability, sNTPState=sNTPState, sNMPCommunityString=sNMPCommunityString, l2CPPriorityTable=l2CPPriorityTable, phyControl=phyControl, bridge=bridge, sNTPServerTableEntry=sNTPServerTableEntry, subnetMask=subnetMask, licence=licence, managementPortStatus=managementPortStatus, telecomsChannelSelection=telecomsChannelSelection, tFTPServerPortNumber=tFTPServerPortNumber, managementPortConfigurationMismatch=managementPortConfigurationMismatch, secondarySoftwareVersion=secondarySoftwareVersion, wirelessLinkStatus=wirelessLinkStatus, userTableNumber=userTableNumber, syslogStateTrap=syslogStateTrap, telecomsChannelBStatusTrap=telecomsChannelBStatusTrap, sMTPServerPortNumber=sMTPServerPortNumber, sNTPServerInternetAddressType=sNTPServerInternetAddressType, softwareVersion=softwareVersion, configurationGroup=configurationGroup, dfsGroup=dfsGroup, dataPortAutoNegotiation=dataPortAutoNegotiation, sntp=sntp, encryption=encryption, rxColorCode=rxColorCode, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, phyStatusGroup=phyStatusGroup, hAZLOCConfiguration=hAZLOCConfiguration, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, constantPowerSpectralDensity=constantPowerSpectralDensity, useVLANForManagementInterfaces=useVLANForManagementInterfaces, dfsPeaks=dfsPeaks, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType)
