...
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", linkSymmetry=linkSymmetry, tDDSynchronizationMode=tDDSynchronizationMode, installStatus=installStatus, mPLSTCPriorityTableIndex=mPLSTCPriorityTableIndex, syslogStateTrap=syslogStateTrap, vLANManagementVID=vLANManagementVID, sNMPTrapTable=sNMPTrapTable, phyStatus=phyStatus, sNTPServerInternetAddress=sNTPServerInternetAddress, altitude=altitude, syslogClient=syslogClient, dSCPManagementPriority=dSCPManagementPriority, iPv6GatewayAddress=iPv6GatewayAddress, channelALineCode=channelALineCode, ethernet=ethernet, bandwidthVariant=bandwidthVariant, mPLSTCPriorityTable=mPLSTCPriorityTable, regionCodeTrap=regionCodeTrap, userPassword=userPassword, daylightSaving=daylightSaving, tFTPSoftwareUpgradeStatus=tFTPSoftwareUpgradeStatus, rangingMode=rangingMode, dfsTable=dfsTable, syslogControl=syslogControl, sNTPServerTableEntry=sNTPServerTableEntry, managementPortDisabledWarningTrap=managementPortDisabledWarningTrap, supplementaryGroup=supplementaryGroup, maximumTransmitPower=maximumTransmitPower, targetMACAddress=targetMACAddress, tDDControl=tDDControl, sNTPPrimaryServer=sNTPPrimaryServer, mPLSTCPriorityTableEntry=mPLSTCPriorityTableEntry, userTableEntry=userTableEntry, targetRange=targetRange, sNMPTrapInternetAddress=sNMPTrapInternetAddress, receiveDataRate=receiveDataRate, range=range, receiveModulationMode=receiveModulationMode, longitude=longitude, telecom=telecom, byteErrorRatio=byteErrorRatio, sNMPPortNumber=sNMPPortNumber, iPv6PrefixLength=iPv6PrefixLength, qOSPriorityScheme=qOSPriorityScheme, iPDSCPPriorityTableIndex=iPDSCPPriorityTableIndex, sNMPTrapIPAddress=sNMPTrapIPAddress, iPVersion=iPVersion, telecomsInterface=telecomsInterface, dfsMeans=dfsMeans, tFTPServerPortNumber=tFTPServerPortNumber, systemClock=systemClock, softwareVersion=softwareVersion, alarmsGroup=alarmsGroup, ethernetPriorityTableNumber=ethernetPriorityTableNumber, sNTPServerTableIndex=sNTPServerTableIndex, mPLSTCPriorityQueueMapping=mPLSTCPriorityQueueMapping, telecomsChannelAStatus=telecomsChannelAStatus, tDDSynchronizationAlarmTrap=tDDSynchronizationAlarmTrap, remoteInternetAddressType=remoteInternetAddressType, managementPortStatus=managementPortStatus, rawReceivePower=rawReceivePower, wirelessLinkDisabledWarningTrap=wirelessLinkDisabledWarningTrap, smtpGroup=smtpGroup, channelBLoopback=channelBLoopback, dataPortAutoNegotiation=dataPortAutoNegotiation, hAZLOCConfiguration=hAZLOCConfiguration, channelChangeTrap=channelChangeTrap, dataPortConfigurationMismatch=dataPortConfigurationMismatch, snmpControlGroup=snmpControlGroup, constantPowerSpectralDensity=constantPowerSpectralDensity, channelBandwidth=channelBandwidth, vectorError=vectorError, l2CPPriorityTableNumber=l2CPPriorityTableNumber, configurationGroup=configurationGroup, installStatusTrap=installStatusTrap, ethernetPriorityQueueMapping=ethernetPriorityQueueMapping, telecomsChannelBStatusTrap=telecomsChannelBStatusTrap, sNTPState=sNTPState, channelBLoopbackTrap=channelBLoopbackTrap, siteName=siteName, unitOutOfCalibrationTrap=unitOutOfCalibrationTrap, ethernetPriorityTableEntry=ethernetPriorityTableEntry, transmitPower=transmitPower, dataPortAutoMdix=dataPortAutoMdix, managementPortConfigurationMismatchTrap=managementPortConfigurationMismatchTrap, sNMPSendAllTrapsAtStartup=sNMPSendAllTrapsAtStartup, licenceGroup=licenceGroup, versionsGroup=versionsGroup, encryptionAlgorithm=encryptionAlgorithm, syslogState=syslogState, telecomsChannelAStatusTrap=telecomsChannelAStatusTrap, cambium=cambium, dfsNineNinePointNinePercentiles=dfsNineNinePointNinePercentiles, ptp=ptp, sMTPServerInternetAddress=sMTPServerInternetAddress, incompatibleRegionCodesTrap=incompatibleRegionCodesTrap, tDDControlGroup=tDDControlGroup, iPDSCPPriorityTable=iPDSCPPriorityTable, ethernetCappedMaxWirelessSpeed=ethernetCappedMaxWirelessSpeed, telnetPortNumber=telnetPortNumber, receiveFreqKHz=receiveFreqKHz, userTableNumber=userTableNumber, incompatibleMasterAndSlaveTrap=incompatibleMasterAndSlaveTrap, receiveFreqMHz=receiveFreqMHz, sNTPLastSync=sNTPLastSync, linkModeOptimizationMismatchTrap=linkModeOptimizationMismatchTrap, dfsPeaks=dfsPeaks, linkModeOptimizationMismatch=linkModeOptimizationMismatch, sNMPTrapPortNumber=sNMPTrapPortNumber, sMTPSourceEmailAddress=sMTPSourceEmailAddress, tFTPSoftwareUpgradeStatusAdditionalText=tFTPSoftwareUpgradeStatusAdditionalText, PYSNMP_MODULE_ID=cambium, managementPortConfigurationMismatch=managementPortConfigurationMismatch, dataPortFiberStatusTrap=dataPortFiberStatusTrap, sNMPTrapVersion=sNMPTrapVersion, useVLANForManagementInterfaces=useVLANForManagementInterfaces, channelALoopbackTrap=channelALoopbackTrap, ptpTraps=ptpTraps, telecomGroup=telecomGroup, notificationsGroup=notificationsGroup, ethernetGroup=ethernetGroup, sNMPTrapTableNumber=sNMPTrapTableNumber, userName=userName, pubStatsGroup=pubStatsGroup, sMTPEmailAlert=sMTPEmailAlert, iPDSCPPriorityTableNumber=iPDSCPPriorityTableNumber, telecomsChannelBStatus=telecomsChannelBStatus, telecomsChannelSelection=telecomsChannelSelection, wirelessLinkDisabledWarning=wirelessLinkDisabledWarning, sNTPPollInterval=sNTPPollInterval, ethernetPriorityTable=ethernetPriorityTable, enabledDiagnosticAlarms=enabledDiagnosticAlarms, dfsGroup=dfsGroup, securityLevel=securityLevel, searchState=searchState, sNTPSync=sNTPSync, sNTPServerTableNumber=sNTPServerTableNumber, linkModeOptimisation=linkModeOptimisation, hTTPPortNumber=hTTPPortNumber, remoteRxTargetPower=remoteRxTargetPower, transmitFreqMHz=transmitFreqMHz, latitude=latitude, managementPortStatusTrap=managementPortStatusTrap, sNMPTrapTableIndex=sNMPTrapTableIndex, masterSlaveMode=masterSlaveMode, iPv6AutoConfiguredLinkLocalAddress=iPv6AutoConfiguredLinkLocalAddress, ptpGroups=ptpGroups, userConfiguredMaxModulationMode=userConfiguredMaxModulationMode, aAAControl=aAAControl, tDDSynchronizationStatus=tDDSynchronizationStatus, receiveModulationModeDetail=receiveModulationModeDetail, dfsTableEntry=dfsTableEntry, sNTPServerPortNumber=sNTPServerPortNumber, tFTPSoftwareUpgradeFileName=tFTPSoftwareUpgradeFileName, eIRP=eIRP, versions=versions, signalStrengthRatio=signalStrengthRatio, sMTPServerInternetAddressType=sMTPServerInternetAddressType, phyControlGroup=phyControlGroup, smtp=smtp, sNMPTrapInternetAddressType=sNMPTrapInternetAddressType, sNTPServerStatus=sNTPServerStatus, vLANManagementPriority=vLANManagementPriority, antennaGain=antennaGain, dataPortStatus=dataPortStatus, transmitFreqKHz=transmitFreqKHz, gatewayIPAddress=gatewayIPAddress, aAAControlGroup=aAAControlGroup, cableLoss=cableLoss, iPDSCPPriorityQueueMapping=iPDSCPPriorityQueueMapping, productName=productName, bridgeGroup=bridgeGroup, subnetMask=subnetMask, telnetAccessEnabled=telnetAccessEnabled, iPv6Address=iPv6Address, ptmp=ptmp, managementPortWirelessDownAlert=managementPortWirelessDownAlert, tFTPServerInternetAddress=tFTPServerInternetAddress, frequencyVariant=frequencyVariant, userTableIndex=userTableIndex, ptpTrapPrefix=ptpTrapPrefix, unitOutOfCalibration=unitOutOfCalibration, hTTPSPortNumber=hTTPSPortNumber, bridge=bridge, resetGroup=resetGroup, managementGroup=managementGroup, remoteInternetAddress=remoteInternetAddress, rxColorCode=rxColorCode, userEnabled=userEnabled, encryption=encryption, noWirelessChannelAvailable=noWirelessChannelAvailable, linkLoss=linkLoss, iPDSCPPriorityTableEntry=iPDSCPPriorityTableEntry, sMTPServerPortNumber=sMTPServerPortNumber, unknownNetworkPriorityQueueMapping=unknownNetworkPriorityQueueMapping, secondarySoftwareVersion=secondarySoftwareVersion, dataPortConfigurationMismatchTrap=dataPortConfigurationMismatchTrap, pubStats=pubStats, localPacketFiltering=localPacketFiltering, ptp600=ptp600, sNMPEnabledTraps=sNMPEnabledTraps, tFTPStartSoftwareUpgrade=tFTPStartSoftwareUpgrade, phyControl=phyControl, aggregateDataRate=aggregateDataRate, sNTPSyncTrap=sNTPSyncTrap, channelBLineCode=channelBLineCode, channelACableLength=channelACableLength, dataPortAutoNegAdvertisement=dataPortAutoNegAdvertisement, dataPortSpeedAndDuplex=dataPortSpeedAndDuplex, sNMPv3Enable=sNMPv3Enable, configuration=configuration, ethernetBridgingStatusAlarmTrap=ethernetBridgingStatusAlarmTrap, licence=licence, timeZone=timeZone, groupID=groupID, syslogLocalWrappedTrap=syslogLocalWrappedTrap, dataPortFiberStatus=dataPortFiberStatus, transmitDataRate=transmitDataRate, ptpCompliance=ptpCompliance, phyStatusGroup=phyStatusGroup, dfsTableNumber=dfsTableNumber, dfsTableIndex=dfsTableIndex, hTTPSAccessEnabled=hTTPSAccessEnabled, installArmStateTrap=installArmStateTrap, managementMode=managementMode, transmitChannel=transmitChannel, syslogLocalNearlyFullTrap=syslogLocalNearlyFullTrap, secureModeAlarm=secureModeAlarm, iPv4Address=iPv4Address, supplementary=supplementary, txColorCode=txColorCode, sNTPServerRetries=sNTPServerRetries, sNTPServerTimeout=sNTPServerTimeout, receiveChannel=receiveChannel, hardwareVersion=hardwareVersion, linkName=linkName, bootVersion=bootVersion, noWirelessChannelAvailableTrap=noWirelessChannelAvailableTrap, dataPortWirelessDownAlert=dataPortWirelessDownAlert, l2CPPriorityTableIndex=l2CPPriorityTableIndex, productVariant=productVariant, ethernetFiberSupport=ethernetFiberSupport, dataPortDisabledWarning=dataPortDisabledWarning, snmpControl=snmpControl, dataPortStatusTrap=dataPortStatusTrap, dataPortDisabledWarningTrap=dataPortDisabledWarningTrap, ethernetBridgingAvailability=ethernetBridgingAvailability, sNTPServerIPAddress=sNTPServerIPAddress, receivePower=receivePower, dfs=dfs, l2CPPriorityTable=l2CPPriorityTable, transmitModulationMode=transmitModulationMode, automaticTxPowerControl=automaticTxPowerControl, sNMPTrapTableEntry=sNMPTrapTableEntry)
mibBuilder.exportSymbols("CAMBIUM-PTP600-MIB", dfsImpulsiveInterferenceTrap=dfsImpulsiveInterferenceTrap, channelALoopback=channelALoopback, userRole=userRole, tFTPServerIPAddress=tFTPServerIPAddress, tDDSynchronizationAlarm=tDDSynchronizationAlarm, mPLSTCPriorityTableNumber=mPLSTCPriorityTableNumber, installArmState=installArmState, secureModeAlarmTrap=secureModeAlarmTrap, systemReset=systemReset, syslogControlGroup=syslogControlGroup, remoteIPAddress=remoteIPAddress, sNMPCommunityString=sNMPCommunityString, managementPortSpeedAndDuplex=managementPortSpeedAndDuplex, wirelessLinkStatus=wirelessLinkStatus, ethernetBridgingStatusAlarm=ethernetBridgingStatusAlarm, regionCode=regionCode, channelBCableLength=channelBCableLength, sNTPPrimaryServerDeadTime=sNTPPrimaryServerDeadTime, remoteMaximumTransmitPower=remoteMaximumTransmitPower, alarms=alarms, ethernetPriorityTableIndex=ethernetPriorityTableIndex, tFTPServerInternetAddressType=tFTPServerInternetAddressType, sNMPTrapReceiverEnabled=sNMPTrapReceiverEnabled, reset=reset, ethernetBridgingStatus=ethernetBridgingStatus, incompatibleRegionCodes=incompatibleRegionCodes, sMTPDestinationEmailAddress=sMTPDestinationEmailAddress, encryptionGroup=encryptionGroup, managementPortDisabledWarning=managementPortDisabledWarning, sntp=sntp, sMTPEnabledMessages=sMTPEnabledMessages, l2CPPriorityTableEntry=l2CPPriorityTableEntry, userTable=userTable, sNTPServerTable=sNTPServerTable, sntpGroup=sntpGroup, wirelessLinkAvailability=wirelessLinkAvailability, incompatibleMasterAndSlave=incompatibleMasterAndSlave, sMTPServerIPAddress=sMTPServerIPAddress, hTTPAccessEnabled=hTTPAccessEnabled, sNTPServerInternetAddressType=sNTPServerInternetAddressType, tFTPSoftwareUpgradeStatusText=tFTPSoftwareUpgradeStatusText, l2CPPriorityQueueMapping=l2CPPriorityQueueMapping, management=management, syslogClientTrap=syslogClientTrap, accessMethod=accessMethod)
