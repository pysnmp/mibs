...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, PYSNMP_MODULE_ID=dlsw, dlswTConn=dlswTConn, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitGroup=dlswCircuitGroup, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, TAddress=TAddress, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnSpecific=dlswTConnSpecific, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirNBEntries=dlswDirNBEntries, dlswDirLocateNBName=dlswDirLocateNBName, dlswCompliances=dlswCompliances, dlswNodeStatus=dlswNodeStatus, dlswMIB=dlswMIB, dlswCircuitEntry=dlswCircuitEntry, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnConfigOpens=dlswTConnConfigOpens, dlsw=dlsw, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitOrigin=dlswCircuitOrigin, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnStat=dlswTConnStat, dlswDirNBIndex=dlswDirNBIndex, dlswDirNBLocation=dlswDirNBLocation, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswConformance=dlswConformance, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirNBName=dlswDirNBName, dlswNode=dlswNode, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTrapTConnDown=dlswTrapTConnDown, dlswTrapControl=dlswTrapControl, dlswDirNBStatus=dlswDirNBStatus, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswIfSapList=dlswIfSapList, dlswDomains=dlswDomains, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTraps=dlswTraps, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitStat=dlswCircuitStat, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswIfTable=dlswIfTable, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirCache=dlswDirCache, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswNodeGroup=dlswNodeGroup, MacAddressNC=MacAddressNC, dlswInterface=dlswInterface, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswNodeVersion=dlswNodeVersion, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirCompliance=dlswDirCompliance, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirMacTable=dlswDirMacTable, DlswTCPAddress=DlswTCPAddress, dlswDirMacIndex=dlswDirMacIndex, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirMacLocation=dlswDirMacLocation, dlswDirNBTable=dlswDirNBTable, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperState=dlswTConnOperState, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirMacLocationType=dlswDirMacLocationType, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, NBName=NBName, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirGroup=dlswDirGroup, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperEntry=dlswTConnOperEntry, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirMacMac=dlswDirMacMac, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswSdlcLsEntry=dlswSdlcLsEntry, LFSize=LFSize, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnStatGroup=dlswTConnStatGroup, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirStat=dlswDirStat, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirNBNameType=dlswDirNBNameType, dlswDirLocate=dlswDirLocate, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, null=null, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTCPDomain=dlswTCPDomain, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, DlcType=DlcType, dlswCircuit=dlswCircuit, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirLocateMacMac=dlswDirLocateMacMac, EndStationLocation=EndStationLocation, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitTable=dlswCircuitTable, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswSdlc=dlswSdlc, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitState=dlswCircuitState, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswNotificationGroup=dlswNotificationGroup, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswGroups=dlswGroups, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirectory=dlswDirectory, dlswDirMacMask=dlswDirMacMask, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswIfEntry=dlswIfEntry, dlswTConnTcp=dlswTConnTcp)
