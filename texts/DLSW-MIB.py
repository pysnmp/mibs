...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirMacIndex=dlswDirMacIndex, dlswTConn=dlswTConn, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirNBEntry=dlswDirNBEntry, NBName=NBName, dlswTConnOperState=dlswTConnOperState, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirGroup=dlswDirGroup, dlswDirNBName=dlswDirNBName, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnConfigTable=dlswTConnConfigTable, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitState=dlswCircuitState, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswConformance=dlswConformance, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirStat=dlswDirStat, dlswDirMacMac=dlswDirMacMac, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitGroup=dlswCircuitGroup, dlswTCPDomain=dlswTCPDomain, dlswCircuitTable=dlswCircuitTable, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswNodeVendorID=dlswNodeVendorID, dlswNode=dlswNode, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswIfSapList=dlswIfSapList, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirMacTable=dlswDirMacTable, dlswDirMacEntryType=dlswDirMacEntryType, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswNodeVersion=dlswNodeVersion, dlswTrapCircuitUp=dlswTrapCircuitUp, null=null, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirLocate=dlswDirLocate, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuit=dlswCircuit, dlswDomains=dlswDomains, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, TAddress=TAddress, dlswCoreCompliance=dlswCoreCompliance, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswIfRowStatus=dlswIfRowStatus, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCompliances=dlswCompliances, PYSNMP_MODULE_ID=dlsw, dlswDirMacEntries=dlswDirMacEntries, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirCache=dlswDirCache, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirNBLocation=dlswDirNBLocation, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitS2Location=dlswCircuitS2Location, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, LFSize=LFSize, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitStat=dlswCircuitStat, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswIfTable=dlswIfTable, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswNotificationGroup=dlswNotificationGroup, dlswDirCompliance=dlswDirCompliance, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirNBEntries=dlswDirNBEntries, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswNodeVersionString=dlswNodeVersionString, dlsw=dlsw, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirMacEntry=dlswDirMacEntry, dlswTConnStat=dlswTConnStat, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswMIB=dlswMIB, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTrapControl=dlswTrapControl, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirNBTable=dlswDirNBTable, dlswDirNBStatus=dlswDirNBStatus, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswIfEntry=dlswIfEntry, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTraps=dlswTraps, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirMacStatus=dlswDirMacStatus, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswNodeStatus=dlswNodeStatus, dlswNodeGroup=dlswNodeGroup, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnTcp=dlswTConnTcp, dlswSdlc=dlswSdlc, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, DlswTCPAddress=DlswTCPAddress, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirNBGroup=dlswDirNBGroup, dlswDirMacMask=dlswDirMacMask, DlcType=DlcType, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnConfigSetupType=dlswTConnConfigSetupType, EndStationLocation=EndStationLocation, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuitEntry=dlswCircuitEntry, MacAddressNC=MacAddressNC, dlswDirectory=dlswDirectory, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswGroups=dlswGroups, dlswInterface=dlswInterface, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl)
