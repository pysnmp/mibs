...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnTcp=dlswTConnTcp, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirNBStatus=dlswDirNBStatus, PYSNMP_MODULE_ID=dlsw, dlswDirMacIndex=dlswDirMacIndex, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuit=dlswCircuit, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirMacEntryType=dlswDirMacEntryType, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitState=dlswCircuitState, dlswSdlc=dlswSdlc, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTrapControl=dlswTrapControl, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswInterface=dlswInterface, dlswDirNBNameType=dlswDirNBNameType, dlswNodeStatus=dlswNodeStatus, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirMacEntries=dlswDirMacEntries, dlswDirNBIndex=dlswDirNBIndex, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswNodeVersion=dlswNodeVersion, dlswTraps=dlswTraps, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirGroup=dlswDirGroup, dlswNode=dlswNode, dlswTConnStat=dlswTConnStat, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswNodeVendorID=dlswNodeVendorID, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirLocate=dlswDirLocate, dlswCircuitPriority=dlswCircuitPriority, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuitOrigin=dlswCircuitOrigin, dlswNodeGroup=dlswNodeGroup, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnSpecific=dlswTConnSpecific, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitStat=dlswCircuitStat, dlswDirCache=dlswDirCache, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirCompliance=dlswDirCompliance, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, LFSize=LFSize, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitTable=dlswCircuitTable, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswInterfaceGroup=dlswInterfaceGroup, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirNBName=dlswDirNBName, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlsw=dlsw, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitS1DlcType=dlswCircuitS1DlcType, DlcType=DlcType, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTCPDomain=dlswTCPDomain, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirMacMac=dlswDirMacMac, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirMacLFSize=dlswDirMacLFSize, TAddress=TAddress, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswIfTable=dlswIfTable, dlswIfEntry=dlswIfEntry, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDomains=dlswDomains, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirNBLocation=dlswDirNBLocation, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswConformance=dlswConformance, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswNodeVersionString=dlswNodeVersionString, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirNBGroup=dlswDirNBGroup, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirNBEntries=dlswDirNBEntries, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, DlswTCPAddress=DlswTCPAddress, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirectory=dlswDirectory, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswSdlcGroup=dlswSdlcGroup, dlswTConnConfigTable=dlswTConnConfigTable, dlswDirMacMask=dlswDirMacMask, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCompliances=dlswCompliances, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, null=null, dlswNodeNBGroup=dlswNodeNBGroup, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirStat=dlswDirStat, dlswDirMacTable=dlswDirMacTable, dlswMIB=dlswMIB, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirLocateMacLocation=dlswDirLocateMacLocation, MacAddressNC=MacAddressNC, dlswCircuitGroup=dlswCircuitGroup, EndStationLocation=EndStationLocation, dlswTConn=dlswTConn, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswIfSapList=dlswIfSapList, dlswDirNBTable=dlswDirNBTable, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswGroups=dlswGroups, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirLocateGroup=dlswDirLocateGroup, dlswNotificationGroup=dlswNotificationGroup, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirMacLocation=dlswDirMacLocation, NBName=NBName, dlswTConnOperState=dlswTConnOperState, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize)
