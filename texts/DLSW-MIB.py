...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirMacLocationType=dlswDirMacLocationType, dlswSdlcCompliance=dlswSdlcCompliance, PYSNMP_MODULE_ID=dlsw, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperTable=dlswTConnOperTable, dlswDirMacEntries=dlswDirMacEntries, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitStatActives=dlswCircuitStatActives, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitStat=dlswCircuitStat, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswNodeUpTime=dlswNodeUpTime, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTrapTConnDown=dlswTrapTConnDown, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirGroup=dlswDirGroup, dlswIfEntry=dlswIfEntry, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, TAddress=TAddress, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswDirMacTable=dlswDirMacTable, DlswTCPAddress=DlswTCPAddress, dlswDirNBEntry=dlswDirNBEntry, dlswDirNBStatus=dlswDirNBStatus, dlswDirLocateGroup=dlswDirLocateGroup, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirNBTable=dlswDirNBTable, dlswNodeVendorID=dlswNodeVendorID, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirMacStatus=dlswDirMacStatus, dlswDirMacEntry=dlswDirMacEntry, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitGroup=dlswCircuitGroup, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCoreCompliance=dlswCoreCompliance, dlswDirMacIndex=dlswDirMacIndex, dlswDirMacMask=dlswDirMacMask, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirMacMac=dlswDirMacMac, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirNBIndex=dlswDirNBIndex, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswGroups=dlswGroups, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirNBEntries=dlswDirNBEntries, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswMIB=dlswMIB, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirectory=dlswDirectory, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, EndStationLocation=EndStationLocation, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCompliances=dlswCompliances, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnTcp=dlswTConnTcp, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirLocate=dlswDirLocate, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuitS2Location=dlswCircuitS2Location, MacAddressNC=MacAddressNC, dlswTConnStatGroup=dlswTConnStatGroup, dlswCircuitPriority=dlswCircuitPriority, NBName=NBName, dlswInterface=dlswInterface, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocateNBName=dlswDirLocateNBName, dlswTrapControl=dlswTrapControl, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitState=dlswCircuitState, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswIfSapList=dlswIfSapList, dlswCircuitTable=dlswCircuitTable, dlswCircuit=dlswCircuit, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswNodeGroup=dlswNodeGroup, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirNBEntryType=dlswDirNBEntryType, dlswDomains=dlswDomains, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTCPDomain=dlswTCPDomain, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswNodeVersionString=dlswNodeVersionString, dlsw=dlsw, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirNBName=dlswDirNBName, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperState=dlswTConnOperState, dlswTConnStat=dlswTConnStat, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswNodeStatus=dlswNodeStatus, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTraps=dlswTraps, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirNBGroup=dlswDirNBGroup, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirCompliance=dlswDirCompliance, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirStat=dlswDirStat, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswNodeVersion=dlswNodeVersion, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirCache=dlswDirCache, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswIfTable=dlswIfTable, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitStatGroup=dlswCircuitStatGroup, DlcType=DlcType, null=null, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswNode=dlswNode, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitEntry=dlswCircuitEntry, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswConformance=dlswConformance, dlswTConnOperEntry=dlswTConnOperEntry, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, LFSize=LFSize, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConn=dlswTConn, dlswSdlc=dlswSdlc, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswDirLocateMacMac=dlswDirLocateMacMac)
