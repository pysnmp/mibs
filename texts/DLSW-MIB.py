...
mibBuilder.exportSymbols("DLSW-MIB", DlswTCPAddress=DlswTCPAddress, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswNodeVersion=dlswNodeVersion, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperTable=dlswTConnOperTable, dlswDirNBEntries=dlswDirNBEntries, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirMacTable=dlswDirMacTable, dlswNodeVendorID=dlswNodeVendorID, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswNotificationGroup=dlswNotificationGroup, dlswDirGroup=dlswDirGroup, dlswDirMacMask=dlswDirMacMask, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswSdlcCompliance=dlswSdlcCompliance, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirCompliance=dlswDirCompliance, null=null, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnTcp=dlswTConnTcp, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswSdlcGroup=dlswSdlcGroup, EndStationLocation=EndStationLocation, dlswNodeVersionString=dlswNodeVersionString, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTrapTConnUp=dlswTrapTConnUp, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnStat=dlswTConnStat, dlswDirStat=dlswDirStat, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperGroup=dlswTConnOperGroup, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnOperState=dlswTConnOperState, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswSdlc=dlswSdlc, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTraps=dlswTraps, dlswDirNBGroup=dlswDirNBGroup, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCoreCompliance=dlswCoreCompliance, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirectory=dlswDirectory, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitStat=dlswCircuitStat, dlswDirNBIndex=dlswDirNBIndex, NBName=NBName, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirNBEntry=dlswDirNBEntry, dlswDirNBName=dlswDirNBName, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirLocateGroup=dlswDirLocateGroup, dlswIfRowStatus=dlswIfRowStatus, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirLocateNBLocation=dlswDirLocateNBLocation, DlcType=DlcType, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirLocateNBName=dlswDirLocateNBName, dlswGroups=dlswGroups, TAddress=TAddress, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirMacEntries=dlswDirMacEntries, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirMacIndex=dlswDirMacIndex, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuit=dlswCircuit, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCircuitS2Location=dlswCircuitS2Location, dlswIfSapList=dlswIfSapList, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswConformance=dlswConformance, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitTable=dlswCircuitTable, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswNodeStatus=dlswNodeStatus, dlswDirLocate=dlswDirLocate, dlswInterfaceGroup=dlswInterfaceGroup, dlswInterface=dlswInterface, PYSNMP_MODULE_ID=dlsw, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswNodeGroup=dlswNodeGroup, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswNode=dlswNode, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConn=dlswTConn, dlswTConnSpecific=dlswTConnSpecific, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirMacStatus=dlswDirMacStatus, dlswCompliances=dlswCompliances, MacAddressNC=MacAddressNC, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirNBTable=dlswDirNBTable, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitGroup=dlswCircuitGroup, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitState=dlswCircuitState, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirCache=dlswDirCache, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirMacLFSize=dlswDirMacLFSize, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlsw=dlsw, LFSize=LFSize, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswIfEntry=dlswIfEntry, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirMacMac=dlswDirMacMac, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTrapControl=dlswTrapControl, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswMIB=dlswMIB, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDomains=dlswDomains, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTCPDomain=dlswTCPDomain, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswIfTable=dlswIfTable, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperDiscTime=dlswTConnOperDiscTime)
