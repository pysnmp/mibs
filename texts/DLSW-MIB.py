...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirMacLFSize=dlswDirMacLFSize, dlswDirMacIndex=dlswDirMacIndex, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswNotificationGroup=dlswNotificationGroup, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperState=dlswTConnOperState, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswCircuitStat=dlswCircuitStat, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswNodeUpTime=dlswNodeUpTime, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTrapControl=dlswTrapControl, dlswDirLocateGroup=dlswDirLocateGroup, dlswCircuitGroup=dlswCircuitGroup, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirNBIndex=dlswDirNBIndex, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswSdlcGroup=dlswSdlcGroup, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCompliances=dlswCompliances, dlswDirStat=dlswDirStat, MacAddressNC=MacAddressNC, NBName=NBName, dlswSdlc=dlswSdlc, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirMacTable=dlswDirMacTable, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, DlcType=DlcType, dlswTConnConfigTable=dlswTConnConfigTable, TAddress=TAddress, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirMacEntryType=dlswDirMacEntryType, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitStatActives=dlswCircuitStatActives, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirNBNameType=dlswDirNBNameType, dlswDirNBStatus=dlswDirNBStatus, dlswTConnStatGroup=dlswTConnStatGroup, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswNodeGroup=dlswNodeGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswCircuit=dlswCircuit, dlswIfEntry=dlswIfEntry, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirMacEntries=dlswDirMacEntries, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswDirLocateMacLocation=dlswDirLocateMacLocation, LFSize=LFSize, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswIfSapList=dlswIfSapList, PYSNMP_MODULE_ID=dlsw, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCoreCompliance=dlswCoreCompliance, dlswTCPDomain=dlswTCPDomain, dlswDirNBTable=dlswDirNBTable, dlswDirCompliance=dlswDirCompliance, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitPriority=dlswCircuitPriority, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswGroups=dlswGroups, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirMacMask=dlswDirMacMask, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirNBLocation=dlswDirNBLocation, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswInterface=dlswInterface, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTrapCntlTConn=dlswTrapCntlTConn, null=null, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswNodeStatus=dlswNodeStatus, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirNBName=dlswDirNBName, dlswCircuitState=dlswCircuitState, dlswTConn=dlswTConn, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirectory=dlswDirectory, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCircuitStateTime=dlswCircuitStateTime, dlswSdlcLsTable=dlswSdlcLsTable, dlswDirMacMac=dlswDirMacMac, dlswConformance=dlswConformance, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswNodeNBGroup=dlswNodeNBGroup, EndStationLocation=EndStationLocation, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitTable=dlswCircuitTable, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnSpecific=dlswTConnSpecific, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnTcp=dlswTConnTcp, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperGroup=dlswTConnOperGroup, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirLocate=dlswDirLocate, dlswIfTable=dlswIfTable, dlswSdlcLsEntry=dlswSdlcLsEntry, dlsw=dlsw, dlswMIB=dlswMIB, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTraps=dlswTraps, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirGroup=dlswDirGroup, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswNodeVendorID=dlswNodeVendorID, dlswNode=dlswNode, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, DlswTCPAddress=DlswTCPAddress, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDomains=dlswDomains, dlswTConnStat=dlswTConnStat, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirNBEntries=dlswDirNBEntries, dlswDirCache=dlswDirCache, dlswTrapCircuitUp=dlswTrapCircuitUp)
