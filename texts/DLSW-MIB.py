...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirNBNameType=dlswDirNBNameType, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBEntry=dlswDirNBEntry, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnSpecific=dlswTConnSpecific, dlswDirMacMask=dlswDirMacMask, dlswNotificationGroup=dlswNotificationGroup, dlswDirNBEntries=dlswDirNBEntries, dlswDirectory=dlswDirectory, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswIfSapList=dlswIfSapList, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitPriority=dlswCircuitPriority, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirMacLocation=dlswDirMacLocation, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswSdlcLsTable=dlswSdlcLsTable, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirLocateMacEntry=dlswDirLocateMacEntry, DlcType=DlcType, dlswCoreCompliance=dlswCoreCompliance, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswNodeVersionString=dlswNodeVersionString, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirMacIndex=dlswDirMacIndex, dlswDirCompliance=dlswDirCompliance, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirGroup=dlswDirGroup, dlswSdlcCompliance=dlswSdlcCompliance, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswCircuitStat=dlswCircuitStat, dlswMIB=dlswMIB, dlswDirLocateNBName=dlswDirLocateNBName, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswNode=dlswNode, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperState=dlswTConnOperState, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlsw=dlsw, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirNBGroup=dlswDirNBGroup, dlswInterfaceGroup=dlswInterfaceGroup, EndStationLocation=EndStationLocation, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacEntries=dlswDirMacEntries, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirMacTable=dlswDirMacTable, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirCache=dlswDirCache, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuit=dlswCircuit, dlswTConnOperEntry=dlswTConnOperEntry, dlswDomains=dlswDomains, dlswDirMacEntry=dlswDirMacEntry, dlswDirMacEntryType=dlswDirMacEntryType, dlswTCPDomain=dlswTCPDomain, dlswDirMacMac=dlswDirMacMac, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswGroups=dlswGroups, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswNodeVendorID=dlswNodeVendorID, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitTable=dlswCircuitTable, dlswDirNBLocation=dlswDirNBLocation, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitState=dlswCircuitState, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirNBRowStatus=dlswDirNBRowStatus, DlswTCPAddress=DlswTCPAddress, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitGroup=dlswCircuitGroup, dlswTConnTcp=dlswTConnTcp, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswSdlc=dlswSdlc, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnStat=dlswTConnStat, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswInterface=dlswInterface, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirStat=dlswDirStat, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirLocate=dlswDirLocate, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirNBTable=dlswDirNBTable, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCompliances=dlswCompliances, dlswIfEntry=dlswIfEntry, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswNodeVersion=dlswNodeVersion, LFSize=LFSize, dlswDirNBName=dlswDirNBName, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, TAddress=TAddress, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, null=null, dlswConformance=dlswConformance, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConn=dlswTConn, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCircuitS2Location=dlswCircuitS2Location, dlswTraps=dlswTraps, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, MacAddressNC=MacAddressNC, dlswNodeGroup=dlswNodeGroup, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperGroup=dlswTConnOperGroup, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirMacLocationType=dlswDirMacLocationType, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswIfTable=dlswIfTable, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, PYSNMP_MODULE_ID=dlsw, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitEntry=dlswCircuitEntry, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, NBName=NBName, dlswDirNBLFSize=dlswDirNBLFSize, dlswTrapControl=dlswTrapControl, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswNodeStatus=dlswNodeStatus, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswNodeUpTime=dlswNodeUpTime, dlswDirNBStatus=dlswDirNBStatus)
