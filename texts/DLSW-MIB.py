...
mibBuilder.exportSymbols("DLSW-MIB", dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitGroup=dlswCircuitGroup, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswNodeUpTime=dlswNodeUpTime, dlswDirMacEntryType=dlswDirMacEntryType, PYSNMP_MODULE_ID=dlsw, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCoreCompliance=dlswCoreCompliance, dlswDirNBEntries=dlswDirNBEntries, dlswDirCompliance=dlswDirCompliance, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswIfRowStatus=dlswIfRowStatus, LFSize=LFSize, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConn=dlswTConn, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDomains=dlswDomains, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirNBGroup=dlswDirNBGroup, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirNBLocation=dlswDirNBLocation, DlswTCPAddress=DlswTCPAddress, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswNode=dlswNode, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTraps=dlswTraps, dlswConformance=dlswConformance, dlswTConnOperEntry=dlswTConnOperEntry, dlswNodeVersionString=dlswNodeVersionString, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperTable=dlswTConnOperTable, dlswSdlc=dlswSdlc, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswCircuitStatActives=dlswCircuitStatActives, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirNBStatus=dlswDirNBStatus, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirGroup=dlswDirGroup, dlswDirectory=dlswDirectory, dlswDirMacMask=dlswDirMacMask, MacAddressNC=MacAddressNC, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnSpecific=dlswTConnSpecific, dlswDirMacEntry=dlswDirMacEntry, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirNBEntry=dlswDirNBEntry, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirMacMac=dlswDirMacMac, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTrapTConnDown=dlswTrapTConnDown, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirCache=dlswDirCache, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswIfEntry=dlswIfEntry, dlswTConnTcp=dlswTConnTcp, dlswNodeGroup=dlswNodeGroup, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlsw=dlsw, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCircuitPriority=dlswCircuitPriority, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswNodeStatus=dlswNodeStatus, dlswDirNBTable=dlswDirNBTable, dlswCircuitTable=dlswCircuitTable, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTCPDomain=dlswTCPDomain, dlswTConnConfigTable=dlswTConnConfigTable, dlswDirMacTable=dlswDirMacTable, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, NBName=NBName, dlswTrapTConnUp=dlswTrapTConnUp, dlswIfTable=dlswIfTable, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacIndex=dlswDirMacIndex, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswMIB=dlswMIB, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswNodeVendorID=dlswNodeVendorID, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirNBName=dlswDirNBName, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswInterfaceGroup=dlswInterfaceGroup, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitStat=dlswCircuitStat, dlswGroups=dlswGroups, dlswDirNBIndex=dlswDirNBIndex, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnStat=dlswTConnStat, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirStat=dlswDirStat, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswNodeNBGroup=dlswNodeNBGroup, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, TAddress=TAddress, dlswNodeVersion=dlswNodeVersion, dlswTrapControl=dlswTrapControl, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, DlcType=DlcType, dlswCircuit=dlswCircuit, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCompliances=dlswCompliances, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitState=dlswCircuitState, dlswInterface=dlswInterface, dlswTConnOperDiscTime=dlswTConnOperDiscTime, null=null, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswIfSapList=dlswIfSapList, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirLocateCompliance=dlswDirLocateCompliance, EndStationLocation=EndStationLocation, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperState=dlswTConnOperState, dlswDirLocate=dlswDirLocate, dlswTConnTcpOperTable=dlswTConnTcpOperTable)
