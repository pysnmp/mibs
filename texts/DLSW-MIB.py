...
mibBuilder.exportSymbols("DLSW-MIB", dlswNodeUpTime=dlswNodeUpTime, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirLocate=dlswDirLocate, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitState=dlswCircuitState, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswSdlcGroup=dlswSdlcGroup, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnTcp=dlswTConnTcp, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, NBName=NBName, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnSpecific=dlswTConnSpecific, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirNBName=dlswDirNBName, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, DlcType=DlcType, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCoreCompliance=dlswCoreCompliance, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswConformance=dlswConformance, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitS1Dlc=dlswCircuitS1Dlc, TAddress=TAddress, null=null, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswNode=dlswNode, dlswDirectory=dlswDirectory, dlswMIB=dlswMIB, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirCache=dlswDirCache, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirNBIndex=dlswDirNBIndex, dlswCompliances=dlswCompliances, dlswCircuitStatActives=dlswCircuitStatActives, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirMacMac=dlswDirMacMac, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirStat=dlswDirStat, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuit=dlswCircuit, dlswDirNBEntryType=dlswDirNBEntryType, dlswGroups=dlswGroups, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConn=dlswTConn, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswNodeVersion=dlswNodeVersion, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTrapControl=dlswTrapControl, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnConfigTable=dlswTConnConfigTable, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitEntry=dlswCircuitEntry, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirMacMask=dlswDirMacMask, dlswIfEntry=dlswIfEntry, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswNodeStatus=dlswNodeStatus, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswNodeNBGroup=dlswNodeNBGroup, dlswTraps=dlswTraps, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirNBLocation=dlswDirNBLocation, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTCPDomain=dlswTCPDomain, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswIfSapList=dlswIfSapList, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirMacTable=dlswDirMacTable, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirNBNameType=dlswDirNBNameType, dlswDomains=dlswDomains, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswCircuitS1Sap=dlswCircuitS1Sap, EndStationLocation=EndStationLocation, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirGroup=dlswDirGroup, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswNodeVersionString=dlswNodeVersionString, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirMacStatus=dlswDirMacStatus, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnStat=dlswTConnStat, dlsw=dlsw, dlswDirMacLocation=dlswDirMacLocation, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswIfTable=dlswIfTable, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, MacAddressNC=MacAddressNC, dlswCircuitStat=dlswCircuitStat, dlswTConnOperTable=dlswTConnOperTable, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperState=dlswTConnOperState, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirMacIndex=dlswDirMacIndex, dlswInterface=dlswInterface, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitGroup=dlswCircuitGroup, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitTable=dlswCircuitTable, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperGroup=dlswTConnOperGroup, PYSNMP_MODULE_ID=dlsw, dlswDirNBGroup=dlswDirNBGroup, dlswSdlc=dlswSdlc, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirMacEntries=dlswDirMacEntries, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, DlswTCPAddress=DlswTCPAddress, dlswNodeGroup=dlswNodeGroup, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswNotificationGroup=dlswNotificationGroup, dlswDirCompliance=dlswDirCompliance, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirNBTable=dlswDirNBTable, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnConfigEntryType=dlswTConnConfigEntryType, LFSize=LFSize, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirLocateNBEntry=dlswDirLocateNBEntry)
