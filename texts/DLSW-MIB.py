...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnConfigSetupType=dlswTConnConfigSetupType, NBName=NBName, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitStateTime=dlswCircuitStateTime, LFSize=LFSize, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, MacAddressNC=MacAddressNC, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswNotificationGroup=dlswNotificationGroup, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, DlcType=DlcType, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperTable=dlswTConnOperTable, dlswMIB=dlswMIB, null=null, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirNBTable=dlswDirNBTable, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirectory=dlswDirectory, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswNodeVersion=dlswNodeVersion, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, PYSNMP_MODULE_ID=dlsw, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswNodeStatus=dlswNodeStatus, dlswCircuitTable=dlswCircuitTable, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnTcp=dlswTConnTcp, dlswIfEntry=dlswIfEntry, dlswCircuitEntry=dlswCircuitEntry, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTraps=dlswTraps, dlswConformance=dlswConformance, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswNodeVersionString=dlswNodeVersionString, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirNBName=dlswDirNBName, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlsw=dlsw, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTrapControl=dlswTrapControl, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitState=dlswCircuitState, dlswTConnStat=dlswTConnStat, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswNodeNBGroup=dlswNodeNBGroup, EndStationLocation=EndStationLocation, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirCache=dlswDirCache, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirMacMac=dlswDirMacMac, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirNBStatus=dlswDirNBStatus, dlswDomains=dlswDomains, dlswDirMacLocation=dlswDirMacLocation, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswNodeVendorID=dlswNodeVendorID, dlswDirMacMask=dlswDirMacMask, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirStat=dlswDirStat, dlswNode=dlswNode, dlswDirMacIndex=dlswDirMacIndex, TAddress=TAddress, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswIfTable=dlswIfTable, dlswDirMacEntries=dlswDirMacEntries, dlswTConn=dlswTConn, dlswDirNBIndex=dlswDirNBIndex, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswCircuit=dlswCircuit, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirMacStatus=dlswDirMacStatus, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlc=dlswSdlc, dlswInterfaceGroup=dlswInterfaceGroup, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirMacEntry=dlswDirMacEntry, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperState=dlswTConnOperState, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirNBEntry=dlswDirNBEntry, dlswTCPDomain=dlswTCPDomain, dlswSdlcGroup=dlswSdlcGroup, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswSdlcLsTable=dlswSdlcLsTable, DlswTCPAddress=DlswTCPAddress, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswCircuitGroup=dlswCircuitGroup, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswGroups=dlswGroups, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswCircuitS2Location=dlswCircuitS2Location, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswIfSapList=dlswIfSapList, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswNodeGroup=dlswNodeGroup, dlswDirLocate=dlswDirLocate, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCoreCompliance=dlswCoreCompliance, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswInterface=dlswInterface, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswCompliances=dlswCompliances, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnSpecific=dlswTConnSpecific, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitStat=dlswCircuitStat, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitPriority=dlswCircuitPriority, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirGroup=dlswDirGroup, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirMacTable=dlswDirMacTable, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirCompliance=dlswDirCompliance, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections)
