...
mibBuilder.exportSymbols("DLSW-MIB", dlswNotificationGroup=dlswNotificationGroup, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirStat=dlswDirStat, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitPriority=dlswCircuitPriority, dlswNode=dlswNode, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTCPDomain=dlswTCPDomain, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitEntry=dlswCircuitEntry, dlswCoreCompliance=dlswCoreCompliance, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirNBName=dlswDirNBName, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirNBStatus=dlswDirNBStatus, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirMacStatus=dlswDirMacStatus, dlswCircuit=dlswCircuit, dlswMIB=dlswMIB, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDomains=dlswDomains, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirMacIndex=dlswDirMacIndex, dlswNodeGroup=dlswNodeGroup, NBName=NBName, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswSdlcCompliance=dlswSdlcCompliance, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirNBIndex=dlswDirNBIndex, dlswDirMacLocation=dlswDirMacLocation, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswIfSapList=dlswIfSapList, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswGroups=dlswGroups, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConn=dlswTConn, dlswIfEntry=dlswIfEntry, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTraps=dlswTraps, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirGroup=dlswDirGroup, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswNodeStatus=dlswNodeStatus, dlswDirNBLocation=dlswDirNBLocation, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswIfTable=dlswIfTable, dlswDirLocate=dlswDirLocate, dlswCircuitGroup=dlswCircuitGroup, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirCache=dlswDirCache, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswNodeVersion=dlswNodeVersion, dlswCircuitStat=dlswCircuitStat, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswInterface=dlswInterface, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitStatActives=dlswCircuitStatActives, MacAddressNC=MacAddressNC, dlswTConnStat=dlswTConnStat, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirNBEntries=dlswDirNBEntries, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirMacLFSize=dlswDirMacLFSize, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirNBGroup=dlswDirNBGroup, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswNodeVersionString=dlswNodeVersionString, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirNBTable=dlswDirNBTable, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswIfRowStatus=dlswIfRowStatus, dlswConformance=dlswConformance, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, DlswTCPAddress=DlswTCPAddress, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnSpecific=dlswTConnSpecific, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnTcp=dlswTConnTcp, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirNBEntry=dlswDirNBEntry, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, null=null, dlswSdlcGroup=dlswSdlcGroup, EndStationLocation=EndStationLocation, dlswTConnOperConnectTime=dlswTConnOperConnectTime, TAddress=TAddress, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirMacMac=dlswDirMacMac, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTrapControl=dlswTrapControl, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirectory=dlswDirectory, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirMacMask=dlswDirMacMask, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, DlcType=DlcType, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswInterfaceGroup=dlswInterfaceGroup, PYSNMP_MODULE_ID=dlsw, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitS2Location=dlswCircuitS2Location, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitTable=dlswCircuitTable, dlswDirCompliance=dlswDirCompliance, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirMacTable=dlswDirMacTable, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperState=dlswTConnOperState, dlswSdlc=dlswSdlc, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswCircuitStateTime=dlswCircuitStateTime, LFSize=LFSize, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlsw=dlsw, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswCompliances=dlswCompliances, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitState=dlswCircuitState)
