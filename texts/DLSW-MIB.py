...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirLocate=dlswDirLocate, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitState=dlswCircuitState, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswCompliances=dlswCompliances, dlswGroups=dlswGroups, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnTcp=dlswTConnTcp, dlswDirMacLFSize=dlswDirMacLFSize, PYSNMP_MODULE_ID=dlsw, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirGroup=dlswDirGroup, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswMIB=dlswMIB, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswInterface=dlswInterface, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirNBTable=dlswDirNBTable, dlswTConnNBGroup=dlswTConnNBGroup, dlswIfSapList=dlswIfSapList, dlswDirNBIndex=dlswDirNBIndex, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswNodeUpTime=dlswNodeUpTime, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswSdlcLsTable=dlswSdlcLsTable, dlswDomains=dlswDomains, NBName=NBName, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirNBEntries=dlswDirNBEntries, dlswDirMacEntryType=dlswDirMacEntryType, dlswNodeStatus=dlswNodeStatus, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirNBLocation=dlswDirNBLocation, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirNBGroup=dlswDirNBGroup, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConn=dlswTConn, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswConformance=dlswConformance, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitStat=dlswCircuitStat, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirMacMac=dlswDirMacMac, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswIfTable=dlswIfTable, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnStatGroup=dlswTConnStatGroup, dlswTrapControl=dlswTrapControl, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirectory=dlswDirectory, dlswCircuitGroup=dlswCircuitGroup, dlswDirMacStatus=dlswDirMacStatus, dlswCoreCompliance=dlswCoreCompliance, null=null, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitS2Location=dlswCircuitS2Location, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitEntryTime=dlswCircuitEntryTime, MacAddressNC=MacAddressNC, dlswNode=dlswNode, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, EndStationLocation=EndStationLocation, dlswDirMacTable=dlswDirMacTable, dlswTConnOperState=dlswTConnOperState, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperEntry=dlswTConnOperEntry, dlswTCPDomain=dlswTCPDomain, dlswDirStat=dlswDirStat, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTrapTConnUp=dlswTrapTConnUp, dlsw=dlsw, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirNBStatus=dlswDirNBStatus, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswNodeVersion=dlswNodeVersion, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitPriority=dlswCircuitPriority, dlswDirMacEntry=dlswDirMacEntry, dlswSdlc=dlswSdlc, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirNBNameType=dlswDirNBNameType, dlswDirNBName=dlswDirNBName, dlswDirLocateGroup=dlswDirLocateGroup, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswIfEntry=dlswIfEntry, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswNodeGroup=dlswNodeGroup, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTraps=dlswTraps, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperDiscReason=dlswTConnOperDiscReason, LFSize=LFSize, dlswDirCompliance=dlswDirCompliance, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirMacEntries=dlswDirMacEntries, dlswDirMacMask=dlswDirMacMask, dlswNodeVersionString=dlswNodeVersionString, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, DlcType=DlcType, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnStat=dlswTConnStat, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuit=dlswCircuit, dlswNotificationGroup=dlswNotificationGroup, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirCache=dlswDirCache, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperNRexSents=dlswTConnOperNRexSents, TAddress=TAddress, dlswCircuitEntry=dlswCircuitEntry, dlswInterfaceGroup=dlswInterfaceGroup, DlswTCPAddress=DlswTCPAddress, dlswCircuitTable=dlswCircuitTable, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirMacIndex=dlswDirMacIndex, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnOperGroup=dlswTConnOperGroup)
