...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswIfTable=dlswIfTable, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswCircuitS1Sap=dlswCircuitS1Sap, dlsw=dlsw, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, NBName=NBName, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirMacIndex=dlswDirMacIndex, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperState=dlswTConnOperState, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitEntry=dlswCircuitEntry, dlswDirStat=dlswDirStat, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswDirMacLocation=dlswDirMacLocation, dlswSdlcLsTable=dlswSdlcLsTable, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswNodeVersionString=dlswNodeVersionString, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperCirCreates=dlswTConnOperCirCreates, null=null, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTrapControl=dlswTrapControl, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, MacAddressNC=MacAddressNC, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirNBLocationType=dlswDirNBLocationType, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, DlcType=DlcType, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuit=dlswCircuit, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirNBName=dlswDirNBName, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirMacLocationType=dlswDirMacLocationType, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswInterface=dlswInterface, dlswNodeVendorID=dlswNodeVendorID, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirectory=dlswDirectory, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswNodeVersion=dlswNodeVersion, EndStationLocation=EndStationLocation, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCoreCompliance=dlswCoreCompliance, PYSNMP_MODULE_ID=dlsw, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirMacMac=dlswDirMacMac, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirNBStatus=dlswDirNBStatus, dlswTConnStat=dlswTConnStat, dlswDirCache=dlswDirCache, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirLocateGroup=dlswDirLocateGroup, dlswSdlc=dlswSdlc, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswNode=dlswNode, dlswNodeStatus=dlswNodeStatus, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirGroup=dlswDirGroup, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswIfSapList=dlswIfSapList, LFSize=LFSize, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConn=dlswTConn, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirCompliance=dlswDirCompliance, dlswNotificationGroup=dlswNotificationGroup, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitState=dlswCircuitState, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnConfigTable=dlswTConnConfigTable, DlswTCPAddress=DlswTCPAddress, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacMask=dlswDirMacMask, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperTable=dlswTConnOperTable, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitGroup=dlswCircuitGroup, dlswCompliances=dlswCompliances, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswMIB=dlswMIB, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitPriority=dlswCircuitPriority, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnSpecific=dlswTConnSpecific, dlswDirMacTable=dlswDirMacTable, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirNBEntry=dlswDirNBEntry, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirNBIndex=dlswDirNBIndex, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, TAddress=TAddress, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirNBTable=dlswDirNBTable, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswIfEntry=dlswIfEntry, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitStat=dlswCircuitStat, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswNodeGroup=dlswNodeGroup, dlswDirNBEntries=dlswDirNBEntries, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTrapTConnDown=dlswTrapTConnDown, dlswTraps=dlswTraps, dlswDomains=dlswDomains, dlswTConnTcp=dlswTConnTcp, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswGroups=dlswGroups, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTCPDomain=dlswTCPDomain, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswNodeUpTime=dlswNodeUpTime, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirMacEntries=dlswDirMacEntries, dlswDirLocate=dlswDirLocate, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswConformance=dlswConformance, dlswDirMacLFSize=dlswDirMacLFSize, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitTable=dlswCircuitTable, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId)
