...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirNBEntry=dlswDirNBEntry, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConn=dlswTConn, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, EndStationLocation=EndStationLocation, dlswTrapCircuitDown=dlswTrapCircuitDown, TAddress=TAddress, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTraps=dlswTraps, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswNodeVersion=dlswNodeVersion, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnOperState=dlswTConnOperState, dlswTCPDomain=dlswTCPDomain, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirMacStatus=dlswDirMacStatus, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswNodeStatus=dlswNodeStatus, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirNBNameType=dlswDirNBNameType, dlswTConnSpecific=dlswTConnSpecific, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswMIB=dlswMIB, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, DlswTCPAddress=DlswTCPAddress, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCompliances=dlswCompliances, dlswNodeGroup=dlswNodeGroup, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitGroup=dlswCircuitGroup, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirLocate=dlswDirLocate, dlswSdlcCompliance=dlswSdlcCompliance, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswSdlcGroup=dlswSdlcGroup, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlsw=dlsw, dlswDomains=dlswDomains, dlswNodeVendorID=dlswNodeVendorID, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswNotificationGroup=dlswNotificationGroup, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswIfTable=dlswIfTable, dlswDirStat=dlswDirStat, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswIfEntry=dlswIfEntry, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnTcp=dlswTConnTcp, dlswNodeUpTime=dlswNodeUpTime, dlswNode=dlswNode, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirectory=dlswDirectory, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswIfSapList=dlswIfSapList, dlswDirMacLocation=dlswDirMacLocation, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, LFSize=LFSize, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirNBEntries=dlswDirNBEntries, dlswDirCache=dlswDirCache, dlswDirMacLFSize=dlswDirMacLFSize, dlswDirMacMask=dlswDirMacMask, dlswInterface=dlswInterface, dlswDirGroup=dlswDirGroup, dlswDirMacMac=dlswDirMacMac, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswCoreCompliance=dlswCoreCompliance, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnOperTable=dlswTConnOperTable, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswCircuit=dlswCircuit, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperGroup=dlswTConnOperGroup, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTrapControl=dlswTrapControl, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTrapCntlTConn=dlswTrapCntlTConn, DlcType=DlcType, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswConformance=dlswConformance, dlswDirNBIndex=dlswDirNBIndex, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswNodeVersionString=dlswNodeVersionString, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswSdlc=dlswSdlc, dlswCircuitTable=dlswCircuitTable, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirNBTable=dlswDirNBTable, PYSNMP_MODULE_ID=dlsw, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirMacIndex=dlswDirMacIndex, NBName=NBName, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirNBStatus=dlswDirNBStatus, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswIfRowStatus=dlswIfRowStatus, MacAddressNC=MacAddressNC, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirNBName=dlswDirNBName, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswDirCompliance=dlswDirCompliance, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, null=null, dlswCircuitState=dlswCircuitState, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitStat=dlswCircuitStat, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnStat=dlswTConnStat, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirMacTable=dlswDirMacTable, dlswInterfaceGroup=dlswInterfaceGroup, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswGroups=dlswGroups, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl)
