...
mibBuilder.exportSymbols("DLSW-MIB", dlswNodeVersion=dlswNodeVersion, MacAddressNC=MacAddressNC, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnConfigSetupType=dlswTConnConfigSetupType, PYSNMP_MODULE_ID=dlsw, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirNBName=dlswDirNBName, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswIfEntry=dlswIfEntry, dlswDirMacIndex=dlswDirMacIndex, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, DlswTCPAddress=DlswTCPAddress, dlswIfTable=dlswIfTable, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirCompliance=dlswDirCompliance, dlswDirStat=dlswDirStat, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirGroup=dlswDirGroup, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirMacTable=dlswDirMacTable, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnTcp=dlswTConnTcp, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswNode=dlswNode, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswInterface=dlswInterface, dlswSdlcLsTable=dlswSdlcLsTable, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirNBEntry=dlswDirNBEntry, dlswSdlcGroup=dlswSdlcGroup, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacLocation=dlswDirMacLocation, dlswDirNBGroup=dlswDirNBGroup, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirLocate=dlswDirLocate, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswNodeStatus=dlswNodeStatus, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperState=dlswTConnOperState, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswConformance=dlswConformance, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitGroup=dlswCircuitGroup, dlswNotificationGroup=dlswNotificationGroup, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnOperGroup=dlswTConnOperGroup, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTrapControl=dlswTrapControl, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirCache=dlswDirCache, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCoreCompliance=dlswCoreCompliance, dlswCircuitTable=dlswCircuitTable, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperTable=dlswTConnOperTable, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswSdlc=dlswSdlc, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, LFSize=LFSize, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuit=dlswCircuit, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswIfRowStatus=dlswIfRowStatus, dlswTConn=dlswTConn, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnConfigTable=dlswTConnConfigTable, dlsw=dlsw, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirMacMask=dlswDirMacMask, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirNBEntries=dlswDirNBEntries, EndStationLocation=EndStationLocation, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, TAddress=TAddress, dlswIfSapList=dlswIfSapList, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCircuitStat=dlswCircuitStat, dlswNodeVendorID=dlswNodeVendorID, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirMacCacheHits=dlswDirMacCacheHits, null=null, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirNBTable=dlswDirNBTable, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirMacMac=dlswDirMacMac, dlswTraps=dlswTraps, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirLocateGroup=dlswDirLocateGroup, dlswTCPDomain=dlswTCPDomain, dlswDirectory=dlswDirectory, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDomains=dlswDomains, dlswDirNBLFSize=dlswDirNBLFSize, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, DlcType=DlcType, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirMacEntry=dlswDirMacEntry, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswMIB=dlswMIB, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCompliances=dlswCompliances, dlswDirLocateMacMac=dlswDirLocateMacMac, NBName=NBName, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnStat=dlswTConnStat, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswGroups=dlswGroups, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirMacLFSize=dlswDirMacLFSize, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirNBStatus=dlswDirNBStatus, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswNodeGroup=dlswNodeGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitState=dlswCircuitState)
