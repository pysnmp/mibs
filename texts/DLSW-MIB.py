...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnStatGroup=dlswTConnStatGroup, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirNBName=dlswDirNBName, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswGroups=dlswGroups, dlswTCPDomain=dlswTCPDomain, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirLocateGroup=dlswDirLocateGroup, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTrapTConnUp=dlswTrapTConnUp, dlsw=dlsw, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitStat=dlswCircuitStat, dlswTConnStat=dlswTConnStat, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnConfigIndex=dlswTConnConfigIndex, TAddress=TAddress, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirNBLocation=dlswDirNBLocation, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirMacIndex=dlswDirMacIndex, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswNodeVendorID=dlswNodeVendorID, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswIfTable=dlswIfTable, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswInterface=dlswInterface, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuit=dlswCircuit, dlswDirNBNameType=dlswDirNBNameType, dlswDirNBGroup=dlswDirNBGroup, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirLocate=dlswDirLocate, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswMIB=dlswMIB, PYSNMP_MODULE_ID=dlsw, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswNodeUpTime=dlswNodeUpTime, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnConfigTable=dlswTConnConfigTable, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswIfEntry=dlswIfEntry, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirMacMac=dlswDirMacMac, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitState=dlswCircuitState, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswSdlcLsEntry=dlswSdlcLsEntry, null=null, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, LFSize=LFSize, dlswTraps=dlswTraps, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirCache=dlswDirCache, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnOperState=dlswTConnOperState, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirNBTable=dlswDirNBTable, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDomains=dlswDomains, EndStationLocation=EndStationLocation, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirNBEntries=dlswDirNBEntries, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirMacMask=dlswDirMacMask, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnSpecific=dlswTConnSpecific, dlswSdlc=dlswSdlc, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTrapCircuitDown=dlswTrapCircuitDown, DlswTCPAddress=DlswTCPAddress, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirNBRowStatus=dlswDirNBRowStatus, DlcType=DlcType, dlswNodeStatus=dlswNodeStatus, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswNotificationGroup=dlswNotificationGroup, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswNode=dlswNode, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirMacTable=dlswDirMacTable, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirMacEntries=dlswDirMacEntries, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirNBEntry=dlswDirNBEntry, dlswNodeVersion=dlswNodeVersion, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirGroup=dlswDirGroup, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCircuitGroup=dlswCircuitGroup, dlswIfSapList=dlswIfSapList, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirStat=dlswDirStat, dlswTConnTcp=dlswTConnTcp, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswConformance=dlswConformance, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirMacLocation=dlswDirMacLocation, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTrapControl=dlswTrapControl, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConn=dlswTConn, dlswNodeVersionString=dlswNodeVersionString, dlswDirectory=dlswDirectory, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitStatActives=dlswCircuitStatActives, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitTable=dlswCircuitTable, dlswTConnOperTable=dlswTConnOperTable, dlswCompliances=dlswCompliances, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCoreCompliance=dlswCoreCompliance, dlswTrapTConnDown=dlswTrapTConnDown, dlswNodeGroup=dlswNodeGroup, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, NBName=NBName, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, MacAddressNC=MacAddressNC)
