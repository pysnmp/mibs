...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirMacStatus=dlswDirMacStatus, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswNodeVersion=dlswNodeVersion, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, PYSNMP_MODULE_ID=dlsw, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswCompliances=dlswCompliances, dlswSdlcCompliance=dlswSdlcCompliance, dlswTCPDomain=dlswTCPDomain, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswIfSapList=dlswIfSapList, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswNodeStatus=dlswNodeStatus, dlswTConnSpecific=dlswTConnSpecific, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirNBNameType=dlswDirNBNameType, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirCache=dlswDirCache, dlswDirMacEntry=dlswDirMacEntry, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitPriority=dlswCircuitPriority, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirNBGroup=dlswDirNBGroup, dlswIfRowStatus=dlswIfRowStatus, DlcType=DlcType, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDomains=dlswDomains, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, EndStationLocation=EndStationLocation, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswCircuitStat=dlswCircuitStat, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacLFSize=dlswDirMacLFSize, dlswNodeVendorID=dlswNodeVendorID, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacIndex=dlswDirMacIndex, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocate=dlswDirLocate, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirNBTable=dlswDirNBTable, dlswNotificationGroup=dlswNotificationGroup, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirNBName=dlswDirNBName, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswNodeGroup=dlswNodeGroup, dlswDirMacMac=dlswDirMacMac, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswIfEntry=dlswIfEntry, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnNBGroup=dlswTConnNBGroup, dlswConformance=dlswConformance, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitGroup=dlswCircuitGroup, null=null, dlswGroups=dlswGroups, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, DlswTCPAddress=DlswTCPAddress, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirMacMask=dlswDirMacMask, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirStat=dlswDirStat, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswDirCompliance=dlswDirCompliance, dlswCircuitS1Mac=dlswCircuitS1Mac, dlsw=dlsw, dlswIfTable=dlswIfTable, dlswCircuit=dlswCircuit, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTrapTConnUp=dlswTrapTConnUp, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnStatGroup=dlswTConnStatGroup, dlswInterface=dlswInterface, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTraps=dlswTraps, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConn=dlswTConn, dlswDirNBLocation=dlswDirNBLocation, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCircuitTable=dlswCircuitTable, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswSdlc=dlswSdlc, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, MacAddressNC=MacAddressNC, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperTable=dlswTConnOperTable, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTrapControl=dlswTrapControl, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnTcp=dlswTConnTcp, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirectory=dlswDirectory, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirNBCacheHits=dlswDirNBCacheHits, LFSize=LFSize, dlswDirNBEntries=dlswDirNBEntries, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirMacTable=dlswDirMacTable, dlswCircuitState=dlswCircuitState, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCoreCompliance=dlswCoreCompliance, dlswDirMacEntries=dlswDirMacEntries, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperState=dlswTConnOperState, dlswNode=dlswNode, dlswTConnStat=dlswTConnStat, dlswNodeVersionString=dlswNodeVersionString, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirGroup=dlswDirGroup, NBName=NBName, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswMIB=dlswMIB, dlswSdlcLsEntries=dlswSdlcLsEntries, TAddress=TAddress, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirNBStatus=dlswDirNBStatus, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitEntry=dlswCircuitEntry)
