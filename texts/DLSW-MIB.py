...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperEntry=dlswTConnOperEntry, dlswTCPDomain=dlswTCPDomain, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswIfTable=dlswIfTable, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswCircuitStat=dlswCircuitStat, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirCache=dlswDirCache, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswGroups=dlswGroups, dlswDirNBGroup=dlswDirNBGroup, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirMacCacheHits=dlswDirMacCacheHits, DlcType=DlcType, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirMacStatus=dlswDirMacStatus, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlsw=dlsw, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitStatActives=dlswCircuitStatActives, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswSdlcCompliance=dlswSdlcCompliance, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperGroup=dlswTConnOperGroup, dlswNodeStatus=dlswNodeStatus, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitS1Mac=dlswCircuitS1Mac, NBName=NBName, dlswDirGroup=dlswDirGroup, dlswDirNBEntries=dlswDirNBEntries, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirMacTable=dlswDirMacTable, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswNotificationGroup=dlswNotificationGroup, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuit=dlswCircuit, DlswTCPAddress=DlswTCPAddress, dlswIfEntry=dlswIfEntry, dlswDirMacMask=dlswDirMacMask, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCoreCompliance=dlswCoreCompliance, dlswTConn=dlswTConn, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnStatGroup=dlswTConnStatGroup, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirNBEntry=dlswDirNBEntry, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswMIB=dlswMIB, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitEntry=dlswCircuitEntry, dlswDirNBTable=dlswDirNBTable, dlswCircuitPriority=dlswCircuitPriority, dlswConformance=dlswConformance, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswNodeVendorID=dlswNodeVendorID, dlswDirNBLocationType=dlswDirNBLocationType, TAddress=TAddress, dlswDirNBLocation=dlswDirNBLocation, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirNBEntryType=dlswDirNBEntryType, dlswInterface=dlswInterface, dlswDirMacLocation=dlswDirMacLocation, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnNBGroup=dlswTConnNBGroup, dlswSdlcLsTable=dlswSdlcLsTable, dlswNodeUpTime=dlswNodeUpTime, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswDomains=dlswDomains, null=null, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswNodeGroup=dlswNodeGroup, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswSdlc=dlswSdlc, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirNBName=dlswDirNBName, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswNode=dlswNode, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswIfSapList=dlswIfSapList, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswSdlcGroup=dlswSdlcGroup, dlswNodeVersion=dlswNodeVersion, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, MacAddressNC=MacAddressNC, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirNBStatus=dlswDirNBStatus, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTraps=dlswTraps, dlswDirStat=dlswDirStat, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, LFSize=LFSize, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnStat=dlswTConnStat, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirLocate=dlswDirLocate, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitState=dlswCircuitState, PYSNMP_MODULE_ID=dlsw, dlswCompliances=dlswCompliances, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirMacEntry=dlswDirMacEntry, dlswTConnSpecific=dlswTConnSpecific, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocateGroup=dlswDirLocateGroup, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirMacMac=dlswDirMacMac, dlswTConnTcp=dlswTConnTcp, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirectory=dlswDirectory, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirNBIndex=dlswDirNBIndex, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnOperState=dlswTConnOperState, dlswDirMacEntries=dlswDirMacEntries, dlswTrapControl=dlswTrapControl, dlswDirMacIndex=dlswDirMacIndex, EndStationLocation=EndStationLocation, dlswCircuitTable=dlswCircuitTable, dlswNodeVersionString=dlswNodeVersionString, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData)
