...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnStat=dlswTConnStat, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperState=dlswTConnOperState, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirMacEntryType=dlswDirMacEntryType, dlswNodeVersionString=dlswNodeVersionString, dlswCircuit=dlswCircuit, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, NBName=NBName, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswConformance=dlswConformance, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnConfigTDomain=dlswTConnConfigTDomain, TAddress=TAddress, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnSpecific=dlswTConnSpecific, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswNodeNBGroup=dlswNodeNBGroup, LFSize=LFSize, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirNBGroup=dlswDirNBGroup, PYSNMP_MODULE_ID=dlsw, dlswNodeGroup=dlswNodeGroup, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTraps=dlswTraps, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswGroups=dlswGroups, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirMacTable=dlswDirMacTable, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirMacIndex=dlswDirMacIndex, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTCPDomain=dlswTCPDomain, dlswDirMacMask=dlswDirMacMask, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, MacAddressNC=MacAddressNC, dlswDirMacStatus=dlswDirMacStatus, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirNBName=dlswDirNBName, dlswTConnStatGroup=dlswTConnStatGroup, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswNotificationGroup=dlswNotificationGroup, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirectory=dlswDirectory, dlswNodeVendorID=dlswNodeVendorID, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswIfEntry=dlswIfEntry, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirStat=dlswDirStat, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnNBGroup=dlswTConnNBGroup, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswMIB=dlswMIB, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswNode=dlswNode, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswIfTable=dlswIfTable, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirGroup=dlswDirGroup, dlswDirMacEntry=dlswDirMacEntry, dlswDirNBLocation=dlswDirNBLocation, dlswTConnOperTable=dlswTConnOperTable, dlswTrapTConnUp=dlswTrapTConnUp, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirCache=dlswDirCache, EndStationLocation=EndStationLocation, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitState=dlswCircuitState, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirMacEntries=dlswDirMacEntries, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswNodeStatus=dlswNodeStatus, dlswSdlc=dlswSdlc, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitTable=dlswCircuitTable, dlswDirNBIndex=dlswDirNBIndex, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, null=null, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitPriority=dlswCircuitPriority, dlswCoreCompliance=dlswCoreCompliance, dlswDirLocate=dlswDirLocate, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacLocationType=dlswDirMacLocationType, dlswInterfaceGroup=dlswInterfaceGroup, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTrapControl=dlswTrapControl, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswInterface=dlswInterface, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitOrigin=dlswCircuitOrigin, dlswNodeVersion=dlswNodeVersion, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirMacMac=dlswDirMacMac, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitStat=dlswCircuitStat, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, DlcType=DlcType, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConn=dlswTConn, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnTcp=dlswTConnTcp, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDomains=dlswDomains, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswCompliances=dlswCompliances, dlswIfSapList=dlswIfSapList, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDirCompliance=dlswDirCompliance, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitGroup=dlswCircuitGroup, dlsw=dlsw, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirNBEntries=dlswDirNBEntries, dlswDirMacLocation=dlswDirMacLocation, dlswDirNBTable=dlswDirNBTable, DlswTCPAddress=DlswTCPAddress, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperICRexSents=dlswTConnOperICRexSents)
