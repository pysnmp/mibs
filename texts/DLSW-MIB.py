...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirNBName=dlswDirNBName, dlswDirMacStatus=dlswDirMacStatus, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, NBName=NBName, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTCPDomain=dlswTCPDomain, dlswSdlc=dlswSdlc, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirMacTable=dlswDirMacTable, dlswMIB=dlswMIB, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocate=dlswDirLocate, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirNBEntry=dlswDirNBEntry, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswNodeGroup=dlswNodeGroup, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, LFSize=LFSize, dlswCircuitGroup=dlswCircuitGroup, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTrapControl=dlswTrapControl, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlsw=dlsw, dlswDirMacEntries=dlswDirMacEntries, dlswGroups=dlswGroups, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswNodeVersion=dlswNodeVersion, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitEntry=dlswCircuitEntry, dlswIfEntry=dlswIfEntry, DlswTCPAddress=DlswTCPAddress, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBTable=dlswDirNBTable, dlswIfTable=dlswIfTable, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirCache=dlswDirCache, dlswTraps=dlswTraps, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswInterface=dlswInterface, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirMacIndex=dlswDirMacIndex, dlswDirNBIndex=dlswDirNBIndex, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirMacMask=dlswDirMacMask, dlswDirMacMac=dlswDirMacMac, dlswDirMacLocation=dlswDirMacLocation, DlcType=DlcType, dlswDirNBEntryType=dlswDirNBEntryType, PYSNMP_MODULE_ID=dlsw, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirGroup=dlswDirGroup, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitTable=dlswCircuitTable, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCompliances=dlswCompliances, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswNode=dlswNode, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitStat=dlswCircuitStat, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, TAddress=TAddress, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirMacLFSize=dlswDirMacLFSize, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnTcp=dlswTConnTcp, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirectory=dlswDirectory, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswNodeVersionString=dlswNodeVersionString, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirNBStatus=dlswDirNBStatus, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperState=dlswTConnOperState, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirNBCacheMisses=dlswDirNBCacheMisses, null=null, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirStat=dlswDirStat, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnStat=dlswTConnStat, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTrapTConnUp=dlswTrapTConnUp, MacAddressNC=MacAddressNC, dlswTConnConfigTable=dlswTConnConfigTable, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuit=dlswCircuit, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswNodeStatus=dlswNodeStatus, dlswIfSapList=dlswIfSapList, dlswDirNBEntries=dlswDirNBEntries, dlswDirNBLocation=dlswDirNBLocation, dlswSdlcCompliance=dlswSdlcCompliance, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitState=dlswCircuitState, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswIfRowStatus=dlswIfRowStatus, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDomains=dlswDomains, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConn=dlswTConn, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswConformance=dlswConformance, EndStationLocation=EndStationLocation, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperCURexSents=dlswTConnOperCURexSents)
