...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitTable=dlswCircuitTable, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswSdlcGroup=dlswSdlcGroup, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirectory=dlswDirectory, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, DlcType=DlcType, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirLocate=dlswDirLocate, DlswTCPAddress=DlswTCPAddress, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConn=dlswTConn, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuit=dlswCircuit, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacLocation=dlswDirMacLocation, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, null=null, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitStat=dlswCircuitStat, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitPriority=dlswCircuitPriority, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswIfTable=dlswIfTable, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswNodeStatus=dlswNodeStatus, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCoreCompliance=dlswCoreCompliance, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswNode=dlswNode, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswGroups=dlswGroups, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirMacIndex=dlswDirMacIndex, dlswTCPDomain=dlswTCPDomain, dlswIfEntry=dlswIfEntry, dlswMIB=dlswMIB, MacAddressNC=MacAddressNC, LFSize=LFSize, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTrapControl=dlswTrapControl, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitS2Mac=dlswCircuitS2Mac, EndStationLocation=EndStationLocation, dlswInterface=dlswInterface, dlswTConnStat=dlswTConnStat, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswDirNBTable=dlswDirNBTable, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnTcp=dlswTConnTcp, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirCompliance=dlswDirCompliance, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirMacTable=dlswDirMacTable, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirNBGroup=dlswDirNBGroup, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirMacEntry=dlswDirMacEntry, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitStatActives=dlswCircuitStatActives, dlswDirMacStatus=dlswDirMacStatus, dlswConformance=dlswConformance, dlswDirNBLocationType=dlswDirNBLocationType, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTraps=dlswTraps, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirNBIndex=dlswDirNBIndex, dlswDirMacEntries=dlswDirMacEntries, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirCache=dlswDirCache, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirMacMask=dlswDirMacMask, dlswNotificationGroup=dlswNotificationGroup, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswIfSapList=dlswIfSapList, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperState=dlswTConnOperState, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTrapTConnUp=dlswTrapTConnUp, dlswCircuitState=dlswCircuitState, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswNodeGroup=dlswNodeGroup, dlswCircuitS2Location=dlswCircuitS2Location, dlswSdlcLsTable=dlswSdlcLsTable, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirStat=dlswDirStat, NBName=NBName, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirMacMac=dlswDirMacMac, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTrapTConnDown=dlswTrapTConnDown, dlswCompliances=dlswCompliances, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, TAddress=TAddress, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitGroup=dlswCircuitGroup, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, PYSNMP_MODULE_ID=dlsw, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCircuitOrigin=dlswCircuitOrigin, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirNBName=dlswDirNBName, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDomains=dlswDomains, dlswSdlc=dlswSdlc, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirNBLocation=dlswDirNBLocation, dlswDirGroup=dlswDirGroup, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswNodeVersionString=dlswNodeVersionString, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirNBStatus=dlswDirNBStatus, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswIfRowStatus=dlswIfRowStatus, dlsw=dlsw, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData)
