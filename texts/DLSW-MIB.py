...
mibBuilder.exportSymbols("DLSW-MIB", PYSNMP_MODULE_ID=dlsw, DlswTCPAddress=DlswTCPAddress, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitState=dlswCircuitState, dlswDomains=dlswDomains, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirCache=dlswDirCache, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswIfRowStatus=dlswIfRowStatus, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswInterface=dlswInterface, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirMacLocationType=dlswDirMacLocationType, dlswCompliances=dlswCompliances, null=null, dlswDirCompliance=dlswDirCompliance, dlswDirNBEntryType=dlswDirNBEntryType, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirStat=dlswDirStat, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirMacLFSize=dlswDirMacLFSize, dlswGroups=dlswGroups, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswDirMacTable=dlswDirMacTable, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirectory=dlswDirectory, dlswCircuitS2Location=dlswCircuitS2Location, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswNodeStatus=dlswNodeStatus, dlswNotificationGroup=dlswNotificationGroup, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirNBName=dlswDirNBName, dlswCoreCompliance=dlswCoreCompliance, dlswNodeGroup=dlswNodeGroup, dlswDirGroup=dlswDirGroup, MacAddressNC=MacAddressNC, dlswDirMacEntries=dlswDirMacEntries, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, EndStationLocation=EndStationLocation, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, TAddress=TAddress, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTrapTConnDown=dlswTrapTConnDown, dlsw=dlsw, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirNBLocation=dlswDirNBLocation, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswNodeVersion=dlswNodeVersion, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswSdlcGroup=dlswSdlcGroup, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirMacMask=dlswDirMacMask, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperState=dlswTConnOperState, dlswDirNBEntry=dlswDirNBEntry, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConn=dlswTConn, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTrapControl=dlswTrapControl, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirMacStatus=dlswDirMacStatus, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitGroup=dlswCircuitGroup, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTraps=dlswTraps, dlswNodeVersionString=dlswNodeVersionString, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirNBTable=dlswDirNBTable, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirLocateGroup=dlswDirLocateGroup, dlswCircuit=dlswCircuit, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirMacLocation=dlswDirMacLocation, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswIfEntry=dlswIfEntry, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitStat=dlswCircuitStat, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswSdlcLsTable=dlswSdlcLsTable, dlswDirMacEntryType=dlswDirMacEntryType, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, NBName=NBName, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperEntry=dlswTConnOperEntry, LFSize=LFSize, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirMacEntry=dlswDirMacEntry, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirLocate=dlswDirLocate, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswIfTable=dlswIfTable, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnTcp=dlswTConnTcp, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirNBGroup=dlswDirNBGroup, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnSpecific=dlswTConnSpecific, dlswTCPDomain=dlswTCPDomain, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswSdlc=dlswSdlc, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirMacMac=dlswDirMacMac, DlcType=DlcType, dlswDirNBStatus=dlswDirNBStatus, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswCircuitTable=dlswCircuitTable, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswMIB=dlswMIB, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnStat=dlswTConnStat, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnStatGroup=dlswTConnStatGroup, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswNodeNBGroup=dlswNodeNBGroup, dlswNode=dlswNode, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswIfSapList=dlswIfSapList, dlswConformance=dlswConformance, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirMacIndex=dlswDirMacIndex, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds)
