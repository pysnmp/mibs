...
mibBuilder.exportSymbols("DLSW-MIB", dlswNodeVersionString=dlswNodeVersionString, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirMacMask=dlswDirMacMask, dlswCircuitStat=dlswCircuitStat, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnStat=dlswTConnStat, dlswDirectory=dlswDirectory, dlswTConnOperState=dlswTConnOperState, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswCircuitState=dlswCircuitState, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirCache=dlswDirCache, dlswDirStat=dlswDirStat, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, EndStationLocation=EndStationLocation, dlswDirNBIndex=dlswDirNBIndex, dlswSdlcGroup=dlswSdlcGroup, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitGroup=dlswCircuitGroup, TAddress=TAddress, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirMacEntries=dlswDirMacEntries, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirNBNameType=dlswDirNBNameType, dlswNodeGroup=dlswNodeGroup, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTCPDomain=dlswTCPDomain, dlswDirLocate=dlswDirLocate, dlswTConnSpecific=dlswTConnSpecific, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswNodeUpTime=dlswNodeUpTime, dlswNotificationGroup=dlswNotificationGroup, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirMacLocation=dlswDirMacLocation, dlswTrapControl=dlswTrapControl, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswNodeVersion=dlswNodeVersion, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlsw=dlsw, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnConfigTable=dlswTConnConfigTable, NBName=NBName, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitTable=dlswCircuitTable, dlswTrapCntlTConn=dlswTrapCntlTConn, null=null, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperGroup=dlswTConnOperGroup, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirMacEntry=dlswDirMacEntry, dlswConformance=dlswConformance, dlswTraps=dlswTraps, dlswDirCompliance=dlswDirCompliance, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswMIB=dlswMIB, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, PYSNMP_MODULE_ID=dlsw, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, LFSize=LFSize, dlswDomains=dlswDomains, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, MacAddressNC=MacAddressNC, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuit=dlswCircuit, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirNBStatus=dlswDirNBStatus, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswIfTable=dlswIfTable, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnOperCURexSents=dlswTConnOperCURexSents, DlcType=DlcType, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitEntry=dlswCircuitEntry, dlswDirGroup=dlswDirGroup, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirNBEntries=dlswDirNBEntries, dlswDirMacTable=dlswDirMacTable, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitStatActives=dlswCircuitStatActives, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirMacMac=dlswDirMacMac, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnTcp=dlswTConnTcp, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswNodeStatus=dlswNodeStatus, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswGroups=dlswGroups, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirNBLocation=dlswDirNBLocation, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperTable=dlswTConnOperTable, dlswDirNBTable=dlswDirNBTable, dlswIfSapList=dlswIfSapList, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirNBName=dlswDirNBName, DlswTCPAddress=DlswTCPAddress, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConn=dlswTConn, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirMacIndex=dlswDirMacIndex, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswInterface=dlswInterface, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitS2Location=dlswCircuitS2Location, dlswSdlc=dlswSdlc, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitOrigin=dlswCircuitOrigin, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirLocateGroup=dlswDirLocateGroup, dlswCompliances=dlswCompliances, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswIfEntry=dlswIfEntry, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCoreCompliance=dlswCoreCompliance, dlswNode=dlswNode)
