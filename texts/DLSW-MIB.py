...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnTcp=dlswTConnTcp, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirLocate=dlswDirLocate, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTrapTConnUp=dlswTrapTConnUp, MacAddressNC=MacAddressNC, dlswTConn=dlswTConn, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirNBName=dlswDirNBName, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswIfTable=dlswIfTable, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitStatGroup=dlswCircuitStatGroup, LFSize=LFSize, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTraps=dlswTraps, dlswDirMacEntryType=dlswDirMacEntryType, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, TAddress=TAddress, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTrapTConnDown=dlswTrapTConnDown, dlswNodeUpTime=dlswNodeUpTime, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirNBIndex=dlswDirNBIndex, dlswInterface=dlswInterface, dlswDirMacIndex=dlswDirMacIndex, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCompliances=dlswCompliances, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirNBNameType=dlswDirNBNameType, dlswDirNBEntries=dlswDirNBEntries, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirMacMask=dlswDirMacMask, dlswDirMacMac=dlswDirMacMac, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirNBTable=dlswDirNBTable, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnSpecific=dlswTConnSpecific, dlswDirNBLFSize=dlswDirNBLFSize, dlswIfSapList=dlswIfSapList, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswNodeGroup=dlswNodeGroup, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswConformance=dlswConformance, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswGroups=dlswGroups, dlswCircuitStatActives=dlswCircuitStatActives, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitState=dlswCircuitState, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirMacStatus=dlswDirMacStatus, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswCoreCompliance=dlswCoreCompliance, dlswNode=dlswNode, DlcType=DlcType, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirCompliance=dlswDirCompliance, dlswDirMacCacheHits=dlswDirMacCacheHits, NBName=NBName, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperTable=dlswTConnOperTable, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirMacEntry=dlswDirMacEntry, dlswTrapControl=dlswTrapControl, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswNotificationGroup=dlswNotificationGroup, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTCPDomain=dlswTCPDomain, dlswSdlc=dlswSdlc, dlswDirMacLocation=dlswDirMacLocation, dlswDirectory=dlswDirectory, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCircuitGroup=dlswCircuitGroup, dlsw=dlsw, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswMIB=dlswMIB, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswNodeVendorID=dlswNodeVendorID, dlswDirNBLocationType=dlswDirNBLocationType, dlswInterfaceGroup=dlswInterfaceGroup, dlswCircuitStat=dlswCircuitStat, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirMacLFSize=dlswDirMacLFSize, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuit=dlswCircuit, dlswDirNBStatus=dlswDirNBStatus, dlswTConnTcpOperTable=dlswTConnTcpOperTable, null=null, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitOrigin=dlswCircuitOrigin, PYSNMP_MODULE_ID=dlsw, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitTable=dlswCircuitTable, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDomains=dlswDomains, dlswTConnOperState=dlswTConnOperState, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswNodeVersionString=dlswNodeVersionString, dlswCircuitS2Location=dlswCircuitS2Location, EndStationLocation=EndStationLocation, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirCache=dlswDirCache, dlswDirMacTable=dlswDirMacTable, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirMacLocationType=dlswDirMacLocationType, dlswIfEntry=dlswIfEntry, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswNodeStatus=dlswNodeStatus, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitPriority=dlswCircuitPriority, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirStat=dlswDirStat, dlswTConnConfigTable=dlswTConnConfigTable, dlswNodeVersion=dlswNodeVersion, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnStat=dlswTConnStat, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirLocateNBName=dlswDirLocateNBName, DlswTCPAddress=DlswTCPAddress, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirGroup=dlswDirGroup, dlswSdlcGroup=dlswSdlcGroup, dlswDirNBLocation=dlswDirNBLocation)
