...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, DlswTCPAddress=DlswTCPAddress, EndStationLocation=EndStationLocation, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirNBEntry=dlswDirNBEntry, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirectory=dlswDirectory, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirCache=dlswDirCache, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitS2Sap=dlswCircuitS2Sap, NBName=NBName, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitTable=dlswCircuitTable, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirMacTable=dlswDirMacTable, dlswTConnOperState=dlswTConnOperState, dlswDirMacEntries=dlswDirMacEntries, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, PYSNMP_MODULE_ID=dlsw, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirNBCacheMisses=dlswDirNBCacheMisses, TAddress=TAddress, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitGroup=dlswCircuitGroup, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuit=dlswCircuit, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirMacIndex=dlswDirMacIndex, dlswIfEntry=dlswIfEntry, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswCoreCompliance=dlswCoreCompliance, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswInterface=dlswInterface, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTCPDomain=dlswTCPDomain, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnTcpOperTable=dlswTConnTcpOperTable, LFSize=LFSize, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswNodeVersion=dlswNodeVersion, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirMacMac=dlswDirMacMac, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, DlcType=DlcType, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirNBName=dlswDirNBName, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswIfRowStatus=dlswIfRowStatus, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirNBLocation=dlswDirNBLocation, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitEntry=dlswCircuitEntry, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirNBNameType=dlswDirNBNameType, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswNotificationGroup=dlswNotificationGroup, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperTable=dlswTConnOperTable, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswNode=dlswNode, dlswIfSapList=dlswIfSapList, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswSdlcLsTable=dlswSdlcLsTable, dlswConformance=dlswConformance, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswSdlcGroup=dlswSdlcGroup, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswNodeStatus=dlswNodeStatus, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConn=dlswTConn, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirGroup=dlswDirGroup, dlswNodeGroup=dlswNodeGroup, dlswDirNBTable=dlswDirNBTable, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswMIB=dlswMIB, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswNodeVendorID=dlswNodeVendorID, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnSpecific=dlswTConnSpecific, MacAddressNC=MacAddressNC, dlswCircuitPriority=dlswCircuitPriority, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirMacLFSize=dlswDirMacLFSize, dlswSdlcCompliance=dlswSdlcCompliance, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacLocation=dlswDirMacLocation, dlswDirMacMask=dlswDirMacMask, dlswDirLocate=dlswDirLocate, dlswTrapControl=dlswTrapControl, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnStatGroup=dlswTConnStatGroup, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirMacLocationType=dlswDirMacLocationType, dlswInterfaceGroup=dlswInterfaceGroup, dlswCircuitState=dlswCircuitState, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswGroups=dlswGroups, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCompliances=dlswCompliances, dlswNodeVersionString=dlswNodeVersionString, dlswDomains=dlswDomains, dlswTConnOperGroup=dlswTConnOperGroup, null=null, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTraps=dlswTraps, dlswSdlc=dlswSdlc, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnTcp=dlswTConnTcp, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirNBLocationType=dlswDirNBLocationType, dlsw=dlsw, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnStat=dlswTConnStat, dlswCircuitStat=dlswCircuitStat, dlswDirStat=dlswDirStat, dlswIfTable=dlswIfTable, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity)
