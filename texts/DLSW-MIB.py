...
mibBuilder.exportSymbols("DLSW-MIB", dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, TAddress=TAddress, dlswDirGroup=dlswDirGroup, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswIfSapList=dlswIfSapList, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConn=dlswTConn, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBLocationType=dlswDirNBLocationType, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirLocate=dlswDirLocate, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, PYSNMP_MODULE_ID=dlsw, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirMacMac=dlswDirMacMac, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswNode=dlswNode, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirMacEntries=dlswDirMacEntries, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuitPriority=dlswCircuitPriority, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswNodeVersionString=dlswNodeVersionString, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, NBName=NBName, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswDirNBIndex=dlswDirNBIndex, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, EndStationLocation=EndStationLocation, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTCPDomain=dlswTCPDomain, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperState=dlswTConnOperState, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirCompliance=dlswDirCompliance, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswSdlcLsTable=dlswSdlcLsTable, dlswMIB=dlswMIB, dlswTConnOperEntry=dlswTConnOperEntry, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswNodeStatus=dlswNodeStatus, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitStat=dlswCircuitStat, dlswCompliances=dlswCompliances, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswIfRowStatus=dlswIfRowStatus, dlswDirMacLFSize=dlswDirMacLFSize, dlswNodeGroup=dlswNodeGroup, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperGroup=dlswTConnOperGroup, dlswDomains=dlswDomains, DlcType=DlcType, dlswDirNBEntries=dlswDirNBEntries, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnNBGroup=dlswTConnNBGroup, LFSize=LFSize, dlswNodeVendorID=dlswNodeVendorID, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirNBNameType=dlswDirNBNameType, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswConformance=dlswConformance, dlswDirMacMask=dlswDirMacMask, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirNBName=dlswDirNBName, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, null=null, dlswTConnSpecific=dlswTConnSpecific, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTrapControl=dlswTrapControl, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswDirLocateGroup=dlswDirLocateGroup, dlsw=dlsw, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswCircuitOrigin=dlswCircuitOrigin, dlswNotificationGroup=dlswNotificationGroup, MacAddressNC=MacAddressNC, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitTable=dlswCircuitTable, dlswDirMacStatus=dlswDirMacStatus, dlswDirNBStatus=dlswDirNBStatus, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTrapTConnUp=dlswTrapTConnUp, dlswInterface=dlswInterface, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitGroup=dlswCircuitGroup, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirNBTable=dlswDirNBTable, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswIfEntry=dlswIfEntry, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirectory=dlswDirectory, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswSdlcGroup=dlswSdlcGroup, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirNBEntry=dlswDirNBEntry, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, DlswTCPAddress=DlswTCPAddress, dlswDirMacIndex=dlswDirMacIndex, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTraps=dlswTraps, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitEntry=dlswCircuitEntry, dlswSdlc=dlswSdlc, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirStat=dlswDirStat, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnTcp=dlswTConnTcp, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuit=dlswCircuit, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswCircuitState=dlswCircuitState, dlswTConnOperTable=dlswTConnOperTable, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswIfTable=dlswIfTable, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirMacTable=dlswDirMacTable, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirCache=dlswDirCache, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswGroups=dlswGroups, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirNBEntryType=dlswDirNBEntryType, dlswNodeVersion=dlswNodeVersion, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnStat=dlswTConnStat, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswNodeUpTime=dlswNodeUpTime, dlswTConnConfigTable=dlswTConnConfigTable)
