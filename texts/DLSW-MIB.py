...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitPriority=dlswCircuitPriority, dlswCompliances=dlswCompliances, dlswNodeVersionString=dlswNodeVersionString, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswConformance=dlswConformance, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirLocate=dlswDirLocate, dlswDirMacMask=dlswDirMacMask, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirNBStatus=dlswDirNBStatus, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswNodeStatus=dlswNodeStatus, TAddress=TAddress, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, MacAddressNC=MacAddressNC, dlswDirNBNameType=dlswDirNBNameType, dlswNodeUpTime=dlswNodeUpTime, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswInterface=dlswInterface, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswNodeGroup=dlswNodeGroup, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirMacEntry=dlswDirMacEntry, dlswTConnConfigTable=dlswTConnConfigTable, dlswDirNBIndex=dlswDirNBIndex, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswMIB=dlswMIB, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirNBEntry=dlswDirNBEntry, dlswDirMacLocationType=dlswDirMacLocationType, dlswSdlcLsTable=dlswSdlcLsTable, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirNBTable=dlswDirNBTable, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnNBGroup=dlswTConnNBGroup, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnOperState=dlswTConnOperState, dlswTConnOperTable=dlswTConnOperTable, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitStateTime=dlswCircuitStateTime, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirMacTable=dlswDirMacTable, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirNBName=dlswDirNBName, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirMacLocation=dlswDirMacLocation, dlswDirMacMac=dlswDirMacMac, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirCache=dlswDirCache, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTrapControl=dlswTrapControl, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, PYSNMP_MODULE_ID=dlsw, dlswTraps=dlswTraps, dlswCircuitGroup=dlswCircuitGroup, dlswCircuitOrigin=dlswCircuitOrigin, dlswGroups=dlswGroups, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswSdlc=dlswSdlc, dlswCoreCompliance=dlswCoreCompliance, dlswIfTable=dlswIfTable, dlswIfEntry=dlswIfEntry, dlswIfRowStatus=dlswIfRowStatus, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnSpecific=dlswTConnSpecific, dlswCircuit=dlswCircuit, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnOperGroup=dlswTConnOperGroup, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirectory=dlswDirectory, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitS2Location=dlswCircuitS2Location, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, NBName=NBName, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTCPDomain=dlswTCPDomain, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, LFSize=LFSize, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirStat=dlswDirStat, dlsw=dlsw, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnTcp=dlswTConnTcp, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswNodeVersion=dlswNodeVersion, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirNBEntries=dlswDirNBEntries, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswIfSapList=dlswIfSapList, dlswDirLocateMacEntry=dlswDirLocateMacEntry, EndStationLocation=EndStationLocation, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirNBEntryType=dlswDirNBEntryType, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitState=dlswCircuitState, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConn=dlswTConn, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitStat=dlswCircuitStat, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitTable=dlswCircuitTable, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitS1Mac=dlswCircuitS1Mac, DlcType=DlcType, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDomains=dlswDomains, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, DlswTCPAddress=DlswTCPAddress, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswSdlcCompliance=dlswSdlcCompliance, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirMacIndex=dlswDirMacIndex, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirMacCacheHits=dlswDirMacCacheHits, null=null, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnStat=dlswTConnStat, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirGroup=dlswDirGroup, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswSdlcGroup=dlswSdlcGroup, dlswDirMacEntries=dlswDirMacEntries, dlswNode=dlswNode, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB)
