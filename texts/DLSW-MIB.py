...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirCompliance=dlswDirCompliance, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitGroup=dlswCircuitGroup, dlswTConn=dlswTConn, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperConnectTime=dlswTConnOperConnectTime, DlswTCPAddress=DlswTCPAddress, dlswDirNBEntry=dlswDirNBEntry, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, PYSNMP_MODULE_ID=dlsw, dlswTConnConfigTable=dlswTConnConfigTable, dlswNodeVersionString=dlswNodeVersionString, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirCache=dlswDirCache, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitPriority=dlswCircuitPriority, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBName=dlswDirNBName, dlswNodeGroup=dlswNodeGroup, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirMacEntries=dlswDirMacEntries, LFSize=LFSize, dlswCompliances=dlswCompliances, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTCPDomain=dlswTCPDomain, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirNBNameType=dlswDirNBNameType, dlswTrapTConnUp=dlswTrapTConnUp, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlsw=dlsw, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswCircuitTable=dlswCircuitTable, dlswDirMacStatus=dlswDirMacStatus, DlcType=DlcType, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperICRexSents=dlswTConnOperICRexSents, TAddress=TAddress, dlswNodeStatus=dlswNodeStatus, dlswIfRowStatus=dlswIfRowStatus, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperEntry=dlswTConnOperEntry, dlswGroups=dlswGroups, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirLocate=dlswDirLocate, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitStatActives=dlswCircuitStatActives, dlswTrapControl=dlswTrapControl, dlswDirMacMask=dlswDirMacMask, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitStat=dlswCircuitStat, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswNodeUpTime=dlswNodeUpTime, dlswDomains=dlswDomains, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirMacLocationType=dlswDirMacLocationType, dlswNode=dlswNode, dlswDirNBEntries=dlswDirNBEntries, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswIfTable=dlswIfTable, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitStateTime=dlswCircuitStateTime, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTraps=dlswTraps, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirStat=dlswDirStat, dlswTConnStat=dlswTConnStat, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirNBStatus=dlswDirNBStatus, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirMacLFSize=dlswDirMacLFSize, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswCircuit=dlswCircuit, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, null=null, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswInterface=dlswInterface, dlswDirNBTable=dlswDirNBTable, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacMac=dlswDirMacMac, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswSdlcGroup=dlswSdlcGroup, dlswNodeVersion=dlswNodeVersion, dlswCircuitEntry=dlswCircuitEntry, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswConformance=dlswConformance, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswIfEntry=dlswIfEntry, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, MacAddressNC=MacAddressNC, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, EndStationLocation=EndStationLocation, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswMIB=dlswMIB, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswNodeNBGroup=dlswNodeNBGroup, dlswCoreCompliance=dlswCoreCompliance, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirectory=dlswDirectory, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswSdlc=dlswSdlc, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirGroup=dlswDirGroup, dlswCircuitState=dlswCircuitState, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, NBName=NBName, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnTcp=dlswTConnTcp, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperState=dlswTConnOperState, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirMacIndex=dlswDirMacIndex, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirMacLocation=dlswDirMacLocation, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirMacTable=dlswDirMacTable, dlswIfSapList=dlswIfSapList, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTrapCircuitUp=dlswTrapCircuitUp)
