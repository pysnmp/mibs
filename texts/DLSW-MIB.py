...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirMacTable=dlswDirMacTable, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswNodeVendorID=dlswNodeVendorID, dlswNodeStatus=dlswNodeStatus, dlswTConnOperState=dlswTConnOperState, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuit=dlswCircuit, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirNBName=dlswDirNBName, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswInterface=dlswInterface, dlswDirNBTable=dlswDirNBTable, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirMacIndex=dlswDirMacIndex, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswIfRowStatus=dlswIfRowStatus, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswSdlcGroup=dlswSdlcGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, DlswTCPAddress=DlswTCPAddress, dlswTrapTConnDown=dlswTrapTConnDown, dlsw=dlsw, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswSdlcCompliance=dlswSdlcCompliance, dlswDomains=dlswDomains, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirNBLocation=dlswDirNBLocation, null=null, dlswDirNBIndex=dlswDirNBIndex, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswNotificationGroup=dlswNotificationGroup, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirLocate=dlswDirLocate, dlswNodeUpTime=dlswNodeUpTime, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnStat=dlswTConnStat, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTrapTConnUp=dlswTrapTConnUp, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitStatCreates=dlswCircuitStatCreates, MacAddressNC=MacAddressNC, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCircuitState=dlswCircuitState, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirStat=dlswDirStat, dlswDirMacMac=dlswDirMacMac, dlswDirMacLocation=dlswDirMacLocation, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswMIB=dlswMIB, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirNBNameType=dlswDirNBNameType, dlswDirGroup=dlswDirGroup, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswIfSapList=dlswIfSapList, dlswIfEntry=dlswIfEntry, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirectory=dlswDirectory, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTrapControl=dlswTrapControl, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacStatus=dlswDirMacStatus, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConn=dlswTConn, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCompliances=dlswCompliances, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswInterfaceGroup=dlswInterfaceGroup, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, PYSNMP_MODULE_ID=dlsw, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCoreCompliance=dlswCoreCompliance, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, DlcType=DlcType, dlswDirCache=dlswDirCache, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirCompliance=dlswDirCompliance, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTraps=dlswTraps, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswConformance=dlswConformance, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitStat=dlswCircuitStat, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, NBName=NBName, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitTable=dlswCircuitTable, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnSpecific=dlswTConnSpecific, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirMacMask=dlswDirMacMask, dlswIfTable=dlswIfTable, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirNBGroup=dlswDirNBGroup, EndStationLocation=EndStationLocation, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswSdlc=dlswSdlc, dlswGroups=dlswGroups, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswDirMacEntries=dlswDirMacEntries, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, LFSize=LFSize, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswNode=dlswNode, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirNBEntry=dlswDirNBEntry, dlswTConnTcp=dlswTConnTcp, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTCPDomain=dlswTCPDomain, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, TAddress=TAddress, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirNBLocationType=dlswDirNBLocationType, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperGroup=dlswTConnOperGroup, dlswCircuitGroup=dlswCircuitGroup, dlswDirMacEntry=dlswDirMacEntry, dlswNodeGroup=dlswNodeGroup, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitPriority=dlswCircuitPriority)
