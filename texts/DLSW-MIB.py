...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCoreCompliance=dlswCoreCompliance, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswIfRowStatus=dlswIfRowStatus, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswGroups=dlswGroups, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitStatActives=dlswCircuitStatActives, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirNBLFSize=dlswDirNBLFSize, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDomains=dlswDomains, dlswDirNBGroup=dlswDirNBGroup, dlswSdlc=dlswSdlc, dlswDirNBEntry=dlswDirNBEntry, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirNBTable=dlswDirNBTable, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacEntries=dlswDirMacEntries, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuit=dlswCircuit, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirCompliance=dlswDirCompliance, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswIfTable=dlswIfTable, dlswNodeGroup=dlswNodeGroup, dlswTraps=dlswTraps, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswNodeStatus=dlswNodeStatus, dlswDirMacEntryType=dlswDirMacEntryType, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperEntry=dlswTConnOperEntry, dlswNodeVersion=dlswNodeVersion, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswNodeVendorID=dlswNodeVendorID, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswSdlcLsEntries=dlswSdlcLsEntries, PYSNMP_MODULE_ID=dlsw, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, DlcType=DlcType, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswNodeUpTime=dlswNodeUpTime, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, null=null, dlswTConn=dlswTConn, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswInterfaceGroup=dlswInterfaceGroup, dlsw=dlsw, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitState=dlswCircuitState, EndStationLocation=EndStationLocation, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperState=dlswTConnOperState, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTCPDomain=dlswTCPDomain, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirMacMac=dlswDirMacMac, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitStat=dlswCircuitStat, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirMacTable=dlswDirMacTable, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswMIB=dlswMIB, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirStat=dlswDirStat, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswConformance=dlswConformance, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswNodeVersionString=dlswNodeVersionString, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, NBName=NBName, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCompliances=dlswCompliances, dlswDirNBNameType=dlswDirNBNameType, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitTable=dlswCircuitTable, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperTable=dlswTConnOperTable, dlswTrapControl=dlswTrapControl, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnTcp=dlswTConnTcp, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitGroup=dlswCircuitGroup, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnNBGroup=dlswTConnNBGroup, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnSpecific=dlswTConnSpecific, dlswDirNBLocationType=dlswDirNBLocationType, MacAddressNC=MacAddressNC, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirCache=dlswDirCache, dlswCircuitStateTime=dlswCircuitStateTime, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswIfEntry=dlswIfEntry, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnStat=dlswTConnStat, dlswNode=dlswNode, TAddress=TAddress, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTrapTConnUp=dlswTrapTConnUp, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirMacMask=dlswDirMacMask, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirMacLocation=dlswDirMacLocation, dlswDirMacIndex=dlswDirMacIndex, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirNBName=dlswDirNBName, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswNotificationGroup=dlswNotificationGroup, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirLocate=dlswDirLocate, dlswDirectory=dlswDirectory, dlswIfSapList=dlswIfSapList, DlswTCPAddress=DlswTCPAddress, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswSdlcCompliance=dlswSdlcCompliance, LFSize=LFSize, dlswInterface=dlswInterface, dlswDirGroup=dlswDirGroup, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitS1Sap=dlswCircuitS1Sap)
