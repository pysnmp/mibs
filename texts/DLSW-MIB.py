...
mibBuilder.exportSymbols("DLSW-MIB", dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, LFSize=LFSize, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirStat=dlswDirStat, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperGroup=dlswTConnOperGroup, dlswConformance=dlswConformance, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirMacEntries=dlswDirMacEntries, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitTable=dlswCircuitTable, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnOperState=dlswTConnOperState, dlswDirNBLocation=dlswDirNBLocation, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswGroups=dlswGroups, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswIfEntry=dlswIfEntry, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswCircuitGroup=dlswCircuitGroup, dlswTConnTcp=dlswTConnTcp, DlswTCPAddress=DlswTCPAddress, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswIfSapList=dlswIfSapList, dlswDirMacIndex=dlswDirMacIndex, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirMacRowStatus=dlswDirMacRowStatus, MacAddressNC=MacAddressNC, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswDirMacEntryType=dlswDirMacEntryType, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperTable=dlswTConnOperTable, dlswInterfaceGroup=dlswInterfaceGroup, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirNBNameType=dlswDirNBNameType, dlswDirectory=dlswDirectory, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswInterface=dlswInterface, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTCPDomain=dlswTCPDomain, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, DlcType=DlcType, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirMacTable=dlswDirMacTable, dlswTConnStat=dlswTConnStat, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitStat=dlswCircuitStat, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswCoreCompliance=dlswCoreCompliance, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, NBName=NBName, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTrapControl=dlswTrapControl, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitState=dlswCircuitState, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDomains=dlswDomains, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, EndStationLocation=EndStationLocation, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswNode=dlswNode, dlswCircuit=dlswCircuit, dlswDirMacStatus=dlswDirMacStatus, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirMacMac=dlswDirMacMac, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, null=null, dlswDirNBTable=dlswDirNBTable, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswNodeUpTime=dlswNodeUpTime, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacMask=dlswDirMacMask, PYSNMP_MODULE_ID=dlsw, dlswTConnOperEntry=dlswTConnOperEntry, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswNodeVersionString=dlswNodeVersionString, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBStatus=dlswDirNBStatus, dlswIfRowStatus=dlswIfRowStatus, dlswSdlc=dlswSdlc, dlswDirGroup=dlswDirGroup, dlswDirCache=dlswDirCache, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswMIB=dlswMIB, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswNotificationGroup=dlswNotificationGroup, dlswTConn=dlswTConn, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTraps=dlswTraps, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirLocateNBName=dlswDirLocateNBName, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswCompliances=dlswCompliances, dlswNodeVersion=dlswNodeVersion, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnOperCirCreates=dlswTConnOperCirCreates, TAddress=TAddress, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswNodeGroup=dlswNodeGroup, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirNBName=dlswDirNBName, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitPriority=dlswCircuitPriority, dlswIfTable=dlswIfTable, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirCompliance=dlswDirCompliance, dlswNodeStatus=dlswNodeStatus, dlswTConnSpecific=dlswTConnSpecific, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitS2Location=dlswCircuitS2Location, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlsw=dlsw, dlswDirLocate=dlswDirLocate, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTrapTConnUp=dlswTrapTConnUp, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup)
