...
mibBuilder.exportSymbols("DLSW-MIB", dlswTraps=dlswTraps, dlswDirNBIndex=dlswDirNBIndex, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirNBGroup=dlswDirNBGroup, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirMacTable=dlswDirMacTable, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnOperState=dlswTConnOperState, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirMacEntry=dlswDirMacEntry, dlswNodeVersion=dlswNodeVersion, dlswDirNBName=dlswDirNBName, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswCircuitPriority=dlswCircuitPriority, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswSdlc=dlswSdlc, dlswCircuitEntry=dlswCircuitEntry, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, DlswTCPAddress=DlswTCPAddress, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirGroup=dlswDirGroup, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitStat=dlswCircuitStat, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirStat=dlswDirStat, NBName=NBName, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, null=null, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswDirNBEntries=dlswDirNBEntries, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitState=dlswCircuitState, dlswCoreCompliance=dlswCoreCompliance, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuit=dlswCircuit, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswInterface=dlswInterface, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTrapControl=dlswTrapControl, DlcType=DlcType, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswNodeVendorID=dlswNodeVendorID, dlswIfEntry=dlswIfEntry, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswConformance=dlswConformance, dlswDirNBTable=dlswDirNBTable, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperGroup=dlswTConnOperGroup, dlswNodeGroup=dlswNodeGroup, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswDomains=dlswDomains, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCompliances=dlswCompliances, dlswDirectory=dlswDirectory, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswNode=dlswNode, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirMacEntries=dlswDirMacEntries, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswNodeStatus=dlswNodeStatus, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperTable=dlswTConnOperTable, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitOrigin=dlswCircuitOrigin, LFSize=LFSize, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitTable=dlswCircuitTable, dlswMIB=dlswMIB, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswIfTable=dlswIfTable, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnConfigEntry=dlswTConnConfigEntry, MacAddressNC=MacAddressNC, EndStationLocation=EndStationLocation, dlswDirMacMask=dlswDirMacMask, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirMacIndex=dlswDirMacIndex, dlswTCPDomain=dlswTCPDomain, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswNodeUpTime=dlswNodeUpTime, dlswNodeVersionString=dlswNodeVersionString, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnStat=dlswTConnStat, dlswDirLocate=dlswDirLocate, dlsw=dlsw, dlswIfSapList=dlswIfSapList, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirCache=dlswDirCache, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirCompliance=dlswDirCompliance, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, PYSNMP_MODULE_ID=dlsw, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnTcp=dlswTConnTcp, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswIfRowStatus=dlswIfRowStatus, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirMacMac=dlswDirMacMac, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirMacLFSize=dlswDirMacLFSize, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswGroups=dlswGroups, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirNBLocation=dlswDirNBLocation, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, TAddress=TAddress, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitStatActives=dlswCircuitStatActives, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConn=dlswTConn, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap)
