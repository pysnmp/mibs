...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirMacLocation=dlswDirMacLocation, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, NBName=NBName, dlswGroups=dlswGroups, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlsw=dlsw, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswSdlcLsTable=dlswSdlcLsTable, dlswNodeStatus=dlswNodeStatus, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnConfigGroup=dlswTConnConfigGroup, null=null, dlswCircuitTable=dlswCircuitTable, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswIfTable=dlswIfTable, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnStat=dlswTConnStat, dlswNodeVersionString=dlswNodeVersionString, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitPriority=dlswCircuitPriority, dlswTrapControl=dlswTrapControl, dlswDirMacMask=dlswDirMacMask, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, MacAddressNC=MacAddressNC, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitStat=dlswCircuitStat, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirMacEntries=dlswDirMacEntries, dlswDirCompliance=dlswDirCompliance, dlswIfEntry=dlswIfEntry, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperCURexSents=dlswTConnOperCURexSents, DlcType=DlcType, dlswTrapTConnDown=dlswTrapTConnDown, TAddress=TAddress, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnTcp=dlswTConnTcp, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswSdlc=dlswSdlc, dlswDirMacTable=dlswDirMacTable, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirMacEntry=dlswDirMacEntry, dlswDirMacMac=dlswDirMacMac, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirCache=dlswDirCache, dlswCompliances=dlswCompliances, dlswTConnNBGroup=dlswTConnNBGroup, dlswSdlcGroup=dlswSdlcGroup, dlswDirNBNameType=dlswDirNBNameType, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirectory=dlswDirectory, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTraps=dlswTraps, dlswCoreCompliance=dlswCoreCompliance, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitStatGroup=dlswCircuitStatGroup, LFSize=LFSize, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirLocate=dlswDirLocate, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitS2Location=dlswCircuitS2Location, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirNBTable=dlswDirNBTable, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperState=dlswTConnOperState, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDomains=dlswDomains, dlswTConnOperGroup=dlswTConnOperGroup, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirMacCacheMisses=dlswDirMacCacheMisses, PYSNMP_MODULE_ID=dlsw, dlswDirMacIndex=dlswDirMacIndex, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTCPDomain=dlswTCPDomain, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirNBLocation=dlswDirNBLocation, dlswDirNBName=dlswDirNBName, dlswCircuitStatActives=dlswCircuitStatActives, dlswTrapTConnUp=dlswTrapTConnUp, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirGroup=dlswDirGroup, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswNodeVersion=dlswNodeVersion, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitEntry=dlswCircuitEntry, dlswNodeGroup=dlswNodeGroup, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswNodeUpTime=dlswNodeUpTime, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswConformance=dlswConformance, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswInterface=dlswInterface, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnConfigTable=dlswTConnConfigTable, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuit=dlswCircuit, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTrapCntlTConn=dlswTrapCntlTConn, EndStationLocation=EndStationLocation, dlswNodeVendorID=dlswNodeVendorID, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirStat=dlswDirStat, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConn=dlswTConn, DlswTCPAddress=DlswTCPAddress, dlswNotificationGroup=dlswNotificationGroup, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitGroup=dlswCircuitGroup, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitState=dlswCircuitState, dlswMIB=dlswMIB, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirMacLFSize=dlswDirMacLFSize, dlswNode=dlswNode, dlswIfSapList=dlswIfSapList, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswIfVirtualSegment=dlswIfVirtualSegment)
