...
mibBuilder.exportSymbols("DLSW-MIB", dlswMIB=dlswMIB, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitPriority=dlswCircuitPriority, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirMacTable=dlswDirMacTable, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswDirNBLocation=dlswDirNBLocation, dlswTConnStat=dlswTConnStat, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirNBTable=dlswDirNBTable, dlswTConn=dlswTConn, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirGroup=dlswDirGroup, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirLocateMacMac=dlswDirLocateMacMac, LFSize=LFSize, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitS2Mac=dlswCircuitS2Mac, EndStationLocation=EndStationLocation, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswSdlcGroup=dlswSdlcGroup, dlswNodeNBGroup=dlswNodeNBGroup, dlswNode=dlswNode, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswConformance=dlswConformance, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswIfTable=dlswIfTable, dlswDirMacLocation=dlswDirMacLocation, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswIfSapList=dlswIfSapList, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirCache=dlswDirCache, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirMacMask=dlswDirMacMask, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTrapControl=dlswTrapControl, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswInterface=dlswInterface, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswNodeVendorID=dlswNodeVendorID, dlswIfEntry=dlswIfEntry, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnStatGroup=dlswTConnStatGroup, null=null, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCompliances=dlswCompliances, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirMacMac=dlswDirMacMac, dlswGroups=dlswGroups, dlswNotificationGroup=dlswNotificationGroup, dlswDirNBRowStatus=dlswDirNBRowStatus, MacAddressNC=MacAddressNC, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitTable=dlswCircuitTable, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, TAddress=TAddress, NBName=NBName, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirLocateNBName=dlswDirLocateNBName, dlswCoreCompliance=dlswCoreCompliance, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, DlswTCPAddress=DlswTCPAddress, dlswCircuitStat=dlswCircuitStat, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitEntry=dlswCircuitEntry, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirNBStatus=dlswDirNBStatus, dlswDirCompliance=dlswDirCompliance, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswNodeVersion=dlswNodeVersion, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnTcp=dlswTConnTcp, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperState=dlswTConnOperState, dlswTraps=dlswTraps, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacIndex=dlswDirMacIndex, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirNBEntry=dlswDirNBEntry, dlsw=dlsw, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswSdlc=dlswSdlc, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, PYSNMP_MODULE_ID=dlsw, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirStat=dlswDirStat, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswNodeGroup=dlswNodeGroup, dlswDirLocate=dlswDirLocate, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswNodeStatus=dlswNodeStatus, DlcType=DlcType, dlswCircuitState=dlswCircuitState, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirNBGroup=dlswDirNBGroup, dlswDirectory=dlswDirectory, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnSpecific=dlswTConnSpecific, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuit=dlswCircuit, dlswDirNBName=dlswDirNBName, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDomains=dlswDomains, dlswTCPDomain=dlswTCPDomain, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirMacEntries=dlswDirMacEntries, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition)
