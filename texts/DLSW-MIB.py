...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitTable=dlswCircuitTable, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirMacEntries=dlswDirMacEntries, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCoreCompliance=dlswCoreCompliance, PYSNMP_MODULE_ID=dlsw, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirNBName=dlswDirNBName, dlswTConn=dlswTConn, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirGroup=dlswDirGroup, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnConfigOpens=dlswTConnConfigOpens, TAddress=TAddress, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswConformance=dlswConformance, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuit=dlswCircuit, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirCompliance=dlswDirCompliance, dlswNodeStatus=dlswNodeStatus, dlswNodeVendorID=dlswNodeVendorID, dlsw=dlsw, dlswDirLocateNBMatch=dlswDirLocateNBMatch, null=null, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswNodeGroup=dlswNodeGroup, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswCompliances=dlswCompliances, dlswInterface=dlswInterface, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocate=dlswDirLocate, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDomains=dlswDomains, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswIfRowStatus=dlswIfRowStatus, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirNBTable=dlswDirNBTable, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirMacMask=dlswDirMacMask, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, EndStationLocation=EndStationLocation, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswGroups=dlswGroups, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirNBNameType=dlswDirNBNameType, dlswMIB=dlswMIB, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacMac=dlswDirMacMac, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirNBLocation=dlswDirNBLocation, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirMacTable=dlswDirMacTable, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTraps=dlswTraps, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswIfEntry=dlswIfEntry, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnStat=dlswTConnStat, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswIfTable=dlswIfTable, dlswDirNBIndex=dlswDirNBIndex, dlswDirMacStatus=dlswDirMacStatus, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirMacIndex=dlswDirMacIndex, dlswDirNBStatus=dlswDirNBStatus, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirMacEntry=dlswDirMacEntry, dlswTConnTcp=dlswTConnTcp, NBName=NBName, dlswTConnOperState=dlswTConnOperState, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswSdlc=dlswSdlc, LFSize=LFSize, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTrapControl=dlswTrapControl, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitStat=dlswCircuitStat, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitS2Location=dlswCircuitS2Location, DlcType=DlcType, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnOperTable=dlswTConnOperTable, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirectory=dlswDirectory, dlswInterfaceGroup=dlswInterfaceGroup, dlswSdlcLsTable=dlswSdlcLsTable, dlswDirNBGroup=dlswDirNBGroup, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, MacAddressNC=MacAddressNC, dlswNodeVersion=dlswNodeVersion, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirMacLFSize=dlswDirMacLFSize, dlswSdlcCompliance=dlswSdlcCompliance, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswSdlcGroup=dlswSdlcGroup, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnConfigTable=dlswTConnConfigTable, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnStatGroup=dlswTConnStatGroup, dlswCircuitState=dlswCircuitState, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitS1DlcType=dlswCircuitS1DlcType, DlswTCPAddress=DlswTCPAddress, dlswTConnSpecific=dlswTConnSpecific, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirStat=dlswDirStat, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswNotificationGroup=dlswNotificationGroup, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswNode=dlswNode, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirNBEntries=dlswDirNBEntries, dlswDirCache=dlswDirCache, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCircuitStatActives=dlswCircuitStatActives, dlswTCPDomain=dlswTCPDomain, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswDirMacEntryType=dlswDirMacEntryType, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswIfSapList=dlswIfSapList, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds)
