...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnStat=dlswTConnStat, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, NBName=NBName, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTrapCntlTConn=dlswTrapCntlTConn, LFSize=LFSize, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirCompliance=dlswDirCompliance, EndStationLocation=EndStationLocation, dlsw=dlsw, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTraps=dlswTraps, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, TAddress=TAddress, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTrapControl=dlswTrapControl, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnSpecific=dlswTConnSpecific, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirMacEntryType=dlswDirMacEntryType, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswInterface=dlswInterface, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirNBStatus=dlswDirNBStatus, dlswSdlcGroup=dlswSdlcGroup, dlswTCPDomain=dlswTCPDomain, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswNodeUpTime=dlswNodeUpTime, dlswDirectory=dlswDirectory, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswSdlcCompliance=dlswSdlcCompliance, dlswCircuitS2Location=dlswCircuitS2Location, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnOperGroup=dlswTConnOperGroup, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswSdlc=dlswSdlc, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirNBGroup=dlswDirNBGroup, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswNodeGroup=dlswNodeGroup, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswConformance=dlswConformance, PYSNMP_MODULE_ID=dlsw, dlswDirNBEntry=dlswDirNBEntry, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswNotificationGroup=dlswNotificationGroup, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswMIB=dlswMIB, dlswDirNBLocation=dlswDirNBLocation, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirLocateNBName=dlswDirLocateNBName, dlswIfSapList=dlswIfSapList, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnTcp=dlswTConnTcp, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirNBNameType=dlswDirNBNameType, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswCircuitState=dlswCircuitState, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDomains=dlswDomains, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnOperEntry=dlswTConnOperEntry, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirNBTable=dlswDirNBTable, dlswTConnOperConnectTime=dlswTConnOperConnectTime, null=null, dlswDirMacEntry=dlswDirMacEntry, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCompliances=dlswCompliances, dlswDirMacLFSize=dlswDirMacLFSize, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConn=dlswTConn, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitPriority=dlswCircuitPriority, DlcType=DlcType, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswNode=dlswNode, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, DlswTCPAddress=DlswTCPAddress, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitTable=dlswCircuitTable, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirMacIndex=dlswDirMacIndex, dlswDirCache=dlswDirCache, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirMacStatus=dlswDirMacStatus, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirGroup=dlswDirGroup, dlswDirMacMask=dlswDirMacMask, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitEntry=dlswCircuitEntry, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuitGroup=dlswCircuitGroup, dlswCoreCompliance=dlswCoreCompliance, dlswDirNBName=dlswDirNBName, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitStat=dlswCircuitStat, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnConfigTable=dlswTConnConfigTable, dlswIfEntry=dlswIfEntry, dlswDirMacMac=dlswDirMacMac, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperState=dlswTConnOperState, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirLocate=dlswDirLocate, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuit=dlswCircuit, dlswDirStat=dlswDirStat, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswNodeStatus=dlswNodeStatus, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswIfTable=dlswIfTable, dlswTConnStatGroup=dlswTConnStatGroup, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirMacTable=dlswDirMacTable, dlswDirMacEntries=dlswDirMacEntries, MacAddressNC=MacAddressNC, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswGroups=dlswGroups, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirMacLocation=dlswDirMacLocation, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirLocateMacTable=dlswDirLocateMacTable)
