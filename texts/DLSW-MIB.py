...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirMacTable=dlswDirMacTable, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitStateTime=dlswCircuitStateTime, NBName=NBName, dlswNodeVendorID=dlswNodeVendorID, dlswDirMacMac=dlswDirMacMac, null=null, dlswDirLocate=dlswDirLocate, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, MacAddressNC=MacAddressNC, dlswSdlcCompliance=dlswSdlcCompliance, dlswGroups=dlswGroups, dlswTrapControl=dlswTrapControl, dlswCircuit=dlswCircuit, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirNBLFSize=dlswDirNBLFSize, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTraps=dlswTraps, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswNodeUpTime=dlswNodeUpTime, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnConfigTable=dlswTConnConfigTable, PYSNMP_MODULE_ID=dlsw, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirMacEntries=dlswDirMacEntries, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitEntry=dlswCircuitEntry, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirCompliance=dlswDirCompliance, dlswIfSapList=dlswIfSapList, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitTable=dlswCircuitTable, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacMask=dlswDirMacMask, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitGroup=dlswCircuitGroup, dlswNotificationGroup=dlswNotificationGroup, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirNBGroup=dlswDirNBGroup, dlswDomains=dlswDomains, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitS2Location=dlswCircuitS2Location, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCompliances=dlswCompliances, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirNBTable=dlswDirNBTable, dlswNodeGroup=dlswNodeGroup, dlswTConnNBGroup=dlswTConnNBGroup, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswConformance=dlswConformance, dlswNodeVersion=dlswNodeVersion, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitStat=dlswCircuitStat, dlswNodeVersionString=dlswNodeVersionString, dlswTConnStat=dlswTConnStat, dlswTConnOperState=dlswTConnOperState, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirCache=dlswDirCache, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirNBEntry=dlswDirNBEntry, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitS1Mac=dlswCircuitS1Mac, TAddress=TAddress, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, EndStationLocation=EndStationLocation, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTrapTConnUp=dlswTrapTConnUp, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirGroup=dlswDirGroup, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperGroup=dlswTConnOperGroup, LFSize=LFSize, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirNBName=dlswDirNBName, dlswMIB=dlswMIB, dlsw=dlsw, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswIfTable=dlswIfTable, dlswDirNBStatus=dlswDirNBStatus, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswSdlcLsTable=dlswSdlcLsTable, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirectory=dlswDirectory, dlswTConnConfigEntryType=dlswTConnConfigEntryType, DlcType=DlcType, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnTcp=dlswTConnTcp, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswIfEntry=dlswIfEntry, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirStat=dlswDirStat, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirNBNameType=dlswDirNBNameType, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirMacIndex=dlswDirMacIndex, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitState=dlswCircuitState, dlswNode=dlswNode, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTCPDomain=dlswTCPDomain, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirMacLocation=dlswDirMacLocation, DlswTCPAddress=DlswTCPAddress, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswSdlc=dlswSdlc, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswInterface=dlswInterface, dlswNodeStatus=dlswNodeStatus, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirMacEntryType=dlswDirMacEntryType, dlswCoreCompliance=dlswCoreCompliance, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConn=dlswTConn, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirMacLFSize=dlswDirMacLFSize, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport)
