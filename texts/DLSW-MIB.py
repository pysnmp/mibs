...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirMacMask=dlswDirMacMask, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitGroup=dlswCircuitGroup, dlswDirMacTable=dlswDirMacTable, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTraps=dlswTraps, dlswCircuitState=dlswCircuitState, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswNodeStatus=dlswNodeStatus, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCoreCompliance=dlswCoreCompliance, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirMacIndex=dlswDirMacIndex, dlswDirLocateGroup=dlswDirLocateGroup, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnStat=dlswTConnStat, dlsw=dlsw, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitStateTime=dlswCircuitStateTime, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswNodeVersionString=dlswNodeVersionString, dlswDirNBLFSize=dlswDirNBLFSize, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswIfTable=dlswIfTable, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswIfEntry=dlswIfEntry, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirMacEntryType=dlswDirMacEntryType, dlswNodeGroup=dlswNodeGroup, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirNBName=dlswDirNBName, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswConformance=dlswConformance, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswInterface=dlswInterface, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirNBEntry=dlswDirNBEntry, dlswNodeUpTime=dlswNodeUpTime, dlswDirCache=dlswDirCache, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, MacAddressNC=MacAddressNC, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswIfRowStatus=dlswIfRowStatus, EndStationLocation=EndStationLocation, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitStat=dlswCircuitStat, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, TAddress=TAddress, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirNBLocation=dlswDirNBLocation, DlswTCPAddress=DlswTCPAddress, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnSpecific=dlswTConnSpecific, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperGroup=dlswTConnOperGroup, dlswSdlc=dlswSdlc, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnTcp=dlswTConnTcp, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, DlcType=DlcType, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswNode=dlswNode, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirMacEntries=dlswDirMacEntries, dlswTConnNBGroup=dlswTConnNBGroup, LFSize=LFSize, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirMacLocation=dlswDirMacLocation, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirMacEntry=dlswDirMacEntry, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirGroup=dlswDirGroup, null=null, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTCPDomain=dlswTCPDomain, dlswDirNBIndex=dlswDirNBIndex, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirStat=dlswDirStat, dlswCircuitTable=dlswCircuitTable, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnOperState=dlswTConnOperState, dlswGroups=dlswGroups, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswSdlcLsTable=dlswSdlcLsTable, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperTable=dlswTConnOperTable, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacMac=dlswDirMacMac, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswNotificationGroup=dlswNotificationGroup, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTrapControl=dlswTrapControl, dlswDirNBStatus=dlswDirNBStatus, dlswDirNBTable=dlswDirNBTable, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswCompliances=dlswCompliances, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirectory=dlswDirectory, PYSNMP_MODULE_ID=dlsw, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswIfSapList=dlswIfSapList, dlswCircuit=dlswCircuit, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirNBGroup=dlswDirNBGroup, dlswNodeVersion=dlswNodeVersion, NBName=NBName, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDomains=dlswDomains, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConn=dlswTConn, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswInterfaceGroup=dlswInterfaceGroup, dlswMIB=dlswMIB, dlswDirLocate=dlswDirLocate, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswCircuitPriority=dlswCircuitPriority, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitStatActives=dlswCircuitStatActives)
