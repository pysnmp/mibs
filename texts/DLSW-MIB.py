...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirNBName=dlswDirNBName, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnStat=dlswTConnStat, dlswMIB=dlswMIB, dlswCircuitStat=dlswCircuitStat, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTrapControl=dlswTrapControl, dlswCircuitS1Sap=dlswCircuitS1Sap, DlswTCPAddress=DlswTCPAddress, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCompliances=dlswCompliances, dlswConformance=dlswConformance, LFSize=LFSize, DlcType=DlcType, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitGroup=dlswCircuitGroup, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirMacMac=dlswDirMacMac, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitTable=dlswCircuitTable, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirMacMask=dlswDirMacMask, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswSdlc=dlswSdlc, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConn=dlswTConn, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuit=dlswCircuit, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswNodeNBGroup=dlswNodeNBGroup, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitEntry=dlswCircuitEntry, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirectory=dlswDirectory, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirCompliance=dlswDirCompliance, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswDirMacIndex=dlswDirMacIndex, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswNodeVendorID=dlswNodeVendorID, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTCPDomain=dlswTCPDomain, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDomains=dlswDomains, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirStat=dlswDirStat, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTraps=dlswTraps, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCoreCompliance=dlswCoreCompliance, dlswDirLocate=dlswDirLocate, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswNodeVersion=dlswNodeVersion, dlswIfEntry=dlswIfEntry, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswInterfaceGroup=dlswInterfaceGroup, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperTable=dlswTConnOperTable, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswNode=dlswNode, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlsw=dlsw, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirMacLFSize=dlswDirMacLFSize, dlswInterface=dlswInterface, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirNBNameType=dlswDirNBNameType, dlswDirNBEntries=dlswDirNBEntries, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirLocateGroup=dlswDirLocateGroup, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirMacTable=dlswDirMacTable, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswNotificationGroup=dlswNotificationGroup, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnOperGroup=dlswTConnOperGroup, dlswCircuitState=dlswCircuitState, dlswDirNBLocationType=dlswDirNBLocationType, dlswTrapTConnUp=dlswTrapTConnUp, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, MacAddressNC=MacAddressNC, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswNodeGroup=dlswNodeGroup, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirLocateNBName=dlswDirLocateNBName, NBName=NBName, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswGroups=dlswGroups, dlswTConnTcp=dlswTConnTcp, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, EndStationLocation=EndStationLocation, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperCURexSents=dlswTConnOperCURexSents, null=null, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswIfTable=dlswIfTable, PYSNMP_MODULE_ID=dlsw, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirNBStatus=dlswDirNBStatus, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswNodeUpTime=dlswNodeUpTime, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitPriority=dlswCircuitPriority, dlswTConnConfigTable=dlswTConnConfigTable, dlswNodeStatus=dlswNodeStatus, dlswDirNBTable=dlswDirNBTable, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitStatActives=dlswCircuitStatActives, dlswDirGroup=dlswDirGroup, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirMacLocation=dlswDirMacLocation, dlswDirMacEntry=dlswDirMacEntry, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirMacEntries=dlswDirMacEntries, TAddress=TAddress, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswIfSapList=dlswIfSapList, dlswTConnOperState=dlswTConnOperState, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirCache=dlswDirCache, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswSdlcCompliance=dlswSdlcCompliance)
