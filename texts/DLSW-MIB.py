...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, MacAddressNC=MacAddressNC, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnStat=dlswTConnStat, dlswInterfaceGroup=dlswInterfaceGroup, dlswSdlcLsTable=dlswSdlcLsTable, dlsw=dlsw, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperState=dlswTConnOperState, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswDirNBStatus=dlswDirNBStatus, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnNBGroup=dlswTConnNBGroup, dlswIfEntry=dlswIfEntry, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswNodeVersion=dlswNodeVersion, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitEntry=dlswCircuitEntry, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCoreCompliance=dlswCoreCompliance, DlswTCPAddress=DlswTCPAddress, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirNBIndex=dlswDirNBIndex, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, EndStationLocation=EndStationLocation, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirCache=dlswDirCache, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirNBLocation=dlswDirNBLocation, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswConformance=dlswConformance, dlswIfTable=dlswIfTable, dlswTConn=dlswTConn, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirGroup=dlswDirGroup, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnTcp=dlswTConnTcp, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirCompliance=dlswDirCompliance, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperEntry=dlswTConnOperEntry, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTraps=dlswTraps, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirMacEntry=dlswDirMacEntry, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswNodeStatus=dlswNodeStatus, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitStat=dlswCircuitStat, dlswDirStat=dlswDirStat, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswMIB=dlswMIB, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirMacLocationType=dlswDirMacLocationType, dlswIfRowStatus=dlswIfRowStatus, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitStateTime=dlswCircuitStateTime, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirNBEntries=dlswDirNBEntries, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirMacMac=dlswDirMacMac, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, PYSNMP_MODULE_ID=dlsw, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, LFSize=LFSize, dlswInterface=dlswInterface, dlswCircuitTable=dlswCircuitTable, dlswDomains=dlswDomains, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuit=dlswCircuit, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirNBTable=dlswDirNBTable, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, DlcType=DlcType, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuitPriority=dlswCircuitPriority, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTrapControl=dlswTrapControl, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirMacLocation=dlswDirMacLocation, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirMacTable=dlswDirMacTable, dlswDirLocateNBName=dlswDirLocateNBName, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnSpecific=dlswTConnSpecific, dlswDirMacIndex=dlswDirMacIndex, dlswTCPDomain=dlswTCPDomain, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirMacMask=dlswDirMacMask, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitGroup=dlswCircuitGroup, dlswNodeGroup=dlswNodeGroup, dlswDirectory=dlswDirectory, dlswNotificationGroup=dlswNotificationGroup, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswNodeVendorID=dlswNodeVendorID, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, NBName=NBName, dlswDirLocate=dlswDirLocate, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlc=dlswSdlc, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, null=null, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswGroups=dlswGroups, dlswIfSapList=dlswIfSapList, dlswCircuitState=dlswCircuitState, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitS2Location=dlswCircuitS2Location, dlswNode=dlswNode, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCompliances=dlswCompliances, dlswCircuitStatActives=dlswCircuitStatActives, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirNBName=dlswDirNBName, TAddress=TAddress, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents)
