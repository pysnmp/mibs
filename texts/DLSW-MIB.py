...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitS2Location=dlswCircuitS2Location, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnOperGroup=dlswTConnOperGroup, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitState=dlswCircuitState, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswCircuitEntry=dlswCircuitEntry, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTrapTConnUp=dlswTrapTConnUp, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswInterface=dlswInterface, dlswCompliances=dlswCompliances, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswSdlcLsTable=dlswSdlcLsTable, TAddress=TAddress, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirNBEntries=dlswDirNBEntries, dlswDirMacMask=dlswDirMacMask, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitStat=dlswCircuitStat, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConn=dlswTConn, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirMacLocation=dlswDirMacLocation, dlswDirLocateNBEntry=dlswDirLocateNBEntry, null=null, dlswSdlc=dlswSdlc, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirMacEntry=dlswDirMacEntry, dlswNodeGroup=dlswNodeGroup, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirMacTable=dlswDirMacTable, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTrapControl=dlswTrapControl, dlswDirLocateGroup=dlswDirLocateGroup, dlswNodeStatus=dlswNodeStatus, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswIfEntry=dlswIfEntry, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirMacIndex=dlswDirMacIndex, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswCoreCompliance=dlswCoreCompliance, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswNode=dlswNode, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, LFSize=LFSize, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperState=dlswTConnOperState, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitPriority=dlswCircuitPriority, DlswTCPAddress=DlswTCPAddress, dlswDirNBTable=dlswDirNBTable, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswNodeVendorID=dlswNodeVendorID, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirLocate=dlswDirLocate, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswIfTable=dlswIfTable, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswConformance=dlswConformance, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperEntry=dlswTConnOperEntry, dlswTraps=dlswTraps, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDomains=dlswDomains, dlswSdlcCompliance=dlswSdlcCompliance, dlsw=dlsw, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirCache=dlswDirCache, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirNBLocationType=dlswDirNBLocationType, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnStat=dlswTConnStat, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuit=dlswCircuit, dlswDirectory=dlswDirectory, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnTcp=dlswTConnTcp, dlswDirMacMac=dlswDirMacMac, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, EndStationLocation=EndStationLocation, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirNBName=dlswDirNBName, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswNotificationGroup=dlswNotificationGroup, dlswCircuitTable=dlswCircuitTable, NBName=NBName, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswNodeVersion=dlswNodeVersion, dlswDirMacLFSize=dlswDirMacLFSize, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirCompliance=dlswDirCompliance, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, MacAddressNC=MacAddressNC, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitStateTime=dlswCircuitStateTime, dlswGroups=dlswGroups, dlswMIB=dlswMIB, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirNBGroup=dlswDirNBGroup, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirNBStatus=dlswDirNBStatus, dlswDirNBIndex=dlswDirNBIndex, dlswIfSapList=dlswIfSapList, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirGroup=dlswDirGroup, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitStatActives=dlswCircuitStatActives, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswNodeVersionString=dlswNodeVersionString, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, PYSNMP_MODULE_ID=dlsw, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswInterfaceGroup=dlswInterfaceGroup, dlswIfRowStatus=dlswIfRowStatus, dlswDirStat=dlswDirStat, DlcType=DlcType, dlswTCPDomain=dlswTCPDomain, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirNBLocation=dlswDirNBLocation)
