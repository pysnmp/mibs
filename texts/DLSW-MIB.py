...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, LFSize=LFSize, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirMacMask=dlswDirMacMask, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswSdlcLsTable=dlswSdlcLsTable, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswNodeVersion=dlswNodeVersion, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, PYSNMP_MODULE_ID=dlsw, dlswIfTable=dlswIfTable, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirNBName=dlswDirNBName, dlswIfEntry=dlswIfEntry, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirMacMac=dlswDirMacMac, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitS2Sap=dlswCircuitS2Sap, dlsw=dlsw, dlswNodeUpTime=dlswNodeUpTime, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirMacEntry=dlswDirMacEntry, dlswDirNBIndex=dlswDirNBIndex, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirMacIndex=dlswDirMacIndex, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperTable=dlswTConnOperTable, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperICRexSents=dlswTConnOperICRexSents, DlswTCPAddress=DlswTCPAddress, dlswDirNBTable=dlswDirNBTable, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCompliances=dlswCompliances, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnStat=dlswTConnStat, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirNBNameType=dlswDirNBNameType, dlswIfRowStatus=dlswIfRowStatus, dlswDirNBLocationType=dlswDirNBLocationType, dlswIfSapList=dlswIfSapList, dlswCircuitEntry=dlswCircuitEntry, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDomains=dlswDomains, TAddress=TAddress, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswGroups=dlswGroups, dlswTConnTcp=dlswTConnTcp, DlcType=DlcType, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswMIB=dlswMIB, dlswTCPDomain=dlswTCPDomain, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperState=dlswTConnOperState, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, NBName=NBName, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirNBEntry=dlswDirNBEntry, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirCompliance=dlswDirCompliance, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirMacLocation=dlswDirMacLocation, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswNodeStatus=dlswNodeStatus, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirGroup=dlswDirGroup, dlswCircuitS1Dlc=dlswCircuitS1Dlc, EndStationLocation=EndStationLocation, dlswCircuitGroup=dlswCircuitGroup, dlswCoreCompliance=dlswCoreCompliance, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirCache=dlswDirCache, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnSpecific=dlswTConnSpecific, dlswDirMacTable=dlswDirMacTable, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswNode=dlswNode, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuit=dlswCircuit, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTrapTConnDown=dlswTrapTConnDown, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirNBLocation=dlswDirNBLocation, dlswDirMacEntries=dlswDirMacEntries, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitState=dlswCircuitState, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTrapTConnUp=dlswTrapTConnUp, dlswConformance=dlswConformance, dlswTrapControl=dlswTrapControl, dlswTConn=dlswTConn, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswSdlc=dlswSdlc, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTraps=dlswTraps, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswNodeGroup=dlswNodeGroup, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirStat=dlswDirStat, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswNodeVendorID=dlswNodeVendorID, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswInterface=dlswInterface, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirLocate=dlswDirLocate, dlswCircuitStat=dlswCircuitStat, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirectory=dlswDirectory, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, null=null, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, MacAddressNC=MacAddressNC, dlswCircuitTable=dlswCircuitTable, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirNBLFSize=dlswDirNBLFSize, dlswNotificationGroup=dlswNotificationGroup, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirMacLFSize=dlswDirMacLFSize)
