...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitTable=dlswCircuitTable, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperState=dlswTConnOperState, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDomains=dlswDomains, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnConfigTable=dlswTConnConfigTable, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitS2Location=dlswCircuitS2Location, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitS2TDomain=dlswCircuitS2TDomain, PYSNMP_MODULE_ID=dlsw, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirMacEntry=dlswDirMacEntry, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirMacEntries=dlswDirMacEntries, dlswTCPDomain=dlswTCPDomain, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswNodeGroup=dlswNodeGroup, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirNBTable=dlswDirNBTable, dlswNodeVendorID=dlswNodeVendorID, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirNBName=dlswDirNBName, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, LFSize=LFSize, EndStationLocation=EndStationLocation, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTrapTConnUp=dlswTrapTConnUp, dlswNodeVersionString=dlswNodeVersionString, dlswTConnTcp=dlswTConnTcp, dlswDirLocate=dlswDirLocate, dlswDirNBNameType=dlswDirNBNameType, dlswDirMacTable=dlswDirMacTable, dlswInterfaceGroup=dlswInterfaceGroup, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCircuitEntry=dlswCircuitEntry, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswNode=dlswNode, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, MacAddressNC=MacAddressNC, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswIfSapList=dlswIfSapList, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirectory=dlswDirectory, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswCircuitState=dlswCircuitState, dlswTrapControl=dlswTrapControl, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, DlcType=DlcType, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, TAddress=TAddress, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirMacEntryType=dlswDirMacEntryType, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirMacMask=dlswDirMacMask, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirCompliance=dlswDirCompliance, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperDiscReason=dlswTConnOperDiscReason, null=null, dlswTConnSpecific=dlswTConnSpecific, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConn=dlswTConn, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTraps=dlswTraps, dlswDirStat=dlswDirStat, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirLocateGroup=dlswDirLocateGroup, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuit=dlswCircuit, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswNodeVersion=dlswNodeVersion, dlswConformance=dlswConformance, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswIfEntry=dlswIfEntry, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitStat=dlswCircuitStat, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirMacIndex=dlswDirMacIndex, dlswDirNBIndex=dlswDirNBIndex, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswNodeStatus=dlswNodeStatus, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnStat=dlswTConnStat, NBName=NBName, dlswCompliances=dlswCompliances, dlswDirCache=dlswDirCache, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirLocateMacLocation=dlswDirLocateMacLocation, DlswTCPAddress=DlswTCPAddress, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswMIB=dlswMIB, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswGroups=dlswGroups, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacStatus=dlswDirMacStatus, dlswDirMacMac=dlswDirMacMac, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswNotificationGroup=dlswNotificationGroup, dlswInterface=dlswInterface, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirGroup=dlswDirGroup, dlsw=dlsw, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswIfTable=dlswIfTable, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlc=dlswSdlc, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirNBLocation=dlswDirNBLocation, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirNBGroup=dlswDirNBGroup)
