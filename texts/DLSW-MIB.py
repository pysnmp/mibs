...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirGroup=dlswDirGroup, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, NBName=NBName, dlswMIB=dlswMIB, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswDirNBIndex=dlswDirNBIndex, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirNBNameType=dlswDirNBNameType, dlswTConnTcp=dlswTConnTcp, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitStat=dlswCircuitStat, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlc=dlswSdlc, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTrapControl=dlswTrapControl, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitState=dlswCircuitState, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswConformance=dlswConformance, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswIfEntry=dlswIfEntry, dlswSdlcLsTable=dlswSdlcLsTable, DlswTCPAddress=DlswTCPAddress, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDirectory=dlswDirectory, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitS2Location=dlswCircuitS2Location, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCoreCompliance=dlswCoreCompliance, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswIfSapList=dlswIfSapList, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCircuitEntry=dlswCircuitEntry, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirMacLocation=dlswDirMacLocation, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirNBName=dlswDirNBName, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirMacMac=dlswDirMacMac, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswNode=dlswNode, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitS1Dlc=dlswCircuitS1Dlc, PYSNMP_MODULE_ID=dlsw, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTrapTConnDown=dlswTrapTConnDown, dlswDomains=dlswDomains, dlswTConnStat=dlswTConnStat, null=null, dlswNodeGroup=dlswNodeGroup, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirMacMask=dlswDirMacMask, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnOperEntry=dlswTConnOperEntry, LFSize=LFSize, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirLocate=dlswDirLocate, dlswNotificationGroup=dlswNotificationGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnNBGroup=dlswTConnNBGroup, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirMacTable=dlswDirMacTable, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswCircuitStatGroup=dlswCircuitStatGroup, dlsw=dlsw, dlswTCPDomain=dlswTCPDomain, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, TAddress=TAddress, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswNodeVendorID=dlswNodeVendorID, EndStationLocation=EndStationLocation, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswIfTable=dlswIfTable, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnSpecific=dlswTConnSpecific, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswNodeVersionString=dlswNodeVersionString, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswGroups=dlswGroups, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirMacIndex=dlswDirMacIndex, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirMacLFSize=dlswDirMacLFSize, dlswDirCompliance=dlswDirCompliance, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, DlcType=DlcType, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCompliances=dlswCompliances, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperState=dlswTConnOperState, dlswDirCache=dlswDirCache, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, MacAddressNC=MacAddressNC, dlswDirNBEntry=dlswDirNBEntry, dlswDirNBTable=dlswDirNBTable, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirStat=dlswDirStat, dlswDirNBStatus=dlswDirNBStatus, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswSdlcGroup=dlswSdlcGroup, dlswDirNBEntries=dlswDirNBEntries, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTraps=dlswTraps, dlswNodeStatus=dlswNodeStatus, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBEntryType=dlswDirNBEntryType, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConn=dlswTConn, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirNBLocation=dlswDirNBLocation, dlswCircuit=dlswCircuit, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitTable=dlswCircuitTable, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswInterface=dlswInterface, dlswCircuitGroup=dlswCircuitGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt)
