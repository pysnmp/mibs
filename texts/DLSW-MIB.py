...
mibBuilder.exportSymbols("DLSW-MIB", dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitStat=dlswCircuitStat, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirMacEntries=dlswDirMacEntries, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirMacMask=dlswDirMacMask, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlsw=dlsw, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, MacAddressNC=MacAddressNC, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCompliances=dlswCompliances, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswIfEntry=dlswIfEntry, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperTable=dlswTConnOperTable, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuit=dlswCircuit, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswNode=dlswNode, dlswTConnSpecific=dlswTConnSpecific, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirGroup=dlswDirGroup, dlswTConn=dlswTConn, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswNodeStatus=dlswNodeStatus, dlswCircuitStatActives=dlswCircuitStatActives, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnConfigEntryType=dlswTConnConfigEntryType, NBName=NBName, dlswTraps=dlswTraps, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswDirNBNameType=dlswDirNBNameType, TAddress=TAddress, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCoreCompliance=dlswCoreCompliance, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirStat=dlswDirStat, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTrapTConnDown=dlswTrapTConnDown, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswNotificationGroup=dlswNotificationGroup, dlswDirMacStatus=dlswDirMacStatus, dlswDirNBRowStatus=dlswDirNBRowStatus, DlcType=DlcType, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirNBTable=dlswDirNBTable, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirLocateGroup=dlswDirLocateGroup, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswNodeUpTime=dlswNodeUpTime, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswCircuitTable=dlswCircuitTable, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirNBIndex=dlswDirNBIndex, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirNBEntries=dlswDirNBEntries, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswSdlc=dlswSdlc, dlswCircuitEntry=dlswCircuitEntry, dlswInterface=dlswInterface, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnTcp=dlswTConnTcp, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirNBName=dlswDirNBName, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnConfigTable=dlswTConnConfigTable, dlswTrapControl=dlswTrapControl, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswNodeGroup=dlswNodeGroup, dlswDirectory=dlswDirectory, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswGroups=dlswGroups, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirCache=dlswDirCache, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswCircuitS2Location=dlswCircuitS2Location, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswNodeVersionString=dlswNodeVersionString, dlswTConnConfigSapList=dlswTConnConfigSapList, null=null, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswMIB=dlswMIB, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswIfTable=dlswIfTable, dlswTConnOperCircuits=dlswTConnOperCircuits, DlswTCPAddress=DlswTCPAddress, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirMacMac=dlswDirMacMac, dlswDirLocate=dlswDirLocate, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnStatGroup=dlswTConnStatGroup, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitPriority=dlswCircuitPriority, dlswDirMacEntry=dlswDirMacEntry, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitState=dlswCircuitState, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, LFSize=LFSize, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswIfRowStatus=dlswIfRowStatus, dlswIfSapList=dlswIfSapList, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTCPDomain=dlswTCPDomain, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDomains=dlswDomains, dlswTConnOperState=dlswTConnOperState, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirCompliance=dlswDirCompliance, dlswDirMacIndex=dlswDirMacIndex, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnStat=dlswTConnStat, dlswConformance=dlswConformance, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTrapCircuitDown=dlswTrapCircuitDown, EndStationLocation=EndStationLocation, dlswNodeVersion=dlswNodeVersion, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirMacTable=dlswDirMacTable, PYSNMP_MODULE_ID=dlsw)
