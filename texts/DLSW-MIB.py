...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitTable=dlswCircuitTable, dlswCompliances=dlswCompliances, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, NBName=NBName, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirMacIndex=dlswDirMacIndex, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswCircuitGroup=dlswCircuitGroup, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswIfEntry=dlswIfEntry, dlswTrapControl=dlswTrapControl, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswSdlcLsTable=dlswSdlcLsTable, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitStat=dlswCircuitStat, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirNBStatus=dlswDirNBStatus, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirMacEntries=dlswDirMacEntries, dlswNode=dlswNode, dlswDirMacMask=dlswDirMacMask, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswNodeNBGroup=dlswNodeNBGroup, dlswInterface=dlswInterface, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswSdlc=dlswSdlc, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuitStateTime=dlswCircuitStateTime, dlswNodeGroup=dlswNodeGroup, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirNBIndex=dlswDirNBIndex, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocate=dlswDirLocate, dlswCircuit=dlswCircuit, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirectory=dlswDirectory, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDomains=dlswDomains, dlswDirMacMac=dlswDirMacMac, dlswDirMacLocation=dlswDirMacLocation, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTrapCircuitUp=dlswTrapCircuitUp, EndStationLocation=EndStationLocation, dlswDirCache=dlswDirCache, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswNodeVersionString=dlswNodeVersionString, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswNodeVendorID=dlswNodeVendorID, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitEntryTime=dlswCircuitEntryTime, TAddress=TAddress, dlswDirCompliance=dlswDirCompliance, dlswTConnOperState=dlswTConnOperState, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirNBTable=dlswDirNBTable, dlswSdlcCompliance=dlswSdlcCompliance, dlswTraps=dlswTraps, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswIfTable=dlswIfTable, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirMacStatus=dlswDirMacStatus, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirLocateMacMac=dlswDirLocateMacMac, DlcType=DlcType, dlswDirMacEntry=dlswDirMacEntry, dlswSdlcGroup=dlswSdlcGroup, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, DlswTCPAddress=DlswTCPAddress, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswIfVirtualSegment=dlswIfVirtualSegment, MacAddressNC=MacAddressNC, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCoreCompliance=dlswCoreCompliance, dlswIfSapList=dlswIfSapList, dlswMIB=dlswMIB, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTrapTConnDown=dlswTrapTConnDown, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirGroup=dlswDirGroup, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswGroups=dlswGroups, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitEntry=dlswCircuitEntry, null=null, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirNBEntryType=dlswDirNBEntryType, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnNBGroup=dlswTConnNBGroup, dlswConformance=dlswConformance, dlswTConn=dlswTConn, dlsw=dlsw, PYSNMP_MODULE_ID=dlsw, dlswNodeStatus=dlswNodeStatus, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCircuitState=dlswCircuitState, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnStat=dlswTConnStat, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirMacTable=dlswDirMacTable, LFSize=LFSize, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnTcp=dlswTConnTcp, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirStat=dlswDirStat, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirNBName=dlswDirNBName, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTCPDomain=dlswTCPDomain, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswNodeVersion=dlswNodeVersion, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnSpecific=dlswTConnSpecific, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds)
