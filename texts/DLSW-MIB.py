...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCoreCompliance=dlswCoreCompliance, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitStatActives=dlswCircuitStatActives, dlswCompliances=dlswCompliances, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswCircuit=dlswCircuit, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirMacTable=dlswDirMacTable, dlswDirMacEntry=dlswDirMacEntry, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirNBGroup=dlswDirNBGroup, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirStat=dlswDirStat, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswNodeGroup=dlswNodeGroup, dlswDirectory=dlswDirectory, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTrapControl=dlswTrapControl, dlswDirMacStatus=dlswDirMacStatus, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitTable=dlswCircuitTable, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirCompliance=dlswDirCompliance, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperState=dlswTConnOperState, dlswSdlcGroup=dlswSdlcGroup, MacAddressNC=MacAddressNC, dlswDirMacEntries=dlswDirMacEntries, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnStat=dlswTConnStat, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirLocate=dlswDirLocate, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnSpecific=dlswTConnSpecific, dlswTraps=dlswTraps, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswIfRowStatus=dlswIfRowStatus, dlswDirNBEntries=dlswDirNBEntries, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswConformance=dlswConformance, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswNodeStatus=dlswNodeStatus, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlcLsTable=dlswSdlcLsTable, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperEntry=dlswTConnOperEntry, dlswSdlcLsEntries=dlswSdlcLsEntries, LFSize=LFSize, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswCircuitGroup=dlswCircuitGroup, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswNode=dlswNode, dlswTConnOperTable=dlswTConnOperTable, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirMacLFSize=dlswDirMacLFSize, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCircuitPriority=dlswCircuitPriority, dlswDirNBName=dlswDirNBName, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswInterface=dlswInterface, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirNBTable=dlswDirNBTable, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirGroup=dlswDirGroup, dlswCircuitState=dlswCircuitState, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, PYSNMP_MODULE_ID=dlsw, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswGroups=dlswGroups, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswNodeVersionString=dlswNodeVersionString, dlswMIB=dlswMIB, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConn=dlswTConn, dlswNodeUpTime=dlswNodeUpTime, dlswDirNBNameType=dlswDirNBNameType, dlswInterfaceGroup=dlswInterfaceGroup, dlswCircuitStatCreates=dlswCircuitStatCreates, dlsw=dlsw, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitS1Sap=dlswCircuitS1Sap, TAddress=TAddress, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswIfEntry=dlswIfEntry, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTCPDomain=dlswTCPDomain, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDomains=dlswDomains, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitStat=dlswCircuitStat, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, NBName=NBName, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirMacMask=dlswDirMacMask, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirNBEntry=dlswDirNBEntry, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnConfigGroup=dlswTConnConfigGroup, EndStationLocation=EndStationLocation, dlswNodeVendorID=dlswNodeVendorID, dlswIfSapList=dlswIfSapList, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirMacIndex=dlswDirMacIndex, dlswDirMacMac=dlswDirMacMac, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, DlswTCPAddress=DlswTCPAddress, dlswIfTable=dlswIfTable, dlswDirCache=dlswDirCache, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswCircuitS2TAddress=dlswCircuitS2TAddress, DlcType=DlcType, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswSdlc=dlswSdlc, null=null, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnTcp=dlswTConnTcp, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswNodeNBGroup=dlswNodeNBGroup, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigEntryType=dlswTConnConfigEntryType)
