...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirCompliance=dlswDirCompliance, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirectory=dlswDirectory, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnTcp=dlswTConnTcp, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswSdlc=dlswSdlc, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirLocate=dlswDirLocate, dlswCircuitStateTime=dlswCircuitStateTime, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperState=dlswTConnOperState, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitStatCreates=dlswCircuitStatCreates, dlsw=dlsw, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTraps=dlswTraps, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirNBGroup=dlswDirNBGroup, dlswIfSapList=dlswIfSapList, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitStat=dlswCircuitStat, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswNode=dlswNode, dlswNodeNBGroup=dlswNodeNBGroup, dlswSdlcGroup=dlswSdlcGroup, dlswTConnSpecific=dlswTConnSpecific, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirMacMask=dlswDirMacMask, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnNBGroup=dlswTConnNBGroup, dlswTCPDomain=dlswTCPDomain, dlswDirLocateMacLocation=dlswDirLocateMacLocation, DlcType=DlcType, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswConformance=dlswConformance, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswMIB=dlswMIB, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswNodeStatus=dlswNodeStatus, dlswDomains=dlswDomains, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirMacLFSize=dlswDirMacLFSize, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, DlswTCPAddress=DlswTCPAddress, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitPriority=dlswCircuitPriority, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirCache=dlswDirCache, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, MacAddressNC=MacAddressNC, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirNBLocationType=dlswDirNBLocationType, dlswInterface=dlswInterface, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswIfEntry=dlswIfEntry, TAddress=TAddress, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, EndStationLocation=EndStationLocation, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirNBEntry=dlswDirNBEntry, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, null=null, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirMacEntryType=dlswDirMacEntryType, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirGroup=dlswDirGroup, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirMacMac=dlswDirMacMac, dlswTConn=dlswTConn, dlswCircuitTable=dlswCircuitTable, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswNodeVendorID=dlswNodeVendorID, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitState=dlswCircuitState, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirMacTable=dlswDirMacTable, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuit=dlswCircuit, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirMacIndex=dlswDirMacIndex, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswNodeVersion=dlswNodeVersion, dlswTConnStat=dlswTConnStat, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirMacEntry=dlswDirMacEntry, dlswIfTable=dlswIfTable, dlswTrapControl=dlswTrapControl, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirNBLFSize=dlswDirNBLFSize, dlswGroups=dlswGroups, dlswDirStat=dlswDirStat, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, PYSNMP_MODULE_ID=dlsw, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswNodeGroup=dlswNodeGroup, dlswCoreCompliance=dlswCoreCompliance, LFSize=LFSize, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswDirNBTable=dlswDirNBTable, dlswIfRowStatus=dlswIfRowStatus, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnConfigEntryType=dlswTConnConfigEntryType, NBName=NBName, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCompliances=dlswCompliances, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirNBName=dlswDirNBName)
