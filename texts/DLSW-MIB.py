...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTrapControl=dlswTrapControl, dlswSdlcCompliance=dlswSdlcCompliance, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, TAddress=TAddress, DlswTCPAddress=DlswTCPAddress, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTCPDomain=dlswTCPDomain, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswSdlc=dlswSdlc, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitS2Location=dlswCircuitS2Location, dlswDirMacLocation=dlswDirMacLocation, dlswConformance=dlswConformance, dlswGroups=dlswGroups, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswIfEntry=dlswIfEntry, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirCache=dlswDirCache, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirNBLocation=dlswDirNBLocation, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnStat=dlswTConnStat, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitTable=dlswCircuitTable, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirCompliance=dlswDirCompliance, dlswDirMacStatus=dlswDirMacStatus, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirNBEntries=dlswDirNBEntries, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, PYSNMP_MODULE_ID=dlsw, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitEntryTime=dlswCircuitEntryTime, NBName=NBName, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswIfTable=dlswIfTable, DlcType=DlcType, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, EndStationLocation=EndStationLocation, dlswDirNBGroup=dlswDirNBGroup, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswNodeVendorID=dlswNodeVendorID, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirLocateNBGroup=dlswDirLocateNBGroup, MacAddressNC=MacAddressNC, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirMacIndex=dlswDirMacIndex, dlswCoreCompliance=dlswCoreCompliance, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDomains=dlswDomains, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnStatGroup=dlswTConnStatGroup, dlswInterface=dlswInterface, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuit=dlswCircuit, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitStateTime=dlswCircuitStateTime, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswIfSapList=dlswIfSapList, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirStat=dlswDirStat, dlswTraps=dlswTraps, dlswNodeUpTime=dlswNodeUpTime, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswNodeGroup=dlswNodeGroup, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswMIB=dlswMIB, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirMacMask=dlswDirMacMask, dlswDirNBStatus=dlswDirNBStatus, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitS1Sap=dlswCircuitS1Sap, null=null, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuitPriority=dlswCircuitPriority, dlswDirNBIndex=dlswDirNBIndex, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswSdlcGroup=dlswSdlcGroup, dlswDirectory=dlswDirectory, dlswNodeVersion=dlswNodeVersion, dlswTConnOperState=dlswTConnOperState, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnSpecific=dlswTConnSpecific, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirMacEntry=dlswDirMacEntry, LFSize=LFSize, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirGroup=dlswDirGroup, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlsw=dlsw, dlswNodeStatus=dlswNodeStatus, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitStat=dlswCircuitStat, dlswTConnTcp=dlswTConnTcp, dlswTConnOperTable=dlswTConnOperTable, dlswDirNBName=dlswDirNBName, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswIfRowStatus=dlswIfRowStatus, dlswNodeVersionString=dlswNodeVersionString, dlswCompliances=dlswCompliances, dlswDirNBTable=dlswDirNBTable, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitEntry=dlswCircuitEntry, dlswTConn=dlswTConn, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirLocate=dlswDirLocate, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswNode=dlswNode, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirMacTable=dlswDirMacTable, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirMacMac=dlswDirMacMac, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCircuitState=dlswCircuitState)
