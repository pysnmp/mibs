...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnStat=dlswTConnStat, dlswIfEntry=dlswIfEntry, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirNBNameType=dlswDirNBNameType, dlswNode=dlswNode, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswIfSapList=dlswIfSapList, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDirMacMask=dlswDirMacMask, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirNBName=dlswDirNBName, dlswDirNBLocationType=dlswDirNBLocationType, MacAddressNC=MacAddressNC, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, null=null, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswInterface=dlswInterface, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, DlswTCPAddress=DlswTCPAddress, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswInterfaceGroup=dlswInterfaceGroup, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuit=dlswCircuit, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswConformance=dlswConformance, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirNBIndex=dlswDirNBIndex, dlswCompliances=dlswCompliances, TAddress=TAddress, dlswDirNBLocation=dlswDirNBLocation, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDomains=dlswDomains, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCoreCompliance=dlswCoreCompliance, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitPriority=dlswCircuitPriority, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswNodeVendorID=dlswNodeVendorID, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirLocate=dlswDirLocate, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnOperTable=dlswTConnOperTable, dlswTrapControl=dlswTrapControl, dlswTrapTConnUp=dlswTrapTConnUp, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswIfRowStatus=dlswIfRowStatus, PYSNMP_MODULE_ID=dlsw, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuitStat=dlswCircuitStat, dlswCircuitState=dlswCircuitState, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConn=dlswTConn, DlcType=DlcType, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnTcp=dlswTConnTcp, LFSize=LFSize, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswMIB=dlswMIB, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirLocateGroup=dlswDirLocateGroup, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswNodeUpTime=dlswNodeUpTime, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacIndex=dlswDirMacIndex, dlswTraps=dlswTraps, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirMacStatus=dlswDirMacStatus, dlswDirMacEntry=dlswDirMacEntry, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnOperGroup=dlswTConnOperGroup, dlswIfTable=dlswIfTable, dlswSdlcGroup=dlswSdlcGroup, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswDirCache=dlswDirCache, dlswDirectory=dlswDirectory, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirGroup=dlswDirGroup, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperState=dlswTConnOperState, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitGroup=dlswCircuitGroup, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirNBEntryType=dlswDirNBEntryType, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, EndStationLocation=EndStationLocation, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnStatGroup=dlswTConnStatGroup, dlsw=dlsw, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTrapTConnDown=dlswTrapTConnDown, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswNodeVersion=dlswNodeVersion, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirLocateNBTable=dlswDirLocateNBTable, NBName=NBName, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirMacLocationType=dlswDirMacLocationType, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirCompliance=dlswDirCompliance, dlswTCPDomain=dlswTCPDomain, dlswSdlc=dlswSdlc, dlswDirMacMac=dlswDirMacMac, dlswNodeStatus=dlswNodeStatus, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirStat=dlswDirStat, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswNodeGroup=dlswNodeGroup, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnNBGroup=dlswTConnNBGroup, dlswNotificationGroup=dlswNotificationGroup, dlswCircuitTable=dlswCircuitTable, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirMacTable=dlswDirMacTable, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswNodeVersionString=dlswNodeVersionString, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirNBTable=dlswDirNBTable, dlswGroups=dlswGroups, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirNBEntries=dlswDirNBEntries, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnSpecific=dlswTConnSpecific)
