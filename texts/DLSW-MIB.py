...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, LFSize=LFSize, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirCompliance=dlswDirCompliance, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDomains=dlswDomains, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswNodeStatus=dlswNodeStatus, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswIfEntry=dlswIfEntry, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTrapTConnUp=dlswTrapTConnUp, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTCPDomain=dlswTCPDomain, dlswCircuitStatActives=dlswCircuitStatActives, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTraps=dlswTraps, dlsw=dlsw, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirLocate=dlswDirLocate, dlswTConnOperState=dlswTConnOperState, dlswTConnStat=dlswTConnStat, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswIfSapList=dlswIfSapList, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, MacAddressNC=MacAddressNC, dlswDirNBLocationType=dlswDirNBLocationType, dlswSdlc=dlswSdlc, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirMacMask=dlswDirMacMask, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswNodeVersionString=dlswNodeVersionString, dlswTConn=dlswTConn, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTrapTConnDown=dlswTrapTConnDown, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswNodeVersion=dlswNodeVersion, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnSpecific=dlswTConnSpecific, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswConformance=dlswConformance, dlswTConnOperTable=dlswTConnOperTable, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitTable=dlswCircuitTable, dlswCircuitEntry=dlswCircuitEntry, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, PYSNMP_MODULE_ID=dlsw, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, null=null, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirectory=dlswDirectory, dlswTrapControl=dlswTrapControl, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswGroups=dlswGroups, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirCache=dlswDirCache, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswNode=dlswNode, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirGroup=dlswDirGroup, dlswCircuitGroup=dlswCircuitGroup, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitState=dlswCircuitState, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswNodeVendorID=dlswNodeVendorID, dlswDirMacMac=dlswDirMacMac, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuitStat=dlswCircuitStat, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirNBIndex=dlswDirNBIndex, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitS2TDomain=dlswCircuitS2TDomain, TAddress=TAddress, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnConfigOpens=dlswTConnConfigOpens, EndStationLocation=EndStationLocation, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, DlswTCPAddress=DlswTCPAddress, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirMacEntry=dlswDirMacEntry, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswNotificationGroup=dlswNotificationGroup, dlswCircuit=dlswCircuit, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirMacLFSize=dlswDirMacLFSize, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnTcp=dlswTConnTcp, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswIfTable=dlswIfTable, dlswDirNBStatus=dlswDirNBStatus, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTrapCircuitDown=dlswTrapCircuitDown, NBName=NBName, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperEntry=dlswTConnOperEntry, dlswInterface=dlswInterface, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirStat=dlswDirStat, dlswDirMacTable=dlswDirMacTable, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirNBNameType=dlswDirNBNameType, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCoreCompliance=dlswCoreCompliance, dlswNodeGroup=dlswNodeGroup, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirNBTable=dlswDirNBTable, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirMacEntries=dlswDirMacEntries, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnConfigTable=dlswTConnConfigTable, dlswCompliances=dlswCompliances, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirMacIndex=dlswDirMacIndex, dlswCircuitS2Location=dlswCircuitS2Location, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirNBEntries=dlswDirNBEntries, dlswDirNBName=dlswDirNBName, dlswIfRowStatus=dlswIfRowStatus, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirLocateNBEntry=dlswDirLocateNBEntry, DlcType=DlcType, dlswMIB=dlswMIB, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirNBLFSize=dlswDirNBLFSize, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswCircuitS1Dlc=dlswCircuitS1Dlc)
