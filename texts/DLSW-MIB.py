...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswSdlcCompliance=dlswSdlcCompliance, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswSdlc=dlswSdlc, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirNBIndex=dlswDirNBIndex, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCircuitStatCreates=dlswCircuitStatCreates, DlcType=DlcType, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswNodeVendorID=dlswNodeVendorID, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswDirMacStatus=dlswDirMacStatus, dlswCircuitTable=dlswCircuitTable, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirLocateMacMac=dlswDirLocateMacMac, TAddress=TAddress, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswDirNBEntries=dlswDirNBEntries, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswIfRowStatus=dlswIfRowStatus, NBName=NBName, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswNodeVersionString=dlswNodeVersionString, EndStationLocation=EndStationLocation, dlswDirNBNameType=dlswDirNBNameType, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirMacIndex=dlswDirMacIndex, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirMacEntry=dlswDirMacEntry, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacEntries=dlswDirMacEntries, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirMacTable=dlswDirMacTable, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnStatGroup=dlswTConnStatGroup, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTCPDomain=dlswTCPDomain, dlswNode=dlswNode, dlswTConnSpecific=dlswTConnSpecific, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswNotificationGroup=dlswNotificationGroup, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswNodeNBGroup=dlswNodeNBGroup, dlswIfSapList=dlswIfSapList, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitGroup=dlswCircuitGroup, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitState=dlswCircuitState, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnConfigTable=dlswTConnConfigTable, dlswTraps=dlswTraps, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirNBGroup=dlswDirNBGroup, dlswGroups=dlswGroups, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDomains=dlswDomains, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirNBTable=dlswDirNBTable, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, PYSNMP_MODULE_ID=dlsw, MacAddressNC=MacAddressNC, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswCircuit=dlswCircuit, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirStat=dlswDirStat, dlswIfEntry=dlswIfEntry, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswInterface=dlswInterface, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCoreCompliance=dlswCoreCompliance, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnConfigEntry=dlswTConnConfigEntry, null=null, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnStat=dlswTConnStat, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacLFSize=dlswDirMacLFSize, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirMacMac=dlswDirMacMac, dlswCircuitStat=dlswCircuitStat, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitPriority=dlswCircuitPriority, dlswSdlcLsTable=dlswSdlcLsTable, dlswConformance=dlswConformance, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitEntry=dlswCircuitEntry, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswNodeGroup=dlswNodeGroup, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, LFSize=LFSize, DlswTCPAddress=DlswTCPAddress, dlswDirectory=dlswDirectory, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswNodeStatus=dlswNodeStatus, dlswDirNBLocation=dlswDirNBLocation, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswIfTable=dlswIfTable, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperState=dlswTConnOperState, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCompliances=dlswCompliances, dlswTConnTcp=dlswTConnTcp, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirNBName=dlswDirNBName, dlswMIB=dlswMIB, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirMacMask=dlswDirMacMask, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswNodeVersion=dlswNodeVersion, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirCache=dlswDirCache, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperTable=dlswTConnOperTable, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnOperGroup=dlswTConnOperGroup, dlswTrapTConnUp=dlswTrapTConnUp, dlswSdlcGroup=dlswSdlcGroup, dlswDirGroup=dlswDirGroup, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswInterfaceGroup=dlswInterfaceGroup, dlswTrapControl=dlswTrapControl, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirLocate=dlswDirLocate, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConn=dlswTConn, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlsw=dlsw, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirNBCacheHits=dlswDirNBCacheHits)
