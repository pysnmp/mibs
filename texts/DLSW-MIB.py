...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswInterface=dlswInterface, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlsw=dlsw, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswNotificationGroup=dlswNotificationGroup, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswNode=dlswNode, dlswDirMacStatus=dlswDirMacStatus, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirMacIndex=dlswDirMacIndex, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitTable=dlswCircuitTable, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirNBStatus=dlswDirNBStatus, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswConformance=dlswConformance, dlswCircuitEntry=dlswCircuitEntry, dlswSdlcGroup=dlswSdlcGroup, null=null, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirNBNameType=dlswDirNBNameType, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswNodeNBGroup=dlswNodeNBGroup, dlswInterfaceGroup=dlswInterfaceGroup, dlswMIB=dlswMIB, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTrapTConnUp=dlswTrapTConnUp, dlswDirNBName=dlswDirNBName, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirNBLocation=dlswDirNBLocation, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswCompliances=dlswCompliances, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitPriority=dlswCircuitPriority, dlswDirCompliance=dlswDirCompliance, dlswDirNBTable=dlswDirNBTable, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswIfTable=dlswIfTable, dlswDirStat=dlswDirStat, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswSdlc=dlswSdlc, dlswIfEntry=dlswIfEntry, dlswTConnStat=dlswTConnStat, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswIfSapList=dlswIfSapList, dlswNodeVersionString=dlswNodeVersionString, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnTcp=dlswTConnTcp, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswNodeVersion=dlswNodeVersion, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirMacTable=dlswDirMacTable, dlswCircuitState=dlswCircuitState, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswNodeGroup=dlswNodeGroup, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirMacMask=dlswDirMacMask, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTrapControl=dlswTrapControl, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnSpecific=dlswTConnSpecific, dlswTConnOperState=dlswTConnOperState, dlswTConn=dlswTConn, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnConfigOpens=dlswTConnConfigOpens, DlswTCPAddress=DlswTCPAddress, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirNBGroup=dlswDirNBGroup, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, MacAddressNC=MacAddressNC, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTraps=dlswTraps, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnOperCURexSents=dlswTConnOperCURexSents, NBName=NBName, dlswDirMacMac=dlswDirMacMac, dlswDirGroup=dlswDirGroup, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswIfRowStatus=dlswIfRowStatus, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswNodeStatus=dlswNodeStatus, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTCPDomain=dlswTCPDomain, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswSdlcLsTable=dlswSdlcLsTable, dlswDirCache=dlswDirCache, dlswCircuitStateTime=dlswCircuitStateTime, PYSNMP_MODULE_ID=dlsw, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirLocate=dlswDirLocate, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDomains=dlswDomains, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswSdlcCompliance=dlswSdlcCompliance, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirMacEntry=dlswDirMacEntry, dlswDirNBLFSize=dlswDirNBLFSize, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswDirNBIndex=dlswDirNBIndex, dlswNodeUpTime=dlswNodeUpTime, DlcType=DlcType, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswGroups=dlswGroups, dlswCoreCompliance=dlswCoreCompliance, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirNBEntries=dlswDirNBEntries, dlswDirLocateMacLocation=dlswDirLocateMacLocation, LFSize=LFSize, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirMacEntries=dlswDirMacEntries, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswCircuit=dlswCircuit, dlswCircuitStat=dlswCircuitStat, EndStationLocation=EndStationLocation, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirectory=dlswDirectory, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperCirCreates=dlswTConnOperCirCreates, TAddress=TAddress, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirNBCacheMisses=dlswDirNBCacheMisses)
