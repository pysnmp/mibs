...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, LFSize=LFSize, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlsw=dlsw, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTrapTConnUp=dlswTrapTConnUp, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswSdlcLsTable=dlswSdlcLsTable, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, null=null, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirNBTable=dlswDirNBTable, dlswDirNBIndex=dlswDirNBIndex, dlswDirectory=dlswDirectory, EndStationLocation=EndStationLocation, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuit=dlswCircuit, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperEntry=dlswTConnOperEntry, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCoreCompliance=dlswCoreCompliance, dlswSdlc=dlswSdlc, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswMIB=dlswMIB, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTraps=dlswTraps, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirMacEntryType=dlswDirMacEntryType, dlswConformance=dlswConformance, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswNodeVersionString=dlswNodeVersionString, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswInterface=dlswInterface, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswNodeUpTime=dlswNodeUpTime, dlswDirNBStatus=dlswDirNBStatus, dlswTConnConfigTable=dlswTConnConfigTable, dlswIfTable=dlswIfTable, dlswIfEntry=dlswIfEntry, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswCircuitState=dlswCircuitState, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitGroup=dlswCircuitGroup, dlswDirMacEntry=dlswDirMacEntry, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswNodeStatus=dlswNodeStatus, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnOperState=dlswTConnOperState, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirNBName=dlswDirNBName, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirStat=dlswDirStat, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTrapTConnDown=dlswTrapTConnDown, dlswNotificationGroup=dlswNotificationGroup, dlswTConnStatGroup=dlswTConnStatGroup, dlswNodeGroup=dlswNodeGroup, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDomains=dlswDomains, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswSdlcCompliance=dlswSdlcCompliance, dlswCompliances=dlswCompliances, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirMacIndex=dlswDirMacIndex, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitPriority=dlswCircuitPriority, dlswDirMacStatus=dlswDirMacStatus, DlcType=DlcType, dlswDirMacMask=dlswDirMacMask, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnStat=dlswTConnStat, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitStateTime=dlswCircuitStateTime, dlswNodeVersion=dlswNodeVersion, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirMacEntries=dlswDirMacEntries, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperTable=dlswTConnOperTable, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnSpecific=dlswTConnSpecific, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitStatActives=dlswCircuitStatActives, dlswDirNBCacheHits=dlswDirNBCacheHits, NBName=NBName, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswIfSapList=dlswIfSapList, dlswDirMacMac=dlswDirMacMac, dlswTConn=dlswTConn, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswGroups=dlswGroups, dlswDirGroup=dlswDirGroup, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirCompliance=dlswDirCompliance, PYSNMP_MODULE_ID=dlsw, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswCircuitStat=dlswCircuitStat, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirLocate=dlswDirLocate, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitTable=dlswCircuitTable, dlswCircuitS2Location=dlswCircuitS2Location, dlswNode=dlswNode, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirCache=dlswDirCache, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnConfigTDomain=dlswTConnConfigTDomain, MacAddressNC=MacAddressNC, dlswDirMacTable=dlswDirMacTable, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnTcp=dlswTConnTcp, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirNBLocationType=dlswDirNBLocationType, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTCPDomain=dlswTCPDomain, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirNBLocation=dlswDirNBLocation, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, DlswTCPAddress=DlswTCPAddress, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirNBEntries=dlswDirNBEntries, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, TAddress=TAddress, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitEntry=dlswCircuitEntry, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTrapControl=dlswTrapControl)
