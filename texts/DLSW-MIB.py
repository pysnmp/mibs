...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitStatCreates=dlswCircuitStatCreates, dlswDirNBTable=dlswDirNBTable, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswMIB=dlswMIB, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswNodeGroup=dlswNodeGroup, dlswNodeVersion=dlswNodeVersion, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswNodeVendorID=dlswNodeVendorID, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDomains=dlswDomains, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswSdlc=dlswSdlc, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConn=dlswTConn, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirNBLocation=dlswDirNBLocation, dlswIfSapList=dlswIfSapList, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, DlswTCPAddress=DlswTCPAddress, dlswDirMacEntry=dlswDirMacEntry, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirNBLFSize=dlswDirNBLFSize, dlswConformance=dlswConformance, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirNBGroup=dlswDirNBGroup, dlswDirectory=dlswDirectory, dlswDirNBName=dlswDirNBName, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirGroup=dlswDirGroup, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirNBIndex=dlswDirNBIndex, dlswNodeStatus=dlswNodeStatus, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswGroups=dlswGroups, dlswTraps=dlswTraps, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitPriority=dlswCircuitPriority, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, MacAddressNC=MacAddressNC, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitTable=dlswCircuitTable, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitGroup=dlswCircuitGroup, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswIfRowStatus=dlswIfRowStatus, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirNBEntry=dlswDirNBEntry, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperState=dlswTConnOperState, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirMacMac=dlswDirMacMac, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswCircuitS1Mac=dlswCircuitS1Mac, PYSNMP_MODULE_ID=dlsw, dlswCircuitState=dlswCircuitState, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswNodeVersionString=dlswNodeVersionString, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirLocateGroup=dlswDirLocateGroup, dlswCompliances=dlswCompliances, dlswCircuitStat=dlswCircuitStat, null=null, dlswTCPDomain=dlswTCPDomain, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirMacTable=dlswDirMacTable, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswNodeUpTime=dlswNodeUpTime, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirMacEntries=dlswDirMacEntries, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirLocate=dlswDirLocate, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirMacIndex=dlswDirMacIndex, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswNode=dlswNode, dlswDirMacMask=dlswDirMacMask, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, DlcType=DlcType, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswIfEntry=dlswIfEntry, dlswDirCache=dlswDirCache, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswInterface=dlswInterface, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnTcp=dlswTConnTcp, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirCompliance=dlswDirCompliance, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirMacStatus=dlswDirMacStatus, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuit=dlswCircuit, dlswCircuitStatActives=dlswCircuitStatActives, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswIfTable=dlswIfTable, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirLocateNBMatch=dlswDirLocateNBMatch, EndStationLocation=EndStationLocation, TAddress=TAddress, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, LFSize=LFSize, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacLFSize=dlswDirMacLFSize, dlswDirNBStatus=dlswDirNBStatus, dlswInterfaceGroup=dlswInterfaceGroup, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirNBEntries=dlswDirNBEntries, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswDirStat=dlswDirStat, NBName=NBName, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTrapControl=dlswTrapControl, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlsw=dlsw, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnStat=dlswTConnStat, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion)
