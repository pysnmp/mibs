...
mibBuilder.exportSymbols("DLSW-MIB", dlswSdlcGroup=dlswSdlcGroup, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswIfEntry=dlswIfEntry, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlsw=dlsw, dlswTConn=dlswTConn, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnSpecific=dlswTConnSpecific, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirLocate=dlswDirLocate, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirNBLocation=dlswDirNBLocation, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCircuitStatActives=dlswCircuitStatActives, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnStat=dlswTConnStat, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitGroup=dlswCircuitGroup, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirMacRowStatus=dlswDirMacRowStatus, DlswTCPAddress=DlswTCPAddress, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswIfSapList=dlswIfSapList, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, MacAddressNC=MacAddressNC, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnStatGroup=dlswTConnStatGroup, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirLocateNBMatch=dlswDirLocateNBMatch, NBName=NBName, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirCache=dlswDirCache, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCompliances=dlswCompliances, dlswSdlcCompliance=dlswSdlcCompliance, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDomains=dlswDomains, dlswIfRowStatus=dlswIfRowStatus, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirMacMac=dlswDirMacMac, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirNBName=dlswDirNBName, dlswDirNBTable=dlswDirNBTable, dlswInterface=dlswInterface, dlswDirNBEntries=dlswDirNBEntries, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuit=dlswCircuit, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirMacEntries=dlswDirMacEntries, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirGroup=dlswDirGroup, dlswDirStat=dlswDirStat, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswGroups=dlswGroups, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirMacLocation=dlswDirMacLocation, LFSize=LFSize, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirectory=dlswDirectory, dlswTConnConfigOpens=dlswTConnConfigOpens, PYSNMP_MODULE_ID=dlsw, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitStat=dlswCircuitStat, dlswTConnNBGroup=dlswTConnNBGroup, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswNodeVersionString=dlswNodeVersionString, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswCoreCompliance=dlswCoreCompliance, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirMacTable=dlswDirMacTable, dlswNodeGroup=dlswNodeGroup, TAddress=TAddress, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitState=dlswCircuitState, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirMacMask=dlswDirMacMask, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswSdlc=dlswSdlc, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitS2Mac=dlswCircuitS2Mac, DlcType=DlcType, dlswTConnTcp=dlswTConnTcp, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirMacIndex=dlswDirMacIndex, null=null, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswConformance=dlswConformance, dlswDirNBEntry=dlswDirNBEntry, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperState=dlswTConnOperState, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirNBGroup=dlswDirNBGroup, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswNodeStatus=dlswNodeStatus, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTCPDomain=dlswTCPDomain, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswIfTable=dlswIfTable, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswMIB=dlswMIB, dlswNode=dlswNode, dlswDirNBRowStatus=dlswDirNBRowStatus, EndStationLocation=EndStationLocation, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCircuitS2Location=dlswCircuitS2Location, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperTable=dlswTConnOperTable, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitTable=dlswCircuitTable, dlswNodeVendorID=dlswNodeVendorID, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirMacStatus=dlswDirMacStatus, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuitStateTime=dlswCircuitStateTime, dlswTrapControl=dlswTrapControl, dlswCircuitPriority=dlswCircuitPriority, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTraps=dlswTraps, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswNodeNBGroup=dlswNodeNBGroup, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitEntry=dlswCircuitEntry, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts)
