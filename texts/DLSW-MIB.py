...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnOperDiscReason=dlswTConnOperDiscReason, EndStationLocation=EndStationLocation, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswNode=dlswNode, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswMIB=dlswMIB, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitState=dlswCircuitState, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirStat=dlswDirStat, TAddress=TAddress, null=null, dlswDirNBTable=dlswDirNBTable, dlswTConnStat=dlswTConnStat, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswNodeVersionString=dlswNodeVersionString, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, PYSNMP_MODULE_ID=dlsw, dlswDirMacStatus=dlswDirMacStatus, dlswConformance=dlswConformance, dlswNodeVersion=dlswNodeVersion, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswGroups=dlswGroups, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, LFSize=LFSize, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirNBLocation=dlswDirNBLocation, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitStat=dlswCircuitStat, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitEntry=dlswCircuitEntry, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, DlswTCPAddress=DlswTCPAddress, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCompliances=dlswCompliances, dlswTConnOperState=dlswTConnOperState, dlswDirCache=dlswDirCache, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswIfRowStatus=dlswIfRowStatus, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTCPDomain=dlswTCPDomain, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirMacEntries=dlswDirMacEntries, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnStatGroup=dlswTConnStatGroup, dlswIfEntry=dlswIfEntry, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitS2Location=dlswCircuitS2Location, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirNBNameType=dlswDirNBNameType, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirNBName=dlswDirNBName, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirLocateNBName=dlswDirLocateNBName, dlswTraps=dlswTraps, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirNBEntry=dlswDirNBEntry, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswSdlcGroup=dlswSdlcGroup, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlsw=dlsw, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswSdlc=dlswSdlc, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocate=dlswDirLocate, dlswDirMacMask=dlswDirMacMask, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirNBGroup=dlswDirNBGroup, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswInterface=dlswInterface, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDomains=dlswDomains, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirMacTable=dlswDirMacTable, dlswTConnTcp=dlswTConnTcp, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirCompliance=dlswDirCompliance, dlswNodeStatus=dlswNodeStatus, dlswCircuitTable=dlswCircuitTable, dlswDirMacIndex=dlswDirMacIndex, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnSpecific=dlswTConnSpecific, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, NBName=NBName, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnOperTable=dlswTConnOperTable, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirMacEntry=dlswDirMacEntry, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswNodeVendorID=dlswNodeVendorID, dlswDirMacCacheHits=dlswDirMacCacheHits, DlcType=DlcType, dlswCircuit=dlswCircuit, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswCircuitOrigin=dlswCircuitOrigin, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConn=dlswTConn, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswSdlcLsTable=dlswSdlcLsTable, dlswIfSapList=dlswIfSapList, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswNodeGroup=dlswNodeGroup, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirectory=dlswDirectory, dlswDirNBEntryType=dlswDirNBEntryType, MacAddressNC=MacAddressNC, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswIfTable=dlswIfTable, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirGroup=dlswDirGroup, dlswDirMacEntryType=dlswDirMacEntryType, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTrapControl=dlswTrapControl, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswDirMacMac=dlswDirMacMac, dlswTConnConfigEntry=dlswTConnConfigEntry)
