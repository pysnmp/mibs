...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitPriority=dlswCircuitPriority, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswIfTable=dlswIfTable, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnConfigTable=dlswTConnConfigTable, dlswTrapTConnUp=dlswTrapTConnUp, dlswSdlcGroup=dlswSdlcGroup, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, DlcType=DlcType, dlsw=dlsw, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, DlswTCPAddress=DlswTCPAddress, dlswDomains=dlswDomains, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswNodeVersion=dlswNodeVersion, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswCircuitState=dlswCircuitState, dlswNode=dlswNode, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswIfRowStatus=dlswIfRowStatus, dlswTConnStat=dlswTConnStat, dlswDirLocateNBGroup=dlswDirLocateNBGroup, MacAddressNC=MacAddressNC, dlswTraps=dlswTraps, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswNodeNBGroup=dlswNodeNBGroup, null=null, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTrapTConnDown=dlswTrapTConnDown, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirCache=dlswDirCache, dlswDirMacEntries=dlswDirMacEntries, NBName=NBName, dlswTConnSpecific=dlswTConnSpecific, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswIfEntry=dlswIfEntry, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlc=dlswSdlc, dlswNotificationGroup=dlswNotificationGroup, TAddress=TAddress, dlswDirNBEntry=dlswDirNBEntry, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitTable=dlswCircuitTable, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswNodeGroup=dlswNodeGroup, dlswTCPDomain=dlswTCPDomain, dlswIfSapList=dlswIfSapList, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirMacMac=dlswDirMacMac, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswGroups=dlswGroups, dlswTConnNBGroup=dlswTConnNBGroup, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswCoreCompliance=dlswCoreCompliance, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnTcp=dlswTConnTcp, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswConformance=dlswConformance, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, PYSNMP_MODULE_ID=dlsw, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitStatActives=dlswCircuitStatActives, dlswDirNBEntries=dlswDirNBEntries, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacStatus=dlswDirMacStatus, dlswTrapControl=dlswTrapControl, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswNodeVersionString=dlswNodeVersionString, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCompliances=dlswCompliances, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirGroup=dlswDirGroup, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuit=dlswCircuit, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswDirNBName=dlswDirNBName, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswMIB=dlswMIB, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitStat=dlswCircuitStat, dlswDirectory=dlswDirectory, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswSdlcLsTable=dlswSdlcLsTable, dlswDirMacEntry=dlswDirMacEntry, dlswDirLocate=dlswDirLocate, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, LFSize=LFSize, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswNodeStatus=dlswNodeStatus, dlswTConn=dlswTConn, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirMacTable=dlswDirMacTable, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirMacIndex=dlswDirMacIndex, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswNodeVendorID=dlswNodeVendorID, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswDirNBStatus=dlswDirNBStatus, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswInterface=dlswInterface, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnOperState=dlswTConnOperState, dlswDirMacMask=dlswDirMacMask, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirNBTable=dlswDirNBTable, dlswDirStat=dlswDirStat, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, EndStationLocation=EndStationLocation, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnOperDiscTime=dlswTConnOperDiscTime)
