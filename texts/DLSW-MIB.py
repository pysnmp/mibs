...
mibBuilder.exportSymbols("DLSW-MIB", dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTCPDomain=dlswTCPDomain, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswNodeStatus=dlswNodeStatus, dlswNotificationGroup=dlswNotificationGroup, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswNode=dlswNode, dlswDirNBEntries=dlswDirNBEntries, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, DlswTCPAddress=DlswTCPAddress, dlswDirMacLFSize=dlswDirMacLFSize, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswCompliances=dlswCompliances, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirLocate=dlswDirLocate, dlswCircuitStat=dlswCircuitStat, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTraps=dlswTraps, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCircuit=dlswCircuit, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswIfTable=dlswIfTable, dlswDirMacLocationType=dlswDirMacLocationType, dlswTrapTConnDown=dlswTrapTConnDown, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirStat=dlswDirStat, dlswDirMacStatus=dlswDirMacStatus, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirMacMac=dlswDirMacMac, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitGroup=dlswCircuitGroup, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTrapControl=dlswTrapControl, dlswDirNBLocation=dlswDirNBLocation, dlswDirMacIndex=dlswDirMacIndex, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirNBTable=dlswDirNBTable, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConn=dlswTConn, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, NBName=NBName, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswSdlc=dlswSdlc, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperEntry=dlswTConnOperEntry, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirGroup=dlswDirGroup, dlswGroups=dlswGroups, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswConformance=dlswConformance, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswInterface=dlswInterface, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnTcp=dlswTConnTcp, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswDirLocateGroup=dlswDirLocateGroup, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnStatGroup=dlswTConnStatGroup, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswIfSapList=dlswIfSapList, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitEntry=dlswCircuitEntry, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, DlcType=DlcType, MacAddressNC=MacAddressNC, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswIfRowStatus=dlswIfRowStatus, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirMacTable=dlswDirMacTable, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswNodeGroup=dlswNodeGroup, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswInterfaceGroup=dlswInterfaceGroup, dlswDirMacEntries=dlswDirMacEntries, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswNodeVendorID=dlswNodeVendorID, TAddress=TAddress, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirMacMask=dlswDirMacMask, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirMacEntry=dlswDirMacEntry, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswSdlcLsTable=dlswSdlcLsTable, dlswMIB=dlswMIB, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirNBName=dlswDirNBName, dlswCircuitState=dlswCircuitState, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnNBGroup=dlswTConnNBGroup, null=null, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirCompliance=dlswDirCompliance, dlswDirectory=dlswDirectory, EndStationLocation=EndStationLocation, dlswDirNBIndex=dlswDirNBIndex, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperTable=dlswTConnOperTable, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnOperState=dlswTConnOperState, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitPriority=dlswCircuitPriority, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswNodeUpTime=dlswNodeUpTime, dlswDirNBNameType=dlswDirNBNameType, PYSNMP_MODULE_ID=dlsw, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirLocateNBName=dlswDirLocateNBName, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTrapTConnUp=dlswTrapTConnUp, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDomains=dlswDomains, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnStat=dlswTConnStat, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, LFSize=LFSize, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlsw=dlsw, dlswDirCache=dlswDirCache, dlswCircuitTable=dlswCircuitTable, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswIfEntry=dlswIfEntry, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitOrigin=dlswCircuitOrigin)
