...
mibBuilder.exportSymbols("DLSW-MIB", dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswGroups=dlswGroups, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirNBLocationType=dlswDirNBLocationType, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirLocate=dlswDirLocate, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirLocateGroup=dlswDirLocateGroup, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirMacEntry=dlswDirMacEntry, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirNBTable=dlswDirNBTable, dlswDirLocateMacTable=dlswDirLocateMacTable, LFSize=LFSize, dlswCircuit=dlswCircuit, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTrapCircuitUp=dlswTrapCircuitUp, DlswTCPAddress=DlswTCPAddress, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirMacLocation=dlswDirMacLocation, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirMacLFSize=dlswDirMacLFSize, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirNBGroup=dlswDirNBGroup, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperEntry=dlswTConnOperEntry, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTraps=dlswTraps, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, NBName=NBName, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirNBStatus=dlswDirNBStatus, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, EndStationLocation=EndStationLocation, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnOperTable=dlswTConnOperTable, dlswInterface=dlswInterface, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitStatActives=dlswCircuitStatActives, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirNBEntry=dlswDirNBEntry, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnTcp=dlswTConnTcp, dlswIfEntry=dlswIfEntry, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitPriority=dlswCircuitPriority, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswNodeUpTime=dlswNodeUpTime, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirMacTable=dlswDirMacTable, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswIfSapList=dlswIfSapList, MacAddressNC=MacAddressNC, dlswIfTable=dlswIfTable, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitState=dlswCircuitState, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, TAddress=TAddress, dlswSdlcCompliance=dlswSdlcCompliance, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswNodeStatus=dlswNodeStatus, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacMac=dlswDirMacMac, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperState=dlswTConnOperState, dlswNode=dlswNode, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswInterfaceGroup=dlswInterfaceGroup, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswNotificationGroup=dlswNotificationGroup, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswNodeGroup=dlswNodeGroup, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirMacStatus=dlswDirMacStatus, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirGroup=dlswDirGroup, dlswDirNBLFSize=dlswDirNBLFSize, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswNodeVersion=dlswNodeVersion, dlswDirMacMask=dlswDirMacMask, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswConformance=dlswConformance, dlswTConn=dlswTConn, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswMIB=dlswMIB, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswCircuitTable=dlswCircuitTable, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnOperGroup=dlswTConnOperGroup, dlswSdlc=dlswSdlc, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTCPDomain=dlswTCPDomain, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirNBLocation=dlswDirNBLocation, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirMacIndex=dlswDirMacIndex, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, null=null, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswDirStat=dlswDirStat, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCompliances=dlswCompliances, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitStateTime=dlswCircuitStateTime, dlswNodeVersionString=dlswNodeVersionString, dlswTrapTConnUp=dlswTrapTConnUp, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirCache=dlswDirCache, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirLocateNBName=dlswDirLocateNBName, PYSNMP_MODULE_ID=dlsw, dlswDirMacEntries=dlswDirMacEntries, dlswSdlcLsTable=dlswSdlcLsTable, dlswSdlcGroup=dlswSdlcGroup, dlswTrapControl=dlswTrapControl, dlswDomains=dlswDomains, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswDirNBNameType=dlswDirNBNameType, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswIfRowStatus=dlswIfRowStatus, dlswDirNBName=dlswDirNBName, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlsw=dlsw, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnStat=dlswTConnStat, dlswCircuitStat=dlswCircuitStat, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, DlcType=DlcType, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirCompliance=dlswDirCompliance, dlswCircuitGroup=dlswCircuitGroup, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirectory=dlswDirectory, dlswTConnSpecific=dlswTConnSpecific)
