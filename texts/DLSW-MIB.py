...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswIfSapList=dlswIfSapList, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirMacIndex=dlswDirMacIndex, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswCircuitGroup=dlswCircuitGroup, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswNotificationGroup=dlswNotificationGroup, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, NBName=NBName, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirMacEntries=dlswDirMacEntries, dlswDirNBEntries=dlswDirNBEntries, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswNodeVersion=dlswNodeVersion, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswCircuitS2Location=dlswCircuitS2Location, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnSpecific=dlswTConnSpecific, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswNodeStatus=dlswNodeStatus, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswInterface=dlswInterface, dlswDirNBEntry=dlswDirNBEntry, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitTable=dlswCircuitTable, dlswConformance=dlswConformance, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswCircuitState=dlswCircuitState, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswDirNBGroup=dlswDirNBGroup, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirMacLFSize=dlswDirMacLFSize, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswDirMacTable=dlswDirMacTable, dlswTConnTcp=dlswTConnTcp, dlswDirNBTable=dlswDirNBTable, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperTable=dlswTConnOperTable, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitS1Sap=dlswCircuitS1Sap, DlswTCPAddress=DlswTCPAddress, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswDirMacMask=dlswDirMacMask, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswNodeUpTime=dlswNodeUpTime, dlswGroups=dlswGroups, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirMacEntry=dlswDirMacEntry, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTrapControl=dlswTrapControl, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTCPDomain=dlswTCPDomain, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDomains=dlswDomains, dlswIfTable=dlswIfTable, dlswDirNBLocation=dlswDirNBLocation, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirMacMac=dlswDirMacMac, dlswTConnStat=dlswTConnStat, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, LFSize=LFSize, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswDirNBName=dlswDirNBName, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnTcpOperTable=dlswTConnTcpOperTable, null=null, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswCircuitEntry=dlswCircuitEntry, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirNBNameType=dlswDirNBNameType, dlswDirGroup=dlswDirGroup, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConn=dlswTConn, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTrapTConnUp=dlswTrapTConnUp, EndStationLocation=EndStationLocation, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswSdlc=dlswSdlc, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitStat=dlswCircuitStat, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswDirCache=dlswDirCache, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperState=dlswTConnOperState, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswDirStat=dlswDirStat, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswCompliances=dlswCompliances, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlsw=dlsw, dlswInterfaceGroup=dlswInterfaceGroup, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswNodeGroup=dlswNodeGroup, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswNode=dlswNode, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnConfigIndex=dlswTConnConfigIndex, MacAddressNC=MacAddressNC, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswNodeNBGroup=dlswNodeNBGroup, dlswTraps=dlswTraps, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, TAddress=TAddress, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirectory=dlswDirectory, DlcType=DlcType, dlswIfRowStatus=dlswIfRowStatus, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswMIB=dlswMIB, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswCircuit=dlswCircuit, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswIfEntry=dlswIfEntry, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnStatGroup=dlswTConnStatGroup, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirCompliance=dlswDirCompliance, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, PYSNMP_MODULE_ID=dlsw, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsTable=dlswSdlcLsTable, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperGroup=dlswTConnOperGroup, dlswDirLocate=dlswDirLocate, dlswDirMacStatus=dlswDirMacStatus, dlswDirNBIndex=dlswDirNBIndex, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswTConnConfigEntryType=dlswTConnConfigEntryType)
