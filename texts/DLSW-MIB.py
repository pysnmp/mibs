...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirMacLFSize=dlswDirMacLFSize, MacAddressNC=MacAddressNC, dlswDirNBEntries=dlswDirNBEntries, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirNBEntryType=dlswDirNBEntryType, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCircuit=dlswCircuit, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTConnStat=dlswTConnStat, dlswNotificationGroup=dlswNotificationGroup, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswInterface=dlswInterface, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnOperGroup=dlswTConnOperGroup, dlswNodeUpTime=dlswNodeUpTime, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacLocation=dlswDirMacLocation, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirNBTable=dlswDirNBTable, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirStat=dlswDirStat, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlsw=dlsw, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirGroup=dlswDirGroup, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswNode=dlswNode, dlswDirNBEntry=dlswDirNBEntry, EndStationLocation=EndStationLocation, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirMacIndex=dlswDirMacIndex, dlswTConnStatGroup=dlswTConnStatGroup, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperState=dlswTConnOperState, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacStatus=dlswDirMacStatus, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCoreCompliance=dlswCoreCompliance, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswNodeStatus=dlswNodeStatus, dlswTConnConfigTable=dlswTConnConfigTable, dlswTrapControl=dlswTrapControl, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswDirMacEntries=dlswDirMacEntries, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswSdlcGroup=dlswSdlcGroup, DlswTCPAddress=DlswTCPAddress, dlswDirMacEntryType=dlswDirMacEntryType, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswSdlcLsTable=dlswSdlcLsTable, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTCPDomain=dlswTCPDomain, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswCircuitGroup=dlswCircuitGroup, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswIfSapList=dlswIfSapList, dlswCircuitState=dlswCircuitState, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirNBStatus=dlswDirNBStatus, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirLocate=dlswDirLocate, dlswTConnOperEntry=dlswTConnOperEntry, dlswDirectory=dlswDirectory, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperTable=dlswTConnOperTable, dlswTConnSpecific=dlswTConnSpecific, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswIfRowStatus=dlswIfRowStatus, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnOperDiscReason=dlswTConnOperDiscReason, LFSize=LFSize, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitStat=dlswCircuitStat, DlcType=DlcType, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswIfEntry=dlswIfEntry, dlswCircuitOrigin=dlswCircuitOrigin, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirNBLocation=dlswDirNBLocation, dlswDirLocateGroup=dlswDirLocateGroup, dlswNodeVersion=dlswNodeVersion, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirLocateNBName=dlswDirLocateNBName, dlswNodeGroup=dlswNodeGroup, dlswCircuitStateTime=dlswCircuitStateTime, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirMacMask=dlswDirMacMask, null=null, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswIfTable=dlswIfTable, dlswSdlc=dlswSdlc, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswConformance=dlswConformance, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConn=dlswTConn, dlswTConnTcp=dlswTConnTcp, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswDirCompliance=dlswDirCompliance, dlswGroups=dlswGroups, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirNBIndex=dlswDirNBIndex, dlswDirMacTable=dlswDirMacTable, dlswDirNBLFSize=dlswDirNBLFSize, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDomains=dlswDomains, dlswDirNBName=dlswDirNBName, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswCompliances=dlswCompliances, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnOperDiscTime=dlswTConnOperDiscTime, PYSNMP_MODULE_ID=dlsw, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitTable=dlswCircuitTable, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTrapTConnDown=dlswTrapTConnDown, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswNodeVersionString=dlswNodeVersionString, dlswDirMacMac=dlswDirMacMac, dlswDirCache=dlswDirCache, TAddress=TAddress, dlswCircuitPriority=dlswCircuitPriority, dlswInterfaceGroup=dlswInterfaceGroup, NBName=NBName, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTraps=dlswTraps, dlswMIB=dlswMIB, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap)
