...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswCircuitS2Location=dlswCircuitS2Location, dlswIfSapList=dlswIfSapList, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitState=dlswCircuitState, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTraps=dlswTraps, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitStateTime=dlswCircuitStateTime, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirMacMac=dlswDirMacMac, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnStat=dlswTConnStat, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirNBTable=dlswDirNBTable, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitOrigin=dlswCircuitOrigin, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnOperState=dlswTConnOperState, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswDirNBName=dlswDirNBName, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswConformance=dlswConformance, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlsw=dlsw, LFSize=LFSize, dlswNodeStatus=dlswNodeStatus, dlswIfEntry=dlswIfEntry, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswDirLocateNBName=dlswDirLocateNBName, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswDirGroup=dlswDirGroup, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirMacEntries=dlswDirMacEntries, dlswIfTable=dlswIfTable, null=null, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirCompliance=dlswDirCompliance, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswInterface=dlswInterface, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswCircuitStat=dlswCircuitStat, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswSdlc=dlswSdlc, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitTable=dlswCircuitTable, dlswCircuitGroup=dlswCircuitGroup, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnTcp=dlswTConnTcp, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, NBName=NBName, dlswDirCache=dlswDirCache, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTrapCntlTConn=dlswTrapCntlTConn, DlswTCPAddress=DlswTCPAddress, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswNodeVersionString=dlswNodeVersionString, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuit=dlswCircuit, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirNBNameType=dlswDirNBNameType, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswCircuitPriority=dlswCircuitPriority, dlswDirMacMask=dlswDirMacMask, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswSdlcGroup=dlswSdlcGroup, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirMacStatus=dlswDirMacStatus, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswCircuitEntryTime=dlswCircuitEntryTime, EndStationLocation=EndStationLocation, dlswDirMacLocation=dlswDirMacLocation, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirNBLFSize=dlswDirNBLFSize, dlswDirMacIndex=dlswDirMacIndex, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirMacTable=dlswDirMacTable, PYSNMP_MODULE_ID=dlsw, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirNBIndex=dlswDirNBIndex, dlswDomains=dlswDomains, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnSpecific=dlswTConnSpecific, dlswDirMacLocationType=dlswDirMacLocationType, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnOperEntry=dlswTConnOperEntry, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirMacLFSize=dlswDirMacLFSize, dlswDirNBLocation=dlswDirNBLocation, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswNodeNBGroup=dlswNodeNBGroup, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirNBEntries=dlswDirNBEntries, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCompliances=dlswCompliances, dlswNodeGroup=dlswNodeGroup, dlswTConnOperEntryTime=dlswTConnOperEntryTime, DlcType=DlcType, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirMacEntry=dlswDirMacEntry, dlswMIB=dlswMIB, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswIfRowStatus=dlswIfRowStatus, dlswTCPDomain=dlswTCPDomain, dlswDirStat=dlswDirStat, TAddress=TAddress, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnNBGroup=dlswTConnNBGroup, MacAddressNC=MacAddressNC, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTrapControl=dlswTrapControl, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCoreCompliance=dlswCoreCompliance, dlswTConnOperGroup=dlswTConnOperGroup, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConn=dlswTConn, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirLocate=dlswDirLocate, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirectory=dlswDirectory, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswNode=dlswNode, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnStatGroup=dlswTConnStatGroup, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswGroups=dlswGroups, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts)
