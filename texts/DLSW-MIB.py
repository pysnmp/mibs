...
mibBuilder.exportSymbols("DLSW-MIB", dlswConformance=dlswConformance, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirCache=dlswDirCache, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswTConnTcp=dlswTConnTcp, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTraps=dlswTraps, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, null=null, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnNBGroup=dlswTConnNBGroup, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDomains=dlswDomains, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, PYSNMP_MODULE_ID=dlsw, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswCoreCompliance=dlswCoreCompliance, dlswTrapControl=dlswTrapControl, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, EndStationLocation=EndStationLocation, dlswSdlc=dlswSdlc, dlswDirNBLocationType=dlswDirNBLocationType, dlswDirNBLocation=dlswDirNBLocation, dlswNodeVendorID=dlswNodeVendorID, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswNode=dlswNode, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirMacEntry=dlswDirMacEntry, dlswDirNBNameType=dlswDirNBNameType, dlswDirMacTable=dlswDirMacTable, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirMacStatus=dlswDirMacStatus, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswNodeStatus=dlswNodeStatus, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirNBIndex=dlswDirNBIndex, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTrapTConnDown=dlswTrapTConnDown, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTConnOperState=dlswTConnOperState, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswCircuit=dlswCircuit, dlswDirMacIndex=dlswDirMacIndex, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTrapTConnUp=dlswTrapTConnUp, DlswTCPAddress=DlswTCPAddress, dlswTConnStat=dlswTConnStat, dlswDirLocate=dlswDirLocate, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswDirLocateMacMatch=dlswDirLocateMacMatch, MacAddressNC=MacAddressNC, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswGroups=dlswGroups, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCircuitGroup=dlswCircuitGroup, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswCircuitS2Location=dlswCircuitS2Location, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirNBEntries=dlswDirNBEntries, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuitStateTime=dlswCircuitStateTime, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirNBEntryType=dlswDirNBEntryType, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswDirStat=dlswDirStat, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswNodeUpTime=dlswNodeUpTime, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitPriority=dlswCircuitPriority, dlswDirMacMac=dlswDirMacMac, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirMacLocationType=dlswDirMacLocationType, dlswDirMacEntryType=dlswDirMacEntryType, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperTable=dlswTConnOperTable, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswDirMacEntries=dlswDirMacEntries, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitStat=dlswCircuitStat, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirLocateNBName=dlswDirLocateNBName, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswMIB=dlswMIB, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswIfRowStatus=dlswIfRowStatus, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnSpecific=dlswTConnSpecific, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswDirectory=dlswDirectory, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTConnOperEntry=dlswTConnOperEntry, dlswIfTable=dlswIfTable, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswSdlcGroup=dlswSdlcGroup, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTCPDomain=dlswTCPDomain, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirLocateMacEntry=dlswDirLocateMacEntry, TAddress=TAddress, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswCircuitTable=dlswCircuitTable, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirMacMask=dlswDirMacMask, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirCompliance=dlswDirCompliance, dlswCompliances=dlswCompliances, dlswNodeVersion=dlswNodeVersion, dlswDirGroup=dlswDirGroup, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswSdlcLsTable=dlswSdlcLsTable, dlswTrapCntlCircuit=dlswTrapCntlCircuit, DlcType=DlcType, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirNBName=dlswDirNBName, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitState=dlswCircuitState, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, LFSize=LFSize, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswCircuitEntry=dlswCircuitEntry, dlswNodeGroup=dlswNodeGroup, dlswDirNBStatus=dlswDirNBStatus, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConn=dlswTConn, dlswNotificationGroup=dlswNotificationGroup, dlswInterface=dlswInterface, dlswDirNBTable=dlswDirNBTable, dlsw=dlsw, dlswIfEntry=dlswIfEntry, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswIfSapList=dlswIfSapList, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, NBName=NBName, dlswTConnOperGroup=dlswTConnOperGroup, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections)
