...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitEntry=dlswCircuitEntry, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, DlswTCPAddress=DlswTCPAddress, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnOperGroup=dlswTConnOperGroup, dlswNodeVendorID=dlswNodeVendorID, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, null=null, dlswInterface=dlswInterface, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswNodeStatus=dlswNodeStatus, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirMacEntryType=dlswDirMacEntryType, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswIfTable=dlswIfTable, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitTable=dlswCircuitTable, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswDirNBGroup=dlswDirNBGroup, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswCircuitState=dlswCircuitState, dlswDirMacEntries=dlswDirMacEntries, MacAddressNC=MacAddressNC, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirNBName=dlswDirNBName, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirGroup=dlswDirGroup, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitPriority=dlswCircuitPriority, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswDirLocateGroup=dlswDirLocateGroup, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswTConn=dlswTConn, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswDirCache=dlswDirCache, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswCoreCompliance=dlswCoreCompliance, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswTConnOperEntry=dlswTConnOperEntry, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, LFSize=LFSize, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswTConnConfigTable=dlswTConnConfigTable, DlcType=DlcType, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswDirMacTable=dlswDirMacTable, dlswTCPDomain=dlswTCPDomain, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirMacMac=dlswDirMacMac, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswDirMacMask=dlswDirMacMask, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswNodeUpTime=dlswNodeUpTime, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswNodeGroup=dlswNodeGroup, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswNodeVersionString=dlswNodeVersionString, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDomains=dlswDomains, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirStat=dlswDirStat, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswIfSapList=dlswIfSapList, dlswTrapControl=dlswTrapControl, dlswDirMacStatus=dlswDirMacStatus, dlswNodeVersion=dlswNodeVersion, PYSNMP_MODULE_ID=dlsw, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, TAddress=TAddress, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswGroups=dlswGroups, dlswMIB=dlswMIB, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirNBNameType=dlswDirNBNameType, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, NBName=NBName, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTrapTConnUp=dlswTrapTConnUp, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswDirMacEntry=dlswDirMacEntry, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswCircuitStat=dlswCircuitStat, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswNodeNBGroup=dlswNodeNBGroup, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswDirNBEntries=dlswDirNBEntries, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswCircuit=dlswCircuit, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirLocate=dlswDirLocate, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswCircuitGroup=dlswCircuitGroup, dlsw=dlsw, dlswDirNBStatus=dlswDirNBStatus, dlswDirMacIndex=dlswDirMacIndex, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswDirectory=dlswDirectory, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConnOperTable=dlswTConnOperTable, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswCompliances=dlswCompliances, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirMacLocation=dlswDirMacLocation, dlswTConnStat=dlswTConnStat, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswTConnTcp=dlswTConnTcp, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswDirNBTable=dlswDirNBTable, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswIfEntry=dlswIfEntry, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswDirNBLocation=dlswDirNBLocation, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswCircuitStatActives=dlswCircuitStatActives, dlswNode=dlswNode, dlswTConnOperState=dlswTConnOperState, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, EndStationLocation=EndStationLocation, dlswDirCompliance=dlswDirCompliance, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTraps=dlswTraps, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswSdlc=dlswSdlc, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswTConnSpecific=dlswTConnSpecific, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswNotificationGroup=dlswNotificationGroup, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirNBLFSize=dlswDirNBLFSize, dlswConformance=dlswConformance, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr)
