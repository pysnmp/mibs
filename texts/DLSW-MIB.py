...
mibBuilder.exportSymbols("DLSW-MIB", dlswCircuitPriority=dlswCircuitPriority, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirNBLFSize=dlswDirNBLFSize, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswDirLocate=dlswDirLocate, DlcType=DlcType, dlswSdlc=dlswSdlc, dlswDirMacStatus=dlswDirMacStatus, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswTConnOperDiscReason=dlswTConnOperDiscReason, PYSNMP_MODULE_ID=dlsw, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswCircuitStatActives=dlswCircuitStatActives, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTConnConfigSetupType=dlswTConnConfigSetupType, NBName=NBName, dlswDirMacIndex=dlswDirMacIndex, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswDirNBEntry=dlswDirNBEntry, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTrapControl=dlswTrapControl, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswInterface=dlswInterface, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswDirMacMask=dlswDirMacMask, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswDirectory=dlswDirectory, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswGroups=dlswGroups, dlsw=dlsw, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswDirMacMac=dlswDirMacMac, dlswCompliances=dlswCompliances, dlswMIB=dlswMIB, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswNodeVendorID=dlswNodeVendorID, dlswDirNBEntryType=dlswDirNBEntryType, dlswDirNBLocation=dlswDirNBLocation, dlswNode=dlswNode, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswIfSapList=dlswIfSapList, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswCircuitTable=dlswCircuitTable, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswCircuitGroup=dlswCircuitGroup, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswCoreCompliance=dlswCoreCompliance, dlswDirGroup=dlswDirGroup, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswDirNBName=dlswDirNBName, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperGroup=dlswTConnOperGroup, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswTraps=dlswTraps, dlswDirMacEntryType=dlswDirMacEntryType, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, null=null, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswNotificationGroup=dlswNotificationGroup, dlswTConnOperState=dlswTConnOperState, dlswDirStat=dlswDirStat, dlswNodeVersion=dlswNodeVersion, dlswDirNBRowStatus=dlswDirNBRowStatus, LFSize=LFSize, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswDirCompliance=dlswDirCompliance, dlswDirNBLocationType=dlswDirNBLocationType, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnSpecific=dlswTConnSpecific, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswNodeVersionString=dlswNodeVersionString, dlswTConnStat=dlswTConnStat, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConn=dlswTConn, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswDirLocateNBLocation=dlswDirLocateNBLocation, MacAddressNC=MacAddressNC, EndStationLocation=EndStationLocation, dlswNodeUpTime=dlswNodeUpTime, dlswDomains=dlswDomains, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswDirNBNameType=dlswDirNBNameType, dlswCircuitState=dlswCircuitState, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswDirLocateGroup=dlswDirLocateGroup, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswIfRowStatus=dlswIfRowStatus, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswCircuitOrigin=dlswCircuitOrigin, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, TAddress=TAddress, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, DlswTCPAddress=DlswTCPAddress, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswDirNBTable=dlswDirNBTable, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirNBEntries=dlswDirNBEntries, dlswCircuitEntry=dlswCircuitEntry, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswTConnConfigTable=dlswTConnConfigTable, dlswIfEntry=dlswIfEntry, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswDirMacLFSize=dlswDirMacLFSize, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswTConnTcp=dlswTConnTcp, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswIfTable=dlswIfTable, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswNodeGroup=dlswNodeGroup, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacLocation=dlswDirMacLocation, dlswCircuitStateTime=dlswCircuitStateTime, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnNBGroup=dlswTConnNBGroup, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTCPDomain=dlswTCPDomain, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswDirCache=dlswDirCache, dlswSdlcLsTable=dlswSdlcLsTable, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperTable=dlswTConnOperTable, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswCircuit=dlswCircuit, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperEntry=dlswTConnOperEntry, dlswNodeStatus=dlswNodeStatus, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswSdlcGroup=dlswSdlcGroup, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswConformance=dlswConformance, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswDirNBGroup=dlswDirNBGroup, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitStat=dlswCircuitStat, dlswDirMacTable=dlswDirMacTable, dlswDirNBIndex=dlswDirNBIndex, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswDirMacEntries=dlswDirMacEntries)
