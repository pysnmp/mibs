...
mibBuilder.exportSymbols("DLSW-MIB", dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, dlswCircuitS2Location=dlswCircuitS2Location, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, PYSNMP_MODULE_ID=dlsw, dlswDirMacTable=dlswDirMacTable, dlswTConnOperGroup=dlswTConnOperGroup, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswCircuitGroup=dlswCircuitGroup, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswDirMacEntries=dlswDirMacEntries, dlswTrapControl=dlswTrapControl, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirLocate=dlswDirLocate, dlswDirMacMac=dlswDirMacMac, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswNodeVersion=dlswNodeVersion, dlswDirStat=dlswDirStat, DlswTCPAddress=DlswTCPAddress, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnOperTable=dlswTConnOperTable, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswCoreCompliance=dlswCoreCompliance, dlswDirNBGroup=dlswDirNBGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswSdlcLsTable=dlswSdlcLsTable, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswNodeUpTime=dlswNodeUpTime, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswCircuitTable=dlswCircuitTable, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswDirMacEntryType=dlswDirMacEntryType, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnNBGroup=dlswTConnNBGroup, dlswDirGroup=dlswDirGroup, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry, dlswDirMacIndex=dlswDirMacIndex, dlswDirLocateGroup=dlswDirLocateGroup, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswDirNBLocation=dlswDirNBLocation, dlswDirLocateNBName=dlswDirLocateNBName, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswNodeVendorID=dlswNodeVendorID, dlswIfTable=dlswIfTable, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirMacCacheMisses=dlswDirMacCacheMisses, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswIfSapList=dlswIfSapList, dlswDomains=dlswDomains, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnStat=dlswTConnStat, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitStat=dlswCircuitStat, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitEntry=dlswCircuitEntry, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlswNodeStatus=dlswNodeStatus, dlswDirCache=dlswDirCache, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTraps=dlswTraps, dlswCircuitEntryTime=dlswCircuitEntryTime, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, EndStationLocation=EndStationLocation, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, LFSize=LFSize, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTrapTConnUp=dlswTrapTConnUp, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswDirLocateMacLocation=dlswDirLocateMacLocation, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswInterfaceGroup=dlswInterfaceGroup, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswTCPDomain=dlswTCPDomain, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswTConn=dlswTConn, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirNBEntry=dlswDirNBEntry, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, MacAddressNC=MacAddressNC, dlswTConnOperState=dlswTConnOperState, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswCircuit=dlswCircuit, dlswDirLocateNBLocation=dlswDirLocateNBLocation, TAddress=TAddress, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswSdlcGroup=dlswSdlcGroup, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, dlswIfEntry=dlswIfEntry, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswDirNBIndex=dlswDirNBIndex, dlswDirNBName=dlswDirNBName, dlswConformance=dlswConformance, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswTConnTcp=dlswTConnTcp, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswDirCompliance=dlswDirCompliance, dlswSdlcCompliance=dlswSdlcCompliance, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnStatGroup=dlswTConnStatGroup, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswDirectory=dlswDirectory, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, dlswSdlc=dlswSdlc, dlswDirNBLocationType=dlswDirNBLocationType, dlswCompliances=dlswCompliances, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCircuitState=dlswCircuitState, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnSpecific=dlswTConnSpecific, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswNodeVersionString=dlswNodeVersionString, dlswDirNBCacheHits=dlswDirNBCacheHits, dlsw=dlsw, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswDirNBNameType=dlswDirNBNameType, dlswDirMacMask=dlswDirMacMask, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswMIB=dlswMIB, dlswDirNBTable=dlswDirNBTable, dlswCircuitPriority=dlswCircuitPriority, dlswTConnConfigOpens=dlswTConnConfigOpens, NBName=NBName, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, null=null, dlswNodeGroup=dlswNodeGroup, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswTConnConfigTable=dlswTConnConfigTable, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswDirMacEntry=dlswDirMacEntry, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswDirMacRowStatus=dlswDirMacRowStatus, dlswInterface=dlswInterface, dlswNode=dlswNode, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswGroups=dlswGroups, dlswNotificationGroup=dlswNotificationGroup, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, DlcType=DlcType, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswDirNBStatus=dlswDirNBStatus, dlswTConnOperEntry=dlswTConnOperEntry, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswDirMacLocationType=dlswDirMacLocationType, dlswCircuitOrigin=dlswCircuitOrigin, dlswDirNBEntryType=dlswDirNBEntryType, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswIfRowStatus=dlswIfRowStatus, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswCircuitStatActives=dlswCircuitStatActives, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswDirNBEntries=dlswDirNBEntries, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswTrapTConnDown=dlswTrapTConnDown, dlswDirMacStatus=dlswDirMacStatus)
