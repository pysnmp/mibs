...
mibBuilder.exportSymbols("DLSW-MIB", dlswDirNBNameType=dlswDirNBNameType, dlswDirNBGroup=dlswDirNBGroup, dlswTConnConfigLocalTAddr=dlswTConnConfigLocalTAddr, dlswDirLocateNBGroup=dlswDirLocateNBGroup, dlswDirNBCacheNextIndex=dlswDirNBCacheNextIndex, dlswDirNBLocationType=dlswDirNBLocationType, dlswCircuitS1CircuitId=dlswCircuitS1CircuitId, dlswSdlcLsLocalIdBlock=dlswSdlcLsLocalIdBlock, dlswTConnConfigSapList=dlswTConnConfigSapList, dlswTConnOperICRexSents=dlswTConnOperICRexSents, dlswGroups=dlswGroups, dlswDirMacTable=dlswDirMacTable, dlswDirCompliance=dlswDirCompliance, dlswTraps=dlswTraps, dlswCircuitStateTime=dlswCircuitStateTime, dlswCircuitS1DlcType=dlswCircuitS1DlcType, dlswDirMacIndex=dlswDirMacIndex, dlswTConnOperDiscReason=dlswTConnOperDiscReason, dlswDirNBCacheHits=dlswDirNBCacheHits, dlswCircuitFCLargestSendGranted=dlswCircuitFCLargestSendGranted, dlswTrapTConnProtViolation=dlswTrapTConnProtViolation, dlswDirLocateCompliance=dlswDirLocateCompliance, dlswCoreCompliance=dlswCoreCompliance, dlswIfRowStatus=dlswIfRowStatus, dlswCircuitStatActives=dlswCircuitStatActives, dlswDomains=dlswDomains, dlswTConnOperEntry=dlswTConnOperEntry, dlswNodeVersionString=dlswNodeVersionString, dlswTConnOperPartnerNBExcl=dlswTConnOperPartnerNBExcl, dlswDirMacEntry=dlswDirMacEntry, dlswTConnOperPartnerVersion=dlswTConnOperPartnerVersion, dlswTConnTcpCompliance=dlswTConnTcpCompliance, dlswSdlcLsRowStatus=dlswSdlcLsRowStatus, dlswNodeUpTime=dlswNodeUpTime, dlswDirLocateNBMatch=dlswDirLocateNBMatch, dlswTConnNBGroup=dlswTConnNBGroup, dlswCircuitFCLargestRecvGranted=dlswCircuitFCLargestRecvGranted, dlswCompliances=dlswCompliances, dlswDirLocateNBTable=dlswDirLocateNBTable, dlswDirLocateNBLocation=dlswDirLocateNBLocation, dlswTConnConfigLastModifyTime=dlswTConnConfigLastModifyTime, dlswSdlcLsEntries=dlswSdlcLsEntries, dlswTrapCntlTConnProtViolation=dlswTrapCntlTConnProtViolation, dlswNodeResourceNBExclusivity=dlswNodeResourceNBExclusivity, dlswTConnTcpConfigTcpConnections=dlswTConnTcpConfigTcpConnections, dlswIfTable=dlswIfTable, dlswDirNBIndex=dlswDirNBIndex, dlswTConnTcpConfigKeepAliveInt=dlswTConnTcpConfigKeepAliveInt, dlswTConnOperPartnerMacInfo=dlswTConnOperPartnerMacInfo, dlswTConnTcpOperTable=dlswTConnTcpOperTable, dlswSdlcCompliance=dlswSdlcCompliance, dlswDirNBTable=dlswDirNBTable, dlswTConnConfigGroupDefinition=dlswTConnConfigGroupDefinition, dlswCircuitS2TAddress=dlswCircuitS2TAddress, dlswNodeVersion=dlswNodeVersion, dlswTConnConfigTDomain=dlswTConnConfigTDomain, dlswTConnOperInCntlPkts=dlswTConnOperInCntlPkts, dlswCircuitFCRecvGrantedUnits=dlswCircuitFCRecvGrantedUnits, dlswDirLocateMacMac=dlswDirLocateMacMac, dlswTConnOperDiscActiveCir=dlswTConnOperDiscActiveCir, dlswSdlcLsRemoteMac=dlswSdlcLsRemoteMac, dlswTConnOperCirCreates=dlswTConnOperCirCreates, dlswSdlcGroup=dlswSdlcGroup, dlswTConnTcpConfigEntry=dlswTConnTcpConfigEntry, dlswTConnConfigEntryType=dlswTConnConfigEntryType, dlswCircuitFCSendGrantedUnits=dlswCircuitFCSendGrantedUnits, dlswTrapCntlCircuit=dlswTrapCntlCircuit, dlswTConnStat=dlswTConnStat, dlswNodeVendorID=dlswNodeVendorID, dlswTConnOperEntryTime=dlswTConnOperEntryTime, dlswTConnConfigAdvertiseMacNB=dlswTConnConfigAdvertiseMacNB, dlswTConnOperCURexRcvds=dlswTConnOperCURexRcvds, dlswTConnTcpConfigTable=dlswTConnTcpConfigTable, dlsw=dlsw, dlswIfEntry=dlswIfEntry, dlswDirLocate=dlswDirLocate, dlswDirLocateNBName=dlswDirLocateNBName, dlswDirMacMask=dlswDirMacMask, dlswTConnOperState=dlswTConnOperState, dlswCircuitDiscReasonRemote=dlswCircuitDiscReasonRemote, dlswCircuitStatGroup=dlswCircuitStatGroup, dlswTConnConfigOpens=dlswTConnConfigOpens, dlswTConnOperGroup=dlswTConnOperGroup, dlswCircuitS1IfIndex=dlswCircuitS1IfIndex, PYSNMP_MODULE_ID=dlsw, dlswTConnOperFlowCntlMode=dlswTConnOperFlowCntlMode, dlswCircuitStat=dlswCircuitStat, dlswCircuitDiscReasonLocal=dlswCircuitDiscReasonLocal, dlswCircuitFCHalveWndwRcvds=dlswCircuitFCHalveWndwRcvds, dlswTCPDomain=dlswTCPDomain, dlswTConnOperCircuits=dlswTConnOperCircuits, dlswTConnOperPartnerVersionStr=dlswTConnOperPartnerVersionStr, dlswTConnStatActiveConnections=dlswTConnStatActiveConnections, dlswTConnOperTDomain=dlswTConnOperTDomain, dlswTConnConfigRowStatus=dlswTConnConfigRowStatus, dlswInterfaceGroup=dlswInterfaceGroup, dlswMIB=dlswMIB, dlswTConnTcpOperGroup=dlswTConnTcpOperGroup, dlswTConnConfigTable=dlswTConnConfigTable, dlswCircuitFCResetOpSents=dlswCircuitFCResetOpSents, dlswCircuitFCResetOpRcvds=dlswCircuitFCResetOpRcvds, dlswTConn=dlswTConn, dlswTConnOperNRexRcvds=dlswTConnOperNRexRcvds, dlswDirMacStatus=dlswDirMacStatus, dlswDirMacRowStatus=dlswDirMacRowStatus, MacAddressNC=MacAddressNC, dlswDirLocateMacMatch=dlswDirLocateMacMatch, dlswTConnOperOutDataPkts=dlswTConnOperOutDataPkts, dlswTConnSpecific=dlswTConnSpecific, dlswTConnTcpOperTcpConnections=dlswTConnTcpOperTcpConnections, dlswTConnConfigInitCirRecvWndw=dlswTConnConfigInitCirRecvWndw, dlswTConnOperOutDataOctets=dlswTConnOperOutDataOctets, dlswDirMacCacheNextIndex=dlswDirMacCacheNextIndex, dlswSdlcLsRemoteSap=dlswSdlcLsRemoteSap, dlswDirNBEntry=dlswDirNBEntry, dlswDirMacLocationType=dlswDirMacLocationType, dlswTConnOperRemoteTAddr=dlswTConnOperRemoteTAddr, dlswTConnOperConfigIndex=dlswTConnOperConfigIndex, dlswDirNBRowStatus=dlswDirNBRowStatus, dlswDirMacLFSize=dlswDirMacLFSize, dlswCircuit=dlswCircuit, dlswDirGroup=dlswDirGroup, dlswDirNBEntryType=dlswDirNBEntryType, dlswConformance=dlswConformance, dlswTConnTcpConfigMaxSegmentSize=dlswTConnTcpConfigMaxSegmentSize, dlswSdlcLsEntry=dlswSdlcLsEntry, dlswDirLocateMacLocation=dlswDirLocateMacLocation, DlswTCPAddress=DlswTCPAddress, dlswDirMacCacheHits=dlswDirMacCacheHits, dlswCircuitDiscReasonRemoteData=dlswCircuitDiscReasonRemoteData, dlswCircuitS1Sap=dlswCircuitS1Sap, dlswCircuitPriority=dlswCircuitPriority, dlswDirNBName=dlswDirNBName, dlswNodeGroup=dlswNodeGroup, dlswTConnOperInDataPkts=dlswTConnOperInDataPkts, dlswTConnOperInDataOctets=dlswTConnOperInDataOctets, dlswTConnOperCURexSents=dlswTConnOperCURexSents, dlswDirStat=dlswDirStat, dlswTConnStatGroup=dlswTConnStatGroup, dlswDirMacCacheMisses=dlswDirMacCacheMisses, LFSize=LFSize, dlswNode=dlswNode, dlswCircuitGroup=dlswCircuitGroup, dlswDirMacEntryType=dlswDirMacEntryType, dlswCircuitS1Dlc=dlswCircuitS1Dlc, dlswNodeVirtualSegmentLFSize=dlswNodeVirtualSegmentLFSize, dlswDirNBCacheMisses=dlswDirNBCacheMisses, dlswCircuitStatCreates=dlswCircuitStatCreates, dlswTConnTcpOperKeepAliveInt=dlswTConnTcpOperKeepAliveInt, dlswCircuitEntry=dlswCircuitEntry, dlswTConnOperPartnerInitPacingWndw=dlswTConnOperPartnerInitPacingWndw, dlswDirNBStatus=dlswDirNBStatus, dlswCircuitS2Location=dlswCircuitS2Location, dlswTConnConfigIndex=dlswTConnConfigIndex, dlswTConnOperLocalTAddr=dlswTConnOperLocalTAddr, dlswTConnOperICRexRcvds=dlswTConnOperICRexRcvds, dlswIfSapList=dlswIfSapList, dlswTConnConfigRemoteTAddr=dlswTConnConfigRemoteTAddr, dlswCircuitFCHalveWndwSents=dlswCircuitFCHalveWndwSents, dlswSdlcLsLocalIdNum=dlswSdlcLsLocalIdNum, dlswDirNBLocation=dlswDirNBLocation, dlswTrapControl=dlswTrapControl, dlswTrapTConnUp=dlswTrapTConnUp, NBName=NBName, dlswTConnOperPartnerSapList=dlswTConnOperPartnerSapList, dlswSdlcLsTable=dlswSdlcLsTable, dlswTConnTcp=dlswTConnTcp, dlswDirMacEntries=dlswDirMacEntries, dlswTrapTConnDown=dlswTrapTConnDown, dlswTConnConfigSetupType=dlswTConnConfigSetupType, dlswDirectory=dlswDirectory, dlswTConnTcpConfigGroup=dlswTConnTcpConfigGroup, dlswTConnTcpOperPrefTcpConnections=dlswTConnTcpOperPrefTcpConnections, dlswSdlcLsLocalMac=dlswSdlcLsLocalMac, dlswDirLocateGroup=dlswDirLocateGroup, dlswDirNBEntries=dlswDirNBEntries, dlswTrapCntlTConnPartnerReject=dlswTrapCntlTConnPartnerReject, dlswDirMacLocation=dlswDirMacLocation, dlswTConnOperPartnerMacExcl=dlswTConnOperPartnerMacExcl, dlswNodeStdPacingSupport=dlswNodeStdPacingSupport, dlswCircuitFCRecvCurrentWndw=dlswCircuitFCRecvCurrentWndw, EndStationLocation=EndStationLocation, dlswInterface=dlswInterface, dlswNodeNBGroup=dlswNodeNBGroup, dlswTConnOperNRexSents=dlswTConnOperNRexSents, dlswTConnStatCloseBusys=dlswTConnStatCloseBusys, dlswTConnConfigEntry=dlswTConnConfigEntry, dlswTrapCntlTConn=dlswTrapCntlTConn, dlswTConnOperDiscTime=dlswTConnOperDiscTime, dlswCircuitEntryTime=dlswCircuitEntryTime, null=null, dlswCircuitS1Mac=dlswCircuitS1Mac, dlswCircuitS2Sap=dlswCircuitS2Sap, dlswTConnStatCloseIdles=dlswTConnStatCloseIdles, dlswCircuitTable=dlswCircuitTable, dlswDirCache=dlswDirCache, dlswDirNBLFSize=dlswDirNBLFSize, dlswTConnConfigGroup=dlswTConnConfigGroup, dlswTConnOperNQexRcvds=dlswTConnOperNQexRcvds, dlswNodeStatus=dlswNodeStatus, dlswDirMacMac=dlswDirMacMac, dlswNodeResourceMacExclusivity=dlswNodeResourceMacExclusivity, DlcType=DlcType, dlswCircuitOrigin=dlswCircuitOrigin, dlswCircuitS2CircuitId=dlswCircuitS2CircuitId, dlswCircuitS1RouteInfo=dlswCircuitS1RouteInfo, dlswCircuitState=dlswCircuitState, dlswDirLocateMacTable=dlswDirLocateMacTable, dlswDirLocateNBEntry=dlswDirLocateNBEntry, dlswSdlcLsLocalSap=dlswSdlcLsLocalSap, dlswTConnOperTable=dlswTConnOperTable, dlswTrapTConnPartnerReject=dlswTrapTConnPartnerReject, dlswIfVirtualSegment=dlswIfVirtualSegment, dlswSdlc=dlswSdlc, dlswTConnOperPartnerNBInfo=dlswTConnOperPartnerNBInfo, dlswDirLocateMacEntry=dlswDirLocateMacEntry, dlswCircuitFCSendCurrentWndw=dlswCircuitFCSendCurrentWndw, dlswTrapCircuitDown=dlswTrapCircuitDown, dlswTConnOperOutCntlPkts=dlswTConnOperOutCntlPkts, TAddress=TAddress, dlswTConnOperConnectTime=dlswTConnOperConnectTime, dlswTConnOperPartnerVendorID=dlswTConnOperPartnerVendorID, dlswCircuitS2TDomain=dlswCircuitS2TDomain, dlswNotificationGroup=dlswNotificationGroup, dlswTrapCircuitUp=dlswTrapCircuitUp, dlswTConnOperNQexSents=dlswTConnOperNQexSents, dlswCircuitS2Mac=dlswCircuitS2Mac, dlswTConnTcpOperEntry=dlswTConnTcpOperEntry)
