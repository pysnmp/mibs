...
# Produced by pysmi-1.1.8 at Mon Jan 17 21:46:44 2022
...
ObjectIdentity, Integer32, MibIdentifier, Unsigned32, Counter32, Bits, Gauge32, Counter64, NotificationType, IpAddress, iso, enterprises, TimeTicks, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn = mibBuilder.importSymbols("SNMPv2-SMI", "ObjectIdentity", "Integer32", "MibIdentifier", "Unsigned32", "Counter32", "Bits", "Gauge32", "Counter64", "NotificationType", "IpAddress", "iso", "enterprises", "TimeTicks", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn")
...
mibBuilder.exportSymbols("DKSF-70-6-X-X-1", npIoSinglePulseStart=npIoSinglePulseStart, npRelayMode=npRelayMode, npIo=npIo, npIoTrapMemo=npIoTrapMemo, npRelHumSafeRangeHigh=npRelHumSafeRangeHigh, npIoLineN=npIoLineN, npIoTrapLevelLegend=npIoTrapLevelLegend, npForcedReboot=npForcedReboot, npRelHumTrapTempAboveSafe=npRelHumTrapTempAboveSafe, npCurLoopPower=npCurLoopPower, npCurLoop=npCurLoop, npRelayTable=npRelayTable, npRelHumTrapTempFail=npRelHumTrapTempFail, npCurLoopV=npCurLoopV, npSmokeEntry=npSmokeEntry, npThermoTrapPrefix=npThermoTrapPrefix, npRelHumEntry=npRelHumEntry, npCurLoopEntry=npCurLoopEntry, npRelHumValue=npRelHumValue, npThermoTrapMemo=npThermoTrapMemo, npGsmRegistration=npGsmRegistration, npGsmStrength=npGsmStrength, npRelHumTrap=npRelHumTrap, npSmokeReset=npSmokeReset, npIoTable=npIoTable, npRelHumTrapTempAllEvents=npRelHumTrapTempAllEvents, npSmokeStatus=npSmokeStatus, npSmokeTraps=npSmokeTraps, npRelHumTrapSafe=npRelHumTrapSafe, npRelHumTrapBelowSafe=npRelHumTrapBelowSafe, npCurLoopTraps=npCurLoopTraps, npRelHumMemo=npRelHumMemo, npRelHumTrapTempAllChannels=npRelHumTrapTempAllChannels, npRelHumTrapFail=npRelHumTrapFail, npThermoTrap=npThermoTrap, npIrPlayCmd=npIrPlayCmd, npIoTrapLineN=npIoTrapLineN, npRelHumTempValue=npRelHumTempValue, npThermoTrapSensorN=npThermoTrapSensorN, npGsmTrapPrefix=npGsmTrapPrefix, npIrCtrl=npIrCtrl, npIrReset=npIrReset, npCurLoopN=npCurLoopN, npThermoHigh=npThermoHigh, uniPingServerSolutionV3=uniPingServerSolutionV3, npReboot=npReboot, npThermoMemo=npThermoMemo, npRelHumTrapAllChannels=npRelHumTrapAllChannels, npRelHumTrapTemp=npRelHumTrapTemp, npSmokeSensorN=npSmokeSensorN, npRelHumTrapAboveSafe=npRelHumTrapAboveSafe, npRelayMemo=npRelayMemo, npGsmTrap=npGsmTrap, npRelHumTrapTempSafe=npRelHumTrapTempSafe, PYSNMP_MODULE_ID=uniPingServerSolutionV3, npSmokeTrapStatus=npSmokeTrapStatus, npCurLoopTrapStatus=npCurLoopTrapStatus, lightcom=lightcom, npIrStatus=npIrStatus, npCurLoopTrapI=npCurLoopTrapI, npRelayStartReset=npRelayStartReset, npRelHumTempSafeRangeLow=npRelHumTempSafeRangeLow, npRelHumTempStatus=npRelHumTempStatus, npSmokeMemo=npSmokeMemo, npIoLevelOut=npIoLevelOut, npRelHumTempSafeRangeHigh=npRelHumTempSafeRangeHigh, npRelHumTrapData=npRelHumTrapData, npThermoStatus=npThermoStatus, npRelHumTrapDataSafeRangeHigh=npRelHumTrapDataSafeRangeHigh, npThermoEntry=npThermoEntry, npRelHumTrapDataN=npRelHumTrapDataN, npIoLevelIn=npIoLevelIn, npCurLoopTrapR=npCurLoopTrapR, npRelHumTable=npRelHumTable, npRelayEntry=npRelayEntry, npThermoTrapHigh=npThermoTrapHigh, npRelHumTrapDataSafeRangeLow=npRelHumTrapDataSafeRangeLow, npRelHumTrapDataStatus=npRelHumTrapDataStatus, FixedPoint1000=FixedPoint1000, npRelHumStatus=npRelHumStatus, npRelHumTrapAllEvents=npRelHumTrapAllEvents, npRelayN=npRelayN, npRelHumN=npRelHumN, npIoMemo=npIoMemo, npIoTrap=npIoTrap, npIoSinglePulseDuration=npIoSinglePulseDuration, npRelHumTrapDataValue=npRelHumTrapDataValue, npRelHumSafeRangeLow=npRelHumSafeRangeLow, npSmokeTable=npSmokeTable, npRelayState=npRelayState, npGsmFailed=npGsmFailed, npSmokeTrapMemo=npSmokeTrapMemo, npSmokeTrapPrefix=npSmokeTrapPrefix, npCurLoopR=npCurLoopR, npCurLoopTrapN=npCurLoopTrapN, npIoTrapLevelIn=npIoTrapLevelIn, npIr=npIr, npThermoTraps=npThermoTraps, npThermoTrapLow=npThermoTrapLow, npResetStack=npResetStack, npCurLoopStatus=npCurLoopStatus, npGsm=npGsm, npThermoLow=npThermoLow, npSmoke=npSmoke, npTrapEmailTo=npTrapEmailTo, npGsmInfo=npGsmInfo, npIoEntry=npIoEntry, npThermoValue=npThermoValue, npSmokeTrapSensorN=npSmokeTrapSensorN, npCurLoopTable=npCurLoopTable, npRelHumTrapTempBelowSafe=npRelHumTrapTempBelowSafe, npCurLoopTrapV=npCurLoopTrapV, npGsmTraps=npGsmTraps, npRelHumTrapDataMemo=npRelHumTrapDataMemo, npTrapInfo=npTrapInfo, npSmokeTrap=npSmokeTrap, npIoTrapPrefix=npIoTrapPrefix, npThermoSensorN=npThermoSensorN, npCurLoopTrapPower=npCurLoopTrapPower, npSoftReboot=npSoftReboot, npSmokePower=npSmokePower, npThermo=npThermo, npThermoTable=npThermoTable, npCurLoopTrap=npCurLoopTrap, npIoPulseCounter=npIoPulseCounter, npThermoTrapValue=npThermoTrapValue, npCurLoopTrapPrefix=npCurLoopTrapPrefix, npThermoTrapStatus=npThermoTrapStatus, npCurLoopI=npCurLoopI, npGsmSendSmsUtf8=npGsmSendSmsUtf8, npIoTraps=npIoTraps, npRelHumidity=npRelHumidity, npThermoValuePrecise=npThermoValuePrecise, npRelay=npRelay, npGsmSendSmsWin1251=npGsmSendSmsWin1251)
