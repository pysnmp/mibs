...
# Produced by pysmi-1.1.3 at Wed Dec  1 17:05:16 2021
...
mibBuilder.exportSymbols("DKSF-70-6-X-X-1", npThermoSensorN=npThermoSensorN, npRelayEntry=npRelayEntry, npSmokeTrapPrefix=npSmokeTrapPrefix, npCurLoopV=npCurLoopV, npThermo=npThermo, npCurLoop=npCurLoop, npRelHumTrapTempAllChannels=npRelHumTrapTempAllChannels, uniPingServerSolutionV3=uniPingServerSolutionV3, npRelHumTrapAllEvents=npRelHumTrapAllEvents, npIoMemo=npIoMemo, npGsmSendSmsWin1251=npGsmSendSmsWin1251, npRelHumTrapDataMemo=npRelHumTrapDataMemo, npCurLoopTrapI=npCurLoopTrapI, npIoTrapLevelIn=npIoTrapLevelIn, npSmoke=npSmoke, npRelHumN=npRelHumN, npIoTrapPrefix=npIoTrapPrefix, npSmokeSensorN=npSmokeSensorN, npSmokeTraps=npSmokeTraps, npIoSinglePulseStart=npIoSinglePulseStart, FixedPoint1000=FixedPoint1000, npSmokeTrapSensorN=npSmokeTrapSensorN, npRelHumTrapTempSafe=npRelHumTrapTempSafe, npRelay=npRelay, npRelHumTrapDataStatus=npRelHumTrapDataStatus, npIoEntry=npIoEntry, npTrapEmailTo=npTrapEmailTo, npThermoLow=npThermoLow, npCurLoopTrapN=npCurLoopTrapN, npRelayState=npRelayState, npGsmSendSmsUtf8=npGsmSendSmsUtf8, npRelHumTrapDataSafeRangeLow=npRelHumTrapDataSafeRangeLow, npRelHumTrapDataValue=npRelHumTrapDataValue, npResetStack=npResetStack, npIoPulseCounter=npIoPulseCounter, npRelHumTable=npRelHumTable, npGsmTrap=npGsmTrap, npGsmTrapPrefix=npGsmTrapPrefix, npIrReset=npIrReset, npGsmTraps=npGsmTraps, npSmokePower=npSmokePower, npIrCtrl=npIrCtrl, npThermoEntry=npThermoEntry, npThermoTrapSensorN=npThermoTrapSensorN, npTrapInfo=npTrapInfo, npCurLoopN=npCurLoopN, npRelayMemo=npRelayMemo, npRelHumTrapSafe=npRelHumTrapSafe, npReboot=npReboot, npSoftReboot=npSoftReboot, npSmokeTrapMemo=npSmokeTrapMemo, npCurLoopTrapStatus=npCurLoopTrapStatus, npRelHumValue=npRelHumValue, npThermoMemo=npThermoMemo, npCurLoopTrapPrefix=npCurLoopTrapPrefix, npRelayTable=npRelayTable, npRelHumStatus=npRelHumStatus, npRelHumTrapDataN=npRelHumTrapDataN, npIoLineN=npIoLineN, npRelHumTrap=npRelHumTrap, npRelHumTrapAllChannels=npRelHumTrapAllChannels, npRelayMode=npRelayMode, npSmokeEntry=npSmokeEntry, npCurLoopPower=npCurLoopPower, npIoSinglePulseDuration=npIoSinglePulseDuration, npRelayN=npRelayN, npIo=npIo, npRelHumEntry=npRelHumEntry, npCurLoopR=npCurLoopR, npIoTrapLineN=npIoTrapLineN, npSmokeTable=npSmokeTable, npRelHumSafeRangeLow=npRelHumSafeRangeLow, npSmokeReset=npSmokeReset, npIoTrapLevelLegend=npIoTrapLevelLegend, npRelHumTrapTempBelowSafe=npRelHumTrapTempBelowSafe, npThermoTrapValue=npThermoTrapValue, npRelHumTrapFail=npRelHumTrapFail, lightcom=lightcom, npThermoTrap=npThermoTrap, npSmokeStatus=npSmokeStatus, npSmokeTrap=npSmokeTrap, npCurLoopTrap=npCurLoopTrap, npRelHumTrapDataSafeRangeHigh=npRelHumTrapDataSafeRangeHigh, npGsmStrength=npGsmStrength, npIoTraps=npIoTraps, npRelHumTempStatus=npRelHumTempStatus, npCurLoopStatus=npCurLoopStatus, npRelHumTrapBelowSafe=npRelHumTrapBelowSafe, npThermoHigh=npThermoHigh, npGsmInfo=npGsmInfo, npForcedReboot=npForcedReboot, npRelHumSafeRangeHigh=npRelHumSafeRangeHigh, npGsmFailed=npGsmFailed, npThermoStatus=npThermoStatus, PYSNMP_MODULE_ID=uniPingServerSolutionV3, npRelHumTrapTempFail=npRelHumTrapTempFail, npThermoTrapStatus=npThermoTrapStatus, npThermoValuePrecise=npThermoValuePrecise, npRelHumTempValue=npRelHumTempValue, npGsmRegistration=npGsmRegistration, npIoTrap=npIoTrap, npSmokeMemo=npSmokeMemo, npGsm=npGsm, npThermoTable=npThermoTable, npIr=npIr, npCurLoopTraps=npCurLoopTraps, npSmokeTrapStatus=npSmokeTrapStatus, npRelHumTrapTempAllEvents=npRelHumTrapTempAllEvents, npThermoTrapHigh=npThermoTrapHigh, npThermoTrapLow=npThermoTrapLow, npRelayStartReset=npRelayStartReset, npRelHumMemo=npRelHumMemo, npRelHumTrapTemp=npRelHumTrapTemp, npThermoTrapMemo=npThermoTrapMemo, npCurLoopTable=npCurLoopTable, npRelHumidity=npRelHumidity, npRelHumTrapAboveSafe=npRelHumTrapAboveSafe, npCurLoopI=npCurLoopI, npRelHumTrapTempAboveSafe=npRelHumTrapTempAboveSafe, npIoTrapMemo=npIoTrapMemo, npIoTable=npIoTable, npRelHumTrapData=npRelHumTrapData, npIrPlayCmd=npIrPlayCmd, npThermoTrapPrefix=npThermoTrapPrefix, npIrStatus=npIrStatus, npCurLoopTrapV=npCurLoopTrapV, npThermoTraps=npThermoTraps, npThermoValue=npThermoValue, npRelHumTempSafeRangeLow=npRelHumTempSafeRangeLow, npRelHumTempSafeRangeHigh=npRelHumTempSafeRangeHigh, npIoLevelIn=npIoLevelIn, npCurLoopTrapPower=npCurLoopTrapPower, npIoLevelOut=npIoLevelOut, npCurLoopEntry=npCurLoopEntry, npCurLoopTrapR=npCurLoopTrapR)
