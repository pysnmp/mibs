...
# Produced by pysmi-1.1.8 at Mon Sep 19 07:33:01 2022
...
mibBuilder.exportSymbols("DKSF-70-6-X-X-1", npIoTrap=npIoTrap, npIoSinglePulseDuration=npIoSinglePulseDuration, npSmokeSensorN=npSmokeSensorN, npRelHumTrapDataMemo=npRelHumTrapDataMemo, npThermoTrapMemo=npThermoTrapMemo, npCurLoopR=npCurLoopR, npThermoStatus=npThermoStatus, npCurLoopTrapStatus=npCurLoopTrapStatus, npRelHumStatus=npRelHumStatus, npIoEntry=npIoEntry, npGsmFailed=npGsmFailed, npGsmTraps=npGsmTraps, npRelayEntry=npRelayEntry, npRelHumTempStatus=npRelHumTempStatus, npIoTrapLineN=npIoTrapLineN, npThermoValue=npThermoValue, npGsmStrength=npGsmStrength, npThermoTrapStatus=npThermoTrapStatus, npRelayTable=npRelayTable, npRelHumTrapTempSafe=npRelHumTrapTempSafe, npRelHumTrapBelowSafe=npRelHumTrapBelowSafe, npIo=npIo, npCurLoopTrapV=npCurLoopTrapV, npIoMemo=npIoMemo, npRelHumSafeRangeLow=npRelHumSafeRangeLow, npRelHumValue=npRelHumValue, npRelHumTrapSafe=npRelHumTrapSafe, npCurLoopPower=npCurLoopPower, npCurLoopI=npCurLoopI, npSmokeTrapMemo=npSmokeTrapMemo, npRelayState=npRelayState, npRelHumTrapDataN=npRelHumTrapDataN, npCurLoopV=npCurLoopV, npIoTable=npIoTable, lightcom=lightcom, npThermoTrapSensorN=npThermoTrapSensorN, npRelHumTrapDataSafeRangeLow=npRelHumTrapDataSafeRangeLow, npThermoTrapHigh=npThermoTrapHigh, npThermoTrapLow=npThermoTrapLow, npRelHumTrapDataSafeRangeHigh=npRelHumTrapDataSafeRangeHigh, npRelHumidity=npRelHumidity, npIoLineN=npIoLineN, npSmokeReset=npSmokeReset, npThermoSensorN=npThermoSensorN, npSmokeTraps=npSmokeTraps, npCurLoopTable=npCurLoopTable, npThermoHigh=npThermoHigh, npSmoke=npSmoke, uniPingServerSolutionV3=uniPingServerSolutionV3, npSoftReboot=npSoftReboot, npRelayN=npRelayN, npCurLoopTrap=npCurLoopTrap, npRelay=npRelay, npCurLoopN=npCurLoopN, npIoTraps=npIoTraps, npRelHumTable=npRelHumTable, npIoSinglePulseStart=npIoSinglePulseStart, npRelHumTrapDataValue=npRelHumTrapDataValue, FixedPoint1000=FixedPoint1000, npRelHumTempSafeRangeLow=npRelHumTempSafeRangeLow, npRelHumTrapData=npRelHumTrapData, npSmokeTrapPrefix=npSmokeTrapPrefix, npCurLoopTrapR=npCurLoopTrapR, npForcedReboot=npForcedReboot, npCurLoopTrapPower=npCurLoopTrapPower, npSmokeEntry=npSmokeEntry, npCurLoop=npCurLoop, npRelHumTrapAllChannels=npRelHumTrapAllChannels, npThermoLow=npThermoLow, npRelHumTrapAllEvents=npRelHumTrapAllEvents, npSmokeMemo=npSmokeMemo, npThermoTable=npThermoTable, npCurLoopEntry=npCurLoopEntry, npThermoValuePrecise=npThermoValuePrecise, npThermoTraps=npThermoTraps, npRelHumTrapAboveSafe=npRelHumTrapAboveSafe, npGsmTrapPrefix=npGsmTrapPrefix, npTrapInfo=npTrapInfo, npRelHumTrapTempFail=npRelHumTrapTempFail, PYSNMP_MODULE_ID=uniPingServerSolutionV3, npThermoTrapValue=npThermoTrapValue, npRelayMode=npRelayMode, npGsmTrap=npGsmTrap, npThermoTrapPrefix=npThermoTrapPrefix, npIrCtrl=npIrCtrl, npSmokeTrapStatus=npSmokeTrapStatus, npThermo=npThermo, npGsmSendSmsUtf8=npGsmSendSmsUtf8, npSmokeTrapSensorN=npSmokeTrapSensorN, npRelHumSafeRangeHigh=npRelHumSafeRangeHigh, npThermoMemo=npThermoMemo, npIrStatus=npIrStatus, npIoTrapLevelIn=npIoTrapLevelIn, npSmokeTable=npSmokeTable, npCurLoopTraps=npCurLoopTraps, npRelHumEntry=npRelHumEntry, npGsmRegistration=npGsmRegistration, npRelHumN=npRelHumN, npResetStack=npResetStack, npRelayMemo=npRelayMemo, npRelHumTrapTempAllChannels=npRelHumTrapTempAllChannels, npIrReset=npIrReset, npCurLoopTrapI=npCurLoopTrapI, npTrapEmailTo=npTrapEmailTo, npGsmInfo=npGsmInfo, npIoTrapLevelLegend=npIoTrapLevelLegend, npSmokePower=npSmokePower, npReboot=npReboot, npRelHumTrapTempBelowSafe=npRelHumTrapTempBelowSafe, npSmokeTrap=npSmokeTrap, npRelHumTempSafeRangeHigh=npRelHumTempSafeRangeHigh, npRelHumTrapTemp=npRelHumTrapTemp, npCurLoopTrapPrefix=npCurLoopTrapPrefix, npIrPlayCmd=npIrPlayCmd, npCurLoopStatus=npCurLoopStatus, npSmokeStatus=npSmokeStatus, npRelHumTrapTempAboveSafe=npRelHumTrapTempAboveSafe, npIoTrapMemo=npIoTrapMemo, npRelHumMemo=npRelHumMemo, npIoPulseCounter=npIoPulseCounter, npIr=npIr, npThermoTrap=npThermoTrap, npRelHumTrap=npRelHumTrap, npCurLoopTrapN=npCurLoopTrapN, npRelHumTrapFail=npRelHumTrapFail, npGsm=npGsm, npRelHumTempValue=npRelHumTempValue, npRelHumTrapDataStatus=npRelHumTrapDataStatus, npRelayStartReset=npRelayStartReset, npRelHumTrapTempAllEvents=npRelHumTrapTempAllEvents, npGsmSendSmsWin1251=npGsmSendSmsWin1251, npThermoEntry=npThermoEntry, npIoLevelIn=npIoLevelIn, npIoTrapPrefix=npIoTrapPrefix, npIoLevelOut=npIoLevelOut)
