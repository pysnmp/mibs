...
Integer32, Bits, iso, IpAddress, Gauge32, Counter32, NotificationType, Counter64, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, enterprises, MibIdentifier, Unsigned32, TimeTicks, ModuleIdentity = mibBuilder.importSymbols("SNMPv2-SMI", "Integer32", "Bits", "iso", "IpAddress", "Gauge32", "Counter32", "NotificationType", "Counter64", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "enterprises", "MibIdentifier", "Unsigned32", "TimeTicks", "ModuleIdentity")
...
mibBuilder.exportSymbols("DKSF-70-6-X-X-1", npThermoTrapPrefix=npThermoTrapPrefix, npRelHumTrap=npRelHumTrap, npThermoTable=npThermoTable, npReboot=npReboot, npRelHumTrapTempAllEvents=npRelHumTrapTempAllEvents, npRelHumTable=npRelHumTable, npRelHumStatus=npRelHumStatus, npRelHumTrapDataN=npRelHumTrapDataN, npGsmSendSmsUtf8=npGsmSendSmsUtf8, npRelHumTrapTempSafe=npRelHumTrapTempSafe, npSmokeStatus=npSmokeStatus, npThermoStatus=npThermoStatus, npIrPlayCmd=npIrPlayCmd, npIoTrapPrefix=npIoTrapPrefix, npCurLoop=npCurLoop, npRelayTable=npRelayTable, npRelHumTempValue=npRelHumTempValue, npRelayStartReset=npRelayStartReset, npRelHumSafeRangeHigh=npRelHumSafeRangeHigh, npRelHumTrapAllChannels=npRelHumTrapAllChannels, npSmokeReset=npSmokeReset, npThermoTrap=npThermoTrap, npGsmInfo=npGsmInfo, npThermoTrapLow=npThermoTrapLow, npCurLoopEntry=npCurLoopEntry, npSmokeEntry=npSmokeEntry, npForcedReboot=npForcedReboot, npSmokeTrap=npSmokeTrap, npResetStack=npResetStack, npRelHumMemo=npRelHumMemo, npCurLoopTrapV=npCurLoopTrapV, npIoTrapLineN=npIoTrapLineN, npRelHumEntry=npRelHumEntry, npCurLoopR=npCurLoopR, npIrStatus=npIrStatus, npCurLoopTrapI=npCurLoopTrapI, npIoEntry=npIoEntry, npRelHumTrapTempBelowSafe=npRelHumTrapTempBelowSafe, npRelHumTempStatus=npRelHumTempStatus, npIoTable=npIoTable, npGsmTrap=npGsmTrap, npRelHumTrapTempAllChannels=npRelHumTrapTempAllChannels, npIoPulseCounter=npIoPulseCounter, npThermoMemo=npThermoMemo, npIoSinglePulseDuration=npIoSinglePulseDuration, npTrapInfo=npTrapInfo, npRelayN=npRelayN, npSmokeSensorN=npSmokeSensorN, npRelHumTrapSafe=npRelHumTrapSafe, npIoSinglePulseStart=npIoSinglePulseStart, npSmokeTrapPrefix=npSmokeTrapPrefix, npRelHumTrapTempFail=npRelHumTrapTempFail, PYSNMP_MODULE_ID=uniPingServerSolutionV3, npCurLoopTrap=npCurLoopTrap, npThermoValue=npThermoValue, uniPingServerSolutionV3=uniPingServerSolutionV3, FixedPoint1000=FixedPoint1000, npIoTrap=npIoTrap, npThermoTrapSensorN=npThermoTrapSensorN, npCurLoopTrapR=npCurLoopTrapR, npThermoValuePrecise=npThermoValuePrecise, npGsmStrength=npGsmStrength, npThermoTrapHigh=npThermoTrapHigh, npRelHumidity=npRelHumidity, npThermoTrapMemo=npThermoTrapMemo, npCurLoopTrapStatus=npCurLoopTrapStatus, npRelHumTrapAllEvents=npRelHumTrapAllEvents, npRelayMemo=npRelayMemo, npIoTrapMemo=npIoTrapMemo, npRelHumSafeRangeLow=npRelHumSafeRangeLow, npIrCtrl=npIrCtrl, npIoLevelOut=npIoLevelOut, npSmokeTrapMemo=npSmokeTrapMemo, npGsmSendSmsWin1251=npGsmSendSmsWin1251, npCurLoopTrapPower=npCurLoopTrapPower, npSmokeMemo=npSmokeMemo, npThermoEntry=npThermoEntry, npRelHumTrapDataMemo=npRelHumTrapDataMemo, npSmoke=npSmoke, npSmokeTable=npSmokeTable, npRelHumValue=npRelHumValue, npRelHumTrapDataValue=npRelHumTrapDataValue, npCurLoopTable=npCurLoopTable, npSmokeTrapStatus=npSmokeTrapStatus, npGsmTraps=npGsmTraps, npTrapEmailTo=npTrapEmailTo, npGsmFailed=npGsmFailed, npThermoHigh=npThermoHigh, npRelHumTrapFail=npRelHumTrapFail, npGsmRegistration=npGsmRegistration, npGsm=npGsm, npCurLoopTraps=npCurLoopTraps, npThermo=npThermo, npSmokePower=npSmokePower, npSmokeTrapSensorN=npSmokeTrapSensorN, npIo=npIo, npIoTrapLevelIn=npIoTrapLevelIn, lightcom=lightcom, npRelayMode=npRelayMode, npRelay=npRelay, npThermoLow=npThermoLow, npCurLoopStatus=npCurLoopStatus, npIrReset=npIrReset, npCurLoopI=npCurLoopI, npIoTraps=npIoTraps, npThermoTrapValue=npThermoTrapValue, npThermoTraps=npThermoTraps, npSoftReboot=npSoftReboot, npRelHumTrapTemp=npRelHumTrapTemp, npThermoSensorN=npThermoSensorN, npIoTrapLevelLegend=npIoTrapLevelLegend, npGsmTrapPrefix=npGsmTrapPrefix, npCurLoopTrapN=npCurLoopTrapN, npRelHumTrapData=npRelHumTrapData, npRelHumTempSafeRangeLow=npRelHumTempSafeRangeLow, npRelHumTrapAboveSafe=npRelHumTrapAboveSafe, npSmokeTraps=npSmokeTraps, npIoMemo=npIoMemo, npCurLoopN=npCurLoopN, npCurLoopV=npCurLoopV, npThermoTrapStatus=npThermoTrapStatus, npRelHumTrapBelowSafe=npRelHumTrapBelowSafe, npIoLineN=npIoLineN, npRelHumTrapTempAboveSafe=npRelHumTrapTempAboveSafe, npRelHumTrapDataSafeRangeLow=npRelHumTrapDataSafeRangeLow, npRelHumTempSafeRangeHigh=npRelHumTempSafeRangeHigh, npRelHumTrapDataSafeRangeHigh=npRelHumTrapDataSafeRangeHigh, npRelayEntry=npRelayEntry, npRelHumN=npRelHumN, npRelHumTrapDataStatus=npRelHumTrapDataStatus, npCurLoopPower=npCurLoopPower, npRelayState=npRelayState, npIoLevelIn=npIoLevelIn, npIr=npIr, npCurLoopTrapPrefix=npCurLoopTrapPrefix)
