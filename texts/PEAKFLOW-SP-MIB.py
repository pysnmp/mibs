...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDetector=spDetector, spDuration=spDuration, peakflowSPCMI=peakflowSPCMI, spVersion=spVersion, spSystemErrorType=spSystemErrorType, deviceHealth=deviceHealth, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spLicenseErrDescription=spLicenseErrDescription, spInterfaceUsageHC=spInterfaceUsageHC, managedObjectUsageDone=managedObjectUsageDone, spBlackholeCommunity=spBlackholeCommunity, spBlackholeTimeout=spBlackholeTimeout, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spScriptPort=spScriptPort, spInterfaceIndex=spInterfaceIndex, spInterfaceSpeedHC=spInterfaceSpeedHC, spImpactBps=spImpactBps, spCommFailureDestination=spCommFailureDestination, spBGPTrapPrefix=spBGPTrapPrefix, devicePhysicalMemory=devicePhysicalMemory, cloudSignalingFault=cloudSignalingFault, spScriptStart=spScriptStart, fingerprintUsageDone=fingerprintUsageDone, snmpUp=snmpUp, spSystemErrorDescription=spSystemErrorDescription, spReportID=spReportID, spBlackholeNexthop=spBlackholeNexthop, spThirdPartyAddr=spThirdPartyAddr, spAlertDetectionSignatures=spAlertDetectionSignatures, collectorDown=collectorDown, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, peakflowSPMIB=peakflowSPMIB, spcommFailure=spcommFailure, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spDNSObserved=spDNSObserved, spInetAddress=spInetAddress, greDownDone=greDownDone, serviceUsageDone=serviceUsageDone, spMitigationID=spMitigationID, spGreTunnelName=spGreTunnelName, greDown=greDown, spBGPInstability=spBGPInstability, notificationLimit=notificationLimit, spInterfaceUsage=spInterfaceUsage, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, collectorStart=collectorStart, spTMSCommunity=spTMSCommunity, routingFailover=routingFailover, spHijackAttr=spHijackAttr, spDNSObservedMean=spDNSObservedMean, spScriptCommand=spScriptCommand, spDetectedCountries=spDetectedCountries, spBGPTrapName=spBGPTrapName, mitigationDone=mitigationDone, interfaceUsage=interfaceUsage, trafficAutoMitigation=trafficAutoMitigation, spFingerprintMessage=spFingerprintMessage, spLicenseErrLimit=spLicenseErrLimit, deviceDiskUsage=deviceDiskUsage, spInetAddressType=spInetAddressType, spBGPTrapEvent=spBGPTrapEvent, fingerprintReceive=fingerprintReceive, spThirdPartyZone=spThirdPartyZone, spReportName=spReportName, cloudSignalingFaultDone=cloudSignalingFaultDone, spFingerprintFeedback=spFingerprintFeedback, spFlowspecTimeout=spFlowspecTimeout, dnsBaselineDone=dnsBaselineDone, spDNSExpected=spDNSExpected, spCloudSignalFaultDescription=spCloudSignalFaultDescription, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, hardwareFailureDone=hardwareFailureDone, spScriptHost=spScriptHost, spThreshold=spThreshold, spFlowspecCommunity=spFlowspecCommunity, spTMSTimeout=spTMSTimeout, deviceSystemErrorDone=deviceSystemErrorDone, spDNSName=spDNSName, spLicenseErrCount=spLicenseErrCount, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPTrapNewAttributes=spBGPTrapNewAttributes, flowDown=flowDown, spCommFailureDescription=spCommFailureDescription, alertScript=alertScript, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spCollector=spCollector, mitigationTMSStartV2=mitigationTMSStartV2, licenseError=licenseError, spDNSObservedMax=spDNSObservedMax, spBlackholePrefix=spBlackholePrefix, flowUp=flowUp, spInterfaceSpeed=spInterfaceSpeed, interfaceUsageDone=interfaceUsageDone, reportDone=reportDone, bgpDown=bgpDown, spAlertID=spAlertID, configChange=configChange, deviceTotalFlows=deviceTotalFlows, spPravail=spPravail, deviceSwapSpaceInUse=deviceSwapSpaceInUse, peakflowSPMgr=peakflowSPMgr, autoclassifyStart=autoclassifyStart, spMitigationName=spMitigationName, bgpHijack=bgpHijack, spFingerprintSender=spFingerprintSender, spServiceName=spServiceName, mitigationFlowspecStart=mitigationFlowspecStart, spUnit=spUnit, spHijackLocal=spHijackLocal, hardwareFailure=hardwareFailure, spLicenseErrType=spLicenseErrType, snmpDown=snmpDown, spImpactPps=spImpactPps, bgpTrap=bgpTrap, serviceUsage=serviceUsage, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spUsageType=spUsageType, spApplicationName=spApplicationName, peakflowSPTraps=peakflowSPTraps, spTMSMultiPrefix=spTMSMultiPrefix, fingerprintFeedback=fingerprintFeedback, spUsage=spUsage, spTMSPrefix=spTMSPrefix, mitigationBlackholeStart=mitigationBlackholeStart, spServiceElement=spServiceElement, dosHostDetectionAlert=dosHostDetectionAlert, licenseErrorDone=licenseErrorDone, deviceSystemError=deviceSystemError, dosHostDetectionAlertDone=dosHostDetectionAlertDone, PYSNMP_MODULE_ID=peakflowSPMIB, fingerprintUsage=fingerprintUsage, spManagedObject=spManagedObject, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, bgpInstability=bgpInstability, spReportURL=spReportURL, managedObjectUsage=managedObjectUsage, spGreTunnelDestination=spGreTunnelDestination, bgpUp=bgpUp, spHijackRoute=spHijackRoute, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spManagedObjectFamily=spManagedObjectFamily, spMitigationStart=spMitigationStart, spFingerprintName=spFingerprintName, dnsBaseline=dnsBaseline, spManagedObjects=spManagedObjects, spBGPTrapOldAttributes=spBGPTrapOldAttributes, bgpInstabilityDone=bgpInstabilityDone, collectorUp=collectorUp, spBGPEvent=spBGPEvent, spUsername=spUsername, deviceTotalFlowsHC=deviceTotalFlowsHC, deviceSwapSpace=deviceSwapSpace, spInterface=spInterface, spHardwareFailureDescription=spHardwareFailureDescription, mitigationTMSStart=mitigationTMSStart, spcommFailureDone=spcommFailureDone, spRouter=spRouter)
