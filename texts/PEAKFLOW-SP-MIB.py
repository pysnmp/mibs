...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spcommFailureDone=spcommFailureDone, deviceTotalFlowsHC=deviceTotalFlowsHC, deviceHealth=deviceHealth, spBGPTrapNewAttributes=spBGPTrapNewAttributes, bgpInstabilityDone=bgpInstabilityDone, spVersion=spVersion, deviceDiskUsage=deviceDiskUsage, spFingerprintName=spFingerprintName, spBGPTrapEvent=spBGPTrapEvent, spSystemErrorType=spSystemErrorType, dnsBaselineDone=dnsBaselineDone, spTMSPrefix=spTMSPrefix, spUsage=spUsage, licenseError=licenseError, reportDone=reportDone, spServiceElement=spServiceElement, interfaceUsage=interfaceUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spGreTunnelDestination=spGreTunnelDestination, spImpactPps=spImpactPps, snmpDown=snmpDown, spUsageType=spUsageType, spHijackLocal=spHijackLocal, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spApplicationName=spApplicationName, spLicenseErrCount=spLicenseErrCount, configChange=configChange, spCommFailureDestination=spCommFailureDestination, spDetectedCountries=spDetectedCountries, spManagedObjectFamily=spManagedObjectFamily, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, fingerprintReceive=fingerprintReceive, spHijackAttr=spHijackAttr, spTMSCommunity=spTMSCommunity, spReportID=spReportID, spCommFailureDescription=spCommFailureDescription, spFingerprintMessage=spFingerprintMessage, mitigationTMSStart=mitigationTMSStart, spInterfaceSpeed=spInterfaceSpeed, spImpactBps=spImpactBps, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, snmpUp=snmpUp, deviceSwapSpaceUsage=deviceSwapSpaceUsage, fingerprintUsageDone=fingerprintUsageDone, mitigationThirdPartyStart=mitigationThirdPartyStart, bgpUp=bgpUp, spInterfaceUsage=spInterfaceUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, peakflowSPMIB=peakflowSPMIB, spScriptCommand=spScriptCommand, deviceSwapSpaceInUse=deviceSwapSpaceInUse, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, fingerprintUsage=fingerprintUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, collectorDown=collectorDown, spBlackholeNexthop=spBlackholeNexthop, flowDown=flowDown, spInterfaceUsageHC=spInterfaceUsageHC, peakflowSPCMI=peakflowSPCMI, spInetAddress=spInetAddress, spFlowspecTimeout=spFlowspecTimeout, bgpInstability=bgpInstability, spBGPTrapName=spBGPTrapName, spLicenseErrDescription=spLicenseErrDescription, spReportURL=spReportURL, spScriptPort=spScriptPort, mitigationFlowspecStart=mitigationFlowspecStart, spDetector=spDetector, spFingerprintSender=spFingerprintSender, autoclassifyStart=autoclassifyStart, dosNetworkProfiledAlert=dosNetworkProfiledAlert, dosHostDetectionAlert=dosHostDetectionAlert, serviceUsageDone=serviceUsageDone, spGreTunnelName=spGreTunnelName, greDown=greDown, collectorUp=collectorUp, spTMSMultiPrefix=spTMSMultiPrefix, greDownDone=greDownDone, spDNSObservedMean=spDNSObservedMean, spThirdPartyZone=spThirdPartyZone, managedObjectUsageDone=managedObjectUsageDone, bgpDown=bgpDown, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, hardwareFailure=hardwareFailure, spFlowspecCommunity=spFlowspecCommunity, spTMSTimeout=spTMSTimeout, spUnit=spUnit, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spSystemErrorDescription=spSystemErrorDescription, spInterface=spInterface, spUsername=spUsername, spInterfaceSpeedHC=spInterfaceSpeedHC, spInterfaceIndex=spInterfaceIndex, trafficAutoMitigation=trafficAutoMitigation, spInetAddressType=spInetAddressType, spFingerprintFeedback=spFingerprintFeedback, spBGPTrapPrefix=spBGPTrapPrefix, spMitigationName=spMitigationName, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spMitigationID=spMitigationID, spRouter=spRouter, spCollector=spCollector, spLicenseErrLimit=spLicenseErrLimit, spThreshold=spThreshold, spMitigationStart=spMitigationStart, spDuration=spDuration, spScriptStart=spScriptStart, peakflowSPMgr=peakflowSPMgr, spPravail=spPravail, PYSNMP_MODULE_ID=peakflowSPMIB, deviceSystemError=deviceSystemError, deviceTotalFlows=deviceTotalFlows, mitigationDone=mitigationDone, spDNSName=spDNSName, bgpTrap=bgpTrap, spAlertDetectionSignatures=spAlertDetectionSignatures, cloudSignalingFault=cloudSignalingFault, spBlackholeCommunity=spBlackholeCommunity, peakflowSPTraps=peakflowSPTraps, alertScript=alertScript, spHardwareFailureDescription=spHardwareFailureDescription, routingFailover=routingFailover, spScriptHost=spScriptHost, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, bgpHijack=bgpHijack, spReportName=spReportName, deviceSystemErrorDone=deviceSystemErrorDone, notificationLimit=notificationLimit, serviceUsage=serviceUsage, spBGPEvent=spBGPEvent, spManagedObject=spManagedObject, fingerprintFeedback=fingerprintFeedback, managedObjectUsage=managedObjectUsage, spManagedObjects=spManagedObjects, spBGPInstability=spBGPInstability, mitigationTMSStartV2=mitigationTMSStartV2, spDNSObservedMax=spDNSObservedMax, spAlertID=spAlertID, dnsBaseline=dnsBaseline, interfaceUsageDone=interfaceUsageDone, flowUp=flowUp, spDNSObserved=spDNSObserved, spThirdPartyAddr=spThirdPartyAddr, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, devicePhysicalMemory=devicePhysicalMemory, collectorStart=collectorStart, licenseErrorDone=licenseErrorDone, spcommFailure=spcommFailure, spBlackholePrefix=spBlackholePrefix, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spLicenseErrType=spLicenseErrType, spBlackholeTimeout=spBlackholeTimeout, deviceSwapSpace=deviceSwapSpace, spCloudSignalFaultDescription=spCloudSignalFaultDescription, mitigationBlackholeStart=mitigationBlackholeStart, spDNSExpected=spDNSExpected, cloudSignalingFaultDone=cloudSignalingFaultDone, spHijackRoute=spHijackRoute, spServiceName=spServiceName, hardwareFailureDone=hardwareFailureDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min)
