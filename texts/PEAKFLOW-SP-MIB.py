...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spMitigationID=spMitigationID, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, mitigationDone=mitigationDone, spServiceName=spServiceName, spManagedObjectFamily=spManagedObjectFamily, spInetAddressType=spInetAddressType, snmpUp=snmpUp, licenseError=licenseError, spReportURL=spReportURL, spBGPInstability=spBGPInstability, collectorStart=collectorStart, spInterface=spInterface, spImpactPps=spImpactPps, spDNSObserved=spDNSObserved, interfaceUsageDone=interfaceUsageDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spVersion=spVersion, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, managedObjectUsage=managedObjectUsage, flowUp=flowUp, spHijackLocal=spHijackLocal, bgpHijack=bgpHijack, spThirdPartyZone=spThirdPartyZone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, mitigationBlackholeStart=mitigationBlackholeStart, spDuration=spDuration, spGreTunnelName=spGreTunnelName, spBGPEvent=spBGPEvent, spManagedObjects=spManagedObjects, spInterfaceUsageHC=spInterfaceUsageHC, spGreTunnelDestination=spGreTunnelDestination, spFingerprintName=spFingerprintName, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, bgpTrap=bgpTrap, deviceSystemErrorDone=deviceSystemErrorDone, spHijackAttr=spHijackAttr, spScriptStart=spScriptStart, spTMSPrefix=spTMSPrefix, spManagedObject=spManagedObject, deviceDiskUsage=deviceDiskUsage, spDetector=spDetector, routingFailover=routingFailover, spLicenseErrLimit=spLicenseErrLimit, cloudSignalingFault=cloudSignalingFault, spReportID=spReportID, managedObjectUsageDone=managedObjectUsageDone, spCommFailureDestination=spCommFailureDestination, spUnit=spUnit, deviceHealth=deviceHealth, cloudSignalingFaultDone=cloudSignalingFaultDone, notificationLimit=notificationLimit, spInetAddress=spInetAddress, spUsage=spUsage, spRouter=spRouter, peakflowSPTraps=peakflowSPTraps, deviceSwapSpace=deviceSwapSpace, mitigationThirdPartyStart=mitigationThirdPartyStart, spImpactBps=spImpactBps, spDNSObservedMean=spDNSObservedMean, bgpDown=bgpDown, spLicenseErrDescription=spLicenseErrDescription, spDNSExpected=spDNSExpected, greDown=greDown, deviceTotalFlowsHC=deviceTotalFlowsHC, spThirdPartyAddr=spThirdPartyAddr, dnsBaseline=dnsBaseline, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFingerprintFeedback=spFingerprintFeedback, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spScriptCommand=spScriptCommand, flowDown=flowDown, fingerprintReceive=fingerprintReceive, spTMSMultiPrefix=spTMSMultiPrefix, spBGPTrapOldAttributes=spBGPTrapOldAttributes, mitigationTMSStartV2=mitigationTMSStartV2, spTMSCommunity=spTMSCommunity, spSystemErrorDescription=spSystemErrorDescription, spAlertID=spAlertID, spThreshold=spThreshold, spApplicationName=spApplicationName, spFingerprintMessage=spFingerprintMessage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spDNSObservedMax=spDNSObservedMax, spTMSTimeout=spTMSTimeout, reportDone=reportDone, spPravail=spPravail, fingerprintUsage=fingerprintUsage, collectorUp=collectorUp, spUsageType=spUsageType, trafficAutoMitigation=trafficAutoMitigation, serviceUsage=serviceUsage, devicePhysicalMemory=devicePhysicalMemory, mitigationTMSStart=mitigationTMSStart, spHardwareFailureDescription=spHardwareFailureDescription, spHijackRoute=spHijackRoute, interfaceUsage=interfaceUsage, peakflowSPMIB=peakflowSPMIB, bgpInstability=bgpInstability, spBGPTrapName=spBGPTrapName, hardwareFailure=hardwareFailure, fingerprintFeedback=fingerprintFeedback, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInterfaceIndex=spInterfaceIndex, spFlowspecTimeout=spFlowspecTimeout, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, bgpInstabilityDone=bgpInstabilityDone, spInterfaceUsage=spInterfaceUsage, spBlackholeCommunity=spBlackholeCommunity, spcommFailure=spcommFailure, spCollector=spCollector, spScriptHost=spScriptHost, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spFlowspecCommunity=spFlowspecCommunity, collectorDown=collectorDown, spcommFailureDone=spcommFailureDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSName=spDNSName, spInterfaceSpeed=spInterfaceSpeed, autoclassifyStart=autoclassifyStart, configChange=configChange, fingerprintUsageDone=fingerprintUsageDone, PYSNMP_MODULE_ID=peakflowSPMIB, spSystemErrorType=spSystemErrorType, dosHostDetectionAlert=dosHostDetectionAlert, spBGPTrapEvent=spBGPTrapEvent, spBlackholeTimeout=spBlackholeTimeout, deviceTotalFlows=deviceTotalFlows, bgpUp=bgpUp, spDetectedCountries=spDetectedCountries, serviceUsageDone=serviceUsageDone, spMitigationStart=spMitigationStart, peakflowSPMgr=peakflowSPMgr, spUsername=spUsername, spBGPTrapPrefix=spBGPTrapPrefix, spBlackholeNexthop=spBlackholeNexthop, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spAlertDetectionSignatures=spAlertDetectionSignatures, spServiceElement=spServiceElement, dnsBaselineDone=dnsBaselineDone, mitigationFlowspecStart=mitigationFlowspecStart, snmpDown=snmpDown, spLicenseErrCount=spLicenseErrCount, licenseErrorDone=licenseErrorDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spMitigationName=spMitigationName, spLicenseErrType=spLicenseErrType, spCommFailureDescription=spCommFailureDescription, spFingerprintSender=spFingerprintSender, spScriptPort=spScriptPort, spInterfaceSpeedHC=spInterfaceSpeedHC, alertScript=alertScript, hardwareFailureDone=hardwareFailureDone, greDownDone=greDownDone, peakflowSPCMI=peakflowSPCMI, spReportName=spReportName, deviceSystemError=deviceSystemError, spBlackholePrefix=spBlackholePrefix, spCloudSignalFaultDescription=spCloudSignalFaultDescription)
