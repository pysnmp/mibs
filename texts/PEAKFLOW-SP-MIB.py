...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spAlertDetectionSignatures=spAlertDetectionSignatures, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spTMSPrefix=spTMSPrefix, spUnit=spUnit, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, bgpInstabilityDone=bgpInstabilityDone, flowDown=flowDown, spUsage=spUsage, greDownDone=greDownDone, spCommFailureDescription=spCommFailureDescription, managedObjectUsage=managedObjectUsage, spBGPTrapEvent=spBGPTrapEvent, spTMSMultiPrefix=spTMSMultiPrefix, cloudSignalingFault=cloudSignalingFault, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spFlowspecCommunity=spFlowspecCommunity, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, dnsBaselineDone=dnsBaselineDone, snmpDown=snmpDown, spReportID=spReportID, cloudSignalingFaultDone=cloudSignalingFaultDone, spManagedObject=spManagedObject, spBGPTrapPrefix=spBGPTrapPrefix, deviceHealth=deviceHealth, spThirdPartyAddr=spThirdPartyAddr, mitigationDone=mitigationDone, spImpactBps=spImpactBps, spHijackLocal=spHijackLocal, spcommFailureDone=spcommFailureDone, spGreTunnelName=spGreTunnelName, interfaceUsage=interfaceUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spHardwareFailureDescription=spHardwareFailureDescription, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spGreTunnelDestination=spGreTunnelDestination, spSystemErrorDescription=spSystemErrorDescription, bgpHijack=bgpHijack, deviceSystemErrorDone=deviceSystemErrorDone, licenseErrorDone=licenseErrorDone, deviceSystemError=deviceSystemError, fingerprintReceive=fingerprintReceive, mitigationTMSStartV2=mitigationTMSStartV2, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, notificationLimit=notificationLimit, spThreshold=spThreshold, spDetectedCountries=spDetectedCountries, spLicenseErrLimit=spLicenseErrLimit, autoclassifyStart=autoclassifyStart, spcommFailure=spcommFailure, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spInterfaceUsage=spInterfaceUsage, spBGPTrapName=spBGPTrapName, spBGPEvent=spBGPEvent, reportDone=reportDone, spScriptCommand=spScriptCommand, fingerprintUsageDone=fingerprintUsageDone, spBlackholeCommunity=spBlackholeCommunity, bgpInstability=bgpInstability, spInetAddressType=spInetAddressType, spCommFailureDestination=spCommFailureDestination, spMitigationStart=spMitigationStart, spBlackholeNexthop=spBlackholeNexthop, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spReportURL=spReportURL, managedObjectUsageDone=managedObjectUsageDone, spDetector=spDetector, spServiceElement=spServiceElement, spInterfaceIndex=spInterfaceIndex, spDNSExpected=spDNSExpected, spTMSCommunity=spTMSCommunity, spScriptHost=spScriptHost, spInterface=spInterface, mitigationBlackholeStart=mitigationBlackholeStart, serviceUsage=serviceUsage, spCollector=spCollector, spLicenseErrDescription=spLicenseErrDescription, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spServiceName=spServiceName, hardwareFailureDone=hardwareFailureDone, spBGPInstability=spBGPInstability, spManagedObjects=spManagedObjects, spFingerprintSender=spFingerprintSender, devicePhysicalMemory=devicePhysicalMemory, spTMSTimeout=spTMSTimeout, flowUp=flowUp, mitigationThirdPartyStart=mitigationThirdPartyStart, greDown=greDown, interfaceUsageDone=interfaceUsageDone, collectorUp=collectorUp, snmpUp=snmpUp, peakflowSPTraps=peakflowSPTraps, spRouter=spRouter, deviceSwapSpaceUsage=deviceSwapSpaceUsage, bgpTrap=bgpTrap, routingFailover=routingFailover, peakflowSPMgr=peakflowSPMgr, spAlertID=spAlertID, spManagedObjectFamily=spManagedObjectFamily, spImpactPps=spImpactPps, deviceSwapSpace=deviceSwapSpace, spDNSObservedMean=spDNSObservedMean, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spDNSName=spDNSName, spHijackRoute=spHijackRoute, spThirdPartyZone=spThirdPartyZone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpUp=bgpUp, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDuration=spDuration, spScriptStart=spScriptStart, spDNSObserved=spDNSObserved, spPravail=spPravail, spBlackholePrefix=spBlackholePrefix, deviceDiskUsage=deviceDiskUsage, spHijackAttr=spHijackAttr, spScriptPort=spScriptPort, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceTotalFlows=deviceTotalFlows, spReportName=spReportName, spUsername=spUsername, mitigationTMSStart=mitigationTMSStart, spMitigationName=spMitigationName, spInterfaceSpeed=spInterfaceSpeed, spUsageType=spUsageType, peakflowSPCMI=peakflowSPCMI, mitigationFlowspecStart=mitigationFlowspecStart, serviceUsageDone=serviceUsageDone, alertScript=alertScript, spLicenseErrType=spLicenseErrType, collectorDown=collectorDown, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterfaceSpeedHC=spInterfaceSpeedHC, spInetAddress=spInetAddress, spVersion=spVersion, fingerprintFeedback=fingerprintFeedback, spFingerprintName=spFingerprintName, spFlowspecTimeout=spFlowspecTimeout, spApplicationName=spApplicationName, trafficAutoMitigation=trafficAutoMitigation, spLicenseErrCount=spLicenseErrCount, peakflowSPMIB=peakflowSPMIB, spInterfaceUsageHC=spInterfaceUsageHC, configChange=configChange, dnsBaseline=dnsBaseline, spMitigationID=spMitigationID, bgpDown=bgpDown, spBGPTrapOldAttributes=spBGPTrapOldAttributes, dosHostDetectionAlert=dosHostDetectionAlert, spBlackholeTimeout=spBlackholeTimeout, PYSNMP_MODULE_ID=peakflowSPMIB, fingerprintUsage=fingerprintUsage, hardwareFailure=hardwareFailure, licenseError=licenseError, spFingerprintFeedback=spFingerprintFeedback, spSystemErrorType=spSystemErrorType, spDNSObservedMax=spDNSObservedMax, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFingerprintMessage=spFingerprintMessage, collectorStart=collectorStart)
