...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceTotalFlowsHC=deviceTotalFlowsHC, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBlackholeCommunity=spBlackholeCommunity, spBlackholePrefix=spBlackholePrefix, spFingerprintSender=spFingerprintSender, bgpDown=bgpDown, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSTimeout=spTMSTimeout, spGreTunnelName=spGreTunnelName, mitigationTMSStartV2=mitigationTMSStartV2, spBGPTrapName=spBGPTrapName, spUsage=spUsage, spDNSName=spDNSName, spUnit=spUnit, spBlackholeNexthop=spBlackholeNexthop, autoclassifyStart=autoclassifyStart, peakflowSPCMI=peakflowSPCMI, spFlowspecCommunity=spFlowspecCommunity, mitigationThirdPartyStart=mitigationThirdPartyStart, trafficAutoMitigation=trafficAutoMitigation, collectorStart=collectorStart, collectorUp=collectorUp, spUsername=spUsername, serviceUsageDone=serviceUsageDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, notificationLimit=notificationLimit, spCollector=spCollector, deviceSwapSpace=deviceSwapSpace, greDownDone=greDownDone, spAlertDetectionSignatures=spAlertDetectionSignatures, spMitigationID=spMitigationID, spScriptHost=spScriptHost, peakflowSPMIB=peakflowSPMIB, spThirdPartyZone=spThirdPartyZone, spInetAddressType=spInetAddressType, spUsageType=spUsageType, fingerprintUsageDone=fingerprintUsageDone, spTMSPrefix=spTMSPrefix, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spDNSExpected=spDNSExpected, spDNSObserved=spDNSObserved, bgpTrap=bgpTrap, spReportURL=spReportURL, fingerprintReceive=fingerprintReceive, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, alertScript=alertScript, spInterface=spInterface, deviceSwapSpaceInUse=deviceSwapSpaceInUse, interfaceUsageDone=interfaceUsageDone, peakflowSPMgr=peakflowSPMgr, spHijackRoute=spHijackRoute, spSystemErrorDescription=spSystemErrorDescription, spBGPTrapEvent=spBGPTrapEvent, spSystemErrorType=spSystemErrorType, dnsBaselineDone=dnsBaselineDone, PYSNMP_MODULE_ID=peakflowSPMIB, spFingerprintMessage=spFingerprintMessage, spDNSObservedMax=spDNSObservedMax, devicePhysicalMemory=devicePhysicalMemory, spInterfaceSpeedHC=spInterfaceSpeedHC, spManagedObject=spManagedObject, interfaceUsage=interfaceUsage, deviceHealth=deviceHealth, spApplicationName=spApplicationName, mitigationFlowspecStart=mitigationFlowspecStart, hardwareFailure=hardwareFailure, spCommFailureDescription=spCommFailureDescription, spImpactBps=spImpactBps, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spTMSCommunity=spTMSCommunity, reportDone=reportDone, serviceUsage=serviceUsage, configChange=configChange, spImpactPps=spImpactPps, spLicenseErrCount=spLicenseErrCount, cloudSignalingFaultDone=cloudSignalingFaultDone, dosHostDetectionAlert=dosHostDetectionAlert, spThirdPartyAddr=spThirdPartyAddr, spFlowspecTimeout=spFlowspecTimeout, cloudSignalingFault=cloudSignalingFault, fingerprintFeedback=fingerprintFeedback, deviceSystemError=deviceSystemError, greDown=greDown, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spLicenseErrLimit=spLicenseErrLimit, bgpHijack=bgpHijack, spVersion=spVersion, dnsBaseline=dnsBaseline, spBlackholeTimeout=spBlackholeTimeout, spGreTunnelDestination=spGreTunnelDestination, flowDown=flowDown, spDetector=spDetector, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spLicenseErrType=spLicenseErrType, mitigationDone=mitigationDone, peakflowSPTraps=peakflowSPTraps, bgpInstabilityDone=bgpInstabilityDone, spServiceName=spServiceName, spReportID=spReportID, spHardwareFailureDescription=spHardwareFailureDescription, spBGPInstability=spBGPInstability, spDetectedCountries=spDetectedCountries, managedObjectUsageDone=managedObjectUsageDone, spInterfaceUsageHC=spInterfaceUsageHC, mitigationTMSStart=mitigationTMSStart, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spCommFailureDestination=spCommFailureDestination, spMitigationStart=spMitigationStart, spcommFailure=spcommFailure, spMitigationName=spMitigationName, spBGPEvent=spBGPEvent, spLicenseErrDescription=spLicenseErrDescription, spThreshold=spThreshold, spPravail=spPravail, managedObjectUsage=managedObjectUsage, mitigationBlackholeStart=mitigationBlackholeStart, spFingerprintName=spFingerprintName, spManagedObjectFamily=spManagedObjectFamily, licenseError=licenseError, spScriptStart=spScriptStart, spRouter=spRouter, spHijackAttr=spHijackAttr, collectorDown=collectorDown, spHijackLocal=spHijackLocal, spInterfaceIndex=spInterfaceIndex, spCloudSignalFaultDescription=spCloudSignalFaultDescription, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spReportName=spReportName, spInterfaceUsage=spInterfaceUsage, snmpUp=snmpUp, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spServiceElement=spServiceElement, spFingerprintFeedback=spFingerprintFeedback, deviceDiskUsage=deviceDiskUsage, routingFailover=routingFailover, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spScriptCommand=spScriptCommand, licenseErrorDone=licenseErrorDone, snmpDown=snmpDown, hardwareFailureDone=hardwareFailureDone, spBGPTrapPrefix=spBGPTrapPrefix, spInterfaceSpeed=spInterfaceSpeed, spcommFailureDone=spcommFailureDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, flowUp=flowUp, bgpInstability=bgpInstability, spManagedObjects=spManagedObjects, spAlertID=spAlertID, deviceTotalFlows=deviceTotalFlows, fingerprintUsage=fingerprintUsage, spScriptPort=spScriptPort, spTMSMultiPrefix=spTMSMultiPrefix, spInetAddress=spInetAddress, deviceSystemErrorDone=deviceSystemErrorDone, spDuration=spDuration, spDNSObservedMean=spDNSObservedMean, bgpUp=bgpUp)
