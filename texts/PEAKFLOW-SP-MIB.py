...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spScriptStart=spScriptStart, cloudSignalingFaultDone=cloudSignalingFaultDone, spBlackholeNexthop=spBlackholeNexthop, spServiceName=spServiceName, deviceHealth=deviceHealth, spLicenseErrLimit=spLicenseErrLimit, spBGPTrapPrefix=spBGPTrapPrefix, spHijackLocal=spHijackLocal, spDNSObservedMean=spDNSObservedMean, spUsage=spUsage, spInterfaceUsageHC=spInterfaceUsageHC, flowUp=flowUp, deviceSwapSpace=deviceSwapSpace, mitigationBlackholeStart=mitigationBlackholeStart, peakflowSPCMI=peakflowSPCMI, spSystemErrorDescription=spSystemErrorDescription, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, hardwareFailure=hardwareFailure, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spReportURL=spReportURL, spGreTunnelName=spGreTunnelName, deviceDiskUsage=deviceDiskUsage, bgpDown=bgpDown, deviceSystemError=deviceSystemError, spBGPEvent=spBGPEvent, spInterfaceSpeedHC=spInterfaceSpeedHC, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spLicenseErrCount=spLicenseErrCount, spImpactPps=spImpactPps, PYSNMP_MODULE_ID=peakflowSPMIB, trafficAutoMitigation=trafficAutoMitigation, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spFingerprintFeedback=spFingerprintFeedback, managedObjectUsage=managedObjectUsage, alertScript=alertScript, spInterfaceUsage=spInterfaceUsage, routingFailover=routingFailover, interfaceUsage=interfaceUsage, spTMSMultiPrefix=spTMSMultiPrefix, spFingerprintSender=spFingerprintSender, spInterfaceSpeed=spInterfaceSpeed, collectorStart=collectorStart, greDown=greDown, spFlowspecCommunity=spFlowspecCommunity, spAlertID=spAlertID, spRouter=spRouter, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, fingerprintReceive=fingerprintReceive, spManagedObject=spManagedObject, spInetAddress=spInetAddress, spTMSPrefix=spTMSPrefix, spServiceElement=spServiceElement, spHijackRoute=spHijackRoute, spAlertDetectionSignatures=spAlertDetectionSignatures, cloudSignalingFault=cloudSignalingFault, spCloudSignalFaultDescription=spCloudSignalFaultDescription, snmpUp=snmpUp, spCommFailureDescription=spCommFailureDescription, spDetector=spDetector, spHijackAttr=spHijackAttr, interfaceUsageDone=interfaceUsageDone, mitigationThirdPartyStart=mitigationThirdPartyStart, spcommFailureDone=spcommFailureDone, dnsBaselineDone=dnsBaselineDone, spDNSObservedMax=spDNSObservedMax, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, managedObjectUsageDone=managedObjectUsageDone, spInterfaceIndex=spInterfaceIndex, peakflowSPMIB=peakflowSPMIB, spGreTunnelDestination=spGreTunnelDestination, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, collectorDown=collectorDown, spMitigationID=spMitigationID, spCommFailureDestination=spCommFailureDestination, dosHostDetectionAlert=dosHostDetectionAlert, spTMSCommunity=spTMSCommunity, configChange=configChange, spBlackholeTimeout=spBlackholeTimeout, spApplicationName=spApplicationName, spThirdPartyAddr=spThirdPartyAddr, spPravail=spPravail, spDNSName=spDNSName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBlackholePrefix=spBlackholePrefix, spcommFailure=spcommFailure, spDuration=spDuration, greDownDone=greDownDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, serviceUsage=serviceUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, licenseErrorDone=licenseErrorDone, deviceSystemErrorDone=deviceSystemErrorDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spFlowspecTimeout=spFlowspecTimeout, deviceTotalFlows=deviceTotalFlows, deviceTotalFlowsHC=deviceTotalFlowsHC, deviceSwapSpaceUsage=deviceSwapSpaceUsage, bgpTrap=bgpTrap, spReportID=spReportID, spThreshold=spThreshold, flowDown=flowDown, spFingerprintMessage=spFingerprintMessage, spMitigationName=spMitigationName, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spManagedObjects=spManagedObjects, licenseError=licenseError, spUnit=spUnit, fingerprintUsage=fingerprintUsage, spMitigationStart=spMitigationStart, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spInterface=spInterface, spManagedObjectFamily=spManagedObjectFamily, spDNSExpected=spDNSExpected, spFingerprintName=spFingerprintName, spUsername=spUsername, bgpInstabilityDone=bgpInstabilityDone, spCollector=spCollector, spUsageType=spUsageType, hardwareFailureDone=hardwareFailureDone, dnsBaseline=dnsBaseline, mitigationFlowspecStart=mitigationFlowspecStart, spImpactBps=spImpactBps, spInetAddressType=spInetAddressType, peakflowSPMgr=peakflowSPMgr, autoclassifyStart=autoclassifyStart, spBGPTrapEvent=spBGPTrapEvent, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spScriptCommand=spScriptCommand, bgpInstability=bgpInstability, spLicenseErrType=spLicenseErrType, mitigationTMSStartV2=mitigationTMSStartV2, spDNSObserved=spDNSObserved, spDetectedCountries=spDetectedCountries, collectorUp=collectorUp, fingerprintFeedback=fingerprintFeedback, spVersion=spVersion, spSystemErrorType=spSystemErrorType, spHardwareFailureDescription=spHardwareFailureDescription, mitigationDone=mitigationDone, spBGPInstability=spBGPInstability, notificationLimit=notificationLimit, mitigationTMSStart=mitigationTMSStart, fingerprintUsageDone=fingerprintUsageDone, serviceUsageDone=serviceUsageDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spScriptHost=spScriptHost, spScriptPort=spScriptPort, spTMSTimeout=spTMSTimeout, spLicenseErrDescription=spLicenseErrDescription, devicePhysicalMemory=devicePhysicalMemory, spBlackholeCommunity=spBlackholeCommunity, spThirdPartyZone=spThirdPartyZone, peakflowSPTraps=peakflowSPTraps, spReportName=spReportName, snmpDown=snmpDown, reportDone=reportDone, bgpHijack=bgpHijack, bgpUp=bgpUp, spBGPTrapName=spBGPTrapName)
