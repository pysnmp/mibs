...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spScriptHost=spScriptHost, deviceHealth=deviceHealth, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spImpactPps=spImpactPps, bgpInstability=bgpInstability, spBGPTrapEvent=spBGPTrapEvent, spInterfaceIndex=spInterfaceIndex, spPravail=spPravail, deviceSystemError=deviceSystemError, spScriptStart=spScriptStart, spHijackRoute=spHijackRoute, spBlackholePrefix=spBlackholePrefix, deviceDiskUsage=deviceDiskUsage, routingFailover=routingFailover, bgpInstabilityDone=bgpInstabilityDone, spDetectedCountries=spDetectedCountries, peakflowSPCMI=peakflowSPCMI, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spInterface=spInterface, spReportURL=spReportURL, spFingerprintName=spFingerprintName, spFingerprintMessage=spFingerprintMessage, spUsage=spUsage, spTMSTimeout=spTMSTimeout, bgpHijack=bgpHijack, mitigationTMSStart=mitigationTMSStart, spFlowspecTimeout=spFlowspecTimeout, spImpactBps=spImpactBps, reportDone=reportDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBGPTrapPrefix=spBGPTrapPrefix, spBGPTrapName=spBGPTrapName, mitigationFlowspecStart=mitigationFlowspecStart, PYSNMP_MODULE_ID=peakflowSPMIB, spSystemErrorDescription=spSystemErrorDescription, hardwareFailureDone=hardwareFailureDone, deviceTotalFlows=deviceTotalFlows, spTMSPrefix=spTMSPrefix, cloudSignalingFaultDone=cloudSignalingFaultDone, spMitigationStart=spMitigationStart, spInetAddress=spInetAddress, flowUp=flowUp, spcommFailureDone=spcommFailureDone, trafficAutoMitigation=trafficAutoMitigation, fingerprintReceive=fingerprintReceive, spInterfaceUsage=spInterfaceUsage, snmpDown=snmpDown, spApplicationName=spApplicationName, flowDown=flowDown, spTMSMultiPrefix=spTMSMultiPrefix, spThirdPartyZone=spThirdPartyZone, licenseErrorDone=licenseErrorDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, devicePhysicalMemory=devicePhysicalMemory, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spBGPInstability=spBGPInstability, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, greDown=greDown, spDNSObservedMean=spDNSObservedMean, spInterfaceUsageHC=spInterfaceUsageHC, spManagedObject=spManagedObject, fingerprintFeedback=fingerprintFeedback, spLicenseErrType=spLicenseErrType, spInterfaceSpeedHC=spInterfaceSpeedHC, dosHostDetectionAlertDone=dosHostDetectionAlertDone, licenseError=licenseError, spScriptPort=spScriptPort, spReportID=spReportID, spThirdPartyAddr=spThirdPartyAddr, spDNSExpected=spDNSExpected, spServiceName=spServiceName, deviceSwapSpace=deviceSwapSpace, dnsBaselineDone=dnsBaselineDone, spVersion=spVersion, spBlackholeTimeout=spBlackholeTimeout, deviceSwapSpaceUsage=deviceSwapSpaceUsage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, greDownDone=greDownDone, peakflowSPTraps=peakflowSPTraps, deviceTotalFlowsHC=deviceTotalFlowsHC, collectorUp=collectorUp, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, mitigationTMSStartV2=mitigationTMSStartV2, interfaceUsage=interfaceUsage, spCollector=spCollector, peakflowSPMgr=peakflowSPMgr, spInterfaceSpeed=spInterfaceSpeed, spcommFailure=spcommFailure, spDetector=spDetector, dnsBaseline=dnsBaseline, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, bgpDown=bgpDown, notificationLimit=notificationLimit, spServiceElement=spServiceElement, fingerprintUsage=fingerprintUsage, spCommFailureDestination=spCommFailureDestination, bgpUp=bgpUp, deviceSystemErrorDone=deviceSystemErrorDone, interfaceUsageDone=interfaceUsageDone, spBGPEvent=spBGPEvent, spHijackLocal=spHijackLocal, peakflowSPMIB=peakflowSPMIB, spRouter=spRouter, spThreshold=spThreshold, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBlackholeCommunity=spBlackholeCommunity, collectorDown=collectorDown, serviceUsage=serviceUsage, spUsername=spUsername, spFingerprintSender=spFingerprintSender, hardwareFailure=hardwareFailure, spAlertID=spAlertID, spLicenseErrDescription=spLicenseErrDescription, spManagedObjectFamily=spManagedObjectFamily, autoclassifyStart=autoclassifyStart, snmpUp=snmpUp, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spGreTunnelDestination=spGreTunnelDestination, spDuration=spDuration, cloudSignalingFault=cloudSignalingFault, alertScript=alertScript, spUnit=spUnit, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spLicenseErrCount=spLicenseErrCount, spMitigationID=spMitigationID, spLicenseErrLimit=spLicenseErrLimit, spDNSObservedMax=spDNSObservedMax, fingerprintUsageDone=fingerprintUsageDone, spReportName=spReportName, spBlackholeNexthop=spBlackholeNexthop, spFingerprintFeedback=spFingerprintFeedback, spCommFailureDescription=spCommFailureDescription, spTMSCommunity=spTMSCommunity, spDNSName=spDNSName, managedObjectUsage=managedObjectUsage, spMitigationName=spMitigationName, configChange=configChange, mitigationBlackholeStart=mitigationBlackholeStart, bgpTrap=bgpTrap, spSystemErrorType=spSystemErrorType, spInetAddressType=spInetAddressType, managedObjectUsageDone=managedObjectUsageDone, spScriptCommand=spScriptCommand, spFlowspecCommunity=spFlowspecCommunity, spHardwareFailureDescription=spHardwareFailureDescription, serviceUsageDone=serviceUsageDone, spManagedObjects=spManagedObjects, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spDNSObserved=spDNSObserved, dosHostDetectionAlert=dosHostDetectionAlert, mitigationDone=mitigationDone, spUsageType=spUsageType, spAlertDetectionSignatures=spAlertDetectionSignatures, spHijackAttr=spHijackAttr, spGreTunnelName=spGreTunnelName, collectorStart=collectorStart)
