...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spPravail=spPravail, spLicenseErrDescription=spLicenseErrDescription, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, flowUp=flowUp, spBlackholeTimeout=spBlackholeTimeout, spFingerprintFeedback=spFingerprintFeedback, spReportURL=spReportURL, deviceTotalFlowsHC=deviceTotalFlowsHC, licenseError=licenseError, spGreTunnelName=spGreTunnelName, greDown=greDown, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spScriptPort=spScriptPort, spFingerprintName=spFingerprintName, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spManagedObjects=spManagedObjects, spUsage=spUsage, spThreshold=spThreshold, spUsageType=spUsageType, fingerprintFeedback=fingerprintFeedback, spcommFailureDone=spcommFailureDone, autoclassifyStart=autoclassifyStart, spMitigationName=spMitigationName, serviceUsage=serviceUsage, mitigationTMSStartV2=mitigationTMSStartV2, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spInterface=spInterface, spLicenseErrLimit=spLicenseErrLimit, spTMSPrefix=spTMSPrefix, spDNSObservedMax=spDNSObservedMax, configChange=configChange, deviceDiskUsage=deviceDiskUsage, spInterfaceSpeed=spInterfaceSpeed, deviceSwapSpaceInUse=deviceSwapSpaceInUse, fingerprintReceive=fingerprintReceive, mitigationFlowspecStart=mitigationFlowspecStart, bgpHijack=bgpHijack, dosHostDetectionAlertDone=dosHostDetectionAlertDone, deviceTotalFlows=deviceTotalFlows, PYSNMP_MODULE_ID=peakflowSPMIB, spInterfaceUsage=spInterfaceUsage, spVersion=spVersion, spInterfaceSpeedHC=spInterfaceSpeedHC, bgpDown=bgpDown, spBGPTrapName=spBGPTrapName, spThirdPartyZone=spThirdPartyZone, spHijackRoute=spHijackRoute, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, dosHostDetectionAlert=dosHostDetectionAlert, spInterfaceIndex=spInterfaceIndex, cloudSignalingFaultDone=cloudSignalingFaultDone, spManagedObjectFamily=spManagedObjectFamily, trafficAutoMitigation=trafficAutoMitigation, spLicenseErrType=spLicenseErrType, devicePhysicalMemory=devicePhysicalMemory, spBGPTrapEvent=spBGPTrapEvent, spFingerprintSender=spFingerprintSender, spHijackAttr=spHijackAttr, peakflowSPMgr=peakflowSPMgr, spBlackholeNexthop=spBlackholeNexthop, spAlertID=spAlertID, spLicenseErrCount=spLicenseErrCount, mitigationBlackholeStart=mitigationBlackholeStart, spImpactPps=spImpactPps, greDownDone=greDownDone, bgpTrap=bgpTrap, licenseErrorDone=licenseErrorDone, spUsername=spUsername, spHardwareFailureDescription=spHardwareFailureDescription, managedObjectUsageDone=managedObjectUsageDone, spBGPEvent=spBGPEvent, alertScript=alertScript, fingerprintUsage=fingerprintUsage, spCommFailureDestination=spCommFailureDestination, deviceSystemErrorDone=deviceSystemErrorDone, peakflowSPMIB=peakflowSPMIB, reportDone=reportDone, spFlowspecCommunity=spFlowspecCommunity, spDetector=spDetector, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, dnsBaseline=dnsBaseline, mitigationThirdPartyStart=mitigationThirdPartyStart, spTMSMultiPrefix=spTMSMultiPrefix, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, snmpUp=snmpUp, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, collectorDown=collectorDown, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInetAddress=spInetAddress, deviceHealth=deviceHealth, snmpDown=snmpDown, bgpInstabilityDone=bgpInstabilityDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spCommFailureDescription=spCommFailureDescription, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spScriptStart=spScriptStart, hardwareFailure=hardwareFailure, dnsBaselineDone=dnsBaselineDone, spHijackLocal=spHijackLocal, spServiceElement=spServiceElement, spSystemErrorType=spSystemErrorType, spServiceName=spServiceName, spBGPTrapNewAttributes=spBGPTrapNewAttributes, cloudSignalingFault=cloudSignalingFault, spTMSCommunity=spTMSCommunity, peakflowSPTraps=peakflowSPTraps, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spManagedObject=spManagedObject, spDetectedCountries=spDetectedCountries, spFlowspecTimeout=spFlowspecTimeout, fingerprintUsageDone=fingerprintUsageDone, spReportName=spReportName, spMitigationStart=spMitigationStart, spDNSExpected=spDNSExpected, routingFailover=routingFailover, spInterfaceUsageHC=spInterfaceUsageHC, spcommFailure=spcommFailure, interfaceUsage=interfaceUsage, spCollector=spCollector, spDNSObserved=spDNSObserved, spFingerprintMessage=spFingerprintMessage, interfaceUsageDone=interfaceUsageDone, hardwareFailureDone=hardwareFailureDone, spBGPInstability=spBGPInstability, spUnit=spUnit, notificationLimit=notificationLimit, bgpInstability=bgpInstability, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spRouter=spRouter, spBlackholePrefix=spBlackholePrefix, serviceUsageDone=serviceUsageDone, collectorStart=collectorStart, mitigationTMSStart=mitigationTMSStart, spScriptHost=spScriptHost, spScriptCommand=spScriptCommand, spSystemErrorDescription=spSystemErrorDescription, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBlackholeCommunity=spBlackholeCommunity, bgpUp=bgpUp, mitigationDone=mitigationDone, peakflowSPCMI=peakflowSPCMI, spDuration=spDuration, collectorUp=collectorUp, managedObjectUsage=managedObjectUsage, spInetAddressType=spInetAddressType, spApplicationName=spApplicationName, spReportID=spReportID, spGreTunnelDestination=spGreTunnelDestination, spDNSObservedMean=spDNSObservedMean, spMitigationID=spMitigationID, spTMSTimeout=spTMSTimeout, deviceSystemError=deviceSystemError, deviceSwapSpace=deviceSwapSpace, spThirdPartyAddr=spThirdPartyAddr, spBGPTrapPrefix=spBGPTrapPrefix, spAlertDetectionSignatures=spAlertDetectionSignatures, spImpactBps=spImpactBps, flowDown=flowDown, spDNSName=spDNSName, spCloudSignalFaultDescription=spCloudSignalFaultDescription)
