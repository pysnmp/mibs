...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDNSName=spDNSName, spLicenseErrType=spLicenseErrType, mitigationBlackholeStart=mitigationBlackholeStart, spScriptPort=spScriptPort, autoclassifyStart=autoclassifyStart, fingerprintReceive=fingerprintReceive, spServiceName=spServiceName, spLicenseErrLimit=spLicenseErrLimit, spcommFailure=spcommFailure, dnsBaseline=dnsBaseline, spImpactBps=spImpactBps, spReportName=spReportName, mitigationDone=mitigationDone, spBGPTrapEvent=spBGPTrapEvent, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceDiskUsage=deviceDiskUsage, spFingerprintFeedback=spFingerprintFeedback, spDNSObserved=spDNSObserved, spTMSTimeout=spTMSTimeout, spcommFailureDone=spcommFailureDone, spUsage=spUsage, snmpUp=snmpUp, spThirdPartyZone=spThirdPartyZone, spCommFailureDescription=spCommFailureDescription, spScriptHost=spScriptHost, spBlackholeCommunity=spBlackholeCommunity, spServiceElement=spServiceElement, interfaceUsage=interfaceUsage, serviceUsage=serviceUsage, licenseErrorDone=licenseErrorDone, spSystemErrorDescription=spSystemErrorDescription, serviceUsageDone=serviceUsageDone, collectorDown=collectorDown, spAlertDetectionSignatures=spAlertDetectionSignatures, spBGPTrapOldAttributes=spBGPTrapOldAttributes, hardwareFailure=hardwareFailure, dnsBaselineDone=dnsBaselineDone, spDuration=spDuration, spReportID=spReportID, spFingerprintName=spFingerprintName, bgpUp=bgpUp, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spPravail=spPravail, spInetAddress=spInetAddress, spTMSCommunity=spTMSCommunity, flowDown=flowDown, peakflowSPTraps=peakflowSPTraps, spHijackLocal=spHijackLocal, spLicenseErrDescription=spLicenseErrDescription, spApplicationName=spApplicationName, configChange=configChange, spTMSPrefix=spTMSPrefix, notificationLimit=notificationLimit, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spScriptCommand=spScriptCommand, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, dosHostDetectionAlert=dosHostDetectionAlert, collectorUp=collectorUp, spDNSObservedMax=spDNSObservedMax, spSystemErrorType=spSystemErrorType, deviceSystemError=deviceSystemError, spDNSObservedMean=spDNSObservedMean, alertScript=alertScript, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spAlertID=spAlertID, bgpDown=bgpDown, spInterfaceUsageHC=spInterfaceUsageHC, spRouter=spRouter, spMitigationStart=spMitigationStart, bgpInstability=bgpInstability, mitigationFlowspecStart=mitigationFlowspecStart, spLicenseErrCount=spLicenseErrCount, bgpHijack=bgpHijack, fingerprintUsageDone=fingerprintUsageDone, spManagedObjectFamily=spManagedObjectFamily, spReportURL=spReportURL, spUnit=spUnit, collectorStart=collectorStart, hardwareFailureDone=hardwareFailureDone, PYSNMP_MODULE_ID=peakflowSPMIB, spFingerprintSender=spFingerprintSender, spScriptStart=spScriptStart, spInterface=spInterface, spBlackholePrefix=spBlackholePrefix, spVersion=spVersion, interfaceUsageDone=interfaceUsageDone, greDown=greDown, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceSystemErrorDone=deviceSystemErrorDone, spInterfaceSpeed=spInterfaceSpeed, spBGPTrapNewAttributes=spBGPTrapNewAttributes, mitigationThirdPartyStart=mitigationThirdPartyStart, flowUp=flowUp, mitigationTMSStart=mitigationTMSStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, managedObjectUsage=managedObjectUsage, spDetector=spDetector, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBGPTrapName=spBGPTrapName, deviceTotalFlows=deviceTotalFlows, spBlackholeNexthop=spBlackholeNexthop, spGreTunnelName=spGreTunnelName, reportDone=reportDone, managedObjectUsageDone=managedObjectUsageDone, deviceSwapSpace=deviceSwapSpace, spBGPEvent=spBGPEvent, routingFailover=routingFailover, spInterfaceUsage=spInterfaceUsage, spGreTunnelDestination=spGreTunnelDestination, spManagedObjects=spManagedObjects, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spFlowspecCommunity=spFlowspecCommunity, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, bgpTrap=bgpTrap, spInterfaceIndex=spInterfaceIndex, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, snmpDown=snmpDown, spUsername=spUsername, spImpactPps=spImpactPps, spThreshold=spThreshold, fingerprintFeedback=fingerprintFeedback, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spDNSExpected=spDNSExpected, spHijackAttr=spHijackAttr, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spFlowspecTimeout=spFlowspecTimeout, spUsageType=spUsageType, spHardwareFailureDescription=spHardwareFailureDescription, trafficAutoMitigation=trafficAutoMitigation, spBlackholeTimeout=spBlackholeTimeout, licenseError=licenseError, devicePhysicalMemory=devicePhysicalMemory, deviceSwapSpaceUsage=deviceSwapSpaceUsage, greDownDone=greDownDone, peakflowSPCMI=peakflowSPCMI, spHijackRoute=spHijackRoute, spMitigationName=spMitigationName, spBGPTrapPrefix=spBGPTrapPrefix, spBGPInstability=spBGPInstability, fingerprintUsage=fingerprintUsage, spTMSMultiPrefix=spTMSMultiPrefix, peakflowSPMIB=peakflowSPMIB, spMitigationID=spMitigationID, mitigationTMSStartV2=mitigationTMSStartV2, peakflowSPMgr=peakflowSPMgr, deviceTotalFlowsHC=deviceTotalFlowsHC, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spCommFailureDestination=spCommFailureDestination, bgpInstabilityDone=bgpInstabilityDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spInetAddressType=spInetAddressType, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceHealth=deviceHealth, spFingerprintMessage=spFingerprintMessage, spDetectedCountries=spDetectedCountries, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spThirdPartyAddr=spThirdPartyAddr, spManagedObject=spManagedObject, cloudSignalingFault=cloudSignalingFault, spCollector=spCollector)
