...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spGreTunnelDestination=spGreTunnelDestination, spLicenseErrCount=spLicenseErrCount, spInterfaceUsageHC=spInterfaceUsageHC, spInetAddressType=spInetAddressType, spDNSExpected=spDNSExpected, spBlackholeTimeout=spBlackholeTimeout, spDNSObservedMean=spDNSObservedMean, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spServiceName=spServiceName, peakflowSPMIB=peakflowSPMIB, collectorDown=collectorDown, spInterfaceUsage=spInterfaceUsage, licenseError=licenseError, bgpInstabilityDone=bgpInstabilityDone, spUsage=spUsage, spImpactPps=spImpactPps, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spHijackLocal=spHijackLocal, spDetector=spDetector, managedObjectUsageDone=managedObjectUsageDone, deviceDiskUsage=deviceDiskUsage, spFingerprintName=spFingerprintName, spFingerprintMessage=spFingerprintMessage, autoclassifyStart=autoclassifyStart, spUsername=spUsername, spBGPTrapNewAttributes=spBGPTrapNewAttributes, mitigationDone=mitigationDone, dnsBaselineDone=dnsBaselineDone, deviceSystemError=deviceSystemError, trafficAutoMitigation=trafficAutoMitigation, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSCommunity=spTMSCommunity, spCommFailureDescription=spCommFailureDescription, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, greDownDone=greDownDone, spApplicationName=spApplicationName, spDNSName=spDNSName, spImpactBps=spImpactBps, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spCommFailureDestination=spCommFailureDestination, bgpHijack=bgpHijack, spScriptPort=spScriptPort, cloudSignalingFaultDone=cloudSignalingFaultDone, spGreTunnelName=spGreTunnelName, mitigationTMSStartV2=mitigationTMSStartV2, spLicenseErrType=spLicenseErrType, spLicenseErrDescription=spLicenseErrDescription, spSystemErrorType=spSystemErrorType, spDNSObserved=spDNSObserved, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceTotalFlowsHC=deviceTotalFlowsHC, cloudSignalingFault=cloudSignalingFault, dosHostDetectionAlertDone=dosHostDetectionAlertDone, peakflowSPCMI=peakflowSPCMI, spMitigationStart=spMitigationStart, collectorUp=collectorUp, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, devicePhysicalMemory=devicePhysicalMemory, spMitigationID=spMitigationID, snmpUp=snmpUp, spInterface=spInterface, mitigationBlackholeStart=mitigationBlackholeStart, greDown=greDown, flowDown=flowDown, deviceSwapSpace=deviceSwapSpace, spTMSMultiPrefix=spTMSMultiPrefix, spThirdPartyZone=spThirdPartyZone, mitigationTMSStart=mitigationTMSStart, peakflowSPMgr=peakflowSPMgr, hardwareFailureDone=hardwareFailureDone, fingerprintReceive=fingerprintReceive, spRouter=spRouter, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spManagedObjects=spManagedObjects, spScriptCommand=spScriptCommand, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spReportID=spReportID, spManagedObjectFamily=spManagedObjectFamily, spCloudSignalFaultDescription=spCloudSignalFaultDescription, bgpDown=bgpDown, spInterfaceSpeedHC=spInterfaceSpeedHC, hardwareFailure=hardwareFailure, spBlackholeNexthop=spBlackholeNexthop, managedObjectUsage=managedObjectUsage, spDNSObservedMax=spDNSObservedMax, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spFlowspecTimeout=spFlowspecTimeout, collectorStart=collectorStart, spBlackholePrefix=spBlackholePrefix, snmpDown=snmpDown, interfaceUsageDone=interfaceUsageDone, bgpUp=bgpUp, deviceHealth=deviceHealth, spManagedObject=spManagedObject, spDuration=spDuration, fingerprintFeedback=fingerprintFeedback, spBlackholeCommunity=spBlackholeCommunity, spBGPTrapPrefix=spBGPTrapPrefix, spHardwareFailureDescription=spHardwareFailureDescription, spScriptHost=spScriptHost, reportDone=reportDone, notificationLimit=notificationLimit, spBGPTrapEvent=spBGPTrapEvent, serviceUsage=serviceUsage, spUnit=spUnit, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spFingerprintSender=spFingerprintSender, spAlertDetectionSignatures=spAlertDetectionSignatures, spReportURL=spReportURL, spThreshold=spThreshold, interfaceUsage=interfaceUsage, fingerprintUsageDone=fingerprintUsageDone, spReportName=spReportName, fingerprintUsage=fingerprintUsage, routingFailover=routingFailover, spMitigationName=spMitigationName, spThirdPartyAddr=spThirdPartyAddr, spUsageType=spUsageType, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spLicenseErrLimit=spLicenseErrLimit, spCollector=spCollector, licenseErrorDone=licenseErrorDone, spFingerprintFeedback=spFingerprintFeedback, alertScript=alertScript, configChange=configChange, spBGPEvent=spBGPEvent, bgpTrap=bgpTrap, spVersion=spVersion, spBGPInstability=spBGPInstability, spFlowspecCommunity=spFlowspecCommunity, spServiceElement=spServiceElement, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spAlertID=spAlertID, spInterfaceSpeed=spInterfaceSpeed, deviceSystemErrorDone=deviceSystemErrorDone, deviceTotalFlows=deviceTotalFlows, spInterfaceIndex=spInterfaceIndex, spTMSTimeout=spTMSTimeout, mitigationThirdPartyStart=mitigationThirdPartyStart, spHijackRoute=spHijackRoute, spSystemErrorDescription=spSystemErrorDescription, bgpInstability=bgpInstability, spDetectedCountries=spDetectedCountries, spcommFailureDone=spcommFailureDone, mitigationFlowspecStart=mitigationFlowspecStart, flowUp=flowUp, peakflowSPTraps=peakflowSPTraps, spScriptStart=spScriptStart, spInetAddress=spInetAddress, dnsBaseline=dnsBaseline, spBGPTrapName=spBGPTrapName, spTMSPrefix=spTMSPrefix, spPravail=spPravail, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, PYSNMP_MODULE_ID=peakflowSPMIB, dosHostDetectionAlert=dosHostDetectionAlert, spcommFailure=spcommFailure, serviceUsageDone=serviceUsageDone, spHijackAttr=spHijackAttr)
