...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", devicePhysicalMemory=devicePhysicalMemory, collectorUp=collectorUp, spFingerprintMessage=spFingerprintMessage, peakflowSPMgr=peakflowSPMgr, bgpTrap=bgpTrap, spVersion=spVersion, spHijackAttr=spHijackAttr, spDNSObserved=spDNSObserved, spDNSObservedMean=spDNSObservedMean, spFingerprintSender=spFingerprintSender, dosHostDetectionAlert=dosHostDetectionAlert, greDownDone=greDownDone, spServiceName=spServiceName, spThirdPartyZone=spThirdPartyZone, mitigationTMSStart=mitigationTMSStart, spManagedObjectFamily=spManagedObjectFamily, spFingerprintFeedback=spFingerprintFeedback, reportDone=reportDone, spTMSCommunity=spTMSCommunity, spGreTunnelDestination=spGreTunnelDestination, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceSwapSpaceInUse=deviceSwapSpaceInUse, mitigationTMSStartV2=mitigationTMSStartV2, trafficAutoMitigation=trafficAutoMitigation, mitigationBlackholeStart=mitigationBlackholeStart, spBGPTrapPrefix=spBGPTrapPrefix, spLicenseErrType=spLicenseErrType, notificationLimit=notificationLimit, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spApplicationName=spApplicationName, cloudSignalingFaultDone=cloudSignalingFaultDone, spAlertID=spAlertID, dnsBaselineDone=dnsBaselineDone, spUnit=spUnit, spLicenseErrCount=spLicenseErrCount, spInetAddressType=spInetAddressType, interfaceUsage=interfaceUsage, spRouter=spRouter, snmpDown=snmpDown, spDNSObservedMax=spDNSObservedMax, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spScriptStart=spScriptStart, autoclassifyStart=autoclassifyStart, spInterfaceSpeed=spInterfaceSpeed, spDetector=spDetector, spSystemErrorDescription=spSystemErrorDescription, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, hardwareFailure=hardwareFailure, spDNSName=spDNSName, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceSystemError=deviceSystemError, mitigationDone=mitigationDone, spcommFailure=spcommFailure, interfaceUsageDone=interfaceUsageDone, deviceSwapSpace=deviceSwapSpace, spCommFailureDestination=spCommFailureDestination, spScriptCommand=spScriptCommand, spBGPEvent=spBGPEvent, mitigationFlowspecStart=mitigationFlowspecStart, fingerprintReceive=fingerprintReceive, spCommFailureDescription=spCommFailureDescription, managedObjectUsageDone=managedObjectUsageDone, spBGPInstability=spBGPInstability, deviceSwapSpaceUsage=deviceSwapSpaceUsage, bgpInstabilityDone=bgpInstabilityDone, spTMSPrefix=spTMSPrefix, PYSNMP_MODULE_ID=peakflowSPMIB, spBGPTrapName=spBGPTrapName, peakflowSPCMI=peakflowSPCMI, spMitigationStart=spMitigationStart, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spUsageType=spUsageType, greDown=greDown, spCollector=spCollector, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, snmpUp=snmpUp, spcommFailureDone=spcommFailureDone, spBlackholeTimeout=spBlackholeTimeout, spLicenseErrDescription=spLicenseErrDescription, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spGreTunnelName=spGreTunnelName, collectorDown=collectorDown, managedObjectUsage=managedObjectUsage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, configChange=configChange, spImpactBps=spImpactBps, licenseError=licenseError, spUsername=spUsername, spImpactPps=spImpactPps, alertScript=alertScript, deviceHealth=deviceHealth, bgpUp=bgpUp, mitigationThirdPartyStart=mitigationThirdPartyStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spPravail=spPravail, spInetAddress=spInetAddress, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, bgpHijack=bgpHijack, spMitigationID=spMitigationID, spReportID=spReportID, spBlackholeCommunity=spBlackholeCommunity, spSystemErrorType=spSystemErrorType, hardwareFailureDone=hardwareFailureDone, spBGPTrapEvent=spBGPTrapEvent, fingerprintUsageDone=fingerprintUsageDone, spManagedObjects=spManagedObjects, spScriptPort=spScriptPort, peakflowSPMIB=peakflowSPMIB, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpInstability=bgpInstability, spMitigationName=spMitigationName, collectorStart=collectorStart, deviceSystemErrorDone=deviceSystemErrorDone, serviceUsageDone=serviceUsageDone, routingFailover=routingFailover, spServiceElement=spServiceElement, spLicenseErrLimit=spLicenseErrLimit, spInterfaceIndex=spInterfaceIndex, fingerprintUsage=fingerprintUsage, flowUp=flowUp, spDuration=spDuration, bgpDown=bgpDown, fingerprintFeedback=fingerprintFeedback, spReportName=spReportName, flowDown=flowDown, spBlackholePrefix=spBlackholePrefix, dnsBaseline=dnsBaseline, spFlowspecCommunity=spFlowspecCommunity, spHardwareFailureDescription=spHardwareFailureDescription, spInterface=spInterface, spHijackLocal=spHijackLocal, deviceDiskUsage=deviceDiskUsage, spInterfaceUsage=spInterfaceUsage, spDNSExpected=spDNSExpected, spFingerprintName=spFingerprintName, spScriptHost=spScriptHost, spThirdPartyAddr=spThirdPartyAddr, spTMSTimeout=spTMSTimeout, spHijackRoute=spHijackRoute, licenseErrorDone=licenseErrorDone, spDetectedCountries=spDetectedCountries, spFlowspecTimeout=spFlowspecTimeout, spUsage=spUsage, spManagedObject=spManagedObject, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBlackholeNexthop=spBlackholeNexthop, spInterfaceUsageHC=spInterfaceUsageHC, spReportURL=spReportURL, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spTMSMultiPrefix=spTMSMultiPrefix, deviceTotalFlows=deviceTotalFlows, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, peakflowSPTraps=peakflowSPTraps, spInterfaceSpeedHC=spInterfaceSpeedHC, spThreshold=spThreshold, cloudSignalingFault=cloudSignalingFault, serviceUsage=serviceUsage)
