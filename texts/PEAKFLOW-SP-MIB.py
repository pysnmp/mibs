...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInetAddress=spInetAddress, spGreTunnelName=spGreTunnelName, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spImpactPps=spImpactPps, spSystemErrorType=spSystemErrorType, hardwareFailure=hardwareFailure, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, fingerprintUsage=fingerprintUsage, spUsername=spUsername, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, mitigationTMSStartV2=mitigationTMSStartV2, serviceUsage=serviceUsage, spBGPInstability=spBGPInstability, spInterfaceUsageHC=spInterfaceUsageHC, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, snmpUp=snmpUp, spMitigationName=spMitigationName, mitigationTMSStart=mitigationTMSStart, spHijackRoute=spHijackRoute, spPravail=spPravail, collectorDown=collectorDown, managedObjectUsage=managedObjectUsage, spHardwareFailureDescription=spHardwareFailureDescription, spDNSName=spDNSName, spDNSObservedMax=spDNSObservedMax, spScriptPort=spScriptPort, spScriptStart=spScriptStart, peakflowSPTraps=peakflowSPTraps, spTMSTimeout=spTMSTimeout, spLicenseErrType=spLicenseErrType, bgpHijack=bgpHijack, spcommFailureDone=spcommFailureDone, spUsage=spUsage, deviceSystemErrorDone=deviceSystemErrorDone, licenseErrorDone=licenseErrorDone, deviceSwapSpace=deviceSwapSpace, deviceSystemError=deviceSystemError, cloudSignalingFaultDone=cloudSignalingFaultDone, spServiceElement=spServiceElement, flowUp=flowUp, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDetector=spDetector, hardwareFailureDone=hardwareFailureDone, spcommFailure=spcommFailure, spServiceName=spServiceName, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, peakflowSPCMI=peakflowSPCMI, notificationLimit=notificationLimit, reportDone=reportDone, spFingerprintMessage=spFingerprintMessage, deviceHealth=deviceHealth, spDuration=spDuration, trafficAutoMitigation=trafficAutoMitigation, spBGPTrapEvent=spBGPTrapEvent, spApplicationName=spApplicationName, spHijackLocal=spHijackLocal, spThirdPartyZone=spThirdPartyZone, mitigationDone=mitigationDone, spAlertDetectionSignatures=spAlertDetectionSignatures, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spCommFailureDescription=spCommFailureDescription, spThreshold=spThreshold, spGreTunnelDestination=spGreTunnelDestination, spAlertID=spAlertID, dnsBaselineDone=dnsBaselineDone, spTMSPrefix=spTMSPrefix, deviceDiskUsage=deviceDiskUsage, spCloudSignalFaultDescription=spCloudSignalFaultDescription, bgpUp=bgpUp, spManagedObjects=spManagedObjects, spInterfaceSpeed=spInterfaceSpeed, greDownDone=greDownDone, dosHostDetectionAlert=dosHostDetectionAlert, spFlowspecTimeout=spFlowspecTimeout, spDNSObservedMean=spDNSObservedMean, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, fingerprintReceive=fingerprintReceive, flowDown=flowDown, alertScript=alertScript, spFingerprintSender=spFingerprintSender, spReportURL=spReportURL, spInterfaceSpeedHC=spInterfaceSpeedHC, spInetAddressType=spInetAddressType, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, PYSNMP_MODULE_ID=peakflowSPMIB, devicePhysicalMemory=devicePhysicalMemory, interfaceUsage=interfaceUsage, spUsageType=spUsageType, cloudSignalingFault=cloudSignalingFault, spMitigationStart=spMitigationStart, spFingerprintFeedback=spFingerprintFeedback, bgpInstabilityDone=bgpInstabilityDone, spBlackholePrefix=spBlackholePrefix, spLicenseErrLimit=spLicenseErrLimit, autoclassifyStart=autoclassifyStart, spMitigationID=spMitigationID, routingFailover=routingFailover, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spSystemErrorDescription=spSystemErrorDescription, spDetectedCountries=spDetectedCountries, spBGPEvent=spBGPEvent, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, fingerprintFeedback=fingerprintFeedback, greDown=greDown, spScriptHost=spScriptHost, fingerprintUsageDone=fingerprintUsageDone, spVersion=spVersion, collectorStart=collectorStart, spManagedObjectFamily=spManagedObjectFamily, spImpactBps=spImpactBps, deviceTotalFlows=deviceTotalFlows, snmpDown=snmpDown, collectorUp=collectorUp, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInterface=spInterface, deviceTotalFlowsHC=deviceTotalFlowsHC, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, mitigationFlowspecStart=mitigationFlowspecStart, managedObjectUsageDone=managedObjectUsageDone, spFingerprintName=spFingerprintName, dosHostDetectionAlertDone=dosHostDetectionAlertDone, serviceUsageDone=serviceUsageDone, spBlackholeNexthop=spBlackholeNexthop, licenseError=licenseError, mitigationBlackholeStart=mitigationBlackholeStart, spDNSObserved=spDNSObserved, spDNSExpected=spDNSExpected, spReportID=spReportID, configChange=configChange, spUnit=spUnit, spRouter=spRouter, spLicenseErrCount=spLicenseErrCount, spBlackholeTimeout=spBlackholeTimeout, bgpDown=bgpDown, spInterfaceUsage=spInterfaceUsage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBGPTrapPrefix=spBGPTrapPrefix, interfaceUsageDone=interfaceUsageDone, spHijackAttr=spHijackAttr, spReportName=spReportName, mitigationThirdPartyStart=mitigationThirdPartyStart, spBlackholeCommunity=spBlackholeCommunity, bgpTrap=bgpTrap, spBGPTrapName=spBGPTrapName, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spThirdPartyAddr=spThirdPartyAddr, peakflowSPMgr=peakflowSPMgr, spFlowspecCommunity=spFlowspecCommunity, bgpInstability=bgpInstability, peakflowSPMIB=peakflowSPMIB, spScriptCommand=spScriptCommand, spTMSMultiPrefix=spTMSMultiPrefix, spCollector=spCollector, spManagedObject=spManagedObject, spInterfaceIndex=spInterfaceIndex, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spCommFailureDestination=spCommFailureDestination, dnsBaseline=dnsBaseline, spTMSCommunity=spTMSCommunity)
