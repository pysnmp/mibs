...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", dosHostDetectionAlert=dosHostDetectionAlert, spReportName=spReportName, spThirdPartyZone=spThirdPartyZone, spBGPEvent=spBGPEvent, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spUsage=spUsage, greDown=greDown, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, PYSNMP_MODULE_ID=peakflowSPMIB, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spHijackLocal=spHijackLocal, spManagedObject=spManagedObject, spDuration=spDuration, spLicenseErrCount=spLicenseErrCount, spFingerprintSender=spFingerprintSender, spLicenseErrDescription=spLicenseErrDescription, spHardwareFailureDescription=spHardwareFailureDescription, spDNSObservedMean=spDNSObservedMean, fingerprintUsage=fingerprintUsage, serviceUsage=serviceUsage, reportDone=reportDone, spBGPTrapPrefix=spBGPTrapPrefix, spFingerprintMessage=spFingerprintMessage, spAlertID=spAlertID, snmpDown=snmpDown, spBlackholePrefix=spBlackholePrefix, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spcommFailure=spcommFailure, interfaceUsageDone=interfaceUsageDone, deviceSwapSpace=deviceSwapSpace, spThirdPartyAddr=spThirdPartyAddr, spUsername=spUsername, trafficAutoMitigation=trafficAutoMitigation, deviceHealth=deviceHealth, interfaceUsage=interfaceUsage, spInterfaceIndex=spInterfaceIndex, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceTotalFlowsHC=deviceTotalFlowsHC, spMitigationID=spMitigationID, fingerprintUsageDone=fingerprintUsageDone, spFlowspecTimeout=spFlowspecTimeout, managedObjectUsage=managedObjectUsage, spCommFailureDescription=spCommFailureDescription, spTMSPrefix=spTMSPrefix, spTMSTimeout=spTMSTimeout, spBGPTrapEvent=spBGPTrapEvent, spGreTunnelDestination=spGreTunnelDestination, hardwareFailureDone=hardwareFailureDone, peakflowSPCMI=peakflowSPCMI, spFlowspecCommunity=spFlowspecCommunity, spTMSMultiPrefix=spTMSMultiPrefix, collectorUp=collectorUp, bgpInstability=bgpInstability, cloudSignalingFaultDone=cloudSignalingFaultDone, spDetectedCountries=spDetectedCountries, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInetAddressType=spInetAddressType, spPravail=spPravail, spServiceName=spServiceName, spVersion=spVersion, notificationLimit=notificationLimit, spFingerprintName=spFingerprintName, spScriptHost=spScriptHost, spLicenseErrType=spLicenseErrType, spScriptCommand=spScriptCommand, collectorStart=collectorStart, autoclassifyStart=autoclassifyStart, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, serviceUsageDone=serviceUsageDone, spUsageType=spUsageType, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spDNSExpected=spDNSExpected, configChange=configChange, deviceTotalFlows=deviceTotalFlows, bgpTrap=bgpTrap, spImpactPps=spImpactPps, spBGPTrapName=spBGPTrapName, mitigationBlackholeStart=mitigationBlackholeStart, spScriptStart=spScriptStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceDiskUsage=deviceDiskUsage, peakflowSPTraps=peakflowSPTraps, spInterfaceUsageHC=spInterfaceUsageHC, spDetector=spDetector, spBGPInstability=spBGPInstability, cloudSignalingFault=cloudSignalingFault, alertScript=alertScript, spApplicationName=spApplicationName, bgpUp=bgpUp, hardwareFailure=hardwareFailure, mitigationTMSStart=mitigationTMSStart, routingFailover=routingFailover, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spServiceElement=spServiceElement, spManagedObjectFamily=spManagedObjectFamily, spMitigationStart=spMitigationStart, licenseErrorDone=licenseErrorDone, spTMSCommunity=spTMSCommunity, spSystemErrorDescription=spSystemErrorDescription, mitigationFlowspecStart=mitigationFlowspecStart, spHijackRoute=spHijackRoute, deviceSwapSpaceInUse=deviceSwapSpaceInUse, collectorDown=collectorDown, bgpInstabilityDone=bgpInstabilityDone, spcommFailureDone=spcommFailureDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spManagedObjects=spManagedObjects, spBlackholeNexthop=spBlackholeNexthop, spBlackholeCommunity=spBlackholeCommunity, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBlackholeTimeout=spBlackholeTimeout, greDownDone=greDownDone, spCommFailureDestination=spCommFailureDestination, flowUp=flowUp, spSystemErrorType=spSystemErrorType, spInetAddress=spInetAddress, spGreTunnelName=spGreTunnelName, spRouter=spRouter, snmpUp=snmpUp, spReportURL=spReportURL, spScriptPort=spScriptPort, fingerprintFeedback=fingerprintFeedback, spFingerprintFeedback=spFingerprintFeedback, mitigationThirdPartyStart=mitigationThirdPartyStart, spMitigationName=spMitigationName, spInterfaceSpeedHC=spInterfaceSpeedHC, fingerprintReceive=fingerprintReceive, mitigationDone=mitigationDone, deviceSystemErrorDone=deviceSystemErrorDone, spHijackAttr=spHijackAttr, spDNSObserved=spDNSObserved, bgpDown=bgpDown, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, devicePhysicalMemory=devicePhysicalMemory, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceSystemError=deviceSystemError, spInterfaceSpeed=spInterfaceSpeed, dnsBaselineDone=dnsBaselineDone, spDNSObservedMax=spDNSObservedMax, spInterface=spInterface, spThreshold=spThreshold, bgpHijack=bgpHijack, peakflowSPMgr=peakflowSPMgr, managedObjectUsageDone=managedObjectUsageDone, flowDown=flowDown, spCollector=spCollector, spReportID=spReportID, spDNSName=spDNSName, spInterfaceUsage=spInterfaceUsage, dnsBaseline=dnsBaseline, spLicenseErrLimit=spLicenseErrLimit, mitigationTMSStartV2=mitigationTMSStartV2, peakflowSPMIB=peakflowSPMIB, spUnit=spUnit, spImpactBps=spImpactBps, licenseError=licenseError)
