...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spCommFailureDescription=spCommFailureDescription, spAlertDetectionSignatures=spAlertDetectionSignatures, spBGPTrapPrefix=spBGPTrapPrefix, spAlertID=spAlertID, spManagedObjectFamily=spManagedObjectFamily, spVersion=spVersion, deviceSystemErrorDone=deviceSystemErrorDone, dnsBaseline=dnsBaseline, spLicenseErrType=spLicenseErrType, bgpInstability=bgpInstability, mitigationBlackholeStart=mitigationBlackholeStart, spDetector=spDetector, deviceSwapSpace=deviceSwapSpace, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, interfaceUsageDone=interfaceUsageDone, snmpUp=snmpUp, spHijackRoute=spHijackRoute, spCommFailureDestination=spCommFailureDestination, bgpTrap=bgpTrap, fingerprintReceive=fingerprintReceive, spReportName=spReportName, snmpDown=snmpDown, spMitigationID=spMitigationID, spHijackLocal=spHijackLocal, alertScript=alertScript, PYSNMP_MODULE_ID=peakflowSPMIB, spInetAddressType=spInetAddressType, spFingerprintMessage=spFingerprintMessage, mitigationThirdPartyStart=mitigationThirdPartyStart, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spRouter=spRouter, spReportID=spReportID, deviceTotalFlowsHC=deviceTotalFlowsHC, collectorStart=collectorStart, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spTMSPrefix=spTMSPrefix, peakflowSPMIB=peakflowSPMIB, spFingerprintSender=spFingerprintSender, mitigationTMSStart=mitigationTMSStart, deviceTotalFlows=deviceTotalFlows, spInterface=spInterface, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spThreshold=spThreshold, mitigationTMSStartV2=mitigationTMSStartV2, spTMSMultiPrefix=spTMSMultiPrefix, flowUp=flowUp, dosHostDetectionAlertDone=dosHostDetectionAlertDone, serviceUsage=serviceUsage, bgpDown=bgpDown, autoclassifyStart=autoclassifyStart, spBlackholeTimeout=spBlackholeTimeout, trafficAutoMitigation=trafficAutoMitigation, spPravail=spPravail, dosNetworkProfiledAlert=dosNetworkProfiledAlert, licenseError=licenseError, deviceDiskUsage=deviceDiskUsage, spServiceElement=spServiceElement, spServiceName=spServiceName, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spUnit=spUnit, devicePhysicalMemory=devicePhysicalMemory, greDownDone=greDownDone, spcommFailureDone=spcommFailureDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spcommFailure=spcommFailure, managedObjectUsage=managedObjectUsage, spBGPTrapName=spBGPTrapName, cloudSignalingFault=cloudSignalingFault, spDNSObservedMean=spDNSObservedMean, spFingerprintFeedback=spFingerprintFeedback, spScriptCommand=spScriptCommand, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, mitigationFlowspecStart=mitigationFlowspecStart, licenseErrorDone=licenseErrorDone, spLicenseErrDescription=spLicenseErrDescription, bgpInstabilityDone=bgpInstabilityDone, spManagedObject=spManagedObject, notificationLimit=notificationLimit, spScriptHost=spScriptHost, collectorDown=collectorDown, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spUsername=spUsername, spBGPEvent=spBGPEvent, hardwareFailureDone=hardwareFailureDone, spTMSCommunity=spTMSCommunity, spDetectedCountries=spDetectedCountries, spThirdPartyAddr=spThirdPartyAddr, spDuration=spDuration, configChange=configChange, dnsBaselineDone=dnsBaselineDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spImpactPps=spImpactPps, spLicenseErrCount=spLicenseErrCount, spInetAddress=spInetAddress, bgpHijack=bgpHijack, spBlackholePrefix=spBlackholePrefix, spCollector=spCollector, spSystemErrorDescription=spSystemErrorDescription, bgpUp=bgpUp, spInterfaceSpeedHC=spInterfaceSpeedHC, mitigationDone=mitigationDone, spManagedObjects=spManagedObjects, hardwareFailure=hardwareFailure, spInterfaceUsageHC=spInterfaceUsageHC, routingFailover=routingFailover, spFlowspecCommunity=spFlowspecCommunity, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spHijackAttr=spHijackAttr, peakflowSPMgr=peakflowSPMgr, spBGPInstability=spBGPInstability, spDNSName=spDNSName, spUsage=spUsage, spInterfaceIndex=spInterfaceIndex, flowDown=flowDown, spHardwareFailureDescription=spHardwareFailureDescription, interfaceUsage=interfaceUsage, deviceHealth=deviceHealth, spMitigationName=spMitigationName, fingerprintUsageDone=fingerprintUsageDone, spUsageType=spUsageType, spReportURL=spReportURL, spScriptPort=spScriptPort, fingerprintUsage=fingerprintUsage, spThirdPartyZone=spThirdPartyZone, spSystemErrorType=spSystemErrorType, spFlowspecTimeout=spFlowspecTimeout, spApplicationName=spApplicationName, spDNSObserved=spDNSObserved, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spGreTunnelDestination=spGreTunnelDestination, spGreTunnelName=spGreTunnelName, managedObjectUsageDone=managedObjectUsageDone, spBlackholeNexthop=spBlackholeNexthop, cloudSignalingFaultDone=cloudSignalingFaultDone, serviceUsageDone=serviceUsageDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInterfaceUsage=spInterfaceUsage, spTMSTimeout=spTMSTimeout, spBlackholeCommunity=spBlackholeCommunity, collectorUp=collectorUp, reportDone=reportDone, spFingerprintName=spFingerprintName, deviceSystemError=deviceSystemError, fingerprintFeedback=fingerprintFeedback, greDown=greDown, spLicenseErrLimit=spLicenseErrLimit, spInterfaceSpeed=spInterfaceSpeed, dosHostDetectionAlert=dosHostDetectionAlert, spDNSExpected=spDNSExpected, peakflowSPCMI=peakflowSPCMI, spMitigationStart=spMitigationStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSObservedMax=spDNSObservedMax, spImpactBps=spImpactBps, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spScriptStart=spScriptStart, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBGPTrapEvent=spBGPTrapEvent, spCloudSignalFaultDescription=spCloudSignalFaultDescription, peakflowSPTraps=peakflowSPTraps)
