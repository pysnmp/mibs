...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spGreTunnelName=spGreTunnelName, spTMSPrefix=spTMSPrefix, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spImpactBps=spImpactBps, spThreshold=spThreshold, mitigationTMSStart=mitigationTMSStart, spAlertDetectionSignatures=spAlertDetectionSignatures, spInterfaceUsage=spInterfaceUsage, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSObservedMean=spDNSObservedMean, spMitigationStart=spMitigationStart, snmpDown=snmpDown, spCommFailureDestination=spCommFailureDestination, fingerprintUsage=fingerprintUsage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, mitigationDone=mitigationDone, flowDown=flowDown, configChange=configChange, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spMitigationName=spMitigationName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spManagedObjects=spManagedObjects, deviceHealth=deviceHealth, bgpUp=bgpUp, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spUnit=spUnit, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, notificationLimit=notificationLimit, spFingerprintName=spFingerprintName, mitigationFlowspecStart=mitigationFlowspecStart, fingerprintReceive=fingerprintReceive, spFlowspecTimeout=spFlowspecTimeout, spImpactPps=spImpactPps, spScriptPort=spScriptPort, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, deviceSystemErrorDone=deviceSystemErrorDone, collectorDown=collectorDown, spFingerprintMessage=spFingerprintMessage, bgpTrap=bgpTrap, spCommFailureDescription=spCommFailureDescription, spUsageType=spUsageType, spInterfaceUsageHC=spInterfaceUsageHC, spManagedObject=spManagedObject, mitigationBlackholeStart=mitigationBlackholeStart, mitigationTMSStartV2=mitigationTMSStartV2, spPravail=spPravail, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spScriptCommand=spScriptCommand, spTMSMultiPrefix=spTMSMultiPrefix, serviceUsage=serviceUsage, spTMSCommunity=spTMSCommunity, spDetectedCountries=spDetectedCountries, fingerprintUsageDone=fingerprintUsageDone, spBlackholeNexthop=spBlackholeNexthop, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDNSName=spDNSName, greDown=greDown, PYSNMP_MODULE_ID=peakflowSPMIB, spCollector=spCollector, spMitigationID=spMitigationID, spThirdPartyAddr=spThirdPartyAddr, spBlackholeCommunity=spBlackholeCommunity, serviceUsageDone=serviceUsageDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spReportID=spReportID, interfaceUsage=interfaceUsage, spBlackholePrefix=spBlackholePrefix, bgpInstabilityDone=bgpInstabilityDone, deviceTotalFlows=deviceTotalFlows, spLicenseErrDescription=spLicenseErrDescription, spLicenseErrCount=spLicenseErrCount, spLicenseErrType=spLicenseErrType, spFingerprintFeedback=spFingerprintFeedback, fingerprintFeedback=fingerprintFeedback, cloudSignalingFaultDone=cloudSignalingFaultDone, spThirdPartyZone=spThirdPartyZone, spDuration=spDuration, devicePhysicalMemory=devicePhysicalMemory, snmpUp=snmpUp, spScriptHost=spScriptHost, spServiceName=spServiceName, bgpDown=bgpDown, dnsBaselineDone=dnsBaselineDone, spDNSExpected=spDNSExpected, spFingerprintSender=spFingerprintSender, reportDone=reportDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, dosHostDetectionAlert=dosHostDetectionAlert, spLicenseErrLimit=spLicenseErrLimit, spVersion=spVersion, licenseError=licenseError, spApplicationName=spApplicationName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spDNSObserved=spDNSObserved, collectorStart=collectorStart, spBGPTrapName=spBGPTrapName, peakflowSPMgr=peakflowSPMgr, bgpHijack=bgpHijack, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spSystemErrorType=spSystemErrorType, spBGPEvent=spBGPEvent, hardwareFailureDone=hardwareFailureDone, spReportName=spReportName, spDNSObservedMax=spDNSObservedMax, flowUp=flowUp, deviceDiskUsage=deviceDiskUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spHardwareFailureDescription=spHardwareFailureDescription, spTMSTimeout=spTMSTimeout, spcommFailure=spcommFailure, spAlertID=spAlertID, spBGPTrapEvent=spBGPTrapEvent, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spcommFailureDone=spcommFailureDone, spUsage=spUsage, autoclassifyStart=autoclassifyStart, deviceTotalFlowsHC=deviceTotalFlowsHC, spReportURL=spReportURL, spSystemErrorDescription=spSystemErrorDescription, routingFailover=routingFailover, spRouter=spRouter, spHijackLocal=spHijackLocal, peakflowSPTraps=peakflowSPTraps, spInterface=spInterface, spHijackRoute=spHijackRoute, spManagedObjectFamily=spManagedObjectFamily, alertScript=alertScript, hardwareFailure=hardwareFailure, spInterfaceIndex=spInterfaceIndex, spBGPInstability=spBGPInstability, spServiceElement=spServiceElement, mitigationThirdPartyStart=mitigationThirdPartyStart, spGreTunnelDestination=spGreTunnelDestination, deviceSystemError=deviceSystemError, spBlackholeTimeout=spBlackholeTimeout, licenseErrorDone=licenseErrorDone, spFlowspecCommunity=spFlowspecCommunity, spHijackAttr=spHijackAttr, managedObjectUsage=managedObjectUsage, bgpInstability=bgpInstability, managedObjectUsageDone=managedObjectUsageDone, peakflowSPCMI=peakflowSPCMI, spInterfaceSpeed=spInterfaceSpeed, dnsBaseline=dnsBaseline, collectorUp=collectorUp, spDetector=spDetector, greDownDone=greDownDone, spUsername=spUsername, deviceSwapSpace=deviceSwapSpace, spInetAddressType=spInetAddressType, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spScriptStart=spScriptStart, spInetAddress=spInetAddress, spBGPTrapPrefix=spBGPTrapPrefix, cloudSignalingFault=cloudSignalingFault, trafficAutoMitigation=trafficAutoMitigation, interfaceUsageDone=interfaceUsageDone, peakflowSPMIB=peakflowSPMIB, dosHostDetectionAlertDone=dosHostDetectionAlertDone)
