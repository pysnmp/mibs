...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceHealth=deviceHealth, spScriptPort=spScriptPort, spThirdPartyZone=spThirdPartyZone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spcommFailureDone=spcommFailureDone, spVersion=spVersion, spDNSName=spDNSName, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spGreTunnelDestination=spGreTunnelDestination, fingerprintReceive=fingerprintReceive, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceSystemErrorDone=deviceSystemErrorDone, spDuration=spDuration, interfaceUsageDone=interfaceUsageDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spUsername=spUsername, licenseError=licenseError, spHijackLocal=spHijackLocal, bgpDown=bgpDown, spInterfaceUsage=spInterfaceUsage, spAlertID=spAlertID, spDNSObservedMean=spDNSObservedMean, spScriptHost=spScriptHost, spMitigationID=spMitigationID, spCommFailureDestination=spCommFailureDestination, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, collectorUp=collectorUp, spBlackholeTimeout=spBlackholeTimeout, spUsage=spUsage, peakflowSPMIB=peakflowSPMIB, spFingerprintSender=spFingerprintSender, fingerprintFeedback=fingerprintFeedback, deviceSystemError=deviceSystemError, mitigationFlowspecStart=mitigationFlowspecStart, spBlackholeNexthop=spBlackholeNexthop, mitigationTMSStartV2=mitigationTMSStartV2, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spDNSObservedMax=spDNSObservedMax, greDownDone=greDownDone, spInterfaceUsageHC=spInterfaceUsageHC, peakflowSPCMI=peakflowSPCMI, serviceUsageDone=serviceUsageDone, routingFailover=routingFailover, spLicenseErrType=spLicenseErrType, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spMitigationStart=spMitigationStart, bgpUp=bgpUp, spInterfaceSpeedHC=spInterfaceSpeedHC, spInterface=spInterface, spUnit=spUnit, spFlowspecTimeout=spFlowspecTimeout, devicePhysicalMemory=devicePhysicalMemory, peakflowSPTraps=peakflowSPTraps, spReportName=spReportName, spHijackRoute=spHijackRoute, hardwareFailureDone=hardwareFailureDone, cloudSignalingFault=cloudSignalingFault, dnsBaseline=dnsBaseline, spTMSCommunity=spTMSCommunity, spMitigationName=spMitigationName, mitigationBlackholeStart=mitigationBlackholeStart, bgpInstability=bgpInstability, spRouter=spRouter, alertScript=alertScript, managedObjectUsage=managedObjectUsage, spUsageType=spUsageType, bgpInstabilityDone=bgpInstabilityDone, spFingerprintFeedback=spFingerprintFeedback, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spReportID=spReportID, spInetAddress=spInetAddress, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFlowspecCommunity=spFlowspecCommunity, spImpactBps=spImpactBps, spLicenseErrDescription=spLicenseErrDescription, deviceSwapSpace=deviceSwapSpace, collectorStart=collectorStart, spLicenseErrCount=spLicenseErrCount, snmpDown=snmpDown, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, cloudSignalingFaultDone=cloudSignalingFaultDone, interfaceUsage=interfaceUsage, spSystemErrorType=spSystemErrorType, spThirdPartyAddr=spThirdPartyAddr, spTMSPrefix=spTMSPrefix, dnsBaselineDone=dnsBaselineDone, spLicenseErrLimit=spLicenseErrLimit, notificationLimit=notificationLimit, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spDNSObserved=spDNSObserved, spBlackholePrefix=spBlackholePrefix, spImpactPps=spImpactPps, spPravail=spPravail, spApplicationName=spApplicationName, greDown=greDown, flowUp=flowUp, peakflowSPMgr=peakflowSPMgr, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDNSExpected=spDNSExpected, deviceTotalFlows=deviceTotalFlows, PYSNMP_MODULE_ID=peakflowSPMIB, spServiceElement=spServiceElement, spTMSTimeout=spTMSTimeout, spThreshold=spThreshold, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, reportDone=reportDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spBlackholeCommunity=spBlackholeCommunity, spDetector=spDetector, spcommFailure=spcommFailure, spFingerprintMessage=spFingerprintMessage, mitigationDone=mitigationDone, spAlertDetectionSignatures=spAlertDetectionSignatures, hardwareFailure=hardwareFailure, spHijackAttr=spHijackAttr, spTMSMultiPrefix=spTMSMultiPrefix, spBGPEvent=spBGPEvent, spBGPInstability=spBGPInstability, serviceUsage=serviceUsage, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBGPTrapName=spBGPTrapName, spSystemErrorDescription=spSystemErrorDescription, dosNetworkProfiledAlert=dosNetworkProfiledAlert, fingerprintUsage=fingerprintUsage, bgpHijack=bgpHijack, trafficAutoMitigation=trafficAutoMitigation, managedObjectUsageDone=managedObjectUsageDone, fingerprintUsageDone=fingerprintUsageDone, spManagedObjects=spManagedObjects, configChange=configChange, spGreTunnelName=spGreTunnelName, licenseErrorDone=licenseErrorDone, flowDown=flowDown, dosHostDetectionAlert=dosHostDetectionAlert, spInterfaceSpeed=spInterfaceSpeed, spServiceName=spServiceName, spDetectedCountries=spDetectedCountries, spManagedObject=spManagedObject, spCommFailureDescription=spCommFailureDescription, spInterfaceIndex=spInterfaceIndex, deviceTotalFlowsHC=deviceTotalFlowsHC, spCollector=spCollector, snmpUp=snmpUp, spReportURL=spReportURL, spManagedObjectFamily=spManagedObjectFamily, collectorDown=collectorDown, autoclassifyStart=autoclassifyStart, spScriptStart=spScriptStart, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceDiskUsage=deviceDiskUsage, spInetAddressType=spInetAddressType, bgpTrap=bgpTrap, spScriptCommand=spScriptCommand, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBGPTrapEvent=spBGPTrapEvent, spBGPTrapPrefix=spBGPTrapPrefix, spFingerprintName=spFingerprintName, mitigationThirdPartyStart=mitigationThirdPartyStart, mitigationTMSStart=mitigationTMSStart, spHardwareFailureDescription=spHardwareFailureDescription)
