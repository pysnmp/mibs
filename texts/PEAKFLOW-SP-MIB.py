...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBlackholeNexthop=spBlackholeNexthop, notificationLimit=notificationLimit, interfaceUsage=interfaceUsage, bgpUp=bgpUp, deviceSystemErrorDone=deviceSystemErrorDone, trafficAutoMitigation=trafficAutoMitigation, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDetector=spDetector, spUnit=spUnit, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spBGPTrapPrefix=spBGPTrapPrefix, dnsBaseline=dnsBaseline, spScriptHost=spScriptHost, bgpInstabilityDone=bgpInstabilityDone, spPravail=spPravail, mitigationTMSStartV2=mitigationTMSStartV2, spScriptCommand=spScriptCommand, fingerprintReceive=fingerprintReceive, spBGPInstability=spBGPInstability, spUsageType=spUsageType, mitigationFlowspecStart=mitigationFlowspecStart, spManagedObjectFamily=spManagedObjectFamily, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spRouter=spRouter, spFingerprintFeedback=spFingerprintFeedback, deviceSystemError=deviceSystemError, snmpUp=snmpUp, collectorStart=collectorStart, spManagedObject=spManagedObject, spHijackLocal=spHijackLocal, deviceSwapSpace=deviceSwapSpace, mitigationBlackholeStart=mitigationBlackholeStart, configChange=configChange, spBlackholePrefix=spBlackholePrefix, managedObjectUsageDone=managedObjectUsageDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spUsername=spUsername, spCommFailureDestination=spCommFailureDestination, interfaceUsageDone=interfaceUsageDone, spCommFailureDescription=spCommFailureDescription, flowDown=flowDown, spFingerprintName=spFingerprintName, bgpHijack=bgpHijack, deviceTotalFlows=deviceTotalFlows, licenseErrorDone=licenseErrorDone, spSystemErrorType=spSystemErrorType, spAlertDetectionSignatures=spAlertDetectionSignatures, spTMSMultiPrefix=spTMSMultiPrefix, spBGPEvent=spBGPEvent, spInterfaceUsageHC=spInterfaceUsageHC, alertScript=alertScript, spInterfaceIndex=spInterfaceIndex, spTMSPrefix=spTMSPrefix, spScriptPort=spScriptPort, spBlackholeCommunity=spBlackholeCommunity, spSystemErrorDescription=spSystemErrorDescription, spDNSName=spDNSName, fingerprintFeedback=fingerprintFeedback, greDownDone=greDownDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spReportURL=spReportURL, spTMSTimeout=spTMSTimeout, spInetAddressType=spInetAddressType, spBGPTrapName=spBGPTrapName, managedObjectUsage=managedObjectUsage, spThirdPartyAddr=spThirdPartyAddr, flowUp=flowUp, spcommFailureDone=spcommFailureDone, spServiceElement=spServiceElement, peakflowSPTraps=peakflowSPTraps, spFingerprintSender=spFingerprintSender, devicePhysicalMemory=devicePhysicalMemory, bgpTrap=bgpTrap, spInterfaceUsage=spInterfaceUsage, spTMSCommunity=spTMSCommunity, spFingerprintMessage=spFingerprintMessage, spBlackholeTimeout=spBlackholeTimeout, deviceHealth=deviceHealth, spGreTunnelName=spGreTunnelName, spLicenseErrType=spLicenseErrType, spImpactPps=spImpactPps, serviceUsage=serviceUsage, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spApplicationName=spApplicationName, spReportID=spReportID, cloudSignalingFault=cloudSignalingFault, spMitigationName=spMitigationName, spcommFailure=spcommFailure, spImpactBps=spImpactBps, greDown=greDown, licenseError=licenseError, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, hardwareFailureDone=hardwareFailureDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, autoclassifyStart=autoclassifyStart, mitigationTMSStart=mitigationTMSStart, spInetAddress=spInetAddress, mitigationThirdPartyStart=mitigationThirdPartyStart, spHijackRoute=spHijackRoute, cloudSignalingFaultDone=cloudSignalingFaultDone, spScriptStart=spScriptStart, spThirdPartyZone=spThirdPartyZone, deviceDiskUsage=deviceDiskUsage, spReportName=spReportName, peakflowSPMIB=peakflowSPMIB, spMitigationStart=spMitigationStart, spFlowspecTimeout=spFlowspecTimeout, spLicenseErrCount=spLicenseErrCount, snmpDown=snmpDown, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpDown=bgpDown, spDNSExpected=spDNSExpected, spDuration=spDuration, spVersion=spVersion, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spUsage=spUsage, mitigationDone=mitigationDone, dnsBaselineDone=dnsBaselineDone, spManagedObjects=spManagedObjects, spHijackAttr=spHijackAttr, spCollector=spCollector, spDNSObserved=spDNSObserved, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spMitigationID=spMitigationID, PYSNMP_MODULE_ID=peakflowSPMIB, spHardwareFailureDescription=spHardwareFailureDescription, spFlowspecCommunity=spFlowspecCommunity, spInterface=spInterface, bgpInstability=bgpInstability, peakflowSPCMI=peakflowSPCMI, hardwareFailure=hardwareFailure, spGreTunnelDestination=spGreTunnelDestination, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosHostDetectionAlert=dosHostDetectionAlert, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spLicenseErrLimit=spLicenseErrLimit, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spAlertID=spAlertID, dosHostDetectionAlertDone=dosHostDetectionAlertDone, fingerprintUsageDone=fingerprintUsageDone, collectorDown=collectorDown, spDNSObservedMean=spDNSObservedMean, spBGPTrapEvent=spBGPTrapEvent, routingFailover=routingFailover, spInterfaceSpeedHC=spInterfaceSpeedHC, spServiceName=spServiceName, collectorUp=collectorUp, fingerprintUsage=fingerprintUsage, spInterfaceSpeed=spInterfaceSpeed, reportDone=reportDone, spDetectedCountries=spDetectedCountries, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, peakflowSPMgr=peakflowSPMgr, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spDNSObservedMax=spDNSObservedMax, spLicenseErrDescription=spLicenseErrDescription, spThreshold=spThreshold, serviceUsageDone=serviceUsageDone)
