...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spSystemErrorDescription=spSystemErrorDescription, licenseError=licenseError, spHardwareFailureDescription=spHardwareFailureDescription, spMitigationName=spMitigationName, spUnit=spUnit, spBlackholeNexthop=spBlackholeNexthop, dosHostDetectionAlert=dosHostDetectionAlert, mitigationTMSStartV2=mitigationTMSStartV2, spScriptCommand=spScriptCommand, flowUp=flowUp, spDetectedCountries=spDetectedCountries, spHijackAttr=spHijackAttr, spDNSExpected=spDNSExpected, serviceUsageDone=serviceUsageDone, spScriptHost=spScriptHost, managedObjectUsageDone=managedObjectUsageDone, alertScript=alertScript, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spImpactBps=spImpactBps, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spTMSTimeout=spTMSTimeout, spTMSPrefix=spTMSPrefix, spUsage=spUsage, spPravail=spPravail, dnsBaseline=dnsBaseline, spApplicationName=spApplicationName, snmpUp=snmpUp, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBGPTrapPrefix=spBGPTrapPrefix, spcommFailure=spcommFailure, flowDown=flowDown, deviceSystemErrorDone=deviceSystemErrorDone, interfaceUsageDone=interfaceUsageDone, spInterfaceSpeed=spInterfaceSpeed, spMitigationStart=spMitigationStart, fingerprintUsage=fingerprintUsage, bgpHijack=bgpHijack, interfaceUsage=interfaceUsage, autoclassifyStart=autoclassifyStart, mitigationFlowspecStart=mitigationFlowspecStart, spInetAddress=spInetAddress, serviceUsage=serviceUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spUsername=spUsername, spVersion=spVersion, mitigationThirdPartyStart=mitigationThirdPartyStart, configChange=configChange, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spScriptPort=spScriptPort, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, deviceTotalFlowsHC=deviceTotalFlowsHC, spDNSObservedMean=spDNSObservedMean, spLicenseErrLimit=spLicenseErrLimit, fingerprintReceive=fingerprintReceive, spThirdPartyZone=spThirdPartyZone, spFlowspecCommunity=spFlowspecCommunity, spInterfaceUsage=spInterfaceUsage, spAlertDetectionSignatures=spAlertDetectionSignatures, spTMSCommunity=spTMSCommunity, peakflowSPMIB=peakflowSPMIB, spServiceElement=spServiceElement, spMitigationID=spMitigationID, spDNSObserved=spDNSObserved, spDNSName=spDNSName, deviceSwapSpace=deviceSwapSpace, fingerprintFeedback=fingerprintFeedback, spRouter=spRouter, dnsBaselineDone=dnsBaselineDone, spUsageType=spUsageType, spFingerprintSender=spFingerprintSender, managedObjectUsage=managedObjectUsage, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spHijackRoute=spHijackRoute, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spLicenseErrType=spLicenseErrType, spServiceName=spServiceName, spImpactPps=spImpactPps, spCloudSignalFaultDescription=spCloudSignalFaultDescription, PYSNMP_MODULE_ID=peakflowSPMIB, spFingerprintMessage=spFingerprintMessage, bgpUp=bgpUp, peakflowSPMgr=peakflowSPMgr, cloudSignalingFaultDone=cloudSignalingFaultDone, routingFailover=routingFailover, spReportURL=spReportURL, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spTMSMultiPrefix=spTMSMultiPrefix, spInterface=spInterface, spInetAddressType=spInetAddressType, spBGPInstability=spBGPInstability, spBlackholeTimeout=spBlackholeTimeout, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceHealth=deviceHealth, deviceDiskUsage=deviceDiskUsage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, bgpTrap=bgpTrap, notificationLimit=notificationLimit, spLicenseErrDescription=spLicenseErrDescription, deviceTotalFlows=deviceTotalFlows, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInterfaceSpeedHC=spInterfaceSpeedHC, licenseErrorDone=licenseErrorDone, spAlertID=spAlertID, collectorStart=collectorStart, hardwareFailure=hardwareFailure, collectorUp=collectorUp, spSystemErrorType=spSystemErrorType, spDetector=spDetector, spFlowspecTimeout=spFlowspecTimeout, spCommFailureDescription=spCommFailureDescription, spThreshold=spThreshold, spcommFailureDone=spcommFailureDone, spGreTunnelName=spGreTunnelName, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spScriptStart=spScriptStart, spManagedObject=spManagedObject, cloudSignalingFault=cloudSignalingFault, spGreTunnelDestination=spGreTunnelDestination, spFingerprintFeedback=spFingerprintFeedback, trafficAutoMitigation=trafficAutoMitigation, spManagedObjects=spManagedObjects, spBGPEvent=spBGPEvent, spLicenseErrCount=spLicenseErrCount, spReportID=spReportID, deviceSystemError=deviceSystemError, snmpDown=snmpDown, peakflowSPTraps=peakflowSPTraps, spReportName=spReportName, bgpInstabilityDone=bgpInstabilityDone, bgpInstability=bgpInstability, spDNSObservedMax=spDNSObservedMax, bgpDown=bgpDown, spCommFailureDestination=spCommFailureDestination, spCollector=spCollector, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, greDownDone=greDownDone, spManagedObjectFamily=spManagedObjectFamily, peakflowSPCMI=peakflowSPCMI, spThirdPartyAddr=spThirdPartyAddr, spFingerprintName=spFingerprintName, mitigationTMSStart=mitigationTMSStart, fingerprintUsageDone=fingerprintUsageDone, spBGPTrapName=spBGPTrapName, spInterfaceUsageHC=spInterfaceUsageHC, spBlackholeCommunity=spBlackholeCommunity, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBGPTrapEvent=spBGPTrapEvent, mitigationBlackholeStart=mitigationBlackholeStart, reportDone=reportDone, spInterfaceIndex=spInterfaceIndex, spHijackLocal=spHijackLocal, collectorDown=collectorDown, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, greDown=greDown, mitigationDone=mitigationDone, spDuration=spDuration, hardwareFailureDone=hardwareFailureDone, spBlackholePrefix=spBlackholePrefix, spBGPTrapOldAttributes=spBGPTrapOldAttributes, devicePhysicalMemory=devicePhysicalMemory)
