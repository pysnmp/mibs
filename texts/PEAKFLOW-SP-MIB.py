...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spThreshold=spThreshold, spSystemErrorType=spSystemErrorType, spGreTunnelName=spGreTunnelName, spThirdPartyAddr=spThirdPartyAddr, mitigationTMSStartV2=mitigationTMSStartV2, spFlowspecCommunity=spFlowspecCommunity, collectorDown=collectorDown, spScriptHost=spScriptHost, spInterfaceIndex=spInterfaceIndex, spCollector=spCollector, spBlackholeTimeout=spBlackholeTimeout, spDetectedCountries=spDetectedCountries, fingerprintReceive=fingerprintReceive, spHijackLocal=spHijackLocal, spInetAddressType=spInetAddressType, bgpUp=bgpUp, autoclassifyStart=autoclassifyStart, spReportID=spReportID, spDNSObserved=spDNSObserved, spInterfaceSpeedHC=spInterfaceSpeedHC, fingerprintUsageDone=fingerprintUsageDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spManagedObjects=spManagedObjects, spMitigationID=spMitigationID, spReportURL=spReportURL, mitigationBlackholeStart=mitigationBlackholeStart, deviceSwapSpaceInUse=deviceSwapSpaceInUse, licenseError=licenseError, spImpactBps=spImpactBps, spGreTunnelDestination=spGreTunnelDestination, spInterface=spInterface, spFingerprintMessage=spFingerprintMessage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBGPTrapPrefix=spBGPTrapPrefix, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceSwapSpace=deviceSwapSpace, spManagedObject=spManagedObject, trafficAutoMitigation=trafficAutoMitigation, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDNSExpected=spDNSExpected, spBlackholeNexthop=spBlackholeNexthop, spDNSName=spDNSName, spDNSObservedMean=spDNSObservedMean, managedObjectUsage=managedObjectUsage, fingerprintUsage=fingerprintUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceSwapSpaceUsage=deviceSwapSpaceUsage, interfaceUsageDone=interfaceUsageDone, spFlowspecTimeout=spFlowspecTimeout, spHardwareFailureDescription=spHardwareFailureDescription, deviceSystemErrorDone=deviceSystemErrorDone, spUsage=spUsage, spThirdPartyZone=spThirdPartyZone, deviceSystemError=deviceSystemError, bgpHijack=bgpHijack, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spScriptStart=spScriptStart, peakflowSPMgr=peakflowSPMgr, spBGPTrapEvent=spBGPTrapEvent, snmpDown=snmpDown, spBGPInstability=spBGPInstability, spScriptPort=spScriptPort, spVersion=spVersion, peakflowSPTraps=peakflowSPTraps, flowDown=flowDown, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spAlertDetectionSignatures=spAlertDetectionSignatures, bgpTrap=bgpTrap, reportDone=reportDone, spLicenseErrDescription=spLicenseErrDescription, hardwareFailure=hardwareFailure, spHijackRoute=spHijackRoute, dosHostDetectionAlert=dosHostDetectionAlert, deviceHealth=deviceHealth, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spPravail=spPravail, greDown=greDown, spLicenseErrType=spLicenseErrType, greDownDone=greDownDone, deviceTotalFlows=deviceTotalFlows, spReportName=spReportName, licenseErrorDone=licenseErrorDone, deviceTotalFlowsHC=deviceTotalFlowsHC, devicePhysicalMemory=devicePhysicalMemory, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, configChange=configChange, spLicenseErrCount=spLicenseErrCount, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, notificationLimit=notificationLimit, spRouter=spRouter, routingFailover=routingFailover, spInterfaceUsageHC=spInterfaceUsageHC, spUsername=spUsername, spServiceName=spServiceName, spServiceElement=spServiceElement, spMitigationStart=spMitigationStart, spDuration=spDuration, mitigationFlowspecStart=mitigationFlowspecStart, spInterfaceUsage=spInterfaceUsage, spTMSMultiPrefix=spTMSMultiPrefix, spMitigationName=spMitigationName, cloudSignalingFault=cloudSignalingFault, spBlackholeCommunity=spBlackholeCommunity, spBGPTrapName=spBGPTrapName, spDNSObservedMax=spDNSObservedMax, collectorUp=collectorUp, spFingerprintName=spFingerprintName, spUsageType=spUsageType, peakflowSPCMI=peakflowSPCMI, serviceUsageDone=serviceUsageDone, spBGPEvent=spBGPEvent, bgpInstabilityDone=bgpInstabilityDone, spBlackholePrefix=spBlackholePrefix, snmpUp=snmpUp, spFingerprintFeedback=spFingerprintFeedback, spDetector=spDetector, spAlertID=spAlertID, hardwareFailureDone=hardwareFailureDone, spHijackAttr=spHijackAttr, spInetAddress=spInetAddress, spFingerprintSender=spFingerprintSender, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spScriptCommand=spScriptCommand, flowUp=flowUp, spInterfaceSpeed=spInterfaceSpeed, spCommFailureDestination=spCommFailureDestination, spSystemErrorDescription=spSystemErrorDescription, bgpInstability=bgpInstability, spTMSTimeout=spTMSTimeout, mitigationTMSStart=mitigationTMSStart, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spcommFailureDone=spcommFailureDone, cloudSignalingFaultDone=cloudSignalingFaultDone, dnsBaselineDone=dnsBaselineDone, spApplicationName=spApplicationName, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, PYSNMP_MODULE_ID=peakflowSPMIB, spManagedObjectFamily=spManagedObjectFamily, collectorStart=collectorStart, managedObjectUsageDone=managedObjectUsageDone, spUnit=spUnit, bgpDown=bgpDown, spTMSPrefix=spTMSPrefix, interfaceUsage=interfaceUsage, spLicenseErrLimit=spLicenseErrLimit, mitigationDone=mitigationDone, spTMSCommunity=spTMSCommunity, alertScript=alertScript, peakflowSPMIB=peakflowSPMIB, dnsBaseline=dnsBaseline, spcommFailure=spcommFailure, spImpactPps=spImpactPps, fingerprintFeedback=fingerprintFeedback, spCommFailureDescription=spCommFailureDescription, deviceDiskUsage=deviceDiskUsage, serviceUsage=serviceUsage)
