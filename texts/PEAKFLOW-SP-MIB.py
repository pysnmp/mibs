...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spScriptStart=spScriptStart, devicePhysicalMemory=devicePhysicalMemory, spLicenseErrType=spLicenseErrType, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spDNSObservedMax=spDNSObservedMax, spUsage=spUsage, spBlackholeTimeout=spBlackholeTimeout, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceSwapSpace=deviceSwapSpace, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spHijackAttr=spHijackAttr, cloudSignalingFault=cloudSignalingFault, spManagedObject=spManagedObject, deviceSystemErrorDone=deviceSystemErrorDone, spFingerprintMessage=spFingerprintMessage, alertScript=alertScript, spLicenseErrLimit=spLicenseErrLimit, spInterface=spInterface, spTMSCommunity=spTMSCommunity, spManagedObjects=spManagedObjects, fingerprintReceive=fingerprintReceive, spUsageType=spUsageType, spBlackholeCommunity=spBlackholeCommunity, bgpDown=bgpDown, cloudSignalingFaultDone=cloudSignalingFaultDone, spServiceName=spServiceName, mitigationTMSStart=mitigationTMSStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDetector=spDetector, hardwareFailure=hardwareFailure, spDNSName=spDNSName, spAlertDetectionSignatures=spAlertDetectionSignatures, spCommFailureDescription=spCommFailureDescription, spInetAddress=spInetAddress, spVersion=spVersion, snmpDown=snmpDown, notificationLimit=notificationLimit, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spInterfaceIndex=spInterfaceIndex, reportDone=reportDone, spScriptHost=spScriptHost, greDown=greDown, spInterfaceSpeed=spInterfaceSpeed, bgpTrap=bgpTrap, deviceSystemError=deviceSystemError, spFingerprintName=spFingerprintName, spDetectedCountries=spDetectedCountries, deviceHealth=deviceHealth, spFingerprintFeedback=spFingerprintFeedback, deviceSwapSpaceUsage=deviceSwapSpaceUsage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, configChange=configChange, fingerprintUsageDone=fingerprintUsageDone, licenseError=licenseError, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceTotalFlows=deviceTotalFlows, greDownDone=greDownDone, spBGPTrapEvent=spBGPTrapEvent, spManagedObjectFamily=spManagedObjectFamily, spApplicationName=spApplicationName, spAlertID=spAlertID, spBlackholePrefix=spBlackholePrefix, spInterfaceUsage=spInterfaceUsage, spDuration=spDuration, spImpactBps=spImpactBps, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spUnit=spUnit, bgpInstability=bgpInstability, interfaceUsageDone=interfaceUsageDone, mitigationBlackholeStart=mitigationBlackholeStart, peakflowSPTraps=peakflowSPTraps, serviceUsage=serviceUsage, licenseErrorDone=licenseErrorDone, spScriptPort=spScriptPort, spBGPEvent=spBGPEvent, spFlowspecCommunity=spFlowspecCommunity, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spCommFailureDestination=spCommFailureDestination, trafficAutoMitigation=trafficAutoMitigation, spImpactPps=spImpactPps, spPravail=spPravail, spMitigationStart=spMitigationStart, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spHardwareFailureDescription=spHardwareFailureDescription, spTMSTimeout=spTMSTimeout, bgpInstabilityDone=bgpInstabilityDone, bgpUp=bgpUp, mitigationTMSStartV2=mitigationTMSStartV2, mitigationDone=mitigationDone, PYSNMP_MODULE_ID=peakflowSPMIB, peakflowSPCMI=peakflowSPCMI, spHijackLocal=spHijackLocal, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spServiceElement=spServiceElement, spGreTunnelName=spGreTunnelName, fingerprintUsage=fingerprintUsage, spThirdPartyZone=spThirdPartyZone, spDNSExpected=spDNSExpected, spCollector=spCollector, mitigationFlowspecStart=mitigationFlowspecStart, interfaceUsage=interfaceUsage, spInterfaceUsageHC=spInterfaceUsageHC, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosHostDetectionAlertDone=dosHostDetectionAlertDone, collectorStart=collectorStart, flowDown=flowDown, spFingerprintSender=spFingerprintSender, autoclassifyStart=autoclassifyStart, deviceTotalFlowsHC=deviceTotalFlowsHC, spRouter=spRouter, spcommFailure=spcommFailure, spReportURL=spReportURL, spGreTunnelDestination=spGreTunnelDestination, spInetAddressType=spInetAddressType, spLicenseErrDescription=spLicenseErrDescription, spMitigationName=spMitigationName, spFlowspecTimeout=spFlowspecTimeout, mitigationThirdPartyStart=mitigationThirdPartyStart, spScriptCommand=spScriptCommand, spUsername=spUsername, managedObjectUsageDone=managedObjectUsageDone, spBGPTrapPrefix=spBGPTrapPrefix, spTMSMultiPrefix=spTMSMultiPrefix, collectorUp=collectorUp, spSystemErrorType=spSystemErrorType, dosHostDetectionAlert=dosHostDetectionAlert, spBGPTrapName=spBGPTrapName, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spDNSObservedMean=spDNSObservedMean, spSystemErrorDescription=spSystemErrorDescription, managedObjectUsage=managedObjectUsage, spThreshold=spThreshold, spTMSPrefix=spTMSPrefix, flowUp=flowUp, snmpUp=snmpUp, spBGPInstability=spBGPInstability, spReportID=spReportID, serviceUsageDone=serviceUsageDone, spReportName=spReportName, spDNSObserved=spDNSObserved, spLicenseErrCount=spLicenseErrCount, peakflowSPMgr=peakflowSPMgr, deviceDiskUsage=deviceDiskUsage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spHijackRoute=spHijackRoute, dnsBaseline=dnsBaseline, spInterfaceSpeedHC=spInterfaceSpeedHC, spThirdPartyAddr=spThirdPartyAddr, hardwareFailureDone=hardwareFailureDone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, routingFailover=routingFailover, fingerprintFeedback=fingerprintFeedback, dnsBaselineDone=dnsBaselineDone, bgpHijack=bgpHijack, peakflowSPMIB=peakflowSPMIB, spBlackholeNexthop=spBlackholeNexthop, spMitigationID=spMitigationID, spcommFailureDone=spcommFailureDone, collectorDown=collectorDown)
