...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spLicenseErrCount=spLicenseErrCount, spReportID=spReportID, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBGPTrapName=spBGPTrapName, spDNSName=spDNSName, spMitigationName=spMitigationName, spImpactBps=spImpactBps, deviceTotalFlows=deviceTotalFlows, spSystemErrorDescription=spSystemErrorDescription, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, dnsBaselineDone=dnsBaselineDone, spAlertDetectionSignatures=spAlertDetectionSignatures, spMitigationID=spMitigationID, spDuration=spDuration, deviceTotalFlowsHC=deviceTotalFlowsHC, spTMSCommunity=spTMSCommunity, fingerprintUsage=fingerprintUsage, peakflowSPMIB=peakflowSPMIB, spApplicationName=spApplicationName, spImpactPps=spImpactPps, bgpDown=bgpDown, spManagedObjectFamily=spManagedObjectFamily, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spTMSTimeout=spTMSTimeout, managedObjectUsage=managedObjectUsage, spGreTunnelName=spGreTunnelName, spHijackAttr=spHijackAttr, spVersion=spVersion, serviceUsageDone=serviceUsageDone, spDNSObservedMean=spDNSObservedMean, spBlackholeNexthop=spBlackholeNexthop, spTMSMultiPrefix=spTMSMultiPrefix, spDetectedCountries=spDetectedCountries, collectorStart=collectorStart, deviceSystemErrorDone=deviceSystemErrorDone, interfaceUsage=interfaceUsage, spInterfaceIndex=spInterfaceIndex, spcommFailureDone=spcommFailureDone, hardwareFailure=hardwareFailure, dosHostDetectionAlert=dosHostDetectionAlert, mitigationThirdPartyStart=mitigationThirdPartyStart, spThirdPartyAddr=spThirdPartyAddr, deviceHealth=deviceHealth, spManagedObject=spManagedObject, deviceSwapSpaceUsage=deviceSwapSpaceUsage, mitigationFlowspecStart=mitigationFlowspecStart, spUsage=spUsage, spScriptCommand=spScriptCommand, spScriptStart=spScriptStart, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spFingerprintSender=spFingerprintSender, spMitigationStart=spMitigationStart, spFlowspecCommunity=spFlowspecCommunity, hardwareFailureDone=hardwareFailureDone, spReportName=spReportName, spThreshold=spThreshold, spInetAddressType=spInetAddressType, greDown=greDown, spFlowspecTimeout=spFlowspecTimeout, dosHostDetectionAlertDone=dosHostDetectionAlertDone, licenseError=licenseError, spLicenseErrDescription=spLicenseErrDescription, fingerprintFeedback=fingerprintFeedback, flowUp=flowUp, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, alertScript=alertScript, spThirdPartyZone=spThirdPartyZone, spCommFailureDestination=spCommFailureDestination, spFingerprintFeedback=spFingerprintFeedback, PYSNMP_MODULE_ID=peakflowSPMIB, collectorDown=collectorDown, snmpDown=snmpDown, spTMSPrefix=spTMSPrefix, spInterfaceSpeedHC=spInterfaceSpeedHC, flowDown=flowDown, bgpHijack=bgpHijack, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spDNSObservedMax=spDNSObservedMax, spFingerprintMessage=spFingerprintMessage, spBGPTrapPrefix=spBGPTrapPrefix, spPravail=spPravail, spCloudSignalFaultDescription=spCloudSignalFaultDescription, interfaceUsageDone=interfaceUsageDone, spHijackLocal=spHijackLocal, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFingerprintName=spFingerprintName, mitigationBlackholeStart=mitigationBlackholeStart, spDetector=spDetector, dnsBaseline=dnsBaseline, mitigationTMSStart=mitigationTMSStart, spGreTunnelDestination=spGreTunnelDestination, spUsername=spUsername, fingerprintUsageDone=fingerprintUsageDone, spScriptPort=spScriptPort, bgpInstabilityDone=bgpInstabilityDone, peakflowSPCMI=peakflowSPCMI, managedObjectUsageDone=managedObjectUsageDone, spCommFailureDescription=spCommFailureDescription, spCollector=spCollector, spUnit=spUnit, peakflowSPMgr=peakflowSPMgr, spSystemErrorType=spSystemErrorType, spBlackholeTimeout=spBlackholeTimeout, bgpUp=bgpUp, routingFailover=routingFailover, spScriptHost=spScriptHost, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spBlackholeCommunity=spBlackholeCommunity, spUsageType=spUsageType, spLicenseErrType=spLicenseErrType, bgpTrap=bgpTrap, spInterfaceUsageHC=spInterfaceUsageHC, deviceSwapSpace=deviceSwapSpace, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBGPEvent=spBGPEvent, configChange=configChange, deviceSystemError=deviceSystemError, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spLicenseErrLimit=spLicenseErrLimit, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spInterface=spInterface, spInetAddress=spInetAddress, peakflowSPTraps=peakflowSPTraps, spRouter=spRouter, licenseErrorDone=licenseErrorDone, cloudSignalingFaultDone=cloudSignalingFaultDone, cloudSignalingFault=cloudSignalingFault, spBGPTrapEvent=spBGPTrapEvent, autoclassifyStart=autoclassifyStart, bgpInstability=bgpInstability, spHardwareFailureDescription=spHardwareFailureDescription, spBGPInstability=spBGPInstability, spManagedObjects=spManagedObjects, spBlackholePrefix=spBlackholePrefix, collectorUp=collectorUp, spReportURL=spReportURL, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spServiceElement=spServiceElement, snmpUp=snmpUp, spDNSExpected=spDNSExpected, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, devicePhysicalMemory=devicePhysicalMemory, deviceDiskUsage=deviceDiskUsage, fingerprintReceive=fingerprintReceive, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, reportDone=reportDone, serviceUsage=serviceUsage, trafficAutoMitigation=trafficAutoMitigation, mitigationTMSStartV2=mitigationTMSStartV2, spcommFailure=spcommFailure, mitigationDone=mitigationDone, spInterfaceUsage=spInterfaceUsage, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spHijackRoute=spHijackRoute, spDNSObserved=spDNSObserved, greDownDone=greDownDone, spAlertID=spAlertID, spInterfaceSpeed=spInterfaceSpeed, spServiceName=spServiceName, notificationLimit=notificationLimit)
