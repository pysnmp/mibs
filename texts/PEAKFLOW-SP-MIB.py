...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceUsage=spInterfaceUsage, spTMSPrefix=spTMSPrefix, spMitigationStart=spMitigationStart, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, trafficAutoMitigation=trafficAutoMitigation, spDNSObservedMean=spDNSObservedMean, spCloudSignalFaultDescription=spCloudSignalFaultDescription, hardwareFailure=hardwareFailure, collectorUp=collectorUp, spSystemErrorDescription=spSystemErrorDescription, spPravail=spPravail, deviceSwapSpaceUsage=deviceSwapSpaceUsage, fingerprintReceive=fingerprintReceive, spRouter=spRouter, bgpTrap=bgpTrap, spInterfaceIndex=spInterfaceIndex, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceDiskUsage=deviceDiskUsage, greDownDone=greDownDone, spGreTunnelDestination=spGreTunnelDestination, flowUp=flowUp, bgpInstability=bgpInstability, spCommFailureDescription=spCommFailureDescription, spTMSCommunity=spTMSCommunity, spDetector=spDetector, spImpactBps=spImpactBps, spBGPTrapNewAttributes=spBGPTrapNewAttributes, bgpDown=bgpDown, spFingerprintFeedback=spFingerprintFeedback, dnsBaselineDone=dnsBaselineDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spLicenseErrDescription=spLicenseErrDescription, spBGPEvent=spBGPEvent, fingerprintUsageDone=fingerprintUsageDone, spUsername=spUsername, spLicenseErrCount=spLicenseErrCount, spReportID=spReportID, spMitigationName=spMitigationName, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spReportName=spReportName, deviceTotalFlowsHC=deviceTotalFlowsHC, spUsage=spUsage, spScriptCommand=spScriptCommand, spScriptHost=spScriptHost, reportDone=reportDone, spThirdPartyZone=spThirdPartyZone, spBGPTrapName=spBGPTrapName, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBGPInstability=spBGPInstability, spFingerprintName=spFingerprintName, spInterfaceSpeed=spInterfaceSpeed, peakflowSPTraps=peakflowSPTraps, spHijackLocal=spHijackLocal, managedObjectUsageDone=managedObjectUsageDone, deviceHealth=deviceHealth, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spUnit=spUnit, spDNSName=spDNSName, configChange=configChange, spReportURL=spReportURL, interfaceUsageDone=interfaceUsageDone, spBGPTrapPrefix=spBGPTrapPrefix, spDetectedCountries=spDetectedCountries, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spLicenseErrLimit=spLicenseErrLimit, spImpactPps=spImpactPps, flowDown=flowDown, spInterfaceSpeedHC=spInterfaceSpeedHC, spInterfaceUsageHC=spInterfaceUsageHC, spGreTunnelName=spGreTunnelName, greDown=greDown, spManagedObjectFamily=spManagedObjectFamily, deviceSystemErrorDone=deviceSystemErrorDone, mitigationTMSStartV2=mitigationTMSStartV2, spFingerprintSender=spFingerprintSender, spHijackRoute=spHijackRoute, spFlowspecCommunity=spFlowspecCommunity, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spVersion=spVersion, mitigationBlackholeStart=mitigationBlackholeStart, collectorDown=collectorDown, spDNSExpected=spDNSExpected, spScriptPort=spScriptPort, spDuration=spDuration, interfaceUsage=interfaceUsage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spAlertDetectionSignatures=spAlertDetectionSignatures, cloudSignalingFaultDone=cloudSignalingFaultDone, spBlackholePrefix=spBlackholePrefix, bgpUp=bgpUp, peakflowSPMIB=peakflowSPMIB, licenseError=licenseError, autoclassifyStart=autoclassifyStart, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spManagedObject=spManagedObject, spCollector=spCollector, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, dosHostDetectionAlert=dosHostDetectionAlert, spBlackholeTimeout=spBlackholeTimeout, PYSNMP_MODULE_ID=peakflowSPMIB, snmpUp=snmpUp, spLicenseErrType=spLicenseErrType, spTMSMultiPrefix=spTMSMultiPrefix, deviceTotalFlows=deviceTotalFlows, collectorStart=collectorStart, spcommFailureDone=spcommFailureDone, spDNSObserved=spDNSObserved, spServiceName=spServiceName, spTMSTimeout=spTMSTimeout, spMitigationID=spMitigationID, deviceSwapSpace=deviceSwapSpace, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spUsageType=spUsageType, notificationLimit=notificationLimit, deviceSystemError=deviceSystemError, spAlertID=spAlertID, spInterface=spInterface, spFingerprintMessage=spFingerprintMessage, fingerprintUsage=fingerprintUsage, spSystemErrorType=spSystemErrorType, mitigationFlowspecStart=mitigationFlowspecStart, peakflowSPMgr=peakflowSPMgr, licenseErrorDone=licenseErrorDone, serviceUsageDone=serviceUsageDone, spBlackholeCommunity=spBlackholeCommunity, routingFailover=routingFailover, bgpHijack=bgpHijack, spThirdPartyAddr=spThirdPartyAddr, spServiceElement=spServiceElement, managedObjectUsage=managedObjectUsage, mitigationTMSStart=mitigationTMSStart, spInetAddressType=spInetAddressType, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spScriptStart=spScriptStart, bgpInstabilityDone=bgpInstabilityDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, cloudSignalingFault=cloudSignalingFault, spFlowspecTimeout=spFlowspecTimeout, alertScript=alertScript, snmpDown=snmpDown, spBlackholeNexthop=spBlackholeNexthop, fingerprintFeedback=fingerprintFeedback, spManagedObjects=spManagedObjects, spHardwareFailureDescription=spHardwareFailureDescription, spBGPTrapEvent=spBGPTrapEvent, spInetAddress=spInetAddress, hardwareFailureDone=hardwareFailureDone, devicePhysicalMemory=devicePhysicalMemory, spApplicationName=spApplicationName, spDNSObservedMax=spDNSObservedMax, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, serviceUsage=serviceUsage, spThreshold=spThreshold, mitigationDone=mitigationDone, peakflowSPCMI=peakflowSPCMI, dnsBaseline=dnsBaseline, spcommFailure=spcommFailure, spCommFailureDestination=spCommFailureDestination, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spHijackAttr=spHijackAttr)
