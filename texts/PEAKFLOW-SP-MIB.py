...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDNSExpected=spDNSExpected, spBlackholeTimeout=spBlackholeTimeout, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spApplicationName=spApplicationName, mitigationThirdPartyStart=mitigationThirdPartyStart, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, peakflowSPTraps=peakflowSPTraps, spGreTunnelDestination=spGreTunnelDestination, notificationLimit=notificationLimit, trafficAutoMitigation=trafficAutoMitigation, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, bgpHijack=bgpHijack, snmpUp=snmpUp, spBlackholeNexthop=spBlackholeNexthop, spMitigationName=spMitigationName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, mitigationDone=mitigationDone, hardwareFailureDone=hardwareFailureDone, greDownDone=greDownDone, spMitigationID=spMitigationID, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spRouter=spRouter, spReportID=spReportID, alertScript=alertScript, spFlowspecCommunity=spFlowspecCommunity, fingerprintFeedback=fingerprintFeedback, spUsageType=spUsageType, spDetectedCountries=spDetectedCountries, spInterfaceUsageHC=spInterfaceUsageHC, spFingerprintMessage=spFingerprintMessage, spBGPTrapPrefix=spBGPTrapPrefix, spBlackholeCommunity=spBlackholeCommunity, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spUnit=spUnit, serviceUsage=serviceUsage, spInterfaceUsage=spInterfaceUsage, spInetAddress=spInetAddress, flowDown=flowDown, spImpactPps=spImpactPps, dosHostDetectionAlert=dosHostDetectionAlert, spHardwareFailureDescription=spHardwareFailureDescription, collectorDown=collectorDown, spLicenseErrType=spLicenseErrType, spUsername=spUsername, spInetAddressType=spInetAddressType, interfaceUsage=interfaceUsage, spBGPTrapName=spBGPTrapName, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spTMSCommunity=spTMSCommunity, spGreTunnelName=spGreTunnelName, spcommFailure=spcommFailure, devicePhysicalMemory=devicePhysicalMemory, spcommFailureDone=spcommFailureDone, spUsage=spUsage, bgpUp=bgpUp, spFingerprintFeedback=spFingerprintFeedback, spReportName=spReportName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, routingFailover=routingFailover, spLicenseErrLimit=spLicenseErrLimit, spDNSObservedMean=spDNSObservedMean, configChange=configChange, hardwareFailure=hardwareFailure, fingerprintUsage=fingerprintUsage, spDuration=spDuration, spBGPEvent=spBGPEvent, spScriptHost=spScriptHost, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spThirdPartyZone=spThirdPartyZone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spCommFailureDestination=spCommFailureDestination, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spReportURL=spReportURL, deviceHealth=deviceHealth, snmpDown=snmpDown, spManagedObjectFamily=spManagedObjectFamily, spManagedObject=spManagedObject, deviceSystemErrorDone=deviceSystemErrorDone, peakflowSPMIB=peakflowSPMIB, spInterface=spInterface, spPravail=spPravail, mitigationTMSStart=mitigationTMSStart, deviceTotalFlowsHC=deviceTotalFlowsHC, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceSystemError=deviceSystemError, spTMSMultiPrefix=spTMSMultiPrefix, spThirdPartyAddr=spThirdPartyAddr, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, fingerprintReceive=fingerprintReceive, spLicenseErrDescription=spLicenseErrDescription, spVersion=spVersion, spScriptStart=spScriptStart, spSystemErrorType=spSystemErrorType, spFingerprintSender=spFingerprintSender, spBlackholePrefix=spBlackholePrefix, spInterfaceIndex=spInterfaceIndex, spBGPTrapEvent=spBGPTrapEvent, spDNSName=spDNSName, spFingerprintName=spFingerprintName, deviceDiskUsage=deviceDiskUsage, autoclassifyStart=autoclassifyStart, licenseErrorDone=licenseErrorDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, managedObjectUsage=managedObjectUsage, bgpInstability=bgpInstability, reportDone=reportDone, spBGPInstability=spBGPInstability, PYSNMP_MODULE_ID=peakflowSPMIB, spLicenseErrCount=spLicenseErrCount, spDNSObservedMax=spDNSObservedMax, spInterfaceSpeedHC=spInterfaceSpeedHC, spCollector=spCollector, collectorStart=collectorStart, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spScriptCommand=spScriptCommand, spTMSTimeout=spTMSTimeout, cloudSignalingFault=cloudSignalingFault, dnsBaseline=dnsBaseline, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spAlertID=spAlertID, spHijackLocal=spHijackLocal, spServiceName=spServiceName, spDetector=spDetector, dnsBaselineDone=dnsBaselineDone, peakflowSPCMI=peakflowSPCMI, spManagedObjects=spManagedObjects, serviceUsageDone=serviceUsageDone, spTMSPrefix=spTMSPrefix, deviceTotalFlows=deviceTotalFlows, spMitigationStart=spMitigationStart, spServiceElement=spServiceElement, fingerprintUsageDone=fingerprintUsageDone, deviceSwapSpace=deviceSwapSpace, mitigationFlowspecStart=mitigationFlowspecStart, managedObjectUsageDone=managedObjectUsageDone, interfaceUsageDone=interfaceUsageDone, spScriptPort=spScriptPort, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spAlertDetectionSignatures=spAlertDetectionSignatures, peakflowSPMgr=peakflowSPMgr, collectorUp=collectorUp, mitigationBlackholeStart=mitigationBlackholeStart, spHijackRoute=spHijackRoute, deviceSwapSpaceUsage=deviceSwapSpaceUsage, mitigationTMSStartV2=mitigationTMSStartV2, spInterfaceSpeed=spInterfaceSpeed, bgpTrap=bgpTrap, flowUp=flowUp, spCommFailureDescription=spCommFailureDescription, spHijackAttr=spHijackAttr, greDown=greDown, spFlowspecTimeout=spFlowspecTimeout, licenseError=licenseError, spThreshold=spThreshold, spSystemErrorDescription=spSystemErrorDescription, dosNetworkProfiledAlert=dosNetworkProfiledAlert, bgpInstabilityDone=bgpInstabilityDone, bgpDown=bgpDown, spDNSObserved=spDNSObserved, spImpactBps=spImpactBps)
