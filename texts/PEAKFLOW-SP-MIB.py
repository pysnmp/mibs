...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", fingerprintReceive=fingerprintReceive, licenseError=licenseError, mitigationDone=mitigationDone, spUsageType=spUsageType, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spLicenseErrLimit=spLicenseErrLimit, mitigationFlowspecStart=mitigationFlowspecStart, spBGPEvent=spBGPEvent, spSystemErrorDescription=spSystemErrorDescription, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spLicenseErrType=spLicenseErrType, mitigationBlackholeStart=mitigationBlackholeStart, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, autoclassifyStart=autoclassifyStart, spBGPTrapName=spBGPTrapName, managedObjectUsage=managedObjectUsage, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spTMSMultiPrefix=spTMSMultiPrefix, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spApplicationName=spApplicationName, spManagedObjectFamily=spManagedObjectFamily, spLicenseErrDescription=spLicenseErrDescription, spFingerprintSender=spFingerprintSender, spCommFailureDescription=spCommFailureDescription, devicePhysicalMemory=devicePhysicalMemory, spFingerprintFeedback=spFingerprintFeedback, serviceUsageDone=serviceUsageDone, mitigationTMSStartV2=mitigationTMSStartV2, spReportName=spReportName, spMitigationID=spMitigationID, spDNSObserved=spDNSObserved, spInterfaceSpeed=spInterfaceSpeed, managedObjectUsageDone=managedObjectUsageDone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spInterfaceUsageHC=spInterfaceUsageHC, spBGPTrapEvent=spBGPTrapEvent, spDNSObservedMean=spDNSObservedMean, bgpTrap=bgpTrap, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, interfaceUsage=interfaceUsage, spCloudSignalFaultDescription=spCloudSignalFaultDescription, collectorUp=collectorUp, spUsername=spUsername, spBlackholeCommunity=spBlackholeCommunity, spDNSName=spDNSName, spRouter=spRouter, peakflowSPMgr=peakflowSPMgr, spTMSCommunity=spTMSCommunity, spReportID=spReportID, bgpInstability=bgpInstability, peakflowSPCMI=peakflowSPCMI, spInterface=spInterface, spScriptPort=spScriptPort, spGreTunnelName=spGreTunnelName, spBlackholeTimeout=spBlackholeTimeout, dosHostDetectionAlertDone=dosHostDetectionAlertDone, fingerprintUsageDone=fingerprintUsageDone, reportDone=reportDone, spLicenseErrCount=spLicenseErrCount, bgpHijack=bgpHijack, deviceTotalFlowsHC=deviceTotalFlowsHC, spcommFailure=spcommFailure, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceDiskUsage=deviceDiskUsage, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spCommFailureDestination=spCommFailureDestination, trafficAutoMitigation=trafficAutoMitigation, spBGPInstability=spBGPInstability, greDown=greDown, spGreTunnelDestination=spGreTunnelDestination, spThirdPartyAddr=spThirdPartyAddr, spBGPTrapPrefix=spBGPTrapPrefix, spInetAddressType=spInetAddressType, collectorStart=collectorStart, spUsage=spUsage, snmpUp=snmpUp, spBlackholeNexthop=spBlackholeNexthop, bgpDown=bgpDown, spFlowspecCommunity=spFlowspecCommunity, spFingerprintName=spFingerprintName, spScriptHost=spScriptHost, flowDown=flowDown, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceTotalFlows=deviceTotalFlows, spAlertDetectionSignatures=spAlertDetectionSignatures, spReportURL=spReportURL, mitigationThirdPartyStart=mitigationThirdPartyStart, spImpactPps=spImpactPps, deviceHealth=deviceHealth, deviceSystemErrorDone=deviceSystemErrorDone, PYSNMP_MODULE_ID=peakflowSPMIB, spInterfaceSpeedHC=spInterfaceSpeedHC, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSTimeout=spTMSTimeout, bgpUp=bgpUp, interfaceUsageDone=interfaceUsageDone, spPravail=spPravail, spServiceName=spServiceName, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spUnit=spUnit, spTMSPrefix=spTMSPrefix, peakflowSPMIB=peakflowSPMIB, spBlackholePrefix=spBlackholePrefix, spDNSExpected=spDNSExpected, spAlertID=spAlertID, collectorDown=collectorDown, dnsBaselineDone=dnsBaselineDone, spThirdPartyZone=spThirdPartyZone, cloudSignalingFaultDone=cloudSignalingFaultDone, snmpDown=snmpDown, spScriptStart=spScriptStart, spSystemErrorType=spSystemErrorType, spInetAddress=spInetAddress, spCollector=spCollector, spDNSObservedMax=spDNSObservedMax, peakflowSPTraps=peakflowSPTraps, spMitigationName=spMitigationName, spThreshold=spThreshold, spVersion=spVersion, configChange=configChange, spInterfaceIndex=spInterfaceIndex, routingFailover=routingFailover, greDownDone=greDownDone, spDetectedCountries=spDetectedCountries, spFingerprintMessage=spFingerprintMessage, spServiceElement=spServiceElement, fingerprintUsage=fingerprintUsage, hardwareFailure=hardwareFailure, spFlowspecTimeout=spFlowspecTimeout, dosHostDetectionAlert=dosHostDetectionAlert, deviceSystemError=deviceSystemError, spManagedObject=spManagedObject, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, licenseErrorDone=licenseErrorDone, spManagedObjects=spManagedObjects, spHijackLocal=spHijackLocal, flowUp=flowUp, spDetector=spDetector, serviceUsage=serviceUsage, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dnsBaseline=dnsBaseline, spHardwareFailureDescription=spHardwareFailureDescription, bgpInstabilityDone=bgpInstabilityDone, spDuration=spDuration, cloudSignalingFault=cloudSignalingFault, fingerprintFeedback=fingerprintFeedback, spHijackAttr=spHijackAttr, spcommFailureDone=spcommFailureDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spImpactBps=spImpactBps, mitigationTMSStart=mitigationTMSStart, spScriptCommand=spScriptCommand, notificationLimit=notificationLimit, deviceSwapSpace=deviceSwapSpace, alertScript=alertScript, hardwareFailureDone=hardwareFailureDone, spInterfaceUsage=spInterfaceUsage, spMitigationStart=spMitigationStart, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spHijackRoute=spHijackRoute)
