...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceSwapSpaceInUse=deviceSwapSpaceInUse, spBlackholePrefix=spBlackholePrefix, spTMSMultiPrefix=spTMSMultiPrefix, spLicenseErrCount=spLicenseErrCount, flowDown=flowDown, spBGPInstability=spBGPInstability, mitigationTMSStart=mitigationTMSStart, spRouter=spRouter, spInterface=spInterface, spTMSCommunity=spTMSCommunity, greDown=greDown, spBGPTrapNewAttributes=spBGPTrapNewAttributes, mitigationBlackholeStart=mitigationBlackholeStart, spcommFailureDone=spcommFailureDone, routingFailover=routingFailover, spFingerprintMessage=spFingerprintMessage, spDNSName=spDNSName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, bgpUp=bgpUp, deviceSystemError=deviceSystemError, spMitigationID=spMitigationID, managedObjectUsageDone=managedObjectUsageDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBGPTrapEvent=spBGPTrapEvent, peakflowSPTraps=peakflowSPTraps, licenseError=licenseError, spUsername=spUsername, hardwareFailureDone=hardwareFailureDone, serviceUsageDone=serviceUsageDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spUsage=spUsage, spCollector=spCollector, spThirdPartyAddr=spThirdPartyAddr, spApplicationName=spApplicationName, spcommFailure=spcommFailure, spReportID=spReportID, bgpInstability=bgpInstability, spManagedObjectFamily=spManagedObjectFamily, spUsageType=spUsageType, dnsBaseline=dnsBaseline, spServiceName=spServiceName, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spManagedObject=spManagedObject, interfaceUsage=interfaceUsage, interfaceUsageDone=interfaceUsageDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, mitigationThirdPartyStart=mitigationThirdPartyStart, spImpactPps=spImpactPps, spUnit=spUnit, devicePhysicalMemory=devicePhysicalMemory, mitigationFlowspecStart=mitigationFlowspecStart, collectorUp=collectorUp, spBGPTrapPrefix=spBGPTrapPrefix, spFingerprintName=spFingerprintName, deviceTotalFlows=deviceTotalFlows, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spImpactBps=spImpactBps, deviceHealth=deviceHealth, spBGPEvent=spBGPEvent, mitigationDone=mitigationDone, PYSNMP_MODULE_ID=peakflowSPMIB, spDNSObservedMean=spDNSObservedMean, spBlackholeCommunity=spBlackholeCommunity, spDNSObserved=spDNSObserved, spBGPTrapName=spBGPTrapName, bgpInstabilityDone=bgpInstabilityDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDetector=spDetector, reportDone=reportDone, spReportURL=spReportURL, snmpUp=snmpUp, collectorStart=collectorStart, spFingerprintFeedback=spFingerprintFeedback, spBlackholeTimeout=spBlackholeTimeout, spFlowspecTimeout=spFlowspecTimeout, spInterfaceUsageHC=spInterfaceUsageHC, spGreTunnelName=spGreTunnelName, spDetectedCountries=spDetectedCountries, spGreTunnelDestination=spGreTunnelDestination, collectorDown=collectorDown, deviceDiskUsage=deviceDiskUsage, spServiceElement=spServiceElement, spLicenseErrLimit=spLicenseErrLimit, spDNSObservedMax=spDNSObservedMax, bgpTrap=bgpTrap, spAlertID=spAlertID, spLicenseErrDescription=spLicenseErrDescription, spCommFailureDestination=spCommFailureDestination, dnsBaselineDone=dnsBaselineDone, autoclassifyStart=autoclassifyStart, spInetAddressType=spInetAddressType, spVersion=spVersion, spDNSExpected=spDNSExpected, bgpDown=bgpDown, spLicenseErrType=spLicenseErrType, spBlackholeNexthop=spBlackholeNexthop, spInterfaceSpeed=spInterfaceSpeed, spThreshold=spThreshold, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spHijackRoute=spHijackRoute, spTMSPrefix=spTMSPrefix, deviceTotalFlowsHC=deviceTotalFlowsHC, alertScript=alertScript, bgpHijack=bgpHijack, fingerprintFeedback=fingerprintFeedback, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, fingerprintUsageDone=fingerprintUsageDone, spScriptHost=spScriptHost, spFingerprintSender=spFingerprintSender, snmpDown=snmpDown, peakflowSPMgr=peakflowSPMgr, fingerprintUsage=fingerprintUsage, spHijackLocal=spHijackLocal, spMitigationStart=spMitigationStart, spCommFailureDescription=spCommFailureDescription, spInterfaceSpeedHC=spInterfaceSpeedHC, hardwareFailure=hardwareFailure, spManagedObjects=spManagedObjects, configChange=configChange, spReportName=spReportName, spThirdPartyZone=spThirdPartyZone, spAlertDetectionSignatures=spAlertDetectionSignatures, spHardwareFailureDescription=spHardwareFailureDescription, spDuration=spDuration, deviceSystemErrorDone=deviceSystemErrorDone, spPravail=spPravail, peakflowSPMIB=peakflowSPMIB, spInterfaceIndex=spInterfaceIndex, deviceSwapSpace=deviceSwapSpace, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spHijackAttr=spHijackAttr, peakflowSPCMI=peakflowSPCMI, dosHostDetectionAlertDone=dosHostDetectionAlertDone, fingerprintReceive=fingerprintReceive, deviceSwapSpaceUsage=deviceSwapSpaceUsage, trafficAutoMitigation=trafficAutoMitigation, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spScriptPort=spScriptPort, spFlowspecCommunity=spFlowspecCommunity, spScriptCommand=spScriptCommand, managedObjectUsage=managedObjectUsage, spInetAddress=spInetAddress, cloudSignalingFault=cloudSignalingFault, flowUp=flowUp, spSystemErrorType=spSystemErrorType, licenseErrorDone=licenseErrorDone, spScriptStart=spScriptStart, spMitigationName=spMitigationName, spInterfaceUsage=spInterfaceUsage, mitigationTMSStartV2=mitigationTMSStartV2, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosHostDetectionAlert=dosHostDetectionAlert, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spSystemErrorDescription=spSystemErrorDescription, greDownDone=greDownDone, spTMSTimeout=spTMSTimeout, notificationLimit=notificationLimit, serviceUsage=serviceUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert)
