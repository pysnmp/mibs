...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spCommFailureDescription=spCommFailureDescription, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spGreTunnelDestination=spGreTunnelDestination, dosHostDetectionAlert=dosHostDetectionAlert, spFingerprintSender=spFingerprintSender, dosNetworkProfiledAlert=dosNetworkProfiledAlert, bgpInstabilityDone=bgpInstabilityDone, licenseErrorDone=licenseErrorDone, spInterfaceUsage=spInterfaceUsage, spScriptHost=spScriptHost, spInterfaceUsageHC=spInterfaceUsageHC, spInterfaceSpeed=spInterfaceSpeed, hardwareFailure=hardwareFailure, mitigationDone=mitigationDone, spCollector=spCollector, spManagedObjects=spManagedObjects, spReportURL=spReportURL, spGreTunnelName=spGreTunnelName, spUnit=spUnit, fingerprintUsage=fingerprintUsage, bgpTrap=bgpTrap, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spHijackRoute=spHijackRoute, spThirdPartyZone=spThirdPartyZone, spUsage=spUsage, dnsBaselineDone=dnsBaselineDone, spHijackLocal=spHijackLocal, spDNSExpected=spDNSExpected, deviceSystemErrorDone=deviceSystemErrorDone, devicePhysicalMemory=devicePhysicalMemory, peakflowSPCMI=peakflowSPCMI, fingerprintReceive=fingerprintReceive, peakflowSPTraps=peakflowSPTraps, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, cloudSignalingFault=cloudSignalingFault, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, mitigationTMSStart=mitigationTMSStart, snmpUp=snmpUp, cloudSignalingFaultDone=cloudSignalingFaultDone, spLicenseErrLimit=spLicenseErrLimit, spSystemErrorDescription=spSystemErrorDescription, spMitigationID=spMitigationID, bgpHijack=bgpHijack, hardwareFailureDone=hardwareFailureDone, interfaceUsage=interfaceUsage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spFlowspecCommunity=spFlowspecCommunity, spFlowspecTimeout=spFlowspecTimeout, spAlertID=spAlertID, deviceSystemError=deviceSystemError, spInetAddressType=spInetAddressType, spThreshold=spThreshold, spBlackholeNexthop=spBlackholeNexthop, spBlackholeCommunity=spBlackholeCommunity, spTMSPrefix=spTMSPrefix, bgpDown=bgpDown, spScriptPort=spScriptPort, flowUp=flowUp, spScriptStart=spScriptStart, spMitigationName=spMitigationName, spReportName=spReportName, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spThirdPartyAddr=spThirdPartyAddr, managedObjectUsage=managedObjectUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, snmpDown=snmpDown, fingerprintUsageDone=fingerprintUsageDone, spScriptCommand=spScriptCommand, spInetAddress=spInetAddress, spcommFailureDone=spcommFailureDone, spImpactBps=spImpactBps, spVersion=spVersion, spAlertDetectionSignatures=spAlertDetectionSignatures, spImpactPps=spImpactPps, spLicenseErrType=spLicenseErrType, spTMSCommunity=spTMSCommunity, dnsBaseline=dnsBaseline, spDetector=spDetector, peakflowSPMIB=peakflowSPMIB, spHardwareFailureDescription=spHardwareFailureDescription, spBlackholePrefix=spBlackholePrefix, autoclassifyStart=autoclassifyStart, spBlackholeTimeout=spBlackholeTimeout, spBGPTrapName=spBGPTrapName, spManagedObject=spManagedObject, mitigationFlowspecStart=mitigationFlowspecStart, PYSNMP_MODULE_ID=peakflowSPMIB, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, serviceUsageDone=serviceUsageDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spManagedObjectFamily=spManagedObjectFamily, routingFailover=routingFailover, greDown=greDown, spBGPTrapOldAttributes=spBGPTrapOldAttributes, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, notificationLimit=notificationLimit, spBGPEvent=spBGPEvent, spHijackAttr=spHijackAttr, interfaceUsageDone=interfaceUsageDone, spPravail=spPravail, spCommFailureDestination=spCommFailureDestination, spDNSName=spDNSName, spBGPTrapEvent=spBGPTrapEvent, flowDown=flowDown, greDownDone=greDownDone, deviceDiskUsage=deviceDiskUsage, deviceHealth=deviceHealth, spFingerprintFeedback=spFingerprintFeedback, spcommFailure=spcommFailure, spServiceElement=spServiceElement, collectorDown=collectorDown, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceTotalFlowsHC=deviceTotalFlowsHC, licenseError=licenseError, spDNSObservedMax=spDNSObservedMax, serviceUsage=serviceUsage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spServiceName=spServiceName, collectorStart=collectorStart, spTMSTimeout=spTMSTimeout, mitigationBlackholeStart=mitigationBlackholeStart, deviceSwapSpace=deviceSwapSpace, deviceTotalFlows=deviceTotalFlows, spRouter=spRouter, spFingerprintMessage=spFingerprintMessage, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBGPTrapPrefix=spBGPTrapPrefix, spUsageType=spUsageType, trafficAutoMitigation=trafficAutoMitigation, alertScript=alertScript, spDuration=spDuration, bgpUp=bgpUp, configChange=configChange, spDNSObservedMean=spDNSObservedMean, spTMSMultiPrefix=spTMSMultiPrefix, spLicenseErrCount=spLicenseErrCount, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spBGPInstability=spBGPInstability, peakflowSPMgr=peakflowSPMgr, collectorUp=collectorUp, mitigationThirdPartyStart=mitigationThirdPartyStart, spReportID=spReportID, managedObjectUsageDone=managedObjectUsageDone, mitigationTMSStartV2=mitigationTMSStartV2, spFingerprintName=spFingerprintName, spSystemErrorType=spSystemErrorType, spDetectedCountries=spDetectedCountries, reportDone=reportDone, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSObserved=spDNSObserved, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spMitigationStart=spMitigationStart, fingerprintFeedback=fingerprintFeedback, spApplicationName=spApplicationName, spInterface=spInterface, spUsername=spUsername, spLicenseErrDescription=spLicenseErrDescription, bgpInstability=bgpInstability, spInterfaceIndex=spInterfaceIndex)
