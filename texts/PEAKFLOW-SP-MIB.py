...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spHardwareFailureDescription=spHardwareFailureDescription, spBlackholePrefix=spBlackholePrefix, spDNSObserved=spDNSObserved, spReportName=spReportName, spServiceElement=spServiceElement, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, flowUp=flowUp, spCommFailureDestination=spCommFailureDestination, spReportURL=spReportURL, spBGPEvent=spBGPEvent, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, trafficAutoMitigation=trafficAutoMitigation, configChange=configChange, spRouter=spRouter, spInterfaceSpeed=spInterfaceSpeed, spScriptCommand=spScriptCommand, spBGPInstability=spBGPInstability, mitigationFlowspecStart=mitigationFlowspecStart, spInterfaceIndex=spInterfaceIndex, spInetAddressType=spInetAddressType, serviceUsage=serviceUsage, collectorUp=collectorUp, spDetector=spDetector, spInterfaceUsage=spInterfaceUsage, mitigationThirdPartyStart=mitigationThirdPartyStart, spVersion=spVersion, spBGPTrapEvent=spBGPTrapEvent, deviceHealth=deviceHealth, spScriptHost=spScriptHost, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spDuration=spDuration, dosHostDetectionAlert=dosHostDetectionAlert, spThirdPartyAddr=spThirdPartyAddr, deviceSwapSpaceUsage=deviceSwapSpaceUsage, collectorStart=collectorStart, spApplicationName=spApplicationName, spFingerprintFeedback=spFingerprintFeedback, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spLicenseErrDescription=spLicenseErrDescription, hardwareFailure=hardwareFailure, spMitigationStart=spMitigationStart, spPravail=spPravail, dnsBaseline=dnsBaseline, peakflowSPMgr=peakflowSPMgr, spMitigationName=spMitigationName, deviceSwapSpace=deviceSwapSpace, spLicenseErrType=spLicenseErrType, managedObjectUsage=managedObjectUsage, spBlackholeCommunity=spBlackholeCommunity, cloudSignalingFaultDone=cloudSignalingFaultDone, spSystemErrorDescription=spSystemErrorDescription, cloudSignalingFault=cloudSignalingFault, bgpInstability=bgpInstability, spcommFailure=spcommFailure, fingerprintReceive=fingerprintReceive, deviceDiskUsage=deviceDiskUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spFlowspecTimeout=spFlowspecTimeout, greDownDone=greDownDone, PYSNMP_MODULE_ID=peakflowSPMIB, spHijackAttr=spHijackAttr, mitigationTMSStart=mitigationTMSStart, spcommFailureDone=spcommFailureDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spHijackRoute=spHijackRoute, spImpactBps=spImpactBps, fingerprintUsage=fingerprintUsage, bgpTrap=bgpTrap, spTMSCommunity=spTMSCommunity, deviceSwapSpaceInUse=deviceSwapSpaceInUse, mitigationDone=mitigationDone, snmpDown=snmpDown, spManagedObjects=spManagedObjects, spGreTunnelName=spGreTunnelName, spFlowspecCommunity=spFlowspecCommunity, spDNSObservedMax=spDNSObservedMax, collectorDown=collectorDown, managedObjectUsageDone=managedObjectUsageDone, spManagedObjectFamily=spManagedObjectFamily, spScriptPort=spScriptPort, spInetAddress=spInetAddress, spUsername=spUsername, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spAlertDetectionSignatures=spAlertDetectionSignatures, spDetectedCountries=spDetectedCountries, spTMSMultiPrefix=spTMSMultiPrefix, spAlertID=spAlertID, spServiceName=spServiceName, spLicenseErrCount=spLicenseErrCount, licenseError=licenseError, spHijackLocal=spHijackLocal, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, autoclassifyStart=autoclassifyStart, deviceSystemError=deviceSystemError, deviceTotalFlows=deviceTotalFlows, spDNSObservedMean=spDNSObservedMean, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceSystemErrorDone=deviceSystemErrorDone, spTMSTimeout=spTMSTimeout, spSystemErrorType=spSystemErrorType, peakflowSPCMI=peakflowSPCMI, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spInterfaceUsageHC=spInterfaceUsageHC, spUnit=spUnit, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, dnsBaselineDone=dnsBaselineDone, fingerprintFeedback=fingerprintFeedback, spBlackholeNexthop=spBlackholeNexthop, flowDown=flowDown, interfaceUsageDone=interfaceUsageDone, spReportID=spReportID, spCommFailureDescription=spCommFailureDescription, interfaceUsage=interfaceUsage, licenseErrorDone=licenseErrorDone, notificationLimit=notificationLimit, spThirdPartyZone=spThirdPartyZone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spFingerprintSender=spFingerprintSender, snmpUp=snmpUp, routingFailover=routingFailover, bgpInstabilityDone=bgpInstabilityDone, greDown=greDown, spLicenseErrLimit=spLicenseErrLimit, hardwareFailureDone=hardwareFailureDone, spFingerprintName=spFingerprintName, spImpactPps=spImpactPps, spDNSExpected=spDNSExpected, spBlackholeTimeout=spBlackholeTimeout, peakflowSPMIB=peakflowSPMIB, alertScript=alertScript, serviceUsageDone=serviceUsageDone, peakflowSPTraps=peakflowSPTraps, devicePhysicalMemory=devicePhysicalMemory, spFingerprintMessage=spFingerprintMessage, mitigationBlackholeStart=mitigationBlackholeStart, spBGPTrapName=spBGPTrapName, spUsage=spUsage, bgpUp=bgpUp, spGreTunnelDestination=spGreTunnelDestination, spBGPTrapPrefix=spBGPTrapPrefix, deviceTotalFlowsHC=deviceTotalFlowsHC, fingerprintUsageDone=fingerprintUsageDone, mitigationTMSStartV2=mitigationTMSStartV2, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spUsageType=spUsageType, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spThreshold=spThreshold, bgpDown=bgpDown, bgpHijack=bgpHijack, spManagedObject=spManagedObject, reportDone=reportDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterface=spInterface, spScriptStart=spScriptStart, spDNSName=spDNSName, spInterfaceSpeedHC=spInterfaceSpeedHC, spTMSPrefix=spTMSPrefix, spMitigationID=spMitigationID, spCollector=spCollector)
