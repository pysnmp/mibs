...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spHijackAttr=spHijackAttr, spTMSMultiPrefix=spTMSMultiPrefix, spcommFailure=spcommFailure, configChange=configChange, mitigationFlowspecStart=mitigationFlowspecStart, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBGPTrapPrefix=spBGPTrapPrefix, spDNSName=spDNSName, deviceDiskUsage=deviceDiskUsage, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spServiceName=spServiceName, reportDone=reportDone, peakflowSPTraps=peakflowSPTraps, peakflowSPCMI=peakflowSPCMI, spInterfaceUsage=spInterfaceUsage, serviceUsageDone=serviceUsageDone, spThirdPartyZone=spThirdPartyZone, spBlackholeTimeout=spBlackholeTimeout, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spFingerprintFeedback=spFingerprintFeedback, spInetAddress=spInetAddress, trafficAutoMitigation=trafficAutoMitigation, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spInterfaceUsageHC=spInterfaceUsageHC, spDNSObserved=spDNSObserved, spHardwareFailureDescription=spHardwareFailureDescription, fingerprintFeedback=fingerprintFeedback, fingerprintUsageDone=fingerprintUsageDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spcommFailureDone=spcommFailureDone, mitigationDone=mitigationDone, spUsername=spUsername, spBlackholePrefix=spBlackholePrefix, spImpactPps=spImpactPps, interfaceUsage=interfaceUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spFlowspecCommunity=spFlowspecCommunity, bgpTrap=bgpTrap, notificationLimit=notificationLimit, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spScriptHost=spScriptHost, dnsBaselineDone=dnsBaselineDone, deviceTotalFlows=deviceTotalFlows, spTMSPrefix=spTMSPrefix, PYSNMP_MODULE_ID=peakflowSPMIB, spServiceElement=spServiceElement, spImpactBps=spImpactBps, spBGPTrapEvent=spBGPTrapEvent, spDuration=spDuration, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spVersion=spVersion, fingerprintUsage=fingerprintUsage, spBlackholeCommunity=spBlackholeCommunity, spFlowspecTimeout=spFlowspecTimeout, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spFingerprintName=spFingerprintName, spFingerprintMessage=spFingerprintMessage, spSystemErrorType=spSystemErrorType, dnsBaseline=dnsBaseline, spManagedObjects=spManagedObjects, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceSwapSpaceInUse=deviceSwapSpaceInUse, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, bgpDown=bgpDown, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, bgpInstabilityDone=bgpInstabilityDone, bgpInstability=bgpInstability, spLicenseErrDescription=spLicenseErrDescription, snmpUp=snmpUp, hardwareFailureDone=hardwareFailureDone, spScriptStart=spScriptStart, spDNSObservedMean=spDNSObservedMean, spManagedObjectFamily=spManagedObjectFamily, spReportURL=spReportURL, spUnit=spUnit, spTMSTimeout=spTMSTimeout, spHijackLocal=spHijackLocal, bgpUp=bgpUp, dosHostDetectionAlert=dosHostDetectionAlert, spBGPInstability=spBGPInstability, spScriptCommand=spScriptCommand, spAlertID=spAlertID, licenseErrorDone=licenseErrorDone, mitigationTMSStartV2=mitigationTMSStartV2, greDown=greDown, alertScript=alertScript, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, deviceTotalFlowsHC=deviceTotalFlowsHC, spReportName=spReportName, managedObjectUsage=managedObjectUsage, collectorStart=collectorStart, spUsageType=spUsageType, spBGPTrapName=spBGPTrapName, spManagedObject=spManagedObject, collectorDown=collectorDown, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spCollector=spCollector, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spPravail=spPravail, spAlertDetectionSignatures=spAlertDetectionSignatures, spCommFailureDestination=spCommFailureDestination, mitigationBlackholeStart=mitigationBlackholeStart, licenseError=licenseError, spInetAddressType=spInetAddressType, spReportID=spReportID, autoclassifyStart=autoclassifyStart, spInterface=spInterface, peakflowSPMIB=peakflowSPMIB, spGreTunnelDestination=spGreTunnelDestination, serviceUsage=serviceUsage, spHijackRoute=spHijackRoute, spLicenseErrLimit=spLicenseErrLimit, cloudSignalingFaultDone=cloudSignalingFaultDone, peakflowSPMgr=peakflowSPMgr, spScriptPort=spScriptPort, spMitigationStart=spMitigationStart, spLicenseErrType=spLicenseErrType, flowDown=flowDown, managedObjectUsageDone=managedObjectUsageDone, spDetector=spDetector, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spTMSCommunity=spTMSCommunity, spInterfaceIndex=spInterfaceIndex, spGreTunnelName=spGreTunnelName, deviceSystemError=deviceSystemError, interfaceUsageDone=interfaceUsageDone, spThreshold=spThreshold, spThirdPartyAddr=spThirdPartyAddr, routingFailover=routingFailover, spCommFailureDescription=spCommFailureDescription, spDNSObservedMax=spDNSObservedMax, deviceSwapSpace=deviceSwapSpace, spRouter=spRouter, greDownDone=greDownDone, snmpDown=snmpDown, spInterfaceSpeed=spInterfaceSpeed, spInterfaceSpeedHC=spInterfaceSpeedHC, collectorUp=collectorUp, flowUp=flowUp, mitigationTMSStart=mitigationTMSStart, spDetectedCountries=spDetectedCountries, spBlackholeNexthop=spBlackholeNexthop, spDNSExpected=spDNSExpected, bgpHijack=bgpHijack, spSystemErrorDescription=spSystemErrorDescription, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spLicenseErrCount=spLicenseErrCount, fingerprintReceive=fingerprintReceive, spMitigationID=spMitigationID, hardwareFailure=hardwareFailure, deviceSystemErrorDone=deviceSystemErrorDone, cloudSignalingFault=cloudSignalingFault, spBGPEvent=spBGPEvent, deviceHealth=deviceHealth, spUsage=spUsage, spApplicationName=spApplicationName, devicePhysicalMemory=devicePhysicalMemory, mitigationThirdPartyStart=mitigationThirdPartyStart, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spFingerprintSender=spFingerprintSender, spMitigationName=spMitigationName)
