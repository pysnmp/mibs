...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceIndex=spInterfaceIndex, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFingerprintName=spFingerprintName, deviceSystemError=deviceSystemError, spDNSObservedMax=spDNSObservedMax, spDNSName=spDNSName, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spBlackholeCommunity=spBlackholeCommunity, spInterfaceSpeedHC=spInterfaceSpeedHC, spFingerprintFeedback=spFingerprintFeedback, spcommFailure=spcommFailure, spBlackholeTimeout=spBlackholeTimeout, spAlertDetectionSignatures=spAlertDetectionSignatures, spCollector=spCollector, fingerprintReceive=fingerprintReceive, managedObjectUsageDone=managedObjectUsageDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFingerprintSender=spFingerprintSender, spScriptHost=spScriptHost, spManagedObjects=spManagedObjects, spCommFailureDescription=spCommFailureDescription, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spApplicationName=spApplicationName, spManagedObject=spManagedObject, collectorUp=collectorUp, interfaceUsage=interfaceUsage, spHijackRoute=spHijackRoute, spUnit=spUnit, spBGPTrapEvent=spBGPTrapEvent, snmpDown=snmpDown, spFlowspecTimeout=spFlowspecTimeout, spTMSMultiPrefix=spTMSMultiPrefix, spLicenseErrDescription=spLicenseErrDescription, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, bgpHijack=bgpHijack, spInterfaceSpeed=spInterfaceSpeed, collectorDown=collectorDown, devicePhysicalMemory=devicePhysicalMemory, spDetectedCountries=spDetectedCountries, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, dosHostDetectionAlert=dosHostDetectionAlert, spDNSExpected=spDNSExpected, notificationLimit=notificationLimit, spMitigationName=spMitigationName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spInetAddress=spInetAddress, mitigationFlowspecStart=mitigationFlowspecStart, spTMSCommunity=spTMSCommunity, spDNSObserved=spDNSObserved, dnsBaselineDone=dnsBaselineDone, collectorStart=collectorStart, spTMSPrefix=spTMSPrefix, spDuration=spDuration, spMitigationStart=spMitigationStart, deviceSwapSpace=deviceSwapSpace, deviceHealth=deviceHealth, mitigationThirdPartyStart=mitigationThirdPartyStart, spSystemErrorDescription=spSystemErrorDescription, autoclassifyStart=autoclassifyStart, routingFailover=routingFailover, flowUp=flowUp, spTMSTimeout=spTMSTimeout, dnsBaseline=dnsBaseline, flowDown=flowDown, spBGPEvent=spBGPEvent, spGreTunnelName=spGreTunnelName, mitigationTMSStart=mitigationTMSStart, spDNSObservedMean=spDNSObservedMean, spUsername=spUsername, mitigationBlackholeStart=mitigationBlackholeStart, spLicenseErrType=spLicenseErrType, bgpInstabilityDone=bgpInstabilityDone, hardwareFailure=hardwareFailure, fingerprintFeedback=fingerprintFeedback, spBlackholePrefix=spBlackholePrefix, spScriptPort=spScriptPort, spInterface=spInterface, spFingerprintMessage=spFingerprintMessage, spRouter=spRouter, spManagedObjectFamily=spManagedObjectFamily, spReportURL=spReportURL, spReportName=spReportName, spVersion=spVersion, spPravail=spPravail, bgpTrap=bgpTrap, spAlertID=spAlertID, greDown=greDown, bgpDown=bgpDown, spHijackLocal=spHijackLocal, interfaceUsageDone=interfaceUsageDone, spHijackAttr=spHijackAttr, serviceUsageDone=serviceUsageDone, greDownDone=greDownDone, spCommFailureDestination=spCommFailureDestination, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spFlowspecCommunity=spFlowspecCommunity, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, peakflowSPMIB=peakflowSPMIB, cloudSignalingFaultDone=cloudSignalingFaultDone, spHardwareFailureDescription=spHardwareFailureDescription, spImpactBps=spImpactBps, licenseErrorDone=licenseErrorDone, reportDone=reportDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spServiceElement=spServiceElement, spScriptStart=spScriptStart, spMitigationID=spMitigationID, spUsageType=spUsageType, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spThirdPartyZone=spThirdPartyZone, hardwareFailureDone=hardwareFailureDone, spcommFailureDone=spcommFailureDone, spBGPTrapName=spBGPTrapName, peakflowSPCMI=peakflowSPCMI, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, serviceUsage=serviceUsage, spServiceName=spServiceName, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spReportID=spReportID, spGreTunnelDestination=spGreTunnelDestination, managedObjectUsage=managedObjectUsage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, cloudSignalingFault=cloudSignalingFault, licenseError=licenseError, spThirdPartyAddr=spThirdPartyAddr, alertScript=alertScript, mitigationDone=mitigationDone, spSystemErrorType=spSystemErrorType, spBlackholeNexthop=spBlackholeNexthop, peakflowSPMgr=peakflowSPMgr, peakflowSPTraps=peakflowSPTraps, spImpactPps=spImpactPps, trafficAutoMitigation=trafficAutoMitigation, spDetector=spDetector, spInetAddressType=spInetAddressType, spThreshold=spThreshold, deviceTotalFlowsHC=deviceTotalFlowsHC, spInterfaceUsage=spInterfaceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, deviceTotalFlows=deviceTotalFlows, spBGPTrapPrefix=spBGPTrapPrefix, spScriptCommand=spScriptCommand, spInterfaceUsageHC=spInterfaceUsageHC, bgpUp=bgpUp, fingerprintUsageDone=fingerprintUsageDone, deviceSystemErrorDone=deviceSystemErrorDone, spUsage=spUsage, bgpInstability=bgpInstability, spCloudSignalFaultDescription=spCloudSignalFaultDescription, configChange=configChange, spLicenseErrLimit=spLicenseErrLimit, mitigationTMSStartV2=mitigationTMSStartV2, snmpUp=snmpUp, spBGPInstability=spBGPInstability, fingerprintUsage=fingerprintUsage, spLicenseErrCount=spLicenseErrCount, deviceDiskUsage=deviceDiskUsage, dosNetworkProfiledAlert=dosNetworkProfiledAlert)
