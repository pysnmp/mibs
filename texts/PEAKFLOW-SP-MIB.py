...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, peakflowSPMgr=peakflowSPMgr, spReportID=spReportID, configChange=configChange, hardwareFailure=hardwareFailure, spDNSObservedMean=spDNSObservedMean, spThirdPartyZone=spThirdPartyZone, managedObjectUsage=managedObjectUsage, spInterfaceIndex=spInterfaceIndex, spFingerprintFeedback=spFingerprintFeedback, spServiceName=spServiceName, dnsBaselineDone=dnsBaselineDone, spSystemErrorDescription=spSystemErrorDescription, snmpDown=snmpDown, spFlowspecTimeout=spFlowspecTimeout, spCommFailureDescription=spCommFailureDescription, spVersion=spVersion, spDNSObservedMax=spDNSObservedMax, flowDown=flowDown, spLicenseErrCount=spLicenseErrCount, dnsBaseline=dnsBaseline, deviceSystemError=deviceSystemError, collectorUp=collectorUp, spLicenseErrLimit=spLicenseErrLimit, spDuration=spDuration, spCommFailureDestination=spCommFailureDestination, notificationLimit=notificationLimit, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spGreTunnelDestination=spGreTunnelDestination, spPravail=spPravail, peakflowSPMIB=peakflowSPMIB, serviceUsage=serviceUsage, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, mitigationTMSStartV2=mitigationTMSStartV2, spBGPEvent=spBGPEvent, spBlackholeNexthop=spBlackholeNexthop, spFingerprintMessage=spFingerprintMessage, spScriptPort=spScriptPort, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceTotalFlows=deviceTotalFlows, mitigationThirdPartyStart=mitigationThirdPartyStart, snmpUp=snmpUp, spDNSExpected=spDNSExpected, spFingerprintSender=spFingerprintSender, deviceHealth=deviceHealth, spUsage=spUsage, bgpHijack=bgpHijack, greDownDone=greDownDone, mitigationTMSStart=mitigationTMSStart, spThreshold=spThreshold, spBlackholeTimeout=spBlackholeTimeout, fingerprintUsage=fingerprintUsage, spManagedObjects=spManagedObjects, bgpInstabilityDone=bgpInstabilityDone, spLicenseErrType=spLicenseErrType, bgpUp=bgpUp, spBGPTrapEvent=spBGPTrapEvent, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spManagedObjectFamily=spManagedObjectFamily, trafficAutoMitigation=trafficAutoMitigation, cloudSignalingFault=cloudSignalingFault, spTMSPrefix=spTMSPrefix, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spScriptCommand=spScriptCommand, peakflowSPTraps=peakflowSPTraps, spScriptHost=spScriptHost, autoclassifyStart=autoclassifyStart, licenseErrorDone=licenseErrorDone, greDown=greDown, spHijackAttr=spHijackAttr, spDetectedCountries=spDetectedCountries, dosHostDetectionAlertDone=dosHostDetectionAlertDone, dosHostDetectionAlert=dosHostDetectionAlert, spTMSMultiPrefix=spTMSMultiPrefix, fingerprintUsageDone=fingerprintUsageDone, bgpDown=bgpDown, spUnit=spUnit, spMitigationID=spMitigationID, spGreTunnelName=spGreTunnelName, cloudSignalingFaultDone=cloudSignalingFaultDone, mitigationBlackholeStart=mitigationBlackholeStart, interfaceUsageDone=interfaceUsageDone, deviceSystemErrorDone=deviceSystemErrorDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spInetAddress=spInetAddress, spAlertID=spAlertID, peakflowSPCMI=peakflowSPCMI, managedObjectUsageDone=managedObjectUsageDone, PYSNMP_MODULE_ID=peakflowSPMIB, deviceSwapSpace=deviceSwapSpace, flowUp=flowUp, bgpTrap=bgpTrap, spMitigationName=spMitigationName, spBlackholeCommunity=spBlackholeCommunity, spcommFailure=spcommFailure, spInterfaceSpeed=spInterfaceSpeed, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spAlertDetectionSignatures=spAlertDetectionSignatures, spInetAddressType=spInetAddressType, spLicenseErrDescription=spLicenseErrDescription, fingerprintFeedback=fingerprintFeedback, spReportURL=spReportURL, mitigationFlowspecStart=mitigationFlowspecStart, spHijackRoute=spHijackRoute, collectorStart=collectorStart, spUsername=spUsername, spInterface=spInterface, spBlackholePrefix=spBlackholePrefix, fingerprintReceive=fingerprintReceive, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, serviceUsageDone=serviceUsageDone, devicePhysicalMemory=devicePhysicalMemory, spRouter=spRouter, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, reportDone=reportDone, spDNSName=spDNSName, spInterfaceSpeedHC=spInterfaceSpeedHC, spApplicationName=spApplicationName, spScriptStart=spScriptStart, spReportName=spReportName, spInterfaceUsageHC=spInterfaceUsageHC, deviceSwapSpaceInUse=deviceSwapSpaceInUse, alertScript=alertScript, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spManagedObject=spManagedObject, spMitigationStart=spMitigationStart, spUsageType=spUsageType, spThirdPartyAddr=spThirdPartyAddr, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spFingerprintName=spFingerprintName, spCollector=spCollector, bgpInstability=bgpInstability, interfaceUsage=interfaceUsage, routingFailover=routingFailover, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spBGPTrapPrefix=spBGPTrapPrefix, spImpactPps=spImpactPps, spcommFailureDone=spcommFailureDone, deviceTotalFlowsHC=deviceTotalFlowsHC, spTMSTimeout=spTMSTimeout, spDNSObserved=spDNSObserved, spFlowspecCommunity=spFlowspecCommunity, licenseError=licenseError, spBGPInstability=spBGPInstability, spHardwareFailureDescription=spHardwareFailureDescription, spServiceElement=spServiceElement, collectorDown=collectorDown, mitigationDone=mitigationDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spHijackLocal=spHijackLocal, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spTMSCommunity=spTMSCommunity, hardwareFailureDone=hardwareFailureDone, spInterfaceUsage=spInterfaceUsage, spSystemErrorType=spSystemErrorType, spImpactBps=spImpactBps, spDetector=spDetector, deviceDiskUsage=deviceDiskUsage, spBGPTrapName=spBGPTrapName)
