...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDNSObserved=spDNSObserved, spInterfaceUsageHC=spInterfaceUsageHC, fingerprintFeedback=fingerprintFeedback, bgpInstabilityDone=bgpInstabilityDone, spHijackRoute=spHijackRoute, spcommFailure=spcommFailure, peakflowSPCMI=peakflowSPCMI, spFingerprintName=spFingerprintName, spFlowspecCommunity=spFlowspecCommunity, spFingerprintMessage=spFingerprintMessage, mitigationTMSStart=mitigationTMSStart, spManagedObjects=spManagedObjects, bgpUp=bgpUp, deviceSystemError=deviceSystemError, spFingerprintSender=spFingerprintSender, routingFailover=routingFailover, spBlackholeCommunity=spBlackholeCommunity, spApplicationName=spApplicationName, snmpUp=snmpUp, spBGPEvent=spBGPEvent, spCommFailureDescription=spCommFailureDescription, dosHostDetectionAlertDone=dosHostDetectionAlertDone, deviceHealth=deviceHealth, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, interfaceUsage=interfaceUsage, fingerprintReceive=fingerprintReceive, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spFlowspecTimeout=spFlowspecTimeout, mitigationDone=mitigationDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, interfaceUsageDone=interfaceUsageDone, spLicenseErrType=spLicenseErrType, spBlackholeNexthop=spBlackholeNexthop, spServiceElement=spServiceElement, cloudSignalingFaultDone=cloudSignalingFaultDone, spCollector=spCollector, managedObjectUsageDone=managedObjectUsageDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, snmpDown=snmpDown, spTMSCommunity=spTMSCommunity, spLicenseErrDescription=spLicenseErrDescription, greDownDone=greDownDone, peakflowSPMgr=peakflowSPMgr, dnsBaseline=dnsBaseline, spTMSMultiPrefix=spTMSMultiPrefix, spImpactPps=spImpactPps, hardwareFailure=hardwareFailure, spBlackholeTimeout=spBlackholeTimeout, licenseError=licenseError, spTMSPrefix=spTMSPrefix, spTMSTimeout=spTMSTimeout, flowUp=flowUp, spScriptPort=spScriptPort, spReportID=spReportID, spBlackholePrefix=spBlackholePrefix, spLicenseErrLimit=spLicenseErrLimit, spHardwareFailureDescription=spHardwareFailureDescription, bgpHijack=bgpHijack, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spFingerprintFeedback=spFingerprintFeedback, configChange=configChange, spVersion=spVersion, spUsage=spUsage, spAlertID=spAlertID, spServiceName=spServiceName, spManagedObject=spManagedObject, spDNSObservedMean=spDNSObservedMean, collectorStart=collectorStart, fingerprintUsage=fingerprintUsage, deviceSwapSpace=deviceSwapSpace, spBGPInstability=spBGPInstability, reportDone=reportDone, spLicenseErrCount=spLicenseErrCount, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spGreTunnelName=spGreTunnelName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spScriptStart=spScriptStart, spcommFailureDone=spcommFailureDone, spDNSExpected=spDNSExpected, spHijackLocal=spHijackLocal, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spCommFailureDestination=spCommFailureDestination, serviceUsageDone=serviceUsageDone, spBGPTrapPrefix=spBGPTrapPrefix, spInetAddress=spInetAddress, deviceTotalFlows=deviceTotalFlows, spRouter=spRouter, spMitigationName=spMitigationName, spGreTunnelDestination=spGreTunnelDestination, deviceTotalFlowsHC=deviceTotalFlowsHC, spInterface=spInterface, alertScript=alertScript, spPravail=spPravail, cloudSignalingFault=cloudSignalingFault, spBGPTrapName=spBGPTrapName, flowDown=flowDown, spBGPTrapNewAttributes=spBGPTrapNewAttributes, PYSNMP_MODULE_ID=peakflowSPMIB, spScriptCommand=spScriptCommand, spDetectedCountries=spDetectedCountries, spUsername=spUsername, bgpDown=bgpDown, mitigationBlackholeStart=mitigationBlackholeStart, dnsBaselineDone=dnsBaselineDone, deviceSystemErrorDone=deviceSystemErrorDone, hardwareFailureDone=hardwareFailureDone, spReportName=spReportName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spDuration=spDuration, spInterfaceSpeed=spInterfaceSpeed, greDown=greDown, spAlertDetectionSignatures=spAlertDetectionSignatures, autoclassifyStart=autoclassifyStart, fingerprintUsageDone=fingerprintUsageDone, spHijackAttr=spHijackAttr, mitigationTMSStartV2=mitigationTMSStartV2, devicePhysicalMemory=devicePhysicalMemory, spSystemErrorType=spSystemErrorType, spInterfaceUsage=spInterfaceUsage, serviceUsage=serviceUsage, spManagedObjectFamily=spManagedObjectFamily, notificationLimit=notificationLimit, licenseErrorDone=licenseErrorDone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spScriptHost=spScriptHost, trafficAutoMitigation=trafficAutoMitigation, spUsageType=spUsageType, spThreshold=spThreshold, spThirdPartyZone=spThirdPartyZone, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, mitigationFlowspecStart=mitigationFlowspecStart, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, peakflowSPMIB=peakflowSPMIB, spUnit=spUnit, spMitigationStart=spMitigationStart, deviceDiskUsage=deviceDiskUsage, mitigationThirdPartyStart=mitigationThirdPartyStart, collectorUp=collectorUp, managedObjectUsage=managedObjectUsage, spInterfaceIndex=spInterfaceIndex, spReportURL=spReportURL, bgpTrap=bgpTrap, spBGPTrapEvent=spBGPTrapEvent, spDNSObservedMax=spDNSObservedMax, spDNSName=spDNSName, spSystemErrorDescription=spSystemErrorDescription, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spMitigationID=spMitigationID, spInetAddressType=spInetAddressType, spDetector=spDetector, bgpInstability=bgpInstability, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spThirdPartyAddr=spThirdPartyAddr, collectorDown=collectorDown, peakflowSPTraps=peakflowSPTraps, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosHostDetectionAlert=dosHostDetectionAlert, spImpactBps=spImpactBps, deviceSwapSpaceInUse=deviceSwapSpaceInUse)
