...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", mitigationTMSStartV2=mitigationTMSStartV2, spTMSTimeout=spTMSTimeout, spBlackholePrefix=spBlackholePrefix, peakflowSPMgr=peakflowSPMgr, spCollector=spCollector, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceSwapSpace=deviceSwapSpace, spReportID=spReportID, spImpactBps=spImpactBps, spAlertID=spAlertID, managedObjectUsage=managedObjectUsage, spReportName=spReportName, fingerprintUsage=fingerprintUsage, deviceTotalFlowsHC=deviceTotalFlowsHC, spTMSMultiPrefix=spTMSMultiPrefix, spFingerprintSender=spFingerprintSender, deviceSystemError=deviceSystemError, spBlackholeCommunity=spBlackholeCommunity, licenseErrorDone=licenseErrorDone, spManagedObject=spManagedObject, peakflowSPTraps=peakflowSPTraps, spCommFailureDescription=spCommFailureDescription, snmpUp=snmpUp, notificationLimit=notificationLimit, deviceDiskUsage=deviceDiskUsage, spFingerprintName=spFingerprintName, spTMSPrefix=spTMSPrefix, spAlertDetectionSignatures=spAlertDetectionSignatures, hardwareFailure=hardwareFailure, spBGPInstability=spBGPInstability, spFlowspecTimeout=spFlowspecTimeout, alertScript=alertScript, spFlowspecCommunity=spFlowspecCommunity, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spHardwareFailureDescription=spHardwareFailureDescription, dnsBaseline=dnsBaseline, cloudSignalingFaultDone=cloudSignalingFaultDone, interfaceUsage=interfaceUsage, spInterfaceUsageHC=spInterfaceUsageHC, spImpactPps=spImpactPps, spInterface=spInterface, spUnit=spUnit, spDNSExpected=spDNSExpected, configChange=configChange, spSystemErrorType=spSystemErrorType, routingFailover=routingFailover, spDetectedCountries=spDetectedCountries, spInterfaceUsage=spInterfaceUsage, deviceTotalFlows=deviceTotalFlows, spcommFailureDone=spcommFailureDone, spInetAddress=spInetAddress, spLicenseErrDescription=spLicenseErrDescription, peakflowSPMIB=peakflowSPMIB, spHijackAttr=spHijackAttr, spThreshold=spThreshold, spDNSObservedMax=spDNSObservedMax, spServiceName=spServiceName, bgpUp=bgpUp, spUsage=spUsage, spRouter=spRouter, spBGPTrapNewAttributes=spBGPTrapNewAttributes, flowUp=flowUp, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, cloudSignalingFault=cloudSignalingFault, deviceHealth=deviceHealth, bgpInstabilityDone=bgpInstabilityDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPTrapName=spBGPTrapName, spInetAddressType=spInetAddressType, bgpDown=bgpDown, spHijackLocal=spHijackLocal, licenseError=licenseError, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceSystemErrorDone=deviceSystemErrorDone, spBGPTrapEvent=spBGPTrapEvent, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spInterfaceSpeedHC=spInterfaceSpeedHC, spBGPEvent=spBGPEvent, spBGPTrapPrefix=spBGPTrapPrefix, spScriptCommand=spScriptCommand, spPravail=spPravail, spGreTunnelName=spGreTunnelName, dosHostDetectionAlertDone=dosHostDetectionAlertDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spDNSObservedMean=spDNSObservedMean, dosHostDetectionAlert=dosHostDetectionAlert, spMitigationStart=spMitigationStart, spDNSName=spDNSName, dnsBaselineDone=dnsBaselineDone, spHijackRoute=spHijackRoute, spDuration=spDuration, fingerprintReceive=fingerprintReceive, bgpInstability=bgpInstability, spInterfaceSpeed=spInterfaceSpeed, snmpDown=snmpDown, spFingerprintMessage=spFingerprintMessage, spDNSObserved=spDNSObserved, deviceSwapSpaceUsage=deviceSwapSpaceUsage, hardwareFailureDone=hardwareFailureDone, managedObjectUsageDone=managedObjectUsageDone, spServiceElement=spServiceElement, greDown=greDown, spTMSCommunity=spTMSCommunity, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, devicePhysicalMemory=devicePhysicalMemory, peakflowSPCMI=peakflowSPCMI, PYSNMP_MODULE_ID=peakflowSPMIB, spSystemErrorDescription=spSystemErrorDescription, spLicenseErrLimit=spLicenseErrLimit, spUsername=spUsername, spScriptStart=spScriptStart, mitigationFlowspecStart=mitigationFlowspecStart, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBlackholeTimeout=spBlackholeTimeout, bgpHijack=bgpHijack, mitigationBlackholeStart=mitigationBlackholeStart, spScriptPort=spScriptPort, spApplicationName=spApplicationName, collectorUp=collectorUp, trafficAutoMitigation=trafficAutoMitigation, spCloudSignalFaultDescription=spCloudSignalFaultDescription, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, interfaceUsageDone=interfaceUsageDone, spManagedObjects=spManagedObjects, spReportURL=spReportURL, serviceUsageDone=serviceUsageDone, bgpTrap=bgpTrap, spcommFailure=spcommFailure, reportDone=reportDone, spGreTunnelDestination=spGreTunnelDestination, spMitigationName=spMitigationName, spManagedObjectFamily=spManagedObjectFamily, spFingerprintFeedback=spFingerprintFeedback, spLicenseErrCount=spLicenseErrCount, flowDown=flowDown, fingerprintFeedback=fingerprintFeedback, collectorDown=collectorDown, spThirdPartyZone=spThirdPartyZone, spUsageType=spUsageType, autoclassifyStart=autoclassifyStart, collectorStart=collectorStart, serviceUsage=serviceUsage, spLicenseErrType=spLicenseErrType, spMitigationID=spMitigationID, spScriptHost=spScriptHost, spCommFailureDestination=spCommFailureDestination, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, mitigationDone=mitigationDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spDetector=spDetector, mitigationTMSStart=mitigationTMSStart, greDownDone=greDownDone, spInterfaceIndex=spInterfaceIndex, spBlackholeNexthop=spBlackholeNexthop, spVersion=spVersion, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spThirdPartyAddr=spThirdPartyAddr, fingerprintUsageDone=fingerprintUsageDone)
