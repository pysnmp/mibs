...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBGPTrapNewAttributes=spBGPTrapNewAttributes, spDuration=spDuration, deviceTotalFlows=deviceTotalFlows, spDNSObservedMax=spDNSObservedMax, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spManagedObject=spManagedObject, spInterfaceUsage=spInterfaceUsage, spServiceElement=spServiceElement, spHijackAttr=spHijackAttr, spDetector=spDetector, alertScript=alertScript, bgpInstability=bgpInstability, greDown=greDown, mitigationBlackholeStart=mitigationBlackholeStart, spcommFailureDone=spcommFailureDone, spScriptCommand=spScriptCommand, spInetAddress=spInetAddress, mitigationTMSStart=mitigationTMSStart, deviceSystemError=deviceSystemError, spTMSCommunity=spTMSCommunity, spUsage=spUsage, licenseErrorDone=licenseErrorDone, spManagedObjects=spManagedObjects, autoclassifyStart=autoclassifyStart, spInterfaceUsageHC=spInterfaceUsageHC, fingerprintReceive=fingerprintReceive, spBGPInstability=spBGPInstability, spMitigationStart=spMitigationStart, bgpUp=bgpUp, dnsBaseline=dnsBaseline, spVersion=spVersion, spFingerprintMessage=spFingerprintMessage, spInterface=spInterface, spGreTunnelName=spGreTunnelName, spReportName=spReportName, spCommFailureDescription=spCommFailureDescription, flowDown=flowDown, peakflowSPMgr=peakflowSPMgr, collectorUp=collectorUp, PYSNMP_MODULE_ID=peakflowSPMIB, spMitigationName=spMitigationName, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, mitigationFlowspecStart=mitigationFlowspecStart, spTMSMultiPrefix=spTMSMultiPrefix, spHijackRoute=spHijackRoute, spBlackholeNexthop=spBlackholeNexthop, bgpTrap=bgpTrap, spLicenseErrType=spLicenseErrType, bgpHijack=bgpHijack, spScriptHost=spScriptHost, greDownDone=greDownDone, spBGPTrapPrefix=spBGPTrapPrefix, spReportID=spReportID, spHijackLocal=spHijackLocal, spDNSExpected=spDNSExpected, spLicenseErrCount=spLicenseErrCount, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, peakflowSPMIB=peakflowSPMIB, spcommFailure=spcommFailure, licenseError=licenseError, fingerprintUsageDone=fingerprintUsageDone, spDNSObserved=spDNSObserved, spScriptStart=spScriptStart, spFingerprintSender=spFingerprintSender, spAlertID=spAlertID, spFingerprintName=spFingerprintName, flowUp=flowUp, mitigationThirdPartyStart=mitigationThirdPartyStart, devicePhysicalMemory=devicePhysicalMemory, routingFailover=routingFailover, trafficAutoMitigation=trafficAutoMitigation, spScriptPort=spScriptPort, managedObjectUsage=managedObjectUsage, reportDone=reportDone, spImpactPps=spImpactPps, spSystemErrorDescription=spSystemErrorDescription, spHardwareFailureDescription=spHardwareFailureDescription, spTMSPrefix=spTMSPrefix, spLicenseErrLimit=spLicenseErrLimit, fingerprintUsage=fingerprintUsage, spThreshold=spThreshold, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, interfaceUsageDone=interfaceUsageDone, deviceHealth=deviceHealth, deviceSystemErrorDone=deviceSystemErrorDone, spReportURL=spReportURL, configChange=configChange, spDetectedCountries=spDetectedCountries, spUsername=spUsername, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpInstabilityDone=bgpInstabilityDone, spBlackholeTimeout=spBlackholeTimeout, managedObjectUsageDone=managedObjectUsageDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, snmpUp=snmpUp, spCollector=spCollector, spDNSName=spDNSName, spManagedObjectFamily=spManagedObjectFamily, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, snmpDown=snmpDown, spImpactBps=spImpactBps, dosHostDetectionAlert=dosHostDetectionAlert, spBlackholeCommunity=spBlackholeCommunity, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spCommFailureDestination=spCommFailureDestination, spInterfaceSpeedHC=spInterfaceSpeedHC, spThirdPartyZone=spThirdPartyZone, cloudSignalingFaultDone=cloudSignalingFaultDone, fingerprintFeedback=fingerprintFeedback, spRouter=spRouter, hardwareFailure=hardwareFailure, deviceDiskUsage=deviceDiskUsage, spThirdPartyAddr=spThirdPartyAddr, spGreTunnelDestination=spGreTunnelDestination, spSystemErrorType=spSystemErrorType, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spFlowspecCommunity=spFlowspecCommunity, spBlackholePrefix=spBlackholePrefix, spInterfaceIndex=spInterfaceIndex, spBGPEvent=spBGPEvent, spUsageType=spUsageType, spInetAddressType=spInetAddressType, deviceSwapSpace=deviceSwapSpace, serviceUsage=serviceUsage, cloudSignalingFault=cloudSignalingFault, mitigationTMSStartV2=mitigationTMSStartV2, bgpDown=bgpDown, spPravail=spPravail, collectorDown=collectorDown, spFlowspecTimeout=spFlowspecTimeout, spServiceName=spServiceName, interfaceUsage=interfaceUsage, spFingerprintFeedback=spFingerprintFeedback, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spApplicationName=spApplicationName, spUnit=spUnit, mitigationDone=mitigationDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, dnsBaselineDone=dnsBaselineDone, spBGPTrapEvent=spBGPTrapEvent, spLicenseErrDescription=spLicenseErrDescription, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, hardwareFailureDone=hardwareFailureDone, serviceUsageDone=serviceUsageDone, peakflowSPTraps=peakflowSPTraps, peakflowSPCMI=peakflowSPCMI, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spDNSObservedMean=spDNSObservedMean, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, notificationLimit=notificationLimit, spInterfaceSpeed=spInterfaceSpeed, spMitigationID=spMitigationID, spTMSTimeout=spTMSTimeout, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spBGPTrapName=spBGPTrapName, spAlertDetectionSignatures=spAlertDetectionSignatures, collectorStart=collectorStart)
