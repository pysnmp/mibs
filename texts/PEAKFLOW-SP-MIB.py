...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDetectedCountries=spDetectedCountries, interfaceUsageDone=interfaceUsageDone, greDownDone=greDownDone, deviceHealth=deviceHealth, spReportURL=spReportURL, spBlackholePrefix=spBlackholePrefix, spTMSPrefix=spTMSPrefix, spImpactPps=spImpactPps, dnsBaseline=dnsBaseline, spRouter=spRouter, spCommFailureDescription=spCommFailureDescription, spGreTunnelName=spGreTunnelName, peakflowSPCMI=peakflowSPCMI, deviceTotalFlows=deviceTotalFlows, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInetAddress=spInetAddress, spReportName=spReportName, alertScript=alertScript, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, dnsBaselineDone=dnsBaselineDone, mitigationDone=mitigationDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spUsername=spUsername, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spInterfaceUsage=spInterfaceUsage, spDNSObservedMean=spDNSObservedMean, spSystemErrorDescription=spSystemErrorDescription, spLicenseErrCount=spLicenseErrCount, spBGPTrapNewAttributes=spBGPTrapNewAttributes, bgpTrap=bgpTrap, spPravail=spPravail, spScriptStart=spScriptStart, interfaceUsage=interfaceUsage, spDNSName=spDNSName, hardwareFailureDone=hardwareFailureDone, spAlertID=spAlertID, deviceSwapSpace=deviceSwapSpace, spLicenseErrType=spLicenseErrType, serviceUsage=serviceUsage, spUsage=spUsage, bgpUp=bgpUp, peakflowSPMgr=peakflowSPMgr, deviceSystemErrorDone=deviceSystemErrorDone, spImpactBps=spImpactBps, spHijackRoute=spHijackRoute, spThirdPartyZone=spThirdPartyZone, hardwareFailure=hardwareFailure, peakflowSPMIB=peakflowSPMIB, collectorUp=collectorUp, spMitigationID=spMitigationID, spBlackholeCommunity=spBlackholeCommunity, trafficAutoMitigation=trafficAutoMitigation, spTMSTimeout=spTMSTimeout, snmpDown=snmpDown, cloudSignalingFault=cloudSignalingFault, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spInterfaceIndex=spInterfaceIndex, spScriptHost=spScriptHost, deviceSystemError=deviceSystemError, autoclassifyStart=autoclassifyStart, spTMSCommunity=spTMSCommunity, spCommFailureDestination=spCommFailureDestination, deviceDiskUsage=deviceDiskUsage, spBlackholeTimeout=spBlackholeTimeout, spHijackAttr=spHijackAttr, spFingerprintSender=spFingerprintSender, spScriptPort=spScriptPort, spInterface=spInterface, spScriptCommand=spScriptCommand, licenseErrorDone=licenseErrorDone, spFlowspecTimeout=spFlowspecTimeout, spLicenseErrDescription=spLicenseErrDescription, flowUp=flowUp, spDNSObserved=spDNSObserved, deviceTotalFlowsHC=deviceTotalFlowsHC, fingerprintReceive=fingerprintReceive, reportDone=reportDone, spThreshold=spThreshold, spcommFailureDone=spcommFailureDone, bgpDown=bgpDown, spBGPTrapName=spBGPTrapName, spManagedObjectFamily=spManagedObjectFamily, bgpInstability=bgpInstability, spHijackLocal=spHijackLocal, spReportID=spReportID, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spServiceElement=spServiceElement, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spThirdPartyAddr=spThirdPartyAddr, spBGPTrapEvent=spBGPTrapEvent, greDown=greDown, spApplicationName=spApplicationName, spInterfaceSpeed=spInterfaceSpeed, spBGPTrapPrefix=spBGPTrapPrefix, spTMSMultiPrefix=spTMSMultiPrefix, configChange=configChange, spMitigationStart=spMitigationStart, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDNSObservedMax=spDNSObservedMax, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spInterfaceSpeedHC=spInterfaceSpeedHC, spUnit=spUnit, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spInterfaceUsageHC=spInterfaceUsageHC, peakflowSPTraps=peakflowSPTraps, bgpHijack=bgpHijack, spLicenseErrLimit=spLicenseErrLimit, dosHostDetectionAlertDone=dosHostDetectionAlertDone, mitigationTMSStartV2=mitigationTMSStartV2, spFingerprintFeedback=spFingerprintFeedback, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spSystemErrorType=spSystemErrorType, spVersion=spVersion, spDuration=spDuration, spHardwareFailureDescription=spHardwareFailureDescription, spAlertDetectionSignatures=spAlertDetectionSignatures, mitigationThirdPartyStart=mitigationThirdPartyStart, spMitigationName=spMitigationName, devicePhysicalMemory=devicePhysicalMemory, spcommFailure=spcommFailure, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spGreTunnelDestination=spGreTunnelDestination, spManagedObjects=spManagedObjects, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFlowspecCommunity=spFlowspecCommunity, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spFingerprintMessage=spFingerprintMessage, spServiceName=spServiceName, fingerprintUsage=fingerprintUsage, flowDown=flowDown, spDetector=spDetector, managedObjectUsage=managedObjectUsage, dosHostDetectionAlert=dosHostDetectionAlert, bgpInstabilityDone=bgpInstabilityDone, serviceUsageDone=serviceUsageDone, mitigationTMSStart=mitigationTMSStart, mitigationFlowspecStart=mitigationFlowspecStart, spBlackholeNexthop=spBlackholeNexthop, managedObjectUsageDone=managedObjectUsageDone, collectorDown=collectorDown, spCollector=spCollector, spFingerprintName=spFingerprintName, fingerprintUsageDone=fingerprintUsageDone, notificationLimit=notificationLimit, licenseError=licenseError, spUsageType=spUsageType, spBGPInstability=spBGPInstability, snmpUp=snmpUp, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, routingFailover=routingFailover, spBGPEvent=spBGPEvent, spDNSExpected=spDNSExpected, fingerprintFeedback=fingerprintFeedback, mitigationBlackholeStart=mitigationBlackholeStart, spManagedObject=spManagedObject, PYSNMP_MODULE_ID=peakflowSPMIB, spInetAddressType=spInetAddressType, collectorStart=collectorStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, cloudSignalingFaultDone=cloudSignalingFaultDone)
