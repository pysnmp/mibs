...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, licenseErrorDone=licenseErrorDone, peakflowSPMIB=peakflowSPMIB, spHijackLocal=spHijackLocal, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, peakflowSPMgr=peakflowSPMgr, collectorUp=collectorUp, licenseError=licenseError, spManagedObjects=spManagedObjects, hardwareFailure=hardwareFailure, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spBlackholePrefix=spBlackholePrefix, spCommFailureDescription=spCommFailureDescription, spFlowspecTimeout=spFlowspecTimeout, spAlertDetectionSignatures=spAlertDetectionSignatures, spcommFailure=spcommFailure, managedObjectUsageDone=managedObjectUsageDone, spDNSName=spDNSName, snmpUp=snmpUp, spBGPTrapNewAttributes=spBGPTrapNewAttributes, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceSystemError=deviceSystemError, spRouter=spRouter, spAlertID=spAlertID, alertScript=alertScript, spScriptHost=spScriptHost, spTMSMultiPrefix=spTMSMultiPrefix, spManagedObjectFamily=spManagedObjectFamily, spInterface=spInterface, spBlackholeTimeout=spBlackholeTimeout, mitigationFlowspecStart=mitigationFlowspecStart, spFlowspecCommunity=spFlowspecCommunity, dosHostDetectionAlert=dosHostDetectionAlert, collectorStart=collectorStart, spPravail=spPravail, managedObjectUsage=managedObjectUsage, fingerprintUsageDone=fingerprintUsageDone, spImpactPps=spImpactPps, routingFailover=routingFailover, cloudSignalingFaultDone=cloudSignalingFaultDone, dnsBaselineDone=dnsBaselineDone, spHardwareFailureDescription=spHardwareFailureDescription, bgpInstabilityDone=bgpInstabilityDone, trafficAutoMitigation=trafficAutoMitigation, spApplicationName=spApplicationName, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, dnsBaseline=dnsBaseline, spMitigationStart=spMitigationStart, snmpDown=snmpDown, interfaceUsage=interfaceUsage, bgpDown=bgpDown, spManagedObject=spManagedObject, spBlackholeNexthop=spBlackholeNexthop, bgpUp=bgpUp, devicePhysicalMemory=devicePhysicalMemory, PYSNMP_MODULE_ID=peakflowSPMIB, spHijackAttr=spHijackAttr, spInetAddress=spInetAddress, spLicenseErrLimit=spLicenseErrLimit, spGreTunnelDestination=spGreTunnelDestination, spDNSObservedMean=spDNSObservedMean, cloudSignalingFault=cloudSignalingFault, spReportID=spReportID, spLicenseErrType=spLicenseErrType, greDownDone=greDownDone, spBGPInstability=spBGPInstability, spDuration=spDuration, spMitigationID=spMitigationID, spThirdPartyZone=spThirdPartyZone, spGreTunnelName=spGreTunnelName, peakflowSPTraps=peakflowSPTraps, mitigationDone=mitigationDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spCommFailureDestination=spCommFailureDestination, spInterfaceUsage=spInterfaceUsage, fingerprintReceive=fingerprintReceive, spScriptCommand=spScriptCommand, spCollector=spCollector, spUsage=spUsage, spBGPTrapEvent=spBGPTrapEvent, spReportName=spReportName, spServiceElement=spServiceElement, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapOldAttributes=spBGPTrapOldAttributes, mitigationTMSStartV2=mitigationTMSStartV2, spBGPTrapName=spBGPTrapName, configChange=configChange, deviceDiskUsage=deviceDiskUsage, deviceTotalFlows=deviceTotalFlows, spBGPTrapPrefix=spBGPTrapPrefix, spMitigationName=spMitigationName, spUnit=spUnit, spTMSCommunity=spTMSCommunity, spSystemErrorDescription=spSystemErrorDescription, spBlackholeCommunity=spBlackholeCommunity, spcommFailureDone=spcommFailureDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBGPEvent=spBGPEvent, mitigationBlackholeStart=mitigationBlackholeStart, spThirdPartyAddr=spThirdPartyAddr, spInterfaceSpeedHC=spInterfaceSpeedHC, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, fingerprintUsage=fingerprintUsage, spDetector=spDetector, serviceUsage=serviceUsage, spInterfaceUsageHC=spInterfaceUsageHC, spThreshold=spThreshold, notificationLimit=notificationLimit, spFingerprintSender=spFingerprintSender, spDNSObservedMax=spDNSObservedMax, greDown=greDown, spDNSObserved=spDNSObserved, spUsageType=spUsageType, spTMSTimeout=spTMSTimeout, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, autoclassifyStart=autoclassifyStart, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spInterfaceIndex=spInterfaceIndex, deviceSwapSpace=deviceSwapSpace, spSystemErrorType=spSystemErrorType, flowDown=flowDown, bgpInstability=bgpInstability, flowUp=flowUp, mitigationTMSStart=mitigationTMSStart, serviceUsageDone=serviceUsageDone, spReportURL=spReportURL, spLicenseErrCount=spLicenseErrCount, fingerprintFeedback=fingerprintFeedback, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInetAddressType=spInetAddressType, bgpHijack=bgpHijack, deviceTotalFlowsHC=deviceTotalFlowsHC, spScriptPort=spScriptPort, spVersion=spVersion, spFingerprintName=spFingerprintName, spImpactBps=spImpactBps, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spUsername=spUsername, deviceSystemErrorDone=deviceSystemErrorDone, spScriptStart=spScriptStart, spHijackRoute=spHijackRoute, spInterfaceSpeed=spInterfaceSpeed, spFingerprintMessage=spFingerprintMessage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, collectorDown=collectorDown, interfaceUsageDone=interfaceUsageDone, spDNSExpected=spDNSExpected, reportDone=reportDone, spServiceName=spServiceName, peakflowSPCMI=peakflowSPCMI, spTMSPrefix=spTMSPrefix, spDetectedCountries=spDetectedCountries, deviceHealth=deviceHealth, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, bgpTrap=bgpTrap, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFingerprintFeedback=spFingerprintFeedback, hardwareFailureDone=hardwareFailureDone)
