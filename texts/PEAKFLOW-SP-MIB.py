...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spImpactBps=spImpactBps, notificationLimit=notificationLimit, spInterfaceSpeed=spInterfaceSpeed, spLicenseErrDescription=spLicenseErrDescription, greDown=greDown, spBlackholeNexthop=spBlackholeNexthop, spSystemErrorDescription=spSystemErrorDescription, spCommFailureDescription=spCommFailureDescription, dnsBaseline=dnsBaseline, collectorStart=collectorStart, spScriptCommand=spScriptCommand, spBGPTrapEvent=spBGPTrapEvent, spCommFailureDestination=spCommFailureDestination, spInetAddressType=spInetAddressType, fingerprintUsageDone=fingerprintUsageDone, deviceDiskUsage=deviceDiskUsage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceSystemErrorDone=deviceSystemErrorDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spRouter=spRouter, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spUsage=spUsage, spLicenseErrType=spLicenseErrType, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, peakflowSPCMI=peakflowSPCMI, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spUsageType=spUsageType, spTMSMultiPrefix=spTMSMultiPrefix, snmpUp=snmpUp, spInterfaceSpeedHC=spInterfaceSpeedHC, spThreshold=spThreshold, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spTMSTimeout=spTMSTimeout, spManagedObject=spManagedObject, spScriptHost=spScriptHost, spApplicationName=spApplicationName, spPravail=spPravail, autoclassifyStart=autoclassifyStart, managedObjectUsage=managedObjectUsage, alertScript=alertScript, spInetAddress=spInetAddress, interfaceUsageDone=interfaceUsageDone, collectorUp=collectorUp, spReportID=spReportID, licenseErrorDone=licenseErrorDone, dosHostDetectionAlert=dosHostDetectionAlert, managedObjectUsageDone=managedObjectUsageDone, interfaceUsage=interfaceUsage, hardwareFailureDone=hardwareFailureDone, mitigationTMSStartV2=mitigationTMSStartV2, spReportName=spReportName, peakflowSPTraps=peakflowSPTraps, spSystemErrorType=spSystemErrorType, spInterface=spInterface, hardwareFailure=hardwareFailure, spInterfaceUsageHC=spInterfaceUsageHC, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spAlertDetectionSignatures=spAlertDetectionSignatures, spImpactPps=spImpactPps, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, fingerprintUsage=fingerprintUsage, spHijackRoute=spHijackRoute, spLicenseErrCount=spLicenseErrCount, spBGPTrapName=spBGPTrapName, spHijackLocal=spHijackLocal, spMitigationStart=spMitigationStart, configChange=configChange, spScriptPort=spScriptPort, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, greDownDone=greDownDone, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, mitigationFlowspecStart=mitigationFlowspecStart, spServiceName=spServiceName, licenseError=licenseError, mitigationTMSStart=mitigationTMSStart, bgpUp=bgpUp, spBGPEvent=spBGPEvent, dosHostDetectionAlertDone=dosHostDetectionAlertDone, serviceUsageDone=serviceUsageDone, cloudSignalingFault=cloudSignalingFault, bgpTrap=bgpTrap, routingFailover=routingFailover, spManagedObjects=spManagedObjects, mitigationBlackholeStart=mitigationBlackholeStart, spFingerprintName=spFingerprintName, spBlackholeCommunity=spBlackholeCommunity, spFingerprintMessage=spFingerprintMessage, spDNSObservedMean=spDNSObservedMean, spDuration=spDuration, spDNSName=spDNSName, spThirdPartyAddr=spThirdPartyAddr, fingerprintReceive=fingerprintReceive, spDetectedCountries=spDetectedCountries, spDNSExpected=spDNSExpected, spcommFailure=spcommFailure, spInterfaceIndex=spInterfaceIndex, spDNSObserved=spDNSObserved, bgpInstabilityDone=bgpInstabilityDone, peakflowSPMgr=peakflowSPMgr, serviceUsage=serviceUsage, spBGPInstability=spBGPInstability, spDNSObservedMax=spDNSObservedMax, dnsBaselineDone=dnsBaselineDone, mitigationDone=mitigationDone, snmpDown=snmpDown, PYSNMP_MODULE_ID=peakflowSPMIB, deviceSwapSpace=deviceSwapSpace, spTMSCommunity=spTMSCommunity, spGreTunnelName=spGreTunnelName, spBGPTrapPrefix=spBGPTrapPrefix, spDetector=spDetector, flowDown=flowDown, spCollector=spCollector, fingerprintFeedback=fingerprintFeedback, spReportURL=spReportURL, spAlertID=spAlertID, spInterfaceUsage=spInterfaceUsage, devicePhysicalMemory=devicePhysicalMemory, bgpDown=bgpDown, flowUp=flowUp, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spMitigationID=spMitigationID, mitigationThirdPartyStart=mitigationThirdPartyStart, bgpInstability=bgpInstability, deviceTotalFlowsHC=deviceTotalFlowsHC, spBlackholeTimeout=spBlackholeTimeout, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spFlowspecTimeout=spFlowspecTimeout, trafficAutoMitigation=trafficAutoMitigation, spUnit=spUnit, spGreTunnelDestination=spGreTunnelDestination, spScriptStart=spScriptStart, spServiceElement=spServiceElement, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spMitigationName=spMitigationName, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spTMSPrefix=spTMSPrefix, spFlowspecCommunity=spFlowspecCommunity, spHijackAttr=spHijackAttr, reportDone=reportDone, deviceSystemError=deviceSystemError, spUsername=spUsername, peakflowSPMIB=peakflowSPMIB, spcommFailureDone=spcommFailureDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceTotalFlows=deviceTotalFlows, spThirdPartyZone=spThirdPartyZone, collectorDown=collectorDown, spManagedObjectFamily=spManagedObjectFamily, spFingerprintFeedback=spFingerprintFeedback, spLicenseErrLimit=spLicenseErrLimit, deviceHealth=deviceHealth, spHardwareFailureDescription=spHardwareFailureDescription, bgpHijack=bgpHijack, spFingerprintSender=spFingerprintSender, spVersion=spVersion, spBlackholePrefix=spBlackholePrefix)
