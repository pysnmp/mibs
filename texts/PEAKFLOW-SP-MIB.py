...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spMitigationName=spMitigationName, spScriptHost=spScriptHost, spScriptStart=spScriptStart, spInetAddress=spInetAddress, spReportURL=spReportURL, deviceDiskUsage=deviceDiskUsage, spInterfaceIndex=spInterfaceIndex, spBGPTrapName=spBGPTrapName, trafficAutoMitigation=trafficAutoMitigation, spThreshold=spThreshold, spFingerprintSender=spFingerprintSender, spInetAddressType=spInetAddressType, spFlowspecTimeout=spFlowspecTimeout, interfaceUsage=interfaceUsage, hardwareFailureDone=hardwareFailureDone, spLicenseErrType=spLicenseErrType, spFingerprintFeedback=spFingerprintFeedback, spDNSExpected=spDNSExpected, mitigationDone=mitigationDone, spThirdPartyAddr=spThirdPartyAddr, spLicenseErrCount=spLicenseErrCount, mitigationThirdPartyStart=mitigationThirdPartyStart, cloudSignalingFault=cloudSignalingFault, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, autoclassifyStart=autoclassifyStart, spDNSName=spDNSName, spBlackholeNexthop=spBlackholeNexthop, spAlertID=spAlertID, fingerprintFeedback=fingerprintFeedback, spBlackholePrefix=spBlackholePrefix, flowDown=flowDown, dnsBaselineDone=dnsBaselineDone, spInterfaceUsage=spInterfaceUsage, deviceTotalFlowsHC=deviceTotalFlowsHC, mitigationTMSStart=mitigationTMSStart, spImpactPps=spImpactPps, peakflowSPCMI=peakflowSPCMI, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, greDownDone=greDownDone, spDuration=spDuration, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spTMSMultiPrefix=spTMSMultiPrefix, spInterface=spInterface, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceSystemErrorDone=deviceSystemErrorDone, spAlertDetectionSignatures=spAlertDetectionSignatures, greDown=greDown, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceTotalFlows=deviceTotalFlows, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spcommFailure=spcommFailure, licenseError=licenseError, spSystemErrorType=spSystemErrorType, peakflowSPTraps=peakflowSPTraps, PYSNMP_MODULE_ID=peakflowSPMIB, spHijackRoute=spHijackRoute, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spBlackholeTimeout=spBlackholeTimeout, spRouter=spRouter, spCollector=spCollector, collectorUp=collectorUp, collectorStart=collectorStart, deviceSwapSpace=deviceSwapSpace, deviceSwapSpaceUsage=deviceSwapSpaceUsage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spGreTunnelName=spGreTunnelName, spFingerprintMessage=spFingerprintMessage, spUsageType=spUsageType, spBGPInstability=spBGPInstability, spUsername=spUsername, spBGPTrapEvent=spBGPTrapEvent, spInterfaceSpeedHC=spInterfaceSpeedHC, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spServiceName=spServiceName, licenseErrorDone=licenseErrorDone, fingerprintUsageDone=fingerprintUsageDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spDNSObservedMean=spDNSObservedMean, spFingerprintName=spFingerprintName, deviceHealth=deviceHealth, interfaceUsageDone=interfaceUsageDone, spScriptCommand=spScriptCommand, serviceUsage=serviceUsage, spCommFailureDescription=spCommFailureDescription, cloudSignalingFaultDone=cloudSignalingFaultDone, mitigationBlackholeStart=mitigationBlackholeStart, bgpInstability=bgpInstability, routingFailover=routingFailover, bgpUp=bgpUp, spVersion=spVersion, configChange=configChange, spDNSObserved=spDNSObserved, spDNSObservedMax=spDNSObservedMax, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spServiceElement=spServiceElement, spLicenseErrLimit=spLicenseErrLimit, hardwareFailure=hardwareFailure, dnsBaseline=dnsBaseline, bgpDown=bgpDown, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spTMSPrefix=spTMSPrefix, spManagedObjects=spManagedObjects, deviceSystemError=deviceSystemError, bgpTrap=bgpTrap, spUnit=spUnit, spInterfaceSpeed=spInterfaceSpeed, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, snmpUp=snmpUp, spMitigationStart=spMitigationStart, spHardwareFailureDescription=spHardwareFailureDescription, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spScriptPort=spScriptPort, spPravail=spPravail, spMitigationID=spMitigationID, spDetectedCountries=spDetectedCountries, spInterfaceUsageHC=spInterfaceUsageHC, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, managedObjectUsage=managedObjectUsage, spUsage=spUsage, dosHostDetectionAlert=dosHostDetectionAlert, spBGPEvent=spBGPEvent, spCommFailureDestination=spCommFailureDestination, spLicenseErrDescription=spLicenseErrDescription, spReportID=spReportID, mitigationFlowspecStart=mitigationFlowspecStart, spcommFailureDone=spcommFailureDone, serviceUsageDone=serviceUsageDone, reportDone=reportDone, spFlowspecCommunity=spFlowspecCommunity, alertScript=alertScript, spTMSCommunity=spTMSCommunity, spBlackholeCommunity=spBlackholeCommunity, spApplicationName=spApplicationName, notificationLimit=notificationLimit, managedObjectUsageDone=managedObjectUsageDone, peakflowSPMIB=peakflowSPMIB, spHijackAttr=spHijackAttr, peakflowSPMgr=peakflowSPMgr, spDetector=spDetector, fingerprintReceive=fingerprintReceive, spManagedObject=spManagedObject, spGreTunnelDestination=spGreTunnelDestination, collectorDown=collectorDown, spBGPTrapPrefix=spBGPTrapPrefix, devicePhysicalMemory=devicePhysicalMemory, snmpDown=snmpDown, spManagedObjectFamily=spManagedObjectFamily, spTMSTimeout=spTMSTimeout, bgpHijack=bgpHijack, bgpInstabilityDone=bgpInstabilityDone, flowUp=flowUp, spThirdPartyZone=spThirdPartyZone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spImpactBps=spImpactBps, spHijackLocal=spHijackLocal, spSystemErrorDescription=spSystemErrorDescription, mitigationTMSStartV2=mitigationTMSStartV2, spReportName=spReportName, fingerprintUsage=fingerprintUsage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate)
