...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceSpeedHC=spInterfaceSpeedHC, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spPravail=spPravail, hardwareFailure=hardwareFailure, spBlackholeNexthop=spBlackholeNexthop, spInetAddress=spInetAddress, spRouter=spRouter, bgpInstability=bgpInstability, spMitigationName=spMitigationName, peakflowSPCMI=peakflowSPCMI, interfaceUsageDone=interfaceUsageDone, spReportID=spReportID, spBGPTrapPrefix=spBGPTrapPrefix, collectorStart=collectorStart, bgpUp=bgpUp, spInetAddressType=spInetAddressType, spScriptPort=spScriptPort, spDNSObservedMean=spDNSObservedMean, deviceSystemError=deviceSystemError, spUsername=spUsername, spTMSCommunity=spTMSCommunity, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spScriptStart=spScriptStart, dosHostDetectionAlertDone=dosHostDetectionAlertDone, licenseError=licenseError, devicePhysicalMemory=devicePhysicalMemory, spInterfaceSpeed=spInterfaceSpeed, spFingerprintFeedback=spFingerprintFeedback, greDownDone=greDownDone, spFingerprintSender=spFingerprintSender, routingFailover=routingFailover, spServiceElement=spServiceElement, collectorDown=collectorDown, spTMSTimeout=spTMSTimeout, spSystemErrorDescription=spSystemErrorDescription, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, bgpInstabilityDone=bgpInstabilityDone, dnsBaselineDone=dnsBaselineDone, peakflowSPTraps=peakflowSPTraps, spMitigationStart=spMitigationStart, spScriptHost=spScriptHost, greDown=greDown, spGreTunnelDestination=spGreTunnelDestination, dosHostDetectionAlert=dosHostDetectionAlert, deviceSwapSpaceInUse=deviceSwapSpaceInUse, mitigationDone=mitigationDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spCollector=spCollector, spDetector=spDetector, flowDown=flowDown, serviceUsage=serviceUsage, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spCommFailureDestination=spCommFailureDestination, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spImpactPps=spImpactPps, spScriptCommand=spScriptCommand, spBlackholeTimeout=spBlackholeTimeout, spGreTunnelName=spGreTunnelName, spAlertID=spAlertID, managedObjectUsage=managedObjectUsage, mitigationFlowspecStart=mitigationFlowspecStart, trafficAutoMitigation=trafficAutoMitigation, spReportName=spReportName, spDNSName=spDNSName, configChange=configChange, spBGPTrapOldAttributes=spBGPTrapOldAttributes, dnsBaseline=dnsBaseline, spTMSMultiPrefix=spTMSMultiPrefix, autoclassifyStart=autoclassifyStart, spThirdPartyZone=spThirdPartyZone, snmpDown=snmpDown, spLicenseErrDescription=spLicenseErrDescription, spSystemErrorType=spSystemErrorType, mitigationThirdPartyStart=mitigationThirdPartyStart, spAlertDetectionSignatures=spAlertDetectionSignatures, hardwareFailureDone=hardwareFailureDone, deviceDiskUsage=deviceDiskUsage, deviceSystemErrorDone=deviceSystemErrorDone, spServiceName=spServiceName, spUsageType=spUsageType, spManagedObject=spManagedObject, spDuration=spDuration, bgpHijack=bgpHijack, PYSNMP_MODULE_ID=peakflowSPMIB, deviceTotalFlows=deviceTotalFlows, reportDone=reportDone, spBGPEvent=spBGPEvent, spVersion=spVersion, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpDown=bgpDown, spBGPTrapNewAttributes=spBGPTrapNewAttributes, fingerprintUsageDone=fingerprintUsageDone, spHardwareFailureDescription=spHardwareFailureDescription, spThreshold=spThreshold, spThirdPartyAddr=spThirdPartyAddr, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spLicenseErrCount=spLicenseErrCount, spUsage=spUsage, spImpactBps=spImpactBps, serviceUsageDone=serviceUsageDone, peakflowSPMIB=peakflowSPMIB, spDNSObservedMax=spDNSObservedMax, spInterfaceUsage=spInterfaceUsage, collectorUp=collectorUp, peakflowSPMgr=peakflowSPMgr, spHijackAttr=spHijackAttr, spManagedObjects=spManagedObjects, managedObjectUsageDone=managedObjectUsageDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, fingerprintFeedback=fingerprintFeedback, licenseErrorDone=licenseErrorDone, spFlowspecCommunity=spFlowspecCommunity, spDNSExpected=spDNSExpected, notificationLimit=notificationLimit, spManagedObjectFamily=spManagedObjectFamily, bgpTrap=bgpTrap, interfaceUsage=interfaceUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBGPTrapName=spBGPTrapName, spInterface=spInterface, cloudSignalingFaultDone=cloudSignalingFaultDone, fingerprintReceive=fingerprintReceive, spcommFailure=spcommFailure, spApplicationName=spApplicationName, deviceSwapSpace=deviceSwapSpace, snmpUp=snmpUp, spCloudSignalFaultDescription=spCloudSignalFaultDescription, alertScript=alertScript, deviceSwapSpaceUsage=deviceSwapSpaceUsage, mitigationBlackholeStart=mitigationBlackholeStart, mitigationTMSStartV2=mitigationTMSStartV2, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBGPInstability=spBGPInstability, spTMSPrefix=spTMSPrefix, spMitigationID=spMitigationID, spCommFailureDescription=spCommFailureDescription, spFingerprintName=spFingerprintName, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spHijackRoute=spHijackRoute, spDNSObserved=spDNSObserved, spReportURL=spReportURL, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, flowUp=flowUp, deviceHealth=deviceHealth, spHijackLocal=spHijackLocal, mitigationTMSStart=mitigationTMSStart, spFlowspecTimeout=spFlowspecTimeout, cloudSignalingFault=cloudSignalingFault, spInterfaceIndex=spInterfaceIndex, spFingerprintMessage=spFingerprintMessage, spDetectedCountries=spDetectedCountries, spBGPTrapEvent=spBGPTrapEvent, fingerprintUsage=fingerprintUsage, spUnit=spUnit, spBlackholeCommunity=spBlackholeCommunity, spcommFailureDone=spcommFailureDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spLicenseErrLimit=spLicenseErrLimit, spLicenseErrType=spLicenseErrType, spBlackholePrefix=spBlackholePrefix, spInterfaceUsageHC=spInterfaceUsageHC)
