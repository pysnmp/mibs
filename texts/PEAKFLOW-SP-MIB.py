...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spScriptPort=spScriptPort, spHijackAttr=spHijackAttr, autoclassifyStart=autoclassifyStart, spLicenseErrDescription=spLicenseErrDescription, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, mitigationTMSStartV2=mitigationTMSStartV2, reportDone=reportDone, spBGPTrapPrefix=spBGPTrapPrefix, spMitigationID=spMitigationID, dnsBaseline=dnsBaseline, mitigationTMSStart=mitigationTMSStart, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, interfaceUsage=interfaceUsage, snmpDown=snmpDown, spSystemErrorDescription=spSystemErrorDescription, spTMSMultiPrefix=spTMSMultiPrefix, spManagedObjects=spManagedObjects, deviceTotalFlows=deviceTotalFlows, spUsage=spUsage, deviceSystemError=deviceSystemError, spcommFailure=spcommFailure, spHardwareFailureDescription=spHardwareFailureDescription, spBlackholeCommunity=spBlackholeCommunity, spLicenseErrLimit=spLicenseErrLimit, devicePhysicalMemory=devicePhysicalMemory, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spBlackholeNexthop=spBlackholeNexthop, mitigationDone=mitigationDone, spTMSCommunity=spTMSCommunity, cloudSignalingFault=cloudSignalingFault, collectorUp=collectorUp, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterfaceSpeedHC=spInterfaceSpeedHC, flowUp=flowUp, spManagedObjectFamily=spManagedObjectFamily, fingerprintUsageDone=fingerprintUsageDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, mitigationBlackholeStart=mitigationBlackholeStart, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, PYSNMP_MODULE_ID=peakflowSPMIB, spScriptCommand=spScriptCommand, spImpactBps=spImpactBps, spDetectedCountries=spDetectedCountries, spDNSExpected=spDNSExpected, spUnit=spUnit, collectorDown=collectorDown, spGreTunnelDestination=spGreTunnelDestination, spBGPTrapNewAttributes=spBGPTrapNewAttributes, dosHostDetectionAlert=dosHostDetectionAlert, bgpInstabilityDone=bgpInstabilityDone, mitigationThirdPartyStart=mitigationThirdPartyStart, spReportName=spReportName, spBGPTrapName=spBGPTrapName, bgpDown=bgpDown, collectorStart=collectorStart, spInetAddressType=spInetAddressType, deviceTotalFlowsHC=deviceTotalFlowsHC, spDetector=spDetector, spBGPEvent=spBGPEvent, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spBGPInstability=spBGPInstability, spThirdPartyZone=spThirdPartyZone, spFingerprintSender=spFingerprintSender, peakflowSPTraps=peakflowSPTraps, spTMSPrefix=spTMSPrefix, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spLicenseErrType=spLicenseErrType, peakflowSPMgr=peakflowSPMgr, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spScriptHost=spScriptHost, deviceSwapSpace=deviceSwapSpace, hardwareFailure=hardwareFailure, fingerprintFeedback=fingerprintFeedback, interfaceUsageDone=interfaceUsageDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spThirdPartyAddr=spThirdPartyAddr, spPravail=spPravail, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDNSName=spDNSName, spScriptStart=spScriptStart, licenseErrorDone=licenseErrorDone, snmpUp=snmpUp, spDNSObservedMax=spDNSObservedMax, spRouter=spRouter, serviceUsageDone=serviceUsageDone, dnsBaselineDone=dnsBaselineDone, spInterface=spInterface, peakflowSPCMI=peakflowSPCMI, configChange=configChange, spMitigationName=spMitigationName, hardwareFailureDone=hardwareFailureDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spMitigationStart=spMitigationStart, flowDown=flowDown, spHijackLocal=spHijackLocal, spInterfaceIndex=spInterfaceIndex, spApplicationName=spApplicationName, spThreshold=spThreshold, spServiceName=spServiceName, bgpUp=bgpUp, spVersion=spVersion, spSystemErrorType=spSystemErrorType, spFingerprintMessage=spFingerprintMessage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, fingerprintUsage=fingerprintUsage, spFlowspecTimeout=spFlowspecTimeout, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, peakflowSPMIB=peakflowSPMIB, spInterfaceUsage=spInterfaceUsage, spDNSObservedMean=spDNSObservedMean, deviceSwapSpaceInUse=deviceSwapSpaceInUse, managedObjectUsage=managedObjectUsage, spFingerprintName=spFingerprintName, spAlertDetectionSignatures=spAlertDetectionSignatures, spBlackholeTimeout=spBlackholeTimeout, serviceUsage=serviceUsage, spcommFailureDone=spcommFailureDone, spTMSTimeout=spTMSTimeout, spManagedObject=spManagedObject, licenseError=licenseError, spReportURL=spReportURL, spBGPTrapEvent=spBGPTrapEvent, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spReportID=spReportID, bgpInstability=bgpInstability, deviceSystemErrorDone=deviceSystemErrorDone, spCommFailureDestination=spCommFailureDestination, spInterfaceSpeed=spInterfaceSpeed, bgpTrap=bgpTrap, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spUsageType=spUsageType, spFingerprintFeedback=spFingerprintFeedback, spCommFailureDescription=spCommFailureDescription, notificationLimit=notificationLimit, spServiceElement=spServiceElement, managedObjectUsageDone=managedObjectUsageDone, greDown=greDown, routingFailover=routingFailover, spInetAddress=spInetAddress, spDNSObserved=spDNSObserved, bgpHijack=bgpHijack, spImpactPps=spImpactPps, trafficAutoMitigation=trafficAutoMitigation, alertScript=alertScript, spHijackRoute=spHijackRoute, spDuration=spDuration, spAlertID=spAlertID, deviceDiskUsage=deviceDiskUsage, fingerprintReceive=fingerprintReceive, spFlowspecCommunity=spFlowspecCommunity, spInterfaceUsageHC=spInterfaceUsageHC, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spCollector=spCollector, spBlackholePrefix=spBlackholePrefix, spGreTunnelName=spGreTunnelName, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, mitigationFlowspecStart=mitigationFlowspecStart, spLicenseErrCount=spLicenseErrCount, deviceHealth=deviceHealth, greDownDone=greDownDone, spUsername=spUsername)
