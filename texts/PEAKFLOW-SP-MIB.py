...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spCollector=spCollector, spInterfaceIndex=spInterfaceIndex, serviceUsageDone=serviceUsageDone, notificationLimit=notificationLimit, mitigationDone=mitigationDone, mitigationTMSStartV2=mitigationTMSStartV2, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spVersion=spVersion, spDNSName=spDNSName, spCommFailureDescription=spCommFailureDescription, cloudSignalingFaultDone=cloudSignalingFaultDone, bgpInstabilityDone=bgpInstabilityDone, deviceDiskUsage=deviceDiskUsage, deviceSystemErrorDone=deviceSystemErrorDone, spServiceName=spServiceName, greDown=greDown, mitigationThirdPartyStart=mitigationThirdPartyStart, peakflowSPMgr=peakflowSPMgr, flowDown=flowDown, spMitigationStart=spMitigationStart, spLicenseErrLimit=spLicenseErrLimit, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spReportID=spReportID, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, routingFailover=routingFailover, spInterfaceSpeed=spInterfaceSpeed, spInterfaceSpeedHC=spInterfaceSpeedHC, spTMSCommunity=spTMSCommunity, flowUp=flowUp, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, cloudSignalingFault=cloudSignalingFault, spReportName=spReportName, spManagedObject=spManagedObject, collectorStart=collectorStart, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spGreTunnelDestination=spGreTunnelDestination, spImpactPps=spImpactPps, spBGPInstability=spBGPInstability, spUsername=spUsername, spFlowspecTimeout=spFlowspecTimeout, spScriptHost=spScriptHost, trafficAutoMitigation=trafficAutoMitigation, spSystemErrorType=spSystemErrorType, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBlackholeCommunity=spBlackholeCommunity, hardwareFailureDone=hardwareFailureDone, spUsage=spUsage, spDetectedCountries=spDetectedCountries, interfaceUsageDone=interfaceUsageDone, spAlertID=spAlertID, spImpactBps=spImpactBps, spTMSMultiPrefix=spTMSMultiPrefix, spUsageType=spUsageType, spLicenseErrDescription=spLicenseErrDescription, spInterfaceUsageHC=spInterfaceUsageHC, hardwareFailure=hardwareFailure, spMitigationID=spMitigationID, spTMSPrefix=spTMSPrefix, spServiceElement=spServiceElement, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spLicenseErrType=spLicenseErrType, spManagedObjects=spManagedObjects, peakflowSPTraps=peakflowSPTraps, spDNSExpected=spDNSExpected, bgpDown=bgpDown, spCommFailureDestination=spCommFailureDestination, spPravail=spPravail, spUnit=spUnit, collectorUp=collectorUp, fingerprintFeedback=fingerprintFeedback, bgpUp=bgpUp, spInterfaceUsage=spInterfaceUsage, managedObjectUsageDone=managedObjectUsageDone, spMitigationName=spMitigationName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, peakflowSPCMI=peakflowSPCMI, spLicenseErrCount=spLicenseErrCount, deviceTotalFlowsHC=deviceTotalFlowsHC, mitigationFlowspecStart=mitigationFlowspecStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spRouter=spRouter, fingerprintReceive=fingerprintReceive, alertScript=alertScript, bgpTrap=bgpTrap, spcommFailureDone=spcommFailureDone, peakflowSPMIB=peakflowSPMIB, serviceUsage=serviceUsage, bgpHijack=bgpHijack, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, devicePhysicalMemory=devicePhysicalMemory, spDetector=spDetector, spInetAddressType=spInetAddressType, dosNetworkProfiledAlert=dosNetworkProfiledAlert, dnsBaseline=dnsBaseline, spBlackholeNexthop=spBlackholeNexthop, spcommFailure=spcommFailure, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInterface=spInterface, dosHostDetectionAlert=dosHostDetectionAlert, deviceHealth=deviceHealth, deviceTotalFlows=deviceTotalFlows, licenseError=licenseError, spAlertDetectionSignatures=spAlertDetectionSignatures, spBlackholePrefix=spBlackholePrefix, spReportURL=spReportURL, spInetAddress=spInetAddress, spSystemErrorDescription=spSystemErrorDescription, spFingerprintName=spFingerprintName, spThirdPartyZone=spThirdPartyZone, spGreTunnelName=spGreTunnelName, spDuration=spDuration, spBGPEvent=spBGPEvent, spTMSTimeout=spTMSTimeout, autoclassifyStart=autoclassifyStart, spBGPTrapEvent=spBGPTrapEvent, deviceSwapSpaceInUse=deviceSwapSpaceInUse, greDownDone=greDownDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, PYSNMP_MODULE_ID=peakflowSPMIB, spHijackAttr=spHijackAttr, spHijackRoute=spHijackRoute, spHijackLocal=spHijackLocal, spScriptCommand=spScriptCommand, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spBGPTrapNewAttributes=spBGPTrapNewAttributes, configChange=configChange, spFingerprintSender=spFingerprintSender, snmpUp=snmpUp, fingerprintUsage=fingerprintUsage, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, snmpDown=snmpDown, managedObjectUsage=managedObjectUsage, spScriptPort=spScriptPort, spBlackholeTimeout=spBlackholeTimeout, spThreshold=spThreshold, bgpInstability=bgpInstability, fingerprintUsageDone=fingerprintUsageDone, deviceSystemError=deviceSystemError, spDNSObservedMean=spDNSObservedMean, spDNSObserved=spDNSObserved, collectorDown=collectorDown, interfaceUsage=interfaceUsage, spApplicationName=spApplicationName, spFingerprintFeedback=spFingerprintFeedback, spScriptStart=spScriptStart, licenseErrorDone=licenseErrorDone, spBGPTrapName=spBGPTrapName, spBGPTrapPrefix=spBGPTrapPrefix, deviceSwapSpace=deviceSwapSpace, spManagedObjectFamily=spManagedObjectFamily, spThirdPartyAddr=spThirdPartyAddr, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, mitigationTMSStart=mitigationTMSStart, dnsBaselineDone=dnsBaselineDone, mitigationBlackholeStart=mitigationBlackholeStart, spFlowspecCommunity=spFlowspecCommunity, spHardwareFailureDescription=spHardwareFailureDescription, spFingerprintMessage=spFingerprintMessage, spDNSObservedMax=spDNSObservedMax, reportDone=reportDone)
