...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBGPTrapEvent=spBGPTrapEvent, spInterfaceIndex=spInterfaceIndex, spMitigationStart=spMitigationStart, spReportName=spReportName, snmpDown=snmpDown, spDetectedCountries=spDetectedCountries, flowUp=flowUp, notificationLimit=notificationLimit, spBlackholeNexthop=spBlackholeNexthop, mitigationFlowspecStart=mitigationFlowspecStart, bgpHijack=bgpHijack, spBGPTrapNewAttributes=spBGPTrapNewAttributes, reportDone=reportDone, spReportID=spReportID, spDetector=spDetector, mitigationThirdPartyStart=mitigationThirdPartyStart, spBlackholePrefix=spBlackholePrefix, spLicenseErrLimit=spLicenseErrLimit, spLicenseErrType=spLicenseErrType, fingerprintUsage=fingerprintUsage, greDown=greDown, flowDown=flowDown, spManagedObjectFamily=spManagedObjectFamily, deviceDiskUsage=deviceDiskUsage, spcommFailure=spcommFailure, spCommFailureDescription=spCommFailureDescription, greDownDone=greDownDone, spFlowspecTimeout=spFlowspecTimeout, spBGPTrapPrefix=spBGPTrapPrefix, deviceSystemErrorDone=deviceSystemErrorDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, peakflowSPMIB=peakflowSPMIB, spManagedObject=spManagedObject, spDNSExpected=spDNSExpected, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSCommunity=spTMSCommunity, spInterface=spInterface, spServiceName=spServiceName, spInterfaceUsageHC=spInterfaceUsageHC, mitigationTMSStartV2=mitigationTMSStartV2, peakflowSPMgr=peakflowSPMgr, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spThirdPartyZone=spThirdPartyZone, spReportURL=spReportURL, spImpactPps=spImpactPps, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, devicePhysicalMemory=devicePhysicalMemory, PYSNMP_MODULE_ID=peakflowSPMIB, spUsage=spUsage, configChange=configChange, peakflowSPTraps=peakflowSPTraps, spTMSTimeout=spTMSTimeout, spDNSName=spDNSName, spLicenseErrCount=spLicenseErrCount, spUsageType=spUsageType, spScriptPort=spScriptPort, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, managedObjectUsageDone=managedObjectUsageDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spVersion=spVersion, spBlackholeTimeout=spBlackholeTimeout, spAlertID=spAlertID, spDNSObservedMean=spDNSObservedMean, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, mitigationTMSStart=mitigationTMSStart, hardwareFailureDone=hardwareFailureDone, spRouter=spRouter, spScriptStart=spScriptStart, spScriptCommand=spScriptCommand, spAlertDetectionSignatures=spAlertDetectionSignatures, spSystemErrorType=spSystemErrorType, spFingerprintFeedback=spFingerprintFeedback, spInetAddress=spInetAddress, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBGPEvent=spBGPEvent, interfaceUsage=interfaceUsage, dosHostDetectionAlert=dosHostDetectionAlert, collectorStart=collectorStart, bgpTrap=bgpTrap, spApplicationName=spApplicationName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spTMSMultiPrefix=spTMSMultiPrefix, spLicenseErrDescription=spLicenseErrDescription, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, mitigationDone=mitigationDone, spFingerprintName=spFingerprintName, dnsBaselineDone=dnsBaselineDone, spHijackAttr=spHijackAttr, spPravail=spPravail, bgpInstability=bgpInstability, bgpInstabilityDone=bgpInstabilityDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spSystemErrorDescription=spSystemErrorDescription, fingerprintReceive=fingerprintReceive, deviceTotalFlows=deviceTotalFlows, bgpUp=bgpUp, mitigationBlackholeStart=mitigationBlackholeStart, dnsBaseline=dnsBaseline, spCloudSignalFaultDescription=spCloudSignalFaultDescription, hardwareFailure=hardwareFailure, autoclassifyStart=autoclassifyStart, peakflowSPCMI=peakflowSPCMI, deviceHealth=deviceHealth, spFingerprintMessage=spFingerprintMessage, cloudSignalingFault=cloudSignalingFault, dosHostDetectionAlertDone=dosHostDetectionAlertDone, fingerprintUsageDone=fingerprintUsageDone, deviceSystemError=deviceSystemError, spHardwareFailureDescription=spHardwareFailureDescription, spInterfaceUsage=spInterfaceUsage, spBGPTrapName=spBGPTrapName, spCollector=spCollector, spServiceElement=spServiceElement, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInterfaceSpeed=spInterfaceSpeed, spFingerprintSender=spFingerprintSender, spCommFailureDestination=spCommFailureDestination, spGreTunnelDestination=spGreTunnelDestination, spcommFailureDone=spcommFailureDone, spBGPInstability=spBGPInstability, interfaceUsageDone=interfaceUsageDone, spDNSObserved=spDNSObserved, spInterfaceSpeedHC=spInterfaceSpeedHC, spManagedObjects=spManagedObjects, spImpactBps=spImpactBps, licenseError=licenseError, spGreTunnelName=spGreTunnelName, trafficAutoMitigation=trafficAutoMitigation, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spUnit=spUnit, snmpUp=snmpUp, serviceUsage=serviceUsage, cloudSignalingFaultDone=cloudSignalingFaultDone, spMitigationName=spMitigationName, spScriptHost=spScriptHost, spTMSPrefix=spTMSPrefix, spBlackholeCommunity=spBlackholeCommunity, spMitigationID=spMitigationID, spFlowspecCommunity=spFlowspecCommunity, licenseErrorDone=licenseErrorDone, bgpDown=bgpDown, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spDuration=spDuration, spDNSObservedMax=spDNSObservedMax, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spUsername=spUsername, spThreshold=spThreshold, collectorUp=collectorUp, routingFailover=routingFailover, spHijackRoute=spHijackRoute, spHijackLocal=spHijackLocal, fingerprintFeedback=fingerprintFeedback, spInetAddressType=spInetAddressType, deviceTotalFlowsHC=deviceTotalFlowsHC, managedObjectUsage=managedObjectUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, deviceSwapSpace=deviceSwapSpace, serviceUsageDone=serviceUsageDone, alertScript=alertScript, collectorDown=collectorDown, spThirdPartyAddr=spThirdPartyAddr)
