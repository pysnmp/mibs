...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDNSObservedMax=spDNSObservedMax, spMitigationStart=spMitigationStart, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, bgpUp=bgpUp, deviceSystemError=deviceSystemError, spInterface=spInterface, spSystemErrorDescription=spSystemErrorDescription, peakflowSPMIB=peakflowSPMIB, spBGPTrapName=spBGPTrapName, spUnit=spUnit, spAlertDetectionSignatures=spAlertDetectionSignatures, spScriptCommand=spScriptCommand, fingerprintUsageDone=fingerprintUsageDone, deviceSwapSpace=deviceSwapSpace, spVersion=spVersion, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spInterfaceSpeed=spInterfaceSpeed, spTMSPrefix=spTMSPrefix, spFingerprintFeedback=spFingerprintFeedback, spManagedObject=spManagedObject, spImpactPps=spImpactPps, spReportID=spReportID, spInterfaceUsage=spInterfaceUsage, spInterfaceIndex=spInterfaceIndex, spAlertID=spAlertID, spSystemErrorType=spSystemErrorType, spManagedObjects=spManagedObjects, spInetAddress=spInetAddress, spcommFailureDone=spcommFailureDone, mitigationTMSStartV2=mitigationTMSStartV2, spLicenseErrLimit=spLicenseErrLimit, managedObjectUsageDone=managedObjectUsageDone, spDNSName=spDNSName, bgpInstabilityDone=bgpInstabilityDone, mitigationTMSStart=mitigationTMSStart, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, interfaceUsage=interfaceUsage, fingerprintFeedback=fingerprintFeedback, spInetAddressType=spInetAddressType, deviceTotalFlowsHC=deviceTotalFlowsHC, spUsername=spUsername, licenseError=licenseError, spcommFailure=spcommFailure, spScriptStart=spScriptStart, dnsBaselineDone=dnsBaselineDone, spBGPTrapEvent=spBGPTrapEvent, greDownDone=greDownDone, flowDown=flowDown, spDuration=spDuration, spBlackholeCommunity=spBlackholeCommunity, devicePhysicalMemory=devicePhysicalMemory, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, peakflowSPMgr=peakflowSPMgr, spReportName=spReportName, hardwareFailureDone=hardwareFailureDone, alertScript=alertScript, spHijackLocal=spHijackLocal, spMitigationID=spMitigationID, bgpHijack=bgpHijack, fingerprintReceive=fingerprintReceive, serviceUsage=serviceUsage, spTMSTimeout=spTMSTimeout, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spGreTunnelName=spGreTunnelName, collectorStart=collectorStart, snmpDown=snmpDown, spBGPTrapPrefix=spBGPTrapPrefix, spManagedObjectFamily=spManagedObjectFamily, spBGPEvent=spBGPEvent, spDetector=spDetector, spTMSCommunity=spTMSCommunity, routingFailover=routingFailover, licenseErrorDone=licenseErrorDone, spImpactBps=spImpactBps, spBlackholeTimeout=spBlackholeTimeout, interfaceUsageDone=interfaceUsageDone, bgpDown=bgpDown, spApplicationName=spApplicationName, deviceSystemErrorDone=deviceSystemErrorDone, dnsBaseline=dnsBaseline, spUsageType=spUsageType, spMitigationName=spMitigationName, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spCommFailureDescription=spCommFailureDescription, collectorUp=collectorUp, bgpInstability=bgpInstability, spHijackAttr=spHijackAttr, spBlackholePrefix=spBlackholePrefix, deviceHealth=deviceHealth, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, mitigationThirdPartyStart=mitigationThirdPartyStart, fingerprintUsage=fingerprintUsage, spServiceName=spServiceName, spDNSExpected=spDNSExpected, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, peakflowSPTraps=peakflowSPTraps, trafficAutoMitigation=trafficAutoMitigation, cloudSignalingFaultDone=cloudSignalingFaultDone, configChange=configChange, spInterfaceUsageHC=spInterfaceUsageHC, deviceDiskUsage=deviceDiskUsage, spDNSObserved=spDNSObserved, snmpUp=snmpUp, spThirdPartyAddr=spThirdPartyAddr, spLicenseErrDescription=spLicenseErrDescription, spScriptHost=spScriptHost, spPravail=spPravail, spLicenseErrType=spLicenseErrType, collectorDown=collectorDown, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spUsage=spUsage, spFingerprintName=spFingerprintName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, mitigationFlowspecStart=mitigationFlowspecStart, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, cloudSignalingFault=cloudSignalingFault, spBGPTrapOldAttributes=spBGPTrapOldAttributes, bgpTrap=bgpTrap, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, dosHostDetectionAlert=dosHostDetectionAlert, spRouter=spRouter, spHardwareFailureDescription=spHardwareFailureDescription, spInterfaceSpeedHC=spInterfaceSpeedHC, autoclassifyStart=autoclassifyStart, serviceUsageDone=serviceUsageDone, reportDone=reportDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spServiceElement=spServiceElement, spThirdPartyZone=spThirdPartyZone, spGreTunnelDestination=spGreTunnelDestination, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spCommFailureDestination=spCommFailureDestination, PYSNMP_MODULE_ID=peakflowSPMIB, spTMSMultiPrefix=spTMSMultiPrefix, mitigationDone=mitigationDone, mitigationBlackholeStart=mitigationBlackholeStart, spFingerprintMessage=spFingerprintMessage, managedObjectUsage=managedObjectUsage, spCollector=spCollector, spScriptPort=spScriptPort, spHijackRoute=spHijackRoute, spDetectedCountries=spDetectedCountries, peakflowSPCMI=peakflowSPCMI, spBGPInstability=spBGPInstability, hardwareFailure=hardwareFailure, spBlackholeNexthop=spBlackholeNexthop, spFingerprintSender=spFingerprintSender, notificationLimit=notificationLimit, deviceSwapSpaceInUse=deviceSwapSpaceInUse, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, greDown=greDown, deviceTotalFlows=deviceTotalFlows, spFlowspecTimeout=spFlowspecTimeout, spFlowspecCommunity=spFlowspecCommunity, spThreshold=spThreshold, flowUp=flowUp, spDNSObservedMean=spDNSObservedMean, spLicenseErrCount=spLicenseErrCount, spReportURL=spReportURL, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min)
