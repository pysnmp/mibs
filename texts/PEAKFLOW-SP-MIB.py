...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", snmpUp=snmpUp, spHijackRoute=spHijackRoute, spHijackLocal=spHijackLocal, spScriptPort=spScriptPort, deviceTotalFlows=deviceTotalFlows, collectorStart=collectorStart, spBGPEvent=spBGPEvent, spInetAddress=spInetAddress, devicePhysicalMemory=devicePhysicalMemory, spInterfaceUsageHC=spInterfaceUsageHC, spBlackholeCommunity=spBlackholeCommunity, spCollector=spCollector, spApplicationName=spApplicationName, fingerprintUsageDone=fingerprintUsageDone, spInterface=spInterface, spAlertDetectionSignatures=spAlertDetectionSignatures, dosHostDetectionAlert=dosHostDetectionAlert, spManagedObject=spManagedObject, deviceSystemError=deviceSystemError, spLicenseErrDescription=spLicenseErrDescription, collectorDown=collectorDown, mitigationFlowspecStart=mitigationFlowspecStart, spMitigationID=spMitigationID, spMitigationName=spMitigationName, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spUnit=spUnit, spScriptCommand=spScriptCommand, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, fingerprintReceive=fingerprintReceive, dnsBaselineDone=dnsBaselineDone, spUsageType=spUsageType, spMitigationStart=spMitigationStart, bgpTrap=bgpTrap, trafficAutoMitigation=trafficAutoMitigation, spLicenseErrType=spLicenseErrType, fingerprintFeedback=fingerprintFeedback, spTMSCommunity=spTMSCommunity, flowDown=flowDown, snmpDown=snmpDown, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, mitigationBlackholeStart=mitigationBlackholeStart, spThreshold=spThreshold, spDNSExpected=spDNSExpected, cloudSignalingFaultDone=cloudSignalingFaultDone, spDNSName=spDNSName, peakflowSPCMI=peakflowSPCMI, spInterfaceIndex=spInterfaceIndex, spBGPTrapEvent=spBGPTrapEvent, spThirdPartyZone=spThirdPartyZone, peakflowSPMIB=peakflowSPMIB, spImpactPps=spImpactPps, spRouter=spRouter, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, alertScript=alertScript, spInterfaceSpeed=spInterfaceSpeed, spLicenseErrCount=spLicenseErrCount, flowUp=flowUp, dnsBaseline=dnsBaseline, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, peakflowSPTraps=peakflowSPTraps, spBlackholeTimeout=spBlackholeTimeout, serviceUsage=serviceUsage, spReportURL=spReportURL, spDNSObservedMax=spDNSObservedMax, serviceUsageDone=serviceUsageDone, licenseError=licenseError, spImpactBps=spImpactBps, spFingerprintFeedback=spFingerprintFeedback, spServiceName=spServiceName, spGreTunnelName=spGreTunnelName, spScriptStart=spScriptStart, hardwareFailureDone=hardwareFailureDone, spDetector=spDetector, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spBGPTrapPrefix=spBGPTrapPrefix, interfaceUsage=interfaceUsage, spDuration=spDuration, spHijackAttr=spHijackAttr, bgpDown=bgpDown, deviceHealth=deviceHealth, licenseErrorDone=licenseErrorDone, mitigationDone=mitigationDone, spFingerprintSender=spFingerprintSender, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spHardwareFailureDescription=spHardwareFailureDescription, notificationLimit=notificationLimit, spcommFailure=spcommFailure, cloudSignalingFault=cloudSignalingFault, spCommFailureDestination=spCommFailureDestination, spSystemErrorType=spSystemErrorType, deviceSystemErrorDone=deviceSystemErrorDone, fingerprintUsage=fingerprintUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spServiceElement=spServiceElement, configChange=configChange, deviceSwapSpace=deviceSwapSpace, routingFailover=routingFailover, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spDNSObservedMean=spDNSObservedMean, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spReportID=spReportID, spThirdPartyAddr=spThirdPartyAddr, spAlertID=spAlertID, spDetectedCountries=spDetectedCountries, spCommFailureDescription=spCommFailureDescription, spInterfaceUsage=spInterfaceUsage, spFingerprintName=spFingerprintName, spInetAddressType=spInetAddressType, spFingerprintMessage=spFingerprintMessage, mitigationThirdPartyStart=mitigationThirdPartyStart, spBlackholeNexthop=spBlackholeNexthop, bgpInstability=bgpInstability, peakflowSPMgr=peakflowSPMgr, spDNSObserved=spDNSObserved, collectorUp=collectorUp, hardwareFailure=hardwareFailure, spTMSTimeout=spTMSTimeout, interfaceUsageDone=interfaceUsageDone, spTMSPrefix=spTMSPrefix, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, bgpInstabilityDone=bgpInstabilityDone, spTMSMultiPrefix=spTMSMultiPrefix, spScriptHost=spScriptHost, greDown=greDown, spVersion=spVersion, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, autoclassifyStart=autoclassifyStart, spGreTunnelDestination=spGreTunnelDestination, deviceTotalFlowsHC=deviceTotalFlowsHC, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spFlowspecCommunity=spFlowspecCommunity, reportDone=reportDone, spPravail=spPravail, spUsername=spUsername, managedObjectUsageDone=managedObjectUsageDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spUsage=spUsage, spFlowspecTimeout=spFlowspecTimeout, deviceDiskUsage=deviceDiskUsage, spSystemErrorDescription=spSystemErrorDescription, spReportName=spReportName, bgpHijack=bgpHijack, spManagedObjectFamily=spManagedObjectFamily, spInterfaceSpeedHC=spInterfaceSpeedHC, spManagedObjects=spManagedObjects, dosNetworkProfiledAlert=dosNetworkProfiledAlert, mitigationTMSStartV2=mitigationTMSStartV2, PYSNMP_MODULE_ID=peakflowSPMIB, spcommFailureDone=spcommFailureDone, managedObjectUsage=managedObjectUsage, mitigationTMSStart=mitigationTMSStart, spBGPInstability=spBGPInstability, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBlackholePrefix=spBlackholePrefix, greDownDone=greDownDone, bgpUp=bgpUp, spLicenseErrLimit=spLicenseErrLimit, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBGPTrapName=spBGPTrapName)
