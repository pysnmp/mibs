...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spVersion=spVersion, serviceUsage=serviceUsage, spUsageType=spUsageType, spLicenseErrCount=spLicenseErrCount, spServiceElement=spServiceElement, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, mitigationDone=mitigationDone, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpDown=bgpDown, spApplicationName=spApplicationName, spUsername=spUsername, spInterfaceIndex=spInterfaceIndex, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFingerprintSender=spFingerprintSender, spBGPTrapOldAttributes=spBGPTrapOldAttributes, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInterfaceUsageHC=spInterfaceUsageHC, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, interfaceUsageDone=interfaceUsageDone, fingerprintFeedback=fingerprintFeedback, spUsage=spUsage, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFlowspecTimeout=spFlowspecTimeout, devicePhysicalMemory=devicePhysicalMemory, collectorDown=collectorDown, spManagedObjectFamily=spManagedObjectFamily, spInterface=spInterface, autoclassifyStart=autoclassifyStart, peakflowSPMgr=peakflowSPMgr, spDNSExpected=spDNSExpected, spImpactPps=spImpactPps, spServiceName=spServiceName, spFingerprintMessage=spFingerprintMessage, greDownDone=greDownDone, deviceHealth=deviceHealth, spDNSObserved=spDNSObserved, spFlowspecCommunity=spFlowspecCommunity, spTMSCommunity=spTMSCommunity, spHardwareFailureDescription=spHardwareFailureDescription, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, greDown=greDown, spPravail=spPravail, spBGPTrapEvent=spBGPTrapEvent, licenseError=licenseError, mitigationTMSStartV2=mitigationTMSStartV2, spDuration=spDuration, spInterfaceSpeedHC=spInterfaceSpeedHC, spHijackAttr=spHijackAttr, spImpactBps=spImpactBps, spTMSPrefix=spTMSPrefix, spInetAddress=spInetAddress, peakflowSPMIB=peakflowSPMIB, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceTotalFlows=deviceTotalFlows, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceSystemError=deviceSystemError, spBGPTrapName=spBGPTrapName, dosHostDetectionAlertDone=dosHostDetectionAlertDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDNSName=spDNSName, deviceSystemErrorDone=deviceSystemErrorDone, spCollector=spCollector, spBGPTrapPrefix=spBGPTrapPrefix, managedObjectUsageDone=managedObjectUsageDone, spScriptStart=spScriptStart, spCommFailureDescription=spCommFailureDescription, bgpInstabilityDone=bgpInstabilityDone, spMitigationName=spMitigationName, spcommFailureDone=spcommFailureDone, spManagedObjects=spManagedObjects, spBlackholeTimeout=spBlackholeTimeout, peakflowSPTraps=peakflowSPTraps, collectorStart=collectorStart, spTMSTimeout=spTMSTimeout, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spBlackholePrefix=spBlackholePrefix, spThirdPartyAddr=spThirdPartyAddr, mitigationThirdPartyStart=mitigationThirdPartyStart, interfaceUsage=interfaceUsage, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spManagedObject=spManagedObject, spMitigationID=spMitigationID, spGreTunnelName=spGreTunnelName, licenseErrorDone=licenseErrorDone, spUnit=spUnit, spLicenseErrLimit=spLicenseErrLimit, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spFingerprintFeedback=spFingerprintFeedback, fingerprintUsageDone=fingerprintUsageDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBlackholeCommunity=spBlackholeCommunity, spTMSMultiPrefix=spTMSMultiPrefix, spScriptCommand=spScriptCommand, spSystemErrorType=spSystemErrorType, spReportID=spReportID, flowDown=flowDown, spSystemErrorDescription=spSystemErrorDescription, spBlackholeNexthop=spBlackholeNexthop, fingerprintReceive=fingerprintReceive, spBGPTrapNewAttributes=spBGPTrapNewAttributes, snmpDown=snmpDown, deviceSwapSpace=deviceSwapSpace, routingFailover=routingFailover, alertScript=alertScript, spAlertID=spAlertID, bgpHijack=bgpHijack, spBGPEvent=spBGPEvent, collectorUp=collectorUp, bgpInstability=bgpInstability, spThreshold=spThreshold, configChange=configChange, serviceUsageDone=serviceUsageDone, spBGPInstability=spBGPInstability, spDetector=spDetector, spHijackRoute=spHijackRoute, bgpUp=bgpUp, spInterfaceSpeed=spInterfaceSpeed, managedObjectUsage=managedObjectUsage, spInterfaceUsage=spInterfaceUsage, spMitigationStart=spMitigationStart, spLicenseErrDescription=spLicenseErrDescription, spAlertDetectionSignatures=spAlertDetectionSignatures, peakflowSPCMI=peakflowSPCMI, hardwareFailure=hardwareFailure, spRouter=spRouter, flowUp=flowUp, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spHijackLocal=spHijackLocal, bgpTrap=bgpTrap, spcommFailure=spcommFailure, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dnsBaselineDone=dnsBaselineDone, mitigationTMSStart=mitigationTMSStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, deviceDiskUsage=deviceDiskUsage, spScriptPort=spScriptPort, spReportName=spReportName, dnsBaseline=dnsBaseline, spCommFailureDestination=spCommFailureDestination, reportDone=reportDone, spDNSObservedMax=spDNSObservedMax, spGreTunnelDestination=spGreTunnelDestination, mitigationFlowspecStart=mitigationFlowspecStart, spThirdPartyZone=spThirdPartyZone, snmpUp=snmpUp, fingerprintUsage=fingerprintUsage, dosHostDetectionAlert=dosHostDetectionAlert, spDNSObservedMean=spDNSObservedMean, notificationLimit=notificationLimit, hardwareFailureDone=hardwareFailureDone, spReportURL=spReportURL, cloudSignalingFault=cloudSignalingFault, PYSNMP_MODULE_ID=peakflowSPMIB, mitigationBlackholeStart=mitigationBlackholeStart, spScriptHost=spScriptHost, spInetAddressType=spInetAddressType, trafficAutoMitigation=trafficAutoMitigation, spLicenseErrType=spLicenseErrType, spDetectedCountries=spDetectedCountries, spFingerprintName=spFingerprintName)
