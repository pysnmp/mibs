...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", dnsBaseline=dnsBaseline, spBGPInstability=spBGPInstability, spcommFailure=spcommFailure, spUsage=spUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBGPTrapEvent=spBGPTrapEvent, configChange=configChange, spLicenseErrLimit=spLicenseErrLimit, cloudSignalingFault=cloudSignalingFault, spSystemErrorType=spSystemErrorType, mitigationFlowspecStart=mitigationFlowspecStart, spInterfaceUsage=spInterfaceUsage, hardwareFailureDone=hardwareFailureDone, spAlertDetectionSignatures=spAlertDetectionSignatures, trafficAutoMitigation=trafficAutoMitigation, managedObjectUsageDone=managedObjectUsageDone, spThirdPartyZone=spThirdPartyZone, bgpTrap=bgpTrap, spCommFailureDestination=spCommFailureDestination, deviceSwapSpaceUsage=deviceSwapSpaceUsage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spUsageType=spUsageType, devicePhysicalMemory=devicePhysicalMemory, spBGPTrapPrefix=spBGPTrapPrefix, bgpUp=bgpUp, notificationLimit=notificationLimit, interfaceUsageDone=interfaceUsageDone, spManagedObject=spManagedObject, spFingerprintName=spFingerprintName, snmpDown=snmpDown, spThreshold=spThreshold, bgpInstability=bgpInstability, spGreTunnelDestination=spGreTunnelDestination, greDownDone=greDownDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spLicenseErrType=spLicenseErrType, mitigationBlackholeStart=mitigationBlackholeStart, spDNSObservedMax=spDNSObservedMax, deviceSystemErrorDone=deviceSystemErrorDone, fingerprintFeedback=fingerprintFeedback, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, alertScript=alertScript, spImpactPps=spImpactPps, spUsername=spUsername, spLicenseErrCount=spLicenseErrCount, spMitigationID=spMitigationID, serviceUsageDone=serviceUsageDone, spDNSObservedMean=spDNSObservedMean, deviceDiskUsage=deviceDiskUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, interfaceUsage=interfaceUsage, spHijackRoute=spHijackRoute, spcommFailureDone=spcommFailureDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spBGPTrapNewAttributes=spBGPTrapNewAttributes, serviceUsage=serviceUsage, spScriptHost=spScriptHost, peakflowSPMgr=peakflowSPMgr, spBlackholeTimeout=spBlackholeTimeout, spFingerprintFeedback=spFingerprintFeedback, collectorUp=collectorUp, spInetAddressType=spInetAddressType, dosHostDetectionAlert=dosHostDetectionAlert, spSystemErrorDescription=spSystemErrorDescription, licenseErrorDone=licenseErrorDone, spHijackLocal=spHijackLocal, collectorStart=collectorStart, fingerprintUsage=fingerprintUsage, mitigationTMSStartV2=mitigationTMSStartV2, spBGPTrapName=spBGPTrapName, spDetectedCountries=spDetectedCountries, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spManagedObjectFamily=spManagedObjectFamily, spScriptCommand=spScriptCommand, managedObjectUsage=managedObjectUsage, peakflowSPCMI=peakflowSPCMI, greDown=greDown, bgpHijack=bgpHijack, spInterface=spInterface, spFlowspecCommunity=spFlowspecCommunity, deviceTotalFlows=deviceTotalFlows, spServiceElement=spServiceElement, hardwareFailure=hardwareFailure, deviceHealth=deviceHealth, spFingerprintMessage=spFingerprintMessage, collectorDown=collectorDown, fingerprintReceive=fingerprintReceive, spDuration=spDuration, bgpDown=bgpDown, flowUp=flowUp, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spHardwareFailureDescription=spHardwareFailureDescription, spFingerprintSender=spFingerprintSender, spImpactBps=spImpactBps, spTMSMultiPrefix=spTMSMultiPrefix, routingFailover=routingFailover, spMitigationStart=spMitigationStart, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spAlertID=spAlertID, spApplicationName=spApplicationName, spManagedObjects=spManagedObjects, spInterfaceUsageHC=spInterfaceUsageHC, spVersion=spVersion, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, dnsBaselineDone=dnsBaselineDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, peakflowSPMIB=peakflowSPMIB, deviceSystemError=deviceSystemError, spReportID=spReportID, spRouter=spRouter, licenseError=licenseError, spBGPEvent=spBGPEvent, deviceSwapSpace=deviceSwapSpace, spDNSObserved=spDNSObserved, spTMSTimeout=spTMSTimeout, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spScriptStart=spScriptStart, mitigationThirdPartyStart=mitigationThirdPartyStart, spDetector=spDetector, fingerprintUsageDone=fingerprintUsageDone, spThirdPartyAddr=spThirdPartyAddr, spGreTunnelName=spGreTunnelName, spReportName=spReportName, spInterfaceIndex=spInterfaceIndex, spTMSPrefix=spTMSPrefix, peakflowSPTraps=peakflowSPTraps, spHijackAttr=spHijackAttr, spBlackholePrefix=spBlackholePrefix, spBlackholeNexthop=spBlackholeNexthop, PYSNMP_MODULE_ID=peakflowSPMIB, spCollector=spCollector, spScriptPort=spScriptPort, spTMSCommunity=spTMSCommunity, mitigationDone=mitigationDone, spInetAddress=spInetAddress, spServiceName=spServiceName, spLicenseErrDescription=spLicenseErrDescription, spBlackholeCommunity=spBlackholeCommunity, mitigationTMSStart=mitigationTMSStart, bgpInstabilityDone=bgpInstabilityDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spDNSName=spDNSName, spMitigationName=spMitigationName, spDNSExpected=spDNSExpected, spReportURL=spReportURL, deviceTotalFlowsHC=deviceTotalFlowsHC, flowDown=flowDown, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spCommFailureDescription=spCommFailureDescription, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, snmpUp=snmpUp, spInterfaceSpeedHC=spInterfaceSpeedHC, reportDone=reportDone, spPravail=spPravail, spInterfaceSpeed=spInterfaceSpeed, spUnit=spUnit, spFlowspecTimeout=spFlowspecTimeout, autoclassifyStart=autoclassifyStart, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min)
