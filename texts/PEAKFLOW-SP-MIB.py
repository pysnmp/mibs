...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spMitigationStart=spMitigationStart, spHijackLocal=spHijackLocal, spGreTunnelDestination=spGreTunnelDestination, peakflowSPTraps=peakflowSPTraps, spBGPTrapOldAttributes=spBGPTrapOldAttributes, devicePhysicalMemory=devicePhysicalMemory, spHardwareFailureDescription=spHardwareFailureDescription, flowUp=flowUp, spcommFailure=spcommFailure, interfaceUsage=interfaceUsage, dosHostDetectionAlert=dosHostDetectionAlert, bgpInstability=bgpInstability, spFingerprintMessage=spFingerprintMessage, spFingerprintFeedback=spFingerprintFeedback, fingerprintUsage=fingerprintUsage, spServiceElement=spServiceElement, spLicenseErrCount=spLicenseErrCount, licenseErrorDone=licenseErrorDone, spCollector=spCollector, PYSNMP_MODULE_ID=peakflowSPMIB, mitigationTMSStart=mitigationTMSStart, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spUsage=spUsage, bgpInstabilityDone=bgpInstabilityDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, serviceUsage=serviceUsage, bgpHijack=bgpHijack, autoclassifyStart=autoclassifyStart, spInterfaceUsageHC=spInterfaceUsageHC, spSystemErrorType=spSystemErrorType, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spDetectedCountries=spDetectedCountries, collectorDown=collectorDown, routingFailover=routingFailover, dnsBaselineDone=dnsBaselineDone, bgpTrap=bgpTrap, spUnit=spUnit, spScriptHost=spScriptHost, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBGPEvent=spBGPEvent, spReportID=spReportID, spBlackholeTimeout=spBlackholeTimeout, fingerprintUsageDone=fingerprintUsageDone, spTMSCommunity=spTMSCommunity, spDNSName=spDNSName, spReportURL=spReportURL, spInterfaceUsage=spInterfaceUsage, configChange=configChange, mitigationBlackholeStart=mitigationBlackholeStart, dosNetworkProfiledAlert=dosNetworkProfiledAlert, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, fingerprintReceive=fingerprintReceive, spThirdPartyAddr=spThirdPartyAddr, spBGPTrapPrefix=spBGPTrapPrefix, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spImpactPps=spImpactPps, peakflowSPMgr=peakflowSPMgr, mitigationFlowspecStart=mitigationFlowspecStart, spManagedObjects=spManagedObjects, spTMSMultiPrefix=spTMSMultiPrefix, deviceSwapSpaceUsage=deviceSwapSpaceUsage, fingerprintFeedback=fingerprintFeedback, peakflowSPMIB=peakflowSPMIB, mitigationThirdPartyStart=mitigationThirdPartyStart, spThirdPartyZone=spThirdPartyZone, spBGPInstability=spBGPInstability, spInterface=spInterface, greDownDone=greDownDone, spGreTunnelName=spGreTunnelName, spManagedObject=spManagedObject, spImpactBps=spImpactBps, spBGPTrapName=spBGPTrapName, spBlackholePrefix=spBlackholePrefix, spCommFailureDescription=spCommFailureDescription, deviceSystemError=deviceSystemError, cloudSignalingFault=cloudSignalingFault, spDNSObserved=spDNSObserved, managedObjectUsageDone=managedObjectUsageDone, spLicenseErrType=spLicenseErrType, spUsageType=spUsageType, spLicenseErrLimit=spLicenseErrLimit, spBlackholeCommunity=spBlackholeCommunity, spCloudSignalFaultDescription=spCloudSignalFaultDescription, notificationLimit=notificationLimit, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceDiskUsage=deviceDiskUsage, deviceSwapSpace=deviceSwapSpace, snmpDown=snmpDown, spScriptCommand=spScriptCommand, spInetAddressType=spInetAddressType, collectorUp=collectorUp, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spTMSTimeout=spTMSTimeout, hardwareFailure=hardwareFailure, greDown=greDown, spHijackAttr=spHijackAttr, spFingerprintName=spFingerprintName, alertScript=alertScript, spRouter=spRouter, spFingerprintSender=spFingerprintSender, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, collectorStart=collectorStart, spThreshold=spThreshold, spVersion=spVersion, spManagedObjectFamily=spManagedObjectFamily, spTMSPrefix=spTMSPrefix, spcommFailureDone=spcommFailureDone, spBGPTrapEvent=spBGPTrapEvent, spScriptStart=spScriptStart, spInterfaceSpeed=spInterfaceSpeed, licenseError=licenseError, dosHostDetectionAlertDone=dosHostDetectionAlertDone, trafficAutoMitigation=trafficAutoMitigation, dnsBaseline=dnsBaseline, spDuration=spDuration, spLicenseErrDescription=spLicenseErrDescription, spServiceName=spServiceName, deviceTotalFlows=deviceTotalFlows, interfaceUsageDone=interfaceUsageDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, deviceHealth=deviceHealth, bgpDown=bgpDown, spPravail=spPravail, mitigationTMSStartV2=mitigationTMSStartV2, spHijackRoute=spHijackRoute, spAlertID=spAlertID, hardwareFailureDone=hardwareFailureDone, spDNSExpected=spDNSExpected, spMitigationID=spMitigationID, flowDown=flowDown, mitigationDone=mitigationDone, spApplicationName=spApplicationName, spDNSObservedMean=spDNSObservedMean, spMitigationName=spMitigationName, spCommFailureDestination=spCommFailureDestination, spReportName=spReportName, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, managedObjectUsage=managedObjectUsage, peakflowSPCMI=peakflowSPCMI, bgpUp=bgpUp, deviceSystemErrorDone=deviceSystemErrorDone, spScriptPort=spScriptPort, spAlertDetectionSignatures=spAlertDetectionSignatures, spFlowspecCommunity=spFlowspecCommunity, spInterfaceSpeedHC=spInterfaceSpeedHC, reportDone=reportDone, serviceUsageDone=serviceUsageDone, spDNSObservedMax=spDNSObservedMax, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spDetector=spDetector, spInetAddress=spInetAddress, deviceTotalFlowsHC=deviceTotalFlowsHC, spInterfaceIndex=spInterfaceIndex, spFlowspecTimeout=spFlowspecTimeout, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBlackholeNexthop=spBlackholeNexthop, spSystemErrorDescription=spSystemErrorDescription, spUsername=spUsername, snmpUp=snmpUp)
