...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spFlowspecCommunity=spFlowspecCommunity, spServiceElement=spServiceElement, snmpDown=snmpDown, spAlertID=spAlertID, spBGPTrapName=spBGPTrapName, spUsername=spUsername, spImpactBps=spImpactBps, spBGPInstability=spBGPInstability, spFingerprintName=spFingerprintName, spScriptCommand=spScriptCommand, managedObjectUsage=managedObjectUsage, spDuration=spDuration, spFingerprintMessage=spFingerprintMessage, peakflowSPCMI=peakflowSPCMI, spBlackholeNexthop=spBlackholeNexthop, trafficAutoMitigation=trafficAutoMitigation, spHijackLocal=spHijackLocal, spDetectedCountries=spDetectedCountries, spTMSMultiPrefix=spTMSMultiPrefix, spCommFailureDescription=spCommFailureDescription, cloudSignalingFault=cloudSignalingFault, spDNSObserved=spDNSObserved, fingerprintUsage=fingerprintUsage, spDetector=spDetector, spcommFailureDone=spcommFailureDone, spMitigationID=spMitigationID, collectorDown=collectorDown, serviceUsageDone=serviceUsageDone, spLicenseErrLimit=spLicenseErrLimit, spLicenseErrDescription=spLicenseErrDescription, collectorUp=collectorUp, spFingerprintFeedback=spFingerprintFeedback, deviceTotalFlows=deviceTotalFlows, devicePhysicalMemory=devicePhysicalMemory, reportDone=reportDone, spLicenseErrType=spLicenseErrType, deviceTotalFlowsHC=deviceTotalFlowsHC, cloudSignalingFaultDone=cloudSignalingFaultDone, notificationLimit=notificationLimit, spTMSPrefix=spTMSPrefix, spReportID=spReportID, alertScript=alertScript, licenseErrorDone=licenseErrorDone, dnsBaselineDone=dnsBaselineDone, dnsBaseline=dnsBaseline, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, serviceUsage=serviceUsage, greDown=greDown, snmpUp=snmpUp, spInterfaceUsageHC=spInterfaceUsageHC, spVersion=spVersion, deviceDiskUsage=deviceDiskUsage, spInterfaceSpeedHC=spInterfaceSpeedHC, spcommFailure=spcommFailure, spMitigationStart=spMitigationStart, peakflowSPMgr=peakflowSPMgr, flowDown=flowDown, mitigationBlackholeStart=mitigationBlackholeStart, spReportURL=spReportURL, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spThirdPartyZone=spThirdPartyZone, deviceSystemErrorDone=deviceSystemErrorDone, spPravail=spPravail, spUsageType=spUsageType, spBlackholeCommunity=spBlackholeCommunity, dosHostDetectionAlertDone=dosHostDetectionAlertDone, fingerprintFeedback=fingerprintFeedback, managedObjectUsageDone=managedObjectUsageDone, deviceHealth=deviceHealth, fingerprintReceive=fingerprintReceive, bgpInstability=bgpInstability, spDNSObservedMax=spDNSObservedMax, licenseError=licenseError, spThreshold=spThreshold, spRouter=spRouter, bgpInstabilityDone=bgpInstabilityDone, spInetAddressType=spInetAddressType, bgpTrap=bgpTrap, spBGPTrapEvent=spBGPTrapEvent, spHijackRoute=spHijackRoute, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spBlackholePrefix=spBlackholePrefix, mitigationTMSStartV2=mitigationTMSStartV2, spHardwareFailureDescription=spHardwareFailureDescription, spInterfaceIndex=spInterfaceIndex, peakflowSPTraps=peakflowSPTraps, spCommFailureDestination=spCommFailureDestination, spSystemErrorType=spSystemErrorType, interfaceUsageDone=interfaceUsageDone, spThirdPartyAddr=spThirdPartyAddr, spImpactPps=spImpactPps, dosNetworkProfiledAlert=dosNetworkProfiledAlert, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spScriptStart=spScriptStart, configChange=configChange, spUsage=spUsage, spServiceName=spServiceName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, bgpUp=bgpUp, spCollector=spCollector, spTMSCommunity=spTMSCommunity, hardwareFailureDone=hardwareFailureDone, spLicenseErrCount=spLicenseErrCount, spDNSExpected=spDNSExpected, spInetAddress=spInetAddress, spSystemErrorDescription=spSystemErrorDescription, spFingerprintSender=spFingerprintSender, deviceSystemError=deviceSystemError, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBGPTrapPrefix=spBGPTrapPrefix, spHijackAttr=spHijackAttr, spScriptPort=spScriptPort, dosHostDetectionAlert=dosHostDetectionAlert, spApplicationName=spApplicationName, spInterface=spInterface, collectorStart=collectorStart, spBlackholeTimeout=spBlackholeTimeout, spDNSObservedMean=spDNSObservedMean, mitigationThirdPartyStart=mitigationThirdPartyStart, interfaceUsage=interfaceUsage, spReportName=spReportName, spGreTunnelDestination=spGreTunnelDestination, spMitigationName=spMitigationName, mitigationTMSStart=mitigationTMSStart, PYSNMP_MODULE_ID=peakflowSPMIB, spInterfaceSpeed=spInterfaceSpeed, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spScriptHost=spScriptHost, mitigationDone=mitigationDone, spInterfaceUsage=spInterfaceUsage, spManagedObject=spManagedObject, spDNSName=spDNSName, autoclassifyStart=autoclassifyStart, bgpHijack=bgpHijack, spAlertDetectionSignatures=spAlertDetectionSignatures, bgpDown=bgpDown, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFlowspecTimeout=spFlowspecTimeout, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, mitigationFlowspecStart=mitigationFlowspecStart, peakflowSPMIB=peakflowSPMIB, hardwareFailure=hardwareFailure, routingFailover=routingFailover, spTMSTimeout=spTMSTimeout, deviceSwapSpace=deviceSwapSpace, spGreTunnelName=spGreTunnelName, spBGPEvent=spBGPEvent, spManagedObjects=spManagedObjects, fingerprintUsageDone=fingerprintUsageDone, greDownDone=greDownDone, spUnit=spUnit, flowUp=flowUp, spManagedObjectFamily=spManagedObjectFamily)
