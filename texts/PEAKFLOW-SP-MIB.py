...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, licenseError=licenseError, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spRouter=spRouter, spVersion=spVersion, spHijackLocal=spHijackLocal, spScriptHost=spScriptHost, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spInterfaceUsage=spInterfaceUsage, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosHostDetectionAlert=dosHostDetectionAlert, spCollector=spCollector, alertScript=alertScript, spDetector=spDetector, deviceDiskUsage=deviceDiskUsage, spGreTunnelDestination=spGreTunnelDestination, spcommFailureDone=spcommFailureDone, collectorDown=collectorDown, collectorStart=collectorStart, spBGPTrapName=spBGPTrapName, flowDown=flowDown, deviceSystemErrorDone=deviceSystemErrorDone, bgpInstabilityDone=bgpInstabilityDone, PYSNMP_MODULE_ID=peakflowSPMIB, spInterfaceSpeedHC=spInterfaceSpeedHC, peakflowSPCMI=peakflowSPCMI, fingerprintUsage=fingerprintUsage, spSystemErrorType=spSystemErrorType, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spFingerprintName=spFingerprintName, peakflowSPMIB=peakflowSPMIB, spBlackholePrefix=spBlackholePrefix, hardwareFailure=hardwareFailure, autoclassifyStart=autoclassifyStart, spManagedObjectFamily=spManagedObjectFamily, spFingerprintFeedback=spFingerprintFeedback, reportDone=reportDone, spInterfaceSpeed=spInterfaceSpeed, hardwareFailureDone=hardwareFailureDone, collectorUp=collectorUp, trafficAutoMitigation=trafficAutoMitigation, cloudSignalingFault=cloudSignalingFault, spDNSName=spDNSName, mitigationBlackholeStart=mitigationBlackholeStart, deviceTotalFlowsHC=deviceTotalFlowsHC, routingFailover=routingFailover, deviceTotalFlows=deviceTotalFlows, spBlackholeCommunity=spBlackholeCommunity, bgpHijack=bgpHijack, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spInterfaceIndex=spInterfaceIndex, bgpInstability=bgpInstability, dnsBaselineDone=dnsBaselineDone, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spUsage=spUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBlackholeTimeout=spBlackholeTimeout, spTMSPrefix=spTMSPrefix, spInterfaceUsageHC=spInterfaceUsageHC, managedObjectUsage=managedObjectUsage, flowUp=flowUp, mitigationFlowspecStart=mitigationFlowspecStart, spFingerprintSender=spFingerprintSender, spBGPInstability=spBGPInstability, spFlowspecCommunity=spFlowspecCommunity, spDNSObservedMax=spDNSObservedMax, spBGPTrapNewAttributes=spBGPTrapNewAttributes, peakflowSPTraps=peakflowSPTraps, spUsername=spUsername, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spServiceElement=spServiceElement, spTMSMultiPrefix=spTMSMultiPrefix, licenseErrorDone=licenseErrorDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, managedObjectUsageDone=managedObjectUsageDone, spDNSExpected=spDNSExpected, fingerprintFeedback=fingerprintFeedback, peakflowSPMgr=peakflowSPMgr, spHardwareFailureDescription=spHardwareFailureDescription, spUsageType=spUsageType, spThirdPartyAddr=spThirdPartyAddr, spCloudSignalFaultDescription=spCloudSignalFaultDescription, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBGPEvent=spBGPEvent, spInetAddress=spInetAddress, mitigationTMSStart=mitigationTMSStart, spHijackRoute=spHijackRoute, spInterface=spInterface, spDNSObserved=spDNSObserved, spCommFailureDescription=spCommFailureDescription, spThreshold=spThreshold, spApplicationName=spApplicationName, spImpactPps=spImpactPps, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spSystemErrorDescription=spSystemErrorDescription, spPravail=spPravail, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spThirdPartyZone=spThirdPartyZone, fingerprintUsageDone=fingerprintUsageDone, spLicenseErrCount=spLicenseErrCount, spBlackholeNexthop=spBlackholeNexthop, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spScriptStart=spScriptStart, bgpDown=bgpDown, greDownDone=greDownDone, devicePhysicalMemory=devicePhysicalMemory, dnsBaseline=dnsBaseline, spCommFailureDestination=spCommFailureDestination, deviceSystemError=deviceSystemError, interfaceUsage=interfaceUsage, spServiceName=spServiceName, snmpDown=snmpDown, snmpUp=snmpUp, mitigationDone=mitigationDone, spAlertDetectionSignatures=spAlertDetectionSignatures, bgpUp=bgpUp, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapPrefix=spBGPTrapPrefix, deviceHealth=deviceHealth, spAlertID=spAlertID, spMitigationName=spMitigationName, spLicenseErrType=spLicenseErrType, spDuration=spDuration, spTMSCommunity=spTMSCommunity, spImpactBps=spImpactBps, spScriptPort=spScriptPort, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, serviceUsage=serviceUsage, mitigationTMSStartV2=mitigationTMSStartV2, cloudSignalingFaultDone=cloudSignalingFaultDone, spFlowspecTimeout=spFlowspecTimeout, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spGreTunnelName=spGreTunnelName, spMitigationID=spMitigationID, spUnit=spUnit, deviceSwapSpace=deviceSwapSpace, spInetAddressType=spInetAddressType, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spManagedObjects=spManagedObjects, spReportName=spReportName, notificationLimit=notificationLimit, spTMSTimeout=spTMSTimeout, greDown=greDown, fingerprintReceive=fingerprintReceive, interfaceUsageDone=interfaceUsageDone, spScriptCommand=spScriptCommand, spBGPTrapEvent=spBGPTrapEvent, spLicenseErrLimit=spLicenseErrLimit, spReportURL=spReportURL, spHijackAttr=spHijackAttr, spDNSObservedMean=spDNSObservedMean, bgpTrap=bgpTrap, spManagedObject=spManagedObject, spReportID=spReportID, spMitigationStart=spMitigationStart, spcommFailure=spcommFailure, serviceUsageDone=serviceUsageDone, configChange=configChange, spDetectedCountries=spDetectedCountries, spFingerprintMessage=spFingerprintMessage)
