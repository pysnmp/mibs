...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spHijackAttr=spHijackAttr, mitigationThirdPartyStart=mitigationThirdPartyStart, interfaceUsage=interfaceUsage, spTMSMultiPrefix=spTMSMultiPrefix, spScriptHost=spScriptHost, autoclassifyStart=autoclassifyStart, interfaceUsageDone=interfaceUsageDone, spUsername=spUsername, fingerprintUsageDone=fingerprintUsageDone, mitigationBlackholeStart=mitigationBlackholeStart, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spPravail=spPravail, spRouter=spRouter, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spInterfaceIndex=spInterfaceIndex, snmpUp=snmpUp, cloudSignalingFaultDone=cloudSignalingFaultDone, spDNSName=spDNSName, fingerprintUsage=fingerprintUsage, spThirdPartyAddr=spThirdPartyAddr, spAlertID=spAlertID, spBGPInstability=spBGPInstability, spBGPTrapEvent=spBGPTrapEvent, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, bgpHijack=bgpHijack, deviceSystemErrorDone=deviceSystemErrorDone, spReportURL=spReportURL, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spManagedObjectFamily=spManagedObjectFamily, spBGPTrapName=spBGPTrapName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, greDownDone=greDownDone, spServiceElement=spServiceElement, devicePhysicalMemory=devicePhysicalMemory, spcommFailure=spcommFailure, spBGPEvent=spBGPEvent, spThreshold=spThreshold, PYSNMP_MODULE_ID=peakflowSPMIB, spMitigationID=spMitigationID, flowDown=flowDown, spManagedObject=spManagedObject, spReportName=spReportName, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, licenseErrorDone=licenseErrorDone, hardwareFailureDone=hardwareFailureDone, dosHostDetectionAlert=dosHostDetectionAlert, deviceTotalFlowsHC=deviceTotalFlowsHC, fingerprintReceive=fingerprintReceive, deviceTotalFlows=deviceTotalFlows, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, mitigationFlowspecStart=mitigationFlowspecStart, managedObjectUsageDone=managedObjectUsageDone, bgpDown=bgpDown, peakflowSPMIB=peakflowSPMIB, spInetAddress=spInetAddress, spDetectedCountries=spDetectedCountries, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, managedObjectUsage=managedObjectUsage, bgpUp=bgpUp, spScriptCommand=spScriptCommand, spInetAddressType=spInetAddressType, spFlowspecCommunity=spFlowspecCommunity, spReportID=spReportID, hardwareFailure=hardwareFailure, alertScript=alertScript, configChange=configChange, spFingerprintName=spFingerprintName, spDNSObservedMax=spDNSObservedMax, spCommFailureDescription=spCommFailureDescription, spLicenseErrLimit=spLicenseErrLimit, routingFailover=routingFailover, serviceUsage=serviceUsage, spScriptStart=spScriptStart, spSystemErrorDescription=spSystemErrorDescription, spHijackLocal=spHijackLocal, licenseError=licenseError, spFlowspecTimeout=spFlowspecTimeout, spDuration=spDuration, spDNSObservedMean=spDNSObservedMean, spInterfaceUsage=spInterfaceUsage, spLicenseErrDescription=spLicenseErrDescription, collectorStart=collectorStart, mitigationTMSStartV2=mitigationTMSStartV2, bgpInstability=bgpInstability, spDNSObserved=spDNSObserved, spUnit=spUnit, spDNSExpected=spDNSExpected, spCommFailureDestination=spCommFailureDestination, flowUp=flowUp, serviceUsageDone=serviceUsageDone, spHardwareFailureDescription=spHardwareFailureDescription, collectorDown=collectorDown, spMitigationStart=spMitigationStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spHijackRoute=spHijackRoute, snmpDown=snmpDown, dnsBaseline=dnsBaseline, spThirdPartyZone=spThirdPartyZone, spTMSTimeout=spTMSTimeout, notificationLimit=notificationLimit, spServiceName=spServiceName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spFingerprintFeedback=spFingerprintFeedback, spGreTunnelDestination=spGreTunnelDestination, spBGPTrapPrefix=spBGPTrapPrefix, deviceHealth=deviceHealth, spBlackholeNexthop=spBlackholeNexthop, spBlackholePrefix=spBlackholePrefix, deviceDiskUsage=deviceDiskUsage, spInterfaceUsageHC=spInterfaceUsageHC, peakflowSPCMI=peakflowSPCMI, spInterface=spInterface, spCollector=spCollector, spInterfaceSpeed=spInterfaceSpeed, spVersion=spVersion, spFingerprintMessage=spFingerprintMessage, spLicenseErrCount=spLicenseErrCount, spMitigationName=spMitigationName, deviceSystemError=deviceSystemError, spTMSCommunity=spTMSCommunity, spDetector=spDetector, spImpactBps=spImpactBps, spAlertDetectionSignatures=spAlertDetectionSignatures, spApplicationName=spApplicationName, peakflowSPTraps=peakflowSPTraps, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spManagedObjects=spManagedObjects, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, cloudSignalingFault=cloudSignalingFault, spSystemErrorType=spSystemErrorType, spcommFailureDone=spcommFailureDone, mitigationTMSStart=mitigationTMSStart, spUsageType=spUsageType, spBlackholeTimeout=spBlackholeTimeout, reportDone=reportDone, spTMSPrefix=spTMSPrefix, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spLicenseErrType=spLicenseErrType, spInterfaceSpeedHC=spInterfaceSpeedHC, dosNetworkProfiledAlert=dosNetworkProfiledAlert, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, deviceSwapSpace=deviceSwapSpace, spGreTunnelName=spGreTunnelName, fingerprintFeedback=fingerprintFeedback, collectorUp=collectorUp, spBlackholeCommunity=spBlackholeCommunity, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spFingerprintSender=spFingerprintSender, spScriptPort=spScriptPort, bgpInstabilityDone=bgpInstabilityDone, spImpactPps=spImpactPps, peakflowSPMgr=peakflowSPMgr, mitigationDone=mitigationDone, trafficAutoMitigation=trafficAutoMitigation, spUsage=spUsage, dnsBaselineDone=dnsBaselineDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, greDown=greDown, bgpTrap=bgpTrap)
