...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spHijackRoute=spHijackRoute, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, bgpHijack=bgpHijack, spDNSObservedMean=spDNSObservedMean, deviceSwapSpaceUsage=deviceSwapSpaceUsage, mitigationFlowspecStart=mitigationFlowspecStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spScriptHost=spScriptHost, mitigationTMSStartV2=mitigationTMSStartV2, spBlackholeCommunity=spBlackholeCommunity, spCommFailureDescription=spCommFailureDescription, spScriptCommand=spScriptCommand, fingerprintUsage=fingerprintUsage, spFingerprintMessage=spFingerprintMessage, spReportURL=spReportURL, spBlackholePrefix=spBlackholePrefix, bgpInstabilityDone=bgpInstabilityDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spUnit=spUnit, greDown=greDown, spUsername=spUsername, spBGPTrapName=spBGPTrapName, spScriptPort=spScriptPort, hardwareFailure=hardwareFailure, spImpactPps=spImpactPps, spSystemErrorType=spSystemErrorType, spFlowspecCommunity=spFlowspecCommunity, devicePhysicalMemory=devicePhysicalMemory, fingerprintUsageDone=fingerprintUsageDone, spInterface=spInterface, mitigationTMSStart=mitigationTMSStart, spBGPTrapNewAttributes=spBGPTrapNewAttributes, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spThirdPartyAddr=spThirdPartyAddr, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, peakflowSPCMI=peakflowSPCMI, spLicenseErrCount=spLicenseErrCount, spDetectedCountries=spDetectedCountries, reportDone=reportDone, spReportName=spReportName, spBGPTrapPrefix=spBGPTrapPrefix, spBGPInstability=spBGPInstability, spFingerprintName=spFingerprintName, hardwareFailureDone=hardwareFailureDone, spTMSMultiPrefix=spTMSMultiPrefix, licenseErrorDone=licenseErrorDone, spHardwareFailureDescription=spHardwareFailureDescription, interfaceUsageDone=interfaceUsageDone, spGreTunnelName=spGreTunnelName, spHijackLocal=spHijackLocal, spDNSExpected=spDNSExpected, bgpTrap=bgpTrap, spcommFailure=spcommFailure, spManagedObjects=spManagedObjects, spFingerprintSender=spFingerprintSender, fingerprintFeedback=fingerprintFeedback, spCommFailureDestination=spCommFailureDestination, spPravail=spPravail, bgpInstability=bgpInstability, managedObjectUsage=managedObjectUsage, deviceTotalFlowsHC=deviceTotalFlowsHC, cloudSignalingFaultDone=cloudSignalingFaultDone, spInetAddressType=spInetAddressType, dnsBaselineDone=dnsBaselineDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, bgpDown=bgpDown, configChange=configChange, spFlowspecTimeout=spFlowspecTimeout, licenseError=licenseError, spMitigationStart=spMitigationStart, spManagedObject=spManagedObject, spReportID=spReportID, trafficAutoMitigation=trafficAutoMitigation, spImpactBps=spImpactBps, peakflowSPMgr=peakflowSPMgr, interfaceUsage=interfaceUsage, alertScript=alertScript, spLicenseErrLimit=spLicenseErrLimit, bgpUp=bgpUp, spFingerprintFeedback=spFingerprintFeedback, spRouter=spRouter, greDownDone=greDownDone, notificationLimit=notificationLimit, spAlertID=spAlertID, serviceUsage=serviceUsage, spInterfaceSpeed=spInterfaceSpeed, deviceHealth=deviceHealth, spLicenseErrDescription=spLicenseErrDescription, spcommFailureDone=spcommFailureDone, snmpDown=snmpDown, spInterfaceUsage=spInterfaceUsage, spUsage=spUsage, spApplicationName=spApplicationName, collectorStart=collectorStart, spTMSCommunity=spTMSCommunity, spThreshold=spThreshold, spMitigationID=spMitigationID, mitigationBlackholeStart=mitigationBlackholeStart, spGreTunnelDestination=spGreTunnelDestination, spInterfaceIndex=spInterfaceIndex, spSystemErrorDescription=spSystemErrorDescription, spInterfaceUsageHC=spInterfaceUsageHC, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceSystemError=deviceSystemError, spDetector=spDetector, fingerprintReceive=fingerprintReceive, spBlackholeNexthop=spBlackholeNexthop, serviceUsageDone=serviceUsageDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, routingFailover=routingFailover, snmpUp=snmpUp, spDNSObservedMax=spDNSObservedMax, spHijackAttr=spHijackAttr, spBlackholeTimeout=spBlackholeTimeout, dnsBaseline=dnsBaseline, spManagedObjectFamily=spManagedObjectFamily, spTMSPrefix=spTMSPrefix, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, mitigationThirdPartyStart=mitigationThirdPartyStart, spMitigationName=spMitigationName, autoclassifyStart=autoclassifyStart, flowUp=flowUp, spUsageType=spUsageType, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceSwapSpace=deviceSwapSpace, PYSNMP_MODULE_ID=peakflowSPMIB, spLicenseErrType=spLicenseErrType, deviceSwapSpaceInUse=deviceSwapSpaceInUse, mitigationDone=mitigationDone, spCollector=spCollector, spDNSObserved=spDNSObserved, spScriptStart=spScriptStart, flowDown=flowDown, deviceTotalFlows=deviceTotalFlows, dosHostDetectionAlert=dosHostDetectionAlert, spServiceName=spServiceName, spInterfaceSpeedHC=spInterfaceSpeedHC, spCloudSignalFaultDescription=spCloudSignalFaultDescription, peakflowSPTraps=peakflowSPTraps, peakflowSPMIB=peakflowSPMIB, spThirdPartyZone=spThirdPartyZone, spVersion=spVersion, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, managedObjectUsageDone=managedObjectUsageDone, collectorUp=collectorUp, spDuration=spDuration, spBGPEvent=spBGPEvent, deviceSystemErrorDone=deviceSystemErrorDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceDiskUsage=deviceDiskUsage, spServiceElement=spServiceElement, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, cloudSignalingFault=cloudSignalingFault, spInetAddress=spInetAddress, spDNSName=spDNSName, spBGPTrapEvent=spBGPTrapEvent, spTMSTimeout=spTMSTimeout, collectorDown=collectorDown)
