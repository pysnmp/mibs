...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInetAddress=spInetAddress, peakflowSPCMI=peakflowSPCMI, spBlackholeCommunity=spBlackholeCommunity, bgpHijack=bgpHijack, spFingerprintFeedback=spFingerprintFeedback, spThreshold=spThreshold, spMitigationName=spMitigationName, spDNSObserved=spDNSObserved, spCollector=spCollector, spUsageType=spUsageType, spDNSExpected=spDNSExpected, spLicenseErrDescription=spLicenseErrDescription, peakflowSPMgr=peakflowSPMgr, spUnit=spUnit, spReportURL=spReportURL, mitigationFlowspecStart=mitigationFlowspecStart, spGreTunnelName=spGreTunnelName, spVersion=spVersion, spFlowspecCommunity=spFlowspecCommunity, licenseErrorDone=licenseErrorDone, mitigationTMSStart=mitigationTMSStart, spUsername=spUsername, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, bgpUp=bgpUp, spBGPInstability=spBGPInstability, deviceDiskUsage=deviceDiskUsage, mitigationBlackholeStart=mitigationBlackholeStart, spAlertDetectionSignatures=spAlertDetectionSignatures, spInterfaceUsageHC=spInterfaceUsageHC, spManagedObject=spManagedObject, cloudSignalingFault=cloudSignalingFault, greDownDone=greDownDone, mitigationTMSStartV2=mitigationTMSStartV2, spBlackholeNexthop=spBlackholeNexthop, spServiceName=spServiceName, spFlowspecTimeout=spFlowspecTimeout, spAlertID=spAlertID, spScriptCommand=spScriptCommand, deviceHealth=deviceHealth, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spScriptPort=spScriptPort, spSystemErrorDescription=spSystemErrorDescription, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spHijackLocal=spHijackLocal, spInterfaceSpeedHC=spInterfaceSpeedHC, spDuration=spDuration, peakflowSPTraps=peakflowSPTraps, spThirdPartyAddr=spThirdPartyAddr, spDetector=spDetector, spTMSCommunity=spTMSCommunity, flowDown=flowDown, spTMSMultiPrefix=spTMSMultiPrefix, cloudSignalingFaultDone=cloudSignalingFaultDone, snmpUp=snmpUp, fingerprintReceive=fingerprintReceive, deviceTotalFlowsHC=deviceTotalFlowsHC, peakflowSPMIB=peakflowSPMIB, spCommFailureDescription=spCommFailureDescription, spInetAddressType=spInetAddressType, interfaceUsage=interfaceUsage, spHardwareFailureDescription=spHardwareFailureDescription, autoclassifyStart=autoclassifyStart, spInterfaceIndex=spInterfaceIndex, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFingerprintSender=spFingerprintSender, spcommFailure=spcommFailure, spImpactBps=spImpactBps, deviceSwapSpace=deviceSwapSpace, interfaceUsageDone=interfaceUsageDone, spBGPTrapName=spBGPTrapName, spUsage=spUsage, bgpInstability=bgpInstability, spBGPEvent=spBGPEvent, managedObjectUsage=managedObjectUsage, spLicenseErrType=spLicenseErrType, PYSNMP_MODULE_ID=peakflowSPMIB, spManagedObjects=spManagedObjects, spImpactPps=spImpactPps, configChange=configChange, alertScript=alertScript, spHijackRoute=spHijackRoute, spLicenseErrCount=spLicenseErrCount, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceSystemErrorDone=deviceSystemErrorDone, bgpDown=bgpDown, fingerprintFeedback=fingerprintFeedback, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBlackholePrefix=spBlackholePrefix, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceTotalFlows=deviceTotalFlows, collectorStart=collectorStart, snmpDown=snmpDown, greDown=greDown, spBGPTrapPrefix=spBGPTrapPrefix, dnsBaseline=dnsBaseline, deviceSwapSpaceInUse=deviceSwapSpaceInUse, bgpTrap=bgpTrap, hardwareFailure=hardwareFailure, fingerprintUsage=fingerprintUsage, dnsBaselineDone=dnsBaselineDone, spDNSName=spDNSName, spTMSTimeout=spTMSTimeout, spGreTunnelDestination=spGreTunnelDestination, spcommFailureDone=spcommFailureDone, trafficAutoMitigation=trafficAutoMitigation, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, bgpInstabilityDone=bgpInstabilityDone, routingFailover=routingFailover, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spMitigationID=spMitigationID, spScriptStart=spScriptStart, spReportID=spReportID, dosHostDetectionAlertDone=dosHostDetectionAlertDone, notificationLimit=notificationLimit, spReportName=spReportName, spFingerprintMessage=spFingerprintMessage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spRouter=spRouter, spInterfaceUsage=spInterfaceUsage, spPravail=spPravail, spScriptHost=spScriptHost, licenseError=licenseError, spInterface=spInterface, devicePhysicalMemory=devicePhysicalMemory, spBGPTrapEvent=spBGPTrapEvent, spHijackAttr=spHijackAttr, spCloudSignalFaultDescription=spCloudSignalFaultDescription, flowUp=flowUp, deviceSystemError=deviceSystemError, dosHostDetectionAlert=dosHostDetectionAlert, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spInterfaceSpeed=spInterfaceSpeed, spBGPTrapNewAttributes=spBGPTrapNewAttributes, serviceUsageDone=serviceUsageDone, spSystemErrorType=spSystemErrorType, spLicenseErrLimit=spLicenseErrLimit, spServiceElement=spServiceElement, spTMSPrefix=spTMSPrefix, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spMitigationStart=spMitigationStart, spCommFailureDestination=spCommFailureDestination, managedObjectUsageDone=managedObjectUsageDone, spFingerprintName=spFingerprintName, collectorUp=collectorUp, serviceUsage=serviceUsage, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spManagedObjectFamily=spManagedObjectFamily, reportDone=reportDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSObservedMean=spDNSObservedMean, spApplicationName=spApplicationName, hardwareFailureDone=hardwareFailureDone, mitigationDone=mitigationDone, fingerprintUsageDone=fingerprintUsageDone, spDNSObservedMax=spDNSObservedMax, spBlackholeTimeout=spBlackholeTimeout, collectorDown=collectorDown, spDetectedCountries=spDetectedCountries, spThirdPartyZone=spThirdPartyZone)
