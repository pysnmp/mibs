...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceUsage=spInterfaceUsage, spHardwareFailureDescription=spHardwareFailureDescription, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, mitigationThirdPartyStart=mitigationThirdPartyStart, fingerprintUsageDone=fingerprintUsageDone, spcommFailureDone=spcommFailureDone, dnsBaseline=dnsBaseline, configChange=configChange, spLicenseErrLimit=spLicenseErrLimit, dosHostDetectionAlertDone=dosHostDetectionAlertDone, dnsBaselineDone=dnsBaselineDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spLicenseErrDescription=spLicenseErrDescription, spHijackRoute=spHijackRoute, spSystemErrorType=spSystemErrorType, spVersion=spVersion, spThreshold=spThreshold, spDNSExpected=spDNSExpected, spBlackholeNexthop=spBlackholeNexthop, bgpInstabilityDone=bgpInstabilityDone, spGreTunnelDestination=spGreTunnelDestination, cloudSignalingFault=cloudSignalingFault, spScriptCommand=spScriptCommand, spHijackLocal=spHijackLocal, spManagedObject=spManagedObject, mitigationTMSStartV2=mitigationTMSStartV2, cloudSignalingFaultDone=cloudSignalingFaultDone, spUsage=spUsage, spTMSTimeout=spTMSTimeout, collectorStart=collectorStart, greDown=greDown, spDuration=spDuration, spImpactBps=spImpactBps, snmpDown=snmpDown, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, licenseError=licenseError, spInetAddressType=spInetAddressType, spScriptStart=spScriptStart, peakflowSPMgr=peakflowSPMgr, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, mitigationFlowspecStart=mitigationFlowspecStart, spCommFailureDestination=spCommFailureDestination, spUsername=spUsername, spLicenseErrType=spLicenseErrType, deviceTotalFlowsHC=deviceTotalFlowsHC, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spMitigationID=spMitigationID, bgpInstability=bgpInstability, collectorUp=collectorUp, routingFailover=routingFailover, deviceSystemError=deviceSystemError, hardwareFailure=hardwareFailure, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInterface=spInterface, bgpHijack=bgpHijack, spMitigationName=spMitigationName, mitigationDone=mitigationDone, mitigationBlackholeStart=mitigationBlackholeStart, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, flowUp=flowUp, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInterfaceSpeedHC=spInterfaceSpeedHC, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spHijackAttr=spHijackAttr, spRouter=spRouter, hardwareFailureDone=hardwareFailureDone, spManagedObjectFamily=spManagedObjectFamily, spImpactPps=spImpactPps, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spCommFailureDescription=spCommFailureDescription, notificationLimit=notificationLimit, flowDown=flowDown, spPravail=spPravail, spBGPInstability=spBGPInstability, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spThirdPartyZone=spThirdPartyZone, spDNSObserved=spDNSObserved, spServiceName=spServiceName, spBGPTrapPrefix=spBGPTrapPrefix, deviceSystemErrorDone=deviceSystemErrorDone, devicePhysicalMemory=devicePhysicalMemory, spInterfaceUsageHC=spInterfaceUsageHC, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, fingerprintReceive=fingerprintReceive, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spBlackholeCommunity=spBlackholeCommunity, spUsageType=spUsageType, snmpUp=snmpUp, fingerprintFeedback=fingerprintFeedback, dosHostDetectionAlert=dosHostDetectionAlert, spReportName=spReportName, spBGPEvent=spBGPEvent, mitigationTMSStart=mitigationTMSStart, spBlackholeTimeout=spBlackholeTimeout, spDNSName=spDNSName, deviceHealth=deviceHealth, spSystemErrorDescription=spSystemErrorDescription, spServiceElement=spServiceElement, spThirdPartyAddr=spThirdPartyAddr, PYSNMP_MODULE_ID=peakflowSPMIB, spFlowspecCommunity=spFlowspecCommunity, spManagedObjects=spManagedObjects, greDownDone=greDownDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, fingerprintUsage=fingerprintUsage, spDNSObservedMax=spDNSObservedMax, reportDone=reportDone, bgpUp=bgpUp, trafficAutoMitigation=trafficAutoMitigation, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spReportURL=spReportURL, spDNSObservedMean=spDNSObservedMean, interfaceUsage=interfaceUsage, interfaceUsageDone=interfaceUsageDone, managedObjectUsageDone=managedObjectUsageDone, peakflowSPCMI=peakflowSPCMI, spFingerprintMessage=spFingerprintMessage, spReportID=spReportID, spCollector=spCollector, spInterfaceIndex=spInterfaceIndex, deviceSwapSpace=deviceSwapSpace, deviceTotalFlows=deviceTotalFlows, alertScript=alertScript, serviceUsage=serviceUsage, peakflowSPTraps=peakflowSPTraps, collectorDown=collectorDown, spDetectedCountries=spDetectedCountries, spBlackholePrefix=spBlackholePrefix, deviceDiskUsage=deviceDiskUsage, managedObjectUsage=managedObjectUsage, spLicenseErrCount=spLicenseErrCount, spFingerprintFeedback=spFingerprintFeedback, spInterfaceSpeed=spInterfaceSpeed, serviceUsageDone=serviceUsageDone, spInetAddress=spInetAddress, autoclassifyStart=autoclassifyStart, spMitigationStart=spMitigationStart, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spFlowspecTimeout=spFlowspecTimeout, bgpDown=bgpDown, bgpTrap=bgpTrap, spApplicationName=spApplicationName, spDetector=spDetector, peakflowSPMIB=peakflowSPMIB, spUnit=spUnit, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, licenseErrorDone=licenseErrorDone, spBGPTrapName=spBGPTrapName, spTMSCommunity=spTMSCommunity, spcommFailure=spcommFailure, spTMSMultiPrefix=spTMSMultiPrefix, spAlertID=spAlertID, spAlertDetectionSignatures=spAlertDetectionSignatures, spScriptPort=spScriptPort, spBGPTrapEvent=spBGPTrapEvent, spFingerprintName=spFingerprintName, spTMSPrefix=spTMSPrefix, spFingerprintSender=spFingerprintSender, spScriptHost=spScriptHost, spGreTunnelName=spGreTunnelName)
