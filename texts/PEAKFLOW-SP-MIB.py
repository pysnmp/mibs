...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBGPTrapNewAttributes=spBGPTrapNewAttributes, spLicenseErrType=spLicenseErrType, deviceDiskUsage=deviceDiskUsage, deviceSystemErrorDone=deviceSystemErrorDone, spTMSTimeout=spTMSTimeout, deviceTotalFlows=deviceTotalFlows, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spScriptStart=spScriptStart, fingerprintReceive=fingerprintReceive, spcommFailureDone=spcommFailureDone, spHijackRoute=spHijackRoute, configChange=configChange, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spFlowspecCommunity=spFlowspecCommunity, mitigationTMSStartV2=mitigationTMSStartV2, spHijackLocal=spHijackLocal, spInterfaceIndex=spInterfaceIndex, spLicenseErrCount=spLicenseErrCount, fingerprintUsage=fingerprintUsage, spCommFailureDestination=spCommFailureDestination, peakflowSPMgr=peakflowSPMgr, spBGPTrapEvent=spBGPTrapEvent, managedObjectUsage=managedObjectUsage, deviceSwapSpaceUsage=deviceSwapSpaceUsage, bgpInstabilityDone=bgpInstabilityDone, spInterface=spInterface, spFingerprintFeedback=spFingerprintFeedback, spHardwareFailureDescription=spHardwareFailureDescription, devicePhysicalMemory=devicePhysicalMemory, reportDone=reportDone, managedObjectUsageDone=managedObjectUsageDone, dnsBaselineDone=dnsBaselineDone, spUsageType=spUsageType, spTMSCommunity=spTMSCommunity, spDetectedCountries=spDetectedCountries, spGreTunnelName=spGreTunnelName, spDNSObservedMax=spDNSObservedMax, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, hardwareFailureDone=hardwareFailureDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, greDownDone=greDownDone, spScriptHost=spScriptHost, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceSystemError=deviceSystemError, bgpInstability=bgpInstability, spTMSMultiPrefix=spTMSMultiPrefix, spAlertDetectionSignatures=spAlertDetectionSignatures, spLicenseErrLimit=spLicenseErrLimit, spFingerprintName=spFingerprintName, spSystemErrorDescription=spSystemErrorDescription, spLicenseErrDescription=spLicenseErrDescription, spInetAddressType=spInetAddressType, mitigationTMSStart=mitigationTMSStart, spReportName=spReportName, spInterfaceSpeed=spInterfaceSpeed, spServiceElement=spServiceElement, spMitigationName=spMitigationName, spHijackAttr=spHijackAttr, fingerprintUsageDone=fingerprintUsageDone, serviceUsage=serviceUsage, spBGPInstability=spBGPInstability, spReportID=spReportID, bgpDown=bgpDown, peakflowSPCMI=peakflowSPCMI, mitigationThirdPartyStart=mitigationThirdPartyStart, spScriptCommand=spScriptCommand, interfaceUsage=interfaceUsage, spBGPTrapPrefix=spBGPTrapPrefix, spImpactBps=spImpactBps, spScriptPort=spScriptPort, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, interfaceUsageDone=interfaceUsageDone, bgpHijack=bgpHijack, snmpDown=snmpDown, fingerprintFeedback=fingerprintFeedback, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, licenseErrorDone=licenseErrorDone, spInterfaceUsage=spInterfaceUsage, collectorDown=collectorDown, spBlackholePrefix=spBlackholePrefix, spAlertID=spAlertID, spUsage=spUsage, cloudSignalingFaultDone=cloudSignalingFaultDone, spServiceName=spServiceName, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, dosHostDetectionAlert=dosHostDetectionAlert, spDNSObservedMean=spDNSObservedMean, spInterfaceSpeedHC=spInterfaceSpeedHC, alertScript=alertScript, serviceUsageDone=serviceUsageDone, spBlackholeTimeout=spBlackholeTimeout, spUnit=spUnit, spCommFailureDescription=spCommFailureDescription, spManagedObjects=spManagedObjects, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spDetector=spDetector, mitigationBlackholeStart=mitigationBlackholeStart, peakflowSPMIB=peakflowSPMIB, hardwareFailure=hardwareFailure, flowDown=flowDown, trafficAutoMitigation=trafficAutoMitigation, spDNSExpected=spDNSExpected, collectorUp=collectorUp, spUsername=spUsername, spInetAddress=spInetAddress, spcommFailure=spcommFailure, spManagedObject=spManagedObject, spDNSObserved=spDNSObserved, cloudSignalingFault=cloudSignalingFault, peakflowSPTraps=peakflowSPTraps, spSystemErrorType=spSystemErrorType, routingFailover=routingFailover, mitigationFlowspecStart=mitigationFlowspecStart, deviceTotalFlowsHC=deviceTotalFlowsHC, snmpUp=snmpUp, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spCollector=spCollector, spBGPTrapName=spBGPTrapName, spVersion=spVersion, dnsBaseline=dnsBaseline, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spPravail=spPravail, autoclassifyStart=autoclassifyStart, spInterfaceUsageHC=spInterfaceUsageHC, collectorStart=collectorStart, spThirdPartyZone=spThirdPartyZone, spFingerprintMessage=spFingerprintMessage, spImpactPps=spImpactPps, PYSNMP_MODULE_ID=peakflowSPMIB, spThreshold=spThreshold, notificationLimit=notificationLimit, deviceHealth=deviceHealth, spBGPEvent=spBGPEvent, spGreTunnelDestination=spGreTunnelDestination, bgpTrap=bgpTrap, spFingerprintSender=spFingerprintSender, flowUp=flowUp, spMitigationID=spMitigationID, licenseError=licenseError, spThirdPartyAddr=spThirdPartyAddr, spBlackholeCommunity=spBlackholeCommunity, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spMitigationStart=spMitigationStart, spApplicationName=spApplicationName, bgpUp=bgpUp, spBlackholeNexthop=spBlackholeNexthop, greDown=greDown, spReportURL=spReportURL, spFlowspecTimeout=spFlowspecTimeout, spDuration=spDuration, spManagedObjectFamily=spManagedObjectFamily, spTMSPrefix=spTMSPrefix, spRouter=spRouter, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceSwapSpace=deviceSwapSpace, mitigationDone=mitigationDone, spDNSName=spDNSName)
