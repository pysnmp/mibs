...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBlackholeCommunity=spBlackholeCommunity, cloudSignalingFault=cloudSignalingFault, deviceSystemError=deviceSystemError, spAlertDetectionSignatures=spAlertDetectionSignatures, spDNSObserved=spDNSObserved, spTMSPrefix=spTMSPrefix, spScriptHost=spScriptHost, snmpUp=snmpUp, deviceTotalFlows=deviceTotalFlows, deviceSwapSpaceInUse=deviceSwapSpaceInUse, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spScriptCommand=spScriptCommand, fingerprintUsageDone=fingerprintUsageDone, spHijackAttr=spHijackAttr, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dnsBaseline=dnsBaseline, spVersion=spVersion, bgpTrap=bgpTrap, spHijackRoute=spHijackRoute, snmpDown=snmpDown, spPravail=spPravail, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, peakflowSPCMI=peakflowSPCMI, spThirdPartyAddr=spThirdPartyAddr, spTMSCommunity=spTMSCommunity, mitigationFlowspecStart=mitigationFlowspecStart, spLicenseErrDescription=spLicenseErrDescription, deviceSwapSpace=deviceSwapSpace, mitigationDone=mitigationDone, spBGPInstability=spBGPInstability, collectorDown=collectorDown, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spCollector=spCollector, reportDone=reportDone, mitigationTMSStart=mitigationTMSStart, spImpactPps=spImpactPps, spMitigationStart=spMitigationStart, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spManagedObject=spManagedObject, bgpHijack=bgpHijack, spUsage=spUsage, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, mitigationBlackholeStart=mitigationBlackholeStart, fingerprintUsage=fingerprintUsage, hardwareFailureDone=hardwareFailureDone, spCommFailureDestination=spCommFailureDestination, spSystemErrorDescription=spSystemErrorDescription, licenseErrorDone=licenseErrorDone, deviceSystemErrorDone=deviceSystemErrorDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, greDown=greDown, spBGPTrapPrefix=spBGPTrapPrefix, PYSNMP_MODULE_ID=peakflowSPMIB, peakflowSPMIB=peakflowSPMIB, devicePhysicalMemory=devicePhysicalMemory, bgpUp=bgpUp, interfaceUsageDone=interfaceUsageDone, spUnit=spUnit, spTMSMultiPrefix=spTMSMultiPrefix, spCommFailureDescription=spCommFailureDescription, spRouter=spRouter, spInterfaceUsage=spInterfaceUsage, spDetectedCountries=spDetectedCountries, spFlowspecCommunity=spFlowspecCommunity, spDetector=spDetector, fingerprintReceive=fingerprintReceive, spHardwareFailureDescription=spHardwareFailureDescription, autoclassifyStart=autoclassifyStart, spFingerprintName=spFingerprintName, routingFailover=routingFailover, collectorUp=collectorUp, spImpactBps=spImpactBps, spManagedObjects=spManagedObjects, managedObjectUsage=managedObjectUsage, deviceHealth=deviceHealth, spAlertID=spAlertID, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spMitigationID=spMitigationID, serviceUsage=serviceUsage, licenseError=licenseError, configChange=configChange, spReportURL=spReportURL, mitigationThirdPartyStart=mitigationThirdPartyStart, spHijackLocal=spHijackLocal, spServiceName=spServiceName, spFingerprintFeedback=spFingerprintFeedback, collectorStart=collectorStart, spBGPTrapName=spBGPTrapName, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spServiceElement=spServiceElement, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spFingerprintSender=spFingerprintSender, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, cloudSignalingFaultDone=cloudSignalingFaultDone, peakflowSPMgr=peakflowSPMgr, spDNSObservedMax=spDNSObservedMax, bgpDown=bgpDown, spInetAddressType=spInetAddressType, spFingerprintMessage=spFingerprintMessage, spReportID=spReportID, spLicenseErrLimit=spLicenseErrLimit, hardwareFailure=hardwareFailure, interfaceUsage=interfaceUsage, spManagedObjectFamily=spManagedObjectFamily, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spInterfaceSpeed=spInterfaceSpeed, peakflowSPTraps=peakflowSPTraps, spTMSTimeout=spTMSTimeout, spInterfaceUsageHC=spInterfaceUsageHC, alertScript=alertScript, serviceUsageDone=serviceUsageDone, spGreTunnelName=spGreTunnelName, spThirdPartyZone=spThirdPartyZone, mitigationTMSStartV2=mitigationTMSStartV2, spInetAddress=spInetAddress, spScriptStart=spScriptStart, managedObjectUsageDone=managedObjectUsageDone, spReportName=spReportName, spcommFailureDone=spcommFailureDone, spThreshold=spThreshold, spUsageType=spUsageType, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBGPTrapEvent=spBGPTrapEvent, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spInterfaceIndex=spInterfaceIndex, spDNSName=spDNSName, spBlackholePrefix=spBlackholePrefix, spBlackholeTimeout=spBlackholeTimeout, spFlowspecTimeout=spFlowspecTimeout, spApplicationName=spApplicationName, spGreTunnelDestination=spGreTunnelDestination, spScriptPort=spScriptPort, flowDown=flowDown, bgpInstabilityDone=bgpInstabilityDone, spDNSObservedMean=spDNSObservedMean, notificationLimit=notificationLimit, spDuration=spDuration, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, fingerprintFeedback=fingerprintFeedback, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, greDownDone=greDownDone, bgpInstability=bgpInstability, spBGPEvent=spBGPEvent, spInterface=spInterface, spBlackholeNexthop=spBlackholeNexthop, dosHostDetectionAlert=dosHostDetectionAlert, spDNSExpected=spDNSExpected, spSystemErrorType=spSystemErrorType, dnsBaselineDone=dnsBaselineDone, spUsername=spUsername, spcommFailure=spcommFailure, deviceTotalFlowsHC=deviceTotalFlowsHC, spInterfaceSpeedHC=spInterfaceSpeedHC, flowUp=flowUp, spLicenseErrType=spLicenseErrType, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spMitigationName=spMitigationName, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spLicenseErrCount=spLicenseErrCount, trafficAutoMitigation=trafficAutoMitigation, deviceDiskUsage=deviceDiskUsage)
