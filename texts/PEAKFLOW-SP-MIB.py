...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBGPEvent=spBGPEvent, fingerprintReceive=fingerprintReceive, spFingerprintSender=spFingerprintSender, mitigationDone=mitigationDone, spMitigationName=spMitigationName, spTMSTimeout=spTMSTimeout, spDetector=spDetector, spBlackholePrefix=spBlackholePrefix, spInterfaceSpeedHC=spInterfaceSpeedHC, spInetAddressType=spInetAddressType, devicePhysicalMemory=devicePhysicalMemory, mitigationThirdPartyStart=mitigationThirdPartyStart, spcommFailure=spcommFailure, spUnit=spUnit, trafficAutoMitigation=trafficAutoMitigation, PYSNMP_MODULE_ID=peakflowSPMIB, bgpInstabilityDone=bgpInstabilityDone, spPravail=spPravail, collectorDown=collectorDown, spApplicationName=spApplicationName, spManagedObjectFamily=spManagedObjectFamily, snmpUp=snmpUp, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosNetworkProfiledAlert=dosNetworkProfiledAlert, peakflowSPTraps=peakflowSPTraps, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, peakflowSPCMI=peakflowSPCMI, bgpInstability=bgpInstability, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spScriptHost=spScriptHost, spDNSObserved=spDNSObserved, spInetAddress=spInetAddress, spFlowspecTimeout=spFlowspecTimeout, hardwareFailureDone=hardwareFailureDone, spDNSName=spDNSName, spInterface=spInterface, spFingerprintName=spFingerprintName, spThirdPartyZone=spThirdPartyZone, cloudSignalingFault=cloudSignalingFault, spSystemErrorDescription=spSystemErrorDescription, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceSystemErrorDone=deviceSystemErrorDone, spDNSObservedMax=spDNSObservedMax, deviceSystemError=deviceSystemError, deviceSwapSpace=deviceSwapSpace, serviceUsageDone=serviceUsageDone, spBGPTrapPrefix=spBGPTrapPrefix, spManagedObject=spManagedObject, spAlertDetectionSignatures=spAlertDetectionSignatures, spSystemErrorType=spSystemErrorType, mitigationTMSStart=mitigationTMSStart, spHijackLocal=spHijackLocal, spDetectedCountries=spDetectedCountries, spLicenseErrType=spLicenseErrType, spServiceName=spServiceName, bgpTrap=bgpTrap, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, collectorUp=collectorUp, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spcommFailureDone=spcommFailureDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, bgpHijack=bgpHijack, hardwareFailure=hardwareFailure, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spCommFailureDescription=spCommFailureDescription, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, licenseError=licenseError, spInterfaceUsageHC=spInterfaceUsageHC, bgpDown=bgpDown, fingerprintUsage=fingerprintUsage, spScriptCommand=spScriptCommand, spReportURL=spReportURL, spImpactPps=spImpactPps, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spLicenseErrLimit=spLicenseErrLimit, spManagedObjects=spManagedObjects, deviceDiskUsage=deviceDiskUsage, spAlertID=spAlertID, autoclassifyStart=autoclassifyStart, spThirdPartyAddr=spThirdPartyAddr, spInterfaceUsage=spInterfaceUsage, spHijackAttr=spHijackAttr, spServiceElement=spServiceElement, spDuration=spDuration, spInterfaceIndex=spInterfaceIndex, spGreTunnelDestination=spGreTunnelDestination, dosHostDetectionAlertDone=dosHostDetectionAlertDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, mitigationBlackholeStart=mitigationBlackholeStart, dosHostDetectionAlert=dosHostDetectionAlert, spGreTunnelName=spGreTunnelName, spMitigationStart=spMitigationStart, serviceUsage=serviceUsage, spFlowspecCommunity=spFlowspecCommunity, spFingerprintMessage=spFingerprintMessage, mitigationFlowspecStart=mitigationFlowspecStart, routingFailover=routingFailover, spTMSMultiPrefix=spTMSMultiPrefix, dnsBaselineDone=dnsBaselineDone, spUsageType=spUsageType, spDNSExpected=spDNSExpected, snmpDown=snmpDown, spHardwareFailureDescription=spHardwareFailureDescription, fingerprintFeedback=fingerprintFeedback, collectorStart=collectorStart, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spThreshold=spThreshold, spScriptPort=spScriptPort, spBGPTrapName=spBGPTrapName, spUsage=spUsage, spDNSObservedMean=spDNSObservedMean, spReportID=spReportID, spMitigationID=spMitigationID, notificationLimit=notificationLimit, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, configChange=configChange, spImpactBps=spImpactBps, spBGPTrapOldAttributes=spBGPTrapOldAttributes, reportDone=reportDone, mitigationTMSStartV2=mitigationTMSStartV2, spBlackholeTimeout=spBlackholeTimeout, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spScriptStart=spScriptStart, spRouter=spRouter, flowUp=flowUp, deviceTotalFlows=deviceTotalFlows, deviceHealth=deviceHealth, spUsername=spUsername, spLicenseErrCount=spLicenseErrCount, greDownDone=greDownDone, interfaceUsage=interfaceUsage, interfaceUsageDone=interfaceUsageDone, spBGPInstability=spBGPInstability, peakflowSPMIB=peakflowSPMIB, managedObjectUsageDone=managedObjectUsageDone, greDown=greDown, spFingerprintFeedback=spFingerprintFeedback, spBlackholeNexthop=spBlackholeNexthop, spHijackRoute=spHijackRoute, spInterfaceSpeed=spInterfaceSpeed, dnsBaseline=dnsBaseline, fingerprintUsageDone=fingerprintUsageDone, spCommFailureDestination=spCommFailureDestination, deviceTotalFlowsHC=deviceTotalFlowsHC, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spLicenseErrDescription=spLicenseErrDescription, spTMSPrefix=spTMSPrefix, bgpUp=bgpUp, spVersion=spVersion, deviceSwapSpaceInUse=deviceSwapSpaceInUse, managedObjectUsage=managedObjectUsage, spBGPTrapEvent=spBGPTrapEvent, alertScript=alertScript, spCollector=spCollector, spReportName=spReportName, licenseErrorDone=licenseErrorDone, spTMSCommunity=spTMSCommunity, spBlackholeCommunity=spBlackholeCommunity, flowDown=flowDown, peakflowSPMgr=peakflowSPMgr)
