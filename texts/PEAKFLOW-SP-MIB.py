...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spCloudSignalFaultDescription=spCloudSignalFaultDescription, spReportURL=spReportURL, spManagedObject=spManagedObject, spUsageType=spUsageType, spHijackAttr=spHijackAttr, notificationLimit=notificationLimit, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spBlackholeNexthop=spBlackholeNexthop, greDownDone=greDownDone, peakflowSPCMI=peakflowSPCMI, spDNSName=spDNSName, spFlowspecTimeout=spFlowspecTimeout, spcommFailure=spcommFailure, configChange=configChange, spDNSObservedMean=spDNSObservedMean, spDNSObserved=spDNSObserved, serviceUsage=serviceUsage, bgpTrap=bgpTrap, spInterfaceSpeedHC=spInterfaceSpeedHC, spUsername=spUsername, spRouter=spRouter, spBGPTrapPrefix=spBGPTrapPrefix, routingFailover=routingFailover, spScriptStart=spScriptStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spInterfaceIndex=spInterfaceIndex, fingerprintUsage=fingerprintUsage, collectorStart=collectorStart, deviceSwapSpace=deviceSwapSpace, spHijackLocal=spHijackLocal, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterfaceUsage=spInterfaceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, dnsBaselineDone=dnsBaselineDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spServiceName=spServiceName, spTMSCommunity=spTMSCommunity, fingerprintReceive=fingerprintReceive, deviceHealth=deviceHealth, spBGPTrapEvent=spBGPTrapEvent, spcommFailureDone=spcommFailureDone, spDetectedCountries=spDetectedCountries, spTMSTimeout=spTMSTimeout, bgpUp=bgpUp, mitigationThirdPartyStart=mitigationThirdPartyStart, spReportID=spReportID, spAlertDetectionSignatures=spAlertDetectionSignatures, spHijackRoute=spHijackRoute, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBlackholePrefix=spBlackholePrefix, spFlowspecCommunity=spFlowspecCommunity, devicePhysicalMemory=devicePhysicalMemory, licenseError=licenseError, spAlertID=spAlertID, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, bgpInstabilityDone=bgpInstabilityDone, managedObjectUsageDone=managedObjectUsageDone, spLicenseErrLimit=spLicenseErrLimit, spManagedObjectFamily=spManagedObjectFamily, fingerprintFeedback=fingerprintFeedback, spReportName=spReportName, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spVersion=spVersion, flowDown=flowDown, autoclassifyStart=autoclassifyStart, spApplicationName=spApplicationName, spInterfaceSpeed=spInterfaceSpeed, cloudSignalingFaultDone=cloudSignalingFaultDone, collectorUp=collectorUp, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, serviceUsageDone=serviceUsageDone, spBlackholeCommunity=spBlackholeCommunity, spMitigationName=spMitigationName, deviceSystemError=deviceSystemError, cloudSignalingFault=cloudSignalingFault, bgpDown=bgpDown, spHardwareFailureDescription=spHardwareFailureDescription, spMitigationStart=spMitigationStart, spManagedObjects=spManagedObjects, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spLicenseErrType=spLicenseErrType, licenseErrorDone=licenseErrorDone, hardwareFailure=hardwareFailure, spUnit=spUnit, spDNSObservedMax=spDNSObservedMax, fingerprintUsageDone=fingerprintUsageDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, mitigationTMSStartV2=mitigationTMSStartV2, spScriptCommand=spScriptCommand, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBGPTrapName=spBGPTrapName, greDown=greDown, spThreshold=spThreshold, spThirdPartyAddr=spThirdPartyAddr, spTMSPrefix=spTMSPrefix, trafficAutoMitigation=trafficAutoMitigation, spDetector=spDetector, spCommFailureDestination=spCommFailureDestination, spFingerprintSender=spFingerprintSender, spDNSExpected=spDNSExpected, mitigationFlowspecStart=mitigationFlowspecStart, spThirdPartyZone=spThirdPartyZone, alertScript=alertScript, spCollector=spCollector, spScriptPort=spScriptPort, spBGPEvent=spBGPEvent, interfaceUsageDone=interfaceUsageDone, interfaceUsage=interfaceUsage, spLicenseErrDescription=spLicenseErrDescription, hardwareFailureDone=hardwareFailureDone, spFingerprintName=spFingerprintName, spTMSMultiPrefix=spTMSMultiPrefix, snmpUp=snmpUp, deviceDiskUsage=deviceDiskUsage, reportDone=reportDone, spScriptHost=spScriptHost, peakflowSPMIB=peakflowSPMIB, spLicenseErrCount=spLicenseErrCount, deviceSystemErrorDone=deviceSystemErrorDone, peakflowSPMgr=peakflowSPMgr, dosHostDetectionAlert=dosHostDetectionAlert, spImpactBps=spImpactBps, spSystemErrorType=spSystemErrorType, managedObjectUsage=managedObjectUsage, spInetAddress=spInetAddress, collectorDown=collectorDown, deviceTotalFlows=deviceTotalFlows, spServiceElement=spServiceElement, spInetAddressType=spInetAddressType, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spSystemErrorDescription=spSystemErrorDescription, spMitigationID=spMitigationID, bgpHijack=bgpHijack, spGreTunnelDestination=spGreTunnelDestination, spDuration=spDuration, spBlackholeTimeout=spBlackholeTimeout, spImpactPps=spImpactPps, dnsBaseline=dnsBaseline, spFingerprintFeedback=spFingerprintFeedback, bgpInstability=bgpInstability, spPravail=spPravail, spCommFailureDescription=spCommFailureDescription, mitigationTMSStart=mitigationTMSStart, spInterfaceUsageHC=spInterfaceUsageHC, snmpDown=snmpDown, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, mitigationDone=mitigationDone, spBGPInstability=spBGPInstability, spFingerprintMessage=spFingerprintMessage, peakflowSPTraps=peakflowSPTraps, deviceSwapSpaceInUse=deviceSwapSpaceInUse, mitigationBlackholeStart=mitigationBlackholeStart, spUsage=spUsage, flowUp=flowUp, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceTotalFlowsHC=deviceTotalFlowsHC, spInterface=spInterface, spGreTunnelName=spGreTunnelName)
