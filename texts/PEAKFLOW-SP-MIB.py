...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", peakflowSPTraps=peakflowSPTraps, spDNSObserved=spDNSObserved, spBGPTrapPrefix=spBGPTrapPrefix, spInterfaceSpeed=spInterfaceSpeed, bgpHijack=bgpHijack, spInterfaceUsage=spInterfaceUsage, spHardwareFailureDescription=spHardwareFailureDescription, spInterfaceIndex=spInterfaceIndex, spBGPTrapNewAttributes=spBGPTrapNewAttributes, fingerprintFeedback=fingerprintFeedback, peakflowSPMIB=peakflowSPMIB, serviceUsageDone=serviceUsageDone, snmpUp=snmpUp, spBGPEvent=spBGPEvent, spServiceName=spServiceName, spBlackholePrefix=spBlackholePrefix, serviceUsage=serviceUsage, spLicenseErrType=spLicenseErrType, spUsageType=spUsageType, spUnit=spUnit, spFlowspecTimeout=spFlowspecTimeout, spPravail=spPravail, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spRouter=spRouter, spFingerprintMessage=spFingerprintMessage, spInetAddress=spInetAddress, spBGPInstability=spBGPInstability, spThirdPartyAddr=spThirdPartyAddr, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spCommFailureDescription=spCommFailureDescription, cloudSignalingFaultDone=cloudSignalingFaultDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBlackholeNexthop=spBlackholeNexthop, hardwareFailureDone=hardwareFailureDone, reportDone=reportDone, spDNSObservedMean=spDNSObservedMean, mitigationBlackholeStart=mitigationBlackholeStart, spAlertID=spAlertID, interfaceUsage=interfaceUsage, licenseError=licenseError, mitigationDone=mitigationDone, spDetector=spDetector, spCollector=spCollector, spGreTunnelName=spGreTunnelName, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, bgpInstabilityDone=bgpInstabilityDone, spInterface=spInterface, spCommFailureDestination=spCommFailureDestination, spTMSMultiPrefix=spTMSMultiPrefix, spManagedObject=spManagedObject, spDetectedCountries=spDetectedCountries, deviceTotalFlowsHC=deviceTotalFlowsHC, spDNSName=spDNSName, deviceSystemErrorDone=deviceSystemErrorDone, spBlackholeTimeout=spBlackholeTimeout, greDown=greDown, alertScript=alertScript, spLicenseErrCount=spLicenseErrCount, mitigationFlowspecStart=mitigationFlowspecStart, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spApplicationName=spApplicationName, spcommFailureDone=spcommFailureDone, spLicenseErrLimit=spLicenseErrLimit, peakflowSPCMI=peakflowSPCMI, spHijackRoute=spHijackRoute, fingerprintReceive=fingerprintReceive, spBlackholeCommunity=spBlackholeCommunity, deviceSwapSpace=deviceSwapSpace, spDNSExpected=spDNSExpected, mitigationThirdPartyStart=mitigationThirdPartyStart, bgpTrap=bgpTrap, spThreshold=spThreshold, bgpUp=bgpUp, fingerprintUsage=fingerprintUsage, spScriptStart=spScriptStart, bgpDown=bgpDown, spHijackAttr=spHijackAttr, spFingerprintSender=spFingerprintSender, spImpactPps=spImpactPps, spManagedObjects=spManagedObjects, cloudSignalingFault=cloudSignalingFault, spMitigationName=spMitigationName, deviceSystemError=deviceSystemError, managedObjectUsage=managedObjectUsage, spTMSCommunity=spTMSCommunity, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spManagedObjectFamily=spManagedObjectFamily, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, managedObjectUsageDone=managedObjectUsageDone, spSystemErrorDescription=spSystemErrorDescription, spFlowspecCommunity=spFlowspecCommunity, flowDown=flowDown, spFingerprintName=spFingerprintName, dnsBaselineDone=dnsBaselineDone, spLicenseErrDescription=spLicenseErrDescription, spTMSTimeout=spTMSTimeout, mitigationTMSStart=mitigationTMSStart, collectorStart=collectorStart, devicePhysicalMemory=devicePhysicalMemory, deviceDiskUsage=deviceDiskUsage, spCloudSignalFaultDescription=spCloudSignalFaultDescription, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spReportName=spReportName, greDownDone=greDownDone, spDuration=spDuration, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spInterfaceSpeedHC=spInterfaceSpeedHC, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spMitigationStart=spMitigationStart, licenseErrorDone=licenseErrorDone, routingFailover=routingFailover, bgpInstability=bgpInstability, spHijackLocal=spHijackLocal, mitigationTMSStartV2=mitigationTMSStartV2, dosHostDetectionAlert=dosHostDetectionAlert, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spVersion=spVersion, spUsage=spUsage, spImpactBps=spImpactBps, configChange=configChange, spFingerprintFeedback=spFingerprintFeedback, hardwareFailure=hardwareFailure, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spScriptHost=spScriptHost, spDNSObservedMax=spDNSObservedMax, interfaceUsageDone=interfaceUsageDone, spServiceElement=spServiceElement, dosHostDetectionAlertDone=dosHostDetectionAlertDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, trafficAutoMitigation=trafficAutoMitigation, collectorDown=collectorDown, spcommFailure=spcommFailure, spBGPTrapName=spBGPTrapName, notificationLimit=notificationLimit, spReportURL=spReportURL, spMitigationID=spMitigationID, spThirdPartyZone=spThirdPartyZone, flowUp=flowUp, PYSNMP_MODULE_ID=peakflowSPMIB, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceTotalFlows=deviceTotalFlows, spInterfaceUsageHC=spInterfaceUsageHC, deviceHealth=deviceHealth, spTMSPrefix=spTMSPrefix, spInetAddressType=spInetAddressType, spScriptCommand=spScriptCommand, autoclassifyStart=autoclassifyStart, spGreTunnelDestination=spGreTunnelDestination, snmpDown=snmpDown, spUsername=spUsername, spReportID=spReportID, peakflowSPMgr=peakflowSPMgr, spAlertDetectionSignatures=spAlertDetectionSignatures, dnsBaseline=dnsBaseline, spBGPTrapEvent=spBGPTrapEvent, collectorUp=collectorUp, fingerprintUsageDone=fingerprintUsageDone, spScriptPort=spScriptPort, spSystemErrorType=spSystemErrorType)
