...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spManagedObject=spManagedObject, spTMSMultiPrefix=spTMSMultiPrefix, spTMSTimeout=spTMSTimeout, deviceDiskUsage=deviceDiskUsage, spBGPEvent=spBGPEvent, spLicenseErrType=spLicenseErrType, deviceTotalFlows=deviceTotalFlows, deviceSystemErrorDone=deviceSystemErrorDone, spUsageType=spUsageType, spBGPTrapNewAttributes=spBGPTrapNewAttributes, peakflowSPMgr=peakflowSPMgr, mitigationBlackholeStart=mitigationBlackholeStart, spDetectedCountries=spDetectedCountries, spRouter=spRouter, fingerprintUsage=fingerprintUsage, spBlackholeCommunity=spBlackholeCommunity, snmpDown=snmpDown, peakflowSPMIB=peakflowSPMIB, spUsername=spUsername, peakflowSPCMI=peakflowSPCMI, spHardwareFailureDescription=spHardwareFailureDescription, licenseError=licenseError, spServiceName=spServiceName, mitigationTMSStartV2=mitigationTMSStartV2, spScriptHost=spScriptHost, spScriptPort=spScriptPort, bgpDown=bgpDown, hardwareFailure=hardwareFailure, mitigationFlowspecStart=mitigationFlowspecStart, spcommFailure=spcommFailure, spPravail=spPravail, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spImpactBps=spImpactBps, serviceUsage=serviceUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, fingerprintFeedback=fingerprintFeedback, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spMitigationStart=spMitigationStart, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, dnsBaseline=dnsBaseline, spReportName=spReportName, spMitigationName=spMitigationName, deviceTotalFlowsHC=deviceTotalFlowsHC, spUnit=spUnit, spImpactPps=spImpactPps, spReportURL=spReportURL, spCollector=spCollector, spAlertDetectionSignatures=spAlertDetectionSignatures, spLicenseErrLimit=spLicenseErrLimit, flowUp=flowUp, serviceUsageDone=serviceUsageDone, spFingerprintMessage=spFingerprintMessage, spFingerprintFeedback=spFingerprintFeedback, mitigationThirdPartyStart=mitigationThirdPartyStart, spSystemErrorDescription=spSystemErrorDescription, spScriptCommand=spScriptCommand, spReportID=spReportID, mitigationDone=mitigationDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spFingerprintSender=spFingerprintSender, deviceHealth=deviceHealth, managedObjectUsageDone=managedObjectUsageDone, spTMSCommunity=spTMSCommunity, spBGPTrapPrefix=spBGPTrapPrefix, interfaceUsageDone=interfaceUsageDone, spcommFailureDone=spcommFailureDone, collectorDown=collectorDown, deviceSwapSpace=deviceSwapSpace, spHijackAttr=spHijackAttr, spLicenseErrCount=spLicenseErrCount, spBGPTrapEvent=spBGPTrapEvent, licenseErrorDone=licenseErrorDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, peakflowSPTraps=peakflowSPTraps, spAlertID=spAlertID, spBGPTrapName=spBGPTrapName, managedObjectUsage=managedObjectUsage, spInetAddressType=spInetAddressType, collectorStart=collectorStart, spThirdPartyZone=spThirdPartyZone, spLicenseErrDescription=spLicenseErrDescription, spInterface=spInterface, spHijackRoute=spHijackRoute, spApplicationName=spApplicationName, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, autoclassifyStart=autoclassifyStart, deviceSystemError=deviceSystemError, bgpInstability=bgpInstability, spBlackholePrefix=spBlackholePrefix, spUsage=spUsage, reportDone=reportDone, greDown=greDown, dosHostDetectionAlert=dosHostDetectionAlert, trafficAutoMitigation=trafficAutoMitigation, spThreshold=spThreshold, spDuration=spDuration, spDNSObserved=spDNSObserved, cloudSignalingFaultDone=cloudSignalingFaultDone, configChange=configChange, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spDNSObservedMean=spDNSObservedMean, spHijackLocal=spHijackLocal, spDNSName=spDNSName, spGreTunnelName=spGreTunnelName, bgpHijack=bgpHijack, spGreTunnelDestination=spGreTunnelDestination, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, bgpInstabilityDone=bgpInstabilityDone, spFlowspecTimeout=spFlowspecTimeout, interfaceUsage=interfaceUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spMitigationID=spMitigationID, snmpUp=snmpUp, mitigationTMSStart=mitigationTMSStart, notificationLimit=notificationLimit, spScriptStart=spScriptStart, spDNSExpected=spDNSExpected, spBlackholeNexthop=spBlackholeNexthop, devicePhysicalMemory=devicePhysicalMemory, spSystemErrorType=spSystemErrorType, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, greDownDone=greDownDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spThirdPartyAddr=spThirdPartyAddr, spDetector=spDetector, spFingerprintName=spFingerprintName, fingerprintReceive=fingerprintReceive, collectorUp=collectorUp, spManagedObjects=spManagedObjects, fingerprintUsageDone=fingerprintUsageDone, routingFailover=routingFailover, spCloudSignalFaultDescription=spCloudSignalFaultDescription, alertScript=alertScript, bgpTrap=bgpTrap, dnsBaselineDone=dnsBaselineDone, spManagedObjectFamily=spManagedObjectFamily, spInterfaceUsageHC=spInterfaceUsageHC, hardwareFailureDone=hardwareFailureDone, spDNSObservedMax=spDNSObservedMax, spCommFailureDescription=spCommFailureDescription, spBlackholeTimeout=spBlackholeTimeout, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spInterfaceIndex=spInterfaceIndex, bgpUp=bgpUp, dosHostDetectionAlertDone=dosHostDetectionAlertDone, flowDown=flowDown, spFlowspecCommunity=spFlowspecCommunity, spTMSPrefix=spTMSPrefix, spInterfaceUsage=spInterfaceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, spCommFailureDestination=spCommFailureDestination, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterfaceSpeed=spInterfaceSpeed, cloudSignalingFault=cloudSignalingFault, spServiceElement=spServiceElement, spBGPInstability=spBGPInstability, spInetAddress=spInetAddress, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spVersion=spVersion, deviceSwapSpaceUsage=deviceSwapSpaceUsage)
