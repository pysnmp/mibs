...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceSwapSpace=deviceSwapSpace, spInterfaceSpeed=spInterfaceSpeed, hardwareFailure=hardwareFailure, spManagedObjectFamily=spManagedObjectFamily, spAlertDetectionSignatures=spAlertDetectionSignatures, hardwareFailureDone=hardwareFailureDone, spReportURL=spReportURL, licenseError=licenseError, spLicenseErrCount=spLicenseErrCount, bgpTrap=bgpTrap, spImpactBps=spImpactBps, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBGPTrapEvent=spBGPTrapEvent, peakflowSPTraps=peakflowSPTraps, spDNSObservedMax=spDNSObservedMax, deviceHealth=deviceHealth, spThreshold=spThreshold, spLicenseErrDescription=spLicenseErrDescription, spCollector=spCollector, snmpDown=snmpDown, spInterfaceUsageHC=spInterfaceUsageHC, bgpDown=bgpDown, deviceDiskUsage=deviceDiskUsage, collectorStart=collectorStart, spManagedObjects=spManagedObjects, spDNSObserved=spDNSObserved, deviceSystemError=deviceSystemError, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spUsername=spUsername, licenseErrorDone=licenseErrorDone, bgpHijack=bgpHijack, spDNSExpected=spDNSExpected, peakflowSPCMI=peakflowSPCMI, fingerprintFeedback=fingerprintFeedback, spUsageType=spUsageType, collectorUp=collectorUp, spServiceElement=spServiceElement, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInetAddress=spInetAddress, spBGPTrapPrefix=spBGPTrapPrefix, spFingerprintName=spFingerprintName, spMitigationName=spMitigationName, spInterface=spInterface, spTMSTimeout=spTMSTimeout, spManagedObject=spManagedObject, greDown=greDown, spBGPEvent=spBGPEvent, peakflowSPMgr=peakflowSPMgr, peakflowSPMIB=peakflowSPMIB, spSystemErrorDescription=spSystemErrorDescription, spBlackholeNexthop=spBlackholeNexthop, fingerprintReceive=fingerprintReceive, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceTotalFlowsHC=deviceTotalFlowsHC, spTMSPrefix=spTMSPrefix, bgpInstability=bgpInstability, fingerprintUsage=fingerprintUsage, spBGPInstability=spBGPInstability, alertScript=alertScript, spLicenseErrLimit=spLicenseErrLimit, spServiceName=spServiceName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceSystemErrorDone=deviceSystemErrorDone, spBlackholeTimeout=spBlackholeTimeout, spcommFailure=spcommFailure, spInterfaceIndex=spInterfaceIndex, dnsBaseline=dnsBaseline, spTMSCommunity=spTMSCommunity, interfaceUsageDone=interfaceUsageDone, spThirdPartyAddr=spThirdPartyAddr, spMitigationStart=spMitigationStart, mitigationTMSStartV2=mitigationTMSStartV2, deviceTotalFlows=deviceTotalFlows, bgpUp=bgpUp, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spImpactPps=spImpactPps, flowUp=flowUp, mitigationThirdPartyStart=mitigationThirdPartyStart, spFlowspecTimeout=spFlowspecTimeout, spGreTunnelName=spGreTunnelName, mitigationBlackholeStart=mitigationBlackholeStart, cloudSignalingFaultDone=cloudSignalingFaultDone, autoclassifyStart=autoclassifyStart, spScriptCommand=spScriptCommand, spHardwareFailureDescription=spHardwareFailureDescription, spCommFailureDescription=spCommFailureDescription, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, snmpUp=snmpUp, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFingerprintMessage=spFingerprintMessage, spcommFailureDone=spcommFailureDone, spReportID=spReportID, serviceUsage=serviceUsage, spUsage=spUsage, spDuration=spDuration, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spInetAddressType=spInetAddressType, spLicenseErrType=spLicenseErrType, spFingerprintSender=spFingerprintSender, cloudSignalingFault=cloudSignalingFault, spVersion=spVersion, spUnit=spUnit, spHijackAttr=spHijackAttr, greDownDone=greDownDone, spScriptHost=spScriptHost, collectorDown=collectorDown, configChange=configChange, notificationLimit=notificationLimit, serviceUsageDone=serviceUsageDone, spScriptStart=spScriptStart, spFlowspecCommunity=spFlowspecCommunity, spDNSObservedMean=spDNSObservedMean, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInterfaceSpeedHC=spInterfaceSpeedHC, interfaceUsage=interfaceUsage, spHijackLocal=spHijackLocal, trafficAutoMitigation=trafficAutoMitigation, dnsBaselineDone=dnsBaselineDone, spBlackholeCommunity=spBlackholeCommunity, fingerprintUsageDone=fingerprintUsageDone, spDetector=spDetector, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, mitigationTMSStart=mitigationTMSStart, spRouter=spRouter, devicePhysicalMemory=devicePhysicalMemory, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spApplicationName=spApplicationName, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, managedObjectUsage=managedObjectUsage, deviceSwapSpaceUsage=deviceSwapSpaceUsage, managedObjectUsageDone=managedObjectUsageDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spDetectedCountries=spDetectedCountries, reportDone=reportDone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBlackholePrefix=spBlackholePrefix, bgpInstabilityDone=bgpInstabilityDone, spBGPTrapName=spBGPTrapName, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spHijackRoute=spHijackRoute, spCommFailureDestination=spCommFailureDestination, spPravail=spPravail, spAlertID=spAlertID, flowDown=flowDown, spMitigationID=spMitigationID, spReportName=spReportName, spTMSMultiPrefix=spTMSMultiPrefix, routingFailover=routingFailover, mitigationFlowspecStart=mitigationFlowspecStart, spDNSName=spDNSName, spScriptPort=spScriptPort, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spGreTunnelDestination=spGreTunnelDestination, mitigationDone=mitigationDone, dosHostDetectionAlert=dosHostDetectionAlert, spFingerprintFeedback=spFingerprintFeedback, PYSNMP_MODULE_ID=peakflowSPMIB, spSystemErrorType=spSystemErrorType, spInterfaceUsage=spInterfaceUsage, spThirdPartyZone=spThirdPartyZone)
