...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spScriptPort=spScriptPort, spTMSPrefix=spTMSPrefix, spReportURL=spReportURL, collectorUp=collectorUp, spBGPTrapPrefix=spBGPTrapPrefix, spBGPTrapEvent=spBGPTrapEvent, spHijackLocal=spHijackLocal, spManagedObject=spManagedObject, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spDNSName=spDNSName, spFingerprintMessage=spFingerprintMessage, routingFailover=routingFailover, spDuration=spDuration, spDNSObservedMax=spDNSObservedMax, spGreTunnelDestination=spGreTunnelDestination, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spBlackholeCommunity=spBlackholeCommunity, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceTotalFlows=deviceTotalFlows, spScriptCommand=spScriptCommand, dnsBaseline=dnsBaseline, cloudSignalingFault=cloudSignalingFault, mitigationTMSStart=mitigationTMSStart, flowUp=flowUp, spCommFailureDescription=spCommFailureDescription, spDetectedCountries=spDetectedCountries, spLicenseErrDescription=spLicenseErrDescription, fingerprintUsage=fingerprintUsage, spApplicationName=spApplicationName, interfaceUsage=interfaceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, spImpactBps=spImpactBps, spDNSExpected=spDNSExpected, spHijackAttr=spHijackAttr, peakflowSPCMI=peakflowSPCMI, deviceHealth=deviceHealth, spBlackholePrefix=spBlackholePrefix, spAlertID=spAlertID, deviceSwapSpace=deviceSwapSpace, spInterfaceIndex=spInterfaceIndex, spInetAddressType=spInetAddressType, spVersion=spVersion, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInetAddress=spInetAddress, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spImpactPps=spImpactPps, managedObjectUsageDone=managedObjectUsageDone, spRouter=spRouter, fingerprintFeedback=fingerprintFeedback, spDNSObserved=spDNSObserved, interfaceUsageDone=interfaceUsageDone, spInterfaceSpeedHC=spInterfaceSpeedHC, spDetector=spDetector, bgpInstability=bgpInstability, spBGPInstability=spBGPInstability, managedObjectUsage=managedObjectUsage, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, bgpUp=bgpUp, spPravail=spPravail, peakflowSPMgr=peakflowSPMgr, fingerprintReceive=fingerprintReceive, spTMSMultiPrefix=spTMSMultiPrefix, collectorDown=collectorDown, spcommFailureDone=spcommFailureDone, spServiceElement=spServiceElement, dosHostDetectionAlert=dosHostDetectionAlert, snmpUp=snmpUp, spCollector=spCollector, spFlowspecTimeout=spFlowspecTimeout, spManagedObjects=spManagedObjects, hardwareFailureDone=hardwareFailureDone, flowDown=flowDown, spcommFailure=spcommFailure, spBGPTrapName=spBGPTrapName, spSystemErrorType=spSystemErrorType, bgpHijack=bgpHijack, peakflowSPMIB=peakflowSPMIB, spFlowspecCommunity=spFlowspecCommunity, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, configChange=configChange, spBlackholeNexthop=spBlackholeNexthop, fingerprintUsageDone=fingerprintUsageDone, deviceDiskUsage=deviceDiskUsage, autoclassifyStart=autoclassifyStart, snmpDown=snmpDown, spBlackholeTimeout=spBlackholeTimeout, spGreTunnelName=spGreTunnelName, mitigationDone=mitigationDone, spLicenseErrCount=spLicenseErrCount, mitigationBlackholeStart=mitigationBlackholeStart, spInterfaceUsage=spInterfaceUsage, deviceSystemErrorDone=deviceSystemErrorDone, spInterface=spInterface, deviceTotalFlowsHC=deviceTotalFlowsHC, licenseErrorDone=licenseErrorDone, alertScript=alertScript, spUnit=spUnit, mitigationThirdPartyStart=mitigationThirdPartyStart, licenseError=licenseError, mitigationTMSStartV2=mitigationTMSStartV2, spAlertDetectionSignatures=spAlertDetectionSignatures, spInterfaceSpeed=spInterfaceSpeed, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spFingerprintName=spFingerprintName, spServiceName=spServiceName, spTMSCommunity=spTMSCommunity, spTMSTimeout=spTMSTimeout, bgpTrap=bgpTrap, spUsage=spUsage, spFingerprintSender=spFingerprintSender, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spHardwareFailureDescription=spHardwareFailureDescription, deviceSystemError=deviceSystemError, spThirdPartyZone=spThirdPartyZone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, bgpInstabilityDone=bgpInstabilityDone, bgpDown=bgpDown, spScriptStart=spScriptStart, reportDone=reportDone, spCommFailureDestination=spCommFailureDestination, mitigationFlowspecStart=mitigationFlowspecStart, devicePhysicalMemory=devicePhysicalMemory, spSystemErrorDescription=spSystemErrorDescription, spFingerprintFeedback=spFingerprintFeedback, spBGPEvent=spBGPEvent, spManagedObjectFamily=spManagedObjectFamily, spScriptHost=spScriptHost, spMitigationID=spMitigationID, greDownDone=greDownDone, collectorStart=collectorStart, spHijackRoute=spHijackRoute, notificationLimit=notificationLimit, spThirdPartyAddr=spThirdPartyAddr, dnsBaselineDone=dnsBaselineDone, serviceUsage=serviceUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spMitigationName=spMitigationName, spMitigationStart=spMitigationStart, serviceUsageDone=serviceUsageDone, spReportID=spReportID, spLicenseErrLimit=spLicenseErrLimit, spLicenseErrType=spLicenseErrType, trafficAutoMitigation=trafficAutoMitigation, peakflowSPTraps=peakflowSPTraps, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spUsageType=spUsageType, spThreshold=spThreshold, spReportName=spReportName, hardwareFailure=hardwareFailure, spInterfaceUsageHC=spInterfaceUsageHC, spUsername=spUsername, greDown=greDown, spDNSObservedMean=spDNSObservedMean, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse)
