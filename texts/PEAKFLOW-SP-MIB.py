...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceUsage=spInterfaceUsage, spUsage=spUsage, bgpTrap=bgpTrap, collectorUp=collectorUp, spBlackholeTimeout=spBlackholeTimeout, collectorStart=collectorStart, deviceTotalFlows=deviceTotalFlows, spLicenseErrLimit=spLicenseErrLimit, spVersion=spVersion, spFingerprintFeedback=spFingerprintFeedback, mitigationDone=mitigationDone, deviceSwapSpace=deviceSwapSpace, spAlertID=spAlertID, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spMitigationName=spMitigationName, mitigationTMSStart=mitigationTMSStart, managedObjectUsage=managedObjectUsage, spManagedObjects=spManagedObjects, spDetectedCountries=spDetectedCountries, snmpDown=snmpDown, spInetAddressType=spInetAddressType, spManagedObject=spManagedObject, spServiceElement=spServiceElement, spInetAddress=spInetAddress, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, deviceTotalFlowsHC=deviceTotalFlowsHC, spReportID=spReportID, spDetector=spDetector, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spImpactBps=spImpactBps, cloudSignalingFault=cloudSignalingFault, dnsBaseline=dnsBaseline, spDNSExpected=spDNSExpected, spServiceName=spServiceName, interfaceUsageDone=interfaceUsageDone, spDNSName=spDNSName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, autoclassifyStart=autoclassifyStart, peakflowSPTraps=peakflowSPTraps, hardwareFailureDone=hardwareFailureDone, managedObjectUsageDone=managedObjectUsageDone, spAlertDetectionSignatures=spAlertDetectionSignatures, licenseError=licenseError, deviceHealth=deviceHealth, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, configChange=configChange, dosHostDetectionAlertDone=dosHostDetectionAlertDone, greDownDone=greDownDone, spHijackAttr=spHijackAttr, fingerprintReceive=fingerprintReceive, spGreTunnelDestination=spGreTunnelDestination, spTMSMultiPrefix=spTMSMultiPrefix, deviceDiskUsage=deviceDiskUsage, spBGPTrapPrefix=spBGPTrapPrefix, spUsername=spUsername, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spInterfaceIndex=spInterfaceIndex, spReportName=spReportName, spBGPInstability=spBGPInstability, spDuration=spDuration, spTMSTimeout=spTMSTimeout, spApplicationName=spApplicationName, mitigationBlackholeStart=mitigationBlackholeStart, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBlackholeNexthop=spBlackholeNexthop, deviceSystemError=deviceSystemError, bgpDown=bgpDown, PYSNMP_MODULE_ID=peakflowSPMIB, spThreshold=spThreshold, routingFailover=routingFailover, bgpInstabilityDone=bgpInstabilityDone, spUnit=spUnit, bgpUp=bgpUp, dosHostDetectionAlert=dosHostDetectionAlert, spTMSCommunity=spTMSCommunity, peakflowSPCMI=peakflowSPCMI, deviceSystemErrorDone=deviceSystemErrorDone, serviceUsageDone=serviceUsageDone, devicePhysicalMemory=devicePhysicalMemory, spInterfaceSpeedHC=spInterfaceSpeedHC, spLicenseErrDescription=spLicenseErrDescription, spThirdPartyZone=spThirdPartyZone, fingerprintUsageDone=fingerprintUsageDone, spBlackholeCommunity=spBlackholeCommunity, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBGPTrapName=spBGPTrapName, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spScriptCommand=spScriptCommand, spManagedObjectFamily=spManagedObjectFamily, spImpactPps=spImpactPps, mitigationTMSStartV2=mitigationTMSStartV2, snmpUp=snmpUp, spSystemErrorType=spSystemErrorType, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSObservedMean=spDNSObservedMean, spReportURL=spReportURL, spFingerprintMessage=spFingerprintMessage, spInterfaceUsageHC=spInterfaceUsageHC, spLicenseErrCount=spLicenseErrCount, mitigationFlowspecStart=mitigationFlowspecStart, spFingerprintSender=spFingerprintSender, spCollector=spCollector, spFingerprintName=spFingerprintName, spFlowspecTimeout=spFlowspecTimeout, trafficAutoMitigation=trafficAutoMitigation, spHijackLocal=spHijackLocal, spTMSPrefix=spTMSPrefix, spCommFailureDestination=spCommFailureDestination, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, bgpInstability=bgpInstability, spHijackRoute=spHijackRoute, serviceUsage=serviceUsage, spDNSObserved=spDNSObserved, spScriptPort=spScriptPort, spRouter=spRouter, spBGPEvent=spBGPEvent, cloudSignalingFaultDone=cloudSignalingFaultDone, fingerprintFeedback=fingerprintFeedback, alertScript=alertScript, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spLicenseErrType=spLicenseErrType, interfaceUsage=interfaceUsage, spMitigationStart=spMitigationStart, bgpHijack=bgpHijack, licenseErrorDone=licenseErrorDone, spBlackholePrefix=spBlackholePrefix, flowUp=flowUp, peakflowSPMIB=peakflowSPMIB, notificationLimit=notificationLimit, spHardwareFailureDescription=spHardwareFailureDescription, spGreTunnelName=spGreTunnelName, spcommFailure=spcommFailure, spThirdPartyAddr=spThirdPartyAddr, greDown=greDown, spSystemErrorDescription=spSystemErrorDescription, spInterface=spInterface, spInterfaceSpeed=spInterfaceSpeed, mitigationThirdPartyStart=mitigationThirdPartyStart, spScriptStart=spScriptStart, hardwareFailure=hardwareFailure, peakflowSPMgr=peakflowSPMgr, spFlowspecCommunity=spFlowspecCommunity, flowDown=flowDown, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spScriptHost=spScriptHost, spDNSObservedMax=spDNSObservedMax, dnsBaselineDone=dnsBaselineDone, reportDone=reportDone, spPravail=spPravail, spBGPTrapEvent=spBGPTrapEvent, spUsageType=spUsageType, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spMitigationID=spMitigationID, collectorDown=collectorDown, spcommFailureDone=spcommFailureDone, fingerprintUsage=fingerprintUsage, spCommFailureDescription=spCommFailureDescription)
