...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", mitigationFlowspecStart=mitigationFlowspecStart, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spDetectedCountries=spDetectedCountries, spDNSExpected=spDNSExpected, spInterfaceSpeed=spInterfaceSpeed, alertScript=alertScript, licenseError=licenseError, spLicenseErrDescription=spLicenseErrDescription, spApplicationName=spApplicationName, spReportURL=spReportURL, deviceHealth=deviceHealth, collectorDown=collectorDown, spReportID=spReportID, spCommFailureDestination=spCommFailureDestination, spMitigationID=spMitigationID, spFingerprintMessage=spFingerprintMessage, spManagedObjects=spManagedObjects, deviceSystemErrorDone=deviceSystemErrorDone, spUsage=spUsage, cloudSignalingFault=cloudSignalingFault, spScriptPort=spScriptPort, spFingerprintFeedback=spFingerprintFeedback, deviceTotalFlowsHC=deviceTotalFlowsHC, spHijackRoute=spHijackRoute, collectorStart=collectorStart, spTMSCommunity=spTMSCommunity, fingerprintUsageDone=fingerprintUsageDone, peakflowSPMIB=peakflowSPMIB, licenseErrorDone=licenseErrorDone, mitigationThirdPartyStart=mitigationThirdPartyStart, spThirdPartyAddr=spThirdPartyAddr, spInetAddressType=spInetAddressType, serviceUsageDone=serviceUsageDone, flowDown=flowDown, spHardwareFailureDescription=spHardwareFailureDescription, spPravail=spPravail, mitigationTMSStartV2=mitigationTMSStartV2, bgpHijack=bgpHijack, spDNSName=spDNSName, spBlackholeCommunity=spBlackholeCommunity, spThirdPartyZone=spThirdPartyZone, fingerprintUsage=fingerprintUsage, bgpDown=bgpDown, devicePhysicalMemory=devicePhysicalMemory, spcommFailure=spcommFailure, cloudSignalingFaultDone=cloudSignalingFaultDone, spBGPTrapPrefix=spBGPTrapPrefix, spBGPTrapName=spBGPTrapName, configChange=configChange, reportDone=reportDone, deviceSwapSpace=deviceSwapSpace, spAlertID=spAlertID, interfaceUsageDone=interfaceUsageDone, spDetector=spDetector, snmpDown=snmpDown, spImpactBps=spImpactBps, deviceDiskUsage=deviceDiskUsage, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spInetAddress=spInetAddress, mitigationTMSStart=mitigationTMSStart, serviceUsage=serviceUsage, spTMSMultiPrefix=spTMSMultiPrefix, peakflowSPCMI=peakflowSPCMI, managedObjectUsageDone=managedObjectUsageDone, spMitigationName=spMitigationName, spSystemErrorDescription=spSystemErrorDescription, peakflowSPTraps=peakflowSPTraps, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, peakflowSPMgr=peakflowSPMgr, dnsBaseline=dnsBaseline, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spSystemErrorType=spSystemErrorType, bgpInstabilityDone=bgpInstabilityDone, spCommFailureDescription=spCommFailureDescription, bgpInstability=bgpInstability, interfaceUsage=interfaceUsage, spThreshold=spThreshold, mitigationDone=mitigationDone, spUnit=spUnit, spDNSObservedMax=spDNSObservedMax, managedObjectUsage=managedObjectUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spInterfaceSpeedHC=spInterfaceSpeedHC, spInterfaceUsage=spInterfaceUsage, autoclassifyStart=autoclassifyStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spMitigationStart=spMitigationStart, spLicenseErrCount=spLicenseErrCount, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, bgpUp=bgpUp, spTMSTimeout=spTMSTimeout, spRouter=spRouter, spDNSObservedMean=spDNSObservedMean, spFlowspecTimeout=spFlowspecTimeout, spReportName=spReportName, spUsername=spUsername, dnsBaselineDone=dnsBaselineDone, spBlackholeNexthop=spBlackholeNexthop, deviceSystemError=deviceSystemError, spBGPTrapEvent=spBGPTrapEvent, spScriptHost=spScriptHost, spLicenseErrLimit=spLicenseErrLimit, fingerprintFeedback=fingerprintFeedback, deviceTotalFlows=deviceTotalFlows, spManagedObject=spManagedObject, bgpTrap=bgpTrap, spTMSPrefix=spTMSPrefix, spBGPEvent=spBGPEvent, PYSNMP_MODULE_ID=peakflowSPMIB, spInterfaceUsageHC=spInterfaceUsageHC, spAlertDetectionSignatures=spAlertDetectionSignatures, snmpUp=snmpUp, collectorUp=collectorUp, spDNSObserved=spDNSObserved, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBlackholeTimeout=spBlackholeTimeout, spUsageType=spUsageType, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spLicenseErrType=spLicenseErrType, hardwareFailureDone=hardwareFailureDone, spHijackAttr=spHijackAttr, spGreTunnelName=spGreTunnelName, spHijackLocal=spHijackLocal, spServiceElement=spServiceElement, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFingerprintSender=spFingerprintSender, spInterface=spInterface, spFlowspecCommunity=spFlowspecCommunity, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spServiceName=spServiceName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, routingFailover=routingFailover, spVersion=spVersion, spBlackholePrefix=spBlackholePrefix, spImpactPps=spImpactPps, spCollector=spCollector, spcommFailureDone=spcommFailureDone, spGreTunnelDestination=spGreTunnelDestination, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, mitigationBlackholeStart=mitigationBlackholeStart, dosHostDetectionAlertDone=dosHostDetectionAlertDone, greDown=greDown, spScriptStart=spScriptStart, hardwareFailure=hardwareFailure, notificationLimit=notificationLimit, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spDuration=spDuration, dosHostDetectionAlert=dosHostDetectionAlert, spManagedObjectFamily=spManagedObjectFamily, flowUp=flowUp, spScriptCommand=spScriptCommand, spInterfaceIndex=spInterfaceIndex, trafficAutoMitigation=trafficAutoMitigation, greDownDone=greDownDone, spBGPInstability=spBGPInstability, spFingerprintName=spFingerprintName, fingerprintReceive=fingerprintReceive)
