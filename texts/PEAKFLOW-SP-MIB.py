...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceDiskUsage=deviceDiskUsage, spDNSObservedMax=spDNSObservedMax, spGreTunnelDestination=spGreTunnelDestination, spDetector=spDetector, spScriptHost=spScriptHost, bgpTrap=bgpTrap, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFingerprintMessage=spFingerprintMessage, peakflowSPMgr=peakflowSPMgr, spInterfaceSpeedHC=spInterfaceSpeedHC, spMitigationStart=spMitigationStart, peakflowSPCMI=peakflowSPCMI, spDuration=spDuration, spManagedObjects=spManagedObjects, spLicenseErrLimit=spLicenseErrLimit, deviceSwapSpaceUsage=deviceSwapSpaceUsage, snmpUp=snmpUp, deviceSystemError=deviceSystemError, bgpHijack=bgpHijack, spMitigationID=spMitigationID, deviceSwapSpace=deviceSwapSpace, dosHostDetectionAlert=dosHostDetectionAlert, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spDNSObservedMean=spDNSObservedMean, spScriptPort=spScriptPort, mitigationFlowspecStart=mitigationFlowspecStart, spBlackholeNexthop=spBlackholeNexthop, spManagedObjectFamily=spManagedObjectFamily, cloudSignalingFault=cloudSignalingFault, spServiceName=spServiceName, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spBGPTrapEvent=spBGPTrapEvent, spFingerprintSender=spFingerprintSender, spHijackAttr=spHijackAttr, spThirdPartyAddr=spThirdPartyAddr, deviceTotalFlowsHC=deviceTotalFlowsHC, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spManagedObject=spManagedObject, spLicenseErrType=spLicenseErrType, deviceTotalFlows=deviceTotalFlows, greDown=greDown, spSystemErrorDescription=spSystemErrorDescription, spRouter=spRouter, spAlertID=spAlertID, spcommFailure=spcommFailure, licenseError=licenseError, autoclassifyStart=autoclassifyStart, spUnit=spUnit, PYSNMP_MODULE_ID=peakflowSPMIB, peakflowSPTraps=peakflowSPTraps, collectorStart=collectorStart, spTMSPrefix=spTMSPrefix, flowUp=flowUp, spHijackRoute=spHijackRoute, spInterfaceIndex=spInterfaceIndex, spScriptCommand=spScriptCommand, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spBGPTrapPrefix=spBGPTrapPrefix, routingFailover=routingFailover, fingerprintFeedback=fingerprintFeedback, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spUsername=spUsername, spInterfaceUsageHC=spInterfaceUsageHC, spInterfaceSpeed=spInterfaceSpeed, spHardwareFailureDescription=spHardwareFailureDescription, spBGPEvent=spBGPEvent, spCommFailureDestination=spCommFailureDestination, spDNSObserved=spDNSObserved, spBlackholeCommunity=spBlackholeCommunity, spReportID=spReportID, fingerprintReceive=fingerprintReceive, spServiceElement=spServiceElement, spInetAddressType=spInetAddressType, mitigationThirdPartyStart=mitigationThirdPartyStart, spUsage=spUsage, spLicenseErrCount=spLicenseErrCount, spCollector=spCollector, spUsageType=spUsageType, spFlowspecTimeout=spFlowspecTimeout, spMitigationName=spMitigationName, collectorDown=collectorDown, spAlertDetectionSignatures=spAlertDetectionSignatures, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spcommFailureDone=spcommFailureDone, bgpInstability=bgpInstability, cloudSignalingFaultDone=cloudSignalingFaultDone, spInetAddress=spInetAddress, spDNSName=spDNSName, alertScript=alertScript, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spFingerprintName=spFingerprintName, bgpInstabilityDone=bgpInstabilityDone, deviceHealth=deviceHealth, dnsBaselineDone=dnsBaselineDone, spBGPTrapName=spBGPTrapName, spThirdPartyZone=spThirdPartyZone, reportDone=reportDone, spBlackholePrefix=spBlackholePrefix, spImpactBps=spImpactBps, bgpDown=bgpDown, serviceUsage=serviceUsage, spGreTunnelName=spGreTunnelName, serviceUsageDone=serviceUsageDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFingerprintFeedback=spFingerprintFeedback, spTMSTimeout=spTMSTimeout, spImpactPps=spImpactPps, flowDown=flowDown, trafficAutoMitigation=trafficAutoMitigation, spLicenseErrDescription=spLicenseErrDescription, snmpDown=snmpDown, notificationLimit=notificationLimit, collectorUp=collectorUp, spInterfaceUsage=spInterfaceUsage, interfaceUsageDone=interfaceUsageDone, spReportURL=spReportURL, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spTMSMultiPrefix=spTMSMultiPrefix, spVersion=spVersion, spInterface=spInterface, peakflowSPMIB=peakflowSPMIB, spPravail=spPravail, mitigationBlackholeStart=mitigationBlackholeStart, spBlackholeTimeout=spBlackholeTimeout, fingerprintUsageDone=fingerprintUsageDone, bgpUp=bgpUp, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spThreshold=spThreshold, spBGPTrapNewAttributes=spBGPTrapNewAttributes, devicePhysicalMemory=devicePhysicalMemory, spCommFailureDescription=spCommFailureDescription, spFlowspecCommunity=spFlowspecCommunity, configChange=configChange, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, mitigationDone=mitigationDone, managedObjectUsage=managedObjectUsage, mitigationTMSStartV2=mitigationTMSStartV2, hardwareFailure=hardwareFailure, spSystemErrorType=spSystemErrorType, managedObjectUsageDone=managedObjectUsageDone, interfaceUsage=interfaceUsage, deviceSystemErrorDone=deviceSystemErrorDone, spDNSExpected=spDNSExpected, dnsBaseline=dnsBaseline, spApplicationName=spApplicationName, licenseErrorDone=licenseErrorDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spScriptStart=spScriptStart, spDetectedCountries=spDetectedCountries, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spHijackLocal=spHijackLocal, greDownDone=greDownDone, hardwareFailureDone=hardwareFailureDone, fingerprintUsage=fingerprintUsage, spTMSCommunity=spTMSCommunity, mitigationTMSStart=mitigationTMSStart, spBGPInstability=spBGPInstability, spReportName=spReportName)
