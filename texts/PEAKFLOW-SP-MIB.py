...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceTotalFlowsHC=deviceTotalFlowsHC, spDNSObservedMean=spDNSObservedMean, spThirdPartyAddr=spThirdPartyAddr, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, fingerprintUsageDone=fingerprintUsageDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, trafficAutoMitigation=trafficAutoMitigation, spGreTunnelDestination=spGreTunnelDestination, spCommFailureDescription=spCommFailureDescription, peakflowSPMIB=peakflowSPMIB, PYSNMP_MODULE_ID=peakflowSPMIB, spLicenseErrLimit=spLicenseErrLimit, spRouter=spRouter, spInterfaceSpeedHC=spInterfaceSpeedHC, spBGPTrapEvent=spBGPTrapEvent, spApplicationName=spApplicationName, spCollector=spCollector, spInterfaceIndex=spInterfaceIndex, managedObjectUsageDone=managedObjectUsageDone, deviceSystemErrorDone=deviceSystemErrorDone, spSystemErrorDescription=spSystemErrorDescription, dosHostDetectionAlert=dosHostDetectionAlert, mitigationFlowspecStart=mitigationFlowspecStart, dnsBaseline=dnsBaseline, spScriptHost=spScriptHost, spTMSPrefix=spTMSPrefix, spLicenseErrType=spLicenseErrType, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceTotalFlows=deviceTotalFlows, mitigationTMSStart=mitigationTMSStart, spGreTunnelName=spGreTunnelName, snmpDown=snmpDown, spHijackRoute=spHijackRoute, spInterfaceUsage=spInterfaceUsage, spPravail=spPravail, spAlertDetectionSignatures=spAlertDetectionSignatures, spDNSObservedMax=spDNSObservedMax, licenseErrorDone=licenseErrorDone, spDNSExpected=spDNSExpected, spServiceElement=spServiceElement, spInetAddressType=spInetAddressType, spLicenseErrCount=spLicenseErrCount, spReportURL=spReportURL, mitigationTMSStartV2=mitigationTMSStartV2, spScriptCommand=spScriptCommand, spUsername=spUsername, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceSwapSpace=deviceSwapSpace, spBGPTrapPrefix=spBGPTrapPrefix, serviceUsage=serviceUsage, bgpInstability=bgpInstability, hardwareFailureDone=hardwareFailureDone, routingFailover=routingFailover, peakflowSPTraps=peakflowSPTraps, spInterfaceUsageHC=spInterfaceUsageHC, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spTMSTimeout=spTMSTimeout, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, collectorUp=collectorUp, spMitigationID=spMitigationID, flowUp=flowUp, dosNetworkProfiledAlert=dosNetworkProfiledAlert, alertScript=alertScript, spScriptStart=spScriptStart, spBGPTrapName=spBGPTrapName, deviceSwapSpaceInUse=deviceSwapSpaceInUse, serviceUsageDone=serviceUsageDone, spLicenseErrDescription=spLicenseErrDescription, spThreshold=spThreshold, peakflowSPMgr=peakflowSPMgr, spBGPInstability=spBGPInstability, spImpactPps=spImpactPps, greDown=greDown, spDNSObserved=spDNSObserved, spBlackholeNexthop=spBlackholeNexthop, cloudSignalingFault=cloudSignalingFault, spFingerprintFeedback=spFingerprintFeedback, bgpDown=bgpDown, deviceDiskUsage=deviceDiskUsage, spMitigationStart=spMitigationStart, spcommFailure=spcommFailure, interfaceUsage=interfaceUsage, spInterface=spInterface, bgpHijack=bgpHijack, interfaceUsageDone=interfaceUsageDone, spAlertID=spAlertID, spVersion=spVersion, spFingerprintSender=spFingerprintSender, spFingerprintMessage=spFingerprintMessage, spDuration=spDuration, dosHostDetectionAlertDone=dosHostDetectionAlertDone, snmpUp=snmpUp, spBGPEvent=spBGPEvent, bgpTrap=bgpTrap, licenseError=licenseError, notificationLimit=notificationLimit, dnsBaselineDone=dnsBaselineDone, spTMSMultiPrefix=spTMSMultiPrefix, spMitigationName=spMitigationName, hardwareFailure=hardwareFailure, spBlackholePrefix=spBlackholePrefix, spManagedObjectFamily=spManagedObjectFamily, spSystemErrorType=spSystemErrorType, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spcommFailureDone=spcommFailureDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, fingerprintReceive=fingerprintReceive, fingerprintUsage=fingerprintUsage, spUsageType=spUsageType, spUsage=spUsage, spBlackholeCommunity=spBlackholeCommunity, configChange=configChange, spHijackLocal=spHijackLocal, devicePhysicalMemory=devicePhysicalMemory, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spInterfaceSpeed=spInterfaceSpeed, spReportID=spReportID, spTMSCommunity=spTMSCommunity, spFlowspecTimeout=spFlowspecTimeout, spServiceName=spServiceName, spImpactBps=spImpactBps, spManagedObjects=spManagedObjects, mitigationThirdPartyStart=mitigationThirdPartyStart, bgpUp=bgpUp, bgpInstabilityDone=bgpInstabilityDone, greDownDone=greDownDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spHardwareFailureDescription=spHardwareFailureDescription, autoclassifyStart=autoclassifyStart, spReportName=spReportName, spFingerprintName=spFingerprintName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, flowDown=flowDown, spScriptPort=spScriptPort, collectorStart=collectorStart, spUnit=spUnit, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spHijackAttr=spHijackAttr, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spFlowspecCommunity=spFlowspecCommunity, fingerprintFeedback=fingerprintFeedback, spManagedObject=spManagedObject, managedObjectUsage=managedObjectUsage, spThirdPartyZone=spThirdPartyZone, peakflowSPCMI=peakflowSPCMI, deviceSystemError=deviceSystemError, spCommFailureDestination=spCommFailureDestination, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, collectorDown=collectorDown, mitigationDone=mitigationDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spDetectedCountries=spDetectedCountries, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, reportDone=reportDone, spDNSName=spDNSName, spInetAddress=spInetAddress, spBlackholeTimeout=spBlackholeTimeout, deviceHealth=deviceHealth, spDetector=spDetector, mitigationBlackholeStart=mitigationBlackholeStart)
