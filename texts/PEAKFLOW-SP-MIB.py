...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInetAddress=spInetAddress, spCollector=spCollector, deviceTotalFlows=deviceTotalFlows, peakflowSPTraps=peakflowSPTraps, spReportID=spReportID, spLicenseErrLimit=spLicenseErrLimit, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spBGPEvent=spBGPEvent, spBlackholeCommunity=spBlackholeCommunity, alertScript=alertScript, spManagedObjectFamily=spManagedObjectFamily, interfaceUsageDone=interfaceUsageDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spImpactBps=spImpactBps, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceSwapSpaceInUse=deviceSwapSpaceInUse, cloudSignalingFaultDone=cloudSignalingFaultDone, peakflowSPMgr=peakflowSPMgr, spPravail=spPravail, snmpDown=snmpDown, notificationLimit=notificationLimit, spCommFailureDescription=spCommFailureDescription, spHijackLocal=spHijackLocal, fingerprintReceive=fingerprintReceive, greDown=greDown, deviceDiskUsage=deviceDiskUsage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, mitigationFlowspecStart=mitigationFlowspecStart, spLicenseErrCount=spLicenseErrCount, dnsBaselineDone=dnsBaselineDone, spTMSCommunity=spTMSCommunity, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spInterfaceUsage=spInterfaceUsage, spRouter=spRouter, spServiceName=spServiceName, licenseError=licenseError, spcommFailure=spcommFailure, flowDown=flowDown, spUnit=spUnit, cloudSignalingFault=cloudSignalingFault, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSName=spDNSName, spInterfaceIndex=spInterfaceIndex, spBGPTrapEvent=spBGPTrapEvent, spUsageType=spUsageType, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spServiceElement=spServiceElement, spDetectedCountries=spDetectedCountries, spMitigationName=spMitigationName, deviceTotalFlowsHC=deviceTotalFlowsHC, collectorDown=collectorDown, spScriptHost=spScriptHost, spBGPTrapOldAttributes=spBGPTrapOldAttributes, hardwareFailureDone=hardwareFailureDone, spBlackholeNexthop=spBlackholeNexthop, mitigationBlackholeStart=mitigationBlackholeStart, routingFailover=routingFailover, mitigationDone=mitigationDone, spLicenseErrType=spLicenseErrType, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spDuration=spDuration, trafficAutoMitigation=trafficAutoMitigation, peakflowSPMIB=peakflowSPMIB, mitigationTMSStart=mitigationTMSStart, spGreTunnelDestination=spGreTunnelDestination, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spVersion=spVersion, spFingerprintFeedback=spFingerprintFeedback, spSystemErrorType=spSystemErrorType, spBlackholePrefix=spBlackholePrefix, spCommFailureDestination=spCommFailureDestination, spFlowspecTimeout=spFlowspecTimeout, spBlackholeTimeout=spBlackholeTimeout, fingerprintUsageDone=fingerprintUsageDone, spSystemErrorDescription=spSystemErrorDescription, spDNSObservedMax=spDNSObservedMax, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, devicePhysicalMemory=devicePhysicalMemory, spDNSExpected=spDNSExpected, spThirdPartyZone=spThirdPartyZone, spScriptStart=spScriptStart, spApplicationName=spApplicationName, serviceUsage=serviceUsage, spTMSMultiPrefix=spTMSMultiPrefix, bgpInstability=bgpInstability, spcommFailureDone=spcommFailureDone, spInterface=spInterface, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spTMSPrefix=spTMSPrefix, fingerprintUsage=fingerprintUsage, spManagedObjects=spManagedObjects, managedObjectUsageDone=managedObjectUsageDone, spAlertID=spAlertID, spThreshold=spThreshold, snmpUp=snmpUp, dosNetworkProfiledAlert=dosNetworkProfiledAlert, bgpDown=bgpDown, spLicenseErrDescription=spLicenseErrDescription, bgpHijack=bgpHijack, bgpUp=bgpUp, fingerprintFeedback=fingerprintFeedback, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spDetector=spDetector, managedObjectUsage=managedObjectUsage, spFingerprintMessage=spFingerprintMessage, spHijackRoute=spHijackRoute, spMitigationStart=spMitigationStart, spImpactPps=spImpactPps, greDownDone=greDownDone, spFlowspecCommunity=spFlowspecCommunity, deviceHealth=deviceHealth, collectorStart=collectorStart, dosHostDetectionAlert=dosHostDetectionAlert, spBGPTrapPrefix=spBGPTrapPrefix, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, bgpTrap=bgpTrap, spTMSTimeout=spTMSTimeout, spGreTunnelName=spGreTunnelName, mitigationTMSStartV2=mitigationTMSStartV2, spDNSObservedMean=spDNSObservedMean, spBGPInstability=spBGPInstability, spDNSObserved=spDNSObserved, dosHostDetectionAlertDone=dosHostDetectionAlertDone, bgpInstabilityDone=bgpInstabilityDone, spReportURL=spReportURL, spInetAddressType=spInetAddressType, spAlertDetectionSignatures=spAlertDetectionSignatures, collectorUp=collectorUp, mitigationThirdPartyStart=mitigationThirdPartyStart, spThirdPartyAddr=spThirdPartyAddr, spFingerprintName=spFingerprintName, flowUp=flowUp, autoclassifyStart=autoclassifyStart, interfaceUsage=interfaceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, deviceSystemErrorDone=deviceSystemErrorDone, spUsage=spUsage, spBGPTrapName=spBGPTrapName, spReportName=spReportName, spHardwareFailureDescription=spHardwareFailureDescription, spFingerprintSender=spFingerprintSender, spMitigationID=spMitigationID, hardwareFailure=hardwareFailure, dnsBaseline=dnsBaseline, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, configChange=configChange, reportDone=reportDone, serviceUsageDone=serviceUsageDone, deviceSwapSpace=deviceSwapSpace, licenseErrorDone=licenseErrorDone, spInterfaceSpeed=spInterfaceSpeed, spInterfaceUsageHC=spInterfaceUsageHC, spHijackAttr=spHijackAttr, spScriptPort=spScriptPort, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, deviceSystemError=deviceSystemError, spScriptCommand=spScriptCommand, peakflowSPCMI=peakflowSPCMI, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spUsername=spUsername, spManagedObject=spManagedObject)
