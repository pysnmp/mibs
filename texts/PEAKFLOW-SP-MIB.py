...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spAlertDetectionSignatures=spAlertDetectionSignatures, spFingerprintName=spFingerprintName, spBlackholeCommunity=spBlackholeCommunity, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBlackholeNexthop=spBlackholeNexthop, peakflowSPCMI=peakflowSPCMI, greDownDone=greDownDone, deviceTotalFlows=deviceTotalFlows, spFingerprintMessage=spFingerprintMessage, deviceSwapSpace=deviceSwapSpace, spDuration=spDuration, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spBGPTrapEvent=spBGPTrapEvent, fingerprintUsage=fingerprintUsage, spBGPInstability=spBGPInstability, mitigationTMSStart=mitigationTMSStart, spInterfaceSpeedHC=spInterfaceSpeedHC, spAlertID=spAlertID, spCloudSignalFaultDescription=spCloudSignalFaultDescription, collectorStart=collectorStart, notificationLimit=notificationLimit, spMitigationName=spMitigationName, dnsBaseline=dnsBaseline, spManagedObjects=spManagedObjects, spPravail=spPravail, spThreshold=spThreshold, spInterfaceIndex=spInterfaceIndex, spScriptHost=spScriptHost, snmpDown=snmpDown, spRouter=spRouter, fingerprintFeedback=fingerprintFeedback, spBGPTrapPrefix=spBGPTrapPrefix, spHijackAttr=spHijackAttr, spFlowspecCommunity=spFlowspecCommunity, bgpTrap=bgpTrap, collectorUp=collectorUp, spBGPTrapOldAttributes=spBGPTrapOldAttributes, bgpDown=bgpDown, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, bgpInstabilityDone=bgpInstabilityDone, spcommFailure=spcommFailure, PYSNMP_MODULE_ID=peakflowSPMIB, spDNSObservedMean=spDNSObservedMean, spGreTunnelDestination=spGreTunnelDestination, interfaceUsageDone=interfaceUsageDone, spLicenseErrType=spLicenseErrType, spVersion=spVersion, spTMSCommunity=spTMSCommunity, spUsage=spUsage, deviceSystemErrorDone=deviceSystemErrorDone, spDNSObserved=spDNSObserved, spApplicationName=spApplicationName, spFlowspecTimeout=spFlowspecTimeout, spUnit=spUnit, spInterface=spInterface, mitigationBlackholeStart=mitigationBlackholeStart, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spDNSName=spDNSName, spDNSObservedMax=spDNSObservedMax, dosNetworkProfiledAlert=dosNetworkProfiledAlert, reportDone=reportDone, spScriptCommand=spScriptCommand, bgpUp=bgpUp, peakflowSPMgr=peakflowSPMgr, spUsageType=spUsageType, routingFailover=routingFailover, spInterfaceUsageHC=spInterfaceUsageHC, configChange=configChange, spReportName=spReportName, spBlackholeTimeout=spBlackholeTimeout, spCommFailureDescription=spCommFailureDescription, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spServiceElement=spServiceElement, spCommFailureDestination=spCommFailureDestination, spDetectedCountries=spDetectedCountries, spReportURL=spReportURL, spReportID=spReportID, spInterfaceSpeed=spInterfaceSpeed, spHardwareFailureDescription=spHardwareFailureDescription, spLicenseErrCount=spLicenseErrCount, hardwareFailureDone=hardwareFailureDone, serviceUsageDone=serviceUsageDone, flowDown=flowDown, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spSystemErrorDescription=spSystemErrorDescription, deviceDiskUsage=deviceDiskUsage, spHijackLocal=spHijackLocal, spSystemErrorType=spSystemErrorType, spGreTunnelName=spGreTunnelName, managedObjectUsageDone=managedObjectUsageDone, mitigationDone=mitigationDone, mitigationTMSStartV2=mitigationTMSStartV2, bgpInstability=bgpInstability, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spManagedObject=spManagedObject, peakflowSPTraps=peakflowSPTraps, spInterfaceUsage=spInterfaceUsage, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInetAddress=spInetAddress, spTMSPrefix=spTMSPrefix, spDetector=spDetector, managedObjectUsage=managedObjectUsage, alertScript=alertScript, fingerprintUsageDone=fingerprintUsageDone, spThirdPartyZone=spThirdPartyZone, devicePhysicalMemory=devicePhysicalMemory, mitigationFlowspecStart=mitigationFlowspecStart, spTMSMultiPrefix=spTMSMultiPrefix, licenseError=licenseError, spFingerprintFeedback=spFingerprintFeedback, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spMitigationID=spMitigationID, collectorDown=collectorDown, spImpactBps=spImpactBps, peakflowSPMIB=peakflowSPMIB, spThirdPartyAddr=spThirdPartyAddr, trafficAutoMitigation=trafficAutoMitigation, fingerprintReceive=fingerprintReceive, cloudSignalingFaultDone=cloudSignalingFaultDone, spDNSExpected=spDNSExpected, spCollector=spCollector, spcommFailureDone=spcommFailureDone, spInetAddressType=spInetAddressType, serviceUsage=serviceUsage, spMitigationStart=spMitigationStart, spBlackholePrefix=spBlackholePrefix, bgpHijack=bgpHijack, spBGPTrapName=spBGPTrapName, spServiceName=spServiceName, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFingerprintSender=spFingerprintSender, spLicenseErrLimit=spLicenseErrLimit, deviceTotalFlowsHC=deviceTotalFlowsHC, deviceSystemError=deviceSystemError, deviceHealth=deviceHealth, spHijackRoute=spHijackRoute, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, dosHostDetectionAlert=dosHostDetectionAlert, spScriptPort=spScriptPort, licenseErrorDone=licenseErrorDone, spTMSTimeout=spTMSTimeout, spLicenseErrDescription=spLicenseErrDescription, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spImpactPps=spImpactPps, hardwareFailure=hardwareFailure, mitigationThirdPartyStart=mitigationThirdPartyStart, cloudSignalingFault=cloudSignalingFault, spScriptStart=spScriptStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spManagedObjectFamily=spManagedObjectFamily, flowUp=flowUp, greDown=greDown, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spBGPEvent=spBGPEvent, interfaceUsage=interfaceUsage, dnsBaselineDone=dnsBaselineDone, autoclassifyStart=autoclassifyStart, snmpUp=snmpUp, spUsername=spUsername, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix)
