...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceIndex=spInterfaceIndex, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSMultiPrefix=spTMSMultiPrefix, spHijackRoute=spHijackRoute, peakflowSPCMI=peakflowSPCMI, spInterfaceUsageHC=spInterfaceUsageHC, spImpactBps=spImpactBps, spHardwareFailureDescription=spHardwareFailureDescription, spReportName=spReportName, spBlackholeNexthop=spBlackholeNexthop, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spVersion=spVersion, spScriptStart=spScriptStart, spDNSObservedMax=spDNSObservedMax, spLicenseErrCount=spLicenseErrCount, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, snmpDown=snmpDown, bgpDown=bgpDown, spInterfaceSpeed=spInterfaceSpeed, mitigationTMSStart=mitigationTMSStart, spInetAddress=spInetAddress, alertScript=alertScript, spSystemErrorDescription=spSystemErrorDescription, spInterfaceUsage=spInterfaceUsage, reportDone=reportDone, serviceUsageDone=serviceUsageDone, peakflowSPMIB=peakflowSPMIB, spGreTunnelDestination=spGreTunnelDestination, spFingerprintSender=spFingerprintSender, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spGreTunnelName=spGreTunnelName, spTMSTimeout=spTMSTimeout, spMitigationName=spMitigationName, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceTotalFlowsHC=deviceTotalFlowsHC, interfaceUsageDone=interfaceUsageDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, managedObjectUsage=managedObjectUsage, spDNSObserved=spDNSObserved, trafficAutoMitigation=trafficAutoMitigation, spBGPEvent=spBGPEvent, spFlowspecCommunity=spFlowspecCommunity, hardwareFailure=hardwareFailure, spThirdPartyZone=spThirdPartyZone, mitigationThirdPartyStart=mitigationThirdPartyStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spManagedObjects=spManagedObjects, spServiceName=spServiceName, spInetAddressType=spInetAddressType, spApplicationName=spApplicationName, peakflowSPTraps=peakflowSPTraps, spDetectedCountries=spDetectedCountries, spDetector=spDetector, spcommFailureDone=spcommFailureDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spTMSPrefix=spTMSPrefix, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spReportURL=spReportURL, spInterface=spInterface, devicePhysicalMemory=devicePhysicalMemory, flowUp=flowUp, spServiceElement=spServiceElement, spUsage=spUsage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spScriptHost=spScriptHost, deviceSystemErrorDone=deviceSystemErrorDone, deviceHealth=deviceHealth, collectorDown=collectorDown, configChange=configChange, greDown=greDown, cloudSignalingFault=cloudSignalingFault, routingFailover=routingFailover, spAlertDetectionSignatures=spAlertDetectionSignatures, spScriptCommand=spScriptCommand, spBlackholePrefix=spBlackholePrefix, spHijackLocal=spHijackLocal, flowDown=flowDown, mitigationTMSStartV2=mitigationTMSStartV2, licenseErrorDone=licenseErrorDone, autoclassifyStart=autoclassifyStart, spcommFailure=spcommFailure, spSystemErrorType=spSystemErrorType, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spUsageType=spUsageType, spFingerprintFeedback=spFingerprintFeedback, managedObjectUsageDone=managedObjectUsageDone, spThirdPartyAddr=spThirdPartyAddr, spDuration=spDuration, spBlackholeCommunity=spBlackholeCommunity, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, notificationLimit=notificationLimit, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spManagedObjectFamily=spManagedObjectFamily, spLicenseErrType=spLicenseErrType, spBGPTrapName=spBGPTrapName, spDNSExpected=spDNSExpected, fingerprintReceive=fingerprintReceive, spCommFailureDescription=spCommFailureDescription, dnsBaseline=dnsBaseline, bgpInstability=bgpInstability, interfaceUsage=interfaceUsage, bgpUp=bgpUp, spHijackAttr=spHijackAttr, greDownDone=greDownDone, spBGPInstability=spBGPInstability, spFingerprintMessage=spFingerprintMessage, spCollector=spCollector, deviceTotalFlows=deviceTotalFlows, dnsBaselineDone=dnsBaselineDone, peakflowSPMgr=peakflowSPMgr, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, fingerprintUsageDone=fingerprintUsageDone, spRouter=spRouter, spDNSObservedMean=spDNSObservedMean, mitigationFlowspecStart=mitigationFlowspecStart, spUsername=spUsername, deviceSwapSpace=deviceSwapSpace, dosHostDetectionAlert=dosHostDetectionAlert, licenseError=licenseError, spBlackholeTimeout=spBlackholeTimeout, spThreshold=spThreshold, PYSNMP_MODULE_ID=peakflowSPMIB, bgpHijack=bgpHijack, spFlowspecTimeout=spFlowspecTimeout, spImpactPps=spImpactPps, spLicenseErrLimit=spLicenseErrLimit, spScriptPort=spScriptPort, spDNSName=spDNSName, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, collectorUp=collectorUp, deviceSystemError=deviceSystemError, spUnit=spUnit, spFingerprintName=spFingerprintName, spManagedObject=spManagedObject, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spTMSCommunity=spTMSCommunity, deviceDiskUsage=deviceDiskUsage, collectorStart=collectorStart, snmpUp=snmpUp, hardwareFailureDone=hardwareFailureDone, spInterfaceSpeedHC=spInterfaceSpeedHC, spPravail=spPravail, bgpInstabilityDone=bgpInstabilityDone, spLicenseErrDescription=spLicenseErrDescription, fingerprintUsage=fingerprintUsage, spAlertID=spAlertID, spMitigationID=spMitigationID, serviceUsage=serviceUsage, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, fingerprintFeedback=fingerprintFeedback, mitigationBlackholeStart=mitigationBlackholeStart, mitigationDone=mitigationDone, spBGPTrapPrefix=spBGPTrapPrefix, spMitigationStart=spMitigationStart, cloudSignalingFaultDone=cloudSignalingFaultDone, spBGPTrapEvent=spBGPTrapEvent, dosNetworkProfiledAlert=dosNetworkProfiledAlert, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spReportID=spReportID, bgpTrap=bgpTrap, spCommFailureDestination=spCommFailureDestination)
