...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBGPTrapName=spBGPTrapName, spBGPTrapPrefix=spBGPTrapPrefix, spLicenseErrLimit=spLicenseErrLimit, dnsBaseline=dnsBaseline, flowUp=flowUp, hardwareFailure=hardwareFailure, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, licenseErrorDone=licenseErrorDone, deviceSystemError=deviceSystemError, spUsage=spUsage, spDNSName=spDNSName, snmpDown=snmpDown, deviceHealth=deviceHealth, spMitigationStart=spMitigationStart, spReportURL=spReportURL, collectorStart=collectorStart, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, interfaceUsage=interfaceUsage, spBlackholeCommunity=spBlackholeCommunity, spManagedObjects=spManagedObjects, spHijackRoute=spHijackRoute, autoclassifyStart=autoclassifyStart, spVersion=spVersion, spGreTunnelDestination=spGreTunnelDestination, spInterfaceUsage=spInterfaceUsage, spCollector=spCollector, PYSNMP_MODULE_ID=peakflowSPMIB, spManagedObjectFamily=spManagedObjectFamily, spDetector=spDetector, mitigationFlowspecStart=mitigationFlowspecStart, spBGPTrapEvent=spBGPTrapEvent, deviceDiskUsage=deviceDiskUsage, deviceSwapSpaceInUse=deviceSwapSpaceInUse, managedObjectUsageDone=managedObjectUsageDone, spFingerprintFeedback=spFingerprintFeedback, fingerprintFeedback=fingerprintFeedback, greDownDone=greDownDone, peakflowSPTraps=peakflowSPTraps, spMitigationID=spMitigationID, serviceUsage=serviceUsage, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSObservedMax=spDNSObservedMax, managedObjectUsage=managedObjectUsage, deviceTotalFlows=deviceTotalFlows, peakflowSPMIB=peakflowSPMIB, spHijackAttr=spHijackAttr, flowDown=flowDown, routingFailover=routingFailover, bgpUp=bgpUp, fingerprintUsage=fingerprintUsage, spImpactBps=spImpactBps, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spCommFailureDestination=spCommFailureDestination, serviceUsageDone=serviceUsageDone, spInetAddress=spInetAddress, spAlertID=spAlertID, spScriptCommand=spScriptCommand, spFingerprintName=spFingerprintName, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spUsername=spUsername, spUsageType=spUsageType, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spReportID=spReportID, spBlackholeTimeout=spBlackholeTimeout, fingerprintUsageDone=fingerprintUsageDone, configChange=configChange, dosHostDetectionAlert=dosHostDetectionAlert, spServiceElement=spServiceElement, spFlowspecCommunity=spFlowspecCommunity, spInetAddressType=spInetAddressType, snmpUp=snmpUp, spDNSObserved=spDNSObserved, spReportName=spReportName, deviceTotalFlowsHC=deviceTotalFlowsHC, spTMSMultiPrefix=spTMSMultiPrefix, spTMSTimeout=spTMSTimeout, spScriptPort=spScriptPort, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBlackholePrefix=spBlackholePrefix, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFingerprintSender=spFingerprintSender, spTMSPrefix=spTMSPrefix, spManagedObject=spManagedObject, spSystemErrorDescription=spSystemErrorDescription, mitigationTMSStartV2=mitigationTMSStartV2, spDuration=spDuration, mitigationBlackholeStart=mitigationBlackholeStart, licenseError=licenseError, spDNSObservedMean=spDNSObservedMean, cloudSignalingFault=cloudSignalingFault, collectorUp=collectorUp, spThreshold=spThreshold, deviceSwapSpace=deviceSwapSpace, alertScript=alertScript, spThirdPartyZone=spThirdPartyZone, spInterface=spInterface, spFingerprintMessage=spFingerprintMessage, spCommFailureDescription=spCommFailureDescription, spApplicationName=spApplicationName, spPravail=spPravail, spLicenseErrType=spLicenseErrType, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, mitigationDone=mitigationDone, spBlackholeNexthop=spBlackholeNexthop, spGreTunnelName=spGreTunnelName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spLicenseErrCount=spLicenseErrCount, bgpInstabilityDone=bgpInstabilityDone, mitigationTMSStart=mitigationTMSStart, spSystemErrorType=spSystemErrorType, spDNSExpected=spDNSExpected, spServiceName=spServiceName, bgpDown=bgpDown, cloudSignalingFaultDone=cloudSignalingFaultDone, collectorDown=collectorDown, greDown=greDown, spAlertDetectionSignatures=spAlertDetectionSignatures, bgpHijack=bgpHijack, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spScriptStart=spScriptStart, spRouter=spRouter, spLicenseErrDescription=spLicenseErrDescription, bgpTrap=bgpTrap, interfaceUsageDone=interfaceUsageDone, peakflowSPCMI=peakflowSPCMI, deviceSystemErrorDone=deviceSystemErrorDone, spMitigationName=spMitigationName, spThirdPartyAddr=spThirdPartyAddr, trafficAutoMitigation=trafficAutoMitigation, spBGPEvent=spBGPEvent, spcommFailure=spcommFailure, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, dnsBaselineDone=dnsBaselineDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDetectedCountries=spDetectedCountries, reportDone=reportDone, spcommFailureDone=spcommFailureDone, spBGPInstability=spBGPInstability, spHijackLocal=spHijackLocal, spUnit=spUnit, spFlowspecTimeout=spFlowspecTimeout, bgpInstability=bgpInstability, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spInterfaceSpeed=spInterfaceSpeed, spScriptHost=spScriptHost, spInterfaceIndex=spInterfaceIndex, hardwareFailureDone=hardwareFailureDone, devicePhysicalMemory=devicePhysicalMemory, spCloudSignalFaultDescription=spCloudSignalFaultDescription, peakflowSPMgr=peakflowSPMgr, notificationLimit=notificationLimit, spHardwareFailureDescription=spHardwareFailureDescription, spInterfaceUsageHC=spInterfaceUsageHC, spImpactPps=spImpactPps, spTMSCommunity=spTMSCommunity, fingerprintReceive=fingerprintReceive)
