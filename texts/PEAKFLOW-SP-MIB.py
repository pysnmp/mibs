...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", bgpDown=bgpDown, spUsername=spUsername, spBGPInstability=spBGPInstability, spLicenseErrLimit=spLicenseErrLimit, serviceUsage=serviceUsage, spThirdPartyZone=spThirdPartyZone, spBlackholeCommunity=spBlackholeCommunity, PYSNMP_MODULE_ID=peakflowSPMIB, spHijackRoute=spHijackRoute, spInterfaceUsage=spInterfaceUsage, spFingerprintName=spFingerprintName, hardwareFailure=hardwareFailure, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBGPTrapName=spBGPTrapName, spDNSExpected=spDNSExpected, spSystemErrorDescription=spSystemErrorDescription, notificationLimit=notificationLimit, deviceSystemErrorDone=deviceSystemErrorDone, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceHealth=deviceHealth, managedObjectUsageDone=managedObjectUsageDone, spDuration=spDuration, spServiceName=spServiceName, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, cloudSignalingFaultDone=cloudSignalingFaultDone, spAlertDetectionSignatures=spAlertDetectionSignatures, spGreTunnelName=spGreTunnelName, spImpactPps=spImpactPps, spBlackholePrefix=spBlackholePrefix, spCommFailureDestination=spCommFailureDestination, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spHardwareFailureDescription=spHardwareFailureDescription, flowUp=flowUp, spFlowspecTimeout=spFlowspecTimeout, spDNSObservedMean=spDNSObservedMean, spLicenseErrType=spLicenseErrType, spInetAddress=spInetAddress, spDNSObservedMax=spDNSObservedMax, spFingerprintFeedback=spFingerprintFeedback, mitigationFlowspecStart=mitigationFlowspecStart, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, mitigationThirdPartyStart=mitigationThirdPartyStart, spScriptHost=spScriptHost, spVersion=spVersion, dosHostDetectionAlert=dosHostDetectionAlert, trafficAutoMitigation=trafficAutoMitigation, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, bgpTrap=bgpTrap, serviceUsageDone=serviceUsageDone, spSystemErrorType=spSystemErrorType, spMitigationStart=spMitigationStart, dnsBaseline=dnsBaseline, collectorDown=collectorDown, deviceSwapSpace=deviceSwapSpace, snmpDown=snmpDown, spDetector=spDetector, spTMSMultiPrefix=spTMSMultiPrefix, configChange=configChange, spBlackholeTimeout=spBlackholeTimeout, spBGPTrapPrefix=spBGPTrapPrefix, spInterfaceSpeed=spInterfaceSpeed, spTMSCommunity=spTMSCommunity, dnsBaselineDone=dnsBaselineDone, spLicenseErrCount=spLicenseErrCount, fingerprintUsageDone=fingerprintUsageDone, spThreshold=spThreshold, collectorStart=collectorStart, spPravail=spPravail, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spServiceElement=spServiceElement, spCollector=spCollector, spBGPTrapOldAttributes=spBGPTrapOldAttributes, alertScript=alertScript, peakflowSPCMI=peakflowSPCMI, autoclassifyStart=autoclassifyStart, mitigationTMSStart=mitigationTMSStart, managedObjectUsage=managedObjectUsage, spBGPTrapEvent=spBGPTrapEvent, bgpUp=bgpUp, deviceDiskUsage=deviceDiskUsage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, devicePhysicalMemory=devicePhysicalMemory, spcommFailureDone=spcommFailureDone, spReportURL=spReportURL, spRouter=spRouter, spFingerprintSender=spFingerprintSender, spReportID=spReportID, spcommFailure=spcommFailure, peakflowSPMgr=peakflowSPMgr, cloudSignalingFault=cloudSignalingFault, peakflowSPMIB=peakflowSPMIB, spDNSObserved=spDNSObserved, spFingerprintMessage=spFingerprintMessage, spUnit=spUnit, spMitigationID=spMitigationID, deviceTotalFlows=deviceTotalFlows, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, licenseError=licenseError, dosNetworkProfiledAlert=dosNetworkProfiledAlert, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBlackholeNexthop=spBlackholeNexthop, spManagedObjects=spManagedObjects, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, bgpInstabilityDone=bgpInstabilityDone, fingerprintUsage=fingerprintUsage, spInterfaceIndex=spInterfaceIndex, bgpHijack=bgpHijack, spReportName=spReportName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, peakflowSPTraps=peakflowSPTraps, interfaceUsage=interfaceUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, routingFailover=routingFailover, spDetectedCountries=spDetectedCountries, mitigationBlackholeStart=mitigationBlackholeStart, bgpInstability=bgpInstability, spManagedObjectFamily=spManagedObjectFamily, spInterface=spInterface, spInetAddressType=spInetAddressType, greDownDone=greDownDone, spInterfaceUsageHC=spInterfaceUsageHC, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spAlertID=spAlertID, greDown=greDown, spTMSTimeout=spTMSTimeout, spThirdPartyAddr=spThirdPartyAddr, spScriptStart=spScriptStart, deviceTotalFlowsHC=deviceTotalFlowsHC, flowDown=flowDown, spCommFailureDescription=spCommFailureDescription, spHijackAttr=spHijackAttr, spGreTunnelDestination=spGreTunnelDestination, spMitigationName=spMitigationName, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spManagedObject=spManagedObject, spLicenseErrDescription=spLicenseErrDescription, fingerprintReceive=fingerprintReceive, fingerprintFeedback=fingerprintFeedback, hardwareFailureDone=hardwareFailureDone, spFlowspecCommunity=spFlowspecCommunity, spUsage=spUsage, mitigationTMSStartV2=mitigationTMSStartV2, spScriptPort=spScriptPort, reportDone=reportDone, spTMSPrefix=spTMSPrefix, spScriptCommand=spScriptCommand, interfaceUsageDone=interfaceUsageDone, spUsageType=spUsageType, spImpactBps=spImpactBps, spBGPEvent=spBGPEvent, spApplicationName=spApplicationName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDNSName=spDNSName, snmpUp=snmpUp, spHijackLocal=spHijackLocal, collectorUp=collectorUp, licenseErrorDone=licenseErrorDone, mitigationDone=mitigationDone, deviceSystemError=deviceSystemError)
