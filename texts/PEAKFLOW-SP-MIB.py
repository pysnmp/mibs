...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, fingerprintFeedback=fingerprintFeedback, mitigationBlackholeStart=mitigationBlackholeStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, dosNetworkProfiledAlert=dosNetworkProfiledAlert, collectorUp=collectorUp, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spFingerprintSender=spFingerprintSender, managedObjectUsage=managedObjectUsage, spReportID=spReportID, deviceTotalFlows=deviceTotalFlows, spManagedObjects=spManagedObjects, spUsageType=spUsageType, spSystemErrorType=spSystemErrorType, spInetAddressType=spInetAddressType, spCommFailureDestination=spCommFailureDestination, fingerprintUsage=fingerprintUsage, spInterface=spInterface, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spMitigationID=spMitigationID, spScriptPort=spScriptPort, interfaceUsageDone=interfaceUsageDone, cloudSignalingFault=cloudSignalingFault, spDetector=spDetector, mitigationTMSStart=mitigationTMSStart, spDNSObservedMean=spDNSObservedMean, mitigationTMSStartV2=mitigationTMSStartV2, peakflowSPCMI=peakflowSPCMI, notificationLimit=notificationLimit, spDuration=spDuration, spLicenseErrDescription=spLicenseErrDescription, spManagedObject=spManagedObject, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceHealth=deviceHealth, bgpHijack=bgpHijack, autoclassifyStart=autoclassifyStart, deviceSwapSpace=deviceSwapSpace, serviceUsage=serviceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, greDown=greDown, collectorDown=collectorDown, flowDown=flowDown, spLicenseErrCount=spLicenseErrCount, spThreshold=spThreshold, spCollector=spCollector, collectorStart=collectorStart, spTMSPrefix=spTMSPrefix, bgpUp=bgpUp, spBlackholeNexthop=spBlackholeNexthop, spServiceElement=spServiceElement, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spDNSObservedMax=spDNSObservedMax, spBlackholeTimeout=spBlackholeTimeout, interfaceUsage=interfaceUsage, spRouter=spRouter, licenseError=licenseError, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spApplicationName=spApplicationName, licenseErrorDone=licenseErrorDone, spFingerprintFeedback=spFingerprintFeedback, spBGPTrapEvent=spBGPTrapEvent, spHijackRoute=spHijackRoute, spImpactPps=spImpactPps, spScriptCommand=spScriptCommand, mitigationFlowspecStart=mitigationFlowspecStart, snmpUp=snmpUp, spManagedObjectFamily=spManagedObjectFamily, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, bgpDown=bgpDown, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spDNSName=spDNSName, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInetAddress=spInetAddress, bgpTrap=bgpTrap, spBGPEvent=spBGPEvent, spCommFailureDescription=spCommFailureDescription, spInterfaceSpeed=spInterfaceSpeed, devicePhysicalMemory=devicePhysicalMemory, serviceUsageDone=serviceUsageDone, spBlackholeCommunity=spBlackholeCommunity, spBGPInstability=spBGPInstability, spBGPTrapPrefix=spBGPTrapPrefix, bgpInstability=bgpInstability, dnsBaseline=dnsBaseline, dosHostDetectionAlert=dosHostDetectionAlert, routingFailover=routingFailover, spMitigationName=spMitigationName, spAlertID=spAlertID, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spGreTunnelName=spGreTunnelName, spFlowspecCommunity=spFlowspecCommunity, spInterfaceUsage=spInterfaceUsage, spLicenseErrType=spLicenseErrType, configChange=configChange, spFingerprintName=spFingerprintName, spHardwareFailureDescription=spHardwareFailureDescription, greDownDone=greDownDone, spPravail=spPravail, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, hardwareFailure=hardwareFailure, managedObjectUsageDone=managedObjectUsageDone, spScriptHost=spScriptHost, spScriptStart=spScriptStart, spDNSObserved=spDNSObserved, spHijackLocal=spHijackLocal, spReportURL=spReportURL, spMitigationStart=spMitigationStart, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spThirdPartyAddr=spThirdPartyAddr, spTMSMultiPrefix=spTMSMultiPrefix, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, deviceSystemErrorDone=deviceSystemErrorDone, spFlowspecTimeout=spFlowspecTimeout, peakflowSPMIB=peakflowSPMIB, spcommFailure=spcommFailure, spServiceName=spServiceName, spDNSExpected=spDNSExpected, spThirdPartyZone=spThirdPartyZone, flowUp=flowUp, spcommFailureDone=spcommFailureDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDetectedCountries=spDetectedCountries, deviceDiskUsage=deviceDiskUsage, fingerprintReceive=fingerprintReceive, trafficAutoMitigation=trafficAutoMitigation, deviceSystemError=deviceSystemError, spImpactBps=spImpactBps, alertScript=alertScript, spUnit=spUnit, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spSystemErrorDescription=spSystemErrorDescription, mitigationDone=mitigationDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceTotalFlowsHC=deviceTotalFlowsHC, spGreTunnelDestination=spGreTunnelDestination, hardwareFailureDone=hardwareFailureDone, spLicenseErrLimit=spLicenseErrLimit, bgpInstabilityDone=bgpInstabilityDone, spTMSCommunity=spTMSCommunity, peakflowSPMgr=peakflowSPMgr, dnsBaselineDone=dnsBaselineDone, snmpDown=snmpDown, spUsage=spUsage, spReportName=spReportName, spInterfaceUsageHC=spInterfaceUsageHC, spInterfaceSpeedHC=spInterfaceSpeedHC, peakflowSPTraps=peakflowSPTraps, spUsername=spUsername, spBGPTrapName=spBGPTrapName, spHijackAttr=spHijackAttr, spVersion=spVersion, spFingerprintMessage=spFingerprintMessage, mitigationThirdPartyStart=mitigationThirdPartyStart, fingerprintUsageDone=fingerprintUsageDone, reportDone=reportDone, spTMSTimeout=spTMSTimeout, spInterfaceIndex=spInterfaceIndex, spBlackholePrefix=spBlackholePrefix)
