...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBGPTrapEvent=spBGPTrapEvent, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFlowspecTimeout=spFlowspecTimeout, spManagedObject=spManagedObject, routingFailover=routingFailover, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, peakflowSPMgr=peakflowSPMgr, spThirdPartyZone=spThirdPartyZone, spFingerprintFeedback=spFingerprintFeedback, spcommFailure=spcommFailure, spLicenseErrLimit=spLicenseErrLimit, spUsageType=spUsageType, spCommFailureDestination=spCommFailureDestination, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPEvent=spBGPEvent, peakflowSPCMI=peakflowSPCMI, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceSwapSpaceUsage=deviceSwapSpaceUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spAlertID=spAlertID, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, bgpDown=bgpDown, hardwareFailure=hardwareFailure, licenseError=licenseError, autoclassifyStart=autoclassifyStart, spDNSObservedMax=spDNSObservedMax, spHijackRoute=spHijackRoute, spDuration=spDuration, spLicenseErrType=spLicenseErrType, spInterfaceSpeed=spInterfaceSpeed, spInetAddressType=spInetAddressType, spDNSObserved=spDNSObserved, managedObjectUsage=managedObjectUsage, mitigationDone=mitigationDone, spcommFailureDone=spcommFailureDone, greDown=greDown, spImpactPps=spImpactPps, spDNSExpected=spDNSExpected, spManagedObjectFamily=spManagedObjectFamily, spGreTunnelName=spGreTunnelName, spScriptHost=spScriptHost, notificationLimit=notificationLimit, spLicenseErrDescription=spLicenseErrDescription, devicePhysicalMemory=devicePhysicalMemory, cloudSignalingFault=cloudSignalingFault, deviceTotalFlowsHC=deviceTotalFlowsHC, dosNetworkProfiledAlert=dosNetworkProfiledAlert, mitigationBlackholeStart=mitigationBlackholeStart, spBGPTrapName=spBGPTrapName, spAlertDetectionSignatures=spAlertDetectionSignatures, spHijackAttr=spHijackAttr, spReportName=spReportName, spImpactBps=spImpactBps, deviceDiskUsage=deviceDiskUsage, mitigationTMSStart=mitigationTMSStart, spFingerprintMessage=spFingerprintMessage, spScriptPort=spScriptPort, spInterfaceUsageHC=spInterfaceUsageHC, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, collectorDown=collectorDown, snmpUp=snmpUp, spHijackLocal=spHijackLocal, spMitigationStart=spMitigationStart, spBlackholeCommunity=spBlackholeCommunity, spServiceElement=spServiceElement, spDetectedCountries=spDetectedCountries, mitigationFlowspecStart=mitigationFlowspecStart, trafficAutoMitigation=trafficAutoMitigation, flowUp=flowUp, spUsername=spUsername, spThirdPartyAddr=spThirdPartyAddr, spPravail=spPravail, spSystemErrorDescription=spSystemErrorDescription, bgpHijack=bgpHijack, spGreTunnelDestination=spGreTunnelDestination, spRouter=spRouter, greDownDone=greDownDone, spBlackholeTimeout=spBlackholeTimeout, dosHostDetectionAlert=dosHostDetectionAlert, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spFingerprintName=spFingerprintName, spTMSPrefix=spTMSPrefix, spThreshold=spThreshold, spCommFailureDescription=spCommFailureDescription, deviceSwapSpace=deviceSwapSpace, bgpInstability=bgpInstability, mitigationTMSStartV2=mitigationTMSStartV2, spScriptStart=spScriptStart, cloudSignalingFaultDone=cloudSignalingFaultDone, spTMSCommunity=spTMSCommunity, spBlackholePrefix=spBlackholePrefix, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spManagedObjects=spManagedObjects, dnsBaselineDone=dnsBaselineDone, licenseErrorDone=licenseErrorDone, fingerprintUsageDone=fingerprintUsageDone, deviceTotalFlows=deviceTotalFlows, bgpTrap=bgpTrap, spInterfaceIndex=spInterfaceIndex, spHardwareFailureDescription=spHardwareFailureDescription, collectorUp=collectorUp, bgpInstabilityDone=bgpInstabilityDone, spReportID=spReportID, fingerprintReceive=fingerprintReceive, spServiceName=spServiceName, spReportURL=spReportURL, serviceUsageDone=serviceUsageDone, spDNSName=spDNSName, managedObjectUsageDone=managedObjectUsageDone, spMitigationID=spMitigationID, flowDown=flowDown, deviceHealth=deviceHealth, peakflowSPTraps=peakflowSPTraps, configChange=configChange, PYSNMP_MODULE_ID=peakflowSPMIB, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInterface=spInterface, spDetector=spDetector, fingerprintFeedback=fingerprintFeedback, interfaceUsageDone=interfaceUsageDone, snmpDown=snmpDown, spCloudSignalFaultDescription=spCloudSignalFaultDescription, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, serviceUsage=serviceUsage, spUnit=spUnit, spFingerprintSender=spFingerprintSender, alertScript=alertScript, spSystemErrorType=spSystemErrorType, interfaceUsage=interfaceUsage, spTMSMultiPrefix=spTMSMultiPrefix, spInterfaceUsage=spInterfaceUsage, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, reportDone=reportDone, spApplicationName=spApplicationName, spUsage=spUsage, hardwareFailureDone=hardwareFailureDone, spScriptCommand=spScriptCommand, spFlowspecCommunity=spFlowspecCommunity, spLicenseErrCount=spLicenseErrCount, spInetAddress=spInetAddress, collectorStart=collectorStart, deviceSystemError=deviceSystemError, spBGPInstability=spBGPInstability, dosHostDetectionAlertDone=dosHostDetectionAlertDone, peakflowSPMIB=peakflowSPMIB, bgpUp=bgpUp, spCollector=spCollector, deviceSystemErrorDone=deviceSystemErrorDone, dnsBaseline=dnsBaseline, spDNSObservedMean=spDNSObservedMean, spBlackholeNexthop=spBlackholeNexthop, fingerprintUsage=fingerprintUsage, spTMSTimeout=spTMSTimeout, spInterfaceSpeedHC=spInterfaceSpeedHC, spBGPTrapPrefix=spBGPTrapPrefix, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spMitigationName=spMitigationName, spVersion=spVersion)
