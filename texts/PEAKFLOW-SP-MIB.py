...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", mitigationTMSStart=mitigationTMSStart, deviceSystemError=deviceSystemError, spLicenseErrDescription=spLicenseErrDescription, collectorUp=collectorUp, spDNSObservedMax=spDNSObservedMax, spImpactBps=spImpactBps, spScriptPort=spScriptPort, licenseError=licenseError, cloudSignalingFaultDone=cloudSignalingFaultDone, spDNSExpected=spDNSExpected, spDetectedCountries=spDetectedCountries, fingerprintUsageDone=fingerprintUsageDone, fingerprintReceive=fingerprintReceive, spTMSTimeout=spTMSTimeout, deviceDiskUsage=deviceDiskUsage, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spManagedObjects=spManagedObjects, peakflowSPMgr=peakflowSPMgr, greDown=greDown, spAlertID=spAlertID, spBlackholeCommunity=spBlackholeCommunity, spFingerprintFeedback=spFingerprintFeedback, spInetAddressType=spInetAddressType, reportDone=reportDone, peakflowSPMIB=peakflowSPMIB, spFingerprintSender=spFingerprintSender, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spUsername=spUsername, spHardwareFailureDescription=spHardwareFailureDescription, spUsage=spUsage, spReportID=spReportID, spCommFailureDestination=spCommFailureDestination, deviceHealth=deviceHealth, spBlackholeNexthop=spBlackholeNexthop, licenseErrorDone=licenseErrorDone, bgpInstabilityDone=bgpInstabilityDone, spHijackLocal=spHijackLocal, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spLicenseErrType=spLicenseErrType, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBlackholeTimeout=spBlackholeTimeout, peakflowSPTraps=peakflowSPTraps, managedObjectUsage=managedObjectUsage, spImpactPps=spImpactPps, spInetAddress=spInetAddress, dnsBaselineDone=dnsBaselineDone, spSystemErrorType=spSystemErrorType, bgpTrap=bgpTrap, spFingerprintName=spFingerprintName, spThreshold=spThreshold, spGreTunnelName=spGreTunnelName, spInterfaceIndex=spInterfaceIndex, trafficAutoMitigation=trafficAutoMitigation, spMitigationID=spMitigationID, spScriptHost=spScriptHost, fingerprintUsage=fingerprintUsage, spcommFailure=spcommFailure, spHijackAttr=spHijackAttr, spInterfaceSpeedHC=spInterfaceSpeedHC, spTMSMultiPrefix=spTMSMultiPrefix, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spHijackRoute=spHijackRoute, snmpUp=snmpUp, spGreTunnelDestination=spGreTunnelDestination, greDownDone=greDownDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, mitigationTMSStartV2=mitigationTMSStartV2, bgpUp=bgpUp, interfaceUsageDone=interfaceUsageDone, spBlackholePrefix=spBlackholePrefix, mitigationBlackholeStart=mitigationBlackholeStart, spLicenseErrLimit=spLicenseErrLimit, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceSystemErrorDone=deviceSystemErrorDone, deviceTotalFlows=deviceTotalFlows, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, snmpDown=snmpDown, spDetector=spDetector, serviceUsage=serviceUsage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, hardwareFailure=hardwareFailure, spMitigationStart=spMitigationStart, managedObjectUsageDone=managedObjectUsageDone, spReportName=spReportName, spDNSObservedMean=spDNSObservedMean, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, fingerprintFeedback=fingerprintFeedback, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFlowspecCommunity=spFlowspecCommunity, bgpInstability=bgpInstability, spcommFailureDone=spcommFailureDone, spFlowspecTimeout=spFlowspecTimeout, dnsBaseline=dnsBaseline, spThirdPartyAddr=spThirdPartyAddr, peakflowSPCMI=peakflowSPCMI, flowUp=flowUp, configChange=configChange, mitigationDone=mitigationDone, spServiceElement=spServiceElement, spTMSCommunity=spTMSCommunity, spPravail=spPravail, autoclassifyStart=autoclassifyStart, spMitigationName=spMitigationName, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, interfaceUsage=interfaceUsage, spInterfaceUsage=spInterfaceUsage, bgpHijack=bgpHijack, spManagedObjectFamily=spManagedObjectFamily, spCommFailureDescription=spCommFailureDescription, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, serviceUsageDone=serviceUsageDone, spBGPTrapEvent=spBGPTrapEvent, mitigationFlowspecStart=mitigationFlowspecStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spServiceName=spServiceName, deviceTotalFlowsHC=deviceTotalFlowsHC, spThirdPartyZone=spThirdPartyZone, spScriptStart=spScriptStart, spInterface=spInterface, spSystemErrorDescription=spSystemErrorDescription, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBGPInstability=spBGPInstability, deviceSwapSpace=deviceSwapSpace, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, alertScript=alertScript, spTMSPrefix=spTMSPrefix, spBGPEvent=spBGPEvent, cloudSignalingFault=cloudSignalingFault, spReportURL=spReportURL, spScriptCommand=spScriptCommand, spApplicationName=spApplicationName, flowDown=flowDown, hardwareFailureDone=hardwareFailureDone, mitigationThirdPartyStart=mitigationThirdPartyStart, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spInterfaceSpeed=spInterfaceSpeed, spRouter=spRouter, bgpDown=bgpDown, devicePhysicalMemory=devicePhysicalMemory, spVersion=spVersion, spDuration=spDuration, spBGPTrapName=spBGPTrapName, collectorStart=collectorStart, spDNSName=spDNSName, spUsageType=spUsageType, spManagedObject=spManagedObject, deviceSwapSpaceInUse=deviceSwapSpaceInUse, notificationLimit=notificationLimit, PYSNMP_MODULE_ID=peakflowSPMIB, spUnit=spUnit, spFingerprintMessage=spFingerprintMessage, spInterfaceUsageHC=spInterfaceUsageHC, routingFailover=routingFailover, collectorDown=collectorDown, spDNSObserved=spDNSObserved, spBGPTrapPrefix=spBGPTrapPrefix, spLicenseErrCount=spLicenseErrCount, spCollector=spCollector, dosHostDetectionAlert=dosHostDetectionAlert)
