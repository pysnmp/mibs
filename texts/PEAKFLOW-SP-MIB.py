...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spLicenseErrDescription=spLicenseErrDescription, dnsBaselineDone=dnsBaselineDone, spPravail=spPravail, mitigationThirdPartyStart=mitigationThirdPartyStart, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, bgpUp=bgpUp, fingerprintUsage=fingerprintUsage, dosHostDetectionAlert=dosHostDetectionAlert, spInterface=spInterface, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, fingerprintFeedback=fingerprintFeedback, spDNSObservedMax=spDNSObservedMax, dosNetworkProfiledAlert=dosNetworkProfiledAlert, serviceUsage=serviceUsage, spImpactPps=spImpactPps, peakflowSPCMI=peakflowSPCMI, spHardwareFailureDescription=spHardwareFailureDescription, spCommFailureDescription=spCommFailureDescription, snmpUp=snmpUp, licenseError=licenseError, spFingerprintSender=spFingerprintSender, spFingerprintMessage=spFingerprintMessage, spFingerprintName=spFingerprintName, alertScript=alertScript, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterfaceUsage=spInterfaceUsage, spDNSObservedMean=spDNSObservedMean, spBGPTrapEvent=spBGPTrapEvent, spBlackholeNexthop=spBlackholeNexthop, spcommFailure=spcommFailure, mitigationBlackholeStart=mitigationBlackholeStart, mitigationTMSStartV2=mitigationTMSStartV2, spRouter=spRouter, greDown=greDown, spSystemErrorType=spSystemErrorType, cloudSignalingFaultDone=cloudSignalingFaultDone, spcommFailureDone=spcommFailureDone, spServiceName=spServiceName, deviceSwapSpace=deviceSwapSpace, spBGPTrapName=spBGPTrapName, bgpTrap=bgpTrap, spInterfaceSpeedHC=spInterfaceSpeedHC, spReportID=spReportID, deviceSwapSpaceUsage=deviceSwapSpaceUsage, trafficAutoMitigation=trafficAutoMitigation, autoclassifyStart=autoclassifyStart, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spGreTunnelName=spGreTunnelName, spDetectedCountries=spDetectedCountries, snmpDown=snmpDown, peakflowSPMgr=peakflowSPMgr, collectorDown=collectorDown, cloudSignalingFault=cloudSignalingFault, deviceTotalFlows=deviceTotalFlows, spUsage=spUsage, hardwareFailure=hardwareFailure, spManagedObjects=spManagedObjects, spReportName=spReportName, spDNSExpected=spDNSExpected, configChange=configChange, spUnit=spUnit, spMitigationID=spMitigationID, deviceHealth=deviceHealth, spLicenseErrCount=spLicenseErrCount, spInterfaceSpeed=spInterfaceSpeed, spScriptPort=spScriptPort, peakflowSPMIB=peakflowSPMIB, bgpDown=bgpDown, spCollector=spCollector, spTMSPrefix=spTMSPrefix, collectorStart=collectorStart, spTMSMultiPrefix=spTMSMultiPrefix, deviceSystemError=deviceSystemError, spApplicationName=spApplicationName, spThirdPartyZone=spThirdPartyZone, dnsBaseline=dnsBaseline, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, mitigationDone=mitigationDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, devicePhysicalMemory=devicePhysicalMemory, spMitigationName=spMitigationName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spBGPTrapPrefix=spBGPTrapPrefix, mitigationFlowspecStart=mitigationFlowspecStart, fingerprintUsageDone=fingerprintUsageDone, spImpactBps=spImpactBps, spScriptCommand=spScriptCommand, spBlackholeTimeout=spBlackholeTimeout, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, bgpInstabilityDone=bgpInstabilityDone, bgpHijack=bgpHijack, managedObjectUsageDone=managedObjectUsageDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, licenseErrorDone=licenseErrorDone, spScriptHost=spScriptHost, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, reportDone=reportDone, spCommFailureDestination=spCommFailureDestination, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spInetAddressType=spInetAddressType, spBGPEvent=spBGPEvent, spDNSName=spDNSName, spTMSTimeout=spTMSTimeout, interfaceUsageDone=interfaceUsageDone, spLicenseErrLimit=spLicenseErrLimit, spThirdPartyAddr=spThirdPartyAddr, spThreshold=spThreshold, spSystemErrorDescription=spSystemErrorDescription, peakflowSPTraps=peakflowSPTraps, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, managedObjectUsage=managedObjectUsage, greDownDone=greDownDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, flowUp=flowUp, spFlowspecCommunity=spFlowspecCommunity, deviceSystemErrorDone=deviceSystemErrorDone, spInetAddress=spInetAddress, spReportURL=spReportURL, interfaceUsage=interfaceUsage, hardwareFailureDone=hardwareFailureDone, spScriptStart=spScriptStart, serviceUsageDone=serviceUsageDone, spManagedObjectFamily=spManagedObjectFamily, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, mitigationTMSStart=mitigationTMSStart, spDetector=spDetector, spBlackholePrefix=spBlackholePrefix, spManagedObject=spManagedObject, collectorUp=collectorUp, spServiceElement=spServiceElement, spBlackholeCommunity=spBlackholeCommunity, spLicenseErrType=spLicenseErrType, deviceDiskUsage=deviceDiskUsage, spVersion=spVersion, fingerprintReceive=fingerprintReceive, spGreTunnelDestination=spGreTunnelDestination, routingFailover=routingFailover, spHijackLocal=spHijackLocal, spUsername=spUsername, spInterfaceUsageHC=spInterfaceUsageHC, spUsageType=spUsageType, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, flowDown=flowDown, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spAlertDetectionSignatures=spAlertDetectionSignatures, spFingerprintFeedback=spFingerprintFeedback, spDuration=spDuration, spHijackAttr=spHijackAttr, spHijackRoute=spHijackRoute, spInterfaceIndex=spInterfaceIndex, spBGPInstability=spBGPInstability, spDNSObserved=spDNSObserved, spMitigationStart=spMitigationStart, PYSNMP_MODULE_ID=peakflowSPMIB, spTMSCommunity=spTMSCommunity, spAlertID=spAlertID, spFlowspecTimeout=spFlowspecTimeout, notificationLimit=notificationLimit, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpInstability=bgpInstability)
