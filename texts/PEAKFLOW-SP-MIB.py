...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBlackholePrefix=spBlackholePrefix, spTMSTimeout=spTMSTimeout, peakflowSPMIB=peakflowSPMIB, deviceSwapSpace=deviceSwapSpace, spDetector=spDetector, spHijackRoute=spHijackRoute, spUnit=spUnit, spApplicationName=spApplicationName, greDown=greDown, snmpDown=snmpDown, spHijackAttr=spHijackAttr, spCollector=spCollector, cloudSignalingFault=cloudSignalingFault, devicePhysicalMemory=devicePhysicalMemory, spFlowspecTimeout=spFlowspecTimeout, serviceUsageDone=serviceUsageDone, reportDone=reportDone, spTMSMultiPrefix=spTMSMultiPrefix, spInetAddress=spInetAddress, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spGreTunnelName=spGreTunnelName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, fingerprintUsage=fingerprintUsage, spUsage=spUsage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spVersion=spVersion, spDNSObservedMax=spDNSObservedMax, spCommFailureDestination=spCommFailureDestination, spAlertDetectionSignatures=spAlertDetectionSignatures, flowDown=flowDown, spBGPTrapPrefix=spBGPTrapPrefix, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpInstabilityDone=bgpInstabilityDone, spScriptHost=spScriptHost, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBGPInstability=spBGPInstability, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceSystemError=deviceSystemError, spBGPEvent=spBGPEvent, alertScript=alertScript, spInterfaceIndex=spInterfaceIndex, managedObjectUsageDone=managedObjectUsageDone, spManagedObject=spManagedObject, mitigationFlowspecStart=mitigationFlowspecStart, snmpUp=snmpUp, spServiceName=spServiceName, spLicenseErrLimit=spLicenseErrLimit, spManagedObjectFamily=spManagedObjectFamily, spcommFailureDone=spcommFailureDone, fingerprintFeedback=fingerprintFeedback, spServiceElement=spServiceElement, deviceTotalFlows=deviceTotalFlows, spUsageType=spUsageType, spTMSPrefix=spTMSPrefix, spThirdPartyAddr=spThirdPartyAddr, spCommFailureDescription=spCommFailureDescription, spBlackholeNexthop=spBlackholeNexthop, spDNSExpected=spDNSExpected, spBGPTrapEvent=spBGPTrapEvent, spLicenseErrDescription=spLicenseErrDescription, bgpDown=bgpDown, spSystemErrorDescription=spSystemErrorDescription, autoclassifyStart=autoclassifyStart, spMitigationID=spMitigationID, spScriptPort=spScriptPort, spBlackholeTimeout=spBlackholeTimeout, managedObjectUsage=managedObjectUsage, spSystemErrorType=spSystemErrorType, spThirdPartyZone=spThirdPartyZone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceSwapSpaceInUse=deviceSwapSpaceInUse, mitigationDone=mitigationDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spImpactBps=spImpactBps, spHijackLocal=spHijackLocal, peakflowSPMgr=peakflowSPMgr, peakflowSPCMI=peakflowSPCMI, spFingerprintFeedback=spFingerprintFeedback, spScriptStart=spScriptStart, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, collectorStart=collectorStart, spFingerprintMessage=spFingerprintMessage, spInterfaceUsage=spInterfaceUsage, interfaceUsage=interfaceUsage, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceDiskUsage=deviceDiskUsage, spLicenseErrType=spLicenseErrType, spDNSName=spDNSName, interfaceUsageDone=interfaceUsageDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spAlertID=spAlertID, deviceHealth=deviceHealth, bgpHijack=bgpHijack, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, bgpUp=bgpUp, mitigationThirdPartyStart=mitigationThirdPartyStart, bgpTrap=bgpTrap, spBGPTrapName=spBGPTrapName, spThreshold=spThreshold, hardwareFailureDone=hardwareFailureDone, dosHostDetectionAlert=dosHostDetectionAlert, spBGPTrapNewAttributes=spBGPTrapNewAttributes, trafficAutoMitigation=trafficAutoMitigation, cloudSignalingFaultDone=cloudSignalingFaultDone, hardwareFailure=hardwareFailure, spDNSObservedMean=spDNSObservedMean, spScriptCommand=spScriptCommand, spInterfaceSpeed=spInterfaceSpeed, spDNSObserved=spDNSObserved, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spUsername=spUsername, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spTMSCommunity=spTMSCommunity, spInterface=spInterface, peakflowSPTraps=peakflowSPTraps, spImpactPps=spImpactPps, spDetectedCountries=spDetectedCountries, bgpInstability=bgpInstability, serviceUsage=serviceUsage, dnsBaselineDone=dnsBaselineDone, mitigationBlackholeStart=mitigationBlackholeStart, notificationLimit=notificationLimit, fingerprintUsageDone=fingerprintUsageDone, spRouter=spRouter, spPravail=spPravail, greDownDone=greDownDone, configChange=configChange, spFlowspecCommunity=spFlowspecCommunity, spGreTunnelDestination=spGreTunnelDestination, spManagedObjects=spManagedObjects, spReportID=spReportID, fingerprintReceive=fingerprintReceive, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spFingerprintName=spFingerprintName, spInetAddressType=spInetAddressType, dnsBaseline=dnsBaseline, routingFailover=routingFailover, spInterfaceUsageHC=spInterfaceUsageHC, mitigationTMSStart=mitigationTMSStart, PYSNMP_MODULE_ID=peakflowSPMIB, spBlackholeCommunity=spBlackholeCommunity, licenseError=licenseError, spMitigationName=spMitigationName, spHardwareFailureDescription=spHardwareFailureDescription, mitigationTMSStartV2=mitigationTMSStartV2, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spcommFailure=spcommFailure, spFingerprintSender=spFingerprintSender, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spLicenseErrCount=spLicenseErrCount, collectorUp=collectorUp, flowUp=flowUp, spReportURL=spReportURL, collectorDown=collectorDown, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceSystemErrorDone=deviceSystemErrorDone, spDuration=spDuration, spReportName=spReportName, spMitigationStart=spMitigationStart, licenseErrorDone=licenseErrorDone)
