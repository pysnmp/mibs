...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spHijackAttr=spHijackAttr, licenseError=licenseError, bgpDown=bgpDown, dosNetworkProfiledAlert=dosNetworkProfiledAlert, mitigationTMSStart=mitigationTMSStart, flowDown=flowDown, spBGPEvent=spBGPEvent, spSystemErrorDescription=spSystemErrorDescription, spServiceElement=spServiceElement, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spMitigationID=spMitigationID, spScriptPort=spScriptPort, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFingerprintSender=spFingerprintSender, trafficAutoMitigation=trafficAutoMitigation, spBGPTrapOldAttributes=spBGPTrapOldAttributes, reportDone=reportDone, spManagedObjectFamily=spManagedObjectFamily, deviceHealth=deviceHealth, spUnit=spUnit, collectorDown=collectorDown, spCollector=spCollector, deviceDiskUsage=deviceDiskUsage, spcommFailure=spcommFailure, configChange=configChange, spBlackholePrefix=spBlackholePrefix, spBGPTrapEvent=spBGPTrapEvent, spBlackholeTimeout=spBlackholeTimeout, peakflowSPTraps=peakflowSPTraps, bgpInstability=bgpInstability, greDown=greDown, licenseErrorDone=licenseErrorDone, hardwareFailure=hardwareFailure, autoclassifyStart=autoclassifyStart, spBGPInstability=spBGPInstability, spManagedObjects=spManagedObjects, deviceSwapSpaceInUse=deviceSwapSpaceInUse, devicePhysicalMemory=devicePhysicalMemory, flowUp=flowUp, spThirdPartyAddr=spThirdPartyAddr, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBlackholeCommunity=spBlackholeCommunity, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, peakflowSPCMI=peakflowSPCMI, spTMSMultiPrefix=spTMSMultiPrefix, spInetAddress=spInetAddress, spInterfaceSpeed=spInterfaceSpeed, spDNSName=spDNSName, fingerprintReceive=fingerprintReceive, snmpUp=snmpUp, spHijackLocal=spHijackLocal, spReportURL=spReportURL, bgpUp=bgpUp, spUsageType=spUsageType, spBlackholeNexthop=spBlackholeNexthop, spServiceName=spServiceName, spThreshold=spThreshold, spImpactPps=spImpactPps, spDetectedCountries=spDetectedCountries, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, managedObjectUsage=managedObjectUsage, spInterfaceIndex=spInterfaceIndex, spLicenseErrCount=spLicenseErrCount, fingerprintUsage=fingerprintUsage, bgpInstabilityDone=bgpInstabilityDone, spDuration=spDuration, spFingerprintName=spFingerprintName, spAlertDetectionSignatures=spAlertDetectionSignatures, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spManagedObject=spManagedObject, spGreTunnelDestination=spGreTunnelDestination, interfaceUsage=interfaceUsage, spInterfaceUsage=spInterfaceUsage, managedObjectUsageDone=managedObjectUsageDone, spCommFailureDescription=spCommFailureDescription, spApplicationName=spApplicationName, spDetector=spDetector, spInterfaceUsageHC=spInterfaceUsageHC, spInterface=spInterface, deviceSystemError=deviceSystemError, spGreTunnelName=spGreTunnelName, spFingerprintFeedback=spFingerprintFeedback, spFlowspecTimeout=spFlowspecTimeout, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceSystemErrorDone=deviceSystemErrorDone, fingerprintUsageDone=fingerprintUsageDone, spHardwareFailureDescription=spHardwareFailureDescription, spDNSObservedMean=spDNSObservedMean, spLicenseErrType=spLicenseErrType, spHijackRoute=spHijackRoute, spTMSPrefix=spTMSPrefix, dosHostDetectionAlert=dosHostDetectionAlert, spReportName=spReportName, hardwareFailureDone=hardwareFailureDone, spDNSObserved=spDNSObserved, bgpHijack=bgpHijack, spScriptCommand=spScriptCommand, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spAlertID=spAlertID, spBGPTrapPrefix=spBGPTrapPrefix, spReportID=spReportID, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spThirdPartyZone=spThirdPartyZone, spcommFailureDone=spcommFailureDone, spMitigationName=spMitigationName, spMitigationStart=spMitigationStart, deviceTotalFlowsHC=deviceTotalFlowsHC, spCommFailureDestination=spCommFailureDestination, dnsBaseline=dnsBaseline, serviceUsage=serviceUsage, bgpTrap=bgpTrap, spLicenseErrLimit=spLicenseErrLimit, interfaceUsageDone=interfaceUsageDone, spSystemErrorType=spSystemErrorType, peakflowSPMgr=peakflowSPMgr, snmpDown=snmpDown, spCloudSignalFaultDescription=spCloudSignalFaultDescription, serviceUsageDone=serviceUsageDone, spTMSCommunity=spTMSCommunity, dnsBaselineDone=dnsBaselineDone, spDNSObservedMax=spDNSObservedMax, alertScript=alertScript, deviceTotalFlows=deviceTotalFlows, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, PYSNMP_MODULE_ID=peakflowSPMIB, cloudSignalingFaultDone=cloudSignalingFaultDone, spDNSExpected=spDNSExpected, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceSwapSpace=deviceSwapSpace, routingFailover=routingFailover, spFlowspecCommunity=spFlowspecCommunity, spScriptStart=spScriptStart, collectorUp=collectorUp, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, greDownDone=greDownDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spImpactBps=spImpactBps, spTMSTimeout=spTMSTimeout, mitigationBlackholeStart=mitigationBlackholeStart, spUsage=spUsage, mitigationTMSStartV2=mitigationTMSStartV2, spBGPTrapName=spBGPTrapName, spScriptHost=spScriptHost, notificationLimit=notificationLimit, spFingerprintMessage=spFingerprintMessage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, collectorStart=collectorStart, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spUsername=spUsername, spPravail=spPravail, mitigationDone=mitigationDone, peakflowSPMIB=peakflowSPMIB, spLicenseErrDescription=spLicenseErrDescription, mitigationFlowspecStart=mitigationFlowspecStart, fingerprintFeedback=fingerprintFeedback, spVersion=spVersion, mitigationThirdPartyStart=mitigationThirdPartyStart, cloudSignalingFault=cloudSignalingFault, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spInetAddressType=spInetAddressType, spRouter=spRouter)
