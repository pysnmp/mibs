...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDNSObservedMean=spDNSObservedMean, spUnit=spUnit, dnsBaselineDone=dnsBaselineDone, fingerprintUsageDone=fingerprintUsageDone, spHijackAttr=spHijackAttr, reportDone=reportDone, serviceUsageDone=serviceUsageDone, licenseError=licenseError, spHardwareFailureDescription=spHardwareFailureDescription, PYSNMP_MODULE_ID=peakflowSPMIB, spMitigationName=spMitigationName, spBlackholeNexthop=spBlackholeNexthop, spInterfaceIndex=spInterfaceIndex, spManagedObjectFamily=spManagedObjectFamily, spCommFailureDescription=spCommFailureDescription, spTMSMultiPrefix=spTMSMultiPrefix, peakflowSPMIB=peakflowSPMIB, dnsBaseline=dnsBaseline, cloudSignalingFaultDone=cloudSignalingFaultDone, spFlowspecTimeout=spFlowspecTimeout, spPravail=spPravail, cloudSignalingFault=cloudSignalingFault, peakflowSPMgr=peakflowSPMgr, deviceSystemError=deviceSystemError, collectorDown=collectorDown, flowUp=flowUp, spSystemErrorType=spSystemErrorType, spGreTunnelName=spGreTunnelName, hardwareFailure=hardwareFailure, mitigationTMSStartV2=mitigationTMSStartV2, spAlertDetectionSignatures=spAlertDetectionSignatures, configChange=configChange, spBGPEvent=spBGPEvent, spReportName=spReportName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, collectorUp=collectorUp, spAlertID=spAlertID, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, mitigationTMSStart=mitigationTMSStart, snmpDown=snmpDown, spcommFailure=spcommFailure, flowDown=flowDown, deviceHealth=deviceHealth, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spHijackLocal=spHijackLocal, spTMSTimeout=spTMSTimeout, spSystemErrorDescription=spSystemErrorDescription, spDetectedCountries=spDetectedCountries, spThirdPartyZone=spThirdPartyZone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spFingerprintMessage=spFingerprintMessage, spBGPInstability=spBGPInstability, spInetAddressType=spInetAddressType, spBlackholePrefix=spBlackholePrefix, deviceSwapSpace=deviceSwapSpace, devicePhysicalMemory=devicePhysicalMemory, spInterfaceUsage=spInterfaceUsage, spManagedObject=spManagedObject, spDNSObserved=spDNSObserved, greDown=greDown, bgpTrap=bgpTrap, serviceUsage=serviceUsage, spLicenseErrType=spLicenseErrType, spInetAddress=spInetAddress, trafficAutoMitigation=trafficAutoMitigation, autoclassifyStart=autoclassifyStart, spFingerprintFeedback=spFingerprintFeedback, mitigationDone=mitigationDone, mitigationThirdPartyStart=mitigationThirdPartyStart, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spImpactPps=spImpactPps, spVersion=spVersion, spInterfaceSpeedHC=spInterfaceSpeedHC, snmpUp=snmpUp, spServiceElement=spServiceElement, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, managedObjectUsage=managedObjectUsage, fingerprintReceive=fingerprintReceive, spFlowspecCommunity=spFlowspecCommunity, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, bgpHijack=bgpHijack, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spScriptStart=spScriptStart, deviceDiskUsage=deviceDiskUsage, spManagedObjects=spManagedObjects, spMitigationStart=spMitigationStart, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapNewAttributes=spBGPTrapNewAttributes, licenseErrorDone=licenseErrorDone, spCollector=spCollector, spInterfaceUsageHC=spInterfaceUsageHC, spScriptHost=spScriptHost, spFingerprintSender=spFingerprintSender, spUsername=spUsername, bgpInstabilityDone=bgpInstabilityDone, spMitigationID=spMitigationID, spHijackRoute=spHijackRoute, interfaceUsage=interfaceUsage, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spLicenseErrLimit=spLicenseErrLimit, routingFailover=routingFailover, dosHostDetectionAlert=dosHostDetectionAlert, spGreTunnelDestination=spGreTunnelDestination, spBGPTrapName=spBGPTrapName, spDetector=spDetector, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceTotalFlows=deviceTotalFlows, spRouter=spRouter, mitigationBlackholeStart=mitigationBlackholeStart, spcommFailureDone=spcommFailureDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, bgpInstability=bgpInstability, hardwareFailureDone=hardwareFailureDone, managedObjectUsageDone=managedObjectUsageDone, spBGPTrapPrefix=spBGPTrapPrefix, spTMSCommunity=spTMSCommunity, fingerprintFeedback=fingerprintFeedback, fingerprintUsage=fingerprintUsage, notificationLimit=notificationLimit, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spDuration=spDuration, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spInterface=spInterface, spInterfaceSpeed=spInterfaceSpeed, collectorStart=collectorStart, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, deviceSystemErrorDone=deviceSystemErrorDone, spServiceName=spServiceName, spScriptCommand=spScriptCommand, interfaceUsageDone=interfaceUsageDone, spScriptPort=spScriptPort, spCommFailureDestination=spCommFailureDestination, spThirdPartyAddr=spThirdPartyAddr, peakflowSPTraps=peakflowSPTraps, spFingerprintName=spFingerprintName, spTMSPrefix=spTMSPrefix, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBlackholeCommunity=spBlackholeCommunity, alertScript=alertScript, spDNSName=spDNSName, spReportID=spReportID, spBGPTrapEvent=spBGPTrapEvent, greDownDone=greDownDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spThreshold=spThreshold, peakflowSPCMI=peakflowSPCMI, spLicenseErrCount=spLicenseErrCount, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDNSObservedMax=spDNSObservedMax, bgpUp=bgpUp, mitigationFlowspecStart=mitigationFlowspecStart, spImpactBps=spImpactBps, spReportURL=spReportURL, deviceTotalFlowsHC=deviceTotalFlowsHC, spApplicationName=spApplicationName, spUsage=spUsage, spDNSExpected=spDNSExpected, spBlackholeTimeout=spBlackholeTimeout, spUsageType=spUsageType, bgpDown=bgpDown, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone)
