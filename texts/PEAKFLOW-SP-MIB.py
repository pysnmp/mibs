...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", mitigationFlowspecStart=mitigationFlowspecStart, spGreTunnelName=spGreTunnelName, mitigationBlackholeStart=mitigationBlackholeStart, spTMSTimeout=spTMSTimeout, mitigationDone=mitigationDone, spInterfaceUsage=spInterfaceUsage, spThirdPartyZone=spThirdPartyZone, devicePhysicalMemory=devicePhysicalMemory, spManagedObjectFamily=spManagedObjectFamily, licenseErrorDone=licenseErrorDone, bgpDown=bgpDown, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, hardwareFailureDone=hardwareFailureDone, configChange=configChange, spcommFailure=spcommFailure, spTMSPrefix=spTMSPrefix, spImpactBps=spImpactBps, snmpDown=snmpDown, spDetector=spDetector, fingerprintReceive=fingerprintReceive, spGreTunnelDestination=spGreTunnelDestination, collectorDown=collectorDown, deviceDiskUsage=deviceDiskUsage, dosNetworkProfiledAlert=dosNetworkProfiledAlert, dnsBaseline=dnsBaseline, deviceSwapSpaceUsage=deviceSwapSpaceUsage, bgpInstabilityDone=bgpInstabilityDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spFingerprintMessage=spFingerprintMessage, spRouter=spRouter, notificationLimit=notificationLimit, serviceUsage=serviceUsage, spScriptPort=spScriptPort, spReportName=spReportName, dnsBaselineDone=dnsBaselineDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFlowspecCommunity=spFlowspecCommunity, spLicenseErrLimit=spLicenseErrLimit, spManagedObjects=spManagedObjects, bgpUp=bgpUp, fingerprintFeedback=fingerprintFeedback, spApplicationName=spApplicationName, spMitigationStart=spMitigationStart, spHijackAttr=spHijackAttr, hardwareFailure=hardwareFailure, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spFlowspecTimeout=spFlowspecTimeout, dosHostDetectionAlert=dosHostDetectionAlert, spInterface=spInterface, spManagedObject=spManagedObject, bgpHijack=bgpHijack, spUsername=spUsername, deviceSwapSpace=deviceSwapSpace, interfaceUsageDone=interfaceUsageDone, mitigationTMSStart=mitigationTMSStart, spInterfaceSpeedHC=spInterfaceSpeedHC, spScriptCommand=spScriptCommand, collectorStart=collectorStart, spCommFailureDestination=spCommFailureDestination, managedObjectUsage=managedObjectUsage, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spCloudSignalFaultDescription=spCloudSignalFaultDescription, flowDown=flowDown, collectorUp=collectorUp, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPEvent=spBGPEvent, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spReportID=spReportID, spSystemErrorType=spSystemErrorType, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, peakflowSPMgr=peakflowSPMgr, spAlertID=spAlertID, spBGPTrapEvent=spBGPTrapEvent, greDownDone=greDownDone, alertScript=alertScript, spInterfaceSpeed=spInterfaceSpeed, spBGPTrapPrefix=spBGPTrapPrefix, autoclassifyStart=autoclassifyStart, spFingerprintFeedback=spFingerprintFeedback, spVersion=spVersion, deviceHealth=deviceHealth, spHardwareFailureDescription=spHardwareFailureDescription, spScriptHost=spScriptHost, interfaceUsage=interfaceUsage, spTMSCommunity=spTMSCommunity, spBlackholeTimeout=spBlackholeTimeout, fingerprintUsageDone=fingerprintUsageDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spThreshold=spThreshold, deviceSystemErrorDone=deviceSystemErrorDone, spUnit=spUnit, spMitigationName=spMitigationName, spDNSObservedMax=spDNSObservedMax, spcommFailureDone=spcommFailureDone, bgpInstability=bgpInstability, greDown=greDown, spBlackholePrefix=spBlackholePrefix, spDNSExpected=spDNSExpected, spScriptStart=spScriptStart, spInetAddress=spInetAddress, trafficAutoMitigation=trafficAutoMitigation, spDNSName=spDNSName, bgpTrap=bgpTrap, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, peakflowSPCMI=peakflowSPCMI, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, mitigationTMSStartV2=mitigationTMSStartV2, spServiceElement=spServiceElement, peakflowSPMIB=peakflowSPMIB, cloudSignalingFault=cloudSignalingFault, managedObjectUsageDone=managedObjectUsageDone, peakflowSPTraps=peakflowSPTraps, reportDone=reportDone, spInetAddressType=spInetAddressType, spBGPTrapName=spBGPTrapName, cloudSignalingFaultDone=cloudSignalingFaultDone, spUsageType=spUsageType, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spCommFailureDescription=spCommFailureDescription, spBlackholeCommunity=spBlackholeCommunity, spLicenseErrType=spLicenseErrType, spHijackRoute=spHijackRoute, spHijackLocal=spHijackLocal, spReportURL=spReportURL, spPravail=spPravail, deviceTotalFlowsHC=deviceTotalFlowsHC, spDNSObservedMean=spDNSObservedMean, spLicenseErrDescription=spLicenseErrDescription, licenseError=licenseError, deviceSwapSpaceInUse=deviceSwapSpaceInUse, serviceUsageDone=serviceUsageDone, flowUp=flowUp, spImpactPps=spImpactPps, spLicenseErrCount=spLicenseErrCount, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spCollector=spCollector, spBGPInstability=spBGPInstability, spMitigationID=spMitigationID, fingerprintUsage=fingerprintUsage, spDuration=spDuration, spFingerprintName=spFingerprintName, spTMSMultiPrefix=spTMSMultiPrefix, spUsage=spUsage, spSystemErrorDescription=spSystemErrorDescription, spThirdPartyAddr=spThirdPartyAddr, spInterfaceIndex=spInterfaceIndex, spBlackholeNexthop=spBlackholeNexthop, deviceTotalFlows=deviceTotalFlows, spServiceName=spServiceName, spDNSObserved=spDNSObserved, PYSNMP_MODULE_ID=peakflowSPMIB, spDetectedCountries=spDetectedCountries, routingFailover=routingFailover, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spAlertDetectionSignatures=spAlertDetectionSignatures, spInterfaceUsageHC=spInterfaceUsageHC, deviceSystemError=deviceSystemError, spFingerprintSender=spFingerprintSender, snmpUp=snmpUp)
