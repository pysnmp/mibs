...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDuration=spDuration, spReportName=spReportName, spTMSCommunity=spTMSCommunity, collectorDown=collectorDown, snmpDown=snmpDown, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, deviceSystemErrorDone=deviceSystemErrorDone, spUsageType=spUsageType, peakflowSPTraps=peakflowSPTraps, spThirdPartyZone=spThirdPartyZone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spScriptHost=spScriptHost, managedObjectUsage=managedObjectUsage, dnsBaselineDone=dnsBaselineDone, mitigationThirdPartyStart=mitigationThirdPartyStart, spDetector=spDetector, managedObjectUsageDone=managedObjectUsageDone, spServiceElement=spServiceElement, spInterfaceSpeedHC=spInterfaceSpeedHC, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, trafficAutoMitigation=trafficAutoMitigation, spDetectedCountries=spDetectedCountries, spVersion=spVersion, spInterfaceIndex=spInterfaceIndex, deviceSwapSpaceInUse=deviceSwapSpaceInUse, interfaceUsage=interfaceUsage, spDNSExpected=spDNSExpected, spFlowspecTimeout=spFlowspecTimeout, peakflowSPMIB=peakflowSPMIB, spMitigationStart=spMitigationStart, spUnit=spUnit, fingerprintUsage=fingerprintUsage, spRouter=spRouter, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spUsage=spUsage, spPravail=spPravail, peakflowSPCMI=peakflowSPCMI, autoclassifyStart=autoclassifyStart, spBlackholeTimeout=spBlackholeTimeout, spBlackholeCommunity=spBlackholeCommunity, flowDown=flowDown, serviceUsage=serviceUsage, spTMSTimeout=spTMSTimeout, spFingerprintMessage=spFingerprintMessage, spCommFailureDescription=spCommFailureDescription, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spHardwareFailureDescription=spHardwareFailureDescription, spImpactBps=spImpactBps, spCollector=spCollector, spManagedObjects=spManagedObjects, spInterfaceSpeed=spInterfaceSpeed, deviceTotalFlowsHC=deviceTotalFlowsHC, spInterfaceUsage=spInterfaceUsage, spApplicationName=spApplicationName, mitigationTMSStart=mitigationTMSStart, spBGPInstability=spBGPInstability, spCommFailureDestination=spCommFailureDestination, spUsername=spUsername, spFingerprintSender=spFingerprintSender, spDNSName=spDNSName, spFingerprintFeedback=spFingerprintFeedback, deviceTotalFlows=deviceTotalFlows, fingerprintReceive=fingerprintReceive, spLicenseErrCount=spLicenseErrCount, spHijackAttr=spHijackAttr, deviceDiskUsage=deviceDiskUsage, spFingerprintName=spFingerprintName, spcommFailure=spcommFailure, spMitigationName=spMitigationName, spScriptStart=spScriptStart, spHijackLocal=spHijackLocal, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, alertScript=alertScript, spTMSPrefix=spTMSPrefix, cloudSignalingFaultDone=cloudSignalingFaultDone, spDNSObservedMax=spDNSObservedMax, greDownDone=greDownDone, spFlowspecCommunity=spFlowspecCommunity, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceSwapSpaceUsage=deviceSwapSpaceUsage, routingFailover=routingFailover, mitigationTMSStartV2=mitigationTMSStartV2, spBGPTrapEvent=spBGPTrapEvent, spInetAddress=spInetAddress, collectorStart=collectorStart, interfaceUsageDone=interfaceUsageDone, mitigationFlowspecStart=mitigationFlowspecStart, spBGPEvent=spBGPEvent, spReportURL=spReportURL, bgpHijack=bgpHijack, spcommFailureDone=spcommFailureDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, flowUp=flowUp, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, bgpInstabilityDone=bgpInstabilityDone, dosHostDetectionAlert=dosHostDetectionAlert, peakflowSPMgr=peakflowSPMgr, spInterface=spInterface, serviceUsageDone=serviceUsageDone, spDNSObserved=spDNSObserved, spSystemErrorType=spSystemErrorType, licenseErrorDone=licenseErrorDone, notificationLimit=notificationLimit, spBlackholeNexthop=spBlackholeNexthop, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spAlertDetectionSignatures=spAlertDetectionSignatures, spBlackholePrefix=spBlackholePrefix, spGreTunnelName=spGreTunnelName, configChange=configChange, spThreshold=spThreshold, deviceHealth=deviceHealth, spTMSMultiPrefix=spTMSMultiPrefix, spMitigationID=spMitigationID, devicePhysicalMemory=devicePhysicalMemory, spThirdPartyAddr=spThirdPartyAddr, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, bgpInstability=bgpInstability, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spManagedObject=spManagedObject, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spReportID=spReportID, dnsBaseline=dnsBaseline, spLicenseErrDescription=spLicenseErrDescription, hardwareFailureDone=hardwareFailureDone, spScriptCommand=spScriptCommand, greDown=greDown, spImpactPps=spImpactPps, spHijackRoute=spHijackRoute, collectorUp=collectorUp, spInetAddressType=spInetAddressType, cloudSignalingFault=cloudSignalingFault, spBGPTrapNewAttributes=spBGPTrapNewAttributes, mitigationBlackholeStart=mitigationBlackholeStart, hardwareFailure=hardwareFailure, licenseError=licenseError, spAlertID=spAlertID, PYSNMP_MODULE_ID=peakflowSPMIB, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceSystemError=deviceSystemError, bgpTrap=bgpTrap, mitigationDone=mitigationDone, fingerprintUsageDone=fingerprintUsageDone, bgpDown=bgpDown, spLicenseErrType=spLicenseErrType, bgpUp=bgpUp, spGreTunnelDestination=spGreTunnelDestination, spScriptPort=spScriptPort, spManagedObjectFamily=spManagedObjectFamily, snmpUp=snmpUp, reportDone=reportDone, spServiceName=spServiceName, spBGPTrapName=spBGPTrapName, spLicenseErrLimit=spLicenseErrLimit, deviceSwapSpace=deviceSwapSpace, fingerprintFeedback=fingerprintFeedback, spInterfaceUsageHC=spInterfaceUsageHC, spDNSObservedMean=spDNSObservedMean, spBGPTrapPrefix=spBGPTrapPrefix, spSystemErrorDescription=spSystemErrorDescription)
