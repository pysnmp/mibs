...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spGreTunnelName=spGreTunnelName, trafficAutoMitigation=trafficAutoMitigation, spUnit=spUnit, spHijackLocal=spHijackLocal, spLicenseErrLimit=spLicenseErrLimit, spVersion=spVersion, flowUp=flowUp, spDNSObservedMean=spDNSObservedMean, spManagedObjectFamily=spManagedObjectFamily, spCommFailureDescription=spCommFailureDescription, mitigationFlowspecStart=mitigationFlowspecStart, peakflowSPTraps=peakflowSPTraps, spFingerprintMessage=spFingerprintMessage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spcommFailure=spcommFailure, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spUsageType=spUsageType, spBlackholeCommunity=spBlackholeCommunity, spApplicationName=spApplicationName, spFlowspecTimeout=spFlowspecTimeout, routingFailover=routingFailover, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, peakflowSPMgr=peakflowSPMgr, spcommFailureDone=spcommFailureDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInterfaceUsage=spInterfaceUsage, hardwareFailureDone=hardwareFailureDone, deviceDiskUsage=deviceDiskUsage, snmpDown=snmpDown, alertScript=alertScript, spBGPTrapName=spBGPTrapName, spInterface=spInterface, spReportID=spReportID, spLicenseErrDescription=spLicenseErrDescription, mitigationTMSStart=mitigationTMSStart, spPravail=spPravail, autoclassifyStart=autoclassifyStart, spScriptCommand=spScriptCommand, spBGPInstability=spBGPInstability, greDownDone=greDownDone, spInterfaceSpeedHC=spInterfaceSpeedHC, licenseError=licenseError, configChange=configChange, spTMSMultiPrefix=spTMSMultiPrefix, serviceUsage=serviceUsage, deviceSwapSpace=deviceSwapSpace, spCollector=spCollector, collectorUp=collectorUp, spHijackRoute=spHijackRoute, spDetectedCountries=spDetectedCountries, spFingerprintName=spFingerprintName, dnsBaseline=dnsBaseline, spDuration=spDuration, managedObjectUsage=managedObjectUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, deviceTotalFlows=deviceTotalFlows, greDown=greDown, spInterfaceIndex=spInterfaceIndex, collectorDown=collectorDown, spScriptHost=spScriptHost, spInetAddress=spInetAddress, serviceUsageDone=serviceUsageDone, spInetAddressType=spInetAddressType, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, mitigationTMSStartV2=mitigationTMSStartV2, spDNSName=spDNSName, collectorStart=collectorStart, spManagedObject=spManagedObject, spAlertID=spAlertID, deviceSystemError=deviceSystemError, spUsage=spUsage, spBGPTrapEvent=spBGPTrapEvent, spImpactBps=spImpactBps, spDNSObserved=spDNSObserved, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spDetector=spDetector, interfaceUsageDone=interfaceUsageDone, spBlackholePrefix=spBlackholePrefix, flowDown=flowDown, spTMSCommunity=spTMSCommunity, spFingerprintFeedback=spFingerprintFeedback, spBGPEvent=spBGPEvent, spDNSObservedMax=spDNSObservedMax, bgpHijack=bgpHijack, bgpInstabilityDone=bgpInstabilityDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, fingerprintUsageDone=fingerprintUsageDone, spHijackAttr=spHijackAttr, spLicenseErrType=spLicenseErrType, deviceTotalFlowsHC=deviceTotalFlowsHC, dnsBaselineDone=dnsBaselineDone, spBGPTrapPrefix=spBGPTrapPrefix, spScriptStart=spScriptStart, reportDone=reportDone, spHardwareFailureDescription=spHardwareFailureDescription, deviceSystemErrorDone=deviceSystemErrorDone, spAlertDetectionSignatures=spAlertDetectionSignatures, peakflowSPCMI=peakflowSPCMI, spThirdPartyAddr=spThirdPartyAddr, spSystemErrorDescription=spSystemErrorDescription, bgpDown=bgpDown, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spDNSExpected=spDNSExpected, fingerprintUsage=fingerprintUsage, mitigationDone=mitigationDone, spManagedObjects=spManagedObjects, bgpTrap=bgpTrap, snmpUp=snmpUp, notificationLimit=notificationLimit, spImpactPps=spImpactPps, dosHostDetectionAlert=dosHostDetectionAlert, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spTMSTimeout=spTMSTimeout, bgpInstability=bgpInstability, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spThreshold=spThreshold, spSystemErrorType=spSystemErrorType, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spTMSPrefix=spTMSPrefix, spFingerprintSender=spFingerprintSender, spScriptPort=spScriptPort, cloudSignalingFault=cloudSignalingFault, spInterfaceUsageHC=spInterfaceUsageHC, spRouter=spRouter, cloudSignalingFaultDone=cloudSignalingFaultDone, spFlowspecCommunity=spFlowspecCommunity, bgpUp=bgpUp, PYSNMP_MODULE_ID=peakflowSPMIB, deviceHealth=deviceHealth, spBlackholeNexthop=spBlackholeNexthop, devicePhysicalMemory=devicePhysicalMemory, spBlackholeTimeout=spBlackholeTimeout, spMitigationID=spMitigationID, spMitigationStart=spMitigationStart, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spServiceName=spServiceName, spThirdPartyZone=spThirdPartyZone, spUsername=spUsername, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInterfaceSpeed=spInterfaceSpeed, spCommFailureDestination=spCommFailureDestination, spServiceElement=spServiceElement, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, managedObjectUsageDone=managedObjectUsageDone, fingerprintReceive=fingerprintReceive, peakflowSPMIB=peakflowSPMIB, licenseErrorDone=licenseErrorDone, interfaceUsage=interfaceUsage, spMitigationName=spMitigationName, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, mitigationBlackholeStart=mitigationBlackholeStart, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spReportName=spReportName, spReportURL=spReportURL, spLicenseErrCount=spLicenseErrCount, hardwareFailure=hardwareFailure, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, fingerprintFeedback=fingerprintFeedback, mitigationThirdPartyStart=mitigationThirdPartyStart, spGreTunnelDestination=spGreTunnelDestination)
