...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", bgpUp=bgpUp, dnsBaseline=dnsBaseline, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, bgpInstability=bgpInstability, spScriptHost=spScriptHost, peakflowSPCMI=peakflowSPCMI, notificationLimit=notificationLimit, dnsBaselineDone=dnsBaselineDone, snmpUp=snmpUp, collectorUp=collectorUp, serviceUsageDone=serviceUsageDone, spDNSName=spDNSName, spGreTunnelName=spGreTunnelName, spDetectedCountries=spDetectedCountries, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInterfaceUsageHC=spInterfaceUsageHC, spManagedObjectFamily=spManagedObjectFamily, spBGPInstability=spBGPInstability, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spSystemErrorType=spSystemErrorType, spPravail=spPravail, mitigationThirdPartyStart=mitigationThirdPartyStart, spDNSExpected=spDNSExpected, mitigationBlackholeStart=mitigationBlackholeStart, spReportURL=spReportURL, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spImpactBps=spImpactBps, spDNSObservedMean=spDNSObservedMean, spTMSTimeout=spTMSTimeout, spSystemErrorDescription=spSystemErrorDescription, autoclassifyStart=autoclassifyStart, spcommFailureDone=spcommFailureDone, mitigationTMSStart=mitigationTMSStart, spUsageType=spUsageType, mitigationFlowspecStart=mitigationFlowspecStart, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spInetAddressType=spInetAddressType, spScriptPort=spScriptPort, spThirdPartyZone=spThirdPartyZone, deviceTotalFlows=deviceTotalFlows, mitigationDone=mitigationDone, spFingerprintFeedback=spFingerprintFeedback, peakflowSPMIB=peakflowSPMIB, spScriptStart=spScriptStart, spInterfaceIndex=spInterfaceIndex, spImpactPps=spImpactPps, spBlackholePrefix=spBlackholePrefix, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spFingerprintSender=spFingerprintSender, spTMSMultiPrefix=spTMSMultiPrefix, spLicenseErrLimit=spLicenseErrLimit, deviceHealth=deviceHealth, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spMitigationID=spMitigationID, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spDuration=spDuration, spcommFailure=spcommFailure, spRouter=spRouter, spDNSObservedMax=spDNSObservedMax, hardwareFailureDone=hardwareFailureDone, mitigationTMSStartV2=mitigationTMSStartV2, spBGPEvent=spBGPEvent, spFingerprintMessage=spFingerprintMessage, spTMSCommunity=spTMSCommunity, spMitigationName=spMitigationName, spScriptCommand=spScriptCommand, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spLicenseErrCount=spLicenseErrCount, hardwareFailure=hardwareFailure, spUsage=spUsage, spCommFailureDescription=spCommFailureDescription, spCommFailureDestination=spCommFailureDestination, bgpTrap=bgpTrap, spInterfaceSpeed=spInterfaceSpeed, spGreTunnelDestination=spGreTunnelDestination, flowDown=flowDown, spInterfaceUsage=spInterfaceUsage, spBlackholeTimeout=spBlackholeTimeout, spHijackLocal=spHijackLocal, spDetector=spDetector, cloudSignalingFaultDone=cloudSignalingFaultDone, peakflowSPMgr=peakflowSPMgr, collectorStart=collectorStart, greDownDone=greDownDone, PYSNMP_MODULE_ID=peakflowSPMIB, spLicenseErrDescription=spLicenseErrDescription, spVersion=spVersion, spMitigationStart=spMitigationStart, devicePhysicalMemory=devicePhysicalMemory, fingerprintReceive=fingerprintReceive, licenseErrorDone=licenseErrorDone, spFingerprintName=spFingerprintName, spLicenseErrType=spLicenseErrType, deviceSwapSpaceUsage=deviceSwapSpaceUsage, bgpHijack=bgpHijack, deviceSwapSpace=deviceSwapSpace, spBlackholeNexthop=spBlackholeNexthop, spThirdPartyAddr=spThirdPartyAddr, spReportID=spReportID, spBGPTrapPrefix=spBGPTrapPrefix, spCollector=spCollector, spServiceElement=spServiceElement, spTMSPrefix=spTMSPrefix, licenseError=licenseError, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spAlertDetectionSignatures=spAlertDetectionSignatures, spThreshold=spThreshold, managedObjectUsage=managedObjectUsage, trafficAutoMitigation=trafficAutoMitigation, flowUp=flowUp, managedObjectUsageDone=managedObjectUsageDone, spHardwareFailureDescription=spHardwareFailureDescription, fingerprintUsage=fingerprintUsage, spAlertID=spAlertID, deviceSystemErrorDone=deviceSystemErrorDone, spServiceName=spServiceName, spApplicationName=spApplicationName, fingerprintUsageDone=fingerprintUsageDone, bgpDown=bgpDown, spFlowspecCommunity=spFlowspecCommunity, spUsername=spUsername, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spInetAddress=spInetAddress, deviceDiskUsage=deviceDiskUsage, spManagedObject=spManagedObject, deviceTotalFlowsHC=deviceTotalFlowsHC, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosNetworkProfiledAlert=dosNetworkProfiledAlert, serviceUsage=serviceUsage, spUnit=spUnit, snmpDown=snmpDown, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBGPTrapName=spBGPTrapName, deviceSystemError=deviceSystemError, interfaceUsageDone=interfaceUsageDone, spManagedObjects=spManagedObjects, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosHostDetectionAlert=dosHostDetectionAlert, spReportName=spReportName, spHijackRoute=spHijackRoute, spHijackAttr=spHijackAttr, greDown=greDown, fingerprintFeedback=fingerprintFeedback, spInterface=spInterface, spDNSObserved=spDNSObserved, reportDone=reportDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFlowspecTimeout=spFlowspecTimeout, interfaceUsage=interfaceUsage, configChange=configChange, spInterfaceSpeedHC=spInterfaceSpeedHC, routingFailover=routingFailover, cloudSignalingFault=cloudSignalingFault, collectorDown=collectorDown, alertScript=alertScript, spBGPTrapEvent=spBGPTrapEvent, peakflowSPTraps=peakflowSPTraps, bgpInstabilityDone=bgpInstabilityDone, spBlackholeCommunity=spBlackholeCommunity)
