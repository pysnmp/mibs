...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceSwapSpaceInUse=deviceSwapSpaceInUse, spCommFailureDestination=spCommFailureDestination, dnsBaseline=dnsBaseline, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, snmpUp=snmpUp, spDNSObservedMax=spDNSObservedMax, bgpDown=bgpDown, spLicenseErrCount=spLicenseErrCount, reportDone=reportDone, spInterfaceUsageHC=spInterfaceUsageHC, spFingerprintMessage=spFingerprintMessage, spImpactBps=spImpactBps, collectorDown=collectorDown, licenseError=licenseError, peakflowSPCMI=peakflowSPCMI, spGreTunnelDestination=spGreTunnelDestination, fingerprintUsageDone=fingerprintUsageDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spApplicationName=spApplicationName, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spThirdPartyAddr=spThirdPartyAddr, spScriptHost=spScriptHost, spFlowspecCommunity=spFlowspecCommunity, spReportID=spReportID, spBlackholeTimeout=spBlackholeTimeout, spImpactPps=spImpactPps, spHijackLocal=spHijackLocal, spLicenseErrLimit=spLicenseErrLimit, autoclassifyStart=autoclassifyStart, spUsername=spUsername, spDetectedCountries=spDetectedCountries, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spHijackRoute=spHijackRoute, spMitigationStart=spMitigationStart, spDuration=spDuration, spInetAddress=spInetAddress, spDetector=spDetector, spBGPInstability=spBGPInstability, managedObjectUsageDone=managedObjectUsageDone, deviceTotalFlows=deviceTotalFlows, spFlowspecTimeout=spFlowspecTimeout, spScriptCommand=spScriptCommand, deviceTotalFlowsHC=deviceTotalFlowsHC, spBGPTrapName=spBGPTrapName, deviceSystemErrorDone=deviceSystemErrorDone, spBGPTrapPrefix=spBGPTrapPrefix, spThirdPartyZone=spThirdPartyZone, spcommFailureDone=spcommFailureDone, interfaceUsageDone=interfaceUsageDone, serviceUsageDone=serviceUsageDone, cloudSignalingFaultDone=cloudSignalingFaultDone, bgpInstability=bgpInstability, spDNSExpected=spDNSExpected, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spcommFailure=spcommFailure, dosHostDetectionAlert=dosHostDetectionAlert, spDNSObservedMean=spDNSObservedMean, mitigationFlowspecStart=mitigationFlowspecStart, spDNSName=spDNSName, spScriptPort=spScriptPort, mitigationTMSStart=mitigationTMSStart, spDNSObserved=spDNSObserved, greDown=greDown, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spTMSTimeout=spTMSTimeout, fingerprintReceive=fingerprintReceive, spAlertDetectionSignatures=spAlertDetectionSignatures, devicePhysicalMemory=devicePhysicalMemory, snmpDown=snmpDown, spMitigationID=spMitigationID, spTMSMultiPrefix=spTMSMultiPrefix, collectorUp=collectorUp, spCommFailureDescription=spCommFailureDescription, trafficAutoMitigation=trafficAutoMitigation, flowUp=flowUp, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, peakflowSPMIB=peakflowSPMIB, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spUnit=spUnit, peakflowSPTraps=peakflowSPTraps, fingerprintFeedback=fingerprintFeedback, collectorStart=collectorStart, spThreshold=spThreshold, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, bgpHijack=bgpHijack, fingerprintUsage=fingerprintUsage, spInterfaceSpeedHC=spInterfaceSpeedHC, spBlackholeNexthop=spBlackholeNexthop, hardwareFailure=hardwareFailure, mitigationTMSStartV2=mitigationTMSStartV2, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBGPTrapEvent=spBGPTrapEvent, spUsage=spUsage, spVersion=spVersion, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spBlackholeCommunity=spBlackholeCommunity, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spReportName=spReportName, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spMitigationName=spMitigationName, hardwareFailureDone=hardwareFailureDone, bgpUp=bgpUp, spInterfaceIndex=spInterfaceIndex, spSystemErrorType=spSystemErrorType, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceSystemError=deviceSystemError, spAlertID=spAlertID, spFingerprintName=spFingerprintName, spInetAddressType=spInetAddressType, configChange=configChange, routingFailover=routingFailover, spManagedObjectFamily=spManagedObjectFamily, spLicenseErrType=spLicenseErrType, interfaceUsage=interfaceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, spInterfaceSpeed=spInterfaceSpeed, spBGPEvent=spBGPEvent, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBlackholePrefix=spBlackholePrefix, deviceDiskUsage=deviceDiskUsage, notificationLimit=notificationLimit, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spInterface=spInterface, spScriptStart=spScriptStart, managedObjectUsage=managedObjectUsage, spManagedObject=spManagedObject, spLicenseErrDescription=spLicenseErrDescription, spGreTunnelName=spGreTunnelName, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, dnsBaselineDone=dnsBaselineDone, spManagedObjects=spManagedObjects, spHardwareFailureDescription=spHardwareFailureDescription, spSystemErrorDescription=spSystemErrorDescription, spInterfaceUsage=spInterfaceUsage, bgpInstabilityDone=bgpInstabilityDone, spFingerprintFeedback=spFingerprintFeedback, deviceHealth=deviceHealth, mitigationDone=mitigationDone, serviceUsage=serviceUsage, spHijackAttr=spHijackAttr, spServiceName=spServiceName, bgpTrap=bgpTrap, spTMSCommunity=spTMSCommunity, spRouter=spRouter, mitigationThirdPartyStart=mitigationThirdPartyStart, cloudSignalingFault=cloudSignalingFault, spCollector=spCollector, spFingerprintSender=spFingerprintSender, alertScript=alertScript, spReportURL=spReportURL, spPravail=spPravail, deviceSwapSpace=deviceSwapSpace, flowDown=flowDown, spUsageType=spUsageType, mitigationBlackholeStart=mitigationBlackholeStart, spTMSPrefix=spTMSPrefix, spServiceElement=spServiceElement, greDownDone=greDownDone, licenseErrorDone=licenseErrorDone, peakflowSPMgr=peakflowSPMgr)
