...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spCommFailureDestination=spCommFailureDestination, spImpactPps=spImpactPps, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, dnsBaseline=dnsBaseline, spLicenseErrLimit=spLicenseErrLimit, spUsername=spUsername, spAlertDetectionSignatures=spAlertDetectionSignatures, spGreTunnelName=spGreTunnelName, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spManagedObject=spManagedObject, spCollector=spCollector, fingerprintFeedback=fingerprintFeedback, fingerprintUsageDone=fingerprintUsageDone, mitigationBlackholeStart=mitigationBlackholeStart, greDown=greDown, deviceSystemErrorDone=deviceSystemErrorDone, spSystemErrorDescription=spSystemErrorDescription, deviceSwapSpace=deviceSwapSpace, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spMitigationID=spMitigationID, spTMSPrefix=spTMSPrefix, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, fingerprintUsage=fingerprintUsage, spDNSExpected=spDNSExpected, spTMSCommunity=spTMSCommunity, bgpInstability=bgpInstability, interfaceUsageDone=interfaceUsageDone, bgpHijack=bgpHijack, devicePhysicalMemory=devicePhysicalMemory, spCommFailureDescription=spCommFailureDescription, licenseError=licenseError, spServiceName=spServiceName, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spDNSName=spDNSName, spDetector=spDetector, deviceHealth=deviceHealth, spInterfaceIndex=spInterfaceIndex, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spDNSObserved=spDNSObserved, managedObjectUsage=managedObjectUsage, spMitigationStart=spMitigationStart, spLicenseErrCount=spLicenseErrCount, spServiceElement=spServiceElement, spHardwareFailureDescription=spHardwareFailureDescription, spScriptStart=spScriptStart, spFingerprintName=spFingerprintName, spDNSObservedMean=spDNSObservedMean, spRouter=spRouter, spThirdPartyAddr=spThirdPartyAddr, spDNSObservedMax=spDNSObservedMax, spLicenseErrDescription=spLicenseErrDescription, spAlertID=spAlertID, spScriptPort=spScriptPort, autoclassifyStart=autoclassifyStart, routingFailover=routingFailover, spFingerprintFeedback=spFingerprintFeedback, spInterfaceUsage=spInterfaceUsage, peakflowSPTraps=peakflowSPTraps, hardwareFailure=hardwareFailure, mitigationFlowspecStart=mitigationFlowspecStart, spInterfaceUsageHC=spInterfaceUsageHC, dosHostDetectionAlert=dosHostDetectionAlert, cloudSignalingFault=cloudSignalingFault, trafficAutoMitigation=trafficAutoMitigation, spVersion=spVersion, mitigationThirdPartyStart=mitigationThirdPartyStart, peakflowSPMgr=peakflowSPMgr, peakflowSPCMI=peakflowSPCMI, spBGPTrapOldAttributes=spBGPTrapOldAttributes, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBGPInstability=spBGPInstability, spDuration=spDuration, spLicenseErrType=spLicenseErrType, bgpDown=bgpDown, spFlowspecCommunity=spFlowspecCommunity, spBlackholeNexthop=spBlackholeNexthop, spBlackholePrefix=spBlackholePrefix, alertScript=alertScript, spFlowspecTimeout=spFlowspecTimeout, spBGPTrapEvent=spBGPTrapEvent, spBGPTrapPrefix=spBGPTrapPrefix, mitigationTMSStartV2=mitigationTMSStartV2, spUsageType=spUsageType, spSystemErrorType=spSystemErrorType, spInterfaceSpeed=spInterfaceSpeed, spBlackholeCommunity=spBlackholeCommunity, spManagedObjectFamily=spManagedObjectFamily, mitigationTMSStart=mitigationTMSStart, collectorUp=collectorUp, bgpUp=bgpUp, spBGPEvent=spBGPEvent, spHijackLocal=spHijackLocal, spHijackAttr=spHijackAttr, PYSNMP_MODULE_ID=peakflowSPMIB, spApplicationName=spApplicationName, licenseErrorDone=licenseErrorDone, spThreshold=spThreshold, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spScriptHost=spScriptHost, spInetAddressType=spInetAddressType, collectorStart=collectorStart, bgpTrap=bgpTrap, spReportName=spReportName, configChange=configChange, deviceTotalFlowsHC=deviceTotalFlowsHC, interfaceUsage=interfaceUsage, deviceSwapSpaceInUse=deviceSwapSpaceInUse, hardwareFailureDone=hardwareFailureDone, spDetectedCountries=spDetectedCountries, spFingerprintSender=spFingerprintSender, notificationLimit=notificationLimit, spInetAddress=spInetAddress, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spReportID=spReportID, spThirdPartyZone=spThirdPartyZone, spGreTunnelDestination=spGreTunnelDestination, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spPravail=spPravail, collectorDown=collectorDown, mitigationDone=mitigationDone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceTotalFlows=deviceTotalFlows, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spMitigationName=spMitigationName, spInterfaceSpeedHC=spInterfaceSpeedHC, spHijackRoute=spHijackRoute, deviceDiskUsage=deviceDiskUsage, managedObjectUsageDone=managedObjectUsageDone, deviceSystemError=deviceSystemError, serviceUsageDone=serviceUsageDone, spImpactBps=spImpactBps, cloudSignalingFaultDone=cloudSignalingFaultDone, serviceUsage=serviceUsage, peakflowSPMIB=peakflowSPMIB, spBlackholeTimeout=spBlackholeTimeout, spTMSMultiPrefix=spTMSMultiPrefix, spUnit=spUnit, dnsBaselineDone=dnsBaselineDone, fingerprintReceive=fingerprintReceive, spcommFailure=spcommFailure, snmpDown=snmpDown, snmpUp=snmpUp, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spScriptCommand=spScriptCommand, spFingerprintMessage=spFingerprintMessage, flowDown=flowDown, bgpInstabilityDone=bgpInstabilityDone, flowUp=flowUp, spInterface=spInterface, spUsage=spUsage, spcommFailureDone=spcommFailureDone, greDownDone=greDownDone, spBGPTrapName=spBGPTrapName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spManagedObjects=spManagedObjects, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spTMSTimeout=spTMSTimeout, reportDone=reportDone, spReportURL=spReportURL)
