...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInetAddress=spInetAddress, spUsername=spUsername, spBGPTrapName=spBGPTrapName, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceSystemError=deviceSystemError, spImpactPps=spImpactPps, spBGPTrapNewAttributes=spBGPTrapNewAttributes, interfaceUsage=interfaceUsage, peakflowSPMgr=peakflowSPMgr, spMitigationName=spMitigationName, spLicenseErrCount=spLicenseErrCount, deviceSwapSpaceInUse=deviceSwapSpaceInUse, bgpTrap=bgpTrap, spcommFailureDone=spcommFailureDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spServiceElement=spServiceElement, spDNSObservedMax=spDNSObservedMax, bgpInstabilityDone=bgpInstabilityDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, fingerprintUsage=fingerprintUsage, spFingerprintSender=spFingerprintSender, peakflowSPCMI=peakflowSPCMI, spGreTunnelDestination=spGreTunnelDestination, spBGPTrapPrefix=spBGPTrapPrefix, spTMSCommunity=spTMSCommunity, spApplicationName=spApplicationName, spScriptStart=spScriptStart, dnsBaselineDone=dnsBaselineDone, collectorStart=collectorStart, serviceUsage=serviceUsage, trafficAutoMitigation=trafficAutoMitigation, flowUp=flowUp, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spFingerprintMessage=spFingerprintMessage, mitigationFlowspecStart=mitigationFlowspecStart, spBGPInstability=spBGPInstability, mitigationBlackholeStart=mitigationBlackholeStart, greDownDone=greDownDone, deviceSystemErrorDone=deviceSystemErrorDone, spHijackLocal=spHijackLocal, spTMSTimeout=spTMSTimeout, licenseErrorDone=licenseErrorDone, spSystemErrorDescription=spSystemErrorDescription, deviceDiskUsage=deviceDiskUsage, hardwareFailureDone=hardwareFailureDone, bgpDown=bgpDown, licenseError=licenseError, mitigationTMSStartV2=mitigationTMSStartV2, spCollector=spCollector, spCloudSignalFaultDescription=spCloudSignalFaultDescription, devicePhysicalMemory=devicePhysicalMemory, mitigationDone=mitigationDone, peakflowSPTraps=peakflowSPTraps, spInterfaceIndex=spInterfaceIndex, spCommFailureDestination=spCommFailureDestination, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spSystemErrorType=spSystemErrorType, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spReportURL=spReportURL, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spUsageType=spUsageType, spCommFailureDescription=spCommFailureDescription, spBlackholePrefix=spBlackholePrefix, cloudSignalingFaultDone=cloudSignalingFaultDone, spFlowspecTimeout=spFlowspecTimeout, managedObjectUsage=managedObjectUsage, spInetAddressType=spInetAddressType, collectorDown=collectorDown, deviceHealth=deviceHealth, configChange=configChange, spHijackAttr=spHijackAttr, spHijackRoute=spHijackRoute, spBGPTrapEvent=spBGPTrapEvent, spTMSMultiPrefix=spTMSMultiPrefix, spFingerprintName=spFingerprintName, spVersion=spVersion, spImpactBps=spImpactBps, spAlertDetectionSignatures=spAlertDetectionSignatures, alertScript=alertScript, spBGPEvent=spBGPEvent, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, peakflowSPMIB=peakflowSPMIB, bgpUp=bgpUp, spDNSExpected=spDNSExpected, snmpDown=snmpDown, spInterfaceSpeedHC=spInterfaceSpeedHC, collectorUp=collectorUp, spFlowspecCommunity=spFlowspecCommunity, dosHostDetectionAlert=dosHostDetectionAlert, spManagedObjects=spManagedObjects, spUnit=spUnit, managedObjectUsageDone=managedObjectUsageDone, PYSNMP_MODULE_ID=peakflowSPMIB, cloudSignalingFault=cloudSignalingFault, flowDown=flowDown, spPravail=spPravail, greDown=greDown, spServiceName=spServiceName, routingFailover=routingFailover, dnsBaseline=dnsBaseline, spScriptCommand=spScriptCommand, spLicenseErrType=spLicenseErrType, spDetectedCountries=spDetectedCountries, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, mitigationThirdPartyStart=mitigationThirdPartyStart, spManagedObjectFamily=spManagedObjectFamily, spMitigationID=spMitigationID, spTMSPrefix=spTMSPrefix, snmpUp=snmpUp, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, bgpHijack=bgpHijack, spDNSObserved=spDNSObserved, spInterfaceUsageHC=spInterfaceUsageHC, spInterface=spInterface, spInterfaceSpeed=spInterfaceSpeed, spGreTunnelName=spGreTunnelName, spThirdPartyZone=spThirdPartyZone, deviceTotalFlows=deviceTotalFlows, spDNSName=spDNSName, autoclassifyStart=autoclassifyStart, spScriptPort=spScriptPort, fingerprintFeedback=fingerprintFeedback, spInterfaceUsage=spInterfaceUsage, spUsage=spUsage, spDNSObservedMean=spDNSObservedMean, deviceTotalFlowsHC=deviceTotalFlowsHC, spBlackholeTimeout=spBlackholeTimeout, fingerprintReceive=fingerprintReceive, spFingerprintFeedback=spFingerprintFeedback, spDetector=spDetector, spBlackholeNexthop=spBlackholeNexthop, spLicenseErrLimit=spLicenseErrLimit, spHardwareFailureDescription=spHardwareFailureDescription, serviceUsageDone=serviceUsageDone, spScriptHost=spScriptHost, fingerprintUsageDone=fingerprintUsageDone, spManagedObject=spManagedObject, spAlertID=spAlertID, spRouter=spRouter, spLicenseErrDescription=spLicenseErrDescription, interfaceUsageDone=interfaceUsageDone, spDuration=spDuration, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, deviceSwapSpaceUsage=deviceSwapSpaceUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spcommFailure=spcommFailure, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, hardwareFailure=hardwareFailure, spThreshold=spThreshold, spReportID=spReportID, deviceSwapSpace=deviceSwapSpace, mitigationTMSStart=mitigationTMSStart, spThirdPartyAddr=spThirdPartyAddr, notificationLimit=notificationLimit, spMitigationStart=spMitigationStart, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, bgpInstability=bgpInstability, spReportName=spReportName, spBlackholeCommunity=spBlackholeCommunity, reportDone=reportDone)
