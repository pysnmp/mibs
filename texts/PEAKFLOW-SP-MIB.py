...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceSpeedHC=spInterfaceSpeedHC, spBlackholeNexthop=spBlackholeNexthop, spMitigationName=spMitigationName, spScriptPort=spScriptPort, collectorDown=collectorDown, spBGPInstability=spBGPInstability, spInterface=spInterface, deviceSwapSpace=deviceSwapSpace, bgpHijack=bgpHijack, peakflowSPTraps=peakflowSPTraps, notificationLimit=notificationLimit, peakflowSPCMI=peakflowSPCMI, spDetectedCountries=spDetectedCountries, managedObjectUsageDone=managedObjectUsageDone, spInetAddressType=spInetAddressType, fingerprintReceive=fingerprintReceive, spGreTunnelDestination=spGreTunnelDestination, deviceTotalFlowsHC=deviceTotalFlowsHC, spManagedObjects=spManagedObjects, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, devicePhysicalMemory=devicePhysicalMemory, spImpactPps=spImpactPps, interfaceUsage=interfaceUsage, spBGPEvent=spBGPEvent, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spTMSMultiPrefix=spTMSMultiPrefix, spBlackholeTimeout=spBlackholeTimeout, spHijackLocal=spHijackLocal, spServiceName=spServiceName, deviceTotalFlows=deviceTotalFlows, spcommFailure=spcommFailure, reportDone=reportDone, interfaceUsageDone=interfaceUsageDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, trafficAutoMitigation=trafficAutoMitigation, flowDown=flowDown, spServiceElement=spServiceElement, spReportID=spReportID, spCommFailureDestination=spCommFailureDestination, deviceHealth=deviceHealth, spThirdPartyZone=spThirdPartyZone, spFingerprintName=spFingerprintName, peakflowSPMgr=peakflowSPMgr, spScriptHost=spScriptHost, mitigationTMSStart=mitigationTMSStart, spBGPTrapName=spBGPTrapName, mitigationThirdPartyStart=mitigationThirdPartyStart, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, deviceDiskUsage=deviceDiskUsage, spTMSCommunity=spTMSCommunity, fingerprintUsage=fingerprintUsage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spDNSObservedMean=spDNSObservedMean, bgpInstability=bgpInstability, spDuration=spDuration, dnsBaseline=dnsBaseline, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spFlowspecCommunity=spFlowspecCommunity, spGreTunnelName=spGreTunnelName, hardwareFailureDone=hardwareFailureDone, spLicenseErrType=spLicenseErrType, spHijackAttr=spHijackAttr, spHardwareFailureDescription=spHardwareFailureDescription, mitigationTMSStartV2=mitigationTMSStartV2, hardwareFailure=hardwareFailure, spCollector=spCollector, spUsage=spUsage, serviceUsage=serviceUsage, deviceSwapSpaceInUse=deviceSwapSpaceInUse, dosHostDetectionAlert=dosHostDetectionAlert, spTMSPrefix=spTMSPrefix, spCloudSignalFaultDescription=spCloudSignalFaultDescription, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spMitigationID=spMitigationID, greDownDone=greDownDone, spCommFailureDescription=spCommFailureDescription, spReportName=spReportName, PYSNMP_MODULE_ID=peakflowSPMIB, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, collectorUp=collectorUp, spAlertID=spAlertID, spcommFailureDone=spcommFailureDone, flowUp=flowUp, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spSystemErrorType=spSystemErrorType, spFingerprintSender=spFingerprintSender, mitigationBlackholeStart=mitigationBlackholeStart, licenseError=licenseError, spLicenseErrDescription=spLicenseErrDescription, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBlackholeCommunity=spBlackholeCommunity, spDNSObserved=spDNSObserved, spInetAddress=spInetAddress, greDown=greDown, spUnit=spUnit, spApplicationName=spApplicationName, deviceSystemError=deviceSystemError, routingFailover=routingFailover, bgpDown=bgpDown, spInterfaceUsageHC=spInterfaceUsageHC, spAlertDetectionSignatures=spAlertDetectionSignatures, dosNetworkProfiledAlert=dosNetworkProfiledAlert, serviceUsageDone=serviceUsageDone, cloudSignalingFault=cloudSignalingFault, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spManagedObject=spManagedObject, spFingerprintFeedback=spFingerprintFeedback, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBGPTrapEvent=spBGPTrapEvent, spRouter=spRouter, managedObjectUsage=managedObjectUsage, dnsBaselineDone=dnsBaselineDone, spFlowspecTimeout=spFlowspecTimeout, spDNSName=spDNSName, mitigationDone=mitigationDone, spUsageType=spUsageType, collectorStart=collectorStart, mitigationFlowspecStart=mitigationFlowspecStart, snmpDown=snmpDown, spBGPTrapPrefix=spBGPTrapPrefix, spBlackholePrefix=spBlackholePrefix, spInterfaceSpeed=spInterfaceSpeed, spImpactBps=spImpactBps, spUsername=spUsername, spScriptCommand=spScriptCommand, snmpUp=snmpUp, cloudSignalingFaultDone=cloudSignalingFaultDone, spInterfaceUsage=spInterfaceUsage, spThreshold=spThreshold, spReportURL=spReportURL, spThirdPartyAddr=spThirdPartyAddr, configChange=configChange, deviceSystemErrorDone=deviceSystemErrorDone, spDNSExpected=spDNSExpected, fingerprintUsageDone=fingerprintUsageDone, spDNSObservedMax=spDNSObservedMax, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spSystemErrorDescription=spSystemErrorDescription, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spScriptStart=spScriptStart, spManagedObjectFamily=spManagedObjectFamily, autoclassifyStart=autoclassifyStart, spPravail=spPravail, spVersion=spVersion, spLicenseErrCount=spLicenseErrCount, spMitigationStart=spMitigationStart, spInterfaceIndex=spInterfaceIndex, spHijackRoute=spHijackRoute, licenseErrorDone=licenseErrorDone, bgpTrap=bgpTrap, bgpUp=bgpUp, spTMSTimeout=spTMSTimeout, spDetector=spDetector, alertScript=alertScript, peakflowSPMIB=peakflowSPMIB, fingerprintFeedback=fingerprintFeedback, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spLicenseErrLimit=spLicenseErrLimit, bgpInstabilityDone=bgpInstabilityDone, spFingerprintMessage=spFingerprintMessage)
