...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", mitigationThirdPartyStart=mitigationThirdPartyStart, spCommFailureDestination=spCommFailureDestination, peakflowSPTraps=peakflowSPTraps, deviceSystemErrorDone=deviceSystemErrorDone, spReportName=spReportName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spDNSObservedMean=spDNSObservedMean, spServiceElement=spServiceElement, bgpUp=bgpUp, managedObjectUsage=managedObjectUsage, dosHostDetectionAlert=dosHostDetectionAlert, mitigationTMSStartV2=mitigationTMSStartV2, spInterfaceSpeed=spInterfaceSpeed, spRouter=spRouter, spScriptPort=spScriptPort, spFingerprintMessage=spFingerprintMessage, deviceTotalFlowsHC=deviceTotalFlowsHC, spMitigationStart=spMitigationStart, spBGPTrapNewAttributes=spBGPTrapNewAttributes, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spInterface=spInterface, spDNSExpected=spDNSExpected, spThirdPartyAddr=spThirdPartyAddr, spDNSObserved=spDNSObserved, spImpactPps=spImpactPps, deviceDiskUsage=deviceDiskUsage, spLicenseErrType=spLicenseErrType, dnsBaseline=dnsBaseline, spDuration=spDuration, spInterfaceIndex=spInterfaceIndex, collectorUp=collectorUp, alertScript=alertScript, deviceSystemError=deviceSystemError, licenseErrorDone=licenseErrorDone, dnsBaselineDone=dnsBaselineDone, interfaceUsage=interfaceUsage, hardwareFailure=hardwareFailure, fingerprintUsage=fingerprintUsage, spLicenseErrCount=spLicenseErrCount, autoclassifyStart=autoclassifyStart, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInterfaceSpeedHC=spInterfaceSpeedHC, spScriptStart=spScriptStart, spFingerprintFeedback=spFingerprintFeedback, spSystemErrorDescription=spSystemErrorDescription, spBGPTrapName=spBGPTrapName, spDNSObservedMax=spDNSObservedMax, fingerprintUsageDone=fingerprintUsageDone, serviceUsageDone=serviceUsageDone, greDownDone=greDownDone, spScriptHost=spScriptHost, spThirdPartyZone=spThirdPartyZone, spBlackholeNexthop=spBlackholeNexthop, spTMSPrefix=spTMSPrefix, spServiceName=spServiceName, deviceTotalFlows=deviceTotalFlows, fingerprintFeedback=fingerprintFeedback, spBGPInstability=spBGPInstability, spBlackholeTimeout=spBlackholeTimeout, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spUsage=spUsage, PYSNMP_MODULE_ID=peakflowSPMIB, interfaceUsageDone=interfaceUsageDone, spAlertID=spAlertID, peakflowSPCMI=peakflowSPCMI, spDNSName=spDNSName, serviceUsage=serviceUsage, snmpUp=snmpUp, collectorStart=collectorStart, routingFailover=routingFailover, deviceSwapSpace=deviceSwapSpace, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spTMSCommunity=spTMSCommunity, spCommFailureDescription=spCommFailureDescription, mitigationTMSStart=mitigationTMSStart, spMitigationName=spMitigationName, spBGPTrapOldAttributes=spBGPTrapOldAttributes, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spCollector=spCollector, spLicenseErrLimit=spLicenseErrLimit, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spMitigationID=spMitigationID, flowDown=flowDown, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spScriptCommand=spScriptCommand, reportDone=reportDone, spInterfaceUsageHC=spInterfaceUsageHC, spVersion=spVersion, bgpDown=bgpDown, notificationLimit=notificationLimit, spUsername=spUsername, spSystemErrorType=spSystemErrorType, dosNetworkProfiledAlert=dosNetworkProfiledAlert, greDown=greDown, spBGPEvent=spBGPEvent, spManagedObjects=spManagedObjects, spAlertDetectionSignatures=spAlertDetectionSignatures, spGreTunnelDestination=spGreTunnelDestination, spHijackRoute=spHijackRoute, bgpHijack=bgpHijack, peakflowSPMIB=peakflowSPMIB, bgpInstabilityDone=bgpInstabilityDone, collectorDown=collectorDown, managedObjectUsageDone=managedObjectUsageDone, spReportID=spReportID, spBlackholePrefix=spBlackholePrefix, fingerprintReceive=fingerprintReceive, spHijackLocal=spHijackLocal, spBGPTrapPrefix=spBGPTrapPrefix, flowUp=flowUp, licenseError=licenseError, cloudSignalingFault=cloudSignalingFault, spFingerprintName=spFingerprintName, deviceSwapSpaceInUse=deviceSwapSpaceInUse, trafficAutoMitigation=trafficAutoMitigation, mitigationFlowspecStart=mitigationFlowspecStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spLicenseErrDescription=spLicenseErrDescription, mitigationBlackholeStart=mitigationBlackholeStart, spcommFailure=spcommFailure, spBGPTrapEvent=spBGPTrapEvent, spUsageType=spUsageType, spFlowspecTimeout=spFlowspecTimeout, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceHealth=deviceHealth, spTMSTimeout=spTMSTimeout, spHijackAttr=spHijackAttr, spInetAddress=spInetAddress, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spThreshold=spThreshold, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInetAddressType=spInetAddressType, spGreTunnelName=spGreTunnelName, bgpTrap=bgpTrap, cloudSignalingFaultDone=cloudSignalingFaultDone, spImpactBps=spImpactBps, spTMSMultiPrefix=spTMSMultiPrefix, mitigationDone=mitigationDone, configChange=configChange, spDetectedCountries=spDetectedCountries, spBlackholeCommunity=spBlackholeCommunity, snmpDown=snmpDown, spInterfaceUsage=spInterfaceUsage, spPravail=spPravail, spManagedObjectFamily=spManagedObjectFamily, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, hardwareFailureDone=hardwareFailureDone, bgpInstability=bgpInstability, spFingerprintSender=spFingerprintSender, spDetector=spDetector, spHardwareFailureDescription=spHardwareFailureDescription, spApplicationName=spApplicationName, spFlowspecCommunity=spFlowspecCommunity, spReportURL=spReportURL, spUnit=spUnit, peakflowSPMgr=peakflowSPMgr, spcommFailureDone=spcommFailureDone, devicePhysicalMemory=devicePhysicalMemory, spManagedObject=spManagedObject)
