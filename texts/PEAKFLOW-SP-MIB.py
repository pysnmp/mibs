...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spSystemErrorDescription=spSystemErrorDescription, spHardwareFailureDescription=spHardwareFailureDescription, deviceSystemErrorDone=deviceSystemErrorDone, spBGPInstability=spBGPInstability, managedObjectUsage=managedObjectUsage, spAlertID=spAlertID, spScriptStart=spScriptStart, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spDNSExpected=spDNSExpected, spCommFailureDescription=spCommFailureDescription, PYSNMP_MODULE_ID=peakflowSPMIB, greDownDone=greDownDone, spThirdPartyZone=spThirdPartyZone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spHijackRoute=spHijackRoute, routingFailover=routingFailover, hardwareFailure=hardwareFailure, fingerprintUsageDone=fingerprintUsageDone, spLicenseErrType=spLicenseErrType, spBGPTrapPrefix=spBGPTrapPrefix, spFlowspecTimeout=spFlowspecTimeout, spGreTunnelName=spGreTunnelName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, mitigationTMSStartV2=mitigationTMSStartV2, bgpInstabilityDone=bgpInstabilityDone, mitigationBlackholeStart=mitigationBlackholeStart, spScriptCommand=spScriptCommand, spImpactPps=spImpactPps, spcommFailure=spcommFailure, deviceTotalFlowsHC=deviceTotalFlowsHC, collectorStart=collectorStart, configChange=configChange, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spTMSPrefix=spTMSPrefix, spDNSObserved=spDNSObserved, spDetector=spDetector, alertScript=alertScript, spSystemErrorType=spSystemErrorType, spCollector=spCollector, spInterfaceUsage=spInterfaceUsage, spBGPEvent=spBGPEvent, deviceDiskUsage=deviceDiskUsage, spInterfaceIndex=spInterfaceIndex, spCommFailureDestination=spCommFailureDestination, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spUnit=spUnit, interfaceUsageDone=interfaceUsageDone, serviceUsageDone=serviceUsageDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceSystemError=deviceSystemError, hardwareFailureDone=hardwareFailureDone, spReportURL=spReportURL, spMitigationStart=spMitigationStart, spMitigationID=spMitigationID, spApplicationName=spApplicationName, spDNSObservedMean=spDNSObservedMean, spBGPTrapOldAttributes=spBGPTrapOldAttributes, peakflowSPMIB=peakflowSPMIB, spInterfaceUsageHC=spInterfaceUsageHC, spFlowspecCommunity=spFlowspecCommunity, spTMSCommunity=spTMSCommunity, spInetAddressType=spInetAddressType, spServiceElement=spServiceElement, spFingerprintName=spFingerprintName, deviceHealth=deviceHealth, spBGPTrapEvent=spBGPTrapEvent, spRouter=spRouter, bgpUp=bgpUp, spcommFailureDone=spcommFailureDone, spVersion=spVersion, dnsBaselineDone=dnsBaselineDone, spBlackholePrefix=spBlackholePrefix, spThirdPartyAddr=spThirdPartyAddr, spInterface=spInterface, spHijackAttr=spHijackAttr, trafficAutoMitigation=trafficAutoMitigation, spHijackLocal=spHijackLocal, dosHostDetectionAlertDone=dosHostDetectionAlertDone, mitigationTMSStart=mitigationTMSStart, spGreTunnelDestination=spGreTunnelDestination, spReportID=spReportID, flowDown=flowDown, spReportName=spReportName, mitigationDone=mitigationDone, interfaceUsage=interfaceUsage, spUsage=spUsage, mitigationFlowspecStart=mitigationFlowspecStart, deviceTotalFlows=deviceTotalFlows, collectorDown=collectorDown, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spUsername=spUsername, spManagedObjectFamily=spManagedObjectFamily, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spBGPTrapNewAttributes=spBGPTrapNewAttributes, managedObjectUsageDone=managedObjectUsageDone, peakflowSPCMI=peakflowSPCMI, cloudSignalingFaultDone=cloudSignalingFaultDone, spInterfaceSpeed=spInterfaceSpeed, spBlackholeTimeout=spBlackholeTimeout, spLicenseErrCount=spLicenseErrCount, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, collectorUp=collectorUp, dnsBaseline=dnsBaseline, spAlertDetectionSignatures=spAlertDetectionSignatures, fingerprintFeedback=fingerprintFeedback, cloudSignalingFault=cloudSignalingFault, spTMSTimeout=spTMSTimeout, spServiceName=spServiceName, spDuration=spDuration, spPravail=spPravail, flowUp=flowUp, spScriptHost=spScriptHost, snmpDown=snmpDown, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, dosHostDetectionAlert=dosHostDetectionAlert, bgpTrap=bgpTrap, spFingerprintFeedback=spFingerprintFeedback, fingerprintUsage=fingerprintUsage, notificationLimit=notificationLimit, autoclassifyStart=autoclassifyStart, spUsageType=spUsageType, spFingerprintMessage=spFingerprintMessage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spMitigationName=spMitigationName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSObservedMax=spDNSObservedMax, spImpactBps=spImpactBps, spFingerprintSender=spFingerprintSender, peakflowSPMgr=peakflowSPMgr, spManagedObjects=spManagedObjects, spDNSName=spDNSName, spTMSMultiPrefix=spTMSMultiPrefix, spBGPTrapName=spBGPTrapName, greDown=greDown, spDetectedCountries=spDetectedCountries, spManagedObject=spManagedObject, mitigationThirdPartyStart=mitigationThirdPartyStart, serviceUsage=serviceUsage, licenseErrorDone=licenseErrorDone, spScriptPort=spScriptPort, spLicenseErrDescription=spLicenseErrDescription, bgpDown=bgpDown, snmpUp=snmpUp, spThreshold=spThreshold, spBlackholeCommunity=spBlackholeCommunity, spInetAddress=spInetAddress, reportDone=reportDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, peakflowSPTraps=peakflowSPTraps, fingerprintReceive=fingerprintReceive, spLicenseErrLimit=spLicenseErrLimit, deviceSwapSpace=deviceSwapSpace, bgpHijack=bgpHijack, spBlackholeNexthop=spBlackholeNexthop, devicePhysicalMemory=devicePhysicalMemory, licenseError=licenseError, bgpInstability=bgpInstability)
