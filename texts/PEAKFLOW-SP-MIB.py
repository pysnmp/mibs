...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spGreTunnelDestination=spGreTunnelDestination, spDNSObservedMax=spDNSObservedMax, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spGreTunnelName=spGreTunnelName, spBlackholeCommunity=spBlackholeCommunity, bgpUp=bgpUp, spMitigationID=spMitigationID, spTMSPrefix=spTMSPrefix, fingerprintReceive=fingerprintReceive, dnsBaseline=dnsBaseline, mitigationBlackholeStart=mitigationBlackholeStart, routingFailover=routingFailover, licenseErrorDone=licenseErrorDone, trafficAutoMitigation=trafficAutoMitigation, spLicenseErrType=spLicenseErrType, licenseError=licenseError, deviceTotalFlowsHC=deviceTotalFlowsHC, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spDNSName=spDNSName, spScriptCommand=spScriptCommand, spThirdPartyAddr=spThirdPartyAddr, spDetectedCountries=spDetectedCountries, spInterfaceUsageHC=spInterfaceUsageHC, bgpInstability=bgpInstability, deviceSystemErrorDone=deviceSystemErrorDone, spInterfaceSpeedHC=spInterfaceSpeedHC, mitigationTMSStartV2=mitigationTMSStartV2, deviceSystemError=deviceSystemError, hardwareFailure=hardwareFailure, greDown=greDown, spBGPTrapName=spBGPTrapName, cloudSignalingFaultDone=cloudSignalingFaultDone, spFingerprintMessage=spFingerprintMessage, spDNSObserved=spDNSObserved, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spUsername=spUsername, spMitigationStart=spMitigationStart, hardwareFailureDone=hardwareFailureDone, spImpactPps=spImpactPps, spScriptStart=spScriptStart, flowDown=flowDown, spReportID=spReportID, spUnit=spUnit, spRouter=spRouter, spcommFailureDone=spcommFailureDone, deviceSwapSpace=deviceSwapSpace, spVersion=spVersion, alertScript=alertScript, fingerprintUsage=fingerprintUsage, cloudSignalingFault=cloudSignalingFault, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceDiskUsage=deviceDiskUsage, fingerprintUsageDone=fingerprintUsageDone, serviceUsage=serviceUsage, peakflowSPMgr=peakflowSPMgr, interfaceUsage=interfaceUsage, spManagedObjectFamily=spManagedObjectFamily, snmpUp=snmpUp, bgpInstabilityDone=bgpInstabilityDone, dnsBaselineDone=dnsBaselineDone, spAlertID=spAlertID, spUsageType=spUsageType, spFlowspecCommunity=spFlowspecCommunity, interfaceUsageDone=interfaceUsageDone, spSystemErrorDescription=spSystemErrorDescription, bgpTrap=bgpTrap, dosHostDetectionAlertDone=dosHostDetectionAlertDone, collectorUp=collectorUp, spInterfaceUsage=spInterfaceUsage, spLicenseErrLimit=spLicenseErrLimit, spFingerprintName=spFingerprintName, spBGPEvent=spBGPEvent, spThreshold=spThreshold, autoclassifyStart=autoclassifyStart, spUsage=spUsage, spBlackholeNexthop=spBlackholeNexthop, serviceUsageDone=serviceUsageDone, bgpDown=bgpDown, collectorStart=collectorStart, spBGPTrapPrefix=spBGPTrapPrefix, spInetAddress=spInetAddress, spFlowspecTimeout=spFlowspecTimeout, spReportName=spReportName, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInterfaceSpeed=spInterfaceSpeed, spHardwareFailureDescription=spHardwareFailureDescription, spTMSCommunity=spTMSCommunity, spPravail=spPravail, spHijackRoute=spHijackRoute, dosHostDetectionAlert=dosHostDetectionAlert, spReportURL=spReportURL, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spSystemErrorType=spSystemErrorType, mitigationDone=mitigationDone, spCommFailureDestination=spCommFailureDestination, snmpDown=snmpDown, spManagedObject=spManagedObject, spBlackholePrefix=spBlackholePrefix, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spTMSTimeout=spTMSTimeout, spServiceName=spServiceName, bgpHijack=bgpHijack, deviceTotalFlows=deviceTotalFlows, spInterfaceIndex=spInterfaceIndex, spScriptPort=spScriptPort, spThirdPartyZone=spThirdPartyZone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spBGPInstability=spBGPInstability, spCollector=spCollector, spLicenseErrCount=spLicenseErrCount, configChange=configChange, spScriptHost=spScriptHost, managedObjectUsageDone=managedObjectUsageDone, spManagedObjects=spManagedObjects, spHijackLocal=spHijackLocal, spApplicationName=spApplicationName, spMitigationName=spMitigationName, collectorDown=collectorDown, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, flowUp=flowUp, spInterface=spInterface, spServiceElement=spServiceElement, peakflowSPTraps=peakflowSPTraps, spAlertDetectionSignatures=spAlertDetectionSignatures, spHijackAttr=spHijackAttr, deviceSwapSpaceUsage=deviceSwapSpaceUsage, reportDone=reportDone, spTMSMultiPrefix=spTMSMultiPrefix, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spcommFailure=spcommFailure, peakflowSPCMI=peakflowSPCMI, spDuration=spDuration, spFingerprintSender=spFingerprintSender, greDownDone=greDownDone, spLicenseErrDescription=spLicenseErrDescription, fingerprintFeedback=fingerprintFeedback, peakflowSPMIB=peakflowSPMIB, mitigationTMSStart=mitigationTMSStart, deviceHealth=deviceHealth, spBGPTrapEvent=spBGPTrapEvent, managedObjectUsage=managedObjectUsage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, PYSNMP_MODULE_ID=peakflowSPMIB, spCommFailureDescription=spCommFailureDescription, spFingerprintFeedback=spFingerprintFeedback, spDNSObservedMean=spDNSObservedMean, spBGPTrapOldAttributes=spBGPTrapOldAttributes, mitigationThirdPartyStart=mitigationThirdPartyStart, spBlackholeTimeout=spBlackholeTimeout, notificationLimit=notificationLimit, spInetAddressType=spInetAddressType, devicePhysicalMemory=devicePhysicalMemory, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spDNSExpected=spDNSExpected, mitigationFlowspecStart=mitigationFlowspecStart, spDetector=spDetector, spImpactBps=spImpactBps)
