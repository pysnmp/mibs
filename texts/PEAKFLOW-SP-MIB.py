...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInterface=spInterface, greDown=greDown, spServiceElement=spServiceElement, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spScriptPort=spScriptPort, mitigationTMSStartV2=mitigationTMSStartV2, collectorUp=collectorUp, spCommFailureDescription=spCommFailureDescription, spLicenseErrType=spLicenseErrType, dosNetworkProfiledAlert=dosNetworkProfiledAlert, collectorDown=collectorDown, spScriptCommand=spScriptCommand, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spDNSObserved=spDNSObserved, spReportURL=spReportURL, spTMSMultiPrefix=spTMSMultiPrefix, spDuration=spDuration, greDownDone=greDownDone, spServiceName=spServiceName, spMitigationName=spMitigationName, spPravail=spPravail, spDetectedCountries=spDetectedCountries, spCollector=spCollector, spBlackholePrefix=spBlackholePrefix, spInetAddressType=spInetAddressType, notificationLimit=notificationLimit, spBlackholeCommunity=spBlackholeCommunity, deviceSwapSpaceUsage=deviceSwapSpaceUsage, dnsBaseline=dnsBaseline, autoclassifyStart=autoclassifyStart, spLicenseErrCount=spLicenseErrCount, deviceTotalFlowsHC=deviceTotalFlowsHC, spManagedObject=spManagedObject, spDNSObservedMax=spDNSObservedMax, snmpDown=snmpDown, spBGPTrapName=spBGPTrapName, bgpDown=bgpDown, spHijackLocal=spHijackLocal, spFlowspecTimeout=spFlowspecTimeout, spDNSExpected=spDNSExpected, cloudSignalingFault=cloudSignalingFault, peakflowSPCMI=peakflowSPCMI, dnsBaselineDone=dnsBaselineDone, PYSNMP_MODULE_ID=peakflowSPMIB, peakflowSPMIB=peakflowSPMIB, spLicenseErrLimit=spLicenseErrLimit, hardwareFailureDone=hardwareFailureDone, spManagedObjectFamily=spManagedObjectFamily, spImpactBps=spImpactBps, spAlertDetectionSignatures=spAlertDetectionSignatures, spDNSObservedMean=spDNSObservedMean, licenseErrorDone=licenseErrorDone, spcommFailureDone=spcommFailureDone, spBGPTrapEvent=spBGPTrapEvent, spReportName=spReportName, spApplicationName=spApplicationName, spBGPEvent=spBGPEvent, deviceDiskUsage=deviceDiskUsage, bgpUp=bgpUp, dosHostDetectionAlert=dosHostDetectionAlert, spBlackholeNexthop=spBlackholeNexthop, bgpTrap=bgpTrap, alertScript=alertScript, serviceUsage=serviceUsage, mitigationBlackholeStart=mitigationBlackholeStart, spUsageType=spUsageType, spGreTunnelDestination=spGreTunnelDestination, serviceUsageDone=serviceUsageDone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, bgpInstabilityDone=bgpInstabilityDone, spInterfaceUsageHC=spInterfaceUsageHC, spHijackRoute=spHijackRoute, deviceTotalFlows=deviceTotalFlows, spcommFailure=spcommFailure, fingerprintReceive=fingerprintReceive, deviceHealth=deviceHealth, spGreTunnelName=spGreTunnelName, flowDown=flowDown, spSystemErrorDescription=spSystemErrorDescription, fingerprintUsage=fingerprintUsage, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spThirdPartyZone=spThirdPartyZone, spFingerprintMessage=spFingerprintMessage, collectorStart=collectorStart, managedObjectUsageDone=managedObjectUsageDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spInterfaceSpeed=spInterfaceSpeed, spInterfaceUsage=spInterfaceUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spScriptStart=spScriptStart, routingFailover=routingFailover, mitigationThirdPartyStart=mitigationThirdPartyStart, mitigationDone=mitigationDone, spMitigationStart=spMitigationStart, spBGPInstability=spBGPInstability, spAlertID=spAlertID, spThreshold=spThreshold, spFingerprintSender=spFingerprintSender, spLicenseErrDescription=spLicenseErrDescription, spInterfaceIndex=spInterfaceIndex, fingerprintUsageDone=fingerprintUsageDone, spBlackholeTimeout=spBlackholeTimeout, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, trafficAutoMitigation=trafficAutoMitigation, spSystemErrorType=spSystemErrorType, bgpInstability=bgpInstability, reportDone=reportDone, spTMSTimeout=spTMSTimeout, spDetector=spDetector, spCommFailureDestination=spCommFailureDestination, spScriptHost=spScriptHost, configChange=configChange, spFlowspecCommunity=spFlowspecCommunity, snmpUp=snmpUp, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spDNSName=spDNSName, spBGPTrapPrefix=spBGPTrapPrefix, spManagedObjects=spManagedObjects, spFingerprintFeedback=spFingerprintFeedback, mitigationFlowspecStart=mitigationFlowspecStart, spHardwareFailureDescription=spHardwareFailureDescription, spHijackAttr=spHijackAttr, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, devicePhysicalMemory=devicePhysicalMemory, spInterfaceSpeedHC=spInterfaceSpeedHC, spTMSCommunity=spTMSCommunity, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spVersion=spVersion, spImpactPps=spImpactPps, licenseError=licenseError, bgpHijack=bgpHijack, flowUp=flowUp, deviceSystemError=deviceSystemError, spTMSPrefix=spTMSPrefix, peakflowSPMgr=peakflowSPMgr, spUsername=spUsername, spCloudSignalFaultDescription=spCloudSignalFaultDescription, mitigationTMSStart=mitigationTMSStart, spThirdPartyAddr=spThirdPartyAddr, interfaceUsageDone=interfaceUsageDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, hardwareFailure=hardwareFailure, spInetAddress=spInetAddress, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spRouter=spRouter, deviceSystemErrorDone=deviceSystemErrorDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spFingerprintName=spFingerprintName, spReportID=spReportID, deviceSwapSpace=deviceSwapSpace, fingerprintFeedback=fingerprintFeedback, spMitigationID=spMitigationID, spUsage=spUsage, interfaceUsage=interfaceUsage, managedObjectUsage=managedObjectUsage, peakflowSPTraps=peakflowSPTraps, spUnit=spUnit)
