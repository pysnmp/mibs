...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceSwapSpaceInUse=deviceSwapSpaceInUse, spSystemErrorDescription=spSystemErrorDescription, dosHostDetectionAlert=dosHostDetectionAlert, managedObjectUsageDone=managedObjectUsageDone, spHardwareFailureDescription=spHardwareFailureDescription, spHijackLocal=spHijackLocal, fingerprintUsage=fingerprintUsage, spUnit=spUnit, alertScript=alertScript, deviceSwapSpaceUsage=deviceSwapSpaceUsage, cloudSignalingFaultDone=cloudSignalingFaultDone, devicePhysicalMemory=devicePhysicalMemory, spScriptHost=spScriptHost, spBGPTrapPrefix=spBGPTrapPrefix, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, bgpTrap=bgpTrap, spAlertID=spAlertID, spBlackholeNexthop=spBlackholeNexthop, spInterfaceSpeedHC=spInterfaceSpeedHC, mitigationDone=mitigationDone, spLicenseErrType=spLicenseErrType, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, cloudSignalingFault=cloudSignalingFault, spImpactPps=spImpactPps, spApplicationName=spApplicationName, peakflowSPTraps=peakflowSPTraps, spTMSPrefix=spTMSPrefix, spInterface=spInterface, spReportID=spReportID, spServiceName=spServiceName, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, mitigationTMSStartV2=mitigationTMSStartV2, spImpactBps=spImpactBps, bgpDown=bgpDown, spReportName=spReportName, managedObjectUsage=managedObjectUsage, peakflowSPMIB=peakflowSPMIB, spBlackholeCommunity=spBlackholeCommunity, spLicenseErrDescription=spLicenseErrDescription, hardwareFailureDone=hardwareFailureDone, spScriptStart=spScriptStart, spFlowspecTimeout=spFlowspecTimeout, spMitigationStart=spMitigationStart, spRouter=spRouter, spCommFailureDescription=spCommFailureDescription, spCommFailureDestination=spCommFailureDestination, bgpHijack=bgpHijack, peakflowSPMgr=peakflowSPMgr, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spUsageType=spUsageType, spHijackRoute=spHijackRoute, spFingerprintName=spFingerprintName, flowUp=flowUp, spDetector=spDetector, fingerprintFeedback=fingerprintFeedback, spThirdPartyAddr=spThirdPartyAddr, spBGPInstability=spBGPInstability, collectorUp=collectorUp, spMitigationName=spMitigationName, routingFailover=routingFailover, spFlowspecCommunity=spFlowspecCommunity, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, licenseError=licenseError, spMitigationID=spMitigationID, spScriptPort=spScriptPort, spDetectedCountries=spDetectedCountries, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, snmpDown=snmpDown, collectorStart=collectorStart, bgpUp=bgpUp, spUsage=spUsage, peakflowSPCMI=peakflowSPCMI, dnsBaselineDone=dnsBaselineDone, deviceHealth=deviceHealth, deviceSwapSpace=deviceSwapSpace, spGreTunnelName=spGreTunnelName, spDuration=spDuration, spUsername=spUsername, spManagedObjectFamily=spManagedObjectFamily, spLicenseErrCount=spLicenseErrCount, spDNSObservedMean=spDNSObservedMean, greDownDone=greDownDone, bgpInstabilityDone=bgpInstabilityDone, spFingerprintFeedback=spFingerprintFeedback, spPravail=spPravail, autoclassifyStart=autoclassifyStart, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, reportDone=reportDone, flowDown=flowDown, spVersion=spVersion, spHijackAttr=spHijackAttr, spBGPEvent=spBGPEvent, fingerprintUsageDone=fingerprintUsageDone, fingerprintReceive=fingerprintReceive, spThreshold=spThreshold, spCollector=spCollector, spFingerprintMessage=spFingerprintMessage, spInterfaceUsageHC=spInterfaceUsageHC, spBGPTrapEvent=spBGPTrapEvent, notificationLimit=notificationLimit, collectorDown=collectorDown, spTMSCommunity=spTMSCommunity, spBlackholeTimeout=spBlackholeTimeout, spManagedObjects=spManagedObjects, spInetAddress=spInetAddress, deviceTotalFlows=deviceTotalFlows, deviceSystemError=deviceSystemError, trafficAutoMitigation=trafficAutoMitigation, spLicenseErrLimit=spLicenseErrLimit, spSystemErrorType=spSystemErrorType, spInterfaceIndex=spInterfaceIndex, spDNSObserved=spDNSObserved, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInterfaceSpeed=spInterfaceSpeed, spDNSName=spDNSName, bgpInstability=bgpInstability, serviceUsageDone=serviceUsageDone, interfaceUsageDone=interfaceUsageDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, PYSNMP_MODULE_ID=peakflowSPMIB, spReportURL=spReportURL, spDNSExpected=spDNSExpected, spScriptCommand=spScriptCommand, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPTrapName=spBGPTrapName, mitigationTMSStart=mitigationTMSStart, spTMSTimeout=spTMSTimeout, spInetAddressType=spInetAddressType, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spAlertDetectionSignatures=spAlertDetectionSignatures, configChange=configChange, deviceSystemErrorDone=deviceSystemErrorDone, mitigationFlowspecStart=mitigationFlowspecStart, snmpUp=snmpUp, spcommFailureDone=spcommFailureDone, spInterfaceUsage=spInterfaceUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spBlackholePrefix=spBlackholePrefix, spServiceElement=spServiceElement, spcommFailure=spcommFailure, dosNetworkProfiledAlert=dosNetworkProfiledAlert, hardwareFailure=hardwareFailure, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spManagedObject=spManagedObject, greDown=greDown, spThirdPartyZone=spThirdPartyZone, spTMSMultiPrefix=spTMSMultiPrefix, interfaceUsage=interfaceUsage, spFingerprintSender=spFingerprintSender, spDNSObservedMax=spDNSObservedMax, mitigationBlackholeStart=mitigationBlackholeStart, dnsBaseline=dnsBaseline, licenseErrorDone=licenseErrorDone, serviceUsage=serviceUsage, deviceDiskUsage=deviceDiskUsage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spGreTunnelDestination=spGreTunnelDestination, deviceTotalFlowsHC=deviceTotalFlowsHC)
