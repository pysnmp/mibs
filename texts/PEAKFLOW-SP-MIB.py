...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spHijackRoute=spHijackRoute, collectorStart=collectorStart, cloudSignalingFaultDone=cloudSignalingFaultDone, peakflowSPMIB=peakflowSPMIB, spMitigationID=spMitigationID, spCommFailureDestination=spCommFailureDestination, spcommFailureDone=spcommFailureDone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spFlowspecTimeout=spFlowspecTimeout, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spDNSExpected=spDNSExpected, spDNSObservedMax=spDNSObservedMax, spFingerprintSender=spFingerprintSender, managedObjectUsageDone=managedObjectUsageDone, spUsername=spUsername, notificationLimit=notificationLimit, alertScript=alertScript, spReportID=spReportID, spLicenseErrLimit=spLicenseErrLimit, spFlowspecCommunity=spFlowspecCommunity, spRouter=spRouter, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceSystemErrorDone=deviceSystemErrorDone, dnsBaseline=dnsBaseline, spTMSCommunity=spTMSCommunity, bgpInstability=bgpInstability, spManagedObject=spManagedObject, spLicenseErrCount=spLicenseErrCount, greDown=greDown, licenseErrorDone=licenseErrorDone, spBlackholeNexthop=spBlackholeNexthop, deviceTotalFlowsHC=deviceTotalFlowsHC, spAlertDetectionSignatures=spAlertDetectionSignatures, interfaceUsage=interfaceUsage, spGreTunnelName=spGreTunnelName, spUsage=spUsage, routingFailover=routingFailover, spThirdPartyAddr=spThirdPartyAddr, spBlackholeTimeout=spBlackholeTimeout, spThirdPartyZone=spThirdPartyZone, deviceSystemError=deviceSystemError, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spInterface=spInterface, fingerprintUsageDone=fingerprintUsageDone, spCollector=spCollector, spUsageType=spUsageType, serviceUsageDone=serviceUsageDone, spFingerprintFeedback=spFingerprintFeedback, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceSwapSpace=deviceSwapSpace, spVersion=spVersion, configChange=configChange, trafficAutoMitigation=trafficAutoMitigation, spBGPTrapPrefix=spBGPTrapPrefix, spScriptHost=spScriptHost, spInterfaceUsageHC=spInterfaceUsageHC, mitigationFlowspecStart=mitigationFlowspecStart, spThreshold=spThreshold, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spApplicationName=spApplicationName, spManagedObjectFamily=spManagedObjectFamily, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spMitigationStart=spMitigationStart, deviceDiskUsage=deviceDiskUsage, spBGPEvent=spBGPEvent, spImpactBps=spImpactBps, spDetector=spDetector, deviceTotalFlows=deviceTotalFlows, spManagedObjects=spManagedObjects, spImpactPps=spImpactPps, spPravail=spPravail, spBlackholePrefix=spBlackholePrefix, dnsBaselineDone=dnsBaselineDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, devicePhysicalMemory=devicePhysicalMemory, hardwareFailure=hardwareFailure, spUnit=spUnit, spInetAddressType=spInetAddressType, spHardwareFailureDescription=spHardwareFailureDescription, collectorDown=collectorDown, peakflowSPTraps=peakflowSPTraps, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spDNSObservedMean=spDNSObservedMean, spAlertID=spAlertID, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spFingerprintMessage=spFingerprintMessage, spServiceElement=spServiceElement, bgpUp=bgpUp, managedObjectUsage=managedObjectUsage, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, interfaceUsageDone=interfaceUsageDone, autoclassifyStart=autoclassifyStart, spFingerprintName=spFingerprintName, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spLicenseErrDescription=spLicenseErrDescription, mitigationThirdPartyStart=mitigationThirdPartyStart, spSystemErrorDescription=spSystemErrorDescription, dosHostDetectionAlert=dosHostDetectionAlert, spReportURL=spReportURL, fingerprintFeedback=fingerprintFeedback, spcommFailure=spcommFailure, serviceUsage=serviceUsage, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spSystemErrorType=spSystemErrorType, spCloudSignalFaultDescription=spCloudSignalFaultDescription, collectorUp=collectorUp, flowUp=flowUp, spMitigationName=spMitigationName, greDownDone=greDownDone, bgpInstabilityDone=bgpInstabilityDone, spBGPInstability=spBGPInstability, peakflowSPMgr=peakflowSPMgr, spScriptCommand=spScriptCommand, cloudSignalingFault=cloudSignalingFault, spServiceName=spServiceName, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBlackholeCommunity=spBlackholeCommunity, spDuration=spDuration, spInetAddress=spInetAddress, spScriptPort=spScriptPort, bgpDown=bgpDown, spInterfaceUsage=spInterfaceUsage, spDNSObserved=spDNSObserved, flowDown=flowDown, bgpHijack=bgpHijack, mitigationTMSStartV2=mitigationTMSStartV2, mitigationBlackholeStart=mitigationBlackholeStart, fingerprintReceive=fingerprintReceive, PYSNMP_MODULE_ID=peakflowSPMIB, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spDNSName=spDNSName, spLicenseErrType=spLicenseErrType, spTMSMultiPrefix=spTMSMultiPrefix, mitigationTMSStart=mitigationTMSStart, spBGPTrapEvent=spBGPTrapEvent, snmpUp=snmpUp, spScriptStart=spScriptStart, spDetectedCountries=spDetectedCountries, spInterfaceIndex=spInterfaceIndex, spCommFailureDescription=spCommFailureDescription, spBGPTrapName=spBGPTrapName, peakflowSPCMI=peakflowSPCMI, deviceHealth=deviceHealth, licenseError=licenseError, spHijackAttr=spHijackAttr, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, fingerprintUsage=fingerprintUsage, snmpDown=snmpDown, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spGreTunnelDestination=spGreTunnelDestination, spTMSTimeout=spTMSTimeout, hardwareFailureDone=hardwareFailureDone, spInterfaceSpeed=spInterfaceSpeed, bgpTrap=bgpTrap, spReportName=spReportName, mitigationDone=mitigationDone, reportDone=reportDone, spTMSPrefix=spTMSPrefix, spHijackLocal=spHijackLocal)
