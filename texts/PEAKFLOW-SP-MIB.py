...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceUsageHC=spInterfaceUsageHC, fingerprintReceive=fingerprintReceive, managedObjectUsageDone=managedObjectUsageDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spAlertID=spAlertID, spScriptStart=spScriptStart, spFlowspecCommunity=spFlowspecCommunity, spPravail=spPravail, spTMSMultiPrefix=spTMSMultiPrefix, spManagedObjectFamily=spManagedObjectFamily, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, collectorStart=collectorStart, spCommFailureDescription=spCommFailureDescription, spBGPTrapOldAttributes=spBGPTrapOldAttributes, collectorUp=collectorUp, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spDNSObservedMean=spDNSObservedMean, spUsername=spUsername, dnsBaseline=dnsBaseline, greDownDone=greDownDone, spBGPTrapPrefix=spBGPTrapPrefix, spBlackholePrefix=spBlackholePrefix, spThreshold=spThreshold, spInterfaceUsage=spInterfaceUsage, devicePhysicalMemory=devicePhysicalMemory, deviceSystemError=deviceSystemError, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spGreTunnelDestination=spGreTunnelDestination, spDetector=spDetector, deviceTotalFlows=deviceTotalFlows, serviceUsageDone=serviceUsageDone, bgpInstability=bgpInstability, spManagedObject=spManagedObject, spInterfaceIndex=spInterfaceIndex, spHijackRoute=spHijackRoute, deviceSwapSpaceInUse=deviceSwapSpaceInUse, collectorDown=collectorDown, mitigationTMSStart=mitigationTMSStart, spFingerprintMessage=spFingerprintMessage, spDetectedCountries=spDetectedCountries, spFingerprintFeedback=spFingerprintFeedback, spInetAddressType=spInetAddressType, spcommFailure=spcommFailure, spUsage=spUsage, fingerprintFeedback=fingerprintFeedback, spMitigationName=spMitigationName, fingerprintUsage=fingerprintUsage, spLicenseErrType=spLicenseErrType, dosHostDetectionAlert=dosHostDetectionAlert, spBGPEvent=spBGPEvent, spCollector=spCollector, spInterfaceSpeed=spInterfaceSpeed, deviceDiskUsage=deviceDiskUsage, mitigationBlackholeStart=mitigationBlackholeStart, flowUp=flowUp, spDNSName=spDNSName, managedObjectUsage=managedObjectUsage, peakflowSPMIB=peakflowSPMIB, spInterface=spInterface, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spFingerprintSender=spFingerprintSender, spMitigationStart=spMitigationStart, spTMSPrefix=spTMSPrefix, spApplicationName=spApplicationName, spHijackLocal=spHijackLocal, configChange=configChange, snmpUp=snmpUp, fingerprintUsageDone=fingerprintUsageDone, spFingerprintName=spFingerprintName, spScriptPort=spScriptPort, bgpHijack=bgpHijack, deviceTotalFlowsHC=deviceTotalFlowsHC, spUnit=spUnit, licenseError=licenseError, mitigationTMSStartV2=mitigationTMSStartV2, serviceUsage=serviceUsage, alertScript=alertScript, spSystemErrorType=spSystemErrorType, spDNSObserved=spDNSObserved, interfaceUsage=interfaceUsage, spVersion=spVersion, peakflowSPTraps=peakflowSPTraps, spLicenseErrDescription=spLicenseErrDescription, spBlackholeTimeout=spBlackholeTimeout, spThirdPartyAddr=spThirdPartyAddr, spGreTunnelName=spGreTunnelName, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spDuration=spDuration, spCommFailureDestination=spCommFailureDestination, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spManagedObjects=spManagedObjects, spTMSCommunity=spTMSCommunity, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceSwapSpace=deviceSwapSpace, peakflowSPCMI=peakflowSPCMI, spHardwareFailureDescription=spHardwareFailureDescription, deviceSystemErrorDone=deviceSystemErrorDone, spBGPTrapEvent=spBGPTrapEvent, hardwareFailureDone=hardwareFailureDone, spcommFailureDone=spcommFailureDone, spFlowspecTimeout=spFlowspecTimeout, PYSNMP_MODULE_ID=peakflowSPMIB, cloudSignalingFault=cloudSignalingFault, flowDown=flowDown, spDNSExpected=spDNSExpected, spThirdPartyZone=spThirdPartyZone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, bgpDown=bgpDown, dnsBaselineDone=dnsBaselineDone, spInetAddress=spInetAddress, mitigationThirdPartyStart=mitigationThirdPartyStart, trafficAutoMitigation=trafficAutoMitigation, bgpInstabilityDone=bgpInstabilityDone, hardwareFailure=hardwareFailure, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spAlertDetectionSignatures=spAlertDetectionSignatures, cloudSignalingFaultDone=cloudSignalingFaultDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBlackholeCommunity=spBlackholeCommunity, spReportURL=spReportURL, spSystemErrorDescription=spSystemErrorDescription, reportDone=reportDone, spLicenseErrCount=spLicenseErrCount, mitigationFlowspecStart=mitigationFlowspecStart, spHijackAttr=spHijackAttr, spMitigationID=spMitigationID, spServiceName=spServiceName, interfaceUsageDone=interfaceUsageDone, spDNSObservedMax=spDNSObservedMax, spScriptCommand=spScriptCommand, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, notificationLimit=notificationLimit, spInterfaceSpeedHC=spInterfaceSpeedHC, licenseErrorDone=licenseErrorDone, spUsageType=spUsageType, spBGPInstability=spBGPInstability, spImpactPps=spImpactPps, spBGPTrapName=spBGPTrapName, spReportID=spReportID, spImpactBps=spImpactBps, greDown=greDown, spBGPTrapNewAttributes=spBGPTrapNewAttributes, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spServiceElement=spServiceElement, deviceSwapSpaceUsage=deviceSwapSpaceUsage, mitigationDone=mitigationDone, spReportName=spReportName, bgpTrap=bgpTrap, spBlackholeNexthop=spBlackholeNexthop, snmpDown=snmpDown, autoclassifyStart=autoclassifyStart, spLicenseErrLimit=spLicenseErrLimit, routingFailover=routingFailover, spRouter=spRouter, spTMSTimeout=spTMSTimeout, spScriptHost=spScriptHost, deviceHealth=deviceHealth, peakflowSPMgr=peakflowSPMgr, bgpUp=bgpUp)
