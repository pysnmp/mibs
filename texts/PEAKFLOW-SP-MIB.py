...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBlackholeNexthop=spBlackholeNexthop, hardwareFailure=hardwareFailure, spSystemErrorDescription=spSystemErrorDescription, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBGPTrapEvent=spBGPTrapEvent, spBlackholePrefix=spBlackholePrefix, spMitigationStart=spMitigationStart, spBGPEvent=spBGPEvent, spBlackholeTimeout=spBlackholeTimeout, peakflowSPCMI=peakflowSPCMI, spInetAddress=spInetAddress, spApplicationName=spApplicationName, bgpInstabilityDone=bgpInstabilityDone, spFingerprintFeedback=spFingerprintFeedback, spGreTunnelName=spGreTunnelName, spHijackRoute=spHijackRoute, deviceSystemError=deviceSystemError, spUsageType=spUsageType, collectorDown=collectorDown, spTMSCommunity=spTMSCommunity, spInterfaceIndex=spInterfaceIndex, spFingerprintMessage=spFingerprintMessage, deviceSwapSpace=deviceSwapSpace, mitigationBlackholeStart=mitigationBlackholeStart, mitigationFlowspecStart=mitigationFlowspecStart, licenseError=licenseError, spMitigationID=spMitigationID, spImpactPps=spImpactPps, fingerprintReceive=fingerprintReceive, spServiceName=spServiceName, mitigationTMSStartV2=mitigationTMSStartV2, bgpInstability=bgpInstability, spGreTunnelDestination=spGreTunnelDestination, greDownDone=greDownDone, reportDone=reportDone, deviceTotalFlows=deviceTotalFlows, serviceUsage=serviceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, spInterface=spInterface, spFingerprintSender=spFingerprintSender, spFlowspecTimeout=spFlowspecTimeout, spVersion=spVersion, flowDown=flowDown, cloudSignalingFaultDone=cloudSignalingFaultDone, flowUp=flowUp, alertScript=alertScript, fingerprintFeedback=fingerprintFeedback, spInterfaceUsage=spInterfaceUsage, spManagedObject=spManagedObject, spBGPTrapPrefix=spBGPTrapPrefix, spRouter=spRouter, bgpUp=bgpUp, snmpDown=snmpDown, bgpHijack=bgpHijack, spUnit=spUnit, dosHostDetectionAlert=dosHostDetectionAlert, spDetector=spDetector, spLicenseErrLimit=spLicenseErrLimit, spUsername=spUsername, mitigationDone=mitigationDone, devicePhysicalMemory=devicePhysicalMemory, spScriptHost=spScriptHost, spHijackAttr=spHijackAttr, spManagedObjects=spManagedObjects, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, collectorUp=collectorUp, spBGPTrapName=spBGPTrapName, spReportURL=spReportURL, notificationLimit=notificationLimit, spPravail=spPravail, spCloudSignalFaultDescription=spCloudSignalFaultDescription, interfaceUsage=interfaceUsage, autoclassifyStart=autoclassifyStart, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spDNSObservedMax=spDNSObservedMax, spThreshold=spThreshold, spInterfaceSpeedHC=spInterfaceSpeedHC, spLicenseErrCount=spLicenseErrCount, spHardwareFailureDescription=spHardwareFailureDescription, spcommFailureDone=spcommFailureDone, spTMSPrefix=spTMSPrefix, spThirdPartyZone=spThirdPartyZone, fingerprintUsageDone=fingerprintUsageDone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spHijackLocal=spHijackLocal, spUsage=spUsage, peakflowSPMgr=peakflowSPMgr, spDNSName=spDNSName, spFlowspecCommunity=spFlowspecCommunity, spCommFailureDescription=spCommFailureDescription, mitigationThirdPartyStart=mitigationThirdPartyStart, configChange=configChange, greDown=greDown, routingFailover=routingFailover, spScriptStart=spScriptStart, spThirdPartyAddr=spThirdPartyAddr, hardwareFailureDone=hardwareFailureDone, spLicenseErrType=spLicenseErrType, bgpDown=bgpDown, spDNSObserved=spDNSObserved, deviceHealth=deviceHealth, interfaceUsageDone=interfaceUsageDone, spMitigationName=spMitigationName, serviceUsageDone=serviceUsageDone, spManagedObjectFamily=spManagedObjectFamily, bgpTrap=bgpTrap, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, deviceDiskUsage=deviceDiskUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, deviceSystemErrorDone=deviceSystemErrorDone, dnsBaselineDone=dnsBaselineDone, peakflowSPTraps=peakflowSPTraps, spcommFailure=spcommFailure, spDetectedCountries=spDetectedCountries, deviceSwapSpaceUsage=deviceSwapSpaceUsage, cloudSignalingFault=cloudSignalingFault, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, mitigationTMSStart=mitigationTMSStart, spInterfaceSpeed=spInterfaceSpeed, deviceTotalFlowsHC=deviceTotalFlowsHC, snmpUp=snmpUp, spBlackholeCommunity=spBlackholeCommunity, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spDNSExpected=spDNSExpected, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spCollector=spCollector, collectorStart=collectorStart, managedObjectUsageDone=managedObjectUsageDone, dnsBaseline=dnsBaseline, spServiceElement=spServiceElement, spImpactBps=spImpactBps, spBGPInstability=spBGPInstability, peakflowSPMIB=peakflowSPMIB, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, managedObjectUsage=managedObjectUsage, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spReportName=spReportName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spInetAddressType=spInetAddressType, spCommFailureDestination=spCommFailureDestination, spScriptCommand=spScriptCommand, fingerprintUsage=fingerprintUsage, spAlertDetectionSignatures=spAlertDetectionSignatures, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spFingerprintName=spFingerprintName, spTMSMultiPrefix=spTMSMultiPrefix, spInterfaceUsageHC=spInterfaceUsageHC, licenseErrorDone=licenseErrorDone, spScriptPort=spScriptPort, trafficAutoMitigation=trafficAutoMitigation, spTMSTimeout=spTMSTimeout, spReportID=spReportID, spDNSObservedMean=spDNSObservedMean, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spLicenseErrDescription=spLicenseErrDescription, spSystemErrorType=spSystemErrorType, spDuration=spDuration, spAlertID=spAlertID)
