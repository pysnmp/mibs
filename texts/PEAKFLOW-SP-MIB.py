...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSObserved=spDNSObserved, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spScriptCommand=spScriptCommand, spVersion=spVersion, bgpInstabilityDone=bgpInstabilityDone, spBGPTrapPrefix=spBGPTrapPrefix, deviceSwapSpaceUsage=deviceSwapSpaceUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spApplicationName=spApplicationName, peakflowSPMgr=peakflowSPMgr, PYSNMP_MODULE_ID=peakflowSPMIB, fingerprintUsage=fingerprintUsage, spDNSExpected=spDNSExpected, peakflowSPMIB=peakflowSPMIB, spMitigationStart=spMitigationStart, spCollector=spCollector, cloudSignalingFault=cloudSignalingFault, deviceHealth=deviceHealth, spMitigationID=spMitigationID, spServiceElement=spServiceElement, spcommFailure=spcommFailure, spFingerprintFeedback=spFingerprintFeedback, dnsBaseline=dnsBaseline, spcommFailureDone=spcommFailureDone, spCommFailureDestination=spCommFailureDestination, trafficAutoMitigation=trafficAutoMitigation, spDNSName=spDNSName, dosNetworkProfiledAlert=dosNetworkProfiledAlert, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spBlackholePrefix=spBlackholePrefix, spSystemErrorDescription=spSystemErrorDescription, configChange=configChange, spReportName=spReportName, spManagedObject=spManagedObject, spReportURL=spReportURL, spBGPTrapNewAttributes=spBGPTrapNewAttributes, bgpTrap=bgpTrap, spUsageType=spUsageType, spBlackholeTimeout=spBlackholeTimeout, deviceTotalFlowsHC=deviceTotalFlowsHC, peakflowSPCMI=peakflowSPCMI, snmpUp=snmpUp, spImpactBps=spImpactBps, spTMSMultiPrefix=spTMSMultiPrefix, spInterface=spInterface, bgpUp=bgpUp, mitigationTMSStartV2=mitigationTMSStartV2, spGreTunnelDestination=spGreTunnelDestination, mitigationTMSStart=mitigationTMSStart, deviceDiskUsage=deviceDiskUsage, autoclassifyStart=autoclassifyStart, notificationLimit=notificationLimit, interfaceUsageDone=interfaceUsageDone, bgpInstability=bgpInstability, spDetectedCountries=spDetectedCountries, interfaceUsage=interfaceUsage, fingerprintFeedback=fingerprintFeedback, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, deviceTotalFlows=deviceTotalFlows, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spManagedObjects=spManagedObjects, collectorStart=collectorStart, spFlowspecTimeout=spFlowspecTimeout, collectorDown=collectorDown, spFingerprintMessage=spFingerprintMessage, spTMSTimeout=spTMSTimeout, spHijackLocal=spHijackLocal, spUsage=spUsage, mitigationBlackholeStart=mitigationBlackholeStart, spAlertID=spAlertID, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spLicenseErrLimit=spLicenseErrLimit, bgpHijack=bgpHijack, spBlackholeNexthop=spBlackholeNexthop, spReportID=spReportID, spBGPEvent=spBGPEvent, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, mitigationThirdPartyStart=mitigationThirdPartyStart, fingerprintReceive=fingerprintReceive, serviceUsage=serviceUsage, dnsBaselineDone=dnsBaselineDone, spBlackholeCommunity=spBlackholeCommunity, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spHijackRoute=spHijackRoute, spPravail=spPravail, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spFingerprintName=spFingerprintName, hardwareFailure=hardwareFailure, snmpDown=snmpDown, spDNSObservedMean=spDNSObservedMean, spScriptStart=spScriptStart, spTMSPrefix=spTMSPrefix, devicePhysicalMemory=devicePhysicalMemory, bgpDown=bgpDown, deviceSystemErrorDone=deviceSystemErrorDone, spScriptHost=spScriptHost, spGreTunnelName=spGreTunnelName, spDuration=spDuration, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spCommFailureDescription=spCommFailureDescription, mitigationFlowspecStart=mitigationFlowspecStart, spHijackAttr=spHijackAttr, spInetAddressType=spInetAddressType, spRouter=spRouter, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spInterfaceIndex=spInterfaceIndex, spManagedObjectFamily=spManagedObjectFamily, spUsername=spUsername, spDetector=spDetector, licenseError=licenseError, spTMSCommunity=spTMSCommunity, spImpactPps=spImpactPps, deviceSwapSpace=deviceSwapSpace, spFingerprintSender=spFingerprintSender, spInterfaceUsage=spInterfaceUsage, spThirdPartyZone=spThirdPartyZone, serviceUsageDone=serviceUsageDone, spLicenseErrCount=spLicenseErrCount, routingFailover=routingFailover, licenseErrorDone=licenseErrorDone, deviceSystemError=deviceSystemError, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, fingerprintUsageDone=fingerprintUsageDone, spFlowspecCommunity=spFlowspecCommunity, flowUp=flowUp, spThirdPartyAddr=spThirdPartyAddr, reportDone=reportDone, managedObjectUsage=managedObjectUsage, cloudSignalingFaultDone=cloudSignalingFaultDone, spUnit=spUnit, spBGPInstability=spBGPInstability, collectorUp=collectorUp, greDown=greDown, spScriptPort=spScriptPort, spInterfaceSpeed=spInterfaceSpeed, spSystemErrorType=spSystemErrorType, spInetAddress=spInetAddress, spDNSObservedMax=spDNSObservedMax, managedObjectUsageDone=managedObjectUsageDone, peakflowSPTraps=peakflowSPTraps, spBGPTrapName=spBGPTrapName, spServiceName=spServiceName, mitigationDone=mitigationDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spHardwareFailureDescription=spHardwareFailureDescription, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spLicenseErrType=spLicenseErrType, alertScript=alertScript, spAlertDetectionSignatures=spAlertDetectionSignatures, spThreshold=spThreshold, spBGPTrapEvent=spBGPTrapEvent, dosHostDetectionAlert=dosHostDetectionAlert, spLicenseErrDescription=spLicenseErrDescription, hardwareFailureDone=hardwareFailureDone, flowDown=flowDown, greDownDone=greDownDone, spMitigationName=spMitigationName, spInterfaceUsageHC=spInterfaceUsageHC, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix)
