...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spManagedObject=spManagedObject, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spImpactPps=spImpactPps, spcommFailure=spcommFailure, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spInterfaceSpeedHC=spInterfaceSpeedHC, collectorDown=collectorDown, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spFlowspecTimeout=spFlowspecTimeout, routingFailover=routingFailover, spDNSObservedMean=spDNSObservedMean, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spBlackholeNexthop=spBlackholeNexthop, spHardwareFailureDescription=spHardwareFailureDescription, alertScript=alertScript, spInterfaceUsageHC=spInterfaceUsageHC, spScriptStart=spScriptStart, deviceHealth=deviceHealth, spTMSCommunity=spTMSCommunity, mitigationThirdPartyStart=mitigationThirdPartyStart, dosHostDetectionAlert=dosHostDetectionAlert, spLicenseErrCount=spLicenseErrCount, spDNSName=spDNSName, bgpHijack=bgpHijack, collectorUp=collectorUp, spFingerprintFeedback=spFingerprintFeedback, spFingerprintName=spFingerprintName, spScriptCommand=spScriptCommand, collectorStart=collectorStart, deviceTotalFlows=deviceTotalFlows, spFingerprintSender=spFingerprintSender, spBGPInstability=spBGPInstability, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spHijackRoute=spHijackRoute, spBGPTrapPrefix=spBGPTrapPrefix, hardwareFailure=hardwareFailure, spScriptHost=spScriptHost, fingerprintUsage=fingerprintUsage, spRouter=spRouter, dnsBaselineDone=dnsBaselineDone, spInterfaceIndex=spInterfaceIndex, bgpDown=bgpDown, spGreTunnelDestination=spGreTunnelDestination, spInetAddressType=spInetAddressType, spUnit=spUnit, bgpUp=bgpUp, managedObjectUsageDone=managedObjectUsageDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spScriptPort=spScriptPort, deviceDiskUsage=deviceDiskUsage, spMitigationName=spMitigationName, fingerprintUsageDone=fingerprintUsageDone, spTMSMultiPrefix=spTMSMultiPrefix, fingerprintReceive=fingerprintReceive, mitigationTMSStart=mitigationTMSStart, spBGPTrapNewAttributes=spBGPTrapNewAttributes, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spApplicationName=spApplicationName, spVersion=spVersion, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, dnsBaseline=dnsBaseline, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, cloudSignalingFault=cloudSignalingFault, spBlackholeCommunity=spBlackholeCommunity, spUsage=spUsage, spDetector=spDetector, spReportID=spReportID, spLicenseErrType=spLicenseErrType, spCommFailureDestination=spCommFailureDestination, deviceTotalFlowsHC=deviceTotalFlowsHC, greDownDone=greDownDone, spcommFailureDone=spcommFailureDone, bgpInstabilityDone=bgpInstabilityDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spTMSTimeout=spTMSTimeout, trafficAutoMitigation=trafficAutoMitigation, licenseErrorDone=licenseErrorDone, peakflowSPCMI=peakflowSPCMI, spThreshold=spThreshold, spTMSPrefix=spTMSPrefix, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceSystemError=deviceSystemError, spInetAddress=spInetAddress, deviceSwapSpaceUsage=deviceSwapSpaceUsage, configChange=configChange, spInterface=spInterface, spThirdPartyZone=spThirdPartyZone, spGreTunnelName=spGreTunnelName, peakflowSPMgr=peakflowSPMgr, spDNSObservedMax=spDNSObservedMax, greDown=greDown, spDetectedCountries=spDetectedCountries, spManagedObjectFamily=spManagedObjectFamily, spSystemErrorDescription=spSystemErrorDescription, spBGPEvent=spBGPEvent, spFlowspecCommunity=spFlowspecCommunity, deviceSwapSpaceInUse=deviceSwapSpaceInUse, reportDone=reportDone, peakflowSPMIB=peakflowSPMIB, notificationLimit=notificationLimit, fingerprintFeedback=fingerprintFeedback, snmpDown=snmpDown, spFingerprintMessage=spFingerprintMessage, spInterfaceUsage=spInterfaceUsage, spLicenseErrDescription=spLicenseErrDescription, snmpUp=snmpUp, spCloudSignalFaultDescription=spCloudSignalFaultDescription, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spReportName=spReportName, spDNSObserved=spDNSObserved, mitigationFlowspecStart=mitigationFlowspecStart, spAlertID=spAlertID, serviceUsage=serviceUsage, peakflowSPTraps=peakflowSPTraps, spImpactBps=spImpactBps, spDuration=spDuration, spCollector=spCollector, deviceSwapSpace=deviceSwapSpace, spMitigationID=spMitigationID, spSystemErrorType=spSystemErrorType, spServiceElement=spServiceElement, flowUp=flowUp, mitigationBlackholeStart=mitigationBlackholeStart, bgpInstability=bgpInstability, interfaceUsage=interfaceUsage, spThirdPartyAddr=spThirdPartyAddr, spHijackLocal=spHijackLocal, spManagedObjects=spManagedObjects, spAlertDetectionSignatures=spAlertDetectionSignatures, hardwareFailureDone=hardwareFailureDone, serviceUsageDone=serviceUsageDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, autoclassifyStart=autoclassifyStart, spDNSExpected=spDNSExpected, spServiceName=spServiceName, PYSNMP_MODULE_ID=peakflowSPMIB, spBlackholeTimeout=spBlackholeTimeout, interfaceUsageDone=interfaceUsageDone, spPravail=spPravail, spMitigationStart=spMitigationStart, deviceSystemErrorDone=deviceSystemErrorDone, flowDown=flowDown, spBlackholePrefix=spBlackholePrefix, spReportURL=spReportURL, managedObjectUsage=managedObjectUsage, spUsageType=spUsageType, cloudSignalingFaultDone=cloudSignalingFaultDone, spCommFailureDescription=spCommFailureDescription, spBGPTrapEvent=spBGPTrapEvent, devicePhysicalMemory=devicePhysicalMemory, bgpTrap=bgpTrap, spInterfaceSpeed=spInterfaceSpeed, spUsername=spUsername, spHijackAttr=spHijackAttr, mitigationDone=mitigationDone, spLicenseErrLimit=spLicenseErrLimit, licenseError=licenseError, mitigationTMSStartV2=mitigationTMSStartV2, spBGPTrapName=spBGPTrapName, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone)
