...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spUnit=spUnit, deviceSwapSpaceInUse=deviceSwapSpaceInUse, managedObjectUsageDone=managedObjectUsageDone, cloudSignalingFaultDone=cloudSignalingFaultDone, dnsBaseline=dnsBaseline, spInterface=spInterface, peakflowSPCMI=peakflowSPCMI, managedObjectUsage=managedObjectUsage, spInterfaceSpeed=spInterfaceSpeed, spVersion=spVersion, spReportURL=spReportURL, spCommFailureDestination=spCommFailureDestination, fingerprintReceive=fingerprintReceive, mitigationFlowspecStart=mitigationFlowspecStart, spLicenseErrCount=spLicenseErrCount, spManagedObjectFamily=spManagedObjectFamily, snmpUp=snmpUp, spServiceName=spServiceName, mitigationTMSStart=mitigationTMSStart, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spLicenseErrType=spLicenseErrType, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, PYSNMP_MODULE_ID=peakflowSPMIB, spDuration=spDuration, spHijackRoute=spHijackRoute, mitigationDone=mitigationDone, spBGPTrapEvent=spBGPTrapEvent, spFingerprintFeedback=spFingerprintFeedback, licenseErrorDone=licenseErrorDone, bgpDown=bgpDown, spScriptPort=spScriptPort, hardwareFailureDone=hardwareFailureDone, spThreshold=spThreshold, dnsBaselineDone=dnsBaselineDone, interfaceUsageDone=interfaceUsageDone, spMitigationID=spMitigationID, bgpTrap=bgpTrap, spScriptCommand=spScriptCommand, configChange=configChange, spBlackholeTimeout=spBlackholeTimeout, spInetAddress=spInetAddress, dosHostDetectionAlert=dosHostDetectionAlert, peakflowSPTraps=peakflowSPTraps, deviceTotalFlows=deviceTotalFlows, fingerprintFeedback=fingerprintFeedback, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, flowDown=flowDown, greDown=greDown, spFingerprintName=spFingerprintName, spPravail=spPravail, spUsername=spUsername, spHardwareFailureDescription=spHardwareFailureDescription, spTMSTimeout=spTMSTimeout, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBlackholePrefix=spBlackholePrefix, mitigationThirdPartyStart=mitigationThirdPartyStart, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spMitigationStart=spMitigationStart, spGreTunnelDestination=spGreTunnelDestination, spCommFailureDescription=spCommFailureDescription, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, peakflowSPMIB=peakflowSPMIB, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, serviceUsage=serviceUsage, hardwareFailure=hardwareFailure, devicePhysicalMemory=devicePhysicalMemory, spUsage=spUsage, spDNSName=spDNSName, spInterfaceUsageHC=spInterfaceUsageHC, spApplicationName=spApplicationName, spRouter=spRouter, interfaceUsage=interfaceUsage, spTMSPrefix=spTMSPrefix, spScriptStart=spScriptStart, snmpDown=snmpDown, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spServiceElement=spServiceElement, spInterfaceSpeedHC=spInterfaceSpeedHC, spReportName=spReportName, spBGPTrapName=spBGPTrapName, spInterfaceUsage=spInterfaceUsage, spFingerprintMessage=spFingerprintMessage, cloudSignalingFault=cloudSignalingFault, mitigationBlackholeStart=mitigationBlackholeStart, spDetectedCountries=spDetectedCountries, spTMSMultiPrefix=spTMSMultiPrefix, mitigationTMSStartV2=mitigationTMSStartV2, spManagedObject=spManagedObject, spcommFailure=spcommFailure, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, greDownDone=greDownDone, bgpUp=bgpUp, fingerprintUsageDone=fingerprintUsageDone, spDNSExpected=spDNSExpected, spSystemErrorDescription=spSystemErrorDescription, spBGPEvent=spBGPEvent, trafficAutoMitigation=trafficAutoMitigation, spImpactBps=spImpactBps, spReportID=spReportID, spFlowspecTimeout=spFlowspecTimeout, spBlackholeCommunity=spBlackholeCommunity, serviceUsageDone=serviceUsageDone, fingerprintUsage=fingerprintUsage, spInterfaceIndex=spInterfaceIndex, deviceTotalFlowsHC=deviceTotalFlowsHC, notificationLimit=notificationLimit, spDNSObserved=spDNSObserved, spCollector=spCollector, spAlertDetectionSignatures=spAlertDetectionSignatures, spLicenseErrLimit=spLicenseErrLimit, spLicenseErrDescription=spLicenseErrDescription, deviceHealth=deviceHealth, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, flowUp=flowUp, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, alertScript=alertScript, spSystemErrorType=spSystemErrorType, licenseError=licenseError, spFingerprintSender=spFingerprintSender, spThirdPartyZone=spThirdPartyZone, spManagedObjects=spManagedObjects, spFlowspecCommunity=spFlowspecCommunity, spBGPTrapNewAttributes=spBGPTrapNewAttributes, collectorDown=collectorDown, bgpInstability=bgpInstability, spHijackAttr=spHijackAttr, spGreTunnelName=spGreTunnelName, spMitigationName=spMitigationName, spBGPTrapPrefix=spBGPTrapPrefix, spBlackholeNexthop=spBlackholeNexthop, collectorStart=collectorStart, spInetAddressType=spInetAddressType, reportDone=reportDone, spDNSObservedMean=spDNSObservedMean, spThirdPartyAddr=spThirdPartyAddr, spDetector=spDetector, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceSwapSpace=deviceSwapSpace, spHijackLocal=spHijackLocal, peakflowSPMgr=peakflowSPMgr, collectorUp=collectorUp, spBGPInstability=spBGPInstability, spDNSObservedMax=spDNSObservedMax, routingFailover=routingFailover, spScriptHost=spScriptHost, deviceSystemErrorDone=deviceSystemErrorDone, deviceSystemError=deviceSystemError, spTMSCommunity=spTMSCommunity, autoclassifyStart=autoclassifyStart, spAlertID=spAlertID, deviceDiskUsage=deviceDiskUsage, spImpactPps=spImpactPps, spcommFailureDone=spcommFailureDone, bgpInstabilityDone=bgpInstabilityDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spUsageType=spUsageType, bgpHijack=bgpHijack)
