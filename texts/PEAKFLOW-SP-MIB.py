...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spFlowspecCommunity=spFlowspecCommunity, peakflowSPMIB=peakflowSPMIB, spDNSExpected=spDNSExpected, collectorUp=collectorUp, spFingerprintSender=spFingerprintSender, spGreTunnelDestination=spGreTunnelDestination, collectorStart=collectorStart, spAlertID=spAlertID, spApplicationName=spApplicationName, dosNetworkProfiledAlert=dosNetworkProfiledAlert, greDownDone=greDownDone, spLicenseErrLimit=spLicenseErrLimit, spTMSCommunity=spTMSCommunity, spCollector=spCollector, spBlackholeTimeout=spBlackholeTimeout, snmpDown=snmpDown, spReportID=spReportID, spGreTunnelName=spGreTunnelName, spDuration=spDuration, spcommFailureDone=spcommFailureDone, spUsername=spUsername, spBGPTrapName=spBGPTrapName, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spImpactPps=spImpactPps, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, managedObjectUsageDone=managedObjectUsageDone, bgpInstabilityDone=bgpInstabilityDone, spBGPEvent=spBGPEvent, spMitigationName=spMitigationName, fingerprintReceive=fingerprintReceive, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceHealth=deviceHealth, spInetAddressType=spInetAddressType, routingFailover=routingFailover, spBGPTrapEvent=spBGPTrapEvent, peakflowSPTraps=peakflowSPTraps, flowUp=flowUp, spServiceElement=spServiceElement, spScriptCommand=spScriptCommand, spUsage=spUsage, spInterfaceUsage=spInterfaceUsage, autoclassifyStart=autoclassifyStart, mitigationTMSStart=mitigationTMSStart, interfaceUsageDone=interfaceUsageDone, spScriptStart=spScriptStart, spReportName=spReportName, devicePhysicalMemory=devicePhysicalMemory, spCommFailureDescription=spCommFailureDescription, spReportURL=spReportURL, spInterfaceIndex=spInterfaceIndex, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBGPInstability=spBGPInstability, spLicenseErrType=spLicenseErrType, managedObjectUsage=managedObjectUsage, serviceUsage=serviceUsage, spThirdPartyAddr=spThirdPartyAddr, spManagedObject=spManagedObject, trafficAutoMitigation=trafficAutoMitigation, deviceDiskUsage=deviceDiskUsage, bgpTrap=bgpTrap, spCommFailureDestination=spCommFailureDestination, spHardwareFailureDescription=spHardwareFailureDescription, spBlackholeCommunity=spBlackholeCommunity, deviceSystemErrorDone=deviceSystemErrorDone, licenseErrorDone=licenseErrorDone, spDetector=spDetector, notificationLimit=notificationLimit, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spFlowspecTimeout=spFlowspecTimeout, spServiceName=spServiceName, spLicenseErrDescription=spLicenseErrDescription, spSystemErrorDescription=spSystemErrorDescription, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBlackholeNexthop=spBlackholeNexthop, dnsBaselineDone=dnsBaselineDone, configChange=configChange, spBlackholePrefix=spBlackholePrefix, bgpDown=bgpDown, serviceUsageDone=serviceUsageDone, spThreshold=spThreshold, spDNSName=spDNSName, spManagedObjectFamily=spManagedObjectFamily, greDown=greDown, spVersion=spVersion, cloudSignalingFaultDone=cloudSignalingFaultDone, hardwareFailure=hardwareFailure, spSystemErrorType=spSystemErrorType, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spcommFailure=spcommFailure, spThirdPartyZone=spThirdPartyZone, deviceTotalFlows=deviceTotalFlows, spInterfaceUsageHC=spInterfaceUsageHC, peakflowSPMgr=peakflowSPMgr, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spInterface=spInterface, flowDown=flowDown, spBGPTrapPrefix=spBGPTrapPrefix, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, fingerprintUsageDone=fingerprintUsageDone, deviceSwapSpace=deviceSwapSpace, hardwareFailureDone=hardwareFailureDone, spUsageType=spUsageType, spInterfaceSpeedHC=spInterfaceSpeedHC, dnsBaseline=dnsBaseline, spRouter=spRouter, deviceTotalFlowsHC=deviceTotalFlowsHC, cloudSignalingFault=cloudSignalingFault, spMitigationID=spMitigationID, reportDone=reportDone, spDNSObservedMax=spDNSObservedMax, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, dosHostDetectionAlert=dosHostDetectionAlert, spFingerprintFeedback=spFingerprintFeedback, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spHijackLocal=spHijackLocal, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInterfaceSpeed=spInterfaceSpeed, collectorDown=collectorDown, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDNSObservedMean=spDNSObservedMean, spScriptHost=spScriptHost, spMitigationStart=spMitigationStart, spFingerprintName=spFingerprintName, interfaceUsage=interfaceUsage, spInetAddress=spInetAddress, mitigationDone=mitigationDone, deviceSystemError=deviceSystemError, spTMSMultiPrefix=spTMSMultiPrefix, alertScript=alertScript, bgpInstability=bgpInstability, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spScriptPort=spScriptPort, spManagedObjects=spManagedObjects, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spPravail=spPravail, snmpUp=snmpUp, spHijackRoute=spHijackRoute, bgpUp=bgpUp, spUnit=spUnit, licenseError=licenseError, spDNSObserved=spDNSObserved, spFingerprintMessage=spFingerprintMessage, spTMSPrefix=spTMSPrefix, spTMSTimeout=spTMSTimeout, spDetectedCountries=spDetectedCountries, mitigationBlackholeStart=mitigationBlackholeStart, fingerprintUsage=fingerprintUsage, bgpHijack=bgpHijack, peakflowSPCMI=peakflowSPCMI, spLicenseErrCount=spLicenseErrCount, mitigationFlowspecStart=mitigationFlowspecStart, spHijackAttr=spHijackAttr, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, fingerprintFeedback=fingerprintFeedback, PYSNMP_MODULE_ID=peakflowSPMIB, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spImpactBps=spImpactBps, mitigationThirdPartyStart=mitigationThirdPartyStart, mitigationTMSStartV2=mitigationTMSStartV2)
