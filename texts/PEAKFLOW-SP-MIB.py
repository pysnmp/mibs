...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", mitigationDone=mitigationDone, flowUp=flowUp, spCloudSignalFaultDescription=spCloudSignalFaultDescription, alertScript=alertScript, spImpactPps=spImpactPps, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, licenseErrorDone=licenseErrorDone, spInterfaceUsage=spInterfaceUsage, spScriptCommand=spScriptCommand, spManagedObject=spManagedObject, spBlackholeTimeout=spBlackholeTimeout, deviceSwapSpaceInUse=deviceSwapSpaceInUse, serviceUsage=serviceUsage, spServiceName=spServiceName, reportDone=reportDone, spGreTunnelDestination=spGreTunnelDestination, spBlackholePrefix=spBlackholePrefix, spLicenseErrLimit=spLicenseErrLimit, spMitigationName=spMitigationName, spTMSTimeout=spTMSTimeout, spCollector=spCollector, spCommFailureDestination=spCommFailureDestination, spDNSName=spDNSName, snmpDown=snmpDown, peakflowSPMIB=peakflowSPMIB, spVersion=spVersion, bgpUp=bgpUp, collectorStart=collectorStart, deviceSystemError=deviceSystemError, spLicenseErrType=spLicenseErrType, spFlowspecCommunity=spFlowspecCommunity, spThirdPartyZone=spThirdPartyZone, spBlackholeNexthop=spBlackholeNexthop, spFingerprintName=spFingerprintName, deviceTotalFlowsHC=deviceTotalFlowsHC, flowDown=flowDown, spLicenseErrDescription=spLicenseErrDescription, managedObjectUsageDone=managedObjectUsageDone, dnsBaselineDone=dnsBaselineDone, greDown=greDown, spHijackLocal=spHijackLocal, bgpInstabilityDone=bgpInstabilityDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spManagedObjectFamily=spManagedObjectFamily, spApplicationName=spApplicationName, spGreTunnelName=spGreTunnelName, spRouter=spRouter, managedObjectUsage=managedObjectUsage, deviceSystemErrorDone=deviceSystemErrorDone, spUnit=spUnit, bgpTrap=bgpTrap, spManagedObjects=spManagedObjects, mitigationThirdPartyStart=mitigationThirdPartyStart, spReportName=spReportName, spInetAddress=spInetAddress, spScriptHost=spScriptHost, spHardwareFailureDescription=spHardwareFailureDescription, spMitigationID=spMitigationID, spFlowspecTimeout=spFlowspecTimeout, spInterfaceIndex=spInterfaceIndex, interfaceUsageDone=interfaceUsageDone, devicePhysicalMemory=devicePhysicalMemory, fingerprintReceive=fingerprintReceive, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceDiskUsage=deviceDiskUsage, peakflowSPMgr=peakflowSPMgr, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, bgpHijack=bgpHijack, cloudSignalingFaultDone=cloudSignalingFaultDone, spInterface=spInterface, spInetAddressType=spInetAddressType, mitigationTMSStart=mitigationTMSStart, spMitigationStart=spMitigationStart, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceTotalFlows=deviceTotalFlows, dnsBaseline=dnsBaseline, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spCommFailureDescription=spCommFailureDescription, spInterfaceUsageHC=spInterfaceUsageHC, cloudSignalingFault=cloudSignalingFault, spTMSMultiPrefix=spTMSMultiPrefix, spAlertDetectionSignatures=spAlertDetectionSignatures, fingerprintFeedback=fingerprintFeedback, fingerprintUsageDone=fingerprintUsageDone, spBGPTrapName=spBGPTrapName, spReportID=spReportID, spDNSObservedMean=spDNSObservedMean, serviceUsageDone=serviceUsageDone, spDNSObservedMax=spDNSObservedMax, notificationLimit=notificationLimit, collectorDown=collectorDown, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spFingerprintMessage=spFingerprintMessage, licenseError=licenseError, spPravail=spPravail, spThirdPartyAddr=spThirdPartyAddr, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spcommFailure=spcommFailure, spFingerprintFeedback=spFingerprintFeedback, spUsage=spUsage, spLicenseErrCount=spLicenseErrCount, spDuration=spDuration, spBGPInstability=spBGPInstability, dosHostDetectionAlert=dosHostDetectionAlert, spReportURL=spReportURL, peakflowSPTraps=peakflowSPTraps, spHijackRoute=spHijackRoute, deviceHealth=deviceHealth, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, deviceSwapSpace=deviceSwapSpace, bgpDown=bgpDown, greDownDone=greDownDone, routingFailover=routingFailover, mitigationFlowspecStart=mitigationFlowspecStart, spBlackholeCommunity=spBlackholeCommunity, spDetector=spDetector, hardwareFailure=hardwareFailure, spDNSExpected=spDNSExpected, configChange=configChange, spBGPTrapEvent=spBGPTrapEvent, spThreshold=spThreshold, hardwareFailureDone=hardwareFailureDone, spUsername=spUsername, spScriptPort=spScriptPort, spDetectedCountries=spDetectedCountries, spTMSPrefix=spTMSPrefix, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spServiceElement=spServiceElement, spInterfaceSpeed=spInterfaceSpeed, spBGPEvent=spBGPEvent, spHijackAttr=spHijackAttr, trafficAutoMitigation=trafficAutoMitigation, spFingerprintSender=spFingerprintSender, spSystemErrorType=spSystemErrorType, spcommFailureDone=spcommFailureDone, bgpInstability=bgpInstability, spScriptStart=spScriptStart, spBGPTrapNewAttributes=spBGPTrapNewAttributes, fingerprintUsage=fingerprintUsage, interfaceUsage=interfaceUsage, spBGPTrapPrefix=spBGPTrapPrefix, mitigationBlackholeStart=mitigationBlackholeStart, spInterfaceSpeedHC=spInterfaceSpeedHC, spUsageType=spUsageType, autoclassifyStart=autoclassifyStart, PYSNMP_MODULE_ID=peakflowSPMIB, spImpactBps=spImpactBps, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, collectorUp=collectorUp, snmpUp=snmpUp, spAlertID=spAlertID, peakflowSPCMI=peakflowSPCMI, spSystemErrorDescription=spSystemErrorDescription, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, mitigationTMSStartV2=mitigationTMSStartV2, spTMSCommunity=spTMSCommunity, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spDNSObserved=spDNSObserved)
