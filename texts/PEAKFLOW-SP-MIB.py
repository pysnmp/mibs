...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", bgpTrap=bgpTrap, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, flowDown=flowDown, spSystemErrorType=spSystemErrorType, spInterfaceSpeed=spInterfaceSpeed, spBlackholeTimeout=spBlackholeTimeout, spInterfaceIndex=spInterfaceIndex, spCommFailureDescription=spCommFailureDescription, spServiceName=spServiceName, dosHostDetectionAlert=dosHostDetectionAlert, spFingerprintFeedback=spFingerprintFeedback, spDuration=spDuration, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spBGPTrapPrefix=spBGPTrapPrefix, spBGPInstability=spBGPInstability, spUnit=spUnit, spMitigationID=spMitigationID, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spPravail=spPravail, spDNSName=spDNSName, spTMSMultiPrefix=spTMSMultiPrefix, fingerprintFeedback=fingerprintFeedback, notificationLimit=notificationLimit, spFingerprintName=spFingerprintName, spBGPTrapName=spBGPTrapName, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, PYSNMP_MODULE_ID=peakflowSPMIB, bgpInstability=bgpInstability, spInterfaceUsageHC=spInterfaceUsageHC, spInetAddressType=spInetAddressType, mitigationDone=mitigationDone, peakflowSPMgr=peakflowSPMgr, spcommFailure=spcommFailure, mitigationBlackholeStart=mitigationBlackholeStart, cloudSignalingFault=cloudSignalingFault, routingFailover=routingFailover, spReportID=spReportID, flowUp=flowUp, dosHostDetectionAlertDone=dosHostDetectionAlertDone, mitigationFlowspecStart=mitigationFlowspecStart, spHijackRoute=spHijackRoute, spApplicationName=spApplicationName, spGreTunnelDestination=spGreTunnelDestination, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spcommFailureDone=spcommFailureDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDNSObserved=spDNSObserved, spSystemErrorDescription=spSystemErrorDescription, spAlertDetectionSignatures=spAlertDetectionSignatures, spServiceElement=spServiceElement, peakflowSPTraps=peakflowSPTraps, snmpDown=snmpDown, spUsername=spUsername, bgpInstabilityDone=bgpInstabilityDone, spMitigationName=spMitigationName, spCommFailureDestination=spCommFailureDestination, spFingerprintSender=spFingerprintSender, peakflowSPMIB=peakflowSPMIB, mitigationTMSStartV2=mitigationTMSStartV2, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSExpected=spDNSExpected, dnsBaseline=dnsBaseline, fingerprintUsageDone=fingerprintUsageDone, autoclassifyStart=autoclassifyStart, dnsBaselineDone=dnsBaselineDone, spTMSTimeout=spTMSTimeout, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceSystemError=deviceSystemError, spDNSObservedMean=spDNSObservedMean, fingerprintUsage=fingerprintUsage, bgpDown=bgpDown, interfaceUsage=interfaceUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spRouter=spRouter, serviceUsage=serviceUsage, spFlowspecCommunity=spFlowspecCommunity, snmpUp=snmpUp, spReportURL=spReportURL, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, deviceTotalFlows=deviceTotalFlows, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceSystemErrorDone=deviceSystemErrorDone, spBlackholeNexthop=spBlackholeNexthop, spUsage=spUsage, deviceSwapSpace=deviceSwapSpace, mitigationTMSStart=mitigationTMSStart, spHardwareFailureDescription=spHardwareFailureDescription, fingerprintReceive=fingerprintReceive, alertScript=alertScript, licenseErrorDone=licenseErrorDone, spScriptStart=spScriptStart, spInterfaceUsage=spInterfaceUsage, spLicenseErrCount=spLicenseErrCount, spThirdPartyAddr=spThirdPartyAddr, spManagedObjectFamily=spManagedObjectFamily, spBlackholePrefix=spBlackholePrefix, bgpHijack=bgpHijack, devicePhysicalMemory=devicePhysicalMemory, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spLicenseErrType=spLicenseErrType, spCollector=spCollector, spScriptCommand=spScriptCommand, spTMSCommunity=spTMSCommunity, collectorStart=collectorStart, spReportName=spReportName, spInterface=spInterface, greDownDone=greDownDone, spInetAddress=spInetAddress, greDown=greDown, serviceUsageDone=serviceUsageDone, spScriptPort=spScriptPort, spLicenseErrLimit=spLicenseErrLimit, spBlackholeCommunity=spBlackholeCommunity, configChange=configChange, managedObjectUsageDone=managedObjectUsageDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spVersion=spVersion, spThreshold=spThreshold, mitigationThirdPartyStart=mitigationThirdPartyStart, spUsageType=spUsageType, spDetectedCountries=spDetectedCountries, collectorDown=collectorDown, hardwareFailureDone=hardwareFailureDone, licenseError=licenseError, deviceTotalFlowsHC=deviceTotalFlowsHC, spDetector=spDetector, deviceDiskUsage=deviceDiskUsage, spThirdPartyZone=spThirdPartyZone, interfaceUsageDone=interfaceUsageDone, hardwareFailure=hardwareFailure, spImpactBps=spImpactBps, dosNetworkProfiledAlert=dosNetworkProfiledAlert, peakflowSPCMI=peakflowSPCMI, deviceHealth=deviceHealth, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spManagedObjects=spManagedObjects, collectorUp=collectorUp, cloudSignalingFaultDone=cloudSignalingFaultDone, spGreTunnelName=spGreTunnelName, spHijackLocal=spHijackLocal, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapEvent=spBGPTrapEvent, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spImpactPps=spImpactPps, spManagedObject=spManagedObject, spFingerprintMessage=spFingerprintMessage, spScriptHost=spScriptHost, trafficAutoMitigation=trafficAutoMitigation, managedObjectUsage=managedObjectUsage, spMitigationStart=spMitigationStart, bgpUp=bgpUp, spBGPEvent=spBGPEvent, spFlowspecTimeout=spFlowspecTimeout, spAlertID=spAlertID, reportDone=reportDone, spTMSPrefix=spTMSPrefix, spHijackAttr=spHijackAttr, spDNSObservedMax=spDNSObservedMax)
