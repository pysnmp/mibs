...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", peakflowSPCMI=peakflowSPCMI, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, fingerprintUsageDone=fingerprintUsageDone, spUsername=spUsername, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spServiceName=spServiceName, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spVersion=spVersion, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceTotalFlows=deviceTotalFlows, reportDone=reportDone, spBGPTrapName=spBGPTrapName, spReportID=spReportID, managedObjectUsage=managedObjectUsage, spMitigationID=spMitigationID, deviceSystemError=deviceSystemError, peakflowSPTraps=peakflowSPTraps, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spHijackRoute=spHijackRoute, flowDown=flowDown, serviceUsageDone=serviceUsageDone, spUsageType=spUsageType, spManagedObject=spManagedObject, interfaceUsageDone=interfaceUsageDone, spDNSExpected=spDNSExpected, managedObjectUsageDone=managedObjectUsageDone, spLicenseErrDescription=spLicenseErrDescription, devicePhysicalMemory=devicePhysicalMemory, spcommFailureDone=spcommFailureDone, spManagedObjects=spManagedObjects, autoclassifyStart=autoclassifyStart, spDNSName=spDNSName, spManagedObjectFamily=spManagedObjectFamily, spImpactPps=spImpactPps, hardwareFailureDone=hardwareFailureDone, deviceSwapSpace=deviceSwapSpace, collectorStart=collectorStart, spHijackAttr=spHijackAttr, spFlowspecCommunity=spFlowspecCommunity, spHardwareFailureDescription=spHardwareFailureDescription, spScriptHost=spScriptHost, peakflowSPMIB=peakflowSPMIB, spCommFailureDestination=spCommFailureDestination, dnsBaselineDone=dnsBaselineDone, spCollector=spCollector, hardwareFailure=hardwareFailure, spBlackholeTimeout=spBlackholeTimeout, spAlertID=spAlertID, bgpHijack=bgpHijack, bgpTrap=bgpTrap, peakflowSPMgr=peakflowSPMgr, spInterface=spInterface, spAlertDetectionSignatures=spAlertDetectionSignatures, bgpDown=bgpDown, dnsBaseline=dnsBaseline, spCommFailureDescription=spCommFailureDescription, spMitigationName=spMitigationName, deviceHealth=deviceHealth, spTMSTimeout=spTMSTimeout, fingerprintFeedback=fingerprintFeedback, spLicenseErrCount=spLicenseErrCount, spLicenseErrLimit=spLicenseErrLimit, fingerprintUsage=fingerprintUsage, spHijackLocal=spHijackLocal, spcommFailure=spcommFailure, spInterfaceSpeedHC=spInterfaceSpeedHC, configChange=configChange, spTMSCommunity=spTMSCommunity, spGreTunnelDestination=spGreTunnelDestination, spInterfaceUsageHC=spInterfaceUsageHC, spReportName=spReportName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, flowUp=flowUp, spReportURL=spReportURL, licenseErrorDone=licenseErrorDone, spRouter=spRouter, collectorDown=collectorDown, spThreshold=spThreshold, spGreTunnelName=spGreTunnelName, dosHostDetectionAlert=dosHostDetectionAlert, greDownDone=greDownDone, spInterfaceIndex=spInterfaceIndex, spDuration=spDuration, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, deviceSwapSpaceInUse=deviceSwapSpaceInUse, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spUnit=spUnit, spBlackholeNexthop=spBlackholeNexthop, trafficAutoMitigation=trafficAutoMitigation, mitigationTMSStartV2=mitigationTMSStartV2, mitigationBlackholeStart=mitigationBlackholeStart, bgpInstability=bgpInstability, collectorUp=collectorUp, cloudSignalingFaultDone=cloudSignalingFaultDone, alertScript=alertScript, spBlackholePrefix=spBlackholePrefix, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spServiceElement=spServiceElement, spDetectedCountries=spDetectedCountries, bgpUp=bgpUp, cloudSignalingFault=cloudSignalingFault, spBlackholeCommunity=spBlackholeCommunity, deviceSwapSpaceUsage=deviceSwapSpaceUsage, routingFailover=routingFailover, spInterfaceSpeed=spInterfaceSpeed, spInetAddressType=spInetAddressType, dosHostDetectionAlertDone=dosHostDetectionAlertDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spTMSPrefix=spTMSPrefix, snmpUp=snmpUp, mitigationFlowspecStart=mitigationFlowspecStart, bgpInstabilityDone=bgpInstabilityDone, spInetAddress=spInetAddress, spSystemErrorDescription=spSystemErrorDescription, deviceTotalFlowsHC=deviceTotalFlowsHC, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spFingerprintName=spFingerprintName, spDetector=spDetector, deviceDiskUsage=deviceDiskUsage, licenseError=licenseError, spFingerprintMessage=spFingerprintMessage, interfaceUsage=interfaceUsage, spFingerprintFeedback=spFingerprintFeedback, spInterfaceUsage=spInterfaceUsage, spBGPTrapPrefix=spBGPTrapPrefix, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceSystemErrorDone=deviceSystemErrorDone, spLicenseErrType=spLicenseErrType, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spImpactBps=spImpactBps, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, notificationLimit=notificationLimit, spApplicationName=spApplicationName, snmpDown=snmpDown, spFingerprintSender=spFingerprintSender, spBGPTrapEvent=spBGPTrapEvent, spThirdPartyAddr=spThirdPartyAddr, spDNSObserved=spDNSObserved, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spThirdPartyZone=spThirdPartyZone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, serviceUsage=serviceUsage, spScriptCommand=spScriptCommand, spScriptStart=spScriptStart, mitigationDone=mitigationDone, spBGPEvent=spBGPEvent, spDNSObservedMean=spDNSObservedMean, mitigationTMSStart=mitigationTMSStart, PYSNMP_MODULE_ID=peakflowSPMIB, spTMSMultiPrefix=spTMSMultiPrefix, spSystemErrorType=spSystemErrorType, spDNSObservedMax=spDNSObservedMax, spUsage=spUsage, spFlowspecTimeout=spFlowspecTimeout, fingerprintReceive=fingerprintReceive, spBGPInstability=spBGPInstability, greDown=greDown, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spScriptPort=spScriptPort, spPravail=spPravail, spMitigationStart=spMitigationStart)
