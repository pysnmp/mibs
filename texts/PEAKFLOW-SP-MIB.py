...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spServiceElement=spServiceElement, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBGPTrapNewAttributes=spBGPTrapNewAttributes, notificationLimit=notificationLimit, spCollector=spCollector, licenseError=licenseError, snmpUp=snmpUp, dosHostDetectionAlert=dosHostDetectionAlert, spBGPInstability=spBGPInstability, spInetAddressType=spInetAddressType, spServiceName=spServiceName, spInetAddress=spInetAddress, spFingerprintSender=spFingerprintSender, spcommFailureDone=spcommFailureDone, deviceSystemErrorDone=deviceSystemErrorDone, spDNSExpected=spDNSExpected, spUsageType=spUsageType, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spFingerprintName=spFingerprintName, bgpTrap=bgpTrap, spVersion=spVersion, mitigationThirdPartyStart=mitigationThirdPartyStart, spLicenseErrDescription=spLicenseErrDescription, spUsername=spUsername, deviceHealth=deviceHealth, spFingerprintMessage=spFingerprintMessage, dnsBaseline=dnsBaseline, spInterface=spInterface, spLicenseErrLimit=spLicenseErrLimit, spScriptStart=spScriptStart, spLicenseErrCount=spLicenseErrCount, interfaceUsageDone=interfaceUsageDone, spFlowspecTimeout=spFlowspecTimeout, serviceUsage=serviceUsage, spBlackholeTimeout=spBlackholeTimeout, flowDown=flowDown, greDown=greDown, peakflowSPTraps=peakflowSPTraps, spSystemErrorDescription=spSystemErrorDescription, spRouter=spRouter, spBlackholeCommunity=spBlackholeCommunity, bgpInstability=bgpInstability, spMitigationID=spMitigationID, trafficAutoMitigation=trafficAutoMitigation, fingerprintUsage=fingerprintUsage, spScriptPort=spScriptPort, bgpInstabilityDone=bgpInstabilityDone, spBGPTrapEvent=spBGPTrapEvent, fingerprintFeedback=fingerprintFeedback, spcommFailure=spcommFailure, mitigationTMSStartV2=mitigationTMSStartV2, spInterfaceUsage=spInterfaceUsage, spReportName=spReportName, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spImpactPps=spImpactPps, dosHostDetectionAlertDone=dosHostDetectionAlertDone, mitigationFlowspecStart=mitigationFlowspecStart, deviceSystemError=deviceSystemError, peakflowSPCMI=peakflowSPCMI, reportDone=reportDone, spDetector=spDetector, serviceUsageDone=serviceUsageDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spFlowspecCommunity=spFlowspecCommunity, spTMSTimeout=spTMSTimeout, licenseErrorDone=licenseErrorDone, spScriptHost=spScriptHost, spBlackholePrefix=spBlackholePrefix, snmpDown=snmpDown, spUsage=spUsage, spManagedObject=spManagedObject, spAlertID=spAlertID, spTMSCommunity=spTMSCommunity, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDNSName=spDNSName, spTMSPrefix=spTMSPrefix, spThreshold=spThreshold, spTMSMultiPrefix=spTMSMultiPrefix, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, peakflowSPMIB=peakflowSPMIB, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spInterfaceSpeed=spInterfaceSpeed, bgpUp=bgpUp, autoclassifyStart=autoclassifyStart, alertScript=alertScript, spInterfaceSpeedHC=spInterfaceSpeedHC, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, PYSNMP_MODULE_ID=peakflowSPMIB, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, devicePhysicalMemory=devicePhysicalMemory, spAlertDetectionSignatures=spAlertDetectionSignatures, spBGPTrapName=spBGPTrapName, peakflowSPMgr=peakflowSPMgr, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, bgpDown=bgpDown, interfaceUsage=interfaceUsage, spDuration=spDuration, collectorDown=collectorDown, bgpHijack=bgpHijack, spBlackholeNexthop=spBlackholeNexthop, spInterfaceUsageHC=spInterfaceUsageHC, spBGPEvent=spBGPEvent, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spHijackRoute=spHijackRoute, flowUp=flowUp, spGreTunnelName=spGreTunnelName, collectorUp=collectorUp, spMitigationStart=spMitigationStart, spReportURL=spReportURL, cloudSignalingFault=cloudSignalingFault, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spCommFailureDescription=spCommFailureDescription, spDetectedCountries=spDetectedCountries, mitigationBlackholeStart=mitigationBlackholeStart, spMitigationName=spMitigationName, cloudSignalingFaultDone=cloudSignalingFaultDone, spLicenseErrType=spLicenseErrType, spScriptCommand=spScriptCommand, routingFailover=routingFailover, spReportID=spReportID, spGreTunnelDestination=spGreTunnelDestination, spThirdPartyAddr=spThirdPartyAddr, deviceTotalFlowsHC=deviceTotalFlowsHC, managedObjectUsage=managedObjectUsage, dnsBaselineDone=dnsBaselineDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBGPTrapPrefix=spBGPTrapPrefix, fingerprintUsageDone=fingerprintUsageDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spHijackLocal=spHijackLocal, spManagedObjectFamily=spManagedObjectFamily, hardwareFailureDone=hardwareFailureDone, hardwareFailure=hardwareFailure, spManagedObjects=spManagedObjects, fingerprintReceive=fingerprintReceive, spCommFailureDestination=spCommFailureDestination, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, collectorStart=collectorStart, deviceDiskUsage=deviceDiskUsage, spFingerprintFeedback=spFingerprintFeedback, deviceTotalFlows=deviceTotalFlows, greDownDone=greDownDone, managedObjectUsageDone=managedObjectUsageDone, spHardwareFailureDescription=spHardwareFailureDescription, spImpactBps=spImpactBps, spPravail=spPravail, mitigationDone=mitigationDone, mitigationTMSStart=mitigationTMSStart, spHijackAttr=spHijackAttr, deviceSwapSpace=deviceSwapSpace, spThirdPartyZone=spThirdPartyZone, spInterfaceIndex=spInterfaceIndex, spApplicationName=spApplicationName, spDNSObservedMax=spDNSObservedMax, spDNSObserved=spDNSObserved, spUnit=spUnit, spSystemErrorType=spSystemErrorType, configChange=configChange, spDNSObservedMean=spDNSObservedMean)
