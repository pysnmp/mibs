...
pdosAnomalyStart, pdosAnomalyDuration, pdosAnomalyDirection, pdosUrl, pdosAnomalyClassification = mibBuilder.importSymbols("PEAKFLOW-DOS-MIB", "pdosAnomalyStart", "pdosAnomalyDuration", "pdosAnomalyDirection", "pdosUrl", "pdosAnomalyClassification")
...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", licenseError=licenseError, spRouter=spRouter, spBlackholeNexthop=spBlackholeNexthop, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spThreshold=spThreshold, spCommFailureDescription=spCommFailureDescription, mitigationTMSStart=mitigationTMSStart, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spTMSMultiPrefix=spTMSMultiPrefix, spScriptCommand=spScriptCommand, fingerprintReceive=fingerprintReceive, spInterfaceSpeed=spInterfaceSpeed, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBlackholeTimeout=spBlackholeTimeout, spSystemErrorDescription=spSystemErrorDescription, fingerprintUsage=fingerprintUsage, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceDiskUsage=deviceDiskUsage, hardwareFailure=hardwareFailure, spScriptHost=spScriptHost, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spSystemErrorType=spSystemErrorType, mitigationFlowspecStart=mitigationFlowspecStart, routingFailover=routingFailover, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceSwapSpace=deviceSwapSpace, spGreTunnelDestination=spGreTunnelDestination, serviceUsage=serviceUsage, spInterfaceIndex=spInterfaceIndex, peakflowSPMIB=peakflowSPMIB, spImpactBps=spImpactBps, deviceHealth=deviceHealth, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFlowspecCommunity=spFlowspecCommunity, spDuration=spDuration, spScriptPort=spScriptPort, spUsername=spUsername, managedObjectUsageDone=managedObjectUsageDone, PYSNMP_MODULE_ID=peakflowSPMIB, spReportURL=spReportURL, spHijackAttr=spHijackAttr, dnsBaselineDone=dnsBaselineDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spThirdPartyZone=spThirdPartyZone, flowDown=flowDown, collectorStart=collectorStart, spMitigationStart=spMitigationStart, spCollector=spCollector, dosHostDetectionAlertDone=dosHostDetectionAlertDone, configChange=configChange, spBGPEvent=spBGPEvent, dosNetworkProfiledAlert=dosNetworkProfiledAlert, snmpUp=snmpUp, interfaceUsageDone=interfaceUsageDone, spThirdPartyAddr=spThirdPartyAddr, bgpInstability=bgpInstability, spReportID=spReportID, spInterface=spInterface, bgpInstabilityDone=bgpInstabilityDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spTMSCommunity=spTMSCommunity, spFingerprintName=spFingerprintName, spApplicationName=spApplicationName, cloudSignalingFault=cloudSignalingFault, spReportName=spReportName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, greDown=greDown, spLicenseErrLimit=spLicenseErrLimit, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBGPTrapPrefix=spBGPTrapPrefix, spGreTunnelName=spGreTunnelName, spScriptStart=spScriptStart, devicePhysicalMemory=devicePhysicalMemory, spBGPTrapEvent=spBGPTrapEvent, peakflowSPTraps=peakflowSPTraps, spcommFailure=spcommFailure, reportDone=reportDone, spDNSName=spDNSName, spInterfaceUsage=spInterfaceUsage, trafficAutoMitigation=trafficAutoMitigation, managedObjectUsage=managedObjectUsage, spServiceElement=spServiceElement, spHardwareFailureDescription=spHardwareFailureDescription, flowUp=flowUp, spDNSExpected=spDNSExpected, spMitigationID=spMitigationID, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spImpactPps=spImpactPps, spInetAddressType=spInetAddressType, spManagedObjects=spManagedObjects, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spVersion=spVersion, spTMSPrefix=spTMSPrefix, collectorUp=collectorUp, spBGPTrapName=spBGPTrapName, deviceSystemError=deviceSystemError, fingerprintFeedback=fingerprintFeedback, notificationLimit=notificationLimit, bgpHijack=bgpHijack, spFingerprintSender=spFingerprintSender, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spServiceName=spServiceName, cloudSignalingFaultDone=cloudSignalingFaultDone, peakflowSPCMI=peakflowSPCMI, deviceSystemErrorDone=deviceSystemErrorDone, greDownDone=greDownDone, spDetector=spDetector, serviceUsageDone=serviceUsageDone, spHijackRoute=spHijackRoute, bgpTrap=bgpTrap, spLicenseErrType=spLicenseErrType, spUnit=spUnit, spcommFailureDone=spcommFailureDone, snmpDown=snmpDown, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spFlowspecTimeout=spFlowspecTimeout, spUsageType=spUsageType, autoclassifyStart=autoclassifyStart, spAlertDetectionSignatures=spAlertDetectionSignatures, licenseErrorDone=licenseErrorDone, peakflowSPMgr=peakflowSPMgr, mitigationDone=mitigationDone, collectorDown=collectorDown, dnsBaseline=dnsBaseline, mitigationTMSStartV2=mitigationTMSStartV2, spFingerprintMessage=spFingerprintMessage, spTMSTimeout=spTMSTimeout, spManagedObject=spManagedObject, spManagedObjectFamily=spManagedObjectFamily, spInterfaceUsageHC=spInterfaceUsageHC, spMitigationName=spMitigationName, spDNSObserved=spDNSObserved, spFingerprintFeedback=spFingerprintFeedback, spDetectedCountries=spDetectedCountries, spInetAddress=spInetAddress, spDNSObservedMean=spDNSObservedMean, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spDNSObservedMax=spDNSObservedMax, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceTotalFlows=deviceTotalFlows, bgpUp=bgpUp, mitigationThirdPartyStart=mitigationThirdPartyStart, interfaceUsage=interfaceUsage, spAlertID=spAlertID, spUsage=spUsage, dosHostDetectionAlert=dosHostDetectionAlert, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spLicenseErrCount=spLicenseErrCount, fingerprintUsageDone=fingerprintUsageDone, mitigationBlackholeStart=mitigationBlackholeStart, spBlackholeCommunity=spBlackholeCommunity, spCommFailureDestination=spCommFailureDestination, spBGPInstability=spBGPInstability, spHijackLocal=spHijackLocal, bgpDown=bgpDown, hardwareFailureDone=hardwareFailureDone, deviceTotalFlowsHC=deviceTotalFlowsHC, spPravail=spPravail, alertScript=alertScript, spBlackholePrefix=spBlackholePrefix)
