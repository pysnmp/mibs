...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spTMSPrefix=spTMSPrefix, deviceSwapSpace=deviceSwapSpace, cloudSignalingFaultDone=cloudSignalingFaultDone, spLicenseErrType=spLicenseErrType, spBGPEvent=spBGPEvent, spServiceName=spServiceName, licenseErrorDone=licenseErrorDone, spBGPInstability=spBGPInstability, greDownDone=greDownDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spUsageType=spUsageType, spInterfaceIndex=spInterfaceIndex, snmpDown=snmpDown, spDetector=spDetector, spHardwareFailureDescription=spHardwareFailureDescription, bgpDown=bgpDown, spLicenseErrDescription=spLicenseErrDescription, spFingerprintFeedback=spFingerprintFeedback, dnsBaseline=dnsBaseline, spBlackholeNexthop=spBlackholeNexthop, snmpUp=snmpUp, spLicenseErrLimit=spLicenseErrLimit, spImpactBps=spImpactBps, devicePhysicalMemory=devicePhysicalMemory, spDuration=spDuration, PYSNMP_MODULE_ID=peakflowSPMIB, spInterfaceUsageHC=spInterfaceUsageHC, licenseError=licenseError, mitigationDone=mitigationDone, spManagedObjects=spManagedObjects, fingerprintReceive=fingerprintReceive, trafficAutoMitigation=trafficAutoMitigation, deviceSwapSpaceInUse=deviceSwapSpaceInUse, dosNetworkProfiledAlert=dosNetworkProfiledAlert, peakflowSPMgr=peakflowSPMgr, deviceTotalFlowsHC=deviceTotalFlowsHC, fingerprintUsage=fingerprintUsage, spThirdPartyZone=spThirdPartyZone, mitigationTMSStartV2=mitigationTMSStartV2, spVersion=spVersion, spGreTunnelName=spGreTunnelName, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spCommFailureDescription=spCommFailureDescription, spImpactPps=spImpactPps, spThreshold=spThreshold, spHijackAttr=spHijackAttr, spSystemErrorType=spSystemErrorType, dosHostDetectionAlert=dosHostDetectionAlert, managedObjectUsage=managedObjectUsage, spLicenseErrCount=spLicenseErrCount, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, collectorStart=collectorStart, greDown=greDown, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, hardwareFailure=hardwareFailure, spReportName=spReportName, spDNSObservedMean=spDNSObservedMean, spMitigationID=spMitigationID, collectorDown=collectorDown, spcommFailureDone=spcommFailureDone, spInterfaceUsage=spInterfaceUsage, mitigationFlowspecStart=mitigationFlowspecStart, spScriptStart=spScriptStart, spFlowspecCommunity=spFlowspecCommunity, spSystemErrorDescription=spSystemErrorDescription, spAlertDetectionSignatures=spAlertDetectionSignatures, spFingerprintSender=spFingerprintSender, configChange=configChange, spBGPTrapPrefix=spBGPTrapPrefix, spDetectedCountries=spDetectedCountries, spApplicationName=spApplicationName, spTMSTimeout=spTMSTimeout, managedObjectUsageDone=managedObjectUsageDone, bgpInstabilityDone=bgpInstabilityDone, spTMSCommunity=spTMSCommunity, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, serviceUsage=serviceUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spMitigationStart=spMitigationStart, deviceTotalFlows=deviceTotalFlows, spDNSObservedMax=spDNSObservedMax, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spGreTunnelDestination=spGreTunnelDestination, spPravail=spPravail, spManagedObjectFamily=spManagedObjectFamily, dosHostDetectionAlertDone=dosHostDetectionAlertDone, peakflowSPCMI=peakflowSPCMI, peakflowSPMIB=peakflowSPMIB, hardwareFailureDone=hardwareFailureDone, spBlackholeCommunity=spBlackholeCommunity, spScriptCommand=spScriptCommand, interfaceUsageDone=interfaceUsageDone, spInetAddress=spInetAddress, notificationLimit=notificationLimit, bgpTrap=bgpTrap, spMitigationName=spMitigationName, spInetAddressType=spInetAddressType, spCollector=spCollector, deviceSystemError=deviceSystemError, spManagedObject=spManagedObject, spFlowspecTimeout=spFlowspecTimeout, deviceHealth=deviceHealth, spInterfaceSpeed=spInterfaceSpeed, spReportURL=spReportURL, fingerprintUsageDone=fingerprintUsageDone, alertScript=alertScript, spUsername=spUsername, spDNSExpected=spDNSExpected, interfaceUsage=interfaceUsage, spReportID=spReportID, spBGPTrapEvent=spBGPTrapEvent, spTMSMultiPrefix=spTMSMultiPrefix, autoclassifyStart=autoclassifyStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, peakflowSPTraps=peakflowSPTraps, serviceUsageDone=serviceUsageDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spUsage=spUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spAlertID=spAlertID, spHijackLocal=spHijackLocal, collectorUp=collectorUp, mitigationTMSStart=mitigationTMSStart, deviceDiskUsage=deviceDiskUsage, routingFailover=routingFailover, spFingerprintName=spFingerprintName, spInterface=spInterface, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, dnsBaselineDone=dnsBaselineDone, spBlackholePrefix=spBlackholePrefix, reportDone=reportDone, deviceSystemErrorDone=deviceSystemErrorDone, spScriptHost=spScriptHost, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spServiceElement=spServiceElement, spBlackholeTimeout=spBlackholeTimeout, bgpInstability=bgpInstability, spDNSName=spDNSName, spRouter=spRouter, spBGPTrapNewAttributes=spBGPTrapNewAttributes, cloudSignalingFault=cloudSignalingFault, spHijackRoute=spHijackRoute, spDNSObserved=spDNSObserved, spcommFailure=spcommFailure, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, fingerprintFeedback=fingerprintFeedback, flowDown=flowDown, mitigationBlackholeStart=mitigationBlackholeStart, spUnit=spUnit, spCommFailureDestination=spCommFailureDestination, spBGPTrapName=spBGPTrapName, spFingerprintMessage=spFingerprintMessage, bgpUp=bgpUp, spScriptPort=spScriptPort, spThirdPartyAddr=spThirdPartyAddr, flowUp=flowUp, bgpHijack=bgpHijack, spInterfaceSpeedHC=spInterfaceSpeedHC)
