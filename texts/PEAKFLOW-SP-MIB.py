...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spHijackRoute=spHijackRoute, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spDNSExpected=spDNSExpected, spThirdPartyAddr=spThirdPartyAddr, spDNSObserved=spDNSObserved, spInterface=spInterface, spTMSMultiPrefix=spTMSMultiPrefix, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spGreTunnelDestination=spGreTunnelDestination, managedObjectUsage=managedObjectUsage, spUsername=spUsername, bgpTrap=bgpTrap, fingerprintUsageDone=fingerprintUsageDone, dosHostDetectionAlert=dosHostDetectionAlert, spHardwareFailureDescription=spHardwareFailureDescription, deviceTotalFlowsHC=deviceTotalFlowsHC, hardwareFailureDone=hardwareFailureDone, spInterfaceUsageHC=spInterfaceUsageHC, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, hardwareFailure=hardwareFailure, licenseError=licenseError, mitigationDone=mitigationDone, spDetector=spDetector, spBGPEvent=spBGPEvent, spHijackLocal=spHijackLocal, spManagedObjects=spManagedObjects, managedObjectUsageDone=managedObjectUsageDone, serviceUsageDone=serviceUsageDone, spBlackholePrefix=spBlackholePrefix, spSystemErrorType=spSystemErrorType, peakflowSPMIB=peakflowSPMIB, spImpactBps=spImpactBps, deviceDiskUsage=deviceDiskUsage, flowDown=flowDown, collectorStart=collectorStart, spVersion=spVersion, configChange=configChange, spReportID=spReportID, spMitigationName=spMitigationName, collectorUp=collectorUp, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spUnit=spUnit, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spScriptStart=spScriptStart, licenseErrorDone=licenseErrorDone, dnsBaseline=dnsBaseline, spScriptHost=spScriptHost, cloudSignalingFaultDone=cloudSignalingFaultDone, spCommFailureDescription=spCommFailureDescription, spFlowspecCommunity=spFlowspecCommunity, routingFailover=routingFailover, spBGPTrapEvent=spBGPTrapEvent, spTMSTimeout=spTMSTimeout, deviceHealth=deviceHealth, deviceTotalFlows=deviceTotalFlows, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spSystemErrorDescription=spSystemErrorDescription, spDNSObservedMean=spDNSObservedMean, dnsBaselineDone=dnsBaselineDone, spInterfaceIndex=spInterfaceIndex, autoclassifyStart=autoclassifyStart, spBGPInstability=spBGPInstability, snmpUp=snmpUp, spAlertDetectionSignatures=spAlertDetectionSignatures, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spAlertID=spAlertID, trafficAutoMitigation=trafficAutoMitigation, spPravail=spPravail, serviceUsage=serviceUsage, spApplicationName=spApplicationName, spReportURL=spReportURL, spDNSObservedMax=spDNSObservedMax, spDetectedCountries=spDetectedCountries, peakflowSPTraps=peakflowSPTraps, spThreshold=spThreshold, spCommFailureDestination=spCommFailureDestination, spLicenseErrLimit=spLicenseErrLimit, greDownDone=greDownDone, bgpHijack=bgpHijack, spManagedObject=spManagedObject, mitigationTMSStartV2=mitigationTMSStartV2, spReportName=spReportName, spInetAddress=spInetAddress, spDNSName=spDNSName, spInterfaceSpeedHC=spInterfaceSpeedHC, spUsageType=spUsageType, spServiceName=spServiceName, spGreTunnelName=spGreTunnelName, mitigationTMSStart=mitigationTMSStart, mitigationBlackholeStart=mitigationBlackholeStart, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spFingerprintMessage=spFingerprintMessage, mitigationFlowspecStart=mitigationFlowspecStart, spcommFailureDone=spcommFailureDone, spLicenseErrCount=spLicenseErrCount, spManagedObjectFamily=spManagedObjectFamily, spLicenseErrType=spLicenseErrType, mitigationThirdPartyStart=mitigationThirdPartyStart, spTMSPrefix=spTMSPrefix, spBGPTrapName=spBGPTrapName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spcommFailure=spcommFailure, spScriptCommand=spScriptCommand, deviceSystemErrorDone=deviceSystemErrorDone, spBlackholeNexthop=spBlackholeNexthop, bgpInstability=bgpInstability, snmpDown=snmpDown, spServiceElement=spServiceElement, flowUp=flowUp, bgpUp=bgpUp, spBGPTrapNewAttributes=spBGPTrapNewAttributes, interfaceUsage=interfaceUsage, spFingerprintSender=spFingerprintSender, spInterfaceSpeed=spInterfaceSpeed, interfaceUsageDone=interfaceUsageDone, peakflowSPMgr=peakflowSPMgr, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spScriptPort=spScriptPort, spLicenseErrDescription=spLicenseErrDescription, spThirdPartyZone=spThirdPartyZone, deviceSwapSpace=deviceSwapSpace, spDuration=spDuration, spUsage=spUsage, bgpInstabilityDone=bgpInstabilityDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spBGPTrapPrefix=spBGPTrapPrefix, spBGPTrapOldAttributes=spBGPTrapOldAttributes, fingerprintReceive=fingerprintReceive, alertScript=alertScript, peakflowSPCMI=peakflowSPCMI, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, greDown=greDown, spTMSCommunity=spTMSCommunity, cloudSignalingFault=cloudSignalingFault, collectorDown=collectorDown, spRouter=spRouter, dosHostDetectionAlertDone=dosHostDetectionAlertDone, notificationLimit=notificationLimit, spHijackAttr=spHijackAttr, devicePhysicalMemory=devicePhysicalMemory, spBlackholeTimeout=spBlackholeTimeout, fingerprintFeedback=fingerprintFeedback, spImpactPps=spImpactPps, fingerprintUsage=fingerprintUsage, spFlowspecTimeout=spFlowspecTimeout, deviceSystemError=deviceSystemError, spMitigationID=spMitigationID, spMitigationStart=spMitigationStart, spBlackholeCommunity=spBlackholeCommunity, spCollector=spCollector, spInetAddressType=spInetAddressType, spFingerprintFeedback=spFingerprintFeedback, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFingerprintName=spFingerprintName, spInterfaceUsage=spInterfaceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, reportDone=reportDone, bgpDown=bgpDown, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces)
