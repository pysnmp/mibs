...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInetAddress=spInetAddress, spTMSPrefix=spTMSPrefix, spGreTunnelName=spGreTunnelName, bgpUp=bgpUp, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, snmpUp=snmpUp, deviceHealth=deviceHealth, spCommFailureDescription=spCommFailureDescription, deviceSystemError=deviceSystemError, bgpHijack=bgpHijack, spTMSMultiPrefix=spTMSMultiPrefix, deviceTotalFlowsHC=deviceTotalFlowsHC, spScriptStart=spScriptStart, spBlackholeTimeout=spBlackholeTimeout, spDetector=spDetector, spFlowspecCommunity=spFlowspecCommunity, trafficAutoMitigation=trafficAutoMitigation, spUsage=spUsage, spFingerprintFeedback=spFingerprintFeedback, spLicenseErrCount=spLicenseErrCount, spDNSObservedMean=spDNSObservedMean, mitigationDone=mitigationDone, spInetAddressType=spInetAddressType, spcommFailure=spcommFailure, spSystemErrorDescription=spSystemErrorDescription, spUsername=spUsername, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spThirdPartyAddr=spThirdPartyAddr, interfaceUsage=interfaceUsage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, peakflowSPTraps=peakflowSPTraps, fingerprintReceive=fingerprintReceive, spDNSObserved=spDNSObserved, mitigationBlackholeStart=mitigationBlackholeStart, spDetectedCountries=spDetectedCountries, fingerprintFeedback=fingerprintFeedback, collectorUp=collectorUp, spServiceName=spServiceName, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spReportURL=spReportURL, spInterfaceUsage=spInterfaceUsage, spThirdPartyZone=spThirdPartyZone, spFingerprintMessage=spFingerprintMessage, spBlackholeCommunity=spBlackholeCommunity, spThreshold=spThreshold, mitigationFlowspecStart=mitigationFlowspecStart, spRouter=spRouter, licenseErrorDone=licenseErrorDone, spLicenseErrDescription=spLicenseErrDescription, licenseError=licenseError, notificationLimit=notificationLimit, spGreTunnelDestination=spGreTunnelDestination, spManagedObjects=spManagedObjects, spHijackRoute=spHijackRoute, spManagedObject=spManagedObject, spDuration=spDuration, spDNSName=spDNSName, spBGPTrapName=spBGPTrapName, flowUp=flowUp, PYSNMP_MODULE_ID=peakflowSPMIB, spUsageType=spUsageType, collectorDown=collectorDown, alertScript=alertScript, spBGPTrapEvent=spBGPTrapEvent, cloudSignalingFault=cloudSignalingFault, mitigationTMSStartV2=mitigationTMSStartV2, spBlackholePrefix=spBlackholePrefix, spLicenseErrLimit=spLicenseErrLimit, deviceTotalFlows=deviceTotalFlows, managedObjectUsage=managedObjectUsage, hardwareFailureDone=hardwareFailureDone, spCommFailureDestination=spCommFailureDestination, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceDiskUsage=deviceDiskUsage, spMitigationName=spMitigationName, spFingerprintName=spFingerprintName, collectorStart=collectorStart, reportDone=reportDone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBGPInstability=spBGPInstability, deviceSystemErrorDone=deviceSystemErrorDone, spReportName=spReportName, snmpDown=snmpDown, bgpInstabilityDone=bgpInstabilityDone, flowDown=flowDown, spInterfaceIndex=spInterfaceIndex, routingFailover=routingFailover, spFlowspecTimeout=spFlowspecTimeout, spApplicationName=spApplicationName, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, peakflowSPCMI=peakflowSPCMI, spAlertID=spAlertID, spHijackLocal=spHijackLocal, bgpInstability=bgpInstability, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spcommFailureDone=spcommFailureDone, spFingerprintSender=spFingerprintSender, managedObjectUsageDone=managedObjectUsageDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, peakflowSPMgr=peakflowSPMgr, dosNetworkProfiledAlert=dosNetworkProfiledAlert, bgpDown=bgpDown, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSObservedMax=spDNSObservedMax, dnsBaseline=dnsBaseline, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spMitigationStart=spMitigationStart, devicePhysicalMemory=devicePhysicalMemory, spPravail=spPravail, fingerprintUsage=fingerprintUsage, fingerprintUsageDone=fingerprintUsageDone, spBGPTrapPrefix=spBGPTrapPrefix, dosHostDetectionAlert=dosHostDetectionAlert, spTMSTimeout=spTMSTimeout, spBGPEvent=spBGPEvent, hardwareFailure=hardwareFailure, mitigationTMSStart=mitigationTMSStart, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, peakflowSPMIB=peakflowSPMIB, spCollector=spCollector, spSystemErrorType=spSystemErrorType, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spTMSCommunity=spTMSCommunity, spImpactBps=spImpactBps, spHijackAttr=spHijackAttr, spServiceElement=spServiceElement, spDNSExpected=spDNSExpected, spCloudSignalFaultDescription=spCloudSignalFaultDescription, configChange=configChange, autoclassifyStart=autoclassifyStart, serviceUsage=serviceUsage, spHardwareFailureDescription=spHardwareFailureDescription, deviceSwapSpace=deviceSwapSpace, spManagedObjectFamily=spManagedObjectFamily, spImpactPps=spImpactPps, dnsBaselineDone=dnsBaselineDone, spScriptPort=spScriptPort, spUnit=spUnit, serviceUsageDone=serviceUsageDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spLicenseErrType=spLicenseErrType, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, cloudSignalingFaultDone=cloudSignalingFaultDone, greDownDone=greDownDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spAlertDetectionSignatures=spAlertDetectionSignatures, spVersion=spVersion, spInterfaceUsageHC=spInterfaceUsageHC, greDown=greDown, spScriptCommand=spScriptCommand, spMitigationID=spMitigationID, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, bgpTrap=bgpTrap, spBlackholeNexthop=spBlackholeNexthop, interfaceUsageDone=interfaceUsageDone, spInterface=spInterface, spInterfaceSpeed=spInterfaceSpeed, spScriptHost=spScriptHost, spReportID=spReportID)
