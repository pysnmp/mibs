...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spGreTunnelDestination=spGreTunnelDestination, notificationLimit=notificationLimit, hardwareFailure=hardwareFailure, dosHostDetectionAlert=dosHostDetectionAlert, dnsBaseline=dnsBaseline, spLicenseErrLimit=spLicenseErrLimit, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spCollector=spCollector, hardwareFailureDone=hardwareFailureDone, spUnit=spUnit, peakflowSPTraps=peakflowSPTraps, cloudSignalingFault=cloudSignalingFault, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spServiceName=spServiceName, interfaceUsageDone=interfaceUsageDone, spBGPTrapEvent=spBGPTrapEvent, bgpDown=bgpDown, collectorDown=collectorDown, bgpHijack=bgpHijack, spLicenseErrCount=spLicenseErrCount, spBGPInstability=spBGPInstability, fingerprintUsage=fingerprintUsage, mitigationBlackholeStart=mitigationBlackholeStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, peakflowSPMgr=peakflowSPMgr, spBGPTrapName=spBGPTrapName, deviceDiskUsage=deviceDiskUsage, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spInterfaceUsageHC=spInterfaceUsageHC, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, autoclassifyStart=autoclassifyStart, fingerprintReceive=fingerprintReceive, spHardwareFailureDescription=spHardwareFailureDescription, fingerprintFeedback=fingerprintFeedback, spMitigationName=spMitigationName, mitigationTMSStart=mitigationTMSStart, spInterfaceUsage=spInterfaceUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFingerprintSender=spFingerprintSender, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spSystemErrorType=spSystemErrorType, snmpDown=snmpDown, spBGPEvent=spBGPEvent, deviceTotalFlowsHC=deviceTotalFlowsHC, spFingerprintFeedback=spFingerprintFeedback, spHijackRoute=spHijackRoute, spGreTunnelName=spGreTunnelName, spDNSObserved=spDNSObserved, configChange=configChange, interfaceUsage=interfaceUsage, peakflowSPMIB=peakflowSPMIB, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spUsageType=spUsageType, spThirdPartyAddr=spThirdPartyAddr, spCommFailureDestination=spCommFailureDestination, spFlowspecTimeout=spFlowspecTimeout, greDownDone=greDownDone, spReportURL=spReportURL, spDNSObservedMean=spDNSObservedMean, serviceUsage=serviceUsage, deviceTotalFlows=deviceTotalFlows, spManagedObjects=spManagedObjects, spDuration=spDuration, spDetectedCountries=spDetectedCountries, spHijackLocal=spHijackLocal, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spManagedObjectFamily=spManagedObjectFamily, spTMSPrefix=spTMSPrefix, spBlackholeNexthop=spBlackholeNexthop, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spVersion=spVersion, mitigationThirdPartyStart=mitigationThirdPartyStart, mitigationFlowspecStart=mitigationFlowspecStart, spTMSCommunity=spTMSCommunity, spAlertID=spAlertID, deviceSwapSpaceUsage=deviceSwapSpaceUsage, routingFailover=routingFailover, spManagedObject=spManagedObject, spLicenseErrType=spLicenseErrType, bgpInstabilityDone=bgpInstabilityDone, collectorStart=collectorStart, deviceSystemError=deviceSystemError, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spScriptStart=spScriptStart, spImpactPps=spImpactPps, spPravail=spPravail, spScriptHost=spScriptHost, spInetAddressType=spInetAddressType, snmpUp=snmpUp, deviceHealth=deviceHealth, licenseErrorDone=licenseErrorDone, spDNSObservedMax=spDNSObservedMax, spApplicationName=spApplicationName, deviceSwapSpace=deviceSwapSpace, spUsername=spUsername, dnsBaselineDone=dnsBaselineDone, spFingerprintMessage=spFingerprintMessage, spBlackholePrefix=spBlackholePrefix, spImpactBps=spImpactBps, spcommFailureDone=spcommFailureDone, greDown=greDown, spThreshold=spThreshold, flowDown=flowDown, bgpUp=bgpUp, devicePhysicalMemory=devicePhysicalMemory, reportDone=reportDone, spInterfaceSpeedHC=spInterfaceSpeedHC, spHijackAttr=spHijackAttr, spSystemErrorDescription=spSystemErrorDescription, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spMitigationStart=spMitigationStart, licenseError=licenseError, PYSNMP_MODULE_ID=peakflowSPMIB, spBlackholeTimeout=spBlackholeTimeout, spMitigationID=spMitigationID, alertScript=alertScript, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spReportID=spReportID, bgpTrap=bgpTrap, fingerprintUsageDone=fingerprintUsageDone, peakflowSPCMI=peakflowSPCMI, spDNSExpected=spDNSExpected, spInterfaceSpeed=spInterfaceSpeed, spRouter=spRouter, serviceUsageDone=serviceUsageDone, spcommFailure=spcommFailure, spServiceElement=spServiceElement, spInterfaceIndex=spInterfaceIndex, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, flowUp=flowUp, spDNSName=spDNSName, collectorUp=collectorUp, spInetAddress=spInetAddress, spFlowspecCommunity=spFlowspecCommunity, trafficAutoMitigation=trafficAutoMitigation, spUsage=spUsage, managedObjectUsageDone=managedObjectUsageDone, spScriptPort=spScriptPort, spThirdPartyZone=spThirdPartyZone, spDetector=spDetector, spScriptCommand=spScriptCommand, spTMSTimeout=spTMSTimeout, spBlackholeCommunity=spBlackholeCommunity, spCommFailureDescription=spCommFailureDescription, spLicenseErrDescription=spLicenseErrDescription, managedObjectUsage=managedObjectUsage, deviceSystemErrorDone=deviceSystemErrorDone, spTMSMultiPrefix=spTMSMultiPrefix, spFingerprintName=spFingerprintName, spBGPTrapPrefix=spBGPTrapPrefix, spReportName=spReportName, spInterface=spInterface, mitigationDone=mitigationDone, bgpInstability=bgpInstability, mitigationTMSStartV2=mitigationTMSStartV2)
