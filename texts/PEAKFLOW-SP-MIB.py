...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBGPTrapName=spBGPTrapName, spInterfaceIndex=spInterfaceIndex, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spInterfaceUsageHC=spInterfaceUsageHC, spApplicationName=spApplicationName, spMitigationID=spMitigationID, deviceSystemErrorDone=deviceSystemErrorDone, peakflowSPCMI=peakflowSPCMI, flowDown=flowDown, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spLicenseErrDescription=spLicenseErrDescription, spThirdPartyAddr=spThirdPartyAddr, spManagedObjects=spManagedObjects, bgpUp=bgpUp, autoclassifyStart=autoclassifyStart, devicePhysicalMemory=devicePhysicalMemory, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFingerprintFeedback=spFingerprintFeedback, deviceSwapSpace=deviceSwapSpace, spBGPTrapEvent=spBGPTrapEvent, snmpUp=snmpUp, spUsage=spUsage, spScriptStart=spScriptStart, spReportName=spReportName, spTMSTimeout=spTMSTimeout, spCommFailureDestination=spCommFailureDestination, managedObjectUsageDone=managedObjectUsageDone, spLicenseErrType=spLicenseErrType, deviceHealth=deviceHealth, spVersion=spVersion, bgpInstability=bgpInstability, mitigationFlowspecStart=mitigationFlowspecStart, spServiceElement=spServiceElement, spServiceName=spServiceName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceTotalFlowsHC=deviceTotalFlowsHC, spManagedObject=spManagedObject, spDuration=spDuration, spSystemErrorType=spSystemErrorType, spBGPEvent=spBGPEvent, spRouter=spRouter, spAlertDetectionSignatures=spAlertDetectionSignatures, spBlackholeCommunity=spBlackholeCommunity, licenseErrorDone=licenseErrorDone, bgpDown=bgpDown, spInterface=spInterface, spBlackholePrefix=spBlackholePrefix, collectorUp=collectorUp, interfaceUsageDone=interfaceUsageDone, spcommFailure=spcommFailure, mitigationTMSStart=mitigationTMSStart, hardwareFailureDone=hardwareFailureDone, mitigationBlackholeStart=mitigationBlackholeStart, spInterfaceUsage=spInterfaceUsage, spcommFailureDone=spcommFailureDone, spDetector=spDetector, dnsBaselineDone=dnsBaselineDone, hardwareFailure=hardwareFailure, spImpactPps=spImpactPps, flowUp=flowUp, spUsageType=spUsageType, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, interfaceUsage=interfaceUsage, spReportID=spReportID, dosHostDetectionAlert=dosHostDetectionAlert, spInetAddress=spInetAddress, configChange=configChange, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, serviceUsage=serviceUsage, spMitigationStart=spMitigationStart, dnsBaseline=dnsBaseline, spInterfaceSpeed=spInterfaceSpeed, spScriptCommand=spScriptCommand, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spUsername=spUsername, deviceSwapSpaceUsage=deviceSwapSpaceUsage, serviceUsageDone=serviceUsageDone, spDNSObservedMean=spDNSObservedMean, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spManagedObjectFamily=spManagedObjectFamily, trafficAutoMitigation=trafficAutoMitigation, bgpHijack=bgpHijack, spScriptPort=spScriptPort, spCommFailureDescription=spCommFailureDescription, reportDone=reportDone, spCollector=spCollector, spLicenseErrLimit=spLicenseErrLimit, bgpTrap=bgpTrap, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spTMSMultiPrefix=spTMSMultiPrefix, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, cloudSignalingFaultDone=cloudSignalingFaultDone, spSystemErrorDescription=spSystemErrorDescription, spDetectedCountries=spDetectedCountries, snmpDown=snmpDown, spFingerprintName=spFingerprintName, fingerprintFeedback=fingerprintFeedback, mitigationThirdPartyStart=mitigationThirdPartyStart, peakflowSPMIB=peakflowSPMIB, spDNSObserved=spDNSObserved, spThreshold=spThreshold, fingerprintReceive=fingerprintReceive, deviceTotalFlows=deviceTotalFlows, spDNSExpected=spDNSExpected, spThirdPartyZone=spThirdPartyZone, spFlowspecCommunity=spFlowspecCommunity, spScriptHost=spScriptHost, managedObjectUsage=managedObjectUsage, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spHijackAttr=spHijackAttr, greDown=greDown, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spPravail=spPravail, spUnit=spUnit, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, alertScript=alertScript, spAlertID=spAlertID, mitigationTMSStartV2=mitigationTMSStartV2, spInetAddressType=spInetAddressType, spBGPInstability=spBGPInstability, spInterfaceSpeedHC=spInterfaceSpeedHC, spHijackRoute=spHijackRoute, spBGPTrapOldAttributes=spBGPTrapOldAttributes, peakflowSPMgr=peakflowSPMgr, spBlackholeTimeout=spBlackholeTimeout, spTMSCommunity=spTMSCommunity, spFingerprintMessage=spFingerprintMessage, spFingerprintSender=spFingerprintSender, cloudSignalingFault=cloudSignalingFault, PYSNMP_MODULE_ID=peakflowSPMIB, spLicenseErrCount=spLicenseErrCount, spDNSObservedMax=spDNSObservedMax, spHijackLocal=spHijackLocal, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, peakflowSPTraps=peakflowSPTraps, spTMSPrefix=spTMSPrefix, fingerprintUsageDone=fingerprintUsageDone, mitigationDone=mitigationDone, spGreTunnelDestination=spGreTunnelDestination, deviceDiskUsage=deviceDiskUsage, spMitigationName=spMitigationName, notificationLimit=notificationLimit, bgpInstabilityDone=bgpInstabilityDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceSystemError=deviceSystemError, routingFailover=routingFailover, spBGPTrapPrefix=spBGPTrapPrefix, spReportURL=spReportURL, spImpactBps=spImpactBps, greDownDone=greDownDone, collectorDown=collectorDown, fingerprintUsage=fingerprintUsage, licenseError=licenseError, spHardwareFailureDescription=spHardwareFailureDescription, collectorStart=collectorStart, spDNSName=spDNSName, spGreTunnelName=spGreTunnelName, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spBlackholeNexthop=spBlackholeNexthop, spFlowspecTimeout=spFlowspecTimeout)
