...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spHardwareFailureDescription=spHardwareFailureDescription, spBGPTrapPrefix=spBGPTrapPrefix, spFingerprintFeedback=spFingerprintFeedback, spDNSName=spDNSName, spThirdPartyAddr=spThirdPartyAddr, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInetAddressType=spInetAddressType, spMitigationName=spMitigationName, spThirdPartyZone=spThirdPartyZone, spLicenseErrType=spLicenseErrType, autoclassifyStart=autoclassifyStart, spCollector=spCollector, spInetAddress=spInetAddress, spLicenseErrDescription=spLicenseErrDescription, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spRouter=spRouter, mitigationDone=mitigationDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spDNSExpected=spDNSExpected, spImpactPps=spImpactPps, spInterfaceUsageHC=spInterfaceUsageHC, peakflowSPCMI=peakflowSPCMI, spMitigationStart=spMitigationStart, spBGPInstability=spBGPInstability, spFlowspecCommunity=spFlowspecCommunity, reportDone=reportDone, spTMSCommunity=spTMSCommunity, trafficAutoMitigation=trafficAutoMitigation, spInterfaceIndex=spInterfaceIndex, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, bgpDown=bgpDown, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceHealth=deviceHealth, cloudSignalingFault=cloudSignalingFault, snmpDown=snmpDown, bgpHijack=bgpHijack, serviceUsage=serviceUsage, peakflowSPMIB=peakflowSPMIB, spManagedObjectFamily=spManagedObjectFamily, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBlackholeCommunity=spBlackholeCommunity, spScriptCommand=spScriptCommand, spReportName=spReportName, spScriptPort=spScriptPort, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spCommFailureDestination=spCommFailureDestination, spGreTunnelDestination=spGreTunnelDestination, spAlertID=spAlertID, fingerprintUsageDone=fingerprintUsageDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, interfaceUsageDone=interfaceUsageDone, bgpInstabilityDone=bgpInstabilityDone, deviceTotalFlowsHC=deviceTotalFlowsHC, fingerprintReceive=fingerprintReceive, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceTotalFlows=deviceTotalFlows, spFlowspecTimeout=spFlowspecTimeout, managedObjectUsage=managedObjectUsage, spScriptStart=spScriptStart, mitigationFlowspecStart=mitigationFlowspecStart, spBGPEvent=spBGPEvent, spCommFailureDescription=spCommFailureDescription, devicePhysicalMemory=devicePhysicalMemory, flowUp=flowUp, bgpTrap=bgpTrap, fingerprintFeedback=fingerprintFeedback, deviceSystemErrorDone=deviceSystemErrorDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, notificationLimit=notificationLimit, spTMSMultiPrefix=spTMSMultiPrefix, flowDown=flowDown, spInterfaceSpeed=spInterfaceSpeed, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spFingerprintMessage=spFingerprintMessage, spDNSObservedMax=spDNSObservedMax, peakflowSPTraps=peakflowSPTraps, fingerprintUsage=fingerprintUsage, spHijackLocal=spHijackLocal, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spcommFailure=spcommFailure, spBGPTrapName=spBGPTrapName, cloudSignalingFaultDone=cloudSignalingFaultDone, dosHostDetectionAlert=dosHostDetectionAlert, collectorUp=collectorUp, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spApplicationName=spApplicationName, spInterfaceUsage=spInterfaceUsage, spFingerprintName=spFingerprintName, spUsage=spUsage, spAlertDetectionSignatures=spAlertDetectionSignatures, mitigationTMSStart=mitigationTMSStart, deviceSwapSpace=deviceSwapSpace, spThreshold=spThreshold, spGreTunnelName=spGreTunnelName, mitigationTMSStartV2=mitigationTMSStartV2, spDNSObservedMean=spDNSObservedMean, spDetector=spDetector, spBlackholeTimeout=spBlackholeTimeout, spUsername=spUsername, PYSNMP_MODULE_ID=peakflowSPMIB, spUsageType=spUsageType, bgpUp=bgpUp, alertScript=alertScript, mitigationBlackholeStart=mitigationBlackholeStart, deviceSystemError=deviceSystemError, spHijackRoute=spHijackRoute, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spImpactBps=spImpactBps, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spUnit=spUnit, spDNSObserved=spDNSObserved, bgpInstability=bgpInstability, spPravail=spPravail, spServiceElement=spServiceElement, licenseErrorDone=licenseErrorDone, spDetectedCountries=spDetectedCountries, spLicenseErrLimit=spLicenseErrLimit, licenseError=licenseError, collectorDown=collectorDown, spMitigationID=spMitigationID, dnsBaselineDone=dnsBaselineDone, spDuration=spDuration, deviceDiskUsage=deviceDiskUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, serviceUsageDone=serviceUsageDone, spLicenseErrCount=spLicenseErrCount, configChange=configChange, spcommFailureDone=spcommFailureDone, managedObjectUsageDone=managedObjectUsageDone, spTMSPrefix=spTMSPrefix, spBGPTrapEvent=spBGPTrapEvent, spManagedObjects=spManagedObjects, dnsBaseline=dnsBaseline, routingFailover=routingFailover, hardwareFailureDone=hardwareFailureDone, spSystemErrorType=spSystemErrorType, spVersion=spVersion, spScriptHost=spScriptHost, hardwareFailure=hardwareFailure, collectorStart=collectorStart, spReportURL=spReportURL, greDown=greDown, spBlackholeNexthop=spBlackholeNexthop, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spSystemErrorDescription=spSystemErrorDescription, spInterfaceSpeedHC=spInterfaceSpeedHC, spFingerprintSender=spFingerprintSender, spTMSTimeout=spTMSTimeout, spServiceName=spServiceName, interfaceUsage=interfaceUsage, spReportID=spReportID, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spHijackAttr=spHijackAttr, spManagedObject=spManagedObject, peakflowSPMgr=peakflowSPMgr, spBlackholePrefix=spBlackholePrefix, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, snmpUp=snmpUp, greDownDone=greDownDone, spInterface=spInterface)
