...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBGPTrapNewAttributes=spBGPTrapNewAttributes, spThirdPartyZone=spThirdPartyZone, spServiceElement=spServiceElement, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spManagedObjectFamily=spManagedObjectFamily, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spCollector=spCollector, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBlackholePrefix=spBlackholePrefix, spInterface=spInterface, deviceTotalFlowsHC=deviceTotalFlowsHC, spLicenseErrType=spLicenseErrType, spBGPTrapPrefix=spBGPTrapPrefix, spDetectedCountries=spDetectedCountries, spGreTunnelName=spGreTunnelName, spTMSTimeout=spTMSTimeout, spInterfaceUsageHC=spInterfaceUsageHC, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInetAddress=spInetAddress, spFingerprintSender=spFingerprintSender, interfaceUsage=interfaceUsage, spLicenseErrDescription=spLicenseErrDescription, spFingerprintMessage=spFingerprintMessage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spCommFailureDescription=spCommFailureDescription, spBGPTrapEvent=spBGPTrapEvent, trafficAutoMitigation=trafficAutoMitigation, spBlackholeCommunity=spBlackholeCommunity, deviceHealth=deviceHealth, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spScriptPort=spScriptPort, serviceUsage=serviceUsage, spFlowspecTimeout=spFlowspecTimeout, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spThirdPartyAddr=spThirdPartyAddr, spBlackholeTimeout=spBlackholeTimeout, bgpInstabilityDone=bgpInstabilityDone, managedObjectUsageDone=managedObjectUsageDone, autoclassifyStart=autoclassifyStart, collectorUp=collectorUp, spDNSName=spDNSName, spInterfaceSpeedHC=spInterfaceSpeedHC, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spHijackRoute=spHijackRoute, bgpInstability=bgpInstability, spBlackholeNexthop=spBlackholeNexthop, fingerprintFeedback=fingerprintFeedback, spFlowspecCommunity=spFlowspecCommunity, greDown=greDown, spDuration=spDuration, spThreshold=spThreshold, spUsageType=spUsageType, spDetector=spDetector, spTMSMultiPrefix=spTMSMultiPrefix, mitigationDone=mitigationDone, dosHostDetectionAlert=dosHostDetectionAlert, PYSNMP_MODULE_ID=peakflowSPMIB, spLicenseErrLimit=spLicenseErrLimit, dnsBaseline=dnsBaseline, licenseErrorDone=licenseErrorDone, serviceUsageDone=serviceUsageDone, alertScript=alertScript, spInterfaceUsage=spInterfaceUsage, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spScriptHost=spScriptHost, spCommFailureDestination=spCommFailureDestination, spFingerprintName=spFingerprintName, fingerprintReceive=fingerprintReceive, deviceSystemError=deviceSystemError, spAlertDetectionSignatures=spAlertDetectionSignatures, spDNSObservedMean=spDNSObservedMean, spHijackLocal=spHijackLocal, spPravail=spPravail, snmpUp=snmpUp, spFingerprintFeedback=spFingerprintFeedback, devicePhysicalMemory=devicePhysicalMemory, spInterfaceIndex=spInterfaceIndex, spTMSPrefix=spTMSPrefix, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, bgpUp=bgpUp, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spMitigationStart=spMitigationStart, deviceDiskUsage=deviceDiskUsage, flowUp=flowUp, hardwareFailureDone=hardwareFailureDone, spDNSExpected=spDNSExpected, spReportURL=spReportURL, dnsBaselineDone=dnsBaselineDone, spInterfaceSpeed=spInterfaceSpeed, spImpactBps=spImpactBps, dosHostDetectionAlertDone=dosHostDetectionAlertDone, snmpDown=snmpDown, fingerprintUsage=fingerprintUsage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSObservedMax=spDNSObservedMax, spDNSObserved=spDNSObserved, reportDone=reportDone, mitigationBlackholeStart=mitigationBlackholeStart, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spReportID=spReportID, spBGPEvent=spBGPEvent, spManagedObject=spManagedObject, spTMSCommunity=spTMSCommunity, collectorStart=collectorStart, peakflowSPCMI=peakflowSPCMI, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spGreTunnelDestination=spGreTunnelDestination, spInetAddressType=spInetAddressType, spBGPTrapName=spBGPTrapName, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spServiceName=spServiceName, spLicenseErrCount=spLicenseErrCount, peakflowSPMgr=peakflowSPMgr, spScriptCommand=spScriptCommand, interfaceUsageDone=interfaceUsageDone, mitigationFlowspecStart=mitigationFlowspecStart, spMitigationID=spMitigationID, spScriptStart=spScriptStart, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spUsername=spUsername, configChange=configChange, spReportName=spReportName, flowDown=flowDown, cloudSignalingFault=cloudSignalingFault, spMitigationName=spMitigationName, spVersion=spVersion, hardwareFailure=hardwareFailure, spUsage=spUsage, collectorDown=collectorDown, peakflowSPMIB=peakflowSPMIB, spSystemErrorType=spSystemErrorType, spRouter=spRouter, spApplicationName=spApplicationName, spBGPInstability=spBGPInstability, notificationLimit=notificationLimit, managedObjectUsage=managedObjectUsage, spcommFailureDone=spcommFailureDone, greDownDone=greDownDone, spSystemErrorDescription=spSystemErrorDescription, peakflowSPTraps=peakflowSPTraps, bgpHijack=bgpHijack, licenseError=licenseError, spHijackAttr=spHijackAttr, spcommFailure=spcommFailure, spAlertID=spAlertID, spHardwareFailureDescription=spHardwareFailureDescription, fingerprintUsageDone=fingerprintUsageDone, spManagedObjects=spManagedObjects, deviceSwapSpace=deviceSwapSpace, bgpDown=bgpDown, mitigationThirdPartyStart=mitigationThirdPartyStart, mitigationTMSStartV2=mitigationTMSStartV2, spUnit=spUnit, bgpTrap=bgpTrap, routingFailover=routingFailover, mitigationTMSStart=mitigationTMSStart, deviceTotalFlows=deviceTotalFlows, deviceSystemErrorDone=deviceSystemErrorDone, spImpactPps=spImpactPps)
