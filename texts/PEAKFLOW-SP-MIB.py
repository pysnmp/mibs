...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spAlertID=spAlertID, spMitigationName=spMitigationName, spFingerprintSender=spFingerprintSender, spGreTunnelDestination=spGreTunnelDestination, spBlackholePrefix=spBlackholePrefix, spDNSObserved=spDNSObserved, spApplicationName=spApplicationName, spSystemErrorDescription=spSystemErrorDescription, dnsBaselineDone=dnsBaselineDone, spDetector=spDetector, spTMSCommunity=spTMSCommunity, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spManagedObject=spManagedObject, cloudSignalingFault=cloudSignalingFault, mitigationTMSStartV2=mitigationTMSStartV2, spPravail=spPravail, mitigationThirdPartyStart=mitigationThirdPartyStart, spUnit=spUnit, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterfaceSpeedHC=spInterfaceSpeedHC, spTMSMultiPrefix=spTMSMultiPrefix, spHijackAttr=spHijackAttr, peakflowSPMIB=peakflowSPMIB, spScriptCommand=spScriptCommand, spCommFailureDescription=spCommFailureDescription, spBGPTrapName=spBGPTrapName, trafficAutoMitigation=trafficAutoMitigation, spTMSPrefix=spTMSPrefix, spDNSName=spDNSName, flowDown=flowDown, spFingerprintFeedback=spFingerprintFeedback, deviceSystemError=deviceSystemError, dosHostDetectionAlert=dosHostDetectionAlert, peakflowSPMgr=peakflowSPMgr, devicePhysicalMemory=devicePhysicalMemory, deviceSwapSpace=deviceSwapSpace, spHardwareFailureDescription=spHardwareFailureDescription, deviceSystemErrorDone=deviceSystemErrorDone, hardwareFailure=hardwareFailure, spFlowspecTimeout=spFlowspecTimeout, spImpactBps=spImpactBps, spServiceName=spServiceName, reportDone=reportDone, bgpHijack=bgpHijack, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, routingFailover=routingFailover, fingerprintUsage=fingerprintUsage, spcommFailure=spcommFailure, spLicenseErrType=spLicenseErrType, bgpInstability=bgpInstability, spInetAddressType=spInetAddressType, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spInterfaceUsage=spInterfaceUsage, spFlowspecCommunity=spFlowspecCommunity, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spUsername=spUsername, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, notificationLimit=notificationLimit, spReportID=spReportID, managedObjectUsage=managedObjectUsage, serviceUsageDone=serviceUsageDone, spSystemErrorType=spSystemErrorType, spBlackholeCommunity=spBlackholeCommunity, spInterface=spInterface, spInetAddress=spInetAddress, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spManagedObjects=spManagedObjects, snmpDown=snmpDown, spBlackholeNexthop=spBlackholeNexthop, licenseErrorDone=licenseErrorDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spThreshold=spThreshold, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spUsageType=spUsageType, spGreTunnelName=spGreTunnelName, spBlackholeTimeout=spBlackholeTimeout, deviceHealth=deviceHealth, bgpDown=bgpDown, spDNSObservedMean=spDNSObservedMean, fingerprintReceive=fingerprintReceive, spMitigationStart=spMitigationStart, spFingerprintMessage=spFingerprintMessage, spThirdPartyAddr=spThirdPartyAddr, spScriptStart=spScriptStart, spAlertDetectionSignatures=spAlertDetectionSignatures, licenseError=licenseError, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, hardwareFailureDone=hardwareFailureDone, deviceTotalFlows=deviceTotalFlows, spVersion=spVersion, interfaceUsage=interfaceUsage, mitigationTMSStart=mitigationTMSStart, spBGPTrapNewAttributes=spBGPTrapNewAttributes, serviceUsage=serviceUsage, spDetectedCountries=spDetectedCountries, mitigationBlackholeStart=mitigationBlackholeStart, spThirdPartyZone=spThirdPartyZone, deviceDiskUsage=deviceDiskUsage, spInterfaceSpeed=spInterfaceSpeed, mitigationFlowspecStart=mitigationFlowspecStart, interfaceUsageDone=interfaceUsageDone, spScriptHost=spScriptHost, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDuration=spDuration, collectorDown=collectorDown, autoclassifyStart=autoclassifyStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, collectorUp=collectorUp, bgpInstabilityDone=bgpInstabilityDone, spInterfaceUsageHC=spInterfaceUsageHC, spCollector=spCollector, spLicenseErrCount=spLicenseErrCount, fingerprintUsageDone=fingerprintUsageDone, spRouter=spRouter, spBGPTrapPrefix=spBGPTrapPrefix, flowUp=flowUp, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spServiceElement=spServiceElement, spDNSExpected=spDNSExpected, mitigationDone=mitigationDone, spInterfaceIndex=spInterfaceIndex, greDown=greDown, deviceSwapSpaceInUse=deviceSwapSpaceInUse, managedObjectUsageDone=managedObjectUsageDone, configChange=configChange, bgpUp=bgpUp, greDownDone=greDownDone, PYSNMP_MODULE_ID=peakflowSPMIB, peakflowSPCMI=peakflowSPCMI, fingerprintFeedback=fingerprintFeedback, spHijackRoute=spHijackRoute, spImpactPps=spImpactPps, deviceTotalFlowsHC=deviceTotalFlowsHC, snmpUp=snmpUp, spReportURL=spReportURL, spManagedObjectFamily=spManagedObjectFamily, spMitigationID=spMitigationID, spFingerprintName=spFingerprintName, spLicenseErrLimit=spLicenseErrLimit, bgpTrap=bgpTrap, spReportName=spReportName, dnsBaseline=dnsBaseline, spcommFailureDone=spcommFailureDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spLicenseErrDescription=spLicenseErrDescription, spDNSObservedMax=spDNSObservedMax, spBGPTrapEvent=spBGPTrapEvent, collectorStart=collectorStart, alertScript=alertScript, spTMSTimeout=spTMSTimeout, peakflowSPTraps=peakflowSPTraps, spScriptPort=spScriptPort, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spBGPInstability=spBGPInstability, spHijackLocal=spHijackLocal, spBGPEvent=spBGPEvent, spUsage=spUsage, spCommFailureDestination=spCommFailureDestination)
