...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceTotalFlows=deviceTotalFlows, peakflowSPCMI=peakflowSPCMI, spBGPInstability=spBGPInstability, spBGPTrapEvent=spBGPTrapEvent, spBGPEvent=spBGPEvent, spBGPTrapNewAttributes=spBGPTrapNewAttributes, interfaceUsageDone=interfaceUsageDone, mitigationFlowspecStart=mitigationFlowspecStart, spInterfaceUsageHC=spInterfaceUsageHC, spInetAddress=spInetAddress, deviceDiskUsage=deviceDiskUsage, notificationLimit=notificationLimit, spTMSTimeout=spTMSTimeout, bgpInstability=bgpInstability, mitigationThirdPartyStart=mitigationThirdPartyStart, spTMSPrefix=spTMSPrefix, serviceUsageDone=serviceUsageDone, spMitigationID=spMitigationID, PYSNMP_MODULE_ID=peakflowSPMIB, fingerprintUsage=fingerprintUsage, spBlackholeTimeout=spBlackholeTimeout, spFingerprintName=spFingerprintName, spAlertID=spAlertID, licenseError=licenseError, spHijackAttr=spHijackAttr, mitigationDone=mitigationDone, peakflowSPMgr=peakflowSPMgr, flowDown=flowDown, bgpUp=bgpUp, collectorStart=collectorStart, licenseErrorDone=licenseErrorDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spImpactBps=spImpactBps, greDown=greDown, dnsBaseline=dnsBaseline, managedObjectUsageDone=managedObjectUsageDone, spLicenseErrLimit=spLicenseErrLimit, spReportID=spReportID, spBlackholeNexthop=spBlackholeNexthop, spFingerprintMessage=spFingerprintMessage, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spDNSObserved=spDNSObserved, cloudSignalingFault=cloudSignalingFault, spTMSCommunity=spTMSCommunity, spRouter=spRouter, spMitigationName=spMitigationName, devicePhysicalMemory=devicePhysicalMemory, spServiceElement=spServiceElement, spImpactPps=spImpactPps, spHijackRoute=spHijackRoute, deviceSystemError=deviceSystemError, spFingerprintFeedback=spFingerprintFeedback, spVersion=spVersion, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spHijackLocal=spHijackLocal, spInterfaceSpeedHC=spInterfaceSpeedHC, serviceUsage=serviceUsage, spCommFailureDescription=spCommFailureDescription, spLicenseErrType=spLicenseErrType, mitigationTMSStart=mitigationTMSStart, deviceSystemErrorDone=deviceSystemErrorDone, bgpDown=bgpDown, mitigationTMSStartV2=mitigationTMSStartV2, hardwareFailureDone=hardwareFailureDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFlowspecTimeout=spFlowspecTimeout, spReportURL=spReportURL, deviceSwapSpace=deviceSwapSpace, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSName=spDNSName, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spScriptStart=spScriptStart, spDuration=spDuration, spBGPTrapName=spBGPTrapName, spManagedObject=spManagedObject, spAlertDetectionSignatures=spAlertDetectionSignatures, cloudSignalingFaultDone=cloudSignalingFaultDone, spDNSObservedMax=spDNSObservedMax, fingerprintFeedback=fingerprintFeedback, spUsageType=spUsageType, spPravail=spPravail, trafficAutoMitigation=trafficAutoMitigation, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInterfaceIndex=spInterfaceIndex, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, flowUp=flowUp, spDNSObservedMean=spDNSObservedMean, peakflowSPTraps=peakflowSPTraps, interfaceUsage=interfaceUsage, spLicenseErrCount=spLicenseErrCount, spCollector=spCollector, spGreTunnelName=spGreTunnelName, bgpInstabilityDone=bgpInstabilityDone, spReportName=spReportName, fingerprintReceive=fingerprintReceive, hardwareFailure=hardwareFailure, spApplicationName=spApplicationName, spcommFailure=spcommFailure, spTMSMultiPrefix=spTMSMultiPrefix, spScriptCommand=spScriptCommand, spScriptHost=spScriptHost, spThirdPartyZone=spThirdPartyZone, spThreshold=spThreshold, spScriptPort=spScriptPort, spInetAddressType=spInetAddressType, autoclassifyStart=autoclassifyStart, managedObjectUsage=managedObjectUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spUnit=spUnit, spCommFailureDestination=spCommFailureDestination, snmpUp=snmpUp, configChange=configChange, mitigationBlackholeStart=mitigationBlackholeStart, spGreTunnelDestination=spGreTunnelDestination, bgpTrap=bgpTrap, dnsBaselineDone=dnsBaselineDone, spDetector=spDetector, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spFlowspecCommunity=spFlowspecCommunity, spcommFailureDone=spcommFailureDone, deviceHealth=deviceHealth, deviceSwapSpaceInUse=deviceSwapSpaceInUse, greDownDone=greDownDone, deviceTotalFlowsHC=deviceTotalFlowsHC, spInterface=spInterface, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spManagedObjectFamily=spManagedObjectFamily, spSystemErrorDescription=spSystemErrorDescription, spInterfaceUsage=spInterfaceUsage, collectorDown=collectorDown, spManagedObjects=spManagedObjects, spThirdPartyAddr=spThirdPartyAddr, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBGPTrapPrefix=spBGPTrapPrefix, spMitigationStart=spMitigationStart, fingerprintUsageDone=fingerprintUsageDone, snmpDown=snmpDown, routingFailover=routingFailover, collectorUp=collectorUp, reportDone=reportDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spInterfaceSpeed=spInterfaceSpeed, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spLicenseErrDescription=spLicenseErrDescription, spSystemErrorType=spSystemErrorType, spServiceName=spServiceName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosHostDetectionAlert=dosHostDetectionAlert, spFingerprintSender=spFingerprintSender, spBlackholeCommunity=spBlackholeCommunity, spDNSExpected=spDNSExpected, spUsername=spUsername, spDetectedCountries=spDetectedCountries, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, bgpHijack=bgpHijack, alertScript=alertScript, spBlackholePrefix=spBlackholePrefix, peakflowSPMIB=peakflowSPMIB, spHardwareFailureDescription=spHardwareFailureDescription, spUsage=spUsage)
