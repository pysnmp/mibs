...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceSpeedHC=spInterfaceSpeedHC, spServiceElement=spServiceElement, collectorStart=collectorStart, mitigationFlowspecStart=mitigationFlowspecStart, spSystemErrorDescription=spSystemErrorDescription, spTMSMultiPrefix=spTMSMultiPrefix, spDNSObservedMax=spDNSObservedMax, spReportID=spReportID, spDuration=spDuration, spTMSPrefix=spTMSPrefix, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBGPInstability=spBGPInstability, spBGPEvent=spBGPEvent, bgpDown=bgpDown, spTMSCommunity=spTMSCommunity, spFlowspecCommunity=spFlowspecCommunity, spBGPTrapPrefix=spBGPTrapPrefix, spInterfaceIndex=spInterfaceIndex, spGreTunnelDestination=spGreTunnelDestination, deviceSystemError=deviceSystemError, spUnit=spUnit, dosHostDetectionAlert=dosHostDetectionAlert, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, managedObjectUsage=managedObjectUsage, spDNSName=spDNSName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceTotalFlows=deviceTotalFlows, spMitigationID=spMitigationID, spScriptCommand=spScriptCommand, deviceTotalFlowsHC=deviceTotalFlowsHC, spManagedObjectFamily=spManagedObjectFamily, spTMSTimeout=spTMSTimeout, spInetAddress=spInetAddress, greDownDone=greDownDone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBGPTrapName=spBGPTrapName, bgpUp=bgpUp, greDown=greDown, spcommFailure=spcommFailure, spThirdPartyZone=spThirdPartyZone, spCollector=spCollector, peakflowSPMIB=peakflowSPMIB, fingerprintUsage=fingerprintUsage, spHijackLocal=spHijackLocal, bgpTrap=bgpTrap, spThirdPartyAddr=spThirdPartyAddr, spSystemErrorType=spSystemErrorType, spBlackholePrefix=spBlackholePrefix, bgpHijack=bgpHijack, managedObjectUsageDone=managedObjectUsageDone, deviceSwapSpace=deviceSwapSpace, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spReportURL=spReportURL, hardwareFailureDone=hardwareFailureDone, deviceHealth=deviceHealth, spDNSExpected=spDNSExpected, spMitigationName=spMitigationName, spScriptHost=spScriptHost, spInterfaceUsageHC=spInterfaceUsageHC, spBGPTrapOldAttributes=spBGPTrapOldAttributes, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spMitigationStart=spMitigationStart, spBGPTrapEvent=spBGPTrapEvent, mitigationTMSStart=mitigationTMSStart, cloudSignalingFaultDone=cloudSignalingFaultDone, spFlowspecTimeout=spFlowspecTimeout, dnsBaseline=dnsBaseline, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spThreshold=spThreshold, fingerprintReceive=fingerprintReceive, dnsBaselineDone=dnsBaselineDone, spUsername=spUsername, bgpInstability=bgpInstability, spInetAddressType=spInetAddressType, spApplicationName=spApplicationName, spScriptPort=spScriptPort, spBlackholeNexthop=spBlackholeNexthop, interfaceUsage=interfaceUsage, spcommFailureDone=spcommFailureDone, spInterfaceUsage=spInterfaceUsage, spReportName=spReportName, autoclassifyStart=autoclassifyStart, deviceDiskUsage=deviceDiskUsage, deviceSwapSpaceInUse=deviceSwapSpaceInUse, collectorUp=collectorUp, spAlertDetectionSignatures=spAlertDetectionSignatures, spImpactBps=spImpactBps, hardwareFailure=hardwareFailure, spCommFailureDestination=spCommFailureDestination, spManagedObjects=spManagedObjects, notificationLimit=notificationLimit, spLicenseErrCount=spLicenseErrCount, mitigationBlackholeStart=mitigationBlackholeStart, spServiceName=spServiceName, reportDone=reportDone, spRouter=spRouter, spPravail=spPravail, snmpDown=snmpDown, spLicenseErrLimit=spLicenseErrLimit, flowUp=flowUp, deviceSystemErrorDone=deviceSystemErrorDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spGreTunnelName=spGreTunnelName, spInterface=spInterface, spDNSObservedMean=spDNSObservedMean, collectorDown=collectorDown, spVersion=spVersion, spImpactPps=spImpactPps, spScriptStart=spScriptStart, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spDetector=spDetector, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, cloudSignalingFault=cloudSignalingFault, spFingerprintFeedback=spFingerprintFeedback, interfaceUsageDone=interfaceUsageDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spUsageType=spUsageType, spCommFailureDescription=spCommFailureDescription, licenseError=licenseError, spLicenseErrType=spLicenseErrType, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spInterfaceSpeed=spInterfaceSpeed, trafficAutoMitigation=trafficAutoMitigation, spAlertID=spAlertID, alertScript=alertScript, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBlackholeCommunity=spBlackholeCommunity, spDetectedCountries=spDetectedCountries, dosNetworkProfiledAlert=dosNetworkProfiledAlert, flowDown=flowDown, mitigationTMSStartV2=mitigationTMSStartV2, spHijackAttr=spHijackAttr, spFingerprintMessage=spFingerprintMessage, spBlackholeTimeout=spBlackholeTimeout, spLicenseErrDescription=spLicenseErrDescription, fingerprintFeedback=fingerprintFeedback, routingFailover=routingFailover, spUsage=spUsage, configChange=configChange, licenseErrorDone=licenseErrorDone, spFingerprintName=spFingerprintName, mitigationThirdPartyStart=mitigationThirdPartyStart, peakflowSPMgr=peakflowSPMgr, deviceSwapSpaceUsage=deviceSwapSpaceUsage, fingerprintUsageDone=fingerprintUsageDone, PYSNMP_MODULE_ID=peakflowSPMIB, devicePhysicalMemory=devicePhysicalMemory, mitigationDone=mitigationDone, bgpInstabilityDone=bgpInstabilityDone, spManagedObject=spManagedObject, peakflowSPTraps=peakflowSPTraps, snmpUp=snmpUp, spFingerprintSender=spFingerprintSender, serviceUsageDone=serviceUsageDone, spHardwareFailureDescription=spHardwareFailureDescription, spHijackRoute=spHijackRoute, serviceUsage=serviceUsage, peakflowSPCMI=peakflowSPCMI, spDNSObserved=spDNSObserved)
