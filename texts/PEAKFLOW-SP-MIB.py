...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", peakflowSPMgr=peakflowSPMgr, spHijackRoute=spHijackRoute, spTMSPrefix=spTMSPrefix, spBGPInstability=spBGPInstability, spImpactPps=spImpactPps, spGreTunnelDestination=spGreTunnelDestination, deviceTotalFlows=deviceTotalFlows, bgpTrap=bgpTrap, routingFailover=routingFailover, spBGPTrapName=spBGPTrapName, flowUp=flowUp, spBGPTrapEvent=spBGPTrapEvent, spDNSObserved=spDNSObserved, managedObjectUsageDone=managedObjectUsageDone, spAlertID=spAlertID, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDetector=spDetector, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spCommFailureDescription=spCommFailureDescription, spManagedObjectFamily=spManagedObjectFamily, spUsageType=spUsageType, licenseError=licenseError, bgpInstability=bgpInstability, spServiceName=spServiceName, cloudSignalingFault=cloudSignalingFault, spScriptStart=spScriptStart, spManagedObject=spManagedObject, spLicenseErrType=spLicenseErrType, mitigationTMSStartV2=mitigationTMSStartV2, spReportID=spReportID, spUnit=spUnit, spBlackholePrefix=spBlackholePrefix, spBGPTrapNewAttributes=spBGPTrapNewAttributes, licenseErrorDone=licenseErrorDone, spVersion=spVersion, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spManagedObjects=spManagedObjects, greDown=greDown, fingerprintUsage=fingerprintUsage, spScriptCommand=spScriptCommand, spTMSTimeout=spTMSTimeout, spHijackAttr=spHijackAttr, mitigationTMSStart=mitigationTMSStart, spInterfaceIndex=spInterfaceIndex, spTMSMultiPrefix=spTMSMultiPrefix, interfaceUsage=interfaceUsage, spThirdPartyZone=spThirdPartyZone, collectorUp=collectorUp, snmpUp=snmpUp, spDNSObservedMean=spDNSObservedMean, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, alertScript=alertScript, spDNSExpected=spDNSExpected, serviceUsageDone=serviceUsageDone, spScriptHost=spScriptHost, autoclassifyStart=autoclassifyStart, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, collectorDown=collectorDown, managedObjectUsage=managedObjectUsage, spDNSName=spDNSName, cloudSignalingFaultDone=cloudSignalingFaultDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, peakflowSPTraps=peakflowSPTraps, spSystemErrorDescription=spSystemErrorDescription, spInetAddressType=spInetAddressType, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spGreTunnelName=spGreTunnelName, spBlackholeCommunity=spBlackholeCommunity, bgpHijack=bgpHijack, fingerprintFeedback=fingerprintFeedback, spScriptPort=spScriptPort, spAlertDetectionSignatures=spAlertDetectionSignatures, spInterface=spInterface, fingerprintReceive=fingerprintReceive, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBGPTrapPrefix=spBGPTrapPrefix, hardwareFailureDone=hardwareFailureDone, dnsBaseline=dnsBaseline, bgpDown=bgpDown, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spUsage=spUsage, spFlowspecTimeout=spFlowspecTimeout, spFingerprintSender=spFingerprintSender, notificationLimit=notificationLimit, spFingerprintMessage=spFingerprintMessage, spCommFailureDestination=spCommFailureDestination, spHijackLocal=spHijackLocal, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spThreshold=spThreshold, deviceSwapSpace=deviceSwapSpace, greDownDone=greDownDone, dosHostDetectionAlert=dosHostDetectionAlert, spReportName=spReportName, spInterfaceSpeed=spInterfaceSpeed, spCollector=spCollector, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceHealth=deviceHealth, bgpUp=bgpUp, deviceTotalFlowsHC=deviceTotalFlowsHC, spFlowspecCommunity=spFlowspecCommunity, spMitigationName=spMitigationName, dnsBaselineDone=dnsBaselineDone, spSystemErrorType=spSystemErrorType, bgpInstabilityDone=bgpInstabilityDone, spDNSObservedMax=spDNSObservedMax, spLicenseErrLimit=spLicenseErrLimit, spInterfaceUsageHC=spInterfaceUsageHC, mitigationThirdPartyStart=mitigationThirdPartyStart, spMitigationStart=spMitigationStart, deviceSwapSpaceInUse=deviceSwapSpaceInUse, peakflowSPMIB=peakflowSPMIB, hardwareFailure=hardwareFailure, spcommFailureDone=spcommFailureDone, mitigationBlackholeStart=mitigationBlackholeStart, spBlackholeNexthop=spBlackholeNexthop, mitigationFlowspecStart=mitigationFlowspecStart, configChange=configChange, flowDown=flowDown, deviceSystemError=deviceSystemError, spTMSCommunity=spTMSCommunity, spInterfaceUsage=spInterfaceUsage, spInterfaceSpeedHC=spInterfaceSpeedHC, spMitigationID=spMitigationID, spDetectedCountries=spDetectedCountries, spDuration=spDuration, PYSNMP_MODULE_ID=peakflowSPMIB, spLicenseErrCount=spLicenseErrCount, snmpDown=snmpDown, interfaceUsageDone=interfaceUsageDone, spBGPEvent=spBGPEvent, serviceUsage=serviceUsage, spLicenseErrDescription=spLicenseErrDescription, spRouter=spRouter, spHardwareFailureDescription=spHardwareFailureDescription, spImpactBps=spImpactBps, reportDone=reportDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, fingerprintUsageDone=fingerprintUsageDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spUsername=spUsername, spReportURL=spReportURL, spPravail=spPravail, spInetAddress=spInetAddress, collectorStart=collectorStart, spServiceElement=spServiceElement, spcommFailure=spcommFailure, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, mitigationDone=mitigationDone, spFingerprintName=spFingerprintName, spFingerprintFeedback=spFingerprintFeedback, deviceDiskUsage=deviceDiskUsage, peakflowSPCMI=peakflowSPCMI, spThirdPartyAddr=spThirdPartyAddr, spApplicationName=spApplicationName, deviceSystemErrorDone=deviceSystemErrorDone, trafficAutoMitigation=trafficAutoMitigation, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, devicePhysicalMemory=devicePhysicalMemory, spBlackholeTimeout=spBlackholeTimeout)
