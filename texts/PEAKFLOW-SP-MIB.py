...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spFlowspecCommunity=spFlowspecCommunity, spUsageType=spUsageType, spScriptStart=spScriptStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spFlowspecTimeout=spFlowspecTimeout, spFingerprintSender=spFingerprintSender, bgpInstabilityDone=bgpInstabilityDone, reportDone=reportDone, spBGPTrapEvent=spBGPTrapEvent, peakflowSPMgr=peakflowSPMgr, spLicenseErrType=spLicenseErrType, trafficAutoMitigation=trafficAutoMitigation, serviceUsageDone=serviceUsageDone, notificationLimit=notificationLimit, spSystemErrorDescription=spSystemErrorDescription, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceSystemError=deviceSystemError, spImpactBps=spImpactBps, deviceSwapSpace=deviceSwapSpace, mitigationTMSStartV2=mitigationTMSStartV2, spLicenseErrCount=spLicenseErrCount, peakflowSPMIB=peakflowSPMIB, spInterfaceUsage=spInterfaceUsage, spMitigationID=spMitigationID, cloudSignalingFault=cloudSignalingFault, spManagedObject=spManagedObject, spTMSPrefix=spTMSPrefix, fingerprintFeedback=fingerprintFeedback, peakflowSPCMI=peakflowSPCMI, spServiceName=spServiceName, spFingerprintFeedback=spFingerprintFeedback, spUsage=spUsage, collectorStart=collectorStart, licenseError=licenseError, bgpUp=bgpUp, routingFailover=routingFailover, spReportURL=spReportURL, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spCommFailureDescription=spCommFailureDescription, spScriptPort=spScriptPort, mitigationTMSStart=mitigationTMSStart, spInterfaceIndex=spInterfaceIndex, spUsername=spUsername, deviceHealth=deviceHealth, spMitigationStart=spMitigationStart, spImpactPps=spImpactPps, spApplicationName=spApplicationName, spVersion=spVersion, deviceSwapSpaceInUse=deviceSwapSpaceInUse, flowUp=flowUp, deviceSwapSpaceUsage=deviceSwapSpaceUsage, greDown=greDown, flowDown=flowDown, spDetectedCountries=spDetectedCountries, spPravail=spPravail, spTMSMultiPrefix=spTMSMultiPrefix, spSystemErrorType=spSystemErrorType, spHijackLocal=spHijackLocal, spBGPInstability=spBGPInstability, collectorUp=collectorUp, spScriptCommand=spScriptCommand, spcommFailure=spcommFailure, mitigationBlackholeStart=mitigationBlackholeStart, fingerprintReceive=fingerprintReceive, spHijackAttr=spHijackAttr, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceTotalFlows=deviceTotalFlows, autoclassifyStart=autoclassifyStart, spHardwareFailureDescription=spHardwareFailureDescription, spBlackholePrefix=spBlackholePrefix, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spFingerprintMessage=spFingerprintMessage, spBlackholeNexthop=spBlackholeNexthop, spScriptHost=spScriptHost, spInetAddressType=spInetAddressType, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, dosNetworkProfiledAlert=dosNetworkProfiledAlert, interfaceUsageDone=interfaceUsageDone, configChange=configChange, mitigationDone=mitigationDone, spTMSCommunity=spTMSCommunity, bgpTrap=bgpTrap, spManagedObjectFamily=spManagedObjectFamily, spInterfaceUsageHC=spInterfaceUsageHC, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, fingerprintUsage=fingerprintUsage, snmpDown=snmpDown, dosHostDetectionAlertDone=dosHostDetectionAlertDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spCollector=spCollector, spRouter=spRouter, spServiceElement=spServiceElement, spInterfaceSpeed=spInterfaceSpeed, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spReportID=spReportID, PYSNMP_MODULE_ID=peakflowSPMIB, dnsBaseline=dnsBaseline, spDNSExpected=spDNSExpected, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spThreshold=spThreshold, cloudSignalingFaultDone=cloudSignalingFaultDone, alertScript=alertScript, mitigationFlowspecStart=mitigationFlowspecStart, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spBlackholeCommunity=spBlackholeCommunity, bgpHijack=bgpHijack, spAlertDetectionSignatures=spAlertDetectionSignatures, spGreTunnelName=spGreTunnelName, spReportName=spReportName, spBGPTrapName=spBGPTrapName, spBlackholeTimeout=spBlackholeTimeout, spcommFailureDone=spcommFailureDone, spDuration=spDuration, interfaceUsage=interfaceUsage, deviceSystemErrorDone=deviceSystemErrorDone, spDNSObservedMean=spDNSObservedMean, spDNSObservedMax=spDNSObservedMax, dosHostDetectionAlert=dosHostDetectionAlert, spBGPEvent=spBGPEvent, spMitigationName=spMitigationName, spLicenseErrLimit=spLicenseErrLimit, dnsBaselineDone=dnsBaselineDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, peakflowSPTraps=peakflowSPTraps, licenseErrorDone=licenseErrorDone, spBGPTrapPrefix=spBGPTrapPrefix, spInterface=spInterface, spManagedObjects=spManagedObjects, spDNSObserved=spDNSObserved, spHijackRoute=spHijackRoute, bgpDown=bgpDown, spAlertID=spAlertID, deviceTotalFlowsHC=deviceTotalFlowsHC, spUnit=spUnit, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spDNSName=spDNSName, spFingerprintName=spFingerprintName, spGreTunnelDestination=spGreTunnelDestination, spCloudSignalFaultDescription=spCloudSignalFaultDescription, bgpInstability=bgpInstability, devicePhysicalMemory=devicePhysicalMemory, serviceUsage=serviceUsage, spTMSTimeout=spTMSTimeout, hardwareFailure=hardwareFailure, spThirdPartyZone=spThirdPartyZone, spDetector=spDetector, hardwareFailureDone=hardwareFailureDone, managedObjectUsage=managedObjectUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spLicenseErrDescription=spLicenseErrDescription, spCommFailureDestination=spCommFailureDestination, collectorDown=collectorDown, spInetAddress=spInetAddress, spThirdPartyAddr=spThirdPartyAddr, greDownDone=greDownDone, deviceDiskUsage=deviceDiskUsage, fingerprintUsageDone=fingerprintUsageDone, managedObjectUsageDone=managedObjectUsageDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, snmpUp=snmpUp)
