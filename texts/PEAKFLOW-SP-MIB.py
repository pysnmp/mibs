...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", mitigationBlackholeStart=mitigationBlackholeStart, spImpactBps=spImpactBps, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, bgpInstability=bgpInstability, collectorDown=collectorDown, spUsername=spUsername, spPravail=spPravail, deviceDiskUsage=deviceDiskUsage, spDNSObservedMax=spDNSObservedMax, deviceTotalFlows=deviceTotalFlows, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spApplicationName=spApplicationName, deviceSystemErrorDone=deviceSystemErrorDone, bgpTrap=bgpTrap, spTMSMultiPrefix=spTMSMultiPrefix, spBGPTrapPrefix=spBGPTrapPrefix, spBGPEvent=spBGPEvent, peakflowSPMgr=peakflowSPMgr, spThirdPartyZone=spThirdPartyZone, spInterfaceSpeedHC=spInterfaceSpeedHC, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spTMSCommunity=spTMSCommunity, deviceHealth=deviceHealth, mitigationFlowspecStart=mitigationFlowspecStart, spBGPInstability=spBGPInstability, spBlackholeTimeout=spBlackholeTimeout, spCollector=spCollector, dosHostDetectionAlert=dosHostDetectionAlert, dnsBaseline=dnsBaseline, spScriptHost=spScriptHost, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInterfaceSpeed=spInterfaceSpeed, spAlertID=spAlertID, spVersion=spVersion, spMitigationName=spMitigationName, devicePhysicalMemory=devicePhysicalMemory, spCloudSignalFaultDescription=spCloudSignalFaultDescription, fingerprintUsageDone=fingerprintUsageDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, peakflowSPMIB=peakflowSPMIB, spRouter=spRouter, spBGPTrapName=spBGPTrapName, interfaceUsage=interfaceUsage, autoclassifyStart=autoclassifyStart, reportDone=reportDone, alertScript=alertScript, hardwareFailure=hardwareFailure, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spTMSTimeout=spTMSTimeout, spSystemErrorDescription=spSystemErrorDescription, licenseError=licenseError, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceSwapSpace=deviceSwapSpace, mitigationDone=mitigationDone, spBGPTrapEvent=spBGPTrapEvent, spBlackholeNexthop=spBlackholeNexthop, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spUnit=spUnit, spScriptPort=spScriptPort, deviceSwapSpaceUsage=deviceSwapSpaceUsage, serviceUsageDone=serviceUsageDone, serviceUsage=serviceUsage, spServiceName=spServiceName, spFlowspecTimeout=spFlowspecTimeout, spDetector=spDetector, fingerprintReceive=fingerprintReceive, fingerprintUsage=fingerprintUsage, greDown=greDown, fingerprintFeedback=fingerprintFeedback, dnsBaselineDone=dnsBaselineDone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spGreTunnelDestination=spGreTunnelDestination, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, deviceSystemError=deviceSystemError, deviceSwapSpaceInUse=deviceSwapSpaceInUse, collectorStart=collectorStart, spSystemErrorType=spSystemErrorType, managedObjectUsageDone=managedObjectUsageDone, spFingerprintName=spFingerprintName, interfaceUsageDone=interfaceUsageDone, managedObjectUsage=managedObjectUsage, mitigationTMSStart=mitigationTMSStart, bgpHijack=bgpHijack, hardwareFailureDone=hardwareFailureDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spReportURL=spReportURL, licenseErrorDone=licenseErrorDone, spDNSExpected=spDNSExpected, spScriptCommand=spScriptCommand, spLicenseErrType=spLicenseErrType, spScriptStart=spScriptStart, spBlackholePrefix=spBlackholePrefix, spLicenseErrLimit=spLicenseErrLimit, flowUp=flowUp, spInterfaceIndex=spInterfaceIndex, peakflowSPTraps=peakflowSPTraps, spDNSObservedMean=spDNSObservedMean, spUsage=spUsage, spGreTunnelName=spGreTunnelName, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spDetectedCountries=spDetectedCountries, spDNSName=spDNSName, spTMSPrefix=spTMSPrefix, collectorUp=collectorUp, spManagedObject=spManagedObject, spMitigationStart=spMitigationStart, spcommFailureDone=spcommFailureDone, spLicenseErrDescription=spLicenseErrDescription, spInetAddress=spInetAddress, snmpUp=snmpUp, PYSNMP_MODULE_ID=peakflowSPMIB, bgpInstabilityDone=bgpInstabilityDone, configChange=configChange, trafficAutoMitigation=trafficAutoMitigation, snmpDown=snmpDown, spFingerprintFeedback=spFingerprintFeedback, routingFailover=routingFailover, bgpUp=bgpUp, spUsageType=spUsageType, spHijackLocal=spHijackLocal, spServiceElement=spServiceElement, flowDown=flowDown, spReportName=spReportName, spInterfaceUsage=spInterfaceUsage, mitigationThirdPartyStart=mitigationThirdPartyStart, greDownDone=greDownDone, spInterface=spInterface, cloudSignalingFault=cloudSignalingFault, spHijackRoute=spHijackRoute, peakflowSPCMI=peakflowSPCMI, spHijackAttr=spHijackAttr, spBGPTrapOldAttributes=spBGPTrapOldAttributes, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spImpactPps=spImpactPps, spLicenseErrCount=spLicenseErrCount, spReportID=spReportID, spManagedObjectFamily=spManagedObjectFamily, spDNSObserved=spDNSObserved, spHardwareFailureDescription=spHardwareFailureDescription, spThreshold=spThreshold, spThirdPartyAddr=spThirdPartyAddr, deviceTotalFlowsHC=deviceTotalFlowsHC, spManagedObjects=spManagedObjects, bgpDown=bgpDown, notificationLimit=notificationLimit, spcommFailure=spcommFailure, spDuration=spDuration, spCommFailureDestination=spCommFailureDestination, spBlackholeCommunity=spBlackholeCommunity, spInterfaceUsageHC=spInterfaceUsageHC, spFlowspecCommunity=spFlowspecCommunity, spMitigationID=spMitigationID, spCommFailureDescription=spCommFailureDescription, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spFingerprintSender=spFingerprintSender, mitigationTMSStartV2=mitigationTMSStartV2, spInetAddressType=spInetAddressType, spFingerprintMessage=spFingerprintMessage, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min)
