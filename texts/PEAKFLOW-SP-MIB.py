...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceUsage=spInterfaceUsage, spLicenseErrLimit=spLicenseErrLimit, spServiceName=spServiceName, spFingerprintName=spFingerprintName, spScriptPort=spScriptPort, spInterfaceUsageHC=spInterfaceUsageHC, bgpUp=bgpUp, deviceSwapSpace=deviceSwapSpace, spDNSObservedMax=spDNSObservedMax, deviceDiskUsage=deviceDiskUsage, spServiceElement=spServiceElement, cloudSignalingFaultDone=cloudSignalingFaultDone, flowUp=flowUp, spCommFailureDestination=spCommFailureDestination, spTMSPrefix=spTMSPrefix, greDown=greDown, mitigationBlackholeStart=mitigationBlackholeStart, deviceSystemError=deviceSystemError, spVersion=spVersion, spFingerprintSender=spFingerprintSender, spFlowspecTimeout=spFlowspecTimeout, spScriptStart=spScriptStart, spImpactPps=spImpactPps, collectorUp=collectorUp, cloudSignalingFault=cloudSignalingFault, spDNSExpected=spDNSExpected, PYSNMP_MODULE_ID=peakflowSPMIB, licenseError=licenseError, bgpHijack=bgpHijack, spDNSObserved=spDNSObserved, dnsBaselineDone=dnsBaselineDone, spApplicationName=spApplicationName, interfaceUsageDone=interfaceUsageDone, spAlertDetectionSignatures=spAlertDetectionSignatures, interfaceUsage=interfaceUsage, spDNSName=spDNSName, spGreTunnelName=spGreTunnelName, spcommFailureDone=spcommFailureDone, spSystemErrorType=spSystemErrorType, spUsage=spUsage, managedObjectUsage=managedObjectUsage, spThirdPartyAddr=spThirdPartyAddr, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spFingerprintMessage=spFingerprintMessage, spFlowspecCommunity=spFlowspecCommunity, spTMSMultiPrefix=spTMSMultiPrefix, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spManagedObjectFamily=spManagedObjectFamily, fingerprintUsage=fingerprintUsage, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, deviceHealth=deviceHealth, spBGPTrapPrefix=spBGPTrapPrefix, bgpInstability=bgpInstability, spHijackRoute=spHijackRoute, spGreTunnelDestination=spGreTunnelDestination, mitigationDone=mitigationDone, spHijackAttr=spHijackAttr, spFingerprintFeedback=spFingerprintFeedback, trafficAutoMitigation=trafficAutoMitigation, spRouter=spRouter, serviceUsageDone=serviceUsageDone, collectorStart=collectorStart, snmpUp=snmpUp, snmpDown=snmpDown, spLicenseErrDescription=spLicenseErrDescription, spInterfaceIndex=spInterfaceIndex, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dnsBaseline=dnsBaseline, spDetector=spDetector, alertScript=alertScript, spReportName=spReportName, peakflowSPMgr=peakflowSPMgr, spMitigationName=spMitigationName, peakflowSPTraps=peakflowSPTraps, spBlackholeTimeout=spBlackholeTimeout, spImpactBps=spImpactBps, mitigationTMSStart=mitigationTMSStart, spInterfaceSpeedHC=spInterfaceSpeedHC, configChange=configChange, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spcommFailure=spcommFailure, spScriptCommand=spScriptCommand, licenseErrorDone=licenseErrorDone, managedObjectUsageDone=managedObjectUsageDone, fingerprintReceive=fingerprintReceive, spMitigationID=spMitigationID, spInetAddress=spInetAddress, bgpTrap=bgpTrap, spBGPInstability=spBGPInstability, spMitigationStart=spMitigationStart, fingerprintFeedback=fingerprintFeedback, spManagedObject=spManagedObject, fingerprintUsageDone=fingerprintUsageDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spBGPTrapName=spBGPTrapName, spReportURL=spReportURL, spDetectedCountries=spDetectedCountries, mitigationThirdPartyStart=mitigationThirdPartyStart, spHijackLocal=spHijackLocal, deviceTotalFlowsHC=deviceTotalFlowsHC, dosHostDetectionAlert=dosHostDetectionAlert, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spBlackholePrefix=spBlackholePrefix, spPravail=spPravail, spBGPTrapEvent=spBGPTrapEvent, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spThreshold=spThreshold, deviceTotalFlows=deviceTotalFlows, dosNetworkProfiledAlert=dosNetworkProfiledAlert, devicePhysicalMemory=devicePhysicalMemory, hardwareFailureDone=hardwareFailureDone, spThirdPartyZone=spThirdPartyZone, routingFailover=routingFailover, spInetAddressType=spInetAddressType, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spBlackholeCommunity=spBlackholeCommunity, spSystemErrorDescription=spSystemErrorDescription, peakflowSPCMI=peakflowSPCMI, notificationLimit=notificationLimit, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spManagedObjects=spManagedObjects, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, collectorDown=collectorDown, greDownDone=greDownDone, flowDown=flowDown, spUsageType=spUsageType, spBGPEvent=spBGPEvent, serviceUsage=serviceUsage, hardwareFailure=hardwareFailure, spInterfaceSpeed=spInterfaceSpeed, spTMSCommunity=spTMSCommunity, deviceSystemErrorDone=deviceSystemErrorDone, peakflowSPMIB=peakflowSPMIB, bgpInstabilityDone=bgpInstabilityDone, spReportID=spReportID, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spUsername=spUsername, spDNSObservedMean=spDNSObservedMean, spBlackholeNexthop=spBlackholeNexthop, reportDone=reportDone, bgpDown=bgpDown, mitigationFlowspecStart=mitigationFlowspecStart, spLicenseErrCount=spLicenseErrCount, spCollector=spCollector, spDuration=spDuration, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spScriptHost=spScriptHost, spUnit=spUnit, spLicenseErrType=spLicenseErrType, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spAlertID=spAlertID, spTMSTimeout=spTMSTimeout, spCommFailureDescription=spCommFailureDescription, autoclassifyStart=autoclassifyStart, mitigationTMSStartV2=mitigationTMSStartV2, spInterface=spInterface, spHardwareFailureDescription=spHardwareFailureDescription)
