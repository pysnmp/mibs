...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spSystemErrorType=spSystemErrorType, spUnit=spUnit, licenseError=licenseError, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spUsage=spUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spThreshold=spThreshold, routingFailover=routingFailover, mitigationBlackholeStart=mitigationBlackholeStart, spTMSTimeout=spTMSTimeout, spInterfaceUsageHC=spInterfaceUsageHC, spHijackLocal=spHijackLocal, snmpUp=snmpUp, spInterfaceIndex=spInterfaceIndex, hardwareFailureDone=hardwareFailureDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spUsername=spUsername, peakflowSPMgr=peakflowSPMgr, fingerprintFeedback=fingerprintFeedback, spBlackholeNexthop=spBlackholeNexthop, spDNSExpected=spDNSExpected, spVersion=spVersion, spThirdPartyZone=spThirdPartyZone, deviceSwapSpace=deviceSwapSpace, spcommFailureDone=spcommFailureDone, deviceSystemError=deviceSystemError, spInetAddressType=spInetAddressType, spMitigationStart=spMitigationStart, dosHostDetectionAlert=dosHostDetectionAlert, spImpactBps=spImpactBps, spPravail=spPravail, spInterfaceSpeedHC=spInterfaceSpeedHC, mitigationThirdPartyStart=mitigationThirdPartyStart, spServiceElement=spServiceElement, mitigationTMSStartV2=mitigationTMSStartV2, hardwareFailure=hardwareFailure, spLicenseErrCount=spLicenseErrCount, bgpDown=bgpDown, spCollector=spCollector, spReportURL=spReportURL, cloudSignalingFaultDone=cloudSignalingFaultDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, mitigationTMSStart=mitigationTMSStart, configChange=configChange, interfaceUsageDone=interfaceUsageDone, spTMSCommunity=spTMSCommunity, cloudSignalingFault=cloudSignalingFault, spBGPTrapEvent=spBGPTrapEvent, spBGPTrapPrefix=spBGPTrapPrefix, spDNSObservedMean=spDNSObservedMean, spBGPTrapName=spBGPTrapName, spBlackholeCommunity=spBlackholeCommunity, serviceUsage=serviceUsage, peakflowSPMIB=peakflowSPMIB, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spApplicationName=spApplicationName, spDNSName=spDNSName, flowDown=flowDown, managedObjectUsageDone=managedObjectUsageDone, licenseErrorDone=licenseErrorDone, spDuration=spDuration, spGreTunnelName=spGreTunnelName, spDNSObserved=spDNSObserved, collectorUp=collectorUp, reportDone=reportDone, mitigationFlowspecStart=mitigationFlowspecStart, flowUp=flowUp, collectorDown=collectorDown, spScriptPort=spScriptPort, deviceHealth=deviceHealth, spScriptStart=spScriptStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, bgpUp=bgpUp, spHijackRoute=spHijackRoute, spServiceName=spServiceName, spCommFailureDescription=spCommFailureDescription, mitigationDone=mitigationDone, spTMSMultiPrefix=spTMSMultiPrefix, fingerprintReceive=fingerprintReceive, spScriptHost=spScriptHost, spBlackholeTimeout=spBlackholeTimeout, spInterfaceSpeed=spInterfaceSpeed, spLicenseErrLimit=spLicenseErrLimit, deviceTotalFlows=deviceTotalFlows, spFingerprintMessage=spFingerprintMessage, spFlowspecCommunity=spFlowspecCommunity, peakflowSPCMI=peakflowSPCMI, spHardwareFailureDescription=spHardwareFailureDescription, deviceDiskUsage=deviceDiskUsage, peakflowSPTraps=peakflowSPTraps, spTMSPrefix=spTMSPrefix, spInterface=spInterface, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFingerprintName=spFingerprintName, spReportName=spReportName, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spBlackholePrefix=spBlackholePrefix, fingerprintUsage=fingerprintUsage, spFingerprintSender=spFingerprintSender, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spDetectedCountries=spDetectedCountries, spDetector=spDetector, serviceUsageDone=serviceUsageDone, spDNSObservedMax=spDNSObservedMax, spInterfaceUsage=spInterfaceUsage, dnsBaselineDone=dnsBaselineDone, spManagedObject=spManagedObject, dnsBaseline=dnsBaseline, notificationLimit=notificationLimit, spSystemErrorDescription=spSystemErrorDescription, PYSNMP_MODULE_ID=peakflowSPMIB, bgpHijack=bgpHijack, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spThirdPartyAddr=spThirdPartyAddr, spHijackAttr=spHijackAttr, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, snmpDown=snmpDown, spFlowspecTimeout=spFlowspecTimeout, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spFingerprintFeedback=spFingerprintFeedback, trafficAutoMitigation=trafficAutoMitigation, spReportID=spReportID, autoclassifyStart=autoclassifyStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spMitigationID=spMitigationID, spRouter=spRouter, managedObjectUsage=managedObjectUsage, spImpactPps=spImpactPps, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spManagedObjectFamily=spManagedObjectFamily, fingerprintUsageDone=fingerprintUsageDone, spInetAddress=spInetAddress, collectorStart=collectorStart, devicePhysicalMemory=devicePhysicalMemory, spGreTunnelDestination=spGreTunnelDestination, alertScript=alertScript, spScriptCommand=spScriptCommand, spCommFailureDestination=spCommFailureDestination, spMitigationName=spMitigationName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spBGPInstability=spBGPInstability, spBGPEvent=spBGPEvent, deviceTotalFlowsHC=deviceTotalFlowsHC, spUsageType=spUsageType, bgpTrap=bgpTrap, spAlertID=spAlertID, bgpInstability=bgpInstability, bgpInstabilityDone=bgpInstabilityDone, greDownDone=greDownDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spManagedObjects=spManagedObjects, spcommFailure=spcommFailure, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, deviceSystemErrorDone=deviceSystemErrorDone, spLicenseErrType=spLicenseErrType, interfaceUsage=interfaceUsage, greDown=greDown)
