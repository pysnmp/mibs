...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, collectorUp=collectorUp, spManagedObject=spManagedObject, spReportURL=spReportURL, managedObjectUsage=managedObjectUsage, spBGPTrapName=spBGPTrapName, spAlertDetectionSignatures=spAlertDetectionSignatures, spMitigationID=spMitigationID, spDNSObserved=spDNSObserved, spInterfaceUsageHC=spInterfaceUsageHC, snmpUp=snmpUp, deviceDiskUsage=deviceDiskUsage, fingerprintReceive=fingerprintReceive, spImpactBps=spImpactBps, spScriptPort=spScriptPort, deviceHealth=deviceHealth, cloudSignalingFault=cloudSignalingFault, collectorStart=collectorStart, peakflowSPCMI=peakflowSPCMI, spBGPTrapNewAttributes=spBGPTrapNewAttributes, collectorDown=collectorDown, mitigationBlackholeStart=mitigationBlackholeStart, bgpTrap=bgpTrap, spInterfaceUsage=spInterfaceUsage, spUsername=spUsername, spMitigationStart=spMitigationStart, spFingerprintFeedback=spFingerprintFeedback, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spCollector=spCollector, spBGPInstability=spBGPInstability, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spRouter=spRouter, spUnit=spUnit, spDNSExpected=spDNSExpected, spDNSObservedMean=spDNSObservedMean, reportDone=reportDone, hardwareFailure=hardwareFailure, spCommFailureDestination=spCommFailureDestination, spDNSName=spDNSName, flowDown=flowDown, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spThirdPartyAddr=spThirdPartyAddr, snmpDown=snmpDown, dosHostDetectionAlert=dosHostDetectionAlert, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, devicePhysicalMemory=devicePhysicalMemory, spLicenseErrLimit=spLicenseErrLimit, interfaceUsageDone=interfaceUsageDone, spTMSCommunity=spTMSCommunity, spInterfaceSpeedHC=spInterfaceSpeedHC, spBlackholePrefix=spBlackholePrefix, spFingerprintMessage=spFingerprintMessage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, alertScript=alertScript, deviceSwapSpaceUsage=deviceSwapSpaceUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spLicenseErrType=spLicenseErrType, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBGPEvent=spBGPEvent, licenseErrorDone=licenseErrorDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spBlackholeCommunity=spBlackholeCommunity, fingerprintUsage=fingerprintUsage, spDuration=spDuration, spInterfaceIndex=spInterfaceIndex, notificationLimit=notificationLimit, routingFailover=routingFailover, fingerprintFeedback=fingerprintFeedback, spGreTunnelName=spGreTunnelName, spImpactPps=spImpactPps, spcommFailure=spcommFailure, peakflowSPMIB=peakflowSPMIB, spBlackholeNexthop=spBlackholeNexthop, spServiceName=spServiceName, peakflowSPTraps=peakflowSPTraps, spServiceElement=spServiceElement, spBGPTrapEvent=spBGPTrapEvent, autoclassifyStart=autoclassifyStart, spHardwareFailureDescription=spHardwareFailureDescription, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spTMSPrefix=spTMSPrefix, spFlowspecTimeout=spFlowspecTimeout, spCommFailureDescription=spCommFailureDescription, spTMSMultiPrefix=spTMSMultiPrefix, greDownDone=greDownDone, bgpInstabilityDone=bgpInstabilityDone, flowUp=flowUp, deviceSwapSpace=deviceSwapSpace, configChange=configChange, spcommFailureDone=spcommFailureDone, spHijackRoute=spHijackRoute, spScriptCommand=spScriptCommand, spScriptHost=spScriptHost, spAlertID=spAlertID, spUsage=spUsage, deviceTotalFlows=deviceTotalFlows, greDown=greDown, serviceUsageDone=serviceUsageDone, spThirdPartyZone=spThirdPartyZone, spMitigationName=spMitigationName, mitigationDone=mitigationDone, spReportID=spReportID, mitigationTMSStartV2=mitigationTMSStartV2, mitigationTMSStart=mitigationTMSStart, spUsageType=spUsageType, serviceUsage=serviceUsage, bgpInstability=bgpInstability, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, mitigationThirdPartyStart=mitigationThirdPartyStart, spInetAddress=spInetAddress, spBGPTrapPrefix=spBGPTrapPrefix, deviceSystemError=deviceSystemError, spPravail=spPravail, spDetectedCountries=spDetectedCountries, fingerprintUsageDone=fingerprintUsageDone, deviceTotalFlowsHC=deviceTotalFlowsHC, spReportName=spReportName, interfaceUsage=interfaceUsage, spDNSObservedMax=spDNSObservedMax, spHijackAttr=spHijackAttr, deviceSystemErrorDone=deviceSystemErrorDone, spGreTunnelDestination=spGreTunnelDestination, spLicenseErrCount=spLicenseErrCount, spManagedObjectFamily=spManagedObjectFamily, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spInterface=spInterface, bgpHijack=bgpHijack, dnsBaselineDone=dnsBaselineDone, spFingerprintName=spFingerprintName, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spApplicationName=spApplicationName, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInetAddressType=spInetAddressType, spScriptStart=spScriptStart, spVersion=spVersion, spSystemErrorType=spSystemErrorType, deviceSwapSpaceInUse=deviceSwapSpaceInUse, bgpUp=bgpUp, spFlowspecCommunity=spFlowspecCommunity, spDetector=spDetector, spManagedObjects=spManagedObjects, managedObjectUsageDone=managedObjectUsageDone, spBlackholeTimeout=spBlackholeTimeout, PYSNMP_MODULE_ID=peakflowSPMIB, spInterfaceSpeed=spInterfaceSpeed, spTMSTimeout=spTMSTimeout, bgpDown=bgpDown, mitigationFlowspecStart=mitigationFlowspecStart, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spHijackLocal=spHijackLocal, trafficAutoMitigation=trafficAutoMitigation, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, licenseError=licenseError, spFingerprintSender=spFingerprintSender, spSystemErrorDescription=spSystemErrorDescription, dnsBaseline=dnsBaseline, spLicenseErrDescription=spLicenseErrDescription, spThreshold=spThreshold, peakflowSPMgr=peakflowSPMgr, hardwareFailureDone=hardwareFailureDone)
