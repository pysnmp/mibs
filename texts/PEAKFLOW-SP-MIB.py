...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceDiskUsage=deviceDiskUsage, trafficAutoMitigation=trafficAutoMitigation, spReportURL=spReportURL, spCommFailureDestination=spCommFailureDestination, spManagedObject=spManagedObject, flowDown=flowDown, spLicenseErrCount=spLicenseErrCount, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, fingerprintUsageDone=fingerprintUsageDone, spUnit=spUnit, snmpDown=snmpDown, spScriptCommand=spScriptCommand, deviceSwapSpaceInUse=deviceSwapSpaceInUse, peakflowSPCMI=peakflowSPCMI, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spMitigationStart=spMitigationStart, spCommFailureDescription=spCommFailureDescription, licenseError=licenseError, spUsage=spUsage, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spDetectedCountries=spDetectedCountries, spGreTunnelName=spGreTunnelName, spDNSObservedMean=spDNSObservedMean, fingerprintUsage=fingerprintUsage, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spServiceName=spServiceName, mitigationTMSStartV2=mitigationTMSStartV2, spDNSObserved=spDNSObserved, spBGPEvent=spBGPEvent, configChange=configChange, dosHostDetectionAlert=dosHostDetectionAlert, spVersion=spVersion, dnsBaselineDone=dnsBaselineDone, peakflowSPMgr=peakflowSPMgr, bgpDown=bgpDown, reportDone=reportDone, spInterfaceSpeedHC=spInterfaceSpeedHC, spLicenseErrLimit=spLicenseErrLimit, spHijackRoute=spHijackRoute, spBGPTrapEvent=spBGPTrapEvent, spInetAddress=spInetAddress, spImpactPps=spImpactPps, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spSystemErrorDescription=spSystemErrorDescription, alertScript=alertScript, spLicenseErrDescription=spLicenseErrDescription, spInterfaceUsage=spInterfaceUsage, spDNSName=spDNSName, bgpHijack=bgpHijack, fingerprintReceive=fingerprintReceive, spScriptPort=spScriptPort, devicePhysicalMemory=devicePhysicalMemory, spTMSMultiPrefix=spTMSMultiPrefix, notificationLimit=notificationLimit, spScriptStart=spScriptStart, spMitigationName=spMitigationName, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceTotalFlowsHC=deviceTotalFlowsHC, spRouter=spRouter, deviceTotalFlows=deviceTotalFlows, spBGPTrapPrefix=spBGPTrapPrefix, spDNSExpected=spDNSExpected, deviceSwapSpaceUsage=deviceSwapSpaceUsage, autoclassifyStart=autoclassifyStart, spDuration=spDuration, spCollector=spCollector, interfaceUsage=interfaceUsage, spBlackholeTimeout=spBlackholeTimeout, spGreTunnelDestination=spGreTunnelDestination, spInetAddressType=spInetAddressType, mitigationDone=mitigationDone, spInterface=spInterface, greDown=greDown, collectorStart=collectorStart, spManagedObjects=spManagedObjects, spDetector=spDetector, spReportID=spReportID, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceHealth=deviceHealth, spHardwareFailureDescription=spHardwareFailureDescription, spInterfaceSpeed=spInterfaceSpeed, spAlertDetectionSignatures=spAlertDetectionSignatures, managedObjectUsage=managedObjectUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spThirdPartyAddr=spThirdPartyAddr, cloudSignalingFault=cloudSignalingFault, greDownDone=greDownDone, spBGPTrapName=spBGPTrapName, mitigationBlackholeStart=mitigationBlackholeStart, spUsername=spUsername, serviceUsage=serviceUsage, spcommFailure=spcommFailure, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, snmpUp=snmpUp, spFingerprintName=spFingerprintName, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spFingerprintMessage=spFingerprintMessage, deviceSystemError=deviceSystemError, spScriptHost=spScriptHost, deviceSwapSpace=deviceSwapSpace, collectorUp=collectorUp, deviceSystemErrorDone=deviceSystemErrorDone, spDNSObservedMax=spDNSObservedMax, spSystemErrorType=spSystemErrorType, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, peakflowSPMIB=peakflowSPMIB, interfaceUsageDone=interfaceUsageDone, managedObjectUsageDone=managedObjectUsageDone, spThreshold=spThreshold, spBlackholePrefix=spBlackholePrefix, flowUp=flowUp, hardwareFailure=hardwareFailure, fingerprintFeedback=fingerprintFeedback, collectorDown=collectorDown, mitigationFlowspecStart=mitigationFlowspecStart, spServiceElement=spServiceElement, peakflowSPTraps=peakflowSPTraps, spFlowspecTimeout=spFlowspecTimeout, spTMSCommunity=spTMSCommunity, spPravail=spPravail, spManagedObjectFamily=spManagedObjectFamily, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spLicenseErrType=spLicenseErrType, routingFailover=routingFailover, spHijackLocal=spHijackLocal, spThirdPartyZone=spThirdPartyZone, dnsBaseline=dnsBaseline, spApplicationName=spApplicationName, bgpInstability=bgpInstability, spBlackholeCommunity=spBlackholeCommunity, spMitigationID=spMitigationID, hardwareFailureDone=hardwareFailureDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spImpactBps=spImpactBps, spBlackholeNexthop=spBlackholeNexthop, spFingerprintSender=spFingerprintSender, spFlowspecCommunity=spFlowspecCommunity, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spTMSTimeout=spTMSTimeout, spTMSPrefix=spTMSPrefix, bgpTrap=bgpTrap, spBGPInstability=spBGPInstability, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, cloudSignalingFaultDone=cloudSignalingFaultDone, spInterfaceIndex=spInterfaceIndex, serviceUsageDone=serviceUsageDone, spUsageType=spUsageType, licenseErrorDone=licenseErrorDone, PYSNMP_MODULE_ID=peakflowSPMIB, spAlertID=spAlertID, bgpUp=bgpUp, spInterfaceUsageHC=spInterfaceUsageHC, spHijackAttr=spHijackAttr, spReportName=spReportName, bgpInstabilityDone=bgpInstabilityDone, spcommFailureDone=spcommFailureDone, mitigationTMSStart=mitigationTMSStart, spFingerprintFeedback=spFingerprintFeedback, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min)
