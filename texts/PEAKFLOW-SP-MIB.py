...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spMitigationStart=spMitigationStart, trafficAutoMitigation=trafficAutoMitigation, spGreTunnelDestination=spGreTunnelDestination, interfaceUsage=interfaceUsage, spTMSCommunity=spTMSCommunity, reportDone=reportDone, fingerprintUsageDone=fingerprintUsageDone, spBGPTrapPrefix=spBGPTrapPrefix, spReportName=spReportName, spCommFailureDestination=spCommFailureDestination, spCommFailureDescription=spCommFailureDescription, spUnit=spUnit, spBGPTrapName=spBGPTrapName, mitigationTMSStart=mitigationTMSStart, spImpactBps=spImpactBps, spInterfaceSpeed=spInterfaceSpeed, spInterface=spInterface, spCollector=spCollector, greDown=greDown, managedObjectUsage=managedObjectUsage, spTMSPrefix=spTMSPrefix, mitigationFlowspecStart=mitigationFlowspecStart, spThirdPartyAddr=spThirdPartyAddr, dnsBaselineDone=dnsBaselineDone, dosHostDetectionAlert=dosHostDetectionAlert, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spMitigationName=spMitigationName, deviceHealth=deviceHealth, spDuration=spDuration, spAlertID=spAlertID, deviceTotalFlowsHC=deviceTotalFlowsHC, spDNSObserved=spDNSObserved, spCloudSignalFaultDescription=spCloudSignalFaultDescription, bgpDown=bgpDown, licenseError=licenseError, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, peakflowSPCMI=peakflowSPCMI, spFlowspecTimeout=spFlowspecTimeout, spHardwareFailureDescription=spHardwareFailureDescription, spVersion=spVersion, spServiceName=spServiceName, fingerprintReceive=fingerprintReceive, snmpDown=snmpDown, hardwareFailureDone=hardwareFailureDone, spReportID=spReportID, spDetector=spDetector, spHijackRoute=spHijackRoute, interfaceUsageDone=interfaceUsageDone, spcommFailure=spcommFailure, spInterfaceUsageHC=spInterfaceUsageHC, greDownDone=greDownDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spScriptPort=spScriptPort, routingFailover=routingFailover, spInetAddressType=spInetAddressType, managedObjectUsageDone=managedObjectUsageDone, flowUp=flowUp, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, bgpTrap=bgpTrap, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spLicenseErrDescription=spLicenseErrDescription, spImpactPps=spImpactPps, spUsageType=spUsageType, bgpInstability=bgpInstability, snmpUp=snmpUp, spSystemErrorDescription=spSystemErrorDescription, spAlertDetectionSignatures=spAlertDetectionSignatures, mitigationTMSStartV2=mitigationTMSStartV2, spGreTunnelName=spGreTunnelName, spBGPInstability=spBGPInstability, spReportURL=spReportURL, mitigationDone=mitigationDone, cloudSignalingFaultDone=cloudSignalingFaultDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spHijackLocal=spHijackLocal, spFingerprintName=spFingerprintName, spBlackholePrefix=spBlackholePrefix, spApplicationName=spApplicationName, deviceSwapSpace=deviceSwapSpace, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spSystemErrorType=spSystemErrorType, spBlackholeCommunity=spBlackholeCommunity, spLicenseErrCount=spLicenseErrCount, spTMSTimeout=spTMSTimeout, spDNSName=spDNSName, spDetectedCountries=spDetectedCountries, spInetAddress=spInetAddress, spFingerprintMessage=spFingerprintMessage, dosNetworkProfiledAlert=dosNetworkProfiledAlert, peakflowSPTraps=peakflowSPTraps, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, peakflowSPMgr=peakflowSPMgr, dnsBaseline=dnsBaseline, collectorUp=collectorUp, spUsage=spUsage, spDNSObservedMean=spDNSObservedMean, spUsername=spUsername, spLicenseErrType=spLicenseErrType, spServiceElement=spServiceElement, collectorDown=collectorDown, spInterfaceIndex=spInterfaceIndex, deviceSystemErrorDone=deviceSystemErrorDone, licenseErrorDone=licenseErrorDone, deviceDiskUsage=deviceDiskUsage, spFingerprintFeedback=spFingerprintFeedback, spInterfaceUsage=spInterfaceUsage, spBGPTrapEvent=spBGPTrapEvent, configChange=configChange, serviceUsageDone=serviceUsageDone, spPravail=spPravail, spThirdPartyZone=spThirdPartyZone, notificationLimit=notificationLimit, spHijackAttr=spHijackAttr, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spFingerprintSender=spFingerprintSender, spBGPEvent=spBGPEvent, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, mitigationThirdPartyStart=mitigationThirdPartyStart, spTMSMultiPrefix=spTMSMultiPrefix, bgpHijack=bgpHijack, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSExpected=spDNSExpected, autoclassifyStart=autoclassifyStart, fingerprintFeedback=fingerprintFeedback, spScriptStart=spScriptStart, alertScript=alertScript, spManagedObjectFamily=spManagedObjectFamily, spcommFailureDone=spcommFailureDone, flowDown=flowDown, deviceSwapSpaceInUse=deviceSwapSpaceInUse, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, bgpInstabilityDone=bgpInstabilityDone, fingerprintUsage=fingerprintUsage, spThreshold=spThreshold, serviceUsage=serviceUsage, spManagedObjects=spManagedObjects, hardwareFailure=hardwareFailure, spScriptHost=spScriptHost, spBlackholeNexthop=spBlackholeNexthop, bgpUp=bgpUp, spDNSObservedMax=spDNSObservedMax, spMitigationID=spMitigationID, PYSNMP_MODULE_ID=peakflowSPMIB, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spBlackholeTimeout=spBlackholeTimeout, spScriptCommand=spScriptCommand, spLicenseErrLimit=spLicenseErrLimit, deviceTotalFlows=deviceTotalFlows, spManagedObject=spManagedObject, spFlowspecCommunity=spFlowspecCommunity, collectorStart=collectorStart, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, devicePhysicalMemory=devicePhysicalMemory, peakflowSPMIB=peakflowSPMIB, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceSystemError=deviceSystemError, mitigationBlackholeStart=mitigationBlackholeStart, cloudSignalingFault=cloudSignalingFault, spRouter=spRouter)
