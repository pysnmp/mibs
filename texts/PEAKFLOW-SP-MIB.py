...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceSpeedHC=spInterfaceSpeedHC, spInterfaceIndex=spInterfaceIndex, spCommFailureDescription=spCommFailureDescription, spApplicationName=spApplicationName, spDetector=spDetector, dnsBaselineDone=dnsBaselineDone, spScriptHost=spScriptHost, bgpUp=bgpUp, flowDown=flowDown, spUsername=spUsername, dosHostDetectionAlertDone=dosHostDetectionAlertDone, licenseErrorDone=licenseErrorDone, snmpUp=snmpUp, fingerprintUsage=fingerprintUsage, spScriptPort=spScriptPort, greDown=greDown, spScriptCommand=spScriptCommand, spGreTunnelDestination=spGreTunnelDestination, configChange=configChange, spFingerprintSender=spFingerprintSender, collectorUp=collectorUp, spFlowspecCommunity=spFlowspecCommunity, bgpInstabilityDone=bgpInstabilityDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spScriptStart=spScriptStart, spTMSCommunity=spTMSCommunity, bgpTrap=bgpTrap, spBGPTrapPrefix=spBGPTrapPrefix, mitigationTMSStartV2=mitigationTMSStartV2, deviceSwapSpaceInUse=deviceSwapSpaceInUse, flowUp=flowUp, spThirdPartyZone=spThirdPartyZone, serviceUsageDone=serviceUsageDone, spLicenseErrDescription=spLicenseErrDescription, spAlertID=spAlertID, spCollector=spCollector, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, bgpInstability=bgpInstability, spTMSTimeout=spTMSTimeout, spBGPTrapName=spBGPTrapName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spcommFailureDone=spcommFailureDone, spThirdPartyAddr=spThirdPartyAddr, spHardwareFailureDescription=spHardwareFailureDescription, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spServiceName=spServiceName, deviceSystemError=deviceSystemError, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spHijackLocal=spHijackLocal, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spFingerprintName=spFingerprintName, spHijackAttr=spHijackAttr, mitigationDone=mitigationDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spLicenseErrType=spLicenseErrType, spInetAddress=spInetAddress, spReportID=spReportID, spDuration=spDuration, mitigationThirdPartyStart=mitigationThirdPartyStart, snmpDown=snmpDown, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spInterfaceUsage=spInterfaceUsage, spManagedObjectFamily=spManagedObjectFamily, cloudSignalingFault=cloudSignalingFault, spMitigationStart=spMitigationStart, spMitigationID=spMitigationID, routingFailover=routingFailover, spInetAddressType=spInetAddressType, bgpHijack=bgpHijack, spTMSPrefix=spTMSPrefix, notificationLimit=notificationLimit, trafficAutoMitigation=trafficAutoMitigation, spInterfaceUsageHC=spInterfaceUsageHC, spUsage=spUsage, spReportName=spReportName, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBGPInstability=spBGPInstability, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spGreTunnelName=spGreTunnelName, spCommFailureDestination=spCommFailureDestination, spBlackholeNexthop=spBlackholeNexthop, interfaceUsage=interfaceUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spSystemErrorType=spSystemErrorType, deviceTotalFlows=deviceTotalFlows, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, managedObjectUsageDone=managedObjectUsageDone, autoclassifyStart=autoclassifyStart, managedObjectUsage=managedObjectUsage, spReportURL=spReportURL, spImpactPps=spImpactPps, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spServiceElement=spServiceElement, spLicenseErrLimit=spLicenseErrLimit, spDNSObserved=spDNSObserved, mitigationTMSStart=mitigationTMSStart, spBlackholePrefix=spBlackholePrefix, spManagedObjects=spManagedObjects, peakflowSPTraps=peakflowSPTraps, spDetectedCountries=spDetectedCountries, spTMSMultiPrefix=spTMSMultiPrefix, fingerprintReceive=fingerprintReceive, dnsBaseline=dnsBaseline, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spSystemErrorDescription=spSystemErrorDescription, peakflowSPMIB=peakflowSPMIB, spMitigationName=spMitigationName, dosHostDetectionAlert=dosHostDetectionAlert, mitigationFlowspecStart=mitigationFlowspecStart, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInterface=spInterface, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, cloudSignalingFaultDone=cloudSignalingFaultDone, fingerprintUsageDone=fingerprintUsageDone, spVersion=spVersion, licenseError=licenseError, spAlertDetectionSignatures=spAlertDetectionSignatures, PYSNMP_MODULE_ID=peakflowSPMIB, mitigationBlackholeStart=mitigationBlackholeStart, peakflowSPCMI=peakflowSPCMI, deviceDiskUsage=deviceDiskUsage, interfaceUsageDone=interfaceUsageDone, spManagedObject=spManagedObject, spDNSObservedMean=spDNSObservedMean, hardwareFailureDone=hardwareFailureDone, spInterfaceSpeed=spInterfaceSpeed, spHijackRoute=spHijackRoute, alertScript=alertScript, peakflowSPMgr=peakflowSPMgr, collectorStart=collectorStart, spRouter=spRouter, spDNSName=spDNSName, spDNSObservedMax=spDNSObservedMax, serviceUsage=serviceUsage, spFlowspecTimeout=spFlowspecTimeout, spBlackholeTimeout=spBlackholeTimeout, spLicenseErrCount=spLicenseErrCount, deviceHealth=deviceHealth, collectorDown=collectorDown, spBlackholeCommunity=spBlackholeCommunity, bgpDown=bgpDown, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spFingerprintFeedback=spFingerprintFeedback, spImpactBps=spImpactBps, deviceSwapSpace=deviceSwapSpace, hardwareFailure=hardwareFailure, spThreshold=spThreshold, fingerprintFeedback=fingerprintFeedback, spcommFailure=spcommFailure, spDNSExpected=spDNSExpected, devicePhysicalMemory=devicePhysicalMemory, greDownDone=greDownDone, spUnit=spUnit, spFingerprintMessage=spFingerprintMessage, spPravail=spPravail, spUsageType=spUsageType, spBGPEvent=spBGPEvent, deviceSystemErrorDone=deviceSystemErrorDone, deviceTotalFlowsHC=deviceTotalFlowsHC, reportDone=reportDone, spBGPTrapEvent=spBGPTrapEvent)
