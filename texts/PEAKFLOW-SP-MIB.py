...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spRouter=spRouter, spLicenseErrType=spLicenseErrType, spReportID=spReportID, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, serviceUsage=serviceUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spCommFailureDescription=spCommFailureDescription, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spImpactBps=spImpactBps, managedObjectUsageDone=managedObjectUsageDone, spFingerprintSender=spFingerprintSender, snmpDown=snmpDown, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, fingerprintUsageDone=fingerprintUsageDone, deviceSystemError=deviceSystemError, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spAlertDetectionSignatures=spAlertDetectionSignatures, routingFailover=routingFailover, spThirdPartyZone=spThirdPartyZone, spCollector=spCollector, deviceHealth=deviceHealth, alertScript=alertScript, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spLicenseErrCount=spLicenseErrCount, spDNSObserved=spDNSObserved, spScriptHost=spScriptHost, spFlowspecTimeout=spFlowspecTimeout, mitigationFlowspecStart=mitigationFlowspecStart, dnsBaseline=dnsBaseline, PYSNMP_MODULE_ID=peakflowSPMIB, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceSystemErrorDone=deviceSystemErrorDone, cloudSignalingFault=cloudSignalingFault, spDNSExpected=spDNSExpected, mitigationThirdPartyStart=mitigationThirdPartyStart, spScriptPort=spScriptPort, spInetAddressType=spInetAddressType, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, snmpUp=snmpUp, spUsername=spUsername, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapPrefix=spBGPTrapPrefix, spServiceName=spServiceName, spInterfaceUsage=spInterfaceUsage, spTMSCommunity=spTMSCommunity, spUsage=spUsage, spBlackholeCommunity=spBlackholeCommunity, spReportURL=spReportURL, spSystemErrorDescription=spSystemErrorDescription, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, greDownDone=greDownDone, spInterfaceUsageHC=spInterfaceUsageHC, collectorStart=collectorStart, spGreTunnelName=spGreTunnelName, spManagedObjects=spManagedObjects, flowUp=flowUp, spcommFailure=spcommFailure, spInterfaceSpeed=spInterfaceSpeed, hardwareFailure=hardwareFailure, dosHostDetectionAlert=dosHostDetectionAlert, managedObjectUsage=managedObjectUsage, trafficAutoMitigation=trafficAutoMitigation, deviceDiskUsage=deviceDiskUsage, spImpactPps=spImpactPps, bgpUp=bgpUp, spAlertID=spAlertID, reportDone=reportDone, mitigationDone=mitigationDone, spMitigationStart=spMitigationStart, bgpDown=bgpDown, spDuration=spDuration, licenseErrorDone=licenseErrorDone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spCommFailureDestination=spCommFailureDestination, spApplicationName=spApplicationName, bgpInstability=bgpInstability, spFlowspecCommunity=spFlowspecCommunity, bgpTrap=bgpTrap, spTMSPrefix=spTMSPrefix, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, peakflowSPMgr=peakflowSPMgr, spScriptCommand=spScriptCommand, spBlackholeNexthop=spBlackholeNexthop, dnsBaselineDone=dnsBaselineDone, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spTMSTimeout=spTMSTimeout, spInterfaceSpeedHC=spInterfaceSpeedHC, serviceUsageDone=serviceUsageDone, spScriptStart=spScriptStart, spGreTunnelDestination=spGreTunnelDestination, spDNSName=spDNSName, spManagedObjectFamily=spManagedObjectFamily, spBGPInstability=spBGPInstability, spDetector=spDetector, flowDown=flowDown, mitigationTMSStartV2=mitigationTMSStartV2, spHijackLocal=spHijackLocal, spBlackholeTimeout=spBlackholeTimeout, spDNSObservedMax=spDNSObservedMax, spBGPEvent=spBGPEvent, spHijackAttr=spHijackAttr, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spReportName=spReportName, spDNSObservedMean=spDNSObservedMean, peakflowSPCMI=peakflowSPCMI, spUnit=spUnit, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spMitigationID=spMitigationID, deviceTotalFlows=deviceTotalFlows, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, collectorDown=collectorDown, spFingerprintMessage=spFingerprintMessage, hardwareFailureDone=hardwareFailureDone, peakflowSPTraps=peakflowSPTraps, fingerprintFeedback=fingerprintFeedback, spPravail=spPravail, deviceSwapSpace=deviceSwapSpace, interfaceUsageDone=interfaceUsageDone, spTMSMultiPrefix=spTMSMultiPrefix, spFingerprintFeedback=spFingerprintFeedback, licenseError=licenseError, spHijackRoute=spHijackRoute, bgpInstabilityDone=bgpInstabilityDone, spBGPTrapName=spBGPTrapName, spUsageType=spUsageType, spDetectedCountries=spDetectedCountries, collectorUp=collectorUp, spServiceElement=spServiceElement, cloudSignalingFaultDone=cloudSignalingFaultDone, devicePhysicalMemory=devicePhysicalMemory, mitigationTMSStart=mitigationTMSStart, fingerprintReceive=fingerprintReceive, spVersion=spVersion, spThreshold=spThreshold, deviceTotalFlowsHC=deviceTotalFlowsHC, greDown=greDown, fingerprintUsage=fingerprintUsage, spHardwareFailureDescription=spHardwareFailureDescription, spInterface=spInterface, interfaceUsage=interfaceUsage, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spBGPTrapEvent=spBGPTrapEvent, notificationLimit=notificationLimit, spFingerprintName=spFingerprintName, spLicenseErrLimit=spLicenseErrLimit, spManagedObject=spManagedObject, configChange=configChange, bgpHijack=bgpHijack, spMitigationName=spMitigationName, spInterfaceIndex=spInterfaceIndex, mitigationBlackholeStart=mitigationBlackholeStart, autoclassifyStart=autoclassifyStart, spSystemErrorType=spSystemErrorType, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInetAddress=spInetAddress, spBlackholePrefix=spBlackholePrefix, peakflowSPMIB=peakflowSPMIB, spcommFailureDone=spcommFailureDone, spThirdPartyAddr=spThirdPartyAddr)
