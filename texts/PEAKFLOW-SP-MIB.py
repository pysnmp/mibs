...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceUsageHC=spInterfaceUsageHC, spBGPTrapEvent=spBGPTrapEvent, spAlertDetectionSignatures=spAlertDetectionSignatures, serviceUsageDone=serviceUsageDone, spManagedObjectFamily=spManagedObjectFamily, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spFingerprintFeedback=spFingerprintFeedback, spDNSName=spDNSName, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spInterfaceUsage=spInterfaceUsage, spBlackholePrefix=spBlackholePrefix, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spcommFailure=spcommFailure, spPravail=spPravail, mitigationBlackholeStart=mitigationBlackholeStart, spCommFailureDestination=spCommFailureDestination, spcommFailureDone=spcommFailureDone, spRouter=spRouter, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, fingerprintFeedback=fingerprintFeedback, spReportID=spReportID, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spGreTunnelDestination=spGreTunnelDestination, dosHostDetectionAlert=dosHostDetectionAlert, spLicenseErrDescription=spLicenseErrDescription, cloudSignalingFault=cloudSignalingFault, autoclassifyStart=autoclassifyStart, spBGPTrapName=spBGPTrapName, spServiceName=spServiceName, dnsBaseline=dnsBaseline, deviceTotalFlowsHC=deviceTotalFlowsHC, spFingerprintName=spFingerprintName, spManagedObjects=spManagedObjects, spTMSMultiPrefix=spTMSMultiPrefix, spHijackAttr=spHijackAttr, routingFailover=routingFailover, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spDetectedCountries=spDetectedCountries, mitigationTMSStartV2=mitigationTMSStartV2, spAlertID=spAlertID, spBGPEvent=spBGPEvent, alertScript=alertScript, mitigationThirdPartyStart=mitigationThirdPartyStart, spMitigationStart=spMitigationStart, interfaceUsage=interfaceUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spScriptCommand=spScriptCommand, collectorStart=collectorStart, spApplicationName=spApplicationName, spFlowspecCommunity=spFlowspecCommunity, deviceSwapSpace=deviceSwapSpace, spImpactPps=spImpactPps, flowDown=flowDown, collectorDown=collectorDown, collectorUp=collectorUp, spLicenseErrCount=spLicenseErrCount, fingerprintUsageDone=fingerprintUsageDone, spReportURL=spReportURL, managedObjectUsageDone=managedObjectUsageDone, fingerprintUsage=fingerprintUsage, spDNSObservedMean=spDNSObservedMean, spTMSCommunity=spTMSCommunity, spHardwareFailureDescription=spHardwareFailureDescription, licenseError=licenseError, managedObjectUsage=managedObjectUsage, spGreTunnelName=spGreTunnelName, bgpHijack=bgpHijack, reportDone=reportDone, spImpactBps=spImpactBps, spInetAddress=spInetAddress, peakflowSPTraps=peakflowSPTraps, hardwareFailure=hardwareFailure, bgpInstabilityDone=bgpInstabilityDone, spSystemErrorType=spSystemErrorType, spManagedObject=spManagedObject, spLicenseErrLimit=spLicenseErrLimit, trafficAutoMitigation=trafficAutoMitigation, mitigationFlowspecStart=mitigationFlowspecStart, spScriptHost=spScriptHost, spVersion=spVersion, spBGPInstability=spBGPInstability, cloudSignalingFaultDone=cloudSignalingFaultDone, spFlowspecTimeout=spFlowspecTimeout, dnsBaselineDone=dnsBaselineDone, spBlackholeNexthop=spBlackholeNexthop, hardwareFailureDone=hardwareFailureDone, licenseErrorDone=licenseErrorDone, spUsageType=spUsageType, deviceHealth=deviceHealth, spFingerprintSender=spFingerprintSender, spBGPTrapOldAttributes=spBGPTrapOldAttributes, peakflowSPMgr=peakflowSPMgr, spTMSPrefix=spTMSPrefix, spDNSObserved=spDNSObserved, spBlackholeCommunity=spBlackholeCommunity, mitigationTMSStart=mitigationTMSStart, spScriptStart=spScriptStart, peakflowSPCMI=peakflowSPCMI, spMitigationName=spMitigationName, spLicenseErrType=spLicenseErrType, spThreshold=spThreshold, bgpInstability=bgpInstability, fingerprintReceive=fingerprintReceive, spHijackLocal=spHijackLocal, flowUp=flowUp, spUnit=spUnit, devicePhysicalMemory=devicePhysicalMemory, spHijackRoute=spHijackRoute, spInterfaceSpeed=spInterfaceSpeed, interfaceUsageDone=interfaceUsageDone, spDetector=spDetector, spUsername=spUsername, bgpUp=bgpUp, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, peakflowSPMIB=peakflowSPMIB, PYSNMP_MODULE_ID=peakflowSPMIB, spCollector=spCollector, spCloudSignalFaultDescription=spCloudSignalFaultDescription, snmpUp=snmpUp, mitigationDone=mitigationDone, deviceSystemErrorDone=deviceSystemErrorDone, spInterface=spInterface, spCommFailureDescription=spCommFailureDescription, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spSystemErrorDescription=spSystemErrorDescription, spTMSTimeout=spTMSTimeout, spMitigationID=spMitigationID, spBlackholeTimeout=spBlackholeTimeout, greDown=greDown, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spServiceElement=spServiceElement, spReportName=spReportName, spUsage=spUsage, spScriptPort=spScriptPort, spFingerprintMessage=spFingerprintMessage, spDNSObservedMax=spDNSObservedMax, spThirdPartyAddr=spThirdPartyAddr, spDuration=spDuration, bgpTrap=bgpTrap, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceDiskUsage=deviceDiskUsage, serviceUsage=serviceUsage, spBGPTrapPrefix=spBGPTrapPrefix, bgpDown=bgpDown, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, greDownDone=greDownDone, deviceSystemError=deviceSystemError, notificationLimit=notificationLimit, spInterfaceSpeedHC=spInterfaceSpeedHC, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spThirdPartyZone=spThirdPartyZone, configChange=configChange, spDNSExpected=spDNSExpected, spInetAddressType=spInetAddressType, deviceTotalFlows=deviceTotalFlows, dosNetworkProfiledAlert=dosNetworkProfiledAlert, snmpDown=snmpDown, spInterfaceIndex=spInterfaceIndex)
