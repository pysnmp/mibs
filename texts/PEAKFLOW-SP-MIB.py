...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spFlowspecCommunity=spFlowspecCommunity, peakflowSPMgr=peakflowSPMgr, spInterface=spInterface, spInterfaceUsage=spInterfaceUsage, mitigationTMSStart=mitigationTMSStart, deviceTotalFlows=deviceTotalFlows, spInterfaceSpeed=spInterfaceSpeed, spBlackholeCommunity=spBlackholeCommunity, spDetectedCountries=spDetectedCountries, spBGPTrapPrefix=spBGPTrapPrefix, fingerprintReceive=fingerprintReceive, spPravail=spPravail, spScriptStart=spScriptStart, spLicenseErrType=spLicenseErrType, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, reportDone=reportDone, configChange=configChange, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spVersion=spVersion, spMitigationStart=spMitigationStart, spHijackAttr=spHijackAttr, mitigationTMSStartV2=mitigationTMSStartV2, peakflowSPTraps=peakflowSPTraps, autoclassifyStart=autoclassifyStart, interfaceUsage=interfaceUsage, spCollector=spCollector, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceHealth=deviceHealth, spDetector=spDetector, bgpInstabilityDone=bgpInstabilityDone, spUsage=spUsage, spAlertID=spAlertID, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, managedObjectUsage=managedObjectUsage, spImpactBps=spImpactBps, licenseError=licenseError, spDNSObservedMean=spDNSObservedMean, spBlackholeTimeout=spBlackholeTimeout, spTMSMultiPrefix=spTMSMultiPrefix, spBlackholePrefix=spBlackholePrefix, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, deviceSwapSpace=deviceSwapSpace, spTMSPrefix=spTMSPrefix, spUnit=spUnit, spInetAddressType=spInetAddressType, hardwareFailure=hardwareFailure, spcommFailureDone=spcommFailureDone, spSystemErrorDescription=spSystemErrorDescription, spTMSTimeout=spTMSTimeout, spDuration=spDuration, spThreshold=spThreshold, spScriptHost=spScriptHost, hardwareFailureDone=hardwareFailureDone, alertScript=alertScript, mitigationBlackholeStart=mitigationBlackholeStart, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spFingerprintMessage=spFingerprintMessage, spCommFailureDestination=spCommFailureDestination, serviceUsageDone=serviceUsageDone, spFingerprintSender=spFingerprintSender, interfaceUsageDone=interfaceUsageDone, spLicenseErrCount=spLicenseErrCount, spManagedObjects=spManagedObjects, spLicenseErrLimit=spLicenseErrLimit, fingerprintUsage=fingerprintUsage, spImpactPps=spImpactPps, spThirdPartyAddr=spThirdPartyAddr, spSystemErrorType=spSystemErrorType, spBGPEvent=spBGPEvent, deviceSystemError=deviceSystemError, spDNSName=spDNSName, spGreTunnelName=spGreTunnelName, spHijackRoute=spHijackRoute, spServiceElement=spServiceElement, collectorDown=collectorDown, spServiceName=spServiceName, managedObjectUsageDone=managedObjectUsageDone, spManagedObjectFamily=spManagedObjectFamily, spcommFailure=spcommFailure, spBGPTrapEvent=spBGPTrapEvent, spDNSObserved=spDNSObserved, snmpUp=snmpUp, spApplicationName=spApplicationName, spFingerprintName=spFingerprintName, spManagedObject=spManagedObject, bgpInstability=bgpInstability, peakflowSPMIB=peakflowSPMIB, spAlertDetectionSignatures=spAlertDetectionSignatures, flowDown=flowDown, mitigationFlowspecStart=mitigationFlowspecStart, bgpUp=bgpUp, flowUp=flowUp, spRouter=spRouter, spReportID=spReportID, spInetAddress=spInetAddress, deviceTotalFlowsHC=deviceTotalFlowsHC, spMitigationName=spMitigationName, peakflowSPCMI=peakflowSPCMI, spDNSExpected=spDNSExpected, spFlowspecTimeout=spFlowspecTimeout, cloudSignalingFaultDone=cloudSignalingFaultDone, dosHostDetectionAlert=dosHostDetectionAlert, spBGPTrapName=spBGPTrapName, spScriptCommand=spScriptCommand, spUsageType=spUsageType, licenseErrorDone=licenseErrorDone, greDownDone=greDownDone, spScriptPort=spScriptPort, spLicenseErrDescription=spLicenseErrDescription, spBlackholeNexthop=spBlackholeNexthop, spHardwareFailureDescription=spHardwareFailureDescription, bgpHijack=bgpHijack, routingFailover=routingFailover, collectorStart=collectorStart, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spGreTunnelDestination=spGreTunnelDestination, spInterfaceIndex=spInterfaceIndex, spBGPInstability=spBGPInstability, spReportURL=spReportURL, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceDiskUsage=deviceDiskUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spBGPTrapNewAttributes=spBGPTrapNewAttributes, trafficAutoMitigation=trafficAutoMitigation, cloudSignalingFault=cloudSignalingFault, spThirdPartyZone=spThirdPartyZone, bgpDown=bgpDown, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spUsername=spUsername, spInterfaceSpeedHC=spInterfaceSpeedHC, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spReportName=spReportName, collectorUp=collectorUp, devicePhysicalMemory=devicePhysicalMemory, spInterfaceUsageHC=spInterfaceUsageHC, mitigationDone=mitigationDone, spMitigationID=spMitigationID, spTMSCommunity=spTMSCommunity, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spHijackLocal=spHijackLocal, fingerprintUsageDone=fingerprintUsageDone, snmpDown=snmpDown, spCommFailureDescription=spCommFailureDescription, notificationLimit=notificationLimit, dnsBaselineDone=dnsBaselineDone, greDown=greDown, PYSNMP_MODULE_ID=peakflowSPMIB, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceSystemErrorDone=deviceSystemErrorDone, serviceUsage=serviceUsage, bgpTrap=bgpTrap, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dnsBaseline=dnsBaseline, spFingerprintFeedback=spFingerprintFeedback, fingerprintFeedback=fingerprintFeedback, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spDNSObservedMax=spDNSObservedMax)
