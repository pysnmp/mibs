...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", dosHostDetectionAlertDone=dosHostDetectionAlertDone, spHardwareFailureDescription=spHardwareFailureDescription, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spInetAddressType=spInetAddressType, devicePhysicalMemory=devicePhysicalMemory, spDNSName=spDNSName, spDetector=spDetector, spThreshold=spThreshold, spImpactBps=spImpactBps, spDNSObservedMean=spDNSObservedMean, fingerprintUsageDone=fingerprintUsageDone, bgpDown=bgpDown, spDetectedCountries=spDetectedCountries, configChange=configChange, spSystemErrorType=spSystemErrorType, spDuration=spDuration, spCommFailureDestination=spCommFailureDestination, spApplicationName=spApplicationName, managedObjectUsageDone=managedObjectUsageDone, spReportID=spReportID, deviceTotalFlowsHC=deviceTotalFlowsHC, bgpUp=bgpUp, spFingerprintName=spFingerprintName, spInterfaceSpeed=spInterfaceSpeed, bgpInstability=bgpInstability, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spTMSTimeout=spTMSTimeout, dnsBaselineDone=dnsBaselineDone, spThirdPartyAddr=spThirdPartyAddr, spPravail=spPravail, spLicenseErrLimit=spLicenseErrLimit, peakflowSPTraps=peakflowSPTraps, spInterfaceSpeedHC=spInterfaceSpeedHC, spThirdPartyZone=spThirdPartyZone, spBlackholeNexthop=spBlackholeNexthop, mitigationDone=mitigationDone, spGreTunnelName=spGreTunnelName, greDownDone=greDownDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spInterfaceUsage=spInterfaceUsage, dosHostDetectionAlert=dosHostDetectionAlert, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, mitigationBlackholeStart=mitigationBlackholeStart, peakflowSPCMI=peakflowSPCMI, spHijackLocal=spHijackLocal, cloudSignalingFault=cloudSignalingFault, spCommFailureDescription=spCommFailureDescription, spAlertDetectionSignatures=spAlertDetectionSignatures, mitigationFlowspecStart=mitigationFlowspecStart, spAlertID=spAlertID, spUsageType=spUsageType, collectorUp=collectorUp, bgpHijack=bgpHijack, spTMSCommunity=spTMSCommunity, flowDown=flowDown, deviceHealth=deviceHealth, spTMSPrefix=spTMSPrefix, spcommFailureDone=spcommFailureDone, spBlackholeTimeout=spBlackholeTimeout, reportDone=reportDone, spUsage=spUsage, spFingerprintMessage=spFingerprintMessage, trafficAutoMitigation=trafficAutoMitigation, spSystemErrorDescription=spSystemErrorDescription, snmpDown=snmpDown, spServiceElement=spServiceElement, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterface=spInterface, fingerprintFeedback=fingerprintFeedback, spCollector=spCollector, greDown=greDown, spScriptPort=spScriptPort, spMitigationID=spMitigationID, collectorStart=collectorStart, interfaceUsageDone=interfaceUsageDone, hardwareFailure=hardwareFailure, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, licenseErrorDone=licenseErrorDone, spBGPTrapName=spBGPTrapName, deviceSystemError=deviceSystemError, spReportURL=spReportURL, notificationLimit=notificationLimit, peakflowSPMgr=peakflowSPMgr, spScriptStart=spScriptStart, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceTotalFlows=deviceTotalFlows, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spLicenseErrDescription=spLicenseErrDescription, bgpInstabilityDone=bgpInstabilityDone, interfaceUsage=interfaceUsage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spBGPInstability=spBGPInstability, spMitigationStart=spMitigationStart, spFlowspecTimeout=spFlowspecTimeout, spManagedObjects=spManagedObjects, collectorDown=collectorDown, spBGPTrapPrefix=spBGPTrapPrefix, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBGPTrapNewAttributes=spBGPTrapNewAttributes, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, snmpUp=snmpUp, spcommFailure=spcommFailure, deviceDiskUsage=deviceDiskUsage, deviceSystemErrorDone=deviceSystemErrorDone, fingerprintReceive=fingerprintReceive, spImpactPps=spImpactPps, spFingerprintSender=spFingerprintSender, spHijackAttr=spHijackAttr, spTMSMultiPrefix=spTMSMultiPrefix, mitigationTMSStartV2=mitigationTMSStartV2, spScriptCommand=spScriptCommand, spDNSExpected=spDNSExpected, spLicenseErrCount=spLicenseErrCount, spInterfaceUsageHC=spInterfaceUsageHC, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, PYSNMP_MODULE_ID=peakflowSPMIB, spGreTunnelDestination=spGreTunnelDestination, spDNSObservedMax=spDNSObservedMax, licenseError=licenseError, spServiceName=spServiceName, serviceUsage=serviceUsage, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceSwapSpaceInUse=deviceSwapSpaceInUse, managedObjectUsage=managedObjectUsage, mitigationTMSStart=mitigationTMSStart, spHijackRoute=spHijackRoute, fingerprintUsage=fingerprintUsage, hardwareFailureDone=hardwareFailureDone, spVersion=spVersion, spBlackholeCommunity=spBlackholeCommunity, routingFailover=routingFailover, spDNSObserved=spDNSObserved, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spFingerprintFeedback=spFingerprintFeedback, spLicenseErrType=spLicenseErrType, flowUp=flowUp, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spFlowspecCommunity=spFlowspecCommunity, spUsername=spUsername, serviceUsageDone=serviceUsageDone, spBGPTrapEvent=spBGPTrapEvent, spBlackholePrefix=spBlackholePrefix, spInetAddress=spInetAddress, alertScript=alertScript, deviceSwapSpace=deviceSwapSpace, spManagedObjectFamily=spManagedObjectFamily, spUnit=spUnit, spMitigationName=spMitigationName, dnsBaseline=dnsBaseline, spScriptHost=spScriptHost, spInterfaceIndex=spInterfaceIndex, spManagedObject=spManagedObject, bgpTrap=bgpTrap, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, autoclassifyStart=autoclassifyStart, peakflowSPMIB=peakflowSPMIB, spRouter=spRouter, spBGPEvent=spBGPEvent, spReportName=spReportName)
