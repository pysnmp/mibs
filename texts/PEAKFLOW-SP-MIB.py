...
iso, NotificationType, MibScalar, MibTable, MibTableRow, MibTableColumn, MibIdentifier, ModuleIdentity, Integer32, IpAddress, Bits, ObjectIdentity, Counter32, Unsigned32, Gauge32, TimeTicks, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "iso", "NotificationType", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "MibIdentifier", "ModuleIdentity", "Integer32", "IpAddress", "Bits", "ObjectIdentity", "Counter32", "Unsigned32", "Gauge32", "TimeTicks", "Counter64")
...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spImpactBps=spImpactBps, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, mitigationTMSStart=mitigationTMSStart, spcommFailureDone=spcommFailureDone, spGreTunnelDestination=spGreTunnelDestination, greDownDone=greDownDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, cloudSignalingFaultDone=cloudSignalingFaultDone, spRouter=spRouter, dnsBaseline=dnsBaseline, spDNSObserved=spDNSObserved, spBGPInstability=spBGPInstability, spThreshold=spThreshold, spLicenseErrDescription=spLicenseErrDescription, serviceUsage=serviceUsage, deviceSystemError=deviceSystemError, reportDone=reportDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBGPEvent=spBGPEvent, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spInterfaceUsage=spInterfaceUsage, spLicenseErrType=spLicenseErrType, spBlackholeCommunity=spBlackholeCommunity, interfaceUsageDone=interfaceUsageDone, spMitigationStart=spMitigationStart, spHijackAttr=spHijackAttr, deviceTotalFlows=deviceTotalFlows, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spPravail=spPravail, spMitigationName=spMitigationName, spManagedObjects=spManagedObjects, spDetectedCountries=spDetectedCountries, fingerprintUsage=fingerprintUsage, spHijackRoute=spHijackRoute, spDetector=spDetector, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterfaceUsageHC=spInterfaceUsageHC, autoclassifyStart=autoclassifyStart, deviceHealth=deviceHealth, mitigationDone=mitigationDone, spFingerprintSender=spFingerprintSender, managedObjectUsage=managedObjectUsage, dosHostDetectionAlert=dosHostDetectionAlert, peakflowSPTraps=peakflowSPTraps, routingFailover=routingFailover, spUsage=spUsage, spScriptPort=spScriptPort, snmpUp=snmpUp, spHijackLocal=spHijackLocal, spUsageType=spUsageType, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, PYSNMP_MODULE_ID=peakflowSPMIB, bgpUp=bgpUp, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spInterfaceSpeed=spInterfaceSpeed, spDNSName=spDNSName, spBGPTrapNewAttributes=spBGPTrapNewAttributes, collectorStart=collectorStart, bgpTrap=bgpTrap, cloudSignalingFault=cloudSignalingFault, spThirdPartyZone=spThirdPartyZone, collectorDown=collectorDown, spSystemErrorDescription=spSystemErrorDescription, spImpactPps=spImpactPps, hardwareFailureDone=hardwareFailureDone, spCommFailureDescription=spCommFailureDescription, trafficAutoMitigation=trafficAutoMitigation, spInterface=spInterface, spServiceName=spServiceName, spLicenseErrLimit=spLicenseErrLimit, licenseError=licenseError, spFingerprintName=spFingerprintName, spCollector=spCollector, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, mitigationFlowspecStart=mitigationFlowspecStart, alertScript=alertScript, spHardwareFailureDescription=spHardwareFailureDescription, spLicenseErrCount=spLicenseErrCount, bgpInstability=bgpInstability, spScriptCommand=spScriptCommand, spReportURL=spReportURL, spDNSObservedMax=spDNSObservedMax, spAlertDetectionSignatures=spAlertDetectionSignatures, configChange=configChange, deviceSystemErrorDone=deviceSystemErrorDone, deviceDiskUsage=deviceDiskUsage, bgpDown=bgpDown, fingerprintUsageDone=fingerprintUsageDone, spBGPTrapName=spBGPTrapName, spTMSMultiPrefix=spTMSMultiPrefix, spInterfaceSpeedHC=spInterfaceSpeedHC, deviceSwapSpace=deviceSwapSpace, spBlackholeTimeout=spBlackholeTimeout, dosHostDetectionAlertDone=dosHostDetectionAlertDone, peakflowSPMIB=peakflowSPMIB, spReportID=spReportID, flowUp=flowUp, spReportName=spReportName, spTMSPrefix=spTMSPrefix, bgpHijack=bgpHijack, fingerprintFeedback=fingerprintFeedback, spAlertID=spAlertID, spTMSTimeout=spTMSTimeout, spScriptStart=spScriptStart, spBGPTrapEvent=spBGPTrapEvent, deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceTotalFlowsHC=deviceTotalFlowsHC, spSystemErrorType=spSystemErrorType, managedObjectUsageDone=managedObjectUsageDone, spInetAddress=spInetAddress, spInterfaceIndex=spInterfaceIndex, spApplicationName=spApplicationName, spUsername=spUsername, spFlowspecTimeout=spFlowspecTimeout, spFingerprintMessage=spFingerprintMessage, mitigationThirdPartyStart=mitigationThirdPartyStart, spInetAddressType=spInetAddressType, spTMSCommunity=spTMSCommunity, licenseErrorDone=licenseErrorDone, spServiceElement=spServiceElement, fingerprintReceive=fingerprintReceive, spThirdPartyAddr=spThirdPartyAddr, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spBGPTrapPrefix=spBGPTrapPrefix, spFlowspecCommunity=spFlowspecCommunity, collectorUp=collectorUp, spManagedObjectFamily=spManagedObjectFamily, serviceUsageDone=serviceUsageDone, devicePhysicalMemory=devicePhysicalMemory, dnsBaselineDone=dnsBaselineDone, greDown=greDown, spDuration=spDuration, spGreTunnelName=spGreTunnelName, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, notificationLimit=notificationLimit, peakflowSPMgr=peakflowSPMgr, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spManagedObject=spManagedObject, spMitigationID=spMitigationID, spcommFailure=spcommFailure, mitigationBlackholeStart=mitigationBlackholeStart, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spDNSObservedMean=spDNSObservedMean, spBlackholeNexthop=spBlackholeNexthop, snmpDown=snmpDown, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spDNSExpected=spDNSExpected, interfaceUsage=interfaceUsage, spScriptHost=spScriptHost, spCommFailureDestination=spCommFailureDestination, spFingerprintFeedback=spFingerprintFeedback, hardwareFailure=hardwareFailure, spBlackholePrefix=spBlackholePrefix, flowDown=flowDown, spVersion=spVersion, spUnit=spUnit, mitigationTMSStartV2=mitigationTMSStartV2, peakflowSPCMI=peakflowSPCMI, bgpInstabilityDone=bgpInstabilityDone)
