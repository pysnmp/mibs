...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spUsage=spUsage, fingerprintReceive=fingerprintReceive, spApplicationName=spApplicationName, spFingerprintName=spFingerprintName, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInterfaceIndex=spInterfaceIndex, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spImpactPps=spImpactPps, interfaceUsageDone=interfaceUsageDone, spBGPTrapName=spBGPTrapName, spHijackRoute=spHijackRoute, dnsBaselineDone=dnsBaselineDone, alertScript=alertScript, serviceUsageDone=serviceUsageDone, dnsBaseline=dnsBaseline, spDNSExpected=spDNSExpected, spTMSPrefix=spTMSPrefix, managedObjectUsageDone=managedObjectUsageDone, collectorDown=collectorDown, deviceTotalFlowsHC=deviceTotalFlowsHC, spDetector=spDetector, configChange=configChange, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, mitigationTMSStart=mitigationTMSStart, spBlackholeTimeout=spBlackholeTimeout, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spBlackholeNexthop=spBlackholeNexthop, spGreTunnelDestination=spGreTunnelDestination, spAlertID=spAlertID, spHardwareFailureDescription=spHardwareFailureDescription, spDNSObserved=spDNSObserved, spScriptCommand=spScriptCommand, spInetAddressType=spInetAddressType, mitigationDone=mitigationDone, spCollector=spCollector, collectorUp=collectorUp, spReportName=spReportName, spSystemErrorDescription=spSystemErrorDescription, spGreTunnelName=spGreTunnelName, spInterfaceUsageHC=spInterfaceUsageHC, spFingerprintMessage=spFingerprintMessage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spServiceName=spServiceName, spInterfaceSpeedHC=spInterfaceSpeedHC, greDown=greDown, spTMSMultiPrefix=spTMSMultiPrefix, spScriptStart=spScriptStart, PYSNMP_MODULE_ID=peakflowSPMIB, deviceTotalFlows=deviceTotalFlows, spPravail=spPravail, spBlackholeCommunity=spBlackholeCommunity, spLicenseErrDescription=spLicenseErrDescription, bgpTrap=bgpTrap, spLicenseErrType=spLicenseErrType, spManagedObjectFamily=spManagedObjectFamily, dosNetworkProfiledAlert=dosNetworkProfiledAlert, hardwareFailure=hardwareFailure, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spTMSCommunity=spTMSCommunity, mitigationTMSStartV2=mitigationTMSStartV2, spThreshold=spThreshold, spReportID=spReportID, deviceHealth=deviceHealth, flowDown=flowDown, interfaceUsage=interfaceUsage, managedObjectUsage=managedObjectUsage, licenseError=licenseError, devicePhysicalMemory=devicePhysicalMemory, spFlowspecCommunity=spFlowspecCommunity, spAlertDetectionSignatures=spAlertDetectionSignatures, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, peakflowSPTraps=peakflowSPTraps, cloudSignalingFaultDone=cloudSignalingFaultDone, bgpUp=bgpUp, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spReportURL=spReportURL, spHijackLocal=spHijackLocal, fingerprintUsage=fingerprintUsage, fingerprintFeedback=fingerprintFeedback, spMitigationID=spMitigationID, snmpUp=snmpUp, bgpInstabilityDone=bgpInstabilityDone, flowUp=flowUp, mitigationFlowspecStart=mitigationFlowspecStart, spManagedObjects=spManagedObjects, mitigationBlackholeStart=mitigationBlackholeStart, spFingerprintSender=spFingerprintSender, spUsername=spUsername, deviceDiskUsage=deviceDiskUsage, serviceUsage=serviceUsage, spMitigationName=spMitigationName, deviceSystemError=deviceSystemError, spBGPInstability=spBGPInstability, spcommFailure=spcommFailure, spInterfaceUsage=spInterfaceUsage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSName=spDNSName, spInterfaceSpeed=spInterfaceSpeed, peakflowSPMgr=peakflowSPMgr, spThirdPartyAddr=spThirdPartyAddr, dosHostDetectionAlert=dosHostDetectionAlert, greDownDone=greDownDone, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spScriptPort=spScriptPort, spBGPTrapEvent=spBGPTrapEvent, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spImpactBps=spImpactBps, bgpDown=bgpDown, reportDone=reportDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInetAddress=spInetAddress, spLicenseErrLimit=spLicenseErrLimit, spBlackholePrefix=spBlackholePrefix, spcommFailureDone=spcommFailureDone, bgpHijack=bgpHijack, spUnit=spUnit, autoclassifyStart=autoclassifyStart, spLicenseErrCount=spLicenseErrCount, spMitigationStart=spMitigationStart, routingFailover=routingFailover, spBGPTrapPrefix=spBGPTrapPrefix, spHijackAttr=spHijackAttr, spDuration=spDuration, peakflowSPCMI=peakflowSPCMI, collectorStart=collectorStart, spSystemErrorType=spSystemErrorType, spServiceElement=spServiceElement, spDetectedCountries=spDetectedCountries, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, snmpDown=snmpDown, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, hardwareFailureDone=hardwareFailureDone, peakflowSPMIB=peakflowSPMIB, spVersion=spVersion, spTMSTimeout=spTMSTimeout, fingerprintUsageDone=fingerprintUsageDone, licenseErrorDone=licenseErrorDone, spUsageType=spUsageType, spManagedObject=spManagedObject, spDNSObservedMax=spDNSObservedMax, spCommFailureDescription=spCommFailureDescription, spRouter=spRouter, spScriptHost=spScriptHost, spDNSObservedMean=spDNSObservedMean, spCommFailureDestination=spCommFailureDestination, trafficAutoMitigation=trafficAutoMitigation, spInterface=spInterface, deviceSwapSpace=deviceSwapSpace, spThirdPartyZone=spThirdPartyZone, notificationLimit=notificationLimit, mitigationThirdPartyStart=mitigationThirdPartyStart, spFlowspecTimeout=spFlowspecTimeout, spFingerprintFeedback=spFingerprintFeedback, deviceSystemErrorDone=deviceSystemErrorDone, cloudSignalingFault=cloudSignalingFault, spBGPEvent=spBGPEvent, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, bgpInstability=bgpInstability, deviceSwapSpaceUsage=deviceSwapSpaceUsage)
