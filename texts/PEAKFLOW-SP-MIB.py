...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", peakflowSPCMI=peakflowSPCMI, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, interfaceUsage=interfaceUsage, spBlackholeTimeout=spBlackholeTimeout, spFingerprintName=spFingerprintName, spLicenseErrDescription=spLicenseErrDescription, snmpUp=snmpUp, spReportName=spReportName, spScriptStart=spScriptStart, devicePhysicalMemory=devicePhysicalMemory, spFingerprintFeedback=spFingerprintFeedback, interfaceUsageDone=interfaceUsageDone, spManagedObjectFamily=spManagedObjectFamily, hardwareFailureDone=hardwareFailureDone, spUsername=spUsername, mitigationTMSStart=mitigationTMSStart, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, managedObjectUsageDone=managedObjectUsageDone, mitigationTMSStartV2=mitigationTMSStartV2, spGreTunnelName=spGreTunnelName, spRouter=spRouter, fingerprintFeedback=fingerprintFeedback, spThirdPartyAddr=spThirdPartyAddr, peakflowSPTraps=peakflowSPTraps, configChange=configChange, mitigationBlackholeStart=mitigationBlackholeStart, fingerprintReceive=fingerprintReceive, spcommFailure=spcommFailure, greDownDone=greDownDone, peakflowSPMgr=peakflowSPMgr, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, cloudSignalingFaultDone=cloudSignalingFaultDone, spBlackholeNexthop=spBlackholeNexthop, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, mitigationDone=mitigationDone, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, hardwareFailure=hardwareFailure, dnsBaseline=dnsBaseline, collectorStart=collectorStart, bgpHijack=bgpHijack, fingerprintUsage=fingerprintUsage, deviceDiskUsage=deviceDiskUsage, spFlowspecTimeout=spFlowspecTimeout, bgpInstability=bgpInstability, spMitigationID=spMitigationID, snmpDown=snmpDown, spDNSObserved=spDNSObserved, spInetAddress=spInetAddress, managedObjectUsage=managedObjectUsage, spVersion=spVersion, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spThirdPartyZone=spThirdPartyZone, bgpUp=bgpUp, spReportID=spReportID, mitigationFlowspecStart=mitigationFlowspecStart, spCommFailureDescription=spCommFailureDescription, spLicenseErrCount=spLicenseErrCount, spBlackholePrefix=spBlackholePrefix, spDetector=spDetector, notificationLimit=notificationLimit, spAlertDetectionSignatures=spAlertDetectionSignatures, spMitigationStart=spMitigationStart, cloudSignalingFault=cloudSignalingFault, mitigationThirdPartyStart=mitigationThirdPartyStart, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, licenseError=licenseError, spUnit=spUnit, spSystemErrorType=spSystemErrorType, collectorDown=collectorDown, spFingerprintSender=spFingerprintSender, spCollector=spCollector, spDuration=spDuration, serviceUsage=serviceUsage, spDNSObservedMean=spDNSObservedMean, spHijackLocal=spHijackLocal, spHijackAttr=spHijackAttr, flowUp=flowUp, spcommFailureDone=spcommFailureDone, spTMSCommunity=spTMSCommunity, deviceSystemError=deviceSystemError, spScriptPort=spScriptPort, spBGPInstability=spBGPInstability, spDNSName=spDNSName, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spThreshold=spThreshold, spHardwareFailureDescription=spHardwareFailureDescription, spInterfaceUsage=spInterfaceUsage, spBGPTrapEvent=spBGPTrapEvent, spFingerprintMessage=spFingerprintMessage, spDetectedCountries=spDetectedCountries, bgpDown=bgpDown, spDNSExpected=spDNSExpected, spInterfaceSpeed=spInterfaceSpeed, spInterfaceSpeedHC=spInterfaceSpeedHC, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spTMSPrefix=spTMSPrefix, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spSystemErrorDescription=spSystemErrorDescription, bgpTrap=bgpTrap, spTMSTimeout=spTMSTimeout, peakflowSPMIB=peakflowSPMIB, spAlertID=spAlertID, autoclassifyStart=autoclassifyStart, spImpactBps=spImpactBps, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spMitigationName=spMitigationName, fingerprintUsageDone=fingerprintUsageDone, spBGPTrapPrefix=spBGPTrapPrefix, alertScript=alertScript, spScriptCommand=spScriptCommand, spServiceName=spServiceName, spBlackholeCommunity=spBlackholeCommunity, licenseErrorDone=licenseErrorDone, flowDown=flowDown, spManagedObjects=spManagedObjects, spPravail=spPravail, spInterface=spInterface, spBGPTrapName=spBGPTrapName, greDown=greDown, spFlowspecCommunity=spFlowspecCommunity, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spCommFailureDestination=spCommFailureDestination, spScriptHost=spScriptHost, dnsBaselineDone=dnsBaselineDone, deviceTotalFlows=deviceTotalFlows, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBGPEvent=spBGPEvent, deviceHealth=deviceHealth, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInetAddressType=spInetAddressType, spReportURL=spReportURL, spDNSObservedMax=spDNSObservedMax, collectorUp=collectorUp, spApplicationName=spApplicationName, spUsage=spUsage, spInterfaceUsageHC=spInterfaceUsageHC, dosHostDetectionAlert=dosHostDetectionAlert, serviceUsageDone=serviceUsageDone, spLicenseErrType=spLicenseErrType, spCloudSignalFaultDescription=spCloudSignalFaultDescription, bgpInstabilityDone=bgpInstabilityDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceTotalFlowsHC=deviceTotalFlowsHC, spUsageType=spUsageType, reportDone=reportDone, spHijackRoute=spHijackRoute, deviceSwapSpaceUsage=deviceSwapSpaceUsage, PYSNMP_MODULE_ID=peakflowSPMIB, deviceSwapSpace=deviceSwapSpace, spLicenseErrLimit=spLicenseErrLimit, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spServiceElement=spServiceElement, spManagedObject=spManagedObject, routingFailover=routingFailover, trafficAutoMitigation=trafficAutoMitigation, spImpactPps=spImpactPps, spInterfaceIndex=spInterfaceIndex, spTMSMultiPrefix=spTMSMultiPrefix, deviceSystemErrorDone=deviceSystemErrorDone, spGreTunnelDestination=spGreTunnelDestination)
