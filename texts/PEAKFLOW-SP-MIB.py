...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spScriptHost=spScriptHost, trafficAutoMitigation=trafficAutoMitigation, spMitigationStart=spMitigationStart, spUsername=spUsername, peakflowSPCMI=peakflowSPCMI, snmpUp=snmpUp, collectorStart=collectorStart, spInterfaceSpeedHC=spInterfaceSpeedHC, spFlowspecTimeout=spFlowspecTimeout, spTMSMultiPrefix=spTMSMultiPrefix, routingFailover=routingFailover, flowUp=flowUp, spInterfaceSpeed=spInterfaceSpeed, spFingerprintName=spFingerprintName, mitigationTMSStart=mitigationTMSStart, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spBGPTrapName=spBGPTrapName, deviceSystemError=deviceSystemError, licenseErrorDone=licenseErrorDone, managedObjectUsage=managedObjectUsage, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spInetAddress=spInetAddress, spInterfaceUsage=spInterfaceUsage, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spManagedObjectFamily=spManagedObjectFamily, spRouter=spRouter, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spInterfaceIndex=spInterfaceIndex, spImpactPps=spImpactPps, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spcommFailure=spcommFailure, bgpUp=bgpUp, spPravail=spPravail, spcommFailureDone=spcommFailureDone, spThirdPartyZone=spThirdPartyZone, spScriptPort=spScriptPort, spHardwareFailureDescription=spHardwareFailureDescription, serviceUsage=serviceUsage, cloudSignalingFault=cloudSignalingFault, spReportID=spReportID, spLicenseErrDescription=spLicenseErrDescription, configChange=configChange, deviceSwapSpace=deviceSwapSpace, fingerprintUsageDone=fingerprintUsageDone, spAlertID=spAlertID, interfaceUsage=interfaceUsage, spScriptCommand=spScriptCommand, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceSystemErrorDone=deviceSystemErrorDone, deviceHealth=deviceHealth, fingerprintFeedback=fingerprintFeedback, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spDNSExpected=spDNSExpected, spBGPTrapNewAttributes=spBGPTrapNewAttributes, dosHostDetectionAlert=dosHostDetectionAlert, spHijackAttr=spHijackAttr, spSystemErrorType=spSystemErrorType, spVersion=spVersion, spThreshold=spThreshold, spCommFailureDestination=spCommFailureDestination, deviceTotalFlows=deviceTotalFlows, spDetectedCountries=spDetectedCountries, greDownDone=greDownDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spReportURL=spReportURL, notificationLimit=notificationLimit, deviceDiskUsage=deviceDiskUsage, alertScript=alertScript, spThirdPartyAddr=spThirdPartyAddr, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spGreTunnelDestination=spGreTunnelDestination, dnsBaseline=dnsBaseline, fingerprintUsage=fingerprintUsage, flowDown=flowDown, PYSNMP_MODULE_ID=peakflowSPMIB, spServiceElement=spServiceElement, spDuration=spDuration, spBGPTrapPrefix=spBGPTrapPrefix, spManagedObjects=spManagedObjects, hardwareFailureDone=hardwareFailureDone, spDNSObservedMax=spDNSObservedMax, spImpactBps=spImpactBps, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, collectorUp=collectorUp, spScriptStart=spScriptStart, spBlackholeTimeout=spBlackholeTimeout, mitigationFlowspecStart=mitigationFlowspecStart, spHijackRoute=spHijackRoute, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dnsBaselineDone=dnsBaselineDone, spLicenseErrCount=spLicenseErrCount, spTMSTimeout=spTMSTimeout, spFlowspecCommunity=spFlowspecCommunity, spDNSName=spDNSName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spFingerprintSender=spFingerprintSender, spMitigationName=spMitigationName, mitigationTMSStartV2=mitigationTMSStartV2, spMitigationID=spMitigationID, managedObjectUsageDone=managedObjectUsageDone, devicePhysicalMemory=devicePhysicalMemory, spCollector=spCollector, spUnit=spUnit, spServiceName=spServiceName, reportDone=reportDone, bgpHijack=bgpHijack, spInterface=spInterface, spInterfaceUsageHC=spInterfaceUsageHC, spTMSPrefix=spTMSPrefix, spBlackholePrefix=spBlackholePrefix, spLicenseErrType=spLicenseErrType, licenseError=licenseError, spBlackholeCommunity=spBlackholeCommunity, spInetAddressType=spInetAddressType, spReportName=spReportName, autoclassifyStart=autoclassifyStart, mitigationDone=mitigationDone, bgpInstabilityDone=bgpInstabilityDone, spDNSObserved=spDNSObserved, spUsage=spUsage, bgpTrap=bgpTrap, snmpDown=snmpDown, bgpDown=bgpDown, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, dosHostDetectionAlertDone=dosHostDetectionAlertDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spBGPInstability=spBGPInstability, bgpInstability=bgpInstability, spHijackLocal=spHijackLocal, spAlertDetectionSignatures=spAlertDetectionSignatures, peakflowSPMIB=peakflowSPMIB, spBlackholeNexthop=spBlackholeNexthop, spLicenseErrLimit=spLicenseErrLimit, deviceTotalFlowsHC=deviceTotalFlowsHC, spBGPEvent=spBGPEvent, spApplicationName=spApplicationName, peakflowSPTraps=peakflowSPTraps, mitigationBlackholeStart=mitigationBlackholeStart, spManagedObject=spManagedObject, spGreTunnelName=spGreTunnelName, spUsageType=spUsageType, spDetector=spDetector, spCommFailureDescription=spCommFailureDescription, spBGPTrapEvent=spBGPTrapEvent, interfaceUsageDone=interfaceUsageDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, hardwareFailure=hardwareFailure, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, peakflowSPMgr=peakflowSPMgr, spTMSCommunity=spTMSCommunity, spFingerprintMessage=spFingerprintMessage, collectorDown=collectorDown, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spFingerprintFeedback=spFingerprintFeedback, serviceUsageDone=serviceUsageDone, fingerprintReceive=fingerprintReceive, greDown=greDown, mitigationThirdPartyStart=mitigationThirdPartyStart, spSystemErrorDescription=spSystemErrorDescription, spDNSObservedMean=spDNSObservedMean)
