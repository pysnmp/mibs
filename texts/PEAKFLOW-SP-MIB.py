...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spFlowspecCommunity=spFlowspecCommunity, spInterfaceIndex=spInterfaceIndex, bgpTrap=bgpTrap, spBGPTrapEvent=spBGPTrapEvent, spBlackholeTimeout=spBlackholeTimeout, spScriptCommand=spScriptCommand, spBGPTrapName=spBGPTrapName, spManagedObject=spManagedObject, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSObserved=spDNSObserved, peakflowSPTraps=peakflowSPTraps, mitigationDone=mitigationDone, spUsername=spUsername, spBlackholeNexthop=spBlackholeNexthop, spCollector=spCollector, spScriptPort=spScriptPort, spReportURL=spReportURL, bgpUp=bgpUp, cloudSignalingFaultDone=cloudSignalingFaultDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, bgpInstability=bgpInstability, bgpDown=bgpDown, spSystemErrorDescription=spSystemErrorDescription, deviceSwapSpaceUsage=deviceSwapSpaceUsage, routingFailover=routingFailover, spFingerprintMessage=spFingerprintMessage, spInterfaceUsageHC=spInterfaceUsageHC, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosHostDetectionAlert=dosHostDetectionAlert, spMitigationName=spMitigationName, spBlackholeCommunity=spBlackholeCommunity, spFingerprintFeedback=spFingerprintFeedback, fingerprintReceive=fingerprintReceive, collectorUp=collectorUp, spUsage=spUsage, reportDone=reportDone, alertScript=alertScript, peakflowSPMgr=peakflowSPMgr, spTMSMultiPrefix=spTMSMultiPrefix, spInterfaceSpeedHC=spInterfaceSpeedHC, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spPravail=spPravail, collectorStart=collectorStart, spHijackRoute=spHijackRoute, spUsageType=spUsageType, spMitigationID=spMitigationID, spVersion=spVersion, mitigationFlowspecStart=mitigationFlowspecStart, spLicenseErrLimit=spLicenseErrLimit, dnsBaselineDone=dnsBaselineDone, licenseErrorDone=licenseErrorDone, mitigationBlackholeStart=mitigationBlackholeStart, spcommFailureDone=spcommFailureDone, spManagedObjects=spManagedObjects, deviceTotalFlows=deviceTotalFlows, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spManagedObjectFamily=spManagedObjectFamily, licenseError=licenseError, spSystemErrorType=spSystemErrorType, spAlertDetectionSignatures=spAlertDetectionSignatures, spMitigationStart=spMitigationStart, spFingerprintName=spFingerprintName, deviceDiskUsage=deviceDiskUsage, spServiceElement=spServiceElement, hardwareFailureDone=hardwareFailureDone, spcommFailure=spcommFailure, spGreTunnelDestination=spGreTunnelDestination, cloudSignalingFault=cloudSignalingFault, managedObjectUsage=managedObjectUsage, snmpUp=snmpUp, spApplicationName=spApplicationName, spThirdPartyZone=spThirdPartyZone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, interfaceUsageDone=interfaceUsageDone, deviceTotalFlowsHC=deviceTotalFlowsHC, mitigationTMSStartV2=mitigationTMSStartV2, deviceSwapSpaceInUse=deviceSwapSpaceInUse, serviceUsageDone=serviceUsageDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spRouter=spRouter, spDNSObservedMean=spDNSObservedMean, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spReportID=spReportID, peakflowSPCMI=peakflowSPCMI, hardwareFailure=hardwareFailure, spHardwareFailureDescription=spHardwareFailureDescription, autoclassifyStart=autoclassifyStart, spTMSCommunity=spTMSCommunity, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spGreTunnelName=spGreTunnelName, dnsBaseline=dnsBaseline, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spCommFailureDescription=spCommFailureDescription, spInetAddress=spInetAddress, deviceHealth=deviceHealth, spHijackLocal=spHijackLocal, spFlowspecTimeout=spFlowspecTimeout, deviceSystemErrorDone=deviceSystemErrorDone, devicePhysicalMemory=devicePhysicalMemory, dosNetworkProfiledAlert=dosNetworkProfiledAlert, mitigationTMSStart=mitigationTMSStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spLicenseErrType=spLicenseErrType, interfaceUsage=interfaceUsage, flowDown=flowDown, spBlackholePrefix=spBlackholePrefix, spDetector=spDetector, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spFingerprintSender=spFingerprintSender, bgpInstabilityDone=bgpInstabilityDone, spLicenseErrCount=spLicenseErrCount, spInterface=spInterface, serviceUsage=serviceUsage, peakflowSPMIB=peakflowSPMIB, spImpactBps=spImpactBps, mitigationThirdPartyStart=mitigationThirdPartyStart, spTMSTimeout=spTMSTimeout, configChange=configChange, spServiceName=spServiceName, flowUp=flowUp, spInetAddressType=spInetAddressType, spReportName=spReportName, spThirdPartyAddr=spThirdPartyAddr, bgpHijack=bgpHijack, collectorDown=collectorDown, spUnit=spUnit, spDNSObservedMax=spDNSObservedMax, spHijackAttr=spHijackAttr, fingerprintUsageDone=fingerprintUsageDone, fingerprintUsage=fingerprintUsage, fingerprintFeedback=fingerprintFeedback, dosHostDetectionAlertDone=dosHostDetectionAlertDone, greDown=greDown, trafficAutoMitigation=trafficAutoMitigation, greDownDone=greDownDone, spDNSName=spDNSName, PYSNMP_MODULE_ID=peakflowSPMIB, spBGPEvent=spBGPEvent, spDetectedCountries=spDetectedCountries, spTMSPrefix=spTMSPrefix, spAlertID=spAlertID, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapNewAttributes=spBGPTrapNewAttributes, snmpDown=snmpDown, spBGPTrapPrefix=spBGPTrapPrefix, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBGPInstability=spBGPInstability, spDuration=spDuration, deviceSwapSpace=deviceSwapSpace, spThreshold=spThreshold, spImpactPps=spImpactPps, notificationLimit=notificationLimit, managedObjectUsageDone=managedObjectUsageDone, spScriptStart=spScriptStart, spCommFailureDestination=spCommFailureDestination, spInterfaceSpeed=spInterfaceSpeed, spDNSExpected=spDNSExpected, deviceSystemError=deviceSystemError, spInterfaceUsage=spInterfaceUsage, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spScriptHost=spScriptHost)
