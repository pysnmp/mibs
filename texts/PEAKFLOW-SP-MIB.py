...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spImpactPps=spImpactPps, deviceDiskUsage=deviceDiskUsage, spHardwareFailureDescription=spHardwareFailureDescription, deviceSystemErrorDone=deviceSystemErrorDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, cloudSignalingFault=cloudSignalingFault, spMitigationID=spMitigationID, collectorDown=collectorDown, spHijackAttr=spHijackAttr, bgpHijack=bgpHijack, mitigationBlackholeStart=mitigationBlackholeStart, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, bgpDown=bgpDown, notificationLimit=notificationLimit, spFlowspecCommunity=spFlowspecCommunity, greDown=greDown, spUsageType=spUsageType, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spScriptStart=spScriptStart, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spMitigationStart=spMitigationStart, bgpUp=bgpUp, spcommFailureDone=spcommFailureDone, reportDone=reportDone, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spScriptCommand=spScriptCommand, fingerprintUsageDone=fingerprintUsageDone, deviceSystemError=deviceSystemError, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spAlertDetectionSignatures=spAlertDetectionSignatures, spGreTunnelDestination=spGreTunnelDestination, spDNSObservedMax=spDNSObservedMax, flowUp=flowUp, spCollector=spCollector, spTMSTimeout=spTMSTimeout, spBGPTrapNewAttributes=spBGPTrapNewAttributes, flowDown=flowDown, spServiceName=spServiceName, spUsage=spUsage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBlackholeNexthop=spBlackholeNexthop, bgpTrap=bgpTrap, spApplicationName=spApplicationName, dosHostDetectionAlert=dosHostDetectionAlert, spAlertID=spAlertID, spPravail=spPravail, spDetectedCountries=spDetectedCountries, spCommFailureDescription=spCommFailureDescription, spFingerprintMessage=spFingerprintMessage, spSystemErrorType=spSystemErrorType, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spReportURL=spReportURL, spLicenseErrDescription=spLicenseErrDescription, spGreTunnelName=spGreTunnelName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, alertScript=alertScript, deviceTotalFlows=deviceTotalFlows, spInterfaceSpeedHC=spInterfaceSpeedHC, mitigationDone=mitigationDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spHijackRoute=spHijackRoute, spDNSExpected=spDNSExpected, spBlackholeCommunity=spBlackholeCommunity, deviceTotalFlowsHC=deviceTotalFlowsHC, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spScriptPort=spScriptPort, greDownDone=greDownDone, spLicenseErrLimit=spLicenseErrLimit, spHijackLocal=spHijackLocal, spServiceElement=spServiceElement, spFingerprintName=spFingerprintName, spCommFailureDestination=spCommFailureDestination, hardwareFailure=hardwareFailure, mitigationTMSStartV2=mitigationTMSStartV2, spTMSPrefix=spTMSPrefix, spInterface=spInterface, spDNSObserved=spDNSObserved, spcommFailure=spcommFailure, dnsBaselineDone=dnsBaselineDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, bgpInstability=bgpInstability, spReportID=spReportID, managedObjectUsage=managedObjectUsage, peakflowSPTraps=peakflowSPTraps, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spThirdPartyZone=spThirdPartyZone, spInetAddress=spInetAddress, spFingerprintSender=spFingerprintSender, snmpDown=snmpDown, spInetAddressType=spInetAddressType, collectorStart=collectorStart, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInterfaceUsageHC=spInterfaceUsageHC, spBGPEvent=spBGPEvent, spBlackholeTimeout=spBlackholeTimeout, peakflowSPMgr=peakflowSPMgr, spFingerprintFeedback=spFingerprintFeedback, fingerprintFeedback=fingerprintFeedback, licenseError=licenseError, spReportName=spReportName, spTMSCommunity=spTMSCommunity, spBlackholePrefix=spBlackholePrefix, peakflowSPMIB=peakflowSPMIB, autoclassifyStart=autoclassifyStart, spDNSObservedMean=spDNSObservedMean, spInterfaceUsage=spInterfaceUsage, spMitigationName=spMitigationName, spImpactBps=spImpactBps, spFlowspecTimeout=spFlowspecTimeout, dnsBaseline=dnsBaseline, PYSNMP_MODULE_ID=peakflowSPMIB, devicePhysicalMemory=devicePhysicalMemory, interfaceUsageDone=interfaceUsageDone, spManagedObject=spManagedObject, spLicenseErrType=spLicenseErrType, deviceSwapSpace=deviceSwapSpace, spDetector=spDetector, licenseErrorDone=licenseErrorDone, serviceUsage=serviceUsage, fingerprintUsage=fingerprintUsage, spDuration=spDuration, spSystemErrorDescription=spSystemErrorDescription, spUsername=spUsername, spThreshold=spThreshold, mitigationFlowspecStart=mitigationFlowspecStart, spScriptHost=spScriptHost, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spLicenseErrCount=spLicenseErrCount, spThirdPartyAddr=spThirdPartyAddr, spTMSMultiPrefix=spTMSMultiPrefix, collectorUp=collectorUp, trafficAutoMitigation=trafficAutoMitigation, spManagedObjectFamily=spManagedObjectFamily, peakflowSPCMI=peakflowSPCMI, configChange=configChange, spBGPTrapEvent=spBGPTrapEvent, spUnit=spUnit, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, mitigationTMSStart=mitigationTMSStart, spBGPTrapName=spBGPTrapName, spBGPTrapPrefix=spBGPTrapPrefix, spManagedObjects=spManagedObjects, spInterfaceSpeed=spInterfaceSpeed, spDNSName=spDNSName, serviceUsageDone=serviceUsageDone, spRouter=spRouter, fingerprintReceive=fingerprintReceive, spInterfaceIndex=spInterfaceIndex, bgpInstabilityDone=bgpInstabilityDone, interfaceUsage=interfaceUsage, deviceHealth=deviceHealth, snmpUp=snmpUp, managedObjectUsageDone=managedObjectUsageDone, spBGPInstability=spBGPInstability, mitigationThirdPartyStart=mitigationThirdPartyStart, hardwareFailureDone=hardwareFailureDone, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spVersion=spVersion, routingFailover=routingFailover, cloudSignalingFaultDone=cloudSignalingFaultDone)
