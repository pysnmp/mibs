...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spCommFailureDestination=spCommFailureDestination, spDNSObservedMax=spDNSObservedMax, spLicenseErrType=spLicenseErrType, spSystemErrorType=spSystemErrorType, spHijackRoute=spHijackRoute, spHijackAttr=spHijackAttr, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spScriptStart=spScriptStart, fingerprintReceive=fingerprintReceive, spInterfaceSpeedHC=spInterfaceSpeedHC, serviceUsage=serviceUsage, spBlackholeNexthop=spBlackholeNexthop, cloudSignalingFault=cloudSignalingFault, licenseError=licenseError, spLicenseErrCount=spLicenseErrCount, spBlackholeTimeout=spBlackholeTimeout, spApplicationName=spApplicationName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spTMSPrefix=spTMSPrefix, mitigationBlackholeStart=mitigationBlackholeStart, spImpactPps=spImpactPps, bgpTrap=bgpTrap, spServiceName=spServiceName, spcommFailureDone=spcommFailureDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, peakflowSPMIB=peakflowSPMIB, licenseErrorDone=licenseErrorDone, spDNSExpected=spDNSExpected, spTMSMultiPrefix=spTMSMultiPrefix, fingerprintUsage=fingerprintUsage, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spFlowspecCommunity=spFlowspecCommunity, collectorDown=collectorDown, spBGPEvent=spBGPEvent, spFingerprintSender=spFingerprintSender, hardwareFailure=hardwareFailure, greDownDone=greDownDone, autoclassifyStart=autoclassifyStart, spInterfaceUsage=spInterfaceUsage, dnsBaseline=dnsBaseline, bgpDown=bgpDown, spRouter=spRouter, spImpactBps=spImpactBps, bgpInstabilityDone=bgpInstabilityDone, spBGPTrapName=spBGPTrapName, spFingerprintFeedback=spFingerprintFeedback, bgpHijack=bgpHijack, hardwareFailureDone=hardwareFailureDone, spAlertDetectionSignatures=spAlertDetectionSignatures, mitigationThirdPartyStart=mitigationThirdPartyStart, spScriptHost=spScriptHost, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInterface=spInterface, spBGPTrapNewAttributes=spBGPTrapNewAttributes, peakflowSPTraps=peakflowSPTraps, deviceDiskUsage=deviceDiskUsage, peakflowSPCMI=peakflowSPCMI, PYSNMP_MODULE_ID=peakflowSPMIB, spTMSTimeout=spTMSTimeout, spSystemErrorDescription=spSystemErrorDescription, spUsername=spUsername, routingFailover=routingFailover, spFingerprintName=spFingerprintName, deviceHealth=deviceHealth, spVersion=spVersion, spPravail=spPravail, mitigationDone=mitigationDone, spDNSObservedMean=spDNSObservedMean, spThirdPartyAddr=spThirdPartyAddr, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBlackholePrefix=spBlackholePrefix, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spDNSName=spDNSName, spScriptCommand=spScriptCommand, spThirdPartyZone=spThirdPartyZone, spCommFailureDescription=spCommFailureDescription, dosHostDetectionAlert=dosHostDetectionAlert, spServiceElement=spServiceElement, flowDown=flowDown, peakflowSPMgr=peakflowSPMgr, deviceSystemErrorDone=deviceSystemErrorDone, spUnit=spUnit, bgpUp=bgpUp, spMitigationName=spMitigationName, flowUp=flowUp, spReportURL=spReportURL, spFingerprintMessage=spFingerprintMessage, spMitigationID=spMitigationID, spInetAddressType=spInetAddressType, spDetector=spDetector, mitigationTMSStartV2=mitigationTMSStartV2, spHardwareFailureDescription=spHardwareFailureDescription, spDNSObserved=spDNSObserved, bgpInstability=bgpInstability, spGreTunnelName=spGreTunnelName, mitigationFlowspecStart=mitigationFlowspecStart, deviceTotalFlows=deviceTotalFlows, interfaceUsageDone=interfaceUsageDone, devicePhysicalMemory=devicePhysicalMemory, spBGPTrapOldAttributes=spBGPTrapOldAttributes, serviceUsageDone=serviceUsageDone, spTMSCommunity=spTMSCommunity, spBGPInstability=spBGPInstability, cloudSignalingFaultDone=cloudSignalingFaultDone, spInterfaceUsageHC=spInterfaceUsageHC, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spHijackLocal=spHijackLocal, spcommFailure=spcommFailure, greDown=greDown, collectorStart=collectorStart, deviceSwapSpace=deviceSwapSpace, spDetectedCountries=spDetectedCountries, spManagedObjectFamily=spManagedObjectFamily, spUsageType=spUsageType, spBlackholeCommunity=spBlackholeCommunity, spReportID=spReportID, deviceSystemError=deviceSystemError, configChange=configChange, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spInterfaceSpeed=spInterfaceSpeed, trafficAutoMitigation=trafficAutoMitigation, spUsage=spUsage, snmpDown=snmpDown, alertScript=alertScript, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spAlertID=spAlertID, spCollector=spCollector, spInetAddress=spInetAddress, spBGPTrapPrefix=spBGPTrapPrefix, spManagedObjects=spManagedObjects, spFlowspecTimeout=spFlowspecTimeout, deviceSwapSpaceInUse=deviceSwapSpaceInUse, collectorUp=collectorUp, managedObjectUsageDone=managedObjectUsageDone, fingerprintUsageDone=fingerprintUsageDone, dnsBaselineDone=dnsBaselineDone, mitigationTMSStart=mitigationTMSStart, interfaceUsage=interfaceUsage, spDuration=spDuration, spManagedObject=spManagedObject, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, deviceTotalFlowsHC=deviceTotalFlowsHC, spGreTunnelDestination=spGreTunnelDestination, fingerprintFeedback=fingerprintFeedback, spReportName=spReportName, snmpUp=snmpUp, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, notificationLimit=notificationLimit, spMitigationStart=spMitigationStart, spThreshold=spThreshold, spLicenseErrLimit=spLicenseErrLimit, spInterfaceIndex=spInterfaceIndex, reportDone=reportDone, managedObjectUsage=managedObjectUsage, spLicenseErrDescription=spLicenseErrDescription, spBGPTrapEvent=spBGPTrapEvent, spScriptPort=spScriptPort)
