...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceSwapSpace=deviceSwapSpace, mitigationThirdPartyStart=mitigationThirdPartyStart, bgpHijack=bgpHijack, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spDetectedCountries=spDetectedCountries, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInterfaceSpeed=spInterfaceSpeed, devicePhysicalMemory=devicePhysicalMemory, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, reportDone=reportDone, spApplicationName=spApplicationName, spLicenseErrType=spLicenseErrType, deviceTotalFlowsHC=deviceTotalFlowsHC, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spManagedObject=spManagedObject, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spThirdPartyAddr=spThirdPartyAddr, configChange=configChange, spFingerprintFeedback=spFingerprintFeedback, spHijackLocal=spHijackLocal, spFlowspecTimeout=spFlowspecTimeout, licenseError=licenseError, fingerprintUsage=fingerprintUsage, spSystemErrorType=spSystemErrorType, spFingerprintMessage=spFingerprintMessage, spBlackholeNexthop=spBlackholeNexthop, spServiceName=spServiceName, dnsBaseline=dnsBaseline, dosHostDetectionAlertDone=dosHostDetectionAlertDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spDNSExpected=spDNSExpected, spLicenseErrDescription=spLicenseErrDescription, serviceUsage=serviceUsage, fingerprintReceive=fingerprintReceive, spImpactBps=spImpactBps, spPravail=spPravail, spCommFailureDescription=spCommFailureDescription, interfaceUsageDone=interfaceUsageDone, greDown=greDown, bgpTrap=bgpTrap, spDetector=spDetector, spMitigationStart=spMitigationStart, deviceTotalFlows=deviceTotalFlows, spCollector=spCollector, spDNSObservedMean=spDNSObservedMean, bgpUp=bgpUp, dosNetworkProfiledAlert=dosNetworkProfiledAlert, bgpInstability=bgpInstability, spDNSName=spDNSName, spInterface=spInterface, spTMSCommunity=spTMSCommunity, flowDown=flowDown, spInetAddressType=spInetAddressType, snmpDown=snmpDown, spBGPEvent=spBGPEvent, spUnit=spUnit, deviceSystemErrorDone=deviceSystemErrorDone, spBGPTrapPrefix=spBGPTrapPrefix, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spcommFailure=spcommFailure, peakflowSPMgr=peakflowSPMgr, spScriptCommand=spScriptCommand, spMitigationID=spMitigationID, bgpInstabilityDone=bgpInstabilityDone, deviceSystemError=deviceSystemError, managedObjectUsageDone=managedObjectUsageDone, dosHostDetectionAlert=dosHostDetectionAlert, spInterfaceUsageHC=spInterfaceUsageHC, spHijackRoute=spHijackRoute, spBGPInstability=spBGPInstability, hardwareFailureDone=hardwareFailureDone, spVersion=spVersion, spBGPTrapEvent=spBGPTrapEvent, spMitigationName=spMitigationName, spThirdPartyZone=spThirdPartyZone, deviceHealth=deviceHealth, spSystemErrorDescription=spSystemErrorDescription, mitigationBlackholeStart=mitigationBlackholeStart, interfaceUsage=interfaceUsage, autoclassifyStart=autoclassifyStart, mitigationDone=mitigationDone, peakflowSPTraps=peakflowSPTraps, managedObjectUsage=managedObjectUsage, spReportName=spReportName, spLicenseErrLimit=spLicenseErrLimit, spTMSMultiPrefix=spTMSMultiPrefix, peakflowSPCMI=peakflowSPCMI, spManagedObjectFamily=spManagedObjectFamily, spFingerprintName=spFingerprintName, notificationLimit=notificationLimit, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spCommFailureDestination=spCommFailureDestination, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spBlackholeCommunity=spBlackholeCommunity, spFingerprintSender=spFingerprintSender, spBlackholeTimeout=spBlackholeTimeout, spDNSObserved=spDNSObserved, mitigationTMSStart=mitigationTMSStart, hardwareFailure=hardwareFailure, spServiceElement=spServiceElement, spAlertID=spAlertID, flowUp=flowUp, snmpUp=snmpUp, trafficAutoMitigation=trafficAutoMitigation, mitigationTMSStartV2=mitigationTMSStartV2, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, collectorUp=collectorUp, spScriptHost=spScriptHost, alertScript=alertScript, spAlertDetectionSignatures=spAlertDetectionSignatures, spReportURL=spReportURL, cloudSignalingFaultDone=cloudSignalingFaultDone, spReportID=spReportID, spTMSTimeout=spTMSTimeout, spScriptStart=spScriptStart, serviceUsageDone=serviceUsageDone, spImpactPps=spImpactPps, dnsBaselineDone=dnsBaselineDone, spInterfaceUsage=spInterfaceUsage, spFlowspecCommunity=spFlowspecCommunity, spInetAddress=spInetAddress, licenseErrorDone=licenseErrorDone, spDuration=spDuration, spBGPTrapOldAttributes=spBGPTrapOldAttributes, mitigationFlowspecStart=mitigationFlowspecStart, spBGPTrapName=spBGPTrapName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spHijackAttr=spHijackAttr, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, deviceDiskUsage=deviceDiskUsage, spThreshold=spThreshold, routingFailover=routingFailover, spBlackholePrefix=spBlackholePrefix, spGreTunnelName=spGreTunnelName, spcommFailureDone=spcommFailureDone, PYSNMP_MODULE_ID=peakflowSPMIB, spUsage=spUsage, spRouter=spRouter, collectorDown=collectorDown, spInterfaceIndex=spInterfaceIndex, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spTMSPrefix=spTMSPrefix, cloudSignalingFault=cloudSignalingFault, spUsageType=spUsageType, spHardwareFailureDescription=spHardwareFailureDescription, spScriptPort=spScriptPort, greDownDone=greDownDone, fingerprintUsageDone=fingerprintUsageDone, spUsername=spUsername, spManagedObjects=spManagedObjects, collectorStart=collectorStart, fingerprintFeedback=fingerprintFeedback, peakflowSPMIB=peakflowSPMIB, spLicenseErrCount=spLicenseErrCount, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spGreTunnelDestination=spGreTunnelDestination, spInterfaceSpeedHC=spInterfaceSpeedHC, spDNSObservedMax=spDNSObservedMax, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, bgpDown=bgpDown, deviceSwapSpaceUsage=deviceSwapSpaceUsage)
