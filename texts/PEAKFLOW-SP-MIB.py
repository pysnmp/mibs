...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", deviceSwapSpaceInUse=deviceSwapSpaceInUse, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, routingFailover=routingFailover, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spBGPEvent=spBGPEvent, spcommFailure=spcommFailure, spDNSObservedMean=spDNSObservedMean, spMitigationStart=spMitigationStart, fingerprintReceive=fingerprintReceive, spInetAddress=spInetAddress, alertScript=alertScript, greDownDone=greDownDone, spServiceElement=spServiceElement, peakflowSPTraps=peakflowSPTraps, deviceSwapSpace=deviceSwapSpace, spLicenseErrCount=spLicenseErrCount, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, serviceUsage=serviceUsage, managedObjectUsage=managedObjectUsage, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, mitigationDone=mitigationDone, spThirdPartyAddr=spThirdPartyAddr, spSystemErrorDescription=spSystemErrorDescription, spDNSObserved=spDNSObserved, deviceSystemError=deviceSystemError, hardwareFailure=hardwareFailure, dnsBaseline=dnsBaseline, spReportName=spReportName, interfaceUsage=interfaceUsage, autoclassifyStart=autoclassifyStart, flowUp=flowUp, deviceTotalFlowsHC=deviceTotalFlowsHC, spReportURL=spReportURL, dosHostDetectionAlert=dosHostDetectionAlert, bgpDown=bgpDown, spDetector=spDetector, configChange=configChange, snmpUp=snmpUp, spLicenseErrDescription=spLicenseErrDescription, spPravail=spPravail, spCommFailureDescription=spCommFailureDescription, devicePhysicalMemory=devicePhysicalMemory, spBlackholeNexthop=spBlackholeNexthop, fingerprintUsage=fingerprintUsage, spBlackholeTimeout=spBlackholeTimeout, spFingerprintFeedback=spFingerprintFeedback, managedObjectUsageDone=managedObjectUsageDone, spDetectedCountries=spDetectedCountries, bgpUp=bgpUp, spInterface=spInterface, deviceSystemErrorDone=deviceSystemErrorDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSName=spDNSName, spTMSPrefix=spTMSPrefix, greDown=greDown, peakflowSPCMI=peakflowSPCMI, spFingerprintSender=spFingerprintSender, spLicenseErrType=spLicenseErrType, spFingerprintMessage=spFingerprintMessage, spThreshold=spThreshold, spInterfaceIndex=spInterfaceIndex, mitigationBlackholeStart=mitigationBlackholeStart, spRouter=spRouter, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, trafficAutoMitigation=trafficAutoMitigation, spInetAddressType=spInetAddressType, deviceHealth=deviceHealth, spFingerprintName=spFingerprintName, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, hardwareFailureDone=hardwareFailureDone, spHardwareFailureDescription=spHardwareFailureDescription, spManagedObjects=spManagedObjects, spInterfaceSpeedHC=spInterfaceSpeedHC, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, peakflowSPMgr=peakflowSPMgr, spCollector=spCollector, spScriptHost=spScriptHost, cloudSignalingFault=cloudSignalingFault, spMitigationID=spMitigationID, spManagedObjectFamily=spManagedObjectFamily, bgpTrap=bgpTrap, dnsBaselineDone=dnsBaselineDone, collectorUp=collectorUp, spDNSExpected=spDNSExpected, spVersion=spVersion, serviceUsageDone=serviceUsageDone, spDNSObservedMax=spDNSObservedMax, licenseErrorDone=licenseErrorDone, spBGPTrapEvent=spBGPTrapEvent, cloudSignalingFaultDone=cloudSignalingFaultDone, spApplicationName=spApplicationName, spUnit=spUnit, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spAlertDetectionSignatures=spAlertDetectionSignatures, mitigationTMSStartV2=mitigationTMSStartV2, spBGPTrapPrefix=spBGPTrapPrefix, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spUsername=spUsername, spScriptCommand=spScriptCommand, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPInstability=spBGPInstability, spDuration=spDuration, spBlackholePrefix=spBlackholePrefix, spGreTunnelDestination=spGreTunnelDestination, spServiceName=spServiceName, spThirdPartyZone=spThirdPartyZone, peakflowSPMIB=peakflowSPMIB, spcommFailureDone=spcommFailureDone, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spSystemErrorType=spSystemErrorType, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, collectorDown=collectorDown, spFlowspecCommunity=spFlowspecCommunity, spInterfaceSpeed=spInterfaceSpeed, PYSNMP_MODULE_ID=peakflowSPMIB, spTMSCommunity=spTMSCommunity, spFlowspecTimeout=spFlowspecTimeout, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, snmpDown=snmpDown, fingerprintFeedback=fingerprintFeedback, bgpInstability=bgpInstability, spCommFailureDestination=spCommFailureDestination, spAlertID=spAlertID, spHijackRoute=spHijackRoute, spMitigationName=spMitigationName, spHijackLocal=spHijackLocal, fingerprintUsageDone=fingerprintUsageDone, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spScriptStart=spScriptStart, interfaceUsageDone=interfaceUsageDone, licenseError=licenseError, spHijackAttr=spHijackAttr, spReportID=spReportID, spManagedObject=spManagedObject, spBGPTrapName=spBGPTrapName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spTMSMultiPrefix=spTMSMultiPrefix, spInterfaceUsage=spInterfaceUsage, spLicenseErrLimit=spLicenseErrLimit, bgpInstabilityDone=bgpInstabilityDone, notificationLimit=notificationLimit, reportDone=reportDone, deviceDiskUsage=deviceDiskUsage, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, collectorStart=collectorStart, spUsageType=spUsageType, deviceTotalFlows=deviceTotalFlows, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spInterfaceUsageHC=spInterfaceUsageHC, spBlackholeCommunity=spBlackholeCommunity, flowDown=flowDown, spTMSTimeout=spTMSTimeout, mitigationFlowspecStart=mitigationFlowspecStart, spImpactPps=spImpactPps, spGreTunnelName=spGreTunnelName, spUsage=spUsage, dosNetworkProfiledAlert=dosNetworkProfiledAlert, mitigationTMSStart=mitigationTMSStart, spScriptPort=spScriptPort, spImpactBps=spImpactBps, bgpHijack=bgpHijack)
