...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spReportID=spReportID, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spInterfaceUsage=spInterfaceUsage, spDNSExpected=spDNSExpected, peakflowSPTraps=peakflowSPTraps, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spBlackholeCommunity=spBlackholeCommunity, cloudSignalingFaultDone=cloudSignalingFaultDone, spRouter=spRouter, spBGPInstability=spBGPInstability, dnsBaselineDone=dnsBaselineDone, routingFailover=routingFailover, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spInterfaceIndex=spInterfaceIndex, flowDown=flowDown, spImpactPps=spImpactPps, spInterfaceSpeed=spInterfaceSpeed, spVersion=spVersion, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spDNSObservedMean=spDNSObservedMean, bgpTrap=bgpTrap, spInterfaceSpeedHC=spInterfaceSpeedHC, mitigationTMSStartV2=mitigationTMSStartV2, spScriptPort=spScriptPort, spBGPTrapEvent=spBGPTrapEvent, spMitigationStart=spMitigationStart, interfaceUsageDone=interfaceUsageDone, peakflowSPMIB=peakflowSPMIB, spAlertID=spAlertID, spHijackAttr=spHijackAttr, spFingerprintMessage=spFingerprintMessage, spTMSMultiPrefix=spTMSMultiPrefix, spInterface=spInterface, spDNSObserved=spDNSObserved, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spcommFailure=spcommFailure, serviceUsageDone=serviceUsageDone, spFlowspecCommunity=spFlowspecCommunity, devicePhysicalMemory=devicePhysicalMemory, spApplicationName=spApplicationName, mitigationBlackholeStart=mitigationBlackholeStart, spUsage=spUsage, spFingerprintFeedback=spFingerprintFeedback, dosNetworkProfiledAlert=dosNetworkProfiledAlert, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, trafficAutoMitigation=trafficAutoMitigation, deviceTotalFlows=deviceTotalFlows, spUsername=spUsername, spLicenseErrLimit=spLicenseErrLimit, spCloudSignalFaultDescription=spCloudSignalFaultDescription, deviceSwapSpace=deviceSwapSpace, greDown=greDown, interfaceUsage=interfaceUsage, spInetAddress=spInetAddress, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFlowspecTimeout=spFlowspecTimeout, spHardwareFailureDescription=spHardwareFailureDescription, spServiceElement=spServiceElement, spDNSObservedMax=spDNSObservedMax, deviceDiskUsage=deviceDiskUsage, deviceTotalFlowsHC=deviceTotalFlowsHC, flowUp=flowUp, spFingerprintName=spFingerprintName, alertScript=alertScript, spBlackholeNexthop=spBlackholeNexthop, mitigationThirdPartyStart=mitigationThirdPartyStart, spAlertDetectionSignatures=spAlertDetectionSignatures, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, dosHostDetectionAlertDone=dosHostDetectionAlertDone, peakflowSPMgr=peakflowSPMgr, dosHostDetectionAlert=dosHostDetectionAlert, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, mitigationDone=mitigationDone, spTMSCommunity=spTMSCommunity, spManagedObjects=spManagedObjects, deviceSwapSpaceInUse=deviceSwapSpaceInUse, peakflowSPCMI=peakflowSPCMI, bgpDown=bgpDown, spBGPEvent=spBGPEvent, greDownDone=greDownDone, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spLicenseErrCount=spLicenseErrCount, spGreTunnelDestination=spGreTunnelDestination, spTMSPrefix=spTMSPrefix, spCommFailureDescription=spCommFailureDescription, dnsBaseline=dnsBaseline, mitigationTMSStart=mitigationTMSStart, bgpInstabilityDone=bgpInstabilityDone, spDetector=spDetector, collectorStart=collectorStart, managedObjectUsageDone=managedObjectUsageDone, spTMSTimeout=spTMSTimeout, snmpDown=snmpDown, spMitigationName=spMitigationName, spMitigationID=spMitigationID, spFingerprintSender=spFingerprintSender, spcommFailureDone=spcommFailureDone, fingerprintUsageDone=fingerprintUsageDone, spLicenseErrDescription=spLicenseErrDescription, spScriptCommand=spScriptCommand, fingerprintReceive=fingerprintReceive, reportDone=reportDone, fingerprintFeedback=fingerprintFeedback, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spPravail=spPravail, deviceSystemErrorDone=deviceSystemErrorDone, hardwareFailure=hardwareFailure, bgpInstability=bgpInstability, deviceSwapSpaceUsage=deviceSwapSpaceUsage, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, collectorDown=collectorDown, mitigationFlowspecStart=mitigationFlowspecStart, spUnit=spUnit, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spManagedObject=spManagedObject, licenseErrorDone=licenseErrorDone, spCommFailureDestination=spCommFailureDestination, spReportURL=spReportURL, spScriptHost=spScriptHost, spGreTunnelName=spGreTunnelName, spDetectedCountries=spDetectedCountries, deviceHealth=deviceHealth, spSystemErrorType=spSystemErrorType, spImpactBps=spImpactBps, spThirdPartyZone=spThirdPartyZone, spServiceName=spServiceName, spDuration=spDuration, spScriptStart=spScriptStart, serviceUsage=serviceUsage, spSystemErrorDescription=spSystemErrorDescription, spCollector=spCollector, notificationLimit=notificationLimit, deviceSystemError=deviceSystemError, spManagedObjectFamily=spManagedObjectFamily, spHijackLocal=spHijackLocal, spThirdPartyAddr=spThirdPartyAddr, fingerprintUsage=fingerprintUsage, autoclassifyStart=autoclassifyStart, licenseError=licenseError, spBlackholeTimeout=spBlackholeTimeout, bgpHijack=bgpHijack, spHijackRoute=spHijackRoute, spReportName=spReportName, configChange=configChange, spInterfaceUsageHC=spInterfaceUsageHC, spBGPTrapName=spBGPTrapName, hardwareFailureDone=hardwareFailureDone, spLicenseErrType=spLicenseErrType, managedObjectUsage=managedObjectUsage, spDNSName=spDNSName, spInetAddressType=spInetAddressType, collectorUp=collectorUp, cloudSignalingFault=cloudSignalingFault, spBlackholePrefix=spBlackholePrefix, snmpUp=snmpUp, spUsageType=spUsageType, bgpUp=bgpUp, spBGPTrapPrefix=spBGPTrapPrefix, spThreshold=spThreshold, PYSNMP_MODULE_ID=peakflowSPMIB)
