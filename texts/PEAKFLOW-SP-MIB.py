...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDNSName=spDNSName, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, trafficAutoMitigation=trafficAutoMitigation, spBGPInstability=spBGPInstability, mitigationTMSStartV2=mitigationTMSStartV2, dosNetworkProfiledAlert=dosNetworkProfiledAlert, serviceUsageDone=serviceUsageDone, interfaceUsageDone=interfaceUsageDone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spFingerprintFeedback=spFingerprintFeedback, spHijackLocal=spHijackLocal, snmpDown=snmpDown, bgpInstability=bgpInstability, deviceHealth=deviceHealth, spFlowspecTimeout=spFlowspecTimeout, bgpTrap=bgpTrap, spTMSTimeout=spTMSTimeout, spTMSMultiPrefix=spTMSMultiPrefix, greDownDone=greDownDone, spLicenseErrCount=spLicenseErrCount, devicePhysicalMemory=devicePhysicalMemory, snmpUp=snmpUp, mitigationBlackholeStart=mitigationBlackholeStart, spReportName=spReportName, spInterfaceSpeedHC=spInterfaceSpeedHC, notificationLimit=notificationLimit, spVersion=spVersion, spThirdPartyZone=spThirdPartyZone, bgpDown=bgpDown, spScriptHost=spScriptHost, spCollector=spCollector, spRouter=spRouter, managedObjectUsageDone=managedObjectUsageDone, deviceSystemError=deviceSystemError, hardwareFailure=hardwareFailure, spInterfaceIndex=spInterfaceIndex, spMitigationID=spMitigationID, spHijackRoute=spHijackRoute, spBGPTrapName=spBGPTrapName, spTMSCommunity=spTMSCommunity, fingerprintUsageDone=fingerprintUsageDone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spDNSExpected=spDNSExpected, spInterfaceUsage=spInterfaceUsage, flowDown=flowDown, deviceSwapSpace=deviceSwapSpace, spReportURL=spReportURL, spCommFailureDestination=spCommFailureDestination, spScriptStart=spScriptStart, spcommFailureDone=spcommFailureDone, spFlowspecCommunity=spFlowspecCommunity, spGreTunnelDestination=spGreTunnelDestination, deviceDiskUsage=deviceDiskUsage, collectorDown=collectorDown, collectorStart=collectorStart, dnsBaseline=dnsBaseline, bgpHijack=bgpHijack, spUsername=spUsername, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spInterfaceSpeed=spInterfaceSpeed, spScriptPort=spScriptPort, spLicenseErrLimit=spLicenseErrLimit, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBlackholeTimeout=spBlackholeTimeout, deviceSystemErrorDone=deviceSystemErrorDone, fingerprintUsage=fingerprintUsage, spManagedObjects=spManagedObjects, spBlackholeCommunity=spBlackholeCommunity, mitigationFlowspecStart=mitigationFlowspecStart, spImpactPps=spImpactPps, spServiceName=spServiceName, fingerprintReceive=fingerprintReceive, spUsageType=spUsageType, spPravail=spPravail, spFingerprintMessage=spFingerprintMessage, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, mitigationDone=mitigationDone, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, peakflowSPCMI=peakflowSPCMI, managedObjectUsage=managedObjectUsage, fingerprintFeedback=fingerprintFeedback, routingFailover=routingFailover, reportDone=reportDone, dosHostDetectionAlert=dosHostDetectionAlert, spManagedObjectFamily=spManagedObjectFamily, spBGPTrapPrefix=spBGPTrapPrefix, spThreshold=spThreshold, spMitigationName=spMitigationName, hardwareFailureDone=hardwareFailureDone, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, flowUp=flowUp, serviceUsage=serviceUsage, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spFingerprintSender=spFingerprintSender, cloudSignalingFault=cloudSignalingFault, dnsBaselineDone=dnsBaselineDone, spTMSPrefix=spTMSPrefix, spBlackholeNexthop=spBlackholeNexthop, licenseError=licenseError, licenseErrorDone=licenseErrorDone, spLicenseErrDescription=spLicenseErrDescription, spCommFailureDescription=spCommFailureDescription, spDetector=spDetector, bgpInstabilityDone=bgpInstabilityDone, spDuration=spDuration, spUnit=spUnit, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spServiceElement=spServiceElement, peakflowSPMIB=peakflowSPMIB, spApplicationName=spApplicationName, spAlertID=spAlertID, spDNSObservedMean=spDNSObservedMean, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, deviceTotalFlowsHC=deviceTotalFlowsHC, peakflowSPMgr=peakflowSPMgr, spInterface=spInterface, spCloudSignalFaultDescription=spCloudSignalFaultDescription, interfaceUsage=interfaceUsage, bgpUp=bgpUp, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spScriptCommand=spScriptCommand, greDown=greDown, dosHostDetectionAlertDone=dosHostDetectionAlertDone, configChange=configChange, spBGPTrapEvent=spBGPTrapEvent, spBGPEvent=spBGPEvent, spSystemErrorType=spSystemErrorType, spHijackAttr=spHijackAttr, PYSNMP_MODULE_ID=peakflowSPMIB, spAlertDetectionSignatures=spAlertDetectionSignatures, spBlackholePrefix=spBlackholePrefix, deviceSwapSpaceInUse=deviceSwapSpaceInUse, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spcommFailure=spcommFailure, spGreTunnelName=spGreTunnelName, spDNSObservedMax=spDNSObservedMax, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, alertScript=alertScript, spThirdPartyAddr=spThirdPartyAddr, spReportID=spReportID, spSystemErrorDescription=spSystemErrorDescription, peakflowSPTraps=peakflowSPTraps, spLicenseErrType=spLicenseErrType, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, collectorUp=collectorUp, spDetectedCountries=spDetectedCountries, spImpactBps=spImpactBps, spManagedObject=spManagedObject, spHardwareFailureDescription=spHardwareFailureDescription, spInetAddress=spInetAddress, spUsage=spUsage, spInterfaceUsageHC=spInterfaceUsageHC, cloudSignalingFaultDone=cloudSignalingFaultDone, spMitigationStart=spMitigationStart, deviceTotalFlows=deviceTotalFlows, spDNSObserved=spDNSObserved, spInetAddressType=spInetAddressType, mitigationTMSStart=mitigationTMSStart, spFingerprintName=spFingerprintName, autoclassifyStart=autoclassifyStart)
