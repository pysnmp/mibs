...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spInterfaceSpeedHC=spInterfaceSpeedHC, spTMSTimeout=spTMSTimeout, licenseError=licenseError, spInterface=spInterface, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spTMSPrefix=spTMSPrefix, spReportName=spReportName, spSystemErrorType=spSystemErrorType, spThreshold=spThreshold, cloudSignalingFaultDone=cloudSignalingFaultDone, mitigationDone=mitigationDone, spServiceName=spServiceName, cloudSignalingFault=cloudSignalingFault, spManagedObjects=spManagedObjects, autoclassifyStart=autoclassifyStart, spImpactPps=spImpactPps, spFingerprintFeedback=spFingerprintFeedback, reportDone=reportDone, spBlackholeTimeout=spBlackholeTimeout, spMitigationID=spMitigationID, spDetector=spDetector, spBlackholeNexthop=spBlackholeNexthop, fingerprintFeedback=fingerprintFeedback, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spCommFailureDescription=spCommFailureDescription, spFingerprintName=spFingerprintName, spDetectedCountries=spDetectedCountries, interfaceUsageDone=interfaceUsageDone, devicePhysicalMemory=devicePhysicalMemory, serviceUsageDone=serviceUsageDone, trafficAutoMitigation=trafficAutoMitigation, spInterfaceIndex=spInterfaceIndex, deviceSwapSpace=deviceSwapSpace, routingFailover=routingFailover, spScriptPort=spScriptPort, spLicenseErrCount=spLicenseErrCount, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, greDown=greDown, deviceHealth=deviceHealth, spInterfaceUsageHC=spInterfaceUsageHC, spCommFailureDestination=spCommFailureDestination, flowDown=flowDown, spPravail=spPravail, spInetAddress=spInetAddress, spLicenseErrLimit=spLicenseErrLimit, spReportID=spReportID, spBGPTrapPrefix=spBGPTrapPrefix, spFingerprintMessage=spFingerprintMessage, flowUp=flowUp, spFlowspecTimeout=spFlowspecTimeout, interfaceUsage=interfaceUsage, spRouter=spRouter, spFingerprintSender=spFingerprintSender, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, bgpHijack=bgpHijack, spUsage=spUsage, spBGPTrapName=spBGPTrapName, spGreTunnelName=spGreTunnelName, spAlertDetectionSignatures=spAlertDetectionSignatures, deviceSystemError=deviceSystemError, spInterfaceSpeed=spInterfaceSpeed, spcommFailureDone=spcommFailureDone, spGreTunnelDestination=spGreTunnelDestination, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spBGPTrapEvent=spBGPTrapEvent, spDNSObservedMean=spDNSObservedMean, spCloudSignalFaultDescription=spCloudSignalFaultDescription, dnsBaseline=dnsBaseline, spLicenseErrType=spLicenseErrType, mitigationTMSStartV2=mitigationTMSStartV2, snmpDown=snmpDown, spDNSExpected=spDNSExpected, spApplicationName=spApplicationName, spReportURL=spReportURL, mitigationFlowspecStart=mitigationFlowspecStart, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, peakflowSPMIB=peakflowSPMIB, deviceTotalFlows=deviceTotalFlows, mitigationThirdPartyStart=mitigationThirdPartyStart, fingerprintReceive=fingerprintReceive, configChange=configChange, spDNSObserved=spDNSObserved, spThirdPartyZone=spThirdPartyZone, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spUnit=spUnit, spMitigationName=spMitigationName, spSystemErrorDescription=spSystemErrorDescription, spTMSMultiPrefix=spTMSMultiPrefix, spHijackRoute=spHijackRoute, spBGPInstability=spBGPInstability, spHijackLocal=spHijackLocal, spBGPTrapNewAttributes=spBGPTrapNewAttributes, bgpInstabilityDone=bgpInstabilityDone, spBlackholeCommunity=spBlackholeCommunity, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, deviceSystemErrorDone=deviceSystemErrorDone, spFlowspecCommunity=spFlowspecCommunity, spImpactBps=spImpactBps, licenseErrorDone=licenseErrorDone, spVersion=spVersion, deviceSwapSpaceUsage=deviceSwapSpaceUsage, bgpDown=bgpDown, collectorUp=collectorUp, spThirdPartyAddr=spThirdPartyAddr, spMitigationStart=spMitigationStart, spTMSCommunity=spTMSCommunity, spUsername=spUsername, spScriptStart=spScriptStart, dosNetworkProfiledAlert=dosNetworkProfiledAlert, bgpTrap=bgpTrap, spCollector=spCollector, spBGPEvent=spBGPEvent, deviceDiskUsage=deviceDiskUsage, spScriptCommand=spScriptCommand, spUsageType=spUsageType, spInetAddressType=spInetAddressType, peakflowSPTraps=peakflowSPTraps, bgpUp=bgpUp, mitigationBlackholeStart=mitigationBlackholeStart, fingerprintUsage=fingerprintUsage, managedObjectUsage=managedObjectUsage, spLicenseErrDescription=spLicenseErrDescription, snmpUp=snmpUp, dosHostDetectionAlertDone=dosHostDetectionAlertDone, fingerprintUsageDone=fingerprintUsageDone, spAlertID=spAlertID, collectorDown=collectorDown, PYSNMP_MODULE_ID=peakflowSPMIB, spDNSObservedMax=spDNSObservedMax, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spScriptHost=spScriptHost, spHijackAttr=spHijackAttr, peakflowSPCMI=peakflowSPCMI, spManagedObjectFamily=spManagedObjectFamily, spManagedObject=spManagedObject, deviceTotalFlowsHC=deviceTotalFlowsHC, dosHostDetectionAlert=dosHostDetectionAlert, peakflowSPMgr=peakflowSPMgr, spDNSName=spDNSName, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spInterfaceUsage=spInterfaceUsage, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, collectorStart=collectorStart, mitigationTMSStart=mitigationTMSStart, spBlackholePrefix=spBlackholePrefix, notificationLimit=notificationLimit, dnsBaselineDone=dnsBaselineDone, alertScript=alertScript, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, managedObjectUsageDone=managedObjectUsageDone, spcommFailure=spcommFailure, hardwareFailureDone=hardwareFailureDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, bgpInstability=bgpInstability, spDuration=spDuration, greDownDone=greDownDone, spServiceElement=spServiceElement, hardwareFailure=hardwareFailure, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, serviceUsage=serviceUsage, spHardwareFailureDescription=spHardwareFailureDescription)
