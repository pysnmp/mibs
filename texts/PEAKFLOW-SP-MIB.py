...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDNSObserved=spDNSObserved, spManagedObjectFamily=spManagedObjectFamily, spDuration=spDuration, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spImpactPps=spImpactPps, flowUp=flowUp, bgpInstability=bgpInstability, deviceSwapSpace=deviceSwapSpace, peakflowSPMIB=peakflowSPMIB, spRouter=spRouter, spLicenseErrDescription=spLicenseErrDescription, spSystemErrorType=spSystemErrorType, spInterfaceIndex=spInterfaceIndex, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spHardwareFailureDescription=spHardwareFailureDescription, dnsBaseline=dnsBaseline, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spFingerprintName=spFingerprintName, spTMSTimeout=spTMSTimeout, spInterface=spInterface, serviceUsageDone=serviceUsageDone, spBlackholeNexthop=spBlackholeNexthop, notificationLimit=notificationLimit, mitigationBlackholeStart=mitigationBlackholeStart, spBlackholeCommunity=spBlackholeCommunity, snmpUp=snmpUp, spScriptPort=spScriptPort, spInetAddressType=spInetAddressType, greDownDone=greDownDone, fingerprintReceive=fingerprintReceive, deviceSystemError=deviceSystemError, cloudSignalingFaultDone=cloudSignalingFaultDone, fingerprintUsageDone=fingerprintUsageDone, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spCollector=spCollector, collectorStart=collectorStart, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, mitigationFlowspecStart=mitigationFlowspecStart, spThirdPartyZone=spThirdPartyZone, managedObjectUsageDone=managedObjectUsageDone, bgpTrap=bgpTrap, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceTotalFlowsHC=deviceTotalFlowsHC, spGreTunnelName=spGreTunnelName, mitigationTMSStart=mitigationTMSStart, mitigationTMSStartV2=mitigationTMSStartV2, spLicenseErrType=spLicenseErrType, fingerprintUsage=fingerprintUsage, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, snmpDown=snmpDown, spFlowspecTimeout=spFlowspecTimeout, bgpHijack=bgpHijack, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spBlackholeTimeout=spBlackholeTimeout, licenseErrorDone=licenseErrorDone, spReportID=spReportID, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spInetAddress=spInetAddress, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spMitigationStart=spMitigationStart, spUsageType=spUsageType, deviceTotalFlows=deviceTotalFlows, spServiceElement=spServiceElement, spPravail=spPravail, alertScript=alertScript, licenseError=licenseError, spTMSMultiPrefix=spTMSMultiPrefix, spInterfaceUsage=spInterfaceUsage, spBGPInstability=spBGPInstability, peakflowSPMgr=peakflowSPMgr, spUsername=spUsername, spDetectedCountries=spDetectedCountries, peakflowSPCMI=peakflowSPCMI, autoclassifyStart=autoclassifyStart, spLicenseErrLimit=spLicenseErrLimit, managedObjectUsage=managedObjectUsage, dnsBaselineDone=dnsBaselineDone, spHijackAttr=spHijackAttr, spHijackRoute=spHijackRoute, spVersion=spVersion, spLicenseErrCount=spLicenseErrCount, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spBGPEvent=spBGPEvent, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spInterfaceSpeedHC=spInterfaceSpeedHC, bgpUp=bgpUp, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, cloudSignalingFault=cloudSignalingFault, spApplicationName=spApplicationName, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spBlackholePrefix=spBlackholePrefix, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, peakflowSPTraps=peakflowSPTraps, bgpDown=bgpDown, spInterfaceSpeed=spInterfaceSpeed, dosHostDetectionAlert=dosHostDetectionAlert, spDNSObservedMean=spDNSObservedMean, spImpactBps=spImpactBps, spDetector=spDetector, spCommFailureDescription=spCommFailureDescription, spHijackLocal=spHijackLocal, flowDown=flowDown, spScriptStart=spScriptStart, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spUnit=spUnit, interfaceUsage=interfaceUsage, interfaceUsageDone=interfaceUsageDone, spServiceName=spServiceName, spBGPTrapName=spBGPTrapName, spAlertID=spAlertID, trafficAutoMitigation=trafficAutoMitigation, spDNSExpected=spDNSExpected, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spFingerprintMessage=spFingerprintMessage, deviceSystemErrorDone=deviceSystemErrorDone, spDNSName=spDNSName, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDNSObservedMax=spDNSObservedMax, spMitigationID=spMitigationID, spManagedObjects=spManagedObjects, spScriptCommand=spScriptCommand, routingFailover=routingFailover, spThreshold=spThreshold, spSystemErrorDescription=spSystemErrorDescription, PYSNMP_MODULE_ID=peakflowSPMIB, deviceDiskUsage=deviceDiskUsage, mitigationDone=mitigationDone, hardwareFailure=hardwareFailure, collectorDown=collectorDown, spBGPTrapPrefix=spBGPTrapPrefix, spReportURL=spReportURL, fingerprintFeedback=fingerprintFeedback, spManagedObject=spManagedObject, spGreTunnelDestination=spGreTunnelDestination, spFingerprintSender=spFingerprintSender, spInterfaceUsageHC=spInterfaceUsageHC, bgpInstabilityDone=bgpInstabilityDone, spcommFailureDone=spcommFailureDone, spUsage=spUsage, devicePhysicalMemory=devicePhysicalMemory, greDown=greDown, serviceUsage=serviceUsage, spThirdPartyAddr=spThirdPartyAddr, spcommFailure=spcommFailure, spReportName=spReportName, spTMSCommunity=spTMSCommunity, deviceHealth=deviceHealth, spFlowspecCommunity=spFlowspecCommunity, spBGPTrapEvent=spBGPTrapEvent, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spFingerprintFeedback=spFingerprintFeedback, spTMSPrefix=spTMSPrefix, mitigationThirdPartyStart=mitigationThirdPartyStart, spCommFailureDestination=spCommFailureDestination, hardwareFailureDone=hardwareFailureDone, spScriptHost=spScriptHost, collectorUp=collectorUp, reportDone=reportDone, spAlertDetectionSignatures=spAlertDetectionSignatures, configChange=configChange, spMitigationName=spMitigationName)
