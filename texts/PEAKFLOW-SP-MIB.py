...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spDNSObservedMax=spDNSObservedMax, spHijackRoute=spHijackRoute, mitigationThirdPartyStart=mitigationThirdPartyStart, spCommFailureDescription=spCommFailureDescription, spInetAddressType=spInetAddressType, spFlowspecCommunity=spFlowspecCommunity, spTMSMultiPrefix=spTMSMultiPrefix, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, bgpUp=bgpUp, spDNSObservedMean=spDNSObservedMean, spScriptHost=spScriptHost, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spDetectedCountries=spDetectedCountries, spTMSCommunity=spTMSCommunity, bgpHijack=bgpHijack, spBGPInstability=spBGPInstability, interfaceUsage=interfaceUsage, notificationLimit=notificationLimit, dnsBaselineDone=dnsBaselineDone, bgpTrap=bgpTrap, spAlertID=spAlertID, devicePhysicalMemory=devicePhysicalMemory, spManagedObjects=spManagedObjects, licenseErrorDone=licenseErrorDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spLicenseErrDescription=spLicenseErrDescription, deviceSystemError=deviceSystemError, spFingerprintMessage=spFingerprintMessage, spBlackholeTimeout=spBlackholeTimeout, spFlowspecTimeout=spFlowspecTimeout, spFingerprintFeedback=spFingerprintFeedback, spHijackAttr=spHijackAttr, configChange=configChange, spcommFailureDone=spcommFailureDone, dosHostDetectionAlert=dosHostDetectionAlert, deviceTotalFlows=deviceTotalFlows, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spUsage=spUsage, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spGreTunnelDestination=spGreTunnelDestination, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, snmpDown=snmpDown, spFingerprintName=spFingerprintName, mitigationBlackholeStart=mitigationBlackholeStart, spInterfaceSpeed=spInterfaceSpeed, spDNSExpected=spDNSExpected, snmpUp=snmpUp, peakflowSPMIB=peakflowSPMIB, autoclassifyStart=autoclassifyStart, spDNSObserved=spDNSObserved, spScriptPort=spScriptPort, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceDiskUsage=deviceDiskUsage, spRouter=spRouter, spMitigationStart=spMitigationStart, greDownDone=greDownDone, mitigationTMSStart=mitigationTMSStart, spInterfaceSpeedHC=spInterfaceSpeedHC, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spBlackholePrefix=spBlackholePrefix, peakflowSPTraps=peakflowSPTraps, bgpDown=bgpDown, spcommFailure=spcommFailure, spScriptCommand=spScriptCommand, spAlertDetectionSignatures=spAlertDetectionSignatures, spThirdPartyAddr=spThirdPartyAddr, mitigationTMSStartV2=mitigationTMSStartV2, collectorStart=collectorStart, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spThirdPartyZone=spThirdPartyZone, PYSNMP_MODULE_ID=peakflowSPMIB, spMitigationID=spMitigationID, spServiceName=spServiceName, spInterfaceUsage=spInterfaceUsage, flowDown=flowDown, managedObjectUsageDone=managedObjectUsageDone, spFingerprintSender=spFingerprintSender, spTMSPrefix=spTMSPrefix, spDuration=spDuration, bgpInstability=bgpInstability, spCollector=spCollector, dnsBaseline=dnsBaseline, spMitigationName=spMitigationName, spManagedObjectFamily=spManagedObjectFamily, spBGPEvent=spBGPEvent, spServiceElement=spServiceElement, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spScriptStart=spScriptStart, spBlackholeCommunity=spBlackholeCommunity, spImpactBps=spImpactBps, spUsageType=spUsageType, peakflowSPMgr=peakflowSPMgr, managedObjectUsage=managedObjectUsage, spReportURL=spReportURL, peakflowSPCMI=peakflowSPCMI, spDetector=spDetector, licenseError=licenseError, deviceSystemErrorDone=deviceSystemErrorDone, spUnit=spUnit, dosNetworkProfiledAlert=dosNetworkProfiledAlert, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, mitigationFlowspecStart=mitigationFlowspecStart, interfaceUsageDone=interfaceUsageDone, spImpactPps=spImpactPps, spReportID=spReportID, routingFailover=routingFailover, spPravail=spPravail, spSystemErrorDescription=spSystemErrorDescription, greDown=greDown, fingerprintUsage=fingerprintUsage, spTMSTimeout=spTMSTimeout, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spVersion=spVersion, spInterfaceUsageHC=spInterfaceUsageHC, collectorUp=collectorUp, spLicenseErrType=spLicenseErrType, spBGPTrapName=spBGPTrapName, flowUp=flowUp, deviceHealth=deviceHealth, spUsername=spUsername, deviceSwapSpace=deviceSwapSpace, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, serviceUsageDone=serviceUsageDone, mitigationDone=mitigationDone, fingerprintFeedback=fingerprintFeedback, cloudSignalingFaultDone=cloudSignalingFaultDone, hardwareFailure=hardwareFailure, hardwareFailureDone=hardwareFailureDone, spHijackLocal=spHijackLocal, fingerprintReceive=fingerprintReceive, deviceTotalFlowsHC=deviceTotalFlowsHC, serviceUsage=serviceUsage, cloudSignalingFault=cloudSignalingFault, spCommFailureDestination=spCommFailureDestination, spInetAddress=spInetAddress, spApplicationName=spApplicationName, spThreshold=spThreshold, alertScript=alertScript, spDNSName=spDNSName, deviceSwapSpaceUsage=deviceSwapSpaceUsage, fingerprintUsageDone=fingerprintUsageDone, spBGPTrapPrefix=spBGPTrapPrefix, collectorDown=collectorDown, spHardwareFailureDescription=spHardwareFailureDescription, spSystemErrorType=spSystemErrorType, spLicenseErrCount=spLicenseErrCount, dosHostDetectionAlertDone=dosHostDetectionAlertDone, reportDone=reportDone, spReportName=spReportName, spBlackholeNexthop=spBlackholeNexthop, spGreTunnelName=spGreTunnelName, spInterface=spInterface, spLicenseErrLimit=spLicenseErrLimit, bgpInstabilityDone=bgpInstabilityDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, trafficAutoMitigation=trafficAutoMitigation, spInterfaceIndex=spInterfaceIndex, spManagedObject=spManagedObject, spBGPTrapEvent=spBGPTrapEvent)
