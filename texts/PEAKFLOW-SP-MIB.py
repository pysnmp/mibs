...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spThirdPartyAddr=spThirdPartyAddr, spDNSName=spDNSName, spFingerprintSender=spFingerprintSender, spHijackLocal=spHijackLocal, spBGPInstability=spBGPInstability, spUsername=spUsername, spMitigationName=spMitigationName, spGreTunnelDestination=spGreTunnelDestination, spImpactBps=spImpactBps, spApplicationName=spApplicationName, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spMitigationStart=spMitigationStart, bgpUp=bgpUp, mitigationFlowspecStart=mitigationFlowspecStart, spPravail=spPravail, spScriptHost=spScriptHost, fingerprintUsage=fingerprintUsage, spFlowspecCommunity=spFlowspecCommunity, flowUp=flowUp, dosHostDetectionAlert=dosHostDetectionAlert, spTMSCommunity=spTMSCommunity, mitigationTMSStart=mitigationTMSStart, bgpInstability=bgpInstability, spAlertDetectionSignatures=spAlertDetectionSignatures, spRouter=spRouter, serviceUsage=serviceUsage, peakflowSPCMI=peakflowSPCMI, spBlackholePrefix=spBlackholePrefix, spLicenseErrLimit=spLicenseErrLimit, interfaceUsage=interfaceUsage, fingerprintUsageDone=fingerprintUsageDone, spFingerprintName=spFingerprintName, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, bgpTrap=bgpTrap, deviceSwapSpaceUsage=deviceSwapSpaceUsage, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spReportID=spReportID, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spDuration=spDuration, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, collectorDown=collectorDown, spAlertID=spAlertID, collectorUp=collectorUp, reportDone=reportDone, fingerprintFeedback=fingerprintFeedback, spReportName=spReportName, hardwareFailureDone=hardwareFailureDone, spReportURL=spReportURL, spGreTunnelName=spGreTunnelName, spLicenseErrDescription=spLicenseErrDescription, spImpactPps=spImpactPps, spTMSPrefix=spTMSPrefix, spSystemErrorType=spSystemErrorType, peakflowSPMgr=peakflowSPMgr, bgpDown=bgpDown, spInterfaceSpeed=spInterfaceSpeed, spBlackholeNexthop=spBlackholeNexthop, spDNSObservedMean=spDNSObservedMean, spHijackAttr=spHijackAttr, spUnit=spUnit, autoclassifyStart=autoclassifyStart, deviceTotalFlowsHC=deviceTotalFlowsHC, spUsageType=spUsageType, spFingerprintFeedback=spFingerprintFeedback, deviceSwapSpace=deviceSwapSpace, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spHijackRoute=spHijackRoute, spManagedObject=spManagedObject, cloudSignalingFault=cloudSignalingFault, deviceHealth=deviceHealth, peakflowSPMIB=peakflowSPMIB, spTMSMultiPrefix=spTMSMultiPrefix, hardwareFailure=hardwareFailure, serviceUsageDone=serviceUsageDone, notificationLimit=notificationLimit, managedObjectUsage=managedObjectUsage, deviceSystemErrorDone=deviceSystemErrorDone, licenseErrorDone=licenseErrorDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spInterfaceSpeedHC=spInterfaceSpeedHC, spFingerprintMessage=spFingerprintMessage, spHardwareFailureDescription=spHardwareFailureDescription, spServiceElement=spServiceElement, spInterface=spInterface, dnsBaselineDone=dnsBaselineDone, greDownDone=greDownDone, PYSNMP_MODULE_ID=peakflowSPMIB, spCommFailureDescription=spCommFailureDescription, spVersion=spVersion, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spSystemErrorDescription=spSystemErrorDescription, mitigationDone=mitigationDone, deviceDiskUsage=deviceDiskUsage, interfaceUsageDone=interfaceUsageDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, licenseError=licenseError, spBlackholeTimeout=spBlackholeTimeout, deviceSwapSpaceInUse=deviceSwapSpaceInUse, spCommFailureDestination=spCommFailureDestination, deviceTotalFlows=deviceTotalFlows, dnsBaseline=dnsBaseline, spInterfaceUsage=spInterfaceUsage, spDNSObservedMax=spDNSObservedMax, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, mitigationThirdPartyStart=mitigationThirdPartyStart, spBGPTrapEvent=spBGPTrapEvent, trafficAutoMitigation=trafficAutoMitigation, spDetectedCountries=spDetectedCountries, spScriptCommand=spScriptCommand, spScriptPort=spScriptPort, snmpUp=snmpUp, spTMSTimeout=spTMSTimeout, spcommFailure=spcommFailure, fingerprintReceive=fingerprintReceive, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spBGPTrapPrefix=spBGPTrapPrefix, alertScript=alertScript, spThreshold=spThreshold, spUsage=spUsage, spBGPEvent=spBGPEvent, peakflowSPTraps=peakflowSPTraps, snmpDown=snmpDown, spInterfaceUsageHC=spInterfaceUsageHC, mitigationBlackholeStart=mitigationBlackholeStart, bgpHijack=bgpHijack, spDNSObserved=spDNSObserved, spMitigationID=spMitigationID, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, collectorStart=collectorStart, spCollector=spCollector, spScriptStart=spScriptStart, spLicenseErrCount=spLicenseErrCount, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, flowDown=flowDown, spBGPTrapName=spBGPTrapName, spDetector=spDetector, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spLicenseErrType=spLicenseErrType, spInterfaceIndex=spInterfaceIndex, spManagedObjectFamily=spManagedObjectFamily, managedObjectUsageDone=managedObjectUsageDone, configChange=configChange, spBlackholeCommunity=spBlackholeCommunity, bgpInstabilityDone=bgpInstabilityDone, spInetAddressType=spInetAddressType, spDNSExpected=spDNSExpected, spManagedObjects=spManagedObjects, spThirdPartyZone=spThirdPartyZone, spInetAddress=spInetAddress, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spBGPTrapOldAttributes=spBGPTrapOldAttributes, greDown=greDown, dosNetworkProfiledAlert=dosNetworkProfiledAlert, routingFailover=routingFailover, mitigationTMSStartV2=mitigationTMSStartV2, deviceSystemError=deviceSystemError, cloudSignalingFaultDone=cloudSignalingFaultDone, spcommFailureDone=spcommFailureDone, spServiceName=spServiceName, spFlowspecTimeout=spFlowspecTimeout, devicePhysicalMemory=devicePhysicalMemory)
