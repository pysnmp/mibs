...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spScriptCommand=spScriptCommand, spFingerprintSender=spFingerprintSender, snmpDown=snmpDown, spImpactPps=spImpactPps, bgpInstability=bgpInstability, mitigationThirdPartyStart=mitigationThirdPartyStart, spImpactBps=spImpactBps, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spSystemErrorDescription=spSystemErrorDescription, trafficAutoMitigation=trafficAutoMitigation, licenseErrorDone=licenseErrorDone, mitigationDone=mitigationDone, spDNSObservedMean=spDNSObservedMean, spManagedObjects=spManagedObjects, collectorDown=collectorDown, spInterfaceUsage=spInterfaceUsage, peakflowSPTraps=peakflowSPTraps, spCommFailureDescription=spCommFailureDescription, flowUp=flowUp, spDNSObserved=spDNSObserved, collectorUp=collectorUp, mitigationBlackholeStart=mitigationBlackholeStart, spInetAddressType=spInetAddressType, spInterfaceUsageHC=spInterfaceUsageHC, spLicenseErrCount=spLicenseErrCount, dnsBaselineDone=dnsBaselineDone, spFingerprintMessage=spFingerprintMessage, bgpInstabilityDone=bgpInstabilityDone, spReportID=spReportID, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spFingerprintFeedback=spFingerprintFeedback, configChange=configChange, spServiceName=spServiceName, spBGPInstability=spBGPInstability, spPravail=spPravail, spThreshold=spThreshold, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spInterfaceIndex=spInterfaceIndex, peakflowSPCMI=peakflowSPCMI, spTMSCommunity=spTMSCommunity, spInetAddress=spInetAddress, spCollector=spCollector, spRouter=spRouter, spScriptHost=spScriptHost, spHijackRoute=spHijackRoute, spManagedObjectFamily=spManagedObjectFamily, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spTMSTimeout=spTMSTimeout, spMitigationStart=spMitigationStart, spLicenseErrLimit=spLicenseErrLimit, spFlowspecCommunity=spFlowspecCommunity, peakflowSPMIB=peakflowSPMIB, interfaceUsageDone=interfaceUsageDone, peakflowSPMgr=peakflowSPMgr, spSystemErrorType=spSystemErrorType, spReportName=spReportName, bgpDown=bgpDown, deviceSwapSpace=deviceSwapSpace, spThirdPartyAddr=spThirdPartyAddr, spBlackholeCommunity=spBlackholeCommunity, alertScript=alertScript, spInterface=spInterface, spFingerprintName=spFingerprintName, spVersion=spVersion, fingerprintReceive=fingerprintReceive, serviceUsage=serviceUsage, spHijackAttr=spHijackAttr, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, snmpUp=snmpUp, spMitigationID=spMitigationID, deviceTotalFlows=deviceTotalFlows, spLicenseErrDescription=spLicenseErrDescription, spGreTunnelDestination=spGreTunnelDestination, spDNSExpected=spDNSExpected, fingerprintUsage=fingerprintUsage, spHardwareFailureDescription=spHardwareFailureDescription, spTMSMultiPrefix=spTMSMultiPrefix, dosHostDetectionAlertDone=dosHostDetectionAlertDone, mitigationTMSStart=mitigationTMSStart, spInterfaceSpeed=spInterfaceSpeed, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, hardwareFailure=hardwareFailure, spcommFailureDone=spcommFailureDone, spServiceElement=spServiceElement, spAlertDetectionSignatures=spAlertDetectionSignatures, spBGPTrapName=spBGPTrapName, spMitigationName=spMitigationName, deviceSwapSpaceInUse=deviceSwapSpaceInUse, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, mitigationFlowspecStart=mitigationFlowspecStart, spBGPTrapEvent=spBGPTrapEvent, spDNSName=spDNSName, serviceUsageDone=serviceUsageDone, spDuration=spDuration, fingerprintFeedback=fingerprintFeedback, managedObjectUsage=managedObjectUsage, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, interfaceUsage=interfaceUsage, reportDone=reportDone, spHijackLocal=spHijackLocal, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, licenseError=licenseError, spThirdPartyZone=spThirdPartyZone, spReportURL=spReportURL, hardwareFailureDone=hardwareFailureDone, spUnit=spUnit, spBGPEvent=spBGPEvent, routingFailover=routingFailover, spDetector=spDetector, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, deviceSystemErrorDone=deviceSystemErrorDone, spScriptPort=spScriptPort, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, bgpUp=bgpUp, spUsername=spUsername, cloudSignalingFault=cloudSignalingFault, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spDNSObservedMax=spDNSObservedMax, autoclassifyStart=autoclassifyStart, PYSNMP_MODULE_ID=peakflowSPMIB, bgpTrap=bgpTrap, managedObjectUsageDone=managedObjectUsageDone, deviceDiskUsage=deviceDiskUsage, spUsageType=spUsageType, deviceTotalFlowsHC=deviceTotalFlowsHC, spFlowspecTimeout=spFlowspecTimeout, bgpHijack=bgpHijack, collectorStart=collectorStart, spcommFailure=spcommFailure, devicePhysicalMemory=devicePhysicalMemory, deviceHealth=deviceHealth, spBlackholeNexthop=spBlackholeNexthop, spBlackholeTimeout=spBlackholeTimeout, spCommFailureDestination=spCommFailureDestination, spUsage=spUsage, cloudSignalingFaultDone=cloudSignalingFaultDone, spDetectedCountries=spDetectedCountries, deviceSwapSpaceUsage=deviceSwapSpaceUsage, dnsBaseline=dnsBaseline, spBlackholePrefix=spBlackholePrefix, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spAlertID=spAlertID, spApplicationName=spApplicationName, dosHostDetectionAlert=dosHostDetectionAlert, flowDown=flowDown, spBGPTrapPrefix=spBGPTrapPrefix, mitigationTMSStartV2=mitigationTMSStartV2, spManagedObject=spManagedObject, dosNetworkProfiledAlert=dosNetworkProfiledAlert, greDown=greDown, greDownDone=greDownDone, fingerprintUsageDone=fingerprintUsageDone, notificationLimit=notificationLimit, spGreTunnelName=spGreTunnelName, spLicenseErrType=spLicenseErrType, spInterfaceSpeedHC=spInterfaceSpeedHC, spTMSPrefix=spTMSPrefix, spScriptStart=spScriptStart, deviceSystemError=deviceSystemError)
