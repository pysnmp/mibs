...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spMitigationName=spMitigationName, spScriptCommand=spScriptCommand, bgpUp=bgpUp, spDNSObservedMean=spDNSObservedMean, spInterface=spInterface, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, spScriptPort=spScriptPort, deviceSwapSpace=deviceSwapSpace, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spTMSMultiPrefix=spTMSMultiPrefix, spBlackholePrefix=spBlackholePrefix, spSystemErrorDescription=spSystemErrorDescription, spLicenseErrDescription=spLicenseErrDescription, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spHijackRoute=spHijackRoute, spDNSObservedMax=spDNSObservedMax, deviceSwapSpaceInUse=deviceSwapSpaceInUse, serviceUsage=serviceUsage, spManagedObjects=spManagedObjects, fingerprintReceive=fingerprintReceive, spScriptHost=spScriptHost, spImpactBps=spImpactBps, spInterfaceUsage=spInterfaceUsage, bgpTrap=bgpTrap, interfaceUsage=interfaceUsage, spAlertID=spAlertID, spServiceName=spServiceName, peakflowSPCMI=peakflowSPCMI, spBlackholeCommunity=spBlackholeCommunity, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spMitigationStart=spMitigationStart, collectorDown=collectorDown, spHijackLocal=spHijackLocal, trafficAutoMitigation=trafficAutoMitigation, spApplicationName=spApplicationName, dosHostDetectionAlert=dosHostDetectionAlert, bgpInstability=bgpInstability, reportDone=reportDone, managedObjectUsageDone=managedObjectUsageDone, spThreshold=spThreshold, spLicenseErrLimit=spLicenseErrLimit, spMitigationID=spMitigationID, spFingerprintFeedback=spFingerprintFeedback, collectorStart=collectorStart, hardwareFailure=hardwareFailure, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spHijackAttr=spHijackAttr, spFingerprintMessage=spFingerprintMessage, spTMSTimeout=spTMSTimeout, spTMSPrefix=spTMSPrefix, spManagedObjectFamily=spManagedObjectFamily, deviceHealth=deviceHealth, spManagedObject=spManagedObject, licenseErrorDone=licenseErrorDone, spBGPInstability=spBGPInstability, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, devicePhysicalMemory=devicePhysicalMemory, hardwareFailureDone=hardwareFailureDone, spDNSName=spDNSName, spInetAddress=spInetAddress, interfaceUsageDone=interfaceUsageDone, spcommFailure=spcommFailure, configChange=configChange, deviceTotalFlows=deviceTotalFlows, spSystemErrorType=spSystemErrorType, spBGPTrapEvent=spBGPTrapEvent, spInetAddressType=spInetAddressType, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spScriptStart=spScriptStart, fingerprintFeedback=fingerprintFeedback, spVersion=spVersion, spReportName=spReportName, deviceTotalFlowsHC=deviceTotalFlowsHC, peakflowSPTraps=peakflowSPTraps, spServiceElement=spServiceElement, greDownDone=greDownDone, spAlertDetectionSignatures=spAlertDetectionSignatures, spUsage=spUsage, mitigationFlowspecStart=mitigationFlowspecStart, deviceDiskUsage=deviceDiskUsage, spGreTunnelName=spGreTunnelName, spDuration=spDuration, spBGPTrapOldAttributes=spBGPTrapOldAttributes, mitigationTMSStart=mitigationTMSStart, snmpDown=snmpDown, bgpInstabilityDone=bgpInstabilityDone, spThirdPartyZone=spThirdPartyZone, mitigationTMSStartV2=mitigationTMSStartV2, spPravail=spPravail, dnsBaselineDone=dnsBaselineDone, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spBGPTrapName=spBGPTrapName, greDown=greDown, mitigationThirdPartyStart=mitigationThirdPartyStart, flowDown=flowDown, managedObjectUsage=managedObjectUsage, spFingerprintName=spFingerprintName, mitigationBlackholeStart=mitigationBlackholeStart, spFingerprintSender=spFingerprintSender, spBGPTrapPrefix=spBGPTrapPrefix, fingerprintUsage=fingerprintUsage, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spInterfaceSpeed=spInterfaceSpeed, peakflowSPMgr=peakflowSPMgr, spCommFailureDescription=spCommFailureDescription, flowUp=flowUp, spFlowspecCommunity=spFlowspecCommunity, snmpUp=snmpUp, spRouter=spRouter, spReportURL=spReportURL, collectorUp=collectorUp, licenseError=licenseError, spFlowspecTimeout=spFlowspecTimeout, spBGPTrapNewAttributes=spBGPTrapNewAttributes, spGreTunnelDestination=spGreTunnelDestination, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, spTMSCommunity=spTMSCommunity, spCollector=spCollector, peakflowSPMIB=peakflowSPMIB, routingFailover=routingFailover, serviceUsageDone=serviceUsageDone, cloudSignalingFaultDone=cloudSignalingFaultDone, spInterfaceIndex=spInterfaceIndex, spBlackholeTimeout=spBlackholeTimeout, spBGPEvent=spBGPEvent, spDNSExpected=spDNSExpected, spDetectedCountries=spDetectedCountries, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, deviceSystemErrorDone=deviceSystemErrorDone, spInterfaceSpeedHC=spInterfaceSpeedHC, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, bgpDown=bgpDown, spHardwareFailureDescription=spHardwareFailureDescription, spUsageType=spUsageType, spcommFailureDone=spcommFailureDone, cloudSignalingFault=cloudSignalingFault, spUsername=spUsername, spUnit=spUnit, spDetector=spDetector, spLicenseErrCount=spLicenseErrCount, fingerprintUsageDone=fingerprintUsageDone, notificationLimit=notificationLimit, spBlackholeNexthop=spBlackholeNexthop, deviceSwapSpaceUsage=deviceSwapSpaceUsage, alertScript=alertScript, spDNSObserved=spDNSObserved, mitigationDone=mitigationDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, PYSNMP_MODULE_ID=peakflowSPMIB, spThirdPartyAddr=spThirdPartyAddr, autoclassifyStart=autoclassifyStart, deviceSystemError=deviceSystemError, dnsBaseline=dnsBaseline, spLicenseErrType=spLicenseErrType, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spInterfaceUsageHC=spInterfaceUsageHC, bgpHijack=bgpHijack, spImpactPps=spImpactPps, spReportID=spReportID, spCommFailureDestination=spCommFailureDestination)
