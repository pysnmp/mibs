...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", spBGPTrapPrefix=spBGPTrapPrefix, spAlertID=spAlertID, spReportName=spReportName, mitigationTMSStartV2=mitigationTMSStartV2, spGreTunnelDestination=spGreTunnelDestination, greDown=greDown, spBGPTrapName=spBGPTrapName, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spLicenseErrType=spLicenseErrType, reportDone=reportDone, spImpactBps=spImpactBps, spHijackLocal=spHijackLocal, snmpDown=snmpDown, spReportURL=spReportURL, spManagedObject=spManagedObject, spDNSObservedMean=spDNSObservedMean, spDNSExpected=spDNSExpected, interfaceUsage=interfaceUsage, autoclassifyStart=autoclassifyStart, spInterfaceSpeedHC=spInterfaceSpeedHC, spBGPTrapEvent=spBGPTrapEvent, spUsageType=spUsageType, greDownDone=greDownDone, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spManagedObjectFamily=spManagedObjectFamily, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spServiceName=spServiceName, licenseError=licenseError, deviceTotalFlowsHC=deviceTotalFlowsHC, spRouter=spRouter, spMitigationID=spMitigationID, spMitigationStart=spMitigationStart, spVersion=spVersion, bgpTrap=bgpTrap, spGreTunnelName=spGreTunnelName, spHardwareFailureDescription=spHardwareFailureDescription, spCommFailureDestination=spCommFailureDestination, bgpInstabilityDone=bgpInstabilityDone, spDNSObserved=spDNSObserved, spHijackAttr=spHijackAttr, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInterfaceSpeed=spInterfaceSpeed, dosNetworkProfiledAlert=dosNetworkProfiledAlert, spBlackholeTimeout=spBlackholeTimeout, dnsBaselineDone=dnsBaselineDone, deviceSystemErrorDone=deviceSystemErrorDone, dosHostDetectionAlert=dosHostDetectionAlert, peakflowSPCMI=peakflowSPCMI, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spInterface=spInterface, mitigationBlackholeStart=mitigationBlackholeStart, deviceSystemError=deviceSystemError, bgpHijack=bgpHijack, spDetector=spDetector, deviceSwapSpaceInUse=deviceSwapSpaceInUse, snmpUp=snmpUp, spServiceElement=spServiceElement, fingerprintUsageDone=fingerprintUsageDone, spThirdPartyAddr=spThirdPartyAddr, spUsage=spUsage, spLicenseErrLimit=spLicenseErrLimit, managedObjectUsage=managedObjectUsage, spTMSMultiPrefix=spTMSMultiPrefix, managedObjectUsageDone=managedObjectUsageDone, spBGPInstability=spBGPInstability, fingerprintReceive=fingerprintReceive, spTMSCommunity=spTMSCommunity, spInterfaceUsageHC=spInterfaceUsageHC, deviceHealth=deviceHealth, alertScript=alertScript, spBlackholeNexthop=spBlackholeNexthop, spImpactPps=spImpactPps, spBGPTrapNewAttributes=spBGPTrapNewAttributes, bgpDown=bgpDown, bgpUp=bgpUp, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, notificationLimit=notificationLimit, spcommFailureDone=spcommFailureDone, spFingerprintFeedback=spFingerprintFeedback, spcommFailure=spcommFailure, fingerprintUsage=fingerprintUsage, spInetAddressType=spInetAddressType, spScriptStart=spScriptStart, spTMSTimeout=spTMSTimeout, spHijackRoute=spHijackRoute, spFingerprintMessage=spFingerprintMessage, serviceUsageDone=serviceUsageDone, spBGPEvent=spBGPEvent, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, spDNSName=spDNSName, spDetectedCountries=spDetectedCountries, bgpInstability=bgpInstability, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, configChange=configChange, interfaceUsageDone=interfaceUsageDone, spPravail=spPravail, peakflowSPMIB=peakflowSPMIB, trafficAutoMitigation=trafficAutoMitigation, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, mitigationTMSStart=mitigationTMSStart, peakflowSPMgr=peakflowSPMgr, spLicenseErrDescription=spLicenseErrDescription, spFlowspecCommunity=spFlowspecCommunity, cloudSignalingFault=cloudSignalingFault, deviceDiskUsage=deviceDiskUsage, spCollector=spCollector, spReportID=spReportID, spBlackholeCommunity=spBlackholeCommunity, spCommFailureDescription=spCommFailureDescription, spSystemErrorType=spSystemErrorType, mitigationDone=mitigationDone, devicePhysicalMemory=devicePhysicalMemory, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spDNSObservedMax=spDNSObservedMax, PYSNMP_MODULE_ID=peakflowSPMIB, spFlowspecTimeout=spFlowspecTimeout, flowDown=flowDown, spScriptCommand=spScriptCommand, collectorUp=collectorUp, mitigationThirdPartyStart=mitigationThirdPartyStart, spScriptPort=spScriptPort, spManagedObjects=spManagedObjects, deviceTotalFlows=deviceTotalFlows, spApplicationName=spApplicationName, spSystemErrorDescription=spSystemErrorDescription, spLicenseErrCount=spLicenseErrCount, hardwareFailureDone=hardwareFailureDone, deviceSwapSpace=deviceSwapSpace, hardwareFailure=hardwareFailure, serviceUsage=serviceUsage, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spScriptHost=spScriptHost, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, collectorStart=collectorStart, spCloudSignalFaultDescription=spCloudSignalFaultDescription, routingFailover=routingFailover, spInterfaceUsage=spInterfaceUsage, spTMSPrefix=spTMSPrefix, spThirdPartyZone=spThirdPartyZone, spDuration=spDuration, spFingerprintSender=spFingerprintSender, fingerprintFeedback=fingerprintFeedback, spBlackholePrefix=spBlackholePrefix, flowUp=flowUp, spThreshold=spThreshold, spUnit=spUnit, collectorDown=collectorDown, spUsername=spUsername, spInterfaceIndex=spInterfaceIndex, licenseErrorDone=licenseErrorDone, spAlertDetectionSignatures=spAlertDetectionSignatures, mitigationFlowspecStart=mitigationFlowspecStart, spInetAddress=spInetAddress, peakflowSPTraps=peakflowSPTraps, spFingerprintName=spFingerprintName, cloudSignalingFaultDone=cloudSignalingFaultDone, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spMitigationName=spMitigationName, dnsBaseline=dnsBaseline)
