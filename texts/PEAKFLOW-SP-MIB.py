...
mibBuilder.exportSymbols("PEAKFLOW-SP-MIB", flowDown=flowDown, deviceCpuLoadAvg1min=deviceCpuLoadAvg1min, spDNSExpected=spDNSExpected, snmpDown=snmpDown, spLicenseErrType=spLicenseErrType, spDNSObservedMax=spDNSObservedMax, interfaceUsage=interfaceUsage, peakflowSPCMI=peakflowSPCMI, deviceSystemErrorDone=deviceSystemErrorDone, spDetector=spDetector, spAlertID=spAlertID, collectorStart=collectorStart, spUnit=spUnit, spFingerprintMessage=spFingerprintMessage, spThirdPartyZone=spThirdPartyZone, spInterfaceSpeedHC=spInterfaceSpeedHC, bgpUp=bgpUp, spInetAddressType=spInetAddressType, spVersion=spVersion, spUsername=spUsername, spHardwareFailureDescription=spHardwareFailureDescription, cloudSignalingFaultDone=cloudSignalingFaultDone, snmpUp=snmpUp, spInterface=spInterface, peakflowSPMIB=peakflowSPMIB, spRoutingFailoverInterfaces=spRoutingFailoverInterfaces, spHijackAttr=spHijackAttr, reportDone=reportDone, spGreTunnelName=spGreTunnelName, spSystemErrorType=spSystemErrorType, spBGPTrapNewAttributes=spBGPTrapNewAttributes, deviceDiskUsage=deviceDiskUsage, deviceTotalFlows=deviceTotalFlows, bgpInstabilityDone=bgpInstabilityDone, spDNSName=spDNSName, alertScript=alertScript, routingFailover=routingFailover, devicePhysicalMemory=devicePhysicalMemory, spHijackLocal=spHijackLocal, fingerprintFeedback=fingerprintFeedback, licenseErrorDone=licenseErrorDone, spInterfaceSpeed=spInterfaceSpeed, spCommFailureDestination=spCommFailureDestination, devicePhysicalMemoryUsage=devicePhysicalMemoryUsage, spTMSMultiPrefix=spTMSMultiPrefix, PYSNMP_MODULE_ID=peakflowSPMIB, spFingerprintName=spFingerprintName, spUsageType=spUsageType, dosNetworkProfiledAlertDone=dosNetworkProfiledAlertDone, configChange=configChange, routingFailoverInterfaceDownAlertDone=routingFailoverInterfaceDownAlertDone, spInterfaceUsage=spInterfaceUsage, spImpactBps=spImpactBps, mitigationDone=mitigationDone, deviceSwapSpaceUsage=deviceSwapSpaceUsage, spInetAddress=spInetAddress, spHijackRoute=spHijackRoute, spRouter=spRouter, dnsBaseline=dnsBaseline, dosNetworkProfiledAlert=dosNetworkProfiledAlert, serviceUsage=serviceUsage, spLicenseErrLimit=spLicenseErrLimit, spMitigationName=spMitigationName, deviceCpuLoadAvg5min=deviceCpuLoadAvg5min, spDNSObservedMean=spDNSObservedMean, spTMSPrefix=spTMSPrefix, devicePhysicalMemoryInUse=devicePhysicalMemoryInUse, spGreTunnelDestination=spGreTunnelDestination, hardwareFailure=hardwareFailure, mitigationTMSStart=mitigationTMSStart, fingerprintUsage=fingerprintUsage, spMitigationID=spMitigationID, deviceSwapSpaceInUse=deviceSwapSpaceInUse, cloudSignalingMitigationRequest=cloudSignalingMitigationRequest, spScriptHost=spScriptHost, spFingerprintFeedback=spFingerprintFeedback, spReportURL=spReportURL, dnsBaselineDone=dnsBaselineDone, spFingerprintSender=spFingerprintSender, spApplicationName=spApplicationName, spImpactPps=spImpactPps, deviceSwapSpace=deviceSwapSpace, cloudSignalingFault=cloudSignalingFault, spReportName=spReportName, spLicenseErrCount=spLicenseErrCount, spBlackholePrefix=spBlackholePrefix, dosHostDetectionAlertDone=dosHostDetectionAlertDone, spManagedObject=spManagedObject, autoclassifyStart=autoclassifyStart, bgpHijack=bgpHijack, mitigationTMSStartV2=mitigationTMSStartV2, spServiceName=spServiceName, spScriptPort=spScriptPort, spCloudSignalFaultDescription=spCloudSignalFaultDescription, spScriptCommand=spScriptCommand, spCollector=spCollector, spBGPEvent=spBGPEvent, spCommFailureDescription=spCommFailureDescription, spBGPTrapOldAttributes=spBGPTrapOldAttributes, spTMSCommunity=spTMSCommunity, deviceSystemError=deviceSystemError, hardwareFailureDone=hardwareFailureDone, peakflowSPTrapsEnumerate=peakflowSPTrapsEnumerate, spInterfaceUsageHC=spInterfaceUsageHC, bgpTrap=bgpTrap, spPravail=spPravail, mitigationFlowspecStart=mitigationFlowspecStart, spBlackholeNexthop=spBlackholeNexthop, spSystemErrorDescription=spSystemErrorDescription, mitigationBlackholeStart=mitigationBlackholeStart, spThreshold=spThreshold, managedObjectUsage=managedObjectUsage, spManagedObjectFamily=spManagedObjectFamily, bgpDown=bgpDown, bgpInstability=bgpInstability, notificationLimit=notificationLimit, managedObjectUsageDone=managedObjectUsageDone, spcommFailure=spcommFailure, spBGPTrapPrefix=spBGPTrapPrefix, spServiceElement=spServiceElement, spBGPInstability=spBGPInstability, spFlowspecCommunity=spFlowspecCommunity, spUsage=spUsage, licenseError=licenseError, deviceCpuLoadAvg15min=deviceCpuLoadAvg15min, dosHostDetectionAlert=dosHostDetectionAlert, flowUp=flowUp, spThirdPartyAddr=spThirdPartyAddr, spInterfaceIndex=spInterfaceIndex, spBlackholeTimeout=spBlackholeTimeout, greDownDone=greDownDone, peakflowSPTraps=peakflowSPTraps, spLicenseErrDescription=spLicenseErrDescription, spTMSTimeout=spTMSTimeout, spReportID=spReportID, spDetectedCountries=spDetectedCountries, deviceTotalFlowsHC=deviceTotalFlowsHC, spDNSObserved=spDNSObserved, spFlowspecTimeout=spFlowspecTimeout, spTMSMultiDiversionPrefix=spTMSMultiDiversionPrefix, interfaceUsageDone=interfaceUsageDone, spBGPTrapEvent=spBGPTrapEvent, spBGPTrapName=spBGPTrapName, deviceHealth=deviceHealth, spcommFailureDone=spcommFailureDone, spScriptStart=spScriptStart, spManagedObjects=spManagedObjects, greDown=greDown, collectorDown=collectorDown, peakflowSPMgr=peakflowSPMgr, serviceUsageDone=serviceUsageDone, routingFailoverInterfaceDownAlert=routingFailoverInterfaceDownAlert, spBlackholeCommunity=spBlackholeCommunity, mitigationThirdPartyStart=mitigationThirdPartyStart, spMitigationStart=spMitigationStart, collectorUp=collectorUp, spAlertDetectionSignatures=spAlertDetectionSignatures, fingerprintReceive=fingerprintReceive, trafficAutoMitigation=trafficAutoMitigation, fingerprintUsageDone=fingerprintUsageDone, spDuration=spDuration)
