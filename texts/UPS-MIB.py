...
# Produced by pysmi-1.1.3 at Sun Nov 28 20:35:09 2021
...
mibBuilder.exportSymbols("UPS-MIB", upsAlarmOnBattery=upsAlarmOnBattery, upsBypassFrequency=upsBypassFrequency, upsTrapAlarmEntryAdded=upsTrapAlarmEntryAdded, upsCompliances=upsCompliances, upsAlarmInputBad=upsAlarmInputBad, upsFullGroups=upsFullGroups, upsTestNoTestsInitiated=upsTestNoTestsInitiated, upsOutputEntry=upsOutputEntry, upsInputNumLines=upsInputNumLines, upsBypassEntry=upsBypassEntry, upsStartupAfterDelay=upsStartupAfterDelay, upsOutputVoltage=upsOutputVoltage, upsInputLineBads=upsInputLineBads, upsTestElapsedTime=upsTestElapsedTime, upsAlarmBypassBad=upsAlarmBypassBad, upsTestResultsSummary=upsTestResultsSummary, upsOutputPercentLoad=upsOutputPercentLoad, upsAlarmDiagnosticTestFailed=upsAlarmDiagnosticTestFailed, upsBatteryStatus=upsBatteryStatus, upsAlarmShutdownImminent=upsAlarmShutdownImminent, upsBypassNumLines=upsBypassNumLines, upsTest=upsTest, upsOutputSource=upsOutputSource, upsConfigInputFreq=upsConfigInputFreq, upsAlarmShutdownPending=upsAlarmShutdownPending, PositiveInteger=PositiveInteger, upsBypassLineIndex=upsBypassLineIndex, upsTestGeneralSystemsTest=upsTestGeneralSystemsTest, upsSubsetGroups=upsSubsetGroups, upsAlarmDepletedBattery=upsAlarmDepletedBattery, upsTestQuickBatteryTest=upsTestQuickBatteryTest, upsSubsetCompliance=upsSubsetCompliance, upsBatteryVoltage=upsBatteryVoltage, upsTestStartTime=upsTestStartTime, upsBasicIdentGroup=upsBasicIdentGroup, upsIdentAttachedDevices=upsIdentAttachedDevices, upsBypass=upsBypass, upsConfigLowBattTime=upsConfigLowBattTime, upsShutdownAfterDelay=upsShutdownAfterDelay, upsTrapAlarmEntryRemoved=upsTrapAlarmEntryRemoved, upsInputTruePower=upsInputTruePower, upsSubsetConfigGroup=upsSubsetConfigGroup, upsAutoRestart=upsAutoRestart, upsEstimatedChargeRemaining=upsEstimatedChargeRemaining, upsConfigOutputVoltage=upsConfigOutputVoltage, upsTraps=upsTraps, upsTrapOnBattery=upsTrapOnBattery, upsObjects=upsObjects, upsFullOutputGroup=upsFullOutputGroup, upsSecondsOnBattery=upsSecondsOnBattery, upsOutputFrequency=upsOutputFrequency, upsAlarmId=upsAlarmId, upsFullBypassGroup=upsFullBypassGroup, upsBasicAlarmGroup=upsBasicAlarmGroup, upsBypassVoltage=upsBypassVoltage, upsInputFrequency=upsInputFrequency, upsConfigHighVoltageTransferPoint=upsConfigHighVoltageTransferPoint, upsIdentModel=upsIdentModel, upsBypassCurrent=upsBypassCurrent, upsBasicInputGroup=upsBasicInputGroup, upsBasicOutputGroup=upsBasicOutputGroup, upsSubsetBatteryGroup=upsSubsetBatteryGroup, upsBasicTestGroup=upsBasicTestGroup, upsAlarmFanFailure=upsAlarmFanFailure, upsAlarmCommunicationsLost=upsAlarmCommunicationsLost, upsAlarmTestInProgress=upsAlarmTestInProgress, upsTestAbortTestInProgress=upsTestAbortTestInProgress, upsFullAlarmGroup=upsFullAlarmGroup, upsInputEntry=upsInputEntry, upsAlarmTable=upsAlarmTable, upsBypassTable=upsBypassTable, upsMIB=upsMIB, upsAlarmOutputOffAsRequested=upsAlarmOutputOffAsRequested, upsWellKnownTests=upsWellKnownTests, upsAlarmDescr=upsAlarmDescr, upsSubsetControlGroup=upsSubsetControlGroup, upsBasicGroups=upsBasicGroups, upsOutput=upsOutput, upsInputLineIndex=upsInputLineIndex, upsConfigInputVoltage=upsConfigInputVoltage, upsConfigOutputVA=upsConfigOutputVA, upsIdentName=upsIdentName, upsConfigOutputPower=upsConfigOutputPower, upsSubsetInputGroup=upsSubsetInputGroup, upsTestResultsDetail=upsTestResultsDetail, upsInputTable=upsInputTable, upsBasicBatteryGroup=upsBasicBatteryGroup, upsBatteryCurrent=upsBatteryCurrent, upsBattery=upsBattery, upsBypassPower=upsBypassPower, upsOutputPower=upsOutputPower, upsAlarmUpsOutputOff=upsAlarmUpsOutputOff, upsAlarmsPresent=upsAlarmsPresent, upsAlarmGeneralFault=upsAlarmGeneralFault, upsIdentUPSSoftwareVersion=upsIdentUPSSoftwareVersion, PYSNMP_MODULE_ID=upsMIB, upsAlarm=upsAlarm, upsIdentManufacturer=upsIdentManufacturer, upsOutputCurrent=upsOutputCurrent, upsAlarmAwaitingPower=upsAlarmAwaitingPower, upsFullIdentGroup=upsFullIdentGroup, upsBasicCompliance=upsBasicCompliance, upsInputVoltage=upsInputVoltage, upsTestDeepBatteryCalibration=upsTestDeepBatteryCalibration, upsConfigLowVoltageTransferPoint=upsConfigLowVoltageTransferPoint, upsOutputNumLines=upsOutputNumLines, upsSubsetIdentGroup=upsSubsetIdentGroup, upsBasicControlGroup=upsBasicControlGroup, upsAlarmLowBattery=upsAlarmLowBattery, upsFullConfigGroup=upsFullConfigGroup, upsSubsetOutputGroup=upsSubsetOutputGroup, upsConfigAudibleStatus=upsConfigAudibleStatus, upsIdent=upsIdent, upsAlarmOutputBad=upsAlarmOutputBad, upsTrapTestCompleted=upsTrapTestCompleted, upsAlarmOnBypass=upsAlarmOnBypass, upsFullCompliance=upsFullCompliance, upsFullTestGroup=upsFullTestGroup, upsEstimatedMinutesRemaining=upsEstimatedMinutesRemaining, upsOutputTable=upsOutputTable, upsAlarmBatteryBad=upsAlarmBatteryBad, upsAlarmFuseFailure=upsAlarmFuseFailure, upsControl=upsControl, NonNegativeInteger=NonNegativeInteger, upsOutputLineIndex=upsOutputLineIndex, upsConformance=upsConformance, upsAlarmTime=upsAlarmTime, upsFullBatteryGroup=upsFullBatteryGroup, upsConfigOutputFreq=upsConfigOutputFreq, upsSubsetAlarmGroup=upsSubsetAlarmGroup, upsAlarmOutputOverload=upsAlarmOutputOverload, upsWellKnownAlarms=upsWellKnownAlarms, upsTestSpinLock=upsTestSpinLock, upsTestId=upsTestId, upsAlarmUpsOffAsRequested=upsAlarmUpsOffAsRequested, upsGroups=upsGroups, upsShutdownType=upsShutdownType, upsFullInputGroup=upsFullInputGroup, upsAlarmTempBad=upsAlarmTempBad, upsInput=upsInput, upsAlarmUpsSystemOff=upsAlarmUpsSystemOff, upsAlarmChargerFailed=upsAlarmChargerFailed, upsFullControlGroup=upsFullControlGroup, upsBatteryTemperature=upsBatteryTemperature, upsConfig=upsConfig, upsBasicConfigGroup=upsBasicConfigGroup, upsInputCurrent=upsInputCurrent, upsBasicBypassGroup=upsBasicBypassGroup, upsIdentAgentSoftwareVersion=upsIdentAgentSoftwareVersion, upsAlarmEntry=upsAlarmEntry, upsRebootWithDuration=upsRebootWithDuration)
