...
TimeTicks, Integer32, Unsigned32, Gauge32, ObjectIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Bits, NotificationType, IpAddress, ModuleIdentity, Counter32, iso, MibIdentifier, Counter64 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Integer32", "Unsigned32", "Gauge32", "ObjectIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Bits", "NotificationType", "IpAddress", "ModuleIdentity", "Counter32", "iso", "MibIdentifier", "Counter64")
...
mibBuilder.exportSymbols("MPIOS-MIB", rtrUdpechoEntityId=rtrUdpechoEntityId, sysIfPktPriority=sysIfPktPriority, rtrScheduleAfterTime=rtrScheduleAfterTime, sysIfInErrors=sysIfInErrors, rtrScheduleLifeFlag=rtrScheduleLifeFlag, sysRtrFlowStatisticsTable=sysRtrFlowStatisticsTable, avgBlockSizeFree=avgBlockSizeFree, taskStackEnd=taskStackEnd, rtrJitterSdMin=rtrJitterSdMin, taskPriority=taskPriority, taskStackSize=taskStackSize, sysRtrScheduleEntry=sysRtrScheduleEntry, rtrFlStaInputMinFlow=rtrFlStaInputMinFlow, cpuTaskTable=cpuTaskTable, rtrScheduleInterval=rtrScheduleInterval, rtrEntityRowStatus=rtrEntityRowStatus, rtrJitterPktLossSd=rtrJitterPktLossSd, rtrUdpechoTimeout=rtrUdpechoTimeout, sysIfInNUcastPkts=sysIfInNUcastPkts, taskId=taskId, checkCpuTimeInterval=checkCpuTimeInterval, rtrJitterSchInterval=rtrJitterSchInterval, sysIfInOctets=sysIfInOctets, taskStackPtr=taskStackPtr, rtrScheduleStartTimeFlag=rtrScheduleStartTimeFlag, rtrIcmpEchoSchInterval=rtrIcmpEchoSchInterval, iosSystem=iosSystem, numBlocksFree=numBlocksFree, cpuTaskId=cpuTaskId, taskEntry=taskEntry, rtrJitterEntityId=rtrJitterEntityId, rtrFlStaInputMaxPkts=rtrFlStaInputMaxPkts, rtrFlStaTmInputMaxFlow=rtrFlStaTmInputMaxFlow, rtrFlStaTmOutputMinPkts=rtrFlStaTmOutputMinPkts, rtrUdpechoDelayMax=rtrUdpechoDelayMax, rtrFlStaTmOutputMaxFlow=rtrFlStaTmOutputMaxFlow, cpuTaskCurrentUtil=cpuTaskCurrentUtil, rtrIcmpEchoVrfName=rtrIcmpEchoVrfName, sysMainBoardAlertTemper=sysMainBoardAlertTemper, sysTask=sysTask, numBlocksAlloc=numBlocksAlloc, iosObjects=iosObjects, sysRtrScheduleMgt=sysRtrScheduleMgt, rtrEntityThreshold=rtrEntityThreshold, rtrFlStaTmInputMinPkts=rtrFlStaTmInputMinPkts, cpuUtilTotalUsedTicks=cpuUtilTotalUsedTicks, rtrUdpechoVrfName=rtrUdpechoVrfName, sysTemperature=sysTemperature, cpuTaskTotalTicks=cpuTaskTotalTicks, rtrJitterIcpifMin=rtrJitterIcpifMin, EnabledStatus=EnabledStatus, sysRtrUdpechoMgt=sysRtrUdpechoMgt, rtrGroupName=rtrGroupName, sysRtrFlowStatisticsEntry=sysRtrFlowStatisticsEntry, sysIfOutErrors=sysIfOutErrors, sysNFI=sysNFI, rtrGroupId=rtrGroupId, sysRtrScheduleTable=sysRtrScheduleTable, rtrJitterTimeout=rtrJitterTimeout, rtrIcmpEchoPktNum=rtrIcmpEchoPktNum, sysRtrEntityEntry=sysRtrEntityEntry, rtrIcmpEchoTos=rtrIcmpEchoTos, rtrFlStatisticsEntityId=rtrFlStatisticsEntityId, rtrFlStaOutputMaxFlow=rtrFlStaOutputMaxFlow, rtrUdpechoSourcePort=rtrUdpechoSourcePort, rtrJitterSourceIp=rtrJitterSourceIp, rtrIcmpEchoRowStatus=rtrIcmpEchoRowStatus, rtrEntityLogMaxSize=rtrEntityLogMaxSize, cpuUtilTable=cpuUtilTable, rtrJitterPktLossDs=rtrJitterPktLossDs, cpuUtilAverageUtil=cpuUtilAverageUtil, rtrFlStaTmInputMaxPkts=rtrFlStaTmInputMaxPkts, rtrUdpechoTargetPort=rtrUdpechoTargetPort, rtrJitterMinRtt=rtrJitterMinRtt, numBytesAlloc=numBytesAlloc, rtrGroupInterval=rtrGroupInterval, rtrEntityId=rtrEntityId, rtrScheduleLifeTime=rtrScheduleLifeTime, rtrScheduleRepeat=rtrScheduleRepeat, cpuUtilDeltaTimes=cpuUtilDeltaTimes, sysCpuAlertTemper=sysCpuAlertTemper, rtrScheduleObjId=rtrScheduleObjId, rtrUdpechoPktLoss=rtrUdpechoPktLoss, rtrIcmpEchoVerifyData=rtrIcmpEchoVerifyData, rtrJitterIcpifMax=rtrJitterIcpifMax, rtrJitterDsMax=rtrJitterDsMax, rtrGroupEntityMembers=rtrGroupEntityMembers, taskName=taskName, cpuUtilDeltaUsedTicks=cpuUtilDeltaUsedTicks, rtrJitterDelayDsMin=rtrJitterDelayDsMin, sysIfOutDiscards=sysIfOutDiscards, rtrJitterDsMin=rtrJitterDsMin, allocBytesPercent=allocBytesPercent, sysIfPktPriStaEntry=sysIfPktPriStaEntry, cpuTaskDeltaUtil=cpuTaskDeltaUtil, rtrJitterMosMax=rtrJitterMosMax, rtrScheduleRowStatus=rtrScheduleRowStatus, PYSNMP_MODULE_ID=mpios, numBytesFree=numBytesFree, rtrScheduleType=rtrScheduleType, rtrJitterDelaySdMax=rtrJitterDelaySdMax, rtrEntityLogFilter=rtrEntityLogFilter, avgBlockSizeAlloc=avgBlockSizeAlloc, sysRtrIcmpEchoMgt=sysRtrIcmpEchoMgt, sysIfInUcastPkts=sysIfInUcastPkts, rtrFlStaOutputMaxPkts=rtrFlStaOutputMaxPkts, rtrIcmpEchoSourceIp=rtrIcmpEchoSourceIp, rtrJitterTargetPort=rtrJitterTargetPort, rtrFlStatisticsIfName=rtrFlStatisticsIfName, rtrIcmpEchoTimeout=rtrIcmpEchoTimeout, taskMain=taskMain, sysRtrCtrl=sysRtrCtrl, cpuTaskDeltaTicks=cpuTaskDeltaTicks, taskDescr=taskDescr, cpuUtilCpuId=cpuUtilCpuId, sysAlertTrapInt=sysAlertTrapInt, rtrFlStaInputMaxFlow=rtrFlStaInputMaxFlow, rtrFlStaInputMinPkts=rtrFlStaInputMinPkts, rtrUdpechoSourceIp=rtrUdpechoSourceIp, sysIfPktPriStatistics=sysIfPktPriStatistics, rtrIcmpEchoSuccessRate=rtrIcmpEchoSuccessRate, rtrIcmpEchoTargetIp=rtrIcmpEchoTargetIp, rtrFlStaTmOutputMinFlow=rtrFlStaTmOutputMinFlow, cpuUtilDeltaUtil=cpuUtilDeltaUtil, sysRtrGbl=sysRtrGbl, rtrIcmpEchoPktTotalRcvd=rtrIcmpEchoPktTotalRcvd, rtrJitterPktNum=rtrJitterPktNum, maxBlockSizeFree=maxBlockSizeFree, sysRtrJitterEntry=sysRtrJitterEntry, rtrScheduleAgeOut=rtrScheduleAgeOut, rtrUdpechoTos=rtrUdpechoTos, rtrUdpechoDelay=rtrUdpechoDelay, rtrJitterState=rtrJitterState, taskOptions=taskOptions, rtrFlStaOutputMinPkts=rtrFlStaOutputMinPkts, cpuTaskPri=cpuTaskPri, cpuUtilTotalTicks=cpuUtilTotalTicks, cpuUtilTotalTimes=cpuUtilTotalTimes, rtrScheduleStartTime=rtrScheduleStartTime, rtrJitterMosMin=rtrJitterMosMin, taskStackFree=taskStackFree, taskStatus=taskStatus, rtrIcmpEchoEntityId=rtrIcmpEchoEntityId, rtrJitterMaxRtt=rtrJitterMaxRtt, rtrJitterSdMax=rtrJitterSdMax, sysIfStatistic=sysIfStatistic, sysMainBoardTemper=sysMainBoardTemper, rtrGroupRowStatus=rtrGroupRowStatus, rtrJitterTos=rtrJitterTos, rtrUdpechoRowStatus=rtrUdpechoRowStatus, cpuTaskName=cpuTaskName, rtrJitterVrfName=rtrJitterVrfName, sysIfDesc=sysIfDesc, rtrFlStaTmOutputMaxPkts=rtrFlStaTmOutputMaxPkts, rtrIcmpEchoPktLen=rtrIcmpEchoPktLen, rtrJitterDelaySdMin=rtrJitterDelaySdMin, rtrIcmpEchoSetDf=rtrIcmpEchoSetDf, cpuUtilCurrentUtil=cpuUtilCurrentUtil, sysRtrIcmpEchoEntry=sysRtrIcmpEchoEntry, taskStackPos=taskStackPos, rtrJitterTargetIp=rtrJitterTargetIp, sysRtrIcmpEchoTable=sysRtrIcmpEchoTable, rtrIcmpEchoPktTotalSend=rtrIcmpEchoPktTotalSend, taskStackMaxUsed=taskStackMaxUsed, rtrFlStaTmInputMinFlow=rtrFlStaTmInputMinFlow, sysCpuStatus=sysCpuStatus, rtrFlStaOutputMinFlow=rtrFlStaOutputMinFlow, sysRtrGroupMgt=sysRtrGroupMgt, rtrUdpechoSchInterval=rtrUdpechoSchInterval, sysIfInUnkownProtos=sysIfInUnkownProtos, rtrUdpechoTargetIp=rtrUdpechoTargetIp, sysRtrUdpechoEntry=sysRtrUdpechoEntry, rtrFlStatisticsInterval=rtrFlStatisticsInterval, taskStackBase=taskStackBase, rtrIcmpEchoIsScheduling=rtrIcmpEchoIsScheduling, taskErrorStatus=taskErrorStatus, rtrEntityName=rtrEntityName, sysIfOutOctets=sysIfOutOctets, rtrEntityLogType=rtrEntityLogType, rtrJitterSourcePort=rtrJitterSourcePort, sysRtrGroupTable=sysRtrGroupTable, sysIfIndex=sysIfIndex, sysRtrEntityMgt=sysRtrEntityMgt, rtrIcmpEchoExtendFlag=rtrIcmpEchoExtendFlag, cpuUtilEntry=cpuUtilEntry, memoryTotalBytes=memoryTotalBytes, sysRtrEntityTable=sysRtrEntityTable, rtrUdpechoPktSucc=rtrUdpechoPktSucc, sysIfPktPriStaTable=sysIfPktPriStaTable, rtrJitterPktInterval=rtrJitterPktInterval, rtrScheduleId=rtrScheduleId, sysMemory=sysMemory, sysRtrJitterMgt=sysRtrJitterMgt, sysRtrJitterTable=sysRtrJitterTable, rtrFlowStatisticsRowStatus=rtrFlowStatisticsRowStatus, sysCpuTaskTabView=sysCpuTaskTabView, rtrJitterRowStatus=rtrJitterRowStatus, sysIfOutNUcastPkts=sysIfOutNUcastPkts, sysRtrGroupEntry=sysRtrGroupEntry, sysCpuTemper=sysCpuTemper, rtrUdpechoPktLen=rtrUdpechoPktLen, sysIfOutUcastPkts=sysIfOutUcastPkts, sysRtrResponder=sysRtrResponder, cpuTaskEntry=cpuTaskEntry, taskStackSizeUsage=taskStackSizeUsage, sysCpu=sysCpu, taskTable=taskTable, cpuTaskAverageUtil=cpuTaskAverageUtil, rtrJitterPktLen=rtrJitterPktLen, rtrEntityType=rtrEntityType, sysRtrFlowStatisticsMgt=sysRtrFlowStatisticsMgt, cpuUtilDeltaTicks=cpuUtilDeltaTicks, rtrJitterDelayDsMax=rtrJitterDelayDsMax, rtrUdpechoState=rtrUdpechoState, sysRtrUdpechoTable=sysRtrUdpechoTable, sysIfInDiscards=sysIfInDiscards, rtrJitterCodec=rtrJitterCodec, mpios=mpios, rtrUdpechoDelayMin=rtrUdpechoDelayMin)
