...
TimeTicks, Bits, Counter64, ObjectIdentity, IpAddress, MibIdentifier, ModuleIdentity, Gauge32, Unsigned32, NotificationType, Integer32, iso, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter32 = mibBuilder.importSymbols("SNMPv2-SMI", "TimeTicks", "Bits", "Counter64", "ObjectIdentity", "IpAddress", "MibIdentifier", "ModuleIdentity", "Gauge32", "Unsigned32", "NotificationType", "Integer32", "iso", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter32")
...
mibBuilder.exportSymbols("MPIOS-MIB", sysCpuTemper=sysCpuTemper, rtrIcmpEchoPktNum=rtrIcmpEchoPktNum, rtrUdpechoSchInterval=rtrUdpechoSchInterval, rtrIcmpEchoIsScheduling=rtrIcmpEchoIsScheduling, rtrIcmpEchoSetDf=rtrIcmpEchoSetDf, taskStackSize=taskStackSize, sysIfInNUcastPkts=sysIfInNUcastPkts, taskMain=taskMain, sysRtrEntityEntry=sysRtrEntityEntry, rtrUdpechoTimeout=rtrUdpechoTimeout, rtrScheduleType=rtrScheduleType, rtrEntityName=rtrEntityName, cpuUtilDeltaTicks=cpuUtilDeltaTicks, sysTask=sysTask, rtrUdpechoSourceIp=rtrUdpechoSourceIp, sysMainBoardAlertTemper=sysMainBoardAlertTemper, sysIfPktPriStaEntry=sysIfPktPriStaEntry, sysCpuTaskTabView=sysCpuTaskTabView, rtrJitterPktLossSd=rtrJitterPktLossSd, rtrUdpechoDelayMin=rtrUdpechoDelayMin, taskEntry=taskEntry, rtrIcmpEchoSuccessRate=rtrIcmpEchoSuccessRate, sysRtrFlowStatisticsMgt=sysRtrFlowStatisticsMgt, cpuTaskId=cpuTaskId, sysIfIndex=sysIfIndex, rtrFlStaOutputMaxPkts=rtrFlStaOutputMaxPkts, sysRtrUdpechoMgt=sysRtrUdpechoMgt, cpuUtilTotalTimes=cpuUtilTotalTimes, sysIfOutDiscards=sysIfOutDiscards, rtrJitterSourcePort=rtrJitterSourcePort, rtrUdpechoDelayMax=rtrUdpechoDelayMax, rtrUdpechoTargetIp=rtrUdpechoTargetIp, sysRtrUdpechoTable=sysRtrUdpechoTable, rtrJitterMinRtt=rtrJitterMinRtt, rtrUdpechoDelay=rtrUdpechoDelay, sysIfInDiscards=sysIfInDiscards, rtrScheduleObjId=rtrScheduleObjId, sysIfStatistic=sysIfStatistic, rtrUdpechoPktSucc=rtrUdpechoPktSucc, cpuUtilDeltaUtil=cpuUtilDeltaUtil, sysCpuStatus=sysCpuStatus, sysIfInOctets=sysIfInOctets, rtrJitterSdMin=rtrJitterSdMin, rtrJitterDelaySdMax=rtrJitterDelaySdMax, rtrJitterCodec=rtrJitterCodec, sysRtrFlowStatisticsTable=sysRtrFlowStatisticsTable, rtrFlStatisticsIfName=rtrFlStatisticsIfName, sysRtrJitterTable=sysRtrJitterTable, sysIfOutOctets=sysIfOutOctets, rtrGroupRowStatus=rtrGroupRowStatus, rtrIcmpEchoPktLen=rtrIcmpEchoPktLen, maxBlockSizeFree=maxBlockSizeFree, rtrJitterSchInterval=rtrJitterSchInterval, rtrJitterDelaySdMin=rtrJitterDelaySdMin, rtrJitterEntityId=rtrJitterEntityId, rtrJitterIcpifMax=rtrJitterIcpifMax, iosObjects=iosObjects, sysMemory=sysMemory, cpuUtilTotalTicks=cpuUtilTotalTicks, sysTemperature=sysTemperature, rtrFlStaInputMaxFlow=rtrFlStaInputMaxFlow, rtrScheduleRepeat=rtrScheduleRepeat, rtrJitterRowStatus=rtrJitterRowStatus, sysRtrCtrl=sysRtrCtrl, memoryTotalBytes=memoryTotalBytes, cpuUtilTotalUsedTicks=cpuUtilTotalUsedTicks, rtrUdpechoPktLen=rtrUdpechoPktLen, rtrEntityLogType=rtrEntityLogType, sysIfInUnkownProtos=sysIfInUnkownProtos, cpuTaskAverageUtil=cpuTaskAverageUtil, rtrGroupId=rtrGroupId, numBytesFree=numBytesFree, taskOptions=taskOptions, rtrGroupName=rtrGroupName, rtrIcmpEchoVrfName=rtrIcmpEchoVrfName, rtrIcmpEchoVerifyData=rtrIcmpEchoVerifyData, rtrIcmpEchoTos=rtrIcmpEchoTos, avgBlockSizeAlloc=avgBlockSizeAlloc, sysIfOutNUcastPkts=sysIfOutNUcastPkts, rtrFlStaInputMaxPkts=rtrFlStaInputMaxPkts, rtrFlStaTmInputMinPkts=rtrFlStaTmInputMinPkts, rtrGroupEntityMembers=rtrGroupEntityMembers, taskDescr=taskDescr, rtrFlStaTmOutputMinFlow=rtrFlStaTmOutputMinFlow, taskStatus=taskStatus, sysIfDesc=sysIfDesc, rtrScheduleAgeOut=rtrScheduleAgeOut, rtrJitterMosMax=rtrJitterMosMax, rtrUdpechoState=rtrUdpechoState, cpuUtilDeltaUsedTicks=cpuUtilDeltaUsedTicks, taskTable=taskTable, rtrFlStaTmOutputMaxPkts=rtrFlStaTmOutputMaxPkts, cpuUtilDeltaTimes=cpuUtilDeltaTimes, sysRtrJitterEntry=sysRtrJitterEntry, rtrJitterDelayDsMax=rtrJitterDelayDsMax, rtrUdpechoRowStatus=rtrUdpechoRowStatus, rtrScheduleStartTime=rtrScheduleStartTime, rtrScheduleInterval=rtrScheduleInterval, rtrIcmpEchoRowStatus=rtrIcmpEchoRowStatus, rtrScheduleId=rtrScheduleId, sysRtrGroupTable=sysRtrGroupTable, rtrIcmpEchoTimeout=rtrIcmpEchoTimeout, rtrJitterPktInterval=rtrJitterPktInterval, taskStackEnd=taskStackEnd, rtrFlStaInputMinPkts=rtrFlStaInputMinPkts, rtrEntityRowStatus=rtrEntityRowStatus, PYSNMP_MODULE_ID=mpios, rtrJitterDsMax=rtrJitterDsMax, rtrUdpechoSourcePort=rtrUdpechoSourcePort, rtrEntityId=rtrEntityId, sysIfPktPriStaTable=sysIfPktPriStaTable, rtrJitterSdMax=rtrJitterSdMax, rtrFlStaOutputMinFlow=rtrFlStaOutputMinFlow, cpuTaskName=cpuTaskName, rtrUdpechoTargetPort=rtrUdpechoTargetPort, sysIfOutUcastPkts=sysIfOutUcastPkts, taskStackMaxUsed=taskStackMaxUsed, rtrIcmpEchoExtendFlag=rtrIcmpEchoExtendFlag, sysRtrIcmpEchoMgt=sysRtrIcmpEchoMgt, rtrUdpechoVrfName=rtrUdpechoVrfName, rtrJitterMaxRtt=rtrJitterMaxRtt, cpuTaskTable=cpuTaskTable, rtrGroupInterval=rtrGroupInterval, sysRtrGroupEntry=sysRtrGroupEntry, sysRtrIcmpEchoEntry=sysRtrIcmpEchoEntry, avgBlockSizeFree=avgBlockSizeFree, sysIfInErrors=sysIfInErrors, sysIfOutErrors=sysIfOutErrors, rtrJitterPktLossDs=rtrJitterPktLossDs, cpuTaskDeltaUtil=cpuTaskDeltaUtil, sysRtrGbl=sysRtrGbl, rtrFlowStatisticsRowStatus=rtrFlowStatisticsRowStatus, cpuTaskCurrentUtil=cpuTaskCurrentUtil, cpuUtilCurrentUtil=cpuUtilCurrentUtil, taskStackPos=taskStackPos, sysRtrScheduleTable=sysRtrScheduleTable, taskErrorStatus=taskErrorStatus, cpuUtilEntry=cpuUtilEntry, checkCpuTimeInterval=checkCpuTimeInterval, mpios=mpios, taskStackPtr=taskStackPtr, sysRtrGroupMgt=sysRtrGroupMgt, rtrIcmpEchoPktTotalRcvd=rtrIcmpEchoPktTotalRcvd, taskStackSizeUsage=taskStackSizeUsage, rtrIcmpEchoEntityId=rtrIcmpEchoEntityId, rtrJitterTimeout=rtrJitterTimeout, numBlocksFree=numBlocksFree, rtrEntityThreshold=rtrEntityThreshold, rtrEntityLogFilter=rtrEntityLogFilter, rtrScheduleLifeFlag=rtrScheduleLifeFlag, rtrJitterTargetIp=rtrJitterTargetIp, rtrIcmpEchoTargetIp=rtrIcmpEchoTargetIp, cpuUtilTable=cpuUtilTable, sysIfInUcastPkts=sysIfInUcastPkts, sysRtrEntityMgt=sysRtrEntityMgt, rtrFlStaOutputMaxFlow=rtrFlStaOutputMaxFlow, rtrEntityLogMaxSize=rtrEntityLogMaxSize, rtrFlStatisticsInterval=rtrFlStatisticsInterval, rtrFlStatisticsEntityId=rtrFlStatisticsEntityId, sysIfPktPriority=sysIfPktPriority, sysRtrUdpechoEntry=sysRtrUdpechoEntry, sysRtrResponder=sysRtrResponder, rtrIcmpEchoSchInterval=rtrIcmpEchoSchInterval, taskId=taskId, rtrJitterIcpifMin=rtrJitterIcpifMin, rtrUdpechoPktLoss=rtrUdpechoPktLoss, cpuTaskPri=cpuTaskPri, rtrFlStaTmOutputMaxFlow=rtrFlStaTmOutputMaxFlow, rtrScheduleLifeTime=rtrScheduleLifeTime, sysCpuAlertTemper=sysCpuAlertTemper, sysNFI=sysNFI, rtrUdpechoEntityId=rtrUdpechoEntityId, sysRtrEntityTable=sysRtrEntityTable, cpuTaskTotalTicks=cpuTaskTotalTicks, cpuUtilCpuId=cpuUtilCpuId, rtrIcmpEchoSourceIp=rtrIcmpEchoSourceIp, rtrUdpechoTos=rtrUdpechoTos, rtrIcmpEchoPktTotalSend=rtrIcmpEchoPktTotalSend, rtrJitterDelayDsMin=rtrJitterDelayDsMin, rtrJitterMosMin=rtrJitterMosMin, rtrFlStaTmInputMaxFlow=rtrFlStaTmInputMaxFlow, rtrJitterState=rtrJitterState, rtrJitterPktNum=rtrJitterPktNum, rtrJitterTos=rtrJitterTos, rtrJitterPktLen=rtrJitterPktLen, rtrScheduleAfterTime=rtrScheduleAfterTime, sysRtrScheduleMgt=sysRtrScheduleMgt, numBlocksAlloc=numBlocksAlloc, taskStackFree=taskStackFree, allocBytesPercent=allocBytesPercent, sysMainBoardTemper=sysMainBoardTemper, sysRtrFlowStatisticsEntry=sysRtrFlowStatisticsEntry, sysRtrScheduleEntry=sysRtrScheduleEntry, rtrJitterTargetPort=rtrJitterTargetPort, cpuTaskEntry=cpuTaskEntry, sysCpu=sysCpu, taskName=taskName, numBytesAlloc=numBytesAlloc, iosSystem=iosSystem, sysAlertTrapInt=sysAlertTrapInt, taskPriority=taskPriority, cpuUtilAverageUtil=cpuUtilAverageUtil, rtrEntityType=rtrEntityType, rtrScheduleRowStatus=rtrScheduleRowStatus, rtrJitterVrfName=rtrJitterVrfName, rtrFlStaTmInputMaxPkts=rtrFlStaTmInputMaxPkts, rtrFlStaInputMinFlow=rtrFlStaInputMinFlow, EnabledStatus=EnabledStatus, rtrFlStaTmInputMinFlow=rtrFlStaTmInputMinFlow, rtrFlStaTmOutputMinPkts=rtrFlStaTmOutputMinPkts, rtrFlStaOutputMinPkts=rtrFlStaOutputMinPkts, cpuTaskDeltaTicks=cpuTaskDeltaTicks, taskStackBase=taskStackBase, sysRtrIcmpEchoTable=sysRtrIcmpEchoTable, rtrJitterDsMin=rtrJitterDsMin, sysRtrJitterMgt=sysRtrJitterMgt, sysIfPktPriStatistics=sysIfPktPriStatistics, rtrJitterSourceIp=rtrJitterSourceIp, rtrScheduleStartTimeFlag=rtrScheduleStartTimeFlag)
