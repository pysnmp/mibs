...
# ASN.1 source INNO-MIB
# Source digest sha256:bb9f29cc4c8f5b75cd61294d50e3d097cdc9267014a1e1b4b6acd322b9356107
...
l2Table = MibTable((1, 3, 6, 1, 4, 1, 6666, 1, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: l2Table.setStatus('mandatory')
if mibBuilder.loadTexts: l2Table.setDescription('ISDN layer2 table')
l2Entry = MibTableRow((1, 3, 6, 1, 4, 1, 6666, 1, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "INNO-MIB", "l2Label"))
...
l1PriTable = MibTable((1, 3, 6, 1, 4, 1, 6666, 1, 2), ).setMaxAccess("notaccessible")
...
l1PriEntry = MibTableRow((1, 3, 6, 1, 4, 1, 6666, 1, 2, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "INNO-MIB", "l1PriLabel"))
...
l1BriTable = MibTable((1, 3, 6, 1, 4, 1, 6666, 1, 3), ).setMaxAccess("notaccessible")
...
l1BriEntry = MibTableRow((1, 3, 6, 1, 4, 1, 6666, 1, 3, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "INNO-MIB", "l1BriLabel"))
...
l3Table = MibTable((1, 3, 6, 1, 4, 1, 6666, 1, 4), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: l3Table.setStatus('mandatory')
if mibBuilder.loadTexts: l3Table.setDescription('Layer3 table')
l3Entry = MibTableRow((1, 3, 6, 1, 4, 1, 6666, 1, 4, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "INNO-MIB", "l3Label"))
...
voiceIfTable = MibTable((1, 3, 6, 1, 4, 1, 6666, 2, 1, 1), ).setMaxAccess("notaccessible")
...
voiceIfEntry = MibTableRow((1, 3, 6, 1, 4, 1, 6666, 2, 1, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "INNO-MIB", "voiceIfIndex"), (0, "INNO-MIB", "voiceIfAliasIndex"))
...
mibBuilder.exportSymbols("INNO-MIB", DisplayString=DisplayString, gatekeeper=gatekeeper, gateway=gateway, innoAuthenticationFailure=innoAuthenticationFailure, innoColdStart=innoColdStart, innoDiagAlarm=innoDiagAlarm, innoDiagAlarmClear=innoDiagAlarmClear, innoIsdnFailure=innoIsdnFailure, innoLinkDown=innoLinkDown, innoLinkUp=innoLinkUp, innoWarmStart=innoWarmStart, innovaphone=innovaphone, isdn=isdn, l1BriEntry=l1BriEntry, l1BriLabel=l1BriLabel, l1BriState=l1BriState, l1BriTable=l1BriTable, l1Label=l1Label, l1PriEntry=l1PriEntry, l1PriErrAlarmInd=l1PriErrAlarmInd, l1PriErrCrc4=l1PriErrCrc4, l1PriErrFrameAlignmentLoss=l1PriErrFrameAlignmentLoss, l1PriErrFrameAlignmentTOut=l1PriErrFrameAlignmentTOut, l1PriErrRemAlarmInd=l1PriErrRemAlarmInd, l1PriErrSigLoss=l1PriErrSigLoss, l1PriErrSlip=l1PriErrSlip, l1PriLabel=l1PriLabel, l1PriState=l1PriState, l1PriTable=l1PriTable, l1PriTest=l1PriTest, l2Entry=l2Entry, l2Label=l2Label, l2Mode=l2Mode, l2State=l2State, l2Table=l2Table, l3CallsBoot=l3CallsBoot, l3Entry=l3Entry, l3Label=l3Label, l3NumBchan=l3NumBchan, l3NumBchanActive=l3NumBchanActive, l3Protocol=l3Protocol, l3Table=l3Table, trapDisplayStringParm=trapDisplayStringParm, trapDummyGroup=trapDummyGroup, trapGaugeParm=trapGaugeParm, trapIntegerParm=trapIntegerParm, voiceIfAddr=voiceIfAddr, voiceIfAliasIndex=voiceIfAliasIndex, voiceIfEntry=voiceIfEntry, voiceIfGwName=voiceIfGwName, voiceIfIndex=voiceIfIndex, voiceIfName=voiceIfName, voiceIfNumber=voiceIfNumber, voiceIfProduct=voiceIfProduct, voiceIfState=voiceIfState, voiceIfTable=voiceIfTable, voiceIfType=voiceIfType)
