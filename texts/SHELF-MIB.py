...
mibBuilder.exportSymbols("SHELF-MIB", softInventoryIndex=softInventoryIndex, ProtectionType=ProtectionType, hardInventoryBoard=hardInventoryBoard, laserALSRestart2s=laserALSRestart2s, softInventoryNumber=softInventoryNumber, boardSlotName=boardSlotName, adrHotReset=adrHotReset, boardProtectionType=boardProtectionType, boardExpectType=boardExpectType, boardMissingSev=boardMissingSev, eowSrcPtr=eowSrcPtr, EOWType=EOWType, boardListType=boardListType, hardInventoryDescr=hardInventoryDescr, ledStatus=ledStatus, adrFailHoldOnTime=adrFailHoldOnTime, laserEntry=laserEntry, boardDefectiveSev=boardDefectiveSev, laserNumber=laserNumber, hardInventoryTable=hardInventoryTable, ledType=ledType, boardNumber=boardNumber, softInventoryCode=softInventoryCode, board=board, led=led, boardListTable=boardListTable, hardInventoryNumber=hardInventoryNumber, softInventoryExt=softInventoryExt, LedType=LedType, boardFirstPortIndex=boardFirstPortIndex, softInv=softInv, laserIndex=laserIndex, boardListEntry=boardListEntry, ledPosition=ledPosition, ledEntry=ledEntry, eowSinkType=eowSinkType, ledTable=ledTable, boardActive=boardActive, eowSrcType=eowSrcType, laserALSEnable=laserALSEnable, holdTime=holdTime, PYSNMP_MODULE_ID=shelf, boardListIndex=boardListIndex, boardListNumber=boardListNumber, laserTxEnable=laserTxEnable, ledNumber=ledNumber, boardIndex=boardIndex, boardSeverity=boardSeverity, eowNumber=eowNumber, eowTable=eowTable, eowName=eowName, boardMonitor=boardMonitor, eowSinkPtr=eowSinkPtr, eowClockMode=eowClockMode, softInventoryType=softInventoryType, hardInventoryEntry=hardInventoryEntry, boardInsertFamily=boardInsertFamily, softInventoryEntry=softInventoryEntry, eowEntry=eowEntry, softInventoryBoard=softInventoryBoard, shelf=shelf, boardMismatchSev=boardMismatchSev, eowIndex=eowIndex, HoldTime=HoldTime, boardTable=boardTable, boardEntry=boardEntry, boardList=boardList, boardExpectFamily=boardExpectFamily, BoardFailure=BoardFailure, hardInventoryIndex=hardInventoryIndex, boardFailure=boardFailure, ledIndex=ledIndex, laserTable=laserTable, boardListSlot=boardListSlot, laserALSRestart90s=laserALSRestart90s, LedStatus=LedStatus, eow=eow, EOWClockMode=EOWClockMode, adrFailHoldOffTime=adrFailHoldOffTime, hardInv=hardInv, laser=laser, softInventoryTable=softInventoryTable, boardInsertType=boardInsertType)
