...
mibBuilder.exportSymbols("SHELF-MIB", laser=laser, ProtectionType=ProtectionType, adrFailHoldOffTime=adrFailHoldOffTime, ledStatus=ledStatus, boardTable=boardTable, eow=eow, EOWType=EOWType, eowEntry=eowEntry, PYSNMP_MODULE_ID=shelf, boardProtectionType=boardProtectionType, softInventoryExt=softInventoryExt, boardListIndex=boardListIndex, boardSlotName=boardSlotName, laserEntry=laserEntry, softInventoryBoard=softInventoryBoard, hardInventoryIndex=hardInventoryIndex, softInventoryType=softInventoryType, laserALSRestart90s=laserALSRestart90s, boardEntry=boardEntry, laserTable=laserTable, softInventoryTable=softInventoryTable, eowSrcPtr=eowSrcPtr, ledEntry=ledEntry, hardInventoryBoard=hardInventoryBoard, laserALSRestart2s=laserALSRestart2s, boardList=boardList, boardListType=boardListType, softInv=softInv, boardListSlot=boardListSlot, boardNumber=boardNumber, LedType=LedType, softInventoryEntry=softInventoryEntry, softInventoryNumber=softInventoryNumber, shelf=shelf, adrHotReset=adrHotReset, adrFailHoldOnTime=adrFailHoldOnTime, HoldTime=HoldTime, boardDefectiveSev=boardDefectiveSev, boardActive=boardActive, led=led, boardInsertType=boardInsertType, boardListNumber=boardListNumber, eowSinkPtr=eowSinkPtr, boardMismatchSev=boardMismatchSev, laserIndex=laserIndex, hardInventoryDescr=hardInventoryDescr, eowNumber=eowNumber, board=board, eowSinkType=eowSinkType, ledNumber=ledNumber, boardIndex=boardIndex, boardExpectFamily=boardExpectFamily, eowName=eowName, ledIndex=ledIndex, holdTime=holdTime, EOWClockMode=EOWClockMode, hardInv=hardInv, ledTable=ledTable, eowIndex=eowIndex, boardListTable=boardListTable, boardSeverity=boardSeverity, ledType=ledType, eowTable=eowTable, boardInsertFamily=boardInsertFamily, boardListEntry=boardListEntry, laserNumber=laserNumber, boardMissingSev=boardMissingSev, softInventoryIndex=softInventoryIndex, laserTxEnable=laserTxEnable, eowClockMode=eowClockMode, hardInventoryTable=hardInventoryTable, boardFailure=boardFailure, eowSrcType=eowSrcType, hardInventoryNumber=hardInventoryNumber, boardExpectType=boardExpectType, laserALSEnable=laserALSEnable, boardMonitor=boardMonitor, hardInventoryEntry=hardInventoryEntry, softInventoryCode=softInventoryCode, BoardFailure=BoardFailure, LedStatus=LedStatus, ledPosition=ledPosition, boardFirstPortIndex=boardFirstPortIndex)
