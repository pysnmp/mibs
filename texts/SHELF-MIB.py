...
mibBuilder.exportSymbols("SHELF-MIB", softInventoryExt=softInventoryExt, boardExpectFamily=boardExpectFamily, EOWType=EOWType, softInventoryTable=softInventoryTable, softInventoryBoard=softInventoryBoard, eowClockMode=eowClockMode, boardListTable=boardListTable, boardMismatchSev=boardMismatchSev, eowEntry=eowEntry, holdTime=holdTime, shelf=shelf, eowTable=eowTable, ledType=ledType, eowName=eowName, laserEntry=laserEntry, laserALSRestart90s=laserALSRestart90s, boardInsertFamily=boardInsertFamily, hardInventoryEntry=hardInventoryEntry, laserIndex=laserIndex, boardListIndex=boardListIndex, boardIndex=boardIndex, eowSrcPtr=eowSrcPtr, softInv=softInv, ledEntry=ledEntry, ledStatus=ledStatus, softInventoryNumber=softInventoryNumber, boardFailure=boardFailure, eowNumber=eowNumber, led=led, boardProtectionType=boardProtectionType, laserTxEnable=laserTxEnable, boardInsertType=boardInsertType, softInventoryCode=softInventoryCode, eowSinkType=eowSinkType, boardNumber=boardNumber, softInventoryIndex=softInventoryIndex, boardListType=boardListType, ledIndex=ledIndex, boardMissingSev=boardMissingSev, boardSeverity=boardSeverity, adrHotReset=adrHotReset, boardListEntry=boardListEntry, adrFailHoldOffTime=adrFailHoldOffTime, eowIndex=eowIndex, ledTable=ledTable, HoldTime=HoldTime, laserNumber=laserNumber, softInventoryEntry=softInventoryEntry, laser=laser, boardExpectType=boardExpectType, LedType=LedType, boardActive=boardActive, laserALSRestart2s=laserALSRestart2s, hardInventoryDescr=hardInventoryDescr, eow=eow, boardListSlot=boardListSlot, ProtectionType=ProtectionType, hardInv=hardInv, ledPosition=ledPosition, hardInventoryTable=hardInventoryTable, boardEntry=boardEntry, EOWClockMode=EOWClockMode, hardInventoryBoard=hardInventoryBoard, boardMonitor=boardMonitor, eowSinkPtr=eowSinkPtr, LedStatus=LedStatus, laserALSEnable=laserALSEnable, boardTable=boardTable, boardFirstPortIndex=boardFirstPortIndex, boardListNumber=boardListNumber, eowSrcType=eowSrcType, boardList=boardList, PYSNMP_MODULE_ID=shelf, hardInventoryNumber=hardInventoryNumber, BoardFailure=BoardFailure, boardSlotName=boardSlotName, boardDefectiveSev=boardDefectiveSev, hardInventoryIndex=hardInventoryIndex, board=board, adrFailHoldOnTime=adrFailHoldOnTime, laserTable=laserTable, ledNumber=ledNumber, softInventoryType=softInventoryType)
