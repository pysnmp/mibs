...
mibBuilder.exportSymbols("SHELF-MIB", hardInventoryIndex=hardInventoryIndex, ledNumber=ledNumber, hardInv=hardInv, boardActive=boardActive, ledIndex=ledIndex, eowTable=eowTable, softInventoryIndex=softInventoryIndex, LedStatus=LedStatus, eow=eow, eowSrcType=eowSrcType, laserTable=laserTable, boardSeverity=boardSeverity, board=board, PYSNMP_MODULE_ID=shelf, laserALSEnable=laserALSEnable, adrFailHoldOnTime=adrFailHoldOnTime, boardExpectType=boardExpectType, shelf=shelf, boardListEntry=boardListEntry, led=led, laserNumber=laserNumber, softInventoryCode=softInventoryCode, softInventoryEntry=softInventoryEntry, laserTxEnable=laserTxEnable, boardFailure=boardFailure, eowEntry=eowEntry, softInventoryExt=softInventoryExt, laserALSRestart90s=laserALSRestart90s, boardFirstPortIndex=boardFirstPortIndex, boardEntry=boardEntry, BoardFailure=BoardFailure, softInventoryBoard=softInventoryBoard, LedType=LedType, holdTime=holdTime, hardInventoryTable=hardInventoryTable, boardTable=boardTable, boardListNumber=boardListNumber, boardIndex=boardIndex, HoldTime=HoldTime, adrHotReset=adrHotReset, hardInventoryNumber=hardInventoryNumber, boardInsertType=boardInsertType, boardListType=boardListType, boardMissingSev=boardMissingSev, hardInventoryEntry=hardInventoryEntry, eowName=eowName, boardInsertFamily=boardInsertFamily, laserIndex=laserIndex, eowIndex=eowIndex, ProtectionType=ProtectionType, boardExpectFamily=boardExpectFamily, ledStatus=ledStatus, boardListSlot=boardListSlot, boardSlotName=boardSlotName, softInv=softInv, adrFailHoldOffTime=adrFailHoldOffTime, softInventoryType=softInventoryType, eowSinkType=eowSinkType, boardProtectionType=boardProtectionType, boardMismatchSev=boardMismatchSev, boardList=boardList, softInventoryTable=softInventoryTable, EOWType=EOWType, ledPosition=ledPosition, ledType=ledType, ledTable=ledTable, boardDefectiveSev=boardDefectiveSev, laser=laser, boardListTable=boardListTable, laserEntry=laserEntry, boardListIndex=boardListIndex, hardInventoryBoard=hardInventoryBoard, eowClockMode=eowClockMode, laserALSRestart2s=laserALSRestart2s, eowSinkPtr=eowSinkPtr, eowSrcPtr=eowSrcPtr, eowNumber=eowNumber, EOWClockMode=EOWClockMode, ledEntry=ledEntry, boardMonitor=boardMonitor, hardInventoryDescr=hardInventoryDescr, boardNumber=boardNumber, softInventoryNumber=softInventoryNumber)
