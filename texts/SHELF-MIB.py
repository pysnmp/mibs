...
mibBuilder.exportSymbols("SHELF-MIB", hardInventoryEntry=hardInventoryEntry, eowTable=eowTable, boardFirstPortIndex=boardFirstPortIndex, laserALSRestart2s=laserALSRestart2s, board=board, LedType=LedType, boardListNumber=boardListNumber, laserALSRestart90s=laserALSRestart90s, BoardFailure=BoardFailure, ledEntry=ledEntry, adrFailHoldOnTime=adrFailHoldOnTime, hardInventoryNumber=hardInventoryNumber, boardProtectionType=boardProtectionType, eowClockMode=eowClockMode, eowSinkPtr=eowSinkPtr, softInv=softInv, eow=eow, boardInsertFamily=boardInsertFamily, boardMissingSev=boardMissingSev, hardInventoryTable=hardInventoryTable, boardListIndex=boardListIndex, eowName=eowName, softInventoryTable=softInventoryTable, eowNumber=eowNumber, boardMonitor=boardMonitor, boardEntry=boardEntry, laserNumber=laserNumber, ledType=ledType, boardSeverity=boardSeverity, PYSNMP_MODULE_ID=shelf, boardFailure=boardFailure, boardListEntry=boardListEntry, adrHotReset=adrHotReset, boardExpectFamily=boardExpectFamily, eowSrcPtr=eowSrcPtr, softInventoryType=softInventoryType, softInventoryNumber=softInventoryNumber, softInventoryCode=softInventoryCode, ProtectionType=ProtectionType, boardActive=boardActive, ledIndex=ledIndex, hardInventoryDescr=hardInventoryDescr, shelf=shelf, softInventoryExt=softInventoryExt, softInventoryEntry=softInventoryEntry, HoldTime=HoldTime, boardSlotName=boardSlotName, laserEntry=laserEntry, EOWClockMode=EOWClockMode, boardList=boardList, boardListTable=boardListTable, laser=laser, adrFailHoldOffTime=adrFailHoldOffTime, hardInv=hardInv, boardDefectiveSev=boardDefectiveSev, hardInventoryIndex=hardInventoryIndex, ledNumber=ledNumber, laserTable=laserTable, eowSrcType=eowSrcType, boardTable=boardTable, LedStatus=LedStatus, boardListSlot=boardListSlot, ledTable=ledTable, holdTime=holdTime, softInventoryIndex=softInventoryIndex, ledStatus=ledStatus, hardInventoryBoard=hardInventoryBoard, EOWType=EOWType, laserTxEnable=laserTxEnable, boardNumber=boardNumber, laserIndex=laserIndex, boardMismatchSev=boardMismatchSev, eowSinkType=eowSinkType, eowIndex=eowIndex, eowEntry=eowEntry, ledPosition=ledPosition, boardInsertType=boardInsertType, boardListType=boardListType, boardIndex=boardIndex, led=led, boardExpectType=boardExpectType, softInventoryBoard=softInventoryBoard, laserALSEnable=laserALSEnable)
