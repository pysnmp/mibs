...
mibBuilder.exportSymbols("SHELF-MIB", eowSinkType=eowSinkType, shelf=shelf, boardListNumber=boardListNumber, softInventoryIndex=softInventoryIndex, eowSinkPtr=eowSinkPtr, boardListEntry=boardListEntry, eowIndex=eowIndex, boardMissingSev=boardMissingSev, boardMonitor=boardMonitor, boardProtectionType=boardProtectionType, HoldTime=HoldTime, adrHotReset=adrHotReset, laserTable=laserTable, boardFailure=boardFailure, adrFailHoldOffTime=adrFailHoldOffTime, softInventoryEntry=softInventoryEntry, boardTable=boardTable, softInventoryExt=softInventoryExt, boardMismatchSev=boardMismatchSev, hardInventoryNumber=hardInventoryNumber, softInventoryBoard=softInventoryBoard, boardListIndex=boardListIndex, boardExpectFamily=boardExpectFamily, softInv=softInv, boardExpectType=boardExpectType, boardListType=boardListType, boardActive=boardActive, boardFirstPortIndex=boardFirstPortIndex, ledIndex=ledIndex, ledType=ledType, eowSrcPtr=eowSrcPtr, boardListSlot=boardListSlot, hardInventoryBoard=hardInventoryBoard, ledTable=ledTable, softInventoryCode=softInventoryCode, PYSNMP_MODULE_ID=shelf, boardSeverity=boardSeverity, laserALSRestart90s=laserALSRestart90s, boardDefectiveSev=boardDefectiveSev, hardInventoryTable=hardInventoryTable, ProtectionType=ProtectionType, eowEntry=eowEntry, eowNumber=eowNumber, softInventoryTable=softInventoryTable, hardInventoryEntry=hardInventoryEntry, laserIndex=laserIndex, board=board, laserNumber=laserNumber, eowName=eowName, holdTime=holdTime, EOWClockMode=EOWClockMode, softInventoryType=softInventoryType, adrFailHoldOnTime=adrFailHoldOnTime, laserALSRestart2s=laserALSRestart2s, eowTable=eowTable, laserEntry=laserEntry, eow=eow, eowSrcType=eowSrcType, ledNumber=ledNumber, ledPosition=ledPosition, boardInsertFamily=boardInsertFamily, boardIndex=boardIndex, hardInventoryIndex=hardInventoryIndex, EOWType=EOWType, hardInventoryDescr=hardInventoryDescr, boardList=boardList, boardSlotName=boardSlotName, ledStatus=ledStatus, boardInsertType=boardInsertType, LedStatus=LedStatus, BoardFailure=BoardFailure, softInventoryNumber=softInventoryNumber, LedType=LedType, laserALSEnable=laserALSEnable, led=led, laser=laser, boardListTable=boardListTable, boardEntry=boardEntry, laserTxEnable=laserTxEnable, eowClockMode=eowClockMode, boardNumber=boardNumber, hardInv=hardInv, ledEntry=ledEntry)
