...
mibBuilder.exportSymbols("SHELF-MIB", board=board, boardFailure=boardFailure, hardInventoryEntry=hardInventoryEntry, hardInventoryDescr=hardInventoryDescr, softInventoryEntry=softInventoryEntry, ledType=ledType, LedType=LedType, boardExpectType=boardExpectType, laserIndex=laserIndex, eowIndex=eowIndex, boardSeverity=boardSeverity, boardInsertFamily=boardInsertFamily, laserTable=laserTable, eow=eow, eowName=eowName, HoldTime=HoldTime, BoardFailure=BoardFailure, boardListSlot=boardListSlot, ledTable=ledTable, boardDefectiveSev=boardDefectiveSev, softInventoryNumber=softInventoryNumber, ProtectionType=ProtectionType, softInventoryTable=softInventoryTable, EOWType=EOWType, laserEntry=laserEntry, boardListType=boardListType, laserNumber=laserNumber, softInventoryExt=softInventoryExt, PYSNMP_MODULE_ID=shelf, ledIndex=ledIndex, softInventoryBoard=softInventoryBoard, hardInventoryTable=hardInventoryTable, laserALSRestart90s=laserALSRestart90s, softInventoryType=softInventoryType, holdTime=holdTime, laserTxEnable=laserTxEnable, eowNumber=eowNumber, eowSinkType=eowSinkType, boardList=boardList, hardInventoryBoard=hardInventoryBoard, boardActive=boardActive, eowSrcType=eowSrcType, eowSrcPtr=eowSrcPtr, boardListEntry=boardListEntry, adrHotReset=adrHotReset, boardListTable=boardListTable, boardTable=boardTable, boardProtectionType=boardProtectionType, boardEntry=boardEntry, boardListNumber=boardListNumber, hardInventoryNumber=hardInventoryNumber, laser=laser, ledStatus=ledStatus, softInv=softInv, hardInv=hardInv, boardFirstPortIndex=boardFirstPortIndex, LedStatus=LedStatus, ledPosition=ledPosition, eowTable=eowTable, boardNumber=boardNumber, boardExpectFamily=boardExpectFamily, laserALSEnable=laserALSEnable, ledEntry=ledEntry, softInventoryIndex=softInventoryIndex, shelf=shelf, eowEntry=eowEntry, led=led, laserALSRestart2s=laserALSRestart2s, boardSlotName=boardSlotName, hardInventoryIndex=hardInventoryIndex, boardListIndex=boardListIndex, eowSinkPtr=eowSinkPtr, boardIndex=boardIndex, ledNumber=ledNumber, boardInsertType=boardInsertType, boardMismatchSev=boardMismatchSev, EOWClockMode=EOWClockMode, boardMonitor=boardMonitor, adrFailHoldOffTime=adrFailHoldOffTime, eowClockMode=eowClockMode, softInventoryCode=softInventoryCode, adrFailHoldOnTime=adrFailHoldOnTime, boardMissingSev=boardMissingSev)
