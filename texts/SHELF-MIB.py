...
mibBuilder.exportSymbols("SHELF-MIB", boardExpectFamily=boardExpectFamily, softInventoryBoard=softInventoryBoard, hardInventoryIndex=hardInventoryIndex, laser=laser, laserALSRestart90s=laserALSRestart90s, EOWClockMode=EOWClockMode, laserIndex=laserIndex, softInventoryTable=softInventoryTable, adrHotReset=adrHotReset, boardInsertFamily=boardInsertFamily, boardListEntry=boardListEntry, eowTable=eowTable, boardMismatchSev=boardMismatchSev, boardProtectionType=boardProtectionType, boardListType=boardListType, boardExpectType=boardExpectType, softInventoryExt=softInventoryExt, hardInventoryEntry=hardInventoryEntry, boardInsertType=boardInsertType, ledStatus=ledStatus, boardList=boardList, hardInventoryNumber=hardInventoryNumber, laserEntry=laserEntry, LedType=LedType, laserTxEnable=laserTxEnable, ledTable=ledTable, boardIndex=boardIndex, boardListTable=boardListTable, boardTable=boardTable, ledIndex=ledIndex, adrFailHoldOffTime=adrFailHoldOffTime, laserTable=laserTable, softInv=softInv, ledEntry=ledEntry, boardEntry=boardEntry, hardInv=hardInv, eowSrcPtr=eowSrcPtr, laserALSRestart2s=laserALSRestart2s, boardActive=boardActive, eowSinkType=eowSinkType, boardListNumber=boardListNumber, eowName=eowName, boardMissingSev=boardMissingSev, boardDefectiveSev=boardDefectiveSev, led=led, softInventoryIndex=softInventoryIndex, boardMonitor=boardMonitor, laserNumber=laserNumber, ledType=ledType, ledNumber=ledNumber, softInventoryType=softInventoryType, boardNumber=boardNumber, hardInventoryDescr=hardInventoryDescr, HoldTime=HoldTime, LedStatus=LedStatus, eowEntry=eowEntry, boardFirstPortIndex=boardFirstPortIndex, boardSeverity=boardSeverity, softInventoryNumber=softInventoryNumber, ledPosition=ledPosition, BoardFailure=BoardFailure, hardInventoryTable=hardInventoryTable, shelf=shelf, softInventoryCode=softInventoryCode, holdTime=holdTime, boardListSlot=boardListSlot, board=board, laserALSEnable=laserALSEnable, boardFailure=boardFailure, ProtectionType=ProtectionType, softInventoryEntry=softInventoryEntry, eowClockMode=eowClockMode, eowSinkPtr=eowSinkPtr, adrFailHoldOnTime=adrFailHoldOnTime, EOWType=EOWType, boardListIndex=boardListIndex, PYSNMP_MODULE_ID=shelf, boardSlotName=boardSlotName, eowNumber=eowNumber, hardInventoryBoard=hardInventoryBoard, eowSrcType=eowSrcType, eowIndex=eowIndex, eow=eow)
