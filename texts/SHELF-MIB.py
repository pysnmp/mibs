...
mibBuilder.exportSymbols("SHELF-MIB", boardInsertType=boardInsertType, softInventoryNumber=softInventoryNumber, boardListIndex=boardListIndex, eowEntry=eowEntry, ledEntry=ledEntry, boardInsertFamily=boardInsertFamily, boardIndex=boardIndex, softInventoryType=softInventoryType, HoldTime=HoldTime, boardNumber=boardNumber, laserTxEnable=laserTxEnable, boardSeverity=boardSeverity, boardListEntry=boardListEntry, ledStatus=ledStatus, boardList=boardList, boardFailure=boardFailure, boardProtectionType=boardProtectionType, LedType=LedType, eowClockMode=eowClockMode, BoardFailure=BoardFailure, ledNumber=ledNumber, boardFirstPortIndex=boardFirstPortIndex, hardInventoryBoard=hardInventoryBoard, adrFailHoldOffTime=adrFailHoldOffTime, shelf=shelf, adrFailHoldOnTime=adrFailHoldOnTime, laserIndex=laserIndex, hardInventoryNumber=hardInventoryNumber, softInventoryIndex=softInventoryIndex, boardListSlot=boardListSlot, boardListNumber=boardListNumber, laserALSEnable=laserALSEnable, boardActive=boardActive, eowNumber=eowNumber, boardExpectType=boardExpectType, boardSlotName=boardSlotName, laserALSRestart90s=laserALSRestart90s, hardInventoryIndex=hardInventoryIndex, softInventoryTable=softInventoryTable, eowTable=eowTable, boardMismatchSev=boardMismatchSev, hardInventoryTable=hardInventoryTable, laserALSRestart2s=laserALSRestart2s, softInventoryExt=softInventoryExt, ledIndex=ledIndex, boardListTable=boardListTable, ledTable=ledTable, holdTime=holdTime, ProtectionType=ProtectionType, boardMissingSev=boardMissingSev, eowName=eowName, eowSinkType=eowSinkType, hardInventoryEntry=hardInventoryEntry, board=board, ledPosition=ledPosition, eowSrcType=eowSrcType, laserTable=laserTable, softInv=softInv, softInventoryEntry=softInventoryEntry, EOWType=EOWType, boardExpectFamily=boardExpectFamily, PYSNMP_MODULE_ID=shelf, led=led, eow=eow, laserEntry=laserEntry, boardDefectiveSev=boardDefectiveSev, EOWClockMode=EOWClockMode, eowIndex=eowIndex, adrHotReset=adrHotReset, boardTable=boardTable, hardInv=hardInv, laserNumber=laserNumber, eowSrcPtr=eowSrcPtr, LedStatus=LedStatus, laser=laser, softInventoryBoard=softInventoryBoard, boardMonitor=boardMonitor, boardListType=boardListType, ledType=ledType, hardInventoryDescr=hardInventoryDescr, softInventoryCode=softInventoryCode, boardEntry=boardEntry, eowSinkPtr=eowSinkPtr)
