...
mibBuilder.exportSymbols("SHELF-MIB", boardListIndex=boardListIndex, adrFailHoldOffTime=adrFailHoldOffTime, EOWClockMode=EOWClockMode, boardListEntry=boardListEntry, eowIndex=eowIndex, ledEntry=ledEntry, ledType=ledType, softInventoryIndex=softInventoryIndex, laserALSRestart90s=laserALSRestart90s, laserIndex=laserIndex, boardInsertFamily=boardInsertFamily, boardExpectType=boardExpectType, eowSinkType=eowSinkType, ledPosition=ledPosition, eowTable=eowTable, boardMissingSev=boardMissingSev, boardTable=boardTable, boardIndex=boardIndex, hardInventoryBoard=hardInventoryBoard, boardNumber=boardNumber, laserTable=laserTable, boardActive=boardActive, softInventoryType=softInventoryType, hardInventoryDescr=hardInventoryDescr, laserALSEnable=laserALSEnable, laserNumber=laserNumber, boardDefectiveSev=boardDefectiveSev, softInventoryTable=softInventoryTable, boardFirstPortIndex=boardFirstPortIndex, adrHotReset=adrHotReset, ledStatus=ledStatus, hardInventoryIndex=hardInventoryIndex, eowSrcType=eowSrcType, eowEntry=eowEntry, boardListType=boardListType, softInventoryBoard=softInventoryBoard, boardFailure=boardFailure, laserALSRestart2s=laserALSRestart2s, ledNumber=ledNumber, boardList=boardList, boardListTable=boardListTable, eowNumber=eowNumber, boardEntry=boardEntry, eow=eow, board=board, laserEntry=laserEntry, ProtectionType=ProtectionType, boardProtectionType=boardProtectionType, eowSrcPtr=eowSrcPtr, holdTime=holdTime, LedStatus=LedStatus, eowSinkPtr=eowSinkPtr, laserTxEnable=laserTxEnable, ledIndex=ledIndex, hardInventoryTable=hardInventoryTable, hardInv=hardInv, eowName=eowName, BoardFailure=BoardFailure, softInventoryCode=softInventoryCode, softInventoryEntry=softInventoryEntry, softInv=softInv, EOWType=EOWType, boardListNumber=boardListNumber, boardExpectFamily=boardExpectFamily, boardInsertType=boardInsertType, hardInventoryNumber=hardInventoryNumber, boardListSlot=boardListSlot, LedType=LedType, boardMismatchSev=boardMismatchSev, PYSNMP_MODULE_ID=shelf, HoldTime=HoldTime, hardInventoryEntry=hardInventoryEntry, adrFailHoldOnTime=adrFailHoldOnTime, ledTable=ledTable, shelf=shelf, led=led, softInventoryNumber=softInventoryNumber, eowClockMode=eowClockMode, boardSlotName=boardSlotName, laser=laser, softInventoryExt=softInventoryExt, boardMonitor=boardMonitor, boardSeverity=boardSeverity)
