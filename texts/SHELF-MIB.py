...
mibBuilder.exportSymbols("SHELF-MIB", shelf=shelf, laserALSRestart2s=laserALSRestart2s, laserTable=laserTable, laserTxEnable=laserTxEnable, ledStatus=ledStatus, BoardFailure=BoardFailure, ledTable=ledTable, ledType=ledType, board=board, boardListType=boardListType, softInventoryBoard=softInventoryBoard, adrFailHoldOnTime=adrFailHoldOnTime, boardList=boardList, PYSNMP_MODULE_ID=shelf, boardEntry=boardEntry, boardTable=boardTable, eowEntry=eowEntry, eowTable=eowTable, softInv=softInv, adrFailHoldOffTime=adrFailHoldOffTime, softInventoryEntry=softInventoryEntry, boardListNumber=boardListNumber, boardFailure=boardFailure, boardListIndex=boardListIndex, laserALSRestart90s=laserALSRestart90s, boardInsertType=boardInsertType, boardListEntry=boardListEntry, eowClockMode=eowClockMode, hardInv=hardInv, boardMissingSev=boardMissingSev, hardInventoryTable=hardInventoryTable, boardIndex=boardIndex, softInventoryTable=softInventoryTable, hardInventoryIndex=hardInventoryIndex, softInventoryCode=softInventoryCode, led=led, boardMismatchSev=boardMismatchSev, LedStatus=LedStatus, holdTime=holdTime, softInventoryNumber=softInventoryNumber, eowSinkPtr=eowSinkPtr, eowIndex=eowIndex, boardDefectiveSev=boardDefectiveSev, laser=laser, eowName=eowName, boardFirstPortIndex=boardFirstPortIndex, boardListTable=boardListTable, softInventoryExt=softInventoryExt, ledNumber=ledNumber, boardInsertFamily=boardInsertFamily, laserALSEnable=laserALSEnable, EOWClockMode=EOWClockMode, ProtectionType=ProtectionType, boardActive=boardActive, eowSinkType=eowSinkType, laserNumber=laserNumber, ledEntry=ledEntry, LedType=LedType, softInventoryType=softInventoryType, hardInventoryBoard=hardInventoryBoard, boardProtectionType=boardProtectionType, softInventoryIndex=softInventoryIndex, ledIndex=ledIndex, laserEntry=laserEntry, HoldTime=HoldTime, hardInventoryDescr=hardInventoryDescr, laserIndex=laserIndex, hardInventoryNumber=hardInventoryNumber, boardListSlot=boardListSlot, boardMonitor=boardMonitor, EOWType=EOWType, boardNumber=boardNumber, boardSlotName=boardSlotName, boardExpectFamily=boardExpectFamily, boardExpectType=boardExpectType, hardInventoryEntry=hardInventoryEntry, eowSrcPtr=eowSrcPtr, ledPosition=ledPosition, eowSrcType=eowSrcType, adrHotReset=adrHotReset, eow=eow, eowNumber=eowNumber, boardSeverity=boardSeverity)
