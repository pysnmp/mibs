...
mibBuilder.exportSymbols("SHELF-MIB", laserALSRestart2s=laserALSRestart2s, hardInventoryDescr=hardInventoryDescr, eowSrcPtr=eowSrcPtr, hardInventoryNumber=hardInventoryNumber, eowIndex=eowIndex, boardList=boardList, softInventoryTable=softInventoryTable, laserTxEnable=laserTxEnable, eowNumber=eowNumber, boardSeverity=boardSeverity, ledStatus=ledStatus, boardListNumber=boardListNumber, softInventoryNumber=softInventoryNumber, PYSNMP_MODULE_ID=shelf, boardInsertType=boardInsertType, laserALSEnable=laserALSEnable, eowSinkType=eowSinkType, softInventoryCode=softInventoryCode, eowEntry=eowEntry, adrFailHoldOffTime=adrFailHoldOffTime, boardExpectType=boardExpectType, boardSlotName=boardSlotName, laser=laser, boardEntry=boardEntry, softInv=softInv, boardMismatchSev=boardMismatchSev, eowSinkPtr=eowSinkPtr, eowTable=eowTable, boardMissingSev=boardMissingSev, ledTable=ledTable, EOWType=EOWType, eowSrcType=eowSrcType, boardListTable=boardListTable, boardListIndex=boardListIndex, boardInsertFamily=boardInsertFamily, ledPosition=ledPosition, eowClockMode=eowClockMode, softInventoryType=softInventoryType, ledIndex=ledIndex, boardListType=boardListType, boardActive=boardActive, LedStatus=LedStatus, boardExpectFamily=boardExpectFamily, HoldTime=HoldTime, boardFirstPortIndex=boardFirstPortIndex, hardInventoryEntry=hardInventoryEntry, LedType=LedType, BoardFailure=BoardFailure, laserTable=laserTable, ledEntry=ledEntry, laserIndex=laserIndex, eow=eow, shelf=shelf, boardProtectionType=boardProtectionType, hardInventoryTable=hardInventoryTable, boardDefectiveSev=boardDefectiveSev, EOWClockMode=EOWClockMode, boardFailure=boardFailure, boardIndex=boardIndex, boardListSlot=boardListSlot, laserNumber=laserNumber, softInventoryBoard=softInventoryBoard, adrHotReset=adrHotReset, hardInventoryIndex=hardInventoryIndex, boardNumber=boardNumber, holdTime=holdTime, softInventoryExt=softInventoryExt, eowName=eowName, boardMonitor=boardMonitor, boardTable=boardTable, laserEntry=laserEntry, hardInv=hardInv, softInventoryIndex=softInventoryIndex, adrFailHoldOnTime=adrFailHoldOnTime, led=led, hardInventoryBoard=hardInventoryBoard, ledNumber=ledNumber, board=board, laserALSRestart90s=laserALSRestart90s, boardListEntry=boardListEntry, ProtectionType=ProtectionType, softInventoryEntry=softInventoryEntry, ledType=ledType)
