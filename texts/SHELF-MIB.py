...
mibBuilder.exportSymbols("SHELF-MIB", eowIndex=eowIndex, boardListEntry=boardListEntry, ledNumber=ledNumber, boardMonitor=boardMonitor, adrHotReset=adrHotReset, boardActive=boardActive, laserALSRestart90s=laserALSRestart90s, softInventoryExt=softInventoryExt, laserNumber=laserNumber, boardFirstPortIndex=boardFirstPortIndex, boardSeverity=boardSeverity, hardInventoryTable=hardInventoryTable, softInventoryType=softInventoryType, boardListNumber=boardListNumber, boardMissingSev=boardMissingSev, eowSrcPtr=eowSrcPtr, ledPosition=ledPosition, boardListType=boardListType, softInv=softInv, BoardFailure=BoardFailure, boardMismatchSev=boardMismatchSev, hardInventoryBoard=hardInventoryBoard, eowSrcType=eowSrcType, boardEntry=boardEntry, eowNumber=eowNumber, hardInventoryEntry=hardInventoryEntry, softInventoryCode=softInventoryCode, laser=laser, board=board, HoldTime=HoldTime, ProtectionType=ProtectionType, holdTime=holdTime, laserIndex=laserIndex, adrFailHoldOffTime=adrFailHoldOffTime, ledType=ledType, eowClockMode=eowClockMode, eowSinkPtr=eowSinkPtr, boardExpectFamily=boardExpectFamily, ledStatus=ledStatus, ledTable=ledTable, boardListTable=boardListTable, shelf=shelf, eowName=eowName, boardDefectiveSev=boardDefectiveSev, boardExpectType=boardExpectType, boardListIndex=boardListIndex, laserALSRestart2s=laserALSRestart2s, hardInventoryNumber=hardInventoryNumber, LedStatus=LedStatus, laserALSEnable=laserALSEnable, eowEntry=eowEntry, boardInsertType=boardInsertType, boardList=boardList, led=led, boardNumber=boardNumber, ledIndex=ledIndex, laserTable=laserTable, PYSNMP_MODULE_ID=shelf, eowTable=eowTable, boardInsertFamily=boardInsertFamily, laserEntry=laserEntry, laserTxEnable=laserTxEnable, EOWClockMode=EOWClockMode, boardSlotName=boardSlotName, boardProtectionType=boardProtectionType, boardTable=boardTable, LedType=LedType, eowSinkType=eowSinkType, hardInventoryIndex=hardInventoryIndex, softInventoryNumber=softInventoryNumber, hardInv=hardInv, softInventoryTable=softInventoryTable, softInventoryIndex=softInventoryIndex, softInventoryBoard=softInventoryBoard, ledEntry=ledEntry, hardInventoryDescr=hardInventoryDescr, EOWType=EOWType, softInventoryEntry=softInventoryEntry, eow=eow, boardListSlot=boardListSlot, adrFailHoldOnTime=adrFailHoldOnTime, boardFailure=boardFailure, boardIndex=boardIndex)
