...
mibBuilder.exportSymbols("SHELF-MIB", HoldTime=HoldTime, boardListSlot=boardListSlot, laserTable=laserTable, ledEntry=ledEntry, softInventoryExt=softInventoryExt, ledPosition=ledPosition, eowName=eowName, hardInventoryDescr=hardInventoryDescr, eowEntry=eowEntry, eowNumber=eowNumber, eow=eow, boardFirstPortIndex=boardFirstPortIndex, boardMissingSev=boardMissingSev, boardExpectFamily=boardExpectFamily, softInventoryType=softInventoryType, softInventoryCode=softInventoryCode, boardFailure=boardFailure, eowIndex=eowIndex, laserALSEnable=laserALSEnable, softInv=softInv, ledNumber=ledNumber, laserALSRestart90s=laserALSRestart90s, boardActive=boardActive, laserALSRestart2s=laserALSRestart2s, EOWClockMode=EOWClockMode, boardList=boardList, boardListIndex=boardListIndex, shelf=shelf, boardTable=boardTable, boardMismatchSev=boardMismatchSev, boardMonitor=boardMonitor, softInventoryEntry=softInventoryEntry, laserNumber=laserNumber, boardDefectiveSev=boardDefectiveSev, eowTable=eowTable, eowSinkPtr=eowSinkPtr, boardListNumber=boardListNumber, boardIndex=boardIndex, LedType=LedType, boardListTable=boardListTable, boardNumber=boardNumber, boardInsertType=boardInsertType, EOWType=EOWType, laser=laser, eowSrcPtr=eowSrcPtr, softInventoryTable=softInventoryTable, boardSeverity=boardSeverity, PYSNMP_MODULE_ID=shelf, adrHotReset=adrHotReset, ledStatus=ledStatus, adrFailHoldOnTime=adrFailHoldOnTime, LedStatus=LedStatus, eowSinkType=eowSinkType, laserIndex=laserIndex, BoardFailure=BoardFailure, hardInventoryTable=hardInventoryTable, eowClockMode=eowClockMode, hardInv=hardInv, hardInventoryIndex=hardInventoryIndex, eowSrcType=eowSrcType, holdTime=holdTime, hardInventoryBoard=hardInventoryBoard, led=led, boardListType=boardListType, hardInventoryEntry=hardInventoryEntry, ProtectionType=ProtectionType, board=board, ledTable=ledTable, ledIndex=ledIndex, boardExpectType=boardExpectType, boardListEntry=boardListEntry, softInventoryIndex=softInventoryIndex, adrFailHoldOffTime=adrFailHoldOffTime, boardSlotName=boardSlotName, softInventoryNumber=softInventoryNumber, laserTxEnable=laserTxEnable, boardInsertFamily=boardInsertFamily, boardProtectionType=boardProtectionType, laserEntry=laserEntry, boardEntry=boardEntry, softInventoryBoard=softInventoryBoard, hardInventoryNumber=hardInventoryNumber, ledType=ledType)
