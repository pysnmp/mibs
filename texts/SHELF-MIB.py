...
mibBuilder.exportSymbols("SHELF-MIB", hardInventoryTable=hardInventoryTable, laserNumber=laserNumber, eowClockMode=eowClockMode, boardListType=boardListType, softInv=softInv, boardMonitor=boardMonitor, EOWClockMode=EOWClockMode, eowName=eowName, boardNumber=boardNumber, LedType=LedType, laserTable=laserTable, ledPosition=ledPosition, eowSinkType=eowSinkType, board=board, boardInsertType=boardInsertType, boardEntry=boardEntry, eow=eow, laserALSRestart90s=laserALSRestart90s, boardExpectType=boardExpectType, boardListSlot=boardListSlot, softInventoryCode=softInventoryCode, eowTable=eowTable, eowSrcType=eowSrcType, boardFirstPortIndex=boardFirstPortIndex, ledType=ledType, softInventoryTable=softInventoryTable, boardMissingSev=boardMissingSev, boardListEntry=boardListEntry, boardActive=boardActive, eowSinkPtr=eowSinkPtr, hardInventoryNumber=hardInventoryNumber, laser=laser, boardSlotName=boardSlotName, boardTable=boardTable, laserEntry=laserEntry, boardListTable=boardListTable, boardListIndex=boardListIndex, boardFailure=boardFailure, adrFailHoldOnTime=adrFailHoldOnTime, boardSeverity=boardSeverity, hardInventoryDescr=hardInventoryDescr, softInventoryIndex=softInventoryIndex, softInventoryExt=softInventoryExt, HoldTime=HoldTime, laserALSRestart2s=laserALSRestart2s, hardInventoryEntry=hardInventoryEntry, eowIndex=eowIndex, boardListNumber=boardListNumber, hardInventoryBoard=hardInventoryBoard, softInventoryBoard=softInventoryBoard, ledStatus=ledStatus, hardInventoryIndex=hardInventoryIndex, ledEntry=ledEntry, LedStatus=LedStatus, adrFailHoldOffTime=adrFailHoldOffTime, boardProtectionType=boardProtectionType, BoardFailure=BoardFailure, boardList=boardList, boardMismatchSev=boardMismatchSev, hardInv=hardInv, led=led, PYSNMP_MODULE_ID=shelf, adrHotReset=adrHotReset, softInventoryEntry=softInventoryEntry, boardInsertFamily=boardInsertFamily, laserIndex=laserIndex, holdTime=holdTime, ProtectionType=ProtectionType, boardDefectiveSev=boardDefectiveSev, boardIndex=boardIndex, ledTable=ledTable, boardExpectFamily=boardExpectFamily, ledNumber=ledNumber, laserTxEnable=laserTxEnable, eowSrcPtr=eowSrcPtr, softInventoryType=softInventoryType, shelf=shelf, eowNumber=eowNumber, softInventoryNumber=softInventoryNumber, EOWType=EOWType, ledIndex=ledIndex, laserALSEnable=laserALSEnable, eowEntry=eowEntry)
