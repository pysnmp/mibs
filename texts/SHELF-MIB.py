...
mibBuilder.exportSymbols("SHELF-MIB", laserNumber=laserNumber, softInv=softInv, eowIndex=eowIndex, adrFailHoldOnTime=adrFailHoldOnTime, boardTable=boardTable, ledStatus=ledStatus, softInventoryBoard=softInventoryBoard, boardMonitor=boardMonitor, softInventoryExt=softInventoryExt, laserIndex=laserIndex, shelf=shelf, softInventoryCode=softInventoryCode, BoardFailure=BoardFailure, boardSeverity=boardSeverity, boardInsertFamily=boardInsertFamily, eowName=eowName, boardListIndex=boardListIndex, PYSNMP_MODULE_ID=shelf, softInventoryNumber=softInventoryNumber, ledIndex=ledIndex, eowSrcPtr=eowSrcPtr, boardFirstPortIndex=boardFirstPortIndex, EOWType=EOWType, boardActive=boardActive, eowClockMode=eowClockMode, laserTxEnable=laserTxEnable, eowTable=eowTable, eow=eow, boardMissingSev=boardMissingSev, boardSlotName=boardSlotName, adrHotReset=adrHotReset, softInventoryType=softInventoryType, boardListEntry=boardListEntry, holdTime=holdTime, softInventoryEntry=softInventoryEntry, boardIndex=boardIndex, ledPosition=ledPosition, hardInventoryEntry=hardInventoryEntry, board=board, laser=laser, LedType=LedType, hardInventoryDescr=hardInventoryDescr, boardListNumber=boardListNumber, hardInventoryNumber=hardInventoryNumber, boardList=boardList, softInventoryIndex=softInventoryIndex, eowEntry=eowEntry, boardListType=boardListType, ledEntry=ledEntry, boardProtectionType=boardProtectionType, hardInventoryBoard=hardInventoryBoard, adrFailHoldOffTime=adrFailHoldOffTime, softInventoryTable=softInventoryTable, hardInv=hardInv, EOWClockMode=EOWClockMode, hardInventoryTable=hardInventoryTable, boardNumber=boardNumber, laserTable=laserTable, laserALSEnable=laserALSEnable, ledNumber=ledNumber, LedStatus=LedStatus, boardDefectiveSev=boardDefectiveSev, boardEntry=boardEntry, boardExpectType=boardExpectType, laserALSRestart90s=laserALSRestart90s, hardInventoryIndex=hardInventoryIndex, boardListSlot=boardListSlot, eowSinkPtr=eowSinkPtr, laserEntry=laserEntry, led=led, boardFailure=boardFailure, eowSinkType=eowSinkType, boardExpectFamily=boardExpectFamily, ProtectionType=ProtectionType, eowSrcType=eowSrcType, ledType=ledType, laserALSRestart2s=laserALSRestart2s, eowNumber=eowNumber, HoldTime=HoldTime, boardInsertType=boardInsertType, ledTable=ledTable, boardListTable=boardListTable, boardMismatchSev=boardMismatchSev)
