...
mibBuilder.exportSymbols("SHELF-MIB", boardSlotName=boardSlotName, boardActive=boardActive, softInventoryEntry=softInventoryEntry, hardInventoryEntry=hardInventoryEntry, laserEntry=laserEntry, softInventoryBoard=softInventoryBoard, boardMonitor=boardMonitor, ledTable=ledTable, BoardFailure=BoardFailure, led=led, hardInv=hardInv, PYSNMP_MODULE_ID=shelf, softInventoryTable=softInventoryTable, ledNumber=ledNumber, ProtectionType=ProtectionType, boardExpectFamily=boardExpectFamily, boardProtectionType=boardProtectionType, laserNumber=laserNumber, EOWClockMode=EOWClockMode, eow=eow, boardListTable=boardListTable, softInventoryCode=softInventoryCode, adrFailHoldOnTime=adrFailHoldOnTime, hardInventoryIndex=hardInventoryIndex, boardDefectiveSev=boardDefectiveSev, boardInsertType=boardInsertType, laserALSRestart90s=laserALSRestart90s, boardEntry=boardEntry, laserALSRestart2s=laserALSRestart2s, ledIndex=ledIndex, softInventoryNumber=softInventoryNumber, laserTable=laserTable, boardNumber=boardNumber, boardMissingSev=boardMissingSev, eowClockMode=eowClockMode, ledType=ledType, boardTable=boardTable, boardListIndex=boardListIndex, boardSeverity=boardSeverity, adrFailHoldOffTime=adrFailHoldOffTime, laserALSEnable=laserALSEnable, hardInventoryTable=hardInventoryTable, eowEntry=eowEntry, ledPosition=ledPosition, eowSinkPtr=eowSinkPtr, hardInventoryBoard=hardInventoryBoard, eowTable=eowTable, softInv=softInv, softInventoryType=softInventoryType, boardIndex=boardIndex, adrHotReset=adrHotReset, hardInventoryDescr=hardInventoryDescr, ledStatus=ledStatus, eowName=eowName, laser=laser, boardListEntry=boardListEntry, boardFirstPortIndex=boardFirstPortIndex, eowSrcPtr=eowSrcPtr, eowIndex=eowIndex, EOWType=EOWType, laserTxEnable=laserTxEnable, boardListNumber=boardListNumber, boardListType=boardListType, softInventoryIndex=softInventoryIndex, HoldTime=HoldTime, board=board, boardFailure=boardFailure, boardInsertFamily=boardInsertFamily, LedType=LedType, boardMismatchSev=boardMismatchSev, softInventoryExt=softInventoryExt, ledEntry=ledEntry, LedStatus=LedStatus, hardInventoryNumber=hardInventoryNumber, shelf=shelf, boardListSlot=boardListSlot, laserIndex=laserIndex, holdTime=holdTime, boardList=boardList, eowSrcType=eowSrcType, eowSinkType=eowSinkType, boardExpectType=boardExpectType, eowNumber=eowNumber)
