...
mibBuilder.exportSymbols("SHELF-MIB", boardInsertFamily=boardInsertFamily, boardMissingSev=boardMissingSev, softInventoryExt=softInventoryExt, laserIndex=laserIndex, eowSinkPtr=eowSinkPtr, adrFailHoldOffTime=adrFailHoldOffTime, boardActive=boardActive, boardInsertType=boardInsertType, softInv=softInv, eowTable=eowTable, boardList=boardList, holdTime=holdTime, boardDefectiveSev=boardDefectiveSev, hardInventoryEntry=hardInventoryEntry, eowEntry=eowEntry, boardMonitor=boardMonitor, softInventoryNumber=softInventoryNumber, BoardFailure=BoardFailure, boardListIndex=boardListIndex, boardExpectFamily=boardExpectFamily, LedStatus=LedStatus, laserTxEnable=laserTxEnable, boardIndex=boardIndex, boardSlotName=boardSlotName, shelf=shelf, softInventoryTable=softInventoryTable, adrFailHoldOnTime=adrFailHoldOnTime, LedType=LedType, ledPosition=ledPosition, laserEntry=laserEntry, hardInv=hardInv, EOWClockMode=EOWClockMode, eowSrcType=eowSrcType, hardInventoryDescr=hardInventoryDescr, ledTable=ledTable, boardListNumber=boardListNumber, eow=eow, eowIndex=eowIndex, laserALSEnable=laserALSEnable, laserALSRestart2s=laserALSRestart2s, PYSNMP_MODULE_ID=shelf, softInventoryType=softInventoryType, boardProtectionType=boardProtectionType, HoldTime=HoldTime, board=board, boardFirstPortIndex=boardFirstPortIndex, boardListEntry=boardListEntry, eowNumber=eowNumber, softInventoryCode=softInventoryCode, hardInventoryTable=hardInventoryTable, ledType=ledType, boardTable=boardTable, boardNumber=boardNumber, eowSrcPtr=eowSrcPtr, eowName=eowName, ProtectionType=ProtectionType, hardInventoryBoard=hardInventoryBoard, ledNumber=ledNumber, eowSinkType=eowSinkType, boardExpectType=boardExpectType, hardInventoryIndex=hardInventoryIndex, hardInventoryNumber=hardInventoryNumber, eowClockMode=eowClockMode, led=led, boardSeverity=boardSeverity, laserALSRestart90s=laserALSRestart90s, boardFailure=boardFailure, boardMismatchSev=boardMismatchSev, adrHotReset=adrHotReset, softInventoryBoard=softInventoryBoard, ledEntry=ledEntry, softInventoryEntry=softInventoryEntry, laser=laser, boardEntry=boardEntry, boardListType=boardListType, boardListSlot=boardListSlot, laserNumber=laserNumber, boardListTable=boardListTable, laserTable=laserTable, ledIndex=ledIndex, softInventoryIndex=softInventoryIndex, ledStatus=ledStatus, EOWType=EOWType)
