...
mibBuilder.exportSymbols("SHELF-MIB", boardListIndex=boardListIndex, softInventoryNumber=softInventoryNumber, boardActive=boardActive, softInventoryTable=softInventoryTable, EOWType=EOWType, softInventoryType=softInventoryType, softInventoryIndex=softInventoryIndex, softInventoryCode=softInventoryCode, boardExpectType=boardExpectType, laserALSEnable=laserALSEnable, led=led, shelf=shelf, boardFirstPortIndex=boardFirstPortIndex, HoldTime=HoldTime, eowTable=eowTable, boardFailure=boardFailure, softInventoryExt=softInventoryExt, hardInv=hardInv, LedType=LedType, holdTime=holdTime, LedStatus=LedStatus, softInventoryEntry=softInventoryEntry, boardMonitor=boardMonitor, PYSNMP_MODULE_ID=shelf, boardInsertFamily=boardInsertFamily, ledEntry=ledEntry, eowIndex=eowIndex, eowSinkPtr=eowSinkPtr, laserIndex=laserIndex, ledStatus=ledStatus, boardList=boardList, boardEntry=boardEntry, boardInsertType=boardInsertType, boardMissingSev=boardMissingSev, BoardFailure=BoardFailure, eowSrcPtr=eowSrcPtr, hardInventoryTable=hardInventoryTable, adrFailHoldOnTime=adrFailHoldOnTime, laserEntry=laserEntry, ledPosition=ledPosition, boardListType=boardListType, ledType=ledType, eow=eow, eowSrcType=eowSrcType, eowClockMode=eowClockMode, EOWClockMode=EOWClockMode, boardDefectiveSev=boardDefectiveSev, eowEntry=eowEntry, adrHotReset=adrHotReset, hardInventoryDescr=hardInventoryDescr, boardMismatchSev=boardMismatchSev, eowNumber=eowNumber, softInv=softInv, eowName=eowName, hardInventoryNumber=hardInventoryNumber, laserTxEnable=laserTxEnable, laserALSRestart90s=laserALSRestart90s, boardListTable=boardListTable, boardListNumber=boardListNumber, hardInventoryEntry=hardInventoryEntry, softInventoryBoard=softInventoryBoard, boardSeverity=boardSeverity, hardInventoryIndex=hardInventoryIndex, laserTable=laserTable, boardListEntry=boardListEntry, ProtectionType=ProtectionType, boardListSlot=boardListSlot, boardProtectionType=boardProtectionType, board=board, ledNumber=ledNumber, boardTable=boardTable, ledTable=ledTable, eowSinkType=eowSinkType, laser=laser, laserNumber=laserNumber, hardInventoryBoard=hardInventoryBoard, adrFailHoldOffTime=adrFailHoldOffTime, boardNumber=boardNumber, laserALSRestart2s=laserALSRestart2s, boardSlotName=boardSlotName, ledIndex=ledIndex, boardExpectFamily=boardExpectFamily, boardIndex=boardIndex)
