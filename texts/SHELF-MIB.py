...
mibBuilder.exportSymbols("SHELF-MIB", shelf=shelf, softInventoryCode=softInventoryCode, laserEntry=laserEntry, hardInventoryIndex=hardInventoryIndex, EOWClockMode=EOWClockMode, adrFailHoldOffTime=adrFailHoldOffTime, eowSinkPtr=eowSinkPtr, adrHotReset=adrHotReset, laserIndex=laserIndex, boardInsertType=boardInsertType, boardSeverity=boardSeverity, boardActive=boardActive, board=board, eowTable=eowTable, boardListIndex=boardListIndex, ledTable=ledTable, LedType=LedType, ledEntry=ledEntry, ledPosition=ledPosition, boardListEntry=boardListEntry, softInventoryEntry=softInventoryEntry, hardInventoryDescr=hardInventoryDescr, hardInventoryTable=hardInventoryTable, laserALSEnable=laserALSEnable, EOWType=EOWType, BoardFailure=BoardFailure, boardFirstPortIndex=boardFirstPortIndex, laserNumber=laserNumber, boardExpectType=boardExpectType, boardSlotName=boardSlotName, softInventoryBoard=softInventoryBoard, hardInventoryEntry=hardInventoryEntry, holdTime=holdTime, hardInventoryNumber=hardInventoryNumber, boardInsertFamily=boardInsertFamily, boardIndex=boardIndex, eowSrcPtr=eowSrcPtr, boardList=boardList, softInventoryExt=softInventoryExt, boardMismatchSev=boardMismatchSev, ledIndex=ledIndex, HoldTime=HoldTime, boardExpectFamily=boardExpectFamily, softInv=softInv, eowClockMode=eowClockMode, eowIndex=eowIndex, eow=eow, eowEntry=eowEntry, hardInv=hardInv, softInventoryTable=softInventoryTable, PYSNMP_MODULE_ID=shelf, boardListTable=boardListTable, laserALSRestart90s=laserALSRestart90s, boardMissingSev=boardMissingSev, LedStatus=LedStatus, ledType=ledType, laserALSRestart2s=laserALSRestart2s, boardTable=boardTable, softInventoryType=softInventoryType, boardEntry=boardEntry, eowNumber=eowNumber, boardMonitor=boardMonitor, softInventoryIndex=softInventoryIndex, ProtectionType=ProtectionType, eowSrcType=eowSrcType, eowName=eowName, boardDefectiveSev=boardDefectiveSev, laserTxEnable=laserTxEnable, hardInventoryBoard=hardInventoryBoard, ledStatus=ledStatus, boardListNumber=boardListNumber, softInventoryNumber=softInventoryNumber, boardListType=boardListType, eowSinkType=eowSinkType, laser=laser, boardProtectionType=boardProtectionType, boardFailure=boardFailure, ledNumber=ledNumber, laserTable=laserTable, boardNumber=boardNumber, led=led, adrFailHoldOnTime=adrFailHoldOnTime, boardListSlot=boardListSlot)
