...
mibBuilder.exportSymbols("SHELF-MIB", softInventoryNumber=softInventoryNumber, hardInv=hardInv, boardListTable=boardListTable, boardFailure=boardFailure, softInventoryType=softInventoryType, boardIndex=boardIndex, laserALSRestart2s=laserALSRestart2s, boardListNumber=boardListNumber, softInventoryEntry=softInventoryEntry, boardListEntry=boardListEntry, laserALSRestart90s=laserALSRestart90s, hardInventoryEntry=hardInventoryEntry, boardExpectFamily=boardExpectFamily, boardSlotName=boardSlotName, boardFirstPortIndex=boardFirstPortIndex, BoardFailure=BoardFailure, softInv=softInv, boardListIndex=boardListIndex, boardMismatchSev=boardMismatchSev, boardSeverity=boardSeverity, laserIndex=laserIndex, ledStatus=ledStatus, boardExpectType=boardExpectType, boardList=boardList, ledIndex=ledIndex, softInventoryIndex=softInventoryIndex, hardInventoryDescr=hardInventoryDescr, boardDefectiveSev=boardDefectiveSev, eowSinkType=eowSinkType, hardInventoryIndex=hardInventoryIndex, laserTxEnable=laserTxEnable, eowSrcPtr=eowSrcPtr, softInventoryTable=softInventoryTable, LedStatus=LedStatus, softInventoryCode=softInventoryCode, boardInsertType=boardInsertType, boardNumber=boardNumber, hardInventoryBoard=hardInventoryBoard, board=board, led=led, boardTable=boardTable, adrFailHoldOffTime=adrFailHoldOffTime, eowClockMode=eowClockMode, softInventoryExt=softInventoryExt, ledPosition=ledPosition, LedType=LedType, boardMissingSev=boardMissingSev, boardProtectionType=boardProtectionType, holdTime=holdTime, laser=laser, eowSrcType=eowSrcType, eowIndex=eowIndex, eowTable=eowTable, softInventoryBoard=softInventoryBoard, ledNumber=ledNumber, adrFailHoldOnTime=adrFailHoldOnTime, PYSNMP_MODULE_ID=shelf, eowEntry=eowEntry, HoldTime=HoldTime, eowName=eowName, hardInventoryNumber=hardInventoryNumber, boardMonitor=boardMonitor, laserNumber=laserNumber, ledEntry=ledEntry, ProtectionType=ProtectionType, EOWClockMode=EOWClockMode, laserEntry=laserEntry, laserALSEnable=laserALSEnable, ledType=ledType, eow=eow, boardListSlot=boardListSlot, boardEntry=boardEntry, boardListType=boardListType, hardInventoryTable=hardInventoryTable, laserTable=laserTable, boardInsertFamily=boardInsertFamily, eowSinkPtr=eowSinkPtr, boardActive=boardActive, EOWType=EOWType, adrHotReset=adrHotReset, eowNumber=eowNumber, shelf=shelf, ledTable=ledTable)
