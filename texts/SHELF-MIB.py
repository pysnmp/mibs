...
mibBuilder.exportSymbols("SHELF-MIB", laserALSEnable=laserALSEnable, boardListEntry=boardListEntry, softInv=softInv, laserALSRestart2s=laserALSRestart2s, eowIndex=eowIndex, led=led, eowNumber=eowNumber, hardInventoryTable=hardInventoryTable, softInventoryEntry=softInventoryEntry, softInventoryExt=softInventoryExt, ledStatus=ledStatus, laserTxEnable=laserTxEnable, EOWClockMode=EOWClockMode, laser=laser, boardTable=boardTable, boardListSlot=boardListSlot, boardSeverity=boardSeverity, eow=eow, hardInventoryDescr=hardInventoryDescr, laserEntry=laserEntry, hardInv=hardInv, boardListNumber=boardListNumber, LedStatus=LedStatus, adrFailHoldOnTime=adrFailHoldOnTime, boardListIndex=boardListIndex, eowSrcType=eowSrcType, adrHotReset=adrHotReset, ledIndex=ledIndex, boardMismatchSev=boardMismatchSev, eowClockMode=eowClockMode, boardMonitor=boardMonitor, softInventoryBoard=softInventoryBoard, boardIndex=boardIndex, softInventoryCode=softInventoryCode, ProtectionType=ProtectionType, ledNumber=ledNumber, softInventoryTable=softInventoryTable, boardActive=boardActive, boardSlotName=boardSlotName, EOWType=EOWType, eowName=eowName, board=board, boardFailure=boardFailure, boardExpectType=boardExpectType, laserTable=laserTable, softInventoryType=softInventoryType, boardInsertType=boardInsertType, boardEntry=boardEntry, eowTable=eowTable, boardDefectiveSev=boardDefectiveSev, ledEntry=ledEntry, LedType=LedType, boardList=boardList, boardFirstPortIndex=boardFirstPortIndex, BoardFailure=BoardFailure, ledTable=ledTable, boardInsertFamily=boardInsertFamily, softInventoryNumber=softInventoryNumber, boardProtectionType=boardProtectionType, eowEntry=eowEntry, boardListType=boardListType, adrFailHoldOffTime=adrFailHoldOffTime, laserIndex=laserIndex, HoldTime=HoldTime, ledPosition=ledPosition, eowSinkPtr=eowSinkPtr, eowSinkType=eowSinkType, boardListTable=boardListTable, laserALSRestart90s=laserALSRestart90s, PYSNMP_MODULE_ID=shelf, hardInventoryEntry=hardInventoryEntry, boardNumber=boardNumber, holdTime=holdTime, laserNumber=laserNumber, eowSrcPtr=eowSrcPtr, boardMissingSev=boardMissingSev, ledType=ledType, hardInventoryNumber=hardInventoryNumber, hardInventoryBoard=hardInventoryBoard, boardExpectFamily=boardExpectFamily, hardInventoryIndex=hardInventoryIndex, shelf=shelf, softInventoryIndex=softInventoryIndex)
