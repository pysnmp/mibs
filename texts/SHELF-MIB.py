...
mibBuilder.exportSymbols("SHELF-MIB", led=led, board=board, hardInventoryIndex=hardInventoryIndex, hardInventoryDescr=hardInventoryDescr, laserALSRestart2s=laserALSRestart2s, eow=eow, eowNumber=eowNumber, laser=laser, holdTime=holdTime, eowTable=eowTable, boardInsertType=boardInsertType, eowEntry=eowEntry, boardExpectType=boardExpectType, boardDefectiveSev=boardDefectiveSev, hardInv=hardInv, boardListIndex=boardListIndex, boardNumber=boardNumber, boardSlotName=boardSlotName, laserTable=laserTable, shelf=shelf, eowIndex=eowIndex, ledPosition=ledPosition, ledEntry=ledEntry, boardExpectFamily=boardExpectFamily, ledType=ledType, boardListTable=boardListTable, eowClockMode=eowClockMode, BoardFailure=BoardFailure, laserALSRestart90s=laserALSRestart90s, EOWClockMode=EOWClockMode, boardMismatchSev=boardMismatchSev, laserEntry=laserEntry, LedStatus=LedStatus, softInventoryEntry=softInventoryEntry, eowName=eowName, softInventoryBoard=softInventoryBoard, laserALSEnable=laserALSEnable, boardListEntry=boardListEntry, softInventoryNumber=softInventoryNumber, softInv=softInv, boardFailure=boardFailure, boardListSlot=boardListSlot, hardInventoryNumber=hardInventoryNumber, boardInsertFamily=boardInsertFamily, HoldTime=HoldTime, ledTable=ledTable, ProtectionType=ProtectionType, eowSinkPtr=eowSinkPtr, LedType=LedType, boardMissingSev=boardMissingSev, boardEntry=boardEntry, laserNumber=laserNumber, ledNumber=ledNumber, PYSNMP_MODULE_ID=shelf, softInventoryTable=softInventoryTable, adrFailHoldOnTime=adrFailHoldOnTime, boardSeverity=boardSeverity, ledStatus=ledStatus, softInventoryIndex=softInventoryIndex, boardFirstPortIndex=boardFirstPortIndex, laserIndex=laserIndex, laserTxEnable=laserTxEnable, EOWType=EOWType, eowSrcType=eowSrcType, hardInventoryBoard=hardInventoryBoard, boardListNumber=boardListNumber, boardIndex=boardIndex, boardTable=boardTable, eowSinkType=eowSinkType, boardProtectionType=boardProtectionType, adrHotReset=adrHotReset, boardListType=boardListType, eowSrcPtr=eowSrcPtr, adrFailHoldOffTime=adrFailHoldOffTime, softInventoryCode=softInventoryCode, boardActive=boardActive, softInventoryExt=softInventoryExt, softInventoryType=softInventoryType, boardMonitor=boardMonitor, hardInventoryEntry=hardInventoryEntry, ledIndex=ledIndex, hardInventoryTable=hardInventoryTable, boardList=boardList)
