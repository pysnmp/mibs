...
mibBuilder.exportSymbols("SHELF-MIB", eowName=eowName, laserALSRestart90s=laserALSRestart90s, softInventoryEntry=softInventoryEntry, softInventoryNumber=softInventoryNumber, hardInventoryNumber=hardInventoryNumber, ledTable=ledTable, boardListType=boardListType, laserALSRestart2s=laserALSRestart2s, softInventoryExt=softInventoryExt, laserTxEnable=laserTxEnable, eowIndex=eowIndex, boardListIndex=boardListIndex, ledNumber=ledNumber, boardExpectFamily=boardExpectFamily, ledType=ledType, eowSinkPtr=eowSinkPtr, boardMissingSev=boardMissingSev, eowClockMode=eowClockMode, shelf=shelf, boardMonitor=boardMonitor, boardTable=boardTable, HoldTime=HoldTime, softInventoryBoard=softInventoryBoard, BoardFailure=BoardFailure, laserNumber=laserNumber, boardListEntry=boardListEntry, adrHotReset=adrHotReset, boardSlotName=boardSlotName, hardInventoryDescr=hardInventoryDescr, ledEntry=ledEntry, hardInventoryIndex=hardInventoryIndex, LedType=LedType, boardNumber=boardNumber, hardInventoryEntry=hardInventoryEntry, ledIndex=ledIndex, boardDefectiveSev=boardDefectiveSev, adrFailHoldOffTime=adrFailHoldOffTime, softInv=softInv, hardInventoryTable=hardInventoryTable, EOWType=EOWType, boardMismatchSev=boardMismatchSev, laserTable=laserTable, LedStatus=LedStatus, boardExpectType=boardExpectType, eowSrcType=eowSrcType, hardInv=hardInv, eowSinkType=eowSinkType, eowEntry=eowEntry, boardListTable=boardListTable, softInventoryIndex=softInventoryIndex, boardListSlot=boardListSlot, laserEntry=laserEntry, boardIndex=boardIndex, laserIndex=laserIndex, boardListNumber=boardListNumber, boardSeverity=boardSeverity, softInventoryCode=softInventoryCode, ProtectionType=ProtectionType, boardInsertType=boardInsertType, PYSNMP_MODULE_ID=shelf, softInventoryType=softInventoryType, eow=eow, boardFailure=boardFailure, boardFirstPortIndex=boardFirstPortIndex, hardInventoryBoard=hardInventoryBoard, ledPosition=ledPosition, softInventoryTable=softInventoryTable, eowSrcPtr=eowSrcPtr, eowTable=eowTable, boardEntry=boardEntry, laserALSEnable=laserALSEnable, led=led, holdTime=holdTime, EOWClockMode=EOWClockMode, boardList=boardList, boardActive=boardActive, eowNumber=eowNumber, ledStatus=ledStatus, board=board, boardInsertFamily=boardInsertFamily, laser=laser, adrFailHoldOnTime=adrFailHoldOnTime, boardProtectionType=boardProtectionType)
