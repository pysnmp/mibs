...
mibBuilder.exportSymbols("SHELF-MIB", hardInventoryBoard=hardInventoryBoard, ledStatus=ledStatus, laserTxEnable=laserTxEnable, hardInventoryDescr=hardInventoryDescr, BoardFailure=BoardFailure, boardFirstPortIndex=boardFirstPortIndex, holdTime=holdTime, adrFailHoldOffTime=adrFailHoldOffTime, ledTable=ledTable, boardExpectType=boardExpectType, hardInventoryIndex=hardInventoryIndex, boardSlotName=boardSlotName, eowNumber=eowNumber, PYSNMP_MODULE_ID=shelf, boardListIndex=boardListIndex, EOWClockMode=EOWClockMode, softInventoryIndex=softInventoryIndex, eowSrcPtr=eowSrcPtr, boardDefectiveSev=boardDefectiveSev, eowName=eowName, boardMissingSev=boardMissingSev, boardListSlot=boardListSlot, ledPosition=ledPosition, boardListEntry=boardListEntry, ledType=ledType, shelf=shelf, laserIndex=laserIndex, softInv=softInv, laserALSRestart2s=laserALSRestart2s, softInventoryExt=softInventoryExt, eowIndex=eowIndex, boardActive=boardActive, LedStatus=LedStatus, laserNumber=laserNumber, laserALSRestart90s=laserALSRestart90s, boardListNumber=boardListNumber, eow=eow, laserTable=laserTable, hardInv=hardInv, boardInsertFamily=boardInsertFamily, LedType=LedType, boardEntry=boardEntry, ProtectionType=ProtectionType, boardIndex=boardIndex, adrHotReset=adrHotReset, softInventoryTable=softInventoryTable, hardInventoryTable=hardInventoryTable, eowSinkPtr=eowSinkPtr, ledEntry=ledEntry, EOWType=EOWType, hardInventoryNumber=hardInventoryNumber, boardMismatchSev=boardMismatchSev, boardListType=boardListType, board=board, softInventoryType=softInventoryType, hardInventoryEntry=hardInventoryEntry, boardSeverity=boardSeverity, adrFailHoldOnTime=adrFailHoldOnTime, softInventoryCode=softInventoryCode, eowTable=eowTable, laserEntry=laserEntry, eowClockMode=eowClockMode, softInventoryNumber=softInventoryNumber, eowEntry=eowEntry, boardNumber=boardNumber, boardList=boardList, softInventoryBoard=softInventoryBoard, eowSinkType=eowSinkType, eowSrcType=eowSrcType, ledIndex=ledIndex, laserALSEnable=laserALSEnable, ledNumber=ledNumber, boardExpectFamily=boardExpectFamily, boardProtectionType=boardProtectionType, boardTable=boardTable, boardMonitor=boardMonitor, HoldTime=HoldTime, softInventoryEntry=softInventoryEntry, boardInsertType=boardInsertType, boardListTable=boardListTable, laser=laser, led=led, boardFailure=boardFailure)
