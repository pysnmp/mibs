...
mibBuilder.exportSymbols("SHELF-MIB", boardListNumber=boardListNumber, boardFirstPortIndex=boardFirstPortIndex, HoldTime=HoldTime, boardExpectType=boardExpectType, hardInventoryEntry=hardInventoryEntry, boardListEntry=boardListEntry, laserALSEnable=laserALSEnable, boardNumber=boardNumber, eowEntry=eowEntry, eowSrcPtr=eowSrcPtr, boardMissingSev=boardMissingSev, eowTable=eowTable, boardInsertFamily=boardInsertFamily, hardInventoryTable=hardInventoryTable, softInventoryIndex=softInventoryIndex, led=led, laserEntry=laserEntry, boardListTable=boardListTable, softInventoryBoard=softInventoryBoard, eowClockMode=eowClockMode, softInventoryEntry=softInventoryEntry, ledNumber=ledNumber, eowName=eowName, PYSNMP_MODULE_ID=shelf, laserNumber=laserNumber, laserTxEnable=laserTxEnable, eowSinkPtr=eowSinkPtr, ledStatus=ledStatus, boardMonitor=boardMonitor, boardListIndex=boardListIndex, ledIndex=ledIndex, softInventoryType=softInventoryType, LedStatus=LedStatus, boardListType=boardListType, adrHotReset=adrHotReset, EOWType=EOWType, boardProtectionType=boardProtectionType, softInv=softInv, boardTable=boardTable, eowSrcType=eowSrcType, hardInventoryIndex=hardInventoryIndex, boardExpectFamily=boardExpectFamily, laserALSRestart90s=laserALSRestart90s, holdTime=holdTime, eowIndex=eowIndex, softInventoryCode=softInventoryCode, boardActive=boardActive, boardSlotName=boardSlotName, shelf=shelf, laserTable=laserTable, laserALSRestart2s=laserALSRestart2s, boardFailure=boardFailure, softInventoryNumber=softInventoryNumber, eowSinkType=eowSinkType, softInventoryExt=softInventoryExt, hardInventoryBoard=hardInventoryBoard, ledPosition=ledPosition, boardIndex=boardIndex, hardInv=hardInv, boardList=boardList, ledEntry=ledEntry, boardDefectiveSev=boardDefectiveSev, softInventoryTable=softInventoryTable, boardInsertType=boardInsertType, ledType=ledType, LedType=LedType, laser=laser, ledTable=ledTable, board=board, hardInventoryDescr=hardInventoryDescr, laserIndex=laserIndex, boardEntry=boardEntry, boardListSlot=boardListSlot, adrFailHoldOnTime=adrFailHoldOnTime, BoardFailure=BoardFailure, boardMismatchSev=boardMismatchSev, EOWClockMode=EOWClockMode, eow=eow, eowNumber=eowNumber, hardInventoryNumber=hardInventoryNumber, ProtectionType=ProtectionType, adrFailHoldOffTime=adrFailHoldOffTime, boardSeverity=boardSeverity)
