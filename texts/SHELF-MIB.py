...
mibBuilder.exportSymbols("SHELF-MIB", softInv=softInv, hardInventoryTable=hardInventoryTable, boardSlotName=boardSlotName, holdTime=holdTime, boardFailure=boardFailure, laserEntry=laserEntry, laser=laser, adrHotReset=adrHotReset, laserTable=laserTable, eowEntry=eowEntry, boardSeverity=boardSeverity, boardListTable=boardListTable, boardExpectType=boardExpectType, boardInsertFamily=boardInsertFamily, BoardFailure=BoardFailure, softInventoryTable=softInventoryTable, softInventoryIndex=softInventoryIndex, boardListSlot=boardListSlot, adrFailHoldOnTime=adrFailHoldOnTime, boardMismatchSev=boardMismatchSev, softInventoryNumber=softInventoryNumber, boardListIndex=boardListIndex, boardEntry=boardEntry, boardList=boardList, PYSNMP_MODULE_ID=shelf, board=board, boardProtectionType=boardProtectionType, eowSrcPtr=eowSrcPtr, LedType=LedType, ProtectionType=ProtectionType, EOWType=EOWType, laserALSRestart90s=laserALSRestart90s, ledTable=ledTable, shelf=shelf, boardListType=boardListType, boardNumber=boardNumber, ledStatus=ledStatus, led=led, hardInv=hardInv, hardInventoryDescr=hardInventoryDescr, ledNumber=ledNumber, laserNumber=laserNumber, ledType=ledType, eowSinkType=eowSinkType, ledPosition=ledPosition, eow=eow, boardTable=boardTable, laserALSRestart2s=laserALSRestart2s, eowName=eowName, boardMissingSev=boardMissingSev, ledIndex=ledIndex, boardListEntry=boardListEntry, hardInventoryIndex=hardInventoryIndex, hardInventoryBoard=hardInventoryBoard, ledEntry=ledEntry, boardMonitor=boardMonitor, softInventoryExt=softInventoryExt, softInventoryCode=softInventoryCode, eowSrcType=eowSrcType, EOWClockMode=EOWClockMode, eowIndex=eowIndex, eowClockMode=eowClockMode, boardDefectiveSev=boardDefectiveSev, hardInventoryNumber=hardInventoryNumber, laserTxEnable=laserTxEnable, laserIndex=laserIndex, laserALSEnable=laserALSEnable, hardInventoryEntry=hardInventoryEntry, boardListNumber=boardListNumber, HoldTime=HoldTime, eowNumber=eowNumber, boardActive=boardActive, softInventoryBoard=softInventoryBoard, eowTable=eowTable, softInventoryType=softInventoryType, adrFailHoldOffTime=adrFailHoldOffTime, boardIndex=boardIndex, LedStatus=LedStatus, boardFirstPortIndex=boardFirstPortIndex, boardInsertType=boardInsertType, softInventoryEntry=softInventoryEntry, eowSinkPtr=eowSinkPtr, boardExpectFamily=boardExpectFamily)
