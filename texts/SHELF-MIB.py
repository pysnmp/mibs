...
mibBuilder.exportSymbols("SHELF-MIB", shelf=shelf, ledEntry=ledEntry, eowTable=eowTable, EOWClockMode=EOWClockMode, boardEntry=boardEntry, PYSNMP_MODULE_ID=shelf, LedStatus=LedStatus, laserALSEnable=laserALSEnable, hardInventoryBoard=hardInventoryBoard, softInventoryType=softInventoryType, boardMonitor=boardMonitor, softInventoryTable=softInventoryTable, eowSinkType=eowSinkType, boardActive=boardActive, boardFailure=boardFailure, boardExpectType=boardExpectType, boardMissingSev=boardMissingSev, ledIndex=ledIndex, boardListSlot=boardListSlot, softInventoryNumber=softInventoryNumber, ledNumber=ledNumber, ledStatus=ledStatus, softInventoryEntry=softInventoryEntry, boardListNumber=boardListNumber, HoldTime=HoldTime, eowSinkPtr=eowSinkPtr, eowNumber=eowNumber, boardFirstPortIndex=boardFirstPortIndex, hardInventoryIndex=hardInventoryIndex, boardListType=boardListType, boardInsertType=boardInsertType, BoardFailure=BoardFailure, laser=laser, softInventoryCode=softInventoryCode, laserALSRestart90s=laserALSRestart90s, boardListTable=boardListTable, LedType=LedType, laserNumber=laserNumber, boardNumber=boardNumber, eow=eow, laserTxEnable=laserTxEnable, boardListIndex=boardListIndex, eowSrcType=eowSrcType, hardInventoryTable=hardInventoryTable, boardListEntry=boardListEntry, softInventoryIndex=softInventoryIndex, boardInsertFamily=boardInsertFamily, softInventoryExt=softInventoryExt, eowIndex=eowIndex, holdTime=holdTime, ledTable=ledTable, softInventoryBoard=softInventoryBoard, boardIndex=boardIndex, laserALSRestart2s=laserALSRestart2s, ledPosition=ledPosition, ledType=ledType, board=board, laserEntry=laserEntry, led=led, adrFailHoldOffTime=adrFailHoldOffTime, hardInventoryEntry=hardInventoryEntry, laserIndex=laserIndex, adrFailHoldOnTime=adrFailHoldOnTime, boardList=boardList, boardDefectiveSev=boardDefectiveSev, eowClockMode=eowClockMode, EOWType=EOWType, eowEntry=eowEntry, hardInventoryNumber=hardInventoryNumber, softInv=softInv, hardInv=hardInv, boardSeverity=boardSeverity, hardInventoryDescr=hardInventoryDescr, boardMismatchSev=boardMismatchSev, ProtectionType=ProtectionType, adrHotReset=adrHotReset, eowSrcPtr=eowSrcPtr, laserTable=laserTable, boardProtectionType=boardProtectionType, eowName=eowName, boardSlotName=boardSlotName, boardTable=boardTable, boardExpectFamily=boardExpectFamily)
