...
mibBuilder.exportSymbols("SHELF-MIB", boardSeverity=boardSeverity, boardDefectiveSev=boardDefectiveSev, laserALSRestart2s=laserALSRestart2s, eowSrcType=eowSrcType, boardListTable=boardListTable, boardList=boardList, laserALSEnable=laserALSEnable, laserIndex=laserIndex, led=led, boardActive=boardActive, ledNumber=ledNumber, EOWClockMode=EOWClockMode, boardInsertType=boardInsertType, hardInventoryEntry=hardInventoryEntry, hardInventoryBoard=hardInventoryBoard, ProtectionType=ProtectionType, boardTable=boardTable, eowSinkPtr=eowSinkPtr, boardMonitor=boardMonitor, boardExpectType=boardExpectType, softInventoryEntry=softInventoryEntry, boardExpectFamily=boardExpectFamily, laserTxEnable=laserTxEnable, boardProtectionType=boardProtectionType, softInventoryExt=softInventoryExt, hardInventoryDescr=hardInventoryDescr, softInventoryTable=softInventoryTable, hardInventoryTable=hardInventoryTable, HoldTime=HoldTime, softInventoryIndex=softInventoryIndex, BoardFailure=BoardFailure, ledEntry=ledEntry, eowIndex=eowIndex, laserALSRestart90s=laserALSRestart90s, ledTable=ledTable, eowName=eowName, holdTime=holdTime, eowEntry=eowEntry, boardFailure=boardFailure, boardNumber=boardNumber, softInventoryType=softInventoryType, boardMismatchSev=boardMismatchSev, LedStatus=LedStatus, laser=laser, ledType=ledType, laserTable=laserTable, hardInv=hardInv, boardMissingSev=boardMissingSev, softInventoryNumber=softInventoryNumber, PYSNMP_MODULE_ID=shelf, boardListNumber=boardListNumber, boardListSlot=boardListSlot, hardInventoryNumber=hardInventoryNumber, ledStatus=ledStatus, laserNumber=laserNumber, softInventoryCode=softInventoryCode, LedType=LedType, boardListType=boardListType, adrFailHoldOnTime=adrFailHoldOnTime, board=board, hardInventoryIndex=hardInventoryIndex, boardInsertFamily=boardInsertFamily, eowSinkType=eowSinkType, EOWType=EOWType, eowNumber=eowNumber, boardFirstPortIndex=boardFirstPortIndex, adrHotReset=adrHotReset, boardSlotName=boardSlotName, laserEntry=laserEntry, softInv=softInv, eow=eow, boardListEntry=boardListEntry, softInventoryBoard=softInventoryBoard, boardIndex=boardIndex, boardEntry=boardEntry, ledPosition=ledPosition, eowTable=eowTable, ledIndex=ledIndex, adrFailHoldOffTime=adrFailHoldOffTime, eowSrcPtr=eowSrcPtr, eowClockMode=eowClockMode, shelf=shelf, boardListIndex=boardListIndex)
