...
mibBuilder.exportSymbols("SHELF-MIB", boardEntry=boardEntry, boardExpectType=boardExpectType, eowNumber=eowNumber, led=led, softInventoryCode=softInventoryCode, boardFailure=boardFailure, ledIndex=ledIndex, laserALSRestart2s=laserALSRestart2s, eow=eow, boardActive=boardActive, hardInventoryTable=hardInventoryTable, eowTable=eowTable, eowSrcType=eowSrcType, softInventoryTable=softInventoryTable, softInventoryIndex=softInventoryIndex, ledTable=ledTable, boardExpectFamily=boardExpectFamily, board=board, holdTime=holdTime, boardNumber=boardNumber, boardListType=boardListType, ledEntry=ledEntry, eowClockMode=eowClockMode, laserALSEnable=laserALSEnable, ledNumber=ledNumber, PYSNMP_MODULE_ID=shelf, laserEntry=laserEntry, boardListEntry=boardListEntry, BoardFailure=BoardFailure, hardInventoryBoard=hardInventoryBoard, adrFailHoldOnTime=adrFailHoldOnTime, boardIndex=boardIndex, laserNumber=laserNumber, boardListIndex=boardListIndex, softInventoryNumber=softInventoryNumber, hardInventoryDescr=hardInventoryDescr, boardMonitor=boardMonitor, laser=laser, laserTxEnable=laserTxEnable, boardProtectionType=boardProtectionType, softInventoryExt=softInventoryExt, softInv=softInv, HoldTime=HoldTime, boardMissingSev=boardMissingSev, boardListTable=boardListTable, laserTable=laserTable, eowIndex=eowIndex, boardSlotName=boardSlotName, softInventoryType=softInventoryType, eowSinkPtr=eowSinkPtr, boardInsertFamily=boardInsertFamily, EOWType=EOWType, laserALSRestart90s=laserALSRestart90s, LedStatus=LedStatus, adrFailHoldOffTime=adrFailHoldOffTime, hardInventoryIndex=hardInventoryIndex, ProtectionType=ProtectionType, hardInv=hardInv, boardFirstPortIndex=boardFirstPortIndex, boardMismatchSev=boardMismatchSev, LedType=LedType, eowName=eowName, shelf=shelf, EOWClockMode=EOWClockMode, boardList=boardList, boardTable=boardTable, ledStatus=ledStatus, adrHotReset=adrHotReset, boardInsertType=boardInsertType, hardInventoryEntry=hardInventoryEntry, boardListNumber=boardListNumber, boardSeverity=boardSeverity, ledType=ledType, softInventoryBoard=softInventoryBoard, softInventoryEntry=softInventoryEntry, ledPosition=ledPosition, eowSinkType=eowSinkType, boardDefectiveSev=boardDefectiveSev, eowSrcPtr=eowSrcPtr, eowEntry=eowEntry, hardInventoryNumber=hardInventoryNumber, boardListSlot=boardListSlot, laserIndex=laserIndex)
