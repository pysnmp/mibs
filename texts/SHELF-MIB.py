...
mibBuilder.exportSymbols("SHELF-MIB", laserALSEnable=laserALSEnable, shelf=shelf, BoardFailure=BoardFailure, laser=laser, EOWType=EOWType, boardExpectType=boardExpectType, laserTable=laserTable, softInv=softInv, boardFailure=boardFailure, ledStatus=ledStatus, LedType=LedType, eowSrcType=eowSrcType, hardInventoryEntry=hardInventoryEntry, laserEntry=laserEntry, boardInsertFamily=boardInsertFamily, softInventoryIndex=softInventoryIndex, eowSinkType=eowSinkType, softInventoryType=softInventoryType, ledEntry=ledEntry, laserIndex=laserIndex, HoldTime=HoldTime, hardInventoryDescr=hardInventoryDescr, softInventoryExt=softInventoryExt, boardActive=boardActive, boardMonitor=boardMonitor, boardDefectiveSev=boardDefectiveSev, adrHotReset=adrHotReset, laserALSRestart90s=laserALSRestart90s, eow=eow, boardFirstPortIndex=boardFirstPortIndex, ledType=ledType, eowTable=eowTable, softInventoryCode=softInventoryCode, led=led, eowName=eowName, boardSlotName=boardSlotName, adrFailHoldOnTime=adrFailHoldOnTime, laserALSRestart2s=laserALSRestart2s, boardListNumber=boardListNumber, softInventoryTable=softInventoryTable, ledTable=ledTable, boardTable=boardTable, hardInventoryNumber=hardInventoryNumber, softInventoryBoard=softInventoryBoard, softInventoryNumber=softInventoryNumber, boardListTable=boardListTable, hardInv=hardInv, EOWClockMode=EOWClockMode, laserNumber=laserNumber, PYSNMP_MODULE_ID=shelf, hardInventoryBoard=hardInventoryBoard, softInventoryEntry=softInventoryEntry, boardList=boardList, boardListEntry=boardListEntry, holdTime=holdTime, eowNumber=eowNumber, eowSrcPtr=eowSrcPtr, eowClockMode=eowClockMode, boardNumber=boardNumber, eowSinkPtr=eowSinkPtr, eowEntry=eowEntry, boardIndex=boardIndex, ledNumber=ledNumber, boardListIndex=boardListIndex, boardMissingSev=boardMissingSev, boardMismatchSev=boardMismatchSev, ProtectionType=ProtectionType, ledPosition=ledPosition, boardListSlot=boardListSlot, adrFailHoldOffTime=adrFailHoldOffTime, boardListType=boardListType, boardProtectionType=boardProtectionType, boardInsertType=boardInsertType, board=board, boardEntry=boardEntry, hardInventoryIndex=hardInventoryIndex, LedStatus=LedStatus, hardInventoryTable=hardInventoryTable, boardExpectFamily=boardExpectFamily, ledIndex=ledIndex, eowIndex=eowIndex, laserTxEnable=laserTxEnable, boardSeverity=boardSeverity)
