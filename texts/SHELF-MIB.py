...
mibBuilder.exportSymbols("SHELF-MIB", ledStatus=ledStatus, hardInv=hardInv, boardSeverity=boardSeverity, adrFailHoldOffTime=adrFailHoldOffTime, eowSinkPtr=eowSinkPtr, eowClockMode=eowClockMode, boardListType=boardListType, boardSlotName=boardSlotName, laserIndex=laserIndex, boardDefectiveSev=boardDefectiveSev, eowSrcType=eowSrcType, boardProtectionType=boardProtectionType, eowTable=eowTable, holdTime=holdTime, EOWClockMode=EOWClockMode, boardInsertType=boardInsertType, boardInsertFamily=boardInsertFamily, hardInventoryNumber=hardInventoryNumber, eowSinkType=eowSinkType, board=board, boardExpectFamily=boardExpectFamily, softInv=softInv, boardList=boardList, boardEntry=boardEntry, boardNumber=boardNumber, softInventoryEntry=softInventoryEntry, boardMissingSev=boardMissingSev, hardInventoryIndex=hardInventoryIndex, boardListTable=boardListTable, laserALSRestart2s=laserALSRestart2s, boardIndex=boardIndex, laserALSEnable=laserALSEnable, ledNumber=ledNumber, boardListSlot=boardListSlot, eowSrcPtr=eowSrcPtr, eowNumber=eowNumber, LedType=LedType, BoardFailure=BoardFailure, led=led, HoldTime=HoldTime, LedStatus=LedStatus, ledPosition=ledPosition, shelf=shelf, PYSNMP_MODULE_ID=shelf, hardInventoryTable=hardInventoryTable, laser=laser, laserEntry=laserEntry, laserALSRestart90s=laserALSRestart90s, softInventoryNumber=softInventoryNumber, hardInventoryDescr=hardInventoryDescr, boardFirstPortIndex=boardFirstPortIndex, hardInventoryBoard=hardInventoryBoard, softInventoryIndex=softInventoryIndex, laserNumber=laserNumber, adrFailHoldOnTime=adrFailHoldOnTime, softInventoryCode=softInventoryCode, eowName=eowName, boardTable=boardTable, boardMismatchSev=boardMismatchSev, softInventoryBoard=softInventoryBoard, boardActive=boardActive, boardListEntry=boardListEntry, EOWType=EOWType, ProtectionType=ProtectionType, ledTable=ledTable, eow=eow, ledIndex=ledIndex, boardListIndex=boardListIndex, softInventoryType=softInventoryType, hardInventoryEntry=hardInventoryEntry, softInventoryTable=softInventoryTable, boardMonitor=boardMonitor, boardListNumber=boardListNumber, softInventoryExt=softInventoryExt, eowEntry=eowEntry, eowIndex=eowIndex, boardExpectType=boardExpectType, ledEntry=ledEntry, laserTxEnable=laserTxEnable, adrHotReset=adrHotReset, boardFailure=boardFailure, ledType=ledType, laserTable=laserTable)
