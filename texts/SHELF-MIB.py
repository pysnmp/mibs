...
mibBuilder.exportSymbols("SHELF-MIB", boardListSlot=boardListSlot, ledType=ledType, boardSlotName=boardSlotName, shelf=shelf, boardListNumber=boardListNumber, eowTable=eowTable, EOWType=EOWType, boardMonitor=boardMonitor, eowEntry=eowEntry, hardInventoryNumber=hardInventoryNumber, eow=eow, softInventoryTable=softInventoryTable, PYSNMP_MODULE_ID=shelf, laserTable=laserTable, boardMismatchSev=boardMismatchSev, laserALSRestart90s=laserALSRestart90s, hardInventoryEntry=hardInventoryEntry, softInv=softInv, board=board, laserNumber=laserNumber, hardInventoryDescr=hardInventoryDescr, boardTable=boardTable, boardDefectiveSev=boardDefectiveSev, boardInsertType=boardInsertType, laserTxEnable=laserTxEnable, boardSeverity=boardSeverity, BoardFailure=BoardFailure, ledIndex=ledIndex, hardInv=hardInv, eowNumber=eowNumber, laserEntry=laserEntry, eowSinkPtr=eowSinkPtr, softInventoryBoard=softInventoryBoard, laserALSRestart2s=laserALSRestart2s, boardExpectType=boardExpectType, EOWClockMode=EOWClockMode, LedType=LedType, eowSrcPtr=eowSrcPtr, ledEntry=ledEntry, boardFailure=boardFailure, boardEntry=boardEntry, softInventoryExt=softInventoryExt, boardList=boardList, softInventoryIndex=softInventoryIndex, boardActive=boardActive, ledNumber=ledNumber, softInventoryEntry=softInventoryEntry, boardMissingSev=boardMissingSev, ProtectionType=ProtectionType, holdTime=holdTime, hardInventoryTable=hardInventoryTable, softInventoryNumber=softInventoryNumber, boardInsertFamily=boardInsertFamily, eowClockMode=eowClockMode, laserALSEnable=laserALSEnable, eowIndex=eowIndex, eowName=eowName, boardNumber=boardNumber, LedStatus=LedStatus, softInventoryType=softInventoryType, boardIndex=boardIndex, boardProtectionType=boardProtectionType, adrFailHoldOnTime=adrFailHoldOnTime, eowSinkType=eowSinkType, boardListTable=boardListTable, adrHotReset=adrHotReset, hardInventoryIndex=hardInventoryIndex, hardInventoryBoard=hardInventoryBoard, ledStatus=ledStatus, boardFirstPortIndex=boardFirstPortIndex, boardListType=boardListType, HoldTime=HoldTime, ledPosition=ledPosition, boardExpectFamily=boardExpectFamily, ledTable=ledTable, boardListEntry=boardListEntry, eowSrcType=eowSrcType, softInventoryCode=softInventoryCode, boardListIndex=boardListIndex, led=led, laserIndex=laserIndex, adrFailHoldOffTime=adrFailHoldOffTime, laser=laser)
