...
mibBuilder.exportSymbols("SHELF-MIB", hardInventoryEntry=hardInventoryEntry, adrHotReset=adrHotReset, BoardFailure=BoardFailure, ledType=ledType, boardMonitor=boardMonitor, laserNumber=laserNumber, laserTxEnable=laserTxEnable, LedType=LedType, boardIndex=boardIndex, PYSNMP_MODULE_ID=shelf, boardSlotName=boardSlotName, eowSrcType=eowSrcType, boardListIndex=boardListIndex, ProtectionType=ProtectionType, ledTable=ledTable, boardListType=boardListType, eowSinkPtr=eowSinkPtr, HoldTime=HoldTime, softInventoryEntry=softInventoryEntry, boardList=boardList, EOWClockMode=EOWClockMode, board=board, boardListSlot=boardListSlot, LedStatus=LedStatus, laserIndex=laserIndex, ledStatus=ledStatus, eowName=eowName, softInventoryBoard=softInventoryBoard, EOWType=EOWType, boardActive=boardActive, holdTime=holdTime, led=led, shelf=shelf, softInventoryExt=softInventoryExt, eowIndex=eowIndex, eowNumber=eowNumber, eowClockMode=eowClockMode, softInventoryType=softInventoryType, ledEntry=ledEntry, boardEntry=boardEntry, boardTable=boardTable, laserTable=laserTable, boardListTable=boardListTable, hardInv=hardInv, hardInventoryTable=hardInventoryTable, softInventoryNumber=softInventoryNumber, boardDefectiveSev=boardDefectiveSev, laser=laser, softInventoryIndex=softInventoryIndex, boardFailure=boardFailure, boardSeverity=boardSeverity, boardMissingSev=boardMissingSev, hardInventoryDescr=hardInventoryDescr, boardFirstPortIndex=boardFirstPortIndex, boardListNumber=boardListNumber, laserALSRestart2s=laserALSRestart2s, boardListEntry=boardListEntry, hardInventoryNumber=hardInventoryNumber, softInv=softInv, eowEntry=eowEntry, eowTable=eowTable, boardInsertFamily=boardInsertFamily, boardExpectType=boardExpectType, adrFailHoldOffTime=adrFailHoldOffTime, eowSinkType=eowSinkType, hardInventoryIndex=hardInventoryIndex, laserALSRestart90s=laserALSRestart90s, ledIndex=ledIndex, boardProtectionType=boardProtectionType, laserALSEnable=laserALSEnable, adrFailHoldOnTime=adrFailHoldOnTime, eowSrcPtr=eowSrcPtr, softInventoryTable=softInventoryTable, softInventoryCode=softInventoryCode, ledNumber=ledNumber, boardExpectFamily=boardExpectFamily, ledPosition=ledPosition, boardNumber=boardNumber, laserEntry=laserEntry, boardInsertType=boardInsertType, boardMismatchSev=boardMismatchSev, eow=eow, hardInventoryBoard=hardInventoryBoard)
