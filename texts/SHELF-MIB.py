...
mibBuilder.exportSymbols("SHELF-MIB", boardTable=boardTable, eowTable=eowTable, eowSinkPtr=eowSinkPtr, HoldTime=HoldTime, boardListTable=boardListTable, boardEntry=boardEntry, boardInsertType=boardInsertType, hardInventoryNumber=hardInventoryNumber, boardIndex=boardIndex, softInv=softInv, boardFailure=boardFailure, boardSlotName=boardSlotName, boardFirstPortIndex=boardFirstPortIndex, BoardFailure=BoardFailure, laserNumber=laserNumber, boardListType=boardListType, laserEntry=laserEntry, eowEntry=eowEntry, laserIndex=laserIndex, boardListEntry=boardListEntry, LedType=LedType, laserALSEnable=laserALSEnable, ProtectionType=ProtectionType, boardNumber=boardNumber, boardSeverity=boardSeverity, eowClockMode=eowClockMode, laserALSRestart2s=laserALSRestart2s, shelf=shelf, hardInventoryIndex=hardInventoryIndex, laser=laser, eowIndex=eowIndex, softInventoryType=softInventoryType, softInventoryCode=softInventoryCode, board=board, hardInventoryTable=hardInventoryTable, ledStatus=ledStatus, laserTable=laserTable, softInventoryEntry=softInventoryEntry, ledEntry=ledEntry, hardInventoryDescr=hardInventoryDescr, eowSrcType=eowSrcType, softInventoryNumber=softInventoryNumber, ledNumber=ledNumber, hardInv=hardInv, softInventoryExt=softInventoryExt, ledPosition=ledPosition, ledType=ledType, ledIndex=ledIndex, adrFailHoldOffTime=adrFailHoldOffTime, boardMissingSev=boardMissingSev, boardExpectType=boardExpectType, boardMismatchSev=boardMismatchSev, boardProtectionType=boardProtectionType, hardInventoryEntry=hardInventoryEntry, eowSrcPtr=eowSrcPtr, adrFailHoldOnTime=adrFailHoldOnTime, softInventoryBoard=softInventoryBoard, softInventoryTable=softInventoryTable, eowSinkType=eowSinkType, boardListNumber=boardListNumber, boardActive=boardActive, LedStatus=LedStatus, softInventoryIndex=softInventoryIndex, laserTxEnable=laserTxEnable, adrHotReset=adrHotReset, boardListIndex=boardListIndex, eowNumber=eowNumber, boardListSlot=boardListSlot, EOWType=EOWType, eow=eow, boardList=boardList, boardExpectFamily=boardExpectFamily, hardInventoryBoard=hardInventoryBoard, boardDefectiveSev=boardDefectiveSev, PYSNMP_MODULE_ID=shelf, boardMonitor=boardMonitor, eowName=eowName, boardInsertFamily=boardInsertFamily, EOWClockMode=EOWClockMode, laserALSRestart90s=laserALSRestart90s, led=led, ledTable=ledTable, holdTime=holdTime)
