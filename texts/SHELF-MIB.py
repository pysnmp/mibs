...
mibBuilder.exportSymbols("SHELF-MIB", LedStatus=LedStatus, boardListTable=boardListTable, laserALSRestart90s=laserALSRestart90s, boardList=boardList, boardNumber=boardNumber, ledPosition=ledPosition, hardInv=hardInv, laserALSRestart2s=laserALSRestart2s, softInventoryCode=softInventoryCode, BoardFailure=BoardFailure, eowSrcType=eowSrcType, hardInventoryIndex=hardInventoryIndex, ledTable=ledTable, laser=laser, boardListIndex=boardListIndex, boardMissingSev=boardMissingSev, softInventoryIndex=softInventoryIndex, eowEntry=eowEntry, HoldTime=HoldTime, boardIndex=boardIndex, boardListEntry=boardListEntry, adrHotReset=adrHotReset, boardSlotName=boardSlotName, eowTable=eowTable, ledStatus=ledStatus, hardInventoryBoard=hardInventoryBoard, adrFailHoldOnTime=adrFailHoldOnTime, boardListSlot=boardListSlot, boardListType=boardListType, laserALSEnable=laserALSEnable, softInventoryType=softInventoryType, laserTable=laserTable, eowSinkPtr=eowSinkPtr, hardInventoryEntry=hardInventoryEntry, shelf=shelf, eowIndex=eowIndex, boardSeverity=boardSeverity, hardInventoryTable=hardInventoryTable, boardExpectType=boardExpectType, boardActive=boardActive, hardInventoryDescr=hardInventoryDescr, laserIndex=laserIndex, softInventoryEntry=softInventoryEntry, boardInsertType=boardInsertType, boardFailure=boardFailure, board=board, boardEntry=boardEntry, ledType=ledType, boardMismatchSev=boardMismatchSev, eowName=eowName, softInventoryNumber=softInventoryNumber, eow=eow, adrFailHoldOffTime=adrFailHoldOffTime, EOWClockMode=EOWClockMode, softInv=softInv, led=led, eowNumber=eowNumber, softInventoryExt=softInventoryExt, softInventoryTable=softInventoryTable, eowSinkType=eowSinkType, boardMonitor=boardMonitor, laserTxEnable=laserTxEnable, boardFirstPortIndex=boardFirstPortIndex, eowClockMode=eowClockMode, boardListNumber=boardListNumber, hardInventoryNumber=hardInventoryNumber, boardExpectFamily=boardExpectFamily, EOWType=EOWType, LedType=LedType, softInventoryBoard=softInventoryBoard, ledEntry=ledEntry, boardProtectionType=boardProtectionType, ledNumber=ledNumber, boardDefectiveSev=boardDefectiveSev, eowSrcPtr=eowSrcPtr, ledIndex=ledIndex, laserNumber=laserNumber, ProtectionType=ProtectionType, laserEntry=laserEntry, boardTable=boardTable, holdTime=holdTime, PYSNMP_MODULE_ID=shelf, boardInsertFamily=boardInsertFamily)
