...
mibBuilder.exportSymbols("SHELF-MIB", boardExpectType=boardExpectType, boardMismatchSev=boardMismatchSev, EOWClockMode=EOWClockMode, softInventoryBoard=softInventoryBoard, holdTime=holdTime, boardActive=boardActive, laserEntry=laserEntry, boardListSlot=boardListSlot, laserALSRestart2s=laserALSRestart2s, ledPosition=ledPosition, laser=laser, hardInventoryTable=hardInventoryTable, eowSinkType=eowSinkType, ledEntry=ledEntry, led=led, hardInventoryNumber=hardInventoryNumber, laserALSRestart90s=laserALSRestart90s, boardFirstPortIndex=boardFirstPortIndex, boardProtectionType=boardProtectionType, boardListIndex=boardListIndex, laserIndex=laserIndex, boardInsertType=boardInsertType, ledNumber=ledNumber, softInventoryExt=softInventoryExt, boardExpectFamily=boardExpectFamily, eowSinkPtr=eowSinkPtr, ledType=ledType, LedType=LedType, hardInv=hardInv, adrFailHoldOnTime=adrFailHoldOnTime, eowClockMode=eowClockMode, boardMonitor=boardMonitor, boardEntry=boardEntry, hardInventoryEntry=hardInventoryEntry, eowNumber=eowNumber, boardListNumber=boardListNumber, boardFailure=boardFailure, PYSNMP_MODULE_ID=shelf, ledStatus=ledStatus, softInventoryTable=softInventoryTable, HoldTime=HoldTime, laserTxEnable=laserTxEnable, ledIndex=ledIndex, softInventoryType=softInventoryType, hardInventoryBoard=hardInventoryBoard, softInventoryIndex=softInventoryIndex, boardInsertFamily=boardInsertFamily, ProtectionType=ProtectionType, eowTable=eowTable, ledTable=ledTable, hardInventoryIndex=hardInventoryIndex, boardNumber=boardNumber, adrFailHoldOffTime=adrFailHoldOffTime, boardDefectiveSev=boardDefectiveSev, boardListType=boardListType, boardSeverity=boardSeverity, shelf=shelf, board=board, eowSrcType=eowSrcType, softInventoryEntry=softInventoryEntry, eowSrcPtr=eowSrcPtr, boardTable=boardTable, eowName=eowName, laserTable=laserTable, eowEntry=eowEntry, boardListTable=boardListTable, LedStatus=LedStatus, boardMissingSev=boardMissingSev, boardListEntry=boardListEntry, laserNumber=laserNumber, softInventoryNumber=softInventoryNumber, hardInventoryDescr=hardInventoryDescr, laserALSEnable=laserALSEnable, EOWType=EOWType, boardList=boardList, adrHotReset=adrHotReset, boardIndex=boardIndex, boardSlotName=boardSlotName, softInv=softInv, BoardFailure=BoardFailure, softInventoryCode=softInventoryCode, eow=eow, eowIndex=eowIndex)
