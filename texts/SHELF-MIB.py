...
mibBuilder.exportSymbols("SHELF-MIB", softInventoryExt=softInventoryExt, shelf=shelf, softInventoryNumber=softInventoryNumber, laser=laser, softInventoryIndex=softInventoryIndex, LedStatus=LedStatus, eow=eow, boardListType=boardListType, ledTable=ledTable, eowEntry=eowEntry, BoardFailure=BoardFailure, adrFailHoldOnTime=adrFailHoldOnTime, softInv=softInv, boardMismatchSev=boardMismatchSev, ledType=ledType, eowTable=eowTable, softInventoryTable=softInventoryTable, boardListNumber=boardListNumber, adrHotReset=adrHotReset, hardInventoryBoard=hardInventoryBoard, laserTxEnable=laserTxEnable, boardListIndex=boardListIndex, EOWClockMode=EOWClockMode, laserNumber=laserNumber, boardSlotName=boardSlotName, softInventoryEntry=softInventoryEntry, boardFirstPortIndex=boardFirstPortIndex, hardInventoryIndex=hardInventoryIndex, PYSNMP_MODULE_ID=shelf, boardListEntry=boardListEntry, led=led, boardMissingSev=boardMissingSev, boardEntry=boardEntry, hardInventoryDescr=hardInventoryDescr, holdTime=holdTime, hardInv=hardInv, boardMonitor=boardMonitor, hardInventoryNumber=hardInventoryNumber, boardExpectFamily=boardExpectFamily, eowSinkPtr=eowSinkPtr, boardProtectionType=boardProtectionType, ledPosition=ledPosition, eowClockMode=eowClockMode, eowSrcType=eowSrcType, adrFailHoldOffTime=adrFailHoldOffTime, boardDefectiveSev=boardDefectiveSev, hardInventoryTable=hardInventoryTable, ledEntry=ledEntry, boardInsertType=boardInsertType, laserEntry=laserEntry, laserIndex=laserIndex, boardSeverity=boardSeverity, boardExpectType=boardExpectType, boardIndex=boardIndex, softInventoryBoard=softInventoryBoard, laserALSEnable=laserALSEnable, eowIndex=eowIndex, boardActive=boardActive, EOWType=EOWType, eowSinkType=eowSinkType, eowNumber=eowNumber, boardListTable=boardListTable, eowSrcPtr=eowSrcPtr, ledNumber=ledNumber, ProtectionType=ProtectionType, boardListSlot=boardListSlot, boardList=boardList, laserALSRestart90s=laserALSRestart90s, softInventoryType=softInventoryType, hardInventoryEntry=hardInventoryEntry, eowName=eowName, boardNumber=boardNumber, LedType=LedType, softInventoryCode=softInventoryCode, laserTable=laserTable, boardInsertFamily=boardInsertFamily, laserALSRestart2s=laserALSRestart2s, ledStatus=ledStatus, boardTable=boardTable, boardFailure=boardFailure, ledIndex=ledIndex, HoldTime=HoldTime, board=board)
