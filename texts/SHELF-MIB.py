...
mibBuilder.exportSymbols("SHELF-MIB", boardSeverity=boardSeverity, boardList=boardList, ProtectionType=ProtectionType, boardNumber=boardNumber, ledNumber=ledNumber, ledType=ledType, eowName=eowName, boardFailure=boardFailure, boardActive=boardActive, board=board, laserALSRestart2s=laserALSRestart2s, boardProtectionType=boardProtectionType, hardInv=hardInv, boardEntry=boardEntry, LedStatus=LedStatus, softInventoryExt=softInventoryExt, eowTable=eowTable, HoldTime=HoldTime, laserTxEnable=laserTxEnable, led=led, boardInsertType=boardInsertType, softInventoryEntry=softInventoryEntry, boardExpectFamily=boardExpectFamily, boardDefectiveSev=boardDefectiveSev, hardInventoryBoard=hardInventoryBoard, hardInventoryIndex=hardInventoryIndex, eowIndex=eowIndex, hardInventoryTable=hardInventoryTable, eowSrcPtr=eowSrcPtr, laserNumber=laserNumber, eowNumber=eowNumber, softInv=softInv, boardFirstPortIndex=boardFirstPortIndex, hardInventoryDescr=hardInventoryDescr, hardInventoryEntry=hardInventoryEntry, laserALSRestart90s=laserALSRestart90s, BoardFailure=BoardFailure, boardMonitor=boardMonitor, ledEntry=ledEntry, boardIndex=boardIndex, softInventoryIndex=softInventoryIndex, boardListType=boardListType, laserEntry=laserEntry, boardListTable=boardListTable, adrFailHoldOnTime=adrFailHoldOnTime, softInventoryNumber=softInventoryNumber, shelf=shelf, boardListNumber=boardListNumber, eowSinkPtr=eowSinkPtr, laserTable=laserTable, boardMismatchSev=boardMismatchSev, boardInsertFamily=boardInsertFamily, boardListIndex=boardListIndex, hardInventoryNumber=hardInventoryNumber, EOWClockMode=EOWClockMode, ledStatus=ledStatus, adrHotReset=adrHotReset, ledTable=ledTable, softInventoryCode=softInventoryCode, ledPosition=ledPosition, boardMissingSev=boardMissingSev, eowSinkType=eowSinkType, eow=eow, EOWType=EOWType, LedType=LedType, adrFailHoldOffTime=adrFailHoldOffTime, boardListEntry=boardListEntry, boardExpectType=boardExpectType, boardListSlot=boardListSlot, laserIndex=laserIndex, ledIndex=ledIndex, laserALSEnable=laserALSEnable, boardSlotName=boardSlotName, laser=laser, softInventoryType=softInventoryType, PYSNMP_MODULE_ID=shelf, softInventoryBoard=softInventoryBoard, eowSrcType=eowSrcType, eowClockMode=eowClockMode, softInventoryTable=softInventoryTable, eowEntry=eowEntry, boardTable=boardTable, holdTime=holdTime)
