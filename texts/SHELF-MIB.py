...
mibBuilder.exportSymbols("SHELF-MIB", hardInventoryIndex=hardInventoryIndex, ledType=ledType, eowClockMode=eowClockMode, eowNumber=eowNumber, eowSinkType=eowSinkType, eowIndex=eowIndex, softInventoryCode=softInventoryCode, boardListIndex=boardListIndex, PYSNMP_MODULE_ID=shelf, boardActive=boardActive, ProtectionType=ProtectionType, eowSrcPtr=eowSrcPtr, boardNumber=boardNumber, eowEntry=eowEntry, boardListType=boardListType, laserIndex=laserIndex, boardInsertType=boardInsertType, boardListEntry=boardListEntry, ledPosition=ledPosition, boardSlotName=boardSlotName, boardListNumber=boardListNumber, softInventoryTable=softInventoryTable, laserALSRestart2s=laserALSRestart2s, hardInventoryDescr=hardInventoryDescr, boardProtectionType=boardProtectionType, softInventoryIndex=softInventoryIndex, EOWClockMode=EOWClockMode, hardInv=hardInv, EOWType=EOWType, boardListTable=boardListTable, boardFirstPortIndex=boardFirstPortIndex, hardInventoryNumber=hardInventoryNumber, boardExpectFamily=boardExpectFamily, boardList=boardList, BoardFailure=BoardFailure, holdTime=holdTime, boardEntry=boardEntry, softInv=softInv, softInventoryExt=softInventoryExt, laser=laser, adrFailHoldOffTime=adrFailHoldOffTime, adrHotReset=adrHotReset, softInventoryNumber=softInventoryNumber, ledStatus=ledStatus, board=board, boardIndex=boardIndex, laserALSRestart90s=laserALSRestart90s, boardDefectiveSev=boardDefectiveSev, laserALSEnable=laserALSEnable, boardListSlot=boardListSlot, boardFailure=boardFailure, eowSinkPtr=eowSinkPtr, ledTable=ledTable, hardInventoryBoard=hardInventoryBoard, softInventoryEntry=softInventoryEntry, boardExpectType=boardExpectType, laserTable=laserTable, eow=eow, softInventoryBoard=softInventoryBoard, led=led, adrFailHoldOnTime=adrFailHoldOnTime, boardSeverity=boardSeverity, ledIndex=ledIndex, boardInsertFamily=boardInsertFamily, HoldTime=HoldTime, eowName=eowName, hardInventoryEntry=hardInventoryEntry, LedType=LedType, eowSrcType=eowSrcType, boardMonitor=boardMonitor, laserTxEnable=laserTxEnable, LedStatus=LedStatus, softInventoryType=softInventoryType, boardMissingSev=boardMissingSev, laserEntry=laserEntry, laserNumber=laserNumber, boardTable=boardTable, ledNumber=ledNumber, eowTable=eowTable, hardInventoryTable=hardInventoryTable, shelf=shelf, ledEntry=ledEntry, boardMismatchSev=boardMismatchSev)
