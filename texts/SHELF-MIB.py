...
mibBuilder.exportSymbols("SHELF-MIB", ledIndex=ledIndex, boardEntry=boardEntry, ledStatus=ledStatus, LedStatus=LedStatus, adrHotReset=adrHotReset, EOWClockMode=EOWClockMode, softInventoryType=softInventoryType, eowTable=eowTable, laserTable=laserTable, eowSrcType=eowSrcType, adrFailHoldOnTime=adrFailHoldOnTime, softInventoryBoard=softInventoryBoard, hardInventoryNumber=hardInventoryNumber, softInventoryNumber=softInventoryNumber, ledType=ledType, ProtectionType=ProtectionType, laserIndex=laserIndex, eowIndex=eowIndex, board=board, boardActive=boardActive, boardProtectionType=boardProtectionType, BoardFailure=BoardFailure, LedType=LedType, eowClockMode=eowClockMode, led=led, softInventoryExt=softInventoryExt, softInventoryTable=softInventoryTable, eowNumber=eowNumber, shelf=shelf, adrFailHoldOffTime=adrFailHoldOffTime, boardListIndex=boardListIndex, eowEntry=eowEntry, ledNumber=ledNumber, laserTxEnable=laserTxEnable, eowSrcPtr=eowSrcPtr, laserEntry=laserEntry, softInventoryCode=softInventoryCode, boardListSlot=boardListSlot, boardListNumber=boardListNumber, boardTable=boardTable, hardInventoryTable=hardInventoryTable, eowSinkType=eowSinkType, softInv=softInv, laserALSRestart90s=laserALSRestart90s, hardInventoryEntry=hardInventoryEntry, eow=eow, hardInventoryDescr=hardInventoryDescr, boardList=boardList, PYSNMP_MODULE_ID=shelf, boardListTable=boardListTable, holdTime=holdTime, hardInventoryIndex=hardInventoryIndex, boardExpectFamily=boardExpectFamily, boardListEntry=boardListEntry, hardInv=hardInv, boardMissingSev=boardMissingSev, ledPosition=ledPosition, boardInsertFamily=boardInsertFamily, eowSinkPtr=eowSinkPtr, hardInventoryBoard=hardInventoryBoard, boardDefectiveSev=boardDefectiveSev, boardExpectType=boardExpectType, softInventoryIndex=softInventoryIndex, boardFailure=boardFailure, laserNumber=laserNumber, boardListType=boardListType, boardSeverity=boardSeverity, HoldTime=HoldTime, boardMonitor=boardMonitor, laserALSEnable=laserALSEnable, EOWType=EOWType, boardSlotName=boardSlotName, eowName=eowName, ledEntry=ledEntry, boardMismatchSev=boardMismatchSev, softInventoryEntry=softInventoryEntry, laserALSRestart2s=laserALSRestart2s, laser=laser, boardInsertType=boardInsertType, ledTable=ledTable, boardNumber=boardNumber, boardFirstPortIndex=boardFirstPortIndex, boardIndex=boardIndex)
