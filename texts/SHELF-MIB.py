...
mibBuilder.exportSymbols("SHELF-MIB", boardList=boardList, boardSeverity=boardSeverity, laserIndex=laserIndex, hardInventoryTable=hardInventoryTable, laserALSEnable=laserALSEnable, softInventoryType=softInventoryType, eowSrcPtr=eowSrcPtr, boardListIndex=boardListIndex, boardExpectType=boardExpectType, eowSrcType=eowSrcType, softInventoryNumber=softInventoryNumber, eowName=eowName, ledTable=ledTable, boardActive=boardActive, laserALSRestart90s=laserALSRestart90s, adrFailHoldOffTime=adrFailHoldOffTime, laserEntry=laserEntry, ProtectionType=ProtectionType, softInventoryTable=softInventoryTable, softInv=softInv, softInventoryIndex=softInventoryIndex, hardInventoryIndex=hardInventoryIndex, adrFailHoldOnTime=adrFailHoldOnTime, shelf=shelf, LedStatus=LedStatus, softInventoryCode=softInventoryCode, boardListEntry=boardListEntry, led=led, boardListType=boardListType, boardMonitor=boardMonitor, laserALSRestart2s=laserALSRestart2s, adrHotReset=adrHotReset, eowIndex=eowIndex, boardFailure=boardFailure, board=board, boardTable=boardTable, laser=laser, boardIndex=boardIndex, EOWType=EOWType, EOWClockMode=EOWClockMode, ledEntry=ledEntry, HoldTime=HoldTime, boardInsertFamily=boardInsertFamily, ledStatus=ledStatus, BoardFailure=BoardFailure, boardProtectionType=boardProtectionType, PYSNMP_MODULE_ID=shelf, ledIndex=ledIndex, hardInventoryDescr=hardInventoryDescr, eowSinkType=eowSinkType, LedType=LedType, hardInventoryNumber=hardInventoryNumber, eowSinkPtr=eowSinkPtr, laserTxEnable=laserTxEnable, holdTime=holdTime, ledPosition=ledPosition, ledType=ledType, boardInsertType=boardInsertType, eowEntry=eowEntry, boardListTable=boardListTable, boardMissingSev=boardMissingSev, eowNumber=eowNumber, boardListSlot=boardListSlot, laserTable=laserTable, boardMismatchSev=boardMismatchSev, boardExpectFamily=boardExpectFamily, softInventoryEntry=softInventoryEntry, hardInventoryBoard=hardInventoryBoard, boardDefectiveSev=boardDefectiveSev, hardInventoryEntry=hardInventoryEntry, boardListNumber=boardListNumber, ledNumber=ledNumber, softInventoryExt=softInventoryExt, boardFirstPortIndex=boardFirstPortIndex, boardNumber=boardNumber, softInventoryBoard=softInventoryBoard, eow=eow, boardEntry=boardEntry, laserNumber=laserNumber, eowClockMode=eowClockMode, boardSlotName=boardSlotName, eowTable=eowTable, hardInv=hardInv)
