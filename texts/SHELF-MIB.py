...
mibBuilder.exportSymbols("SHELF-MIB", hardInventoryBoard=hardInventoryBoard, softInventoryEntry=softInventoryEntry, eow=eow, led=led, hardInventoryIndex=hardInventoryIndex, adrFailHoldOffTime=adrFailHoldOffTime, boardListIndex=boardListIndex, hardInventoryNumber=hardInventoryNumber, eowTable=eowTable, laserALSRestart2s=laserALSRestart2s, boardMonitor=boardMonitor, LedType=LedType, boardListType=boardListType, softInventoryExt=softInventoryExt, softInventoryType=softInventoryType, boardExpectFamily=boardExpectFamily, ledStatus=ledStatus, BoardFailure=BoardFailure, boardExpectType=boardExpectType, laserIndex=laserIndex, boardFirstPortIndex=boardFirstPortIndex, EOWClockMode=EOWClockMode, eowSinkType=eowSinkType, laserTable=laserTable, boardListNumber=boardListNumber, hardInventoryTable=hardInventoryTable, adrFailHoldOnTime=adrFailHoldOnTime, boardListTable=boardListTable, boardListEntry=boardListEntry, ledNumber=ledNumber, ledIndex=ledIndex, boardProtectionType=boardProtectionType, laserALSEnable=laserALSEnable, boardSlotName=boardSlotName, softInventoryIndex=softInventoryIndex, boardNumber=boardNumber, ledTable=ledTable, LedStatus=LedStatus, eowSinkPtr=eowSinkPtr, board=board, hardInventoryEntry=hardInventoryEntry, softInventoryNumber=softInventoryNumber, eowSrcType=eowSrcType, boardSeverity=boardSeverity, boardTable=boardTable, hardInv=hardInv, softInv=softInv, boardDefectiveSev=boardDefectiveSev, shelf=shelf, laserTxEnable=laserTxEnable, boardList=boardList, softInventoryCode=softInventoryCode, boardIndex=boardIndex, eowEntry=eowEntry, eowSrcPtr=eowSrcPtr, eowName=eowName, boardListSlot=boardListSlot, PYSNMP_MODULE_ID=shelf, eowClockMode=eowClockMode, boardInsertType=boardInsertType, boardMismatchSev=boardMismatchSev, hardInventoryDescr=hardInventoryDescr, eowNumber=eowNumber, boardActive=boardActive, laserALSRestart90s=laserALSRestart90s, boardFailure=boardFailure, laserNumber=laserNumber, boardMissingSev=boardMissingSev, eowIndex=eowIndex, softInventoryTable=softInventoryTable, laser=laser, ledEntry=ledEntry, adrHotReset=adrHotReset, ledType=ledType, softInventoryBoard=softInventoryBoard, EOWType=EOWType, ProtectionType=ProtectionType, HoldTime=HoldTime, holdTime=holdTime, boardEntry=boardEntry, laserEntry=laserEntry, boardInsertFamily=boardInsertFamily, ledPosition=ledPosition)
