...
mibBuilder.exportSymbols("SHELF-MIB", PYSNMP_MODULE_ID=shelf, laserIndex=laserIndex, laserALSRestart90s=laserALSRestart90s, boardMissingSev=boardMissingSev, EOWType=EOWType, hardInventoryEntry=hardInventoryEntry, adrFailHoldOnTime=adrFailHoldOnTime, eowEntry=eowEntry, boardDefectiveSev=boardDefectiveSev, boardMismatchSev=boardMismatchSev, boardListType=boardListType, laserTable=laserTable, hardInventoryNumber=hardInventoryNumber, laserEntry=laserEntry, boardSlotName=boardSlotName, boardFirstPortIndex=boardFirstPortIndex, hardInventoryDescr=hardInventoryDescr, board=board, boardListIndex=boardListIndex, boardProtectionType=boardProtectionType, boardSeverity=boardSeverity, boardActive=boardActive, softInv=softInv, ledNumber=ledNumber, ledStatus=ledStatus, ProtectionType=ProtectionType, boardListTable=boardListTable, eowClockMode=eowClockMode, HoldTime=HoldTime, boardFailure=boardFailure, hardInv=hardInv, softInventoryEntry=softInventoryEntry, LedType=LedType, laserNumber=laserNumber, LedStatus=LedStatus, ledPosition=ledPosition, laserALSEnable=laserALSEnable, boardListEntry=boardListEntry, softInventoryNumber=softInventoryNumber, eowSinkType=eowSinkType, boardTable=boardTable, ledIndex=ledIndex, softInventoryIndex=softInventoryIndex, BoardFailure=BoardFailure, hardInventoryIndex=hardInventoryIndex, boardNumber=boardNumber, boardList=boardList, boardIndex=boardIndex, holdTime=holdTime, boardMonitor=boardMonitor, adrFailHoldOffTime=adrFailHoldOffTime, ledEntry=ledEntry, eowSrcType=eowSrcType, boardEntry=boardEntry, hardInventoryBoard=hardInventoryBoard, led=led, laserTxEnable=laserTxEnable, EOWClockMode=EOWClockMode, laser=laser, eowName=eowName, boardInsertFamily=boardInsertFamily, softInventoryCode=softInventoryCode, eowIndex=eowIndex, softInventoryBoard=softInventoryBoard, hardInventoryTable=hardInventoryTable, boardListNumber=boardListNumber, eowTable=eowTable, ledTable=ledTable, shelf=shelf, boardExpectType=boardExpectType, softInventoryExt=softInventoryExt, adrHotReset=adrHotReset, boardExpectFamily=boardExpectFamily, laserALSRestart2s=laserALSRestart2s, softInventoryTable=softInventoryTable, eowNumber=eowNumber, eowSrcPtr=eowSrcPtr, boardInsertType=boardInsertType, eow=eow, ledType=ledType, softInventoryType=softInventoryType, boardListSlot=boardListSlot, eowSinkPtr=eowSinkPtr)
