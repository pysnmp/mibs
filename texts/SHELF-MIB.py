...
mibBuilder.exportSymbols("SHELF-MIB", boardListTable=boardListTable, boardNumber=boardNumber, eowSinkPtr=eowSinkPtr, softInventoryBoard=softInventoryBoard, ledType=ledType, softInv=softInv, laserALSRestart90s=laserALSRestart90s, shelf=shelf, softInventoryTable=softInventoryTable, LedStatus=LedStatus, adrHotReset=adrHotReset, boardExpectFamily=boardExpectFamily, boardListNumber=boardListNumber, boardProtectionType=boardProtectionType, adrFailHoldOffTime=adrFailHoldOffTime, laser=laser, hardInventoryBoard=hardInventoryBoard, laserNumber=laserNumber, ledIndex=ledIndex, boardActive=boardActive, laserALSEnable=laserALSEnable, boardListSlot=boardListSlot, eowEntry=eowEntry, board=board, ProtectionType=ProtectionType, softInventoryIndex=softInventoryIndex, laserIndex=laserIndex, softInventoryType=softInventoryType, holdTime=holdTime, EOWType=EOWType, boardTable=boardTable, laserTxEnable=laserTxEnable, softInventoryCode=softInventoryCode, boardEntry=boardEntry, boardListIndex=boardListIndex, EOWClockMode=EOWClockMode, boardMismatchSev=boardMismatchSev, boardListType=boardListType, laserTable=laserTable, boardMonitor=boardMonitor, eowClockMode=eowClockMode, laserALSRestart2s=laserALSRestart2s, boardIndex=boardIndex, eowSrcPtr=eowSrcPtr, led=led, eowSinkType=eowSinkType, eowIndex=eowIndex, boardFailure=boardFailure, hardInventoryIndex=hardInventoryIndex, ledStatus=ledStatus, eowName=eowName, softInventoryExt=softInventoryExt, eowNumber=eowNumber, boardFirstPortIndex=boardFirstPortIndex, boardSlotName=boardSlotName, BoardFailure=BoardFailure, hardInventoryNumber=hardInventoryNumber, boardExpectType=boardExpectType, adrFailHoldOnTime=adrFailHoldOnTime, ledPosition=ledPosition, HoldTime=HoldTime, ledEntry=ledEntry, boardListEntry=boardListEntry, laserEntry=laserEntry, hardInventoryDescr=hardInventoryDescr, ledNumber=ledNumber, hardInventoryEntry=hardInventoryEntry, hardInventoryTable=hardInventoryTable, softInventoryEntry=softInventoryEntry, eow=eow, boardList=boardList, boardMissingSev=boardMissingSev, hardInv=hardInv, softInventoryNumber=softInventoryNumber, boardInsertFamily=boardInsertFamily, PYSNMP_MODULE_ID=shelf, boardSeverity=boardSeverity, ledTable=ledTable, boardDefectiveSev=boardDefectiveSev, eowSrcType=eowSrcType, boardInsertType=boardInsertType, LedType=LedType, eowTable=eowTable)
