...
mibBuilder.exportSymbols("SHELF-MIB", eowSrcType=eowSrcType, boardListSlot=boardListSlot, laserTable=laserTable, LedType=LedType, ProtectionType=ProtectionType, boardListIndex=boardListIndex, boardTable=boardTable, shelf=shelf, eowName=eowName, boardExpectFamily=boardExpectFamily, softInventoryCode=softInventoryCode, boardInsertFamily=boardInsertFamily, boardSlotName=boardSlotName, softInventoryTable=softInventoryTable, ledPosition=ledPosition, laserALSRestart90s=laserALSRestart90s, hardInventoryNumber=hardInventoryNumber, EOWClockMode=EOWClockMode, led=led, boardInsertType=boardInsertType, boardProtectionType=boardProtectionType, ledStatus=ledStatus, eowTable=eowTable, eow=eow, adrHotReset=adrHotReset, boardExpectType=boardExpectType, boardMismatchSev=boardMismatchSev, hardInv=hardInv, EOWType=EOWType, boardIndex=boardIndex, laserEntry=laserEntry, BoardFailure=BoardFailure, eowNumber=eowNumber, ledType=ledType, boardMonitor=boardMonitor, ledNumber=ledNumber, softInventoryNumber=softInventoryNumber, softInventoryExt=softInventoryExt, softInventoryType=softInventoryType, softInventoryIndex=softInventoryIndex, adrFailHoldOffTime=adrFailHoldOffTime, boardDefectiveSev=boardDefectiveSev, eowEntry=eowEntry, boardFailure=boardFailure, boardListNumber=boardListNumber, softInventoryEntry=softInventoryEntry, laserIndex=laserIndex, laser=laser, laserTxEnable=laserTxEnable, boardList=boardList, hardInventoryEntry=hardInventoryEntry, boardActive=boardActive, boardListEntry=boardListEntry, laserALSEnable=laserALSEnable, eowIndex=eowIndex, hardInventoryBoard=hardInventoryBoard, eowSinkType=eowSinkType, board=board, LedStatus=LedStatus, PYSNMP_MODULE_ID=shelf, eowClockMode=eowClockMode, holdTime=holdTime, HoldTime=HoldTime, ledIndex=ledIndex, boardMissingSev=boardMissingSev, hardInventoryDescr=hardInventoryDescr, softInventoryBoard=softInventoryBoard, hardInventoryIndex=hardInventoryIndex, laserALSRestart2s=laserALSRestart2s, adrFailHoldOnTime=adrFailHoldOnTime, hardInventoryTable=hardInventoryTable, laserNumber=laserNumber, ledTable=ledTable, ledEntry=ledEntry, eowSinkPtr=eowSinkPtr, boardEntry=boardEntry, boardSeverity=boardSeverity, eowSrcPtr=eowSrcPtr, boardListType=boardListType, boardListTable=boardListTable, boardFirstPortIndex=boardFirstPortIndex, boardNumber=boardNumber, softInv=softInv)
