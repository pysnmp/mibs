...
mibBuilder.exportSymbols("SHELF-MIB", laserTxEnable=laserTxEnable, eowSrcType=eowSrcType, boardMissingSev=boardMissingSev, boardExpectType=boardExpectType, adrHotReset=adrHotReset, ledStatus=ledStatus, ledTable=ledTable, laserNumber=laserNumber, boardIndex=boardIndex, eowSinkType=eowSinkType, eow=eow, hardInventoryBoard=hardInventoryBoard, laser=laser, hardInventoryIndex=hardInventoryIndex, laserTable=laserTable, boardFirstPortIndex=boardFirstPortIndex, boardNumber=boardNumber, boardDefectiveSev=boardDefectiveSev, boardProtectionType=boardProtectionType, boardEntry=boardEntry, hardInventoryEntry=hardInventoryEntry, hardInventoryDescr=hardInventoryDescr, softInv=softInv, boardListNumber=boardListNumber, LedType=LedType, ProtectionType=ProtectionType, shelf=shelf, HoldTime=HoldTime, laserIndex=laserIndex, boardInsertFamily=boardInsertFamily, laserEntry=laserEntry, EOWType=EOWType, boardListEntry=boardListEntry, boardListTable=boardListTable, boardListSlot=boardListSlot, softInventoryExt=softInventoryExt, boardMonitor=boardMonitor, adrFailHoldOffTime=adrFailHoldOffTime, LedStatus=LedStatus, holdTime=holdTime, softInventoryCode=softInventoryCode, ledNumber=ledNumber, laserALSEnable=laserALSEnable, eowSrcPtr=eowSrcPtr, eowName=eowName, softInventoryType=softInventoryType, boardListType=boardListType, led=led, softInventoryEntry=softInventoryEntry, boardExpectFamily=boardExpectFamily, ledPosition=ledPosition, eowSinkPtr=eowSinkPtr, boardInsertType=boardInsertType, eowEntry=eowEntry, hardInv=hardInv, softInventoryNumber=softInventoryNumber, boardTable=boardTable, laserALSRestart90s=laserALSRestart90s, eowNumber=eowNumber, eowIndex=eowIndex, boardFailure=boardFailure, EOWClockMode=EOWClockMode, adrFailHoldOnTime=adrFailHoldOnTime, BoardFailure=BoardFailure, boardSlotName=boardSlotName, eowClockMode=eowClockMode, softInventoryBoard=softInventoryBoard, laserALSRestart2s=laserALSRestart2s, ledType=ledType, PYSNMP_MODULE_ID=shelf, ledEntry=ledEntry, hardInventoryNumber=hardInventoryNumber, board=board, ledIndex=ledIndex, boardListIndex=boardListIndex, boardList=boardList, hardInventoryTable=hardInventoryTable, boardMismatchSev=boardMismatchSev, softInventoryIndex=softInventoryIndex, eowTable=eowTable, boardActive=boardActive, boardSeverity=boardSeverity, softInventoryTable=softInventoryTable)
