...
mibBuilder.exportSymbols("SHELF-MIB", hardInventoryIndex=hardInventoryIndex, HoldTime=HoldTime, laserALSRestart2s=laserALSRestart2s, boardEntry=boardEntry, eow=eow, laserALSRestart90s=laserALSRestart90s, softInventoryType=softInventoryType, boardDefectiveSev=boardDefectiveSev, softInventoryNumber=softInventoryNumber, laserNumber=laserNumber, eowClockMode=eowClockMode, eowSrcPtr=eowSrcPtr, boardListTable=boardListTable, boardFailure=boardFailure, hardInventoryNumber=hardInventoryNumber, hardInventoryEntry=hardInventoryEntry, laserTable=laserTable, EOWClockMode=EOWClockMode, eowEntry=eowEntry, PYSNMP_MODULE_ID=shelf, hardInventoryDescr=hardInventoryDescr, ledIndex=ledIndex, softInventoryExt=softInventoryExt, adrFailHoldOffTime=adrFailHoldOffTime, eowSinkPtr=eowSinkPtr, laserEntry=laserEntry, softInventoryEntry=softInventoryEntry, boardInsertType=boardInsertType, ledTable=ledTable, boardMissingSev=boardMissingSev, hardInventoryTable=hardInventoryTable, EOWType=EOWType, boardSeverity=boardSeverity, boardFirstPortIndex=boardFirstPortIndex, softInventoryTable=softInventoryTable, boardNumber=boardNumber, shelf=shelf, eowNumber=eowNumber, BoardFailure=BoardFailure, laserIndex=laserIndex, laserTxEnable=laserTxEnable, boardListSlot=boardListSlot, eowName=eowName, ledStatus=ledStatus, eowSrcType=eowSrcType, boardListType=boardListType, hardInventoryBoard=hardInventoryBoard, boardInsertFamily=boardInsertFamily, laserALSEnable=laserALSEnable, ledType=ledType, boardMismatchSev=boardMismatchSev, boardProtectionType=boardProtectionType, boardExpectFamily=boardExpectFamily, eowIndex=eowIndex, boardExpectType=boardExpectType, ledNumber=ledNumber, board=board, boardListIndex=boardListIndex, adrHotReset=adrHotReset, laser=laser, boardActive=boardActive, boardMonitor=boardMonitor, LedStatus=LedStatus, softInventoryIndex=softInventoryIndex, softInventoryBoard=softInventoryBoard, boardListNumber=boardListNumber, ProtectionType=ProtectionType, softInv=softInv, hardInv=hardInv, boardIndex=boardIndex, boardSlotName=boardSlotName, boardListEntry=boardListEntry, boardList=boardList, holdTime=holdTime, eowSinkType=eowSinkType, adrFailHoldOnTime=adrFailHoldOnTime, eowTable=eowTable, ledEntry=ledEntry, led=led, boardTable=boardTable, LedType=LedType, softInventoryCode=softInventoryCode, ledPosition=ledPosition)
