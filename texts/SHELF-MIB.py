...
mibBuilder.exportSymbols("SHELF-MIB", eowTable=eowTable, boardInsertType=boardInsertType, boardListTable=boardListTable, laserNumber=laserNumber, ledType=ledType, eow=eow, boardDefectiveSev=boardDefectiveSev, boardExpectFamily=boardExpectFamily, ledNumber=ledNumber, eowSinkPtr=eowSinkPtr, boardListEntry=boardListEntry, boardMismatchSev=boardMismatchSev, shelf=shelf, laserTable=laserTable, hardInventoryDescr=hardInventoryDescr, softInventoryCode=softInventoryCode, boardMissingSev=boardMissingSev, hardInventoryBoard=hardInventoryBoard, boardActive=boardActive, boardTable=boardTable, hardInventoryEntry=hardInventoryEntry, HoldTime=HoldTime, EOWClockMode=EOWClockMode, LedType=LedType, boardSeverity=boardSeverity, adrFailHoldOnTime=adrFailHoldOnTime, softInventoryType=softInventoryType, laserALSEnable=laserALSEnable, adrHotReset=adrHotReset, hardInv=hardInv, eowSrcType=eowSrcType, ledStatus=ledStatus, BoardFailure=BoardFailure, eowSrcPtr=eowSrcPtr, boardListSlot=boardListSlot, softInventoryTable=softInventoryTable, boardInsertFamily=boardInsertFamily, eowClockMode=eowClockMode, laserALSRestart90s=laserALSRestart90s, LedStatus=LedStatus, softInventoryNumber=softInventoryNumber, ledTable=ledTable, boardIndex=boardIndex, boardMonitor=boardMonitor, holdTime=holdTime, boardFirstPortIndex=boardFirstPortIndex, boardListIndex=boardListIndex, laserTxEnable=laserTxEnable, boardNumber=boardNumber, boardEntry=boardEntry, ledIndex=ledIndex, hardInventoryNumber=hardInventoryNumber, PYSNMP_MODULE_ID=shelf, eowIndex=eowIndex, laserEntry=laserEntry, boardProtectionType=boardProtectionType, eowEntry=eowEntry, eowSinkType=eowSinkType, softInventoryEntry=softInventoryEntry, eowNumber=eowNumber, EOWType=EOWType, boardListNumber=boardListNumber, ledPosition=ledPosition, boardExpectType=boardExpectType, adrFailHoldOffTime=adrFailHoldOffTime, eowName=eowName, boardSlotName=boardSlotName, boardList=boardList, laser=laser, ProtectionType=ProtectionType, softInv=softInv, ledEntry=ledEntry, laserALSRestart2s=laserALSRestart2s, led=led, boardFailure=boardFailure, board=board, boardListType=boardListType, softInventoryExt=softInventoryExt, hardInventoryIndex=hardInventoryIndex, laserIndex=laserIndex, hardInventoryTable=hardInventoryTable, softInventoryIndex=softInventoryIndex, softInventoryBoard=softInventoryBoard)
