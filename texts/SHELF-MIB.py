...
mibBuilder.exportSymbols("SHELF-MIB", ledTable=ledTable, ledType=ledType, boardProtectionType=boardProtectionType, boardActive=boardActive, boardIndex=boardIndex, eowSinkType=eowSinkType, hardInv=hardInv, boardListNumber=boardListNumber, boardExpectType=boardExpectType, boardList=boardList, softInventoryCode=softInventoryCode, eowEntry=eowEntry, hardInventoryIndex=hardInventoryIndex, adrFailHoldOnTime=adrFailHoldOnTime, hardInventoryBoard=hardInventoryBoard, softInventoryNumber=softInventoryNumber, boardListEntry=boardListEntry, ledPosition=ledPosition, eowClockMode=eowClockMode, softInventoryTable=softInventoryTable, laserNumber=laserNumber, hardInventoryTable=hardInventoryTable, laserEntry=laserEntry, ledEntry=ledEntry, eowIndex=eowIndex, boardEntry=boardEntry, eowNumber=eowNumber, laserIndex=laserIndex, hardInventoryEntry=hardInventoryEntry, boardMismatchSev=boardMismatchSev, boardTable=boardTable, softInv=softInv, softInventoryExt=softInventoryExt, eow=eow, boardInsertFamily=boardInsertFamily, ledIndex=ledIndex, shelf=shelf, laserTxEnable=laserTxEnable, laserALSRestart2s=laserALSRestart2s, boardNumber=boardNumber, boardListIndex=boardListIndex, boardListSlot=boardListSlot, boardFirstPortIndex=boardFirstPortIndex, EOWClockMode=EOWClockMode, boardFailure=boardFailure, holdTime=holdTime, eowName=eowName, eowTable=eowTable, softInventoryType=softInventoryType, boardSlotName=boardSlotName, boardDefectiveSev=boardDefectiveSev, HoldTime=HoldTime, laserTable=laserTable, eowSrcPtr=eowSrcPtr, laser=laser, led=led, LedStatus=LedStatus, softInventoryBoard=softInventoryBoard, boardSeverity=boardSeverity, adrFailHoldOffTime=adrFailHoldOffTime, eowSinkPtr=eowSinkPtr, boardExpectFamily=boardExpectFamily, laserALSEnable=laserALSEnable, EOWType=EOWType, ledStatus=ledStatus, adrHotReset=adrHotReset, boardInsertType=boardInsertType, eowSrcType=eowSrcType, boardMonitor=boardMonitor, softInventoryIndex=softInventoryIndex, ledNumber=ledNumber, board=board, hardInventoryDescr=hardInventoryDescr, LedType=LedType, boardListTable=boardListTable, boardListType=boardListType, ProtectionType=ProtectionType, PYSNMP_MODULE_ID=shelf, laserALSRestart90s=laserALSRestart90s, boardMissingSev=boardMissingSev, BoardFailure=BoardFailure, softInventoryEntry=softInventoryEntry, hardInventoryNumber=hardInventoryNumber)
