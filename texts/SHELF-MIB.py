...
mibBuilder.exportSymbols("SHELF-MIB", eowIndex=eowIndex, eowSrcType=eowSrcType, eowSinkPtr=eowSinkPtr, eowEntry=eowEntry, softInventoryCode=softInventoryCode, boardSlotName=boardSlotName, PYSNMP_MODULE_ID=shelf, boardMissingSev=boardMissingSev, boardActive=boardActive, LedType=LedType, HoldTime=HoldTime, laserTable=laserTable, adrFailHoldOnTime=adrFailHoldOnTime, hardInv=hardInv, boardFailure=boardFailure, boardExpectType=boardExpectType, eowSinkType=eowSinkType, eow=eow, EOWClockMode=EOWClockMode, boardListNumber=boardListNumber, hardInventoryIndex=hardInventoryIndex, ledStatus=ledStatus, hardInventoryNumber=hardInventoryNumber, hardInventoryDescr=hardInventoryDescr, boardMonitor=boardMonitor, boardSeverity=boardSeverity, ledNumber=ledNumber, boardInsertFamily=boardInsertFamily, shelf=shelf, boardListType=boardListType, boardDefectiveSev=boardDefectiveSev, ProtectionType=ProtectionType, softInventoryBoard=softInventoryBoard, softInventoryType=softInventoryType, EOWType=EOWType, led=led, adrHotReset=adrHotReset, boardMismatchSev=boardMismatchSev, softInventoryNumber=softInventoryNumber, boardInsertType=boardInsertType, softInventoryIndex=softInventoryIndex, softInventoryExt=softInventoryExt, eowTable=eowTable, boardListEntry=boardListEntry, boardIndex=boardIndex, laserALSRestart90s=laserALSRestart90s, ledTable=ledTable, laserALSRestart2s=laserALSRestart2s, laserTxEnable=laserTxEnable, boardListTable=boardListTable, softInventoryTable=softInventoryTable, ledIndex=ledIndex, boardEntry=boardEntry, eowNumber=eowNumber, holdTime=holdTime, ledEntry=ledEntry, laser=laser, boardFirstPortIndex=boardFirstPortIndex, hardInventoryTable=hardInventoryTable, boardListIndex=boardListIndex, eowClockMode=eowClockMode, board=board, boardListSlot=boardListSlot, boardNumber=boardNumber, eowName=eowName, softInventoryEntry=softInventoryEntry, laserALSEnable=laserALSEnable, boardExpectFamily=boardExpectFamily, eowSrcPtr=eowSrcPtr, laserIndex=laserIndex, LedStatus=LedStatus, adrFailHoldOffTime=adrFailHoldOffTime, softInv=softInv, ledType=ledType, hardInventoryEntry=hardInventoryEntry, boardList=boardList, BoardFailure=BoardFailure, ledPosition=ledPosition, laserEntry=laserEntry, boardProtectionType=boardProtectionType, laserNumber=laserNumber, hardInventoryBoard=hardInventoryBoard, boardTable=boardTable)
