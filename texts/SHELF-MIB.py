...
mibBuilder.exportSymbols("SHELF-MIB", softInv=softInv, eowSrcType=eowSrcType, boardListSlot=boardListSlot, boardFailure=boardFailure, softInventoryType=softInventoryType, eow=eow, softInventoryNumber=softInventoryNumber, ledIndex=ledIndex, laser=laser, boardTable=boardTable, boardDefectiveSev=boardDefectiveSev, LedType=LedType, boardList=boardList, boardListEntry=boardListEntry, boardActive=boardActive, laserALSEnable=laserALSEnable, shelf=shelf, holdTime=holdTime, eowIndex=eowIndex, ledEntry=ledEntry, boardInsertFamily=boardInsertFamily, hardInv=hardInv, boardMonitor=boardMonitor, adrFailHoldOnTime=adrFailHoldOnTime, HoldTime=HoldTime, boardSeverity=boardSeverity, eowSinkPtr=eowSinkPtr, eowTable=eowTable, eowEntry=eowEntry, adrFailHoldOffTime=adrFailHoldOffTime, hardInventoryDescr=hardInventoryDescr, BoardFailure=BoardFailure, eowName=eowName, hardInventoryNumber=hardInventoryNumber, boardMissingSev=boardMissingSev, laserIndex=laserIndex, boardListIndex=boardListIndex, ledType=ledType, ledTable=ledTable, boardProtectionType=boardProtectionType, softInventoryEntry=softInventoryEntry, boardEntry=boardEntry, led=led, laserALSRestart90s=laserALSRestart90s, laserTable=laserTable, LedStatus=LedStatus, laserALSRestart2s=laserALSRestart2s, eowNumber=eowNumber, eowSrcPtr=eowSrcPtr, ledStatus=ledStatus, PYSNMP_MODULE_ID=shelf, laserEntry=laserEntry, adrHotReset=adrHotReset, EOWType=EOWType, ProtectionType=ProtectionType, boardListTable=boardListTable, softInventoryBoard=softInventoryBoard, boardListNumber=boardListNumber, boardMismatchSev=boardMismatchSev, ledPosition=ledPosition, hardInventoryEntry=hardInventoryEntry, softInventoryTable=softInventoryTable, boardListType=boardListType, EOWClockMode=EOWClockMode, boardFirstPortIndex=boardFirstPortIndex, boardExpectFamily=boardExpectFamily, boardNumber=boardNumber, eowClockMode=eowClockMode, hardInventoryBoard=hardInventoryBoard, board=board, hardInventoryTable=hardInventoryTable, softInventoryExt=softInventoryExt, hardInventoryIndex=hardInventoryIndex, eowSinkType=eowSinkType, laserNumber=laserNumber, softInventoryCode=softInventoryCode, softInventoryIndex=softInventoryIndex, boardSlotName=boardSlotName, laserTxEnable=laserTxEnable, boardExpectType=boardExpectType, ledNumber=ledNumber, boardIndex=boardIndex, boardInsertType=boardInsertType)
