...
MibIdentifier, Gauge32, enterprises, ModuleIdentity, MibScalar, MibTable, MibTableRow, MibTableColumn, Counter64, iso, NotificationType, Integer32, Counter32, Bits, IpAddress, ObjectIdentity, Unsigned32, TimeTicks = mibBuilder.importSymbols("SNMPv2-SMI", "MibIdentifier", "Gauge32", "enterprises", "ModuleIdentity", "MibScalar", "MibTable", "MibTableRow", "MibTableColumn", "Counter64", "iso", "NotificationType", "Integer32", "Counter32", "Bits", "IpAddress", "ObjectIdentity", "Unsigned32", "TimeTicks")
...
mibBuilder.exportSymbols("ISILON-MIB", nodePerCPUIdle=nodePerCPUIdle, quotaUsageWithOverhead=quotaUsageWithOverhead, clusterCPUInterrupt=clusterCPUInterrupt, nodeCPUPerf=nodeCPUPerf, fanName=fanName, nodeNetworkPerf=nodeNetworkPerf, licenseExpirationDate=licenseExpirationDate, nodeCPUNice=nodeCPUNice, quotaID=quotaID, nodeNetworkOutBitsPerSecond=nodeNetworkOutBitsPerSecond, clusterCPUUser=clusterCPUUser, nodePerCPUSystem=nodePerCPUSystem, inMinBytes=inMinBytes, quotaType=quotaType, ifsAvailableBytes=ifsAvailableBytes, accessTimeEnabled=accessTimeEnabled, licenseEntry=licenseEntry, tempSensorTableGroup=tempSensorTableGroup, clusterPerformance=clusterPerformance, nodePerCPUNice=nodePerCPUNice, quotaDomainID=quotaDomainID, snapshotScheduledCreateEnabled=snapshotScheduledCreateEnabled, nodeCPUPerfGroup=nodeCPUPerfGroup, chassisSerialNumber=chassisSerialNumber, nodeStatusGroup=nodeStatusGroup, cluster=cluster, licenseStatus=licenseStatus, snapshotScheduleAlias=snapshotScheduleAlias, latencyMin=latencyMin, snapshotAliasFor=snapshotAliasFor, clusterIfsOutBytes=clusterIfsOutBytes, tempSensorDescription=tempSensorDescription, snapshotSettingsGroup=snapshotSettingsGroup, diskPerfTable=diskPerfTable, isilonCompliance=isilonCompliance, diskFirmwareVersion=diskFirmwareVersion, latencyMax=latencyMax, chassisNumber=chassisNumber, conformance=conformance, clusterIfsPerf=clusterIfsPerf, clusterNetworkInBytes=clusterNetworkInBytes, quotaGracePeriod=quotaGracePeriod, clusterNetworkOutBitsPerSecond=clusterNetworkOutBitsPerSecond, diskDeviceName=diskDeviceName, diskStatus=diskStatus, fanNumber=fanNumber, quotaIncludesOverhead=quotaIncludesOverhead, snapshotSettings=snapshotSettings, protocolOpsPerSecond=protocolOpsPerSecond, clusterCPUNice=clusterCPUNice, onlineNodes=onlineNodes, credentialBindings=credentialBindings, diskTableGroup=diskTableGroup, snapshotsGroup=snapshotsGroup, outMinBytes=outMinBytes, quotaInodeUsage=quotaInodeUsage, diskModel=diskModel, PYSNMP_MODULE_ID=isilon, licensesGroup=licensesGroup, nodeName=nodeName, snapshotRootAccessLocal=snapshotRootAccessLocal, configuredNodes=configuredNodes, nodeCPUPerfEntry=nodeCPUPerfEntry, nodePerCPUUser=nodePerCPUUser, snapshotScheduleIndex=snapshotScheduleIndex, clusterStatusGroup=clusterStatusGroup, nodeCPUInterrupt=nodeCPUInterrupt, clusterGUID=clusterGUID, diskBay=diskBay, fanDescription=fanDescription, clusterNetworkPerf=clusterNetworkPerf, diskEntry=diskEntry, nodeCPUUser=nodeCPUUser, quotaEntry=quotaEntry, quotaUsage=quotaUsage, chassisConfigNumber=chassisConfigNumber, clusterIfsOutBitsPerSecond=clusterIfsOutBitsPerSecond, nodeIfsPerfGroup=nodeIfsPerfGroup, powerSensorNumber=powerSensorNumber, nodeIfsInBitsPerSecond=nodeIfsInBitsPerSecond, snapshotRootVisibilityCIFS=snapshotRootVisibilityCIFS, snapshotIndex=snapshotIndex, nodeGroups=nodeGroups, snapshotCreated=snapshotCreated, nodeNetworkPerfGroup=nodeNetworkPerfGroup, readOnly=readOnly, nodeCPUPerfTable=nodeCPUPerfTable, chassisEntry=chassisEntry, snapshotName=snapshotName, nodePerCPUID=nodePerCPUID, nodeNetworkInBytes=nodeNetworkInBytes, snapshotRootAccessNFS=snapshotRootAccessNFS, quotaHardThresholdDefined=quotaHardThresholdDefined, nodeCPUSystem=nodeCPUSystem, clusterName=clusterName, ifsTotalBytes=ifsTotalBytes, clusterNetworkInBitsPerSecond=clusterNetworkInBitsPerSecond, snapshotScheduleTable=snapshotScheduleTable, nodeCPUIdle=nodeCPUIdle, outMaxBytes=outMaxBytes, snapshotPath=snapshotPath, ifsFreeBytes=ifsFreeBytes, accessTimeGracePeriod=accessTimeGracePeriod, nodeCount=nodeCount, fanSpeed=fanSpeed, clusterGroups=clusterGroups, isilon=isilon, snapshotReservedPct=snapshotReservedPct, fanEntry=fanEntry, tempSensorTable=tempSensorTable, clusterPerformanceGroups=clusterPerformanceGroups, quotaTable=quotaTable, quotas=quotas, snapshotSubdirAccessCIFS=snapshotSubdirAccessCIFS, tempSensorName=tempSensorName, latencyStdDev=latencyStdDev, licenseTable=licenseTable, fanTable=fanTable, quotaHardThreshold=quotaHardThreshold, quotaAdvisoryThreshold=quotaAdvisoryThreshold, powerSensorTable=powerSensorTable, powerSensorEntry=powerSensorEntry, clusterNetworkPerfGroup=clusterNetworkPerfGroup, local=local, snapshotSize=snapshotSize, diskChassisNumber=diskChassisNumber, node=node, ifsFilesystemGroup=ifsFilesystemGroup, snapshotLocked=snapshotLocked, quotasGroup=quotasGroup, quotaIncludesSnapshotUsage=quotaIncludesSnapshotUsage, inAvgBytes=inAvgBytes, diskPerfBay=diskPerfBay, diskTable=diskTable, chassisTable=chassisTable, clusterHealth=clusterHealth, clusterCPUPerfGroup=clusterCPUPerfGroup, clusterCPUIdlePct=clusterCPUIdlePct, clusterCPUPerf=clusterCPUPerf, clusterStatus=clusterStatus, clusterIfsInBitsPerSecond=clusterIfsInBitsPerSecond, snapshotRootAccessCIFS=snapshotRootAccessCIFS, snapshotSubdirAccessNFS=snapshotSubdirAccessNFS, snapshotRootVisibilityLocal=snapshotRootVisibilityLocal, snapshotEntry=snapshotEntry, nodeIfsOutBitsPerSecond=nodeIfsOutBitsPerSecond, tempSensorNumber=tempSensorNumber, snapshotRootVisibilityNFS=snapshotRootVisibilityNFS, powerSensorDescription=powerSensorDescription, ifsUsedBytes=ifsUsedBytes, clusterNetworkOutBytes=clusterNetworkOutBytes, licenseModuleName=licenseModuleName, latencyAverage=latencyAverage, licenseIndex=licenseIndex, nodeIfsInBytes=nodeIfsInBytes, nodeNetworkInBitsPerSecond=nodeNetworkInBitsPerSecond, snapshotTable=snapshotTable, nodeProtocolPerfEntry=nodeProtocolPerfEntry, snapshotScheduleNamingPattern=snapshotScheduleNamingPattern, nodeNetworkOutBytes=nodeNetworkOutBytes, licenses=licenses, nodePerformance=nodePerformance, diskPerfDeviceName=diskPerfDeviceName, diskLogicalNumber=diskLogicalNumber, quotaAdvisoryThresholdDefined=quotaAdvisoryThresholdDefined, snapshotScheduleName=snapshotScheduleName, chassisModel=chassisModel, powerSensorTableGroup=powerSensorTableGroup, quotaSoftThreshold=quotaSoftThreshold, nodeIfsPerf=nodeIfsPerf, inStdDevBytes=inStdDevBytes, snapshotScheduledDeleteEnabled=snapshotScheduledDeleteEnabled, fanTableGroup=fanTableGroup, snapshots=snapshots, snapshotScheduleTableGroup=snapshotScheduleTableGroup, snapshotExpires=snapshotExpires, clusterCPUSystem=clusterCPUSystem, powerSensorValue=powerSensorValue, diskPerfOutBitsPerSecond=diskPerfOutBitsPerSecond, snapshotScheduleExpiration=snapshotScheduleExpiration, offlineNodes=offlineNodes, nodePerCPUInterrupt=nodePerCPUInterrupt, snapshotSchedulePath=snapshotSchedulePath, diskPerfEntry=diskPerfEntry, nodeProtocolPerfTableGroup=nodeProtocolPerfTableGroup, quotaPath=quotaPath, nodeType=nodeType, nodePerformanceGroup=nodePerformanceGroup, diskPerfOpsPerSecond=diskPerfOpsPerSecond, tempSensorEntry=tempSensorEntry, clusterIfsInBytes=clusterIfsInBytes, snapshotScheduleSchedule=snapshotScheduleSchedule, quotaSoftThresholdDefined=quotaSoftThresholdDefined, powerSensorName=powerSensorName, diskPerfTableGroup=diskPerfTableGroup, snapshotSubdirAccessLocal=snapshotSubdirAccessLocal, nodeHealth=nodeHealth, snapshotScheduleEntry=snapshotScheduleEntry, outAvgBytes=outAvgBytes, snapshotTableGroup=snapshotTableGroup, chassisTableGroup=chassisTableGroup, inMaxBytes=inMaxBytes, diskSizeBytes=diskSizeBytes, nodeProtocolPerfTable=nodeProtocolPerfTable, chassisUnitIDLEDOn=chassisUnitIDLEDOn, tempSensorValue=tempSensorValue, nodeIfsOutBytes=nodeIfsOutBytes, diskPerfInBitsPerSecond=diskPerfInBitsPerSecond, clusterIfsPerfGroup=clusterIfsPerfGroup, outStdDevBytes=outStdDevBytes, ifsFilesystem=ifsFilesystem, outBitsPerSecond=outBitsPerSecond, nodeStatus=nodeStatus, protocolOpCount=protocolOpCount, protocolName=protocolName, inBitsPerSecond=inBitsPerSecond, diskSerialNumber=diskSerialNumber)
