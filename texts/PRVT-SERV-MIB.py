...
mibBuilder.exportSymbols("PRVT-SERV-MIB", serviceMacAddressSapPortId=serviceMacAddressSapPortId, sapIngressQosSchedulerPolicy=sapIngressQosSchedulerPolicy, customerDeleted=customerDeleted, PYSNMP_MODULE_ID=prvtServicesMIB, sapType=sapType, serviceDescription=serviceDescription, sdpFarEndIpAddress=sdpFarEndIpAddress, customerContact=customerContact, sdpType=sdpType, sapEntry=sapEntry, serviceAdminStatus=serviceAdminStatus, serviceMacAddressSapPriority=serviceMacAddressSapPriority, serviceTable=serviceTable, servicePwRedundOperModeMesh=servicePwRedundOperModeMesh, serviceMacAddressSapMacAddress=serviceMacAddressSapMacAddress, serviceMacAddressSdpId=serviceMacAddressSdpId, sdpDynamicVcEgressLabel=sdpDynamicVcEgressLabel, sapCreated=sapCreated, sapEgressQosPolicyId=sapEgressQosPolicyId, serviceRowStatus=serviceRowStatus, sapPortId=sapPortId, serviceMacAddressSapServiceId=serviceMacAddressSapServiceId, sapCustMultSvcSiteName=sapCustMultSvcSiteName, serviceMtu=serviceMtu, serviceMacAddressSdpPriority=serviceMacAddressSdpPriority, serviceNumSaps=serviceNumSaps, sapRowStatus=sapRowStatus, sdpVCType=sdpVCType, sapEncapValue=sapEncapValue, sdpAdminEgressLabel=sdpAdminEgressLabel, sdpOperStatus=sdpOperStatus, serviceVlanAction=serviceVlanAction, sdpPwActivate=sdpPwActivate, serviceVpnId=serviceVpnId, sdpOperFlags=sdpOperFlags, serviceMacAddressSdpMacAddress=serviceMacAddressSdpMacAddress, serviceMacAddressSapEntry=serviceMacAddressSapEntry, sapIngressQosPolicyId=sapIngressQosPolicyId, sdpAdminIngressLabel=sdpAdminIngressLabel, sapEgressQosSchedulerPolicy=sapEgressQosSchedulerPolicy, sdpTransportTunnelName=sdpTransportTunnelName, sdpPwPrecedence=sdpPwPrecedence, sapUntaggedMode=sapUntaggedMode, sdpCreated=sdpCreated, serviceMacAddressSapRowStatus=serviceMacAddressSapRowStatus, serviceMacAddressSdpServiceId=serviceMacAddressSdpServiceId, serviceVlanEtherType=serviceVlanEtherType, sdpGroupIdentifier=sdpGroupIdentifier, sapDescription=sapDescription, customerEntry=customerEntry, serviceId=serviceId, prvtServicesMIBNotifications=prvtServicesMIBNotifications, customerTable=customerTable, serviceRevertTimer=serviceRevertTimer, serviceMacAddressSdpEntry=serviceMacAddressSdpEntry, serviceLastMgmtChange=serviceLastMgmtChange, sdpId=sdpId, customerPhone=customerPhone, serviceMacAddressSapTable=serviceMacAddressSapTable, prvtServicesMIB=prvtServicesMIB, sdpRowStatus=sdpRowStatus, serviceOperStatus=serviceOperStatus, sapOperFlags=sapOperFlags, sapLearnEnable=sapLearnEnable, sdpBindVlanTag=sdpBindVlanTag, customerRowStatus=customerRowStatus, servicePwRedundOperModeSpoke=servicePwRedundOperModeSpoke, sdpOutInterface=sdpOutInterface, sapDeleted=sapDeleted, sapAdminStatus=sapAdminStatus, sdpDescription=sdpDescription, sdpDeleted=sdpDeleted, serviceMacAddressSdpRowStatus=serviceMacAddressSdpRowStatus, serviceMacAddressSdpTable=serviceMacAddressSdpTable, sdpLastMgmtChange=sdpLastMgmtChange, serviceType=serviceType, serviceDeleted=serviceDeleted, serviceMacAddressSapEncapValue=serviceMacAddressSapEncapValue, sapLastMgmtChange=sapLastMgmtChange, sdpLearnEnable=sdpLearnEnable, customerCreated=customerCreated, sapLearnMode=sapLearnMode, prvtServicesMIBObjects=prvtServicesMIBObjects, serviceNumSdps=serviceNumSdps, sdpAdminStatus=sdpAdminStatus, sapOperStatus=sapOperStatus, serviceEnableSecureSaps=serviceEnableSecureSaps, prvtSapEventPropagationProfile=prvtSapEventPropagationProfile, sapProfileName=sapProfileName, sdpSignalPwStatus=sdpSignalPwStatus, sdpDynamicVcIngressLabel=sdpDynamicVcIngressLabel, sapMacLearningProfileState=sapMacLearningProfileState, prvtTlsSapEventPropagationProfile=prvtTlsSapEventPropagationProfile, sdpEntry=sdpEntry, serviceEntry=serviceEntry, serviceCustName=serviceCustName, sapCurrentMacCount=sapCurrentMacCount, sdpTable=sdpTable, sdpMtu=sdpMtu, sdpDelivery=sdpDelivery, sdpNextFreeId=sdpNextFreeId, serviceCreated=serviceCreated, sdpLabelSignaling=sdpLabelSignaling, customerName=customerName, serviceBackupVlanEtherType=serviceBackupVlanEtherType, sdpLdpEnabled=sdpLdpEnabled, sdpSecuredEnable=sdpSecuredEnable, sapTable=sapTable, sdpPwRedundancyEnable=sdpPwRedundancyEnable)
