...
RowStatus, TextualConvention, TruthValue, TimeStamp, DisplayString, MacAddress = mibBuilder.importSymbols("SNMPv2-TC", "RowStatus", "TextualConvention", "TruthValue", "TimeStamp", "DisplayString", "MacAddress")
...
mibBuilder.exportSymbols("PRVT-SERV-MIB", serviceCustName=serviceCustName, sdpFarEndIpAddress=sdpFarEndIpAddress, sapCustMultSvcSiteName=sapCustMultSvcSiteName, serviceMacAddressSapPriority=serviceMacAddressSapPriority, sdpTransportTunnelName=sdpTransportTunnelName, sdpLearnEnable=sdpLearnEnable, serviceMacAddressSapTable=serviceMacAddressSapTable, serviceDescription=serviceDescription, sdpType=sdpType, serviceMacAddressSdpEntry=serviceMacAddressSdpEntry, serviceBackupVlanEtherType=serviceBackupVlanEtherType, customerEntry=customerEntry, sdpDynamicVcEgressLabel=sdpDynamicVcEgressLabel, sdpSecuredEnable=sdpSecuredEnable, serviceCreated=serviceCreated, sapLearnMode=sapLearnMode, serviceLastMgmtChange=serviceLastMgmtChange, sapRowStatus=sapRowStatus, sdpDelivery=sdpDelivery, sapOperStatus=sapOperStatus, serviceMacAddressSdpMacAddress=serviceMacAddressSdpMacAddress, sdpGroupIdentifier=sdpGroupIdentifier, sapType=sapType, sapDeleted=sapDeleted, serviceMtu=serviceMtu, servicePwRedundOperModeSpoke=servicePwRedundOperModeSpoke, customerCreated=customerCreated, sdpSignalPwStatus=sdpSignalPwStatus, sdpPwPrecedence=sdpPwPrecedence, sdpPwRedundancyEnable=sdpPwRedundancyEnable, serviceNumSaps=serviceNumSaps, serviceType=serviceType, customerName=customerName, customerDeleted=customerDeleted, PYSNMP_MODULE_ID=prvtServicesMIB, serviceMacAddressSdpRowStatus=serviceMacAddressSdpRowStatus, sdpLastMgmtChange=sdpLastMgmtChange, customerContact=customerContact, sapPortId=sapPortId, serviceMacAddressSapPortId=serviceMacAddressSapPortId, sdpAdminEgressLabel=sdpAdminEgressLabel, sapAdminStatus=sapAdminStatus, customerRowStatus=customerRowStatus, sdpEntry=sdpEntry, sdpDynamicVcIngressLabel=sdpDynamicVcIngressLabel, serviceEnableSecureSaps=serviceEnableSecureSaps, sapCurrentMacCount=sapCurrentMacCount, sdpLdpEnabled=sdpLdpEnabled, sdpId=sdpId, prvtTlsSapEventPropagationProfile=prvtTlsSapEventPropagationProfile, serviceNumSdps=serviceNumSdps, serviceDeleted=serviceDeleted, sdpVCType=sdpVCType, serviceMacAddressSapEntry=serviceMacAddressSapEntry, sdpDeleted=sdpDeleted, prvtServicesMIB=prvtServicesMIB, sapUntaggedMode=sapUntaggedMode, sdpNextFreeId=sdpNextFreeId, serviceMacAddressSdpTable=serviceMacAddressSdpTable, sdpOutInterface=sdpOutInterface, sapEncapValue=sapEncapValue, sapLastMgmtChange=sapLastMgmtChange, sapCreated=sapCreated, sdpAdminIngressLabel=sdpAdminIngressLabel, serviceEntry=serviceEntry, sdpTable=sdpTable, customerPhone=customerPhone, sapOperFlags=sapOperFlags, serviceMacAddressSapMacAddress=serviceMacAddressSapMacAddress, serviceVlanAction=serviceVlanAction, sapIngressQosPolicyId=sapIngressQosPolicyId, sdpAdminStatus=sdpAdminStatus, sapLearnEnable=sapLearnEnable, serviceId=serviceId, sapEgressQosPolicyId=sapEgressQosPolicyId, sapProfileName=sapProfileName, serviceMacAddressSdpPriority=serviceMacAddressSdpPriority, sapEgressQosSchedulerPolicy=sapEgressQosSchedulerPolicy, serviceRevertTimer=serviceRevertTimer, customerTable=customerTable, serviceTable=serviceTable, prvtSapEventPropagationProfile=prvtSapEventPropagationProfile, serviceVlanEtherType=serviceVlanEtherType, serviceRowStatus=serviceRowStatus, sapIngressQosSchedulerPolicy=sapIngressQosSchedulerPolicy, serviceMacAddressSdpId=serviceMacAddressSdpId, serviceOperStatus=serviceOperStatus, sdpBindVlanTag=sdpBindVlanTag, sdpLabelSignaling=sdpLabelSignaling, prvtServicesMIBNotifications=prvtServicesMIBNotifications, serviceAdminStatus=serviceAdminStatus, sdpRowStatus=sdpRowStatus, sdpPwActivate=sdpPwActivate, sapEntry=sapEntry, sapDescription=sapDescription, sdpCreated=sdpCreated, sdpOperStatus=sdpOperStatus, sapMacLearningProfileState=sapMacLearningProfileState, sapTable=sapTable, serviceMacAddressSapServiceId=serviceMacAddressSapServiceId, serviceMacAddressSapRowStatus=serviceMacAddressSapRowStatus, sdpMtu=sdpMtu, servicePwRedundOperModeMesh=servicePwRedundOperModeMesh, serviceMacAddressSdpServiceId=serviceMacAddressSdpServiceId, sdpOperFlags=sdpOperFlags, serviceVpnId=serviceVpnId, sdpDescription=sdpDescription, prvtServicesMIBObjects=prvtServicesMIBObjects, serviceMacAddressSapEncapValue=serviceMacAddressSapEncapValue)
