...
mibBuilder.exportSymbols("PRVT-SERV-MIB", serviceMtu=serviceMtu, sdpSecuredEnable=sdpSecuredEnable, sapIngressQosSchedulerPolicy=sapIngressQosSchedulerPolicy, sdpPwActivate=sdpPwActivate, prvtSapEventPropagationProfile=prvtSapEventPropagationProfile, sdpEntry=sdpEntry, serviceNumSaps=serviceNumSaps, sapEgressQosPolicyId=sapEgressQosPolicyId, sdpOperStatus=sdpOperStatus, sdpDynamicVcEgressLabel=sdpDynamicVcEgressLabel, customerPhone=customerPhone, sapLastMgmtChange=sapLastMgmtChange, customerCreated=customerCreated, customerRowStatus=customerRowStatus, customerEntry=customerEntry, sapDescription=sapDescription, sdpLearnEnable=sdpLearnEnable, serviceEntry=serviceEntry, serviceEnableSecureSaps=serviceEnableSecureSaps, sapEncapValue=sapEncapValue, serviceNumSdps=serviceNumSdps, serviceDescription=serviceDescription, sdpDelivery=sdpDelivery, prvtServicesMIBNotifications=prvtServicesMIBNotifications, sdpOperFlags=sdpOperFlags, serviceMacAddressSdpId=serviceMacAddressSdpId, servicePwRedundOperModeMesh=servicePwRedundOperModeMesh, sapOperStatus=sapOperStatus, sapCreated=sapCreated, serviceMacAddressSapRowStatus=serviceMacAddressSapRowStatus, serviceMacAddressSdpPriority=serviceMacAddressSdpPriority, sapMacLearningProfileState=sapMacLearningProfileState, sdpLastMgmtChange=sdpLastMgmtChange, prvtServicesMIBObjects=prvtServicesMIBObjects, sdpLdpEnabled=sdpLdpEnabled, serviceCreated=serviceCreated, serviceDeleted=serviceDeleted, serviceRowStatus=serviceRowStatus, serviceMacAddressSapEntry=serviceMacAddressSapEntry, serviceMacAddressSapPriority=serviceMacAddressSapPriority, sdpDynamicVcIngressLabel=sdpDynamicVcIngressLabel, sdpAdminIngressLabel=sdpAdminIngressLabel, serviceMacAddressSapTable=serviceMacAddressSapTable, serviceId=serviceId, serviceVlanAction=serviceVlanAction, serviceMacAddressSdpTable=serviceMacAddressSdpTable, sapRowStatus=sapRowStatus, sdpLabelSignaling=sdpLabelSignaling, serviceTable=serviceTable, sapTable=sapTable, sapAdminStatus=sapAdminStatus, sapEntry=sapEntry, sapEgressQosSchedulerPolicy=sapEgressQosSchedulerPolicy, customerName=customerName, serviceOperStatus=serviceOperStatus, sdpPwRedundancyEnable=sdpPwRedundancyEnable, sdpAdminEgressLabel=sdpAdminEgressLabel, sdpAdminStatus=sdpAdminStatus, prvtServicesMIB=prvtServicesMIB, sapCustMultSvcSiteName=sapCustMultSvcSiteName, sapProfileName=sapProfileName, customerDeleted=customerDeleted, sdpVCType=sdpVCType, serviceVpnId=serviceVpnId, sdpId=sdpId, sdpCreated=sdpCreated, sapOperFlags=sapOperFlags, serviceMacAddressSdpEntry=serviceMacAddressSdpEntry, serviceLastMgmtChange=serviceLastMgmtChange, customerContact=customerContact, prvtTlsSapEventPropagationProfile=prvtTlsSapEventPropagationProfile, serviceMacAddressSdpServiceId=serviceMacAddressSdpServiceId, sdpDeleted=sdpDeleted, serviceMacAddressSapMacAddress=serviceMacAddressSapMacAddress, serviceMacAddressSdpMacAddress=serviceMacAddressSdpMacAddress, serviceMacAddressSapPortId=serviceMacAddressSapPortId, sapPortId=sapPortId, PYSNMP_MODULE_ID=prvtServicesMIB, serviceBackupVlanEtherType=serviceBackupVlanEtherType, sdpDescription=sdpDescription, sapDeleted=sapDeleted, serviceMacAddressSdpRowStatus=serviceMacAddressSdpRowStatus, sdpTransportTunnelName=sdpTransportTunnelName, serviceMacAddressSapEncapValue=serviceMacAddressSapEncapValue, sapUntaggedMode=sapUntaggedMode, serviceCustName=serviceCustName, sdpType=sdpType, sdpPwPrecedence=sdpPwPrecedence, servicePwRedundOperModeSpoke=servicePwRedundOperModeSpoke, serviceMacAddressSapServiceId=serviceMacAddressSapServiceId, sapCurrentMacCount=sapCurrentMacCount, serviceType=serviceType, serviceAdminStatus=serviceAdminStatus, sdpTable=sdpTable, sapIngressQosPolicyId=sapIngressQosPolicyId, sdpFarEndIpAddress=sdpFarEndIpAddress, sdpBindVlanTag=sdpBindVlanTag, serviceRevertTimer=serviceRevertTimer, sdpNextFreeId=sdpNextFreeId, sdpRowStatus=sdpRowStatus, sdpSignalPwStatus=sdpSignalPwStatus, sapType=sapType, serviceVlanEtherType=serviceVlanEtherType, customerTable=customerTable, sdpOutInterface=sdpOutInterface, sdpGroupIdentifier=sdpGroupIdentifier, sapLearnMode=sapLearnMode, sapLearnEnable=sapLearnEnable, sdpMtu=sdpMtu)
