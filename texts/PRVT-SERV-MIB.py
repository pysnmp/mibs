...
mibBuilder.exportSymbols("PRVT-SERV-MIB", sdpSecuredEnable=sdpSecuredEnable, sdpTable=sdpTable, sdpAdminStatus=sdpAdminStatus, sdpPwPrecedence=sdpPwPrecedence, prvtServicesMIBNotifications=prvtServicesMIBNotifications, sdpOperStatus=sdpOperStatus, customerName=customerName, serviceMacAddressSapPortId=serviceMacAddressSapPortId, prvtServicesMIBObjects=prvtServicesMIBObjects, sapLastMgmtChange=sapLastMgmtChange, customerEntry=customerEntry, PYSNMP_MODULE_ID=prvtServicesMIB, sapMacLearningProfileState=sapMacLearningProfileState, serviceDeleted=serviceDeleted, sdpDynamicVcEgressLabel=sdpDynamicVcEgressLabel, serviceMacAddressSapMacAddress=serviceMacAddressSapMacAddress, serviceRevertTimer=serviceRevertTimer, sapProfileName=sapProfileName, serviceEnableSecureSaps=serviceEnableSecureSaps, sdpDeleted=sdpDeleted, serviceType=serviceType, sapType=sapType, serviceMacAddressSdpMacAddress=serviceMacAddressSdpMacAddress, prvtSapEventPropagationProfile=prvtSapEventPropagationProfile, sdpGroupIdentifier=sdpGroupIdentifier, serviceNumSdps=serviceNumSdps, customerDeleted=customerDeleted, serviceVlanEtherType=serviceVlanEtherType, sdpDelivery=sdpDelivery, serviceMacAddressSapServiceId=serviceMacAddressSapServiceId, serviceAdminStatus=serviceAdminStatus, sapDeleted=sapDeleted, serviceBackupVlanEtherType=serviceBackupVlanEtherType, prvtTlsSapEventPropagationProfile=prvtTlsSapEventPropagationProfile, sdpLabelSignaling=sdpLabelSignaling, sdpSignalPwStatus=sdpSignalPwStatus, sdpLearnEnable=sdpLearnEnable, sdpDynamicVcIngressLabel=sdpDynamicVcIngressLabel, prvtServicesMIB=prvtServicesMIB, sdpPwActivate=sdpPwActivate, serviceMacAddressSapPriority=serviceMacAddressSapPriority, customerContact=customerContact, serviceEntry=serviceEntry, sdpRowStatus=sdpRowStatus, sapCurrentMacCount=sapCurrentMacCount, sapUntaggedMode=sapUntaggedMode, sdpVCType=sdpVCType, serviceCreated=serviceCreated, sapLearnEnable=sapLearnEnable, serviceMacAddressSdpServiceId=serviceMacAddressSdpServiceId, sdpMtu=sdpMtu, serviceMacAddressSdpTable=serviceMacAddressSdpTable, serviceMacAddressSapEntry=serviceMacAddressSapEntry, serviceTable=serviceTable, serviceMacAddressSdpPriority=serviceMacAddressSdpPriority, serviceVpnId=serviceVpnId, sdpEntry=sdpEntry, sdpOutInterface=sdpOutInterface, customerPhone=customerPhone, serviceLastMgmtChange=serviceLastMgmtChange, serviceMacAddressSdpRowStatus=serviceMacAddressSdpRowStatus, serviceMacAddressSapTable=serviceMacAddressSapTable, customerTable=customerTable, sapDescription=sapDescription, sdpTransportTunnelName=sdpTransportTunnelName, sdpFarEndIpAddress=sdpFarEndIpAddress, sdpAdminIngressLabel=sdpAdminIngressLabel, sdpBindVlanTag=sdpBindVlanTag, sdpOperFlags=sdpOperFlags, serviceOperStatus=serviceOperStatus, sdpCreated=sdpCreated, sapIngressQosSchedulerPolicy=sapIngressQosSchedulerPolicy, sapEgressQosSchedulerPolicy=sapEgressQosSchedulerPolicy, sapOperFlags=sapOperFlags, servicePwRedundOperModeMesh=servicePwRedundOperModeMesh, serviceRowStatus=serviceRowStatus, sapTable=sapTable, customerRowStatus=customerRowStatus, sdpDescription=sdpDescription, sapRowStatus=sapRowStatus, sdpAdminEgressLabel=sdpAdminEgressLabel, sapPortId=sapPortId, sapEntry=sapEntry, sapCustMultSvcSiteName=sapCustMultSvcSiteName, sapIngressQosPolicyId=sapIngressQosPolicyId, serviceCustName=serviceCustName, sapEncapValue=sapEncapValue, serviceNumSaps=serviceNumSaps, sdpLdpEnabled=sdpLdpEnabled, sdpType=sdpType, sapAdminStatus=sapAdminStatus, sapCreated=sapCreated, sapEgressQosPolicyId=sapEgressQosPolicyId, serviceDescription=serviceDescription, serviceMacAddressSapEncapValue=serviceMacAddressSapEncapValue, serviceMacAddressSapRowStatus=serviceMacAddressSapRowStatus, sapLearnMode=sapLearnMode, serviceId=serviceId, servicePwRedundOperModeSpoke=servicePwRedundOperModeSpoke, serviceMacAddressSdpEntry=serviceMacAddressSdpEntry, sdpNextFreeId=sdpNextFreeId, serviceMacAddressSdpId=serviceMacAddressSdpId, sdpPwRedundancyEnable=sdpPwRedundancyEnable, sdpLastMgmtChange=sdpLastMgmtChange, serviceVlanAction=serviceVlanAction, customerCreated=customerCreated, serviceMtu=serviceMtu, sdpId=sdpId, sapOperStatus=sapOperStatus)
