...
mibBuilder.exportSymbols("PRVT-SERV-MIB", customerContact=customerContact, sapProfileName=sapProfileName, serviceMacAddressSapEntry=serviceMacAddressSapEntry, customerDeleted=customerDeleted, sdpCreated=sdpCreated, customerRowStatus=customerRowStatus, serviceVpnId=serviceVpnId, sdpTransportTunnelName=sdpTransportTunnelName, sdpFarEndIpAddress=sdpFarEndIpAddress, serviceVlanEtherType=serviceVlanEtherType, sdpPwActivate=sdpPwActivate, servicePwRedundOperModeMesh=servicePwRedundOperModeMesh, sdpDeleted=sdpDeleted, serviceMacAddressSapServiceId=serviceMacAddressSapServiceId, serviceMacAddressSdpRowStatus=serviceMacAddressSdpRowStatus, sdpDynamicVcEgressLabel=sdpDynamicVcEgressLabel, sdpPwPrecedence=sdpPwPrecedence, serviceMacAddressSdpTable=serviceMacAddressSdpTable, sdpOperFlags=sdpOperFlags, sdpDescription=sdpDescription, serviceMacAddressSapPriority=serviceMacAddressSapPriority, sdpType=sdpType, sapIngressQosSchedulerPolicy=sapIngressQosSchedulerPolicy, customerCreated=customerCreated, sapRowStatus=sapRowStatus, serviceBackupVlanEtherType=serviceBackupVlanEtherType, serviceMacAddressSdpPriority=serviceMacAddressSdpPriority, sapCustMultSvcSiteName=sapCustMultSvcSiteName, prvtServicesMIB=prvtServicesMIB, serviceCreated=serviceCreated, sdpId=sdpId, sapOperStatus=sapOperStatus, serviceDescription=serviceDescription, sapEncapValue=sapEncapValue, sapOperFlags=sapOperFlags, serviceTable=serviceTable, serviceType=serviceType, prvtTlsSapEventPropagationProfile=prvtTlsSapEventPropagationProfile, customerTable=customerTable, sdpGroupIdentifier=sdpGroupIdentifier, sdpLdpEnabled=sdpLdpEnabled, sapLearnEnable=sapLearnEnable, sdpLabelSignaling=sdpLabelSignaling, sapAdminStatus=sapAdminStatus, serviceMacAddressSdpServiceId=serviceMacAddressSdpServiceId, sdpTable=sdpTable, sapEgressQosPolicyId=sapEgressQosPolicyId, sapDeleted=sapDeleted, serviceRowStatus=serviceRowStatus, sdpNextFreeId=sdpNextFreeId, serviceLastMgmtChange=serviceLastMgmtChange, serviceCustName=serviceCustName, sapEntry=sapEntry, serviceNumSdps=serviceNumSdps, sdpAdminEgressLabel=sdpAdminEgressLabel, prvtServicesMIBObjects=prvtServicesMIBObjects, PYSNMP_MODULE_ID=prvtServicesMIB, serviceMacAddressSapEncapValue=serviceMacAddressSapEncapValue, serviceEntry=serviceEntry, customerEntry=customerEntry, sapType=sapType, serviceMacAddressSdpId=serviceMacAddressSdpId, sapCurrentMacCount=sapCurrentMacCount, sdpEntry=sdpEntry, sapDescription=sapDescription, sapMacLearningProfileState=sapMacLearningProfileState, sdpOutInterface=sdpOutInterface, customerName=customerName, sapLastMgmtChange=sapLastMgmtChange, sdpDynamicVcIngressLabel=sdpDynamicVcIngressLabel, sapPortId=sapPortId, serviceNumSaps=serviceNumSaps, sdpBindVlanTag=sdpBindVlanTag, serviceRevertTimer=serviceRevertTimer, serviceMacAddressSapRowStatus=serviceMacAddressSapRowStatus, sdpPwRedundancyEnable=sdpPwRedundancyEnable, sapCreated=sapCreated, serviceDeleted=serviceDeleted, sdpVCType=sdpVCType, sdpAdminStatus=sdpAdminStatus, serviceId=serviceId, sdpMtu=sdpMtu, serviceAdminStatus=serviceAdminStatus, sdpOperStatus=sdpOperStatus, sdpSecuredEnable=sdpSecuredEnable, serviceMacAddressSapTable=serviceMacAddressSapTable, sdpRowStatus=sdpRowStatus, sdpLastMgmtChange=sdpLastMgmtChange, sdpSignalPwStatus=sdpSignalPwStatus, customerPhone=customerPhone, serviceMacAddressSapPortId=serviceMacAddressSapPortId, sapEgressQosSchedulerPolicy=sapEgressQosSchedulerPolicy, servicePwRedundOperModeSpoke=servicePwRedundOperModeSpoke, serviceMacAddressSdpEntry=serviceMacAddressSdpEntry, prvtServicesMIBNotifications=prvtServicesMIBNotifications, serviceMacAddressSdpMacAddress=serviceMacAddressSdpMacAddress, sapUntaggedMode=sapUntaggedMode, serviceEnableSecureSaps=serviceEnableSecureSaps, serviceMtu=serviceMtu, serviceOperStatus=serviceOperStatus, serviceVlanAction=serviceVlanAction, sdpDelivery=sdpDelivery, sapLearnMode=sapLearnMode, prvtSapEventPropagationProfile=prvtSapEventPropagationProfile, sdpAdminIngressLabel=sdpAdminIngressLabel, sapTable=sapTable, serviceMacAddressSapMacAddress=serviceMacAddressSapMacAddress, sdpLearnEnable=sdpLearnEnable, sapIngressQosPolicyId=sapIngressQosPolicyId)
