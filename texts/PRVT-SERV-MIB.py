...
mibBuilder.exportSymbols("PRVT-SERV-MIB", sapCurrentMacCount=sapCurrentMacCount, serviceEntry=serviceEntry, sdpTransportTunnelName=sdpTransportTunnelName, sdpMtu=sdpMtu, sdpBindVlanTag=sdpBindVlanTag, serviceId=serviceId, serviceRevertTimer=serviceRevertTimer, sapIngressQosPolicyId=sapIngressQosPolicyId, customerContact=customerContact, sdpVCType=sdpVCType, sdpLdpEnabled=sdpLdpEnabled, serviceMacAddressSdpTable=serviceMacAddressSdpTable, customerTable=customerTable, serviceMacAddressSapMacAddress=serviceMacAddressSapMacAddress, sapAdminStatus=sapAdminStatus, sapLearnMode=sapLearnMode, sdpTable=sdpTable, sdpAdminIngressLabel=sdpAdminIngressLabel, customerDeleted=customerDeleted, servicePwRedundOperModeMesh=servicePwRedundOperModeMesh, serviceRowStatus=serviceRowStatus, prvtSapEventPropagationProfile=prvtSapEventPropagationProfile, sdpPwRedundancyEnable=sdpPwRedundancyEnable, serviceMtu=serviceMtu, sdpOperStatus=sdpOperStatus, serviceMacAddressSdpServiceId=serviceMacAddressSdpServiceId, sdpOutInterface=sdpOutInterface, sdpSecuredEnable=sdpSecuredEnable, sapRowStatus=sapRowStatus, sdpLearnEnable=sdpLearnEnable, serviceMacAddressSapEntry=serviceMacAddressSapEntry, sdpType=sdpType, sdpSignalPwStatus=sdpSignalPwStatus, sapType=sapType, serviceMacAddressSdpId=serviceMacAddressSdpId, serviceMacAddressSapTable=serviceMacAddressSapTable, sapLastMgmtChange=sapLastMgmtChange, sdpOperFlags=sdpOperFlags, sapIngressQosSchedulerPolicy=sapIngressQosSchedulerPolicy, serviceDeleted=serviceDeleted, sdpDescription=sdpDescription, sdpDynamicVcEgressLabel=sdpDynamicVcEgressLabel, serviceMacAddressSapServiceId=serviceMacAddressSapServiceId, customerPhone=customerPhone, serviceType=serviceType, sapOperStatus=sapOperStatus, sdpRowStatus=sdpRowStatus, sdpLastMgmtChange=sdpLastMgmtChange, serviceMacAddressSapRowStatus=serviceMacAddressSapRowStatus, serviceCreated=serviceCreated, sapTable=sapTable, sdpAdminEgressLabel=sdpAdminEgressLabel, prvtServicesMIBObjects=prvtServicesMIBObjects, sapMacLearningProfileState=sapMacLearningProfileState, sdpFarEndIpAddress=sdpFarEndIpAddress, serviceTable=serviceTable, sapDeleted=sapDeleted, sdpNextFreeId=sdpNextFreeId, prvtServicesMIB=prvtServicesMIB, serviceAdminStatus=serviceAdminStatus, sdpPwActivate=sdpPwActivate, serviceVlanAction=serviceVlanAction, prvtServicesMIBNotifications=prvtServicesMIBNotifications, sdpCreated=sdpCreated, sdpDelivery=sdpDelivery, serviceVlanEtherType=serviceVlanEtherType, serviceMacAddressSapPriority=serviceMacAddressSapPriority, servicePwRedundOperModeSpoke=servicePwRedundOperModeSpoke, sapOperFlags=sapOperFlags, sdpLabelSignaling=sdpLabelSignaling, serviceMacAddressSapEncapValue=serviceMacAddressSapEncapValue, sapCreated=sapCreated, PYSNMP_MODULE_ID=prvtServicesMIB, sdpGroupIdentifier=sdpGroupIdentifier, customerCreated=customerCreated, prvtTlsSapEventPropagationProfile=prvtTlsSapEventPropagationProfile, sdpDeleted=sdpDeleted, sdpId=sdpId, sapLearnEnable=sapLearnEnable, serviceNumSaps=serviceNumSaps, serviceCustName=serviceCustName, sapEntry=sapEntry, serviceEnableSecureSaps=serviceEnableSecureSaps, sdpEntry=sdpEntry, serviceMacAddressSdpRowStatus=serviceMacAddressSdpRowStatus, serviceVpnId=serviceVpnId, serviceBackupVlanEtherType=serviceBackupVlanEtherType, sapEncapValue=sapEncapValue, customerEntry=customerEntry, serviceNumSdps=serviceNumSdps, sapPortId=sapPortId, sapProfileName=sapProfileName, sdpPwPrecedence=sdpPwPrecedence, customerRowStatus=customerRowStatus, customerName=customerName, serviceLastMgmtChange=serviceLastMgmtChange, sapDescription=sapDescription, serviceMacAddressSdpMacAddress=serviceMacAddressSdpMacAddress, serviceDescription=serviceDescription, sdpAdminStatus=sdpAdminStatus, serviceMacAddressSdpPriority=serviceMacAddressSdpPriority, sapUntaggedMode=sapUntaggedMode, sapEgressQosSchedulerPolicy=sapEgressQosSchedulerPolicy, serviceMacAddressSapPortId=serviceMacAddressSapPortId, serviceMacAddressSdpEntry=serviceMacAddressSdpEntry, sdpDynamicVcIngressLabel=sdpDynamicVcIngressLabel, serviceOperStatus=serviceOperStatus, sapCustMultSvcSiteName=sapCustMultSvcSiteName, sapEgressQosPolicyId=sapEgressQosPolicyId)
