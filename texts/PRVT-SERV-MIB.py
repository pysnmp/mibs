...
TruthValue, TimeStamp, RowStatus, TextualConvention, MacAddress, DisplayString = mibBuilder.importSymbols("SNMPv2-TC", "TruthValue", "TimeStamp", "RowStatus", "TextualConvention", "MacAddress", "DisplayString")
...
mibBuilder.exportSymbols("PRVT-SERV-MIB", serviceDescription=serviceDescription, PYSNMP_MODULE_ID=prvtServicesMIB, sdpRowStatus=sdpRowStatus, sdpAdminIngressLabel=sdpAdminIngressLabel, sapMacLearningProfileState=sapMacLearningProfileState, sapCurrentMacCount=sapCurrentMacCount, customerContact=customerContact, sapIngressQosPolicyId=sapIngressQosPolicyId, serviceMacAddressSapPortId=serviceMacAddressSapPortId, sdpSecuredEnable=sdpSecuredEnable, serviceRevertTimer=serviceRevertTimer, sdpId=sdpId, sdpEntry=sdpEntry, sdpAdminStatus=sdpAdminStatus, serviceEnableSecureSaps=serviceEnableSecureSaps, serviceVlanAction=serviceVlanAction, sapEncapValue=sapEncapValue, serviceVlanEtherType=serviceVlanEtherType, customerDeleted=customerDeleted, sapAdminStatus=sapAdminStatus, sdpOperFlags=sdpOperFlags, sdpSignalPwStatus=sdpSignalPwStatus, serviceMacAddressSdpPriority=serviceMacAddressSdpPriority, serviceId=serviceId, sdpPwActivate=sdpPwActivate, servicePwRedundOperModeSpoke=servicePwRedundOperModeSpoke, sdpDynamicVcIngressLabel=sdpDynamicVcIngressLabel, serviceEntry=serviceEntry, serviceMacAddressSapEncapValue=serviceMacAddressSapEncapValue, sapCreated=sapCreated, sapPortId=sapPortId, sdpPwRedundancyEnable=sdpPwRedundancyEnable, serviceMacAddressSapPriority=serviceMacAddressSapPriority, sdpDeleted=sdpDeleted, serviceMacAddressSapRowStatus=serviceMacAddressSapRowStatus, sapIngressQosSchedulerPolicy=sapIngressQosSchedulerPolicy, sapDescription=sapDescription, customerRowStatus=customerRowStatus, serviceNumSaps=serviceNumSaps, serviceTable=serviceTable, sapEntry=sapEntry, sdpLastMgmtChange=sdpLastMgmtChange, serviceBackupVlanEtherType=serviceBackupVlanEtherType, serviceMacAddressSdpId=serviceMacAddressSdpId, customerTable=customerTable, sdpTable=sdpTable, sapEgressQosSchedulerPolicy=sapEgressQosSchedulerPolicy, prvtTlsSapEventPropagationProfile=prvtTlsSapEventPropagationProfile, sdpPwPrecedence=sdpPwPrecedence, sapDeleted=sapDeleted, serviceMtu=serviceMtu, servicePwRedundOperModeMesh=servicePwRedundOperModeMesh, prvtServicesMIBNotifications=prvtServicesMIBNotifications, serviceMacAddressSdpEntry=serviceMacAddressSdpEntry, serviceDeleted=serviceDeleted, customerEntry=customerEntry, sdpOutInterface=sdpOutInterface, sapRowStatus=sapRowStatus, serviceAdminStatus=serviceAdminStatus, sdpLabelSignaling=sdpLabelSignaling, sapEgressQosPolicyId=sapEgressQosPolicyId, sdpAdminEgressLabel=sdpAdminEgressLabel, sapOperStatus=sapOperStatus, prvtServicesMIB=prvtServicesMIB, sdpMtu=sdpMtu, customerName=customerName, serviceNumSdps=serviceNumSdps, sdpDynamicVcEgressLabel=sdpDynamicVcEgressLabel, serviceMacAddressSapEntry=serviceMacAddressSapEntry, sapOperFlags=sapOperFlags, sdpVCType=sdpVCType, customerPhone=customerPhone, serviceMacAddressSapMacAddress=serviceMacAddressSapMacAddress, sdpLdpEnabled=sdpLdpEnabled, sdpDescription=sdpDescription, serviceRowStatus=serviceRowStatus, sdpFarEndIpAddress=sdpFarEndIpAddress, serviceMacAddressSdpRowStatus=serviceMacAddressSdpRowStatus, prvtServicesMIBObjects=prvtServicesMIBObjects, sapType=sapType, serviceMacAddressSdpMacAddress=serviceMacAddressSdpMacAddress, sdpType=sdpType, sapTable=sapTable, customerCreated=customerCreated, serviceMacAddressSapServiceId=serviceMacAddressSapServiceId, serviceMacAddressSdpServiceId=serviceMacAddressSdpServiceId, sdpBindVlanTag=sdpBindVlanTag, serviceCustName=serviceCustName, serviceCreated=serviceCreated, sdpOperStatus=sdpOperStatus, sdpDelivery=sdpDelivery, sapLastMgmtChange=sapLastMgmtChange, serviceOperStatus=serviceOperStatus, sdpGroupIdentifier=sdpGroupIdentifier, sdpNextFreeId=sdpNextFreeId, sapProfileName=sapProfileName, sdpLearnEnable=sdpLearnEnable, sapLearnEnable=sapLearnEnable, sdpCreated=sdpCreated, sapUntaggedMode=sapUntaggedMode, serviceType=serviceType, serviceVpnId=serviceVpnId, sdpTransportTunnelName=sdpTransportTunnelName, sapCustMultSvcSiteName=sapCustMultSvcSiteName, sapLearnMode=sapLearnMode, serviceLastMgmtChange=serviceLastMgmtChange, serviceMacAddressSdpTable=serviceMacAddressSdpTable, serviceMacAddressSapTable=serviceMacAddressSapTable, prvtSapEventPropagationProfile=prvtSapEventPropagationProfile)
