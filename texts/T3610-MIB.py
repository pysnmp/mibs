...
# Produced by pysmi-1.1.8 at Fri Dec  2 15:48:40 2022
...
mibBuilder.exportSymbols("T3610-MIB", temp=temp, tempMax=tempMax, tempAlarm=tempAlarm, compValHighInt=compValHighInt, messageString=messageString, trapTest=trapTest, humUnit=humUnit, trapNTPError=trapNTPError, trapTempClrAlarm=trapTempClrAlarm, tempAlarmInt=tempAlarmInt, values=values, trapCompValClrAlarm=trapCompValClrAlarm, trapCompValHighAlarm=trapCompValHighAlarm, compValAlarmInt=compValAlarmInt, trapHumHighAlarm=trapHumHighAlarm, humMin=humMin, trapSOAPErrDelivery=trapSOAPErrDelivery, valuesInt=valuesInt, trapHumError=trapHumError, traps=traps, trapEmailErrAuth=trapEmailErrAuth, compVal=compVal, tempUnit=tempUnit, tempInt=tempInt, trapSOAPErrFile=trapSOAPErrFile, trapHumLowAlarm=trapHumLowAlarm, settings=settings, trapEmailErrSome=trapEmailErrSome, compValMin=compValMin, humAlarmInt=humAlarmInt, humHystInt=humHystInt, trapCompValLowAlarm=trapCompValLowAlarm, pysmi_global=pysmi_global, trapHumClrAlarm=trapHumClrAlarm, tables=tables, serialNumber=serialNumber, compValAlarm=compValAlarm, tempMin=tempMin, histHum=histHum, historyEntry=historyEntry, trapEmailErrLogin=trapEmailErrLogin, historyTable=historyTable, humHighInt=humHighInt, histCompVal=histCompVal, trapEmailErrDNS=trapEmailErrDNS, trapSOAPErrDNS=trapSOAPErrDNS, DisplayString=DisplayString, trapEmailErrSocket=trapEmailErrSocket, compValLowInt=compValLowInt, trapSOAPErrSocket=trapSOAPErrSocket, trapTempHighAlarm=trapTempHighAlarm, trapCompValError=trapCompValError, products=products, humDelayInt=humDelayInt, humMax=humMax, sensorName=sensorName, compValInt=compValInt, humLowInt=humLowInt, histTemp=histTemp, comet=comet, humAlarm=humAlarm, compValDelayInt=compValDelayInt, trapTempLowAlarm=trapTempLowAlarm, tempLowInt=tempLowInt, compValMax=compValMax, t3610=t3610, tempHighInt=tempHighInt, tempHystInt=tempHystInt, compValHystInt=compValHystInt, tempDelayInt=tempDelayInt, hum=hum, deviceType=deviceType, compValUnit=compValUnit, humInt=humInt, trapTempError=trapTempError)
