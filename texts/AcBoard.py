...
# ASN.1 source AcBoard
# Source digest sha256:dc393f2bc25d04b043c11f003b64b57faaff9cbd3851f92e055e7619fa6c6535
...
if mibBuilder.loadTexts: acBoard.setLastUpdated('2012-08-13 11:37')
if mibBuilder.loadTexts: acBoard.setOrganization('AudioCodes Ltd')
if mibBuilder.loadTexts: acBoard.setContactInfo('Postal: Support\n\t     AudioCodes LTD\n             1 Hayarden Street\n             Airport City \n\t     Lod, ISRAEL 70151\n     Tel:    972-3-9764000\n     Fax:    972-3-9764040\n     Email:  support@audiocodes.com\n     Web:    www.audiocodes.com')
if mibBuilder.loadTexts: acBoard.setDescription("AudioCodes Proprietary MIB \n     Contains The managed objects of AudioCodes systems.\n     Through this MIB, the User can set up the board configuration parameters, reset the board, \n\t and monitor the board's operational robustness and quality of service during run-time. \n\n    This MIB has the following Groups: \n\t    boardConfiguration     \n\t    boardInformation   \n\t    channelConfiguration \n\t    channelStatus \n\t    notifications    \n\t    reset\n\n    boardConfiguration:     \n        When The user Get Parameter from most of sub-groups in boardConfiguration group, \n\t    he gets the value that should be burned to the board after the next Remote Reset. \n\t    He can Set new values to many parameters, and these values will burned to the board after the next remote Reset \n        In the sub-group mGCPDefaultChannelSettings the parameters are the actual parameters, \n\t    that will take place when opening channel. (Reset is not needed for that sub-group).\n\n    BoardInformation: \n        Contains Read-only parameters like: board name, serial number, number of channels, software version.\n\n    ChannelConfiguration:\n        Read-only Parameters to see the configuratin of each channel: voice settings, fax settings, etc.         \n\n    ChannelStatus:\n        Read-only parameters about the status of channel, like: Is the channle active, is it in Silence Period, \n\t\tis it in Fax mode, ON/Off hook, etc. \n        This group contains also counters: packets to DSP, errors, lost packets, underruns, etc.\n\n    Notifications:\n        In this version, the notification we have are: acEvBoardStarted, and acEvResetingBoard. \n        The board should send these notifications to its manager \n\t\tthat the user defined in INI file with the parameter: NMPManagerIP = (IP address). \n\n    Reset:\n        To remote reset the board, the user should set an integer value to the parameter remoteReset.\n\t\tThen he should get the trap acEvResettingBoard. \n        Note: Before Reset, The current boardConfiguration parameters will be burned  to the board. \n\t\t      The user should not use INI file, if he wants these parameters to take place.\n\n\t\tTo set the default values, the user should set an integer value to the parameter acSetDefaults.\n\t\t")
...
if mibBuilder.loadTexts: tDMBusLocalReference.setDescription('When ClockSource is set to acH110BusClockSource_Network this parameter selects the TrunkID to be used as the clock synchronization source of the board.\nWhen using H.110/H.100 bus this parameter also selects the trunk which is used as the clock source for the NETREF clock generation (in this case ClockSource might not be set to acH110BusClockSource_Network).')
...
if mibBuilder.loadTexts: tDMBusSpeed.setDescription('Selects the TDM Bus Speed. Speed is set according to the Bus Type. The following are allowed speeds for each bus type: SC = 0/2/3\nH.110/H.100 = 3\nMVIP = 0')
...
if mibBuilder.loadTexts: acOverrideDefaultIdlePCMPattern.setDescription('Override Default Idle PCM Pattern.\n  0 - use defaults (0xff-Mulaw, 0x55-Alaw)\n  1 - use IdlePCMPattern.')
...
if mibBuilder.loadTexts: acOverrideDefaultIdleABCDPattern.setDescription('Description: Override Default Idle ABCD Pattern.\n  0 - use defaults (0xf)\n  1 - use IdleABCDPattern.')
...
if mibBuilder.loadTexts: defaultGatewayAddr.setDescription("Default Gateway Address.\n Board's Default Gateway IP address. \n (in host byte order e.g. 192.10.1.255 = 0xc00a01ff).")
...
if mibBuilder.loadTexts: boardIPAddr.setDescription('Board IP Address. \n in host byte order e.g. 192.10.1.2 = 0xc00a0102). \n  Users should set the subnet mask to a valid value per the IP address.')
...
if mibBuilder.loadTexts: boardSubNetAddr.setDescription('Board SubNet Address.\n  (in host byte order e.g. 255.255.255.0 = 0xffffff00). \n  default =255.255.255.0.')
...
if mibBuilder.loadTexts: acMGControlProtocolType.setDescription('This parameter defines the control protocol type\n 0 = none, 1 = MGCP, 2 = MEGACO, 4 = H323, 8 = SIP')
...
snmpManagersTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 2, 21, 1), ).setMaxAccess("notaccessible")
...
snmpManagersEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 2, 21, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "snmpManagerIndex"))
...
acWEBAccessTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 2, 23, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: acWEBAccessTable.setStatus('obsolete')
if mibBuilder.loadTexts: acWEBAccessTable.setDescription('')
acWEBAccessEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 2, 23, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "acWEBAccessIndex"))
...
trunkSettingsTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 4, 1), ).setMaxAccess("notaccessible")
...
trunkSettingsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 4, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "trunkId"))
...
if mibBuilder.loadTexts: protocolType.setDescription("Protocol Type.  Sets the PSTN protocol to be used for this trunk. \nThe protocol type parameter defines the interface \n  and the specific telephony protocol. \n Each protocol defines the following:\n              1. E1 or T1 physical line configuration.\n              2. Fixed connection of the relevant time-slots, \n              which are the Trunks' B-channels, mapping to the 60 \n        (128 for TrunkPack®-VoIP/400) DSP channels. \n        This limitation is about to change in versions higher than 3.4.\n              3. Layer 3 specific telephony protocol or transparent mode of operation.\nCurrently only identical configurations (protocol type, framing method, etc.) \n  for the two trunks are supported.(see at the TrunkPack®-VoIP User Manual \n  for available protocols families)  relevant only when TDMBusType=FRAMERS. \n")
...
if mibBuilder.loadTexts: lineBuildOutXPM1.setDescription("Controls the Framer's XPM1 register value (line pulse shape control).\nApplicable only when TrunkConfig.LineBuildOut.Overwrite=1.\nShould be used only by expert Users.")
...
if mibBuilder.loadTexts: acIsdnNfasInterfaceId.setDescription("Defines the Interface ID\nworks with NS_EXPLICIT_INTERFACE_ID bit. Refer to the Appendix A.8 'ISDN Flexible Behavior' in the VoPLib Reference Library User's Manual Document #: LTRT-00740. ")
...
if mibBuilder.loadTexts: callAgentIP.setDescription('MGCP Call Agent IP address to be used for the initial RestartInProgress MGCP message (RSIP)\n(e.g., 192.10.1.255). Set to 0.0.0.0 to avoid sending RSIP. This parameter if in use overrides the Call Agent IP address provided in BootP.')
...
if mibBuilder.loadTexts: acChDefaultFaxRelayRedundancyDepth.setDescription(' Default: Fax Relay Redundancy Depth. \n  Number of repetitions to be applied to each fax relay payload \n  when transmitting to network.')
...
if mibBuilder.loadTexts: acChDefaultT38ProtectionMode.setDescription(' Default: T38 Protection Mode. \n  Use Forward Error Correction (FEC) algorithm \n  to protect T.38 fax relay stream.')
...
if mibBuilder.loadTexts: acChDefaultDTMFTransportType.setDescription('0 = Mute = erases digit from voice stream does not relay to remote.\n2 = Transparent = digits remains in voice stream.\n3 = RFC 2833 relay = erase digit from voice stream relay to remote using RFC 2833 syntax. \nThe other values as seen in the Value List (1 and 7) are not supported as of  version 4.4.')
...
if mibBuilder.loadTexts: acChDefaultMFTransportType.setDescription('0 = erase MFs from voice transport not relayed to remote.\n2 = MFs not erased not relayed to remote.\n3 = MFs are muted from the voice stream & relayed according to RFC 2833.')
...
if mibBuilder.loadTexts: acChDefaultTestMode.setDescription('Testing mode applied.\n0 = Coder Loopback performs an encoder/decoder loopback inside DSP device.\n1 = PCMLoopback loops back incoming PCM to the outgoing PCM.\n2 = ToneInjection generates a 1000 Hz tone to outgoing PCM.\n3 = NoLoopback sets the channel to work in normal mode. ')
...
if mibBuilder.loadTexts: acChDefaultVoiceVolume.setDescription('Voice  output gain control\nthe range is -31 dB to +31 dB in 1 dB steps\n-32 = mute. ')
...
acSS7SettingsTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 8, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: acSS7SettingsTable.setStatus('obsolete')
if mibBuilder.loadTexts: acSS7SettingsTable.setDescription('')
acSS7SettingsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 8, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "acSS7LinkId"))
...
acCasTablesTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 11, 21, 1), ).setMaxAccess("notaccessible")
if mibBuilder.loadTexts: acCasTablesTable.setStatus('obsolete')
if mibBuilder.loadTexts: acCasTablesTable.setDescription('')
acCasTablesEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 11, 21, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "acCasTableIndex"))
...
if mibBuilder.loadTexts: megacoGatewayName.setDescription("MGCP:Gateway's identification name towards the MGCP Call Agent. If empty Gateway name holds the IP address of the board\nMEGACO: Prefix of the gateway part of the termination name")
...
if mibBuilder.loadTexts: megacoTrunkName.setDescription("MGCP: Together with 'EndpointPrefix' field generates local endpoint name on trunk enabled gateways\nMEGACO: Prefix of the trunk part of the termination name")
...
megacoCallAgentsTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 13, 7, 1), ).setMaxAccess("notaccessible")
...
megacoCallAgentsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 1, 13, 7, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "megacoCallAgentId"))
...
if mibBuilder.loadTexts: acIsdnDuplicateQ931BuffMode.setDescription('Activates / de-activates delivery of raw Q.931 messages.\nThis Object is obsolete and is replaces by acTrunkISDNCommonDuplicateQ931BuffMode in the acPSTN-MIB.')
...
if mibBuilder.loadTexts: amsNumOfConferencePorts.setDescription('Number Of Conference ports.\xa0 How many ports on the TPM are allocated to the conferencing service.')
...
if mibBuilder.loadTexts: amsNumOfTestTrunkPorts.setDescription('Number Of Test Trunk ports.\xa0 How many ports on the TPM are allocated to the Test Trunk service.')
...
if mibBuilder.loadTexts: amsNumOfLawfulInterceptPorts.setDescription('Number Of Test Trunk ports.\xa0 How many ports on the TPM are allocated to the Lawful Intercept service.')
...
if mibBuilder.loadTexts: amsNumOfAnnouncementPorts.setDescription('Number Of Test Trunk ports.\xa0 How many ports on the TPM are allocated to the Announcement service.')
...
if mibBuilder.loadTexts: amsProfile.setDescription("AMS Profile Bit Mask: \n1'st bit - APS Audio Enable/Disable\n2'nd bit - Trunks Interface On/Off\n3'td bit - Board Operation State change on ATM Port failure Enable/Disable.")
...
if mibBuilder.loadTexts: acDspVersionTemplate.setDescription('selects the DSP load number, which defines the supported set of coders. \nDefault=0\n\nEach template assigns a specific version to each DSP. \nEach DSP version supports a set of coders and features. \n\nThe number of channels supported by the DSP can also vary between DSP versions.\n The total number of channels supported by the board depends on the selected template. ')
...
voiceSettingsTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 2, 1), ).setMaxAccess("notaccessible")
...
voiceSettingsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 2, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "cID"))
...
if mibBuilder.loadTexts: testMode.setDescription('Testing mode to be applied. \n Valid Range:CoderLoopback will perform an encoder-decoder loopback inside the DSP device, \n  PCMLoopback will loopback the incoming PCM to the outgoing PCM, \n  ToneInjection will generate a1000 Hz tone to outgoing PCM and \n  NoLoopback will set the channel to work in a normal mode. \n  Refer to TTestCode enumeration \n  (default =3= NoLoopBack).\n')
...
if mibBuilder.loadTexts: voiceVolume.setDescription('Voice Volume sets the voice decoder’s output gain\n  Range: 0-63 corresponding to -31 dB to +31 dB in 1 dB steps')
...
if mibBuilder.loadTexts: inputGain.setDescription('PCM input gain sets the gain at the encoder’s input.')
...
if mibBuilder.loadTexts: m.setDescription('This Packet Multiply Factor is the Number of 20 msec payloads \n  (30 msec for G.723) \n  to be used for generating one RTP packet. \n  Range: 1 - Until max Tx/Rx buffer size is reached')
...
if mibBuilder.loadTexts: rTPRedundancyDepth.setDescription('RTP Redundancy Depth.\n  Number of repetitions to be applied to each RTP payload \n  when transmitting to network.\n   apply redundancy sequence per RFC2198. \n  Valid range:  Disable redundancy packets generation = 0.\n  Enable generation of RFC2198 redundancy packets = 1.')
...
if mibBuilder.loadTexts: eCLength.setDescription('Length of echo canceler tail to be used\n  according to TECLength enumeration. \n  Note that EC length has different length limitations for different boards.')
...
if mibBuilder.loadTexts: eCHybridLoss.setDescription('Echo Cancler Hybrid Loss.\n  sets the worst case ratio between the signal level transmitted to the hybrid\n  and the echo level returning from the hybrid.\n  User should set this value according to the worst hybrid\n  in the system in terms of echo return loss\n  according to enum TECHybridLoss enumeration')
faxModemSettings = MibIdentifier((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 3))
faxModemSettingsTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 3, 1), ).setMaxAccess("notaccessible")
...
faxModemSettingsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 3, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "cID"))
...
if mibBuilder.loadTexts: faxRelayMaxRate.setDescription('Fax Relay Max Rate\nLimits the maximum rate at which fax messages are transmitted. \nRefer to acTFaxModemRelayRate enumeration. \n (Default =14400 bps).\n')
...
if mibBuilder.loadTexts: modemRelayMaxRate.setDescription('Modem Relay Max Rate\nLimits the maximum rate at which modem relay messages are transmitted. \n  Refer to acTFaxModemRelayRate enumeration. \n  (Default =14400 bps).')
...
if mibBuilder.loadTexts: faxRelayECMEnable.setDescription('Fax Relay ECM Enable\n  Enable using ECM mode during Fax Relay.')
...
if mibBuilder.loadTexts: t38FaxRelayProtectionMode.setDescription('T38 Fax Relay Protection Mode\n  Use Forward Error Correction (FEC) algorithm \n  to protect T.38 fax relay stream.')
...
if mibBuilder.loadTexts: faxRelayRedundancyDepth.setDescription('Fax Relay Redundancy Depth\nNumber of repetitions to be applied to each fax relay payload \n  when transmitting to network (applicable only when acTFaxModemCmdacTFaxModemSettings.T38FaxRelayProtectionMode=0).\n')
...
if mibBuilder.loadTexts: enhancedFaxRelayRedundancyDepth.setDescription('Enhanced Fax Relay Redundancy Depth\n Number of repetitions to be applied to control packets when using T.38 standard.')
...
if mibBuilder.loadTexts: modemRelayRedundancyDepth.setDescription('Modem Relay Redundancy Depth\nNumber of repetitions to be applied to each modem relay payload \nwhen transmitting to network.')
...
if mibBuilder.loadTexts: faxModemRelayVolume.setDescription('Fax Modem Relay Volume\n Fax gain control. \n  Range: -18 - 15 corresponding to -18.5 dBm to -3.5 dBm in 1 dB steps. \n  The reason the range extends to 15 is for backward compatibility.\n  (Default =6).')
...
if mibBuilder.loadTexts: faxModemBypassCoderType.setDescription('Fax Modem Bypass Coder Type\n  Coder to be used while performing fax/modem bypass. \n  Usually, high bit rate coders such as G.711 and G.726/G.727 should be used.\n  (Default = G.711 A-law).')
...
if mibBuilder.loadTexts: faxModemBypassM.setDescription('Fax Modem Bypass M\n  Number of 20 msec payloads to be used for generating one RTP fax/modem bypass packet.\n  Range: 1 - Until max Tx/Rx buffer size is reached')
...
if mibBuilder.loadTexts: useT38orFRF11.setDescription('Use T38 or FRF11\n  Use T.38 protocol or proprietary FRF.11 syntax, \n  to send/receive fax relay.')
dJBSettings = MibIdentifier((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 4))
dJBSettingsTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 4, 1), ).setMaxAccess("notaccessible")
...
dJBSettingsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 4, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "cID"))
...
if mibBuilder.loadTexts: dJBufMinDelay.setDescription('[Millisec] Dynamic Jitter Buffer Minimum Delay.\n  0 - 150  (default =150)')
...
if mibBuilder.loadTexts: dJBufOptFactor.setDescription('Dynamic Jitter Buffer Optimization Factor.\nDynamic jitter buffer frame error/delay optimization.\n0 - 12  (default =7)')
channelTDMBusSettings = MibIdentifier((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 5))
channelTDMBusSettingsTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 5, 1), ).setMaxAccess("notaccessible")
...
channelTDMBusSettingsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 5, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "cID"))
...
if mibBuilder.loadTexts: tDMBusInputPort.setDescription('TDM Bus Input Port.  \n  Together with TDMBusInputChannel, \n  defines the TDM bus timeslot to be used as input for this channel. \n  Note that the valid region for this parameter varies according to the type of TDM bus used.\n  When using TDMBusType=acFramers this parameter selects the trunkID, \n  in which the incoming B-channel resides. 0-127. ')
...
if mibBuilder.loadTexts: tDMBusInputChannel.setDescription('TDM Bus Input Channel. \n  Together with TDMBusInputPort, \n  defines the TDM bus timeslot to be used as input for this channel. \n Note that the valid region for this parameter varies according to the TDMBusSpeed. \n  When using TDMBusType=acFramers, this parameter selects the incoming B-channel \n  (in this case the valid region is 0-31).')
...
if mibBuilder.loadTexts: tDMBusOutputDisable.setDescription('TDM Bus Output Disable. \n 0: Places TDM bus output timeslot in tri-state mode. \n     Ignored in case of PSTN interface.\n 1: Enables TDM bus output timeslot. \n     Ignored in case of PSTN interface.')
...
if mibBuilder.loadTexts: tDMBusOutputPort.setDescription('TDM Bus Output Port. \nTogether with TDMBusOutputChannel, defines the TDM bus timeslot \n  to be used as output for this channel. \n Note that the valid region for this parameter varies according to the type of TDM bus used. \n  When this parameter is set to 0xff=255 the boardwise setting (set in acOpenBoard()) will be used. \n  When using TDMBusType=acFramers, this parameter is ignored.\n  0-31 or 255.')
...
if mibBuilder.loadTexts: tDMBusOutputChannel.setDescription('TDM Bus Output Channel. \n Together with TDMBusOutputPort, defines the TDM bus timeslot \n  to be used as output for this channel. \n Note that the valid region for this parameter varies according to the TDMBusSpeed. \n When using TDMBusType=acFramers, this parameter is ignored.')
transportSettings = MibIdentifier((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 6))
transportSettingsTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 6, 1), ).setMaxAccess("notaccessible")
...
transportSettingsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 6, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "cID"))
...
if mibBuilder.loadTexts: useNIorPCI.setDescription('Description: Use NI or PCI interface. \n  =1 if channel will use NI interface, =0 if PCI interface.\n  0- PCI interface: Compressed traffic will be sent/received through the PCI interface.\n 1- NI interface: RTP-RTCP packets sent/received to/from on board Ethernet interface.\n(default=1).')
...
if mibBuilder.loadTexts: disableSoftIPLoopback.setDescription('Disable Soft IP Loopback\n  =1 in order to diable soft IP loopback. \n  0- Enables the hair pin connection made in case two channels on the same board are connected together.\n  1- Disables the hair pin connection made in case two channels on the same board are connected together.')
...
if mibBuilder.loadTexts: uniDirectionalRTP.setDescription('uni Directional RTP.\n  enables one sided RTP stream - according to niDirectionalRTPMode.\n  enables the User to set the channel to work with unidirectional RTP streams. \n  When UniDirectionalRTP is set to acRTPTxOnly, \n  the receiver part of the channel will be disabled, and only the transmitter (encoder) will work. \n  When UniDirectionalRTP is set to acRTPRxOnly, \n  the transmitter part of the channel will be disabled, and only the receiver (decoder) will work. \n  When UniDirectionalRTP is set to acRTPTxRx, both sides will be enabled.\n  Users should note that when configuring a session between two endpoints, \n  some of the channel configuration parameters must be identical on both sides. \n  Such parameters are the voice coder type, the Fax/Modem transport type and bypass coders. \n  All other configuration parameters are independent.')
rTPRTCPSettings = MibIdentifier((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 7))
rTPRTCPSettingsTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 7, 1), ).setMaxAccess("notaccessible")
...
rTPRTCPSettingsEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 3, 7, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "cID"))
...
if mibBuilder.loadTexts: cNAME.setDescription("RTP Canonical Name. \n  Should be unique among all participants within one RTP session. \n  Refer to RFC1890 for more information.\n  Up to 255 characters string. \n  (default ='ChX',  X=CID).")
...
if mibBuilder.loadTexts: iPPrecedence.setDescription('IP Precedence.  \n  Sets the value of the IP precedence field in the IP header \n  for all the packets to be generated from this channel. \n  Follow the acTIPPrecedence enumeration.\n  Valid Range: 0-7  (default =0)\n')
...
if mibBuilder.loadTexts: iPTOS.setDescription('iP Type Of Service. \n  sets the value of the IP Type Of Service field in the IP header \n  for all the packets to be generated from this channel. \n  acNormalServiceTOS=0,\n  acMinimizeMonetaryCostTOS=1,\n  acMaximizeReliabilityTOS=2,\n  acMaximizeThroughputTOS=4,\n  acMinimizeDelayTOS=8\n   (Follow the acTIPTOS enumeration). ')
...
if mibBuilder.loadTexts: localRTPPort.setDescription('Local RTP Port.\n Local UDP Port to be used for incoming RTP stream to this channel. \n  This parameter also sets the local RTCP and local T.38 ports \n  in offset of 1,2 from LocalRTPPort respectively. \n  The local RTP port must be a number dividable by 10 \n  and must reside within the following range \n  BoardParams.BaseUDPPort-> BoardParams.BaseUDPPort +10*MAX_CID_NUM. ')
...
if mibBuilder.loadTexts: rTCPMeanTxInterval.setDescription("RTCP Mean Tx Interval.\n  Mean interval [msec] between channel's RTCP reports. ")
...
if mibBuilder.loadTexts: rxRTPPayloadType.setDescription('Rx RTP Payload Type\n  -1 = use the default coder RTP payload type for incoming voice packets. \n  -2 = allow any RTP payload type for incoming packets. \n  Otherwise, = sets the RTP payload type to be used for incoming voice packets \n  (this will override the default RTP payload type defined for each coder). \n   Valid Range: -1/-2 or any unoccupied RTPPT in the range 0-127 . \n  (default =-1). ')
...
if mibBuilder.loadTexts: txRTPPayloadType.setDescription('Tx RTP Payload Type\n  -1 = use the default coder RTP payload type for incoming voice packets. \n  -2 = allow any RTP payload type for incoming packets. \n  Otherwise, = sets the RTP payload type to be used for incoming voice packets \n  (this will override the default RTP payload type defined for each coder). \n   Valid Range: -1/-2 or any unoccupied RTPPT in the range 0-127. \n  (default =-1). ')
channelStatus = MibIdentifier((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 4))
channelStatusTable = MibTable((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 4, 1), ).setMaxAccess("notaccessible")
...
channelStatusEntry = MibTableRow((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 4, 1, 1), ).setMaxAccess("notaccessible").setIndexNames((0, "AcBoard", "cID"))
...
if mibBuilder.loadTexts: remoteReset.setDescription('remote Reset. \n  To remote reset the board, set an integer value to this parameter. \n  Note: Before Reset, The current boardConfiguration parameters will be burned to the board.  ')
...
if mibBuilder.loadTexts: acgwAdminState.setDescription("Determines the gateway's operational state.  \n0 - locked: Shutdown the GW In the time frame set by acgwAdminStateLockControl\n1 - shuttingDown: (read only) Graceful Shutdown is being carried out  \n2 - unlocked: GW is in service.  ")
...
if mibBuilder.loadTexts: acgwAdminStateLockControl.setDescription('(in sec) time left for the graceful shutdown to complete.\n  -1 indicates unlimited time frame. \n  0 indicates immediate (forced) lock. \nImplementing the control requires setting acgwAdminState to locked.  ')
...
if mibBuilder.loadTexts: acSaveConfigToSystem.setDescription('This will save all current configuration in the board onto the flash. \nAny digit will call on the function. \nNOTE: Do NOT save the configuration while voice is running - this will cause a delay.')
...
if mibBuilder.loadTexts: remoteResetControl.setDescription('Determine the remote reset action\n1 - burn staged load image, if any, and config to flash, perform a reset, and load from flash\n2 - perform a reset, and load from flash (staged load image, if any, and config is not burned to flash)\n3 - perform a reset, and load from bootp/tftp (staged load image, if any, and config burn is not needed)\n* - an error is returned for any other value\nNote: The staged load image is an image of a cmp file that was previously loaded to a area in RAM via an HTTP upload\n\n')
...
if mibBuilder.loadTexts: acEnhancedBITStatus.setDescription('This trap is used to for the status of the BIT (Built In Test).\nThe information in the trap contains board HW elements being tested and their status.\nThe information is presented in the additional info fields.')
...
if mibBuilder.loadTexts: acFanTrayAlarm.setDescription('This alarm is raised when the fan tray is missing or some of the fans are faulty.\nIf the fan tray is missing or more then X fans are faulty the severity will be Critical.\nIf more then one and less the X fans are faulty the severity will be Major.\n')
...
if mibBuilder.loadTexts: acSAMissingAlarm.setDescription('This alarm is raised when the SA module in the M3K MG is missing. Whenthere is no SAT there are no other chassis HW related alarms.\nNote - relevant only for M3K.')
...
if mibBuilder.loadTexts: acHASystemFaultAlarm.setDescription('This alarm is raised when the HA (High Availability) feature is active and the system is NOT HA.\nCritical severity - \n1.Wrong HW Configuration.\n2. Redundant module is missing (not in slot).\n3. The redundant module is non operable.\nMinor severity -  waiting for redunadant to connect (after reset!). ')
...
if mibBuilder.loadTexts: acDChannelStatus.setDescription('This non alarm trap is sent when establishment, re-establishment or release of LAPD link with its peer connection occurs.\nThe trap will be sent out with one of the following in the textual description:\nD-channel synchronized\nD-channel not-synchronized')
...
if mibBuilder.loadTexts: acHitlessUpdateStatus.setDescription('Notification trap that is sent out at the beginning and the end of a Hitless SW update.\nFailure during the process will also instigate the trap.\nRelevant to HA systems only.')
...
acBoardWanLinkAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 79)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acBoardWanLinkAlarm.setStatus('current')
if mibBuilder.loadTexts: acBoardWanLinkAlarm.setDescription('Wan Link is down.')
acPowerOverEthernetStatus = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 80)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acPowerOverEthernetStatus.setStatus('current')
if mibBuilder.loadTexts: acPowerOverEthernetStatus.setDescription('Raised when PEO resources are over allocated')
acMediaProcessOverloadAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 81)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acMediaProcessOverloadAlarm.setStatus('current')
if mibBuilder.loadTexts: acMediaProcessOverloadAlarm.setDescription('Raised when media process overloads')
acWirelessCellularModemAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 82)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acWirelessCellularModemAlarm.setStatus('current')
if mibBuilder.loadTexts: acWirelessCellularModemAlarm.setDescription('Raised when either the wireless modem is down or in backup mode.')
acDataInterfaceStatus = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 83)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acDataInterfaceStatus.setStatus('current')
if mibBuilder.loadTexts: acDataInterfaceStatus.setDescription('Raised when a DSL interface state changes to up or down.')
acNFASGroupAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 84)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acNFASGroupAlarm.setStatus('current')
if mibBuilder.loadTexts: acNFASGroupAlarm.setDescription('Raised when an NFAS group goes Out-Of-Service')
acBChannelAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 85)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acBChannelAlarm.setStatus('current')
if mibBuilder.loadTexts: acBChannelAlarm.setDescription('Raised when B-Channel service state changes')
acEthernetGroupAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 86)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acEthernetGroupAlarm.setStatus('current')
if mibBuilder.loadTexts: acEthernetGroupAlarm.setDescription('Raised when the Ethernet group has no Ethernet port with its link up. Cleared when at least on port has established a link.')
acMediaRealmBWThresholdAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 87)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acMediaRealmBWThresholdAlarm.setStatus('current')
if mibBuilder.loadTexts: acMediaRealmBWThresholdAlarm.setDescription('Raised when a BW threshold is crossed')
acNqmConnectivityAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 88)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acNqmConnectivityAlarm.setStatus('current')
if mibBuilder.loadTexts: acNqmConnectivityAlarm.setDescription('Connectivity with NQM probe destination is lost')
acNqmRttAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 89)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acNqmRttAlarm.setStatus('current')
if mibBuilder.loadTexts: acNqmRttAlarm.setDescription('Detected high Round Trip Time towards NQM probe destination')
acNqmJitterAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 90)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acNqmJitterAlarm.setStatus('current')
if mibBuilder.loadTexts: acNqmJitterAlarm.setDescription('Detected high Jitter towards NQM probe destination')
acNqmPacketLossAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 91)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acNqmPacketLossAlarm.setStatus('current')
if mibBuilder.loadTexts: acNqmPacketLossAlarm.setDescription('Detected high Packet Loss towards NQM probe destination')
acCertificateExpiryNotifiaction = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 92)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acCertificateExpiryNotifiaction.setStatus('current')
if mibBuilder.loadTexts: acCertificateExpiryNotifiaction.setDescription('Send before the expiration of the installed credentials, which cannot be renewed automatically.')
acWEBUserAccessDisabled = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 93)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acWEBUserAccessDisabled.setStatus('current')
if mibBuilder.loadTexts: acWEBUserAccessDisabled.setDescription('Web user was disabled due to inactivity.')
acProxyConnectionLost = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 94)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acProxyConnectionLost.setStatus('current')
if mibBuilder.loadTexts: acProxyConnectionLost.setDescription('Raised when all connections in the proxy set are down.')
acNqmCqMosAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 95)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acNqmCqMosAlarm.setStatus('current')
if mibBuilder.loadTexts: acNqmCqMosAlarm.setDescription('Detected low conversational voice quality towards NQM probe destination')
acNqmLqMosAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 96)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acNqmLqMosAlarm.setStatus('current')
if mibBuilder.loadTexts: acNqmLqMosAlarm.setDescription('Detected low listening voice quality towards NQM probe destination')
acRedundantBoardAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 97)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acRedundantBoardAlarm.setStatus('current')
if mibBuilder.loadTexts: acRedundantBoardAlarm.setDescription('Active board sends notification when an alarm or notification is raised in the redundant board.\nThe information is as follow:\nacBoardTrapGlobalsAdditionalInfo1: redundant’s original info\nacBoardTrapGlobalsAdditionalInfo2: trap Id\nacBoardTrapGlobalsAdditionalInfo3: redundant’s serial number\n')
acHANetworkWatchdogStatusAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 98)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acHANetworkWatchdogStatusAlarm.setStatus('current')
if mibBuilder.loadTexts: acHANetworkWatchdogStatusAlarm.setDescription('Sent when the module was enabled by the user but it is not running.')
acIDSPolicyAlarm = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 99)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acIDSPolicyAlarm.setStatus('current')
if mibBuilder.loadTexts: acIDSPolicyAlarm.setDescription('IDS Policy Alarm')
acIDSThresholdCrossNotification = NotificationType((1, 3, 6, 1, 4, 1, 5003, 9, 10, 1, 21, 2, 0, 100)).setObjects(("AcBoard", "acBoardTrapGlobalsName"), ("AcBoard", "acBoardTrapGlobalsTextualDescription"), ("AcBoard", "acBoardTrapGlobalsSource"), ("AcBoard", "acBoardTrapGlobalsSeverity"), ("AcBoard", "acBoardTrapGlobalsUniqID"), ("AcBoard", "acBoardTrapGlobalsType"), ("AcBoard", "acBoardTrapGlobalsProbableCause"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo1"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo2"), ("AcBoard", "acBoardTrapGlobalsAdditionalInfo3"), ("AcBoard", "acBoardTrapGlobalsDateAndTime"))
if mibBuilder.loadTexts: acIDSThresholdCrossNotification.setStatus('current')
if mibBuilder.loadTexts: acIDSThresholdCrossNotification.setDescription('Sent when IDS Threshold is crossed')
mibBuilder.exportSymbols("AcBoard", PYSNMP_MODULE_ID=acBoard, aMsConfiguration=aMsConfiguration, aTMLoopBack=aTMLoopBack, aTMPHYType=aTMPHYType, aTMSettings=aTMSettings, acAMSProcedureResult=acAMSProcedureResult, acActiveAlarmTableOverflow=acActiveAlarmTableOverflow, acActiveFeaturesList=acActiveFeaturesList, acAnalogPortGroundFaultOutOfService=acAnalogPortGroundFaultOutOfService, acAnalogPortHighTemperature=acAnalogPortHighTemperature, acAnalogPortSPIOutOfService=acAnalogPortSPIOutOfService, acAtmPortAlarm=acAtmPortAlarm, acAudioProvisioningAlarm=acAudioProvisioningAlarm, acBChannelAlarm=acBChannelAlarm, acBoard=acBoard, acBoardCallResourcesAlarm=acBoardCallResourcesAlarm, acBoardConfigurationError=acBoardConfigurationError, acBoardControllerFailureAlarm=acBoardControllerFailureAlarm, acBoardEthernetLinkAlarm=acBoardEthernetLinkAlarm, acBoardEvBoardStarted=acBoardEvBoardStarted, acBoardEvResettingBoard=acBoardEvResettingBoard, acBoardFatalError=acBoardFatalError, acBoardFxsOrFxo=acBoardFxsOrFxo, acBoardMibs=acBoardMibs, acBoardOverloadAlarm=acBoardOverloadAlarm, acBoardTemperature=acBoardTemperature, acBoardTemperatureAlarm=acBoardTemperatureAlarm, acBoardTrapDefinitions=acBoardTrapDefinitions, acBoardTrapGlobals=acBoardTrapGlobals, acBoardTrapGlobalsAdditionalInfo1=acBoardTrapGlobalsAdditionalInfo1, acBoardTrapGlobalsAdditionalInfo2=acBoardTrapGlobalsAdditionalInfo2, acBoardTrapGlobalsAdditionalInfo3=acBoardTrapGlobalsAdditionalInfo3, acBoardTrapGlobalsDateAndTime=acBoardTrapGlobalsDateAndTime, acBoardTrapGlobalsName=acBoardTrapGlobalsName, acBoardTrapGlobalsProbableCause=acBoardTrapGlobalsProbableCause, acBoardTrapGlobalsSeverity=acBoardTrapGlobalsSeverity, acBoardTrapGlobalsSource=acBoardTrapGlobalsSource, acBoardTrapGlobalsTextualDescription=acBoardTrapGlobalsTextualDescription, acBoardTrapGlobalsType=acBoardTrapGlobalsType, acBoardTrapGlobalsUniqID=acBoardTrapGlobalsUniqID, acBoardWanLinkAlarm=acBoardWanLinkAlarm, acCPControlDiffServ=acCPControlDiffServ, acCallerIDType=acCallerIDType, acCasTabeName=acCasTabeName, acCasTableIndex=acCasTableIndex, acCasTables=acCasTables, acCasTablesEntry=acCasTablesEntry, acCasTablesTable=acCasTablesTable, acCertificateExpiryNotifiaction=acCertificateExpiryNotifiaction, acChDefaultCallerIDType=acChDefaultCallerIDType, acChDefaultDJBufMinDelay=acChDefaultDJBufMinDelay, acChDefaultDJBufOptFactor=acChDefaultDJBufOptFactor, acChDefaultDTMFDetectionPoint=acChDefaultDTMFDetectionPoint, acChDefaultDTMFTransportType=acChDefaultDTMFTransportType, acChDefaultDTMFVolume=acChDefaultDTMFVolume, acChDefaultEchoCancler=acChDefaultEchoCancler, acChDefaultEchoCanclerHybridLoss=acChDefaultEchoCanclerHybridLoss, acChDefaultFaxModemBypassCoderType=acChDefaultFaxModemBypassCoderType, acChDefaultFaxModemBypassM=acChDefaultFaxModemBypassM, acChDefaultFaxModemRelayVolume=acChDefaultFaxModemRelayVolume, acChDefaultFaxRelayECMEnable=acChDefaultFaxRelayECMEnable, acChDefaultFaxRelayMaxRate=acChDefaultFaxRelayMaxRate, acChDefaultFaxRelayRedundancyDepth=acChDefaultFaxRelayRedundancyDepth, acChDefaultFaxTransportMode=acChDefaultFaxTransportMode, acChDefaultFlashHookPeriod=acChDefaultFlashHookPeriod, acChDefaultInputGain=acChDefaultInputGain, acChDefaultM=acChDefaultM, acChDefaultMFTransportType=acChDefaultMFTransportType, acChDefaultModemRelayMaxRate=acChDefaultModemRelayMaxRate, acChDefaultModemRelayRedundancyDepth=acChDefaultModemRelayRedundancyDepth, acChDefaultPacketizationPeriod=acChDefaultPacketizationPeriod, acChDefaultRTPRedundancyDepth=acChDefaultRTPRedundancyDepth, acChDefaultRtpIpPrecedence=acChDefaultRtpIpPrecedence, acChDefaultRtpIpTos=acChDefaultRtpIpTos, acChDefaultSilenceSuppression=acChDefaultSilenceSuppression, acChDefaultT38ProtectionMode=acChDefaultT38ProtectionMode, acChDefaultTestMode=acChDefaultTestMode, acChDefaultUseT38orFRF11=acChDefaultUseT38orFRF11, acChDefaultV21ModemTransportType=acChDefaultV21ModemTransportType, acChDefaultV22ModemTransportType=acChDefaultV22ModemTransportType, acChDefaultV23ModemTransportType=acChDefaultV23ModemTransportType, acChDefaultV32ModemTransportType=acChDefaultV32ModemTransportType, acChDefaultV34ModemTransportType=acChDefaultV34ModemTransportType, acChDefaultVoiceVolume=acChDefaultVoiceVolume, acConfigFiles=acConfigFiles, acContrlProtocolTransportType=acContrlProtocolTransportType, acCptFile=acCptFile, acDChannelStatus=acDChannelStatus, acDS3AISAlarm=acDS3AISAlarm, acDS3LOFAlarm=acDS3LOFAlarm, acDS3LOSAlarm=acDS3LOSAlarm, acDS3RAIAlarm=acDS3RAIAlarm, acDataInterfaceStatus=acDataInterfaceStatus, acDefaultChannelSettings=acDefaultChannelSettings, acDialPlanFileReplaced=acDialPlanFileReplaced, acDisableWEBConfig=acDisableWEBConfig, acDspType=acDspType, acDspVersionTemplate=acDspVersionTemplate, acEnableSyslog=acEnableSyslog, acEnhancedBITStatus=acEnhancedBITStatus, acEthernetGroupAlarm=acEthernetGroupAlarm, acFanTrayAlarm=acFanTrayAlarm, acFeatureKey=acFeatureKey, acFeatureKeyError=acFeatureKeyError, acFeatureKeyString=acFeatureKeyString, acFirstPortDuplexMode=acFirstPortDuplexMode, acFirstPortSpeed=acFirstPortSpeed, acFlashVersion=acFlashVersion, acFxoCoefficients=acFxoCoefficients, acFxs=acFxs, acFxsCoefficients=acFxsCoefficients, acGWSASEmergencyModeAlarm=acGWSASEmergencyModeAlarm, acGeneric=acGeneric, acH248LostConnectionWithCA=acH248LostConnectionWithCA, acHANetworkWatchdogStatusAlarm=acHANetworkWatchdogStatusAlarm, acHASystemConfigMismatchAlarm=acHASystemConfigMismatchAlarm, acHASystemFaultAlarm=acHASystemFaultAlarm, acHASystemSwitchOverAlarm=acHASystemSwitchOverAlarm, acHTTPDownloadResult=acHTTPDownloadResult, acHitlessUpdateStatus=acHitlessUpdateStatus, acHwFailureAlarm=acHwFailureAlarm, acIDSPolicyAlarm=acIDSPolicyAlarm, acIDSThresholdCrossNotification=acIDSThresholdCrossNotification, acIPv6ErrorAlarm=acIPv6ErrorAlarm, acIsdnDuplicateQ931BuffMode=acIsdnDuplicateQ931BuffMode, acIsdnGeneralCCBehavior=acIsdnGeneralCCBehavior, acIsdnIncomingCallsBehavior=acIsdnIncomingCallsBehavior, acIsdnNfasGroupNumber=acIsdnNfasGroupNumber, acIsdnNfasInterfaceId=acIsdnNfasInterfaceId, acIsdnOutgoingCallsBehavior=acIsdnOutgoingCallsBehavior, acIsdnQ931LayerResponseBehavior=acIsdnQ931LayerResponseBehavior, acKeepAlive=acKeepAlive, acLDAPLostConnection=acLDAPLostConnection, acLedStatusColor=acLedStatusColor, acLedStatusRate=acLedStatusRate, acMGControlProtocolType=acMGControlProtocolType, acMaxFreeChannels=acMaxFreeChannels, acMeanFreeChannels=acMeanFreeChannels, acMediaProcessOverloadAlarm=acMediaProcessOverloadAlarm, acMediaRealmBWThresholdAlarm=acMediaRealmBWThresholdAlarm, acNATTraversalAlarm=acNATTraversalAlarm, acNFASGroupAlarm=acNFASGroupAlarm, acNTPServerIPAddress=acNTPServerIPAddress, acNTPServerStatusAlarm=acNTPServerStatusAlarm, acNTPSettings=acNTPSettings, acNTPUpdateInterval=acNTPUpdateInterval, acNTPUtcOffset=acNTPUtcOffset, acNqmConnectivityAlarm=acNqmConnectivityAlarm, acNqmCqMosAlarm=acNqmCqMosAlarm, acNqmJitterAlarm=acNqmJitterAlarm, acNqmLqMosAlarm=acNqmLqMosAlarm, acNqmPacketLossAlarm=acNqmPacketLossAlarm, acNqmRttAlarm=acNqmRttAlarm, acOCSPServerStatusAlarm=acOCSPServerStatusAlarm, acOperationalState=acOperationalState, acOperationalStateChange=acOperationalStateChange, acOverrideDefaultIdleABCDPattern=acOverrideDefaultIdleABCDPattern, acOverrideDefaultIdlePCMPattern=acOverrideDefaultIdlePCMPattern, acPEMAlarm=acPEMAlarm, acPMStatus=acPMStatus, acPSTNCASTableNum=acPSTNCASTableNum, acPSTNIuaInterfaceId=acPSTNIuaInterfaceId, acPSTNParameters=acPSTNParameters, acPerformanceMonitoringThresholdCrossing=acPerformanceMonitoringThresholdCrossing, acPhysicalModCount=acPhysicalModCount, acPolarityReversalType=acPolarityReversalType, acPowerOverEthernetStatus=acPowerOverEthernetStatus, acPowerSupplyAlarm=acPowerSupplyAlarm, acProducts=acProducts, acProxyConnectionLost=acProxyConnectionLost, acQ931RELAYMODE=acQ931RELAYMODE, acRedundantBoardAlarm=acRedundantBoardAlarm, acRegistrations=acRegistrations, acSAMissingAlarm=acSAMissingAlarm, acSNMPManagerIP=acSNMPManagerIP, acSS7AliasPcStateChangeAlarm=acSS7AliasPcStateChangeAlarm, acSS7LinkBlockStateChangeAlarm=acSS7LinkBlockStateChangeAlarm, acSS7LinkCongestionStateChangeAlarm=acSS7LinkCongestionStateChangeAlarm, acSS7LinkId=acSS7LinkId, acSS7LinkInhibitStateChangeAlarm=acSS7LinkInhibitStateChangeAlarm, acSS7LinkSetStateChangeAlarm=acSS7LinkSetStateChangeAlarm, acSS7LinkStateChangeAlarm=acSS7LinkStateChangeAlarm, acSS7RedundancyAlarm=acSS7RedundancyAlarm, acSS7RouteSetStateChangeAlarm=acSS7RouteSetStateChangeAlarm, acSS7SNSetStateChangeAlarm=acSS7SNSetStateChangeAlarm, acSS7Settings=acSS7Settings, acSS7SettingsEntry=acSS7SettingsEntry, acSS7SettingsTable=acSS7SettingsTable, acSS7UalGroupStateChangeAlarm=acSS7UalGroupStateChangeAlarm, acSS7traceLevel=acSS7traceLevel, acSSHConnectionStatus=acSSHConnectionStatus, acSWUpgradeAlarm=acSWUpgradeAlarm, acSaveConfigToSystem=acSaveConfigToSystem, acSetDefaults=acSetDefaults, acSonetIfHwFailureAlarm=acSonetIfHwFailureAlarm, acSonetLineAISAlarm=acSonetLineAISAlarm, acSonetLineRDIAlarm=acSonetLineRDIAlarm, acSonetPathSTSAISAlarm=acSonetPathSTSAISAlarm, acSonetPathSTSLOPAlarm=acSonetPathSTSLOPAlarm, acSonetPathSTSRDIAlarm=acSonetPathSTSRDIAlarm, acSonetPathSignalLabelMismatchAlarm=acSonetPathSignalLabelMismatchAlarm, acSonetPathUnequippedAlarm=acSonetPathUnequippedAlarm, acSonetSectionLOFAlarm=acSonetSectionLOFAlarm, acSonetSectionLOSAlarm=acSonetSectionLOSAlarm, acSysUpTime=acSysUpTime, acSyslogServerIP=acSyslogServerIP, acTMInconsistentRemoteAndLocalPLLStatus=acTMInconsistentRemoteAndLocalPLLStatus, acTMReferenceChange=acTMReferenceChange, acTMReferenceStatus=acTMReferenceStatus, acThreeWayConferenceOutOfResources=acThreeWayConferenceOutOfResources, acTrap=acTrap, acTrunkAdminState=acTrunkAdminState, acTrunkCasTableIndex=acTrunkCasTableIndex, acTrunksAlarmFarEndLOF=acTrunksAlarmFarEndLOF, acTrunksAlarmNearEndLOF=acTrunksAlarmNearEndLOF, acTrunksAlarmNearEndLOS=acTrunksAlarmNearEndLOS, acTrunksAlarmRcvAIS=acTrunksAlarmRcvAIS, acTrunkslCount=acTrunkslCount, acUserInputAlarm=acUserInputAlarm, acV52InterfaceAlarm=acV52InterfaceAlarm, acV5InterfaceTrunkGroupId=acV5InterfaceTrunkGroupId, acV5LinkIdOld=acV5LinkIdOld, acVpFile=acVpFile, acWEBAccess=acWEBAccess, acWEBAccessAction=acWEBAccessAction, acWEBAccessActionResult=acWEBAccessActionResult, acWEBAccessEntry=acWEBAccessEntry, acWEBAccessIndex=acWEBAccessIndex, acWEBAccessRowStatus=acWEBAccessRowStatus, acWEBAccessTable=acWEBAccessTable, acWEBAccessUserCode=acWEBAccessUserCode, acWEBAccessUserName=acWEBAccessUserName, acWEBUserAccessDisabled=acWEBUserAccessDisabled, acWeakRedundancy=acWeakRedundancy, acWirelessCellularModemAlarm=acWirelessCellularModemAlarm, acgwAdminState=acgwAdminState, acgwAdminStateChange=acgwAdminStateChange, acgwAdminStateLockControl=acgwAdminStateLockControl, amsAASPackagesProfile=amsAASPackagesProfile, amsApsIpAddress=amsApsIpAddress, amsApsPort=amsApsPort, amsNumOfAnnouncementPorts=amsNumOfAnnouncementPorts, amsNumOfConferencePorts=amsNumOfConferencePorts, amsNumOfLawfulInterceptPorts=amsNumOfLawfulInterceptPorts, amsNumOfTestTrunkPorts=amsNumOfTestTrunkPorts, amsPrimaryLanguage=amsPrimaryLanguage, amsProfile=amsProfile, amsSecondaryLanguage=amsSecondaryLanguage, audioCodes=audioCodes, auxiliarySettings=auxiliarySettings, baseUDPPort=baseUDPPort, boardConfiguration=boardConfiguration, boardIPAddr=boardIPAddr, boardInformation=boardInformation)
mibBuilder.exportSymbols("AcBoard", boardName=boardName, boardSubNetAddr=boardSubNetAddr, boardTDMBusOutputPort=boardTDMBusOutputPort, boardTDMBusSettings=boardTDMBusSettings, boardType=boardType, bypassNIC=bypassNIC, cID=cID, cNAME=cNAME, cPUSpeed=cPUSpeed, callAgentDomainName=callAgentDomainName, callAgentIP=callAgentIP, callAgentPort=callAgentPort, channelConfiguration=channelConfiguration, channelHookState=channelHookState, channelStatus=channelStatus, channelStatusEntry=channelStatusEntry, channelStatusTable=channelStatusTable, channelTDMBusSettings=channelTDMBusSettings, channelTDMBusSettingsEntry=channelTDMBusSettingsEntry, channelTDMBusSettingsTable=channelTDMBusSettingsTable, channelsCount=channelsCount, clockMaster=clockMaster, coder=coder, dJBSettings=dJBSettings, dJBSettingsEntry=dJBSettingsEntry, dJBSettingsTable=dJBSettingsTable, dJBufMinDelay=dJBufMinDelay, dJBufOptFactor=dJBufOptFactor, dSPCount=dSPCount, dchConfig=dchConfig, dePopulatedChannelsNumber=dePopulatedChannelsNumber, defaultGatewayAddr=defaultGatewayAddr, disablePayloadScrambling=disablePayloadScrambling, disableSoftIPLoopback=disableSoftIPLoopback, eCE=eCE, eCHybridLoss=eCHybridLoss, eCLength=eCLength, enableDiagnostics=enableDiagnostics, endPointName=endPointName, enhancedFaxRelayRedundancyDepth=enhancedFaxRelayRedundancyDepth, externalClk=externalClk, fAXTransportType=fAXTransportType, fXSorFXO=fXSorFXO, faxModemBypassCoderType=faxModemBypassCoderType, faxModemBypassM=faxModemBypassM, faxModemRelayVolume=faxModemRelayVolume, faxModemSettings=faxModemSettings, faxModemSettingsEntry=faxModemSettingsEntry, faxModemSettingsTable=faxModemSettingsTable, faxRelayECMEnable=faxRelayECMEnable, faxRelayMaxRate=faxRelayMaxRate, faxRelayRedundancyDepth=faxRelayRedundancyDepth, framingMethod=framingMethod, gateWayName=gateWayName, hPFE=hPFE, iPPrecedence=iPPrecedence, iPTOS=iPTOS, idleABCD=idleABCD, idlePCMPattern=idlePCMPattern, iniFileVersion=iniFileVersion, inputGain=inputGain, jitterBufAccumDelay=jitterBufAccumDelay, jitterBufErrorCnt=jitterBufErrorCnt, jitterBufForcedPacketAddition=jitterBufForcedPacketAddition, jitterBufForcedPacketLost=jitterBufForcedPacketLost, jitterBufOverRunCnt=jitterBufOverRunCnt, jitterBufUnderRunCnt=jitterBufUnderRunCnt, lineBuildOutLoss=lineBuildOutLoss, lineBuildOutOverwrite=lineBuildOutOverwrite, lineBuildOutXPM0=lineBuildOutXPM0, lineBuildOutXPM1=lineBuildOutXPM1, lineBuildOutXPM2=lineBuildOutXPM2, lineCode=lineCode, localRTPPort=localRTPPort, m=m, mGCPCommunicationLayerTimeout=mGCPCommunicationLayerTimeout, mGCPSettings=mGCPSettings, masterSlaveMode=masterSlaveMode, megacoActiveCallAgentIp=megacoActiveCallAgentIp, megacoActiveCallAgentPort=megacoActiveCallAgentPort, megacoCallAgentId=megacoCallAgentId, megacoCallAgentIp=megacoCallAgentIp, megacoCallAgentIsUsed=megacoCallAgentIsUsed, megacoCallAgentPort=megacoCallAgentPort, megacoCallAgents=megacoCallAgents, megacoCallAgentsEntry=megacoCallAgentsEntry, megacoCallAgentsTable=megacoCallAgentsTable, megacoCheckLegalityOfMGC=megacoCheckLegalityOfMGC, megacoCurrentProfile=megacoCurrentProfile, megacoEndpointName=megacoEndpointName, megacoGatewayName=megacoGatewayName, megacoSettings=megacoSettings, megacoTrunkName=megacoTrunkName, modemRelayMaxRate=modemRelayMaxRate, modemRelayRedundancyDepth=modemRelayRedundancyDepth, multyPhy=multyPhy, networkSettings=networkSettings, notifications=notifications, pCMLawSelect=pCMLawSelect, pFE=pFE, pHYClkSource=pHYClkSource, packetsToDSPCnt=packetsToDSPCnt, pendingIdle=pendingIdle, protocolType=protocolType, rTCPMeanTxInterval=rTCPMeanTxInterval, rTPActive=rTPActive, rTPRTCPSettings=rTPRTCPSettings, rTPRTCPSettingsEntry=rTPRTCPSettingsEntry, rTPRTCPSettingsTable=rTPRTCPSettingsTable, rTPRedundancyDepth=rTPRedundancyDepth, redundantAgentIP=redundantAgentIP, redundantAgentPort=redundantAgentPort, redundantCallAgentDomainName=redundantCallAgentDomainName, remoteRTPAddr=remoteRTPAddr, remoteRTPPort=remoteRTPPort, remoteReset=remoteReset, remoteResetControl=remoteResetControl, remoteT38Addr=remoteT38Addr, remoteT38Port=remoteT38Port, reset=reset, rxDTMFPeriod=rxDTMFPeriod, rxFaxMode=rxFaxMode, rxRTPPayloadType=rxRTPPayloadType, rxSilencePeriod=rxSilencePeriod, sAALLink0VCI=sAALLink0VCI, sAALLink0VPI=sAALLink0VPI, sAALLink1VCI=sAALLink1VCI, sAALLink1VPI=sAALLink1VPI, sCE=sCE, sendIdleCASUponLinkFail=sendIdleCASUponLinkFail, serialNum=serialNum, slavePhyNum=slavePhyNum, slotNumber=slotNumber, snmpManagerIndex=snmpManagerIndex, snmpManagerIp=snmpManagerIp, snmpManagerIsUsed=snmpManagerIsUsed, snmpManagerTrapPort=snmpManagerTrapPort, snmpManagerTrapSendingEnable=snmpManagerTrapSendingEnable, snmpManagers=snmpManagers, snmpManagersEntry=snmpManagersEntry, snmpManagersTable=snmpManagersTable, softwareVersion=softwareVersion, supplementary=supplementary, supplementaryField=supplementaryField, t38FaxRelayProtectionMode=t38FaxRelayProtectionMode, tDMBusClockSource=tDMBusClockSource, tDMBusEnableFallBack=tDMBusEnableFallBack, tDMBusInputChannel=tDMBusInputChannel, tDMBusInputPort=tDMBusInputPort, tDMBusLocalReference=tDMBusLocalReference, tDMBusMasterSlaveSelection=tDMBusMasterSlaveSelection, tDMBusNetRefSpeed=tDMBusNetRefSpeed, tDMBusOutputChannel=tDMBusOutputChannel, tDMBusOutputDisable=tDMBusOutputDisable, tDMBusOutputPort=tDMBusOutputPort, tDMBusOutputStartingChannel=tDMBusOutputStartingChannel, tDMBusSpeed=tDMBusSpeed, tDMBusType=tDMBusType, tPNCPVCI=tPNCPVCI, tPNCPVPI=tPNCPVPI, terminationSide=terminationSide, testMode=testMode, traceLevel=traceLevel, transportSettings=transportSettings, transportSettingsEntry=transportSettingsEntry, transportSettingsTable=transportSettingsTable, trunkId=trunkId, trunkPackSoftwareDate=trunkPackSoftwareDate, trunkSettings=trunkSettings, trunkSettingsEntry=trunkSettingsEntry, trunkSettingsTable=trunkSettingsTable, txDTMFPeriod=txDTMFPeriod, txFaxMode=txFaxMode, txRTPPayloadType=txRTPPayloadType, txSilencePeriod=txSilencePeriod, uniDirectionalRTP=uniDirectionalRTP, useNIorPCI=useNIorPCI, useT38orFRF11=useT38orFRF11, utopiaBus16=utopiaBus16, utopiaSlave=utopiaSlave, v21ModemTransportType=v21ModemTransportType, v22ModemTransportType=v22ModemTransportType, v23ModemTransportType=v23ModemTransportType, v32ModemTransportType=v32ModemTransportType, v34ModemTransportType=v34ModemTransportType, vCMask=vCMask, vPMask=vPMask, vccProfile=vccProfile, voiceSettings=voiceSettings, voiceSettingsEntry=voiceSettingsEntry, voiceSettingsTable=voiceSettingsTable, voiceVolume=voiceVolume)
