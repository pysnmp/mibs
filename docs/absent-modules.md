...
**Declined**: 13 modules another collection ships that this distribution was
...
against ours left fourteen standard modules carried nowhere; one of them went
into pysmi's bundle in pysnmp/pysmi#325 and these thirteen did not.
...
| **Experimental, unused.** pysmi bundles an Experimental RFC module only while something imports symbols it alone defines; nothing here imports any of these. Its exclusion list names all five (pysmi#325) | AGGREGATE-MIB, TIME-AGGREGATE-MIB, RSERPOOL-MIB, SMF-MIB, IANA-SMF-MIB |
| **Never published.** No RFC publishes it, and it names no `ORGANIZATION` at all | SAVI-MIB |
